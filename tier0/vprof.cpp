  ...
  if (GetRoot() != pNode) {
    if (g_pStartNode && pNode->GetTotalCalls() > 0 &&
      ...
      iter = g_TimeSumsMap.find(
          pNode->GetName()); // intenionally using address of string rather than
                             // string compare (toml 01-27-03)
...
const char *CVProfile::GetCPUTraceFilename() {
  ...
  case kAllNodesInFrame_Recording:
    _snprintf(retBuf, sizeof(retBuf), "e:\\%.128s%.4d.pix2", m_CPUTraceFilename,
...
void CVProfile::DumpNodes(CVProfNode *pNode, int indent,
  ...
  if (fIsRoot || pNode == g_pStartNode) {
    if (bAverageAndCountOnly) {
      ...
      m_pOutputStream(_T("       Sum (ms)         Avg Time/Frame (ms)     Avg ")
                      _T("Time/Call (ms)\n"));
      m_pOutputStream(_T("[ func+child   func ]  [ func+child   func ]  [ ")
                      _T("func+child   func ]  Count   Peak\n"));
      m_pOutputStream(_T("  ---------- ------      ---------- ------      ")
                      _T("---------- ------   ------ ------\n"));
    }
  }
    ...
    if (bAverageAndCountOnly) {
      m_pOutputStream(_T("  %10.3f %6.2f      %6d  %s%s\n"),
                      (pNode->GetTotalCalls() > 0)
                          ...
                          : 0,
                      (pNode->GetTotalCalls() > 0)
                          ? dNodeTime / (double)NumFramesSampled()
                          : 0,
                      pNode->GetTotalCalls(), indentText, pNode->GetName());
    } else {
      m_pOutputStream(
          _T("  %10.3f %6.2f      %10.3f %6.2f      %10.3f %6.2f   %6d %6.2f  ")
          _T("%s%s\n"),
          ...
          (pNode->GetTotalCalls() > 0)
              ...
              : 0,
          pNode->GetTotalCalls(), pNode->GetPeakTime(), indentText,
          pNode->GetName());
    }
  }
...
//-------------------------------------
static void DumpSorted(CVProfile::StreamOut_t outputStream,
                       const tchar *pszHeading, double totalTime,
  ...
  outputStream(_T("%s\n"), pszHeading);
  outputStream(_T("  Scope                                                    ")
               _T("  Calls Calls/Frame  Time+Child    Pct        Time    Pct  ")
               _T(" Avg/Frame    Avg/Call Avg-NoChild        Peak\n"));
  outputStream(_T("  ---------------------------------------------------- ")
               _T("----------- ----------- ----------- ------ ----------- ")
               _T("------ ----------- ----------- ----------- -----------\n"));
...
void CVProfile::OutputReport(int type, const tchar *pszStartNode,
...
#if (_MSC_VER < 1300)
  m_pOutputStream(
      _T("  (note: this report exceeds the output capacity of MSVC debug ")
  ...
  if (NumFramesSampled() == 0 || GetTotalTimeSampled() == 0)
    ...
    if (type & VPRT_LIST_BY_TIME) {
      DumpSorted(m_pOutputStream,
                 _T("-- Profile scopes sorted by time (including children) --"),
                 GetTotalTimeSampled(), TimeCompare, maxLen);
      m_pOutputStream(_T("\n"));
    }
    if (type & VPRT_LIST_BY_TIME_LESS_CHILDREN) {
      DumpSorted(m_pOutputStream,
                 _T("-- Profile scopes sorted by time (without children) --"),
                 GetTotalTimeSampled(), TimeLessChildrenCompare, maxLen);
      m_pOutputStream(_T("\n"));
    }
    if (type & VPRT_LIST_BY_AVG_TIME) {
      DumpSorted(m_pOutputStream,
                 _T("-- Profile scopes sorted by average time (including ")
                 _T("children) --"),
    ...
    if (type & VPRT_LIST_BY_PEAK_TIME) {
      DumpSorted(m_pOutputStream, _T("-- Profile scopes sorted by peak --"),
...
CVProfile::CVProfile()
    : m_Root(_T("Root"), 0, NULL, VPROF_BUDGETGROUP_OTHER_UNACCOUNTED, 0),
      m_pCurNode(&m_Root), m_nFrames(0), m_enabled(0), m_pausedEnabledDepth(0),
      m_fAtRoot(true), m_pOutputStream(Msg) {
#ifdef VPROF_VTUNE_GROUP
...
static const TmU32 TELEMETRY_ARENA_SIZE =
    8 * 1024 * 1024; // How much memory we want Telemetry to use.
...
static void UpdateTelemetryThreadNames() {
  if (g_bThreadNameArrayChanged) {
    // Go through and add any new thread names we got in our thread safe list to
    // our thread names array.
    for (ThreadNameInfo_t *pThreadNameInfo = g_ThreadNamesList.Pop();
         pThreadNameInfo; pThreadNameInfo = g_ThreadNamesList.Pop()) {
    ...
    for (int i = 0; i < g_ThreadNameArrayCount; i++) {
      tmThreadName(g_tmContext, g_ThreadNameArray[i]->ThreadID,
                   g_ThreadNameArray[i]->szName);
    }
...
static bool TelemetryInitialize() {
  ...
  if (!g_TelemetryLoaded) {
    ...
    retVal = 0;
    if (retVal != TM_OK) {
      Warning("TelemetryInit() failed: tmStartup() returned %d, "
              "tmLoadTelemetry() returned %d.\n",
    ...
    // retVal = tmInitializeContext( &g_tmContext, g_pTmMemoryArena,
    // TELEMETRY_ARENA_SIZE );
    retVal = 0;
    if (retVal != TM_OK) {
  ...
  // If you've got \\perforce\symbols on your _NT_SYMBOL_PATH, tmOpen() can take
  // a massively long
  //	time in the symInitialize() routine. Since we don't really need that,
  //kill it here.
  putenv("_NT_SYMBOL_PATH=");
#endif

  const char *pServerAddress =
      g_Telemetry.ServerAddress[0] ? g_Telemetry.ServerAddress : "localhost";
  TmConnectionType tmType =
      !V_tier0_stricmp(pServerAddress, "FILE") ? TMCT_FILE : TMCT_TCP;

  Msg("TELEMETRY: Calling tmOpen( %s )...\n", pServerAddress);

  char szBuildInfo[2048];
  _snprintf(szBuildInfo, ARRAYSIZE(szBuildInfo), "%s: %s", __DATE__ __TIME__,
  ...
  // retVal = tmOpen( g_tmContext, pGameName, szBuildInfo, pServerAddress,
  // tmType, 	TELEMETRY_DEFAULT_PORT, TmOpenFlags, 1000 );
  retVal = 0;
  if (retVal != TM_OK) {
...
static void TelemetryShutdown(bool InDtor = false) {
  if (g_tmContext) {
    ...
    // Discontinue new usage of the context before shutting it down
    // (multithreading).
...
static void TelemetryPlots() {
  if (g_Telemetry.playbacktick) {
    tmPlotU32(TELEMETRY_LEVEL1, TMPT_INTEGER, 0, g_Telemetry.playbacktick,
              "game/PlaybackTick");
...
PLATFORM_INTERFACE void TelemetryTick() {
  ...
  if (g_Telemetry.Level && g_Telemetry.DemoTickEnd &&
      ((uint32)g_Telemetry.playbacktick > g_Telemetry.DemoTickEnd)) {
    TelemetrySetLevel(0);
  ...
  if (g_fTelemetryLevelChanged) {
    ...
    //  	TM_SET_TIMELINE_SECTION_NAME( g_tmContext, "Level:0x%x",
    //  g_Telemetry.Level );
