...
HWND CreateWindowEx(DWORD dwExStyle, LPCTSTR lpClassName, LPCTSTR lpWindowName,
                    ...
                    LPVOID lpParam) {
  return CreateWindow(lpClassName, lpWindowName, dwStyle, x, y, nWidth, nHeight,
...
int GetSystemMetrics(int nIndex) {
  ...
  // override for cases where we need to have a different backbuffer either for
  // memory reasons or for dealing with anamorphic modes.
  if (!videoMode.fIsWideScreen && videoMode.dwDisplayWidth == 640 &&
      videoMode.dwDisplayHeight == 576) {
    // PAL normal
    nFrameBufferWidth = 640;
    nFrameBufferHeight = 480;
  } else if (videoMode.fIsWideScreen && videoMode.dwDisplayWidth == 640 &&
             videoMode.dwDisplayHeight == 576) {
    // PAL widescreen
    nFrameBufferWidth = 848;
    nFrameBufferHeight = 480;
  } else if (videoMode.fIsWideScreen && videoMode.dwDisplayWidth == 640 &&
             videoMode.dwDisplayHeight == 480) {
    // anamorphic
    nFrameBufferWidth = 848;
    nFrameBufferHeight = 480;
  } else if (videoMode.fIsWideScreen && videoMode.dwDisplayWidth == 1024 &&
             videoMode.dwDisplayHeight == 768) {
    // anamorphic
    nFrameBufferWidth = 1280;
    nFrameBufferHeight = 720;
  } else if (videoMode.dwDisplayWidth == 1280 &&
             videoMode.dwDisplayHeight == 760) {
    nFrameBufferWidth = 1280;
    nFrameBufferHeight = 720;
  } else if (videoMode.dwDisplayWidth == 1280 &&
             videoMode.dwDisplayHeight == 768) {
    nFrameBufferWidth = 1280;
    nFrameBufferHeight = 720;
  } else if (!videoMode.fIsWideScreen && videoMode.dwDisplayWidth == 1280 &&
             videoMode.dwDisplayHeight == 1024) {
    ...
    nFrameBufferWidth = 1280;
    nFrameBufferHeight = 720;
  } else if (videoMode.dwDisplayWidth == 1360 &&
             videoMode.dwDisplayHeight == 768) {
    nFrameBufferWidth = 1280;
    nFrameBufferHeight = 720;
  } else if (videoMode.dwDisplayWidth == 1920 &&
...
BOOL ShowWindow(HWND hWnd, int nCmdShow) {
  ...
  if ((nCmdShow == SW_SHOWDEFAULT) || (nCmdShow == SW_SHOWNORMAL) ||
      (nCmdShow == SW_SHOW))
    g_focusWindow = hWnd;
