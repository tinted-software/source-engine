...
static int NotificationToWindowsMessage(DWORD id) {
  ...
  case XN_FRIENDS_FRIEND_ADDED:
    return WM_FRIENDS_FRIEND_ADDED;
  case XN_FRIENDS_FRIEND_REMOVED:
    return WM_FRIENDS_FRIEND_REMOVED;
  // deprecated in Jun08 XDK: case XN_CUSTOM_GAMEBANNERPRESSED:		return
  // WM_CUSTOM_GAMEBANNERPRESSED;
  ...
  while (XNotifyGetNext(g_hListenHandle, 0, &id, &parameter)) {
    ...
    case XN_SYS_STORAGEDEVICESCHANGED: {
      ...
      if (ret != ERROR_SUCCESS) {
        ...
        XBX_QueueEvent(XEV_LISTENER_NOTIFICATION,
                       NotificationToWindowsMessage(id), 0, 0);
      }
      break;
    }
    ...
    default:
      XBX_QueueEvent(XEV_LISTENER_NOTIFICATION,
                     NotificationToWindowsMessage(id), parameter, 0);
      break;
    }
  }
...
//-----------------------------------------------------------------------------
DWORD XBX_GetStorageDeviceId(void) { return g_iStorageDeviceId; }
...
//-----------------------------------------------------------------------------
void XBX_SetStorageDeviceId(DWORD id) { g_iStorageDeviceId = id; }
...
//-----------------------------------------------------------------------------
DWORD XBX_GetPrimaryUserId(void) { return g_iPrimaryUserId; }
...
//-----------------------------------------------------------------------------
void XBX_SetPrimaryUserId(DWORD idx) { g_iPrimaryUserId = idx; }

//-----------------------------------------------------------------------------
// Purpose: Returns the stored session ID for a cross-game invite
//-----------------------------------------------------------------------------
XNKID XBX_GetInviteSessionId(void) { return g_InviteSessionId; }
...
DWORD XBX_GetInvitedUserId(void) { return g_InvitedUserId; }

//-----------------------------------------------------------------------------
// Purpose: Set the Id of the user who received an invite
//-----------------------------------------------------------------------------
void XBX_SetInvitedUserId(DWORD nUserId) { g_InvitedUserId = nUserId; }

static CXboxLaunch g_XBoxLaunch;
CXboxLaunch *XboxLaunch() { return &g_XBoxLaunch; }
