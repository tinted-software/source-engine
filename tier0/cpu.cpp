...
static bool Check3DNowTechnology(void) {
    ...
    (defined(PLATFORM_BSD) && defined(COMPILER_CLANG))
  return false;
#else
  uint32 eax, unused;
...
static bool HTSupported(void) {
  ...
  const unsigned int HT_BIT =
      0x10000000; // EDX[28] - Bit 28 set indicates Hyper-Threading Technology
                  // is supported in hardware.
  const unsigned int FAMILY_ID =
      0x0f00; // EAX[11:8] - Bit 11 thru 8 contains family processor id
  const unsigned int EXT_FAMILY_ID =
      0x0f00000; // EAX[23:20] - Bit 23 thru 20 contains extended family
                 // processor id
  ...
  if (((reg_eax & FAMILY_ID) == PENTIUM4_ID) || (reg_eax & EXT_FAMILY_ID))
    if (vendor_id[0] == 0x756E6547 && vendor_id[1] == 0x49656E69 &&
  ...
  if (fpCpuInfo) {
    ...
    while (fgets(buf, ARRAYSIZE(buf), fpCpuInfo)) {
      if (char *value = strchr(buf, ':')) {
        ...
        if (nProcId != -1 &&
            nCoreId != -1) // as soon as we have a complete id, process it
        {
