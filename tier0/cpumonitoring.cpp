...
//============
//
// A non-trivial number of Valve customers hit performance problems because
// their CPUs overheat and are thermally throttled. While thermal throttling is
// better than melting it is still a hardware flaw and it leads to a bad user
// experience. In some cases the CPU frequency drops (constantly or
// occasionally) by 50-75%, leading to equal or greater framerate drops.
//
// This is equivalent to a car that goes into limp-home mode to let it continue
// running after the radiator fails -- it's better than destroying the engine,
// but clearly it needs to be fixed.
//
// When CPU monitoring is enabled a bunch of background threads are created that
// wake up at the set frequency, spin in a loop to measure the actual usable CPU
// frequency, then sleep again. A delay loop is used to measure the frequency
// because this is portable (it works for Intel and AMD and handles both
// frequency throttling and duty-cycle reductions) and it doesn't require
// administrator privileges. This technique has been used in VTrace for a while.
//
// This code doesn't use normal worker threads because of the special purpose
// nature of this work. The threads are started on demand and are never
// terminated, in order to simplify the code.
//
//===============================================================================
...
// This is the delay between measurements when measurements are 'disabled'. If
// it is zero then the measurements are truly disabled.
...
static double GetTime() {
  ...
  // Subtracting off the base time gives us a zero point at application start up
  // and gives us more precision.
  return (value.QuadPart - s_QPCbase.QuadPart) /
         double(s_QPCfrequency.QuadPart);
}

static float GetFrequency() {
  double start = GetTime();
  // This should cause a delay of 500,000 cycles (50 * spinCount) which should
  // be a fraction of a millisecond on any reasonable processor, thus ensuring
  // that the sampling interrupt will not be hit too frequently.
  SpinALot(10000);
...
// This semaphore is used to release all of the measurement threads
// simultaneously.
static HANDLE g_releaseSemaphore;
// This semaphore is used to wait for all of the measurement threads to
// complete.
...
static DWORD WINAPI MeasureThread(LPVOID vThreadNum) {
  ...
  for (;;) {
    ...
    // Seven seems like a good number of times to measure the frequency -- it
    // makes it likely that a couple of the tests will not hit any interrupts.
...
static DWORD WINAPI HeartbeatThread(LPVOID) {
  ...
  for (;;) {
    unsigned delay;
    {
      // Read and write all the state that is shared with the main thread while
      // holding the lock.
      AUTO_LOCK(s_lock);
    ...
    if (delay) {
      ...
      CallNtPowerInformation(ProcessorInformation, NULL, 0, &processorInfo,
                             sizeof(processorInfo[0]) * g_numCPUs);
      ...
      // This will wake up all of the worker threads. It is possible that some
      // of the threads will take a long time to wake up in which case the same
      // thread might wake up multiple times but this should be harmless.
      ...
      {
        // Read and write all the state that is shared with the main thread
        ...
        if (s_results.m_lowestPercentage == 0 ||
            s_results.m_percentage < s_results.m_lowestPercentage)
...
GetCPUFrequencyResults(bool fGetDisabledResults) {
  AUTO_LOCK(s_lock);
...
PLATFORM_INTERFACE void SetCPUMonitoringInterval(unsigned nDelayMilliseconds) {
  ...
  if (!s_initialized) {
    ...
    // Create n threads, affinitize them, and set them to high priority. This
    // will (mostly) ensure that they will run promptly on a specific CPU.
    for (DWORD i = 0; i < g_numCPUs; ++i) {
      HANDLE thread =
          CreateThread(NULL, 0x10000, MeasureThread, (void *)i, 0, NULL);
      SetThreadAffinityMask(thread, 1u << i);
      SetThreadPriority(thread, THREAD_PRIORITY_HIGHEST);
    }

    // Create the thread which tells the measurement threads to wake up
    // periodically
  ...
  s_nDelayMilliseconds =
      nDelayMilliseconds ? nDelayMilliseconds : kDelayMillisecondsWhenDisabled;
