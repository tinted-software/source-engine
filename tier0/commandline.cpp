...
public:
  ...
  // A bool return of whether param exists, useful for just checking if param
  // that is just a flag is set
  ...
  virtual const char *ParmValue(const char *psz,
                                const char *pDefaultVal = NULL) const OVERRIDE;
  ...
  virtual const char *
  ParmValueByIndex(int nIndex, const char *pDefaultVal = 0) const OVERRIDE;
...
void CCommandLine::CreateCmdLine(const char *commandline) {
  ...
  while (*pSrc) {
    ...
    if (*pSrc == '@') {
      if (pSrc == commandline || (!bInQuotes && isspace(pSrc[-1])) ||
...
const char *CCommandLine::ParmValueByIndex(int nIndex,
                                           const char *pDefaultVal) const {
  if ((nIndex == 0) || (nIndex == m_nParmCount - 1))
    return pDefaultVal;
