...
static X86HardwareBreakpointState_t s_BreakpointState = {
...
static void X86ApplyBreakpointsToThread(DWORD dwThreadId) {
  ...
  HANDLE hThread =
      OpenThread(THREAD_SUSPEND_RESUME | THREAD_SET_CONTEXT, FALSE, dwThreadId);
...
static DWORD STDCALL ThreadProcX86SetDataBreakpoints(LPVOID pvParam) {
  ...
  // This function races against creation and destruction of new threads. Try to
  ...
  if (hSnap != INVALID_HANDLE_VALUE) {
    ...
    // Thread32First/Thread32Next may adjust dwSize to be smaller. It's weird.
    // Read the doc.
    const DWORD dwMinSize =
        (char *)(&threadEntry.th32OwnerProcessID + 1) - (char *)&threadEntry;
    ...
    while (bContinue) {
      if (threadEntry.dwSize >= dwMinSize) {
        if (threadEntry.th32OwnerProcessID == dwProcId &&
            threadEntry.th32ThreadID != dwThisThreadId) {
...
void Plat_SetHardwareDataBreakpoint(const void *pAddress, int nWatchBytes,
                                    bool bBreakOnRead) {
  Assert(pAddress);
  Assert(nWatchBytes == 0 || nWatchBytes == 1 || nWatchBytes == 2 ||
         nWatchBytes == 4 || nWatchBytes == 8);
  ...
  if (nWatchBytes == 0) {
    for (int i = 0; i < 4; ++i) {
      if (pAddress == s_BreakpointState.pAddress[i]) {
        for (; i < 3; ++i) {
          ...
          s_BreakpointState.nWatchBytes[i] =
              s_BreakpointState.nWatchBytes[i + 1];
          s_BreakpointState.bBreakOnRead[i] =
              s_BreakpointState.bBreakOnRead[i + 1];
        }
    ...
    // Replace first null entry or first existing entry at this address, or bump
    // all entries down
    for (int i = 0; i < 4; ++i) {
      if (s_BreakpointState.pAddress[i] &&
          s_BreakpointState.pAddress[i] != pAddress && i < 3)
        continue;
      ...
      if (s_BreakpointState.pAddress[i] &&
        ...
        for (int j = 0; j < 3; ++j) {
          ...
          s_BreakpointState.nWatchBytes[j] =
              s_BreakpointState.nWatchBytes[j + 1];
          s_BreakpointState.bBreakOnRead[j] =
              s_BreakpointState.bBreakOnRead[j + 1];
        }
      }
      s_BreakpointState.pAddress[i] = pAddress;
  ...
  HANDLE hWorkThread =
      CreateThread(NULL, NULL, &ThreadProcX86SetDataBreakpoints, NULL, 0, NULL);
...
void Plat_ApplyHardwareDataBreakpointsToNewThread(unsigned long dwThreadID) {
  ...
  if (dwThreadID != GetCurrentThreadId()) {
    ...
    HANDLE hWorkThread = CreateThread(
        NULL, NULL, &ThreadProcX86SetDataBreakpoints, &dwThreadID, 0, NULL);
    if (hWorkThread != INVALID_HANDLE_VALUE) {
      WaitForSingleObject(hWorkThread, INFINITE);
      CloseHandle(hWorkThread);
    }
