...
VCRMode_t g_OldVCRMode = VCR_Invalid; // Stored temporarily between
...
int g_LastEventThread; // The thread index of the thread that g_LastReadEvent is
                       // intended for.

int g_bVCREnabled = 0;

// ------------------------------------------------------------------------------------------
// // These wrappers exist because for some reason thread-blocking functions
// nuke the last function on the call stack, so it's very hard to debug without
// these wrappers.
// ------------------------------------------------------------------------------------------
// //
...
inline unsigned long Wrap_WaitForMultipleObjects(uint32 nHandles,
                                                 const void **pHandles,
                                                 int bWaitAll, uint32 timeout) {
  return WaitForMultipleObjects(nHandles, (const HANDLE *)pHandles, bWaitAll,
                                timeout);
}
...
public:
  ...
  HANDLE m_hWaitEvent; // Used to get the signal that there is an event for this
                       // thread.
  bool m_bEnabled; // By default, this is true, but it can be set to false to
                   // temporarily disable a thread's VCR usage.
};
CVCRThreadInfo *g_pVCRThreads =
    NULL; // This gets allocated to MAX_VCR_THREADS size if we're doing any VCR
...
template <class T> static void VCR_ReadVal(T &val) {
...
static long VCR_Hook_RegOpenKeyEx(void *hKey, const char *lpSubKey,
  ...
  long ret;
  if (g_VCRMode == VCR_Playback) {
    VCR_ReadVal(ret); // (don't actually write anything to the person's registry
                      // when playing back).
  } else {
    ret =
...
static long VCR_Hook_RegSetValueEx(void *hKey, tchar const *lpValueName,
                                   unsigned long Reserved, unsigned long dwType,
                                   unsigned char const *lpData,
                                   unsigned long cbData) {
  // Preamble.
  if (!IsVCRModeEnabledForThisThread())
    return RegSetValueEx((HKEY)hKey, lpValueName, Reserved, dwType, lpData,
                         cbData);
  ...
  if (g_VCRMode == VCR_Playback) {
    ...
    ret = RegSetValueEx((HKEY)hKey, lpValueName, Reserved, dwType, lpData,
                        cbData);
...
static long VCR_Hook_RegQueryValueEx(void *hKey, tchar const *lpValueName,
  ...
  // Doesn't support this being null right now (although it would be trivial to
  // add support).
  assert(lpData);
...
static long VCR_Hook_RegCreateKeyEx(void *hKey, char const *lpSubKey,
  ...
  // Preamble.
  if (!IsVCRModeEnabledForThisThread())
    return RegCreateKeyEx((HKEY)hKey, lpSubKey, Reserved, lpClass, dwOptions,
                          samDesired,
  ...
  if (g_VCRMode == VCR_Playback) {
    ...
    ret =
        RegCreateKeyEx((HKEY)hKey, lpSubKey, Reserved, lpClass, dwOptions,
                       samDesired, (LPSECURITY_ATTRIBUTES)lpSecurityAttributes,
                       (HKEY *)phkResult, lpdwDisposition);
...
int VCR_GenericPlaybackInternal(const char *pEventName, void *pOutData,
  ...
  VCR_ReadVal(dataLen);
  if (dataLen > maxLen) {
    VCR_Error("VCR_GenericPlayback( %s ) - generic data too long (greater than "
              "maxLen: %d)",
              pEventName, maxLen);
    return 0;
  } else if (bForceSameLen && dataLen != maxLen) {
    VCR_Error("VCR_GenericPlayback( %s ) - data size in file (%d) different "
              "than desired (%d)",
...
void VCR_GenericValueVerify(const tchar *pEventName, const void *pData,
                            int maxLen) {
  // Preamble.
  if (!IsVCRModeEnabledForThisThread())
    return;
...
void *VCR_CreateThread(void *lpThreadAttributes, unsigned long dwStackSize,
                       ...
                       unsigned long dwCreationFlags, uintp *lpThreadID) {
  ...
  g_pVCRThreads[iNewThread].m_hWaitEvent =
...
unsigned long VCR_WaitForMultipleObjects(uint32 nHandles, const void **pHandles,
                                         int bWaitAll, uint32 timeout) {
  // Preamble.
  if (!IsVCRModeEnabledForThisThread())
