...
// + lpszIniPath Note: There is no size check for lpszSymbolPath!
...
BOOL UninitSymInfo() { return SymCleanup(GetCurrentProcess()); }
...
BOOL GetFunctionInfoFromAddresses(ULONG fnAddress, ULONG stackAddress,
                                  LPTSTR lpszSymbol) {
  BOOL ret = FALSE;
  DWORD dwDisp = 0;
  ...
  if (SymGetSymFromAddr(GetCurrentProcess(), (ULONG)fnAddress, &dwDisp, pSym)) {
    ...
    // Let's go through the stack, and modify the function prototype, and insert
    // the actual parameter values from the stack
    if (_tcsstr(lpszUnDSymbol, _T("(void)")) == NULL &&
        _tcsstr(lpszUnDSymbol, _T("()")) == NULL) {
      ULONG index = 0;
      for (;; index++) {
        ...
        *lpszParamSep = _T('\0');
        ...
        _stprintf(lpszSymbol + _tcslen(lpszSymbol), _T("=0x%08X,"),
                  *((ULONG *)(stackAddress) + 2 + index));
...
void SrcLinkTrace(LPCTSTR lpszMessage, LPCTSTR lpszFileName,
                  ULONG nLineNumber) {
  OutputDebugStringFormat(_T("%s(%d) : %s"), lpszFileName, nLineNumber,
                          lpszMessage);
}
...
void StackTrace(HANDLE hThread, LPCTSTR lpszMessage) {
  ...
  for (ULONG index = 0;; index++) {
    ...
    GetFunctionInfoFromAddresses(callStack.AddrPC.Offset,
                                 callStack.AddrFrame.Offset, symInfo);
...
void FunctionParameterInfo() {
  ...
  if (bResult && callStack.AddrFrame.Offset != 0) {
    GetFunctionInfoFromAddresses(callStack.AddrPC.Offset,
                                 callStack.AddrFrame.Offset, lpszFnInfo);
