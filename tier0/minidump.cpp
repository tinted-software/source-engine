...
//minidump to write. 			ptchMinidumpFileNameBuffer	- if not-NULL points to a
//writable tchar buffer 										  of length at least _MAX_PATH to contain the name 										  of the
//written minidump file on return.
//-----------------------------------------------------------------------------
bool WriteMiniDumpUsingExceptionInfo(
  ...
  if (pfnMiniDumpWrite) {
    ...
    // If they didn't set a dump prefix, then set one for them using the module
    // name
    ...
    if (hFile) {
      ...
      if (g_rgchMinidumpComment[0] != '\0') {
        MINIDUMP_USER_STREAM *pCommentStream =
            &UserStreams[StreamInformationHeader.UserStreamCount++];
    ...
    if (!bMinidumpResult) {
      ...
      _sntprintf(rgchFailedFileName, sizeof(rgchFailedFileName) / sizeof(tchar),
...
void InternalWriteMiniDumpUsingExceptionInfo(
  ...
  // If this is is a real crash (not an assert or one we purposefully
  // triggered), then try to write a full dump only do this on our GC (currently
  // GC is 64-bit, so we can use a #define rather than some run-time switch
#ifdef _WIN64
  if (uStructuredExceptionCode != EXCEPTION_BREAKPOINT) {
    if (WriteMiniDumpUsingExceptionInfo(uStructuredExceptionCode,
                                        pExceptionInfo, MiniDumpWithFullMemory,
                                        pszFilenameSuffix)) {
      return;
    }
  }
  ...
  if (!WriteMiniDumpUsingExceptionInfo(uStructuredExceptionCode, pExceptionInfo,
    ...
    WriteMiniDumpUsingExceptionInfo(uStructuredExceptionCode, pExceptionInfo,
                                    (MINIDUMP_TYPE)iType, pszFilenameSuffix);
  }
}
...
void SetMinidumpFilenamePrefix(const char *pszPrefix) {
  ...
  mbstowcs(g_rgchMinidumpFilenamePrefix, pszPrefix,
           sizeof(g_rgchMinidumpFilenamePrefix) /
                   sizeof(g_rgchMinidumpFilenamePrefix[0]) -
               1);
#else
...
void WriteMiniDump(const char *pszFilenameSuffix) {
  ...
  __except (g_pfnWriteMiniDump(EXCEPTION_BREAKPOINT, GetExceptionInformation(),
                               pszFilenameSuffix),
...
//-----------------------------------------------------------------------------
// Purpose: Catches and writes out any exception throw by the specified
// function. Input:	pfn -			Function to call within
// protective exception block
//			pv -			Void pointer to pass that
//function
//-----------------------------------------------------------------------------
...
enum ECatchAndWriteFunctionType {
  ...
  k_eSCatchAndWriteFunctionTypeWMain =
      1, // typedef void (*FnWMain)( int , tchar *[] );
  k_eSCatchAndWriteFunctionTypeWMainIntReg =
      2, // typedef int (*FnWMainIntRet)( int , tchar *[] );
  k_eSCatchAndWriteFunctionTypeVoidPtr =
      3, // typedef void (*FnVoidPtrFn)( void * );
};
...
struct CatchAndWriteContext_t {
  ...
  void Set(ECatchAndWriteFunctionType eType,
           ECatchAndWriteMinidumpAction eAction, void *pfn, int *pargc,
...
// Input:	pfn -			Function to call within protective
// exception block
//			pv -			Void pointer to pass that
//function 			eAction -		Specifies what to do if it catches an exception
//-----------------------------------------------------------------------------
#if defined(_PS3)
...
// Win32 code gets to use a special handler
  ...
  __try {
    return ctx.Invoke();
  } __except (g_pfnWriteMiniDump(GetExceptionCode(), GetExceptionInformation(),
                                 GetExceptionCodeName(GetExceptionCode())),
              EXCEPTION_EXECUTE_HANDLER) {
    TerminateProcess(GetCurrentProcess(),
                     EXIT_FAILURE); // die, die RIGHT NOW! (don't call exit() so
                                    // destructors will not get run)
  }
  ...
  //	CCrashHandlerScope scope( handler, g_DumpHelper.GetProduct(),
  //g_DumpHelper.GetVersion(), g_DumpHelper.GetBuildID(), false ); 	if ( handler
...
//function 			eAction -		Specifies what to do if it catches an exception
//-----------------------------------------------------------------------------
void CatchAndWriteMiniDumpEx(FnWMain pfn, int argc, tchar *argv[],
                             ECatchAndWriteMinidumpAction eAction) {
  CatchAndWriteContext_t ctx;
  ctx.Set(k_eSCatchAndWriteFunctionTypeWMain, eAction, (void *)pfn, &argc,
          &argv, NULL);
...
//function 			eAction -		Specifies what to do if it catches an exception
//-----------------------------------------------------------------------------
int CatchAndWriteMiniDumpExReturnsInt(FnWMainIntRet pfn, int argc,
                                      tchar *argv[],
                                      ECatchAndWriteMinidumpAction eAction) {
  CatchAndWriteContext_t ctx;
  ctx.Set(k_eSCatchAndWriteFunctionTypeWMainIntReg, eAction, (void *)pfn, &argc,
          &argv, NULL);
...
//function 			eAction -		Specifies what to do if it catches an exception
//-----------------------------------------------------------------------------
void CatchAndWriteMiniDumpExForVoidPtrFn(FnVoidPtrFn pfn, void *pv,
                                         ECatchAndWriteMinidumpAction eAction) {
  CatchAndWriteContext_t ctx;
  ctx.Set(k_eSCatchAndWriteFunctionTypeVoidPtr, eAction, (void *)pfn, NULL,
          NULL, &pv);
  CatchAndWriteMiniDump_Impl(ctx);
}
...
// exception block
//			pv -			Void pointer to pass that
//function 			bExitQuietly -	If true (for client) just exit after mindump,
//with no visible error for user 							If false, re-throws.
//-----------------------------------------------------------------------------
void CatchAndWriteMiniDumpForVoidPtrFn(FnVoidPtrFn pfn, void *pv,
                                       bool bExitQuietly) {
  return CatchAndWriteMiniDumpExForVoidPtrFn(
      pfn, pv,
      bExitQuietly ? k_ECatchAndWriteMiniDumpAbort
                   : k_ECatchAndWriteMiniDumpReThrow);
}
...
silently swallowing *some* exceptions. As a concrete example, consider this
code:
...
It's in a WindowProc handling a paint message so it will generally be called
from kernel mode. Therefore the crash in the middle of it is, by default,
'handled' for us. The "delete p;" and EndPaint() never happen. This means that
the process is left in an indeterminate state. It also means that our error
reporting never sees the exception. It is effectively as though there is a
__try/__except handler at the kernel boundary and any crashes cause the stack to
be unwound (without destructors being run) to the kernel boundary where
execution continues.
...
For more scary details see this article. It actually suggests using the
compatibility manifest, but that does not appear to work.
...
// Array of 32 round robin 128 byte strings set via
...
void MinidumpUserStreamInfoSetHeader(const char *pFormat, ...) {
  ...
  _vsnprintf(g_UserStreamInfoHeader, ARRAYSIZE(g_UserStreamInfoHeader), pFormat,
             marker);
...
const char *MinidumpUserStreamInfoGet(int Index) {
  if ((Index < 0) || (Index >= (ARRAYSIZE(g_UserStreamInfo) +
                                1))) //+1 because we map 0 to the header
    return NULL;
  ...
  Index =
      ((Index + (ARRAYSIZE(g_UserStreamInfo) -
                 1)) + // subtract 1 in a way that circularly wraps. Since 0
                       // maps to the header, the comment indices are 1 based
