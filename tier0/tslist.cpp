...
class CQueueOps : public CTestOps {
  ...
  bool Validate() { return g_TestQueue.ValidateQueue(); }
...
void ClearBuckets() { memset(g_pTestBuckets, 0, sizeof(int) * NUM_TEST); }
...
void MTPushPopPopInterleaved(bool bDistribute) {
  ...
  for (int i = 0; i < NUM_THREADS; i++) {
    ThreadHandle_t hThread =
        CreateSimpleThread(&PushPopInterleavedTestThreadFunc, NULL);
    g_ThreadHandles.push_back(hThread);
    if (bDistribute) {
      int32 mask = 1 << (i % NUM_PROCESSORS);
      ThreadSetAffinity(hThread, mask);
