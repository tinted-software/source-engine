...
int ETWGetRenderFrameNumber() { return s_nRenderFrameCount[0]; }

// Insert a render frame marker using the Valve-FrameRate provider.
// Automatically count the frame number and frame time.
void ETWRenderFrameMark(bool bIsServerProcess) {
  ...
  if (s_nRenderFrameCount[bIsServerProcess]) {
    flElapsedFrameTime =
        QPCToMS(nCurrentFrameTime - s_lastFrameTime[bIsServerProcess]);
  }

  if (bIsServerProcess) {
    EventWriteServerRenderFrameMark(s_nRenderFrameCount[bIsServerProcess],
                                    flElapsedFrameTime);
  } else {
    EventWriteRenderFrameMark(s_nRenderFrameCount[bIsServerProcess],
                              flElapsedFrameTime);
  }
...
void ETWSimFrameMark(bool bIsServerProcess) {
  ...
  if (bIsServerProcess) {
    EventWriteServerSimFrameMark(s_nFrameCount[bIsServerProcess],
                                 flElapsedFrameTime);
  } else {
