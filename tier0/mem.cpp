...
void MemAllocOOMError(size_t nSize) {
  if (!s_oomerror_called) {
    ...
    // A basic advice to try lowering texture settings is just most-likely to
    // help users who are exhausting address space, but not necessarily the
    // cause.  Ideally the engine wouldn't let you get here because of too-high
    // settings.
    Error("Out of memory or address space.  Texture quality setting may be too "
          "high.\n");
  }
}
