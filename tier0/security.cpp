...
// #define VALVE_DESKEY_ID "uW"	// Identity Password, Uniquely identifies HL2
// keys
...
bool Plat_VerifyHardwareKeyPrompt() {
  ...
  if (!DK2DriverInstalled()) {
    if (IDCANCEL ==
        MessageBox(NULL,
                   "No drivers detected for the hardware key, please install "
                   "them and re-run the application.\n",
  ...
  while (!Plat_VerifyHardwareKey()) {
    ...
    for (int i = 0; i < 2; i++) {
      ...
      // Sleep 2 / 3 of a second before trying again, in case the os recognizes
      // the key slightly after it's being inserted:
      Sleep(666);
    }
  }
