...
// SBH not enabled for LINUX right now. Unlike on Windows, we can't globally
// hook malloc. Well,
//  we can and did in override_init_hook(), but that unfortunately causes all
//  malloc functions
//	to get hooked - including the nVidia driver, etc. And these hooks appear
//to happen after 	nVidia has alloc'd some memory and it crashes when they try to
//free that.
// So we need things to work without this global hook - which means we rely on
// memdbgon.h / memdbgoff.h.
//  Unfortunately, that stuff always comes in source files after the headers are
//  included, and that means any alloc calls in the header files call the real
//  libc functions. It's a mess.
// I believe I've cleaned most of it up, and it appears to be working. However
// right now we are totally 	gated on other performance issues, and the SBH
// doesn't give us any win, so I've disabled it for now. Once those perf issues
// are worked out, it might make sense to do perf tests with SBH, libc, and
// tcmalloc.
//
...
private:
  ...
  static CX360SmallBlockPool
      *gm_AddressToPool[BYTES_X360_SBH / PAGESIZE_X360_SBH];
