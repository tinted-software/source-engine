...
void CDbgLogger::Init(const char *logfile) {
  ...
  if (file) {
    ...
    fprintf(file,
            ">>> Engine(arch:%s commit:" GIT_COMMIT_HASH ") started at %s\n",
...
void _ExitOnFatalAssert(const tchar *pFile, int line) {
  ...
  if (!Plat_IsInDebugSession()) {
    char rgchSuffix[512];
    _snprintf(rgchSuffix, sizeof(rgchSuffix), "fatalassert_%s_%d",
              SkipToFname(pFile), line);
...
static SpewRetval_t _SpewMessage(SpewType_t spewType, const char *pGroupName,
  ...
  assert(
      _tcslen(pMsgFormat) <
      sizeof(
          pTempBuffer)); // check that we won't artifically truncate the string

  /* Printf the file and line for warning + assert only... */
  int len = 0;
  if (spewType == SPEW_ASSERT) {
    len = _sntprintf(pTempBuffer, sizeof(pTempBuffer) - 1, _T("%s (%d) : "),
                     s_pFileName, s_Line);
  }
  ...
  /* Create the message.... */
  int val = _vsntprintf(&pTempBuffer[len], sizeof(pTempBuffer) - len - 1,
                        pMsgFormat, args);
  if (val == -1)
    return SPEW_ABORT;

  len += val;
  assert(len * sizeof(*pMsgFormat) <
         sizeof(pTempBuffer)); /* use normal assert here; to avoid recursion. */
  ...
  assert(len < sizeof(pTempBuffer) / sizeof(pTempBuffer[0]) -
                   1); /* use normal assert here; to avoid recursion. */
...
SpewRetval_t _DSpewMessage(const tchar *pGroupName, int level,
                           const tchar *pMsgFormat, ...) {
...
void SpewActivate(const tchar *pGroupName, int level) {
  ...
  if (!FindSpewGroup(pGroupName, &ind)) {
    ...
    ++s_GroupCount;
    if (s_pSpewGroups) {
      s_pSpewGroups = (SpewGroup_t *)PvRealloc(
      ...
      s_pSpewGroups =
          (SpewGroup_t *)PvAlloc(s_GroupCount * sizeof(SpewGroup_t));
    }
...
void COM_TimestampedLog(char const *fmt, ...) {
  ...
  if (!s_bChecked) {
    s_bShouldLog =
        (IsX360() || CommandLine()->CheckParm("-profile")) ? true : false;
    s_bShouldLogToETW =
...
//-----------------------------------------------------------------------------
void CallAssertFailedNotifyFunc(const char *pchFile, int nLine,
