...
// If you want to try using rdtsc for Plat_FloatTime(), enable
// USE_RDTSC_FOR_FLOATTIME:
//
// Make sure you know what you're doing. This was disabled due to the long
// startup time, and
//  in our testing, even though constant_tsc was set, we couldn't rely on the
//  max frequency result returned from CalculateCPUFreq() (ie
...
size_t ApproximateProcessMemoryUsage(void) {
  ...
  */
  ...
  if (pFile) {
    size_t nSize, nResident, nShare, nText, nLib_Unused, nDataPlusStack,
        nDt_Unused;
    if (fscanf(pFile, "%zu %zu %zu %zu %zu %zu %zu", &nSize, &nResident,
               &nShare, &nText, &nLib_Unused, &nDataPlusStack,
               &nDt_Unused) >= 2) {
      nRet = 4096 * nResident;
    }
    fclose(pFile);
...
static void InitTimeSystem(void) {
  ...
  // now, see if we can use rdtsc instead. If this is one of the chips with a
  // separate constant clock for rdtsc, we can
  ...
  if (pCpuInfo) {
    ...
    fclose(pCpuInfo);
    if (!bAnyBadCores) {
      // this system appears to have the proper cpu setup to use rdtsc from
      // reliable timing. Let's either read the cpu frequency from an
      // environment variable, or time it ourselves
      char const *pEnv = getenv("RDTSC_FREQUENCY");
      if (pEnv) {
        // the environment variable is allowed to hold either a benchmark
        // result, or a string such as "disable"
        ...
        std::make_heap(flRDTSCToMicroSeconds,
                       flRDTSCToMicroSeconds +
                           ARRAYSIZE(flRDTSCToMicroSeconds) - 1);
        std::sort_heap(flRDTSCToMicroSeconds,
                       flRDTSCToMicroSeconds +
                           ARRAYSIZE(flRDTSCToMicroSeconds) - 1);
        s_flRDTSCToMicroSeconds =
            flRDTSCToMicroSeconds[ARRAYSIZE(flRDTSCToMicroSeconds) / 2];
        s_flRDTSCScale = 1.0 / (1000.0 * 1000.0 * s_flRDTSCToMicroSeconds);
        printf("Finished RDTSC test. To prevent the startup delay from this "
               "benchmark, set the environment variable RDTSC_FREQUENCY to %f "
               "on this system."
               " This value is dependent upon the CPU clock speed and "
               "architecture and should be determined separately for each "
               "server. The use of this mechanism"
...
double Plat_FloatTime() {
  ...
  if (!start_time) {
    ...
    conversion =
        1e-9 * (double)sTimebaseInfo.numer / (double)sTimebaseInfo.denom;
  }
  ...
  if (s_bUseRDTSC) {
    ...
    return VCRHook_Sys_FloatTime((tp.tv_sec - s_nSecBase) +
...
time_t Plat_timegm(struct tm *timeptr) { return timegm(timeptr); }
...
bool vtune(bool resume) { return 0; }
...
PLATFORM_INTERFACE void Plat_Free(void *ptr) { g_pMemAlloc->Free(ptr); }
...
bool Plat_IsInDebugSession() {
  // For linux:
  // http://stackoverflow.com/questions/3596781/detect-if-gdb-is-running Don't
  // use "if (ptrace(PTRACE_TRACEME, 0, NULL, 0) == -1)" as it means debuggers
  // can't attach. 	Other solutions they mention involve forking. Ugh.
  //
...
void Plat_DebugString(const char *psz) { printf("%s", psz); }
...
PLATFORM_INTERFACE const tchar *Plat_GetCommandLine() {
  ...
  if (!g_CmdLine[0]) {
    ...
    if (fp) {
      ...
      nCharRead =
          fread(g_CmdLine, sizeof(g_CmdLine[0]), ARRAYSIZE(g_CmdLine) - 1, fp);
      if (feof(fp) &&
          !ferror(fp)) // Should have read the whole command line without error
      {
...
PLATFORM_INTERFACE bool
GetMemoryInformation(MemoryInformation *pOutMemoryInfo) {
#if defined(LINUX) || defined(OSX) || defined(PLATFORM_BSD)
  return false;
#else
#error                                                                         \
    "Need to fill out GetMemoryInformation or at least return false for this platform"
#endif
}
...
PLATFORM_INTERFACE void Plat_ExitProcess(int nCode) { _exit(nCode); }
...
PLATFORM_INTERFACE int Plat_GetWatchdogTime(void) { return s_WatchdogTime; }

PLATFORM_INTERFACE void
Plat_SetWatchdogHandlerFunction(Plat_WatchDogHandlerFunction_t function) {
  s_pWatchDogHandlerFunction = function;
}

#ifndef NO_HOOK_MALLOC

// memory logging this functionality is portable code, except for the way in
// which it hooks malloc/free. glibc contains the ability for the app to install
// hooks into malloc/free.
...
struct CLinuxMallocContext {
  ...
  CLinuxMallocContext(void) { memset(this, 0, sizeof(*this)); }
};
...
struct RememberedAlloc_t {
  RememberedAlloc_t *m_pNext,
      *m_pPrev; // all addresses that hash to the same value are linked

  CLinuxMallocContext *m_pAllocContext;
  ptrint_t
      m_nAddress; // the address of the memory that came from malloc/realloc
  size_t m_nSize;
  ...
  void AdjustSize(size_t nsize) {
    ...
    m_pAllocContext->m_nMaximumSize = MAX(m_pAllocContext->m_nMaximumSize,
                                          m_pAllocContext->m_nCurrentAllocSize);
  }
};
...
static void AddMemoryAllocation(void *pResult, size_t size) {
  if (pResult) {
    ...
    for (CLinuxMallocContext *i = s_ContextHash[nHash].m_pHead; i;
         i = i->m_pNext) {
...
static inline bool SortLessFunc(CLinuxMallocContext *const &left,
                                CLinuxMallocContext *const &right) {
...
void DumpMemoryLog(int nThresh) {
  ...
  for (int i = 0; i < memList.size(); i++) {
    ...
    Msg("Context cursize=%d nallocs=%d maxsize=%d total_allocs=%d\n",
        p->m_nCurrentAllocSize, p->m_nNumAllocsInUse, p->m_nMaximumSize,
        p->m_TotalNumAllocs);
    Msg("  stack\n");
...
void DumpChangedMemory(int nThresh) {
  ...
  for (int i = 0; i < memList.size(); i++) {
    CLinuxMallocContext *p = memList[i];
    char **strings = backtrace_symbols(p->pStackTraceBack, MAX_STACK_TRACEBACK);
    Msg("Context cursize=%d lastsize=%d nallocs=%d maxsize=%d "
        "total_allocs=%d\n",
        p->m_nCurrentAllocSize, p->m_nLastAllocSize, p->m_nNumAllocsInUse,
...
void DumpMemorySummary(void) {
  Msg("Total memory in use = %d, NumMallocs=%d, Num Frees=%d approx process "
      "usage=%ul\n",
      g_LinuxMemStats.nTotalMallocInUse, g_LinuxMemStats.nNumMallocs,
      g_LinuxMemStats.nNumFrees, (unsigned int)ApproximateProcessMemoryUsage());
}

#endif // !NO_HOOK_MALLOC

// Turn off memdbg macros (turned on up top) since this is included like a
// header
