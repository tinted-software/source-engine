...
int WalkStack(void **ppAddresses, int nMaxAddresses, int nSkip = 0) {
  ...
  while (nMaxAddresses - nWalked > 0) {
    if (!StackWalk64(IMAGE_FILE_MACHINE_I386, hProcess, hThread, &frame, NULL,
...
bool GetCallerModule(char *pDest) {
  ...
  if (!bInit) {
    ...
    psUserSearchPath = "u:\\data\\game\\bin\\;u:\\data\\game\\episodic\\bin\\;"
                       "u:\\data\\game\\hl2\\bin\\;\\\\perforce\\symbols";
  ...
  for (int k = 0; k < nMaxAddresses; ++k) {
    ...
    if (SymGetLineFromAddr64(GetCurrentProcess(), (DWORD64)ppAddresses[k],
      ...
      char const *pchFileName =
          hlpLine.FileName ? hlpLine.FileName + strlen(hlpLine.FileName) : NULL;
...
{
  size_t nLogicalSize;
  byte reserved[12]; // MS allocator always returns mem aligned on 16 bytes,
...
#ifdef OSX
  void *pAllocedMem =
      malloc_zone_malloc(malloc_default_zone(), nSize + sizeof(DbgMemHeader_t));
  if (!pAllocedMem) {
    return NULL;
  }
...
inline void *InternalRealloc(void *pMem, size_t nNewSize, const char *pFileName,
  ...
  pNewAllocedMem = (void *)malloc_zone_realloc(
      malloc_default_zone(), pMem, nNewSize + sizeof(DbgMemHeader_t));
...
template <class T> class CNoRecurseAllocator {
public:
  // type definitions
...
#ifdef WIN32
extern "C" BOOL APIENTRY MemDbgDllMain(HMODULE hDll, DWORD dwReason,
                                       PVOID pvReserved) {
  UNREFERENCED_PARAMETER(pvReserved);

  // Check if we are shutting down
  if (dwReason == DLL_PROCESS_DETACH) {
    // CDbgMemAlloc is a global object and destructs after the _Lockit object in
    // the CRT runtime,
    //  so we can't actually operate on the STL object in a normal destructor
    //  here as its support libraries have been turned off already
...
void CDbgMemAlloc::RegisterDeallocation(MemInfo_t &info, size_t nLogicalSize,
  ...
  // It is technically legal for code to request allocations of zero bytes, and
  // there are a number of places in our code that do. So only assert that
  // nLogicalSize >= 0.
...
void CDbgMemAlloc::DumpMemInfo(const char *pAllocationName, int line,
  ...
  m_OutputFunc("%s, line %i\t%.1f\t%.1f\t%.1f\t%.1f\t%.1f\t%d\t%d\t%d\t%d",
               ...
               info.m_nPeakOverheadSize / 1024.0f, (int)(info.m_nTime / 1000),
...
void CDbgMemAlloc::DumpStatsFileBase(char const *pchFileBase) {
...
#else
    _snprintf(szFileName, sizeof(szFileName), "%s%s%d.txt", pPath, pchFileBase,
              s_FileCount);
#endif
  ...
  m_OutputFunc("Allocation type\tCurrent Size(k)\tPeak Size(k)\tTotal "
...
#endif
    vm_protect(mach_task_self(), (uintptr_t)malloc_zone, sizeof(malloc_zone_t),
...
static inline void set_osx_hooks(void) {
  ...
  // *))osx_size_hook; malloc_zone->calloc = (void* (*)(_malloc_zone_t*, size_t,
