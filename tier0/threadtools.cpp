...
// this is used to provide a unique threadid for each running thread in
// g_nThreadID ( a thread local variable ).
...
PLATFORM_INTERFACE void AllocateThreadID(void) {
  ...
  Error("Out of thread ids. Decrease the number of threads or increase "
        "MAX_THREAD_IDS\n");
}
...
void TlsSetValue(uint32 index, void *pValue) { gTLSValues[index] = pValue; }
#endif //_PS3
...
static bool InternalLookupHandleToThreadIDMap(HANDLE hThread,
                                              CThreadHandleToIDMap *&pMap,
...
static uint LookupThreadIDFromHandle(HANDLE hThread) {
  ...
  while (Plat_FloatTime() - flStartTime < 2) {
    ...
    // We should only get here if a thread that is just starting up is currently
    // in AddThreadHandleToIDMap. Give up the timeslice and try again.
    ThreadSleep(1);
  }
...
ThreadHandle_t *CreateTestThreads(ThreadFunc_t fnThread, int numThreads,
  ...
  for (int i = 0; i < numThreads; ++i) {
    ...
    const unsigned int nDefaultStackSize =
        64 * 1024; // this stack size is used in case stackSize == 0
    hThread =
        CreateSimpleThread(fnThread, INT_TO_POINTER(i), nDefaultStackSize);
    ...
    /*
                    ...
                    if ( int nError = sys_ppu_thread_create( &hThread,
       ThreadProcConvertUnion, info.val64, 1001, nDefaultStackSize,
...
ThreadHandle_t CreateSimpleThread(ThreadFunc_t pfnThread, void *pParam,
  ...
  HANDLE hThread = (HANDLE)CreateThread(
      NULL, stackSize, ThreadProcConvert,
      new ThreadProcInfo_t(pfnThread, pParam),
      stackSize ? STACK_SIZE_PARAM_IS_A_RESERVATION : 0, &threadID);
  ...
  const unsigned int nDefaultStackSize =
      64 * 1024; // this stack size is used in case stackSize == 0
  if (sys_ppu_thread_create(&th, ThreadProcConvertUnion, info.val64, 1001,
                            stackSize ? stackSize : nDefaultStackSize,
                            SYS_PPU_THREAD_CREATE_JOINABLE,
                            "SimpleThread") != CELL_OK) {
...
#elif POSIX
  pthread_t tid;
  pthread_create(&tid, NULL, ThreadProcConvert,
                 new ThreadProcInfo_t(pfnThread, pParam));
...
bool ThreadJoin(ThreadHandle_t hThread, unsigned timeout) {
  ...
  if (dwWait != WAIT_OBJECT_0 &&
      (dwWait != WAIT_FAILED && GetLastError() != 0)) {
    Assert(0);
    return false;
  }
#elif defined(_PS3)
...
PLATFORM_INTERFACE void
SetThreadedLoadLibraryFunc(ThreadedLoadLibraryFunc_t func) {
...
#endif
{
  ...
  if (cellAtomicCompareAndSwap32(&m_bstaticMutexInitializing, false, true) ==
      false) {
    ...
    // Another thread is already in the process of initialising the mutex, wait
    // for it
    while (!m_bstaticMutexInitialized) {
      // sys_ppu_thread_yield doesn't seem to function properly, so sleep
      // instead.
      //			sys_timer_usleep( 60 );
...
#ifdef THREADS_DEBUG
#if PS3
  AssertMsg(m_bstaticMutexInitialized,
  ...
  if (m_cSet > 0) {
    ...
    while (!m_bWakeForEvent && ret != ETIMEDOUT) {
      ...
      if (dwTimeout == TT_INFINITE && m_bManualReset)
        actualTimeout = 10; // just wait 10 msec at most for manual reset events
                            // and loop instead

      volatile uint64 nNanoSec =
          (uint64)tv.tv_usec * 1000 + (uint64)actualTimeout * 1000000;
      tm.tv_sec = tv.tv_sec + nNanoSec / 1000000000;
      tm.tv_nsec = nNanoSec % 1000000000;

      do {
        ret = pthread_cond_timedwait(&m_Condition, &m_Mutex,
                                     (const timespec *)&tm);
      } while (ret == EINTR);
...
uint32 CThreadSyncObject::WaitForMultiple(int nObjects,
                                          CThreadSyncObject **ppObjects,
                                          bool bWaitAll, uint32 dwTimeout) {
#if defined(_WIN32)

  CThreadSyncObject *pHandles =
      (CThreadSyncObject *)stackalloc(sizeof(CThreadSyncObject) * nObjects);
...
uint32 CThreadSyncObject::WaitForMultiple(int nObjects,
  ...
  if (ret == WAIT_TIMEOUT)
    ...
    return (int)(ret - WAIT_OBJECT_0);
           ...
           (ret - WAIT_ABANDONED_0) < (uint32)nObjects)
...
uint32 CThreadEvent::WaitForMultiple(int nObjects, CThreadEvent *pObjects,
                                     bool bWaitAll, uint32 dwTimeout) {
  // If data ever gets added to CThreadEvent, then we need a different
  // implementation.
#ifdef _PS3
  CThreadEvent **ppObjects =
      (CThreadEvent **)stackalloc(sizeof(CThreadEvent *) * nObjects);
  for (int i = 0; i < nObjects; i++) {
    ppObjects[i] = &pObjects[i];
  }
  return WaitForMultipleObjects(nObjects, ppObjects, bWaitAll, dwTimeout);
#else
  COMPILE_TIME_ASSERT(sizeof(CThreadSyncObject) == 0 ||
                      sizeof(CThreadEvent) == sizeof(CThreadSyncObject));
  return CThreadSyncObject::WaitForMultiple(
      nObjects, (CThreadSyncObject *)pObjects, bWaitAll, dwTimeout);
#endif
}

uint32 CThreadEvent::WaitForMultiple(int nObjects, CThreadEvent **ppObjects,
                                     bool bWaitAll, uint32 dwTimeout) {
#ifdef _PS3
  return WaitForMultipleObjects(nObjects, ppObjects, bWaitAll, dwTimeout);
#else
  ...
  COMPILE_TIME_ASSERT(sizeof(CThreadSyncObject) == 0 ||
                      sizeof(CThreadEvent) == sizeof(CThreadSyncObject));
  return CThreadSyncObject::WaitForMultiple(
      nObjects, (CThreadSyncObject **)ppObjects, bWaitAll, dwTimeout);
#endif
}
...
CThreadEvent::LLUnlinkNode(CThreadEventWaitObject *node) {
  // <sergiy> Note: if you have a null-access crash here, it may mean that
  // CTHREADEVENT_MAX_WAITING_THREADS is not high enough
...
CThreadEventWaitObject *CThreadEvent::LLLinkNode(CThreadEventWaitObject *list,
...
// We do NOT need this to be atomic because if a number of events fire it
// doesn't matter which one of these we pick
//-----------------------------------------------------------------------------
...
//
void CThreadEvent::RegisterWaitingThread(sys_semaphore_t *pSemaphore, int index,
                                         int *flag) {
  ...
  if (m_bSet) {
    ...
    if (!m_pWaitObjectsPool->m_pNext) {
      DEBUG_ERROR(
          "CThreadEvent: Ran out of events; cannot register waiting thread\n");
    }

    // add this semaphore to linked list - can be added more than once it
    // doesn't matter

    CThreadEventWaitObject *pWaitObject =
        LLUnlinkNode(m_pWaitObjectsPool->m_pNext);
...
#ifdef _WIN32
CThreadEvent::CThreadEvent(const char *name, bool initialState,
                           bool bManualReset) {
...
NamedEventResult_t CThreadEvent::CheckNamedEvent(const char *name,
                                                 uint32 dwTimeout) {
...
//////////////////////////////////////////////////////////////
  ...
  if (m_bManualReset) {
    ...
    // If any threads are already waiting then signal ONE to run, else signal
    // next to run
...
//////////////////////////////////////////////////////////////
  ...
  while (m_pWaitObjectsList->m_pNext) {
    CThreadEventWaitObject *pWaitObject =
        LLUnlinkNode(m_pWaitObjectsList->m_pNext);
...
//////////////////////////////////////////////////////////////
...

  {
    ...
    while (timeout = MIN(1, dwTimeout)) {
      ...
      // #error Untested code:
      ret = sys_semaphore_wait(m_Semaphore,
                               timeout == TT_INFINITE ? 0 : timeout * 1000);
...
// CThreadSemaphore
//
// To get Posix implementation, try
...
bool CThreadSemaphore::Wait(uint32 dwTimeout) {
  ...
  if (!AddWaitingThread()) {
    // Waiting thread NOT added because semaphore was already in a signaled
    // state
    return true;
  }

  int ret = sys_semaphore_wait(m_Semaphore,
                               dwTimeout == TT_INFINITE ? 0 : dwTimeout * 1000);
  Assert((ret == CELL_OK) || (ret == ETIMEDOUT));
...
#endif
#ifdef _WIN32
  return (ReleaseSemaphore(m_hSyncObject, releaseCount,
...
int32 ThreadInterlockedCompareExchange(int32 volatile *pDest, int32 value,
                                       int32 comperand) {
  Assert((size_t)pDest % 4 == 0);
...
bool ThreadInterlockedAssignIf(int32 volatile *pDest, int32 value,
                               int32 comperand) {
  Assert((size_t)pDest % 4 == 0);
...
int64 ThreadInterlockedCompareExchange64(int64 volatile *pDest, int64 value,
                                         int64 comperand) {
...
bool ThreadInterlockedAssignIf64(volatile int64 *pDest, int64 value,
                                 int64 comperand) {
  Assert((size_t)pDest % 8 == 0);
...
#ifdef _WIN64
bool ThreadInterlockedAssignIf128(volatile int128 *pDest, const int128 &value,
                                  const int128 &comperand) {
  DbgAssert(((size_t)pDest % 16) == 0);
  // Must copy comperand to stack because the intrinsic uses it as an in/out
  // param
  int64 comperandInOut[2] = {comperand.m128i_i64[0], comperand.m128i_i64[1]};

  // Description:
  //  The CMPXCHG16B instruction compares the 128-bit value in the RDX:RAX and
  //  RCX:RBX registers with a 128-bit memory location. If the values are equal,
  //  the zero flag (ZF) is set, and the RCX:RBX value is copied to the memory
  //  location. Otherwise, the ZF flag is cleared, and the memory value is
  //  copied to RDX:RAX.

  // _InterlockedCompareExchange128:
  // http://msdn.microsoft.com/en-us/library/bb514094.aspx
  if (_InterlockedCompareExchange128((volatile int64 *)pDest,
                                     value.m128i_i64[1], value.m128i_i64[0],
                                     comperandInOut))
    return true;
  return false;
}
...
void *ThreadInterlockedCompareExchangePointer(void *volatile *p, void *value,
                                              void *comparand) {
  return (void *)((intp)ThreadInterlockedCompareExchange64(
      reinterpret_cast<intp volatile *>(p), reinterpret_cast<intp>(value),
      reinterpret_cast<intp>(comparand)));
}
#endif
...
bool ThreadInterlockedAssignIf64(int64 volatile *pDest, int64 value,
                                 int64 comperand) {
  return __sync_bool_compare_and_swap(pDest, comperand, value);
}
...
#error                                                                         \
    "Falling back to mutexed interlocked operations, you really don't have intrinsics you can use?"ß
...
PLATFORM_INTERFACE int64 ThreadInterlockedOr64(int64 volatile *pDest,
                                               int64 value) {
  int64 Old;
...
PLATFORM_INTERFACE int64 ThreadInterlockedAnd64(int64 volatile *pDest,
                                                int64 value) {
  int64 Old;
...
PLATFORM_INTERFACE int64 ThreadInterlockedExchangeAdd64(int64 volatile *pDest,
                                                        int64 value) {
  int64 Old;
...
CThreadMutex::~CThreadMutex() { sys_mutex_destroy(m_Mutex); }
#elif !defined(POSIX)
CThreadMutex::CThreadMutex() {
...
static CDynamicFunction<TryEnterCriticalSectionFunc_t>
    DynTryEnterCriticalSection("Kernel32.dll", "TryEnterCriticalSection");
#elif defined(_X360)
...
bool CThreadMutex::TryLock() {
  ...
  if (DynTryEnterCriticalSection != NULL) {
    if ((*DynTryEnterCriticalSection)((CRITICAL_SECTION *)&m_CriticalSection) !=
        FALSE) {
#ifdef THREAD_MUTEX_TRACING_ENABLED
      if (m_lockCount == 0) {
...
// This is meant to be used in combination with breakpoints and in-debugee, so
// we turn the optimizer off
#pragma optimize("", off)
CThreadFastMutex *
    g_pIgnoredMutexes[256]; // Ignore noisy non-problem mutex. Probably could be
                            // an array. Right now needed only for sound thread
float g_MutexTimingTolerance = 5;
bool g_bMutexTimingOutput;

void TrapMutexTimings(uint32 probableBlocker, uint32 thisThread,
                      volatile CThreadFastMutex *pMutex, CFastTimer &spikeTimer,
                      CAverageCycleCounter &sleepTimer) {
  spikeTimer.End();
  if (spikeTimer.GetDuration().GetMillisecondsF() > g_MutexTimingTolerance) {
    bool bIgnore = false;
    for (int j = 0; j < ARRAYSIZE(g_pIgnoredMutexes) && g_pIgnoredMutexes[j];
         j++) {
    ...
    if (!bIgnore && spikeTimer.GetDuration().GetMillisecondsF() < 100) {
      volatile float FastMutexDuration =
          spikeTimer.GetDuration().GetMillisecondsF();
      ...
      if (g_bMutexTimingOutput) {
        char szBuf[256];
        Msg("M (%.8x): [%.8x <-- %.8x] (%f,%f,%f)\n", pMutex, probableBlocker,
            thisThread, FastMutexDuration, average, peak);
      }
    }
  }
}
...
void CThreadFastMutex::Lock(const uint32 threadId,
...
#endif
        if (TryLock(threadId)) {
          TrapMutexTimings(currentOwner, threadId, this, spikeTimer,
                           sleepTimer);
          return;
        }
...
when new implementation settles) void CThreadSpinRWLock::SpinLockForWrite( const
uint32 threadId )
{
        int i;
        ...
        for ( ;; ) // coded as for instead of while to make easy to breakpoint
success
        {
                if ( TryLockForWrite( threadId ) )
                {
...
m_lockInfo.m_nReaders == 0 ); static const LockInfo_t newValue = { { 0, 0 } };
        ThreadMemoryBarrier();
        ThreadInterlockedExchange64(  (int64 *)&m_lockInfo, *((int64
*)&newValue) ); m_nWriters--;
}
*/
#endif
...
// This function is implemented here rather than the header because g_pCurThread
// resolves to GetCurThread() on PS3 and we don't want to create a dependency on
// the ELF stub for everyone who includes the header.
...
// It makes a difference of more than 1ms/frame for other platforms, we include
// the .inl in the .cpp file where it existed before
...
//---------------------------------------------------------
static uint32 DefaultWaitFunc(uint32 nHandles, CThreadEvent **ppHandles,
                              int bWaitAll, uint32 timeout) {
  return CThreadEvent::WaitForMultiple(nHandles, ppHandles, bWaitAll != 0,
                                       timeout);
}
...
int CWorkerThread::WaitForReply(unsigned timeout, WaitFunc_t pfnWait) {
  ...
  do {
    ...
    AssertMsg(timeout != TT_INFINITE || result != TW_TIMEOUT,
 ...
 *******************************************************************************/
BOOL ResetEvent(CThreadEvent *pEvent) { return pEvent->Reset(); }
...
DWORD WaitForMultipleObjects(DWORD nCount, CThreadEvent **lppHandles,
  ...
  while (bRunning) {
    // Check for a timeout
    if (bRunning && (dwMilliseconds != INFINITE) &&
        (timeDiffMS > dwMilliseconds)) {
      result = TW_TIMEOUT;
      bRunning = false;
    }
...
#error This code was written expecting to be run on PS3.
  ...
  // in the Source API, a timeOut of 0 means very short timeOut, not (as in the
  // PS3 spec) an infinite timeout. TT_INFINITE is #defined to 2^31-1, which
  // means "infinite timeout" on PC and "72 minutes, 35 seconds" on PS3.
  // conversely, the code below (around deltaTime) expects to be able to compare
  // against the timeout value given here, so we cannot just replace 0 with 1
  // and TT_INFINITE with 0. So, we replace 0 with 1, meaning "a very short
  // time", and test for the special value TT_INFINITE at the moment of calling
  // sys_semaphore_wait, where we replace it with the real "infinite timeout"
  // value. It isn't safe to simply increase the declaration size of
  // TT_INFINITE, because as you can see it is often assigned to uint32s. Also,
  // Source timeouts are specified in milliseconds, and PS3 timeouts are in
  // microseconds, so we need to multiply by one thousand.
  ...
  COMPILE_TIME_ASSERT(
      TT_INFINITE !=
      0); // The code here was written expecting (working around) that
          // TT_INFINITE is MAXINT, so if you changed this number, please read
          // the comment above and carefully examine the code here to make sure
          // that timeouts still work correctly on PS3. Be aware that in many
          // places in Source, a timeout of 0 has some special meaning other
          // than "infinite timeout", so track those down too.
  ...
  if (bWaitAll) {
    while (numEvent < nCount) {
      ...
      res = sys_semaphore_wait(gWaitObjectsSemaphore,
                               timeOut == TT_INFINITE ? 0 : timeOut);
