...
inline bool ValidStackAddress(void *pAddress, const void *pNoLessThan,
  ...
  if ((uintp)pAddress & 3)
    return false;
  if (pAddress <
      pNoLessThan) // frame pointer traversal should always increase the pointer
    return false;
...
int GetCallStack_Fast(void **pReturnAddressesOut, int iArrayCount,
  ...
  */
  ...
  CStackTop_FriendFuncs *pTop =
      (CStackTop_FriendFuncs *)(CStackTop_Base *)g_StackTop;
  if (pTop != NULL) // we can do fewer error checks if we have a valid reference
                    // point for the top of the stack
  {
...
public:
  ...
  void
  SetStackTranslationSymbolSearchPath(const char *szSemicolonSeparatedList) {
    AUTO_LOCK(m_Mutex);
  ...
  bool TranslatePointer(const void *const pAddress, tchar *pTranslationOut,
    ...
    //  u:\Dev\L4D\src\tier0\stacktools.cpp(162) + 4 bytes" fallback translation
    //  -	"tier0.dll!0x01234567"
...
int CrawlStack_StackWalk64(CONTEXT *pExceptionContext,
  ...
  int i;
  for (i = 0; i != iSkipCount; ++i) // skip entries that the requesting function
                                    // thinks are uninformative
  {
...
#pragma pack(1)
  struct StackAddressInfo_t {
    StackAddressInfo_t(void)
        : szModule(NULL), szFileName(NULL), szSymbol(NULL), iLine(0),
          iSymbolOffset(0) {}
    const char *szModule;
...
int GetCallStack(void **pReturnAddressesOut, int iArrayCount, int iSkipCount) {
  ...
  if (DmCaptureStackBackTrace(iArrayCount + iSkipCount, pAllResults) ==
    ...
    for (int i = 0; i != iSkipCount; ++i) {
      if (*pAllResults ==
          NULL) // DmCaptureStackBackTrace() NULL terminates the list instead of
                // telling us how many were returned
        return AppendParentStackTrace(pReturnAddressesOut, iArrayCount, 0);
...
int TranslateStackInfo(const void *const *pCallStack, int iCallStackCount,
  ...
  // binary]"
  int iEncodedSize =
      -EncodeBinaryToString(NULL, iDataSize, NULL, 0); // get needed buffer size
...
int EncodeBinaryToString(const void *pToEncode, int iDataLength,
  ...
  while (pEncodeRead < pEncodeStop) {
    if (iControl == 0) {
      ...
      *pEncodeWrite =
          *pEncodeRead | 0x80; // encoded data always has the MSB bit set (cheap
                               // avoidance of null terminators)
