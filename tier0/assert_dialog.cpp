...
typedef int(SDLCALL FUNC_SDL_ShowMessageBox)(
...
DBG_INTERFACE struct SDL_Window *GetAssertDialogParent() { return g_SDLWindow; }
#endif
...
DBG_INTERFACE bool DoNewAssertDialog(const tchar *pFilename, int line,
...
#define COLOR_END "\033[0m"
    fprintf(stderr,
            COLOR_YELLOW "ASSERT:" COLOR_END " " COLOR_RED "%s" COLOR_GREEN
                         ":%i:" COLOR_END " " COLOR_RED "%s" COLOR_END "\n",
  ...
  // Now create the dialog. Just return true for old-style debug break upon
  // failure.
  ...
  if (g_VXConsoleAssertReturnValue == -1) {
    ...
    default:
      // Error... just break
      XBX_Error("DoNewAssertDialog: invalid Assert response returned from "
...
#endif
    if (ret) {
      pfnSDLShowMessageBox =
          (FUNC_SDL_ShowMessageBox *)dlsym(ret, "SDL_ShowMessageBox");
    }
  }
