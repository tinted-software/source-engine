...
#elif defined(_WIN32)
// Define this to force using the OS Heap* functions for allocations. This is
// useful in conjunction with AppVerifier/PageHeap in order to find memory
// problems, and also allows ETW/xperf tracing to be used to record allocations.
...
public:
  ...
  void Init(bool bZeroMemory) {
    ...
    if (s_bPageHeapEnabled) {
      OutputDebugStringA(
          "PageHeap is on. Memory use will be larger than normal.\n");
    } else {
  ...
  virtual void *Realloc(void *pMem, size_t nSize) {
    // If you pass zero to HeapReAlloc then it fails (with GetLastError() saying
    // S_OK!) so only call HeapReAlloc if pMem is non-zero.
    if (pMem) {
      if (!nSize) {
...
#error
  ...
  virtual void DumpStats() {
    ...
    Msg("Sorry -- no stats saved to file memstats.txt when the heap allocator "
        "is enabled.\n");
    ...
    // Print memory after adding in reserved page after every allocation. Do
    // 64-bit calculations because the pageHeap required memory can easily go
    // over 4 GB.
    __int64 pageHeapBytes =
        m_nOutstandingPageHeapBytes + m_nOutstandingAllocations * 4096LL;
  ...
  virtual MemAllocFailHandler_t
  SetAllocFailHandler(MemAllocFailHandler_t pfnMemAllocFailHandler) {
    return 0;
  }
...
// Check whether PageHeap (part of App Verifier) has been enabled for this
// process. It specifically checks whether it was enabled by the
// EnableAppVerifier.bat batch file. This can be used to automatically enable
// -processheap when App Verifier is in use.
    ...
    if (GetModuleFileNameA(exeHandle, appName, ARRAYSIZE(appName))) {
      ...
      if (pFilePart) {
        ...
        if (len > 0 && pFilePart[len - 1] == ' ') {
          OutputDebugStringA(
              "Trailing space on executable name! This will cause Application "
              "Verifier and ETW Heap tracing to fail!\n");
        ...
        if (regResult == ERROR_SUCCESS) {
          // If PageHeapFlags exists then that means that App Verifier is
          // enabled for this application. The StackTraceDatabaseSizeInMB is
          // only set by Valve's enabling batch file so this indicates that a
          // developer at Valve is using App Verifier.
          if (RegQueryValueExA(key, "StackTraceDatabaseSizeInMB", 0, NULL, NULL,
                               NULL) == ERROR_SUCCESS &&
              RegQueryValueExA(key, "PageHeapFlags", 0, NULL, NULL, NULL) ==
                  ERROR_SUCCESS) {
            result = true;
          }

          if (RegQueryValueExA(key, "TracingFlags", 0, NULL, NULL, NULL) ==
              ERROR_SUCCESS)
...
// Check for various allocator overrides such as -processheap and
// -reservelowmem. Returns true if -processheap is enabled, by a command line
...
bool CheckWindowsAllocSettings(const char *upperCommandLine) {
  ...
  // However this needs to be off by default to avoid causing compatibility
  // problems, with Steam detours and other systems. It should also be disabled
  // when PageHeap is on because for some reason the combination turns into 4 GB
  // of working set, which can easily cause problems.
  ...
  // statistics with ETW/xperf. When using App Verifier -processheap is
  ...
  // Unless specifically disabled, turn on -processheap if pageheap or ETWHeap
  // tracing are enabled.
  if (!strstr(upperCommandLine, "-NOPROCESSHEAP") &&
      (s_bPageHeapEnabled || bETWHeapEnabled))
    bProcessHeap = true;
...
//
//-----------------------------------------------------------------------------
template <typename T> inline T MemAlign(T val, size_t alignment) {
...
void CSmallBlockPool::Init(unsigned nBlockSize, byte *pBase,
                           unsigned initialCommit) {
...
int CSmallBlockPool::CountAllocatedBlocks() {
  return CountCommittedBlocks() -
         (CountFreeBlocks() +
          (m_pCommitLimit - (byte *)m_pNextAlloc) / GetBlockSize());
}
...
CSmallBlockHeap::CSmallBlockHeap() {
  ...
  m_pBase = (byte *)VirtualAlloc(NULL, NUM_POOLS * MAX_POOL_REGION,
...
void CSmallBlockHeap::DumpStats(FILE *pFile) {
  ...
  if (pFile) {
    for (int i = 0; i < NUM_POOLS; i++) {
      ...
      fprintf(pFile,
              "Pool %i: Size: %llu Allocated: %i Free: %i Committed: %i "
              "CommittedSize: %i\n",
              i, (uint64)m_Pools[i].GetBlockSize(),
              m_Pools[i].CountAllocatedBlocks(), m_Pools[i].CountFreeBlocks(),
  ...
  if (bSpew) {
    ...
    for (int i = 0; i < NUM_POOLS; i++) {
      Msg("Pool %i: (size: %llu) blocks: allocated:%i free:%i committed:%i "
          "(committed size:%u kb)\n",
          i, (uint64)m_Pools[i].GetBlockSize(),
          m_Pools[i].CountAllocatedBlocks(), m_Pools[i].CountFreeBlocks(),
...
CX360SmallBlockPool
    *CX360SmallBlockPool::gm_AddressToPool[BYTES_X360_SBH / PAGESIZE_X360_SBH];
...
void CX360SmallBlockPool::Init(unsigned nBlockSize) {
  if (!gm_pPhysicalBlock) {
    gm_pPhysicalBase = (byte *)XPhysicalAlloc(
        BYTES_X360_SBH, MAXULONG_PTR, 4096, PAGE_READWRITE | MEM_16MB_PAGES);
...
size_t CX360SmallBlockPool::GetBlockSize() { return m_nBlockSize; }

bool CX360SmallBlockPool::IsOwner(void *p) { return (FindPool(p) == this); }
        ...
        if (pCurBlockEnd == m_pCurBlockEnd) {
          for (;;) {
            ...
            if (ThreadInterlockedAssignPointerIf(
                    (void **)&gm_pPhysicalBlock,
                    (void *)(pPhysicalBlock + PAGESIZE_X360_SBH),
                    (void *)pPhysicalBlock)) {
              int index = (size_t)((byte *)pPhysicalBlock - gm_pPhysicalBase) /
                          PAGESIZE_X360_SBH;
...
// Count the free blocks.
int CX360SmallBlockPool::CountFreeBlocks() { return m_FreeList.Count(); }

// Size of committed memory managed by this heap:
int CX360SmallBlockPool::GetCommittedSize() { return m_CommittedSize; }
...
int CX360SmallBlockPool::CountAllocatedBlocks() {
  int nBytesPossible =
      (m_pNextAlloc) ? (m_pCurBlockEnd - (byte *)m_pNextAlloc) : 0;
  return CountCommittedBlocks() -
         (CountFreeBlocks() + nBytesPossible / GetBlockSize());
}
...
bool CX360SmallBlockHeap::IsOwner(void *p) {
  int index = (size_t)((byte *)p - CX360SmallBlockPool::gm_pPhysicalBase) /
              PAGESIZE_X360_SBH;
  return (
      UsingSBH() &&
      (index >= 0 && index < ARRAYSIZE(CX360SmallBlockPool::gm_AddressToPool)));
}
...
void CX360SmallBlockHeap::DumpStats(FILE *pFile) {
  ...
  if (pFile) {
    for (int i = 0; i < NUM_POOLS; i++) {
      // output for vxconsole parsing
      fprintf(pFile,
              "Pool %i: Size: %u Allocated: %i Free: %i Committed: %i "
              "CommittedSize: %i\n",
...
CSmallBlockHeap *CX360SmallBlockHeap::GetStandardSBH() {
  return &(
      GET_OUTER(CStdMemAlloc, m_LargePageSmallBlockHeap)->m_SmallBlockHeap);
}
...
int CStdMemAlloc::CrtCheckMemory(void) { return 1; }
...
void CStdMemAlloc::DumpStatsFileBase(char const *pchFileBase) {
  ...
  _snprintf(filename, sizeof(filename) - 1,
            (IsX360()) ? "D:\\%s.txt" : "%s.txt", pchFileBase);
...
void CStdMemAlloc::GlobalMemoryStatus(size_t *pUsedMemory,
  ...
  *pFreeMemory = stat.dwAvailPhys;
  ...
  // NOTE: we do not count free memory inside our small block heaps, as this
  // could be misleading
  //       (even with lots of SBH memory free, a single allocation over 2kb can
  //       still fail)
...
void ReserveBottomMemory() {
  ...
  // If AppVerifier is enabled then memory reservations get turned into
  // committed memory in the working set. This means that ReserveBottomMemory()
  // can end up adding almost 4 GB to the working set, which is a significant
  // problem if you run many processes in parallel. Therefore, if vfbasics.dll
  // (part of AppVerifier) is loaded, don't do the reservation.
  ...
  for (size_t blockSize = 256 * 1024 * 1024; blockSize >= 1024 * 1024;
       blockSize /= 2) {
    for (;;) {
  ...
  // that was previously reserved and is available to be handed out by some
  // allocators.
  // char buffer[1000];
  // sprintf_s( buffer, "Reserved %1.3f MB (%d vallocs, %d heap allocs) to keep
  // allocations out of low-memory.\n", 			totalReservation / (1024 * 1024.0),
