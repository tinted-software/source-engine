...
// our global error callback function. Note that this is not initialized, but
// static space guarantees this is NULL at app start. If you initialize, it will
// set to zero again when the CPP runs its static initializers, which could
// stomp the value if another CPP sets this value while initializing its static
// space
...
uint64 Plat_USTime() {
  ...
  return (uint64)((CurrentTime.QuadPart - g_ClockStart.QuadPart) *
                  g_PerformanceCounterToUS);
}
...
void Plat_ExitProcess(int nCode) {
  ...
  if (nCode || (strstr(pchCmdLineA, "gc.exe") &&
                strstr(pchCmdLineA, "gc.dll") && strstr(pchCmdLineA, "-gc"))) {
    int *x = NULL;
    *x = 1; // cause a hard crash, GC is not allowed to exit voluntarily from
            // gc.dll
  }
...
bool GetMemoryInformation(MemoryInformation *pOutMemoryInfo) {
  ...
  case 0:
    ...
    (*pOutMemoryInfo).m_nVirtualRamMbAvailable =
        memStat.ullAvailVirtual / cOneMb;
    break;
...
int Plat_GetWatchdogTime(void) { return 0; }
...
bool Is64BitOS() {
  ...
  if (bInitialized)
    ...
    bInitialized = true;
    return pfnIsWow64Process &&
           pfnIsWow64Process(GetCurrentProcess(), &bIs64bit) && bIs64bit;
  }
}
