...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CSourceVirtualReality, ISourceVirtualReality,
                                  SOURCE_VIRTUAL_REALITY_INTERFACE_VERSION,
                                  g_SourceVirtualReality);
...
static ConCommand
    vr_reset_home_pos("vr_reset_home_pos", CC_VR_Reset_Home_Pos,
                      "Sets the current HMD position as the zero point");
...
static ConCommand vr_track_reinit("vr_track_reinit", CC_VR_Track_Reinit,
                                  "Reinitializes HMD tracking");

// Disable distortion processing altogether.
ConVar vr_distortion_enable("vr_distortion_enable", "1");

// Disable distortion by changing the distortion texture itself, so that the
// rendering path is otherwise identical. This won't take effect until the
// texture is refresed.
ConVar vr_debug_nodistortion("vr_debug_nodistortion", "0");

// Disable just the chromatic aberration correction in the distortion texture,
// to make undistort quality/artifacts easier to see and debug.  As above, won't
// take effect until the texture is refreshed.
...
ConCommand vr_refresh_distortion_texture("vr_refresh_distortion_texture",
                                         CC_vr_refresh_distortion_texture);

ConVar
    vr_use_offscreen_render_target("vr_use_offscreen_render_target", "0", 0,
                                   "Experimental: Use larger offscreen render "
                                   "target for pre-distorted scene in VR");
...
InitReturnVal_t CSourceVirtualReality::Init() {
  ...
  m_pDistortionTextureLeft.Init(materials->CreateProceduralTexture(
      "vr_distort_map_left", TEXTURE_GROUP_PIXEL_SHADERS, distortionTextureSize,
      distortionTextureSize, IMAGE_FORMAT_RGBA16161616,
  ...
  m_pDistortionTextureRight.Init(materials->CreateProceduralTexture(
      "vr_distort_map_right", TEXTURE_GROUP_PIXEL_SHADERS,
...
void CDistortionTextureRegen::RegenerateTextureBits(ITexture *pTexture,
  ...
  for (y = 0; y < height; y++) {
    for (x = 0; x < width; x++) {
      ...
      imageData[offset + 0] =
          (unsigned short)(Clamp(coords.rfRed[0], 0.f, 1.f) * 65535.f);
      imageData[offset + 1] =
          (unsigned short)(Clamp(coords.rfRed[1], 0.f, 1.f) * 65535.f);
      imageData[offset + 2] =
          (unsigned short)(Clamp(coords.rfBlue[0], 0.f, 1.f) * 65535.f);
      imageData[offset + 3] =
          (unsigned short)(Clamp(coords.rfBlue[1], 0.f, 1.f) * 65535.f);
    }
  }
}
...
// --------------------------------------------------------------------
void CSourceVirtualReality::CreateRenderTargets(
    IMaterialSystem *pMaterialSystem) {
  if (!m_pHmd || !m_bActive)
    return;

  g_StereoGuiTexture.Init(materials->CreateNamedRenderTargetTextureEx2(
      "_rt_gui", 640, 480, RT_SIZE_OFFSCREEN, materials->GetBackBufferFormat(),
  ...
  if (UsingOffscreenRenderTarget()) {
    ...
    m_pPredistortRT.Init(pMaterialSystem->CreateNamedRenderTargetTextureEx2(
        "_rt_vr_predistort", nWidth, nHeight, RT_SIZE_LITERAL,
        IMAGE_FORMAT_RGBA8888, MATERIAL_RT_DEPTH_SEPARATE,
    ...
    m_pPredistortRTDepth.Init(
        pMaterialSystem->CreateNamedRenderTargetTextureEx2(
            "_rt_vr_predistort_depth", nWidth, nHeight, RT_SIZE_LITERAL,
            IMAGE_FORMAT_NV_DST24, MATERIAL_RT_DEPTH_NONE,
...
void CSourceVirtualReality::GetRenderTargetFrameBufferDimensions(int &nWidth,
                                                                 int &nHeight) {
...
// --------------------------------------------------------------------
ITexture *CSourceVirtualReality::GetRenderTarget(
    ISourceVirtualReality::VREye eEye,
    ISourceVirtualReality::EWhichRenderTarget eWhich) {
...
// Purpose: Let the caller know if we're in VR mode
// --------------------------------------------------------------------
void CSourceVirtualReality::GetViewportBounds(VREye eEye, int *pnX, int *pnY,
...
// ----------------------------------------------------------------------
// Purpose: Create a 4x4 projection transform from eye projection and distortion
// parameters
// ----------------------------------------------------------------------
inline static void ComposeProjectionTransform(float fLeft, float fRight,
...
// ----------------------------------------------------------------------
bool CSourceVirtualReality::GetEyeProjectionMatrix(VMatrix *pResult, VREye eEye,
                                                   float zNear, float zFar,
                                                   float fovScale) {
  Assert(pResult != NULL);
  ...
  m_pHmd->GetProjectionRaw(SourceEyeToHmdEye(eEye), &fLeft, &fRight, &fTop,
                           &fBottom);

  ComposeProjectionTransform(fLeft, fRight, fTop, fBottom, zNear, zFar,
                             fovScale, pResult);
  return true;
}
...
VMatrix CSourceVirtualReality::GetMidEyeFromEye(VREye eEye) {
  if (m_pHmd) {
    vr::HmdMatrix34_t matMidEyeFromEye =
        m_pHmd->GetEyeToHeadTransform(SourceEyeToHmdEye(eEye));
...
bool CSourceVirtualReality::WillDriftInYaw() {
  if (m_pHmd)
    return m_pHmd->GetBoolTrackedDeviceProperty(vr::k_unTrackedDeviceIndex_Hmd,
...
bool CSourceVirtualReality::SampleTrackingState(float PlayerGameFov,
                                                float fPredictionSeconds) {
  if (!m_pHmd || !m_bActive)
    return false;
  ...
  if (m_pHmd->IsTrackedDeviceConnected(k_unTrackedDeviceIndex_Hmd)) {
    ...
    float fFrameDuration = 1.f / m_pHmd->GetFloatTrackedDeviceProperty(
                                     vr::k_unTrackedDeviceIndex_Hmd,
                                     vr::Prop_DisplayFrequency_Float);
    float fPredictedSecondsFromNow =
        fFrameDuration - fSecondsSinceLastVsync +
        m_pHmd->GetFloatTrackedDeviceProperty(
            vr::k_unTrackedDeviceIndex_Hmd,
            vr::Prop_SecondsFromVsyncToPhotons_Float);

    // Use Seated here because everything using this interface or older is
    // expecting a seated experience
    m_pHmd->GetDeviceToAbsoluteTrackingPose(vr::TrackingUniverseSeated,
                                            fPredictedSecondsFromNow, &pose, 1);
  ...
  m_ZeroFromHeadPose = OpenVRToSourceCoordinateSystem(
      VMatrixFrom34(pose.mDeviceToAbsoluteTracking.m));
...
bool CSourceVirtualReality::DoDistortionProcessing(VREye eEye) {
  ...
  pRenderContext->DrawScreenSpaceRectangle(
      pDistortMaterial, x, y, w, h, 0, 0, distortionTextureSize - 1,
      distortionTextureSize - 1, distortionTextureSize, distortionTextureSize);
...
// --------------------------------------------------------------------
bool CSourceVirtualReality::CompositeHud(VREye eEye, float ndcHudBounds[4],
  ...
  IMaterial *pDistortHUDMaterial = (eEye == VREye_Left)
                                       ? m_DistortHUDLeftMaterial
                                       : m_DistortHUDRightMaterial;

  // The translucency flag will enable/disable both blending and alpha test. The
  // only case where we don't want them enabled is when we're blacking out the
  // entire screen (we use blending to smooth the edges of the HUD, and we use
  // alpha test to kill the pixels outside the HUD).  Note that right now I'm
  // not expecting to see a mode with bTranslucent and bBlackout both true
  // (maybe happens in sniper mode?).
  pDistortHUDMaterial->SetMaterialVarFlag(MATERIAL_VAR_TRANSLUCENT, !bBlackout);

  // The ndcHudBounds are the min x, min y, max x, max y of where we want to
  // paste the HUD texture in NDC coordinates of the main 3D view.  We conver to
  // UV (0->1) space here for the shader.
  float huduvs[4];
  ...
  // Fix up coordinates depending on whether we're rendering to a buffer sized
  // for one eye or two. (note that disabling distortion also disables use of
  ...
  pRenderContext->DrawScreenSpaceRectangle(
      pDistortHUDMaterial, x, y, w, h, 0, 0, distortionTextureSize - 1,
      distortionTextureSize - 1, distortionTextureSize, distortionTextureSize);
...
static VMatrix OpenVRToSourceCoordinateSystem(const VMatrix &vortex) {
  ...
  //              0 0 0 1];
  //
  // We want to compute vmatrix = s_from_v * vortex * v_from_s; v_from_s =
  // s_from_v' Given vortex = [00    01    02    03
  //  10    11    12    13
  //  20    21    22    23
  //  30    31    32    33]
  //
  ...
  VMatrix result(v[2][2], v[2][0], -v[2][1], -v[2][3] * inchesPerMeter, v[0][2],
                 v[0][0], -v[0][1], -v[0][3] * inchesPerMeter, -v[1][2],
                 -v[1][0], v[1][1], v[1][3] * inchesPerMeter, -v[3][2],
                 -v[3][0], v[3][1], v[3][3]);
...
static VMatrix VMatrixFrom44(const float v[4][4]) {
  return VMatrix(v[0][0], v[0][1], v[0][2], v[0][3], v[1][0], v[1][1], v[1][2],
                 v[1][3], v[2][0], v[2][1], v[2][2], v[2][3], v[3][0], v[3][1],
                 v[3][2], v[3][3]);
}

static VMatrix VMatrixFrom34(const float v[3][4]) {
  return VMatrix(v[0][0], v[0][1], v[0][2], v[0][3], v[1][0], v[1][1], v[1][2],
                 v[1][3], v[2][0], v[2][1], v[2][2], v[2][3], 0, 0, 0, 1);
}

static VMatrix VMatrixFrom33(const float v[3][3]) {
  return VMatrix(v[0][0], v[0][1], v[0][2], 0, v[1][0], v[1][1], v[1][2], 0,
