...
void CImportSFMV1::ConvertTimeAttribute(CDmElement *pElementInternal,
  ...
  if (!pOldAttr) {
    ...
    Warning("*** Problem in file encountered!\n");
    Warning("*** TimeFrame \"%s\" has attribute \"%s\" with an unexpected type "
            "(expected float)!\n",
            pElementInternal->GetName(), pOldName);
  } else {
  ...
  if (!pNewAttr) {
    Assert(0);
    Warning("*** Converter error - element \"%s\" already has a non-int "
            "attribute \"%s\"!\n",
            pElementInternal->GetName(), pNewName);
    return;
  }
...
void CImportSFMV1::FixupLog(CDmElement *pElementInternal) {
  ...
  if (pAttr->GetType() != AT_INT_ARRAY) {
    Warning("*** Problem in file encountered!\n");
    Warning("*** Log \"%s\" has attribute \"%s\" with an unexpected type "
            "(expected int array)!\n",
            pElementInternal->GetName(), "times");
    return;
  }
