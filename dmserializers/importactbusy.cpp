...
void CImportActBusy::SerializeHeader(CUtlBuffer &buf) {
  buf.Printf("// \"act busy name\"\t\tThis is the name that a mapmaker must "
             "specify in the hint node.\n");
  buf.Printf("// {\n");
  buf.Printf("// \t\"busy_anim\"\t\t\t\"Activity Name\".\n");
  buf.Printf("// \t\"entry_anim\"\t\t\"Activity Name\"\n");
  buf.Printf("// \t\"exit_anim\"\t\t\t\"Activity Name\"\n");
  buf.Printf("// \t\"busy_sequence\"\t\t\"Sequence Name\". If specified, this "
             "is used over the activity name. Specify it in the hint node.\n");
  buf.Printf("// \t\"entry_sequence\"\t\"Sequence Name\". If specified, this "
             "is used over the entry anim.\n");
  buf.Printf("// \t\"exit_sequence\"\t\t\"Sequence Name\". If specified, this "
             "is used over the exit anim.\n");
  buf.Printf(
      "// \t\"min_time\"\t\t\t\"Minimum time to spend in this busy anim\"\n");
  buf.Printf("// \t\"max_time\"\t\t\t\"Maximum time to spend in this busy "
             "anim\"	0 = only stop when interrupted by external event\n");
  buf.Printf("// \t\"interrupts\"\t\tOne of:\n");
  buf.Printf("// \t\t\t\t\t\t\"BA_INT_NONE\"\t\tbreak out only when time runs "
             "out. No external influence will break me out.\n");
  buf.Printf("// \t\t\t\t\t\t\"BA_INT_DANGER\"\t\tbreak out of this anim only "
             "if threatened\n");
  buf.Printf("// \t\t\t\t\t\t\"BA_INT_PLAYER\"\t\tbreak out of this anim if I "
             "can see the player, or I'm threatened\n");
  buf.Printf("// \t\t\t\t\t\t\"BA_INT_AMBUSH\"\t\tsomeone please define this - "
             "I have no idea what it does\n");
  buf.Printf("// \t\t\t\t\t\t\"BA_INT_COMBAT\"\t\tbreak out of this anim if "
             "combat occurs in my line of sight (bullet hits, grenades, etc), "
             "-OR- the max time is reached\n");
  buf.Printf("// }\n");
  buf.Printf("//\n");
}
