...
//
template <typename T> class CLessSimple {
public:
...
//-- compute new pointers/offsets that will happen after insertions 		MemMove
...
public:
  ...
  template <typename T> void InsertElements(T *ptr, int count = 1) {
...
//schedule removals 		Finalize
//-- finalize removal information 		ComputePointer / ComputeOffset	[*]
//-- compute new pointers/offsets that will happen after removals 		MemMove
...
public:
  ...
  template <typename T> void RemoveElements(T *ptr, int count = 1) {
...
//
template <typename T> class CGrowableVector : public CUtlVector<T> {
public:
  T &operator[](int idx) {
    while (idx >= Count())
      AddToTail(T());
