...
bool CMdlStripInfo::Serialize(CUtlBuffer &bufStorage) const {
  ...
  case MODE_STRIP_LOD_1N:
    ...
    for (uint32 const *pdwBase = m_vtxVerts.Base(),
    ...
    for (MdlRangeItem const *pmri = m_vtxMdlOffsets.Base(),
      ...
      bufStorage.PutInt(pmri->m_offOld), bufStorage.PutInt(pmri->m_offNew),
          bufStorage.PutInt(pmri->m_numOld), bufStorage.PutInt(pmri->m_numNew);
...
bool CMdlStripInfo::UnSerialize(CUtlBuffer &bufData) {
  ...
  case MODE_STRIP_LOD_1N:
    ...
    for (uint32 *pdwBase = m_vtxVerts.Base(),
                *pdwEnd = pdwBase + m_vtxVerts.GetNumDWords();
         pdwBase < pdwEnd; ++pdwBase)
...
//the original model submitted for stripping 	plChecksumStripped		if
//non-NULL will hold the resulting checksum of the stripped model
...
bool CMdlStripInfo::StripVertexDataBuffer(CUtlBuffer &vvdBuffer) {
  ...
  vvdHdr->vertexDataStart -=
      ALIGN_VALUE(sizeof(vertexFileFixup_t) * vvdHdr->numFixups, 16);
  vvdHdr->numFixups = 0;
  DECLARE_PTR(mstudiovertex_t, vvdVertexNew,
              BYTE_OFF_PTR(vvdHdr, vvdHdr->vertexDataStart));
