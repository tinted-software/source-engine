...
void CRedirect::Run(LPCTSTR szCommand, CEdit *pEdit,
  ...
  for (;;) {
    ...
    //------------------------------------------------------------------
    if (!m_hThread || MsgWaitForMultipleObjects(1, &m_hThread, FALSE, 20,
...
void CRedirect::PeekAndPump() {
  ...
  while (::PeekMessage(&Msg, NULL, 0, 0, PM_NOREMOVE)) {
    (void)AfxGetApp()->PumpMessage(); // lint !e1924 (warning about C-style
                                      // cast)
  }
}

void CRedirect::Stop() { m_bStopped = true; }
...
void CRedirect::WriteStdOut(LPCSTR pszOutput) { AppendText(pszOutput); }
