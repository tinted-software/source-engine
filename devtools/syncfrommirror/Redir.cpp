...
CRedirector::CRedirector()
    : m_hStdinWrite(NULL), m_hStdoutRead(NULL), m_hChildProcess(NULL),
      m_hThread(NULL), m_hEvtStop(NULL), m_dwThreadId(0), m_dwWaitTime(1000) {}
...
BOOL CRedirector::Open(LPCTSTR pszCmdLine, LPCTSTR pszCurrentDirectory) {
  ...
  __try {
    ...
    if (!::DuplicateHandle(::GetCurrentProcess(), hStdoutReadTmp,
                           ::GetCurrentProcess(), &m_hStdoutRead, 0,
                           FALSE, // make it uninheritable.
                           DUPLICATE_SAME_ACCESS))
      __leave;

    if (!::DuplicateHandle(::GetCurrentProcess(), hStdinWriteTmp,
    ...
    if (!LaunchChild(pszCmdLine, pszCurrentDirectory, hStdoutWrite, hStdinRead,
                     hStderrWrite))
      __leave;
...
BOOL CRedirector::Printf(LPCTSTR pszFormat, ...) {
  ...
  DWORD dwWritten;
  return ::WriteFile(m_hStdinWrite, (LPCTSTR)strInput, strInput.GetLength(),
                     &dwWritten, NULL);
}

BOOL CRedirector::LaunchChild(LPCTSTR pszCmdLine, LPCTSTR pszCurrentDirectory,
  ...
  if (!::CreateProcess(NULL, (LPTSTR)pszCmdLine, NULL, NULL, TRUE,
                       CREATE_NEW_CONSOLE, NULL, pszCurrentDirectory, &si, &pi))
    return FALSE;
...
int CRedirector::RedirectStdout() {
  ...
  for (;;) {
    DWORD dwAvail = 0;
    if (!::PeekNamedPipe(m_hStdoutRead, NULL, 0, NULL, &dwAvail, NULL)) // error
      break;
    ...
    DWORD dwRead = 0;
    if (!::ReadFile(m_hStdoutRead, szOutput, min(16 * 1024, dwAvail), &dwRead,
                    NULL) ||
...
DWORD WINAPI CRedirector::OutputThread(LPVOID lpvThreadParam) {
  ...
  for (;;) {
    ...
    DWORD dwRc =
        ::WaitForMultipleObjects(3, aHandles, FALSE, pRedir->m_dwWaitTime);
    ...
    // If we don't sleep here, then syncfrommirror will eat lots of CPU looping
    // here.
    Sleep(20);
  }
