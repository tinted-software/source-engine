...
BOOL CSyncfrommirrorDlg::OnInitDialog() {
  ...
  m_bMakeCommandPrompts =
      AfxGetApp()->GetProfileInt("settings", "makecommandprompts", 0);
...
HCURSOR CSyncfrommirrorDlg::OnQueryDragIcon() { return (HCURSOR)m_hIcon; }

void CSyncfrommirrorDlg::RunSync(const char *pszType) {
  static const char *pszCmdLine =
      "cmd.exe /c "
      "\\\\hl2vss.valvesoftware.com\\hl2vss\\win32\\syncfrommirror.bat %s %s";
...
void CSyncfrommirrorDlg::RunCommandLine(const char *pCmdLine,
  ...
  CreateProcess(NULL, (char *)pCmdLine, NULL, NULL, FALSE, CREATE_NEW_CONSOLE,
...
void CSyncfrommirrorDlg::OnOK() {
  ...
  AfxGetApp()->WriteProfileInt("settings", "makecommandprompts",
                               m_bMakeCommandPrompts);
  AfxGetApp()->WriteProfileString("settings", "working_folder",
                                  m_WorkingFolder);

  if (m_WorkingFolder.GetLength() == 0) {
    MessageBox("You must set your working folder (e.g. u:\\dev)", "Error",
               MB_ICONWARNING);
    return;
  }
...
void CSyncfrommirrorDlg::OnStop() { m_Redirector.Stop(); }
...
void CSyncfrommirrorDlg::OnSize(UINT nType, int cx, int cy) {
  if (GetDlgItem(IDC_EDIT1)) {
    ...
    GetDlgItem(IDC_EDIT1)->MoveWindow(
        ...
        (rectDlg.right - m_nEditRightMargin) - rectEdit.left,
        (rectDlg.bottom - m_nEditBottomMargin) - rectEdit.top, TRUE);

    CRect rectButton;
    GetDlgItem(IDOK)->GetWindowRect(&rectButton);
    ScreenToClient(rectButton);
    GetDlgItem(IDOK)->MoveWindow(rectDlg.right - m_nButtonOffset,
                                 rectButton.top, rectButton.Width(),
                                 rectButton.Height(), TRUE);

    GetDlgItem(IDCANCEL)->GetWindowRect(&rectButton);
    ScreenToClient(rectButton);
    GetDlgItem(IDCANCEL)->MoveWindow(rectDlg.right - m_nButtonOffset,
                                     rectButton.top, rectButton.Width(),
                                     rectButton.Height(), TRUE);
    ...
    GetDlgItem(IDC_BUTTON1)
        ->MoveWindow(rectDlg.right - m_nButtonOffset, rectButton.top,
