...
//
// Purpose: Checks to see if the user has completely viewed the EULA before
// activing the accept/decline radio buttons
//
//=============================================================================//
#include < msi.h >
#include < msiquery.h >
...
BOOL CALLBACK EnumChildWindowsProc(HWND hwnd, LPARAM lParam) {
  ...
  EnumChildWindows(hwnd, EnumChildrenLookingForSpecialControl,
                   (LPARAM)&special);
  if (special.wnd != NULL) {
    p->wnd = hwnd;
    return FALSE;
...
//-----------------------------------------------------------------------------
// Purpose: Finds given a root window, finds a child window which itself has a
// special child window with the specified text string as the window title.
//  e.g.:
// if root == NULL (desktop), then you can search the top level dialogs
// (Half-Life 2 Setup,e.g.) for a subwindow with "vHackxxx" as the text, this
// would
...
//**---------------------------------------------------------
//** Usage:
//** 	(1) Find installer window by looking for child with specified text.
//Can't
//**		just use window title because it's translated into foreign
//languages
...
extern "C" __declspec(dllexport) UINT CheckLicenseViewed(MSIHANDLE hMSI) {
  ...
  if (MsiEvaluateCondition(hMSI, "LicenseViewed") == MSICONDITION_FALSE) {
    hWnd =
        FindTopLevelWindowHavingChildWithSpecifiedText(CHILD_WINDOW_OF_LICENSE);
    if (hWnd) {
      ...
      if (hWndChild) {
        ...
        // THIS IS A HACK, but the RichEdit20W control has a bug where while the
        // thumb is being dragged, the position doesn't
        //  get updated.  In fact, it doesn't get updated until the next time
        UINT nScrollPos = ::SendMessage(hWndChild, EM_GETTHUMB, 0, 0);

        bool trackedPastEnd =
            false; // (UINT)sinfo.nTrackPos >= MaxScrollPos ? true : false;
        bool positionedPastEnd = nScrollPos >= MaxScrollPos ? true : false;
        // Note above, this method doesn't always work... but maybe it will work
        // in win98 and the other won't, etc. etc.
        bool positionedPastEnd2 =
            (UINT)sinfo.nPos >= MaxScrollPos ? true : false;
        ...
        char sz[ 256 ];
        _snprintf( sz, sizeof( sz ), "max %i page %i msp %i track %u pos %u pos2
        %u", sinfo.nMax, sinfo.nPage, MaxScrollPos, sinfo.nTrackPos, sinfo.nPos,
                nScrollPos );
