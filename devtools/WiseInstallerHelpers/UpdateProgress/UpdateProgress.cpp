...
//
// Purpose: A simple app which looks for the HL2 wise installer and ticks the
// progress bar due
//  to a bug with installing more than 2GB of data using the current ver of the
//  windows installer
//
//=============================================================================//
...
#define FIND_WINDOW_TEXT_CHANGEDISKDIALOG                                      \
  "vHackWiseProgressDialogChangeCD092304"
...
// The full bar is this many ticks (which are about 100 msec apart, so 30
// seconds to walk bar
...
void SearchForWindow() {
  HWND hProgressDialog = FindTopLevelWindowHavingChildWithSpecifiedText(
      FIND_WINDOW_TEXT_PROGRESSDIALOG);
  if (!hProgressDialog) {
    return;
  }
  ...
  EnumChildWindows(hProgressDialog, EnumFindProgressBarInDialog,
                   (LPARAM)&hWndChild);
  if (!hWndChild)
    return;
...
void DrawProgressBar(HDC dc, bool showTicks, float frac) {
...

  {
    ...
    if (showTicks) {
      HRGN clipRegion =
          (HRGN)CreateRectRgn(rc.left + 1, rc.top, rc.right - 1, rc.bottom);
      ;
...
//-----------------------------------------------------------------------------
void UpdateProgress() {
  // If the "Insert next CD" or "Exit Setup" dialogs are showing, stop advancing
  // the progress bar
  HWND hSwapDiskDialog = FindWindowHavingChildWithSpecifiedText(
      GetDesktopWindow(), FIND_WINDOW_TEXT_CHANGEDISKDIALOG);
  HWND hCancelDialog = FindWindowHavingChildWithSpecifiedText(
      GetDesktopWindow(), FIND_WINDOW_TEXT_CANCELDIALOG);
...
//	The app wil also quit if the dialog was not found after waiting 60
//seconds
//-----------------------------------------------------------------------------
void Think() {
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
  ...
  hwnd = CreateWindow(szAppName, TEXT(""), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT,
