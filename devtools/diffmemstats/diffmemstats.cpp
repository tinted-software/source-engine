...
// DiffMemStats.cpp : processes two "memstats<n>.txt" memory dumps from the game
// into a 'diff' text file
//                    which Excel will display in legible form (useful for
//                    tracking mem leaks)
...
void Usage() {
  ...
  printf("\n");
  printf("Input is a sequence of memstats<n>.txt files (saved from game using "
         "'mem_dump')\n");
  printf("and output is a single tab-separated text file, where each line "
         "represents a\n");
  printf("given allocation's size as it varies over time through the memstats "
         "sequence\n");
  printf("(lines are sorted by maximum change over time - see sortend/sortmax "
         "options).\n");
  printf("This text file can then be graphed in Excel using a 'stacked column' "
         "chart.\n");
  printf("\n");
  printf("NOTE: input files must be in chronological order, from a SINGLE play "
         "session\n");
  ...
  printf("[-skipchains:M]        skip the top M sequences before output "
         "(default: 0)\n");
  printf("[-delta]               output deltas between adjacent values in each "
         "sequence\n");
  printf("                       (the first delta for each sequence will "
         "always be zero)\n");
  printf("[-absolute]            output absolute values (default is to "
         "subtract out the\n");
  printf("                       first value in each sequence), overridden by "
         "'-delta'\n");
  printf("[-sortend]             sort sequences by start-to-end change "
         "(default)\n");
  printf(
      "[-sortmax]             sort sequences by start-to-max-value change\n");
  printf("[-sortabs]             sort by absolute change values\n");
  printf("[-allowmismatch]       don't check that the input file sequence is "
         "in\n");
  printf("                       chronological order and from the same play "
         "session\n");
...
int _tmain(int argc, _TCHAR *argv[]) {
  ...
  if (!isValidSequence && !gAllowArbitraryInputSequence) {
    printf("ERROR: input files did not all come from the same play session, or "
