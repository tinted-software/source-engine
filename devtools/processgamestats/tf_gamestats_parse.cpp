...
void InsertTF2Data(bool bDeathOnly, char const *szStatsFileUserID,
  ...
  for (iMap = g_reportedStats.m_dictMapStats.First();
    ...
    Q_snprintf(
        ...
        "(ServerID,TimeSubmitted,MapName,RoundsPlayed,TotalTime,BlueWins,"
        "RedWins,Stalemates,BlueSuddenDeathWins,RedSuddenDeathWins) "
        "values (\"%s\",%s,\"%s\",%d,%d,%d,%d,%d,%d,%d);",
        szStatsFileUserID, szDate, mapname.c_str(),
        pCurrentMap->m_Header.m_iRoundsPlayed,
    ...
    for (int i = TF_CLASS_UNDEFINED + 1; i < TF_CLASS_CIVILIAN; i++) {
      ...
      Q_snprintf(q, sizeof(q),
                 "INSERT into tf_classdata "
                 "(ServerID,MapName,TimeSubmitted,Class,Spawns,TotalTime,Score,"
                 "Kills,Deaths,Assists,Captures) "
                 "values (\"%s\",\"%s\",%s,%d,%d,%d,%d,%d,%d,%d,%d);",
                 szStatsFileUserID, mapname.c_str(), szDate, i,
                 classStats.iSpawns, classStats.iTotalTime, classStats.iScore,
                 classStats.iKills, classStats.iDeaths, classStats.iAssists,
                 classStats.iCaptures);
    ...
    for (int i = TF_WEAPON_NONE + 1; i < TF_WEAPON_COUNT; i++) {
      ...
      Q_snprintf(q, sizeof(q),
                 "INSERT into tf_weapondata (ServerID, MapName, "
                 "TimeSubmitted,WeaponID,ShotsFired,ShotsFiredCrit,ShotsHit,"
                 "DamageTotal,"
                 "HitsWithKnownDistance,DistanceTotal) "
                 "values (\"%s\",\"%s\",%s,%d,%d,%d,%d,%d,%d,%llu)",
                 szStatsFileUserID, mapname.c_str(), szDate, i,
                 weaponStats.iShotsFired, weaponStats.iCritShotsFired,
                 weaponStats.iHits, weaponStats.iTotalDamage,
                 weaponStats.iHitsWithKnownDistance,
...
int TF_ParseCustomGameStatsData(ParseContext_t *ctx) {
  ...
  if (memcmp(statsBuffer.Base(), "PERFDATA:", 9) == 0) {
    ProcessPerfData(ctx->mysql, sb.st_mtime, "tf_perfdata",
