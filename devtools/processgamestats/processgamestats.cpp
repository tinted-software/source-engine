...
// roll-our-own symbol table class.  Note we don't use CUtlSymbolTable because
// that and related classes have short int deeply baked in as index type, so can
// only hold 64K entries.  We sometimes need to process more than 64K files at a
// time.
struct AnalysisData {
  AnalysisData() { symbols.SetLessFunc(CaselessStringLessThanIgnoreSlashes); }
...
void BuildFileList_R(CUtlVector<int> &files, char const *dir,
...
void BuildFileList(CUtlVector<int> &files, char const *rootdir,
                   char const *extension) {
  files.RemoveAll();
...
int main(int argc, char *argv[]) {
  ...
  if (bSqlOkay || describeonly) {
    ...
    if (ctx.skipcount > 0) {
      printf("Skipped %d samples which appear to be malformed or contain bogus "
             "data\n",
             ctx.skipcount);
    }
...
void InsertKeyDataIntoTable(IMySQL *pSQL, time_t fileTime,
                            char const *pTableName, char const *pPerfData,
                            char const *pKeyWhiteList[], int nNumFields) {
  char szDate[128] = "Now()";
  if (fileTime > 0) {
...
void ProcessPerfData(IMySQL *pSQL, time_t fileTime, char const *pTableName,
                     char const *pPerfData) {
  InsertKeyDataIntoTable(pSQL, fileTime, pTableName, pPerfData, s_PerfKeyList,
