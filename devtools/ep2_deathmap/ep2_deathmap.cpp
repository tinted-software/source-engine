...
bool ImageToJPEGBuffer(Image_t *image, CUtlBuffer &buf, int quality) {
  ...
  // IMAGE_FORMAT_RGB888 ); Copy data in
...
void BuildAggregateStats(IMySQL *mysql, char const *pszMapName,
  ...
  Q_snprintf(q, sizeof(q),
             ...
             "from %s_maps where MapName = \'%s\' %s and Seconds < 15000;",
             gamename, mapname.c_str(), whereClause);
  ...
  Q_snprintf(q, sizeof(q),
             ...
             "Sum(KilledPlayer), avg(KilledPlayer) from %s_entities where "
  ...
  Q_snprintf(q, sizeof(q),
             ...
             "Sum(Damage), Avg(Damage) from %s_weapons where Damage < 100000 "
             "and mapname = \'%s\' %s group by Weapon;",
             gamename, mapname.c_str(), whereClause);
  ...
  // Now show weapon stats
  Q_snprintf(q, sizeof(q),
             "select count(*)/count(distinct(userid) ) from %s_saves where "
             "mapname = \'%s\' %s;",
             gamename, mapname.c_str(), whereClause);
  ...
  Q_snprintf(q, sizeof(q),
             ...
             "from %s_counters where mapname = \'%s\' %s;",
             gamename, mapname.c_str(), whereClause);
...
#define INCHES_TO_MILES (1.0 / (5280.0 * 12.0))
  ...
  Q_snprintf(q, sizeof(q),
             ...
             "from %s_generic where mapname = \'%s\' %s group by StatName;",
             gamename, mapname.c_str(), whereClause);
...
int main(int argc, char *argv[]) {
  ...
  for (KeyValues *map = kv->GetFirstSubKey(); map; map = map->GetNextKey()) {
    ...
    if (bBuildImages) {
      ...
      char outfile[512];
      Q_snprintf(outfile, sizeof(outfile), "%s/%s_deaths.jpg", pathname,
                 pszMapName);
      ...
      if (ReadBitmapRGB(buf, size, &image)) {
        ...
        if (bBugSpots) {
          ...
          if (bStressTest) {
            ...
            Q_snprintf(q, sizeof(q),
                       "select x, y from %s_deaths where MapName = \'%s\' %s;",
                       gamename, mapname.c_str(), whereclause);
