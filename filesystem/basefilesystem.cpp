...
static void LogFileOpen(const char *vpk, const char *pFilename,
                        const char *pAbsPath) {
  ...
  if (f) {
    ...
    // If this was the first time, switch from write to append for further
    // writes
    mode = "at";
  }
}
...
public:
  ...
  virtual int First() const {
    if (!m_bLocked) {
      Error("CBlockingFileItemList::First() w/o calling "
  ...
  virtual int Next(int i) const {
    if (!m_bLocked) {
      Error("CBlockingFileItemList::Next() w/o calling "
            "EnterCriticalSectionFirst!");
    }
  ...
  virtual const FileBlockingItem &Get(int index) const {
    if (!m_bLocked) {
      Error("CBlockingFileItemList::Get( %d ) w/o calling "
            "EnterCriticalSectionFirst!",
            index);
    }
  ...
  virtual void Reset() {
    if (!m_bLocked) {
      Error("CBlockingFileItemList::Reset() w/o calling "
            "EnterCriticalSectionFirst!");
    }
    m_Items.RemoveAll();
  }
...
public:
  ...
  static void IOCallback(const FileAsyncRequest_t &request, int nBytesRead,
                         FSAsyncStatus_t err);
...
void CBaseFileSystem::Shutdown() {
  ...
  if (m_pLogFile) {
    if (CommandLine()->FindParm("-fs_logbins") >= 0) {
      ...
      fprintf(m_pLogFile, "copy \"%%binsrc%%\\bin\\materialsystem.dll\" "
                          "\"%%fs_target%%\\bin\"\n");
      fprintf(m_pLogFile, "copy \"%%binsrc%%\\bin\\shaderapidx9.dll\" "
                          "\"%%fs_target%%\\bin\"\n");
      fprintf(m_pLogFile, "copy \"%%binsrc%%\\bin\\filesystem_stdio.dll\" "
                          "\"%%fs_target%%\\bin\"\n");
      fprintf(m_pLogFile, "copy \"%%binsrc%%\\bin\\soundemittersystem.dll\" "
                          "\"%%fs_target%%\\bin\"\n");
      fprintf(
          m_pLogFile,
      ...
      fprintf(m_pLogFile, "copy \"%%binsrc%%\\bin\\vguimatsurface.dll\" "
                          "\"%%fs_target%%\\bin\"\n");
      fprintf(
          m_pLogFile,
      ...
      fprintf(m_pLogFile, "copy \"%%binsrc%%\\bin\\studiorender.dll\" "
                          "\"%%fs_target%%\\bin\"\n");
      fprintf(m_pLogFile, "copy \"%%binsrc%%\\bin\\vaudio_miles.dll\" "
                          "\"%%fs_target%%\\bin\"\n");
      fprintf(m_pLogFile, "copy \"%%binsrc%%\\hl2\\resource\\*.ttf\" "
                          "\"%%fs_target%%\\hl2\\resource\"\n");
      fprintf(m_pLogFile, "copy \"%%binsrc%%\\hl2\\bin\\gameui.dll\" "
                          "\"%%fs_target%%\\hl2\\bin\"\n");
    }
    ...
    fprintf(m_pLogFile, "echo "
                        "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
                        "!!!!!!!!!!!!!!!\"\n");
    fprintf(m_pLogFile, "echo ERROR: must set fs_target=targetpath (ie. \"set "
                        "fs_target=u:\\destdir\")!\n");
    fprintf(m_pLogFile, "echo "
                        "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
                        "!!!!!!!!!!!!!!!\"\n");
...
void CBaseFileSystem::LogAccessToFile(char const *accesstype,
  ...
  if (m_fwLevel >= FILESYSTEM_WARNING_REPORTALLACCESSES &&
      !V_stristr(fullpath, "console.log")) {
...
// Output : FILE
//-----------------------------------------------------------------------------
FILE *CBaseFileSystem::Trace_FOpen(const char *filenameT, const char *options,
                                   unsigned flags, int64 *size) {
  AUTOBLOCKREPORTER_FN(Trace_FOpen, this, true, filenameT,
...
void CBaseFileSystem::Trace_FClose(FILE *fp) {
  if (fp) {
    ...
    if (result != -1 /*m_OpenedFiles.InvalidIdx()*/) {
      COpenedFile found = m_OpenedFiles[result];
      if (m_fwLevel >= FILESYSTEM_WARNING_REPORTALLACCESSES &&
          !V_stristr(found.GetName(), "console.log")) {
...
void CBaseFileSystem::Trace_DumpUnclosedFiles(void) {
  ...
  for ( int i = 0 ; i < m_OpenedFiles.Count(); i++ )
  {
          ...
          {
                  // This warning can legitimately trigger because the log file
  is, by design, not
                  // closed. It is not closed at shutdown in order to avoid race
  conditions. It is
                  // not closed at each call to log information because the
  performance costs,
                  // especially if Microsoft Security Essentials is running, are
  extreme.
                  // In addition, when this warning triggers it can, due to the
  state of the process,
                  // actually cause a crash.
                  Warning( FILESYSTEM_WARNING_REPORTUNCLOSED, "File %s was never
  closed\n", found->GetName() );
          }
  }
  */
...
CPackedStoreRefCount::CPackedStoreRefCount(char const *pFileBasename,
                                           char *pszFName, IBaseFileSystem *pFS)
    : CPackedStore(pFileBasename, pszFName, pFS, false) {
// If the VPK is signed, check the signature
//
// !FIXME! This code is simple a linchpin that a hacker could detour to bypass
// sv_pure
#ifdef VPK_ENABLE_SIGNING
  CPackedStore::ESignatureCheckResult eSigResult =
      CPackedStore::CheckSignature(0, NULL);
  m_bSignatureValid =
      (eSigResult == CPackedStore::eSignatureCheckResult_ValidSignature);
#else
...
void CBaseFileSystem::AddVPKFile(char const *pPath, const char *pPathID,
  ...
  if (pVPK == NULL) {
    ...
    pVPK->m_PackFileID =
        m_FileTracker2.NotePackFileOpened(pVPK->FullPathName(), pPathID, 0);
  } else {
    pVPK->AddRef();
  }
  ...
  CSearchPath *sp =
      &m_SearchPaths[(addType == PATH_ADD_TO_TAIL) ? m_SearchPaths.AddToTail()
                                                   : m_SearchPaths.AddToHead()];
...
void CBaseFileSystem::AddPackFiles(const char *pPath, const CUtlSymbol &pathID,
...
#if defined(_X360)
  // localized zips are added last to ensure they appear first in the search
  // path construction localized zips can only appear in the game or mod
  // directories
  ...
  if (XBX_IsLocalized() && (bUseEnglishAudio == false) &&
    ...
    for (int i = 0;; i++) {
      char pakfile[MAX_PATH];
      char fullpath[MAX_PATH];
      V_snprintf(pakfile, sizeof(pakfile), PACK_LOCALIZED_NAME_FORMAT, i,
    ...
    if (!pf) {
      ...
      if (pf->m_hPackFileHandleFS) {
        ...
        if (pf->Prepare(pakSizes[i])) {
          FS_setbufsize(pf->m_hPackFileHandleFS,
                        filesystem_buffer_size.GetInt());
        } else {
          // Failed for some reason, ignore it
          if (pf->m_hPackFileHandleFS) {
            Trace_FClose(pf->m_hPackFileHandleFS);
...
void CBaseFileSystem::AddMapPackFile(const char *pPath, const char *pPathID,
  ...
  // Security nightmares already, should not let things explicitly loading from
  // e.g. "MOD" get surprise untrusted files unless you really really know what
  // you're doing.
  ...
  for (int i = c - 1; i >= 0; i--) {
    ...
    if (V_stricmp(m_SearchPaths[i].GetPackFile()->m_ZipName.Get(), fullpath) ==
        0) {
  ...
  int iStoreId =
      CRC32_ProcessSingleBuffer(fullpath, V_strlen(fullpath)) |
      0x80000000; // set store ID based on .bsp filename, so if we remount the
                  // same map file, it will have the same storeid
  ...
  FOR_EACH_VEC(m_ZipFiles, i) {
    ...
    if (pf && pf->m_bIsMapPath && pf->GetPath() == pathSymbol &&
        V_stricmp(pf->m_ZipName.Get(), fullpath) == 0) {
      CSearchPath *sp = &m_SearchPaths[(addType == PATH_ADD_TO_TAIL)
                                           ? m_SearchPaths.AddToTail()
                                           : m_SearchPaths.AddToHead()];
      pf->AddRef();
      sp->SetPackFile(pf);
...

  {
    ...
    if (packfile->filelen <= sizeof(lump_t)) {
      // It's empty or only contains a file header ( so there are no entries ),
      // so don't add to search paths
      Trace_FClose(fp);
      return;
    }
    ...
    for (int i = 0; i < c; i++) {
      ...
      if (pPackFile && pPackFile->m_bIsMapPath) {
...
#endif
        {
          ...
          //					{
          //						pPackFile->m_hPackFileHandleVPK
          //= FindFileInVPKs( pPackFile->m_ZipName );
          //					}
          // #endif
        }
...
void CBaseFileSystem::AddSearchPathInternal(const char *pPath,
  ...
  if (IsX360() && bAddPackFiles &&
      (!Q_stricmp(pathID, "DEFAULT_WRITE_PATH") ||
       !Q_stricmp(pathID, "LOGDIR"))) {
  ...
  if (IsPC() || !bAddPackFiles || !bAdded) {
    ...
    // 360 needs to find files (for the preload hit) in the zip first for fast
    // loading 360 always adds the non-pack search path *after* the pack file
    // but respects the overall list ordering
    if (addType == PATH_ADD_TO_HEAD) {
      m_SearchPaths.InsertBefore(nIndex);
    } else {
...
CBaseFileSystem::FindSearchPathByStoreId(int storeId) {
...
void CBaseFileSystem::AddSearchPath(const char *pPath, const char *pathID,
  ...
  if (IsX360() && m_DVDMode == DVDMODE_DEV) {
    // dvd development mode clones a search path based on the remote path for
    // fall through
    const char *pRemotePath = CommandLine()->ParmValue("-remote");
    const char *pBasePath = CommandLine()->ParmValue("-basedir");
    if (pRemotePath && pBasePath && !V_stristr(pPath, ".bsp")) {
      ...
      if (!V_strnicmp(pPath, pBasePath, strlen(pBasePath))) {
        ...
        V_strncat(szRemotePath, pPath + strlen(pBasePath),
                  sizeof(szRemotePath));
...
//-----------------------------------------------------------------------------
CBaseFileSystem::CSearchPath *
CBaseFileSystem::FindWritePath(const char *pFilename, const char *pathID) {
  ...
  for (int i = 0; i < c; i++) {
    ...
    if (IsX360() && (m_DVDMode == DVDMODE_DEV) && pFilename &&
        !pSearchPath->m_bIsRemotePath) {
      bool bIgnorePath = false;
      char szExcludePath[MAX_PATH];
      char szFilename[MAX_PATH];
      V_ComposeFileName(pSearchPath->GetPathString(), pFilename, szFilename,
                        sizeof(szFilename));
      for (int j = 0; j < m_ExcludePaths.Count(); j++) {
        if (g_pFullFileSystem->String(m_ExcludePaths[j], szExcludePath,
                                      sizeof(szExcludePath))) {
...
bool CBaseFileSystem::ReadToBuffer(FileHandle_t fp, CUtlBuffer &buf,
  ...
  if (nBytesToRead != 0) {
    ...
    nBytesRead = ReadEx(buf.PeekPut(), buf.Size() - buf.TellPut(),
...
bool CBaseFileSystem::UnzipFile(const char *pFileName, const char *pPath,
  ...
  while (1) {
    ...
    if (szFileName[iFileNameLength - 1] == '/') {
      ...
      Q_snprintf(szFinalName, sizeof(szFinalName), "%s%c%s", pDestination,
                 CORRECT_PATH_SEPARATOR, szFileName);
  ...
  while (1) {
    ...
    if (szFileName[iFileNameLength - 1] != '/') {
      ...
      if (pZip->ReadFileFromZip(hZipFile, szFileName, false, fileBuffer)) {
        ...
        // Make sure the directory actually exists in case the ZIP doesn't list
        // it (our zip utils create zips like this)
...
public:
  CFileOpenInfo(CBaseFileSystem *pFileSystem, const char *pFileName,
                ...
                int flags, char **ppszResolvedFilename)
      : m_pFileSystem(pFileSystem), m_pFileName(pFileName), m_pSearchPath(path),
        m_pOptions(pOptions), m_Flags(flags),
    ...
    if (m_pFileSystem->m_pPureServerWhitelist) {
      m_ePureFileClass =
          m_pFileSystem->m_pPureServerWhitelist->GetFileClass(pFileName);
    }
...
#ifdef SUPPORT_PACKED_STORE
      if (m_pVPKFile) {
        m_pFileSystem->m_FileTracker2.NotePackFileAccess(
            pRelativeFileName, m_pSearchPath->GetPathIDString(),
            m_pSearchPath->m_storeId, m_pFileHandle->m_VPKHandle);
        return;
      }
#endif
      // we always record hashes of everything we load. we may filter later.
      m_pFileSystem->m_FileTracker2.NoteFileLoadedFromDisk(
          pRelativeFileName, m_pSearchPath->GetPathIDString(),
...
#ifdef SUPPORT_PACKED_STORE
  void SetFromPackedStoredFileHandle(const CPackedStoreFileHandle &fHandle,
...
// Loading from VPK?
  ...
  openInfo.SetAbsolutePath("%s%s", openInfo.m_pSearchPath->GetPathString(),
                           szLowercaseFilename);
...
// Purpose:
//-----------------------------------------------------------------------------
FileHandle_t CBaseFileSystem::OpenForRead(const char *pFileNameT,
  ...
  if (!pathID || Q_stricmp(pathID, "GAME") == 0) {
    CMemoryFileBacking *pBacking = NULL;
    {
      ...
      CUtlHashtable<const char *, CMemoryFileBacking *> &table =
          m_MemoryFileHash;
    ...
    if (pBacking) {
      if (pBacking->m_nLength != -1) {
        ...
        pFile->m_type =
            strstr(pOptions, "b") ? FT_MEMORY_BINARY : FT_MEMORY_TEXT;
  ...
  CFileOpenInfo openInfo(this, pFileName, NULL, pOptions, flags,
  ...
  if (V_IsAbsolutePath(pFileName)) {
    ...
    // an absolute path can encode a zip pack file (i.e. caller wants to open
    // the file from within the pack file) format must strictly be ????.zip\????
    // assuming a reasonable restriction that the zip must be a pre-existing
    // search path zip
    char *pZipExt =
        V_stristr(openInfo.m_AbsolutePath, ".zip" CORRECT_PATH_SEPARATOR_S);
    if (!pZipExt)
      pZipExt =
          V_stristr(openInfo.m_AbsolutePath, ".bsp" CORRECT_PATH_SEPARATOR_S);
#if defined(SUPPORT_PACKED_STORE)
    if (!pZipExt)
      pZipExt =
          V_stristr(openInfo.m_AbsolutePath, ".vpk" CORRECT_PATH_SEPARATOR_S);
#endif
...
// In VPK?
        ...
        if (pPackFile) {
          if (Q_stricmp(pPackFile->m_ZipName.Get(), openInfo.m_AbsolutePath) ==
              0) {
            openInfo.m_pSearchPath = &m_SearchPaths[i];
            openInfo.m_pFileHandle =
  ...
  if (IsX360()) {
    if (flags & FSOPEN_NEVERINPACK) {
      ...
      // don't allow disk paths to be searched, which is very expensive on the
      // dvd
    ...
    if (filehandle) {
      ...
      if (!openInfo.m_pSearchPath->m_bIsTrustedForPureServer &&
        ...
        m_FileTracker2.NoteFileIgnoredForPureServer(
            openInfo.m_pFileName, pathID, openInfo.m_pSearchPath->m_storeId);
        Close(filehandle);
        openInfo.m_pFileHandle = NULL;
...
FileHandle_t CBaseFileSystem::OpenEx(const char *pFileName,
  ...
  tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s(%s, %s, %u %s )", __FUNCTION__,
         tmDynamicString(TELEMETRY_LEVEL0, pFileName),
         tmDynamicString(TELEMETRY_LEVEL0, pOptions), flags,
  ...
  if (strstr(pOptions, "r") && !strstr(pOptions, "+")) {
    return OpenForRead(pFileName, pOptions, flags, pathID,
                       ppszResolvedFilename);
  }
...
void CBaseFileSystem::Seek(FileHandle_t file, int pos,
  ...
  tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s (pos=%d, whence=%d)", __FUNCTION__,
         pos, whence);
...
// Output : long
//-----------------------------------------------------------------------------
time_t CBaseFileSystem::FastFileTime(const CSearchPath *path,
                                     const char *pFileName) {
  struct _stat buf;
...
int CBaseFileSystem::FPrintf(FileHandle_t file, const char *pFormat, ...) {
  ...
  /*
          if ( !fh->GetFileHandle() )
          {
                  Warning( FILESYSTEM_WARNING, "FS:  Tried to FPrintf NULL file
     pointer inside valid file handle!\n" ); return 0;
          }
          */
...
char *CBaseFileSystem::ReadLine(char *pOutput, int maxChars,
  ...
  while (nRead < (maxChars - 1)) {
    ...
    if ((fh->m_type == FT_PACK_TEXT || fh->m_type == FT_MEMORY_TEXT) &&
        pOutput[nRead] == '\r') {
      // Ignore \r
      continue;
    }
...
// Output : long
//-----------------------------------------------------------------------------
time_t CBaseFileSystem::GetFileTime(const char *pFileName,
                                    const char *pPathID) {
...
time_t CBaseFileSystem::GetPathTime(const char *pFileName,
                                    const char *pPathID) {
  VPROF_BUDGET("CBaseFileSystem::GetPathTime",
               VPROF_BUDGETGROUP_OTHER_FILESYSTEM);
...
EFileCRCStatus
CBaseFileSystem::CheckCachedFileHash(const char *pPathID,
                                     const char *pRelativeFilename,
                                     int nFileFraction, FileHash_t *pFileHash) {
  return m_FileTracker2.CheckCachedFileHash(pPathID, pRelativeFilename,
                                            nFileFraction, pFileHash);
}

void CBaseFileSystem::EnableWhitelistFileTracking(
    bool bEnable, bool bCacheAllVPKHashes, bool bRecalculateAndCheckHashes) {
  if (IsX360()) {
...
void CBaseFileSystem::CacheAllVPKFileHashes(bool bCacheAllVPKHashes,
  ...
  for (int i = 0; i < m_SearchPaths.Count(); i++) {
    ...
    CUtlVector<ChunkHashFraction_t> &vecChunkHash =
        pVPK->AccessPackFileHashes();
    ...
    if (bRecalculateAndCheckHashes) {
      ...
      if (sPackFileErrors.Length()) {
        ...
        Msg("Verify integrity of your game files, perform memory and disk "
    ...
    if (vecChunkHash.Count() == 0) {
      if (vecChunkHashFractionCopy.Count() == 0)
        Msg("File hash information not found: Hashing all VPK files for pure "
            "server operation.\n");
      ...
      if (vecChunkHashFractionCopy.Count() != 0) {
        if (vecChunkHash.Count() != vecChunkHashFractionCopy.Count()) {
          Msg("VPK hash count does not match. VPK content may be corrupt.\n");
                       ...
                       vecChunkHash.Base(), vecChunkHashFractionCopy.Base(),
          ...
          FOR_EACH_VEC(vecChunkHashFractionCopy, iHash) {
            if (Q_memcmp(vecChunkHashFractionCopy[iHash].m_md5contents.bits,
                         ...
                         sizeof(vecChunkHashFractionCopy[iHash]
                                    .m_md5contents.bits)) != 0) {
    ...
    if (bCacheAllVPKHashes) {
      ...
      FOR_EACH_VEC(vecChunkHash, i) {
        m_FileTracker2.AddFileHashForVPKFile(
            vecChunkHash[i].m_nPackFileNumber, vecChunkHash[i].m_nFileFraction,
            vecChunkHash[i].m_cbChunkLen, vecChunkHash[i].m_md5contents,
            fhandle);
      }
    } else {
      if (cFailures == 0 &&
          vecChunkHash.Count() == vecChunkHashFractionCopy.Count())
...
bool CBaseFileSystem::CheckVPKFileHash(int PackFileID, int nPackFileNumber,
                                       int nFileFraction,
                                       MD5Value_t &md5Value) {
#ifdef SUPPORT_PACKED_STORE
  for (int i = 0; i < m_SearchPaths.Count(); i++) {
    ...
    if (pVPK->FindFileHashFraction(nPackFileNumber, nFileFraction,
                                   fileHashFraction)) {
      CPackedStoreFileHandle fhandle = pVPK->GetHandleForHashingFiles();
      ...
      Q_binarytohex((const byte *)md5Value.bits, sizeof(md5Value.bits), hex,
                    sizeof(hex));

      char hex2[34];
      Q_memset(hex2, 0, sizeof(hex2));
      Q_binarytohex((const byte *)fileHashFraction.m_md5contents.bits,
                    sizeof(fileHashFraction.m_md5contents.bits), hex2,
                    sizeof(hex2));

      if (Q_memcmp(fileHashFraction.m_md5contents.bits, md5Value.bits,
                   sizeof(md5Value.bits)) != 0) {
...
#else
  Error("CBaseFileSystem::CheckVPKFileHash should not be called, "
        "SUPPORT_PACKED_STORE not defined");
#endif
  return false;
}

void CBaseFileSystem::RegisterFileWhitelist(IPureServerWhitelist *pWhiteList,
  ...
  if (pFilesToReload)
    *pFilesToReload =
        m_FileTracker2.GetFilesToUnloadForWhitelistChange(pWhiteList);
}

void CBaseFileSystem::NotifyFileUnloaded(const char *pszFilename,
                                         const char *pPathId) {
...
void CBaseFileSystem::SetSearchPathIsTrustedSource(CSearchPath *pSearchPath) {
  ...
  // Treat map packs as trusted, because we will send the CRC of the map pack to
  // the server
  ...
  for (int iKeyIndex = 0;
       iKeyIndex < m_pPureServerWhitelist->GetTrustedKeyCount(); ++iKeyIndex) {
    int nKeySz = 0;
    const byte *pbKey =
        m_pPureServerWhitelist->GetTrustedKey(iKeyIndex, &nKeySz);
...
int CBaseFileSystem::GetUnverifiedFileHashes(CUnverifiedFileHash *pFiles,
                                             int nMaxFiles) {
...
void CBaseFileSystem::FileTimeToString(char *pString,
  ...
  if (IsX360()) {
    ...
    // We see a linefeed at the end of these strings...if there is one, gobble
    // it up
...
// Input  : *path -
//-----------------------------------------------------------------------------
void CBaseFileSystem::CreateDirHierarchy(const char *pRelativePathT,
                                         const char *pathID) {
  // Allow for UNC-type syntax to specify the path ID.
  char tempPathID[MAX_PATH];
  ParsePathID(pRelativePathT, pathID,
              tempPathID); // use the original path param to preserve "//"
...
const char *CBaseFileSystem::FindFirstHelper(const char *pWildCardT,
                                             const char *pPathID,
                                             FileFindHandle_t *pHandle,
                                             int *pFoundStoreID) {
  ...
  if (Q_IsAbsolutePath(pWildCard)) {
    ...
    for (pFindData->currentSearchPathID = 0; pFindData->currentSearchPathID < c;
      ...
      if (pSearchPath->GetPackFile()) {
        // XXX(johns) This support didn't exist for a long time, and I'm now
        // worried about various things
        //            looking for misc files suddenly finding them in the
        //            (untrusted) BSP and causing security nightmares. For now,
        //            restricting FindFirst() support to BSPs only when the BSP
        //            search path is explicitly requested, but this would
        //            otherwise work fine.
        ...
        pSearchPath->GetPackFile()->GetFileAndDirLists(
            pWildCard, pFindData->m_dirMatchesFromVPKOrPak,
      ...
      if (pSearchPath->GetPackedStore()) {
        Assert(pFindData->m_dirMatchesFromVPKOrPak.Count() == 0);
        Assert(pFindData->m_fileMatchesFromVPKOrPak.Count() == 0);
        pSearchPath->GetPackedStore()->GetFileAndDirLists(
            pWildCard, pFindData->m_dirMatchesFromVPKOrPak,
            pFindData->m_fileMatchesFromVPKOrPak, true);
        bIsVPKOrPak = true;
      }
...
bool CBaseFileSystem::FindNextFileHelper(FindData_t *pFindData,
  ...
  for (; pFindData->currentSearchPathID < c; ++pFindData->currentSearchPathID) {
    ...
    if (pSearchPath->GetPackFile()) {
      ...
      //            search path is explicitly requested, but this would
      //            otherwise work fine.
      if (!pFindData->m_FilterPathID ||
          V_strcmp(g_PathIDTable.String(pFindData->m_FilterPathID), "BSP") !=
              0) {
        continue;
      }
      Assert(pFindData->m_dirMatchesFromVPKOrPak.Count() == 0);
      Assert(pFindData->m_fileMatchesFromVPKOrPak.Count() == 0);
      pSearchPath->GetPackFile()->GetFileAndDirLists(
          pFindData->wildCardString.Base(), pFindData->m_dirMatchesFromVPKOrPak,
          pFindData->m_fileMatchesFromVPKOrPak, true);
      if (FindNextFileInVPKOrPakHelper(pFindData))
        return true;
      continue;
...
bool CBaseFileSystem::FindNextFileInVPKOrPakHelper(FindData_t *pFindData) {
  ...
  if (pFindData->m_fileMatchesFromVPKOrPak.Count() > 0) {
    V_strncpy(pFindData->findData.cFileName,
              V_UnqualifiedFileName(pFindData->m_fileMatchesFromVPKOrPak[0]),
  ...
  if (pFindData->m_dirMatchesFromVPKOrPak.Count() > 0) {
    V_strncpy(pFindData->findData.cFileName,
              V_UnqualifiedFileName(pFindData->m_dirMatchesFromVPKOrPak[0]),
              sizeof(pFindData->findData.cFileName));
...
//          that the game is installed to.  We expect all files to be lower
//          cased there - especially on Linux (where case sensitivity is the
//          norm).
//
...
//-----------------------------------------------------------------------------
bool CBaseFileSystem::FixUpPath(const char *pFileName, char *pFixedUpFileName,
                                int sizeFixedUpFileName) {
  //  If appropriate fixes up the filename to ensure that it's handled properly
  //  by the system.
  //
  ...
  if (!V_IsAbsolutePath(pFixedUpFileName)) {
    ...
    //  Need to get "BASE_PATH" from the filesystem paths, and then check this
    //  name against it.
    //
  ...
  //pFixedUpFileName);  // too noisy

#ifdef NEVER // Useful if you're trying to see why your file may not be found
             // (if you have a mixed case file)
...
const char *CBaseFileSystem::RelativePathToFullPath(
  ...
  // @FD This is arbitrary and seems broken.  If the caller needs this filter,
  // they should
  //     request it with the flag themselves.  As it is, I cannot search all the
  //     file paths for a file using this function because there is no option
  //     that says, "No, really, I mean ALL SEARCH PATHS."  The current problem
  //     I'm trying to fix is that sounds are not working if they are in the
  //     BSP.  I wrote code that assumed that I could just ask for the absolute
  //     path of a file, since we are able to open files with these absolute
  //     filenames, and that each particular filesystem call wouldn't have its
    ...
    if (pPack) {
      if (pPack->ContainsFile(pFileName)) {
        ...
        if (nTotalLen >= maxLenInChars) {
          ::Warning("File %s was found in %s, but resulting abs filename won't "
                    "fit in callers buffer of %d bytes\n",
...
// Found in VPK?
    ...
    char pTmpFileName[MAX_FILEPATH];
    V_sprintf_safe(pTmpFileName, "%s%s", pSearchPath->GetPathString(),
                   pFileName);
...
//-----------------------------------------------------------------------------
bool CBaseFileSystem::FullPathToRelativePathEx(
  ...
  for (int i = 0; i < c; i++) {
    ...
    V_strncpy(pSearchBase, m_SearchPaths[i].GetPathString(),
              sizeof(pSearchBase));
#ifdef _WIN32
...
// Obsolete version
//-----------------------------------------------------------------------------
bool CBaseFileSystem::FullPathToRelativePath(
...
// false
//-----------------------------------------------------------------------------
bool CBaseFileSystem::GetCaseCorrectFullPath_Ptr(
    const char *pFullPath, OUT_Z_CAP(maxLenInChars) char *pDest,
    int maxLenInChars) {
  CHECK_DOUBLE_SLASHES(pFullPath);
  ...
  strSearchName =
      strSearchName.Slice(V_strlen(szCurrentDir), strSearchName.Length());
  ...
  V_MakeAbsolutePath(pDest, maxLenInChars, strFoundCaseCorrectName.String(),
                     szDir);
...
CBaseFileSystem::CSearchPath *CBaseFileSystem::CSearchPathsIterator::GetNext() {
  ...
  for (m_iCurrent++; m_iCurrent < m_SearchPaths.Count(); m_iCurrent++) {
    ...
    if (IsX360() && (m_DVDMode == DVDMODE_DEV) && m_Filename[0] &&
        !pSearchPath->m_bIsRemotePath) {
      bool bIsMapPath = pSearchPath->GetPackFile() &&
                        pSearchPath->GetPackFile()->m_bIsMapPath;
      if (!bIsMapPath) {
        bool bIgnorePath = false;
        char szExcludePath[MAX_PATH];
        ...
        V_ComposeFileName(pSearchPath->GetPathString(), m_Filename, szFilename,
                          sizeof(szFilename));
        for (int i = 0; i < m_ExcludePaths.Count(); i++) {
          if (g_pFullFileSystem->String(m_ExcludePaths[i], szExcludePath,
                                        sizeof(szExcludePath))) {
            if (!V_strnicmp(szFilename, szExcludePath, strlen(szExcludePath))) {
              bIgnorePath = true;
              break;
...
void CBaseFileSystem::CSearchPathsIterator::CopySearchPaths(
    const CUtlVector<CSearchPath> &searchPaths) {
...
#ifdef POSIX
    Q_snprintf(tempPathID, sizeof(tempPathID), "%slib%s",
               m_SearchPaths[i].GetPathString(),
               pFileName); // append the path to this dir.
    pModule = Sys_LoadModule(tempPathID);
    if (pModule)
...
CBaseFileSystem::CPathIDInfo *
CBaseFileSystem::FindOrAddPathIDInfo(const CUtlSymbol &id, int bByRequestOnly) {
...
#ifdef POSIX
    _snwprintf(buf, (bufSizeInBytes / sizeof(wchar_t)) - 1, L"%s File",
               V_strupr(ext)); // Matches what Windows does
#else
...
void CBaseFileSystem::AddFilesToFileCache(FileCacheHandle_t cacheId,
                                          const char **ppFileNames,
                                          int nFileNames, const char *pPathID) {
  // For now, assuming that we're only used with GAME.
  Assert(pPathID && Q_strcasecmp(pPathID, "GAME") == 0);
  return static_cast<CFileCacheObject *>(cacheId)->AddFiles(ppFileNames,
                                                            nFileNames);
}
...
bool CBaseFileSystem::IsFileCacheFileLoaded(FileCacheHandle_t cacheId,
                                            const char *pFileName) {
#ifdef _DEBUG
  ...
  bool bFileIsHeldByCache = false;
  {
    ...
    for (int i = 0; i < pFileCache->m_Infos.Count(); ++i) {
      if (pFileCache->m_Infos[i]->pFileName &&
          Q_strcmp(pFileCache->m_Infos[i]->pFileName, pFileName)) {
...
bool CBaseFileSystem::RegisterMemoryFile(
  ...
  CMemoryFileBacking *pInTable =
      m_MemoryFileHash[m_MemoryFileHash.Insert(pFile->m_pFileName, pFile)];
...
bool CFileHandle::IsOK() {
  ...
  } else if (m_pPackFileHandle || m_type == FT_MEMORY_BINARY ||
             m_type == FT_MEMORY_TEXT) {
    return IsValid();
  }
...
int CFileHandle::Read(void *pBuffer, int nDestSize, int nLength) {
  ...
  if (m_pFile) {
    ...
    return static_cast<CMemoryFileHandle *>(this)->Read(pBuffer, nDestSize,
                                                        nLength);
  }
...
int CMemoryFileHandle::Seek(int64 nOffset64, int nWhence) {
  if (nWhence == SEEK_SET) {
    ...
    m_nPosition +=
        (int)clamp(nOffset64, -(int64)m_nPosition, m_nLength - m_nPosition);
...
void CBaseFileSystem::CFileCacheObject::AddFiles(const char **ppFileNames,
                                                 int nFileNames) {
...
void CBaseFileSystem::CFileCacheObject::IOCallback(
    const FileAsyncRequest_t &request, int nBytesRead, FSAsyncStatus_t err) {
  ...
  if (request.pData) {
    if (err == FSASYNC_OK && nBytesRead > 0) {
      pBacking->m_pData =
          (const char *)request.pData; // transfer data ownership
