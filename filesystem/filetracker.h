...
//
// TF isn’t sending any hashes to the server. (That’s probably what CSGO is
// doing, but not TF.) Comparing hashes doesn’t work when there are optional
// updates. (We release a new client without updating the server.) Also on TF,
// we don’t ship any textures or audio to the dedicated server.
//
// On TF, the client just confirms that the files were loaded from a trusted
// source.
//
...
struct TrackedFile_t {
  ...
  const char *m_path;
  int m_nFileFraction;
  int m_iLoadedSearchPathStoreId; // ID of the search path that we loaded from.
                                  // Zero if we are not currently loaded.

  int m_PackFileID;
  int m_nPackFileNumber;
  bool m_bPackOrVPKFile;
  bool m_bFileInVPK;
  bool m_bIgnoredForPureServer; // Did we ignore a file by this name as a result
                                // of pure server rules, the last time it was
                                // opened?

  // The crcIdentifier is a CRC32 of the filename and path. It could be used for
  // quick comparisons of
  //  path+filename, but since we don't do that at the moment we don't need
  //  this.
  // CRC32_t m_crcIdentifier;

  void RebuildFileName(CStringPool &stringPool, const char *pFilename,
                       const char *pPathID, int nFileFraction);
...
public:
  ...
  EFileCRCStatus CheckCachedFileHash(const char *pPathID,
  ...
  int NotePackFileOpened(const char *pVPKAbsPath, const char *pPathID,
                         int64 nLength);
  void NotePackFileAccess(const char *pFilename, const char *pPathID,
                          int iSearchPathStoreId,
                          CPackedStoreFileHandle &VPKHandle);
  void AddFileHashForVPKFile(int nPackFileNumber, int nFileFraction,
                             int cbFileLen, MD5Value_t &md5,
                             CPackedStoreFileHandle &fhandle);
#endif

  void NoteFileIgnoredForPureServer(const char *pFilename, const char *pPathID,
                                    int iSearchPathStoreId);
  void NoteFileLoadedFromDisk(const char *pFilename, const char *pPathID,
                              int iSearchPathStoreId, FILE *fp, int64 nLength);
  ...
  IFileList *
  GetFilesToUnloadForWhitelistChange(IPureServerWhitelist *pNewWhiteList);

private:
  int IdxFileFromName(const char *pFilename, const char *pPathID,
