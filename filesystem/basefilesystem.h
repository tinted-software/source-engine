...
class CPackedStoreRefCount : public CPackedStore,
                             public CRefCounted<CRefCountServiceMT> {
public:
  CPackedStoreRefCount(char const *pFileBasename, char *pszFName,
                       IBaseFileSystem *pFS);
...
public:
  ...
  virtual bool FixUpPath(const char *pFileName, char *pFixedUpFileName,
  ...
  virtual void CacheFileCRCs(const char *pPathname, ECacheCRCType eType,
                             IFileList *pFilter);
  // void						CacheFileCRCs_R( const
  // char *pPathname, ECacheCRCType eType, IFileList *pFilter, CUtlDict<int,int>
  // &searchPathNames );
  ...
  struct FindData_t {
    ...
    HANDLE findHandle;
    CSearchPathsVisits
        m_VisitedSearchPaths; // This is a copy of IDs for the search paths
                              // we've visited, so avoids searching duplicate
                              // paths.
    ...
    CUtlDict<int, int>
        m_VisitedFiles; // We go through the search paths in priority order, and
                        // we use this to make sure that we don't return the
...
protected:
  ...
  virtual int FS_stat(const char *path, struct _stat *buf,
                      bool *pbLoadedFromSteamCache = NULL) = 0;
...
protected:
  ...
  bool PreparePackFile(CPackFile & packfile, int offsetofpackinmetafile,
                       int64 filelen);
  void AddVPKFile(const char *pPath, const char *pPathID,
                  SearchPathAdd_t addType);
...
public:
  CAutoBlockReporter(CBaseFileSystem *fs, bool synchronous,
      ...
      : m_pFS(fs), m_Item(eBlockType, filename, 0.0f, nTypeOfAccess),
  ...
  CAutoBlockReporter(CBaseFileSystem *fs, bool synchronous, FileHandle_t handle,
      ...
      : m_pFS(fs), m_Item(eBlockType, NULL, 0.0f, nTypeOfAccess),
        m_bSynchronous(synchronous) {
    Assert(m_pFS);
    char name[512];
