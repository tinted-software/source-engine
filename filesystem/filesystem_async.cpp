...
//			asynchronous entry points. The model for reads currently
//is a 			callback model where the callback can take place either in the 			context of
//the main thread or the worker thread. It would be 			easy to do a polled model
//later. Async operations return a 			handle which is used to refer to the
//operation later. The 			handle is actually a pointer to a reference counted "job"
//			object that holds all the context, status, and results
//of an 			operation.
//
//=============================================================================
...
struct AsyncOpenedFile_t
    : CRefCounted<CRefCountServiceNoDelete> // no mutex needed, under control of
                                            // CAsyncOpenedFiles
{
...
public:
  ...
  AsyncOpenedFile_t *Get(FSAsyncFile_t item) {
    ...
    int iEntry =
        (CUtlMap<CUtlString, AsyncOpenedFile_t>::IndexType_t)(intp)item;
...
// Cast to int in order to indicate that we are intentionally comparing
// different enum types, to suppress gcc warnings.
...
public:
  CFileAsyncReadJob(const FileAsyncRequest_t &fromRequest,
                    CBaseFileSystem *pOwnerFileSystem)
      : CFileAsyncJob(ConvertPriority(fromRequest.priority)),
        FileAsyncRequest_t(fromRequest), m_pResultData(NULL), m_nResultSize(0),
        m_pRealContext(fromRequest.pContext),
        m_pfnRealCallback(fromRequest.pfnCallback), m_pCustomFetcher(NULL),
        m_hCustomFetcherHandle(NULL), m_pOwnerFileSystem(pOwnerFileSystem) {
  ...
  virtual JobStatus_t DoExecute() {
    ...
    if (m_pCustomFetcher) {
      Assert(GetRefCount() >
             1); // This will produce self-destruction.  No-can-do
      if (m_pCustomFetcher->FinishSynchronous(m_hCustomFetcherHandle) ==
          FSASYNC_OK) {
        retval = JOB_OK;
      } else {
...
public:
  CFileAsyncWriteJob(const char *pszFilename, const void *pData,
      ...
      : CFileAsyncJob(FSASYNC_WRITE_PRIORITY), m_pData(pData), m_nBytes(nBytes),
        m_bFreeMemory(bFreeMemory), m_bAppend(bAppend) {
#if defined(TRACK_BLOCKING_IO)
    m_Timer.Start();
#endif
...
void CBaseFileSystem::InitAsync() {
...
#endif
  {
    ...
    if (IsX360()) {
      ...
      params.nThreadsMax = MIN(
          params.nThreads, 4); // Limit count of IO threads to a maximum of 4.
...
//-----------------------------------------------------------------------------
FSAsyncStatus_t
CBaseFileSystem::AsyncReadMultiple(const FileAsyncRequest_t *pRequests,
                                   int nRequests,
...
FSAsyncStatus_t CBaseFileSystem::AsyncReadMultipleCreditAlloc(
  ...
  for (int i = 0; i < nRequests; i++) {
    ...
    for (int j = 0; j < m_vecAsyncFetchers.Count(); j++) {
      ...
      FSAsyncStatus_t status = m_vecAsyncFetchers[j]->Start(
          *pJob->GetRequest(), &handle, m_pThreadPool);
...
//-----------------------------------------------------------------------------
FSAsyncStatus_t
CBaseFileSystem::AsyncAppendFile(const char *pAppendToFileName,
...
//-----------------------------------------------------------------------------
FSAsyncStatus_t
CBaseFileSystem::SyncGetFileSize(const FileAsyncRequest_t &request) {
...
//-----------------------------------------------------------------------------
FSAsyncStatus_t
CBaseFileSystem::SyncAppendFile(const char *pAppendToFileName,
                                const char *pAppendFromFileName) {
...
void CBaseFileSystem::DoAsyncCallback(const FileAsyncRequest_t &request,
  ...
  if (pDataToFree) {
    ...
    // The ugly delete[] (void*) method generates a compile warning on osx, as
    // it should.
    free(pDataToFree);
#else
    delete[] pDataToFree;
