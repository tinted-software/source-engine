...
#else
    fprintf(stderr, "Could not acquire necessary game files because the "
                    "connection to Steam servers was lost.");
    exit(-1);
#endif
    return;
  }
...
protected:
  ...
  virtual FILE *FS_fopen(const char *filename, const char *options,
                         unsigned flags, int64 *size, CFileLoadInfo *pInfo);
  ...
  virtual int FS_stat(const char *path, struct _stat *buf,
                      bool *pbLoadedFromSteamCache = NULL);
...
void MountDependencies(int iAppId, CUtlVector<unsigned int> &depList) {
  ...
  for (int i = 0; i < (int)steamApp.uNumDependencies; i++) {
    ...
    if (depList.Find(appDependencyInfo.uAppId) == -1) {
      ...
      steam->IsAppSubscribed(appDependencyInfo.uAppId, &isSubscribed,
                             &isPending, &steamError);
      if (isSubscribed) {
...
#ifdef WIN32
      V_ComposeFileName(pchSteamInstallPath, "steam" DLL_EXT_STRING,
                        szSteamDLLPath, Q_ARRAYSIZE(szSteamDLLPath));
#elif defined(POSIX)
      V_ComposeFileName(pchSteamInstallPath, "libsteam" DLL_EXT_STRING,
                        szSteamDLLPath, Q_ARRAYSIZE(szSteamDLLPath));
#else
#error
#endif
...
#else
    PFSteamCreateInterface pfnSteamCreateInterface =
        (PFSteamCreateInterface)dlsym((void *)m_hSteamDLL, "_f");
#endif
...
//-----------------------------------------------------------------------------
FILE *CFileSystem_Steam::FS_fopen(const char *filenameT, const char *options,
  ...
  if (bWriteable) {
    ...
    if (pFile) {
      ...
      // Win32 has an undocumented feature that is serialized ALL writes to a
      // file across threads (i.e only 1 thread can open a file at a time) so
      // use flock here to mimic that behavior
    ...
    int bLocal = 0;
    f = steam->OpenFileEx(filename, options, pInfo->m_bSteamCacheOnly,
                          &fileSize, &bLocal, &steamError);
  ...
  if (f || pFile) {
    CSteamFile *steamFile =
        new CSteamFile(pFile ? (SteamHandle_t)pFile : f, bWriteable, filename);
...
CFileSystem_Steam::WaitForResources(const char *resourcelist) {
...
void CFileSystem_Steam::FS_fseek(FILE *fp, int64 pos, int seekType) {
  ...
  if (BWriteable(steamFile)) {
    ...
    TSteamError steamError;
    int result;
    result = steam->SeekFile((SteamHandle_t)fp, (int32)pos,
...
#define WRITE_CHUNK (256 * 1024)
    ...
    return fwrite(src, 1, size,
                  fp); // return number of bytes written (because we have size =
                       // 1, count = bytes, so it return bytes)
  } else {
#endif
    TSteamError steamError;
...
void CFileSystem_Steam::GetLocalCopy(const char *pFileName) {
  ...
  "Half-Life Steam Filesystem Error", MB_OK ); return;
                  }
  #else
                  srchPath[0] = '.';
                  srchPath[1] = '\0';
  ...
  #define PATH_SEP ":"
                          ...
                          {
                                  bSearch = false;
                                  // If pEnd is NULL then nSize will be rubbish,
  so calculate
                                  // it sensibly.
                                  nSize = strlen( pStart );
                          }

                          // Is this path even potentially in the base
  directory? if ( nSize > nBaseLen )
                          {
                                  // Create a new path (relative to the base
  directory) by stripping off
                                  // nBaseLen characters and therefore doing
  FS_stat relative to the current
                                  // directory, which should be the base
  directory. Assert( sizeof(srchPath) > nBaseLen + strlen(pFileName) + 2 );
                                  nSize -= nBaseLen+1;
                                  memcpy( srchPath, pStart+nBaseLen+1, nSize );
                                  memcpy( srchPath+nSize, pFileName,
  strlen(pFileName)+1 );
                                  // If the path starts with a directory
  separator then we won't get a
                                  ...
                                  if ( srchPath[0] != CORRECT_PATH_SEPARATOR )
                                  {
                                          ...
                                          {
                                                  steam->GetLocalFileCopy(srchPath,
  &steamError); break;
                                          }
                                  }
                          }
  ...
  */
  {
    ...
    // If we got an extension, and this filename has it, then check if it's
    // loaded.
    if (pModuleExtension && V_stristr(pFileName, pModuleExtension)) {
      // We can't be copying files over the top of .so files if they're already
      // loaded
      //	in memory. mmap2( ... MAP_PRIVATE ... ) says "it is unspecified
      //whether changes 	made to the file after the mmap() call are visible in
      //the mapped region." Testing 	and lots of debugging (thanks Pierre-Loup!)
      //has shown that they are, in fact, 	blasted right over your nicely loaded
      //and fixed up object.
      CSysModule *module = Sys_LoadModule(pFileName, SYS_NOLOAD);

      if (module) {
        // Sys_LoadModule( SYS_NOLOAD ) increments the refcount, so bump that
        // back down.
...
void CFileSystem_Steam::ViewSteamCache(const char *szDir, bool bRecurse) {
  ...
  SteamHandle_t h =
      steam->FindFirst(szPath, eSteamFindRemoteOnly, &info, &error);
  int ret = 0;

  if (h != STEAM_INVALID_HANDLE) {
    do {
      Msg("View Steam Cache: '%s%c%s' \n", szDir, CORRECT_PATH_SEPARATOR,
          info.cszName);

      if (bRecurse && info.bIsDir && (0 == V_stristr(info.cszName, "."))) {
        V_snprintf(szPath, sizeof(szPath), "%s%c%s", szDir,
                   CORRECT_PATH_SEPARATOR, info.cszName);
