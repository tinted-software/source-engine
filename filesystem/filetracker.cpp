    ...
    while (m_PendingJobs.PopItem(&stuff)) {
...

      {
        ...
        TrackedVPKFile_t &trackedVPKFile =
            m_treeTrackedVPKFiles[stuff.m_idxTrackedVPKFile];
        TrackedFile_t &trackedfile =
            m_treeAllOpenedFiles[trackedVPKFile.m_idxAllOpenedFiles];

        memcpy(trackedfile.m_filehashFinal.m_md5contents.bits,
               stuff.m_md5Value.bits,
        ...
        trackedfile.m_filehashFinal.m_eFileHashType =
            FileHash_t::k_EFileHashTypeEntireFile;
        trackedfile.m_filehashFinal.m_nPackFileNumber =
            trackedVPKFile.m_nPackFileNumber;
...
//-----------------------------------------------------------------------------
int CFileTracker2::SubmitThreadedMD5Request(uint8 *pubBuffer, int cubBuffer,
...
// calculating the MD5 )
//-----------------------------------------------------------------------------
bool CFileTracker2::IsMD5RequestComplete(int iRequest,
                                         MD5Value_t *pMd5ValueOut) {
  AUTO_LOCK(m_Mutex);
  ...
  while (m_CompletedJobs.PopItem(&stuff)) {
    ...
    Q_memcpy(&m_SubmittedJobs[idxList].m_md5Value, &stuff.m_md5Value,
             sizeof(MD5Value_t));
  ...
  if (m_SubmittedJobs[idxListWaiting].m_bFinished) {
    Q_memcpy(pMd5ValueOut, &m_SubmittedJobs[idxListWaiting].m_md5Value,
             sizeof(MD5Value_t));
...
// calculating the MD5 )
//-----------------------------------------------------------------------------
bool CFileTracker2::BlockUntilMD5RequestComplete(int iRequest,
                                                 MD5Value_t *pMd5ValueOut) {
  while (1) {
...
int CFileTracker2::GetUnverifiedFileHashes(CUnverifiedFileHash *pFiles,
                                           int nMaxFiles) {
  return 0;
}

EFileCRCStatus CFileTracker2::CheckCachedFileHash(const char *pPathID,
  ...
  trackedfileFind.RebuildFileName(m_stringPool, pRelativeFilename, pPathID,
                                  nFileFraction);
  ...
  if (idx != m_treeAllOpenedFiles.InvalidIndex()) {
    ...
    if (trackedfile.m_bFileInVPK) {
      // the FileHash is not meaningful, because the file is in a VPK, we have
      // hashed the entire VPK if the user is sending us a hash for this file,
      // it means he has extracted it from the VPK and tricked the client into
      // loading it instead of the version in the VPK.
...
void TrackedFile_t::RebuildFileName(CStringPool &stringPool,
...
void CFileTracker2::NotePackFileAccess(const char *pFilename,
                                       const char *pPathID,
                                       int iSearchPathStoreId,
  ...
  trackedfile.m_nPackFileNumber =
      VPKHandle.m_nFileNumber; // this might be useful to send up
...
IFileList *CFileTracker2::GetFilesToUnloadForWhitelistChange(
    IPureServerWhitelist *pNewWhiteList) {
  FileListToUnloadForWhitelistChange *pResult =
      new FileListToUnloadForWhitelistChange;

  for (int i = m_treeAllOpenedFiles.FirstInorder(); i >= 0;
       i = m_treeAllOpenedFiles.NextInorder(i)) {
    TrackedFile_t &f = m_treeAllOpenedFiles[i];

    // !KLUDGE! If we ignored it at all, just reload it.
    // This is more conservative than we need to be, but the set of files we are
    // ignoring is probably pretty small so it should be fine.
    ...
    if (f.m_iLoadedSearchPathStoreId != 0 && pNewWhiteList &&
        pNewWhiteList->GetFileClass(f.m_filename) ==
            ePureServerFileClass_AnyTrusted) {
      // Check if we loaded it from a path that no longer exists or is no longer
      // trusted
      const CBaseFileSystem::CSearchPath *pSearchPath =
          m_pFileSystem->FindSearchPathByStoreId(f.m_iLoadedSearchPathStoreId);
      if (pSearchPath == NULL) {
#ifdef PURE_SERVER_DEBUG_SPEW
        Msg("%s was loaded from search path that's no longer mounted.  Queuing "
            "for reload\n",
            f.m_filename);
#endif
        pResult->AddFile(f.m_filename);
...
#ifdef PURE_SERVER_DEBUG_SPEW
        Msg("%s was loaded from search path that's not currently trusted.  "
            "Queuing for reload\n",
            f.m_filename);
#endif
        pResult->AddFile(f.m_filename);
...
void CFileTracker2::AddFileHashForVPKFile(int nPackFileNumber,
                                          int nFileFraction, int cbFileLen,
                                          MD5Value_t &md5,
                                          CPackedStoreFileHandle &VPKHandle) {
#if !defined(DEDICATED)
  AUTO_LOCK(m_Mutex);
  ...
  trackedVPKFile.m_idxAllOpenedFiles =
      IdxFileFromName(pszFileName, "GAME", nFileFraction, true);
  ...
  Q_memcpy(trackedfile.m_filehashFinal.m_md5contents.bits, md5.bits,
           sizeof(md5.bits));
#endif // !DEDICATED
}
...
int CFileTracker2::IdxFileFromName(const char *pFilename, const char *pPathID,
                                   int nFileFraction, bool bPackOrVPKFile) {
  TrackedFile_t trackedfile;
...
int CFileTracker2::NotePackFileOpened(const char *pVPKAbsPath,
                                      const char *pPathID, int64 nLength) {
...
void CFileTracker2::NoteFileIgnoredForPureServer(const char *pFilename,
                                                 const char *pPathID,
                                                 int iSearchPathStoreId) {
#if !defined(_GAMECONSOLE)
  AUTO_LOCK(m_Mutex);
...
void CFileTracker2::NoteFileLoadedFromDisk(const char *pFilename,
                                           const char *pPathID,
                                           int iSearchPathStoreId, FILE *fp,
                                           int64 nLength) {
#if !defined(_GAMECONSOLE) && !defined(DEDICATED)
  AUTO_LOCK(m_Mutex);
...
void CFileTracker2::NoteFileUnloaded(const char *pFilename,
                                     const char *pPathID) {
#if !defined(_GAMECONSOLE)
  AUTO_LOCK(m_Mutex);
...
int CFileTracker2::ListOpenedFiles(bool bAllOpened,
  ...
  Msg("#, Path, FileName, (PackFileID, PackFileNumber), FileLen, "
      "FileFraction\n");
  ...
  while (i != InvalidIndex) {
    ...
    if (file.m_PackFileID)
      cPackFiles++;
    if (!pchFilenameFind || Q_strstr(file.m_filename, pchFilenameFind) ||
        Q_strstr(file.m_path, pchFilenameFind)) {
      Msg("%d %s %s ( %d, %d ) %d %d%s%s\n", count, file.m_path,
          file.m_filename, file.m_PackFileID, file.m_nPackFileNumber,
          file.m_filehashFinal.m_cbFileLen,
          file.m_nFileFraction /*, file.m_crcIdentifier*/,
          file.m_bFileInVPK ? " (invpk)" : "",
          file.m_bPackOrVPKFile ? " (vpk)" : "");
    }

    i = bAllOpened ? m_treeAllOpenedFiles.NextInorder(i)
                   : m_treeTrackedVPKFiles.NextInorder(i);
    count++;
  }

  Msg("cThreadedBlocks:%d cDupMD5s:%d\n", m_cThreadBlocks, m_cDupMD5s);
  Msg("TrackedVPKFiles:%d AllOpenedFiles:%d files VPKfiles:%d "
      "StringPoolCount:%d\n",
      m_treeTrackedVPKFiles.Count(), m_treeAllOpenedFiles.Count(), cPackFiles,
      m_stringPool.Count());
...
static ConCommand trackerlistallfiles("trackerlistallfiles",
                                      CC_TrackerListAllFiles,
                                      "TrackerListAllFiles");
...
static ConCommand trackerlistvpkfiles("trackerlistvpkfiles",
                                      CC_TrackerListVPKFiles,
                                      "TrackerListVPKFiles");
