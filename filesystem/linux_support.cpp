...
int FileSelect(const struct dirent *ent) {
  ...
  while (*mask && *name) {
    if (*mask == '*') {
      ...
      while (*name && toupper(*name) !=
                          toupper(*mask)) { // while the two don't meet up again
        name++;
      }
      if (!*name) { // end of the name
        break;
      }
    } else if (*mask != '?') {
      if (toupper(*mask) != toupper(*name)) { // mismatched!
        return 0;
      } else {
        mask++;
        name++;
        if (!*mask && !*name) { // if its at the end of the buffer
          return 1;
        }
      }
...
int FillDataStruct(FIND_DATA *dat) {
  ...
  Q_snprintf(szFullPath, sizeof(szFullPath), "%s/%s", dat->cBaseDir,
  ...
  Q_snprintf(dat->cFileName, sizeof(dat->cFileName), "%s",
             dat->namelist[dat->curMatch]->d_name);
...
// If multiple names that match are found then lowercase letters take
// precedence.
bool findFileInDirCaseInsensitive(const char *file, char *output,
                                  size_t bufSize) {
