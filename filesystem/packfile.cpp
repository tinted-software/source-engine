...
// How many bytes we should decode at a time when doing pseudo-reads to seek
// forward in a compressed file handle, (affects maximum stack allocation by a
// forward seek)
...
//-----------------------------------------------------------------------------
CFileHandle *CZipPackFile::OpenFile(const char *pFileName,
                                    const char *pOptions) {
  ...
  if (GetFileInfo(pFileName, nIndex, nPosition, nOriginalSize, nCompressedSize,
                  nCompressionMethod)) {
    m_mutex.Lock();
#if defined(SUPPORT_PACKED_STORE)
    ...
    if (nCompressionMethod == ZIP_COMPRESSION_LZMA) {
      ph = new CLZMAZipPackFileHandle(this, nPosition, nOriginalSize,
                                      nCompressedSize, nIndex);
    } else {
      AssertMsg(nCompressionMethod == ZIP_COMPRESSION_NONE,
...
int CZipPackFile::ReadFromPack(int nEntryIndex, void *pBuffer, int nDestBytes,
      ...
      if (CLZMA::IsCompressed(pPreloadData)) {
        ...
        if (nLocalOffset + nBytes <= (int)actualSize) {
          ...
          if (nLocalOffset == 0 && nDestBytes >= (int)actualSize &&
            ...
            CLZMA::Uncompress((unsigned char *)pPreloadData,
  ...
  if (m_hPackFileHandleFS) {
    ...
    nBytesRead =
        m_fs->FS_fread(pBuffer, nDestBytes, nBytes, m_hPackFileHandleFS);
  }
#if defined(SUPPORT_PACKED_STORE)
  else {
...
//-----------------------------------------------------------------------------
bool CZipPackFile::GetFileInfo(const char *pFileName, int &nBaseIndex,
                               int64 &nFileOffset, int &nOriginalSize,
                               int &nCompressedSize,
                               unsigned short &nCompressionMethod) {
  ...
  // We may get passed non-canonicalized filenames, so we need to remove the ../
  // from the path
...
bool CZipPackFile::IndexToFilename(int nIndex, char *pBuffer, int nBufferSize) {
  AssertMsg(nIndex >= 0 && nIndex < m_PackFiles.Count(),
...
bool CZipPackFile::ContainsFile(const char *pFileName) {
  ...
  int64 nOffset;
  unsigned short nCompressionMethod;
  bool bFound = GetFileInfo(pFileName, nIndex, nOffset, nOriginalSize,
                            nCompressedSize, nCompressionMethod);
  return bFound;
}

//-----------------------------------------------------------------------------
//	Build a list of matching files and directories given a FindFirst() style
//wildcard
//-----------------------------------------------------------------------------
void CZipPackFile::GetFileAndDirLists(const char *pRawWildCard,
  ...
  // Workaround edge case in crappy path code. ExtractFilePath extracts a/b/
  // from a/b/c/ but FileBase would return the empty string.
  ...
  // We don't support partial wildcards here (foo*bar.*). This support is
  // massively inconsistent in our codebase and there's no one point where we
  // implement it, so rather than trying to match one of our broken
  // implementations (windows stdio is the only one I could find that was
  // actually right), I'm going with "you shouldn't use this API for that".
  ...
  // For each candidate we attempt to walk up its path and consider the
  // directories it represents as well (the directories in a zip only exist in
  // that files contain them, there are no empty directories)
  FOR_EACH_VEC(m_PackFiles, filesIdx) {
    ...
    // Note that we only ensure the prefix is the same. There are no specific
    // entries for directories in a zip, they only exist in that files in the
    // zip reference them, so handle subdirectory matches from filenames as
    // well.
    CUtlDict<int, int>
        ConsideredDirectories; // Will have duplicate directory matches when
                               // multiple files reside in them
    if ((nLenWildcardPath && (0 == V_strnicmp(szCandidateName, szWildCardPath,
                                              nLenWildcardPath))) ||
        (!nLenWildcardPath && strchr(szCandidateName, '/'))) {
      // Check if we matched because of a sub-directory, e.g. a/b/*.* would
      // match /a/b/c/d/foo (in which case we want to add /a/b/c to the matched
      // directories list, ignoring the actual specific file)
      char szCandidateBaseName[MAX_PATH] = {0};
      bool bIsDir = false;
      size_t nSubDirLen = 0;
      char *pSubDirSlash = strchr(szCandidateName + nLenWildcardPath, '/');
      if (pSubDirSlash) {
        // This is a subdirectory match, drop everything after it and continue
        // with it as the filename
        nSubDirLen = (size_t)((ptrdiff_t)pSubDirSlash -
                              (ptrdiff_t)(szCandidateName + nLenWildcardPath));
        V_strncpy(szCandidateBaseName, szCandidateName + nLenWildcardPath,
                  nSubDirLen + 1);
        bIsDir = true;

        // Early out if we already considered this exact directory from another
        // file
        if (ConsideredDirectories.Find(szCandidateBaseName) !=
            ConsideredDirectories.InvalidIndex()) {
          continue;
        }
      ...
      if ((bExtWildcard && pExt) || (!pExt && !bWildcardHasExt))
        ...
        bExtMatch =
            bWildcardHasExt && pExt && (0 == V_stricmp(pExt, szWildCardExt));
...
bool CZipPackFile::Prepare(int64 fileLen, int64 nFileOfs) {
  ...
  zipDirBuff.Get(filename, Min((size_t)zipFileHeader.fileNameLength,
                               sizeof(filename) - 1));
  if (!V_stricmp(filename, PRELOAD_SECTION_NAME)) {
    ...
    int nextOffset = bCompatibleFormat ? zipFileHeader.extraFieldLength +
    ...
    if (IsX360()) {
      ...
      // only during development, maps are allowed to lack them, due to
      // auto-conversion
  ...
  for (int i = firstFileIdx; i < numFilesInZip; ++i) {
    ...
    unsigned int fileNameLen =
        Min((size_t)zipFileHeader.fileNameLength, sizeof(tmpString) - 1);
...
//-----------------------------------------------------------------------------
CZipPackFileHandle::CZipPackFileHandle(CZipPackFile *pOwner, int64 nBase,
...
void CZipPackFileHandle::SetBufferSize(int nBytes) {
  if (m_pOwner->m_hPackFileHandleFS) {
    m_pOwner->FileSystem()->FS_setbufsize(m_pOwner->m_hPackFileHandleFS,
                                          nBytes);
  }
}
...
CLZMAZipPackFileHandle::CLZMAZipPackFileHandle(CZipPackFile *pOwner,
                                               int64 nBase,
  ...
  if (++sLZMAPackFileHandles == PACKFILE_COMPRESSED_FILE_HANDLES_WARNING) {
    // By my count a live filehandle is currently around 270k, mostly due to the
    // LZMA dictionary (256k) with the rest being the read/seek buffers.
    Warning(
...
int CLZMAZipPackFileHandle::Read(void *pBuffer, int nDestSize, int nBytes) {
  ...
  while (nBytesRead < nMaxRead) {
    ...
    bool bSuccess = m_pLZMAStream->Read(
        (unsigned char *)m_ReadBuffer.PeekGet(), nRemainingReadBuffer,
    ...
    if (bSuccess) {
      ...
      AssertMsg(nCompressedBytesRead == (unsigned int)nRemainingReadBuffer ||
                    nBytesRead == nMaxRead,
                "Should have consumed the readbuffer or reached nMaxRead");

      if (nCompressedBytesRead == 0 && nOutputBytesWritten == 0) {
        AssertMsg(
            nCompressedBytesRead > 0 || nOutputBytesWritten > 0,
  ...
  // If we read less than BackSeekBuffer.Size() bytes, shift the end of the old
  // backseek buffer up
  int nOldBackSeek = m_BackSeekBuffer.TellPut();
  int nReuseBackSeek =
      Max(Min(m_BackSeekBuffer.Size() - nBytesRead, nOldBackSeek), 0);
  if (nReuseBackSeek) {
    // Shift the reused chunk to the front
    V_memmove(m_BackSeekBuffer.Base(),
              (unsigned char *)m_BackSeekBuffer.Base() +
                  m_BackSeekBuffer.TellPut() - nReuseBackSeek,
              nReuseBackSeek);
  }
  ...
  int nReadIntoBackSeek =
      Min(m_BackSeekBuffer.Size() - nReuseBackSeek, nBytesRead);
  m_BackSeekBuffer.Put((unsigned char *)pBuffer + nBytesRead -
                           nReadIntoBackSeek,
                       nReadIntoBackSeek);
...
int CLZMAZipPackFileHandle::Seek(int nOffset, int nWhence) {
  ...
  if (nNewPosition < m_nSeekPosition) {
    ...
    if (nBackSeekAvailable >= nDesiredBackSeek) {
      ...
      // Seeking backwards beyond our backseek buffer. Have to restart stream.
      // This kills the performance.
      Warning("LZMA file handle: seeking backwards beyond backseek buffer size "
              "( %u ), "
              "replaying read & decompression of %u bytes. Should avoid large "
              "back seeks in compressed files or "
  ...
  // Forward seek
  if (nNewPosition > m_nSeekPosition) {
    // Can't actually seek forward without making decode progress. Issue fake
    // reads until we've reached our target.
    unsigned char dummyBuffer[COMPRESSED_SEEK_READ_CHUNK];
    while (nNewPosition > m_nSeekPosition) {
      int nReadSize =
          Min(nNewPosition - m_nSeekPosition, COMPRESSED_SEEK_READ_CHUNK);
      unsigned int nBytesRead =
          Read(&dummyBuffer, sizeof(dummyBuffer), nReadSize);
      m_nSeekPosition += nBytesRead;
      if (!nBytesRead) {
        Warning("LZMA file handle: failed reading forward to desired seek "
                "position\n");
        break;
      }
    }
...
int CLZMAZipPackFileHandle::FillReadBuffer() {
  ...
  int nRemainingCompressedBytes =
      CZipPackFileHandle::Size() - CZipPackFileHandle::Tell();
  ...
  int nRefillResult = CZipPackFileHandle::Read(
      m_ReadBuffer.PeekPut(), m_ReadBuffer.Size(), nRefillSize);
