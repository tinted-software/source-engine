...
private:
  CWin32ReadOnlyFile(HANDLE hFileUnbuffered, HANDLE hFileBuffered,
                     int sectorSize, int64 fileSize, bool bOverlapped)
      : m_hFileUnbuffered(hFileUnbuffered), m_hFileBuffered(hFileBuffered),
        m_ReadPos(0), m_Size(fileSize), m_SectorSize(sectorSize),
...
//-----------------------------------------------------------------------------
FILE *CFileSystem_Stdio::FS_fopen(const char *filenameT, const char *options,
                                  unsigned flags, int64 *size) {
...
size_t CFileSystem_Stdio::FS_fwrite(const void *src, size_t size, FILE *fp) {
  tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s %t", __FUNCTION__,
...
//
// This is not well tested or meant to be a proper implementation of stat(), but
// rather a band-aid for XP users only until microsoft pushes a runtime update
// to fix above issue :-/
//-----------------------------------------------------------------------------
...
static int WindowsXPStatShim(const char *pathT, struct _stat *buf) {
  ...
  // Mode
  unsigned short permBits =
      _S_IREAD; // If GetFileAttributes let us see it, we can read it. I think.
  if (fileAttributes.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
    ...
    if (V_strcasecmp(pExt, "exe") == 0 || V_strcasecmp(pExt, "bat") == 0 ||
        V_strcasecmp(pExt, "com") == 0 || V_strcasecmp(pExt, "cmd") == 0) {
  ...
  buf->st_nlink = 1;
  buf->st_size = (uint64_t)fileAttributes.nFileSizeHigh << 32 |
                 fileAttributes.nFileSizeLow;
  // The 90s was a hell of a time I guess.
  uint64_t actualAccessTime =
      (uint64_t)fileAttributes.ftLastAccessTime.dwHighDateTime << 32 |
      (uint64_t)fileAttributes.ftLastAccessTime.dwLowDateTime;
  uint64_t actualModTime =
      (uint64_t)fileAttributes.ftLastWriteTime.dwHighDateTime << 32 |
      (uint64_t)fileAttributes.ftLastWriteTime.dwLowDateTime;
  uint64_t actualCreationTime =
      (uint64_t)fileAttributes.ftCreationTime.dwHighDateTime << 32 |
      (uint64_t)fileAttributes.ftCreationTime.dwLowDateTime;
  uint64_t ullMSUniverseToEveryoneElseUniverse =
      (369 * 365 + 89) * 86400ull; // okay
  buf->st_atime =
      actualAccessTime / 10000000ull - ullMSUniverseToEveryoneElseUniverse;
  buf->st_mtime =
      actualModTime / 10000000ull - ullMSUniverseToEveryoneElseUniverse;
  buf->st_ctime =
      actualCreationTime / 10000000ull - ullMSUniverseToEveryoneElseUniverse;
...
int CFileSystem_Stdio::FS_stat(const char *pathT, struct _stat *buf,
  ...
  // Workaround bug wherein stat() randomly fails on Windows XP.  See comment on
  // function.
  /*
  #if defined(_WIN32) && defined(FILESYSTEM_MSVC2015_STAT_BUG_WORKAROUND)
          if ( rt == -1 )
          {
                  ...
                  if ( eOSType == k_eWin2000 || eOSType == k_eWinXP || eOSType
  == k_eWin2003 )
                  {
...
// Purpose: low-level filesystem wrapper
//-----------------------------------------------------------------------------
HANDLE CFileSystem_Stdio::FS_FindFirstFile(const char *findnameT,
                                           WIN32_FIND_DATA *dat) {
...
CFileSystem_Stdio::WaitForResources(const char *resourcelist) {
#if defined(DEBUG_WAIT_FOR_RESOURCES_API)
...
// Purpose: low-level filesystem wrapper
//-----------------------------------------------------------------------------
CStdioFile *CStdioFile::FS_fopen(const char *filenameT, const char *options,
                                 int64 *size) {
  FILE *pFile = NULL;
  char *p = NULL;
  char filename[MAX_PATH];
  ...
  if (pFile) {
    ...
    if (bWriteable) {
...

      {
        ...
        // add a lock here to mimic that behavior
      ...
      // grab the lock once we have UNLOCKED m_MutexLockedFD so we don't
      // deadlock on a close
      pMutex->Lock();
...
void CStdioFile::FS_setbufsize(unsigned nBytes) {
  ...
  if (nBytes) {
    ...
    // hack to make microsoft stdio not always read one stray byte on odd sized
    // files
    m_pFile->_bufsiz = 1;
#endif
  }
...
size_t CStdioFile::FS_fwrite(const void *src, size_t size) {
  ...
  return fwrite(src, 1, size,
                m_pFile); // return number of bytes written (because we have
                          // size = 1, count = bytes, so it return bytes)
}
...
static HANDLE OpenWin32File(const char *filename, bool bOverlapped,
                            bool bUnbuffered, int64 *pFileSize) {
  HANDLE hFile;
...
size_t CWin32ReadOnlyFile::FS_fread(void *dest, size_t destSize, size_t size) {
  ...
  while (bReadOk && nBytesToRead > 0) {
    ...
    if (bReadOk) {
      if (!m_bOverlapped ||
          GetOverlappedResult(hReadFile, &overlapped, &nCurBytesRead, true)) {
    ...
    if (nBytesToRead > 0 &&
        nCurBytesRead ==
            0) // if you failed to ready anything this time then bail the loop
    {
      ...
      if (IsX360()) {
        if (dwError == ERROR_DISK_CORRUPT || dwError == ERROR_FILE_CORRUPT) {
          FSDirtyDiskReportFunc_t func =
              g_FileSystem_Stdio.GetDirtyDiskReportFunc();
          if (func) {
            func();
            result = 0;
          }
