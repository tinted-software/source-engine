...
// Kyle says: this is here only to change the DLL size to force clients to
// update! This should be removed
//			  by whoever sees this comment after we've shipped a DLL
//using it!
...
void IOAsyncCallback(const FileAsyncRequest_t &asyncRequest, int numReadBytes,
  ...
  if (!g_QueuedLoader.IsDynamic()) {
    g_pThreadPool
        ->QueueCall(IOComputationJob, pFileJob, asyncRequest.pData,
                    numReadBytes, loaderError)
        ->Release();
  } else {
    g_QueuedLoader.QueueDynamicLoadFunctor(
        CreateFunctor(IOComputationJob, pFileJob, asyncRequest.pData,
                      numReadBytes, loaderError));
  }
...
bool CQueuedLoader::CResourceNameLessFunc::Less(
  ...
  case RESOURCEPRELOAD_MATERIAL: {
    ...
    const char *pNameLHS =
        g_QueuedLoader.GetFilename(hFilenameLHS, szNameLHS, sizeof(szNameLHS));
    const char *pNameRHS =
        g_QueuedLoader.GetFilename(hFilenameRHS, szNameRHS, sizeof(szNameRHS));
...
int CQueuedLoader::CFileJobsLessFunc::GetLayoutOrderForFilename(
  ...
  if (XBX_IsLocalized()) {
    if (V_stristr(pFilename, "\\zip") &&
        V_stristr(pFilename, XBX_GetLanguageString())) {
...
void CQueuedLoader::SubmitPendingJobs() {
  ...
  for (int i = 0; i < sortedFiles.Count(); i++) {
    ...
    if (pFileJob->m_bFileExists) {
      ...
      // file absence
      if (IsDynamic())
        QueueDynamicLoadFunctor(CreateFunctor(IOComputationJob, pFileJob,
                                              pFileJob->m_pTargetData, 0,
...
bool CQueuedLoader::AddJob(const LoaderJob_t *pLoaderJob) {
  ...
  if (V_IsAbsolutePath(pLoaderJob->m_pFilename)) {
    ...
    if (IsX360() && (g_pFullFileSystem->GetDVDMode() == DVDMODE_STRICT)) {
      if (V_stristr(pLoaderJob->m_pFilename, ".bsp") ||
          V_stristr(pLoaderJob->m_pFilename, ".ain")) {
...
void CQueuedLoader::GetJobRequests() {
  ...
  if (!IsPC() && !m_bDynamic) {
    ...
    // seconds. These operations flood calls (AddJob) back into the queued
    // loader (which has to mutex its lists), so in fact it's slightly slower to
    ...
    jobs[1] = g_pThreadPool->QueueCall(
        BuildMaterialResources, m_pLoaders[RESOURCEPRELOAD_MATERIAL],
    ...
    BuildResources(m_pLoaders[RESOURCEPRELOAD_CUBEMAP],
                   &m_ResourceNames[RESOURCEPRELOAD_CUBEMAP],
                   &m_LoaderTimes[RESOURCEPRELOAD_CUBEMAP]);
    BuildResources(m_pLoaders[RESOURCEPRELOAD_SOUND],
                   &m_ResourceNames[RESOURCEPRELOAD_SOUND],
                   &m_LoaderTimes[RESOURCEPRELOAD_SOUND]);
    BuildMaterialResources(m_pLoaders[RESOURCEPRELOAD_MATERIAL],
                           &m_ResourceNames[RESOURCEPRELOAD_MATERIAL],
    ...
    BuildResources(m_pLoaders[RESOURCEPRELOAD_STATICPROPLIGHTING],
                   &m_ResourceNames[RESOURCEPRELOAD_STATICPROPLIGHTING],
                   &m_LoaderTimes[RESOURCEPRELOAD_STATICPROPLIGHTING]);
    BuildResources(m_pLoaders[RESOURCEPRELOAD_MODEL],
                   &m_ResourceNames[RESOURCEPRELOAD_MODEL],
                   &m_LoaderTimes[RESOURCEPRELOAD_MODEL]);
    BuildResources(m_pLoaders[RESOURCEPRELOAD_ANONYMOUS],
                   &m_ResourceNames[RESOURCEPRELOAD_ANONYMOUS],
...
bool CQueuedLoader::BeginMapLoading(const char *pMapName, bool bLoadForHDR,
  ...
  // these safety watermarks throttle the i/o from flooding memory, when the
  // cores cannot keep up the delta must be larger than any single operation,
  // otherwise deadlock markers that are too close will cause excessive
  // suspension
  ...
  // map pak will be accessed asynchronously throughout loading and into game
  // frame
  g_pFullFileSystem->BeginMapAccess();

  // remove any prior stale entries
  CleanQueue();
  Assert(m_SubmittedJobs.Count() == 0 && g_nActiveJobs == 0 &&
         g_nQueuedJobs == 0);
  ...
  m_bSameMap = bOptimizeMapReload &&
               (V_stricmp(pMapName, m_szMapNameToCompareSame) == 0);
  if (m_bSameMap) {
    // Data will persist (so reloading a map is v. fast)
  } else {
    // Full load of the new map's data
    V_strncpy(m_szMapNameToCompareSame, pMapName,
              sizeof(m_szMapNameToCompareSame));
  }
  ...
  V_snprintf(szFilename, sizeof(szFilename), "reslists_xbox/%s%s.lst",
             szBaseName, GetPlatformExt());
  ...
  if (!g_pFullFileSystem->ReadFile(szFilename, "GAME", resListBuffer, 0, 0)) {
    ...
    DevWarning(
        "QueuedLoader: Failed to get reslist '%s', Non-Optimal Loading.\n",
        szFilename);
    m_bActive = false;
    return false;
  }
  ...
  if (XBX_IsLocalized()) {
    ...
    V_snprintf(szFilename, sizeof(szFilename), "reslists_xbox/%s%s.lst",
               XBX_GetLanguageString(), GetPlatformExt());
    CUtlBuffer localizedBuffer(0, 0, CUtlBuffer::TEXT_BUFFER);
    if (g_pFullFileSystem->ReadFile(szFilename, "GAME", localizedBuffer, 0,
                                    0)) {
      // append it
      resListBuffer.EnsureCapacity(resListBuffer.TellPut() +
                                   localizedBuffer.TellPut());
  ...
  // disable progress tracking to prevent any late queued operation from
  // updating
...
// Returns true if loader is batching
//-----------------------------------------------------------------------------
bool CQueuedLoader::IsDynamic() const { return m_bDynamic; }
...
void CQueuedLoader::DynamicLoadMapResource(const char *pFilename,
                                           DynamicResourceCallback_t pCallback,
                                           void *pContext, void *pContext2) {
...
bool CQueuedLoader::CompleteDynamicLoad() {
  ...
  if (m_bDynamic) {
    ...
    {
      AUTO_LOCK(m_FunctorQueueMutex);
      bDone = m_FunctorQueue.Count() == 0 && g_nQueuedJobs == 0 &&
              g_nActiveJobs == 0;
    }

    if (bDone) {
      if (m_pfnDynamicCallback) {
        (*m_pfnDynamicCallback)(m_DynamicFileName, m_pDynamicContext,
                                m_pDynamicContext2);
      }
