...
// How many bytes compressed filehandles should hold for seeking backwards.
// Seeks beyond this limit will require rewinding and restarting the
// compression, at significant performance penalty. (Warnings emitted when this
// occurs)
#define PACKFILE_COMPRESSED_FILEHANDLE_SEEK_BUFFER 4096

// How many bytes compressed filehandles should attempt to read (and cache) at a
// time from the underlying compressed data.
#define PACKFILE_COMPRESSED_FILEHANDLE_READ_BUFFER 4096

// Emit warnings in debug builds if we hold more than this many compressed file
// handles alive to alert to the poor memory characteristics.
...
public:
  CZipPackFileHandle(CZipPackFile *pOwner, int64 nBase, unsigned int nLength,
...
public:
  CLZMAZipPackFileHandle(CZipPackFile *pOwner, int64 nBase,
                         unsigned int nOriginalSize,
                         unsigned int nCompressedSize, unsigned int nIndex = -1,
...
private:
  // Ensure there are bytes in the read buffer, assuming we're not at the end of
  // the underlying data
  int FillReadBuffer();
  ...
  // Contains the last PACKFILE_COMPRESSED_FILEHANDLE_SEEK_BUFFER decompressed
  // bytes. The Put and Get locations mimic our filehandle -- TellPut() ==
  // TelGet() when we are not back seeking.
  CUtlBuffer m_BackSeekBuffer;

  // The read buffer from the underlying compressed stream. We read
  // PACKFILE_COMPRESSED_FILEHANDLE_READ_BUFFER bytes into this buffer, then
  // consume it via the buffer get position.
...
public:
  ...
  // Helper for the filesystem's FindFirst/FindNext() API which mimics the old
  // windows equivalent. pWildcard is the same pattern that you would pass to
  // FindFirst, not a true wildcard. Mirrors the VPK code's similar call.
  virtual void GetFileAndDirLists(const char *pFindWildCard,
                                  CUtlStringList &outDirnames,
                                  CUtlStringList &outFilenames,
                                  bool bSortedOutput) = 0;
...
public:
  ...
  virtual CFileHandle *OpenFile(const char *pFileName,
                                const char *pOptions = "rb") OVERRIDE;

  virtual void GetFileAndDirLists(const char *pFindWildCard,
                                  CUtlStringList &outDirnames,
                                  CUtlStringList &outFilenames,
                                  bool bSortedOutput) OVERRIDE;

  virtual int64 GetPackFileBaseOffset() OVERRIDE { return m_nBaseOffset; }

  virtual bool IndexToFilename(int nIndex, char *pBuffer,
                               int nBufferSize) OVERRIDE;

protected:
  virtual int ReadFromPack(int nIndex, void *buffer, int nDestBytes, int nBytes,
                           int64 nOffset) OVERRIDE;
...
#pragma pack()
  ...
  bool GetFileInfo(const char *pFileName, int &nBaseIndex, int64 &nFileOffset,
                   int &nOriginalSize, int &nCompressedSize,
