...
struct CSoundEntryEqualFunctor : CaselessStringEqualFunctor {
  ...
  bool operator()(CSoundEntry *lhs, CSoundEntry *rhs) const {
    return CaselessStringEqualFunctor::operator()(lhs->m_Name.Get(),
                                                  rhs->m_Name.Get());
  }
...
private:
  ...
  void AddSoundsFromFile(const char *filename, bool bPreload,
                         bool bIsOverride = false, bool bRefresh = false);
  ...
  CUtlHashtable<CUtlConstString, gender_t, CaselessStringHashFunctor,
                UTLConstStringCaselessStringEqualFunctor<char>>
      m_ActorGenders;
  CUtlStableHashtable<CSoundEntry *, empty_t, CSoundEntryHashFunctor,
                      CSoundEntryEqualFunctor, uint16, const char *>
      m_Sounds;
