...
CSoundEmitterSystemBase::CSoundEmitterSystemBase()
    : m_nInitCount(0), m_uManifestPlusScriptChecksum(0) {}

//-----------------------------------------------------------------------------
// Purpose:
// Output : int
//-----------------------------------------------------------------------------
int CSoundEmitterSystemBase::First() const { return m_Sounds.FirstHandle(); }
  ...
  */
  ...
  if (filesystem->LoadKeyValues(*manifest, IFileSystem::TYPE_SOUNDEMITTER,
    ...
    for (KeyValues *sub = manifest->GetFirstSubKey(); sub != NULL;
      ...
      Warning("CSoundEmitterSystemBase::BaseInit:  Manifest '%s' with bogus "
              "file type '%s', expecting 'declare_file' or 'precache_file'\n",
...
void CSoundEmitterSystemBase::InternalModShutdown() {
  ...
  for (UtlHashHandle_t nIndex = m_Sounds.FirstHandle();
       nIndex != m_Sounds.InvalidHandle();
...
CSoundEmitterSystemBase::InternalGetParametersForSound(int index) {
  if (!m_Sounds.IsValidHandle(index)) {
    Assert(!"CSoundEmitterSystemBase::InternalGetParametersForSound:  Bogus "
            "index");
    return NULL;
  }
...
void CSoundEmitterSystemBase::GenderExpandString(gender_t gender,
  ...
  default:
  case GENDER_NONE: {
    Assert(!"CSoundEmitterSystemBase::GenderExpandString:  expecting MALE or "
            "FEMALE!");
  } break;
  case GENDER_MALE: {
  ...
  while (pKey) {
    if (!Q_strcasecmp(pKey->GetName(), "channel")) {
      ...
      if (!Q_strncasecmp(pKey->GetString(), "SNDLVL_", strlen("SNDLVL_"))) {
        DevMsg("CSoundEmitterSystemBase::GetParametersForSound:  sound %s has "
      ...
      if (bCompatibilityAttenuation) {
        if (params.GetSoundLevel().range != 0) {
          Warning("CompatibilityAttenuation for sound %s must have same start "
                  "and end values.\n",
                  soundname);
        }
...
//-----------------------------------------------------------------------------
const char *
CSoundEmitterSystemBase::GetWavFileForSound(const char *soundname,
...
void CSoundEmitterSystemBase::AddSoundsFromFile(const char *filename,
  ...
  if (filesystem->LoadKeyValues(*kv, IFileSystem::TYPE_SOUNDEMITTER, filename,
    ...
    while (pKeys) {
      if (pKeys->GetFirstSubKey()) {
        ...
        UtlHashHandle_t lookup =
            m_Sounds.Insert(pEntry); // insert returns existing item if found
  ...
  if (bIsOverride) {
    DevMsg("SoundEmitter:  adding map sound overrides from %s [%i total, %i "
...
//-----------------------------------------------------------------------------
// Purpose: Reload a sound emitter file (used to refresh files after sv_pure is
// turned on)
//-----------------------------------------------------------------------------
void CSoundEmitterSystemBase::ReloadSoundEntriesInList(
  ...
  for (i = 0; i < c; i++) {
    ...
    if (pszFileName && pszFileName[0]) {
      if (processed.Find(pszFileName) == processed.InvalidIndex() &&
          pFilesToReload->IsFileInList(pszFileName)) {
...
//-----------------------------------------------------------------------------
// Purpose: Force ModShutdown and ModInit, skips checks for how many systems
// have requested inits (for con commands).
//-----------------------------------------------------------------------------
...
bool CSoundEmitterSystemBase::AddSound(const char *soundname,
  ...
  if (i == m_SoundKeyValues.InvalidIndex()) {
    Warning("CSoundEmitterSystemBase::AddSound( '%s', '%s', ... ), script file "
            "not list in manifest '%s'\n",
...
void CSoundEmitterSystemBase::MoveSound(const char *soundname,
  ...
  if (newscriptindex == m_SoundKeyValues.InvalidIndex()) {
    Warning("CSoundEmitterSystemBase::MoveSound( '%s', '%s' ), script file not "
            "list in manifest '%s'\n",
...
bool CSoundEmitterSystemBase::GetParametersForSoundEx(
  ...
  if (!params.soundname[0]) {
    DevMsg("CSoundEmitterSystemBase::GetParametersForSound:  sound %s has no "
           "wave or rndwave key!\n",
           soundname);
    return false;
  }
  ...
  if (internal->HadMissingWaveFiles() && params.soundname[0] != CHAR_SENTENCE) {
    ...
    if (!filesystem->FileExists(testfile)) {
      ...
      if (UTL_INVAL_SYMBOL == soundWarnings.Find(key)) {
        ...
        DevMsg("CSoundEmitterSystemBase::GetParametersForSound:  sound '%s' "
...
soundlevel_t
CSoundEmitterSystemBase::LookupSoundLevelByHandle(char const *soundname,
...
void CSoundEmitterSystemBase::ClearSoundOverrides() {
  ...
  for (UtlHashHandle_t i = m_Sounds.FirstHandle();
       i != m_Sounds.InvalidHandle();) {
  ...
  if (removed > 0 || m_SavedOverrides.Count() > 0) {
    Warning("SoundEmitter:  removing map sound overrides [%i to remove, %i to "
            "restore]\n",
            removed, m_SavedOverrides.Count());
  }
