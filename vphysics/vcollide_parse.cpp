...
public:
  ...
  void ParseVehicle(vehicleparams_t *pVehicle,
                    IVPhysicsKeyHandler *unknownKeyHandler);
...
void CVPhysicsParse::ParseSolid(solid_t *pSolid,
                                IVPhysicsKeyHandler *unknownKeyHandler) {
  char key[MAX_KEYVALUE], value[MAX_KEYVALUE];
  key[0] = 0;
...
void CVPhysicsParse::ParseRagdollConstraint(
  ...
  // BUGBUG: HL rotations are counter-clockwise, so reverse these limits at some
  // point!!!
...
void CVPhysicsParse::ParseFluid(fluid_t *pFluid,
                                IVPhysicsKeyHandler *unknownKeyHandler) {
  char key[MAX_KEYVALUE], value[MAX_KEYVALUE];
...
void CVPhysicsParse::ParseSurfaceTable(int *table,
                                       IVPhysicsKeyHandler *unknownKeyHandler) {
  char key[MAX_KEYVALUE], value[MAX_KEYVALUE];
...
void CVPhysicsParse::ParseVehicleSuspension(
...
void CVPhysicsParse::ParseVehicle(vehicleparams_t *pVehicle,
                                  IVPhysicsKeyHandler *unknownKeyHandler) {
  char key[MAX_KEYVALUE], value[MAX_KEYVALUE];
...
void CVPhysicsParse::ParseCustom(void *pCustom,
                                 IVPhysicsKeyHandler *unknownKeyHandler) {
  char key[MAX_KEYVALUE], value[MAX_KEYVALUE];
...
void DestroyVPhysicsKeyParser(IVPhysicsKeyParser *pParser) { delete pParser; }

//-----------------------------------------------------------------------------
// Helper functions for parsing script file
//-----------------------------------------------------------------------------

const char *ParseKeyvalue(const char *pBuffer, char (&key)[MAX_KEYVALUE],
