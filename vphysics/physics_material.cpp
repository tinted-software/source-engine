...
public:
  // IVP_Material
  virtual IVP_DOUBLE get_friction_factor() { return data.physics.friction; }

  virtual IVP_DOUBLE get_elasticity() { return data.physics.elasticity; }
  virtual const char *get_name();
  // UNDONE: not implemented here.
  virtual IVP_DOUBLE get_second_friction_factor() { return 0; }
  virtual IVP_DOUBLE get_adhesion() { return 0; }

  virtual IVP_DOUBLE get_damping() { return data.physics.dampening; }
...
public:
  ...
  virtual IVP_Material *get_material_by_index(IVP_Real_Object *pObject,
                                              const IVP_U_Point *world_position,
                                              int index);

  virtual IVP_DOUBLE
  get_friction_factor(IVP_Contact_Situation
                          *situation) // returns values >0, value of 1.0f means
                                      // object stands on a 45 degres hill
  {
    ...
    if (ShouldOverrideWheelContactFriction(
            &wheelFriction, situation->objects[0], situation->objects[1],
  ...
  virtual IVP_DOUBLE
  get_elasticity(IVP_Contact_Situation
                     *situation) // range [0, 1.0f[, the relative speed after a
...
public:
  ...
  virtual void GetPhysicsParameters(int surfaceDataIndex,
                                    surfacephysicsparams_t *pParamsOut) const;
...
CPhysicsSurfaceProps::CPhysicsSurfaceProps(void)
    : m_fileList(8, 8), m_strings(0, 32, true) {
  m_ivpManager.Init(this);
  // Force index 0 to be the empty string.  Allows game code to check for zero,
  // but still resolve to a string
  m_strings.AddString("");
  m_init = false;
  m_shadowFallback = 0;
}
...
const CSurface *
CPhysicsSurfaceProps::GetInternalSurface(int materialIndex) const {
  return const_cast<CPhysicsSurfaceProps *>(this)->GetInternalSurface(
      materialIndex);
}

void CPhysicsSurfaceProps::GetPhysicsProperties(int materialIndex,
                                                float *density,
                                                float *thickness,
                                                float *friction,
                                                float *elasticity) const {
...
void CPhysicsSurfaceProps::GetPhysicsParameters(
    int surfaceDataIndex, surfacephysicsparams_t *pParamsOut) const {
  if (!pParamsOut)
    return;
...
const char *
CPhysicsSurfaceProps::GetString(unsigned short stringTableIndex) const {
...
const char *
CPhysicsSurfaceProps::GetReservedMaterialName(int materialIndex) const {
...
int CPhysicsSurfaceProps::GetReservedSurfaceIndex(
...
const IVP_Material *
CPhysicsSurfaceProps::GetIVPMaterial(int materialIndex) const {
...
void CPhysicsSurfaceProps::CopyPhysicsProperties(CSurface *pOut,
                                                 int baseIndex) {
...
int CPhysicsSurfaceProps::ParseSurfaceData(const char *pFileName,
  ...
  if (!m_init) {
    ...
    memset(&prop.data, 0, sizeof(prop.data));
    prop.m_name =
        m_strings.AddString(GetReservedMaterialName(MATERIAL_INDEX_SHADOW));
    CopyPhysicsProperties(&prop, baseMaterial);
...
void CPhysicsSurfaceProps::SetWorldMaterialIndexTable(int *pMapArray,
                                                      int mapSize) {
  m_ivpManager.SetPropMap(pMapArray, mapSize);
}

CIVPMaterialManager::CIVPMaterialManager(void)
    : IVP_Material_Manager(IVP_FALSE) {
...
IVP_Material *CIVPMaterialManager::get_material_by_index(
    IVP_Real_Object *pObject, const IVP_U_Point *world_position, int index) {
...
// per-triangle indices on load.  If we wanted to support multiple LUTs, it
// would be better to preprocess/remap the triangles in the collision models at
// load time
