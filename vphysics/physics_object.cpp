...
// UNDONE: Make this a stack variable / member variable of some save/load object
// or function? NOTE: This keeps a list of objects who were saved while asleep,
// but not created asleep So some info will be lost unless it's regenerated
// after loading.
...
// This angular basis is the integral of each differential drag area's torque
// over the whole OBB For each axis, each face, the integral is (1/Iaxis) * (1/3
// * w^2l^3 + 1/2 * w^4l + lw^2h^2) l,w, & h are half widths - where l is in the
// direction of the axis, w is in the plane (l/w plane) of the face, and h is
// perpendicular to the face.  So for each axis, you sum up this integral over 2
// pairs of faces (this function returns the integral for one pair of opposite
// faces, not one face)
static float AngDragIntegral(float invInertia, float l, float w, float h) {
  ...
  return invInertia *
         ((1.f / 3.f) * w2 * l * l2 + 0.5 * w2 * w2 * l + l * w2 * h2);
}
...
void CPhysicsObject::Init(const CPhysCollide *pCollisionModel,
                          IVP_Real_Object *pObject, int materialIndex,
...
CPhysicsObject::~CPhysicsObject(void) {
  ...
  if (m_pObject) {
    ...
    if (pCore->physical_unmoveable == IVP_TRUE &&
        pCore->controllers_of_core.n_elems) {
      ...
      for (int i = pCore->controllers_of_core.len() - 1; i >= 0; i--) {
        IVP_Controller *my_controller =
            pCore->controllers_of_core.element_at(i);
    ...
    MEM_ALLOC_CREDIT();
    if (m_forceSilentDelete || (pVEnv && pVEnv->ShouldQuickDelete()) ||
        !m_hasTouchedDynamic) {
...
void CPhysicsObject::Wake(void) { m_pObject->ensure_in_simulation(); }

void CPhysicsObject::WakeNow(void) { m_pObject->ensure_in_simulation_now(); }

// supported
void CPhysicsObject::Sleep(void) { m_pObject->disable_simulation(); }
...
void CPhysicsObject::NotifySleep(void) {
  if (m_sleepState == OBJ_AWAKE) {
    ...
    // UNDONE: This fails sometimes and we get sleep calls for a sleeping
    // object, debug?
...
unsigned short CPhysicsObject::GetCallbackFlags() const { return m_callbacks; }

void CPhysicsObject::SetGameFlags(unsigned short userFlags) {
  m_gameFlags = userFlags;
}

unsigned short CPhysicsObject::GetGameFlags() const { return m_gameFlags; }

void CPhysicsObject::SetGameIndex(unsigned short gameIndex) {
  m_gameIndex = gameIndex;
}

unsigned short CPhysicsObject::GetGameIndex() const { return m_gameIndex; }
...
void CPhysicsObject::EnableCollisions(bool enable) {
  if (enable) {
    ...
    if (IsCollisionEnabled()) {
      // Delete all contact points with this physics object because it's
      // collision is becoming disabled
...
void CPhysicsObject::RecheckContactPoints() {
  ...
  IPhysicsFrictionSnapshot *pSnapshot = CreateFrictionSnapshot();
  while (pSnapshot->IsValid()) {
    CPhysicsObject *pOther =
        static_cast<CPhysicsObject *>(pSnapshot->GetObject(1));
    if (!coll_filter->check_objects_for_collision_detection(
...
bool CPhysicsObject::IsGravityEnabled() const {
  if (!IsStatic()) {
    return IsControlling(
        m_pObject->get_core()->environment->get_gravity_controller());
  }
...
void CPhysicsObject::RecomputeDragBases() {
  ...
  // scale those extents by appropriate mass/inertia to compute velocity
  // directly (not force) in the controller NOTE: Compute these even if drag
  // coefficients are zero, because the drag coefficient could change later

  // Get an AABB for this object and use the area of each side as a basis for
  // approximating cross-section area for drag
  Vector dragMins, dragMaxs;
  // NOTE: coordinates in/out of physcollision are in HL units, not IVP
  // PERFORMANCE: Cache this?  Expensive.
  physcollision->CollideGetAABB(&dragMins, &dragMaxs, GetCollide(), vec3_origin,
                                vec3_angle);

  Vector areaFractions =
      physcollision->CollideGetOrthographicAreas(GetCollide());
  ...
  const IVP_U_Float_Point *pInvRI =
      m_pObject->get_core()->get_inv_rot_inertia();

  // This angular basis is the integral of each differential drag area's torque
  // over the whole OBB need half lengths for this integral
  delta *= 0.5;
  // rotation about the x axis
  m_angDragBasis.x = areaFractions.z * AngDragIntegral(pInvRI->k[0], delta.x,
                                                       delta.y, delta.z) +
                     areaFractions.y * AngDragIntegral(pInvRI->k[0], delta.x,
                                                       delta.z, delta.y);
  // rotation about the y axis
  m_angDragBasis.y = areaFractions.z * AngDragIntegral(pInvRI->k[1], delta.y,
                                                       delta.x, delta.z) +
                     areaFractions.x * AngDragIntegral(pInvRI->k[1], delta.y,
                                                       delta.z, delta.x);
  // rotation about the z axis
  m_angDragBasis.z = areaFractions.y * AngDragIntegral(pInvRI->k[2], delta.z,
                                                       delta.x, delta.y) +
                     areaFractions.x * AngDragIntegral(pInvRI->k[2], delta.z,
                                                       delta.y, delta.x);
}
...
void CPhysicsObject::DestroyFrictionSnapshot(
...
bool CPhysicsObject::IsMassCenterAtDefault() const {
  ...
  return (delta <= g_PhysicsUnits.collisionSweepIncrementalEpsilon) ? true
                                                                    : false;
}
...
Vector CPhysicsObject::GetMassCenterLocalSpace() const {
  ...
  // so we want the negative core shift (origin relative position of the mass
  // center)
  return -out;
}

void CPhysicsObject::SetGameData(void *pGameData) { m_pGameData = pGameData; }

void *CPhysicsObject::GetGameData(void) const { return m_pGameData; }
...
void CPhysicsObject::SetVolume(float volume) {
  ...
  if (volume != 0.f) {
    ...
    float matDensity;
    physprops->GetPhysicsProperties(GetMaterialIndexInternal(), &matDensity,
                                    NULL, NULL, NULL);
...
float CPhysicsObject::GetVolume() const { return m_volume; }

void CPhysicsObject::SetBuoyancyRatio(float ratio) { m_buoyancyRatio = ratio; }
...
void CPhysicsObject::LocalToWorld(Vector *worldPosition,
                                  const Vector &localPosition) const {
  matrix3x4_t matrix;
...
void CPhysicsObject::WorldToLocal(Vector *localPosition,
...
void CPhysicsObject::LocalToWorldVector(Vector *worldVector,
...
void CPhysicsObject::WorldToLocalVector(Vector *localVector,
                                        const Vector &worldVector) const {
  matrix3x4_t matrix;
  GetPositionMatrix(&matrix);
  // copy in case the src == dest
...
void CPhysicsObject::ApplyForceOffset(const Vector &forceVector,
...
void CPhysicsObject::CalculateForceOffset(const Vector &forceVector,
                                          const Vector &worldPosition,
                                          Vector *centerForce,
...
void CPhysicsObject::CalculateVelocityOffset(
  ...
  cross_point_dir.set_pairwise_mult(&cross_point_dir,
...
void CPhysicsObject::GetImplicitVelocity(
    Vector *velocity, AngularImpulse *angularVelocity) const {
  ...
  if (angularVelocity) {
    // compute the relative transform that was actually integrated in the last
    // psi
    ...
    Quaternion q(q_core_f_core.x, q_core_f_core.y, q_core_f_core.z,
                 q_core_f_core.w);
    AngularImpulse axis;
    float angle;
    ...
    // ConvertDirectionToHL() - convert this ipion direction (in HL type) to HL
    // coords
    float tmpY = axis.z;
...
void CPhysicsObject::GetVelocity(Vector *velocity,
                                 AngularImpulse *angularVelocity) const {
  if (!velocity && !angularVelocity)
    return;
...
void CPhysicsObject::GetVelocityAtPoint(const Vector &worldPosition,
                                        Vector *pVelocity) const {
  IVP_Core *core = m_pObject->get_core();
  IVP_U_Point pos;
  ConvertPositionToIVP(worldPosition, pos);
...
// UNDONE: Limit these?
void CPhysicsObject::AddVelocity(const Vector *velocity,
...
void CPhysicsObject::SetPosition(const Vector &worldPosition,
                                 const QAngle &angles, bool isTeleport) {
  IVP_U_Quat rot;
  IVP_U_Point pos;
...
void CPhysicsObject::SetPositionMatrix(const matrix3x4_t &matrix,
                                       bool isTeleport) {
  if (m_pShadow) {
...
void CPhysicsObject::SetVelocityInstantaneous(
    const Vector *velocity, const AngularImpulse *angularVelocity) {
  Assert(IsMoveable());
  if (!IsMoveable())
    return;
  IVP_Core *core = m_pObject->get_core();
...
void CPhysicsObject::SetVelocity(const Vector *velocity,
                                 const AngularImpulse *angularVelocity) {
  if (!IsMoveable())
    return;
  IVP_Core *core = m_pObject->get_core();
...
void GetWorldCoordFromSynapse(IVP_Synapse_Friction *pfriction,
                              IVP_U_Point &world) {
  world.set(pfriction->get_contact_point()->get_contact_point_ws());
}

bool CPhysicsObject::GetContactPoint(Vector *contactPoint,
                                     IPhysicsObject **contactObject) const {
...
void CPhysicsObject::SetShadow(float maxSpeed, float maxAngularSpeed,
  ...
  if (m_pShadow) {
    ...
    CPhysicsEnvironment *pVEnv = GetVPhysicsEnvironment();
    m_pShadow = pVEnv->CreateShadowController(this, allowPhysicsMovement,
                                              allowPhysicsRotation);
    m_pShadow->MaxSpeed(maxSpeed, maxAngularSpeed);
    // This really should be in the game code, but do this here because the game
    // may (does) use shadow/AI control as a collision filter indicator.
    RecheckCollisionFilter();
  }
}

void CPhysicsObject::UpdateShadow(const Vector &targetPosition,
...
// Back door to allow save/restore of backlink between shadow controller and
// physics object
void CPhysicsObject::RestoreShadowController(
...
float CPhysicsObject::GetDragInDirection(
    const IVP_U_Float_Point &velocity) const {
  IVP_U_Float_Point local;

  const IVP_U_Matrix *m_world_f_core =
      m_pObject->get_core()->get_m_world_f_core_PSI();
  m_world_f_core->vimult3(&velocity, &local);

  return m_dragCoefficient *
             IVP_Inline_Math::fabsd(local.k[0] * m_dragBasis.x) +
         IVP_Inline_Math::fabsd(local.k[1] * m_dragBasis.y) +
         IVP_Inline_Math::fabsd(local.k[2] * m_dragBasis.z);
}

float CPhysicsObject::GetAngularDragInDirection(
    const IVP_U_Float_Point &angVelocity) const {
  return m_angDragCoefficient *
             IVP_Inline_Math::fabsd(angVelocity.k[0] * m_angDragBasis.x) +
         IVP_Inline_Math::fabsd(angVelocity.k[1] * m_angDragBasis.y) +
         IVP_Inline_Math::fabsd(angVelocity.k[2] * m_angDragBasis.z);
}

const char *CPhysicsObject::GetName() const { return m_pObject->get_name(); }
...
float CPhysicsObject::GetEnergy() const {
  ...
  energy = 0.5f * pCore->get_mass() *
           pCore->speed.dot_product(&pCore->speed);                   // 1/2mvv
...
float CPhysicsObject::ComputeShadowControl(
    const hlshadowcontrol_params_t &params, float secondsToArrival, float dt) {
...
float CPhysicsObject::CalculateAngularDrag(
    const Vector &objectSpaceRotationAxis) const {
...
void CPhysicsObject::BecomeTrigger() {
  ...
  if (GetShadowController()) {
    // triggers won't have the standard collisions, so the material change is no
    // longer necessary also: This will fix problems with surfaceprops if the
    // trigger becomes a fluid.
  ...
  trigger.manage_intruding_cores =
      IVP_TRUE;                             // manage a list of intruded objects
...
// sets the object to be hinged.  Fixed it place, but able to rotate around one
// axis.
void CPhysicsObject::BecomeHinged(int localAxis) {
  ...
  if (IsMoveable()) {
    ...
    IVP_U_Float_Hesse *iri =
        (IVP_U_Float_Hesse *)m_pObject->get_core()->get_inv_rot_inertia();
...
void CPhysicsObject::OutputDebugInfo() const {
  ...
  Msg("Inertia: %.3e, %.3e, %.3e (inv %.3e, %.3e, %.3e)\n", inertia.x,
      inertia.y, inertia.z, invInertia.x, invInertia.y, invInertia.z);
  ...
  Msg("Linear Drag: %.2f, %.2f, %.2f (factor %.2f)\n", m_dragBasis.x,
      m_dragBasis.y, m_dragBasis.z, m_dragCoefficient);
  Msg("Angular Drag: %.2f, %.2f, %.2f (factor %.2f)\n", m_angDragBasis.x,
      m_angDragBasis.y, m_angDragBasis.z, m_angDragCoefficient);
  ...
  for (int k = m_pObject->get_core()->controllers_of_core.len() - 1; k >= 0;
       k--) {
    // NOTE: Set a breakpoint here and take a look at what it's hooked to
    IVP_Controller *pController =
        m_pObject->get_core()->controllers_of_core.element_at(k);
    Msg("%d) %s\n", k, pController->get_controller_name());
  }
  Msg("State: %s, Collision %s, Motion %s, %sFlags %04X (game %04x, index "
      "%d)\n",
  ...
  physprops->GetPhysicsProperties(GetMaterialIndexInternal(), &matDensity,
                                  &matThickness, &matFriction, &matElasticity);
  Msg("Material: %s : density(%.1f), thickness(%.2f), friction(%.2f), "
      "elasticity(%.2f)\n",
      physprops->GetPropName(GetMaterialIndexInternal()), matDensity,
      matThickness, matFriction, matElasticity);
  if (GetCollide()) {
...
static void InitObjectTemplate(IVP_Template_Real_Object &objectTemplate,
                               int materialIndex, objectparams_t *pParams,
                               bool isStatic) {
  objectTemplate.mass =
      clamp(pParams->mass, VPHYSICS_MIN_MASS, VPHYSICS_MAX_MASS);
  ...
  objectTemplate.rot_speed_damp_factor.set(
      pParams->rotdamping, pParams->rotdamping, pParams->rotdamping);
  objectTemplate.speed_damp_factor = pParams->damping;
  objectTemplate.auto_check_rot_inertia = pParams->rotInertiaLimit;
}

CPhysicsObject *CreatePhysicsObject(CPhysicsEnvironment *pEnvironment,
  ...
  short collideType;
  IVP_SurfaceManager *pSurman =
      CreateSurfaceManager(pCollisionModel, collideType);
  if (!pSurman)
    return NULL;
  ...
  IVP_Polygon *realObject = pEnvironment->GetIVPEnvironment()->create_polygon(
      pSurman, &objectTemplate, &rotation, &pos);

  pObject->Init(pCollisionModel, realObject, materialIndex, pParams->volume,
                pParams->dragCoefficient, pParams->dragCoefficient);
...
CPhysicsObject *CreatePhysicsSphere(CPhysicsEnvironment *pEnvironment,
                                    float radius, int materialIndex,
  ...
  MEM_ALLOC_CREDIT();
  IVP_Ball *realObject = pEnvironment->GetIVPEnvironment()->create_ball(
      &ballTemplate, &objectTemplate, &rotation, &pos);
  ...
  CPhysicsObject *pObject = new CPhysicsObject();
  pObject->Init(NULL, realObject, materialIndex, volume, 0,
                0); //, pParams->dragCoefficient, pParams->dragCoefficient
  pObject->SetGameData(pParams->pGameData);
...
ISaveRestoreOps *MaterialIndexDataOps() { return &g_MaterialIndexDataOps; }

BEGIN_SIMPLE_DATADESC(vphysics_save_cphysicsobject_t)
//	DEFINE_FIELD( pCollide,			FIELD_???	),	// don't
//save this 	DEFINE_FIELD( pName,				FIELD_???
//),	// don't save this
DEFINE_FIELD(sphereRadius, FIELD_FLOAT), DEFINE_FIELD(isStatic, FIELD_BOOLEAN),
    ...
    DEFINE_FIELD(mass, FIELD_FLOAT), DEFINE_FIELD(rotInertia, FIELD_VECTOR),
    ...
    DEFINE_FIELD(angles, FIELD_VECTOR), DEFINE_FIELD(velocity, FIELD_VECTOR),
...
void CPhysicsObject::WriteToTemplate(
...
void CPhysicsObject::InitFromTemplate(
    CPhysicsEnvironment *pEnvironment, void *pGameData,
  ...
  if (objectTemplate.materialIndex >= 0) {
    ivpObjectTemplate.material =
        physprops->GetIVPMaterial(objectTemplate.materialIndex);
  } else {
    ivpObjectTemplate.material =
        physprops->GetIVPMaterial(physprops->GetSurfaceIndex("default"));
  }
  ...
  ivpObjectTemplate.physical_unmoveable =
      objectTemplate.isStatic ? IVP_TRUE : IVP_FALSE;
  ivpObjectTemplate.rot_inertia_is_factor = IVP_TRUE;

  ivpObjectTemplate.rot_inertia.set(1, 1, 1);
  ivpObjectTemplate.rot_speed_damp_factor.set(objectTemplate.rotSpeedDamping,
  ...
  if (m_collideType == COLLIDE_BALL) {
    ...
    realObject = pEnvironment->GetIVPEnvironment()->create_ball(
        &ballTemplate, &ivpObjectTemplate, &rotation, &pos);
  } else {
    short collideType;
    IVP_SurfaceManager *surman =
        CreateSurfaceManager(objectTemplate.pCollide, collideType);
    m_collideType = collideType;
    realObject = pEnvironment->GetIVPEnvironment()->create_polygon(
        surman, &ivpObjectTemplate, &rotation, &pos);
  }

  m_pObject = realObject;
  SetInertia(objectTemplate.rotInertia);
  Init(objectTemplate.pCollide, realObject, objectTemplate.materialIndex,
       objectTemplate.volume, objectTemplate.dragCoefficient,
       objectTemplate.dragCoefficient);
  ...
  if (objectTemplate.velocity.LengthSqr() != 0 ||
      objectTemplate.angVelocity.LengthSqr() != 0) {
    // will wake up the object
    SetVelocityInstantaneous(&objectTemplate.velocity,
                             &objectTemplate.angVelocity);
...
bool SavePhysicsObject(const physsaveparams_t &params,
                       CPhysicsObject *pObject) {
  vphysics_save_cphysicsobject_t objectTemplate;
...
bool RestorePhysicsObject(const physrestoreparams_t &params,
                          CPhysicsObject **ppObject) {
  vphysics_save_cphysicsobject_t objectTemplate;
  memset(&objectTemplate, 0, sizeof(objectTemplate));
  ...
  *ppObject = new CPhysicsObject();
  ...
  if (objectTemplate.collisionEnabled) {
    // queue up the collision enable for these in case their entities have other
    // dependent physics handlers (like controllers) that need to be restored
    // before callbacks are useful
    entry.pObject = *ppObject;
    entry.enableCollisions = true;
    objectTemplate.collisionEnabled = false;
  }

  (*ppObject)->InitFromTemplate(
      static_cast<CPhysicsEnvironment *>(params.pEnvironment), params.pGameData,
      objectTemplate);

  if ((*ppObject)->IsAsleep() && !(*ppObject)->m_asleepSinceCreation &&
      !(*ppObject)->IsStatic()) {
    entry.pObject = *ppObject;
  ...
  if (objectTemplate.hasShadowController) {
    bool restored = RestorePhysicsShadowControllerInternal(
        params, &objectTemplate.pShadow, *ppObject);
...
IPhysicsObject *CreateObjectFromBuffer(CPhysicsEnvironment *pEnvironment,
                                       void *pGameData, unsigned char *pBuffer,
  ...
  if (bufferSize >= sizeof(vphysics_save_cphysicsobject_t)) {
    ...
    pTemplate->hasShadowController =
        false; // this hasn't been saved separately so cannot be supported via
               // this path
...
IPhysicsObject *CreateObjectFromBuffer_UseExistingMemory(
    CPhysicsEnvironment *pEnvironment, void *pGameData, unsigned char *pBuffer,
    unsigned int bufferSize, CPhysicsObject *pExistingMemory) {
  if (bufferSize >= sizeof(vphysics_save_cphysicsobject_t)) {
    vphysics_save_cphysicsobject_t *pTemplate =
        reinterpret_cast<vphysics_save_cphysicsobject_t *>(pBuffer);
// Allow the placement new. If we don't do this, then it'll get a compile error
// because new might be defined as the special form in MEMALL_DEBUG_NEW.
...
// regenerate the friction systems for these objects.  Because when it was saved
// it had them (came to rest with the contact points). So now we need to
// recreate them or some objects may not wake up when this object (or its
  ...
  for (int i = g_PostRestoreObjectList.Count() - 1; i >= 0; --i) {
    if (g_PostRestoreObjectList[i].pObject) {
      if (g_PostRestoreObjectList[i].growFriction) {
        g_PostRestoreObjectList[i]
            .pObject->GetObject()
            ->force_grow_friction_system();
      }
