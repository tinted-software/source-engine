...
template <typename T> class CDeleteProxy : public IDeleteQueueItem {
public:
...
class CDeleteQueue {
public:
  void Add(IDeleteQueueItem *pItem) { m_list.AddToTail(pItem); }

  template <typename T> void QueueForDelete(T *pItem) {
  ...
  virtual void GetSurfaceNormal(Vector &out) {
    if (m_pContact) {
      ...
      IVP_Contact_Point_API::get_surface_normal_ws(
          const_cast<IVP_Contact_Point *>(m_pPoint), &normal);
...
public:
  ...
  void SetHandler(IPhysicsObjectEvent *pListener) { m_pCallback = pListener; }

  void Remove(int index) {
    // fast remove preserves indices except for the last element (moved into the
    // empty spot)
  ...
  void event_object_deleted(IVP_Event_Object *pEvent) {
    CPhysicsObject *pObject =
        static_cast<CPhysicsObject *>(pEvent->real_object->client_data);
    if (!pObject)
      return;
  ...
  void event_object_revived(IVP_Event_Object *pEvent) {
    ...
    // don't track static objects (like the world).  That way we only track
  ...
  //-----------------------------------------------------------------------------
  // Purpose: This walks the objects in the environment and generates friction
  // events
  //			for any scraping that is occurring.
  //-----------------------------------------------------------------------------
  void ProcessActiveObjects(IVP_Environment *pEnvironment,
    ...
    for (int i = 0; i < m_activeObjects.Count(); i++) {
      ...
      // UNDONE: IVP_Synapse_Friction is supposed to be opaque.  Is there a
      // better way to implement this?  Using the friction listener is much more
      // work for the CPU and considers sleeping objects.
      ...
      while (pfriction) {
        ...
        if (pScrape->CallbackFlags() & CALLBACK_GLOBAL_FRICTION) {
          ...
          if (energy) {
            ...
            // This assumes that the game is interested in some measure of
            // vibration for sound effects.  This also assumes that more massive
            // objects require more energy to vibrate.
            ...
            if (energy > 0.05f) {
              ...
              pEvent->Friction(pObject, ConvertEnergyToHL(energy),
                               pObject->GetMaterialIndexInternal(), hitSurface,
                               &data);
            }
  ...
  void DebugCheckContacts(IVP_Environment *pEnvironment) {
    ...
    for (IVP_Mindist *mdist = pManager->exact_mindists; mdist != NULL;
         mdist = mdist->next) {
      IVP_Real_Object *obj[2];
      mdist->get_objects(obj);
      IVP_BOOL check =
          pEnvironment->get_collision_filter()
              ->check_objects_for_collision_detection(obj[0], obj[1]);
      Assert(check);
      if (!check) {
  ...
  int GetActiveObjectCount(void) const { return m_activeObjects.Count(); }
...
public:
  ...
  virtual void Friction(IPhysicsObject *pObject, float energy, int surfaceProps,
...
//-----------------------------------------------------------------------------
class CPhysicsListenerCollision : public IVP_Listener_Collision,
                                  public IVP_Listener_Phantom {
public:
  ...
  virtual void event_pre_collision(IVP_Event_Collision *pEvent) {
    ...
    CPhysicsObject *pObject1 =
        static_cast<CPhysicsObject *>(contact->objects[0]->client_data);
    CPhysicsObject *pObject2 =
        static_cast<CPhysicsObject *>(contact->objects[1]->client_data);
    ...
    m_event.isCollision =
        (flags1 & flags2 & CALLBACK_GLOBAL_COLLISION) ? true : false;

    // only call shadow collisions if one is shadow and the other isn't (hence
    // the xor) (if both are shadow, the collisions happen in AI - if neither,
    // then no callback)
    m_event.isShadowCollision =
        ((flags1 ^ flags2) & CALLBACK_SHADOW_COLLISION) ? true : false;

    m_event.pObjects[0] = pObject1;
    m_event.pObjects[1] = pObject2;
    m_event.deltaCollisionTime = pEvent->d_time_since_last_collision;
    // This timer must have been reset or something (constructor initializes
    // time to -1000) Fake the time to 50ms (resets happen often in rolling
    ...
    for (int i = 0; i < 2; i++) {
      m_event.surfaceProps[i] =
          physprops->GetIVPMaterialIndex(contact->materials[i]);
  ...
  virtual void mindist_entered_volume(class IVP_Controller_Phantom *controller,
                                      class IVP_Mindist_Base *mindist) {}
  virtual void mindist_left_volume(class IVP_Controller_Phantom *controller,
                                   class IVP_Mindist_Base *mindist) {}

  virtual void core_entered_volume(IVP_Controller_Phantom *controller,
                                   IVP_Core *pCore) {
    CPhysicsFluidController *pFluid =
        static_cast<CPhysicsFluidController *>(controller->client_data);
    ...
    if (pFluid) {
      ...
      CPhysicsObject *pTrigger =
          static_cast<CPhysicsObject *>(pTriggerIVP->client_data);
  ...
  virtual void core_left_volume(IVP_Controller_Phantom *controller,
                                IVP_Core *pCore) {
    CPhysicsFluidController *pFluid =
        static_cast<CPhysicsFluidController *>(controller->client_data);
    IVP_Real_Object *pivp = pCore->objects.element_at(0);
  ...
  void phantom_is_going_to_be_deleted_event(
      class IVP_Controller_Phantom *controller) {}
...
private:
  ...
  void UpdatePairListPSI(CPhysicsEnvironment *pEnvironment) {
    ...
    IVP_Time currentTime =
        pEnvironment->GetIVPEnvironment()->get_current_time();
...
void CPhysicsListenerCollision::event_friction_pair_created(
  ...
  if (m_pairList.IsValidIndex(index)) {
    ...
    if (save.lastImpactTime.get_seconds() >
        pair->last_impact_time_pair.get_seconds()) {
...
void CPhysicsListenerCollision::event_friction_pair_deleted(
    IVP_Friction_Core_Pair *pair) {
  corepair_t test(pair);
  unsigned short index = m_pairList.Find(test);
  if (m_pairList.IsValidIndex(index)) {
...
class CCollisionVisualizer
    : public IVP_Clustering_Visualizer_Shortrange_Callback,
...
public:
  ...
  void visualize_request() {
    ...
    m_pDebug->AddBoxOverlay(origin, -extents, extents, vec3_angle, 0, 255, 0,
                            32, 0.5f);
  }
  ...
  void visualize_request_for_node() {
    ...
    CPhysicsObject *pObject0 =
        static_cast<CPhysicsObject *>(node_object->client_data);
    pObject0->LocalToWorld(boxOrigin, origin);
    QAngle angles;
    pObject0->GetPosition(NULL, &angles);

    m_pDebug->AddBoxOverlay(boxOrigin, -extents, extents, angles, 255, 255, 0,
                            0, 0.5f);
  }
  ...
  void visualize_request_for_intruder_radius() {
    ...
    extents.Init(hlradius, hlradius, hlradius);
    m_pDebug->AddBoxOverlay(origin, -extents, extents, vec3_angle, 0, 0, 255,
                            32, 0.25f);
  }
};
#endif

class CCollisionSolver : public IVP_Collision_Filter,
                         public IVP_Anomaly_Manager {
public:
  ...
  IVP_BOOL check_objects_for_collision_detection(IVP_Real_Object *ivp0,
                                                 IVP_Real_Object *ivp1) {
    if (m_pSolver) {
      CPhysicsObject *pObject0 =
          static_cast<CPhysicsObject *>(ivp0->client_data);
      CPhysicsObject *pObject1 =
          static_cast<CPhysicsObject *>(ivp1->client_data);
      if (pObject0 && pObject1) {
        ...
        if (!m_pSolver->ShouldCollide(pObject0, pObject1,
                                      pObject0->GetGameData(),
  ...
  // IVP_Anomaly_Manager
  virtual void inter_penetration(IVP_Mindist *mindist, IVP_Real_Object *ivp0,
                                 IVP_Real_Object *ivp1,
    ...
    if (m_pSolver) {
      ...
      if (pObject0 && pObject1) {
        ...
        if (!m_pSolver->ShouldSolvePenetration(
                pObject0, pObject1, pObject0->GetGameData(),
  ...
  virtual IVP_BOOL max_collisions_exceeded_check_freezing(IVP_Anomaly_Limits *,
                                                          IVP_Core *pCore) {
    if (m_pSolver) {
      CPhysicsObject *pObject = static_cast<CPhysicsObject *>(
          pCore->objects.element_at(0)->client_data);
  ...
  void max_velocity_exceeded(IVP_Anomaly_Limits *al, IVP_Core *pCore,
  ...
  IVP_BOOL max_contacts_exceeded_check_freezing(IVP_Core **pCoreList,
                                                int coreCount) {
    ...
    return m_pSolver->ShouldFreezeContacts(list.Base(), list.Count())
               ? IVP_TRUE
               : IVP_FALSE;
  }

public:
  void EventPSI(CPhysicsEnvironment *) { m_rescue.RemoveAll(); }
...
private:
  ...
  // UNDONE: Linear search? should be small, but switch to rb tree if this ever
  // gets large
...
public:
  CPhysicsListenerConstraint() { m_pCallback = NULL; }

  void SetHandler(IPhysicsConstraintEvent *pHandler) { m_pCallback = pHandler; }
...
public:
  CDragController(void) { m_airDensity = AIR_DENSITY; }
  virtual ~CDragController(void) {}

  virtual void do_simulation_controller(IVP_Event_Sim *event,
    ...
    for (i = core_list->len() - 1; i >= 0; i--) {
      ...
      float dragForce = -0.5 * pPhys->GetDragInDirection(pCore->speed) *
                        m_airDensity * event->delta_time;
      if (dragForce < -1.0f)
        dragForce = -1.0f;
      if (dragForce < 0) {
      ...
      float angDragForce = -pPhys->GetAngularDragInDirection(pCore->rot_speed) *
                           m_airDensity * event->delta_time;
...
//
// Default implementation of the debug overlay interface so that we never return
// NULL from GetDebugOverlay.
//
...
public:
  ...
  virtual void AddLineOverlay(const Vector &origin, const Vector &dest, int r,
                              int g, int b, bool noDepthTest, float duration) {}
...
// assume that these lists will have at least one object
{
  ...
  // UNDONE: Revisit brush/terrain/object shrinking and tune this number to
  // something larger UNDONE: Expose this to callers, also via physcollision
  m_pPhysEnv->set_global_collision_tolerance(
      ConvertDistanceToIVP(g_PhysicsUnits.globalCollisionTolerance -
                           1e-4f)); // just under 1/4 inch tolerance
...
void CPhysicsEnvironment::NotifyConstraintDisabled(
...
void CPhysicsEnvironment::SetDebugOverlay(
    CreateInterfaceFn debugOverlayFactory) {
  m_pDebugOverlay = NULL;
  if (debugOverlayFactory) {
    m_pDebugOverlay = (IVPhysicsDebugOverlay *)debugOverlayFactory(
        VPHYSICS_DEBUG_OVERLAY_INTERFACE_VERSION, NULL);
  }
...
void CPhysicsEnvironment::SetGravity(const Vector &gravityVector) {
  ...
  m_pPhysEnv->set_global_collision_tolerance(
      m_pPhysEnv->get_global_collision_tolerance(), gravity.real_length());
...
IPhysicsObject *CPhysicsEnvironment::CreatePolyObject(
  ...
  IPhysicsObject *pObject = ::CreatePhysicsObject(
      this, pCollisionModel, materialIndex, position, angles, pParams, false);
  if (pObject) {
...
IPhysicsObject *CPhysicsEnvironment::CreatePolyObjectStatic(
    ...
    const Vector &position, const QAngle &angles, objectparams_t *pParams) {
  IPhysicsObject *pObject = ::CreatePhysicsObject(
      this, pCollisionModel, materialIndex, position, angles, pParams, true);
  if (pObject) {
    m_objects.AddToTail(pObject);
  }
  return pObject;
...
unsigned int
CPhysicsEnvironment::GetObjectSerializeSize(IPhysicsObject *pObject) const {
  return sizeof(vphysics_save_cphysicsobject_t);
}

void CPhysicsEnvironment::SerializeObjectToBuffer(IPhysicsObject *pObject,
                                                  unsigned char *pBuffer,
                                                  unsigned int bufferSize) {
  CPhysicsObject *pPhysics = static_cast<CPhysicsObject *>(pObject);
  if (bufferSize >= sizeof(vphysics_save_cphysicsobject_t)) {
    vphysics_save_cphysicsobject_t *pTemplate =
        reinterpret_cast<vphysics_save_cphysicsobject_t *>(pBuffer);
    pPhysics->WriteToTemplate(*pTemplate);
  }
}

IPhysicsObject *CPhysicsEnvironment::UnserializeObjectFromBuffer(
    void *pGameData, unsigned char *pBuffer, unsigned int bufferSize,
    bool enableCollisions) {
  IPhysicsObject *pObject = ::CreateObjectFromBuffer(
      this, pGameData, pBuffer, bufferSize, enableCollisions);
  if (pObject) {
    m_objects.AddToTail(pObject);
  }
  return pObject;
...
const IPhysicsObject **
CPhysicsEnvironment::GetObjectList(int *pOutputObjectCount) const {
...
extern void ControlPhysicsShadowControllerAttachment_Silent(
    IPhysicsShadowController *pController, IVP_Real_Object *pivp, bool bAttach);
extern void ControlPhysicsPlayerControllerAttachment_Silent(
    IPhysicsPlayerController *pController, IVP_Real_Object *pivp, bool bAttach);

bool CPhysicsEnvironment::TransferObject(
    IPhysicsObject *pObject, IPhysicsEnvironment *pDestinationEnvironment) {
  int iIndex = m_objects.Find(pObject);
  if (iIndex == -1 ||
      (pObject->GetCallbackFlags() & CALLBACK_MARKED_FOR_DELETE))
    return false;
  ...
  // temporarily (and silently) detach any physics controllers we found because
  // destroying the object would destroy them
  if (pController) {
    ...
    IVP_Real_Object *pivp = ((CPhysicsObject *)pObject)->GetObject();
    ControlPhysicsPlayerControllerAttachment_Silent(pPlayerController, pivp,
                                                    false);
  }
  ...
  CPhysicsEnvironment *pDest =
      static_cast<CPhysicsEnvironment *>(pDestinationEnvironment);
  CreateObjectFromBuffer_UseExistingMemory(pDest, pGameData,
                                           (unsigned char *)&objectTemplate,
                                           sizeof(objectTemplate), pPhysics);
  pDest->m_objects.AddToTail(pObject);

  // even if this is going to sleep in a second, put it active right away to fix
  // some object hitching problems
  pPhysics->Wake();
  pPhysics->NotifyWake();
  /*int iActiveIndex = pDest->m_pSleepEvents->m_activeObjects.AddToTail(
  pPhysics ); pPhysics->SetActiveIndex( iActiveIndex );*/
...
IPhysicsSpring *CPhysicsEnvironment::CreateSpring(IPhysicsObject *pObjectStart,
...
CPhysicsEnvironment::CreateFluidController(IPhysicsObject *pFluidObject,
                                           fluidparams_t *pParams) {
  CPhysicsFluidController *pFluid = ::CreateFluidController(
      m_pPhysEnv, static_cast<CPhysicsObject *>(pFluidObject), pParams);
  m_fluids.AddToTail(pFluid);
  return pFluid;
}

IPhysicsConstraint *CPhysicsEnvironment::CreateRagdollConstraint(
  ...
  return ::CreateRagdollConstraint(
      m_pPhysEnv, (CPhysicsObject *)pReferenceObject,
      (CPhysicsObject *)pAttachedObject, pGroup, ragdoll);
}
...
IPhysicsConstraint *CPhysicsEnvironment::CreateHingeConstraint(
  ...
  return ::CreateHingeConstraint(m_pPhysEnv, (CPhysicsObject *)pReferenceObject,
                                 (CPhysicsObject *)pAttachedObject, pGroup,
                                 limitedhinge);
}

IPhysicsConstraint *CPhysicsEnvironment::CreateLimitedHingeConstraint(
    IPhysicsObject *pReferenceObject, IPhysicsObject *pAttachedObject,
    IPhysicsConstraintGroup *pGroup,
...
IPhysicsConstraint *CPhysicsEnvironment::CreateFixedConstraint(
    IPhysicsObject *pReferenceObject, IPhysicsObject *pAttachedObject,
    IPhysicsConstraintGroup *pGroup, const constraint_fixedparams_t &fixed) {
  return ::CreateFixedConstraint(m_pPhysEnv, (CPhysicsObject *)pReferenceObject,
                                 (CPhysicsObject *)pAttachedObject, pGroup,
                                 fixed);
}

IPhysicsConstraint *CPhysicsEnvironment::CreateSlidingConstraint(
    IPhysicsObject *pReferenceObject, IPhysicsObject *pAttachedObject,
    IPhysicsConstraintGroup *pGroup,
    const constraint_slidingparams_t &sliding) {
  return ::CreateSlidingConstraint(
      m_pPhysEnv, (CPhysicsObject *)pReferenceObject,
      (CPhysicsObject *)pAttachedObject, pGroup, sliding);
}
...
IPhysicsConstraint *CPhysicsEnvironment::CreatePulleyConstraint(
    IPhysicsObject *pReferenceObject, IPhysicsObject *pAttachedObject,
    IPhysicsConstraintGroup *pGroup, const constraint_pulleyparams_t &pulley) {
  return ::CreatePulleyConstraint(
      m_pPhysEnv, (CPhysicsObject *)pReferenceObject,
      (CPhysicsObject *)pAttachedObject, pGroup, pulley);
}

IPhysicsConstraint *CPhysicsEnvironment::CreateLengthConstraint(
    IPhysicsObject *pReferenceObject, IPhysicsObject *pAttachedObject,
    IPhysicsConstraintGroup *pGroup, const constraint_lengthparams_t &length) {
  return ::CreateLengthConstraint(
      m_pPhysEnv, (CPhysicsObject *)pReferenceObject,
      (CPhysicsObject *)pAttachedObject, pGroup, length);
}

IPhysicsConstraintGroup *CPhysicsEnvironment::CreateConstraintGroup(
...
void CPhysicsEnvironment::Simulate(float deltaTime) {
  ...
  if (m_pCollisionListener->GetHandler()) {
    m_pSleepEvents->ProcessActiveObjects(m_pPhysEnv,
                                         m_pCollisionListener->GetHandler());
  }
...
void CPhysicsEnvironment::ResetSimulationClock() {
  // UNDONE: You'd think that all of this would make the system deterministic,
  // but it doesn't.
...
// physenv->Simulate())
bool CPhysicsEnvironment::IsInSimulation(void) const { return m_inSimulation; }
...
void CPhysicsEnvironment::DestroyObject(IPhysicsObject *pObject) {
  ...
  // search from the end because we usually delete the most recent objects
  // during run time
  int index = -1;
...
void CPhysicsEnvironment::DestroyFluidController(
    IPhysicsFluidController *pFluid) {
...
void CPhysicsEnvironment::DestroyConstraintGroup(
    IPhysicsConstraintGroup *pGroup) {
  delete pGroup;
}

void CPhysicsEnvironment::TraceBox(trace_t *ptr, const Vector &mins,
                                   const Vector &maxs, const Vector &start,
                                   const Vector &end) {
  // UNDONE: Need this?
}
...
void CPhysicsEnvironment::AddPlayerController(
...
void CPhysicsEnvironment::RemovePlayerController(
    IPhysicsPlayerController *pController) {
  m_playerControllers.FindAndRemove(pController);
}

IPhysicsPlayerController *
CPhysicsEnvironment::FindPlayerController(IPhysicsObject *pPhysicsObject) {
...
void CPhysicsEnvironment::SetCollisionEventHandler(
    IPhysicsCollisionEvent *pCollisionEvents) {
  m_pCollisionListener->SetHandler(pCollisionEvents);
}

void CPhysicsEnvironment::SetObjectEventHandler(
    IPhysicsObjectEvent *pObjectEvents) {
  m_pSleepEvents->SetHandler(pObjectEvents);
}

void CPhysicsEnvironment::SetConstraintEventHandler(
    IPhysicsConstraintEvent *pConstraintEvents) {
  m_pConstraintListener->SetHandler(pConstraintEvents);
}

IPhysicsShadowController *CPhysicsEnvironment::CreateShadowController(
    IPhysicsObject *pObject, bool allowTranslation, bool allowRotation) {
  return ::CreateShadowController(static_cast<CPhysicsObject *>(pObject),
                                  allowTranslation, allowRotation);
}

void CPhysicsEnvironment::DestroyShadowController(
    IPhysicsShadowController *pController) {
  delete pController;
}

IPhysicsPlayerController *
CPhysicsEnvironment::CreatePlayerController(IPhysicsObject *pObject) {
  IPhysicsPlayerController *pController =
      ::CreatePlayerController(static_cast<CPhysicsObject *>(pObject));
  AddPlayerController(pController);
  return pController;
}

void CPhysicsEnvironment::DestroyPlayerController(
    IPhysicsPlayerController *pController) {
...
void CPhysicsEnvironment::DestroyMotionController(
    IPhysicsMotionController *pController) {
  delete pController;
}

IPhysicsVehicleController *CPhysicsEnvironment::CreateVehicleController(
  ...
  return ::CreateVehicleController(
      this, static_cast<CPhysicsObject *>(pVehicleBodyObject), params,
      nVehicleType, pGameTrace);
}

void CPhysicsEnvironment::DestroyVehicleController(
...
void CPhysicsEnvironment::GetActiveObjects(
    IPhysicsObject **pOutputObjectList) const {
...
void CPhysicsEnvironment::PhantomAdd(CPhysicsObject *pObject) {
  IVP_Controller_Phantom *pPhantom =
      pObject->GetObject()->get_controller_phantom();
  if (pPhantom) {
...
IPhysicsObject *CPhysicsEnvironment::CreateSphereObject(
    float radius, int materialIndex, const Vector &position,
    const QAngle &angles, objectparams_t *pParams, bool isStatic) {
  IPhysicsObject *pObject = ::CreatePhysicsSphere(
      this, radius, materialIndex, position, angles, pParams, isStatic);
  m_objects.AddToTail(pObject);
  return pObject;
}

void CPhysicsEnvironment::TraceRay(const Ray_t &ray, unsigned int fMask,
                                   IPhysicsTraceFilter *pTraceFilter,
                                   trace_t *pTrace) {}

void CPhysicsEnvironment::SweepCollideable(
...
void CPhysicsEnvironment::GetPerformanceSettings(
    physics_performanceparams_t *pOutput) const {
  if (!pOutput)
    return;
  ...
  if (limits) {
    ...
    pOutput->maxAngularVelocity =
        ConvertAngleToHL(limits->max_angular_velocity_per_psi) *
        m_pPhysEnv->get_inv_delta_PSI_time();
...
void CPhysicsEnvironment::SetPerformanceSettings(
  ...
  if (limits) {
    ...
    limits->max_angular_velocity_per_psi =
        ...
        m_pPhysEnv->get_delta_PSI_time();
    limits->min_friction_mass =
        clamp(pSettings->minFrictionMass, 1.0f, VPHYSICS_MAX_MASS);
    limits->max_friction_mass =
        clamp(pSettings->maxFrictionMass, 1.0f, VPHYSICS_MAX_MASS);
  }
...
public:
  void AddPair(void *pObject0, void *pObject1) {
    ...
    disable_collision_between_objects((IVP_Real_Object *)pObject0,
                                      (IVP_Real_Object *)pObject1);
  }

  void RemovePair(void *pObject0, void *pObject1) {
    // enabling removes the stored hash pair
    enable_collision_between_objects((IVP_Real_Object *)pObject0,
                                     (IVP_Real_Object *)pObject1);
  }

  bool HasPair(void *pObject0, void *pObject1) {
    // If collision is enabled, the pair is NOT present, so invert the test
    // here.
    return check_objects_for_collision_detection((IVP_Real_Object *)pObject0,
                                                 (IVP_Real_Object *)pObject1)
               ? false
               : true;
  }
...
public:
  CObjectPairHash() { m_pObjectHash = new IVP_VHash_Store(1024); }

  ~CObjectPairHash() { delete m_pObjectHash; }
  ...
  void RemoveFromObjectList(void *pObject, void *pRemove) {
    ...
    for (unsigned short item = m_objectList.Head(listIndex);
         item != m_objectList.InvalidIndex(); item = m_objectList.Next(item)) {
  ...
  virtual int GetPairCountForObject(void *pObject0) {
    ...
    int nCount = 0;
    unsigned short item;
    for (item = m_objectList.Head(listIndex);
         item != m_objectList.InvalidIndex(); item = m_objectList.Next(item)) {
      ++nCount;
    }
    return nCount;
...
private:
  ...
  // this is a hash of pObjects with each element storing an index to the head
  // of its list of disabled collisions
...
IPhysicsObjectPairHash *CreateObjectPairHash() { return new CObjectPairHash; }
