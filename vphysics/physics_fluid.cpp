...
public:
  ...
  CBuoyancyAttacher(IVP_Template_Buoyancy &templ,
...
CPhysicsFluidController::CPhysicsFluidController(
    CBuoyancyAttacher *pBuoy, IVP_Liquid_Surface_Descriptor *pLiquid,
...
void CPhysicsFluidController::GetSurfacePlane(Vector *pNormal,
                                              float *pDist) const {
...
CBuoyancyAttacher::get_parameters_per_core(IVP_Core *pCore) {
  if (pCore) {
    IVP_Real_Object *pivp = pCore->objects.element_at(0);
    CPhysicsObject *pPhys = static_cast<CPhysicsObject *>(pivp->client_data);

    // This ratio is for objects whose mass / (collision model) volume is not
    // equal to their density. Keep the fluid pressure/friction solution for the
    // volume, but scale the buoyant force calculations to be in line with the
    // object's real density.  This is accompilshed by changing the fluid's
    // density on a per-object basis.
    float ratio = pPhys->GetBuoyancyRatio();

    if (pPhys->GetShadowController() ||
        !(pPhys->CallbackFlags() & CALLBACK_DO_FLUID_SIMULATION)) {
...
CBuoyancyAttacher::CBuoyancyAttacher(
    ...
    IVP_Liquid_Surface_Descriptor *liquid_surface_descriptor_)
    : IVP_Attacher_To_Cores_Buoyancy(templ, set_of_cores_,
                                     liquid_surface_descriptor_) {
...
public:
  CLiquidSurfaceDescriptor(CPhysicsObject *pFluidObject, const Vector4D &plane,
  ...
  virtual void
  calc_liquid_surface(IVP_Environment * /*environment*/, IVP_Core * /*core*/,
...
CPhysicsFluidController *CreateFluidController(IVP_Environment *pEnvironment,
                                               CPhysicsObject *pFluidObject,
                                               fluidparams_t *pParams) {
  pFluidObject->BecomeTrigger();

  IVP_Controller_Phantom *pPhantom =
      pFluidObject->GetObject()->get_controller_phantom();
  if (!pPhantom)
    return NULL;

  IVP_Liquid_Surface_Descriptor *lsd = new CLiquidSurfaceDescriptor(
      pFluidObject, pParams->surfacePlane, pParams->currentVelocity);
  ...
  buoyancy_input.medium_density =
      ConvertDensityToIVP(density); // density of water (unit: kg/m^3)
  ...
  // -------------------------------------------------------------------------------
  // create "water" (i.e. buoyancy solver) and attach a dynamic list of object
  // cores
  // -------------------------------------------------------------------------------
  CBuoyancyAttacher *attacher_to_cores_buoyancy = new CBuoyancyAttacher(
      buoyancy_input, pPhantom->get_intruding_cores(), lsd);

  CPhysicsFluidController *pFluid = new CPhysicsFluidController(
      attacher_to_cores_buoyancy, lsd, pFluidObject, pParams->contents);
...
bool SavePhysicsFluidController(const physsaveparams_t &params,
                                CPhysicsFluidController *pFluidObject) {
  return false;
}

bool RestorePhysicsFluidController(const physrestoreparams_t &params,
                                   CPhysicsFluidController **ppFluidObject) {
  return false;
}
