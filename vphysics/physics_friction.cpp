...
CFrictionSnapshot::CFrictionSnapshot(IVP_Real_Object *pObject)
    : m_pObject(pObject) {
  m_pDeleteList = NULL;
  SetFrictionSynapse(pObject->get_first_friction_synapse());
}

CFrictionSnapshot::~CFrictionSnapshot() { delete m_pDeleteList; }
...
bool CFrictionSnapshot::IsValid() { return m_pFriction != NULL ? true : false; }
...
void CFrictionSnapshot::MarkContactForDelete() {
  IVP_Synapse_Friction *pFriction =
      m_pContactPoint->get_synapse(!m_synapseIndex);
...
void CFrictionSnapshot::GetSurfaceNormal(Vector &out) {
  ...
  IVP_Contact_Point_API::get_surface_normal_ws(
      const_cast<IVP_Contact_Point *>(m_pContactPoint), &normal);
...
float CFrictionSnapshot::GetNormalForce() {
  return ConvertDistanceToHL(
      IVP_Contact_Point_API::get_vert_force(m_pContactPoint));
}

float CFrictionSnapshot::GetEnergyAbsorbed() {
  return ConvertEnergyToHL(
      IVP_Contact_Point_API::get_eliminated_energy(m_pContactPoint));
}
...
void DeleteAllFrictionPairs(IVP_Real_Object *pObject0,
