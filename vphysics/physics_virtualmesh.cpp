...
// This is the surfacemanager class for IVP that implements the required
// functions by layering CPhysCollideVirtualMesh
...
public:
  ...
  void
  get_radius_and_radius_dev_to_given_center(const IVP_U_Float_Point *center,
                                            IVP_FLOAT *radius,
  ...
  void
  get_all_terminal_ledges(IVP_U_BigVector<IVP_Compact_Ledge> *resulting_ledges);
...
// The idea is to have only the necessary instances of these in memory at any
// given time - never all of them
class CMeshInstance {
public:
  // resourcemanager
  ...
  const IVP_Compact_Ledge *GetOuterHull() {
    return (m_hullCount == 1)
               ? (const IVP_Compact_Ledge *)(m_pMemory + m_hullOffset)
               : NULL;
  }
  int GetRootLedges(IVP_Compact_Ledge **pLedges, int outCount) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      hullOffset += sizeof(IVP_Compact_Ledge) + (sizeof(IVP_Compact_Triangle) *
                                                 pLedges[i]->get_n_triangles());
    }
    return count;
  }
...
// flat memory footprint has triangleledges (ledge + 2 triangles for terrain),
// then has verts, then optional convex hull
void CMeshInstance::Init(const virtualmeshlist_t &list) {
  ...
  IVP_Compact_Poly_Point *pPoints =
      (IVP_Compact_Poly_Point *)&m_pMemory[ledgeSize];
  ...
  for (i = 0; i < list.triangleCount; i++) {
    ...
    CVPhysicsVirtualMeshWriter::InitTwoSidedTriangleLege(
        &pLedges[i], pPoints, list.indices[i * 3 + 0], list.indices[i * 3 + 1],
        list.indices[i * 3 + 2], 0);
  }
...
#if _DEBUG
    int hullSize = CVPhysicsVirtualMeshWriter::UnpackLedgeListFromHull(
        pMem, pHeader, pPoints);
...
const int g_MeshSize =
    (2048 * 1024 *
     4); // nillerusr: 2 MiB should be enough, old value causes problems in ep2
static CDataManager<CMeshInstance, virtualmeshlist_t, CMeshInstance *,
                    CThreadFastMutex>
    g_MeshManager(g_MeshSize);
static int numIndices = 0, numTriangles = 0, numBaseTriangles = 0,
           numSplits = 0;
//-----------------------------------------------------------------------------
// Purpose: This allows for just-in-time procedural triangle soup data to be
//			instanced & cached as IVP collision data (compact
//ledges)
//-----------------------------------------------------------------------------
// NOTE: This is the permanent in-memory representation.  It holds the
// compressed data and the parameters necessary to request the proxy geometry as
// needed
...
public:
  ...
  // non-const because they may instantiate the cache.  This causes problems
  // with the interface. Maybe the cache stuff should be mutable, but it amounts
  // to the same kind of hackery to cast away const.
  ...
  virtual IVP_SurfaceManager *CreateSurfaceManager(short &collideType) const {
    ...
    return new IVP_SurfaceManager_VirtualMesh(
        const_cast<CPhysCollideVirtualMesh *>(this));
  }
  virtual void GetAllLedges(IVP_U_BigVector<IVP_Compact_Ledge> &ledges) const {
    const triangleledge_t *pLedges =
        const_cast<CPhysCollideVirtualMesh *>(this)->AddRef()->GetLedges();
  ...
  virtual unsigned int SerializeToBuffer(char *pDest,
  ...
  void
  GetAllLedgesWithinRadius(const IVP_U_Point *observer_os, IVP_DOUBLE radius,
                           IVP_U_BigVector<IVP_Compact_Ledge> *resulting_ledges,
    ...
    m_params.pMeshEventHandler->GetTrianglesInSphere(m_params.userData,
                                                     centerHL, radiusHL, &list);
    if (list.triangleCount) {
      ...
      for (int i = 0; i < list.triangleCount; i++) {
        ...
        if (list.triangleIndices[i] < minTriangle ||
            list.triangleIndices[i] >= maxTriangle)
          continue;

        const IVP_Compact_Ledge *ledge =
            &pLedges[list.triangleIndices[i]].ledge;
        Assert(ledge->get_n_triangles() == 2);
        const IVP_Compact_Triangle *triangle = ledge->get_first_triangle();
        IVP_DOUBLE qdist =
            IVP_CLS.calc_qlen_PF_F_space(ledge, triangle, observer_os);
        if (qdist > radiusSq) {
          continue;
        }
  ...
  virtual void OutputDebugInfo() const { Msg("Virtual mesh!\n"); }
  ...
  // adds a lock on the collsion memory :: MUST CALL Release() or FrameRelease
  // corresponding to this call!!!
  CMeshInstance *AddRef();

  void BuildBoundingLedge();
  static virtualmeshhull_t *
  CreateMeshBoundingHull(const virtualmeshlist_t &list);
  ...
  CreateBoundingSurfaceFromRange(const virtualmeshlist_t &list, int firstIndex,
                                 int indexCount);
  ...
  IVP_Compact_Ledge *GetBoundingLedge() {
    IVP_Compact_Ledge *pLedge =
        const_cast<IVP_Compact_Ledge *>(AddRef()->GetOuterHull());
    FrameRelease();
    return pLedge;
  }
  ...
  inline void GetBounds(Vector &mins, Vector &maxs) const {
    m_params.pMeshEventHandler->GetWorldspaceBounds(m_params.userData, &mins,
                                                    &maxs);
  }
...
void VirtualMeshPSI() { FlushFrameLocks(); }
...
virtualmeshhull_t *
CPhysCollideVirtualMesh::CreateMeshBoundingHull(const virtualmeshlist_t &list) {
  virtualmeshhull_t *pHull = NULL;
  if (list.triangleCount) {
    IVP_Compact_Surface *pSurface =
        CreateBoundingSurfaceFromRange(list, 0, list.indexCount);
    if (pSurface) {
      const IVP_Compact_Ledge *pLedge =
          pSurface->get_compact_ledge_tree_root()->get_compact_hull();
      if (CVPhysicsVirtualMeshWriter::LedgeCanBePacked(pLedge, list)) {
        pHull = CVPhysicsVirtualMeshWriter::CreatePackedHullFromLedges(
            list, &pLedge, 1);
      } else {
        // too big to pack to 8-bits, split in two
        IVP_Compact_Surface *pSurface0 =
            CreateBoundingSurfaceFromRange(list, 0, list.indexCount / 2);
        IVP_Compact_Surface *pSurface1 = CreateBoundingSurfaceFromRange(
            list, list.indexCount / 2, list.indexCount / 2);

        const IVP_Compact_Ledge *pLedges[2] = {
            pSurface0->get_compact_ledge_tree_root()->get_compact_hull(),
...
IVP_Compact_Surface *CPhysCollideVirtualMesh::CreateBoundingSurfaceFromRange(
    const virtualmeshlist_t &list, int firstIndex, int indexCount) {
  ...
  IVP_Compact_Ledge *pLedge = const_cast<IVP_Compact_Ledge *>(
      node->get_compact_hull()); // we're going to write into client data on
                                 // each vert before we throw this away
...
void DestroyVirtualMesh(CPhysCollide *pMesh) { delete pMesh; }
...
void IVP_SurfaceManager_VirtualMesh::add_reference_to_ledge(
    const IVP_Compact_Ledge *ledge) {
  m_pMesh->AddRef();
}
void IVP_SurfaceManager_VirtualMesh::remove_reference_to_ledge(
    const IVP_Compact_Ledge *ledge) {
  m_pMesh->Release();
}

// Implement the IVP raycast.  This is done by testing each triangle (front &
// back) - so it's slow
void IVP_SurfaceManager_VirtualMesh::insert_all_ledges_hitting_ray(
  ...
  m_pMesh->GetAllLedgesWithinRadius(&center, ray_solver_os.ray_length * 0.5f,
                                    &ledges);
...
void IVP_SurfaceManager_VirtualMesh::get_radius_and_radius_dev_to_given_center(
    const IVP_U_Float_Point *center, IVP_FLOAT *radius,
    IVP_FLOAT *radius_deviation) const {
  // UNDONE: Check radius_deviation to see if there is a useful optimization to
  // be made here
...
void IVP_SurfaceManager_VirtualMesh::get_mass_center(
...
//-----------------------------------------------------------------------------
void IVP_SurfaceManager_VirtualMesh::get_rotation_inertia(
...
//-----------------------------------------------------------------------------
void IVP_SurfaceManager_VirtualMesh::get_all_ledges_within_radius(
    const IVP_U_Point *observer_os, IVP_DOUBLE radius,
    const IVP_Compact_Ledge *root_ledge, IVP_Real_Object *other_object,
    const IVP_Compact_Ledge *other_reference_ledge,
    IVP_U_BigVector<IVP_Compact_Ledge> *resulting_ledges) {
  if (!root_ledge) {
    ...
    if (count) {
      for (int i = 0; i < count; i++) {
        resulting_ledges->add(
            pLedges[i]); // return the recursive/virtual outer hull
      }
      return;
    }
  }
  m_pMesh->GetAllLedgesWithinRadius(observer_os, radius, resulting_ledges,
                                    root_ledge);
}
...
//-----------------------------------------------------------------------------
void IVP_SurfaceManager_VirtualMesh::get_all_terminal_ledges(
    IVP_U_BigVector<IVP_Compact_Ledge> *resulting_ledges) {
