...
//			also includes code to pack/unpack outer hull ledges to
//8-bit rep
//
//=============================================================================
#include "cbase.h"
...
// searches pVerts for each vert used by pLedge and builds a one way map from
// ledge indices to pVerts indices NOTE: pVerts is in HL coords, pLedge is in
// IVP coords
static void BuildVertMap(vertmap_t &out, const Vector *pVerts, int vertexCount,
  ...
  for (int i = 0; i < pLedge->get_n_triangles(); i++) {
    // iterate each triangle, for each referenced vert that hasn't yet been
    // mapped, search for the nearest match
    const IVP_Compact_Triangle *pTri = pLedge->get_first_triangle() + i;
    for (int j = 0; j < 3; j++) {
      int ivpIndex = pTri->get_edge(j)->get_start_point_index();
...
inline int EdgeIndex(const IVP_Compact_Ledge *pLedge,
                     const IVP_Compact_Edge *pEdge) {
  return pEdge - (const IVP_Compact_Edge *)pLedge->get_first_triangle();
}

// Builds a packedhull_t from a IVP_Compact_Ledge.  Assumes that the utlbuffer
// points at the memory following pHull (pHull is the header, utlbuffer is the
// body)
void PackLedgeIntoBuffer(packedhull_t *pHull, CUtlBuffer &buf,
                         const IVP_Compact_Ledge *pLedge,
  ...
  edgeMap.EnsureCount(pLedge->get_n_triangles() *
                      4); // each triangle also occupies an edge index
  for (int i = 0; i < edgeMap.Count(); i++) {
    edgeMap[i] = -1;
  }

  // we're going to reorder the triangles and edges so that the ones marked
  // virtual appear first in the list.  This way we only need a virtual count,
  // not a per-item flag.

  // also, the edges are stored relative to the first triangle that references
  // them so an edge from 0->1 means that the first triangle that references the
  // edge is 0->1 and the second triangle is 1->0.  This way we store half the
  // edges and the winged edge pointers are implicit
  ...
  for (int i = 0; i < pLedge->get_n_triangles(); i++) {
    const IVP_Compact_Triangle *pTri =
        pLedge->get_first_triangle() + triangleList[i];
    for (int j = 0; j < 3; j++) {
      const IVP_Compact_Edge *pEdge = pTri->get_edge(j);
      if (pEdge->get_is_virtual() &&
          edgeMap[EdgeIndex(pLedge, pEdge->get_opposite())] < 0) {
        edgeMap[EdgeIndex(pLedge, pEdge)] =
            edgeList.AddToTail(EdgeIndex(pLedge, pEdge));
      }
    }
  }
  ...
  for (int i = 0; i < pHull->edgeCount; i++) {
    packededge_t edge;
    const IVP_Compact_Edge *pEdge =
        (const IVP_Compact_Edge *)pLedge->get_first_triangle() + edgeList[i];
    Assert((edgeList[i] & 3) != 0); // must not be a triangle

    int v0 = vertMap.map[pEdge->get_start_point_index()] - pHull->baseVert;
    int v1 = vertMap.map[pEdge->get_next()->get_start_point_index()] -
             pHull->baseVert;
    Assert(v0 >= 0 && v0 < 256);
    Assert(v1 >= 0 && v1 < 256);
    edge.v0 = v0;
...
void CVPhysicsVirtualMeshWriter::UnpackCompactLedgeFromHull(
    IVP_Compact_Ledge *pLedge, int materialIndex,
  ...
  const packedtriangle_t *pPackedTris =
      pHullHeader->GetPackedTriangles(hullIndex);
  // write the ledge
  pLedge->set_offset_ledge_points(
      (int)((char *)pPointList -
            (char *)pLedge)); // byte offset from 'this' to (ledge) point array
  pLedge->set_is_compact(IVP_TRUE);
  pLedge->set_size(
      sizeof(IVP_Compact_Ledge) +
      sizeof(IVP_Compact_Triangle) *
          pHull->triangleCount); // <0 indicates a non compact compact ledge
  pLedge->n_triangles = pHull->triangleCount;
  pLedge->has_chilren_flag = isVirtualLedge ? IVP_TRUE : IVP_FALSE;

  // Make the offset -pLedge so the result is a NULL ledgetree node - we haven't
  // needed to create one of these as of yet
  // pLedge->ledgetree_node_offset = -((int)pLedge);

  // keep track of which triangle edge referenced this edge (so the next one can
  // swap the order and point to the first one)
  int forwardEdgeIndex[255];
  for (int i = 0; i < pHull->edgeCount; i++) {
    forwardEdgeIndex[i] = -1;
  }
  packededge_t *pPackedEdges =
      (packededge_t *)(pPackedTris + pHull->triangleCount);
  ...
  for (int i = 0; i < pHull->triangleCount; i++) {
    ...
    for (int j = 0; j < 3; j++) {
      ...
      if (forwardEdgeIndex[edges[j]] < 0) {
        // this is the first triangle to use this edge, so it's forward (and the
        // other triangle sharing (opposite edge pointer) is unknown)
        int startVert = pPackedEdges[edges[j]].v0 + baseVert;
        pOut[i].c_three_edges[j].set_start_point_index(startVert);
        pOut[i].c_three_edges[j].set_is_virtual(
            edges[j] < pHull->vedgeCount ? IVP_TRUE : IVP_FALSE);
        forwardEdgeIndex[edges[j]] =
            EdgeIndex(pLedge, &pOut[i].c_three_edges[j]);
      } else {
        // this is the second triangle to use this edge, so it's reversed (and
        // the other triangle sharing is in the forward edge table)
        ...
        pOut[i].c_three_edges[j].set_opposite_index(oppositeIndex -
                                                    thisEdgeIndex);
        pOut[i].c_three_edges[j].get_opposite()->set_opposite_index(
            thisEdgeIndex - oppositeIndex);
      }
    }
  }
}
...
static void InitTriangle(IVP_Compact_Triangle *pTri, int index,
                         int materialIndex, int v0, int v1, int v2, int opp0,
                         int opp1, int opp2) {
...
void CVPhysicsVirtualMeshWriter::InitTwoSidedTriangleLege(
    triangleledge_t *pOut, const IVP_Compact_Poly_Point *pPoints, int v0,
    int v1, int v2, int materialIndex) {
  ...
  pLedge->set_is_compact(IVP_TRUE);
  pLedge->set_size(
      sizeof(triangleledge_t)); // <0 indicates a non compact compact ledge
...
bool CVPhysicsVirtualMeshWriter::LedgeCanBePacked(
    const IVP_Compact_Ledge *pLedge, const virtualmeshlist_t &list) {
...
// this builds a packed hull array from a compact ledge array (needs the
// virtualmeshlist for reference)
virtualmeshhull_t *CVPhysicsVirtualMeshWriter::CreatePackedHullFromLedges(
    const virtualmeshlist_t &list, const IVP_Compact_Ledge **pLedges,
    int ledgeCount) {
  int triCount = 0;
  int edgeCount = 0;
  ...
  unsigned int totalSize =
      sizeof(packedtriangle_t) * triCount + sizeof(packededge_t) * edgeCount +
...
unsigned int CVPhysicsVirtualMeshWriter::UnpackLedgeListFromHull(
    byte *pOut, virtualmeshhull_t *pHull, IVP_Compact_Poly_Point *pPoints) {
  ...
  for (int i = 0; i < pHull->hullCount; i++) {
    ...
    CVPhysicsVirtualMeshWriter::UnpackCompactLedgeFromHull(
        pHullLedge, 0, pPoints, pHull, i, true);
...
#define DUMP_FILES 1
static bool DumpListToGLView( const char *pFilename, const virtualmeshlist_t
&list )
{
#if DUMP_FILES
...
list.pVerts[list.indices[i*3+0]].x, list.pVerts[list.indices[i*3+0]].y,
list.pVerts[list.indices[i*3+0]].z ); fprintf( fp, "%6.3f %6.3f %6.3f 0 1 0\n",
list.pVerts[list.indices[i*3+1]].x, list.pVerts[list.indices[i*3+1]].y,
list.pVerts[list.indices[i*3+1]].z ); fprintf( fp, "%6.3f %6.3f %6.3f 0 0 1\n",
...
static bool DumpLedgeToGLView( const char *pFilename, const IVP_Compact_Ledge
*pLedge, float r=1.0f, float g=1.0f, float b=1.0f, float offset=0.0f )
{
#if DUMP_FILES
        FILE *fp = fopen( pFilename, "a+" );
        int ivpIndex;
        ...
        for ( int i = 0; i < pLedge->get_n_triangles(); i++ )
        {
                // iterate each triangle, for each referenced vert that hasn't
yet been mapped, search for the nearest match const IVP_Compact_Triangle *pTri =
pLedge->get_first_triangle() + i; ivpIndex =
pTri->get_edge(2)->get_start_point_index(); ConvertPositionToHL(
&pPoints[ivpIndex], tmp[0] ); ivpIndex =
pTri->get_edge(1)->get_start_point_index(); ConvertPositionToHL(
&pPoints[ivpIndex], tmp[1] ); ivpIndex =
pTri->get_edge(0)->get_start_point_index(); ConvertPositionToHL(
&pPoints[ivpIndex], tmp[2] ); tmp[0].x += offset; tmp[1].x += offset; tmp[2].x
+= offset; fprintf( fp, "2\n" ); fprintf( fp, "%6.3f %6.3f %6.3f %.1f %.1f
%.1f\n", tmp[0].x, tmp[0].y, tmp[0].z, r, g, b ); fprintf( fp, "%6.3f %6.3f
%6.3f %.1f %.1f %.1f\n", tmp[1].x, tmp[1].y, tmp[1].z, r, g, b ); fprintf( fp,
"2\n" ); fprintf( fp, "%6.3f %6.3f %6.3f %.1f %.1f %.1f\n", tmp[1].x, tmp[1].y,
tmp[1].z, r, g, b ); fprintf( fp, "%6.3f %6.3f %6.3f %.1f %.1f %.1f\n",
tmp[2].x, tmp[2].y, tmp[2].z, r, g, b ); fprintf( fp, "2\n" ); fprintf( fp,
"%6.3f %6.3f %6.3f %.1f %.1f %.1f\n", tmp[2].x, tmp[2].y, tmp[2].z, r, g, b );
                fprintf( fp, "%6.3f %6.3f %6.3f %.1f %.1f %.1f\n", tmp[0].x,
tmp[0].y, tmp[0].z, r, g, b );
        }
        fclose( fp );
#endif
        return true;
...
bool CVPhysicsVirtualMeshWriter::CheckHulls( virtualmeshhull_t *pHull0,
virtualmeshhull_t *pHull1, const virtualmeshlist_t &list )
{
        ...
        }
        {
                ...
                int pointSize0 = sizeof(IVP_Compact_Poly_Point) *
list.vertexCount; byte *pMem0 = (byte *)ivp_malloc_aligned( size0+pointSize0, 16
); IVP_Compact_Poly_Point *pPoints = (IVP_Compact_Poly_Point *)pMem0;
                IVP_Compact_Ledge *pLedge0 = (IVP_Compact_Ledge *)(pPoints +
list.vertexCount); for ( int i = 0; i < list.vertexCount; i++ )
                {
                ...
                for ( int i = 0; i < pHull0->hullCount; i++ )
                {
                        if ( i == i ) DumpLedgeToGLView( "c:\\jay.txt", pLedge0,
1, 0, 0, 0 ); pLedge0 = (IVP_Compact_Ledge *)( ((byte *)pLedge0 ) +
pLedge0->get_size() );
                }
...

        {
                ...
                int pointSize1 = sizeof(IVP_Compact_Poly_Point) *
list.vertexCount; byte *pMem1 = (byte *)ivp_malloc_aligned( size1+pointSize1, 16
); IVP_Compact_Poly_Point *pPoints = (IVP_Compact_Poly_Point *)pMem1;
                IVP_Compact_Ledge *pLedge1 = (IVP_Compact_Ledge *)(pPoints +
list.vertexCount); for ( int i = 0; i < list.vertexCount; i++ )
                {
                        ConvertPositionToIVP( list.pVerts[i], pPoints[i] );
                }
                ...
                {
                        if ( i == i ) DumpLedgeToGLView( "c:\\jay.txt", pLedge1,
0, 1, 0, 1024 ); pLedge1 = (IVP_Compact_Ledge *)( ((byte *)pLedge1 ) +
pLedge1->get_size() );
                }
