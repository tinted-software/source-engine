...
DEFINE_VPHYSPTR(m_pCarBody), DEFINE_FIELD(m_wheelCount, FIELD_INTEGER),
    DEFINE_EMBEDDED(m_vehicleData), DEFINE_EMBEDDED(m_currentState),
    ...
    DEFINE_FIELD(engineRPM, FIELD_FLOAT), DEFINE_FIELD(gear, FIELD_INTEGER),
    ...
    DEFINE_FIELD(mass, FIELD_FLOAT), DEFINE_FIELD(inertia, FIELD_FLOAT),
    ...
    DEFINE_FIELD(raytraceOffset, FIELD_VECTOR), DEFINE_EMBEDDED(wheels),
    DEFINE_EMBEDDED(suspension), DEFINE_FIELD(torqueFactor, FIELD_FLOAT),
    ...
    DEFINE_FIELD(speedSlow, FIELD_FLOAT), DEFINE_FIELD(speedFast, FIELD_FLOAT),
    ...
    DEFINE_FIELD(maxSpeed, FIELD_FLOAT), DEFINE_FIELD(maxRevSpeed, FIELD_FLOAT),
    ...
    DEFINE_FIELD(wheelsPerAxle, FIELD_INTEGER), DEFINE_EMBEDDED(body),
    DEFINE_EMBEDDED_AUTO_ARRAY(axles), DEFINE_EMBEDDED(engine),
  ...
  // FIXME: Check why this is null! It occurs when jumping the ravine in
  // seafloor
  if (!pObject)
    return false;
...
bool ShouldOverrideWheelContactFriction(float *pFrictionOut,
  ...
  if (!IVPFloatPointIsZero(pNormal)) {
    ...
    pivp0->get_core()->get_m_world_f_core_PSI()->vimult3(pNormal,
                                                         &normalWheelSpace);
    if (fabs(normalWheelSpace.k[0]) > 0.2588f) // 15 degree wheel cone
    {
      // adjust friction here, this isn't a valid part of the wheel for contact,
      // set friction to zero
      // Vector tmp;ConvertDirectionToHL( normalWheelSpace, tmp );Msg("Wheel
      // sliding on surface %.2f %.2f %.2f\n", tmp.x, tmp.y, tmp.z );
      *pFrictionOut = 0;
      return true;
    }
  }
...
class CVehicleController : public IPhysicsVehicleController,
                           public IVP_Listener_Object {
public:
  CVehicleController();
  CVehicleController(const vehicleparams_t &params, CPhysicsEnvironment *pEnv,
  ...
  void InitFromTemplate(
      CPhysicsEnvironment *pEnv, void *pGameData, IPhysicsGameTrace *pGameTrace,
...
protected:
  ...
  // Update.
  void UpdateSteering(const vehicle_controlparams_t &controls,
                      float flDeltaTime, float flSpeed);
  void UpdatePowerslide(const vehicle_controlparams_t &controls,
                        bool bPowerslide, float flSpeed);
  void UpdateEngine(const vehicle_controlparams_t &controls, float flDeltaTime,
                    float flThrottle, float flBrake, bool bHandbrake,
                    bool bPowerslide);
  bool UpdateEngineTurboStart(const vehicle_controlparams_t &controls,
                              float flDeltaTime);
  void UpdateEngineTurboFinish(void);
  void UpdateHandbrake(const vehicle_controlparams_t &controls,
                       float flThrottle, bool bHandbrake, bool bPowerslide);
  ...
  void CalcEngine(float throttle, float brake_val, bool handbrake,
                  float steeringVal, bool torqueBoost);
...
CVehicleController::CVehicleController(const vehicleparams_t &params,
...
IPhysicsObject *CVehicleController::GetWheel(int index) {
  ...
  if (m_nVehicleType == VEHICLE_TYPE_CAR_WHEELS) {
    ...
    return static_cast<CPhysics_Car_System_Raycast_Wheels *>(m_pCarSystem)
        ->GetWheel(index);
...
bool CVehicleController::GetWheelContactPoint(int index, Vector *pContactPoint,
                                              int *pSurfaceProps) {
  bool bSet = false;
  if (index < m_wheelCount) {
    IPhysicsFrictionSnapshot *pSnapshot =
        m_pWheels[index]->CreateFrictionSnapshot();
    float forceMax = -1.0f;
...
void CVehicleController::event_object_deleted(IVP_Event_Object *pEvent) {
  // the car system's constraint solver is going to delete itself now, so NULL
  // the car system.
...
IVP_Real_Object *CVehicleController::CreateWheel(int wheelIndex,
  ...
  // ALSO NOTE: Save/load puts the results into m_pWheels	regardless of
  // vehicle type!!! That's why I'm not calling GetWheel().
  if (m_pWheels[wheelIndex]) {
    CPhysicsObject *pWheelObject =
        static_cast<CPhysicsObject *>(m_pWheels[wheelIndex]);
  ...
  // needs to be in HL units because we're calling through the "outer" interface
  // to create the wheels
  float radius = axle.wheels.radius;
  float r3 = radius * radius * radius;
  params.volume = (4 / 3) * M_PI * r3;

  CPhysicsObject *pWheel = (CPhysicsObject *)m_pEnv->CreateSphereObject(
      radius, axle.wheels.materialIndex, wheelPositionHL, bodyAngles, &params,
      false);
  pWheel->Wake();
  ...
  TransformIVPToLocal(wheelPositionIVP, wheelPositionBs,
                      m_pCarBody->GetObject(), true);
...
// Purpose:
//-----------------------------------------------------------------------------
void CVehicleController::CreateTraceData(int wheelIndex,
                                         vehicle_axleparams_t &axle) {
  if (wheelIndex >= VEHICLE_MAX_WHEEL_COUNT)
    return;
  ...
  TransformIVPToLocal(tracePositionIVP, tracePositionBs,
                      m_pCarBody->GetObject(), true);
...
void CVehicleController::CreateIVPObjects() {
  ...
  case VEHICLE_TYPE_CAR_RAYCAST:
    m_pCarSystem = new CPhysics_Car_System_Raycast_Wheels(
        m_pEnv->GetIVPEnvironment(), &ivpVehicleData);
    break;

  case VEHICLE_TYPE_AIRBOAT_RAYCAST:
    m_pCarSystem = new CPhysics_Airboat(m_pEnv->GetIVPEnvironment(),
                                        &ivpVehicleData, m_pGameTrace);
    break;
  }
...
void CVehicleController::VehicleDataReload() {
  ...
  m_vehicleData.engine.maxRevSpeed =
      MPH_TO_GAMEVEL(m_vehicleData.engine.maxRevSpeed);
  m_vehicleData.engine.boostMaxSpeed =
      MPH_TO_GAMEVEL(m_vehicleData.engine.boostMaxSpeed);
}

//-----------------------------------------------------------------------------
// Purpose: Setup the body parameters.
//-----------------------------------------------------------------------------
void CVehicleController::InitCarSystemBody(
  ...
  ivpVehicleData.body_counter_torque_factor =
      m_vehicleData.body.counterTorqueFactor;
  ivpVehicleData.body_down_force_vertical_offset =
      ConvertDistanceToIVP(m_vehicleData.body.tiltForceHeight);
  ivpVehicleData.extra_gravity_force_value =
      m_vehicleData.body.addGravity * m_gravityLength * m_bodyMass;
...
//-----------------------------------------------------------------------------
void CVehicleController::InitCarSystemWheels(
    IVP_Template_Car_System &ivpVehicleData) {
  int wheelIndex = 0;
  ...
  for (i = 0; i < m_vehicleData.axleCount; i++) {
    for (int w = 0; w < m_vehicleData.wheelsPerAxle; w++, wheelIndex++) {
      ...
      if (pWheel) {
        ...
        ivpVehicleData.wheel_radius[wheelIndex] =
            pWheel->get_core()->upper_limit_radius;
        ivpVehicleData.wheel_reversed_sign[wheelIndex] = 1.0;
        // only for raycast car

        ivpVehicleData.friction_of_wheel[wheelIndex] =
            m_vehicleData.axles[i].wheels.frictionScale;
        ...
        ivpVehicleData.spring_dampening_compression[wheelIndex] =
            m_vehicleData.axles[i].suspension.springDampingCompression *
            m_bodyMass;
        ivpVehicleData.max_body_force[wheelIndex] =
            m_vehicleData.axles[i].suspension.maxBodyForce * m_bodyMass;
        ivpVehicleData.spring_pre_tension[wheelIndex] = -ConvertDistanceToIVP(
            m_vehicleData.axles[i].wheels.springAdditionalLength);

        ivpVehicleData.wheel_pos_Bos[wheelIndex] =
            m_wheelPosition_Bs[wheelIndex];
        if (m_bTraceData) {
          ivpVehicleData.trace_pos_Bos[wheelIndex] =
              m_tracePosition_Bs[wheelIndex];
        }
    ...
    ivpVehicleData.stabilizer_constant[i] =
        m_vehicleData.axles[i].suspension.stabilizerConstant * m_bodyMass;
    // this should output in radians per second
    float radius = ConvertDistanceToIVP(m_vehicleData.axles[i].wheels.radius);
    float totalMaxSpeed =
        max(m_vehicleData.engine.boostMaxSpeed, m_vehicleData.engine.maxSpeed);
...
//-----------------------------------------------------------------------------
void CVehicleController::UpdateSteering(const vehicle_controlparams_t &controls,
                                        float flDeltaTime, float flSpeed) {
  // Steering - IVP steering is in radians.
  float flSteeringAngle = CalcSteering(flDeltaTime, flSpeed, controls.steering,
                                       controls.bAnalogSteering);
...
//-----------------------------------------------------------------------------
void CVehicleController::UpdatePowerslide(
    const vehicle_controlparams_t &controls, bool bPowerslide, float flSpeed) {
  ...
  if (newTireType != m_nTireType) {
    for (int iAxle = 0; iAxle < m_vehicleData.axleCount; ++iAxle) {
      ...
      if (newTireType == VEHICLE_TIRE_POWERSLIDE &&
          (m_vehicleData.axles[iAxle].wheels.skidMaterialIndex != -1)) {
        materialIndex = m_vehicleData.axles[iAxle].wheels.skidMaterialIndex;
      } else if (newTireType == VEHICLE_TIRE_BRAKING &&
                 (m_vehicleData.axles[iAxle].wheels.brakeMaterialIndex != -1)) {
        materialIndex = m_vehicleData.axles[iAxle].wheels.brakeMaterialIndex;
      }

      for (int iAxleWheel = 0; iAxleWheel < m_vehicleData.wheelsPerAxle;
           ++iAxleWheel, ++iWheel) {
  ...
  if (flSpeed > 0 && (bPowerslideLeft != bPowerslideRight)) {
    ...
    float powerSlide =
        RemapValClamped(flSpeed, m_vehicleData.steering.speedSlow,
                        m_vehicleData.steering.speedFast, 0, 1);
    float powerSlideAccel =
        ConvertDistanceToIVP(m_vehicleData.steering.powerSlideAccel);
    if (bPowerslideLeft) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CVehicleController::UpdateEngine(const vehicle_controlparams_t &controls,
                                      float flDeltaTime, float flThrottle,
                                      float flBrake, bool bHandbrake,
                                      bool bPowerslide) {
...
// Purpose:
//-----------------------------------------------------------------------------
bool CVehicleController::UpdateEngineTurboStart(
    const vehicle_controlparams_t &controls, float flDeltaTime) {
  bool bTorqueBoost = false;
  if (controls.boost > 0) {
    if (m_vehicleData.engine.torqueBoost) {
      // Turbo will be applied at the engine level.
      bTorqueBoost = true;
      m_pCarSystem->activate_booster(0.0f, m_vehicleData.engine.boostDuration,
                                     m_vehicleData.engine.boostDelay);
    } else {
      // Activate the turbo force booster - applied to vehicle body.
      m_pCarSystem->activate_booster(
          m_vehicleData.engine.boostForce * controls.boost,
...
void CVehicleController::UpdateEngineTurboFinish(void) {
  if (m_vehicleData.engine.boostDuration + m_vehicleData.engine.boostDelay >
      0) // watch out for div by zero
  {
    if (m_currentState.boostDelay > 0) {
      m_currentState.boostTimeLeft =
          100 - 100 * (m_currentState.boostDelay /
                       (m_vehicleData.engine.boostDuration +
                        m_vehicleData.engine.boostDelay));
    } else {
...
//-----------------------------------------------------------------------------
void CVehicleController::UpdateHandbrake(
    const vehicle_controlparams_t &controls, float flThrottle, bool bHandbrake,
    bool bPowerslide) {
  ...
  if (!bPowerslide) {
    ...
    if ((flThrottle < 0.0f &&
         m_currentState.speed > THROTTLE_OPPOSING_FORCE_EPSILON) ||
        (flThrottle > 0.0f &&
         m_currentState.speed < -THROTTLE_OPPOSING_FORCE_EPSILON)) {
      bHandbrake = true;
    }
  ...
  if (bHandbrake) {
    // HACKHACK: only allow the handbrake when the wheels have contact with
    // something otherwise they will affect the car in an undesirable way
    bHandbrake = false;
  ...
  bool currentHandbrake =
      (m_vehicleFlags & FVEHICLE_HANDBRAKE_ON) ? true : false;
  if (bHandbrake != currentHandbrake) {
    ...
    for (int iWheel = 0; iWheel < m_wheelCount; ++iWheel) {
      m_pCarSystem->fix_wheel((IVP_POS_WHEEL)iWheel,
                              bHandbrake ? IVP_TRUE : IVP_FALSE);
    }
  }
}
  ...
  if (m_vehicleData.steering.isSkidAllowed) {
    ...
    for (int iWheel = 0; iWheel < m_wheelCount; ++iWheel) {
      if (GetWheelContactPoint(iWheel, &contact, &surfaceProps)) {
        // NOTE: The wheel should be translating by the negative of the speed a
        // point in contact with the surface is moving.  So the net velocity on
        // the surface is zero if that wheel is 100% engaged in driving the car
        // any velocity in excess of this gets compared against the threshold
        // for skidding
        m_pWheels[iWheel]->GetVelocityAtPoint(contact, &velocity);
        float speed = velocity.Length();
        if (speed > m_currentState.skidSpeed ||
            m_currentState.skidSpeed <= 0.0f) {
...
void CVehicleController::UpdateExtraForces(void) {
  ...
  if (m_vehicleData.body.maxAngularVelocity > 0 &&
      angVel.Length() > m_vehicleData.body.maxAngularVelocity) {
...
// Purpose:
//-----------------------------------------------------------------------------
float CVehicleController::CalcSteering(float dt, float speed, float steering,
                                       bool bAnalog) {
  float degrees = RemapValClamped(
      speed, m_vehicleData.steering.speedSlow, m_vehicleData.steering.speedFast,
  ...
  if (speedGame > m_vehicleData.engine.maxSpeed) {
    degrees = RemapValClamped(speedGame, m_vehicleData.engine.maxSpeed,
                              m_vehicleData.engine.boostMaxSpeed,
                              m_vehicleData.steering.degreesFast,
                              m_vehicleData.steering.degreesBoost);
  }
  if (m_vehicleData.steering.steeringExponent != 0) {
    float sign = steering < 0 ? -1 : 1;
    float absSteering = fabs(steering);
    if (bAnalog) {
      // analog steering is directly mapped, not integrated, so go ahead and map
      // the full range using the exponent then clamp to the output cone - keeps
      // stick position:turn rate constant NOTE: Also hardcode exponent to 2
      // because we can't add a script entry at this point
      float output =
          pow(absSteering, 2.0f) * sign * m_vehicleData.steering.degreesSlow;
      return clamp(output, -degrees, degrees);
    }
    // digital steering is integrated, keep time to full turn rate constant
    return pow(absSteering, m_vehicleData.steering.steeringExponent) * sign *
           degrees;
  }
...
void CVehicleController::CalcEngineTransmission(float flThrottle) {
  ...
  for (int iWheel = 0; iWheel < m_wheelCount; ++iWheel) {
    float flRotSpeed =
        fabs(m_pCarSystem->get_wheel_angular_velocity(IVP_POS_WHEEL(iWheel)));
    flAvgRotSpeed += flRotSpeed;
  }
  flAvgRotSpeed *= 0.5f / (float)IVP_PI / m_wheelCount;

  float flEstEngineRPM = flAvgRotSpeed * m_vehicleData.engine.axleRatio *
                         m_vehicleData.engine.gearRatio[m_currentState.gear] *
                         60;
  ...
  // Downshift?
  while ((flEstEngineRPM < m_vehicleData.engine.shiftDownRPM) &&
         (m_currentState.gear > 0)) {
    m_currentState.gear--;
    flEstEngineRPM = flAvgRotSpeed * m_vehicleData.engine.axleRatio *
                     m_vehicleData.engine.gearRatio[m_currentState.gear] * 60;
  }
...
// brake_val [0..1]
//-----------------------------------------------------------------------------
void CVehicleController::CalcEngine(float throttle, float brake_val,
                                    bool handbrake, float steeringVal,
                                    bool torqueBoost) {
  ...
  // Speed governor
  if (IsPC()) {
    float maxSpeed = torqueBoost ? m_vehicleData.engine.boostMaxSpeed
                                 : m_vehicleData.engine.maxSpeed;
    maxSpeed =
        max(1.f, maxSpeed); // make sure this is non-zero before the divide
  ...
  } else // consoles
  {
    if ((throttle > 0) &&
        ((!torqueBoost &&
          flAbsSpeed > (m_vehicleData.engine.maxSpeed * throttle)) ||
         (torqueBoost && flAbsSpeed > m_vehicleData.engine.boostMaxSpeed))) {
      throttle *= 0.1f;
    }
  }

  // Check for reverse - both of these "governors" or horrible and need to be
  // redone before we ship!
  if ((throttle < 0) &&
      (!torqueBoost && (flAbsSpeed > m_vehicleData.engine.maxRevSpeed))) {
    throttle *= 0.1f;
  }
  ...
  if (throttle != 0.0) {
    ...
    float wheel_force_by_throttle =
        throttle * m_vehicleData.engine.horsepower *
        (watt_per_hp * seconds_per_minute) *
        m_vehicleData.engine.gearRatio[m_currentState.gear] *
        m_vehicleData.engine.axleRatio /
    ...
    for (int i = 0; i < m_vehicleData.axleCount; i++) {
      ...
      if (torqueBoost && IsBoosting()) {
        // reduce the boost at low speeds and high turns since this usually just
        // makes the tires spin this means you only get the full boost when
        // travelling in a straight line at high speed
        float speedFactor = RemapValClamped(
            flAbsSpeed, 0, m_vehicleData.engine.maxSpeed, 0.1f, 1.0f);
        float turnFactor = 1.0f - (fabs(steeringVal) * 0.95f);
        float dampedBoost =
            m_vehicleData.engine.boostForce * speedFactor * turnFactor;
      ...
      float axleTorque =
          boostFactor * wheel_force_by_throttle * axleFactor *
    ...
    float wheel_force_by_brake =
        brake_val * m_gravityLength * (m_bodyMass + m_totalWheelMass);

    float sign = m_currentState.speed >= 0.0f ? -1.0f : 1.0f;
    int wheelIndex = 0;
    for (int i = 0; i < m_vehicleData.axleCount; i++) {
      float torque_val =
          0.5 * sign * wheel_force_by_brake *
          m_vehicleData.axles[i].brakeFactor *
          ConvertDistanceToIVP(m_vehicleData.axles[i].wheels.radius);
      for (int w = 0; w < m_vehicleData.wheelsPerAxle; w++, wheelIndex++) {
        m_pCarSystem->change_wheel_torque((IVP_POS_WHEEL)wheelIndex,
                                          torque_val);
      }
    }
...
//-----------------------------------------------------------------------------
void CVehicleController::GetCarSystemDebugData(
  ...
  for (int iWheel = 0; iWheel < VEHICLE_DEBUGRENDERDATA_MAX_WHEELS; ++iWheel) {
    debugCarSystem.vecWheelRaycasts[iWheel][0].x =
        carSystemDebugData.wheelRaycasts[iWheel][0].k[0];
    debugCarSystem.vecWheelRaycasts[iWheel][0].y =
        carSystemDebugData.wheelRaycasts[iWheel][0].k[1];
    debugCarSystem.vecWheelRaycasts[iWheel][0].z =
        carSystemDebugData.wheelRaycasts[iWheel][0].k[2];

    debugCarSystem.vecWheelRaycasts[iWheel][1].x =
        carSystemDebugData.wheelRaycasts[iWheel][1].k[0];
    debugCarSystem.vecWheelRaycasts[iWheel][1].y =
        carSystemDebugData.wheelRaycasts[iWheel][1].k[1];
    debugCarSystem.vecWheelRaycasts[iWheel][1].z =
        carSystemDebugData.wheelRaycasts[iWheel][1].k[2];
    ...
    debugCarSystem.vecWheelRaycastImpacts[iWheel] =
        ...
        (carSystemDebugData.wheelRaycastImpacts[iWheel] *
         (debugCarSystem.vecWheelRaycasts[iWheel][1] -
          debugCarSystem.vecWheelRaycasts[iWheel][0]));
  }

  ConvertPositionToHL(carSystemDebugData.backActuatorLeft,
                      debugCarSystem.vecAxlePos[0]);
  ConvertPositionToHL(carSystemDebugData.backActuatorRight,
                      debugCarSystem.vecAxlePos[1]);
  ConvertPositionToHL(carSystemDebugData.frontActuatorLeft,
                      debugCarSystem.vecAxlePos[2]);
  // vecAxlePos only has three elements so this line is illegal. The mapping of
  // actuators to axles seems dodgy anyway.
  // ConvertPositionToHL( carSystemDebugData.frontActuatorRight,
  // debugCarSystem.vecAxlePos[3] );
}

//-----------------------------------------------------------------------------
// Save/load
//-----------------------------------------------------------------------------
void CVehicleController::WriteToTemplate(
    vphysics_save_cvehiclecontroller_t &controllerTemplate) {
  // Get rid of the handbrake flag.  The car keeps the flag and will reset it
  // fixing wheels, else the system thinks it already fixed the wheels on load
  // and the car roles.
  m_vehicleFlags &= ~FVEHICLE_HANDBRAKE_ON;
  ...
  memcpy(&controllerTemplate.m_currentState, &m_currentState,
         sizeof(m_currentState));
  memcpy(&controllerTemplate.m_vehicleData, &m_vehicleData,
         sizeof(m_vehicleData));
  for (int i = 0; i < VEHICLE_MAX_WHEEL_COUNT; ++i) {
    controllerTemplate.m_pWheels[i] = m_pWheels[i];
    ConvertPositionToHL(m_wheelPosition_Bs[i],
                        controllerTemplate.m_wheelPosition_Bs[i]);
    ConvertPositionToHL(m_tracePosition_Bs[i],
                        controllerTemplate.m_tracePosition_Bs[i]);
  }
  m_flVelocity[0] = m_flVelocity[1] = m_flVelocity[2] = 0.0f;
  if (m_pCarBody) {
...
// JAY: Keep this around for now while we still have a bunch of games saved with
// the old vehicle controls.  We won't ship this, but it lets us debug
...
void CVehicleController::InitFromTemplate(
    CPhysicsEnvironment *pEnv, void *pGameData, IPhysicsGameTrace *pGameTrace,
    const vphysics_save_cvehiclecontroller_t &controllerTemplate) {
  m_pEnv = pEnv;
  m_pGameTrace = pGameTrace;
  ...
  m_pCarSystem = NULL;
  memcpy(&m_currentState, &controllerTemplate.m_currentState,
         sizeof(m_currentState));
  memcpy(&m_vehicleData, &controllerTemplate.m_vehicleData,
         sizeof(m_vehicleData));
  ...
  SET_DEFAULT(m_vehicleData.steering.speedSlow,
              m_vehicleData.engine.maxSpeed * 0.25);
  SET_DEFAULT(m_vehicleData.steering.speedFast,
              m_vehicleData.engine.maxSpeed * 0.75);
  ...
  for (int i = 0; i < VEHICLE_MAX_WHEEL_COUNT; ++i) {
    ...
    ConvertPositionToIVP(controllerTemplate.m_wheelPosition_Bs[i],
                         m_wheelPosition_Bs[i]);
    ConvertPositionToIVP(controllerTemplate.m_tracePosition_Bs[i],
                         m_tracePosition_Bs[i]);
  }

  CreateIVPObjects();

  // HACKHACK: vehicle wheels don't have valid friction at startup, clearing the
  // body's angular velocity keeps this fact from affecting the vehicle dynamics
  // in any noticeable way using growFriction will re-establish the contact
  // point with moveable objects, but the friction that occurs afterward is not
  // the same across the save even when that is extended to include static
  // objects
  if (m_pCarBody) {
    // clear angVel
    m_pCarBody->SetVelocity(NULL, &vec3_origin);
    m_pCarBody->GetObject()->get_core()->speed_change.set(
        m_flVelocity[0], m_flVelocity[1], m_flVelocity[2]);
  }
}
    ...
    for (int iAxle = 0; iAxle < m_vehicleData.axleCount; ++iAxle) {
      for (int iAxleWheel = 0; iAxleWheel < m_vehicleData.wheelsPerAxle;
        ...
        if (m_nTireType != VEHICLE_TIRE_NORMAL) {
          m_pWheels[iWheel]->SetMaterialIndex(
              m_vehicleData.axles[iAxle].wheels.materialIndex);
        }
...
//-----------------------------------------------------------------------------
IPhysicsVehicleController *
CreateVehicleController(CPhysicsEnvironment *pEnv, CPhysicsObject *pBodyObject,
                        const vehicleparams_t &params,
                        unsigned int nVehicleType,
                        IPhysicsGameTrace *pGameTrace) {
  CVehicleController *pController =
      new CVehicleController(params, pEnv, nVehicleType, pGameTrace);
  pController->InitCarSystem(pBodyObject);
  return pController;
}

bool SavePhysicsVehicleController(const physsaveparams_t &params,
                                  CVehicleController *pVehicleController) {
  vphysics_save_cvehiclecontroller_t controllerTemplate;
...
bool RestorePhysicsVehicleController(const physrestoreparams_t &params,
  ...
  *ppVehicleController = new CVehicleController;
  ...
  (*ppVehicleController)
      ->InitFromTemplate(
          static_cast<CPhysicsEnvironment *>(params.pEnvironment),
