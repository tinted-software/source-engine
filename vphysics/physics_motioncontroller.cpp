...
DEFINE_VPHYSPTR_UTLVECTOR(m_objectList),
    ...
    END_DATADESC()
        ...
        class CPhysicsMotionController : public IVP_Controller_Independent,
                                         public IPhysicsMotionController {
public:
  ...
  void
  InitFromTemplate(const vphysics_save_motioncontroller_t &controllerTemplate);

  // returns the number of objects currently attached to the controller
  virtual int CountObjects(void) { return m_coreList.Count(); }
...
CPhysicsMotionController::CPhysicsMotionController(IMotionEvent *pHandler,
...
void CPhysicsMotionController::do_simulation_controller(
    IVP_Event_Sim *event, IVP_U_Vector<IVP_Core> *core_list) {
  if (m_handler) {
    for (int i = 0; i < core_list->len(); i++) {
      ...
      IMotionEvent::simresult_e ret =
          m_handler->Simulate(this, pPhys, event->delta_time, speed, rot);
      ...
      case IMotionEvent::SIM_LOCAL_ACCELERATION: {
        ...
        // UNDONE: Put these values into speed change / rot_speed_change
        // instead?
...
void CPhysicsMotionController::AttachObject(IPhysicsObject *pObject,
  ...
  // UNDONE: On save/load, trigger-based motion controllers re-attach their
  // objects. UNDONE: Do something cheaper about this? OPTIMIZE: Linear search
  // here?
...
// Save/load
void CPhysicsMotionController::WriteToTemplate(
...
void CPhysicsMotionController::InitFromTemplate(
    const vphysics_save_motioncontroller_t &controllerTemplate) {
...
IPhysicsMotionController *CreateMotionController(CPhysicsEnvironment *pPhysEnv,
...
bool SavePhysicsMotionController(const physsaveparams_t &params,
  ...
  CPhysicsMotionController *pControllerImp =
      static_cast<CPhysicsMotionController *>(pMotionController);
...
bool RestorePhysicsMotionController(
  ...
  CPhysicsMotionController *pControllerImp = new CPhysicsMotionController(
      NULL, static_cast<CPhysicsEnvironment *>(params.pEnvironment));
