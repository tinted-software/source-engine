...
#if TEST_BBOX
  Vector mins, maxs;
  physcollision->CollideGetAABB(&mins, &maxs, pCollide, Vector(-500, 200, -100),
                                vec3_angle);
  Vector extents = maxs - mins;
  Vector center = 0.5f * (maxs + mins);
  Msg("bbox: %.2f,%.2f, %.2f @ %.2f, %.2f, %.2f\n", extents.x, extents.y,
      extents.z, center.x, center.y, center.z);
#endif
...
bool CBenchmarkApp::SetupSearchPaths() {
  ...
  g_pFullFileSystem->AddSearchPath(steamInfo.m_GameInfoPath, "SKIN",
                                   PATH_ADD_TO_HEAD);
...
//	loopCount = 3;
  ...
  for (int i = 0; i < loopCount; i++) {
    ...
    Msg("%.2f ms [%.2f X] %d/%d hits\n", results.totalTime,
        IMPROVEMENT_FACTOR(results.totalTime, g_Baselines[i].total),
        results.collisionHits, results.collisionTests);
    Msg("%.2f ms rays \t[%.2f X] \t%.2f ms boxes [%.2f X]\n", results.rayTime,
        IMPROVEMENT_FACTOR(results.rayTime, g_Baselines[i].ray),
        results.boxTime,
  ...
  Msg("\n%.2fs total \t[%.2f X]!\n", totalTime,
      IMPROVEMENT_FACTOR(totalTime, g_TotalBaseline));
  return 0;
}
