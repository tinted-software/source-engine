...
hk_Vector3 TransformHLWorldToHavanaLocal(const Vector &hlWorld,
...
Vector TransformHavanaLocalToHLWorld(const hk_Vector3 &input,
                                     IVP_Real_Object *pObject, bool translate) {
...
void ConvertHLLocalMatrixToHavanaLocal(const matrix3x4_t &hlMatrix,
                                       hk_Transform &out) {
...
void ConvertHavanaLocalMatrixToHL(const hk_Transform &in, matrix3x4_t &hlMatrix,
                                  IVP_Real_Object *pObject) {
  IVP_U_Matrix ivpMatrix;
...
static bool
IsBreakableConstraint(const constraint_breakableparams_t &constraint) {
  return ((constraint.forceLimit != 0 &&
           constraint.forceLimit < UNBREAKABLE_BREAK_LIMIT) ||
          (constraint.torqueLimit != 0 &&
           constraint.torqueLimit < UNBREAKABLE_BREAK_LIMIT) ||
          (constraint.bodyMassScale[0] != 1.0f &&
           constraint.bodyMassScale[0] != 0.0f) ||
          (constraint.bodyMassScale[1] != 1.0f &&
           constraint.bodyMassScale[1] != 0.0f))
             ? true
             : false;
}

struct vphysics_save_cphysicsconstraintgroup_t
    : public constraint_groupparams_t {
  bool isActive;
...
void CPhysicsConstraintGroup::GetErrorParams(
...
void CPhysicsConstraintGroup::SetErrorParams(
...
void CPhysicsConstraintGroup::SolvePenetration(IPhysicsObject *pObj0,
...
CPhysicsConstraintGroup::~CPhysicsConstraintGroup() { delete m_pLCS; }

CPhysicsConstraintGroup::CPhysicsConstraintGroup(
    IVP_Environment *pEnvironment, const constraint_groupparams_t &group) {
  ...
  m_pLCS = new hk_Local_Constraint_System(
      static_cast<hk_Environment *>(pEnvironment), &cs_bp);
...
DEFINE_FIELD(constraintType, FIELD_INTEGER), DEFINE_VPHYSPTR(pGroup),
    DEFINE_VPHYSPTR(pObjReference), DEFINE_VPHYSPTR(pObjAttached),
    END_DATADESC()

        struct vphysics_save_constraintbreakable_t
    : public constraint_breakableparams_t {
  DECLARE_SIMPLE_DATADESC();
};

BEGIN_SIMPLE_DATADESC(vphysics_save_constraintbreakable_t)
DEFINE_FIELD(strength, FIELD_FLOAT), DEFINE_FIELD(forceLimit, FIELD_FLOAT),
    DEFINE_FIELD(torqueLimit, FIELD_FLOAT),
    DEFINE_AUTO_ARRAY(bodyMassScale, FIELD_FLOAT),
    DEFINE_FIELD(isActive, FIELD_BOOLEAN),
    END_DATADESC()

        struct vphysics_save_constraintaxislimit_t
...
DEFINE_FIELD(minRotation, FIELD_FLOAT), DEFINE_FIELD(maxRotation, FIELD_FLOAT),
    ...
    END_DATADESC()
        ...
        struct vphysics_save_constraintfixed_t
    : public constraint_fixedparams_t {
  DECLARE_SIMPLE_DATADESC();
};
...
DEFINE_AUTO_ARRAY2D(attachedRefXform, FIELD_FLOAT),
    ...
    END_DATADESC()
        ...
        struct vphysics_save_constrainthinge_t
...
DEFINE_FIELD(worldPosition, FIELD_POSITION_VECTOR),
    ...
    END_DATADESC()
        ...
        struct vphysics_save_constraintsliding_t
    : public constraint_slidingparams_t {
  DECLARE_SIMPLE_DATADESC();
};
...
DEFINE_AUTO_ARRAY2D(attachedRefXform, FIELD_FLOAT),
    ...
    DEFINE_FIELD(limitMin, FIELD_FLOAT), DEFINE_FIELD(limitMax, FIELD_FLOAT),
    DEFINE_FIELD(friction, FIELD_FLOAT), DEFINE_FIELD(velocity, FIELD_FLOAT),
    DEFINE_EMBEDDED_OVERRIDE(constraint, vphysics_save_constraintbreakable_t),
    END_DATADESC()

        struct vphysics_save_constraintpulley_t
    : public constraint_pulleyparams_t {
  DECLARE_SIMPLE_DATADESC();
};
...
DEFINE_AUTO_ARRAY(pulleyPosition, FIELD_POSITION_VECTOR),
    ...
    DEFINE_FIELD(gearRatio, FIELD_FLOAT), DEFINE_FIELD(isRigid, FIELD_BOOLEAN),
    DEFINE_EMBEDDED_OVERRIDE(constraint, vphysics_save_constraintbreakable_t),
    END_DATADESC()

        struct vphysics_save_constraintlength_t
    : public constraint_lengthparams_t {
  DECLARE_SIMPLE_DATADESC();
};
...
DEFINE_AUTO_ARRAY(objectPosition, FIELD_VECTOR),
    ...
    END_DATADESC()
        ...
        struct vphysics_save_constraintballsocket_t
...
DEFINE_AUTO_ARRAY(constraintPosition, FIELD_VECTOR),
    ...
    END_DATADESC()
        ...
        struct vphysics_save_constraintragdoll_t
    : public constraint_ragdollparams_t {
  DECLARE_SIMPLE_DATADESC();
};
...
public:
  ...
  hk_Constraint *
  CreateBreakableConstraint(hk_Constraint *pRealConstraint,
    ...
    bp.m_linear_strength =
        forceLimit > 0 ? forceLimit : UNBREAKABLE_BREAK_LIMIT;
    bp.m_angular_strength = constraint.torqueLimit > 0
                                ? DEG2RAD(constraint.torqueLimit)
                                : UNBREAKABLE_BREAK_LIMIT;
    bp.m_bodyMassScale[0] =
        constraint.bodyMassScale[0] > 0 ? constraint.bodyMassScale[0] : 1.0f;
    bp.m_bodyMassScale[1] =
        constraint.bodyMassScale[1] > 0 ? constraint.bodyMassScale[1] : 1.0f;
    ;
  ...
  void SetupRagdollAxis(int axis, const constraint_axislimit_t &axisData,
  ...
  bool GetConstraintTransform(matrix3x4_t *pConstraintToReference,
  ...
  void OutputDebugInfo() {
    ...
    if (pLCS) {
      ...
      for (hk_Array<hk_Constraint *>::iterator i = list.start();
           list.is_valid(i); i = list.next(i)) {
...
CPhysicsConstraint::CPhysicsConstraint(CPhysicsObject *pReferenceObject,
                                       CPhysicsObject *pAttachedObject) {
  m_pGameData = NULL;
  m_HkConstraint = NULL;
  m_HkLCS = NULL;
...
// Check to see if this is a single degree of freedom joint, if so, convert to a
// hinge
static bool ConvertRagdollToHinge(constraint_limitedhingeparams_t *pHingeOut,
                                  const constraint_ragdollparams_t &ragdoll,
  ...
  ConcatTransforms(refToWorld, ragdoll.constraintToReference,
                   constraintToWorld);
  ...
  Vector perpCS;
  VectorIRotate(pHingeOut->referencePerpAxisDirection,
                ragdoll.constraintToReference, perpCS);
  VectorRotate(perpCS, ragdoll.constraintToAttached,
               pHingeOut->attachedPerpAxisDirection);

  pHingeOut->hingeAxis = ragdoll.axes[dofIndex];

  // Funky math to insure that the friction is preserved after the math that the
  // hinge code uses.
  pHingeOut->hingeAxis.torque =
      RAD2DEG(pHingeOut->hingeAxis.torque * pReferenceObject->GetMass());
...
// ragdoll constraint
void CPhysicsConstraint::InitRagdoll(
    IVP_Environment *pEnvironment, CPhysicsConstraintGroup *constraint_group,
    const constraint_ragdollparams_t &ragdoll) {
  // UNDONE: If this is a hinge parameterized using the ragdoll params, use a
  // hinge instead!
  ...
  hk_Limited_Ball_Socket_BP ballsocketBP;
  ConvertHLLocalMatrixToHavanaLocal(ragdoll.constraintToReference,
                                    ballsocketBP.m_transform_os_ks[0]);
  ConvertHLLocalMatrixToHavanaLocal(ragdoll.constraintToAttached,
                                    ballsocketBP.m_transform_os_ks[1]);
  ...
  ballsocketBP.m_constrainTranslation =
      ragdoll.onlyAngularLimits ? false : true;

  // swap the input limits if they are clockwise (angles are counter-clockwise)
  if (ragdoll.useClockwiseRotations) {
    for (i = 0; i < 3; i++) {
      float tmp = ballsocketBP.m_angular_limits[i].m_min;
      ballsocketBP.m_angular_limits[i].m_min =
          -ballsocketBP.m_angular_limits[i].m_max;
  ...
  hk_Ragdoll_Constraint_BP *bp =
      (hk_Ragdoll_Constraint_BP *)r_builder.get_blueprint(); // get non const bp
  ...
  for (i = 0; i < 3; i++) {
    ...
    // initialize_from_limited_ball_socket_bp remapped the axes too!  So remap
    // again.
    int hkAxis = revAxisMapHK[ivpAxis];

    const constraint_axislimit_t &axisData = ragdoll.axes[i];
    bp->m_limits[hkAxis].set_motor(DEG2RAD(axisData.angularVelocity),
                                   axisData.torque *
                                       m_pObjReference->GetMass());
    bp->m_tau = 1.0f;
  }

  hk_Local_Constraint_System *lcs =
      constraint_group ? constraint_group->GetLCS() : NULL;
  ...
  if (breakable) {
    hk_Ragdoll_Constraint *pConstraint =
        new hk_Ragdoll_Constraint(hkEnvironment, bp, ref, att);
    m_HkConstraint =
        CreateBreakableConstraint(pConstraint, lcs, ragdoll.constraint);
  } else {
...
// hinge constraint
void CPhysicsConstraint::InitHinge(
    IVP_Environment *pEnvironment, CPhysicsConstraintGroup *constraint_group,
    const constraint_limitedhingeparams_t &hinge) {
  ...
  builder.set_axis_ws((hk_Rigid_Body *)m_pObjReference->GetObject(),
                      (hk_Rigid_Body *)m_pObjAttached->GetObject(),
                      vec(axisIVP_ws));
  builder.set_position_os(
      0, TransformHLWorldToHavanaLocal(hinge.worldPosition,
                                       m_pObjReference->GetObject()));
  builder.set_position_os(
      1, TransformHLWorldToHavanaLocal(hinge.worldPosition,
  ...
  if (hinge.hingeAxis.torque != 0) {
    builder.set_angular_motor(DEG2RAD(hinge.hingeAxis.angularVelocity),
                              DEG2RAD(hinge.hingeAxis.torque));
  }
  if (hinge.hingeAxis.minRotation != hinge.hingeAxis.maxRotation) {
    builder.set_angular_limits(DEG2RAD(hinge.hingeAxis.minRotation),
                               DEG2RAD(hinge.hingeAxis.maxRotation));
  }
  hk_Local_Constraint_System *lcs =
      constraint_group ? constraint_group->GetLCS() : NULL;
  if (!lcs) {
  ...
  if (breakable) {
    hk_Hinge_Constraint *pHinge =
        new hk_Hinge_Constraint(hkEnvironment, builder.get_blueprint(),
                                (hk_Rigid_Body *)m_pObjReference->GetObject(),
                                (hk_Rigid_Body *)m_pObjAttached->GetObject());
    m_HkConstraint = CreateBreakableConstraint(pHinge, lcs, hinge.constraint);
  } else {
    m_HkConstraint =
        new hk_Hinge_Constraint(lcs, builder.get_blueprint(),
                                (hk_Rigid_Body *)m_pObjReference->GetObject(),
                                (hk_Rigid_Body *)m_pObjAttached->GetObject());
  }
...
// fixed constraint
void CPhysicsConstraint::InitFixed(IVP_Environment *pEnvironment,
                                   CPhysicsConstraintGroup *constraint_group,
                                   const constraint_fixedparams_t &fixed) {
  ...
  ConvertHLLocalMatrixToHavanaLocal(fixed.attachedRefXform,
                                    fixed_bp.m_transform_os_ks);
  ...
  if (breakable) {
    hk_Constraint *pFixed = new hk_Fixed_Constraint(
        hkEnvironment, &fixed_bp, (hk_Rigid_Body *)m_pObjReference->GetObject(),
        (hk_Rigid_Body *)m_pObjAttached->GetObject());

    m_HkConstraint = CreateBreakableConstraint(pFixed, lcs, fixed.constraint);
  } else {
    m_HkConstraint = new hk_Fixed_Constraint(
        lcs, &fixed_bp, (hk_Rigid_Body *)m_pObjReference->GetObject(),
        (hk_Rigid_Body *)m_pObjAttached->GetObject());
  }
...
void CPhysicsConstraint::InitBallsocket(
    IVP_Environment *pEnvironment, CPhysicsConstraintGroup *constraint_group,
    const constraint_ballsocketparams_t &ballsocket) {
  ...
  if (breakable) {
    hk_Ball_Socket_Constraint *pConstraint = new hk_Ball_Socket_Constraint(
        hkEnvironment, &builder, (hk_Rigid_Body *)m_pObjReference->GetObject(),
        (hk_Rigid_Body *)m_pObjAttached->GetObject());
    m_HkConstraint =
        CreateBreakableConstraint(pConstraint, lcs, ballsocket.constraint);
  } else {
    m_HkConstraint = new hk_Ball_Socket_Constraint(
        lcs, &builder, (hk_Rigid_Body *)m_pObjReference->GetObject(),
        (hk_Rigid_Body *)m_pObjAttached->GetObject());
  }
...
void CPhysicsConstraint::InitSliding(
    IVP_Environment *pEnvironment, CPhysicsConstraintGroup *constraint_group,
    const constraint_slidingparams_t &sliding) {
  ...
  if (sliding.limitMin != sliding.limitMax) {
    bp.set_limits(ConvertDistanceToIVP(sliding.limitMin),
                  ConvertDistanceToIVP(sliding.limitMax));
  }
  if (sliding.friction) {
    if (sliding.velocity) {
      bp.set_motor(ConvertDistanceToIVP(sliding.velocity),
                   ConvertDistanceToIVP(sliding.friction));
    } else {
  ...
  if (breakable) {
    hk_Constraint *pFixed = new hk_Prismatic_Constraint(
        hkEnvironment, &prismatic_bp,
        (hk_Rigid_Body *)m_pObjReference->GetObject(),
        (hk_Rigid_Body *)m_pObjAttached->GetObject());

    m_HkConstraint = CreateBreakableConstraint(pFixed, lcs, sliding.constraint);
  } else {
    m_HkConstraint = new hk_Prismatic_Constraint(
        lcs, &prismatic_bp, (hk_Rigid_Body *)m_pObjReference->GetObject(),
        (hk_Rigid_Body *)m_pObjAttached->GetObject());
  }
...
void CPhysicsConstraint::InitPulley(IVP_Environment *pEnvironment,
                                    CPhysicsConstraintGroup *constraint_group,
                                    const constraint_pulleyparams_t &pulley) {
  ...
  ConvertPositionToIVP(pulley.objectPosition[0],
                       pulley_bp.m_translation_os_ks[0]);
  ConvertPositionToIVP(pulley.objectPosition[1],
                       pulley_bp.m_translation_os_ks[1]);

  // set the pully positions in world space
  ConvertPositionToIVP(pulley.pulleyPosition[0],
                       pulley_bp.m_worldspace_point[0]);
  ConvertPositionToIVP(pulley.pulleyPosition[1],
                       pulley_bp.m_worldspace_point[1]);
  ...
  if (breakable) {
    hk_Constraint *pPulley =
        new hk_Pulley_Constraint(hkEnvironment, &pulley_bp,
                                 (hk_Rigid_Body *)m_pObjReference->GetObject(),
                                 (hk_Rigid_Body *)m_pObjAttached->GetObject());

    m_HkConstraint = CreateBreakableConstraint(pPulley, lcs, pulley.constraint);
  } else {
    m_HkConstraint = new hk_Pulley_Constraint(
        lcs, &pulley_bp, (hk_Rigid_Body *)m_pObjReference->GetObject(),
        (hk_Rigid_Body *)m_pObjAttached->GetObject());
  }
...
void CPhysicsConstraint::InitLength(IVP_Environment *pEnvironment,
                                    CPhysicsConstraintGroup *constraint_group,
                                    const constraint_lengthparams_t &length) {
  ...
  // set the anchor positions in object space
  ConvertPositionToIVP(length.objectPosition[0],
                       stiff_bp.m_translation_os_ks[0]);
  ConvertPositionToIVP(length.objectPosition[1],
                       stiff_bp.m_translation_os_ks[1]);
  ...
  if (breakable) {
    hk_Constraint *pLength = new hk_Stiff_Spring_Constraint(
        hkEnvironment, &stiff_bp, (hk_Rigid_Body *)m_pObjReference->GetObject(),
        (hk_Rigid_Body *)m_pObjAttached->GetObject());

    m_HkConstraint = CreateBreakableConstraint(pLength, lcs, length.constraint);
  } else {
    m_HkConstraint = new hk_Stiff_Spring_Constraint(
        lcs, &stiff_bp, (hk_Rigid_Body *)m_pObjReference->GetObject(),
        (hk_Rigid_Body *)m_pObjAttached->GetObject());
  }
...
void CPhysicsConstraint::WriteToTemplate(
...
void CPhysicsConstraint::SetLinearMotor(float speed, float maxLinearImpulse) {
  ...
  hk_Prismatic_Constraint *pConstraint =
      (hk_Prismatic_Constraint *)GetRealConstraint();
  pConstraint->set_motor(ConvertDistanceToIVP(speed),
                         ConvertDistanceToIVP(maxLinearImpulse));
}

void CPhysicsConstraint::SetAngularMotor(float rotSpeed,
                                         float maxAngularImpulse) {
  if (m_constraintType == CONSTRAINT_RAGDOLL && rotSpeed == 0) {
    hk_Ragdoll_Constraint *pConstraint =
        (hk_Ragdoll_Constraint *)GetRealConstraint();
    pConstraint->update_friction(ConvertAngleToIVP(maxAngularImpulse));
  } else if (m_constraintType == CONSTRAINT_HINGE) {
    hk_Hinge_Constraint *pConstraint =
        (hk_Hinge_Constraint *)GetRealConstraint();
    pConstraint->set_motor(ConvertAngleToIVP(rotSpeed),
                           ConvertAngleToIVP(fabs(maxAngularImpulse)));
  }
}

void CPhysicsConstraint::UpdateRagdollTransforms(
...
bool CPhysicsConstraint::GetConstraintTransform(
  ...
  if (m_constraintType == CONSTRAINT_RAGDOLL) {
    ...
    if (pConstraintToReference) {
      ConvertHavanaLocalMatrixToHL(pConstraint->get_transform(0),
                                   *pConstraintToReference, NULL);
    }
    if (pConstraintToAttached) {
      ConvertHavanaLocalMatrixToHL(pConstraint->get_transform(1),
                                   *pConstraintToAttached, NULL);
    }
    return true;
  } else if (m_constraintType == CONSTRAINT_BALLSOCKET) {
    hk_Ball_Socket_Constraint *pConstraint =
        (hk_Ball_Socket_Constraint *)GetRealConstraint();
    Vector pos;
    if (pConstraintToReference) {
    ...
    hk_Fixed_Constraint *pConstraint =
        (hk_Fixed_Constraint *)GetRealConstraint();
    if (pConstraintToReference) {
      ConvertHavanaLocalMatrixToHL(pConstraint->get_transform(0),
                                   *pConstraintToReference, NULL);
    }
...
static bool
IsValidConstraint(const vphysics_save_cphysicsconstraint_t &header) {
  if (header.constraintType != CONSTRAINT_UNKNOWN && header.pObjAttached &&
      header.pObjReference)
    return true;

  return false;
}

bool CPhysicsConstraint::GetConstraintParams(
    constraint_breakableparams_t *pParams) const {
  if (!pParams)
    return false;
  vphysics_save_cphysicsconstraint_t header;
...
void CPhysicsConstraint::ReadBreakableConstraint(
    constraint_breakableparams_t &params) const {
  if (m_isBreakable) {
    hk_Breakable_Constraint_BP bp;
    ((hk_Breakable_Constraint *)m_HkConstraint)->write_to_blueprint(&bp);
    ...
    // Assert( m_HkLCS != NULL ); // this is allowed now although breaking
    // inside an LCS won't work yet
  } else {
    params.Defaults();
  }
...
void CPhysicsConstraint::WriteFixed(constraint_fixedparams_t &fixed) const {
  ...
  ConvertHavanaLocalMatrixToHL(fixed_bp.m_transform_os_ks,
                               fixed.attachedRefXform,
                               m_pObjReference->GetObject());
}

void CPhysicsConstraint::WriteRagdoll(
    constraint_ragdollparams_t &ragdoll) const {
  hk_Ragdoll_Constraint *pConstraint =
      (hk_Ragdoll_Constraint *)GetRealConstraint();
  ...
  ConvertHavanaLocalMatrixToHL(ragdoll_bp.m_transform_os_ks[0],
                               ragdoll.constraintToReference,
                               m_pObjReference->GetObject());
  ConvertHavanaLocalMatrixToHL(ragdoll_bp.m_transform_os_ks[1],
                               ragdoll.constraintToAttached,
                               m_pObjAttached->GetObject());
  int revAxisMapHK[3];
  ...
  for (int i = 0; i < 3; i++) {
    ...
    ragdollAxis.torque =
        bpAxis.m_joint_friction * m_pObjReference->GetInvMass();
    // X&Y
    if (i != 2) {
...
void CPhysicsConstraint::WriteHinge(constraint_hingeparams_t &hinge) const {
  ...
  hinge.worldPosition = TransformHavanaLocalToHLWorld(
      hinge_bp.m_axis_os[0].get_origin(), m_pObjReference->GetObject(), true);
  hinge.worldAxisDirection =
      TransformHavanaLocalToHLWorld(hinge_bp.m_axis_os[0].get_direction(),
                                    m_pObjReference->GetObject(), false);
  ...
  if (hinge_bp.m_limit.m_friction_is_enabled) {
    hinge.hingeAxis.angularVelocity =
        RAD2DEG(hinge_bp.m_limit.m_desired_velocity);
    hinge.hingeAxis.torque = RAD2DEG(hinge_bp.m_limit.m_joint_friction);
  }
}

void CPhysicsConstraint::WriteSliding(
    constraint_slidingparams_t &sliding) const {
  sliding.Defaults();
  hk_Prismatic_Constraint *pConstraint =
      (hk_Prismatic_Constraint *)GetRealConstraint();
  ...
  ConvertHavanaLocalMatrixToHL(t, sliding.attachedRefXform,
                               m_pObjReference->GetObject());
  if (prismatic_bp.m_limit.m_friction_is_enabled) {
    sliding.friction =
        ConvertDistanceToHL(prismatic_bp.m_limit.m_joint_friction);
    sliding.velocity =
        ConvertDistanceToHL(prismatic_bp.m_limit.m_desired_velocity);
  }
...
void CPhysicsConstraint::WritePulley(constraint_pulleyparams_t &pulley) const {
  pulley.Defaults();
  hk_Pulley_Constraint *pConstraint =
      (hk_Pulley_Constraint *)GetRealConstraint();
  ...
  // save bp into params
  for (int i = 0; i < 2; i++) {
    ConvertPositionToHL(pulley_bp.m_worldspace_point[i],
                        pulley.pulleyPosition[i]);
    ConvertPositionToHL(pulley_bp.m_translation_os_ks[i],
                        pulley.objectPosition[i]);
  }
...
void CPhysicsConstraint::WriteLength(constraint_lengthparams_t &length) const {
  length.Defaults();
  hk_Stiff_Spring_Constraint *pConstraint =
      (hk_Stiff_Spring_Constraint *)GetRealConstraint();
  ...
  // save bp into params
  for (int i = 0; i < 2; i++) {
    ConvertPositionToHL(stiff_bp.m_translation_os_ks[i],
                        length.objectPosition[i]);
  }
...
void CPhysicsConstraint::WriteBallsocket(
    constraint_ballsocketparams_t &ballsocket) const {
  ballsocket.Defaults();
  hk_Ball_Socket_Constraint *pConstraint =
      (hk_Ball_Socket_Constraint *)GetRealConstraint();
  ...
  // save bp into params
  for (int i = 0; i < 2; i++) {
    ConvertPositionToHL(ballsocket_bp.m_translation_os_ks[i],
                        ballsocket.constraintPosition[i]);
  }
}
...
void CPhysicsConstraint::event_object_deleted(IVP_Event_Object *pEvent) {
  ...
  if (pEvent->environment) {
    CPhysicsEnvironment *pEnvironment =
        (CPhysicsEnvironment *)pEvent->environment->client_data;
...
void CPhysicsConstraint::SetupRagdollAxis(
    int axis, const constraint_axislimit_t &axisData,
    hk_Limited_Ball_Socket_BP *ballsocketBP) {
  // X & Y
  if (axis != 2) {
    ballsocketBP->m_angular_limits[ConvertCoordinateAxisToIVP(axis)].set(
        DEG2RAD(axisData.minRotation), DEG2RAD(axisData.maxRotation));
  }
  // Z
  else {
    ballsocketBP->m_angular_limits[ConvertCoordinateAxisToIVP(axis)].set(
        DEG2RAD(-axisData.maxRotation), DEG2RAD(-axisData.minRotation));
  }
}
...
IPhysicsConstraint *
GetClientDataForHkConstraint(class hk_Breakable_Constraint *pHkConstraint) {
...
CreatePhysicsConstraintGroup(IVP_Environment *pEnvironment,
                             const constraint_groupparams_t &group) {
  MEM_ALLOC_CREDIT();
...
IPhysicsConstraint *CreateRagdollConstraint(
    IVP_Environment *pEnvironment, CPhysicsObject *pReferenceObject,
    CPhysicsObject *pAttachedObject, IPhysicsConstraintGroup *pGroup,
    const constraint_ragdollparams_t &ragdoll) {
  MEM_ALLOC_CREDIT();
  CPhysicsConstraint *pConstraint =
      new CPhysicsConstraint(pReferenceObject, pAttachedObject);
  pConstraint->InitRagdoll(pEnvironment, (CPhysicsConstraintGroup *)pGroup,
                           ragdoll);
  return pConstraint;
}
IPhysicsConstraint *CreateHingeConstraint(
  ...
  CPhysicsConstraint *pConstraint =
      new CPhysicsConstraint(pReferenceObject, pAttachedObject);
  pConstraint->InitHinge(pEnvironment, (CPhysicsConstraintGroup *)pGroup,
                         hinge);
  return pConstraint;
}
...
IPhysicsConstraint *CreateFixedConstraint(
  ...
  CPhysicsConstraint *pConstraint =
      new CPhysicsConstraint(pReferenceObject, pAttachedObject);
  pConstraint->InitFixed(pEnvironment, (CPhysicsConstraintGroup *)pGroup,
                         fixed);
  return pConstraint;
}
...
IPhysicsConstraint *CreateSlidingConstraint(
  ...
  CPhysicsConstraint *pConstraint =
      new CPhysicsConstraint(pReferenceObject, pAttachedObject);
  pConstraint->InitSliding(pEnvironment, (CPhysicsConstraintGroup *)pGroup,
                           sliding);
  return pConstraint;
}
...
IPhysicsConstraint *CreateBallsocketConstraint(
  ...
  CPhysicsConstraint *pConstraint =
      new CPhysicsConstraint(pReferenceObject, pAttachedObject);
  pConstraint->InitBallsocket(pEnvironment, (CPhysicsConstraintGroup *)pGroup,
                              ballsocket);
  return pConstraint;
}
...
IPhysicsConstraint *CreatePulleyConstraint(
  ...
  CPhysicsConstraint *pConstraint =
      new CPhysicsConstraint(pReferenceObject, pAttachedObject);
  pConstraint->InitPulley(pEnvironment, (CPhysicsConstraintGroup *)pGroup,
                          pulley);
  return pConstraint;
}
...
IPhysicsConstraint *CreateLengthConstraint(
  ...
  CPhysicsConstraint *pConstraint =
      new CPhysicsConstraint(pReferenceObject, pAttachedObject);
  pConstraint->InitLength(pEnvironment, (CPhysicsConstraintGroup *)pGroup,
                          length);
  return pConstraint;
}
      ...
      if (pCores->element_at(i)) {
        ...
        if (pivp) {
          IPhysicsObject *pObject =
              static_cast<IPhysicsObject *>(pivp->client_data);
...
bool SavePhysicsConstraint(const physsaveparams_t &params,
                           CPhysicsConstraint *pConstraint) {
  vphysics_save_cphysicsconstraint_t header;
  vphysics_save_constraint_t constraintTemplate;
  memset(&header, 0, sizeof(header));
...
bool RestorePhysicsConstraint(const physrestoreparams_t &params,
  ...
  if (IsValidConstraint(header)) {
    ...
    case CONSTRAINT_HINGE: {
      ...
      CPhysicsEnvironment *pEnvironment =
          (CPhysicsEnvironment *)params.pEnvironment;
    ...
    case CONSTRAINT_SLIDING: {
      ...
      CPhysicsEnvironment *pEnvironment =
          (CPhysicsEnvironment *)params.pEnvironment;
      *ppConstraint =
          (CPhysicsConstraint *)pEnvironment->CreateSlidingConstraint(
              header.pObjReference, header.pObjAttached, header.pGroup,
              sliding);
    } break;
    case CONSTRAINT_PULLEY: {
      vphysics_save_constraintpulley_t pulley;
      ...
      CPhysicsEnvironment *pEnvironment =
          (CPhysicsEnvironment *)params.pEnvironment;
      *ppConstraint =
          (CPhysicsConstraint *)pEnvironment->CreatePulleyConstraint(
    ...
    case CONSTRAINT_LENGTH: {
      ...
      CPhysicsEnvironment *pEnvironment =
          (CPhysicsEnvironment *)params.pEnvironment;
      *ppConstraint =
          (CPhysicsConstraint *)pEnvironment->CreateLengthConstraint(
    ...
    case CONSTRAINT_BALLSOCKET: {
      ...
      CPhysicsEnvironment *pEnvironment =
          (CPhysicsEnvironment *)params.pEnvironment;
      *ppConstraint =
          (CPhysicsConstraint *)pEnvironment->CreateBallsocketConstraint(
              header.pObjReference, header.pObjAttached, header.pGroup,
              ballsocket);
    } break;
    case CONSTRAINT_RAGDOLL: {
      ...
      CPhysicsEnvironment *pEnvironment =
          (CPhysicsEnvironment *)params.pEnvironment;
      *ppConstraint =
          (CPhysicsConstraint *)pEnvironment->CreateRagdollConstraint(
              header.pObjReference, header.pObjAttached, header.pGroup,
              ragdoll);
    } break;
    }
...
bool SavePhysicsConstraintGroup(const physsaveparams_t &params,
                                CPhysicsConstraintGroup *pConstraintGroup) {
...
bool RestorePhysicsConstraintGroup(
  ...
  CPhysicsEnvironment *pEnvironment =
      (CPhysicsEnvironment *)params.pEnvironment;
  *ppConstraintGroup =
      (CPhysicsConstraintGroup *)pEnvironment->CreateConstraintGroup(
          groupTemplate);
