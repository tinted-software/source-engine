...
vphysics_units_t g_PhysicsUnits = {
    METERS_PER_INCH, // float		unitScaleMeters;
                     // // factor that converts game units to meters
    1.0f / METERS_PER_INCH, // float		unitScaleMetersInv;
                            // // factor that converts meters to game units
    0.25f,         // float		globalCollisionTolerance;	// global
                   // collision tolerance in game units
    DIST_EPSILON,  // float		collisionSweepEpsilon;		//
                   // collision sweep tests clip at this, must be the same as
                   // engine's DIST_EPSILON
    1.0f / 256.0f, // float		collisionSweepIncrementalEpsilon;
                   // // near-zero test for incremental steps in collision sweep
                   // tests
};
#else
// game is in meters
vphysics_units_t g_PhysicsUnits = {
    1.0f, // float		unitScaleMeters;			//
          // factor that converts game units to meters
    1.0f, // float		unitScaleMetersInv;			//
          // factor that converts meters to game units
    0.01f, // float		globalCollisionTolerance;	// global
           // collision tolerance in game units
    0.01f, // float		collisionSweepEpsilon;		// collision
           // sweep tests clip at this, must be the same as engine's
           // DIST_EPSILON
    1e-4f, // float		collisionSweepIncrementalEpsilon;	//
           // near-zero test for incremental steps in collision sweep tests
};
#endif
...
void ConvertBoxToIVP(const Vector &mins, const Vector &maxs, Vector &outmins,
                     Vector &outmaxs) {
  float tmpZ;
...
void ConvertRotationToHL(const IVP_U_Matrix3 &in, QAngle &angles) {
  ...
  {
    // (yaw)	y = ATAN( -right.x, right.y );		-- forward is mostly z,
...
// utiltiy code
void TransformIVPToLocal(IVP_U_Point &point, IVP_Real_Object *pObject,
                         bool translate) {
  IVP_U_Point tmp = point;
  TransformIVPToLocal(tmp, point, pObject, translate);
}

void TransformLocalToIVP(IVP_U_Point &point, IVP_Real_Object *pObject,
                         bool translate) {
  IVP_U_Point tmp = point;
...
static const IVP_U_Matrix *GetTmpObjectMatrix(IVP_Real_Object *pObject) {
  ...
  if (!pObject->flags.shift_core_f_object_is_zero) {
    ...
    coreShiftMatrix.vmult4(pObject->get_shift_core_f_object(),
                           &coreShiftMatrix.vv);
...
void TransformIVPToLocal(const IVP_U_Point &pointIn, IVP_U_Point &pointOut,
...
void TransformLocalToIVP(const IVP_U_Point &pointIn, IVP_U_Point &pointOut,
                         IVP_Real_Object *pObject, bool translate) {
#if USE_CACHE_OBJECT
  IVP_Cache_Object *cache = pObject->get_cache_object_no_lock();
...
void TransformLocalToIVP(const IVP_U_Float_Point &pointIn,
...
void TransformLocalToIVP(const IVP_U_Float_Point &pointIn,
                         IVP_U_Float_Point &pointOut, IVP_Real_Object *pObject,
                         bool translate) {
  IVP_U_Point tmpOut;
