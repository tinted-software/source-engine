...
static void ivu_string_print_function(const char *str) { Msg("%s", str); }
...
public:
  ...
  CPhysicsCollisionSet() { memset(m_bits, 0, sizeof(m_bits)); }
...
public:
  ...
  virtual IPhysicsCollisionSet *FindOrCreateCollisionSet(unsigned int id,
                                                         int maxElementCount);
...
void *CPhysicsInterface::QueryInterface(const char *pInterfaceName) {
  ...
  // This includes the backward-compatible interfaces + other vphysics
  // interfaces
...
// NOTE: This is stuffed into vphysics.dll as a sneaky way of sharing the memory
// between client and server in single player.  So you can't have different
// client/server rules.
IPhysicsCollisionSet *
CPhysicsInterface::FindOrCreateCollisionSet(unsigned int id,
                                            int maxElementCount) {
...
// In release build, each of these libraries must contain a symbol that
// indicates it is also a release build You MUST disable this in order to run a
// release vphysics.dll with a debug library. This should not usually be
// necessary #if !defined(_DEBUG) && defined(_WIN32) extern int
// ivp_physics_lib_is_a_release_build; extern int
// ivp_compactbuilder_lib_is_a_release_build; extern int
// hk_base_lib_is_a_release_build; extern int hk_math_lib_is_a_release_build;
