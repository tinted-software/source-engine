...
DEFINE_FIELD(constant, FIELD_FLOAT), DEFINE_FIELD(naturalLength, FIELD_FLOAT),
    DEFINE_FIELD(damping, FIELD_FLOAT),
    DEFINE_FIELD(relativeDamping, FIELD_FLOAT),

    // NOTE: These are stored as *relative* vectors, so we don't use
    ...
    // BUGBUG: No way to delete a spring because springs get auto-deleted
    // without notification when an object they are attached to is deleted. So
    // there is no way to tell if the pointer is valid.
    class CPhysicsSpring : public IPhysicsSpring,
                           public IVP_Listener_Object {
public:
  CPhysicsSpring(CPhysicsObject *pObjectStart, CPhysicsObject *pObjectEnd,
...
CPhysicsSpring::CPhysicsSpring(CPhysicsObject *pObjectStart,
...
void CPhysicsSpring::GetEndpoints(Vector *worldPositionStart,
...
void CPhysicsSpring::SetSpringLength(float flSpringLength) {
  if (m_pSpring) {
    ...
    if (fabs(desiredLengthIVP - currentLengthIVP) <
        ConvertDistanceToIVP(SPRING_LENGTH_EPSILON))
      return;
...
void CPhysicsSpring::WriteToTemplate(vphysics_save_cphysicsspring_t &params) {
  ...
  params.naturalLength =
      ConvertDistanceToHL(m_pSpring->get_spring_length_zero_force());
...
IPhysicsSpring *CreateSpring(IVP_Environment *pEnvironment,
  ...
  spring_template.spring_force_only_on_stretch =
      pParams->onlyStretch ? IVP_TRUE : IVP_FALSE;
  ...
  anchorTemplateObjectStart.set_anchor_position_os(pObjectStart->GetObject(),
                                                   &ivpPosStart);
  anchorTemplateObjectEnd.set_anchor_position_os(pObjectEnd->GetObject(),
                                                 &ivpPosEnd);
...
bool SavePhysicsSpring(const physsaveparams_t &params,
                       CPhysicsSpring *pSpring) {
...
bool RestorePhysicsSpring(const physrestoreparams_t &params,
  ...
  if (springTemplate.pObjStart && springTemplate.pObjEnd) {
    *ppSpring = (CPhysicsSpring *)pEnvironment->CreateSpring(
        springTemplate.pObjStart, springTemplate.pObjEnd, &springTemplate);
  } else {
