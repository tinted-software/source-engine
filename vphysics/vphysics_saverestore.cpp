...
void CPhysicsEnvironment::PreRestore(const physprerestoreparams_t &params) {
  ...
  for (int i = 0; i < params.recreatedObjectCount; i++) {
    AddPtrAssociation(params.recreatedObjectList[i].pOldObject,
...
void CVPhysPtrSaveRestoreOps::Save(const SaveRestoreFieldInfo_t &fieldInfo,
                                   ISave *pSave) {
...
void CVPhysPtrSaveRestoreOps::Restore(const SaveRestoreFieldInfo_t &fieldInfo,
                                      IRestore *pRestore) {
...
void CVPhysPtrUtlVectorSaveRestoreOps::Save(
    const SaveRestoreFieldInfo_t &fieldInfo, ISave *pSave) {
...
void CVPhysPtrUtlVectorSaveRestoreOps::Restore(
    const SaveRestoreFieldInfo_t &fieldInfo, IRestore *pRestore) {
  Assert(fieldInfo.pTypeDesc->fieldSize == 1);
