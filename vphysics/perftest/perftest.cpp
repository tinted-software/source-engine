...
void PhysParseSurfaceData(IPhysicsSurfaceProps *pProps,
  ...
  if (manifest->LoadFromFile(pFileSystem, SURFACEPROP_MANIFEST_FILE, "GAME")) {
    for (KeyValues *sub = manifest->GetFirstSubKey(); sub != NULL;
      ...
      Warning("surfaceprops::Init:  Manifest '%s' with bogus file type '%s', "
              "expecting 'file'\n",
...
struct physicstest_t {
  ...
  int AddObject(IPhysicsObject *pObject) {
    ...
    list[index].pModel =
        physcollision->CreateQueryModel((CPhysCollide *)pObject->GetCollide());
    return index;
  }

  void CreateGround(float size) {
    {
      CPhysCollide *pCollide = physcollision->BBoxToCollide(
          Vector(-size, -size, -24), Vector(size, size, 0));
      objectparams_t params = g_PhysDefaultObjectParams;
      IPhysicsObject *pGround = physenv->CreatePolyObjectStatic(
          pCollide, physprops->GetSurfaceIndex("default"), vec3_origin,
          vec3_angle, &params);
      AddObject(pGround);
    }
    ...
    for (int i = 0; i < 20; i++) {
      CPhysCollide *pCollide = physcollision->BBoxToCollide(
          Vector(-24, -24, -24), Vector(24, 24, 24));
      objectparams_t params = g_PhysDefaultObjectParams;
      params.mass = 150.0f;
      IPhysicsObject *pGround = physenv->CreatePolyObject(
          pCollide, physprops->GetSurfaceIndex("default"),
  ...
  void DrawObject(ICollisionQuery *pModel, IMaterial *pMaterial,
  ...
  void Simulate(float frametime) { physenv->Simulate(frametime); }
};
...
void AppInit(void) {
  ...
  if (!(physics =
            (IPhysics *)physicsFactory(VPHYSICS_INTERFACE_VERSION, NULL)) ||
  ...
  g_materialFlatshaded = g_MaterialSystemApp.m_pMaterialSystem->FindMaterial(
      "debug/debugdrawflatpolygons", TEXTURE_GROUP_OTHER, true);
  g_pWireframeMaterial = g_MaterialSystemApp.m_pMaterialSystem->FindMaterial(
      "shadertest/wireframevertexcolor", TEXTURE_GROUP_OTHER);
  staticTest.InitEnvironment();
  staticTest.CreateGround(1024);
}

void FPSControls(float frametime, float mouseDeltaX, float mouseDeltaY,
                 Vector &cameraPosition, QAngle &cameraAngles, float speed) {
...
void SetupCamera(Vector &cameraPosition, QAngle &cameraAngles) {
  ...
  pRenderContext->Translate(-cameraPosition[0], -cameraPosition[1],
                            -cameraPosition[2]);
}
...
void AppRender(float frametime, float mouseDeltaX, float mouseDeltaY) {
  FPSControls(frametime, mouseDeltaX, mouseDeltaY, cameraPosition, cameraAngles,
              300);
