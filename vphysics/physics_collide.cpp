...
public:
  ...
  CPhysConvex *ConvexFromPlanes(float *pPlanes, int planeCount,
                                float mergeDistance);
  CPhysConvex *ConvexFromConvexPolyhedron(const CPolyhedron &ConvexPolyhedron);
  void ConvexesFromConvexPolygon(const Vector &vPolyNormal,
                                 const Vector *pPoints, int iPointCount,
                                 CPhysConvex **pOutput);
  CPhysConvex *RebuildConvexFromPlanes(CPhysConvex *pConvex,
                                       float mergeDistance);
  ...
  CPhysCollide *
  ConvertConvexToCollideParams(CPhysConvex **pConvex, int convexCount,
                               const convertconvexparams_t &convertParams);

  CPolyhedron *PolyhedronFromConvex(CPhysConvex *const pConvex,
                                    bool bUseTempPolyhedron);
  int GetConvexesUsedInCollideable(const CPhysCollide *pCollideable,
  ...
  void PolysoupAddTriangle(CPhysPolysoup *pSoup, const Vector &a,
  ...
  virtual void CollideGetAABB(Vector *pMins, Vector *pMaxs,
  ...
  virtual void VCollideLoad(vcollide_t *pOutput, int solidCount,
                            const char *pBuffer, int size, bool swap);
  ...
  void TraceBox(const Vector &start, const Vector &end, const Vector &mins,
                const Vector &maxs, const CPhysCollide *pCollide,
                const Vector &collideOrigin, const QAngle &collideAngles,
                trace_t *ptr);
  void TraceBox(const Ray_t &ray, const CPhysCollide *pCollide,
                const Vector &collideOrigin, const QAngle &collideAngles,
                trace_t *ptr);
  void TraceBox(const Ray_t &ray, unsigned int contentsMask,
                IConvexInfo *pConvexInfo, const CPhysCollide *pCollide,
                const Vector &collideOrigin, const QAngle &collideAngles,
                trace_t *ptr);
  // Trace one collide against another
  void TraceCollide(const Vector &start, const Vector &end,
  ...
  // If you delete the text and call members of IVPhysicsKeyParser, it will
  // crash
...
private:
  ...
  void AddBBoxCache(CPhysCollideCompactSurface *pCollide, const Vector &mins,
                    const Vector &maxs);
  CPhysCollideCompactSurface *GetBBoxCache(const Vector &mins,
                                           const Vector &maxs);
  CPhysCollideCompactSurface *
  FastBboxCollide(const CPhysCollideCompactSurface *pCollide,
...
// You can disable all of the havok Mopp collision model building by undefining
// this symbol
...
struct compactsurfaceheader_t : public physcollideheader_t {
  ...
  void CompactSurface(const IVP_Compact_Surface *pSurface,
                      const Vector &orthoAreas) {
...
DEFINE_FIELD(vphysicsID, FIELD_INTEGER), DEFINE_FIELD(version, FIELD_SHORT),
...
public:
  ~CPhysCollideCompactSurface();
  CPhysCollideCompactSurface(const char *pBuffer, unsigned int size, int index,
                             bool swap = false);
  CPhysCollideCompactSurface(const compactsurfaceheader_t *pHeader, int index,
                             bool swap = false);
  CPhysCollideCompactSurface(IVP_Compact_Surface *pSurface);

  void Init(const char *pBuffer, unsigned int size, int index,
            bool swap = false);
...
private:
  ...
  void ComputeHullInfo_r(hullinfo_t *pOut,
...
static const IVP_Compact_Surface *
ConvertPhysCollideToCompactSurface(const CPhysCollide *pCollide) {
  return pCollide->GetCompactSurface();
}

IVP_SurfaceManager *CreateSurfaceManager(const CPhysCollide *pCollisionModel,
                                         short &collideType) {
  return pCollisionModel ? pCollisionModel->CreateSurfaceManager(collideType)
                         : NULL;
}
...
CPhysCollide *CPhysCollide::UnserializeFromBuffer(const char *pBuffer,
                                                  unsigned int size, int index,
                                                  bool swap) {
  const physcollideheader_t *pHeader =
      reinterpret_cast<const physcollideheader_t *>(pBuffer);
  if (pHeader->vphysicsID == VPHYSICS_COLLISION_ID) {
    Assert(pHeader->version == VPHYSICS_COLLISION_VERSION);
    switch (pHeader->modelType) {
    case COLLIDE_POLY:
      return new CPhysCollideCompactSurface((compactsurfaceheader_t *)pHeader,
                                            index, swap);
    case COLLIDE_MOPP:
#if ENABLE_IVP_MOPP
  ...
  const IVP_Compact_Surface *pSurface =
      reinterpret_cast<const IVP_Compact_Surface *>(pBuffer);
...
CPhysCollideMopp::~CPhysCollideMopp() { ivp_free_aligned(m_pMopp); }

void CPhysCollideMopp::GetAllLedges(
    IVP_U_BigVector<IVP_Compact_Ledge> &ledges) const {
  IVP_Compact_Ledge_Solver::get_all_ledges(m_pMopp, &ledges);
}

IVP_SurfaceManager *
CPhysCollideMopp::CreateSurfaceManager(short &collideType) const {
...
unsigned int CPhysCollideMopp::SerializeToBuffer(char *pDest,
                                                 bool bSwap) const {
  moppheader_t header;
  header.Mopp(m_pMopp);
...
void CPhysCollideCompactSurface::InitCollideMap() {
  ...
  if (m_pCompactSurface) {
    ...
    GetAllLedges(ledges);
    // don't make these for really large models because there's a linear search
    // involved in using this atm.
    if (!ledges.len() || ledges.len() > 32)
      return;
    int allocSize =
        sizeof(collidemap_t) + ((ledges.len() - 1) * sizeof(leafmap_t));
...
void CPhysCollideCompactSurface::Init(const char *pBuffer, unsigned int size,
                                      int index, bool bSwap) {
...
IVP_SurfaceManager *
CPhysCollideCompactSurface::CreateSurfaceManager(short &collideType) const {
  collideType = COLLIDE_POLY;
  return new IVP_SurfaceManager_Polygon(m_pCompactSurface);
}

void CPhysCollideCompactSurface::GetAllLedges(
    IVP_U_BigVector<IVP_Compact_Ledge> &ledges) const {
...
unsigned int CPhysCollideCompactSurface::SerializeToBuffer(char *pDest,
                                                           bool bSwap) const {
  ...
  for (int axis = 0; axis < 3; axis++) {
    ...
    for (float u0 = mins[u] + halfSide; u0 < maxs[u]; u0 += side) {
      for (float v0 = mins[v] + halfSide; v0 < maxs[v]; v0 += side) {
        ...
        physcollision->TraceBox(start, end, vec3_origin, vec3_origin, this,
                                vec3_origin, vec3_angle, &tr);
        if (tr.DidHit()) {
          hits++;
        }
        total++;
...
void CPhysCollideCompactSurface::ComputeHullInfo_r(
    hullinfo_t *pOut, const IVP_Compact_Ledgetree_Node *node) const {
  if (!node->is_terminal()) {
...
//-----------------------------------------------------------------------------
CPhysConvex *CPhysicsCollision::ConvexFromVertsFast(Vector **pVerts,
                                                    int vertCount) {
...
CPhysConvex *CPhysicsCollision::RebuildConvexFromPlanes(CPhysConvex *pConvex,
  ...
  for (int j = 0; j < triangleCount; j++) {
    ...
    const IVP_U_Float_Point *p0 =
        IVP_Compact_Ledge_Solver::give_object_coords(pEdge, pLedge);
    const IVP_U_Float_Point *p2 =
        IVP_Compact_Ledge_Solver::give_object_coords(pEdge->get_next(), pLedge);
    const IVP_U_Float_Point *p1 =
        IVP_Compact_Ledge_Solver::give_object_coords(pEdge->get_prev(), pLedge);
  ...
  IVP_Compact_Ledge *pLedgeOut =
      IVP_SurfaceBuilder_Halfspacesoup::convert_halfspacesoup_to_compact_ledge(
          &halfspaces, mergeTolerance);
  return reinterpret_cast<CPhysConvex *>(pLedgeOut);
}

CPhysConvex *CPhysicsCollision::ConvexFromVerts(Vector **pVerts,
                                                int vertCount) {
  CPhysConvex *pConvex = ConvexFromVertsFast(pVerts, vertCount);
  CPhysConvex *pReturn = RebuildConvexFromPlanes(
      pConvex, 0.01f); // remove interior coplanar verts!
  if (pReturn) {
    ConvexFree(pConvex);
    return pReturn;
  }
...
CPhysConvex *CPhysicsCollision::ConvexFromPlanes(float *pPlanes, int planeCount,
                                                 float mergeDistance) {
  ...
  IVP_Compact_Ledge *pLedge =
      IVP_SurfaceBuilder_Halfspacesoup::convert_halfspacesoup_to_compact_ledge(
          &halfspaces, mergeDistance);
  return reinterpret_cast<CPhysConvex *>(pLedge);
}

CPhysConvex *CPhysicsCollision::ConvexFromConvexPolyhedron(
    const CPolyhedron &ConvexPolyhedron) {
  IVP_Template_Polygon polyTemplate(ConvexPolyhedron.iVertexCount,
                                    ConvexPolyhedron.iLineCount,
  ...
  for (int i = 0; i != ConvexPolyhedron.iPolygonCount; ++i) {
    polyTemplate.surfaces[i].init_surface(
        ConvexPolyhedron.pPolygons[i]
            .iIndexCount); // num vertices in a convex polygon == num lines
    polyTemplate.surfaces[i].templ_poly = &polyTemplate;

    ConvertPositionToIVP(ConvexPolyhedron.pPolygons[i].polyNormal,
                         polyTemplate.surfaces[i].normal);
    ...
    for (int j = 0; j != ConvexPolyhedron.pPolygons[i].iIndexCount; ++j) {
      ...
      polyTemplate.surfaces[i].revert_line[j] =
          pLineReferences[j].iEndPointIndex;
    }
  }

  // final conversion
  IVP_Compact_Ledge *pLedge =
      IVP_SurfaceBuilder_Polygon_Convex::convert_template_to_ledge(
          &polyTemplate);
...
CPolyhedron *CPhysicsCollision::PolyhedronFromConvex(CPhysConvex *const pConvex,
                                                     bool bUseTempPolyhedron) {
  IVP_Compact_Ledge *pLedge = (IVP_Compact_Ledge *)pConvex;
  int iTriangles = pLedge->get_n_triangles();

  PolyhedronMesh_Triangle *pTriangles = (PolyhedronMesh_Triangle *)stackalloc(
      iTriangles * sizeof(PolyhedronMesh_Triangle));
  ...
  for (int i = 0; i != iTriangles; ++i) {
    ...
    pTriangles[i].Edges[2].iPointIndices[1] =
        pTriangles[i].Edges[0].iPointIndices[0] =
            pTri->get_edge(2)->get_start_point_index();
    pTriangles[i].Edges[0].iPointIndices[1] =
        pTriangles[i].Edges[1].iPointIndices[0] =
            pTri->get_edge(1)->get_start_point_index();
    pTriangles[i].Edges[1].iPointIndices[1] =
        pTriangles[i].Edges[2].iPointIndices[0] =
            pTri->get_edge(0)->get_start_point_index();

    for (int j = 0; j != 3; ++j) {
      // get_n_points() has a whole bunch of ifdefs that apparently disable it
      // in this case, detect number of points
  ...
  // apparently points might be shared between ledges and not all points will be
  // used. So now we get to compress them into a smaller set
  ...
  for (int i = 0; i != iTriangles; ++i) {
    for (int j = 0; j != 3; ++j) {
      for (int k = 0; k != 2; ++k)
        pTriangles[i].Edges[j].iPointIndices[k] =
            pPointRemapping[pTriangles[i].Edges[j].iPointIndices[k]];
    }
  }
  ...
  for (int i = 0; i != iTriangles; ++i) {
    for (int j = 0; j != 3; ++j) {
      ...
      ++iLinkCount; // this will technically make the link count double the
                    // actual number
  ...
  if (bUseTempPolyhedron)
    pReturn =
        GetTempPolyhedron(iNumPoints, iLinkCount, iLinkCount * 2, iTriangles);
  else
    pReturn = CPolyhedron_AllocByNew::Allocate(iNumPoints, iLinkCount,
                                               iLinkCount * 2, iTriangles);
  ...
  int *pStartIndices = (int *)stackalloc(
      iNumPoints *
      sizeof(int)); // for quicker lookup of which edges to use in polygons

  pStartIndices[0] =
      0; // the lowest index point drives links, so if the first point isn't the
         // first link, then something is extremely messed up
  ...
  for (int i = 0; i != iTriangles; ++i) {
    ...
    for (int j = 0; j != 3; ++j, ++iInsertIndex) {
      ...
      int iLow = (pIndices[0] > pIndices[1]) ? 1 : 0;
      int iLineIndex;
      for (iLineIndex = pStartIndices[pIndices[iLow]]; iLineIndex != iLinkCount;
           ++iLineIndex) {
        if ((pReturn->pLines[iLineIndex].iPointIndices[0] == pIndices[iLow]) &&
            (pReturn->pLines[iLineIndex].iPointIndices[1] ==
             pIndices[1 - iLow])) {
          break;
        }
      }
...
int CPhysicsCollision::GetConvexesUsedInCollideable(
    const CPhysCollide *pCollideable, CPhysConvex **pOutputArray,
    int iOutputArrayLimit) {
  IVP_U_BigVector<IVP_Compact_Ledge> ledges;
  ...
  for (int i = 0; i != iLedgeCount; ++i) {
    IVP_Compact_Ledge *pLedge = ledges.element_at(
        i); // doing as a 2 step since a single convert seems more error prone
            // (without compile error) in this case
...
void CPhysicsCollision::ConvexesFromConvexPolygon(const Vector &vPolyNormal,
                                                  const Vector *pPoints,
                                                  int iPointCount,
                                                  CPhysConvex **pOutput) {
  IVP_U_Point *pIVP_Points =
      (IVP_U_Point *)stackalloc(sizeof(IVP_U_Point) * iPointCount);
  IVP_U_Point **pTriangulator =
      (IVP_U_Point **)stackalloc(sizeof(IVP_U_Point *) * iPointCount);
...
//-----------------------------------------------------------------------------
static float TetrahedronVolume(const Vector &p0, const Vector &p1,
...
static float TriangleArea(const Vector &p0, const Vector &p1,
                          const Vector &p2) {
  Vector e0 = p1 - p0;
  Vector e1 = p2 - p0;
  Vector cross;
...
CPhysCollide *CPhysicsCollision::ConvertConvexToCollide(CPhysConvex **pConvex,
                                                        int convexCount) {
...
CPhysCollide *CPhysicsCollision::ConvertConvexToCollideParams(
  ...
  if (validConvex) {
    ...
    params.force_convex_hull =
        (IVP_Compact_Ledge *)convertParams.pForcedOuterHull;
    params.build_root_convex_hull =
        convertParams.buildOuterConvexHull ? IVP_TRUE : IVP_FALSE;
...
static void InitBoxVerts(Vector *boxVerts, Vector **ppVerts, const Vector &mins,
                         const Vector &maxs) {
  for (int i = 0; i < 8; ++i) {
...
CPhysicsCollision::FastBboxCollide(const CPhysCollideCompactSurface *pCollide,
  ...
  const IVP_Compact_Surface *pSurface =
      ConvertPhysCollideToCompactSurface(pCollide);
  Assert(pSurface);
  const IVP_Compact_Ledgetree_Node *node =
      pSurface->get_compact_ledge_tree_root();
  Assert(node->is_terminal() == IVP_TRUE);
  const IVP_Compact_Ledge *pLedge = node->get_compact_ledge();
  int ledgeSize = pLedge->get_size();
  IVP_Compact_Ledge *pNewLedge =
      (IVP_Compact_Ledge *)ivp_malloc_aligned(ledgeSize, 16);
...
void CPhysicsCollision::InitBBoxCache() {
  ...
  IVP_Compact_Poly_Point *pPoints =
      reinterpret_cast<IVP_Compact_Ledge *>(pConvex)->get_point_array();
  for (int i = 0; i < 8; i++) {
    int nearest = -1;
    float minDist = 0.1;
    Vector tmp;
...
CPhysConvex *CPhysicsCollision::BBoxToConvex(const Vector &mins,
                                             const Vector &maxs) {
  Vector boxVerts[8], *ppVerts[8];
  InitBoxVerts(boxVerts, ppVerts, mins, maxs);
  // Generate a convex hull from the verts
...
CPhysCollide *CPhysicsCollision::BBoxToCollide(const Vector &mins,
                                               const Vector &maxs) {
...
void CPhysicsCollision::AddBBoxCache(CPhysCollideCompactSurface *pCollide,
                                     const Vector &mins, const Vector &maxs) {
...
CPhysCollideCompactSurface *
CPhysicsCollision::GetBBoxCache(const Vector &mins, const Vector &maxs) {
  for (int i = m_bboxCache.Count() - 1; i >= 0; i--) {
...
CPhysCollide *CPhysicsCollision::UnserializeCollide(char *pBuffer, int size,
                                                    int index) {
...
void CPhysicsCollision::PolysoupDestroy(CPhysPolysoup *pSoup) { delete pSoup; }
...
int CPhysicsCollision::CreateDebugMesh(const CPhysCollide *pCollisionModel,
                                       Vector **outVerts) {
  int i;
...
void CPhysicsCollision::SetConvexGameData(CPhysConvex *pConvex,
...
void CPhysicsCollision::TraceBox(const Vector &start, const Vector &end,
  ...
  m_traceapi.SweepBoxIVP(start, end, mins, maxs, pCollide, collideOrigin,
                         collideAngles, ptr);
}
...
void CPhysicsCollision::TraceBox(const Ray_t &ray, unsigned int contentsMask,
                                 ...
                                 const Vector &collideOrigin,
                                 const QAngle &collideAngles, trace_t *ptr) {
  m_traceapi.SweepBoxIVP(ray, contentsMask, pConvexInfo, pCollide,
                         collideOrigin, collideAngles, ptr);
}

// Trace one collide against another
void CPhysicsCollision::TraceCollide(
  ...
  m_traceapi.SweepIVP(start, end, pSweepCollide, sweepAngles, pCollide,
                      collideOrigin, collideAngles, ptr);
}

void CPhysicsCollision::CollideGetAABB(Vector *pMins, Vector *pMaxs,
                                       const CPhysCollide *pCollide,
                                       const Vector &collideOrigin,
                                       const QAngle &collideAngles) {
...
Vector CPhysicsCollision::CollideGetExtent(const CPhysCollide *pCollide,
  ...
  return m_traceapi.GetExtent(pCollide, collideOrigin, collideAngles,
                              direction);
}

bool CPhysicsCollision::IsBoxIntersectingCone(const Vector &boxAbsMins,
...
// loads a set of solids into a vcollide_t
void CPhysicsCollision::VCollideLoad(vcollide_t *pOutput, int solidCount,
                                     const char *pBuffer, int bufferSize,
                                     bool swap) {
  ...
  for (int i = 0; i < solidCount; i++) {
    ...
    pOutput->solids[i] =
        CPhysCollide::UnserializeFromBuffer(tmpbuf, size, i, swap);
    position += size;
...
void CPhysicsCollision::VCollideUnload(vcollide_t *pVCollide) {
  for (int i = 0; i < pVCollide->solidCount; i++) {
    ...
    // GetActiveEnvironmentByIndex() will eventually return NULL when there are
    // no more environments. In HL2 & TF2, there are only 2 environments - so j
    // > 1 is probably an error!
    for (int j = 0; j < 1024; j++) {
      IPhysicsEnvironment *pEnv =
          g_PhysicsInternal->GetActiveEnvironmentByIndex(j);
      if (!pEnv)
        break;
...
// If you delete the vcollide_t and call members of IVCollideParse, it will
// crash
IVPhysicsKeyParser *
CPhysicsCollision::VPhysicsKeyParserCreate(const char *pKeyData) {
...
IPhysicsCollision *CPhysicsCollision::ThreadContextCreate(void) { return this; }

void CPhysicsCollision::ThreadContextDestroy(
    IPhysicsCollision *pThreadContext) {}

void CPhysicsCollision::CollideGetMassCenter(CPhysCollide *pCollide,
                                             Vector *pOutMassCenter) {
  *pOutMassCenter = pCollide->GetMassCenter();
}

void CPhysicsCollision::CollideSetMassCenter(CPhysCollide *pCollide,
                                             const Vector &massCenter) {
...
Vector
CPhysicsCollision::CollideGetOrthographicAreas(const CPhysCollide *pCollide) {
  if (!pCollide)
    return vec3_origin;
  return pCollide->GetOrthographicAreas();
}

void CPhysicsCollision::CollideSetOrthographicAreas(CPhysCollide *pCollide,
                                                    const Vector &areas) {
  if (pCollide)
...
// number of convex pieces in the whole solid
int CCollisionQuery::ConvexCount(void) { return m_ledges.len(); }
...
// Gets the triangle's verts to an array
void CCollisionQuery::GetTriangleVerts(int convexIndex, int triangleIndex,
                                       Vector *verts) {
  IVP_Compact_Ledge *pLedge = m_ledges.element_at(convexIndex);
...
// UNDONE: This doesn't work!!!
void CCollisionQuery::SetTriangleVerts(int convexIndex, int triangleIndex,
                                       const Vector *verts) {
  IVP_Compact_Ledge *pLedge = m_ledges.element_at(convexIndex);
  Triangle(pLedge, triangleIndex);
}

int CCollisionQuery::GetTriangleMaterialIndex(int convexIndex,
                                              int triangleIndex) {
  IVP_Compact_Ledge *pLedge = m_ledges.element_at(convexIndex);
  IVP_Compact_Triangle *pTriangle = Triangle(pLedge, triangleIndex);
...
void CCollisionQuery::SetTriangleMaterialIndex(int convexIndex,
                                               int triangleIndex,
                                               int index7bits) {
  IVP_Compact_Ledge *pLedge = m_ledges.element_at(convexIndex);
  IVP_Compact_Triangle *pTriangle = Triangle(pLedge, triangleIndex);
...
IVP_Compact_Triangle *CCollisionQuery::Triangle(IVP_Compact_Ledge *pLedge,
                                                int triangleIndex) {
  if (!pLedge)
    return NULL;
