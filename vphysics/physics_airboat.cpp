...
#define AIRBOAT_DRY_FRICTION_SCALE                                             \
  0.6f // unitless, reduces our friction on all surfaces other than water
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CPhysics_Airboat::CPhysics_Airboat(IVP_Environment *pEnv,
...
void CPhysics_Airboat::InitAirboat(const IVP_Template_Car_System *pCarSystem) {
  ...
  CPhysicsObject *pBodyObject =
      static_cast<CPhysicsObject *>(pCarSystem->car_body->client_data);

  pBodyObject->EnableGravity(false);

  // We do our own buoyancy simulation.
  pBodyObject->SetCallbackFlags(pBodyObject->GetCallbackFlags() &
...
//-----------------------------------------------------------------------------
// Purpose: Returns an amount to add to the front pontoon raycasts to simulate
// wave noise. Input  : nPontoonIndex - Which pontoon we're dealing with (0 or
// 1).
//			flSpeedRatio - Speed as a ratio of max speed [0..1]
//-----------------------------------------------------------------------------
float CPhysics_Airboat::ComputeFrontPontoonWaveNoise(int nPontoonIndex,
                                                     float flSpeedRatio) {
  // Add in sinusoidal noise cause by undulating water. Reduce the amplitude of
  // the noise at higher speeds.
  IVP_FLOAT flNoiseScale =
      RemapValClamped(1.0 - flSpeedRatio, 0, 1, AIRBOAT_WATER_NOISE_MIN,
                      AIRBOAT_WATER_NOISE_MAX);

  // Apply a phase shift between left and right pontoons to simulate waves
  // passing under the boat.
  IVP_FLOAT flPhaseShift = 0;
  if (flSpeedRatio < 0.3) {
    // BUG: this allows a discontinuity in the waveform - use two superimposed
    // sine waves instead?
    flPhaseShift = nPontoonIndex * AIRBOAT_WATER_PHASE_MAX;
  }

  // Increase the wave frequency as speed increases.
  IVP_FLOAT flFrequency = RemapValClamped(
      flSpeedRatio, 0, 1, AIRBOAT_WATER_FREQ_MIN, AIRBOAT_WATER_FREQ_MAX);

  // Msg( "Wave amp=%f, freq=%f, phase=%f\n", flNoiseScale, flFrequency,
  // flPhaseShift );
  return flNoiseScale *
         sin(flFrequency *
             (m_pCore->environment->get_current_time().get_seconds() +
              flPhaseShift));
}
...
//-----------------------------------------------------------------------------
void CPhysics_Airboat::pre_raycasts_gameside(
    int nRaycastCount, IVP_Ray_Solver_Template *pRays, Ray_t *pGameRays,
    IVP_Raycast_Airboat_Impact *pImpacts) {
  IVP_FLOAT flForwardSpeedRatio =
      clamp(m_vecLocalVelocity.k[2] / 10.0f, 0.f, 1.0f);
  ...
  // This is a little weird. We adjust the front pontoon ray lengths based on
  // forward velocity, but ONLY if both pontoons are in the water, which we
  // won't know until we do the raycast. So we do most of the work here, and
  // cache some of the results to use them later.
  Vector vecStart[4];
  ...
  for (iRaycast = 0; iRaycast < nRaycastCount; ++iRaycast) {
    ...
    ConvertDirectionToHL(pRays[iRaycast].ray_normized_direction,
                         vecDirection[iRaycast]);
    ...
    if (m_pGameTrace->VehiclePointInWater(vecEnd)) {
      ...
      if (iRaycast < 2) {
        ...
        IVP_FLOAT flNoise =
            ComputeFrontPontoonWaveNoise(iRaycast, flSpeedRatio);
  ...
  // If both front pontoons are in the water, add in a bit of lift proportional
  // to our forward speed. We can't do this to only one of the front pontoons
  // because it causes some twist if we do.
  ...
  if (nFrontPontoonsInWater == 2) {
    for (int i = 0; i < 2; i++) {
      ...
      pRays[i].ray_length = RemapValClamped(flForwardSpeedRatio, 0, 1,
                                            AIRBOAT_RAYCAST_DIST_WATER_LOW,
...
// Purpose:
//-----------------------------------------------------------------------------
float CPhysics_Airboat::GetWaterDepth(Ray_t *pGameRay,
  ...
  m_pGameTrace->VehicleTraceRayWithWater(waterRay, pPhysAirboat->GetGameData(),
                                         &trace);
...
//-----------------------------------------------------------------------------
void CPhysics_Airboat::do_raycasts_gameside(
    int nRaycastCount, IVP_Ray_Solver_Template *pRays,
    IVP_Raycast_Airboat_Impact *pImpacts) {
  ...
  for (int iRaycast = 0; iRaycast < nRaycastCount; ++iRaycast) {
    ...
    if (pImpacts[iRaycast].bInWater) {
      ...
      IPhysicsObject *pPhysAirboat =
          static_cast<IPhysicsObject *>(m_pAirboatBody->client_data);
      m_pGameTrace->VehicleTraceRay(gameRays[iRaycast],
                                    pPhysAirboat->GetGameData(), &trace);
      pImpacts[iRaycast].flDepth =
          GetWaterDepth(&gameRays[iRaycast], pPhysAirboat);
    } else {
      // Trace down to find the ground or water.
      IPhysicsObject *pPhysAirboat =
          static_cast<IPhysicsObject *>(m_pAirboatBody->client_data);
      m_pGameTrace->VehicleTraceRayWithWater(
          gameRays[iRaycast], pPhysAirboat->GetGameData(), &trace);
    }

    ConvertPositionToIVP(gameRays[iRaycast].m_Start +
                             gameRays[iRaycast].m_StartOffset,
                         m_CarSystemDebugData.wheelRaycasts[iRaycast][0]);
    ConvertPositionToIVP(gameRays[iRaycast].m_Start +
                             gameRays[iRaycast].m_StartOffset +
                             gameRays[iRaycast].m_Delta,
                         m_CarSystemDebugData.wheelRaycasts[iRaycast][1]);
    m_CarSystemDebugData.wheelRaycastImpacts[iRaycast] =
        trace.fraction * gameRays[iRaycast].m_Delta.Length();
    ...
    if (trace.fraction != 1.0f) {
      ...
      ConvertDirectionToIVP(trace.plane.normal,
                            pImpacts[iRaycast].vecImpactNormalWS);
      ...
      if (pImpacts[iRaycast].vecImpactNormalWS.k[1] < -0.707) {
        // dampening is 1/t, where t is how long it takes to come to a complete
        // stop
...
//-----------------------------------------------------------------------------
void CPhysics_Airboat::do_simulation_controller(IVP_Event_Sim *pEventSim,
  ...
  //m_pCore->controllers_of_core.element_at(i);
  //}
  ...
  for (int iPoint = 0; iPoint < nPontoonPoints; ++iPoint) {
    ...
    if (pPontoonPoint) {
      ...
      matWorldFromCore->vmult3(&pPontoonPoint->raycast_dir_cs,
                               &pImpacts[iPoint].raycast_dir_ws);
      ...
      // Set the ray solver template flags.  This defines which objects you wish
...
//			weak or strong jump. Weak jumps are jumps at below a
//threshold speed, 			and disable the turbine and pitch controller.
// Input  : pImpacts -
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
void CPhysics_Airboat::UpdateAirborneState(IVP_Raycast_Airboat_Impact *pImpacts,
...
// Purpose:
//-----------------------------------------------------------------------------
bool CPhysics_Airboat::PostRaycasts(
  ...
  for (int iPoint = 0; iPoint < nPontoonPoints; ++iPoint) {
    ...
    pImpact->vecImpactPointWS.add_multiple(&pRaySolver->ray_start_point,
                                           &pImpact->raycast_dir_ws,
                                           pPontoonPoint->raycast_dist);

    // Get the speed (velocity) at the impact point.
    m_pCore->get_surface_speed_ws(&pImpact->vecImpactPointWS,
                                  &pImpact->surface_speed_wheel_ws);
    pImpact->projected_surface_speed_wheel_ws.set_orthogonal_part(
        &pImpact->surface_speed_wheel_ws, &pImpact->vecImpactNormalWS);

    matWorldFromCore->vmult3(&pPontoonPoint->axis_direction_cs,
                             &pImpact->axis_direction_ws);
    pImpact->projected_axis_direction_ws.set_orthogonal_part(
        &pImpact->axis_direction_ws, &pImpact->vecImpactNormalWS);
    if (pImpact->projected_axis_direction_ws.normize() == IVP_FAULT) {
      DevMsg("CPhysics_Airboat::do_simulation_controller "
...
// Purpose:
//-----------------------------------------------------------------------------
void CPhysics_Airboat::DoSimulationPontoons(
...
//-----------------------------------------------------------------------------
void CPhysics_Airboat::DoSimulationPontoonsGround(
  ...
  vecSpeedDelta.subtract(&pImpact->projected_surface_speed_wheel_ws,
                         &pImpact->surface_speed_wheel_ws);
  ...
  // NOTE: Spring constants are all mass-independent, so no need to multiply by
  // mass here.
...
//-----------------------------------------------------------------------------
void CPhysics_Airboat::DoSimulationPontoonsWater(
    IVP_Raycast_Airboat_Wheel *pPontoonPoint,
    IVP_Raycast_Airboat_Impact *pImpact, IVP_Event_Sim *pEventSim) {
#define AIRBOAT_BUOYANCY_SCALAR 1.6f
#define PONTOON_AREA_2D                                                        \
  2.8f // 2 pontoons x 16 in x 136 in = 4352 sq inches = 2.8 sq meters
#define PONTOON_HEIGHT 0.41f // 16 inches high = 0.41 meters
  ...
  // Buoyancy forces are equal to the mass of the water displaced, which is 1000
  // kg/m^3 There are 4 pontoon points, so each one can exert 1/4th of the total
  // buoyancy force.
  IVP_FLOAT flForce = AIRBOAT_BUOYANCY_SCALAR * 0.25f * m_pCore->get_mass() *
                      flSubmergedVolume * 1000.0f;
  IVP_DOUBLE flImpulse = flForce * pEventSim->delta_time;
  ...
  //	CPhysicsEnvironment *pEnv = (CPhysicsEnvironment
  //*)m_pAirboatBody->get_core()->environment->client_data;
  //	IVPhysicsDebugOverlay *debugoverlay = pEnv->GetDebugOverlay();
  //	debugoverlay->AddLineOverlay(vecPoint, vecPoint + vecDir * 128, 255, 0,
  //255, false, 10.0 );
}
...
// Purpose:
//-----------------------------------------------------------------------------
void CPhysics_Airboat::PerformFrictionNotification(
    float flEliminatedEnergy, float dt, int nSurfaceProp,
    IPhysicsCollisionData *pCollisionData) {
  CPhysicsObject *pPhysAirboat =
      static_cast<CPhysicsObject *>(m_pAirboatBody->client_data);
  if ((pPhysAirboat->CallbackFlags() & CALLBACK_GLOBAL_FRICTION) == 0)
    return;

  IPhysicsCollisionEvent *pEventHandler =
      pPhysAirboat->GetVPhysicsEnvironment()->GetCollisionEventHandler();
  if (!pEventHandler)
    return;
  ...
  if (flEliminatedEnergy > 0.05f) {
    pEventHandler->Friction(pPhysAirboat, flEliminatedEnergy,
                            pPhysAirboat->GetMaterialIndexInternal(),
...
void CPhysics_Airboat::DoSimulationDrag(IVP_Raycast_Airboat_Impact *pImpacts,
  ...
  // nPointsOnGround, flDebugSpeed, vecAirboatDirLS.k[0], vecAirboatDirLS.k[1],
  // vecAirboatDirLS.k[2]);
  ...
  if (nPointsInWater) {
    ...
    // Water drag is directional -- the pontoons resist left/right motion much
    // more than forward/back.
  ...
  if (nPointsOnGround && (flSpeed > 0)) {
    ...
    IVP_FLOAT flFrictionDrag = m_pCore->get_mass() * AIRBOAT_GRAVITY *
                               AIRBOAT_DRY_FRICTION_SCALE * flGroundFriction;
    ...
    // Ground drag is directional -- the pontoons resist left/right motion much
    // more than forward/back.
    IVP_U_Float_Point vecDragLS;
    ...
    PerformFrictionNotification(flEliminatedEnergy, pEventSim->delta_time,
                                nSurfaceProp, &frictionData);
  }
}
...
void CPhysics_Airboat::DoSimulationTurbine(IVP_Event_Sim *pEventSim) {
  // Reduce the turbine power during weak jumps to avoid unrealistic air
  // control. Also, reduce reverse thrust while airborne.
  ...
  if ((vecForwardWS.k[1] < -0.5) && (flThrust > 0)) {
    // Driving up a slope. Reduce upward thrust to prevent ludicrous climbing of
    // steep surfaces.
    float flFactor = 1 + vecForwardWS.k[1];
    // Msg("FWD: y=%f, factor=%f\n", vecForwardWS.k[1], flFactor);
    flThrust *= flFactor;
  } else if ((vecForwardWS.k[1] > 0.5) && (flThrust < 0)) {
    // Reversing up a slope. Reduce upward thrust to prevent ludicrous climbing
    // of steep surfaces.
    float flFactor = 1 - vecForwardWS.k[1];
  ...
  vecImpulse.set_multiple(&vecForwardWS, flThrust * m_pCore->get_mass() *
...
void CPhysics_Airboat::DoSimulationSteering(IVP_Event_Sim *pEventSim) {
  ...
  // Don't mess with the steering direction while we're steering, unless thrust
  // is applied. This prevents the steering from reversing because we started
  // drifting backwards.
  if ((m_SteeringAngle == 0) || (m_flThrust != 0)) {
    if (!m_bAnalogSteering) {
      ...
      // Else if we are applying forward thrust or moving forward, use forward
      // steering.
      else if ((m_flThrust > 0) || (m_vecLocalVelocity.k[2] > 0)) {
        m_bSteeringReversed = false;
      }
    } else {
      // Create a dead zone through the middle of the joystick where we don't
      // reverse thrust. If we're applying reverse thrust, steering is always
      // reversed.
      if (m_flThrust < -2.0f) {
        m_bSteeringReversed = true;
      }
  ...
  if (fabsf(m_SteeringAngle) > 0.01) {
    ...
    // If we changed steering sign or went from not steering to steering, reset
    // the steer time to blend the new steering force in over time.
    IVP_FLOAT flPrevSteeringSign = m_flPrevSteeringAngle < 0.0f ? -1.0f : 1.0f;
    if ((fabs(m_flPrevSteeringAngle) < 0.01) ||
        (flSteeringSign != flPrevSteeringSign)) {
      m_flSteerTime = 0;
    }
    ...
    if (!m_bAnalogSteering) {
      ...
      flSteerScale =
          RemapValClamped(m_flSteerTime, 0, AIRBOAT_STEERING_INTERVAL,
                          AIRBOAT_STEERING_RATE_MIN, AIRBOAT_STEERING_RATE_MAX);
    } else // consoles
    {
      // Analog steering
      flSteerScale =
          RemapValClamped(fabs(m_SteeringAngle), 0, AIRBOAT_STEERING_INTERVAL,
                          AIRBOAT_STEERING_RATE_MIN, AIRBOAT_STEERING_RATE_MAX);
    }

    flForceSteering =
        flSteerScale * m_pCore->get_mass() * pEventSim->i_delta_time;
  ...
  IVP_FLOAT flRotationalDrag = AIRBOAT_ROT_DRAG * m_pCore->rot_speed.k[1] *
                               m_pCore->rot_speed.k[1] * m_pCore->get_mass() *
                               pEventSim->i_delta_time;
  flRotationalDrag *= flRotSpeedSign;

  // Apply dampening proportional to angular velocity.
  IVP_FLOAT flRotationalDamping = AIRBOAT_ROT_DAMPING *
                                  fabs(m_pCore->rot_speed.k[1]) *
  ...
  IVP_FLOAT flForceRotational =
      flForceSteering + flRotationalDrag + flRotationalDamping;
...
//-----------------------------------------------------------------------------
// Purpose: Returns the number of pontoon raycast points that were found to
// contact
//			the ground or water.
//-----------------------------------------------------------------------------
int CPhysics_Airboat::CountSurfaceContactPoints(
    IVP_Raycast_Airboat_Impact *pImpacts) {
  int nContacts = 0;
  int nPontoonPoints = n_wheels;
  for (int iPoint = 0; iPoint < nPontoonPoints; iPoint++) {
...
void CPhysics_Airboat::DoSimulationKeepUprightPitch(
    IVP_Raycast_Airboat_Impact *pImpacts, IVP_Event_Sim *pEventSim) {
  // Disable pitch control during weak jumps. This reduces the unreal 'floaty'
  // sensation.
  if (m_bWeakJump) {
    return;
  }
  ...
  // vecRotAxisCS.k[0], vecRotAxisCS.k[1], vecRotAxisCS.k[2]);
  ...
  vecAngularImpulse.set_multiple(
      &vecRotAxisCS,
      m_pCore->get_mass() * (0.1f * angle + 0.04f * pEventSim->i_delta_time *
                                                (angle - m_flPitchErrorPrev)));
  ...
  CPhysicsEnvironment *pEnv = (CPhysicsEnvironment *)m_pAirboatBody->get_core()
                                  ->environment->client_data;
  IVPhysicsDebugOverlay *debugoverlay = pEnv->GetDebugOverlay();
  ...
  if (flLastLen &&
      (fabs(flDebugLen - flLastLen) > DEG2RAD(1) * m_pCore->get_mass())) {
    debugoverlay->AddLineOverlay(vecPosHL,
                                 vecPosHL + Vector(0, 0, 10) * flDebugLen, 255,
                                 0, 255, false, 100.0);
  } else {
    debugoverlay->AddLineOverlay(vecPosHL,
                                 vecPosHL + Vector(0, 0, 10) * flDebugLen, 255,
  ...
  debugoverlay->AddLineOverlay(vecPosHL + Vector(0, 0, 10) * flDebugLen,
                               vecPosHL + Vector(0, 0, 10) * flDebugLen +
                                   vecGoalAxisHL * 10,
                               0, 255, 0, false, 100.0);
  debugoverlay->AddLineOverlay(vecPosHL + Vector(0, 0, 10) * flDebugLen,
                               vecPosHL + Vector(0, 0, 10) * flDebugLen +
...
//-----------------------------------------------------------------------------
void CPhysics_Airboat::DoSimulationKeepUprightRoll(
    IVP_Raycast_Airboat_Impact *pImpacts, IVP_Event_Sim *pEventSim) {
  // Reference vector in core space.
  // Pitch back by 10 degrees while airborne.
  IVP_U_Float_Point vecUpCS;
  ...
  vecAngularImpulse.set_multiple(
      &vecRotAxisCS,
      m_pCore->get_mass() * (0.2f * angle + 0.3f * pEventSim->i_delta_time *
                                                (angle - m_flRollErrorPrev)));
...
//			s_angle -
//-----------------------------------------------------------------------------
void CPhysics_Airboat::do_steering_wheel(IVP_POS_WHEEL wheel_nr,
                                         IVP_FLOAT s_angle) {
...
//			spring_constant -
//-----------------------------------------------------------------------------
void CPhysics_Airboat::change_spring_constant(IVP_POS_WHEEL pos,
...
//			spring_dampening -
//-----------------------------------------------------------------------------
void CPhysics_Airboat::change_spring_dampening(IVP_POS_WHEEL pos,
...
//			spring_dampening -
//-----------------------------------------------------------------------------
void CPhysics_Airboat::change_spring_dampening_compression(
...
//			pre_tension_length -
//-----------------------------------------------------------------------------
void CPhysics_Airboat::change_spring_pre_tension(IVP_POS_WHEEL pos,
...
//			spring_length -
//-----------------------------------------------------------------------------
void CPhysics_Airboat::change_spring_length(IVP_POS_WHEEL pos,
...
//			torque -
//-----------------------------------------------------------------------------
void CPhysics_Airboat::change_wheel_torque(IVP_POS_WHEEL pos,
                                           IVP_FLOAT torque) {
  IVP_Raycast_Airboat_Wheel *wheel = get_wheel(pos);
  wheel->torque = torque;
...
//			friction -
//-----------------------------------------------------------------------------
void CPhysics_Airboat::change_friction_of_wheel(IVP_POS_WHEEL pos,
                                                IVP_FLOAT friction) {
  IVP_Raycast_Airboat_Wheel *wheel = get_wheel(pos);
...
//			stabi_constant -
//-----------------------------------------------------------------------------
void CPhysics_Airboat::change_stabilizer_constant(IVP_POS_AXIS pos,
...
//-----------------------------------------------------------------------------
void CPhysics_Airboat::get_skid_info(
...
// Purpose:
//-----------------------------------------------------------------------------
void CPhysics_Airboat::InitRaycastCarEnvironment(
...
// Purpose:
//-----------------------------------------------------------------------------
void CPhysics_Airboat::InitRaycastCarBody(
    const IVP_Template_Car_System *pCarSystemTemplate) {
  // Car body attributes.
...
// Purpose:
//-----------------------------------------------------------------------------
void CPhysics_Airboat::InitRaycastCarWheels(
    const IVP_Template_Car_System *pCarSystemTemplate) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CPhysics_Airboat::InitRaycastCarAxes(
    const IVP_Template_Car_System *pCarSystemTemplate) {
...
//-----------------------------------------------------------------------------
void CPhysics_Airboat::SetCarSystemDebugData(
...
// Purpose: Debug data for use in vphysics and the engine to visualize car data.
//-----------------------------------------------------------------------------
void CPhysics_Airboat::GetCarSystemDebugData(
...
//-----------------------------------------------------------------------------
IVP_U_Vector<IVP_Core> *
CPhysics_Airboat::get_associated_controlled_cores(void) {
