...
extern IPhysicsShadowController *CreateShadowController(CPhysicsObject *pObject,
                                                        bool allowTranslation,
                                                        bool allowRotation);
extern IPhysicsPlayerController *
CreatePlayerController(CPhysicsObject *pObject);
extern void DestroyPlayerController(IPhysicsPlayerController *pController);

extern void ComputeController(IVP_U_Float_Point &currentSpeed,
                              const IVP_U_Float_Point &delta,
                              const IVP_U_Float_Point &maxSpeed,
                              float scaleDelta, float damping);
...
struct shadowcontrol_params_t {
  ...
  IVP_U_Point lastPosition; // estimate for where the object should be, used to
                            // compute error for teleport
