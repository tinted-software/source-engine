...
CPhysics_Car_System_Raycast_Wheels::CPhysics_Car_System_Raycast_Wheels(
    IVP_Environment *pEnv, const IVP_Template_Car_System *pCarSystem)
...
//-----------------------------------------------------------------------------
void CPhysics_Car_System_Raycast_Wheels::InitCarSystemWheels(
...
// Purpose: Setup the car system wheels.
//-----------------------------------------------------------------------------
void CPhysics_Car_System_Raycast_Wheels::do_raycasts(
    IVP_Event_Sim *es, int n_wheels, class IVP_Ray_Solver_Template *t_in,
  ...
  IVP_Ray_Solver_Min *solvers[4] = {&ray_solver0, &ray_solver1, &ray_solver2,
                                    &ray_solver3};
  ...
  for (int i = 0; i < n_wheels; i++) {
    ...
    if (hit) {
      hits_out[i] = *hit;
      friction_of_object_out[i] =
          hit->hit_real_object->l_default_material->get_friction_factor();
...
void CPhysics_Car_System_Raycast_Wheels::update_wheel_positions(void) {
  ...
  for (int iWheel = 0; iWheel < n_wheels; ++iWheel) {
    ...
    hp_cs.add_multiple(
        &pRaycastWheel->hp_cs, &pRaycastWheel->spring_direction_cs,
    ...
    m_core_f_wheel.init_normized3_col(&wheel_cs, IVP_COORDINATE_INDEX(index_x),
                                      &wheel2_cs);

    IVP_U_Matrix3 m_world_f_wheel;
    pCacheObject->m_world_f_object.mmult3(
        &m_core_f_wheel,
