...
public:
  ...
  void Init(const CPhysCollide *pCollisionModel, IVP_Real_Object *pObject,
            int materialIndex, float volume, float drag, float angDrag);
  ...
  void SetPosition(const Vector &worldPosition, const QAngle &angles,
  ...
  void CalculateForceOffset(const Vector &forceVector,
                            const Vector &worldPosition, Vector *centerForce,
                            AngularImpulse *centerTorque) const;
  void CalculateVelocityOffset(const Vector &forceVector,
  ...
  bool GetContactPoint(Vector *contactPoint,
                       IPhysicsObject **contactObject) const;
  void SetShadow(float maxSpeed, float maxAngularSpeed,
                 bool allowPhysicsMovement, bool allowPhysicsRotation);
  void UpdateShadow(const Vector &targetPosition, const QAngle &targetAngles,
  ...
  float ComputeShadowControl(const hlshadowcontrol_params_t &params,
  ...
  // returns true if the mass center is set to the default for the collision
  // model
...
private:
  ...
  friend bool RestorePhysicsObject(const physrestoreparams_t &params,
  ...
  // NOTE: If m_pGameData is not the first member, the constructor debug code
  // must be modified
  void *m_pGameData;
...
private:
  ...
  CreatePhysicsObject(CPhysicsEnvironment *pEnvironment,
  ...
  friend bool CPhysicsEnvironment::TransferObject(
      ...
      IPhysicsEnvironment
          *pDestinationEnvironment); // need direct access to m_pShadow for
                                     // Portal mod's physics object transfer
                                     // system
};

// If you haven't ever touched a dynamic object, there's no need to search for
// contacting objects to wakeup when you are deleted.  So cache a bit here when
...
extern CPhysicsObject *
CreatePhysicsObject(CPhysicsEnvironment *pEnvironment,
                    const CPhysCollide *pCollisionModel, int materialIndex,
                    const Vector &position, const QAngle &angles,
                    objectparams_t *pParams, bool isStatic);
extern CPhysicsObject *CreatePhysicsSphere(CPhysicsEnvironment *pEnvironment,
...
extern IPhysicsObject *CreateObjectFromBuffer(CPhysicsEnvironment *pEnvironment,
                                              void *pGameData,
                                              unsigned char *pBuffer,
                                              unsigned int bufferSize,
                                              bool enableCollisions);
extern IPhysicsObject *CreateObjectFromBuffer_UseExistingMemory(
