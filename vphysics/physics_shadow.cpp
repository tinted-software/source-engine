...
void ComputePDControllerCoefficients(float *coefficientsOut,
  ...
  // speed += (coefficientsOut[0] * (targetPos - currentPos) +
  // coefficientsOut[1] * (targetSpeed - currentSpeed)) * dt
}

void ComputeController(IVP_U_Float_Point &currentSpeed,
                       const IVP_U_Float_Point &delta, float maxSpeed,
                       float maxDampSpeed, float scaleDelta, float damping,
                       IVP_U_Float_Point *pOutImpulse = NULL) {
...
void ComputeController(IVP_U_Float_Point &currentSpeed,
                       ...
                       const IVP_U_Float_Point &maxSpeed, float scaleDelta,
                       float damping, IVP_U_Float_Point *pOutImpulse) {
  // scale by timestep
  IVP_U_Float_Point acceleration;
  acceleration.set_multiple(&delta, scaleDelta);
  ...
  for (int i = 2; i >= 0; i--) {
    ...
    // clip force
    acceleration.k[i] =
        (acceleration.k[i] < 0) ? -maxSpeed.k[i] : maxSpeed.k[i];
  }
...
class CPlayerController : public IVP_Controller_Independent,
...
public:
  ...
  void ForceTeleportToCurrentPosition() { m_forceTeleport = true; }
...
private:
  ...
  CPhysicsObject
      *m_pGround; // Uses object listener to clear - so ok to hold over frames
...
CPlayerController::~CPlayerController(void) { DetachObject(); }
...
void CPlayerController::AttachObject(void) {
  ...
  pivp->get_environment()->get_controller_manager()->add_controller_to_core(
      this, pCore);
  END_IVP_ALLOCATION();
...
void CPlayerController::DetachObject(void) {
  ...
  m_pObject = NULL;
  pivp->get_environment()
      ->get_controller_manager()
      ->remove_controller_from_core(this, pCore);
  SetGround(NULL);
}
...
int CPlayerController::TryTeleportObject(void) {
  ...
  if (pivp->is_collision_detection_enabled()) {
    ...
    pivp->beam_object_to_new_position(&targetOrientation, &m_targetPosition,
                                      IVP_TRUE);
...
public:
  ...
  // and still have a projection, but don't exceed limitVel.  Currently that
  // will stop. when this is done, remove the ground exception below.
...
void CPlayerController::do_simulation_controller(IVP_Event_Sim *es,
  ...
  if (m_pGround) {
    const IVP_U_Matrix *pMatrix =
        m_pGround->GetObject()->get_core()->get_m_world_f_core_PSI();
    pMatrix->vmult4(&m_groundPosition, &m_targetPosition);
    m_pGround->GetObject()->get_core()->get_surface_speed(&m_groundPosition,
                                                          &baseVelocity);
  ...
  if (!m_updatedSinceLast) {
    // we haven't received an update from the game code since the last
    // controller step This means we haven't gotten feedback integrated into the
    // motion plan, so the error may be exaggerated.  Assume that the first
    // updated tick had valid information, and limit all subsequent ticks to the
    // same size impulses. NOTE: Don't update the saved impulse - so any
    // subsequent ticks will still have the last known good information.
    float len = m_lastImpulse.real_length();
    // cap the max speed to the length of the last known good impulse
    IVP_U_Float_Point tmp;
    tmp.set(len, len, len);
    ComputeController(pCore->speed, delta_position, tmp,
                      fraction / es->delta_time, m_dampFactor, NULL);
  } else {
    ComputeController(pCore->speed, delta_position, m_maxSpeed,
                      fraction / es->delta_time, m_dampFactor, &m_lastImpulse);
  }
...
void CPlayerController::SetEventHandler(
    IPhysicsPlayerControllerEvent *handler) {
  m_handler = handler;
}

void CPlayerController::Update(const Vector &position, const Vector &velocity,
...
bool CPlayerController::IsInContact(void) {
  ...
  while (pfriction) {
    extern IVP_Real_Object *GetOppositeSynapseObject(IVP_Synapse_Friction *
                                                     pfriction);
    ...
    if (pobj->flags.collision_detection_enabled) {
      ...
      if (!pobj->get_core()->physical_unmoveable && !pobj->get_core()->pinned) {
        CPhysicsObject *pPhys =
            static_cast<CPhysicsObject *>(pobj->client_data);
...
void GetObjectPosition_IVP(IVP_U_Point &origin, IVP_Real_Object *pivp) {
  const IVP_U_Matrix *m_world_f_core =
      pivp->get_core()->get_m_world_f_core_PSI();
...
float ComputeShadowControllerIVP(IVP_Real_Object *pivp,
  ...
  if (params.teleportDistance > 0) {
    ...
    if (qdist > params.teleportDistance * params.teleportDistance) {
      if (pivp->is_collision_detection_enabled()) {
        ...
        pivp->beam_object_to_new_position(&params.targetRotation,
                                          &params.targetPosition, IVP_TRUE);
  ...
  float invDt = 1.0f / dt;
  IVP_Core *pCore = pivp->get_core();
  ComputeController(pCore->speed, delta_position, params.maxSpeed,
                    params.maxDampSpeed, fraction * invDt, params.dampFactor,
                    &params.lastImpulse);
  ...
  ComputeController(pCore->rot_speed, deltaAngles, params.maxAngular,
                    params.maxDampAngular, fraction * invDt, params.dampFactor,
                    NULL);

  return secondsToArrival;
}

void ConvertShadowControllerToIVP(const hlshadowcontrol_params_t &in,
                                  shadowcontrol_params_t &out) {
...
void ConvertShadowControllerToHL(const shadowcontrol_params_t &in,
                                 hlshadowcontrol_params_t &out) {
...
float ComputeShadowControllerHL(CPhysicsObject *pObject,
                                const hlshadowcontrol_params_t &params,
                                float secondsToArrival, float dt) {
  shadowcontrol_params_t ivpParams;
  ConvertShadowControllerToIVP(params, ivpParams);
  return ComputeShadowControllerIVP(pObject->GetObject(), ivpParams,
                                    secondsToArrival, dt);
}
...
public:
  CShadowController();
  CShadowController(CPhysicsObject *pObject, bool allowTranslation,
                    bool allowRotation);
  ...
  void SetObject(IPhysicsObject *pObject);
  void Update(const Vector &position, const QAngle &angles,
  ...
  void
  InitFromTemplate(const vphysics_save_cshadowcontroller_t &controllerTemplate);
  ...
  virtual void UseShadowMaterial(bool bUseShadowMaterial) {
    ...
    int target =
        bUseShadowMaterial ? MATERIAL_INDEX_SHADOW : m_savedMaterialIndex;
    if (target != current) {
...
CShadowController::CShadowController(CPhysicsObject *pObject,
...
CShadowController::~CShadowController(void) { DetachObject(); }
...
void CShadowController::do_simulation_controller(IVP_Event_Sim *es,
                                                 IVP_U_Vector<IVP_Core> *) {
  if (IsEnabled()) {
    IVP_Real_Object *pivp = m_pObject->GetObject();
    Assert(!pivp->get_core()->pinned && !pivp->get_core()->physical_unmoveable);

    ComputeShadowControllerIVP(pivp, m_shadow, m_secondsToArrival,
                               es->delta_time);
    if (m_allowsTranslation) {
      // UNDONE: Assumes gravity points down
      const IVP_U_Point *pgrav = pivp->get_environment()->get_gravity();
...
// NOTE: This isn't a test for equivalent orientations, it's a test for calling
// update with EXACTLY the same data repeatedly
...
void CShadowController::Update(const Vector &position, const QAngle &angles,
                               float secondsToArrival) {
  ...
  if (IsEqual(targetPosition, m_shadow.targetPosition) &&
      IsEqual(targetRotation, m_shadow.targetRotation))
    return;

  m_pObject->Wake();
}

float CShadowController::GetTargetPosition(Vector *pPositionOut,
                                           QAngle *pAnglesOut) {
  if (pPositionOut)
...
void CShadowController::MaxSpeed(float maxSpeed, float maxAngularSpeed) {
  // UNDONE: Turn this on when shadow controllers are having velocity updated
  // per frame right now this has the effect of making dampspeed zero by
  // default.
#if 0
	IVP_Core *pCore = m_pObject->GetObject()->get_core();
	{
...
void CShadowController::GetMaxSpeed(float *pMaxSpeedOut,
...
DEFINE_FIELD(secondsToArrival, FIELD_FLOAT),
    ...
    END_DATADESC()
        ...
        void CShadowController::WriteToTemplate(
...
void CShadowController::InitFromTemplate(
  ...
  m_pObject->GetObject()
      ->get_environment()
      ->get_controller_manager()
      ->add_controller_to_core(this, m_pObject->GetObject()->get_core());
}

IPhysicsShadowController *CreateShadowController(CPhysicsObject *pObject,
                                                 bool allowTranslation,
                                                 bool allowRotation) {
  return new CShadowController(pObject, allowTranslation, allowRotation);
}

bool SavePhysicsShadowController(const physsaveparams_t &params,
...
bool RestorePhysicsShadowControllerInternal(
    const physrestoreparams_t &params,
...
bool SavePhysicsPlayerController(const physsaveparams_t &params,
                                 CPlayerController *pPlayerController) {
  return false;
}

bool RestorePhysicsPlayerController(const physrestoreparams_t &params,
                                    CPlayerController **ppPlayerController) {
  return false;
}

// HACKHACK: The physics object transfer system needs to temporarily detach a
// shadow controller from an object, then reattach without other repercussions
void ControlPhysicsShadowControllerAttachment_Silent(
    IPhysicsShadowController *pController, IVP_Real_Object *pivp,
    bool bAttach) {
  if (bAttach) {
    pivp->get_environment()->get_controller_manager()->add_controller_to_core(
        (CShadowController *)pController, pivp->get_core());
  } else {
    pivp->get_environment()
        ->get_controller_manager()
        ->remove_controller_from_core((CShadowController *)pController,
                                      pivp->get_core());
  }
}

void ControlPhysicsPlayerControllerAttachment_Silent(
    IPhysicsPlayerController *pController, IVP_Real_Object *pivp,
    bool bAttach) {
  if (bAttach) {
    pivp->get_environment()->get_controller_manager()->add_controller_to_core(
    ...
    pivp->get_environment()
        ->get_controller_manager()
        ->remove_controller_from_core((CPlayerController *)pController,
                                      pivp->get_core());
  }
}
