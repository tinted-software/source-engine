...
// this skips the optimization that shrinks the ray as each intersection is
// encountered
...
struct simplex_t {
  ...
  inline bool EdgeSimplex(const simplexvert_t &newPoint, int outIndex,
                          const Vector &edge, Vector *pOut);
  inline bool TriangleSimplex(const simplexvert_t &newPoint, int outIndex,
                              const Vector &faceNormal, Vector *pOut);
...
// really this is indexing a vertex, but the iteration code needs a triangle +
// edge index. edge is always 0-2 so return it in the bottom 2 bits
static unsigned short GetPackedIndex(const IVP_Compact_Ledge *pLedge,
                                     const IVP_U_Float_Point &dir) {
  ...
  for (i = 0; i < triCount; i++) {
    ...
    // if you exhausted the possibilities for this vert, it must be the best
    // vert
    if (vert != best)
      break;
  }
  ...
  int edgeIndex = 0;
  // just do a search for the edge containing this vert instead of storing it
  // along the way
  for (i = 0; i < 3; i++) {
...
void InitLeafmap(IVP_Compact_Ledge *pLedge, leafmap_t *pLeafmapOut) {
  ...
  if (pLedge && pLedge->is_terminal()) {
    // for small numbers of verts it's much faster to simply do dot products
    // with all verts since the best case for hillclimbing is to touch the start
    // vert plus all neighbors (avg_valence+1 dots) in t
    int triCount = pLedge->get_n_triangles();
    // this is a guess that anything with more than brute_force * 4 tris will
    // have at least brute_force verts
    if (triCount <= BRUTE_FORCE_VERT_COUNT * 4) {
      ...
      // number could be high, but this approach to implementing support is
      // invalid because the vert range is polluted
      if (vertCount < BRUTE_FORCE_VERT_COUNT) {
        ...
        Assert(has);
        byte count = 1;
        for (int i = 1; i < vertCount && spanIndex < BRUTE_FORCE_VERT_COUNT;
             i++) {
  ...
  if (!pLeafmapOut->HasSpans()) {
    // otherwise make a 8-way directional map to pick the best start vert for
    // hillclimbing
...
void GetStartVert(const leafmap_t *pLeafmap,
                  const IVP_U_Float_Point &localDirection, int &triIndex,
                  int &edgeIndex) {
  if (!pLeafmap || !pLeafmap->HasCubemap())
    return;

  // map dir to index
  int cacheIndex = (localDirection.k[0] < 0 ? 1 : 0) +
                   (localDirection.k[1] < 0 ? 2 : 0) +
                   (localDirection.k[2] < 0 ? 4 : 0);
...
public:
  CTraceIVP(const CPhysCollide *pCollide, const Vector &origin,
            const QAngle &angles);
  ~CTraceIVP() {
    if (m_pVisitHash)
  ...
  // UNDONE: Do general ITraceObject center/offset computation and move the ray
  // to account for this delta like we do in TraceSweepIVP() Then we can shrink
  // the radius of objects with mass centers NOT at the origin
  ...
  // UNDONE: Optimize this by storing 3 matrices? (one for each transform that
  // includes rot/scale for HL/IVP)? UNDONE: Not necessary if we remove the
  // coordinate conversion
  inline void TransformDirectionToLocal(const Vector &dir,
                                        IVP_U_Float_Point &local) const {
    IVP_U_Float_Point tmp;
    ConvertDirectionToIVP(dir, tmp);
    m_matrix.vimult3(&tmp, &local);
  }

  inline void RotateRelativePositionToLocal(const Vector &delta,
                                            IVP_U_Float_Point &local) const {
    IVP_U_Float_Point tmp;
  ...
  inline void TransformPositionToLocal(const Vector &pos,
                                       IVP_U_Float_Point &local) const {
    IVP_U_Float_Point tmp;
  ...
  inline void TransformPositionFromLocal(const IVP_U_Float_Point &local,
                                         Vector &out) const {
    VectorTransform(*(Vector *)&local,
                    *((const matrix3x4_t *)&m_ivpLocalToHLWorld), out);
  }
  ...
  bool SetSingleConvex(void) {
    const IVP_Compact_Ledgetree_Node *node =
        m_pSurface->get_compact_ledge_tree_root();
...
private:
  ...
  // transform that includes scale from IVP to HL coords, do not
  // VectorITransform or VectorRotate with this
  float m_radius;
  int m_nPointTest;
  int m_nStartPoint;
...
// static const fltx4 g_IVPToHLPosition = { IVP2HL(1.0f), -IVP2HL(1.0f),
// IVP2HL(1.0f), IVP2HL(1.0f) };
...
CTraceIVP::CTraceIVP(const CPhysCollide *pCollide, const Vector &origin,
                     const QAngle &angles) {
#if USE_COLLIDE_MAP
  ...
  // This vector is supposed to be left, so we'll negate it later, but we don't
  // want to negate the position, so add another minus to cancel out
  right.w = -HL2IVP(origin.y);
  up.w = HL2IVP(origin.z);
  fltx4 rx = ConvertDirectionToIVP(LoadAlignedSIMD(forward.Base()));
  fltx4 ry =
      ConvertDirectionToIVP(SubSIMD(Four_Zeros, LoadAlignedSIMD(right.Base())));
  ...
  m_ivpLocalToHLWorld.m_flMatVal[0][0] =
      m_matrix.get_elem(IVP_INDEX_X, 0) * scale;
  m_ivpLocalToHLWorld.m_flMatVal[0][1] =
      m_matrix.get_elem(IVP_INDEX_X, 1) * scale;
  m_ivpLocalToHLWorld.m_flMatVal[0][2] =
      m_matrix.get_elem(IVP_INDEX_X, 2) * scale;

  m_ivpLocalToHLWorld.m_flMatVal[1][0] =
      m_matrix.get_elem(IVP_INDEX_Z, 0) * scale;
  m_ivpLocalToHLWorld.m_flMatVal[1][1] =
      m_matrix.get_elem(IVP_INDEX_Z, 1) * scale;
  m_ivpLocalToHLWorld.m_flMatVal[1][2] =
      m_matrix.get_elem(IVP_INDEX_Z, 2) * scale;

  m_ivpLocalToHLWorld.m_flMatVal[2][0] =
      m_matrix.get_elem(IVP_INDEX_Y, 0) * negScale;
  m_ivpLocalToHLWorld.m_flMatVal[2][1] =
      m_matrix.get_elem(IVP_INDEX_Y, 1) * negScale;
  m_ivpLocalToHLWorld.m_flMatVal[2][2] =
      m_matrix.get_elem(IVP_INDEX_Y, 2) * negScale;
...
bool CTraceIVP::BuildLeafmapCacheRLE(const leafmap_t *RESTRICT pLeafmap) {
  // iterate the rle spans of verts and output them to a buffer in
  ...
  int spanIndex = 1;
  int baseVert = 0;
  const VectorAligned *RESTRICT pVerts =
      (const VectorAligned *)&m_pLedge->get_point_array()[startPoint];
  for (int i = 0; i < m_cacheCount - 1; i++) {
    if (countThisSpan < 4) {
      ...
      v3 = baseVert++;
      countThisSpan--;
      m_vertCache[i].LoadAndSwizzleAligned(pVerts[v0].Base(), pVerts[v1].Base(),
                                           pVerts[v2].Base(),
                                           pVerts[v3].Base());
    } else {
      // we have four verts in this span, just grab the next four
      m_vertCache[i].LoadAndSwizzleAligned(
          pVerts[baseVert + 0].Base(), pVerts[baseVert + 1].Base(),
  ...
  m_vertCache[m_cacheCount - 1].LoadAndSwizzleAligned(
      pVerts[v[0]].Base(), pVerts[v[1]].Base(), pVerts[v[2]].Base(),
      pVerts[v[3]].Base());
  FourVectors::RotateManyBy(&m_vertCache[0], m_cacheCount,
                            *((const matrix3x4_t *)&m_ivpLocalToHLWorld));
...
bool CTraceIVP::BuildLeafmapCache(const leafmap_t *RESTRICT pLeafmap) {
  ...
  // iterate the span of verts and output them to a buffer in post-transform
  // space just iterate the range if one is specified
  int startPoint = pLeafmap->startVert[0];
  int pointCount = pLeafmap->vertCount;
  m_cacheCount = (pointCount + 3) >> 2;
  ...
  const VectorAligned *RESTRICT pVerts =
      (const VectorAligned *)&m_pLedge->get_point_array()[startPoint];
  for (int i = 0; i < m_cacheCount - 1; i++) {
    m_vertCache[i].LoadAndSwizzleAligned(pVerts[0].Base(), pVerts[1].Base(),
                                         pVerts[2].Base(), pVerts[3].Base());
    pVerts += 4;
  }
  ...
  m_vertCache[m_cacheCount - 1].LoadAndSwizzleAligned(
      pVerts[x0].Base(), pVerts[x1].Base(), pVerts[x2].Base(),
      pVerts[x3].Base());
  FourVectors::RotateManyBy(&m_vertCache[0], m_cacheCount,
                            *((const matrix3x4_t *)&m_ivpLocalToHLWorld));
  return true;
...
int CTraceIVP::SupportMap(const Vector &dir, Vector *pOut) const {
  ...
  if (m_pLeafmap && m_pLeafmap->HasSingleVertexSpan()) {
    ...
    TransformPositionFromLocal(
        pPoints[best], *pOut); // transform point position to world space
    return best;
  } else {
    ...
    const IVP_Compact_Triangle *RESTRICT pTri =
        m_pLedge->get_first_triangle() + triIndex;
    ...
    // This should never happen.  MAX_CONVEX_VERTS is very large (millions),
    // none of our models have anywhere near this many verts in a convex piece
...
class CTraceSolver {
public:
  CTraceSolver(trace_t *ptr, ITraceObject *sweepobject, CTraceRay *ray,
               ITraceObject *obstacle, const Vector &axis) {
    m_pTotalTrace = ptr;
...
public:
  CTraceSolverSweptObject(trace_t *ptr, ITraceObject *sweepobject,
                          CTraceRay *ray, CTraceIVP *obstacle,
                          const Vector &axis, unsigned int contentsMask,
                          IConvexInfo *pConvexInfo);

  void InitOSRay(void);
  void SweepLedgeTree_r(const IVP_Compact_Ledgetree_Node *node);
  inline bool SweepHitsSphereOS(const IVP_U_Float_Point *sphereCenter,
                                float radius);
  virtual void DoSweep(void);
...
private:
  CTraceSolverSweptObject(const CTraceSolverSweptObject
                              &); // no implementation, quells compiler warning
};

CTraceSolverSweptObject::CTraceSolverSweptObject(
    trace_t *ptr, ITraceObject *sweepobject, CTraceRay *ray,
  ...
  m_pConvexInfo =
      (pConvexInfo != NULL) ? pConvexInfo : m_fakeConvexInfo.GetPtr();
}

bool CTraceSolverSweptObject::SweepHitsSphereOS(
    const IVP_U_Float_Point *sphereCenter, float radius) {
  ...
  if (m_rayLengthOS > 0) {
    ...
    // The perpendicular forms a right triangle with the vector between the
    // ray/sphere centers and the ray direction vector.  Calculate the
    // projection of the hypoteneuse along the perpendicular
    IVP_U_Float_Point h;
    ...
    // Could a ray in any direction away from the ray center intersect this
    // sphere?
...
inline void CTraceSolverSweptObject::SweepAgainstNode(
    const IVP_Compact_Ledgetree_Node *node) {
  const IVP_Compact_Ledge *ledge = node->get_compact_ledge();
  unsigned int ledgeContents =
      m_pConvexInfo->GetContents(ledge->get_client_data());
  if (m_contentsMask & ledgeContents) {
    ...
    if (SweepSingleConvex()) {
      if (m_traceLength < m_totalTraceLength) {
        ...
        // shrink the ray to the shortened length, but leave a buffer of
        // collisionSweepEpsilon units at the end to make sure that precision
        // doesn't make you miss something slightly closer
        float testFraction =
            (m_traceLength + m_epsilon * 2) * m_ray->m_ooBaseLength;
        if (testFraction < 1.0f) {
          m_ray->Reset(testFraction);
          // Update OS ray to limit tests
          m_rayLengthOS =
              m_obstacleIVP->TransformLengthToLocal(m_ray->m_length);
          m_rayCenterOS.add_multiple(&m_rayStartOS, &m_rayDeltaOS,
                                     0.5f * testFraction);
        }
#endif
...
void CTraceSolverSweptObject::SweepLedgeTree_r(
    const IVP_Compact_Ledgetree_Node *node) {
  ...
  // this is nearly a priority queue, but not actually, but it's cheaper (and
  // faster in the benchmarks) this code is trying to visit the nodes closest to
  // the ray start first - which helps performance since we're only interested
  // in the first intersection of the swept object with the physcollide.
  while (1) {
...
void CTraceSolverSweptObject::InitOSRay(void) {
  ...
  // add_multiple with 3 params assumes no initial value (should be
  // set_add_multiple)
...
void CPhysicsTrace::SweepBoxIVP(const Vector &start, const Vector &end,
...
void CPhysicsTrace::SweepBoxIVP(const Ray_t &raySrc, unsigned int contentsMask,
                                IConvexInfo *pConvexInfo,
                                const CPhysCollide *pCollide,
                                const Vector &surfaceOrigin,
                                const QAngle &surfaceAngles, trace_t *ptr) {
  ...
  // offset the space of this sweep so that the surface is at the origin of the
  // solution space
  CTraceRay ray(raySrc, -surfaceOrigin);

  CTraceSolverSweptObject solver(ptr, &box, &ray, &ivp, ray.m_start,
                                 contentsMask, pConvexInfo);
  solver.DoSweep();

  VectorAdd(raySrc.m_Start, raySrc.m_StartOffset, ptr->startpos);
  VectorMA(ptr->startpos, ptr->fraction, raySrc.m_Delta, ptr->endpos);
  // The plane was shifted because we shifted everything over by surfaceOrigin,
  // shift it back
  if (ptr->DidHit()) {
...
void CPhysicsTrace::SweepIVP(const Vector &start, const Vector &end,
  ...
  IVP_Compact_Ledge_Solver::get_all_ledges(pSweptSurface->GetCompactSurface(),
                                           &objectLedges);
  for (int i = objectLedges.len() - 1; i >= 0; --i) {
    trace_t tr;
    CM_ClearTrace(&tr);
    sweptObject.SetLedge(objectLedges.element_at(i));
    CTraceSolverSweptObject solver(&tr, &sweptObject, &ray, &ivp,
                                   start - surfaceOrigin, MASK_ALL, NULL);
    // UNDONE: Need just more than 0.25" tolerance here because the output
    // position will be used by vphysics UNDONE: Really this should be the
    // collision radius from the environment.
...
static void CalculateSeparatingPlane(trace_t *ptr, ITraceObject *sweepObject,
                                     CTraceRay *ray, ITraceObject *obstacle,
                                     simplex_t &simplex);
...
// Anyway it slowly builds one of these one vert at a time with a directed
// search. So you start out with a point, then it guesses the next point that
// would be most likely to form a line through the origin.  If the line doesn't
// go quite through the origin it tries to find a third point to capture the
// origin within a triangle.  If that doesn't work it tries to make a tent
// (tetrahedron) out of the triangle to capture the origin.
//
  ...
  for (int i = 0; i < 100; i++) {
    ...
    if (testLen < 0) {
      ...
      if (testLen < m_epsilon && m_ray->m_length > 0) {
        ...
        Vector normal = dir;
        if (testLen > 0) {
          // try to find a better separating plane or clip the ray to the
          // current one
          for (int j = 0; j < 20; j++) {
        ...
        if (dot < -(m_epsilon * 0.1) ||
            (dot < -1e-4f && testLen < (m_epsilon * 0.9))) {
          ...
          // m_trace.endpos = m_ray->m_start*(1.f-m_trace.fraction) +
          // m_ray->m_end*m_trace.fraction;
    ...
    if (simplex.SolveGJKSet(vert, &m_pointClosestToIntersection)) {
      ...
      if (m_ray->m_length != 0) {
        ...
        if (dist < m_ray->m_length && dist > 0.f) {
          ...
          CalculateSeparatingPlane(&m_trace, m_sweepObject, m_ray, m_obstacle,
                                   simplex);
          ...
          // UNDONE: This case happens when you start solid as well as when a
          // false intersection is detected at the very end of the trace
...
FORCEINLINE bool simplex_t::TriangleSimplex(const simplexvert_t &newPoint,
                                            int outIndex,
...
FORCEINLINE bool simplex_t::EdgeSimplex(const simplexvert_t &newPoint,
                                        int outIndex, const Vector &edge,
                                        Vector *pOut) {
  vertCount = 2;
  verts[outIndex] = newPoint;
  Vector cross;
...
FORCEINLINE bool simplex_t::PointSimplex(const simplexvert_t &newPoint,
                                         Vector *pOut) {
  vertCount = 1;
  verts[0] = newPoint;
...
bool simplex_t::SolveVoronoiRegion2(const simplexvert_t &newPoint,
                                    Vector *pOut) {
...
bool simplex_t::SolveVoronoiRegion3(const simplexvert_t &newPoint,
                                    Vector *pOut) {
...
bool simplex_t::SolveVoronoiRegion4(const simplexvert_t &newPoint,
                                    Vector *pOut) {
...
void CalculateSeparatingPlane(trace_t *ptr, ITraceObject *sweepObject,
                              CTraceRay *ray, ITraceObject *obstacle,
                              simplex_t &simplex) {
  ...
  if (testCount == 3) {
    ...
    if (simplex.vertCount) {
      ptr->plane.dist =
          DotProduct(ptr->plane.normal,
                     obstacle->GetVertByIndex(simplex.verts[0].obstacleIndex));
    } else
      ptr->plane.dist = 0.f;
  }
}
...
// It needs to test all 4 faces of the tetrahedron to find the one the ray
// leaves through this is done by finding the closest plane by clipping the ray
// to each plane
Vector simplex_t::ClipRayToTetrahedronBase(const Vector &dir) {
...
#if CHECK_TOI_CALCS
  Assert(DotProduct(verts[2].position, ABC) <
         DotProduct(verts[3].position, ABC));
  Assert(DotProduct(verts[0].position, ACD) <
         DotProduct(verts[3].position, ACD));
  Assert(DotProduct(verts[1].position, ADB) <
         DotProduct(verts[3].position, ADB));
  Assert(DotProduct(verts[3].position, BCD) <
         DotProduct(verts[0].position, BCD));
#endif
  ...
  // Note that we need to preserve winding so that the vector order assumptions
...
// for subsequent iterations you don't need to test one of the faces (face you
// previously left through) so only test the three faces involving the new point
// A
...
float simplex_t::ClipRayToTriangle(const Vector &dir, float epsilon) {
  ...
  // Assert valid triangle/winding order (all normals point away from the
  // origin)
...
// Do this by iteratively clipping the ray to the tetrahedron containing the
// origin when a triangle is found intersecting the ray, reduce the simplex to
// that triangle and then re-expand it to a tetrahedron using the support point
// normal to the triangle (away from the origin) iterate until no new points can
// be found.  That's the surface of the sum.
float CTraceSolver::SolveMeshIntersection(simplex_t &simplex) {
  ...
  for (int i = 0; i < 100; i++) {
    ...
    // map the new separating axis (NOTE: This test is inverted from the GJK -
    // we are trying to get out, not in) found a separating axis, we've moved
    // the sweep back enough
...
// similar to SolveMeshIntersection, but solves projected into the 2D triangle
// simplex remaining this is used for the near miss case
float CTraceSolver::SolveMeshIntersection2D(simplex_t &simplex) {
  AssertMsg(simplex.vertCount == 3, "simplex.vertCount != 3: %d",
            simplex.vertCount);
  if (simplex.vertCount != 3)
    return 0.0f;

  // note: This should really do this iteratively in case the triangle is
  // coplanar with another triangle that is between this one and the edge of the
  // sum in this plane
...
void TraceGetAABB_r(Vector *pMins, Vector *pMaxs,
                    const IVP_Compact_Ledgetree_Node *node, CTraceIVP &ivp) {
  if (node->is_terminal() == IVP_TRUE) {
    Vector tmp;
...
void CPhysicsTrace::GetAABB(Vector *pMins, Vector *pMaxs,
                            const CPhysCollide *pCollide,
  ...
  // JAY: Disable this here, do it in the engine instead.  That way the tools
  // get accurate bboxes
#if 0
...
void TraceGetExtent_r(const IVP_Compact_Ledgetree_Node *node, CTraceIVP &ivp,
                      const Vector &dir, float &dot, Vector &point) {
  if (node->is_terminal() == IVP_TRUE) {
    ivp.SetLedge(node->get_compact_ledge());
    Vector tmp;
...
Vector CPhysicsTrace::GetExtent(const CPhysCollide *pCollide,
                                const Vector &collideOrigin,
                                const QAngle &collideAngles,
...
bool CPhysicsTrace::IsBoxIntersectingCone(const Vector &boxAbsMins,
