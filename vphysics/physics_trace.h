...
public:
  ...
  virtual void
  GetAllLedges(IVP_U_BigVector<IVP_Compact_Ledge> &ledges) const = 0;
...
struct leafmap_t {
  ...
  void SetHasCubemap() { flags = LEAFMAP_HAS_CUBEMAP; }
  ...
  int MaxSpans() { return sizeof(startVert) - sizeof(startVert[0]); }
  const byte *GetSpans() const {
    return reinterpret_cast<const byte *>(&startVert[1]);
  }
  byte *GetSpans() { return reinterpret_cast<byte *>(&startVert[1]); }
  ...
  inline bool HasSpans() const {
    return (flags & (LEAFMAP_HAS_SINGLE_VERTEX_SPAN |
                     LEAFMAP_HAS_MULTIPLE_VERTEX_SPANS))
               ? true
               : false;
  }
...
public:
  static CPhysCollide *UnserializeFromBuffer(const char *pBuffer,
...
public:
  CPhysicsTrace();
  ~CPhysicsTrace();
  // Calculate the intersection of a swept box (mins/maxs) against an IVP
  // object.  All coords are in HL space.
  void SweepBoxIVP(const Vector &start, const Vector &end, const Vector &mins,
                   const Vector &maxs, const CPhysCollide *pSurface,
                   const Vector &surfaceOrigin, const QAngle &surfaceAngles,
                   trace_t *ptr);
  void SweepBoxIVP(const Ray_t &raySrc, unsigned int contentsMask,
                   IConvexInfo *pConvexInfo, const CPhysCollide *pSurface,
                   const Vector &surfaceOrigin, const QAngle &surfaceAngles,
                   trace_t *ptr);

  // Calculate the intersection of a swept compact surface against another
  // compact surface.  All coords are in HL space. NOTE: BUGBUG: swept surface
  // must be single convex!!!
  void SweepIVP(const Vector &start, const Vector &end,
                const CPhysCollide *pSweptSurface, const QAngle &sweptAngles,
                const CPhysCollide *pSurface, const Vector &surfaceOrigin,
                const QAngle &surfaceAngles, trace_t *ptr);

  // get an AABB for an oriented collide
  void GetAABB(Vector *pMins, Vector *pMaxs, const CPhysCollide *pCollide,
               const Vector &collideOrigin, const QAngle &collideAngles);

  // get the support map/extent for a collide along the axis given by
  // "direction"
  Vector GetExtent(const CPhysCollide *pCollide, const Vector &collideOrigin,
...
private:
  // Store the current increment and the vertex ID (rotating hash) to guarantee
  // no collisions
  struct vertmarker_t {
...
// Calculate the intersection of a swept box (mins/maxs) against an IVP object.
// All coords are in HL space.
inline unsigned short CVisitHash::VertIndexToID(int vertIndex) {
  ...
  // That way, the index will not collide until you've hit 64K # hash entries
  // times
...
inline bool CVisitHash::WasVisited(int vertIndex) {
  ...
  if (m_vertVisit[hashIndex].visitID == m_vertVisitID &&
      m_vertVisit[hashIndex].vertID == id)
    return true;
...
extern IVP_SurfaceManager *
CreateSurfaceManager(const CPhysCollide *pCollisionModel, short &collideType);
