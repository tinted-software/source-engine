...
void SetScissorRect(int left, int top, int right, int bottom) {
  ...
  g_bFullScreenScissor =
      (left <= vx && top <= vy && right >= vw && bottom >= vh);
#endif
}
...
//-----------------------------------------------------------------------------
template <class Clipper>
bool ClipLineToPlane(Clipper &clipper, const vgui::Vertex_t *pInVerts,
...
bool ClipRect(const vgui::Vertex_t &inUL, const vgui::Vertex_t &inLR,
  ...
  if (IsX360() && (!g_bScissor || g_bFullScreenScissor ||
                   (inUL.m_Position.x >= g_ScissorRect.left &&
                    inLR.m_Position.x <= g_ScissorRect.right &&
                    inUL.m_Position.y >= g_ScissorRect.top &&
                    inLR.m_Position.y <= g_ScissorRect.bottom))) {
    // clipping is not needed
    // either full screen, and hw will do it or rect is inscribed, and operation
    // is meaningless
    *pOutUL = inUL;
    *pOutLR = inLR;
    return true;
