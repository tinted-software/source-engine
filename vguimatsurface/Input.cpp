...
static LRESULT CALLBACK MatSurfaceWindowProc(HWND hwnd, UINT uMsg,
  ...
  case WM_XCONTROLLER_KEY:
    if (IsX360()) {
      ...
      // xboxissue - as yet HL2 input hasn't been made aware of analog inputs or
      // ports so just digital step on the sample range
...
void InputAttachToWindow(void *hwnd) {
  ...
  if (hwnd && !HushAsserts()) {
    // under OSX we use the Cocoa mgr to route events rather than hooking
    // winprocs and under Linux we use SDL
    Assert(!"Implement me");
  }
#endif
}
...
bool InputHandleInputEvent(const InputEvent_t &event) {
  ...
  case IE_FingerDown: {
    int w, h, x, y;
    g_MatSystemSurface.GetScreenSize(w, h);
    float _x, _y;
