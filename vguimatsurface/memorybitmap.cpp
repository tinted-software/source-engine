...
void MemoryBitmap::Paint() {
  ...
  int wide, tall;
  GetSize(wide, tall);
  g_MatSystemSurface.DrawTexturedRect(_pos[0], _pos[1], _pos[0] + wide,
                                      _pos[1] + tall);
}
...
void MemoryBitmap::ForceUpload(unsigned char *texture, int wide, int tall) {
  ...
  // Not our first time through and the size changed, destroy and recreate
  // texture id...
  ...
  g_MatSystemSurface.DrawSetTextureRGBA(m_iTextureID, texture, wide, tall, true,
                                        true);
...
// Purpose: data accessor
//-----------------------------------------------------------------------------
HTexture MemoryBitmap::GetID() { return m_iTextureID; }
