...
public:
  ...
  void SetSubTextureRGBAEx(int drawX, int drawY, unsigned const char *rgba,
                           int subTextureWide, int subTextureTall,
                           ImageFormat imageFormat);
  void UpdateSubTextureRGBA(int drawX, int drawY, unsigned const char *rgba,
                            int subTextureWide, int subTextureTall,
                            ImageFormat imageFormat);
...
public:
  ...
  int CreateTextureByTexture(ITexture *pTexture,
  ...
  void SetTextureRGBAEx(int id, const char *rgba, int wide, int tall,
                        ImageFormat format, bool bFixupTextCoordsForDimensions);
  void SetSubTextureRGBA(int id, int drawX, int drawY,
                         unsigned const char *rgba, int subTextureWide,
                         int subTextureTall);
  void SetSubTextureRGBAEx(int id, int drawX, int drawY,
                           unsigned const char *rgba, int subTextureWide,
                           int subTextureTall, ImageFormat imageFormat);
  void UpdateSubTextureRGBA(int id, int drawX, int drawY,
                            unsigned const char *rgba, int subTextureWide,
                            int subTextureTall, ImageFormat imageFormat);
...
public:
  ...
  void UpdateBackingBits(Rect_t &subRect, const unsigned char *pBits,
                         Rect_t &uploadRect, ImageFormat format) {
    int size =
        ImageLoader::GetMemRequired(m_nWidth, m_nHeight, 1, m_nFormat, false);
    if (IsPC()) {
      if (!m_pTextureBits)
    ...
    if (ImageLoader::SizeInBytes(m_nFormat) == 4) {
      bool bIsInputFullRect = (subRect.width != uploadRect.width ||
                               subRect.height != uploadRect.height);
      ...
      for (y = 0; y < subRect.height; ++y) {
        ...
        int offset = bIsInputFullRect
                         ? (subRect.y + y) * uploadRect.width + subRect.x
                         : y * uploadRect.width;
        const unsigned int *pSrc = (const unsigned int *)(&pBits[offset << 2]);
        ImageLoader::ConvertImageFormat((const unsigned char *)pSrc, format,
                                        (unsigned char *)pDst, m_nFormat,
                                        subRect.width, 1);
      }
    } else {
    ...
    if (nFormatBytes == 4) {
      if (m_nFormat == pVTFTexture->Format()) {
        ...
        for (int y = pSubRect->y; y < ymax; ++y) {
          ...
          char *pchData = (char *)pVTFTexture->ImageData(0, 0, 0, 0, y) +
                          pSubRect->x * nFormatBytes;
          int size = ImageLoader::GetMemRequired(pSubRect->width, 1, 1,
                                                 m_nFormat, false);
          V_memcpy(pchData,
...
void CMatSystemTexture::SetSubTextureRGBA(int drawX, int drawY,
                                          ...
                                          int subTextureWide,
                                          int subTextureTall) {
  SetSubTextureRGBAEx(drawX, drawY, rgba, subTextureWide, subTextureTall,
                      IMAGE_FORMAT_RGBA8888);
}

void CMatSystemTexture::SetSubTextureRGBAEx(int drawX, int drawY,
                                            unsigned const char *rgba,
                                            int subTextureWide,
                                            int subTextureTall,
                                            ImageFormat format) {
...
void CMatSystemTexture::UpdateSubTextureRGBA(int drawX, int drawY,
                                             unsigned const char *rgba,
                                             int subTextureWide,
                                             int subTextureTall,
                                             ImageFormat imageFormat) {
...
int CTextureDictionary::CreateTextureByTexture(ITexture *pTexture,
...
void CTextureDictionary::DestroyTexture(int id) {
  if (m_Textures.Count() <= 1) {
    // TextureDictionary already destroyed all the textures before this
    // (something destructed late in shutdown)
    return;
  }
...
void CTextureDictionary::SetSubTextureRGBA(int id, int drawX, int drawY,
                                           ...
                                           int subTextureWide,
                                           int subTextureTall) {
  SetSubTextureRGBAEx(id, drawX, drawY, rgba, subTextureWide, subTextureTall,
                      IMAGE_FORMAT_RGBA8888);
}

void CTextureDictionary::SetSubTextureRGBAEx(int id, int drawX, int drawY,
                                             unsigned const char *rgba,
                                             int subTextureWide,
                                             int subTextureTall,
                                             ImageFormat format) {
  ...
  CMatSystemTexture &texture = m_Textures[id];
  texture.SetSubTextureRGBAEx(drawX, drawY, rgba, subTextureWide,
                              subTextureTall, format);
}

void CTextureDictionary::UpdateSubTextureRGBA(int id, int drawX, int drawY,
                                              unsigned const char *rgba,
                                              int subTextureWide,
                                              int subTextureTall,
                                              ImageFormat imageFormat) {
  ...
  CMatSystemTexture &texture = m_Textures[id];
  texture.UpdateSubTextureRGBA(drawX, drawY, rgba, subTextureWide,
                               subTextureTall, imageFormat);
}
