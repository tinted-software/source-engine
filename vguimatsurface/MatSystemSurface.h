...
public:
  ...
  void DrawSetSubTextureRGBA(int textureID, int drawX, int drawY,
                             ...
                             int subTextureTall);
  void DrawUpdateRegionTextureRGBA(int nTextureID, int x, int y,
                                   const unsigned char *pchData, int wide,
  ...
  virtual void GetClippingRect(int &left, int &top, int &right, int &bottom,
                               bool &bClippingDisabled) OVERRIDE;
  virtual void SetClippingRect(int left, int top, int right,
                               int bottom) OVERRIDE;
  ...
  virtual void
  SetFullscreenViewportAndRenderTarget(int x, int y, int w, int h,
                                       ITexture *pRenderTarget) OVERRIDE;
  virtual void
  GetFullscreenViewportAndRenderTarget(int &x, int &y, int &w, int &h,
...
private:
  ...
  void DrawQuadArray(int numQuads, vgui::Vertex_t *pVerts,
                     unsigned char *pColor, bool bShouldClip = true);
  ...
  // If we are in 3d Paint mode, are we rendering to a texture?  (Or directly to
  // the screen)
...
#if GLMDEBUG
class MatSurfFuncLogger // rip off of GLMFuncLogger - figure out a way to
                        // reunify these soon
{
public:
  // simple function log
  ...
  // anything else of interest) no macro for this one, since no easy way to pass
  // through the args as well as the funcname
  MatSurfFuncLogger(char *funcName, PRINTF_FORMAT_STRING const char *fmt, ...) {
    ...
    // which means we need to inject a ">" at the front of the format string to
    // make this work... sigh.
