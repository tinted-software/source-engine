...
CUtlDict<CMatSystemSurface::font_entry, unsigned short>
    CMatSystemSurface::m_FontData;
#endif
...
CMatSystemSurface::CMatSystemSurface()
    : m_pEmbeddedPanel(NULL), m_pWhite(NULL) {
  m_iBoundTexture = -1;
  m_HWnd = NULL;
...
void CMatSystemSurface::InitFullScreenBuffer(const char *pszRenderTargetName) {
  ...
  char pTemp[512];
  Q_snprintf(pTemp, sizeof(pTemp), "VGUI_3DPaint_FullScreen_%s",
             pszRenderTargetName);
...
InitReturnVal_t CMatSystemSurface::Init(void) {
  ...
  if (!(CommandLine()->FindParm("-disable_matsurf_noculls"))) {
    pVMTKeyValues->SetInt("$nocull",
                          1); // skip this if user asks for the switch above
  }
  ...
  if (IsPC()) {
    bValid = system()->GetRegistryString(
        "HKEY_CURRENT_USER\\Software\\Valve\\Source\\Language", language,
        sizeof(language) - 1);
  } else {
...
void CMatSystemSurface::Shutdown(void) {
  ...
  if (gdiModule) {
    pRemoveFontResourceEx = (RemoveFontResourceExProc)::GetProcAddress(
        gdiModule, "RemoveFontResourceExA");
  }
  ...
  for (int i = 0; i < m_CustomFontFileNames.Count(); i++) {
    if (pRemoveFontResourceEx) {
      ...
      int nRetries = 0;
      while ((*pRemoveFontResourceEx)(m_CustomFontFileNames[i].String(), 0x10,
                                      NULL) &&
             (nRetries < 10)) {
        nRetries++;
      ...
      // when we load the font next time we get the real font instead of Ariel.
      int nRetries = 0;
      while (::RemoveFontResource(m_CustomFontFileNames[i].String()) &&
             (nRetries < 10)) {
        nRetries++;
        Msg("Removed font resource %s on attempt %d.\n",
            m_CustomFontFileNames[i].String(), nRetries);
...
void CMatSystemSurface::DrawQuadArray(int quadCount, vgui::Vertex_t *pVerts,
                                      unsigned char *pColor, bool bShouldClip) {
  MAT_FUNC;
...
//-----------------------------------------------------------------------------
// Draws a fade between the fadeStartPt and fadeEndPT with the current draw
// color oriented according to argument
//   Example: DrawFilledRectFastFade( 10, 10, 100, 20, 50, 60, 255, 128, true );
//			  -this will draw
//					a solid rect (10,10,50,20) //alpha 255
//					a solid rect (50,10,60,20) //alpha faded
//from 255 to 128 					a solid rect (60,10,100,20) //alpha 128
//-----------------------------------------------------------------------------
void CMatSystemSurface::DrawFilledRectFastFade(int x0, int y0, int x1, int y1,
...
void CMatSystemSurface::DrawTexturedPolygon(int n, Vertex_t *pVertices,
  ...
  if (bClipVertices) {
    ...
    iCount = ClipPolygon(n, pVertices, m_nTranslateX, m_nTranslateY,
                         &ppClippedVerts);
    if (iCount <= 0)
      return;
    ...
    for (int i = 0; i < n; ++i) {
      meshBuilder.Position3f(pVertices[i].m_Position.x + m_nTranslateX,
                             pVertices[i].m_Position.y + m_nTranslateY,
                             m_flZPos);
      meshBuilder.Color4ubv(m_DrawColor);
...
bool CMatSystemSurface::SetFontGlyphSet(HFont font, const char *windowsFontName,
                                        ...
                                        int scanlines, int flags, int nRangeMin,
                                        int nRangeMax) {
  return FontManager().SetFontGlyphSet(font, windowsFontName, tall, weight,
                                       blur, scanlines, flags, nRangeMin,
                                       nRangeMax);
}
...
//-----------------------------------------------------------------------------
void CMatSystemSurface::GetKernedCharWidth(HFont font, wchar_t ch,
                                           wchar_t chBefore, wchar_t chAfter,
                                           float &wide,
                                           float &abcA) //, float &abcC )
{
  float abcC = 0.0f;
  FontManager().GetKernedCharWidth(font, ch, chBefore, chAfter, wide, abcA,
                                   abcC);
}
...
// for now)
//-----------------------------------------------------------------------------
bool CMatSystemSurface::AddCustomFontFile(const char *fontName,
                                          const char *fontFileName) {
  if (IsX360()) {
    // custom fonts are not supported (not needed) on xbox, all .vfonts are
    // offline converted to ttfs ttfs are mounted/handled elsewhere
    return true;
  }
  MAT_FUNC;
  ...
  // try and use the optimal custom font loader, will makes sure fonts are
  // unloaded properly this function is in a newer version of the gdi library
  // (win2k+), so need to try get it directly
#if defined(WIN32) && !defined(_X360)
  ...
  if (gdiModule) {
    ...
    AddFontResourceExProc pAddFontResourceEx =
        (AddFontResourceExProc)::GetProcAddress(gdiModule,
                                                "AddFontResourceExA");
...
void *CMatSystemSurface::FontDataHelper(const char *pchFontName, int &size,
  ...
  if (fontFileName) {
    // If we were given a fontFileName, then load that bugger and shove it in
    // the cache.
    ...
    const FT_Error error =
        FT_New_Memory_Face(FontManager().GetFontLibraryHandle(),
                           (FT_Byte *)buf.Base(), buf.TellPut(), 0, &face);
    ...
    if (!pchFontName) {
      // If we weren't passed a font name for this thing, then use the one from
      // the face.
...
//-----------------------------------------------------------------------------
void CMatSystemSurface::PrecacheFontCharacters(
    HFont font, const wchar_t *pCharacterString) {
  wchar_t *pCommonChars =
...
void CMatSystemSurface::DrawFlushText() {
...

  {
    ...
    IMaterial *pMaterial =
        TextureDictionary()->GetTextureMaterial(m_iBoundTexture);
    InternalSetMaterial(pMaterial);
    DrawQuadArray(m_nBatchedCharVertCount / 2, m_BatchedCharVerts,
                  m_DrawTextColor);
    m_nBatchedCharVertCount = 0;
  }
}
...
bool CMatSystemSurface::DrawGetUnicodeCharRenderInfo(wchar_t ch,
  ...
  // Because CharRenderInfo has a pointer to the verts, we need to keep
  // m_BatchedCharVerts in sync, so if we
...
void CMatSystemSurface::DrawPrintText(
  ...
  for (int i = 0; i < iTextLen; ++i) {
    ...
    if (i < (iTextLen - 1))
      chAfter = text[i + 1];
    FontManager().GetKernedCharWidth(m_hCurrentFont, ch, chBefore, chAfter,
                                     flWide, flabcA, flabcC);

    int abcA, abcB, abcC;
    // also grab the single char dimensions so we match the texture size to the
    // one in the font page, different to the amount we step ahead once rendered
...
void CMatSystemSurface::GetScreenSize(int &iWide, int &iTall) {
  ...
  // mikesart: This is just sticking in unnecessary BeginRender/EndRender calls
  // to the queue.
  //   CMatRenderContextPtr pRenderContext( g_pMaterialSystem );
...
bool CMatSystemSurface::ForceScreenSizeOverride(bool bState, int wide,
                                                int tall) {
...
// LocalToScreen, ParentLocalToScreen fixups for explicit PaintTraverse calls on
// Panels not at 0, 0 position
...
void CMatSystemSurface::SetAsToolBar(
    VPANEL panel,
    bool
        state) // removes the window's task bar entry (for context menu's, etc.)
{}
...
#elif defined(POSIX)
  // SetCapture on Win32 makes all the mouse messages (move and button up/down)
  // head to
  //	the captured window. From what I can tell, this routine is called for
  //modal dialogs 	when you click down on a button. However the current behavior
  //is to highlight the 	buttons when you're over them, and trigger when you
  //mouse up over the top - so I 	don't believe that SetCapture is needed on
  //Windows, and Linux is behaving exactly 	the same as Win32 in all the tests
  //I've run so far. (I've clicked on a lot of dialogs).
  // I talked with Alfred about this and we haven't done any SetCapture stuff on
  // OSX ever
...
void CMatSystemSurface::SolveTraverse(VPANEL panel,
                                      bool forceApplySchemeSettings) {
  {
    ...
    tmZone(TELEMETRY_LEVEL1, TMZF_NONE, "%s - InternalSchemeSettingsTraverse",
           __FUNCTION__);
...

  {
    ...
    tmZone(TELEMETRY_LEVEL1, TMZF_NONE, "%s - InternalThinkTraverse",
           __FUNCTION__);
...

  {
    ...
    tmZone(TELEMETRY_LEVEL1, TMZF_NONE, "%s - InternalSolveTraverse",
           __FUNCTION__);
...
void CMatSystemSurface::RestrictPaintToSinglePanel(VPANEL panel) {
  ...
  if (!input()->GetAppModalSurface()) {
    input()->SetAppModalSurface(
        panel); // if painting is restricted to this panel, it had better be
                // modal, or else you can get in some bad state...
  }
}
...
void CMatSystemSurface::Begin3DPaint(int iLeft, int iTop, int iRight,
                                     int iBottom, bool bRenderToTexture) {
  MAT_FUNC;
  ...
  if (bRenderToTexture) {
    ...
    // allocates NOTE: We have to grab it here, as opposed to during init,
    // because the mode hasn't been set by now.
    if (!m_FullScreenBuffer) {
      m_FullScreenBuffer.Init(
          materials->FindTexture(m_FullScreenBufferName, "render targets"));
    }
    ...
    pRenderContext->Viewport(clipLeft + iLeft, clipTop + iTop, iRight - iLeft,
                             iBottom - iTop);
  }
...
//-----------------------------------------------------------------------------
void CMatSystemSurface::BeginSkinCompositionPainting() { g_bInDrawing = true; }

//-----------------------------------------------------------------------------
// end drawing when finish skin composition
//-----------------------------------------------------------------------------
void CMatSystemSurface::EndSkinCompositionPainting() { g_bInDrawing = false; }
...
//-----------------------------------------------------------------------------
void CMatSystemSurface::GetClippingRect(int &left, int &top, int &right,
                                        int &bottom, bool &bClippingDisabled) {
  bool bEnabled = true;
...
//-----------------------------------------------------------------------------
void CMatSystemSurface::SetClippingRect(int left, int top, int right,
                                        int bottom) {
...
bool CMatSystemSurface::IsInThink(VPANEL panel) {
  if (m_bInThink) {
    if (panel ==
        m_CurrentThinkPanel) // HasParent() returns true if you pass yourself in
    {
      return false;
    }
...
bool CMatSystemSurface::BHTMLWindowNeedsPaint(IHTML *htmlwin) { return false; }

//-----------------------------------------------------------------------------
/*void CMatSystemSurface::DrawSetTextureRGBA( int id, const unsigned char* rgba,
int wide, int tall )
{
        TextureDictionary()->SetTextureRGBAEx( id, (const char *)rgba, wide,
tall, IMAGE_FORMAT_RGBA8888 );
}*/
...
void CMatSystemSurface::DrawUpdateRegionTextureRGBA(
    int nTextureID, int x, int y, const unsigned char *pchData, int wide,
    int tall, ImageFormat imageFormat) {
  TextureDictionary()->UpdateSubTextureRGBA(nTextureID, x, y, pchData, wide,
                                            tall, imageFormat);
}
...
vgui::IImage *
CMatSystemSurface::GetIconImageForFullPath(char const *pFullPath) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CMatSystemSurface::Set3DPaintTempRenderTarget(
    const char *pRenderTargetName) {
  MAT_FUNC;
...
//			rendering into the frame buffer. This version of the
//function is 			mostly here for backward compatibility and always uses a NULL
...
//-----------------------------------------------------------------------------
// Purpose: Sets the origin of the viewport to render into if we were
//			rendering into the frame buffer. We might actually be
//generally 			using a render target but need to switch back to the frame buffer
//			for some panels.
//-----------------------------------------------------------------------------
void CMatSystemSurface::SetFullscreenViewportAndRenderTarget(
    int x, int y, int w, int h, ITexture *pRenderTarget) {
...
//			for some panels.
//-----------------------------------------------------------------------------
void CMatSystemSurface::GetFullscreenViewportAndRenderTarget(
    int &x, int &y, int &w, int &h, ITexture **ppRenderTarget) {
  if (m_nFullscreenViewportHeight == 0) {
    // this can't actually be zero. If it is, use the height of the screen
    // instead
    x = y = 0;
    GetScreenSize(w, h);
    if (ppRenderTarget)
...
void CMatSystemSurface::PushFullscreenViewport() {
  ...
  // the viewport x/y will be wrong because the render target is only for one
  // eye. Ask the surface for that information instead.
  int vx, vy, vw, vh;
  ITexture *pRenderTarget;
  GetFullscreenViewportAndRenderTarget(vx, vy, vw, vh, &pRenderTarget);
  pRenderContext->PushRenderTargetAndViewport(pRenderTarget, NULL, vx, vy, vw,
                                              vh);
  ...
  //___stop___();
  pRenderContext->Ortho(g_flPixelOffsetX, g_flPixelOffsetY,
                        vw + g_flPixelOffsetX, vh + g_flPixelOffsetY, -1.0f,
                        1.0f);
...
void CMatSystemSurface::DrawSetTextureFrame(int id, int nFrame,
                                            unsigned int *pFrameCache) {
  IMaterial *pMaterial = TextureDictionary()->GetTextureMaterial(id);
  if (!pMaterial) {
    return;
  }
