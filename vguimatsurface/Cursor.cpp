...
void InitCursors() {
  ...
  s_pDefaultCursor[dc_hourglass] =
      SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_WAIT);
  s_pDefaultCursor[dc_crosshair] =
      SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_CROSSHAIR);
  s_pDefaultCursor[dc_waitarrow] =
      SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_WAITARROW);
  s_pDefaultCursor[dc_sizenwse] =
      SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_SIZENWSE);
  s_pDefaultCursor[dc_sizenesw] =
      SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_SIZENESW);
  s_pDefaultCursor[dc_sizewe] =
      SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_SIZEWE);
  s_pDefaultCursor[dc_sizens] =
      SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_SIZENS);
  s_pDefaultCursor[dc_sizeall] =
  ...
  s_pDefaultCursor[dc_sizenwse] =
      (HICON)LoadCursor(NULL, (LPCTSTR)OCR_SIZENWSE);
  s_pDefaultCursor[dc_sizenesw] =
      (HICON)LoadCursor(NULL, (LPCTSTR)OCR_SIZENESW);
...
void CUserCursorManager::Shutdown() {
  for (int i = m_UserCursors.First(); i != m_UserCursors.InvalidIndex();
       i = m_UserCursors.Next(i)) {
    ::DestroyCursor(m_UserCursors[i]);
  }
  m_UserCursors.RemoveAll();
}

vgui::HCursor CUserCursorManager::CreateCursorFromFile(char const *curOrAniFile,
                                                       char const *pPathID) {
  char fn[512];
...
void InitSoftwareCursors() {
  ...
  s_rnSoftwareCursorID[dc_arrow] =
      InitSoftwareCursorTexture("vgui/cursors/arrow");
  s_rnSoftwareCursorID[dc_ibeam] =
      InitSoftwareCursorTexture("vgui/cursors/ibeam");
  s_rnSoftwareCursorID[dc_hourglass] =
      InitSoftwareCursorTexture("vgui/cursors/hourglass");
  s_rnSoftwareCursorID[dc_crosshair] =
      InitSoftwareCursorTexture("vgui/cursors/crosshair");
  s_rnSoftwareCursorID[dc_waitarrow] =
      InitSoftwareCursorTexture("vgui/cursors/waitarrow");
  s_rnSoftwareCursorID[dc_up] = InitSoftwareCursorTexture("vgui/cursors/up");
  s_rnSoftwareCursorID[dc_sizenwse] =
      InitSoftwareCursorTexture("vgui/cursors/sizenwse");
  s_rnSoftwareCursorID[dc_sizenesw] =
      InitSoftwareCursorTexture("vgui/cursors/sizenesw");
  s_rnSoftwareCursorID[dc_sizewe] =
      InitSoftwareCursorTexture("vgui/cursors/sizewe");
  s_rnSoftwareCursorID[dc_sizens] =
      InitSoftwareCursorTexture("vgui/cursors/sizens");
  s_rnSoftwareCursorID[dc_sizeall] =
      InitSoftwareCursorTexture("vgui/cursors/sizeall");
  s_rnSoftwareCursorID[dc_no] = InitSoftwareCursorTexture("vgui/cursors/no");
  s_rnSoftwareCursorID[dc_hand] =
      InitSoftwareCursorTexture("vgui/cursors/hand");
...
void CursorSelect(HCursor hCursor) {
  if ((hCursor == dc_alwaysvisible_push) || (hCursor == dc_alwaysvisible_pop)) {
    // CConPanel in engine/console.cpp does a SetCursor(null). So when the TF2
    // chat window pops up
    //	and there are console commands showing and fading out in the top left,
    //our chat window 	will have a cursor show/hide fight with them. So the
    //cursor flickers or doesn't show up 	at all. Unfortunately on Linux, it's
    //even worse since we recenter the mouse when it's 	not shown - so we added
    //this API call which causes cursor.cpp to always show the cursor.
...
void CursorGetPos(void *hwnd, int &x, int &y) {
  ...
  if (s_bCursorVisible) {
    ...
    SDL_GetWindowSize((SDL_Window *)g_pLauncherMgr->GetWindowRef(),
    ...
    if (!s_bSoftwareCursorActive &&
      ...
      // scale the x/y back into the co-ords of the back buffer, not the scaled
      // up window
      // DevMsg( "Mouse x:%d y:%d %d %d %d %d\n", x, y, width, windowWidth,
      // height, abs( height - windowHeight ) );
      x = x * (float)width / windowWidth;
      y = y * (float)height / windowHeight;
    }
  } else {
    // cursor is invisible, just say we have it pinned to the middle of the
    // screen
    CMatRenderContextPtr pRenderContext(g_pMaterialSystem);
    int rx, ry, width, height;
    pRenderContext->GetViewport(rx, ry, width, height);
...
void EnableSoftwareCursor(bool bEnable) {
  ...
  // set the cursor to the arrow (or none if appropriate) if we're activating
  // the software cursor. VGUI will likely update it again soon, but this will
  // give us some kind of cursor in the meantime
