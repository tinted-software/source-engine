...
// Tests equality
//-----------------------------------------------------------------------------
template <class T> bool IsAttributeEqual(const T &src1, const T &src2) {
  return src1 == src2;
}
...
//
//-----------------------------------------------------------------------------
template <class T> class CDmAttributeOp : public IDmAttributeOp {
public:
...
//-----------------------------------------------------------------------------
CUtlMemoryPool g_DataAlloc4(sizeof(CDmAttribute), 4, CUtlMemoryPool::GROW_SLOW,
                            "4-byte data pool");
CUtlMemoryPool g_DataAlloc8(sizeof(CDmAttribute), 8, CUtlMemoryPool::GROW_SLOW,
                            "8-byte data pool");
...
//-----------------------------------------------------------------------------
template <class T> void *CDmAttributeOp<T>::CreateAttributeData() {
...
template <class T> void CDmAttributeOp<T>::DestroyAttributeData(void *pData) {
  DeleteData<T>(pData);
}
...
//-----------------------------------------------------------------------------
template <class T> void CDmAttributeOp<T>::SetDefaultValue(void *pData) {
  CDmAttributeInfo<T>::SetDefaultValue(*reinterpret_cast<T *>(pData));
}
...
//-----------------------------------------------------------------------------
template <class T> const char *CDmAttributeOp<T>::AttributeTypeName() {
  return CDmAttributeInfo<T>::AttributeTypeName();
}

template <class T> int CDmAttributeOp<T>::DataSize() {
...
//-----------------------------------------------------------------------------
template <class T> bool CDmAttributeOp<T>::SerializesOnMultipleLines() {
  return ::SerializesOnMultipleLines<T>();
}

template <class T> bool CDmAttributeOp<T>::SkipUnserialize(CUtlBuffer &buf) {
  T dummy;
...
//-----------------------------------------------------------------------------
template <class T> class CUndoAttributeSetValueElement : public CUndoElement {
  typedef CUndoElement BaseClass;
...
//-----------------------------------------------------------------------------
template <class T> class CUndoAttributeArrayBase : public CUndoElement {
  typedef CUndoElement BaseClass;
...
public:
  CUndoAttributeArrayInsertCopyBefore(CDmAttribute *pAttribute, int slot,
      ...
      : BaseClass(pAttribute, "CUndoAttributeArrayInsertCopyBefore"),
        m_nIndex(slot), m_newValue(newValue) {
    Assert(pAttribute->GetOwner() &&
           pAttribute->GetOwner()->GetFileId() != DMFILEID_INVALID);
  }
...
// Count
//-----------------------------------------------------------------------------
template <class T> int CDmArrayAttributeOp<T>::Count() const {
  return Data().Count();
}
...
template <class T> inline int CDmArrayAttributeOp<T>::AddToTail(const T &src) {
...
// Removal
//-----------------------------------------------------------------------------
template <class T> void CDmArrayAttributeOp<T>::FastRemove(int elem) {
  if (!m_pAttribute->MarkDirty())
    return;
...
template <class T> void CDmArrayAttributeOp<T>::Remove(int elem) {
...
template <class T> void CDmArrayAttributeOp<T>::RemoveAll() {
  if (!m_pAttribute->MarkDirty())
    return;
...
// Memory deallocation
template <class T> void CDmArrayAttributeOp<T>::Purge() {
  if (!m_pAttribute->MarkDirty())
    return;
...
// Swap Array
//-----------------------------------------------------------------------------
template <class T> void CDmArrayAttributeOp<T>::SwapArray(CUtlVector<T> &src) {
...
// Set value
//-----------------------------------------------------------------------------
template <class T> void CDmArrayAttributeOp<T>::Set(int i, const T &value) {
  if (i < 0 || i >= Data().Count()) {
...
// Swap
//-----------------------------------------------------------------------------
template <class T> void CDmArrayAttributeOp<T>::Swap(int i, int j) {
  if (i == j)
    return;
...
//-----------------------------------------------------------------------------
CUtlMemoryPool g_AttrAlloc(sizeof(CDmAttribute), 32, CUtlMemoryPool::GROW_SLOW,
                           "CDmAttribute pool");
...
//-----------------------------------------------------------------------------
template <class T> bool CDmAttribute::ShouldModify(const T &value) {
...
//-----------------------------------------------------------------------------
template <class T> void CDmAttribute::SetValue(const T &value) {
...
// Constructor
//-----------------------------------------------------------------------------
template <class T, class B> CDmaArrayConstBase<T, B>::CDmaArrayConstBase() {
  m_pAttribute = NULL;
}
...
// Insertion
//-----------------------------------------------------------------------------
template <class T, class B> int CDmaArrayBase<T, B>::AddToTail() {
  T defaultVal;
  CDmAttributeInfo<T>::SetDefaultValue(defaultVal);
...
template <class T, class B> int CDmaArrayBase<T, B>::InsertBefore(int elem) {
  T defaultVal;
  CDmAttributeInfo<T>::SetDefaultValue(defaultVal);
  CDmArrayAttributeOp<T> accessor(this->m_pAttribute);
...
template <class T, class B> int CDmaArrayBase<T, B>::AddToTail(const T &src) {
  CDmArrayAttributeOp<T> accessor(this->m_pAttribute);
...
template <class T, class B> void CDmaArrayBase<T, B>::EnsureCount(int num) {
...
template <class T, class B> void CDmaArrayBase<T, B>::Swap(int i, int j) {
  CDmArrayAttributeOp<T> accessor(this->m_pAttribute);
  accessor.Swap(i, j);
}
...
// Removal
//-----------------------------------------------------------------------------
template <class T, class B> void CDmaArrayBase<T, B>::FastRemove(int elem) {
  CDmArrayAttributeOp<T> accessor(this->m_pAttribute);
  accessor.FastRemove(elem);
}

template <class T, class B> void CDmaArrayBase<T, B>::Remove(int elem) {
  CDmArrayAttributeOp<T> accessor(this->m_pAttribute);
  accessor.Remove(elem);
}

template <class T, class B> void CDmaArrayBase<T, B>::RemoveAll() {
  CDmArrayAttributeOp<T> accessor(this->m_pAttribute);
  accessor.RemoveAll();
}
...
// Memory management
//-----------------------------------------------------------------------------
template <class T, class B> void CDmaArrayBase<T, B>::EnsureCapacity(int num) {
  this->Value().EnsureCapacity(num);
}

template <class T, class B> void CDmaArrayBase<T, B>::Purge() {
  CDmArrayAttributeOp<T> accessor(this->m_pAttribute);
  accessor.Purge();
}
...
//-----------------------------------------------------------------------------
template <class T> class CInstantiateOp {
public:
  CInstantiateOp() {
...
#define DEFINE_ATTRIBUTE_TYPE(_type)                                           \
  ...
  template class CDmrDecoratorConst<                                           \
      _type, CDmaArrayConstBase<_type, CDmaDataExternal<CUtlVector<_type>>>>;
...
template class CDmaDecorator<
    CUtlString, CDmaStringArrayBase<CDmaDataInternal<CUtlVector<CUtlString>>>>;
template class CDmrDecorator<
