...
CUndoManager::CUndoManager()
    : m_bEnabled(true), m_bDiscarded(false), m_nMaxUndoDepth(4096),
      m_nNesting(0), m_nNotifyNesting(0), m_bStreamStart(false),
      m_bTrace(false), m_bSuppressingNotify(false),
      m_nItemsAddedSinceStartOfStream(0), m_nNotifySource(0), m_nNotifyFlags(0),
      m_nChainingID(0), m_PreviousChainingID(0) {}
...
UtlSymId_t CUndoManager::GetUndoDescInternal(const char *context) {
  if (m_nNesting <= 0) {
    ...
    if (s_DescErrors.Find(sym) == s_DescErrors.InvalidIndex()) {
      Warning("CUndoManager::GetUndoDescInternal:  undoable operation missing "
