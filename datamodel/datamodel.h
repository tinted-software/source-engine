...
struct FileElementSet_t {
  FileElementSet_t(UtlSymId_t filename = UTL_INVAL_SYMBOL,
      ...
      : m_filename(filename), m_format(format),
        m_hRoot(DMELEMENT_HANDLE_INVALID), m_bLoaded(true), m_nElements(0) {}
  FileElementSet_t(const FileElementSet_t &that)
      : m_filename(that.m_filename), m_format(that.m_format),
        m_hRoot(DMELEMENT_HANDLE_INVALID), m_bLoaded(that.m_bLoaded),
...
public:
  ...
  FindLegacyUpdater(const char *pLegacyFormatName) const;
  ...
  virtual bool SaveToFile(char const *pFileName, char const *pPathID,
                          ...
                          CDmElement *pRoot);
  virtual DmFileId_t
  RestoreFromFile(char const *pFileName, char const *pPathID,
  ...
  virtual void
  SetKeyValuesElementCallback(IElementForKeyValueCallback *pCallbackInterface);
  ...
  virtual void Redo();
  virtual void
  TraceUndo(bool state); // if true, undo records spew as they are added
  ...
  FirstAttributeReferencingElement(DmElementHandle_t hElement);
  virtual DmAttributeReferenceIterator_t
  NextAttributeReferencingElement(DmAttributeReferenceIterator_t hAttrIter);
