...
static AttributeRemap_t s_pAttributeRemap[] = {
    ...
    {"name", "_name"},
    {"id",
     "_id"}, // FIXME - remove this once we've made id no longer be an attribute
    {NULL, NULL}};
...
bool CDmSerializerKeyValues::SerializeAttributes(CUtlBuffer &buf,
  ...
  for (int i = nAttributes - 1; i >= 0; --i) {
    ...
    case AT_ELEMENT_ARRAY:
      Warning("KeyValues: Can't serialize attribute of type %s into KeyValues "
              "files!\n",
...
//-----------------------------------------------------------------------------
DmElementHandle_t
CDmSerializerKeyValues::CreateDmElement(const char *pElementType,
                                        const char *pElementName) {
...
//-----------------------------------------------------------------------------
DmAttributeType_t
CDmSerializerKeyValues::DetermineAttributeType(KeyValues *pKeyValues) {
...
void CDmSerializerKeyValues::UnserializeAttribute(CDmElement *pElement,
  ...
  if (type == AT_UNKNOWN) {
    ...
    Warning("Dm Unserialize: Attempted to read an attribute (\"%s\") of an "
            "inappropriate type!\n",
            pLowerName.Get());
    return;
  }
...
//-----------------------------------------------------------------------------
DmElementHandle_t
CDmSerializerKeyValues::UnserializeElement(KeyValues *pKeyValues,
                                           int iNestingLevel) {
...
//-----------------------------------------------------------------------------
CDmElement *
CDmSerializerKeyValues::UnserializeFromKeyValues(KeyValues *pKeyValues) {
