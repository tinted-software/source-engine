...
void DmElementReference_t::AddAttribute(CDmAttribute *pAttribute) {
  if (m_attributes.m_hAttribute != DMATTRIBUTE_HANDLE_INVALID) {
    DmAttributeList_t *pLink =
        new DmAttributeList_t; // TODO - create a fixed size allocator for these
...
CDmElement::CDmElement(DmElementHandle_t handle, const char *pElementType,
                       ...
                       DmFileId_t fileid)
    : m_ref(handle), m_Type(g_pDataModel->GetSymbol(pElementType)),
      m_fileId(fileid), m_pAttributes(NULL), m_bDirty(false),
      m_bBeingUnserialized(false), m_bIsAcessible(true) {
  MEM_ALLOC_CREDIT();
...
public:
  CUndoAttributeRemove(CDmElement *pElement, CDmAttribute *pOldAttribute,
      ...
      : BaseClass("CUndoAttributeRemove"), m_pElement(pElement),
        m_pOldAttribute(pOldAttribute), m_symAttribute(attributeName),
        m_Type(type) {
...
public:
  CUndoAttributeAdd(CDmElement *pElement, const char *attributeName)
      : BaseClass("CUndoAttributeAdd"), m_hElement(pElement->GetHandle()),
        m_symAttribute(attributeName), m_pAttribute(NULL),
        m_bHoldingPtr(false) {
    Assert(pElement && pElement->GetFileId() != DMFILEID_INVALID);
  }
