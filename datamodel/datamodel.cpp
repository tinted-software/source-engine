          ...
          // and one of its entries is invalid (0xffff)
{
  return m_SymbolTable
      .GetNumStrings(); // this is only useful because symbols are never removed
}
...
void CDataModel::AddLegacyUpdater(IDmLegacyUpdater *pUpdater) {
  ...
  if (FindLegacyUpdater(pUpdater->GetName())) {
    Warning("Attempted to add two legacy updaters with the same file format "
            "(%s)!\n",
            pUpdater->GetName());
    return;
  }
...
void CDataModel::AddFormatUpdater(IDmFormatUpdater *pUpdater) {
  ...
  if (FindFormatUpdater(pUpdater->GetName())) {
    Warning("Attempted to add two format updaters with the same file format "
            "(%s)!\n",
            pUpdater->GetName());
    return;
  }
...
IDmLegacyUpdater *
CDataModel::FindLegacyUpdater(const char *pLegacyFormatName) const {
  ...
  if (pSerializer->IsBinaryFormat()) {
    if (outBuf.IsText()) {
      if (!outBuf.ContainsCRLF()) {
        Warning("Serialize: Format %s expects to be written to a binary "
                "format, but the buffer is a text-format buffer\n",
                pFormatName);
        return false;
      }
...
const char *
CDataModel::GetEncodingFromLegacyFormat(const char *pLegacyFormatName) const {
...
bool CDataModel::Unserialize(CUtlBuffer &inBuf, const char *pEncodingName,
  ...
  bool bIsCurrentVersion = true; // for formats that don't store a format, files
                                 // are currently always at the current version
...
DmElementHandle_t
CDataModel::FindOrCreateElementHandle(const DmObjectId_t &id) {
...
CDataModel::FindElementReference(DmElementHandle_t hElement,
...
void CDataModel::OnElementReferenceAdded(DmElementHandle_t hElement,
  ...
  //GetElementName( hElement ), 		pAttribute->GetOwner()->GetHandle(),
  //pAttribute->GetName() );
...
void CDataModel::OnElementReferenceAdded(DmElementHandle_t hElement,
  ...
  //handle\n", 		GetString( GetElementType( hElement ) ), hElement,
  //GetElementName( hElement ), 		bRefCount ? "refcounted" : "weak" );
...
void CDataModel::OnElementReferenceRemoved(DmElementHandle_t hElement,
  ...
  //'%s'\n", 		GetString( GetElementType( hElement ) ), hElement, GetElementName(
  //hElement ), 		pAttribute->GetOwner()->GetHandle(), pAttribute->GetName() );
...
DmAttributeReferenceIterator_t
CDataModel::FirstAttributeReferencingElement(DmElementHandle_t hElement) {
...
CDmAttribute *
CDataModel::GetAttribute(DmAttributeReferenceIterator_t hAttrIter) {
  DmAttributeList_t *pList = (DmAttributeList_t *)hAttrIter;
  if (!pList)
    return NULL;
...
CDmElement *CDataModel::CreateElement(const DmElementReference_t &ref,
  ...
  //pElementType, ref.m_hElement, pElementName ? pElementName : "", GetFileName(
  //fileid ), m_elementIds.Count() );
...
void CDataModel::DestroyElement(DmElementHandle_t hElement) {
  DmPhase_t phase = g_pDmElementFramework->GetPhase();
  if (phase != PH_EDIT &&
      phase != PH_EDIT_APPLY) // need to allow edit_apply to delete elements, so
...
//-----------------------------------------------------------------------------
const char *
CDataModel::GetAttributeNameForType(DmAttributeType_t attType) const {
...
//-----------------------------------------------------------------------------
DmAttributeHandle_t
CDataModel::AcquireAttributeHandle(CDmAttribute *pAttribute) {
