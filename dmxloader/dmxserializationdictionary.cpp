...
CDmxSerializationDictionary::Find(CDmxElement *pElement) {
...
DmxSerializationHandle_t
CDmxSerializationDictionary::NextRootElement(DmxSerializationHandle_t h) const {
  ++h;
  int nCount = m_Dict.Count();
  for (; h < nCount; ++h) {
    if (m_Dict[h].m_bRoot)
...
CDmxElement *
CDmxSerializationDictionary::GetRootElement(DmxSerializationHandle_t h) {
