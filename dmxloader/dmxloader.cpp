...
bool CDmxSerializer::Unserialize(CUtlBuffer &buf, int nEncodingVersion,
  ...
  if (nElementCount < 0 || (bReadStringTable && !stringTable)) {
    // Invalid file. Non-empty files with a string table need at least one to
...
#else
    // sscanf considered harmful. We don't have POSIX 2008 support on OS X and
    // "C11 Annex K" is optional... (optional specs considered useful)
    char pTmpEncodingName[sizeof(header)] = {0};
    char pTmpFormatName[sizeof(header)] = {0};
    int nAssigned =
        sscanf(header, "encoding %s %d format %s %d\n", pTmpEncodingName,
               &nEncodingVersion, pTmpFormatName, &nFormatVersion);
    bOk = (V_strlen(pTmpEncodingName) < nEncodingNameLen) &&
          (V_strlen(pTmpFormatName) < nFormatNameLen);
...
bool UnserializeDMX(CUtlBuffer &buf, CDmxElement **ppRoot,
  ...
  *ppRoot = NULL;
  ...
  {
    Warning("reading file '%s' of legacy format '%s' - dmxconvert this file to "
            "a newer format!\n",
