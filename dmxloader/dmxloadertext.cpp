...
//-----------------------------------------------------------------------------
DmxElementDictHandle_t
CDmxElementDictionary::InsertElement(CDmxElement *pElement) {
...
//-----------------------------------------------------------------------------
DmxElementDictHandle_t
CDmxElementDictionary::FindElement(CDmxElement *pElement) {
...
//-----------------------------------------------------------------------------
DmxElementDictHandle_t
CDmxElementDictionary::FindElement(const DmObjectId_t &objectId) {
  int nCount = m_Dict.Count();
  for (int i = 0; i < nCount; ++i) {
...
//-----------------------------------------------------------------------------
CDmxSerializerKeyValues2::TokenType_t
CDmxSerializerKeyValues2::ReadToken(CUtlBuffer &buf, CUtlBuffer &token) {
...
//-----------------------------------------------------------------------------
DmxElementDictHandle_t
CDmxSerializerKeyValues2::CreateDmxElement(const char *pElementType) {
...
bool CDmxSerializerKeyValues2::UnserializeAttribute(
  ...
  if (token != TOKEN_DELIMITED_STRING) {
    g_KeyValues2ErrorStack.ReportError("Expecting quoted attribute value for "
                                       "attribute \"%s\", didn't find one!",
                                       pAttributeName);
    return false;
  }
  ...
  *pHandle = ELEMENT_DICT_HANDLE_INVALID;
  ...
  while (buf.IsValid()) {
    ...
    default:
      if (nAttrType >= AT_FIRST_ARRAY_TYPE) {
        bOk =
            UnserializeArrayAttribute(buf, hElement, pAttributeName, nAttrType);
      } else {
