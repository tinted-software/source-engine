...
const CDmxAttribute *
CDmxElement::GetAttribute(const char *pAttributeName) const {
  int idx = FindAttribute(pAttributeName);
  if (idx >= 0)
    return m_Attributes[idx];
  return NULL;
...
void CDmxElement::UnpackIntoStructure(
    void *pData, size_t DestSizeInBytes,
  ...
  for (; pUnpack->m_AttributeType != AT_UNKNOWN; ++pUnpack) {
    ...
    if (pUnpack->m_AttributeType != pAttribute->GetType()) {
      Warning("CDmxElement::UnpackIntoStructure: Mismatched attribute type in "
              "attribute \"%s\"!\n",
    ...
    if (pAttribute->GetType() == AT_STRING) {
      if (pDest + pUnpack->m_nSize > pDataEnd) {
        Warning("ERROR Memory corruption: CDmxElement::UnpackIntoStructure "
...
//-----------------------------------------------------------------------------
void CDmxElement::AddAttributesFromStructure_Internal(
  ...
  for (; pUnpack->m_AttributeType != AT_UNKNOWN; ++pUnpack) {
    ...
    if (HasAttribute(pUnpack->m_pAttributeName)) {
      AssertMsg(0,
                "CDmxElement::AddAttributesFromStructure: Attribute %s already "
                "exists!\n",
                pUnpack->m_pAttributeName);
      continue;
    }
