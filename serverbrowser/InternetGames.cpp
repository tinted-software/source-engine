...
//-----------------------------------------------------------------------------
// Purpose: Constructor
//			NOTE:	m_Servers can not use more than 96 sockets, else
//it will 					cause internet explorer to Stop working under win98 SE!
//-----------------------------------------------------------------------------
CInternetGames::CInternetGames(vgui::Panel *parent, const char *panelName,
                               EPageType eType)
  ...
  if (kv->LoadFromFile(g_pFullFileSystem, "servers/Regions.vdf", NULL)) {
    ...
    for (KeyValues *srv = kv->GetFirstSubKey(); srv != NULL;
         srv = srv->GetNextKey()) {
    ...
    Assert(!("Could not load file servers/Regions.vdf; server browser will not "
             "function."));
  }
  kv->deleteThis();
...
void CInternetGames::PerformLayout() {
  if (!m_bOfflineMode && m_bRequireUpdate &&
...
// Purpose:
//-----------------------------------------------------------------------------
/*void CInternetGames::ServerFailedToRespond( HServerListRequest hReq, int
iServer )
{
        m_bDirty = true;
...
steamapicontext->SteamMatchmakingServers()->GetServerDetails( hReq, iServer );
        ...
        else
        {
                ...
                // we've never had a good response from this server, remove it
from the list m_iServerRefreshCount++;
        }
}*/
...
void CInternetGames::OnOpenContextMenu(int itemID) {
  ...
  CServerContextMenu *menu =
      ServerBrowserDialog().GetContextMenu(GetActiveList());
...
bool CInternetGames::CheckTagFilter(gameserveritem_t &server) {
  // Servers without tags go in the official games, servers with tags go in
  // custom games
