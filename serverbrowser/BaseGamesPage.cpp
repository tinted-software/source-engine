...
ConVar sb_mod_suggested_maxplayers("sb_mod_suggested_maxplayers", "0",
                                   FCVAR_HIDDEN);
...
#endif
    0,
    "Hides servers running incompatible versions from the server browser.  "
    "(Internet tab only.)");
...
CBaseGamesPage::CBaseGamesPage(vgui::Panel *parent, const char *name,
                               EPageType eType, const char *pCustomResFilename)
    : PropertyPage(parent, name),
      m_CallbackFavoritesMsg(this, &CBaseGamesPage::OnFavoritesMsg),
      m_hRequest(NULL), m_pCustomResFilename(pCustomResFilename) {
  SetSize(624, 278);
  m_szGameFilter[0] = 0;
  m_szMapFilter[0] = 0;
  ...
  // Increment this number if columns are added / removed or some other change
  // is done that requires tossing out old saved user configs.
  m_pGameList->m_nUserConfigFileVersion = 2;

  // Add the column headers
  m_pGameList->AddColumnHeader(
      k_nColumn_Password, "Password", "#ServerBrowser_Password", 16,
      ListPanel::COLUMN_FIXEDSIZE | ListPanel::COLUMN_IMAGE);
  m_pGameList->AddColumnHeader(
      k_nColumn_Secure, "Secure", "#ServerBrowser_Secure", 16,
      ListPanel::COLUMN_FIXEDSIZE | ListPanel::COLUMN_IMAGE);
  ...
  m_pGameList->AddColumnHeader(
      k_nColumn_Replay, "Replay", "#ServerBrowser_Replay", nReplayWidth,
      ListPanel::COLUMN_FIXEDSIZE | ListPanel::COLUMN_IMAGE);
  m_pGameList->AddColumnHeader(
      k_nColumn_Name, "Name", "#ServerBrowser_Servers", 50,
      ListPanel::COLUMN_RESIZEWITHWINDOW | ListPanel::COLUMN_UNHIDABLE);
  m_pGameList->AddColumnHeader(k_nColumn_IPAddr, "IPAddr",
                               "#ServerBrowser_IPAddress", 64,
                               ListPanel::COLUMN_HIDDEN);
  m_pGameList->AddColumnHeader(k_nColumn_GameDesc, "GameDesc",
                               "#ServerBrowser_Game", 112,
                               112, // minwidth
                               300, // maxwidth
                               0    // flags
  );
  m_pGameList->AddColumnHeader(k_nColumn_Players, "Players",
                               "#ServerBrowser_Players", 80,
                               ListPanel::COLUMN_FIXEDSIZE);
  m_pGameList->AddColumnHeader(k_nColumn_Bots, "Bots", "#ServerBrowser_Bots",
                               60, ListPanel::COLUMN_FIXEDSIZE);
  m_pGameList->AddColumnHeader(k_nColumn_Map, "Map", "#ServerBrowser_Map", 90,
                               90,  // minwidth
                               300, // maxwidth
                               0    // flags
  );
  m_pGameList->AddColumnHeader(k_nColumn_Ping, "Ping", "#ServerBrowser_Latency",
                               55, ListPanel::COLUMN_RESIZEWITHWINDOW);

  m_pGameList->SetColumnHeaderTooltip(k_nColumn_Password,
                                      "#ServerBrowser_PasswordColumn_Tooltip");
  m_pGameList->SetColumnHeaderTooltip(k_nColumn_Bots,
                                      "#ServerBrowser_BotColumn_Tooltip");
  m_pGameList->SetColumnHeaderTooltip(k_nColumn_Secure,
                                      "#ServerBrowser_SecureColumn_Tooltip");

  if (bRunningTF2) {
    m_pGameList->SetColumnHeaderTooltip(k_nColumn_Replay,
                                        "#ServerBrowser_ReplayColumn_Tooltip");
  }
  ...
  // In TF2, fill out the max player count so that we sort all >24 player
  // servers below the rest.
  if (bRunningTF2) {
...
void CBaseGamesPage::PerformLayout() {
  ...
  if (SupportsItem(IGameList::ADDSERVER)) {
    // 		m_pFilterString->SetWide( 90 ); // shrink the filter label to
    // fix the add current server button
...
void CBaseGamesPage::ApplySchemeSettings(IScheme *pScheme) {
  ...
  m_nImageIndexSecureVacBanned = imageList->AddImage(
      scheme()->GetImage("servers/icon_secure_deny", false));
  m_nImageIndexReplay =
  ...
  // int botColumnImage =
  // imageList->AddImage(scheme()->GetImage("servers/icon_bots_column", false));
  int secureColumnImage = imageList->AddImage(
      scheme()->GetImage("servers/icon_robotron_column", false));
  int replayColumnImage = imageList->AddImage(
      scheme()->GetImage("servers/icon_replay_column", false));
...
int ServerQualitySort(const serverqualitysort_t *pSQ1,
                      const serverqualitysort_t *pSQ2) {
...
void CBaseGamesPage::PrepareQuickListMap(newgameserver_t *server, int iListID) {
  ...
  const char *pszFriendlyGameTypeName =
      ServerBrowser().GetMapFriendlyNameAndGameType(szMapName, szFriendlyName,
  ...
  if (m_pQuickList) {
    ...
    CQuickListPanel *pQuickListPanel =
        new CQuickListPanel(m_pQuickList, "QuickListPanel");
...
newgameserver_t *CBaseGamesPage::GetServer(unsigned int serverID) {
  if (serverID >= m_serversInfo.Count())
    return NULL;
...
void CBaseGamesPage::CreateFilters() {
  ...
  if (ServerBrowser().IsWorkshopEnabled()) {
    ...
    m_pWorkshopFilter->AddItem("#ServerBrowser_WorkshopFilterWorkshopOnly",
                               NULL);
...
void CBaseGamesPage::LoadFilterSettings() {
  ...
  m_pQuickListCheckButton->SetSelected(
      filter->GetInt("QuickList", IsAndroid()));
...
{
        int nIndex = -1; // start at -1 and work backwards to find the next free
slot for this adhoc query while ( m_mapServers.Find( nIndex ) !=
m_mapServers.InvalidIndex() ) nIndex--; ServerResponded( nIndex, &server );
}*/
...
steamapicontext->SteamMatchmakingServers()->GetServerDetails( hReq, iServer );
        if ( !pServerItem )
        {
        ...
        // FIXME(johns): This is a workaround for a steam bug, where it
inproperly reads signed bytes out of the
        //               message. Once the upstream fix makes it into our
SteamSDK, this block can be removed. pServerItem->m_nPlayers    =
(uint8)(int8)pServerItem->m_nPlayers; pServerItem->m_nBotPlayers =
(uint8)(int8)pServerItem->m_nBotPlayers; pServerItem->m_nMaxPlayers =
(uint8)(int8)pServerItem->m_nMaxPlayers;
...
void CBaseGamesPage::OnButtonToggled(Panel *panel, int state) {
  ...
  if (panel == m_pNoFullServersFilterCheck ||
...
void CBaseGamesPage::UpdateDerivedLayouts(void) {
  ...
  if (m_pCustomResFilename) {
    ...
    if (m_pFilter->IsSelected() && !m_pQuickListCheckButton->IsSelected()) {
      // drop down
      V_strncpy(rgchControlSettings, "servers/InternetGamesPage_Filters.res",
                sizeof(rgchControlSettings));
    } else {
      // hide filter area
      V_strncpy(rgchControlSettings, "servers/InternetGamesPage.res",
                sizeof(rgchControlSettings));
    }
  }
...
void CBaseGamesPage::UpdateStatus() {
  if (m_pGameList->GetItemCount() > 1) {
    ...
    _snwprintf(msg, Q_ARRAYSIZE(msg), L"%S( %d )",
               g_pVGuiLocalize->Find("#ServerBrowser_Servers"),
               m_pGameList->GetItemCount());
    m_pGameList->SetColumnHeaderText(k_nColumn_Name, msg);
  } else {
    m_pGameList->SetColumnHeaderText(
        k_nColumn_Name, g_pVGuiLocalize->Find("#ServerBrowser_Servers"));
  }
}
...
void CBaseGamesPage::UpdateFilterSettings() {
  ...
  if (sb_filter_incompatible_versions.GetBool() && g_pRunGameEngine != NULL) {
    ...
    const char k_VersionFromP4[] =
        "2000"; // magic version string we use when we're running from P4
    if (pszVersion && *pszVersion &&
        (V_strcmp(pszVersion, k_VersionFromP4) != 0)) {
      m_vecServerFilters.AddToTail(
          MatchMakingKeyValuePair_t("version_match", pszVersion));
    }
  }

  // update master filter string text
  if (m_szGameFilter[0] &&
      m_iLimitToAppID.AppID() !=
...
void CBaseGamesPage::RecalculateFilterString() {
  ...
  if (m_szGameFilter[0]) {
    Q_UTF8ToUnicode(ModList().GetModNameForModDir(m_iLimitToAppID), tempUnicode,
                    iTempUnicodeSize);
  ...
  if (m_iMaxPlayerFilter) {
    ...
    wcscat(unicode,
           g_pVGuiLocalize->Find("#ServerBrowser_FilterDescMaxPlayers"));
    Q_UTF8ToUnicode(" <= ", tempUnicode, iTempUnicodeSize);
    wcscat(unicode, tempUnicode);
    Q_UTF8ToUnicode(tmpBuf, tempUnicode, iTempUnicodeSize);
    wcscat(unicode, tempUnicode);
...
bool CBaseGamesPage::CheckPrimaryFilters(newgameserver_t &server) {
  if (m_szGameFilter[0] && server.m_szGameDir[0] &&
...
void CBaseGamesPage::OnKeyCodePressed(vgui::KeyCode code) {
  if (code == KEY_XBUTTON_A || code == STEAMCONTROLLER_A) {
    m_pConnect->DoClick();
             ...
             code == STEAMCONTROLLER_X) {
    StartRefresh();
  } else if (m_pGameList->GetItemCount() > 0 &&
             (code == KEY_XBUTTON_UP || code == KEY_XSTICK1_UP ||
              code == KEY_XSTICK2_UP || code == STEAMCONTROLLER_DPAD_UP ||
              code == KEY_XBUTTON_DOWN || code == KEY_XSTICK1_DOWN ||
...
void CBaseGamesPage::OnAddToFavorites() {
  /*	if ( !steamapicontext->SteamMatchmakingServers() )
     ...
     steamapicontext->SteamMatchmakingServers()->GetServerDetails( m_hRequest,
     serverID ); if ( pServer )
                  {
...
void CBaseGamesPage::OnRefreshServer(int serverID) {
  ...
  for (int i = 0; i < m_pGameList->GetSelectedItemsCount(); i++) {
    ...
    // refresh this server
    steamapicontext->SteamMatchmakingServers()->RefreshServer(m_hRequest,
                                                              serverID);
  }
...
void CBaseGamesPage::StartRefresh() {
  ...
  /*	if ( m_hRequest )
          {
                  steamapicontext->SteamMatchmakingServers()->ReleaseRequest(
     m_hRequest ); m_hRequest = NULL;
          }*/
  switch (m_eMatchMakingType) {
    /*	case eFavoritesServer:
                    m_hRequest =
       steamapicontext->SteamMatchmakingServers()->RequestFavoritesServerList(
       GetFilterAppID().AppID(), &pFilters, nFilters, this ); break; case
       eHistoryServer: m_hRequest =
       steamapicontext->SteamMatchmakingServers()->RequestHistoryServerList(
       GetFilterAppID().AppID(), &pFilters, nFilters, this ); break;*/
  case eInternetServer:
    g_pServersInfo->RequestInternetServerList(COM_GetModDirectory(), this);
    // m_hRequest =
    // steamapicontext->SteamMatchmakingServers()->RequestInternetServerList(
    // GetFilterAppID().AppID(), &pFilters, nFilters, this );
    break;
    /*	case eSpectatorServer:
                    m_hRequest =
       steamapicontext->SteamMatchmakingServers()->RequestSpectatorServerList(
       GetFilterAppID().AppID(), &pFilters, nFilters, this ); break; case
       eLANServer: m_hRequest =
       steamapicontext->SteamMatchmakingServers()->RequestLANServerList(
       GetFilterAppID().AppID(), this ); break; default: Assert( !"Unknown
       server type" ); break;*/
  }
...
int CBaseGamesPage::GetSelectedServerID(KeyValues **pKV) {
  ...
  if (m_pQuickList->IsVisible() == true) {
    ...
    if (m_pQuickList->GetSelectedPanel()) {
      CQuickListPanel *pQuickPanel =
          dynamic_cast<CQuickListPanel *>(m_pQuickList->GetSelectedPanel());
...
//-----------------------------------------------------------------------------
CDialogServerWarning::CDialogServerWarning(vgui::Panel *parent,
                                           IGameList *gameList, int serverID)
    : Frame(parent, "DialogServerWarning") {
  m_pGameList = gameList;
  m_iServerID = serverID;

  m_pDontShowThisAgainCheckButton =
      new CheckButton(this, "DontShowThisAgainCheckbutton", "");
...
void CDialogServerWarning::OnButtonToggled(Panel *panel, int state) {
  ConVarRef sb_dontshow_maxplayer_warning("sb_dontshow_maxplayer_warning",
                                          true);
...
void CBaseGamesPage::OnBeginConnect() {
  ...
  /*	if ( sb_dontshow_maxplayer_warning.IsValid() )
          {
                  // If the server is above the suggested maxplayers, warn the
     player int iMaxP = sb_mod_suggested_maxplayers.GetInt(); if ( iMaxP && pKV
     && !sb_dontshow_maxplayer_warning.GetBool() )
                  {
                          int iMaxCount = pKV->GetInt( "MaxPlayerCount", 0 );
                          if ( iMaxCount > iMaxP )
                          {
                                  CDialogServerWarning *dlg = vgui::SETUP_PANEL(
     new CDialogServerWarning( this, this, serverID ) );
                                  ...
                                  _snwprintf( wszServerMaxPlayers,
     Q_ARRAYSIZE(wszServerMaxPlayers), L"%d", iMaxCount ); _snwprintf(
     wszDesignedMaxPlayers, Q_ARRAYSIZE(wszDesignedMaxPlayers), L"%d", iMaxP );
                                  Q_UTF8ToUnicode(
     ModList().GetModNameForModDir( m_iLimitToAppID ), wszGameName,
     Q_ARRAYSIZE(wszGameName) ); g_pVGuiLocalize->ConstructString( wszWarning,
     sizeof( wszWarning ), g_pVGuiLocalize->Find(
     "#ServerBrowser_ServerWarning_MaxPlayers"), 4, wszServerMaxPlayers,
     wszGameName, wszDesignedMaxPlayers, wszDesignedMaxPlayers );
...
const char *CBaseGamesPage::GetConnectCode() {
  ...
  //
  // E.g.: "connect serverbrowser"   (This command primarily exists so i can
  // grep the code....)
...
void CBaseGamesPage::ServerResponded(newgameserver_t &server) {
  ...
  int nAdjustedForBotsPlayers =
      max(0, pServerItem->m_nPlayers - pServerItem->m_nBotPlayers);

  char buf[32];
  Q_snprintf(buf, sizeof(buf), "%d / %d", nAdjustedForBotsPlayers,
             pServerItem->m_nMaxPlayers);
  kv->SetString("Players", buf);
...
void CBaseGamesPage::OnFavoritesMsg(FavoritesListChanged_t *pFavListChanged) {
  if (!pFavListChanged->m_nIP) // a zero for IP means the whole list was
                               // reloaded and we need to reload
  {
    switch (m_eMatchMakingType) {
    case eInternetServer:
    case eLANServer:
    ...
    case eHistoryServer:
      ...
      if (reinterpret_cast<PropertySheet *>(GetParent())->GetActivePage() ==
              this &&
  ...
  case eHistoryServer: {
    int iIPServer = m_mapServerIP.Find(
        netadr_t(pFavListChanged->m_nIP, pFavListChanged->m_nConnPort));
    if (iIPServer == m_mapServerIP.InvalidIndex()) {
      if (pFavListChanged->m_bAdd) {

        // TODO(nillerusr): implement this
        //	if ( steamapicontext->SteamMatchmakingServers() )
        //		steamapicontext->SteamMatchmakingServers()->PingServer(
