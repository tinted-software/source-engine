...
extern "C" {
DLL_EXPORT bool JoiningSecureServerCall() { return true; }
}

//-----------------------------------------------------------------------------
// Purpose: Comparison function used in query redblack tree
//-----------------------------------------------------------------------------
bool QueryLessFunc(const struct challenge_s &item1,
                   const struct challenge_s &item2) {
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CDialogGameInfo::CDialogGameInfo(vgui::Panel *parent, int serverIP,
                                 int queryPort, unsigned short connectionPort,
                                 const char *pszConnectCode)
    : Frame(parent, "DialogGameInfo"),
      m_CallbackPersonaStateChange(this,
                                   &CDialogGameInfo::OnPersonaStateChange),
  ...
  m_pAutoRetry =
      new ToggleButton(this, "AutoRetry", "#ServerBrowser_AutoRetry");
  m_pAutoRetry->AddActionSignalTarget(this);

  m_pAutoRetryAlert = new RadioButton(this, "AutoRetryAlert",
                                      "#ServerBrowser_AlertMeWhenSlotOpens");
  m_pAutoRetryJoin = new RadioButton(this, "AutoRetryJoin",
                                     "#ServerBrowser_JoinWhenSlotOpens");
  m_pPlayerList = new ListPanel(this, "PlayerList");
  m_pPlayerList->AddColumnHeader(0, "PlayerName", "#ServerBrowser_PlayerName",
                                 156);
...
CDialogGameInfo::~CDialogGameInfo() {
  ...
  if (m_hPlayersQuery != HSERVERQUERY_INVALID)
    steamapicontext->SteamMatchmakingServers()->CancelServerQuery(
        m_hPlayersQuery);
}
...
void CDialogGameInfo::SendPlayerQuery(uint32 unIP, uint16 usQueryPort) {
  ...
  if (m_hPlayersQuery != HSERVERQUERY_INVALID)
    steamapicontext->SteamMatchmakingServers()->CancelServerQuery(
        m_hPlayersQuery);
  m_hPlayersQuery = steamapicontext->SteamMatchmakingServers()->PlayerDetails(
      unIP, usQueryPort, this);
...
//-----------------------------------------------------------------------------
void CDialogGameInfo::ChangeGame(int serverIP, int queryPort,
...
//-----------------------------------------------------------------------------
void CDialogGameInfo::OnPersonaStateChange(
    PersonaStateChange_t *pPersonaStateChange) {
...
void CDialogGameInfo::SetFriend(uint64 ulSteamIDFriend) {
  ...
  SetDialogVariable(
      "game",
      steamapicontext->SteamFriends()->GetFriendPersonaName(ulSteamIDFriend));
  SetDialogVariable(
      "friend",
      steamapicontext->SteamFriends()->GetFriendPersonaName(ulSteamIDFriend));
  ...
  if (steamapicontext->SteamFriends()->GetFriendGamePlayed(ulSteamIDFriend,
                                                           &friendGameInfo)) {
    ...
    ChangeGame(friendGameInfo.m_unGameIP, usConnPort,
               friendGameInfo.m_usGamePort);
  }
}
...
void CDialogGameInfo::PerformLayout() {
  ...
  if (m_Server.m_nMaxPlayers > 0) {
    Q_snprintf(buf, sizeof(buf), "%d / %d", m_Server.m_nPlayers,
               m_Server.m_nMaxPlayers);
  } else {
    buf[0] = 0;
  }
  ...
  SetControlString("ServerIPText",
                   m_Server.m_NetAdr.GetConnectionAddressString());
  ...
  if (m_Server.m_bHadSuccessfulResponse &&
      !(m_Server.m_nPlayers + m_Server.m_nBotPlayers)) {
...
void CDialogGameInfo::OnConnectToGame(int ip, int port) {
  ...
  if (m_Server.m_NetAdr.GetIP() == (uint32)ip &&
      m_Server.m_NetAdr.GetConnectionPort() == (uint16)port) {
    // close this dialog
    Close();
  }
}
...
void CDialogGameInfo::RequestInfo() {
  ...
  if (m_iRequestRetry == 0) {
    ...
    if (m_hPingQuery != HSERVERQUERY_INVALID)
      steamapicontext->SteamMatchmakingServers()->CancelServerQuery(
          m_hPingQuery);
    m_hPingQuery = steamapicontext->SteamMatchmakingServers()->PingServer(
        m_Server.m_NetAdr.GetIP(), m_Server.m_NetAdr.GetQueryPort(), this);
  }
}
...
void CDialogGameInfo::ServerResponded(gameserveritem_t &server) {
  ...
  // FIXME(johns): This is a workaround for a steam bug, where it inproperly
  // reads signed bytes out of the
  //               message. Once the upstream fix makes it into our SteamSDK,
  ...
  // Preserve our connection port, since we may be querying the sourceTV port
  // but getting a response for the real server. This is a limitation of the
  // steam Matchmaking API where it doesn't properly send us a sourcetv response
  // but instead the main server's response (unless we're connecting to a proxy,
  // THEN we get the sourcetv response!)
...
//
// TODO it would be nice to push this logic into the IRunGameEngine interface;
// that way we could ask the engine itself to construct arguments in ways that
// fit. Might be worth the effort as we start to add more engines.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Constructs game options to use when running a game to connect to a
// server
//-----------------------------------------------------------------------------
void CDialogGameInfo::ConstructConnectArgs(char *pchOptions, int cchOptions,
                                           const gameserveritem_t &server) {
  Q_snprintf(pchOptions, cchOptions, " +connect %s",
             server.m_NetAdr.GetConnectionAddressString());
  if (m_szPassword[0]) {
...
void CDialogGameInfo::ConnectToServer() {
  ...
  if (g_pRunGameEngine->IsRunning()) {
    ...
    if ((m_Server.m_bSecure && JoiningSecureServerCall()) ||
        !m_Server.m_bSecure) {
      switch (g_pRunGameEngine->RunEngine(m_Server.m_nAppID, gameDir,
                                          connectArgs)) {
      case IRunGameEngine::k_ERunResultModNotInstalled: {
        MessageBox *dlg = new MessageBox("#ServerBrowser_GameInfoTitle",
                                         "#ServerBrowser_ModNotInstalled");
        dlg->DoModal();
        SetVisible(false);
        return;
      } break;
...
//-----------------------------------------------------------------------------
void CDialogGameInfo::AddPlayerToList(const char *playerName, int score,
...
//-----------------------------------------------------------------------------
int CDialogGameInfo::PlayerTimeColumnSortFunc(ListPanel *pPanel,
                                              const ListPanelItem &p1,
                                              const ListPanelItem &p2) {
