...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CServerBrowser, IServerBrowser,
                                  SERVERBROWSER_INTERFACE_VERSION,
                                  g_ServerBrowserSingleton);
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(
    CServerBrowser, IVGuiModule, "VGuiModuleServerBrowser001",
...
void CServerBrowser::CreateDialog() {
  if (!m_hInternetDlg.Get()) {
    m_hInternetDlg =
        new CServerBrowserDialog(NULL); // SetParent() call below fills this in
...
//-----------------------------------------------------------------------------
bool CServerBrowser::Initialize(CreateInterfaceFn *factorylist,
                                int factoryCount) {
  ...
  for (int i = 0; i < factoryCount; ++i) {
    if (!g_pEngineReplay)
      g_pEngineReplay = (IEngineReplay *)factorylist[i](
          ENGINE_REPLAY_INTERFACE_VERSION, NULL);
    if (!g_pServersInfo)
      g_pServersInfo =
          (IServersInfo *)factorylist[i](SERVERLIST_INTERFACE_VERSION, NULL);
  }
  ...
  for (int i = 0; i < factoryCount; i++) {
    if (!g_pRunGameEngine) {
      g_pRunGameEngine = (IRunGameEngine *)(factorylist[i])(
          RUNGAMEENGINE_INTERFACE_VERSION, NULL);
    }
  }
...
#else
  if (!vgui::VGui_InitInterfacesList("ServerBrowser", factorylist,
                                     factoryCount))
#endif
    return false;
...
//-----------------------------------------------------------------------------
bool CServerBrowser::PostInitialize(CreateInterfaceFn *modules,
                                    int factoryCount) {
  // find the interfaces we need
  for (int i = 0; i < factoryCount; i++) {
    if (!g_pRunGameEngine) {
      g_pRunGameEngine =
          (IRunGameEngine *)(modules[i])(RUNGAMEENGINE_INTERFACE_VERSION, NULL);
    }
  }
...
//-----------------------------------------------------------------------------
// Purpose: Marks that the tool/game loading us intends to feed us workshop
// information
//-----------------------------------------------------------------------------
...
bool CServerBrowser::Activate() {
  ...
  if (firstTimeOpening) {
    m_hInternetDlg
        ->LoadUserData(); // reload the user data the first time the dialog is
                          // made visible, helps with the lag between module
                          // load and steamui getting Deactivate() call
...
//-----------------------------------------------------------------------------
// Purpose: opens a game info dialog to watch the specified server; associated
// with the friend 'userName'
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: joins a specified game - game info dialog will only be opened if the
// server is fully or passworded
//-----------------------------------------------------------------------------
bool CServerBrowser::JoinGame(uint64 ulSteamIDFriend,
                              const char *pszConnectCode) {
  /*	if ( OpenGameInfoDialog( ulSteamIDFriend, pszConnectCode ) )
          {
                  CDialogGameInfo *pDialogGameInfo =
...
#define GAMETYPES_FILE "servers/ServerBrowserGameTypes.txt"
  ...
  for (KeyValues *pData = kv->GetFirstSubKey(); pData != NULL;
...
//-----------------------------------------------------------------------------
const char *CServerBrowser::GetMapFriendlyNameAndGameType(
    const char *pszMapName, char *szFriendlyMapName, int cchFriendlyName) {
  ...
  if (pszFinal) {
    // truncate the _final (or _final1) part of the filename if it's at the end
    // of the name
