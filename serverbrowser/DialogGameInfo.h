...
public:
  CDialogGameInfo(vgui::Panel *parent, int serverIP, int queryPort,
  ...
  virtual void AddPlayerToList(const char *playerName, int score,
...
protected:
  ...
  MESSAGE_FUNC_CHARPTR(OnJoinServerWithPassword, "JoinServerWithPassword",
                       password);
...
private:
  STEAM_CALLBACK(CDialogGameInfo, OnPersonaStateChange, PersonaStateChange_t,
                 m_CallbackPersonaStateChange);

  long m_iRequestRetry; // time at which to retry the request
  static int PlayerTimeColumnSortFunc(vgui::ListPanel *pPanel,
  ...
  void ConstructConnectArgs(char *pchOptions, int cchOptions,
