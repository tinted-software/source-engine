...
//-----------------------------------------------------------------------------
CDialogAddServer::CDialogAddServer(vgui::Panel *parent, IGameList *gameList)
    : Frame(parent, "DialogAddServer") {
  ...
  m_pDiscoveredGames->AddColumnHeader(
  ...
  m_pDiscoveredGames->AddColumnHeader(
      3, "Replay", "#ServerBrowser_Replay", nReplayWidth,
      ListPanel::COLUMN_FIXEDSIZE | ListPanel::COLUMN_IMAGE);
  m_pDiscoveredGames->AddColumnHeader(4, "Name", "#ServerBrowser_Servers", 20,
                                      ListPanel::COLUMN_RESIZEWITHWINDOW |
                                          ListPanel::COLUMN_UNHIDABLE);
  m_pDiscoveredGames->AddColumnHeader(5, "IPAddr", "#ServerBrowser_IPAddress",
                                      60, ListPanel::COLUMN_HIDDEN);
  m_pDiscoveredGames->AddColumnHeader(6, "GameDesc", "#ServerBrowser_Game",
                                      150);
  m_pDiscoveredGames->AddColumnHeader(7, "Players", "#ServerBrowser_Players",
                                      60);
  ...
  m_pDiscoveredGames->SetColumnHeaderTooltip(
      0, "#ServerBrowser_PasswordColumn_Tooltip");
  m_pDiscoveredGames->SetColumnHeaderTooltip(
      1, "#ServerBrowser_BotColumn_Tooltip");
  m_pDiscoveredGames->SetColumnHeaderTooltip(
      2, "#ServerBrowser_SecureColumn_Tooltip");

  if (bGameSupportsReplay) {
    m_pDiscoveredGames->SetColumnHeaderTooltip(
        3, "#ServerBrowser_ReplayColumn_Tooltip");
  }
  ...
  m_pAddSelectedServerButton =
      new vgui::Button(this, "SelectedOKButton", "", this, "addselected");

  m_pTabPanel->AddPage(m_pDiscoveredGames, "#ServerBrowser_Servers");

  LoadControlSettings("Servers/DialogAddServer.res");

  // Setup the buttons. We leave them disabled until there is text in the
  // textbox.
...
void CDialogAddServer::OnCommand(const char *command) {
  if (Q_stricmp(command, "OK") == 0) {
    ...
    if (m_pDiscoveredGames->GetSelectedItemsCount()) {
      // get the server
      int serverID = m_pDiscoveredGames->GetItemUserData(
          m_pDiscoveredGames->GetSelectedItem(0));
      FinishAddServer(m_Servers[serverID]);
      m_pDiscoveredGames->RemoveItem(m_pDiscoveredGames->GetSelectedItem(
          0)); // as we add to favs remove from the list
...
//-----------------------------------------------------------------------------
// Purpose: Handles the OK button being pressed; adds the server to the game
// list
//-----------------------------------------------------------------------------
void CDialogAddServer::OnOK() {
  ...
  if (AllowInvalidIPs() || netaddr.IsValid()) {
    ...
    // We assume here that the query and connection ports are the same. This is
    // why it's much better if they click "Servers" and choose a server in
    // there.
    server.m_NetAdr = netaddr;
    FinishAddServer(server);
  } else {
    // could not parse the ip address, popup an error
    MessageBox *dlg = new MessageBox("#ServerBrowser_AddServerErrorTitle",
...
void CDialogAddServer::TestServers() {
  ...
  if (address[0] == 0) {
    Q_wcsncpy(wstr,
              g_pVGuiLocalize->Find("#ServerBrowser_ServersRespondingLocal"),
              sizeof(wstr));
  } else {
    ...
    g_pVGuiLocalize->ConstructString(
        wstr, sizeof(wstr),
        g_pVGuiLocalize->Find("#ServerBrowser_ServersResponding"), 1, waddress);
  }
  ...
  /*	FOR_EACH_VEC( vecAdress, iAddress )
          {
                  m_Queries.AddToTail(
     steamapicontext->SteamMatchmakingServers()->PingServer( vecAdress[ iAddress
     ].GetIPHostByteOrder(), vecAdress[ iAddress ].GetPort(), this ) );
          }*/
}
...
void CDialogAddServer::ServerResponded(newgameserver_t &server) {
  ...
  char buf[32];
  Q_snprintf(buf, sizeof(buf), "%d / %d", server.m_nPlayers,
