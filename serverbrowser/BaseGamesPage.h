...
//-----------------------------------------------------------------------------
// Purpose: Base property page for all the games lists
// (internet/favorites/lan/etc.)
//-----------------------------------------------------------------------------
class CBaseGamesPage
    : public vgui::PropertyPage,
      public IGameList,
      public IServerListResponse //, public ISteamMatchmakingPingResponse
{
...
public:
  ...
  CBaseGamesPage(vgui::Panel *parent, const char *name, EPageType eType,
                 const char *pCustomResFilename = NULL);
  ~CBaseGamesPage();
  ...
  // This is overridden in the add server dialog - since there is no Connect
  // button, the message never gets handled, but we want to add a server when
  // they dbl-click or press enter.
  ...
  virtual bool IsQuickListButtonChecked() {
    return m_pQuickListCheckButton ? m_pQuickListCheckButton->IsSelected()
                                   : false;
  }

  STEAM_CALLBACK(CBaseGamesPage, OnFavoritesMsg, FavoritesListChanged_t,
                 m_CallbackFavoritesMsg);

  // applies games filters to current list
  void ApplyGameFilters();

  void OnLoadingStarted() { StopRefresh(); }
...
protected:
  ...
  // If true, then we automatically select the first item that comes into the
  // games list.
...
private:
  void RequestServersResponse(
      int iServer, EMatchMakingServerResponse response,
