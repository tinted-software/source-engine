...
CMouseMessageForwardingPanel::CMouseMessageForwardingPanel(Panel *parent,
                                                           const char *name)
...
void CQuickListPanel::ApplySchemeSettings(IScheme *pScheme) {
  ...
  if (pScheme && m_pBGroundPanel) {
    m_pBGroundPanel->SetBgColor(
        pScheme->GetColor("QuickListBGDeselected", Color(255, 255, 255, 0)));
  }
}
...
void CQuickListPanel::SetRefreshing(void) {
  if (m_pServerNameLabel) {
    m_pServerNameLabel->SetText(
        g_pVGuiLocalize->Find("#ServerBrowser_QuickListRefreshing"));
  }
...
void CQuickListPanel::SetServerInfo(KeyValues *pKV, int iListID,
  ...
  g_pVGuiLocalize->ConvertANSIToUnicode(pKV->GetString("players", " "),
                                        playercount, sizeof(playercount));
  ...
  if (iTotalServers == 2) {
    m_pOtherServersLabel->SetText(
        g_pVGuiLocalize->Find("#ServerBrowser_QuickListOtherServer"));
    m_pOtherServersLabel->SetVisible(true);
  } else if (iTotalServers > 2) {
    wchar_t *pwszServers =
        g_pVGuiLocalize->Find("#ServerBrowser_QuickListOtherServers");
    _snwprintf(playercount, Q_ARRAYSIZE(playercount), L"%d",
               (iTotalServers - 1));
    g_pVGuiLocalize->ConstructString(players, sizeof(players), pwszServers, 1,
                                     playercount);
...
void CQuickListPanel::SetImage(const char *pMapName) {
  char path[512];
  Q_snprintf(path, sizeof(path), "materials/vgui/maps/menu_thumb_%s.vmt",
             pMapName);
  ...
  if (m_pMapImage) {
    char imagename[512];
    Q_snprintf(imagename, sizeof(imagename), "..\\vgui\\maps\\menu_thumb_%s",
               pMapName);
...
void CQuickListPanel::OnMousePressed(vgui::MouseCode code) {
  if (m_pListPanelParent) {
    vgui::PanelListPanel *pParent =
        dynamic_cast<vgui::PanelListPanel *>(m_pListPanelParent);

    if (pParent) {
      pParent->SetSelectedPanel(this);
      m_pListPanelParent->CallParentFunction(
          new KeyValues("ItemSelected", "itemID", -1));
    }

    if (code == MOUSE_RIGHT) {
      m_pListPanelParent->CallParentFunction(
          new KeyValues("OpenContextMenu", "itemID", -1));
    }
  }
}
...
void CQuickListPanel::OnMouseDoublePressed(vgui::MouseCode code) {
  ...
  m_pListPanelParent->CallParentFunction(
      new KeyValues("ConnectToServer", "code", code));
}
