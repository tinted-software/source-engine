...
ConVar sb_showblacklists("sb_showblacklists", "0", FCVAR_NONE,
                         "If set to 1, blacklist rules will be printed to the "
                         "console as they're applied.");

//-----------------------------------------------------------------------------
// Purpose: Server name comparison function
//-----------------------------------------------------------------------------
int __cdecl BlacklistedServerNameCompare(ListPanel *pPanel,
                                         const ListPanelItem &p1,
                                         const ListPanelItem &p2) {
  blacklisted_server_t *pSvr1 =
      ServerBrowserDialog().GetBlacklistPage()->GetBlacklistedServer(
          p1.userData);
  blacklisted_server_t *pSvr2 =
      ServerBrowserDialog().GetBlacklistPage()->GetBlacklistedServer(
          p2.userData);
...
//-----------------------------------------------------------------------------
int __cdecl BlacklistedIPAddressCompare(ListPanel *pPanel,
                                        const ListPanelItem &p1,
                                        const ListPanelItem &p2) {
  blacklisted_server_t *pSvr1 =
      ServerBrowserDialog().GetBlacklistPage()->GetBlacklistedServer(
...
//-----------------------------------------------------------------------------
int __cdecl BlacklistedAtCompare(ListPanel *pPanel, const ListPanelItem &p1,
                                 const ListPanelItem &p2) {
  blacklisted_server_t *pSvr1 =
      ServerBrowserDialog().GetBlacklistPage()->GetBlacklistedServer(
          p1.userData);
...
CBlacklistedServers::CBlacklistedServers(vgui::Panel *parent)
  ...
  m_pGameList->AddColumnHeader(
      0, "Name", "#ServerBrowser_BlacklistedServers", 50,
      ListPanel::COLUMN_RESIZEWITHWINDOW | ListPanel::COLUMN_UNHIDABLE);
  m_pGameList->AddColumnHeader(1, "IPAddr", "#ServerBrowser_IPAddress", 64,
                               ListPanel::COLUMN_HIDDEN);
  m_pGameList->AddColumnHeader(2, "BlacklistedAt",
                               "#ServerBrowser_BlacklistedDate", 100);
...
// Purpose: Destructor
//-----------------------------------------------------------------------------
CBlacklistedServers::~CBlacklistedServers() { ClearServerList(); }
...
void CBlacklistedServers::LoadBlacklistedList() {
  ...
  m_blackListTimestamp =
      g_pFullFileSystem->GetFileTime(BLACKLIST_DEFAULT_SAVE_FILE);
...
//-----------------------------------------------------------------------------
bool CBlacklistedServers::AddServersFromFile(const char *pszFilename,
                                             bool bResetTimes) {
  ...
  for (KeyValues *pData = pKV->GetFirstSubKey(); pData != NULL;
    ...
    if (pszNetAddr && pszNetAddr[0] && pszName && pszName[0]) {
      blacklisted_server_t *blackServer =
          m_blackList.AddServer(pszName, pszNetAddr, ullDate);
...
void CBlacklistedServers::AddServer(gameserveritem_t &server) {
  ...
  if (GameSupportsReplay()) {
    // send command to propagate to the client so the client can send it on to
    // the GC
    char command[256];
...
bool CBlacklistedServers::IsServerBlacklisted(gameserveritem_t &server) {
  ...
  if (g_pFullFileSystem->GetFileTime(BLACKLIST_DEFAULT_SAVE_FILE) !=
      m_blackListTimestamp) {
    LoadBlacklistedList();
  }
...
void CBlacklistedServers::OnOpenContextMenu(int itemID) {
  ...
  if (serverID != -1) {
    menu->AddMenuItem("RemoveServer",
                      "#ServerBrowser_RemoveServerFromBlacklist",
...
void CBlacklistedServers::OnAddServerByName() {
  ...
  CDialogAddBlacklistedServer *dlg =
      new CDialogAddBlacklistedServer(&ServerBrowserDialog(), NULL);
...
void CBlacklistedServers::OnRemoveFromBlacklist() {
  ...
  for (int iGame = (m_pGameList->GetSelectedItemsCount() - 1); iGame >= 0;
       iGame--) {
...
void CBlacklistedServers::OnImportBlacklist() {
  ...
  m_hImportDialog =
      new FileOpenDialog(this, "#ServerBrowser_ImportBlacklistTitle", true);
  if (m_hImportDialog.Get()) {
