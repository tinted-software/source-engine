...
// Purpose:
//-----------------------------------------------------------------------------
TagMenuButton::TagMenuButton(Panel *parent, const char *panelName,
                             const char *text)
...
public:
  CCustomServerInfoURLQuery(const char *title, const char *queryText,
                            vgui::Panel *parent)
...
//-----------------------------------------------------------------------------
TagInfoLabel::TagInfoLabel(Panel *parent, const char *panelName,
                           const char *text, const char *pszURL)
...
void TagInfoLabel::OnMousePressed(MouseCode code) {
  if (code == MOUSE_LEFT) {
    if (GetURL()) {
      ...
      CCustomServerInfoURLQuery *qb = new CCustomServerInfoURLQuery(
          "#ServerBrowser_CustomServerURLWarning",
...
CCustomGames::CCustomGames(vgui::Panel *parent)
  ...
  m_pAddTagList =
      new TagMenuButton(this, "AddTagList", "#ServerBrowser_AddCommonTags");
...
void CCustomGames::UpdateDerivedLayouts(void) {
  ...
  if (m_pFilter->IsSelected()) {
    if (g_pFullFileSystem->FileExists("servers/CustomGamesPage_Filters.res",
                                      "MOD")) {
      pPathID = "MOD";
    }

    LoadControlSettings("servers/CustomGamesPage_Filters.res", pPathID, NULL,
                        pConditions);
  } else {
    ...
    LoadControlSettings("servers/CustomGamesPage.res", pPathID, NULL,
                        pConditions);
  }
...
void CCustomGames::OnLoadFilter(KeyValues *filter) {
  ...
  Q_strncpy(m_szTagFilter, filter->GetString("gametype"),
            sizeof(m_szTagFilter));
...
//-----------------------------------------------------------------------------
// Purpose: Checks the workshop filtering setting, taking into account workshop
// filtering might be disabled
//-----------------------------------------------------------------------------
bool CCustomGames::CheckWorkshopFilter(gameserveritem_t &server) {
  ...
  if (workshopMode == eWorkshop_WorkshopOnly) {
    return V_strncasecmp(server.m_szMap, szWorkshopPrefix,
                         sizeof(szWorkshopPrefix) - 1) == 0;
...
void CCustomGames::OnSaveFilter(KeyValues *filter) {
  ...
  if (TagsExclude()) {
    ...
    m_vecServerFilters.AddToTail(
        MatchMakingKeyValuePair_t("gametype", m_szTagFilter));
  }
...
void CCustomGames::RecalculateCommonTags(void) {
    ...
    if (pServer && pServer->m_szGameTags && pServer->m_szGameTags[0]) {
      ...
      for (int iTag = 0; iTag < TagList.Count(); iTag++) {
        ...
        for (int iCheck = 0; iCheck < aTagsInUse.Count(); iCheck++) {
          if (!Q_strnicmp(TagList[iTag], aTagsInUse[iCheck].pszTag,
                          MAX_TAG_CHARACTERS)) {
  ...
  for (int i = 0; i < iTagsToAdd; i++) {
    ...
    m_pTagListMenu->AddMenuItem(pszTag, new KeyValues("AddTag", "tag", pszTag),
                                this, new KeyValues("data", "tag", pszTag));
  }
...
// Purpose:
//-----------------------------------------------------------------------------
void CCustomGames::OnTagMenuButtonOpened(void) { RecalculateCommonTags(); }
