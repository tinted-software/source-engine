...
CDialogGameInfo *CServerBrowserDialog::JoinGame(int serverIP, int serverPort,
  ...
  CDialogGameInfo *gameDialog =
      OpenGameInfoDialog(serverIP, serverPort, serverPort, pszConnectCode);
...
//-----------------------------------------------------------------------------
CDialogGameInfo *
CServerBrowserDialog::OpenGameInfoDialog(IGameList *gameList,
                                         newgameserver_t *pServer) {
  CDialogGameInfo *gameDialog = new CDialogGameInfo(
      NULL, pServer->m_NetAdr.GetIPHostByteOrder(), 0,
      pServer->m_NetAdr.GetPort(), gameList->GetConnectCode());
...
//-----------------------------------------------------------------------------
CDialogGameInfo *
CServerBrowserDialog::OpenGameInfoDialog(int serverIP, uint16 connPort,
                                         uint16 queryPort,
                                         const char *pszConnectCode) {
  CDialogGameInfo *gameDialog =
      new CDialogGameInfo(NULL, serverIP, queryPort, connPort, pszConnectCode);
  gameDialog->AddActionSignalTarget(this);
  gameDialog->SetParent(GetVParent());
  gameDialog->Run("");
  int i = m_GameInfoDialogs.AddToTail();
...
void CServerBrowserDialog::OnConnectToGame(KeyValues *pMessageValues) {
  ...
  if (m_pHistory && steamapicontext->SteamMatchmaking()) {
    steamapicontext->SteamMatchmaking()->AddFavoriteGame(
        0, unIP, connectionPort, queryPort, k_unFavoriteFlagHistory,
        time(NULL));
  ...
  for (int i = 0; i < m_GameInfoDialogs.Count(); i++) {
    vgui::Panel *dlg = m_GameInfoDialogs[i];
    if (dlg) {
      KeyValues *kv = new KeyValues("ConnectedToGame", "ip", unIP,
...
//-----------------------------------------------------------------------------
// Purpose: Called when start start loading, so we can cease server browser
// activity
//-----------------------------------------------------------------------------
...
void CServerBrowserDialog::OnKeyCodePressed(vgui::KeyCode code) {
  ...
  if (nButtonCode == KEY_XBUTTON_B || nButtonCode == STEAMCONTROLLER_B) {
    ...
    CBaseGamesPage *pGamesPage =
        dynamic_cast<CBaseGamesPage *>(m_pTabPanel->GetActivePage());
    if (pGamesPage) {
      ListPanel *pListPanel =
          dynamic_cast<ListPanel *>(pGamesPage->GetActiveList());
      if (pListPanel) {
