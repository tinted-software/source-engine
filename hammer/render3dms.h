...
//
// Maximum number of objects that can be kept in the list of objects to render
// last.
//
...
enum RenderState_t {
  RENDER_CENTER_CROSSHAIR, // Whether to draw the crosshair in the center of the
                           // view.
  ...
  RENDER_POLYGON_OFFSET_LINE, // Whether to offset line polygons (for wireframe
                              // selections)
  RENDER_REVERSE_SELECTION, // Driver issue fix - whether to return the largest
                            // (rather than smallest) Z value when picking
};
...
typedef struct {
  ...
  bool bReverseSelection; // Driver issue fix - whether to return the largest
                          // (rather than smallest) Z value when picking
} RenderStateInfo_t;
...
typedef struct {
  ...
  HitInfo_t *pHitsDest; // Final array in which to place pick hits, passed in by
                        // caller.
  int nMaxHits; // Maximum number of hits to place in the 'pHits' array, passed
                // in by caller, must be <= MAX_PICK_HITS.

  HitInfo_t Hits[MAX_PICK_HITS]; // Temporary array in which to place unsorted
                                 // pick hits.
  int nNumHits; // Number of hits so far in this pick (number of hits in 'Hits'
                // array).
...
typedef struct {
  IEditorTexture
      *pTexture;  // Pointer to the texture object that implements this texture.
  int nTextureID; // Unique ID of this texture across all renderers.
  unsigned int uTexture; // The texture name as returned by OpenGL when the
                         // texture was uploaded in this renderer.
} TextureCache_t;
...
public:
  ...
  virtual void PushInstanceData(CMapInstance *pInstanceClass,
  ...
  virtual void RenderWireframeBox(const Vector &Mins, const Vector &Maxs,
                                  unsigned char chRed, unsigned char chGreen,
                                  unsigned char chBlue);
  void RenderBox(const Vector &Mins, const Vector &Maxs, unsigned char chRed,
                 unsigned char chGreen, unsigned char chBlue,
                 SelectionState_t eBoxSelectionState);
  void RenderArrow(Vector const &vStartPt, Vector const &vEndPt,
                   unsigned char chRed, unsigned char chGreen,
                   unsigned char chBlue);
  void RenderCone(Vector const &vBasePt, Vector const &vTipPt, float fRadius,
                  int nSlices, unsigned char chRed, unsigned char chGreen,
                  unsigned char chBlue);
  void RenderSphere(Vector const &vCenter, float flRadius, int nTheta, int nPhi,
                    unsigned char chRed, unsigned char chGreen,
                    unsigned char chBlue);
  void RenderWireframeSphere(Vector const &vCenter, float flRadius, int nTheta,
                             int nPhi, unsigned char chRed,
                             unsigned char chGreen, unsigned char chBlue);
  void RenderInstanceMapClass(CMapInstance *pInstanceClass,
                              CMapClass *pMapClass, Vector &InstanceOrigin,
                              QAngle &InstanceAngles);

  int ObjectsAt(float x, float y, float fWidth, float fHeight,
                HitInfo_t *pObjects, int nMaxObjects, unsigned nFlags = 0);
  ...
  void
  BuildLightList(CUtlVector<CLightingPreviewLightDescription> *pList) const;
...
protected:
  ...
  float m_fFrameRate; // Framerate in frames per second, calculated once per
                      // second.
  int m_nFramesThisSample;  // Number of frames rendered in the current sample
                            // period.
  ...
  float
      m_fTimeElapsed; // Milliseconds elapsed since the last frame was rendered.

  // context for the last bitmap we sent to lighting preview for ray tracing. we
  // do not send if nothing happens, even if we end up re-rendering
  ...
  Vector4D m_FrustumPlanes[6]; // Plane normals and constants for the current
                               // view frustum.
  ...
  bool m_TranslucentSortRendering; // Used when we want to sort translucent
                                   // objects
  CCamera *m_pDropCamera;          // Dropped camera to use for debugging.

  CUtlPriorityQueue<TranslucentObjects_t>
      m_TranslucentRenderObjects; // List of objects to render after all the
                                  // other objects.
