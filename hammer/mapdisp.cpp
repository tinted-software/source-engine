...
bool CMapDisp::InitDispSurfaceData(CMapFace *pFace, bool bGenerateStartPoint) {
  ...
  pSurf->CalcLuxelCoords(nLightmapScale, false,
                         pFace->texture.UAxis.AsVector3D(),
                         pFace->texture.VAxis.AsVector3D());
  ...
  // reset the has mapping axes flag (surface has been created! - use new method
  // now)
  m_bHasMappingAxes = false;

  // set the s and t texture mapping axes so that tangent spaces can be
  // calculated
...
CMapDisp *CMapDisp::CopyFrom(CMapDisp *pMapDisp, bool bUpdateDependencies) {
  ...
  for (int i = 0; i < renderCount; i++) {
    m_CoreDispInfo.SetRenderIndex(i,
                                  pMapDisp->m_CoreDispInfo.GetRenderIndex(i));
  }
  ...
  for (int iTri = 0; iTri < nTriCount; ++iTri) {
    ...
    m_CoreDispInfo.SetTriIndices(iTri, triIndices[0], triIndices[1],
                                 triIndices[2]);
...
//-----------------------------------------------------------------------------
// Purpose: Returns TRUE if the lightmap scale on this face is within the
// acceptable range.
//-----------------------------------------------------------------------------
bool CMapDisp::ValidLightmapSize(void) {
  ...
  float flSize =
      static_cast<float>(static_cast<int>(flMaxLength * flOOLightmapScale) + 1);
  if (flSize > MAX_DISP_LIGHTMAP_DIM_WITHOUT_BORDER) {
    while (flSize > MAX_DISP_LIGHTMAP_DIM_WITHOUT_BORDER) {
      ...
      flSize = static_cast<float>(
          static_cast<int>(flMaxLength * flOOLightmapScale) + 1);
    }
...
bool CMapDisp::EntityInBoundingBox(Vector const &vOrigin) {
  ...
  if ((vOrigin.x < vMin.x) || (vOrigin.x > vMax.x) || (vOrigin.y < vMin.y) ||
      (vOrigin.y > vMax.y) || (vOrigin.z < vMin.z) || (vOrigin.z > vMax.z))
    return false;
...
void CMapDisp::UpSample(int oldPower) {
  ...
  for (int oh = 0, nh = 0; oh < oldHeight; oh++, nh += 2) {
    for (int ow = 0, nw = 0; ow < oldWidth; ow++, nw += 2) {
      ...
      if ((x + 1) < oldWidth) {
        dist = (GetFieldDistance(oldIndex) + GetFieldDistance(oldIndex + 1)) *
               0.5f;
      ...
      if ((y + 1) < oldHeight) {
        dist = (GetFieldDistance(oldIndex) +
                GetFieldDistance(oldIndex + oldHeight)) *
               0.5f;
        ...
        subdivPositions[newIndex + height] =
            (subPVector[0] + subPVector[1]) * 0.5f;

        GetSubdivNormal(oldIndex, subNVector[0]);
        GetSubdivNormal(oldIndex + oldHeight, subNVector[1]);
        subdivNormals[newIndex + height] =
            (subNVector[0] + subNVector[1]) * 0.5f;

        bUp = true;
      }

      if (bRight && bUp) {
        dist = (GetFieldDistance(oldIndex + 1) +
                GetFieldDistance(oldIndex + oldHeight)) *
               0.5f;
        dists[newIndex + height + 1] = dist;

        alpha =
            (GetAlpha(oldIndex + 1) + GetAlpha(oldIndex + oldHeight)) * 0.5f;
        ...
        GetSubdivPosition(oldIndex + oldHeight, subPVector[1]);
        subdivPositions[newIndex + height + 1] =
            (subPVector[0] + subPVector[1]) * 0.5f;

        GetSubdivNormal(oldIndex + 1, subNVector[0]);
        GetSubdivNormal(oldIndex + oldHeight, subNVector[1]);
        subdivNormals[newIndex + height + 1] =
            (subNVector[0] + subNVector[1]) * 0.5f;
      }
    }
  }
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CMapDisp::SamplePoints(int index, int width, int height,
                            bool *pValidPoints, float *pValue, float *pAlpha,
                            Vector &newDispVector, Vector &newSubdivPos,
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CMapDisp::GetValidSamplePoints(int index, int width, int height,
                                    bool *pValidPoints) {
  int x = index % width;
  int y = index / height;
...
void CMapDisp::DownSample(int oldPower) {
  ...
  for (int oh = 0, nh = 0; oh < oldHeight; oh += 2, nh++) {
    for (int ow = 0, nw = 0; ow < oldWidth; ow += 2, nw++) {
      ...
      Vector newSubdivPos;
      Vector newSubdivNormal;
      SamplePoints(oldIndex, oldWidth, oldHeight, validPoints, &newValue,
                   &newAlpha, newDispVector, newSubdivPos, newSubdivNormal);
...
#define SPLIT_EPSILON 0.001f
  ...
  for (int iVert = 0; iVert < nVertCount; ++iVert) {
    ...
    Vector2D vecDispUV;
    PointInQuadToBarycentric(vecSurfPoints[0], vecSurfPoints[3],
                             vecSurfPoints[2], vecSurfPoints[1], vecVert,
                             vecDispUV);
    ...
    float flNewAlpha;
    m_CoreDispInfo.DispUVToSurf(vecDispUV, vecNewVert, &vecNewNormal,
                                &flNewAlpha);
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CMapDisp::ComparePoints(const Vector &pt1, const Vector &pt2,
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
float CMapDisp::CollideWithTriangles(const Vector &RayStart,
                                     const Vector &RayEnd, Tri_t *pTris,
                                     int triCount, Vector &surfNormal) {
  // create a ray
  Ray_t ray;
  ...
  for (int ndxTri = 0; ndxTri < triCount; ndxTri++) {
    ...
    float fraction =
        IntersectRayWithTriangle(ray, tri.v[0], tri.v[2], tri.v[1], true);
    if (fraction == -1)
      continue;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CMapDisp::CreatePlanesFromBoundingBox(Plane_t *planes, const Vector &bbMin,
                                           const Vector &bbMax) {
  for (int i = 0; i < 6; i++) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CMapDisp::CollideWithBoundingBoxes(const Vector &rayStart,
                                        const Vector &rayEnd, BBox_t *pBBox,
                                        int bboxCount, Tri_t *pTris,
                                        int *triCount) {
...
//-----------------------------------------------------------------------------
bool CMapDisp::TraceLine(Vector &vecHitPos, Vector &vecHitNormal,
                         Vector const &vecRayStart, Vector const &vecRayEnd) {
...
#define UPPER_TOLERANCE 1.1f
  ...
  for (int ndx = 0; ndx < (width - 1); ndx++) {
    ...
    ComputeIntersectionBarycentricCoordinates(ray, tri.v[0], tri.v[1], tri.v[2],
                                              u, v);
...
void CMapDisp::UpdateWalkable(void) {
  ...
  for (int iTri = 0; iTri < nTriCount; ++iTri) {
    if (!IsTriWalkable(iTri)) {
      ...
      newTriIndices[0] =
          m_aWalkableVerts.AddToTail(m_CoreDispInfo.GetDispVert(triIndices[0]));
      newTriIndices[1] =
          m_aWalkableVerts.AddToTail(m_CoreDispInfo.GetDispVert(triIndices[1]));
      newTriIndices[2] =
          m_aWalkableVerts.AddToTail(m_CoreDispInfo.GetDispVert(triIndices[2]));
...
void CMapDisp::UpdateBuildable(void) {
  ...
  for (int iTri = 0; iTri < nTriCount; ++iTri) {
    if (!IsTriBuildable(iTri)) {
      ...
      newTriIndices[0] = m_aBuildableVerts.AddToTail(
          m_CoreDispInfo.GetDispVert(triIndices[0]));
      newTriIndices[1] = m_aBuildableVerts.AddToTail(
          m_CoreDispInfo.GetDispVert(triIndices[1]));
      newTriIndices[2] = m_aBuildableVerts.AddToTail(
          m_CoreDispInfo.GetDispVert(triIndices[2]));
...
void CMapDisp::UpdateTriRemove(void) {
  ...
  for (int iTri = 0; iTri < nTriCount; ++iTri) {
    if (IsTriRemove(iTri)) {
      ...
      newTriIndices[0] =
          m_aRemoveVerts.AddToTail(m_CoreDispInfo.GetDispVert(triIndices[0]));
      newTriIndices[1] =
          m_aRemoveVerts.AddToTail(m_CoreDispInfo.GetDispVert(triIndices[1]));
      newTriIndices[2] =
          m_aRemoveVerts.AddToTail(m_CoreDispInfo.GetDispVert(triIndices[2]));
...
void CMapDisp::CreateShoreOverlays(CMapFace *pFace, Shoreline_t *pShoreline) {
  ...
  for (int iTri = 0; iTri < nTriangleCount; ++iTri) {
    ...
    Vector4D plane;
    plane.Init(pFace->plane.normal.x, pFace->plane.normal.y,
               pFace->plane.normal.z, pFace->plane.dist);
    int nCount = IntersectTriangleWithPlaneBarycentric(v[0], vU, vV, plane,
                                                       vecIntersect);
    if (nCount != 2)
      continue;
    ...
    for (int iVert = 0; iVert < 3; ++iVert) {
      ...
      if (flDist > 0.0f) {
        float flDist2 =
            DotProduct(pFace->plane.normal, v[iVert]) - pFace->plane.dist;
        if (flDist2 < 0.0f) {
          vecNormal.Negate();
          break;
        }
    ...
    if (!VectorsAreEqual(vecPoints[0], vecPoints[1], 0.1f)) {
      pShoreline->AddSegment(vecPoints[0], vecPoints[1], vecNormal,
                             pFace->plane.dist, pFace, GetEditHandle());
    }
  }
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
static void RenderDisplacementNormals(CCoreDispInfo &coreDispInfo,
                                      int numVerts) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
static void RenderDisplacementTangentsS(CCoreDispInfo &coreDispInfo,
                                        int numVerts) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
static void RenderDisplacementTangentsT(CCoreDispInfo &coreDispInfo,
                                        int numVerts) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
static void RenderDisplacementVectorField(CCoreDispInfo &coreDispInfo,
                                          int numVerts) {
  Vector points[4], normal;
...
static void RenderSubdivPositions(CCoreDispInfo &coreDispInfo, int numVerts) {
  ...
  for (int i = 0; i < numVerts; i++) {
    ...
    meshBuilder.Position3f(pt[0] + normal[0], pt[1] + normal[1],
                           pt[2] + normal[2]);
    meshBuilder.Color3f(1.0f, 0.0f, 1.0f);
    meshBuilder.AdvanceVertex();
  }
...
void CMapDisp::CalcColor(CRender3D *pRender, bool bIsSelected,
                         SelectionState_t faceSelectionState, Color &pColor) {
...
void CMapDisp::Render2D(CRender2D *pRender, bool bIsSelected,
...
//-----------------------------------------------------------------------------
void CMapDisp::Render3D(CRender3D *pRender, bool bIsSelected,
                        SelectionState_t faceSelectionState) {
  ...
  bool bDispRemovedVertMode =
      CMapDoc::GetActiveMapDoc()->IsDispDrawRemovedVerts();
...
//-----------------------------------------------------------------------------
void CMapDisp::RenderOverlaySurface(CRender3D *pRender, bool bIsSelected,
                                    SelectionState_t faceSelectionState) {
  if (HasSelectMask())
    return;
...
//-----------------------------------------------------------------------------
void CMapDisp::RenderSurface(CRender3D *pRender, bool bIsSelected,
                             SelectionState_t faceSelectionState) {
  Color color(255, 255, 255, 255);
  CalcColor(pRender, bIsSelected, faceSelectionState, color);
  ...
  for (int i = 0; i < numVerts; ++i) {
    ...
    meshBuilder.Color4ub(color[0], color[1], color[2],
                         255 - (unsigned char)(pVert[i].m_Alpha));
    meshBuilder.Normal3fv(pVert[i].m_Normal.Base());
...
//-----------------------------------------------------------------------------
void CMapDisp::RenderWalkableSurface(CRender3D *pRender, bool bIsSelected,
                                     SelectionState_t faceSelectionState) {
  // Normal
...
//-----------------------------------------------------------------------------
void CMapDisp::RenderBuildableSurface(CRender3D *pRender, bool bIsSelected,
                                      SelectionState_t faceSelectionState) {
  // Normal
  for (int iPass = 0; iPass < 2; ++iPass) {
    Color color;
...
//-----------------------------------------------------------------------------
void CMapDisp::RenderRemoveSurface(CRender3D *pRender, bool bIsSelected,
                                   SelectionState_t faceSelectionState) {
  // Remove Faces
  for (int iPass = 0; iPass < 2; ++iPass) {
    Color color;
...
//-----------------------------------------------------------------------------
void CMapDisp::RenderWireframeSurface(CRender3D *pRender, bool bIsSelected,
                                      SelectionState_t faceSelectionState) {
  if (HasGridMask())
    return;
...
void CMapDisp::UpdateNeighborsOfDispsIntersectingBox(const Vector &bbMin,
                                                     const Vector &bbMax,
                                                     float flPadding) {
...
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapDisp::LoadDispDistancesCallback(CChunkFile *pFile,
                                                      CMapDisp *pDisp) {
...
// Output : ChunkFileResult_t
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapDisp::LoadDispDistancesKeyCallback(const char *szKey,
                                                         const char *szValue,
                                                         CMapDisp *pDisp) {
  float dispDistance;
...
// Output : ChunkFileResult_t
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapDisp::LoadDispOffsetsCallback(CChunkFile *pFile,
                                                    CMapDisp *pDisp) {
...
// Output : ChunkFileResult_t
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapDisp::LoadDispOffsetsKeyCallback(const char *szKey,
                                                       const char *szValue,
                                                       CMapDisp *pDisp) {
  Vector subdivVector;
...
// Output : ChunkFileResult_t
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapDisp::LoadDispOffsetNormalsCallback(CChunkFile *pFile,
                                                          CMapDisp *pDisp) {
  return (
      pFile->ReadChunk((KeyHandler_t)LoadDispOffsetNormalsKeyCallback, pDisp));
}
...
// Output : ChunkFileResult_t
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapDisp::LoadDispOffsetNormalsKeyCallback(
    const char *szKey, const char *szValue, CMapDisp *pDisp) {
  Vector normalVector;
...
// Output :
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapDisp::LoadDispKeyCallback(const char *szKey,
                                                const char *szValue,
                                                CMapDisp *pDisp) {
...
// Output : ChunkFileResult_t
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapDisp::LoadDispAlphasCallback(CChunkFile *pFile,
                                                   CMapDisp *pDisp) {
...
// Output : ChunkFileResult_t
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapDisp::LoadDispAlphasKeyCallback(const char *szKey,
                                                      const char *szValue,
                                                      CMapDisp *pDisp) {
  float alpha;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapDisp::LoadDispTriangleTagsCallback(CChunkFile *pFile,
                                                         CMapDisp *pDisp) {
  return (
      pFile->ReadChunk((KeyHandler_t)LoadDispTriangleTagsKeyCallback, pDisp));
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapDisp::LoadDispTriangleTagsKeyCallback(const char *szKey,
                                                            const char *szValue,
                                                            CMapDisp *pDisp) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapDisp::LoadDispAllowedVertsCallback(CChunkFile *pFile,
                                                         CMapDisp *pDisp) {
  return (
      pFile->ReadChunk((KeyHandler_t)LoadDispAllowedVertsKeyCallback, pDisp));
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapDisp::LoadDispAllowedVertsKeyCallback(const char *szKey,
                                                            const char *szValue,
                                                            CMapDisp *pDisp) {
...
// Output : ChunkFileResult_t
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapDisp::LoadDispNormalsCallback(CChunkFile *pFile,
                                                    CMapDisp *pDisp) {
...
// Output : ChunkFileResult_t
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapDisp::LoadDispNormalsKeyCallback(const char *szKey,
                                                       const char *szValue,
                                                       CMapDisp *pDisp) {
...
ChunkFileResult_t CMapDisp::LoadVMF(CChunkFile *pFile) {
  ...
  Handlers.AddHandler("offset_normals",
                      (ChunkHandler_t)LoadDispOffsetNormalsCallback, this);
  Handlers.AddHandler("alphas", (ChunkHandler_t)LoadDispAlphasCallback, this);
  Handlers.AddHandler("triangle_tags",
                      (ChunkHandler_t)LoadDispTriangleTagsCallback, this);
  Handlers.AddHandler("allowed_verts",
                      (ChunkHandler_t)LoadDispAllowedVertsCallback, this);

  pFile->PushHandlers(&Handlers);
  ChunkFileResult_t eResult =
      pFile->ReadChunk((KeyHandler_t)LoadDispKeyCallback, this);
  pFile->PopHandlers();
    ...
    if (eResult == ChunkFile_Ok) {
      ...
      for (int nRow = 0; nRow < nRows; nRow++) {
        ...
        for (int nCol = 0; nCol < nCols; nCol++) {
          ...
          sprintf(szTemp, "%g %g %g", (double)vectorFieldVector[0],
                  (double)vectorFieldVector[1], (double)vectorFieldVector[2]);
          strcat(szBuf, szTemp);
        }
    ...
    if (eResult == ChunkFile_Ok) {
      ...
      for (int nRow = 0; nRow < nRows; nRow++) {
        ...
        for (int nCol = 0; nCol < nCols; nCol++) {
          ...
          sprintf(szTemp, "%g %g %g", (double)subdivPos[0],
                  (double)subdivPos[1], (double)subdivPos[2]);
          strcat(szBuf, szTemp);
        }
    ...
    if (eResult == ChunkFile_Ok) {
      ...
      for (int nRow = 0; nRow < nRows; nRow++) {
        ...
        for (int nCol = 0; nCol < nCols; nCol++) {
          ...
          sprintf(szTemp, "%g %g %g", (double)subdivNormal[0],
                  (double)subdivNormal[1], (double)subdivNormal[2]);
          strcat(szBuf, szTemp);
        }
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CMapDisp::SerializedLoadMAP(std::fstream &file, CMapFace *pFace,
                                 UINT version) {
  int power;
  float maxData = 1.0f;
  int minTess;
  ...
  if (version < 350) {
    sscanf(buf, "%d [ %f %f %f ] [ %f %f %f ] %f %d %f", &power,
           &m_MapAxes[0][0], &m_MapAxes[0][1], &m_MapAxes[0][2],
           &m_MapAxes[1][0], &m_MapAxes[1][1], &m_MapAxes[1][2], &maxData,
           &minTess, &smoothingAngle);
  } else {
    sscanf(buf, "%d [ %f %f %f ] [ %f %f %f ] %d %f", &power, &m_MapAxes[0][0],
           &m_MapAxes[0][1], &m_MapAxes[0][2], &m_MapAxes[1][0],
           &m_MapAxes[1][1], &m_MapAxes[1][2], &minTess, &smoothingAngle);
  }
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CMapDisp::SerializedLoadRMF(std::fstream &file, CMapFace *pFace,
                                 float version) {
  int power;
  int minTess;
  float smoothingAngle;
...
void CMapDisp::DoTransform(const VMatrix &matrix) {
  ...
  if (bFlip) {
    // get the displacement starting point, relative to the newly "flipped"
    // points NOTE: this seems a bit hacky -- if flip goes NUTS later -- look
    // here!!!
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool SphereTriEdgePlanesIntersection(Vector const &ptCenter, float radius,
                                     cplane_t *pPlanes) {
  // check all planes
  for (int ndxPlane = 0; ndxPlane < 3; ndxPlane++) {
    float dist =
        pPlanes[ndxPlane].normal.Dot(ptCenter) - pPlanes[ndxPlane].dist;
    if (dist > radius)
      return false;
  }
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CMapDisp::PointSurfIntersection(Vector const &ptCenter, float radius,
                                     float &distMin, Vector &ptMin) {
  ...
  for (int i = 0; i < listSize; i += 3) {
    ...
    if (fabs(dist) < distMin) {
      ...
      for (int j = 0; j < 3; j++) {
        ...
        // check normal facing
        float distPt =
            edgePlanes[j].normal.Dot(v[(j + 2) % 3]) - edgePlanes[j].dist;
        if (distPt > 0.0f) {
      ...
      bool bSphereIntersect =
          SphereTriEdgePlanesIntersection(ptCenter, distMin, edgePlanes);
      ...
      if (bSphereIntersect) {
        bool bPointInside =
            SphereTriEdgePlanesIntersection(ptCenter, 0.0f, edgePlanes);
  ...
  if (bNeighborsUndo) {
    for (int ndxNeighbor = 0; ndxNeighbor < 4; ndxNeighbor++) {
      ...
      if (neighborHandle != EDITDISPHANDLE_INVALID) {
        ...
        // displacement pointer could have changed due to the undo/copyfrom
        // above
        pDisp = EditDispMgr()->GetDisp(editHandle);
      }
      ...
      if (cornerCount > 0) {
        for (int ndxCorner = 0; ndxCorner < cornerCount; ndxCorner++) {
          pDisp->GetCornerNeighbor(ndxNeighbor, ndxCorner, neighborHandle,
                                   neighborOrient);
          CMapDisp *pNeighborDisp = EditDispMgr()->GetDisp(neighborHandle);
          CMapFace *pNeighborFace = (CMapFace *)pNeighborDisp->GetParent();
          CMapSolid *pNeighborSolid = (CMapSolid *)pNeighborFace->GetParent();
...
void CMapDisp::UpdateVertPositionForSubdiv(int iVert,
                                           const Vector &vecNewSubdivPos) {
...
int CMapDisp::CollideWithDispTri(const Vector &rayStart, const Vector &rayEnd,
                                 float &flFraction, bool OneSided) {
  int iTriangle = -1;
  flFraction = 1.0f;
  ...
  for (int iTri = 0; iTri < nTriCount; ++iTri) {
    ...
    Ray_t ray;
    ray.Init(rayStart, rayEnd, Vector(0.0f, 0.0f, 0.0f),
