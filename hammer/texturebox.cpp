...
//
// Purpose: Implements an owner-draw combo box containing the names and
// thumbnail
//			images of textures. The textures are gotten from the
//global texture 			manager object, and are filtered by texture format.
//
//=============================================================================
...
void CTextureBox::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) {
  ...
  if (lpDrawItemStruct->itemID != -1) {
    IEditorTexture *pTex =
        (IEditorTexture *)GetItemDataPtr(lpDrawItemStruct->itemID);
    ...
    if (pTex != NULL) {
      ...
      pOldPalette = dc.SelectPalette(
          pTex->HasPalette() ? pTex->GetPalette() : g_pGameConfig->Palette,
          FALSE);
      dc.RealizePalette();
    }
    ...
    if (pTex == NULL) {
      ...
      if ((r.bottom - r.top) > 32) {
        ...
        dc.TextOut(r2.right + 4, r2.top + 4 + iFontHeight, szName,
                   strlen(szName));
      }
...
void CTextureBox::LoadGraphicList(void) {
  ...
  int nIndex = 0;
  IEditorTexture *pTex =
      g_Textures.EnumActiveTextures(&nIndex, g_pGameConfig->GetTextureFormat());
  while (pTex != NULL) {
    char szStr[MAX_PATH];
    pTex->GetShortName(szStr);
    ...
    pTex = g_Textures.EnumActiveTextures(&nIndex,
                                         g_pGameConfig->GetTextureFormat());
  }

  //
  // Hack: Select one that doesn't start with '+', '!', or '*', and doesn't have
  // "door" in it.
  //
  SetCurSel(0);
  ...
  for (int i = 0; i < nSel; i++) {
    ...
    if (pTexSearch != NULL) {
      ...
      if ((szName[0] != 0) && (szName[0] != '*') && (szName[0] != '+') &&
          (szName[0] != '!') && (strstr(szName, "door") == NULL)) {
        // this one is ok
        SetCurSel(i);
        break;
      }
...
//-----------------------------------------------------------------------------
BOOL CTextureBox::Create(DWORD dwStyle, const RECT &rect, CWnd *pParentWnd,
                         UINT nID) {
