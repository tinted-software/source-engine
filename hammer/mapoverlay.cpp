...
void CMapOverlay::Basis_Clear(void) {
  ...
  for (int iAxis = 0; iAxis < 3; iAxis++) {
    m_Basis.m_vecAxes[iAxis].Init(OVERLAY_INVALID_VALUE, OVERLAY_INVALID_VALUE,
                                  OVERLAY_INVALID_VALUE);
...
void CMapOverlay::Basis_BuildAxes(void) {
  ...
  m_Basis.m_vecAxes[OVERLAY_BASIS_V] =
      m_Basis.m_vecAxes[OVERLAY_BASIS_NORMAL].Cross(
          m_Basis.m_vecAxes[OVERLAY_BASIS_U]);
  VectorNormalize(m_Basis.m_vecAxes[OVERLAY_BASIS_V]);

  m_Basis.m_vecAxes[OVERLAY_BASIS_U] = m_Basis.m_vecAxes[OVERLAY_BASIS_V].Cross(
...
void CMapOverlay::Basis_UpdateParentKey(void) {
  ...
  if (pEntity) {
    sprintf(szValue, "%g %g %g", m_Basis.m_vecOrigin.x, m_Basis.m_vecOrigin.y,
            m_Basis.m_vecOrigin.z);
    pEntity->NotifyChildKeyChanged(this, "BasisOrigin", szValue);

    sprintf(szValue, "%g %g %g", m_Basis.m_vecAxes[OVERLAY_BASIS_U].x,
            m_Basis.m_vecAxes[OVERLAY_BASIS_U].y,
            m_Basis.m_vecAxes[OVERLAY_BASIS_U].z);
    pEntity->NotifyChildKeyChanged(this, "BasisU", szValue);

    sprintf(szValue, "%g %g %g", m_Basis.m_vecAxes[OVERLAY_BASIS_V].x,
            m_Basis.m_vecAxes[OVERLAY_BASIS_V].y,
    ...
    sprintf(szValue, "%g %g %g", m_Basis.m_vecAxes[OVERLAY_BASIS_NORMAL].x,
            m_Basis.m_vecAxes[OVERLAY_BASIS_NORMAL].y,
            m_Basis.m_vecAxes[OVERLAY_BASIS_NORMAL].z);
...
void CMapOverlay::Handles_Init(CMapFace *pFace) {
  IEditorTexture *pTexture =
      g_Textures.FindActiveTexture(GetDefaultTextureName());
...
void CMapOverlay::Handles_Render3D(CRender3D *pRender) {
  ...
  for (int iHandle = 0; iHandle < OVERLAY_HANDLES_COUNT; iHandle++) {
    ...
    if (m_Handles.m_iHit == iHandle) {
      pRender->SetHandleColor(ucSelectColor[0], ucSelectColor[1],
                              ucSelectColor[2]);
    } else {
...
// Purpose:
//-----------------------------------------------------------------------------
void CMapOverlay::Handles_SurfToOverlayPlane(CMapFace *pFace,
                                             Vector const &vecSurf,
                                             Vector &vecPoint) {
  Vector vecWorld;
  if (pFace->HasDisp()) {
...
void CMapOverlay::Handles_UpdateParentKey(void) {
  ...
  CMapEntity *pEntity = (CMapEntity *)GetParent();
  if (pEntity) {
    sprintf(szValue, "%g %g %g", m_Handles.m_vecBasisCoords[0].x,
            m_Handles.m_vecBasisCoords[0].y, (float)m_Basis.m_nAxesFlip[0]);
    pEntity->NotifyChildKeyChanged(this, "uv0", szValue);

    sprintf(szValue, "%g %g %g", m_Handles.m_vecBasisCoords[1].x,
            m_Handles.m_vecBasisCoords[1].y, (float)m_Basis.m_nAxesFlip[1]);
    pEntity->NotifyChildKeyChanged(this, "uv1", szValue);

    sprintf(szValue, "%g %g %g", m_Handles.m_vecBasisCoords[2].x,
            m_Handles.m_vecBasisCoords[2].y, (float)m_Basis.m_nAxesFlip[2]);
    pEntity->NotifyChildKeyChanged(this, "uv2", szValue);

    sprintf(szValue, "%g %g %g", m_Handles.m_vecBasisCoords[3].x,
            m_Handles.m_vecBasisCoords[3].y, 0.0f);
  ...
  if (pDst) {
    for (int iPoint = 0; iPoint < pSrc->m_nPointCount; iPoint++) {
      ...
      for (int iTexCoord = 0; iTexCoord < NUM_CLIPFACE_TEXCOORDS; iTexCoord++) {
        pDst->m_aTexCoords[iTexCoord][iPoint] =
            pSrc->m_aTexCoords[iTexCoord][iPoint];
      }

      pDst->m_aBlends[iPoint].m_nType = pSrc->m_aBlends[iPoint].m_nType;
      for (int iBlend = 0; iBlend < 3; iBlend++) {
        pDst->m_aBlends[iPoint].m_iPoints[iBlend] =
            pSrc->m_aBlends[iPoint].m_iPoints[iBlend];
        pDst->m_aBlends[iPoint].m_flBlends[iBlend] =
            pSrc->m_aBlends[iPoint].m_flBlends[iBlend];
      }
    }
  }
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CMapOverlay::ClipFace_GetBounds(ClipFace_t *pClipFace, Vector &vecMin,
                                     Vector &vecMax) {
  if (pClipFace) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CMapOverlay::ClipFace_Clip(ClipFace_t *pClipFace, cplane_t *pClipPlane,
                                float flEpsilon, ClipFace_t **ppFront,
                                ClipFace_t **ppBack) {
  if (!pClipFace)
    return;
  ...
  *ppFront = *ppBack = NULL;
  ...
  for (iPoint = 0; iPoint < pClipFace->m_nPointCount; iPoint++) {
    flDists[iPoint] =
        pClipPlane->normal.Dot(pClipFace->m_aPoints.Element(iPoint)) -
        pClipPlane->dist;
  ...
  // Build new front and back faces. Leave room for two extra points on each
  // side because any point might be on the plane, which would put it into both
  // the front and back sides, and then we need to allow for an additional
  // vertex created by clipping.
  ...
  // For every point on the face being clipped, determine which side of the
  // clipping plane it is on and add it to a either a front list or a back list.
  // Points that are on the plane are added to both lists.
  for (iPoint = 0; iPoint < pClipFace->m_nPointCount; iPoint++) {
    // "On" clip plane.
    if (nSides[iPoint] == SIDE_ON) {
      pFront->m_aPoints[pFront->m_nPointCount] = pClipFace->m_aPoints[iPoint];
      for (int iTexCoord = 0; iTexCoord < NUM_CLIPFACE_TEXCOORDS; iTexCoord++)
        pFront->m_aTexCoords[iTexCoord][pFront->m_nPointCount] =
      ...
      for (int iTexCoord = 0; iTexCoord < NUM_CLIPFACE_TEXCOORDS; iTexCoord++)
        pBack->m_aTexCoords[iTexCoord][pBack->m_nPointCount] =
            pClipFace->m_aTexCoords[iTexCoord][iPoint];
      pBack->m_nPointCount++;
    ...
    Vector vecPoint =
        ...
        (pClipFace->m_aPoints[(iPoint + 1) % pClipFace->m_nPointCount] -
         pClipFace->m_aPoints[iPoint]) *
            fraction;
    for (int iTexCoord = 0; iTexCoord < NUM_CLIPFACE_TEXCOORDS; iTexCoord++) {
      Vector2D vecTexCoord =
          pClipFace->m_aTexCoords[iTexCoord][iPoint] +
          (pClipFace->m_aTexCoords[iTexCoord]
                                  [(iPoint + 1) % pClipFace->m_nPointCount] -
           pClipFace->m_aTexCoords[iTexCoord][iPoint]) *
              fraction;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CMapOverlay::ClipFace_ClipBarycentric(
    ClipFace_t *pClipFace, cplane_t *pClipPlane, float flEpsilon, int iClip,
  ...
  for (iPoint = 0; iPoint < pClipFace->m_nPointCount; iPoint++) {
    flDists[iPoint] =
        pClipPlane->normal.Dot(pClipFace->m_aDispPointUVs.Element(iPoint)) -
        pClipPlane->dist;
  ...
  // NOTE: We are allowing to go over by 2 and then destroy the surface later.
  // The old system
  //       allowed for some bad data and we need to be able to load the map and
  //       destroy the surface!
  ...
  for (iPoint = 0; iPoint < pClipFace->m_nPointCount; iPoint++) {
    ...
    if (nSides[iPoint] == SIDE_ON) {
      pFront->m_aPoints[pFront->m_nPointCount] = pClipFace->m_aPoints[iPoint];
      pFront->m_aDispPointUVs[pFront->m_nPointCount] =
      ...
      pBack->m_aPoints[pBack->m_nPointCount] = pClipFace->m_aPoints[iPoint];
      pBack->m_aDispPointUVs[pBack->m_nPointCount] =
          pClipFace->m_aDispPointUVs[iPoint];
    ...
    Vector vecDispPointUV =
        pClipFace->m_aDispPointUVs[iPoint] +
        (pClipFace->m_aDispPointUVs[(iPoint + 1) % pClipFace->m_nPointCount] -
         pClipFace->m_aDispPointUVs[iPoint]) *
            fraction;

    Vector2D vecUV, vecTexCoord;
    PointInQuadToBarycentric(m_pOverlayFace->m_aPoints[0],
                             m_pOverlayFace->m_aPoints[3],
                             m_pOverlayFace->m_aPoints[2],
                             m_pOverlayFace->m_aPoints[1], vecPoint, vecUV);
    ...
    for (int iTexCoord = 0; iTexCoord < NUM_CLIPFACE_TEXCOORDS; iTexCoord++) {
      TexCoordInQuadFromBarycentric(m_pOverlayFace->m_aTexCoords[iTexCoord][0],
                                    ...
                                    m_pOverlayFace->m_aTexCoords[iTexCoord][1],
                                    vecUV, vecTexCoord);
    ...
    ClipFace_BuildBlend(pFront, pDisp, pClipPlane, iClip, vecDispPointUV,
                        vecPoint);
    pFront->m_nPointCount++;

    pBack->m_aPoints[pBack->m_nPointCount] = vecPoint;
    pBack->m_aDispPointUVs[pBack->m_nPointCount] = vecDispPointUV;
    ClipFace_BuildBlend(pBack, pDisp, pClipPlane, iClip, vecDispPointUV,
                        vecPoint);
    pBack->m_nPointCount++;
  }

  // Check for a bad surface.
  if ((pFront->m_nPointCount > nMaxPointCount) ||
      (pBack->m_nPointCount > nMaxPointCount))
    return;
...
void CMapOverlay::ClipFace_PreClipDisp(ClipFace_t *pClipFace, CMapDisp *pDisp) {
  ...
  for (int iPoint = 0; iPoint < pClipFace->m_nPointCount; ++iPoint) {
    ...
    if (ClipFace_CalcBarycentricCooefs(pDisp, vecVertsUV, vecDispUV, flCoefs)) {
      ClipFace_ResolveBarycentricClip(pDisp, pClipFace, iPoint, vecDispUV,
                                      flCoefs, iTris, vecVertsUV);
    }
  }
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CMapOverlay::ClipFace_CalcBarycentricCooefs(CMapDisp *pDisp,
                                                 Vector2D *pVertsUV,
  ...
  // Cross
  float flSubArea =
      ((vecSegment1.x * vecSegment0.y) - (vecSegment0.x * vecSegment1.y)) *
      0.5f;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CMapOverlay::ClipFace_ResolveBarycentricClip(
    CMapDisp *pDisp, ClipFace_t *pClipFace, int iClipFacePoint,
  ...
  if (nZeroCount == 1) {
    for (int iVert = 0; iVert < 3; ++iVert) {
      if (bZeroPoint[iVert]) {
        ...
        pClipFace->m_aBlends[iClipFacePoint].m_iPoints[0] =
            pTris[(iVert + 1) % 3];
        pClipFace->m_aBlends[iClipFacePoint].m_iPoints[1] =
            pTris[(iVert + 2) % 3];
...
void CMapOverlay::ClipFace_BuildBlend(ClipFace_t *pClipFace, CMapDisp *pDisp,
  ...
  case OVERLAY_ANGLE0: {
    ...
    if (fabs(flFracU) < OVERLAY_DISPSPACE_EPSILON) {
      pClipFace->m_aBlends[pClipFace->m_nPointCount].m_nType =
          OVERLAY_BLENDTYPE_VERT;
      pClipFace->m_aBlends[pClipFace->m_nPointCount].m_iPoints[0] =
          (nWidth * iClip) + nSnapU;
    }
    // Edge type
    else {
      pClipFace->m_aBlends[pClipFace->m_nPointCount].m_nType =
          OVERLAY_BLENDTYPE_EDGE;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CMapOverlay::ClipFace_CopyBlendFrom(ClipFace_t *pClipFace,
                                         BlendData_t *pBlendFrom) {
  pClipFace->m_aBlends[pClipFace->m_nPointCount].m_nType = pBlendFrom->m_nType;
  for (int iPoint = 0; iPoint < 3; iPoint++) {
    pClipFace->m_aBlends[pClipFace->m_nPointCount].m_iPoints[iPoint] =
        pBlendFrom->m_iPoints[iPoint];
    pClipFace->m_aBlends[pClipFace->m_nPointCount].m_flBlends[iPoint] =
        pBlendFrom->m_flBlends[iPoint];
  }
}
...
void CMapOverlay::ClipFace_BuildFacesFromBlendedData(ClipFace_t *pClipFace) {
  if (pClipFace->m_pBuildFace->HasDisp()) {
    ...
    for (int iPoint = 0; iPoint < pClipFace->m_nPointCount; iPoint++) {
      ...
      } else if (pClipFace->m_aBlends[iPoint].m_nType ==
                 OVERLAY_BLENDTYPE_EDGE) {
        pDisp->GetVert(pClipFace->m_aBlends[iPoint].m_iPoints[0], vecPos[0]);
        pDisp->GetVert(pClipFace->m_aBlends[iPoint].m_iPoints[1], vecPos[1]);
        pClipFace->m_aPoints[iPoint] =
            vecPos[0] + (vecPos[1] - vecPos[0]) *
                            pClipFace->m_aBlends[iPoint].m_flBlends[0];
      } else if (pClipFace->m_aBlends[iPoint].m_nType ==
                 OVERLAY_BLENDTYPE_BARY) {
        pDisp->GetVert(pClipFace->m_aBlends[iPoint].m_iPoints[0], vecPos[0]);
        pDisp->GetVert(pClipFace->m_aBlends[iPoint].m_iPoints[1], vecPos[1]);
        pDisp->GetVert(pClipFace->m_aBlends[iPoint].m_iPoints[2], vecPos[2]);
        pClipFace->m_aPoints[iPoint] =
            (vecPos[0] * pClipFace->m_aBlends[iPoint].m_flBlends[0]) +
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
CMapClass *CMapOverlay::CreateMapOverlay(CHelperInfo *pInfo,
                                         CMapEntity *pParent) {
...
//other operations 			that require all world objects to be present.
...
void CMapOverlay::Handles_FixOrder() {
  ...
  // make sure that handle order and plane normal are in sync so CCW culling
  // works correctly
  Vector vNormal = GetNormalFromPoints(
      m_Handles.m_vec3D[0], m_Handles.m_vec3D[1], m_Handles.m_vec3D[2]);
...
void CMapOverlay::OnParentKeyChanged(const char *szKey, const char *szValue) {
  ...
  if (!stricmp(szKey, "uv0")) {
    sscanf(szValue, "%f %f %f", &m_Handles.m_vecBasisCoords[0].x,
           &m_Handles.m_vecBasisCoords[0].y, &flDummy);
    m_Basis.m_nAxesFlip[0] = (int)flDummy;
  }
  if (!stricmp(szKey, "uv1")) {
    sscanf(szValue, "%f %f %f", &m_Handles.m_vecBasisCoords[1].x,
           &m_Handles.m_vecBasisCoords[1].y, &flDummy);
    m_Basis.m_nAxesFlip[1] = (int)flDummy;
  }
  if (!stricmp(szKey, "uv2")) {
    sscanf(szValue, "%f %f %f", &m_Handles.m_vecBasisCoords[2].x,
           &m_Handles.m_vecBasisCoords[2].y, &flDummy);
    m_Basis.m_nAxesFlip[2] = (int)flDummy;
  }
  if (!stricmp(szKey, "uv3")) {
    sscanf(szValue, "%f %f %f", &m_Handles.m_vecBasisCoords[3].x,
           &m_Handles.m_vecBasisCoords[3].y, &flDummy);
  }

  // Read basis data.
  if (!stricmp(szKey, "BasisOrigin")) {
    sscanf(szValue, "%f %f %f", &m_Basis.m_vecOrigin.x, &m_Basis.m_vecOrigin.y,
           &m_Basis.m_vecOrigin.z);
  }

  if (!stricmp(szKey, "BasisU")) {
    sscanf(szValue, "%f %f %f", &m_Basis.m_vecAxes[OVERLAY_BASIS_U].x,
           &m_Basis.m_vecAxes[OVERLAY_BASIS_U].y,
           &m_Basis.m_vecAxes[OVERLAY_BASIS_U].z);
  }

  if (!stricmp(szKey, "BasisV")) {
    sscanf(szValue, "%f %f %f", &m_Basis.m_vecAxes[OVERLAY_BASIS_V].x,
           &m_Basis.m_vecAxes[OVERLAY_BASIS_V].y,
           &m_Basis.m_vecAxes[OVERLAY_BASIS_V].z);
  }
  ...
  if (m_bLoaded) {
    // Clip - this needs to be done for everything other than a material change,
    // so go ahead.
    DoClip();
  ...
  if (!tmpMatrix.IsIdentity()) {
    ...
    if (bIsUnit && bIsPerp) {
      ...
      for (int iHandle = 0; iHandle < OVERLAY_HANDLES_COUNT; iHandle++) {
        ...
        Vector vecPos = (vecUV.x * m_Basis.m_vecAxes[OVERLAY_BASIS_U] +
...
//-----------------------------------------------------------------------------
void CMapOverlay::OnPaste(CMapClass *pCopy, CMapWorld *pSourceWorld,
...
void CMapOverlay::OnClone(CMapClass *pClone, CMapWorld *pWorld,
  ...
  if (pOverlay) {
    if ((GetOverlayType() && OVERLAY_TYPE_SHORE) == 0) {
      // Update the clone's solid dependencies (this doesn't happen on clone
      // generally).
...
//-----------------------------------------------------------------------------
void CMapOverlay::OnNotifyDependent(CMapClass *pObject,
  ...
  //
  // NOTE: the solid moving (changing) can update the overlay/solid(face)
  // dependency
    ...
    if (GetSelectionState() != SELECT_MODIFY) {
      ...
      for (int iFace = 0; iFace < nFaceCount; iFace++) {
        ...
        for (int iPoint = 0; iPoint < pRenderFace->m_nPointCount; iPoint++) {
          ...
          meshBuilder.Position3f(pRenderFace->m_aPoints[iPoint].x,
                                 pRenderFace->m_aPoints[iPoint].y,
...
void CMapOverlay::PreClip(void) {
  ...
  if (m_pOverlayFace) {
    for (int iPoint = 0; iPoint < OVERLAY_HANDLES_COUNT; iPoint++) {
      OverlayUVToOverlayPlane(m_Handles.m_vecBasisCoords[iPoint],
                              m_pOverlayFace->m_aPoints[iPoint]);
      ...
      case 0:
        vTexCoord =
            Vector2D(m_Material.m_vecTextureU.x, m_Material.m_vecTextureV.x);
        break;
      case 1:
        vTexCoord =
            Vector2D(m_Material.m_vecTextureU.x, m_Material.m_vecTextureV.y);
        break;
      case 2:
        vTexCoord =
            Vector2D(m_Material.m_vecTextureU.y, m_Material.m_vecTextureV.y);
        break;
      case 3:
        vTexCoord =
            Vector2D(m_Material.m_vecTextureU.y, m_Material.m_vecTextureV.x);
        break;
      default:
      ...
      if (m_Basis.m_pFace->HasDisp()) {
        ...
        if (pDisp) {
          Vector2D vecTmp;
          pDisp->BaseFacePlaneToDispUV(m_pOverlayFace->m_aPoints[iPoint],
                                       vecTmp);
    ...
    // The second set of texcoords on the overlay is used for alpha by certain
    // shaders, and they want to stretch the texture across the whole overlay.
...
void CMapOverlay::DoClipFace(CMapFace *pFace) {
  ...
  for (int iClipPlane = 0; iClipPlane < nEdgePlaneCount; iClipPlane++) {
    ...
    if (pClippedFace) {
      // Clip the overlay and delete the data (we are done with it - we are only
      // interested in what is left).
      ClipFace_Clip(pClippedFace, &pEdgePlanes[iClipPlane],
                    OVERLAY_WORLDSPACE_EPSILON, &pFront, &pBack);
  ...
  for (int iPoint = 0; iPoint < pClippedFace->m_nPointCount; iPoint++) {
    ...
    PointInQuadToBarycentric(
        m_pOverlayFace->m_aPoints[0], m_pOverlayFace->m_aPoints[3],
...
bool CMapOverlay::BuildEdgePlanes(Vector const *pPoints, int nPointCount,
  ...
  for (int iPoint = 0; iPoint < nPointCount; iPoint++) {
    ...
    pEdgePlanes[iPoint].normal =
        m_Basis.m_vecAxes[OVERLAY_BASIS_NORMAL].Cross(vecEdge);
    pEdgePlanes[iPoint].dist = pEdgePlanes[iPoint].normal.Dot(pPoints[iPoint]);

    // Check normal facing.
    float flDist =
        pEdgePlanes[iPoint].normal.Dot(pPoints[(iPoint + 2) % nPointCount]) -
        pEdgePlanes[iPoint].dist;
    if (flDist > 0.0f) {
      // flip
...
// Purpose:
//-----------------------------------------------------------------------------
void CMapOverlay::Disp_ClipFragments(CMapDisp *pDisp,
                                     ClipFaces_t &aDispFragments) {
  cplane_t clipPlane;
  ...
  for (int iInterval = nLoopStart; iInterval < nLoopEnd;
    ...
    for (int iFrag = 0; iFrag < nFragCount; iFrag++) {
      ...
      if (pClipFrag) {
        ...
        ClipFace_ClipBarycentric(pClipFrag, &clipPlane,
                                 OVERLAY_DISPSPACE_EPSILON, iInterval, pDisp,
                                 &pFront, &pBack);
...
bool CMapOverlay::HandlesHitTest(CMapView *pView, const Vector2D &vPoint) {
  ...
  for (int iPoint = 0; iPoint < 4; iPoint++) {
    ...
    if (vPoint.x < (vHandle.x - handleRadius) ||
        vPoint.x > (vHandle.x + handleRadius))
      continue;

    if (vPoint.y < (vHandle.y - handleRadius) ||
        vPoint.y > (vHandle.y + handleRadius))
      continue;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CMapOverlay::OverlayUVToOverlayPlane(const Vector2D &vecUV,
                                          Vector &vecOverlayPoint) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CMapOverlay::OverlayPlaneToOverlayUV(const Vector &vecOverlayPoint,
                                          Vector2D &vecUV) {
  Vector vecDelta;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CMapOverlay::WorldToOverlayPlane(const Vector &vecWorldPoint,
                                      Vector &vecOverlayPoint) {
  Vector vecDelta = vecWorldPoint - m_Basis.m_vecOrigin;
  float flDist = m_Basis.m_vecAxes[OVERLAY_BASIS_NORMAL].Dot(vecDelta);
  vecOverlayPoint =
      vecWorldPoint - (m_Basis.m_vecAxes[OVERLAY_BASIS_NORMAL] * flDist);
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CMapOverlay::OverlayPlaneToWorld(CMapFace *pFace,
  ...
  float flDistToSurface =
      surfacePlane.normal.Dot(vecOverlayPoint) - surfacePlane.dist;
  float flDist = flDistToSurface;
  float flDot =
      surfacePlane.normal.Dot(m_Basis.m_vecAxes[OVERLAY_BASIS_NORMAL]);
  if (flDot != 0.0f) {
    flDist = (1.0f / flDot) * flDistToSurface;
  }

  vecWorldPoint =
      vecOverlayPoint - (m_Basis.m_vecAxes[OVERLAY_BASIS_NORMAL] * flDist);
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CMapOverlay::OverlayPlaneToSurfFromList(const Vector &vecOverlayPoint,
                                             Vector &vecSurfPoint) {
  ...
  for (int iFace = 0; iFace < nFaceCount; ++iFace) {
    ...
    BuildEdgePlanes(aPoints.Base(), aPoints.Count(), aPlanes.Base(),
                    aPlanes.Count());

    // Check to see if a point lies behind all of the edge planes - this is our
    // face.
    int iPlane;
    for (iPlane = 0; iPlane < aPlanes.Count(); ++iPlane) {
      float flDist =
          aPlanes[iPlane].normal.Dot(vecOverlayPoint) - aPlanes[iPlane].dist;
      if (flDist >= 0.0f)
        break;
    }
...
// Purpose:
//-----------------------------------------------------------------------------
bool CMapOverlay::EntityOnSurfFromListToBaseFacePlane(
    const Vector &vecWorldPoint, Vector &vecBasePoint) {
  int nFaceCount = GetFaceCount();
  for (int iFace = 0; iFace < nFaceCount; ++iFace) {
    CMapFace *pFace = GetFace(iFace);
    if (!pFace)
...
// Purpose:
//-----------------------------------------------------------------------------
void CMapOverlay::GetTriVerts(CMapDisp *pDisp, const Vector2D &vecSurfUV,
...
// Purpose:
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapOverlay::SaveDataToVMF(CChunkFile *pFile,
                                             CSaveInfo *pSaveInfo) {
  ...
  if (eResult == ChunkFile_Ok) {
    eResult =
        pFile->WriteKeyValue("material", m_Material.m_pTexture->GetName());
  }
  ...
  if (eResult == ChunkFile_Ok) {
    eResult = pFile->WriteKeyValueVector3("BasisU",
                                          m_Basis.m_vecAxes[OVERLAY_BASIS_U]);
  }
  if (eResult == ChunkFile_Ok) {
    eResult = pFile->WriteKeyValueVector3("BasisV",
                                          m_Basis.m_vecAxes[OVERLAY_BASIS_V]);
  }
  if (eResult == ChunkFile_Ok) {
    eResult = pFile->WriteKeyValueVector3(
        "BasisNormal", m_Basis.m_vecAxes[OVERLAY_BASIS_NORMAL]);
  }

  if (eResult == ChunkFile_Ok) {
    Vector vecTmp(m_Handles.m_vecBasisCoords[0].x,
                  m_Handles.m_vecBasisCoords[0].y,
                  (float)m_Basis.m_nAxesFlip[0]);
    eResult = pFile->WriteKeyValueVector3("uv0", vecTmp);
  }
  if (eResult == ChunkFile_Ok) {
    Vector vecTmp(m_Handles.m_vecBasisCoords[1].x,
                  m_Handles.m_vecBasisCoords[1].y,
                  (float)m_Basis.m_nAxesFlip[1]);
    eResult = pFile->WriteKeyValueVector3("uv1", vecTmp);
  }
  if (eResult == ChunkFile_Ok) {
    Vector vecTmp(m_Handles.m_vecBasisCoords[2].x,
                  m_Handles.m_vecBasisCoords[2].y,
                  (float)m_Basis.m_nAxesFlip[2]);
    eResult = pFile->WriteKeyValueVector3("uv2", vecTmp);
  }
  if (eResult == ChunkFile_Ok) {
    Vector vecTmp(m_Handles.m_vecBasisCoords[3].x,
                  m_Handles.m_vecBasisCoords[3].y, 0.0f);
  ...
  if (eResult == ChunkFile_Ok) {
    ...
    CMapWorld::FaceID_FaceListsToString(szSetValue, sizeof(szSetValue),
                                        &m_Faces, NULL);
