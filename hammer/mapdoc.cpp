...
ON_UPDATE_COMMAND_UI(ID_TOOLS_INSTANCES_SHOWTINTED,
                     OnUpdateToolsInstancesShowTinted)
ON_COMMAND(ID_TOOLS_INSTANCES_SHOWNORMAL, OnToolsInstancesShowNormal)
ON_UPDATE_COMMAND_UI(ID_TOOLS_INSTANCES_SHOWNORMAL,
                     OnUpdateToolsInstancesShowNormal)
...
ON_UPDATE_COMMAND_UI(ID_INSTANCING_CREATEMANIFEST,
                     &CMapDoc::OnUpdateInstancingCreatemanifest)
ON_COMMAND(ID_VERSIONCONTROL_CHECKINALL, &CMapDoc::OnInstancingCheckinAll)
ON_UPDATE_COMMAND_UI(ID_VERSIONCONTROL_CHECKINALL,
                     &CMapDoc::OnUpdateInstancingCheckinAll)
ON_COMMAND(ID_VERSIONCONTROL_CHECKOUTMANIFEST,
           &CMapDoc::OnInstancingCheckOutManifest)
ON_UPDATE_COMMAND_UI(ID_VERSIONCONTROL_CHECKOUTMANIFEST,
                     &CMapDoc::OnUpdateInstancingCheckOutManifest)
...
ON_UPDATE_COMMAND_UI(ID_VERSIONCONTROL_ADDMANIFEST,
                     &CMapDoc::OnUpdateInstancingAddManifest)
ON_COMMAND(ID_INSTANCES_COLLAPSEALL, &CMapDoc::OnInstancesCollapseAll)
ON_COMMAND(ID_INSTANCES_COLLAPSESELECTION,
           &CMapDoc::OnInstancesCollapseSelection)
...
ON_UPDATE_COMMAND_UI(ID_VIEW_QUICKHIDEVISGROUP,
                     OnQuickHide_UpdateCreateVisGroupFromHidden)
END_MESSAGE_MAP()
...
//Thus, through 			this function, brushes and entities can be located by ordinal,
//as 			reported by the MAP compile tools.
...
bool CMapDoc::LoadVMF(const char *pszFileName, int LoadFlags) {
  ...
  if ((LoadFlags & VMF_LOAD_ACTIVATE)) { // force rendering even if application
                                         // is not active
...
void CMapDoc::Postload(const char *pszFileName) {
  if (pszFileName[0]) { // this path needs to be set early so that instances may
                        // properly find their base path
  ...
  if (CMapSolid::GetBadSolidCount() > 0) {
    ...
    V_sprintf_safe(szError,
                   "For your information, %d solid(s) were not loaded due to "
                   "errors in the file. Would you like to Re-Save your map "
                   "with the invalid solids removed?",
                   CMapSolid::GetBadSolidCount());
    if (GetMainWnd()->MessageBox(szError, "Warning",
                                 MB_YESNO | MB_ICONQUESTION) == IDYES) {
      OnFileSave();
    }
  }
...
//-----------------------------------------------------------------------------
// Purpose: this function will hook up the manifest document to this map
// document Input  : pManifest - the manifest document
//-----------------------------------------------------------------------------
...
//			the selection.  If one is not editable, the entire
...
bool CMapDoc::CreateNewManifest(void) {
  ...
  if (IsModified()) {
    AfxMessageBox("Manifest was NOT created as map was not able to be saved!",
                  MB_OK);
    return false;
  }

  CManifest *pManifest = dynamic_cast<CManifest *>(
      APP()->pManifestDocTemplate->OpenDocumentFile(NULL));
  ...
  if (pManifest->AddExistingMap(GetPathName(), false)) {
    ...
    pManifest->UpdateAllViews(MAPVIEW_UPDATE_SELECTION | MAPVIEW_UPDATE_TOOL |
                              MAPVIEW_RENDER_NOW);
    AfxMessageBox(
        "Manifest was successfully created and has automatically been saved.",
        MB_OK);
  } else {
...
//			pszEntityNamePrefix
//-----------------------------------------------------------------------------
void CMapDoc::ManifestPaste(CMapWorld *pDestWorld, Vector vecOffset,
                            QAngle vecRotate, CMapClass *pParent,
                            bool bMakeEntityNamesUnique,
                            const char *pszEntityNamePrefix) {
  Paste(GetHammerClipboard()->Objects, GetHammerClipboard()->pSourceWorld,
        pDestWorld, vecOffset, vecRotate, pParent, bMakeEntityNamesUnique,
        pszEntityNamePrefix);
...
//-----------------------------------------------------------------------------
// Purpose: this handles the routing of letting instances know their map may
// have been updated Input  : pInstanceMapDoc - the map that was updated
//-----------------------------------------------------------------------------
  ...
  FOR_EACH_OBJ(*pChildren, pos) {
    ...
    if (pEntity && stricmp(pEntity->GetClassName(), "func_instance") == 0) {
      CMapInstance *pMapInstance =
          pEntity->GetChildOfType((CMapInstance *)NULL);
      if (pMapInstance) {
...
//-----------------------------------------------------------------------------
// Purpose: This function will collapse instances into the map ( and any
// children instances )
//-----------------------------------------------------------------------------
void CMapDoc::CollapseInstances(bool bOnlySelected) {
  ...
  if (AfxMessageBox(
          "Collapsing does not perform all of the operations that the BSP "
          "process does for instancing.  There may be some issues or "
          "differences between results.  Are you sure you want to do this?",
    ...
    if (pEntity && stricmp(pEntity->GetClassName(), "func_instance") == 0 &&
      ...
      if (pMapInstance) {
        if (pMapInstance->GetInstancedMap()) {
          ...
          SetActiveMapDoc(
              this); // just in case the last instance copy forces the map to
                     // close, we need to make ourselves active again
          ...
          PasteInstance(GetHammerClipboard()->Objects,
                        pMapInstance->GetInstancedMap()->GetMapWorld(),
  ...
  SetModifiedFlag(true);
  UpdateAllViews(MAPVIEW_UPDATE_SELECTION | MAPVIEW_UPDATE_TOOL |
                 MAPVIEW_RENDER_NOW);

  char temp[256];
  sprintf(temp, "A total of %d instances were collapsed into the main map.",
          InstanceCount);
...
//-----------------------------------------------------------------------------
// Purpose: this function will allow you to iterate through a string looking for
// $variables Input  : Text - the string to search through
//			StartPos - the starting index in the string
// Output : Result - returns the $variable if it exists
//			returns the string index after the variable if one is
//found, otherwise -1 if not found
//-----------------------------------------------------------------------------
...
void CMapDoc::PopulateInstanceParms_r(CMapEntity *pEntity,
                                      const CMapObjectList *pChildren,
  ...
  FOR_EACH_OBJ(*pChildren, pos) {
    ...
    if (pInstanceEntity && pInstanceEntity != pEntity) {
      for (int i = pInstanceEntity->GetFirstKeyValue();
           i != pInstanceEntity->GetInvalidKeyValue();
        ...
        while ((StartPos =
                    FindInstanceParm((char *)pValue, StartPos, Result)) != -1) {
...
//-----------------------------------------------------------------------------
// Purpose: this function will iterate through all entities of the current map
// looking for $variables.
//			any ones that are found will be populated as Parm key
//values.
...
void CMapDoc::PopulateInstanceParms(CMapEntity *pEntity) {
  ...
  for (int i = 0; i < ParmList.Count(); i++) {
    bool bFound = false;
    for (int j = pEntity->GetFirstKeyValue();
         j != pEntity->GetInvalidKeyValue(); j = pEntity->GetNextKeyValue(j)) {
...
//-----------------------------------------------------------------------------
// Purpose: this function will look through the instance for a
// func_instance_parms.  If one is
//			found, then it will populate the replace fields with the
//parm fields.
...
void CMapDoc::PopulateInstance(CMapEntity *pEntity) {
  ...
  pMapInstance->GetInstancedMap()->FindEntitiesByClassName(
      entityList, "func_instance_parms", false);
...
//-----------------------------------------------------------------------------
// Purpose: this function will notify all children that their owning instance
// has been moved.
//			currently not used.
//-----------------------------------------------------------------------------
...
//			manifest.  Generally this is the map layer that is
//editable where all 			entity / brush operations will take place.
...
//VISGROUP_UNDEFINED 				  before calling this with the first object.
//-----------------------------------------------------------------------------
...
void CMapDoc::OnCloseDocument(void) {
  if (m_nExternalReferenceCount >
      0) { // this is an instance, so hide the window
    ShowWindow(false);
...
void CMapDoc::SetActiveMapDoc(CMapDoc *pDoc) {
  ...
  if (m_pMapDoc != NULL) {
    ...
    if (m_pManifest && m_pManifest->GetPrimaryMap()) {
      CHistory::SetHistory(
          m_pManifest->GetPrimaryMap()->m_Map->GetDocHistory());
    } else {
...
//the only 			child of a solid entity, that entity is also deleted.
...
//-----------------------------------------------------------------------------
void CMapDoc::OnEditDelete(void) { Delete(); }
...
void CMapDoc::GetBestPastePoint(Vector &vecPasteOrigin) {
    ...
    if (ptClient.x >= 0 && ptClient.x < width && ptClient.y >= 0 &&
      ...
      if (p3DView) {
        ...
        if (p3DView->ObjectsAt(vPoint, &Hits, 1,
                               FLAG_OBJECTS_AT_RESOLVE_INSTANCES)) {
          ...
          if (pSolid != NULL) {
            ...
            dist = IntersectionLineAABBox(mins, maxs, vFinalStart, vFinalEnd,
                                          face);
  ...
  if (vecPasteOrigin.x == COORD_NOTINIT) {
    ...
    vecPasteOrigin +=
        GetHammerClipboard()->vecOriginalCenter - vecSnappedOriginalCenter;
  }
}

void CMapDoc::Paste(IHammerClipboard *pClipboard, CMapWorld *pDestWorld,
                    Vector vecOffset, QAngle vecRotate, CMapClass *pParent,
                    bool bMakeEntityNamesUnique,
                    const char *pszEntityNamePrefix) {
  CHammerClipboard *pSrc = GetHammerClipboard(pClipboard);

  Paste(pSrc->Objects, pSrc->pSourceWorld, pDestWorld, vecOffset, vecRotate,
        pParent, bMakeEntityNamesUnique, pszEntityNamePrefix);
}

//-----------------------------------------------------------------------------
// Purpose: This function will peform a 'cut' operation on the selected
// entities.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: This function will perform a 'copy' operation on the selected
// entities.
//-----------------------------------------------------------------------------
void CMapDoc::Copy(IHammerClipboard *pClipboard) {
  ...
  m_pSelection->GetBoundsCenter(
      GetHammerClipboard(pClipboard)->vecOriginalCenter);
  m_pSelection->GetBounds(GetHammerClipboard(pClipboard)->Bounds.bmins,
                          GetHammerClipboard(pClipboard)->Bounds.bmaxs);
...
//			pParent -
//-----------------------------------------------------------------------------
void CMapDoc::PasteInstance(CMapObjectList &Objects, CMapWorld *pSourceWorld,
                            CMapWorld *pDestWorld, Vector vecOffset,
                            QAngle vecRotate, CMapClass *pParent,
                            bool bMakeEntityNamesUnique,
                            const char *pszEntityNamePrefix) {
  QAngle vecRotateFix;
...
//-----------------------------------------------------------------------------
// Purpose: This function will perform a 'delete' operation on the selected
// entities.
//-----------------------------------------------------------------------------
...
void CMapDoc::OnEditPaste(void) {
  ...
  Paste(GetHammerClipboard()->Objects, GetHammerClipboard()->pSourceWorld,
        pWorld, vecPasteOffset, QAngle(0, 0, 0), NULL, false, NULL);
...
void CMapDoc::OnUpdateEditSelection(CCmdUI *pCmdUI) {
  bool bMorphSnap = m_pToolManager->GetActiveToolID() == TOOL_MORPH &&
                    pCmdUI->m_nID == ID_TOOLS_SNAPSELECTEDTOGRID;
  bool bStandardCommand =
      (m_pSelection->GetCount() != 0) && m_pSelection->IsCopyable() &&
      (m_pToolManager->GetActiveToolID() != TOOL_FACEEDIT_MATERIAL) &&
      !GetMainWnd()->IsShellSessionActive();
...
void CMapDoc::OnUpdateEditPaste(CCmdUI *pCmdUI) {
  pCmdUI->Enable(
      GetHammerClipboard()->Objects.Count() &&
      (m_pToolManager->GetActiveToolID() != TOOL_FACEEDIT_MATERIAL) &&
      !GetMainWnd()->IsShellSessionActive());
}
...
//.VMF, .RMF) 			strCompileExtension - The extension of the file to compile (eg.
//.VMF, .MAP)
//-----------------------------------------------------------------------------
...
void CMapDoc::OnFileRunmap(void) {
  ...
  if ((g_pGameConfig->GetTextureFormat() == tfWAD) &&
    ...
    AfxMessageBox("There are no texture files defined yet. Add some texture "
                  "files before you run the map.");
  ...
  if ((dlg.m_iQBSP) && (m_pGame->szBSP[0] != '\0')) {
    ...
    sprintf(cmd.szParms, "-game $gamedir %s$path\\$file.$ext",
  ...
  if (dlg.m_bNoQuake == FALSE) {
    ...
    // When running under steam, use applaunch always because if a user forgets
    // to run the game they're trying to launch, it will fail with an obscure
    // error they won't understand.
    {
      strcpy(cmd.szRun, "$game_exe");
      sprintf(cmd.szParms, "-game $gamedir %s +map $file -steam",
              (const char *)dlg.m_strQuakeParms);
    }
...
void CMapDoc::OnEditPastespecial(void) {
  ...
  for (int i = 0; i < dlg.m_iCopies; i++) {
    ...
    //
    Paste(GetHammerClipboard()->Objects, GetHammerClipboard()->pSourceWorld,
          pWorld, vecPasteOffset, vecPasteAngles, pParent, bMakeNamesUnique,
          pszPrefix);
...
void CMapDoc::OnUpdateEditPastespecial(CCmdUI *pCmdUI) {
  pCmdUI->Enable((GetHammerClipboard()->Objects.Count() != 0) &&
                 !GetMainWnd()->IsShellSessionActive());
}
...
//be unique within. 		bMakeUnique - Whether to guarantee that the names are unique
//in the world. 			If necessary, numbers will be appended to make the names unique.
//		szPrefix - A string to prepend to all named entities in the
//tree.
//-----------------------------------------------------------------------------
...
void CMapDoc::OnToolsCreateprefab(void) {
  if (m_pSelection->IsEmpty()) {
    AfxMessageBox("This feature creates a prefab with the selected objects. "
                  "You must select some objects before you can use it.",
...
void CMapDoc::OnToolsSnapselectedtogrid(void) {
  if (m_pToolManager->GetActiveToolID() == TOOL_MORPH) {
    Morph3D *pMorphTool =
        assert_cast<Morph3D *>(m_pToolManager->GetActiveTool());
...
void CMapDoc::OnMapLoadportalfile(void) {
  ...
  str.Format("Load default portal file?\n(%s)",
             m_pPortalFile->fileName.GetBuffer());
...
//size of the 			box, and another solid 1200 units bigger. subtract A from B, and
//			keep those brushes.
...
//the framework 			prompts the user to save changes before closing a document.
...
//-----------------------------------------------------------------------------
void CMapDoc::AddReference(void) { m_nExternalReferenceCount++; }

//-----------------------------------------------------------------------------
// Purpose: this function removes an external reference count from a map.  If it
// is hidden
...
bool CMapDoc::IsCulledBy3DCameraDistance(CMapClass *pObject,
                                         UpdateVisibilityData_t *pData) {
...
//selection.RemoveInvisibles 			may change the contents of the selection.
//Therefore, this should never 			be called from low-level code that might be
//inside an iteration of the 			selection.
//-----------------------------------------------------------------------------
...
//			pDoc -
//-----------------------------------------------------------------------------
BOOL CMapDoc::UpdateVisibilityCallback(CMapClass *pObject,
                                       UpdateVisibilityData_t *pData) {
  Assert(!pObject->IsTemporary());
  ...
  if (bVisible) {
    ...
    if ((dynamic_cast<CMapEntity *>(pObject)) != NULL ||
        (dynamic_cast<CMapWorld *>(pObject)) != NULL) {
...
void CMapDoc::UpdateVisibility(CMapClass *pObject) {
  ...
  if (pObject->IsGroup()) {
    pObject->EnumChildrenRecurseGroupsOnly(
        (ENUMMAPCHILDRENPROC)UpdateVisibilityCallback, (DWORD)&data);
  }
}
  ...
  if (nCount) {
    for (int i = 0; i < nCount; i++) {
      ...
      if (entry.pObject->m_pObject) {
        ...
        if (!bShowedWarning) {
          bShowedWarning = true;
          AfxMessageBox("ProcessNotifyList: encountered a deleted object. Tell "
                        "a programmer.",
                        MB_OK);
        }
      }
...
//-----------------------------------------------------------------------------
// Purpose: Toggles the state of Tools | Instances | Hide. When enabled,
...
//-----------------------------------------------------------------------------
// Purpose: Toggles the state of Tools | Instances | Show Tinted. When enabled,
// instances are tinted.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Toggles the state of Tools | Instances | Show Normal. When enabled,
// instances are shown just like the rest of the world.
//-----------------------------------------------------------------------------
...
//			a manifest, it will being the prompted to ask the user
//if he wants 			to convert it to one.
//-----------------------------------------------------------------------------
void CMapDoc::OnInstancingCreatemanifest() {
  ...
  if (!pManifest) {
    if (AfxMessageBox("Do you want to create a new manifest from this map?",
                      MB_YESNO) == IDNO) {
      return;
    }
...
//-----------------------------------------------------------------------------
// Purpose: This function checks to see if it should enable the Create Manifest
// menu item
//-----------------------------------------------------------------------------
...
void CMapDoc::OnUpdateInstancingCheckOutManifest(CCmdUI *pCmdUI) {
  ...
  if (pManifest->m_bIsVersionControlled == true &&
      pManifest->m_bCheckedOut == false) {
    pCmdUI->Enable(true);
  } else {
    pCmdUI->Enable(false);
  }
...
//-----------------------------------------------------------------------------
// Purpose: This function will collapse all instances into the map ( and any
// children instances )
//-----------------------------------------------------------------------------
void CMapDoc::OnInstancesCollapseAll() { CollapseInstances(false); }

//-----------------------------------------------------------------------------
// Purpose: This function will collapse all instances that are selected into the
// map ( and any children instances )
//-----------------------------------------------------------------------------
void CMapDoc::OnInstancesCollapseSelection() { CollapseInstances(true); }
  ...
  if (pGD != NULL) {
    ...
    if (pEntity != NULL) {
      ...
      for (gindex = 0; gindex < pGD->m_FGDAutoVisGroups.Count(); gindex++) {
        // Class
        for (cindex = 0;
             cindex < pGD->m_FGDAutoVisGroups[gindex].m_Classes.Count();
             cindex++) {
          // Object/Entity
          for (eindex = 0; eindex < pGD->m_FGDAutoVisGroups[gindex]
                                        .m_Classes[cindex]
                                        .szEntities.Count();
               eindex++) {
            //
            if (!V_stricmp(pEntity->GetClassName(),
                           pGD->m_FGDAutoVisGroups[gindex]
                               .m_Classes[cindex]
                               .szEntities[eindex])) {
              AddChildGroupToAutoVisGroup(
                  pEntity,
...
void CMapDoc::QuickHide_HideObjects(void) {
  ...
  for (int index = 0; index < pSelList->Count(); index++) {
    ...
    // If it's a Group or Entity, add each child - otherwise we get ghost group
    // selection (driller:  FIXME)
    ...
    if (pObject->GetParent()) {
      ...
      // Track all parents, so we can check if all children (eventually) end up
      // hidden
      while (pParent) {
        if (pParent->IsGroup() ||
            pParent->IsMapClass(MAPCLASS_TYPE(CMapEntity))) {
          if (m_QuickHideGroupedParents.Find(pParent) ==
              m_QuickHideGroupedParents.InvalidIndex()) {
  ...
  if (m_QuickHideGroupedParents.Count() > 0) {
    ...
    for (int index = 0; index < m_QuickHideGroupedParents.Count(); index++) {
      ...
      if (pParent) {
        ...
        // Child visible?
        while (pChild) {
          // With the way GetChildCount() works, we need to count nested
          // children, too
          nChildCount += pChild->GetChildCount();

          if (m_QuickHideGroup.Find(pChild) !=
              m_QuickHideGroup.InvalidIndex()) {
            nHiddenChildCount++;
          }
...
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapDoc::QuickHide_SaveVMF(CChunkFile *pFile,
                                             CSaveInfo *pSaveInfo) {
  if (m_QuickHideGroup.Count() == 0) {
    return (ChunkFile_Ok);
  }
...
//-----------------------------------------------------------------------------
void CMapDoc::OnQuickHide_HideObjects(void) { QuickHide_HideObjects(); }
...
//-----------------------------------------------------------------------------
void CMapDoc::OnQuickHide_Unhide(void) { QuickHide_Unhide(); }
