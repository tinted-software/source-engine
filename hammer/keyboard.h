...
//
// Defines the maximum number of physical keys. These physical keys correspond
// to the windows virtual key codes shown below. Missing key codes may be up for
// grabs, but it is probably safer to add to the end of the list.
//
...
//
// Defines the maximum number of logical keys. Logical keys are
// application-specific values that are associated with physical keys via
// AddKeyMap.
//
...
public:
  ...
  void AddKeyMap(unsigned int uChar, unsigned int uModifierKeys,
                 unsigned int uLogicalKey);
