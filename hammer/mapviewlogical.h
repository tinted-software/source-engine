...
protected:
  ...
  // Performs a selection which selects an object at the point as well as
  // entities connected to outputs.
...
private:
  ...
  const color32 &GetWireColor(const char *pszName, const bool bSelected,
                              const bool bBroken, const bool bAnySelected);
  void DrawConnectingWire(float x, float y, CMapEntity *pSource,
                          CEntityConnection *pConnection, CMapEntity *pTarget);
  ...
  CUtlVector<CMapClass *>
      m_ConnectionList; // list of all objects which are in the render list of
                        // have connections to something in the renderlist
  CUtlStack<CMapClass *>
      m_ConnectionUpdate; // for iteratively determining connectivity
