...
IMaterialProxy *CMaterialProxyFactory::CreateProxy(const char *proxyName) {
  ...
  Q_snprintf(proxyVersionedName, sizeof(proxyVersionedName), "%s%s", proxyName,
...
CWorldDimsProxy::CWorldDimsProxy() { m_pMinsVar = m_pMaxsVar = NULL; }
...
EXPOSE_INTERFACE(CWorldDimsProxy, IMaterialProxy,
                 "WorldDims" IMATERIAL_PROXY_INTERFACE_VERSION);
