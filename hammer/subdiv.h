...
class CSubdivPoint {
public:
  enum { POINT_ORDINARY = 0, POINT_CORNER = 1, POINT_CREASE = 2 };
  ...
  friend bool CompareSubdivPoints(const CSubdivPoint *pPoint1,
                                  const CSubdivPoint *pPoint2, float tolerance);
  friend bool CompareSubdivPointToPoint(const CSubdivPoint *pSubdivPoint,
...
public:
  ...
  friend bool CompareSubdivEdges(const CSubdivEdge *pEdge1,
                                 const CSubdivEdge *pEdge2);
};
...
private:
  ...
  //	CSubdivPoint	m_Points[MAX_SUBDIV_POINTS];		// mesh list of
  //subdivision verts

  int m_EdgeCount;
  int m_MaxEdgeCount;
  CSubdivEdge *m_pEdges;
  //	CSubdivEdge	    m_Edges[MAX_SUBDIV_POINTS];			// mesh
  //list of subdivision edges
  ...
  void SetEdgeData(CSubdivQuad *pRoot, int index, int parentIndex,
                   int subdivIndex);
