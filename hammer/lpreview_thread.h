...
struct MessageToLPreview {
  ...
  // this structure uses a fat format for the args instead of separate classes
  // for each message. the messages are small anyway, since pointers are used
  // for anything of size.
  FloatBitMap_t *m_pDefferedRenderingBMs[4]; // if LPREVIEW_MSG_G_BUFFERS
  CUtlVector<CLightingPreviewLightDescription>
      *m_pLightList;    // if LPREVIEW_MSG_LIGHT_DATA
