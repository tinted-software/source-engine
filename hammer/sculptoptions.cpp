...
CUtlMap<EditDispHandle_t, CMapDisp *>
    CSculptTool::m_OrigMapDisp(3, 3, CSculptTool::MapDispLessFunc);
...
//-----------------------------------------------------------------------------
CSculptTool::~CSculptTool() {
  FOR_EACH_MAP(m_OrigMapDisp, pos) { delete m_OrigMapDisp.Element(pos); }
  m_OrigMapDisp.Purge();
}
...
// Output : returns true if successful
//-----------------------------------------------------------------------------
bool CSculptTool::Paint(CMapView3D *pView, const Vector2D &vPoint,
...
//-----------------------------------------------------------------------------
// Purpose: determines if any of the special keys ( control, shift, alt ) are
// pressed
//-----------------------------------------------------------------------------
...
// Output : returns true if successful
//-----------------------------------------------------------------------------
bool CSculptTool::OnLMouseUp3D(CMapView3D *pView, UINT nFlags,
...
// Output : returns true if successful
//-----------------------------------------------------------------------------
bool CSculptTool::OnLMouseDown3D(CMapView3D *pView, UINT nFlags,
                                 const Vector2D &vPoint) {
  DetermineKeysDown();
...
// Output : returns true if successful
//-----------------------------------------------------------------------------
bool CSculptTool::OnRMouseUp3D(CMapView3D *pView, UINT nFlags,
                               const Vector2D &vPoint) {
  DetermineKeysDown();
...
// Output : returns true if successful
//-----------------------------------------------------------------------------
bool CSculptTool::OnRMouseDown3D(CMapView3D *pView, UINT nFlags,
                                 const Vector2D &vPoint) {
  DetermineKeysDown();
...
// Output : returns true if successful
//-----------------------------------------------------------------------------
bool CSculptTool::OnMouseMove3D(CMapView3D *pView, UINT nFlags,
                                const Vector2D &vPoint) {
  DetermineKeysDown();
...
bool CSculptTool::PrePaint(CMapView3D *pView, const Vector2D &vPoint) {
  ...
  m_OriginalCollisionValid = FindCollisionIntercept(
      pView->GetCamera(), vPoint, true, m_OriginalCollisionPoint,
      m_OriginalCollisionNormal, m_OriginalCollisionIntercept);
  if (m_OriginalCollisionValid) {
    m_OriginalProjectedRadius =
        CalcDistanceToLine(m_OriginalCollisionPoint, vecStart, vecEnd);
  }

  m_CurrentCollisionValid = FindCollisionIntercept(
      pView->GetCamera(), vPoint, false, m_CurrentCollisionPoint,
      m_CurrentCollisionNormal, m_CurrentCollisionIntercept);
  if (m_CurrentCollisionValid) {
    m_CurrentProjectedRadius =
        CalcDistanceToLine(m_CurrentCollisionPoint, vecStart, vecEnd);
  }

  m_SpatialData.m_flRadius = 128.0f;
  m_SpatialData.m_flRadius2 =
      (m_SpatialData.m_flRadius * m_SpatialData.m_flRadius);
...
//-----------------------------------------------------------------------------
// Purpose: called to dispatch the painting routine across all selected
// displacements Input  : pView - the 3d view
//			vPoint - the mouse point
// Output : returns true if successful
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: checks to see if a given displacement vert lies within the 2d
// screenspace of the circle Input  : pView - the 3d view
//			pDisp - the displacement the vert belongs to
//			pOrigDisp - the displacement prior to any moving
//			nVertIndex - the vert index
//			bUseOrigDisplacement - should we use the vert from the
//original displacement 			bUseCurrentPosition - should we use the current
//collision test point
// Output : returns true if the point is within the circle
//-----------------------------------------------------------------------------
bool CSculptTool::IsPointInScreenCircle(CMapView3D *pView, CMapDisp *pDisp,
                                        CMapDisp *pOrigDisp, int nVertIndex,
                                        bool bUseOrigDisplacement,
                                        bool bUseCurrentPosition,
                                        float *pflLengthPercent) {
...
//			pOrigDisp - the displacement prior to the paint
//operation
// Output :
//-----------------------------------------------------------------------------
void CSculptTool::DoPaintSmooth(CMapView3D *pView, const Vector2D &vPoint,
                                CMapDisp *pDisp, CMapDisp *pOrigDisp) {
  ...
  for (int iVert = 0; iVert < nVertCount; iVert++) {
    if (IsPointInScreenCircle(pView, pDisp, pOrigDisp, iVert, false, true)) {
      ...
      if (DoPaintSmoothOneOverExp(vVert, vPaintPos)) {
        AddToUndo(&pDisp);
        pDisp->Paint_SetValue(iVert, vPaintPos);
        //				Msg( "Vert %d Updated: from %g %g %g to
        //%g %g %g\n", iVert, vVert.x, vVert.y, vVert.z, vPaintPos.x,
        //vPaintPos.y, vPaintPos.z );
      }
    }
  }
}
...
//-----------------------------------------------------------------------------
bool CSculptTool::PaintSphereDispBBoxOverlap(const Vector &vCenter,
                                             float flRadius,
                                             const Vector &vBBoxMin,
...
//-----------------------------------------------------------------------------
bool CSculptTool::IsInSphereRadius(const Vector &vCenter, float flRadius2,
...
//-----------------------------------------------------------------------------
bool CSculptTool::DoPaintSmoothOneOverExp(const Vector &vNewCenter,
                                          Vector &vPaintPos) {
  // Get the displacement manager from the active map document.
  IWorldEditDispMgr *pDispMgr = GetActiveWorldEditDispManager();
  if (!pDispMgr)
  ...
  for (int iDisp = 0; iDisp < nDispCount; iDisp++) {
    ...
    if (pDisp) {
      ...
      if (PaintSphereDispBBoxOverlap(vNewCenter, flNewRadius, vBBoxMin,
                                     vBBoxMax)) {
        Vector vVert;
        int nVertCount = pDisp->GetSize();
        for (int iVert = 0; iVert < nVertCount; iVert++) {
          ...
          if (IsInSphereRadius(vNewCenter, flNewRadius2, vVert, flDistance2)) {
            ...
            Vector vProjectVert;
            float flProjectDist =
                DotProduct(vVert, m_SpatialData.m_vPaintAxis) - flPaintDist;
...
bool CSculptTool::GetStartingSpot(CMapView3D *pView, const Vector2D &vPoint) {
  m_ValidPaintingSpot = FindCollisionIntercept(
      pView->GetCamera(), vPoint, false, m_StartingCollisionPoint,
  ...
  if (m_ValidPaintingSpot) {
    ...
    RadiusPoint.x += m_BrushSize;
    pView->GetCamera()->BuildRay(RadiusPoint, vecStart, vecEnd);
    m_StartingProjectedRadius =
        CalcDistanceToLine(m_StartingCollisionPoint, vecStart, vecEnd);
  }
...
//			Towards - the color to be used if the direction is
//towards the viewer 			Away - the color to be used if the direction is away from
//the view
//-----------------------------------------------------------------------------
void CSculptTool::DrawDirection(CRender3D *pRender, Vector Direction,
                                Color Towards, Color Away) {
  ...
  pRender->DrawLine(Vector(m_MousePoint.x, m_MousePoint.y, 0.0f),
                    Vector(ViewVert.x, ViewVert.y, 0.0f));
  if (bPopMode) {
...
//-----------------------------------------------------------------------------
// Purpose: this function will initialize all selected displacements for
// updating
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: this function will find the collision location within the selected
// displacements Input  : pCamera - the camera
//			vPoint - the 2d point on screen
//			bUseOrigPosition - should we use the original
//displacements prior to updating
// Output : returns true if the point intercepted one of the selected
// displacements
//			vCollisionPoint the 3d interception point
//			vCollisionNormal - the normal of the tri hit
//			flCollisionIntercept - the intercept
//-----------------------------------------------------------------------------
bool CSculptTool::FindCollisionIntercept(CCamera *pCamera,
                                         const Vector2D &vPoint,
  ...
  for (int iDisp = 0; iDisp < nDispCount; iDisp++) {
    ...
    if (pDisp) {
      ...
      if (iTri != -1 &&
          (flLeastFraction == -1.0f || flFraction < flLeastFraction)) {
        ...
        ComputeTrianglePlane(vec1, vec2, vec3, vCollisionNormal,
...
// Output : returns true if successful
//-----------------------------------------------------------------------------
bool CSculptPainter::Paint(CMapView3D *pView, const Vector2D &vPoint,
                           SpatialPaintData_t &SpatialData) {
...
// Output : returns true if successful
//-----------------------------------------------------------------------------
bool CSculptPainter::OnLMouseUp3D(CMapView3D *pView, UINT nFlags,
                                  const Vector2D &vPoint) {
...
// Output : returns true if successful
//-----------------------------------------------------------------------------
bool CSculptPainter::OnLMouseDown3D(CMapView3D *pView, UINT nFlags,
                                    const Vector2D &vPoint) {
...
// Output : returns true if successful
//-----------------------------------------------------------------------------
bool CSculptPainter::OnRMouseUp3D(CMapView3D *pView, UINT nFlags,
                                  const Vector2D &vPoint) {
...
// Output : returns true if successful
//-----------------------------------------------------------------------------
bool CSculptPainter::OnRMouseDown3D(CMapView3D *pView, UINT nFlags,
                                    const Vector2D &vPoint) {
...
// Output : returns true if successful
//-----------------------------------------------------------------------------
bool CSculptPainter::OnMouseMove3D(CMapView3D *pView, UINT nFlags,
                                   const Vector2D &vPoint) {
...
void CSculptPushOptions::DoDataExchange(CDataExchange *pDX) {
  ...
  DDX_Control(pDX, IDC_SCULPT_PUSH_OPTION_OFFSET_DISTANCE,
              m_OffsetDistanceControl);
  ...
  DDX_Control(pDX, IDC_SCULPT_PUSH_OPTION_FALLOFF_POSITION,
              m_FalloffPositionControl);
  DDX_Control(pDX, IDC_SCULPT_PUSH_OPTION_FALLOFF_FINAL, m_FalloffFinalControl);
}

BEGIN_MESSAGE_MAP(CSculptPushOptions, CDialog)
ON_CBN_SELCHANGE(
    IDC_IDC_SCULPT_PUSH_OPTION_NORMAL_MODE,
    &CSculptPushOptions::OnCbnSelchangeIdcSculptPushOptionNormalMode)
ON_CBN_SELCHANGE(IDC_SCULPT_PUSH_OPTION_OFFSET_MODE,
                 &CSculptPushOptions::OnCbnSelchangeSculptPushOptionOffsetMode)
ON_EN_CHANGE(IDC_SCULPT_PUSH_OPTION_OFFSET_DISTANCE,
             &CSculptPushOptions::OnEnChangeSculptPushOptionOffsetDistance)
...
ON_EN_KILLFOCUS(
    IDC_SCULPT_PUSH_OPTION_FALLOFF_POSITION,
...
void CSculptPushOptions::RenderTool3D(CRender3D *pRender) {
  ...
  if (m_InSizingMode) { // yellow for sizing mode
    pRender->BeginClientSpace();
    pRender->SetDrawColor(255, 255, 0);
    pRender->DrawCircle(
        Vector(m_StartSizingPoint.x, m_StartSizingPoint.y, 0.0f),
        Vector(0.0f, 0.0f, 1.0f), m_BrushSize, 32);
    if (m_flFalloffSpot > 0.0f) {
      pRender->SetDrawColor(192, 192, 0);
      pRender->DrawCircle(
          Vector(m_StartSizingPoint.x, m_StartSizingPoint.y, 0.0f),
          Vector(0.0f, 0.0f, 1.0f), m_BrushSize * m_flFalloffSpot, 32);
    }
    pRender->EndClientSpace();
  } else if (m_bShiftDown) { // purple for smoothing
    pRender->SetDrawColor(255, 0, 255);
    pRender->BeginClientSpace();
    pRender->DrawCircle(Vector(m_MousePoint.x, m_MousePoint.y, 0.0f),
                        Vector(0.0f, 0.0f, 1.0f), m_BrushSize, 32);
    pRender->EndClientSpace();
  } else if (m_bCtrlDown) { // red for negative sculpting
    pRender->BeginClientSpace();
    ...
    DrawDirection(pRender, -vPaintAxis, Color(255, 255, 255),
                  Color(255, 128, 128));
  } else { // green for positive sculpting
    pRender->BeginClientSpace();
...
// Output : returns true if successful
//-----------------------------------------------------------------------------
bool CSculptPushOptions::OnRMouseDown3D(CMapView3D *pView, UINT nFlags,
                                        const Vector2D &vPoint) {
  CSculptTool::OnRMouseDown3D(pView, nFlags, vPoint);
  ...
  if (m_bAltDown) {
    ...
    if (FindCollisionIntercept(pView->GetCamera(), vPoint, false,
...
//-----------------------------------------------------------------------------
void CSculptPushOptions::GetPaintAxis(CCamera *pCamera, const Vector2D &vPoint,
                                      Vector &vPaintAxis) {
  switch (m_NormalMode) {
  ...
  case NORMAL_MODE_BRUSH_CENTER:
    if (!m_InPaintingMode) {
      ...
      FindCollisionIntercept(pCamera, vPoint, false, CollisionPoint,
                             CollisionNormal, CollisionIntercept);
...
//			pOrigDisp - the original displacement prior to any
//adjustments
//-----------------------------------------------------------------------------
void CSculptPushOptions::DoPaintOperation(CMapView3D *pView,
                                          const Vector2D &vPoint,
                                          CMapDisp *pDisp,
                                          CMapDisp *pOrigDisp) {
  ...
  int nVertCount = pDisp->GetSize();
  for (int iVert = 0; iVert < nVertCount; iVert++) {
    if (IsPointInScreenCircle(pView, pDisp, pOrigDisp, iVert, true, false,
                              &flLengthPercent)) {
      pDisp->GetVert(iVert, vVert);

      if (flLengthPercent > m_flFalloffSpot) {
        flLengthPercent =
            (flLengthPercent - m_flFalloffSpot) / (1.0f - m_flFalloffSpot);
        flLengthPercent = 1.0 - flLengthPercent;
        flDistance = ((1.0f - m_flFalloffEndingValue) * flLengthPercent *
                      flMaxDistance) +
...
ON_CBN_SELCHANGE(IDC_SCULPT_PUSH_OPTION_OFFSET_MODE,
                 &CSculptCarveOptions::OnCbnSelchangeSculptPushOptionOffsetMode)
ON_EN_CHANGE(IDC_SCULPT_PUSH_OPTION_OFFSET_DISTANCE,
             &CSculptCarveOptions::OnEnChangeSculptPushOptionOffsetDistance)
ON_CBN_SELCHANGE(
    IDC_SCULPT_PUSH_OPTION_DENSITY_MODE,
...
// Output : returns true if successful
//-----------------------------------------------------------------------------
bool CSculptCarveOptions::BeginPaint(CMapView3D *pView,
                                     const Vector2D &vPoint) {
  __super::BeginPaint(pView, vPoint);
...
//			vNormal - the normal we found
//-----------------------------------------------------------------------------
bool CSculptCarveOptions::CalculateQueuePoint(Vector2D &vPoint,
                                              Vector2D &vNormal) {
  float count = 0.0;
  Vector2D vAverage(0.0f, 0.0f);
  ...
  for (int j = PointIndex - 1; j >= 0; j--) {
    ...
    Vector2D vDiff(m_PointQueue[index2].x - m_PointQueue[index].x,
                   m_PointQueue[index2].y - m_PointQueue[index].y);
    float Length = Vector2DNormalize(vDiff);
...
//			bDrawIt - if we should add this point to the draw /
//normal lists
//-----------------------------------------------------------------------------
...
void CSculptCarveOptions::RenderTool3D(CRender3D *pRender) {
  ...
  if (CalculateQueuePoint(vMousePoint, vMouseNormal)) {
    ...
    pRender->DrawLine(Vector(vMousePoint.x, vMousePoint.y, 0.0f),
                      Vector(vMousePoint.x + vRight.x * m_BrushSize,
                             vMousePoint.y + vRight.y * m_BrushSize, 0.0f));
    pRender->DrawLine(Vector(vMousePoint.x, vMousePoint.y, 0.0f),
                      Vector(vMousePoint.x - (vRight.x * m_BrushSize),
                             vMousePoint.y - (vRight.y * m_BrushSize), 0.0f));
  }
...
// Output : returns true if successful
//-----------------------------------------------------------------------------
bool CSculptCarveOptions::OnLMouseUp3D(CMapView3D *pView, UINT nFlags,
                                       const Vector2D &vPoint) {
...
// Output : returns true if successful
//-----------------------------------------------------------------------------
bool CSculptCarveOptions::OnLMouseDown3D(CMapView3D *pView, UINT nFlags,
                                         const Vector2D &vPoint) {
...
// Output : returns true if successful
//-----------------------------------------------------------------------------
bool CSculptCarveOptions::OnRMouseDown3D(CMapView3D *pView, UINT nFlags,
                                         const Vector2D &vPoint) {
...
// Output : returns true if successful
//-----------------------------------------------------------------------------
bool CSculptCarveOptions::OnMouseMove3D(CMapView3D *pView, UINT nFlags,
                                        const Vector2D &vPoint) {
...
// Output : vPaintAxis - the direction the painting should go
//-----------------------------------------------------------------------------
void CSculptCarveOptions::GetPaintAxis(CCamera *pCamera, const Vector2D &vPoint,
                                       Vector &vPaintAxis) {
  switch (m_NormalMode) {
  case NORMAL_MODE_SCREEN:
    pCamera->GetViewForward(vPaintAxis);
...
//-----------------------------------------------------------------------------
bool CSculptCarveOptions::IsPointAffected(CMapView3D *pView, CMapDisp *pDisp,
                                          CMapDisp *pOrigDisp, int vertIndex,
                                          int nBrushPoint, Vector2D &vViewVert,
                                          bool bUseOrigDisplacement,
                                          bool bUseCurrentPosition) {
  Vector vVert, vTestVert;
  ...
  float tolerance =
      DotProduct2D(m_DrawNormal[nBrushPoint], m_DrawNormal[nBrushPoint - 1]);
  if (tolerance <= 0.5f) {
    return false;
  }
  ...
  distance = DotProduct2D(m_DrawNormal[nBrushPoint - 1],
                          m_DrawPoints[nBrushPoint - 1]);
...
//adjustments
//-----------------------------------------------------------------------------
void CSculptCarveOptions::DoPaintOperation(CMapView3D *pView,
                                           const Vector2D &vPoint,
                                           CMapDisp *pDisp,
                                           CMapDisp *pOrigDisp) {
  Vector vPaintPos, vVert, vDirection;
  ...
  int nVertCount = pDisp->GetSize();
  for (int iVert = 0; iVert < nVertCount; iVert++) {
    if (IsPointAffected(pView, pDisp, pOrigDisp, iVert, nTestPoint,
                        vViewVert)) {
      pDisp->GetVert(iVert, vVert);

      Vector2D vRight(-m_DrawNormal[nTestPoint].y, m_DrawNormal[nTestPoint].x);
      float fLineDistance = DotProduct2D(vRight, m_DrawPoints[nTestPoint]) -
                            DotProduct2D(vRight, vViewVert);
...
void CSculptCarveOptions::AdjustBrush(int x, int y) {
  ...
  if (MousePoint.x >= WindowRect.left && MousePoint.x < WindowRect.right &&
    ...
    float center = (WindowRect.bottom + WindowRect.top) / 2;
    float value =
        (center - MousePoint.y) / (WindowRect.bottom - WindowRect.top) * 2.0f;
