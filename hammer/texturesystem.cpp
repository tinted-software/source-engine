...
//
//			There is one texture context per game configuration in
//GameCfg.ini.
//
//=============================================================================//
...
void CMaterialFileChangeWatcher::Init(CTextureSystem *pSystem, intp context) {
  ...
  if (g_pFullFileSystem->GetSearchPath("GAME", false, searchPaths,
    ...
    Warning("Error in GetSearchPath. Dynamic material list updating will not "
            "be available.");
  }
}

void CMaterialFileChangeWatcher::OnFileChange(const char *pRelativeFilename,
...
//-----------------------------------------------------------------------------
// Purpose: Constructor. Creates the "All" group and sets it as the active
// group.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
IEditorTexture *
CTextureSystem::EnumActiveTextures(int *piIndex,
                                   TEXTUREFORMAT eDesiredFormat) const {
  Assert(piIndex != NULL);
    ...
    if (m_pActiveGroup != NULL) {
      ...
      do {
        ...
        if (pTex != NULL) {
          ...
          if ((eDesiredFormat == tfNone) ||
              (pTex->GetTextureFormat() == eDesiredFormat)) {
            return (pTex);
          }
        }
      } while (pTex != NULL);
...
// Output :
//-----------------------------------------------------------------------------
IEditorTexture *CTextureSystem::FindActiveTexture(LPCSTR pszInputName,
                                                  int *piIndex, BOOL bDummy) {

  // The .vmf file format gets confused if there are backslashes in material
  // names, so make sure they're all using forward slashes here.
  ...
  // We're finding by name, so we don't care what the format is as long as the
  // name matches.
  if (m_pActiveGroup) {
  ...
  //
  // Check the list of dummies for a texture with the same name and texture
  // format.
  //
  if (m_pActiveContext) {
...
//-----------------------------------------------------------------------------
// Purpose: Returns the texture context that corresponds to the given game
// config.
//-----------------------------------------------------------------------------
TextureContext_t *
CTextureSystem::FindTextureContextForConfig(CGameConfig *pConfig) {
...
void HammerFileSystem_ReportSearchPath(const char *szPathID) {
  ...
  g_pFullFileSystem->GetSearchPath(szPathID, true, szSearchPath,
                                   sizeof(szSearchPath));
...
void CTextureSystem::LoadAllGraphicsFiles(void) {
  ...
  // for (int nConfig = 0; nConfig < Options.configs.GetGameConfigCount();
  // nConfig++)
  {
...
void CTextureSystem::LoadMaterials(CGameConfig *pConfig) {
  ...
  CMaterial::EnumerateMaterials(this, "materials", (int)pGroup,
  ...
  m_pCubemapTexture =
      MaterialSystemInterface()->FindTexture("editor/cubemap", NULL, true);
...
void CTextureSystem::OnFileChange(const char *pFilename, intp context,
                                  CTextureSystem::EFileType eFileType) {
  ...
  if (eFileType == k_eFileTypeVMT) {
    ...
    // Whether a VTF was added, removed, or modified, we do the same thing..
    // refresh it and any materials that reference it.
    ITexture *pTexture =
        materials->FindTexture(fixedSlashes, TEXTURE_GROUP_UNACCOUNTED, false);
    if (pTexture) {
...
//-----------------------------------------------------------------------------
// Purpose: Load any materials that reference this texture. Used so we can
// refresh a material's preview image if a relevant .vtf changes.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Figure out the file type from its extension. Returns false if we
// don't have an enum for that extension.
//-----------------------------------------------------------------------------
bool CTextureSystem::GetFileTypeFromFilename(
    const char *pFilename, CTextureSystem::EFileType *pFileType) {
  char strRight[16];
...
//-----------------------------------------------------------------------------
static int __cdecl SortTexturesProc(IEditorTexture *const *elem1,
                                    IEditorTexture *const *elem2) {
...
//-----------------------------------------------------------------------------
bool CTextureSystem::FindGraphicsFile(GRAPHICSFILESTRUCT *pFileInfo, DWORD id,
                                      int *piIndex) {
...
//			pGroup -
//-----------------------------------------------------------------------------
void CTextureSystem::LoadGraphicsFileWAD3(GRAPHICSFILESTRUCT *pFile, int fd,
...
DWORD CTextureSystem::LoadGraphicsFile(const char *pFilename) {
  ...
  if (!(dwAttrib & FILE_ATTRIBUTE_DIRECTORY)) {
    ...
    if (!memcmp(buf, "WAD3", 4)) {
      ...
      Q_snprintf(
          str, sizeof(str),
          "The file \"%s\" is not a valid WAD3 file and will not be used.",
          pFilename);
...
// Output : Returns TRUE if textures of a given format are available, FALSE if
// not.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
IEditorTexture *CTextureGroup::FindTextureByName(const char *pName,
                                                 int *piIndex,
