...
//
// Holds undo information for a single object, due to a single operation. Held
// by a CHistoryTrack.
//
class CTrackEntry {
public:
...
protected:
  ...
  union {
    struct {
      CMapClass *pCurrent;    // Pointer to the object as it currently exists in
                              // the world.
      CMapClass *pKeptObject; // Pointer to a copy of the object at the time it
                              // was kept.
    } m_Copy;

    struct {
      CMapClass
          *pDeleted; // Pointer to the object that was deleted from the world.
      CMapClass *pKeptParent; // Pointer to the object's parent at the time of
                              // deletion.
    } m_Delete;

    struct {
      CMapClass *pCreated; // Pointer to the object that was created and added
                           // to the world.
    } m_Create;
  };
...
//
// Tracks all the objects changed by a single operation, such as "Nudge Objects"
// or "Delete". Each track contains one track entry per object affected by the
// operation.
//
class CHistoryTrack {
public:
...
public:
  ...
  // mark undo position:
  void MarkUndoPosition(const CMapObjectList *pSelection = NULL,
                        LPCTSTR pszName = NULL, BOOL = FALSE);
...
//-----------------------------------------------------------------------------
void CHistory::SetDocument(CMapDoc *pDoc) { m_pDoc = pDoc; }

//-----------------------------------------------------------------------------
// Purpose: Returns the document that this undo history belongs to.
//-----------------------------------------------------------------------------
CMapDoc *CHistory::GetDocument(void) { return (m_pDoc); }
