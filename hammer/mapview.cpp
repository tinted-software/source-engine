...
bool CMapView::IsOrthographic() { return m_pCamera->IsOrthographic(); }
...
//			determines whether or not we are exceeding the framerate
//limit.
  ...
  if (bFace) {
    ...
    FOR_EACH_OBJ(*pList, pos) {
      ...
      if (pObject->IsMapClass(MAPCLASS_TYPE(CMapSolid))) {
        pDoc->SelectFace((CMapSolid *)pObject, -1,
                         scSelect | (bMakeFirst ? scClear : 0));
        break;
      }
    }
