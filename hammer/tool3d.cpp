...
void Tool3D::StartTranslation(CMapView *pView, const Vector2D &vClickPoint,
...
bool Tool3D::UpdateTranslation(CMapView *pView, const Vector2D &vPoint,
                               UINT nFlags) {
  Vector vTransform;
...
int Tool3D::HitTest(CMapView *pView, const Vector &vecWorld,
                    bool bTestHandles) {
  Vector2D vClient;
...
bool Tool3D::HitRect(CMapView *pView, const Vector2D &vPoint,
                     const Vector &vCenter, int extent) {
  Vector2D vClientCenter;
  pView->WorldToClient(vClientCenter, vCenter);

  if (vPoint.x < (vClientCenter.x - extent) ||
      vPoint.x > (vClientCenter.x + extent))
    return false;

  if (vPoint.y < (vClientCenter.y - extent) ||
      vPoint.y > (vClientCenter.y + extent))
    return false;
...
void Tool3D::SetTransformationPlane(const Vector &vOrigin, const Vector &vHorz,
                                    const Vector &vVert,
...
void Tool3D::ProjectOnTranslationPlane(const Vector &vWorld, Vector &vTransform,
                                       int nFlags) {
  if (!nFlags) {
    ...
    if (!SolveLinearEquation(v0, m_vPlaneHorz, m_vPlaneVert, m_vPlaneNormal,
                             vOut)) {
      vTransform.Init();
      return;
    }
...
void Tool3D::ProjectTranslation(CMapView *pView, const Vector2D &vPoint,
                                Vector &vTransform, int nFlags) {
  Vector vStart, vEnd;
...
bool Tool3D::OnLMouseDown2D(CMapView2D *pView, UINT nFlags,
...
bool Tool3D::OnLMouseUp2D(CMapView2D *pView, UINT nFlags,
                          const Vector2D &vPoint) {
  m_vMousePos = vPoint;
...
bool Tool3D::OnRMouseDown2D(CMapView2D *pView, UINT nFlags,
                            const Vector2D &vPoint) {
...
bool Tool3D::OnRMouseUp2D(CMapView2D *pView, UINT nFlags,
                          const Vector2D &vPoint) {
  m_vMousePos = vPoint;
...
bool Tool3D::OnMouseMove2D(CMapView2D *pView, UINT nFlags,
                           const Vector2D &vPoint) {
  m_vMousePos = vPoint;
...
bool Tool3D::OnLMouseDown3D(CMapView3D *pView, UINT nFlags,
                            const Vector2D &vPoint) {
  m_bMouseDown[MOUSE_LEFT] = true;
  m_bMouseDragged[MOUSE_LEFT] = false;
  m_vMousePos = m_vMouseStart[MOUSE_LEFT] = vPoint;
...
bool Tool3D::OnLMouseUp3D(CMapView3D *pView, UINT nFlags,
                          const Vector2D &vPoint) {
  m_vMousePos = vPoint;
  m_bMouseDown[MOUSE_LEFT] = false;
  m_bMouseDragged[MOUSE_LEFT] = false;
...
bool Tool3D::OnRMouseDown3D(CMapView3D *pView, UINT nFlags,
                            const Vector2D &vPoint) {
  m_bMouseDown[MOUSE_RIGHT] = true;
  m_bMouseDragged[MOUSE_RIGHT] = false;
  m_vMousePos = m_vMouseStart[MOUSE_RIGHT] = vPoint;
...
bool Tool3D::OnRMouseUp3D(CMapView3D *pView, UINT nFlags,
                          const Vector2D &vPoint) {
  m_vMousePos = vPoint;
  m_bMouseDown[MOUSE_RIGHT] = false;
  m_bMouseDragged[MOUSE_RIGHT] = false;
...
bool Tool3D::OnMouseMove3D(CMapView3D *pView, UINT nFlags,
                           const Vector2D &vPoint) {
  m_vMousePos = vPoint;
  for (int i = 0; i < 2; i++) {
    if (m_bMouseDown[i]) {
