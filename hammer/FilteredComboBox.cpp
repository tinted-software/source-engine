...
void CFilteredComboBox::SetSuggestions(CUtlVector<CString> &suggestions,
                                       int flags) {
  CreateFonts();

  // Verify some of the window styles. This class requires these, and it doesn't
  // get a change to set them unless you call Create on it. If we use owner draw
  // variable, we get the bug described here:
  ...
  // Force it to provide the first one if they only want suggestions and the
  // current text in there is not valid.
...
void CFilteredComboBox::SelectItem(const char *pStr) {
  ...
  if (iCurSel != CB_ERR) {
    ...
    if (Q_stricmp(pStr, str) == 0) {
      // Make sure the edit control has the right text in there. If they called
      // ForceEditControlText, then it might not.
      CString strWindow;
...
CString CFilteredComboBox::GetCurrentItem() { return m_LastTextChangedValue; }
...
void CFilteredComboBox::InternalSetEditControlFont(HFONT hFont,
...
void CFilteredComboBox::SetOnlyProvideSuggestions(
    bool bOnlyProvideSuggestions) {
  m_bOnlyProvideSuggestions = bOnlyProvideSuggestions;
}

void CFilteredComboBox::FillDropdownList(const char *pInitialSel,
                                         bool bEnableRedraw) {
  ...
  if (pInitialSel) {
    ...
    if (m_bOnlyProvideSuggestions) {
      ...
      if (!InternalSelectItemByName(str)) {
        // Add the typed text to the combobox here otherwise it'll select the
        // nearest match when they drop it down with the mouse.
        AddString(str);
...
LRESULT CFilteredComboBox::DefWindowProc(UINT message, WPARAM wParam,
                                         LPARAM lParam) {
  // We handle the enter key specifically because the default combo box behavior
  // is to reset the text and all this stuff we don't want.
...
BOOL CFilteredComboBox::PreCreateWindow(CREATESTRUCT &cs) {
  ...
  // If we use CBS_OWNERDRAWVARIABLE, then we run into this bug:
  // http://support.microsoft.com/kb/813791.
...
void CFilteredComboBox::OnEnterKeyPressed(const char *pForceText) {
  ...
  // Must do this before ShowDropDown because that will change these variables
  // underneath us.
  CString szTypedText;
  DWORD sel;
  if (pForceText) {
  ...
  // They can get into here a variety of ways. Editing followed by enter.
  // Editing+arrow keys, followed by enter, etc.
  if (m_bOnlyProvideSuggestions) {
    CString str;
    if (FindSuggestion(szTypedText) == -1 &&
        m_pCallbacks->OnUnknownEntry(szTypedText)) {
      // They want us to KEEP this unknown entry, so add it to our list and
      // select it.
      m_Suggestions.AddToTail(szTypedText);
      str = szTypedText;
    } else {
      // They returned false, so do the default behavior: go to the best match
      // we can find.
...
BOOL CFilteredComboBox::OnEditChange() {
  ...
  // Add the typed text to the combobox here otherwise it'll select the nearest
  // match when they drop it down with the mouse.
  if (!m_bOnlyProvideSuggestions && FindSuggestion(szTypedText) == -1)
    AddString(szTypedText);

  // Note: for arcane and unspeakable MFC reasons, the placement of this call is
  // VERY sensitive. For example, if CTargetNameComboBox changes from a bold
  // font to a normal font, then if this call comes before ResetContent(), it
  // will resize the dropdown listbox to a small size and not size it back until
  // it is cloesd and opened again.
  ShowDropDown();
...
void CFilteredComboBox::GetItemsMatchingString(
    const char *pStringToMatch, CUtlVector<CString> &matchingItems) {
  for (int i = 0; i < m_Suggestions.Count(); i++) {
...
int CFilteredComboBox::SortFn(const CString *pItem1, const CString *pItem2) {
  // If one of them matches the prefix we're looking at, then that one should be
  // listed first. Otherwise, just do an alphabetical sort.
  bool bPrefixMatch1 = false, bPrefixMatch2 = false;
  if (s_pStringToMatch) {
    bPrefixMatch1 =
        (V_strnistr(*pItem1, s_pStringToMatch, s_iStringToMatchLen) != NULL);
    bPrefixMatch2 =
        (V_strnistr(*pItem2, s_pStringToMatch, s_iStringToMatchLen) != NULL);
  }
...
bool CFilteredComboBox::MatchString(const char *pStringToMatchStart,
  ...
  while (*pTestStringStart) {
    ...
    while (1) {
      ...
      // If we're at the end of pStringToMatch with no mismatch, then treat this
      // as a prefix match. If we're at the end of pTestString, but
      // pStringToMatch has more to go, then it's not a match.
      if (*pStringToMatch == 0)
        return true;
