...
public:
  ...
  // Factory function.
  //
  static CMapClass *CreateMapSideList(CHelperInfo *pHelperInfo,
                                      CMapEntity *pParent);
...
protected:
  ...
  void ReplaceFacesInCopy(CMapSideList *pCopy,
                          const CMapObjectList &OriginalList,
                          CMapObjectList &NewList);
  ...
  char m_szKeyName[80]; // The name of the key in our parent entity that we
                        // represent.
  CMapFaceList m_Faces; // The list of solid faces that this object manages.
  CUtlVector<int>
      m_LostFaceIDs; // The list of face IDs that were in m_Faces, but were lost
                     // during this session through the deletion of their solid.
};
