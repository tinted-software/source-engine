...
public:
  ...
  // Factories.
  //
  static CMapClass *CreateMapSprite(CHelperInfo *pHelperInfo,
                                    CMapEntity *pParent);
  ...
  void RenderLogicalAt(CRender2D *pRender, const Vector2D &vecMins,
...
protected:
  ...
  void SpriteColor(unsigned char *pColor, int eRenderMode, colorVec RenderColor,
                   int alpha);
  ...
  float m_fSecondsPerFrame;      // How many seconds to render each frame before
                                 // advancing.
  float m_fElapsedTimeThisFrame; // How many seconds we have rendered this
                                 // sprite frame so far.
  ...
  bool m_bIsIcon; // If true, this sprite is an iconic representation of an
                  // entity.
};
