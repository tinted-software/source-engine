...
//
#define CREATE_BUILD_PLANE_POINTS                                              \
  0x0001 // Whether to regenerate the 3 plane points from the generated face
         // points.
...
public:
  ...
  bool Subtract(CMapObjectList *pInside, CMapObjectList *pOutside,
  ...
  virtual void SetRenderColor(unsigned char uchRed, unsigned char uchGreen,
  ...
  int GetFaceIndex(
      CMapFace *pFace); // Returns the index (you could use it with GetFace) or
                        // -1 if the face doesn't exist in this solid.
...
protected:
  ...
  // dvs: brought in from old carve code; should be reconciled with AddFace,
  // Split, Subtract
  bool AddPlane(const CMapFace *p);
  bool Carve(CMapObjectList *pInside, CMapObjectList *pOutside,
             CMapSolid *pCarver);
  ...
  // Serialization.
  //
  static ChunkFileResult_t LoadSideCallback(CChunkFile *pFile,
                                            CMapSolid *pSolid);
  ...
  HL1_SolidType_t m_eSolidType; // Used for HalfLife 1 maps only - solid, water,
                                // slime, lava.
};
