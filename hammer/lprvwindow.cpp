...
void CLightingPreviewResultsWindow::Create(CWnd *pParentWnd) {
  ...
  CWnd::CreateEx(0, LPreviewWndClassName, "LightingPreviewWindow",
                 WS_OVERLAPPEDWINDOW | WS_SIZEBOX, rect, NULL, NULL, NULL);
}
//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
...
void CLightingPreviewResultsWindow::OnPaint(void) {
  ...
  if (g_pLPreviewOutputBitmap) {
    ...
    StretchDIBits(dc.GetSafeHdc(), clientrect.left, clientrect.top,
                  1 + (clientrect.right - clientrect.left),
                  1 + (clientrect.bottom - clientrect.top), 0, 0,
                  g_pLPreviewOutputBitmap->Width(),
                  g_pLPreviewOutputBitmap->Height(),
