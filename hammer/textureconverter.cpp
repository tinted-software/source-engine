...
void CTextureConverter::ConvertWorldTextures(CMapWorld *pWorld) {
  ...
  AfxMessageBox("Conversion complete.  Check the Hammer \"Messages\" window "
...
void CTextureConverter::ConvertDecalTexture(CMapEntity *pEnt) {
  ...
  default:
    ...
    MsgConvertDecal(pEnt,
                    "WARNING: Multiple matches found.  Using first match (%s).",
                    pNewTexture->GetName());
    m_nWarnings++;
...
void CTextureConverter::ReplaceFaceTexture(CMapFace *pFace,
  ...
  {
    MsgConvertFace(pFace, "WARNING: Couldn't load new material.  Texture "
...
void CTextureConverter::RescaleFaceTexture(CMapFace *pFace,
  ...
  pFace->CalcTextureCoords(); // recompute internals base on the new scaling and
                              // shifting.
}
