...
struct CheckListItem {
  ...
  inline bool operator==(const CheckListItem &first) const {
    return ((first.nItemBit == nItemBit) &&
            (!Q_strcmp(first.pszItemString, pszItemString)));
  }
};
...
public:
  ...
  // This needs to be set because we have to notify the entity page when the
  // value changes.
  ...
  // Called when the entity tab changes the spawnflags, which renders our data
  // obsolete.
