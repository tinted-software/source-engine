...
//         e3
//
// Note: edges refer to internal edge points only, corners "contain" all
// surfaces
//       touching the corner (surfaces that only touch the corner, as well as
//       those "edges" that end/begin at the corner(s))
//
...
struct SewEdgeData_t {
  ...
  CMapFace
      *pFaces[DISPSEW_FACES_AT_EDGE];  // the faces contributing to the edge sew
  int ndxEdges[DISPSEW_FACES_AT_EDGE]; // the faces' edge indices contributing
                                       // to the edge sew
  int type[DISPSEW_FACES_AT_EDGE];     // the type of edge t-junction, match
                                       // t-junction start, etc....
};

struct SewCornerData_t {
  int faceCount; // number of faces contributing to the corner sew
  CMapFace *pFaces[DISPSEW_FACES_AT_CORNER]; // the faces contributing to the
                                             // corner sew
  int ndxCorners[DISPSEW_FACES_AT_CORNER];   // the faces' corner indices
                                             // contributing to the corner sew
};

struct SewTJuncData_t {
  int faceCount; // number of faces contributing to the t-junction sew
  CMapFace *pFaces[DISPSEW_FACES_AT_TJUNC]; // the faces contributing to the
                                            // t-junction sew
  int ndxCorners[DISPSEW_FACES_AT_TJUNC];   // the faces' corner indices
                                          // contributing to the t-junction sew
  int ndxEdges[DISPSEW_FACES_AT_TJUNC]; // the faces' edge (midpoint) indices
                                        // contributing to the t-junction sew
};
...
//          tolerance
//-----------------------------------------------------------------------------
bool PointCompareWithTolerance(Vector const &pt1, Vector const &pt2,
                               float tolerance) {
  for (int i = 0; i < 3; i++) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool EdgeCompare(Vector *pEdgePts1, Vector *pEdgePts2, int &edgeType1,
                 int &edgeType2) {
  Vector edge1[3];
  Vector edge2[3];
  ...
  for (int ndx1 = 0; ndx1 < 3; ndx1++) {
    for (int ndx2 = 0; ndx2 < 3; ndx2++) {
      if (PointCompareWithTolerance(edge1[ndx1], edge2[ndx2],
                                    DISPSEW_POINT_TOLERANCE)) {
        // no midpoint to midpoint sharing allowed (midpoints are odd index
        // values)
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int GetEdgePointIndex(CMapDisp *pDisp, int edgeIndex, int edgePtIndex,
                      bool bCCW) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void AverageVectorFieldData(CMapDisp *pDisp1, int ndx1, CMapDisp *pDisp2,
                            int ndx2) {
  //
...
void SewCorner_Build(void) {
  ...
  for (int ndxFace = 0; ndxFace < faceCount; ndxFace++) {
    ...
    for (int ndxPt = 0; ndxPt < ptCount; ndxPt++) {
      ...
      //
      // compare this point to all of the other points on all the other faces in
      // the list
      //
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void SewTJunc_Add(SewTJuncData_t *pTJData, CMapFace *pFace, int ndxCorner,
                  int ndxEdge) {
...
void SewTJunc_AddToList(SewTJuncData_t *pTJData) {
  ...
  //
  // check to see if the t-junction point already exists in the t-junction data
  // list
  //
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool SewEdge_Add(SewEdgeData_t *pEdgeData, CMapFace *pFace, int ndxEdge,
                 int type) {
...
bool SewEdge_AddToListMerge(SewEdgeData_t *pEdgeData, SewEdgeData_t *pCmpData) {
  ...
  for (int i = 0; i < pEdgeData->faceCount; i++) {
    ...
    if (j == pCmpData->faceCount) {
      if (!SewEdge_Add(pCmpData, pEdgeData->pFaces[i], pEdgeData->ndxEdges[i],
                       pEdgeData->type[i])) {
        bReturn = false;
      }
    }
...
bool SewEdge_AddToListTJunc(SewEdgeData_t *pEdgeData) {
  ...
  Vector edgePts[2];
  GetPointFromSurface(pEdgeData->pFaces[ndxTJ], pEdgeData->ndxEdges[ndxTJ],
                      edgePts[0]);
  GetPointFromSurface(pEdgeData->pFaces[ndxTJ],
                      (pEdgeData->ndxEdges[ndxTJ] + 1) % ptCount, edgePts[1]);
  ...
  for (int i = 0; i < edgeCount; i++) {
    ...
    Vector edgePts2[2];
    GetPointFromSurface(pCmpData->pFaces[ndxCmp], pCmpData->ndxEdges[ndxCmp],
                        edgePts2[0]);
    GetPointFromSurface(pCmpData->pFaces[ndxCmp],
                        (pCmpData->ndxEdges[ndxCmp] + 1) % ptCount2,
                        edgePts2[1]);
...
void SewEdge_AddToListNormal(SewEdgeData_t *pEdgeData) {
  ...
  GetPointFromSurface(pEdgeData->pFaces[0],
                      (pEdgeData->ndxEdges[0] + 1) % ptCount, edgePts[1]);
  ...
  for (int i = 0; i < edgeCount; i++) {
    ...
    for (int j = 0; j < pCmpData->faceCount; j++) {
      ...
      Vector edgePts2[2];
      GetPointFromSurface(pCmpData->pFaces[j], pCmpData->ndxEdges[j],
                          edgePts2[0]);
      GetPointFromSurface(pCmpData->pFaces[j],
                          (pCmpData->ndxEdges[j] + 1) % ptCount2, edgePts2[1]);
...
void SewEdge_Build(void) {
  ...
  for (int ndxFace = 0; ndxFace < faceCount; ndxFace++) {
    ...
    for (int ndxPt = 0; ndxPt < ptCount; ndxPt++) {
      ...
      //
      // compare this edge to all of the other edges on all the other faces in
      // the list
      //
      for (int ndxFace2 = 0; ndxFace2 < faceCount; ndxFace2++) {
        // don't compare to itself
  ...
  if (bError) {
    AfxMessageBox(
        "Not all selected faces could be sewn because too many selected faces "
        "share a single edge.\n\nLook for places where 3 or more selected "
        "faces (displacement or non-displacement) all share an edge.");
  }
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void SewEdge_ResolveDispTJunc(SewEdgeData_t *pEdgeData, int ndxTJ,
                              int ndxTJNeighbor, bool bStart) {
  //
  ...
  if ((tjEdgeHandle == EDITDISPHANDLE_INVALID) ||
      (edgeHandle == EDITDISPHANDLE_INVALID))
    return;
  ...
  if (bStart) {
    if (bFlip) {
      for (int i = 1, j = ratio; i < edgeInterval; i++, j += ratio) {
        ...
        ndxTJPt =
            GetEdgePointIndex(pTJEdgeDisp, pEdgeData->ndxEdges[ndxTJ], j, true);
        ndxEdgePt = GetEdgePointIndex(
            pEdgeDisp, pEdgeData->ndxEdges[ndxTJNeighbor], i, false);
      ...
      for (int i = 1, j = ratio; i < tjInterval; i++, j += ratio) {
        ...
        ndxTJPt =
            GetEdgePointIndex(pTJEdgeDisp, pEdgeData->ndxEdges[ndxTJ], i, true);
        ndxEdgePt = GetEdgePointIndex(
            pEdgeDisp, pEdgeData->ndxEdges[ndxTJNeighbor], j, false);
    ...
    if (bFlip) {
      for (int i = 1, j = ratio; i < edgeWidth; i++, j += ratio) {
        int ndxTJPt, ndxEdgePt;
        ndxTJPt = GetEdgePointIndex(pTJEdgeDisp, pEdgeData->ndxEdges[ndxTJ], j,
                                    false);
        ndxEdgePt = GetEdgePointIndex(
            pEdgeDisp, pEdgeData->ndxEdges[ndxTJNeighbor], i, true);

        // average
        AverageVectorFieldData(pEdgeDisp, ndxEdgePt, pTJEdgeDisp, ndxTJPt);
      }
    } else {
      for (int i = (tjInterval + 1), j = ratio; i < (tjWidth - 1);
           i++, j += ratio) {
        int ndxTJPt, ndxEdgePt;
        ndxTJPt =
            GetEdgePointIndex(pTJEdgeDisp, pEdgeData->ndxEdges[ndxTJ], i, true);
  ...
  if (bFlip) {
    for (int i = 0; i < (tjWidth - ratio); i += ratio) {
      ...
      int ndxStartPt = GetEdgePointIndex(
          pTJEdgeDisp, pEdgeData->ndxEdges[ndxTJ], ndxStart, true);
      int ndxEndPt = GetEdgePointIndex(pTJEdgeDisp, pEdgeData->ndxEdges[ndxTJ],
                                       ndxEnd, true);
      ...
      for (int j = (ndxStart + 1); j < ndxEnd; j++) {
        ...
        BlendVectorFieldData(pTJEdgeDisp, ndxStartPt, ndxDst, pTJEdgeDisp,
                             ndxEndPt, ndxDst, blend);
      }
    }
  } else {
    for (int i = 0; i < (edgeWidth - ratio); i += ratio) {
      ...
      int ndxStartPt = GetEdgePointIndex(
          pEdgeDisp, pEdgeData->ndxEdges[ndxTJNeighbor], ndxStart, true);
      int ndxEndPt = GetEdgePointIndex(
          pEdgeDisp, pEdgeData->ndxEdges[ndxTJNeighbor], ndxEnd, true);

      for (int j = (ndxStart + 1); j < ndxEnd; j++) {
        float blend = blendRatio * (j - ndxStart);

        int ndxDst = GetEdgePointIndex(
            pEdgeDisp, pEdgeData->ndxEdges[ndxTJNeighbor], j, true);

        BlendVectorFieldData(pEdgeDisp, ndxStartPt, ndxDst, pEdgeDisp, ndxEndPt,
                             ndxDst, blend);
      }
    }
  }
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void SewEdge_ResolveSolidTJunc(SewEdgeData_t *pEdgeData, int type,
                               bool bStart) {
...
void SewEdge_ResolveDispNormal(SewEdgeData_t *pEdgeData) {
  ...
  if ((handle1 == EDITDISPHANDLE_INVALID) ||
      (handle2 == EDITDISPHANDLE_INVALID))
    return;
  ...
  // average "like" points
  //
  for (int ndxSm = 1, ndxLg = ratio; ndxSm < (smInterval - 1);
       ndxSm++, ndxLg += ratio) {
  ...
  for (int ndxLg = 0; ndxLg < (lgInterval - 1); ndxLg += ratio) {
    ...
    for (int ndx = (ndxStart + 1); ndx < ndxEnd; ndx++) {
      ...
      BlendVectorFieldData(pLgDisp, ndxStartPt, ndxDst, pLgDisp, ndxEndPt,
                           ndxDst, blend);
    }
  }
}
...
bool PrePlanarizeDependentVerts(void) {
  ...
  for (int iDisp = 0; iDisp < m_aCoreDispInfos.Count(); ++iDisp) {
    m_aCoreDispInfos[iDisp]->SetDispUtilsHelperInfo(m_aCoreDispInfos.Base(),
                                                    m_aCoreDispInfos.Count());
  }
  ...
  for (int iHgt = 0; iHgt < pCoreDisp->GetHeight(); ++iHgt) {
    for (int iWid = 0; iWid < pCoreDisp->GetWidth(); ++iWid) {
      ...
      if (!(vertsTouched[nIndex])) {
        ...
        if (FindEnclosingTri(vertCoords[nIndex], vertCoords, indices,
                             &iStartVert, flBCoords)) {
          ...
          Vector vNewPos =
              A * flBCoords[0] + B * flBCoords[1] + C * flBCoords[2];
