...
public: // functions
  ...
  virtual int NumSharedPoints(CMapDisp *pDisp, CMapDisp *pNeighborDisp,
                              int *edge1, int *edge2);
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CWorldEditDispMgr::TestNeighbors(CMapDisp *pDisp,
                                      CMapDisp *pNeighborDisp) {
  //
  // find the number of shared points between the two displacements (corners,
  // edges) NOTE: should use only 2, but face may be right on top of one another
  //
  int edge1[4], edge2[4];
  ...
  if (sharedPointCount == 1) {
    ...
    if ((cornerIndex != -1) && (neighborCornerIndex != -1)) {
      ...
      pDisp->AddCornerNeighbor(cornerIndex, pNeighborFace->GetDisp(),
                               neighborCornerIndex);
    }
    ...
    if ((edgeIndex != -1) && (neighborEdgeIndex != -1)) {
      CMapFace *pNeighborFace = (CMapFace *)pNeighborDisp->GetParent();
      pDisp->SetEdgeNeighbor(edgeIndex, pNeighborFace->GetDisp(),
                             neighborEdgeIndex);
    }
  }
}
    ...
    for (int iNeighbor = 0; iNeighbor < 4; ++iNeighbor) {
      ...
      if (pDisp) {
        ...
        for (int iCorner = 0; iCorner < nCornerCount; ++iCorner) {
          pDisp = EditDispMgr()->GetDisp(hDisp);
          if (pDisp) {
            pDisp->GetCornerNeighbor(iNeighbor, iCorner, hNeighbor,
                                     nNeighborOrient);

            CMapDisp *pNeighborDisp = EditDispMgr()->GetDisp(hNeighbor);
            if (pNeighborDisp) {
              CMapFace *pNeighborFace = (CMapFace *)pNeighborDisp->GetParent();
              CMapSolid *pNeighborSolid =
                  (CMapSolid *)pNeighborFace->GetParent();
              CMapClass *pNeighborObject = (CMapClass *)pNeighborSolid;
              if (!IsInKeptList(pNeighborObject)) {
                m_aKeptList.AddToTail(pNeighborObject);
                GetHistory()->Keep(pNeighborObject);
