...
//-----------------------------------------------------------------------------
CMapClass *CMapSideList::CreateMapSideList(CHelperInfo *pHelperInfo,
                                           CMapEntity *pParent) {
...
//-----------------------------------------------------------------------------
// Gets the keyvalue from our parent entity and rebuilds the face list from
// that.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
void CMapSideList::BuildFaceListForValue(char const *pszValue,
                                         CMapWorld *pWorld) {
...
//				other objects in the world when it copies
//pointers.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
CMapFace *CMapSideList::FindFaceIDInList(int nFaceID,
                                         const CMapObjectList &List) {
...
//			face list based on the the objects that were cloned with
//it.
// Input  : pClone -
//			OriginalList -
//			NewList -
//-----------------------------------------------------------------------------
void CMapSideList::OnClone(CMapClass *pCloneObject, CMapWorld *pWorld,
...
//			NewList -
//-----------------------------------------------------------------------------
void CMapSideList::OnPaste(CMapClass *pCopyObject, CMapWorld *pSourceWorld,
  ...
  //
  // HACK: This is kinda nasty. Build a list of face IDs from our parent
  // keyvalue so we can remap them to faces in the clipboard.
  //
...
//			value -
//-----------------------------------------------------------------------------
void CMapSideList::OnParentKeyChanged(char const *pszKey,
                                      char const *pszValue) {
  CMapWorld *pWorld = GetWorldObject(this);
  if (pWorld == NULL) {
...
//			cloned object) to faces in the original list of objects
//with references 			to corresponding faces in the new list of objects.
// Input  : pClone -
//			OriginalList -
//			NewList -
//-----------------------------------------------------------------------------
void CMapSideList::ReplaceFacesInCopy(CMapSideList *pCopy,
                                      const CMapObjectList &OriginalList,
                                      CMapObjectList &NewList) {
...
//			in the one solid with references to corresponding face
//IDs in 			another solid.
...
//-----------------------------------------------------------------------------
bool CMapSideList::ReplaceSolidFaces(CMapSolid *pOrigSolid,
                                     CMapSolid *pNewSolid) {
...
//			dependencies. Try to reacquire face IDs in our deleted
//faces list.
// Input  : pWorld -
//-----------------------------------------------------------------------------
...
//			current list of faces and submits the keyvalue to our
//parent for 			storage.
//-----------------------------------------------------------------------------
