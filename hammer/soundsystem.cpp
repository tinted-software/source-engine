...
// Sounds we're playing are loaded into here for Windows to access while playing
// them.
...
CSoundSystem::~CSoundSystem() { ShutDown(); }
...
bool CSoundSystem::BuildSoundList(SoundType_t type) {
  ...
  switch (type) {
  case SOUND_TYPE_RAW:
    return RecurseIntoDirectories("sound",
                                  &CSoundSystem::ProcessDirectory_RawFileList);

  case SOUND_TYPE_GAMESOUND:
    return BuildGameSoundList();

  case SOUND_TYPE_SCENE:
    return RecurseIntoDirectories(
        "scenes", &CSoundSystem::ProcessDirectory_SceneFileList);
  }
...
//-----------------------------------------------------------------------------
CSoundSystem::StringCache_t *
CSoundSystem::CreateStringCache(CSoundSystem::StringCache_t *pPrevious) {
...
char *CSoundSystem::AddStringToCache(SoundType_t type, const char *pString) {
  ...
  if ((!pCache) ||
      (copyLen + pCache->m_nTailIndex > StringCache_t::STRING_CACHE_SIZE)) {
...
//-----------------------------------------------------------------------------
void CSoundSystem::AddSoundToList(SoundType_t type, const char *pSoundName,
...
//-----------------------------------------------------------------------------
void CSoundSystem::BuildFileListInDirectory(char const *pDirectoryName,
                                            const char *pExt,
  ...
  for (; pFileName; pFileName = g_pFullFileSystem->FindNext(findHandle)) {
    ...
    const char *pStartPos =
        max(strchr(pDirectoryName, '/'), strchr(pDirectoryName, '\\'));
    if (pStartPos)
      Q_snprintf(pFileNameWithPath, nAllocSize, "%s%c%s", pStartPos + 1,
...
//-----------------------------------------------------------------------------
bool CSoundSystem::RecurseIntoDirectories(char const *pDirectoryName,
                                          pDirCallbackFn fn) {
...
//-----------------------------------------------------------------------------
void CSoundSystem::AddGameSoundToList(const char *pGameSound,
  ...
  SplitName(pFileName, offset, duration, before, sizeof(before), after,
            sizeof(after));
  ...
  for (KeyValues *pKeys = kv; pKeys; pKeys = pKeys->GetNextKey()) {
    ...
    if (pRawFile) {
      ...
      if (pRndWave) {
        ...
        if (pFirstFile) {
          AddGameSoundToList(pKeys->GetName(), pFirstFile->GetString(),
                             pSourceFile);
        }
      }
    }
...
bool CSoundSystem::FindSoundByName(const char *pFilename, SoundType_t *type,
                                   int *nIndex) {
...
bool CSoundSystem::PlayScene(const char *pFileName) {
  ...
  V_snprintf(fullFilename, sizeof(fullFilename), "scenes%c%s",
             CORRECT_PATH_SEPARATOR, pFileName);
...
bool CSoundSystem::Play(SoundType_t type, int nIndex) {
  ...
  if (fp) {
    ...
    if (g_pFileSystem->Read(g_SoundPlayData.Base(), g_SoundPlayData.Count(),
                            fp) == g_SoundPlayData.Count()) {
      return (PlaySound(g_SoundPlayData.Base(), NULL, SND_ASYNC | SND_MEMORY) !=
              FALSE);
    }
