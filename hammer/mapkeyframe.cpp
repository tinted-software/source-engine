...
//-----------------------------------------------------------------------------
CMapClass *CMapKeyFrame::CreateMapKeyFrame(CHelperInfo *pHelperInfo,
                                           CMapEntity *pParent) {
...
CMapClass *CMapKeyFrame::CopyFrom(CMapClass *pObj, bool bUpdateDependencies) {
  ...
  if (bUpdateDependencies) {
    m_pNextKeyFrame = (CMapKeyFrame *)UpdateDependency(m_pNextKeyFrame,
                                                       pFrom->m_pNextKeyFrame);
  } else {
...
//			inserts the clone into the correct place in the keyframe
//list
// Input  : *pClone -
//-----------------------------------------------------------------------------
void CMapKeyFrame::OnClone(CMapClass *pClone, CMapWorld *pWorld,
...
// Input  : *outQuat -
//-----------------------------------------------------------------------------
void CMapKeyFrame::GetQuatAngles(Quaternion &outQuat) { outQuat = m_qAngles; }
  ...
  if (m_pNextKeyFrame && m_flSpeed > 0) {
    ...
    if (GetRenderFrame() != pRender->GetRenderFrame()) {
      ...
      for (int i = 0; i < MAX_LINE_POINTS; i++) {
        ...
        meshBuilder.Position3f(m_LinePoints[i][0], m_LinePoints[i][1],
                               m_LinePoints[i][2]);
        meshBuilder.AdvanceVertex();
      }
...
//-----------------------------------------------------------------------------
// Purpose: Returns the total time remaining in the animation sequence in
// seconds.
//-----------------------------------------------------------------------------
...
void CMapKeyFrame::OnParentKeyChanged(const char *key, const char *value) {
  if (!stricmp(key, "NextKey")) {
    ...
    sscanf(value, "%f %f %f", &m_Angles[PITCH], &m_Angles[YAW],
           &m_Angles[ROLL]);
...
//			the movement speed, and the distance to the next
//keyframe
//-----------------------------------------------------------------------------
void CMapKeyFrame::RecalculateTimeFromSpeed(void) {
  ...
  // "NextTime" key removed until we get a real-time updating entity properties
  // dialog
  /*
...
void CMapKeyFrame::BuildPathSegment(CMapKeyFrame *pPrev) {
  ...
  for (int i = 0; i < MAX_LINE_POINTS; i++) {
    if (pAnim != NULL) {
      CMapAnimator::GetAnimationAtTime(
          this, pPrev, MoveTime() * (float)(i + 1) / (float)MAX_LINE_POINTS,
          m_LinePoints[i], qAngles, pAnim->m_iPositionInterpolator,
          pAnim->m_iRotationInterpolator);
    } else {
      // FIXME: If we aren't connected to an animator yet, just draw straight
      // lines. This code is never hit, because
      //		 BuildPathSegment is only called from CMapAnimator. To
      //make matters worse, we can only reliably find 		 pPrev through an animator.
      CMapAnimator::GetAnimationAtTime(
          this, pPrev, MoveTime() * (float)(i + 1) / (float)MAX_LINE_POINTS,
  ...
  // HACK: we shouldn't need to do this. CalcBounds alone should work (but it
  // doesn't because of where we call RebuildPath from). Make this work more
  // like other objects.
  if (m_pParent) {
...
//-----------------------------------------------------------------------------
void CMapKeyFrame::OnNotifyDependent(CMapClass *pObject,
...
bool CMapKeyFrame::IsAnyKeyInSequenceSelected(void) {
  ...
  // search forward
  for (CMapKeyFrame *find = m_pAnimator; find != NULL;
       find = find->m_pNextKeyFrame) {
...
CMapKeyFrame::SetupPositionInterpolator(int iInterpolator) {
  if (iInterpolator != m_iPositionInterpolator) {
    ...
    if (pEnt) {
      for (int i = pEnt->GetFirstKeyValue(); i != pEnt->GetInvalidKeyValue();
           i = pEnt->GetNextKeyValue(i)) {
        m_pPositionInterpolator->ProcessKey(pEnt->GetKey(i),
                                            pEnt->GetKeyValue(i));
      }
    }
  }
...
//-----------------------------------------------------------------------------
// Purpose: Marks that we need to relink any pointers defined by
// target/targetname pairs
//-----------------------------------------------------------------------------
