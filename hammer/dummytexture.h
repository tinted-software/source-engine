...
public:
  ...
  inline bool IsDummy() const { return (true); }

  int GetImageDataRGB(void *pImageRGB);
  int GetImageDataRGBA(void *pImageRGBA);

  inline int GetImageWidth() const { return (0); }

  inline int GetImageHeight() const { return (0); }

  inline float GetDecalScale() const { return (1.0f); }

  CPalette *GetPalette() const { return (NULL); }
  ...
  inline TEXTUREFORMAT GetTextureFormat() const { return (m_eTextureFormat); }

  inline int GetSurfaceAttributes() const { return (0); }
  ...
  inline bool HasData() const { return (false); }

  inline bool HasPalette() const { return (false); }
