...
//			volume that acts as a visibility filter. Only objects
//that intersect 			the cordon are rendered in the views. When saving the MAP file
//while 			the cordon tool is active, only brushes that intersect the cordon 			bounds
//are saved. The cordon box is replaced by brushes in order to 			seal the map.
//
//=============================================================================//
...
// Purpose: Constructor.
//-----------------------------------------------------------------------------
Cordon3D::Cordon3D(void) { SetDrawColors(RGB(0, 255, 255), RGB(255, 0, 0)); }
...
//-----------------------------------------------------------------------------
bool Cordon3D::OnLMouseDown2D(CMapView2D *pView, UINT nFlags,
...
// Output : Returns true if the message was handled, false if not.
//-----------------------------------------------------------------------------
bool Cordon3D::OnLMouseUp2D(CMapView2D *pView, UINT nFlags,
                            const Vector2D &vPoint) {
...
// Output : Returns true if the message was handled, false if not.
//-----------------------------------------------------------------------------
bool Cordon3D::OnMouseMove2D(CMapView2D *pView, UINT nFlags,
                             const Vector2D &vPoint) {
...
// Output :
//-----------------------------------------------------------------------------
bool Cordon3D::OnKeyDown2D(CMapView2D *pView, UINT nChar, UINT nRepCnt,
                           UINT nFlags) {
  if (nChar == VK_ESCAPE) {
    OnEscape();
    return true;
...
// Output :
//-----------------------------------------------------------------------------
bool Cordon3D::OnKeyDown3D(CMapView3D *pView, UINT nChar, UINT nRepCnt,
                           UINT nFlags) {
  if (nChar == VK_ESCAPE) {
    OnEscape();
    return true;
