...
//			pszSpritePath - The full path of the .MDL file, which is
//used as a 				key in the sprite cache.
...
//			client code when a pointer to the sprite is copied,
...
CSpriteModel::CSpriteModel(void)
    : m_pMaterial(0), m_NumFrames(-1), m_fScale(1.0), m_Origin(0, 0, 0),
      m_UL(0, 0), m_LR(0, 0), m_TexUL(0, 1), m_TexLR(1, 0), m_bInvert(false) {}
...
//			up -
//-----------------------------------------------------------------------------
void CSpriteModel::GetSpriteAxes(QAngle &Angles, int type, Vector &forward,
                                 Vector &right, Vector &up, Vector &ViewUp,
...
void CSpriteModel::SetMaterialPrimitiveType(
    const MaterialPrimitiveType_t type) {
...
void CSpriteModel::DrawSprite3D(CRender3D *pRender, unsigned char color[3]) {
  ...
  GetSpriteAxes(m_Angles, GetType(), spritez, spritex, spritey, ViewUp,
                ViewRight, ViewForward);
...
CSpriteDataCache *LookupSpriteDataCache(const char *pSpritePath) {
  ...
  if (i == g_SpriteDataCache.InvalidIndex()) {
    ...
    if (pData->m_pMaterial && pData->m_pMaterial->GetMaterial()) {
      ...
      pData->m_pFrameVar =
          pData->m_pMaterial->GetMaterial()->FindVar("$spriteFrame", 0);
      pData->m_pRenderModeVar =
          pData->m_pMaterial->GetMaterial()->FindVar("$spriterendermode", 0);

      pData->m_pOrientationVar = pData->m_pMaterial->GetMaterial()->FindVar(
          "$spriteOrientation", &pData->m_bOrientationVarFound, false);
      pData->m_pOriginVar = pData->m_pMaterial->GetMaterial()->FindVar(
          "$spriteorigin", &pData->m_bOriginVarFound);
    }
  } else {
