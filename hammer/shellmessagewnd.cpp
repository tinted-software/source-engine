...
//-----------------------------------------------------------------------------
// Purpose: Attaches a shell command handler to this message window. All
// commands
//			received by this message window will be sent to the
//command handler.
...
//			pCopyData - Copy data struct with shell command in the
//lpData field.
// Output : Returns TRUE on success, FALSE on failure.
//-----------------------------------------------------------------------------
BOOL CShellMessageWnd::OnCopyData(CWnd *pWnd, COPYDATASTRUCT *pCopyData) {
  if (m_pShell != NULL) {
    if (pCopyData->lpData != NULL) {
      return (m_pShell->RunCommand((const char *)pCopyData->lpData) ? TRUE
                                                                    : FALSE);
    }
  }
