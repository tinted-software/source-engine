...
ON_UPDATE_COMMAND_UI(ID_VIEW_SELECTION_MODE_BAR,
...
static UINT indicators[] = {ID_SEPARATOR, // status line indicator
                            ID_INDICATOR_SELECTION, ID_INDICATOR_COORDS,
                            ID_INDICATOR_SIZE,      ID_INDICATOR_GRIDZOOM,
                            ID_INDICATOR_SNAP};
...
//			via the shell. The user interface is disabled to prevent
//a mismatched 			versions between Hammer and the shell client.
//-----------------------------------------------------------------------------
void CMainFrame::BeginShellSession(void) { m_bShellSessionActive = true; }

//-----------------------------------------------------------------------------
// Purpose: Called through the shell to end a session of editing the map
//			via the engine. The user interface is enabled.
//-----------------------------------------------------------------------------
void CMainFrame::EndShellSession(void) { m_bShellSessionActive = false; }

//-----------------------------------------------------------------------------
// Purpose: If we get here there is no active 3D view. Uncheck the button.
//-----------------------------------------------------------------------------
void CMainFrame::OnUpdateView3d(CCmdUI *pCmdUI) { pCmdUI->SetCheck(FALSE); }
...
void CMainFrame::OnOpaqueMaterials() {
  MaterialSystemConfig().bNoTransparency =
      !MaterialSystemConfig().bNoTransparency;
...
//-----------------------------------------------------------------------------
void CMainFrame::OnUpdateView2d(CCmdUI *pCmdUI) { pCmdUI->SetCheck(FALSE); }

void CMainFrame::OnEnterMenuLoop(BOOL bIsTrackPopupMenu) {
  CMapDoc *pDoc = CMapDoc::GetActiveMapDoc();

  // if we are translation objects with a tool right now, dont switch to Menu
  // mode
  if (pDoc) {
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct) {
  ...
  if (!m_wndMapToolBar.Create(this, dwDefStyles, IDCB_MAPVIEWBAR) ||
      !m_wndMapToolBar.LoadToolBar(IDR_MAPDOC_VALVE)) {
  ...
  if (!m_wndMapOps.Create(this, dwDefStyles, IDCB_MAPOPERATIONS) ||
      !m_wndMapOps.LoadToolBar(IDR_MAPOPERATIONS_VALVE)) {
    TRACE0("Failed to create toolbar\n");
    return -1; // fail to create
  }
  ...
  if (!m_wndStatusBar.Create(this) ||
      !m_wndStatusBar.SetIndicators(NULL, NUMSTATUSPANES)) {
    TRACE0("Failed to create status bar\n");
    return -1; // fail to create
  }

  for (int i = 0; i < NUMSTATUSPANES; i++) {
    m_wndStatusBar.SetPaneInfo(paneinfo[i].nIndex, paneinfo[i].nID,
                               paneinfo[i].nStyle, paneinfo[i].cxWidth);
  }

  EnableDocking(CBRS_ALIGN_ANY);

  m_wndMapToolBar.SetBarStyle(m_wndMapToolBar.GetBarStyle() | CBRS_TOOLTIPS |
  ...
  m_wndMapEditToolBar.SetBarStyle(m_wndMapEditToolBar.GetBarStyle() |
                                  CBRS_TOOLTIPS | CBRS_FLYBY |
                                  CBRS_SIZE_DYNAMIC);
  m_wndMapOps.SetBarStyle(m_wndMapOps.GetBarStyle() | CBRS_TOOLTIPS |
                          CBRS_FLYBY | CBRS_SIZE_DYNAMIC);
  ...
  m_ObjectBar.SetBarStyle(m_ObjectBar.GetBarStyle() | CBRS_TOOLTIPS |
  ...
  m_FilterControl.SetBarStyle(m_FilterControl.GetBarStyle() | CBRS_TOOLTIPS |
                              CBRS_FLYBY | CBRS_SIZE_FIXED);
  ...
  m_TextureBar.SetBarStyle(m_TextureBar.GetBarStyle() | CBRS_TOOLTIPS |
                           CBRS_FLYBY | CBRS_SIZE_FIXED);
  ...
  // m_AnimationDlg.SetBarStyle(m_TextureBar.GetBarStyle() | CBRS_TOOLTIPS |
  // CBRS_FLYBY | CBRS_SIZE_FIXED); m_AnimationDlg.EnableDocking(CBRS_ALIGN_LEFT
  // | CBRS_ALIGN_RIGHT); DockControlBarLeftOf(&m_AnimationDlg, &m_TextureBar);

  //
  // Create the selection mode dialog bar.
  //
  m_SelectModeDlg.Create(this);
  m_SelectModeDlg.SetBarStyle(m_TextureBar.GetBarStyle() | CBRS_TOOLTIPS |
                              CBRS_FLYBY | CBRS_SIZE_FIXED);
  ...
  pObjectProperties->Create(this, WS_SYSMENU | WS_POPUP | WS_CAPTION |
                                      DS_MODALFRAME | WS_THICKFRAME);
  ...
  g_pwndMessage->CreateMessageWindow(this, CRect(0, clientrect.Height() - 90,
                                                 clientrect.Width(),
                                                 clientrect.Height()));
  ...
  // HACK: Spackle up the maximized window position to (0, 0) to fix an
  // intermittent bug. =(
  WINDOWPLACEMENT wp;
  ...
  //
  // !!!NOTE: Always do this last to ensure that the layout does not get
  // recalculated before the
  //			window is maximized. This prevents control bars from
  //being incorrectly wrapped to 			the next column.
  //
  if (VerifyBarState()) {
...
void CMainFrame::OnUpdateToolUI(CCmdUI *pUI) {
  if (IsShellSessionActive()) {
    ...
    if (pUI->m_nID == ID_TOOLS_APPLYDECALS || pUI->m_nID == ID_TOOLS_OVERLAY ||
        pUI->m_nID == ID_TOOLS_CLIPPER || pUI->m_nID == ID_TOOLS_MORPH) {
...
//-----------------------------------------------------------------------------
void CMainFrame::OnViewMessages(void) { g_pwndMessage->ToggleMessageWindow(); }
...
void CMainFrame::OnEditProperties(void) {
  pObjectProperties->ShowWindow(pObjectProperties->IsWindowVisible() ? SW_HIDE
                                                                     : SW_SHOW);
}
...
#endif
{
  ...
  // Had to change this code to not call GetWindowPlacement because for some
  // reason that prevented Hammer from maximizing properly -- it would leave
  // space on top. So I cache the minimized state instead, which fixes the
  // problem. Sigh.
  if (!m_bMinimized) {
...
//-----------------------------------------------------------------------------
// Purpose: Handles resize messages. Resizes any children that depend on our
// size. Input  : nType -
//			cx -
//			cy -
//-----------------------------------------------------------------------------
void CMainFrame::OnSize(UINT nType, int cx, int cy) {
  ...
  if (g_pwndMessage != NULL) {
    ...
    g_pwndMessage->Resize(CRect(0, clientrect.Height() - 130,
...
//-----------------------------------------------------------------------------
CMainFrame *GetMainWnd(void) { return pMainWnd; }
...
//-----------------------------------------------------------------------------
void CMainFrame::OnToolsOptions(void) { Configure(); }
...
//-----------------------------------------------------------------------------
// Purpose: This is called ONCE when the splash wnd is to be destroyed.
// OnPaint()
//			sets the timer.  This is now also called for the
//autosave timer.
// Input  : nIDEvent -
//-----------------------------------------------------------------------------
void CMainFrame::OnTimer(UINT nIDEvent) {
  ...
  if (nIDEvent == FIRST_TIMER) // for the splash scren window destruction
  {
    ...
    if (Options.general.iMaxAutosavesPerMap != 0) {
      SetTimer(AUTOSAVE_TIMER, Options.general.iTimeBetweenSaves * 60 * 1000,
               NULL);
    }
  }
}
...
//			materials editing tool and the displacement editing
//tool.
//
//			The tool itself is set by the OnSetActive handler for
//each property 			page of the face properties sheet.
//-----------------------------------------------------------------------------
BOOL CMainFrame::OnApplicator(UINT nID) {
  bool bNewFaceEditMode = !IsInFaceEditMode();

  //
  // Show/hide face edit sheet/texturebar and update the selection set if need
  // be.
  //
...
//			When we are in face edit mode, the texture bar is hidden
//and the 			face edit property sheet is shown. The active tool is changed 			to
//either the material tool or the displacement tool based on which 			page in the
...
//-----------------------------------------------------------------------------
// Purpose: Manages the state of the texture applicator toobar button and menu
// item.
//-----------------------------------------------------------------------------
...
// Output : Returns TRUE on success, FALSE on failure.
//-----------------------------------------------------------------------------
BOOL CMainFrame::OnHelpInfo(HELPINFO *pHelpInfo) { return (Default()); }

//-----------------------------------------------------------------------------
// Purpose: Opens a URL in the default web browser.
//-----------------------------------------------------------------------------
void CMainFrame::OpenURL(const char *pszURL) { APP()->OpenURL(pszURL, m_hWnd); }

//-----------------------------------------------------------------------------
// Purpose: Opens a URL in the default web browser by string ID.
//-----------------------------------------------------------------------------
void CMainFrame::OpenURL(UINT nID) { APP()->OpenURL(nID, m_hWnd); }
...
//-----------------------------------------------------------------------------
void CMainFrame::OnEditUndoredoactive(void) { SetUndoActive(!m_bUndoActive); }
...
//-----------------------------------------------------------------------------
// Purpose: Saves the position and types of all 2D and 3D views in the active
// document. dvs: This really needs to be a text file instead of a binary file!
// Input  : *pFile -
//-----------------------------------------------------------------------------
...
void CMainFrame::LoadWindowStates(std::fstream *pFile) {
  ...
  while (1) {
    ...
    while (p != NULL) {
      ...
      if (iViewType == WINSTATE2DVIEW &&
          !pThisView->IsKindOf(RUNTIME_CLASS(CMapView2D)))
        continue;

      if (iViewType == WINSTATELOGICALVIEW &&
          !pThisView->IsKindOf(RUNTIME_CLASS(CMapViewLogical)))
        continue;

      if (iViewType == WINSTATE3DVIEW &&
          !pThisView->IsKindOf(RUNTIME_CLASS(CMapView3D)))
        continue;
...
// Purpose:
//-----------------------------------------------------------------------------
void CMainFrame::OnSavewindowstate(void) { SaveWindowStates(); }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CMainFrame::OnLoadwindowstate(void) { LoadWindowStates(); }
...
BOOL CMainFrame::PreTranslateMessage(MSG *pMsg) {
  ...
  {
          if ((GetFocus() == &m_ObjectBar) || (GetFocus() ==
  &this->m_FilterControl))
          {
                  AfxMessageBox("Ok");
                  return(TRUE);
          }
...
//-----------------------------------------------------------------------------
// Purpose: Returns true if we are currently editing via the engine, false if
// not.
//-----------------------------------------------------------------------------
bool CMainFrame::IsShellSessionActive(void) { return (m_bShellSessionActive); }
...
//
//			Makes sure we don't have bogus dialog bar information in
//the registry. 			This prevents a crash in the MFC code if a version of the editor
//with 			different toolbars was run before us.
//
// TODO: fix the registry settings if they are bad so we can still load the bar
// state
//
// Output : Returns true if it is safe to load the toolbar settings, false if
// not.
//-----------------------------------------------------------------------------
