...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CMapAnimationDlg::OnHScroll(UINT nSBCode, UINT nPos,
                                 CScrollBar *pScrollBar) {
  // get the new time from the slider bar
  m_flAnimTime = ((float)m_TimeSlider.GetPos() / ANIMSLIDER_NUMTICS) *
                 m_flAnimationDuration;
...
//-----------------------------------------------------------------------------
// Purpose: Creates a new keyframe in the cycle at the current time in the
// animation
//-----------------------------------------------------------------------------
  ...
  for (int i = 0; i < pSelection->Count(); i++) {
    ...
    if (ent && ent->IsAnimationController()) {
      ...
      if (anim) {
        ...
        CMapDoc::GetActiveMapDoc()->SelectObject(pNewEntity,
                                                 scClear | scSaveChanges);
...
void CMapAnimationDlg::AdvanceAnimationTime(void) {
  ...
  m_TimeSlider.SetPos((m_flAnimTime / m_flAnimationDuration) *
                      ANIMSLIDER_NUMTICS);
...
void CMapAnimationDlg::SelectionChanged(CMapObjectList &selection) {
  ...
  FOR_EACH_OBJ(selection, pos) {
    ...
    if (ent) {
      if (ent->IsAnimationController() &&
          ent->GetChildOfType((CMapAnimator *)NULL)) {
        m_bEnabled = true;
        break;
      }
    }
