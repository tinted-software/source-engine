...
void CListBoxEx::SetStyle(DWORD dwStyle_) { this->dwStyle = dwStyle_; }
...
void CListBoxEx::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) {
  ...
  if (lpDrawItemStruct->itemID != -1 &&
    ...
    if (!bDrawCaptionOnly) {
      ...
      dc.TextOut(r.left + iCaptionWidthPixels + 1, r.top + 1, szText,
                 strlen(szText));
    }
...
void CListBoxEx::CreateEditControl() {
  ...
  if (lbis.iEditType != lbeString && lbis.iEditType != lbeInteger &&
...
void CListBoxEx::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags) {
  ...
  case VK_RETURN:
    ...
    if (lbis.iDataType == lbdBool) {
      ...
      CreateComboControl();
    } else if (lbis.iEditType == lbeString || lbis.iEditType == lbeInteger ||
               lbis.iEditType == lbeTexture) {
      CreateEditControl();
    }
    break;
