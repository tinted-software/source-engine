...
void PASCAL DDV_UndoLevels(CDataExchange *pDX, int value) {
  if (value < 5) {
    AfxMessageBox("Undo levels must be at least 5.",
...
void PASCAL DDV_MaxCameras(CDataExchange *pDX, int value) {
  if (value < 1 || value > 100) {
    AfxMessageBox("Max cameras must be between 1 and 1000.",
                  MB_ICONEXCLAMATION | MB_OK);
    pDX->Fail();
  }
}
...
void PASCAL DDV_AutosaveSpace(CDataExchange *pDX, int value) {
  if (value > 10000) {
    AfxMessageBox("You have selected too much space for autosaving. The "
...
void PASCAL DDV_NumberAutosaves(CDataExchange *pDX, int value) {
  if (value > 999) {
    AfxMessageBox("Number of autosaves must be 0-999.",
                  MB_ICONEXCLAMATION | MB_OK);
    pDX->Fail();
  }
}
...
void PASCAL DDV_AutosaveTimer(CDataExchange *pDX, int value) {
  if (value < 1 || value > 120) {
    AfxMessageBox("Time must be between 1 - 120 minutes.",
                  MB_ICONEXCLAMATION | MB_OK);
    pDX->Fail();
  }
}
...
BOOL COPTGeneral::OnApply(void) {
  ...
  if ((Options.general.iMaxAutosavesPerMap != 0 &&
       m_iMaxAutosavesPerMap == 0) ||
      (Options.general.iMaxAutosavesPerMap == 0 &&
       m_iMaxAutosavesPerMap != 0)) {
