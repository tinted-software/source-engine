...
//-----------------------------------------------------------------------------
ChunkFileResult_t
CDispMapImageFilter::LoadImageCallback(CChunkFile *pFile,
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
ChunkFileResult_t CDispMapImageFilter::LoadImageKeyCallback(
    const char *szKey, const char *szValue, CDispMapImageFilter *pFilter) {
  //
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
ChunkFileResult_t CDispMapImageFilter::LoadFilterKeyCallback(
    const char *szKey, const char *szValue, CDispMapImageFilter *pFilter) {
...
ChunkFileResult_t CDispMapImageFilter::LoadFilter(CChunkFile *pFile) {
  ...
  ChunkFileResult_t eResult =
      pFile->ReadChunk((KeyHandler_t)LoadFilterKeyCallback, this);
  pFile->PopHandlers();
...
//-----------------------------------------------------------------------------
inline void
CDispMapImageFilterManager::SetImageValue(CMapDisp *pDisp,
                                          CDispMapImageFilter *pFilter,
...
//-----------------------------------------------------------------------------
inline void
CDispMapImageFilterManager::GetImageValue(CMapDisp *pDisp,
                                          CDispMapImageFilter *pFilter,
                                          int ndxDisp, Vector &vPaintValue) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
inline void CDispMapImageFilterManager::GetImageFlatSubdivValue(
    CMapDisp *pDisp, CDispMapImageFilter *pFilter, int ndxDisp,
    Vector &vPaintValue) {
  if (pFilter->m_DataType == DISPPAINT_CHANNEL_POSITION) {
    Vector vSPos;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
inline void CDispMapImageFilterManager::GetImageFieldData(
    CMapDisp *pDisp, CDispMapImageFilter *pFilter, int ndxDisp, Vector &vNormal,
    float &dist) {
  if (pFilter->m_DataType == DISPPAINT_CHANNEL_POSITION) {
...
// Purpose:
//-----------------------------------------------------------------------------
bool CDispMapImageFilterManager::Apply(CDispMapImageFilter *pFilter,
                                       CMapDisp *pDisp, int paintDirType,
...
// Purpose:
//-----------------------------------------------------------------------------
void CDispMapImageFilterManager::ApplyAt(CDispMapImageFilter *pFilter,
...
// Purpose:
//-----------------------------------------------------------------------------
bool CDispMapImageFilterManager::IsNeighborInSelectionSet(
...
void CDispMapImageFilterManager::HitData_Setup(PosHitData_t &hitData,
  ...
  // east
  if ((ndxHgt >= 0.0f) && (ndxHgt <= (imgHgt - 1)) &&
      (ndxWid >= (imgWid - 1))) {
  ...
  // main "image"
  //
  if ((ndxHgt >= 0.0f) && (ndxHgt < imgHgt) && (ndxWid >= 0.0f) &&
      (ndxWid < imgWid)) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CDispMapImageFilterManager::GetCornerImageCount(CMapDisp *pDisp,
                                                    int ndxCorner) {
  switch (ndxCorner) {
...
CMapDisp *CDispMapImageFilterManager::GetImage(CDispMapImageFilter *pFilter,
                                               CMapDisp *pDisp, int ndxHgt,
                                               int ndxWid, int ndxImg,
                                               int imgCount, int &orient) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CDispMapImageFilterManager::GetImageFieldValues(
    CDispMapImageFilter *pFilter, CMapDisp *pDisp, int ndxHgt, int ndxWid,
  ...
  int orient;
  CMapDisp *pNeighborDisp =
      GetImage(pFilter, pDisp, ndxHgt, ndxWid, ndxImg, imgCount, orient);
  if (!pNeighborDisp)
    return false;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CDispMapImageFilterManager::GetImageFlatSubdivValues(
    CDispMapImageFilter *pFilter, CMapDisp *pDisp, int ndxHgt, int ndxWid,
  ...
  case IMAGEFILTER_SOUTHWEST: {
    ...
    if (count != 0) {
      for (int i = 0; i < count; i++) {
        ...
        if (neighborHandle != EDITDISPHANDLE_INVALID) {
          pNeighborDisp = EditDispMgr()->GetDisp(neighborHandle);
          SWImageValue(pFilter, pNeighborDisp, orient, ndxHgt, ndxWid, true,
                       value);
        }
        return;
      }
  ...
  case IMAGEFILTER_NORTHWEST: {
    ...
    if (count != 0) {
      for (int i = 0; i < count; i++) {
        ...
        if (neighborHandle != EDITDISPHANDLE_INVALID) {
          pNeighborDisp = EditDispMgr()->GetDisp(neighborHandle);
          NWImageValue(pFilter, pNeighborDisp, orient, ndxHgt, ndxWid, true,
                       value);
        }
        return;
      }
  ...
  case IMAGEFILTER_NORTHEAST: {
    ...
    if (count != 0) {
      for (int i = 0; i < count; i++) {
        ...
        if (neighborHandle != EDITDISPHANDLE_INVALID) {
          pNeighborDisp = EditDispMgr()->GetDisp(neighborHandle);
          NEImageValue(pFilter, pNeighborDisp, orient, ndxHgt, ndxWid, true,
                       value);
        }
        return;
      }
  ...
  case IMAGEFILTER_SOUTHEAST: {
    ...
    if (count != 0) {
      for (int i = 0; i < count; i++) {
        ...
        if (neighborHandle != EDITDISPHANDLE_INVALID) {
          pNeighborDisp = EditDispMgr()->GetDisp(neighborHandle);
          SEImageValue(pFilter, pNeighborDisp, orient, ndxHgt, ndxWid, true,
                       value);
        }
        return;
      }
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CDispMapImageFilterManager::GetSWImageIndex(CMapDisp *pDisp, int orient,
                                                int ndxHgt, int ndxWid) {
  //
  // get the image height and width
  //
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CDispMapImageFilterManager::GetWImageIndex(CMapDisp *pDisp, int orient,
                                               int ndxHgt, int ndxWid) {
  //
  // get the image height and width
  //
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CDispMapImageFilterManager::GetNWImageIndex(CMapDisp *pDisp, int orient,
                                                int ndxHgt, int ndxWid) {
  //
  // get the image height and width
  //
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CDispMapImageFilterManager::GetNImageIndex(CMapDisp *pDisp, int orient,
                                               int ndxHgt, int ndxWid) {
  //
  // get the image height and width
  //
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CDispMapImageFilterManager::GetNEImageIndex(CMapDisp *pDisp, int orient,
                                                int ndxHgt, int ndxWid) {
  //
  // get the image height and width
  //
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CDispMapImageFilterManager::GetEImageIndex(CMapDisp *pDisp, int orient,
                                               int ndxHgt, int ndxWid) {
  //
  // get the image height and width
  //
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CDispMapImageFilterManager::GetSEImageIndex(CMapDisp *pDisp, int orient,
                                                int ndxHgt, int ndxWid) {
  //
  // get the image height and width
  //
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CDispMapImageFilterManager::GetSImageIndex(CMapDisp *pDisp, int orient,
                                               int ndxHgt, int ndxWid) {
  //
  // get the image height and width
  //
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CDispMapImageFilterManager::ClampValues(CDispMapImageFilter *pFilter,
                                             Vector &v) {
...
bool CDispMapImageFilterManager::GetFilterVector(CDispMapImageFilter *pFilter,
                                                 CMapDisp *pDisp, int ndxHgt,
                                                 int ndxWid, int ndxImg,
                                                 int imgCount, int ndxFilter,
                                                 Vector &vFilterDir) {
  //
  // handle the alpha case
  //
  if (pFilter->m_DataType == DISPPAINT_CHANNEL_ALPHA) {
    vFilterDir.Init((pFilter->m_pImage[ndxFilter] * pFilter->m_Scale), 0.0f,
                    0.0f);
    return true;
  }
    ...
    for (int iWid = 0; iWid < nFilterWidth; ++iWid) {
      ...
      if (hitData.m_CornerCount != 0) {
        for (int iCorner = 0; iCorner < hitData.m_CornerCount; ++iCorner) {
          int nCount =
              GetCornerImageCount(pDisp, hitData.m_ndxCorners[iCorner]);
          for (int iCurCorner = 0; iCurCorner < nCount; ++iCurCorner) {
            if (GetImageValues(pFilter, pDisp, nAdjHeight, nAdjWidth,
                               hitData.m_ndxCorners[iCorner], iCurCorner,
                               vecImage)) {
              Vector vecFilter;
              GetFilterVector(pFilter, pDisp, nAdjHeight, nAdjWidth,
                              hitData.m_ndxCorners[iCorner], iCurCorner,
              ...
              SetImageValues(pFilter, pDisp, nAdjHeight, nAdjWidth,
                             hitData.m_ndxCorners[iCorner], iCurCorner,
                             vecImage);
            }
          }
      ...
      if (hitData.m_EdgeCount != 0) {
        for (int iEdge = 0; iEdge < hitData.m_EdgeCount; ++iEdge) {
          if (GetImageValues(pFilter, pDisp, nAdjHeight, nAdjWidth,
                             hitData.m_ndxEdges[iEdge], 0, vecImage)) {
            Vector vecFilter;
            GetFilterVector(pFilter, pDisp, nAdjHeight, nAdjWidth,
                            hitData.m_ndxEdges[iEdge], 0,
                            (iHgt * nFilterHeight + iWid), vecFilter);
            vecImage += vecFilter;
      ...
      if (hitData.m_bMain) {
        if (GetImageValues(pFilter, pDisp, nAdjHeight, nAdjWidth,
                           IMAGEFILTER_MAIN, 0, vecImage)) {
          Vector vecFilter;
          GetFilterVector(pFilter, pDisp, nAdjHeight, nAdjWidth,
                          IMAGEFILTER_MAIN, 0, (iHgt * nFilterHeight + iWid),
                          vecFilter);
          vecImage += vecFilter;
    ...
    for (int ndxWid = 0; ndxWid < filterWid; ndxWid++) {
      ...
      if (hitData.m_CornerCount != 0) {
        for (int i = 0; i < hitData.m_CornerCount; i++) {
          ...
          for (int j = 0; j < count; j++) {
            if (GetImageValues(pFilter, pDisp, adjHgt, adjWid,
                               hitData.m_ndxCorners[i], j, vImg)) {
              Vector vFilter;
              GetFilterVector(pFilter, pDisp, adjHgt, adjWid,
                              hitData.m_ndxCorners[i], j,
              ...
              SetImageValues(pFilter, pDisp, adjHgt, adjWid,
                             hitData.m_ndxCorners[i], j, vImg);
            }
          }
        }
      ...
      if (hitData.m_EdgeCount != 0) {
        for (int i = 0; i < hitData.m_EdgeCount; i++) {
          if (GetImageValues(pFilter, pDisp, adjHgt, adjWid,
                             hitData.m_ndxEdges[i], 0, vImg)) {
            Vector vFilter;
            GetFilterVector(pFilter, pDisp, adjHgt, adjWid,
                            hitData.m_ndxEdges[i], 0,
                            (ndxHgt * filterHgt + ndxWid), vFilter);
            vImg += vFilter;
      ...
      // update main
      if (hitData.m_bMain) {
        if (GetImageValues(pFilter, pDisp, adjHgt, adjWid, IMAGEFILTER_MAIN, 0,
                           vImg)) {
          Vector vFilter;
          ...
          SetImageValues(pFilter, pDisp, adjHgt, adjWid, IMAGEFILTER_MAIN, 0,
                         vImg);
        }
      }
    }
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CDispMapImageFilterManager::Apply3x3SmoothFilter(
    CDispMapImageFilter *pFilter, CMapDisp *pDisp, int ndxVert, Vector &vPos) {
  //
  // get displacement image and filter height and width data
  //
  int imgHgt = pDisp->GetHeight();
    ...
    for (int ndxWid = 0; ndxWid < filterWid; ndxWid++) {
      ...
      if (hitData.m_CornerCount != 0) {
        for (int i = 0; i < hitData.m_CornerCount; i++) {
          int count = GetCornerImageCount(pDisp, hitData.m_ndxCorners[i]);
          for (int j = 0; j < count; j++) {
            if (GetImageFieldValues(pFilter, pDisp, adjHgt, adjWid,
                                    hitData.m_ndxCorners[i], j, vNormal,
                                    dist)) {
              vNormals +=
                  (vNormal * pFilter->m_pImage[ndxHgt * filterHgt + ndxWid]);
      ...
      if (hitData.m_EdgeCount != 0) {
        for (int i = 0; i < hitData.m_EdgeCount; i++) {
          if (GetImageFieldValues(pFilter, pDisp, adjHgt, adjWid,
                                  hitData.m_ndxEdges[i], 0, vNormal, dist)) {
            vNormals +=
                (vNormal * pFilter->m_pImage[ndxHgt * filterHgt + ndxWid]);
            dists += (dist * pFilter->m_pImage[ndxHgt * filterHgt + ndxWid]);
            totalFrac += pFilter->m_pImage[ndxHgt * filterHgt + ndxWid];
      ...
      if (hitData.m_bMain) {
        if (GetImageFieldValues(pFilter, pDisp, adjHgt, adjWid,
                                IMAGEFILTER_MAIN, 0, vNormal, dist)) {
          vNormals +=
              (vNormal * pFilter->m_pImage[ndxHgt * filterHgt + ndxWid]);
          dists += (dist * pFilter->m_pImage[ndxHgt * filterHgt + ndxWid]);
          totalFrac += pFilter->m_pImage[ndxHgt * filterHgt + ndxWid];
    ...
    for (int ndxWid = 0; ndxWid < areaWid; ndxWid++) {
      ...
      if (hitData.m_CornerCount != 0) {
        for (int i = 0; i < hitData.m_CornerCount; i++) {
          ...
          for (int j = 0; j < count; j++) {
            ...
            //
            int orient;
            CMapDisp *pAdjDisp = GetImage(pFilter, pDisp, adjHgt, adjWid,
                                          hitData.m_ndxCorners[i], j, orient);
            if (pAdjDisp) {
              int adjIndex = GetAdjustedIndex(pAdjDisp, orient, adjHgt, adjWid,
                                              hitData.m_ndxCorners[i]);
      ...
      if (hitData.m_EdgeCount != 0) {
        for (int i = 0; i < hitData.m_EdgeCount; i++) {
          ...
          int orient;
          CMapDisp *pAdjDisp = GetImage(pFilter, pDisp, adjHgt, adjWid,
                                        hitData.m_ndxEdges[i], 0, orient);
          if (pAdjDisp) {
            int adjIndex = GetAdjustedIndex(pAdjDisp, orient, adjHgt, adjWid,
                                            hitData.m_ndxEdges[i]);
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CDispMapImageFilterManager::IsEqualMask(CDispMapImageFilter *pFilter,
                                             int ndxFilter) {
    ...
    for (int ndxWid = 0; ndxWid < filterWid; ndxWid++) {
      ...
      if (hitData.m_CornerCount != 0) {
        for (int i = 0; i < hitData.m_CornerCount; i++) {
          int count = GetCornerImageCount(pDisp, hitData.m_ndxCorners[i]);
          for (int j = 0; j < count; j++) {
            if (GetImageFlatSubdivValues(pFilter, pDisp, adjHgt, adjWid,
                                         hitData.m_ndxCorners[i], j, vImg)) {
