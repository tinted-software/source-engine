...
enum { FORWARD = 0, STRAFE, VERTICAL };
...
protected:
  ...
  CKeyboard m_Keyboard; // handles binding of keys and mouse buttons to logical
                        // functions
  DWORD
      m_TimeLastInputSample; // used for framerate-independent input processing.
  float m_Speed[3];    // speed in world units/sec = forward, side-to-side, and
                       // up-down
  ...
  float Accelerate(float vel, float accel, float accelScale, float timeScale,
                   float velMax);
