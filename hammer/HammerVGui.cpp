...
bool CHammerVGui::Init(HWND hWindow) {
  ...
  // load scheme
  if (!vgui::scheme()->LoadSchemeFromFile("Resource/SourceScheme.res",
                                          "Hammer")) {
    return false;
  }
...
void CHammerVGui::SetFocus(CVGuiWnd *pVGuiWnd) {
  ...
  if (pVGuiWnd) {
    ...
    g_pMatSystemSurface->AttachToWindow(pVGuiWnd->GetParentWnd()->GetSafeHwnd(),
                                        false);
...
void CHammerVGui::Simulate() {
  ...
  // run vgui animations
  vgui::GetAnimationController()->UpdateAnimations(
      vgui::system()->GetCurrentTime());
}
