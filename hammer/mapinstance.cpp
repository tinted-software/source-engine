...
// Input  : pHelperInfo - Pointer to helper info class which gives us
// information
...
//located
//-----------------------------------------------------------------------------
bool CMapInstance::DeterminePath(const char *pszBaseFileName,
...
// Purpose: Constructor.
//-----------------------------------------------------------------------------
CMapInstance::CMapInstance(void) { Initialize(); }

//-----------------------------------------------------------------------------
// Purpose: Constructor.
// Input  : pszBaseFileName - the root path of where the instance will be loaded
// from.
//			pszInstanceFileName - the relative name of the instance
//to be loaded.
// Output :
//-----------------------------------------------------------------------------
CMapInstance::CMapInstance(const char *pszBaseFileName,
                           const char *pszInstanceFileName) {
  Initialize();

  if (pszInstanceFileName[0] &&
      DeterminePath(pszBaseFileName, pszInstanceFileName, m_FileName)) {
...
GDIV_TYPE CMapInstance::GetFieldType(const char *pInstanceValue) {
  ...
  m_pInstancedMap->FindEntitiesByClassName(entityList, "func_instance_parms",
                                           false);
...
void CMapInstance::FindTargetNames(CUtlVector<const char *> &Names) {
  ...
  for (int j = pEntity->GetFirstKeyValue(); j != pEntity->GetInvalidKeyValue();
    ...
    if (strnicmp(pInstanceKey, "replace", strlen("replace")) == 0) {
      ...
      if (FieldType == ivTargetDest || FieldType == ivTargetNameOrClass ||
...
void CMapInstance::ReplaceTargetname(const char *szOldName,
...
//-----------------------------------------------------------------------------
// Purpose: This function is called when the owning entity's Key/Value pairs
// have
//			been updated.  This will attempt to load a new instance
//if the map has 			been changed.
// Input  : none.
// Output : none.
//-----------------------------------------------------------------------------
bool CMapInstance::OnApply(void) {
  ...
  // loading this instance will bring it forward in the MDI - we want to show
  // the original map though
...
void CMapInstance::CalcBounds(BOOL bFullUpdate) {
  ...
  if (m_pInstancedMap && GetParent() && m_pInstancedMap->GetMapWorld()) {
    ...
    TransformAABB(Instance3x4Matrix, vecMins, vecMaxs, vecExpandedMins,
                  vecExpandedMaxs);
    m_CullBox.UpdateBounds(vecExpandedMins, vecExpandedMaxs);
...
//-----------------------------------------------------------------------------
// Purpose: Will calculate the bounding box of the instance as a child has
// changed Input  : pChild - Pointer to the object that changed.
//-----------------------------------------------------------------------------
void CMapInstance::UpdateChild(CMapClass *pChild) { CalcBounds(TRUE); }

//-----------------------------------------------------------------------------
// Purpose: this function will attempt to find a child inside of the instance.
// If the bool and matrix
...
//-----------------------------------------------------------------------------
CMapEntity *CMapInstance::FindChildByKeyValue(const char *key,
                                              const char *value,
                                              bool *bIsInInstance,
                                              VMatrix *InstanceMatrix) {
  if (m_pInstancedMap && bIsInInstance) {
    CMapEntity *result =
        m_pInstancedMap->GetMapWorld()->FindChildByKeyValue(key, value);
    if (result) {
...
//-----------------------------------------------------------------------------
CMapClass *CMapInstance::CopyFrom(CMapClass *pObject,
...
//-----------------------------------------------------------------------------
void CMapInstance::GetAngles(QAngle &Angles) { Angles = m_Angles; }
...
void CMapInstance::Render3D(CRender3D *pRender) {
  if (m_pInstancedMap) {
    pRender->RenderInstanceMapClass(this, m_pInstancedMap->GetMapWorld(),
                                    m_Origin, m_Angles);
  }
}

//-----------------------------------------------------------------------------
// Purpose: this function will display an instance map window.  it will also set
// the
...
void CMapInstance::SwitchTo(void) {
  ...
  while (PositionCurrentView && PositionNewView) {
    CMapView *pViewCurrent =
        dynamic_cast<CMapView *>(pCurrentDoc->GetNextView(PositionCurrentView));
    ...
    CMapView *pViewNew =
        dynamic_cast<CMapView *>(m_pInstancedMap->GetNextView(PositionNewView));
    ...
    if (pViewCurrent2D) {
      ...
      CameraAngles.Init(CurrentCamera->GetPitch(), CurrentCamera->GetYaw(),
      ...
      //			pViewNew3D->GetCamera()->SetRoll( CameraAngles.z
      //);			we probably don't want to set this!
    }
...
// Output : int
//-----------------------------------------------------------------------------
int CMapInstance::SerializeRMF(std::fstream &File, BOOL bRMF) { return (0); }
...
// Output : int
//-----------------------------------------------------------------------------
int CMapInstance::SerializeMAP(std::fstream &File, BOOL bRMF) { return (0); }
...
//-----------------------------------------------------------------------------
// Purpose: Notifies that this object's parent entity has had a key value
// change. Input  : szKey - The key that changed.
//			szValue - The new value of the key.
//-----------------------------------------------------------------------------
void CMapInstance::OnParentKeyChanged(const char *szKey, const char *szValue) {
  if (!stricmp(szKey, "angles")) {
    sscanf(szValue, "%f %f %f", &m_Angles[PITCH], &m_Angles[YAW],
           &m_Angles[ROLL]);
...
//-----------------------------------------------------------------------------
bool CMapInstance::ShouldRenderLast(void) { return (false); }
...
void CMapInstance::Render2D(CRender2D *pRender) {
  ...
  if (m_pInstancedMap) {
    pView->RenderInstance(this, m_pInstancedMap->GetMapWorld(), m_Origin,
                          m_Angles);
  }
...
//-----------------------------------------------------------------------------
void CMapInstance::RenderLogicalAt(CRender2D *pRender, const Vector2D &vecMins,
                                   const Vector2D &vecMaxs) {}

//-----------------------------------------------------------------------------
// Purpose: Returns if this instance is editable.  A pure instance is not
// editable.
//			If it is part of a manifest, it must be the primary map
//of the manifest 			in order for it to be editable.
//-----------------------------------------------------------------------------
...
bool CMapInstance::IsInstanceVisible(void) {
  if (IsInstance()) {
    if (CMapDoc::GetActiveMapDoc() &&
        CMapDoc::GetActiveMapDoc()->GetShowInstance() == INSTANCES_HIDE) {
      return false;
    }
  } else {
...
//-----------------------------------------------------------------------------
void CMapInstance::UpdateInstanceMap(void) { CalcBounds(); }
