...
//-----------------------------------------------------------------------------
// Purpose: Creates the hidden window that receives context menu commands for
// the
//			entity tool.
...
bool CToolEntityMessageWnd::Create(void) {
  ...
  return (CWnd::CreateEx(0, g_pszClassName, g_pszClassName, 0,
                         CRect(0, 0, 10, 10), NULL, 0) == TRUE);
}

//-----------------------------------------------------------------------------
// Purpose: Attaches the entity tool to this window before activating the
// context
//			menu.
//-----------------------------------------------------------------------------
void CToolEntityMessageWnd::PreMenu2D(CToolEntity *pToolEntity,
                                      CMapView2D *pView) {
...
CToolEntity::CToolEntity(void) {
  ...
  if (s_hcurEntity == NULL) {
    s_hcurEntity =
        LoadCursor(AfxGetInstanceHandle(), MAKEINTRESOURCE(IDC_ENTITY));
  }
}
...
// Output :
//-----------------------------------------------------------------------------
int CToolEntity::HitTest(CMapView *pView, const Vector2D &ptClient,
                         bool bTestHandles) {
...
bool CToolEntity::UpdateTranslation(const Vector &vUpdate, UINT uFlags) {
  ...
  if (uFlags) {
    ProjectOnTranslationPlane(m_vecPos + m_vTranslation, m_vTranslation,
                              uFlags);
...
void CToolEntity::RenderTool2D(CRender2D *pRender) {
  ...
  // Draw crosshair
  //
  pRender->DrawLine(Vector(g_MIN_MAP_COORD, v.y, v.z),
                    Vector(g_MAX_MAP_COORD, v.y, v.z));
  pRender->DrawLine(Vector(v.x, g_MIN_MAP_COORD, v.z),
                    Vector(v.x, g_MAX_MAP_COORD, v.z));
  pRender->DrawLine(Vector(v.x, v.y, g_MIN_MAP_COORD),
                    Vector(v.x, v.y, g_MAX_MAP_COORD));
}
...
// Output :
//-----------------------------------------------------------------------------
bool CToolEntity::OnContextMenu2D(CMapView2D *pView, UINT nFlags,
  ...
  if (!IsEmpty()) {
    ...
    if (HitTest(pView, vPoint, false)) {
      ...
      menuCreate.TrackPopupMenu(TPM_LEFTBUTTON | TPM_RIGHTBUTTON |
                                    TPM_LEFTALIGN,
                                ptScreen.x, ptScreen.y, &s_wndToolMessage);
    }
  }
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CToolEntity::OnKeyDown2D(CMapView2D *pView, UINT nChar, UINT nRepCnt,
                              UINT nFlags) {
  switch (nChar) {
  case VK_RETURN: {
    if (!IsEmpty()) {
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CToolEntity::OnLMouseDown2D(CMapView2D *pView, UINT nFlags,
                                 const Vector2D &vPoint) {
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CToolEntity::OnLMouseUp2D(CMapView2D *pView, UINT nFlags,
                               const Vector2D &vPoint) {
...
//-----------------------------------------------------------------------------
bool CToolEntity::OnMouseMove2D(CMapView2D *pView, UINT nFlags,
                                const Vector2D &vPoint) {
...
// Returns true if the message was handled, false otherwise.
//-----------------------------------------------------------------------------
bool CToolEntity::OnMouseMove3D(CMapView3D *pView, UINT nFlags,
                                const Vector2D &vPoint) {
  return true;
}
...
// Returns true if the message was handled, false otherwise.
//-----------------------------------------------------------------------------
bool CToolEntity::OnKeyDown3D(CMapView3D *pView, UINT nChar, UINT nRepCnt,
                              UINT nFlags) {
  CMapDoc *pDoc = pView->GetMapDoc();
  if (pDoc == NULL) {
    return false;
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CToolEntity::OnLMouseDown3D(CMapView3D *pView, UINT nFlags,
                                 const Vector2D &vPoint) {
  ULONG ulFace;
  VMatrix LocalMatrix, LocalMatrixNeg;
  CMapClass *pObject = pView->NearestObjectAt(
      vPoint, ulFace, FLAG_OBJECTS_AT_RESOLVE_INSTANCES, &LocalMatrix);
  ...
  if (pObject != NULL) {
    ...
    if (pFace->TraceLine(HitPos, HitNormal, vFinalStart, vFinalEnd)) {
      ...
      if (GetMainWnd()->m_ObjectBar.IsEntityToolCreatingPrefab()) {
        ...
        GetHistory()->MarkUndoPosition(m_pDocument->GetSelection()->GetList(),
                                       "New Prefab");

        // Get prefab object
        CMapClass *pPrefabObject =
            GetMainWnd()->m_ObjectBar.BuildPrefabObjectAtPoint(vFinalHitPos);
        ...
        VPlane BeforeTransform(pFace->plane.normal, pFace->plane.dist),
            AfterTransform;
        ...
        pEntity->AlignOnPlane(vFinalHitPos, &NewPlane,
                              (vFinalHitNormal.z > 0.0f)
                                  ? CMapEntity::ALIGN_BOTTOM
                                  : CMapEntity::ALIGN_TOP);
      ...
      if (pNewObject) {
        if (GetMainWnd()->m_ObjectBar.UseRandomYawOnEntityPlacement()) {
          // They checked "random yaw" on the object bar, so come up with a
          // random yaw.
          ...
          // Setup a matrix that translates them to the origin, rotates it, then
          // translates back.
        ...
        m_pDocument->SelectObject(pNewObject,
                                  scClear | scSelect | scSaveChanges);
...
void CToolEntity::CreateMapObject(CMapView2D *pView) {
  ...
  if (GetMainWnd()->m_ObjectBar.IsEntityToolCreatingPrefab()) {
    ...
    CMapClass *pPrefabObject =
        GetMainWnd()->m_ObjectBar.BuildPrefabObjectAtPoint(m_vecPos);
