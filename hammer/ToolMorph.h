...
public:
  ...
  void SelectHandle2D(CMapView2D *pView, MORPHHANDLE *pInfo,
                      UINT cmd = scSelect);
...
private:
  ...
  bool MorphHitTest(CMapView *pView, const Vector2D &vPoint,
                    MORPHHANDLE *pInfo);

  void GetHandlePos(MORPHHANDLE *pInfo, Vector &pt);

  SSHANDLE Get2DMatches(CMapView2D *pView, CSSolid *pStrucSolid,
                        SSHANDLEINFO &hi,
  ...
  //
  int HitTest(CMapView *pView, const Vector2D &ptClient,
  ...
  // handle mode:
  enum { hmBoth = 0x01 | 0x02, hmVertex = 0x01, hmEdge = 0x02 };
...
//-----------------------------------------------------------------------------
inline int Morph3D::GetObjectCount(void) { return (m_StrucSolids.Count()); }
