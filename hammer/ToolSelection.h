...
protected:
  ...
  bool StartBoxSelection(CMapView *pView, const Vector2D &vPoint,
                         const Vector &vStart);
  ...
  GDinputvariable *ChooseEyedropperVar(CMapView *pView,
  ...
  CBaseTool *GetToolObject(CMapView2D *pView, const Vector2D &ptScreen,
                           bool bAttach);
  CBaseTool *GetToolObjectLogical(CMapViewLogical *pView,
                                  const Vector2D &vPoint, bool bAttach);
  ...
  // Tool3D implementation.
  //
  virtual int HitTest(CMapView *pView, const Vector2D &pt,
                      bool bTestHandles = false);
  ...
  Vector2D
      m_vLDownLogicalClient; // Logical client pos at which lbutton was pressed.
  Vector2D m_vecLogicalSelBoxMins;
  Vector2D m_vecLogicalSelBoxMaxs;
  bool m_bInLogicalBoxSelection; // Are we doing box selection in the logical
                                 // mode?
  COLORREF m_clrLogicalBox;      // The color of the logical box
  Vector2D
      m_vLastLogicalDragPoint; // Last point at which we dragged (world coords)
