...
//
//			TODO: consider making face selection a mode of the
//selection tool
//
// $NoKeywords: $
//=============================================================================//
...
//-----------------------------------------------------------------------------
bool CToolMaterial::OnLMouseDown2D(CMapView2D *pView, UINT nFlags,
...
// Output : Returns true if the message was handled, false if not.
//-----------------------------------------------------------------------------
bool CToolMaterial::OnLMouseDown3D(CMapView3D *pView, UINT nFlags,
                                   const Vector2D &vPoint) {
...
// Output : Returns true if the message was handled, false if not.
//-----------------------------------------------------------------------------
bool CToolMaterial::OnRMouseDown3D(CMapView3D *pView, UINT nFlags,
                                   const Vector2D &vPoint) {
  BOOL bShift = ((GetAsyncKeyState(VK_SHIFT) & 0x8000) != 0);
  ...
  if (pObject != NULL) {
    if (pObject->IsMapClass(MAPCLASS_TYPE(CMapSolid))) {
      ...
      // If we're in a lightmap grid preview window, only apply the lightmap
      // scale.
      int eMode;
      ...
      // If not, apply to a single face.
        ...
        GetMainWnd()->m_pFaceEditSheet->ClickFace(pSolid, ulFace, cmdFlags,
                                                  eMode);
      }
    }
  }
...
//-----------------------------------------------------------------------------
bool CToolMaterial::OnMouseMove3D(CMapView3D *pView, UINT nFlags,
                                  const Vector2D &vPoint) {
  //
  // Manage the cursor.
  //
  ...
  if (!hcurFacePaint) {
    hcurFacePaint =
        LoadCursor(AfxGetInstanceHandle(), MAKEINTRESOURCE(IDC_FACEPAINT));
  }
...
// Output : Returns true if the message was handled, false if not.
//-----------------------------------------------------------------------------
bool CToolMaterial::OnKeyDown3D(CMapView3D *pView, UINT nChar, UINT nRepCnt,
                                UINT nFlags) {
  // TO DO:  undo?
  //		   justify (SHIFT?)
