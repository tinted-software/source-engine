...
public:
  ...
  virtual bool Load(void) = 0; // ensure that texture is loaded. could this be
                               // done internally?
  virtual void
  Reload(bool bFullReload) = 0; // The texture changed. If bFullReload is true,
                                // then the material system reloads it too.
  ...
  // textures
  //			serve as placeholders for textures that were found in
  //the map, but 			not in the WAD (or the materials tree). The dummy texture
  //enables us 			to bind the texture, find it by name, etc.
  //-----------------------------------------------------------------------------
  ...
  //is assigned 			by the first renderer that actually binds the texture thru
  //BindTexture.
  //
