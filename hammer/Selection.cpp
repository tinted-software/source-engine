...
//			what gets selected when the user clicks on something -
//the group, 			the entity, or the solid.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Returns true if the selection is copyable.  CManifestInstance
// classes
//			are not copyable.
//-----------------------------------------------------------------------------
...
void CSelection::SetMode(SelectMode_t eNewSelectMode) {
  ...
  if ((eOldSelectMode == selectSolids) ||
    ...
    //
    // If we are going from a more specific selection mode to a less specific
    // one, clear the selection. This avoids unexpectedly selecting new things.
    //
    SelectObject(NULL, scClear | scSaveChanges);
  } else {
    //
    // Put all the children of the selected objects in a list, along with their
    // children.
    //
...
void CSelection::SetCurrentHit(int iIndex, bool bCascading) {
  ...
  if (bCascading) {
    ...
    CUtlRBTree<CMapClass *, unsigned short> tree(0, 0,
    ...
    for (unsigned short h = tree.FirstInorder(); h != tree.InvalidIndex();
         h = tree.NextInorder(h)) {
...
bool CSelection::SelectObject(CMapClass *pObj, int cmd) {
  ...
  if (cmd & scSaveChanges) {
    // changing the selection automatically saves changes made to the properties
    // dialog
...
//-----------------------------------------------------------------------------
// Purpose: Clears the current selection and selects everything in the given
// list. Input  : pList - Objects to select.
//-----------------------------------------------------------------------------
