...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CToolDisplace::OnLMouseDown3D(CMapView3D *pView, UINT nFlags,
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CToolDisplace::OnLMouseUp3D(CMapView3D *pView, UINT nFlags,
                                 const Vector2D &vPoint) {
  // left button up
  m_bLMBDown = false;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CToolDisplace::OnRMouseDown3D(CMapView3D *pView, UINT nFlags,
                                   const Vector2D &vPoint) {
  // left button down
  m_bRMBDown = true;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CToolDisplace::OnRMouseUp3D(CMapView3D *pView, UINT nFlags,
                                 const Vector2D &vPoint) {
  // left button up
  m_bRMBDown = false;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CToolDisplace::OnMouseMove3D(CMapView3D *pView, UINT nFlags,
                                  const Vector2D &vPoint) {
  m_MousePoint = vPoint;
  ...
  else if ((m_uiTool == DISPTOOL_PAINT) &&
           (GetAsyncKeyState(VK_MENU) & 0x8000) && m_bLMBDown &&
           m_bSpatialRadius) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CToolDisplace::Nudge_Activate(CMapView3D *pView,
                                   EditDispHandle_t dispHandle) {
  IWorldEditDispMgr *pDispMgr = GetActiveWorldEditDispManager();
  if (!pDispMgr)
    return;
...
void CToolDisplace::Nudge_Do(void) {
  ...
  if (!IsSpatialPainting()) {
    ...
    pFilter->m_Scale =
        (delta * 0.25) * (float)((int)(m_flPaintValueGeo / 10.0f) + 1);

    // apply the filter to the displacement surface(s)
    m_FilterRaiseLowerMgr.Apply(pFilter, pNudgeDisp, m_iPaintAxis,
                                m_vecPaintAxis, m_bAutoSew);
  } else {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CToolDisplace::ApplyPaintTool(UINT nFlags, const Vector2D &vPoint,
                                   CMapDisp *pDisp) {
  switch (m_uiEffect) {
  case DISPPAINT_EFFECT_RAISELOWER: {
    ...
    if (pFilter) {
      ...
      // apply the filter to the displacement surface(s)
      m_FilterRaiseLowerMgr.Apply(pFilter, pDisp, m_iPaintAxis, m_vecPaintAxis,
                                  m_bAutoSew);
    }
    return;
  }
  ...
  case DISPPAINT_EFFECT_SMOOTH: {
    ...
    if (pFilter) {
      ...
      // apply the filter to the displacement surface(s)
      m_FilterSmoothMgr.Apply(pFilter, pDisp, m_iPaintAxis, m_vecPaintAxis,
                              m_bAutoSew);
    }
    return;
  }
  case DISPPAINT_EFFECT_RAISETO: {
    ...
    if (pFilter) {
      ...
      // apply the filter to the displacement surface(s)
      m_FilterRaiseToMgr.Apply(pFilter, pDisp, m_iPaintAxis, m_vecPaintAxis,
                               m_bAutoSew);
    }
    return;
  }
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CToolDisplace::ApplySpatialPaintTool(UINT nFlags, const Vector2D &vPoint,
                                          CMapDisp *pDisp) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CToolDisplace::ApplySculptSpatialPaintTool(CMapView3D *pView, UINT nFlags,
                                                const Vector2D &vPoint) {
  // Initialize the spatial paint data.
  SpatialPaintData_t spatialData;
...
// Purpose: Handle the overriding of displacement triangle tag.
//-----------------------------------------------------------------------------
void CToolDisplace::HandleTaggingReset(CMapView3D *pView,
                                       const Vector2D &vPoint) {
  // Get the displacement face (if any) at the 2d point.
  ULONG ulFace;
  CMapClass *pObject = NULL;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
EditDispHandle_t CToolDisplace::GetHitPos(CMapView3D *pView,
                                          const Vector2D &vPoint) {
  //
  // get ray info
  //
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
EditDispHandle_t CToolDisplace::CollideWithSelectedDisps(const Vector &rayStart,
                                                         const Vector &rayEnd) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CToolDisplace::RayAABBTest(CMapDisp *pDisp, const Vector &rayStart,
                                const Vector &rayEnd) {
  //
  // make planes
  //
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CToolDisplace::BuildParallelepiped(const Vector &boxMin,
                                        const Vector &boxMax, PLANE planes[6]) {
  int planeIndex = 0;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CToolDisplace::RayPlaneTest(PLANE *pPlane, const Vector &rayStart,
                                 const Vector &rayEnd /*, float *fraction*/) {
  //
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
float CToolDisplace::DistFromPointToRay(const Vector &rayStart,
                                        const Vector &rayEnd,
                                        const Vector &point) {
  //
  // calculate the ray
...
bool CToolDisplace::LoadFilters(const char *filename) {
  ...
  if (eResult == ChunkFile_Ok) {
    ...
    Handlers.AddHandler(
        "Filter", (ChunkHandler_t)CToolDisplace::LoadFiltersCallback, this);
...
//-----------------------------------------------------------------------------
ChunkFileResult_t
CToolDisplace::LoadFiltersCallback(CChunkFile *pFile,
                                   CToolDisplace *pDisplaceTool) {
  //
...
void CToolDisplace::RenderPaintSphere(CRender3D *pRender) {
  ...
  pRender->RenderArrow(vCenter, vCenter + (vPaintAxis * (flRadius + flBloat)),
                       255, 255, 0);
