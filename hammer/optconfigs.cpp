...
bool GetPersistentEnvironmentVariable(const char *pName, char *pReturn,
  ...
  if (RegOpenKeyEx(HKEY_CURRENT_USER, VPROJECT_REG_KEY, 0, KEY_QUERY_VALUE,
...
void SetPersistentEnvironmentVariable(const char *pName, const char *pValue) {
  ...
  if (RegOpenKeyEx(HKEY_CURRENT_USER, VPROJECT_REG_KEY, 0, KEY_ALL_ACCESS,
                   &hregkey) != ERROR_SUCCESS)
    return;
...
BOOL COPTConfigs::OnApply(void) {
  ...
  if (pConfig != NULL && ConfigChanged(pConfig)) {
    ...
    AfxMessageBox("Your changes to the active configuration will not take "
                  "effect until the next time you run Hammer.");
  }
