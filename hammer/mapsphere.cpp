...
//
// Purpose: Implements a sphere helper for entities that have a radius of
// effect.
...
void CMapSphere::CalcBounds(BOOL bFullUpdate) {
  ...
  m_BoundingBox.ResetBounds(); // we don't want to use the bounds of the sphere
...
// Output :
//-----------------------------------------------------------------------------
bool CMapSphere::HitTest2D(CMapView2D *pView, const Vector2D &point,
                           HitInfo_t &HitData) {
...
void CMapSphere::Render2D(CRender2D *pRender) {
  if (m_pParent->IsSelected() && (m_flRadius > 0)) {
    ...
    pRender->TransformNormal(ptClientRadius,
