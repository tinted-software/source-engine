...
static int WrapFunctionWithMinidumpHandler(int (*pfn)(void *pParam),
                                           void *pParam) {
  int nRetVal;
...
//-----------------------------------------------------------------------------
CDocument *CHammerDocTemplate::OpenDocumentFile(LPCTSTR lpszPathName,
                                                BOOL bMakeVisible) {
  CDocument *pDoc =
      __super::OpenDocumentFile(lpszPathName, CHammer::IsNewDocumentVisible());

  return pDoc;
}

//-----------------------------------------------------------------------------
// Purpose: this function will attempt an orderly shutdown of all maps.  It will
// attempt to
//			close only documents that have no references, hopefully
//freeing up additional documents
// Input  : bEndSession - ignored
//-----------------------------------------------------------------------------
void CHammerDocTemplate::CloseAllDocuments(BOOL bEndSession) {
  bool bFound = true;

  // rough loop to always remove the first map doc that has no references, then
  // start over, try again. if we still have maps with references ( that's bad
  // ), we'll exit out of this loop and just do the default shutdown to force
  // them all to close.
  while (bFound) {
    bFound = false;
...
//-----------------------------------------------------------------------------
// Purpose: This function will allow hammer to control the initial visibility of
// an opening document Input  : pFrame - the new document's frame
//			pDoc - the new document
//			bMakeVisible - ignored as a parameter
//-----------------------------------------------------------------------------
void CHammerDocTemplate::InitialUpdateFrame(CFrameWnd *pFrame, CDocument *pDoc,
                                            BOOL bMakeVisible) {
...
//-----------------------------------------------------------------------------
// Purpose: this function will let all other maps know that an instanced map has
// been updated ( usually for volume size ) Input  : pInstanceMapDoc - the
...
InitReturnVal_t CHammer::Init() {
  return (InitReturnVal_t)WrapFunctionWithMinidumpHandler(
      &CHammer::StaticHammerInternalInit, this);
}
...
statically #endif
  */
  ...
  pManifestDocTemplate = new CHammerDocTemplate(
      IDR_MANIFESTDOC, RUNTIME_CLASS(CManifest),
      RUNTIME_CLASS(CChildFrame), // custom MDI child frame
      RUNTIME_CLASS(CMapView2D));
  HINSTANCE hInst = AfxFindResourceHandle(MAKEINTRESOURCE(IDR_MAPDOC), RT_MENU);
  pManifestDocTemplate->m_hMenuShared =
      ::LoadMenu(hInst, MAKEINTRESOURCE(IDR_MAPDOC));
  hInst = AfxFindResourceHandle(MAKEINTRESOURCE(IDR_MAPDOC), RT_ACCELERATOR);
  pManifestDocTemplate->m_hAccelTable =
      ::LoadAccelerators(hInst, MAKEINTRESOURCE(IDR_MAPDOC));
...
//-----------------------------------------------------------------------------
// Purpose: this function sets the global flag indicating if new documents
// should
//			be visible.
...
//-----------------------------------------------------------------------------
// Purpose: this functionr eturns the global flag indicating if new documents
// should
//			be visible.
//-----------------------------------------------------------------------------
...
void CHammer::OnFileOpen(void) {
  ...
  CFileDialog dlg(TRUE, NULL, NULL,
                  ...
                  "Valve Map Files (*.vmf;*.vmm)|*.vmf;*.vmm|Valve Map Files "
                  "Autosave (*.vmf_autosave)|*.vmf_autosave|Worldcraft RMFs "
...
//-----------------------------------------------------------------------------
// Returns true if this is a key message that is not a special dialog navigation
// message.
//-----------------------------------------------------------------------------
...
BOOL CHammer::PreTranslateMessage(MSG *pMsg) {
  ...
  if (IsKeyStrokeMessage(pMsg)) {
    ...
    // The classname of dialog window in the VGUI model browser and particle
    // browser is AfxWnd100sd in Debug and AfxWnd100s in Release
...
//directory size 			pstrMapTitle - the name of the current map to be saved
...
//immediately after a new 			directory is entered in the options screen and during
//every autosave call. 			If called with a directory, the input directory is
//checked for correctness. 			Otherwise, the system directory DIR_AUTOSAVE is
//checked
//-----------------------------------------------------------------------------
bool CHammer::VerifyAutosaveDirectory(char *szAutosaveDirectory) const {
  ...
  if (szRootDir[0] == 0) {
    AfxMessageBox("No autosave directory has been selected.\nThe autosave "
                  "feature will be disabled until a directory is entered.",
                  MB_OK);
    return false;
  }
  CString strAutosaveDirectory(szRootDir);
  ...
  if (hDir == INVALID_HANDLE_VALUE) {
    ...
    if (!bDirResult) {
      AfxMessageBox("The current autosave directory does not exist and could "
                    "not be created.  \nThe autosave feature will be disabled "
                    "until a new directory is entered.",
                    MB_OK);
      return false;
    }
  } else {
    ...
    if (hTestFile == INVALID_HANDLE_VALUE) {
      if (GetLastError() == ERROR_ACCESS_DENIED) {
        ...
        AfxMessageBox("There is a problem with the autosave directory.  Please "
                      "select a new directory in Tools->Options->General.\nThe "
                      "autosave feature will be disabled.",
                      MB_OK);
        return false;
      }
    }
