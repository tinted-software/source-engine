...
protected:
  ...
  void StartNew(CMapView *pView, const Vector2D &vPoint, const Vector &vecStart,
  ...
  unsigned long UpdateCursor(CMapView *pView, const Vector &vHandleHit,
                             TransformMode_t eTransformMode);

  void HandleToWorld(Vector &vWorld, const Vector &vHandle,
                     const Vector *pCustomHandleBox = NULL);
  const Vector NearestCorner(const Vector2D &vPoint, CMapView *pView,
                             const Vector *pCustomHandleBox = NULL);
  int GetVisibleHandles(Vector *handles, CMapView *, int nMode);

  void RenderHandles2D(CRender2D *pRender, const Vector &mins,
                       const Vector &maxs);
  void RenderHandles3D(CRender3D *pRender, const Vector &mins,
                       const Vector &maxs);
...
public:
  ...
  // If pCustomHandleBox is non-null, it points at an array 2 vectors (min and
  // max), and it will use those bounds to figure out the corners that it will
  // align to the grid.
  virtual void StartTranslation(CMapView *pView, const Vector2D &vPoint,
                                const Vector &vHandleOrigin,
                                const Vector *pRefPoint = NULL,
                                const Vector *pCustomHandleBox = NULL);
...
protected:
  ...
  Vector m_vTranslationFixPoint;   // fix point, meaning it remains unchanged by
                                   // translation, eg rotation center etc.
...
// Purpose:
//-----------------------------------------------------------------------------
WorldUnits_t Box3D::GetWorldUnits(void) { return (m_eWorldUnits); }
