...
void CAngleBox::OnMouseMove(UINT nFlags, CPoint point) {
  if (m_bDragging) {
    ...
    //
    int nNewYaw =
        fixang(180 - (int)lineangle(point.x, point.y, m_ptClientCenter.x,
                                    m_ptClientCenter.y));
    m_vecAngles.Init();
...
void CAngleBox::DrawAngleLine(CDC *pDC) {
  ...
  CPoint pt;
  pt.x = m_ptClientCenter.x +
         sin(DEG2RAD((double)(m_vecAngles[YAW] + 90))) * rad + 0.5;
  pt.y = m_ptClientCenter.y +
         cos(DEG2RAD((double)(m_vecAngles[YAW] + 90))) * rad + 0.5;
...
char *CAngleBox::GetAngles(char *szAngles) {
  QAngle vecAngles;
  GetAngles(vecAngles);
  sprintf(szAngles, "%g %g %g", (double)vecAngles[0], (double)vecAngles[1],
          (double)vecAngles[2]);
  return (szAngles);
}
...
//			This is used for setting the text in the companion edit
//control.
...
char *CAngleBox::GetAngleEditText(char *szBuf) {
  ...
  if (m_bDifferent) {
    strcpy(szBuf, "(diff)");
  } else if ((m_vecAngles[PITCH] == 90) && (m_vecAngles[YAW] == 0) &&
             (m_vecAngles[ROLL] == 0)) {
    strcpy(szBuf, "Down");
  } else if ((m_vecAngles[PITCH] == -90) && (m_vecAngles[YAW] == 0) &&
             (m_vecAngles[ROLL] == 0)) {
...
//-----------------------------------------------------------------------------
// Purpose: Called internally and by the linked combo box, this updates the
// angles
...
void CAngleBox::SetAngles(const char *szAngles, bool bRedraw) {
  ...
  sscanf(szAngles, "%f %f %f", &vecAngles[PITCH], &vecAngles[YAW],
         &vecAngles[ROLL]);
...
//			angles to avoid mucking with the angles unless they
//explicitly set 			them to something new.
//-----------------------------------------------------------------------------
...
//			internal functions so we don't get a reflected
//notification, mucking 			up our state.
//-----------------------------------------------------------------------------
