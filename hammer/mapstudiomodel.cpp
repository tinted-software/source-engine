...
//-----------------------------------------------------------------------------
CMapClass *CMapStudioModel::CreateMapStudioModel(CHelperInfo *pHelperInfo,
                                                 CMapEntity *pParent) {
  ...
  if (pszModel != NULL) {
    ...
    bool bOrientedBounds =
        (bLightProp | !stricmp(pHelperInfo->GetName(), "studioprop"));
...
//				to each path in the	application search path
//until the model is found. 			bOrientedBounds - Whether the bounding box should
//consider the orientation of the model.
// Output : Returns a pointer to the newly created CMapStudioModel object.
//-----------------------------------------------------------------------------
CMapStudioModel *CMapStudioModel::CreateMapStudioModel(const char *pszModelPath,
                                                       bool bOrientedBounds,
                                                       bool bReversePitch) {
...
void CMapStudioModel::CalcBounds(BOOL bFullUpdate) {
  ...
  if (m_pStudioModel != NULL) {
    //
    // The 3D bounds are the bounds of the oriented model's first sequence, so
    // that frustum culling works properly in the 3D view.
    //
    QAngle angles;
    ...
    if (m_bOrientedBounds) {
      ...
      //
      // The 2D bounds are the movement bounding box of the model, which is not
      // affected by the entity's orientation. This is used for character models
      // for which we want to render a meaningful collision box in the editor.
      //
...
// Output : Returns this.
//-----------------------------------------------------------------------------
CMapClass *CMapStudioModel::CopyFrom(CMapClass *pObject,
...
//-----------------------------------------------------------------------------
void CMapStudioModel::OnParentKeyChanged(const char *szKey,
                                         const char *szValue) {
...
// Draws basis vectors
//-----------------------------------------------------------------------------
static void DrawBasisVectors(CRender3D *pRender, const Vector &origin,
                             const QAngle &angles) {
  ...
  meshBuilder.Position3f(origin[0] + (100 * fCurrentMatrix[0][0]),
                         origin[1] + (100 * fCurrentMatrix[1][0]),
  ...
  meshBuilder.Position3f(origin[0] + (100 * fCurrentMatrix[0][1]),
                         origin[1] + (100 * fCurrentMatrix[1][1]),
  ...
  meshBuilder.Position3f(origin[0] + (100 * fCurrentMatrix[0][2]),
                         origin[1] + (100 * fCurrentMatrix[1][2]),
...
void CMapStudioModel::Render2D(CRender2D *pRender) {
  ...
  bool bDrawAsModel =
      (Options.view2d.bDrawModels && ((sizeX + sizeY) > 50)) || IsSelected() ||
  ...
  if (bDrawAsModel) {
    ...
    if (GetSelectionState() == SELECT_NORMAL ||
        (pRender->IsInLocalTransformMode() &&
         pRender->GetInstanceRendering() == false)) {
  ...
  if (m_pStudioModel != NULL) {
    ...
    if ((fabs(ViewPoint[0] - Origin[0]) < m_fRenderDistance) &&
      ...
      if (GetSelectionState() != SELECT_NONE) {
        ...
        // If the user disabled collisions on this instance of the model, color
        // the wireframe differently
        if (m_iSolid != -1) {
          if (m_iSolid == 0) {
            rgbColor.r =
                GetRValue(Options.colors.clrModelCollisionWireframeDisabled);
            rgbColor.g =
                GetGValue(Options.colors.clrModelCollisionWireframeDisabled);
            rgbColor.b =
                GetBValue(Options.colors.clrModelCollisionWireframeDisabled);
            rgbColor.a = 255;
          } else {
      ...
      bool bWireframe =
          pRender->GetCurrentRenderMode() == RENDER_MODE_WIREFRAME;
      ...
      if (IsSelected()) {
        pRender->RenderWireframeBox(m_Render2DBox.bmins, m_Render2DBox.bmaxs,
                                    255, 255, 0);
      }
    } else {
      pRender->BeginRenderHitTarget(this);
      pRender->RenderBox(m_Render2DBox.bmins, m_Render2DBox.bmaxs,
...
//			boxes. If this is set to zero, studio models are never
//rendered.
