...
//-----------------------------------------------------------------------------
// Purpose: this function is called when the file name edit box is updated.  it
...
CManifestListBox::CManifestListBox(void) : CListBox() {
  ...
  m_ManifestFilterSecondaryMenu.Attach(
      ::GetSubMenu(m_ManifestFilterMenu.m_hMenu, 0));
  m_ManifestFilterPrimaryMenu.Attach(
      ::GetSubMenu(m_ManifestFilterMenu.m_hMenu, 1));
  m_ManifestFilterBlankMenu.Attach(
      ::GetSubMenu(m_ManifestFilterMenu.m_hMenu, 2));

  m_pTrackerManifestMap = NULL;
}

//-----------------------------------------------------------------------------
// Purpose: this function handles the owner draw of the list box for each
// manifest sub map Input  : lpDrawItemStruct - the list box item being rendered
//-----------------------------------------------------------------------------
void CManifestListBox::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) {
  ...
  if (pManifestMap) {
    ...
    dc.Draw3dRect(&VisibleRect, ::GetSysColor(COLOR_3DSHADOW),
    ...
    dc.DrawText(pManifestMap->m_FriendlyName, -1, &VisibleRect,
                DT_LEFT | DT_VCENTER);
  }
...
//-----------------------------------------------------------------------------
// Purpose: this function responds to the left mouse button being pressed.  the
// left side of the manifest item
//			is the visibility toggle.  the right side handles the
//selection to make it the primary map.
...
//-----------------------------------------------------------------------------
// Purpose: passes along the double click message if it is on the right side of
// a list item Input  : nFlags - not used
...
//			are over the primary map, a different sub map, or an
//empty spot.
...
void CManifestListBox::OnRButtonUp(UINT nFlags, CPoint point) {
  ...
  Invalidate();
  pWhichMenu->TrackPopupMenu(TPM_LEFTBUTTON | TPM_RIGHTBUTTON | TPM_LEFTALIGN,
...
//-----------------------------------------------------------------------------
// Purpose: this function handles the menu command for moving the selection to
// an existing sub map
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: this function handles the menu command for moving the selection to a
// new sub map
//-----------------------------------------------------------------------------
void CManifestListBox::OnMoveSelectionToNewSubMap() {
  ...
  if (ManifestMove.DoModal() == IDOK) {
    ...
    CManifest *pManifest = activeDoc->GetManifest();
    CManifestMap *pNewManifestMap = pManifest->MoveSelectionToNewSubmap(
        FriendlyName, FileName, ManifestMove.GetCenterContents());
    if (pNewManifestMap) {
    }
  }
}
...
void CManifestListBox::OnInsertEmptySubMap() {
  ...
  if (ManifestMove.DoModal() == IDOK) {
    ...
    CManifestMap *pNewManifestMap =
        pManifest->AddNewSubmap(FriendlyName, FileName);
    if (pNewManifestMap) {
    }
  }
}
...
//-----------------------------------------------------------------------------
// Purpose: this function will handle bringing up the properties dialog of a sub
// map
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: this function will remove an existing sub map from the manifest, but
// will
//			not delete the file.
//-----------------------------------------------------------------------------
void CManifestListBox::OnManifestRemove() {
  ...
  if (AfxMessageBox(
          "Are you sure you want to remove this sub map from the manifest?",
  ...
  AfxMessageBox("The sub map has been removed from the manifest, but the file "
...
BOOL CManifestFilter::Create(CWnd *pParentWnd) {
  if (!CHammerBar::Create(pParentWnd, IDD_MANIFEST_CONTROL,
                          CBRS_RIGHT | CBRS_SIZE_DYNAMIC, IDCB_MANIFEST_CONTROL,
                          "Manifest Control")) {
    return FALSE;
  }
...
//-----------------------------------------------------------------------------
// Purpose: this function will update the manifest list ( for when sub maps are
// added / removed
...
ON_LBN_SELCHANGE(IDC_MANIFEST_LIST,
                 &CManifestFilter::OnLbnSelchangeManifestList)
...
//-----------------------------------------------------------------------------
// Purpose: this function will handle the selection of a sub map to set it to
// the primary map
//-----------------------------------------------------------------------------
  ...
  if ((nIndex != LB_ERR) && (nCount > 1)) {
    ...
    if (activeDoc && activeDoc->GetManifest()) {
      ...
      CManifestMap *pManifestMap =
          pManifest->GetMap((int)m_ManifestList.GetItemData(nIndex));
...
//-----------------------------------------------------------------------------
// Purpose: this function sets the text and background color of the custom list
// box Input  : pDC - the display context
...
//-----------------------------------------------------------------------------
// Purpose: this function handles the destruction of the dialog to make sure the
// brush is destroyed
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
CManifestMapDlg::CManifestMapDlg(CManifestMap *pManifestMap,
                                 CWnd *pParent /*=NULL*/)
...
BOOL CManifestCheckin::OnInitDialog() {
  ...
  m_CheckinListCtrl.SetExtendedStyle(m_CheckinListCtrl.GetExtendedStyle() |
  ...
  if (activeDoc && activeDoc->GetManifest()) {
    ...
    if (pManifest->m_bCheckedOut) {
      if (p4->GetFileInfo(pManifest->GetPathName(), &FileInfo) == true) {
        ...
        if (pManifest->m_bDefaultCheckin) {
          ListView_SetItemState(m_CheckinListCtrl.m_hWnd, nIndex,
    ...
    for (int i = 0; i < pManifest->GetNumMaps(); i++) {
      ...
      if (pManifestMap->m_bCheckedOut) {
        if (p4->GetFileInfo(pManifestMap->m_AbsoluteMapFileName, &FileInfo) ==
            true) {
          int nIndex = m_CheckinListCtrl.InsertItem(nCount, "");
          nCount++;
          ...
          m_CheckinListCtrl.SetItemText(nIndex, 2,
                                        p4->String(FileInfo.m_sName));
          m_CheckinListCtrl.SetItemText(nIndex, 3,
                                        p4->String(FileInfo.m_sPath));
...
void CManifestCheckin::OnBnClickedOk() {
  ...
  for (int i = 0; i < m_CheckinListCtrl.GetItemCount(); i++) {
    if (m_CheckinListCtrl.GetItemState(i, LVIS_STATEIMAGEMASK) ==
        INDEXTOSTATEIMAGEMASK(LVIS_SELECTED)) {
      nFileCount++;
    }
  }
  ...
  if (nFileCount > 0) {
    ...
    if (Description.GetLength() < 2) {
      AfxMessageBox("Please put in something descriptive for the description.  "
                    "I took the time to type this dialog, the least you could "
    ...
    const char **ppFileNames =
        (const char **)stackalloc(nFileCount * sizeof(char *));
    ...
    for (int i = 0; i < m_CheckinListCtrl.GetItemCount(); i++) {
      if (m_CheckinListCtrl.GetItemState(i, LVIS_STATEIMAGEMASK) ==
          INDEXTOSTATEIMAGEMASK(LVIS_SELECTED)) {
        CManifestMap *pManifestMap =
            (CManifestMap *)m_CheckinListCtrl.GetItemData(i);
