...
//          information, as opposed to building a clip plane during
//          "translation"
...
void Clipper3D::SetClipObjects(const CMapObjectList *pList) {
  ...
  FOR_EACH_OBJ(*m_pOrigObjects, pos) {
    ...
    pObject->EnumChildren(ENUMMAPCHILDRENPROC(AddToClipList), DWORD(this),
...
void Clipper3D::SaveClipSolid(CMapSolid *pSolid, CMapSolid *pOrigSolid) {
  ...
  //
  // Add the new solid to the original solid's parent (group, entity, world,
  // etc.).
  //
  m_pDocument->AddObjectToWorld(pSolid, pOrigSolid->GetParent());

  //
  // handle linking solid into selection -- via selection set when parent is the
  // world and selected, or set the selection state if parent is group or entity
  // in selection set
  //
...
//-----------------------------------------------------------------------------
// Purpose: This function saves all the clipped solid information.  If new
// solids
...
//			nFlags -
//-----------------------------------------------------------------------------
void Clipper3D::DrawBrushExtents(CRender2D *pRender, CMapSolid *pSolid,
                                 int nFlags) {
  //
  ...
  //
  if ((m_ClipPlane.normal[0] == 0) && (m_ClipPlane.normal[1] == 0) &&
      (m_ClipPlane.normal[2] == 0))
    return;
...
// Output : int
//-----------------------------------------------------------------------------
int Clipper3D::HitTest(CMapView *pView, const Vector2D &ptClient,
                       bool bTestHandles) {
  // check points
...
//			nFlags -
//-----------------------------------------------------------------------------
bool Clipper3D::OnKeyDown2D(CMapView2D *pView, UINT nChar, UINT nRepCnt,
                            UINT nFlags) {
  switch (nChar) {
  case 'O': {
    //
...
//-----------------------------------------------------------------------------
bool Clipper3D::OnLMouseDown2D(CMapView2D *pView, UINT nFlags,
...
// Output : Returns true if the message was handled, false if not.
//-----------------------------------------------------------------------------
bool Clipper3D::OnLMouseUp2D(CMapView2D *pView, UINT nFlags,
                             const Vector2D &vPoint) {
...
// Output : Returns true if the message was handled, false if not.
//-----------------------------------------------------------------------------
bool Clipper3D::OnMouseMove2D(CMapView2D *pView, UINT nFlags,
                              const Vector2D &vPoint) {
...
// Output : Returns true if the message was handled, false if not.
//-----------------------------------------------------------------------------
bool Clipper3D::OnKeyDown3D(CMapView3D *pView, UINT nChar, UINT nRepCnt,
                            UINT nFlags) {
  switch (nChar) {
  case VK_RETURN: {
