...
//-----------------------------------------------------------------------------
// Purpose: Returns true if we are dragging a camera, false if not. // dvs:
// rename
//-----------------------------------------------------------------------------
...
// Output : int
//-----------------------------------------------------------------------------
int Camera3D::HitTest(CMapView *pView, const Vector2D &ptClient,
                      bool bTestHandles) {
...
void Camera3D::RenderTool2D(CRender2D *pRender) {
  for (int i = 0; i < Cameras.Count(); i++) {
    ...
    pRender->DrawLine(pDrawCam->position[MovePos],
                      pDrawCam->position[MoveLook]);
...
//-----------------------------------------------------------------------------
ChunkFileResult_t Camera3D::LoadCameraKeyCallback(const char *szKey,
                                                  const char *szValue,
                                                  CAMSTRUCT *pCam) {
...
// Output : Returns ChunkFile_Ok to indicate success.
//-----------------------------------------------------------------------------
ChunkFileResult_t Camera3D::LoadCamerasKeyCallback(const char *szKey,
                                                   const char *szValue,
                                                   Camera3D *pCameras) {
...
//-----------------------------------------------------------------------------
ChunkFileResult_t Camera3D::LoadCameraCallback(CChunkFile *pFile,
                                               Camera3D *pCameras) {
  CAMSTRUCT Cam;
  memset(&Cam, 0, sizeof(Cam));

  ChunkFileResult_t eResult =
      pFile->ReadChunk((KeyHandler_t)LoadCameraKeyCallback, &Cam);
...
ChunkFileResult_t Camera3D::LoadVMF(CChunkFile *pFile) {
  ...
  ChunkFileResult_t eResult =
      pFile->ReadChunk((KeyHandler_t)LoadCamerasKeyCallback, this);
  pFile->PopHandlers();
...
ChunkFileResult_t Camera3D::SaveVMF(CChunkFile *pFile, CSaveInfo *pSaveInfo) {
  ...
  if (eResult == ChunkFile_Ok) {
    for (int i = 0; i < Cameras.Count(); i++) {
      ...
      if (eResult == ChunkFile_Ok) {
        eResult = pFile->WriteKeyValueVector3("position",
                                              Cameras[i].position[MovePos]);
      }

      if (eResult == ChunkFile_Ok) {
        eResult =
            pFile->WriteKeyValueVector3("look", Cameras[i].position[MoveLook]);
      }
...
//-----------------------------------------------------------------------------
bool Camera3D::OnKeyDown2D(CMapView2D *pView, UINT nChar, UINT nRepCnt,
                           UINT nFlags) {
...
// Output : Returns true if the message was handled, false if not.
//-----------------------------------------------------------------------------
bool Camera3D::OnLMouseDown2D(CMapView2D *pView, UINT nFlags,
...
// Output : Returns true if the message was handled, false if not.
//-----------------------------------------------------------------------------
bool Camera3D::OnLMouseUp2D(CMapView2D *pView, UINT nFlags,
                            const Vector2D &vPoint) {
  ReleaseCapture();
...
// Output : Returns true if the message was handled, false if not.
//-----------------------------------------------------------------------------
bool Camera3D::OnMouseMove2D(CMapView2D *pView, UINT nFlags,
                             const Vector2D &vPoint) {
  CMapDoc *pDoc = pView->GetMapDoc();
  if (!pDoc) {
    return true;
...
// Output : Returns true if the message was handled, false if not.
//-----------------------------------------------------------------------------
bool Camera3D::OnLMouseDown3D(CMapView3D *pView, UINT nFlags,
                              const Vector2D &vPoint) {
...
// Output : Returns true if the message was handled, false if not.
//-----------------------------------------------------------------------------
bool Camera3D::OnLMouseUp3D(CMapView3D *pView, UINT nFlags,
                            const Vector2D &vPoint) {
...
// Output : Returns true if the message was handled, false if not.
//-----------------------------------------------------------------------------
bool Camera3D::OnRMouseDown3D(CMapView3D *pView, UINT nFlags,
                              const Vector2D &vPoint) {
...
// Output : Returns true if the message was handled, false if not.
//-----------------------------------------------------------------------------
bool Camera3D::OnRMouseUp3D(CMapView3D *pView, UINT nFlags,
                            const Vector2D &vPoint) {
...
// Output : Returns true if the message was handled, false if not.
//-----------------------------------------------------------------------------
bool Camera3D::OnKeyDown3D(CMapView3D *pView, UINT nChar, UINT nRepCnt,
                           UINT nFlags) {
  if (nChar == VK_DELETE || nChar == VK_NEXT || nChar == VK_PRIOR) {
    CMapDoc *pDoc = pView->GetMapDoc();
