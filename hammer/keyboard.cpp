...
//
static KeyMap_t ModifierKeyTable[] = {{VK_SHIFT, KEY_MOD_SHIFT, 0},
...
// Input  : uChar - The virtual keycode of the primary key that must be held
// down.
//			uModifierKeys - Bitflags specifying which modifier keys
//must be 				held down along with the key specified by uChar. 			uLogicalKey - An
//application-specific value that indicates which 				logical function
//-----------------------------------------------------------------------------
void CKeyboard::AddKeyMap(unsigned int uChar, unsigned int uModifierKeys,
...
void CKeyboard::ClearImpulseFlags(void) {
  ...
  //
  for (nKey = 0;
       nKey < sizeof(g_uPhysicalKeyState) / sizeof(g_uPhysicalKeyState[0]);
       nKey++) {
  ...
  //
  for (nKey = 0;
       nKey < sizeof(g_uLogicalKeyState) / sizeof(g_uLogicalKeyState[0]);
       nKey++) {
...
//			0.25 if a key was pressed and released during the sample
//period, 			0.5 if it was pressed and held, 			0 if held then released, and 			1.0 if
...
float CKeyboard::GetKeyScale(unsigned int uLogicalKey) {
  ...
  //
  // If we have both a leading and trailing edge, it was either released and
  // repressed this frame, or pressed and released this frame.
  //
  if (bImpulseDown && bImpulseUp) {
    if (bDown) {
      //
...
// Input  : uModifierKey - The virtual key code corresponding to the modifier
// key. Output : The modifier key's bitmask.
//-----------------------------------------------------------------------------
unsigned int CKeyboard::GetModifierKeyBit(unsigned int uChar) {
  for (int nKey = 0;
       nKey < sizeof(ModifierKeyTable) / sizeof(ModifierKeyTable[0]); nKey++) {
...
//-----------------------------------------------------------------------------
// Purpose: Checks to see if all of the modifier keys specified by bits in
// uModifierKeys
//			are currently held down.
// Input  : uModifierKeys - Contains bits indicating which modifier keys to
// check:
//				KEY_MOD_SHIFT
//				KEY_MOD_CONTROL
//				KEY_MOD_ALT
...
//			can be used in conjunction with other keys when
//performing key 			bindings.
...
//			bPressed - True if the key was pressed, false if it was
//released.
//-----------------------------------------------------------------------------
void CKeyboard::UpdateLogicalKeys(unsigned int uChar, bool bPressed) {
  ...
  for (unsigned int nKey = 0; nKey < g_uKeyMaps; nKey++) {
    ...
    if ((uPhysicalKey == uChar) || (uModifierKeys & uModifierKeyBit)) {
      ...
      //
      bool bLogicalKeyPressed =
          IsKeyPressed(g_uKeyMap[nKey].uChar, g_uKeyMap[nKey].uModifierKeys);
