...
public:
  ...
  // Factories.
  //
  static CMapClass *CreateMapStudioModel(CHelperInfo *pHelperInfo,
                                         CMapEntity *pParent);
  static CMapStudioModel *CreateMapStudioModel(const char *pszModelPath,
                                               bool bOrientedBBox,
                                               bool bReversePitch);
...
protected:
  ...
  bool m_bOrientedBounds; // Whether the bounding box should consider the
                          // orientation of the model. Note that this is not a
                          // true oriented bounding box, but an axial box
                          // indicating the extents of the oriented model.

  bool m_bReversePitch; // Lights negate pitch, so models representing light
                        // sources in Hammer must do so as well.

  bool m_bScreenSpaceFade; // If true, min & max dist are pixel size in screen
                           // space.
  float m_flFadeScale;   // Multiplied by distance to camera before calculating
                         // fade.
  float m_flFadeMinDist; // The distance/pixels at which this model is fully
                         // visible.
  float m_flFadeMaxDist; // The distance/pixels at which this model is fully
                         // invisible.
  int m_iSolid; // The collision setting of this model: 0 = not solid, 2 =
                // bounding box, 6 = vphysics

  //
  // Data that is common to all studio models.
  //
  static float m_fRenderDistance; // Distance beyond which studio models render
                                  // as bounding boxes.
...
//			Note that this is not a true oriented bounding box, but
//an axial box 			indicating the extents of the oriented model.
//-----------------------------------------------------------------------------
