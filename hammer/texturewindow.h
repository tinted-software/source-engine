...
public:
  ...
  struct TWENUMPOS {
    ...
    int largest_y;
    int iTexIndex;
    int nUsageCount; // The number of times this texture is used in the map.
                     // Only set in "Used Textures Only" mode.
  };
  ...
  enum {
    ...
    TYPEFILTER_ALL = (TYPEFILTER_OPAQUE | TYPEFILTER_TRANSLUCENT |
                      TYPEFILTER_SELFILLUM | TYPEFILTER_ENVMASK),
  };
...
protected:
  ...
  int m_nFilters;       // The number of names that were parsed out of the name
                        // filter.
  char *m_Filters[64];  // The individual name filters.

  char m_szKeywords[128]; // Keyword filter, space, comma, or semicolon
                          // delimited.
  int m_nKeywords; // The number of keywords that were parsed out of the name
                   // filter.
