...
SolidTypeInfo_t SolidTypes[] = {
    {"block", 0, 6, 6, false},    {"wedge", 0, 5, 5, false},
    {"cylinder", 8, 3, 32, true}, {"spike", 4, 3, 32, true},
    {"sphere", 8, 3, 16, true},   {"arch", 8, 3, 128, true},
    {"torus", 0, 4, 128, false},
};
...
bool CObjectBar::UseRandomYawOnEntityPlacement() {
  return ::SendMessage(::GetDlgItem(GetSafeHwnd(), IDC_RANDOMYAW), BM_GETCHECK,
                       0, 0) == BST_CHECKED;
}
...
CPrefab *CObjectBar::FindPrefabByName(const char *pName) {
  CPrefabLibrary *pLibrary = CPrefabLibrary::FindID(
      m_CategoryList.GetItemData(m_CategoryList.GetCurSel()));
  if (pLibrary) {
...
CMapClass *CObjectBar::CreateInBox(BoundBox *pBox, CMapView *pView) {
  ...
  if (ListType == listPrimitives) {
    ...
    if ((SolidTypes[nSolidIndex].bEnableFaceControl) &&
        (nFaces < nFacesMin || nFaces > nFacesMax)) {
      CString str;
    ...
    if (nSolidIndex < 5) {
      ...
      dlg.m_iSides = nFaces;
      dlg.SetMaxWallWidth(
          min((int)sizebounds[axHorz], (int)sizebounds[axVert]));
      if (dlg.DoModal() != IDOK)
        return NULL;
      ...
      CMapClass *pArch =
          CreateArch(pBox, dlg.m_fAngle, dlg.m_iSides, dlg.m_fArc,
                     dlg.m_iWallWidth, dlg.m_iAddHeight, FALSE);
      ...
      CMapClass *pTorus = CreateTorus(
          pBox, dlg.m_fAngle, dlg.m_iSides, dlg.m_fArc, dlg.m_iWallWidth,
...
//-----------------------------------------------------------------------------
// Purpose: Returns the classname of the default entity for the entity creation
// tool.
//-----------------------------------------------------------------------------
LPCTSTR CObjectBar::GetDefaultEntityClass(void) { return _szNewObjName; }
...
BOOL CObjectBar::Create(CWnd *pParentWnd) {
  if (!CHammerBar::Create(pParentWnd, IDD_OBJECTBAR, CBRS_RIGHT,
                          IDCB_OBJECTBAR)) {
    return FALSE;
  }
...
//-----------------------------------------------------------------------------
// Purpose: Load the category list with the list of primitives and object
// libraries.
//-----------------------------------------------------------------------------
...
void CObjectBar::OnTextChanged(const char *pSelection) {
  ...
  case listPrimitives: {
    ...
    //
    // Save current value from the faces edit control. The next time this
    // primitive is selected the value will be restored.
    //
    if (iBlockSel != -1) {
      int nSolidIndex = iBlockSel;
      m_Faces.GetWindowText(szBuf, 128);
  ...
  m_FacesSpin.SetRange(SolidTypes[nSolidIndex].nFacesMin,
                       SolidTypes[nSolidIndex].nFacesMax);
...
void CObjectBar::UpdateListForTool(int iTool) {
  ...
  if (m_iLastTool == TOOL_BLOCK) {
    ...
    if (iCategory != -1) {
      m_CategoryList.GetLBText(iCategory,
