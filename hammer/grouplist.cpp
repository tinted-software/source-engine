...
//			posts a registered windows message to the list view's
//parent window 			when visgroups are hidden or shown.
//
//=============================================================================//
...
static const unsigned int g_uToggleStateMsg =
    ::RegisterWindowMessage(GROUPLIST_MSG_TOGGLE_STATE);
static const unsigned int g_uLeftDragDropMsg =
    ::RegisterWindowMessage(GROUPLIST_MSG_LEFT_DRAG_DROP);
static const unsigned int g_uRightDragDropMsg =
    ::RegisterWindowMessage(GROUPLIST_MSG_RIGHT_DRAG_DROP);
static const unsigned int g_uSelChangeMsg =
    ::RegisterWindowMessage(GROUPLIST_MSG_SEL_CHANGE);
...
//			hItemParent -
//-----------------------------------------------------------------------------
void CGroupList::AddVisGroupRecursive(CVisGroup *pVisGroup,
...
void CGroupList::DeleteAllItems(void) {
  // Un-set all item data because sometimes during a delete it'll trigger
  // selection change notifications which might crash things later.
...
//-----------------------------------------------------------------------------
HTREEITEM CGroupList::FindVisGroupItemRecursive(HTREEITEM hItem,
                                                CVisGroup *pVisGroup) {
  if (hItem) {
...
//-----------------------------------------------------------------------------
// Purpose: Returns the tree item associated with the given visgroup, NULL if
// none.
//-----------------------------------------------------------------------------
    ...
    if (hItemHit != NULL) {
      if (uFlags & TVHT_ONITEMICON) {
        ...
        if (pwndParent != NULL) {
          ...
          pwndParent->PostMessage(g_uToggleStateMsg, (WPARAM)pGroup,
                                  nCheckState);
        }
...
void CGroupList::BeginDrag(CPoint point, HTREEITEM hItem) {
  ...
  if (m_hDragItem) {
    ...
    // Timer handles scrolling the list control when dragging outside the window
    // bounds.
...
void CGroupList::Drop(DropType_t eDropType, UINT nFlags, CPoint point) {
  ...
  if (pwndParent != NULL) {
    if (eDropType == DROP_LEFT) {
      pwndParent->PostMessage(g_uLeftDragDropMsg, (WPARAM)pDragGroup,
                              (LPARAM)pDropGroup);
    } else {
      pwndParent->PostMessage(g_uRightDragDropMsg, (WPARAM)pDragGroup,
                              (LPARAM)pDropGroup);
    }
  }
}
...
void CGroupList::OnMouseMove(UINT nFlags, CPoint point) {
  ...
  if (m_bRButtonDown && !m_hDragItem && (point.x != m_ptRButtonDown.x) &&
      (point.y != m_ptRButtonDown.y)) {
...
// Input  : pVisGroup -
//			nCheckState - 0=not checked, 1=checked, -1=gray check
//(undefined)
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
int CGroupList::GetVisGroupCount() { return m_VisGroups.Count(); }
...
int CGroupList::GetGroupPairCount(void) { return m_GroupPairs.Count(); }
