...
//			the map information dialog. Routes each object to the
...
//			unique textures in the map and total texture memory
//used. Adds this 			texture's WAD file to the list of used WADs.
...
BOOL CMapInfoDlg::OnInitDialog(void) {
  ...
  sprintf(szBuf, "%u bytes (%.2f MB)", m_uTextureMemory,
          (float)m_uTextureMemory / 1024000.0);
