...
public:
  ...
  typedef struct {
    CMapDoc *pMapDoc; // From whence it came, so if we close the CMapDoc we can
                      // unselect it.
    CMapFace *pMapFace;
    CMapSolid *pMapSolid;
  } StoredFace_t;
  ...
  //
  CFaceEditSheet(LPCTSTR pszCaption, CWnd *pParentWnd = NULL,
                 UINT iSelectPage = 0);
  ...
  // Called when the CMapDoc goes away. Don't hang onto old face pointers that
  // come from this doc.
  ...
  enum {
    ...
    cfEdgeAlign = 0x10 // align face texture coordinates to 3d view alignment -
                       // should be here???
  };
...
protected:
  ...
  //	CFaceEditMaterialPage				m_MaterialPage;
  //// material "page" 	CFaceEditDispPage
  //m_DispPage;				// displacement "page"
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
inline void CFaceEditSheet::SetClickMode(int mode) { m_ClickMode = mode; }

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
inline int CFaceEditSheet::GetClickMode(void) { return m_ClickMode; }

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
inline int CFaceEditSheet::GetFaceListCount(void) { return m_Faces.Count(); }
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
inline bool CFaceEditSheet::HasUpdateEnabled(void) { return m_bEnableUpdate; }
