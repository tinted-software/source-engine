...
void COPTTextures::OnAddtexfile(void) {
  ...
  CFileDialog dlg(TRUE, "wad", NULL,
                  OFN_ALLOWMULTISELECT | OFN_HIDEREADONLY | OFN_NOCHANGEDIR |
                      OFN_FILEMUSTEXIST,
                  "Texture files (*.wad;*.pak)|*.wad; *.pak||");

  if (szInitialDir[0] == '\0') {
    Q_snprintf(szInitialDir, sizeof(szInitialDir), "%s\\wads\\",
               g_pGameConfig->m_szModDir);
  }
...
void COPTTextures::OnAddtexfile2() {
  ...
  if (pidlNew) {
    ...
    if (AfxMessageBox("Add all subdirectories as separate Texture Groups?",
                      MB_YESNO) == IDYES)
    // if (!strcmpi("\\textures", &szPathName[strlen(szPathName) -
    // strlen("\\textures")]))
    {
...
BOOL COPTTextures::BrowseForFolder(char *pszTitle, char *pszDirectory) {
  ...
  if (pshDesktop) {
    ULONG ulEaten;
    ULONG ulAttributes;
    pshDesktop->ParseDisplayName(NULL, NULL, A2OLE(s_szStartFolder), &ulEaten,
                                 &pidlStartFolder, &ulAttributes);
  }
...
void COPTTextures::MaterialExcludeUpdate(void) {
  ...
  for (int i = 0; i < m_pMaterialConfig->m_MaterialExcludeCount; i++) {
    int result = m_MaterialExcludeList.AddString(
        m_pMaterialConfig->m_MaterialExclusions[i].szDirectory);
...
void COPTTextures::OnMaterialExcludeAdd(void) {
  ...
  Q_strncpy(m_pMaterialConfig->m_MaterialExclusions[index].szDirectory,
            szSubDirName,
