...
public:
  ...
  virtual int HitTest(CMapView *pView, const Vector2D &ptClient,
                      bool bTestHandles = false) = 0;
  int HitTest(CMapView *pView, const Vector &ptWorld,
              bool bTestHandles = false);

  bool HitRect(CMapView *pView, const Vector2D &ptHit, const Vector &vecCenter,
               int extent);
  ...
  void StartTranslation(CMapView *pView, const Vector2D &vClickPoint,
                        bool bUseDefaultPlane = true);
  void ProjectTranslation(CMapView *pView, const Vector2D &vPoint,
                          Vector &vTransform, int nFlags = 0);
  void ProjectOnTranslationPlane(const Vector &vWorld, Vector &vTransform,
                                 int nFlags = 0);
  void SetTransformationPlane(const Vector &vOrigin, const Vector &vHorz,
                              const Vector &vVert, const Vector &vNormal);
