...
enum FaceState_t {
  FaceState_Select = 0, //
  FaceState_Partial, // Used for multiselect; the face is in at least one of the
                     // face lists being edited.
  FaceState_None, // Used for multiselect; to deselect partially selected faces.
                  // Otherwise they are removed from the list.
};
...
public:
  ...
  void GetSelectedFaces(CMapFaceList &FaceListFull,
                        CMapFaceList &FaceListPartial);
  void SetSelectedFaces(CMapFaceList &FaceListFull,
                        CMapFaceList &FaceListPartial);
...
protected:
  ...
  bool
      m_bAllowMultiSelect; // If false, only one face can be selected at a time.
  CUtlVector<SelectedFace_t>
      m_Faces; // Picked faces and their selection state (partial or full).
};
