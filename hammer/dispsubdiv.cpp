...
private: // typedefs, enums, structs
  ...
  struct SubdivQuad_t {
    ...
    SubdivQuadHandle_t
        m_ndxChild[EDITDISP_QUADSIZE]; // chilren (4 of them) indices

    // quad data
    SubdivPointHandle_t
        m_PointHandles[EDITDISP_QUADSIZE]; // point indices - unique list
    SubdivEdgeHandle_t
        m_EdgeHandles[EDITDISP_QUADSIZE]; // edge indices - unique list

    // disp/quad mapping
    EditDispHandle_t m_EditDispHandle;
    short m_Level; // level of quad in the hierarchy (tree)
    short m_QuadIndices[EDITDISP_QUADSIZE]; // quad indices (in the X x X
...
private: // functions
  ...
  bool CompareSubdivPoints(Vector const &pt1, Vector const &pt2,
                           float flTolerance);
  bool CompareSubdivEdges(SubdivPointHandle_t ptEdge0Handle0,
                          SubdivPointHandle_t ptEdge0Handle1,
                          SubdivPointHandle_t ptEdge1Handle0,
                          SubdivPointHandle_t ptEdge1Handle1);

  SubdivPointHandle_t BuildSubdivPoint(Vector const &vPoint,
                                       Vector const &vNormal);
  SubdivEdgeHandle_t BuildSubdivEdge(int ndxEdge, SubdivQuadHandle_t quadHandle,
                                     SubdivQuadHandle_t parentHandle,
                                     int ndxChild);
  SubdivQuadHandle_t BuildSubdivQuad(int ndxChild,
                                     SubdivQuadHandle_t parentHandle);
...
// Purpose:
//-----------------------------------------------------------------------------
CEditDispSubdivMesh::SubdivPoint_t *
CEditDispSubdivMesh::GetPoint(SubdivPointHandle_t ptHandle) {
...
// Purpose:
//-----------------------------------------------------------------------------
CEditDispSubdivMesh::SubdivEdge_t *
CEditDispSubdivMesh::GetEdge(SubdivEdgeHandle_t edgeHandle) {
...
// Purpose:
//-----------------------------------------------------------------------------
CEditDispSubdivMesh::SubdivQuad_t *
CEditDispSubdivMesh::GetQuad(SubdivQuadHandle_t quadHandle) {
...
void CEditDispSubdivMesh::Edge_CalcNewPoint(SubdivEdgeHandle_t edgeHandle) {
  ...
  if ((pEdge->m_QuadHandles[1] != m_Edges.InvalidIndex()) &&
      (pEdge->m_flSharpness != 1.0f)) {
...
// Purpose:
//-----------------------------------------------------------------------------
bool CEditDispSubdivMesh::CompareSubdivPoints(Vector const &pt1,
                                              Vector const &pt2,
                                              float flTolerance) {
...
// Purpose:
//-----------------------------------------------------------------------------
bool CEditDispSubdivMesh::CompareSubdivEdges(
    SubdivPointHandle_t ptEdge0Handle0, SubdivPointHandle_t ptEdge0Handle1,
    SubdivPointHandle_t ptEdge1Handle0, SubdivPointHandle_t ptEdge1Handle1) {
  if (((ptEdge0Handle0 == ptEdge1Handle0) &&
       (ptEdge0Handle1 == ptEdge1Handle1)) ||
      ((ptEdge0Handle0 == ptEdge1Handle1) &&
       (ptEdge0Handle1 == ptEdge1Handle0)))
    return true;
...
// Purpose:
//-----------------------------------------------------------------------------
CEditDispSubdivMesh::SubdivPointHandle_t
CEditDispSubdivMesh::BuildSubdivPoint(Vector const &vPoint,
...
// Purpose:
//-----------------------------------------------------------------------------
CEditDispSubdivMesh::SubdivEdgeHandle_t
CEditDispSubdivMesh::BuildSubdivEdge(int ndxEdge, SubdivQuadHandle_t quadHandle,
                                     SubdivQuadHandle_t parentHandle,
                                     int ndxChild) {
  // get the quad
  SubdivQuad_t *pQuad = GetQuad(quadHandle);
  ...
  for (edgeHandle = m_Edges.Head(); edgeHandle != m_Edges.InvalidIndex();
    ...
    if (pEdge) {
      ...
      if (CompareSubdivEdges(pQuad->m_PointHandles[ndxEdge],
                             pQuad->m_PointHandles[(ndxEdge + 1) % 4],
                             pEdge->m_PointHandles[0],
                             pEdge->m_PointHandles[1])) {
  ...
  if (ndxChild != -1) {
    if ((ndxEdge == ndxChild) || (ndxEdge == ((ndxChild + 3) % 4))) {
      ...
      if (pParentQuad) {
        SubdivEdge_t *pParentEdge =
            GetEdge(pParentQuad->m_EdgeHandles[ndxEdge]);
...
// Purpose:
//-----------------------------------------------------------------------------
CEditDispSubdivMesh::SubdivQuadHandle_t
CEditDispSubdivMesh::BuildSubdivQuad(int ndxChild,
                                     SubdivQuadHandle_t parentHandle) {
  // get parent quad
  SubdivQuad_t *pParentQuad = GetQuad(parentHandle);
  if (!pParentQuad)
  ...
  case 0: {
    ...
    pQuad->m_QuadIndices[1] =
        (pParentQuad->m_QuadIndices[0] + pParentQuad->m_QuadIndices[1]) * 0.5f;
    pQuad->m_QuadIndices[2] =
        (pParentQuad->m_QuadIndices[0] + pParentQuad->m_QuadIndices[2]) * 0.5f;
    pQuad->m_QuadIndices[3] =
        (pParentQuad->m_QuadIndices[0] + pParentQuad->m_QuadIndices[3]) * 0.5f;
    ...
    if (pEdge0 && pEdge3) {
      ...
      pQuad->m_PointHandles[1] =
          BuildSubdivPoint(pEdge0->m_vNewEdgePoint, pEdge0->m_vNewEdgeNormal);
      pQuad->m_PointHandles[2] =
          BuildSubdivPoint(pParentQuad->m_vCentroid, pParentQuad->m_vNormal);
      pQuad->m_PointHandles[3] =
          BuildSubdivPoint(pEdge3->m_vNewEdgePoint, pEdge3->m_vNewEdgeNormal);
    }
  ...
  case 1: {
    // displacement quad indices
    pQuad->m_QuadIndices[0] =
        (pParentQuad->m_QuadIndices[0] + pParentQuad->m_QuadIndices[1]) * 0.5f;
    pQuad->m_QuadIndices[1] = pParentQuad->m_QuadIndices[1];
    pQuad->m_QuadIndices[2] =
        (pParentQuad->m_QuadIndices[1] + pParentQuad->m_QuadIndices[2]) * 0.5f;
    pQuad->m_QuadIndices[3] =
        (pParentQuad->m_QuadIndices[0] + pParentQuad->m_QuadIndices[2]) * 0.5f;
    ...
    if (pEdge0 && pEdge1) {
      pQuad->m_PointHandles[0] =
          BuildSubdivPoint(pEdge0->m_vNewEdgePoint, pEdge0->m_vNewEdgeNormal);
      pQuad->m_PointHandles[1] = pParentQuad->m_PointHandles[1];
      pQuad->m_PointHandles[2] =
          BuildSubdivPoint(pEdge1->m_vNewEdgePoint, pEdge1->m_vNewEdgeNormal);
      pQuad->m_PointHandles[3] =
          BuildSubdivPoint(pParentQuad->m_vCentroid, pParentQuad->m_vNormal);
    }
  ...
  case 2: {
    ...
    pQuad->m_QuadIndices[3] =
        (pParentQuad->m_QuadIndices[2] + pParentQuad->m_QuadIndices[3]) * 0.5f;
    ...
    if (pEdge1 && pEdge2) {
      ...
      pQuad->m_PointHandles[3] =
          BuildSubdivPoint(pEdge2->m_vNewEdgePoint, pEdge2->m_vNewEdgeNormal);
    }
  ...
  for (int ndxEdge = 0; ndxEdge < 4; ndxEdge++) {
    pQuad->m_EdgeHandles[ndxEdge] =
        BuildSubdivEdge(ndxEdge, quadHandle, parentHandle, ndxChild);
  }
...
void CEditDispSubdivMesh::AddDispTo(CMapDisp *pDisp) {
  ...
  // NOTE: this is slow -- should write a faster version (is offline process, do
  // later)
  //
  IWorldEditDispMgr *pDispMgr = GetActiveWorldEditDispManager();
  if (!pDispMgr)
  ...
  for (int ndxPt = 0; ndxPt < EDITDISP_QUADSIZE; ndxPt++) {
    pQuad->m_PointHandles[ndxPt] =
        BuildSubdivPoint(vPoints[ndxPt], vPointNormals[ndxPt]);
  }

  // build subdivision edges
  for (int ndxEdge = 0; ndxEdge < EDITDISP_QUADSIZE; ndxEdge++) {
    pQuad->m_EdgeHandles[ndxEdge] =
        BuildSubdivEdge(ndxEdge, quadHandle, m_Quads.InvalidIndex(), -1);
  }
}
...
void CEditDispSubdivMesh::GetDispFrom(CMapDisp *pDisp) {
  ...
  //
  for (SubdivQuadHandle_t quadHandle = m_Quads.Head();
       quadHandle != m_Quads.InvalidIndex();
    ...
    if (pQuad) {
      ...
      // get the data at the appropriate level -- (based on the size of the
      // displacement)
      ...
      for (int ndxPt = 0; ndxPt < 4; ndxPt++) {
        ...
        if (pPoint) {
          ...
          pDisp->UpdateVertPositionForSubdiv(pQuad->m_QuadIndices[ndxPt],
                                             vSubVert);
          pDisp->SetSubdivNormal(pQuad->m_QuadIndices[ndxPt],
                                 pPoint->m_vNormal);
        }
      }
    }
...
void CEditDispSubdivMesh::CatmullClarkSubdivision(void) {
  ...
  //
  for (SubdivEdgeHandle_t edgeHandle = m_Edges.Head();
       edgeHandle != m_Edges.InvalidIndex();
       edgeHandle = m_Edges.Next(edgeHandle)) {
  ...
  //
  for (SubdivPointHandle_t ptHandle = m_Points.Head();
       ptHandle != m_Points.InvalidIndex();
       ptHandle = m_Points.Next(ptHandle)) {
    for (SubdivEdgeHandle_t edgeHandle = m_Edges.Head();
         edgeHandle != m_Edges.InvalidIndex();
         edgeHandle = m_Edges.Next(edgeHandle)) {
      ...
      if ((ptHandle == pEdge->m_PointHandles[0]) ||
          (ptHandle == pEdge->m_PointHandles[1])) {
        SubdivPoint_t *pPoint = GetPoint(ptHandle);
