...
CToolDisplace *GetDisplacementTool() {
  return dynamic_cast<CToolDisplace *>(
      ToolManager()->GetToolForID(TOOL_FACEEDIT_DISP));
}
...
//}}AFX_MSG_MAP
ON_BN_CLICKED(ID_DISP_SCULPT_PAINT,
              &CFaceEditDispPage::OnBnClickedDispSculptPaint)
END_MESSAGE_MAP()
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CFaceEditDispPage::ClickFace(CMapSolid *pSolid, int faceIndex, int cmd,
                                  int clickMode) {
...
void CFaceEditDispPage::UpdateEditControls(bool bAllDisps, bool bHasFace) {
  ...
  CSpinButtonCtrl *pspinPower =
      (CSpinButtonCtrl *)GetDlgItem(ID_SPIN_DISP_POWER);
  CEdit *peditElevation = (CEdit *)GetDlgItem(ID_DISP_ELEVATION);
  CSpinButtonCtrl *pspinElevation =
      (CSpinButtonCtrl *)GetDlgItem(ID_SPIN_DISP_ELEVATION);
  ...
  // if there aren't any faces selected then the only active item should be
  // selection
  if (!bHasFace)
    return;
...
void CFaceEditDispPage::OnSelectAdjacent() {
  ...
  for (int ndxFace = 0; ndxFace < faceCount; ndxFace++) {
    ...
    // Now test all other disps in the world. We could do the code commented
    // below, but then we won't be selecting adjacent neighbors of a different
    // power.
    //
    ...
    for (int iTestDisp = 0; iTestDisp < totalDispCount; iTestDisp++) {
      ...
      if (!pNeighbor || pNeighbor == pDisp ||
          pDispMgr->NumSharedPoints(pDisp, pNeighbor, edge1, edge2) != 2)
        continue;
      ...
      // The function we use to select a face wants the face's index into its
      // CMapSolid, so we need that.
      int iFaceIndex = pSolid->GetFaceIndex(pFace);
      if (iFaceIndex == -1)
        continue;

      // Finally, select the face.
      pSheet->ClickFace(pSolid, iFaceIndex, CFaceEditSheet::cfSelect,
                        CFaceEditSheet::ModeSelect);
      bSelectedAny = true;
    }
  }
