...
private:
  ...
  virtual void OnFileChange(const char *pRelativeFilename,
...
//			in the cache, a pointer to that model is returned. If
//not, a new one 			is created and added to the cache.
...
//			pszModelPath - The full path of the .MDL file, which is
//used as a 				key in the model cache.
...
//-----------------------------------------------------------------------------
BOOL CStudioModelCache::AddModel(StudioModel *pModel,
...
//-----------------------------------------------------------------------------
// Purpose: Advances the animation of all models in the cache for the given
// interval. Input  : flInterval - delta time in seconds.
//-----------------------------------------------------------------------------
...
void CStudioFileChangeWatcher::Init() {
  ...
  if (g_pFullFileSystem->GetSearchPath("GAME", false, searchPaths,
                                       sizeof(searchPaths)) > 0) {
    ...
    Warning("Error in GetSearchPath. Hammer will not automatically reload "
            "modified models.");
  }
}

void CStudioFileChangeWatcher::OnFileChange(const char *pRelativeFilename,
                                            const char *pFullFilename) {
  ...
  if (V_stricmp(pExt, "mdl") == 0 || V_stricmp(pExt, "vtx") == 0 ||
      V_stricmp(pExt, "phy") == 0 || V_stricmp(pExt, "vvd") == 0) {
    ...
    // Now it might have a "dx80" or "dx90" or some other extension. Get rid of
    // that too.
    ...
    // Now we've got the filename with any extension or "dx80"-type stuff at the
    // end.
    V_strncat(filename, ".mdl", sizeof(filename));

    // Queue up the list of changes because if they copied all the files for a
    // model, we'd like to only reload it once.
...
void CStudioFileChangeWatcher::Update() {
  ...
  if (m_ChangedModels.Count() > 0) {
    ...
    for (int i = m_ChangedModels.First(); i != m_ChangedModels.InvalidIndex();
         i = m_ChangedModels.Next(i)) {
...
//-----------------------------------------------------------------------------
bool StudioModel::Initialize() { return true; }
...
void StudioModel::SetUpBones(bool bUpdatePose, matrix3x4_t *pBoneToWorld) {
  ...
  if (bUpdatePose) {
    ...
    boneSetup.AccumulatePose(m_pPosePos, m_pPoseAng, m_sequence, m_cycle, 1.0f,
                             0.0f, NULL);
  }
  ...
  for (int i = 0; i < pStudioHdr->numbones(); i++) {
    ...
    if (pbones[i].parent == -1) {
      ...
      ConcatTransforms(pBoneToWorld[pbones[i].parent], bonematrix,
                       pBoneToWorld[i]);
    }
  }
}
...
// Purpose:
//-----------------------------------------------------------------------------
void StudioModel::DrawModel3D(CRender3D *pRender, float flAlpha,
                              bool bWireframe) {
...
void StudioModel::DrawModel2D(CRender2D *pRender, float flAlpha,
                              bool bWireFrame) {
  studiohdr_t *pStudioHdr = GetStudioRenderHdr();
  if (!pStudioHdr)
    return;
...
bool StudioModel::IsTranslucent() {
  ...
  int lodID;
  for (lodID = pHardwareData->m_RootLOD; lodID < pHardwareData->m_NumLODs;
       lodID++) {
...
//			bounds of the rotated box. This is used to take the
//rotation angles 			into consideration when returning the bounding box. Note that
//this 			can produce a larger than optimal bounding box.
// Input  : Mins -
//			Maxs -
//			Angles -
//-----------------------------------------------------------------------------
...
void StudioModel::GetSequenceInfo(float *pflFrameRate, float *pflGroundSpeed) {
  ...
  if (t > 0) {
    *pflFrameRate = 1.0 / t;
    *pflGroundSpeed =
        0; // sqrt( pseqdesc->linearmovement[0]*pseqdesc->linearmovement[0]+
           // pseqdesc->linearmovement[1]*pseqdesc->linearmovement[1]+
           // pseqdesc->linearmovement[2]*pseqdesc->linearmovement[2] );
    // *pflGroundSpeed = *pflGroundSpeed * pseqdesc->fps / (pseqdesc->numframes
    // - 1);
  } else {
    *pflFrameRate = 1.0;
    *pflGroundSpeed = 0.0;
...
void StudioModel::SetOrigin(const Vector &v) { m_origin = v; }
...
void StudioModel::GetOrigin(Vector &v) { v = m_origin; }
...
tris.m_MaterialBatches[batchID];
                ...
                {
                        GetTriangles_Vertex_t &vert =
materialBatch.m_Verts[vertID]; const Vector &pos = vert.m_Position;

                        Vector newPos(0,0,0);

                        for ( int k = 0; k < vert.m_NumBones; k++ )
                        {
                                const matrix3x4_t &poseToWorld =
tris.m_PoseToWorld[ vert.m_BoneIndex[k] ]; Vector tmp; VectorTransform( pos,
poseToWorld, tmp ); newPos += vert.m_BoneWeight[k] * tmp;
                        }
...
//			pRender->TransformPoint3D( points[vertID], newPos );
                ...
                for ( int strip = 0; strip < numStrips; strip++ )
                {
                        ...
                        {
                                POINT pt = points[
materialBatch.m_TriListIndices[stripIndex++] ];
                        ...
                        // for performance sake bypass the renderer interface,
buuuhhh

                        if ( numPoints == 2 )
                        {
                                g_pMatSystemSurface->DrawLine( ptx[0], pty[0],
ptx[1], pty[1] );
                        }
                        else if ( numPoints == 3 )
                        {
...
void InitStudioFileChangeWatcher() { g_StudioFileChangeWatcher.Init(); }

void UpdateStudioFileChangeWatcher() { g_StudioFileChangeWatcher.Update(); }
