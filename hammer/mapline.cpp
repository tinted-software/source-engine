...
//
//			<red> <green> <blue> <start key> <start key value> <end
//key> <end key value>
//
//			The line helper looks in the given keys in its parent
//entity and 			attaches itself to the entities with those key values. If only one
//			endpoint entity is specified, the other end is assumed
//to be the parent 			entity.
//
//=============================================================================//
...
CMapClass *CMapLine::Create(CHelperInfo *pHelperInfo, CMapEntity *pParent) {
  ...
  pLine =
      new CMapLine(pszStartKey, pszStartValueKey, pszEndKey, pszEndValueKey);
...
//			pszStartValueKey - The key in our parent entity from
//which to get a search term for the start entity ex 'beamstart01'. 			pszEndKey -
//The key to search in other entities for a match against the value of
...
//-----------------------------------------------------------------------------
CMapLine::CMapLine(const char *pszStartKey, const char *pszStartValueKey,
                   const char *pszEndKey, const char *pszEndValueKey) {
  Initialize();
...
//-----------------------------------------------------------------------------
void CMapLine::OnNotifyDependent(CMapClass *pObject,
