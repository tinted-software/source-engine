...
// Purpose:
//
//			TODO: add an autoregistration system for tools a la
...
//-----------------------------------------------------------------------------
CToolManager::~CToolManager() { Shutdown(); }
...
// Purpose:
//-----------------------------------------------------------------------------
CBaseTool *CToolManager::GetActiveTool() { return m_pActiveTool; }
...
//			tool will be deactivated and reactivated when PopTool is
//called.
//-----------------------------------------------------------------------------
...
//			If the stack is underflowed somehow, the pointer is
//restored.
//-----------------------------------------------------------------------------
...
void CToolManager::SetTool(ToolID_t eNewTool) {
  ...
  // FIXME: When we start up, we end up here before the main window is created
  // because
  //		  CFaceEditDispPage::OnSetActive() calls
  //SetTool(TOOL_FACEEDIT_DISP). This 		  behavior is rather nonsensical during
  //startup.
...
ChunkFileResult_t CToolManager::SaveVMF(CChunkFile *pFile,
                                        CSaveInfo *pSaveInfo) {
...
ChunkFileResult_t CToolManager::LoadCallback(CChunkFile *pFile,
                                             CBaseTool *pTool) {
...
void CToolManager::AddToolHandlers(CChunkHandlerMap *pHandlersMap) {
  for (int i = 0; i < m_Tools.Count(); i++) {
    if (m_Tools[i]->GetVMFChunkName() != NULL) {
      pHandlersMap->AddHandler(m_Tools[i]->GetVMFChunkName(),
                               (ChunkHandler_t)LoadCallback, m_Tools[i]);
    }
  }
}
