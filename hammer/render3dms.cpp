...
//-----------------------------------------------------------------------------
// Purpose: Callback comparison function for sorting objects clicked on while
//			in selection mode. The reverse sort is used for cards
//that return 			depth values in reverse (larger numbers are closer to the camera).
// Input  : pHit1 - First hit to compare.
//			pHit2 - Second hit to compare.
...
void CRender3D::BeginRenderHitTarget(CMapAtom *pObject, unsigned int uHandle) {
  ...
  if ((m_Pick.m_nFlags & FLAG_OBJECTS_AT_RESOLVE_INSTANCES) == 0 &&
      m_bInstanceRendering && !GetInstanceClass()->IsEditable()) {
...
void CRender3D::EndRenderFrame(void) {
  ...
  if (!m_Pick.bPicking) {
    ...
    if ((m_eCurrentRenderMode == RENDER_MODE_LIGHT_PREVIEW_RAYTRACED) &&
      ...
      mybmh.biSizeImage =
          g_pLPreviewOutputBitmap->Width() * g_pLPreviewOutputBitmap->Height();
...
void CRender3D::PushInstanceData(CMapInstance *pInstanceClass,
...
//-----------------------------------------------------------------------------
// Purpose: this will handle all translucent rendering, including local
// transforms for instance items Input  : none Output : none
//-----------------------------------------------------------------------------
  ...
  while (m_TranslucentRenderObjects.Count() > 0) {
    ...
    if (current.m_InstanceState.m_pInstanceClass) {
      if (pInstanceClass != current.m_InstanceState.m_pInstanceClass ||
          !m_bInstanceRendering ||
          current.m_InstanceState.m_InstanceMatrix !=
              m_CurrentInstanceState.m_InstanceMatrix) {
        if (bAddedTransform) {
          EndLocalTransfrom();
        }
        bAddedTransform = true;
        BeginLocalTransfrom(current.m_InstanceState.m_InstanceRenderMatrix,
                            false);
        ...
        if (pInstanceClass->IsEditable()) {
          ...
          SetInstanceRendering(current.m_bInstanceSelected
...
void CRender3D::RenderBox(const Vector &Mins, const Vector &Maxs,
  ...
  for (int nPass = 1; nPass <= nPasses; nPass++) {
    ...
    for (int nFace = 0; nFace < 6; nFace++) {
      ...
      CMatRenderContextPtr pRenderContext(MaterialSystemInterface());
      IMesh *pMesh = pRenderContext->GetDynamicMesh();
      meshBuilder3D.DrawQuad(pMesh, FacePoints[nP1].Base(),
...
void CRender3D::RenderPointsAndPortals(void) {
  ...
  if (pDoc->m_PFPoints.Count()) {
    ...
    for (int i = 0; i < nPFPoints; i++) {
      meshBuilder3D.Position3f(pPFPoints[i][0], pPFPoints[i][1],
                               pPFPoints[i][2]);
      meshBuilder3D.Color3ub(255, 0, 0);
      meshBuilder3D.AdvanceVertex();
    }
...
void CRender3D::RenderMapClass(CMapClass *pMapClass) {
  ...
  if ((pMapClass != NULL) && (pMapClass->GetRenderFrame() != m_nFrameCount)) {
    if (pMapClass->IsVisible()) {
      ...
      if (should_appear == true && m_Pick.bPicking == true &&
          (m_Pick.m_nFlags & FLAG_OBJECTS_AT_ONLY_SOLIDS) != 0) {
...
//-----------------------------------------------------------------------------
// Purpose: this function will render an instance map at the specific offset and
// rotation Input  : pInstanceClass - the map class of the func_instance
//			pMapClass - the map class of the world spawn of the
//instance 			InstanceOrigin - the translation offset 			InstanceAngles - the axis
//rotation
// Output : none
//-----------------------------------------------------------------------------
void CRender3D::RenderInstanceMapClass(CMapInstance *pInstanceClass,
                                       CMapClass *pMapClass,
                                       Vector &InstanceOrigin,
                                       QAngle &InstanceAngles) {
  ...
  if (m_TranslucentSortRendering ==
      false) { // translucent objects will do their own transforms
    RenderTranslucentObjects();
  }
...
//-----------------------------------------------------------------------------
// Purpose: this function will recursively render an instance and all of its
// children Input  : pObject - the object to be rendered Output : none
//-----------------------------------------------------------------------------
void CRender3D::RenderInstanceMapClass_r(CMapClass *pMapClass) {
  Assert(pMapClass != NULL);

  if ((pMapClass != NULL) &&
      (pMapClass->GetRenderFrame() != m_nInstanceCount)) {
    if (pMapClass->IsVisible()) {
      //
      // Render this object's culling box if it is enabled.
      //
      if (g_bRenderCullBoxes) {
        ...
        TransformInstanceAABB(vecMins, vecMaxs, vecExpandedMins,
                              vecExpandedMaxs);
...
//			bForce - If true, don't check for visibility, just
//render the node 				and all of its children.
//-----------------------------------------------------------------------------
