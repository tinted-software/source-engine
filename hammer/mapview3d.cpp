...
//			for 3D camera control, activating tools in the 3D view,
//calling 			into the renderer when necessary, and synchronizing the 2D camera
...
BOOL CMapView3D::PreCreateWindow(CREATESTRUCT &cs) {
  ...
  if (className.IsEmpty()) {
    //
    // We need the CS_OWNDC bit so that we don't need to call GetDC every time
    // we render. That fixes the flicker under Win98.
    //
...
//			cursor is shown and not locked in the center of the 3D
//view.
...
void CMapView3D::SetDrawType(DrawType_t eDrawType) {
  ...
  if (m_eDrawType == VIEW3D_SMOOTHING_GROUP) {
    ...
    if (pMainFrame) {
      CFaceSmoothingVisualDlg *pSmoothDlg =
          pMainFrame->GetSmoothingGroupDialog();
...
void CMapView3D::Dump(CDumpContext &dc) const { CView::Dump(dc); }
#endif //_DEBUG
...
void CMapView3D::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) {
  ...
  // 'z' toggles mouselook.
  //
  if (((char)tolower(nChar) == 'z') && !(nFlags & 0x4000) &&
      (Options.view3d.bUseMouseLook)) {
    if (pDoc != NULL) {
  ...
  // Got to check for m_pToolManager here because otherwise it can crash on
  // startup if they have keys pressed.
  if (m_pToolManager) {
    //
    // Pass the message to the active tool.
    //
...
void CMapView3D::GetBestTransformPlane(Vector &horzAxis, Vector &vertAxis,
                                       Vector &thirdAxis) {
  Vector vAxis;
...
void CMapView3D::OnInitialUpdate(void) {
  ...
  if (Options.view3d.nTimeToMaxSpeed != 0) {
    m_fForwardAcceleration =
        m_fForwardSpeedMax / (Options.view3d.nTimeToMaxSpeed / 1000.0f);
    m_fStrafeAcceleration =
        m_fStrafeSpeedMax / (Options.view3d.nTimeToMaxSpeed / 1000.0f);
    m_fVerticalAcceleration =
        m_fVerticalSpeedMax / (Options.view3d.nTimeToMaxSpeed / 1000.0f);
  } else {
  ...
  if (Options.general.bRadiusCulling) {
    // Hack!  Don't use frustum culling when doing radial distance culling (slam
    // the distance to 10K)
    m_pCamera->SetPerspective(Options.view3d.fFOV, CAMERA_FRONT_PLANE_DISTANCE,
                              10000);
  } else {
    m_pCamera->SetPerspective(Options.view3d.fFOV, CAMERA_FRONT_PLANE_DISTANCE,
                              Options.view3d.iBackPlane);
  }
  ...
  //
  m_pRender->RenderEnable(RENDER_REVERSE_SELECTION,
                          (Options.view3d.bReverseSelection == TRUE));
  ...
  //
  m_pRender->RenderEnable(RENDER_FILTER_TEXTURES,
                          (Options.view3d.bFilterTextures == TRUE));

  // Get the initial viewpoint and view direction from the default camera in the
  // document.
...
//-----------------------------------------------------------------------------
// Purpose: Determines the object at the point (point.x, point.y) in the 3D
// view. Input  : point - Point to use for hit test.
//			ulFace - Index of face in object that was hit.
// Output : Returns a pointer to the CMapClass object at the coordinates, NULL
// if none.
//-----------------------------------------------------------------------------
CMapClass *CMapView3D::NearestObjectAt(const Vector2D &vPoint, ULONG &ulFace,
  ...
  if (m_pRender->ObjectsAt(vPoint.x, vPoint.y, 1, 1, &Hits, 1, nFlags) != 0) {
    //
    // If they clicked on a solid, the index of the face they clicked on is
    // stored in array index [1].
    //
...
//-----------------------------------------------------------------------------
// Purpose: Casts a ray from the viewpoint through the given plane and
// determines
...
//			pos - Returns the point on the plane that projects to
//the given point.
//-----------------------------------------------------------------------------
...
bool CMapView3D::HitTest(const Vector2D &vPoint, const Vector &mins,
                         const Vector &maxs) {
  Vector vStart, vEnd;
  int nFace;
...
//-----------------------------------------------------------------------------
int CMapView3D::ObjectsAt(const Vector2D &vPoint, HitInfo_t *pObjects,
                          int nMaxObjects, unsigned int nFlags) {
  if (m_pRender != NULL) {
...
//-----------------------------------------------------------------------------
// Purpose: Applies an acceleration to a velocity, allowing instantaneous
// direction
//			change and zeroing the velocity in the absence of
//acceleration.
// Input  : fVelocity - Current velocity.
//			fAccel - Amount of acceleration to apply.
//			fTimeScale - The time for which the acceleration should
//be applied. 			fMaxVelocity - The maximum velocity to allow.
// Output : Returns the new velocity.
//-----------------------------------------------------------------------------
static float Accelerate(float fVelocity, float fAccel, float fAccelScale,
                        float fTimeScale, float fVelocityMax) {
  //
  // If we have a finite acceleration in this direction, apply it to the
  // velocity.
  //
...
void CMapView3D::ProcessMovementKeys(float fElapsedTime) {
  ...
  //
  // Apply the accelerations to the forward, strafe, and vertical speeds. They
  // are actually velocities because they are signed values.
  //
  m_fForwardSpeed = Accelerate(m_fForwardSpeed, m_fForwardAcceleration,
                               fMoveForward, fElapsedTime, m_fForwardSpeedMax);
  m_fStrafeSpeed = Accelerate(m_fStrafeSpeed, m_fStrafeAcceleration, fMoveRight,
                              fElapsedTime, m_fStrafeSpeedMax);
  m_fVerticalSpeed = Accelerate(m_fVerticalSpeed, m_fVerticalAcceleration,
                                fMoveUp, fElapsedTime, m_fVerticalSpeedMax);
...
//			function samples the cursor position and takes the
//appropriate 			action based on the current mode (camera, morphing).
//-----------------------------------------------------------------------------
...
// Input  : bEnable - true to lock and hide the cursor, false to unlock and show
// it.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Enables or disables mouselook. When mouselooking, the cursor is
// hidden
...
//-----------------------------------------------------------------------------
// Purpose: Enables or disables camera rotating. When rotating, the cursor is
// hidden
//			and a crosshair is rendered in the center of the view.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Enables or disables camera strafing. When strafing, the cursor is
// hidden
//			and a crosshair is rendered in the center of the view.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Moves the camera forward by flDistance units. Negative units move
// back.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Moves the camera right by flDistance units. Negative units move
// left.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Pitches the camera forward by flDegrees degrees. Negative units
// pitch back.
//-----------------------------------------------------------------------------
