...
CMapPathNode &CMapPathNode::operator=(const CMapPathNode &src) {
  ...
  kv.RemoveAll();
  for (int i = src.kv.GetFirst(); i != src.kv.GetInvalidIndex();
       i = src.kv.GetNext(i)) {
...
void CMapPath::SerializeRMF(std::fstream &file, BOOL fIsStoring) {
  ...
  if (fIsStoring) {
    ...
    for (int i = 0; i < m_Nodes.Count(); i++) {
      ...
      iSize = 0;
      for (int z = kv.GetFirst(); z != kv.GetInvalidIndex();
           z = kv.GetNext(z)) {
        ++iSize;
      }
      file.write((char *)&iSize, sizeof(iSize));
      ...
      // Write keyvalues.
      //
      for (int k = kv.GetFirst(); k != kv.GetInvalidIndex();
           k = kv.GetNext(k)) {
...
//			*pIntersecting -
//-----------------------------------------------------------------------------
void CMapPath::SerializeMAP(std::fstream &file, BOOL fIsStoring,
...
SaveAgain:
  while (1) {
    ...
    if (bStoreTarget) {
      kvTemp.Set("target",
                 (iDirec == 1) ? node.szTargets[0] : node.szTargets[1]);
      kvTemp.SerializeMAP(file, TRUE);
    }
...
CMapEntity *CMapPath::CreateEntityForNode(DWORD dwNodeID) {
  ...
  for (int k = pNode->kv.GetFirst(); k != pNode->kv.GetInvalidIndex();
       k = pNode->kv.GetNext(k)) {
...
void CMapPath::CopyNodeFromEntity(DWORD dwNodeID, CMapEntity *pEntity) {
  ...
  //
  // Copy all the keys except target and targetname from the entity to the
  // pathnode.
  //
  for (int i = pEntity->GetFirstKeyValue(); i != pEntity->GetInvalidKeyValue();
       i = pEntity->GetNextKeyValue(i)) {
    if (!strcmp(pEntity->GetKey(i), "target") ||
        !strcmp(pEntity->GetKey(i), "targetname")) {
      continue;
    }
...
UNDONE: Nobody uses the path tool because the user interface is so poor.
                Path support has been pulled until the tool itself can be fixed
or replaced.

CChunkFileResult_t CMapPathNode::LoadKeyCallback(const char *szKey, const char
*szValue, CMapPathNode *pNode)
{
...
ChunkFileResult_t CMapPathNode::SaveVMF(CChunkFile *pFile, CSaveInfo *pSaveInfo)
{
        ...
        if (eResult == ChunkFile_Ok)
        {
                ...
                for (int k = 0; k < iSize; k++)
                {
                        ...
                        {
                                eResult =
pFile->WriteKeyValue(KeyValue.GetKey(), KeyValue.GetValue());
                        }
                }
        }
...
// Output : CChunkFileResult_t
//-----------------------------------------------------------------------------
CChunkFileResult_t CMapPath::LoadKeyCallback(const char *szKey, const char
*szValue, CMapPath *pPath)
{
