...
bool CIncrementalLightInfo::IsLowerPriorityThan(
  ...
  case INCR_STATE_PARTIAL_RESULTS + 16 * INCR_STATE_PARTIAL_RESULTS: {
    // if incrmental vs incremental, and no light from either, do most recently
    // lit one
  ...
  for (int y = 0; y < src.m_nHeight; y++)
    for (int x = 0; x < src.m_nWidth; x++) {
      ...
      *(ret_bm->GetPixel(x, y) + 0) =
          (uint8)min(255, (int)(255.0 * pow(color.z, (float)(1 / 2.2))));
      *(ret_bm->GetPixel(x, y) + 1) =
          (uint8)min(255, (int)(255.0 * pow(color.y, (float)(1 / 2.2))));
      *(ret_bm->GetPixel(x, y) + 2) =
          (uint8)min(255, (int)(255.0 * pow(color.x, (float)(1 / 2.2))));
...
void HandleLightingPreview(void) {
  if (GetMainWnd()->m_pLightingPreviewOutputWindow &&
      !GetMainWnd()->m_bLightingPreviewOutputWindowShowing) {
  ...
  while (g_LPreviewToHammerMsgQueue.MessageWaiting()) {
    ...
    case LPREVIEW_MSG_DISPLAY_RESULT: {
      ...
      //g_nBitmapGenerationCounter )
      {
        ...
        if (g_pLPreviewOutputBitmap &&
            (g_pLPreviewOutputBitmap->Width() > 10)) {
