...
class CMapView2DBase : public CView,
                       public CMapView,
                       public Axes2,
                       public CVGuiWnd {
  // Operations
public:
  ...
  int ObjectsAt(CMapWorld *pWorld, const Vector2D &vPoint, HitInfo_t *pObjects,
                int nMaxObjects, unsigned int nFlags = 0);
  ...
  bool CheckDistance(const Vector2D &vecCheck, const Vector2D &vecRef,
                     int nDist);
...
protected:
  ...
  void DrawGrid(CRender2D *pRender, int xAxis, int yAxis, float depth,
...
protected:
  ...
  POINT m_ptLDownClient; // client pos at which lbutton was pressed, for
                         // dragging the view
  ...
  float m_flMinZoom; // Minimum legal zoom factor (should be sufficient to
                     // display entire map in the view)
...
inline bool CMapView2DBase::PointInClientRect(const Vector2D &point) {
  return (point.x >= 0 && point.y >= 0 && point.x < m_ClientWidth &&
          point.y < m_ClientHeight);
}
