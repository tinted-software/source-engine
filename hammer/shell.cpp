...
//problems with 			editing out of sync versions of the map via the engine.
...
bool CShell::DoVersionCheck(const char *pszArguments) {
  if (m_pDoc != NULL) {
    ...
    if (sscanf(pszArguments, "%s %d", szEngineMapPath, &nEngineMapVersion) ==
        2) {
      ...
      if (!stricmp(szDocName, szEngineMapName) &&
...
//assign it, and 			x, y, z coordinate at which to create the node.
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CShell::NodeCreate(const char *pszCommand, const char *pszArguments) {
  if ((m_pDoc != NULL) && m_pDoc->IsShellSessionActive()) {
    ...
    if (sscanf(pszArguments, "%s %d %f %f %f", szClassName, &nID, &x, &y, &z) ==
        5) {
      ...
      if (m_pDoc->FindEntitiesByClassName(Found, "info_node_link", false)) {
        FOR_EACH_OBJ(Found, pos) {
          ...
          if ((pszNode1 != NULL) && (pszNode2 != NULL)) {
            if (((!stricmp(pszNode1, szIDStart)) &&
                 (!stricmp(pszNode2, szIDEnd))) ||
                ((!stricmp(pszNode1, szIDEnd)) &&
                 (!stricmp(pszNode2, szIDStart)))) {
              m_pDoc->DeleteObject(pEntity);
              bFound = true;
            }
          }
