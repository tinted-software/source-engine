...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CToolOverlay::OnLMouseUp3D(CMapView3D *pView, UINT nFlags,
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CToolOverlay::OnLMouseDown3D(CMapView3D *pView, UINT nFlags,
                                  const Vector2D &vPoint) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CToolOverlay::OnMouseMove3D(CMapView3D *pView, UINT nFlags,
                                 const Vector2D &vPoint) {
  if (m_bDragging) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CToolOverlay::CreateOverlay(CMapSolid *pSolid, ULONG iFace,
                                 CMapView3D *pView, Vector2D point) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CToolOverlay::OverlaySelection(CMapView3D *pView, UINT nFlags,
                                    const Vector2D &vPoint) {
    ...
    if (pHitObject) {
      if (pHitObject->IsMapClass(MAPCLASS_TYPE(CMapEntity))) {
        ...
        FOR_EACH_OBJ(*pChildren, pos2) {
          CMapOverlay *pOverlay =
              dynamic_cast<CMapOverlay *>(pChildren->Element(pos2));
          if (pOverlay) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CToolOverlay::OnContextMenu2D(CMapView2D *pView, UINT nFlags,
                                   const Vector2D &vPoint) {
  ...
  if (!IsEmpty()) {
    if (HitTest(pView, vPoint, false)) {
      ...
      menuOverlay.TrackPopupMenu(TPM_LEFTBUTTON | TPM_RIGHTBUTTON |
                                     TPM_LEFTALIGN,
                                 ptScreen.x, ptScreen.y, pView);
    }
  }
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CToolOverlay::HandleInBBox(CMapOverlay *pOverlay,
                                Vector const &vecHandlePt) {
  ...
  for (int iAxis = 0; iAxis < 3; iAxis++) {
    ...
    if ((vecHandlePt[iAxis] < vecMin[iAxis]) ||
        (vecHandlePt[iAxis] > vecMax[iAxis]))
      return false;
  }
  ...
  while (pChild) {
    ...
    if (pEntity) {
      ...
      FOR_EACH_OBJ(*pChildren, pos) {
        CMapOverlay *pOverlay =
            dynamic_cast<CMapOverlay *>(pChildren->Element(pos));
        if (pOverlay && pOverlay != m_pActiveOverlay &&
            pOverlay->IsSelected()) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CToolOverlay::OnDrag(Vector const &vecRayStart, Vector const &vecRayEnd,
                          bool bShift) {
  ...
  for (iFace = 0; iFace < nFaceCount; iFace++) {
    ...
    if (pFace) {
      if (pFace->TraceLineInside(vecImpact, vecImpactNormal, vecRayStart,
                                 vecRayEnd))
        break;
    }
  }
