...
#ifdef _DEBUG
void CPakViewFiles::AssertValid() const { CListView::AssertValid(); }

void CPakViewFiles::Dump(CDumpContext &dc) const { CListView::Dump(dc); }
#endif //_DEBUG
...
void CPakViewFiles::OnInitialUpdate() {
  ...
  SetWindowLong(lc.m_hWnd, GWL_STYLE,
                (dwStyle & ~LVS_TYPEMASK) | LVS_ALIGNLEFT | LVS_AUTOARRANGE |
                    LVS_REPORT | // LVS_ICON |
                                 // LVS_NOITEMDATA |
                    LVS_SORTASCENDING);
