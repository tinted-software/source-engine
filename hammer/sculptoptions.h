...
public:
  ...
  virtual bool Paint(CMapView3D *pView, const Vector2D &vPoint,
...
protected:
  ...
  virtual void DoPaintOperation(CMapView3D *pView, const Vector2D &vPoint,
                                CMapDisp *pDisp, CMapDisp *pOrigDisp) = 0;

  bool GetStartingSpot(CMapView3D *pView, const Vector2D &vPoint);

  bool IsPointInScreenCircle(CMapView3D *pView, CMapDisp *pDisp,
                             CMapDisp *pOrigDisp, int nVertIndex,
                             bool bUseOrigDisplacement = true,
                             bool bUseCurrentPosition = false,
                             float *pflLengthPercent = NULL);

  void DoPaintSmooth(CMapView3D *pView, const Vector2D &vPoint, CMapDisp *pDisp,
                     CMapDisp *pOrigDisp);
  bool PaintSphereDispBBoxOverlap(const Vector &vCenter, float flRadius,
                                  const Vector &vBBoxMin,
                                  const Vector &vBBoxMax);
  bool IsInSphereRadius(const Vector &vCenter, float flRadius2,
  ...
  void DrawDirection(CRender3D *pRender, Vector vDirection, Color Towards,
                     Color Away);
  void DuplicateSelectedDisp();
  void PrepareDispForPainting();
  bool FindCollisionIntercept(CCamera *pCamera, const Vector2D &vPoint,
                              bool bUseOrigPosition, Vector &vCollisionPoint,
                              Vector &vCollisionNormal,
                              float &vCollisionIntercept);

private:
  void DetermineKeysDown();

protected:
  static bool MapDispLessFunc(EditDispHandle_t const &a,
                              EditDispHandle_t const &b) {
    return a < b;
  }

  static CUtlMap<EditDispHandle_t, CMapDisp *> m_OrigMapDisp;

  Vector m_StartingCollisionPoint, m_StartingCollisionNormal,
      m_OriginalCollisionPoint, m_OriginalCollisionNormal,
      m_CurrentCollisionPoint, m_CurrentCollisionNormal;
  float m_StartingCollisionIntercept, m_OriginalCollisionIntercept,
      m_CurrentCollisionIntercept;
  float m_StartingProjectedRadius, m_OriginalProjectedRadius,
...
public:
  ...
  typedef enum { OFFSET_MODE_ADAPTIVE, OFFSET_MODE_ABSOLUTE } OffsetMode;
...
private:
  ...
  void GetPaintAxis(CCamera *pCamera, const Vector2D &vPoint,
                    Vector &vPaintAxis);
...
protected:
  ...
  void DoSmoothOperation(CMapView3D *pView, const Vector2D &vPoint,
                         CMapDisp *pDisp, CMapDisp *pOrigDisp);
...
protected:
  bool IsPointAffected(CMapView3D *pView, CMapDisp *pDisp, CMapDisp *pOrigDisp,
                       int nVertIndex, int nBrushPoint, Vector2D &vViewVert,
                       bool bUseOrigDisplacement = true,
                       bool bUseCurrentPosition = false);
  virtual void DoPaintOperation(CMapView3D *pView, const Vector2D &vPoint,
                                CMapDisp *pDisp, CMapDisp *pOrigDisp);
