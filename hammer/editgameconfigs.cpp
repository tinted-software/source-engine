...
CEditGameConfigs::CEditGameConfigs(BOOL bSelectOnly, CWnd *pParent /*=NULL*/)
...
void CEditGameConfigs::OnRemove() {
  ...
  int iArrayIndex;
  CGameConfig *pConfig =
      Options.configs.FindConfig(m_cConfigs.GetItemData(iCurSel), &iArrayIndex);
  ...
  // Check to see if this is the last configuation and prompt for the user to
  // make a decision
  ...
  // FIXME: This will apply the change even if you cancel the dialog.  This
  // needs to store a copy
  //		  of the data which then reconciles the two versions on OK or
  //Apply! -- jdw
