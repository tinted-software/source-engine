...
CTorusDlg::CTorusDlg(Vector &boxmins, Vector &boxmaxs, CWnd *pParent /*=NULL*/)
  ...
  m_iRotationSides =
      AfxGetApp()->GetProfileInt(pszSection, "Rotation Sides", 16);
...
float CTorusDlg::MaxTorusCrossSectionRadius() const {
  ...
  if ((m_fRotationArc > 360) ||
      ((m_fRotationArc == 360) && (m_iAddHeight != 0))) {
...
void CTorusDlg::DrawTorusCrossSection(CDC *pDC) {
  ...
  MakeArcCenterRadius(0, 0, flTorusRadius + iWallWidth,
                      flTorusRadius + iWallWidth, iSides, fStartAngle, fArc,
                      fOuterPoints);

  MakeArcCenterRadius(0, 0, flTorusRadius, flTorusRadius, iSides, fStartAngle,
                      fArc, fInnerPoints);
...
void CTorusDlg::DrawTorusTopView(CDC *pDC) {
  ...
  MakeArcCenterRadius(xCenter, yCenter, xRad, yRad, iSides, fStartAngle, fArc,
                      fInnerPoints);
