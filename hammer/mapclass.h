...
//stored, but 			they should be set via UpdateDependency rather than directly. This
//			insures proper linkage to the other object so that if it
//moves, is 			removed from the world, or changes in any other way, the dependent
...
protected:
  ...
  BoundBox m_CullBox; // Our bounds for culling in the 3D views and intersecting
                      // with the cordon.
  BoundBox m_BoundingBox; // Our bounds for brushes / entities themselves.  This
                          // size may be smaller than m_CullBox ( i.e. spheres
                          // are not included )
  ...
  bool m_bVisible2D; // Whether this object is visible in the 2D view. Currently
                     // only used for morphing.
  ...
  CVisGroup *m_pColorVisGroup; // The visgroup from which we get our color, NULL
                               // if none.
  ...
  friend class CTrackEntry; // Friends with Undo/Redo system so that parentage
                            // can be changed.
  friend void
  FixHiddenObject(MapError *pError); // So that the Check for Problems dialog
                                     // can fix visgroups problems.
};
