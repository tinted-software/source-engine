...
public:
  ...
  public:
    ...
    virtual void GetItemColor(int iItem, COLORREF *pBackgroundColor,
                              COLORREF *pTextColor) = 0;

    // This is called for every item so you can draw custom stuff in its value
    // column. The RECT inside the DRAWITEMSTRUCT contains the whole row, and
    // pRect contains the rect for the value column only. Return true if you
    // don't want CColoredListControl to draw its value.
    virtual bool CustomDrawItemValue(const LPDRAWITEMSTRUCT p,
                                     const RECT *pRect) = 0;
  };
...
//-----------------------------------------------------------------------------
// Purpose: A little glue object that connects the angles picker tool to our
// dialog.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: A little glue object that connects the entity picker tool to our
// dialog.
//			Currently this gets the value of a given key and puts
//that into the smart 			control.
//-----------------------------------------------------------------------------
...
private:
  char m_szKey[MAX_KEYVALUE_LEN]; // The name of the key we are going to slurp
                                  // out of the entity.
...
//-----------------------------------------------------------------------------
// Purpose: A little glue object that connects the face picker tool to our
// dialog.
//-----------------------------------------------------------------------------
...
enum EKeyState {
  k_EKeyState_DefaultFGDValue =
      0, // This key is unmodified from its default value in the FGD.
  k_EKeyState_Modified =
      1, // This key is in the FGD, and its value has been modified.
  k_EKeyState_AddedManually, // This key was added manually (i.e. it does not
                             // exist in the FGD).
...
class COP_Entity : public CObjectPage,
                   ...
                   public CColoredListCtrl::IItemColorCallback {
  DECLARE_DYNCREATE(COP_Entity)
  typedef CObjectPage BaseClass;

  friend int InternalSortByColumn(COP_Entity *pDlg, const char *pShortName1,
                                  const char *pShortName2, int iColumn);
  friend int CALLBACK SortByItemEditedState(LPARAM iItem1, LPARAM iItem2,
                                            LPARAM lpParam);
...
protected:
  ...
  void GetKeyState(const char *pShortName, EKeyState *pState,
                   bool *pMissingTarget);
  void ResortItems();
  void LoadClassList();
  ...
  void CreateSmartControls_Angle(GDinputvariable *pVar, CRect &ctrlrect,
                                 HFONT hControlFont, bool *bShowSmartAngles);
  void CreateSmartControls_Choices(GDinputvariable *pVar, CRect &ctrlrect,
                                   HFONT hControlFont);
  void CreateSmartControls_TargetName(GDinputvariable *pVar, CRect &ctrlrect,
                                      HFONT hControlFont);
  void
  CreateSmartControls_BasicEditControl(GDinputvariable *pVar, CRect &ctrlrect,
                                       HFONT hControlFont,
                                       CUtlVector<const char *> *pHelperType);
  void CreateSmartControls_BrowseAndPlayButtons(GDinputvariable *pVar,
                                                CRect &ctrlrect,
                                                HFONT hControlFont);
  void CreateSmartControls_MarkAndEyedropperButtons(GDinputvariable *pVar,
                                                    CRect &ctrlrect,
                                                    HFONT hControlFont);
  void CreateSmartControls_PickButton(GDinputvariable *pVar, CRect &ctrlrect,
                                      HFONT hControlFont);
  void CreateSmartControls_InstanceVariable(GDinputvariable *pVar,
                                            CRect &ctrlrect,
                                            HFONT hControlFont);
  void CreateSmartControls_InstanceParm(GDinputvariable *pVar, CRect &ctrlrect,
                                        HFONT hControlFont);
  ...
  void GetFaceIDListsForKey(CMapFaceIDList &FullFaces,
                            CMapFaceIDList &PartialFaces, const char *pszKey);
  void GetFaceListsForKey(CMapFaceList &FullFaces, CMapFaceList &PartialFaces,
                          const char *pszKey);
  void ApplyKeyValueToObject(CEditGameClass *pObject, const char *pszKey,
                             const char *pszValue);
...
private:
  ...
  int GetKeyValueRowByShortName(
      const char *pShortName); // Find the row in the listctrl that the var is
                               // at. Returns -1 if not found.
...
private:
  ...
  bool m_bEnableControlUpdate; // Whether to reflect changes to the edit control
                               // into other controls.
  ...
  GDclass *
      m_pDisplayClass; // The class that the dialog is showing. Can be different
                       // from m_pEditClass until the user hits Apply.
  ...
  WCKeyValues
      m_kvAdded; // Corresponding keys set to value "1" if they were added

  GDIV_TYPE m_eEditType; // The type of the currently selected key when
                         // SmartEdit is enabled.

  bool m_bIgnoreKVChange; // Set to ignore Windows notifications when setting up
                          // controls.
  bool m_bChangingKeyName;

  int m_iLastClassListSolidClasses; // Used to prevent reinitializing the class
                                    // list unnecessarily.

  bool m_bPicking;        // A picking tool is currently active.
  ToolID_t m_ToolPrePick; // The tool that was active before we activated the
                          // picking tool.
  ...
  // Used when multiselecting classes to remember whether they've selected a
  // class or not yet.
