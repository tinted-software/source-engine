...
bool CMapSolid::AddPlane(const CMapFace *p) {
  ...
  //
  // Copy the info from the carving face, including the plane, but not the
  // points.
  //
  ...
  //
  NewFace.InitializeTextureAxes(Options.GetTextureAlignment(),
                                INIT_TEXTURE_ALL | INIT_TEXTURE_FORCE);
...
// Input  : Inside - Receives the pieces of this solid that were inside the
// carver.
//			Outside - Receives the pieces of this solid that were
//outside the carver. 			pCarver - The solid that is being subtracted from us.
//-----------------------------------------------------------------------------
bool CMapSolid::Carve(CMapObjectList *pInside, CMapObjectList *pOutside,
                      CMapSolid *pCarver) {
  int i;
  ...
  for (i = 0; i < pCarver->GetFaceCount(); i++) {
    ...
    //
    // If there was no back piece, we have found a face the solid is completely
    // in front of. Per the separating axis theorem, the two solids cannot
    // intersect, so we are done.
    //
    if (back == NULL) {
      return (false);
    }
    ...
    //
    // The next clip uses the back piece from this clip to prevent the carve
...
//			f - Returns the part of the solid that was in front of
//the clipping 				face (in the direction of the face normal). 			b - Returns the part
//of the solid that was in back of the clipping 				face (in the opposite direction
//of the face normal).
//-----------------------------------------------------------------------------
void CMapSolid::ClipByFace(const CMapFace *fa, CMapSolid **fsolid,
                           CMapSolid **bSolid) {
...
//-----------------------------------------------------------------------------
// Purpose: Returns true if this solid contains a face with the given ID, false
// if not. Input  : nFaceID - unique face ID to look for.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
void CMapSolid::OnPreClone(CMapClass *pClone, CMapWorld *pWorld,
...
//to. 			OriginalList - The list of original objects that were copied. 			NewList -
//The list of copied.
//-----------------------------------------------------------------------------
void CMapSolid::OnPrePaste(CMapClass *pCopy, CMapWorld *pSourceWorld,
...
//          function returns whether or not an actual split happened
//          (TRUE/FALSE)
...
int CMapSolid::Split(PLANE *pPlane, CMapSolid **pFront, CMapSolid **pBack) {
  ...
  //
  // If we're all on one side of the splitting plane, copy ourselves into the
  // appropriate destination solid.
  //
  if ((frontCount == 0) || (backCount == 0)) {
    ...
    //
    // Rebuild the solid because mappers are accustomed to using the clipper
    // tool as a way of verifying that geometry is valid.
    //
    if (pReturnSolid->CreateFromPlanes(CREATE_FROM_PLANES_CLIPPING)) {
      // Initialize the texture axes only of the newly created faces. Leave the
      // others alone.
      pReturnSolid->InitializeTextureAxes(Options.GetTextureAlignment(),
                                          INIT_TEXTURE_ALL);
  ...
  if (pFront) {
    ...
    //
    // The new face doesn't have plane points, only a normal and a distance, so
    // we tell CreateFromPlanes to generate new plane points for us after
    // creation.
    //
    if (pFrontSolid->CreateFromPlanes(CREATE_BUILD_PLANE_POINTS |
                                      CREATE_FROM_PLANES_CLIPPING)) {
      // Initialize the texture axes only of the newly created faces. Leave the
      // others alone.
      pFrontSolid->InitializeTextureAxes(Options.GetTextureAlignment(),
                                         INIT_TEXTURE_ALL);
  ...
  if (pBack) {
    ...
    // creation.
    //
    if (pBackSolid->CreateFromPlanes(CREATE_BUILD_PLANE_POINTS |
                                     CREATE_FROM_PLANES_CLIPPING)) {
      // Initialize the texture axes only of the newly created faces. Leave the
      // others alone.
      pBackSolid->InitializeTextureAxes(Options.GetTextureAlignment(),
                                        INIT_TEXTURE_ALL);
...
//-----------------------------------------------------------------------------
// Purpose: Returns the index (you could use it with GetFace) or -1 if the face
// doesn't exist in this solid.
//-----------------------------------------------------------------------------
...
//				array, so it can be safely freed when AddFace
//returns.
//-----------------------------------------------------------------------------
...
void CMapSolid::CalcBounds(BOOL bFullUpdate) {
  ...
  for (int i = 0; i < nFaces; i++) {
    ...
    //
    // Get the 2d render bounds of this face and update the solid. 2D render
    // bounds can be different from 3D culling bounds because the 2D bounds do
    // not consider displacement faces.
    //
    Vector mins, maxs;
...
//-----------------------------------------------------------------------------
// Purpose: Sets the render color of all of our faces when our render color is
// set.
//-----------------------------------------------------------------------------
...
// set.
//-----------------------------------------------------------------------------
void CMapSolid::SetRenderColor(unsigned char uchRed, unsigned char uchGreen,
...
//-----------------------------------------------------------------------------
// Purpose: Creates the solid using the plane information from the solid's
// faces.
//
//			ASSUMPTIONS: This solid's faces are assumed to have
//valid plane points.
//
// Input  : dwFlags - Can be any or none of the following flags:
//
//				CREATE_BUILD_PLANE_POINTS - if this flag is set,
//the 3-point 					definition of each face plane will be regenerated based 					on the
//face points after the solid is generated.
//
...
int CMapSolid::CreateFromPlanes(DWORD dwFlags) {
  ...
  for (i = 0; i < nFaces; i++) {
    ...
    for (j = 0; j < i; j++) {
      ...
      //
      if ((DotProduct(f1, f2) > 0.999) &&
          (fabs(f->dist - pFaceCheck->plane.dist) < 0.01)) {
        useplane[j] = FALSE;
        break;
      }
    }
  ...
  //
  // Now we have a set of planes, indicated by TRUE values in the 'useplanes'
  // array, from which we will build a solid.
  //
  ...
  for (i = 0; i < nFaces; i++) {
    ...
    if (w != NULL) {
      ...
      //
      // The above rounding process may have created duplicate points. Eliminate
      // them.
      //
  ...
  if (!bGotFaces) {
    ...
    while (nFace > 0) {
      ...
      if ((!useplane[nFace]) || (pFace->GetPointCount() == 0)) {
        ...
        memcpy(useplane + nFace, useplane + nFace + 1,
               MAPSOLID_MAX_FACES - (nFace + 1));
      }
    }
  }
...
//-----------------------------------------------------------------------------
void CMapSolid::InitializeTextureAxes(TextureAlignment_t eAlignment,
                                      DWORD dwFlags) {
  int nFaces = Faces.GetCount();
  for (int i = 0; i < nFaces; i++) {
...
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapSolid::LoadSideCallback(CChunkFile *pFile,
                                              CMapSolid *pSolid) {
  ChunkFileResult_t eResult = ChunkFile_Ok;

  //
  // this is hear in place of the AddFace -- may want to handle this better
  // later!!!
  //
...
ChunkFileResult_t CMapSolid::LoadVMF(CChunkFile *pFile, bool &bValid) {
  ...
  ChunkFileResult_t eResult =
      pFile->ReadChunk((KeyHandler_t)LoadEditorKeyCallback, this);
  pFile->PopHandlers();
  ...
  if (eResult == ChunkFile_Ok) {
    ...
    if (CreateFromPlanes()) {
      ...
      //
      // create all of the displacement surfaces for faces with the displacement
      // property
      //
      int faceCount = GetFaceCount();
      for (int i = 0; i < faceCount; i++) {
...
//			in the VMF file. This should only occur after the first
//load of an 			old RMF file.
//-----------------------------------------------------------------------------
...
//Use PostloadWorld 				  to do similar bookkeeping after map load.
//
...
void CMapSolid::OnAddToWorld(CMapWorld *pWorld) {
  ...
  //
  // First, the common case: all our face IDs are zero. Assign new IDs to all
  // faces with zero IDs. Add unhandled faces to a list. Those we will need to
  // check against the world for uniqueness.
  //
  ...
  if (CheckList.Count() > 0) {
    //
    // The less common case: make sure all our face IDs are unique in this
    // world. We do it here instead of in CMapFace in order to save world tree
    // traversals.
    //
...
CMapClass *CMapSolid::PrepareSelection(SelectMode_t eSelectMode) {
  ...
  // select it instead.
  //
  if ((eSelectMode != selectSolids) && (m_pParent != NULL) &&
    ...
    if ((eSelectMode == selectGroups) ||
        (dynamic_cast<CMapEntity *>(m_pParent) != NULL)) {
...
//			currently selected, it will be rendered with a yellow
//wireframe 			in a second pass.
// Input  : pRender - Rendering interface.
//-----------------------------------------------------------------------------
void CMapSolid::Render3D(CRender3D *pRender) {
  //
  // determine whether or not this is a displacement solid - i.e. one of the
  // faces on this solid is displaced
  //
  ...
  //
  // Determine whether we need to render in one or two passes. If we are
  // selected, and rendering in flat or textured mode, we need to render using
  // two passes.
  //
  int nPasses = 1;
  int iStartPass = 1;
  ...
  if ((eSolidSelectionState != SELECT_NONE) &&
      (eDefaultRenderMode != RENDER_MODE_WIREFRAME)) {
    nPasses = 2;
  }
...
//			pSubParent - Receives the results of the subtraction as
//children.
// Output : Returns true if the objects intersected (subtraction was performed),
//			false if the objects did not intersect (no subtraction
//was performed).
//-----------------------------------------------------------------------------
bool CMapSolid::Subtract(CMapObjectList *pInside, CMapObjectList *pOutside,
...
void CMapSolid::Render2D(CRender2D *pRender) {
  ...
  //
  // Draw center handle if the solid is larger than the handle along either
  // axis.
  //
  if (bDrawHandles) {
  ...
  if (bDrawVertices) {
    ...
    pRender->SetHandleColor(GetRValue(Options.colors.clrVertex),
                            GetGValue(Options.colors.clrVertex),
...
// Output :
//-----------------------------------------------------------------------------
bool CMapSolid::HitTest2D(CMapView2D *pView, const Vector2D &point,
                          HitInfo_t &HitData) {
  if (!IsVisible())
    return false;
