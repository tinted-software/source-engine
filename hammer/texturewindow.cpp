...
void CTextureWindow::Create(CWnd *pParentWnd, RECT &rect) {
  ...
  if (TextureWndClassName.IsEmpty()) {
    // create class
    TextureWndClassName = AfxRegisterWndClass(
...
//-----------------------------------------------------------------------------
bool CTextureWindow::MatchKeywords(const char *pszSearch, char **pszKeyword,
                                   int nKeywords) {
  if (nKeywords != 0) {
...
BOOL CTextureWindow::EnumTexturePositions(TWENUMPOS *pTE, BOOL bStart) {
  ...
  do {
    pTE->pTex =
        g_Textures.EnumActiveTextures(&pTE->iTexIndex, m_eTextureFormat);
    ...
    // If we are iterating a specific list of textures, make sure it is in the
    // list. dvs: inefficient, the specific list should control the loop, not
    // act as a filter
...
doresize:
  ...
  SetRect(&texrect, pTE->cur_x, pTE->cur_y, pTE->cur_x + iDisplaySize,
          pTE->cur_y + iDisplaySize);

  // if we've got one texture on this row already, and this one goes out of
  // the client area, jump to the next row. we want to have at least one texture
  // on each row, or we will sit in an infinite loop.
...
//-----------------------------------------------------------------------------
// Purpose: Sets the name filter that is used to filter the texture window
// contents. Input  : pszFilter - Space, comma, or semicolon delimited names to
// filter against.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Sets the keywords that are used to filter the texture window
// contents. Input  : pszFilter - Space, comma, or semicolon delimited keywords
// to filter against.
//-----------------------------------------------------------------------------
...
void CTextureWindow::OnPaint(void) {
  ...
  while (bNotDone) {
    ...
    if (dc.RectVisible(&TE.texrect)) {
      ...
      CPalette *pOld =
          dc.SelectPalette(TE.pTex->HasPalette() ? TE.pTex->GetPalette()
                                                 : g_pGameConfig->Palette,
                           FALSE);
      dc.RealizePalette();
      ...
      TE.pTex->Draw(&dc, TE.texrect, iTexNameFontHeight, iTexIconHeight,
                    DrawTexData);
...
//			pScrollBar -
//-----------------------------------------------------------------------------
void CTextureWindow::OnHScroll(UINT nSBCode, UINT nPos,
...
//			pScrollBar -
//-----------------------------------------------------------------------------
void CTextureWindow::OnVScroll(UINT nSBCode, UINT nPos,
                               CScrollBar *pScrollBar) {
  SCROLLINFO si;
  ...
  while (bNotDone) {
    if (pTex == TE.pTex) {
      ...
      if (IsWindow(m_hWnd)) {
        ...
        if (iScrollPos + iClientHeight < TE.texrect.top ||
            TE.texrect.bottom < iScrollPos) {
...
//-----------------------------------------------------------------------------
// Purpose: Sets the contents of the texture window to a specific list of
// textures. Input  : pList - Textures with which to populate the texture
// window.
//-----------------------------------------------------------------------------
