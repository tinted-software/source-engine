...
// It can be made to recursively call the clipping code and only copy the vertex
// list once
...
static void R_DecalComputeBasis(Vector const &surfaceNormal,
                                Vector const *pSAxis, bool flipNormal,
...
//-----------------------------------------------------------------------------
int CreateClippedPoly(CMapFace *pFace, IEditorTexture *pDecalTex, Vector &org,
                      vec5_t *pOutPoints, int nOutSize) {
  float outvert[MAX_CLIPVERT][VERTEXSIZE];
  Assert(nOutSize <= MAX_CLIPVERT); // This code uses temp buffers of this size.

  /*#ifdef SDK_BUILD
          BUG: THIS IS THE HL1 VERSION! SWITCH BETWEEN THESE ALGORITHMS AT
  RUNTIME Vector vecOrg, vecSAxis, vecTAxis;
          ...
          for (int j = 0; j < pFace->nPoints; j++, pVertex++)
          {
                  ...
                  // Generate U & V
                  g_vert[j][3] = (curU - u) * scalex;		// Decal
  relative texture coordinates g_vert[j][4] = (curV - v) * scaley;
          }
  #else */
  ...
  g_outCount =
      SHClip(g_vert[0], pFace->nPoints, outvert[0], nMaxVerts, 0); // clip left
  g_outCount =
      SHClip(outvert[0], g_outCount, g_vert[0], nMaxVerts, 1); // clip right
  g_outCount =
      SHClip(g_vert[0], g_outCount, outvert[0], nMaxVerts, 2); // clip top
  g_outCount =
      SHClip(outvert[0], g_outCount, g_vert[0], nMaxVerts, 3); // clip bottom
