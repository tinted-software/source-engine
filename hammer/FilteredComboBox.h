...
#define SETSUGGESTIONS_CALLBACK                                                \
  0x0002 // Calls OnTextChanged for whatever it winds up selecting.

// CFilteredComboBox is a glorified EDIT control.
// The user can type stuff into the edit control, and it will provide
// autocomplete suggestions in its combo box. The user of this class provides
// those suggestions.
//
// To use this class:
//
...
//
// NOTE: Use CComboBox functions with caution! You could screw up the
// CFilteredComboBox's operation.
...
public:
  ...
  public:
    ...
    // This is sort of a backdoor for "only provide suggestions" mode. Normally,
    // it'll only call OnTextChanged with entries that are in the suggestions
    // list. But, it will call OnUnknownEntry if they type in something that's
    // not in your suggestion list first. If you return TRUE, it will add that
    // entry to the suggestions list. If you return FALSE (the default
    // behavior), it will find the closest match to what the user typed and use
    // that.
  ...
  void SetSuggestions(CUtlVector<CString> &suggestions,
                      int flags = SETSUGGESTIONS_SELECTFIRST |
                                  SETSUGGESTIONS_CALLBACK);
  ...
  //
  // If you pass true, then it will only ever call ICallbacks::OnTextChanged
  // with values that are in your suggestions, and it does its best to
  // autocomplete to those suggestions (so if the user types a partial string
  // and closes the box, it will find the first possible substring match OR it
  // will revert to the last valid suggestion it was on).
  //
  // If you pass false, then it will call OnTextChanged for anything that gets
  // entered into the textbox. This is used for the entity properties targetname
  // box, and the entity name changes right along as you type. When the user
  // presses enter, it does NOT automatically select the first suggestion. They
  // have to use the arrow keys for that.
  ...
  // Helper functions.
public:
  // This takes the string the user has entered (pStringToMatch passed into
  // GetItemsMatchingString) and returns true if pTestString matches it. It
  // ignores underscores in both strings.
...
protected:
  ...
  bool
      m_bNotifyParent; // Whether we allow our parent to hook our notification
                       // messages. This is necessary because
                       // CControlBar-derived classes result in multiple message
                       // reflections unless we disable parent notification.
...
protected:
  ...
  virtual void GetItemsMatchingString(const char *pStringToMatch,
                                      CUtlVector<CString> &matchingItems);
...
private:
  void InternalSetEditControlFont(HFONT hFont, const char *pEditText,
                                  DWORD sel);
  void CreateFonts();
...
private:
  ...
  // We go back here if they type text that we can't give a suggestion on and
  // press enter (or lose focus).
