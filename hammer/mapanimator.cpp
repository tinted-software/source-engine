...
//-----------------------------------------------------------------------------
CMapClass *CMapAnimator::CreateMapAnimator(CHelperInfo *pHelperInfo,
                                           CMapEntity *pParent) {
...
void CMapAnimator::OnParentKeyChanged(const char *key, const char *value) {
  if (!stricmp(key, "TimeModifier")) {
    ...
    // HACK: Force everything in the path to update. Better to follow our path
    // and update only it.
...
//-----------------------------------------------------------------------------
float CMapAnimator::GetKeyFramesAtTime(float time, CMapKeyFrame *&pKeyFrame,
...
CMapEntity *CMapAnimator::CreateNewKeyFrame(float time) {
  ...
  QAngle angles;
  Quaternion qAngles;
  GetAnimationAtTime(key, pPrevKey, partialTime, vOrigin, qAngles,
                     m_iPositionInterpolator, m_iRotationInterpolator);
...
//-----------------------------------------------------------------------------
// Purpose: stops CMapKeyframe from doing it's auto-connect behavior when
// cloning Input  : pClone -
//-----------------------------------------------------------------------------
void CMapAnimator::OnClone(CMapClass *pClone, CMapWorld *pWorld,
...
//			*newAngles -
//-----------------------------------------------------------------------------
void CMapAnimator::GetAnimationAtTime(float animTime, Vector &newOrigin,
  ...
  CMapKeyFrame *pPrevKeyFrame;
  float posTime = GetKeyFramesAtTime(newTime * totalAnimTime,
                                     m_pCurrentKeyFrame, pPrevKeyFrame);

  // find the position from that keyframe
  GetAnimationAtTime(m_pCurrentKeyFrame, pPrevKeyFrame, posTime, newOrigin,
                     newAngles, m_iPositionInterpolator,
                     m_iRotationInterpolator);
}

//-----------------------------------------------------------------------------
// Purpose: calculates the position of the animating object between two
// keyframes Input  : currentKey -
//			pPrevKey -
//			partialTime -
//			newOrigin -
//			newAngles -
...
//			rotInterpolator -
//-----------------------------------------------------------------------------
void CMapAnimator::GetAnimationAtTime(CMapKeyFrame *currentKey,
                                      CMapKeyFrame *pPrevKey, float partialTime,
                                      Vector &newOrigin, Quaternion &newAngles,
                                      int posInterpolator,
                                      int rotInterpolator) {
  ...
  IPositionInterpolator *pInterp =
      currentKey->SetupPositionInterpolator(posInterpolator);
...
//-----------------------------------------------------------------------------
// Purpose: Builds the animation transformation matrix for the entity if it's
// animating
//-----------------------------------------------------------------------------
...
//			samples the interpolator function to get an
//approximation of the curve
//-----------------------------------------------------------------------------
void CMapAnimator::RebuildPath(void) {
  CMapWorld *pWorld = GetWorldObject(this);
  if (!pWorld) {
    // Sometimes the object isn't linked back into the world yet when
    // RebuildPath() is called... we will be get called again when needed, but
    // may cause an incorrect (linear-only) path to get drawn temporarily.
    return;
  }
  ...
  while (pCurKey != NULL) {
    ...
    //
    CMapEntity *pNextEnt =
        pWorld->FindEntityByName(pCurEnt->GetKeyValue("NextKey"));
