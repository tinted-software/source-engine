...
enum {
  ...
  MAPVIEW_UPDATE_ONLY_LOGICAL =
      0x080, // update only Logical views for some reason
...
enum { VI_ZOOM = 0x01, VI_CENTER = 0x02 };

// Display modes for instances
enum ShowInstance_t {
  INSTANCES_HIDE = 0,    // instance contents are not displayed
  INSTANCES_SHOW_TINTED, // instance contents are displayed with a yellowish
                         // tint
...
//
// The doc holds a list of objects with dependents that changed since the last
// render. The list must be processed before rendering so that interobject
...
public:
  ...
  //
  // Interface to the map document. Functions for object creation, deletion,
  // etc.
  //
  ...
  bool FindEntitiesByKeyValue(CMapEntityList &Found, const char *szKey,
                              const char *szValue, bool bVisiblesOnly);
  bool FindEntitiesByName(CMapEntityList &Found, const char *szName,
                          bool bVisiblesOnly);
  bool FindEntitiesByClassName(CMapEntityList &Found, const char *szClassName,
                               bool bVisiblesOnly);
  bool FindEntitiesByNameOrClassName(CMapEntityList &Found, const char *pszName,
                                     bool bVisiblesOnly);
  ...
  void SelectObjectList(const CMapObjectList *pList,
                        int cmd = (scSelect | scClear | scSaveChanges));
  void SelectRegion(BoundBox *pBox, bool bInsideOnly,
                    bool ResetSelection = true);
  void SelectLogicalRegion(const Vector2D &vecMins, const Vector2D &vecMaxs,
                           bool bInsideOnly);
  ...
  void Paste(CMapObjectList &Objects, CMapWorld *pSourceWorld,
  ...
  void PasteInstance(CMapObjectList &Objects, CMapWorld *pSourceWorld,
                     CMapWorld *pDestWorld, Vector vecOffset, QAngle vecRotate,
                     CMapClass *pParent, bool bMakeEntityNamesUnique,
                     const char *pszEntityNamePrefix);
  void Paste(IHammerClipboard *pClipboard, CMapWorld *pDestWorld,
  ...
  void GetNudgeVector(const Vector &vHorz, const Vector &vVert, int nChar,
                      bool bSnap, Vector &vecNudge);
  ...
  void ReplaceTextures(LPCTSTR pszFind, LPCTSTR pszReplace, BOOL bEverything,
                       int iAction, BOOL bHidden,
  ...
  // Radius culling
  bool IsCulledBy3DCameraDistance(CMapClass *pObject,
  ...
  void VisGroups_CreateNamedVisGroup(CMapObjectList &Objects,
                                     const char *szName, bool bHide,
                                     bool bRemoveFromOtherVisGroups);
  void VisGroups_AddObjectsToVisGroup(CMapObjectList &Objects,
  ...
  static BOOL VisGroups_CheckForGroupCallback(CMapClass *pObject,
                                              CVisGroup *pGroup);
...
public:
  ...
  void ManifestPaste(CMapWorld *pDestWorld, Vector vecOffset, QAngle vecRotate,
                     CMapClass *pParent, bool bMakeEntityNamesUnique,
                     const char *pszEntityNamePrefix);
  virtual void UpdateInstanceMap(CMapDoc *pInstanceMapDoc);
  void CollapseInstances(bool bOnlySelected);
  void PopulateInstanceParms_r(CMapEntity *pEntity,
  ...
  void UpdateAllCameras(const Vector *vecViewPos, const Vector *vecLookAt,
                        const float *fZoom);
  ...
  void AddChildGroupToAutoVisGroup(CMapClass *pObject,
  ...
  void
  BuildCascadingSelectionList(CMapClass *pObj,
                              CUtlRBTree<CMapClass *, unsigned short> &list,
                              bool bRecursive);
...
protected:
  ...
  ChunkFileResult_t SaveViewSettingsVMF(CChunkFile *pFile,
                                        CSaveInfo *pSaveInfo);

  static bool HandleLoadError(CChunkFile *pFile, const char *szChunkName,
                              ChunkFileResult_t eError, CMapDoc *pDoc);
  static ChunkFileResult_t LoadCordonCallback(CChunkFile *pFile, CMapDoc *pDoc);
  static ChunkFileResult_t LoadCordonKeyCallback(const char *pszKey,
                                                 const char *pszValue,
                                                 CMapDoc *pDoc);
  ...
  static ChunkFileResult_t LoadGroupKeyCallback(const char *szKey,
                                                const char *szValue,
                                                CMapGroup *pGroup);
  static ChunkFileResult_t LoadVersionInfoCallback(CChunkFile *pFile,
                                                   CMapDoc *pDoc);
  static ChunkFileResult_t LoadVersionInfoKeyCallback(const char *szKey,
                                                      const char *szValue,
                                                      CMapDoc *pDoc);
  static ChunkFileResult_t LoadAutosaveCallback(CChunkFile *pFile,
                                                CMapDoc *pDoc);
  static ChunkFileResult_t LoadAutosaveKeyCallback(const char *szKey,
                                                   const char *szValue,
                                                   CMapDoc *pDoc);
  static ChunkFileResult_t LoadWorldCallback(CChunkFile *pFile, CMapDoc *pDoc);
  static ChunkFileResult_t LoadViewSettingsCallback(CChunkFile *pFile,
                                                    CMapDoc *pDoc);
  static ChunkFileResult_t LoadViewSettingsKeyCallback(const char *szKey,
                                                       const char *szValue,
                                                       CMapDoc *pDoc);
  ...
  static BOOL UpdateVisibilityCallback(CMapClass *pObject,
                                       UpdateVisibilityData_t *pData);

  bool GetChildrenToHide(CMapClass *pObject, bool bSelected,
                         CMapObjectList &List);
  ...
  void DispatchNotifyDependents(CMapClass *pObject,
                                Notify_Dependent_t eNotifyType);

  CUtlVector<NotifyListEntry_t> m_NotifyList;

  CMapWorld *m_pWorld;         // The world that this document represents.
  CMapObjectList m_UpdateList; // List of objects that have changed since the
                               // last call to Update.
  CString m_strLastExportFileName; // The full path that we last exported this
                                   // document to.
  int m_nDocVersion; // A number that increments every time the doc is modified
                     // after being saved.
  BOOL m_bNeedsAutosave; // True if the document has been changed and needs
                         // autosaved.
  BOOL m_bIsAutosave;
  ...
  int m_nNextMapObjectID; // The ID that will be assigned to the next CMapClass
                          // object in this document.
  int m_nNextNodeID; // The ID that will be assigned to the next "info_node_xxx"
                     // object created in this document.
  ...
  int m_nExternalReferenceCount; // Indicates how many external references (
                                 // instances ) are pointing to this map
  ShowInstance_t
      m_tShowInstance; // Indicates how instance contents should be displayed
  ...
  //
  // Expands %i keyword in prefab targetnames to generate unique targetnames for
  // this map.
  //
  bool ExpandTargetNameKeywords(char *szNewTargetName,
                                const char *szOldTargetName, CMapWorld *pWorld);
  bool DoExpandKeywords(CMapClass *pObject, CMapWorld *pWorld,
                        char *szOldKeyword, char *szNewKeyword);

  // Renames all named entities in pRoot
  void RenameEntities(CMapClass *pRoot, CMapWorld *pWorld, bool bMakeUnique,
                      const char *pszPrefix);

  void CenterOriginsRecursive(CMapClass *pObject);
  void SnapObjectsRecursive(CMapClass *pObject);

  // Add all entities connected to all entities in the selection list
  // recursively
  void AddConnectedNodes(CMapClass *pClass,
                         CUtlRBTree<CMapClass *, unsigned short> &list);
...
//-----------------------------------------------------------------------------
CMapWorld *CMapDoc::GetMapWorld(void) { return (m_pWorld); }

//-----------------------------------------------------------------------------
// Purpose: All map objects in a given document are assigned a unique ID.
//-----------------------------------------------------------------------------
int CMapDoc::GetNextMapObjectID(void) { return (m_nNextMapObjectID++); }

//-----------------------------------------------------------------------------
// Purpose: Returns the next unique ID for an AI node. Called when an AI node
//			is created so that each one can have a unique ID.
//
//			We can't use the unique object ID (above) for this
//because of 			problems with in-engine editing of nodes and node connections.
//-----------------------------------------------------------------------------
int CMapDoc::GetNextNodeID(void) { return (m_nNextNodeID++); }

//-----------------------------------------------------------------------------
// Purpose: Sets the next unique ID for a AI node creation. Called when an AI
// node
//			is created from the shell so that node IDs continue from
//there.
//-----------------------------------------------------------------------------
void CMapDoc::SetNextNodeID(int nID) { m_nNextNodeID = nID; }

//-----------------------------------------------------------------------------
// Purpose: Returns whether or not grid snap is enabled. Called by the tools and
//			views to determine snap behavior.
//-----------------------------------------------------------------------------
bool CMapDoc::IsSnapEnabled(void) { return m_bSnapToGrid; }
