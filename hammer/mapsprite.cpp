...
//-----------------------------------------------------------------------------
CMapClass *CMapSprite::CreateMapSprite(CHelperInfo *pHelperInfo,
                                       CMapEntity *pParent) {
  ...
  if (pszSprite) {
    if (!strnicmp(pszSprite, "materials", 9) &&
        ((pszSprite[9] == '/') || (pszSprite[9] == '\\'))) {
      pszSprite += 10;
    }
  }
...
// Purpose: Constructor.
//-----------------------------------------------------------------------------
CMapSprite::CMapSprite(void) { Initialize(); }

//-----------------------------------------------------------------------------
// Purpose: Destructor.
//-----------------------------------------------------------------------------
CMapSprite::~CMapSprite(void) { CSpriteCache::Release(m_pSpriteInfo); }
...
void CMapSprite::CalcBounds(BOOL bFullUpdate) {
  ...
  if (m_pSpriteInfo) {
    fRadius = max(m_pSpriteInfo->GetWidth(), m_pSpriteInfo->GetHeight()) *
              m_fScale / 2.0;
    if (fRadius == 0) {
      fRadius = 8;
    }
...
// Input  : Angles -
//-----------------------------------------------------------------------------
void CMapSprite::GetAngles(QAngle &Angles) { Angles = m_Angles; }
...
void CMapSprite::Render3D(CRender3D *pRender) {
  ...
  if (m_pSpriteInfo) {
    ...
    for (int nPass = 0; nPass < nPasses; nPass++) {
      ...
      MaterialPrimitiveType_t type =
          (nPass > 0) ? MATERIAL_LINE_LOOP : MATERIAL_POLYGON;
  ...
  //
    ...
    pRender->BeginRenderHitTarget(this);
    pRender->RenderBox(m_Render2DBox.bmins, m_Render2DBox.bmaxs, r, g, b,
                       GetSelectionState());
    pRender->EndRenderHitTarget();
  }
}
...
// Output : int
//-----------------------------------------------------------------------------
int CMapSprite::SerializeRMF(std::fstream &File, BOOL bRMF) { return (0); }
...
// Output : int
//-----------------------------------------------------------------------------
int CMapSprite::SerializeMAP(std::fstream &File, BOOL bRMF) { return (0); }
...
//			alpha -
//-----------------------------------------------------------------------------
void CMapSprite::SpriteColor(unsigned char *pColor, int eRenderMode,
                             colorVec RenderColor, int alpha) {
  int a;

  if ((eRenderMode == kRenderTransAdd) || (eRenderMode == kRenderGlow) ||
      (eRenderMode == kRenderWorldGlow)) {
    a = alpha;
  } else {
    a = 256;
  }
...
void CMapSprite::OnParentKeyChanged(const char *szKey, const char *szValue) {
  ...
  //
  // If we are the child of a light entity and its color is changing, change our
  // render color.
  //
  else if (!stricmp(szKey, "_light")) {
    sscanf(szValue, "%d %d %d", &m_RenderColor.r, &m_RenderColor.g,
           &m_RenderColor.b);
...
//-----------------------------------------------------------------------------
bool CMapSprite::ShouldRenderLast(void) { return (true); }
...
//-----------------------------------------------------------------------------
void CMapSprite::RenderLogicalAt(CRender2D *pRender, const Vector2D &vecMins,
