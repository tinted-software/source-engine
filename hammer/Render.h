...
//
// Colors for selected faces and edges. Kinda hacky; should probably be
// elsewhere.
//
...
public:
  ...
  virtual void SetRenderMode(EditorRenderMode_t eRenderMode,
                             bool force = false);
  ...
  virtual void DrawBox(const Vector &vMins, const Vector &vMaxs,
                       bool bFill = false);
  void DrawBoxExt(const Vector &vCenter, float extend, bool bFill = false);
  void DrawSphere(const Vector &vCenter, int nRadius);
  void DrawCircle(const Vector &vCenter, const Vector &vNormal, float flRadius,
                  int nSegments);
  void DrawPolyLine(int nPoints, const Vector *Points);
  void DrawText(const char *text, int x, int y,
                int nFlags); // Uses pixel coordinates
  void DrawText(const char *text, const Vector2D &vPos, int nOffsetX,
                int nOffsetY, int nFlags); // Uses "world" coordinates
  ...
  void DrawPlane(const Vector &p0, const Vector &p1, const Vector &p2,
                 const Vector &p3, bool bFill = false);

  // drawing complex objects
  void DrawModel(DrawModelInfo_t *pInfo, matrix3x4_t *pBoneToWorld,
                 const Vector &vOrigin, float fAlpha = 1,
  ...
  void TransformInstanceAABB(Vector &InMins, Vector &InMaxs, Vector &OutMins,
                             Vector &OutMaxs) {
    TransformAABB(m_CurrentInstanceState.m_InstanceMatrix.As3x4(), InMins,
                  InMaxs, OutMins, OutMaxs);
  }
...
protected:
  ...
  void DrawFilledRect(Vector2D &pt1, Vector2D &pt2, unsigned char *pColor,
                      bool bBorder);
  void DrawCross(Vector2D &pt1, Vector2D &pt2, unsigned char *pColor);
  void DrawCircle(Vector2D &vCenter, float fRadius, int nSegments,
...
protected:
  ...
  // render modes
  EditorRenderMode_t m_eCurrentRenderMode; // Current render mode setting -
                                           // Wireframe, flat, or textured.
  EditorRenderMode_t m_eDefaultRenderMode; // Default render mode - Wireframe,
                                           // flat, or textured.
  CUtlStack<EditorRenderMode_t> m_RenderModeStack;

  // instance
  int m_nInstanceCount; // increases each time an instance is drawn regardless
                        // of view
  ...
  TInstanceState m_CurrentInstanceState; // the current instance state ( with
                                         // current transforms )
  ...
  int m_nNumInstancesRendered; // number of instances rendered that impacted the
                               // stencil buffer
  CUtlVector<InstanceRenderingState_t>
      m_InstanceRenderingState; // the instance rendering state stack
};
