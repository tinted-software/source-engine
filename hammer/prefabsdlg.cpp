...
void CPrefabsDlg::AddToObjectList(CPrefab *pPrefab, int iItem, BOOL bReplace) {
  ...
  iItem =
      m_Objects.InsertItem(iItem, pPrefab->GetName(),
...
void CPrefabsDlg::OnExportobject() {
  ...
  while (iIndex != -1) {
    ...
    if (pPrefab) {
      ...
      CFileDialog dlg(FALSE, "map", strFilename,
                      OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT,
                      "Map files|*.map;*.rmf|", this);
      if (dlg.DoModal() == IDCANCEL)
        return; // nevermind
...
CPrefabLibrary *CPrefabsDlg::GetCurrentLibrary(int *piSel) {
  ...
  CPrefabLibrary *pLibrary =
      CPrefabLibrary::FindID(m_Libraries.GetItemData(iSel));
...
void CPrefabsDlg::OnSelchangeLibraries() {
  // get last library
  CPrefabLibrary *pLibrary =
      CPrefabLibrary::FindID(m_Libraries.GetItemData(iCurLibrary));
...
static int AskAboutInvalidFilename() {
  return AfxMessageBox(
      "That's not a valid name - some of the characters aren't\n"
...
void CPrefabsDlg::OnRemovelibrary() {
  ...
  if (AfxMessageBox(
          "Are you sure you want to delete this library from your hard drive?",
          MB_YESNO) == IDYES) {
    pLibrary->DeleteFile();
    delete pLibrary;
