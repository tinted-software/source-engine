...
// Purpose:
//-----------------------------------------------------------------------------
CMapClassManager::~CMapClassManager(void) { s_Classes.RemoveAll(); }
...
CMapClass::CMapClass(void) {
  ...
  //
  // The document manages the unique object IDs. Eventually all object
  // construction should be done through the document, eliminating the need for
  // CMapClass to know about CMapDoc.
  //
...
CMapClass::~CMapClass(void) {
  ...
  if (bCheckSafeObjects && m_pSafeObject->GetRefCount() != 1) {
    int ret = AfxMessageBox("Warning: a CMapClass is being deleted but is "
                            "still referenced by a CMapDoc.\n"
...
//-----------------------------------------------------------------------------
// Purpose: Returns a copy of this object. We should never call this
// implementation
//			since CMapClass cannot be instantiated.
// Input  : bUpdateDependencies - Whether to update object dependencies when
...
CMapClass *CMapClass::CopyFrom(CMapClass *pFrom, bool bUpdateDependencies) {
  ...
  //
  // Don't link to the parent if we're not updating dependencies, just copy the
  // pointer.
  //
...
//			FIXME: this can be removed if we do all our 2D rendering
//in this->Render2D.
// Input  : mins - receives the minima for culling
//			maxs - receives the maxima for culling.
//-----------------------------------------------------------------------------
...
//			section of the VMF. These keys are held until they are
//handled, then 			the memory is freed.
//-----------------------------------------------------------------------------
...
//			NOTE: this is used for unique keys and will return the
//value for the 			FIRST key with the given name.
//-----------------------------------------------------------------------------
...
CMapClass *CMapClass::GetNextDescendent(EnumChildrenPos_t &pos) {
  while (pos.nDepth >= 0) {
    ...
    //
    // Finished with this object's children, pop the stack and return the
    // object.
    //
    pos.nDepth--;
    if (pos.nDepth >= 0) {
...
//-----------------------------------------------------------------------------
// Purpose: Removes the given visgroup from the list of visgroups that this
// object
//			belongs to.
//-----------------------------------------------------------------------------
...
// Purpose:
//-----------------------------------------------------------------------------
int CMapClass::GetVisGroupCount(void) { return m_VisGroups.Count(); }
...
void CMapClass::RemoveAllChildren(void) {
  ...
  //
  FOR_EACH_OBJ(m_Children, pos) { m_Children[pos]->m_pParent = NULL; }
...
//			bUpdateBounds - TRUE to calculate new bounds, FALSE not
//to.
//-----------------------------------------------------------------------------
...
//			NOTE: The child objects are replicated, not merely added
//as children.
...
//-----------------------------------------------------------------------------
// Purpose: Recalculate's this object's bounding boxes. CMapClass-derived
// classes
//			should call this first, then update using their local
//data.
...
// Input  : uchRed, uchGreen, uchBlue - Color components.
//-----------------------------------------------------------------------------
void CMapClass::SetRenderColor(unsigned char uchRed, unsigned char uchGreen,
...
//			of a given type, recursively enumerating their children
//also.
// Input  : pfn - Enumeration callback function. Called once per child.
//			dwParam - User data to pass into the enumerating
//callback. 			Type - Unless NULL, only objects of the given type will be
//enumerated.
// Output : Returns FALSE if the enumeration was terminated early, TRUE if it
// completed.
//-----------------------------------------------------------------------------
BOOL CMapClass::EnumChildren(ENUMMAPCHILDRENPROC pfn, unsigned int dwParam,
                             MAPCLASSTYPE Type) {
  FOR_EACH_OBJ(m_Children, pos) {
    CMapClass *pChild = m_Children.Element(pos);
...
// completed.
//-----------------------------------------------------------------------------
BOOL CMapClass::EnumChildrenRecurseGroupsOnly(ENUMMAPCHILDRENPROC pfn,
...
CMapEntity *CMapClass::FindChildByKeyValue(const char *key, const char *value,
  ...
  FOR_EACH_OBJ(m_Children, pos) {
    CMapClass *pChild = m_Children.Element(pos);
    CMapEntity *e =
        pChild->FindChildByKeyValue(key, value, bIsInInstance, InstanceMatrix);
    if (e)
      return e;
  }
...
//			iterates through and notifies all the children of their
//cloned state 			NOTE: assumes that the children are in the same order in both the
//			original and the clone
// Input  : pNewObj - the clone of this object
//			OriginalList - The list of objects that were cloned
//			NewList - The parallel list of clones of objects in
//OriginalList
//-----------------------------------------------------------------------------
void CMapClass::OnClone(CMapClass *pNewObj, CMapWorld *pWorld,
...
//OriginalList
//-----------------------------------------------------------------------------
void CMapClass::OnPreClone(CMapClass *pNewObj, CMapWorld *pWorld,
                           const CMapObjectList &OriginalList,
                           CMapObjectList &NewList) {
  Assert(m_Children.Count() == pNewObj->m_Children.Count());
...
//			Allows the object to generate new unique IDs in the copy
//of itself.
// Input  : pCopy -
//			pSourceWorld -
//			pDestWorld -
...
//			NewList -
//-----------------------------------------------------------------------------
void CMapClass::OnPrePaste(CMapClass *pCopy, CMapWorld *pSourceWorld,
  ...
  FOR_EACH_OBJ(m_Children, pos) {
    ...
    pChild->OnPrePaste(pCopyChild, pSourceWorld, pDestWorld, OriginalList,
                       NewList);
  }
}
...
//			Allows the object to fixup any references to other
//objects in the 			clipboard with references to their copies.
// Input  : pCopy -
//			pSourceWorld -
//			pDestWorld -
//			OriginalList -
//			NewList -
//-----------------------------------------------------------------------------
void CMapClass::OnPaste(CMapClass *pCopy, CMapWorld *pSourceWorld,
                        CMapWorld *pDestWorld,
                        const CMapObjectList &OriginalList,
                        CMapObjectList &NewList) {
  Assert(m_Children.Count() == pCopy->m_Children.Count());
  ...
  FOR_EACH_OBJ(m_Children, pos) {
    ...
    pChild->OnPaste(pCopyChild, pSourceWorld, pDestWorld, OriginalList,
                    NewList);
  }
}
...
// Purpose:
//-----------------------------------------------------------------------------
size_t CMapClass::GetSize(void) { return (sizeof(*this)); }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
bool CMapClass::HitTest2D(CMapView2D *pView, const Vector2D &point,
                          HitInfo_t &HitData) {
...
// Purpose:
//-----------------------------------------------------------------------------
bool CMapClass::HitTestLogical(CMapViewLogical *pView, const Vector2D &point,
                               HitInfo_t &hitData) {
  if (!IsVisibleLogical())
    return false;
...
//-----------------------------------------------------------------------------
SelectionState_t
CMapClass::SetSelectionState(SelectionState_t eSelectionState) {
  FOR_EACH_OBJ(m_Children, pos) {
...
// Output :
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapClass::LoadEditorCallback(CChunkFile *pFile,
                                                CMapClass *pObject) {
  return (pFile->ReadChunk((KeyHandler_t)LoadEditorKeyCallback, pObject));
}

//-----------------------------------------------------------------------------
// Purpose: Handles keyvalues when loading the editor chunk of an object from
// the
//			MAP file. Keys are transferred to a special keyvalue
//list for use after 			the entire map has been loaded.
// Input  : szKey - Key to handle.
//			szValue - Value of key.
//			pObject - Object being loaded.
// Output : Returns ChunkFile_Ok.
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapClass::LoadEditorKeyCallback(const char *szKey,
                                                   const char *szValue,
                                                   CMapClass *pObject) {
...
//			etc. Notifies dependents and updates the parent with
//this object's 			new size.
//-----------------------------------------------------------------------------
...
//			giving us the opportunity to update ourselves
//accordingly.
// Input  : pObject - Object that we are dependent upon that has changed.
//-----------------------------------------------------------------------------
void CMapClass::OnNotifyDependent(CMapClass *pObject,
                                  Notify_Dependent_t eNotifyType) {}

//-----------------------------------------------------------------------------
// Purpose: Default implementation for saving editor-specific data. Does
// nothing. Input  : pFile - Output : ChunkFileResult_t
//-----------------------------------------------------------------------------
...
ChunkFileResult_t CMapClass::SaveVMF(CChunkFile *pFile, CSaveInfo *pSaveInfo) {
  ...
  if (eResult == ChunkFile_Ok) {
    eResult =
        pFile->WriteKeyValueBool("visgroupautoshown", m_bVisGroupAutoShown);
  }
...
//-----------------------------------------------------------------------------
// Purpose: Frees all the keys that were loaded from the editor chunk of the MAP
// file.
//-----------------------------------------------------------------------------
...
//			*szNewName -
//-----------------------------------------------------------------------------
void CMapClass::ReplaceTargetname(const char *szOldName,
...
//			on changes to the old object, and dependent on changes
//to the new object.
// Input  : pOldAttached - Object that this object was attached to (possibly
// NULL).
//			pNewAttached - New object being attached to (possibly
//NULL).
// Output : Returns pNewAttached.
//-----------------------------------------------------------------------------
CMapClass *CMapClass::UpdateDependency(CMapClass *pOldAttached,
                                       CMapClass *pNewAttached) {
  if (pOldAttached != pNewAttached) {
    ...
    //
    // Attach ourselves as a dependent of the other object. We will now be
    // notified of any changes to that object.
    //
...
//-----------------------------------------------------------------------------
// Purpose: Updates this object's parent, removing it from it's old parent (if
// any)
...
//			FIXME: Should our children necessarily have the same
//origin as us? 				   Seems like we should translate our children by our origin delta
//-----------------------------------------------------------------------------
...
void CMapClass::VisGroupShow(bool bShow, VisGroupSelection eVisGroup) {
  ...
  if (eVisGroup == USER) {
    // since user visgroup visibility has precedence over auto, it is possible
    // to change an object's auto visibility through an action in a user
    // visgroup.
    if (bShow) {
      m_bVisGroupAutoShown = bShow;
    }
...
//-----------------------------------------------------------------------------
// Purpose: Causes all objects in the world to update any object dependencies
// (pointers)
...
//			without being assigned to one.  This solves the problem
//of objects 			being destructively hidden by obsolete visgroups.
//-----------------------------------------------------------------------------
bool CMapClass::CheckVisibility(bool bLoading) {
  ...
  if (!bInUser && !m_bVisGroupShown) {
    ...
    if (bLoading && pDoc->VisGroups_ObjectCanBelongToVisGroup(this)) {
      // if this object is an orphan, we want it to be hidden but placed in a
      // new visgroup.
      bFoundOrphans = true;
...
//			will not be editable if it is located in a separate
//instance or 			submap.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: this function will notify all children that the instance they belong
// to has been moved.
//			it will also notify dependents of a translation.  this
//function is currently not 			used but may be.
//-----------------------------------------------------------------------------
