...
//-----------------------------------------------------------------------------
// Purpose: this function will parse through the known keys for the manifest map
// entry Input  : szKey - the key name
//			szValue - the value
//			pManifestMap - the manifest map this belongs to
// Output : ChunkFileResult_t - result of the parsing
//-----------------------------------------------------------------------------
ChunkFileResult_t CManifest::LoadKeyInfoCallback(const char *szKey,
                                                 const char *szValue,
                                                 CManifest *pDoc) {
...
//-----------------------------------------------------------------------------
// Purpose: this function is responsible for setting up the manifest map about
// to be read in Input  : pFile - the chunk file being read
//			pDoc - the owning manifest document
// Output : ChunkFileResult_t - result of the parsing
//-----------------------------------------------------------------------------
ChunkFileResult_t CManifest::LoadManifestInfoCallback(CChunkFile *pFile,
                                                      CManifest *pDoc) {
  ChunkFileResult_t eResult =
      pFile->ReadChunk((KeyHandler_t)LoadKeyInfoCallback, pDoc);
...
// Output : ChunkFileResult_t - result of the parsing
//-----------------------------------------------------------------------------
ChunkFileResult_t CManifest::LoadKeyCallback(const char *szKey,
                                             const char *szValue,
  ...
  if (!stricmp(szKey, "InternalID")) {
    ...
    if (!pManifestMap->m_Map->LoadVMF(pManifestMap->m_AbsoluteMapFileName,
                                      VMF_LOAD_ACTIVATE | VMF_LOAD_IS_SUBMAP)) {
...
// Output : ChunkFileResult_t - result of the parsing
//-----------------------------------------------------------------------------
ChunkFileResult_t CManifest::LoadManifestVMFCallback(CChunkFile *pFile,
                                                     CManifest *pDoc) {
  ...
  ChunkFileResult_t eResult =
      pFile->ReadChunk((KeyHandler_t)LoadKeyCallback, pManifestMap);
...
// Output : ChunkFileResult_t - result of the parsing
//-----------------------------------------------------------------------------
ChunkFileResult_t CManifest::LoadManifestMapsCallback(CChunkFile *pFile,
                                                      CManifest *pDoc) {
...
//-----------------------------------------------------------------------------
ChunkFileResult_t
CManifest::LoadKeyPrefsCallback(const char *szKey, const char *szValue,
                                TManifestLoadPrefs *pManifestLoadPrefs) {
  if (!stricmp(szKey, "InternalID")) {
    pManifestLoadPrefs->pManifestMap =
        pManifestLoadPrefs->pDoc->FindMapByID(atoi(szValue));
...
// Output : ChunkFileResult_t - result of the parsing
//-----------------------------------------------------------------------------
ChunkFileResult_t CManifest::LoadManifestVMFPrefsCallback(CChunkFile *pFile,
                                                          CManifest *pDoc) {
  ...
  ChunkFileResult_t eResult =
      pFile->ReadChunk((KeyHandler_t)LoadKeyPrefsCallback, &ManifestLoadPrefs);
...
// Output : ChunkFileResult_t - result of the parsing
//-----------------------------------------------------------------------------
ChunkFileResult_t CManifest::LoadManifestMapsPrefsCallback(CChunkFile *pFile,
                                                           CManifest *pDoc) {
  CChunkHandlerMap Handlers;
  Handlers.AddHandler("VMF", (ChunkHandler_t)LoadManifestVMFPrefsCallback,
                      pDoc);
  pFile->PushHandlers(&Handlers);
...
//-----------------------------------------------------------------------------
ChunkFileResult_t
CManifest::LoadManifestCordoningPrefsCallback(CChunkFile *pFile,
                                              CManifest *pDoc) {
  CChunkHandlerMap Handlers;
  Handlers.AddHandler("cordons", (ChunkHandler_t)CMapDoc::LoadCordonCallback,
                      pDoc);
  pFile->PushHandlers(&Handlers);
...
bool CManifest::LoadVMFManifest(const char *pszFileName) {
  ...
  if (eResult == ChunkFile_Ok) {
    ...
    CChunkHandlerMap Handlers;
    Handlers.AddHandler(
        "Info", (ChunkHandler_t)CManifest::LoadManifestInfoCallback, this);
    Handlers.AddHandler(
        "Maps", (ChunkHandler_t)CManifest::LoadManifestMapsCallback, this);
  ...
  if (eResult == ChunkFile_Ok) {
    ...
    GetMainWnd()->MessageBox(File.GetErrorText(eResult),
  ...
  if (GetNumMaps() == 0) {
    GetMainWnd()->MessageBox(File.GetErrorText(eResult),
                             "Manifest file does not contain any maps!",
                             MB_OK | MB_ICONEXCLAMATION);
    return false;
  }
  ...
  for (int i = 0; i < GetNumMaps(); i++) {
    ...
    if (pManifestMap->m_Map == NULL ||
        pManifestMap->m_Map->GetMapWorld() == NULL) {
...
bool CManifest::LoadVMFManifestUserPrefs(const char *pszFileName) {
  ...
  if (eResult == ChunkFile_Ok) {
    ...
    CChunkHandlerMap Handlers;
    Handlers.AddHandler(
        "Maps", (ChunkHandler_t)CManifest::LoadManifestMapsPrefsCallback, this);
    Handlers.AddHandler(
        "cordoning",
        (ChunkHandler_t)CManifest::LoadManifestCordoningPrefsCallback, this);
  ...
  if (eResult == ChunkFile_Ok) {
    ...
    //		GetMainWnd()->MessageBox( File.GetErrorText( eResult ), "Error
    //loading manifest!", MB_OK | MB_ICONEXCLAMATION );
  }
...
//-----------------------------------------------------------------------------
// Purpose: This function will save the manifest, the associated maps, and user
// prefs. Input  : pszFileName - the name of the manifest
//			bForce - if true, we need to save all files, as we are
//relocating.
...
//-----------------------------------------------------------------------------
// Purpose: this function will save the manifest file and all modified maps.  If
// we are
//			relocating the manifest to a new place, all maps will be
//saved relative to 			the new place
// Input  : the file name the manifest should be saved as
// Output : returns true if the save was completely successful.  A partial save
// will
//			return false.
//-----------------------------------------------------------------------------
    ...
    if (eResult == ChunkFile_Ok) {
      for (int i = 0; i < GetNumMaps(); i++) {
        ...
        if (eResult == ChunkFile_Ok) {
          ...
          eResult =
              File.WriteKeyValue("File", pManifestMap->m_RelativeMapFileName);
          eResult =
              File.WriteKeyValueInt("InternalID", pManifestMap->m_InternalID);
...
//-----------------------------------------------------------------------------
// Purpose: This function will save all maps associated with a manifest.  Only
// modified
...
bool CManifest::SaveVMFManifestMaps(const char *pszFileName) {
  ...
  for (int i = 0; i < GetNumMaps(); i++) {
    ...
    if ((pManifestMap->m_Map->IsModified() || m_bRelocateSave)) {
      if (pManifestMap->m_Map->SaveVMF(pManifestMap->m_AbsoluteMapFileName,
                                       0) == false) {
        bSaved = false;
      }
    }
  ...
  if (!bSaved) {
    GetMainWnd()->MessageBox("Not all pieces of the manifest were saved!",
                             "Error saving Manifest!",
                             MB_OK | MB_ICONEXCLAMATION);
  }
...
bool CManifest::SaveVMFManifestUserPrefs(const char *pszFileName) {
  ...
  if (eResult != ChunkFile_Ok) {
    GetMainWnd()->MessageBox(File.GetErrorText(eResult),
                             "Error saving Manifest User Prefs!",
                             MB_OK | MB_ICONEXCLAMATION);
    bSaved = false;
  } else {
    eResult = File.BeginChunk("Maps");
...
//-----------------------------------------------------------------------------
// Purpose: this function allows you to indicate if the user prefs have been
// modified. Input  : bModified - the new status of the user prefs
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: this function handles the routing of the manifest's modified flag
// down to the primary map Input  : bModified - the new modified status
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
void CManifest::GetFullMapPath(const char *pManifestMapFileName,
                               char *pOutputPath) {
...
//-----------------------------------------------------------------------------
// Purpose: this function will attempt to find the manifest map that owns the
// map doc Input  : pMap - the map doc to look up Output : returns a pointer to
// the owning manifest map, otherwise NULL
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: this function will set the manifest map as the primary map.  All
// entity / brush
...
//-----------------------------------------------------------------------------
// Purpose: this function will create and default a new manifest map, add it to
// the world Input  : AbsoluteFileName - the full path of the vmf file
//			RelativeFileName - the relative path of the vmf file
// Output : returns a pointer to the newly created manifest map.
//-----------------------------------------------------------------------------
CManifestMap *CManifest::CreateNewMap(const char *AbsoluteFileName,
  ...
  CMapClass *pHelper =
      CHelperFactory::CreateHelper(&HI, pManifestMap->m_Entity);
  if (pHelper != NULL) {
  ...
  CMapInstance *pMapInstance =
      pManifestMap->m_Entity->GetChildOfType((CMapInstance *)NULL);
  if (pMapInstance) {
...
//-----------------------------------------------------------------------------
// Purpose: This function will move the selection of the active map to a new sub
// map. Input  : pManifestMap - the sub map the selection shoud be moved to
//			CenterContents - if the contents should be centered
//-----------------------------------------------------------------------------
void CManifest::MoveSelectionToSubmap(CManifestMap *pManifestMap,
                                      bool CenterContents) {
#if 0
  ...
  pManifestMap->m_Map->ManifestPaste(
      pManifestMap->m_Map->GetMapWorld(), Vector(0.0f, 0.0f, 0.0f),
...
//-----------------------------------------------------------------------------
// Purpose: this function will cut the selection and move it into a newly
// created
//			map doc and manifest map.
// Input  : FriendlyName - this is the text friendly name that the user can
// refer
//				to the map as
//			FileName - The relative file name for this new map to be
//saved as 			CenterContents - whether or not we should center the contents in the
//new map
// Output : returns a pointer to the newly created manifest map.
//-----------------------------------------------------------------------------
CManifestMap *CManifest::MoveSelectionToNewSubmap(CString &FriendlyName,
                                                  CString &FileName,
                                                  bool CenterContents) {
#if 0
	if ( s_Clipboard.Objects.Count() != 0 )
  ...
  if (pManifestMap->m_Map->SaveVMF(pManifestMap->m_AbsoluteMapFileName, 0) ==
      false) {
    m_bLoading = false;
    SetActiveMapDoc(this);
    delete pManifestMap;
...
//-----------------------------------------------------------------------------
CManifestMap *CManifest::AddNewSubmap(CString &FriendlyName,
                                      CString &FileName) {
  char AbsoluteFileName[MAX_PATH];
...
//			bFromInstance - if the map is coming from a
//func_instance somewhere
...
bool CManifest::AddExistingMap(const char *pszFileName, bool bFromInstance) {
  ...
  _splitpath_s(pszFileName, NULL, 0, NULL, 0, RelativeFileName,
               sizeof(RelativeFileName), FileExt, sizeof(FileExt));
  strcat(RelativeFileName, FileExt);

  strcpy(AbsoluteFileName, m_ManifestDir);
  strcat(AbsoluteFileName, RelativeFileName);
  CManifestMap *pManifestMap =
      CreateNewMap(AbsoluteFileName, RelativeFileName, true);

  m_bLoading = true;
  if (!pManifestMap->m_Map->LoadVMF(pszFileName,
                                    VMF_LOAD_ACTIVATE | VMF_LOAD_IS_SUBMAP)) {
    m_bLoading = false;
    SetActiveMapDoc(this);
    delete pManifestMap;
...
//-----------------------------------------------------------------------------
// Purpose: This function will allow the user to browse to an exist map to add
// to the manifest.
//-----------------------------------------------------------------------------
...
void CManifest::CheckFileStatus(void) {
  ...
  if (p4 != NULL && p4->GetFileInfo(GetPathName(), &FileInfo) == true) {
    ...
    if (FileInfo.m_eOpenState == P4FILE_OPENED_FOR_ADD ||
        FileInfo.m_eOpenState == P4FILE_OPENED_FOR_EDIT) {
      m_bCheckedOut = true;
    }
  }
  ...
  for (int i = 0; i < GetNumMaps(); i++) {
    ...
    pManifestMap->m_bReadOnly =
        !g_pFullFileSystem->IsFileWritable(pManifestMap->m_AbsoluteMapFileName);
    pManifestMap->m_bCheckedOut = false;
    pManifestMap->m_bIsVersionControlled = false;

    if (p4 != NULL && p4->GetFileInfo(pManifestMap->m_AbsoluteMapFileName,
                                      &FileInfo) == true) {
...
//-----------------------------------------------------------------------------
// Purpose: This function will pass on the notification that a map has been
// updated Input  : pInstanceMapDoc - the map that was updated
//-----------------------------------------------------------------------------
...
//			pParent - a pointer to the parent of this object
//-----------------------------------------------------------------------------
void CManifest::AddManifestObjectToWorld(CMapClass *pObject,
                                         CMapClass *pParent) {
...
//			bChildren - whether we're removing the object's children
//as well.
//-----------------------------------------------------------------------------
void CManifest::RemoveManifestObjectFromWorld(CMapClass *pObject,
...
//			allow all of the submaps to be relocated relative to a
//new save location 			of the manifest itself.  Directories should be created
//automatically.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: this function will delete the manifest world and rest of the
// contents.
//-----------------------------------------------------------------------------
