...
void CArchDlg::OnCircle() { m_cArcSpin.SetPos(360); }
...
void CArchDlg::DrawArch(CDC *pDC) {
  ...
  MakeArc(bmins[0], bmins[1], bmaxs[0], bmaxs[1], iSides, fStartAngle, fArc,
          fOuterPoints);

  MakeArc(bmins[0] + iWallWidth, bmins[1] + iWallWidth, bmaxs[0] - iWallWidth,
          bmaxs[1] - iWallWidth, iSides, fStartAngle, fArc, fInnerPoints);
  ...
  pDC->MoveTo(pt.x + (int)((bmins[0] - fCenter[0])*fScaleX), pt.y -
  (int)((bmins[1] - fCenter[1])*fScaleY)); pDC->LineTo(pt.x + (int)((bmins[0] -
  fCenter[0])*fScaleX), pt.y - (int)((bmaxs[1] - fCenter[1])*fScaleY));
  pDC->LineTo(pt.x + (int)((bmaxs[0] - fCenter[0])*fScaleX), pt.y -
  (int)((bmaxs[1] - fCenter[1])*fScaleY)); pDC->LineTo(pt.x + (int)((bmaxs[0] -
  fCenter[0])*fScaleX), pt.y - (int)((bmins[1] - fCenter[1])*fScaleY));
  */
