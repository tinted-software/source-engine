...
// Purpose:
//-----------------------------------------------------------------------------
CPrefab3D::CPrefab3D() { m_pWorld = NULL; }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
CPrefab3D::~CPrefab3D() { FreeData(); }
...
CMapClass *CPrefab3D::Create(void) {
  ...
  // HACK: must calculate bounds here due to a hack in
  // CMapClass::CopyChildrenFrom
  pCopy->CalcBounds();
...
//-----------------------------------------------------------------------------
// Purpose: Returns true if the prefab data has been loaded from disk, false if
// not.
//-----------------------------------------------------------------------------
bool CPrefab3D::IsLoaded(void) { return (m_pWorld != NULL); }
...
int CPrefabRMF::Load(DWORD dwFlags) {
  ...
  //
  CPrefabLibraryRMF *pLibrary =
      dynamic_cast<CPrefabLibraryRMF *>(CPrefabLibrary::FindID(dwLibID));
  if (!pLibrary) {
    return -1;
  }
...
bool CPrefabVMF::IsLoaded(void) {
  ...
  //
  // We have loaded this prefab at least once this session. Check the file
  // date/time against our cached date/time to see if we need to reload it.
  //
  struct _stat info;
...
int CPrefabVMF::Load(DWORD dwFlags) {
  ...
  if (eResult == ChunkFile_Ok) {
    ...
    Handlers.AddHandler("world", (ChunkHandler_t)CPrefabVMF::LoadWorldCallback,
                        this);
    Handlers.AddHandler("entity",
                        (ChunkHandler_t)CPrefabVMF::LoadEntityCallback, this);
    // dvs:
    // Handlers.SetErrorHandler((ChunkErrorHandler_t)CPrefabVMF::HandleLoadError,
    // this);

    File.PushHandlers(&Handlers);

    // CMapDoc::SetLoadingMapDoc( this );  dvs: fix - without this, no
    // displacements in prefabs
  ...
  if (eResult == ChunkFile_Ok) {
    ...
    // GetMainWnd()->MessageBox(File.GetErrorText(eResult), "Error loading
    // prefab", MB_OK | MB_ICONEXCLAMATION);
  }
...
//-----------------------------------------------------------------------------
ChunkFileResult_t CPrefabVMF::LoadEntityCallback(CChunkFile *pFile,
                                                 CPrefabVMF *pPrefab) {
...
// Output : ChunkFileResult_t
//-----------------------------------------------------------------------------
ChunkFileResult_t CPrefabVMF::LoadWorldCallback(CChunkFile *pFile,
                                                CPrefabVMF *pPrefab) {
  CMapWorld *pWorld = pPrefab->GetWorld();
...
// Output : int
//-----------------------------------------------------------------------------
int CPrefabVMF::Save(LPCTSTR pszFilename, DWORD dwFlags) { return 1; }
