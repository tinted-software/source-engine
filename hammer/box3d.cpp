...
// Output :
//-----------------------------------------------------------------------------
int Box3D::HitTest(CMapView *pView, const Vector2D &ptClient,
                   bool bTestHandles) {
  bool bHit = false;
...
//-----------------------------------------------------------------------------
// Purpose: Set the cursor based on the hit test results and current translate
// mode. Input  : eHandleHit - The handle that the cursor is over.
//			eTransformMode - The current transform mode of the tool
//- scale, rotate, or shear.
//-----------------------------------------------------------------------------
unsigned long Box3D::UpdateCursor(CMapView *pView, const Vector &vHandleHit,
                                  TransformMode_t eTransformMode) {
...
// Input  : bEnable -
//-----------------------------------------------------------------------------
void Box3D::EnableHandles(bool bEnable) { m_bEnableHandles = bEnable; }
...
//-----------------------------------------------------------------------------
const Vector Box3D::NearestCorner(const Vector2D &vPoint, CMapView *pView,
...
//-----------------------------------------------------------------------------
void Box3D::StartTranslation(CMapView *pView, const Vector2D &vPoint,
...
void Box3D::GetStatusString(char *pszBuf) {
  *pszBuf = '\0';
  ...
  if (!IsTranslating() || m_TranslateMode == modeScale ||
      m_TranslateMode == modeMove) {
    if (!IsEmpty()) {
      ...
      case Units_None: {
        sprintf(pszBuf, " %dw %dl %dh @(%.0f %.0f %.0f)", (int)fabs(size.x),
                (int)fabs(size.y), (int)fabs(size.z), center.x, center.y,
                center.z);
        break;
      }

      case Units_Inches: {
        sprintf(pszBuf, " %d\"w %d\"l %d\"h", (int)fabs(size.x),
                (int)fabs(size.y), (int)fabs(size.z));
        break;
      }
      ...
      case Units_Feet_Inches: {
        ...
        sprintf(pszBuf, " %d' %d\"w %d' %d\"l %d' %d\"h", nFeetWide,
                nInchesWide, nFeetLong, nInchesLong, nFeetHigh, nInchesHigh);
        break;
      }
      }
    }
  } else if (m_TranslateMode == modeShear) {
    sprintf(pszBuf, " shear: %d %d %d ", (int)m_vTranslation.x,
            (int)m_vTranslation.y, (int)m_vTranslation.z);
    ...
    if (rotAxis != -1) {
      ...
      sprintf(pszBuf, " %.2f %.2f %.2f%c", m_vTranslation.x, m_vTranslation.y,
              m_vTranslation.z, 0xF8);
    }
  } else {
    Assert(0);
  }
    ...
    for (int y = -1; y < 2; y++) {
      ...
      for (int z = -1; z < 2; z++) {
        ...
        if (!bIs2D) {
          ...
          float fIntersection = IntersectionLineAABBox(
              bmins, bmaxs, vViewPoint, vViewPoint + vDelta * 99999, nFace);
...
bool Box3D::UpdateTranslation(const Vector &vUpdate, UINT uConstraints) {
  if (m_TranslateMode == modeNone) {
    ...
    if (uConstraints) {
      ...
      ProjectOnTranslationPlane(pos + m_vTranslation, m_vTranslation,
                                uConstraints);
      m_vTranslation -= pos;
    }
...
const VMatrix &Box3D::GetTransformMatrix() { return m_TransformMatrix; }
...
void Box3D::UpdateTransformMatrix() {
  ...
  if (m_TranslateMode == modeNone) {
    ...
    m_TransformMatrix.m[axisA][axisS] =
        (m_vTranslation[axisA]) / (vSize[axisS]);
    m_TransformMatrix.m[axisB][axisS] =
        (m_vTranslation[axisB]) / (vSize[axisS]);
  } else if (m_TranslateMode == modeRotate) {
...
// Input  : dwFlags -
//-----------------------------------------------------------------------------
void Box3D::SetDrawFlags(DWORD dwFlags) { m_dwDrawFlags = dwFlags; }

void Box3D::RenderHandles2D(CRender2D *pRender, const Vector &mins,
                            const Vector &maxs) {
  Vector handles[3 * 3 * 3];

  int numHandles =
      GetVisibleHandles(handles, pRender->GetView(), m_TranslateMode);

  if (numHandles == 0)
    return;

  pRender->SetHandleColor(GetRValue(m_clrHandle), GetGValue(m_clrHandle),
                          GetBValue(m_clrHandle));
...
void Box3D::RenderHandles3D(CRender3D *pRender, const Vector &mins,
                            const Vector &maxs) {
  Vector handles[3 * 3 * 3];
...
void Box3D::HandleToWorld(Vector &vWorld, const Vector &vHandle,
                          const Vector *pCustomHandleBox) {
...
void Box3D::RenderTool2D(CRender2D *pRender) {
  ...
  if (IsTranslating()) {
    ...
    pRender->PushRenderMode(RENDER_MODE_DOTTED);
    pRender->SetDrawColor(GetRValue(m_clrBox), GetGValue(m_clrBox),
                          GetBValue(m_clrBox));
  } else {
    pRender->PushRenderMode(RENDER_MODE_FLAT_NOZ);
    pRender->SetDrawColor(GetRValue(m_clrBox), GetGValue(m_clrBox),
  ...
  if (IsTranslating()) {
    if (m_TranslateMode == modeMove || m_TranslateMode == modeRotate) {
      ...
      pRender->SetHandleColor(GetRValue(Options.colors.clrToolDrag),
                              GetGValue(Options.colors.clrToolDrag),
                              GetBValue(Options.colors.clrToolDrag));
...
//			*maxs -
//-----------------------------------------------------------------------------
void Box3D::StartNew(CMapView *pView, const Vector2D &vPoint,
                     const Vector &vecStart, const Vector &vecSize) {
  // Setup our info
  m_TranslateMode = modeScale;
