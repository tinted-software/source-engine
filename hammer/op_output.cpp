...
//			list of entities. The connections are displayed in a
//grid control, 			each row of the grid control representing a connection that is
//			common to all entities being edited. For example, given
//these two ents:
//
//			Button01
...
//				OnPressed Door01 Open 0 0
//
//			If these two entities were selected, the grid control
//would show:
//
//				OnPressed Door01 Open 0 0
//
//			because it is the only connection that is common to both
//entities. 			Editing an entry in the grid control modifies the corresponding
//			connection in all selected entities.
//
// TODO: persist sort column index, sort directions, and column sizes
// TODO: implement an external mode, where the grid shows all connections to
// unselected ents
//
//=============================================================================//
...
// Static data.
//
CEntityConnectionList *COP_Output::m_pConnectionBuffer =
    new CEntityConnectionList;
CImageList *COP_Output::m_pImageList = NULL;

//-----------------------------------------------------------------------------
// Returns true if any of the target entities in the connection list are
// visible.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
static int CALLBACK ListCompareDelaysSecondary(COutputConnection *pOutputConn1,
...
//-----------------------------------------------------------------------------
static int CALLBACK ListCompareDelays(COutputConnection *pOutputConn1,
                                      COutputConnection *pOutputConn2,
                                      SortDirection_t eDirection) {
  CEntityConnectionList *pConnList1 = pOutputConn1->m_pConnList;
  CEntityConnectionList *pConnList2 = pOutputConn2->m_pConnList;
...
//-----------------------------------------------------------------------------
static int CALLBACK ListCompareOutputNames(COutputConnection *pOutputConn1,
                                           COutputConnection *pOutputConn2,
                                           SortDirection_t eDirection) {
  CEntityConnectionList *pConnList1 = pOutputConn1->m_pConnList;
  CEntityConnectionList *pConnList2 = pOutputConn2->m_pConnList;
...
//-----------------------------------------------------------------------------
static int CALLBACK ListCompareInputNames(COutputConnection *pOutputConn1,
                                          COutputConnection *pOutputConn2,
                                          SortDirection_t eDirection) {
  CEntityConnectionList *pConnList1 = pOutputConn1->m_pConnList;
  CEntityConnectionList *pConnList2 = pOutputConn2->m_pConnList;
...
//-----------------------------------------------------------------------------
static int CALLBACK ListCompareSourceNames(COutputConnection *pOutputConn1,
                                           COutputConnection *pOutputConn2,
                                           SortDirection_t eDirection) {
  CEntityConnectionList *pConnList1 = pOutputConn1->m_pConnList;
  CEntityConnectionList *pConnList2 = pOutputConn2->m_pConnList;
...
//-----------------------------------------------------------------------------
static int CALLBACK ListCompareTargetNames(COutputConnection *pOutputConn1,
                                           COutputConnection *pOutputConn2,
                                           SortDirection_t eDirection) {
  CEntityConnectionList *pConnList1 = pOutputConn1->m_pConnList;
  CEntityConnectionList *pConnList2 = pOutputConn2->m_pConnList;
...
void COP_Output::UpdateItemValidity(int nItem) {
  COutputConnection *pOutputConn =
      (COutputConnection *)m_ListCtrl.GetItemData(nItem);
...
bool COP_Output::ValidateConnections(COutputConnection *pOutputConn,
  ...
  for (int i = 0; i < nCount; i++) {
    ...
    if (pConnection != NULL) {
      ...
      if (!CEntityConnection::ValidateOutput(pOutputConn->m_pEntityList,
                                             pConnection->GetOutputName())) {
        return false;
      }

      // Check validity of target entity (is it in the map?)
      if (!CEntityConnection::ValidateTarget(m_pMapEntityList, bVisibilityCheck,
                                             pConnection->GetTargetName())) {
        return false;
      }

      // Check validity of input
      if (!CEntityConnection::ValidateInput(pConnection->GetTargetName(),
                                            pConnection->GetInputName(),
                                            bVisibilityCheck)) {
        return false;
      }
    }
  ...
  for (int i = 0; i < nConnCount; i++) {
    ...
    if (pConnection != NULL) {
      ...
      if (!bFound) {
        ...
        m_ListCtrl.InsertItem(LVIF_IMAGE, nItemCount, "", 0, 0, ICON_CONN_GOOD,
                              0);

        m_ListCtrl.SetItemText(nItemCount, OUTPUT_NAME_COLUMN,
                               pConnection->GetOutputName());
        m_ListCtrl.SetItemText(nItemCount, TARGET_NAME_COLUMN,
                               pConnection->GetTargetName());
        m_ListCtrl.SetItemText(nItemCount, INPUT_NAME_COLUMN,
                               pConnection->GetInputName());
        ...
        m_ListCtrl.SetItemText(
            nItemCount, ONLY_ONCE_COLUMN,
            (pConnection->GetTimesToFire() == EVENT_FIRE_ALWAYS) ? "No"
                                                                 : "Yes");
        m_ListCtrl.SetItemText(nItemCount, PARAMETER_COLUMN,
                               pConnection->GetParam());
...
void COP_Output::UpdateEditControls(void) {
  ...
  if (m_EditList.Count() > 0) {
    ...
    // We must ignore the text changed event here or else it'll set all selected
    // outputs to the same value.
  ...
  if (m_ListCtrl.GetSelectedCount() != 0) {
    ...
    if (nCount > 0) {
      for (int nItem = nCount - 1; nItem >= 0; nItem--) {
        if (m_ListCtrl.GetItemState(nItem, LVIS_SELECTED) & LVIS_SELECTED) {
          //
          // Each item in the list control is a list of identical connections
          // that are contained in multiple entities. Add each selected
          // connection to the selected entities.
          //
          COutputConnection *pOutputConn =
              (COutputConnection *)m_ListCtrl.GetItemData(nItem);
          CEntityConnectionList *pConnList = pOutputConn->m_pConnList;
    ...
    if (nCount > 0) {
      for (int nItem = nCount - 1; nItem >= 0; nItem--) {
        if (m_ListCtrl.GetItemState(nItem, LVIS_SELECTED) & LVIS_SELECTED) {
          //
          // Each item in the list control is a list of identical connections
          // that are contained in multiple entities. Since we don't store the
          // containing entity along with the connection, just try to remove all
          // the connections in the list from all the selected entities.
          //
          COutputConnection *pOutputConn =
              (COutputConnection *)m_ListCtrl.GetItemData(nItem);
          CEntityConnectionList *pConnList = pOutputConn->m_pConnList;
          ...
          if (pConnList != NULL) {
            ...
            for (int nConn = 0; nConn < nConnCount; nConn++) {
              ...
              if (pConnection != NULL) {
                ...
                //
                // Remove the connection from the upstream list of all entities
                // it targets.
                //
                CMapEntityList *pTargetList =
                    pConnection->GetTargetEntityList();
                if (pTargetList) {
    ...
    for (int nItem = nCount - 1; nItem >= 0; nItem--) {
      if (m_ListCtrl.GetItemState(nItem, LVIS_SELECTED) & LVIS_SELECTED) {
        ...
        if (pDocActive != NULL) {
          ...
          pDocActive->FindEntitiesByName(
              Found, m_ListCtrl.GetItemText(nItem, TARGET_NAME_COLUMN), false);
...
BOOL COP_Output::OnInitDialog(void) {
  ...
  // Force an update of the column header text so that the sort indicator is
  // shown.
  ...
  CAnchorDef anchorDefs[] = {
      ...
      CAnchorDef(IDC_OUTPUTS_STATIC_PANEL, k_eAnchorLeft, k_eAnchorBottom,
  ...
  if (pnmh->idFrom == IDC_LIST) {
    ...
    case LVN_COLUMNCLICK: {
      ...
      if (pnmv->iSubItem < OUTPUT_LIST_NUM_COLUMNS) {
        ...
        //
        // If they clicked on the current sort column, reverse the sort
        // direction.
        //
    ...
    case LVN_ITEMCHANGED: {
      ...
      if ((pnmv->uNewState & LVIS_SELECTED) !=
        ...
        // Forget the saved param, because it was for a different I/O
        // connection.
        m_strLastParam.Empty();
      }
...
void COP_Output::UpdateEntityList(void) {
  ...
  if (m_pObjectList != NULL) {
    FOR_EACH_OBJ(*m_pObjectList, pos) {
      ...
      if ((pObject != NULL) &&
          (pObject->IsMapClass(MAPCLASS_TYPE(CMapEntity)))) {
...
void COP_Output::SetSortColumn(int nColumn, SortDirection_t eDirection) {
  ...
  //
  // If the sort column or direction changed, update the new sort column header
  // text.
  //
  if ((m_nSortColumn != nColumn) ||
...
void COP_Output::UpdateConnectionList(void) {
  ...
  Assert(pWorld != NULL); // dvs: I've seen pWorld be NULL on app shutdown, not
                          // sure why we ended up here though
  if (!pWorld)
    return;
...
void COP_Output::SetSelectedConnections(CEntityConnectionList &List) {
  ...
  for (int nItem = 0; nItem < nItemCount; nItem++) {
    ...
    m_ListCtrl.SetItemState(nItem, bFound ? LVIS_SELECTED : ~LVIS_SELECTED,
                            LVIS_SELECTED);
  }
...
//			bSortColumn - true if this column is the sort column,
//false if not. 			eDirection - Direction of sort, Sort_Ascending or
//Sort_Descending.
//-----------------------------------------------------------------------------
void COP_Output::UpdateColumnHeaderText(int nColumn, bool bIsSortColumn,
                                        SortDirection_t eDirection) {
...
void COP_Output::UpdateEditedFireOnce(void) {
  ...
  if (pButton->IsWindowEnabled()) {
    ...
    for (int nItem = 0; nItem < m_ListCtrl.GetItemCount(); nItem++) {
      if (m_ListCtrl.GetItemState(nItem, LVIS_SELECTED) & LVIS_SELECTED) {
        m_ListCtrl.SetItemText(nItem, ONLY_ONCE_COLUMN,
                               nChecked ? "Yes" : "No");
      }
    }
    ResizeColumns();
...
void COP_Output::SetConnection(CEntityConnectionList *pConnectionList) {
  ...
  for (int nConn = 0; nConn < nConnCount; nConn++) {
    CEntityConnection *pConnection =
        (CEntityConnection *)pConnectionList->Element(nConn);
    if (pConnection == NULL)
      continue;
...
void COP_Output::FillInputList(void) {
  ...
  FOR_EACH_OBJ(*m_pMapEntityList, pos) {
    ...
    //
    // Get the entity's class, which contains the list of inputs that this
    // entity exposes.
    //
    GDclass *pClass = pEntity->GetClass();
    ...
    for (int i = 0; i < nCount; i++) {
      ...
      //
      // Don't add the input to the combo box if another input with the same
      // name and type is already there.
      //
      int nIndex = m_ComboInput.FindStringExact(-1, pInput->GetName());
      if (nIndex != CB_ERR) {
        CClassInput *pExistingInput =
            (CClassInput *)m_ComboInput.GetItemDataPtr(nIndex);
...
//-----------------------------------------------------------------------------
// Purpose: Fills the list of outputs with outputs common to all the selected
// entities.
//-----------------------------------------------------------------------------
...
//			in the given entity's output list. Used when multiple
//entities are 			selected into the Entity Properties dialog.
  ...
  if (pTargets != NULL) {
    ...
    if (nCount > 0) {
      for (int i = nCount - 1; i >= 0; i--) {
        ...
        if (!MapEntityList_HasInput(pTargets, pInput->GetName(),
                                    pInput->GetType())) {
...
//-----------------------------------------------------------------------------
void COP_Output::OnEditDelay(void) { UpdateEditedDelays(); }

//-----------------------------------------------------------------------------
// Purpose: Called when the contents of the target combo edit box change.
//-----------------------------------------------------------------------------
void COP_Output::OnFireOnce(void) { UpdateEditedFireOnce(); }
...
void COP_Output::OnShowHiddenTargetsAsBroken() {
  ...
  Options.general.bShowHiddenTargetsAsBroken =
      (m_ctlShowHiddenTargetsAsBroken.GetCheck() != FALSE);
...
//-----------------------------------------------------------------------------
void COP_Output::OnSelChangeInput(void) { InputChanged(); }

//-----------------------------------------------------------------------------
// Purpose: Called when the contents of the input combo edit box change.
//-----------------------------------------------------------------------------
void COP_Output::OnEditUpdateInput(void) { InputChanged(); }
...
//-----------------------------------------------------------------------------
void COP_Output::OnSelChangeOutput(void) { OutputChanged(); }

//-----------------------------------------------------------------------------
// Purpose: Called when the contents of the output combo edit box change.
//-----------------------------------------------------------------------------
void COP_Output::OnEditUpdateOutput(void) { OutputChanged(); }
...
//-----------------------------------------------------------------------------
void COP_Output::OnEditUpdateParam(void) { UpdateEditedParams(); }
...
void COP_Output::OnSize(UINT nType, int cx, int cy) { m_AnchorMgr.OnSize(); }
