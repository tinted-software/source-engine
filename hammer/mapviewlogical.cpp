...
// Unselected and selected color values for the connections palette against the
// background color
...
static color32
    s_pWireColors[LOGICAL_CONN_COLOR_COUNT][LOGICAL_CONN_SELECTION_STATES] = {
        {{MID, MID, 0, 255}, /* Mid Yellow */ {BRITE, BRITE, 0, 255},
         /* Bright Yellow  */},
        {{MID, DARK, 0, 255}, /* Dark Orange */ {BRITE, MID, 0, 255},
         /* Bright Orange */},
        {{0, DARK, 0, 255}, /* Dark Green */ {0, BRITE, 0, 255},
         /* Bright Green */},
        {{0, MID, MID, 255}, /* Mid Cyan */ {0, BRITE, BRITE, 255},
         /* Bright Cyan */},
        {{0, 0, MID, 255}, /* Mid Blue */ {0, MID, BRITE, 255},
         /* Bright Baby Blue */},
        {{MID, 0, MID, 255}, /* Mid Magenta */ {BRITE, 0, BRITE, 255},
         /* Bright Magenta */},
        {{DARK, DARK, DARK, 255}, /* Dark Gray */ {BRITE, BRITE, BRITE, 255},
         /* Bright White */},
};
...
CMapViewLogical::CMapViewLogical(void)
    : m_RenderDict(0, 1024, DefLessFunc(CMapClass *)) {
...
void CMapViewLogical::OnInitialUpdate(void) {
  ...
  // FIXME: Hardcoded light gray background - should be from a new "Logical
  // View" options settings dialog
...
//-----------------------------------------------------------------------------
bool CMapViewLogical::SelectAtCascading(const Vector2D &ptClient,
                                        bool bMakeFirst) {
...
//-----------------------------------------------------------------------------
void CMapViewLogical::OnRenderListDirty() { m_bUpdateRenderObjects = true; }
...
void CMapViewLogical::AddToRenderLists(CMapClass *pObject) {
  ...
  if (!pObject->IsGroup() && pObject->IsLogical()) {
    ...
    // Always paint all the entities to ensure that any inter-entity connections
    // are visible
    //		if ( !IsValidBox( vecMins, vecMaxs ) || IsInClientView( vecMins,
    //vecMaxs ) )
    {
  ...
  FOR_EACH_OBJ(*pChildren, pos) { AddToRenderLists(pChildren->Element(pos)); }
}
...
const color32 &CMapViewLogical::GetWireColor(const char *pszName,
                                             const bool bSelected,
                                             const bool bError,
                                             const bool bAnySelected) {
  // Select the connecting color based on the string passed in
  // (using OutputName for instance, gives varying "wire colors" by entity
  // output type).

  Assert(LOGICAL_CONN_COLOR_COUNT == (sizeof(s_pWireColors) / sizeof(color32)) /
                                         LOGICAL_CONN_SELECTION_STATES);
  ...
  if (!bError) {
    ...
    //		bool bBlinkingState = bSelected ? (GetTickCount() /
    //TIMER_BLINK_INTERVAL) & 1 : 0;
    return s_pWireColors[nIndex][bSelected];
  } else {
    // Only blink errors if nothing is selected, or this is selected
    bool bBlinkingState = bSelected || !bAnySelected
                              ? (GetTickCount() / TIMER_BLINK_INTERVAL) & 1
                              : bSelected;
...
void CMapViewLogical::DrawConnectingWire(float x, float y, CMapEntity *pSource,
  ...
  // Compensate for multiple inputs -- fan-in the connections from the various
  // pSource entities
  BOOL bFound = false;
  if (nInputs) {
    int nInput;
    ...
    if (bFound) {
      ...
      vecEndPosition.y += ((nInputs - 1) * LOGICAL_CONN_VERT_SPACING / 2) / 2 -
                          (nInput * LOGICAL_CONN_VERT_SPACING / 2);
  ...
  pRender->DrawLineTo(
      Vector(vecEndPosition.x - BACKWARD_WIRE_OVERSHOOT, flHalfY, 0.0f));
  pRender->DrawLineTo(Vector(vecEndPosition.x - BACKWARD_WIRE_OVERSHOOT,
                             vecEndPosition.y, 0.0f));
  pRender->DrawLineTo(Vector(vecEndPosition.x, vecEndPosition.y, 0.0f));
}

void CMapViewLogical::RenderConnections(const bool bDrawSelected,
                                        const bool bAnySelected) {
  Vector2D pt, pt2;
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    pMapClass->GetLogicalConnectionPosition(LOGICAL_CONNECTION_OUTPUT,
                                            vecStartPosition);

    CRender2D *pRender = GetRender();

    float x = vecStartPosition.x + LOGICAL_CONN_SPREAD_DIST +
              LOGICAL_CONN_TEXT_LENGTH;
    float y = vecStartPosition.y +
              (nConnectionCount - 1) * LOGICAL_CONN_VERT_SPACING / 2;
    ...
    for (int j = 0; j < nConnectionCount; ++j) {
      ...
      int nInputCount = pEntityList->Count();
      bool bBadInput =
          !MapEntityList_HasInput(pEntityList, pConn->GetInputName());
      bool bBadConnection = (nInputCount == 0);

      // Stop drawing entity connection text once the entity itself gets too
      // small.
      ...
      // Make sure we only draw the selected OR unselected connections as
      // requested
      if (bDrawSelected == (bEntitySelected || bInputSelected)) {
        ...
        if (bDrawDelay || bDrawOutput || bDrawInput || bDrawTarget) {
          ...
          int nChars = 0;
          if (bDrawOutput)
            nChars += Q_snprintf(pBuf + nChars, sizeof(pBuf), "%s",
                                 pConn->GetOutputName());
          if (bDrawDelay)
            nChars += Q_snprintf(pBuf + nChars, sizeof(pBuf), "(%.2f)",
                                 pConn->GetDelay());

          if (nChars)
            pRender->DrawText(
                pBuf,
                Vector2D(vecStartPosition.x + LOGICAL_CONN_SPREAD_DIST, y), 2,
                1, CRender2D::TEXT_JUSTIFY_TOP | CRender2D::TEXT_JUSTIFY_RIGHT);
        ...
        pRender->DrawLineTo(
            Vector(vecStartPosition.x + LOGICAL_CONN_SPREAD_DIST, y, 0.0f));
        pRender->DrawLineTo(Vector(x, y, 0.0f));

        if (bBadConnection) {
          // Draw an X for a bogus connection.
          pRender->MoveTo(Vector(x - LOGICAL_CONN_CROSS_SIZE,
                                 y - LOGICAL_CONN_CROSS_SIZE, 0.0f));
          pRender->DrawLineTo(Vector(x + LOGICAL_CONN_CROSS_SIZE,
                                     y + LOGICAL_CONN_CROSS_SIZE, 0.0f));
          pRender->MoveTo(Vector(x - LOGICAL_CONN_CROSS_SIZE,
                                 y + LOGICAL_CONN_CROSS_SIZE, 0.0f));
          pRender->DrawLineTo(Vector(x + LOGICAL_CONN_CROSS_SIZE,
          ...
          pRender->DrawCircle(
              Vector(x + LOGICAL_CONN_MULTI_CIRCLE_RADIUS, y, 0.0f),
          ...
          for (int k = 0; k < nInputCount; ++k) {
            // bBadInput = false; // This should be based on whether downstream
            // entity has the specificied named input
            bInputSelected =
                (pEntityList->Element(k)->GetSelectionState() != SELECT_NONE);
            color32 col = GetWireColor(pConn->GetOutputName(),
                                       bEntitySelected || bInputSelected,
                                       bBadInput, bAnySelected);
            ...
            DrawConnectingWire(mx, my, pMapClass, pConn,
                               pEntityList->Element(k));
      ...
      x += LOGICAL_CONN_HORIZ_SPACING * (nInputCount + 1) +
           2 * LOGICAL_CONN_MULTI_CIRCLE_RADIUS;
...
//-----------------------------------------------------------------------------
// convert client view space to map world coordinates (2D versions for
// convenience)
//-----------------------------------------------------------------------------
void CMapViewLogical::WorldToClient(Vector2D &ptClient,
                                    const Vector2D &vWorld) {
