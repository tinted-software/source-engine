...
void CModelBrowser::SetSkin(int nSkin) { m_pPicker->SelectSkin(nSkin); }
...
void CModelBrowser::SaveLoadSettings(bool bSave) {
  ...
  if (bSave) {
    ...
    if (!str.IsEmpty()) {
      sscanf(str, "%d %d %d %d", &rect.left, &rect.top, &rect.right,
             &rect.bottom);
...
void CModelBrowser::Resize() {
  ...
  m_pStatusLine->SetBounds(160, rect.Height() - 30,
                           max(100, rect.Width() - 166), 24);
}
...
BOOL CModelBrowser::OnInitDialog() {
  ...
  m_VGuiWindow.Create(NULL, _T("ModelViewer"), WS_VISIBLE | WS_CHILD,
                      CRect(0, 0, 100, 100), this, 1001);

  vgui::EditablePanel *pMainPanel =
      new CModelBrowserPanel(this, "ModelBrowerPanel");
