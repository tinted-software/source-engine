...
// visible.
//			Sets a timer to do the scroll so that we don't much with
//the view state 			while the tool is handling a mouse message.
...
//			nIntensity - Percentage of original color intensity to
//keep (0 - 100). 			bReverse - True ramps toward black, false ramps toward the
//given color.
...
void CMapView2DBase::DrawGrid(CRender2D *pRender, int xAxis, int yAxis,
  ...
  int xMin = SnapToGrid(
      (int)max(g_MIN_MAP_COORD, m_ViewMin[xAxis] - nGridSpacing), nGridSpacing);
  int xMax = SnapToGrid(
      (int)min(g_MAX_MAP_COORD, m_ViewMax[xAxis] + nGridSpacing), nGridSpacing);
  int yMin = SnapToGrid(
      (int)max(g_MIN_MAP_COORD, m_ViewMin[yAxis] - nGridSpacing), nGridSpacing);
  int yMax = SnapToGrid(
      (int)min(g_MAX_MAP_COORD, m_ViewMax[yAxis] + nGridSpacing), nGridSpacing);
...
void CMapView2DBase::UpdateClientView(void) {
  ...
  m_flMinZoom =
      min(m_ClientWidth / flMaxExtents, m_ClientHeight / flMaxExtents);
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
int CMapView2DBase::ObjectsAt(CMapWorld *pWorld, const Vector2D &vPoint,
                              HitInfo_t *pHitData, int nMaxObjects,
                              unsigned int nFlags) {
  int nIndex = 0;
  ...
  FOR_EACH_OBJ(*pChildren, pos) {
    ...
    if (pWorldChild) {
      nIndex += ObjectsAt(pWorldChild, vPoint, &pHitData[nIndex],
                          nMaxObjects - nIndex);
    } else if (IsLogical()) {
