...
//			maxs2 -
//-----------------------------------------------------------------------------
bool BoxesIntersect(Vector const &mins1, Vector const &maxs1,
                    Vector const &mins2, Vector const &maxs2) {
  if ((maxs1[0] < mins2[0]) || (mins1[0] > maxs2[0]) || (maxs1[1] < mins2[1]) ||
      (mins1[1] > maxs2[1]) || (maxs1[2] < mins2[2]) || (mins1[2] > maxs2[2])) {
    return (false);
  }
...
//
//			NOTE: Do not call this during file load!! Similar (but
//different) 				  bookkeeping is done in PostloadWorld during serialization.
//
...
//-----------------------------------------------------------------------------
// Purpose: Sorts all the objects in the world into three lists: entities,
// solids,
//			and groups. These lists are then serialized in SaveVMF.
// Input  : pSaveLists - Receives lists of objects.
//-----------------------------------------------------------------------------
BOOL CMapWorld::BuildSaveListsCallback(CMapClass *pObject,
                                       SaveLists_t *pSaveLists) {
...
//			entities that are in this world. Called whenever an
//object is added 			to this world.
...
//			descendents from this world's entity list. Called when
//an object is 			removed from this world.
...
void CMapWorld::RemoveChild(CMapClass *pChild, bool bUpdateBounds) {
  ...
  //
  // Remove the object from the culling tree because it is no longer a
  // root-level child.
  //
  if (m_pCullTree != NULL) {
...
//			bIsInInstance - optional parameter to indicate if the
//found entity is inside of an instance 			InstanceMatrix - optional parameter to
//set the localized matrix of the instance stack
//-----------------------------------------------------------------------------
CMapEntity *CMapWorld::FindChildByKeyValue(const char *key, const char *value,
                                           bool *bIsInInstance,
  ...
  return __super::FindChildByKeyValue(key, value, bIsInInstance,
                                      InstanceMatrix);
}
...
//as well.
//-----------------------------------------------------------------------------
void CMapWorld::RemoveObjectFromWorld(CMapClass *pObject,
...
//			notifies the document that an object's bounding box has
//changed.
// Input  : pChild -
//-----------------------------------------------------------------------------
void CMapWorld::UpdateChild(CMapClass *pChild) {
  ...
  //
  if (!IsTemporary()) // HACK: check to avoid prefab objects ending up in the
                      // doc's update list
  {
...
void CMapWorld::GetUsedTextures(CUsedTextureList &List) {
  List.RemoveAll();
  EnumChildren((ENUMMAPCHILDRENPROC)AddUsedTextures, (DWORD)&List,
               MAPCLASS_TYPE(CMapSolid));
  EnumChildren((ENUMMAPCHILDRENPROC)AddOverlayTextures, (DWORD)&List,
               MAPCLASS_TYPE(CMapOverlay));
}
...
//			This does not delete the map objects that the culling
//tree contains, 			only the leaves and nodes themselves.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Determines if this node is a node or a leaf. If it is a node, it
// will
//			be split into eight children and each child will be
//populated with 			objects whose bounding boxes intersect them, then split
//recursively. 			If this node is a leaf, no action is taken and recursion
//terminates.
// Input  : pNode -
//-----------------------------------------------------------------------------
#define MIN_NODE_DIM 1024 // Minimum node size of 170 x 170 x 170 feet
#define MIN_NODE_OBJECT_SPLIT                                                  \
  2 // Don't split nodes with fewer than two objects.
...
void CMapWorld::CullTree_SplitNode(CCullTreeNode *pNode) {
  ...
  if ((Size[0] > MIN_NODE_DIM) && (Size[1] > MIN_NODE_DIM) &&
      (Size[2] > MIN_NODE_DIM)) {
    Vector Mids;
    int nChild;
    ...
    for (nChild = 0; nChild < 8; nChild++) {
      ...
      //
      // Check all objects in this node against the child's bounding box, adding
      // the objects that intersect to the child's object list.
      //
    ...
    //
    // Remove all objects from this node's object list (since we are not a
    // leaf).
    //
...
//			PostLoadWorld function for every object in the world,
//then 			builds the culling tree.
//-----------------------------------------------------------------------------
void CMapWorld::PostloadWorld(void) {
  ...
  // Since s_bLoadingVMF was on before, a bunch of stuff got delayed. Now let's
  // do that stuff.
  ...
  CalcBounds(FALSE); // Recalculate the world's bounds now that everyone else's
                     // bounds are upadted.
}
...
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapWorld::LoadGroupCallback(CChunkFile *pFile,
                                               CMapWorld *pWorld) {
...
// Output : ChunkFileResult_t
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapWorld::LoadHiddenCallback(CChunkFile *pFile,
                                                CMapWorld *pWorld) {
  //
...
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapWorld::LoadKeyCallback(const char *szKey,
                                             const char *szValue,
                                             CMapWorld *pWorld) {
  if (!stricmp(szKey, "id")) {
...
// Output : ChunkFileResult_t
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapWorld::LoadSolidCallback(CChunkFile *pFile,
                                               CMapWorld *pWorld) {
...
// Output :
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapWorld::SaveSolids(CChunkFile *pFile, CSaveInfo *pSaveInfo,
                                        int saveFlags) {
  PresaveWorld();

  SaveLists_t SaveLists;
  EnumChildrenRecurseGroupsOnly((ENUMMAPCHILDRENPROC)BuildSaveListsCallback,
                                (DWORD)&SaveLists);
...
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapWorld::SaveVMF(CChunkFile *pFile, CSaveInfo *pSaveInfo,
                                     int saveFlags) {
  PresaveWorld();
  ...
  if (!(saveFlags & SAVEFLAGS_LIGHTSONLY)) {
    ...
    //
    // HACK: Save map version. This is already being saved in the version info
    // block by the doc.
    //
    if (eResult == ChunkFile_Ok) {
      eResult = pFile->WriteKeyValueInt(
          "mapversion", CMapDoc::GetActiveMapDoc()->GetDocVersion());
    }
    ...
    if (eResult == ChunkFile_Ok) {
      eResult =
          SaveObjectListVMF(pFile, pSaveInfo, &SaveLists.Solids, saveFlags);
    }
    ...
    if (eResult == ChunkFile_Ok) {
      eResult =
          SaveObjectListVMF(pFile, pSaveInfo, &SaveLists.Groups, saveFlags);
    }
  ...
  if (eResult == ChunkFile_Ok) {
    eResult =
        SaveObjectListVMF(pFile, pSaveInfo, &SaveLists.Entities, saveFlags);
  }
...
// Output : ChunkFileResult_t
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapWorld::SaveObjectListVMF(CChunkFile *pFile,
                                               CSaveInfo *pSaveInfo,
                                               const CMapObjectList *pList,
                                               int saveFlags) {
  FOR_EACH_OBJ(*pList, pos) {
    ...
    if (saveFlags & SAVEFLAGS_LIGHTSONLY) {
      ...
      bool bIsLight =
          pMapEnt && strncmp(pMapEnt->GetClassName(), "light", 5) == 0;
      if (!bIsLight)
        continue;
    }
...
//-----------------------------------------------------------------------------
// Purpose: Adds a given character to the end of a string if there isn't one
// already. Input  : psz - String to add the backslash to.
//			ch - Character to check for (and add if not found).
//			nSize - Size of buffer pointer to by psz.
// Output : Returns true if there was enough space in the dest buffer, false if
// not.
//-----------------------------------------------------------------------------
...
//			FIXME: AAARGH, slow!! Need to build a table or
//something.
// Input  : nFaceID -
//-----------------------------------------------------------------------------
...
static bool AppendString(char *szDest, char const *szSrc, int nDestSize) {
  ...
  *psz = '\0';
  ...
  if (*szSrc != '\0') {
    // If we ran out of room, don't append anything. We don't want partial
    // strings.
    *pszStart = '\0';
  }
...
//
//			This is used for multiselect editing of sidelist
//keyvalues.
//
// Input  : pszValue - The buffer to receive the face lists encoded as a string.
//			pFullFaceList - the list of faces that are considered
//fully in the list 			pPartialFaceList - the list of faces that are partially in
//the list
//-----------------------------------------------------------------------------
bool CMapWorld::FaceID_FaceIDListsToString(char *pszList, int nSize,
  ...
  if (pFullFaceIDList != NULL) {
    for (int i = 0; i < pFullFaceIDList->Count(); i++) {
      ...
      if (!EnsureTrailingChar(pszList, ' ', nSize) ||
          !AppendString(pszList, szID, nSize)) {
        return (false);
      }
    }
  }
...
//the list
//-----------------------------------------------------------------------------
bool CMapWorld::FaceID_FaceListsToString(char *pszList, int nSize,
...
//the list
//-----------------------------------------------------------------------------
void CMapWorld::FaceID_StringToFaceIDLists(CMapFaceIDList *pFullFaceList,
                                           CMapFaceIDList *pPartialFaceList,
                                           const char *pszValue) {
  if (pFullFaceList != NULL) {
...
//the list
//-----------------------------------------------------------------------------
void CMapWorld::FaceID_StringToFaceLists(CMapFaceList *pFullFaceList,
                                         CMapFaceList *pPartialFaceList,
                                         const char *pszValue) {
...
//			startName - the name of the original entity - assumed to
//already exist in the map 			outputName - the new name based on the original name,
//guaranteed to be unique 			szPrefix - a string to prepend to the new name
//			newNameBufferSize -
//			bMakeUnique - if true, the generated name will be unique
//in this world and pRoot 			szPrefix - prefix to prepend to the new name 			pRoot -
//an optional tree of objects to look in for uniqueness
//-----------------------------------------------------------------------------
bool CMapWorld::GenerateNewTargetname(const char *startName, char *outputName,
                                      int newNameBufferSize, bool bMakeUnique,
...
void CMapWorld::PostloadVisGroups() {
  ...
  if (bFoundOrphans == true) {
    pDoc->VisGroups_CreateNamedVisGroup(orphans, "_orphaned hidden", true,
                                        false);
    GetMainWnd()->MessageBox("Orphaned objects were found and placed into the "
                             "\"_orphaned hidden\" visgroup.",
...
//-----------------------------------------------------------------------------
bool CMapWorld::FindEntitiesByClassName(CMapEntityList &Found,
...
//-----------------------------------------------------------------------------
bool CMapWorld::FindEntitiesByKeyValue(CMapEntityList &Found,
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CMapWorld::FindEntitiesByName(CMapEntityList &Found, const char *pszName,
                                   bool bVisiblesOnly) {
  Found.RemoveAll();
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CMapWorld::FindEntitiesByNameOrClassName(CMapEntityList &Found,
                                              const char *pszName,
                                              bool bVisiblesOnly) {
  Found.RemoveAll();
...
//-----------------------------------------------------------------------------
// Tell all our children to update their dependencies because of the given
// object.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Returns if this map world is editable.  If it is not part of an
// instance
//			or manifest, then it is editable.  Otherwise, it lets
//the owning document 			determine the editing state.
//-----------------------------------------------------------------------------
