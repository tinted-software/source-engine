...
//
//			Materials are kept in a directory tree containing pairs
//of VMT 			and VTF files. Each pair of files represents a material.
//
//=============================================================================//
...
public:
  ...
  //-----------------------------------------------------------------------------
  static PreviewImageRetVal_t
  GetPreviewImageProperties(IMaterial *pMaterial, int *width, int *height,
    ...
    if (!IsIgnoredMaterial(pFileName)) {
      if ((pFileName[0] != '.') || (pFileName[1] != '.' && pFileName[1] != 0)) {
        if (g_pFullFileSystem->FindIsDirectory(findHandle)) {
          ...
          if (!InitDirectoryRecursive(pFileNameWithPath, pEnum, nContext,
                                      nFlags))
            return false;
        }
      }
...
bool CMaterial::ShouldSkipMaterial(const char *pszName, int nFlags) {
  ...
  for (int i = 0; i < g_pGameConfig->m_MaterialExcludeCount; i++) {
    ...
    const char *pMatchFound =
        Q_stristr(pszName, g_pGameConfig->m_MaterialExclusions[i].szDirectory);
  ...
  if (pGD != NULL) {
    for (int i = 0; i < pGD->m_FGDMaterialExclusions.Count(); i++) {
      const char *pMatchFound =
          Q_stristr(pszName, pGD->m_FGDMaterialExclusions[i].szDirectory);
      if (pMatchFound == pszName)
        return true;
    }
  }
...
void CMaterial::Reload(bool bFullReload) {
  ...
  // Make sure to bump the refcount again. Not sure why this wasn't always done
  // (check for leaks).
  if (m_pMaterial) {
...
void CMaterial::DrawIcon(CDC *pDC, CMaterial *pIcon, RECT &dstRect) {
  ...
  // FIXME: Workaround the fact that materials must be power of 2, I want 12
  // bite
  rect.top = 2;
