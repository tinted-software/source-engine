...
//-----------------------------------------------------------------------------
// Purpose: Called by the framework when the user chooses the OK or the Apply
// Now button. Output : Nonzero if the changes are accepted; otherwise zero.
//-----------------------------------------------------------------------------
BOOL COPTView3D::OnApply(void) {
  if (Options.view3d.bFilterTextures != m_bOldFilterTextures) {
    AfxMessageBox(
        "The changes to the 'Filter textures' setting will not take effect for "
        "any currently visible textures. Close all 3D views and reopen them "
        "for the new setting to completely take effect.");
  }
...
void COPTView3D::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar *pScrollBar) {
  ...
  //
  // Else if it is the model distance scroll bar, update the model distance
  // text. Also, notify the 3D view so that it can update in realtime.
  //
  ...
  //
  // Else if it is the detail distance scroll bar, update the detail distance
  // text. Also, notify the 3D view so that it can update in realtime.
  //
  ...
  //
  // Else if it is the maximum forward speed scroll bar, update the maximum
  // forward speed.
  //
  ...
  //
  // Else if it is the time to max speed scroll bar, update the time to max
  // speed.
  //
