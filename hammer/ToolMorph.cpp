...
void Morph3D::OnActivate() {
  if (IsActiveTool()) {
    ...
    for (int i = 0; i < pSelection->Count(); i++) {
      ...
      pobj->EnumChildren((ENUMMAPCHILDRENPROC)AddToMorph, (DWORD)this,
...
//-----------------------------------------------------------------------------
bool Morph3D::CanDeactivate(void) { return CanDeselectList(); }
...
void Morph3D::OnDeactivate() {
  ...
  if (!IsEmpty()) {
    ...
    for (int i = 0; i < nObjectCount; i++) {
      ...
      CMapClass *pSelect =
          pSolid->PrepareSelection(m_pDocument->GetSelection()->GetMode());
      if (pSelect) {
  ...
  FOR_EACH_OBJ(m_StrucSolids, pos) {
    ...
    if (pSSolid) {
      if (!pSSolid->IsValidWithDisps()) {
        // Ask
        if (AfxMessageBox("Invalid solid, destroy displacement(s)?",
                          MB_YESNO) == IDYES) {
...
//			and the CMapSolid is attached to it. The map solid is
//removed from 			the views, since it will be represented by the structured solid
...
int Morph3D::HitTest(CMapView *pView, const Vector2D &ptClient,
                     bool bTestHandles) {
  if (m_bBoxSelecting) {
...
bool CompareMorphHandles(const MORPHHANDLE &mh1, const MORPHHANDLE &mh2) {
  return ((mh1.pMapSolid == mh2.pMapSolid) &&
          (mh1.pStrucSolid == mh2.pStrucSolid) && (mh1.ssh == mh2.ssh));
}
...
//-----------------------------------------------------------------------------
bool Morph3D::MorphHitTest(CMapView *pView, const Vector2D &vPoint,
                           MORPHHANDLE *pInfo) {
  SSHANDLE hnd = 0;
  ...
  FOR_EACH_OBJ(m_StrucSolids, pos) {
    ...
    if (hnd) {
      if (bIs2D) {
        ...
        SSHANDLE hMatch =
            Get2DMatches(dynamic_cast<CMapView2D *>(pView), pStrucSolid, hi);
...
//-----------------------------------------------------------------------------
SSHANDLE Morph3D::Get2DMatches(CMapView2D *pView, CSSolid *pStrucSolid,
                               SSHANDLEINFO &hi,
...
//			position in the current 2D projection as the given
//handle.
// Input  : pInfo -
//			cmd -
...
// 3d translation --
void Morph3D::StartTranslation(CMapView *pView, const Vector2D &vPoint,
                               MORPHHANDLE *pInfo) {
  if (m_bScaling) {
    // back to 1
...
bool Morph3D::UpdateTranslation(const Vector &vUpdate, UINT uFlags) {
  ...
  if (uFlags != 0) {
    ProjectOnTranslationPlane(m_OrigHandlePos + m_vTranslation, m_vTranslation,
                              uFlags);
...
bool Morph3D::StartBoxSelection(CMapView *pView, const Vector2D &vPoint,
                                const Vector &vStart) {
...
bool Morph3D::CanSplitFace() {
  // along two edges.
  if (m_SelectedHandles.Count() != 2 ||
      (m_SelectedType != shtEdge && m_SelectedType != shtVertex))
    return false;
...
void Morph3D::GetSelectedCenter(Vector &pt) {
  ...
  //
  // If we have selected handles, our bounds center is the center of those
  // handles.
  //
  ...
  //
  // If no handles are selected, our bounds center is the center of all selected
  // solids.
  //
  else {
    FOR_EACH_OBJ(m_StrucSolids, pos) {
...
void Morph3D::OnScaleCmd(BOOL bReInit) {
  ...
  if (!bReInit) {
    ...
    m_ScaleDlg.SetWindowPos(NULL, pt.x, pt.y, 0, 0,
...
void Morph3D::UpdateScale() {
  ...
  for (int i = 0; i < m_SelectedHandles.Count(); i++) {
    ...
    hnd.pStrucSolid->SetVertexPosition(hi.iIndex, newpos[0], newpos[1],
                                       newpos[2]);
...
//-----------------------------------------------------------------------------
bool Morph3D::OnKeyDown2D(CMapView2D *pView, UINT nChar, UINT nRepCnt,
                          UINT nFlags) {
  bool bSnap =
      m_pDocument->IsSnapEnabled() && !(GetKeyState(VK_CONTROL) & 0x8000);
  if (nChar == VK_UP || nChar == VK_DOWN || nChar == VK_LEFT ||
      nChar == VK_RIGHT) {
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool Morph3D::OnChar2D(CMapView2D *pView, UINT nChar, UINT nRepCnt,
                       UINT nFlags) {
  return false;
}
...
// Output : Returns true if the message was handled, false if not.
//-----------------------------------------------------------------------------
bool Morph3D::OnLMouseDown2D(CMapView2D *pView, UINT nFlags,
...
// Output : Returns true if the message was handled, false if not.
//-----------------------------------------------------------------------------
bool Morph3D::OnLMouseUp2D(CMapView2D *pView, UINT nFlags,
                           const Vector2D &vPoint) {
  ...
  if (!IsTranslating()) {
    if (m_DragHandle.ssh != 0) {
      //
      // They clicked on a handle and released the left button without moving
      // the mouse. Change the selection state of the handle that was clicked
      // on.
      //
      UINT cmd = scClear | scSelect;
      if (m_bLButtonDownControlState) {
...
// Output : Returns true if the message was handled, false if not.
//-----------------------------------------------------------------------------
bool Morph3D::OnMouseMove2D(CMapView2D *pView, UINT nFlags,
                            const Vector2D &vPoint) {
  ...
  if (m_bMouseDown[MOUSE_LEFT]) {
    if (IsTranslating()) {
      ...
      //
      // If they are not already dragging a handle and they clicked on a valid
      // handle, see if they have moved the mouse far enough to begin dragging
      // the handle.
      //

      if (m_bLButtonDownControlState && !IsSelected(m_DragHandle)) {
        //
        // If they control-clicked on an unselected handle and then dragged the
        // mouse, select the handle that they clicked on now.
        //
...
// Output : Returns true if the message was handled, false if not.
//-----------------------------------------------------------------------------
bool Morph3D::OnLMouseDown3D(CMapView3D *pView, UINT nFlags,
                             const Vector2D &vPoint) {
  m_bHit = false;
...
bool Morph3D::SelectAt(CMapView *pView, UINT nFlags, const Vector2D &vPoint) {
  ...
  int nHits =
      pView->ObjectsAt(vPoint, Objects, sizeof(Objects) / sizeof(Objects[0]));

  // We now have an array of pointers to CMapAtoms. Any that can be upcast to
  // CMapClass we add to a list of hits.
...
// Output : Returns true if the message was handled, false if not.
//-----------------------------------------------------------------------------
bool Morph3D::OnLMouseUp3D(CMapView3D *pView, UINT nFlags,
                           const Vector2D &vPoint) {
...
void Morph3D::SnapSelectedToGrid(int nGridSpacing) {
  ...
  SelectHandle(NULL, scClear);
  FOR_EACH_VEC(vecHandles, i) { SelectHandle(&vecHandles[i], scSelect); }
...
bool Morph3D::NudgeHandles(CMapView *pView, UINT nChar, bool bSnap) {
  ...
  if (bSnap && (GetSelectedHandleCount() == 1) &&
      (GetSelectedType() == shtVertex)) {
...
// Output : Returns true if the message was handled, false if not.
//-----------------------------------------------------------------------------
bool Morph3D::OnKeyDown3D(CMapView3D *pView, UINT nChar, UINT nRepCnt,
                          UINT nFlags) {
  bool bSnap =
      m_pDocument->IsSnapEnabled() && !(GetAsyncKeyState(VK_CONTROL) & 0x8000);
...
// Output : Returns true if the message was handled, false if not.
//-----------------------------------------------------------------------------
bool Morph3D::OnMouseMove3D(CMapView3D *pView, UINT nFlags,
                            const Vector2D &vPoint) {
