    ...
    if (pEdit != NULL) {
      for (int i = 0; i < m_CheckListItems.Count(); i++) {
        ...
        if (m_CheckList.GetCheck(i) != 2) {
          pEdit->SetSpawnFlag(currentItem.nItemBit,
                              m_CheckList.GetCheck(i) ? TRUE : FALSE);
        }
      }
    }
  }
...
//			It is called to place all the flags belonging to the
//first 			selected object into the temporary CheckListItems vector
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: This function is called to combine flags when multiple objects are
// selected
//			It removes flags from the CheckListItem vector that are
//not present in all selected objects
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Creates the checklist by stepping through the CheckListItems vector
// that
...
void COP_Flags::OnCheckListChange() {
  ...
  for (int i = 0; i < m_CheckListItems.Count(); i++) {
    ...
    // If multiple of the selected entities have a different value for this
    // flag, note that. The entity page will use triStateMask to denote flags
    // that it should leave alone.
    if (m_CheckList.GetCheck(i) == 2)
