...
void COP_Model::UpdateData(int Mode, PVOID pData, bool bCanEdit) {
  ...
  if (Mode == LoadFirstData) {
    ...
    if (pModel) {
      // If they were on a previous animation, remember it and we'll set the
      // combo box to that after we tell it the list of suggestions.
      char txt[512];
      txt[0] = 0;
...
CMapStudioModel *COP_Model::GetModelHelper(void) {
  ...
  if (pObject != NULL) {
    ...
    if (pEntity != NULL) {
      CMapStudioModel *pModel =
          pEntity->GetChildOfType((CMapStudioModel *)NULL);
      return pModel;
    }
  }
  return NULL;
