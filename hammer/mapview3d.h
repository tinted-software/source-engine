...
public:
  ...
  void GetBestTransformPlane(Vector &horzAxis, Vector &vertAxis,
                             Vector &thirdAxis);
...
private:
  ...
  enum { MVTIMER_PICKNEXT = 0 };

  bool m_bMouseLook; // Set to true when we override the mouse processing to use
                     // mouselook.
  bool m_bStrafing;
  bool m_bRotating;
  CPoint m_ptLastMouseMovement; // Last position used for tracking the mouse for
                                // camera control.

  DWORD m_dwTimeLastSample;      // Used for calculating rendering framerate.
  DWORD m_dwTimeLastInputSample; // Used for framerate-independent input
                                 // processing.

  float m_fForwardSpeed; // Current forward speed, in world units per second.
  float
      m_fStrafeSpeed; // Current side-to-side speed, in world units per second.
  ...
  float m_fStrafeSpeedMax;  // Maximum side-to-side speed, in world units per
                            // second.
  float
      m_fVerticalSpeedMax; // Maximum up-down speed, in world units per second.

  float m_fForwardAcceleration; // Forward acceleration, in world units per
                                // second squared.
  float m_fStrafeAcceleration;  // Side-to-side acceleration, in world units per
                                // second squared.
  float m_fVerticalAcceleration; // Up-down acceleration, in world units per
                                 // second squared.

  DrawType_t m_eDrawType; // How we render - wireframe, flat, textured, lightmap
                          // grid, or lighting preview.
  bool m_bLightingPreview;

  CTitleWnd *m_pwndTitle; // Title window.

  CRender3D *m_pRender; // Performs the 3D rendering in our window.
  CKeyboard m_Keyboard; // Handles binding of keys and mouse buttons to logical
                        // functions.
