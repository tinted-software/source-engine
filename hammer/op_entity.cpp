...
ON_EN_CHANGE(IDC_SMARTCONTROL_INSTANCE_VARIABLE,
             OnChangeInstanceVariableControl)
...
COP_Entity::COP_Entity()
    : CObjectPage(COP_Entity::IDD), m_cClasses(this),
      m_SmartControlTargetNameRouter(this), m_VarList(this),
...
void COP_Entity::GetKeyState(const char *pShortName, EKeyState *pState,
  ...
  *pMissingTarget = false;
  ...
  if (!pVar) {
    if (m_InstanceParmData.Find(pShortName) !=
        m_InstanceParmData.InvalidIndex()) {
...
//"different" if the value 			is different from the value in our list.
...
//			indicate that not all objects have the same value for
//the key.
// Input  : pszKey -
//-----------------------------------------------------------------------------
  ...
  if (Mode == LoadFirstData) {
    ...
    m_cClasses.AddSuggestion(
        szBuf); // If we don't make sure it has this item in its list, it will
                ...
                // missing an entity that is in the map file. In that case, just
                // let it be. Check For Problems should ID the problem for them.
...
void COP_Entity::PresentProperties() {
  ...
  if (!m_bSmartedit || !m_pDisplayClass) {
    ...
    if (m_pObjectList->Count() == 1) {
      CMapEntity *pEntity =
          static_cast<CMapEntity *>(m_pObjectList->Element(0));
            ...
            if (strnicmp(pInstanceKey, "parm", strlen("parm")) == 0) {
              ...
              for (int j = pEntity->GetFirstKeyValue();
                ...
                if (strnicmp(pValue, pVariable, strlen(pVariable)) == 0) {
                  ...
                  InstanceParmData.m_ParmVariable =
                      new GDinputvariable(pReplace, pVariable);
                  InstanceParmData.m_ParmKey = pKey;
                  InstanceParmData.m_VariableName = pVariable;
                  int InsertIndex = m_InstanceParmData.Insert(
                      InstanceParmData.m_VariableName, InstanceParmData);

                  const char *ptr =
                      m_InstanceParmData[InsertIndex].m_VariableName;
    ...
    // rj: this must be after the instancing check above, as adding keyvalues to
    // m_kv can cause the list to get reallocated, causing the SetItemData to
    // have an invalid pointer Also add any keyvalues they added in dumbedit
    ...
    for (int i = m_kv.GetFirst(); i != m_kv.GetInvalidIndex();
      ...
      if (!m_pDisplayClass->VarForName(KeyValue.szKey) &&
          m_InstanceParmData.Find(KeyValue.szKey) ==
              m_InstanceParmData.InvalidIndex()) {
        int iItem = m_VarList.InsertItem(i, KeyValue.szKey);
        m_VarList.SetItemData(iItem, (DWORD)KeyValue.szKey);
      }
...
void COP_Entity::CreateSmartControls_PickButton(GDinputvariable *pVar,
  ...
  CButton *pButton = new CButton;
  pButton->CreateEx(0, "Button", "Pick...",
                    WS_TABSTOP | WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX |
                        BS_PUSHLIKE,
                    ButtonRect.left, ButtonRect.top, ButtonRect.Width(),
                    ButtonRect.Height(), GetSafeHwnd(), (HMENU)IDC_PICK_FACES);
  pButton->SendMessage(WM_SETFONT, (WPARAM)hControlFont);
...
//-----------------------------------------------------------------------------
// Purpose: this function will set up the smart controls for an instance
// variable Input  : pVar - the kv pair
//			ctrlrect - the rect space of this area
//			hControlFont - the standard font
//-----------------------------------------------------------------------------
void COP_Entity::CreateSmartControls_InstanceVariable(GDinputvariable *pVar,
                                                      CRect &ctrlrect,
                                                      HFONT hControlFont) {
  const char *pValue = m_kv.GetValue(pVar->GetName());
  char ValueData[KEYVALUE_MAX_KEY_LENGTH];
        ...
        for (int i = pInstanceParmsEntity->GetFirstKeyValue();
          ...
          if (strnicmp(pKey, "parm", strlen("parm")) == 0) {
            if (strnicmp(psValue, pVariable, strlen(pVariable)) == 0) {
              ...
              m_pInstanceVar =
                  new GDinputvariable("color255", "temp_parm_value");
              CUtlVector<const char *> helperNames;

              m_pDisplayClass->GetHelperForGDVar(m_pInstanceVar, &helperNames);

              //
              // Update the keyvalue help text control with this variable's help
              // info.
              //
              m_KeyValueHelpText.SetWindowText(
                  m_pInstanceVar->GetDescription());
  ...
  pStaticInstanceVariable->CreateEx(
      WS_EX_LEFT, "STATIC", "Variable:", WS_CHILD | WS_VISIBLE | SS_LEFT,
      ctrlrect.left, ctrlrect.top, 50, 24, GetSafeHwnd(), HMENU(IDC_STATIC));
  pStaticInstanceVariable->SendMessage(WM_SETFONT, (WPARAM)hControlFont);

  m_pEditInstanceVariable = new CEdit;
  ctrlrect.bottom += 2;
  m_pEditInstanceVariable->CreateEx(
      WS_EX_CLIENTEDGE, "EDIT", "",
      WS_TABSTOP | WS_CHILD | WS_VISIBLE | WS_BORDER | ES_AUTOHSCROLL,
      ctrlrect.left + 50, ctrlrect.top, ctrlrect.Width() - 50, 24,
      GetSafeHwnd(), HMENU(IDC_SMARTCONTROL_INSTANCE_VARIABLE));
  ...
  pStaticInstanceValue->CreateEx(
      WS_EX_LEFT, "STATIC", "Value:", WS_CHILD | WS_VISIBLE | SS_LEFT,
      ctrlrect.left, ctrlrect.top, 50, 24, GetSafeHwnd(), HMENU(IDC_STATIC));
  pStaticInstanceValue->SendMessage(WM_SETFONT, (WPARAM)hControlFont);

  m_pEditInstanceValue = new CEdit;
  m_pEditInstanceValue->CreateEx(
      WS_EX_CLIENTEDGE, "EDIT", "",
      WS_TABSTOP | WS_CHILD | WS_VISIBLE | WS_BORDER | ES_AUTOHSCROLL,
      ctrlrect.left + 50, ctrlrect.top, ctrlrect.Width() - 50, 24,
      GetSafeHwnd(), HMENU(IDC_SMARTCONTROL_INSTANCE_VALUE));
  m_pEditInstanceValue->SendMessage(WM_SETFONT, (WPARAM)hControlFont);
  m_pEditInstanceValue->SetWindowText(pReplace);
  m_pEditInstanceVariable->SetLimitText(
      KEYVALUE_MAX_KEY_LENGTH - VariableLimit -
...
//-----------------------------------------------------------------------------
// Purpose: this function will set up the smart controls for an instance
// parameter Input  : pVar - the kv pair
//			ctrlrect - the rect space of this area
//			hControlFont - the standard font
//-----------------------------------------------------------------------------
void COP_Entity::CreateSmartControls_InstanceParm(GDinputvariable *pVar,
                                                  CRect &ctrlrect,
                                                  HFONT hControlFont) {
  const char *pValue = m_kv.GetValue(pVar->GetName());
  char ValueData[KEYVALUE_MAX_KEY_LENGTH];
  ...
  m_pEditInstanceVariable->CreateEx(
      ...
      WS_TABSTOP | WS_CHILD | WS_VISIBLE | WS_BORDER | ES_AUTOHSCROLL,
      ctrlrect.left + 50, ctrlrect.top, ctrlrect.Width() - 50, 24,
      GetSafeHwnd(), HMENU(IDC_SMARTCONTROL_INSTANCE_PARM));
  m_pEditInstanceVariable->SendMessage(WM_SETFONT, (WPARAM)hControlFont);
  m_pEditInstanceVariable->SetWindowText(pVariable);
  m_pEditInstanceVariable->SetLimitText(VariableLimit);
  ...
  ctrlrect.bottom += 150;
  ctrlrect.left += 50;
  m_pComboInstanceParmType->Create(
  ...
  for (int i = 0; i < ivMax; i++) {
    m_pComboInstanceParmType->AddString(
        GDinputvariable::GetVarTypeName((GDIV_TYPE)i));
  }
...
void COP_Entity::BrowseTextures(const char *szFilter, bool bSprite) {
  ...
  if (pBrowser->DoModal() == IDOK) {
    ...
    if (bSprite && g_pGameConfig->GetTextureFormat() == tfVMT) {
      ...
      // Strcat is being zee stupido. I prolly have to strip the other string or
      // something. Q_strcat( szName, sprExt, MAX_PATH );
    }
...
//contents are 			converted to keyvalue data in SmartEdit mode.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: this function is called whenever the instance variable or value is
// changed
//-----------------------------------------------------------------------------
void COP_Entity::OnChangeInstanceVariableControl(void) {
  if (m_pEditInstanceVariable && m_pEditInstanceValue) {
    char szVariable[KEYVALUE_MAX_VALUE_LENGTH],
        szValue[KEYVALUE_MAX_VALUE_LENGTH];
...
void COP_Entity::OnBrowse(void) {
  ...
  //
  int ret;
  if (1 /*g_pFullFileSystem->IsSteam()*/ ||
      CommandLine()->FindParm("-NewDialogs"))
    ret = pDlg->DoModal();
  else
  ...
  if (pButton != NULL) {
    if (pButton->GetCheck()) {
      ...
      if (nSel != LB_ERR) {
        ...
        if (pVar != NULL) {
          Assert((pVar->GetType() == ivSideList) ||
                 (pVar->GetType() == ivSide));
          ...
          //
          // Build the list of faces that are in all selected entities, and a
          // list of faces that are in at least one selected entity, so that we
          // can do multiselect properly.
          //
...
//-----------------------------------------------------------------------------
// Purpose: this function will attempt to look up a variable from the variable
// map Input  : index - non-negative, it is the index into the variable map.
//					-1 = invalid
//					negative starting at the
//INSTANCE_VAR_MAP_START indicates it is a 						custom instance parameter.
// Output :
//-----------------------------------------------------------------------------
GDinputvariable *COP_Entity::GetVariableAt(int index) {
  ...
  if (m_VarMap[index] <= INSTANCE_VAR_MAP_START) {
    return m_InstanceParmData[INSTANCE_VAR_MAP_START - m_VarMap[index]]
        .m_ParmVariable;
  }
...
void COP_Entity::UpdatePickFaceText(CToolPickFace *pTool) {
  ...
  if (!CMapWorld::FaceID_FaceListsToString(szList, sizeof(szList),
    ...
    MessageBox("Too many faces selected for this keyvalue to hold. Deselect "
               "some faces.",
               "Error", MB_OK);
  }
