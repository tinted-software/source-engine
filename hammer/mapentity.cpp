...
void CMapEntity::AddHelper(CMapClass *pHelper, bool bLoading) {
  ...
  //eliminate the knowledge 		that CMapEntity has about its animator child.
  //
...
//rendering, UI, and 			bookkeeping functions for their parent entities. If the
//class 			definition does not specify any helpers, or none of the helpers 			could be
//added, a box helper is added so that the entity has some 			visual
//representation.
// Inputs : pClass -
//			bLoading - True if this is being called from Postload,
//false otherwise.
...
//-----------------------------------------------------------------------------
void CMapEntity::GetRenderColor(CRender2D *pRender, unsigned char &red,
...
//entities if 			they have at least one solid child, point entities if not.
//-----------------------------------------------------------------------------
...
CMapEntity *CMapEntity::FindChildByKeyValue(LPCSTR key, LPCSTR value,
  ...
  return CMapClass::FindChildByKeyValue(key, value, bIsInInstance,
                                        InstanceMatrix);
}
...
void CMapEntity::Render2D(CRender2D *pRender) {
  ...
  if (pRender->GetInstanceRendering()) {
    ...
    pRender->TransformInstanceAABB(vecMins, vecMaxs, vecExpandedMins,
                                   vecExpandedMaxs);
...
void CMapEntity::RenderLogical(CRender2D *pRender) {
  ...
  color32 rgbColor = GetRenderColor(pRender);
  color32 rgbHighlight = {(byte)(7 * rgbColor.r / 8),
                          (byte)(7 * rgbColor.g / 8),
                          (byte)(7 * rgbColor.b / 8), (byte)255};
  color32 rgbLowlight = {(byte)(5 * rgbColor.r / 8), (byte)(5 * rgbColor.g / 8),
                         (byte)(5 * rgbColor.b / 8), (byte)255};
  color32 rgbEdgeColor = {(byte)(3 * rgbColor.r / 8),
                          (byte)(3 * rgbColor.g / 8),
                          (byte)(3 * rgbColor.b / 8), (byte)255};
  color32 rgbInterior = {(byte)(2 * rgbColor.r / 8), (byte)(2 * rgbColor.g / 8),
                         (byte)(2 * rgbColor.b / 8), (byte)255};
