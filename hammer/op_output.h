...
public:
  ...
  static CEntityConnectionList
      *m_pConnectionBuffer; // Stores connections for copy/pasting
...
protected:
  ...
  void UpdateColumnHeaderText(int nColumn, bool bIsSortColumn,
                              SortDirection_t eDirection);

  CMapEntityList
      m_EntityList; // Filtered m_pObjectList that only includes map entities
...
protected:
  ...
  SortDirection_t
      m_eSortDirection[OUTPUT_LIST_NUM_COLUMNS]; // Last sort direction per
                                                 // column.
