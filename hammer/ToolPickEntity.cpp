...
//-----------------------------------------------------------------------------
void CToolPickEntity::OnDeactivate() { DeselectAll(); }
...
//			point - Point in client coordinates where the event
//occurred.
// Output : Returns true if the message was handled by the tool, false if not.
//-----------------------------------------------------------------------------
bool CToolPickEntity::OnLMouseUp3D(CMapView3D *pView, UINT nFlags,
...
// Output : Returns true if the message was handled by the tool, false if not.
//-----------------------------------------------------------------------------
bool CToolPickEntity::OnLMouseDown3D(CMapView3D *pView, UINT nFlags,
                                     const Vector2D &vPoint) {
...
// Output : Returns true if the message was handled by the tool, false if not.
//-----------------------------------------------------------------------------
bool CToolPickEntity::OnLMouseDownLogical(CMapViewLogical *pView, UINT nFlags,
                                          const Vector2D &vPoint) {
  bool bControl = ((nFlags & MK_CONTROL) != 0);
...
// Output : Returns true if the message was handled by the tool, false if not.
//-----------------------------------------------------------------------------
bool CToolPickEntity::OnLMouseDblClk3D(CMapView3D *pView, UINT nFlags,
                                       const Vector2D &vPoint) {
  return true;
}
...
// Output : Returns true if the message was handled by the tool, false if not.
//-----------------------------------------------------------------------------
bool CToolPickEntity::OnRMouseUp3D(CMapView3D *pView, UINT nFlags,
                                   const Vector2D &vPoint) {
  return true;
}
...
// Output : Returns true if the message was handled by the tool, false if not.
//-----------------------------------------------------------------------------
bool CToolPickEntity::OnRMouseDown3D(CMapView3D *pView, UINT nFlags,
                                     const Vector2D &vPoint) {
  return true;
}
...
// Output : Returns true if the message was handled by the tool, false if not.
//-----------------------------------------------------------------------------
bool CToolPickEntity::OnMouseMove3D(CMapView3D *pView, UINT nFlags,
                                    const Vector2D &vPoint) {
  SetEyedropperCursor();
  return true;
}

bool CToolPickEntity::OnMouseMoveLogical(CMapViewLogical *pView, UINT nFlags,
                                         const Vector2D &vPoint) {
  SetEyedropperCursor();
  return true;
}
...
//-----------------------------------------------------------------------------
// Purpose: Sets the fully selected and partially selected entities for the
// picker. Input  : EntityListFull -
//			EntityListPartial -
//-----------------------------------------------------------------------------
void CToolPickEntity::SetSelectedEntities(CMapEntityList &EntityListFull,
...
void CToolPickEntity::DeselectEntity(int nIndex) {
  ...
  if (m_Entities.IsValidIndex(nIndex)) {
    if (m_Entities[nIndex].eOriginalState != EntityState_Partial) {
      ...
      //
      // Just set the state to deselected so we can cycle back to partial
      // selection.
      //
      m_Entities[nIndex].pEntity->SetSelectionState(SELECT_NONE);
...
//-----------------------------------------------------------------------------
void CToolPickEntity::GetSelectedEntities(CMapEntityList &EntityListFull,
                                          CMapEntityList &EntityListPartial) {
