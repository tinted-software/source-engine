...
//-----------------------------------------------------------------------------
CMapClass *CMapDecal::CreateMapDecal(CHelperInfo *pHelperInfo,
                                     CMapEntity *pParent) {
...
//			normal distance to the solid face. We still may not lie
//within the 			face; that is determined by the clipping code.
// Input  : pSolid - Solid to check.
//			ppFaces - Returns with pointers to the faces that are
//eligible.
...
int CMapDecal::DecalSolid(CMapSolid *pSolid) {
  ...
  if (nTestFaces != 0) {
    ...
    for (int nFace = 0; nFace < nTestFaces; nFace++) {
      ...
      int nPointCount =
          CreateClippedPoly(pFace, m_pTexture, m_Origin, ClipPoints,
                            sizeof(ClipPoints) / sizeof(ClipPoints[0]));
      ...
      if (nPointCount != 0) {
        ...
        for (int nPoint = 0; nPoint < nPointCount; nPoint++) {
          pDecalFace->pFace->SetTextureCoords(nPoint, ClipPoints[nPoint][3],
                                              ClipPoints[nPoint][4]);
        }
...
// Output : Returns true if the decal is still attached to the solid, false if
// not.
//-----------------------------------------------------------------------------
void CMapDecal::OnNotifyDependent(CMapClass *pObject,
  ...
  if (pSolid != NULL) {
    ...
    //
    // We could not re-attach to the solid because it was moved out of range or
    // deleted. If we are no longer attached to any solids, remove our entity
    // from the world.
    //
...
void CMapDecal::Render3D(CRender3D *pRender) {
  ...
  if ((eSelectionState != SELECT_NONE) &&
...
//-----------------------------------------------------------------------------
void CMapDecal::OnPaste(CMapClass *pCopy, CMapWorld *pSourceWorld,
