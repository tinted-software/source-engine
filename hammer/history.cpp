...
static CHistory
    *pCurHistory; // The Undo/Redo history associated with the active doc.
...
// Input  : pNewSelection - List to populate with the new selection set after
// the Undo.
//-----------------------------------------------------------------------------
...
//			bFromOpposite -
//-----------------------------------------------------------------------------
void CHistory::MarkUndoPosition(const CMapObjectList *pSelection,
                                LPCTSTR pszName, BOOL bFromOpposite) {
...
//-----------------------------------------------------------------------------
// Purpose: Sets the given history object as the one to use for all Undo
// operations.
//-----------------------------------------------------------------------------
...
CTrackEntry::CTrackEntry(TrackType_t eType, ...) {
  ...
  switch (m_eType) {
  //
  // Keep track of an object that was modified by the user. An Undo will cause
  // this object to revert to its original state.
  //
  case ttCopy: {
  ...
  //
  // Keep track of an object that was created by the user. An Undo will cause
  // this object to be removed from the world.
  //
  case ttCreate: {
  ...
  //
  // Keep track of an object that was deleted by the user. An Undo will cause
  // this object to be added back into the world.
  //
  case ttDelete: {
...
//			history. The goal here is to clean up any copies of
//objects that 			were kept in the history.
//
//			Once a track entry object is destroyed, the user event
//that it 			tracks can no longer be undone or redone.
//-----------------------------------------------------------------------------
CTrackEntry::~CTrackEntry() {
  ...
  //
  // We kept a pointer to an object that was deleted from the world. We need to
  // delete the object, because the object's deletion can no longer be undone.
  //
  case ttDelete: {
    //
    // If this entry was undone, the object has been added back into the world,
    // so we should not delete the object.
    //
    if (!m_bUndone) {
...
//-----------------------------------------------------------------------------
// Purpose: Performs the undo by restoring the kept object to its original
// state. Input  : Opposite - Pointer to the opposite history track. If we are
// in the
//				undo history, it points to the redo history, and
//vice-versa.
//-----------------------------------------------------------------------------
void CTrackEntry::Undo(CHistory *Opposite) {
  ...
  case ttDelete: {
    //
    // First restore the deleted object's parent so that it is properly kept in
    // the opposite history track. The opposite history track sees this as a new
    ...
    //
    Opposite->GetDocument()->AddObjectToWorld(m_Delete.pDeleted,
                                              m_Delete.pKeptParent);
    break;
  }
...
//			undo entries have been handled so that objects are
//dealing with the 			correct data set when they calculate bounds, etc.
//-----------------------------------------------------------------------------
...
// Output :
//-----------------------------------------------------------------------------
CHistoryTrack::CHistoryTrack(CHistory *pParent,
                             const CMapObjectList *pSelected) {
  Parent = pParent;
...
//			Marks all entries in this track for autodestruction, so
//that when 			their destructor gets called, they free any object pointers that
//they 			hold.
//-----------------------------------------------------------------------------
