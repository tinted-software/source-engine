...
ON_COMMAND_EX_RANGE(CFaceEditSheet::id_SwitchModeStart,
                    CFaceEditSheet::id_SwitchModeEnd, OnSwitchMode)
...
CFaceEditMaterialPage::FaceAttributeInfo_t FaceAttributes[] = {
    ...
    // Contents.
    //
    {IDC_CONTENTS_AREAPORTAL, &CFaceEditMaterialPage::m_FaceContents,
     CONTENTS_AREAPORTAL},
    {IDC_CONTENTS_PLAYERCLIP, &CFaceEditMaterialPage::m_FaceContents,
     CONTENTS_PLAYERCLIP},
    {IDC_CONTENTS_MONSTERCLIP, &CFaceEditMaterialPage::m_FaceContents,
     CONTENTS_MONSTERCLIP},
    {IDC_CONTENTS_ORIGIN, &CFaceEditMaterialPage::m_FaceContents,
     CONTENTS_ORIGIN},
    {IDC_CONTENTS_DETAIL, &CFaceEditMaterialPage::m_FaceContents,
     CONTENTS_DETAIL},
    {IDC_CONTENTS_TRANSLUCENT, &CFaceEditMaterialPage::m_FaceContents,
     CONTENTS_TRANSLUCENT},
    {IDC_CONTENTS_LADDER, &CFaceEditMaterialPage::m_FaceContents,
     CONTENTS_LADDER},
...
void CFaceEditMaterialPage::Init(void) {
  ...
  ::PostMessage(pWnd->m_hWnd, UDM_SETRANGE, 0,
                MAKELONG(MAX_TEXTUREWIDTH, -MAX_TEXTUREWIDTH));

  pWnd = GetDlgItem(IDC_SPINSHIFTY);
  ::PostMessage(pWnd->m_hWnd, UDM_SETRANGE, 0,
                MAKELONG(MAX_TEXTUREHEIGHT, -MAX_TEXTUREHEIGHT));
...
//-----------------------------------------------------------------------------
// Purpose: Fetches a string value from a window and places it in an integer.
// The
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CFaceEditMaterialPage::ClickFace(CMapSolid *pSolid, int faceIndex, int cmd,
                                      int clickMode) {
  // get the face
...
//-----------------------------------------------------------------------------
void CFaceEditMaterialPage::CopyTCoordSystem(const CMapFace *pFrom,
                                             CMapFace *pTo) {
  ...
  // To find a point on the plane, we make a plane from the edge vector and find
  // the intersection between the three planes (without the third plane, there
  ...
  if (PlaneIntersection(VPlane(pFrom->plane.normal, pFrom->plane.dist),
    ...
    // Ok, in this case, the planes are parallel so we don't need to rotate
    // around the edge anyway!
    bRotate = false;
  }
  ...
  vOrigin = axis[0] * fShift[0] * pFrom->texture.scale[0] +
            axis[1] * fShift[1] * pFrom->texture.scale[1];
...
void CFaceEditMaterialPage::Apply(CMapFace *pOnlyFace, int flags) {
  ...
  for (i = 0; i < faceCount || pOnlyFace; i++) {
    ...
    if (flags & FACE_APPLY_CONTENTS_DATA) {
      ...
      //
      for (int nItem = 0;
           nItem < sizeof(FaceAttributes) / sizeof(FaceAttributes[0]);
           nItem++) {
        CButton *pButton =
            (CButton *)GetDlgItem(FaceAttributes[nItem].uControlID);
        if (pButton != NULL) {
...
void CFaceEditMaterialPage::UpdateDialogData(CMapFace *pOnlyFace) {
  ...
  for (int i = 0; i < faceCount || pOnlyFace; i++) {
    ...
    if (bFirst) {
      ...
      //
      // Get the face's orientation. This is used by Apply to make intelligent
      // decisions.
      //
      ...
      for (int nItem = 0;
           nItem < sizeof(FaceAttributes) / sizeof(FaceAttributes[0]);
           nItem++) {
        int nSet = ((*FaceAttributes[nItem].puAttribute &
                     FaceAttributes[nItem].uFlag) != 0);
        CButton *pButton =
            (CButton *)GetDlgItem(FaceAttributes[nItem].uControlID);
        if (pButton != NULL) {
      ...
      //
      // Update the checkbox state for the face attributes. If any of this
      // face's attributes are different from the current checkbox state, set
      // the checkbox to the undefined state.
      //
      m_FaceContents = t.q2contents;
      m_FaceSurface = t.q2surface;
  ...
  IntegerToSpin(nLightmapScale,
                (CSpinButtonCtrl *)GetDlgItem(IDC_SPIN_LIGHTMAP_SCALE));
...
BOOL CFaceEditMaterialPage::OnAlign(UINT uCmd) {
  ...
  for (int i = 0; i < faceCount; i++) {
    ...
    case IDC_ALIGN_WORLD: {
      pFace->InitializeTextureAxes(TEXTURE_ALIGN_WORLD,
                                   INIT_TEXTURE_AXES | INIT_TEXTURE_FORCE);
      break;
    }

    case IDC_ALIGN_FACE: {
      pFace->InitializeTextureAxes(TEXTURE_ALIGN_FACE,
                                   INIT_TEXTURE_AXES | INIT_TEXTURE_FORCE);
      break;
    }
    }
...
void CFaceEditMaterialPage::OnHideMask(void) {
  ...
  CMapDoc::GetActiveMapDoc()->UpdateAllViews(
      MAPVIEW_UPDATE_ONLY_3D | MAPVIEW_UPDATE_OBJECTS | MAPVIEW_UPDATE_COLOR);
}
...
BOOL CFaceEditMaterialPage::OnJustify(UINT uCmd) {
  ...
  // If multiple faces are selected, use the m_bTreatManyAsOneFace variable to
  // determine how to perform the justification.
  if (faceCount > 1) {
...
void CFaceEditMaterialPage::OnMode() {
  ...
  menu.AppendMenu(MF_STRING, CFaceEditSheet::ModeApplyAll,
  ...
  menu.TrackPopupMenu(TPM_LEFTALIGN | TPM_LEFTBUTTON, r.left, r.bottom, this,
                      NULL);
}
...
//			*pScrollBar -
//-----------------------------------------------------------------------------
void CFaceEditMaterialPage::OnVScroll(UINT nSBCode, UINT nPos,
...
//			pResult -
//-----------------------------------------------------------------------------
void CFaceEditMaterialPage::OnDeltaPosFloatSpin(NMHDR *pNMHDR,
                                                LRESULT *pResult) {
...
//			cy -
//-----------------------------------------------------------------------------
void CFaceEditMaterialPage::OnSize(UINT nType, int cx, int cy) { return; }
...
void CFaceEditMaterialPage::SelectTexture(LPCSTR pszTextureName) {
  ...
  if (nIndex != LB_ERR) {
    IEditorTexture *pTex =
        (IEditorTexture *)m_TextureList.GetItemDataPtr(nIndex);
