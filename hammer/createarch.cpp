...
// Output :
//-----------------------------------------------------------------------------
static CMapSolid *CreateSegment(float fZMin, float fZMax,
                                float fOuterPoints[][2],
                                float fInnerPoints[][2], int iStart, int iEnd,
                                BOOL bCreateSouthFace) {
  CMapFace Face;
  ...
  pSolid->InitializeTextureAxes(Options.GetTextureAlignment(),
...
// Output :
//-----------------------------------------------------------------------------
static CMapSolid *CreateSegment(float fStartOuterPoints[][3],
                                float fStartInnerPoints[][3],
                                float fEndOuterPoints[][3],
                                float fEndInnerPoints[][3], int iStart,
...
// Make a 2d arc
//-----------------------------------------------------------------------------
void MakeArcCenterRadius(float xCenter, float yCenter, float xrad, float yrad,
                         int npoints, float start_ang, float fArc,
                         float points[][2]) {
  int point;
...
void MakeArc(float x1, float y1, float x2, float y2, int npoints,
  ...
  MakeArcCenterRadius(xCenter, yCenter, xrad, yrad, npoints, start_ang, fArc,
                      points);
}
...
CMapClass *CreateArch(BoundBox *pBox, float fStartAngle, int iSides, float fArc,
  ...
  // create outer points
  //
  MakeArc(pBox->bmins[AXIS_X], pBox->bmins[AXIS_Y], pBox->bmaxs[AXIS_X],
          pBox->bmaxs[AXIS_Y], iSides, fStartAngle, fArc, fOuterPoints);

  //
  // create inner points
  //
  MakeArc(pBox->bmins[AXIS_X] + iWallWidth, pBox->bmins[AXIS_Y] + iWallWidth,
          pBox->bmaxs[AXIS_X] - iWallWidth, pBox->bmaxs[AXIS_Y] - iWallWidth,
  ...
  for (int i = 0; i < iSides; i++) {
    ...
    CMapSolid *pSolid = CreateSegment(fMinZ, fMaxZ, fOuterPoints, fInnerPoints,
  ...
  if (Options.general.bStretchArches) {
    ...
    if (boundsize[AXIS_X] > objsize[AXIS_X] ||
      ...
      scale[AXIS_Z] =
          1.0f; // xxxYWB scaling by 0 causes veneers, so I changed to 1.0
      Vector center;
      pBox->GetBoundsCenter(center);
...
CMapClass *CreateTorus(BoundBox *pBox, float fStartAngle, int iSides,
                       float fArc, int iWallWidth, float flCrossSectionalRadius,
                       float fRotationStartAngle, int iRotationSides,
                       float fRotationArc, int iAddHeight, BOOL bPreview) {
  ...
  MakeArcCenterRadius(0.0f, 0.0f, flCrossSectionalRadius + iWallWidth,
                      flCrossSectionalRadius + iWallWidth, iSides, fStartAngle,
                      fArc, fOuterPoints);

  BOOL bCreateSouthFace = TRUE;
  if (flCrossSectionalRadius != 0.0f) {
    // create inner points (unrotated)
    MakeArcCenterRadius(0.0f, 0.0f, flCrossSectionalRadius,
                        flCrossSectionalRadius, iSides, fStartAngle, fArc,
                        fInnerPoints);
  } else {
    for (int i = 0; i < iSides; i++) {
    ...
    do {
      ...
      if ((i != 0) && (jPrevPoint != -1)) {
        CMapSolid *pSolid = CreateSegment(
            *pStartOuterPoints, *pStartInnerPoints, *pEndOuterPoints,
