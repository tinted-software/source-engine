...
//-----------------------------------------------------------------------------
// Purpose: Defines the interface to a set of textures of a given texture
// format.
//			The textures are stored as an index into the global
//array of textures.
//-----------------------------------------------------------------------------
class CTextureGroup {
public:
...
protected:
  ...
  CUtlDict<int, int>
      m_TextureNameMap; // Maps the texture name to an index into m_Textures
                        // (the key is IEditorTexture::GetName).
...
//
// When the user switches game configs, all the textures and materials are
// switched. This structure holds all the context necessary to accomplish this.
//
struct TextureContext_t {
  CGameConfig
      *pConfig; // The game config that this texture context corresponds to.
  CTextureGroup *pAllGroup;
  TextureGroupList_t Groups;
  EditorTextureList_t
      MRU; // List of Most Recently Used textures, first is the most recent.
  EditorTextureList_t Dummies; // List of Dummy textures - textures that were
                               // created to hold the place of missing textures.
};
...
public:
  ...
  bool FindGraphicsFile(GRAPHICSFILESTRUCT *pFileInfo, DWORD id,
                        int *piIndex = NULL);
  ...
  //
  // Exposes a list of active textures based on the currently active texture
  // group.
  //
  inline int GetActiveTextureCount(void) const;
  inline IEditorTexture *GetActiveTexture(int nIndex) const;
  IEditorTexture *EnumActiveTextures(int *piIndex,
                                     TEXTUREFORMAT eDesiredFormat) const;
  IEditorTexture *FindActiveTexture(LPCSTR pszName, int *piIndex = NULL,
                                    BOOL bDummy = TRUE);
  ...
  //
  // Holds a list of placeholder textures used when a map refers to missing
  // textures.
  //
  ...
  // IMaterialEnumerator interface, Used to add all the world materials into the
  // material list.
...
protected:
  // CMaterialFileChangeWatcher stuff - watches for changes to VMTs or VTFs and
  // handles them.

  enum EFileType { k_eFileTypeVMT, k_eFileTypeVTF };
  ...
  static bool GetFileTypeFromFilename(const char *pFilename,
                                      CTextureSystem::EFileType *pFileType);
  ...
  void LoadGraphicsFileWAD3(GRAPHICSFILESTRUCT *pFile, int fd,
  ...
  TextureContext_t
      *m_pActiveContext; // Points to the active entry in m_TextureContexts.
  CTextureGroup
      *m_pActiveGroup; // Points to the active entry in m_TextureContexts.
...
//			nIndex -
//-----------------------------------------------------------------------------
void CTextureSystem::FilesGetInfo(GRAPHICSFILESTRUCT *pFileInfo,
                                  int nIndex) const {
  if (pFileInfo != NULL) {
