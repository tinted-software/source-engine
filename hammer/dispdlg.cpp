...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CDispCreateDlg::OnVScroll(UINT nSBCode, UINT nPos,
...
BOOL CDispPaintDistDlg::OnInitDialog(void) {
  ...
  if (!bInit) {
    ...
    SetWindowPos(&wndTop, m_DialogPosRect.left, m_DialogPosRect.top,
                 m_DialogPosRect.Width(), m_DialogPosRect.Height(),
                 SWP_NOZORDER);
  }
...
void CDispPaintDistDlg::InitRadius(void) {
  ...
  m_sliderRadius.SetRange(DISPPAINT_SPATIALRADIUS_MIN,
                          DISPPAINT_SPATIALRADIUS_MAX);
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CDispPaintDistDlg::FilterComboBoxBrushGeo(unsigned int nEffect,
                                               bool bInit) {
  //
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CDispPaintDistDlg::UpdateSliderDistance(float flDistance,
                                             bool bForceInit) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CDispPaintDistDlg::UpdateEditBoxDistance(float flDistance,
                                              bool bForceInit) {
  if ((flDistance != m_flPrevDistance) || bForceInit) {
    CString strDistance;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CDispPaintDistDlg::OnHScroll(UINT nSBCode, UINT nPos,
                                  CScrollBar *pScrollBar) {
  // Get the displacement tool.
  CToolDisplace *pTool = GetDisplacementTool();
  if (pTool) {
    // Get the distance slider control.
    CSliderCtrl *pDistSlider =
        (CSliderCtrl *)GetDlgItem(ID_DISP_PAINT_DIST_SLIDER_DISTANCE);
    if (pDistSlider) {
    ...
    CSliderCtrl *pRadiusSlider =
        (CSliderCtrl *)GetDlgItem(ID_DISP_PAINT_DIST_SLIDER_RADIUS);
    if (pRadiusSlider) {
      // Get the slider position.
...
BOOL CPaintSculptDlg::OnInitDialog() {
  ...
  m_PushOptions->SetWindowPos(NULL, 10, OptionsLoc.top - ThisLoc.top - 20, 0, 0,
                              SWP_NOSIZE | SWP_SHOWWINDOW);
  m_CarveOptions->SetWindowPos(NULL, 10, OptionsLoc.top - ThisLoc.top - 20, 0,
                               0, SWP_NOSIZE | SWP_SHOWWINDOW);
  //	m_ProjectOptions->SetWindowPos( NULL, 10, OptionsLoc.top - ThisLoc.top -
  //20, 0, 0, SWP_NOSIZE | SWP_SHOWWINDOW );
...
void CPaintSculptDlg::OnLButtonUp(UINT nFlags, CPoint point) {
  CToolDisplace *pDispTool = GetDisplacementTool();
  CSculptPainter *painter =
      dynamic_cast<CSculptPainter *>(pDispTool->GetSculptPainter());
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CDispPaintDataDlg::FilterComboBoxBrushData(unsigned int uiEffect,
                                                bool bInit) {
  //
  // remove all the old combo box data
  //
  ...
  if (pDispTool) {
    ...
    if (pFilterMgr) {
      ...
      // for each filter - add its icon to the icon combo box
      //
      for (int ndxFilter = 0; ndxFilter < pFilterMgr->GetFilterCount();
           ndxFilter++) {
        // get the current filter
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CDispPaintDataDlg::OnHScroll(UINT nSBCode, UINT nPos,
                                  CScrollBar *pScrollBar) {
  // get the displacement tool
  CToolDisplace *pDispTool = GetDisplacementTool();
  if (pDispTool) {
    ...
    CSliderCtrl *pSlider =
        (CSliderCtrl *)GetDlgItem(ID_DISP_PAINT_DATA_SLIDER_VALUE);
    if (pSlider) {
      // get the slider position
