...
//			selected - if it is selected or not ( selected appears
//on top ) 			faceSelectionState - if the face is individual selected
//-----------------------------------------------------------------------------
void CMapFace::AddFaceToQueue(CMapFace *pMapFace, IEditorTexture *pTexture,
...
void CMapFace::RenderOpaqueFaces(CRender3D *pRender) {
  ...
  MapFaceRender_t **ppMapFaces = (MapFaceRender_t **)_alloca(
      g_CurrentOpaqueFaces->Count() * sizeof(MapFaceRender_t *));
  int nFaceCount = 0;

  int nLastRenderMode = RENDER_MODE_NONE;
  IEditorTexture *pLastTexture = NULL;

  for (int i = g_CurrentOpaqueFaces->FirstInorder();
       i != g_CurrentOpaqueFaces->InvalidIndex();
