...
public:
  ...
  //-----------------------------------------------------------------------------
  virtual void GetRenderColor(unsigned char &red, unsigned char &green,
                              unsigned char &blue) {
    red = r;
    green = g;
    blue = b;
  ...
  // Input  : pParent -
  //-----------------------------------------------------------------------------
  virtual void SetParent(CMapAtom *pParent) { m_pParent = pParent; }

  //-----------------------------------------------------------------------------
  // Purpose: Returns the parent of this CMapAtom.
  // Output : Parent pointer, NULL if this object has no parent.
  //-----------------------------------------------------------------------------
  virtual CMapAtom *GetParent(void) const { return (m_pParent); }

  //-----------------------------------------------------------------------------
  // Purpose: Preloads any rendering info (textures, etc.). Called once per
  // object
  //			from the renderer's Initialize function.
  // Input  : pRender - Pointer to the 3D rendering interface.
  //			bNewContext - True if the renderer pointed to by pRender
  //is a new 				rendering context, false if not.
  //-----------------------------------------------------------------------------
  ...
  // Input  : matrix -
  // Output : returns true if a new matrix is returned, false if the new matrix
  // is bad
  //-----------------------------------------------------------------------------
  ...
  //-----------------------------------------------------------------------------
  // Override this for helpers. This indicates whether a helper provides a
  // visual representation for the entity that it belongs to. Entities with no
  ...
  //-----------------------------------------------------------------------------
  virtual bool ShouldRenderLast(void) { return (false); }
...
protected:
  ...
  SelectionState_t
      m_eSelectionState; // The current selection state of this object.
