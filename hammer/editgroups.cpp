...
void CEditGroups::OnColor(void) {
  ...
  if (pGroup != NULL) {
    ...
    if (dlg.DoModal() == IDOK) {
      // change group color
      pGroup->SetColor(GetRValue(dlg.m_cc.rgbResult),
      ...
      GetActiveWorld()->EnumChildren(ENUMMAPCHILDRENPROC(UpdateObjectColor),
                                     DWORD(pGroup));
...
//			the currently selected group with the new edit control
//contents.
//-----------------------------------------------------------------------------
...
//			Deletes the selected visgroup and removes all references
//to it.
//-----------------------------------------------------------------------------
...
//			text controls with the name and colot of the selected
//visgroup.
//-----------------------------------------------------------------------------
...
//			visgroup dialog bar in case any visgroup changes were
//made.
//-----------------------------------------------------------------------------
