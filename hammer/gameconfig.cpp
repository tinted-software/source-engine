...
//-----------------------------------------------------------------------------
// Purpose: Constructor. Maintains a static	counter uniquely identifying
// each
//			game configuration.
//-----------------------------------------------------------------------------
...
bool CGameConfig::Load(KeyValues *pkv) {
  ...
  m_fDefaultTextureScale =
      pkvHammer->GetFloat("DefaultTextureScale", DEFAULT_TEXTURE_SCALE);
  if (m_fDefaultTextureScale == 0) {
    m_fDefaultTextureScale = DEFAULT_TEXTURE_SCALE;
  }

  m_nDefaultLightmapScale =
      pkvHammer->GetInt("DefaultLightmapScale", DEFAULT_LIGHTMAP_SCALE);

  Q_strncpy(szExecutable, pkvHammer->GetString("GameExe"),
            sizeof(szExecutable));
  Q_strncpy(szDefaultSolid, pkvHammer->GetString("DefaultSolidEntity"),
            sizeof(szDefaultSolid));
  Q_strncpy(szDefaultPoint, pkvHammer->GetString("DefaultPointEntity"),
            sizeof(szDefaultPoint));
  ...
  Q_strncpy(m_szGameExeDir, pkvHammer->GetString("GameExeDir"),
            sizeof(m_szGameExeDir));
  ...
  for (int i = 0; i < m_MaterialExcludeCount; i++) {
    ...
    Q_strncpy(m_MaterialExclusions[index].szDirectory,
              pkvHammer->GetString(szExcludeDir),
...
void CGameConfig::CopyFrom(CGameConfig *pConfig) {
  ...
  for (int i = 0; i < m_MaterialExcludeCount; i++) {
    strcpy(m_MaterialExclusions[i].szDirectory,
           pConfig->m_MaterialExclusions[i].szDirectory);
  }
}
...
void CGameConfig::LoadGDFiles(void) {
  ...
  for (int i = 0; i < CMapDoc::GetDocumentCount(); i++) {
    ...
    if (pDoc->GetGame() == this) {
      ...
      pWorld->EnumChildren((ENUMMAPCHILDRENPROC)UpdateClassPointer, (DWORD)&GD,
...
//			szStartDir - the folder to start searching from, towards
//the root. 			szFoundPath - receives the full path of the FOLDER where szFile was
//found.
// Output : Returns true if the file was found, false if not. If the file was
//			found the full path (not including the filename) is
//returned in szFoundPath.
//-----------------------------------------------------------------------------
bool FindFileInTree(const char *szFile, const char *szStartDir,
                    char *szFoundPath) {
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool FindSteamUserDir(const char *szAppDir, const char *szSteamDir,
                      char *szSteamUserDir) {
  if ((szAppDir == NULL) || (szSteamDir == NULL) || (szSteamUserDir == NULL)) {
    return false;
  }

  // If the szAppDir was run from within the steam tree, we should be able to
  // find the steam user dir.
  int nSteamDirLen = strlen(szSteamDir);
  if (!Q_strnicmp(szAppDir, szSteamDir, nSteamDirLen) &&
      (szAppDir[nSteamDirLen] == '\\')) {
