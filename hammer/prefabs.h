...
public:
  ...
  // set info:
  void SetName(LPCTSTR pszName) { strcpy(szName, pszName); }
  void SetNotes(LPCTSTR pszNotes) { strcpy(szNotes, pszNotes); }
  ...
  typedef enum { pftUnknown, pftRMF, pftMAP, pftScript } pfiletype_t;
...
public:
  ...
  static CPrefabLibrary *EnumLibraries(POSITION &p,
                                       LibraryType_t eType = LibType_None);
...
//			.ol files to represent prefab libraries, Half-Life 2
//uses a folder 			of VMF files.
//-----------------------------------------------------------------------------
bool CPrefabLibrary::IsType(LibraryType_t eType) { return (m_eType == eType); }
