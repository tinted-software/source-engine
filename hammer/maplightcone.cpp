...
//-----------------------------------------------------------------------------
CMapClass *CMapLightCone::Create(CHelperInfo *pHelperInfo,
                                 CMapEntity *pParent) {
...
void CMapLightCone::BuildCone(void) {
  ...
  //
  if ((m_fQuadraticAttn == 0) && (m_fLinearAttn == 0) &&
      (m_fConstantAttn == 0)) {
    m_fConstantAttn = 1;
  }

  //
  // Solve for the lighting scale factor by which the brightness will be
  // multiplied.
  //
  float fScaleFactor =
      m_fQuadraticAttn * 10000 + m_fLinearAttn * 100 + m_fConstantAttn;
  if (fScaleFactor == 0) {
    return;
  }
  ...
  //
  float fOffsetDist = 0;
  // Constant attenuation factor doesn't actually offset the cone yet. If it
  // does, uncomment this:
  // SolveQuadratic(fOffsetDist, 0, m_fQuadraticAttn, m_fLinearAttn,
  // -m_fConstantAttn);

  float fZoneDist[NUM_LIGHTCONE_ZONES];
  memset(fZoneDist, 0, sizeof(fZoneDist));
  fZoneDist[0] = 0;
  SolveQuadratic(fZoneDist[1], 0.25 * fScaleFactor, m_fQuadraticAttn,
                 m_fLinearAttn, m_fConstantAttn);
  SolveQuadratic(fZoneDist[2], fScaleFactor, m_fQuadraticAttn, m_fLinearAttn,
                 m_fConstantAttn);
  SolveQuadratic(fZoneDist[3], 4 * fScaleFactor, m_fQuadraticAttn,
                 m_fLinearAttn, m_fConstantAttn);
  SolveQuadratic(fZoneDist[4], Options.view3d.fLightConeLength * fScaleFactor,
                 m_fQuadraticAttn, m_fLinearAttn, m_fConstantAttn);
  ...
  if (m_fOuterConeAngle < 90) {
    ...
    for (int i = 0; i < NUM_LIGHTCONE_ZONES; i++) {
      fZoneRadius[i] =
          (fOffsetDist + fZoneDist[i]) * tan(DEG2RAD(m_fOuterConeAngle));
    }
  ...
  for (float fPitch = fPitchStepSize;
       fPitch < m_fOuterConeAngle + fPitchStepSize; fPitch += fPitchStepSize) {
    float fSin0 = 0;
    float fCos0 = 1;
    ...
    // float fFocusRadius1 = sin(DEG2RAD(fPitch)) * fZoneRadius[1] / sin(DEG_RAD
    // * m_fConeAngle);
    ...
    for (int fAngle = fStepSize; fAngle <= 361; fAngle += fStepSize) {
      ...
      CMapFace *pFace = new CMapFace;
      pFace->SetRenderColor(r * fIllumination, g * fIllumination,
                            b * fIllumination);
      pFace->SetRenderAlpha(180);
      pFace->CreateFace(Points, nPoints);
      pFace->RenderUnlit(true);
...
// Output : CMapClass
//-----------------------------------------------------------------------------
CMapClass *CMapLightCone::CopyFrom(CMapClass *pObject,
  ...
  Q_strncpy(m_szColorKeyName, pFrom->m_szColorKeyName,
            sizeof(m_szColorKeyName));
  Q_strncpy(m_szInnerConeKeyName, pFrom->m_szInnerConeKeyName,
            sizeof(m_szInnerConeKeyName));
  Q_strncpy(m_szOuterConeKeyName, pFrom->m_szOuterConeKeyName,
            sizeof(m_szOuterConeKeyName));
...
void CMapLightCone::OnParentKeyChanged(const char *szKey, const char *szValue) {
  ...
  } else if (!stricmp(szKey, m_szInnerConeKeyName) ||
             !stricmp(szKey, m_szOuterConeKeyName)) {
...
//-----------------------------------------------------------------------------
bool CMapLightCone::SolveQuadratic(float &x, float y, float A, float B,
                                   float C) {
  C -= y;
