...
public:
  class ICallbacks {
  public:
    // Note: this is called if the file is added, removed, or modified. It's up
    // to the app to figure out what it wants to do with the change.
    virtual void OnFileChange(const char *pRelativeFilename,
                              const char *pFullFilename) = 0;
  };
  ...
  // pSearchPathBase would be like "c:\valve\hl2" and pDirName would be like
  // "materials".
  bool AddDirectory(const char *pSearchPathBase, const char *pDirName,
                    bool bRecursive);
  void Term();

  // Call this periodically to update. It'll call your ICallbacks functions for
  // anything it finds. Returns the number of updates it got.
  int Update();
...
private:
  ...
  void SendNotification(CFileChangeWatcher::CDirWatch *pDirWatch,
