...
//-----------------------------------------------------------------------------
void CToolSweptPlayerHull::Attach(CMapSweptPlayerHull *pSweptHull,
                                  int nPointIndex) {
...
//-----------------------------------------------------------------------------
bool CToolSweptPlayerHull::OnLMouseDown2D(CMapView2D *pView, UINT nFlags,
...
// Output : Returns true if the message was handled, false if not.
//-----------------------------------------------------------------------------
bool CToolSweptPlayerHull::OnLMouseUp2D(CMapView2D *pView, UINT nFlags,
                                        const Vector2D &vPoint) {
...
// Output : Returns true if the message was handled, false if not.
//-----------------------------------------------------------------------------
bool CToolSweptPlayerHull::OnMouseMove2D(CMapView2D *pView, UINT nFlags,
                                         const Vector2D &vPoint) {
  //
  ...
  //
  char szBuf[128];
  sprintf(szBuf, " (%.0f %.0f %0.f) ", vecPos[m_nPointIndex][0],
          vecPos[m_nPointIndex][1], vecPos[m_nPointIndex][2]);
...
void CToolSweptPlayerHull::RenderTool2D(CRender2D *pRender) {
  SelectionState_t eState =
      m_pSweptHull->SetSelectionState(SELECT_MODIFY, m_nPointIndex);
