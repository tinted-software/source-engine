...
struct LoadVisGroupData_t {
  ...
  CVisGroup *pParent; // The parent visgroup of the visgroup being loaded, NULL
                      // if this is a root-level group.
};
...
//			kept in the VMF, whereas now it's purely a function of
//the visibility 			of the member objects. So when loading old maps, we skip the
//step 			in which we generate the visgroup state from the objects.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
ChunkFileResult_t CVisGroup::LoadKeyCallback(const char *szKey,
                                             const char *szValue,
                                             CVisGroup *pGroup) {
...
ChunkFileResult_t CVisGroup::LoadVMF(CChunkFile *pFile, CMapDoc *pDoc) {
  ...
  Handlers.AddHandler("visgroup", (ChunkHandler_t)LoadVisGroupCallback,
                      &LoadData);
...
//-----------------------------------------------------------------------------
ChunkFileResult_t
CVisGroup::LoadVisGroupCallback(CChunkFile *pFile,
...
// Output : ChunkFileResult_t
//-----------------------------------------------------------------------------
ChunkFileResult_t CVisGroup::LoadVisGroupsCallback(CChunkFile *pFile,
                                                   CMapDoc *pDoc) {
...
//-----------------------------------------------------------------------------
VisGroupState_t CVisGroup::GetVisible(void) { return m_eVisible; }

//-----------------------------------------------------------------------------
// Purpose: Returns whether or not visgroup visibility is being overridden by
//			the "Show All" button.
//-----------------------------------------------------------------------------
bool CVisGroup::IsShowAllActive(void) { return s_bShowAll; }
...
ChunkFileResult_t CVisGroup::SaveVMF(CChunkFile *pFile, CSaveInfo *pSaveInfo) {
  ...
  if (eResult == ChunkFile_Ok) {
    ...
    eResult =
        pFile->WriteKeyValueColor("color", rgbColor.r, rgbColor.g, rgbColor.b);
  }
...
//-----------------------------------------------------------------------------
void CVisGroup::ShowAllVisGroups(bool bShow) { s_bShowAll = bShow; }
...
bool CVisGroup::IsAutoVisGroup() { return m_bIsAuto; }

void CVisGroup::SetAuto(bool bAuto) { m_bIsAuto = bAuto; }
