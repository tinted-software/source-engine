...
CRender::CRender(void) {
  ...
  vgui::IScheme *pScheme =
      vgui::scheme()->GetIScheme(vgui::scheme()->GetDefaultScheme());
  if (pScheme) {
    ...
    if (!s_bOnce) {
      s_bOnce = true;
      MessageBox(NULL,
                 "Failed to load the default scheme file. The map views may be "
  ...
  PushInstanceData(NULL, Vector(0.0f, 0.0f, 0.0f),
                   QAngle(0.0f, 0.0f, 0.0f)); // always add a default state
...
// Output : none
//-----------------------------------------------------------------------------
void CRender::PushInstanceData(CMapInstance *pInstanceClass,
                               Vector &InstanceOrigin, QAngle &InstanceAngles) {
  ...
  AngleMatrix(InstanceState.m_InstanceAngles, InstanceState.m_InstanceOrigin,
              Instance3x4Matrix);
  ...
  //	RotateInstanceVector( ( Vector )InstanceState.m_InstanceAngles,
  //m_CurrentInstanceState.m_InstanceAngles );		no one uses this right
  //now   make sure to store it in the same fashion as vecTransformedOrigin

  if (m_InstanceState.Count() > 0) { // first push is just a default state
    ...
    if (m_CurrentInstanceState.m_pTopInstanceClass == NULL) {
      ...
      InstanceState.m_pTopInstanceClass =
          m_CurrentInstanceState.m_pTopInstanceClass;
    }
    if (pInstanceClass->IsSelected() || m_InstanceSelectionDepth) {
      m_InstanceSelectionDepth++;
    }
    InstanceState.m_InstanceMatrix = m_CurrentInstanceState.m_InstanceMatrix *
                                     InstanceState.m_InstanceMatrix;
  ...
  } else if (pInstanceClass->IsEditable() ||
             CMapDoc::GetActiveMapDoc()->GetShowInstance() ==
                 INSTANCES_SHOW_NORMAL) {
    PushInstanceRendering(INSTANCE_STATE_OFF);
  } else {
    PushInstanceRendering(GetInstanceClass()->IsSelected()
                              ? INSTANCE_STACE_SELECTED
                              : INSTANCE_STATE_ON);
  }
}
...
void CRender::PopInstanceData(void) {
  ...
  if (m_InstanceState.Count() > 1) { // first push is just a default state
    m_bInstanceRendering = true;
  } else {
    m_bInstanceRendering = false;
  }
...
//-----------------------------------------------------------------------------
// Purpose: this function will draw the various alpha color 2d rectangles for
// the instance states
//-----------------------------------------------------------------------------
void CRender::DrawInstanceStencil(void) {
  ...
  if (m_nNumInstancesRendered > 0) {
    ...
    pRenderContext->SetStencilState(m_ShaderStencilState);
#endif // STENCIL_AS_CALLS
    DrawFilledRect(Vector2D(0.0f, 0.0f), Vector2D(width, height),
                   (byte *)&InstanceColoring, false);
...
void CRender::DrawText(const char *text, int x, int y, int nFlags) {
  ...
  bool bJustifyText =
      nFlags & (TEXT_JUSTIFY_LEFT | TEXT_JUSTIFY_TOP |
                TEXT_JUSTIFY_HORZ_CENTER | TEXT_JUSTIFY_VERT_CENTER);
...
//-----------------------------------------------------------------------------
void CRender::DrawText(const char *text, const Vector2D &vPos, int nOffsetX,
                       int nOffsetY, int nFlags) {
...
//-----------------------------------------------------------------------------
void CRender::SetTextColor(unsigned char r, unsigned char g, unsigned char b,
                           unsigned char a) {
...
void CRender::SetHandleColor(unsigned char r, unsigned char g,
                             unsigned char b) {
...
void CRender::EndRenderFrame() {
  ...
  CMatRenderContextPtr pRenderContext(MaterialSystemInterface());
  pRenderContext->SetIntRenderingParameter(INT_RENDERPARM_ENABLE_FIXED_LIGHTING,
                                           0);
...
bool CRender::IsInLocalTransformMode() { return m_bIsLocalTransform; }
...
void CRender::DrawCircle(const Vector &vCenter, const Vector &vNormal,
...
void CRender::DrawCircle(Vector2D &vCenter, float fRadius, int nSegments,
...
void CRender::DrawPlane(const Vector &p0, const Vector &p1, const Vector &p2,
                        const Vector &p3, bool bFill) {
  if (bFill) {
...
void CRender::DrawFilledRect(Vector2D &ul, Vector2D &lr, unsigned char *pColor,
                             bool bBorder) {
...
CCamera *CRender::GetCamera() { return m_pView->GetCamera(); }
...
bool CRender::SetView(CMapView *pView) {
  ...
  if (!GetRequiredMaterial("editor/wireframe", m_pWireframe[0]) ||
      ...
      !GetRequiredMaterial("editor/translucentflatdecal",
                           m_pTranslucentFlat[1]) ||
...
bool CRender::IsActiveView() { return m_pView->IsActive(); }
...
void CRender::GetDrawColor(Color &color) { color = m_DrawColor; }
...
void CRender::DrawArrow(Vector const &vStart, Vector const &vEnd) { Assert(0); }
...
void CRender::DrawModel(DrawModelInfo_t *pInfo, matrix3x4_t *pBoneToWorld,
                        const Vector &vOrigin, float fAlpha, bool bWireFrame) {
  ...
  g_pStudioRender->SetEyeViewTarget(pInfo->m_pStudioHdr, pInfo->m_Body,
                                    viewOrigin);

  g_pStudioRender->DrawModel(NULL, *pInfo, pBoneToWorld, NULL, NULL, vOrigin,
...
void CRender::DrawCollisionModel(MDLHandle_t mdlHandle,
  ...
  Vector *outVerts;
  int vertCount =
      g_pPhysicsCollision->CreateDebugMesh(pCollide->solids[0], &outVerts);
...
void CRender::DrawSphere(const Vector &vCenter, int nRadius) { Assert(0); }
...
void CRender::DrawHandles(int nPoints, const Vector *Points) { Assert(0); }
...
void CRender::SetRenderMode(EditorRenderMode_t eRenderMode, bool bForce) {
  ...
  case RENDER_MODE_LIGHT_PREVIEW_RAYTRACED:
    if (m_pBoundMaterial) {
      if (m_pBoundMaterial->GetPropertyFlag(
  ...
  if (eRenderMode == RENDER_MODE_TEXTURED_SHADED)
    pRenderContext->SetIntRenderingParameter(
        INT_RENDERPARM_ENABLE_FIXED_LIGHTING, 1);

  if ((eRenderMode == RENDER_MODE_LIGHT_PREVIEW2) ||
      (eRenderMode == RENDER_MODE_LIGHT_PREVIEW_RAYTRACED))
    pRenderContext->SetIntRenderingParameter(
        INT_RENDERPARM_ENABLE_FIXED_LIGHTING, 2);

  m_pMesh =
      pRenderContext->GetDynamicMesh(true, NULL, NULL, m_pCurrentMaterial);
