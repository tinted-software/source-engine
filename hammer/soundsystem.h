...
public:
  ...
  bool PlayScene(
      const char *pFileName); // Play the first sound in the specified scene.
...
private:
  ...
  void AddSoundToList(SoundType_t type, const char *pSoundName,
                      const char *pActualFile, const char *pSourceFile);
  ...
  void BuildFileListInDirectory(char const *pDirectoryName, const char *pExt,
                                SoundType_t soundType);

  // Gamesounds may have macros embedded in them
  void AddGameSoundToList(const char *pGameSound, char const *pFileName,
