...
//-----------------------------------------------------------------------------
GDinputvariable *
Selection3D::ChooseEyedropperVar(CMapView *pView,
  ...
  CPoint point;
  GetCursorPos(&point);
  int nID = menu.TrackPopupMenu(TPM_LEFTALIGN | TPM_LEFTBUTTON | TPM_NONOTIFY |
                                    TPM_RETURNCMD,
                                point.x, point.y, NULL, NULL);
  if (nID == 0) {
    return NULL;
  }
...
//			in this obscure place, away from all the other 2D
//rendering code.
...
bool Selection3D::OnContextMenu2D(CMapView2D *pView, UINT nFlags,
  ...
  if (!IsEmpty() && !IsBoxSelecting()) {
    if (HitTest(pView, vPoint, false)) {
      ...
      menuSelection.TrackPopupMenu(TPM_LEFTBUTTON | TPM_RIGHTBUTTON |
                                       TPM_LEFTALIGN,
...
bool Selection3D::OnMouseMove2D(CMapView2D *pView, UINT nFlags,
  ...
  //
    ...
    if (IsEditable && !bCtrl &&
        HitTest(pView, m_vMouseStart[MOUSE_LEFT], true)) {
  ...
  if (!m_pSelection->IsEmpty()) {
    if (m_pSelection->GetCount() == 1) {
      ...
      CManifestInstance *pManifestInstance =
          dynamic_cast<CManifestInstance *>(pObject);
