...
//			iSelectPage -
//-----------------------------------------------------------------------------
CObjectProperties::CObjectProperties(UINT nIDCaption, CWnd *pParentWnd,
                                     UINT iSelectPage)
...
//			iSelectPage -
//-----------------------------------------------------------------------------
CObjectProperties::CObjectProperties(LPCTSTR pszCaption, CWnd *pParentWnd,
                                     UINT iSelectPage)
...
//			Not all will be used depending on the types of objects
//being edited.
//-----------------------------------------------------------------------------
...
//			pType -
//-----------------------------------------------------------------------------
PVOID CObjectProperties::GetEditObjectFromMapObject(CMapClass *pobj,
                                                    CRuntimeClass *pType) {
  // VPROF_BUDGET( "CObjectProperties::GetEditObjectFromMapObject", "Object
  // Properties" );
...
void CObjectProperties::CopyDataToEditObjects(CMapClass *pobj) {
  // VPROF_BUDGET( "CObjectProperties::CopyDataToEditObjects", "Object
  // Properties" );
...
void CObjectProperties::SetOutputButtonState(int nState) {
  // VPROF_BUDGET( "CObjectProperties::SetOutputButtonState", "Object
  // Properties" );
...
void CObjectProperties::UpdateOutputButton(void) {
  ...
  FOR_EACH_OBJ(m_DstObjects, pos) {
    ...
    if ((pObject != NULL) && (pObject->IsMapClass(MAPCLASS_TYPE(CMapEntity)))) {
      ...
      int nStatus = CEntityConnection::ValidateOutputConnections(
          pEntity, true, bIgnoreHiddenTargets);
...
void CObjectProperties::CreateButtons(void) {
  ...
  CButton *pOKButton = reinterpret_cast<CButton *>(GetDlgItem(IDOK));
  pOKButton->EnableWindow();
  // Dont show the window, just make it active to forward <enter> -> IDOK ->
  // OnApply

  // Grab and enable & show the hidden Apply button too
  CButton *pApplyButton = reinterpret_cast<CButton *>(GetDlgItem(ID_APPLY_NOW));
  pApplyButton->SetButtonStyle(pApplyButton->GetButtonStyle() |
                               BS_DEFPUSHBUTTON);
  ...
  m_pInputButton->Create(
      _T("My button"), WS_CHILD | WS_VISIBLE | BS_ICON | BS_FLAT,
      CRect(6, rect.bottom - 34, 38, rect.bottom - 2), this, IDI_INPUT);

  m_pOutputButton = new CButton;
  m_pOutputButton->Create(
      _T("My button"), WS_CHILD | WS_VISIBLE | BS_ICON | BS_FLAT,
      CRect(40, rect.bottom - 34, 72, rect.bottom - 2), this, IDI_OUTPUT);

  m_pInstanceButton = new CButton;
  m_pInstanceButton->Create(_T( "Edit Instance" ),
                            WS_CHILD | WS_VISIBLE | BS_TEXT,
                            CRect(6, rect.bottom - 28, 140, rect.bottom - 4),
                            this, IDD_EDIT_INSTANCE);
}

//-----------------------------------------------------------------------------
// Purpose: Returns the appropriate page layout for the current object list.
//-----------------------------------------------------------------------------
void CObjectProperties::GetTabsForLayout(LayoutType_t eLayoutType,
                                         bool &bEntity, bool &bGroups,
                                         bool &bFlags, bool &bModel) {
...
BOOL CObjectProperties::SetupPages(void) {
  ...
  //
  // If the layout has not changed, we're done. All the pages are already set
  // up.
  //
  ...
  struct {
    ...
    bool m_bWantVisible;
    CObjectPage *m_pPage;
  } pages[] = {{false, bEntity, m_pEntity}, {false, bEntity, m_pOutput},
               {false, bEntity, m_pInput},  {false, bModel, m_pModel},
               {false, bFlags, m_pFlags},   {false, bGroups, m_pGroups}};
  ...
  // We're about to add pages, but it'll only add them to the right of what's
  // already there, so we must get rid of anything to the right of our leftmost
  // addition.
  for (int i = 0; i < ARRAYSIZE(pages); i++) {
    if (!pages[i].m_bIsVisible && pages[i].m_bWantVisible) {
...
void CObjectProperties::LoadDataForPages(int iPage) {
  ...
  FOR_EACH_OBJ(m_DstObjects, pos) {
    ...
    if (iPage != -1) {
      ...
      void *pObject = GetEditObjectFromMapObject(
          pobj, m_ppPages[iPage]->GetEditObjectRuntimeClass());
      if (pObject != NULL) {
      ...
      for (int i = 0; i < m_nPages; i++) {
        ...
        void *pObject = GetEditObjectFromMapObject(
            pobj, m_ppPages[i]->GetEditObjectRuntimeClass());
        if (pObject != NULL) {
...
void CObjectProperties::AddObjectExpandGroups(CMapClass *pObject) {
  // VPROF_BUDGET( "CObjectProperties::AddObjectExpandGroups", "Object
  // Properties" );
...
void CObjectProperties::ReloadData() {
  ...
  if (m_DstObjects.Count() == 1) {
    ...
    CManifestInstance *pManifestInstance =
        dynamic_cast<CManifestInstance *>(pobj);
    if (pManifestInstance) {
      ...
      if (pManifest) {
        ShowWindow(SW_HIDE);
        if (pDoc) {
          pDoc->UpdateAllViews(MAPVIEW_UPDATE_SELECTION | MAPVIEW_UPDATE_TOOL |
                               MAPVIEW_RENDER_NOW);
        }
...
void CObjectProperties::UpdateAnchors(CWnd *pPage) {
  ...
  CAnchorDef anchorDefs[] = {
      ...
      CAnchorDef(pPage ? pPage->GetSafeHwnd() : (HWND)NULL,
...
void CObjectProperties::OnApply(void) {
  ...
  // We lock visgroup updates here because activities in the object properties
  // dialog can change visgroups which, if updated, will change the object
  // properties, causing problems. All visgroup updates will occur at the end of
