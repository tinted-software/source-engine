...
//			texture was loaded.
//-----------------------------------------------------------------------------
const char *CWADTexture::GetFileName(void) const { return (m_szFileName); }
...
BOOL CWADTexture::AdjustTexture(char *pLoadBuf) {
  ...
  // scale up to data
  ScaleBitmap(CSize(m_nWidth, m_nHeight), CSize(m_datawidth, m_dataheight),
              pLoadBuf, (char *)m_pData);

  return TRUE;
}

bool CWADTexture::IsLoaded() const { return (m_pData != NULL); }
...
BOOL CWADTexture::Load(int fd, HANDLE hFile) {
  ...
  if (fileInfo.format == tfWAD3) {
    ...
    _lseek(fd,
           (m_nWidth / 2 * m_nHeight / 2) + (m_nWidth / 4 * m_nHeight / 4) +
               (m_nWidth / 8 * m_nHeight / 8),
           SEEK_CUR);
    ...
    if ((nPal > 0) && (nPal < 1024)) {
      ...
      // setup palette
      m_pPalette =
          (LOGPALETTE *)malloc(sizeof(WORD) * 2 + sizeof(PALETTEENTRY) * nPal);
      ...
      for (int i = 0; i < nPal; i++) {
        ...
        m_pPalette->palPalEntry[i].peFlags =
            D3DRMPALETTE_READONLY | PC_NOCOLLAPSE;
      }
...
//			dwFlags -
//-----------------------------------------------------------------------------
void CWADTexture::Draw(CDC *pDC, RECT &rect, int iFontHeight, int iIconHeight,
  ...
  if (StretchDIBits(pDC->m_hDC, rect.left, rect.top, dest_width, dest_height, 0,
                    0, m_datawidth, m_dataheight, m_pData, (BITMAPINFO *)&bmi,
                    DIB_PAL_COLORS, SRCCOPY) == GDI_ERROR) {
...
//-----------------------------------------------------------------------------
bool CWADTexture::Initialize(void) { return (AllocateLoadBuffer(g_nLoadSize)); }
