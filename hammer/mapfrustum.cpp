...
CMapClass *CMapFrustum::Create(CHelperInfo *pHelperInfo, CMapEntity *pParent) {
  ...
  if (new1 != NULL) {
    ...
    if (pszKeyName)
      V_strncpy(new1->m_szNearPlaneKeyName, pszKeyName,
                sizeof(new1->m_szNearPlaneKeyName));

    // Third parameter should be the far plane name.
    pszKeyName = pHelperInfo->GetParameter(2);
    if (pszKeyName)
      V_strncpy(new1->m_szFarPlaneKeyName, pszKeyName,
                sizeof(new1->m_szFarPlaneKeyName));

    pszKeyName = pHelperInfo->GetParameter(3);
    if (pszKeyName != NULL) {
      V_strncpy(new1->m_szColorKeyName, pszKeyName,
                sizeof(new1->m_szColorKeyName));
    }
...
CMapFace *CMapFrustum::CreateMapFace(const Vector &v1, const Vector &v2,
...
//-----------------------------------------------------------------------------
// Purpose: Builds the light cone faces in local space. Does NOT call
// CalcBounds,
//			because that CalcBounds updates the parent, which causes
//problems 			in the undo system.
//-----------------------------------------------------------------------------
void CMapFrustum::BuildFrustumFaces(void) {
  ...
  vNearFace[0].Init(m_flNearPlane, -flScaleFactor * m_flNearPlane,
                    -flScaleFactor * m_flNearPlane);
  vNearFace[1].Init(m_flNearPlane, +flScaleFactor * m_flNearPlane,
                    -flScaleFactor * m_flNearPlane);
  vNearFace[2].Init(m_flNearPlane, +flScaleFactor * m_flNearPlane,
                    +flScaleFactor * m_flNearPlane);
  vNearFace[3].Init(m_flNearPlane, -flScaleFactor * m_flNearPlane,
                    +flScaleFactor * m_flNearPlane);

  vFarFace[0].Init(m_flFarPlane, -flScaleFactor * m_flFarPlane,
                   -flScaleFactor * m_flFarPlane);
  vFarFace[1].Init(m_flFarPlane, +flScaleFactor * m_flFarPlane,
                   -flScaleFactor * m_flFarPlane);
  vFarFace[2].Init(m_flFarPlane, +flScaleFactor * m_flFarPlane,
                   +flScaleFactor * m_flFarPlane);
  vFarFace[3].Init(m_flFarPlane, -flScaleFactor * m_flFarPlane,
                   +flScaleFactor * m_flFarPlane);

  // Build the near and far faces.
  m_Faces.AddToTail(CreateMapFace(vNearFace[0], vNearFace[1], vNearFace[2],
                                  vNearFace[3], flBaseAlpha));
  m_Faces.AddToTail(CreateMapFace(vFarFace[3], vFarFace[2], vFarFace[1],
                                  vFarFace[0], flBaseAlpha));

  // Build the 4 cap faces.
  for (int i = 0; i < 4; i++) {
    m_Faces.AddToTail(CreateMapFace(vNearFace[i], vFarFace[i],
                                    vFarFace[(i + 1) % 4],
                                    vNearFace[(i + 1) % 4], flBaseAlpha));
  }

  // Also build some really translucent faces from the origin to the near plane.
  float flOriginFacesAlpha = 40.0f;
  for (int i = 0; i < 4; i++) {
    m_Faces.AddToTail(CreateMapFace(Vector(0, 0, 0), vNearFace[i],
                                    vNearFace[(i + 1) % 4], Vector(0, 0, 0),
                                    flOriginFacesAlpha));
  }
}
...
CMapClass *CMapFrustum::CopyFrom(CMapClass *pObject, bool bUpdateDependencies) {
  ...
  V_strncpy(m_szNearPlaneKeyName, pFrom->m_szNearPlaneKeyName,
            sizeof(m_szNearPlaneKeyName));
  V_strncpy(m_szFarPlaneKeyName, pFrom->m_szFarPlaneKeyName,
            sizeof(m_szFarPlaneKeyName));
  V_strncpy(m_szColorKeyName, pFrom->m_szColorKeyName,
            sizeof(m_szColorKeyName));
