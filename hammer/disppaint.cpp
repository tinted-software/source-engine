...
bool CDispPaintMgr::DoPaint(SpatialPaintData_t &spatialData) {
  ...
  for (int iDisp = 0; iDisp < nDispCount; iDisp++) {
    ...
    if (pDisp) {
      ...
      if (PaintSphereDispBBoxOverlap(spatialData.m_vCenter,
                                     spatialData.m_flRadius, vBBoxMin,
                                     vBBoxMax)) {
...
void CDispPaintMgr::DoNudgeAdd(SpatialPaintData_t &spatialData) {
  ...
  for (int iNudge = 0; iNudge < nNudgeCount; iNudge++) {
    ...
    if (IsInSphereRadius(spatialData.m_vCenter, spatialData.m_flRadius2, vVert,
                         flDistance2)) {
...
// Purpose:
//-----------------------------------------------------------------------------
bool CDispPaintMgr::PaintSphereDispBBoxOverlap(const Vector &vCenter,
                                               float flRadius,
...
// Purpose:
//-----------------------------------------------------------------------------
void CDispPaintMgr::DoPaintAdd(SpatialPaintData_t &spatialData,
                               CMapDisp *pDisp) {
  Vector vPaintPos, vVert;
  float flDistance2;
...
// Purpose:
//-----------------------------------------------------------------------------
void CDispPaintMgr::DoPaintEqual(SpatialPaintData_t &spatialData,
                                 CMapDisp *pDisp) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CDispPaintMgr::DoPaintSmooth(SpatialPaintData_t &spatialData,
                                  CMapDisp *pDisp) {
  Vector vPaintPos, vVert;
  float flDistance2;
...
// Purpose:
//-----------------------------------------------------------------------------
float CDispPaintMgr::CalcSmoothRadius2(const SpatialPaintData_t &spatialData,
                                       const Vector &vPoint) {
  Vector vTmp;
...
// Purpose:
//-----------------------------------------------------------------------------
bool CDispPaintMgr::DoPaintSmoothOneOverExp(
    const SpatialPaintData_t &spatialData, const Vector &vNewCenter,
    Vector &vPaintPos) {
  // Get the displacement manager from the active map document.
  IWorldEditDispMgr *pDispMgr = GetActiveWorldEditDispManager();
  if (!pDispMgr)
      ...
      if (PaintSphereDispBBoxOverlap(vNewCenter, flNewRadius, vBBoxMin,
        ...
        for (int iVert = 0; iVert < nVertCount; iVert++) {
          ...
          if (IsInSphereRadius(vNewCenter, flNewRadius2, vVert, flDistance2)) {
            ...
            Vector vProjectVert;
            float flProjectDist =
                DotProduct(vVert, spatialData.m_vPaintAxis) - flPaintDist;
