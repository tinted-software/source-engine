...
public:
  ...
  void GetLogicalConnectionPosition(LogicalConnection_t i,
  ...
  virtual void OnPrePaste(CMapClass *pCopy, CMapWorld *pSourceWorld,
                          ...
                          const CMapObjectList &OriginalList,
                          CMapObjectList &NewList);
  virtual void OnPaste(CMapClass *pCopyObject, CMapWorld *pSourceWorld,
                       CMapWorld *pDestWorld,
                       const CMapObjectList &OriginalList,
                       CMapObjectList &NewList);
  ...
  void NotifyChildKeyChanged(CMapClass *pChild, const char *szKey,
                             const char *szValue);
  ...
  //-----------------------------------------------------------------------------
  // Purpose: If the first child of this entity is of type MapClass, this
  // function
  //			returns a pointer to that child.
  // Output : Returns a pointer to the MapClass that is a child of this
  //			entity, NULL if the first child of this entity is not
  //MapClass.
  //-----------------------------------------------------------------------------
  template <class MapClass> MapClass *GetChildOfType(MapClass *null) {
  ...
  //
  virtual void GetRenderColor(CRender2D *pRender, unsigned char &red,
...
private:
  ...
  void OnKeyValueChanged(const char *pszKey, const char *pszOldValue,
                         const char *pszValue);
  ...
  LoadKeyCallback(const char *szKey, const char *szValue, CMapEntity *pEntity);
  static ChunkFileResult_t LoadEditorKeyCallback(const char *szKey,
                                                 const char *szValue,
                                                 CMapEntity *pEntity);

  static bool
      s_bShowEntityNames; // Whether to render entity names in the 2D views.
  static bool s_bShowEntityConnections;   // Whether to render lines indicating
                                          // entity connections in the 2D views.
  static bool s_bShowUnconnectedEntities; // Whether to render unconnected
                                          // entities in logical views
...
bool MapEntityList_HasInput(const CMapEntityList *pList, const char *szInput,
                            InputOutputType_t eType = iotInvalid);
