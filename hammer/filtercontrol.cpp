...
BOOL CFilterControl::Create(CWnd *pParentWnd) {
  if (!CHammerBar::Create(pParentWnd, IDD_FILTERCONTROL,
                          CBRS_RIGHT | CBRS_SIZE_DYNAMIC, IDCB_FILTERCONTROL,
                          "Filter Control"))
...
void CFilterControl::UpdateGroupList(void) {
  ...
  for (int i = 0; i < nCount; i++) {
    ...
    if ((compareValue == 0 && m_bShowingAuto) ||
        (compareValue != 0 && !m_bShowingAuto)) {
...
//			visgroups are currently overridden by the "Show All"
//button.
//-----------------------------------------------------------------------------
...
//			bDisableIfNoHndler -
//-----------------------------------------------------------------------------
void CFilterControl::OnUpdateCmdUI(CFrameWnd *pTarget,
    ...
    if (pVisGroup) {
      ...
      while (pChild) {
        if (pChild->IsInVisGroup(pVisGroup)) {
          if (pChild->IsVisible()) {
            CMapClass *pSelectObject =
                pChild->PrepareSelection(pDoc->GetSelection()->GetMode());
            if (pSelectObject) {
...
LRESULT CFilterControl::OnListToggleState(WPARAM wParam, LPARAM lParam) {
  ...
  if (pDoc != NULL) {
    ...
    if (pVisGroup != NULL) {
      pDoc->VisGroups_ShowVisGroup(pVisGroup,
                                   pVisGroup->GetVisible() == VISGROUP_HIDDEN);
    }
  ...
  if (pDoc != NULL) {
    ...
    if (pDropGroup != NULL) {
      if (pDragGroup->FindDescendent(pDropGroup)) {
        CString str;
        str.Format(
            "Cannot combine the groups because '%s' is a sub-group of '%s'.",
            pDropGroup->GetName(), pDragGroup->GetName());
        AfxMessageBox(str);
        UpdateGroupList();
        return 0;
      }
      ...
      CString str;
      str.Format("Combine group '%s' into group '%s'?", pDragGroup->GetName(),
                 pDropGroup->GetName());
      ...
      // Show the visgroup that's being deleted so that member objects are
      // shown.
...
LRESULT CFilterControl::OnListRightDragDrop(WPARAM wParam, LPARAM lParam) {
  ...
  if (pDoc != NULL) {
    ...
    if (pDragGroup->FindDescendent(pDropGroup)) {
      CString str;
      str.Format("Cannot move the group because '%s' is a sub-group of '%s'.",
                 pDropGroup->GetName(), pDragGroup->GetName());
      AfxMessageBox(str);
      return 0;
    }
...
// Purpose:
//-----------------------------------------------------------------------------
BOOL CFilterControl::OnInitDialog(void) { return TRUE; }
