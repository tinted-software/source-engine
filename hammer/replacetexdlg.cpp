...
void CReplaceTexDlg::OnBrowsereplace() { BrowseTex(IDC_REPLACE); }

void CReplaceTexDlg::OnBrowsefind() { BrowseTex(IDC_FIND); }
...
void CReplaceTexDlg::DoReplaceTextures() {
  ...
  if (pDoc) {
    pDoc->ReplaceTextures(m_strFind, m_strReplace, m_iSearchAll,
                          m_iAction | (m_bMarkOnly ? 0x100 : 0), m_bHidden,
