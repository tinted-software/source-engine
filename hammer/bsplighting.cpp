...
bool CBSPLighting::Load(char const *pFilename) {
  ...
  for (int iFace = 0; iFace < file.numfaces; iFace++) {
    ...
    pStoredFace->m_pMaterial =
        FindOrAddMaterial(file, pTexData->nameStringTableID);
    if (pStoredFace->m_pMaterial)
      pStoredFace->m_pMaterial->m_Faces.AddToTail(pStoredFace);

    // Setup its lightmap.
    memcpy(pOut->m_LightmapVecs,
           file.texinfo[pIn->texinfo].lightmapVecsLuxelsPerWorldUnits,
           sizeof(pOut->m_LightmapVecs));
    memcpy(pOut->m_LightmapTextureMinsInLuxels,
           pIn->m_LightmapTextureMinsInLuxels,
    ...
    for (int iEdge = 0; iEdge < pIn->numedges; iEdge++) {
      ...
      if (edgeVal < 0)
        verts[pOut->m_iVertStart + iEdge].m_vPos =
            file.dvertexes[file.dedges[-edgeVal].v[1]].point;
      else
        verts[pOut->m_iVertStart + iEdge].m_vPos =
            file.dvertexes[file.dedges[edgeVal].v[0]].point;
    }
  ...
  FOR_EACH_LL(m_FaceMaterials, iMat) {
    ...
    bool bNeedsBumpmap = pMat->m_pMaterial->GetPropertyFlag(
    ...
    FOR_EACH_LL(pMat->m_Faces, iFace) {
      ...
      pOut->m_LightmapSortID = pMatSys->AllocateLightmap(
          bumpedSize, pStoredFace->m_LightmapSize[1],
          pStoredFace->m_OffsetIntoLightmapPage, pMat->m_pMaterial);
    }
  }
  ...
  for (int iFace = 0; iFace < faces.Count(); iFace++) {
    m_StoredFaces[iFace].m_LightmapPageID =
        sortInfos[faces[iFace].m_LightmapSortID].lightmapPageID;
  }
  ...
  for (int iFace = 0; iFace < faces.Size(); iFace++) {
    ...
    pMatSys->GetLightmapPageSize(pFace->m_pStoredFace->m_LightmapPageID,
                                 &lightmapPageSize[0], &lightmapPageSize[1]);

    pStoredFace->m_BumpSTexCoordOffset =
        (float)pStoredFace->m_LightmapSize[0] / lightmapPageSize[0];
    ...
    for (int iVert = 0; iVert < pFace->m_nVerts; iVert++) {
      ...
      for (int iCoord = 0; iCoord < 2; iCoord++) {
        ...
        float flVal = lmVec[0] * vPos[0] + lmVec[1] * vPos[1] +
                      lmVec[2] * vPos[2] + lmVec[3] -
        ...
        pVert->m_vTexCoords[iCoord] =
            DotProduct(
                vPos,
                *((Vector *)pTexInfo->textureVecsTexelsPerWorldUnits[iCoord])) +
            pTexInfo->textureVecsTexelsPerWorldUnits[iCoord][3];

        if (pStoredFace->m_pMaterial) {
          if (iCoord == 0)
            pVert->m_vTexCoords[iCoord] /=
                pStoredFace->m_pMaterial->m_pMaterial->GetMappingWidth();
          else
            pVert->m_vTexCoords[iCoord] /=
                pStoredFace->m_pMaterial->m_pMaterial->GetMappingHeight();
        }
      }
    }
  }
...
bool CBSPLighting::Serialize() {
  if (m_pBSPLightingThread) {
    ...
    if (m_pBSPLightingThread->GetCurrentState() ==
        IBSPLightingThread::STATE_FINISHED)
...
void DrawLightmapPage(IMaterialSystem *materialSystemInterface,
                      int lightmapPageID) {
  IMaterial *g_materialDebugLightmap = materialSystemInterface->FindMaterial(
      "debug/debuglightmap", TEXTURE_GROUP_OTHER);
  ...
  materialSystemInterface->GetLightmapPageSize(
      lightmapPageID, &lightmapPageWidth, &lightmapPageHeight);
  ...
  FOR_EACH_LL(m_FaceMaterials, iMat) {
    ...
    FOR_EACH_LL(pMat->m_MaterialBufs, iBuf) {
      ...
      for (int iCmd = 0; iCmd < pBuf->m_DrawCommands.Count(); iCmd++) {
        ...
        pBuf->m_pMesh->Draw(pCmd->m_PrimLists.Base(),
                            pCmd->m_PrimLists.Count());
      }
    }
  }
...
void CBSPLighting::AssignFaceMaterialCounts(CBSPInfo &file,
                                            CUtlVector<CFace> &faces) {
...
VertexFormat_t CBSPLighting::ComputeLMGroupVertexFormat(IMaterial *pMaterial) {
  ...
  // FIXME: check for and strip unused vertex elements (bone weights+indices,
  // TANGENT_S/T?) - requires reliable material vertex formats first

  return vertexFormat;
}

void CBSPLighting::BuildLMGroups(CBSPInfo &file, CUtlVector<CFace> &faces,
                                 CUtlVector<CVert> &verts,
                                 CUtlVector<CDispInfoFaces> &dispInfos) {
  ...
  FOR_EACH_LL(m_FaceMaterials, iMat) {
    ...
    FOR_EACH_LL(pMat->m_MaterialBufs, iBuf) {
      ...
      VertexFormat_t vertexFormat =
          ComputeLMGroupVertexFormat(pMat->m_pMaterial);
      pBuf->m_pMesh = pRenderContext->CreateStaticMesh(vertexFormat, "terd",
                                                       pMat->m_pMaterial);
      if (!pBuf->m_pMesh)
        continue;
      ...
      CMeshBuilder mb;
      mb.Begin(pBuf->m_pMesh, MATERIAL_TRIANGLES, pBuf->m_nVerts,
               pBuf->m_nIndices);
      ...
      FOR_EACH_LL(pBuf->m_Faces, iFace) {
        ...
        if (pFace->m_iDispInfo == -1) {
          ...
          unsigned short indices[(1 << MAX_MAP_DISP_POWER) *
                                 (1 << MAX_MAP_DISP_POWER) * 6];
...
bool FindDrawCommand(CUtlVector<CBSPLighting::CDrawCommand *> &drawCommands,
                     int lmPageID, int &index) {
  ...
  FOR_EACH_LL(m_FaceMaterials, iMat) {
    ...
    FOR_EACH_LL(pMat->m_MaterialBufs, iBuf) {
      ...
      FOR_EACH_LL(pBuf->m_Faces, iFace) {
        ...
        int index;
        if (!FindDrawCommand(pBuf->m_DrawCommands, pFace->m_LightmapPageID,
                             index)) {
          index = pBuf->m_DrawCommands.AddToTail(new CDrawCommand);
          pBuf->m_DrawCommands[index]->m_LightmapPageID =
...
void CBSPLighting::ReloadLightmaps() {
  ...
  Vector4D blocklights[4][MAX_LIGHTMAP_DIM_INCLUDING_BORDER *
  ...
  for (int iFace = 0; iFace < m_StoredFaces.Count(); iFace++) {
    ...
    bool bNeedsBumpmap = pFace->m_pMaterial->m_pMaterial->GetPropertyFlag(
        MATERIAL_PROPERTY_NEEDS_BUMPED_LIGHTMAPS);

    texinfo_t *pTexInfo =
        &bspInfo.texinfo[bspInfo.dfaces[pFace->m_iMapFace].texinfo];
    ...
    ColorRGBExp32 *pLightmap =
        (ColorRGBExp32 *)&bspInfo.dlightdata[pIn->lightofs];
    int iLightmap;
    for (iLightmap = 0; iLightmap < nLightmaps; iLightmap++) {
      for (int iLuxel = 0; iLuxel < nLuxels; iLuxel++) {
        blocklights[iLightmap][iLuxel][0] =
            TexLightToLinear(pLightmap->r, pLightmap->exponent);
        blocklights[iLightmap][iLuxel][1] =
            TexLightToLinear(pLightmap->g, pLightmap->exponent);
        blocklights[iLightmap][iLuxel][2] =
            TexLightToLinear(pLightmap->b, pLightmap->exponent);
    ...
    if (bNeedsBumpmap && !bHasBumpmap) {
      for (iLightmap = 1; iLightmap < 4; iLightmap++) {
        memcpy(blocklights[iLightmap], blocklights[0],
               nLuxels * sizeof(blocklights[0][0]));
      }
    }

    if (bNeedsBumpmap) {
      pMatSys->UpdateLightmap(pFace->m_LightmapPageID, pFace->m_LightmapSize,
                              pFace->m_OffsetIntoLightmapPage,
                              (float *)blocklights[0], (float *)blocklights[1],
                              (float *)blocklights[2], (float *)blocklights[3]);
    } else {
      pMatSys->UpdateLightmap(pFace->m_LightmapPageID, pFace->m_LightmapSize,
                              pFace->m_OffsetIntoLightmapPage,
...
void CBSPLighting::CreateDisplacements(CBSPInfo &file, CUtlVector<CFace> &faces,
                                       CUtlVector<CDispInfoFaces> &dispInfos) {
  /*
          IMaterialSystem *pMatSys = MaterialSystemInterface();
          ...
          for( int iFace=0; iFace < faces.Size(); iFace++ )
          {
                  ...
                  int lightmapPageSize[2];
                  pMatSys->GetLightmapPageSize(
     pFace->m_pStoredFace->m_LightmapPageID, &lightmapPageSize[0],
     &lightmapPageSize[1] );

                  for( int iVert=0; iVert < pInDisp->m_LODs[0].m_nVerts; iVert++
     )
                  {
                          ddisp_lod_vert_t *pInVert = &file.ddispverts[
     pInDisp->m_LODs[0].m_iVertStart + iVert ]; CVert *pOutVert =
     &pOutDisp->m_Verts[iVert];
                          ...
                          for( int iCoord=0; iCoord < 2; iCoord++ )
                          {
                                  ...
                                  flVal +=
     pFace->m_pStoredFace->m_OffsetIntoLightmapPage[iCoord]; flVal += 0.5f;
                                  flVal /= lightmapPageSize[iCoord];
                                  Assert( _finite(flVal) );
                                  pOutVert->m_vLightCoords[iCoord] = flVal;

                                  pOutVert->m_vTexCoords[iCoord] =
     pInVert->m_TexCoords[iCoord];

                                  if( iCoord == 0 )
                                          pOutVert->m_vTexCoords[iCoord] /=
     pStoredFace->m_pMaterial->m_pMaterial->GetMappingWidth(); else
                                          pOutVert->m_vTexCoords[iCoord] /=
     pStoredFace->m_pMaterial->m_pMaterial->GetMappingHeight();
                          }
                  }
...
CBSPLighting::CFaceMaterial *
CBSPLighting::FindOrAddMaterial(CBSPInfo &file, int stringTableID) {
  ...
  if (m_StringTableIDToMaterial[stringTableID]) {
    ...
    char *pMaterialName =
        &file.texDataStringData[file.texDataStringTable[stringTableID]];
    if (pMaterialName)
      pMaterial = MaterialSystemInterface()->FindMaterial(pMaterialName,
                                                          TEXTURE_GROUP_OTHER);
    ...
    // This is lovely. We have to call this stuff to get it to precalculate the
    // data it needs.
