...
public:
  ...
  // Factories.
  //
  static CMapClass *CreateMapAnimator(CHelperInfo *pHelperInfo,
                                      CMapEntity *pParent);
  ...
  float GetKeyFramesAtTime(float time, CMapKeyFrame *&pKeyFrame,
                           CMapKeyFrame *&pPrevKeyFrame);
  CMapEntity *CreateNewKeyFrame(float time);
  void GetAnimationAtTime(float animTime, Vector &newOrigin,
                          Quaternion &newAngles);
  static void GetAnimationAtTime(CMapKeyFrame *currentKey,
                                 CMapKeyFrame *pPrevKey, float animTime,
                                 Vector &newOrigin, Quaternion &newAngles,
                                 int posInterpolator, int rotInterpolator);

  int GetNumKeysChanged() { return m_nKeysChanged; }

private:
  // Used by the keys to detect when changes need to be sent to the position
  // interpolators.
  int m_nKeysChanged;
