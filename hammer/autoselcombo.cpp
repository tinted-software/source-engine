...
//			user types in the edit control. It has the additional
//feature of 			being able to easily set the text color.
//
//=============================================================================//
...
void CAutoSelComboBox::OnUpdateText(void) {
  ...
  // the current selection. This won't do anything if they are deleting
  ...
  if (nNewSel != m_nLastSel) {
    GetParent()->SendMessage(
        WM_COMMAND, MAKEWPARAM(GetDlgCtrlID(), CBN_SELCHANGE), (LPARAM)m_hWnd);
    m_nLastSel = nNewSel;
  }
}
...
//-----------------------------------------------------------------------------
void CAutoSelComboBox::OnSetFocus(CWnd *pOldWnd) { m_szLastText[0] = '\0'; }
