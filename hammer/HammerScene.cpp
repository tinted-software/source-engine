...
char *ExpandPath(char *path) {
  ...
  g_pFullFileSystem->RelativePathToFullPath(path, "GAME", fullpath,
                                            sizeof(fullpath));
  return fullpath;
}
...
CChoreoScene *HammerLoadScene(const char *pFilename) {
  if (g_pFullFileSystem->FileExists(pFilename)) {
    ...
    CChoreoScene *scene =
        ChoreoLoadScene(pFilename, NULL, &g_TokenProcessor, Msg);
    return scene;
  }
...
bool GetFirstSoundInScene(const char *pSceneFilename, char *pSoundName,
                          int soundNameLen) {
