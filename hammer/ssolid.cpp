...
BOOL CheckFace(Vector *Points, int nPoints, Vector *pNormal, float dist,
...
// Purpose: Destructor.
//-----------------------------------------------------------------------------
CSSolid::~CSSolid() { memset(this, 0, sizeof(this)); }

SSHANDLE CSSolid::GetNewID() { return m_curid++; }
...
int CSSolid::GetFaceIndex(const Vector &Point, float fLeniency) { return -1; }
...
void CSSolid::Attach(CMapSolid *pMapSolid) { m_pMapSolid = pMapSolid; }
...
void CSSolid::ToMapSolid(CMapSolid *p) {
  ...
  for (int i = 0; i < m_nFaces; i++) {
    ...
    //
    // Vertex manipulation; the face orientation may have changed. If one of the
    // texture axes is now perpendicular to the face, recalculate the texture
    // axes using the default alignment (world or face). Ideally we would
    // transform the texture axes so that their orientation relative to the face
    // is preserved. By reinitializing the axes we risk having the axes rotate
    // unpredictably.
    //
    if (!SolidFace.IsTextureAxisValid()) {
      SolidFace.InitializeTextureAxes(Options.GetTextureAlignment(),
...
CSSEdge **CSSolid::FindAffectedEdges(SSHANDLE *pHandles, int iNumHandles,
                                     int &iNumEdges) {
...
void CSSolid::MoveSelectedHandles(const Vector &Delta) {
  ...
  for (int i = 0; i < nMoveVertices; i++) {
    ...
    SetVertexPosition(hi.iIndex, pVertex->pos[0] + Delta[0],
                      pVertex->pos[1] + Delta[1], pVertex->pos[2] + Delta[2]);
  }
...
BOOL CSSolid::SplitFaceByEdges(CSSEdge *pEdge1, CSSEdge *pEdge2) {
  ...
  */
  ...
  SSHANDLE *phVertexList = CreateNewVertexList(
      pFace, pEdge1, pEdge2, nv1index, nv2index, pNewVertex1, pNewVertex2);
  int nVertices = pFace->nEdges;
  if (nv1index != -1)
    ++nVertices;
  if (nv2index != -1)
...
DoNextFace:
  ...
  for (int iFace = 0; iFace < m_nFaces; iFace++) {
    ...
    phVertexList = CreateNewVertexList(pUpdFace, pEdge1, pEdge2, nv1index,
                                       nv2index, pNewVertex1, pNewVertex2);
...
void CSSolid::DeleteEdge(int iEdge) {
  ...
  for (int f = 0; f < m_nFaces; f++) {
    ...
    for (int e = 0; e < face.nEdges; e++) {
      ...
      memcpy(&face.Edges[e], &face.Edges[e + 1],
             (face.nEdges - e) * sizeof(face.Edges[0]));
      --face.nEdges;
...
SSHANDLE *CSSolid::CreateNewVertexList(CSSFace *pFace, CSSEdge *pEdge1,
                                       CSSEdge *pEdge2, int &nv1index,
                                       int &nv2index, CSSVertex *pNewVertex1,
...
DoVertices:
  for (int v1 = 0; v1 < m_nVertices; v1++) {
    for (int v2 = 0; v2 < m_nVertices; v2++) {
      if (v1 == v2)
        continue;                                                   // no!
      if (!VectorCompare(m_Vertices[v1].pos, m_Vertices[v2].pos)) { // no match
        continue;
      }
  ...
  DoConcurrentEdges:
    for (int ef1 = 0; ef1 < face.nEdges; ef1++) {
      for (int ef2 = 0; ef2 < face.nEdges; ef2++) {
        ...
        // delete this ref
        memcpy(&face.Edges[ef2], &face.Edges[ef2 + 1],
               (face.nEdges - ef2) * sizeof(face.Edges[0]));
        --face.nEdges;
...
// Purpose: Constructor.
//-----------------------------------------------------------------------------
CSSEdge::CSSEdge(void) { Faces[0] = Faces[1] = 0; }

//-----------------------------------------------------------------------------
// Purpose: Destructor.
//-----------------------------------------------------------------------------
CSSEdge::~CSSEdge() { memset(this, 0, sizeof(this)); }
...
//-----------------------------------------------------------------------------
void CSSEdge::GetCenterPoint(Vector &Point) { Point = ptCenter; }
...
//-----------------------------------------------------------------------------
void CSSVertex::GetPosition(Vector &Position) { Position = pos; }
