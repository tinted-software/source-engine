...
private:
  ...
  struct Basis_t {
    ...
    int m_nAxesFlip[3];  // u, v, n flip values (3 bits x,y,z) - saved off the z
                         // components of the first 3 uv points
  };
  ...
  struct Handles_t {
    ...
    Vector2D
        m_vecBasisCoords[OVERLAY_HANDLES_COUNT]; // U,V coordinates of the 4
                                                 // corners in the editable
                                                 // plane (use basis)
    Vector
        m_vec3D[OVERLAY_HANDLES_COUNT]; // World space handles for snap testing
  };

  void Handles_Clear(void);
  void Handles_Build3D();
  void Handles_Render3D(CRender3D *pRender);
  void Handles_SurfToOverlayPlane(CMapFace *pFace, Vector const &vSurf,
                                  Vector &vPoint);
  ...
  struct ClipFace_t {
    ...
    CUtlVector<Vector> m_aDispPointUVs; // z is always 0 (need to be this way to
                                        // share functions!)
  ...
  void ClipFace_GetBounds(ClipFace_t *pClipFace, Vector &vecMin,
                          Vector &vecMax);

  void ClipFace_Clip(ClipFace_t *pClipFace, cplane_t *pClipPlane,
                     float flEpsilon, ClipFace_t **ppFront,
                     ClipFace_t **ppBack);
  void ClipFace_ClipBarycentric(ClipFace_t *pClipFace, cplane_t *pClipPlane,
                                float flEpsilon, int iClip, CMapDisp *pDisp,
  ...
  void ClipFace_ResolveBarycentricClip(CMapDisp *pDisp, ClipFace_t *pClipFace,
                                       int iClipFacePoint,
                                       const Vector2D &vecPointUV,
                                       float *pCoefs, int *pTris,
                                       Vector2D *pVertsUV);
  bool ClipFace_CalcBarycentricCooefs(CMapDisp *pDisp, Vector2D *pVertsUV,
                                      const Vector2D &vecPointUV,
                                      float *pCoefs);

  int ClipFace_GetAxisType(cplane_t *pClipPlane);
  void ClipFace_BuildBlend(ClipFace_t *pClipFace, CMapDisp *pDisp,
                           cplane_t *pClipPlane, int iClip, const Vector &vecUV,
  ...
  void Disp_DoClip(CMapDisp *pDisp, ClipFaces_t &aDispFragments,
  ...
  void OverlayPlaneToWorld(CMapFace *pFace, const Vector &vecPlane,
                           Vector &vecWorld);
  void OverlayPlaneToSurfFromList(const Vector &vecOverlayPoint,
                                  Vector &vecSurfPoint);
  bool EntityOnSurfFromListToBaseFacePlane(const Vector &vecWorldPoint,
                                           Vector &vecBasePoint);

  bool BuildEdgePlanes(Vector const *pPoints, int pointCount,
                       cplane_t *pEdgePlanes, int edgePlaneCount);
  void UpdateDispBarycentric(void);
  void PostModified(void);
  void GetTriVerts(CMapDisp *pDisp, const Vector2D &vecSurfUV, int *pTris,
                   Vector2D *pVertsUV);
