...
//--------------------------------------------------------------------------------------------------
// Returns true if the passed string matches the filename style glob, false
// otherwise
// * matches any characters, ? matches any single character, otherwise case
...
//--------------------------------------------------------------------------------------------------
// Inserts pkvToInsert after pkvAfter but setting pkvAfter's NextKey to
// pkvInsert
//--------------------------------------------------------------------------------------------------
...
//
//--------------------------------------------------------------------------------------------------
static KeyValues *ReplaceSubKeyWithCopy(KeyValues *pkvParent,
                                        KeyValues *pkvToReplace,
                                        KeyValues *pkvReplaceWith) {
...
//--------------------------------------------------------------------------------------------------
// Handles a KeyValues #insert macro.  Replaces the #insert KeyValues with the
// specified file if it can be loaded.  This is not called #include because base
// KeyValue's already has #include but it has two issues.  The #include is
// relative to the keyvalues, these paths are resolved normally via IFileSystem
// and #include only works at the top level, #insert works no matter how deep
// the #insert macro is
//--------------------------------------------------------------------------------------------------
static KeyValues *HandleKeyValuesMacro_Insert(KeyValues *pkvInsert,
                                              KeyValues *pkvParent) {
  ...
  if (pkvInsert->GetFirstSubKey()) {
    ...
    Msg("Error: #insert on key with subkeys, can only do #insert with simple "
  ...
  if (pkvInsert->GetDataType() != KeyValues::TYPE_STRING) {
    ...
    Msg("Error: #insert on key without a data type of string, can only do "
        "#insert with simple key/value with string value\n");
    return nullptr;
  }
  ...
  if (!pszInsert && *pszInsert == '\0') {
    ...
    Msg("Error: #insert on key with empty string value, can only do #insert "
        "with simple key/value with string value\n");
    return nullptr;
  }
  ...
  uint nBufSize = g_pFullFileSystem->GetOptimalReadSize(
      f, nFileSize + 2 /* null termination */ + 8 /* "\"x\"\n{\n}\n" */);
  ...
  bool bRetOK =
      (g_pFullFileSystem->ReadEx(pBuf + 6, nBufSize - 6, nFileSize, f) != 0);
  ...
  if (bRetOK) {
    ...
    while (pkvNewSubKey) {
      ...
      if (pkvNewSubKey->GetFirstSubKey() == nullptr) {
        for (KeyValues *pkvChild = pkvParent->GetFirstSubKey(); pkvChild;
             pkvChild = pkvChild->GetNextKey()) {
...
static void UpdateKeyValuesBlock(KeyValues *pkvDst, KeyValues *pkvUpdate) {
  ...
  for (KeyValues *pkvUpdateSubKey = pkvUpdate->GetFirstSubKey();
       pkvUpdateSubKey; pkvUpdateSubKey = pkvUpdateSubKey->GetNextKey()) {
    const int nSrcName = pkvUpdateSubKey->GetNameSymbol();

    if (pkvUpdateSubKey->GetFirstSubKey()) {
      Msg("Error: #update has a key with subkeys, only simple key/values are "
          "allowed for #update, skipping: %s\n",
    ...
    for (KeyValues *pkvDstSubKey = pkvDst->GetFirstSubKey(); pkvDstSubKey;
         pkvDstSubKey = pkvDstSubKey->GetNextKey()) {
    ...
    if (!pkvNew) {
      ...
      pkvDst->AddSubKey(
          pkvNew); // TODO: Perhaps add this right after the #update block?
    }
...
//
// An #update must be a KeyValue block with a KeyValue block as a parent.  It
// will look at sibling KeyValue blocks which match an optional "#glob", or all
// sibling KeyValue blocks if no "#glob" is specified and will merge all of the
// #update block's subkeys into each sibling block. overwriting KeyValues if
// they already exist and adding new KeyValues if they don't.
//
// Example:
//
// Before:
...
//
//--------------------------------------------------------------------------------------------------
static KeyValues *HandleKeyValuesMacro_Update(KeyValues *pkvUpdate,
                                              KeyValues *pkvParent,
                                              bool *pbDidUpdate) {
  ...
  if (pkvUpdate->GetFirstSubKey() == nullptr) {
    // Invalid, has sub keys
    Msg("Error: #insert on key without subkeys, can only do #update with a key "
        "with subkeys\n");
    return nullptr;
  }
  ...
  if (pkvUpdate->GetDataType() != KeyValues::TYPE_NONE) {
    ...
    Msg("Error: #update on key without a data type of NONE, can only do "
        "#update with a key with subkeys\n");
    return nullptr;
  }
  ...
  for (KeyValues *pkvParentSubKey = pkvParent->GetFirstSubKey();
       pkvParentSubKey; pkvParentSubKey = pkvParentSubKey->GetNextKey()) {
...
//--------------------------------------------------------------------------------------------------
KeyValues *HandleKeyValuesMacros(KeyValues *kv,
                                 KeyValues *pkvParent /* = nullptr */) {
