...
CP4File::CP4File(char const *szFilename) {
  ...
  // On windows, get the pathname of the file on disk first before using that as
  // a perforce path this avoids invalid Adds().  Have to go through
  // GetShortPathName and then GetLongPathName from the short path name

  TCHAR szShortPathName[MAX_PATH] = TEXT("");
  const DWORD shortRetVal =
      GetShortPathName(szFilename, szShortPathName, ARRAYSIZE(szShortPathName));

  if (shortRetVal > 0 && shortRetVal <= ARRAYSIZE(szShortPathName)) {
    TCHAR szLongPathName[MAX_PATH] = TEXT("");

    const DWORD longRetVal = GetLongPathName(szShortPathName, szLongPathName,
                                             ARRAYSIZE(szLongPathName));
