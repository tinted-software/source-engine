...
void GetModSubdirectory(const char *pSubDir, char *pBuf, int nBufLen) {
  ...
  Assert(g_pFullFileSystem->GetSearchPath("MOD_WRITE", false, NULL, 0) <
         nBufLen);
  if (g_pFullFileSystem->GetSearchPath("MOD_WRITE", false, pBuf, nBufLen) ==
      0) {
...
void ComputeModFilename(const char *pContentFileName, char *pBuf,
  ...
  if (!g_pFullFileSystem->FullPathToRelativePathEx(pContentFileName,
                                                   "CONTENTROOT", pRelativePath,
  ...
  g_pFullFileSystem->GetSearchPath("GAMEROOT", false, pGameRoot,
                                   sizeof(pGameRoot));
...
void ComputeModContentFilename(const char *pGameFileName, char *pBuf,
  ...
  if (!g_pFullFileSystem->FullPathToRelativePathEx(
          pGameFileName, "GAMEROOT", pRelativePath, sizeof(pRelativePath))) {
    Q_strncpy(pBuf, pGameFileName, (int)nBufLen);
    return;
  }

  char pContentRoot[MAX_PATH];
  g_pFullFileSystem->GetSearchPath("CONTENTROOT", false, pContentRoot,
                                   sizeof(pContentRoot));
