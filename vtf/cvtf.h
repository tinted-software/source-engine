...
public:
  ...
  //		arrTypesBuffer			buffer to be filled with
  //resource types available. 		numTypesBufferElems		how many
  //resource types the buffer can accomodate.
  // Returns:
  ...
  virtual bool UnserializeEx(CUtlBuffer &buf, bool bHeaderOnly = false,
                             int nForceFlags = 0, int nSkipMipLevels = 0);
...
#if defined(_X360)
  virtual int UpdateOrCreate(const char *pFilename, const char *pPathID = NULL,
                             bool bForce = false);
...
private:
  ...
  CUtlVector<ResourceMemorySection>
      m_arrResourcesData_ForReuse; // Maintained to keep allocated memory blocks
                                   // when unserializing from files
