...
DEFINE_ARRAY(fileTypeString, FIELD_CHARACTER, 4),
    ...
    END_DATADESC()
        ...
        BEGIN_BYTESWAP_DATADESC(VTFFileHeaderV7_1_t)
            DEFINE_ARRAY(fileTypeString, FIELD_CHARACTER, 4),
    DEFINE_ARRAY(version, FIELD_INTEGER, 2),
    DEFINE_FIELD(headerSize, FIELD_INTEGER), DEFINE_FIELD(width, FIELD_SHORT),
    DEFINE_FIELD(height, FIELD_SHORT), DEFINE_FIELD(flags, FIELD_INTEGER),
    ...
    DEFINE_FIELD(lowResImageWidth, FIELD_CHARACTER),
    DEFINE_FIELD(lowResImageHeight, FIELD_CHARACTER),
    DEFINE_FIELD(depth, FIELD_SHORT), DEFINE_FIELD(numResources, FIELD_INTEGER),
    END_DATADESC()
...
int VTFFileHeaderSize(int nMajorVersion, int nMinorVersion) {
  ...
  case VTF_MAJOR_VERSION:
    ...
    case VTF_MINOR_VERSION:
      ...
      // printf("\n VTFFileHeaderSize (%i %i) is %i + %i ->
      // %i",nMajorVersion,nMinorVersion, size1, size2, result );
      return result;
    }
    break;
...
static bool GenericAllocateReusableData(unsigned char **ppData,
  ...
  if (*pNumAllocated < numRequested ||
      (numRequested > 0 && *pNumAllocated > 16 * numRequested)) {
    delete[] *ppData;
...
bool CVTFTexture::AllocateImageData(int nMemorySize) {
  return GenericAllocateReusableData(&m_pImageData, &m_nImageAllocSize,
                                     nMemorySize);
}
...
bool CVTFTexture::AllocateLowResImageData(int nMemorySize) {
  return GenericAllocateReusableData(&m_pLowResImageData,
...
bool CVTFTexture::Init(int nWidth, int nHeight, int nDepth, ImageFormat fmt,
  ...
  if ((fmt == IMAGE_FORMAT_DXT1) || (fmt == IMAGE_FORMAT_DXT3) ||
      (fmt == IMAGE_FORMAT_DXT5) || (fmt == IMAGE_FORMAT_DXT1_RUNTIME) ||
...
void CVTFTexture::ImageFileInfo(int nFrame, int nFace, int nMipLevel,
  ...
  if (pImageDataInfo == NULL) {
    // This should never happen for real, but can happen if someone
    // intentionally fed us a bad VTF.
...
int CVTFTexture::FileSize(int nMipSkipCount) const {
  ...
  if (pImageDataInfo == NULL) {
    // Still do the assert so we can catch this in debug--we don't expect this
...
bool CVTFTexture::LoadImageData(CUtlBuffer &buf, const VTFFileHeader_t &header,
  ...
  if (IsCubeMap() && (header.version[0] == 7) && (header.version[1] == 4)) {
    ...
    int nFileSize =
        ComputeFaceSize(nSkipMipLevels) * m_nFaceCount * m_nFrameCount;
...
retryCubemapLoad:
  for (int iMip = m_nMipCount; --iMip >= 0;) {
    ...
    for (int iFrame = 0; iFrame < m_nFrameCount; ++iFrame) {
      for (int iFace = 0; iFace < m_nFaceCount; ++iFace) {
        // printf("\n tex %p mip %i frame %i face %i  size %i  buf offset %i",
        // this, iMip, iFrame, iFace, iMipSize, buf.TellGet() );
        unsigned char *pMipBits = ImageData(iFrame, iFace, iMip);
        buf.Get(pMipBits, iMipSize);
      }

      // Strip out the spheremap in older versions
      if (IsCubeMap() && !bNoSkip && (header.version[0] == 7) &&
          (header.version[1] >= 1) && (header.version[1] < 5)) {
  ...
  if (!bOk && IsCubeMap() && (header.version[0] == 7) &&
      (header.version[1] <= 4)) {
    if (!bNoSkip) {
      bNoSkip = true;
...
void *CVTFTexture::SetResourceData(uint32 eType, void const *pData,
                                   size_t nNumBytes) {
...
bool CVTFTexture::ReadHeader(CUtlBuffer &buf, VTFFileHeader_t &header) {
  if (IsX360() && SetupByteSwap(buf)) {
    ...
    m_Swap.SwapFieldsToTargetEndian(&baseHeader,
                                    (VTFFileBaseHeader_t *)buf.PeekGet());
    ...
    if (baseHeader.version[0] == VTF_MAJOR_VERSION) {
      ...
      } else if (baseHeader.version[1] >= 4 &&
                 baseHeader.version[1] <= VTF_MINOR_VERSION) {
...
bool CVTFTexture::UnserializeEx(CUtlBuffer &buf, bool bHeaderOnly,
                                int nForceFlags, int nSkipMipLevels) {
  tmZone(TELEMETRY_LEVEL0, TMZF_NONE,
         "%s (header only: %d, nForceFlags: %d, skipMips: %d)", __FUNCTION__,
  ...
  if ((header.imageFormat < IMAGE_FORMAT_UNKNOWN) ||
      (header.imageFormat >= NUM_IMAGE_FORMATS)) {
    Warning("*** Encountered VTF invalid image format!\n");
    return false;
  }

  // If the header says we should be doing a texture allocation of more than
  // 32M, just tell the caller we failed.
  const int cMaxImageSizeLog2 = Q_log2(32 * 1024 * 1024);
  if ((Q_log2(header.width) + Q_log2(header.height) + Q_log2(header.depth) +
           Q_log2(header.numFrames) >
       cMaxImageSizeLog2) ||
      (header.numResources > MAX_RSRC_DICTIONARY_ENTRIES)) {
    STAGING_ONLY_EXEC(DevWarning("Asked for a large texture to be created (%d "
                                 "h x %d w x %d d x %d f). Nope.\n",
  ...
  if ((m_LowResImageFormat < IMAGE_FORMAT_UNKNOWN) ||
      (m_LowResImageFormat >= NUM_IMAGE_FORMATS))
    return false;
...
ResourceEntryInfo const *
CVTFTexture::FindResourceEntryInfo(uint32 eType) const {
  Assert((eType & RSRCF_MASK) == 0);
...
int CVTFTexture::GetImageOffset(int iFrame, int iFace, int iMipLevel,
  ...
  if (IsX360() && (m_nVersion[0] == VTF_X360_MAJOR_VERSION)) {
    ...
    int nFaceSize = ImageLoader::GetMemRequired(iMipWidth, iMipHeight,
                                                iMipDepth, fmt, false);
...
void CVTFTexture::GenerateMipmaps() {
  ...
  Assert(m_Format == IMAGE_FORMAT_RGBA8888 ||
         ...
         m_Format == IMAGE_FORMAT_RGBA32323232F);
...
void CVTFTexture::SetPostProcessingSettings(
  ...
  memset(&m_Options, 0, sizeof(m_Options));
  memcpy(&m_Options, pOptions,
         min((uint32)sizeof(m_Options), pOptions->cbSize));
  m_Options.cbSize = sizeof(m_Options);
...
Test code used to draw the cubemap into a scratchpad file. Useful for debugging,
        ...
        for ( int iFace=0; iFace < 6; iFace++ )
        {
                ...
                for ( int y=0; y < nMipHeight; y++ )
                {
                        for( int x=0; x < nMipWidth; x++ )
                        {
                                ...
                                const char *pBlock = S3TC_GetBlock( pFace1Data,
m_Format, nMipWidth/4, x/4, y/4 ); unsigned char a0 = pBlock[0]; unsigned char
a1 = pBlock[1];
                                ...
                                {
                                        data[y*nMipWidth+x] =
((8-(int)index.m_AlphaIndex)*a0 + ((int)index.m_AlphaIndex-1)*a1) / 7;
                                }
                                else
                                {
                                        if ( index.m_AlphaIndex == 6 )
                                                ...
                                                data[y*nMipWidth+x] =
((6-(int)index.m_AlphaIndex)*a0 + ((int)index.m_AlphaIndex-1)*a1) / 5;
                                }
                        }
                }
                ...
                for ( int iCorner=0; iCorner < 4; iCorner++ )
                        vCorners[iCorner] =
vertPositions[g_FaceVerts[iFace][iCorner]];

                pPad->DrawImageBW( data.Base(), nMipWidth, nMipHeight,
nMipWidth, false, true, vCorners );

                CTextParams params;
                params.m_vPos = (vCorners[0] + vCorners[1] + vCorners[2] +
vCorners[3]) / 4; params.m_bCentered = true; params.m_vColor.Init( 1, 0, 0 );
                params.m_bTwoSided = true;
                params.m_flLetterWidth = 10;

                Vector vNormal = (vCorners[1] - vCorners[0]).Cross( vCorners[2]
- vCorners[1] ); VectorNormalize( vNormal ); params.m_vPos += vNormal*5;
