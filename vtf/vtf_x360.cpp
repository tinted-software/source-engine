...
//
// Purpose: The 360 VTF file format I/O class to help simplify access to 360 VTF
// files. 360 Formatted VTF's are stored ascending 1x1 up to NxN. Disk format
// and unserialized formats are expected to be the same.
//
//=====================================================================================//
...
//-----------------------------------------------------------------------------
static bool VTFCreateCallback(const char *pSourceName, const char *pTargetName,
...
//-----------------------------------------------------------------------------
int CVTFTexture::UpdateOrCreate(const char *pFilename, const char *pPathID,
                                bool bForce) {
  return ::UpdateOrCreate(pFilename, NULL, 0, pPathID, VTFCreateCallback,
                          bForce, NULL);
}
...
int CVTFTexture::FileSize(bool bPreloadOnly, int nMipSkipCount) const {
  ...
  const ResourceEntryInfo *pEntryInfo =
...
//-----------------------------------------------------------------------------
bool CVTFTexture::LoadImageData(CUtlBuffer &buf, bool bBufferIsVolatile,
                                int nMipSkipCount) {
  ...
  // The mip levels are stored on disk ascending from smallest (1x1) to largest
  // (NxN) to allow for picmip truncated reads
...
bool CVTFTexture::ReadHeader(CUtlBuffer &buf, VTFFileHeaderX360_t &header) {
  ...
  if (header.version[0] != VTF_X360_MAJOR_VERSION &&
...
//-----------------------------------------------------------------------------
// Unserialization. Can optionally alias image components to a non-volatile
// buffer, which prevents unecessary copies.  Disk format and memory format of
// the image components are explicitly the same.
//-----------------------------------------------------------------------------
bool CVTFTexture::UnserializeFromBuffer(CUtlBuffer &buf, bool bBufferIsVolatile,
  ...
  // must first release any prior owned memory or reset aliases, otherwise
  // corruption if types intermingled
  ReleaseImageMemory();
  ReleaseResources();
  ...
  if (header.lowResImageSample[3]) {
    ...
    *(unsigned int *)m_LowResImageSample =
        *(unsigned int *)header.lowResImageSample;
  } else {
...
// Attributes...
//-----------------------------------------------------------------------------
bool CVTFTexture::IsPreTiled() const { return false; }

int CVTFTexture::MappingWidth() const { return m_nWidth << m_nMipSkipCount; }

int CVTFTexture::MappingHeight() const { return m_nHeight << m_nMipSkipCount; }

int CVTFTexture::MappingDepth() const { return m_nDepth << m_nMipSkipCount; }

int CVTFTexture::MipSkipCount() const { return m_nMipSkipCount; }
