...
// working from a commandline tool. It came from
// tools/ifm/createsfmanimation.cpp This file needs to die almost immediately +
// be replaced with a better solution that can be used both by the sfm + sfmgen.
//
//=============================================================================
...
#define ANIMATION_SET_DEFAULT_GROUP_MAPPING_FILE                               \
...
//-----------------------------------------------------------------------------
static CDmeChannelsClip *CreateChannelsClip(CDmeAnimationSet *pAnimationSet,
                                            CDmeFilmClip *pOwnerClip) {
  CDmeTrackGroup *pTrackGroup =
      pOwnerClip->FindOrAddTrackGroup("channelTrackGroup");
  if (!pTrackGroup) {
    Assert(0);
    return NULL;
  }

  CDmeTrack *pAnimSetEditorTrack =
      pTrackGroup->FindOrAddTrack("animSetEditorChannels", DMECLIP_CHANNEL);
  Assert(pAnimSetEditorTrack);

  CDmeChannelsClip *pChannelsClip = CreateElement<CDmeChannelsClip>(
      pAnimationSet->GetName(), pAnimationSet->GetFileId());
  ...
  DmeTime_t childMediaDuration =
      pOwnerClip->ToChildMediaDuration(pOwnerClip->GetDuration());
  pChannelsClip->SetDuration(childMediaDuration + CLIP_PREROLL_TIME +
                             CLIP_POSTROLL_TIME);
...
//-----------------------------------------------------------------------------
template <class T>
CDmeChannel *CreateConstantValuedLog(CDmeChannelsClip *channelsClip,
                                     const char *basename, const char *pName,
                                     CDmElement *pToElement,
                                     const char *pToAttr, const T &value) {
  char name[256];
  Q_snprintf(name, sizeof(name), "%s_%s channel", basename, pName);

  CDmeChannel *pChannel =
      CreateElement<CDmeChannel>(name, channelsClip->GetFileId());
...
//-----------------------------------------------------------------------------
static void CreateTransformChannels(CDmeTransform *pTransform,
                                    const char *pBaseName, int bi,
  ...
  CDmeChannel *pPosChannel =
      CreateElement<CDmeChannel>(name, pChannelsClip->GetFileId());
...
static void CreateAnimationLogs(CDmeChannelsClip *channelsClip,
                                CDmeGameModel *pModel, studiohdr_t *pStudioHdr,
                                const char *basename, int sequence,
  ...
  for (float t = flStartTime; t <= flStartTime + flDuration; t += flTimeStep) {
    ...
    for (bi = 0; bi < numbones; ++bi) {
      ((CDmeVector3Log *)poschannels[bi]->GetLog())
          ->InsertKey(DmeTime_t(t), pos[bi]);
      ((CDmeQuaternionLog *)rotchannels[bi]->GetLog())
          ->InsertKey(DmeTime_t(t), q[bi]);
    }
  }
}

static CDmeChannelsClip *
FindChannelsClipTargetingDmeGameModel(CDmeFilmClip *pClip,
...
static void RetimeLogData(CDmeChannelsClip *pSrcChannelsClip,
                          CDmeChannelsClip *pDstChannelsClip, CDmeLog *pLog) {
  ...
  if (srcScale != dstScale || srcOffset != dstOffset) {
    // for speed, I pulled out the math converting out of one timeframe into
    // another: t = (t/f0-o0+s0 -s1+o1)*f1
...
//-----------------------------------------------------------------------------
// Purpose: Once bones have been setup and flex channels moved, the only things
// left should be:
...
//-----------------------------------------------------------------------------
static void TransferRemainingChannels(CDmeFilmClip *shot,
                                      CDmeChannelsClip *destClip,
                                      CDmeChannelsClip *srcClip) {
  if (srcClip == destClip)
    return;
  ...
  // Now find the track which contains the srcClip and remove the srcClip from
  // the track
  for (DmAttributeReferenceIterator_t it =
           g_pDataModel->FirstAttributeReferencingElement(srcClip->GetHandle());
...
static void SetupBoneTransform(CDmeFilmClip *shot,
                               CDmeChannelsClip *srcChannelsClip,
                               CDmeChannelsClip *channelsClip,
                               CDmElement *control, CDmeGameModel *gameModel,
                               const char *basename, studiohdr_t *hdr,
                               int bonenum, const char *boneName,
  ...
  for (i = 0; i < 2; ++i) {
    char szName[512];
    Q_snprintf(szName, sizeof(szName), "%s_bone%s %d", basename, suffix[i],
               bonenum);

    CDmeChannel *pAttachChannel = NULL;
    if (srcChannelsClip) {
      pAttachChannel = FindChannelTargetingElement(srcChannelsClip, pBoneTxForm,
                                                   channelNames[i]);
    }
    ...
    if (!pAttachChannel) {
      // Create one
      pAttachChannel =
          CreateElement<CDmeChannel>(szName, channelsClip->GetFileId());
    ...
    if (bAttachToGameRecording && pOriginalLog && srcChannelsClip) {
      ...
      DmeTime_t logtime =
          channelsClip->ToChildMediaTime(channelscliptime, false);

      tLogToGlobal[0] = curtime - logtime;

      DmeTime_t attachlogtime =
          srcChannelsClip->ToChildMediaTime(channelscliptime, false);
...
//-----------------------------------------------------------------------------
static void SetupRootTransform(CDmeFilmClip *shot,
                               CDmeChannelsClip *srcChannelsClip,
                               CDmeChannelsClip *channelsClip,
                               CDmElement *control, CDmeGameModel *gameModel,
                               const char *basename,
  ...
  for (i = 0; i < 2; ++i) {
    char szName[512];
    Q_snprintf(szName, sizeof(szName), "%s_root%s channel", basename,
               suffix[i]);

    CDmeChannel *pAttachChannel = NULL;
    if (srcChannelsClip) {
      pAttachChannel = FindChannelTargetingElement(
          srcChannelsClip, gameModel->GetTransform(), channelNames[i]);
    }
...
static void CreateProceduralPreset(CDmePresetGroup *pPresetGroup,
                                   const char *pPresetName,
                                   const CDmaElementArray<CDmElement> &controls,
                                   bool bIdentity, float flForceValue = 0.5f) {
...
// Creates preset groups for new animation sets
//-----------------------------------------------------------------------------
static void CreatePresetGroups(CDmeAnimationSet *pAnimationSet,
                               const char *pModelName) {
  CDmaElementArray<CDmElement> &controls = pAnimationSet->GetControls();

  // Now create some presets
  CDmePresetGroup *pProceduralPresets =
      pAnimationSet->FindOrAddPresetGroup("procedural");
...
void LoadDefaultGroupMappings(CUtlDict<CUtlString, int> &defaultGroupMapping,
  ...
  if (!pGroupFile->LoadFromFile(g_pFullFileSystem,
  ...
  // Fill in defaults
  for (KeyValues *sub = pGroupFile->GetFirstSubKey(); sub;
       sub = sub->GetNextKey()) {
    ...
    for (KeyValues *pControl = sub->GetFirstSubKey(); pControl;
         pControl = pControl->GetNextKey()) {
...
CDmElement *
FindOrAddDefaultGroupForControls(const char *pGroupName,
                                 CDmaElementArray<CDmElement> &groups,
                                 DmFileId_t fileid) {
...
static void BuildGroupMappings(CDmeAnimationSet *pAnimationSet) {
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    CDmElement *pGroup =
        CreateElement<CDmElement>(pGroupName, pAnimationSet->GetFileId());
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    } else if (Q_stristr(pControlName, "root") ||
               Q_stristr(pControlName, "Valve")) {
      pGroupName = "Root";
    }
    ...
    CDmElement *pGroup = FindOrAddDefaultGroupForControls(
        pGroupName, groups, pAnimationSet->GetFileId());
...
void AddIllumPositionAttribute(CDmeGameModel *pGameModel) {
  ...
  CDmAttribute *pAttr =
      pGameModel->AddAttributeElement<CDmeDag>("illumPositionDag");
  Assert(pAttr);
  if (!pAttr)
    return;
...
CDmeAnimationSet *CreateAnimationSet(CDmeFilmClip *pMovie, CDmeFilmClip *pShot,
                                     CDmeGameModel *pGameModel,
                                     const char *pAnimationSetName,
                                     int nSequenceToUse,
                                     bool bAttachToGameRecording) {
  CDmeAnimationSet *pAnimationSet =
      CreateElement<CDmeAnimationSet>(pAnimationSetName, pMovie->GetFileId());
  ...
  builder.CreateAnimationSetControls(pMovie, pAnimationSet, pGameModel, pShot,
                                     pChannelsClip, bAttachToGameRecording);

  // Create animation data if there wasn't any already in the model
  if (!bAttachToGameRecording) {
    CreateConstantValuedLog(pChannelsClip, pAnimationSetName, "skin",
                            pGameModel, "skin", (int)0);
    CreateConstantValuedLog(pChannelsClip, pAnimationSetName, "body",
                            pGameModel, "body", (int)0);
    CreateConstantValuedLog(pChannelsClip, pAnimationSetName, "sequence",
                            pGameModel, "sequence", (int)0);

    CreateAnimationLogs(pChannelsClip, pGameModel, hdr, pAnimationSetName,
                        nSequenceToUse, 0.0f, 1.0f, 0.05f);
  }

  CDmeChannelsClip *srcChannelsClip =
      FindChannelsClipTargetingDmeGameModel(pShot, pGameModel);
  CDmaElementArray<> &controls = pAnimationSet->GetControls();
  ...
  // First the root transform
  {
    ...
    SetupRootTransform(pShot, srcChannelsClip, pChannelsClip, ctrl, pGameModel,
                       pAnimationSetName, bAttachToGameRecording);
  }
  ...
  // Now add the bone transforms as well
  {
    ...
    for (int b = 0; b < numbones; ++b) {
      ...
      ctrl->SetValue<bool>("transform", true);
      controls.AddToTail(ctrl);
      SetupBoneTransform(pShot, srcChannelsClip, pChannelsClip, ctrl,
                         pGameModel, pAnimationSetName, hdr, b, name,
                         bAttachToGameRecording);
    }
  }

  // Now copy all remaining logs, and retime them, over to the animation set
  // channels clip...
  if (srcChannelsClip) {
  ...
  // Builds the preset groups displayed in the upper left of the animation set
  // panel
