...
//-----------------------------------------------------------------------------
static void ComputeExportChannelScaleBias(double *pScale, DmeTime_t *pBias,
                                          ExportInfo_t &info,
                                          CDmeChannel *pChannel) {
  DmeClipStack_t channelToGlobal;
  if (pChannel->BuildClipStack(&channelToGlobal, info.m_pMovie, info.m_pShot)) {
    DmeTime_t tOffset =
        CDmeClip::FromChildMediaTime(channelToGlobal, DMETIME_ZERO, false);
    DmeTime_t tScale =
        CDmeClip::FromChildMediaTime(channelToGlobal, DmeTime_t(1.0f), false);
    *pBias = tOffset - info.m_pShot->GetStartTime();
    *pScale = (tScale - tOffset).GetSeconds();
  }
}

static void GetExportTimeRange(DmeTime_t *pExportStart, DmeTime_t *pExportEnd,
                               CDmeFilmClip *pShot) {
...
//-----------------------------------------------------------------------------
static void AddLogLayerForExport(ExportInfo_t &info, CDmElement *pRoot,
  ...
  pLogLayer->InsertKeyFromLayer(info.m_tExportStart, pLog->GetLayer(0),
                                tStartTime);
...
static void ExportAnimations(ExportInfo_t &info, CDmElement *pRoot) {
  ...
  const CDmaElementArray<CDmElement> &controls =
      info.m_pAnimationSet->GetControls();
  ...
  for (int i = 0; i < nControlCount; ++i) {
    ...
    if (bIsStereo) {
      CDmeChannel *pValueChannel =
          pControl->GetValueElement<CDmeChannel>("valuechannel");
      CDmeChannel *pBalanceChannel =
      ...
      if (pValueLog && pBalanceLog && pValueLog->GetNumLayers() != 0 &&
          pBalanceLog->GetNumLayers() != 0) {
        DmeTime_t tValueBias, tBalanceBias;
        double flValueScale, flBalanceScale;
        ComputeExportChannelScaleBias(&flValueScale, &tValueBias, info,
                                      pValueChannel);
        ComputeExportChannelScaleBias(&flBalanceScale, &tBalanceBias, info,
                                      pBalanceChannel);
        ...
        DmeTime_t tStartTime =
            (info.m_tExportStart - tValueBias) / flValueScale;
        DmeTime_t tEndTime = (info.m_tExportEnd - tValueBias) / flValueScale;
        pValueLogLayer->InsertKeyFromLayer(info.m_tExportStart,
                                           pValueLog->GetLayer(0), tStartTime);
        pValueLogLayer->InsertKeyFromLayer(info.m_tExportEnd,
                                           pValueLog->GetLayer(0), tEndTime);

        tStartTime = (info.m_tExportStart - tBalanceBias) / flBalanceScale;
        tEndTime = (info.m_tExportEnd - tBalanceBias) / flBalanceScale;
        pBalanceLogLayer->InsertKeyFromLayer(
            info.m_tExportStart, pBalanceLog->GetLayer(0), tStartTime);
        pBalanceLogLayer->InsertKeyFromLayer(
            info.m_tExportEnd, pBalanceLog->GetLayer(0), tEndTime);

        pValueLogLayer->RemoveKeysOutsideRange(info.m_tExportStart,
                                               info.m_tExportEnd);
        pBalanceLogLayer->RemoveKeysOutsideRange(info.m_tExportStart,
                                                 info.m_tExportEnd);

        char pControlName[512];
        Q_snprintf(pControlName, sizeof(pControlName), "value_%s",
                   pControl->GetName());
        pValueLogLayer->SetName(pControlName);
        animations.AddToTail(pValueLogLayer);

        Q_snprintf(pControlName, sizeof(pControlName), "balance_%s",
                   pControl->GetName());
    ...
    if (pControl->GetValue<bool>("multi")) {
      char pControlName[512];
      Q_snprintf(pControlName, sizeof(pControlName), "multi_%s",
                 pControl->GetName());
      CDmeChannel *pMultiChannel =
...
//-----------------------------------------------------------------------------
static void ExportSounds(ExportInfo_t &info, CDmElement *pRoot, CDmeClip *pClip,
                         DmeTime_t tOffset) {
  ...
  for (int i = 0; i < gc; ++i) {
    ...
    DMETRACKGROUP_FOREACH_CLIP_TYPE_START(CDmeSoundClip, pTrackGroup, pTrack,
                                          pSoundClip)
    ...
    DmeTime_t tStart =
        CDmeClip::FromChildMediaTime(soundToGlobal, DMETIME_ZERO, false);
    DmeTime_t tEnd = CDmeClip::FromChildMediaTime(
        soundToGlobal, pSoundClip->GetDuration(), false);
    tStart -= tOffset;
    tEnd -= tOffset;
...
static void ExportSounds_R(ExportInfo_t &info, CDmElement *pRoot,
                           CDmeClip *pClip, DmeTime_t tOffset) {
  ...
  for (int i = 0; i < gc; ++i) {
    ...
    DmeTime_t tStart =
        CDmeClip::FromChildMediaTime(childToGlobal, DMETIME_ZERO, false);
    DmeTime_t tEnd = CDmeClip::FromChildMediaTime(childToGlobal,
                                                  pChild->GetDuration(), false);
    tStart -= tOffset;
    tEnd -= tOffset;
    if (tStart >= info.m_tExportEnd || tEnd <= info.m_tExportStart)
      continue;
...
// Exports an .fac file
//-----------------------------------------------------------------------------
bool ExportFacialAnimation(const char *pFileName, CDmeFilmClip *pMovie,
                           CDmeFilmClip *pShot,
  ...
  CDmeGameModel *pGameModel =
      pAnimationSet->GetValueElement<CDmeGameModel>("gameModel");
  if (pGameModel) {
