...
// Constructor
//-----------------------------------------------------------------------------
CSFMSession::CSFMSession() { m_hRoot = NULL; }

//-----------------------------------------------------------------------------
// Sets the root
//-----------------------------------------------------------------------------
void CSFMSession::SetRoot(CDmElement *pRoot) { m_hRoot = pRoot; }
...
void CSFMSession::Init() {
  ...
  // a movie currently consists of: (this is all just temporary until clips take
  // over more completely) a generic "root" node
...
void CSFMSession::CreateSessionSettings() {
  ...
  if (!pSettings) {
    pSettings =
        CreateElement<CDmElement>("sessionSettings", m_hRoot->GetFileId());
  ...
  if (!pSettings->HasAttribute("timeSelection")) {
    ts =
        CreateElement<CDmeTimeSelection>("timeSelection", m_hRoot->GetFileId());
  ...
  if (pSettings->HasAttribute("frameRate")) {
    ...
    // remove this from the base level settings area since we're going to add it
    // in renderSettings
  ...
  if (!pSettings->HasAttribute("proceduralPresets")) {
    CDmeProceduralPresetSettings *ps =
        CreateElement<CDmeProceduralPresetSettings>("proceduralPresets",
                                                    m_hRoot->GetFileId());
...
void CSFMSession::CreateRenderSettings(CDmElement *pSettings,
                                       float flLegacyFramerate) {
  pSettings->AddAttribute("renderSettings", AT_ELEMENT);

  CDmElement *pRenderSettings =
      pSettings->GetValueElement<CDmElement>("renderSettings");
  if (!pRenderSettings) {
    pRenderSettings =
        CreateElement<CDmElement>("renderSettings", pSettings->GetFileId());
    pSettings->SetValue("renderSettings", pRenderSettings);
  }
  Assert(pRenderSettings);

  pRenderSettings->InitValue("frameRate",
                             flLegacyFramerate); // Default framerate
  pRenderSettings->InitValue("lightAverage",
                             0); // Don't light average by default
  pRenderSettings->InitValue("showFocalPlane",
                             0); // Don't show focal plane by default
...
//-----------------------------------------------------------------------------
void CSFMSession::CreateProgressiveRefinementSettings(
    CDmElement *pRenderSettings) {
  // Do we already have Progressive refinement settings?
  CDmElement *pRefinementSettings =
      pRenderSettings->GetValueElement<CDmElement>("ProgressiveRefinement");
  if (!pRefinementSettings) {
    pRefinementSettings = CreateElement<CDmElement>(
        "ProgressiveRefinementSettings", pRenderSettings->GetFileId());
...
CDmeCamera *CSFMSession::CreateCamera(const DmeCameraParams_t &params) {
  CDmeCamera *pCamera =
      CreateElement<CDmeCamera>(params.name, m_hRoot->GetFileId());
...
//-----------------------------------------------------------------------------
CDmeDag *CSFMSession::FindOrCreateScene(CDmeFilmClip *pShot,
...
//-----------------------------------------------------------------------------
CDmeGameModel *CSFMSession::CreateEditorGameModel(studiohdr_t *hdr,
  ...
  CDmeGameModel *pGameModel =
      CreateElement<CDmeGameModel>(pGameModelName, m_hRoot->GetFileId());

  char pRelativeModelsFileName[MAX_PATH];
  Q_ComposeFileName("models", hdr->pszName(), pRelativeModelsFileName,
                    sizeof(pRelativeModelsFileName));
