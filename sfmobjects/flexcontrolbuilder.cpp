...
//-----------------------------------------------------------------------------
// This builds a list of the desired flex controllers we need to have controls
// for by the time we're all done with this enormous process.
//-----------------------------------------------------------------------------
void CFlexControlBuilder::BuildDesiredFlexControlList(
  ...
  for (LocalFlexController_t i = LocalFlexController_t(0); i < nCount; ++i) {
    ...
    Q_strncpy(info.m_pFlexControlName, pFlex->pszName(),
              sizeof(info.m_pFlexControlName));
    info.m_nGlobalIndex =
        g_pGlobalFlexController->FindGlobalFlexController(pFlex->pszName());
...
void CFlexControlBuilder::BuildDesiredControlList(CDmeGameModel *pGameModel) {
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    if (!Q_strnicmp("right_", pFlexName, 6) && (i < nCount - 1)) {
      ...
      Q_strncpy(controlInfo.m_pControlName, pFlexName + 6,
                sizeof(controlInfo.m_pControlName));
      ...
      Q_strncpy(controlInfo.m_pControlName, pFlexName,
                sizeof(controlInfo.m_pControlName));
    ...
    controlInfo.m_bIsMulti =
        (i + 1 < nCount) &&
        !Q_strnicmp("multi_", m_FlexControllerInfo[i + 1].m_pFlexControlName,
                    6);
    if (controlInfo.m_bIsMulti) {
      ...
      controlInfo.m_pDefaultValue[CONTROL_MULTILEVEL] =
          multiInfo.m_flDefaultValue;
...
//-----------------------------------------------------------------------------
int CFlexControlBuilder::FindDesiredFlexController(
    const char *pFlexControllerName) const {
  int nCount = m_FlexControllerInfo.Count();
  for (int i = 0; i < nCount; ++i) {
    if (!Q_stricmp(pFlexControllerName,
                   m_FlexControllerInfo[i].m_pFlexControlName))
      return i;
  }
  return -1;
}
...
void CFlexControlBuilder::RemoveChannelFromClips(CDmeChannel *pChannel) {
  ...
  for (int i = 0; i < CONTROL_CHANNEL_ATTRIBUTE_COUNT; ++i) {
    UtlSymId_t symChannelControl =
        g_pDataModel->GetSymbol(s_pChannelControls[i]);
    CDmElement *pControl =
        FindReferringElement<CDmElement>(pChannel, symChannelControl);
    if (pControl) {
...
//-----------------------------------------------------------------------------
void CFlexControlBuilder::RemoveStereoOpFromSet(
    CDmeBalanceToStereoCalculatorOperator *pSteroOp) {
  ...
  CDmeAnimationSet *pAnimationSet =
      FindReferringElement<CDmeAnimationSet>(pSteroOp, symOperators);
  if (pAnimationSet) {
...
//-----------------------------------------------------------------------------
void CFlexControlBuilder::CleanupExistingFlexController(
    CDmeGameModel *pGameModel, CDmeGlobalFlexControllerOperator *pOp) {
  ...
  pStereoOp = CastElement<CDmeBalanceToStereoCalculatorOperator>(
      pChannel->GetFromElement());
  ...
  // If we have a stereo op, then blow away all channels targetting that stereo
  // op
  DmAttributeReferenceIterator_t i =
      g_pDataModel->FirstAttributeReferencingElement(pStereoOp->GetHandle());
...
bool RemoveChannelIfUnused(CDmeChannel *pChannel,
...
// finds controls whose channels don't point to anything anymore, and deletes
// both the channels and the control
void CFlexControlBuilder::RemoveUnusedControlsAndChannels(
    CDmeAnimationSet *pAnimationSet, CDmeChannelsClip *pChannelsClip) {
  ...
  for (int i = nControls - 1; i >= 0; --i) {
    ...
    if (pControl) {
      bool bRemoved = RemoveChannelIfUnused(
          pControl->GetValueElement<CDmeChannel>("channel"), pChannelsClip);
      bRemoved = bRemoved ||
                 RemoveChannelIfUnused(
                     pControl->GetValueElement<CDmeChannel>("valuechannel"),
                     pChannelsClip);
      bRemoved = bRemoved ||
                 RemoveChannelIfUnused(
                     pControl->GetValueElement<CDmeChannel>("balancechannel"),
                     pChannelsClip);
      bRemoved = bRemoved ||
                 RemoveChannelIfUnused(pControl->GetValueElement<CDmeChannel>(
                                           "multilevelchannel"),
                                       pChannelsClip);
...
//-----------------------------------------------------------------------------
// This removes existing controls on the animationset that aren't in the desired
// state
//-----------------------------------------------------------------------------
void CFlexControlBuilder::RemoveUnusedExistingFlexControllers(
    CDmeGameModel *pGameModel) {
  // These are the current flex controllers
  // NOTE: Name of these controllers should match the names of the flex
  // controllers
  int nCount = pGameModel->NumGlobalFlexControllers();
  for (int i = nCount; --i >= 0;) {
    CDmeGlobalFlexControllerOperator *pOp =
        pGameModel->GetGlobalFlexController(i);
    Assert(pOp);
...
//-----------------------------------------------------------------------------
void CFlexControlBuilder::GetExistingMonoLog(
    ExistingLogInfo_t *pExistingLog, CDmeFilmClip *pClip,
    CDmeGlobalFlexControllerOperator *pMonoOp) {
  pExistingLog->m_pLog = NULL;

  const static UtlSymId_t symToElement = g_pDataModel->GetSymbol("toElement");
  CDmeChannel *pMonoChannel =
      FindReferringElement<CDmeChannel>(pMonoOp, symToElement);
  if (!pMonoChannel)
    return;
  ...
  if (ComputeChannelTimeTransform(&pExistingLog->m_GlobalOffset,
                                  &pExistingLog->m_flGlobalScale, pClip,
                                  pMonoChannel)) {
...
//-----------------------------------------------------------------------------
CDmeChannelsClip *
CFlexControlBuilder::FindChannelsClipContainingChannel(CDmeFilmClip *pClip,
                                                       CDmeChannel *pSearch) {
...
//-----------------------------------------------------------------------------
void CFlexControlBuilder::ComputeChannelTimeTransform(
    DmeTime_t *pOffset, double *pScale, CDmeChannelsClip *pChannelsClip) {
  ...
  *pOffset = CDmeClip::FromChildMediaTime(srcStack, DMETIME_ZERO, false);
  DmeTime_t duration =
      CDmeClip::FromChildMediaTime(srcStack, DmeTime_t(10000), false);
  duration -= *pOffset;
  *pScale = duration.GetSeconds();
}

bool CFlexControlBuilder::ComputeChannelTimeTransform(DmeTime_t *pOffset,
                                                      double *pScale,
                                                      CDmeFilmClip *pClip,
                                                      CDmeChannel *pChannel) {
  CDmeChannelsClip *pChannelsClip =
      FindChannelsClipContainingChannel(pClip, pChannel);
  if (!pChannelsClip)
    return false;
...
//-----------------------------------------------------------------------------
void CFlexControlBuilder::GetExistingVBLog(
    ExistingLogInfo_t *pLogs, CDmeFilmClip *pClip,
    CDmeBalanceToStereoCalculatorOperator *pStereoOp) {
  // Stereo operators always have value/balance logs attached
  DmAttributeReferenceIterator_t i =
      g_pDataModel->FirstAttributeReferencingElement(pStereoOp->GetHandle());
  for (; i != DMATTRIBUTE_REFERENCE_ITERATOR_INVALID;
    ...
    if (!ComputeChannelTimeTransform(&pLogs[nLogIndex].m_GlobalOffset,
                                     &pLogs[nLogIndex].m_flGlobalScale, pClip,
                                     pChannel))
      continue;
...
static void AddKeyToLogs(CDmeTypedLog<float> *valueLog,
                         CDmeTypedLog<float> *balanceLog,
                         const DmeTime_t &keyTime, float lval, float rval) {
...
static void ConvertLRToVBLog(CDmeFloatLog *pValueLog, CDmeFloatLog *pBalanceLog,
                             CDmeFloatLog *pLeftLog, CDmeFloatLog *pRightLog,
                             DmeTime_t rightOffset, double flRightScale) {
  ...
  while (nLeft < lc || nRight < rc) {
    ...
    DmeTime_t leftKeyTime =
        bUseLeft ? pLeftLog->GetKeyTime(nLeft) : DMETIME_MAXTIME;
    DmeTime_t rightKeyTime =
        bUseRight ? pRightLog->GetKeyTime(nRight) : DMETIME_MAXTIME;
...
void CFlexControlBuilder::ConvertExistingLRLogs(ExistingLogInfo_t *pLogs,
  ...
  // Compute a scale + offset to transform the right log to get it in the same
  // space as the left log
  DmeTime_t leftOffset, rightOffset;
  double flLeftScale, flRightScale;
  if (!ComputeChannelTimeTransform(&leftOffset, &flLeftScale, pClip,
                                   pLeftChannel))
    return;
  if (!ComputeChannelTimeTransform(&rightOffset, &flRightScale, pClip,
                                   pRightChannel))
    return;
  ...
  pLogs[CONTROL_BALANCE].m_GlobalOffset =
      leftOffset; // NOTE: This is correct! All logs are transformed into left
                  // channel time
...
//-----------------------------------------------------------------------------
void CFlexControlBuilder::GetExistingStereoLog(
    ExistingLogInfo_t *pLogs, CDmeFilmClip *pClip,
  ...
  const static UtlSymId_t symToElement = g_pDataModel->GetSymbol("toElement");
  CDmeChannel *pChannel =
      FindReferringElement<CDmeChannel>(pRightOp, symToElement);
  if (!pChannel)
    return;

  // Sometimes a stereo op will be read from by this channel
  CDmeBalanceToStereoCalculatorOperator *pStereoOp =
      CastElement<CDmeBalanceToStereoCalculatorOperator>(
          pChannel->GetFromElement());
  if (pStereoOp) {
  ...
  CDmeChannel *pLeftChannel =
      FindReferringElement<CDmeChannel>(pLeftOp, symToElement);
  if (!pLeftChannel)
    return;
...
//-----------------------------------------------------------------------------
void CFlexControlBuilder::FixupExistingFlexControlLogList(
    CDmeFilmClip *pCurrentClip, CDmeGameModel *pGameModel) {
  ...
  for (int gi = 0; gi < nTrackGroups; ++gi) {
    ...
    for (int ci = 0; ci < nChannels; ++ci) {
      ...
      CDmeGlobalFlexControllerOperator *pOp =
          CastElement<CDmeGlobalFlexControllerOperator>(
              pChannel->GetToElement());
      if (!pOp)
        continue;
      ...
      CDmeGlobalFlexControllerOperator *pFoundOp =
          pGameModel->FindGlobalFlexController(nGlobalIndex);
      if (pFoundOp == pOp)
        continue;
      ...
      if (!pFoundOp) {
        Msg("adding missing flex controller %d %s\n", nGlobalIndex,
            pOp->GetName());
        pFoundOp =
            pGameModel->AddGlobalFlexController(pOp->GetName(), nGlobalIndex);
      }
...
//-----------------------------------------------------------------------------
void CFlexControlBuilder::BuildExistingFlexControlLogList(
    CDmeFilmClip *pCurrentClip, CDmeGameModel *pGameModel) {
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    if (info.m_bIsStereo) {
      ...
      CDmeGlobalFlexControllerOperator *pLeftOp =
          pGameModel->FindGlobalFlexController(pLeftInfo->m_nGlobalIndex);
      if (pRightOp && pLeftOp) {
        Msg("replacing stereo flex controllers %s and %s\n",
            pRightOp->GetName(), pRightOp->GetName());

        GetExistingStereoLog(info.m_pExistingLog, pCurrentClip, pRightOp,
                             pLeftOp);
      ...
      CDmeGlobalFlexControllerOperator *pMonoOp =
          pGameModel->FindGlobalFlexController(pInfo->m_nGlobalIndex);
      if (pMonoOp) {
        Msg("replacing mono flex controller %s\n", pMonoOp->GetName());

        GetExistingMonoLog(&info.m_pExistingLog[CONTROL_VALUE], pCurrentClip,
                           pMonoOp);
    ...
    if (info.m_bIsMulti) {
      ...
      if (pMultiOp) {
        ...
        GetExistingMonoLog(&info.m_pExistingLog[CONTROL_MULTILEVEL],
                           pCurrentClip, pMultiOp);
...
void CFlexControlBuilder::BuildFlexControllerOps(
    CDmeGameModel *pGameModel, CDmeChannelsClip *pChannelsClip,
    ControlInfo_t &info, ControlField_t field) {
  const FlexOpInfo_t &flexInfo =
      s_pFlexOpInfo[(field == CONTROL_VALUE) ? 0 : 1];

  // Get the global flex controller name and index
  const FlexControllerInfo_t &fcInfo =
      m_FlexControllerInfo[info.m_pControllerIndex[field]];

  // Create operator which drives facial flex setting
  CDmeGlobalFlexControllerOperator *pFlexControllerOp =
      pGameModel->AddGlobalFlexController(fcInfo.m_pFlexControlName,
                                          fcInfo.m_nGlobalIndex);

  // Create a channel which passes from the control value to the global flex
  // controller
  char pName[256];
  Q_snprintf(pName, sizeof(pName), "%s_flex_channel",
             fcInfo.m_pFlexControlName);
  info.m_ppControlChannel[field] = pChannelsClip->CreatePassThruConnection(
      pName, info.m_pControl, flexInfo.m_pControlAttributeName,
      pFlexControllerOp, "flexWeight");

  // NOTE: The animation set slider panel looks for these custom attributes
  Q_snprintf(pName, sizeof(pName), "%schannel",
             flexInfo.m_pControlLinkAttributeName);
...
void CFlexControlBuilder::BuildStereoFlexControllerOps(
    CDmeAnimationSet *pAnimationSet, CDmeGameModel *pGameModel,
  ...
  CDmeBalanceToStereoCalculatorOperator *pStereoCalcOp =
      CreateElement<CDmeBalanceToStereoCalculatorOperator>(
          info.m_pControlName, pAnimationSet->GetFileId());
  operators.AddToTail(pStereoCalcOp->GetHandle());

  pStereoCalcOp->SetValue<float>("value", info.m_pDefaultValue[CONTROL_VALUE]);
  pStereoCalcOp->SetValue<float>("balance",
                                 info.m_pDefaultValue[CONTROL_BALANCE]);

  // Connect channels from animation set controls to balance operator to flex
  ...
  for (int i = 0; i < 2; ++i) {
    // Get the global flex controller name and index
    const FlexControllerInfo_t &fcInfo =
        m_FlexControllerInfo[info.m_pControllerIndex[i]];
    ...
    // Now create a channel which connects the output of the stereo op to the
    // flex controller op
    Q_snprintf(pResultName, sizeof(pResultName), "result_%s",
               s_pStereoOutputPrefix[i]);
    Q_snprintf(pChannelName, sizeof(pChannelName), "%s_flex_channel",
               fcInfo.m_pFlexControlName);
    pChannelsClip->CreatePassThruConnection(pChannelName, pStereoCalcOp,
                                            pResultName, pFlexControllerOp,
                                            "flexWeight");

    // Create a channel which connects the control to the input of the stereo op
    Q_snprintf(pChannelName, sizeof(pChannelName), "%s_%s_channel",
               info.m_pControlName, s_pStereoInputPrefix[i]);
    info.m_ppControlChannel[i] = pChannelsClip->CreatePassThruConnection(
        pChannelName, info.m_pControl, s_pStereoInputPrefix[i], pStereoCalcOp,
        s_pStereoInputPrefix[i]);

    // NOTE: The animation set slider panel looks for these custom attributes
    Q_snprintf(pChannelName, sizeof(pChannelName), "%schannel",
               s_pStereoInputPrefix[i]);
...
//-----------------------------------------------------------------------------
// Build the infrastructure of the ops that connect that control to the
// dmegamemodel
//-----------------------------------------------------------------------------
void CFlexControlBuilder::AttachControlsToGameModel(
    CDmeAnimationSet *pAnimationSet, CDmeGameModel *pGameModel,
    CDmeChannelsClip *pChannelsClip) {
  // Build the infrastructure of the ops that connect that control to the
  // dmegamemodel
  ...
  for (int i = 0; i < c; ++i) {
    ControlInfo_t &info = m_ControlInfo[i];
    if (info.m_bIsStereo) {
      BuildStereoFlexControllerOps(pAnimationSet, pGameModel, pChannelsClip,
                                   info);
    } else {
    ...
    if (info.m_bIsMulti) {
      BuildFlexControllerOps(pGameModel, pChannelsClip, info,
                             CONTROL_MULTILEVEL);
    }
  }
}
...
void CFlexControlBuilder::InitializeFlexControl(ControlInfo_t &info) {
  ...
  pControl->SetValue<float>("defaultValue",
                            info.m_pDefaultValue[CONTROL_VALUE]);
  pControl->SetValue<float>("defaultBalance",
                            info.m_pDefaultValue[CONTROL_BALANCE]);
  pControl->SetValue<float>("defaultMultilevel",
                            info.m_pDefaultValue[CONTROL_MULTILEVEL]);

  // These can keep their value if they already exist
  pControl->InitValue<float>("value", info.m_pDefaultValue[CONTROL_VALUE]);
  if (info.m_bIsStereo) {
    pControl->InitValue<float>("balance",
                               info.m_pDefaultValue[CONTROL_BALANCE]);
  } else {
    pControl->RemoveAttribute("balance");
  }
  if (info.m_bIsMulti) {
    pControl->InitValue<float>("multilevel",
                               info.m_pDefaultValue[CONTROL_MULTILEVEL]);
  } else {
...
//-----------------------------------------------------------------------------
void CFlexControlBuilder::SetupLogs(CDmeChannelsClip *pChannelsClip,
  ...
  for (int i = 0; i < c; ++i) {
    ...
    for (int j = 0; j < CONTROL_FIELD_COUNT; ++j) {
      ...
      CDmeFloatLog *pFloatLog =
          CastElement<CDmeFloatLog>(info.m_ppControlChannel[j]->GetLog());
      if (bUseExistingLogs && info.m_pExistingLog[j].m_pLog) {
        ...
        double flTotalScale =
            info.m_pExistingLog[j].m_flGlobalScale * flOOTargetScale;
        DmeTime_t totalOffset =
            info.m_pExistingLog[j].m_GlobalOffset - targetOffset;
...
//-----------------------------------------------------------------------------
void CFlexControlBuilder::CreateAnimationSetControls(
    CDmeFilmClip *pMovie, CDmeAnimationSet *pAnimationSet,
  ...
  // First, look at the current mdl and determine what are its low-level
  // flexcontrollers [these are the outputs eventually driven by the animation
  // set controls]
  BuildDesiredFlexControlList(pGameModel);

  // Next, based on the list of low-level flexcontrollers, determine a
  // high-level set of input controls
  BuildDesiredControlList(pGameModel);

  // Next look at what the animation set currently thinks are the input controls
  // + low-level flexcontrollers and remove the unused ones
  ...
  // Build channels + control logis attaching the input controls to the low
  // level flex controls
  AttachControlsToGameModel(pAnimationSet, pGameModel, pDestClip);

  // Attach existing logs to the new input controls created in
  // CreateFlexControls
