...
static const char
    *s_pDefaultAttributeValueNames[LOG_PREVIEW_FLEX_CHANNEL_COUNT] = {
...
public:
  ...
  virtual void GetAPIInfo(int index, CUtlString *pPrintName,
                          PE_APITYPE *pAPIType);
  virtual void Extract(const PE_APITYPE &apiType, ExtractDesc_t &info,
...
private:
  ...
  bool GetWaveFormat(const char *filename, CUtlBuffer *pFormat, int *pDataSize,
                     CSentence &sentence, bool &bGotSentence);
  void LogPhonemes(int nItemIndex, ExtractDesc_t &info);
  void ClearInterstitialSpaces(CDmeChannelsClip *pChannelsClip,
                               CUtlDict<LogPreview_t *, int> &controlLookup,
                               ExtractDesc_t &info);

  void StampControlValueLogs(CDmePreset *preset, DmeTime_t tHeadPosition,
                             float flIntensity,
                             CUtlDict<LogPreview_t *, int> &controlLookup);
  void WriteCurrentValuesIntoLogLayers(
      DmeTime_t tHeadPosition,
      const CUtlDict<LogPreview_t *, int> &controlLookup);
  void WriteDefaultValuesIntoLogLayers(
      DmeTime_t tHeadPosition,
      const CUtlDict<LogPreview_t *, int> &controlLookup);
  void BuildPhonemeLogList(CUtlVector<LogPreview_t> &list,
                           CUtlVector<CDmeLog *> &logs);
  CDmeChannelsClip *
  FindFacialChannelsClip(const CUtlVector<LogPreview_t> &list);
  void BuildPhonemeToPresetMapping(
      const CUtlVector<CBasePhonemeTag *> &stream, CDmeAnimationSet *pSet,
...
void CSFMPhonemeExtractor::GetAPIInfo(int index, CUtlString *pPrintName,
                                      PE_APITYPE *pAPIType) {
  Assert(pPrintName);
  Assert(pAPIType);
...
bool CSFMPhonemeExtractor::GetWaveFormat(const char *filename, CUtlBuffer *pBuf,
                                         int *pDataSize, CSentence &sentence,
                                         bool &bGotSentence) {
...
bool CSFMPhonemeExtractor::GetSentence(CDmeGameSound *gameSound,
                                       CSentence &sentence) {
  const char *filename = gameSound->m_SoundName.Get();
  Assert(filename && filename[0]);

  char soundname[512];
  // Note, calling PSkipSoundChars to remove any decorator characters used by
  // the engine!!!
  Q_snprintf(soundname, sizeof(soundname), "sound/%s",
             PSkipSoundChars(filename));
  Q_FixSlashes(soundname);

  char fullpath[512];
  g_pFullFileSystem->RelativePathToFullPath(soundname, "GAME", fullpath,
                                            sizeof(fullpath));
...
static void BuildPhonemeStream(CSentence &in,
                               CUtlVector<CBasePhonemeTag *> &list) {
...
static bool SaveSentenceToWavFile(const char *pWavFile, CSentence &sentence) {
  ...
  // NOTE:  Put this in it's own scope so that the destructor for outfileRFF
...
//-----------------------------------------------------------------------------
void CSFMPhonemeExtractor::Extract(const PE_APITYPE &apiType,
                                   ExtractDesc_t &info,
                                   bool bWritePhonemesToWavFiles) {
  if (!info.m_pSet)
    return;
  ...
  for (nWorkItem = 0; nWorkItem < info.m_WorkList.Count(); ++nWorkItem) {
    ...
    // Note, calling PSkipSoundChars to remove any decorator characters used by
    // the engine!!!
    Q_snprintf(pSoundName, sizeof(pSoundName), "sound/%s",
               PSkipSoundChars(pFileName));
    Q_FixSlashes(pSoundName);

    char pFullPath[512];
    g_pFullFileSystem->RelativePathToFullPath(pSoundName, "GAME", pFullPath,
                                              sizeof(pFullPath));
    ...
    WAVEFORMATEX *format;
    int nDataSize;
    if (!GetWaveFormat(pSoundName, &buf, &nDataSize, workItem.m_Sentence,
                       workItem.m_bSentenceValid))
      continue;

    format = (WAVEFORMATEX *)buf.Base();

    if (!(format->wBitsPerSample > (1 << 3))) {
      // Have to warn and early-out here to avoid crashing with "integer divide
      // by zero" below
    ...
    in.CreateEventWordDistribution(workItem.m_sHintText.String(),
                                   workItem.m_flDuration);
    if (!workItem.m_bUseSentence || !workItem.m_bSentenceValid) {
      extractor.extractor->Extract(
          pFullPath,
          (int)(workItem.m_flDuration * flSampleRate * flTrueSampleSize), Msg,
          in, out);

      // Tracker 57389:
      // Total hack to fix a bug where the Lipsinc extractor is messing up the #
      // channels on 16 bit stereo waves
      if (apiType == SPEECH_API_LIPSINC && nChannels == 2 &&
          nBitsPerSample == 16) {
...
//
//-----------------------------------------------------------------------------
static bool UniquePhonemeLessFunc(CBasePhonemeTag *const &lhs,
                                  CBasePhonemeTag *const &rhs) {
  return lhs->GetPhonemeCode() < rhs->GetPhonemeCode();
}

void CSFMPhonemeExtractor::BuildPhonemeToPresetMapping(
    const CUtlVector<CBasePhonemeTag *> &stream, CDmeAnimationSet *pSet,
    CDmePresetGroup *pPresetGroup,
    CUtlDict<CDmePreset *, unsigned short> &phonemeToPresetDict) {
  int i;
  ...
  for (i = uniquePhonemes.FirstInorder(); i != uniquePhonemes.InvalidIndex();
       i = uniquePhonemes.NextInorder(i)) {
    ...
    char remappedph[32];
    // By default we search for a preset name p_xxx where xxx is the phoneme
    // string
    ...
    if (mapping) {
      Q_strncpy(remappedph, mapping->GetValueString("preset"),
                sizeof(remappedph));
    }
...
//-----------------------------------------------------------------------------
CDmeChannelsClip *CSFMPhonemeExtractor::FindFacialChannelsClip(
    const CUtlVector<LogPreview_t> &list) {
  CDmeChannelsClip *pChannelsClip = NULL;

  int i;
  for (i = list.Count() - 1; i >= 0; --i) {
    const LogPreview_t &lp = list[i];
    CDmeChannelsClip *check = FindAncestorReferencingElement<CDmeChannelsClip>(
        (CDmElement *)lp.m_hChannels[0].Get());
...
//-----------------------------------------------------------------------------
void CSFMPhonemeExtractor::BuildPhonemeLogList(CUtlVector<LogPreview_t> &list,
                                               CUtlVector<CDmeLog *> &logs) {
...
//-----------------------------------------------------------------------------
void CSFMPhonemeExtractor::WriteDefaultValuesIntoLogLayers(
    DmeTime_t tHeadPosition,
    const CUtlDict<LogPreview_t *, int> &controlLookup) {
  // Write a zero into all relevant log layers
  for (int j = controlLookup.First(); j != controlLookup.InvalidIndex();
       j = controlLookup.Next(j)) {
    ...
    for (int chIndex = 0; chIndex < LOG_PREVIEW_FLEX_CHANNEL_COUNT; ++chIndex) {
      ...
      CDmeFloatLogLayer *pLayer =
          pFloatLog->GetLayer(pFloatLog->GetTopmostLayer());
      if (!pLayer)
        continue;

      float flDefaultValue =
          pControl->GetValue<float>(s_pDefaultAttributeValueNames[chIndex]);
...
//-----------------------------------------------------------------------------
void CSFMPhonemeExtractor::WriteCurrentValuesIntoLogLayers(
    DmeTime_t tHeadPosition,
    const CUtlDict<LogPreview_t *, int> &controlLookup) {
  // Write a zero into all relevant log layers
  for (int j = controlLookup.First(); j != controlLookup.InvalidIndex();
...
//-----------------------------------------------------------------------------
void CSFMPhonemeExtractor::StampControlValueLogs(
    CDmePreset *preset, DmeTime_t tHeadPosition, float flIntensity,
    CUtlDict<LogPreview_t *, int> &controlLookup) {
  // Now walk the logs required by the preset
  const CDmrElementArray<CDmElement> &controlValues =
      preset->GetControlValues();
  for (int j = 0; j < controlValues.Count(); ++j) {
    ...
    for (int chIndex = 0; chIndex < LOG_PREVIEW_FLEX_CHANNEL_COUNT; ++chIndex) {
      ...
      // Whereas this control contains the "default" value for the slider (since
      // the presetControl won't have that value)
      ...
      CDmeFloatLogLayer *pLayer =
          floatLog->GetLayer(floatLog->GetTopmostLayer());
      if (!pLayer)
        continue;

      float flDefault = defaultValueControl->GetValue<float>(
          s_pDefaultAttributeValueNames[chIndex]);
      float flControlValue =
          presetControl->GetValue<float>(s_pAttributeValueNames[chIndex]);
...
void CSFMPhonemeExtractor::ClearInterstitialSpaces(
  ...
  for (i = 0; i < info.m_WorkList.Count(); ++i) {
    ...
    // NOTE: Time bounds measured in sound media time goes from 0 ->
    // flWaveDuration
    DmeTime_t tSoundMediaStartTime =
        CDmeClip::FromChildMediaTime(srcStack, DMETIME_ZERO, false);
    DmeTime_t tSoundMediaEndTime = CDmeClip::FromChildMediaTime(
        srcStack, DmeTime_t(item.m_flDuration), false);
    ...
    DmeTime_t tChannelMediaStartTime =
        CDmeClip::ToChildMediaTime(dstStack, tSoundMediaStartTime, false);
    DmeTime_t tChannelMediaEndTime =
        CDmeClip::ToChildMediaTime(dstStack, tSoundMediaEndTime, false);

    // Find a scale + offset which transforms data in media space of the sound
    // [namely, the phonemes] into the media space of the channels [the logs
    // that drive the facial animation]
    DmeTime_t tEndDuration = tChannelMediaEndTime - tChannelMediaStartTime;
    double flScale = (item.m_flDuration != 0.0f)
                         ? tEndDuration.GetSeconds() / item.m_flDuration
                         : 0.0f;
...
void AddAnimSetBookmarkAtSoundMediaTime(
    const char *pName, DmeTime_t tStart, DmeTime_t tEnd,
...
void CSFMPhonemeExtractor::LogPhonemes(int nItemIndex, ExtractDesc_t &info) {
  ...
  CUtlDict<CDmePreset *, unsigned short> phonemeToPresetDict;
  BuildPhonemeToPresetMapping(item.m_ApplyTags, info.m_pSet, pPresetGroup,
                              phonemeToPresetDict);
  ...
  for (i = 0; i < info.m_ControlList.Count(); ++i) {
    controlLookup.Insert(info.m_ControlList[i].m_hControl->GetName(),
                         &info.m_ControlList[i]);
  }
  ...
  // Find a scale + offset which transforms data in media space of the sound
  // [namely, the phonemes] into the media space of the channels [the logs that
  ...
  // add bookmarks
  if (info.m_bCreateBookmarks) {
    AddAnimSetBookmarkAtSoundMediaTime("start", tPrePhonemeTime,
                                       tPrePhonemeTime, srcStack, info);
    ...
    for (i = 0; i < item.m_ApplyTags.Count(); ++i) {
      ...
      AddAnimSetBookmarkAtSoundMediaTime(pPhonemeName, tStart, tEnd, srcStack,
                                         info);
    }

    AddAnimSetBookmarkAtSoundMediaTime("end", tPostPhonemeTime,
                                       tPostPhonemeTime, srcStack, info);
  }

  if (info.m_nFilterType == EXTRACT_FILTER_HOLD ||
      info.m_nFilterType == EXTRACT_FILTER_LINEAR) {
    ...
    for (i = 0; i < item.m_ApplyTags.Count(); ++i) {
      ...
      if (info.m_nFilterType == EXTRACT_FILTER_HOLD) {
        ...
        // NOTE - this ignores phoneme length, but since all phonemes directly
        // abut one another, this doesn't matter
        DmeTime_t tLastEnd = tKeyTime - DMETIME_MINDELTA;
        if (tLastEnd > tPrePhonemeTime) {
          WriteDefaultValuesIntoLogLayers(tKeyTime - DMETIME_MINDELTA,
                                          controlLookup);
          if (pLastPreset) {
            StampControlValueLogs(pLastPreset, tKeyTime - DMETIME_MINDELTA,
                                  1.0f, controlLookup);
          }
        }
        pLastPreset = preset;
      ...
      if (info.m_nFilterType == EXTRACT_FILTER_HOLD &&
          i == item.m_ApplyTags.Count() - 1) {
        // stamp value at end of last phoneme
        tKeyTime = tEnd * flScale + tOffset;
        tKeyTime = min(tKeyTime, tPostPhonemeTime);
        WriteDefaultValuesIntoLogLayers(tKeyTime - DMETIME_MINDELTA,
                                        controlLookup);
        StampControlValueLogs(preset, tKeyTime - DMETIME_MINDELTA, 1.0f,
                              controlLookup);

        // stamp default just after end of last phoneme to hold silence until
        // tPostPhonemeTime
        WriteDefaultValuesIntoLogLayers(tKeyTime, controlLookup);
      }
    }
  ...
  for (i = 0; i < logs.Count(); ++i) {
    logs[i]->FlattenLayers(DMELOG_DEFAULT_THRESHHOLD,
                           CDmeLog::FLATTEN_NODISCONTINUITY_FIXUP);
  }
}
