...
COptionsSubVoice::COptionsSubVoice(vgui::Panel *parent)
  ...
  m_pReceiveSliderLabel =
      new Label(this, "ReceiveLabel", "#GameUI_VoiceReceiveVolume");
  m_pReceiveVolume = new CCvarSlider(
      this, "VoiceReceive", "#GameUI_ReceiveVolume", 0.0f, 1.0f, "voice_scale");

  m_pMicrophoneSliderLabel =
      new Label(this, "MicrophoneLabel", "#GameUI_VoiceTransmitVolume");
  ...
  m_pVoiceEnableCheckButton = new CCvarToggleCheckButton(
      this, "voice_modenable", "#GameUI_EnableVoice", "voice_modenable");

  m_pMicBoost = new CheckButton(this, "MicBoost", "#GameUI_BoostMicrophone");
  m_pMicBoost->AddActionSignalTarget(this);

  m_pTestMicrophoneButton =
      new Button(this, "TestMicrophone", "#GameUI_TestMicrophone");
...
void COptionsSubVoice::ResetVoiceParameters() {
  ...
  m_pReceiveVolume->Reset();
  // set the slider to 'new' value, but we've reset the 'start' value where it
  // was
...
void COptionsSubVoice::OnThink() {
  ...
  if (m_bVoiceOn) {
    if (!m_pVoiceTweak->IsStillTweaking()) {
      ...
      width = ((width + (BAR_INCREMENT - 1)) / BAR_INCREMENT) *
              BAR_INCREMENT; // round to nearest BAR_INCREMENT
