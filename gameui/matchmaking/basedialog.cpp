...
// CBaseDialog
//---------------------------------------------------------
CBaseDialog::CBaseDialog(vgui::Panel *pParent, const char *pName)
...
void CBaseDialog::PerformLayout(void) {
  ...
  if (m_pFooterInfo && m_pParent) {
    CMatchmakingBasePanel *pBasePanel =
...
void CBaseDialog::ApplySchemeSettings(vgui::IScheme *pScheme) {
  ...
  m_pTitle->SetFgColor(pScheme->GetColor("MatchmakingDialogTitleColor",
                                         Color(200, 184, 151, 255)));

  char szResourceName[MAX_PATH];
  Q_snprintf(szResourceName, sizeof(szResourceName), "%s.res", GetName());
  KeyValues *pKeys =
      BasePanel()->GetConsoleControlSettings()->FindKey(szResourceName);
...
//---------------------------------------------------------------------
CScenarioInfoPanel::CScenarioInfoPanel(vgui::Panel *parent, const char *pName)
...
void CScenarioInfoPanel::PerformLayout(void) { BaseClass::PerformLayout(); }
...
void CScenarioInfoPanel::ApplySchemeSettings(vgui::IScheme *pScheme) {
  ...
  KeyValues *pKeys = BasePanel()->GetConsoleControlSettings()->FindKey(
