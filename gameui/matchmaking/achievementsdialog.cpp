...
void CAchievementsDialog_XBox::PerformLayout(void) {
  ...
  if (m_nTotalAchievements) {
...
#ifdef WIN32
    V_snwprintf(wszProgressText, ARRAYSIZE(wszProgressText), L"%d%% %s",
    ...
    m_pProgressPercent->SetPos(GetWide() - m_pProgressPercent->GetWide() -
                                   m_nBorderWidth,
                               y + tall + 3);
    ...
    wchar_t wzTotal[8];
    V_snwprintf(wzActiveItem, ARRAYSIZE(wzActiveItem), L"%d",
                m_Menu.GetActiveItemIndex() + 1);
...
CAchievementsDialog::CAchievementsDialog(vgui::Panel *parent)
    : BaseClass(parent, "AchievementsDialog") {
  SetDeleteSelfOnClose(true);

  if (IsProportional())
    SetBounds(0, 0, scheme()->GetProportionalScaledValue(512),
              scheme()->GetProportionalScaledValue(384));
  else
  ...
  m_pSelectionHighlight =
      new ImagePanel(m_pAchievementsList->FindChildByName("PanelListEmbedded"),
                     "SelectionHighlight");
  m_pSelectionHighlight->SetVisible(false);
  m_pSelectionHighlight->SetZPos(100);

  m_pAchievementPackCombo = new ComboBox(this, "achievement_pack_combo",
                                         NUM_COMBO_BOX_LINES_DEFAULT, false);
...
void CAchievementsDialog::ApplySettings(KeyValues *pResourceData) {
  ...
  m_iFixedWidth =
      IsProportional() ? scheme()->GetProportionalScaledValue(width) : width;
...
public:
  bool Less(const IAchievement *src1, const IAchievement *src2, void *pCtx) {
    ...
    if (wcscoll(ACHIEVEMENT_LOCALIZED_NAME_FROM_STR(name1),
                ACHIEVEMENT_LOCALIZED_NAME_FROM_STR(name2)) < 0)
      return true;
    return false;
  }
};
...
void CAchievementsDialog::ApplySchemeSettings(IScheme *pScheme) {
  ...
  V_sprintf_safe(szPercentageText, "%d / %d ( %d%% )", m_nUnlocked,
  ...
  m_pSelectionHighlight->SetFillColor(
      Color(clrHighlight.r(), clrHighlight.g(), clrHighlight.b(), 32));
...
void CAchievementsDialog::ScrollToItem(int nDirection) {
  if (m_pAchievementsList->GetItemCount() > 0) {
    m_nScrollItem = clamp(m_nScrollItem + nDirection, 0,
                          m_pAchievementsList->GetItemCount() - 1);
  ...
  if (nButtonCode == KEY_XBUTTON_B || nButtonCode == STEAMCONTROLLER_B) {
    ...
    if (GameSupportsAchievementTracker() && m_nScrollItem >= 0 &&
      ...
      CAchievementDialogItemPanel *pItem =
          dynamic_cast<CAchievementDialogItemPanel *>(
              m_pAchievementsList->GetItemPanel(m_nScrollItem));
      if (pItem && pItem->IsVisible()) {
        pItem->ToggleShowOnHUD();
      }
    }
  } else if (nButtonCode == KEY_XBUTTON_UP || nButtonCode == KEY_XSTICK1_UP ||
    ...
    m_pAchievementPackCombo->ActivateItemByRow(
        m_pAchievementPackCombo->GetActiveItem() - 1);
...
void CAchievementsDialog::UpdateAchievementDialogInfo(void) {
  ...
  if (achievementmgr) {
    ...
    char szPercentageText[64];
    float flCompletion = (((float)m_AchievementGroups[0].m_iNumUnlocked) /
                          ((float)iAchievementCount));
    V_sprintf_safe(szPercentageText, "%d / %d ( %d%% )",
                   m_AchievementGroups[0].m_iNumUnlocked, m_nTotalAchievements,
  ...
  m_pAchievementPackCombo->ActivateItemByRow(
      m_pAchievementPackCombo->GetActiveItem());
}
...
void CAchievementsDialog::CreateOrUpdateComboItems(bool bCreate) {
  for (int i = 0; i < m_iNumAchievementGroups; i++) {
    ...
    if (wzGroupName) {
      ...
      V_snwprintf(wzNumUnlocked, ARRAYSIZE(wzNumUnlocked), L"%d",
                  m_AchievementGroups[i].m_iNumUnlocked);
    ...
    if (bCreate)
      m_AchievementGroups[i].m_iDropDownGroupID =
          m_pAchievementPackCombo->AddItem(wzGroupTitle, pKV);
    else
      m_pAchievementPackCombo->UpdateItem(
          m_AchievementGroups[i].m_iDropDownGroupID, wzGroupTitle, pKV);
  }

  if (bCreate && (m_iNumAchievementGroups > NUM_COMBO_BOX_LINES_DEFAULT)) {
    if (m_pAchievementPackCombo) {
      m_pAchievementPackCombo->SetNumberOfEditLines(
          (m_iNumAchievementGroups <= NUM_COMBO_BOX_LINES_MAX)
              ? m_iNumAchievementGroups
...
CAchievementDialogItemPanel::CAchievementDialogItemPanel(
  ...
  // CMouseMessageForwardingPanel *panel = new
  // CMouseMessageForwardingPanel(this, NULL); panel->SetZPos(2);
}
    ...
    if (m_pSourceAchievement->IsAchieved()) {
      ...
      if (GameSupportsAchievementTracker()) {
        m_pShowOnHUDCheck->SetVisible(
            !m_pSourceAchievement->ShouldHideUntilAchieved()); // m_pSourceAchievement->GetGoal()
                                                               // > 1 &&
...
void CAchievementDialogItemPanel::PreloadResourceFile(void) {
  ...
  g_pPreloadedAchievementItemLayout->LoadFromFile(g_pFullFileSystem,
                                                  controlResourceName);
}
...
void CAchievementDialogItemPanel::ApplySchemeSettings(IScheme *pScheme) {
  ...
  m_pAchievementIcon = SETUP_PANEL(
      dynamic_cast<vgui::ImagePanel *>(FindChildByName("AchievementIcon")));
  m_pAchievementNameLabel =
      dynamic_cast<vgui::Label *>(FindChildByName("AchievementName"));
  m_pAchievementDescLabel =
      dynamic_cast<vgui::Label *>(FindChildByName("AchievementDesc"));
  m_pPercentageBar = SETUP_PANEL(
      dynamic_cast<vgui::ImagePanel *>(FindChildByName("PercentageBar")));
  m_pPercentageText =
      dynamic_cast<vgui::Label *>(FindChildByName("PercentageText"));
  m_pShowOnHUDCheck =
      dynamic_cast<vgui::CheckButton *>(FindChildByName("ShowOnHUD"));
...
void CAchievementDialogItemPanel::OnCheckButtonChecked(Panel *panel) {
  if (GameSupportsAchievementTracker() && panel == m_pShowOnHUDCheck &&
      m_pSourceAchievement) {
