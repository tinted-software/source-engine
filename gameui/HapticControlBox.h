...
public:
  ControlBoxVisual(vgui::Panel *parent, const char *panelName,
                   CCvarSlider *near, CCvarSlider *right, CCvarSlider *up,
...
protected:
  void DrawCube(float Near = -1, float Right = -1, float Up = -1, float Far = 1,
                float Left = 1, float Down = 1, int specialside = -1);
  enum eBoxID {
    HUI_BOX_UP = 0,
    HUI_BOX_RIGHT,
    HUI_BOX_NEAR,
  ...
  struct CCvarSliderCube {
    CCvarSliderCube(CCvarSlider *n, CCvarSlider *r, CCvarSlider *u,
                    CCvarSlider *f, CCvarSlider *l, CCvarSlider *d) {
      Near = n;
      Right = r;
      Up = u;
      Far = f;
