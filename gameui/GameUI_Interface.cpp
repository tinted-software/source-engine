...
void CGameUI::Initialize(CreateInterfaceFn factory) {
  ...
  g_pEngineClientReplay = (IEngineClientReplay *)factory(
      ENGINE_REPLAY_CLIENT_INTERFACE_VERSION, NULL);

  if (ModInfo().SupportsVR() && CommandLine()->CheckParm("-vr")) {
    g_pSourceVR = (ISourceVirtualReality *)factory(
...
//panel, 		it should be full-screen with an opaque background, and must be a VGUI
//popup.
//-----------------------------------------------------------------------------
...
void CGameUI::PlayGameStartupSound() {
  ...
  if ((Q_stricmp(pGameName, "tf") == 0) ||
      (Q_stricmp(pGameName, "tf_beta") == 0)) {
    ...
    if (GameClientExports()) {
      ...
      if (pszHoliday && pszHoliday[0]) {
        Q_snprintf(path, sizeof(path), "sound/ui/holiday/gamestartup_%s*.mp3",
                   pszHoliday);
        Q_FixSlashes(path);
...
void CGameUI::Start() {
  ...
  if (IsPC()) {
    if (!IsPosix()) {
      // Alfred says this is really, really old code that does some wacky crap
      // that only
      //  happened in the first version of HL and it's the only game that does
      //  this and it was a steam testing type thing and we don't need to do it
      //  on Posix, etc.

      g_hMutex = Sys_CreateMutex("ValvePlatformUIMutex");
      g_hWaitMutex = Sys_CreateMutex("ValvePlatformWaitMutex");
      if (g_hMutex == 0 || g_hWaitMutex == 0 ||
      ...
      if (!(waitResult == SYS_WAIT_OBJECT_0 ||
        ...
        // Friends/ServerBrowser data files) get the wait mutex, so that
        // Steam.exe knows that we're trying to acquire ValveTrackerMutex
  ...
  if (platformDir[0] == '\0') {
    ...
    if (IsPC()) {
      ...
      if (::GetModuleFileName((HINSTANCE)GetModuleHandle(NULL), platformDir,
        ...
        if (lastslash) {
          *lastslash = 0;
          Q_strncat(platformDir, "\\platform\\", bufferSize,
                    COPY_ALL_CHARACTERS);
          return true;
        }
      }
      ...
      if (g_pFullFileSystem->GetSearchPath("PLATFORM", false, platformDir,
                                           bufferSize)) {
...
void CGameUI::RunFrame() {
  ...
  if (IsPC() && ((IsPosix() && m_bTryingToLoadFriends) ||
                 (m_bTryingToLoadFriends && m_iFriendsLoadPauseFrames-- < 1 &&
                  g_hMutex && g_hWaitMutex))) {
    // try and load Steam platform files
    unsigned int waitResult = Sys_WaitForSingleObject(g_hMutex, 0);
    if (IsPosix() ||
        (waitResult == SYS_WAIT_OBJECT_0 || waitResult == SYS_WAIT_ABANDONED)) {
...
void CGameUI::UpdatePlayerInfo(uint64 nPlayerId, const char *pName, int nTeam,
                               byte cVoiceState, int nPlayersNeeded,
                               bool bHost) {
...
// Client DLL is telling us that a main menu command was issued, probably from
// its custom main menu panel
