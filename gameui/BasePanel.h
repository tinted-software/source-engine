...
public:
  ...
  void CreateMessageDialog(const uint nType, const char *pTitle,
...
private:
  ...
  int m_ButtonPinRight; // if not centered, this is the distance from the right
                        // margin that we use to start drawing buttons (right to
                        // left)
...
//			It handles all the menus, background images, and loading
//dialogs
//-----------------------------------------------------------------------------
...
public:
  ...
  //context. 	Job's function "ExecuteAsync" is called on a separate thread. 	After
  //the job finishes the "Completed" function is called on the 	main thread.
  //
...
public:
  ...
  void UpdatePlayerInfo(uint64 nPlayerId, const char *pName, int nTeam,
                        byte cVoiceState, int nPlayersNeeded, bool bHost);
