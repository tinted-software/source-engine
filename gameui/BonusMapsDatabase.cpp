...
bool WriteBonusMapSavedData(KeyValues *data) {
  if (IsX360() && (XBX_GetStorageDeviceId() == XBX_INVALID_STORAGE_ID ||
...
bool SetBooleanStatus(KeyValues *pBonusFilesKey, const char *pchName,
  ...
  if (!IsX360() && !(g_pFullFileSystem->FileExists(pchFileName, "MOD") ||
                     g_pFullFileSystem->IsDirectory(pchFileName, "MOD"))) {
...
bool UpdateChallengeBest(KeyValues *pBonusFilesKey,
  ...
  // Don't create entries for files that don't exist
  if (!IsX360() &&
      !g_pFullFileSystem->FileExists(challenge.szFileName, "MOD")) {
...
void CBonusMapsDatabase::AppendPath(const char *pchAppend) {
  ...
  Q_snprintf(m_szCurrentPath, sizeof(m_szCurrentPath), "%s/%s", szCurPathTmp,
             pchAppend);
}
...
void CBonusMapsDatabase::AddBonus(const char *pCurrentPath,
  ...
  if (!IsX360() && !(g_pFullFileSystem->IsDirectory(szFileName, "MOD") ||
                     g_pFullFileSystem->FileExists(szFileName, "MOD")))
    return;
  ...
  while (kv) {
    ...
    if (pChallenges) {
      for (KeyValues *pChallengeKey = pChallenges->GetFirstSubKey();
        ...
        V_strcpy_safe(pChallenge->szComment,
                      pChallengeKey->GetString("comment"));
