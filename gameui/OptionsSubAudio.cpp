...
COptionsSubAudio::COptionsSubAudio(vgui::Panel *parent)
  ...
  m_pSoundMuteLoseFocusCheckButton = new CCvarToggleCheckButton(
      this, "snd_mute_losefocus", "#GameUI_SndMuteLoseFocus",
      "snd_mute_losefocus");
...
void COptionsSubAudio::OnResetData() {
...
#ifdef POSIX
  // On Posix there is no headphone option, so we upgrade to 2 speakers if
  ...
  if (steamapicontext->SteamApps()) {
    Q_strncpy(szCurrentLanguage,
              steamapicontext->SteamApps()->GetCurrentGameLanguage(),
              sizeof(szCurrentLanguage));
    Q_strncpy(szAvailableLanguages,
              steamapicontext->SteamApps()->GetAvailableGameLanguages(),
