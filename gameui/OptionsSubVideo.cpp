...
//-----------------------------------------------------------------------------
static void GetResolutionName(vmode_t *mode, char *sz, int sizeofsz,
                              int desktopWidth, int desktopHeight) {
  Q_snprintf(sz, sizeofsz, "%i x %i%s", mode->width, mode->height,
             (mode->width == desktopWidth) && (mode->height == desktopHeight)
                 ? " (native)"
                 : "");
}
...
public:
  COptionsSubVideoAdvancedDlg(vgui::Panel *parent)
    ...
    //
    //								2x	4x
    //6x	8x	16x	8x	16xQ 		Texture/Shader Samples	1
    //1	1	1	1	1	1 		Stored Color/Z Samples	2
    //4	6	4	4	8	8 		Coverage Samples		2
    //4	6	8	16	8	16 		MSAA or CSAA			M
    //M	M	C	C	M	C
    //
  ...
  MESSAGE_FUNC_PTR(OnTextChanged, "TextChanged", panel) {
    if (panel == m_pDXLevel && RequiresRestart()) {
      ...
      QueryBox *box =
          new QueryBox("#GameUI_SettingRequiresDisconnect_Title",
                       "#GameUI_SettingRequiresDisconnect_Info", this);
  ...
  void MarkDefaultSettingsAsRecommended() {
    ...
    // It doesn't make sense to retrieve this convar from dxsupport, because
    // we'll then have materialsystem setting this config at loadtime. (Also, it
    // only has very minimal support for CPU related configuration.)
  ...
  virtual void OnResetData() {
    ...
    // We (Rick!) have now switched -2 to mean enabled. So this comment has been
    // rendered obsolete:
    //  -- For testing, we have -2, the legacy default setting as meaning
...
static int getSDLDisplayIndexFullscreen() {
  ...
  return sdl_displayindex_fullscreen.IsValid()
             ? sdl_displayindex_fullscreen.GetInt()
             : -1;
}
...
COptionsSubVideo::COptionsSubVideo(vgui::Panel *parent)
  ...
  //	m_pThirdPartyCredits->SetCommand(new
  //KeyValues("OpenThirdPartyVideoCreditsDialog"));
  ...
  // Multimonitor under Direct3D requires you to destroy and recreate the
  // device, which is an operation we don't support as it currently stands. The
  // user can pass -adapter N to use a different device.
#if defined(USE_SDL) && defined(DX_TO_GL_ABSTRACTION)
  int numVideoDisplays = SDL_GetNumVideoDisplays();

  m_pWindowed =
      new vgui::ComboBox(this, "DisplayModeCombo", 5 + numVideoDisplays, false);
...
void COptionsSubVideo::PrepareResolutionList() {
  ...
  // If we are switching to fullscreen, and this isn't the mode we're currently
  // in, then
  //	fake things out so the native fullscreen resolution is selected. Stuck
  //this in 	because I assume most people will go fullscreen at native
  //resolution, and it's sometimes 	difficult to find the native resolution with
  //all the aspect ratio options.
  bool bNewFullscreenDisplay = (!bWindowed && (getSDLDisplayIndexFullscreen() !=
                                               m_pWindowed->GetActiveItem()));
  ...
  for (int i = 0; i < count; i++, plist++) {
    ...
    // try and find the bestplistplistplist match for the resolution to be
    // selected
...
#if defined(USE_SDL)
    // If we are switching to a new display, or the size is greater than the
    // desktop, then
    //	display the desktop width and height.
    if (bNewFullscreenDisplay || (Width > desktopWidth) ||
...
//-----------------------------------------------------------------------------
// Purpose: hint the engine to load HD content if possible, logic must match
// with engine/common.cpp BLoadHDContent
//-----------------------------------------------------------------------------
...
void COptionsSubVideo::OnApplyChanges() {
  ...
  if ((-1 != config.m_nVRModeAdapter) != bVRMode) {
    ...
    // force windowed. VR mode ignores this flag and desktop mode needs to be in
    // a window always
    windowed = bVRMode;
  }
  ...
  if (!windowed) {
    ...
    if (!SDL_GetDisplayBounds(displayIndexTarget, &rect)) {
      // If we are going non-native fullscreen, tweak the resolution to have the
      // same aspect ratio as the display.
      if ((width != rect.w) || (height != rect.h)) {
        // TODO: We may want a convar to allow folks to mess with their aspect
        // ratio?
  ...
  if (ModInfo().HasHDContent()) {
    if (BUseHDContent() != m_pHDContent->IsSelected()) {
      ...
      // Bring up the confirmation dialog
      MessageBox *box = new MessageBox("#GameUI_OptionsRestartRequired_Title",
...
void COptionsSubVideo::OnTextChanged(Panel *pPanel, const char *pszText) {
  if (pPanel == m_pMode) {
    ...
    if (!m_bDisplayedVRModeMessage) {
      ...
      bool bVRWasEnabled =
          materials->GetCurrentConfigForVideoCard().m_nVRModeAdapter != -1;
      if (bVRWasEnabled != bVRNowEnabled) {
        m_bDisplayedVRModeMessage = true;
        MessageBox *box =
            new MessageBox("#GameUI_VRMode", "#GameUI_VRModeRelaunchMsg", this);
        box->MoveToFront();
        box->DoModal();
      }
    }
