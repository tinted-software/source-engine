...
ControlBoxVisual::ControlBoxVisual(vgui::Panel *parent, const char *panelName,
                                   CCvarSlider *n, CCvarSlider *r,
                                   CCvarSlider *u, CCvarSlider *f,
                                   CCvarSlider *l, CCvarSlider *d)
...
void ControlBoxVisual::Paint() {
  ...
  if (m_iMouseOver != -1) {
    ...
    DrawCube(SlideValues->Near->GetSliderValue() * -1,
             ...
             SlideValues->Far->GetSliderValue(),
             SlideValues->Left->GetSliderValue(),
             SlideValues->Down->GetSliderValue(), m_iMouseOver);
  }
}

void ControlBoxVisual::DrawCube(float n, float r, float u, float f, float l,
                                float d, int specialside) {
  ...
  Vector vdrawsize =
      Vector(1, (float)GetWide() / 10.0f, (float)GetTall() / 10.0f);
  for (int i = 0; i != 4; i++) {
  ...
  for (int i = 0; i != 4; i++) {
    ...
    if (next > 3)
      next = 0;
    vgui::surface()->DrawLine(vdrawsize.y + left[i].y, vdrawsize.z + left[i].z,
                              vdrawsize.y + left[next].y,
                              vdrawsize.z + left[next].z);
    vgui::surface()->DrawLine(
        vdrawsize.y + right[i].y, vdrawsize.z + right[i].z,
        vdrawsize.y + right[next].y, vdrawsize.z + right[next].z);
    vgui::surface()->DrawLine(vdrawsize.y + left[i].y, vdrawsize.z + left[i].z,
                              vdrawsize.y + right[i].y,
