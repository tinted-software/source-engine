  ...
  if (pOption->iCurrentChoice < 0) {
    ...
    if (pOption->szConvar2[0] == '\0') {
      ...
      for (int iChoice = 0; iChoice < pOption->m_Choices.Count(); ++iChoice) {
        ...
        AssertMsg(pchOptionValue2, "Option uses 2 convars but an option "
                                   "doesn't have 2 ; separated values!");
...
void COptionsDialogXbox::ChangeValue(float fChange) {
  ...
  case OPTION_TYPE_SLIDER: {
    if (system()->GetCurrentTime() > m_fNextChangeTime) {
      ...
      float fValue = clamp(fOldValue + fIncValue, m_pSelectedOption->fMinValue,
  ...
  case OPTION_TYPE_CHOICE: {
    ...
    if (m_pSelectedOption->iCurrentChoice >= 0) {
      ...
      if (m_pSelectedOption->szConvar2[0] == '\0') {
        ...
        AssertMsg(pchOptionValue2, "Option uses to convars but an option "
                                   "doesn't have 2 ; separated values!");
      ...
      DevWarning("ConVar \"%s\" set to value that's not a choice used by "
                 "\"%s\" option.",
  ...
  case OPTION_TYPE_BIND: {
    ...
    // After binding we need to update all bindings so they display the correct
    // keys
    UpdateAllBinds(code);
...
void COptionsDialogXbox::UpdateValue(OptionData_t *pOption, int iLabel) {
  ...
  case OPTION_TYPE_SLIDER: {
    ...
    float fNormalizeValue = fabsf((pOption->fMinValue - varOption.GetFloat()) /
                                  (pOption->fMaxValue - pOption->fMinValue));
    if (pOption->fIncValue < 0.0f)
