...
void Sys_SetLastError(unsigned long error) { ::SetLastError(error); }

unsigned long Sys_GetLastError() { return ::GetLastError(); }
...
void Sys_PostMessage(WHANDLE wnd, unsigned int msg, unsigned int wParam,
                     unsigned int lParam) {
...
WHANDLE Sys_CreateWindowEx(const char *windowName) {
  ...
  return (WHANDLE)::CreateWindow(windowName, windowName, 0, 0, 0, 0, 0, 0, 0,
...
void Sys_SetLastError(unsigned long error) { errno = error; }

unsigned long Sys_GetLastError() { return errno; }
...
void Sys_ReleaseMutex(WHANDLE mutexHandle) { Assert(!"Implement me"); }
