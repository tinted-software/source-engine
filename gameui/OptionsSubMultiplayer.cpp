...
static ColorItem_t s_crosshairColors[] = {
    {"#Valve_Green", 50, 250, 50},   {"#Valve_Red", 250, 50, 50},
    {"#Valve_Blue", 50, 50, 250},    {"#Valve_Yellow", 250, 250, 50},
...
public:
  CrosshairImagePanelSimple(Panel *parent, const char *name,
...
CrosshairImagePanelSimple::CrosshairImagePanelSimple(
    Panel *parent, const char *name, COptionsSubMultiplayer *pOptionsPanel)
    : CrosshairImagePanelBase(parent, name) {
  m_pOptionsPanel = pOptionsPanel;
  m_pCrosshairTranslucencyCheckbox = new CCvarToggleCheckButton(
      m_pOptionsPanel, "CrosshairTranslucencyCheckbox", "#GameUI_Translucent",
      "cl_crosshairusealpha");
  m_pCrosshairColorCombo =
      new ComboBox(m_pOptionsPanel, "CrosshairColorComboBox", 6, false);
  m_pCrosshairSizeCombo =
      new CLabeledCommandComboBox(m_pOptionsPanel, "CrosshairSizeComboBox");
...
//-----------------------------------------------------------------------------
void CrosshairImagePanelSimple::DrawCrosshairRect(int x0, int y0, int x1,
                                                  int y1, bool bAdditive) {
  if (bAdditive)
...
void CrosshairImagePanelSimple::UpdateCrosshair() {
  ...
  if (m_pCrosshairSizeCombo) {
    ...
    if (size == 0) {
      ...
      surface()->GetScreenSize(screenWide, screenTall);
      if (screenTall <= 600) {
        // if the screen height is 600 or less, set the crosshair num to 3
        // (large)
        size = 3;
      } else if (screenTall <= 768) {
        // if the screen height is between 600 and 768, set the crosshair num to
        // 2 (medium)
        size = 2;
      } else {
        // if the screen height is greater than 768, set the crosshair num to 1
        // (small)
        size = 1;
      }
    }
...
public:
  CrosshairImagePanelCS(Panel *parent, const char *name,
                        COptionsSubMultiplayer *pOptionsPanel);
  virtual void ResetData();
  virtual void ApplyChanges();
...
CrosshairImagePanelCS::CrosshairImagePanelCS(
  ...
  m_pColorComboBox =
      new ComboBox(m_pOptionsPanel, "CrosshairColorComboBox", 6, false);
  m_pAlphaCheckbox = new CCvarToggleCheckButton(
      m_pOptionsPanel, "CrosshairTranslucencyCheckbox",
      "#GameUI_Crosshair_Blend", "cl_crosshairusealpha");
  m_pDynamicCheckbox = new CCvarToggleCheckButton(
      m_pOptionsPanel, "CrosshairDynamicCheckbox", "#GameUI_CrosshairDynamic",
      "cl_dynamiccrosshair");
  m_pDotCheckbox =
      new CCvarToggleCheckButton(m_pOptionsPanel, "CrosshairDotCheckbox",
                                 "#GameUI_CrosshairDot", "cl_crosshairdot");
  m_pColorAlphaSlider = new CCvarSlider(m_pOptionsPanel, "Alpha Slider",
                                        "#GameUI_CrosshairColor_Alpha", 0.0f,
                                        255.0f, "cl_crosshairalpha");
  m_pColorRSlider = new CCvarSlider(m_pOptionsPanel, "Red Color Slider",
                                    "#GameUI_CrosshairColor_Red", 0.0f, 255.0f,
                                    "cl_crosshaircolor_r");
  m_pColorGSlider = new CCvarSlider(m_pOptionsPanel, "Green Color Slider",
                                    "#GameUI_CrosshairColor_Green", 0.0f,
                                    255.0f, "cl_crosshaircolor_g");
  m_pColorBSlider = new CCvarSlider(m_pOptionsPanel, "Blue Color Slider",
                                    "#GameUI_CrosshairColor_Blue", 0.0f, 255.0f,
                                    "cl_crosshaircolor_b");
  m_pSizeSlider =
      new CCvarSlider(m_pOptionsPanel, "Size Slider", "#GameUI_Crosshair_Size",
                      0.0f, 12.0f, "cl_crosshairsize");
  m_pThicknessSlider = new CCvarSlider(m_pOptionsPanel, "Thickness Slider",
                                       "#GameUI_Crosshair_Thickness", 0.0f,
                                       3.0f, "cl_crosshairthickness");
...
//-----------------------------------------------------------------------------
void CrosshairImagePanelCS::DrawCrosshairRect(int x0, int y0, int x1, int y1,
                                              bool bAdditive) {
  if (bAdditive)
    vgui::surface()->DrawTexturedRect(x0, y0, x1, y1);
  else
...
void CrosshairImagePanelCS::Paint() {
  ...
  int iBarThickness =
      max(1, RoundFloatToInt(m_barThickness * (float)screenTall / 480.0f));
...
void CrosshairImagePanelCS::OnSliderMoved(KeyValues *data) {
  ...
  if (pPanel == m_pColorRSlider || pPanel == m_pColorGSlider ||
      pPanel == m_pColorBSlider) {
...
public:
  CrosshairImagePanelAdvanced(Panel *parent, const char *name,
                              COptionsSubMultiplayer *pOptionsPanel);
...
CrosshairImagePanelAdvanced::CrosshairImagePanelAdvanced(
  ...
  m_pAdvCrosshairRedSlider = new CCvarSlider(pOptionsPanel, "Red Color Slider",
                                             "#GameUI_CrosshairColor_Red", 0.0f,
                                             255.0f, "cl_crosshair_red");
  m_pAdvCrosshairGreenSlider = new CCvarSlider(
      pOptionsPanel, "Green Color Slider", "#GameUI_CrosshairColor_Green", 0.0f,
      255.0f, "cl_crosshair_green");
  m_pAdvCrosshairBlueSlider = new CCvarSlider(
      pOptionsPanel, "Blue Color Slider", "#GameUI_CrosshairColor_Blue", 0.0f,
  ...
  m_pAdvCrosshairScaleSlider =
      new CCvarSlider(pOptionsPanel, "Scale Slider", "#GameUI_CrosshairScale",
                      16.0f, 48.0f, "cl_crosshair_scale");

  m_pAdvCrosshairStyle =
      new CLabeledCommandComboBox(pOptionsPanel, "AdvCrosshairList");
...
//-----------------------------------------------------------------------------
void CrosshairImagePanelAdvanced::SetCrosshairTexture(
...
void CrosshairImagePanelAdvanced::UpdateCrosshair() {
  ...
  if (m_pAdvCrosshairStyle) {
    ...
    if (ModInfo().AdvCrosshairLevel() == 1 &&
        m_pAdvCrosshairStyle->GetActiveItem() ==
            0) // this is the "none" selection
    {
...
COptionsSubMultiplayer::COptionsSubMultiplayer(vgui::Panel *parent)
  ...
  case 0:
    if (!ModInfo().NoCrosshair())
      m_pCrosshairImage =
          new CrosshairImagePanelSimple(this, "CrosshairImage", this);
    break;

  case 1: // TF
  case 2: // DOD
    m_pCrosshairImage =
        new CrosshairImagePanelAdvanced(this, "AdvCrosshairImage", this);
    break;
  ...
  m_pLockRadarRotationCheckbox =
      new CCvarToggleCheckButton(this, "LockRadarRotationCheckbox",
                                 "#Cstrike_RadarLocked", "cl_radar_locked");
  ...
  // this is necessary because some of the game .res files don't have visiblity
  // flags set up correctly for their controls
...
void COptionsSubMultiplayer::OnCommand(const char *command) {
  if (!stricmp(command, "Advanced")) {
...
#else
      m_hImportSprayDialog->AddFilter("*.tga,*.jpg,*.vtf",
                                      "#GameUI_All_ImagesNoBmp", true);
#endif
    ...
    QueryBox *box = new QueryBox("#GameUI_ConfirmResetStatsTitle",
                                 "#GameUI_ConfirmResetStatsText", this);
    box->SetOKButtonText("#GameUI_Reset");
    box->SetOKCommand(
        new KeyValues("Command", "command", "ResetStats_NoConfirm"));
...
void COptionsSubMultiplayer::ConversionError(ConversionErrorType nError) {
  ...
  if (pErrorText) {
    // Create the dialog
    vgui::MessageBox *pErrorDlg =
        new vgui::MessageBox("#GameUI_Spray_Import_Error_Title", pErrorText);
    Assert(pErrorDlg);
...
void COptionsSubMultiplayer::OnFileSelected(const char *fullpath) {
  ...
  ConversionErrorType nErrorCode = ImgUtl_ConvertToVTFAndDumpVMT(
      fullpath, IsPosix() ? "/vgui/logos" : "\\vgui\\logos", 256, 256);
      ...
      if (g_pFullFileSystem->FileExists(filename)) {
        ...
        if (CORRECT_PATH_SEPARATOR == texturepath[0])
          cb->AddItem(displayname,
                      texturepath +
                          1); // ignore the initial "/" in texture path
        else
