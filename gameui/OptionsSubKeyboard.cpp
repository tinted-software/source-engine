  ...
  for (int i = 0; i < BUTTON_CODE_LAST; i++) {
    ...
    if (item) {
      ...
      if (currentKey && currentKey[0]) {
        ...
        // NVNT If we're not using a joystick, falcon bindings override keyboard
        // ones
        if (!bShouldOverride && bFalcon && IsNovintCode((ButtonCode_t)i) &&
            !IsNovintCode(currentBC))
...
public:
  COptionsSubKeyboardAdvancedDlg(vgui::VPANEL hParent) : BaseClass(NULL, NULL) {
    ...
    if (IsProportional())
      SetSize(scheme()->GetProportionalScaledValue(280),
              scheme()->GetProportionalScaledValue(140));
    else
      SetSize(280, 140);
