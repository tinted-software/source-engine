...
void CBonusMapsDialog::OnKeyCodePressed(vgui::KeyCode code) {
  if (code == KEY_XBUTTON_B) {
    OnCommand("Close");
    return;
  } else if (code == KEY_XSTICK1_RIGHT || code == KEY_XSTICK2_RIGHT ||
             code == KEY_XBUTTON_RIGHT || code == KEY_RIGHT) {
    if (m_pGameList->GetItemCount()) {
      Panel *pSelectedPanel = m_pGameList->GetSelectedPanel();
      if (!pSelectedPanel) {
        m_pGameList->SetSelectedPanel(
            m_pGameList->GetItemPanel(m_pGameList->FirstItem()));
        ...
        while (nNextPanelID != m_pGameList->InvalidItemID()) {
          if (m_pGameList->GetItemPanel(nNextPanelID) == pSelectedPanel) {
            ...
            if (nNextPanelID != m_pGameList->InvalidItemID()) {
              m_pGameList->SetSelectedPanel(
                  m_pGameList->GetItemPanel(nNextPanelID));
  ...
  } else if (code == KEY_XSTICK1_LEFT || code == KEY_XSTICK2_LEFT ||
             code == KEY_XBUTTON_LEFT || code == KEY_LEFT) {
    if (m_pGameList->GetItemCount()) {
      Panel *pSelectedPanel = m_pGameList->GetSelectedPanel();
      if (!pSelectedPanel) {
        m_pGameList->SetSelectedPanel(
        ...
        if (m_pGameList->GetItemPanel(nNextPanelID) != pSelectedPanel) {
          while (nNextPanelID != m_pGameList->InvalidItemID()) {
            ...
            if (nNextPanelID != m_pGameList->InvalidItemID()) {
              if (m_pGameList->GetItemPanel(nNextPanelID) == pSelectedPanel) {
                m_pGameList->SetSelectedPanel(
                    m_pGameList->GetItemPanel(nOldPanelID));
  ...
  } else if (code == KEY_XSTICK1_DOWN || code == KEY_XSTICK2_DOWN ||
             code == KEY_XBUTTON_DOWN || code == KEY_DOWN) {
    int nOldActiveItem = m_pChallengeSelection->GetActiveItem();
    int nActiveItem =
        min(nOldActiveItem + 1, m_pChallengeSelection->GetItemCount() - 1);

    if (nOldActiveItem != nActiveItem) {
      m_pChallengeSelection->ActivateItem(nActiveItem);
      return;
    }
  } else if (code == KEY_XSTICK1_UP || code == KEY_XSTICK2_UP ||
             code == KEY_XBUTTON_UP || code == KEY_UP) {
    int nOldActiveItem = m_pChallengeSelection->GetActiveItem();
