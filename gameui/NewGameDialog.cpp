...
public:
  CGameChapterPanel(CNewGameDialog *parent, const char *name,
    ...
    _snwprintf(text, ARRAYSIZE(text), L"%ls %ls",
               chapter ? chapter : L"CHAPTER", num);
...
CNewGameDialog::CNewGameDialog(vgui::Panel *parent, bool bCommentaryMode)
  ...
  if (IsPC() || !IsX360()) {
    ...
    while (bExists && chapterIndex < MAX_CHAPTERS) {
      Q_snprintf(szFullFileName, sizeof(szFullFileName), "cfg/chapter%d.cfg",
      ...
      if (ChapterStringIndex == 10) {
        ...
        FileHandle_t fChap =
            g_pFullFileSystem->Open(szFullFileName, "rb", "MOD");
        if (fChap) {
          Q_strncpy(chapters[chapterIndex].filename, szFullFileName + 4,
                    sizeof(chapters[chapterIndex].filename));
          ++chapterIndex;
  ...
  if (m_ChapterPanels.Count() > 1) {
    ...
    m_PanelXPos[0] = m_PanelXPos[1] = m_PanelXPos[3] = m_PanelXPos[4] =
        m_PanelXPos[2];
  }
  ...
  m_pCenterBg->SetPos(m_PanelXPos[2] - indent,
                      m_PanelYPos[2] - (m_pCenterBg->GetTall() - panelHeight) +
                          indent);
...
void CNewGameDialog::AnimateSelectionPanels(void) {
  ...
  if (GameUI().IsConsoleUI()) {
    ...
    GetAnimationController()->RunAnimationCommand(
        m_pChapterTitleLabels[(unsigned)m_ActiveTitleIdx], "alpha", 255, 0,
...
//slot so 			it holds the index of the panel that is actually in that slot's
//position.
//-----------------------------------------------------------------------------
  ...
  if (m_ChapterPanels.IsValidIndex(m_iSelectedChapter)) {
    ...
    if (IsPC()) {
      ...
      if (m_bCommentaryMode &&
        ...
        // Check our current state and disconnect us from any multiplayer server
        // we're connected to. This fixes an exploit where players would click
        // "start" on the commentary dialog to enable sv_cheats on the client
        // (via the code above) and then hit <esc> to get out of the explanation
        // dialog.
      ...
      if (m_ChapterPanels[m_iSelectedChapter]->HasBonus() &&
          m_iBonusSelection > 0) {
        if (m_iBonusSelection == 1) {
          ...
          BonusMapsDatabase()->SetCurrentChallengeNames(
              m_pBonusMapDescription->szFileName,
...
void CNewGameDialog::OnCommand(const char *command) {
  ...
  if (!stricmp(command, "Play")) {
    ...
    if (m_bCommentaryMode && !commentary.GetBool()) {
      // Using the commentary menu, but not already in commentary mode, explain
      // the rules
      PostMessage((vgui::Panel *)this,
                  new KeyValues("command", "command",
                                "StartNewGameWithCommentaryExplanation"),
                  0.2f);
    } else {
      if (XBX_GetStorageDeviceId() == XBX_INVALID_STORAGE_ID ||
          XBX_GetStorageDeviceId() == XBX_STORAGE_DECLINED ||
          !ModInfo().IsSinglePlayerOnly()) {
        // Multiplayer or no storage device so don't bore them with autosave
        // details
        m_bMapStarting = true;
        ...
        PostMessage((vgui::Panel *)this,
                    new KeyValues("command", "command",
                                  "StartNewGameWithAutosaveExplanation"),
                    0.2f);
      }
    }
    ...
    vgui::surface()->PlaySound("UI/buttonclickrelease.wav");
    BasePanel()->RunAnimationWithCallback(this, "CloseNewGameDialog",
                                          new KeyValues("StartGame"));
  }
#endif
    ...
    if (!m_bScrolling) {
      for (int i = 0; i < m_ChapterPanels.Count(); ++i) {
        if (m_ChapterPanels[i]->IsSelected()) {
          int nNewChapter = i - 1;
          if (nNewChapter >= 0) {
            if (nNewChapter < m_PanelIndex[SLOT_LEFT] &&
                m_PanelIndex[SLOT_LEFT] != -1) {
              ScrollSelectionPanels(SCROLL_RIGHT);
      ...
      for (int i = 0; i < m_ChapterPanels.Count(); ++i) {
        if (m_ChapterPanels[i]->IsSelected()) {
          ...
          if (nNewChapter < m_ChapterPanels.Count()) {
            if (nNewChapter > m_PanelIndex[SLOT_RIGHT] &&
                m_PanelIndex[SLOT_RIGHT] != -1) {
              ScrollSelectionPanels(SCROLL_LEFT);
            } else if (m_ChapterPanels[nNewChapter]->IsEnabled()) {
              SetSelectedChapterIndex(nNewChapter);
            }
