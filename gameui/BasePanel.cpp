...
public:
  ...
  void SetUrl(const char *url) { m_szUrl = url; }
  ...
  virtual void OnScreenSizeChanged(int nOldWidth, int nOldHeight) {
    ...
    Panel::SetPos(nw - scheme()->GetProportionalScaledValue(m_iOldX) - scaled_w,
                  m_iOldY);
...
public:
  ...
  virtual int AddMenuItem(const char *itemName, wchar_t *itemText,
                          const char *command, Panel *target,
                          KeyValues *userData = NULL) {
    MenuItem *item = new CGameMenuItem(this, itemName);
    item->AddActionSignalTarget(target);
  ...
  virtual void OnSetFocus() {
    if (m_hMainMenuOverridePanel) {
      Panel *pMainMenu =
          ipanel()->GetPanel(m_hMainMenuOverridePanel, "ClientDLL");
      if (pMainMenu) {
  ...
  virtual void OnKeyCodePressed(KeyCode code) {
    ...
    if (nDir != 0) {
      ...
      if (VguiPanelNavigateSortedChildButtonList((void *)&vecSortedButtons,
                                                 nDir) != -1) {
        // Handled!
        return;
      }
  ...
  void UpdateMenuItemState(bool isInGame, bool isMultiplayer, bool isInReplay,
                           bool isVREnabled, bool isVRActive) {
    ...
    for (int i = 0; i < GetChildCount(); i++) {
      ...
      if (menuItem) {
        ...
        if (!isInReplay && kv->GetInt("OnlyInReplay")) {
          ...
          shouldBeVisible = false;
        } else if ((!isVRActive || ShouldForceVRActive()) &&
                   kv->GetInt("OnlyWhenVRActive")) {
          shouldBeVisible = false;
  ...
  MESSAGE_FUNC_HANDLE(OnCursorEnteredMenuItem, "CursorEnteredMenuItem",
                      menuItem);
...
CBasePanel::CBasePanel() : Panel(NULL, "BaseGameUIPanel") {
  ...
  if (IsAndroid()) {
    AddUrlButton(this, "vgui/\x64\x69\x73\x63\x6f\x72\x64\x5f\x6c\x6f\x67\x6f",
                 "\x68\x74\x74\x70\x73\x3a\x2f\x2f\x64\x69\x73\x63\x6f\x72\x64"
                 "\x2e\x67\x67\x2f\x68\x5a\x52\x42\x37\x57\x4d\x67\x47\x77");
    AddUrlButton(this, "vgui/\x74\x77\x69\x74\x74\x65\x72\x5f\x6c\x6f\x67\x6f",
                 "\x68\x74\x74\x70\x73\x3a\x2f\x2f\x74\x77\x69\x74\x74\x65\x72"
                 "\x2e\x63\x6f\x6d\x2f\x6e\x69\x6c\x6c\x65\x72\x75\x73\x72");
    AddUrlButton(this,
                 "vgui/\x74\x65\x6c\x65\x67\x72\x61\x6d\x5f\x6c\x6f\x67\x6f",
                 "\x68\x74\x74\x70\x73\x3a\x2f\x2f\x74\x2e\x6d\x65\x2f\x6e\x69"
                 "\x6c\x6c\x65\x72\x75\x73\x72\x5f\x73\x6f\x75\x72\x63\x65");
    AddUrlButton(this, "vgui/\x67\x69\x74\x68\x75\x62\x5f\x6c\x6f\x67\x6f",
                 "\x68\x74\x74\x70\x73\x3a\x2f\x2f\x67\x69\x74\x68\x75\x62\x2e"
                 "\x63\x6f\x6d\x2f\x6e\x69\x6c\x6c\x65\x72\x75\x73\x72\x2f\x73"
                 "\x6f\x75\x72\x63\x65\x2d\x65\x6e\x67\x69\x6e\x65");
  }
}
...
static int
CC_GameMenuCompletionFunc(char const *partial,
...
void CBasePanel::SetBackgroundRenderState(EBackgroundState state) {
  ...
  if (m_eBackgroundState == BACKGROUND_INITIAL &&
      (state == BACKGROUND_DISCONNECTED || state == BACKGROUND_MAINMENU)) {
...
void CBasePanel::DrawBackgroundImage() {
  if (IsX360() && m_bCopyFrameBuffer) {
    ...
    surface()->DrawGetTextureFile(m_iRenderTargetImageID, filename,
                                  sizeof(filename));
  ...
  // plaque
  if (IsPC() && (m_bRenderingBackgroundTransition ||
...
void CBasePanel::UpdateGameMenus() {
  ...
  m_pGameMenu->UpdateMenuItemState(isInGame, isMulti, isInReplay, isVREnabled,
                                   isVRActive);

  if (m_hMainMenuOverridePanel) {
    vgui::ivgui()->PostMessage(m_hMainMenuOverridePanel,
                               new KeyValues("UpdateMenu"), NULL);
  }
...
CGameMenu *CBasePanel::RecursiveLoadGameMenu(KeyValues *datafile) {
  ...
  if (!bFoundServerBrowser && !ModInfo().IsSinglePlayerOnly())
    menu->AddMenuItem("AntiM*dG*yButton", "#GameUI_GameMenu_FindServers",
                      "OpenServerBrowser", this);
  ...
  for (KeyValues *dat = datafile->GetFirstSubKey(); dat != NULL;
    ...
    if (cmd &&
        (!Q_stricmp(cmd, "OpenFriendsDialog") || !Q_stricmp(cmd, "engine bug")))
      continue;
...
void CBasePanel::RunFrame() {
  ...
  if (GameUI().IsConsoleUI()) {
    ...
    if (IsX360() && m_ExitingFrameCount &&
        engine->Time() >= m_flTransitionEndTime) {
  ...
  for (uint i = 0; i < XUSER_MAX_COUNT; ++i) {
    ...
    if (State != eXUserSigninState_NotSignedIn) {
      if (i != XBX_GetPrimaryUserId()) {
        ...
        if (!xboxsystem->UserSetContext(i, X_CONTEXT_PRESENCE,
                                        CONTEXT_PRESENCE_IDLE, true)) {
...
void CBasePanel::PerformLayout() {
  ...
  if (m_pGameLogo) {
    ...
    m_pGameLogo->SetPos(m_iGameMenuPos.x + m_pGameLogo->GetOffsetX(),
                        idealMenuY - m_pGameLogo->GetTall() +
                            m_pGameLogo->GetOffsetY());
  }
...
void CBasePanel::ApplySchemeSettings(IScheme *pScheme) {
  ...
  if (IsX360()) {
    ...
    if (m_iRenderTargetImageID == -1) {
      ...
      surface()->DrawSetTextureFile(m_iRenderTargetImageID,
                                    "console/rt_background", false, false);
    }
  }
  ...
  if (IsPC() || !IsX360()) {
    ...
    V_snprintf(filename, sizeof(filename), "vgui/appchooser/background_%s%s",
               gameName, (bIsWidescreen ? "_widescreen" : ""));
  }
  ...
  if (IsX360()) {
    ...
    V_snprintf(filename, sizeof(filename),
               "vgui/appchooser/background_orange%s",
               (bIsWidescreen ? "_widescreen" : ""));
...
void CBasePanel::RunMenuCommand(const char *command) {
  ...
  //=============================================================================
    ...
    if (!IsX360()) {
      ...
      V_snprintf(szSteamURL, sizeof(szSteamURL), "steam://run/%d/%s",
                 engine->GetAppID(),
      ...
      if (IsPC() && RegOpenKeyEx(HKEY_CURRENT_USER, "Software\\Valve\\Source",
...
bool CBasePanel::ValidateStorageDevice(void) {
  if (m_bUserRefusedStorageDevice == false) {
    ...
    if (XBX_GetStorageDeviceId() == XBX_INVALID_STORAGE_ID) {
      ...
      DWORD nFoundDevice = xboxsystem->DiscoverUserData(XBX_GetPrimaryUserId(),
                                                        COM_GetModDirectory());
...
bool CBasePanel::HandleSignInRequest(const char *command) {
  ...
  bool bValidUser =
      (XUserGetSigninInfo(XBX_GetPrimaryUserId(), 0, &info) == ERROR_SUCCESS);
...
//-----------------------------------------------------------------------------
void CBasePanel::UpdatePlayerInfo(uint64 nPlayerId, const char *pName,
...
// Purpose:
//-----------------------------------------------------------------------------
int CBasePanel::GetMenuAlpha(void) { return m_pGameMenu->GetAlpha(); }
...
void CBasePanel::Reload_Resources(const CCommand &args) {
  ...
  if (m_pConsoleControlSettings->LoadFromFile(g_pFullFileSystem,
...
void CMessageDialogHandler::ShowMessageDialog(int nType, vgui::Panel *pOwner) {
  ...
  case MD_SESSION_CREATE_FAILED:
    CreateMessageDialog(MD_OK, NULL, "#TF_Dlg_CreateFailed", "ReturnToMainMenu",
                        NULL, pOwner);
    break;
  ...
  case MD_SESSION_CONNECT_NOTAVAILABLE:
    CreateMessageDialog(MD_OK, NULL, "#TF_Dlg_JoinRefused", "ReturnToMainMenu",
                        NULL, pOwner);
    break;
  ...
  case MD_PROMPT_STORAGE_DEVICE_REQUIRED:
    CreateMessageDialog(MD_YESNO | MD_WARNING | MD_COMMANDAFTERCLOSE |
                        ...
                        "ShowDeviceSelector", "RequiredStorageDenied", pOwner);
    break;
  ...
  case MD_PROMPT_SIGNIN:
    CreateMessageDialog(MD_YESNO | MD_WARNING | MD_COMMANDAFTERCLOSE |
                        ...
                        "ShowSignInUI", "SignInDenied", pOwner);
    break;
  ...
  case MD_NOT_ONLINE_SIGNEDIN:
    CreateMessageDialog(MD_YESNO | MD_WARNING, "", "#TF_Dlg_NotOnlineSignedIn",
                        "ShowSigninUI", "close_dialog", pOwner);
    break;
  ...
  case MD_DEFAULT_CONTROLS_CONFIRM:
    CreateMessageDialog(MD_YESNO | MD_WARNING | iSimpleFrame | MD_RESTRICTPAINT,
                        ...
                        "#GameUI_ControllerSettingsText", "DefaultControls",
                        "close_dialog", pOwner);
    break;
  ...
  case MD_SAVE_BEFORE_LOAD:
    CreateMessageDialog(MD_OKCANCEL | MD_WARNING | iSimpleFrame |
                            MD_COMMANDAFTERCLOSE | MD_RESTRICTPAINT,
                        "#GameUI_ConfirmLoadGame_Title", "#GameUI_LoadWarning",
                        "LoadGame", "LoadGameCancelled", pOwner);
    break;

  case MD_DELETE_SAVE_CONFIRM:
    CreateMessageDialog(MD_OKCANCEL | MD_WARNING | iSimpleFrame |
                            MD_COMMANDAFTERCLOSE,
  ...
  case MD_SAVING_WARNING:
    CreateMessageDialog(MD_WARNING | iSimpleFrame | MD_COMMANDONFORCECLOSE, "",
                        "#GameUI_SavingWarning", "SaveSuccess", NULL, pOwner,
                        true);
    break;
  ...
  case MD_LOAD_FAILED_WARNING:
    CreateMessageDialog(MD_OK | MD_WARNING | iSimpleFrame, "#GameUI_LoadFailed",
                        "#GameUI_LoadFailed_Description", "close_dialog", NULL,
                        pOwner);
    break;
  ...
  case MD_STORAGE_DEVICES_CORRUPT:
    CreateMessageDialog(MD_OK | MD_WARNING | iSimpleFrame | MD_RESTRICTPAINT,
                        "", "#GameUI_Console_FileCorrupt", "close_dialog", NULL,
                        pOwner);
    break;

  case MD_CHECKING_STORAGE_DEVICE:
    CreateMessageDialog(iSimpleFrame | MD_RESTRICTPAINT, NULL,
                        "#GameUI_Dlg_CheckingStorageDevice", NULL, NULL, pOwner,
                        true);
    break;
...
void CMessageDialogHandler::CreateMessageDialog(
    const uint nType, const char *pTitle, const char *pMsg, const char *pCmdA,
...
static void RefreshOptionsDialog(const CCommand &args) {
  if (g_hOptionsDialog) {
    ...
    if (pBasePanel) {
      ...
      COptionsDialog *pOptionsDialog =
          dynamic_cast<COptionsDialog *>(g_hOptionsDialog.Get());
      if (pOptionsDialog) {
        pOptionsDialog->GetPropertySheet()->SetActivePage(
            pOptionsDialog->GetOptionsSubMultiplayer());
      }
    }
  }
}
static ConCommand refresh_options_dialog("refresh_options_dialog",
                                         RefreshOptionsDialog,
                                         "Refresh the options dialog.", 0);
