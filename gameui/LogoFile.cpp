...
=============
*/
unsigned char AveragePixels(int count) { return pixdata[0]; }
...
==============
*/
int GrabMip(HANDLE hdib, unsigned char *lump_p, char *lumpname, COLORREF crf,
  ...
  *height = h;
  ...
  byteimage = (unsigned char *)((LPSTR)lpbmi + sizeof(BITMAPINFOHEADER) +
                                256 * sizeof(RGBQUAD));

  if ((w & 15) || (h & 15))
    return 0; // Error ("line %i: miptex sizes must be multiples of 16",
              // scriptline);
  ...
  qtex->offsets[0] =
      (unsigned)((unsigned char *)lump_p - (unsigned char *)qtex);
  ...
  for (i = 0; i < 256; i++) {
    for (int j = 0; j < 3; j++) {
      ...
      linearpalette[i][j] = f; // pow((double)f, 2); // assume textures are done
                               // at 2.2, we want to remap them at 1.0
    }
  }
