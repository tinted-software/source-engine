...
CGameSavePanel::CGameSavePanel(CSaveGameBrowserDialog *parent,
  ...
  if (pchChapterName) {
    ...
    Q_snprintf(szChapterImage, sizeof(szChapterImage), "chapters/%s",
               Q_strlower(pchChapterName));
  ...
  if (GameUI().IsConsoleUI()) {
    pKeys =
        BasePanel()->GetConsoleControlSettings()->FindKey("SaveGamePanel.res");
  }
...
CSaveGameBrowserDialog::CSaveGameBrowserDialog(vgui::Panel *parent)
    : BaseClass(parent, "SaveGameDialog"), m_bFilterAutosaves(false),
      m_iSelectedSave(-1), m_bScrolling(false), m_ScrollCt(0),
      m_ScrollSpeed(0.0f), m_ButtonPressed(SCROLL_NONE),
      m_ScrollDirection(SCROLL_NONE), m_nDeletedPanel(INVALID_INDEX),
      m_nAddedPanel(INVALID_INDEX), m_nUsedStorageSpace(0),
...
void CSaveGameBrowserDialog::LayoutPanels(void) {
  ...
  if (HasActivePanels()) {
    ...
    m_pCenterBg->SetPos(m_PanelXPos[2] - 8,
                        m_PanelYPos[2] -
                            (panelHeight - m_nCenterBgTallDefault) + 8);
...
bool CSaveGameBrowserDialog::ParseSaveData(char const *pszFileName,
  ...
  int readok = SaveReadNameAndComment(fh, szMapName, sizeof(szMapName),
