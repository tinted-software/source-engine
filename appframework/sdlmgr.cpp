...
// NOTE: This has to be the last file included! (turned off below, since this is
// included like a header)
...
ConVar rawinput_set_one_time("rawinput_set_one_time", "0",
...
typedef EGLBoolean (*t_eglInitialize)(EGLDisplay display, EGLint *major,
                                      EGLint *minor);
...
void CheckGLError(int line) {
  ...
  // Don't check this in enabled! glGetError() is extremely slow with threaded
  // drivers.
  return;
  // char errbuf[1024];
...
void *VoidFnPtrLookup_GlMgr(const char *fn, bool &okay, const bool bRequired,
  ...
  if ((!okay) && (!bRequired)) // always look up if required (so we get a
                               // complete list of crucial missing symbols).
    return NULL;
#endif

  // The SDL path would work on all these platforms, if we were using SDL there,
  // too...
  ...
  // printf("CDynamicFunctionOpenGL: SDL_GL_GetProcAddress(\"%s\") returned
  // %p\n", fn, retval);
...
#error Unimplemented
  ...
  //  an implementation MAY return NULL in this case, but it doesn't have to
  //  (and doesn't, with the DRI drivers).
...
#endif
  if (bRequired && !okay) {
    // We can't continue execution, because one or more GL function pointers
    // will be NULL.
    Error("Could not find required OpenGL entry point '%s'! Either your video "
...
public:
  ...
  // Get the next N events. The function returns the number of events that were
  // filled into your array.
  ...
  virtual void GetMouseDelta(int &x, int &y,
                             bool bIgnoreNextMouseDelta = false);
...
private:
...
#if defined(OSX)
  // bool					m_leopard;
  // // true if <10.6.3 and we have to do extra work for fullscreen handling
  ...
  bool m_bHasFocus;
  bool m_bFullScreen;
  bool m_SizeWindowFullScreenState; // fullscreen state when SizeWindow() was
                                    // called.
  ...
  CUtlLinkedList<CCocoaEvent, int>
      m_DebugEvents; // intercepted keys which wil be passed over to GLM
...
ConVar sdl_displayindex_fullscreen("sdl_displayindex_fullscreen", "-1",
                                   FCVAR_HIDDEN);

// Display index to show window on.
static bool g_bSDLDisplayindexSet = false;
static void sdl_displayindex_changed(IConVar *pConVar, const char *pOldString,
                                     float flOldValue);
ConVar sdl_displayindex("sdl_displayindex", "0", FCVAR_HIDDEN,
                        "SDL fullscreen display index.",
                        sdl_displayindex_changed);
static void sdl_displayindex_changed(IConVar *pConVar, const char *pOldString,
                                     float flOldValue) {
  int NumVideoDisplays = SDL_GetNumVideoDisplays();

  if ((sdl_displayindex.GetInt() < 0) ||
      (sdl_displayindex.GetInt() >= NumVideoDisplays)) {
...
InitReturnVal_t CSDLMgr::Init() {
  ...
  if (!CommandLine()->FindParm("-nomousegrab") &&
      CommandLine()->FindParm("-mousegrab")) {
  ...
  *(attCursor++) = (int)(value);
  ...
  if (_eglInitialize && _eglInitialize && _eglQueryString) {
    ...
    if (_eglInitialize(display, NULL, NULL) != -1 &&
        strstr(_eglQueryString(display, EGL_EXTENSIONS),
  ...
  if (m_bOGL) {
    ...
    _glGetProcAddress =
        (t_glGetProcAddress)dlsym(l_gl4es, "gl4es_glGetProcAddress");
  }
#endif
...
void CSDLMgr::Disconnect() { SDLAPP_FUNC; }
...
void CSDLMgr::Shutdown() {
  ...
  if (m_Window) {
    // Slam down the window refcount to 1 to guarantee the main GL context and
    // window are killed.
    m_nWindowRefCount = 1;
    DestroyGameWindow();
  }
...
bool CSDLMgr::CreateGameWindow(const char *pTitle, bool bWindowed, int width,
                               int height) {
  SDLAPP_FUNC;

  // CreateGameWindow is being called. The the game initially calls this with
  // width and height == 0.
  //  But we don't want to show the window until it gets resized to what it
  //  should be, so we keep track as to whether ShowWindow / RaiseWindow has
  //  been called yet in here, and if not we do the SDL_ShowWindow in
  ...
  if ((width <= 0) || (height <= 0)) {
    ...
    this->GetNativeDisplayInfo(-1, defaultWidth, defaultHeight,
                               defaultRefreshHz);
  ...
  if (m_Window) {
    ...
    if ((m_bFullScreen != !bWindowed) ||
        (!bWindowed &&
         (sdl_displayindex.GetInt() != sdl_displayindex_fullscreen.GetInt()))) {
...
bool CSDLMgr::CreateHiddenGameWindow(const char *pTitle, int width,
                                     int height) {
  Assert(!m_Window);
  ...
  //-----------------------------------------------------------------------------------------
  //- enforce minimum system requirements for multiplayer branch (CSS / DOD /
  //TF2) : no GMA950, X3100, or NV G7x.
  if (!CommandLine()->FindParm("-glmnosystemcheck")) // escape hatch
  {
    ...
    // forbidden chipsets
    if (rendererInfo.m_atiR5xx || rendererInfo.m_intel95x ||
        rendererInfo.m_intel3100 || rendererInfo.m_nvG7x) {
  ...
  if (l_gl4es) {
    _glGetProcAddress =
        (t_glGetProcAddress)dlsym(l_gl4es, "gl4es_GetProcAddress");
  ...
  static CDynamicFunctionOpenGL<true, const GLubyte *(APIENTRY *)(GLenum name),
                                const GLubyte *>
      glGetString("glGetString");
  static CDynamicFunctionOpenGL<
      true, GLvoid(APIENTRY *)(GLenum pname, GLint * params), GLvoid>
...
bool CSDLMgr::MakeContextCurrent(PseudoGLContextPtr hContext) {
  ...
  // We only ever have one GL context on Linux at the moment, so don't spam
  // these calls.
...
int CSDLMgr::GetEvents(CCocoaEvent *pEvents, int nMaxEventsToReturn,
  ...
  CUtlLinkedList<CCocoaEvent, int> &queue =
      debugEvent ? m_CocoaEvents : m_DebugEvents;
...
int CSDLMgr::PeekAndRemoveKeyboardEvents(bool *pbEsc, bool *pbReturn,
                                         bool *pbSpace, bool debugEvent) {
  SDLAPP_FUNC;
...
bool CSDLMgr::IsDebugEvent(CCocoaEvent &event) {
  ...
  result |= ((event.m_EventType == CocoaEvent_KeyDown) &&
             ((event.m_ModifierKeyMask & (1 << eControlKey)) != 0));
#endif
...
void CSDLMgr::SetMouseVisible(bool bState) {
  ...
  // If this is the first time we've been called in this frame or we're setting
  // it to visible, then store it. This is to handle the case where the game
  // toggles the mouse state between visible and !visible 1 billion times
  //	in a frame.
...
void CSDLMgr::SetMouseCursor(SDL_Cursor *hCursor) {
  ...
  if (m_hCursor != hCursor) {
    if (!hCursor) {
      // SDL_SetCursor( NULL ) just forces a cursor redraw, so don't ever bother
      // doing that.
...
#if defined(OSX) || defined(ANDROID)
    // We default raw input to on on Mac/Android and set it one time for all
    // users since it didn't used to be the default.
...
void CSDLMgr::ShowPixels(CShowPixelsParams *params) {
  ...
  if (gl_swapdebug.GetInt()) {
    ...
    swapLimit = 1; // params->m_vsyncEnable ? 1 : 0;	// no good reason to
                   // turn off swap limit in normal user mode

#ifdef OSX
    // only do the funky forced vsync for NV on 10.6.4 and only if the bypass is
    // not turned on
  ...
  if ((swapInterval != m_lastKnownSwapInterval) ||
      (swapLimit != m_lastKnownSwapLimit)) {

    if (swapInterval != m_lastKnownSwapInterval) {
      // This code hits when we turn on vsync, if we're going to swap tear.
      // We want to do one frame of real vsync to get the engine to sync at the
      // top of the frame refresh.
      if (swapInterval < 0 && (m_lastKnownSwapInterval == 0 ||
                               m_lastKnownSwapInterval == kBogusSwapInterval)) {
        swapInterval = -swapInterval;
      }
...
#ifdef OSX
  if (!params->m_noBlit) {
    if (params->m_useBlit) // FBO blit path - which is what we *should* be
                           // using.  But if the params say no, then don't do it
                           // because the ext is not there.
    {
      ...
      GLenum filter = (((srcxmax - srcxmin) == (dstxmax - dstxmin)) &&
                       ((srcymax - srcymin) == (dstymax - dstymin)))
                          ? GL_NEAREST
                          : GL_LINEAR;
      ...
      glBlitFramebuffer(
          ...
          /* dst min and maxes xy xy */ dstxmin, dstymax, dstxmax,
          dstymin, // note yflip here
      ...
      // later if we switch the Valve side to render into an RBO, then this
      // would turn into an FBO blit note, do not check glGetError in between
      // glBegin/glEnd, lol
...
void CSDLMgr::SetWindowFullScreen(bool bFullScreen, int nWidth, int nHeight) {
  ...
  if ((m_bFullScreen != bFullScreen) ||
    ...
    if (bFullScreen) {
      ...
      // If we have more than one display, center the window in the one we've
      // been assigned to.
      ...
      if (m_bFullScreen == bFullScreen) {
        // TODO: Temporary workaround. SDL doesn't support going fullscreen on
        // one monitor to fullscreen on another. So we switch to windowed here,
        // move our window, then go back fullscreen.
        SDL_SetWindowFullscreen(m_Window, SDL_FALSE);
        ThreadSleep(15);
      }

      // Move the window to the upper left of whatever display we're on, then
      // size to fullscreen.
...
void CSDLMgr::SizeWindow(int width, int tall) {
  ...
  if ((m_WindowWidth == width) && (m_WindowHeight == tall) &&
...
void CSDLMgr::handleKeyInput(const SDL_Event &event) {
  ...
  // key modifiers aren't necessarily reliable in all the cases we'd want, so
  // track it ourselves.
...
#undef KEYSYMCASE
  ...
  // make a decision about this event - does it go in the normal evt queue or
  // into the debug queue.
  bool debug = IsDebugEvent(theEvent);

#if GLMDEBUG
  bool bIsShifted =
      (((theEvent.m_ModifierKeyMask & (1 << eCapsLockKey)) != 0) ||
...
void CSDLMgr::PumpWindowsMessageLoop() {
  ...
  while (SDL_PollEvent(&event) && nEventsProcessed < 100) {
    ...
    case SDL_MOUSEMOTION: {
      ...
      // We still handle WM_MOUSEMOVE in CInputSystem for regular mouse events,
      // only raw goes through SDL. This is done in order to maintain legacy
      // mouse behaviour for Windows users.
    ...
    case SDL_MOUSEBUTTONDOWN: {
      ...
      //  2 = Middle button
      //  3 = Right button
      //  4 = Wheel Forward		; These events are handled by
      //  SDL_MOUSEWHEEL and don't come through here. 5 = Wheel Back
      //  ; 6 = Wheel Tilt Left		; 7 = Wheel Tilt Right	; 8 = Browser
      //  Back 9 = Browser Forward 10 = Task button (probably similar to
      //  Alt-Tab)
      ...
      case 3:
        ...
        break;
      default:
        // For all buttons above 4, map them to 4 & 5 forever. This is because
        // different mice
      ...
      const CocoaMouseButton_t cocoaButton =
          (CocoaMouseButton_t)(1 << (button - 1));
      ...
      if (bPressed) {
        if (m_bGotMouseButtonDown &&
            ((int)(event.button.timestamp - m_MouseButtonDownTimeStamp) <=
             sdl_double_click_time.GetInt()) &&
            (abs(event.button.x - m_MouseButtonDownX) <=
             sdl_double_click_size.GetInt()) &&
            (abs(event.button.y - m_MouseButtonDownY) <=
             sdl_double_click_size.GetInt())) {
          bDoublePress = true;
          m_bGotMouseButtonDown = false;
        } else {
      ...
      CCocoaEvent theEvent;
      theEvent.m_EventType =
          (bPressed) ? CocoaEvent_MouseButtonDown : CocoaEvent_MouseButtonUp;
    ...
    case SDL_WINDOWEVENT:
      ...
      case SDL_WINDOWEVENT_EXPOSED: {
        ...
        if ( iPanel != m_mapWindowToVPanel.InvalidIndex() )
                drawVGUI( m_pXDisplay, ev.xexpose.window, m_mapWindowToVPanel[
        iPanel ], m_GLContext ); m_mapSentInvalidate.RemoveAll();*/
        break;
      }
...
void CSDLMgr::DecWindowRefCount() {
  ...
  if (!m_nWindowRefCount) {
    ...
    // Clear the GL entrypoint pointers, ensuring we crash if someone tries to
    // call GL after we delete the context.
    Msg("%s: Calling ClearOpenGLEntryPoints. Should crash if someone calls GL "
        "after this.\n",
        __FUNCTION__);
...
//
void CSDLMgr::GetNativeDisplayInfo(int nDisplay, uint &nWidth, uint &nHeight,
                                   uint &nRefreshHz) {
  SDL_DisplayMode mode;
  ...
  if (nDisplay == -1) {
    if (g_bSDLDisplayindexSet) {
      ...
      // sdl_displayindex hasn't been parsed yet. This can happen in
      // CMaterialSystem::ModInit()
      //	before the config files have been read, etc. So in this case,
      //just grab the largest 	display we can find and return with that.
      int Width, Height;
...
void CSDLMgr::GetStackCrawl(CStackCrawlParams *params) { SDLAPP_FUNC; }
...
void CSDLMgr::SetGammaRamp(const uint16 *pRed, const uint16 *pGreen,
                           const uint16 *pBlue) {
  if (m_Window) {
...
#if defined(DX_TO_GL_ABSTRACTION)
void CSDLMgr::GetDesiredPixelFormatAttribsAndRendererInfo(
    uint **ptrOut, uint *countOut, GLMRendererInfoFields *rendInfoOut) {
  SDLAPP_FUNC;
...
void GLMDisplayMode::Dump(int which) {
  ...
  GLMPRINTF(("\n             # %-2d  width=%-4d  height=%-4d  refreshHz=%-2d",
             which, m_info.m_modePixelWidth, m_info.m_modePixelHeight,
             m_info.m_modeRefreshHz));
}
...
GLMDisplayDB *CSDLMgr::GetDisplayDB(void) {
  ...
  if (!m_displayDB) {
    m_displayDB = new GLMDisplayDB; // creating the DB object does not do much
                                    // other than init it to a good state.
    ...
    // side effect: we fill in m_force_vsync..
    {
      ...
      m_force_vsync =
          info.m_badDriver1064NV; // just force it if it's the bum NV driver
    }
#endif
  }
  return m_displayDB;
