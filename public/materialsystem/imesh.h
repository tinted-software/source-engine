...
struct VertexDesc_t {
  ...
  int m_ActualVertexSize; // Size of the vertices.. Some of the m_VertexSize_
                          // elements above are set to this value and some are
                          // set to zero depending on which fields exist in a
                          // buffer's vertex format.
...
inline void CVertexBuilder::Bind(IMatRenderContext *pContext, int nStreamID,
  ...
  if (m_pVertexBuffer && (m_nBufferOffset != INVALID_BUFFER_OFFSET)) {
    pContext->BindVertexBuffer(nStreamID, m_pVertexBuffer, m_nBufferOffset,
                               m_nFirstVertex, m_nTotalVertexCount,
                               usage ? usage
                                     : m_pVertexBuffer->GetVertexFormat());
  } else {
...
//-----------------------------------------------------------------------------
inline void
CVertexBuilder::SetCompressionType(VertexCompressionType_t compressionType) {
...
inline void CVertexBuilder::ValidateCompressionType() {
  ...
  if (vbCompressionType != VERTEX_COMPRESSION_NONE) {
    ...
    if (m_CompressionType != vbCompressionType) {
      Warning("ERROR: CVertexBuilder::SetCompressionType() must be called to "
...
inline void CVertexBuilder::VertexDX8ToX360(const ModelVertexDX8_t &vertex) {
  ...
  Assert(
      GetVertexElementSize(VERTEX_ELEMENT_POSITION, VERTEX_COMPRESSION_NONE) ==
  ...
  if (m_VertexSize_Normal) {
    Assert(
        GetVertexElementSize(VERTEX_ELEMENT_NORMAL, VERTEX_COMPRESSION_NONE) ==
  ...
  if (m_VertexSize_Color) {
    Assert(
        GetVertexElementSize(VERTEX_ELEMENT_COLOR, VERTEX_COMPRESSION_NONE) ==
...
inline void CVertexBuilder::Normal3f(float nx, float ny, float nz) {
  Assert(m_CompressionType ==
         VERTEX_COMPRESSION_NONE); // Use the templatized version if you want to
                                   // support compression
  Assert(m_pNormal);
...
inline CIndexBuilder::CIndexBuilder()
    : m_pIndexBuffer(0), m_nIndexCount(0), m_nCurrentIndex(0),
      m_nMaxIndexCount(0) {
...
//-----------------------------------------------------------------------------
inline void
CIndexBuilder::GenerateIndices(MaterialPrimitiveType_t primitiveType,
                               int nIndexCount) {
...
//-----------------------------------------------------------------------------
inline void
CMeshBuilder::ComputeNumVertsAndIndices(int *pMaxVertices, int *pMaxIndices,
                                        MaterialPrimitiveType_t type,
                                        int nPrimitiveCount) {
  switch (type) {
  case MATERIAL_POINTS:
