...
// V081 - 10/25/2016 - Added new Suspend/Resume texture streaming interfaces.
// Might also have added more calls here due
//                     to the streaming work that didn't get bumped, but we're
//                     not guarding versions on the TF branch very judiciously
//                     since we need to audit them when merging to SDK branch
//                     either way.
...
#define CREATERENDERTARGETFLAGS_TEMP                                           \
  0x00000010 // only allocates memory upon first resolve, destroyed at level end
...
enum RenderTargetSizeMode_t {
  ...
  RT_SIZE_LITERAL = 8, // Use the size passed in. Don't clamp it to the frame
                       // buffer size. Really.
  RT_SIZE_LITERAL_PICMIP = 9 // Use the size passed in, don't clamp to the frame
                             // buffer size, but do apply picmip restrictions.
...
public:
  ...
  // Stop attempting to stream in textures in response to usage.  Useful for
  // phases such as loading or other explicit operations that shouldn't take
  // usage of textures as a signal to stream them in at full rez.
  ...
  virtual void
  EndRenderTargetAllocation() = 0; // Simulate an Alt-Tab in here, which causes
                                   // a release/restore of all resources
  ...
  virtual bool GetTrueTypeGlyphs(
      HXUIFONT hFont, int numChars, wchar_t *pWch, int *pOffsetX, int *pOffsetY,
  ...
  // Contains context in so it can make decisions (i.e. if it's a model, ignore
  // certain cheat parameters)
  ...
  // Sets the override sizes for all render target size tests. These replace the
  // frame buffer size. Set them when you are rendering primarily to something
  // larger than the frame buffer (as in VR mode).
  ...
  // mode at the beginning of time. This was a thing that mattered a lot to old
  // hardware, but doesn't matter at all to new hardware, where new is defined
  // to be "anything from the last decade." However, we want to preserve legacy
  // behavior for the old games because it's easier than testing them.
  ...
  virtual ITextureCompositor *NewTextureCompositor(
      ...
      KeyValues *stageDesc, uint32 texCompositeCreateFlags = 0) = 0;

  // Loads the texture with the specified name, calls
  // pRecipient->OnAsyncFindComplete with the result from the main thread. once
  // the texture load is complete. If the texture cannot be found, the returned
  // texture will return true for IsError().
  ...
  virtual bool AddTextureCompositorTemplate(
      const char *pName, KeyValues *pTmplDesc,
      int nTexCompositeTemplateFlags = 0) = 0;

  // Performs final verification of all compositor templates (after they've all
...
public:
  ...
  virtual void CopyTextureToRenderTargetEx(int nRenderTargetID,
                                           ITexture *pTexture, Rect_t *pSrcRect,
                                           Rect_t *pDstRect = NULL) = 0;
  ...
  // screen depth texture (mostly on PC). Use this to mark it as invalid and use
  // a dummy texture for depth reads.
  ...
  // pRecipient->OnAsyncCreateComplete from the main thread. The texture will be
  // created using the destination format, and will optionally have mipmaps
  // generated. In case of error, the provided callback function will be called
  // with the error texture.
...
//-----------------------------------------------------------------------------
template <typename E> class CMatRenderData {
public:
...
template <typename E> inline CMatRenderData<E>::~CMatRenderData() { Release(); }

template <typename E> inline bool CMatRenderData<E>::IsValid() const {
...
template <typename E> inline void CMatRenderData<E>::Release() {
...
template <typename E> inline E *CMatRenderData<E>::Base() {
  return m_pRenderData;
}

template <typename E> inline const E *CMatRenderData<E>::Base() const {
  return m_pRenderData;
}

template <typename E> inline E &CMatRenderData<E>::operator[](int i) {
