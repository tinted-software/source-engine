...
public:
  ...
  // Methods to set/get a gamestats data container so client & server running in
  // same process can send combined data
  ...
  // limitation as ServerGameDLL::CanProvideLevel -- This is non-blocking, so it
  ...
  enum eFindMapResult {
    ...
    eFindMap_NotFound,
    // A fuzzy match for this mapname was found and pMapName was updated to the
    // full name.
    // Ex: cp_dust -> cp_dustbowl
    eFindMap_FuzzyMatch,
    // A match for this map name was found, and the map name was updated to the
    // canonical version of the
    // name.
    // Ex: workshop/1234 -> workshop/cp_qualified_name.ugc1234
    eFindMap_NonCanonical,
    // No currently available match for this map name could be found, but it may
    // be possible to load ( see caveat
...
public:
  ...
  enum ePrepareLevelResourcesResult {
    ...
    // Game DLL is async preparing (e.g. streaming resources). flProgress will
    // be filled if passed.
  ...
  enum eCanProvideLevelResult {
    ...
    // Can provide resources for this level, and pMapName has been updated to
    // the canonical name we would provide it
    // under (as with PrepareLevelResources)
    eCanProvideLevel_CanProvide,
    // We recognize this level name as something we might be able to prepare,
    // but without a blocking/async call to
