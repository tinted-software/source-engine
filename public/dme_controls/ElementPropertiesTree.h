...
public:
  ...
  virtual int AddItem(KeyValues *data, bool allowLabelEditing,
                      int parentItemIndex,
...
public:
  enum RefreshType_t {
    ...
    REFRESH_TREE_VIEW, // Tree topology changed; some attributes may be added or
                       // removed
  };
  ...
  virtual void Init();
  virtual void Refresh(RefreshType_t rebuild = REFRESH_TREE_VIEW,
  ...
  virtual bool GetItemDropContextMenu(int itemIndex, vgui::Menu *menu,
                                      CUtlVector<KeyValues *> &msglist);
  virtual vgui::HCursor GetItemDropCursor(int itemIndex,
                                          CUtlVector<KeyValues *> &msglist);
...
protected:
  ...
  struct TreeItem_t {
    ...
    CDmElement *
        m_pArrayElement; // points to the element referenced in an element array
  };
  ...
  bool BuildExpansionListToFindElement_R(CUtlRBTree<CDmElement *, int> &visited,
                                         int depth, SearchResult_t &sr,
                                         CDmElement *owner, CDmElement *element,
                                         char const *attributeName,
                                         int arrayIndex,
                                         CUtlVector<int> &expandIndices);
  void FindMatchingElements_R(CUtlRBTree<CDmElement *, int> &visited,
                              char const *searchstr, CDmElement *root,
                              CUtlVector<SearchResult_t> &list);
  void NavigateToSearchResult();

  void SpewOpenItems(int depth, OpenItemTree_t &tree, int nOpenTreeIndex,
                     int nItemIndex);
  ...
  void ExpandOpenItems(OpenItemTree_t &tree, int nOpenTreeIndex, int nItemIndex,
                       bool makeVisible);
  ...
  void CreateTreeEntry(int parentNodeIndex, CDmElement *obj,
  ...
  void SetupWidgetInfo(AttributeWidgetInfo_t *pInfo, CDmElement *obj,
                       CDmAttribute *pAttribute, int nArrayIndex = -1);

  // Creates an attribute data widget using a specifically requested widget
  vgui::Panel *CreateAttributeDataWidget(CDmElement *pElement,
  ...
  void InsertAttributeArrayMembers(int parentNodeIndex, CDmElement *obj,
                                   CDmAttribute *array);

  // Adds a single editable attribute of the element to the tree
  void InsertSingleAttribute(int parentNodeIndex, CDmElement *obj,
                             CDmAttribute *pAttribute, int nArrayIndex = -1);
  ...
  // Gets tree view text
  void GetTreeViewText(CDmElement *obj, CDmAttribute *pAttribute,
                       int nArrayIndex, char *pBuffer, int nMaxLen,
                       bool &editableText);
  ...
  KEYBINDING_FUNC(ondelete, KEY_DELETE, 0, OnKeyDelete,
                  "#elementpropertiestree_ondelete_help", 0);
  KEYBINDING_FUNC(onbackspace, KEY_BACKSPACE, 0, OnKeyBackspace,
                  "#elementpropertiestree_ondelete_help", 0);
  KEYBINDING_FUNC(onrefresh, KEY_F5, 0, OnRefresh,
                  "#elementpropertiestree_onrefresh_help", 0);
...
protected:
  ...
  DropOperation_t GetDropOperation(int itemIndex,
                                   CUtlVector<KeyValues *> &msglist);
  ...
  bool ShowSetElementAttributeDialog(CDmElement *pOwner,
                                     const char *pAttributeName, int nArrayItem,
  ...
  // Refreshes the color state of the tree
  void SetTreeItemColor(int nItemID, CDmElement *pEntryElement,
                        bool bIsElementArrayItem, bool bEditableLabel);
  ...
  CPropertiesTreeToolbar
      *m_pToolBar; // Forward/backward navigation and search fields
...
public:
  CElementPropertiesTree(vgui::Panel *parent, IDmNotify *pNotify,
                         CDmElement *pObject,
                         CDmeEditorTypeDictionary *pDict = NULL);

  virtual void Init();
  virtual void Refresh(CElementPropertiesTreeInternal::RefreshType_t rebuild =
                           CElementPropertiesTreeInternal::REFRESH_REBUILD,
                       bool preservePrevSelectedItem = false);
  virtual void GenerateChildrenOfNode(int itemIndex);
  virtual void SetObject(CDmElement *object);
  virtual void OnCommand(const char *cmd);
...
//-----------------------------------------------------------------------------
class CDmeElementPanel : public CElementPropertiesTreeInternal,
                         public IDmNotify {
