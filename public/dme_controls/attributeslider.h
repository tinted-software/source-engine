...
// we can also be driven by ctrl hovering over the preset pane or an individual
// control if we move from control to control in the preset or here, we need to
// be able to decay into/out of the various individual sliders
...
public:
  CAttributeSlider(CBaseAnimSetAttributeSliderPanel *parent,
                   const char *panelName, CDmElement *control);
  ...
  // NOTE: Slider control 'value' defaults to active, 'balance' and 'multilevel'
  ...
  // NOTE: This may not match the value pushed into the control because of
  // fading
...
private:
  ...
  enum SliderMode_t {
    ...
    SLIDER_MODE_LAST_DRAG_MODE =
        SLIDER_MODE_FIRST_DRAG_MODE + ANIM_CONTROL_COUNT - 1,
    SLIDER_MODE_LAST_TEXT_MODE =
        SLIDER_MODE_FIRST_TEXT_MODE + ANIM_CONTROL_COUNT - 1,

    SLIDER_MODE_NONE = -1,

    SLIDER_MODE_DRAG_VALUE = SLIDER_MODE_FIRST_DRAG_MODE + ANIM_CONTROL_VALUE,
    SLIDER_MODE_DRAG_BALANCE =
        SLIDER_MODE_FIRST_DRAG_MODE + ANIM_CONTROL_BALANCE,
    SLIDER_MODE_DRAG_MULTILEVEL =
        SLIDER_MODE_FIRST_DRAG_MODE + ANIM_CONTROL_MULTILEVEL,

    SLIDER_MODE_TEXT_VALUE = SLIDER_MODE_FIRST_TEXT_MODE + ANIM_CONTROL_VALUE,
    SLIDER_MODE_TEXT_BALANCE =
        SLIDER_MODE_FIRST_TEXT_MODE + ANIM_CONTROL_BALANCE,
    SLIDER_MODE_TEXT_MULTILEVEL =
        SLIDER_MODE_FIRST_TEXT_MODE + ANIM_CONTROL_MULTILEVEL,
  };
...
private:
  ...
  // Given a mouse position in (x,y) in local coordinates, which animation
  // control is it over?
  AnimationControlType_t DetermineControl(int x, int y);

  // Draws a tick on a circular control
  void DrawCircularTick(const Color &clr, float flValue, int nCenterX,
                        int nCenterY, float flRadius);

  // Draws a preview of a circular control
  void DrawCircularPreview(AnimationControlType_t type, bool bMainTick,
                           float flRadius);
...
private:
  ...
  CSubRectImage
      *m_pCircleImage; // The background for the balance + multilevel controls
  ...
  // The slider value; it may not match the control attribute value due to
  // blending
  ...
  CAttributeSliderTextEntry *m_pTextField; // if this is a stereo control, then
                                           // this will be the left text field
  ...
  float m_flDragStartValue; // What was the value of the slider before the drag
                            // started?
  float m_flDragStartBalance; // What was the balance of the slider before the
                              // drag started?
...
inline bool CAttributeSlider::IsDragging() const {
  ...
  return (m_SliderMode >= SLIDER_MODE_FIRST_DRAG_MODE &&
          m_SliderMode <= SLIDER_MODE_LAST_DRAG_MODE);
}
...
inline bool CAttributeSlider::IsInTextEntry() const {
  COMPILE_TIME_ASSERT(ANIM_CONTROL_COUNT < 4);
  return (m_SliderMode >= SLIDER_MODE_FIRST_TEXT_MODE &&
          m_SliderMode <= SLIDER_MODE_LAST_TEXT_MODE);
}
