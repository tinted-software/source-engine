...
struct FaderPreview_t {
  FaderPreview_t()
      : name(0), amount(0), isbeingdragged(false), holdingctrl(false),
        values(0) {}
  const char *name;
  float amount;
  bool isbeingdragged;
...
public:
  CBaseAnimSetPresetFaderPanel(vgui::Panel *parent, const char *className,
  ...
  virtual void ProceduralPreset_UpdateCrossfade(CDmePreset *pPreset,
                                                bool bFadeIn);
