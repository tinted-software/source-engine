...
public:
  CBaseAnimSetAttributeSliderPanel(vgui::Panel *parent, const char *className,
...
public:
  ...
  void
  GetChannelsForControl(CDmElement *control,
  ...
  virtual void StampValueIntoLogs(CDmElement *control,
                                  AnimationControlType_t type, float flValue);
...
protected:
  ...
  KEYBINDING_FUNC(deselectall, KEY_ESCAPE, 0, OnKBDeselectAll,
                  "#deselectall_help", 0);
...
protected:
  struct ChannelToSliderLookup_t {
    ...
    static bool Less(const ChannelToSliderLookup_t &lhs,
                     const ChannelToSliderLookup_t &rhs);
  };

  void UpdatePreviewSliderTimes();
  void ActivateControlSetInMode(int mode, int otherChannelsMode,
                                int hiddenChannelsMode,
                                CAttributeSlider *whichSlider = NULL);
  void MaybeAddPreviewLog(CDmeFilmClip *shot, CUtlVector<LogPreview_t> &list,
