...
public:
  virtual void GetKnownParticleDefinitions(
      CUtlVector<CDmeParticleSystemDefinition *> &definitions) = 0;
};
...
class CParticleSystemPropertiesPanel : public vgui::EditablePanel {
  ...
  // Sends the message 'ParticleSystemModified' when the particle system was
  // modified in any way Sends the message 'ParticleFunctionSelChanged' when the
  // selected particle function changed
  //	-- stores the selected CDmeParticleFunction in a subkey called
  //'function'
