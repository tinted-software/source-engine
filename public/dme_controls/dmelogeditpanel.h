...
private:
  ...
  template <class T>
  int FindOrAddKey(DmeTime_t time, DmeTime_t tolerance, int nComps,
                   float flValue);
  template <class T>
  int ModifyKey(int nPoint, DmeTime_t initialTime, DmeTime_t time, int nComps,
                float flValue);
...
//-----------------------------------------------------------------------------
template <class T>
int CDmeLogEditPanel::FindOrAddKey(DmeTime_t time, DmeTime_t tolerance,
                                   int nComps, float flValue) {
  ...
  return CastElement<CDmeTypedLog<T>>(m_hLog)->FindOrAddKey(time, tolerance,
                                                            vec);
}
...
//-----------------------------------------------------------------------------
template <class T>
int CDmeLogEditPanel::ModifyKey(int nPoint, DmeTime_t initialTime,
                                DmeTime_t time, int nComps, float flValue) {
  ...
  return CastElement<CDmeTypedLog<T>>(m_hLog)->FindOrAddKey(time, DmeTime_t(0),
                                                            vec);
}
...
public:
  ...
  void DoModal(CDmeLog *pLog, DmeTime_t startTime, DmeTime_t endTime,
