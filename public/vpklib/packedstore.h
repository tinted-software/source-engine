...
public:
  bool Less(const ChunkHashFraction_t &lhs, const ChunkHashFraction_t &rhs,
            void *pContext) {
...
enum ePackedStoreAddResultCode {
  ...
  EPADD_ADDSAMEFILE, // the file was already present, and the contents are the
                     // same as what you passed.
  EPADD_UPDATEFILE,  // the file was alreayd present and its contents have been
                     // updated
  EPADD_ERROR,       // some error has resulted
};

// Describe a file inside of a VPK file.  Is not memory efficient; only used for
// interface purposes and during file building
struct VPKContentFileInfo_t {
  ...
  // MD5Value_t m_md5Source; // source content before munging & release
  // optimization.  Used for incremental builds
...
public:
  ...
  bool ReadCacheLine(FileHandleTracker_t &fHandle,
                     CachedVPKRead_t &cachedVPKRead);
  bool BCanSatisfyFromReadCache(uint8 *pOutData, CPackedStoreFileHandle &handle,
                                FileHandleTracker_t &fHandle, int nDesiredPos,
                                int nNumBytes, int &nRead);
  bool BCanSatisfyFromReadCacheInternal(uint8 *pOutData,
                                        CPackedStoreFileHandle &handle,
                                        FileHandleTracker_t &fHandle,
                                        int nDesiredPos, int nNumBytes,
                                        int &nRead);
  ...
  CTSQueue<CachedVPKRead_t>
      m_queueCachedVPKReadsRetry; // all the reads that have failed
  CUtlLinkedList<CachedVPKRead_t>
      m_listCachedVPKReadsFailed; // all the reads that have failed
...
class CPackedStore {
public:
  CPackedStore(char const *pFileBasename, char *pszFName, IBaseFileSystem *pFS,
  ...
  /// Add/update the given file to the directory.  Does not write any chunk
  /// files
  void AddFileToDirectory(const VPKContentFileInfo_t &info);

  /// Remove the specified file from the directory.  Returns true if removed,
  /// false if not found
  bool RemoveFileFromDirectory(const char *pszName);

  /// Add file, writing file data to the end
  /// of the current chunk
  ePackedStoreAddResultCode AddFile(char const *pFile, uint16 nMetaDataSize,
                                    const void *pFileData, uint32 nFullFileSize,
                                    bool bMultiChunk,
                                    uint32 const *pCrcToUse = NULL);
  ...
  // Get a list of all the files in the zip You are responsible for freeing the
  // contents of outFilenames (call outFilenames.PurgeAndDeleteElements).
  int GetFileList(CUtlStringList &outFilenames, bool bFormattedOutput,
                  bool bSortedOutput);

  // Get a list of all files that match the given wildcard string
  int GetFileList(const char *pWildCard, CUtlStringList &outFilenames,
                  bool bFormattedOutput, bool bSortedOutput);

  /// Get a list of all files that match the given wildcard string, fetching all
  /// the details at once
  void GetFileList(const char *pWildcard,
                   CUtlVector<VPKContentFileInfo_t> &outVecResults);

  // Get a list of all directories of the given wildcard
  int GetFileAndDirLists(const char *pWildCard, CUtlStringList &outDirnames,
                         CUtlStringList &outFilenames, bool bSortedOutput);
  int GetFileAndDirLists(CUtlStringList &outDirnames,
                         CUtlStringList &outFilenames, bool bSortedOutput);
  ...
  /// Re-hash a single chunk file.  Don't forget to rehash the metadata
  /// afterwords!
  void HashChunkFile(int iChunkFileIndex);

  bool HashEntirePackFile(CPackedStoreFileHandle &handle, int64 &nFileSize,
                          int nFileFraction, int nFractionSize,
                          FileHash_t &fileHash);
  ...
  CUtlSortVector<ChunkHashFraction_t, ChunkHashFractionLess_t> &
  AccessPackFileHashes() {
    return m_vecChunkHashFraction;
  }
  bool FindFileHashFraction(int nPackFileNumber, int nFileFraction,
                            ChunkHashFraction_t &chunkFileHashFraction);
  void GetPackFileLoadErrorSummary(CUtlString &sErrors);

  void GetPackFileName(CPackedStoreFileHandle &handle, char *pchFileNameOut,
                       int cchFileNameOut) const;
  void GetDataFileName(char *pchFileNameOut, int cchFileNameOut,
  ...
  ESignatureCheckResult CheckSignature(int nSignatureSize,
                                       const void *pSignature) const;

  void SetKeysForSigning(int nPrivateKeySize, const void *pPrivateKeyData,
                         int nPublicKeySize, const void *pPublicKeyData);
#endif
...
private:
  ...
  CUtlIntrusiveList<class CFileExtensionData>
      m_pExtensionData[PACKEDFILE_EXT_HASH_SIZE];

  CUtlVector<uint8> m_DirectoryData;
  CUtlBlockVector<uint8> m_EmbeddedChunkData;

  CUtlSortVector<ChunkHashFraction_t, ChunkHashFractionLess_t>
      m_vecChunkHashFraction;
  ...
  struct CFileHeaderFixedData *FindFileEntry(char const *pDirname,
  ...
  CUtlMap<int, CUtlStringList *>
      m_dirContents; // ...is the key to this map of filenames
...
FORCEINLINE void CPackedStoreFileHandle::GetPackFileName(char *pchFileNameOut,
                                                         int cchFileNameOut) {
