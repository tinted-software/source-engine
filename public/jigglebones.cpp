...
#ifdef CLIENT_DLL
//-----------------------------------------------------------------------------
ConVar cl_jiggle_bone_debug(
    "cl_jiggle_bone_debug", "0", FCVAR_CHEAT,
    "Display physics-based 'jiggle bone' debugging information");
ConVar cl_jiggle_bone_debug_yaw_constraints(
    "cl_jiggle_bone_debug_yaw_constraints", "0", FCVAR_CHEAT,
    "Display physics-based 'jiggle bone' debugging information");
ConVar cl_jiggle_bone_debug_pitch_constraints(
    "cl_jiggle_bone_debug_pitch_constraints", "0", FCVAR_CHEAT,
    "Display physics-based 'jiggle bone' debugging information");
#endif // CLIENT_DLL

ConVar
    cl_jiggle_bone_framerate_cutoff("cl_jiggle_bone_framerate_cutoff", "20", 0,
                                    "Skip jiggle bone simulation if framerate "
                                    "drops below this value (frames/second)");

//-----------------------------------------------------------------------------
JiggleData *CJiggleBones::GetJiggleData(int bone, float currenttime,
 ...
 */
void CJiggleBones::BuildJiggleTransformations(
    int boneIndex, float currenttime, const mstudiojigglebone_t *jiggleInfo,
  ...
  JiggleData *data =
      GetJiggleData(boneIndex, currenttime, goalBasePosition, goalTip);
  if (!data) {
    return;
  }
  ...
  // if framerate is too low, skip jigglebones altogether, since movement will
  // be too large between frames to simulate with a simple Euler integration
  float deltaT = currenttime - data->lastUpdate;

  const float thousandHZ = 0.001f;
  bool bMaxDeltaT = deltaT < thousandHZ;
  bool bUseGoalMatrix =
      cl_jiggle_bone_framerate_cutoff.GetFloat() <= 0.0f ||
      deltaT > (1.0f / cl_jiggle_bone_framerate_cutoff.GetFloat());

  if (bUseGoalMatrix) {
    // We hit the jiggle bone framerate cutoff. Reset the useGoalMatrixCount so
    // we
  ...
  // we want lastUpdate here, so if jigglebones were skipped they get
  // reinitialized if they turn back on
  ...
  if (jiggleInfo->flags & (JIGGLE_IS_FLEXIBLE | JIGGLE_IS_RIGID)) {
    ...
    if (jiggleInfo->flags & JIGGLE_IS_FLEXIBLE) {
      ...
      // yaw spring
      float yawAccel = jiggleInfo->yawStiffness * localError.x -
                       jiggleInfo->yawDamping * localVel.x;

      // pitch spring
      float pitchAccel = jiggleInfo->pitchStiffness * localError.y -
                         jiggleInfo->pitchDamping * localVel.y;
      ...
      if (jiggleInfo->flags & JIGGLE_HAS_LENGTH_CONSTRAINT) {
        ...
        // along spring
        float alongAccel = jiggleInfo->alongStiffness * localError.z -
                           jiggleInfo->alongDamping * localVel.z;

        // drive tip towards goal tip position
        data->tipAccel += yawAccel * goalLeft + pitchAccel * goalUp +
                          alongAccel * goalForward;
      }
    }
    ...
    //
    if (jiggleInfo->flags &
        (JIGGLE_HAS_YAW_CONSTRAINT | JIGGLE_HAS_PITCH_CONSTRAINT)) {
      ...
      if (jiggleInfo->flags & JIGGLE_HAS_YAW_CONSTRAINT) {
        ...
        if (isAtLimit) {
          ...
          if (cl_jiggle_bone_debug_yaw_constraints.GetBool()) {
            ...
            if (debugoverlay) {
              debugoverlay->AddLineOverlay(
                  goalBasePosition, goalBasePosition + axisSize * limitLeft, 0,
                  255, 255, true, dT);
              debugoverlay->AddLineOverlay(
                  goalBasePosition, goalBasePosition + axisSize * limitUp, 255,
                  255, 0, true, dT);
              debugoverlay->AddLineOverlay(
                  goalBasePosition, goalBasePosition + axisSize * limitForward,
                  255, 0, 255, true, dT);
            }
          }
#endif // CLIENT_DLL
          ...
          // clip to limit plane
          data->tipPos = goalBasePosition + limitAlong.y * limitUp +
                         limitAlong.z * limitForward;

          // removed friction and velocity clipping against constraint - was
      ...
      if (jiggleInfo->flags & JIGGLE_HAS_PITCH_CONSTRAINT) {
        ...
        if (isAtLimit) {
          ...
          // clip to limit plane
          data->tipPos = goalBasePosition + limitAlong.x * limitLeft +
                         limitAlong.z * limitForward;
    ...
    if (DotProduct(left, data->lastLeft) < 0.0f) {
      ...
      // resulting in the cross product result flipping 180 degrees around the
      // up vector.  Flip it back.
      left = -left;
    }
    data->lastLeft = left;
    ...
    if (cl_jiggle_bone_debug.GetBool()) {
      if (debugoverlay) {
        debugoverlay->AddLineOverlay(goalBasePosition,
                                     goalBasePosition + 10.0f * data->lastLeft,
                                     255, 0, 255, true, 0.01f);
      }
    }
#endif
  ...
  if (jiggleInfo->flags & JIGGLE_HAS_BASE_SPRING) {
    ...
    data->baseAccel += jiggleInfo->baseStiffness * error -
                       jiggleInfo->baseDamping * data->baseVel;
    ...
    if (localError.x < jiggleInfo->baseMinLeft) {
      ...
      // friction
      data->baseAccel -= jiggleInfo->baseLeftFriction *
                         (localVel.y * goalUp + localVel.z * goalForward);
    ...
    if (localError.y < jiggleInfo->baseMinUp) {
      ...
      // friction
      data->baseAccel -= jiggleInfo->baseUpFriction *
                         (localVel.x * goalLeft + localVel.z * goalForward);
    ...
    if (localError.z < jiggleInfo->baseMinForward) {
      ...
      // friction
      data->baseAccel -= jiggleInfo->baseForwardFriction *
                         (localVel.x * goalLeft + localVel.y * goalUp);
    ...
    data->basePos = goalBasePosition + localError.x * goalLeft +
                    localError.y * goalUp + localError.z * goalForward;
    ...
    if (cl_jiggle_bone_debug.GetBool()) {
      if (debugoverlay) {
        debugoverlay->AddLineOverlay(
            data->lastBoingPos, goalBasePosition, 0, 128,
    ...
    if ((speed > minSpeed || data->boingSpeed > minSpeed) &&
        data->boingTime > minReBoingTime) {
      if (fabs(data->boingSpeed - speed) > jiggleInfo->boingImpactSpeed ||
        ...
        if (cl_jiggle_bone_debug.GetBool()) {
          if (debugoverlay) {
            debugoverlay->AddLineOverlay(
                goalBasePosition, goalBasePosition + 5.0f * data->boingDir, 255,
                255, 0, true, 999.9f);
            debugoverlay->AddLineOverlay(goalBasePosition,
                                         goalBasePosition + Vector(0.1, 0, 0),
                                         128, 128, 0, true, 999.9f);
            debugoverlay->AddLineOverlay(goalBasePosition,
                                         goalBasePosition + Vector(0, 0.1, 0),
                                         128, 128, 0, true, 999.9f);
            debugoverlay->AddLineOverlay(goalBasePosition,
                                         goalBasePosition + Vector(0, 0, 0.1),
                                         128, 128, 0, true, 999.9f);
          }
        }
#endif
    ...
    if (damping < 0.01f) {
      ...
      float flex = jiggleInfo->boingAmplitude *
                   cos(jiggleInfo->boingFrequency * data->boingTime) * damping;
      ...
      if (cl_jiggle_bone_debug.GetBool()) {
        ...
        if (debugoverlay) {
          debugoverlay->AddLineOverlay(
              goalBasePosition, goalBasePosition + 50.0f * data->boingDir, 255,
              255, 0, true, dT);
          debugoverlay->AddLineOverlay(goalBasePosition,
                                       goalBasePosition + 50.0f * boingSide,
                                       255, 0, 255, true, dT);
          debugoverlay->AddLineOverlay(
              goalBasePosition, goalBasePosition + 50.0f * boingOtherSide, 0,
              255, 255, true, dT);
        }
      }
#endif
  ...
  if (cl_jiggle_bone_debug.GetBool()) {
    ...
    if (debugoverlay) {
      debugoverlay->AddLineOverlay(goalBasePosition,
                                   goalBasePosition + axisSize * goalLeft, 255,
                                   0, 0, true, dT);
      debugoverlay->AddLineOverlay(goalBasePosition,
                                   goalBasePosition + axisSize * goalUp, 0, 255,
                                   0, true, dT);
      debugoverlay->AddLineOverlay(goalBasePosition,
                                   goalBasePosition + axisSize * goalForward, 0,
                                   0, 255, true, dT);
    }

    if (cl_jiggle_bone_debug.GetInt() > 1) {
      DevMsg("Jiggle bone #%d, basePos( %3.2f, %3.2f, %3.2f ), tipPos( %3.2f, "
             "%3.2f, %3.2f ), left( %3.2f, %3.2f, %3.2f ), up( %3.2f, %3.2f, "
             "%3.2f ), forward( %3.2f, %3.2f, %3.2f )\n",
             data->bone, goalBasePosition.x, goalBasePosition.y,
             goalBasePosition.z, data->tipPos.x, data->tipPos.y, data->tipPos.z,
             goalLeft.x, goalLeft.y, goalLeft.z, goalUp.x, goalUp.y, goalUp.z,
    ...
    if (jiggleInfo->flags & JIGGLE_IS_BOING) {
      if (cl_jiggle_bone_debug.GetInt() > 2) {
        DevMsg("  boingSpeed = %3.2f, boingVelDir( %3.2f, %3.2f, %3.2f )\n",
               data->boingVelDir.Length() / deltaT, data->boingVelDir.x,
               data->boingVelDir.y, data->boingVelDir.z);
      }
    }
  }
#endif // CLIENT_DLL
