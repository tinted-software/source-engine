...
public:
  ...
  // Must be passed in when calling for Apply, will be created and passed back
  // for Extract
...
enum SFMPhonemeExtractType_t {
  EXTRACT_WIPE_RANGE = 0, // Wipe logs from start of first selected clip to end
                          // of last selected clip
  EXTRACT_WIPE_CLIP,   // Wipe all log entries (for facial controls) over entire
                       // clip
  EXTRACT_WIPE_SOUNDS, // Leave logs untouched, except underneath each selected
                       // .wav file
...
public:
  ...
  virtual void GetAPIInfo(int nIndex, CUtlString *pPrintName,
                          PE_APITYPE *pAPIType) = 0;

  virtual void Extract(const PE_APITYPE &apiType, ExtractDesc_t &info,
                       bool bWritePhonemesToWavFiles = false) = 0;
...
inline CExtractInfo::CExtractInfo()
    : m_pClip(0), m_pSound(0), m_bSentenceValid(false), m_bUseSentence(false),
      m_flDuration(0.0f) {}
...
inline CExtractInfo::~CExtractInfo() { ClearTags(); }
