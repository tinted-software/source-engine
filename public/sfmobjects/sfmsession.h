...
public:
  ...
  CDmAttribute *GetSettingsAttribute(const char *pSettingName,
  ...
  CDmeGameModel *CreateEditorGameModel(studiohdr_t *hdr,
...
private:
  ...
  //	CUtlVector< CDmeHandle< CDmeClip > >	m_hSelectedClip[
  //NUM_SELECTION_TYPES ];
};
...
inline const T &CSFMSession::GetSettings(const char *pSettingName) const {
  CDmElement *pSettings =
      m_hRoot.Get() ? m_hRoot->GetValueElement<CDmElement>("settings") : NULL;
  if (pSettings)
...
inline CDmAttribute *CSFMSession::GetSettingsAttribute(const char *pSettingName,
                                                       DmAttributeType_t type) {
  CDmElement *pSettings =
      m_hRoot.Get() ? m_hRoot->GetValueElement<CDmElement>("settings") : NULL;
  if (pSettings)
