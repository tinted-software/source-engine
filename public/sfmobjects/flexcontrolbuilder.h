...
public:
  ...
  void CreateAnimationSetControls(CDmeFilmClip *pMovie,
...
private:
  ...
  // This builds a list of the desired input controls we need to have controls
  ...
  void RemoveUnusedControlsAndChannels(CDmeAnimationSet *pAnimationSet,
  ...
  void FixupExistingFlexControlLogList(CDmeFilmClip *pCurrentClip,
                                       CDmeGameModel *pGameModel);

  // Converts existing logs to balance/value if necessary while building the
  // list Also deletes existing infrastructure (balance ops, channels, flex
  // controller ops).
  void BuildExistingFlexControlLogList(CDmeFilmClip *pCurrentClip,
                                       CDmeGameModel *pGameModel);
  ...
  void CleanupExistingFlexController(CDmeGameModel *pGameModel,
                                     CDmeGlobalFlexControllerOperator *pOp);

  // Finds a channels clip containing a particular channel
  CDmeChannelsClip *FindChannelsClipContainingChannel(CDmeFilmClip *pClip,
                                                      CDmeChannel *pSearch);

  // Returns an existing mono log
  void GetExistingMonoLog(ExistingLogInfo_t *pLog, CDmeFilmClip *pClip,
  ...
  void ConvertExistingLRLogs(ExistingLogInfo_t *pLogs, CDmeFilmClip *pClip,
  ...
  void ComputeChannelTimeTransform(DmeTime_t *pOffset, double *pScale,
                                   CDmeChannelsClip *pChannelsClip);
  bool ComputeChannelTimeTransform(DmeTime_t *pOffset, double *pScale,
                                   CDmeFilmClip *pClip, CDmeChannel *pChannel);
  ...
  // dmegamemodel
  void AttachControlsToGameModel(CDmeAnimationSet *pAnimationSet,
                                 CDmeGameModel *pGameModel,
                                 CDmeChannelsClip *pChannelsClip);

  // Connects a mono control to a single flex controller op
  void BuildFlexControllerOps(CDmeGameModel *pGameModel,
                              CDmeChannelsClip *pChannelsClip,
                              ControlInfo_t &info, ControlField_t field);

  // Connects a stereo control to a two flex controller ops
  void BuildStereoFlexControllerOps(CDmeAnimationSet *pAnimationSet,
                                    CDmeGameModel *pGameModel,
                                    CDmeChannelsClip *pChannelsClip,
                                    ControlInfo_t &info);
