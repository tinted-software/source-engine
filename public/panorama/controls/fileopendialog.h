...
public:
  CFileOpenDialog(CPanel2D *parent, const char *pchPanelID,
                  FileOpenDialogType_t type);
  CFileOpenDialog(panorama::IUIWindow *pParent, const char *pchPanelID,
                  FileOpenDialogType_t type);
  ...
  /*
          messages sent:
                  "FileSelected"
                          ...
                          "filterinfo"	// Returns the filter info associated
     with the active filter "FileSelectionCancelled"
  */

  static bool FileNameWildCardMatch(char const *pchFileName,
                                    char const *pchPattern);
  ...
  bool EventClose();
  bool EventFolderUp();
  bool EventColumnSortingChanged(const CPanelPtr<IUIPanel> &pPanel,
                                 int nColumn);
  bool EventSelectFile(const CPanelPtr<IUIPanel> &pPanel, uint32 unModifiers);
  bool EventDoubleClickFile(const CPanelPtr<IUIPanel> &pPanel,
                            uint32 unModifiers);
...
public:
  ...
  virtual bool
  OnMouseButtonDoubleClick(const panorama::MouseData_t &code) OVERRIDE;
