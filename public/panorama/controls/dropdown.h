...
public:
  ...
  void FindOptionIDsByClass(const char *pchClassName,
                            CUtlVector<CUtlString> &vecIDsOut);
  void SortOptions(int(__cdecl *pfnCompare)(const ClientPanelPtr_t *,
...
protected:
  ...
  bool EventDropDownMenuClosed(bool bSelectionChanged,
                               CPanelPtr<CPanel2D> pPanel);
  bool
  EventResetToDefault(const panorama::CPanelPtr<panorama::IUIPanel> &pPanel);

  virtual void OnInitializedFromLayout();
  void ShowMenu();
  void UpdateSelectedChild(bool bSuppressChangedEvent,
                           bool bInvalidateAlways = false);
...
public:
  ...
  virtual void OnLayoutTraverse(float flFinalWidth, float flFinalHeight);
  virtual void OnResetToDefaultValue();
  virtual IUIPanel *GetLocalizationParent() const {
    return m_pDropDown->UIPanel();
  }
...
protected:
  bool EventPanelActivated(const CPanelPtr<IUIPanel> &ptrPanel,
                           EPanelEventSource_t eSource);
