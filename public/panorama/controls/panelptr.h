...
//
template <class T> class CPanelPtr {
public:
  CPanelPtr() { Clear(); }
  CPanelPtr(const CPanelPtr &rhs) { *this = rhs; }
  CPanelPtr(const IUIPanel *pPanel) { Set(pPanel); }
  CPanelPtr(const IUIPanelClient *pPanel) {
    Set(pPanel ? pPanel->UIPanel() : (IUIPanel *)NULL);
  }

  CPanelPtr<T> &operator=(const CPanelPtr<T> &ptr) {
    m_handle = ptr.m_handle;
    return *this;
  }
  T *operator=(T *pPanel) {
    Set(pPanel);
  ...
  void Clear() { m_handle = PanelHandle_t::InvalidHandle(); }
  ...
  template <class U> bool operator==(const CPanelPtr<U> &rhs) const {
...
template <class T> inline uint32 HashItem(const CPanelPtr<T> &item) {
