...
public:
  ...
  bool OnSetImageSource(const CPanelPtr<IUIPanel> &pPanel,
                        const char *pchImageSource);
  bool OnClearImageSource(const CPanelPtr<IUIPanel> &pPanel);

  IImageSource *GetImage() { return m_pImage; }

  // Set an image from a URL (file://, http://), if pchDefaultImage is specified
  // it must be a file:// url and will be used while the actual image is loaded
  // asynchronously, it will also remain in use if the actual image fails to
  // load
  void SetImage(const char *pchImageURL, const char *pchDefaultImageURL = NULL,
                bool bPrioritizeLoad = false, int nResizeWidth = -1,
                int nResizeHeight = -1);

  // Set an image from an already created IImageSource, you should almost always
  // use the simpler SetImage( pchImageURL, pchDefaultImageURL ) call.
  ...
  void SetAlignment(EImageHorizontalAlignment horAlign,
                    EImageVerticalAlignment verAlign);
