...
DECLARE_PANEL_EVENT1(
    TextEntrySubmit,
    const char *); // always raised when user is done submitting text
DECLARE_PANEL_EVENT0(
    TextEntryChanged); // call CTextEntry::RaiseChangeEvents() to enable
...
public:
  ...
  uint GetCharCount() const {
    return m_vecWCharData.Count() - 1;
  } // m_vecWCharData is terminated, so remove extra char count
  ...
  void RequestControlString() {
    Assert(false);
  } // no op, we already have the string
  ...
  void LockSelection(bool bLockSelection) {
    m_bSelectionLocked = bLockSelection;
  }
  ...
  panorama::CTextInputHandler *GetTextInputHandler() {
    return m_pTextInputHandler.Get();
  }
...
#if defined(SOURCE2_PANORAMA)
  virtual void
  IME_SetLoggingChannel(LoggingChannelID_t loggingChannel) OVERRIDE;
  ...
  virtual void IME_ReplaceCharacters(const wchar_t *pString, uint32 nStart,
                                     uint32 nEnd) OVERRIDE;
  virtual void IME_SetSelection(uint32 nStart, uint32 nEnd) OVERRIDE;
  virtual void IME_SetWideCursor(bool bWide) OVERRIDE;
  virtual void IME_HighlightCompositionStringText(
      uint32 nPos, uint32 nLen, IMETextHighlightStyle highlightStyle) OVERRIDE;
  virtual void IME_DeleteSelection() OVERRIDE;
  virtual void IME_RemoveInputWindow() OVERRIDE;
  virtual void IME_DisplayInputWindow(const wchar_t *pReadingString,
                                      const IMERectF *pPosition) OVERRIDE;
...
private:
  bool OnTextEntryShowTextInputHandler(
      const panorama::CPanelPtr<panorama::IUIPanel> &ptrPanel);
  bool OnTextEntryHideTextInputHandler(
      const panorama::CPanelPtr<panorama::IUIPanel> &ptrPanel);
  bool EventActivated(const CPanelPtr<IUIPanel> &pPanel,
                      EPanelEventSource_t eSource);
  bool OnTextEntryScrollToCursor(
      const panorama::CPanelPtr<panorama::IUIPanel> &ptrPanel);
  bool EventInputFocusTopLevelChanged(CPanelPtr<CPanel2D> ptrPanel);
  bool
  HandleTextInputFinished(const panorama::CPanelPtr<panorama::IUIPanel> &pPanel,
                          bool bFinished, char const *pchText);
  ...
  bool m_bMultiline; // used to determine whether to swallow multiline-only
                     // characters (\n, etc.)
  ...
  // Translation of text in single line entries to scroll to show where the
  // cursor is
  ...
  // only raise text changed events when requested, as we have to convert every
  // character to UTF8 from unicode
