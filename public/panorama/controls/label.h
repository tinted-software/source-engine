...
public:
  ...
  virtual void Paint();
  virtual bool
  BSetProperties(const CUtlVector<ParsedPanelProperty_t> &vecProperties);

  virtual void SetupJavascriptObjectTemplate() OVERRIDE;

  enum ETextType { k_ETextTypePlain, k_ETextTypeUnlocalized, k_ETextTypeHTML };
  virtual void SetText(const char *pchValue,
                       ETextType eTextType = k_ETextTypePlain);
  virtual void AppendText(const char *pchValue,
                          ETextType eTextType = k_ETextTypePlain);
  virtual const char *PchGetText() const {
    return m_pLocText ? m_pLocText->String() : "";
  }

  bool OnLocalizationChanged(
      const CPanelPtr<IUIPanel> &pPanel,
      const ILocalizationString
          *pString); // can't be virtual as it is an event catcher

  void SetMaxChars(EStringTruncationStyle eTruncationStyle, uint32 nMaxChars);
  void SetStyleForRange(int iStartIndex, int iEndIndex,
  ...
  void ValidateLinkVector(CValidator &validator,
                          CUtlVector<CUtlString> *pvecLinks);
#endif
  ...
  virtual void GetDebugPropertyInfo(
      CUtlVector<DebugPropertyOutput_t *> *pvecProperties) OVERRIDE;
...
private:
  struct TextRangeFormat_t {
    ...
    int m_iHREF; // index into m_vecParsedHREFs if clickable. Multiple ranges
                 // could have the same URL, so hold indexes instead of copy
                 // strings
    int m_iMouseOver; // index into m_vecParsedMouseOvers if clickable. Multiple
                      // ranges could have the same URL, so hold indexes instead
                      // of copy strings
    int m_iMouseOut;  // index into m_vecParsedMouseOuts if clickable. Multiple
                      // ranges could have the same URL, so hold indexes instead
                      // of copy strings
    int m_iContextMenu; // index into m_vecParsedContextMenus if clickable.
                        // Multiple ranges could have the same URL, so hold
                        // indexes instead of copy strings
  ...
  void SetTextInternal(const char *pchValue, ETextType eTextType,
                       bool bTrustedSource);
  void SetFromHTMLInternal(const char *pchText, bool bAppend, bool bTrusted);
  int ParseStringFromTag(const char *pchTag, const char *pchString,
                         CUtlVector<CUtlString> **ppVecStrings);
  ...
  TextRangeFormat_t &
  AppendTextRangeFormat(int iStartChar, int iEndChar, uint unHTMLFormatFlags,
                        const CUtlVector<CPanoramaSymbol> &vecStyles, int iHREF,
                        int iMouseOver, int iMouseOut, int iContextMenu,
                        const Color &color, const char *pszChildID,
                        bool bChildOwner);
  void RemoveTextRangeFormats();
  bool BCoordsInTextRange(const TextRangeFormat_t &rangeFormat, float flX,
                          float flY);
  int FindTextRangeAt(float flX, float flY);

  IUITextLayout *CreateTextLayout(float flWidth, float flHeight,
                                  bool bUseChildDesiredSize,
                                  const char *pchOptStringToUse = NULL);
  ...
  static void CloneLinksVector(CUtlVector<CUtlString> *pSourceLinks,
                               CUtlVector<CUtlString> **ppDestinationLinks);
  ...
  CMutableLocalizationString
      m_pLocTextHTML; // the base string with html markup preserved
  ...
  CUtlVector<TextRangeFormat_t>
      m_vecTextRangeFormats; // list of parsed text range formats
  CUtlVector<CUtlString>
      *m_pvecParsedHREFs; // parsed href.. indexes are added to text range
                          // formats. Multiple ranges could have same URL.
  CUtlVector<CUtlString>
      *m_pvecParsedMouseOvers; // parsed onmouseover.. indexes are added to text
                               // range formats. Multiple ranges could have same
                               // URL.
  CUtlVector<CUtlString>
      *m_pvecParsedMouseOuts; // parsed onmouseout.. indexes are added to text
                              // range formats. Multiple ranges could have same
                              // URL.
  CUtlVector<CUtlString>
      *m_pvecParsedContextMenus; // parsed oncontextmenu.. indexes are added to
                                 // text range formats. Multiple ranges could
                                 // have same URL.
  TextRangeFormat_t
      *m_pLastHoverRange; // last text range the mouse was hovering over
