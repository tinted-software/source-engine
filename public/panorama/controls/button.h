...
public:
  ...
  // events
  virtual bool EventActivated(const CPanelPtr<IUIPanel> &pPanel,
                              EPanelEventSource_t eSource);

  virtual void SetupJavascriptObjectTemplate() OVERRIDE;

  virtual const char *PchGetText() const {
    return m_pLabel ? m_pLabel->PchGetText() : "";
  }
...
public:
  ...
  // the specified radio, member of the specified group, has been activated
  // (broadcast)
  bool EventOtherActivated(const CPanelPtr<IUIPanel> &pPanel,
                           const char *szGroup);
