...
public:
  ...
  // Trigger fast motion style temporarily, do this if you are directly setting
  // focus ahead a bunch
  void TriggerFastMotion();
  void BumpFastMotionTimeout();

  void SetHorizontalCount(int nCount) {
    SetHorizontalAndVerticalCount(nCount, m_nVerticalCount);
  }
  void SetVerticalCount(int nCount) {
    SetHorizontalAndVerticalCount(m_nHorizontalCount, nCount);
  }
  int GetHorizontalCount() const { return m_nHorizontalCount; }
  int GetVerticalCount() const { return m_nVerticalCount; }

  void SetHorizontalFocusLimit(int nCount) {
    m_nHorizontalFocusLimit = nCount;
...
private:
  ...
  // Override how far right you can move before all items must shift, should be
  // smaller than m_nHorizontalCount
