...
protected:
  bool EventCancelled(const CPanelPtr<IUIPanel> &pPanel,
...
private:
  bool EventSliderValueChanged(const CPanelPtr<IUIPanel> &pPanel,
                               float flValue);
...
private:
  ...
  static bool SortRepresentations(const Representation_t &lhs,
                                  const Representation_t &rhs);
...
protected:
  ...
  bool EventActivated(const CPanelPtr<IUIPanel> &ptrPanel,
                      EPanelEventSource_t eSource);
  bool EventCancelled(const CPanelPtr<IUIPanel> &ptrPanel,
                      EPanelEventSource_t eSource);
...
private:
  ...
  int m_iDesiredVideoRepresentation; // representation selected by user or -1.
                                     // Video player might not yet have changed
                                     // to playing this rep
};
