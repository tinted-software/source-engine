...
struct HtmlFormHasFocus_t {
  HtmlFormHasFocus_t()
      : m_bInput(false), m_bInputHasMultiplePeers(false),
        m_bUserInputThisPage(false), m_bFocusedElementChanged(true) {}
  void Reset() {
    m_bInput = false;
  ...
  bool operator==(const HtmlFormHasFocus_t &rhs) const {
    return rhs.m_bInput == m_bInput && rhs.m_sName == m_sName &&
...
DECLARE_PANEL_EVENT5(HTMLSecurityStatus, const char *, bool, bool, bool,
                     const char *)
...
DECLARE_PANEL_EVENT2(HTMLFormHasFocus, HtmlFormHasFocus_t,
                     const char * /* URL */)
...
enum CursorCode { eCursorNone, eCursorArrow };

class IUIDoubleBufferedTexture;
class CTransform3D;
extern const int
    k_nExtraScrollRoom; // max number of padding pixels to use if needed

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
class CHTML : public CPanel2D,
              public ITextInputControl
#if !defined(SOURCE2_PANORAMA) && !defined(PANORAMA_PUBLIC_STEAM_SDK)
    ,
...
public:
  ...
  virtual bool BRequiresContentClipLayer() OVERRIDE {
    return true;
  } // BUGBUG Alfred - fix ::Paint to scale u/v offsets rather than requiring a
    // clipping
  ...
  // run input event processing for something that may not be a real input
  // event, so don't bubble to parents, etc.
  bool OnGamePadDownImpl(const GamePadData_t &code,
                         bool *out_pbOptionalResult = nullptr);
  bool OnGamePadAnalogImpl(const GamePadData_t &code,
                           bool *out_pbOptionalResult = nullptr);

  bool ProcessAnalogScroll(float fValueX, float fValueY, double fTimeDelta,
                           float fDeadzoneValue);
  bool ProcessAnalogZoom(float fValueX, float fValueY, double fTimeDelta,
                         float fDeadzoneValue);
  ...
  void Copy();
  void Paste();
  void RequestLinkUnderGamepad() {
    RequestLinkAtPosition(GetActualLayoutWidth() / 2 - GetHScrollOffset(),
                          GetActualLayoutHeight() / 2 - GetVScrollOffset());
  }
  void RequestLinkUnderMouse() {
    RequestLinkAtPosition(m_flCursorX - GetHScrollOffset(),
                          m_flCursorY - GetVScrollOffset());
  }
  ...
  void SetFileDialogChoice(
      const char *pchFileName); // callback if cef wanted us to pick a file

  bool
  BAcceptMouseInput(); // returns true if the control is listening to mouse
                       // input right now, false if gamepad input mode is on
  virtual bool BRequiresFocus() OVERRIDE { return true; }

  bool BIgnoreMouseBackForwardButtons() {
    return m_bIgnoreMouseBackForwardButtons;
  }
  void SetIgnoreMouseBackForwardButtons(bool bIgnore) {
    m_bIgnoreMouseBackForwardButtons = bIgnore;
  }

  const char *PchCurrentURL() {
    return m_sCurrentURL;
  } // the current URL the browser has loaded
  const char *PchCurrentPageTitle() {
    return m_sHTMLTitle;
  } // the title of the currently loaded page

  void SaveCurrentPageToJPEG(const char *pchFileName, int nWide,
                             int nTall); // save this current page to a jpeg
  ...
  public:
    CHTMLVerticalScrollBar(CPanel2D *parent, const char *pchPanelID)
      ...
      if (flHeight > 0.00001f) {
        if (m_bMouseWentDownOnThumb) {
          ...
          SetScrollWindowPosition(
              clamp(flPosition, 0.0f, GetRangeSize() - GetScrollWindowSize()),
              true);
        } else {
          float flPercent = m_flMouseY / flHeight;
          float flPos = GetRangeSize() * flPercent;
          SetScrollWindowPosition(
              clamp(flPos, 0.0f, GetRangeSize() - GetScrollWindowSize()), true);
        }
      }
    }
  ...
  protected:
    virtual void UpdateLayout(bool bImmediateMove) {
      ...
      float flXPosPercent =
          (GetScrollWindowPosition() - GetRangeMin()) / GetRangeSize();
  ...
  public:
    CHTMLHorizontalScrollBar(CPanel2D *parent, const char *pchPanelID)
        : CScrollBar(parent, pchPanelID) {
...
protected:
  ...
  virtual void OnPageLoaded(const char *url, const char *pageTitle,
  ...
  // if true then our html control overrides scrolling and scrollbars, if false
  // we let the web control scroll itself
  void SetManualHTMLScroll(bool bControlScroll) {
    m_bControlPageScrolling = bControlScroll;
  }

private:
  typedef void (CHTML::*ScrollFunc_t)(float, bool);
  bool ProcessAnalogScrollAxis(float fValue, float fDeadzoneValue,
                               double fTimeDelta, ScrollFunc_t ScrollFunc);

  // we used to create the virtual mouse in our constructor, but now we don't
  // know enough information at construction time to know whether we want one
  // (ie., if we're wrapped by CHTMLSimpleNavigationWrapper we disable touchpad
  // navigation). Instead we just try to lazy-create one at first use.
...
#if defined(SOURCE2_PANORAMA) || defined(PANORAMA_PUBLIC_STEAM_SDK)
  STEAM_CALLBACK(CHTML, OnLinkAtPositionResponse, HTML_LinkAtPosition_t,
                 m_LinkAtPosRespose);

  STEAM_CALLBACK(CHTML, OnHTMLNeedsPaint, HTML_NeedsPaint_t, m_HTML_NeedsPaint);
  STEAM_CALLBACK(CHTML, OnHTMLStartRequest, HTML_StartRequest_t,
                 m_HTML_StartRequest);
  STEAM_CALLBACK(CHTML, OnHTMLCloseBrowser, HTML_CloseBrowser_t,
                 m_HTML_CloseBrowser);
  STEAM_CALLBACK(CHTML, OnHTMLURLChanged, HTML_URLChanged_t, m_HTML_URLChanged);
  STEAM_CALLBACK(CHTML, OnHTMLFinishedRequest, HTML_FinishedRequest_t,
                 m_HTML_FinishedRequest);
  STEAM_CALLBACK(CHTML, OnHTMLOpenLinkInNewTab, HTML_OpenLinkInNewTab_t,
                 m_HTML_OpenLinkInNewTab);
  STEAM_CALLBACK(CHTML, OnHTMLChangedTitle, HTML_ChangedTitle_t,
                 m_HTML_ChangedTitle);
  STEAM_CALLBACK(CHTML, OnHTMLSearchResults, HTML_SearchResults_t,
                 m_HTML_SearchResults);
  STEAM_CALLBACK(CHTML, OnHTMLCanGoBackAndForward, HTML_CanGoBackAndForward_t,
                 m_HTML_CanGoBackAndForward);
  STEAM_CALLBACK(CHTML, OnHTMLHorizontalScroll, HTML_HorizontalScroll_t,
                 m_HTML_HorizontalScroll);
  STEAM_CALLBACK(CHTML, OnHTMLVerticalScroll, HTML_VerticalScroll_t,
                 m_HTML_VerticalScroll);
  ...
  STEAM_CALLBACK(CHTML, OnHTMLFileOpenDialog, HTML_FileOpenDialog_t,
                 m_HTML_FileOpenDialog);
  ...
  STEAM_CALLBACK(CHTML, OnHTMLShowToolTip, HTML_ShowToolTip_t,
                 m_HTML_ShowToolTip);
  STEAM_CALLBACK(CHTML, OnHTMLUpdateToolTip, HTML_UpdateToolTip_t,
                 m_HTML_UpdateToolTip);
  STEAM_CALLBACK(CHTML, OnHTMLHideToolTip, HTML_HideToolTip_t,
                 m_HTML_HideToolTip);
#else
  // message handlers for ipc thread
  void BrowserSetIndex(int idx) {
    m_iBrowser = idx;
    SendPendingHTMLMessages();
  }
  ...
  void BrowserHorizontalScrollBarSizeResponse(
      const CMsgHorizontalScrollBarSizeResponse *pCmd);
  void BrowserVerticalScrollBarSizeResponse(
      const CMsgVerticalScrollBarSizeResponse *pCmd);
  ...
  void BrowserZoomToElementAtPositionResponse(
      const CMsgZoomToElementAtPositionResponse *pCmd);
  ...
  void
  BrowserScalePageToValueResponse(const CMsgScalePageToValueResponse *pCmd);
  void BrowserRequestFullScreen(const CMsgRequestFullScreen *pCmd);
  void BrowserExitFullScreen(const CMsgExitFullScreen *pCmd);
  void
  BrowserGetCookiesForURLResponse(const CMsgGetCookiesForURLResponse *pCmd);
  ...
  // event handlers
  bool OnGamepadInput();
  bool OnPropertyTransitionEnd(const CPanelPtr<IUIPanel> &pPanel,
                               CStyleSymbol prop);
  bool OnSetBrowserSize(const CPanelPtr<IUIPanel> &pPanel, int nWide,
                        int nTall);
  ...
  bool
  OnInputFocusLost(const panorama::CPanelPtr<panorama::IUIPanel> &ptrPanel);
  bool OnHTMLScreenShotCaptured(
      const panorama::CPanelPtr<panorama::IUIPanel> &ptrPanel,
      int nThumbNailWidth, int nThumbNailHeight);
  bool OnHTMLCommitZoom(const panorama::CPanelPtr<panorama::IUIPanel> &ptrPanel,
                        float flZoom);
  bool
  OnHTMLRequestRepaint(const panorama::CPanelPtr<panorama::IUIPanel> &ptrPanel);

#if defined(SOURCE2_PANORAMA) || defined(PANORAMA_PUBLIC_STEAM_SDK)
  bool OnFileOpenDialogFilesSelected(
      const panorama::CPanelPtr<panorama::IUIPanel> &ptrPanel,
      const char *pszFiles);
#endif
  ...
  int AdjustPageScrollForTextureOffset(int &nTargetValue, const int nCurScroll,
                                       const int nMaxScroll,
                                       float &flOffsetTextureScroll,
                                       const float flMaxTextureScroll);

  int GetHScrollOffset() { return m_ScrollLeft.m_flOffsetTextureScroll; }

  int GetVScrollOffset() { return m_ScrollUp.m_flOffsetTextureScroll; }
  ...
  bool m_bLastKeyFocus; // tracking for when key focus changes in style
                        // application

  struct ScrollControl_t {
    ScrollControl_t() { Reset(); }
    ...
    float m_flOffsetTextureScroll; // amount the html texture is scrolled around
                                   // the panel itself
    bool m_bScrollingUp; // we are scrolling up (or left) on the page last?
    double m_flLastScrollTime; // when did we scroll in this direction last,
                               // used for accel curve
  };
  ...
  struct ScrollData_t {
    ...
    bool operator==(const ScrollData_t &rhs) const {
      return rhs.m_bVisible == m_bVisible && rhs.m_nScroll == m_nScroll &&
             rhs.m_nPageSize == m_nPageSize && rhs.m_nMaxScroll == m_nMaxScroll;
    }

    bool operator!=(const ScrollData_t &rhs) const { return !operator==(rhs); }
    ...
    int m_nWebScroll; // last scrolled return value from cef, not updated
                      // locally
  };
  bool
  ScrollHelper(ScrollControl_t &scrollControl, float flScrollDelta,
               int iMaxScrollOffset, ScrollData_t &scrollBar,
               float &flScrollHTMLAmount,
               bool bApplyBezier); // shared code when scrolling around the page
  bool SetupScrollBar(const ScrollData_t &scrollData, bool bHorizontal,
                      float flContentSize, float flMaxSize);
  ...
  CCubicBezierCurve<Vector2D>
      m_ScrollBezier; // the curve to scale scroll accel by
  ...
  CHTMLVerticalScrollBar *m_pVerticalScrollBar; // our own copy of the scroll
                                                // bars that we control manually
  ...
  bool m_bCommenceZoomOperationOnTextureUpload; // when the next texture upload
                                                // is ready, should we apply
                                                // scale/offset transforms we
                                                // have saved above
  float m_flHorizScrollOffset; // the offset between page scroll and texture
                               // scroll we had at the start of a zoom
  float m_flVertScrollOffset;  // the offset between page scroll and texture
                               // scroll we had at the start of a zoom
  bool m_bFullScreen;          // are we in fullscreen right now?
  bool m_bConfigureYouTubeHTML5OptIn; // are we doing the forcefully opt into
                                      // youtube html5 beta path
  bool m_bMousePanningActive;         // true if the mouse is in panning mode
  Vector2D m_vecMousePanningPos; // the x,y pos of the mouse over the panel when
                                 // middle panning started
  CImagePanel *m_pMousePanningImage; // the image to show when panning
  CCubicBezierCurve<Vector2D>
      m_MousePanBezier;   // the curve to scale panning accel by
  ...
  bool m_bEmbedded; // if true we are embedded instance, just show html pages
                    // and simple scrolling, not complex interactions
  bool m_bAllowOverScroll; // if true allow scrolling the edge of the texture
                           // beyond the edge of the screen (i.e so you can
                           // hover the recticle at any point)
  ...
  panorama::HtmlFormHasFocus_t m_evtFocus; // used for saving state of controls
                                           // that have focus info dispatched
  ...
  bool m_bDidMousePanWhileMouseDown; // did we do panning with the mouse held
                                     // down
