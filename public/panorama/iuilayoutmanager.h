...
class IUILayoutFile {
public:
  virtual bool BReplaceDefines(CPanoramaSymbol symStyleFile, char *rgchBuffer,
                               uint cubBuffer) = 0;
  virtual void
  BuildMatchingStyleList(CUtlVector<CascadeStyleFileInfo_t> &vecStyles,
                         const CPanelIdentifiers &panelID,
                         IUILayoutFile *pPreviousLayoutFile) = 0;
  ...
  virtual bool ApplyMatchedStylesToPanelStyle(
      ...
      const CUtlVector<CascadeStyleFileInfo_t> &vecStyles,
      EStyleRepaint &eRepaint, bool &bInheritablePropertiesChanged) = 0;
};

//-----------------------------------------------------------------------------
// Purpose: Public interface to layout mamanger.  Manages layout files, styles,
// etc.
//-----------------------------------------------------------------------------
...
public:
  ...
  virtual IUILayoutFile *GetLayoutFile(const char *pchFile,
                                       bool bPartialLayout) = 0;
  ...
  virtual bool LoadStyleIntoBuffer(CPanoramaSymbol symFile,
                                   CUtlBuffer &buffer) = 0;
  virtual bool UpdateStyleInMemory(EUpdateStyleType eUpdateType,
                                   CPanoramaSymbol symStyleFile,
                                   uint unLocation,
                                   const char *pchUpdatedStyle) = 0;
  virtual bool BConvertHTTPPathToLocalP4Path(const char *pchFile,
                                             CUtlString &strOut) = 0;
};
