...
class ISceneLayer;
typedef void(RenderCallbackFunction_t)(ISceneView *, IRenderContext **,
                                       ISceneLayer *, float, float, float,
                                       float);
...
template <class T, class P, class P2, class P3, class P4, class P5, class P6,
          class P7>
inline void ConstructSevenArg(T *pMemory, P const &arg1, P2 const &arg2,
                              P3 const &arg3, P4 const &arg4, P5 const &arg5,
                              P6 const &arg6, P7 const &arg7) {
  HINT(pMemory != 0);
...
enum EAnimationTimingFunction {
  k_EAnimationNone = 0, // Indicates that there is no transition set, do not
                        // animate.  Don't change from zero!
  k_EAnimationEase,
  k_EAnimationEaseIn,
  k_EAnimationEaseOut,
...
enum EStyleFlags : uint16 {
  ...
  k_EStyleFlagActive =
      1 << 2, // panel is being actively used (mouse-down on the panel)
  ...
  k_EStyleFlagDescendantFocused =
      1 << 6, // a descendant of the panel has keyboard focus
  k_EStyleFlagParentDisabled =
      1
      << 7, // a parent of this panel is disabled, thus implicitly disabling it
  k_EStyleFlagLayoutLoading =
      1 << 8, // panel is in-progress loading it's layout file and thus may want
              // to display specially
  k_EStyleFlagLayoutLoadFailed =
      1 << 9, // panel failed to load requested layout file, and thus may want
              // to display specially
  k_EStyleFlagActivationDisabled =
      1 << 10, // panel is disabled for activation, may still be enabled for
               // focus (normal disabled disallows all input/focus)
};
...
//-----------------------------------------------------------------------------
// Purpose: Specifies what category a sound is; each one of these maps to a
...
enum ESoundType {
  ...
  k_ESoundType_Passthrough, // special sound type for volume-changing UI -
                            // bypasses volume controls
};
...
enum ETextInputHandlerType_t {
  ...
  k_ETextInputHandlerTypeDefault =
      k_ETextInputHandlerType_DualTouch, // if we fail at gathering the
                                         // information to figure out which is
                                         // the most appropriate input method,
...
enum EAntialiasing { k_EAntialiasingNone, k_EAntialisingEnabled };
...
// These constants are duplicated in CSS files, so if you change them make sure
// to change the CSS values as well.
...
//-----------------------------------------------------------------------------
enum EPanelFlags {
  ePanelFlags_DontAddAsChild =
      0x1, // don't automatically add this panel as a child to its parent
  ePanelFlags_DontFireOnLoad =
      0x2, // don't fire the onload event when constructed, useful for image
           // panel that waits until its contents load
};
...
class IUIPanel;
class IUIEvent;
typedef IUIEvent *(*PFN_ParseUIEvent)(panorama::IUIPanel *pTarget,
                                      const char *pchEvent,
                                      const char **pchEventEnd);
typedef IUIEvent *(*PFN_MakeUIEvent0)(const panorama::IUIPanelClient *pTarget);
typedef IUIEvent *(*PFN_MakeUIEvent1Source)(
    const panorama::IUIPanelClient *pTarget, EPanelEventSource_t eSource);
typedef IUIEvent *(*PFN_MakeUIEvent1Repeats)(
    const panorama::IUIPanelClient *pTarget, int nRepeats);
...
public:
  CLayoutRefCounted() { m_nRefCount = 0; }
...
//-----------------------------------------------------------------------------
template <class T> class CLayoutRefPtr {
public:
  ...
  bool operator!=(T *p) const { return (m_pObject != p); }
  bool operator==(const CLayoutRefPtr<T> &p) const {
...
enum EWindowFocusBehavior {
  // By default, controls in this window will take focus whenever they're
  // interacted with.
  k_EWindowFocusBehavior_Default,

  // By default, clicking on controls in this window will avoid taking focus
  // unless it's necessary. Useful if you only plan on supporting keyboard focus
  // in a few specific spots and otherwise don't display keyboard focus.
