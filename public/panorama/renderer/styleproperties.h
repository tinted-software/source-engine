...
// Helper for getting actual size of parent, which will expand up to window size
// if top level
void GetParentSizeAvailable(IUIPanel *pPanel, float &flParentWidth,
                            float &flParentHeight, float &flParentPerspective);
void GetAnimationCurveControlPoints(EAnimationTimingFunction eTransitionEffect,
...
struct AnimationProperty_t {
  ...
  bool operator==(const AnimationProperty_t &rhs) const {
    return (
        m_symName == rhs.m_symName && m_flDuration == rhs.m_flDuration &&
        m_eTimingFunction == rhs.m_eTimingFunction &&
        (m_eTimingFunction != k_EAnimationCustomBezier ||
         (m_CubicBezier.ControlPoint(0) == rhs.m_CubicBezier.ControlPoint(0) &&
          ...
          m_CubicBezier.ControlPoint(3) ==
              rhs.m_CubicBezier.ControlPoint(3))) &&
        m_flIteration == rhs.m_flIteration &&
        m_eAnimationDirection == rhs.m_eAnimationDirection &&
        m_flDelay == rhs.m_flDelay);
  }
...
public:
  CActiveAnimation(float flAnimationStart,
                   AnimationProperty_t &animationProperty);
  ~CActiveAnimation();

  CPanoramaSymbol GetName() const { return m_animationData.m_symName; }
  const AnimationProperty_t &GetAnimationData() const {
    return m_animationData;
  }
  void AddFrameData(float flPercent, EAnimationTimingFunction eTimingFunction,
                    const CCubicBezierCurve<Vector2D> &cubicBezier,
                    CStyleProperty *pProperty, float flUIScaleFactor);

  bool BHasFrameDataForProperty(CStyleSymbol symStyleProperty);

  double GetStartTime() const { return m_flAnimationStartTime; }
  double GetStartTimeWithDelay() const {
    return m_flAnimationStartTime + m_animationData.m_flDelay;
  }
  float CalculateAnimationEndTime() const;
  void Reset();
  void GetAffectedPanelLayoutFlags(CPanelStyle *pPanelStyle,
                                   bool *pbAffectsSize,
                                   bool *pbAffectsPosition);
  ...
  struct PropertyFrameData_t {
    ...
    EAnimationTimingFunction
        m_eTimingFunction; // timing function can be overridden per keyframe
    CCubicBezierCurve<Vector2D> m_CubicBezier;
...
private:
  ...
  CUtlMap<CStyleSymbol, VecPropertyFrameData_t *, short, CDefLess<CStyleSymbol>>
      m_mapFrameData;
};

//-----------------------------------------------------------------------------
// Purpose: Represents all components that can be included in an individual CSS
// selector
//-----------------------------------------------------------------------------
...
public:
  ...
  void SetClasses(CPanoramaSymbol *pSymbols, int cSymbols) {
    m_classes.Copy(pSymbols, cSymbols);
  }
  ...
  void SetIDForSearch(const char *pchID) { m_pchID = pchID; }

  // allows setting classes w/o allocationg for searching. Skips memory
  // management
...
private:
  enum ESelectorFlags {
    ...
    k_ESelectorFlagsChildCombinator =
        1, // next selector must match child of panel that matched this style
           // (.A > .B)
  };

  CStyleSelector(CStyleSelector const &) { Assert(0); }

  // id is a char pointer instead of CUtlString so we can do searches w/o
  // allocations
  const char *m_pchID;            // not empty if an ID is required
  CPanoramaSymbol m_symPanelType; // if invalid, applies to all panels
  CUtlPtrArray<CPanoramaSymbol>
      m_classes;             // can be empty if id or panel type are set
  EStyleFlags m_eStyleFlags; // required style flags
  uint32 m_unSelectorFlags;  // ESelectorFlags
};

//-----------------------------------------------------------------------------
// Purpose: Base class for every style property, they all have a symbol for a
// name.
//-----------------------------------------------------------------------------
...
public:
  ...
  // Merges data to the target, this will only overwrite unset properties, and
  // shouldn't clobber already set ones.
  ...
  // Interpolation func for animation of this property, must implement if you
  // override BCanTransition to return true.
  virtual void Interpolate(IUIPanel *pPanel, const CStyleProperty &target,
                           float flProgress /* 0.0->1.0 */) = 0;

  // Parses string and sets value
  virtual bool BSetFromString(CStyleSymbol symParsedName,
                              const char *pchString) = 0;

  // Gets string representation of property
  virtual void ToString(CFmtStr1024 *pfmtBuffer) const = 0;

  // When applying styles to an element, used to determine if all data for this
  // property has been set or if more fields should be found by looking at lower
  // weight styles Example: For margin, if we have only seen margin-length,
  // should return false until top, bottom, right are set
  virtual bool BFullySet() { return true; }

  // called when applied to a panel before comparing with set values. Gives the
  // style an opportunity to change any unset values to defaults
  ...
  // Does this property only affecting compositing when rendering, or does it
  // affect drawing within the composition layer for the panel it is applied to?
  virtual bool BAffectsCompositionOnly() { return false; }

  // Get suggested values based on current text for style property value
  virtual void GetSuggestedValues(CStyleSymbol symAlias,
                                  const char *pchTextSoFar,
  ...
  // What layout attributes can be invalidated when the property is applied to a
  // panel (compared to prior value)
  virtual EStyleInvalidateLayout
  GetInvalidateLayout(CStyleProperty *pCompareProperty) const = 0;

  // Layout pieces that can be invalidated when the property is applied to a
  // panel
  inline bool
  BInvalidatesSizeAndPosition(CStyleProperty *pCompareProperty) const {
    return (GetInvalidateLayout(pCompareProperty) ==
            k_EStyleInvalidateLayoutSizeAndPosition);
  }
  inline bool BInvalidatesPosition(CStyleProperty *pCompareProperty) const {
    return (GetInvalidateLayout(pCompareProperty) ==
            k_EStyleInvalidateLayoutPosition);
  }

  // If the style supports transition (BCanTransition), it may still be that
  // this style instance disables transition and forces immediate apply, this
  // controls that
  void SetDisallowTransition(bool bDisallowTransition) {
    m_bDisallowTransition = bDisallowTransition;
  }
...
typedef CUtlHashMap<panorama::CStyleSymbol, panorama::CStyleProperty *,
                    CDefEquals<panorama::CStyleSymbol>>
    StylePropertyHash_t;
...
struct StyleFromFile_t {
  ...
  ~StyleFromFile_t() {
    if (m_pProperties) {
      FOR_EACH_RBTREE_FAST(*m_pProperties, i) {
        UIEngine()->UIStyleFactory()->FreeStyleProperty(
            m_pProperties->Element(i));
      }
  ...
  void Validate(CValidator &validator, const tchar *pchName) {
    ...
    // Don't validate individual properties, they are pooled and we validate the
    // pools elsewhere.  Do have them validate their own members.
  ...
  CUtlPtrArray<CStyleSelector>
      m_parentSelectors; // selectors for parents, ordered farthest to our
                         // closest parent

  StylePropertyHash_t *m_pProperties; // list of parsed properties
  uint m_unFileLocation; // style offset into the file (Nth style, higher value
                         // means defined later and takes priority when
                         // determining
  // cascading order (CSS2 TR 6.4.1)

  StyleFromFile_t *m_pNext; // allows chaining of objects with the same same
                            // symbol in CStyleFile
};
...
public:
  CStyleKeyFrame(float flPercent, EAnimationTimingFunction eTimingFunc,
                 const CCubicBezierCurve<Vector2D> &cubicBezier,
                 StylePropertyHash_t *pProperties) {
  ...
  EAnimationTimingFunction GetTimingFunction() const {
    return m_eTimingFunction;
  }
  const CCubicBezierCurve<Vector2D> &GetCubicBezier() const {
    return m_CubicBezier;
  }
  StylePropertyHash_t *GetProperties() const { return m_pProperties; }

  // ordered by percent
  bool operator<(const CStyleKeyFrame &rhs) const {
    return m_flPercent < rhs.m_flPercent;
  }
  ...
  void Validate(CValidator &validator, const tchar *pchName) {
    ...
    // Don't validate each individual property, they are pooled and the pool is
    // validated elsewhere
    FOR_EACH_MAP_FAST(*m_pProperties, i) {
...
private:
  ...
  float m_flPercent;
  EAnimationTimingFunction
      m_eTimingFunction; // each keyframe can override the timing funciton
  CCubicBezierCurve<Vector2D> m_CubicBezier;
  StylePropertyHash_t *m_pProperties;
};

typedef CStyleKeyFrame *CStyleKeyFramePtr;
bool StyleKeyFrameLessPtr(const CStyleKeyFramePtr &lhs,
                          const CStyleKeyFramePtr &rhs, void *pCtx);
#if defined(SOURCE2_PANORAMA)
class CUtlSortVectorCStyleKeyFramePtrLess {
public:
  bool Less(const CStyleKeyFramePtr &lhs, const CStyleKeyFramePtr &rhs,
            void *) {
    return *lhs < *rhs;
  }
};

typedef CUtlSortVector<CStyleKeyFrame *, CUtlSortVectorCStyleKeyFramePtrLess>
    VecKeyFrames_t;
#else
...
public:
  CStyleAnimation(CPanoramaSymbol symName, CPanoramaSymbol symStyleFile,
                  uint unFileLocation)
#if !defined(SOURCE2_PANORAMA)
  ...
  ~CStyleAnimation() { ClearFrames(); }

  void ClearFrames() { m_vecKeyFrames.PurgeAndDeleteElements(); }
  ...
  void Validate(CValidator &validator, const tchar *pchName) {
    ...
    FOR_EACH_VEC(m_vecKeyFrames, i) { ValidatePtr(m_vecKeyFrames[i]); }
  }
#endif
...
private:
  ...
  CPanoramaSymbol
      m_symStyleFile;    // path to style file this animation was created from
...
public:
  ...
  virtual void Interpolate(IUIPanel *pPanel, const CStyleProperty &target,
                           float flProgress /* 0.0->1.0 */);

  // Does the style only affect compositing of it's panels and not drawing
  // within a composition layer?
  ...
  // property has been set or if more fields should be found by looking at lower
  // weight styles
  virtual bool BFullySet() { return (x.IsSet() && y.IsSet() && z.IsSet()); }

  // called when applied to a panel. Gives the style an opportunity to change
  // any unset values to defaults
  virtual void ResolveDefaultValues() {
    if (!x.IsSet())
      x.SetLength(0.0f);
  ...
  // Return a description for this property which will be shown in the debugger
  virtual const char *GetDescription(CStyleSymbol symProperty) {
    return "Sets the x, y, z position for a panel. Must not be in a flowing "
           "layout.<br><br>"
           "<b>Example:</b>"
           "<pre>"
           "position: 3% 20px 0px;"
  ...
  // panel
  virtual EStyleInvalidateLayout
  GetInvalidateLayout(CStyleProperty *pCompareProperty) const {
...
public:
  ...
  CStylePropertyTransformOrigin()
      : CStyleProperty(CStylePropertyTransformOrigin::symbol) {
  ...
  virtual void MergeTo(CStyleProperty *pTarget) const {
    if (pTarget->GetPropertySymbol() != GetPropertySymbol()) {
      AssertMsg(false,
                "Mismatched types to CStylePropertyTransformOrigin::MergeTo");
      return;
    }
  ...
  virtual bool BSetFromString(CStyleSymbol symParsedName,
                              const char *pchString) {
    bool bSuccess =
        (CSSHelpers::BParseIntoUILength(&x, pchString, &pchString) &&
         CSSHelpers::BParseIntoUILength(&y, pchString, &pchString));
    pchString = CSSHelpers::SkipSpaces(pchString);
    if (V_strnicmp(pchString, "parent-relative", V_strlen("parent-relative")) ==
        0)
  ...
  // Return a description for this property which will be shown in the debugger
  virtual const char *GetDescription(CStyleSymbol symProperty) {
    return "Sets the transform origin about which transforms will be applied.  "
           "Default is 50% 50% on the panel so a rotation/scale is "
           "centered.<br><br>"
           "<b>Example:</b>"
           "<pre>"
  ...
  virtual bool operator==(const CStyleProperty &other) const {
    ...
    const CStylePropertyTransformOrigin &rhs =
        (const CStylePropertyTransformOrigin &)other;
    return (x == rhs.x && y == rhs.y &&
            m_bParentRelative == rhs.m_bParentRelative);
  }
...
public:
  ...
  CStylePropertyPerspectiveOrigin()
      : CStyleProperty(CStylePropertyPerspectiveOrigin::symbol) {
  ...
  virtual void MergeTo(CStyleProperty *pTarget) const {
    if (pTarget->GetPropertySymbol() != GetPropertySymbol()) {
      AssertMsg(false,
                "Mismatched types to CStylePropertyPerspectiveOrigin::MergeTo");
      return;
    }

    CStyleProperty::MergeTo(pTarget);

    CStylePropertyPerspectiveOrigin *p =
        (CStylePropertyPerspectiveOrigin *)pTarget;
    p->x = x;
    p->y = y;
  ...
  // Return a description for this property which will be shown in the debugger
  virtual const char *GetDescription(CStyleSymbol symProperty) {
    return "Sets the perspective origin which will be used when transforming "
           "children of this panel.  This can be "
           "thought of as the camera x/y position relative to the "
           "panel.<br><br>"
           "<b>Example:</b>"
           "<pre>"
  ...
  virtual bool operator==(const CStyleProperty &other) const {
    ...
    const CStylePropertyPerspectiveOrigin &rhs =
        (const CStylePropertyPerspectiveOrigin &)other;
...
public:
  ...
  CStylePropertyPerspective()
      : CStyleProperty(CStylePropertyPerspective::symbol) {
  ...
  virtual void MergeTo(CStyleProperty *pTarget) const {
    if (pTarget->GetPropertySymbol() != GetPropertySymbol()) {
      AssertMsg(false,
                "Mismatched types to CStylePropertyPerspective::MergeTo");
      return;
    }
  ...
  virtual void Interpolate(IUIPanel *pPanel, const CStyleProperty &target,
    ...
    const CStylePropertyPerspective *p =
        (const CStylePropertyPerspective *)&target;
  ...
  // Return a description for this property which will be shown in the debugger
  virtual const char *GetDescription(CStyleSymbol symProperty) {
    return "Sets the perspective depth space available for children of the "
           "panel.  Default of 1000 would mean that "
           "children at 1000px zpos are right at the viewers eye, -1000px are "
           "just out of view distance faded to nothing.<br><br>"
           "<b>Example:</b>"
           "<pre>"
           "perspective: 1000;"
           "</pre>";
  ...
  virtual bool operator==(const CStyleProperty &other) const {
    ...
    const CStylePropertyPerspective &rhs =
        (const CStylePropertyPerspective &)other;
...
//-----------------------------------------------------------------------------
// Purpose: z-index, this determines z sorting within panels that are peers with
// indentical actual zpos
//-----------------------------------------------------------------------------
...
public:
  ...
  // Return a description for this property which will be shown in the debugger
  virtual const char *GetDescription(CStyleSymbol symProperty) {
    return "Sets the z-index for a panel, panels will be sorted and painted in "
           "order within a parent panel.  The sorting "
           "first sorts by the z-pos computed from position and transforms, "
           "then if panels have matching zpos zindex is used. "
           "z-index is different than z-pos in that it doesn't affect "
           "rendering perspective, just paint/hit-test ordering. "
           "The default z-index value is 0, and any floating point value is "
           "accepted.<br><br>"
           "<b>Example:</b>"
           "<pre>"
           "z-index: 1;"
...
public:
  ...
  // Return a description for this property which will be shown in the debugger
  virtual const char *GetDescription(CStyleSymbol symProperty) {
    return "Sets the opacity or amount of transparency applied to the panel "
           "and all it's children during composition. "
           "Default of 1.0 means fully opaque, 0.0 means fully "
           "transparent.<br><br>"
           "<b>Example:</b>"
           "<pre>"
           "opacity: 0.8;"
...
public:
  ...
  CStylePropertyScale2DCentered()
      : CStyleProperty(CStylePropertyScale2DCentered::symbol) {
    m_flX = GetDefaultX();
    m_flY = GetDefaultY();
  }
  ...
  virtual void MergeTo(CStyleProperty *pTarget) const {
    if (pTarget->GetPropertySymbol() != GetPropertySymbol()) {
      AssertMsg(false,
                "Mismatched types to CStylePropertyScale2DCentered::MergeTo");
      return;
    }
  ...
  virtual void Interpolate(IUIPanel *pPanel, const CStyleProperty &target,
    ...
    const CStylePropertyScale2DCentered *p =
        (const CStylePropertyScale2DCentered *)&target;
  ...
  virtual bool BSetFromString(CStyleSymbol symParsedName,
    ...
    if (!CSSHelpers::BParseCommaSepList(&vecValues, CSSHelpers::BParseNumber,
                                        pchString))
      return false;
  ...
  // Return a description for this property which will be shown in the debugger
  virtual const char *GetDescription(CStyleSymbol symProperty) {
    return "Sets 2 dimensional X/Y scale factors that apply to the quad for "
           "this panel prior to 3 dimensional transforms. "
           "This scaling applies without perspective and leaves the panel "
           "centered at the same spot as it started. "
  ...
  virtual bool operator==(const CStyleProperty &other) const {
    ...
    const CStylePropertyScale2DCentered &rhs =
        (const CStylePropertyScale2DCentered &)other;
...
public:
  ...
  CStylePropertyRotate2DCentered()
      : CStyleProperty(CStylePropertyRotate2DCentered::symbol) {
  ...
  virtual void MergeTo(CStyleProperty *pTarget) const {
    if (pTarget->GetPropertySymbol() != GetPropertySymbol()) {
      AssertMsg(false,
                "Mismatched types to CStylePropertyRotate2DCentered::MergeTo");
      return;
    }

    CStyleProperty::MergeTo(pTarget);

    CStylePropertyRotate2DCentered *p =
        (CStylePropertyRotate2DCentered *)pTarget;
  ...
  virtual void Interpolate(IUIPanel *pPanel, const CStyleProperty &target,
    ...
    const CStylePropertyRotate2DCentered *p =
        (const CStylePropertyRotate2DCentered *)&target;
  ...
  virtual bool BSetFromString(CStyleSymbol symParsedName,
    ...
    // should be "degrees"
    CUtlVector<float> vecValues;
    if (!CSSHelpers::BParseCommaSepList(&vecValues, CSSHelpers::BParseAngle,
                                        pchString))
      return false;
  ...
  // Return a description for this property which will be shown in the debugger
  virtual const char *GetDescription(CStyleSymbol symProperty) {
    return "Sets 2 dimensional rotation degrees that apply to the quad for "
           "this panel prior to 3 dimensional transforms. "
           "This rotation applies without perspective and leaves the panel "
           "centered at the same spot as it started.<br><br>"
           "<b>Example:</b>"
           "<pre>"
  ...
  virtual bool operator==(const CStyleProperty &other) const {
    ...
    const CStylePropertyRotate2DCentered &rhs =
        (const CStylePropertyRotate2DCentered &)other;
...
public:
  ...
  CStylePropertyDesaturation()
      : CStyleProperty(CStylePropertyDesaturation::symbol) {
  ...
  virtual void MergeTo(CStyleProperty *pTarget) const {
    if (pTarget->GetPropertySymbol() != GetPropertySymbol()) {
      AssertMsg(false,
                "Mismatched types to CStylePropertyDesaturation::MergeTo");
      return;
    }
  ...
  virtual void Interpolate(IUIPanel *pPanel, const CStyleProperty &target,
    ...
    if (target.GetPropertySymbol() != GetPropertySymbol()) {
      AssertMsg(false,
                "Mismatched types to CStylePropertyDesaturation::Interpolate");
      return;
    }

    const CStylePropertyDesaturation *p =
        (const CStylePropertyDesaturation *)&target;
  ...
  // Return a description for this property which will be shown in the debugger
  virtual const char *GetDescription(CStyleSymbol symProperty) {
    return "Sets the amount of desaturation to apply to the panel and all it's "
           "children during composition.  "
           "Default of 0.0 means no adjustment, 1.0 means fully desaturated to "
           "gray scale.<br><br>"
           "<b>Example:</b>"
           "<pre>"
           "desaturation: 0.6;"
  ...
  virtual bool operator==(const CStyleProperty &other) const {
    ...
    const CStylePropertyDesaturation &rhs =
        (const CStylePropertyDesaturation &)other;
...
public:
  ...
  virtual bool BSetFromString(CStyleSymbol symParsedName,
                              const char *pchString) {
    if (!CSSHelpers::BParseGaussianBlur(passes, stddevhor, stddevver, pchString,
                                        &pchString))
      return false;
  ...
  // Return a description for this property which will be shown in the debugger
  virtual const char *GetDescription(CStyleSymbol symProperty) {
    return "Sets the amount of blur to apply to the panel and all it's "
           "children during composition.  "
           "Default is no blur, for now Gaussian is the only blur type and "
           "takes a horizontal standard deviation, "
           "vertical standard deviation, and number of passes.  Good std "
           "deviation values are around 0-10, if 10 is "
           "still not intense enough consider more passes, but more than one "
           "pass is bad for perf.  As shorthand you can "
           "specify with just one value, which will be used for the standard "
           "deviation in both directions and 1 pass "
           "will be set.<br><br>"
           "<b>Examples:</b>"
           "<pre>"
           "blur: gaussian( 2.5 );\n"
  ...
  virtual bool operator==(const CStyleProperty &other) const {
    ...
    return (stddevhor == rhs.stddevhor && stddevver == rhs.stddevver &&
            passes == rhs.passes);
  }
...
public:
  ...
  virtual bool BSetFromString(CStyleSymbol symParsedName,
    ...
    while (1) {
      ...
      if (!bInsetParsed &&
          V_strnicmp(pchString, "inset", V_strlen("inset")) == 0) {
      ...
      if (!bFillParsed &&
          V_strnicmp(pchString, "fill", V_strlen("fill")) == 0) {
        pchString += V_strlen("fill");
        bFillParsed = true;
        m_bFill = true;
        continue;
      } else if (!bColorParsed && CSSHelpers::BParseColor(
                                      &m_ShadowColor, pchString, &pchString)) {
        bColorParsed = true;
        continue;
      } else if (!bLengthsParsed) {
        bLengthsParsed = true;
        if (!CSSHelpers::BParseIntoUILength(&m_HorizontalOffset, pchString,
                                            &pchString)) {
          return false;
        }

        if (!CSSHelpers::BParseIntoUILength(&m_VerticalOffset, pchString,
                                            &pchString)) {
          return false;
        }
        ...
        CSSHelpers::BParseIntoUILength(&m_SpreadDistance, pchString,
                                       &pchString);
      } else {
        // Lengths are the only thing required, if we get here and have them, we
        // are good.
        if (bLengthsParsed) {
          return true;
        }
  ...
  virtual void ApplyUIScaleFactor(float flScaleFactor) {
    ...
    // Although this is a blur pixel radius in css terminology it's really a
    // stddev for our gaussian blur internally, hence it should not scale.
  ...
  // Return a description for this property which will be shown in the debugger
  virtual const char *GetDescription(CStyleSymbol symProperty) {
    return "Specifies outer shadows for boxes, or inset shadows/glows.  The "
           "shadow shape will match the border box for the panel,"
           "so use border-radius to affect rounding.  Syntax takes optional "
           "'inset', optional 'fill' then color, and then horizontal offset "
           "pixels, "
           "vertical offset pixels, blur radius pixels, and spread distance in "
           "pixels. Inset means the shadow is an inner shadow/glow, fill is "
           "valid"
           "only on outer shadows and means draw the shadow behind the entire "
           "box, not clipping it to outside the border area only.<br><br>"
           "<b>Examples:</b>"
           "<pre>"
  ...
  virtual bool operator==(const CStyleProperty &other) const {
    ...
    return (m_bFill == rhs.m_bFill && m_bInset == rhs.m_bInset &&
            m_HorizontalOffset == rhs.m_HorizontalOffset &&
            m_VerticalOffset == rhs.m_VerticalOffset &&
            m_BlurRadius == rhs.m_BlurRadius &&
            m_SpreadDistance == rhs.m_SpreadDistance &&
...
public:
  ...
  CStylePropertyTextShadow()
      : CStyleProperty(CStylePropertyTextShadow::symbol) {
    m_ShadowColor = GetDefaultShadowColor();
    m_HorizontalOffset.SetLength(0);
    m_VerticalOffset.SetLength(0);
    m_BlurRadius.SetLength(0);
  ...
  virtual bool BSetFromString(CStyleSymbol symParsedName,
    ...
    while (1) {
      ...
      if (!bColorParsed &&
          CSSHelpers::BParseColor(&m_ShadowColor, pchString, &pchString)) {
        bColorParsed = true;
        continue;
      } else if (!bLengthsParsed) {
        bLengthsParsed = true;
  ...
  // Return a description for this property which will be shown in the debugger
  virtual const char *GetDescription(CStyleSymbol symProperty) {
    return "Specifies text shadows.  The shadow shape will match the text the "
           "panel can generate,"
           "and this is only meaningful for labels.  Syntax takes horizontal "
           "offset pixels, "
           "vertical offset pixels, blur radius pixels, strength, and then "
           "shadow color.<br><br>"
           "<b>Example:</b>"
           "<pre>"
  ...
  virtual bool operator==(const CStyleProperty &other) const {
    ...
    const CStylePropertyTextShadow &rhs =
        (const CStylePropertyTextShadow &)other;
...
public:
  ...
  virtual bool BSetFromString(CStyleSymbol symParsedName,
    ...
    return CSSHelpers::BParseRect(&m_Top, &m_Right, &m_Bottom, &m_Left,
                                  pchString);
  }
  ...
  // Return a description for this property which will be shown in the debugger
  virtual const char *GetDescription(CStyleSymbol symProperty) {
    return "Specifies a clip region within the panel, where contents will be "
           "clipped at render time. "
           "This clipping has no impact on layout, and is fast and supported "
           "for transitions/animations.<br><br>"
           "<b>Example:</b>"
           "<pre>"
...
// the top of the panel after all child rendering is complete, alpha value
// affects how much of the child drawing colors come through.
//-----------------------------------------------------------------------------
...
public:
  ...
  // Return a description for this property which will be shown in the debugger
  virtual const char *GetDescription(CStyleSymbol symProperty) {
    return "Specifies a 'wash' color, which means a color that will be blended "
           "over the panel and all it's children at "
           "composition time, tinting them.  The alpha value of the color "
           "determines the intensity of the tinting.<br><br>"
           "<b>Example:</b>"
           "<pre>"
           "wash-color: #39b0d325;"
           "</pre>";
...
public:
  ...
  bool BParseOverflow(EOverflowValue *peOverflow, const char *pchString,
                      const char **pchAfterParse = NULL) {
    char rgchProperty[128];
    if (!CSSHelpers::BParseIdent(rgchProperty, V_ARRAYSIZE(rgchProperty),
                                 pchString, &pchString))
      return false;
  ...
  void BAppendOverflow(CFmtStr1024 *pfmtBuffer,
                       const EOverflowValue &eOverflow) const {
  ...
  // Return a description for this property which will be shown in the debugger
  virtual const char *GetDescription(CStyleSymbol symProperty) {
    return "Specifies what to do with contents that overflow the available "
           "space for the panel. "
           "Possible values:<br>"
           "\"squish\" - Children are squished to fit within the panel's "
           "bounds if needed (default)<br>"
           "\"clip\" - Children maintain their desired size but their contents "
           "are clipped<br>"
           "\"scroll\" - Children maintain their desired size and a scrollbar "
           "is added to this panel<br><br>"
           "<b>Examples:</b>"
           "<pre>"
           "overflow: squish squish; // squishes contents in horizontal and "
           "vertical directions\n"
  ...
  virtual bool operator==(const CStyleProperty &other) const {
    ...
    return (m_eHorizontal == rhs.m_eHorizontal &&
            m_eVertical == rhs.m_eVertical);
  }
...
public:
  ...
  virtual void Interpolate(IUIPanel *pPanel, const CStyleProperty &target,
                           float flProgress /* 0.0->1.0 */) {
    Assert(!"You can't interpolate a font");
  }
  ...
  virtual void GetSuggestedValues(CStyleSymbol symAlias,
    ...
    if (symAlias == fontFamily) {
      ...
      const CUtlSortVector<CUtlString> &vecValues =
          UIEngine()->GetSortedValidFontNames();
  ...
  // Gets string representation of property
  virtual void ToString(CFmtStr1024 *pfmtBuffer) const {
    pfmtBuffer->AppendFormat("%s %s ", PchNameFromEFontStyle(m_eFontStyle),
                             PchNameFromEFontWeight(m_eFontWeight));
  ...
  virtual bool BFullySet() {
    return (m_flFontSize != k_flFloatNotSet && !m_strFontFamily.IsEmpty() &&
            m_eFontStyle != k_EFontStyleUnset &&
            m_eFontWeight != k_EFontWeightUnset);
  }
  ...
  virtual const char *GetDescription(CStyleSymbol symProperty) {
    if (symProperty == fontFamily) {
      ...
      return "Specifies the font weight to use. Supported values are light, "
             "thin, normal, medium, bold, and black.<br><br>"
             "<b>Examples:</b>"
             "<pre>"
             "font-weight: normal;\n"
             "font-weight: bold;\n"
      ...
      return "Specifies the font style to use. Supported values are normal, "
             "and italic<br><br>"
             "<b>Example:</b>"
             "<pre>"
             "font-style: normal;"
  ...
  virtual bool operator==(const CStyleProperty &other) const {
    ...
    return (m_strFontFamily == rhs.m_strFontFamily &&
            m_flFontSize == rhs.m_flFontSize &&
            m_eFontStyle == rhs.m_eFontStyle &&
            m_eFontWeight == rhs.m_eFontWeight);
  }
...
public:
  ...
  virtual void Interpolate(IUIPanel *pPanel, const CStyleProperty &target,
                           float flProgress /* 0.0->1.0 */) {
    Assert(!"You can't interpolate a text-align");
  }
  ...
  // property has been set or if more fields should be found by looking at lower
  // weight styles
  virtual bool BFullySet() { return (m_eAlign != k_ETextAlignUnset); }

  // called when applied to a panel. Gives the style an opportunity to change
  // any unset values to defaults
  virtual void ResolveDefaultValues() {
    // bugbug jmccaskey - someday maybe default to right aligned on rtl
    // languages?
  ...
  // Return a description for this property which will be shown in the debugger
  virtual const char *GetDescription(CStyleSymbol symProperty) {
    return "Specifies the text alignment for text within this panel, defaults "
           "to left.<br><br>"
           "<b>Examples:</b>"
           "<pre>"
           "text-align: left;\n"
...
public:
  static const CStyleSymbol symbol;
  CStylePropertyTextLetterSpacing()
      : CStyleProperty(CStylePropertyTextLetterSpacing::symbol) {
    m_nLetterSpacing = 0;
  }
  ...
  virtual void MergeTo(CStyleProperty *pTarget) const {
    if (pTarget->GetPropertySymbol() != GetPropertySymbol()) {
      AssertMsg(false,
                "Mismatched types to CStylePropertyTextLetterSpacing::MergeTo");
      return;
    }

    CStylePropertyTextLetterSpacing *p =
        (CStylePropertyTextLetterSpacing *)pTarget;
  ...
  virtual void Interpolate(IUIPanel *pPanel, const CStyleProperty &target,
                           float flProgress /* 0.0->1.0 */) {
    Assert(!"You can't interpolate a letter-spacing");
  }
  ...
  virtual bool operator==(const CStyleProperty &other) const {
    ...
    const CStylePropertyTextLetterSpacing &rhs =
        (const CStylePropertyTextLetterSpacing &)other;
...
public:
  ...
  CStylePropertyTextDecoration()
      : CStyleProperty(CStylePropertyTextDecoration::symbol) {
  ...
  virtual void MergeTo(CStyleProperty *pTarget) const {
    if (pTarget->GetPropertySymbol() != GetPropertySymbol()) {
      AssertMsg(false,
                "Mismatched types to CStylePropertyTextDecoration::MergeTo");
      return;
    }
  ...
  virtual void Interpolate(IUIPanel *pPanel, const CStyleProperty &target,
                           float flProgress /* 0.0->1.0 */) {
    Assert(!"You can't interpolate a text-decorations");
  }
  ...
  // property has been set or if more fields should be found by looking at lower
  // weight styles
  virtual bool BFullySet() { return (m_eDecoration != k_ETextDecorationUnset); }
  ...
  // Return a description for this property which will be shown in the debugger
  virtual const char *GetDescription(CStyleSymbol symProperty) {
    return "Specifies the decoration for text within this panel, defaults to "
           "none. Possible values: none, underline, line-through.<br><br>"
           "<b>Example:</b>"
           "<pre>"
  ...
  virtual bool operator==(const CStyleProperty &other) const {
    ...
    const CStylePropertyTextDecoration &rhs =
        (const CStylePropertyTextDecoration &)other;
...
public:
  ...
  CStylePropertyTextTransform()
      : CStyleProperty(CStylePropertyTextTransform::symbol) {
  ...
  // property has been set or if more fields should be found by looking at lower
  // weight styles
  virtual bool BFullySet() { return (m_eTransform != k_ETextTransformUnset); }
  ...
  // Return a description for this property which will be shown in the debugger
  virtual const char *GetDescription(CStyleSymbol symProperty) {
    return "Specifies the transform for text within this panel, defaults to "
           "none. Possible values: none, uppercase, lowercase.<br><br>"
           "<b>Example:</b>"
           "<pre>"
  ...
  virtual bool operator==(const CStyleProperty &other) const {
    ...
    const CStylePropertyTextTransform &rhs =
        (const CStylePropertyTextTransform &)other;
...
public:
  ...
  CStylePropertyLineHeight()
      : CStyleProperty(CStylePropertyLineHeight::symbol) {
  ...
  virtual void Interpolate(IUIPanel *pPanel, const CStyleProperty &target,
                           float flProgress /* 0.0->1.0 */) {
    Assert(!"You can't interpolate a line height");
  }
  ...
  // property has been set or if more fields should be found by looking at lower
  // weight styles
  virtual bool BFullySet() { return (m_flLineHeight != k_flFloatNotSet); }

  // called when applied to a panel. Gives the style an opportunity to change
  // any unset values to defaults
  virtual void ResolveDefaultValues() {
    // Leave unset rather than defaulting, will mean don't apply and let
    // font-size set
  }

  // Return a description for this property which will be shown in the debugger
  virtual const char *GetDescription(CStyleSymbol symProperty) {
    return "Specifies the line height (distance between top edge of line above "
           "and line below) to use for text.  By default "
           "this is unset and a value that matches the font-size reasonably "
           "will be used automatically.<br><br>"
           "<b>Example:</b>"
           "<pre>"
           "line-height: 20px;"
           "</pre>";
  ...
  virtual bool operator==(const CStyleProperty &other) const {
    ...
    const CStylePropertyLineHeight &rhs =
        (const CStylePropertyLineHeight &)other;
...
public:
  ...
  virtual bool BSetFromString(CStyleSymbol symParsedName,
    ...
    return CSSHelpers::BParseFillBrushCollection(&m_FillBrushCollection,
                                                 pchString, NULL);
  }
...
public:
  static const CStyleSymbol symbol;
  CStylePropertyForegroundColor()
      : CStylePropertyFillColor(CStylePropertyForegroundColor::symbol) {}

  // Return a description for this property which will be shown in the debugger
  virtual const char *GetDescription(CStyleSymbol symProperty) {
    return "Sets the foreground fill color/gradient/combination for a panel.  "
           "This color is the color used to render text within the "
           "panel.<br><br>"
           "<b>Examples:</b>"
           "<pre>"
           "color: #FFFFFFFF;\n"
           "color: gradient( linear, 0% 0%, 0% 100%, from( #cbcbcbff ), to( "
           "#a0a0a0a0 ) );"
           "</pre>";
  }
  ...
  virtual bool BSetFromString(CStyleSymbol symParsedName,
                              const char *pchString) {
    bool bResult =
        CStylePropertyFillColor::BSetFromString(symParsedName, pchString);

#ifdef _DEBUG
    const CUtlVectorFixed<CFillBrushCollection::FillBrush_t,
                          MAX_FILL_BRUSHES_PER_COLLECTION> &vecBrushes =
        m_FillBrushCollection.AccessBrushes();
    FOR_EACH_VEC(vecBrushes, i) {
      AssertMsg(vecBrushes[i].m_Brush.GetType() !=
                    CFillBrush::k_EStrokeTypeParticleSystem,
                "Particle systems not supported as foreground color, only "
                "background.  Won't render.");
    }
#endif
...
public:
  ...
  CStylePropertyTransform3D()
      : CStyleProperty(CStylePropertyTransform3D::symbol) {
    m_bDirty = true;
  ...
  virtual void MergeTo(CStyleProperty *pTarget) const {
    ...
    if (pTarget->GetPropertySymbol() != GetPropertySymbol()) {
      AssertMsg(false,
                "Mismatched types to CStylePropertyTransform3D::MergeTo");
      return;
    }
    ...
    p->m_bDirty = true;
    p->m_Matrix = m_Matrix;
    p->m_vecTransforms.EnsureCapacity(p->m_vecTransforms.Count() +
                                      m_vecTransforms.Count());
  ...
  VMatrix GetTransformMatrix(float flParentWidth, float flParentHeight) const {
    // If dirty, or if called with new width/height (and not interpolated,
    // because once interpolated we can't recompute values), then recompute the
    // matrix with the specified width/height for the parent
    if (m_bDirty ||
        (!m_bInterpolated &&
         (fabs(flParentWidth - m_flCachedParentWidth) > 0.0001f ||
          fabs(flParentHeight - m_flCachedParentHeight) > 0.0001f))) {
      ...
      m_Matrix = VMatrix::GetIdentityMatrix();
      FOR_EACH_VEC(m_vecTransforms, i) {
        m_Matrix = m_vecTransforms[i]->GetTransformMatrix(flParentWidth,
                                                          flParentHeight) *
                   m_Matrix;
      }
    }
  ...
  virtual bool BSetFromString(CStyleSymbol symParsedName,
    ...
    while (pchString[0] != '\0') {
      ...
      if (!CSSHelpers::BParseTransformFunction(&pTransform, pchString,
                                               &pchString))
        return false;
  ...
  virtual void ApplyUIScaleFactor(float flScaleFactor) {
    ...
    if (!m_bInterpolated) {
      ...
      // Can't make an interpolated matrix dirty as we don't have data to
      // recreate it fully, must decompose and scale the appropriate values that
      // way
  ...
  // Return a description for this property which will be shown in the debugger
  virtual const char *GetDescription(CStyleSymbol symProperty) {
    return "Sets the transforms to apply to the panel in 2d or 3d space.  You "
           "can combine various transforms (comma separated) "
           "and they will be applied in order to create a 4x4 3d transform "
           "matrix.  The possible operations are: translate3d( x, y, z ), "
           "translatex( x ), translatey( y ), translatez( z ), scale3d( x, y, "
           "z), rotate3d( x, y, z ), rotatex( x ), rotatey( y ), rotatez( z "
           ").<br><br>"
           "<b>Examples:</b>"
           "<pre>"
           "transform: translate3d( -100px, -100px, 0px );\n"
           "transform: rotateZ( -32deg ) rotateX( 30deg ) translate3d( 125px, "
           "520px, 230px );"
           "</pre>";
  }
...
#pragma optimize("", off)
  ...
  virtual bool operator==(const CStyleProperty &other) const {
    ...
    const CStylePropertyTransform3D &rhs =
        (const CStylePropertyTransform3D &)other;

    if (m_bInterpolated || rhs.m_bInterpolated) {
      AssertMsg(false, "Can't compare two CStylePropertyTransform3D that have "
                       "been interpolated");
      return false;
    }
    ...
    FOR_EACH_VEC(m_vecTransforms, i) {
      ...
      Assert(eType1 == k_ETransform3DRotate ||
             eType1 == k_ETransform3DTranslate ||
             eType1 == k_ETransform3DScale);
      ETransform3DType eType2 = pRHS->GetType();
      Assert(eType2 == k_ETransform3DRotate ||
             eType2 == k_ETransform3DTranslate ||
             eType2 == k_ETransform3DScale);
...
private:
  ...
  bool m_bInterpolated; // once interpolated, only m_Matrix will be set.
                        // m_vecTransforms will be empty.
};
...
public:
  ...
  CStylePropertyTransitionProperties()
      : CStyleProperty(CStylePropertyTransitionProperties::symbol) {
    m_bImmediate = false;
  }
  ...
  virtual void MergeTo(CStyleProperty *pTarget) const {
    if (pTarget->GetPropertySymbol() != GetPropertySymbol()) {
      AssertMsg(
          false,
          "Mismatched types to CStylePropertyTransitionProperties::MergeTo");
      return;
    }

    CStyleProperty::MergeTo(pTarget);

    CStylePropertyTransitionProperties *p =
        (CStylePropertyTransitionProperties *)pTarget;

    // bugbug cboyd - need to think about what to do when property counts dont
    // match. currently, we do not change the number of properties in the target
    // when a property name has been set as that count should override ours
    bool bPropertiesSet =
        (p->m_vecTransitionProperties.Count() > 0)
            ? p->m_vecTransitionProperties[0].m_symProperty.IsValid()
            : false;

    // add properties to target to match our count
    if (!bPropertiesSet) {
      p->m_vecTransitionProperties.EnsureCapacity(
          m_vecTransitionProperties.Count());
      for (int i = p->m_vecTransitionProperties.Count();
           i < m_vecTransitionProperties.Count(); i++)
        p->AddNewProperty();
    }
    ...
    FOR_EACH_VEC(m_vecTransitionProperties, i) {
      ...
      if (transOther.m_flTransitionDelaySeconds == k_flFloatNotSet)
        transOther.m_flTransitionDelaySeconds =
            transUs.m_flTransitionDelaySeconds;
    }
  ...
  void AddNewProperty() {
    ...
    if (iVec == 0) {
      ...
      transition.m_eTimingFunction =
          m_vecTransitionProperties[iVec - 1].m_eTimingFunction;
      transition.m_CubicBezier =
          m_vecTransitionProperties[iVec - 1].m_CubicBezier;
      transition.m_flTransitionSeconds =
          m_vecTransitionProperties[iVec - 1].m_flTransitionSeconds;
      transition.m_flTransitionDelaySeconds =
          m_vecTransitionProperties[iVec - 1].m_flTransitionDelaySeconds;
    }
  }
  ...
  virtual bool BSetFromString(CStyleSymbol symParsedName,
                              const char *pchString) {
    // need a temp buffer that we can terminate if !immediate is on the end of
    // the string
    char rgchBuffer[1024];

    V_strncpy(rgchBuffer, pchString, V_ARRAYSIZE(rgchBuffer));
    if (!BParseAndRemoveImmediate(m_bImmediate ? NULL : &m_bImmediate,
                                  rgchBuffer))
      return false;
    ...
    if (symParsedName == symbol) {
      ...
      while (*pchString != '\0') {
        ...
        // get rest
        if (!CSSHelpers::BParseTime(&transition.m_flTransitionSeconds,
                                    pchString, &pchString) ||
            !CSSHelpers::BParseTimingFunction(&transition.m_eTimingFunction,
                                              &transition.m_CubicBezier,
                                              pchString, &pchString) ||
            !CSSHelpers::BParseTime(&transition.m_flTransitionDelaySeconds,
                                    pchString, &pchString)) {
          return false;
        }
      ...
      if (!CSSHelpers::BParseCommaSepList(
              &vecProperties, CSSHelpers::BParseIdentToStyleSymbol, pchString))
        return false;
      ...
      return BParseAndAddProperty(&TransitionProperty_t::m_flTransitionSeconds,
                                  CSSHelpers::BParseTime, pchString);
    } else if (symParsedName == symbolDelay) {
      return BParseAndAddProperty(
          &TransitionProperty_t::m_flTransitionDelaySeconds,
          CSSHelpers::BParseTime, pchString);
    } else if (symParsedName == symbolTiming) {
      return BParseAndAddTimingFunction(CSSHelpers::BParseTimingFunction,
                                        pchString);
    }
  ...
  template <typename T>
  bool BParseAndAddProperty(T TransitionProperty_t::*pProp,
                            bool (*func)(T *, const char *, const char **),
                            const char *pchString) {
    // parse comma separated list
    CUtlVector<T> vec;
    if (!CSSHelpers::BParseCommaSepList(&vec, func, pchString) ||
        vec.Count() == 0)
      return false;
    ...
    // if provided list is less than the total number of properties, fill in
    // rest with last value
    for (int i = vec.Count(); i < m_vecTransitionProperties.Count(); i++)
      m_vecTransitionProperties[i].*pProp =
          m_vecTransitionProperties[i - 1].*pProp;

    return true;
  }

  bool BParseAndAddTimingFunction(
      bool (*func)(EAnimationTimingFunction *pTimeingFunc,
                   CCubicBezierCurve<Vector2D> *pBezier, const char *,
                   const char **),
      const char *pchString) {
    // parse comma separated list
    CUtlVector<EAnimationTimingFunction> vec;
    CUtlVector<CCubicBezierCurve<Vector2D>> vec2;
    if (!CSSHelpers::BParseCommaSepList(&vec, &vec2, func, pchString) ||
        vec.Count() == 0)
      return false;
    ...
    // rest with last value
    for (int i = vec.Count(); i < m_vecTransitionProperties.Count(); i++) {
      m_vecTransitionProperties[i].m_eTimingFunction =
          m_vecTransitionProperties[i - 1].m_eTimingFunction;
      m_vecTransitionProperties[i].m_CubicBezier =
          m_vecTransitionProperties[i - 1].m_CubicBezier;
    }
  ...
  bool BCanPropertyTransition(CStyleSymbol symProperty) {
    ...
    // bugbug cboyd - need less expensive check for valid property symbol and if
    // it supports transition
    CStyleProperty *pStyleProperty =
        UIEngine()->UIStyleFactory()->CreateStyleProperty(symProperty);
    if (!pStyleProperty)
      return false;
  ...
  virtual bool BFullySet() {
    ...
    // if the first transition property is fully set, all other transition
  ...
  virtual void ResolveDefaultValues() {
    ...
    // if property names aren't set or transition time isn't set (would default
    // to 0), remove all and exit
    TransitionProperty_t &firstProperty = m_vecTransitionProperties[0];
    if (!firstProperty.m_symProperty.IsValid() ||
        firstProperty.m_flTransitionSeconds == k_flFloatNotSet) {
      m_vecTransitionProperties.RemoveAll();
      return;
    }
    ...
    FOR_EACH_VEC(m_vecTransitionProperties, i) {
      ...
      if (transitionProperty.m_eTimingFunction == k_EAnimationUnset) {
        ...
        Vector2D vec[4];
        panorama::GetAnimationCurveControlPoints(
            transitionProperty.m_eTimingFunction, vec);
  ...
  virtual void ToString(CFmtStr1024 *pfmtBuffer) const {
    ...
    FOR_EACH_VEC(m_vecTransitionProperties, i) {
      ...
      if (transition.m_eTimingFunction != k_EAnimationCustomBezier)
        pfmtBuffer->AppendFormat(" %s ", PchNameFromEAnimationTimingFunction(
                                             transition.m_eTimingFunction));
      else
  ...
  virtual const char *GetDescription(CStyleSymbol symProperty) {
    if (symProperty == symbol) {
      return "Specifies which properties should transition smoothly to new "
             "values if a class/pseudo class changes the styles.  "
             "Also specifies duration, timing function, and delay.  Valid "
             "timing functions are: ease, ease-in, ease-out, ease-in-out, "
             "linear.<br><br>"
             "<b>Example:</b>"
             "<pre>"
             "transition: position 2.0s ease-in-out 0.0s, perspective-origin "
             "1.2s ease-in-out 0.8s;"
             "</pre>";
    } else if (symProperty == symbolProperty) {
      return "Specifies which properties should transition smoothly to new "
             "values if a class/pseudo class changes the styles.<br><br>"
             "<b>Examples:</b>"
             "<pre>"
             "transition: position, transform, background-color;"
             "</pre>";
    } else if (symProperty == symbolTiming) {
      return "Specifies the timing function to use for transition properties "
             "on this panel, if more than one comma delimited value is "
             "specified "
             "then the values are applied to each property specified in "
             "'transition-property' in order.  If only one value is specified "
             "then "
             "it applies to all the properties specified in "
             "transition-property. Valid timing functions are: ease, ease-in, "
             "ease-out, ease-in-out, linear.<br><br>"
             "<b>Examples:</b>"
             "<pre>"
             "transition-timing-function: ease-in-out;\n"
             "transition-timing-function: ease-in-out, linear;\n"
             "transition-timing-function: cubic-bezier( 0.785, 0.385, 0.555, "
             "1.505 );"
             "</pre>";

    } else if (symProperty == symbolDuration) {
      return "Specifies the durating in seconds to use for transition "
             "properties on this panel, if more than one comma delimited value "
             "is specified "
             "then the values are applied to each property specified in "
             "'transition-property' in order.  If only one value is specified "
             "then "
             "it applies to all the properties specified in "
             "transition-property.<br><br>"
             "<b>Examples:</b>"
             "<pre>"
      ...
      return "Specifies the delay in seconds to use for transition properties "
             "on this panel, if more than one comma delimited value is "
             "specified "
             "then the values are applied to each property specified in "
             "'transition-property' in order.  If only one value is specified "
  ...
  virtual bool operator==(const CStyleProperty &other) const {
    ...
    const CStylePropertyTransitionProperties &rhs =
        (const CStylePropertyTransitionProperties &)other;
    if (m_vecTransitionProperties.Count() !=
        rhs.m_vecTransitionProperties.Count())
      return false;
...
public:
  static const CStyleSymbol symbol;
  CStylePropertyBackgroundColor()
      : CStylePropertyFillColor(CStylePropertyBackgroundColor::symbol) {}
  ...
  // Return a description for this property which will be shown in the debugger
  virtual const char *GetDescription(CStyleSymbol symProperty) {
    return "Sets the background fill color/gradient/combination for a "
           "panel.<br><br>"
           "<b>Examples:</b>"
           "<pre>"
           "background-color: #FFFFFFFF;\n"
           "background-color: gradient( linear, 0% 0%, 0% 100%, from( "
           "#fbfbfbff ), to( #c0c0c0c0 ) );\n"
           "background-color: gradient( linear, 0% 0%, 0% 100%, from( "
           "#fbfbfbff ), color-stop( 0.3, #ebebebff ), to( #c0c0c0c0 ) );\n"
           "background-color: gradient( radial, 50% 50%, 0% 0%, 80% 80%, from( "
           "#00ff00ff ), to( #0000ffff ) );\n"
           "background-color: #0d1c22ff, gradient( radial, 100% -0%, 100px "
           "-40px, 320% 270%, from( #3a464bff ), color-stop( 0.23, #0d1c22ff "
           "), to( #0d1c22ff ) );"
           "</pre>";
  }
};
...
public:
  ...
  virtual bool BSetFromString(CStyleSymbol symParsedName,
    ...
    if (symParsedName == symbol) {
      ...
      if (CSSHelpers::BParseIntoUILength(&m_rgBorderWidth[0], pchString,
                                         &pchString)) {
        m_rgBorderWidth[1] = m_rgBorderWidth[2] = m_rgBorderWidth[3] =
            m_rgBorderWidth[0];

        if (CSSHelpers::BParseBorderStyle(&m_rgBorderStyle[0], pchString,
                                          &pchString)) {
          m_rgBorderStyle[1] = m_rgBorderStyle[2] = m_rgBorderStyle[3] =
              m_rgBorderStyle[0];

          if (CSSHelpers::BParseColor(&m_rgBorderColor[0], pchString,
                                      &pchString)) {
            m_rgColorsSet[0] = m_rgColorsSet[1] = m_rgColorsSet[2] =
                m_rgColorsSet[3] = true;
            m_rgBorderColor[1] = m_rgBorderColor[2] = m_rgBorderColor[3] =
                m_rgBorderColor[0];
          }
        }

        // bugbug - should reset all values, as well as border-image and
        // border-radius as well, see
      ...
      if (CSSHelpers::BParseIntoUILength(&m_rgBorderWidth[0], pchString,
                                         &pchString)) {
        if (CSSHelpers::BParseIntoUILength(&m_rgBorderWidth[1], pchString,
                                           &pchString)) {
          if (CSSHelpers::BParseIntoUILength(&m_rgBorderWidth[2], pchString,
                                             &pchString)) {
            if (!CSSHelpers::BParseIntoUILength(&m_rgBorderWidth[3], pchString,
                                                &pchString))
      ...
      if (CSSHelpers::BParseBorderStyle(&m_rgBorderStyle[0], pchString,
                                        &pchString)) {
        if (CSSHelpers::BParseBorderStyle(&m_rgBorderStyle[1], pchString,
                                          &pchString)) {
          if (CSSHelpers::BParseBorderStyle(&m_rgBorderStyle[2], pchString,
                                            &pchString)) {
            if (!CSSHelpers::BParseBorderStyle(&m_rgBorderStyle[3], pchString,
                                               &pchString))
      ...
      if (CSSHelpers::BParseColor(&m_rgBorderColor[0], pchString, &pchString)) {
        if (CSSHelpers::BParseColor(&m_rgBorderColor[1], pchString,
                                    &pchString)) {
          if (CSSHelpers::BParseColor(&m_rgBorderColor[2], pchString,
                                      &pchString)) {
            if (!CSSHelpers::BParseColor(&m_rgBorderColor[3], pchString,
                                         &pchString))
  ...
  // helper function to parse the shortcut property for a single edge
  // (border-top, border-right, border-bottom, border-left)
  bool BParseSingleEdgeShortcutProperty(int nEdgeIndex, const char *pchString) {
    // shorthand for width, style, color
    if (CSSHelpers::BParseIntoUILength(&m_rgBorderWidth[nEdgeIndex], pchString,
                                       &pchString)) {
      if (CSSHelpers::BParseBorderStyle(&m_rgBorderStyle[nEdgeIndex], pchString,
                                        &pchString)) {
        if (CSSHelpers::BParseColor(&m_rgBorderColor[nEdgeIndex], pchString,
                                    &pchString)) {
  ...
  // Gets string representation of property
  virtual void ToString(CFmtStr1024 *pfmtBuffer) const {
    // bugbug jmccaskey - this emits an invalid value for now... can't really
    // put it all into just "border:"...

    pfmtBuffer->Append(m_rgBorderStyle[0] == k_EBorderStyleSolid ? "solid "
                                                                 : "none ");
    pfmtBuffer->Append(m_rgBorderStyle[1] == k_EBorderStyleSolid ? "solid "
                                                                 : "none ");
    pfmtBuffer->Append(m_rgBorderStyle[2] == k_EBorderStyleSolid ? "solid "
                                                                 : "none ");
    pfmtBuffer->Append(m_rgBorderStyle[3] == k_EBorderStyleSolid ? "solid "
                                                                 : "none ");
  ...
  virtual const char *GetDescription(CStyleSymbol symProperty) {
    if (symProperty == symbol) {
      return "Shorthand for setting panel border. Specify width, style, and "
             "color.  Supported styles are: solid, none.<br><br>"
             "<b>Examples:</b>"
             "<pre>"
             "border: 2px solid #111111FF;"
             "</pre>";
    } else if (symProperty == symTop) {
      return "Shorthand for setting the top panel border. Specify width, "
             "style, and color.  Supported styles are: solid, none.<br><br>"
             "<b>Examples:</b>"
             "<pre>"
             "border-top: 2px solid #111111FF;"
             "</pre>";
    } else if (symProperty == symRight) {
      return "Shorthand for setting the right panel border. Specify width, "
             "style, and color.  Supported styles are: solid, none.<br><br>"
             "<b>Examples:</b>"
             "<pre>"
      ...
      return "Shorthand for setting the bottom panel border. Specify width, "
             "style, and color.  Supported styles are: solid, none.<br><br>"
             "<b>Examples:</b>"
             "<pre>"
      ...
      return "Shorthand for setting the left panel border. Specify width, "
             "style, and color.  Supported styles are: solid, none.<br><br>"
             "<b>Examples:</b>"
             "<pre>"
      ...
      return "Specifies border color for panel.  If a single color value is "
             "set it applies to all sides, if 2 are set the first is "
             "top/bottom and "
             "the second is left/right, if all four are set then they are top, "
             "right, bottom, left in order.<br><br>"
             "<b>Examples:</b>"
             "<pre>"
      ...
      return "Specifies border width for panel.  If a single width value is "
             "set it applies to all sides, if 2 are set the first is "
             "top/bottom and "
             "the second is left/right, if all four are set then they are top, "
             "right, bottom, left in order.<br><br>"
      ...
      return "Specifies border style for panel.  If a single style value is "
             "set it applies to all sides, if 2 are set the first is "
             "top/bottom and "
             "the second is left/right, if all four are set then they are top, "
             "right, bottom, left in order.<br><br>"
  ...
  virtual bool operator==(const CStyleProperty &other) const {
    ...
    if (V_memcmp(m_rgBorderStyle, rhs.m_rgBorderStyle,
                 sizeof(m_rgBorderStyle)) != 0)
      return false;
  ...
  GetInvalidateLayout(CStyleProperty *pCompareProperty) const {
    ...
    const CStylePropertyBorder &rhs =
        (const CStylePropertyBorder &)*pCompareProperty;
    if (m_rgBorderWidth[0] != rhs.m_rgBorderWidth[0] ||
        m_rgBorderWidth[1] != rhs.m_rgBorderWidth[1] ||
        m_rgBorderWidth[2] != rhs.m_rgBorderWidth[2] ||
        m_rgBorderWidth[3] != rhs.m_rgBorderWidth[3]) {
      return k_EStyleInvalidateLayoutSizeAndPosition;
    }
...
public:
  ...
  CStylePropertyBorderRadius()
      : CStyleProperty(CStylePropertyBorderRadius::symbol) {}

  virtual void MergeTo(CStyleProperty *pTarget) const {
    if (pTarget->GetPropertySymbol() != GetPropertySymbol()) {
      AssertMsg(false,
                "Mismatched types to CStylePropertyBorderRadius::MergeTo");
      return;
    }
    ...
    for (int i = 0; i < k_ECornerMax; ++i) {
      if (!p->m_rgCornerRaddi[i].m_HorizontalRadii.IsSet())
        p->m_rgCornerRaddi[i].m_HorizontalRadii =
            m_rgCornerRaddi[i].m_HorizontalRadii;
      if (!p->m_rgCornerRaddi[i].m_VerticalRadii.IsSet())
        p->m_rgCornerRaddi[i].m_VerticalRadii =
            m_rgCornerRaddi[i].m_VerticalRadii;
    }
  }
  ...
  virtual bool BSetFromString(CStyleSymbol symParsedName,
                              const char *pchString) {
    if (symParsedName == symbol) {
      // horizontal before /, vertical after, vertical matches horizontal if
      // unset
      ...
      if (!CSSHelpers::BParseIntoUILength(&topRight, pchString, &pchString)) {
        ...
        if (!CSSHelpers::BParseIntoUILength(&bottomRight, pchString,
                                            &pchString)) {
          // Bottom right matches top left then, and bottom left matches top
          // right
          m_rgCornerRaddi[k_ECornerBottomRight].m_HorizontalRadii = topLeft;
          ...
          if (!CSSHelpers::BParseIntoUILength(&bottomLeft, pchString,
                                              &pchString)) {
      ...
      // Look for the / then, which will have vertical values after.  If not
      // found set them all to match horizontal.
      pchString = CSSHelpers::SkipSpaces(pchString);
      if (pchString[0] != '/') {
        for (int i = 0; i < k_ECornerMax; ++i)
          m_rgCornerRaddi[i].m_VerticalRadii =
        ...
        // Insist we get one value after the /, having a trailing / with no
        // vertical values is bogus
        if (!CSSHelpers::BParseIntoUILength(&topLeft, pchString, &pchString)) {
      ...
      if (!CSSHelpers::BParseIntoUILength(&length, pchString, &pchString))
        m_rgCornerRaddi[iCorner].m_VerticalRadii =
            m_rgCornerRaddi[iCorner].m_HorizontalRadii;
      else
  ...
  // called when we are ready to apply any scaling factor to the values
  virtual void ApplyUIScaleFactor(float flScaleFactor) {
    m_rgCornerRaddi[k_ECornerTopLeft].m_HorizontalRadii.ScaleLengthValue(
        flScaleFactor);
    m_rgCornerRaddi[k_ECornerTopLeft].m_VerticalRadii.ScaleLengthValue(
        flScaleFactor);
    m_rgCornerRaddi[k_ECornerTopRight].m_HorizontalRadii.ScaleLengthValue(
        flScaleFactor);
    m_rgCornerRaddi[k_ECornerTopRight].m_VerticalRadii.ScaleLengthValue(
        flScaleFactor);
    m_rgCornerRaddi[k_ECornerBottomRight].m_HorizontalRadii.ScaleLengthValue(
        flScaleFactor);
    m_rgCornerRaddi[k_ECornerBottomRight].m_VerticalRadii.ScaleLengthValue(
        flScaleFactor);
    m_rgCornerRaddi[k_ECornerBottomLeft].m_HorizontalRadii.ScaleLengthValue(
        flScaleFactor);
    m_rgCornerRaddi[k_ECornerBottomLeft].m_VerticalRadii.ScaleLengthValue(
        flScaleFactor);
  }

  // Gets string representation of property
  virtual void ToString(CFmtStr1024 *pfmtBuffer) const {
    CSSHelpers::AppendUILength(
        pfmtBuffer, m_rgCornerRaddi[k_ECornerTopLeft].m_HorizontalRadii);
    pfmtBuffer->Append(" ");
    CSSHelpers::AppendUILength(
        pfmtBuffer, m_rgCornerRaddi[k_ECornerTopRight].m_HorizontalRadii);
    pfmtBuffer->Append(" ");
    CSSHelpers::AppendUILength(
        pfmtBuffer, m_rgCornerRaddi[k_ECornerBottomRight].m_HorizontalRadii);
    pfmtBuffer->Append(" ");
    CSSHelpers::AppendUILength(
        pfmtBuffer, m_rgCornerRaddi[k_ECornerBottomLeft].m_HorizontalRadii);

    pfmtBuffer->AppendFormat(" / ");

    CSSHelpers::AppendUILength(
        pfmtBuffer, m_rgCornerRaddi[k_ECornerTopLeft].m_VerticalRadii);
    pfmtBuffer->Append(" ");
    CSSHelpers::AppendUILength(
        pfmtBuffer, m_rgCornerRaddi[k_ECornerTopRight].m_VerticalRadii);
    pfmtBuffer->Append(" ");
    CSSHelpers::AppendUILength(
        pfmtBuffer, m_rgCornerRaddi[k_ECornerBottomRight].m_VerticalRadii);
    pfmtBuffer->Append(" ");
    CSSHelpers::AppendUILength(
        pfmtBuffer, m_rgCornerRaddi[k_ECornerBottomLeft].m_VerticalRadii);
  }
  ...
  virtual const char *GetDescription(CStyleSymbol symProperty) {
    if (symProperty == symbol) {
      return "Shorthand to set border radius for all corners at once.  Border "
             "radius rounds off corners of the panel, adjusting the border to "
             "smoothly round and also clipping background image/color and "
             "contents to the specified elliptical or circular values.  In "
             "this shorthand "
             "version you may specify a single value for all raddi, or "
             "horizontal / vertical separated by the '/' character.  For both "
             "horizontal and "
             "vertical you may specify 1 to 4 values in pixels or %, they will "
             "be taken in order as top-left, top-right, bottom-right, "
             "bottom-left radii "
             "values.<br><br>"
             "<b>Examples:</b>"
             "<pre>"
             ...
             "border-radius: 2px;\n"
             "// Perfect circular or elliptical panel (circular if box was "
             "square)\n"
             "border-radius: 50% / 50%;\n"
             "// 2 px horizontal radii 4px vertical elliptical corners on all "
             "sides\n"
      ...
      return "Specifies border-radius for top-left corner which rounds off "
             "border and clips background/foreground content to rounded edge.  "
             "Takes 1 or 2 values in px or %, first value is horizontal radii "
             "for elliptical corner, second is vertical radii, if only one is "
             "specified "
             "then horizontal/vertical will both be set and corner will be "
             "circular.<br><br>"
             "<b>Examples:</b>"
             "<pre>"
      ...
      return "Specifies border-radius for top-right corner which rounds off "
             "border and clips background/foreground content to rounded edge.  "
             "Takes 1 or 2 values in px or %, first value is horizontal radii "
             "for elliptical corner, second is vertical radii, if only one is "
             "specified "
      ...
      return "Specifies border-radius for bottom-right corner which rounds off "
             "border and clips background/foreground content to rounded edge.  "
             "Takes 1 or 2 values in px or %, first value is horizontal radii "
             "for elliptical corner, second is vertical radii, if only one is "
             "specified "
      ...
      return "Specifies border-radius for bottom-left corner which rounds off "
             "border and clips background/foreground content to rounded edge.  "
             "Takes 1 or 2 values in px or %, first value is horizontal radii "
             "for elliptical corner, second is vertical radii, if only one is "
             "specified "
  ...
  virtual bool operator==(const CStyleProperty &other) const {
    ...
    const CStylePropertyBorderRadius &rhs =
        (const CStylePropertyBorderRadius &)other;
  ...
  struct CornerRadii_t {
    ...
    bool operator==(const CornerRadii_t &rhs) const {
      return (m_HorizontalRadii == rhs.m_HorizontalRadii &&
              m_VerticalRadii == rhs.m_VerticalRadii);
    }
...
public:
  ...
  CStylePropertyBorderImage()
      : CStyleProperty(CStylePropertyBorderImage::symbol) {
    m_bFillCenter = false;
    m_pImage = NULL;
    m_bSlicesSet = false;
    m_bWidthsSet = false;
  ...
  virtual ~CStylePropertyBorderImage() { SAFE_RELEASE(m_pImage); }
  ...
  bool BParseImageWidth(BorderImageWidth_t *pWidth, const char *pchString,
                        const char **pchAfterParse);

  // Parse individual repeat
  bool BParseRepeat(EBorderImageRepeatType *pRepeat, const char *pchString,
                    const char **pchAfterParse);
  ...
  // weight styles
  virtual bool BFullySet() {
    if (m_bSlicesSet && m_bStretchSet && m_bWidthsSet && m_bOutsetsSet &&
        !m_sURLPath.IsEmpty())
      return true;
  ...
  virtual const char *GetDescription(CStyleSymbol symProperty) {
    if (symProperty == symbol) {
      return "Shorthand for specifying all the border-image related properties "
             "at once.\n"
             "Technical syntax is: &lt;border-image-source&gt; || "
             "&lt;border-image-slice&gt; [ / &lt;border-image-width&gt;? [ / "
             "&lt;border-image-outset&gt; ]? ]? || "
             "&lt;border-image-repeat&gt;, "
             "see the explanations for individual properties for details on "
             "each.<br><br>"
             "<b>Examples:</b>"
             "<pre>"
             "border-image: url( \"file://message_border.tga\" ) 25% repeat;\n"
             "border-image: url( \"file://message_border.tga\" ) 25% / 1 / "
             "20px repeat;"
             "</pre>";
    } else if (symProperty == symImageSource) {
      return "Specifies the source image to use as the 9-slice "
             "border-image.<br><br>"
             "<b>Examples:</b>"
             "<pre>"
             "border-image-source: url( \"file://message_border.tga\" );\n"
             "border-image-source: url( "
             "\"http://store.steampowered.com/public/images/steam/"
             "message_border.tga\" );"
             "</pre>";
    } else if (symProperty == symImageSlice) {
      return "Specifies the insets for top, right, bottom, and left (in order) "
             "slice offsets to use for slicing the source image into 9 "
             "regions.  "
             "The 'fill' keyword may optionally appear before or after the "
             "length values and specifies to draw the middle region as a fill "
             "for the body "
             "background of the panel, without it the middle region will not "
             "be drawn.<br><br>"
             "<b>Examples:</b>"
             "<pre>"
      ...
      return "By default after slicing the image as specified in "
             "border-image-slice the 9 regions will be used to fill the space "
             "specified "
             "by the standard border-width property.  This border-image-width "
             "property may be used to override that and specify different "
             "widths.  "
             "The values appear in top, right, bottom, left order, the 2nd "
             "through 4th may be ommited and corresponding earlier values will "
             "be used.  "
             "Values may be straight floats which specify a multiple of the "
             "corresponding border-width value, a percentage (which is "
             "relative "
             "to the size of the border image in the corresponding dimension), "
             "or 'auto' which means to use the intrinsic size of the "
             "corresponding "
      ...
      return "Specifies the amount by which the border image should draw "
             "outside of the normal content/border box, this allows the border "
             "image "
             "to extend into the margin area and draw outside the panels "
             "bounds.  This may still result in clipping of the image by a "
             "parent panel if "
             "the parents bounds are too close to the edges of the panel with "
             "the border-image.  Values are specified as px or % in top, "
             "right, bottom, left "
      ...
      return "Specifies how the top/right/bottom/left/middle images of the 9 "
             "slice regions are stretched to fit the available space.  "
             "Options are stretch, repeat, round or space.  Stretch/repeat are "
             "self explanatory, round means tile (repeat) but scale first"
             "to ensure that a whole number of tiles is used with no partial "
             "tile at the edge of the space, space means tile (repeat) but add "
             "padding between tiles to ensure a whole number of tiles with no "
             "partial tile at the edge is needed.\n"
             "Two values are specified, the first applies to how we stretch "
             "the top/middle/bottom horizontally to fill space, the second "
             "applies to how we stretch the left/middle/right vertically to "
             "fill space.<br><br>"
             "<b>Examples:</b>"
             "<pre>"
  ...
  virtual bool operator==(const CStyleProperty &other) const {
    ...
    const CStylePropertyBorderImage &rhs =
        (const CStylePropertyBorderImage &)other;
    ...
    if (m_bStretchSet &&
        V_memcmp(m_Stretch, rhs.m_Stretch, sizeof(m_Stretch)) != 0)
      return false;
...
public:
  ...
  virtual void Interpolate(IUIPanel *pPanel, const CStyleProperty &target,
                           float flProgress /* 0.0->1.0 */) {
    Assert(!"You can't interpolate an image");
  }
  virtual bool BSetFromString(CStyleSymbol symParsedName,
                              const char *pchString);
  virtual void ToString(CFmtStr1024 *pfmtBuffer) const;
...
public:
  ...
  CStylePropertyOpacityMask()
      : CStyleProperty(CStylePropertyOpacityMask::symbol) {
    m_pImage = NULL;
  ...
  virtual void MergeTo(CStyleProperty *pTarget) const {
    if (pTarget->GetPropertySymbol() != GetPropertySymbol()) {
      AssertMsg(false,
                "Mismatched types to CStylePropertyOpacityMask::MergeTo");
      return;
    }
  ...
  virtual void Interpolate(IUIPanel *pPanel, const CStyleProperty &target,
    ...
    if (!p->m_sURL.IsValid()) {
      ...
      // Fade in
      m_sURL = p->m_sURL;
      m_flOpacityMaskOpacity =
          Lerp(flProgress, 0.0f, p->m_flOpacityMaskOpacity);
    } else {
      m_sURL = p->m_sURL;
      m_flOpacityMaskOpacity =
          Lerp(flProgress, m_flOpacityMaskOpacity, p->m_flOpacityMaskOpacity);
    }
  ...
  virtual bool BSetFromString(CStyleSymbol symParsedName,
                              const char *pchString) {
    if (symParsedName == symbol) {
      if (BParseProperty(pchString, m_flOpacityMaskOpacity, &m_pImage,
                         m_sURL)) {
        m_bWasSet = true;
        return true;
      }
      ...
      return BParseProperty(pchString, m_flOpacityMaskOpacityUp, &m_pImageUp,
                            m_sURLUp);
    } else if (symParsedName == symbolScrollDown) {
      return BParseProperty(pchString, m_flOpacityMaskOpacityDown,
                            &m_pImageDown, m_sURLDown);
    } else if (symParsedName == symbolScrollUpDown) {
      return BParseProperty(pchString, m_flOpacityMaskOpacityUpDown,
                            &m_pImageUpDown, m_sURLUpDown);
    }

    return false;
  }

  bool BParseProperty(const char *pchString, float &flOpacityMaskOpacity,
                      IImageSource **ppImage, CUtlString &sURL) {
    if (V_stricmp("none", pchString) == 0) {
      *ppImage = NULL;
      ...
      pchString = CSSHelpers::SkipSpaces(pchString);
      if (!CSSHelpers::BParseNumber(&flOpacityMaskOpacity, pchString,
                                    &pchString))
  ...
  virtual const char *GetDescription(CStyleSymbol symProperty) {
    if (symProperty == symbol) {
      return "Applies an image as an opacity mask that stretches to the panel "
             "bounds and fades out it's content based on the alpha channel. "
             "The second float value is an optional opacity value for the mask "
             "itself, the image won't interpolate/cross-fade, but you can "
             "animate the opacity to fade the mask in/out. "
             "The -scroll-up, -scroll-down, and -scroll-up-down varients "
             "override the mask and apply only when the various vertical "
             "scroll scenarios affect the panel based on the overflow "
             "property.<br><br>"
             "<b>Examples:</b>"
             "<pre>"
             ...
             "opacity-mask: url( \"file://{images}/upper_row_mask.tga\" ) "
             "0.5;\n"
             "opacity-mask-scroll-up: url( "
             "\"file://{images}/upper_row_mask_up.tga\" ) 0.5;\n"
             "opacity-mask-scroll-down: url( "
             "\"file://{images}/upper_row_mask_down.tga\" ) 0.5;\n"
             "opacity-mask-scroll-up-down: url( "
             "\"file://{images}/upper_row_mask_up_down.tga\" ) 0.5;"
             "</pre>";
    }
    return "";
  }
  ...
  virtual bool operator==(const CStyleProperty &other) const {
    ...
    const CStylePropertyOpacityMask &rhs =
        (const CStylePropertyOpacityMask &)other;
    return (m_sURL == rhs.m_sURL &&
            m_flOpacityMaskOpacity == rhs.m_flOpacityMaskOpacity &&
            m_sURLUp == rhs.m_sURLUp &&
            m_flOpacityMaskOpacityUp == rhs.m_flOpacityMaskOpacityUp &&
            m_sURLDown == rhs.m_sURLDown &&
            m_flOpacityMaskOpacityDown == rhs.m_flOpacityMaskOpacityDown &&
            m_sURLUpDown == rhs.m_sURLUpDown &&
            m_flOpacityMaskOpacityUpDown == rhs.m_flOpacityMaskOpacityUpDown);
  }
...
public:
  static const CStyleSymbol symbol;
  CStylePropertyWhiteSpace()
      : CStyleProperty(CStylePropertyWhiteSpace::symbol) {
    m_bSet = false;
    m_bWrap = true;
  }
  ...
  virtual const char *GetDescription(CStyleSymbol symProperty) {
    if (symProperty == symbol) {
      return "Controls white-space wrapping on rendered text.  \"normal\" "
             "means wrap on whitespace, \"nowrap\" means do no wrapping at "
             "all.<br><br>"
             "<b>Examples:</b>"
             "<pre>"
             "white-space: normal;\n"
  ...
  virtual bool operator==(const CStyleProperty &other) const {
    ...
    const CStylePropertyWhiteSpace &rhs =
        (const CStylePropertyWhiteSpace &)other;
...
public:
  ...
  virtual void MergeTo(CStyleProperty *pTarget) const {
    if (pTarget->GetPropertySymbol() != GetPropertySymbol()) {
      AssertMsg(false,
                "Mismatched types to CStylePropertyTextOverflow::MergeTo");
      return;
    }
  ...
  virtual bool BSetFromString(CStyleSymbol symParsedName,
    ...
    if (V_strnicmp(pchString, "clip", V_ARRAYSIZE("clip")) == 0) {
      ...
      m_bSet = true;
      return true;
    } else if (V_strnicmp(pchString, "ellipsis", V_ARRAYSIZE("ellipsis")) ==
               0) {
      m_bEllipsis = true;
      m_bSet = true;
      return true;
  ...
  virtual const char *GetDescription(CStyleSymbol symProperty) {
    if (symProperty == symbol) {
      return "Controls truncation of text that doesn't fit in a panel.  "
             "\"clip\" means to simply truncate (on char boundaries), "
             "\"ellipsis\" means to end with '...'.\n"
             "We default to ellipsis, which is contrary to the normal CSS "
             "spec.<br><br>"
             "<b>Examples:</b>"
             "<pre>"
  ...
  virtual bool operator==(const CStyleProperty &other) const {
    ...
    const CStylePropertyTextOverflow &rhs =
        (const CStylePropertyTextOverflow &)other;
...
public:
  ...
  virtual void Interpolate(IUIPanel *pPanel, const CStyleProperty &target,
    ...
    GetParentSizeAvailable(pPanel, flParentWidth, flParentHeight,
                           flParentPerspective);
  ...
  virtual bool BSetFromString(CStyleSymbol symParsedName,
                              const char *pchString) {
    bool bSuccess =
        CSSHelpers::BParseIntoUILengthForSizing(&m_Length, pchString, NULL);
  ...
  virtual const char *GetDescription(CStyleSymbol symProperty) {
    return "Sets the width for this panel. "
           "Possible values:<br>"
           "\"fit-children\" - Panel size is set to the required size of all "
           "children (default)<br>"
           "&lt;pixels&gt; - Any fixed pixel value (ex: \"100px\")<br>"
           "&lt;percentage&gt; - Percentage of parent width (ex: \"100%\")<br>"
           "\"fill-parent-flow( &lt;weight&gt; )\" - Fills to remaining parent "
           "width. If multiple children are set to this value, weight is used "
           "to determine final width. For example, if three children are set "
           "to fill-parent-flow of 1.0 and the parent is 300px wide, each "
           "child will be 100px wide. (ex: \"fill-parent-flow( 1.0 )\" )<br>"
           "\"height-percentage( &lt;percentage&gt; )\" - Percentage of the "
           "panel's height, which allows you to enforce a particular aspect "
           "ratio.  The height cannot also be width-percentage.";
  }
...
public:
  ...
  virtual bool BSetFromString(CStyleSymbol symParsedName,
                              const char *pchString) {
    bool bSuccess =
        CSSHelpers::BParseIntoUILengthForSizing(&m_Height, pchString, NULL);
  ...
  virtual const char *GetDescription(CStyleSymbol symProperty) {
    return "Sets the height for this panel. "
           ...
           "\"fill-parent-flow( &lt;weight&gt; )\" - Fills to remaining parent "
           "width. If multiple children are set to this value, weight is used "
           "to determine final height. For example, if three children are set "
           "to fill-parent-flow of 1.0 and the parent is 300px tall, each "
           "child will be 100px tall. (ex: \"fill-parent-flow( 1.0 )\" )<br>"
           "\"width-percentage( &lt;percentage&gt; )\" - Percentage of the "
           "panel's width, which allows you to enforce a particular aspect "
           "ratio.  The width cannot also be height-percentage.";
  }
...
public:
  ...
  virtual bool BSetFromString(CStyleSymbol symParsedName,
                              const char *pchString) {
    bool bSuccess =
        CSSHelpers::BParseIntoUILengthForSizing(&m_minWidth, pchString, NULL);
    return bSuccess;
  }
...
public:
  ...
  virtual bool BSetFromString(CStyleSymbol symParsedName,
                              const char *pchString) {
    bool bSuccess =
        CSSHelpers::BParseIntoUILengthForSizing(&m_minHeight, pchString, NULL);
    return bSuccess;
  }
...
public:
  ...
  virtual void Interpolate(IUIPanel *pPanel, const CStyleProperty &target,
    ...
    m_maxWidth =
        LerpUILength(flProgress, m_maxWidth, p->m_maxWidth, flParentWidth);
  }
  ...
  virtual bool BSetFromString(CStyleSymbol symParsedName,
                              const char *pchString) {
    bool bSuccess =
        CSSHelpers::BParseIntoUILengthForSizing(&m_maxWidth, pchString, NULL);
    return bSuccess;
  }
...
public:
  ...
  virtual void Interpolate(IUIPanel *pPanel, const CStyleProperty &target,
    ...
    m_maxHeight =
        LerpUILength(flProgress, m_maxHeight, p->m_maxHeight, flParentHeight);
  }
  ...
  virtual bool BSetFromString(CStyleSymbol symParsedName,
                              const char *pchString) {
    bool bSuccess =
        CSSHelpers::BParseIntoUILengthForSizing(&m_maxHeight, pchString, NULL);
    return bSuccess;
  }
...
public:
  static const CStyleSymbol symbol;
  CStylePropertyFlowChildren()
      : CStyleProperty(CStylePropertyFlowChildren::symbol) {
  ...
  virtual bool operator==(const CStyleProperty &other) const {
    ...
    const CStylePropertyFlowChildren &rhs =
        (const CStylePropertyFlowChildren &)other;
...
//-----------------------------------------------------------------------------
template <class T> class CStylePropertyDimensionsBase : public CStyleProperty {
public:
  ...
  virtual void MergeTo(CStyleProperty *pTarget) const {
    if (pTarget->GetPropertySymbol() != GetPropertySymbol()) {
      AssertMsg(false,
                "Mismatched types to CStylePropertyDimensionsBase::MergeTo");
      return;
    }
    ...
    if (symParsedName == T::symbol) {
      ...
      if (CSSHelpers::BParseIntoUILength(&m_top, pchString, &pchString)) {
        if (CSSHelpers::BParseIntoUILength(&m_right, pchString, &pchString)) {
          if (CSSHelpers::BParseIntoUILength(&m_bottom, pchString,
                                             &pchString)) {
            if (CSSHelpers::BParseIntoUILength(&m_left, pchString,
                                               &pchString)) {
              // got all 4
            } else {
              m_left = m_right;
  ...
  // weight styles
  virtual bool BFullySet() {
    return (m_left.IsSet() && m_top.IsSet() && m_bottom.IsSet() &&
            m_right.IsSet());
  }
  ...
  virtual bool operator==(const CStyleProperty &other) const {
    ...
    const CStylePropertyDimensionsBase<T> &rhs =
        (const CStylePropertyDimensionsBase<T> &)other;
...
public:
  ...
  CStylePropertyAnimationProperties()
      : CStyleProperty(CStylePropertyAnimationProperties::symbol) {
    m_bNone = false;
  }
  ...
  virtual void MergeTo(CStyleProperty *pTarget) const {
    if (pTarget->GetPropertySymbol() != GetPropertySymbol()) {
      AssertMsg(
          false,
          "Mismatched types to CStylePropertyAnimationProperties::MergeTo");
      return;
    }

    CStylePropertyAnimationProperties *p =
        (CStylePropertyAnimationProperties *)pTarget;
    ...
    if (m_bNone && (p->m_vecAnimationProperties.Count() == 0 ||
                    !p->m_vecAnimationProperties[0].m_symName.IsValid())) {
      p->m_bNone = true;
    ...
    // when a property name has been set as that count should override ours
    bool bPropertiesSet =
        (p->m_vecAnimationProperties.Count() > 0)
            ? p->m_vecAnimationProperties[0].m_symName.IsValid()
            : false;

    // add properties to target to match our count
    if (!bPropertiesSet) {
      p->m_vecAnimationProperties.EnsureCapacity(
          m_vecAnimationProperties.Count());
      for (int i = p->m_vecAnimationProperties.Count();
           i < m_vecAnimationProperties.Count(); i++)
        p->AddNewProperty();
    }
  ...
  void AddNewProperty() {
    ...
    if (iVec == 0) {
      ...
      Vector2D vec[4];
      panorama::GetAnimationCurveControlPoints(transition.m_eTimingFunction,
                                               vec);
      transition.m_CubicBezier.SetControlPoints(vec);
  ...
  void SetAnimation(const char *pchAnimationName, float flDuration,
                    float flDelay, EAnimationTimingFunction eTimingFunc,
                    CCubicBezierCurve<Vector2D> cubicBezier,
                    EAnimationDirection eDirection, float flIterations) {
    m_bNone = false;
  ...
  virtual bool BSetFromString(CStyleSymbol symParsedName,
    ...
    if (symParsedName == symbol) {
      ...
      // transition property format: name duration timing-function delay
      ...
      while (*pchString != '\0') {
        ...
        if (!CSSHelpers::BParseIdent(rgchName, V_ARRAYSIZE(rgchName), pchString,
                                     &pchString))
          return false;
        ...
        // get rest
        if (!CSSHelpers::BParseTime(&animation.m_flDuration, pchString,
                                    &pchString) ||
            !CSSHelpers::BParseTimingFunction(&animation.m_eTimingFunction,
                                              &animation.m_CubicBezier,
                                              pchString, &pchString) ||
            !CSSHelpers::BParseTime(&animation.m_flDelay, pchString,
                                    &pchString) ||
            !BParseIterationCount(&animation.m_flIteration, pchString,
                                  &pchString) ||
            !CSSHelpers::BParseAnimationDirectionFunction(
                &animation.m_eAnimationDirection, pchString, &pchString)) {
          return false;
        }
      ...
      if (!CSSHelpers::BParseCommaSepList(
              &vecNames, CSSHelpers::BParseIdentToSymbol, pchString))
        return false;
      ...
      return BParseAndAddProperty(&AnimationProperty_t::m_flDuration,
                                  CSSHelpers::BParseTime, pchString);
    } else if (symParsedName == symbolDelay) {
      return BParseAndAddProperty(&AnimationProperty_t::m_flDelay,
                                  CSSHelpers::BParseTime, pchString);
    } else if (symParsedName == symbolTiming) {
      return BParseAndAddTimingFunction(CSSHelpers::BParseTimingFunction,
                                        pchString);
    } else if (symParsedName == symbolIteration) {
      return BParseAndAddProperty(&AnimationProperty_t::m_flIteration,
                                  BParseIterationCount, pchString);
    } else if (symParsedName == symbolDirection) {
      return BParseAndAddProperty(&AnimationProperty_t::m_eAnimationDirection,
                                  CSSHelpers::BParseAnimationDirectionFunction,
                                  pchString);
    }
  ...
  virtual void ToString(CFmtStr1024 *pfmtBuffer) const {
    ...
    FOR_EACH_VEC(m_vecAnimationProperties, i) {
      ...
      else if (prop.m_eAnimationDirection ==
  ...
  template <typename T>
  bool BParseAndAddProperty(T AnimationProperty_t::*pProp,
                            bool (*func)(T *, const char *, const char **),
                            const char *pchString) {
    // parse comma separated list
    CUtlVector<T> vec;
    ...
    for (int i = vec.Count(); i < m_vecAnimationProperties.Count(); i++)
      m_vecAnimationProperties[i].*pProp =
          m_vecAnimationProperties[i - 1].*pProp;

    return true;
  }

  bool BParseAndAddTimingFunction(bool (*func)(EAnimationTimingFunction *,
                                               CCubicBezierCurve<Vector2D> *,
    ...
    // rest with last value
    for (int i = vec.Count(); i < m_vecAnimationProperties.Count(); i++) {
      m_vecAnimationProperties[i].m_eTimingFunction =
          m_vecAnimationProperties[i - 1].m_eTimingFunction;
      m_vecAnimationProperties[i].m_CubicBezier =
          m_vecAnimationProperties[i - 1].m_CubicBezier;
    }

    return true;
  }

  static bool BParseIterationCount(float *pflValue, const char *pchString,
                                   const char **pchAfterParse) {
    pchString = CSSHelpers::SkipSpaces(pchString);
  ...
  virtual bool BFullySet() {
    ...
    return (animationProperty.m_symName.IsValid() &&
            ...
            animationProperty.m_eAnimationDirection !=
                k_EAnimationDirectionUnset &&
  ...
  virtual void ResolveDefaultValues() {
    ...
    if (!firstProperty.m_symName.IsValid() ||
        firstProperty.m_flDuration == k_flFloatNotSet) {
      m_vecAnimationProperties.RemoveAll();
      return;
    }
    ...
    FOR_EACH_VEC(m_vecAnimationProperties, i) {
      ...
      if (animationProperty.m_eTimingFunction == k_EAnimationUnset) {
        ...
        Vector2D vec[4];
        panorama::GetAnimationCurveControlPoints(
            animationProperty.m_eTimingFunction, vec);
  ...
  virtual bool operator==(const CStyleProperty &other) const {
    ...
    const CStylePropertyAnimationProperties &rhs =
        (const CStylePropertyAnimationProperties &)other;
    if (m_bNone != rhs.m_bNone)
      return false;

    if (m_vecAnimationProperties.Count() !=
        rhs.m_vecAnimationProperties.Count())
      return false;
...
//-----------------------------------------------------------------------------
class CStylePropertyPadding
    : public CStylePropertyDimensionsBase<CStylePropertyPadding> {
public:
  static const CStyleSymbol symbol;
  ...
  CStylePropertyPadding()
      : CStylePropertyDimensionsBase<CStylePropertyPadding>() {}
};

//-----------------------------------------------------------------------------
// Purpose: Margin property
//-----------------------------------------------------------------------------
class CStylePropertyMargin
    : public CStylePropertyDimensionsBase<CStylePropertyMargin> {
public:
  static const CStyleSymbol symbol;
  static const CStyleSymbol symbolLeft;
  static const CStyleSymbol symbolTop;
  ...
  CStylePropertyMargin()
      : CStylePropertyDimensionsBase<CStylePropertyMargin>() {}
};
...
public:
  static const CStyleSymbol symbol;
  CStylePropertyMixBlendMode()
      : CStyleProperty(CStylePropertyMixBlendMode::symbol) {
    m_bSet = false;
  ...
  virtual void MergeTo(CStyleProperty *pTarget) const {
    if (pTarget->GetPropertySymbol() != GetPropertySymbol()) {
      AssertMsg(false,
                "Mismatched types to CStylePropertyMixBlendMode::MergeTo");
      return;
    }
  ...
  virtual bool BSetFromString(CStyleSymbol symParsedName,
    ...
    if (V_strnicmp(pchString, "normal", V_ARRAYSIZE("normal")) == 0) {
      ...
      m_bSet = true;
      return true;
    } else if (V_strnicmp(pchString, "multiply", V_ARRAYSIZE("multiply")) ==
               0) {
  ...
  virtual const char *GetDescription(CStyleSymbol symProperty) {
    if (symProperty == symbol) {
      return "Controls blending mode for the panel.  See CSS mix-blend-mode "
             "docs on web, except normal for us is with alpha blending.<br><br>"
             "<b>Examples:</b>"
             "<pre>"
  ...
  virtual bool operator==(const CStyleProperty &other) const {
    ...
    const CStylePropertyMixBlendMode &rhs =
        (const CStylePropertyMixBlendMode &)other;
...
public:
  static const CStyleSymbol symbol;
  CStylePropertyTextureSampleMode()
      : CStyleProperty(CStylePropertyTextureSampleMode::symbol) {
    m_bSet = false;
  ...
  virtual void MergeTo(CStyleProperty *pTarget) const {
    if (pTarget->GetPropertySymbol() != GetPropertySymbol()) {
      AssertMsg(false,
                "Mismatched types to CStylePropertyTextureSampleMode::MergeTo");
      return;
    }

    CStylePropertyTextureSampleMode *p =
        (CStylePropertyTextureSampleMode *)pTarget;
    if (!p->m_bSet) {
  ...
  virtual bool BSetFromString(CStyleSymbol symParsedName,
    ...
    if (V_strnicmp(pchString, "normal", V_ARRAYSIZE("normal")) == 0) {
      ...
      m_bSet = true;
      return true;
    } else if (V_strnicmp(pchString, "alpha-only", V_ARRAYSIZE("alpha-only")) ==
               0) {
  ...
  virtual const char *GetDescription(CStyleSymbol symProperty) {
    if (symProperty == symbol) {
      return "Controls texture sampling mode for the panel. Set to alpha-only "
             "to use the textures alpha data across all 3 color "
             "channels.<br><br>"
             "<b>Examples:</b>"
             "<pre>"
  ...
  virtual bool operator==(const CStyleProperty &other) const {
    ...
    const CStylePropertyTextureSampleMode &rhs =
        (const CStylePropertyTextureSampleMode &)other;
    return (m_eTextureSampleMode == rhs.m_eTextureSampleMode &&
            m_bSet == rhs.m_bSet);
  }
...
public:
  ...
  virtual bool BSetFromString(CStyleSymbol symParsedName,
                              const char *pchString) {
    if (symParsedName == symbol) {
      return CSSHelpers::BParseHorizontalAlignment(&m_eHorizontalAlignment,
                                                   pchString, &pchString) &&
             CSSHelpers::BParseVerticalAlignment(&m_eVerticalAlignment,
                                                 pchString, &pchString);
    } else if (symParsedName == symbolHorizontal) {
      return CSSHelpers::BParseHorizontalAlignment(&m_eHorizontalAlignment,
                                                   pchString, &pchString);
    } else if (symParsedName == symbolVertical) {
      return CSSHelpers::BParseVerticalAlignment(&m_eVerticalAlignment,
                                                 pchString, &pchString);
    }
  ...
  virtual void ToString(CFmtStr1024 *pfmtBuffer) const {
    pfmtBuffer->AppendFormat(
        "%s %s", PchNameFromEHorizontalAlignment(m_eHorizontalAlignment),
        PchNameFromEVerticalAlignment(m_eVerticalAlignment));
  }
  ...
  // weight styles
  virtual bool BFullySet() {
    return (m_eHorizontalAlignment != k_EHorizontalAlignmentUnset &&
            m_eVerticalAlignment != k_EVerticalAlignmentUnset);
  }
  ...
  virtual bool operator==(const CStyleProperty &other) const {
    ...
    return (m_eHorizontalAlignment == rhs.m_eHorizontalAlignment &&
            m_eVerticalAlignment == rhs.m_eVerticalAlignment);
  }
...
public:
  CStylePropertyContextUIPosition(CStyleSymbol symPropertyName)
      : CStyleProperty(symPropertyName) {
  ...
  virtual void MergeTo(CStyleProperty *pTarget) const {
    if (pTarget->GetPropertySymbol() != GetPropertySymbol()) {
      AssertMsg(false,
                "Mismatched types to CStylePropertyTooltipPosition::MergeTo");
      return;
    }

    CStylePropertyContextUIPosition *p =
        (CStylePropertyContextUIPosition *)pTarget;
  ...
  virtual bool BSetFromString(CStyleSymbol symParsedName,
    ...
    int i = 0;
    while (*pchString != '\0') {
      if (!CSSHelpers::BParseIdent(rgchPosition, V_ARRAYSIZE(rgchPosition),
                                   pchString, &pchString))
        return i != 0;
  ...
  // called when applied to a panel. Gives the style an opportunity to change
  // any unset values to defaults
  virtual void ResolveDefaultValues() { ResolveDefaultPositions(m_ePositions); }
  ...
  static void ResolveDefaultPositions(EContextUIPosition (&ePositions)[4]) {
    ...
    if (ePositions[1] == k_EContextUIPositionUnset ||
        ePositions[1] == ePositions[0]) {
    ...
    // The third spot is normally the opposite of the first spot, unless those
    // are both already used.  Then it's just right or bottom.
    if (ePositions[2] == k_EContextUIPositionUnset ||
        (ePositions[2] == ePositions[0] || ePositions[2] == ePositions[1])) {
      if ((ePositions[0] == k_EContextUIPositionLeft &&
           ePositions[1] == k_EContextUIPositionRight) ||
          (ePositions[0] == k_EContextUIPositionRight &&
           ePositions[1] == k_EContextUIPositionLeft)) {
        ePositions[2] = k_EContextUIPositionBottom;
                  ...
                  ePositions[1] == k_EContextUIPositionBottom) ||
                 (ePositions[0] == k_EContextUIPositionBottom &&
                  ePositions[1] == k_EContextUIPositionTop)) {
    ...
    if (ePositions[3] == k_EContextUIPositionUnset ||
        (ePositions[3] == ePositions[0] || ePositions[3] == ePositions[1] ||
         ePositions[3] == ePositions[2])) {
      CUtlVector<EContextUIPosition> vecPositions(0, 4);
      const EContextUIPosition arrAllPositions[] = {
          k_EContextUIPositionLeft, k_EContextUIPositionTop,
          k_EContextUIPositionRight, k_EContextUIPositionBottom};
  ...
  virtual bool operator==(const CStyleProperty &other) const {
    ...
    const CStylePropertyContextUIPosition &rhs =
        (const CStylePropertyContextUIPosition &)other;
...
public:
  static const CStyleSymbol symbol;
  CStylePropertyTooltipPosition()
      : CStylePropertyContextUIPosition(CStylePropertyTooltipPosition::symbol) {
  }

  // Return a description for this property which will be shown in the debugger
  virtual const char *GetDescription(CStyleSymbol symProperty) {
    return "Specifies where to position a tooltip relative to this panel. "
           "Valid options include 'left', 'top', 'right', and 'bottom'. "
           "List up to 4 positions to determine the order that positions are "
           "tried if the tooltip doesn't fully fit on screen. Default "
           "is 'right left bottom top'. If less than 4 positions are "
           "specified, the tooltip first tries the opposite of the specified "
           "position along the same axis before switching to the other "
           "axis.<br><br>"
           "<b>Examples:</b>"
           "<pre>"
...
//-----------------------------------------------------------------------------
class CStylePropertyContextMenuPosition
    : public CStylePropertyContextUIPosition {
public:
  static const CStyleSymbol symbol;
  CStylePropertyContextMenuPosition()
      : CStylePropertyContextUIPosition(
            CStylePropertyContextMenuPosition::symbol) {}

  // Return a description for this property which will be shown in the debugger
  virtual const char *GetDescription(CStyleSymbol symProperty) {
    return "Specifies where to position a context menu relative to this panel. "
           "Valid options include 'left', 'top', 'right', and 'bottom'. "
           "List up to 4 positions to determine the order that positions are "
           "tried if the context menu doesn't fully fit on screen. Default "
           "is 'right left bottom top'. If less than 4 positions are "
           "specified, the context menu first tries the opposite of the "
           "specified "
           "position along the same axis before switching to the other "
           "axis.<br><br>"
           "<b>Examples:</b>"
...
public:
  CStylePropertyContextUIComponentPosition(CStyleSymbol symPropertyName)
      : CStyleProperty(symPropertyName) {}

  virtual void MergeTo(CStyleProperty *pTarget) const {
    if (pTarget->GetPropertySymbol() != GetPropertySymbol()) {
      AssertMsg(false, "Mismatched types to "
                       "CStylePropertyTooltipComponentPosition::MergeTo");
      return;
    }

    CStylePropertyContextUIComponentPosition *p =
        (CStylePropertyContextUIComponentPosition *)pTarget;
  ...
  virtual bool BSetFromString(CStyleSymbol symParsedName,
                              const char *pchString) {
    return CSSHelpers::BParseIntoTwoUILengths(
        &m_HorizontalPosition, &m_VerticalPosition, pchString, &pchString);
  }
  ...
  virtual bool operator==(const CStyleProperty &other) const {
    ...
    const CStylePropertyContextUIComponentPosition &rhs =
        (const CStylePropertyContextUIComponentPosition &)other;
    return m_HorizontalPosition == rhs.m_HorizontalPosition &&
           m_VerticalPosition == rhs.m_VerticalPosition;
  }
...
//-----------------------------------------------------------------------------
class CStylePropertyContextUIBodyPosition
    : public CStylePropertyContextUIComponentPosition {
public:
  CStylePropertyContextUIBodyPosition(CStyleSymbol symPropertyName)
      : CStylePropertyContextUIComponentPosition(symPropertyName) {}
  ...
  static void ResolveDefaultBodyPosition(CUILength &horizontalPosition,
                                         CUILength &verticalPosition) {
...
//-----------------------------------------------------------------------------
class CStylePropertyTooltipBodyPosition
    : public CStylePropertyContextUIBodyPosition {
public:
  static const CStyleSymbol symbol;
  CStylePropertyTooltipBodyPosition()
      : CStylePropertyContextUIBodyPosition(
            CStylePropertyTooltipBodyPosition::symbol) {}

  // Return a description for this property which will be shown in the debugger
  virtual const char *GetDescription(CStyleSymbol symProperty) {
    return "Specifies where to position the body of a tooltip relative to this "
           "panel. The first value controls how the body is "
           "aligned horizontally when the tooltip is to the top or bottom of "
           "the panel, and the second value controls how the body is "
           "aligned vertically when the tooltip is to the left or right of the "
           "panel. 0% means left/top aligned, 50% means "
           "center/middle aligned, and 100% means right/bottom aligned. "
           "Default is '0% 0%'.<br><br>"
           "<b>Example:</b>"
           "<pre>"
...
//-----------------------------------------------------------------------------
class CStylePropertyContextMenuBodyPosition
    : public CStylePropertyContextUIBodyPosition {
public:
  static const CStyleSymbol symbol;
  CStylePropertyContextMenuBodyPosition()
      : CStylePropertyContextUIBodyPosition(
            CStylePropertyContextMenuBodyPosition::symbol) {}

  // Return a description for this property which will be shown in the debugger
  virtual const char *GetDescription(CStyleSymbol symProperty) {
    return "Specifies where to position the body of a context menu relative to "
           "this panel. The first value controls how the body is "
           "aligned horizontally when the context menu is to the top or bottom "
           "of the panel, and the second value controls how the body is "
           "aligned vertically when the context menu is to the left or right "
           "of the panel. 0% means left/top aligned, 50% means "
           "center/middle aligned, and 100% means right/bottom aligned. "
           "Default is '0% 0%'.<br><br>"
           "<b>Example:</b>"
           "<pre>"
...
//-----------------------------------------------------------------------------
class CStylePropertyContextUIArrowPosition
    : public CStylePropertyContextUIComponentPosition {
public:
  CStylePropertyContextUIArrowPosition(CStyleSymbol symPropertyName)
      : CStylePropertyContextUIComponentPosition(symPropertyName) {}
  ...
  static void ResolveDefaultArrowPosition(CUILength &horizontalPosition,
                                          CUILength &verticalPosition) {
    if (!horizontalPosition.IsSet()) {
...
//-----------------------------------------------------------------------------
class CStylePropertyTooltipArrowPosition
    : public CStylePropertyContextUIArrowPosition {
public:
  static const CStyleSymbol symbol;
  CStylePropertyTooltipArrowPosition()
      : CStylePropertyContextUIArrowPosition(
            CStylePropertyTooltipArrowPosition::symbol) {}

  // Return a description for this property which will be shown in the debugger
  virtual const char *GetDescription(CStyleSymbol symProperty) {
    return "Specifies where to point the arrow of a tooltip at on this panel. "
           "The first value controls how the arrow is "
           "positioned horizontally when the tooltip is to the top or bottom "
           "of the panel, and the second value controls how the arrow is "
           "positioned vertically when the tooltip is to the left or right of "
           "the panel. Default is '50% 50%'.<br><br>"
           "<b>Example:</b>"
           "<pre>"
...
//-----------------------------------------------------------------------------
class CStylePropertyContextMenuArrowPosition
    : public CStylePropertyContextUIArrowPosition {
public:
  static const CStyleSymbol symbol;
  CStylePropertyContextMenuArrowPosition()
      : CStylePropertyContextUIArrowPosition(
            CStylePropertyContextMenuArrowPosition::symbol) {}

  // Return a description for this property which will be shown in the debugger
  virtual const char *GetDescription(CStyleSymbol symProperty) {
    return "Specifies where to point the arrow of a context menu at on this "
           "panel. The first value controls how the arrow is "
           "positioned horizontally when the context menu is to the top or "
           "bottom of the panel, and the second value controls how the arrow "
           "is "
           "positioned vertically when the context menu is to the left or "
           "right of the panel. Default is '50% 50%'.<br><br>"
           "<b>Example:</b>"
           "<pre>"
...
public:
  CStylePropertySound(CStyleSymbol symPropertyName)
      : CStyleProperty(symPropertyName) {}
  ...
  // property has been set or if more fields should be found by looking at lower
  // weight styles
  virtual bool BFullySet() { return !m_strSoundName.IsEmpty(); }
...
public:
  static const CStyleSymbol symbol;
  CStylePropertyEntranceSound()
      : CStylePropertySound(CStylePropertyEntranceSound::symbol) {}

  // Return a description for this property which will be shown in the debugger
  virtual const char *GetDescription(CStyleSymbol symProperty) {
    return "Specifies a sound name to play when this selector is "
           "applied.<br><br>"
           "<b>Example:</b>"
           "<pre>"
           "sound: \"whoosh_in\";"
...
public:
  static const CStyleSymbol symbol;
  CStylePropertyExitSound()
      : CStylePropertySound(CStylePropertyExitSound::symbol) {}
