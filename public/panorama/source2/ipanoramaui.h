...
public:
  ...
  // Access UI engine, generally use global panorama::UIEngine() accessor
  // instead as shorthand
  virtual panorama::IUIEngine *AccessUIEngine() = 0;

  virtual bool
  HandleInputEvent(const InputEvent_t &event,
                   const CUtlVector<panorama::IUIWindow *> &vecWindowInputOrder,
                   bool bOnlyIfFocused) = 0;
};
...
public:
  virtual panorama::IUIEngine *SetupUIEngine(const char *pszLanguage,
  ...
  virtual panorama::IUIPanelClient *CreatePanel2D(panorama::IUIWindow *pParent,
                                                  const char *pID) = 0;
  virtual IPanoramaClientDebugger *CreateDebugger(panorama::IUIWindow *pParent,
                                                  const char *pID) = 0;
};
