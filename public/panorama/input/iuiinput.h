...
enum EActiveControllerType {
  k_EActiveControllerType_None, // we aren't using a controller or we don't know
                                // to do
...
//
struct KeyData_t {
  EPanelEventSource_t m_eSource; // this needs to be the first field of the
                                 // struct, since it's unioned in InputMessage_t
...
struct GamePadData_t {
  ...
  // For Steam controller analog events this value will indicate the time the
  // users finger went down
  float m_flFingerDown;

  // For Steam controller analog events these values indicate the starting
  // finger position (so you can do some basic swipe stuff)
  float m_fValueXFirst;
  float m_fValueYFirst;

  // For Steam controller analog events this is the raw sampled coordinate
  // without deadzoning
  float m_fValueXRaw;
  float m_fValueYRaw;
};
...
struct InputMessage_t {
  ...
  union {
    EPanelEventSource_t
        m_eSource; // where did this event originate? this is the first field of
                   // the other *Data_t structs below.
    KeyData_t m_KeyData;
...
//
// A combination of event type and specific code data to mean an input event,
// like on key down of the A key
//
struct ActionInput_t {
  ...
  explicit ActionInput_t(EInputType type, KeyCode code, uint32 unModifiers,
  ...
  explicit ActionInput_t(EInputType type, GamePadCode code,
                         const char *pchNamespace) {
    m_InputType = type;
  ...
  explicit ActionInput_t(EInputType type, MouseCode code,
                         const char *pchNamespace) {
    m_InputType = type;
  ...
  // Can't ever make this not 32bits in size, see crazy comparison operators
  // below.  Also, each member must be exactly 32bits, not less with
  // uninitialized data.
  ...
  bool operator<(const ActionInput_t &that) const {
    ...
    if (m_symNameSpace.IsValid() && that.m_symNameSpace.IsValid() &&
        m_symNameSpace != that.m_symNameSpace)
  ...
  bool operator==(const ActionInput_t &that) const {
    if (m_InputType == that.m_InputType &&
        m_unModifiers == that.m_unModifiers &&
        // also if both namespace are valid and equal OR either namespace is
        // unset (i.e "global")
        ((m_symNameSpace.IsValid() && that.m_symNameSpace.IsValid() &&
          m_symNameSpace == that.m_symNameSpace) ||
         (!m_symNameSpace.IsValid() && !that.m_symNameSpace.IsValid()))) {
      switch (m_InputType) {
      default:
...
inline bool IsControlPressed(uint32 unModifiers) {
  return unModifiers & MODIFIER_LCONTROL || unModifiers & MODIFIER_RCONTROL;
}
inline bool IsAltPressed(uint32 unModifiers) {
  return unModifiers & MODIFIER_LALT || unModifiers & MODIFIER_RALT;
}
inline bool IsShiftPressed(uint32 unModifiers) {
  return unModifiers & MODIFIER_LSHIFT || unModifiers & MODIFIER_RSHIFT;
}
inline bool IsWinPressed(uint32 unModifiers) {
  return unModifiers & MODIFIER_LWIN || unModifiers & MODIFIER_RWIN;
}
...
public:
  ...
  virtual bool OnCapturedKeyTyped(IUIPanel *pPanel,
                                  const KeyData_t &unichar) = 0;

  // mouse
  virtual bool OnCapturedMouseMove(IUIPanel *pPanel) = 0;
  virtual bool OnCapturedMouseButtonDown(IUIPanel *pPanel,
                                         const MouseData_t &code) = 0;
  virtual bool OnCapturedMouseButtonUp(IUIPanel *pPanel,
                                       const MouseData_t &code) = 0;
  virtual bool OnCapturedMouseButtonDoubleClick(IUIPanel *pPanel,
                                                const MouseData_t &code) = 0;
  virtual bool OnCapturedMouseButtonTripleClick(IUIPanel *pPanel,
                                                const MouseData_t &code) = 0;
  virtual bool OnCapturedMouseWheel(IUIPanel *pPanel,
                                    const MouseData_t &code) = 0;

  // gamepad
  virtual bool OnCapturedGamePadDown(IUIPanel *pPanel,
                                     const GamePadData_t &code) = 0;
  virtual bool OnCapturedGamePadUp(IUIPanel *pPanel,
                                   const GamePadData_t &code) = 0;
  virtual bool OnCapturedGamePadAnalog(IUIPanel *pPanel,
                                       const GamePadData_t &code) = 0;
};
...
public:
  // keyboard
  virtual bool OnCapturedKeyDown(panorama::IUIPanel *pPanel,
                                 const panorama::KeyData_t &code) OVERRIDE {
    return false;
  }
  virtual bool OnCapturedKeyUp(panorama::IUIPanel *pPanel,
                               const panorama::KeyData_t &code) OVERRIDE {
    return false;
  }
  virtual bool OnCapturedKeyTyped(panorama::IUIPanel *pPanel,
                                  const panorama::KeyData_t &unichar) OVERRIDE {
    return false;
  }

  // mouse
  virtual bool OnCapturedMouseMove(panorama::IUIPanel *pPanel) OVERRIDE {
    return false;
  }
  virtual bool
  OnCapturedMouseButtonDown(panorama::IUIPanel *pPanel,
                            const panorama::MouseData_t &code) OVERRIDE {
    return false;
  }
  virtual bool
  OnCapturedMouseButtonUp(panorama::IUIPanel *pPanel,
                          const panorama::MouseData_t &code) OVERRIDE {
    return false;
  }
  virtual bool
  OnCapturedMouseButtonDoubleClick(panorama::IUIPanel *pPanel,
                                   const panorama::MouseData_t &code) OVERRIDE {
    return false;
  }
  virtual bool
  OnCapturedMouseButtonTripleClick(panorama::IUIPanel *pPanel,
                                   const panorama::MouseData_t &code) OVERRIDE {
    return false;
  }
  virtual bool
  OnCapturedMouseWheel(panorama::IUIPanel *pPanel,
                       const panorama::MouseData_t &code) OVERRIDE {
    return false;
  }
  ...
  // gamepad
  virtual bool
  OnCapturedGamePadDown(panorama::IUIPanel *pPanel,
                        const panorama::GamePadData_t &code) OVERRIDE {
    return false;
  }
  virtual bool
  OnCapturedGamePadUp(panorama::IUIPanel *pPanel,
                      const panorama::GamePadData_t &code) OVERRIDE {
    return false;
  }
  virtual bool
  OnCapturedGamePadAnalog(panorama::IUIPanel *pPanel,
                          const panorama::GamePadData_t &code) OVERRIDE {
    return false;
  }
};
...
public:
  ...
  // Window can temporarily disable all input, used for overlay when the game is
  // focused, but overlay inactive
  virtual bool BAllowInput(InputMessage_t &msg) = 0;

  // panel management
  virtual void SetInputFocus(IUIPanel *pPanel, bool bScrollParentToFit,
                             bool bChangeContextIfNeeded) = 0;
  ...
  // input hooks
  virtual void HookPanelInput(IUIPanel *pPanel,
                              IInputCapture *pInputCapture) = 0;
  virtual void RemovePanelInputHook(IUIPanel *pPanel,
                                    IInputCapture *pInputCapture) = 0;
  ...
  virtual void QueuePanelFocusEvent(IUIPanel *pPanel,
                                    CPanoramaSymbol symPanelEvent) = 0;
...
public:
  ...
  // flags to tell steam controller layer which buttons to treat as mouse and
  // not disable cursor on seeing
  ...
  // return true if we are emulating a gamepad using a simple joystick, so we
  // have less input functionality available
  virtual bool BEmulatingGamePadWithJoystick() = 0;

  // helper for gamepad codes, returns values that are inside the deadzone for
  // this joystick
  ...
  virtual const GamePadCode
  GetGamePadBindForEvent(const char *pchEvent, const IUIPanel *pFromPanel) = 0;

  // is capslock on
  virtual bool BIsCapsLockOn() = 0;

  // If we're trying to show help text for a controller, which type of
  // controller is most relevant (ie., most recently used, exists, etc.)? You
  // probably don't want to call this directly but instead listen for
  ...
  virtual void
  PulseActiveControllerHaptic(IUIEngine::EHapticFeedbackPosition ePosition,
                              IUIEngine::EHapticFeedbackStrength eStrength) = 0;

  // Is finger actively down on steam controller right pad?  Probably means
  // mouse emulation in use.
  ...
  virtual void SetRemoteGamepadButton(int nGamepadID, int nButton,
                                      int nValue) = 0;
  ...
  virtual panorama::GamePadCode
  GamePadCodeFromName(const char *pchGamePadCode) = 0;

  // Check if two gamepad codes are the 'same' button but on different vendor
  // devices
  virtual bool BIsGamePadCodeEquivalentIgnoringVendor(GamePadCode a,
                                                      GamePadCode b) = 0;
};
