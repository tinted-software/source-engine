...
public:
  // Constructor
  CTextInputDualTouch(panorama::IUIWindow *pParent,
                      const CTextInputHandlerSettings &settings,
                      ITextInputControl *pTextControl);
  CTextInputDualTouch(panorama::CPanel2D *parent,
                      const CTextInputHandlerSettings &settings,
                      ITextInputControl *pTextControl);
...
private:
  ...
  class CTouchPad {
  public:
    bool Initialize(CTextInputDualTouch *pParent, const char *pointerID,
                    const char *padID,
    ...
    CPanel2D
        *m_pLastHoverKey; // what was the last key we were hovering over? this
                          // will either match m_pHoverKey or have the last
                          // value m_pHoverKey had if its currently nullptr
  ...
  bool OnPropertyTransitionEnd(const CPanelPtr<IUIPanel> &pPanel,
                               CStyleSymbol prop);
  bool OnRemoveStyleFromLinkedKeys(CPanelPtr<CPanel2D> pPanel,
                                   const char *pszStyle);
  ...
  bool
  OnActiveControllerTypeChanged(EActiveControllerType eActiveControllerType);
  ...
  bool LoadInputConfigurationFile(char const *szConfigFile,
                                  const char *szConfigRootDir);
  bool LoadConfigurationBuffer(char const *pszIncoming);

  void SetModifierKeyState(EDualTouchModifier_t modifier,
                           bool bIsButtonPressEvent);
  ...
  void ClearSuggestionState(bool bFlush = true) {
    ...
    if (bFlush && m_vecPossibleWordsBeingTyped.Count()) {
      ...
      m_pTextInputControl->InsertCharactersAtCursor(s.ToWString(),
                                                    V_wcslen(s.ToWString()));
    }
...
private:
  void ChangeTouchkeyStyle(CPanel2D *pTouchKey, const char *pchStyle,
                           bool bAddStyle);
...
public:
  ...
  uchar32 m_keyLayout[k_DualTouchColumnCount][k_DualTouchRowCount]
                     [k_EDualTouchModifierCount];
