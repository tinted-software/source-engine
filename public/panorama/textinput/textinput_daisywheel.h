...
public:
  // Constructor
  CTextInputDaisyWheel(panorama::IUIWindow *pParent,
                       const CTextInputHandlerSettings &settings,
                       ITextInputControl *pTextControl);
  CTextInputDaisyWheel(panorama::CPanel2D *parent,
                       const CTextInputHandlerSettings &settings,
                       ITextInputControl *pTextControl);
  ...
  virtual void SetYButtonAction(const char *pchLabel,
                                IUIEvent *pEvent) OVERRIDE;

  static void GetSupportedLanguages(CUtlVector<ELanguage> &vecLangs);

private:
  void Initialize(const CTextInputHandlerSettings &settings,
                  ITextInputControl *pTextControl);

  // This isn't part of CTextInputHandler because currently there is no need to
  ...
  static const int k_cItemsPerLayoutMax =
      k_cPetals * k_cItemsPerPetal; // # letters total per layout, max
  ...
  //
  //	A single wheel layout configuration: name and UTF-8 sequences of
  //characters 	Structure is allocated with more memory at the end of the
  //structure, name and 	UTF-8 sequences follow this structure object
  //
  class CDaisyConfig {
  public:
    ...
    // Get a given item in this wheel layout, must be >= zero and < number of
    // items double the index, because each item is NUL terminated
  ...
  typedef CUtlMap<EDaisyConfig_t, CUtlPtr<CDaisyConfig>, int,
                  CDefLess<EDaisyConfig_t>>
      MapConfigEntries_t;
  ...
  //	Types a character from selected group's side of world: "E" | "W" | "N" |
  //"S"
  ...
  bool LoadInputConfigurationFile(const char *szConfig,
                                  const char *szConfigRootDir);

  //	Builds configuration structure based on buffer loaded from config file
  bool LoadConfigurationBuffer(char const *pszBase,
                               MapConfigEntries_t *pmapConfigs);

  // Switch between most recent languages
  bool SwitchLanguage();
  bool ShowThisLanguage(ELanguage language);

  //	For a given config item determine which group and group side the item
  //should be at
  void GetItemLocation(CDaisyConfig *pCfg, int iItem, char const *&szGroup,
                       char const *&szItem);
  ...
  //	Gets a sequential index of the group square indexed by -1|0|1 pair of
  //x,y coordinates
  ...
  bool HandlePropertyTransitionEnd(
      const panorama::CPanelPtr<panorama::IUIPanel> &pPanel, CStyleSymbol sym);

  // Listen for focus lost
  bool
  HandleInputFocusLost(const panorama::CPanelPtr<panorama::IUIPanel> &ptrPanel);
  ...
  void ClearSuggestionVisual() { ShowSuggestion("", ""); }
  ...
  typedef bool (CTextInputDaisyWheel::*PFNGamePadData)(
      const panorama::GamePadData_t &code);

  MapConfigEntries_t m_mapConfigEntries; //	Processed configuration entries
  EDaisyConfig_t
      m_eConfigCurrent;   //	Current config, can be cycled with triggers
  bool m_bRestrictConfig; //	if true, stick to current config (no changing
                          //layouts)

  panorama::CPanel2D *m_pStickPri; // Primary stick control
  float m_flStickPriSelectOct[2];  // Selection octant angles (std: lo=M_PI/6,
                                   // hi=M_PI/3)
  ...
  float m_flStickSndSelectAngleSticky; // Selection angle for secondary stick to
                                       // stick to area
  ...
  panorama::GamePadCode m_repeatGamePadCode; // Which key was pressed (low
                                             // level, for key-up tracking)
  uint32 m_repeatCounter; // How many key repeats have happened
  panorama::CUIScheduledDel
      m_repeatFunction; // Scheduled function triggering key repeats

  // Stick processing routines
  bool
  OnGamePadAnalog_ProcessLeftStickForGroup(const panorama::GamePadData_t &code);
  bool
  OnGamePadAnalog_ProcessRightStickForSide(const panorama::GamePadData_t &code);
  ...
  panorama::CLabel *
      m_plabelSuggestionPrefix; // label containing prefix of current suggestion
  panorama::CLabel *
      m_plabelSuggestionSuffix; // label containing prefix of current suggestion
  ...
  ITextInputControl
      *m_pTextInputControl; // control interface for moving text input between a
                            // control and daisy wheel
