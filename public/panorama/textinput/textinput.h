...
// When text input finished, bool = true for Done, = false for Cancel; char
// const * = string that user typed Dispatched to
// ITextInputControl::GetAssociatedPanel()
DECLARE_PANEL_EVENT2(TextInputFinished, bool, char const *);

// When the handler is up and sees gamepad right-stick input, it passes it
// through to the containing control, so the control can do something with it.
// Dispatched to ITextInputControl::GetAssociatedPanel()
DECLARE_PANEL_EVENT1(TextInputAnalogStickPassthrough, GamePadData_t);

panorama::ETextInputHandlerType_t
ETextInputHandlerType_tFromName(const char *pchName);
...
public:
  ...
  // return true if you own the backing store of the text and can return it
  // immediately on request, 	false otherwise (html returns false here)
...
// Factory methods
CTextInputHandler *
CreateTextInputHandler(panorama::IUIWindow *pParent,
                       const CTextInputHandlerSettings &settings,
                       ITextInputControl *pControl);
CTextInputHandler *
CreateTextInputHandler(panorama::CPanel2D *pParent,
                       const CTextInputHandlerSettings &settings,
                       ITextInputControl *pControl);
