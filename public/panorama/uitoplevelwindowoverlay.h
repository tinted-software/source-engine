...
public:
  ...
  virtual bool
  BInitializeSurface(const char *pchWindowTitle, int nWidth, int nHeight,
                     IUIEngine::ERenderTarget eRenderTarget,
                     bool bFixedSurfaceSize, bool bEnforceWindowAspectRatio,
                     bool bUseCustomMouseCursor, const char *pchTargetMonitor);
  ...
  virtual void SetVisible(bool bVisible) {
    AssertMsg(false, "SetVisible not implemented on CTopLevelWindowOverlay");
  }
  ...
  bool SetGameProcessInfo(AppId_t nAppId, bool bCanSharedSurfaces,
                          int32 eTextureFormat);
  void ProcessInputEvents();

  bool BVisiblityChanged() const {
    return m_bVisibleThisFrame != m_bVisibleLastFrame;
  }

  void PushOverlayRenderCmdStream(CSharedMemStream *pRenderStream,
                                  unsigned long dwPID, float flOpacity,
                                  EOverlayWindowAlignment alignment);
...
public:
  COverlayInterface(CTopLevelWindowOverlay *pWindow) { m_pWindow = pWindow; }

  virtual void
  PushOverlayRenderCmdStream(CSharedMemStream *pRenderStream,
                             unsigned long dwPID, float flOpacity,
                             EOverlayWindowAlignment alignment) OVERRIDE {
    return m_pWindow->PushOverlayRenderCmdStream(pRenderStream, dwPID,
                                                 flOpacity, alignment);
  }
  ...
  virtual bool SetGameProcessInfo(AppId_t nAppId, bool bCanSharedSurfaces,
                                  int32 eTextureFormat) OVERRIDE {
    return m_pWindow->SetGameProcessInfo(nAppId, bCanSharedSurfaces,
                                         eTextureFormat);
  }
  ...
  virtual void SetFixedSurfaceSize(uint32 unSurfaceWidth,
                                   uint32 unSurfaceHeight) OVERRIDE {
