...
public:
  ...
  virtual bool BPresentYUV420Texture(uint nWidth, uint nHeight, void *pPlaneY,
                                     void *pPlaneU, void *pPlaneV,
                                     uint unStrideY, uint unStrideU,
                                     uint unStrideV) OVERRIDE;
...
private:
  bool OnInitAudioMainThread(CVideoPlayerAudioRenderer *pThis, int nSampleRate,
                             int nChannels);
...
public:
  ...
  bool VideoPlayerEventUIThread(CVideoPlayerEventDispatcher *pDispatcher,
                                EVideoPlayerEvent eEvent);
...
public:
  ...
  void RegisterEventListener(IUIPanel *pPanel) {
    m_eventCallback.RegisterEventListener(pPanel);
  }
  void UnregisterEventListener(IUIPanel *pPanel) {
    m_eventCallback.UnregisterEventListener(pPanel);
  }

  void RegisterEventCallback(VideoPlayerEventDelegate_t del) {
    m_eventCallback.RegisterEventCallback(del);
  }
  void UnregisterEventCallback(VideoPlayerEventDelegate_t del) {
    m_eventCallback.UnregisterEventCallback(del);
  }

  void SetPlaybackVolume(float flVolume) {
    m_audioCallback.SetPlaybackVolume(flVolume);
  }
  ...
  virtual void SetPlaybackSpeed(float flPlaybackSpeed) OVERRIDE {
    m_pVideoPlayer->SetPlaybackSpeed(flPlaybackSpeed);
  }
  virtual void Seek(uint unSeekMS) OVERRIDE { m_pVideoPlayer->Seek(unSeekMS); }
  virtual void SetRepeat(bool bRepeat) OVERRIDE {
    m_pVideoPlayer->SetRepeat(bRepeat);
  }
  virtual void SuggestMaxVeritcalResolution(int nHeight) OVERRIDE {
    m_pVideoPlayer->SuggestMaxVeritcalResolution(nHeight);
  }
  virtual EVideoPlayerPlaybackState GetPlaybackState() OVERRIDE {
    return m_pVideoPlayer->GetPlaybackState();
  }
  virtual bool IsStoppedForBuffering() OVERRIDE {
    return m_pVideoPlayer->IsStoppedForBuffering();
  }
  virtual float GetPlaybackSpeed() OVERRIDE {
    return m_pVideoPlayer->GetPlaybackSpeed();
  }
  virtual uint32 GetDuration() OVERRIDE {
    return m_pVideoPlayer->GetDuration();
  }
  virtual uint32 GetCurrentPlaybackTime() OVERRIDE {
    return m_pVideoPlayer->GetCurrentPlaybackTime();
  }
  virtual EVideoPlayerPlaybackError GetPlaybackError() OVERRIDE {
    return m_pVideoPlayer->GetPlaybackError();
  }
  virtual void GetVideoResolution(int *pnWidth, int *pnHeight) OVERRIDE {
    m_pVideoPlayer->GetVideoResolution(pnWidth, pnHeight);
  }
  virtual int GetVideoDownloadRate() OVERRIDE {
    return m_pVideoPlayer->GetVideoDownloadRate();
  }
  virtual int GetVideoRepresentationCount() OVERRIDE {
    return m_pVideoPlayer->GetVideoRepresentationCount();
  }
  virtual bool BGetVideoRepresentationInfo(int iRep, int *pnWidth,
                                           int *pnHeight) OVERRIDE {
    return m_pVideoPlayer->BGetVideoRepresentationInfo(iRep, pnWidth, pnHeight);
  }
  virtual int GetCurrentVideoRepresentation() OVERRIDE {
    return m_pVideoPlayer->GetCurrentVideoRepresentation();
  }
  virtual void ForceVideoRepresentation(int iRep) OVERRIDE {
    return m_pVideoPlayer->ForceVideoRepresentation(iRep);
  }
  virtual void GetVideoSegmentInfo(int *pnCurrent, int *pnTotal) OVERRIDE {
    m_pVideoPlayer->GetVideoSegmentInfo(pnCurrent, pnTotal);
  }
  virtual bool BHasAudioTrack() OVERRIDE {
    return m_pVideoPlayer->BHasAudioTrack();
  }
