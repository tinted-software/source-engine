...
//-----------------------------------------------------------------------------
// Purpose: Basic window interface exposing operations used inside of panorama,
...
public:
#if defined(SOURCE2_PANORAMA)
  virtual void RenderWindow(ISceneView *pView, IRenderContext *pRenderContext,
                            ISceneLayer *pLayer) = 0;
#endif
  ...
  // Get the surface width for the window, this may be larger or smaller than
  // the actual window if we are rendering lower res and scaling output for
  // instance
  virtual uint32 GetSurfaceWidth() = 0;

  // Get the surface height for the window, this may be larger or smaller than
  // the actual window if we are rendering lower res and scaling output for
  // instance
  ...
  // Handle resize event, window has resized, backing surface needs to now
  // resize to match
  ...
  // bForceful in this call means try to bypass OS rules that may prevent focus
  // (like our app wasn't last to receive input), use this sparingly and only if
  // we really know stealing focus from some other app is what the user
  // definately desires!
  ...
  GetTopLevelVisiblePanels() const = 0;
  ...
  // Get low level native window handle, used by streaming, should generally not
...
#if !defined(SOURCE2_PANORAMA)
  virtual void
  PushOverlayRenderCmdStream(CSharedMemStream *pRenderStream,
                             unsigned long dwPID, float flOpacity,
                             EOverlayWindowAlignment alignment) = 0;

  virtual void SetFocus(bool bFocus) = 0;

  virtual bool SetGameProcessInfo(AppId_t nAppId, bool bCanSharedSurfaces,
                                  int32 eTextureFormat) = 0;

  virtual void SetInputEnabled(bool bEnabled) = 0;

  virtual void SetGameWindowSize(uint32 nWidth, uint32 nHeight) = 0;

  virtual void SetFixedSurfaceSize(uint32 unSurfaceWidth,
