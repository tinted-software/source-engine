...
public:
  ...
  virtual VMatrix GetTransformMatrix(float flParentWidth,
                                     float flParentHeight) const = 0;
...
VMatrix InterpolateTransformMatrix(VMatrix from, VMatrix to,
...
public:
  ...
  CTransformRotate3D(float flDegreesPitch, float flDegreesYaw,
                     float flDegreesRoll) {
  ...
  bool BOnlyImpacts2DValues() { return false; }
...
public:
  ...
  CTransformTranslate3D(const CUILength &x, const CUILength &y,
                        const CUILength &z) {
    m_x = x;
    m_y = y;
    m_z = z;
  ...
  VMatrix GetTransformMatrix(float flParentWidth, float flParentHeight) const {
    ...
    mat.SetTranslation(Vector(m_x.GetValueAsLength(flParentWidth),
                              m_y.GetValueAsLength(flParentHeight),
                              m_z.GetValueAsLength(0)));
    return mat;
  }
