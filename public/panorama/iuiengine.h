...
public:
  enum ERenderTarget {
    // these assignments are important because they are cross-cast at the
    // protobuf layer
    k_ERenderToWindow = 1,
    ...
    k_ERenderToLegacyVR = 7, // This one is used for the Steam Client's main
                             // interface and will die eventually
  ...
  // return true if the states are different and a valid transition.  Used to
  // test fullscreen transition
  static bool BValidRenderStateChange(ERenderTarget eCurrentRenderTarget,
                                      ERenderTarget eRequestedRenderTarget);
  static bool BIsRenderingToTexture(ERenderTarget eTarget) {
    return (eTarget == k_ERenderToOverlayTexture ||
            eTarget == k_ERenderToOverlaySharedTexture ||
            eTarget == k_ERenderToOpenVROverlay);
  }
  ...
  static bool BIsRenderingToLegacyVR(ERenderTarget eTarget) {
    return (eTarget == k_ERenderToLegacyVR);
  }
  static bool BIsRenderingToOpenVROverlay(ERenderTarget eTarget) {
    return (eTarget == k_ERenderToOpenVROverlay);
  }
  static bool BIsRenderingToFullScreen(ERenderTarget eTarget) {
    return (eTarget == k_ERenderFullScreen);
  }

public:
  virtual ~IUIEngine() {}

  // Starts up subsystems and makes engine ready for use, you need to call
  // RegisterNamedLocalPath() for at least the <config> path first!
#if defined(SOURCE2_PANORAMA)
  virtual bool StartupSubsystems(IUISettings *psettings,
                                 PlatWindow_t hWindow) = 0;
#else
  virtual bool StartupSubsystems(IUISettings *psettings,
                                 IHTMLChromeController *pHTMLController) = 0;
#endif
  ...
  // Run will run frames and block, letting the UI engine control the whole
  // frame loop.
  virtual void Run() = 0;

  // Run frame will run a single frame, letting you wrap the UI engine in an
  // external frame loop
  virtual void RunFrame() = 0;

  // Will set the UI engine to aggressively limit frame rate it runs at to avoid
  // resource usage
...
#if !defined(SOURCE2_PANORAMA)
  virtual IUIWindow *CreateNewWindow(const char *pchWindowTitle, uint32 width,
                                     uint32 height, ERenderTarget eRenderType,
                                     bool bFixedSurfaceSize,
                                     bool bEnforceWindowAspectRatio,
                                     bool bUseCustomMouseCursor,
                                     const char *pchMonitorName) = 0;
  virtual IUIWindow *
  CreateNewOverlayWindow(const char *pchWindowTitle, uint32 width,
                         uint32 height,
                         panorama::IUIEngine::ERenderTarget eTarget,
                         bool bFixedSize, bool bDrawCustomMouseCursor) = 0;
  virtual IUIWindow *
  CreateNewOpenVROverlayWindow(uint32 width, uint32 height,
                               vr::VROverlayHandle_t ulOverlayHandle) = 0;
#else
  virtual IUIWindow *
  CreateNewUILayerWindow(uint32 xPos, uint32 yPos, uint32 width, uint32 height,
                         bool bFixedSurfaceSize, bool bEnforceWindowAspectRatio,
                         bool bUseCustomMouseCursor, bool bAcceptKBandMouse,
                         const char *pName) = 0;
#endif

  virtual IUITextLayout *
  CreateTextLayout(const char *pchText, const char *pchFontName, float flSize,
                   float flLineHeight, EFontWeight weight, EFontStyle style,
                   ETextAlign align, bool bWrap, bool bEllipsis,
                   int nLetterSpacing, float flMaxWidth, float flMaxHeight) = 0;
  virtual IUITextLayout *
  CreateTextLayout(const wchar_t *pwchText, const char *pchFontName,
                   float flSize, float flLineHeight, EFontWeight weight,
                   EFontStyle style, ETextAlign align, bool bWrap,
                   bool bEllipsis, int nLetterSpacing, float flMaxWidth,
                   float flMaxHeight) = 0;
  ...
  typedef CUtlDelegate<void(const panorama::IUIPanel *, IUIPanel *pOldParent)>
      PanelDestroyedDel_t;
  ...
  // Event management
  virtual void RegisterEventHandler(CPanoramaSymbol symMsg, IUIPanel *pPanel,
                                    CUtlAbstractDelegate pFunc) = 0;
  virtual void UnregisterEventHandler(CPanoramaSymbol symMsg, IUIPanel *pPanel,
                                      CUtlAbstractDelegate pFunc) = 0;
  virtual void RegisterEventHandler(CPanoramaSymbol symMsg,
                                    IUIPanelClient *pPanel,
                                    CUtlAbstractDelegate pFunc) = 0;
  virtual void UnregisterEventHandler(CPanoramaSymbol symMsg,
                                      IUIPanelClient *pPanel,
                                      CUtlAbstractDelegate pFunc) = 0;
  virtual void UnregisterEventHandlersForPanel(IUIPanel *pPanel) = 0;
  virtual void RegisterForUnhandledEvent(CPanoramaSymbol symMsg,
                                         CUtlAbstractDelegate pFunc) = 0;
  virtual void UnregisterForUnhandledEvent(CPanoramaSymbol symMsg,
                                           CUtlAbstractDelegate pFunc) = 0;
  virtual bool
  BHaveEventHandlersRegisteredForType(CPanoramaSymbol symPanelType) = 0;
  virtual void RegisterPanelTypeEventHandler(
      CPanoramaSymbol symMsg, CPanoramaSymbol symPanelType,
      CUtlAbstractDelegate pFunc, bool bThisPtrIsUIPanel = false) = 0;
  virtual bool DispatchEvent(IUIEvent *pEvent) = 0;
  virtual void DispatchEventAsync(float flDelay, IUIEvent *pEvent) = 0;
  virtual bool
  BAnyHandlerRegisteredForEvent(const CPanoramaSymbol &symEvent) = 0;
  ...
  // Register a named local path (executable-relative, readonly on POSIX) for
  // resources, should be something like {images} or {movies}
  virtual void RegisterNamedLocalPath(const char *pathName,
                                      const char *pchLocalPath,
                                      bool bWatchForFileChanges,
                                      bool bAddToOverwriteIfExists = false) = 0;

  // Register a named userdata path (userdata-relative, writable on POSIX) for
  // local read/write storage like {config}
  virtual void RegisterNamedUserPath(const char *pathName,
                                     const char *pchUserPath,
                                     bool bWatchForFileChanges,
                                     bool bAddToOverwriteIfExists = false) = 0;
  ...
  // Get the local path for a named path like {images} or {movies} combined with
  // the relative pathname and also following up the possible overwritten paths
  virtual void GetLocalPathForRelativePath(const char *pchLocalPathName,
                                           const char *pchRelativePathname,
                                           CUtlString &strLocalPath) = 0;

  // Register for a named hostname in http:// paths like {steamcommunity}
  virtual void RegisterNamedRemoteHost(const char *hostName,
                                       const char *pchRemoteHost) = 0;
  ...
  // Register an additional X-Header to be sent when requesting layout from the
  // web
  virtual void RegisterXHeader(const char *pchHeaderName,
  ...
  virtual void GetXHeader(int i, CUtlString &strName,
                          CUtlString &strValue) const = 0;

  // Set the cookie header to use for the named remote host
  virtual void SetCookieHeaderForNamedRemoteHost(
      const char *hostName, const CUtlVector<CUtlString> &vecCookies) = 0;

  // Sets the cookie header to use for the remote host (doesn't require named
  // host use)
  virtual void
  SetCookieHeaderForRemoteHost(const char *hostName,
                               const CUtlVector<CUtlString> &vecCookies) = 0;

  // Get the cookie header string that should be used for the remote host
  virtual const CUtlVector<CUtlString> &
  GetCookieHeadersForNamedRemoteHost(const char *namedRemoteHost) = 0;

  // Get the cookie header string that should be used for the remote host
  virtual const CUtlVector<CUtlString> &
  GetCookieHeadersForRemoteHost(const char *hostName) = 0;

  // Gets the value of a cookie. Returns false if the cookie does not exist.
  virtual bool GetCookieValueForRemoteHost(const char *hostName,
                                           const char *cookieName,
                                           CUtlString *pstrCookieValue) = 0;
  ...
  virtual bool ShowNativeTopMostMessageBox(const char *pchMsg,
                                           const char *pchTitle,
                                           ENativeMessageBoxType_t eType) = 0;
  ...
  virtual void RegisterMouseCanActivateParent(IUIPanel *pPanel,
                                              const char *pchParent) = 0;
  ...
  // See if any window owned by the ui engine is visible and has focus,
  // sometimes overlay windows have focus but are not visible and you generally
  // don't want to consider them as focused then
  ...
  // Get the focused window, there should really be only one, if some bug allows
  // multiple the first found is returned
  ...
  // Called internally from input layer to update last input time to current
  // frame
  ...
  virtual void TrackOverlayForApp(uint64 gameID, uint64 ulPID,
                                  void *pOverlay) = 0;
  virtual void DeleteOverlayInstanceForApp(uint64 gameID, uint64 dwPID,
                                           void *pOverlay) = 0;
  virtual void *OverlayForApp(uint64 gameID, uint64 ulPID) = 0;

  // GPU Information
  virtual bool BGetGPUInformation(char *rgchGPUDesc, uint32 unGPUDescBytes,
                                  uint64 *pulDedicatedGPUMem,
                                  uint64 *pulDedicatedSystemMem,
                                  uint64 *pulSharedMem) = 0;

  // Pool allocations for panel styles
  virtual IUIPanelStyle *AllocPanelStyle(IUIPanel *pStyle,
                                         float flUIScaleFactor) = 0;
  ...
  virtual void
  PulseActiveControllerHaptic(EHapticFeedbackPosition ePosition,
                              EHapticFeedbackStrength eStrength) = 0;
  ...
  virtual void RunScript(IUIPanel *pPanelContext, const char *pchScriptString,
                         const char *pchSourceFilename, int nSourceBeginLine,
                         int nSourceBeginCol, bool bPrintRetValue) = 0;

  // Expose a new object type/template to javascript with the given name,
  // the function pointer passed should setup member accssors/methods with the
  // functions from uijsregistration.h
  virtual void ExposeObjectTypeToJavaScript(const char *pchObjectTypeName,
                                            CUtlAbstractDelegate &del) = 0;

  // Expose an instance of an object type as a global with specified name to
  // javascript
  virtual void ExposeGlobalObjectToJavaScript(const char *pchJSVarName,
                                              void *pInstance,
                                              const char *pchJsTypeName,
                                              bool bTrueGlobal = false) = 0;

  virtual void ClearGlobalObjectForJavaScript(const char *pchJSVarName,
                                              void *pInstance) = 0;

  virtual void DeleteJSObjectInstance(IUIJSObject *pInstance) = 0;

  // Get panel that contains the javascript context
  virtual panorama::IUIPanel *
  GetPanelForJavaScriptContext(v8::Context *pContext) = 0;

  // Get javascript context for a panel (may be NULL)
  virtual v8::Persistent<v8::Context> *
  GetJavaScriptContextForPanel(panorama::IUIPanel *pPanel) = 0;

  // Helper to spew exceptions to console
  virtual void OutputJSExceptionToConsole(v8::TryCatch &try_catch,
                                          IUIPanel *pPanelContext) = 0;

  // Add a function template to global namespace, by default this is really
  // panorama., but you can specify to make it really global as well
  virtual void
  AddGlobalV8FunctionTemplate(const char *pchJSFuncName,
                              v8::Handle<v8::FunctionTemplate> *pFunc,
                              bool bTrueGlobal = false) = 0;
  ...
  virtual v8::Handle<v8::ObjectTemplate>
  GetCurrentV8ObjectTemplateToSetup() = 0;
  ...
  virtual v8::Persistent<v8::Object> *
  CreateV8PanelInstance(IUIPanel *pPanel) = 0;

  // Helper to create a JS object to wrap a given panel style
  virtual v8::Persistent<v8::Object> *
  CreateV8PanelStyleInstance(IUIPanelStyle *pPanelStyle) = 0;

  // Helper to create JS object for given js object type
  virtual v8::Persistent<v8::Object> *
  CreateV8ObjectInstance(const char *pchObjectType, void *pActualObject,
                         IUIJSObject *pJSObject) = 0;

  // Create JSON web api job, use the helpers in uiwebapiclient.h directly
  // instead, this is there for them to use internally
  virtual uint32
  InitiateAsyncJSONWebAPIRequest(EHTTPMethod eMethod, CUtlString strURL,
                                 IUIPanel *pCallbackTargetPanel, void *pContext,
                                 CJSONWebAPIParams *pParams = NULL,
  ...
  virtual uint32
  InitiateAsyncJSONWebAPIRequest(EHTTPMethod eMethod, CUtlString strURL,
                                 JSONWebAPIDelegate_t callback, void *pContext,
                                 CJSONWebAPIParams *pParams = NULL,
                                 HTTPCookieContainerHandle hCookieContainer =
                                     INVALID_HTTPCOOKIE_HANDLE) = 0;
  ...
  // Resolves a path that may contain named path portions, etc, to a full local
  // path
  ...
  virtual void RegisterEventWithEngine(CPanoramaSymbol symEvent,
                                       UIEventFactory factory) = 0;
  ...
  virtual bool IsValidPanelEvent(const CPanoramaSymbol symEvent,
                                 int *pParams) = 0;

  // Create input event from symbol, internal framework use
  virtual IUIEvent *CreateInputEventFromSymbol(CPanoramaSymbol symEvent,
                                               IUIPanel *pPanel,
                                               EPanelEventSource_t eSource,
                                               int nRepeats) = 0;

  // Create an event from a string representation
  virtual IUIEvent *CreateEventFromString(IUIPanel *pCreatingPanel,
                                          const char *pchEvent,
                                          const char **pchEventEnd) = 0;

  // Used internally by initialization code to register panels with framework
  virtual void RegisterPanelFactoryWithEngine(CPanoramaSymbol symPanelType,
                                              CPanel2DFactory *pFactory) = 0;
  ...
  virtual IUIPanelClient *CreatePanel(CPanoramaSymbol symName,
                                      const char *pchID,
                                      panorama::IUIPanel *parent) = 0;
  ...
  // Register any delegate to run at specified time, be sure to use
  // CancelScheduledDelgate if you delete the object the delgate runs on, etc.
  virtual int RegisterScheduledDelegate(double flTargetFrameTime,
                                        CUtlDelegate<void()> del) = 0;

  // Cancel a scheduled delegate by index returned from
  // RegisterScheduledDelegate
  ...
  // Interface to allow animation/render threads to queue a decrement of a ref
  // count on an object next frame in the main thread
  ...
  virtual JSGenericCallbackHandle_t
  RegisterJSGenericCallback(panorama::IUIPanel *pContextPanel,
                            v8::Handle<v8::Function> callbackFunc) = 0;

  // Invoke a callback previously registered with the system. Returns false if
  // the handle has expired (the context panel is gone, or it was explicitly
  // unregistered)
  virtual bool
  InvokeJSGenericCallback(JSGenericCallbackHandle_t nHandle, int nArgs = 0,
                          v8::Handle<v8::Value> *pArgs = NULL,
                          v8::Handle<v8::Value> *pOutRetVal = NULL) = 0;

  // Explicitly remove the callback from the system (future invokes on the
  // handle will do nothing and return false)
  virtual void
  UnregisterJSGenericCallback(JSGenericCallbackHandle_t nHandle) = 0;
  ...
  virtual void GetRegisterJSScopeInfo(int nScope,
                                      RegisterJSScopeInfo_t *pInfo) = 0;

  // Return information on a specific entry in a JS registration scope,
  // such as a method in a class scope.
  virtual void GetRegisterJSEntryInfo(int nScope, int nEntry,
                                      RegisterJSEntryInfo_t *pInfo) = 0;

  // Open a new scope for JS registrations.
  virtual int StartRegisterJSScope(const char *pName,
                                   const char *pDesc = NULL) = 0;
  ...
  // this will return -1.
  virtual int
  NewRegisterJSEntry(const char *pName, uint32 unFlags,
                     const char *pDesc = NULL,
                     RegisterJSType_t eDataType = k_ERegisterJSTypeUnknown) = 0;
  ...
  virtual void SetRegisterJSEntryParams(int nEntry, uint8 unNumParams,
                                        RegisterJSType_t *pParamTypes) = 0;

  // Invalidate cached copies of all layout/style/script files (used eg. by the
  // game when search paths change) (Does NOT rebuild or reload any existing UI,
  // just causes subsequent references to the files to load from scratch.)
  virtual void ClearFileCache() = 0;

  // Spew the current list of all cached files and their refcounts (for
  // debugging)
  ...
  virtual void IncrementPaintCountForPanel(uint64 ulPanelPtrValue,
                                           bool bRequiredCompositionLayer,
                                           double flFrameTime) = 0;

  // Get panel paint info for the panel
  virtual void GetPanelPaintInfo(uint64 ulPanelPtrValue, uint32 &unPaintCount,
                                 bool &bRequiredCompositionLayer,
                                 double &flFrameTimeLastPaint) = 0;
...
//			change is being requested, and it is valid. Helper for
//fullscreen switching.
//-----------------------------------------------------------------------------
inline bool
IUIEngine::BValidRenderStateChange(ERenderTarget eCurrentRenderTarget,
                                   ERenderTarget eRequestedRenderTarget) {
  if (eCurrentRenderTarget == eRequestedRenderTarget)
    return false;

  // if we are in a mode that can switch...
  if (eCurrentRenderTarget == IUIEngine::k_ERenderFullScreen ||
      eCurrentRenderTarget == IUIEngine::k_ERenderToWindow) {
    switch (eRequestedRenderTarget) // test if request is for a state that can
                                    // be switched to
    {
...
inline IUIInput *UIInputEngine() {
  return UIEngine() ? UIEngine()->UIInputEngine() : NULL;
}
#else
IUIEngine *UIEngine();
...
// To expose a generic object via UIEngine()->ExposeObjectTypeToJavaScript() you
// should derive from this interface the GetTypeName member should return the
// same type name you register as exposed
class IUIJSObject {
public:
  ~IUIJSObject() { UIEngine()->DeleteJSObjectInstance(this); }
