...
public:
  ...
  uint32 GetCount() { return m_pTListFreeObjects->Count(); }
...
#else
      pObject->elem =
          (PB_OBJECT_TYPE *)PvAllocNoLeakTracking(sizeof(PB_OBJECT_TYPE));
#endif
  ...
  inline void Free(void *pObjectVoid) { Free((Node *)pObjectVoid); }
  ...
  void Free(Node *pObject) {
    ...
    if (m_unTargetCountHigh > 0) {
      // We always cache for re-use on free, though we may throw out later on
      // alloc to shrink the pool
      pObject->elem->Clear();
...
public:
  ...
  // Has to be public, so we can use in memory pool, should always already be at
  // zero ref count though
  ~CMsgLiteRefCount() { DbgAssert(0 == m_cRef); }

  inline int AddRef() { return ThreadInterlockedIncrement(&m_cRef); }

  int Release();

  inline google::protobuf::MessageLite *AccessMsg() {
    return (google::protobuf::MessageLite
                *)(((CTSList<void *>::Node_t *)m_pTSListNode)->elem);
  }
...
//-----------------------------------------------------------------------------
// CProtoBufMsgMemoryPoolMgr - Manages all the message pools for render messages
// proto buf objects. Should have one global singleton instance of this which
// tracks all the pools for individual message types.
//-----------------------------------------------------------------------------
...
public:
  ...
  ~CUIProtoBufMsgMemoryPoolMgr() {
    CTSList<CMsgLiteRefCount *>::Node_t *pNode =
        m_pTSListMsgLiteRefCount->Pop();
    while (pNode) {
      delete pNode->elem;
      delete pNode;
    ...
    FOR_EACH_VEC(m_vecMsgPools, i) { delete m_vecMsgPools[i]; }
  ...
  void DumpPoolInfo() {
    ...
    FOR_EACH_VEC(m_vecMsgPools, i) {
      ...
      Msg("%43s%12d%12s%12s\n", m_vecMsgPools[i]->GetName().String(),
          m_vecMsgPools[i]->GetCount(),
          V_pretifymem((float)unEstimate, 2, true),
  ...
  void Validate(CValidator &validator, const char *pchName) {
    ...
    FOR_EACH_VEC(m_vecMsgPools, i) { ValidatePtr(m_vecMsgPools[i]); }
    ...
    FOR_EACH_VEC(vecTemp, i) { m_pTSListMsgLiteRefCount->Push(vecTemp[i]); }
  }
#endif // DBGFLAG_VALIDATE
...
public:
  ...
  static void OneTimeInit() {
    ...
    if (!s_bRegisteredWithMemoryPoolMgr) {
      ...
      s_Mutex.Lock();
      if (!s_bRegisteredWithMemoryPoolMgr) {
        s_pMemoryPool = new CUIProtoBufMsgMemoryPool<PB_OBJECT_TYPE>(
            PBMEM_POOL_LOW_TARGET, PBMEM_POOL_HIGH_TARGET);
  ...
  CUIProtoBufMsg() {
    OneTimeInit();
    CTSList<CMsgLiteRefCount *>::Node_t *pRefCountNode =
        UIEngine()->MsgMemoryPoolMgr()->AllocMsgLiteRef();
  ...
  // Expose memory pool for direct allocation of underlying PB msg objects
  static void FreeProtoBufMsgObject(Node *pMsg) { s_pMemoryPool->Free(pMsg); }
  ...
  // Destructor
  virtual ~CUIProtoBufMsg() { CleanupAllocations(); }
  ...
  inline void SerializeInProc(CUtlBuffer *pBuffer) const {
    ...
    pBuffer->EnsureCapacity(pBuffer->TellPut() + sizeof(uint32) +
                            sizeof(uint64));
  ...
  void SerializeCrossProc(CUtlBuffer *pBuffer) const {
    VPROF_BUDGET("CUIProtoBufMsg::SerializeCrossProc",
                 VPROF_BUDGETGROUP_TENFOOT);
    uint32 unSize = m_pMsgRefCount->AccessMsg()->ByteSize();

    // Ensure enough for type, size, and serialized data
    pBuffer->EnsureCapacity(
        pBuffer->TellPut() + sizeof(uint32) * 3 +
        unSize); // bugbug cboyd - drop to * 2 whenpassthrough is removed below
  ...
  bool BDeserializeCrossProc(CUtlBuffer *pBuffer) {
    ...
    bool bSucccess = m_pMsgRefCount->AccessMsg()->ParseFromArray(
        (uint8 *)pBuffer->Base() + pBuffer->TellGet(), unSize);
  ...
  // Accessors
  PB_OBJECT_TYPE &Body() {
    return *((PB_OBJECT_TYPE *)(m_pMsgRefCount->AccessMsg()));
  }
  const PB_OBJECT_TYPE &BodyConst() const {
    return *((const PB_OBJECT_TYPE *)(m_pMsgRefCount->AccessMsg()));
  }
...
// Statics
template <typename PB_OBJECT_TYPE>
bool CUIProtoBufMsg<PB_OBJECT_TYPE>::s_bRegisteredWithMemoryPoolMgr = false;
template <typename PB_OBJECT_TYPE>
CThreadMutex CUIProtoBufMsg<PB_OBJECT_TYPE>::s_Mutex;
template <typename PB_OBJECT_TYPE>
CUIProtoBufMsgMemoryPool<PB_OBJECT_TYPE>
