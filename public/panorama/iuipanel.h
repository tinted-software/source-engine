...
struct DebugPropertyOutput_t {
  ...
  DebugPropertyOutput_t(const char *pchName, const char *pchValue)
      : m_strName(pchName), m_strValue(pchValue) {}
...
//-----------------------------------------------------------------------------
// Purpose: Basic panel interface exposing operations used inside of panorama,
// rather than operations that are part of building/laying out controls in the
// panorama_client module
//-----------------------------------------------------------------------------
class IUIPanel {
public:
  virtual ~IUIPanel() {}
  ...
  // Initialize panel
  virtual void Initialize(IUIWindow *window, IUIPanel *parent,
                          const char *pchID, uint32 ePanelFlags) = 0;
  ...
  virtual void
  RegisterEventHandlersOnPanel2DType(CPanoramaSymbol symPanelType) = 0;

  // Shutdown panel, should only happen right before actual deletion
  virtual void Shutdown() = 0;

  // Fire panel loaded event and mark us as now loaded, images and other panels
  // may call this late
  ...
  virtual bool BLoadLayout(const char *pchFile, bool bOverrideExisting = false,
                           bool bPartialLayout = false) = 0;

  // sets & loads the layout for this panel
  virtual bool BLoadLayoutFromString(const char *pchXMLString,
                                     bool bOverrideExisting = false,
                                     bool bPartialLayout = false) = 0;

  // sets loads the layout file for this panel, asynchronously supporting remote
  // http:// paths
  virtual void LoadLayoutAsync(const char *pchFile,
                               bool bOverrideExisting = false,
                               bool bPartialLayout = false) = 0;

  // loads the layout file for this panel, asynchronously supporting remote
  // http:// paths in css within
  virtual void LoadLayoutFromStringAsync(const char *pchXMLString,
  ...
  // Is the panel transparent (may be 'visible' and affect layout, but fully
  // transparent)
  virtual bool BIsTransparent() const = 0;

  // Set the layout file this panel is loaded from based on a parent, shouldn't
  // need this directly normally
  ...
  // Returns layout file that loaded us which may have been loaded by an
  // ancestor rather than direct parent
  virtual panorama::CPanoramaSymbol GetLayoutFileLoadedFrom() const = 0;

  // Get the current reload count for the panel's layout file - can be used to
  // detect when a reload has occurred Returns -1 if no layout file.
  ...
  virtual bool
  BAppyLayoutFile(panorama::CLayoutFile *pLayoutFile,
                  CUtlVector<panorama::IUIPanel *> *pvecExistingPanels) = 0;
  ...
  // Returns potentially dirty styles data, use for setting during initial
  // construction/setup - or if you want to explicitly ignore transitions, but
  // never for getting and generally not for setting!
  virtual panorama::IUIPanelStyle *AccessIUIStyleDirty() = 0;

  // Apply styles on the panel, resolving them all fully and updating actual
  // panel style
  virtual void ApplyStyles(bool bTraverse) = 0;

  // Set that we want an on styles changed event when styles are applied even if
  // styles aren't actually dirty
  ...
  virtual void DesiredLayoutSizeTraverse(float flMaxWidth,
                                         float flMaxHeight) = 0;
  virtual void DesiredLayoutSizeTraverse(float *pflDesiredWidth,
                                         float *pflDesiredHeight,
                                         float flMaxWidth, float flMaxHeight,
                                         bool bFinalDimensions) = 0;
  ...
  virtual void LayoutTraverse(float x, float y, float flFinalWidth,
                              float flFinalHeight) = 0;

  // Tell us how to actually layout children
  virtual void OnLayoutTraverse(float flFinalWidth, float flFinalHeight) = 0;

  // called by flowing and custom layout passes to set position. Will properly
  // handle transition being applied with styles
  virtual void SetPositionFromLayoutTraverse(CUILength x, CUILength y,
                                             CUILength z) = 0;
  ...
  virtual void RemoveClasses(const CPanoramaSymbol *pSymbols,
                             uint cSymbols) = 0;
  ...
  virtual void SwitchClass(const char *pchAttribute,
                           CPanoramaSymbol symName) = 0;
  ...
  virtual bool BParsePanelEvent(CPanoramaSymbol symPanelEvent,
  ...
  // Explicit call to paint the panel and all it's children, normally called
  // internally by window
  ...
  // Tab index and selection position backing values rather than interpreted
  // (ie. will return k_flSelectionPosAuto rather than the auto-calculated
  // value)
  ...
  // Tell the panel to set focus to the next panel in specified movement/order
  // method
  virtual bool SetFocusToNextPanel(int nRepeats, EFocusMoveDirection moveType,
                                   bool bAllowWrap, float flTabIndexCurrent,
                                   float flXPosCurrent, float flYPosCurrent,
                                   float flXStart, float flYStart) = 0;
  virtual bool SetInputFocusToFirstOrLastChildInFocusOrder(
      EFocusMoveDirection moveType, float flXStart, float flYStart) = 0;
  ...
  virtual void ScrollParentToMakePanelFit(
      ScrollBehavior_t behavior = SCROLL_BEHAVIOR_DEFAULT,
      bool bImmediateScroll = false) = 0;
  virtual void
  ScrollToFitRegion(float x0, float x1, float y0, float y1,
                    ScrollBehavior_t behavior = SCROLL_BEHAVIOR_DEFAULT,
  ...
  virtual void SetSendChildScrolledIntoViewEvents(
      bool bSendChildReadyEvents) = 0; // this must be enabled on your parent
                                       // for ScrolledIntoView events to fire
  ...
  virtual void
  SortChildren(int(__cdecl *pfnCompare)(const ClientPanelPtr_t *,
                                        const ClientPanelPtr_t *)) = 0;
  ...
  virtual int
  AddChildSorted(bool(__cdecl *pfnLessFunc)(ClientPanelPtr_t const &p1,
  ...
  virtual void SetMouseCanActivate(EMouseCanActivate eMouseCanActivate,
                                   const char *pchOptionalParent = NULL) = 0;
  ...
  virtual bool
  BBuildMatchingStyleList(CUtlVector<CascadeStyleFileInfo_t> *pvecStyles) = 0;

  // Getter for panel attributes
  virtual int GetAttribute(const char *pchAttrName, int nDefaultValue) = 0;

  // Getter for panel attributes
  virtual const char *GetAttribute(const char *pchAttrName,
                                   const char *pchDefaultValue) = 0;

  // Getter for panel attributes
  virtual uint32 GetAttribute(const char *pchAttrName,
                              uint32 unDefaultValue) = 0;

  // Getter for panel attributes
  virtual uint64 GetAttribute(const char *pchAttrName,
                              uint64 unDefaultValue) = 0;
  ...
  virtual void SetAnimation(const char *pchAnimationName, float flDuration,
                            float flDelay, EAnimationTimingFunction eTimingFunc,
                            EAnimationDirection eDirection,
                            float flIterations) = 0;

  // Force an immediate update of the visibility list on our window for our
  // current visibility
  ...
  virtual void
  GetDebugPropertyInfo(CUtlVector<DebugPropertyOutput_t *> *pvecProperties) = 0;

  // Populates a vector with all immediate children matching a class
  virtual void
  FindChildrenWithClass(const char *pchClass,
                        CUtlVector<IUIPanel *> &vecMatchingChildren) = 0;

  // Populates a vector with all children matching a class
  virtual void FindChildrenWithClassTraverse(
      const char *pchClass, CUtlVector<IUIPanel *> &vecMatchingChildren) = 0;
  ...
  // panel events
  virtual void SetPanelEvent(CPanoramaSymbol symPanelEvent,
                             IUIEvent *pEvent) = 0;
  virtual void SetPanelEvent(CPanoramaSymbol symPanelEvent,
                             VecUIEvents_t *pEvents) = 0;
  ...
  // Should only be called in the very limited cases where we are creating
  // scrollbars within a layout pass, exposed for HTML but should really only be
  // used inside UIPanel normally
  ...
  // Callback that styles have cleaned up some transitions, we should update
  // cached state about what styles are present
