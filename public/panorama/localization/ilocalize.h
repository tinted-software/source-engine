...
typedef const char *(*PFNLocalizeDialogVariableHandler)(
    const CUtlString &sStringValue, int nIntValue, const IUIPanel *pPanel,
    const char *pszKey, void *pUserData);
...
enum EStringTruncationStyle {
  ...
  k_eStringTruncationStyle_Rear,  // prevent any chars being added above max
                                  // length
...
public:
  CLocStringSafePointer() { m_pString = NULL; }
  ...
  operator const ILocalizationString *() const { return Get(); }

  ~CLocStringSafePointer() { Clear(); }
...
public:
...
#if defined(SOURCE2_PANORAMA)
  // add a loc file to the system, in the form of <prefix>_<language>.txt , i.e
  // dota_french.txt, the files will be loaded from the panorama/localization
  // folder of your mod
  ...
  virtual void InstallCustomDialogVariableHandler(
      const char *pchCustomHandlerName,
      PFNLocalizeDialogVariableHandler pfnLocalizeFunc,
      void *pUserData = NULL) = 0;
  virtual void
  RemoveCustomDialogVariableHandler(const char *pchCustomHandlerName) = 0;

  // find the string corresponding to this localization token, or if we don't
  // find it then just return back the string wrapped in a loc object
  virtual const ILocalizationString *
  PchFindToken(const IUIPanel *pPanel, const char *pchToken, const uint32 ccMax,
               EStringTruncationStyle eTrunkStyle,
               EStringTransformStyle eTransformStyle,
               bool bAllowDialogVariable = false) = 0;

  // give me a localize string wrapper around this string, don't try and apply
  // token localizing on it though, but do optionally allow it to have dialog
  // variables that we parse in it be careful allowing dialog variable parsing,
  // you want to sanitize any user input before allowing it
  virtual const ILocalizationString *
  PchSetString(const IUIPanel *pPanel, const char *pchText, const uint32 ccMax,
               EStringTruncationStyle eTrunkStyle,
               EStringTransformStyle eTransformStyle, bool bAllowDialogVariable,
               bool bStringAlreadyFullyParsed) = 0;

  virtual const ILocalizationString *
  ChangeTransformStyleAndRelease(const ILocalizationString *pLocalizationString,
                                 EStringTransformStyle eTranformStyle) = 0;

  // copy an existing loc string without altering the ref count on the current
  virtual ILocalizationString *
  CloneString(const IUIPanel *pPanel, const ILocalizationString *pLocToken) = 0;

  // return the raw, un-parsed, value for this loc token, returns NULL if we
  // didn't have this token in a loc file from disk
  virtual const char *PchFindRawString(const char *pchToken) = 0;

  virtual bool SetDialogVariable(const IUIPanel *pPanel, const char *pchKey,
                                 const char *pchValue) = 0;
#if defined(SOURCE2_PANORAMA)
  virtual bool SetDialogVariable(const IUIPanel *pPanel, const char *pchKey,
                                 time_t timeVal) = 0;
  virtual bool SetDialogVariable(const IUIPanel *pPanel, const char *pchKey,
                                 CCurrencyAmount amount) = 0;
#else
  virtual bool SetDialogVariable(const IUIPanel *pPanel, const char *pchKey,
                                 CRTime timeVal) = 0;
  virtual bool SetDialogVariable(const IUIPanel *pPanel, const char *pchKey,
  ...
  virtual void CloneDialogVariables(const IUIPanel *pPanelFrom,
                                    IUIPanel *pPanelTo) = 0;

  // force a re-evaluation of a specific dialog variable
  virtual void DirtyDialogVariable(const IUIPanel *pPanel,
                                   const char *pchKey) = 0;

  // given this loc string find the longest string in any language that we could
  // display here and update to use it
  virtual void
  SetLongestStringForToken(const ILocalizationString *pLocalizationString,
                           ILocalizationStringSizeResolver *pResolver) = 0;
};
...
public:
  CMutableLocalizationString() : m_bMutable(false), m_pString(NULL) {}

  CMutableLocalizationString(ILocalizationString *pString)
      : m_bMutable(true), m_pString(pString) {}
  ...
  CMutableLocalizationString(const CMutableLocalizationString &other) {
    m_bMutable = true;
    if (other.m_pString) {
      m_pString = UILocalize()->CloneString(other.m_pString->GetOwningPanel(),
                                            other.m_pString);
    } else {
      m_pString = NULL;
    }
  ...
  ILocalizationString *GetMutable() {
    ...
    m_pString =
        UILocalize()->CloneString(pOldString->GetOwningPanel(), pOldString);
    m_bMutable = true;
  ...
  // The -> operator is only overloaded to return a const string. If you need a
  // mutable version, you must call GetMutable directly.
  const ILocalizationString *operator->() const { return Get(); }
  ...
  operator=(const CMutableLocalizationString &other) {
