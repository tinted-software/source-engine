...
#define REFERENCE_PANEL_CORE(className, layoutName)                            \
  __pragma(comment(linker, "/INCLUDE:?g_" #layoutName                          \
                           "LinkerHack@panorama@@3PAV" #className "@1@A"));
#elif defined(PLATFORM_WINDOWS_PC64) || defined(_WIN64)
#define REFERENCE_PANEL_CORE(className, layoutName)                            \
  __pragma(comment(linker, "/INCLUDE:?g_" #layoutName                          \
                           "LinkerHack@panorama@@3PEAV" #className "@1@EA"));
#else
#define REFERENCE_PANEL_CORE(className, layoutName)                            \
  class className;                                                             \
  extern className *g_##layoutName##LinkerHack;                                \
  className *g_##layoutName##PullInModule SELECTANY =                          \
      g_##layoutName##LinkerHack;
#endif

#define REFERENCE_PANEL(name) REFERENCE_PANEL_CORE(C##name, name)
