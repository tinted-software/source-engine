...
public:
  ...
  // Final step of initialization, post constructor, and post
  // BInitializeSurface() on individidual window type
  virtual bool FinishInitialization();

  // Run any per window frame func logic
  virtual void RunPlatformFrame();

  // Set scaling factor that applies to all x/y values in the UI for the window,
  // used so we can author content at say 1080p but pass 0.6666666f for this to
  // render in 720p on cards with poor fill rates or TVs without 1080p support.
  ...
  virtual void GetWindowBounds(float &left, float &top, float &right,
                               float &bottom) = 0;
  ...
  virtual IUIRenderEngine *UIRenderEngine() OVERRIDE {
    return (IUIRenderEngine *)m_pRenderEngine;
  }
  CUIRenderEngine *GetUIRenderEngine() { return m_pRenderEngine; }
  virtual bool BIsVisible() { return true; }
  virtual bool BIsOverlay() OVERRIDE {
    return UIEngine()->BIsOverlayTarget(m_eRenderTarget);
  }
  virtual bool BIsSteamWMOverlay() OVERRIDE {
    return m_eRenderTarget == IUIEngine::k_ERenderToOverlaySteamWM;
  }

  virtual bool BIsFullscreen() {
    return IUIEngine::BIsRenderingToFullScreen(m_eRenderTarget);
  }
  virtual bool BIsFullscreenBorderlessWindow() {
    return m_eRenderTarget == IUIEngine::k_ERenderBorderlessFullScreenWindow;
  }
  ...
  virtual void GetSessionFPSAverages(float &fpsPaint, float &fpsAnimation,
                                     float &fpsRender) OVERRIDE;
  ...
  // Paint an empty frame so the animation/render threads will run but do
  // nothing but LRU/clear data
  ...
  virtual IUIWindowInput *UIWindowInput() OVERRIDE {
    return (IUIWindowInput *)m_pInputEngine;
  }

  // custom mouse cursor support, returns true if we want our manually drawn
  // one, false for OS specific ones
  ...
  CImageResourceManager *AccessImageManager() {
    return m_pImageResourceManager;
  }

  // Set a context ptr that is attached to the window, just lets other code
  // (panels) that has access to the window access some shared state across the
  // window.
  ...
  virtual uint32 GetNumVisibleTopLevelPanels() const OVERRIDE {
    return (uint32)m_listVisiblePanels.Count();
  }

  virtual const CUtlLinkedList<IUIPanel *> &
  GetTopLevelVisiblePanels() const OVERRIDE {
    return (CUtlLinkedList<IUIPanel *> &)m_listVisiblePanels;
  }
  ...
  // Set a min FPS for the window, this actually just prevents setting the max
  // lower
  ...
  // metrics
  virtual void RecordDaisyWheelUsage(float flEntryTimeInSeconds,
                                     int nWordsEntered, bool bViaKeyboard,
                                     bool bViaGamepad) OVERRIDE;
  virtual void GetDaisyWheelWPM(int &nWordsTyped, float &flMixedWPM,
                                float &flKeyboardOnlyWPM,
                                float &flGamepadOnlyWPM) OVERRIDE;
  ...
  // Access overlay window interface for this window, NULL on non Steam Overlay
  // windows
  virtual IUIOverlayWindow *GetOverlayInterface() OVERRIDE { return NULL; }

  virtual void SetFocusBehavior(EWindowFocusBehavior eFocusBehavior) OVERRIDE {
    m_eFocusBehavior = eFocusBehavior;
  }
  virtual EWindowFocusBehavior GetFocusBehavior() OVERRIDE {
    return m_eFocusBehavior;
  }
  ...
  // Clears the GPU resources associated with the window before the next render
  // frame
...
protected:
  ...
  bool m_bUseCustomMouseCursor; // true if we draw our tenfoot cursors and not
                                // the win32 ones for this panel
  bool m_bCursorWasVisibleLastFrame;
  EMouseCursors m_eCursorCurrent; // current cursor to draw
  CMouseCursorTexture
      *m_pMouseCursor; // contains the image data for the cursors we can display
