...
//-----------------------------------------------------------------------------
// Purpose: Client side of panel interface, this is what client code
// implementing controls must provide to the core UI engine in terms of a
// callback interface
//-----------------------------------------------------------------------------
...
public:
  ...
  // override to change how the panel lays out it's children (instead of default
  // css positioning/flow-children support)
  virtual void OnLayoutTraverse(float flFinalWidth, float flFinalHeight) = 0;

  // Direct key/input handling... better to use binds and events and to not
  // directly override these in your panel in nearly all cases.
  ...
  virtual bool
  BSetProperties(const CUtlVector<ParsedPanelProperty_t> &vecProperties) = 0;

  // Override to handle new properties from xml on your panel
  virtual bool BSetProperty(CPanoramaSymbol symName, const char *pchValue) = 0;

  // Should the panel event be applied only after all children are created as
  // well?
  ...
  // Callback which allows panels to override focus movement behavior, this is
  // called before the base framework behavior is executed and returning true
  // will prevent the base behavior from occuring.
  virtual bool OnSetFocusToNextPanel(int nRepeats,
                                     panorama::EFocusMoveDirection moveType,
                                     bool bAllowWrap, float flTabIndexCurrent,
                                     float flXPosCurrent, float flYPosCurrent,
                                     float flXStart, float fYStart) = 0;

  // Allows overriding the parent who will be considered by the localization
  // system for dialog variables, dropdowns do this to allow the dropdown to
  // have dialog vars set that then impact the created popup menu for the actual
  // list... You should almost never need this... try to avoid!
  virtual IUIPanel *GetLocalizationParent() const = 0;

  // Allows panels that know they may draw outside their bounds to tell us to
  // create a clip layer, we don't always do this since it can be expensive
  virtual bool BRequiresContentClipLayer() = 0;

  // Callback you must first register for with
  // UIEngine()->CallBeforeStyleAndLayout() if you need it
  virtual void OnCallBeforeStyleAndLayout() = 0;

  // bugbug jmccaskey - DELETE ME
  // Callback when a panel event gets set, DO NOT USE, DELETE THIS AS SOON AS WE
  // CAN, ITS HARMFUL
  ...
  // Callback when UI scale factor changes, which may affect panel contents
  // sizing/layout
  ...
  CreateNewVerticalScrollBar(float flInitialScrollPos) = 0;

  // Callback to client panel to create a scrollbar
  virtual IUIScrollBar *
  CreateNewHorizontalScrollBar(float flInitialScrollPos) = 0;
