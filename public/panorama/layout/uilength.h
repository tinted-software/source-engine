...
// Sometimes we want to say "infinite width/height", but we can't use FLT_MAX as
// it's "auto"
...
public:
  ...
  CUILength(float flValue, EUILengthTypes eType)
      : m_flValue(flValue), m_eType(eType) {}
  ...
  bool IsFillParentFlow() const {
    return (m_eType == k_EUILengthFillParentFlow);
  }
  bool IsHeightPercentage() const {
    return (m_eType == k_EUILengthHeightPercentage);
  }
  bool IsWidthPercentage() const {
    return (m_eType == k_EUILengthWidthPercentage);
  }
  ...
  void SetFillParentFlow(float flWeight) {
    Set(flWeight, k_EUILengthFillParentFlow);
  }
  void SetParentFlow(float flValue) { Set(flValue, k_EUILengthFillParentFlow); }
  void SetHeightPercentage(float flValue) {
    Set(flValue, k_EUILengthHeightPercentage);
  }
  void SetWidthPercentage(float flValue) {
    Set(flValue, k_EUILengthWidthPercentage);
  }
  ...
  void ScaleLengthValue(float flScaleFactor) {
    if (m_eType == k_EUILengthLength && m_flValue != k_flFloatAuto &&
        m_flValue != k_flFloatNotSet) {
      // Don't make width/height values of 1.0 shrink, as things like 1 px
      // horizontal rules will go invisible!  Kind of hacky, but this mostly
      // works without issue as we have so few things < 1px.
      if (fabsf(1.0f - fabsf(m_flValue)) < 0.1f && flScaleFactor < 1.0f) {
        // Don't need to adjust
      } else {
        // If the original value was pixel aligned, try to keep that.  This is
        // important so text doesn't accumulate error layers down from us and
        // thus get blurry.  Can't apply this rule to all values though as
        // things like a shadow offset of 5.5 can be common and the half pixel
        // can be important for the shadow to balance on both sides of the
        // layer.
...
CUILength LerpUILength(float flProgress, CUILength start, CUILength end,
                       float flPixelSize);
