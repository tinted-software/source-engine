...
public:
  ...
  EVerticalAlignment GetVeriticalAlignment() const {
  ...
  bool IsVerticalSet() const {
    return (m_eVerticalAlignment != k_EVerticalAlignmentUnset &&
            m_vertical.IsSet());
  }

  bool IsSet() const { return (IsHorizontalSet() && IsVerticalSet()); }

  void Set(EHorizontalAlignment eHorizontal, const CUILength &horizontal,
           EVerticalAlignment eVertical, const CUILength &vertical);
...
public:
  CBackgroundRepeat() {
    Set(k_EBackgroundRepeatUnset, k_EBackgroundRepeatUnset);
  }
  ~CBackgroundRepeat() {}

  bool IsSet() const {
    return (m_eHorizontal != k_EBackgroundRepeatUnset &&
            m_eVertical != k_EBackgroundRepeatUnset);
  }
...
//-----------------------------------------------------------------------------
// Purpose: Can have multiple background image layers, each contains the
// following
//-----------------------------------------------------------------------------
class CBackgroundImageLayer {
private:
  enum EImagePath { k_EImagePathUnset, k_EImagePathNone, k_EImagePathSet };
...
public:
  ...
  void SetPosition(const CBackgroundPosition &position) {
    m_position = position;
  }
  ...
  EBackgroundSizeConstant GetBackgroundSizeConstant() {
    return m_eBackgroundSizeConstant;
  }
  ...
  bool IsCompletelyUnset() {
    return (m_eImagePath == k_EImagePathUnset && !m_position.IsSet() &&
            !m_width.IsSet() && !m_height.IsSet() && !m_repeat.IsSet());
  }
  ...
  bool operator==(const CBackgroundImageLayer &rhs) const {
    return (m_eImagePath == rhs.m_eImagePath && m_sURLPath == rhs.m_sURLPath &&
            m_position == rhs.m_position && m_width == rhs.m_width &&
            m_height == rhs.m_height && m_repeat == rhs.m_repeat &&
  ...
  // helpers for drawing
  void CalculateFinalDimensions(float *pflWidth, float *pflHeight,
                                float flPanelWidth, float flPanelHeight,
                                float flScaleFactor);
  void CalculateFinalPosition(float *px, float *py, float flWidthPanel,
                              float flHeightPanel, float flWidthImage,
                              float flHeightImage);
  void CalculateFinalSpacing(float *px, float *py, float flWidthPanel,
                             float flHeightPanel, float flWidthImage,
                             float flHeightImage);
