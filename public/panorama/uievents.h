...
DECLARE_PANORAMA_EVENT1(
    TopLevelWindowClose,
    IUIWindow *); // fired when top level window is destructing while all
                  // children are still valid
DECLARE_PANORAMA_EVENT1(TopLevelWindowClosed,
                        IUIWindow *); // fired after top level window has
                                      // already destroyed all children
...
DECLARE_PANORAMA_EVENT0(
    None); // short circuited in the bind code not to fire an event
...
void OnActiveControllerTypeChangedDefaultHandler(
    IUIPanel *pPanel, EActiveControllerType eActiveControllerType);
...
// Request from some UI to show a URL in the systems browser, may be hooked and
// handled differently in different applications, not handled by default in
// panorama itself
DECLARE_PANEL_EVENT1(BrowserGoToURL, const char *);

DECLARE_PANORAMA_EVENT0(
    AsyncPanoramaSurfaceLost); // 3d surface detected that its output became
                               // unavailable, currently fired by Linux on VTT
                               // switch
DECLARE_PANORAMA_EVENT0(
    AsyncPanoramaSurfaceReturned); // 3d surface detected that its previously
                                   // lost surface is now renderable again but
                                   // will need a full reload

// When text input handler is coming up or down, bool = true when showing, =
// false for hiding
...
DECLARE_PANORAMA_EVENT2(OverlayGamepadInputMsg, panorama::IUIWindow *,
                        InputMessage_t *)
...
DECLARE_PANEL_EVENT1(
    DragStart,
    IUIPanel **); // first event for drag sent to panel user started dragging on
                  // (must be marked with draggable="true")
DECLARE_PANEL_EVENT1(
    DragEnter, panorama::CPanelPtr<panorama::IUIPanel>); // sent to panel drag
                                                         // is hovering over
DECLARE_PANEL_EVENT1(
    DragDrop,
    panorama::CPanelPtr<panorama::IUIPanel>); // sent to panel where user
                                              // released mouse while dragging
DECLARE_PANEL_EVENT1(
    DragLeave,
    panorama::CPanelPtr<panorama::IUIPanel>); // sent to panel drag stopped
                                              // hovering over
DECLARE_PANEL_EVENT1(
    DragEnd,
    panorama::CPanelPtr<panorama::IUIPanel>); // sent to panel which received
                                              // DragStart after Drop event was
                                              // sent
