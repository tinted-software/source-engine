...
#define SPROP_NOSCALE                                                          \
  (1 << 2) // For floating point, don't scale into range, just take value as is.
...
#define SPROP_PROXY_ALWAYS_YES                                                 \
  (1 << 9) // Set for datatable props using one of the default datatable proxies
           // like SendProxy_DataTableToDataTable that always send the data to
           // all clients.

#define SPROP_CHANGES_OFTEN                                                    \
  (1 << 10) // this is an often changed field, moved to head of sendtable so it
            // gets a small index
...
#define SPROP_COLLAPSIBLE                                                      \
  (1 << 12) // Set automatically if it's a datatable with an offset of 0 that
            ...
            // SendPropDataTable altogether and spare the trouble of walking the
            // hierarchy more than necessary.

#define SPROP_COORD_MP                                                         \
  (1 << 13) // Like SPROP_COORD, but special handling for multiplayer games
#define SPROP_COORD_MP_LOWPRECISION                                            \
  (1 << 14) // Like SPROP_COORD, but special handling for multiplayer games
...
#define SPROP_VARINT                                                           \
  SPROP_NORMAL // reuse existing flag so we don't break demo. note you want to
...
#define EXTERN_SEND_TABLE(tableName)                                           \
  ...
  extern int g_SendTableInit;                                                  \
  }
#define EXTERN_RECV_TABLE(tableName)                                           \
  namespace tableName {                                                        \
  extern RecvTable g_RecvTable;                                                \
  extern int g_RecvTableInit;                                                  \
  }

// MoeMod: ODR Use it to prevent being dropped by linker
#define REFERENCE_SEND_TABLE(tableName)                                        \
  (tableName::g_SendTableInit + &tableName::g_SendTableInit,                   \
   tableName::g_SendTable)
#define REFERENCE_RECV_TABLE(tableName)                                        \
  (tableName::g_RecvTableInit + &tableName::g_RecvTableInit,                   \
   tableName::g_RecvTable)
