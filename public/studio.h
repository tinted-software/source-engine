...
// If you want to embed a pointer into one of the structures that is serialized,
// use this class! It will ensure that the pointers consume the right amount of
// space and work correctly across 32 and 64 bit. It also makes sure that there
// is no surprise about how large the structure is when placed in the middle of
// another structure, and supports Intel's desired behavior on 64-bit that
// pointers are always 8-byte aligned.
#pragma pack(push, 4)
template <class T> struct ALIGN4 serializedstudioptr_t {
  T *m_pData;
...
struct mstudioanimdesc_t {
  ...
  mstudioanim_t *
  pAnim(int *piFrame,
        float &flStall) const; // returns pointer to data and new frame index
  mstudioanim_t *
  pAnim(int *piFrame) const; // returns pointer to data and new frame index
...
struct mstudioseqdesc_t {
  ...
  float *pPoseKey(int iParam, int iAnim) const {
    return (float *)(((byte *)this) + posekeyindex) + iParam * groupsize[0] +
           iAnim;
  }
...
enum FlexControllerRemapType_t {
  ...
  FLEXCONTROLLER_REMAP_2WAY, // Control 0 -> ramps from 1-0 from 0->0.5. Control
                             // 1 -> ramps from 0-1 from 0.5->1
...
struct mstudio_modelvertexdata_t {
  ...
  const void *GetVertexData() const { return pVertexData; }
  const void *GetTangentData() const { return pTangentData; }
};
...
#ifdef PLATFORM_64BITS
    return *(const mstudio_modelvertexdata_t **)((byte *)this +
                                                 index_ptr_modelvertexdata);
#else
...
struct mstudiomodel_t {
  ...
  mstudio_modelvertexdata_t
      vertexdata; // sizeof(mstudio_modelvertexdata_t) == 16
...
inline mstudioboneweight_t *
mstudio_modelvertexdata_t::BoneWeights(int i) const {
...
mstudiomesh_t::GetVertexData(void *pModelData) {
  ...
  vertexdata.index_ptr_modelvertexdata =
      (byte *)&real_modelvertexdata - (byte *)&vertexdata;
#else
...
inline const thinModelVertices_t *
mstudiomesh_t::GetThinVertexData(void *pModelData) {
...
inline const mstudio_modelvertexdata_t *
mstudiomodel_t::GetVertexData(void *pModelData) {
...
// used)
inline const thinModelVertices_t *
mstudiomodel_t::GetThinVertexData(void *pModelData) {
  const vertexFileHeader_t *pVertexHdr = CacheVertexData(pModelData);
  if (!pVertexHdr)
    return NULL;
...
struct studiohdr_t {
  ...
  inline mstudiobonecontroller_t *pBonecontroller(int i) const {
    ...
    return (mstudiobonecontroller_t *)(((byte *)this) + bonecontrollerindex) +
           i;
  };
  ...
  inline mstudioflexcontroller_t *
  pFlexcontroller(LocalFlexController_t i) const {
    Assert(numflexcontrollers == 0 || (i >= 0 && i < numflexcontrollers));
    return (mstudioflexcontroller_t *)(((byte *)this) + flexcontrollerindex) +
           i;
  };
  ...
  //lod. 	numAllowedRootLODs = N	means that lod0 - lod(N-1) can be set as root
  //lod, but not lodN or lower.
  ...
  const mstudiosrcbonetransform_t *SrcBoneTransform(int i) const {
    ...
    return (mstudiosrcbonetransform_t *)(((byte *)this) +
  ...
  inline const mstudioboneflexdriver_t *BoneFlexDriver(int i) const {
    ...
    return studiohdr2index > 0 ? pStudioHdr2()->pBoneFlexDriver(i) : NULL;
  }
...
public:
  ...
  public:
    ...
    struct HashValueType {
      ...
      HashValueType(int _actIdx, int _stIdx, int _ct, int _tW)
          : activityIdx(_actIdx), startingIdx(_stIdx), count(_ct),
            totalWeight(_tW) {}
    ...
    CActivityToSequenceMapping(void)
        : m_pSequenceTuples(NULL), m_iSequenceTuplesCount(0),
          m_ActToSeqHash(8, 0, 0), m_expectedPStudioHdr(NULL),
  ...
  inline int SelectWeightedSequenceFromModifiers(int activity,
                                                 CUtlSymbol *pActivityModifiers,
                                                 int iModifierCount) {
#if STUDIO_SEQUENCE_ACTIVITY_LAZY_INITIALIZE
    // We lazy-initialize the header on demand here, because CStudioHdr::Init()
...
//			**weights - destination for weights array starting at
//index i.
// Output : int
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
inline int
mstudioflexcontrollerui_t::controllerIndex(const CStudioHdr &cStudioHdr) const {
...
inline int
mstudioflexcontrollerui_t::rightIndex(const CStudioHdr &cStudioHdr) const {
...
inline int
mstudioflexcontrollerui_t::leftIndex(const CStudioHdr &cStudioHdr) const {
...
inline int
mstudioflexcontrollerui_t::nWayValueIndex(const CStudioHdr &cStudioHdr) const {
  return remaptype == FLEXCONTROLLER_REMAP_NWAY
...
inline const mstudioflexcontroller_t *
mstudioflexcontrollerui_t::pController(int index) const {
...
#define STUDIO_NWAY                                                            \
  17 // Fetch a value from a 2 Way slider for the 2nd value RemapVal( 0.5, 1.0,
     // 0.0, 1.0 )
#define STUDIO_COMBO                                                           \
  18 // Perform a combo operation (essentially multiply the last N values on the
     // stack)
...
inline bool Studio_ConvertStudioHdrToNewVersion(studiohdr_t *pStudioHdr) {
  COMPILE_TIME_ASSERT(
      STUDIO_VERSION ==
      49); //  put this to make sure this code is updated upon changing version.
...
inline int Studio_LoadVertexes(const vertexFileHeader_t *pTempVvdHdr,
  ...
  target = 0;
  pFixupTable =
      (vertexFileFixup_t *)((byte *)pTempVvdHdr + pTempVvdHdr->fixupTableStart);
  for (i = 0; i < pTempVvdHdr->numFixups; i++) {
    ...
    memcpy((byte *)((mstudiovertex_t *)((byte *)pNewVvdHdr +
                                        pNewVvdHdr->vertexDataStart) +
                    target),
           (byte *)((mstudiovertex_t *)((byte *)pTempVvdHdr +
    ...
    if (bNeedsTangentS) {
      // copy tangents
      memcpy((byte *)((Vector4D *)((byte *)pNewVvdHdr +
                                   pNewVvdHdr->tangentDataStart) +
                      target),
             (byte *)((Vector4D *)((byte *)pTempVvdHdr +
