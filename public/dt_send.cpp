...
void SendProxy_UInt64ToInt64(const SendProp *pProp, const void *pStruct,
                             const void *pData, DVariant *pOut, int iElement,
                             int objectID);
#endif
...
SendProp
SendPropFloat(const char *pVarName,
              // Variable name.
...
SendProp SendPropString(const char *pVarName, int offset, int bufferLen,
  ...
  Assert(bufferLen <= DT_MAX_STRING_BUFFERSIZE); // You can only have strings
                                                 // with 8-bits worth of length.
