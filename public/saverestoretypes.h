...
struct entitytable_t {
  ...
  Vector landmarkModelSpace; // a fixed position in model space for comparison
                             ...
                             // coordiante systems
                             //		in different levels, so this fixes up
                             //local quantities to match 		those differences.
  string_t modelname;
...
#define FENTTABLE_PLAYERCHILD                                                  \
  0x08000000 // this entity is connected to a player, so it must be spawned with
             // it
...
private:
  ...
  CBaseEntity *m_pCurrentEntity; // only valid during the save functions of this
...
CSaveRestoreSegment::FindCreateSymbol(const char *pszToken) {
  unsigned short hash =
...
/// XXX(JohnS): I'm not sure using an intrinsic has any value here, just doing
/// the shift should be recognized by most
