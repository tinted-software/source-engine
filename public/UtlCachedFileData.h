...
public:
  CUtlCachedFileData(
      ...
      : m_Elements(0, 0, FileNameHandleLessFunc),
        m_sRepositoryFileName(repositoryFileName), m_nVersion(version),
        m_pfnMetaChecksum(checksumfunc), m_bDirty(false), m_bInitialized(false),
        m_uCurrentMetaChecksum(0u), m_fileCheckType(fileCheckType),
        m_bNeverCheckDisk(nevercheckdisk), m_bReadOnly(readonly),
  ...
  // If bExpectMissing is set, don't complain if this causes synchronous disk
  // I/O to build the cache.
...
private:
  ...
  struct ElementType_t {
    ElementType_t()
        : handle(0), fileinfo(0),
          diskfileinfo(UTL_CACHED_FILE_DATA_UNDEFINED_DISKINFO), dataIndex(-1) {
    }
...
template <class T> T *CUtlCachedFileData<T>::Get(char const *filename) {
...
template <class T> T *CUtlCachedFileData<T>::operator[](int i) {
  return m_Data[m_Elements[i].dataIndex];
}

template <class T> const T *CUtlCachedFileData<T>::operator[](int i) const {
  return m_Data[m_Elements[i].dataIndex];
}

template <class T> int CUtlCachedFileData<T>::Count() const {
  return m_Elements.Count();
}

template <class T> void CUtlCachedFileData<T>::Reload() {
  Shutdown();
  Init();
}

template <class T> bool CUtlCachedFileData<T>::IsUpToDate() {
...
template <class T> bool CUtlCachedFileData<T>::Init() {
  if (m_bInitialized) {
    return true;
  }
...
template <class T> void CUtlCachedFileData<T>::Save() {
  char path[512];
...
template <class T> void CUtlCachedFileData<T>::Shutdown() {
  if (!m_bInitialized)
    return;
...
template <class T> bool CUtlCachedFileData<T>::ManifestExists() {
...
template <class T> void CUtlCachedFileData<T>::SaveManifest() {
...
template <class T> T *CUtlCachedFileData<T>::RebuildItem(const char *filename) {
  int idx = GetIndex(filename);
  ElementType_t &e = m_Elements[idx];
...
template <class T> void CUtlCachedFileData<T>::ForceRecheckDiskInfo() {
  for (int i = m_Elements.FirstInorder(); i != m_Elements.InvalidIndex();
       i = m_Elements.NextInorder(i)) {
    ElementType_t &element = m_Elements[i];
