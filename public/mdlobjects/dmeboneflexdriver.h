...
public:
  // Sets the bone component to be in the range [STUDIO_BONE_FLEX_TX,
  // STUDIO_BONE_FLEX_RZ]
  int SetBoneComponent(int nBoneComponent);

  CDmaString m_sFlexControllerName; // Name of flex controller to drive
  CDmaVar<int> m_nBoneComponent; // Component of bone to drive flex controller,
                                 // StudioBoneFlexComponent_t
  CDmaVar<float>
      m_flMin; // Min value of bone component mapped to 0 on flex controller
  CDmaVar<float> m_flMax; // Max value of bone component mapped to 1 on flex
                          // controller (inches if T, degress if R)
};
...
public:
  ...
  CDmaElementArray<CDmeBoneFlexDriverControl>
      m_eControlList; // List of flex controllers to drive
};
