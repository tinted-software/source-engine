...
//-----------------------------------------------------------------------------
// Purpose: Sends a command to the editor (if running) to begin an editing
// session
//			of a given map.
// Input  : pszMapName - Name of the bsp, without the path or extension:
// "c1a3a_port"
//			nMapVersion - Map version number, from the BSP file.
// Output : Returns Editor_OK on success, an error code if the session was
// rejected.
//-----------------------------------------------------------------------------
EditorSendResult_t Editor_BeginSession(const char *pszMapName, int nMapVersion,
                                       bool bShowUI) {
  char szCommand[MAX_COMMAND_BUFFER];
  Q_snprintf(szCommand, sizeof(szCommand), "session_begin %s %d", pszMapName,
             nMapVersion);
...
EditorSendResult_t Editor_CheckVersion(const char *pszMapName, int nMapVersion,
                                       bool bShowUI) {
  char szCommand[MAX_COMMAND_BUFFER];
  Q_snprintf(szCommand, sizeof(szCommand), "map_check_version %s %d",
             pszMapName, nMapVersion);
  return (Editor_SendCommand(szCommand, bShowUI));
}
...
EditorSendResult_t Editor_CreateEntity(const char *pszEntity, float x, float y,
                                       float z, bool bShowUI) {
  char szCommand[MAX_COMMAND_BUFFER];
  Q_snprintf(szCommand, sizeof(szCommand), "entity_create %s %g %g %g",
             pszEntity, x, y, z);
  return (Editor_SendCommand(szCommand, bShowUI));
}
...
EditorSendResult_t Editor_CreateNode(const char *pszNodeClass, int nID, float x,
                                     float y, float z, bool bShowUI) {
  char szCommand[MAX_COMMAND_BUFFER];
  Q_snprintf(szCommand, sizeof(szCommand), "node_create %s %d %g %g %g",
             pszNodeClass, nID, x, y, z);
  return (Editor_SendCommand(szCommand, bShowUI));
}
...
EditorSendResult_t Editor_CreateNodeLink(int nStartID, int nEndID,
                                         bool bShowUI) {
  char szCommand[MAX_COMMAND_BUFFER];
  Q_snprintf(szCommand, sizeof(szCommand), "nodelink_create %d %d", nStartID,
             nEndID);
  return (Editor_SendCommand(szCommand, bShowUI));
}
...
EditorSendResult_t Editor_DeleteEntity(const char *pszEntity, float x, float y,
                                       float z, bool bShowUI) {
  char szCommand[MAX_COMMAND_BUFFER];
  Q_snprintf(szCommand, sizeof(szCommand), "entity_delete %s %g %g %g",
             pszEntity, x, y, z);
  return (Editor_SendCommand(szCommand, bShowUI));
}
...
EditorSendResult_t Editor_SetKeyValue(const char *pszEntity, float x, float y,
  ...
  Q_snprintf(szCommand, sizeof(szCommand),
             "entity_set_keyvalue %s %f %f %f \"%s\" \"%s\"", pszEntity, x, y,
             z, pKey, pValue);
  return (Editor_SendCommand(szCommand, bShowUI));
}
...
EditorSendResult_t Editor_RotateEntity(const char *pszEntity, float x, float y,
  ...
  char szCommand[MAX_COMMAND_BUFFER];
  Q_snprintf(szCommand, sizeof(szCommand),
             "entity_rotate_incremental %s %f %f %f %f %f %f", pszEntity, x, y,
             z, incrementalRotation.x, incrementalRotation.y,
...
//			nEndID - unique ID of the other node that the link is
//connected to.
// Output : Returns Editor_OK on success, an error code on failure.
//-----------------------------------------------------------------------------
EditorSendResult_t Editor_DeleteNodeLink(int nStartID, int nEndID,
                                         bool bShowUI) {
  char szCommand[MAX_COMMAND_BUFFER];
  Q_snprintf(szCommand, sizeof(szCommand), "nodelink_delete %d %d", nStartID,
             nEndID);
  return (Editor_SendCommand(szCommand, bShowUI));
}
...
//				Editor_OK - The command was executed
//successfully. 				Editor_NotRunning - Unable to establish a communications channel
//with the editor. 				Editor_BadCommand - The editor did not accept the command.
//-----------------------------------------------------------------------------
EditorSendResult_t Editor_SendCommand(const char *pszCommand, bool bShowUI) {
#ifdef _WIN32
  HWND hwnd =
      FindWindow("Worldcraft_ShellMessageWnd", "Worldcraft_ShellMessageWnd");
  if (hwnd != NULL) {
    //
    ...
    if (!SendMessage(hwnd, WM_COPYDATA, 0, (LPARAM)&CopyData)) {
      if (bShowUI) {
        ...
        Q_snprintf(szError, sizeof(szError),
                   ...
                   "sure the command is valid and that Worldcraft is still "
                   "running properly.",
                   pszCommand);
