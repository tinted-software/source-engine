...
bool IsPointInBox(const Vector &pt, const Vector &boxMin,
  ...
  if (IsX360()) {
    return IsPointInBox(LoadUnaligned3SIMD(pt.Base()),
                        LoadUnaligned3SIMD(boxMin.Base()),
...
bool FASTCALL IsBoxIntersectingRay(const Vector &boxMin, const Vector &boxMax,
  ...
  if (IsX360()) {
    ...
    return IsBoxIntersectingRay(
        ...
        LoadUnaligned3SIMD(origin.Base()), delta,
        ReciprocalSIMD(delta),   // ray parameters
...
#if defined(_X360)
  if (IsX360()) {
    return IsBoxIntersectingRay(LoadUnaligned3SIMD(vecBoxMin.Base()),
                                LoadUnaligned3SIMD(vecBoxMax.Base()), ray,
                                flTolerance);
  }
#endif
...
#ifdef _X360
bool FASTCALL
IsBoxIntersectingRay(fltx4 boxMin, fltx4 boxMax, fltx4 origin, fltx4 delta,
...
QuadBarycentricRetval_t
PointInQuadToBarycentric(const Vector &v1, const Vector &v2, const Vector &v3,
                         const Vector &v4, const Vector &point, Vector2D &uv) {
...
bool OBBHasFullyContainedIntersectionWithQuad(
  ...
  for (int i = 0; i != 8; ++i) {
    if (iSides[i] == 2) // point behind the plane
    {
      ...
      iAxisCrossings[1] =
          ((i + 1) & 3) +
          (i & 4); // cycle to the next element while staying within the upper 4
                   // or lower 4, this will cross either vOBBExtent2 or
                   // vOBBExtent3 axis, we don't care which
      iAxisCrossings[2] =
          ((i - 1) & 3) +
          (i & 4); // cylce to the previous element while staying within the
                   // upper 4 or lower 4, this will cross the axis
  ...
  return true; // there were lines crossing the quad plane, and every line
               // crossing that plane had its intersection with the plane within
               // the quad's boundaries
}
...
bool RayHasFullyContainedIntersectionWithQuad(
  ...
  Vector ptPlaneIntersections[(
      12 + 12 +
      8)]; // absolute max possible: 12 lines to connect the start box, 12 more
           // to connect the end box, 8 to connect the boxes to eachother
      ...
      for (i = 0; i != 2; ++i) {
        if (fDists[i] <= 0.0f) {
          ...
          if (fDists[j] >= 0.0f) {
            float fInvTotalDist =
                1.0f /
                (fDists[j] - fDists[i]); // fDists[i] <= 0, ray is swept so no
                                         // chance that the denom was 0
      ...
      if (iSideMask[k] == 3) // box is split by the plane
      {
        for (int i = 0; i != 8; ++i) {
          if (iSides[k][i] == 2) // point behind the plane
          {
            ...
            iAxisCrossings[1] =
                ((i + 1) & 3) +
                (i & 4); // cycle to the next element while staying within the
                         // upper 4 or lower 4, this will cross either Y or Z
                         // axis, we don't care which
            iAxisCrossings[2] =
                ((i - 1) & 3) +
                (i & 4); // cylce to the previous element while staying within
                         // the upper 4 or lower 4, this will cross the axis
                         // iAxisCrossings[1] didn't cross
                  ...
                  1) // point in front of the plane
              {
                ...
                float fInvTotalDist =
                    1.0f / (fDists[k][iAxisCrossings[j]] -
                            fDists[k][i]); // remember that fDists[k][i] is a
                                           // negative value
                ptPlaneIntersections[iPlaneIntersectionsCount] =
        ...
        if (iSides[0][i] != iSides[1][i]) {
          ...
          float fInvTotalDist =
              1.0f / (fDists[iPosSide][i] -
                      fDists[iNegSide][i]); // remember that fDists[iNegSide][i]
                                            // is a negative value
          ptPlaneIntersections[iPlaneIntersectionsCount] =
