...
enum ESteamNetworkingConfigurationValue {
  ...
  /// Minimum number of lifetime pings we need to send, before we think our
  /// estimate is solid.  The first ping to each cluster is very often delayed
  /// because of NAT, routers not having the best route, etc.  Until we've sent
  /// a sufficient number of pings, our estimate is often inaccurate.  Keep
  /// pinging until we get this many pings.
  ...
  // know our SteamID or can't get a cert.  On the server, it means that we
  // won't automatically reject a connection due to a failure to authenticate.
...
enum ESteamNetworkingConfigurationString {
  // Code of relay cluster to use.  If not empty, we will only use relays in
  // that cluster.  E.g. 'iad'
  ...
  k_ESteamNetworkingConfigurationString_Count =
      k_ESteamNetworkingConfigurationString_ClientForceProxyAddr + 1,
};
...
enum ESteamNetworkingSendType {
  ...
  // if there is no NAT-traversal accomplished or there is a recognized
...
enum ESteamNetworkingConnectionState {
  ...
  ///   We have completed a series of basic handshake packets, and the
  ///   connection is ready to be accepted using AcceptConnection.
  ///
  ...
  // API.
  // We document them here to provide a little insight into the state machine
  // that is used
  // under the hood.
  //

  /// We've disconnected on our side, and from an API perspective the connection
  /// is closed. No more data may be sent or received.  All reliable data has
  /// been flushed, or else we've given up and discarded it.  We do not yet know
  /// for sure that the client knows the connection has been closed, however, so
  /// we're just hanging around so that if we do get a packet from them, we can
  /// send them the appropriate packets so that they can know why the connection
  /// was closed (and not have to rely on a timeout, which makes it appear as if
  /// something is wrong).
  ///
  /// Note that, unlike TCP connections, Steam networking sessions have
  /// additional session identifiers that make the TIME_WAIT state unnecessary.
  /// (If another connection is established on the same port, and we get packets
  /// from an old session, we are able to identify this and either ignore those
  /// packets, or inform the remote host that the old session has been closed,
  /// if appropriate).
  k_ESteamNetworkingConnectionState_FinWait = -1,

  /// We've disconnected on our side, and from an API perspective the connection
  /// is closed. No more data may be sent or received.  From a network
  /// perspective, however, on the wire, we have not yet given any indication to
  /// the peer that the connection is closed. We are in the process of flushing
  /// out the last bit of reliable data.  Once that is done, we will inform the
  /// peer that the connection has been closed, and transition to the FinWait
  /// state.
  ///
...
CalculateSteamNetworkingPOPIDFromString(const char *pszCode) {
  ...
  // this nontrivial, and there are already some IDs stored in SQL.  Ug, so the
  // 4 character code "abcd" will be encoded with the digits like "0xddaabbcc"
  return (uint32(pszCode[3]) << 24U) | (uint32(pszCode[0]) << 16U) |
         (uint32(pszCode[1]) << 8U) | uint32(pszCode[2]);
}
...
protected:
  ...
  inline ~ISteamNetworkingMessage() {
  }; // Destructor hidden - use Release()!  But make it inline and empty, in
     // case you want to derive your own type that satisfies this interface for
     // use in your code.
};
...
struct SteamNetConnectionInfo_t {
  ...
  /// Local port that we're bound to for this connection.  Might not be
  /// applicable for all connection types.
