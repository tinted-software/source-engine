...
public:
  ...
  /// That's because this function uses a slightly more complete description
  virtual int EstimatePingTimeFromLocalHost(
      const SteamNetworkPingLocation_t &remoteLocation) = 0;
  ...
  //
  // Low level ticket stuff.  I need to get some advice and talk through how
  // this should work or how best to tuck it away and make it transparent.
  //

  virtual bool
  ReceivedTicket(const void *pvTicket, int cbTicket,
  ...
  /// Fetch directly measured ping time from local host to a particular network
  /// PoP. Most games will not need to call this.
...
/// Get ISteamNetworkingUtils object.  This will eventually go in Steam_api.h
/// with all the rest of its kin
