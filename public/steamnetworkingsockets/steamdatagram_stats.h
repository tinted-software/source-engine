...
STEAMDATAGRAMLIB_INTERFACE void SteamDatagramLinkInstantaneousStats_Clear(
    SteamDatagramLinkInstantaneousStats *pThis);
STEAMDATAGRAMLIB_INTERFACE void
SteamDatagramLinkLifetimeStats_Clear(SteamDatagramLinkLifetimeStats *pThis);
STEAMDATAGRAMLIB_INTERFACE int
SteamDatagramLinkLifetimeStats_PingHistogramTotalCount(
    const SteamDatagramLinkLifetimeStats *pThis);
STEAMDATAGRAMLIB_INTERFACE int
SteamDatagramLinkLifetimeStats_QualityHistogramTotalCount(
    const SteamDatagramLinkLifetimeStats *pThis);
STEAMDATAGRAMLIB_INTERFACE int
SteamDatagramLinkLifetimeStats_JitterHistogramTotalCount(
    const SteamDatagramLinkLifetimeStats *pThis);
STEAMDATAGRAMLIB_INTERFACE int
SteamDatagramLinkLifetimeStats_TXSpeedHistogramTotalCount(
    const SteamDatagramLinkLifetimeStats *pThis);
STEAMDATAGRAMLIB_INTERFACE int
SteamDatagramLinkLifetimeStats_RXSpeedHistogramTotalCount(
    const SteamDatagramLinkLifetimeStats *pThis);
STEAMDATAGRAMLIB_INTERFACE void
SteamDatagramLinkStats_Clear(SteamDatagramLinkStats *pThis);
STEAMDATAGRAMLIB_INTERFACE void SteamNetworkingDetailedConnectionStatus_Clear(
    SteamNetworkingDetailedConnectionStatus *pThis);
STEAMDATAGRAMLIB_INTERFACE int SteamNetworkingDetailedConnectionStatus_Print(
    const SteamNetworkingDetailedConnectionStatus *pThis, char *pszBuf,
    int cbBuf);
...
struct SteamDatagramLinkInstantaneousStats {
  ...
  /// 0...1, estimated number of packets that were sent to us, but we failed to
  /// receive. <0 if we haven't received any sequenced packets and so we don't
  /// have any way to estimate this.
  float m_flPacketsDroppedPct;

  /// Packets received with a sequence number abnormality, other than basic
  /// packet loss.  (Duplicated, out of order, lurch.) <0 if we haven't received
  /// any sequenced packets and so we don't have any way to estimate this.
  float m_flPacketsWeirdSequenceNumberPct;

  /// Peak jitter
  int m_usecMaxJitter;

  /// Current sending rate, this can be low at connection start until the slow
  /// start ramps it up.  It's adjusted as packets are lost and congestion is
  /// encountered during the connection
  int m_nSendRate;

  /// How many pending bytes are waiting to be sent.  This is data that is
  /// currently waiting to be sent and in outgoing buffers.  If this is zero,
  /// then the connection is idle and all pending data has been sent.  Note that
  /// in case of packet loss any pending reliable data might be re-sent.  This
  /// does not include data that has been sent and is waiting for
  /// acknowledgment.
  int m_nPendingBytes;
...
struct SteamDatagramLinkLifetimeStats {
  ...
  int64 m_nBytesSent;
  int64 m_nPacketsRecv; // total number of packets received, some of which might
                        // not have had a sequence number.  Don't use this
                        // number to try to estimate lifetime packet loss, use
                        // m_nPacketsRecvSequenced
  int64 m_nBytesRecv;
  int64 m_nPktsRecvSequenced; // packets that we received that had a sequence
                              // number.
  ...
  inline int PingHistogramTotalCount() const {
    return SteamDatagramLinkLifetimeStats_PingHistogramTotalCount(this);
  }

  // Distribution.
  // NOTE: Some of these might be -1 if we didn't have enough data to make a
  // meaningful estimate! It takes fewer samples to make an estimate of the
  // median than the 98th percentile!
  ...
  //
  int m_nQualityHistogram100; // This means everything was perfect.  If we
                              // delivered over 100 packets in the interval and
                              // were less than perfect, but greater than 99.5%,
                              // we will use 99% instead.
  ...
  int m_nQualityHistogramDead; // we received nothing during the interval; it
                               // looks like the connection dropped
  inline int QualityHistogramTotalCount() const {
    return SteamDatagramLinkLifetimeStats_QualityHistogramTotalCount(this);
  }
  ...
  inline int JitterHistogramTotalCount() const {
    return SteamDatagramLinkLifetimeStats_JitterHistogramTotalCount(this);
  }
  ...
  inline int TXSpeedHistogramTotalCount() const {
    return SteamDatagramLinkLifetimeStats_TXSpeedHistogramTotalCount(this);
  }
  ...
  inline int RXSpeedHistogramTotalCount() const {
    return SteamDatagramLinkLifetimeStats_RXSpeedHistogramTotalCount(this);
  }
...
/// Link stats.  Pretty much everything you might possibly want to know about
/// the connection
...
enum ESteamDatagramAvailability {
  k_ESteamDatagramAvailability_CannotTry =
      -3, // A dependent resource is missing, so this service is unavailable.
          // (E.g. we cannot talk to routers because Internet is down or we
          // don't have the network config.)
  k_ESteamDatagramAvailability_Failed =
      -2, // We have tried for enough time that we would expect to have been
          // successful by now.  We have never been successful
  k_ESteamDatagramAvailability_Previously =
      -1, // We tried and were successful at one time, but now it looks like we
          // have a problem
  k_ESteamDatagramAvailability_Unknown =
      0, // Unknown, or not applicable in this context
  k_ESteamDatagramAvailability_NeverTried =
      1, // We don't know because we haven't ever checked
  k_ESteamDatagramAvailability_Attempting =
      2, // We're trying now, but are not yet successful.  This is not an error,
         // but it's not success, either.
...
struct SteamNetworkingDetailedConnectionStatus {
  ...
  /// Do we have a valid network configuration?  We cannot do anything without
  /// this.
  ESteamDatagramAvailability m_eAvailNetworkConfig;

  //		/// Does it look like we have a connection to the Internet at
  //all? 		EAvailability m_eAvailInternet;
  ...
  inline int Print(char *pszBuf, int cbBuf) const {
    return SteamNetworkingDetailedConnectionStatus_Print(this, pszBuf, cbBuf);
  }
};
