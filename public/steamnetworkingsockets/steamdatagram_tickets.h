...
#pragma pack(8)
    ...
    defined(STEAMDATAGRAMLIB_FOREXPORT)
...
struct SteamDatagramServiceNetID {
  ...
  uint64 ConvertToUint64() const {
    return (uint64(m_unIP) << 16U) | uint64(m_unPort);
  }
  ...
  inline bool operator==(const SteamDatagramServiceNetID &x) const {
    return m_unIP == x.m_unIP && m_unPort == x.m_unPort;
  }
};
...
struct SteamDatagramRelayAuthTicket {
  ...
  struct ExtraField {
    enum EType {
      k_EType_String,
      k_EType_Int, // For most small integral values.  Uses google protobuf
                   // sint64, so it's small on the wire.  WARNING: In some
                   // places this value may be transmitted in JSON, in which
                   // case precision may be lost in backend analytics.  Don't
                   // use this afor an "identifier", use it for a scalar
                   // quantity.
      k_EType_Fixed64, // 64 arbitrary bits.  This value is treated as an
                       // "identifier".  In places where JSON format is used, it
                       // will be serialized as a string.  No aggregation /
                       // analytics can be performed on this value.
    };
    int /* EType */ m_eType;
    char m_szName[28];
...
/// is exposed in both.
STEAMDATAGRAM_TICKET_INTERFACE bool
SteamDatagramRelayAuthTicket_Parse(const void *pvTicket, int cbTicket,
                                   SteamDatagramRelayAuthTicket *pOutTicket,
