...
public:
  ...
  // Returns true if the engine is playing back a "locally recorded" demo, which
  // includes both SourceTV and replay demos, since they're recorded locally (on
  // servers), as opposed to a client recording a demo while connected to a
  // remote server.
...
public:
  ...
  // Called once the client is initialized to setup client-side replay interface
  // pointers
  ...
  // Give the client a chance to modify sound settings however they want before
  // the sound plays. This is used for things like adjusting pitch of voice
  // lines in Pyroland in TF2.
