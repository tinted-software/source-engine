...
enum CompiledVtfFlags {
  ...
  TEXTUREFLAGS_STREAMABLE =
      (TEXTUREFLAGS_STREAMABLE_COARSE | TEXTUREFLAGS_STREAMABLE_FINE)
...
struct VtfProcessingOptions {
  ...
  uint8 numNotDecayMips[4]; // Number of first mips unaffected by decay (0 means
                            // all below mip0)
...
public:
  ...
  virtual void
  SetPostProcessingSettings(VtfProcessingOptions const *pOptions) = 0;

  // Like Unserialize, but allows you to additionally specify some flags to
  // forcibly enable.
  virtual bool UnserializeEx(CUtlBuffer &buf, bool bHeaderOnly = false,
                             int nForceFlags = 0, int nSkipMipLevels = 0) = 0;

  // Data is included in [ finest, coarsest ] mips--other ranges have garbage.
  // This is particularly useful for streaming textures.
...
//	for each animation frame
//		for each face
//			for each mip level (starting with the largest, and
//getting smaller) 				store the image data for the face
//
...
// nillerusr: try to avoid problems with pragma pack, remove c++ inheritance to
// make this structs platform-independent
