...
public:
  ...
  AddGlobalFlexController(char const *controllerName, int globalIndex);
  ...
  CDmeGlobalFlexControllerOperator *
  GetGlobalFlexController(int localIndex); // localIndex is in order of calls to
...
protected:
  ...
  CDmaArray<float> m_flexWeights; // These are global flex weights (so there can
                                  // be gaps, unused indices)
