...
struct skinning_info_t {
  ...
  int index;
  float weight;
  bool operator<(const skinning_info_t &info) { return weight < info.weight; }
};
...
struct vertex_t {
  ...
  bool operator==(const vertex_t &vert) {
    return
        //			skinning == vert.skinning && // TODO - the
        //original studiomdl doesn't do this, but...
        coord == vert.coord && texcoord == vert.texcoord &&
...
private:
  ...
  // Sets up the bones
  void SetUpBones(CDmeTransform *pTransform, int nMaxBoneCount,
