...
public:
  ...
  bool BuildClipStack(DmeClipStack_t *pStack, CDmeClip *pRoot,
                      CDmeClip *pShot = NULL);

  // Clip stack versions of time conversion
  static DmeTime_t ToChildMediaTime(const DmeClipStack_t &stack,
                                    DmeTime_t globalTime, bool bClamp = true);
  static DmeTime_t FromChildMediaTime(const DmeClipStack_t &stack,
                                      DmeTime_t localTime, bool bClamp = true);
  static DmeTime_t ToChildMediaDuration(const DmeClipStack_t &stack,
                                        DmeTime_t globalDuration);
  static DmeTime_t FromChildMediaDuration(const DmeClipStack_t &stack,
                                          DmeTime_t localDuration);
  static void ToChildMediaTime(DmeLog_TimeSelection_t &params,
  ...
  void AddTrackGroupBefore(CDmeTrackGroup *pTrackGroup,
  ...
  // Clip finding
  virtual CDmeTrack *
  FindTrackForClip(CDmeClip *pClip, CDmeTrackGroup **ppTrackGroup = NULL) const;
  bool FindMultiTrackGroupForClip(CDmeClip *pClip, int *pTrackGroupIndex,
                                  int *pTrackIndex = NULL,
                                  int *pClipIndex = NULL) const;

  // Finding clips in tracks by time
  virtual void FindClipsAtTime(DmeClipType_t clipType, DmeTime_t time,
                               DmeClipSkipFlag_t flags,
                               CUtlVector<CDmeClip *> &clips) const;
  virtual void FindClipsWithinTime(DmeClipType_t clipType, DmeTime_t startTime,
...
inline void CDmeClip::SetMute(bool state) { m_bMute = state; }

inline bool CDmeClip::IsMute() const { return m_bMute; }
...
public:
  ...
  CDmeChannel *
  CreatePassThruConnection(char const *passThruName, CDmElement *pFrom,
...
public:
  ...
  enum { MAX_FX_INPUT_TEXTURES = 2 };

  // All effects must be able to apply their effect
  virtual void ApplyEffect(DmeTime_t time, Rect_t &currentRect,
                           Rect_t &totalRect,
                           ITexture *pTextures[MAX_FX_INPUT_TEXTURES]) {}

  // Global list of FX clip types
  static void InstallFXClipType(const char *pElementType,
...
//-----------------------------------------------------------------------------
template <class T> class CDmFXClipFactory : public CDmElementFactory<T> {
public:
...
//-----------------------------------------------------------------------------
// All effects must use IMPLEMENT_FX_CLIP_ELEMENT_FACTORY instead of
...
#define IMPLEMENT_FX_CLIP_ELEMENT_FACTORY(lookupName, className, description)  \
  ...
  CDmFXClipFactory<className> g_##className##_Factory(#lookupName,             \
                                                      description);            \
...
public:
  ...
  void BuildClipAssociations(CUtlVector<ClipAssociation_t> &association,
                             bool bHandleGaps = true);
  ...
  void RollAssociatedClips(CDmeClip *pClip,
                           CUtlVector<ClipAssociation_t> &association,
                           DmeTime_t dt);

  // Shifts associated clips so they remain in the same relative time when pClip
  // is scaled
  void ScaleAssociatedClips(CDmeClip *pClip,
                            CUtlVector<ClipAssociation_t> &association,
                            float ratio, DmeTime_t oldOffset);

private:
  virtual int AllowedClipTypes() const {
    return (1 << DMECLIP_CHANNEL) | (1 << DMECLIP_SOUND) | (1 << DMECLIP_FX) |
           (1 << DMECLIP_FILM);
  }
...
//-----------------------------------------------------------------------------
template <class T> class CDmeClipInfo {
public:
