...
public:
  // NOTE: If you add fields to this, add to g_pStandardFieldNames in
  // dmevertexdata.cpp
  enum StandardFields_t {
    ...
    FIELD_WEIGHT, // Weight is just the different between the base position and
                  // the delta position
  ...
  // Creates a new vertex field. NOTE: This cannot be used to create joint
  // weights + indices
  template <class T> FieldIndex_t CreateField(const char *pFieldName);
  ...
  void CreateJointWeightsAndIndices(int nJointCount,
  ...
  // Sets vertex data
  void SetVertexData(FieldIndex_t nFieldIndex, int nFirstVertex, int nCount,
                     DmAttributeType_t valueType, const void *pData);
  void SetVertexIndices(FieldIndex_t nFieldIndex, int nFirstIndex, int nCount,
                        const int *pIndices);
  ...
  FindVertexIndicesFromDataIndex(FieldIndex_t nFieldIndex, int nDataIndex);
  const CUtlVector<int> &
  FindVertexIndicesFromDataIndex(StandardFields_t nFieldIndex, int nDataIndex);
...
protected:
  ...
  void UpdateStandardFieldInfo(int nFieldIndex, const char *pFieldName,
...
//-----------------------------------------------------------------------------
inline FieldIndex_t
CDmeVertexDataBase::FindFieldIndex(StandardFields_t nFieldIndex) const {
...
//-----------------------------------------------------------------------------
inline int CDmeVertexDataBase::VertexCount() const { return m_nVertexCount; }

//-----------------------------------------------------------------------------
// Returns the number of joints per vertex
//-----------------------------------------------------------------------------
inline int CDmeVertexDataBase::JointCount() const { return m_nJointCount; }
...
//-----------------------------------------------------------------------------
inline CDmAttribute *
CDmeVertexDataBase::GetVertexData(FieldIndex_t nFieldIndex) {
  return m_FieldInfo[nFieldIndex].m_pVertexData;
}

inline const CDmAttribute *
CDmeVertexDataBase::GetVertexData(FieldIndex_t nFieldIndex) const {
  return m_FieldInfo[nFieldIndex].m_pVertexData;
}
...
//-----------------------------------------------------------------------------
inline CDmAttribute *
CDmeVertexDataBase::GetIndexData(FieldIndex_t nFieldIndex) {
  return m_FieldInfo[nFieldIndex].m_pIndexData;
}

inline const CDmAttribute *
CDmeVertexDataBase::GetIndexData(FieldIndex_t nFieldIndex) const {
  return m_FieldInfo[nFieldIndex].m_pIndexData;
}
...
//-----------------------------------------------------------------------------
inline int CDmeVertexDataBase::GetFieldIndex(int nVertexIndex,
                                             StandardFields_t nFieldId) const {
...
public:
  ...
  // NOTE: Pass in negative scales to get 'compression', positive to get
  // 'expansion'
  void GenerateWrinkleDelta(CDmeVertexData *pBindState, float flScale,
                            bool bOverwrite);

  // Computes a float map which is the distance between the base and delta
  // position The maximum distance any vertex is moved is returned
