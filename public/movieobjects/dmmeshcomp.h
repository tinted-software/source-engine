...
public:
  ...
  class CVert {
  public:
    CVert(int nPositionIndex, const CUtlVector<int> *pVertexIndices,
  ...
  protected:
    ...
    const CUtlVector<int> *m_pVertexIndices; // Pointer to a list of the vertex
                                             // indices for this vertex
    const Vector *m_pPosition;
    CUtlVector<CEdge *>
        m_edges; // An array of pointers to the edges containing this vertex
  ...
  public:
    ...
    bool ConnectedTo(const CEdge *pEdge) const {
      return m_pVert0 == pEdge->m_pVert0 || m_pVert0 == pEdge->m_pVert1 ||
             m_pVert1 == pEdge->m_pVert0 || m_pVert1 == pEdge->m_pVert1;
    }

    Vector EdgeVector() const;

    // Returns true if the edge starts and stops at the same point in local
    // space
  ...
  CFace *CreateFace(const CUtlVector<CVert *> &verts,
