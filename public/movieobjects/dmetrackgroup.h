...
public:
  ...
  bool FindTrackForClip(CDmeClip *pClip, int *pTrackIndex,
                        int *pClipIndex = NULL) const;

  // Finding clips at a particular time
  void FindClipsAtTime(DmeClipType_t clipType, DmeTime_t time,
                       DmeClipSkipFlag_t flags,
                       CUtlVector<CDmeClip *> &clips) const;
  void FindClipsIntersectingTime(DmeClipType_t clipType, DmeTime_t startTime,
                                 DmeTime_t endTime, DmeClipSkipFlag_t flags,
                                 CUtlVector<CDmeClip *> &clips) const;
  void FindClipsWithinTime(DmeClipType_t clipType, DmeTime_t startTime,
                           DmeTime_t endTime, DmeClipSkipFlag_t flags,
                           CUtlVector<CDmeClip *> &clips) const;
...
// Inline methods
//-----------------------------------------------------------------------------
inline int CDmeTrackGroup::GetTrackCount() const { return m_Tracks.Count(); }
...
inline bool CDmeTrackGroup::IsVisible() const { return m_bIsVisible; }
...
inline bool CDmeTrackGroup::IsMinimized() const { return m_bMinimized; }
...
inline int CDmeTrackGroup::GetDisplaySize() const { return m_nDisplaySize; }
...
#define DMETRACKGROUP_FOREACH_CLIP_TYPE_START(_clipType, _dmeTrackGroup,       \
                                              _dmeTrack, _dmeClip)             \
  {                                                                            \
    int _tc = (_dmeTrackGroup)->GetTrackCount();                               \
    for (int _i = 0; _i < _tc; ++_i) {                                         \
      ...
      for (int _j = 0; _j < _cc; ++_j) {                                       \
        _clipType *_dmeClip =                                                  \
            static_cast<_clipType *>(_dmeTrack->GetClip(_j));                  \
        if (!_dmeClip)                                                         \
          continue;
