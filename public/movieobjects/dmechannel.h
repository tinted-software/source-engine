...
//
// The channel class - connects elements together, and allows for logging of
// data
//
//=============================================================================
...
public:
  ...
  void StartLayerRecording(const char *pUndoRedoDesc,
                           const DmeLog_TimeSelection_t *pTimeSelection = NULL);
  void FinishLayerRecording(float flThreshold, bool bFlattenLayers = true);

  // Adds a channel to the recording layer
  void AddChannelToRecordingLayer(CDmeChannel *pChannel, CDmeClip *pRoot = NULL,
                                  CDmeClip *pShot = NULL);

  // Used to iterate over all channels currently being recorded
  // NOTE: Use CDmeChannel::AddToRecordingLayer to add a channel to the
  // recording layer
  ...
  void UpdateRecordingTimeSelectionTimes(
  ...
  void SetProceduralTarget(int nProceduralMode,
                           const CUtlVector<KeyValues *> &list);
...
public:
  ...
  void SetCurrentTime(
      DmeTime_t time); // Simple version. Only works if multiple active channels
                       // clips do not reference the same channels
  DmeTime_t GetCurrentTime() const;

  void SetChannelToPlayToSelf(const char *outputAttributeName,
                              float defaultValue, bool force = false);

  // need this until we have the EditApply message queue
  void OnAttributeChanged(CDmAttribute *pAttribute);

  template <class T> bool GetCurrentPlaybackValue(T &value);
  template <class T> bool GetPlaybackValueAtTime(DmeTime_t time, T &value);
  ...
  bool BuildClipStack(DmeClipStack_t *pClipStack, CDmeClip *pRoot,
                      CDmeClip *pShot);
...
// Inline methods
//-----------------------------------------------------------------------------
template <class T> inline CDmeTypedLog<T> *CDmeChannel::CreateLog() {
  return CastElement<CDmeTypedLog<T>>(
      CreateLog(CDmAttributeInfo<T>::AttributeType()));
}
...
template <class T> inline bool CDmeChannel::GetCurrentPlaybackValue(T &value) {
