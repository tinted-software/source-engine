...
public:
  ...
  static bool RemoveFacesWithMaterial(CDmeMesh *pMesh,
                                      const char *pMaterialName);

  static bool RemoveFacesWithMoreThanNVerts(CDmeMesh *pMesh,
  ...
  static bool RemapMaterial(CDmeMesh *pMesh, const CUtlString &src,
                            const CUtlString &dst);

  static bool RemapMaterial(CDmeMesh *pMesh, const int nMaterialIndex,
                            const CUtlString &dst);
  ...
  static bool Merge(CDmeMesh *pSrcMesh, CDmeMesh *pDstMesh,
                    int nSkinningJointIndex);

  static bool CreateDeltasFromPresets(
  ...
  enum WrinkleOp { kReplace, kAdd };

  static bool CreateWrinkleDeltaFromBaseState(CDmeVertexDeltaData *pDelta,
                                              float flScale = 1.0f,
                                              WrinkleOp wrinkleOp = kReplace,
                                              CDmeMesh *pMesh = NULL,
                                              CDmeVertexData *pBind = NULL,
                                              CDmeVertexData *pCurrent = NULL);

  static int FindMergeSocket(
      const CUtlVector<CUtlVector<CDmMeshComp::CEdge *>> &srcBorderEdgesList,
      CDmeMesh *pDstMesh);

  static bool Merge(CDmMeshComp &srcComp,
                    const CUtlVector<CDmMeshComp::CEdge *> &edgeList,
                    CDmeMesh *pDstMesh);

protected:
  static const int *
  BuildDataMirrorMap(CDmeVertexData *pBase, int axis,
                     CDmeVertexData::StandardFields_t standardField,
                     CUtlVector<int> &dataMirrorMap);

  static bool MirrorVertices(CDmeMesh *pMesh, CDmeVertexData *pBase, int axis,
                             CUtlVector<int> &mirrorMap);

  static bool MirrorVertices(CDmeVertexData *pBase, int axis,
  ...
  static void MirrorVertices(CDmeVertexData *pBase, FieldIndex_t fieldIndex,
                             int nOldVertexCount, int nMirrorCount,
                             const CUtlVector<int> &baseIndices,
                             const CUtlVector<int> &mirrorMap);

  static bool MirrorDelta(CDmeVertexDeltaData *pDelta, int axis,
                          const CUtlVector<int> &posMirrorMap,
                          const CUtlVector<int> &normalMirrorMap,
                          const CUtlVector<int> &uvMirrorMap);
  ...
  static void CreateDeltasFromPresetGroup(
  ...
  static void
  PurgeUnreferencedDeltas(CDmeMesh *pMesh,
                          CUtlStringMap<CDmePreset *> &presetMap,
...
public:
  ...
  CDmMeshFaceIt(const CDmeMesh *pMesh,
                const CDmeVertexData *pVertexData = NULL);

  // Resets the iterator to the start of the specified mesh or for another
  // iteration of the current mesh if the specified mesh is NULL
  bool Reset(const CDmeMesh *pMesh = NULL,
             const CDmeVertexData *pVertexData = NULL);
  ...
  // Returns opposite sense of IsDone(), i.e. !IsDone() so true if iterator
  ...
  // Gets the specified vertex data for the vertices of this face (if it exists)
  // and the type matches
  template <class T_t>
  bool GetVertexData(CUtlVector<T_t> &vertexData,
...
protected:
  ...
  int m_nFaceSetCount; // Number of face sets in current mesh:
                       // m_pMesh->FaceSetCount();
  int m_nFaceSetIndex; // Index of current face set: [ 0, m_nFaceSetCount ]

  const CDmeFaceSet
      *m_pFaceSet; // Current face set: m_pMesh->GetFaceSet( m_nFaceSetIndex )

  int m_nFaceSetIndexCount; // Number of indices in current face set:
                            // m_pFaceSet->NumIndices();
  int m_nFaceSetIndexIndex; // Index of current face set's indices: [ 0,
                            // m_nFaceSetIndexCount ]
...
inline bool CDmMeshFaceIt::GetVertexData(
  ...
  CDmeVertexData *pBase =
      pPassedBase ? pPassedBase : m_pMesh->GetCurrentBaseState();
  if (!pBase)
    return false;
  ...
  if (pDataAttr->GetType() !=
      CDmAttributeInfo<CUtlVector<T_t>>().AttributeType())
    return false;
