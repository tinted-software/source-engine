...
inline float CDmeTimeFrame::GetTimeScale() const { return m_Scale; }
...
inline void CDmeTimeFrame::SetDuration(DmeTime_t flDuration) {
  Assert(m_Duration >=
         0); // if you want a reversed clip, set the timescale negative
...
inline void CDmeTimeFrame::SetTimeScale(float flScale) { m_Scale = flScale; }

//-----------------------------------------------------------------------------
// Convert back + forth between my media time and child media time
//-----------------------------------------------------------------------------
inline DmeTime_t CDmeTimeFrame::ToChildMediaTime(DmeTime_t t,
                                                 bool bClamp) const {
  t -= DmeTime_t(m_Start);
  if (bClamp) {
...
inline DmeTime_t CDmeTimeFrame::FromChildMediaTime(DmeTime_t t,
                                                   bool bClamp) const {
