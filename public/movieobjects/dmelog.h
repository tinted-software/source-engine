...
public:
  DmeLog_TimeSelection_t()
      : m_flIntensity(1.0f), m_bAttachedMode(true), m_bTimeAdvancing(false),
        m_bResampleMode(true),
        m_nResampleInterval(
            DmeTime_t(.05f)), // 50 msec sampling interval by default
        m_flThreshold(DMELOG_DEFAULT_THRESHHOLD), m_pPresetValue(0),
...
public:
  ...
  virtual void SetKey(DmeTime_t time, const CDmAttribute *pAttr, uint index = 0,
  ...
  // attribute
  virtual void GetValue(DmeTime_t time, CDmAttribute * pAttr, uint index = 0)
      const = 0;
...
public:
  ...
  virtual void StampKeyAtHead(DmeTime_t tHeadPosition,
                              ...
                              const CDmAttribute *pAttr, uint index = 0) = 0;
  ...
  virtual void GetValueSkippingTopmostLayer(
      DmeTime_t time, CDmAttribute * pAttr, uint index = 0) const = 0;
...
//-----------------------------------------------------------------------------
template <class T> class CDmeTypedCurveInfo : public CDmeCurveInfo {
...
//-----------------------------------------------------------------------------
template <class T> class CDmeTypedLogLayer : public CDmeLogLayer {
...
//-----------------------------------------------------------------------------
template <class T> class CDmeTypedLog : public CDmeLog {
...
// Template methods
//-----------------------------------------------------------------------------
template <class T> DmAttributeType_t CDmeTypedLogLayer<T>::GetDataType() const {
  return CDmAttributeInfo<T>::AttributeType();
}

template <class T> bool CDmeTypedLogLayer<T>::IsConstantValued() const {
  if (m_values.Count() < 2)
    return true;
...
// Template methods
//-----------------------------------------------------------------------------
template <class T> DmAttributeType_t CDmeTypedLog<T>::GetDataType() const {
  return CDmAttributeInfo<T>::AttributeType();
}
...
template <class T> float CDmeTypedLog<T>::GetDefaultValueThreshold() {
  return s_defaultThreshold;
}

template <class T> void CDmeTypedLog<T>::SetValueThreshold(float thresh) {
  m_threshold = thresh;
}

template <class T> bool CDmeTypedLog<T>::IsConstantValued() const {
...
template <class T> void CDmeTypedLog<T>::RemoveRedundantKeys() {
...
template <class T> inline float Normalize(const T &val) {
  Assert(0);
  return 0.5f;
}
...
template <> inline float Normalize(const bool &val) {
  return val ? 1.0f : 0.0f;
}

template <> inline float Normalize(const Color &val) {
  float sum = 0.0f;
  for (int i = 0; i < 4; ++i) {
    sum += val[i];
  }
...
template <> inline float Normalize(const QAngle &val) {
  float sum = 0.0f;
  for (int i = 0; i < 3; ++i) {
    float ang = val[i];
    if (ang < 0.0f) {
...
template <> inline float Normalize(const Quaternion &val) {
  QAngle angle;
...
template <class T>
inline void
CDmeTypedLog<T>::BuildNormalizedLayer(CDmeTypedLogLayer<float> *pTarget) {
  Assert(pTarget);
...
template <class T> T CDmeTypedLog<T>::ClampValue(const T &value) {
  return value;
}
...
template <class T> bool CDmeTypedCurveInfo<T>::IsEdgeActive(int edge) const {
  Assert(0);
  return false;
}
template <class T>
