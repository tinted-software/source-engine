...
(material name) Material name to use when searching for vertices to consider as
the “white” of the eye (used in dynamically texture mapping the iris and cornea
onto the eye).

(diameter) Diameter of the ball of the eye

(angle) Default yaw offset from “forward” for iris. Humans are typically 2-4
degrees walleyed. Not setting this correctly will result in your either
characters appearing cross-eyed, or if you’ve compensated by misplacing the ball
of the eye, them not tracking side to side.
...
eyeball (name) (bone name) (X) (Y) (Z) (material name) (diameter) (angle) (iris
material) (pupil scale)
...
public:
  ...
  CDmaVar<float> m_flYawAngle;   // Yaw offset from "forward" for iris.  Humans
                                 // are typically 2-4 degrees walleyed.
