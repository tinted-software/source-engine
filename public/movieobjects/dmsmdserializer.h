...
public:
  ...
  CDmSmdSerializer()
      : m_bOptAutoStripPrefix(false), m_bOptImportSkeleton(true),
        m_bOptAnimation(false), m_flFrameRate(30.0f) {
    SetUpAxis(Z_AXIS);
  }
  ...
  virtual bool Unserialize(CUtlBuffer &utlBuf, const char *pszEncodingName,
  ...
  // Methods used for importing (only should return non-NULL for serializers
  // that return false from StoresVersionInFile)
  virtual const char *GetImportedFormat() const { return NULL; }
  virtual int GetImportedVersion() const { return 1; }

  // CDmSmdSerializer
  CDmElement *ReadSMD(const char *pszFilename,
  ...
  struct SmdJoint_t {
    ...
    int m_nActualId; // The actual node id which is created after sorting and
                     // creating all joints in order with no gaps in numbering,
                     // corresponds to joitnIndex in DmeModel
    CUtlString m_sName;
    int m_nParentId;
    int m_nLineNumber;
    CDmeDag *m_pDmeDag;
    ...
    SmdJoint_t()
        : m_nId(-1), m_nActualId(-1), m_nParentId(-1), m_nLineNumber(-1),
          m_pDmeDag(NULL) {}
  };
...
protected:
  ...
  bool ParserHandleSkeletonLine(const char *pszBuf, CUtlString &sName, int &nId,
                                int &nParentId) const;

  CDmElement *CDmSmdSerializer::ReadSMD(CUtlBuffer &inUtlBuf,
  ...
  public:
    CNodeData()
        : m_nParentIndex(-1), m_bSkinned(false), m_nInfluenceIndex(0),
          m_pDmeDag(NULL) {}

    bool Valid() const { return m_pDmeDag != NULL; }

    void Reset() { m_pDmeDag = NULL; }
  ...
  void ParserSetJoint(const SmdJointMap_t &smdJointMap, int nFrame, int nId,
  ...
  matrix3x4_t
      m_mAdjNormal; // Matrix to adjust normals, inverse transpose of m_mAdj
