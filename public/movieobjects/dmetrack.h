...
public:
  ...
  void FindClipsAtTime(DmeTime_t time, DmeClipSkipFlag_t flags,
                       CUtlVector<CDmeClip *> &clips) const;
  void FindClipsIntersectingTime(DmeTime_t startTime, DmeTime_t endTime,
                                 DmeClipSkipFlag_t flags,
                                 CUtlVector<CDmeClip *> &clips) const;
  void FindClipsWithinTime(DmeTime_t startTime, DmeTime_t endTime,
                           DmeClipSkipFlag_t flags,
                           CUtlVector<CDmeClip *> &clips) const;

  // Methods to shift clips around
  // These methods shift clips that straddle the start/end time (NOTE: time is
  // measured in local time) NOTE: bTestStartingTime true means if the starting
  // time is after the start time, then shift Setting it to false means if the
  // clip intersects the time at all, then shift
  void ShiftAllClipsAfter(DmeTime_t startTime, DmeTime_t dt,
                          bool bTestStartingTime = true);
  void ShiftAllClipsBefore(DmeTime_t endTime, DmeTime_t dt,
                           bool bTestEndingTime = true);
  void ShiftAllClips(DmeTime_t dt);

  // A version that works only on film clips
  void ShiftAllFilmClipsAfter(CDmeClip *pClip, DmeTime_t dt,
                              bool bShiftClip = false);
  void ShiftAllFilmClipsBefore(CDmeClip *pClip, DmeTime_t dt,
                               bool bShiftClip = false);
  ...
  void FindAdjacentFilmClips(CDmeClip *pClip, CDmeClip *&pPrevClip,
                             CDmeClip *&pNextClip);
  void FindAdjacentFilmClips(DmeTime_t localTime, CDmeClip *&pPrevClip,
                             CDmeClip *&pNextClip);
  ...
  CDmeClip *FindFirstFilmClipIntesectingTime(DmeTime_t localStartTime,
  ...
  void FillAllGapsWithSlugs(const char *pSlugName, DmeTime_t startTime,
                            DmeTime_t endTime);
...
private:
  ...
  public:
    ...
    ~CSuppressAutoFixup() { m_pTrack->m_Flags.ClearFlag(m_nFlags); }
