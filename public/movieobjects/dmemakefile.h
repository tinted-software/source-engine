...
struct DmeMakefileType_t {
  ...
  bool m_bIsSingleton;
  const char
      *m_pDefaultDirectoryID; // NOTE: Use CDmeMakefile::GetDefaultDirectory,
                              // passing this in to crack it.
...
public:
  ...
  // If this source can be built by another makefile, return the type of
  // makefile NOTE: This can be a base class of a number of makefile types
...
inline const char *CDmeSource::GetRelativeFileName() const { return m_Name; }
...
public:
  // NOTE: Adding or removing sources of the specified type will invalidate the
  // index NOTE: This index is the same index used in GetSources()
  CDmeSource *AddSource(const char *pSourceType, const char *pFullPath);
  template <class T> T *AddSource(const char *pFullPath);
  CDmeSource *SetSingleSource(const char *pSourceType, const char *pFullPath);
  template <class T> T *SetSingleSource(const char *pFullPath);
  ...
  void GetSources(const char *pSourceType,
                  CUtlVector<CDmeHandle<CDmeSource>> &sources);
  template <class T> void GetSources(CUtlVector<CDmeHandle<T>> &sources);
  ...
  // path
  bool GetDefaultDirectory(const char *pDefaultDirectoryID, char *pFullPath,
                           int nBufLen);
...
private:
  ...
  void RelativePathToFullPath(const char *pRelativePath, char *pFullPath,
                              int nBufLen);

  // Fullpath to relative path
  void FullPathToRelativePath(const char *pFullPath, char *pRelativePath,
                              int nBufLen);

  // Updates the source names to be relative to a particular path
  bool UpdateSourceNames(const char *pOldRootDir, const char *pNewRootDir,
                         bool bApplyChanges);
...
// Dirty, dirty!
//-----------------------------------------------------------------------------
inline bool CDmeMakefile::IsDirty() const { return m_bIsDirty; }

inline void CDmeMakefile::SetDirty(bool bDirty) { m_bIsDirty = bDirty; }
...
//-----------------------------------------------------------------------------
template <class T> T *CDmeMakefile::AddSource(const char *pFullPath) {
  return CastElement<T>(
      AddSource(g_pDataModel->GetString(T::GetStaticTypeSymbol()), pFullPath));
}

//-----------------------------------------------------------------------------
// Template version to set a single source
//-----------------------------------------------------------------------------
template <class T> T *CDmeMakefile::SetSingleSource(const char *pFullPath) {
  return CastElement<T>(SetSingleSource(
      g_pDataModel->GetString(T::GetStaticTypeSymbol()), pFullPath));
}
