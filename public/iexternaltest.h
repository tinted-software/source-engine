...
public:
  ...
  GetISteamGameServerStats(const char *pchVersion) = 0;
  ...
  virtual void GetServerDetails(AppId_t *pnAppIdServed, uint32 *punIPGameServer,
                                uint16 *pusPortGameServer, bool *pbSecure) = 0;
  ...
  virtual bool BYieldingAwaitNewCallback(void *pCallback, uint32 unCallbackSize,
                                         int nCallbackType) = 0;
  virtual bool BYieldingAwaitQueuedCallback(void *pCallback,
                                            uint32 unCallbackSize,
                                            int nCallbackType) = 0;
  virtual bool BGetQueuedCallbackOfType(void *pCallback, uint32 unCallbackSize,
                                        int nCallbackType) = 0;

  // Retrieves the current status of the API call. Returns false if the call is
  // still in progress
  virtual bool BYieldingGetAPICallResult(SteamAPICall_t hSteamAPICall,
                                         void *pCallback, uint32 unCallbackSize,
                                         int nCallback,
...
public:
  ...
  virtual IExternalTestClient *
  YieldingCreateUserClient(IExternalTestClient *pGameserver) = 0;

  // creates a single gameserver client at the specified (fake) IP addr and port
  virtual IExternalTestClient *
  YieldingCreateGameserverClient(uint32 unAddr, uint16 usPort) = 0;
  ...
  // Logs on a bunch of clients all at once and returns when they are all logged
  // on.
  ...
  virtual ExternalTestBuffer_t
  YieldingSimulateWGRequest(const CSteamID &actorID, const void *pvRequestKV,
  ...
  virtual bool
  BYieldingHTTPRequest(int nWebMethod, bool bUseSSL, const char *pchURL,
                       const KeyValues *pkvPostParams, const char *pchAPIKey,
                       const CSteamID &steamID, ExternalTestBuffer_t *punBuffer,
                       int *pnResultCode) = 0;
  ...
  // Sets the persona name for this user via the wg call so we can actually tell
  // when it's been set.
...
public:
  ...
  virtual bool
  BDoPerATSInit(IExternalTestUtils *
