...
#ifdef CLIENT_DLL
ConVar hap_HasDevice("hap_HasDevice", "0", FCVAR_USERINFO /*|FCVAR_HIDDEN*/,
                     "falcon is connected");
...
#endif
ConVar hap_damagescale_game("hap_damagescale_game",
...
#else
  if (!weapon->IsPredicted() && weapon->GetOwner() &&
...
void ConnectHaptics(CreateInterfaceFn appFactory) {
  ...
  if (pFalconModule) {
    ...
    if (factory) {
      haptics = reinterpret_cast<IHaptics *>(
          factory(HAPTICS_INTERFACE_VERSION, NULL));
      if (haptics &&
          haptics->Initialize(engine, view, g_InputInternal, gpGlobals,
                              appFactory, g_pVGuiInput->GetIMEWindow(),
                              filesystem, enginevgui, ActivityList_IndexForName,
...
void HapticsHandleMsg_SPHapWeapEvent(int iActivity) {
  ...
  if (weap)
    haptics->ProcessHapticEvent(4, "Weapons", weap->GetName(), "Activities",
                                VarArgs("%i", iActivity));
}
...
#ifdef TERROR
ConVar hap_zombie_damage_scale("hap_zombie_damage_scale", "0.25",
                               FCVAR_RELEASE | FCVAR_NEVER_AS_STRING);
#endif
void HapticsHandleMsg_HapDmg(float pitch, float yaw, float damage,
                             int damageType) {
  if (!haptics->HasDevice())
    return;
...
ConVar hap_melee_scale("hap_melee_scale", "0.8",
                       FCVAR_RELEASE | FCVAR_NEVER_AS_STRING);
void HapticsHandleMsg_HapMeleeContact() {
  haptics->HapticsPunch(hap_melee_scale.GetFloat(), QAngle(0, 0, 0));
}
ConVar hap_noclip_avatar_scale("hap_noclip_avatar_scale", "0.10f",
                               FCVAR_RELEASE | FCVAR_NEVER_AS_STRING);
void UpdateAvatarEffect(void) {
  ...
  if (pPlayer->IsInAVehicle() && pPlayer->GetVehicle()) {
    ...
    if (!Q_stristr(pPlayer->GetVehicle()->GetVehicleEnt()->GetClassname(),
                   "choreo")) {
      eye[YAW] += 90;
    }
...
void HapticsEnteredVehicle(C_BaseEntity *vehicle,
  ...
  C_PropVehicleDriveable *drivable =
      dynamic_cast<C_PropVehicleDriveable *>(vehicle);
  bool hasgun = false;
  if (drivable)
  ...
  if (Q_stristr(vehicle->GetClassname(), "airboat")) {
    ...
    haptics->SetNavigationClass(
        "vehicle_gun_nofix"); // Give this a bit of aiming
  } else {
    haptics->ProcessHapticEvent(2, "Movement", "BaseVehicle");
    haptics->SetNavigationClass("vehicle_nogun");
  }
