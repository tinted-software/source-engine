...
enum EVideoPlaybackFlags_t // Options when playing a video file
{
  ...
  VALID_FULLSCREEN_FLAGS =
      0x0007771F, // Playback Flags that are valid for playing videos fullscreen
...
public:
  ...
  virtual const char *GetVideoFileName() = 0; // Gets the file name of the video
  ...
  GetVideoFrameRate() = 0; // Returns the frame rate of the associated video in
                           // FPS
  ...
  virtual void
  SetLooping(bool bLoopVideo) = 0; // Sets the video to loop (or not)
  virtual bool IsLooping() = 0;    // Queries if the video is looping

  virtual void
  SetPaused(bool bPauseState) = 0; // Pauses or Unpauses video playback
  ...
  virtual bool SetFrame(
      int FrameNum) = 0; // Sets the current frame # in the video to play next
  virtual int GetCurrentFrame() = 0; // Gets the current frame # for the video
                                     // playback, 0 based
  ...
  virtual void GetVideoTexCoordRange(
      float *pMaxU,
      float *pMaxV) = 0; // Returns the max texture coordinate of the video
                         // portion of the material surface ( 0.0, 0.0 to U, V )
  virtual void
  GetVideoImageSize(int *pWidth,
                    int *pHeight) = 0; // Returns the frame size of the Video
                                       // Image Frame in pixels ( the stored in
                                       // a subrect of the material itself)
};
...
public:
  virtual bool EstimateMovieFileSize(size_t *pEstSize, int movieWidth,
                                     int movieHeight, VideoFrameRate_t movieFps,
                                     float movieDuration,
                                     VideoEncodeCodec_t theCodec,
                                     int videoQuality,
  ...
  virtual bool
  SetMovieSourceImageParameters(VideoEncodeSourceFormat_t srcImageFormat,
...
public:
  ...
  virtual VideoSystemStatus_t
  GetVideoSystemStatus(VideoSystem_t videoSystem) = 0;
  virtual VideoSystemFeature_t
  GetVideoSystemFeatures(VideoSystem_t videoSystem) = 0;
  virtual const char *GetVideoSystemName(VideoSystem_t videoSystem) = 0;

  virtual VideoSystem_t
  FindNextSystemWithFeature(VideoSystemFeature_t features,
  ...
  virtual const char *GetSupportedFileExtension(VideoSystem_t videoSystem,
                                                int extNum = 0) = 0;
  virtual VideoSystemFeature_t
  GetSupportedFileExtensionFeatures(VideoSystem_t videoSystem,
                                    int extNum = 0) = 0;
  ...
  virtual VideoResult_t
  DestroyVideoMaterial(IVideoMaterial *pVideoMaterial) = 0;
  ...
  virtual VideoResult_t
  DestroyVideoRecorder(IVideoRecorder *pVideoRecorder) = 0;

  // Plays a given video file until it completes or the user presses ESC, SPACE,
  // or ENTER
