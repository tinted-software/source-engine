...
struct DataCacheLimits_t {
  DataCacheLimits_t(unsigned _nMaxBytes = (unsigned)-1,
      ...
      : nMaxBytes(_nMaxBytes), nMaxItems(_nMaxItems), nMinBytes(_nMinBytes),
...
public:
  ...
  LOCK_TYPE CacheGet(DataCacheHandle_t handle, bool bFrameLock = true) {
    return (LOCK_TYPE)(((STORAGE_TYPE *)m_pCache->Get(handle, bFrameLock))
                           ->GetData());
  }

  LOCK_TYPE CacheGetNoTouch(DataCacheHandle_t handle) {
    return (
        LOCK_TYPE)(((STORAGE_TYPE *)m_pCache->GetNoTouch(handle))->GetData());
  }
  ...
  DataCacheHandle_t CacheCreate(const CREATE_PARAMS &createParams,
    ...
    m_pCache->AddEx((DataCacheClientID_t)(uintp)pStore, pStore, pStore->Size(),
                    flags, &handle);
    return handle;
  }
