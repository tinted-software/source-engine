...
public:
  ...
  virtual bool
  NeedsFullFrameBufferTexture(IMaterialVar **params,
                              bool bCheckSpecificToThisFrame = true) const;
...
public:
  ...
  virtual void
  OnDrawElements(IMaterialVar **params, IShaderShadow *pShaderShadow,
                 IShaderDynamicAPI *pShaderAPI,
                 VertexCompressionType_t vertexCompression,
  ...
  // Finds a particular parameter	(works because the lowest parameters
  // match the shader)
  ...
  GetShaderAPITextureBindHandle(int nTextureVar, int nFrameVar,
  ...
  void
  SetFixedFunctionTextureScaledTransform(MaterialMatrixMode_t textureTransform,
  ...
  void FixedFunctionSelfIlluminationPass(Sampler_t sampler, int baseTextureVar,
                                         int frameVar,
                                         int baseTextureTransformVar,
  ...
  void
  GetColorParameter(IMaterialVar **params,
                    float *pColorOut) const; // return tint color (color*color2)
...
public:
  ...
  bool m_bMaterialVarsChanged; // set by mat system when material vars change.
