...
public:
  ...
  //	iQueryResult >= 0					-
  //iQueryResult is the number of pixels rendered 	OCCLUSION_QUERY_RESULT_PENDING
  //-	query results are not available yet 	OCCLUSION_QUERY_RESULT_ERROR
  //-	query failed
  ...
  virtual void EnableVSync_360(
      bool bEnable) = 0; // 360 allows us to bypass vsync blocking up to 60 fps
  ...
  virtual void CopyTextureToTexture(ShaderAPITextureHandle_t srcTex,
                                    ShaderAPITextureHandle_t dstTex) = 0;
};
