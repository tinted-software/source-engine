...
struct ShaderDisplayMode_t {
  ...
  int m_nRefreshRateNumerator; // Refresh rate. Use 0 in numerator + denominator
...
struct ShaderDeviceInfo_t {
  ...
  int m_nWindowedSizeLimitWidth; // Used if m_bLimitWindowedSize is set, defines
                                 // max bounds for the back buffer
  ...
  bool m_bLimitWindowedSize : 1; // In windowed mode, should we prevent the back
                                 // buffer from getting too large?
...
//-----------------------------------------------------------------------------
inline VertexShaderHandle_t
IShaderDevice::CreateVertexShader(CUtlBuffer &buf, const char *pShaderVersion) {
...
inline VertexShaderHandle_t
IShaderDevice::CreateVertexShader(const char *pProgram, size_t nBufLen,
                                  const char *pShaderVersion) {
...
inline GeometryShaderHandle_t
IShaderDevice::CreateGeometryShader(CUtlBuffer &buf,
                                    const char *pShaderVersion) {
  // NOTE: Text buffers are assumed to have source-code shader files
  // Binary buffers are assumed to have compiled shader files
  if (buf.IsText()) {
...
inline GeometryShaderHandle_t
IShaderDevice::CreateGeometryShader(const char *pProgram, size_t nBufLen,
                                    const char *pShaderVersion) {
...
inline PixelShaderHandle_t
IShaderDevice::CreatePixelShader(CUtlBuffer &buf, const char *pShaderVersion) {
  // NOTE: Text buffers are assumed to have source-code shader files
  // Binary buffers are assumed to have compiled shader files
  if (buf.IsText()) {
    Assert(pShaderVersion);
...
inline PixelShaderHandle_t
IShaderDevice::CreatePixelShader(const char *pProgram, size_t nBufLen,
                                 const char *pShaderVersion) {
