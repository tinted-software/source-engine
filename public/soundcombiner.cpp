...
bool CSoundCombiner::CreateWorkList(IFileSystem *pFilesystem,
                                    CUtlVector<CombinerEntry> &info) {
  m_Work.RemoveAll();
  ...
  for (int i = 0; i < c; ++i) {
    ...
    if (!LoadSentenceFromWavFile(fullpath, workitem->sentence)) {
      Warning("CSoundCombiner::CreateWorkList couldn't load %s for work item "
              "(%d)\n",
              fullpath, i);
      return false;
    }
...
bool CSoundCombiner::InternalCombineSoundFiles(
    IFileSystem *pFilesystem, char const *outfile,
    CUtlVector<CombinerEntry> &info) {
  Q_strncpy(m_szOutFile, outfile, sizeof(m_szOutFile));
  if (info.Count() <= 0) {
    Warning("CSoundCombiner::InternalCombineSoundFiles:  work item count is "
            "zero\n");
    return false;
  }
...
bool CSoundCombiner::CombineSoundFiles(IFileSystem *pFilesystem,
                                       char const *outfile,
                                       CUtlVector<CombinerEntry> &info) {
...
unsigned int CSoundCombiner::CheckSumWork(IFileSystem *pFilesystem,
                                          CUtlVector<CombinerEntry> &info) {
  if (info.Count() <= 0) {
...
bool CSoundCombiner::VerifyFilesExist(IFileSystem *pFilesystem,
                                      CUtlVector<CombinerEntry> &info) {
  int c = info.Count();
  for (int i = 0; i < c; ++i) {
...
bool CSoundCombiner::InitSplicer(IFileSystem *pFilesystem, int samplerate,
...
bool CSoundCombiner::PerformSplicingOnWorkItems(IFileSystem *pFilesystem) {
  ...
  if (!InitSplicer(pFilesystem, WAVEOUTPUT_FREQUENCY, bestNumChannels,
                   bitsPerChannel)) {
    return false;
  }
