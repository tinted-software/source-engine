...
mstudiolocalhierarchy_t *mstudioanimdesc_t::pHierarchy(int i) const {
  if (localhierarchyindex) {
    if (animblock == 0) {
      return (mstudiolocalhierarchy_t *)(((byte *)this) + localhierarchyindex) +
             i;
    } else {
      byte *pAnimBlocks = pStudiohdr()->GetAnimBlock(animblock);
...
const virtualmodel_t *
CStudioHdr::ResetVModel(const virtualmodel_t *pVModel) const {
  if (pVModel != NULL) {
...
mstudioseqdesc_t &CStudioHdr::pSeqdesc(int i) {
  ...
  if (i < 0 || i >= GetNumSeq()) {
    if (GetNumSeq() <= 0) {
      ...
      //	returning a reference to garbage. It should now see numevents is
      //0, 	and bail.
...
int CStudioHdr::GetSharedPoseParameter(int iSequence, int iLocalPose) const {
  ...
  virtualgroup_t *pGroup = m_pVModel->m_group.IsValidIndex(group)
                               ? &m_pVModel->m_group[group]
                               : NULL;
...
void CStudioHdr::RunFlexRules(const float *src, float *dest) {
  ...
  for (int i = 0; i < numflexrules(); i++) {
    ...
    for (int j = 0; j < prule->numops; j++) {
      ...
      case STUDIO_NWAY: {
        ...
        if (flValue <= filterRamp.x || flValue >= filterRamp.w) {
          ...
          flValue =
              RemapValClamped(flValue, filterRamp.x, filterRamp.y, 0.0f, 1.0f);
        } else if (flValue > filterRamp.z) {
          flValue =
              RemapValClamped(flValue, filterRamp.z, filterRamp.w, 1.0f, 0.0f);
        } else {
          flValue = 1.0f;
        }
    ...
    /*
                    // JasonM hack
                    if ( nFlexRulesToRun == 0)
    // 0 means run all rules correctly
                    {
                            dest[prule->flex] = stack[0];
                    }
...
void CStudioHdr::CActivityToSequenceMapping::Initialize(
  ...
  for (int i = 0; i < NumSeq; ++i) {
    ...
    if (seqdesc.activity >= 0) {
      ...
      if (m_ActToSeqHash.IsValidHandle(handle)) {
        ...
        if (!HushAsserts()) {
          AssertMsg(toUpdate->totalWeight > 0, "toUpdate->totalWeight: %d",
                    toUpdate->totalWeight);
        }
      } else {
  ...
  int sequenceCount = 0;
  int topActivity = 0; // this will store the highest seen activity number (used
                       // later to make an ad hoc map on the stack)
  ...
  m_pSequenceTuples =
      tupleList; // save it off -- NEVER USE m_pSequenceTuples in this function!
  ...
  for (int i = 0; i < NumSeq; ++i) {
    ...
    if (seqdesc.activity >= 0) {
      ...
      if (seqdesc.numactivitymodifiers > 0) {
        ...
        (tupleList + element.startingIdx + tupleOffset)->iNumActivityModifiers =
            seqdesc.numactivitymodifiers;

        for (int k = 0; k < seqdesc.numactivitymodifiers; k++) {
          (tupleList + element.startingIdx + tupleOffset)
              ->pActivityModifiers[k] = g_ActivityModifiersTable.AddString(
              seqdesc.pActivityModifier(k)->pszName());
        }
      } else {
        (tupleList + element.startingIdx + tupleOffset)->pActivityModifiers =
            NULL;
