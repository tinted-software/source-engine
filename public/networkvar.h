...
// CNetworkVars in it.
template <class T> static inline void DispatchNetworkStateChanged(T *pObj) {
...
#define CNetworkVarEmbedded(type, name)                                        \
  ...
  class NetworkVar_##name : public type {                                      \
    template <class T> NetworkVar_##name &operator=(const T &val) {            \
      *((type *)this) = val;                                                   \
      return *this;                                                            \
    }                                                                          \
                                                                               \
...
template <typename T> FORCEINLINE void NetworkVarConstruct(T &x) { x = T(0); }
FORCEINLINE void NetworkVarConstruct(color32_s &x) {
  x.r = x.g = x.b = x.a = 0;
}

template <class Type, class Changer> class CNetworkVarBase {
public:
  inline CNetworkVarBase() { NetworkVarConstruct(m_Value); }

  template <class C> const Type &operator=(const C &val) {
