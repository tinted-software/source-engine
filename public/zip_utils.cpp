...
// If we are going to include windows.h then we need to disable
// protected_things.h or else we get many warnings.
...
public:
  static HANDLE CreateTempFile(CUtlString &WritePath, CUtlString &FileName) {
    ...
    if (WritePath.IsEmpty()) {
      ...
      sprintf(uniqueFilename, "%d_%d_%d_%d_%d.tmp", tm->tm_wday, tm->tm_hour,
              tm->tm_min, tm->tm_sec, ++counter);
      V_ComposeFileName(WritePath.String(), uniqueFilename, tempFileName,
                        sizeof(tempFileName));
    }
  ...
  static unsigned int FileTell(HANDLE hFile) { return ftello((FILE *)hFile); }
...
public:
  ...
  void AddBufferToZip(const char *relativename, void *data, int length,
                      bool bTextMode, IZip::eCompressionType compressionType);
...
void CZipFile::ParseFromBuffer(void *buffer, int bufferlength) {
  ...
  ANALYZE_SUPPRESS(
      6293); // warning C6293: Ill-defined for-loop: counts down from minimum
  for (unsigned int startOffset = offset; offset <= startOffset; offset--) {
    ...
    if (rec.signature == PKID(5, 6)) {
      ...
      if (rec.commentLength) {
        ...
        int commentLength =
            min((int)rec.commentLength, (int)sizeof(commentString));
  ...
  for (i = 0; i < rec.nCentralDirectoryEntries_Total; i++) {
    ...
    buf.Get(tmpString, Min((unsigned int)zipFileHeader.fileNameLength,
                           (unsigned int)sizeof(tmpString)));
    ...
    newfiles[i].compressionType =
        (IZip::eCompressionType)zipFileHeader.compressionMethod;
...
void CZipFile::AddBufferToZip(const char *relativename, void *data, int length,
  ...
  if (bTextMode) {
    ...
    CopyTextData((char *)textTransform.Base(), (char *)outData, textLen,
                 outLength);
  ...
  if (compressionType == IZip::eCompressionType_LZMA) {
    ...
    unsigned char *pCompressedOutput =
        LZMA_Compress((unsigned char *)outData, outLength, &compressedSize);
    ...
    unsigned int finalCompressedSize =
        compressedSize - sizeof(lzma_header_t) + nZIPHeader;
    ...
    // properties size
    uint16 nSwappedPropertiesSize =
        LittleWord(sizeof(lzma_header_t().properties));
    compressionTransform.Put(&nSwappedPropertiesSize,
                             sizeof(nSwappedPropertiesSize));
    // properties
    compressionTransform.Put(
        &(((lzma_header_t *)pCompressedOutput)->properties),
        sizeof(lzma_header_t().properties));
    // payload
    compressionTransform.Put(pCompressedOutput + sizeof(lzma_header_t),
...
//-----------------------------------------------------------------------------
// Reads a file from the zip. Requires the zip file handle if this zip was
// loaded via ParseFromDisk
//-----------------------------------------------------------------------------
bool CZipFile::ReadFileFromZip(HANDLE hZipFile, const char *pRelativeName,
  ...
  if (!pData && hZipFile) {
    ...
    if (!CWin32File::FileRead(hZipFile, readBuffer.Base(),
                              pEntry->m_nCompressedSize)) {
      return false;
    }
  ...
  if (pEntry->m_eCompressionType != IZip::eCompressionType_None) {
    if (pEntry->m_eCompressionType == IZip::eCompressionType_LZMA) {
      ...
      decompressStream.InitZIPHeader(pEntry->m_nCompressedSize,
                                     pEntry->m_nUncompressedSize);

      unsigned int nCompressedBytesRead = 0;
      unsigned int nOutputBytesWritten = 0;
      bool bSuccess = decompressStream.Read(
          (unsigned char *)pData, pEntry->m_nCompressedSize,
          (unsigned char *)decompressTransform.Base(),
          decompressTransform.Size(), nCompressedBytesRead,
          nOutputBytesWritten);
...
void CZipFile::SaveToBuffer(CUtlBuffer &buf) {
  // Estimate size for buffer, since the linear growth of CUtlBuffer is a
  // virtual memory steamroller. This is best-effort. Ideally CUtlBuffer's
  // growth strategy would be sane and this would be unnecessary.
  int sizeEstimate = 0;
  for (int i = m_Files.FirstInorder(); i != m_Files.InvalidIndex();
       i = m_Files.NextInorder(i)) {
    CZipEntry *e = &m_Files[i];
  ...
  for (i = m_Files.FirstInorder(); i != m_Files.InvalidIndex();
    ...
    if (e->m_nCompressedSize > 0 && e->m_pData != NULL) {
      ...
      hdr.versionNeededToExtract =
          10; // No special features or even compression here, set to 1.0
#ifdef ZIP_SUPPORT_LZMA_ENCODE
...
public:
  ...
  virtual void ForceAlignment(bool aligned, bool bCompatibleFormat,
                              unsigned int alignmentSize) OVERRIDE;
...
void CZip::PrintDirectory(void) { m_ZipFile.PrintDirectory(); }

void CZip::Reset() { m_ZipFile.Reset(); }

unsigned int CZip::EstimateSize(void) { return m_ZipFile.CalculateSize(); }

// Add buffer to zip as a file with given name
void CZip::AddBufferToZip(const char *relativename, void *data, int length,
                          bool bTextMode, eCompressionType compressionType) {
