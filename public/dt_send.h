...
public:
  ...
  void SetRecipient(int iClient); // Note: these are CLIENT indices, not entity
                                  // indices (so the first player's index is 0).
...
public:
  ...
  const char *m_pExcludeDTName; // If this is an exclude prop, then this is the
                                // name of the datatable to exclude a prop from.
...
// Normal offset of is invalid on non-array-types, this is dubious as hell. The
// rest of the codebase converted to the legit offsetof from the C headers, so
// we'll use the old impl here to avoid exposing temptation to others
...
#define SENDINFO(varName)                                                      \
  #varName,                                                                    \
      _hacky_dtsend_offsetof(currentSendDTClass::MakeANetworkVar_##varName,    \
                             varName),                                         \
...
#define SENDINFO_VECTORELEM(varName, i)                                        \
  #varName "[" #i "]",                                                         \
      -(int)_hacky_dtsend_offsetof(                                            \
          currentSendDTClass::MakeANetworkVar_##varName, varName.m_Value[i]),  \
      sizeof(((currentSendDTClass *)0)->varName.m_Value[0])

#define SENDINFO_STRUCTELEM(varName)                                           \
  #varName, _hacky_dtsend_offsetof(currentSendDTClass, varName),               \
      sizeof(((currentSendDTClass *)0)->varName.m_Value)
#define SENDINFO_STRUCTARRAYELEM(varName, i)                                   \
  #varName "[" #i "]",                                                         \
      _hacky_dtsend_offsetof(currentSendDTClass, varName.m_Value[i]),          \
      sizeof(((currentSendDTClass *)0)->varName.m_Value[0])
...
#define SENDINFO_DT_NAME(varName, remoteVarName)                               \
  #remoteVarName, _hacky_dtsend_offsetof(currentSendDTClass, varName)
...
SendProp SendPropVector(
    ...
    int nBits = 32, // Number of bits (for each floating-point component) to use
                    // when encoding.
...
SendProp
SendPropInt(const char *pVarName, int offset,
            int sizeofVar = SIZEOF_IGNORE, // Handled by SENDINFO macro.
            int nBits = -1, // Set to -1 to automatically pick (max) number of
                            // bits based on size of element.
...
SendProp
SendPropDataTable(const char *pVarName, int offset, SendTable *pTable,
                  SendTableProxyFn varProxy = SendProxy_DataTableToDataTable);

SendProp
SendPropArray3(const char *pVarName, int offset, int sizeofVar, int elements,
               SendProp pArrayProp,
               SendTableProxyFn varProxy = SendProxy_DataTableToDataTable);
