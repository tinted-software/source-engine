...
public:
  ...
  unsigned int
      m_AllowedVerts[ALLOWEDVERTS_SIZE]; // This is built based on the layout
                                         // and sizes of our neighbors and tells
                                         // us which vertices are allowed to be
                                         // active.
};
...
#define DWL_FLAGS_CASTENTITYSHADOWS                                            \
  0x0002 // This says that the light will cast shadows from entities

// Old version of the worldlight struct, used for backward compatibility
// loading.
...
struct dworldlight_t {
  ...
  Vector intensity;
  Vector normal;             // for surfaces and spotlights
  Vector shadow_cast_offset; // gets added to the light origin when this light
                             // is used as a shadow caster (only if
                             // DWL_FLAGS_CASTENTITYSHADOWS flag is set)
  int cluster;
  emittype_t type;
  int style;
  float stopdot;  // start of penumbra for emit_spotlight
