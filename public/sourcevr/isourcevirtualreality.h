...
public:
  ...
  // Composites the HUD directly onto the backbuffer / render target, including
  // undistort.
  ...
  // might span two displays if we're dealing with a two-input display. Returns
  ...
  // Returns the transform from the mid-eye to the specified eye. Multiply this
  // by the tweaked (for mouse rotation and WASD translation) mideye position to
  // get the view matrix. This matrix takes the user's IPD into account.
