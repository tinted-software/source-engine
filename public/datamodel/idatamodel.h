...
public:
  ...
  virtual DmElementHandle_t
  CreateElement(UtlSymId_t typeSymbol, const char *pElementName,
                DmFileId_t fileid = DMFILEID_INVALID,
                const DmObjectId_t *pObjectID = NULL) = 0;
  virtual DmElementHandle_t
  CreateElement(const char *pTypeName, const char *pElementName,
                DmFileId_t fileid = DMFILEID_INVALID,
                const DmObjectId_t *pObjectID = NULL) = 0;
  ...
  virtual const DmObjectId_t &
  GetElementId(DmElementHandle_t hElement) const = 0;

  virtual const char *
  GetAttributeNameForType(DmAttributeType_t attType) const = 0;
  ...
  virtual bool
  DoesEncodingStoreVersionInFile(const char *pEncodingName) const = 0;
  ...
  // file format
  virtual bool
  UpdateUnserializedElements(const char *pSourceFormatName,
                             int nSourceFormatVersion, DmFileId_t fileid,
                             DmConflictResolution_t idConflictResolution,
                             CDmElement **ppRoot) = 0;
  ...
  virtual IDmLegacyUpdater *
  FindLegacyUpdater(const char *pLegacyFormatName) const = 0;
  virtual IDmFormatUpdater *
  FindFormatUpdater(const char *pFormatName) const = 0;
  ...
  virtual DmFileId_t
  RestoreFromFile(const char *pFileName, const char *pPathID,
                  const char *pFormatHint, CDmElement **ppRoot,
  ...
  virtual void Redo() = 0;
  virtual void
  TraceUndo(bool state) = 0; // if true, undo records spew as they are added
  ...
  virtual void
  SetClipboardData(CUtlVector<KeyValues *> &data,
  ...
  virtual DmAttributeReferenceIterator_t
  FirstAttributeReferencingElement(DmElementHandle_t hElement) = 0;
  virtual DmAttributeReferenceIterator_t
  NextAttributeReferencingElement(DmAttributeReferenceIterator_t hAttrIter) = 0;
  virtual CDmAttribute *
  GetAttribute(DmAttributeReferenceIterator_t hAttrIter) = 0;
