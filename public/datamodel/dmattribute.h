...
// Fast dynamic cast
//-----------------------------------------------------------------------------
template <class E> inline E *CastElement(CDmElement *pElement) {
...
// from actual type
//-----------------------------------------------------------------------------
template <class E> inline E *GetElement(DmElementHandle_t hElement) {
...
template <class T> inline const T &CDmAttribute::GetValue() const {
...
template <class E> inline E *CDmAttribute::GetValueElement() const {
...
//-----------------------------------------------------------------------------
template <class E> inline void CDmAttribute::SetValue(E *pValue) {
  Assert(GetType() == AT_ELEMENT);
  if (GetType() == AT_ELEMENT) {
...
template <> inline void CDmAttribute::SetValue(const char *pValue) {
  int nLen = pValue ? Q_strlen(pValue) + 1 : 0;
  CUtlString str(pValue, nLen);
  return SetValue(str);
}

template <> inline void CDmAttribute::SetValue(char *pValue) {
...
CDmElement::GetAttribute(const char *pAttributeName,
...
template <class E>
inline CDmAttribute *
CDmElement::AddAttributeElement(const char *pAttributeName) {
...
template <class E>
inline CDmAttribute *
CDmElement::AddAttributeElementArray(const char *pAttributeName) {
...
inline const char *
CDmElement::GetValueString(const char *pAttributeName) const {
...
template <class T> T *FindAncestorReferencingElement(CDmElement *target) {
  if (!target)
    return NULL;
...
template <class T> T *FindAncestorReferencingElement_R(CDmElement *target) {
  if (!target)
    return NULL;
