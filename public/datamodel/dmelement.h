...
struct DmElementReference_t {
  ...
  bool
  IsWeaklyReferenced() // should we keep this element's DmElementHandle_t mapped
                       // to it's id (even if the element is deleted)
  {
  ...
  unsigned short m_nWeakHandleCount; // CDmeHandle<T> - for auto-hookup once the
                                     // element comes back, mainly used by UI
  unsigned short m_nStrongHandleCount; // CDmeCountedElementRef - for preventing
                                       // elements from being truly deleted,
...
//-----------------------------------------------------------------------------
template <class T> int DmeEstimateMemorySize(T *pElement) { return sizeof(T); }
