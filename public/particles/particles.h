...
public:
  ...
  virtual bool IsPointInControllingObjectHitBox(CParticleCollection *pParticles,
  ...
  virtual void GetRandomPointsOnControllingObjectHitBox(
      ...
      Vector *pHitBoxRelativeCoordOut = NULL, int *pHitBoxIndexOut = NULL) = 0;

  virtual int
  GetControllingObjectHitBoxInfo(CParticleCollection *pParticles,
                                 int nControlPointNumber,
  ...
  virtual void GetLocalPlayerEyeVectors(Vector *pForward, Vector *pRight = NULL,
                                        Vector *pUp = NULL) {
...
public:
  ...
  // create a particle system by name. returns null if one of that name does not
  // exist
  ...
  GetParticleSystemIndex(const char *pParticleSystemName);
  ...
  GetAvailableParticleOperatorList(ParticleFunctionType_t nWhichList);
  ...
  void
  CommitProfileInformation(bool bCommit); // call after simulation, if you want
  ...
  float ParticleThrottleScaling() const; // Returns 1.0 = not restricted, 0.0 =
                                         // fully restricted (i.e. don't draw!)
  bool
  ParticleThrottleRandomEnable() const; // Retruns a randomish bool to say if
                                        // you should draw this particle.
...
public:
  ...
  CreateInstance(const DmObjectId_t &id) const = 0;
...
public:
  ...
  // Indicates if this initializer is scrub-safe (initializers don't use random
  // numbers, for example)
...
#define BEGIN_PARTICLE_RENDER_OPERATOR_UNPACK(_className)                      \
  ...
  DMXELEMENT_UNPACK_FIELD("Visibility Proxy Input Control Point Number", "-1", \
                          int, VisibilityInputs.m_nCPin)                       \
...
struct ParticleRenderData_t {
  ...
  uint8
      m_nAlpha; // effective alpha, combining alpha and alpha2 and vis. 0 - 255
...
public:
  ...
  const ParticleRenderData_t *
  GetRenderList(IMatRenderContext *pRenderContext, bool bSorted,
                int *pNparticles, CParticleVisibilityData *pVisibilityData);
...
private:
  ...
  bool
  SystemContainsParticlesWithBoolSet(bool CParticleCollection::*pField) const;
...
private:
  unsigned char *m_pParticleMemory; // fixed size at initialization. Must be
                                    // aligned for SSE
  unsigned char *m_pParticleInitialMemory; // fixed size at initialization. Must
                                           // be aligned for SSE
  ...
  int m_nPerParticleReadInitialAttributeMask; // What fields do operators want
                                              // to see initial attribute values
                                              // for?
  ...
  uint64 m_nControlPointReadMask; // Mask indicating which control points have
                                  // been accessed
  ...
  bool m_bUsesPowerOfTwoFrameBufferTexture; // whether or not we use this, _not_
                                            // our children
...
inline void
CParticleCollection::LoanKillListTo(CParticleCollection *pBorrower) const {
...
inline void
CParticleCollection::SetControlPointOrientation(int nWhichPoint,
                                                const Quaternion &q) {
  matrix3x4_t mat;
...
inline const float *
CParticleCollection::GetFloatAttributePtr(int nAttribute,
                                          int nParticleNumber) const {
...
inline int *
CParticleCollection::GetIntAttributePtrForWrite(int nAttribute,
                                                int nParticleNumber) {
...
inline const int *
CParticleCollection::GetIntAttributePtr(int nAttribute,
                                        int nParticleNumber) const {
  return (int *)GetFloatAttributePtr(nAttribute, nParticleNumber);
}

inline const fltx4 *
CParticleCollection::GetM128AttributePtr(int nAttribute,
                                         size_t *pStrideOut) const {
  *(pStrideOut) = m_nParticleFloatStrides[nAttribute] / 4;
  return reinterpret_cast<fltx4 *>(m_pParticleAttributes[nAttribute]);
}

inline const FourInts *
CParticleCollection::Get4IAttributePtr(int nAttribute,
                                       size_t *pStrideOut) const {
  *(pStrideOut) = m_nParticleFloatStrides[nAttribute] / 4;
...
inline const FourVectors *
CParticleCollection::Get4VAttributePtr(int nAttribute,
                                       size_t *pStrideOut) const {
...
inline FourVectors *
CParticleCollection::Get4VAttributePtrForWrite(int nAttribute,
                                               size_t *pStrideOut) {
  *(pStrideOut) = m_nParticleFloatStrides[nAttribute] / 12;
...
inline const FourVectors *
CParticleCollection::GetInitial4VAttributePtr(int nAttribute,
                                              size_t *pStrideOut) const {
...
inline float *
CParticleCollection::GetFloatAttributePtrForWrite(int nAttribute,
                                                  int nParticleNumber) {
...
inline fltx4 *
CParticleCollection::GetM128AttributePtrForWrite(int nAttribute,
                                                 size_t *pStrideOut) {
  // NOTE: If you hit this assertion, it means your particle operator isn't
  // returning the appropriate fields in the RequiredAttributesMask call
  if (!HushAsserts()) {
...
inline const float *
CParticleCollection::GetInitialFloatAttributePtr(int nAttribute,
                                                 int nParticleNumber) const {
  Assert(nParticleNumber < m_nAllocatedParticles);
  int block_ofs = nParticleNumber / 4;
...
inline const fltx4 *
CParticleCollection::GetInitialM128AttributePtr(int nAttribute,
                                                size_t *pStrideOut) const {
...
inline float *
CParticleCollection::GetInitialFloatAttributePtrForWrite(int nAttribute,
                                                         int nParticleNumber) {
  Assert(nParticleNumber < m_nAllocatedParticles);
...
inline fltx4 *
CParticleCollection::GetInitialM128AttributePtrForWrite(int nAttribute,
                                                        size_t *pStrideOut) {
  Assert(m_nPerParticleReadInitialAttributeMask & (1 << nAttribute));
  *(pStrideOut) = m_nParticleInitialFloatStrides[nAttribute] / 4;
  return reinterpret_cast<fltx4 *>(m_pParticleInitialAttributes[nAttribute]);
...
private:
  ...
  GetOperatorList(ParticleFunctionType_t type);
...
inline const char *
CParticleSystemDefinition::GetCullReplacementDefinition() const {
...
// Retirement
inline bool
CParticleSystemDefinition::HasRetirementBeenChecked(int nFrame) const {
...
FORCEINLINE int
CParticleCollection::GetControlPointParent(int nControlPoint) const {
  Assert(nControlPoint <= GetHighestControlPoint());
  Assert(m_pDef->ReadsControlPoint(nControlPoint));
