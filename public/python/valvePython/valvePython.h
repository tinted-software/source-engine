...
//-----------------------------------------------------------------------------
CreateInterfaceFn
ValvePythonAppFactory(const char *pGetAppFactory = "GetAppFactory");
...
public:
  // Constructor
  CValvePythonCommand(const char *pName, PyCFunction pMeth, int nFlags,
                      const char *pDoc)
      : m_name(pName), m_pMeth(pMeth), m_nFlags(nFlags), m_doc(pDoc) {
...
#define PYTHON_COMMAND(_name, _flags, _doc)                                    \
  extern "C" static PyObject *_name##_pythonFunc(PyObject *pSelf,              \
                                                 PyObject *pArgs);             \
  static CValvePythonCommand _name##_command(#_name, _name##_pythonFunc,       \
                                             _flags, _doc);                    \
  extern "C" static PyObject *_name##_pythonFunc(PyObject *pSelf,              \
                                                 PyObject *pArgs)
...
#define PYTHON_COMMAND_KEYWORDS(_name, _flags, _doc)                           \
  extern "C" static PyObject *_name##_pythonFunc(                              \
      PyObject *pSelf, PyObject *pArgs, PyObject *pKeywords);                  \
  static CValvePythonCommand _name##_command(                                  \
      #_name, reinterpret_cast<PyCFunction>(_name##_pythonFunc), _flags,       \
      _doc);                                                                   \
  extern "C" static PyObject *_name##_pythonFunc(                              \
      PyObject *pSelf, PyObject *pArgs, PyObject *pKeywords)
...
public:
  ...
  CValvePythonSubModule(const char *pName, PythonInitFunc_t pInitFunc)
      : m_name("_"), m_pInitFunc(pInitFunc) {
    m_name += pName;
    m_pNextFactory = s_pFirstFactory;
    s_pFirstFactory = this;
  }
...
// Support for implemented extended python commands which can take multiple
// position arguments and specified keyword arguments
//
typedef enum _pytypes {
  PY_FIELD_VOID = 0, // No type or value
  PY_FIELD_BOOLEAN,  // boolean, implemented as an int, I may use this as a hint
                     // for compression
...
#define _PY_PARAMETER(var_name, type, parameter_name, help)                    \
  {type,                                                                       \
   #var_name,                                                                  \
   ...
   help,                                                                       \
   NULL,                                                                       \
   sizeof(((classNameTypedef *)0)->var_name)}
#define _PY_ARGUMENT(var_name, type, argument_index, help)                     \
  {type,                                                                       \
   #var_name,                                                                  \
   offsetof(classNameTypedef, var_name),                                       \
   argument_index,                                                             \
...
#define DEFINE_PY_PARAMETER(var_name, type, parameter_name, help)              \
  _PY_PARAMETER(var_name, type, parameter_name, help)
#define DEFINE_PY_ARGUMENT(var_name, type, argument_index, help)               \
  _PY_ARGUMENT(var_name, type, argument_index, help)
...
#define BEGIN_PY_PARAMETERS(className)                                         \
  ...
  pydatamap_t *className::GetBaseParameterMap() {                              \
    pydatamap_t *pResult;                                                      \
    ParameterMapAccess((BaseClass *)NULL, &pResult);                           \
    return pResult;                                                            \
  }                                                                            \
...
#define BEGIN_PY_PARAMETERS_GUTS(className)                                    \
  ...
  namespace className##_ParameterMapInit {                                     \
    pydatamap_t *g_DataMapHolder = ParameterMapInit(                           \
        (className *)                                                          \
...
#define END_PY_PARAMETERS()                                                    \
  ...
  if (sizeof(dataDesc) > sizeof(dataDesc[0])) {                                \
    classNameTypedef::m_ParameterMap.dataNumFields =                           \
...
public:
  CParameterGeneratedNameHolder(const char *pszBase) : m_pszBase(pszBase) {
...
#define PYTHON_COMMAND_EXTENDED(_className)                                    \
  ...
  extern "C" static PyObject *g_##_className##Dispatch(                        \
      PyObject *pSelf, PyObject *pArgs, PyObject *pKeywords) {                 \
    return g_##_className##Instance.DispatchRaw(pSelf, pArgs, pKeywords);      \
  }                                                                            \
  static CValvePythonCommand _className##_command(                             \
      g_##_className##Instance.GetName(),                                      \
      reinterpret_cast<PyCFunction>(g_##_className##Dispatch),                 \
      g_##_className##Instance.GetFlags(), g_##_className##Instance.GetDoc());

class CBasePythonCommand {
  DECLARE_PY_PARAMETER_DESC();

public:
  CBasePythonCommand(const char *pchCommandName, int nFlags,
                     char const *pchHelpText, pydatamap_t *pDataMap);
  ...
  virtual PyObject *Dispatch(CUtlVector<CUtlString> &args,
                             CUtlVector<int> &typedArgs) = 0;
  virtual void SetDefaults() = 0;

private:
  void InitParameters();
  void ProcessArguments(PyObject *pArgs, PyObject *pKeywords,
                        CUtlVector<CUtlString> &args,
                        CUtlVector<int> &typedArgs);
  pytypedescription_t *FindParameter(char const *pchName);
  pytypedescription_t *FindArgument(int index);
  void ExtractParameter(char const *pchName,
                        pytypedescription_t *pTypeDescription,
                        PyObject *pValue);
  template <class T> inline bool IsParameterTypeValid(pytype_t, T *) const;

protected:
  CUtlString BuildAutoGeneratedField(char const *pchParameterName,
                                     char const *pchPrefix, int *pCounter);
