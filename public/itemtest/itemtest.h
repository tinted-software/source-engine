...
class CItemUpload {
public:
  // Returns the Steam AccountID as an 0x prefixed hex string, true on success,
  // false on failure
  ...
  static bool GetSourceSDKFromExe(CUtlString &sSourceSDK,
                                  CUtlString &sSourceSDKBin);

  static bool GetBinDirectory(CUtlString &sSDKBinDir);
  static bool RunCommandLine(const char *pszCmdLine, const char *pszWorkingDir,
                             CItemLog *pLog);
  static bool FileExists(const char *pszFilename);
  static bool CopyFiles(const char *pszSourceDir, const char *pszPattern,
  ...
  static bool
  GetCurrentExecutableFileName(CUtlString &sCurrentExecutableFileName);
  static bool GetSteamAppInstallLocation(CUtlString &sSteamAppInstallLocation,
                                         int nAppId);
...
public:
  ...
  const char *GetClass(int nClass) {
    return (nClass >= 0 && nClass < m_vecClasses.Count())
               ? m_vecClasses[nClass].Get()
               : NULL;
  }
  const char *GetClassVMTTemplate(int nClass) {
    return (nClass >= 0 && nClass < m_vecClassTemplates.Count())
               ? m_vecClassTemplates[nClass]
               : NULL;
  }
  ...
  const char *GetTextureType(int nTexture) {
    return (nTexture >= 0 && nTexture < m_vecTextureTypes.Count())
               ? m_vecTextureTypes[nTexture].pszTextureType
               : NULL;
  }
  bool IsTextureTypeRequired(int nTexture) {
    return (nTexture >= 0 && nTexture < m_vecTextureTypes.Count())
               ? !m_vecTextureTypes[nTexture].bOptional
               : false;
  }
  ...
  const char *GetItemDirectory(void) {
    return m_strItemDirectoryOverride.IsEmpty()
               ? m_pItemDirectory
               : m_strItemDirectoryOverride.String();
  }
  const char *GetAnimationDirectory(void) {
    return m_strAnimationDirectoryOverride.IsEmpty()
               ? m_pAnimationDirectory
               : m_strAnimationDirectoryOverride.String();
  }
  const char *GetIconDirectory(void) {
    return m_strIconDirectoryOverride.IsEmpty()
               ? m_pIconDirectory
               : m_strIconDirectoryOverride.String();
  }
  ...
  int GetQCLODDistance(int nLOD) {
    return (nLOD > 0 && nLOD <= m_vecQCLODDistances.Count())
               ? m_vecQCLODDistances[nLOD - 1]
               : nLOD;
  }
...
private:
  bool ParseStringsFromManifest(KeyValues *pKV, const char *pszKeyName,
...
//
//=============================================================================
template <class T> class CItemUploadGame : public CItemUpload {
public:
...
public:
  ...
  // Is this target valid?  A target is valid if all of it's inputs are valid
  // and the settings make sense This function can do any sort of validity
  // checking required.  Not called IsValid to avoid confusion with
  // CSmartPtr::IsValid which most CTargetBase items are wrapped in.  If returns
  // true, sMsg string is unchanged, if returns false, sMsg contains description
  // of first encountered problem.
  ...
  // Returns a string indicating the type of target, i.e. MDL, QC, DMX, VMT,
  // VTF, TGA, etc...
  virtual const char *GetTypeString() const = 0;

  // Return the number of files that are output from this CTarget, essentially
  ...
  enum PathFlags {
    PATH_FLAG_FILE =
        0x20, // If set the filename is included in the returned path
    PATH_FLAG_EXTENSION = 0x10, // If set and PATH_FLAG_FILE, the returned path
                                // will include the extension on the filename
    PATH_FLAG_PATH =
        0x08, // If set the pathname is included in the returned path
    PATH_FLAG_ABSOLUTE = 0x04, // If set and PATH_FLAG_PATH, the returned path
                               // will be absolute (default relative)
    PATH_FLAG_MODELS =
        0x02, // if not set and PATH_FLAG_PATH & !PATH_FLAG_ABSOLUTE, the
              // returned path will include the 'models' prefix, !MODELS implies
              // !PREFIX
    PATH_FLAG_PREFIX =
        0x01, // If not set and PATH_FLAG_PATH & !PATH_FLAG_ABSOLUTE, the
              // returned path will include the prefix (materials | materialsrc)
    PATH_FLAG_ALL = 0x0ffff,
    PATH_FLAG_ZIP = 0x10000
  };

  // Gets the specified output path with the specified flags.  See flag usage
  // below.  sOutputPath cleared on error.
  bool GetOutputPath(CUtlString &sOutputPath, int nIndex = 0,
                     uint nPathFlags = PATH_FLAG_ALL) const;

  // Appends all output paths to the passed array with the specified flags.  See
  // flag usage below.  If bRecurse is true, calls it for each output
  // CTargetBase as well
  bool GetOutputPaths(CUtlVector<CUtlString> &sOutputPaths,
                      uint nPathFlags = PATH_FLAG_ALL,
                      bool bRecurse = false) const;

  // First compile all inputs and then compile this target which produces the
  // expected output files Can do all sorts of things, like load and save files
  // or call vtex, studiomdl, etc...
  virtual bool Compile();

  // Returns the output filenames for this target and optionally for all inputs
  // recursively, either relative or absolute
  bool GetOutputPaths(CUtlVector<CUtlString> &sOutputPaths, bool bRelative,
                      bool bRecurse, bool bExtension,
                      bool bPrefix = true) const;

  // Returns the input filenames for this target and optionally for all inputs
  // recursively, either relative or absolute
  virtual bool GetInputPaths(CUtlVector<CUtlString> &sInputPaths,
                             bool bRelative = true, bool bRecurse = true,
  ...
  // Returns the extensions for the outputs of this CTargetBase.  Extensions
  // should include the .
  virtual const ExtensionList *GetExtensionsAndCount(void) const {
    return NULL;
  }

  // Returns a prefix between the mod directory and the relative path, used for
  // 'materials' & 'materialsrc'
  virtual const char *GetPrefix() const { return NULL; }

  // The name of this target without any extension, i.e. the filename where it
  // should be written
...
protected:
  ...
  virtual bool GetDirName(CUtlString &sDirName,
                          uint nPathFlags = PATH_FLAG_ALL) const;
...
public:
  ...
  static const char *ColorAlphaTypeToString(ColorAlpha_t nColorAlphaType) {
    static const char *pszColorAlphaTypeStr[] = {"NoColorAlpha", "Transparency",
                                                 "Paintable", "ColorSpecPhong"};

    if (nColorAlphaType < 0 ||
        nColorAlphaType >= ARRAYSIZE(pszColorAlphaTypeStr)) {
      return "Unknown";
    }
  ...
  enum NormalAlpha_t { kNoNormalAlpha, kNormalSpecPhong };

  static const char *NormalAlphaTypeToString(NormalAlpha_t nNormalAlphaType) {
    static const char *pszNormalAlphaTypeStr[] = {"NoNormalAlpha",
                                                  "NormalSpecPhong"};

    if (nNormalAlphaType < 0 ||
        nNormalAlphaType >= ARRAYSIZE(pszNormalAlphaTypeStr)) {
      return "Unknown";
    }
  ...
  bool SetTargetVTF(
      const char *pszTextureType, const char *pszFilename,
  ...
  CTargetVTF *GetTargetVTF(int iVTF, int nSkinIndex = 0) {
    if (nSkinIndex >= 0 && nSkinIndex < m_vecTargetVTFs.Count()) {
      if (iVTF >= 0 && iVTF < m_vecTargetVTFs[nSkinIndex].Count())
        return m_vecTargetVTFs[nSkinIndex][iVTF].IsValid()
                   ? m_vecTargetVTFs[nSkinIndex][iVTF].GetObject()
                   : NULL;
    }
    return NULL;
  }
    ...
    FOR_EACH_VEC(m_vecTargetVTFs, i) {
      FOR_EACH_VEC(m_vecTargetVTFs[i], j) {
        if (m_vecTargetVTFs[i][j].IsValid() &&
            m_vecTargetVTFs[i][j]->GetTargetTGA() == pTGA)
...
protected:
  ...
  bool SetTargetVTF(CSmartPtr<CTargetVTF> &pTargetVTF, const char *pszFilename,
...
public:
  ...
  virtual const char *GetItemDirectory() const {
    return m_strQCITemplate.IsEmpty()
               ? CItemUpload::Manifest()->GetItemDirectory()
...
protected:
  ...
  void SkinToBipHead_R(CDmeModel *pDstModel, CDmeDag *pSrcDag,
                       const Vector &vBipHead);

  bool OutputQCIFile();
  void OutputSounds(CUtlBuffer &buf, int nIndentLevel,
...
public:
  ...
  // Add a DMX Target to a QC which is a pathname to a geometry file (SMD, OBJ
  // or DMX) in some arbitrary location
...
public:
  ...
  bool GetRelativeDir(CUtlString &sRelativeDir, const char *pszPrefix,
                      const CTargetBase *pTarget) const;

  // Returns GetSDKContentDir()/GetRelativeDir() or
  // GetVProjectDir()/GetRelativeDir()
  bool GetAbsoluteDir(CUtlString &sAbsoluteDir, const char *pszPrefix,
                      const CTargetBase *pTarget) const;
  ...
  CSmartPtr<CTargetVMT> FindOrAddMaterial(const char *pszMaterial,
                                          int nMaterialType);
  ...
  const char *CheckRedundantOutputFilePath(const char *pszInputFilePath,
...
//
//=============================================================================
template <typename T> class CAssetGame : public CAsset {
...
//d:/dev/main/game/tf/materials/models/player/items/pyro/0x00017714/foo/foo.vmt
...
//materials/models/player/items/pyro/0x00017714/foo/foo.vmt 	true	true
//true	false	true	false	models/player/items/pyro/0x00017714/foo/foo.vmt
//	true	true	true	false	false	true
//player/items/pyro/0x00017714/foo/foo.vmt 	true	true	true	false	false
//false	player/items/pyro/0x00017714/foo/foo.vmt
//
...
//d:/dev/main/game/tf/materials/models/player/items/pyro/0x00017714/foo/foo 	true
//false	true	true	true	false
//d:/dev/main/game/tf/materials/models/player/items/pyro/0x00017714/foo/foo 	true
//false	true	true	false	true
//d:/dev/main/game/tf/materials/models/player/items/pyro/0x00017714/foo/foo 	true
//false	true	true	false	false
//d:/dev/main/game/tf/materials/models/player/items/pyro/0x00017714/foo/foo
//
//	true	false	true	false	true	true
//materials/models/player/items/pyro/0x00017714/foo/foo 	true	false	true
//false	true	false	models/player/items/pyro/0x00017714/foo/foo 	true
//false	true	false	false	true	player/items/pyro/0x00017714/foo/foo
//	true	false	true	false	false	false
//player/items/pyro/0x00017714/foo/foo
//
...
//d:/dev/main/game/tf/materials/models/player/items/pyro/0x00017714/foo 	false
...
//materials/models/player/items/pyro/0x00017714/foo 	false	true	true
//false	true	false	models/player/items/pyro/0x00017714/foo 	false	true
//true	false	false	true	player/items/pyro/0x00017714/foo 	false	true
//true	false	false	false	player/items/pyro/0x00017714/foo
//
...
//materials/models/player/items/pyro/0x00017714/foo/foo 	false	false	true
//false	true	false	models/player/items/pyro/0x00017714/foo/foo 	false
//false	true	false	false	true	player/items/pyro/0x00017714/foo/foo
