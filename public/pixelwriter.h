...
public:
  ...
  FORCEINLINE void WritePixelNoAdvance(int r, int g, int b,
                                       int a = 255) RESTRICT;
...
FORCEINLINE_PIXEL void CPixelWriter::SetPixelMemory(ImageFormat format,
  ...
  switch (format) {
  case IMAGE_FORMAT_R32F: // NOTE! : the low order bits are first in this naming
                          // convention.
    m_Size = 4;
    m_RShift = 0;
    m_GShift = 0;
  ...
  default: {
    ...
    m_Size = 0; // set to zero so that we don't stomp memory for formats that we
                // don't understand.
    m_RShift = 0;
    m_GShift = 0;
    m_BShift = 0;
...
FORCEINLINE_PIXEL void CPixelWriter::WritePixelNoAdvance(FLTX4 rgba) RESTRICT {
  ...
  case 4: {
    ...
    default:
      AssertMsg1(false,
                 "Unknown four-byte pixel format %d in lightmap write.\n",
                 m_Format);
    }
    break;
  }
...
FORCEINLINE void CPixelWriter::WritePixelNoAdvance_RGBA8888(FLTX4 rgba) {
  ...
  __stvewx(
      N, m_pBits,
      0); // store whatever word happens to be aligned with m_pBits to that word
}
...
FORCEINLINE void
CPixelWriter::WritePixelNoAdvance_BGRA8888(FLTX4 rgba,
                                           void *RESTRICT pBits) RESTRICT {
...
FORCEINLINE void
CPixelWriter::WriteFourPixelsExplicitLocation_BGRA8888(FLTX4 rgba, int offset) {
