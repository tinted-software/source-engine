...
// DBGFLAG_MEMORY:			Enables our memory debugging system,
// which overrides malloc & free DBGFLAG_MEMORY_NEWDEL:	Enables new / delete
...
// checking integrity and memory leaks DBGFLAG_ASSERT:			Turns
// Assert on or off (when off, it isn't compiled at all) DBGFLAG_ASSERTFATAL:
// Turns AssertFatal on or off (when off, it isn't compiled at all)
// DBGFLAG_ASSERTDLG:		Turns assert dialogs on or off and debug breaks
// on or off when not under the debugger.
//								(Dialogs will
//always be on when process is being debugged.)
// DBGFLAG_STRINGS:			Turns on hardcore string validation
// (slow but safe)
