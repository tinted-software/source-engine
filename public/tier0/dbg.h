...
#define AssertFatalMsg(_exp, _msg, ...)                                        \
  _AssertMsg(_exp, (const tchar *)CDbgFmtMsg(_msg, ##__VA_ARGS__), ((void)0),  \
             true)
...
// NOTE: On GCC / Clang, assert_cast can sometimes fire even if the type is
// correct. We should just workaround these. The situation where this would
// occur is
// 1. You create an object of a low level type in a DLL, and it really gets
// created there.
// 2. You pass it across a DLL boundary
// 3. You use assert_cast to verify it in the second DLL boundary (where it also
...
template <class Type> class CDataWatcher {
public:
  ...
  template <class C> const Type &operator&=(C val) {
...
// This is horrible, but we don't want to integrate CS:GO new logging system
// atm.
