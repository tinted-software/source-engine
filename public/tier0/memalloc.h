...
inline bool
ValueIsPowerOfTwo(size_t value) // don't clash with mathlib definition
{
...
#define MEM_ALLOC_CREDIT_(tag)                                                 \
  CMemAllocAttributeAlloction MEM_ALLOC_CREDIT_JOIN(                           \
      memAllocAttributeAlloction, __LINE__)(tag, __LINE__)
...
MemoryLogMessage(char const *s); // throw a message into the memory log
...
public:
  /*
  Note that this class does not overload operator new[] and delete[] which means
  that classes that depend on this for alignment may end up misaligned if an
  array is allocated. This problem is now mostly theoretical because this class
  is mostly obsolete.
  */
