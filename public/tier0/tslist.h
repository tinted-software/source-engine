...
#ifdef POSIX
inline bool ThreadInterlockedAssignIf128(int128 volatile *pDest,
                                         const int128 &value,
                                         const int128 &comparand) {
...
union TSLIST_HEAD_ALIGN TSLHead_t {
  struct Value_t {
    ...
    //          to avoid carrying digits from Sequence. Carrying digits from
    //          Depth to Sequence is ok, because Sequence can be pretty much
    //          random. We could operate on both of them separately, but it
    //          could perhaps (?) lead to problems with store forwarding. I
    //          don't know 'cause I didn't performance-test or design original
    //          code, I'm just making it work on PowerPC.
...
public:
  ...
  static void *operator new(size_t size) {
    CTSListBase *pNode = (CTSListBase *)MemAlloc_AllocAlignedFileLine(
...
public:
  ...
  TSLNodeBase_t *Push(TSLNodeBase_t *pNode) {
    ...
    for (;;) {
      ...
      if (ThreadInterlockedAssignIf64x128(
...
// on T's memory
template <class T> class TSLIST_HEAD_ALIGN CTSPool : public CTSListBase {
...
template <typename T> class TSLIST_HEAD_ALIGN CTSList : public CTSListBase {
public:
  struct TSLIST_NODE_ALIGN Node_t : public TSLNodeBase_t {
    ...
    // override new/delete so we can guarantee 8-byte aligned allocs
    static void *operator new(size_t size) {
      Node_t *pNode = (Node_t *)MemAlloc_AllocAlignedFileLine(
...
public:
  // override new/delete so we can guarantee 8-byte aligned allocs
  static void *operator new(size_t size) {
    CTSQueue *pNode = (CTSQueue *)MemAlloc_AllocAlignedFileLine(
        size, TSLIST_HEAD_ALIGNMENT, __FILE__, __LINE__);
    return pNode;
  }
...
public:
  ...
  union TSLIST_HEAD_ALIGN NodeLink_t {
    // override new/delete so we can guarantee 8-byte aligned allocs
    static void *operator new(size_t size) {
      NodeLink_t *pNode = (NodeLink_t *)MemAlloc_AllocAlignedFileLine(
          size, TSLIST_HEAD_ALIGNMENT, __FILE__, __LINE__);
      return pNode;
    }
  ...
  Node_t *Push(Node_t *pNode) {
    ...
    FinishPush(
        pNode,
        oldTail); // This can fail if another thread pushed between the sequence
                  // and node grabs above. Later pushes or pops corrects
