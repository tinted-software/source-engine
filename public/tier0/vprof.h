...
// prior to the token concatenation, but just ignore the _INTERNAL macros and
// use the VPROF_VAR_NAME
...
public:
  ...
  enum CPUTraceState {
    kDisabled,
    kFirstHitNode, // record from the first time we hit the node until that node
                   // ends
...
#define MAX_GROUP_STACK_DEPTH 1024
  ...
  int GetBudgetGroupFlags(
      int budgetGroupID) const; // Returns a combination of BUDGETFLAG_ defines.
  ...
  int *
  FindOrCreateCounter(const tchar *pName,
                      CounterGroup_t eCounterGroup = COUNTER_GROUP_DEFAULT);
...
inline CVProfNode::CVProfNode(const tchar *pszName, int detailLevel,
                              CVProfNode *pParent,
                              const tchar *pBudgetGroupName, int budgetFlags)
    : m_pszName(pszName), m_nCurFrameCalls(0), m_nPrevFrameCalls(0),
      m_nRecursions(0), m_pParent(pParent), m_pChild(NULL), m_pSibling(NULL),
      m_iClientData(-1)
#ifdef _X360
      ,
      m_iBitFlags(0)
