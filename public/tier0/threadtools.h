...
PLATFORM_INTERFACE inline int32
ThreadInterlockedCompareExchange(int32 volatile *ea, int32 value,
                                 int32 comperand) {
  return cellAtomicCompareAndSwap32((uint32_t *)ea, comperand, value);
}
PLATFORM_INTERFACE inline bool
ThreadInterlockedAssignIf(int32 volatile *ea, int32 value, int32 comperand) {
  return (cellAtomicCompareAndSwap32((uint32_t *)ea, comperand, value) ==
          (uint32_t)comperand);
}

PLATFORM_INTERFACE inline int64
ThreadInterlockedCompareExchange64(int64 volatile *pDest, int64 value,
                                   int64 comperand) {
  return cellAtomicCompareAndSwap64((uint64_t *)pDest, comperand, value);
}
PLATFORM_INTERFACE inline bool
ThreadInterlockedAssignIf64(volatile int64 *pDest, int64 value,
                            int64 comperand) {
  return (cellAtomicCompareAndSwap64((uint64_t *)pDest, comperand, value) ==
          (uint64_t)comperand);
}
...
PLATFORM_INTERFACE void *ThreadInterlockedExchangePointer(void *volatile *,
                                                          void *value) NOINLINE;
PLATFORM_INTERFACE void *
ThreadInterlockedCompareExchangePointer(void *volatile *, void *value,
                                        void *comperand) NOINLINE;
PLATFORM_INTERFACE bool
ThreadInterlockedAssignPointerIf(void *volatile *, void *value,
                                 void *comperand) NOINLINE;
#endif
...
inline void const *
ThreadInterlockedExchangePointerToConst(void const *volatile *p,
                                        void const *value) {
...
inline int64 ThreadInterlockedCompareExchange64(int64 volatile *p, int64 value,
                                                int64 comparand) {
...
PLATFORM_INTERFACE bool
ThreadInterlockedAssignIf128(volatile int128 *pDest, const int128 &value,
...
template <class T = int32> class CThreadLocalInt : public CThreadLocal<T> {
public:
...
public:
  ...
  bool AssignIf(T conditionValue, T newValue) {
    if (sizeof(T) == sizeof(int32))
      ...
      return ThreadInterlockedAssignIf64((int64 *)&m_value, (int64)newValue,
                                         (int64)conditionValue);
  }
...
public:
  ...
  T *operator++() {
    return ((T *)_InterlockedExchangeAdd64((volatile __int64 *)&m_value,
                                           sizeof(T))) +
           1;
  }
  T *operator++(int) {
  ...
  bool AssignIf(T *conditionValue, T *newValue) {
    return _InterlockedCompareExchangePointer((void *volatile *)&m_value,
                                              newValue,
                                              conditionValue) == conditionValue;
  }

  T *operator=(T *newValue) {
    _InterlockedExchangePointer((void *volatile *)&m_value, newValue);
    return newValue;
  }

  void operator+=(int add) {
    _InterlockedExchangeAdd64((volatile __int64 *)&m_value, add * sizeof(T));
  }
...
#define THREADINTERLOCKEDEXCHANGEADD(_dest, _value)                            \
  ThreadInterlockedExchangeAdd64((int64 *)(_dest), _value)
#else // PLATFORM_64BITS
    COMPILE_TIME_ASSERT(sizeof(T *) == sizeof(int32));
#define THREADINTERLOCKEDEXCHANGEADD(_dest, _value)                            \
  ThreadInterlockedExchangeAdd((int32 *)_dest, _value)
...
public:
  ...
  virtual bool Start(unsigned nBytesStack = 0,
                     ThreadPriorityEnum_t nPriority = TP_PRIORITY_DEFAULT);
...
private:
  ...
  // lwss add - Thread params. These were previously allocated on the heap and
  // leaked.
...
inline bool CThreadMutex::AssertOwnedByCurrentThread() {
  ...
  if (ThreadGetCurrentId() == m_currentOwnerID)
    return true;
  AssertMsg3(0, "Expected thread %u as owner of lock 0x%p, but %u owns",
             ThreadGetCurrentId(), (CRITICAL_SECTION *)&m_CriticalSection,
             m_currentOwnerID);
  return false;
#elif defined(_PS3)
...
inline CThreadRWLock::CThreadRWLock()
    : m_CanRead(true), m_nWriters(0), m_nActiveReaders(0),
      m_nPendingReaders(0) {}
