...
const char *MakeModuleFileName() {
  if (g_pMemAlloc && g_pMemAlloc->IsDebugHeap()) {
    char *pszModuleName = (char *)HeapAlloc(GetProcessHeap(), 0,
                                            MAX_PATH); // small leak, debug only
...
//-----------------------------------------------------------------------------
// Purpose: helper class to detect when static construction has been done by the
// CRT
//-----------------------------------------------------------------------------
...
#if !defined(_MSC_VER) ||                                                      \
    (_MSC_VER >= 1900) //  VC 2015 and above, with the UCRT, will crash if you
                       //  use a static before it is constructed
...
#else
ALLOC_CALL void *_malloc_base(size_t nSize) { return AllocUnattributed(nSize); }
#endif
...
inline void *MemAlloc_Unalign(void *pMemBlock) {
  ...
  pAlloc =
      (unsigned *)(((size_t)pAlloc & ~(sizeof(void *) - 1)) - sizeof(void *));
...
#if _MSC_VER >= 1900
int __cdecl _VCrtDbgReportA(int nRptType, void *ReturnAddress,
                            const char *szFile, int nLine, const char *szModule,
...
// 64-bit
#ifdef _WIN64
static void *__cdecl realloc_help(void *pUserData, size_t *pnNewSize,
                                  int nBlockUse, const char *szFileName,
                                  int nLine, int fRealloc) {
  assert(0); // Shouldn't be needed
  return NULL;
}
#else
static void *__cdecl realloc_help(void *pUserData, size_t nNewSize,
                                  int nBlockUse, const char *szFileName,
                                  int nLine, int fRealloc) {
  assert(0); // Shouldn't be needed
  return NULL;
...
static int __cdecl CheckBytes(unsigned char *pb, unsigned char bCheck,
                              size_t nSize) {
  int bOkay = TRUE;
  return bOkay;
}
...
#if _MSC_VER >= 1400
static void __cdecl _printMemBlockData(_locale_t plocinfo,
                                       _CrtMemBlockHeader *pHead) {}

static void __cdecl _CrtMemDumpAllObjectsSince_stat(const _CrtMemState *state,
                                                    _locale_t plocinfo) {}
#endif
...
LPVOID WINAPI XMemAlloc(SIZE_T dwSize, DWORD dwAllocAttributes) {
  ...
  if (!bPhysical && !pAttribs->dwHeapTracksAttributes &&
      pAttribs->dwAllocatorId != eXALLOCAllocatorId_XUI) {
    MEM_ALLOC_CREDIT();
