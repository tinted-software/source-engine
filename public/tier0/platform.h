...
// !!! NOTE: if you get a compile error here, you are using VALIGNOF on an
// abstract type :NOTE !!!
...
#else
#error                                                                         \
    "PORT: Code only tested with MSVC! Must validate with new compiler, and use built-in keyword if available."
#endif
...
#pragma warning(                                                               \
    disable : 4239) // nonstandard extension used : 'argument' ( conversion from
...
#pragma warning(                                                               \
    disable : 4748) // warning C4748: /GS can not protect parameters and local
                    // variables from local buffer overrun because optimizations
...
#pragma warning(                                                               \
    disable : 4530) // warning C4530: C++ exception handler used, but unwind
                    // semantics are not enabled. Specify /EHsc (disabled due to
                    // std headers having exception syntax)
#endif
...
#else
FORCEINLINE uint32 LoadLittleDWord(const uint32 *base,
                                   unsigned int dwordIndex) {
  return LittleDWord(base[dwordIndex]);
}

FORCEINLINE void StoreLittleDWord(uint32 *base, unsigned int dwordIndex,
                                  uint32 dword) {
...
// called to exit the process due to a fatal error. This allows for the
// application to handle providing a hook as well which can be called before
// exiting
PLATFORM_INTERFACE void
Plat_ExitProcessWithError(int nCode, bool bGenerateMinidump = false);

// sets the callback that will be triggered by Plat_ExitProcessWithError. NULL
// is valid. The return value true indicates that the exit has been handled and
// no further processing should be performed. False will cause a minidump to be
// generated, and the process to be terminated
typedef bool (*ExitProcessWithErrorCBFn)(int nCode);
PLATFORM_INTERFACE void
Plat_SetExitProcessWithErrorCB(ExitProcessWithErrorCBFn pfnCB);
...
// Returns true if the passed in MemoryInformation structure was filled out,
// otherwise false.
...
PLATFORM_INTERFACE void
Plat_ApplyHardwareDataBreakpointsToNewThread(unsigned long dwThreadID);
...
// The above will error when binding to a type of: foo(*)[] -- there is no
// provision in c++ for knowing how many objects to destruct without preserving
// the count and calling the necessary destructors.
