...
class CBoneSetup {
public:
  CBoneSetup(const CStudioHdr *pStudioHdr, int boneMask,
             const float poseParameter[], IPoseDebugger *pPoseDebugger = NULL);
...
private:
  void AddSequenceLayers(Vector pos[], Quaternion q[],
                         mstudioseqdesc_t &seqdesc, int sequence, float cycle,
                         float flWeight, float flTime, CIKContext *pIKContext);
  void AddLocalLayers(Vector pos[], Quaternion q[], mstudioseqdesc_t &seqdesc,
                      int sequence, float cycle, float flWeight, float flTime,
                      CIKContext *pIKContext);
...
static void
CalcDecompressedAnimation(const mstudiocompressedikerror_t *pCompressed,
                          int iFrame, float fraq, Vector &pos, Quaternion &q) {
  if (fraq > 0.0001f) {
    Vector p1, p2;
...
static void CalcLocalHierarchyAnimation(
  ...
  static ALIGN16 matrix3x4_t rootXform ALIGN16_POST(1.0f, 0, 0, 0, 0, 1.0f, 0,
                                                    0, 0, 0, 1.0f, 0);
...
static void CalcZeroframeData(const CStudioHdr *pStudioHdr,
  ...
  if (animdesc.zeroframecount == 1) {
    ...
    if (index >= animdesc.zeroframecount - 1) {
      ...
      s1 = clamp((fFrame - index * animdesc.zeroframespan) /
...
static void CalcVirtualAnimation(virtualmodel_t *pVModel,
  ...
  mstudioanimdesc_t &animdesc =
      ((CStudioHdr *)pStudioHdr)->pAnimdesc(baseanimation);
  ...
  if (!panim) {
    CalcZeroframeData(((CStudioHdr *)pStudioHdr), pAnimStudioHdr, pAnimGroup,
                      pAnimbone, animdesc, fFrame, pos, q, boneMask, 1.0);
    return;
  }
    ...
    for (i = 0; i < animdesc.numlocalhierarchy; i++) {
      ...
      if (iBone >= 0 && (pStudioHdr->boneFlags(iBone) & boneMask)) {
        if (pHierarchy->iNewParent != -1) {
          ...
          if (iNewParent >= 0 &&
              (pStudioHdr->boneFlags(iNewParent) & boneMask)) {
          ...
          CalcLocalHierarchyAnimation(pStudioHdr, boneToWorld, boneComputed,
                                      pos, q, pbone, pHierarchy, iBone, -1,
...
static void CalcAnimation(const CStudioHdr *pStudioHdr, Vector *pos,
  ...
  mstudioanimdesc_t &animdesc =
      ((CStudioHdr *)pStudioHdr)->pAnimdesc(animation);
...
void Studio_LocalPoseParameter(const CStudioHdr *pStudioHdr,
  ...
  const mstudioposeparamdesc_t &Pose =
      ((CStudioHdr *)pStudioHdr)->pPoseParameter(iPose);
...
bool CalcPoseSingle(const CStudioHdr *pStudioHdr, Vector pos[], Quaternion q[],
  ...
  if (seqdesc.flags & STUDIO_REALTIME) {
    ...
    if (iPose != -1) {
      /*
      const mstudioposeparamdesc_t &Pose = ((CStudioHdr
      *)pStudioHdr)->pPoseParameter( iPose ); cycle = poseParameter[ iPose ] *
      (Pose.end - Pose.start) + Pose.start;
      */
...
//-----------------------------------------------------------------------------
void CBoneSetup::AddSequenceLayers(Vector pos[], Quaternion q[],
                                   mstudioseqdesc_t &seqdesc, int sequence,
                                   float cycle, float flWeight, float flTime,
                                   CIKContext *pIKContext) {
  for (int i = 0; i < seqdesc.numautolayers; i++) {
    ...
    if (pLayer->start != pLayer->end) {
      ...
      if (!(pLayer->flags & STUDIO_AL_POSE)) {
        ...
        int iPose =
            m_pStudioHdr->GetSharedPoseParameter(iSequence, pLayer->iPose);
        if (iPose != -1) {
          const mstudioposeparamdesc_t &Pose =
              ((CStudioHdr *)m_pStudioHdr)->pPoseParameter(iPose);
          index =
              m_flPoseParameter[iPose] * (Pose.end - Pose.start) + Pose.start;
        } else {
          index = 0;
        }
      }
    ...
    int iSequence = m_pStudioHdr->iRelativeSeq(sequence, pLayer->iSequence);
    AccumulatePose(pos, q, iSequence, layerCycle, layerWeight, flTime,
                   pIKContext);
  }
}
...
//			adds autolayers, runs local ik rukes
//-----------------------------------------------------------------------------
void CBoneSetup::AddLocalLayers(Vector pos[], Quaternion q[],
                                mstudioseqdesc_t &seqdesc, int sequence,
                                float cycle, float flWeight, float flTime,
                                CIKContext *pIKContext) {
...
IBoneSetup::IBoneSetup(const CStudioHdr *pStudioHdr, int boneMask,
                       const float poseParameter[],
                       IPoseDebugger *pPoseDebugger) {
  m_pBoneSetup =
      new CBoneSetup(pStudioHdr, boneMask, poseParameter, pPoseDebugger);
}
...
void IBoneSetup::AccumulatePose(Vector pos[], Quaternion q[], int sequence,
                                float cycle, float flWeight, float flTime,
                                CIKContext *pIKContext) {
  m_pBoneSetup->AccumulatePose(pos, q, sequence, cycle, flWeight, flTime,
                               pIKContext);
}

void IBoneSetup::CalcAutoplaySequences(Vector pos[], Quaternion q[],
                                       float flRealTime,
                                       CIKContext *pIKContext) {
...
// to pos[], and q[].
void IBoneSetup::CalcBoneAdj(Vector pos[], Quaternion q[],
                             const float controllers[]) {
  ::CalcBoneAdj(m_pBoneSetup->m_pStudioHdr, pos, q, controllers,
                m_pBoneSetup->m_boneMask);
}
...
CBoneSetup::CBoneSetup(const CStudioHdr *pStudioHdr, int boneMask,
                       const float poseParameter[],
                       IPoseDebugger *pPoseDebugger) {
...
//			adds autolayers, runs local ik rukes
//-----------------------------------------------------------------------------
void CBoneSetup::AccumulatePose(Vector pos[], Quaternion q[], int sequence,
                                float cycle, float flWeight, float flTime,
                                CIKContext *pIKContext) {
  ...
  if (m_pPoseDebugger) {
    m_pPoseDebugger->AccumulatePose(m_pStudioHdr, pIKContext, pos, q, sequence,
                                    cycle, m_flPoseParameter, m_boneMask,
                                    flWeight, flTime);
  }
#endif
  ...
  if (seqdesc.numiklocks) {
    seq_ik.Init(
        m_pStudioHdr, vec3_angle, vec3_origin, 0.0, 0,
  ...
  if (CalcPoseSingle(m_pStudioHdr, pos2, q2, seqdesc, sequence, cycle,
                     m_flPoseParameter, m_boneMask, flTime)) {
    ...
    SlerpBones(m_pStudioHdr, q, pos, seqdesc, sequence, q2, pos2, flWeight,
               m_boneMask);
  }

  if (pIKContext) {
    pIKContext->AddDependencies(seqdesc, sequence, cycle, m_flPoseParameter,
                                flWeight);
  }

  AddSequenceLayers(pos, q, seqdesc, sequence, cycle, flWeight, flTime,
                    pIKContext);
...
void CIKContext::AddAutoplayLocks(Vector pos[], Quaternion q[]) {
  ...
  for (int i = 0; i < m_pStudioHdr->GetNumIKAutoplayLocks(); i++) {
    const mstudioiklock_t &lock =
        ((CStudioHdr *)m_pStudioHdr)->pIKAutoplayLock(i);
      ...
      case IK_GROUND:
        // case IK_SELF:
        {
          ...
          if (pRule->flRuleWeight == 1.0f || pTarget->est.flWeight == 0.0f) {
            ...
            pTarget->est.release =
                Lerp(pRule->flRuleWeight, pTarget->est.release, pRule->release);
...
void CIKContext::SolveDependencies(Vector pos[], Quaternion q[],
  ...
  for (j = 0; j < m_ikChainRule.Count(); j++) {
    for (i = 0; i < m_ikChainRule.Element(j).Count(); i++) {
      ...
      case IK_UNLATCH: {
        /*
        pChainResult->flWeight = pChainResult->flWeight * (1 - pRule->flWeight)
        + pRule->flWeight;

        pChainResult->pos = pChainResult->pos * (1.0 - pRule->flWeight ) +
        pChainResult->local.pos * pRule->flWeight; QuaternionSlerp(
        pChainResult->q, pChainResult->local.q, pRule->flWeight, pChainResult->q
        );
        */
      } break;
      }
...
// poseParameters
//-----------------------------------------------------------------------------
void CBoneSetup::CalcAutoplaySequences(Vector pos[], Quaternion q[],
                                       float flRealTime,
                                       CIKContext *pIKContext) {
  //	ASSERT_NO_REENTRY();
  ...
  for (i = 0; i < count; i++) {
    ...
    mstudioseqdesc_t &seqdesc =
        ((CStudioHdr *)m_pStudioHdr)->pSeqdesc(sequenceIndex);
    if (seqdesc.flags & STUDIO_AUTOPLAY) {
      float cycle = 0;
      float cps =
          Studio_CPS(m_pStudioHdr, seqdesc, sequenceIndex, m_flPoseParameter);
...
//that column 			points down the 6 cardinal axis (all negative weights are clamped
//to 0).
//-----------------------------------------------------------------------------
...
void DoAimAtBone(mstudiobone_t *pBones, int iBone, CBoneAccessor &bonetoworld,
  ...
  */
  ...
  if (pStudioHdr) {
    ...
    const mstudioattachment_t &attachment(
        ((CStudioHdr *)pStudioHdr)->pAttachment(pProc->aim));
...
void Studio_CalcDefaultPoseParameters(const CStudioHdr *pStudioHdr,
                                      float flPoseParameter[], int nCount) {
  ...
  for (int i = 0; i < nNumParams; ++i) {
    ...
    if (i < nPoseCount) {
      const mstudioposeparamdesc_t &Pose =
          ((CStudioHdr *)pStudioHdr)->pPoseParameter(i);

      // Want to try for a zero state.  If one doesn't exist set it to .5 by
      // default.
...
float Studio_SetPoseParameter(const CStudioHdr *pStudioHdr, int iParameter,
  ...
  const mstudioposeparamdesc_t &PoseParam =
      ((CStudioHdr *)pStudioHdr)->pPoseParameter(iParameter);
...
bool TraceToStudio(IPhysicsSurfaceProps *pProps, const Ray_t &ray,
                   CStudioHdr *pStudioHdr, mstudiohitboxset_t *set,
                   matrix3x4_t **hitboxbones, int fContentsMask,
                   const Vector &vecOrigin, float flScale, trace_t &tr) {
  if (!ray.m_IsRay) {
  ...
  for (int i = 0; i < set->numhitboxes; i++) {
    ...
    // Because we're sending in a matrix with scale data, and because the matrix
    // inversion in the hitbox code does not handle that case, we pre-scale the
    // bones and ray down here and do our collision checks in unscaled space. We
    // can then rescale the results afterwards.
...
void Studio_SeqAnims(const CStudioHdr *pStudioHdr, mstudioseqdesc_t &seqdesc,
  ...
  panim[0] = &((CStudioHdr *)pStudioHdr)
                  ->pAnimdesc(pStudioHdr->iRelativeAnim(iSequence,
                                                        seqdesc.anim(i0, i1)));
  weight[0] = (1 - s0) * (1 - s1);

  panim[1] = &((CStudioHdr *)pStudioHdr)
                  ->pAnimdesc(pStudioHdr->iRelativeAnim(
                      iSequence, seqdesc.anim(i0 + 1, i1)));
  weight[1] = (s0) * (1 - s1);

  panim[2] = &((CStudioHdr *)pStudioHdr)
                  ->pAnimdesc(pStudioHdr->iRelativeAnim(
                      iSequence, seqdesc.anim(i0, i1 + 1)));
  weight[2] = (1 - s0) * (s1);

  panim[3] = &((CStudioHdr *)pStudioHdr)
                  ->pAnimdesc(pStudioHdr->iRelativeAnim(
                      iSequence, seqdesc.anim(i0 + 1, i1 + 1)));
  weight[3] = (s0) * (s1);
...
bool Studio_AnimPosition(mstudioanimdesc_t *panim, float flCycle,
  ...
  for (int i = 0; i < panim->nummovements; i++) {
    ...
    if (pmove.endframe >= flFrame) {
      ...
      if (iLoops != 0) {
        mstudiomovement_t *pmoveAnim =
            panim->pMovement(panim->nummovements - 1);
...
// animation cycle Output:	updated position and angle, relative to
// CycleFrom being at the origin
//			returns false if animation is not a movement animation
//-----------------------------------------------------------------------------
...
// sequences cycle Output:	updated position and angle, relative to
// CycleFrom being at the origin
...
int Studio_FindRandomAttachment(const CStudioHdr *pStudioHdr,
  ...
  if (pStudioHdr) {
    ...
    // Extract the bone index from the name
    for (int i = 0; i < pStudioHdr->GetNumAttachments(); i++) {
      if (strstr(((CStudioHdr *)pStudioHdr)->pAttachment(i).pszName(),
                 pAttachmentName)) {
...
bool Studio_PrefetchSequence(const CStudioHdr *pStudioHdr, int iSequence) {
  ...
  for (int i = 0; i < size0; ++i) {
    for (int j = 0; j < size1; ++j) {
      mstudioanimdesc_t &animdesc =
          ((CStudioHdr *)pStudioHdr)->pAnimdesc(seqdesc.anim(i, j));
      int iFrame = 0;
...
//-----------------------------------------------------------------------------
void Studio_RunBoneFlexDrivers(float *pflFlexControllerWeights,
                               const CStudioHdr *pStudioHdr,
                               const Vector *pvPositions,
  ...
  for (int i = 0; i < nBoneFlexDriverCount; ++i) {
    const mstudioboneflexdriver_t *pBoneFlexDriver =
        pStudioHdr->BoneFlexDriver(i);
    const mstudiobone_t *pStudioBone =
        pStudioHdr->pBone(pBoneFlexDriver->m_nBoneIndex);

    const int nControllerCount = pBoneFlexDriver->m_nControlCount;

    if (pStudioBone->flags & BONE_USED_BY_BONE_MERGE) {
      // The local space version of the bone is not available if this is a
      // bonemerged bone so do the slow computation of the local version of the
      // bone from boneToWorld
      ...
      if (pStudioBone->parent < 0) {
        ...
        MatrixMultiply(mRootToWorldInv,
                       pBoneToWorld[pBoneFlexDriver->m_nBoneIndex], mBoneLocal);
      } else {
        MatrixInvert(pBoneToWorld[pStudioBone->parent], mParentInv);
        MatrixMultiply(mParentInv, pBoneToWorld[pBoneFlexDriver->m_nBoneIndex],
                       mBoneLocal);
      }

      for (int j = 0; j < nControllerCount; ++j) {
        const mstudioboneflexdrivercontrol_t *pController =
            pBoneFlexDriver->pBoneFlexDriverControl(j);
        const mstudioflexcontroller_t *pFlexController =
            pStudioHdr->pFlexcontroller(static_cast<LocalFlexController_t>(
                pController->m_nFlexControllerIndex));
        ...
        Assert(pController->m_nBoneComponent >= 0 &&
               pController->m_nBoneComponent <= 2);
        pflFlexControllerWeights[pFlexController->localToGlobal] =
            RemapValClamped(mBoneLocal[pController->m_nBoneComponent][3],
                            pController->m_flMin, pController->m_flMax, 0.0f,
                            1.0f);
      }
    } else {
      // Use the local space version of the bone directly for non-bonemerged
      // bones
      ...
      for (int j = 0; j < nControllerCount; ++j) {
        ...
        Assert(pController->m_nBoneComponent >= 0 &&
               pController->m_nBoneComponent <= 2);
        pflFlexControllerWeights[pFlexController->localToGlobal] =
            RemapValClamped(position[pController->m_nBoneComponent],
                            pController->m_flMin, pController->m_flMax, 0.0f,
                            1.0f);
      }
    }
