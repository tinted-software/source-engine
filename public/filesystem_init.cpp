...
#define BASESOURCEPATHS_TOKEN "|all_source_engine_paths|"
  ...
  if (ExtraVpkPaths) {
    ...
    FOR_EACH_VEC(vecPaths, idxExtraPath) {
      FileSystem_AddLoadedSearchPath(initInfo, "PLATFORM",
                                     vecPaths[idxExtraPath], false);
      FileSystem_AddLoadedSearchPath(initInfo, "GAME", vecPaths[idxExtraPath],
                                     false);
    }
  }
...
FSReturnCode_t FileSystem_GetFileSystemDLLName(char *pFileSystemDLL,
  ...
  // Assume we'll use local files
  Q_snprintf(pFileSystemDLL, nMaxLen, "%s%clibfilesystem_stdio" DLL_EXT_STRING,
