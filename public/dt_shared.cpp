...
DataTableProp
PropFloat(char *pVarName, // Variable name.
...
#if !defined(CLIENT_DLL)
  return SendPropFloat(pVarName, offset, sizeofVar, nBits, flags, fLowValue,
                       fHighValue);
#else
...
DataTableProp
PropVector(char *pVarName, int offset, int sizeofVar,
           int nBits, // Number of bits (for each floating-point component) to
                      // use when encoding.
           int flags,
           float fLowValue, // For floating point, low and high values.
           float fHighValue // High value. If HIGH_DEFAULT, it's (1<<nBits).
...
#if !defined(CLIENT_DLL)
  return SendPropVector(pVarName, offset, sizeofVar, nBits, flags, fLowValue,
                        fHighValue);
#else
...
DataTableProp PropAngle(char *pVarName, int offset, int sizeofVar, int nBits,
                        int flags) {
#if !defined(CLIENT_DLL)
...
DataTableProp PropInt(char *pVarName, int offset,
                      ...
                      int nBits, // Set to -1 to automatically pick (max) number
                                 // of bits based on size of element.
