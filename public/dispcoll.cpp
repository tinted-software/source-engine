...
const float CDispCollTree::ONE_MINUS_COLLISION_EPSILON =
    1.0f - COLLISION_EPSILON;
...
// Purpose:
//-----------------------------------------------------------------------------
void CDispCollTree::CreateNodes_r(CCoreDispInfo *pDisp, int nodeIndex,
                                  int termLevel) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CDispCollTree::RayTriTest(Vector const &rayStart, Vector const &rayDir,
  ...
  //
  // utility calculations - inverse determinant and distance from v0 to ray
  // start
  //
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CDispCollTree::RayTriListTest(CDispCollTreeTempData *pTemp,
                                   CDispCollData *pData) {
  ...
  for (int i = 0; i < pTemp->m_TriListCount; i++) {
    float fraction = 1.0f;
    bool bResult = RayTriTest(pData->m_StartPos, rayDir, rayLength,
                              pTemp->m_ppTriList[i], &fraction);
    if (!bResult)
      continue;
...
// Purpose:
//-----------------------------------------------------------------------------
bool CDispCollTree::RayAABBTest(CDispCollTreeTempData *pTemp, Vector &rayStart,
                                Vector &rayEnd) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CDispCollTree::CreatePlanesFromBounds(CDispCollTreeTempData *pTemp,
                                           Vector const &bbMin,
                                           Vector const &bbMax) {
  //
...
// Purpose:
//-----------------------------------------------------------------------------
void CDispCollTree::RayNodeTest_r(CDispCollTreeTempData *pTemp, int nodeIndex,
...
bool CDispCollTree::RayTestAllTris(CDispCollData *pData, int power) {
  ...
  for (int index = startIndex; index < endIndex; index++) {
    for (int j = 0; j < 2; j++) {
      bool bResult = RayTriTest(pData->m_StartPos, rayDir, rayLength,
                                &m_pNodes[index].m_Tris[j], &fraction);
      if (!bResult)
        continue;
...
// Purpose:
//-----------------------------------------------------------------------------
bool CDispCollTree::SweptAABBTriIntersect(Vector &rayStart, Vector &rayEnd,
                                          Vector &extents,
  ...
  *fraction = 1.0f;
  ...
  for (dir = 0; dir < 3; dir++) {
    ...
    } else if ((distStart < -COLLISION_EPSILON) &&
               (distEnd > COLLISION_EPSILON)) {
      t = (distStart + COLLISION_EPSILON) / (distStart - distEnd);
      VectorScale(rayDir, t, rayPt);
  ...
  dist = (normal.y * (boxPt.y - pTri->m_Points[0].y)) +
         (normal.z * (boxPt.z - pTri->m_Points[0].z));
  ...
  // extents adjusted dist
  dist = (normal.x * (boxPt.x - pTri->m_Points[0].x)) +
         (normal.z * (boxPt.z - pTri->m_Points[0].z));
  ...
  // extents adjusted dist
  dist = (normal.x * (boxPt.x - pTri->m_Points[0].x)) +
         (normal.y * (boxPt.y - pTri->m_Points[0].y));
...
// Purpose:
//-----------------------------------------------------------------------------
bool CDispCollTree::AABBTriIntersect(CDispCollTreeTempData *pTemp,
                                     CDispCollData *pData) {
  bool bResult = false;
  ...
  for (int i = 0; i < pTemp->m_TriListCount; i++) {
    if (pTemp->m_ppTriList[i]->IsIntersect()) {
      bResult = SweptAABBTriIntersect(pData->m_StartPos, pData->m_EndPos,
                                      pData->m_Extents, pTemp->m_ppTriList[i],
...
bool CDispCollTree::IntersectAABBTriTest(Vector &rayStart, Vector &extents,
  ...
  //
  // add a test here to see if triangle face normal is close to axial -- done if
  // so!!!
  //
  if ((pTri->m_Normal[0] > ONE_MINUS_COLLISION_EPSILON) ||
      (pTri->m_Normal[1] > ONE_MINUS_COLLISION_EPSILON) ||
  ...
  normal.y = -edge.z;
  normal.z = edge.y;
  if (((normal.y * (boxPt.y - pTri->m_Points[0].y)) +
       (normal.z * (boxPt.z - pTri->m_Points[0].z))) > 0.0f)
    return false;
  ...
  normal.y = 0.0f;
  normal.z = edge.y;
  if (((normal.x * (boxPt.x - pTri->m_Points[0].x)) +
       (normal.z * (boxPt.z - pTri->m_Points[0].z))) > 0.0f)
    return false;
  ...
  normal.z = 0.0f;
  if (((normal.x * (boxPt.x - pTri->m_Points[0].x)) +
       (normal.y * (boxPt.y - pTri->m_Points[0].y))) > 0.0f)
    return false;
...
// Purpose:
//-----------------------------------------------------------------------------
bool CDispCollTree::SweptAABBTriTest(Vector &rayStart, Vector &rayEnd,
                                     Vector &extents,
                                     CDispCollTri const *pTri) {
  // get ray direction
  Vector rayDir = rayEnd - rayStart;
  ...
  //
  // quick and dirty test -- test to see if the object is traveling away from
  // triangle surface???
  //
  ...
  //
  // calc the swept triangle face (negate the ray -- opposite direction of box
  // travel)
  //
  rayDir.Negate();
  ...
  // find the closest box point - (is written opposite to normal due to negating
  // ray)
  Vector boxPt(0.0f, 0.0f, 0.0f);
  for (dir = 0; dir < 3; dir++) {
    if (rayDir[dir] < 0.0f) {
  ...
  normal.y = -edge.z;
  normal.z = edge.y;
  if (((normal.y * (boxPt.y - points[0].y)) +
       (normal.z * (boxPt.z - points[0].z))) > 0.0f)
    return false;
  ...
  normal.y = 0.0f;
  normal.z = edge.y;
  if (((normal.x * (boxPt.x - points[0].x)) +
       (normal.z * (boxPt.z - points[0].z))) > 0.0f)
    return false;
  ...
  normal.z = 0.0f;
  if (((normal.x * (boxPt.x - points[0].x)) +
       (normal.y * (boxPt.y - points[0].y))) > 0.0f)
    return false;
...
// Purpose:
//-----------------------------------------------------------------------------
bool CDispCollTree::CullTriList(CDispCollTreeTempData *pTemp, Vector &rayStart,
                                Vector &rayEnd, Vector &extents,
                                bool bIntersect) {
  //
...
// Purpose:
//-----------------------------------------------------------------------------
bool CDispCollTree::IntersectAABBAABBTest(CDispCollTreeTempData *pTemp,
                                          const Vector &pos,
...
// Purpose:
//-----------------------------------------------------------------------------
bool CDispCollTree::SweptAABBAABBTest(CDispCollTreeTempData *pTemp,
  ...
  for (dir = 0; dir < 3; dir++) {
    ...
    //
    distStart =
        -(rayStart[dir] - (pTemp->m_AABBDistances[(dir >> 1)] - extents[dir]));
    distEnd =
        -(rayEnd[dir] - (pTemp->m_AABBDistances[(dir >> 1)] - extents[dir]));
    ...
    //
    distStart =
        rayStart[dir] - (pTemp->m_AABBDistances[(dir >> 1) + 1] + extents[dir]);
    distEnd =
        rayEnd[dir] - (pTemp->m_AABBDistances[(dir >> 1) + 1] + extents[dir]);
...
// Purpose:
//-----------------------------------------------------------------------------
void CDispCollTree::BuildTriList_r(CDispCollTreeTempData *pTemp, int nodeIndex,
                                   Vector &rayStart, Vector &rayEnd,
  ...
  if (bResult) {
    ...
    // continue recursion
      ...
      BuildTriList_r(pTemp, GetChildNode(nodeIndex, 0), rayStart, rayEnd,
                     extents, bIntersect);
      BuildTriList_r(pTemp, GetChildNode(nodeIndex, 1), rayStart, rayEnd,
                     extents, bIntersect);
      BuildTriList_r(pTemp, GetChildNode(nodeIndex, 2), rayStart, rayEnd,
                     extents, bIntersect);
      BuildTriList_r(pTemp, GetChildNode(nodeIndex, 3), rayStart, rayEnd,
                     extents, bIntersect);
    }
  }
}
...
bool CDispCollTree::AABBSweep(CDispCollData *pData) {
  ...
  BuildTriList_r(&tmp, 0, pData->m_StartPos, pData->m_EndPos, pData->m_Extents,
                 false);

  // find collision triangles
  if (CullTriList(&tmp, pData->m_StartPos, pData->m_EndPos, pData->m_Extents,
                  false)) {
...
bool CDispCollTree::AABBIntersect(CDispCollData *pData) {
  ...
  // sweep the AABB against the tree
  BuildTriList_r(&tmp, 0, pData->m_StartPos, pData->m_StartPos,
                 pData->m_Extents, true);

  // find collision triangles
  return CullTriList(&tmp, pData->m_StartPos, pData->m_StartPos,
                     pData->m_Extents, true);
}
