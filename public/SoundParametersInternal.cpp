...
static SoundChannels g_pChannelNames[] = {
    ...
    {CHAN_STATIC, "CHAN_STATIC"}, {CHAN_VOICE2, "CHAN_VOICE2"},
};
...
int TextToChannel(const char *name) {
  ...
  DevMsg("CSoundEmitterSystem:  Warning, unknown channel type in sounds.txt "
         "(%s)\n",
         name);
...
void CSoundParametersInternal::CopyFrom(const CSoundParametersInternal &src) {
  ...
  if (m_nConvertedNames) {
    if (m_nConvertedNames > 1) {
      m_pConvertedNames =
          (SoundFile *)malloc(sizeof(SoundFile) * m_nConvertedNames);
...
void CSoundParametersInternal::AddToTail(SoundFile **pDest, uint16 *pDestCount,
