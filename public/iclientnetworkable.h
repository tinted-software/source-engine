...
enum ShouldTransmitState_t {
  SHOULDTRANSMIT_START =
      0, // The entity is starting to be transmitted (maybe it entered the PVS).
  ...
  SHOULDTRANSMIT_END // Called when the entity isn't being transmitted by the
                     ...
                     // until next time the server wants to transmit its state.
};
...
enum DataUpdateType_t {
  ...
  //	DATA_UPDATE_DESTROYED,		// FIXME: Could enable this, but it's a
  //little worrying
...
public:
  ...
  //
  // In 90% of the cases, you should hook OnPreDataChanged/OnDataChanged instead
  // of PreDataUpdate/PostDataUpdate.
  //
  // The DataChanged events are only called once per frame whereas
  // Pre/PostDataUpdate are called once per packet (and sometimes multiple times
  // per frame).
  //
  // OnDataChanged is called during simulation where entity origins are correct
  // and attachments can be used. whereas PostDataUpdate is called while parsing
  // packets so attachments and other entity origins may not be valid yet.
  //
