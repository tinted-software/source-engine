...
public:
  ...
  // extended arrays include their element name as an object in JSON and VDF
  // output formats
  ...
  bool BEmitFormattedOutput(EWebAPIOutputFormat eFormat,
                            CUtlBuffer &outputBuffer, size_t unMaxResultSize);
...
private:
  void InitInternal(CWebAPIValues *pParent, int nNamePos,
                    EWebAPIValueType eValueType,
                    const char *pchArrayElementNames);
  CWebAPIValues(CWebAPIValues *pParent, int nNamePos,
                EWebAPIValueType eValueType,
                const char *pchArrayElementNames = NULL);
  CWebAPIValues(CWebAPIValues *pParent, const char *pchName,
                EWebAPIValueType eValueType,
                const char *pchArrayElementNames = NULL);
...
public:
  ...
  // Add a child object to the array, this should only be called on objects that
  // are of the array type
  CWebAPIValues *AddChildObjectToArray();

  // Add a child array to the  array, this should only be called on objects that
  // are of the array type
  ...
  // Find first matching child by name, O(N) on number of children, this class
  // isn't designed for searching
  ...
  // Call this on the returned value from GetFirstChild() or a previous
  ...
  const char *GetName() const {
    return m_nNamePos >= 0
               ? ((const char *)m_pStringBuffer->Base() + m_nNamePos)
               : NULL;
  }

  // get the name of the elements of this numeric array (if this is an array)
  const char *GetElementName() const {
    return m_nArrayChildElementNamePos >= 0
               ? ((const char *)m_pStringBuffer->Base() +
                  m_nArrayChildElementNamePos)
               : NULL;
  }
  ...
  // Set binary blob value
  void SetChildBinaryValue(const char *pchChildName, const uint8 *pValue,
                           uint32 unBytes);
  ...
  // Get uint32 value
  uint32 GetChildUInt32Value(const char *pchChildName,
                             uint32 unDefault = 0) const;

  // Get int64 value
  int64 GetChildInt64Value(const char *pchChildName, int64 lDefault = 0) const;

  // Get uint64 value
  uint64 GetChildUInt64Value(const char *pchChildName,
                             uint64 ulDefault = 0) const;

  // Get double value
  double GetChildDoubleValue(const char *pchChildName,
                             double flDefault = 0) const;

  // Get string value
  void GetChildStringValue(CUtlString &stringOut, const char *pchChildName,
                           const char *pchDefault) const;

  // Get binary blob value (returns false if the child wasn't found)
  bool BGetChildBinaryValue(CUtlBuffer &bufferOut,
  ...
  // Emits JSON formatted representation of response
  static bool BEmitJSONRecursive(const CWebAPIValues *pCurrent,
                                 CUtlBuffer &outputBuffer, int nTabLevel,
                                 size_t unMaxResultSize,
                                 bool bIncludeArrayElementName = true);

  // Emits KeyValues .vdf style formatted representation of response
  static bool BEmitVDFRecursive(const CWebAPIValues *pCurrent,
                                CUtlBuffer &outputBuffer, int nTabLevel,
                                uint32 nArrayElement, size_t unMaxResultSize,
                                bool bIncludeArrayElementName = true);

  // Emits XML formatted representation of response
  static bool BEmitXMLRecursive(const CWebAPIValues *pCurrent,
                                CUtlBuffer &outputBuffer, int nTabLevel,
                                size_t unMaxResultSize);
...
private:
  ...
  // Assert that we don't have any child nodes, this is used when setting a
  // native type value.  We don't support having both our own value and
...
#define FOR_EACH_WEBAPI_CHILD(pParentParam, pChildParam)                       \
  for (CWebAPIValues *pChildParam = pParentParam->GetFirstChild();             \
       pChildParam != NULL; pChildParam = pChildParam->GetNextChild())
...
public:
  CWebAPIValuesAD() { m_pwav = NULL; }

  // create a webapivalues object of the object type
  CWebAPIValuesAD(const char *pchName) { m_pwav = new CWebAPIValues(pchName); }
...
bool Base64Encode(const uint8 *pubData, uint32 cubData, char *pchEncodedData,
                  uint32 *pcchEncodedData, const char *pszLineBreak = NULL);
uint32 Base64EncodeMaxOutput(const uint32 cubData,
                             const char *pszLineBreak = NULL);

inline bool Base64EncodeIntoUTLMemory(const uint8 *pubData, uint32 cubData,
                                      CUtlMemory<char> &out_pBuffer) {
...
bool RecursiveAddProtoBufToWebAPIValues(GCSDK::CWebAPIValues *pWebAPIRoot,
                                        const ::google::protobuf::Message &msg);
bool ParseWebAPIValues(::google::protobuf::Message &msg,
                       const GCSDK::CWebAPIValues *pWebAPIRoot,
                       bool bEnforceRequired = false);
} // namespace ProtoBufHelper
