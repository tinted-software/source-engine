...
template <class T> class CRWLockAutoWrite {
  T &m_RWLock;

public:
  CRWLockAutoWrite(T &RWLock) : m_RWLock(RWLock) { m_RWLock.LockForWrite(); }

  ~CRWLockAutoWrite() { m_RWLock.UnlockWrite(); }
};

#define AUTO_LOCK_WRITE(mutex) CRWLockAutoWrite<CThreadRWLock> UNIQUE_ID(mutex)
#define AUTO_LOCK_SPIN_WRITE(mutex)                                            \
  CRWLockAutoWrite<CThreadSpinRWLock> UNIQUE_ID(mutex)
