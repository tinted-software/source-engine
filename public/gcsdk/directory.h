...
public:
  CGCDirTypeInstance(const char *pszTypeName, uint32 nType, uint32 nInstance,
                     const KeyValues *pKVConfig, const CGCDirProcess *pProcess);
  ~CGCDirTypeInstance();
...
class CGCDirProcess {
public:
  CGCDirProcess(uint32 nDirIndex, const char *pchProcessName,
                const char *pchProcessType, const KeyValues *pKVConfig);
  ~CGCDirProcess();
  ...
  // gets the list of unique types contained within this process as IDs (note
...
public:
  ...
  const CGCDirTypeInstance *GetGCInstanceForType(int32 nTypeID,
                                                 int32 nInstance) const;

  const char *GetNameForGCType(int32 nTypeID) const;
  int32 GetGCTypeForName(const char *pchName) const;

  void RegisterProcessTypeFactory(const char *pszProcessType,
                                  GCFactory_t pfnFactory);
  GCFactory_t GetFactoryForProcessType(const char *pszProcessType) const;

  // given a GC type, and an instance number, this will return the GC directory
  // index used for routing. If the instance number is greater than what is
  // available, it will be modulo'd into the acceptable range. If no match for
  // that type can be found, -1 will be returned
  int32 GetGCDirIndexForInstance(int32 nTypeID, uint32 nInstanceIndex) const;

  // given a GC type, this will add all of the GC indices associated with that
  // type onto the provided vector
  void GetGCDirIndicesForType(int32 nTypeID,
                              CUtlVector<uint32> &nIndices) const;
...
// the maximum value that a GC type can use. This is to enable bit masks to
// encode type information primarily (also don't allow the high bit to avoid SQL
// signed issues)
...
#define REG_GC_PROCESS_TYPE(gcClass, typeName)                                 \
  ...
  public:                                                                      \
    static CGCBase *Factory(const CGCDirProcess *pDirGC) {                     \
      return new gcClass(pDirGC);                                              \
    }                                                                          \
    CRegProcessType_##typeName() {                                             \
      GDirectory()->RegisterProcessTypeFactory((#typeName), &Factory);         \
    }                                                                          \
