...
// ENABLE_SO_OVERWRITE_PARANOIA can be set to either 0 or 1. If enabled, it will
// add extra fields to every CSharedObject instance to try and detect overwrites
// at the cost of additional runtime memory.
#define ENABLE_SO_OVERWRITE_PARANOIA 0

// ENABLE_SO_CONSTRUCT_DESTRUCT_PARANOIA can be set to either 0 or 1. If
// enabled, it will add extra fields to every CSharedObject instance to try and
// detect issues with constructions/destruction (ie., double-deletes, etc.),
...
//			a gameserver/client. These can also be stored in the
//database.
//----------------------------------------------------------------------------
class CSharedObject {
...
public:
  ...
  virtual ~CSharedObject() {
    ...
    AssertMsg2(m_nRefCount == 0,
               "Deleting shared object type %d with refcount %d", m_nSOTypeID,
               m_nRefCount);
  ...
  static void RegisterFactory(int nTypeID, SOCreationFunc_t fnFactory,
  ...
  virtual bool BYieldingAddWriteToTransaction(CSQLAccess &sqlAccess,
  ...
  bool BSendCreateToSteamID(const CSteamID &steamID, const CSteamID &ownerID,
                            uint64 ulVersion) const;
  bool BSendDestroyToSteamID(const CSteamID &steamID, const CSteamID &ownerID,
                             uint64 ulVersion) const;
...
protected:
/*
        // Dirty bit modification. Do not call these directly on SharedObjects.
   Call them
        // on the cache that owns the object so they can be added/removed from
   the right lists. virtual void DirtyField( int nField ) = 0; virtual void
   MakeDatabaseClean() = 0; virtual void MakeNetworkClean() = 0;
*/
#endif // GC
...
// this class manages the stats for the shared objects, such as how many exist,
// how many have been sent, how many have been created, how many destroyed
class CSharedObjectStats {
public:
  CSharedObjectStats();

  // called to register a shared object class given a type ID and name. This
  // will return false if a type of this ID is already registered
  void RegisterSharedObjectType(int nTypeID, const char *pszTypeName);

  // called to track when a shared object is created/destroyed to update the
  // running total
...
private:
  struct SOStats_t {
    ...
    CUtlString m_sName;
    // the number of outstanding SO cache objects of this type. This is not
    // cleared
    uint32 m_nNumActive;
    // the type ID of this stat
    int m_nTypeID;

    // the total number that have been created/destroyed. Active is difference
    // between these
    uint32 m_nNumCreated;
    uint32 m_nNumDestroyed;
    // how many bytes we have sent (raw, and multiplexed to multiple
    // subscribers)
  ...
  // sort function that controls the order that the SO stats are presented in
  // the report
  static bool SortSOStatsSent(const SOStats_t *pLhs, const SOStats_t *pRhs);
  static bool SortSOStatsSubscribe(const SOStats_t *pLhs,
                                   const SOStats_t *pRhs);

  // to compact the memory space for wide ranging type IDs, the list of stats
  // has two components, an array that can be looked up via the type ID which
  // maps to a stat index (or invalid index if no mapping is registered)
...
// Version that always asserts and returns NULL, for SOs that should not be
// auto-created this way
template <int nSharedObjectType>
CSharedObject *CreateSharedObjectSubclassProhibited() {
  AssertMsg(false,
            "Attempting to auto-create object of type %d which does not allow "
            "SO-based creation",
            nSharedObjectType);
  return NULL;
}
...
#ifdef GC
#define REG_SHARED_OBJECT_SUBCLASS(derivedClass, flags)                        \
  GCSDK::CSharedObject::RegisterFactory(                                       \
      derivedClass::k_nTypeID,                                                 \
      GCSDK::CreateSharedObjectSubclass<derivedClass>, (flags), #derivedClass)
// GC only -- sharedobjects that cannot be auto-created by the SO code, and
// might not have a specific derived class that publicly implements the
// interface
#define REG_SHARED_OBJECT_SUBCLASS_BY_ID_NOCREATE(strName, nTypeID, flags)     \
  GCSDK::CSharedObject::RegisterFactory(                                       \
      nTypeID, CreateSharedObjectSubclassProhibited<nTypeID>, (flags),         \
      strName)
#else
#define REG_SHARED_OBJECT_SUBCLASS(derivedClass)                               \
  GCSDK::CSharedObject::RegisterFactory(                                       \
      derivedClass::k_nTypeID,                                                 \
      GCSDK::CreateSharedObjectSubclass<derivedClass>, 0, #derivedClass)
#endif
