...
//-----------------------------------------------------------------------------
#define DO_NOT_YIELD_THIS_SCOPE()                                              \
  GCSDK::CDoNotYieldScope doNotYieldScope_##line(FILE_AND_LINE)
...
//-----------------------------------------------------------------------------
// Purpose: reason as to why the current job has yielded to the main thread
// (paused)
//			if this is updated, k_prgchJobPauseReason[] in job.cpp
...
#define BYieldingAcquireLock(lock)                                             \
  _BYieldingAcquireLock(lock, __FILE__, __LINE__)
#define BAcquireLockImmediate(lock)                                            \
  _BAcquireLockImmediate(lock, __FILE__, __LINE__)
#define ReleaseLock(lock) _ReleaseLock(lock, false, __FILE__, __LINE__)

//-----------------------------------------------------------------------------
// Purpose: A job is any server operation that requires state.  Typically, we
// use jobs for
//			operations that need to pause waiting for responses from
//other servers.  The 			job object persists the state of the operation while it
//waits, and the reply 			from the remote server re-activates the job.
//-----------------------------------------------------------------------------
class CJob {
public:
  // Constructors & destructors, when overriding job name a static string
  ...
  void StartJobFromNetworkMsg(IMsgNetPacket *pNetPacket,
  ...
  // schedules the job for execution, but does not interrup the currently
  // running job. Effectively starts the job on the yielding list as if it had
  // immediately yielded although is more efficient than actually doing so
  ...
  // --- locking pointers
  bool _BYieldingAcquireLock(CLock *pLock, const char *filename = "unknown",
                             int line = 0);
  bool _BAcquireLockImmediate(CLock *pLock, const char *filename = "unknown",
                              int line = 0);
  void _ReleaseLock(CLock *pLock, bool bForce = false,
                    const char *filename = "unknown", int line = 0);
  ...
  bool BYield();
  // Yield IF JobMgr thinks we need to based on how long we've run and our
  // priority
  ...
  bool BYieldingWaitForMsg(CProtoBufMsgBase *pMsg, MsgType_t eMsg,
                           const CSteamID &expectedID);
  bool BYieldingRunQuery(CGCSQLQueryGroup *pQueryGroup,
                         ESchemaCatalog eSchemaCatalog);
#endif

  bool BYieldingWaitTimeWithLimit(uint32 cMicrosecondsToSleep,
                                  CJobTime &stimeStarted, int64 nMicroSecLimit);
  bool BYieldingWaitTimeWithLimitRealTime(uint32 cMicrosecondsToSleep,
                                          int nSecLimit);
  ...
  // You can pass a string that describes what sort of work item you are waiting
  // on. WARNING: This function saves the pointer to the string, it doesn't copy
  // the string
  ...
  // marks a packet as being finished with, releases the packet and frees the
  // memory
...
protected:
  // main job implementation, in the coroutine.  Every job must implement at
  ...
  // Called by CJobMgr to send heartbeat message to our listeners during long
  // operations
  void Heartbeat();
  ...
  uint32 m_bRunFromMsg : 1,
      m_bWorkItemCanceled : 1, // true if the work item we were waiting on was
                               // canceled
...
private:
  ...
  // all these flags indicate some kind of failure and we will want to report
  // them
  union {
    struct {
      uint32 m_bJobFailed : 1, // true if BYieldingRunJob returned false
          m_bLocksFailed : 1, m_bLocksLongHeld : 1, m_bLocksLongWait : 1,
          m_bWaitTimeout : 1, m_bLongInterYield : 1, m_bTimeoutNetMsg : 1,
          m_bTimeoutOther : 1, m_uUnused : 24;
    } m_bits;
    uint32 m_uFlags;
  } m_flags;
  ...
  CJobTime
      m_STimeSwitched; // time (frame) at which we were last paused or continued
  CJobTime
      m_STimeNextHeartbeat; // Time at which next heartbeat should be performed
  CFastTimer
      m_FastTimerDelta; // How much time we've been running for without yielding
  ...
  CUtlVectorFixedGrowable<IMsgNetPacket *, 1>
      m_vecNetPackets; // list of tcp packets currently held by this job (ie,
                       // needing release on job exit)
  ...
  // A stack of do not yield guards so we can print the right warning if they're
  // nested
...
//			add this object to any classes you want jobs to be able
//to lock
//-----------------------------------------------------------------------------
class CLock {
public:
  ...
  void Dump(const char *pszPrefix = "\t\t", int nPrintMax = 1,
            bool bPrintWaiting = true) const;
...
class CAutoCLock {
public:
  CAutoCLock(CLock &refLock) : m_pLock(&refLock) {
