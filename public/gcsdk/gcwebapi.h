...
enum EWebAPIPrivilege {
  ...
  k_EWebApiPriv_PublisherKeyOwnsApp =
      3, // Requires publisher key and publisher owns appid
         // k_EWebApiPriv_Account	= 1,
  // // user must have a Steam account with password set
};
...
#define DECLARE_GCWEBAPI_METHOD(pchMethodName, unVersion, eHTTPMethod,         \
                                pchJobName, ePriv)                             \
  {                                                                            \
    KeyValues *pkvMethod =                                                     \
        pkvInterface->FindKey(pchMethodName #unVersion, true);                 \
...
#define REQUIRED_GCWEBAPI_PARAM(pchName, eType, pchDescription)                \
  {                                                                            \
    ...
    AssertMsg(Q_stricmp(pchName, "format") != 0,                               \
              "'format' is a magic reserved API param for specifying output "  \
              "format!");                                                      \
...
#define END_GCWEB_INTERFACE_BLOCK(pchInterfaceName)                            \
  ...
  CGCWebAPIInterfaceMapRegistrar                                               \
      g_Register_GCWebAPIInterfaceMapCreator_##pchInterfaceName(               \
          &CreateWebAPIInterfaceMap_##pchInterfaceName);
