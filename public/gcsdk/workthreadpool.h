...
// forward declaration for CTSQueue which we can't statically allocate as our
// member because of alignment issues on Win64
template <class T, bool bTestOptimizer> class CTSQueue;
...
// these functions return pointers to fixed string in the code section. We need
// this for VPROF nodes
...
//-----------------------------------------------------------------------------
// Purpose: Work item base class.  Derive from this for specific work item
// types.
//			The derived type ideally should be self-contained with
//all data it 			needs to perform the work.
//-----------------------------------------------------------------------------
class CWorkItem : public CRefCount {
public:
  CWorkItem()
      : m_JobID(k_GIDNil), m_bRunning(false), m_bResubmit(false),
        m_bCanceled(false), m_ulSequenceNumber(0) {
    m_jobtimeTimeout.SetLTime(0);
    m_jobtimeQueued.SetToJobTime();
  }

  CWorkItem(JobID_t jobID)
      : m_JobID(jobID), m_bRunning(false), m_bResubmit(false),
        m_bCanceled(false), m_ulSequenceNumber(0) {
    m_jobtimeTimeout.SetLTime(0);
    m_jobtimeQueued.SetToJobTime();
  }
  ...
  bool HasTimedOut() const {
    return m_jobtimeTimeout.LTime() != 0 &&
           m_jobtimeTimeout.CServerMicroSecsPassed() > 0;
  }
  ...
  // Work threads can call this to force a work item to be reprocessed (added to
  ...
  // Return false if your operation failed in some way that you would want to
  // know about The CWorkThreadPool will count the failures.
  virtual bool
  ThreadProcess(CWorkThread *pThread) = 0; // called by the worker thread
  virtual bool DispatchCompletedWorkItem(
      CJobMgr *jobMgr); // called by main loop after item completed
...
protected:
  // note: destructor is private.  This is a ref-counted object, private
  // destructor ensures callers can't accidentally delete directly, or declare
  // on stack
  virtual ~CWorkItem() {}
  ...
  uint64
      m_ulSequenceNumber;   // Sequence number for the work item, used when
                            // enforcing output ordering as matching input order
  CCycleCount m_CycleCount; // A record of how long it took to execute this
...
private:
  ...
  CJobTime m_jobtimeTimeout; // time at which this result is no longer valid, so
                             // it shouldn't start to be processed
...
protected:
  ...
  volatile bool m_bExitThread; // set by CWorkThreadPool::StopWorkerThreads and
                               // possibly by subclasses of CWorkThread
  volatile bool m_bFinished; // set by CWorkThread::Run [note: must still check
                             // IsThreadRunning, and/or call Join]
...
public:
  virtual CWorkThread *
  CreateWorkerThread(class CWorkThreadPool *pWorkThreadPool) = 0;
};

//-----------------------------------------------------------------------------
// reusable trivial implementation of IWorkThreadFactory
//-----------------------------------------------------------------------------
template <class T> class CWorkThreadFactory : public IWorkThreadFactory {
public:
  virtual CWorkThread *
  CreateWorkerThread(class CWorkThreadPool *pWorkThreadPool) {
...
public:
  ...
  // eventually it might be nice to be able to resize these pools via console
  // command in that case, we'd want a constructor like this, and a PoolSize
  // accessor/mutator pair it makes this class much more complicated, however
  // (growing the pool is easy, shrinking it is less easy) so we'll punt for
  // now.
  /* CWorkThreadPool( const char *pszName = "unnamed thread" ) :
   * CWorkThreadPool( pszName, -1 ); */

  virtual ~CWorkThreadPool();

  // Setting this will ensure that items of the same priority complete and get
  // dispatched in the same order they are added to the threadpool.  This has a
  // small additional locking overhead and can increase latency as items that
  // are actually completed out-of-order have to queue waiting on earlier items.
  ...
  void SetWorkThreadAutoConstruct(int cMaxThreads,
                                  IWorkThreadFactory *pWorkThreadConstructor);

  bool
  AddWorkItem(CWorkItem *pWorkItem); // add a work item to the queue to process
  CWorkItem *GetNextCompletedWorkItem(); // get next completed work item and
                                         // it's priority if needed
  ...
  bool BDispatchCompletedWorkItems(const CLimitTimer &limitTimer,
                                   CJobMgr *pJobMgr);
...
protected:
  ...
  // Vectors of completed, but out of order and waiting work items, only used
  // when bEnsureOutputOrdering == true
  CThreadMutex m_MutexOnItemCompletedOrdered;
  CUtlVector<CWorkItem *> m_vecCompletedAndWaiting;

  // Should we emit work items in the same order they are received (on a per
  // priority basis)
  ...
  CThreadEvent m_EventNewWorkItem;    // event set when a new work item is
  ...
  // override this method if you want to do any special handling of completed
  // work items.  Default implementation puts work items in our completed item
  // queue.
