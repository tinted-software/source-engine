...
// Task granularity -- the longest time in microseconds you should spend without
// yielding
const int k_cMicroSecTaskGranularity = 5 * k_nThousand;

// If a frame runs longer than it's supposed to (which they always will by at
// least a little), we subtract the overage from the next frame.  This is the
// maximum amount to correct each frame.
const int k_cMicroSecMaxFrameCorrection = 25 * k_nThousand;

// if server time is too far behind, we will start skipping frames to
// re-synchronize it with real time
...
// Default Max number of job heartbeat intervals to allow a job to be blocked on
// I/O
const int k_cJobHeartbeatsBeforeTimeoutDefault =
    k_cMicroSecJobPausedTimeout / k_cMicroSecJobHeartbeat;
...
const uint64 k_cMicroSecLockLifetime =
    (uint64)k_nSecondsPerHour * (uint64)k_nMillion;
...
const int k_cSQLObjectNameMax = 128; // max length of a SQL identifier (column
                                     // name, index name, table name ... )
const int k_cchSQLStatementTextMax =
    8192; // nominal max length of a SQL statement
const uint32 k_cubRecordMax = 4 * k_nMillion; // Max size of a single record
const uint32 k_cubVarFieldMax =
    1 * k_nMillion; // Max size of a variable-length field
const int k_cMaxCol = 50;
const uint32 k_cMaxBindParams =
    500; // the largest number of bind parameters allowed in a single query
const uint32 k_cMaxFreeBindParamBuffers =
    2000; // How many bind param buffers should we leave free before we start
          // deleting some
const uint32 k_cMaxFreeBindWebAPIBuffers =
    32; // How many WebAPI buffers should we leave free before we start deleting
        // some
...
const uint32 k_cchWebAPIKeyStringMax =
    (k_cubWebAPIKey * 2) + 1; // size of string representing web API key (hex
                              // encoded so twice the size + NUL)
...
const uint32 k_cubGameServerToken =
    16; // size of the game server identity token key
const uint32 k_cchGameServerTokenStringMax =
    (k_cubGameServerToken * 2) +
    1; // size of string representing game server identity token (hex encoded so
       // twice the size + NUL)
...
const int k_cInitialNetworkBuffers =
    10; // # of network buffers to see the system with

const int k_cubMaxExpectedMsgDataSize =
    5 * k_nMegabyte; // the maximum application data that we EXPECT to be sent
                     // in a single message
...
const int k_cGCUserSessionInit =
    10 * k_nThousand; // Can grow indefinitely by this increment
const int k_cBucketGCUserSession = 100 * k_nThousand; // Fixed size
const int k_cGCGSSessionInit =
    1 * k_nThousand; // Can grow indefinitely by this increment
const int k_cBucketGCGSSession = 10 * k_nThousand; // Fixed size
const int k_cAccountDetailsInit =
    10 * k_nThousand; // Can grow indefinitely by this increment
const int k_cBucketAccountDetails = 100 * k_nThousand; // Fixed size
const int k_cGCLocksInit =
    50 * k_nThousand; // Can grow indefinitely by this increment
