...
public:
  ...
  bool BIsSubscribed(const CSteamID &steamID) const {
    return FindSubscriber(steamID) != m_vecSubscribers.InvalidIndex();
  }

  const CUtlVector<Subscriber_t> &GetSubscribers() const {
    return m_vecSubscribers;
  }
  const CSteamID &GetOwner() const { return m_steamIDOwner; }

private:
  // finds a steam ID within the subscriber list, returns the index, invalid if
  // it can't be found
...
enum ESOTypeFlags {
  ...
  k_ESOFlag_SendToOwner = 1 << 0, // will go to the owner of the cache (user or
                                  // gameserver), if he is subscribed
  k_ESOFlag_SendToOtherUsers =
      1 << 1, // will go to subscribed users who are not the owner of the cache
  k_ESOFlag_SendToOtherGameservers = 1
                                     << 2, // will go to subscribed gameservers
                                           // who are not the owner of the cache
  k_ESOFlag_SendToQuestObjectiveTrackers = 1 << 3,
  k_ESOFlag_LastFlag = k_ESOFlag_SendToQuestObjectiveTrackers,
};

//----------------------------------------------------------------------------
// Purpose: Filter object used to determine whether a type cache's objects
// should
...
public:
  ...
  void BuildCacheSubscribedMsg(CMsgSOCacheSubscribed_SubscribedType *pMsgType,
                               uint32 unFlags,
...
//			is shared between clients, gameservers, and the GC and
//is 			responsible for sending messages from the GC to cause object
//			creation/destruction/updating on the
//clients/gameservers.
//----------------------------------------------------------------------------
...
public:
  ...
  const CUtlVector<CSharedObjectContext::Subscriber_t> &GetSubscribers() const {
    return m_context.GetSubscribers();
  }

  CGCSharedObjectTypeCache *FindTypeCache(int nClassID) const {
    return (CGCSharedObjectTypeCache *)FindBaseTypeCache(nClassID);
  }
  CGCSharedObjectTypeCache *CreateTypeCache(int nClassID) {
    return (CGCSharedObjectTypeCache *)CreateBaseTypeCache(nClassID);
  }
  ...
  bool BYieldingLoadSchObjects(IGCSQLResultSet *pResultSet,
                               const CColumnSet &csRead,
                               const SOClass_t &schDefaults);

  template <typename SOClass_t>
  bool BYieldingLoadSchSingleton(IGCSQLResultSet *pResultSet,
                                 const CColumnSet &csRead,
                                 const SOClass_t &schDefaults);

  template <typename SOClass_t, typename SchClass_t>
  bool BYieldingLoadProtoBufObjects(IGCSQLResultSet *pResultSet,
                                    const CColumnSet &csRead);

  template <typename SOClass_t, typename SchClass_t>
  bool BYieldingLoadProtoBufSingleton(IGCSQLResultSet *pResultSet,
                                      const CColumnSet &csRead,
                                      const SchClass_t &schDefaults);

  // @todo temporary for trading and item subscriptions (to be removed once we
  // get cross-game trading)
  ...
  void AddSubscriber(const CSteamID &steamID,
                     bool bForceSendSubscriptionMsg = false);
  void RemoveSubscriber(const CSteamID &steamID);
  void RemoveAllSubscribers();
  void SendSubscriberMessage(const CSteamID &steamID);
  bool BIsSubscribed(const CSteamID &steamID) {
    return m_context.BIsSubscribed(steamID);
  }
  void ClearCachedSubscriptionMessage();

  bool BIsDatabaseDirty() const {
    return m_databaseDirtyList.NumDirtyObjects() > 0;
  }
  ...
  // Add a specific object write to a transaction. The cache is expected to
  // remain locked until this transaction is closed.  If the transaction is
  // rolled back, the object will be returned to the dirty list.
  bool BYieldingAddWriteToTransaction(CSharedObject *pObj,
                                      CSQLAccess &sqlAccess);

  // Add all pending object writes to a transaction.  The cache is expected to
  // remain locked until this transaction is closed.  If the transaction
  // successfully commits, the dirty list will be flushed.
  //
  // This is intended for use in writeback -- no changes to the dirty objects
  // list may occur until this transaction closes.
  ...
  bool IsObjectCached(const CSharedObject *pObj) const {
    return IsObjectCached(pObj, pObj->GetTypeID());
  }
  // the same as the above, but takes in the type since if called from a
  // destructor, you can't use virtual functions
  ...
  void SetDetectVersionChanges(bool bState) {
    m_bDetectVersionChanges = bState;
  }

protected:
  virtual CSharedObjectTypeCache *
  AllocateTypeCache(int nClassID) const OVERRIDE {
    return new CGCSharedObjectTypeCache(nClassID, m_context);
  }
...
protected:
  ...
  // this is a flag that when set will cause any version changes to trigger an
  // assert. This can be used during times like loading to ensure we don't have
  // inappropriate version changes which can cause inefficiencies
...
//			schDefaults - A schema object that defines the values to
//set in 				the new objects for fields that were not read in the query. 				Typically
//this will be whatever fields were in the WHERE 				clause of the query. 			csRead - A
//columnSet defining the fields that were read in the query.
//----------------------------------------------------------------------------
template <typename SOClass_t>
bool CGCSharedObjectCache::BYieldingLoadSchObjects(
    IGCSQLResultSet *pResultSet, const CColumnSet &csRead,
    const SOClass_t &objDefaults) {
  if (NULL == pResultSet)
    return false;
  ...
  if (pResultSet->GetRowCount() > 0) {
    CGCSharedObjectTypeCache *pTypeCache =
        CreateTypeCache(SOClass_t::k_nTypeID);
...
//----------------------------------------------------------------------------
template <typename SOClass_t>
bool CGCSharedObjectCache::BYieldingLoadSchSingleton(
    IGCSQLResultSet *pResultSet, const CColumnSet &csRead,
    const SOClass_t &objDefaults) {
  if (NULL == pResultSet)
    return false;
  ...
  if (pResultSet->GetRowCount() > 1) {
    EmitError(
        SPEW_SHAREDOBJ,
        "Multiple rows passed to BYieldingLoadSchSingleton() on type %d\n",
...
//----------------------------------------------------------------------------
template <typename SOClass_t, typename SchClass_t>
bool CGCSharedObjectCache::BYieldingLoadProtoBufObjects(
    IGCSQLResultSet *pResultSet, const CColumnSet &csRead) {
  if (NULL == pResultSet)
    return false;
...
//----------------------------------------------------------------------------
template <typename SOClass_t, typename SchClass_t>
bool CGCSharedObjectCache::BYieldingLoadProtoBufSingleton(
    IGCSQLResultSet *pResultSet, const CColumnSet &csRead,
    const SchClass_t &schDefaults) {
  if (NULL == pResultSet)
    return false;
  ...
  if (pResultSet->GetRowCount() > 1) {
    EmitError(
        SPEW_SHAREDOBJ,
        "Multiple rows passed to BYieldingLoadProtoBufSingleton() on type %d\n",
        SOClass_t::k_nTypeID);
    return false;
  }
  ...
  if (record.IsValid()) {
    ...
    CSQLAccess sqlAccess;
    if (!sqlAccess.BYieldingInsertRecord(
            const_cast<SchClass_t *>(&schDefaults)))
      return false;
    schRead = schDefaults;
  }
