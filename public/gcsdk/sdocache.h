...
#define REG_SDO(classname)                                                     \
  GSDOCache().RegisterSDO(classname::k_eType, #classname)

// A string used to tell the difference between nil objects and actual objects
// in memcached
...
//-----------------------------------------------------------------------------
template <int SAMPLES> class CMovingAverage {
public:
  CMovingAverage() { Reset(); }
...
public:
  ...
  // SQL loading
  virtual bool BYldLoadFromSQL(CUtlVector<ISDO *> &vecSDOToLoad,
                               CUtlVector<bool> &vecResults) const = 0;
  ...
  // Returns true if this is not the actual object, but a placeholder to
  // remember that this object doesn't actually exist
  virtual bool BNilObject() const = 0;

  // Allocs an SDO that must return true for IsEqual( this ) and BNilObject().
  // This is so we can cache load attempts for objects that don't exist
...
public:
  ...
  // Implement this in a subclass if there's some value like 0 or -1 that's
  // invalid so the system can know to not even attempt to load it
  ...
  // default load from SQL is no-op as not all types have permanent storage -
  // override to create a batch load
  virtual bool BYldLoadFromSQL(CUtlVector<ISDO *> &vecSDOToLoad,
                               CUtlVector<bool> &vecResults) const;

  // override to do initialization after load
  virtual void PostLoadInit() {}

  // compares the serialized versions by default. Override to have more specific
  // behavior
  ...
  // Creates a key name that looks like "Prefix_%u" but faster. Also makes sure
  ...
  void CreateSimpleMemcachedName(CFmtStr &strDest,
                                 ...
                                 uint32 unSuffix) {
    CSDOCache::CreateSimpleMemcachedName(strDest, rgchPrefix, prefixBufSize - 1,
                                         unSuffix);
  }
...
public:
  CNilSDO(const KeyType &key, const char *pchMemcachedKeyName)
      : CBaseSDO<KeyType, eSDOType, ProtoMsg>(key),
        m_sMemcachedKeyName(pchMemcachedKeyName) {}
  ...
  virtual void WriteToBuffer(CUtlBuffer &memBuffer) {
    memBuffer.PutString(k_rgchNilObjSerializedValue);
  }
  virtual void SerializeToProtobuf(ProtoMsg &msg) {}
  virtual bool DeserializeFromProtobuf(const ProtoMsg &msg) { return false; }
  virtual size_t CubBytesUsed() {
    return sizeof(*this) + m_sMemcachedKeyName.Length();
  }
  virtual void GetMemcachedKeyName(CFmtStr &strKey) {
    V_strncpy(strKey.Access(), m_sMemcachedKeyName, FMTSTR_STD_LEN);
  }
...
public:
  CSDORef() { m_pSDO = NULL; }
  explicit CSDORef(CSDORef<T> &SDORef) {
    m_pSDO = SDORef.Get();
    if (m_pSDO)
      m_pSDO->AddRef();
  }
  explicit CSDORef(T *pSDO) {
    m_pSDO = pSDO;
    if (m_pSDO)
      m_pSDO->AddRef();
  }
...
public:
  ...
  // A struct to hold stats for the system. This is generated code in Steam. It
  // would be great to make it generated code here if we could bring Steam's
  // operational stats system in the GC
  ...
  template <class T>
  bool BYldLoadSDO(CSDORef<T> *pPSDORef, const typename T::KeyType_t &key,
                   bool *pbTimeoutLoading = NULL);
  ...
  bool BGetLoadedSDO(CSDORef<T> *pPSDORef, const typename T::KeyType_t &key,
                     bool *pbFoundNil = NULL);

  // starts loading a SDO you're going to reference soon with the above
  // BYldLoadSDO() use this to batch up requests, hinting a set then getting
  // reference to a set is significantly faster
  template <class T> void HintLoadSDO(const typename T::KeyType_t &key);
  ...
  template <class T> void RemoveNil(const typename T::KeyType_t &key);

  // force a deletes a SDO from the cache - waits until the object is not
  // referenced
  template <class T>
  bool BYldDeleteSDO(const typename T::KeyType_t &key,
                     uint64 unMicrosecondsToWaitForUnreferenced);
  ...
  static void CreateSimpleMemcachedName(CFmtStr &strDest, const char *pchPrefix,
                                        uint32 unPrefixLen, uint32 unSuffix);

  // memcached verification - returns the number of mismatches
  //**tempcomment**		void YldVerifyMemcachedData( CreateSDOFunc_t
  //pCreateSDOFunc, CUtlVector<uint32> &vecIDs, int *pcMatches, int
  //*pcMismatches );
  ...
  virtual bool
  BFrameFuncRunMemcachedQueriesUntilCompleted(CLimitTimer &limitTimer);
...
private:
  ...
  public:
    ...
    inline bool operator()(const ISDO *lhs, const ISDO *rhs) const {
  ...
  template <class T> int FindLoadedSDO(const typename T::KeyType_t &key);

  template <class T> int QueueLoad(const typename T::KeyType_t &key);
  int QueueMemcachedLoad(ISDO *pSDO);

  // items already loaded - Maps the SDO to the LRU position
  CUtlHashMapLarge<ISDO *, int, CDefPISDOEquals, CPISDOHashFunctor>
      m_mapISDOLoaded;

  // items we have queued to load, in the state of either being loaded from
  // memcached or SQL maps SDO to a list of jobs waiting on the load
  CUtlHashMapLarge<ISDO *, CCopyableUtlVector<JobID_t>, CDefPISDOEquals,
                   CPISDOHashFunctor>
      m_mapQueuedRequests;
  ...
  struct TypeStats_t {
    TypeStats_t()
        : m_nLoaded(0), m_nRefed(0), m_cubUnrefed(0), m_nNilObjects(0) {}
...
//-----------------------------------------------------------------------------
template <typename KeyType, int ESDOType, class ProtoMsg>
bool CBaseSDO<KeyType, ESDOType, ProtoMsg>::BReadFromBuffer(const byte *pubData,
                                                            int cubData) {
  ProtoMsg msg;
...
//-----------------------------------------------------------------------------
template <typename KeyType, int ESDOType, class ProtoMsg>
void CBaseSDO<KeyType, ESDOType, ProtoMsg>::WriteToBuffer(
    CUtlBuffer &memBuffer) {
  ...
  pMsg->SerializeWithCachedSizesToArray((uint8 *)memBuffer.Base() +
                                        memBuffer.TellPut());
...
bool CBaseSDO<KeyType, ESDOType, ProtoMsg>::IsEqual(const ISDO *pSDO) const {
  ...
  return (GetKey() ==
          static_cast<const CBaseSDO<KeyType, ESDOType, ProtoMsg> *>(pSDO)
              ->GetKey());
}
...
//-----------------------------------------------------------------------------
template <typename KeyType, int ESDOType, class ProtoMsg>
bool CBaseSDO<KeyType, ESDOType, ProtoMsg>::BYldLoadFromSQL(
    CUtlVector<ISDO *> &vecSDOToLoad, CUtlVector<bool> &vecResults) const {
  FOR_EACH_VEC(vecResults, i) { vecResults[i] = true; }
...
//	load queue
//-----------------------------------------------------------------------------
template <class T> int CSDOCache::QueueLoad(const typename T::KeyType_t &key) {
  T probe(key);
...
void CSDOCache::HintLoadSDO(const CUtlVector<typename T::KeyType_t> &vecKeys) {
  FOR_EACH_VEC(vecKeys, i) { HintLoadSDO<T>(vecKeys[i]); }
}
...
//-----------------------------------------------------------------------------
template <class T>
bool CSDOCache::BGetLoadedSDO(CSDORef<T> *pPSDORef,
                              const typename T::KeyType_t &key,
                              bool *pbFoundNil) {
  if (NULL != pbFoundNil) {
    *pbFoundNil = false;
  }
...
//-----------------------------------------------------------------------------
template <class T>
bool CSDOCache::BYldLoadSDO(CSDORef<T> *pPSDORef,
                            const typename T::KeyType_t &key,
                            bool *pbTimeoutLoading /* = NULL */) {
  ...
  *pPSDORef = NULL;
  ...
  // If we've already tried to look this up in the past don't bother looking it
  // up again
  if (bFoundNil)
    return false;
...
// Clears a nil object if one exists for this key.
//-----------------------------------------------------------------------------
template <class T> void CSDOCache::RemoveNil(const typename T::KeyType_t &key) {
  ...
  // Remove the object from memcached. Do this here because while we have for
  // sure removed any nil that was in memory this may have been a nil that was
  // not in memory but instead cached in memcached.
  T temp(key);
...
//-----------------------------------------------------------------------------
template <class T>
bool CSDOCache::BYldDeleteSDO(const typename T::KeyType_t &key,
                              uint64 unMicrosecondsToWaitForUnreferenced) {
  // see if we have it in cache first
  int iMap = FindLoadedSDO<T>(key);
  if (!m_mapISDOLoaded.IsValidIndex(iMap)) {
  ...
  // check the ref count
  int64 cAttempts = MAX(1LL, (int64)(unMicrosecondsToWaitForUnreferenced /
...
//-----------------------------------------------------------------------------
// Purpose: A class to factor out the common code in most SDO SQL loading
// funcitons
//-----------------------------------------------------------------------------
template <class T> class CSDOSQLLoadHelper {
public:
  // this is the results of the load helper, abstracted to provide a more
  // efficient means to lookup the queries without having to do very expensive
  // memory management
  template <class SCH> class CResults {
  public:
    // given a key, this will return the range of indices that include this key.
    // -1 will be set to the start if no match is found. This will return the
    // number of matches found
    int GetKeyIndexRange(typename T::KeyType_t Key, int &nStart,
                         int &nEnd) const;

    // given a key, this will return the first schema that matches this key, or
    // NULL if none is found. Only use this if you know there is a maximum of a
    // single value
    const SCH *GetSingleResultForKey(typename T::KeyType_t Key) const;

    // used to start iteration over a group of results. Given a key, this will
    // return the index that can be used to get the result or -1 if no match is
    // found
    int GetFirstResultIndex(typename T::KeyType_t Key) const;
    // given an index that was previously obtained from
    // GetFirst/NextResultIndex, this will get the next result that has the same
    // key, or return -1 if no further keys of that type are found
    int GetNextResultIndex(int nOldResultIndex) const;
    // given an index from GetFirst/NextResultIndex that is valid (i.e. not -1),
    // this will return the result that was obtained
    const SCH *GetResultFromIndex(int nIndex) const;
    // given an index returned by the GetFirst/NextResultIndex, this will
    // determine if it is valid
    static int InvalidIndex() { return -1; }

  private:
    // given a key, this maps to the specific result
    template <class TKeyType> struct SKeyToResult {
      bool operator<(const SKeyToResult<TKeyType> &rhs) const {
  ...
  CSDOSQLLoadHelper(const CUtlVector<ISDO *> *vecSDOToLoad,
                    const char *pchProfileName);

  // Loads all rows in the SCH table whose field nFieldID match the key of an
  // SDO being loaded
  template <class SCH>
  bool BYieldingExecuteSingleTable(int nFieldID, CResults<SCH> &Results);
  // Loads the specified columns for all rows in the SCH table whose field
  // nFieldID match the key of an SDO being loaded
  template <class SCH>
  bool BYieldingExecuteSingleTable(int nFieldID, const CColumnSet &csetRead,
                                   CResults<SCH> &Results);

  // Functions to load rows from more than one table at a time

  // Loads all rows in the SCH table whose field nFieldID match the key of an
  // SDO being loaded
  template <class SCH> void AddTableToQuery(int nFieldID);
  ...
  template <class SCH> bool BGetResults(int nQuery, CResults<SCH> &Results);
...
private:
  ...
  struct Query_t {
    Query_t(const CColumnSet &columnSet, int nKeyCol)
        : m_ColumnSet(columnSet), m_nKeyCol(nKeyCol) {}
...
#define FOR_EACH_SQL_LOAD_RESULT(Results, Key, Index)                          \
  for (int Index = Results.GetFirstResultIndex(Key);                           \
       Index != Results.InvalidIndex();                                        \
       Index = Results.GetNextResultIndex(Index))

//-----------------------------------------------------------------------------
// Purpose: Constructor. Initializes with the vector of objects being loaded
//-----------------------------------------------------------------------------
template <class T>
CSDOSQLLoadHelper<T>::CSDOSQLLoadHelper(const CUtlVector<ISDO *> *vecSDOToLoad,
                                        const char *pchProfileName)
...
template <class T>
template <class SCH>
bool CSDOSQLLoadHelper<T>::BYieldingExecuteSingleTable(int nFieldID,
                                                       CResults<SCH> &Results) {
...
template <class T>
template <class SCH>
bool CSDOSQLLoadHelper<T>::BYieldingExecuteSingleTable(
    int nFieldID, const CColumnSet &csetRead, CResults<SCH> &Results) {
...
template <class SCH>
void CSDOSQLLoadHelper<T>::AddTableToQuery(int nFieldID,
                                           const CColumnSet &csetRead) {
  Assert(csetRead.GetRecordInfo() ==
         GSchemaFull().GetSchema(SCH::k_iTable).GetRecordInfo());

  // Bind the params
  FOR_EACH_VEC(m_vecKeys, i) { m_sqlAccess.AddBindParam(m_vecKeys[i]); }

  // Build the query
  CUtlString sCommand;
  {
    TSQLCmdStr sSelect;
    const char *pchColumnName = GSchemaFull()
                                    .GetSchema(SCH::k_iTable)
                                    .GetRecordInfo()
                                    ->GetColumnInfo(nFieldID)
                                    .GetName();

    BuildSelectStatementText(&sSelect, csetRead);
    sCommand.Format("%s WHERE %s IN (%.*s)", sSelect.Access(), pchColumnName,
                    GetInsertArgStringChars(m_vecKeys.Count()),
                    GetInsertArgString());
  }
...
//-----------------------------------------------------------------------------
template <class T> bool CSDOSQLLoadHelper<T>::BYieldingExecute() {
...
bool CSDOSQLLoadHelper<T>::BGetResults(int nQuery, CResults<SCH> &Results) {
  ...
  Assert(pSQLResults && nQuery >= 0 &&
         (uint32)nQuery < pSQLResults->GetResultSetCount() &&
         pSQLResults->GetResultSetCount() == (uint32)m_vecQueries.Count());
  if (NULL == pSQLResults || nQuery < 0 ||
      (uint32)nQuery >= pSQLResults->GetResultSetCount() ||
      pSQLResults->GetResultSetCount() != (uint32)m_vecQueries.Count())
    return false;

  Assert(m_vecQueries[nQuery].m_ColumnSet.GetRecordInfo()->GetTableID() ==
         SCH::k_iTable);
  if (m_vecQueries[nQuery].m_ColumnSet.GetRecordInfo()->GetTableID() !=
      SCH::k_iTable)
    return false;

  // copy the results from the SQL queries over to our result list
  Results.m_Results.EnsureCapacity(
      pSQLResults->GetResultSet(nQuery)->GetRowCount());
  if (!CopyResultToSchVector(pSQLResults->GetResultSet(nQuery),
                             m_vecQueries[nQuery].m_ColumnSet,
                             &Results.m_Results))
    return false;

  // Make a map that counts maps from our results into a sorted list for fast
  // lookup
  ...
  FOR_EACH_VEC(Results.m_Results, nCurrResult) {
    ...
    uint8 *pubData;
    uint32 cubData;
    if (!Results.m_Results[nCurrResult].BGetField(
            m_vecQueries[nQuery].m_nKeyCol, &pubData, &cubData))
      return false;
...
int CSDOSQLLoadHelper<T>::CResults<SCH>::GetKeyIndexRange(
    typename T::KeyType_t Key, int &nStart, int &nEnd) const {
  ...
  for (nEnd = nStart + 1;
       (nEnd < m_KeyToResult.Count()) && (m_KeyToResult[nEnd].m_Key == Key);
       nEnd++) {
  }
...
template <typename T>
template <typename SCH>
const SCH *CSDOSQLLoadHelper<T>::CResults<SCH>::GetSingleResultForKey(
    typename T::KeyType_t Key) const {
  int nIndex = GetFirstResultIndex(Key);
  AssertMsg((nIndex == InvalidIndex()) ||
                (GetNextResultIndex(nIndex) == InvalidIndex()),
            "Requested a result from a SQL load helper assuming it was a "
            "singular entry, but found multiple instances of it");
  return GetResultFromIndex(nIndex);
}

template <typename T>
template <typename SCH>
int CSDOSQLLoadHelper<T>::CResults<SCH>::GetFirstResultIndex(
    typename T::KeyType_t Key) const {
  ...
  const SKeyToResult<typename T::KeyType_t> *pMatch =
      std::lower_bound(m_KeyToResult.begin(), m_KeyToResult.end(), SearchKey);
...
template <typename T>
template <typename SCH>
int CSDOSQLLoadHelper<T>::CResults<SCH>::GetNextResultIndex(
    int nOldResultIndex) const {
  // handle out of range elements. Either invalid, or the last element or beyond
  // in our array
  if ((nOldResultIndex < 0) || (nOldResultIndex + 1 >= m_KeyToResult.Count()))
    return InvalidIndex();

  // see if we are less than the next value in the list. If so, we aren't equal
  // and need to stop iteration
...
template <typename SCH>
const SCH *
CSDOSQLLoadHelper<T>::CResults<SCH>::GetResultFromIndex(int nIndex) const {
