...
// a free standing utility function that takes in a newly allocated job, and
// tells the job to start executing. An example of this would be pStore =
// StartNewJobDelayed( new CFooJob( Params ) );
template <typename T> inline typename T *StartNewJobDelayed(T *pNewJob) {
  if (pNewJob)
...
//-----------------------------------------------------------------------------
// Purpose: Control access to web api accounts so that they can be rate
// limited/blocked/exempt, etc
//-----------------------------------------------------------------------------
...
enum EWebAPIAccountLevel {
  ...
  eWebAPIAccountLevel_Elevated =
      3, // like rate limiting, but at a higher threshold
};
...
public:
  ...
  CGCJob(CGCBase *pGC, const char *pchJobName = NULL)
      : CJob(pGC->GetJobMgr(), pchJobName), m_pGC(pGC),
        m_cHeartbeatsBeforeTimeout(k_cJobHeartbeatsBeforeTimeoutDefault) {}
  ...
  bool BYldSendMessageAndGetReply(CSteamID &steamIDTarget, CGCMsgBase &msgOut,
                                  uint nTimeoutSec, CGCMsgBase *pMsgIn,
                                  MsgType_t eMsg);
  bool BYldSendMessageAndGetReply(CSteamID &steamIDTarget, CGCMsgBase &msgOut,
                                  uint nTimeoutSec,
                                  IMsgNetPacket **ppNetPacket);
  bool BYldSendMessageAndGetReply(CSteamID &steamIDTarget,
                                  CProtoBufMsgBase &msgOut, uint nTimeoutSec,
  ...
  void SetJobTimeout(uint nTimeoutSec) {
    m_cHeartbeatsBeforeTimeout =
        1 + ((nTimeoutSec * k_nMillion - 1) / k_cMicroSecJobHeartbeat);
  }
...
// This template class is designed to be derived from various job types and
// provides an adapter for the run from message hook, and handles converting it
// over to an appropriate protobuf message for the caller as well as validating
// that it parsed properly. It will bail on processing the message if the
// protobuf does not parse.
template <typename TGCJobClass, typename TGCType, typename TProtoMsgClass>
class TProtoMsgJob : public TGCJobClass {
public:
...
// a partial specialization of the proto message job to make it easier to use
// with GCJob
...
public:
  ...
  bool BYieldingRunGCJob(
      IMsgNetPacket *pNetPacket); // invokes BYieldingRunJobFromRequest
  virtual bool BYieldingRunJobFromRequest(KeyValues *pkvRequest,
                                          KeyValues *pkvResponse) = 0;
  virtual bool BVerifyParams(const CGCMsg<MsgGCWGRequest_t> &msg,
                             KeyValues *pkvRequest,
                             const WebApiFunc_t *pWebApiFunc);
  void SetWebApiFunc(const WebApiFunc_t *pWebApiFunc) {
    m_pWebApiFunc = pWebApiFunc;
  }

  void SetErrorMessage(KeyValues *pkvErr, const char *pchErrorMsg,
                       int32 nResult);
...
public:
  CGCJobVerifySession(CGCBase *pGC, const CSteamID &steamID)
      : CGCJob(pGC), m_steamID(steamID) {}
...
public:
  CWebAPIJob(CGCBase *pGC, EWebAPIOutputFormat eDefaultOutputFormat =
                               k_EWebAPIOutputFormat_JSON);
  ~CWebAPIJob();

  // Called by jobmgr, and then invokes appropriate run function for specific
  // API version requested
  bool BYieldingRunJobFromMsg(IMsgNetPacket *pNetPacket);

  // Implemented by each individual WebAPI job, should be declared using
  // BEGIN_WEBAPI_JOB_VERSION_ADAPTERS and related macros.
  virtual bool
  BYieldingRunJobFromAPIRequest(const char *pchInterface, const char *pchMethod,
                                uint32 unVersion, CHTTPRequest *pRequest,
                                CHTTPResponse *pResponse,
                                CWebAPIResponse *pWebAPIResponse) = 0;

protected:
  static void AddLocalizedString(CWebAPIValues *pOutDefn,
                                 const char *pchFieldName,
                                 const char *pchKeyName, ELanguage eLang,
                                 bool bReturnTokenIfNotFound = true);
  static void ThreadedEmitFormattedOutputWrapper(CWebAPIResponse *pResponse,
                                                 EWebAPIOutputFormat eFormat,
...
#define BEGIN_WEBAPI_JOB_VERSION_ADAPTERS(interface, method)                   \
  bool BYieldingRunJobFromAPIRequest(                                          \
      const char *pchInterface, const char *pchMethod, uint32 unVersion,       \
      CHTTPRequest *pRequest, CHTTPResponse *pResponse,                        \
      CWebAPIResponse *pWebAPIResponse) {                                      \
    if (Q_strnicmp(pchInterface, interface, Q_strlen(interface)) != 0) {       \
      AssertMsg2(false,                                                        \
                 "WebAPIJob recieved request for unexpected interface (got "   \
                 "%s, expected %s)!",                                          \
                 pchInterface, interface);                                     \
    ...
    if (Q_stricmp(pchMethod, method) != 0) {                                   \
      AssertMsg2(false,                                                        \
                 "WebAPIJob received request fo unexpected method (got %s, "   \
                 "expected %s)!",                                              \
                 pchMethod, method);                                           \
      pResponse->SetStatusCode(k_EHTTPStatusCode500InternalServerError);       \
      return false;                                                            \
...
#define END_WEBAPI_JOB_VERSION_ADAPTERS()                                      \
  default:                                                                     \
    ...
    AssertMsg3(bFoundVersion,                                                  \
               "WebAPIJob for %s/%s received unhandled version %d",            \
               pchInterface, pchMethod, unVersion);                            \
    return bResult;                                                            \
    }
