...
//-----------------------------------------------------------------------------
// Purpose: Header for messages from a client or gameserver to or from the GC
//			That contains source and destination jobs for the
//purpose of 			replying messages.
//-----------------------------------------------------------------------------
struct GCMsgHdrEx_t {
  MsgType_t m_eMsg;   // The message type
  uint64 m_ulSteamID; // User's SteamID
...
struct ProtoBufMsgHeader_t {
  int32 m_EMsgFlagged; // High bit should be set to indicate this message header
                       // type is in use.  The rest of the bits indicate message
                       // type.
  uint32 m_cubProtoBufExtHdr; // Size of the extended header which is a
                              // serialized protobuf object.  Indicates where it
                              // ends and the serialized body protobuf begins.

  ProtoBufMsgHeader_t() : m_EMsgFlagged(0), m_cubProtoBufExtHdr(0) {}
  ProtoBufMsgHeader_t(MsgType_t eMsg, uint32 cubProtoBufExtHdr)
      : m_EMsgFlagged(eMsg | k_EMsgProtoBufFlag),
        m_cubProtoBufExtHdr(cubProtoBufExtHdr) {}
...
private:
  virtual ~CStructNetPacket() { m_pNetPacket->Release(); }

  CNetPacket *m_pNetPacket;
  GCMsgHdrEx_t *m_pHeader;
};

#define GCMSG_EX_HEADER_SIZE                                                   \
  (sizeof(GCSDK::GCMsgHdrEx_t) - sizeof(GCSDK::GCMsgHdr_t))
...
// Handles a message with a header of type GCMsgHdrEx_t, a payload of type
// MSG_BODY_TYPE, and optional variable length data
//-----------------------------------------------------------------------------
...
template <typename MSG_BODY_TYPE> class CGCMsg : public CGCMsgBase {
public:
  ...
  CGCMsg(uint8 *pubPkt, uint32 cubPkt)
      : CGCMsgBase(sizeof(GCMsgHdrEx_t), sizeof(MSG_BODY_TYPE), pubPkt,
                   cubPkt) {}

  // Receive constructor
  // Use this constructor when creating a message from a received network packet
  CGCMsg(CIMsgNetPacketAutoRelease &refNetPacket)
      : CGCMsgBase(sizeof(GCMsgHdrEx_t), sizeof(MSG_BODY_TYPE),
                   refNetPacket->PubData(), refNetPacket->CubData()) {}

  // Receive constructor
  // Use this constructor when creating a message from a received network packet
  CGCMsg(IMsgNetPacket *pNetPacket)
      : CGCMsgBase(sizeof(GCMsgHdrEx_t), sizeof(MSG_BODY_TYPE),
                   pNetPacket->PubData(), pNetPacket->CubData()) {}
  ...
  // empty constructor
  CGCMsg() : CGCMsgBase(sizeof(GCMsgHdrEx_t), sizeof(MSG_BODY_TYPE), NULL, 0) {}
