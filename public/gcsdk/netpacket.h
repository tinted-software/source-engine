...
public:
  ...
  // called to allocate a buffer for the net packet of the specified size. This
  // takes an optional pointer of which it will copy into the data if
  // appropriate
  void Init(uint32 cubData, const void *pCopyData = NULL);

  // called when working with a net packet that you want to reference a separate
  // buffer
