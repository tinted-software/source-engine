...
// GCSqlWriteQueue.h
//
// A utility class that allows for templating based upon a SQL schema type, and
// then queuing up a number of those records to be written to SQL. This will
// buffer them until either a certain number have been queued or a period of
// time has elapsed, at which point it will flush them to SQL in a single
// transaction.
//
//===================================================================
...
template <typename TSqlClass> class CGCSQLWriteQueue {
public:
  CGCSQLWriteQueue(uint32 nMaxToCache, uint32 nMaxMSToWrite)
      : m_nMaxToCache(nMaxToCache), m_nMaxMSToWrite(nMaxMSToWrite) {
    m_QueuedRecords.EnsureCapacity(nMaxToCache);
  }

  // called to queue the record for writing, which will occur either when the
  // maximum time between writebacks has occurred, or when the cache has filled
  // up
  void QueueRecord(const TSqlClass &sch) {
    ...
    if ((uint32)m_QueuedRecords.Count() >= m_nMaxToCache) {
      // unschedule first. This way if while we are yielded, we add another
      // entry, it can schedule a new timeout
      m_TimeCommit.Cancel();
      CreateJobToCommitSQL();
    } else if (!m_TimeCommit.BIsScheduled()) {
      m_TimeCommit.Schedule(this,
                            &CGCSQLWriteQueue<TSqlClass>::CreateJobToCommitSQL,
                            m_nMaxMSToWrite);
    }
  }
...
private:
  ...
  template <typename TSqlClass> class CTimeExpiredCommitJob : public CGCJob {
  public:
    CTimeExpiredCommitJob(CGCBase *pGC, CGCSQLWriteQueue<TSqlClass> *pClass)
        : CGCJob(pGC), m_pClass(pClass) {}
  ...
  // the function called when time expires to start a job and commit the
  // requests to SQL
  ...
  void YieldingFlushQueuedViewsToSQL() {
    ...
    // move the contents into a local vector so we don't have any conflicts of
    // global state
    CUtlVector<TSqlClass> localQueue;
    localQueue.Swap(m_QueuedRecords);
    // prepare the queue for the next batch (so we don't have intermediate
    // resizes)
    ...
    sqlAccess.BBeginTransaction(
        "CGCWatchDownloadedReplayJob::FlushQueuedViewsToSQL");
