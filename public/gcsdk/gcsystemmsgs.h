...
//			global system messages used by the GC. These are usually
//sent by 			the GC Host so be very careful of versioning issues when you consider
//			changing them.  Note that we never use types with
//undefined length 			(like int).  Always use an explicit type (like int32).
//
//=============================================================================
...
#pragma pack(push, 8) // this is a 8 instead of a 1 to maintain backward
...
struct MsgGCWGRequest_t {
  ...
  uint32 m_unPrivilege;  // The EGCWebApiPrivilege value that the request was
                         // made with
  uint32 m_cubKeyValues; // length of the key values data blob in message
                         // (starts after string request name data)
                         // var data -
                         //		request name
