...
// Protobuf headers interfere with the valve min/max/malloc overrides. so we
// need to do all this funky wrapping to make the include happy.
...
static const int MT_GC =
    0x01; // this message is sent to or from a Game Coordinator (will be proxied
          // by servers along the way)
static const int MT_GC_SYSTEM =
    0x02; // this message was sent to or from the steam servers as a system
          // message. Clients can't send these
...
public:
  ...
  void TallySendMessage(MsgType_t eMsg, uint32 uMsgSize,
                        uint32 nSourceMask = 0);
  void TallyReceiveMessage(MsgType_t eMsg, uint32 uMsgSize,
                           uint32 nSourceMask = 0);
  void TallyMultiplexedMessage(MsgType_t eMsg, uint32 uSent, uint32 cRecipients,
                               uint32 uMsgSize, uint32 nSourceMask = 0);

  // profiling
  void EnableProfiling(bool bEnableProfiling);

  // print out our stats
  void PrintStats(bool bShowAll, bool bSortByFrequency,
                  MsgInfo_t::EStatsGroup eGroup, MsgInfo_t::EStatsType eType,
                  uint32 nSourceMask = 0) const;
  void PrintMultiplexStats(MsgInfo_t::EStatsGroup eGroup, bool bSortByFrequency,
                           uint32 nSourceMask = 0) const;

  // Window management - This is similar to profiling in many ways, but is
  // separate so that the base system can monitor traffic rates without
...
private:
  void TallyMessageInternal(MsgInfo_t &msgInfo, MsgInfo_t::EStatsType eBucket,
  ...
  // given a group, this will return the time that the stats have been collected
  // over
  uint64 GetGroupDuration(MsgInfo_t::EStatsGroup eGroup) const;

private:
  // totalled stats for the current window. It would be too costly to total
  // these all the time
...
//-----------------------------------------------------------------------------
// Purpose: Returns the true if the specified message is a valid GC system
// message. Input  : eMsg -		message type to test
