...
//	out from the host which actual GC class to instantiate, and to pass
//through 	all other calls
//-----------------------------------------------------------------------------
...
public:
  ...
  // message sources that we tag message tracking with so they can be filtered
  // accordingly
  enum EMsgSource {
  ...
  bool BProcessSystemMessage(uint32 unGCSysMsgType, const void *pubData,
                             uint32 cubData);
  bool BSendMessageToClient(uint64 ullSteamID, uint32 unMsgType,
                            const void *pubData, uint32 cubData);
  bool BSendMessageToGC(int iGCServerIDTarget, uint32 unMsgType,
                        const void *pubData, uint32 cubData);
  void EmitSpew(const char *pchGroupName, SpewType_t spewType, int iSpewLevel,
                int iLevelLog, const char *pchMsg);
  ...
  bool BSaveConvars();
  void RecordAssert(const char *pchFile, int nLine, const char *pchMessage,
  ...
  virtual bool BAsyncInit(uint32 unAppID, const char *pchDebugName,
                          int iGCIndex, IGameCoordinatorHost *pHost) OVERRIDE;
  virtual bool BMainLoopOncePerFrame(uint64 ulLimitMicroseconds) OVERRIDE;
  virtual bool
  BMainLoopUntilFrameCompletion(uint64 ulLimitMicroseconds) OVERRIDE;
  virtual bool BAsyncShutdown() OVERRIDE;
  virtual void Unload() OVERRIDE;
  virtual void HandleMessageFromClient(uint64 ullSenderID, uint32 unMsgType,
                                       void *pubData, uint32 cubData) OVERRIDE;
  virtual void HandleMessageFromSystem(uint32 unGCSysMsgType, void *pubData,
                                       uint32 cubData) OVERRIDE;
  virtual void HandleMessageFromGC(int iGCServerIDSender, uint32 unMsgType,
                                   void *pubData, uint32 cubData) OVERRIDE;
  ...
  // provides access to the name of the binary for this GC for development
  // builds. This is blank for non-development builds
  ...
  // called to reset the assert window, this is useful for tracking how many
  // asserts fired within a specific time range
  void ClearAssertWindowCounts();

  // called to add a substring that console output will be filtered against.
  // This is only intended for urgent text squelching
  void AddBlockEmitString(const char *pszStr, bool bBlockConsole,
                          bool bBlockLog);
  void ClearBlockEmitStrings();

  // asserts are always rate limited. If you absolutely need to avoid that, you
  // can use this object to force asserts to be recorded regardless of rate
  // limiting
...
private:
  ...
  // this will handle loading the config file for the GC and initializing the
  // directory, and will return the config key values so that it can be used to
  // load the convars from
  bool BReadConfigDirectory(KeyValuesAD &configValues);

  // handles loading in the convars from the key values loaded with
  // BReadConfigDirectory, and will setup the convars based upon what GC type
  // this is
  ...
  // the version of the GC. This is for reporting purposes only, and will be
  // zero for development builds
  uint32 m_nVersion;
