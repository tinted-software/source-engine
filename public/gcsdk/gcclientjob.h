...
public:
  CGCClientJob(CGCClient *pGCClient)
      : CJob(pGCClient->GetJobMgr()), m_pGCClient(pGCClient),
...
protected:
  ...
  bool BYldSendMessageAndGetReply(CGCMsgBase &msgOut, uint nTimeoutSec,
  ...
  BYldSendMessageAndGetReply_t
  BYldSendMessageAndGetReplyEx(CProtoBufMsgBase &msgOut, uint nTimeoutSec,
  ...
  bool BYldSendMessageAndGetReply(CProtoBufMsgBase &msgOut, uint nTimeoutSec,
                                  CProtoBufMsgBase *pMsgIn, MsgType_t eMsg) {
    if (BYldSendMessageAndGetReplyEx(msgOut, nTimeoutSec, pMsgIn, eMsg) !=
        BYLDREPLY_SUCCESS)
      return false;
    return true;
  }
  ...
  void SetJobTimeout(uint nTimeoutSec) {
    m_cHeartbeatsBeforeTimeout =
        1 + ((nTimeoutSec * k_nMillion) / k_cMicroSecJobHeartbeat);
  }

private:
  virtual bool BYieldingRunJobFromMsg(IMsgNetPacket *pNetPacket) {
    // Protection against a NULL GCClient. Yields so the job is not deleted
    // instantly
    if (!m_pGCClient) {
