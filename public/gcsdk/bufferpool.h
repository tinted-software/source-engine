...
//	together to improve performance. The buffers will naturally grow over
//time 	to accommodate the largest consumers of the feature.
//----------------------------------------------------------------------------
class CBufferPool {
public:
  CBufferPool(const char *pchName, const GCConVar &cvMaxSizeMB,
              const GCConVar &cvInitBufferSize, int nFlags = 0);
  ~CBufferPool();
...
// thread safe version of the above which synchronizes access at the buffer
// allocate/release
class CBufferPoolMT {
public:
  CBufferPoolMT(const char *pchName, const GCConVar &cvMaxSizeMB,
                const GCConVar &cvInitBufferSize, int nFlags = 0)
