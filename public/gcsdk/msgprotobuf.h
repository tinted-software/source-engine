...
//-----------------------------------------------------------------------------
// CProtoBufNetPacket
// Thin wrapper around raw CNetPacket which implements our IMsgNetPacket
// interface.
//-----------------------------------------------------------------------------
...
public:
  CProtoBufNetPacket(CNetPacket *pNetPacket, GCProtoBufMsgSrc eReplyType,
                     const CSteamID steamID, uint32 nGCDirIndex,
                     MsgType_t msgType);
  ...
  // called to obtain access to the body portion of the net packet associated
  // with this message. Will return NULL if not in a valid state
...
public:
  ...
  bool BAsyncSendWithPreSerializedBody(IProtoBufSendHandler &pSender,
                                       const byte *pubBody,
  ...
  static bool BAsyncSendWithPreSerializedBody(IProtoBufSendHandler &sender,
                                              MsgType_t eMsgType,
                                              const CMsgProtoBufHeader &hdr,
                                              const byte *pubBody,
                                              uint32 cubBody);
  // similar to the above, but sends a protobuf object that will be serialized
  // into the buffer
  static bool BAsyncSendProto(IProtoBufSendHandler &sender, MsgType_t eMsgType,
                              const CMsgProtoBufHeader &hdr,
  ...
  // Must be implemented by subclasses. Returns the body. The templated
  // subclasses have their own body accessor that returns the body as the
  // specific type
...
private:
  ...
  // provided header and specified body size and writing the header into the
  // pool. This will return a pointer to the memory, as well as the header size.
  static uint8 *AllocateMessageMemory(MsgType_t eMsgType,
                                      const CMsgProtoBufHeader &hdr,
                                      uint32 cubBodySize,
  ...
  // Pointer to an external net packet if we have one. If we have one then we
  // will not have allocated m_pProtoBufHdr ourselves
...
//-----------------------------------------------------------------------------
// CProtoBufMsgMemoryPoolBase - Interface to allocation pools for each
// protobufmsg type
//-----------------------------------------------------------------------------
...
private:
  ...
  // These counters are important to get correct, so interlocked in case of
  // allocating on threads
...
// The rest of the file needs memdbgon because the code in the templates do
// actual allocation
...
//-----------------------------------------------------------------------------
// CProtoBufMsgMemoryPool - Implementation for allocation pools for
// protobufmsgs. We create one of these per protobuf msg type, created on first
// construction of an object of that type.
//-----------------------------------------------------------------------------
...
public:
  CProtoBufMsgMemoryPool()
      : CProtoBufMsgMemoryPoolBase(
            PB_OBJECT_TYPE::descriptor()->options().GetExtension(
                msgpool_soft_limit),
            PB_OBJECT_TYPE::descriptor()->options().GetExtension(
                msgpool_hard_limit)) {}
...
//-----------------------------------------------------------------------------
// CProtoBufPtrMsg
// Similar to a CProtoBufMsg, but the constructor simply takes in a pointer
// which is a pointer to the protobuf object that is being wrapped by a message.
// This memory is managed by the caller, this object does nothing to free the
// memory
//-----------------------------------------------------------------------------
...
// Handles a message with a header of type MsgHdr_t, a body of type T, and
...
public:
  // Used to alloc a protobuf of this type from the pool. Can be used by
  // functions working with protobufs that aren't messages to take advantage of
  // pooling
  ...
  static void FreeProto(PB_OBJECT_TYPE *pbObj) { s_pMemoryPool->Free(pbObj); }

  // Constructor for an empty message
  CProtoBufMsg(MsgType_t eMsg) : CProtoBufMsgBase(eMsg), m_pProtoBufBody(NULL) {
    VPROF_BUDGET("CProtoBufMsg::CProtoBufMsg( MsgType_t )",
  ...
  CProtoBufMsg(MsgType_t eMsg, CSteamID steamIDClient, int32 nSessionIDClient)
    ...
    VPROF_BUDGET("CProtoBufMsg::CProtoBufMsg( MsgType_t, CSteamID, int32 )",
                 VPROF_BUDGETGROUP_OTHER_NETWORKING);
  ...
  // constructor for use in catching replies or any other place where you have
  // nothing to stuff in the message at construct time
  CProtoBufMsg() : CProtoBufMsgBase(), m_pProtoBufBody(NULL) {
    m_pProtoBufBody = AllocProto();
  }
  ...
  CProtoBufMsg(MsgType_t eMsg, const CProtoBufMsgBase &msgReplyingTo)
      : CProtoBufMsgBase(eMsg), m_pProtoBufBody(NULL) {
    VPROF_BUDGET(
        "CProtoBufMsg::CProtoBufMsg( EMsg, CProtoBufMsgMemoryPoolBase )",
        VPROF_BUDGETGROUP_OTHER_NETWORKING);
    m_pProtoBufBody = AllocProto();
...
template <typename PB_OBJECT_TYPE>
CProtoBufMsgMemoryPool<PB_OBJECT_TYPE>
    *CProtoBufMsg<PB_OBJECT_TYPE>::s_pMemoryPool = NULL;

//-----------------------------------------------------------------------------
// Purpose: Wrapper class to handle alloc/free using the pool allocators
//-----------------------------------------------------------------------------
template <class TMsg> class CProtoBufPoolObj {
private:
  TMsg *m_pMsg;
...
// memdbgon is only supposed to be on in cpp files, turn it off in case the next
// thing includes has a conflict with it
