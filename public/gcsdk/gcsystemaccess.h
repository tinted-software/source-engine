...
// behaviors that can be triggered for system access violation. Each aspect of
// this system has actions that can be enabled or suppressed. The algorithm for
// this is to combine all the enabled actions, then remove all the suppressed
// and perform those operations when a violation is found
...
// a structure that is inserted into a context object and holds onto
// GCAccessSystem index that the context can then initialize itself from.
// Classes can derive from this to initialize itself, but derived class may NOT
// have any members as that would change the binary layout of the object
...
// this will generate a unique system access ID that is guaranteed to be unique
// within a single run of the GC
GCAccessSystem_t GenerateUniqueGCAccessID();

// defines a system access object class which can be used to validate access to
// systems. This class can be registered with the GCA_REGISTER_SYSTEM macro
// using the same name.
#define GCA_SYSTEM(Name)                                                       \
  class CGCA##Name : public CGCAContextSystemRef {                             \
  public:                                                                      \
    static const char *GetName() { return #Name; }                             \
    static GCAccessSystem_t GetID() {                                          \
      static GCAccessSystem_t s_ID = GenerateUniqueGCAccessID();               \
      return s_ID;                                                             \
    }                                                                          \
    CGCA##Name() : CGCAContextSystemRef(GetID()) {}                            \
  };

// similar to the above, but does not auto assign an ID and instead uses the
// fixed specified ID
...
#define GCA_REGISTER_SYSTEM(Name)                                              \
  GGCAccess().RegisterSystem(CGCA##Name::GetName(), CGCA##Name::GetID())

// called to begin a context class which derives from (has the same rights as) a
// parent context. The proper setup for this is:
...
#define GCA_BEGIN_CONTEXT_PARENT(ClassName, Parent)                            \
  ...
  public:                                                                      \
    ClassName()                                                                \
        : m_nFirstSystemMarker_##ClassName(0),                                 \
          m_nLastSystemMarker_##ClassName(0) {                                 \
      Init(#ClassName);                                                        \
      for (const GCAccessSystem_t *pSystem =                                   \
               &m_nFirstSystemMarker_##ClassName + 1;                          \
           pSystem != &m_nLastSystemMarker_##ClassName; pSystem++)             \
        AddSystem(*pSystem);                                                   \
    }                                                                          \
...
#define GCA_BEGIN_CONTEXT(ClassName)                                           \
  GCA_BEGIN_CONTEXT_PARENT(ClassName, CGCAccessContext)

// called to add a context to a system. This must come between a
// BEGIN/END_CONTEXT block
#define GCA_CONTEXT_SYSTEM(SystemName) const CGCA##SystemName SystemName;

// called to add a generic system reference to a context. The reference will use
// the provided class and value name and the specified ID
...
// creates a context that is basically the same as another context but aliased.
// Useful if you want a derived context that doesn't have any additional systems
...
#define GCA_EMPTY_CONTEXT(Context)                                             \
  GCA_BEGIN_CONTEXT(Context) GCA_END_CONTEXT(Context)
...
#define GCA_VALIDATE_ACCESS(SystemName)                                        \
  GGCAccess().ValidateAccess(GCA_GET_SYSTEM_ID(SystemName))
#define GCA_VALIDATE_ACCESS_STEAMID(SystemName, SteamID)                       \
  GGCAccess().ValidateSteamIDAccess(GCA_GET_SYSTEM_ID(SystemName), (SteamID))

// a context, which is a collection of systems that are valid to be accessed.
// Each job and GC has a context to validate system access
class CGCAccessContext {
public:
  CGCAccessContext();

  void Init(const char *pszName, uint32 nActions = 0,
            uint32 nSuppressActions = 0);
  ...
  // given a context, this will verify that it is a proper subset of the
  // provided context
...
// the global GC access tracker which has the systems registered and handles
// accumulating stats, presenting reports, etc
class CGCAccess {
public:
  CGCAccess();
  ...
  void RegisterSystem(const char *pszName, GCAccessSystem_t nID,
                      uint32 nActions = 0, uint32 nSupressActions = 0);

  //-----------------------------------------
  // Validation

  // called to verify access to a system. The return of this will be false if it
  // is an invalid access and the return fail action is specified
  bool ValidateAccess(GCAccessSystem_t nSystem);
  // verify access to a system that also requires that the active SteamID of the
  // current job matches the provided Steam ID, and count it as a fail
  bool ValidateSteamIDAccess(GCAccessSystem_t nSystem, CSteamID steamID);

  // called to suppress tracking of access for the specified access type. This
  // should typically only be accessed via the access supress utility object
  ...
  void FullReport(const char *pszSystemFilter, const char *pszContextFilter,
                  const char *pszJobFilter, EDisplay eDisplay) const;
  // dumps a dependency report for a given system. Essentially for every job
  // that depends upon the named system, what are the other systems that it also
  // relies upon
  void DependencyReport(const char *pszSystem, EDisplay eDisplay) const;

  // called to register a one time assert that will fire the next time the
  // job/context/system pair is hit
  bool CatchSingleAssert(const char *pszSystem, bool bContext,
                         const char *pszContextOrJob);
  // clears all registered single asserts that have not yet fired
  void ClearSingleAsserts();

  // if there is not a specific job in flight, this global context is what will
...
private:
  // handles internally validating the system. Takes an optional parameter
  // indicating if the steam ID check failed
  bool InternalValidateAccess(GCAccessSystem_t nSystem, CSteamID steamID,
                              CSteamID expectedID);
...
// utility class to temporarily disable access tracking for a system while
// within the scope of this object
class CGCAccessSupressTracking {
public:
  CGCAccessSupressTracking(GCAccessSystem_t nSystem) : m_nSystem(nSystem) {
    GGCAccess().SuppressAccess(m_nSystem, false);
  }
