...
// the generic interface for frame functions. These can be derived from and
// should execute the work in BRun. It returns a boolean indicating whether or
// not it has more work to complete
...
public:
  ...
  enum EFrameType {
    ...
    k_EFrameType_RunUntilCompleted, // run tasks that shouldn't wait a frame
                                    // between execution (gets called at least
                                    // once per frame, more if work to do & we
                                    // have time)
    k_EFrameType_RunUntilCompletedLowPri, // as above, but only runs if
                                          // k_EFrameTypeRunUntilCompleted
                                          // function return they have no
  ...
  // called to handle registering for updates and unregistering. Not registered
  // by default
  void Register();
  void Deregister();
...
protected:
  // the frame function manager has access to the internals to avoid exposing
  // them to derived classes
  ...
  uint64
      m_nTrackedTime; // how much time we have tracked with this frame function
...
// a utility class that helps handle registering for frame functions and adapts
// it to a member function call
template <class T> class CFrameFunction : public CBaseFrameFunction {
public:
  typedef bool (T::*func_t)(const CLimitTimer &limitTimer);

  // construct and register all in one
  CFrameFunction(T *pObj, func_t func, const char *pchName,
...
// the main manager that handles registration of all the frame functions and
// tracks performance and dispatches appropriately
...
public:
  ...
  // called to execute the main frame. This should preceded individual frame
  // ticks
  ...
  // called to dump a listing of the performance timings of all the frame
...
private:
  // sort function
  static bool SortFrameFuncByTime(const CBaseFrameFunction *pLhs,
                                  const CBaseFrameFunction *pRhs);

  // called to update the frame functions associated with the specified list
  bool RunFrameList(CBaseFrameFunction::EFrameType eType,
                    const CLimitTimer &limitTimer);

  // the listing of subscribed
  CUtlVector<CBaseFrameFunction *>
      m_MainLoopFrameFuncs[CBaseFrameFunction::k_EFrameType_Count];
