...
struct JobStatsBucket_t {
  JobStatsBucket_t() { memset(this, 0, sizeof(JobStatsBucket_t)); }
  char m_rgchName[64];
  uint64 m_cCompletes;
...
//			It's primarily used for routing incoming messages to
//jobs.
//-----------------------------------------------------------------------------
class CJobMgr {
public:
  ...
  // Route this message to an existing Job, or create a new one if that JobID
  // does not exist
  bool BRouteMsgToJob(void *pParent, IMsgNetPacket *pNetPacket,
  ...
  // called by a job that has just been started to place itself on the yield
  // queue instead of running
  void AddDelayedJobToYieldList(CJob &job);

#ifdef GC
  // resumes the specified job if it is, in fact, waiting for a SQL query to
  // return
  bool BResumeSQLJob(JobID_t jobID);

  // yields waiting for a query response
  bool BYieldingRunQuery(CJob &job, CGCSQLQueryGroup *pQueryGroup,
  ...
  void PassMsgToJob(CJob &job, IMsgNetPacket *pNetPacket,
                    const JobMsgInfo_t &jobMsgInfo);
  ...
  bool BRouteWorkItemCompleted(JobID_t jobID, bool bWorkItemCanceled) {
    return BRouteWorkItemCompletedInternal(jobID, bWorkItemCanceled,
                                           /* bShouldExist */ true,
                                           /* bResumeImmediately */ true);
  }
  ...
  bool BRouteWorkItemCompletedDelayed(JobID_t jobID, bool bWorkItemCanceled) {
    return BRouteWorkItemCompletedInternal(jobID, bWorkItemCanceled,
                                           /* bShouldExist */ true,
                                           /* bResumeImmediately */ false);
  }
  bool BRouteWorkItemCompletedIfExistsDelayed(JobID_t jobID,
  ...
  void
  ProfileJobs(EJobProfileAction ejobProfileAction,
...
private:
  bool BRouteWorkItemCompletedInternal(JobID_t jobID, bool bWorkItemCanceled,
                                       bool bShouldExist,
                                       bool bResumeImmediately);

  // Create a new job for this message
  bool BLaunchJobFromNetworkMsg(void *pParent, const JobMsgInfo_t &jobMsgInfo,
  ...
  bool BGetIJob(JobID_t jobID, EJobPauseReason eJobPauseReason,
  ...
  bool BResumeYieldingJobsFromList(
  ...
  static bool JobSleepingLessFunc(JobSleeping_t const &lhs,
...
//-----------------------------------------------------------------------------
// Purpose: passthrough function just so the CJob internal data can be kept
// private
//-----------------------------------------------------------------------------
...
#define GC_REG_JOB(parentclass, jobclass, jobname, msg, servertype)            \
  ...
  static const GCSDK::JobType_t g_JobType_##jobclass = {                       \
      jobname, (GCSDK::MsgType_t)msg, servertype,                              \
      (GCSDK::JobCreationFunc_t)CreateJob_##jobclass};                         \
  GCSDK::CJob *CreateJob_##jobclass(parentclass *pParent,                      \
                                    void *pvStartParam) {                      \
  ...
  public:                                                                      \
    CRegJob_##jobclass() { Job_RegisterJobType(&g_JobType_##jobclass); }       \
  } g_RegJob_##jobclass;
...
#define REG_WEBAPI_JOB(parentclass, jobclass, jobname, servertype)             \
  ...
  static const JobType_t g_JobType_##jobclass = {                              \
      jobname, k_EGCMsgInvalid, servertype,                                    \
      (JobCreationFunc_t)CreateJob_##jobclass};                                \
  CJob *CreateJob_##jobclass(parentclass *pParent, void *pvStartParam) {       \
