...
public:
  ...
  void Set(const char *pchName, int nSQLColumn, EGCSQLType eGCSQLType,
           int cubFixedSize, int nColFlags, int cubMaxSize);
  ...
  bool BIsAutoIncrement() const {
    return 0 != (m_nColFlags & k_nColFlagAutoIncrement);
  }
  bool BIsPrimaryKey() const {
    return 0 != (m_nColFlags & k_nColFlagPrimaryKey);
  }
  bool BIsExplicitlyIndexed() const {
    return BIsIndexed() && !(BIsPrimaryKey() || BIsUnique());
  }
...
private:
  CColumnInfo(
      CColumnInfo &); // no copy constructor, disable default copy constructor
  CColumnInfo &operator=(CColumnInfo &); // no assignment operator, disable
...
public:
  ...
  const char *GetName() const { return m_rgchName; }
  void AddColumn(const char *pchName, int nSQLColumn, EGCSQLType eGCSQLType,
                 int cubFixedSize, int nColFlags, int cubMaxSize);
  void SetAllColumnsAdded() { m_bAllColumnsAdded = true; }
  void PrepareForUse();
  int GetFixedSize() const { return m_cubFixedSize; }
  int GetNumColumns() const { return m_VecColumnInfo.Count(); }
  const CColumnInfo &GetColumnInfo(uint32 unColumn) const {
    return m_VecColumnInfo[unColumn];
  }
  CColumnInfo &GetColumnInfo(uint32 unColumn) {
    return m_VecColumnInfo[unColumn];
  }
  bool BFindColumnByName(const char *pchName, int *piColumn);
  bool BPreparedForUse() const { return m_bPreparedForUse; }
  void EnsureCapacity(int cColumns) {
    m_VecColumnInfo.EnsureCapacity(cColumns);
  }
  int GetChecksum();
  ...
  const FieldSet_t &GetPKFields() {
    Assert(BHasPrimaryKey());
    return GetIndexFields()[m_iPKIndex];
  }
...
private:
  ...
  CMapIColumnInfo
      m_MapIColumnInfo; // Map of name->column index for quick lookup by name
  EPrimaryKeyType m_nHasPrimaryKey; // Does this table contain a column that is
                                    // a primary key?
  ...
  bool m_bHaveColumnNameIndex; // Have we created a column name index?  (Only
                               // generated if someone asks.)
  bool m_bHaveChecksum; // Have we generated a checksum?  (Only generated if
                        // someone asks.)
  int m_nChecksum;      // checksum of this record info for quick comparisons -
  ...
  CUtlVector<FKData_t>
      m_VecFKData; // vector of all FK relationships defined on this table
