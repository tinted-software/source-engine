...
public:
  ...
  virtual const uint8 *ReadVarDataField(const CVarField &field,
                                        uint32 *pcubField) const;
  virtual bool SetVarCharField(CVarCharField &field, const char *pchString,
                               bool bTruncate, int32 iField);
  virtual void SetVarDataField(CVarField &field, const void *pvData,
                               uint32 cubData);
...
protected:
  // copies the contents of the record. The assignement operator uses this
  // internally
...
//
// This is used as a base class for the CSchTable wrapper classes emitted
// by the schema compiler when the involved table *does* have variable-length
// data
//-----------------------------------------------------------------------------
...
public:
  ...
  virtual CSchema *GetPSchema() { return m_pSchema; }
  virtual const CSchema *GetPSchema() const { return m_pSchema; }
  ...
  // allocated var block means we've allocated a block for the variable part of
  // this record
...
public:
  ...
  virtual ~CRecordExternal() { Cleanup(); }
...
protected:
  ...
  uint8 *m_pubRecordFixedExternal; // If the fixed record is not a part of this
                                   // object, this points to where it is
...
#define WRITE_VAR_CHAR_FIELD(record, field, text)                              \
  (record).SetVarCharField((record).m_##field, text, false,                    \
                           (record).k_iField_##field)

#define WRITE_VAR_CHAR_FIELD_TRUNC(record, field, text)                        \
  (record).SetVarCharField((record).m_##field, text, true,                     \
                           (record).k_iField_##field)
...
// within them
//-----------------------------------------------------------------------------
template <class T, int I, typename F> class CDefSchOps {
public:
  static bool LessFunc(const T &lhs, const T &rhs) {
    ...
    // Read the data and make sure the sizes are correct for the field type we
    // expect
    uint8 *pubLhs;
    uint8 *pubRhs;
    bool bRet;
...
#define DefSchLessFunc(RecordType, FieldIndex, FieldType)                      \
  CDefSchOps<RecordType, FieldIndex, FieldType>::LessFunc
#define DefSchLessFuncCtx(RecordType, FieldIndex, FieldType)                   \
  CDefSchOps<RecordType, FieldIndex, FieldType>::LessFuncCtx

//-----------------------------------------------------------------------------
// Specializations for string fields
//-----------------------------------------------------------------------------
template <class T, int I> class CDefSchOps<T, I, char *> {
public:
  static bool LessFunc(const T &lhs, const T &rhs) {
    // Check that the field number is valid
    COMPILE_TIME_ASSERT(I >= 0 && I < T::k_iFieldMax);
...
template <class T, int I> class CDefSchOps<T, I, const char *> {
public:
  static bool LessFunc(const T &lhs, const T &rhs) {
...
template <typename TRecord> class CRecordTypeConcrete : public CRecordType {
public:
