...
class CTriggerInfo {
public:
  CTriggerInfo() : m_eTriggerType(k_ETriggerType_Invalid), m_bMatched(false) {}
  ...
  char m_szTriggerTableName[k_cSQLObjectNameMax]; // name of the table hosting
                                                  // this trigger
...
public:
  ...
  CSchema *AddNewSchema(int iTable, ESchemaCatalog eCatalog,
                        const char *pstrName) {
  ...
  void AddRenameField(const char *pchTableName, const char *pchFieldNameOld,
                      const char *pchFieldNameNew);
  void AddAlterField(const char *pchTableName, const char *pchFieldNameOld,
                     const char *pchFieldNameNew,
                     PfnAlterField_t pfnAlterField);

  // declare that a trigger is on a table
  void AddTrigger(ESchemaCatalog eCatalog, const char *pchTableName,
                  const char *pchTriggerName, ETriggerType eTriggerType,
                  const char *pchTriggerText);

  // Schema conversion helper: figure out what table to map a table from a
  // different schema to
  bool BCanConvertTable(const char *pchTableSrc, int *piTableDst);

  // full text catalogs
  void AddFullTextCatalog(enum ESchemaCatalog eCatalog,
                          const char *pstrCatalogName, int nFileGroup);
  int GetFTSCatalogByName(enum ESchemaCatalog eCatalog,
  ...
  const CFTSCatalogInfo &GetFTSCatalogInfo(int nIndex) const {
    return m_vecFTSCatalogs[nIndex];
  }

  const CUtlVector<CTriggerInfo> &GetTriggerInfos() const {
    return m_VecTriggers;
  }
...
private:
  ...
  uint8 *m_pubScratchBuffer; // Big enough to hold any record or sparse record
                             // in this schemafull
