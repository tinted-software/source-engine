...
const int k_nColFlagUnique =
    0x0002; // this column has a uniqueness constraint - creates implicit index
const int k_nColFlagPrimaryKey =
    0x0004; // this column has a primary key constraint - creates implicit
            // uniqueness constraint & implicit index
const int k_nColFlagAutoIncrement =
    0x0008; // this column can have it's values created implicitly by the sql
            // counter
...
// EPrimaryKeyType
// This shows if a table has a primary key and, if so, if it has multiple
// columns or not.
...
enum EWipePolicy {
  ...
  k_EWipePolicyPreserveForStress =
      1, // preserve for stress tests, wipe before regular tests
...
struct FKData_t {
  ...
  bool operator==(const FKData_t &other) const {
    ...
    if (m_eOnDeleteAction != other.m_eOnDeleteAction ||
        m_eOnUpdateAction != other.m_eOnUpdateAction)
      return false;
...
struct SerSchema_t {
  ...
  int16 m_cField; // # of fields in the schema (int16 for backward-compatibility
                  // reasons)
  int16 m_ETableGroup; // Our table's TableGroup (int16 for
                       // backward-compatibility reasons) - OBSOLETE
};
...
struct AlterField_t {
  char m_rgchFieldNameOld[k_cSQLObjectNameMax]; // Name of field in the old
                                                // schema
...
class FieldSet_t {
public:
  FieldSet_t(bool bUnique, bool bClustered, const CUtlVector<int> &vecFields,
             const char *pstrIndexName)
  ...
  FieldSet_t(const FieldSet_t &refOther) { DoAssignment(refOther); }
  ...
  void DoAssignment(const FieldSet_t &refOther) {
    ...
    Q_strncpy(m_szIndexName, refOther.m_szIndexName,
              Q_ARRAYSIZE(m_szIndexName));
  }

  // get count of fields in this index
  int GetCount() const { return m_VecFields.Count(); }

  // get count of included fields in this index
  int GetIncludedCount() const { return m_VecIncluded.Count(); }

  void AddIncludedColumn(int nIndex) { m_VecIncluded.AddToTail(nIndex); }
  ...
  int GetField(int nIndex) const { return m_VecFields[nIndex]; }

  int GetIncludedField(int nIndex) const { return m_VecIncluded[nIndex]; }

  // is this index clustered?
  bool IsClustered() const { return m_bClustered; }

  // is this index unique?
  bool IsUnique() const { return m_bUnique; }

  void SetClustered(bool bIsClustered) { m_bClustered = bIsClustered; }
  ...
  int GetFillFactor() const { return m_nFillFactor; }

  const char *GetIndexName() const { return m_szIndexName; }

  // determine if this fieldset is equal to the other one
  static bool CompareFieldSets(const FieldSet_t &refThis,
                               CRecordInfo *pRecordInfoThis,
                               const FieldSet_t &refOther,
                               CRecordInfo *pRecordInfoOther);
...
private:
  CUtlVector<int> m_VecFields;   // ids of fields; indexes into m_VecFields of
                                 // CSchema for a table
  CUtlVector<int> m_VecIncluded; // ids of included fields
  int m_nFillFactor; // fill factor for the index; zero means to use the
                     // server's default
...
public:
  ...
  // called to make final calculations when all fields/indexes/etc have been
  // added and the schema is ready to be used
  void PrepareForUse();

  // Sizing information
  uint32 CubSerialSchema() const {
    return (sizeof(SerSchema_t) + m_VecField.Count() * sizeof(SerField_t));
  }

  // Size of total fixed-length portion of record
  uint32 CubRecordFixed() const { return (m_cubRecord); }

  // Size of the total variable-length portion of record (zero if no var-length
  // fields)
  ...
  // Access field data - fixed or variable (may return NULL/0 for empty var
  // field)
  bool BGetFieldData(const void *pvRecord, int iField, uint8 **ppubField,
                     uint32 *pcubField) const;
  bool BSetFieldData(void *pvRecord, int iField, uint8 *pubField,
                     uint32 cubField, bool *pbVarBlockRealloced);

  bool BGetVarField(const void *pvRecord, const VarField_t *pVarField,
                    uint8 **ppubField, uint32 *pcubField) const;
  bool BSetVarField(void *pvRecord, VarField_t *pVarField, const void *pvData,
                    uint32 cubData, bool *pbRealloced, bool bFreeOnRealloc);

  // Adjust var-block pointer, if present, to point just after the fixed part of
  // the record
  void FixupDeserializedRecord(void *pvRecord);

  // Render a record in text format
  void RenderRecord(uint8 *pubRecord);
  void RenderField(uint8 *pubRecord, int iField, int cchBuffer,
                   char *pchBuffer);
  ...
  VarField_t *GetPVarField(void *pvRecord, int iField) {
    return (VarField_t *)((uint8 *)pvRecord + m_VecField[iField].m_dubOffset);
  }
  ...
  const FieldSet_t &GetPKFields() {
    Assert(m_iPKIndex != -1);
    return GetIndexes()[m_iPKIndex];
  }
  ...
  void SetESchemaCatalog(ESchemaCatalog eSchemaCatalog) {
  ...
  void InitRecordRandom(uint8 *pubRecord, uint32 unPrimaryIndex,
                        bool *pbVarBlockRealloced, bool bFreeVarBlockOnRealloc);
  void SetFieldRandom(uint8 *pubRecord, int iField, bool *pbVarBlockRealloced,
  ...
  void EnsureFieldCount(int cFields) { m_VecField.EnsureCapacity(cFields); }

  // This adds a field from our intrinsic schema to us
  void AddField(char *pchName, char *pchSQLName, EGCSQLType eType,
                uint32 cubSize, int cchMaxLength);
  void AddIntField(char *pchName, char *pchSQLName, EGCSQLType eType,
                   int cubSize);
  ...
  void AddFullTextIndex(CSchemaFull *pSchemaFull, const char *pchCatalogName,
  ...
  int AddIndexToFieldNumber(int iField, const char *pchIndexName,
                            bool bClustered);

  // We want to index a particular set of fields
  //	pchNames includes the names, separated by commas, of each field
  int AddIndexToFieldList(const char *pchNames, const char *pchIndexName,
  ...
  int ClusteredIndexField(int nFillFactor, const char *pchName,
                          const char *pchIndexName);

  // We want to index a particular list of fields in a group
  int ClusteredIndexFields(int nFillFactor, const char *pchIndexName,
                           const char *pchNames);
  ...
  void AddFullTextCatalog(ESchemaCatalog eCatalog, const char *pstrCatalogName,
                          const char *pstrFileGroupName);

  // Adds a FK on the table
  void AddFK(const char *pchName, const char *pchColumn,
  ...
  void AddAlterField(const char *pchFieldNameOld, const char *pchFieldNameNew,
                     PfnAlterField_t pfnAlterField);

  // Schema conversion helper: figure out what field to map a field from a
  // different schema to
  bool BCanConvertField(const char *pchFieldSrc, int *piFieldDst,
  ...
  void Validate(CValidator &validator,
                const char *pchName); // Validate our internal structures
  void
  ValidateRecord(uint8 *pubRecord); // Validate a record that uses our schema
...
private:
  ...
  int m_iPKIndex; // index into of m_VecIndexes of our PK index; k_iFieldNil if
                  // no PK
  ...
  bool m_bAllowWipeInProd; // should we allow WipeTable operations on this table
                           // in the beta/public universe?
  EWipePolicy m_wipePolicy;  // should this table be wiped between all tests, no
                             // tests, or non-stress tests?
  ...
  CRecordInfo
      *m_pRecordInfo; // The record description corresponding to this schema.
                      // (Similar info, record description is new form, have
                      // both for a while during DS->SQL switch)
  CUtlVector<int>
      m_VecFullTextIndexes;    // vector of indexes into m_VecField of fields
                               // covered by this table's full-text index.
  int m_nFullTextIndexCatalog; // index of catalog to use for creating full-text
                               // indexes
  CUtlVector<FKData_t>
      m_VecFKData; // data on foreign keys for this schema object

  uint32 m_cubRecord;       // Binary record length
  int m_nReportingInterval; // reporting interval of this table if stats; 0 if
                            // not stats
  ...
  // Cached queries
  mutable CUtlString
      m_sInsertStatementText; // Cached insert statement for the table
  CUtlString
      m_sMergeStatementTextOnPKWhenMatchedUpdateWhenNotMatchedInsert; // Cached
                                                                      // insert
                                                                      // or
                                                                      // update
                                                                      // via
                                                                      ...
                                                                      // table
  CUtlString
      m_sMergeStatementTextOnPKWhenNotMatchedInsert; // Cached insert via MERGE
