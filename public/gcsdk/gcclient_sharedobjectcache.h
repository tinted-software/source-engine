...
/// Enumerate different events that might trigger a callback to an
...
enum ESOCacheEvent {
  ...
  /// We received a full update from the GC on a cache for which we were already
  /// subscribed. This can happen if connectivity is lost, and then restored
  /// before we realized it was lost.
...
public:
  /// Called when a new object is created in a cache we are currently subscribed
  /// to, or when we are added as a listener to a cache which already has
  /// objects in it
  ///
  ...
  virtual void SOCreated(const CSteamID &steamIDOwner,
                         const CSharedObject *pObject,
                         ESOCacheEvent eEvent) = 0;

  /// Called when we are about to update objects in our cache but haven't done
  /// any of the work yet.
  ///
  /// eEvent will be one of:
  /// - eSOCacheEvent_Resubscribed
  /// - eSOCacheEvent_Incremental
  virtual void PreSOUpdate(const CSteamID &steamIDOwner,
                           ESOCacheEvent eEvent) = 0;

  /// Called when an object is updated in a cache we are currently subscribed
  /// to.
  ///
  /// eEvent will be one of:
  /// - eSOCacheEvent_Resubscribed
  /// - eSOCacheEvent_Incremental
  virtual void SOUpdated(const CSteamID &steamIDOwner,
                         const CSharedObject *pObject,
                         ESOCacheEvent eEvent) = 0;
  ...
  /// - eSOCacheEvent_Resubscribed
  /// - eSOCacheEvent_Incremental
  virtual void PostSOUpdate(const CSteamID &steamIDOwner,
                            ESOCacheEvent eEvent) = 0;

  /// Called when an object is about to be deleted in a cache we are currently
  /// subscribed to. The object will have already been removed from the cache,
  /// but is still valid.
  ///
  /// eEvent will be one of:
  /// - eSOCacheEvent_Incremental
  /// - eSOCacheEvent_Resubscribed
  virtual void SODestroyed(const CSteamID &steamIDOwner,
                           const CSharedObject *pObject,
                           ESOCacheEvent eEvent) = 0;
  ...
  /// - eSOCacheEvent_ListenerAdded
  ///
  /// A listener is guaranteed that it will not receive incremental updates
  /// (SOCreated, SOUpdated, SODestroyed) while not subscribed.  (Before the
  /// SOCacheSubscribed or after SOCacheUnsubscribed.)  However, note that it
  /// may be possible to receive an SOCacheSubscribed message while already
  /// subscribed.  This can happen if the GC loses and restores connection, or
  /// otherwise decides that a full update is necessary.
  virtual void SOCacheSubscribed(const CSteamID &steamIDOwner,
                                 ESOCacheEvent eEvent) = 0;
...
public:
  ...
  void SOCacheSubscribed(const CSteamID &steamIDOwner,
                         ESOCacheEvent eEvent) const;
  void SOCacheUnsubscribed(const CSteamID &steamIDOwner,
                           ESOCacheEvent eEvent) const;
...
public:
  CGCClientSharedObjectTypeCache(int nTypeID,
                                 const CGCClientSharedObjectContext &context);
  virtual ~CGCClientSharedObjectTypeCache();

  bool
  BParseCacheSubscribedMsg(const CMsgSOCacheSubscribed_SubscribedType &msg,
  ...
  CSharedObject *BCreateFromMsg(const void *pvData, uint32 unSize,
                                bool *bUpdatedExisting);
...
private:
  ...
  CGCClientSharedObjectTypeCache *GetTypeCacheByIndex(int nIndex) {
    return (CGCClientSharedObjectTypeCache *)
        CSharedObjectCache::GetTypeCacheByIndex(nIndex);
  }
