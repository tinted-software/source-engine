...
public:
  ...
  bool BIsScheduled() const {
    return m_nAbsScheduleBucket != IGCScheduledFunction::knInvalidBucket;
  }

private:
  // the absolute bucket that we were scheduled in (or invalid). Used to enable
  // deregistering
...
class CGlobalScheduledFunction : public IGCScheduledFunction {
public:
...
// the same as the above, but automatically deletes the object once the event is
// fired
class CGlobalScheduledFunctionAutoDelete : public CGlobalScheduledFunction {
public:
...
template <class T> class CScheduledFunction : public IGCScheduledFunction {
public:
...
template <class T>
class CScheduledFunctionAutoDelete : public CScheduledFunction<T> {
public:
  typedef void (T::*func_t)();

  CScheduledFunctionAutoDelete() {}
  CScheduledFunctionAutoDelete(T *pObj, func_t pfn, uint32 nDelayMS) {
    Schedule(pObj, pfn, nDelayMS);
  }
  virtual void OnEvent() OVERRIDE {
...
public:
  ...
  // called to initialize the starting time for the scheduled function manager.
  // It doesn't need to be globally absolute, just relative to the time values
  // provided to the run function
  void InitStartingTime();

  // called to register a scheduled event for a certain period in the future,
  // with the delay specified in milliseconds. This has resolution of an
  // individual frame, and will unregister from any previously registered time
  // slot
  void ScheduleMS(IGCScheduledFunction *pEvent, uint32 nMSDelay);
  // similar to the above, but instead of having frame resolution, this has
  // second level resolution, and should be used for low granularity tasks
  void ScheduleSecond(IGCScheduledFunction *pEvent, uint32 nSDelay);
  // similar to the above but has minute level resolution which should be used
  // for very low resolution tasks
...
private:
  // called internally by the other schedule functions to schedule the event at
  // the specified resolution in our resolution array
  void InternalSchedule(uint32 nResolution, IGCScheduledFunction *pEvent,
                        uint32 nMSDelay);

  // the list type that we store all of the entries in. We use a single list to
  // avoid the overhead of so many lists
  typedef CUtlLinkedList<IGCScheduledFunction *, uint32> TScheduleList;

  // all information tied to a specific resolution, including the time hash
  // buckets, number of buckets, and which buckets it has executed
  class CScheduleBucket {
  public:
    CScheduleBucket();
    ~CScheduleBucket();
    void Init(TScheduleList &MasterList, uint32 nNumBuckets,
              uint32 nMicroSPerBucket);

    // maps a micro second time to a bucket time
    uint32 GetAbsScheduleBucketIndex(uint64 nMicroSTime) const {
      return (uint32)(nMicroSTime / m_nMicroSPerBucket);
    }

    // called to run registered functions
    void RunFunctions(TScheduleList &MasterList, uint64 nMicroSTime);

    // for each bucket, we insert a node into the master linked list, then our
    // list runs from this node to the next empty node (or end) in the list
    uint32 *m_pBuckets;
  ...
  // the list of all of our entries. We store bucket starts within here, and
  // then insert the events in between them
