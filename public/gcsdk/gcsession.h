...
//-----------------------------------------------------------------------------
// Utility class to handle rate limiting based upon a steam ID and message using
// two console variables to control rate
//-----------------------------------------------------------------------------

// utility class to handle rate limiting based upon a steam ID
class CSteamIDRateLimit {
public:
  CSteamIDRateLimit(const GCConVar &cvNumPerPeriod,
                    const GCConVar *pcvPeriodS = NULL);
  ~CSteamIDRateLimit();
...
//   A utility class to track when messages go over so that we can see
//   users/msgs that are being spammed
//------------------------------------------------------------------------------------------
...
public:
  ...
  void SetIsShuttingDown(bool bIsShuttingDown) {
    m_bIsShuttingDown = bIsShuttingDown;
  }
...
public:
  CGCUserSession(const CSteamID &steamID, CGCSharedObjectCache *pCache)
      : CGCSession(steamID, pCache) {}
...
public:
  CGCGSSession(const CSteamID &steamID, CGCSharedObjectCache *pCache,
  ...
  // virtual and not yielding. For custom behavior override the Pre*() hooks
  // below
...
protected:
  ...
  // very important that these do not yield. If you need to yield, start a job
  // instead
