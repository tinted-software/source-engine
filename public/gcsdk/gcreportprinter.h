...
namespace GCSDK {
//-----------------------------------------------------------------------------------------------------
// CGCReportPrinter - A class that will handle formatting a report for
// outputting to a console or other data sources. Just create an instance of the
// class, add the columns, then add the fields, and print. Below is an example:
//
// CGCReportPrinter rp;
...
public:
  ...
  // what type of summary to report at the end of the report for the column (not
  ...
  // called to handle inserting columns into the report of various data types.
  // These must be called before any data has been added to the report, and will
  // fail if there is outstanding data
  bool AddStringColumn(const char *pszColumn);
  bool AddSteamIDColumn(const char *pszColumn);
  bool AddIntColumn(const char *pszColumn, ESummaryType eSummary,
                    EIntDisplayType eIntDisplay = eIntDisplay_Normal);
  bool AddFloatColumn(const char *pszColumn, ESummaryType eSummary,
