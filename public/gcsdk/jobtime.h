...
//
// Purpose: Encapsultes the job system's version of time (which is != to wall
// clock time)
//
//=============================================================================
...
// CJobTime
// This is our primary job time structure.  It's similar to the Windows FILETIME
// structure, but with 1/10th the nominal resolution. It offers 1 microsecond
// resolution beginning on January 1, 1601. This is NOT wall clock time, it is
// time based proxy for a frame counter. This time PAUSES when you are in the
// debugger. Most timeout checks (things that don't need to be exact time wise
// should use this );
class CJobTime {
public:
  CJobTime();
...
private:
  ...
  static uint64
      sm_lTimeCur; // Cached value of the current time (updated each frame)
};
