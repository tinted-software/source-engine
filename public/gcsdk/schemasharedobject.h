...
#ifndef GC
#error                                                                         \
    "CSchemaSharedObject is only intended for use on GC-only shared objects. It shouldn't be included on the client"
#endif
...
public:
  static bool BYieldingAddInsertToTransaction(CSQLAccess &sqlAccess,
                                              CRecordBase *pRecordBase);
  static bool BYieldingAddWriteToTransaction(CSQLAccess &sqlAccess,
                                             CRecordBase *pRecordBase,
                                             const CColumnSet &csDatabaseDirty);
  static bool BYieldingAddRemoveToTransaction(CSQLAccess &sqlAccess,
                                              CRecordBase *pRecordBase);
...
public:
  ...
  virtual bool BAddDestroyToMessage(CUtlBuffer &bufDestroy) const {
    return false;
  }
  virtual bool BAddDestroyToMessage(std::string *pBuffer) const {
    return false;
  }
  virtual bool BUpdateFromNetwork(const CSharedObject &objUpdate) {
    return false;
  }
...
protected:
  ...
  const CRecordBase *GetPObject() const {
    return const_cast<CSchemaSharedObjectBase *>(this)->GetPObject();
  }
...
public:
  CSchemaSharedObject()
      : CSchemaSharedObjectBase(GCSDK::GSchemaFull()
                                    .GetSchema(SchObject_t::k_iTable)
                                    .GetRecordInfo()) {}

  ~CSchemaSharedObject() {
#ifdef DEBUG
    // Ensure this SO is not in any cache, or we have an error. Note we must
    // provide the type
