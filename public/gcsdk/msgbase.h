...
public:
  ...
  //
  // Client messages only in the old format, optional in any msg in the new
  // format
  //
...
protected:
  // Needed due to CRefCount inheritance which makes this not a pure interface
  // class
...
protected:
  void operator=(const CIMsgNetPacketAutoRelease &that) {
    AssertMsg(false,
              "Not safe to copy since releases references on destruction");
  }
...
//			IMPORTANT: Note the distinction between pubData and
//cubData 			(which refer to the message payload), and pubMsg and cubMsg 			(which
//refer to the entire message, with the header).
//-----------------------------------------------------------------------------
template <typename MSG_HEADER_TYPE> class CMsgBase_t {
public:
  // Send constructor
  ...
  CMsgBase_t(uint32 cubHdr, uint32 cubStruct, uint8 *pubPkt,
             uint32 cubPkt /*, HCONNECTION hConnection = NULL*/);
  ...
  // Read variable-length data (can also read manually using PubVarData(),
  // CubVarData()
  ...
  // returns pointer to data (and size of data in out ptr), and NULLs out own
  // pointer; caller now owns this memory and must free
  ...
  void ResetWritePtr() { m_cubPkt = m_cubMsgHdr + m_cubStruct; }
...
protected:
  ...
  // HCONNECTION m_hConnection;		// Connection on which we received the
  // message
private:
...
template <typename MSG_HEADER_TYPE>
CMsgBase_t<MSG_HEADER_TYPE>::CMsgBase_t(uint32 cubHdr, uint32 cubStruct,
                                        uint8 *pubPkt, uint32 cubPkt)
    : m_cubMsgHdr(cubHdr) {
  Assert(cubHdr != 0);
...
template <typename MSG_HEADER_TYPE> CMsgBase_t<MSG_HEADER_TYPE>::~CMsgBase_t() {
...
//-----------------------------------------------------------------------------
// Purpose: Appends a bool to the variable length data associated with this
// message Input:	bData -	data to append
//-----------------------------------------------------------------------------
template <typename MSG_HEADER_TYPE>
...
//-----------------------------------------------------------------------------
// Purpose: Appends a uint8 to the variable length data associated with this
// message Input:	ubData -	data to append
//-----------------------------------------------------------------------------
template <typename MSG_HEADER_TYPE>
...
//-----------------------------------------------------------------------------
// Purpose: Appends a uint to the variable length data associated with this
// message Input:	unData -	data to append
//-----------------------------------------------------------------------------
template <typename MSG_HEADER_TYPE>
...
//-----------------------------------------------------------------------------
// Purpose: Appends an int to the variable length data associated with this
// message Input:	nData -	data to append
//-----------------------------------------------------------------------------
template <typename MSG_HEADER_TYPE>
...
//-----------------------------------------------------------------------------
// Purpose: Appends an int16 to the variable length data associated with this
// message Input:	nData -	data to append
//-----------------------------------------------------------------------------
template <typename MSG_HEADER_TYPE>
...
//-----------------------------------------------------------------------------
// Purpose: Appends a uint64 to the variable length data associated with this
// message Input:	ulData -	data to append
//-----------------------------------------------------------------------------
template <typename MSG_HEADER_TYPE>
...
//-----------------------------------------------------------------------------
// Purpose: Appends an int64 to the variable length data associated with this
// message Input:	lData -	data to append
//-----------------------------------------------------------------------------
template <typename MSG_HEADER_TYPE>
...
//-----------------------------------------------------------------------------
template <typename MSG_HEADER_TYPE>
void CMsgBase_t<MSG_HEADER_TYPE>::AddVariableLenData(const void *pvData,
                                                     uint cubLen) {
  if (cubLen > 0) {
...
//-----------------------------------------------------------------------------
// Purpose: Appends a float to the variable length data associated with this
// message Input:	nData -	data to append
//-----------------------------------------------------------------------------
template <typename MSG_HEADER_TYPE>
...
//-----------------------------------------------------------------------------
template <typename MSG_HEADER_TYPE>
bool CMsgBase_t<MSG_HEADER_TYPE>::BReadVariableLenData(void *pvBuff,
                                                       uint32 cubRead) {
...
bool CMsgBase_t<MSG_HEADER_TYPE>::BReadStr(char *pchBuff, int cchBuff) {
  ...
  if ((cchRead > cchBuff) || (cchRead > cchLeft)) {
    // at least return an empty string since most code doesn't check the return
    // value
    if (cchBuff > 0)
      pchBuff[0] = 0;
...
void CMsgBase_t<MSG_HEADER_TYPE>::PacketDump() {
  ...
  EmitInfo(
      SPEW_NETWORK, SPEW_NEVER, LOG_ALWAYS,
...
void CMsgBase_t<MSG_HEADER_TYPE>::ReportBufferOverflow() {
  EmitWarning(SPEW_NETWORK, SPEW_ALWAYS,
