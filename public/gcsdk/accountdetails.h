...
public:
  ...
  bool BIsFreeTrialAccountOrDemo() const {
    return m_bFreeTrialAccount || m_unPackage == 0;
  }
...
private:
  ...
  uint64 m_unPhoneID;
  bool m_bValid : 1, m_bPublicProfile : 1, m_bPublicInventory : 1,
      m_bVacBanned : 1, m_bCyberCafe : 1, m_bSchoolAccount : 1,
      m_bFreeTrialAccount : 1, m_bSubscribed : 1, m_bLowViolence : 1,
      m_bLimited : 1, m_bAccountLocked : 1, m_bCommunityBanned : 1,
      m_bTradeBanned : 1, m_bIsSteamGuardEnabled : 1, m_bIsPhoneVerified : 1,
      m_bIsTwoFactorAuthEnabled : 1, m_bIsPhoneIdentifying : 1;
};
...
private:
  ...
  bool BFindAccountDetailsInLocalCache(const CSteamID &steamID,
                                       CAccountDetails **ppAccount);
  void WakeWaitingAccountDetailsJobs(const CSteamID &steamID);

  CTHash<CAccountDetails, uint32> m_hashAccountDetailsCache;
  CUtlHashMapLarge<CSteamID, CCopyableUtlVector<JobID_t>>
      m_mapQueuedAccountDetailsRequests;
  ...
  CUtlHashMapLarge<CSteamID, CCopyableUtlVector<JobID_t>>
      m_mapQueuedPersonaNameRequests;
};
