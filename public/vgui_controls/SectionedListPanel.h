...
public:
  ...
  enum ColumnFlags_e {
    HEADER_IMAGE =
        0x01, // set if the header for the column is an image instead of text
    COLUMN_IMAGE =
        0x02, // set if the column contains an image instead of text (images are
              // looked up by index from the ImageList) (see SetImageList below)
  ...
  // removes an item from the list; returns false if the item does not exist or
  // is already removed
  ...
  */
  ...
  virtual bool GetCellBounds(int itemID, int column, int &x, int &y, int &wide,
                             int &tall);

  // Gets the coordinates of a section header
  virtual bool GetSectionHeaderBounds(int sectionID, int &x, int &y, int &wide,
                                      int &tall);
  ...
  void SetColumnWidthBySection(int sectionID, const char *columnName,
                               int iWidth);
...
public:
  ...
  void SetColorOverrideForCell(int sectionID, int itemID, int columnID,
                               Color clrOverride);
...
private:
  ...
  // Returns the index of a new item button, reusing an existing item button if
  // possible
