...
template <class T> class CvarToggleCheckButton : public CheckButton {
...
// Purpose: Destructor
//-----------------------------------------------------------------------------
template <class T> CvarToggleCheckButton<T>::~CvarToggleCheckButton() {}

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
template <class T> void CvarToggleCheckButton<T>::Paint() {
  if (!m_cvar.IsValid()) {
    BaseClass::Paint();
    return;
  }
...
//-----------------------------------------------------------------------------
template <class T> void CvarToggleCheckButton<T>::OnApplyChanges() {
  ApplyChanges();
}
...
// Purpose:
//-----------------------------------------------------------------------------
template <class T> void CvarToggleCheckButton<T>::ApplyChanges() {
  if (!m_cvar.IsValid())
    return;
...
// Purpose:
//-----------------------------------------------------------------------------
template <class T> void CvarToggleCheckButton<T>::Reset() {
  if (!m_cvar.IsValid())
    return;
...
// Purpose:
//-----------------------------------------------------------------------------
template <class T> bool CvarToggleCheckButton<T>::HasBeenModified() {
...
// Input  : *panel -
//-----------------------------------------------------------------------------
template <class T> void CvarToggleCheckButton<T>::SetSelected(bool state) {
...
// Purpose:
//-----------------------------------------------------------------------------
template <class T> void CvarToggleCheckButton<T>::OnButtonChecked() {
  if (HasBeenModified()) {
