...
class Panel : public IClientPanel,
              virtual public IForceVirtualInheritancePanel {
...
public:
  ...
  const char *GetClassName(); // returns the class name of the panel (eg. Panel,
                              // Label, Button, etc.)
  ...
  void SetAlpha(
      int alpha); // sets alpha modifier for panel and all child panels [0..255]
  ...
  IsWithinTraverse(int x, int y,
                   bool traversePopups); // recursive; returns a pointer to the
  ...
  void
  MakePopup(bool showTaskbarIcon = true,
            bool disabled = false); // turns the panel into a popup window (ie.
                                    // can draw outside of it's parents space)
  virtual void OnMove();
  ...
  virtual void SetAutoDelete(bool state); // if set to true, panel automatically
                                          // frees itself when parent is deleted
  ...
  virtual bool RequestInfo(
      KeyValues *outputData); // returns true if output is successfully written.
                              // You should always chain back to the base class
                              // if info request is not handled
  virtual bool SetInfo(KeyValues *inputData); // sets a specified value in the
                                              // control - inverse of the above
  virtual void
  SetSilentMode(bool bSilent); // change the panel's silent mode; if silent, the
                               // panel will not post any action signals

  // install a mouse handler
  virtual void
  InstallMouseHandler(Panel *pHandler); // mouse events will be send to handler
                                        // panel instead of this panel
  ...
  virtual Panel *
  HasHotkey(wchar_t key); // returns the panel that has this hotkey
  ...
  MESSAGE_FUNC(OnDelete,
               "Delete"); // called to delete the panel; Panel::OnDelete() does
                          // simply { delete this; }
  virtual void
  OnThink(); // called every frame before painting, but only if panel is visible
  ...
  KeyCodeModifiersToDisplayString(KeyCode code,
                                  int modifiers); // L"Ctrl+Alt+Shift+Backspace"

  static KeyCode StringToKeyCode(char const *str);
#if defined(VGUI_USEKEYBINDINGMAPS)
  static KeyBindingContextHandle_t
  CreateKeyBindingsContext(char const *filename, char const *pathID = 0);
  ...
  // if set, Panel gets PerformLayout called after the camera and the renderer's
  // m_matrixWorldToScreen has been setup, so panels can be correctly attached
...
protected:
  ...
  MESSAGE_FUNC_HANDLE_HANDLE(OnRequestFocus, "OnRequestFocus", subFocus,
                             defaultPanel);
...
private:
  ...
  virtual void
  InternalFocusChanged(bool lost); // called when the focus gets changed
  ...
  char *_panelName; // string name of the panel - only unique within the current
                    // context
  IBorder *_border;
  ...
  short m_nPinDeltaY;
  short
      m_nResizeDeltaX; // Relative position of the non-pinned corner to the edge
  short m_nResizeDeltaY;

  HCursor _cursor;
  unsigned int _buildModeFlags; // flags that control how the build mode dialog
                                // handles this panel
  ...
  byte _pinCornerToSibling; // the corner of our panel that we're pinning to our
                            // sibling
...
private:
  char *_tooltipText; // Tool tip text for panels that share tooltip panels with
                      // other panels

  PHandle m_hMouseEventHandler;

  bool m_bWorldPositionCurrentFrame; // if set, Panel gets PerformLayout called
                                     // after the camera and the renderer's
                                     // m_matrixWorldToScreen has been setup, so
                                     // panels can be correctly attached to
