...
public:
  ...
  */
  ...
  void
  SetDrawWidth(int width); // width from right side of window we have to draw in
  int GetDrawWidth();
  void
  SetHorizontalScrolling(bool status); // turn horizontal scrolling on or off.
...
protected:
  ...
  virtual void
  SetCharAt(wchar_t ch,
            int index); // set the value of a char in the text buffer
  ...
  virtual bool
  SelectCheck(bool fromMouse = false); // check if we are in text selection mode
  ...
  virtual void
  OnCursorMoved(int x,
                int y); // respond to moving the cursor with mouse button down
