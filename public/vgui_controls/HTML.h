...
//			This control utilises a hidden IE window to render a
//HTML page for you. 			It can load any valid URL (i.e local files or web pages),
//you cannot dynamically change the 			content however (internally to the control
//that is).
//-----------------------------------------------------------------------------
...
public:
  ...
  /* message posting:
     ...
     (disabled,good,bad) "url" "secure" - true if an ssl page "certerror" - true
     if there is a cert error loading the page "isevcert" - true if this is an
     EV style cert "certname" - name of the entity this cert is issued to
  */
...
private:
  ...
  struct ScrollData_t {
    ...
    bool operator!=(ScrollData_t const &src) const { return !operator==(src); }
