...
public:
  ListPanelItem()
      : kv(0), userData(0), m_pDragData(0), m_bImage(false), m_nImageIndex(-1),
        m_nImageIndexSelected(-1), m_pIcon(0) {}
...
public:
  ...
  // table
  virtual int
  AddItem(const KeyValues *data, uintp userData, bool bScrollToItem,
          bool bSortOnAdd); // Takes a copy of the data for use in the table.
                            // Returns the index the item is at.
  void SetItemDragData(
      int itemID,
      const KeyValues *data); // Makes a copy of the keyvalues to store in the
                              // table. Used when dragging from the table. Only
                              // used if the caller enables drag support
  virtual int GetItemCount(void); // returns the number of VISIBLE items
  virtual int GetItem(const char *itemName); // gets the row index of an item by
                                             // name (data->GetName())
  virtual KeyValues *
  GetItem(int itemID); // returns pointer to data the row holds
  ...
  virtual void
  ApplyItemChanges(int itemID); // applies any changes to the data, performed by
                                // modifying the return of GetItem() above
  virtual void
  RemoveItem(int itemID);        // removes an item from the table (changing the
                                 // indices of all following items)
  ...
  virtual void GetCellText(
      int itemID, int column, OUT_Z_BYTECAP(bufferSizeInBytes) wchar_t *buffer,
      int bufferSizeInBytes); // returns the data held by a specific cell
  virtual IImage *GetCellImage(int itemID,
                               int column); //, ImagePanel *&buffer); // returns
                                            //the image held by a specific cell
  ...
  virtual bool
  GetCellAtPos(int x, int y, int &row,
               int &column); // returns true if any found, row and column are
                             // filled out. x, y are in screen space
