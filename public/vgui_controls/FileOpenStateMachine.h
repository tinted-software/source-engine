...
public:
  enum CompletionState_t {
    ...
    FILE_SAVE_CANCELLED, // The user chose 'cancel' in the dialog asking if he
                         // wanted to save
    FILE_SAVE_NAME_NOT_SPECIFIED, // User hit cancel in the SaveAs dialog
    FILE_NOT_OVERWRITTEN, // Operation aborted; existed file and user chose to
                          // not write over it
  ...
  void OpenFile(const char *pOpenFileName, const char *pOpenFileType,
                KeyValues *pContextKeyValues, const char *pSaveFileName = NULL,
                const char *pSaveFileType = NULL, int nFlags = 0);
  ...
  void SaveFile(KeyValues *pContextKeyValues, const char *pFileName,
                const char *pFileType, int nFlags = FOSM_SHOW_PERFORCE_DIALOGS);

  // Returns the state machine completion state
  CompletionState_t GetCompletionState();

  /* MESSAGES SENT
          "FileStateMachineFinished" - Called when we exit the state machine for
     any reason "completionState" - See the CompletionState_t enum above
     ...
     the file read for OpenFile or written for SaveFile Use GetFirstTrueSubKey()
     to get the context passed into the OpenFile/SaveFile methods
  */
