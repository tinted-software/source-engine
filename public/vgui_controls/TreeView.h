...
public:
  ...
  */
  ...
  void SetMultipleItemDragEnabled(
      bool state); // if this is set, then clicking on one row and dragging will
                   // select a run or items, etc.
...
protected:
  ...
  // called when a node, marked as "Expand", needs to generate it's child nodes
  // when expanded
