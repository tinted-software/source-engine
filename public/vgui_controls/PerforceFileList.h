...
public:
  ...
  // and it may exist in perforce or not. It's specified as a full path on disk
  // though. In the case where a file doesn't exist on disk, but it does exist
  // in perforce specify where that file would appear on disk. This function
  // returns the itemID of the added file If you already know the file exists or
  // is a directory (or not), specify that in the call. -1 means autodetect
