...
const int MAX_SERVER_CONNECTIONS =
    4; // Maximum number of concurrent connections allowed to a server.
       // In the future, this can be set higher and class CSocketConnection
       // should use a growable array of Sockets.

const int MAX_SERVER_CONNECTION_BACKLOG =
    16; // Maximum number of connections allowed in the listening backlog.
...
enum SocketState_t // State of a given socket.
{
  ...
  SSTATE_CONNECTION_IN_PROGRESS, // Socket is initialized and in the process of
...
static const SocketHandle_t InvalidSocketHandle =
    static_cast<SocketHandle_t>(INVALID_SOCKET);

// ----------------------------------------------------------------------------
// CSocketConnection
// Represents a client or server network connection.
//
// todo: Rework connection class to treat endpoints as standalone objects and
// unify interface with named pipes.
// ----------------------------------------------------------------------------
...
public:
  ...
  SocketErrorCode_t
  Init(ConnectionType_t connectionType,
       SocketProtocol_t
           socketProtocol); // Initializes the connection class as either client
                            // or server with specified protocol. Does not
                            // acquire any system resources.

  void Cleanup(); // Cleans up the class and restores it to the default
                  // uninitialized state.

  SocketErrorCode_t
  Listen(uint16 localPort,
         int numAllowedConnections); // (Server-only) Listens for active client
                                     // connections.

  SocketErrorCode_t TryAcceptIncomingConnection(
      int *newEndpointIndex); // (Server-only) Attempts to accept an incoming
                              // connection, if one is available.

  int GetFirstAvailableListeningEndpoint(); // (Server-only) Gets the index of
                                            // the first endpoint which is
                                            // listening for connections.

  SocketState_t GetListeningSocketState(); // (Server-only) Gets the status of
                                           // the listening socket on a server.

  SocketErrorCode_t ConnectToServer(
      const char *hostName,
      uint16 hostPort); // (Client-only) Connects to a remote host using
                        // endpoint 0. In most cases, the status of endpoint 0
                        // will be SSTATE_CONNECTION_IN_PROGRESS. The caller
                        // must periodically call PollClientConnectionState
                        // until the socket is connected before it is usable.

  SocketErrorCode_t PollClientConnectionState(
      bool *isConnected); // (Client-only) Checks the status of endpoint 0 to
                          // see if it is connected successfully to a server.

  SocketState_t GetEndpointSocketState(
      int endpointIndex); // Gets the status of the specified endpoint. (Clients
                          // only use endpoint 0.)

  SocketErrorCode_t
  CanReadFromEndpoint(int endpointIndex,
                      bool *canRead); // Gets a value indicating whether the
                                      // endpoint can be successfully read from.

  SocketErrorCode_t CanWriteToEndpoint(
      int endpointIndex,
      bool *canWrite); // Gets a value indicating whether the endpoint can be
                       // successfully written to.

  SocketErrorCode_t
  ReadFromEndpoint(int endpointIndex, byte *destinationBuffer, int bufferSize,
                   int *bytesRead); // Reads data from the specified endpoint.

  SocketErrorCode_t
  WriteToEndpoint(int endpointIndex, byte *sourceBuffer, int bufferSize,
                  int *bytesWritten); // Writes data to the specified endpoint.

  const char *GetLastSystemErrorString()
      const; // Gets a string containing the last underlying system error
             // reported. Only valid immediately after a function returns an
             // unsuccessful error code.
  const char *GetLastErrorString() const; // Gets a string containing the last
                                          // underlying system error reported.
  ...
  SocketErrorCode_t SetSocketOpt(int endpointIndex, int level, int optname,
...
//
//-----------------------------------------------------------------------------
inline SocketHandle_t
CSocketConnection::GetEndpointSocket(int nEndpointIndex) const {
  AssertMsg(nEndpointIndex >= 0 && nEndpointIndex < MAX_SERVER_CONNECTIONS,
...
// Byte swaps (in-place) a message header between system byte-order and network
// byte-order.
void ByteSwapInPlaceMessageHeader(MessageHeader_t *messageHeader);

// ----------------------------------------------------------------------------
// Signature of a callback invoked once per message parsed by the
// CSocketMessageBuilder::FeedData function.
...
// ----------------------------------------------------------------------------
typedef void (*NetworkMessageHandler)(const MessageHeader_t &header,
                                      const byte *message, void *userContext);

// ----------------------------------------------------------------------------
// Helper class to parse messages from a stream source.
// This class maintains state between successive calls to FeedData so that
// partial messages can be stored until fully parsed and dispatched.
// ----------------------------------------------------------------------------
...
public:
  ...
  // Parses the given stream data and fires a callback for each full message
  // parsed. This function has the side-effect of updating the
  // CSocketMessageBuilder's internal parsing state so that partial messages can
  // be glued together.
  void FeedData(const void *data, int dataLength,
                NetworkMessageHandler networkMessageHandlerFunc,
                void *userContext);
  ...
  SocketErrorCode_t SendDataPacket(CSocketConnection *pConnection, int endpoint,
                                   const void *RESTRICT data, int dataLength);

  SocketErrorCode_t BeginSendPartialDataPacket(uint32 totalSize,
                                               const void *RESTRICT data,
                                               int dataLength);
  SocketErrorCode_t BeginSendPartialDataPacket(CSocketConnection *pConnection,
                                               int endpoint, uint32 totalSize,
                                               const void *RESTRICT data,
                                               int dataLength);
  ...
  int32 PartialMessageBytesRemaining() {
    return m_bSendingPartialMessage
               ? m_PartialMessageBytesTotal - m_PartialMessageBytesSent
               : 0;
  }
...
private:
  ...
  uint32 m_PartialMessageBytesSent; // used to combine multiple calls to
                                    // SendDataPacket/BeginSendPartialDataPacket
