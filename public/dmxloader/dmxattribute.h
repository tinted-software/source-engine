...
template <class T> inline const T &CDmxAttribute::GetValue() const {
...
template <class T> inline const CUtlVector<T> &CDmxAttribute::GetArray() const {
...
template <class T> inline CUtlVector<T> &CDmxAttribute::GetArrayForEdit() {
  if (CDmAttributeInfo<CUtlVector<T>>::AttributeType() == m_Type)
    return *(CUtlVector<T> *)m_pData;
