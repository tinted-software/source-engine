...
public:
  ...
  template <typename T>
  void AddAttributesFromStructure(const T *pData,
...
inline const char *
CDmxElement::GetValueString(const char *pAttributeName) const {
...
template <class T>
inline const CUtlVector<T> &
CDmxElement::GetArray(const char *pAttributeName) const {
  const CDmxAttribute *pAttribute = GetAttribute(pAttributeName);
  if (pAttribute)
...
template <class T>
inline const CUtlVector<T> &
CDmxElement::GetArray(const char *pAttributeName,
                      const CUtlVector<T> &defaultValue) const {
  const CDmxAttribute *pAttribute = GetAttribute(pAttributeName);
  if (pAttribute)
    return pAttribute->GetArray<T>();
