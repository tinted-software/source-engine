...
public:
  ...
  void SetChannels(int _channels) { info.m_channels = _channels; }
  ...
  inline const byte *CachedData() const { return m_pCachedData; }
  ...
  inline const byte *HeaderData() const { return m_pHeader; }
...
public:
  ...
  virtual CAudioSourceCachedInfo *
  GetInfo(int audiosourcetype, bool soundisprecached, CSfxTable *sfx) = 0;
...
enum {
  ...
  STREAMED_SINGLEPLAY = 0x00000002, // non recurring data, buffers don't need to
                                    // persist and can be recycled
  STREAMED_QUEUEDLOAD =
