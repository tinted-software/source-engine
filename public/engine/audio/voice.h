...
#define VOICE_OUTPUT_SAMPLE_RATE_LOW                                           \
  11025 // Sample rate that we feed to the mixer.
#define VOICE_OUTPUT_SAMPLE_RATE_HIGH                                          \
  22050 // Sample rate that we feed to the mixer.
#define VOICE_OUTPUT_SAMPLE_RATE_MAX                                           \
  22050 // Sample rate that we feed to the mixer.
...
//! Inits voice with defaults if it is not initialized normally, e.g. for local
//! mixer use.
void Voice_ForceInit();

//! Get the default sample rate to use for this codec
inline int
Voice_GetDefaultSampleRate(const char *pCodec) // Inline for DEDICATED builds
{
  ...
  if (Q_stricmp(pCodec, "vaudio_speex") == 0) {
    ...
    return 0; // For the steam codec, 0 passed to voice_init means "use optimal
              // steam voice rate"
  }
...
//! The codec voice was initialized with. Empty string if voice is not
//! initialized.
...
bool Voice_RecordStart(
    ...
    //! If this is non-null, the voice manager will use this file for input
    //! instead of the mic.
