...
public:
  ...
  // Called to mix in Bink audio. Returns true if audio was mixed in, false
  // otherwise.
  virtual bool SDLMixerAudioCallback(void *stream, int len) = 0;
#endif

  // Plays a given Bink file until it completes or the user presses ESC, SPACE,
  // or ENTER
  virtual void PlayBinkVideo(const char *filename, void *mainWindow, int width,
                             int height, float forcedMinTime) = 0;
};
