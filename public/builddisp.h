...
protected:
  Vector m_BBox[2]; // displacement node bounding box (take into account size of
                    // children)
  float m_ErrorTerm; // LOD error term (the "precision" of the representation of
                     // the surface at this node's level)
...
public:
  ...
  //
  friend void
  SmoothNeighboringDispSurfNormals(CCoreDispInfo **ppCoreDispInfoList,
                                   int listSize);
...
private:
  ...
  CBitVec<MAX_DISPVERTS>
      m_AllowedVerts; // Built in VBSP. Defines which verts are allowed to exist
                      // based on what the neighbors are.
