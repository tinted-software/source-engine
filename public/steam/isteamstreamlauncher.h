...
enum EStreamLauncherResult {
  ...
  k_EStreamLaunchResultAlreadyStreaming =
      2, // The UI is already streaming to a different application
  k_EStreamLaunchResultInvalidLauncher =
      3, // The streaming launcher doesn't exist, or failed signature check
  k_EStreamLaunchResultNotReady =
      4, // The UI isn't ready to go into streaming mode (Desktop UI login?)
};
...
public:
  // Switch Steam to Big Picture mode and optionally set a launcher to run all
  // games
  virtual EStreamLauncherResult
  StartStreaming(const char *pchLauncher = NULL) = 0;
