...
public:
  ...
  // servers. If false, it means there is no active connection due to either a
  // networking issue on the local machine, or the Steam server is down/busy.
  // The Steam client will automatically be trying to recreate the connection as
  // often as possible.
  ...
  //
  // Parameters:
  // void *pAuthBlob - a pointer to empty memory that will be filled in with the
  // authentication token. int cbMaxAuthBlob - the number of bytes of allocated
  // memory in pBlob. Should be at least 2048 bytes. CSteamID steamIDGameServer
  // - the steamID of the game server, received from the game server by the
  // client CGameID gameID - the ID of the current game. For games without mods,
  // this is just CGameID( <appID> ) uint32 unIPServer, uint16 usPortServer -
  // the IP address of the game server bool bSecure - whether or not the client
  // thinks that the game server is reporting itself as secure (i.e. VAC is
  // running)
  //
  // return value - returns the number of bytes written to pBlob. If the return
  // is 0, then the buffer passed in was too small, and the call has failed The
  // contents of pBlob should then be sent to the game server, for it to use to
  ...
  // needs to occur when the game client leaves the specified game server, needs
  // to match with the InitiateGameConnection() call
  ...
  // get the local storage folder for current Steam account to write application
  // data, e.g. save games, configs etc. this will usually be something like
  // "C:\Progam Files\Steam\userdata\<SteamID>\<AppID>\local"
  ...
  // early, the system will keep recording for a little bit after this function
  // is called. GetVoice() should continue to be called until it returns
  ...
  // uncompressed (the usual case) If you're upgrading from an older Steamworks
  // API, you'll want to pass in 11025 to nUncompressedVoiceDesiredSampleRate
  virtual EVoiceResult
  GetAvailableVoice(uint32 *pcbCompressed, uint32 *pcbUncompressed,
  ...
  // integer, 11025Hz PCM format. Please note that the uncompressed data is not
  // the raw feed from the microphone: data may only be available if audible
  // levels of speech are detected, and may have passed through denoising
  // filters, etc. This function should be called as often as possible once
  // recording has started; once per frame at least. nBytesWritten is set to the
  // number of bytes written to pDestBuffer. nUncompressedBytesWritten is set to
  // the number of bytes written to pUncompressedDestBuffer. You must grab both
  // compressed and uncompressed here at the same time, if you want both.
  // Matching data that is not read during this call will be thrown away.
  // GetAvailableVoice() can be used to determine how much data is actually
  // available. If you're upgrading from an older Steamworks API, you'll want to
  // pass in 11025 to nUncompressedVoiceDesiredSampleRate
  ...
  // kilobytes. The output format of the data is 16-bit signed at the requested
  // samples per second. If you're upgrading from an older Steamworks API,
  // you'll want to pass in 11025 to nDesiredSampleRate
  ...
  // Cancel auth ticket from GetAuthSessionTicket, called when no longer playing
  // game with the entity you gave the ticket to
  ...
  // CSteamID steamIDGameServer - the steamID of the game server, received from
  // the game server by the client uint32 unIPServer, uint16 usPortServer - the
  ...
  // ( This is asynchronous, you must wait for the ticket to be completed by the
  // server )
  ...
  // NOTE: The URL has a very short lifetime to prevent history-snooping
  // attacks, so you should only call this API when you are about to launch the
  // browser, or else immediately navigate to the result URL using a hidden
  // browser window. NOTE 2: The resulting authorization cookie has an
  // expiration time of one day, so it would be a good idea to request and visit
  // a new auth URL every 12 hours.
  ...
  // user for credentials to establish a link. Future calls to LogOn() after the
  // one time link call should succeed as long as the user is connected to PSN.
  ...
  // the PSN ticket to generate a unique account name.  The new account is then
  // linked to the PSN ticket.  This is the faster option for new users who
  // don't have an existing Steam account to get into multiplayer.
  //
  // PARAMS: bInteractive - If set tells Steam to go ahead and show the PS3
  // NetStart dialog if needed to prompt the user for network setup/PSN logon
  // before initiating the Steam side of the logon.
...
//			this means the Steam client now has a working connection
//to the Steam servers 			usually this will have occurred before the game has
...
//			This can usually occur if the user doesn't have rights
//to play on the game server.
//-----------------------------------------------------------------------------
...
//			When getting this message the client should disconnect
//from Steam, reset any stored Steam state and reconnect. 			This usually occurs in
