...
public:
  ...
  // used by game servers, create a steam user that won't be shared with anyone
  // else
...
#define END_CALLBACK_INTERNAL_BEGIN(numvars)                                   \
  ...
  static bool GetMemberVariable(uint32 index, uint32 &varOffset,               \
                                uint32 &varSize, uint32 &varCount,             \
                                const char **pszName, const char **pszType) {  \
    switch (index) {                                                           \
    default:                                                                   \
      return false;
