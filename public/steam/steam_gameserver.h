...
enum EServerMode {
  ...
  eServerModeAuthentication = 2, // Authenticate users, list on the server list,
                                 // don't run VAC on clients that connect
...
inline bool CSteamGameServerAPIContext::Init() {
  ...
  m_pSteamGameServerNetworking = g_pSteamClientGameServer->GetISteamNetworking(
      hSteamUser, hSteamPipe, STEAMNETWORKING_INTERFACE_VERSION);
  if (!m_pSteamGameServerNetworking)
    return false;

  m_pSteamGameServerStats = g_pSteamClientGameServer->GetISteamGameServerStats(
  ...
  m_pSteamInventory = g_pSteamClientGameServer->GetISteamInventory(
      hSteamUser, hSteamPipe, STEAMINVENTORY_INTERFACE_VERSION);
