...
enum EResult {
  ...
  k_EResultDuplicateRequest =
      29, // The request is a duplicate and the action has already occurred in
          // the past, ignored this time
  ...
  k_EResultTryAnotherCM = 48, // The current CM can't service the user making a
                              // request, user should try another.
  k_EResultPasswordRequiredToKickSession =
      49, // You are already logged in elsewhere, this cached credential login
          // has failed.
  ...
  k_EResultExternalAccountAlreadyLinked =
      59, // External account (PSN, Facebook...) is already linked to some other
  ...
  k_EResultAccountLogonDeniedNoMail =
      66, // account login denied due to 2nd factor auth failure - and no mail
          // has been sent
  k_EResultHardwareNotCapableOfIPT = 67, //
  k_EResultIPTInitError = 68,            //
  k_EResultParentalControlRestricted =
      69, // operation failed due to parental control restrictions for current
          // user
...
enum EAuthSessionResponse {
  ...
  k_EAuthSessionResponsePublisherIssuedBan =
      9, // The user is banned for this game. The ban came via the web api and
         // not VAC
};
...
enum EAccountType {
  ...
  k_EAccountTypeConsoleUser = 9, // Fake SteamID for local PSN account on PS3 or
                                 // Live account on 360, etc.
...
enum EAppReleaseState {
  ...
  k_EAppReleaseState_Prerelease =
      2, // can be purchased and is visible in games list, nothing else. Common
         // appInfo section released
...
enum EAppType {
  ...
  k_EAppType_Media_DEPRECATED = 0x010, // legacy - was used for game trailers,
                                       // which are now just videos on the web
  ...
  k_EAppType_Config =
      0x100, // hidden app used to config Steam features (backpack, sales, etc)
  ...
  k_EAppType_DepotOnly =
      0x80000000, // placeholder since depots and apps share the same namespace
};
...
enum EChatRoomEnterResponse {
  ...
  k_EChatRoomEnterResponseNotAllowed = 3, // General Denied - You don't have the
                                          // permissions needed to join the chat
  ...
  k_EChatRoomEnterResponseLimited =
      7, // Joining this chat is not allowed because you are a limited user (no
         // value on account)
  ...
  k_EChatRoomEnterResponseCommunityBan =
      9, // Attempt to join a chat when the user has a community lock on their
         // account
...
enum ELaunchOptionType {
  ...
  k_ELaunchOptionType_Default =
      1, // runs the game, app, whatever in default mode
  ...
  k_ELaunchOptionType_Option1 =
      10, // generic run option, uses description field for game name
  k_ELaunchOptionType_Option2 =
      11, // generic run option, uses description field for game name
  k_ELaunchOptionType_Option3 =
      12, // generic run option, uses description field for game name
...
public:
  ...
  // probable mistake.
  //			See the stubbed out overloads in the private: section
  //for more info.
  //-----------------------------------------------------------------------------
  ...
  void SetFromSteam2(TSteamGlobalUserID *pTSteamGlobalUserID,
    ...
    m_steamid.m_comp.m_EAccountType =
        k_EAccountTypeIndividual; // Steam 2 accounts always map to account type
                                  // of individual
    m_steamid.m_comp.m_unAccountInstance =
  ...
  void ConvertToSteam2(TSteamGlobalUserID *pTSteamGlobalUserID) const {
    ...
    // mapped Assert( m_steamid.m_comp.m_EAccountType ==
  ...
  // representation.
  //			For multiseat accounts, all instances of that account
  //will have the 			same static account key, so they can be grouped together by
  //the static 			account key.
  ...
  //-----------------------------------------------------------------------------
  // Purpose: Is this an anonymous user account? ( used to create an account or
  // reset a password )
  //-----------------------------------------------------------------------------
  ...
  static const char *
  Render(uint64 ulSteamID); // static method to render a uint64 representation
...
private:
  ...
  union SteamID_t {
    struct SteamIDComponent_t {
      ...
      unsigned int
          m_EAccountType : 4; // type of account - can't show as EAccountType,
...
public:
  ...
  static const char *
  Render(uint64 ulGameID); // static method to render a uint64 representation of
