...
enum EFriendRelationship {
  ...
  k_EFriendRelationshipIgnored =
      5, // this is stored; the user has explicit blocked this other user from
         // comments/chat/etc
...
//			that can be a friend, in a group, on a game server or in
//a lobby with the local user
//-----------------------------------------------------------------------------
class ISteamFriends {
public:
  ...
  // that this pointer is not saved off; it will eventually be free'd or
  // re-allocated
  virtual const char *GetPersonaName() = 0;

  // Sets the player name, stores it on the server and publishes the changes to
  // all friends who are online. Changes take place locally immediately, and a
  // PersonaStateChange_t is posted, presuming success.
  //
  ...
  // PersonaStateChange_t will be posted to change the name back, in addition to
  // the SetPersonaNameResponse_t callback.
  ...
  // knows about who meet that criteria then GetFriendByIndex() can then be used
  // to return the id's of each of those users
  ...
  // room or game server the local user will not known the name of the other
  ...
  // accesses old friends names - returns an empty string when their are no more
  ...
  // Returns nickname the current user has set for the specified player. Returns
  // NULL if the no nickname has been set for that player.
  ...
  // gets up to nMembersCount members of the given friends group, if fewer exist
  // than requested those positions' SteamIDs will be invalid
  ...
  // latest
  virtual SteamAPICall_t DownloadClanActivityCounts(ARRAY_COUNT(cClansToRequest)
  ...
  // other users in that lobby steamIDSource can be the steamID of a group, game
  // server, lobby or chat room
  ...
  // valid options are
  //		"steamid" - opens the overlay web browser to the specified user
  //or groups profile 		"chat" - opens a chat window to the specified user, or
  //joins the group chat 		"jointrade" - opens a window to a Steam Trading session
  //that was started with the ISteamEconomy/StartTrade Web API 		"stats" - opens
  //the overlay web browser to the specified user's stats 		"achievements" - opens
  //the overlay web browser to the specified user's achievements 		"friendadd" -
  //opens the overlay in minimal mode prompting the user to add the target user
  //as a friend 		"friendremove" - opens the overlay in minimal mode prompting the
  //user to remove the target friend 		"friendrequestaccept" - opens the overlay
  //in minimal mode prompting the user to accept an incoming friend invite
  //		"friendrequestignore" - opens the overlay in minimal mode
  //prompting the user to ignore an incoming friend invite
  ...
  // activates game overlay to open the invite dialog. Invitations will be sent
  // for the provided lobby.
  virtual void ActivateGameOverlayInviteDialog(CSteamID steamIDLobby) = 0;

  // gets the small (32x32) avatar of the current user, which is a handle to be
  // used in IClientUtils::GetImageRGBA(), or 0 if none set
  virtual int GetSmallFriendAvatar(CSteamID steamIDFriend) = 0;

  // gets the medium (64x64) avatar of the current user, which is a handle to be
  // used in IClientUtils::GetImageRGBA(), or 0 if none set
  ...
  // posted when it's retrieved if returns false, it means that we already have
  // all the details about that user, and functions can be called immediately
  ...
  // and no avatar image is available, call RequestUserInformation( steamID,
  // false ) to download the avatar
  ...
  virtual CSteamID GetClanOfficerByIndex(CSteamID steamIDClan,
                                         int iOfficer) = 0;
  // if current user is chat restricted, he can't send or receive any text/voice
  // chat messages. the user can't see custom avatars. But the user can be
  // online and send/recv game invites. a chat restricted user can't add friends
  // or join any groups.
  ...
  //		"status"  - a UTF-8 string that will show up in the 'view game
  //info' dialog in the Steam friends list 		"connect" - a UTF-8 string that
  ...
  // the behavior is somewhat sophisticated, because the user may or may not be
  // already in the group chat from outside the game or in the overlay use
...
//			the game can use this to be pause or resume single
//player games
//-----------------------------------------------------------------------------
...
// their friends list
//			game client should attempt to connect to specified
//server when this is received
//-----------------------------------------------------------------------------
...
struct GameRichPresenceJoinRequested_t {
  ...
  CSteamID m_steamIDFriend; // the friend they did the join via (will be invalid
                            // if not directly via a friend)
...
struct SetPersonaNameResponse_t {
  ...
  bool m_bLocalSuccess; // true if name change was retained locally.  (We might
                        // not have been able to communicate with Steam)
