...
public:
  ...
  // complete the call handle will return a HTML_BrowserReady_t callback for the
  // HHTMLBrowser of your new browser.
  ...
  // use 1.2 ), zooming around point X,Y in the page (use 0,0 if you don't care)
  ...
  // HTML5 video and audio objects will execute ".pause()" and gain the property
  // "._steam_background_paused = 1". When background mode is disabled, any
  // video or audio objects with that property will resume with ".play()".
...
CALLBACK_MEMBER(1, const char *,
                pBGRA) // a pointer to the B8G8R8A8 data for this surface, valid
                       // until SteamAPI_RunCallbacks is next called
...
CALLBACK_MEMBER(
    4, uint32, unUpdateX) // the offset in X for the damage rect for this update
CALLBACK_MEMBER(
    5, uint32, unUpdateY) // the offset in Y for the damage rect for this update
...
CALLBACK_MEMBER(8, uint32, unScrollX) // the page scroll the browser was at when
                                      // this texture was rendered
CALLBACK_MEMBER(9, uint32, unScrollY) // the page scroll the browser was at when
                                      // this texture was rendered
CALLBACK_MEMBER(10, float,
