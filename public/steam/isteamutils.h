...
enum ESteamAPICallFailure {
  ...
  // SteamServersConnected_t will be sent when the client is able to talk to the
  // Steam servers again
  ...
  k_ESteamAPICallFailureMismatchedCallback =
      3, // GetAPICallResult() was called with the wrong callback type for this
         // API call
};
...
public:
  ...
  // running in (as looked up via an IP-to-location database) e.g "US" or "UK".
  ...
  // out results are returned in RGBA format the destination buffer size should
  // be 4 * height * width * sizeof(char)
  ...
  // Sets the position where the overlay instance for the currently calling game
  // should show notifications. This position is per-game and if this function
  // is called from outside of a game context it will do nothing.
  ...
  GetAPICallFailureReason(SteamAPICall_t hSteamAPICall) = 0;
  ...
  // returns the number of IPC calls made since the last time this function was
  // called Used for perf debugging so you can understand how many IPC calls
  // your game makes per frame Every IPC call is at minimum a thread context
  // switch if not a process one so you want to rate control how often you do
  // them.
  ...
  virtual void
  SetWarningMessageHook(SteamAPIWarningMessageHook_t pFunction) = 0;
  ...
  // Normally this call is unneeded if your game has a constantly running frame
  // loop that calls the D3D Present API, or OGL SwapBuffers API every frame.
  //
  ...
  // Present/SwapBuffers calls to drive it's internal frame loop and it may also
  // need to Present() to the screen any time an even needing a notification
  // happens or when the overlay is brought up over the game by a user.  You can
  // use this API to ask the overlay if it currently need a present in that
  // case, and then you can check for this periodically (roughly 33hz is
  // desirable) and make sure you refresh the screen with Present or SwapBuffers
  // to allow the overlay to do it's work.
  virtual bool BOverlayNeedsPresent() = 0;

#ifndef _PS3
  // Asynchronous call to check if an executable file has been signed using the
  // public key set on the signing tab of the partner site, for example to
  // refuse to load modified executable files. The result is returned in
  // CheckFileSignature_t.
  //   k_ECheckFileSignatureNoSignaturesFoundForThisApp - This app has not been
  //   configured on the signing tab of the partner site to enable this
  ...
  //   k_ECheckFileSignatureInvalidSignature - The file exists, and the signing
  //   tab has been set for this file, but the file is either not signed or the
  //   signature does not match. k_ECheckFileSignatureValidSignature - The file
  //   is signed and the signature is valid.
  ...
  // ISteamApps::GetCurrentGameLanguage instead, this is for very special usage
  // cases
