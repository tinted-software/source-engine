...
enum EWorkshopFileType {
  ...
  k_EWorkshopFileTypeGameManagedItem =
      15, // managed completely by the game, not the user, and not shown on the
          // web
...
enum EUGCReadAction {
  ...
  // when reading large files (over 100MB) in sequential chunks. If the last
  // byte is read, this will behave the same as k_EUGCRead_Close.  Otherwise, it
  // behaves the same as k_EUGCRead_ContinueReading. This value maintains the
  // same behavior as before the EUGCReadAction parameter was introduced.
...
public:
  ...
  virtual bool
  SetSyncPlatforms(const char *pchFile,
                   ERemoteStoragePlatform eRemoteStoragePlatform) = 0;

  // file operations that cause network IO
  virtual UGCFileWriteStreamHandle_t
  FileWriteStreamOpen(const char *pchFile) = 0;
  ...
  virtual bool
  FileWriteStreamCancel(UGCFileWriteStreamHandle_t writeHandle) = 0;
  ...
  virtual int32 UGCRead(UGCHandle_t hContent, void *pvData, int32 cubDataToRead,
                        uint32 cOffset, EUGCReadAction eAction) = 0;
  ...
  // 3, the game controls which files are stored in the cloud, via FilePersist,
  ...
  virtual SteamAPICall_t
  DeletePublishedFile(PublishedFileId_t unPublishedFileId) = 0;
  // enumerate the files that the current user published with this app
  virtual SteamAPICall_t EnumerateUserPublishedFiles(uint32 unStartIndex) = 0;
  virtual SteamAPICall_t
  SubscribePublishedFile(PublishedFileId_t unPublishedFileId) = 0;
  virtual SteamAPICall_t EnumerateUserSubscribedFiles(uint32 unStartIndex) = 0;
  virtual SteamAPICall_t
  UnsubscribePublishedFile(PublishedFileId_t unPublishedFileId) = 0;
  virtual bool UpdatePublishedFileSetChangeDescription(
      PublishedFileUpdateHandle_t updateHandle,
      const char *pchChangeDescription) = 0;
  virtual SteamAPICall_t
  GetPublishedItemVoteDetails(PublishedFileId_t unPublishedFileId) = 0;
  virtual SteamAPICall_t
...
struct RemoteStorageAppSyncProgress_t {
  ...
  double
      m_dAppPercentComplete; // Percent complete that this app's transfers are
