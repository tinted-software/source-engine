...
struct LeaderboardEntry_t {
  ...
  int32 m_nGlobalRank; // [1..N], where N is the number of users with an entry
                       // in the leaderboard
...
public:
  ...
  // If the return value is true, but the unlock time is zero, that means it was
  // unlocked before Steam began tracking achievement unlock times (December
  // 2009). Time is seconds since January 1, 1970.
  ...
  // - "hidden" for retrieving if an achievement is hidden (returns "0" when not
  ...
  // all. Calling this w/ N out of N progress will NOT set the achievement, the
  // game must still do that.
  ...
  // functions because they should have a list of existing achievements compiled
  // into them
  ...
  virtual const char *
  GetLeaderboardName(SteamLeaderboard_t hSteamLeaderboard) = 0;

  // returns the total number of entries in a leaderboard, as of the last
  // request
  virtual int
  GetLeaderboardEntryCount(SteamLeaderboard_t hSteamLeaderboard) = 0;
  ...
  // This call is asynchronous, with the result returned in
  // LeaderboardScoresDownloaded_t LeaderboardScoresDownloaded_t will contain a
  // handle to pull the results from GetDownloadedLeaderboardEntries() (below)
  // You can ask for more entries than exist, and it will return as many as do
  // exist. k_ELeaderboardDataRequestGlobal requests rows in the leaderboard
  // from the full table, with nRangeStart & nRangeEnd in the range [1,
  // TotalEntries] k_ELeaderboardDataRequestGlobalAroundUser requests rows
  // around the current user, nRangeStart being negate
  //   e.g. DownloadLeaderboardEntries( hLeaderboard,
  //   k_ELeaderboardDataRequestGlobalAroundUser, -3, 3 ) will return 7 rows, 3
  //   before the user, 3 after
  ...
  // ELeaderboardDataRequest is k_ELeaderboardDataRequestUsers if a user doesn't
  // have a leaderboard entry, they won't be included in the result a max of 100
  // users can be downloaded at a time, with only one outstanding call at a time
        ...
        //				int32 details[3];		// we
        //know this is how many we've stored previously
        //				GetDownloadedLeaderboardEntry(
...
struct LeaderboardFindResult_t {
  ...
  SteamLeaderboard_t m_hSteamLeaderboard; // handle to the leaderboard serarched
                                          // for, 0 if no leaderboard found
...
struct LeaderboardScoreUploaded_t {
  ...
  uint8 m_bScoreChanged; // true if the score in the leaderboard change, false
                         // if the existing score was better
  ...
  int m_nGlobalRankPrevious; // the previous global rank of the user in this
                             // leaderboard; 0 if the user had no existing entry
                             // in the leaderboard
};
...
struct PS3TrophiesInstalled_t {
  ...
  EResult m_eResult; // The result of the operation
  uint64
      m_ulRequiredDiskSpace; // If m_eResult is k_EResultDiskFull, will contain
                             // the amount of space needed to install trophies
};
