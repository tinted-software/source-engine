...
struct SteamPS3Params_t {
  ...
  char
      m_rgchInstallationPath[STEAM_PS3_PATH_MAX]; // directory containing latest
                                                  // steam prx's and sdata. Can
                                                  // be read only (BDVD)
  char m_rgchSystemCache[STEAM_PS3_PATH_MAX];     // temp working cache, not
                                                  // persistent
  char m_rgchGameData[STEAM_PS3_PATH_MAX]; // persistent game data path for
                                           // storing user data
  ...
  // thread to read console input at all.  Using this let's you use Steam
  // console commands like: profile_on, profile_off, profile_dump, mem_stats,
  // mem_validate.
...
struct SteamPS3Memory_t {
  bool m_bSingleAllocation; // If true, Steam will request one 6MB allocation
                            // and use the returned memory for all future
                            // allocations If false, Steam will make call malloc
                            // for each allocation
