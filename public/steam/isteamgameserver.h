...
public:
  ...
  /// Game product identifier.  This is currently used by the master server for
  /// version checking purposes. It's a required field, but will eventually will
  /// go away, and the AppID will be used for this purpose.
  virtual void SetProduct(const char *pszProduct) = 0;

  /// Description of the game.  This is a required field and is displayed in the
  /// steam server browser....for now. This is a required field, but it will go
  /// away eventually, as the data should be determined from the AppID.
  virtual void SetGameDescription(const char *pszGameDescription) = 0;

  /// If your game is a "mod," pass the string that identifies it.  The default
  /// is an empty string, meaning this application is the original game, not a
  /// mod.
  ///
  ...
  /// Region identifier.  This is an optional field, the default value is empty,
  /// meaning the "world" region
  ...
  // SteamUser()->InitiateGameConnection() and must then be sent up to the game
  // server for authentication.
  //
  // Return Value: returns true if the users ticket passes basic checks.
  // pSteamIDUser will contain the Steam ID of this user. pSteamIDUser must NOT
  // be NULL If the call succeeds then you should expect a GSClientApprove_t or
  // GSClientDeny_t callback which will tell you whether authentication for the
  // user has succeeded or failed (the steamid in the callback will match the
  ...
  //
  // Return Value: Returns a SteamID for the user to be tracked with, you should
  // call HandleUserDisconnect() when this user leaves the server just like you
  ...
  // purposes of preventing a single account being logged into multiple servers,
  // showing who is currently on a server, etc.
  virtual void SendUserDisconnect(CSteamID steamIDUser) = 0;

  // Update the data to be displayed in the server browser and matchmaking
  // interfaces for a user currently connected to the server.  For regular users
  // you must call this after you receive a GSUserValidationSuccess callback.
  //
  // Return Value: true if successful, false if failure (ie, steamIDUser wasn't
  ...
  // using BeginAuthSession API ). pcbTicket retrieves the length of the actual
  // ticket.
  virtual HAuthTicket GetAuthSessionTicket(void *pTicket, int cbMaxTicket,
                                           uint32 *pcbTicket) = 0;

  // Authenticate ticket ( from GetAuthSessionTicket ) from entity steamID to be
  // sure it is valid and isnt reused Registers for callbacks if the entity goes
  // offline or cancels the ticket ( see ValidateAuthTicketResponse_t callback
  // and EAuthSessionResponse )
  ...
  // SendUserConnectAndAuthenticate, use this function to determine if the user
  // owns downloadable content specified by the provided AppID.
  ...
  // server is behind NAT and you want to advertise its IP in a lobby for other
  // clients to directly connect to
  ...
  // AFTER calling HandleIncomingPacket for any packets that came in that frame,
  // call this. This gets a packet that the master server updater needs to send
  // out on UDP. It returns the length of the packet it wants to send, or 0 if
  // there are no more packets to send. Call this each frame until it returns 0.
  ...
  // Call this as often as you like to tell the master server updater whether or
  // not you want it to be active (default: off).
  ...
  ComputeNewPlayerCompatibility(CSteamID steamIDNewPlayer) = 0;
};
...
struct GSGameplayStats_t {
  ...
  uint32 m_unTotalConnects; // Total number of clients who have ever connected
                            // to the server
  uint32 m_unTotalMinutesPlayed; // Total number of minutes ever played on the
                                 // server
};
