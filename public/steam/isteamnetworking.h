...
enum EP2PSessionError {
  ...
  k_EP2PSessionErrorTimeout =
      4, // target isn't responding, perhaps not calling
         // AcceptP2PSessionWithUser() corporate firewalls can also block this
         // (NAT traversal is not firewall traversal) make sure that UDP ports
         // 3478, 4379, and 4380 are open in an outbound direction
...
enum EP2PSend {
  ...
  // NAT-traversal accomplished or there is a recognized adjustment happening on
  // the connection, the packet will be batched until the connection is open
  // again.
  ...
  // change) or ~200ms has passed (Nagle algorithm). Useful if you want to send
  // a set of smaller messages but have the coalesced into a single packet Since
  // the reliable stream is all ordered, you can do several small message sends
  // with k_EP2PSendReliableWithBuffering and then do a normal
...
enum ESNetSocketState {
  ...
  k_ESNetSocketStateReceivedRemoteCandidates =
      12, // we've received information from the remote machine, via the Steam
          // back-end, about their IP info
  ...
  // failure states
  k_ESNetSocketStateDisconnecting = 21, // the API shut it down, and we're in
                                        // the process of telling the other end
  ...
  k_ESNetSocketStateConnectionBroken =
      25, // connection has been broken; either the other end has disappeared or
          // our local network connection has broke
...
public:
  ...
  //
  // nChannel is a routing number you can use to help route message to different
  // systems 	- you'll have to call ReadP2PPacket() with the same channel
  // number in order to retrieve the data on the other end using different
  // channels to talk to the same user will still use the same underlying p2p
  // connection, saving on resources
  virtual bool SendP2PPacket(CSteamID steamIDRemote, const void *pubData,
                             uint32 cubData, EP2PSend eP2PSendType,
                             int nChannel = 0) = 0;

  // returns true if any data is available for read, and the amount of data that
  // will need to be read
  virtual bool IsP2PPacketAvailable(uint32 *pcubMsgSize, int nChannel = 0) = 0;

  // reads in a packet that has been sent from another user via SendP2PPacket()
  // returns the size of the message and the steamID of the user who sent it in
  // the last two parameters if the buffer passed in is too small, the message
  // will be truncated this call is not blocking, and will return false if no
  // data is available
  ...
  // P2PSessionRequest_t callback P2PSessionRequest_t will be posted if another
  // user tries to send you a packet that you haven't talked to yet if you don't
  // want to talk to the user, just ignore the request if the user continues to
  // send you packets, another P2PSessionRequest_t will be posted periodically
  // this may be called multiple times for a single user
  // (if you've called SendP2PPacket() on the other user, this implicitly
  ...
  // closed, the open session to the user will be closed and new data from this
  ...
  // servers if a direct connection or NAT-traversal cannot be established. Only
  // applies to connections created after setting this value, or to existing
  // connections that need to automatically reconnect after this value is set.
  //
  ...
  //
  // This is an older set of functions designed around the Berkeley TCP sockets
  // model it's preferential that you use the above P2P functions, they're more
  // robust and these older functions will be removed eventually
  //
  ////////////////////////////////////////////////////////////////////////////////////////////

  // creates a socket and listens others to connect
  // will trigger a SocketStatusCallback_t callback on another client connecting
  // nVirtualP2PPort is the unique ID that the client will connect to, in case
  ...
  //		pass in 0 if you don't want users to be able to connect via
  //IP/Port, but expect to be always peer-to-peer connections only
  virtual SNetListenSocket_t
  CreateListenSocket(int nVirtualP2PPort, uint32 nIP, uint16 nPort,
                     bool bAllowUseOfPacketRelay) = 0;

  // creates a socket and begin connection to a remote destination
  // can connect via a known steamID (client or game server), or directly to an
  // IP on success will trigger a SocketStatusCallback_t callback on failure or
  // timeout will trigger a SocketStatusCallback_t callback with a failure code
  // in m_eSNetSocketState
  ...
  // bNotifyRemoteEnd is set, socket will not be completely destroyed until the
  // remote end acknowledges the disconnect
  virtual bool DestroySocket(SNetSocket_t hSocket, bool bNotifyRemoteEnd) = 0;
  // destroying a listen socket will automatically kill all the regular sockets
  // generated from it
  ...
  // after this, many routers will start dropping packets use the reliable flag
  // with caution; although the resend rate is pretty aggressive, it can still
  ...
  // packetized, not streaming) if *pcubMsgSize < cubDest, only partial data is
  // written returns false if no data is available
  virtual bool RetrieveDataFromSocket(SNetSocket_t hSocket, void *pubDest,
                                      uint32 cubDest, uint32 *pcubMsgSize) = 0;
  ...
  // streaming) if *pcubMsgSize < cubDest, only partial data is written returns
  // false if no data is available fills out *phSocket with the socket that data
  // is available on
  ...
  // returns information about the specified socket, filling out the contents of
  // the pointers
  ...
  GetSocketConnectionType(SNetSocket_t hSocket) = 0;
...
struct SocketStatusCallback_t {
  ...
  SNetListenSocket_t
      m_hListenSocket; // this is the server socket that we were listening on;
