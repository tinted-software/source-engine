...
struct MatchMakingKeyValuePair_t {
  ...
  MatchMakingKeyValuePair_t(const char *pchKey, const char *pchValue) {
    strncpy(m_szKey, pchKey,
            sizeof(m_szKey)); // this is a public header, use basic c library
                              // string funcs only!
...
enum EMatchMakingServerResponse {
  ...
  eNoServersListedOnMasterServer // for the Internet query type, returned in
                                 // response callback if no servers of this type
                                 // match
};
...
public:
  ...
  char m_szGameDescription[k_cbMaxGameServerGameDescription]; ///< game
                                                              ///< description
  uint32 m_nAppID; ///< Steam App ID of this server
  int m_nPlayers; ///< total number of players currently on the server. INCLUDES
                  ///< BOTS!!
...
public:
  ...
  /// steamID of the game server - invalid if it's doesn't have one (old server,
  /// or not connected to Steam)
  CSteamID m_steamID;
};
