...
public:
  ...
  // HTTPRequestCompleted_t callback after sending the request.  This is just so
  // the caller can easily keep track of which callbacks go with which request
  // data.
  ...
  // sending the request.  Default timeout is 60 seconds if you don't call this.
  // Returns false if the handle is invalid, or the request has already been
  // sent.
  virtual bool
  SetHTTPRequestNetworkActivityTimeout(HTTPRequestHandle hRequest,
                                       uint32 unTimeoutSeconds) = 0;

  // Set a request header value for the request, must be called prior to sending
  // the request.  Will return false if the handle is invalid or the request is
  // already sent.
  virtual bool SetHTTPRequestHeaderValue(HTTPRequestHandle hRequest,
                                         const char *pchHeaderName,
                                         const char *pchHeaderValue) = 0;

  // Set a GET or POST parameter value on the request, which is set will depend
  // on the EHTTPMethod specified when creating the request.  Must be called
  // prior to sending the request.  Will return false if the handle is invalid
  // or the request is already sent.
  ...
  // only-if-cached cache-control header and only do a local cache lookup rather
  // than sending any actual remote request.
  ...
  // many requests queued, and this will move the specified request to the head
  // of the queue.  Returns false on invalid handle, or if the request is not
  // yet sent.
  ...
  // HTTPRequestCompleted_t, will return false if the handle is invalid or is to
  // a streaming response, or if the provided buffer is not the correct size.
  // Use BGetHTTPResponseBodySize first to find out the correct buffer size to
  // use.
  ...
  // HTTPRequestDataReceived_t. Will return false if the handle is invalid or is
  // to a non-streaming response (meaning it wasn't sent with
  // SendHTTPRequestAndStreamResponse), or if the buffer size and offset do not
  // match the size and offset sent in HTTPRequestDataReceived_t.
  virtual bool GetHTTPStreamingResponseBodyData(HTTPRequestHandle hRequest,
                                                uint32 cOffset,
                                                uint8 *pBodyDataBuffer,
                                                uint32 unBufferSize) = 0;

  // Releases an HTTP response handle, should always be called to free resources
  // after receiving a HTTPRequestCompleted_t callback and finishing using the
  // response.
  ...
  // content-length field.  For responses that contain no content-length it will
  // report zero for the duration of the request as the size is unknown until
  ...
  // bAllowResponsesToModify=false than only cookies you explicitly set will be
  // sent.  This API is just for during process lifetime, after steam restarts
  // no cookies are persisted and you have no way to access the cookie container
  // across repeat executions of your process.
  virtual HTTPCookieContainerHandle
  CreateCookieContainer(bool bAllowResponsesToModify) = 0;

  // Release a cookie container you are finished using, freeing it's memory
  virtual bool
  ReleaseCookieContainer(HTTPCookieContainerHandle hCookieContainer) = 0;
  ...
  // Set that https request should require verified SSL certificate via machines
  ...
  // Check if the reason the request failed was because we timed it out (rather
  // than some harder failure)
...
struct HTTPRequestCompleted_t {
  ...
  // (even an error). It will be false if we failed due to an internal error or
  // client side network failure.
  bool m_bRequestSuccessful;

  // Will be the HTTP status code value returned by the server,
  // k_EHTTPStatusCode200OK is the normal OK response, if you get something else
  // you probably need to treat it as a failure.
...
struct HTTPRequestDataReceived_t {
  ...
  // Offset to provide to GetHTTPStreamingResponseBodyData to get this chunk of
  // data
  uint32 m_cOffset;
