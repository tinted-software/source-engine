...
enum EItemState {
  ...
  k_EItemStateNeedsUpdate = 8, // items needs an update. Either because it's not
                               // installed yet or creator updated content
...
struct SteamUGCDetails_t {
  ...
  char m_rgchDescription[k_cchPublishedDocumentDescriptionMax]; // description
                                                                // of document
  uint64 m_ulSteamIDOwner; // Steam ID of the user who created this content.
  uint32 m_rtimeCreated;   // time when the published file was created
  uint32 m_rtimeUpdated;   // time when the published file was last updated
  uint32 m_rtimeAddedToUserList; // time when the user added the published file
  ...
  bool m_bTagsTruncated; // whether the list of tags was too long to be returned
...
class ISteamUGC {
public:
  // Query UGC associated with a user. Creator app id or consumer app id must be
  ...
  // Query for all matching UGC. Creator app id or consumer app id must be valid
  // and be set to the current running app. unPage should start at 1.
  ...
  CreateItem(AppId_t nConsumerAppId,
             EWorkshopFileType eFileType) = 0; // create new item for this app
                                               // with no content attached yet
  ...
  virtual bool
  SetItemTitle(UGCUpdateHandle_t handle,
  ...
  virtual bool SetItemContent(
      UGCUpdateHandle_t handle,
      const char
          *pszContentFolder) = 0; // update item content from this local folder
  ...
  virtual SteamAPICall_t SetUserItemVote(PublishedFileId_t nPublishedFileID,
                                         bool bVoteUp) = 0;
  virtual SteamAPICall_t
  GetUserItemVote(PublishedFileId_t nPublishedFileID) = 0;
  virtual SteamAPICall_t
  AddItemToFavorites(AppId_t nAppId, PublishedFileId_t nPublishedFileID) = 0;
  virtual SteamAPICall_t
  RemoveItemFromFavorites(AppId_t nAppId,
                          PublishedFileId_t nPublishedFileID) = 0;
  virtual SteamAPICall_t SubscribeItem(
      PublishedFileId_t nPublishedFileID) = 0; // subscribe to this item, will
                                               // be installed ASAP
  virtual SteamAPICall_t
  UnsubscribeItem(PublishedFileId_t
                      nPublishedFileID) = 0; // unsubscribe from this item, will
  ...
  // wait for DownloadItemResult_t. If the item is already installed, then files
  // on disk should not be used until callback received. If item is not
