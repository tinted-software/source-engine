...
private:
  bool m_bInsideArray; // Set to true by the engine if this property sits inside
                       // an array.
...
#define RECVINFO(varName)                                                      \
  #varName, _hacky_dtrecv_offsetof(currentRecvDTClass, varName),               \
      sizeof(((currentRecvDTClass *)0)->varName)
#define RECVINFO_NAME(varName, remoteVarName)                                  \
  #remoteVarName, _hacky_dtrecv_offsetof(currentRecvDTClass, varName),         \
      sizeof(((currentRecvDTClass *)0)->varName)
...
RecvProp RecvPropFloat(
    ...
    int sizeofVar = SIZEOF_IGNORE, // Handled by RECVINFO macro, but set to
                                   // SIZEOF_IGNORE if you don't want to bother.
