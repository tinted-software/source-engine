...
enum callbackflags {
  ...
  CALLBACK_MARKED_FOR_DELETE =
      0x0400, // This allows vphysics to skip some work for this object since it
              // will be deleted later this frame. (Set automatically by destroy
              // calls)
  CALLBACK_ENABLING_COLLISION =
      0x0800, // This is active during the time an object is enabling collisions
              // allows us to skip collisions between "new" objects and objects
              // marked for delete
...
public:
  ...
  virtual bool IsFluid()
      const = 0; // fluids are special triggers with fluid controllers attached,
                 // they return true to IsTrigger() as well!
...
struct surfaceaudioparams_t {
  // sounds / audio data
  float reflectivity; // like elasticity, but how much sound should be reflected
                      // by this surface
  float
  ...
  float hardVelocityThreshold; // collision velocity > this causes "hard"
                               ...
                               // Hard impacts must meet both hardnessFactor AND
                               // velocity thresholds
};
...
struct surfacegameprops_t {
  // game movement data
  float
      maxSpeedFactor; // Modulates player max speed when walking on this surface
...
struct springparams_t {
  ...
  float relativeDamping; // relative damping (damping proportional to the change
                         // in the relative position of the objects)
...
#define DEFINE_PIID(type, enumval)                                             \
  template <> inline PhysInterfaceId_t GetPhysIID<type>(type **) {             \
    return enumval;                                                            \
  }

template <class PHYSPTR>
inline PhysInterfaceId_t
GetPhysIID(PHYSPTR **); // will get link error if no match
