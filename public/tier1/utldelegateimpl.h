...
//Intel Itanium ICL. 	27-Jun-04 1.2 * Now works on Borland C++ Builder 5.5
//				  * Now works on /clr "managed C++" code on VC7,
//VC7.1
//				  * Comeau C++ now compiles without warnings.
//				  * Prevent the virtual inheritance case from
//being used on 					  VC6 and earlier, which generate incorrect code.
//				  * Improved warning and error messages.
//Non-standard hacks 					 now have compile-time checks to make them safer.
//				  * implicit_cast used instead of static_cast in
//many cases.
//				  * If calling a const member function, a const
//class pointer can be used.
//				  * UtlMakeDelegate() global helper function
//added to simplify pass-by-value.
...
//				  * Less use of macros. Error messages should be
//more comprehensible.
...
template <typename Signature> class CUtlDelegate;
...
//
//			UtlMakeDelegate(&x, &X::func) returns a fastdelegate of
//the type 			necessary for calling x.func() with the correct number of arguments.
...
CUtlDelegate<FASTDLGT_RETTYPE(Param1)>
UtlMakeDelegate(Y *x, RetType (X::*func)(Param1 p1)) {
  return CUtlDelegate<FASTDLGT_RETTYPE(Param1)>(x, func);
}

template <class X, class Y, class Param1, class RetType>
CUtlDelegate<FASTDLGT_RETTYPE(Param1)>
UtlMakeDelegate(Y *x, RetType (X::*func)(Param1 p1) const) {
  return CUtlDelegate<FASTDLGT_RETTYPE(Param1)>(x, func);
}

template <class Param1, class RetType>
CUtlDelegate<FASTDLGT_RETTYPE(Param1)>
UtlMakeDelegate(RetType (*func)(Param1 p1)) {
  return CUtlDelegate<FASTDLGT_RETTYPE(Param1)>(func);
}

// N=2
template <class X, class Y, class Param1, class Param2, class RetType>
CUtlDelegate<FASTDLGT_RETTYPE(Param1, Param2)>
UtlMakeDelegate(Y *x, RetType (X::*func)(Param1 p1, Param2 p2)) {
  return CUtlDelegate<FASTDLGT_RETTYPE(Param1, Param2)>(x, func);
}

template <class X, class Y, class Param1, class Param2, class RetType>
CUtlDelegate<FASTDLGT_RETTYPE(Param1, Param2)>
UtlMakeDelegate(Y *x, RetType (X::*func)(Param1 p1, Param2 p2) const) {
  return CUtlDelegate<FASTDLGT_RETTYPE(Param1, Param2)>(x, func);
}

template <class Param1, class Param2, class RetType>
CUtlDelegate<FASTDLGT_RETTYPE(Param1, Param2)>
UtlMakeDelegate(RetType (*func)(Param1 p1, Param2 p2)) {
