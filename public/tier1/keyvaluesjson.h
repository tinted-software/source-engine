...
public:
  ...
  /// Parse the whole string.  If there's a problem, returns NULL and sets
  /// m_nLine,m_szErrMsg with more info.
  KeyValues *ParseFile();

  /// Error message is returned here, if there is one.
  char m_szErrMsg[1024];

  /// Line number of current token during parsing, or of the error, of pasring
  /// fails
  int m_nLine;
