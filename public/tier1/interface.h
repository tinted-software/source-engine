...
#define EXPOSE_INTERFACE(className, interfaceName, versionName)                \
  ...
  static InterfaceReg                                                          \
      __g_Create##className##_reg(__Create##className##_interface,             \
                                  versionName);                                \
  }
#endif
...
#define EXPOSE_SINGLE_INTERFACE_GLOBALVAR(className, interfaceName,            \
  ...
  EXPOSE_SINGLE_INTERFACE_GLOBALVAR_WITH_NAMESPACE(className, , interfaceName, \
                                                   versionName, globalVarName)
...
enum Sys_Flags {
  SYS_NOFLAGS = 0x00,
  SYS_NOLOAD =
      0x01 // no loading, no ref-counting, only returns handle if lib is loaded.
};
