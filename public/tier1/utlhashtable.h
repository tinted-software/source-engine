...
template <typename KeyT, typename ValueT = empty_t> class CUtlHashtableEntry {
public:
  ...
  enum {
    FLAG_FREE =
        INT16_STORAGE
            ? 0x8000
            : 0x80000000, // must be high bit for IsValid and IdealIndex to work
  ...
  // Use template tricks to fully define only one function that takes either 16
  // or 32 bits and performs different logic without using "if ( INT16_STORAGE
  // )", because GCC and MSVC sometimes have trouble removing the constant
  // branch, which is dumb... but whatever. 16-bit hashes are simply too narrow
  // for large hashtables; more mask bits than hash bits! So we duplicate the
  // hash bits. (Note: h *= MASK_HASH+2 is the same as h += h<<HASH_BITS)
...
public:
  explicit CUtlHashtable(int minimumSize = 32)
      : m_nUsed(0), m_nMinSize(MAX(8, minimumSize)), m_bSizeLocked(false),
        m_eq(), m_hash() {}
  ...
  CUtlHashtable(entry_t *pMemory, unsigned int nCount,
                ...
                KeyIsEqualT const &eq = KeyIsEqualT())
      : m_nUsed(0), m_nMinSize(8), m_bSizeLocked(false), m_eq(eq),
        m_hash(hash) {
  ...
  void SetExternalBuffer(byte *pRawBuffer, unsigned int nBytes,
                         bool bAssumeOwnership = false, bool bGrowable = false);
  ...
  // Remove by handle, convenient when you look up a handle and do something
  // with it before removing the element
...
int CUtlHashtable<KeyT, ValueT, KeyHashT, KeyIsEqualT,
  ...
  if (allowGrow && !m_bSizeLocked) {
    ...
    if ((newSize * 4 < m_table.Count() && m_table.Count() > m_nMinSize * 2) ||
...
template <typename KeyT, typename ValueT, typename KeyHashT,
          typename KeyIsEqualT, typename AltKeyT>
UtlHashHandle_t
CUtlHashtable<KeyT, ValueT, KeyHashT, KeyIsEqualT, AltKeyT>::RemoveAndAdvance(
    UtlHashHandle_t idx) {
...
template <typename KeyT, typename ValueT, typename KeyHashT,
          typename KeyIsEqualT, typename AltKeyT>
void CUtlHashtable<KeyT, ValueT, KeyHashT, KeyIsEqualT,
                   AltKeyT>::RemoveByHandle(UtlHashHandle_t idx) {
  Assert(IsValidHandle(idx));

  // Copied from RemoveAndAdvance(): TODO optimize, implement DoRemoveAt that
  // does not need to re-evaluate equality in DoLookup
  DoRemove<KeyArg_t>(m_table[idx]->m_key,
                     m_table[idx].flags_and_hash & MASK_HASH);
}
...
template <typename KeyT, typename ValueT, typename KeyHashT,
          typename KeyIsEqualT, typename AltKeyT>
UtlHashHandle_t
CUtlHashtable<KeyT, ValueT, KeyHashT, KeyIsEqualT, AltKeyT>::NextHandle(
    handle_t start) const {
  const entry_t *table = m_table.Base();
...
public:
  ...
  typedef typename ArgumentTypeInfo<ValueT>::Arg_t ValueArg_t;
  typedef typename ArgumentTypeInfo<AlternateKeyT>::Arg_t KeyAlt_t;
  typedef typename CTypeSelect<sizeof(IndexStorageT) == 2, uint16, uint32>::type
      IndexStorage_t;
...
template <typename K, typename V, typename H, typename E, typename S,
          typename A>
template <typename KeyArgumentT>
inline UtlHashHandle_t
CUtlStableHashtable<K, V, H, E, S, A>::DoFind(KeyArgumentT k) const {
  unsigned int hash = m_table.GetHashRef()(k);
  UtlHashHandle_t h = m_table.template DoLookup<KeyArgumentT>(k, hash, NULL);
  if (h != m_table.InvalidHandle())
...
template <typename KeyArgumentT>
inline UtlHashHandle_t
CUtlStableHashtable<K, V, H, E, S, A>::DoInsert(KeyArgumentT k) {
  unsigned int hash = m_table.GetHashRef()(k);
  UtlHashHandle_t h = m_table.template DoLookup<KeyArgumentT>(k, hash, NULL);
  if (h != m_table.InvalidHandle())
    return m_table[h].m_index;
  ...
  m_table.template DoInsertNoCheck<IndirectIndex>(IndirectIndex(idx), empty_t(),
                                                  hash);
  return idx;
}
