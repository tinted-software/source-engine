...
#if defined(OSX)
#ifndef wcsdup
// The mem override tools may provide a copy of this if active, otherwise it is
// not available in OS X's libc due to being introduced in POSIX-20008
...
public:
  ...
  // Get a string with the filename removed (uses V_UnqualifiedFileName and also
  // strips the last slash)
...
private:
  // INTERNALS
  // AllocMemory allocates enough space for length characters plus a terminating
  // zero. Previous characters are preserved, the buffer is null-terminated, but
  // new characters are not touched.
...
template <typename T> class StringFuncs {
public:
...
template <> class StringFuncs<char> {
public:
...
template <typename T = char> class CUtlConstStringBase {
public:
...
template <typename T> void CUtlConstStringBase<T>::Set(const T *pValue) {
  if (pValue != m_pString) {
    free((void *)m_pString);
...
template <typename T> int CUtlConstStringBase<T>::Compare(const T *rhs) const {
  // Empty or null RHS?
  if (!rhs || !rhs[0])
...
template <typename T> struct UTLConstStringCaselessStringLessFunctor {
  bool operator()(const CUtlConstStringBase<T> &a, const char *b) const {
    return StringFuncs<T>::CaselessCompare(a.Get(), b) < 0;
  }
};
template <typename T> struct UTLConstStringCaselessStringEqualFunctor {
  bool operator()(const CUtlConstStringBase<T> &a, const char *b) const {
    return StringFuncs<T>::CaselessCompare(a.Get(), b) == 0;
  }
};
