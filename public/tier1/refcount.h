...
template <typename T> inline void SafeAssign(T **ppInoutDst, T *pInoutSrc) {
  Assert(ppInoutDst);
...
template <typename T> inline void SafeRelease(T **ppInoutPtr) {
  Assert(ppInoutPtr);
  if (*ppInoutPtr)
    (*ppInoutPtr)->Release();
...
template <class T = IRefCounted> class CAutoRef {
public:
...
template <class T> class CBaseAutoPtr {
public:
...
template <class T> class CRefPtr : public CBaseAutoPtr<T> {
