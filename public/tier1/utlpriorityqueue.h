...
template <class T> class CUtlPriorityQueue {
public:
  // Less func typedef
...
template <class T> void CUtlPriorityQueue<T>::RemoveAtHead() {
  m_heap.FastRemove(0);
  int index = 0;
...
template <class T> void CUtlPriorityQueue<T>::RemoveAt(int index) {
...
template <class T> void CUtlPriorityQueue<T>::Insert(T const &element) {
...
template <class T> void CUtlPriorityQueue<T>::Swap(int index1, int index2) {
  T tmp = m_heap[index1];
  m_heap[index1] = m_heap[index2];
  m_heap[index2] = tmp;
}

template <class T> void CUtlPriorityQueue<T>::SetLessFunc(LessFunc_t lessfunc) {
  m_LessFunc = lessfunc;
}
