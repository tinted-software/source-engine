...
#define FCVAR_REPLICATED                                                       \
  (1 << 13) // server setting enforced on clients, TODO rename to FCAR_SERVER at
            // some time
...
#define FCVAR_EXEC_DESPITE_DEFAULT                                             \
  (1 << 31) // -default causes a lot of commands to be ignored (but still be
            // recorded as though they had run). This causes them to be executed
            // anyways.

#define FCVAR_INTERNAL_USE                                                     \
  (1 << 15) // This var isn't archived, but is exposed to players--and its use
            // is allowed in competitive play.
#define FCVAR_ALLOWED_IN_COMPETITIVE                                           \
  (1 << 18) // This convar can be changed in competitive (strict) settings mode
            // even though it is not archived. Meant for one-offs like
            // cl_showfps that are not user facing settings but are benign
