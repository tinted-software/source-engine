...
public:
  ...
  void WriteUBitLong(unsigned int data, int numbits,
                     bool bCheckRange = true) RESTRICT;
...
public:
  ...
  inline bool
  SeekRelative(int iBitDelta); // Seek to an offset from the current position.
...
inline int bf_read::ReadOneBitNoCheck() {
  ...
  unsigned int value =
      ((uint32 * RESTRICT) m_pData)[m_iCurBit >> 5] >> (m_iCurBit & 31);
#else
...
BITBUF_INLINE unsigned int bf_read::ReadUBitLong(int numbits) RESTRICT {
  ...
  unsigned int dw1 =
      LoadLittleDWord((uint32 * RESTRICT) m_pData, iWordOffset1) >> iStartBit;
  unsigned int dw2 = LoadLittleDWord((uint32 * RESTRICT) m_pData, iWordOffset2)
                     << (32 - iStartBit);
