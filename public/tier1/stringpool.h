...
//-----------------------------------------------------------------------------
template <class T> class CCountedStringPoolBase {
...
template <class T> inline CCountedStringPoolBase<T>::~CCountedStringPoolBase() {
  FreeAll();
}

template <class T> inline void CCountedStringPoolBase<T>::FreeAll() {
  int i;
...
template <class T>
inline T
CCountedStringPoolBase<T>::ReferenceStringHandle(const char *pIntrinsic) {
  if (pIntrinsic == NULL)
    return INVALID_ELEMENT;
...
template <>
inline void CCountedStringPoolBase<unsigned short>::VerifyNotOverflowed(
    unsigned int value) {
  Assert(value < 0xffff);
}
...
template <>
inline void
CCountedStringPoolBase<unsigned int>::VerifyNotOverflowed(unsigned int value) {}

template <class T>
inline char *
CCountedStringPoolBase<T>::ReferenceString(const char *pIntrinsic) {
  if (!pIntrinsic)
    return NULL;
...
template <class T>
inline void
CCountedStringPoolBase<T>::DereferenceString(const char *pIntrinsic) {
  ...
  for (T previous = INVALID_ELEMENT; nCurrentBucket != INVALID_ELEMENT;
       nCurrentBucket = m_Elements[nCurrentBucket].nNextElement) {
    if (!Q_stricmp(pIntrinsic, m_Elements[nCurrentBucket].pString)) {
      // Anyone who hits 65k references is permanant
      if (m_Elements[nCurrentBucket].nReferenceCount < MAX_REFERENCE) {
...
template <class T> inline void CCountedStringPoolBase<T>::SpewStrings() {
  int i;
  for (i = 0; i < m_Elements.Count(); i++) {
...
template <>
inline bool
CCountedStringPoolBase<unsigned short>::SaveToBuffer(CUtlBuffer &buffer) {
...
template <>
inline bool
CCountedStringPoolBase<unsigned short>::RestoreFromBuffer(CUtlBuffer &buffer) {
...
template <>
inline bool
CCountedStringPoolBase<unsigned int>::SaveToBuffer(CUtlBuffer &buffer) {
  if (m_Elements.Count() <= 1) {
    // pool is empty, saving nothing
    // caller can check put position of buffer to detect
    return true;
...
template <>
inline bool
CCountedStringPoolBase<unsigned int>::RestoreFromBuffer(CUtlBuffer &buffer) {
  int signature = buffer.GetInt();
  if (signature != STRING_POOL_VERSION) {
    // wrong version
    return false;
