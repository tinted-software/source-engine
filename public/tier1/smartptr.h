...
public:
  template <class T> static void AddRef(T *pObj) { pObj->AddRef(); }
...
public:
  template <class T> static void AddRef(T *pObj) { pObj->AddReference(); }
...
//
template <typename T> class CPlainAutoPtr {
public:
...
template <class T, class RefCountAccessor = CRefCountAccessor> class CSmartPtr {
public:
  CSmartPtr();
  CSmartPtr(T *pObj);
...
//destruction.
//
template <typename T> class CAutoPushPop {
public:
