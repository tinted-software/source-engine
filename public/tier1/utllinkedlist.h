...
#define FOR_EACH_LL(listName, iteratorName)                                    \
  for (auto iteratorName = (listName).Head();                                  \
...
#define FOR_EACH_LL_BACK(listName, iteratorName)                               \
  for (auto iteratorName = (listName).Tail();                                  \
       (listName).IsUtlLinkedList &&                                           \
       iteratorName != (listName).InvalidIndex();                              \
...
template <class T, class I> struct UtlLinkedListElem_t {
  T m_Element;
  I m_Previous;
  I m_Next;
...
public:
  ...
  typedef S IndexType_t; // should really be called IndexStorageType_t, but that
  ...
  template <typename List_t> class _CUtlLinkedList_constiterator_t {
  public:
...
template <class T, class S, bool ML, class I, class M>
inline bool
CUtlLinkedList<T, S, ML, I, M>::IndexInRange(I index) // Static method
{
  ...
  // These COMPILE_TIME_ASSERT checks need to be in individual scopes to avoid
  // build breaks on MacOS and Linux due to a gcc bug.
  {
...
template <typename T> class CUtlPtrLinkedList {
public:
