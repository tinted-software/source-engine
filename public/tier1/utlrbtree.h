...
inline CUtlRBTree<T, I, L, M>::CUtlRBTree(int growSize, int initSize,
    ...
    : m_Elements(growSize, initSize), m_LessFunc(lessfunc),
      m_Root(InvalidIndex()), m_NumElements(0), m_FirstFree(InvalidIndex()),
...
inline bool CUtlRBTree<T, I, L, M>::IsValidIndex(I i) const {
  ...
  if (m_LastAlloc == m_Elements.InvalidIndex() ||
      m_Elements.IsIdxAfter(i, m_LastAlloc))
    return false; // don't read values that have been allocated, but not
                  // constructed
...
template <class T, class I, typename L, class M>
inline typename CUtlRBTree<T, I, L, M>::Links_t &
CUtlRBTree<T, I, L, M>::Links(I i) {
