...
#define FmtStrVSNPrintf(szBuf, nBufSize, bQuietTruncation, ppszFormat,         \
  ...
  do {                                                                         \
    ...
    if (bTruncated && !(bQuietTruncation) && scAsserted < 5) {                 \
      Warning("FmtStrVSNPrintf truncated to %d without QUIET_TRUNCATION "      \
              "specified!\n",                                                  \
              (int)(nBufSize));                                                \
      AssertMsg(                                                               \
          0,                                                                   \
...
template <int SIZE_BUF, bool QUIET_TRUNCATION = false> class CFmtStrN {
public:
  CFmtStrN() {
    InitQuietTruncation();
    m_szBuf[0] = 0;
...
//			Derived off of the Steam CNumStr but with a few tweaks,
//such as 			trimming off the in-our-cases-unnecessary strlen calls (by not 			storing
//the length in the class).
//-----------------------------------------------------------------------------
