...
private:
  // Disallowed
  CUtlSymbolLarge(
      const char *pStr); // they need to go through the table to assign the ptr
  bool operator==(const char *pStr)
      const; // disallow since we don't know if the table this is from was case
...
template <class TreeType, bool CASEINSENSITIVE> class CTreeEntryLess {
public:
...
public:
  ...
  inline int GetElements(int nFirstElement, int nCount,
    ...
    list.EnsureCount(nCount);
    int c =
        CThreadsafeTreeType::GetElements(nFirstElement, nCount, list.Base());
    for (int i = 0; i < c; ++i) {
...
template <class TreeType, bool CASEINSENSITIVE, size_t POOL_SIZE>
inline int
CUtlSymbolTableLargeBase<TreeType, CASEINSENSITIVE,
                         POOL_SIZE>::FindPoolWithSpace(int len) const {
  for (int i = 0; i < m_StringPools.Count(); i++) {
    StringPool_t *pPool = m_StringPools[i];
...
CUtlSymbolTableLargeBase<TreeType, CASEINSENSITIVE, POOL_SIZE>::AddString(
  ...
  int lenDecorated =
      lenString +
      sizeof(LargeSymbolTableHashDecoration_t); // and with its hash decoration
