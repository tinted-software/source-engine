...
public:
  ...
  //the growable string table, you cannot 	share KeyValues pointers directly with
  //any other module. You can serialize them across 	module boundaries. These
  //limitations are acceptable in the Steam backend code 	this option was written
  //for, but may not be in other situations. Make sure to 	understand the
  //implications before using this.
  ...
  FindLastSubKey(); // returns the LAST subkey in the list.  This requires a
                    // linked list iteration to find the key.  Returns NULL if
  ...
  KeyValues *GetFirstValue(); // When you get a value back, you can use GetX and
                              // pass in NULL to get the value.
  ...
  // Allocate & create a new copy of the keys, including the next keys. This is
  // useful for top level files that don't use the usual convention of a root
  // key with lots of children (like soundscape files).
  ...
  void UnpackIntoStructure(struct KeyValuesUnpackStructure const *pUnpackTable,
                           void *pDest, size_t DestSizeInBytes);
...
public:
  ...
  // Functions to get external access to whichever of the above functions we're
  // going to call.
  static intp CallGetSymbolForString(const char *name, bool bCreate = true) {
    return s_pfGetSymbolForString(name, bCreate);
  }
  static const char *CallGetStringForSymbol(intp symbol) {
