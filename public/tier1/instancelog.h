...
template <class Object, class RecordType> class InstanceLogger {
public:
  typedef RecordType RecordType_t;

protected:
  void InstanceLog(const char *pMsg) { m_records.AddToTail(BuildRecord(pMsg)); }
...
// // Your class needs to implement something like this. You don't have to use a
// tuple,
// // but if you don't you also need to implement FormatRecord for your type.
// RecordType BuildRecord( const char* pMsg )
// {
//     return std::make_tuple( this, pMsg, g_FrameNum, Plat_FloatTime(),
//     m_nAllocatedWidth, m_nAllocatedHeight, m_nAllocatedDepth,
//     m_nTargetResidence, m_nCurrentResidence );
// }
