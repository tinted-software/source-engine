...
protected:
  ...
  size_t m_nRowStrideInBytes[MAX_SOA_FIELDS]; // stride from one row datum to
                                              // another per field
  size_t m_nSliceStrideInBytes[MAX_SOA_FIELDS]; // stride from one slice datum
                                                // to another per field
...
public:
  ...
  // #define ATTR_GREEN 1
  // #define ATTR_BLUE 2
  // CSOAContainer myimage( 256, 256, ATTR_RED, ATTRDATATYPE_FLOAT, ATTR_GREEN,
  // ATTRDATATYPE_FLOAT,
  ...
  FORCEINLINE void SetAttributeType(int nAttrIdx, EAttributeDataType nDataType,
  ...
  void AllocateData(
      int nNCols, int nNRows,
      int nSlices = 1); // actually allocate the memory and set the pointers up
  ...
  void FillAttrWithInterpolatedValues(int nAttr, Vector flValue00,
                                      Vector flValue10, Vector const &flValue01,
