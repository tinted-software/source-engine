...
public:
  ...
  static void ConstructString(
    ...
    ConstructStringVArgsInternal(unicodeOuput, unicodeBufferSizeInBytes,
                                 formatString, numFormatParameters, argList);
  ...
  static void ConstructString(
    ...
    ConstructStringKeyValuesInternal(unicodeOutput, unicodeBufferSizeInBytes,
                                     formatString, localizationVariables);
  }

  // Safe version of Construct String that has the compiler infer the buffer
  // size
  template <size_t maxLenInChars, typename T>
  static void ConstructString_safe(OUT_Z_ARRAY T(&pDest)[maxLenInChars],
                                   const T *formatString,
                                   int numFormatParameters, ...) {
    va_list argList;
    va_start(argList, numFormatParameters);

    ConstructStringVArgsInternal(pDest, maxLenInChars * sizeof(*pDest),
                                 formatString, numFormatParameters, argList);
  ...
  static void ConstructString_safe(OUT_Z_ARRAY T(&pDest)[maxLenInChars],
                                   const T *formatString,
                                   KeyValues *localizationVariables) {
    ConstructStringKeyValuesInternal(pDest, maxLenInChars * sizeof(*pDest),
                                     formatString, localizationVariables);
  }

  // Non-static version to be safe version of the virtual functions that utilize
  // KVP
  template <size_t maxLenInChars, typename T>
  void ConstructString_safe(OUT_Z_ARRAY T(&pDest)[maxLenInChars],
                            const char *formatString,
                            KeyValues *localizationVariables) {
    ConstructString(pDest, maxLenInChars * sizeof(*pDest), formatString,
...
//
//			Independently it isn't very useful, though it can be
//used to sort-of- 			intelligently fill out the correct format string. It's
//designed to be 			used for the arguments of CConstructLocalizedString, which can
//be of 			arbitrary number and type.
//
//			If you pass in a (non-specialized) pointer, the code
//will assume that 			you meant that pointer to be used as a localized string. This
//will 			still fail to compile if some non-string type is passed in, but will
//			handle weird combinations of const/volatile/whatever
//automatically.
// --------------------------------------------------------------------------
...
template <typename T> class CLocalizedStringArgStringImpl {
public:
  enum { kIsValid = true };
...
template <typename T> class CLocalizedStringArgPrintfImpl {
public:
  enum { kIsValid = true };
