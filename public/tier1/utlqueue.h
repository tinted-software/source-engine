...
template <class T, class M = CUtlMemory<T>> class CUtlQueue {
public:
...
inline CUtlQueue<T, M>::CUtlQueue(int growSize, int initSize)
    : m_memory(growSize, initSize), m_head(QUEUE_ITERATOR_INVALID),
      m_tail(QUEUE_ITERATOR_INVALID) {}

template <class T, class M>
inline CUtlQueue<T, M>::CUtlQueue(T *pMemory, int numElements)
    : m_memory(pMemory, numElements), m_head(QUEUE_ITERATOR_INVALID),
      m_tail(QUEUE_ITERATOR_INVALID) {}

template <class T, class M> inline T CUtlQueue<T, M>::RemoveAtHead() {
  T temp;
  RemoveAtHead(temp);
  return temp;
}
...
template <class T, class M> inline T CUtlQueue<T, M>::RemoveAtTail() {
  T temp;
  RemoveAtTail(temp);
  return temp;
}
...
template <class T, class M> inline T const &CUtlQueue<T, M>::Head() const {
  Assert(m_head != QUEUE_ITERATOR_INVALID);
  if (m_head == QUEUE_ITERATOR_INVALID) {
    static T dummy;
    return dummy;
...
template <class T, class M> inline T const &CUtlQueue<T, M>::Tail() const {
  Assert(m_tail != QUEUE_ITERATOR_INVALID);
  if (m_tail == QUEUE_ITERATOR_INVALID) {
    static T dummy;
    return dummy;
...
template <class T, class M> void CUtlQueue<T, M>::Insert(T const &element) {
  if (m_tail == QUEUE_ITERATOR_INVALID) {
    // empty
    ...
    if (nextTail ==
        m_head) // if non-empty, and growing by 1 appears to make the queue of
                // length 1, then we were already full before the Insert
    {
      ...
      nextTail = Next_Unchecked(
          m_tail); // if nextTail was 0, then it now should be nOldAllocCount

      if (m_head != QueueIter_t(0)) {
        // if the queue wraps around the end of m_memory, move the part at the
        // end of memory to the new end of memory
...
template <class T, class M> bool CUtlQueue<T, M>::Check(T const element) const {
...
template <class T, class M> QueueIter_t CUtlQueue<T, M>::First() const {
  return m_head;
}
...
template <class T, class M> QueueIter_t CUtlQueue<T, M>::Last() const {
  return m_tail;
}
...
template <class T, class M> int CUtlQueue<T, M>::Count() const {
  if (m_head == QUEUE_ITERATOR_INVALID) {
    Assert(m_tail == QUEUE_ITERATOR_INVALID);
    return 0;
  }
...
template <class T, class M> bool CUtlQueue<T, M>::IsEmpty() const {
  Assert((m_head == QUEUE_ITERATOR_INVALID) ==
         (m_tail == QUEUE_ITERATOR_INVALID));
  return (m_head == QUEUE_ITERATOR_INVALID);
}

template <class T, class M> void CUtlQueue<T, M>::RemoveAll() {
  m_head = m_tail = QUEUE_ITERATOR_INVALID;
}

template <class T, class M> void CUtlQueue<T, M>::Purge() {
  m_head = m_tail = QUEUE_ITERATOR_INVALID;
  m_memory.Purge();
}
