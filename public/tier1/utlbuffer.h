...
template <> const char *GetFmtStr<short>(int nRadix, bool bPrint) {
  Assert(nRadix == 10);
  return "%hd";
}
template <> const char *GetFmtStr<ushort>(int nRadix, bool bPrint) {
  Assert(nRadix == 10);
  return "%hu";
}
template <> const char *GetFmtStr<int>(int nRadix, bool bPrint) {
  Assert(nRadix == 10);
  return "%d";
}
template <> const char *GetFmtStr<uint>(int nRadix, bool bPrint) {
  Assert(nRadix == 10 || nRadix == 16);
  return nRadix == 16 ? "%x" : "%u";
}
template <> const char *GetFmtStr<int64>(int nRadix, bool bPrint) {
  Assert(nRadix == 10);
  return "%lld";
}
template <> const char *GetFmtStr<float>(int nRadix, bool bPrint) {
  Assert(nRadix == 10);
  return "%f";
}
template <> const char *GetFmtStr<double>(int nRadix, bool bPrint) {
  Assert(nRadix == 10);
  return bPrint ? "%.15lf" : "%lf";
} // force Printf to print DBL_DIG=15 digits of precision for doubles - defaults
  // to FLT_DIG=6
#else
template <> static const char *GetFmtStr<short>(int nRadix, bool bPrint) {
  Assert(nRadix == 10);
  return "%hd";
}
template <> static const char *GetFmtStr<ushort>(int nRadix, bool bPrint) {
  Assert(nRadix == 10);
  return "%hu";
}
template <> static const char *GetFmtStr<int>(int nRadix, bool bPrint) {
  Assert(nRadix == 10);
  return "%d";
}
template <> static const char *GetFmtStr<uint>(int nRadix, bool bPrint) {
  Assert(nRadix == 10 || nRadix == 16);
  return nRadix == 16 ? "%x" : "%u";
}
template <> static const char *GetFmtStr<int64>(int nRadix, bool bPrint) {
  Assert(nRadix == 10);
  return "%lld";
}
template <> static const char *GetFmtStr<float>(int nRadix, bool bPrint) {
  Assert(nRadix == 10);
  return "%f";
}
template <> static const char *GetFmtStr<double>(int nRadix, bool bPrint) {
  Assert(nRadix == 10);
  return bPrint ? "%.15lf" : "%lf";
} // force Printf to print DBL_DIG=15 digits of precision for doubles - defaults
  // to FLT_DIG=6
...
public:
  ...
  enum BufferFlags_t {
    ...
    EXTERNAL_GROWABLE =
        0x2, // This is used w/ external buffers and causes the utlbuf to switch
             // to reallocatable memory if an overflow happens when Putting.
  ...
  // UtlBuffer objects should not be copyable; we do a slow copy if you use this
  // but it asserts. (REI: I'd like to delete these but we have some python
  // bindings that currently rely on being able to copy these objects)
  CUtlBuffer(const CUtlBuffer &);            // = delete;
  CUtlBuffer &operator=(const CUtlBuffer &); // = delete;

#if VALVE_CPP11
  // UtlBuffer is non-copyable (same as CUtlMemory), but it is moveable.  We
  // would like to declare these with '= default' but unfortunately VS2013 isn't
  // fully C++11 compliant, so we have to manually declare these in the
  // boilerplate way.
...
template <> inline void CUtlBuffer::GetTypeBin<double>(double &dest) {
...
template <typename T> inline void CUtlBuffer::GetType(T &dest) {
  if (!IsText()) {
    GetTypeBin(dest);
  } else {
    GetTypeText(dest);
...
#if defined(_GAMECONSOLE)
template <> inline void CUtlBuffer::PutTypeBin<float>(float src) {
...
template <> inline void CUtlBuffer::PutTypeBin<double>(double src) {
  if (CheckPut(sizeof(src))) {
    if (m_Byteswap.IsSwappingBytes()) {
...
template <typename T> inline void CUtlBuffer::PutType(T src) {
  if (!IsText()) {
    PutTypeBin(src);
  } else {
...
inline void CUtlBuffer::PutUnsignedInt64(uint64 i) { PutType(i); }

inline void CUtlBuffer::PutUint64(uint64 i) { PutType(i); }
