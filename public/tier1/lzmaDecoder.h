...
public:
  ...
  // to nMaxOutputBytes to pOutput. Makes progress until blocked on input or
  // output. Returns false if read stops due to an error or if called at EOF
  // (GetExpectedBytesRemaining == 0)
...
private:
  ...
  // Init from a zip-embedded LZMA stream. Requires the original size be passed
  // from zip headers.
  ...
  // If InitZIPHeader() was called. We're expecting a zip-style header and have
  // size information.
