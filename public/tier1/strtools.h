...
inline const char *nexttoken(OUT_Z_ARRAY char (&pToken)[maxLenInChars],
...
inline bool _V_strEndsWithInner(const char *pStr, const char *pSuffix) {
  ...
  if (nSuffixLen == 0)
    return true; // All strings end with the empty string (matches Java & .NET
                 // behaviour)
...
inline bool V_iswdigit(int c) { return (((uint)(c - '0')) < 10); }
...
// gcc insists on only having format annotations on declarations, not
// definitions, which is why I have both. Append formatted text to an array in a
// safe manner -- always null-terminated, truncation rather than buffer overrun.
template <size_t maxLenInChars>
...
int V_sprintfcat_safe(INOUT_Z_ARRAY char (&pDest)[maxLenInChars],
  ...
  // This code is here to check against buffer overruns when uninitialized
  // arrays are passed in. It should never be executed. Unfortunately we can't
  // assert in this header file.
  if (usedLength >= maxLenInChars)
    usedLength = 0;
  int result = V_vsnprintf(pDest + usedLength, maxLenInChars - usedLength,
                           pFormat, params);
  va_end(params);
  return result;
}
...
// Remove the initial characters of ppline if they are whitespace (uses
// V_isspace)
...
inline void V_MakeAbsolutePath(char *pOut, int outLen, const char *pPath,
                               const char *pStartingDir, bool bLowercaseName) {
...
void V_SplitString(
    const char *pString, const char *pSeparator,
    CUtlVector<CUtlString, CUtlMemory<CUtlString, int>> &outStrings,
...
// Encode a string for display as HTML -- this only encodes ' " & < >, which are
// the important ones to encode for security and ensuring HTML display doesn't
// break.  Other special chars like the ? sign and so forth will not be encoded
//
// Returns false if there was not enough room in pDest to encode the entire
// source string, otherwise true
bool V_BasicHtmlEntityEncode(OUT_Z_CAP(nDestSize) char *pDest,
                             const int nDestSize, char const *pIn,
                             const int nInSize,
                             bool bPreserveWhitespace = false);

// Decode a string with htmlentities HTML -- this should handle all special
// chars, not just the ones Q_BasicHtmlEntityEncode uses.
//
// Returns false if there was not enough room in pDest to decode the entire
// source string, otherwise true
bool V_HtmlEntityDecodeToUTF8(OUT_Z_CAP(nDestSize) char *pDest,
                              const int nDestSize, char const *pIn,
                              const int nInSize);
...
bool V_ExtractDomainFromURL(const char *pchURL,
                            OUT_Z_CAP(cchDomain) char *pchDomain,
                            int cchDomain);
