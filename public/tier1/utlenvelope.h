...
template <typename T> class CUtlEnvelope : protected CUtlDataEnvelope {
public:
...
template <> class CUtlEnvelope<const char *> {
public:
  ...
  CUtlEnvelope<const char *> &
  operator=(const CUtlEnvelope<const char *> &from) {
    m_string = from.m_string;
    return *this;
  }
...
template <typename T>
inline CUtlEnvelope<T> &
CUtlEnvelope<T>::operator=(const CUtlEnvelope<T> &from) {
  CUtlDataEnvelope::operator=(from);
  return *this;
}

template <typename T> inline CUtlEnvelope<T>::operator T *() {
  return (T *)CUtlDataEnvelope::operator void *();
}

template <typename T> inline CUtlEnvelope<T>::operator T *() const {
  return (T *)((const_cast<CUtlEnvelope<T> *>(this))->operator T *());
}

template <typename T> inline CUtlEnvelope<T>::operator void *() {
  return CUtlDataEnvelope::operator void *();
}

template <typename T> inline CUtlEnvelope<T>::operator void *() const {
