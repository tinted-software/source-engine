...
template <int sel, typename A, typename B> struct CTypeSelect {
  typedef A type;
};

template <typename A, typename B> struct CTypeSelect<0, A, B> {
  typedef B type;
};
...
template <typename Same> struct CTypeEquals<Same, Same, false, false> {
  enum { value = 1 };
};
...
template <typename K, typename V> class CUtlKeyValuePair {
public:
...
template <typename K> class CUtlKeyValuePair<K, empty_t> {
public:
...
template <typename T> struct HasClassAltArgumentType {
...
template <typename ArgT> struct ArgumentTypeInfoImpl<ArgT, undefined_t> {
  enum { has_alt = 0 };
  typedef ArgT Arg_t;
...
template <typename ArgT> struct ArgumentTypeInfoImpl<ArgT, ArgT> {
  enum { has_alt = 0 };
  typedef ArgT Arg_t;
  typedef undefined_t Alt_t;
};
