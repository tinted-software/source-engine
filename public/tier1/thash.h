...
// #define DBGFLAG_THASH			// Perform extra sanity checks
// on the THash
...
template <class Data, typename I = uint32> class CTHash {
private:
  // RecHdr
...
public:
  ...
  // We provide a scheduling service.  Call StartFrameSchedule when you want to
  // start running records in a given frame, and repeatedly call PvRecordRun
  // until it returns NULL (or you run our of time).
...
private:
  ...
  int m_cFramesPerCycle; // Run each of our records once every m_cFramesPerCycle
                         // frames
  RecHdr_t *m_pRecHdrRunNext; // Next record to run (be careful-- this is more
                              // complicated than it sounds)
  int m_iBucketRunMax;        // Stop running when we get to this bucket
  uint m_iCycleCur;  // Current cycle (ie, how many times we've made a complete
                     // scheduler pass)
...
//-----------------------------------------------------------------------------
// Purpose: Constructor
// Input:	cMicroSecRunInterval -		How often we want the scheduler
// to run each of our records
//-----------------------------------------------------------------------------
template <class Data, class I> CTHash<Data, I>::CTHash(int cFramesPerCycle) {
  m_cBucket = 0;
  m_pBucket = NULL;
  ...
  m_pRecHdrRunNext = &m_RecHdrTail; // This will make us start at the beginning
                                    // on our first frame
  m_iBucketRunMax = 0;
  m_iCycleCur = 0;
  m_iCycleLast = 0;
...
// Purpose: Destructor
//-----------------------------------------------------------------------------
template <class Data, class I> CTHash<Data, I>::~CTHash() {
  RemoveAll();
...
void CTHash<Data, I>::Init(int cRecordInit, int cBucket) {
  ...
  m_pMemoryPoolRecord = new CUtlMemoryPool(
      sizeof(Data) + sizeof(RecHdr_t), cRecordInit, CUtlMemoryPool::GROW_SLOW);
...
//-----------------------------------------------------------------------------
template <class Data, class I> Data *CTHash<Data, I>::PvRecordInsert(I unKey) {
...
// Output:	Pointer to the new record
//-----------------------------------------------------------------------------
template <class Data, class I> Data *CTHash<Data, I>::PvRecordInsertAutoKey() {
  // Find a free record
  RecHdr_t *pRecHdr = (RecHdr_t *)m_pMemoryPoolRecord->Alloc();
...
//			unOldKey -		old key (not strictly needed,
//but helpful consistency check) 			unNewKey -		new key to use
//-----------------------------------------------------------------------------
template <class Data, class I>
...
//-----------------------------------------------------------------------------
template <class Data, class I> void CTHash<Data, I>::Remove(I unKey) {
...
//-----------------------------------------------------------------------------
template <class Data, class I> void CTHash<Data, I>::Remove(Data *pvRemove) {
...
//-----------------------------------------------------------------------------
template <class Data, class I> void CTHash<Data, I>::RemoveAll() {
...
//-----------------------------------------------------------------------------
template <class Data, class I> Data *CTHash<Data, I>::PvRecordFirst() const {
...
//			Records are run at a particular time dependent on their
//hash bucket, 			regardless of when they were last run.
// Input:	bNewFrame -		True if this is a new frame.  If false,
// we've run
//							off the end of the list
//and are checking whether 							we need to keep going at the beginning.
//-----------------------------------------------------------------------------
template <class Data, class I>
...
// done)
//-----------------------------------------------------------------------------
template <class Data, class I> Data *CTHash<Data, I>::PvRecordRun() {
  ...
  for (;;) {
    ...
    if (0 != m_pRecHdrRunNext->m_iCycleLast) {
      if (m_pRecHdrRunNext->m_iCycleLast == m_iCycleCur) {
        DMsg(SPEW_CONSOLE, 1,
             "Double cycle: hdr = 0x%x, last frame = %d, curFrame = %d, first "
             "= %d, last = %d, bucket = %d\n",
        ...
        DMsg(SPEW_CONSOLE, 1,
             "Skipped cycle: hdr = 0x%x, cycleLast = %u, cycleCur = %u (missed "
             "%u cycles)\n",
...
//-----------------------------------------------------------------------------
template <class Data, class I> bool CTHash<Data, I>::BCompletedPass() {
...
//-----------------------------------------------------------------------------
template <class Data, class I> int CTHash<Data, I>::IBucket(I unKey) const {
