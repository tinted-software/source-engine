...
public:
  ...
  const char *
  ArgS() const; // All args that occur after the 0th arg, in string form
...
private:
  ...
  void Create(const char *pName, const char *pDefaultValue, int flags = 0,
              ...
              bool bMax = false, float fMax = 0.0, bool bCompMin = false,
...
FORCEINLINE_CVAR bool ConVar::IsCompetitiveRestricted() const {
  ...
  const bool bHasCompSettings =
      m_pParent->m_bHasCompMin || m_pParent->m_bHasCompMax;
  const bool bClientCanAdjust =
      (nFlags & (FCVAR_ARCHIVE | FCVAR_ALLOWED_IN_COMPETITIVE)) != 0;
  const bool bInternalUseOnly =
      (nFlags & (FCVAR_HIDDEN | FCVAR_DEVELOPMENTONLY | FCVAR_INTERNAL_USE |
                 FCVAR_GAMEDLL | FCVAR_REPLICATED | FCVAR_CHEAT)) != 0;
