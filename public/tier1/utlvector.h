...
#define FOR_EACH_VEC(vecName, iteratorName)                                    \
  for (int iteratorName = 0;                                                   \
       (vecName).IsUtlVector && iteratorName < (vecName).Count();              \
       iteratorName++)
#define FOR_EACH_VEC_BACK(vecName, iteratorName)                               \
  for (int iteratorName = (vecName).Count() - 1;                               \
       (vecName).IsUtlVector && iteratorName >= 0; iteratorName--)
...
public:
  ...
  // Set the growth policy and initial capacity. Count will always be zero. This
  // is different from std::vector where the constructor sets count as well as
  // capacity. growSize of zero implies the default growth pattern which is
  // exponential.
  explicit CUtlVector(int growSize = 0, int initialCapacity = 0);

  // Initialize with separately allocated buffer, setting the capacity and
  // count. The container will not be growable.
  ...
  template <typename F> int FindPredicate(F &&predicate) const;
  ...
  // elements Use NumAllocated() to retrieve the current capacity.
  ...
  /// sort using std:: with a predicate. e.g. [] -> bool ( T &a, T &b ) { return
  /// a < b; }
...
#pragma warning(disable : 4815) // warning C4815: 'staticData' : zero-sized
                                // array in stack object will have no elements
#endif
...
// Count
//-----------------------------------------------------------------------------
template <typename T, class A> inline int CUtlVector<T, A>::Size() const {
  return m_Size;
}

template <typename T, class A> inline T &CUtlVector<T, A>::Random() {
  Assert(m_Size > 0);
...
void CUtlVector<T, A>::Shuffle(IUniformRandomStream *pSteam) {
  for (int i = 0; i < m_Size; i++) {
    int j =
        pSteam ? pSteam->RandomInt(i, m_Size - 1) : RandomInt(i, m_Size - 1);
    if (i != j) {
...
//-----------------------------------------------------------------------------
template <typename T, class A> void CUtlVector<T, A>::Reverse() {
  for (int i = 0; i < m_Size / 2; i++) {
    V_swap(m_Memory[i], m_Memory[m_Size - 1 - i]);
#if defined(UTLVECTOR_TRACK_STACKS)
    if (bTrackingEnabled) {
      V_swap(m_pElementStackStatsIndices[i],
             m_pElementStackStatsIndices[m_Size - 1 - i]);
    }
#endif
  }
}
...
template <typename T, class A> void CUtlVector<T, A>::Sort(void) {
  // STACK STATS TODO: Do we care about allocation tracking precision enough to
  // match element origins across a sort?
...
template <typename T, class A> inline T *CUtlVector<T, A>::AddToTailGetPtr() {
...
public:
  ~CUtlStringList(void) { PurgeAndDeleteElementsArray(); }

  void
  CopyAndAddToTail(char const *pString) // clone the string and add to the end
  {
  ...
  CUtlStringList(char const *pString, const char **pSeparators,
                 int nSeparators) {
  ...
  void SplitString2(char const *pString, const char **pSeparators,
                    int nSeparators) {
...
private:
  CUtlStringList(const CUtlStringList
                     &other); // copying directly will cause double-release of
                              // the same strings; maybe we need to do a deep
                              // copy, but unless and until such need arises,
                              // this will guard against double-release
};
...
private:
  char
      *m_szBuffer; // a copy of original string, with '\0' instead of separators
};
