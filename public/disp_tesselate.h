...
inline void TesselateDisplacement_R(T *pHelper, const CVertIndex &nodeIndex,
  ...
  if (iLevel >= pHelper->m_pPowerInfo->m_Power - 1) {
    ...
    for (int iChild = 0; iChild < 4; iChild++) {
      CVertIndex const &childNode =
          pHelper->m_pPowerInfo->m_pChildVerts[iNodeIndex].m_Verts[iChild];

      // Make sure we really can tesselate here (a smaller neighbor displacement
      // could have inactivated certain edge verts.
      int iVertBit = InternalVertIndex(pHelper->m_pPowerInfo, childNode);
      bActiveChildren[iChild] =
          (pHelper->m_pActiveVerts[iVertBit >> 5] & (1 << (iVertBit & 31)));

      if (bActiveChildren[iChild]) {
        TesselateDisplacement_R(pHelper, childNode, iChildNodeBit, iLevel + 1);
      } else {
        // Make sure the triangle counts are cleared on this one because it may
        // visit this node in GenerateDecalFragments_R if nodeInfo's
        // CHILDREN_HAVE_TRIANGLES flag is set.
  ...
  // Now that we've tesselated, figure out how many indices we've added at this
  // node
...
public:
  ...
  // void EndTriangle();
  // // (the 3 indices are in m_TempIndices). DispNodeInfo_t& GetNodeInfo( int
  // iNodeBit );
