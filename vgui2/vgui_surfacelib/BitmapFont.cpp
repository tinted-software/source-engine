...
bool CBitmapFont::Create(const char *pFontFilename, float scalex, float scaley,
  ...
  if (!pFontTable) {
    ...
    if (IsX360()) {
      ...
      swap.SwapFieldsToTargetEndian(
          (BitmapGlyph_t *)((char *)pBuf + sizeof(BitmapFont_t)),
...
void CBitmapFont::GetCharRGBA(wchar_t ch, int rgbaWide, int rgbaTall,
                              unsigned char *prgba) {
  // CBitmapFont derives off CLinuxFont, etc. But you should never call
  // GetCharRGBA on a bitmap font. If we let this fall into the CLinuxFont code,
  // we'd have a difficult to track down bug - so crash
  //	hard here...
  Error("GetCharRGBA called on CBitmapFont.");
}

void CBitmapFont::GetKernedCharWidth(wchar_t ch, wchar_t chBefore,
...
DEFINE_FIELD(x, FIELD_SHORT), DEFINE_FIELD(y, FIELD_SHORT),
    ...
    END_BYTESWAP_DATADESC()
        ...
        BEGIN_BYTESWAP_DATADESC(BitmapFont_t) DEFINE_FIELD(m_id, FIELD_INTEGER),
