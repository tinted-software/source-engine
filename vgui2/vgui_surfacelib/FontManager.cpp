...
bool CFontManager::SetFontGlyphSet(HFont font, const char *windowsFontName,
  ...
  return SetFontGlyphSet(font, windowsFontName, tall, weight, blur, scanlines,
                         flags, 0, 0);
}
...
bool CFontManager::SetFontGlyphSet(HFont font, const char *windowsFontName,
  ...
  if (IsX360()) {
    // discovered xbox only allows glyphs from these languages from the foreign
    // fallback font prefer to have the entire range of chars from the font so
    // UI doesn't suffer from glyph disparity
  ...
  font_t *winFont = CreateOrFindWin32Font(windowsFontName, tall, weight, blur,
                                          scanlines, flags);

  // cycle until valid english/extended font support has been created
  do {
    // add to the amalgam
    if (bForceSingleFontForXbox ||
        IsFontForeignLanguageCapable(windowsFontName)) {
      if (winFont) {
      ...
      font_t *pExtendedFont = CreateOrFindWin32Font(
          localizedFontName, tall, weight, blur, scanlines, flags);
      if (winFont && pExtendedFont) {
        // use the normal font for english characters, and the extended font for
        // the rest
...
//-----------------------------------------------------------------------------
font_t *CFontManager::CreateOrFindWin32Font(const char *windowsFontName,
                                            int tall, int weight, int blur,
                                            int scanlines, int flags) {
  ...
  if (!winFont) {
    ...
    if (!pchFontData) {
      // If we didn't find the font data in the font cache, then get the font
      // filename...
    ...
    if (pchFontData) {
      ...
      if (m_Win32Fonts[i]->CreateFromMemory(windowsFontName, pchFontData,
                                            memSize, tall, weight, blur,
                                            scanlines, flags)) {
        // add to the list
        winFont = m_Win32Fonts[i];
      }
...
static const char *g_szValidAsianFonts[] = {"Marlett", "WenQuanYi Zen Hei",
                                            "unifont", NULL};
...
void CFontManager::GetKernedCharWidth(vgui::HFont font, wchar_t ch,
                                      wchar_t chBefore, wchar_t chAfter,
                                      float &wide, float &flabcA,
                                      float &flabcC) {
  wide = 0.0f;
  flabcA = 0.0f;
