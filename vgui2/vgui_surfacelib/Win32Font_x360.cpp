...
//
// Purpose: Xbox 360 support for TrueType Fonts. The only cuurent solution is to
// use XUI to mount the TTF, and rasterize glyph into a render target. XUI does
// not support rasterization directly to a system memory region.
//
//=====================================================================================//
...
// Purpose: Destructor
//-----------------------------------------------------------------------------
CWin32Font::~CWin32Font() { CloseResource(); }
...
bool CWin32Font::Create(const char *windowsFontName, int tall, int weight,
  ...
  if (!FontManager().GetCachedXUIMetrics(windowsFontName, tall, style,
                                         &fontMetrics, charMetrics)) {
    m_hFont = FontManager().MaterialSystem()->OpenTrueTypeFont(windowsFontName,
                                                               tall, style);
    if (!m_hFont) {
      return false;
    }
    ...
    FontManager().MaterialSystem()->GetTrueTypeFontMetrics(
        m_hFont, &fontMetrics, charMetrics);
    FontManager().SetCachedXUIMetrics(windowsFontName, tall, style,
                                      &fontMetrics, charMetrics);
  }
  ...
  for (int i = 1; i < ABCWIDTHS_CACHE_SIZE; i++) {
    ...
    a = charMetrics[i].fMinX -
        1; // Add one column of padding to make up for font rendering blurring
           // into left column (and adjust in b)
    b = charMetrics[i].fMaxX - charMetrics[i].fMinX + 1;
    c = charMetrics[i].fAdvance -
        charMetrics[i].fMaxX; // NOTE: We probably should add a column here, but
                              // it's rarely needed in our current fonts so
                              // we're opting to save memory instead
    ...
    m_ABCWidthsCache[i].b =
        b + ((m_iBlur + m_iOutlineSize) * 2) + m_iDropShadowOffset;
...
//-----------------------------------------------------------------------------
// Purpose: generates texture data (written into appropriate font page subrects)
// for multiple chars
//-----------------------------------------------------------------------------
void CWin32Font::GetCharsRGBA(newChar_t *newChars, int numNewChars,
                              unsigned char *pRGBA) {
  if (!m_hFont) {
    // demand request for font glyph, re-create font
    int style = GetStyleFromParameters(m_iFlags, m_iWeight);
    m_hFont = FontManager().MaterialSystem()->OpenTrueTypeFont(GetName(),
                                                               m_iTall, style);
  }
  ...
  if (!FontManager().MaterialSystem()->GetTrueTypeGlyphs(
          m_hFont, numNewChars, pWch, pOffsetX, pOffsetY, pWidth, pHeight,
          pRGBA, pRGBAOffset)) {
    // failure
    return;
  }
  ...
  for (int i = 0; i < numNewChars; i++) {
    ...
    ApplyDropShadowToTexture(newChars[i].fontWide, newChars[i].fontTall,
                             pCharRGBA, m_iDropShadowOffset);
    ApplyOutlineToTexture(newChars[i].fontWide, newChars[i].fontTall, pCharRGBA,
                          m_iOutlineSize);
    ApplyGaussianBlurToTexture(newChars[i].fontWide, newChars[i].fontTall,
                               pCharRGBA, m_iBlur);
    ApplyScanlineEffectToTexture(newChars[i].fontWide, newChars[i].fontTall,
                                 pCharRGBA, m_iScanLines);
    ApplyRotaryEffectToTexture(newChars[i].fontWide, newChars[i].fontTall,
                               pCharRGBA, m_bRotary);
  }
}
...
bool CWin32Font::IsEqualTo(const char *windowsFontName, int tall, int weight,
                           int blur, int scanlines, int flags) {
  // do an true comparison that accounts for non-supported behaviors that gets
  // remapped avoids creating fonts that are graphically equivalent, though
  // specified differently
  if (!stricmp(windowsFontName, m_szName.String()) && m_iTall == tall &&
      m_iBlur == blur && m_iScanLines == scanlines) {
    // only these flags affect the font glyphs
    int validFlags =
        vgui::ISurface::FONTFLAG_DROPSHADOW | vgui::ISurface::FONTFLAG_OUTLINE |
    ...
    if ((m_iFlags & validFlags) == (flags & validFlags)) {
      if (GetStyleFromParameters(m_iFlags, m_iWeight) ==
          GetStyleFromParameters(flags, weight)) {
...
void CWin32Font::GetCharABCWidths(int ch, int &a, int &b, int &c) {
  ...
  if (ch < ABCWIDTHS_CACHE_SIZE) {
    ...
    // cannot support getting character metrics outside of the font
    // initialization
...
void CWin32Font::CloseResource() {
  ...
  // save memory and don't hold font open, re-open if glyph actually requested
  // used during draw
...
//-----------------------------------------------------------------------------
void CWin32Font::GetKernedCharWidth(wchar_t ch, wchar_t chBefore,
