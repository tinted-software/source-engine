...
unsigned char CalculatePixelBlur(const unsigned char *src, int nStride,
                                 const float *distribution, int nValues) {
  float accum = 0.0;
...
void ApplyGaussianBlurToTexture(int rgbaWide, int rgbaTall, unsigned char *rgba,
  ...
  float *pGaussianDistribution =
      (float *)stackalloc((nBlur * 2 + 1) * sizeof(float));
  ...
  for (int x = 0; x < rgbaWide; x++) {
    ...
    for (int y = 0; y < rgbaTall; y++) {
      const unsigned char *read_from =
          src + (y * rgbaWide + x + nOffset - nBlur) * 4 + 3;
  ...
  for (int y = 0; y < rgbaTall; y++) {
    ...
    for (int x = 0; x < rgbaWide; x++) {
      const unsigned char *read_from =
          src + ((y + nOffset - nBlur) * rgbaWide + x) * 4 + 3;
      unsigned char *dst = rgba + (y * rgbaWide + x) * 4;
      unsigned char alpha =
          CalculatePixelBlur(read_from, 4 * rgbaWide, dist, nValues);
      dst[0] = dst[1] = dst[2] = alpha > 0 ? 255 : 0;
      dst[3] = alpha;
    }
  }
