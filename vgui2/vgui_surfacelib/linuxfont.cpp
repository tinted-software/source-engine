...
// Freetype uses a lot of fixed float values that are 26.6 splits of a 32 bit
// word. to make it an int, shift down the 6 bits and round up if the high bit
// of the 6 bits was set.
...
//-----------------------------------------------------------------------------
// Purpose: build a map of friendly (char *) name to crazy ATSU bytestream, so
// we can ask for "Tahoma" and actually load it
//-----------------------------------------------------------------------------
void CLinuxFont::CreateFontList() {
  ...
  for (i = 0; i < fontset->nfont; i++) {
    ...
    if (FcPatternGetBool(fontset->fonts[i], FC_SCALABLE, 0, &scalable) ==
            FcResultMatch &&
        !scalable)
      continue;
...
bool CLinuxFont::CreateFromMemory(const char *windowsFontName, void *data,
                                  int datasize, int tall, int weight, int blur,
  ...
  Assert(!m_faceValid);
  FT_Error error = FT_New_Memory_Face(FontManager().GetFontLibraryHandle(),
                                      (FT_Byte *)data, datasize, 0, &m_face);
  if (error) {
  ...
  // Loop through until we get a height that is less than or equal to the
  // requested height.
  //  We tried using the BBOX ascender / descender, but it was overly large
  //  compared to Windows. We also tried using the size metrics, but the
  //  ascender wasn't high enough and accents were cut off. Descender from size
  //  metrics was too low for fonts with no lower case characters.
  // Compromise: Use ascent from O' and descent from bbox. Diffs on textures
  // indicate this is best choice.
  //  Used these command lines vars and convars to help beyond compare linux and
  //  windows:
  //    -precachefontintlchars / -enable_font_bounding_boxes / vgui_spew_fonts /
  ...
  for (;;) {
    ...
    if (error) {
      ...
      // If FT_Set_Pixel_Sizes fails, it should be because we've got a
      // fixed-size font.
      ...
      if (m_face->num_fixed_sizes) {
        ...
        // Loop through all the other available sizes and find the closest
        // match.
    ...
    if (SetPixelSizesFailed) {
      ...
      // than the bitmap_top for the character.  This makes GetCharRGBA() chop
      // off the top of the O and the accent is skipped.  Complete hack here,
      // but we check for the tallest character we know about (O') and bump up
      // the ascender value if it is greater than what we've currently got.
      wchar_t ch = 0xd3;
      ...
      if (!error) {
        ...
        if (!error) {
          ...
          if (ascenderTop > ascender) {
            ...
            ascender =
                FT_MulFix(m_face->bbox.yMax, m_face->size->metrics.y_scale);
          }
        }
      }
    }
    ...
    const int fxpHeight = ascender + -descender +
                          INT_2FIXED6(m_iDropShadowOffset + 2 * m_iOutlineSize);
    m_iHeight = FIXED6_2INT(fxpHeight);

    // If we're exact or we got too small, bail.
    if (SetPixelSizesFailed || (m_iHeight == m_iTall) || (m_iHeight < 7) ||
        (m_iHeightRequested <= 1))
      break;
...
char *TryFindFont(const char *winFontName, bool bBold, int italic) {
  ...
  if (strcmp(winFontName, "Courier New") == 0) {
    ...
    snprintf(fontFile, sizeof fontFile, "%s/files/%s", getenv("APP_DATA_PATH"),
             fontName);
    return fontFile;
  }

  if (strcmp(lang, "japanese") == 0 || strcmp(lang, "koreana") == 0 ||
      strcmp(lang, "korean") == 0 || strcmp(lang, "tchinese") == 0 ||
  ...
  if (fontNamePost)
    snprintf(fontFile, sizeof fontFile, "%s/files/%s-%s.ttf",
             getenv("APP_DATA_PATH"), fontName, fontNamePost);
  else
    snprintf(fontFile, sizeof fontFile, "%s/files/%s.ttf",
             getenv("APP_DATA_PATH"), fontName);
  ...
  if (strcmp(winFontName, "Courier New") == 0) {
    strncpy(fontFile,
            "platform/resource/linux_fonts/liberationmono-regular.ttf",
            sizeof(fontFile));
    return fontFile;
  }
  ...
  if (fontNamePost)
    snprintf(fontFile, sizeof fontFile,
             "platform/resource/linux_fonts/%s-%s.ttf", fontName, fontNamePost);
  else
    snprintf(fontFile, sizeof fontFile, "platform/resource/linux_fonts/%s.ttf",
             fontName);
...
//-----------------------------------------------------------------------------
// Purpose: Given a font name from windows, match it to the filename and return
// that.
//-----------------------------------------------------------------------------
char *CLinuxFont::GetFontFileName(const char *windowsFontName, int flags) {
  ...
  else if (!Q_stricmp(pchFontName, "Arial Black") ||
           Q_stristr(pchFontName, "bold"))
    bBold = true;

#if !HAVE_FC
  char *filename = TryFindFont(windowsFontName, bBold,
...
#else
  const int italic = (flags & vgui::ISurface::FONTFLAG_ITALIC) ? FC_SLANT_ITALIC
                                                               : FC_SLANT_ROMAN;

  const int nFcWeight = bBold ? FC_WEIGHT_BOLD : FC_WEIGHT_NORMAL;
  FcPattern *match =
      FontMatch(FC_FAMILY, FcTypeString, pchFontName, FC_WEIGHT, FcTypeInteger,
                nFcWeight, FC_SLANT, FcTypeInteger, italic, NULL);
  if (!match) {
...
//-----------------------------------------------------------------------------
void CLinuxFont::GetCharRGBA(wchar_t ch, int rgbaWide, int rgbaTall,
                             unsigned char *prgba) {
  ...
  FT_Error error =
      FT_Load_Char(m_face, ch, FT_LOAD_RENDER | FT_LOAD_TARGET_NORMAL);
  if (error) {
  ...
  if (error == 0) {
    ...
    for (int y = yBitmapStart; y < MIN(bitmap.rows, rgbaTall - nSkipRows);
         y++) {
      for (int x = 0; x < Width; x++) {
        int rgbaOffset = 4 * (x + m_iBlur); // +(rgbaTall-y-1)*rgbaWide*4
        uint32 alpha =
            Min(255U, alpha_scale * bitmap.buffer[x + y * bitmap.pitch]);
...
void CLinuxFont::GetKernedCharWidth(wchar_t ch, wchar_t chBefore,
  ...
  int error =
      FT_Load_Glyph(m_face, glyph_index, FT_LOAD_DEFAULT | FT_LOAD_FLAGS);
  if (error) {
...
void CLinuxFont::GetCharABCWidths(int ch, int &a, int &b, int &c) {
  ...
  finder.abc.b = metrics.width / 64 + ((m_iBlur + m_iOutlineSize) * 2) +
                 m_iDropShadowOffset;
  finder.abc.c =
      (metrics.horiAdvance - metrics.horiBearingX - metrics.width) / 64 -
      m_iBlur - m_iDropShadowOffset - m_iOutlineSize;
...
//-----------------------------------------------------------------------------
bool CLinuxFont::IsEqualTo(const char *windowsFontName, int tall, int weight,
...
// resizing code
//-----------------------------------------------------------------------------
int CLinuxFont::GetFlags() { return m_iFlags; }

//-----------------------------------------------------------------------------
// Purpose: Comparison function for abc widths storage
//-----------------------------------------------------------------------------
bool CLinuxFont::ExtendedABCWidthsCacheLessFunc(const abc_cache_t &lhs,
...
// Purpose: Comparison function for abc widths storage
//-----------------------------------------------------------------------------
bool CLinuxFont::ExtendedKernedABCWidthsCacheLessFunc(
