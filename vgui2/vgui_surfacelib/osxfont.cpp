...
// 94: HFont:0x000000b9, TFTypeDeath, TFTypeDeathClientScheme-no, font:tfd,
// tall:27(28) 95: HFont:0x000000ba, TFTypeDeath, TFTypeDeathClientScheme-p,
// font:tfd, tall:55(59)
...
COSXFont::COSXFont()
    : m_ExtendedABCWidthsCache(256, 0, &ExtendedABCWidthsCacheLessFunc),
      m_ExtendedKernedABCWidthsCache(256, 0,
                                     &ExtendedKernedABCWidthsCacheLessFunc) {
  m_iTall = 0;
  m_iWeight = 0;
  m_iFlags = 0;
...
bool COSXFont::CreateStyle(float flFontSize, bool bBold) {
  ...
  Fixed fontsize = FloatToFixed(flFontSize); // font size is based on 72dpi and
                                             // not pixels, so convert to pixels
  Boolean isItalic = m_iFlags & vgui::ISurface::FONTFLAG_ITALIC;
  ATSStyleRenderingOptions renderOpt = kATSStyleNoOptions;

  renderOpt |=
      (m_bAntiAliased ? kATSStyleApplyAntiAliasing : kATSStyleNoAntiAliasing);
  ...
  const ATSUAttributeValuePtr styleValues[] = {
      &m_ATSUFont, &fontsize,     &isItalic, &isBold,
      &renderOpt,  &isUnderlined, &color,
  };
  ...
  err = ATSUSetAttributes(m_ATSUStyle, Q_ARRAYSIZE(styleTags), styleTags,
...
bool COSXFont::CreateTextLayout() {
  ...
  err = ATSUSetLayoutControls(m_ATSUTextLayout, Q_ARRAYSIZE(theTags), theTags,
                              theSizes, theValues);
  if (err != noErr) {
    CHECK_ATSU_ERR(err);
    return false;
...
// in the OS.
//-----------------------------------------------------------------------------
bool COSXFont::Create(const char *windowsFontName, int tall, int weight,
  ...
  CFStringRef fontName = CFStringCreateWithCString(
      kCFAllocatorDefault, windowsFontName, kCFStringEncodingUTF8);
  if (fontName) {
    m_ATSUFont =
        ATSFontFindFromPostScriptName(fontName, kATSOptionFlagsDefault);
    CFRelease(fontName);
  }
  ...
  OSStatus err = ATSFontGetHorizontalMetrics(m_ATSUFont, kATSOptionFlagsDefault,
                                             &aMetrics);
  if (err != noErr) {
    CHECK_ATSU_ERR(err);
    return false;
  ...
  bool bBold = (!Q_stricmp(windowsFontName, "Arial Black") ||
                Q_stristr(windowsFontName, "bold"));
  float flFontSize = ((float)(m_iTall + metricTweaks.m_sizeAdjust) /
                      (aMetrics.ascent - aMetrics.descent + aMetrics.leading));

  m_iAscent = ceil((aMetrics.ascent /
                    (aMetrics.ascent - aMetrics.descent + aMetrics.leading)) *
                   (m_iTall + metricTweaks.m_sizeAdjust) *
                   (metricTweaks.m_ascentMultiplier));
  ...
  m_iMaxCharWidth =
      (metricTweaks.m_leadingMultiplier * aMetrics.maxAdvanceWidth * m_iTall) +
      0.5f;
  ...
  m_ContextRef = CGBitmapContextCreate(
      m_pContextMemory, m_iMaxCharWidth, m_iHeight, bitsPerComponent,
  ...
  if (m_iBlur > 1) {
    ...
    for (int x = 0; x <= (m_iBlur * 2); x++) {
      ...
      m_pGaussianDistribution[x] =
          (float)(1.0f / sqrt(2 * 3.14 * sigma * sigma)) *
...
void COSXFont::GetKernedCharWidth(wchar_t ch, wchar_t chBefore, wchar_t chAfter,
                                  float &wide, float &abcA) {
  ...
  CFCharacterSetRef badCharSetRef =
  ...
  CFStringRef convertedKey = CFStringCreateWithBytes(
      kCFAllocatorDefault, (const UInt8 *)wchString, i * sizeof(wchar_t),
  ...
  CFStringGetBytes(convertedKey, CFRangeMake(0, (int)i),
                   kCFStringEncodingUnicode, 0, false, (UInt8 *)chUTF16Text,
                   Q_ARRAYSIZE(chUTF16Text), &usedBufLen);
  CFRelease(convertedKey);

  OSStatus err = ATSUSetTextPointerLocation(
      m_ATSUTextLayout, (ConstUniCharArrayPtr)chUTF16Text,
  ...
  err = ATSUDirectGetLayoutDataArrayPtrFromTextLayout(
      m_ATSUTextLayout, 0, kATSUDirectDataLayoutRecordATSLayoutRecordCurrent,
      (void **)&layoutRecords, &glyphCount);
  CHECK_ATSU_ERR(err);
  if (err == noErr) {
    ATSGlyphIdealMetrics ScreenMetricts[Q_ARRAYSIZE(wchString)];
    err = ATSUGlyphGetIdealMetrics(m_ATSUStyle, i, &layoutRecords[0].glyphID,
                                   sizeof(ATSLayoutRecord), ScreenMetricts);
    CHECK_ATSU_ERR(err);
    if (err == noErr) {
      wide = ScreenMetricts[iTarget].advance.x;
      abcA = ScreenMetricts[iTarget].sideBearing.x - (float)m_iBlur -
             (float)m_iOutlineSize;
    }
  ...
  if ((wide < 0.001f && abcA < 0.001f) || (ch == chAfter)) {
    // For some fonts the kerning engine has an issue and considers the second
    // character in a pair (i.e the ee in bleed) to be zero width and abcA and
    // the first char to be double width, so in that case fall back to simple 1
    // char metrics.
    int a, b, c;
...
//-----------------------------------------------------------------------------
void COSXFont::GetCharRGBA(wchar_t ch, int rgbaWide, int rgbaTall,
                           unsigned char *rgba) {
  if (!m_ContextRef) {
  ...
  OSStatus err = ATSUSetTextPointerLocation(m_ATSUTextLayout, buffer,
                                            kATSUFromTextBeginning,
                                            kATSUToTextEnd, runLength);
  if (err != noErr) {
    CHECK_ATSU_ERR(err);
    return;
  }
  ...
  // alpha channel since it's going to bitblt in the overlay.  But osx when it
  // renders to a full alpha channel likes to 'antialias' the alpha as well as
  // the color blending. Turning off antialiasing doesn't just make the
  // characters jagged, but when a font is 1 pixel wide it ends up alpha
  // blending into the background.  By rendering it more than once we dominate
  // this alpha.  Looking at worst case, 3 was the magic number. Messing with
  // the anti-aliasing settings on the bitmap, or the alpha config (including
  // alpha-skip-last) caused no change, or complete disabling of antialiasing.
  // Glad these are cached.
  if (m_iHeight < 20) {
    err = ATSUDrawText(m_ATSUTextLayout, kATSUFromTextBeginning, kATSUToTextEnd,
                       Long2Fix(0), Long2Fix(m_iHeight - m_iAscent));
    CHECK_ATSU_ERR(err);
  }
...
void COSXFont::GetCharABCWidths(int ch, int &a, int &b, int &c) {
  ...
  if (err != noErr) {
    // ATSUGlyphGetScreenMetrics fails when font matching happens. When this
    // occurs,
    //  grab the full bounding box for the character and try to fake up some abc
    //  values.
    ATSUTextMeasurement fTextBefore, fTextAfter, fAscent, fDescent;
    err = ATSUGetUnjustifiedBounds(m_ATSUTextLayout, kATSUFromTextBeginning,
                                   kATSUToTextEnd, &fTextBefore, &fTextAfter,
                                   &fAscent, &fDescent);
  ...
  finder.abc.b = gm.deviceAdvance.x + (((float)m_iBlur + m_iOutlineSize) * 2) +
                 m_iDropShadowOffset;
  finder.abc.c = gm.otherSideBearing.x - (float)m_iBlur - m_iDropShadowOffset -
                 m_iOutlineSize;
...
//-----------------------------------------------------------------------------
bool COSXFont::IsEqualTo(const char *windowsFontName, int tall, int weight,
                         int blur, int scanlines, int flags) {
  if (!Q_stricmp(windowsFontName, m_szName.String()) && (m_iTall == tall) &&
...
//-----------------------------------------------------------------------------
bool COSXFont::ExtendedABCWidthsCacheLessFunc(const abc_cache_t &lhs,
...
// Purpose: Comparison function for abc widths storage
//-----------------------------------------------------------------------------
bool COSXFont::ExtendedKernedABCWidthsCacheLessFunc(
    const kerned_abc_cache_t &lhs, const kerned_abc_cache_t &rhs) {
  return (lhs.wch < rhs.wch) ||
         (lhs.wch == rhs.wch && lhs.wchBefore < rhs.wchBefore) ||
         (lhs.wch == rhs.wch && lhs.wchBefore == rhs.wchBefore &&
          lhs.wchAfter < rhs.wchAfter);
}

ATSUStyle *COSXFont::SetAsActiveFont(CGContextRef cgContext) {
  CGContextSelectFont(cgContext, m_szName.String(), m_iHeight,
                      kCGEncodingMacRoman);
  return &m_ATSUStyle;
}
...
void COSXFont::Validate(CValidator &validator, char *pchName) {
  ...
  m_ExtendedKernedABCWidthsCache.Validate(validator,
                                          "m_ExtendedKernedABCWidthsCache");
