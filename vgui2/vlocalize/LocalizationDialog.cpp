...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CLocalizationDialog::CLocalizationDialog(const char *fileName)
    : Frame(NULL, "LocalizationDialog") {
  m_iCurrentToken = -1;
  ...
  if (!g_pVGuiLocalize->AddFile(m_szFileName)) {
    MessageBox *msg =
        new MessageBox("Fatal error", "couldn't load specified file");
  ...
  while (idx != INVALID_LOCALIZE_STRING_INDEX) {
    ...
    m_pTokenList->AddItem(
        new KeyValues("LString", "Token", g_pVGuiLocalize->GetNameByIndex(idx)),
        idx, false, false);
...
void CLocalizationDialog::OnApplyChanges() {
  ...
  g_pVGuiLocalize->ConvertANSIToUnicode(
      buf1, unicodeString, sizeof(unicodeString) / sizeof(wchar_t));
...
void CLocalizationDialog::OnFileSave() {
  if (g_pVGuiLocalize->SaveToFile(m_szFileName)) {
    // success
    MessageBox *box = new MessageBox("Save Successful - VLocalize",
                                     "File was successfully saved.", false);
    box->DoModal();
  } else {
    // failure
    MessageBox *box =
        new MessageBox("Error during save - VLocalize",
                       "Error - File was not successfully saved.", false);
    box->DoModal();
  }
}
...
void CLocalizationDialog::OnTokenCreated(const char *tokenName) {
  ...
  int itemId = m_pTokenList->AddItem(
      new KeyValues("LString", "Token", g_pVGuiLocalize->GetNameByIndex(idx)),
      idx, true, true);
...
MessageMapItem_t CLocalizationDialog::m_MessageMap[] = {
    MAP_MESSAGE(CLocalizationDialog, "RowSelected",
                OnTokenSelected), // message from the m_pTokenList
    MAP_MESSAGE(CLocalizationDialog, "TextChanged",
                OnTextChanged), // message from the text entry
    MAP_MESSAGE(CLocalizationDialog, "ApplyChanges",
                OnApplyChanges), // message from the text entry
    MAP_MESSAGE(CLocalizationDialog, "FileSave", OnFileSave),
    MAP_MESSAGE(CLocalizationDialog, "FileOpen", OnFileOpen),
    MAP_MESSAGE_CONSTCHARPTR(CLocalizationDialog, "TokenCreated",
                             OnTokenCreated, "name"),
};
