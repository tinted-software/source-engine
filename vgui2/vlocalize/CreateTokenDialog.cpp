...
void CCreateTokenDialog::OnOK() {
  ...
  if (Q_strlen(tokenName) < 4) {
    MessageBox *box = new MessageBox("Create Token Error",
                                     ...
                                     "need to be at least 4 characters long.");
    box->DoModal();
  } else {
    // create the token
    wchar_t unicodeString[1024];
    g_pVGuiLocalize->ConvertANSIToUnicode(
        tokenValue, unicodeString, sizeof(unicodeString) / sizeof(wchar_t));
    g_pVGuiLocalize->AddString(tokenName, unicodeString,
                               m_pLocalizationDialog->GetFileName());
