...
static const int WARN_PANEL_NUMBER =
    32768; // in DEBUG if more panels than this are created then throw an
...
public:
  ...
  virtual void AddTickSignalToHead(VPANEL panel,
                                   int intervalMilliseconds = 0) OVERRIDE;
  ...
  // enables VR mode
  virtual void SetVRMode(bool bVRMode) OVERRIDE { m_bVRMode = bVRMode; }
  virtual bool GetVRMode() OVERRIDE { return m_bVRMode; }

  bool IsDispatchingMessages(void) { return m_InDispatcher; }
...
bool IsDispatchingMessageQueue(void) { return g_VGui.IsDispatchingMessages(); }
...
//	m_iDeleteCount = 	m_iDeletePanelCount = 0;
  ...
  if (!bIsReentrant) {
    ...
    // Do the OnTicks before purging messages, since in previous code they were
    // posted after dispatch and wouldn't hit
    //  until next frame
    ...
    for (int i = count - 1; i >= 0; i--) {
      ...
      tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s - Ticks: %s", __FUNCTION__,
             t->panel->Client()->GetName());
    }
...
//-----------------------------------------------------------------------------
// Purpose: Creates or updates a tick signal for a panel.  Returns NULL if
// already ticking.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Adds the panel to the tail of a tick signal list, so the panel
// receives a message every frame
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Adds the panel to the head of a tick signal list, so the panel
// receives a message every frame
//-----------------------------------------------------------------------------
...
#ifdef _X360
      else if (messageItem->_messageTo ==
               0xFFFFFFFE) // special tag to always give message to the active
                           // key focus
      {
...
#endif
  {
    messageItem._messageTo = (target != (VPANEL)MESSAGE_CURSOR_POS)
                                 ? g_pIVgui->PanelToHandle(target)
                                 : 0xFFFFFFFF;
  }
