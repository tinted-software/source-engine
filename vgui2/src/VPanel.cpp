...
//-----------------------------------------------------------------------------
VPanel::~VPanel() {
  // Someone just deleted their parent Panel while it was being used in
  // InternalSolveTraverse(). This will cause a difficult to debug crash, so we
  // spew out the panel name here in hopes
...
void VPanel::TraverseLevel(int val) {
  ...
  // This doesn't work. It appears we add all kinds of children to various
  // panels in the
  //  InternalThinkTraverse functions, and that means the refcount is 0 when
  //  added, and then drops to -1 when we decrement the traverse level.
#if 0
...
void VPanel::Init(IClientPanel *attachedClientPanel) {
  ...
  _clientPanelHandle = g_pVGui->PanelToHandle(
      attachedClientPanel ? attachedClientPanel->GetVPanel() : 0);
}
...
void VPanel::Solve() {
  ...
  if (_pinsibling) {
    ...
    for (int i = 0; i < 2; i++) {
      ...
      // Determine which direction positive values move in. For center pins, we
      // use screen relative signs.
      int iSign = 1;
      if (i == 0 && (_pinsibling_their_corner == PIN_CENTER_LEFT ||
                     _pinsibling_their_corner == PIN_TOPLEFT ||
                     _pinsibling_their_corner == PIN_BOTTOMLEFT)) {
        iSign = -1;
      } else if (i == 1 && (_pinsibling_their_corner == PIN_CENTER_TOP ||
                            _pinsibling_their_corner == PIN_TOPLEFT ||
      ...
      int iPos =
          sibPos[i] + (sibSize[i] * PinDeltas[_pinsibling_their_corner][i]);
...
CUtlVector<VPanel *> &VPanel::GetChildren() { return _childDar; }
...
//-----------------------------------------------------------------------------
void VPanel::SetSiblingPin(VPanel *newSibling, byte iMyCornerToPin,
                           byte iSiblingCornerToPinTo) {
