...
bool CSystem::SetRegistryString(const char *key, const char *value) {
  ...
  if (VCRHook_RegCreateKeyEx(hSlot, key0, null, null, REG_OPTION_NON_VOLATILE,
...
bool CSystem::GetRegistryString(const char *key, char *value, int valueLen) {
  ...
  if (VCRHook_RegOpenKeyEx(hSlot, key0, null, KEY_READ, &hKey) !=
      ERROR_SUCCESS) {
    return false;
  }

  ulong len = valueLen;
  if (VCRHook_RegQueryValueEx(hKey, key1, null, null, (uchar *)value, &len) ==
      ERROR_SUCCESS) {
    VCRHook_RegCloseKey(hKey);
    return true;
  }
...
bool CSystem::SetRegistryInteger(const char *key, int value) {
  ...
  if (VCRHook_RegSetValueEx(hKey, key1, null, REG_DWORD, (uchar *)&value, 4) ==
      ERROR_SUCCESS) {
    VCRHook_RegCloseKey(hKey);
    return true;
  }
