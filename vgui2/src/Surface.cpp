...
public:
  ...
  // LocalToScreen, ParentLocalToScreen fixups for explicit PaintTraverse calls
  // on Panels not at 0, 0 position
  ...
  void GetKernedCharWidth(HFont font, wchar_t ch, wchar_t chBefore,
                          wchar_t chAfter, float &wide, float &flabcA) {
    Assert(0);
    wide = 0.0f;
    flabcA = 0.0f;
  }
  ...
  virtual bool BHTMLWindowNeedsPaint(IHTML *htmlwin) { return false; }
...
private:
  ...
  struct ScreenOverride_t {
    ScreenOverride_t() : m_bActive(false) { m_nValue[0] = m_nValue[1] = 0; }
    bool m_bActive;
    int m_nValue[2];
  };
...
private:
  ...
  virtual ULONG STDMETHODCALLTYPE Release(void) { return --_refCount; }
...
void CWin32Surface::DrawSetTextureFrame(int id, int nFrame,
                                        unsigned int *pFrameCache) {
  // not implemented
}
  ...
  if (panel && PLAT(panel)) {
    if (state) {
      ::SetWindowLong(PLAT(panel)->hwnd, GWL_EXSTYLE,
                      ::GetWindowLong(PLAT(panel)->hwnd, GWL_EXSTYLE) |
                          WS_EX_TOOLWINDOW);
    } else {
      ::SetWindowLong(PLAT(panel)->hwnd, GWL_EXSTYLE,
                      ::GetWindowLong(PLAT(panel)->hwnd, GWL_EXSTYLE) &
...
void CWin32Surface::CreatePopup(VPANEL panel, bool minimised,
  ...
  ::SetWindowLongPtr(plat->hwnd, GWLP_USERDATA,
                     (LONG_PTR)g_pIVgui->PanelToHandle(panel));
...
void CWin32Surface::ApplyChanges() {
  for (int i = 0; i < GetPopupCount(); i++) {
    ...
    if ((x != sx) || (y != sy) || (wide != swide) || (tall != stall)) {
      ::SetWindowPos(Plat->hwnd, null, x, y, wide, tall,
...
// for now)
//-----------------------------------------------------------------------------
bool CWin32Surface::AddCustomFontFile(const char *fontName,
                                      const char *fontFileName) {
...
void CWin32Surface::PrecacheFontCharacters(HFont font,
                                           const wchar_t *pCharacters) {
  Assert(0);
}
...
void CWin32Surface::DrawFilledRectFastFade(int x0, int y0, int x1, int y1,
                                           int fadeStartPt, int fadeEndPt,
                                           unsigned int alpha0,
                                           unsigned int alpha1,
                                           bool bHorizontal) {
...
void CWin32Surface::initStaticData() {
  ...
  staticDefaultCursor[dc_hourglass] =
      (HICON)LoadCursor(null, (LPCTSTR)OCR_WAIT);
  staticDefaultCursor[dc_waitarrow] =
      (HICON)LoadCursor(null, (LPCTSTR)OCR_APPSTARTING);
  staticDefaultCursor[dc_crosshair] =
      (HICON)LoadCursor(null, (LPCTSTR)OCR_CROSS);
  ...
  staticDefaultCursor[dc_sizeall] =
      (HICON)LoadCursor(null, (LPCTSTR)OCR_SIZEALL);
...
//-----------------------------------------------------------------------------
// Purpose: our basic user agent string when doing http requests from the client
// for webkit
//-----------------------------------------------------------------------------
...
void CWin32Surface::PushFullscreenViewport() {
  AssertMsg(false,
            "Fullscreen viewport mode is unimplemented in CWin32Surface");
}
...
static LRESULT CALLBACK staticProc(HWND hwnd, UINT msg, WPARAM wparam,
  ...
  if (staticSurfaceAvailable) {
    panel =
        g_pIVgui->HandleToPanel((LONG)::GetWindowLongPtr(hwnd, GWLP_USERDATA));
  ...
  case WM_PAINT: {
    // surface was by repainted vgui or by windows itself, do the repainting all
    // repainting will goes through here and nowhere else
    ...
    //			g_pInput->PostMessage( panel, new KeyValues("Paint"),
    //NULL );

    // this relies on platTick() being called BEFORE dispatchMessages(), so that
    // painting occurs
