...
public:
  ...
  // returns the next index, or INVALID_LOCALIZE_STRING_INDEX if no more strings
  // available
...
private:
  ...
  struct localizedstring_t {
    ...
    // nameIndex == INVALID_LOCALIZE_STRING_INDEX is used only for searches and
    // implies that pszValueString will be used from union fields.
    union {
      StringIndex_t
          valueIndex; // Used when nameIndex != INVALID_LOCALIZE_STRING_INDEX
      char const *pszValueString; // Used only if nameIndex ==
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR_WITH_NAMESPACE(
    CLocalizedStringTable, vgui::, ILocalize, VGUI_LOCALIZE_INTERFACE_VERSION,
    g_StringTable);
...
bool CLocalizedStringTable::AddFile(const char *szFileName, const char *pPathID,
  ...
  // We do this manually instead of just asking for the first match to support
  ...
  if (V_IsAbsolutePath(fileName)) {
    ...
    for (char *path = strtok(searchPaths, ";"); path;
      ...
      // With bIncludeFallbackSearchPaths we iterate overriding as we go, so
      // push them in reverse order so the highest precendence search paths have
      // the highest precendence. Otherwise push them in order, as we'll only
  ...
  for (int sp = 0; sp < searchList.Count(); ++sp) {
    ...
    int bufferSize =
        g_pFullFileSystem->GetOptimalReadSize(file, fileSize + sizeof(ucs2));
    ucs2 *memBlock =
        (ucs2 *)g_pFullFileSystem->AllocOptimalReadBuffer(file, bufferSize);
    bool bReadOK =
    ...
    while (1) {
      ...
      ucs2 keytoken[128];
      ucs2 *pchNewdata =
          ReadUnicodeToken(data, keytoken, ARRAYSIZE(keytoken), bQuoted);
      if (!keytoken[0])
      ...
      if (state == STATE_BASE) {
        ...
        if (!stricmp(key, "}")) {
          ...
          if ((bEnglishFile) ||
            ...
            ucs2 *tempData = ReadUnicodeToken(data, conditional,
                ...
                conditional[1] == L'$') // wcsstr( conditional, L"[$" ) )
            {
              ...
              // Robin: HACK: Cheesy support for language-based filtering. Main
              // has much better support for this, in all KV files, so this will
              // be obsoleted in post-TF2 products.
              char *pszKey = &cond[2];
              bool bNot = false;
              if (pszKey[0] == '!') {
                bNot = true;
            ...
            if (bAccepted) {
              ...
              for (i = 0; i < MAX_LOCALIZED_CHARS && valuetoken[i] != 0; i++)
                fullString[i] = valuetoken[i]; // explode the ucs2 into a
                                               // wchar_t wide buffer
              fullString[i] = 0;
...
bool CLocalizedStringTable::SaveToFile(const char *szFileName) {
  ...
  for (StringIndex_t idx = GetFirstStringIndex();
       idx != INVALID_LOCALIZE_STRING_INDEX; idx = GetNextStringIndex(idx)) {
...
bool CLocalizedStringTable::SymLess(localizedstring_t const &i1,
  ...
  const char *str1 = (i1.nameIndex == INVALID_LOCALIZE_STRING_INDEX)
                         ? i1.pszValueString
                         : &g_StringTable.m_Names[i1.nameIndex];
  const char *str2 = (i2.nameIndex == INVALID_LOCALIZE_STRING_INDEX)
                         ? i2.pszValueString
...
//-----------------------------------------------------------------------------
// Purpose: returns the next index, or INVALID_LOCALIZE_STRING_INDEX if no more
// strings available
//-----------------------------------------------------------------------------
...
void CLocalizedStringTable::ConstructString(wchar_t *unicodeOutput,
  ...
  ILocalize::ConstructString(unicodeOutput, unicodeBufferSizeInBytes, searchPos,
                             localizationVariables);
}
