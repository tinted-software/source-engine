...
> leave win32 as is and just knobble the asserts where we know we won't
implement it.
>
>> From: Mike Sartain
...
private:
  ...
  struct InputContext_t {
    ...
    VPanel *_mouseFocus; // the panel that has the current mouse focus - same as
                         // _mouseOver unless _mouseCapture is set
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(
    CInputSystem, IInput, VGUI_INPUT_INTERFACE_VERSION,
...
CInputSystem::~CInputSystem() { DestroyCandidateList(); }
...
//-----------------------------------------------------------------------------
CInputSystem::InputContext_t *
CInputSystem::GetInputContext(HInputContext context) {
...
// Associating NULL is valid; it disconnects the panel from the context
//-----------------------------------------------------------------------------
void CInputSystem::AssociatePanelWithInputContext(HInputContext context,
                                                  VPANEL pRoot) {
  ...
  if (g_pSurface->GetPopupCount() > 0) {
    ...
    while (nIndex) {
      ...
      if (top &&
          ...
          top->IsVisible() && top->IsKeyBoardInputEnabled() &&
  ...
  if (context._keyFocus == focus) {
    if (m_nDebugMessages > 0) {
      g_pIVgui->DPrintf2("removing kb focus %s\n",
                         context._keyFocus ? context._keyFocus->GetName()
                                           : "(no name)");
    }
...
void CInputSystem::UpdateMouseFocus(int x, int y) {
  ...
  if (g_pSurface->IsCursorVisible() && g_pSurface->IsWithin(x, y)) {
    ...
    for (int i = c - 1; i >= 0; i--) {
      ...
      if (pContext->_rootPanel &&
          !popup->HasParent((VPanel *)pContext->_rootPanel)) {
...
#endif
      bool wantsMouse =
          panel->IsMouseInputEnabled() && IsChildOfModalSubTree((VPANEL)panel);
      if (!wantsMouse)
        continue;
...
// Passes in a keycode which allows hitting other mouse buttons w/o cancelling
// capture mode
void CInputSystem::SetMouseCaptureEx(VPANEL panel,
...
//mouseCapture 			MouseCaptureLost is sent to the panel that loses the mouse
//capture
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
bool CInputSystem::IsChildOfModalPanel(VPANEL panel,
...
void CInputSystem::SurfaceSetCursorPos(int x, int y) {
                                           ...
                                           // functions for us to use?
  {
    ...
    //   For l4d2, the vguimatsurface/cursor.cpp functions fire in the engine,
    //   the vgui2 ones
...
HCursor CInputSystem::GetCursorOveride() { return _cursorOverride; }
...
bool CInputSystem::InternalCursorMoved(int x, int y) {
  g_pIVgui->PostMessage(
      (VPANEL)MESSAGE_CURSOR_POS,
      new KeyValues("SetCursorPosInternal", "xpos", x, "ypos", y), NULL);
  return true;
}
...
void CInputSystem::PostCursorMessage() {
  ...
  if (pContext->_mouseCapture) {
    ...
    g_pIVgui->PostMessage((VPANEL)pContext->_mouseCapture,
                          new KeyValues("CursorMoved", "xpos",
                                        pContext->m_nCursorX, "ypos",
                                        pContext->m_nCursorY),
                          NULL);
...
// Returns the Language Bar label (Chinese, Korean, Japanese, Russion, Thai,
// etc.)
void CInputSystem::GetIMELanguageName(wchar_t *buf,
                                      int unicodeBufferSizeInBytes) {
  ASSERT_IF_IME_NYI();
...
// Returns the short code for the language (EN, CH, KO, JP, RU, TH, etc. ).
void CInputSystem::GetIMELanguageShortCode(wchar_t *buf,
                                           int unicodeBufferSizeInBytes) {
#ifdef DO_IME
...
DWORD dwConvModeMsg[13] = {
        IME_CMODE_ALPHANUMERIC,		IME_CMODE_NATIVE,
IME_CMODE_KATAKANA, IME_CMODE_LANGUAGE,			IME_CMODE_FULLSHAPE,
IME_CMODE_ROMAN, IME_CMODE_CHARCODE,			IME_CMODE_HANJACONVERT,
IME_CMODE_SOFTKBD, IME_CMODE_NOCONVERSION,		IME_CMODE_EUDC,
IME_CMODE_SYMBOL, IME_CMODE_FIXED};

// Sentence mode message
DWORD dwSentModeMsg[6] = {
        IME_SMODE_NONE,			IME_SMODE_PLAURALCLAUSE,
IME_SMODE_SINGLECONVERT, IME_SMODE_AUTOMATIC,	IME_SMODE_PHRASEPREDICT,
...
static IMESettingsTransform
    g_ConversionMode_CHT_ToChinese(IME_CMODE_ALPHANUMERIC,
                                   IME_CMODE_NATIVE | IME_CMODE_LANGUAGE, 0, 0);
static IMESettingsTransform
    g_ConversionMode_CHT_ToEnglish(IME_CMODE_NATIVE | IME_CMODE_LANGUAGE,
                                   IME_CMODE_ALPHANUMERIC, 0, 0);

static IMESettingsTransform
    g_ConversionMode_CHS_ToChinese(IME_CMODE_ALPHANUMERIC,
                                   IME_CMODE_NATIVE | IME_CMODE_LANGUAGE, 0, 0);
static IMESettingsTransform
    g_ConversionMode_CHS_ToEnglish(IME_CMODE_NATIVE | IME_CMODE_LANGUAGE,
                                   IME_CMODE_ALPHANUMERIC, 0, 0);

static IMESettingsTransform
    g_ConversionMode_KO_ToKorean(IME_CMODE_ALPHANUMERIC,
                                 IME_CMODE_NATIVE | IME_CMODE_LANGUAGE, 0, 0);

static IMESettingsTransform
    g_ConversionMode_KO_ToEnglish(IME_CMODE_NATIVE | IME_CMODE_LANGUAGE,
                                  IME_CMODE_ALPHANUMERIC, 0, 0);

static IMESettingsTransform
    g_ConversionMode_JP_Hiragana(IME_CMODE_ALPHANUMERIC | IME_CMODE_KATAKANA,
...
int CInputSystem::GetIMEConversionModes(ConversionModeItem *dest,
                                        int destcount) {
  ASSERT_IF_IME_NYI();
...
//   "UnhandleMouseClick" message is sent to unhandledMouseClickListener panel
//   if it's set
void CInputSystem::SetModalSubTree(VPANEL subTree,
                                   VPANEL unhandledMouseClickListener,
                                   bool restrictMessagesToSubTree /*= true*/) {
  InputContext_t *pContext = GetInputContext(m_hContext);
  if (!pContext)
...
bool CInputSystem::ShouldModalSubTreeReceiveMessages() const {
  InputContext_t *pContext =
      const_cast<CInputSystem *>(this)->GetInputContext(m_hContext);
  if (!pContext)
    return true;
