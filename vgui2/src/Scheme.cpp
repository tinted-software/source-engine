...
CON_COMMAND(vgui_spew_fonts, "") { g_Scheme.SpewFonts(); }
    ...
    if (fontFile && *fontFile) {
      ...
      for (KeyValues *pData = kv->GetFirstSubKey(); pData != NULL;
        ...
        if (!Q_stricmp(pszKey, "font")) {
          ...
          if (Q_stricmp(language, pszKey) ==
              0) // matches the language we're running?
          {
            // get the range
  ...
  for (KeyValues *kv = m_pData->FindKey("Fonts", true)->GetFirstSubKey();
    ...
    for (int i = 0; i < 2; i++) {
      ...
      const char *fontName = GetMungedFontName(
          kv->GetName(), tag,
          proportionalFont); // first time it adds a normal font, and then a
                             // proportional one
  ...
  FOR_EACH_DICT_FAST(m_FontAliases, i) {
    ...
    for (KeyValues *fontdata = kv->GetFirstSubKey(); fontdata != NULL;
      ...
      if ((!fontYResMin && !fontYResMax) && m_FontAliases[i].m_bProportional) {
        ...
        scalex =
            g_Scheme.GetProportionalScaledValueEx(this, scalex * 10000.0f) *
            0.0001f;
        scaley =
            g_Scheme.GetProportionalScaledValueEx(this, scaley * 10000.0f) *
            0.0001f;
      }
      ...
      if (flags & ISurface::FONTFLAG_BITMAP) {
        ...
        if (GetFontRange(fontdata->GetString("name"), nRangeMin, nRangeMax)) {
          // add the new set
          g_pSurface->SetFontGlyphSet(m_FontAliases[i]._font,
                                      fontdata->GetString("name"), tall,
                                      fontdata->GetInt("weight"), blur,
...
void CScheme::SpewFonts(void) {
  ...
  FOR_EACH_DICT_FAST(m_FontAliases, i) {
    ...
    Msg("  %2d: HFont:0x%8.8x, %s, %s, font:%s, tall:%d(%d). %s\n", i, Font,
        ...
        szFontAlias ? szFontAlias : "??", szFontName ? szFontName : "??",
        g_pSurface->GetFontTall(Font), g_pSurface->GetFontTallRequested(Font),
...
IImage *CSchemeManager::GetImage(const char *imageName, bool hardwareFiltered) {
  if (!imageName ||
      strlen(imageName) <=
          0) // frame icons and the like are in the scheme file and may not be
             // defined, so if this is null then fail silently
  {
    return NULL;
  }
...
//-----------------------------------------------------------------------------
int CScheme::GetBorderCount() const { return m_BorderList.Count(); }
...
//-----------------------------------------------------------------------------
// Purpose: returns a pointer to an existing font, proportional=false means use
// default
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
int CScheme::GetFontCount() const { return m_FontAliases.Count(); }
...
//-----------------------------------------------------------------------------
const KeyValues *CScheme::GetColorData() const { return m_pkvColors; }
