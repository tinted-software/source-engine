...
//-----------------------------------------------------------------------------
long CSystem::GetTimeMillis() { return (long)(Plat_MSTime()); }

//-----------------------------------------------------------------------------
// Purpose: Legacy stub to allow ShellExecute( "open", "file" ) -- doesn't
// otherwise work
//-----------------------------------------------------------------------------
...
void CSystem::SetClipboardText(const char *text, int textLen) {
  ...
  CFDataRef theData =
      CFDataCreate(kCFAllocatorDefault, (const UInt8 *)text, textLen);
  PasteboardPutItemFlavor(m_PasteBoardRef, (PasteboardItemID)1,
                          CFSTR("public.utf8-plain-text"), theData, 0);
  CFRelease(theData);
...
void CSystem::SetClipboardText(const wchar_t *text, int textLen) {
  ...
  CFDataRef theData = CFDataCreate(kCFAllocatorDefault, (const UInt8 *)charStr,
                                   Q_strlen(charStr));
  PasteboardPutItemFlavor(m_PasteBoardRef, (PasteboardItemID)1,
                          CFSTR("public.utf8-plain-text"), theData, 0);
  CFRelease(theData);
...
int CSystem::GetClipboardTextCount() {
  ...
  if (err != noErr)
    return 0;
  CFDataRef outData;
  err = PasteboardCopyItemFlavorData(m_PasteBoardRef, ItemID,
                                     CFSTR("public.utf8-plain-text"), &outData);
  if (err != noErr)
    return 0;
...
void Validate_System(CValidator &validator) { ValidateObj(g_System); }
#endif
