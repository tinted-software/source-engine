...
public:
  ...
  virtual void OnKeyCodeTyped(KeyCode code) {
    if (code == KEY_TAB) {
      ...
      // submit is the default button whose click event will have been called
      // already
    } else {
...
CConsolePanel::CompletionItem &
CConsolePanel::CompletionItem::operator=(const CompletionItem &src) {
  if (this == &src)
    return *this;
...
void CConsolePanel::PerformLayout() {
  ...
  if (!m_bStatusVersion) {
    ...
    const int submitInset =
        7 * scale; // x inset to pull the submit button away from the frame grab
...
void CConsolePanel::AddToHistory(const char *commandText,
  ...
  // If this code gets cleaned up then we should remove the redundant calls to
  // strlen, the check for whether _alloca succeeded, and should use V_strncpy
  // instead of the error prone memset/strncpy sequence.
  char *command =
  ...
  if (command) {
    ...
    // src\vgui2\vgui_controls\consoledialog.cpp(974): warning C6053: The prior
    // call to 'strncpy' might not zero-terminate string 'command'
...
void CConsolePanel::DumpConsoleTextToFile() {
  ...
  if (!found) {
    Print("Can't condump! Too many existing condump output files in the "
          "gamedir!\n");
    return;
  }
