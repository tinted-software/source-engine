...
//selected 			*target - Target panel of the command 			*userData - any user data
...
//manu. 			*message - pointer to the message to send when the item is selected
//			*target - Target panel of the command
//          *cascadeMenu - if the menu item opens a cascading menu, this is a
//							ptr to the menu that
//opens on selecting the item
// Output:  itemID - ID of this item
//-----------------------------------------------------------------------------
...
// Input  : *itemName - Name of item
//			*itemText - Name of item text that will appear in the
//manu. 			*command -  Command text to be sent when menu item is selected 			*target -
//Target panel of the command
// Output:  itemID - ID of this item
//-----------------------------------------------------------------------------
...
//name of the menu item panel. 			*command -  Command text to be sent when menu
...
// Input  : *itemText - Name of item text that will appear in the manu.
//						This will also be used as the
//name of the menu item panel. 			*message - pointer to the message to send when
//the item is selected 			*target - Target panel of the command
//          *cascadeMenu - if the menu item opens a cascading menu, this is a
//							ptr to the menu that
//opens on selecting the item
//-----------------------------------------------------------------------------
...
// Purpose: Add a menu item to the menu.
// Input  : *itemText - Name of item text that will appear in the manu.
//						This will also be the text of
//the command sent when the 						item is selected. 			*target - Target panel of the
//command
//          *cascadeMenu - if the menu item opens a cascading menu, this is a
//							ptr to the menu that
//opens on selecting the item
...
void Menu::PerformLayout() {
  ...
  if (bNeedScrollbar) {
    ...
    MakeItemsVisibleInScrollRange(m_iNumVisibleLines,
                                  min(fullHeightWouldRequire, workTall));
  } else {
    RemoveScrollBar();
       ...
       i++) // use sortedItems instead of MenuItems due to SetPos()
  {
    ...
    if ((!child->IsCheckable()) && (m_iCheckImageWidth > 0)) {
      ...
      child->SetTextInset(0,
                          0); // TODO: for some reason I can't comment this out.
    }
  ...
  FOR_EACH_LL(m_MenuItems, j) {
    m_MenuItems[j]
        ->InvalidateLayout(); // cause each menu item to redo its apply settings
                              // now we have sized ourselves
  }
  ...
  case KEY_PAGEDOWN: {
    if (m_iNumVisibleLines > 1) {
      if (m_iCurrentlySelectedItemID + m_iNumVisibleLines >= GetItemCount()) {
        MoveAlongMenuItemList(
            MENU_DOWN * (GetItemCount() - m_iCurrentlySelectedItemID - 1), 0);
      } else {
...
void Menu::OnHotKey(wchar_t unichar) {
  ...
  FOR_EACH_LL(m_MenuItems, i) {
    ...
    if (panel->IsVisible()) {
      ...
      // if the menuitem is a cascading menuitem and it is open, check its
      // hotkeys too
...
void Menu::OnTypeAhead(wchar_t unichar) {
  // Don't do anything if the menu is empty since there cannot be a selected
  // item.
...
//-----------------------------------------------------------------------------
int Menu::GetItemCount() const { return m_MenuItems.Count(); }
...
MenuItem *MenuBuilder::AddMenuItem(const char *pszButtonText,
                                   const char *pszCommand,
                                   const char *pszCategoryName) {
  AddSepratorIfNeeded(pszCategoryName);
  return m_pMenu->GetMenuItem(
      m_pMenu->AddMenuItem(pszButtonText, pszCommand, m_pActionTarget));
}

MenuItem *MenuBuilder::AddMenuItem(const char *pszButtonText,
                                   KeyValues *kvUserData,
                                   const char *pszCategoryName) {
  AddSepratorIfNeeded(pszCategoryName);
  return m_pMenu->GetMenuItem(
      m_pMenu->AddMenuItem(pszButtonText, kvUserData, m_pActionTarget));
}

MenuItem *MenuBuilder::AddCascadingMenuItem(const char *pszButtonText,
                                            Menu *pSubMenu,
                                            const char *pszCategoryName) {
  AddSepratorIfNeeded(pszCategoryName);
  return m_pMenu->GetMenuItem(
      m_pMenu->AddCascadingMenuItem(pszButtonText, m_pActionTarget, pSubMenu));
}
