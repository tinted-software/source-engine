...
TextEntry::TextEntry(Panel *parent, const char *panelName)
  ...
  REGISTER_COLOR_AS_OVERRIDABLE(_selectionTextColor,
                                "selectionTextColor_override");
  REGISTER_COLOR_AS_OVERRIDABLE(_defaultSelectionBG2Color,
...
void TextEntry::OnKeyCodePressed(KeyCode code) {
  ...
  // Forward on just a few key codes, everything else can be handled by
  // TextEntry itself
  switch (code) {
  case KEY_F1:
  case KEY_F2:
  ...
  if (IsMouseCode(code) || IsNovintButtonCode(code) || IsJoystickCode(code) ||
      IsJoystickButtonCode(code) || IsJoystickPOVCode(code) ||
...
//cursor is not in the window 			scroll left if we need to
//-----------------------------------------------------------------------------
...
//window. 			We do this one char at a time because char width isn't a constant.
//-----------------------------------------------------------------------------
...
//RecalculateLineBreaks 			to figure it all out from scratch
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
void TextEntry::GetText(OUT_Z_BYTECAP(bufLenInBytes) char *buf,
                        int bufLenInBytes) {
  Assert(bufLenInBytes >= sizeof(buf[0]));
  if (m_TextStream.Count()) {
    // temporarily null terminate the text stream so we can use the conversion
    // function
    int nullTerminatorIndex = m_TextStream.AddToTail((wchar_t)0);
    g_pVGuiLocalize->ConvertUnicodeToANSI(m_TextStream.Base(), buf,
                                          bufLenInBytes);
...
//-----------------------------------------------------------------------------
void TextEntry::GetText(OUT_Z_BYTECAP(bufLenInBytes) wchar_t *wbuf,
                        int bufLenInBytes) {
...
void TextEntry::ApplySettings(KeyValues *inResourceData) {
  ...
  _font = scheme()
              ...
              ->GetFont(inResourceData->GetString("font", "Default"),
                        IsProportional());
  SetFont(_font);
