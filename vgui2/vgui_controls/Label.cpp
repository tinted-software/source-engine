...
wchar_t Label::GetHotKey() { return _hotkey; }
...
void Label::SetText(const wchar_t *unicodeString,
  ...
  if (unicodeString && _textImage->GetUText() &&
      !Q_wcscmp(unicodeString, _textImage->GetUText()))
    return;
...
void Label::GetSettings(KeyValues *outResourceData) {
  ...
  if (m_bUseProportionalInsets) {
    outResourceData->SetInt(
        "textinsetx",
        scheme()->GetProportionalNormalizedValueEx(GetScheme(), _textInset[0]));
    outResourceData->SetInt("textinsety", _textInset[1]);
  } else {
    outResourceData->SetInt("textinsetx", _textInset[0]);
    outResourceData->SetInt("textinsety", _textInset[1]);
  }
  outResourceData->SetInt("auto_wide_tocontents",
                          (m_bAutoWideToContents ? 1 : 0));
  outResourceData->SetInt("use_proportional_insets",
                          (m_bUseProportionalInsets ? 1 : 0));
}
...
void Label::ApplySettings(KeyValues *inResourceData) {
  ...
  m_bUseProportionalInsets =
      inResourceData->GetInt("use_proportional_insets", 0) > 0;
  if (m_bUseProportionalInsets) {
