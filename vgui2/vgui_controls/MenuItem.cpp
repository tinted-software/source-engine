...
//			text - the name of the menu item as it appears in the
//menu 			cascadeMenu - if this item triggers the opening of a cascading menu
//			provide a pointer to it.
//			MenuItems cannot be both checkable and trigger a cascade
//menu.
//-----------------------------------------------------------------------------
MenuItem::MenuItem(Menu *parent, const char *panelName, const char *text,
                   Menu *cascadeMenu, bool checkable)
...
//menu.
//-----------------------------------------------------------------------------
MenuItem::MenuItem(Menu *parent, const char *panelName, const wchar_t *wszText,
                   Menu *cascadeMenu, bool checkable)
...
// Purpose:
//-----------------------------------------------------------------------------
Menu *MenuItem::GetParentMenu() { return (Menu *)GetParent(); }
...
void MenuItem::OnCursorEntered() {
  ...
  // tell the parent this menuitem is the one that was entered so it can
  // highlight it
...
//			Menu item buttons highlight if disabled, but won't
//activate.
//-----------------------------------------------------------------------------
...
void MenuItem::FireActionSignal() {
  ...
  // (they trigger the cascading menu to open/close when cursor is moved
  // over/off them)
  if (!m_pCascadeMenu) {
    ...
    //	ivgui()->PostMessage(GetVParent(), new KeyValues("MenuItemSelected"),
    //GetVPanel());
...
//-----------------------------------------------------------------------------
bool MenuItem::HasMenu() { return (m_pCascadeMenu != NULL); }
...
void MenuItem::ApplySchemeSettings(IScheme *pScheme) {
  ...
  SetDepressedColor(
      GetSchemeColor("Menu.ArmedTextColor", GetFgColor(), pScheme),
      GetSchemeColor("Menu.ArmedBgColor", GetBgColor(), pScheme));
  ...
  if (m_pCascadeArrow) {
    ...
    (static_cast<MenuItemCheckImage *>(m_pCheck))
...
//			for normal menu items this is the same as the label
//size, but for 			cascading menus it gives you the size of the text portion only,
//without 			the arrow.
//-----------------------------------------------------------------------------
...
//			For normal menu items this is the same as the label
//size, but for 			cascading menus it sizes textImage portion only, without 			the
//arrow.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
Menu *MenuItem::GetMenu() { return m_pCascadeMenu; }

//-----------------------------------------------------------------------------
// Purpose: Get the border style for the button. Menu items have no border so
//			return null.
//-----------------------------------------------------------------------------
IBorder *MenuItem::GetBorder(bool depressed, bool armed, bool selected,
                             bool keyfocus) {
  return NULL;
}
...
//-----------------------------------------------------------------------------
bool MenuItem::IsChecked() { return m_bChecked; }
...
// Purpose:
//-----------------------------------------------------------------------------
bool MenuItem::CanBeDefaultButton(void) { return false; }
