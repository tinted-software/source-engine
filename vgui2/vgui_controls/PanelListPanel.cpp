...
PanelListPanel::PanelListPanel(vgui::Panel *parent, char const *panelName)
  ...
  if (IsProportional()) {
    m_iDefaultHeight =
        scheme()->GetProportionalScaledValueEx(GetScheme(), DEFAULT_HEIGHT);
    m_iPanelBuffer =
        scheme()->GetProportionalScaledValueEx(GetScheme(), PANELBUFFER);
  } else {
...
//-----------------------------------------------------------------------------
int PanelListPanel::GetItemCount() const { return m_DataItems.Count(); }
...
// items.
//-----------------------------------------------------------------------------
int PanelListPanel::FirstItem() const { return m_DataItems.Head(); }

int PanelListPanel::NextItem(int nItemID) const {
  return m_DataItems.Next(nItemID);
}

int PanelListPanel::InvalidItemID() const { return m_DataItems.InvalidIndex(); }
...
void PanelListPanel::PerformLayout() {
  ...
  m_vbar->SetButtonPressedScrollValue(
      tall / 4); // standard height of labels/buttons etc.
  ...
  m_pPanelEmbedded->SetSize(
      wide - m_vbar->GetWide(),
      vpixels); // scrollbar will sit on top (zpos set explicitly)
...
// Purpose:
//-----------------------------------------------------------------------------
void PanelListPanel::MoveScrollBarToTop() { m_vbar->SetValue(0); }
...
//-----------------------------------------------------------------------------
int PanelListPanel::GetFirstColumnWidth() { return m_iFirstColumnWidth; }

void PanelListPanel::SetNumColumns(int iNumColumns) {
  m_iNumColumns = iNumColumns;
}

int PanelListPanel::GetNumColumns(void) { return m_iNumColumns; }
