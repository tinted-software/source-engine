...
namespace vgui {
ComboBoxButton::ComboBoxButton(ComboBox *parent, const char *panelName,
                               const char *text)
...
IBorder *ComboBoxButton::GetBorder(bool depressed, bool armed, bool selected,
                                   bool keyfocus) {
  return NULL;
...
//			only if the menu is closed, so let the parent handle
//this.
//-----------------------------------------------------------------------------
...
vgui::Panel *ComboBox_Factory() { return new ComboBox(NULL, NULL, 5, true); }
...
//-----------------------------------------------------------------------------
ComboBox::ComboBox(Panel *parent, const char *panelName, int numLines,
                   bool allowEdit)
    : TextEntry(parent, panelName) {
  SetEditable(allowEdit);
  SetHorizontalScrolling(
      false); // do not scroll, always Start at the beginning of the text.
...
// Input  : char *itemText - name of dropdown menu item
//-----------------------------------------------------------------------------
bool ComboBox::UpdateItem(int itemID, const char *itemText,
                          const KeyValues *userData) {
  if (!m_pDropDown->IsValidMenuID(itemID))
    return false;

  // when the menu item is selected it will send the custom message "SetText"
  m_pDropDown->UpdateMenuItem(
      itemID, itemText, new KeyValues("SetText", "text", itemText), userData);
  InvalidateLayout();
  return true;
}
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
bool ComboBox::UpdateItem(int itemID, const wchar_t *itemText,
                          const KeyValues *userData) {
  if (!m_pDropDown->IsValidMenuID(itemID))
    return false;
...
// Purpose:
//-----------------------------------------------------------------------------
int ComboBox::GetItemCount() const { return m_pDropDown->GetItemCount(); }
...
//-----------------------------------------------------------------------------
// Purpose: Activate the item in the menu list, as if that menu item had been
// selected by the user Input  : itemID - itemID from AddItem in list of
// dropdown items
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Activate the item in the menu list, without sending a TextChanged
// message Input  : row - row to activate
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Activate the item in the menu list, without sending a TextChanged
// message Input  : itemID - itemID from AddItem in list of dropdown items
//-----------------------------------------------------------------------------
void ComboBox::SilentActivateItem(int itemID) {
  m_pDropDown->SilentActivateItem(itemID);

  // Now manually call our set text, with a wrapper to ensure we don't send the
...
void ComboBox::PerformLayout() {
  ...
  // Some dropdown button icons in our games are wider than they are taller. We
...
void ComboBox::ApplySchemeSettings(IScheme *pScheme) {
  ...
  SetBorder(pScheme->GetBorder(m_szBorderOverride[0] ? m_szBorderOverride
                                                     : "ComboBoxBorder"));
}
...
void ComboBox::ApplySettings(KeyValues *pInResourceData) {
  ...
  const char *pBorderOverride =
      pInResourceData->GetString("border_override", NULL);
  if (pBorderOverride) {
...
void ComboBox::OnMenuClose() {
  ...
  else if (IsCursorOver()) // make sure it's getting pressed over us (it may not
                           // be due to mouse capture)
  {
    SelectAllText(false);
    OnCursorEntered();
...
void ComboBox::DoClick() {
  ...
  if (itemToSelect >= 0) {
    m_pDropDown->SetCurrentlyHighlightedItem(
        m_pDropDown->GetMenuID(itemToSelect));
  }
...
void ComboBox::OnMenuItemSelected() {
  m_bHighlight = true;
  // For editable cbs, fill in the text field from whatever is chosen from the
  // dropdown...
...
void ComboBox::OnKeyTyped(wchar_t unichar) {
  if (IsEditable() ||
      unichar == '\t') // don't play with key presses in edit mode
  {
