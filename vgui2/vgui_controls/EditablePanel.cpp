...
void EditablePanel::OnKeyCodePressed(KeyCode code) {
  ...
  if (!vgui_nav_lock_default_button.IsValid() ||
      vgui_nav_lock_default_button.GetInt() == 0) {
...
void EditablePanel::OnSizeChanged(int wide, int tall) {
  ...
  for (int i = 0; i < GetChildCount(); i++) {
    ...
    // but we're very close to shipping and this causes artifacts in other vgui
    // panels that now depend on this bug.  So, I've added
    // m_bShouldSkipAutoResize, which defaults to false but can be set using
    // "skip_autoresize" in a .res file
...
void EditablePanel::LoadControlSettings(const char *resourceName,
  ...
  //     editablepanel.cpp (535) : Resource file
  //     "resource\DebugOptionsPanel.res" not found on disk!
  // AssertMsg( g_pFullFileSystem->FileExists( resourceName ), CFmtStr(
  // "Resource file \"%s\" not found on disk!", resourceName ).Access() );
