...
void VControlsListPanel::OnClearBinding() {
  ...
  PostMessage(GetParent()->GetVPanel(),
              new KeyValues("ClearBinding", "item", GetItemOfInterest()));
}
...
CKeyBoardEditorPage::CKeyBoardEditorPage(Panel *parent, Panel *panelToEdit,
                                         KeyBindingContextHandle_t handle)
    : BaseClass(parent, "KeyBoardEditorPage"), m_pPanel(panelToEdit),
      m_Handle(handle) {
  Assert(m_pPanel);
  ...
  if (item) {
    ...
    if (kbMap) {
      KeyBindingMap_t *binding =
          m_pPanel->LookupBindingByKeyCode(code, modifiers);
    ...
    KeyBindingMap_t *bindingMap =
        reinterpret_cast<KeyBindingMap_t *>(item->GetPtr("Unbound", 0));
    if (bindingMap) {
      KeyBindingMap_t *binding =
          m_pPanel->LookupBindingByKeyCode(code, modifiers);
...
void CKeyBoardEditorPage::OnSaveChanges() { ApplyMappings(); }
...
void CKeyBoardEditorPage::GetMappingList(
...
void CKeyBoardEditorPage::AnsiText(char const *token, char *out,
                                   size_t buflen) {
  out[0] = 0;
...
CKeyBoardEditorSheet::CKeyBoardEditorSheet(Panel *parent, Panel *panelToEdit,
                                           KeyBindingContextHandle_t handle)
    : BaseClass(parent, "KeyBoardEditorSheet"), m_bSaveToExternalFile(false),
      m_Handle(handle), m_SaveFileName(UTL_INVAL_SYMBOL),
...
void CKeyBoardEditorSheet::SetKeybindingsSaveFile(char const *filename,
                                                  char const *pathID /*= 0*/) {
  Assert(filename);
...
void CKeyBoardEditorSheet::OnSaveChanges() {
  ...
  if (m_bSaveToExternalFile) {
    m_hPanel->SaveKeyBindingsToFile(
        m_Handle, m_SaveFileName.String(),
...
CKeyBoardEditorDialog::CKeyBoardEditorDialog(Panel *parent, Panel *panelToEdit,
                                             KeyBindingContextHandle_t handle)
  ...
  m_pUseDefaults =
      new Button(this, "Defaults", "#KBEditorUseDefaults", this, "defaults");
...
void CKeyBoardEditorDialog::SetKeybindingsSaveFile(char const *filename,
                                                   char const *pathID /*= 0*/) {
