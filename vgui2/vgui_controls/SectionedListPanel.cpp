...
//-----------------------------------------------------------------------------
SectionedListPanelHeader::SectionedListPanelHeader(SectionedListPanel *parent,
                                                   const char *name,
                                                   int sectionID)
    : Label(parent, name, "") {
  m_pListPanel = parent;
    ...
    for (int i = 0; i < colCount; i++) {
      ...
      if (columnFlags & SectionedListPanel::COLUMN_RIGHT) {
        SetImageBounds(i, xpos + wide - contentWide,
                       wide - SectionedListPanel::COLUMN_DATA_GAP);
      } else {
...
public:
      ...
      for (int i = 0; i < colCount; i++) {
        ...
        if (columnFlags & SectionedListPanel::COLUMN_IMAGE) {
          ...
          if (m_pListPanel->m_pImageList) {
            ...
            Assert(!("Images columns used in SectionedListPanel with no "
                     "ImageList set"));
          }
        } else {
          ...
          if (textImage) {
            ...
            if (!m_bOverrideColors) {
              ...
              if (IsSelected() &&
                ...
                Color *clrOverride = m_pListPanel->GetColorOverrideForCell(
                    m_iSectionID, m_iID, i);
                textImage->SetColor((clrOverride != NULL) ? *clrOverride
                                                          : GetFgColor());
              }
            }
          }
        ...
        if (i == 0 && !(columnFlags & SectionedListPanel::COLUMN_IMAGE)) {
          ...
          SetImageBounds(i, xpos + SectionedListPanel::COLUMN_DATA_INDENT,
                         wide - (SectionedListPanel::COLUMN_DATA_INDENT +
                                 SectionedListPanel::COLUMN_DATA_GAP));
        } else {
          if (columnFlags & SectionedListPanel::COLUMN_CENTER) {
            int offSet = (wide / 2) - (imageWide / 2);
            SetImageBounds(i, xpos + offSet,
                           wide - offSet - SectionedListPanel::COLUMN_DATA_GAP);
          } else if (columnFlags & SectionedListPanel::COLUMN_RIGHT) {
            SetImageBounds(i, xpos + wide - imageWide,
                           wide - SectionedListPanel::COLUMN_DATA_GAP);
          } else {
            SetImageBounds(i, xpos, wide - SectionedListPanel::COLUMN_DATA_GAP);
          }
  ...
  virtual void ApplySchemeSettings(IScheme *pScheme) {
    ...
    if (hFont != INVALID_FONT) {
      ...
      const char *fontName =
          pScheme->GetResourceString("SectionedListPanel.Font");
  ...
  virtual void PaintBackground() {
    ...
    surface()->DrawFilledRect(0, m_nHorizFillInset, wide,
                              tall - m_nHorizFillInset);
  }
  ...
  //=============================================================================
  // HPE_BEGIN:
  // [menglish] gets the local coordinates of a cell using the max width for
  // every column
  //=============================================================================
...
void SectionedListPanel::PerformLayout() {
  ...
  if (m_iContentHeight > ctall && m_bVerticalScrollbarEnabled) {
    ...
    // When we hide the scrollbar, we need to layout the buttons because they'll
    // have more width to work with
    if (bWasVisible) {
      LayoutPanels(m_iContentHeight);
    }
  }
...
void SectionedListPanel::ApplySettings(KeyValues *inResourceData) {
  ...
  if (IsProportional()) {
    m_iSectionGap =
        scheme()->GetProportionalScaledValueEx(GetScheme(), m_iSectionGap);
    m_iLineGap =
        scheme()->GetProportionalScaledValueEx(GetScheme(), m_iLineGap);
  }
}
...
void SectionedListPanel::AddSection(int sectionID, const char *name,
  ...
  SectionedListPanelHeader *header =
      new SectionedListPanelHeader(this, name, sectionID);
...
bool SectionedListPanel::AddColumnToSection(
  ...
  wcsncpy(column.m_szColumnText, columnText,
          sizeof(column.m_szColumnText) / sizeof(wchar_t));
  column.m_szColumnText[sizeof(column.m_szColumnText) / sizeof(wchar_t) - 1] =
      0;
...
void SectionedListPanel::SetSectionMinimumHeight(int sectionID,
                                                 int iMinimumHeight) {
  if (!m_Sections.IsValidIndex(sectionID))
    return;
...
// Purpose:
//-----------------------------------------------------------------------------
void SectionedListPanel::SetColumnWidthBySection(int sectionID,
...
int SectionedListPanel::GetColumnIndexByName(int sectionID, char *name) {
  ...
  for (int columnIndex = 0; columnIndex < m_Sections[index].m_Columns.Count();
       ++columnIndex) {
    if (!V_strcmp(m_Sections[index].m_Columns[columnIndex].m_szColumnName,
                  name))
      return columnIndex;
  }
...
void SectionedListPanel::OnKeyCodePressed(KeyCode code) {
  ...
  if (nButtonCode == KEY_XBUTTON_DOWN || nButtonCode == KEY_XSTICK1_DOWN ||
...
//-----------------------------------------------------------------------------
bool SectionedListPanel::GetSectionHeaderBounds(int sectionID, int &x, int &y,
                                                int &wide, int &tall) {
  x = y = wide = tall = 0;
  int index = FindSectionIndexByID(sectionID);
...
bool SectionedListPanel::GetMaxCellBounds(int itemID, int column, int &x,
                                          int &y, int &wide, int &tall) {
  x = y = wide = tall = 0;
  if (!IsItemIDValid(itemID))
    return false;
...
// Purpose: gets the local coordinates of a cell
//-----------------------------------------------------------------------------
bool SectionedListPanel::GetItemBounds(int itemID, int &x, int &y, int &wide,
                                       int &tall) {
  x = y = wide = tall = 0;
  if (!IsItemIDValid(itemID))
    return false;
...
// Purpose:
//-----------------------------------------------------------------------------
Color *SectionedListPanel::GetColorOverrideForCell(int sectionID, int itemID,
                                                   int columnID) {
...
// Purpose:
//-----------------------------------------------------------------------------
void SectionedListPanel::SetColorOverrideForCell(int sectionID, int itemID,
                                                 int columnID,
                                                 Color clrOverride) {
