...
// This allows the slider to behave like it's larger than what's actually being
// drawn
//-----------------------------------------------------------------------------
// Purpose:
// Input  : bEnable -
...
//			100 -
//-----------------------------------------------------------------------------
void Slider::SetSliderThumbSubRange(bool bEnable, int nMin /*= 0*/,
                                    int nMax /*= 100*/) {
...
//			Warning less than 30 pixels tall and everything probably
//won't fit.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Send a message to interested parties when the user begins dragging
// the slider
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Send a message to interested parties when the user ends dragging the
// slider
//-----------------------------------------------------------------------------
...
void Slider::ApplySchemeSettings(IScheme *pScheme) {
  ...
  m_DisabledTextColor1 =
      pScheme->GetColor("Slider.DisabledTextColor1", GetFgColor());
  m_DisabledTextColor2 =
      pScheme->GetColor("Slider.DisabledTextColor2", GetFgColor());

  _sliderBorder = pScheme->GetBorder("ButtonBorder");
  _insetBorder = pScheme->GetBorder("ButtonDepressedBorder");

  if (_leftCaption) {
    _leftCaption->SetFont(
        pScheme->GetFont("DefaultVerySmall", IsProportional()));
  }

  if (_rightCaption) {
    _rightCaption->SetFont(
        pScheme->GetFont("DefaultVerySmall", IsProportional()));
  }
}
...
const char *Slider::GetDescription() {
  ...
  Q_snprintf(buf, sizeof(buf), "%s, string leftText, string rightText",
...
void Slider::DrawTicks() {
  ...
  if (IsEnabled()) {
    ...
    surface()->DrawSetColor(
        m_DisabledTextColor1); // vgui::Color( 127, 140, 127, 255 ) );
    for (int i = 0; i <= m_nNumTicks; i++) {
      int xpos = (int)(leftpixel + i * pixelspertick);
      surface()->DrawFilledRect(xpos + 1, y + 1, xpos + 2, y + tickHeight + 1);
    }
    surface()->DrawSetColor(
        m_DisabledTextColor2); // vgui::Color( 127, 140, 127, 255 ) );
    for (int i = 0; i <= m_nNumTicks; i++) {
      int xpos = (int)(leftpixel + i * pixelspertick);
      surface()->DrawFilledRect(xpos, y, xpos + 1, y + tickHeight);
    }
...
void Slider::DrawTickLabels() {
  ...
  if (IsEnabled())
    surface()->DrawSetTextColor(
        m_TickColor); // vgui::Color( 127, 140, 127, 255 ) );
  else
    surface()->DrawSetTextColor(
        m_DisabledTextColor1); // vgui::Color( 127, 140, 127, 255 ) );
...
void Slider::DrawNob() {
  ...
  int nobheight =
      IsProportional() ? scheme()->GetProportionalScaledValue(16.f) : 16.f;

  surface()->DrawFilledRect(_nobPos[0], y + tall / 2 - nobheight / 2,
                            _nobPos[1], y + tall / 2 + nobheight / 2);
  // border
  if (_sliderBorder) {
    _sliderBorder->Paint(_nobPos[0], y + tall / 2 - nobheight / 2, _nobPos[1],
...
void Slider::SetThumbWidth(int width) {
  _nobSize = IsProportional()
                 ? scheme()->GetProportionalScaledValue((float)width)
                 : (float)width;
}
