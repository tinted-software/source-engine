...
void CircularProgressBar::Paint() {
  ...
  flEndAngle =
      m_iProgressDirection == PROGRESS_CCW ? (1.0 - flEndAngle) : flEndAngle;
  ...
  for (int i = 0; i < 8; i++) {
    if (flEndProgressRadians > Segments[cur_wedge].minProgressRadians) {
      ...
      float flInternalProgress =
          flEndProgressRadians - Segments[cur_wedge].minProgressRadians;
