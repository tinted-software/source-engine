...
void TextImage::Paint() {
  ...
  for (wchar_t *wsz = _utext; *wsz != 0; wsz++) {
    ...
    if (m_ColorChangeStream.Count() > iNextColorChange) {
      if (m_ColorChangeStream[iNextColorChange].textStreamIndex ==
          (wsz - _utext)) {
...
void TextImage::GetTextSize(int &wide, int &tall) {
  ...
  for (int i = 0; i < textLen; i++) {
    ...
    surface()->GetKernedCharWidth(font, text[i], chBefore, chAfter, flWide,
                                  flabcA);
    if (text[i] == L' ')
      flWide = ceil(flWide);
...
void TextImage::RecalculateNewLinePositions() {
  ...
  for (wchar_t *wsz = &_utext[startChar]; *wsz != 0; wsz++) {
    ...
    if ((wsz[0] == '&' || wsz[0] == 0x01 || wsz[0] == 0x02 || wsz[0] == 0x03) &&
        wsz[1] != 0) {
      wsz++;
    }
...
void TextImage::RecalculateEllipsesPosition() {
  ...
  for (int check = 0; check < (m_bUseFallbackFont ? 2 : 1); ++check) {
    ...
    for (wchar_t *wsz = _utext; *wsz != 0; wsz++) {
      ...
      chAfter = wsz[1];
      float flWide = 0.0f, flabcA = 0.0f;
      surface()->GetKernedCharWidth(font, ch, chBefore, chAfter, flWide,
                                    flabcA);
      ...
      if (x + len + ellipsesWidth > _drawWidth) {
        ...
        for (const wchar_t *rwsz = wsz + 1; *rwsz != 0; rwsz++) {
          ...
          chAfter = rwsz[1];
          float flWide = 0.0f, flabcA = 0.0f;
          surface()->GetKernedCharWidth(font, *rwsz, chBefore, chAfter, flWide,
                                        flabcA);
          int len = floor(flWide + 0.6);
...
void TextImage::SetCenterWrap(bool bWrap) { m_bWrapCenter = bWrap; }
...
void TextImage::SetAllCaps(bool bAllCaps) { m_bAllCaps = bAllCaps; }
...
//-----------------------------------------------------------------------------
// Purpose: For center wrapping of multi-line text images, determines the indent
// each line needs to be centered
//-----------------------------------------------------------------------------
...
void TextImage::SetColorChangeStream(
    CUtlSortVector<label_colorchange_t, CColorChangeListLess> *pUtlVecStream) {
