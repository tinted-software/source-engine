...
void RotatingProgressBar::ApplySettings(KeyValues *inResourceData) {
  ...
  float flDegressPerSecond = DEG2RAD(inResourceData->GetFloat(
      "approach_speed", 360.0)); // default is super fast
  m_flApproachSpeed =
      flDegressPerSecond *
      (m_flTickDelay / 1000.0f); // divide by number of frames in a second
...
void RotatingProgressBar::OnTick(void) {
  float flDesiredAngle =
      RemapVal(GetProgress(), 0.0, 1.0, m_flStartRadians, m_flEndRadians);
...
void RotatingProgressBar::Paint() {
  ...
  vert[1].Init(Vector2D(m_flRotatingX + m_flRotatingWide, m_flRotatingY),
               Vector2D(1, 0));
  vert[2].Init(Vector2D(m_flRotatingX + m_flRotatingWide,
                        m_flRotatingY + m_flRotatingTall),
               Vector2D(1, 1));
  vert[3].Init(Vector2D(m_flRotatingX, m_flRotatingY + m_flRotatingTall),
               Vector2D(0, 1));
  ...
  for (int i = 0; i < 4; i++) {
    ...
    result.x =
        (vert[i].m_Position.x * flCosA - vert[i].m_Position.y * flSinA) + mid_x;
    result.y =
        (vert[i].m_Position.x * flSinA + vert[i].m_Position.y * flCosA) + mid_y;
