...
void PropertyDialog::PerformLayout() {
  ...
  if (IsProportional()) {
    ...
    iWideIndent =
        scheme()->GetProportionalScaledValueEx(GetScheme(), iWideIndent);
    iTallIndent =
        scheme()->GetProportionalScaledValueEx(GetScheme(), iTallIndent);
  }
...
void PropertyDialog::OnKeyCodeTyped(KeyCode code) {
  // this has been removed, since it conflicts with how we use the escape key in
  // the game
  //	if (code == KEY_ESCAPE)
  //	{
  //		OnCommand("Cancel");
