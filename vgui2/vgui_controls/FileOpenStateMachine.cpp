...
// Constructor
//-----------------------------------------------------------------------------
FileOpenStateMachine::FileOpenStateMachine(vgui::Panel *pParent,
...
FileOpenStateMachine::~FileOpenStateMachine() { CleanUpContextKeyValues(); }
...
//-----------------------------------------------------------------------------
FileOpenStateMachine::CompletionState_t
FileOpenStateMachine::GetCompletionState() {
...
//-----------------------------------------------------------------------------
void FileOpenStateMachine::SetCompletionState(
    FileOpenStateMachine::CompletionState_t state) {
  ...
  kv->SetString("fileType",
                m_bIsOpeningFile ? m_OpenFileType.Get() : m_SaveFileType.Get());
  if (m_pContextKeyValues) {
...
//-----------------------------------------------------------------------------
void FileOpenStateMachine::OnOverwriteFile() { CheckOutDialog(); }
...
void FileOpenStateMachine::OverwriteFileDialog() {
  ...
  vgui::MessageBox *pMessageBox =
      new vgui::MessageBox("Overwrite Existing File?", pBuf, GetParent());
...
void FileOpenStateMachine::WriteFile() {
  ...
  if (!m_pClient->OnWriteFileToDisk(m_FileName, m_SaveFileType,
                                    m_pContextKeyValues)) {
  ...
  if (m_bShowPerforceDialogs) {
    ...
    ShowPerforceQuery(GetParent(), m_FileName, this, NULL,
...
void FileOpenStateMachine::MakeFileWriteableDialog() {
  ...
  if (!g_pFullFileSystem->FileExists(m_FileName) ||
      g_pFullFileSystem->IsFileWritable(m_FileName)) {
    WriteFile();
    return;
  }
  ...
  Q_snprintf(
      pBuf, sizeof(pBuf),
      "Encountered read-only file. Should it be made writeable?\n\n\"%s\"\n",
      m_FileName.Get());
  vgui::MessageBox *pMessageBox =
      new vgui::MessageBox("Make File Writeable?", pBuf, GetParent());
  pMessageBox->AddActionSignalTarget(this);
  pMessageBox->SetOKButtonVisible(true);
  pMessageBox->SetOKButtonText("Yes");
  pMessageBox->SetCancelButtonVisible(true);
...
void FileOpenStateMachine::OnSaveFile() {
  if (!m_FileName[0] || !Q_IsAbsolutePath(m_FileName)) {
    ...
    m_pClient->SetupFileOpenDialog(pDialog, false, m_SaveFileType,
                                   m_pContextKeyValues);
...
//-----------------------------------------------------------------------------
void FileOpenStateMachine::SaveFile(KeyValues *pContextKeyValues,
                                    const char *pFileName,
                                    const char *pFileType, int nFlags) {
...
void FileOpenStateMachine::ReadFile() {
  ...
  if (!m_pClient->OnReadFileFromDisk(m_FileName, m_OpenFileType,
                                     m_pContextKeyValues)) {
...
void FileOpenStateMachine::OpenFileDialog() {
  ...
  if (m_OpenFileName.IsEmpty()) {
    ...
    m_pClient->SetupFileOpenDialog(pDialog, true, m_OpenFileType,
                                   m_pContextKeyValues);
    pDialog->SetDeleteSelfOnClose(true);
    pDialog->AddActionSignalTarget(this);
    pDialog->DoModal();
...
//-----------------------------------------------------------------------------
void FileOpenStateMachine::OpenFile(const char *pOpenFileType,
...
//-----------------------------------------------------------------------------
void FileOpenStateMachine::OpenFile(const char *pOpenFileName,
