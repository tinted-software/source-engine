...
// Purpose: data accessor
//-----------------------------------------------------------------------------
void ProgressBar::SetMargin(int pixels) { m_iBarMargin = pixels; }

//-----------------------------------------------------------------------------
// Purpose: data accessor
//-----------------------------------------------------------------------------
int ProgressBar::GetMargin() { return m_iBarMargin; }
...
const char *ProgressBar::GetDescription(void) {
  ...
  _snprintf(buf, sizeof(buf), "%s, string progress, string variable",
...
void ContinuousProgressBar::Paint() {
  ...
  case PROGRESS_EAST:
    if (bUsePrev) {
      if (bGain) {
        surface()->DrawFilledRect(x, y, x + (int)(wide * _prevProgress),
                                  y + tall);

        // Delta
        surface()->DrawSetColor(m_colorGain);
        surface()->DrawFilledRect(x + (int)(wide * _prevProgress), y,
                                  x + (int)(wide * _progress), y + tall);
        break;
      } else {
        // Delta
        surface()->DrawSetColor(m_colorLoss);
        surface()->DrawFilledRect(x + (int)(wide * _progress), y,
                                  x + (int)(wide * _prevProgress), y + tall);
      }
    }
    surface()->DrawSetColor(GetFgColor());
  ...
  case PROGRESS_WEST:
    if (bUsePrev) {
      if (bGain) {
        ...
        surface()->DrawSetColor(m_colorLoss);
        surface()->DrawFilledRect(x + (int)(wide * (1.0f - _prevProgress)), y,
                                  x + (int)(wide * (1.0f - _progress)),
                                  y + tall);
      }
    }
    surface()->DrawSetColor(GetFgColor());
    surface()->DrawFilledRect(x + (int)(wide * (1.0f - _progress)), y, x + wide,
                              y + tall);
    break;
  ...
  case PROGRESS_NORTH:
    if (bUsePrev) {
      if (bGain) {
        surface()->DrawFilledRect(x, y + (int)(tall * (1.0f - _prevProgress)),
                                  x + wide, y + tall);
  ...
  case PROGRESS_SOUTH:
    if (bUsePrev) {
      if (bGain) {
        surface()->DrawFilledRect(x, y, x + wide,
                                  y + (int)(tall * (1.0f - _progress)));
