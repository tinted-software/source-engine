...
// Purpose:
//-----------------------------------------------------------------------------
AnimationController::RelativeAlignment
AnimationController::LookupAlignment(char const *token) {
      ...
      if (!stricmp(token, "animate")) {
        ...
        if (cmdAnimate.variable == m_sPosition) {
          ...
          if (0 == sscanf(token, "%f %f %f %f", &cmdAnimate.target.a,
            ...
            // we don't have a way of seeing if the color is not declared in the
            // scheme, so we use this silly method of trying again with a
            // different default to see if we get the fallback again
            if (col == default_invisible_black) {
              Color error_pink(
                  255, 0, 255,
                  255); // make it extremely obvious if a scheme lookup fails
              col = scheme->GetColor(token, error_pink);

              // commented out for Soldier/Demo release...(getting spammed in
              // console) we'll try to figure this out after the update is out
              // 							if (col
              // == error_pink)
              // 							{
              // 								Warning("Missing
        ...
        Warning("Couldn't parse script sequence '%s': expected <anim command>, "
                "found '%s'\n",
                g_ScriptSymbols.String(seq.name), token);
        return false;
      }
...
void AnimationController::UpdatePostedMessages(bool bRunToCompletion) {
  ...
  for (int i = 0; i < m_PostedMessages.Count(); i++) {
    ...
    case CMD_RUNEVENTCHILD: {
      ...
      curEvent.pParent = msg.parent.Get()->FindChildByName(
          g_ScriptSymbols.String(msg.variable), true);
    ...
    case CMD_SETVISIBLE: {
      Panel *pPanel = msg.parent.Get()->FindChildByName(
          g_ScriptSymbols.String(msg.variable), true);
      if (pPanel) {
...
AnimationController::Value_t AnimationController::GetInterpolatedValue(
  ...
  case INTERPOLATOR_BOUNCE: {
    // fall from startValue to endValue, bouncing a few times and settling out
    // at endValue
...
//-----------------------------------------------------------------------------
bool AnimationController::StopAnimationSequence(Panel *pWithinParent,
                                                const char *sequenceName) {
  Assert(pWithinParent);
...
//-----------------------------------------------------------------------------
AnimationController::Value_t
AnimationController::GetValue(ActiveAnimation_t &anim, Panel *panel,
                              UtlSymId_t var) {
  Value_t val = {0, 0, 0, 0};
  if (var == m_sPosition) {
    int x, y;
...
//-----------------------------------------------------------------------------
PanelAnimationMap *
CPanelAnimationDictionary::FindPanelAnimationMap(char const *className) {
...
//-----------------------------------------------------------------------------
PanelAnimationMap *
CPanelAnimationDictionary::FindOrAddPanelAnimationMap(char const *className) {
