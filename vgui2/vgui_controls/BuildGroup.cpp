...
void BuildGroup::DrawRulers() {
  ...
  surface()->DrawFilledRect(cx + cwide - 1, 0, cx + cwide, cy); // top vert
                                                                // right
...
bool BuildGroup::KeyCodeTyped(KeyCode code, Panel *panel) {
  ...
  if (ctrl) {
    ...
    case KEY_V: {
      ivgui()->PostMessage(m_hBuildDialog->GetVPanel(), new KeyValues("Paste"),
                           NULL);
      break;
    }
    }
...
void BuildGroup::LoadControlSettings(const char *controlResourceName,
  ...
  while (!rDat) {
    ...
    if (!V_stricmp(CommandLine()->ParmValue("-game", "hl2"), "tf")) {
      ...
      if (!bSuccess) {
        bSuccess =
            rDat->LoadFromFile(g_pFullFileSystem, controlResourceName, "vgui");
      }
      if (!bSuccess) {
        bSuccess =
            rDat->LoadFromFile(g_pFullFileSystem, controlResourceName, "BSP");
      }
  ...
  if (bShouldCacheKeys && bUsePrecaching) {
    Assert(m_dictCachedResFiles.Find(controlResourceName) ==
           m_dictCachedResFiles.InvalidIndex());
    m_dictCachedResFiles.Insert(controlResourceName, rDat);
  } else if (bDeleteKeys) {
    Assert(m_dictCachedResFiles.Find(controlResourceName) !=
               m_dictCachedResFiles.InvalidIndex() ||
           pConditions || pPreloadedKeyValues);
    rDat->deleteThis();
  }
}

void BuildGroup::ProcessConditionalKeys(KeyValues *pData,
                                        KeyValues *pConditions) {
    ...
    for (; pSubKey != NULL; pSubKey = pSubKey->GetNextKey()) {
      ...
      for (; pCondition != NULL; pCondition = pCondition->GetNextKey()) {
        ...
        if (pConditionBlock) {
          ...
          for (pOverridingKey = pConditionBlock->GetFirstSubKey();
               pOverridingKey != NULL;
               pOverridingKey = pOverridingKey->GetNextKey()) {
            KeyValues *pExistingKey =
                pSubKey->FindKey(pOverridingKey->GetName());
            if (pExistingKey) {
...
void BuildGroup::ReloadControlSettings() {
  ...
  for (int i = 1; i < _panelDar.Count(); i++) {
    if (!_panelDar[i].Get()) // this can happen if we had two of the same handle
                             // in the list
    {
      _panelDar.Remove(i);
      --i;
...
// applying initial settings. Output:	Panel *newPanel, NULL if failed to
// create new control.
//-----------------------------------------------------------------------------
Panel *BuildGroup::NewControl(KeyValues *controlKeys, int x, int y) {
  ...
  if (controlKeys) {
    ...
    //controlKeys->GetString( "fieldName" ), controlKeys->GetString(
    //"ControlName" ) );
...
bool BuildGroup::PrecacheResFile(const char *pszResFileName) {
  ...
  if (pkvResFile->LoadFromFile(g_pFullFileSystem, pszResFileName, "GAME")) {
    Assert(m_dictCachedResFiles.Find(pszResFileName) ==
           m_dictCachedResFiles.InvalidIndex());
