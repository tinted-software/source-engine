...
#define CLICK_TO_EDIT_DELAY_MSEC 500
  ...
  virtual void OnMousePressed(MouseCode code) {
    if (m_bEditingInPlace) {
      ...
      if (!shift && !ctrl && !m_bArmForEditing && IsLabelEditingAllowed() &&
          bIsOnlyOneItemSelected && IsTextFullySelected() &&
          !IsBeingDragged()) {
  ...
  void FinishEditingInPlace(bool revert = false) {
    ...
    if (revert) {
      ...
      KeyValues *kv = new KeyValues("LabelChanged", "original",
                                    m_OriginalText.Get(), "changed", buf);
      PostActionSignal(kv);
    }
  }
...
public:
  ...
  //!! this could possibly be changed to just disallow mouse input on the image
  //!panel
  virtual void OnMousePressed(MouseCode code) {
    // let parent deal with it
    CallParentFunction(new KeyValues("MousePressed", "code", code));
...
public:
  ...
  virtual void OnMouseDoublePressed(MouseCode code);
  TreeNode *FindItemUnderMouse(int &nStart, int &nCount, int x, int &y, int mx,
                               int my);
  ...
  void FindNodesInRange(CUtlVector<TreeNode *> &list, int startIndex,
                        int endIndex);
...
private:
  void FindNodesInRange_R(CUtlVector<TreeNode *> &list, bool &finished,
                          bool &foundStart, int startIndex, int endIndex);
...
TreeNode::TreeNode(Panel *parent, TreeView *pTreeView)
    : BaseClass(parent, "TreeNode"), m_nClickedItem(0),
...
bool TreeNode::GetDropContextMenu(Menu *menu,
...
int TreeNode::ComputeInsertionPosition(TreeNode *pChild) {
  ...
  while (start <= end) {
    ...
    if (m_pTreeView->m_pSortFunc(m_Children[mid]->m_pData, pChild->m_pData)) {
      start = mid + 1;
    } else if (m_pTreeView->m_pSortFunc(pChild->m_pData,
                                        m_Children[mid]->m_pData)) {
      end = mid - 1;
    } else {
      return mid;
    }
...
void TreeNode::SetNodeExpanded(bool bExpanded) {
  ...
  if (m_bExpand) {
    ...
    // check if we've closed down on one of our children, if so, we get the
    // focus
    int selectedItem = m_pTreeView->GetFirstSelectedItem();
    if (selectedItem != -1 &&
        m_pTreeView->m_NodeList[selectedItem]->HasParent(this)) {
...
void TreeNode::SelectPrevChild(TreeNode *pCurrentChild) {
  ...
  if (i == 0) {
    ...
    // if this child is expanded with children, then we have to find the last
    // child
...
void TreeNode::OnMousePressed(MouseCode code) {
  ...
  if (code == MOUSE_LEFT) {
    ...
    if (x < TREE_INDENT_AMOUNT) {
      ...
      // m_pTreeView->SetSelectedItem(m_ItemIndex);    // explorer doesn't
      // actually select item when it expands an item purposely commented out in
      // case we want to change the behavior
    } else {
    ...
    // If the item was selected, leave selected items alone, otherwise make it
...
void TreeNode::FindNodesInRange(CUtlVector<TreeNode *> &list, int startIndex,
                                int endIndex) {
  list.RemoveAll();
  bool finished = false;
...
void TreeNode::FindNodesInRange_R(CUtlVector<TreeNode *> &list, bool &finished,
                                  bool &foundStart, int startIndex,
                                  int endIndex) {
  if (finished)
    return;
  if (foundStart == true) {
  ...
  for (i = 0; i < c; i++) {
    m_Children[i]->FindNodesInRange_R(list, finished, foundStart, startIndex,
                                      endIndex);
  }
}

void TreeNode::PositionAndSetVisibleNodes(int &nStart, int &nCount, int x,
                                          int &y) {
  // position ourselves
  if (nStart == 0) {
  ...
  for (i = 0; i < GetChildrenCount(); i++) {
    if (nCount > 0 && m_bExpand) {
      ...
      m_Children[i]->SetVisible(
          false); // this will make all grand children hidden as well
    }
  }
}

TreeNode *TreeNode::FindItemUnderMouse(int &nStart, int &nCount, int x, int &y,
                                       int mx, int my) {
  // position ourselves
  if (nStart == 0) {
    int posx, posy;
  ...
  for (i = 0; i < GetChildrenCount(); i++) {
    if (nCount > 0 && m_bExpand) {
      TreeNode *child =
          m_Children[i]->FindItemUnderMouse(nStart, nCount, x, y, mx, my);
      if (child != NULL) {
        return child;
      }
    }
...
void TreeView::RemoveItem(int itemIndex, bool bPromoteChildren,
                          bool bFullDelete) {
  // HACK: there's a bug with RemoveItem where panels are lingering. This gets
...
bool TreeView::ModifyItem(int itemIndex, KeyValues *data) {
  ...
  if (bReSort) {
    ...
    bool bLeftBad = (nChildIndex > 0) &&
                    m_pSortFunc(pNode->m_pData,
                                pParent->m_Children[nChildIndex - 1]->m_pData);
    bool bRightBad = (nChildIndex < nChildren - 1) &&
                     m_pSortFunc(pParent->m_Children[nChildIndex + 1]->m_pData,
                                 pNode->m_pData);
...
void TreeView::SetItemSelectionUnfocusedBgColor(int itemIndex,
                                                const Color &clr) {
  Assert(m_NodeList.IsValidIndex(itemIndex));
  if (!m_NodeList.IsValidIndex(itemIndex))
    return;
...
void TreeView::PerformLayout() {
  ...
  int y = 0;
  m_pRootNode->PositionAndSetVisibleNodes(vbarPos, visibleItemCount, -hbarPos,
                                          y);
...
void TreeView::ApplySchemeSettings(IScheme *pScheme) {
  ...
  SetBgColor(GetSchemeColor("TreeView.BgColor",
                            GetSchemeColor("WindowDisabledBgColor", pScheme),
                            pScheme));
...
void TreeView::OnLabelChanged(int itemIndex, char const *oldString,
...
int TreeView::FindItemUnderMouse(int mx, int my) {
  ...
  if (mx >= TREE_INDENT_AMOUNT) {
    ...
    // need to figure out
    int vbarPos =
        m_pVertScrollBar->IsVisible() ? m_pVertScrollBar->GetValue() : 0;
    int hbarPos =
        m_pHorzScrollBar->IsVisible() ? m_pHorzScrollBar->GetValue() : 0;
    int count = m_pRootNode->CountVisibleNodes();

    int y = 0;
    TreeNode *item =
        m_pRootNode->FindItemUnderMouse(vbarPos, count, -hbarPos, y, mx, my);
    if (item) {
...
void TreeView::FindNodesInRange(int startItem, int endItem,
...
void TreeView::RemoveSelectedItem(int itemIndex) {
  ...
  if (slot != m_SelectedItems.InvalidIndex()) {
    ...
    PostActionSignal(
        new KeyValues("TreeViewItemDeselected", "itemIndex", itemIndex));
...
// Purpose:
//-----------------------------------------------------------------------------
void TreeView::AddSelectedItem(int itemIndex, bool clearCurrentSelection,
  ...
  PostActionSignal(
      new KeyValues("TreeViewItemSelected", "itemIndex", itemIndex));
  InvalidateLayout();
...
bool TreeView::IsItemDroppable(int itemIndex,
                               CUtlVector<KeyValues *> &msglist) {
...
bool TreeView::GetItemDropContextMenu(int itemIndex, Menu *menu,
                                      CUtlVector<KeyValues *> &msglist) {
  return false;
}

HCursor TreeView::GetItemDropCursor(int itemIndex,
                                    CUtlVector<KeyValues *> &msglist) {
  return dc_arrow;
}
...
// if this is set, then clicking on one row and dragging will select a run or
// items, etc.
...
void TreeView::SelectAll() {
  m_SelectedItems.RemoveAll();
  FOR_EACH_LL(m_NodeList, i) { m_SelectedItems.AddToTail(m_NodeList[i]); }

  PostActionSignal(
      new KeyValues("TreeViewItemSelected", "itemIndex", GetRootItemIndex()));
  InvalidateLayout();
}
