...
void WizardPanel::PerformLayout() {
  ...
  if (_currentSubPanel && _currentSubPanel->isNonWizardPanel()) {
    ...
    // only display one of the next or finish buttons (and only if both are
    // visible)
    if (_showButtons) {
...
//-----------------------------------------------------------------------------
// Purpose: if we don't show buttons then let the sub panel occupy the whole
// screen
//-----------------------------------------------------------------------------
...
void WizardPanel::ResetDefaultButton() {
  ...
  /* Don't ever set the cancel button as the default, as it is too easy for
  users to quit the wizard without realizing else if
...
//-----------------------------------------------------------------------------
void WizardPanel::ResetHistory() { _subPanelStack.RemoveAll(); }
...
void WizardPanel::ActivatePrevSubPanel() {
  ...
  if (!prevPanel) {
    ivgui()->DPrintf2("Error: WizardPanel::ActivatePrevSubPanel(): no previous "
                      "panel to go back to\n");
    return;
  }
  ...
  // show the previous panel, but don't Activate it (since it should show just
...
//-----------------------------------------------------------------------------
WizardSubPanel *
WizardPanel::FindNextValidSubPanel(WizardSubPanel *currentPanel) {
  // skip over sub panels if they don't want to be displayed
  while (currentPanel) {
...
void WizardPanel::OnNextButton() {
  if (_currentSubPanel) {
    ...
    if (shouldAdvance) {
      WizardSubPanel *nextPanel =
          FindNextValidSubPanel(_currentSubPanel->GetNextSubPanel());
...
// Purpose:
//-----------------------------------------------------------------------------
KeyValues *WizardPanel::GetWizardData() { return _currentData; }
