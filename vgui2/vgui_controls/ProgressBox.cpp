...
ProgressBox::ProgressBox(const char *title, const char *text,
  ...
  if (ws) {
    wcsncpy(m_wszUnknownTimeString, ws,
            sizeof(m_wszUnknownTimeString) / sizeof(wchar_t));
  } else {
...
ProgressBox::ProgressBox(const wchar_t *wszTitle, const wchar_t *wszText,
    ...
    : Frame(parent, NULL, parent ? false : true) {
  wcsncpy(m_wszTitleString, wszTitle,
          sizeof(m_wszTitleString) / sizeof(wchar_t));
  ...
  wcsncpy(m_wszUnknownTimeString, wszUnknownTimeString,
          sizeof(m_wszUnknownTimeString) / sizeof(wchar_t));
  Init();
}
...
void ProgressBox::UpdateTitle() {
  ...
  if ((int)(m_flCurrentProgress * 100.0f) > 0) {
    _snwprintf(completion, sizeof(completion) / sizeof(wchar_t),
               L"- %d%% complete", (int)(m_flCurrentProgress * 100.0f));
  } else {
    completion[0] = 0;
  }
