...
public:
  PageTab(PropertySheet *parent, const char *panelName, const char *text,
      ...
      : Button((Panel *)parent, panelName, text), m_pParent(parent),
        m_pPage(page), m_pImage(0), m_pszImageName(0),
        m_bShowContextLabel(showContextButton), m_bAttemptingDrop(false),
        m_hoverActivatePageTime(hoverActivatePageTime), m_dropHoverTime(-1) {
  ...
  virtual void ApplySettings(KeyValues *inResourceData) {
    const char *pBorder =
        inResourceData->GetString("activeborder_override", "");
    if (*pBorder) {
...
void PropertySheet::ApplySchemeSettings(IScheme *pScheme) {
  ...
  //=============================================================================
  // HPE_BEGIN:
  // [tj] Here, we used to use a single size variable and overwrite it when we
  // scaled.
  //		This led to problems when we changes resolutions, so now we
  //recalcuate the absolute
  //      size from the relative size each time (based on proportionality)
  //=============================================================================
  if (IsProportional()) {
    m_iTabHeight = scheme()->GetProportionalScaledValueEx(
        GetScheme(), m_iSpecifiedTabHeight);
    m_iTabHeightSmall = scheme()->GetProportionalScaledValueEx(
        GetScheme(), m_iSpecifiedTabHeightSmall);
  } else {
...
void PropertySheet::ApplySettings(KeyValues *inResourceData) {
  ...
  if (pTabWidthKV) {
    _tabWidth = scheme()->GetProportionalScaledValueEx(GetScheme(),
                                                       pTabWidthKV->GetInt());
    for (int i = 0; i < m_PageTabs.Count(); i++) {
...
void PropertySheet::PerformLayout() {
  ...
  if (_showTabs) {
    for (int i = 0; i < limit; i++) {
      int tabHeight =
          IsSmallTabs() ? (m_iTabHeightSmall - 1) : (m_iTabHeight - 1);
