...
//-----------------------------------------------------------------------------
int ListPanel::AddItem(const KeyValues *item, uintp userData,
...
void ListPanel::PerformLayout() {
  ...
  } else             // we're resizing in response to a window resize
  {
    for (int i = 0; i < nColumns; i++) {
      if (m_ColumnsData[m_CurrentColumns[i]]
              .m_bResizesWithWindow // column is resizable in response to window
         ...
         // colums fit! resize if needed
  {
    ...
    int dx =
        buttonMaxXPos - startingBarWidth; // this is how much we grew or shrank.
    // see how many bars we have and now much each should grow/shrink
    dxPerBar = (int)((float)dx / (float)numToResize);
    m_lastBarWidth = buttonMaxXPos;
  }
  ...
  m_iHeaderHeight = m_ColumnsData[0].m_pHeader
                        ? m_ColumnsData[0].m_pHeader->GetTall()
                        : m_iHeaderHeight;
}
...
void ListPanel::OnKeyCodePressed(KeyCode code) {
  ...
  case KEY_XBUTTON_A:
    PostActionSignal(
        new KeyValues("ListPanelItemChosen", "itemID", m_SelectedItems[0]));
    break;
  default:
    BaseClass::OnKeyCodePressed(code);
    break;
...
void ListPanel::ApplyUserConfigSettings(KeyValues *userConfig) {
  // Check for version mismatch, then don't load settings.  (Just revert to the
  // defaults.)
