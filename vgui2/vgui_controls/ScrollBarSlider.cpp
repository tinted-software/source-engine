...
//-----------------------------------------------------------------------------
// The ScrollBarSlider is the scroll bar nob that moves up and down in through a
// range.
//-----------------------------------------------------------------------------
ScrollBarSlider::ScrollBarSlider(Panel *parent, const char *panelName,
                                 bool vertical)
...
void ScrollBarSlider::RecomputeNobPosFromValue() {
  ...
  //	Msg( "fwide: %f  ftall: %f  frange: %f  fvalue: %f  frangewindow: %f
...
void ScrollBarSlider::RecomputeValueFromNobPos() {
  ...
  if (frangewindow > 0) {
    ...
    // Our scroll bar actually doesnt scroll through all frange lines in the
    // truerange, we actually only scroll through frange-frangewindow number of
    // lines so we must take that into account when we calculate the value
  ...
  // Clamp final result
  _value = (_value < (_range[1] - _rangeWindow)) ? _value
                                                 : (_range[1] - _rangeWindow);
...
void ScrollBarSlider::ApplySettings(KeyValues *pInResourceData) {
  ...
  const char *pButtonBorderName =
      pInResourceData->GetString("ButtonBorder", NULL);
  if (pButtonBorderName) {
    _ScrollBarSliderBorder =
        vgui::scheme()->GetIScheme(GetScheme())->GetBorder(pButtonBorderName);
  }
}
...
//-----------------------------------------------------------------------------
// Purpose: Respond to cursor movements, we only care about clicking and
// dragging
//-----------------------------------------------------------------------------
void ScrollBarSlider::OnCursorMoved(int x, int y) {
  ...
  InvalidateLayout(); // not invalidatelayout - because it won't draw while
                      // we're scrolling the slider
...
//-----------------------------------------------------------------------------
// Purpose: Set the number of lines visible in the window the ScrollBarSlider is
// attached to
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Get the number of lines visible in the window the ScrollBarSlider is
// attached to
//-----------------------------------------------------------------------------
