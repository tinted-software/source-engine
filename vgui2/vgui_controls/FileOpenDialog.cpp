...
static ColumnInfo_t g_ColInfo[] = {
    ...
    //	{	"created",
    //"#FileOpenDialog_Col_DateCreated",		125,	20, 10000,
    //ListPanel::COLUMN_HIDDEN,		&ListFileCreatedSortFunc
    //, Label::a_west },
...
void FileOpenDialog::PopulateFileList() {
    ...
    while ((filterPtr != NULL) && (*filterPtr != 0)) {
      ...
      while (pszFileName) {
        if (!g_pFullFileSystem->FindIsDirectory(findHandle) || !IsOSX() ||
            (IsOSX() && g_pFullFileSystem->FindIsDirectory(findHandle) &&
          ...
          kv->SetString(
              "filesize",
              Q_pretifymem(g_pFullFileSystem->Size(pFullPath), 0, true));
          Q_FixSlashes(pFullPath);
          wchar_t fileType[80];
          g_pFullFileSystem->GetFileTypeForFullPath(pFullPath, fileType,
                                                    sizeof(fileType));
          kv->SetWString("type", fileType);

          kv->SetString("attributes",
                        g_pFullFileSystem->IsFileWritable(pFullPath) ? ""
                                                                     : "R");

          time_t fileModified = g_pFullFileSystem->GetFileTime(pFullPath);
          g_pFullFileSystem->FileTimeToString(
              pszFileModified, sizeof(pszFileModified), fileModified);
          kv->SetString("modified", pszFileModified);

          //					kv->SetString( "created",
          //GetFileTimetamp( findData.ftCreationTime ) );
  ...
  while (pszFileName) {
    if (pszFileName[0] != '.' &&
        g_pFullFileSystem->FindIsDirectory(findHandle) &&
      ...
      kv->SetString("attributes",
                    g_pFullFileSystem->IsFileWritable(pFullPath) ? "" : "R");
      ...
      //			kv->SetString( "created", GetFileTimetamp(
