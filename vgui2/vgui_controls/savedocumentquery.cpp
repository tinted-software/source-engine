...
public:
  CSaveDocumentQuery(vgui::Panel *pParent, const char *filename,
                     const char *pFileType, int nContext,
                     vgui::Panel *pActionSignalTarget = 0,
...
void ShowSaveDocumentQuery(vgui::Panel *pParent, const char *pFileName,
  ...
  CSaveDocumentQuery *query =
      new CSaveDocumentQuery(pParent, pFileName, pFileType, nContext,
                             pActionSignalTarget, pPostSaveCommand);
...
// Constructor
//-----------------------------------------------------------------------------
CSaveDocumentQuery::CSaveDocumentQuery(vgui::Panel *pParent,
                                       char const *pFileName,
                                       const char *pFileType, int nContext,
                                       vgui::Panel *pActionSignalTarget,
                                       KeyValues *pPostSaveCommand)
    : BaseClass(pParent, "SaveDocumentQuery"), m_nContext(nContext),
...
void CSaveDocumentQuery::OnCommand(char const *cmd) {
  if (!Q_stricmp(cmd, "yes")) {
    ...
    if (m_pPostSaveKeyValues) {
      vgui::ivgui()->PostMessage(m_pActionSignalTarget->GetVPanel(),
                                 m_pPostSaveKeyValues->MakeCopy(), 0);
    }
    MarkForDeletion();
