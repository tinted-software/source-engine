...
// Purpose: Constructor
//-----------------------------------------------------------------------------
MessageBox::MessageBox(const wchar_t *wszTitle, const wchar_t *wszText,
                       Panel *parent)
    : Frame(parent, NULL, false) {
...
void MessageBox::DoModal(Frame *pFrameOver) {
  ...
  /*
          ...
          if (pFrameOver)
          {
                  ...
                  SetPos((frameWide - wide) / 2 + frameX, (frameTall - tall) / 2
     + frameY);
          }
          else
          {
...
void MessageBox::PerformLayout() {
  ...
  m_pMessageLabel->SetPos((wide / 2) - (m_pMessageLabel->GetWide() / 2) + x,
                          y + 5 * scale);
  if (!m_pCancelButton->IsVisible()) {
    m_pOkButton->SetPos((wide / 2) - (m_pOkButton->GetWide() / 2) + x,
                        tall - m_pOkButton->GetTall() - 15 * scale);
  } else {
    m_pOkButton->SetPos((wide / 4) - (m_pOkButton->GetWide() / 2) + x,
                        tall - m_pOkButton->GetTall() - 15 * scale);
    m_pCancelButton->SetPos((3 * wide / 4) - (m_pOkButton->GetWide() / 2) + x,
                            tall - m_pOkButton->GetTall() - 15 * scale);
  }
