...
// Purpose: Constructor
//-----------------------------------------------------------------------------
Frame::Frame(Panel *parent, const char *panelName,
             bool showTaskbarIcon /*=true*/, bool bPopup /*=true*/)
...
//			if dialog is not currently visible it starts it
...
void Frame::OnFrameFocusChanged(bool bHasFocus) {
  ...
  if (bHasFocus) {
    if (m_flFocusTransitionEffectTime && (!m_bDisableFadeEffect)) {
      GetAnimationController()->RunAnimationCommand(
          this, "BgColor", m_InFocusBgColor, 0.0f,
    ...
    if (m_flFocusTransitionEffectTime && (!m_bDisableFadeEffect)) {
      GetAnimationController()->RunAnimationCommand(
          this, "BgColor", m_OutOfFocusBgColor, 0.0f,
          m_bDisableFadeEffect ? 0.0f : m_flTransitionEffectTime,
          AnimationController::INTERPOLATOR_LINEAR);
    } else {
...
void Frame::PaintBackground() {
  ...
  if (_drawTitleBar) {
    ...
    if (_title) {
      int nTitleX = m_iTitleTextInsetXOverride ? m_iTitleTextInsetXOverride
                                               : m_iTitleTextInsetX;
...
void Frame::ApplySchemeSettings(IScheme *pScheme) {
  ...
  SetOverridableColor(&_titleBarFgColor,
                      GetSchemeColor("FrameTitleBar.TextColor", pScheme));
  SetOverridableColor(&_titleBarBgColor,
                      GetSchemeColor("FrameTitleBar.BgColor", pScheme));
  SetOverridableColor(
      &_titleBarDisabledFgColor,
      GetSchemeColor("FrameTitleBar.DisabledTextColor", pScheme));
  SetOverridableColor(&_titleBarDisabledBgColor,
  ...
  SetOverridableColor(&m_InFocusBgColor,
                      pScheme->GetColor("Frame.BgColor", GetBgColor()));
  SetOverridableColor(
      &m_OutOfFocusBgColor,
...
void Frame::FinishClose() {
  ...
  if (m_bDeleteSelfOnClose) {
    // Must be last because if vgui is not running then this will call delete
    // this!!!
    MarkForDeletion();
  }
}
...
//			it will look enabled or disabled in response to the
//title bar 			but may not activate.
//-----------------------------------------------------------------------------
...
void Frame::LoadControlSettings(const char *dialogResourceName,
  ...
  BaseClass::LoadControlSettings(dialogResourceName, pathID,
                                 pPreloadedKeyValues, pConditions);
