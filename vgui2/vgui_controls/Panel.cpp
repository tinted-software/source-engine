...
const char *g_PinCornerStrings[] = {
    "PIN_TOPLEFT",       "PIN_TOPRIGHT",
    "PIN_BOTTOMLEFT",    "PIN_BOTTOMRIGHT",

    "PIN_CENTER_TOP",    "PIN_CENTER_RIGHT",
    "PIN_CENTER_BOTTOM", "PIN_CENTER_LEFT",
};
...
#ifdef STAGING_ONLY
ConVar tf_strict_mouse_up_events("tf_strict_mouse_up_events", "0",
                                 FCVAR_ARCHIVE,
                                 "Only allow Mouse-Release events to happens "
                                 "on panels we also Mouse-Downed in");
#endif

// Temporary convar to help debug why the MvMVictoryMannUpPanel TabContainer is
// sometimes way off to the left.
ConVar tf_debug_tabcontainer("tf_debug_tabcontainer", "0", FCVAR_HIDDEN,
...
struct vgui::DragDrop_t {
  DragDrop_t()
      : m_bDragEnabled(false), m_bShowDragHelper(true), m_bDropEnabled(false),
        m_bDragStarted(false), m_nDragStartTolerance(8), m_bDragging(false),
        m_lDropHoverTime(0), m_bDropMenuShown(false),
...
Panel::CreateKeyBindingsContext(char const *filename,
                                char const *pathID /*=0*/) {
...
Panel::~Panel() {
  ...
  // Zero out our vtbl pointer. This should hopefully help us catch bad guys
  // using
...
void Panel::PaintTraverse(bool repaint, bool allowForce) {
  ...
  if (IsXbox() && !newAlphaMultiplier) {
    ...
    // xbox panels are compliant and can early out and not traverse their
    // children when they have no opacity
    return;
  }
  ...
  if (bBorderPaintFirst && repaint && _flags.IsFlagSet(PAINT_BORDER_ENABLED) &&
      (_border != null)) {
  ...
  if (repaint) {
    if (!bBorderPaintFirst && _flags.IsFlagSet(PAINT_BORDER_ENABLED) &&
        (_border != null)) {
      // Paint the border over the background with no inset
      surface()->PushMakeCurrent(vpanel, false);
      PaintBorder();
...
#ifdef _DEBUG
    // IsBuildGroupEnabled recurses up all the parents and ends up being very
    // expensive as it wanders all over memory
...
Panel *Panel::FindChildByName(const char *childName, bool recurseDown) {
  tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s - %s finding %s", __FUNCTION__,
         GetName(), childName);

  auto idx = m_dictChidlren.Find(childName);
  if (idx != m_dictChidlren.InvalidIndex()) {
    Panel *pCachedChild =
        ipanel()->GetPanel(m_dictChidlren[idx], GetControlsModuleName());
...
wchar_t const *Panel::KeyCodeModifiersToDisplayString(KeyCode code,
  ...
  V_swprintf_safe(unicode, L"%S%s", sz,
...
void Panel::OnCursorMoved(int x, int y) {
  if (ParentNeedsCursorMoveEvents()) {
    ...
    CallParentFunction(
        new KeyValues("OnCursorMoved", "x", x + thisX, "y", y + thisY));
  }
}
...
void Panel::OnRequestFocus(VPANEL subFocus, VPANEL defaultPanel) {
  CallParentFunction(new KeyValues(
      "OnRequestFocus", "subFocus", ivgui()->PanelToHandle(subFocus),
      "defaultPanel", ivgui()->PanelToHandle(defaultPanel)));
}
...
void Panel::InternalFocusChanged(bool lost) {
  /*
  //if focus is gained tell the focusNavGroup about it so its current can be
  correct if( (!lost) && (_focusNavGroup!=null) )
  {
...
void Panel::InternalPerformLayout() {
  ...
  // At worst, it leads to crashes, at best it does work that we'll redo as soon
  // as the scheme has been applied.
...
void Panel::OnCommand(const char *command) {
  if (!Q_stricmp("performlayout", command)) {
    ...
    // (this is useful for generic dialogs - otherwise, commands just get
    // ignored)
...
void Panel::MarkForDeletion() {
  ...
  // direct delete is never safe because even if ivgui is shutdown we manually
  // do RunFrame() and we can enter here in a think traverse and then delete
...
//-----------------------------------------------------------------------------
// Tells this panel that it should pin itself to the corner of a specified
// sibling panel
//-----------------------------------------------------------------------------
void Panel::PinToSibling(const char *pszSibling, PinCorner_e pinOurCorner,
                         PinCorner_e pinSibling) {
  _pinCornerToSibling = pinOurCorner;
  _pinToSiblingCorner = pinSibling;

  if (m_pinSibling.Get() && _pinToSibling && pszSibling &&
      !Q_strcmp(_pinToSibling, pszSibling))
    return;
...
void Panel::UpdateSiblingPin(void) {
  ...
  if (m_pinSibling.Get()) {
    ipanel()->SetSiblingPin(GetVPanel(), m_pinSibling->GetVPanel(),
                            _pinCornerToSibling, _pinToSiblingCorner);
  } else {
    ipanel()->SetSiblingPin(GetVPanel(), NULL);
  }
}
...
void Panel::PerformApplySchemeSettings() {
  ...
  if (_flags.IsFlagSet(NEEDS_SCHEME_UPDATE)) {
    ...
    AssertOnce(pScheme);
    if (pScheme) // this should NEVER be null, but if it is bad things would
...
ConVar panel_test_title_safe(
    "panel_test_title_safe", "0", FCVAR_CHEAT,
...
void Panel::ApplySettings(KeyValues *inResourceData) {
  ...
  _buildModeFlags &= ~(
      ...
      BUILDMODE_SAVE_WIDE_FULL | BUILDMODE_SAVE_TALL_FULL |
      BUILDMODE_SAVE_PROPORTIONAL_TO_PARENT | BUILDMODE_SAVE_WIDE_PROPORTIONAL |
      BUILDMODE_SAVE_TALL_PROPORTIONAL | BUILDMODE_SAVE_XPOS_PROPORTIONAL_SELF |
  ...
  // get the position
  int alignScreenWide,
      alignScreenTall; // screen dimensions used for pinning in splitscreen
  ...
  if (inResourceData->GetInt("proportionalToParent", 0) == 1) {
    ...
    if (GetParent() != NULL) {
      GetParent()->GetBounds(parentX, parentY, alignScreenWide,
                             alignScreenTall);
    }
  }

  // size
  int wide = ComputeWide(this, _buildModeFlags, inResourceData, alignScreenWide,
                         alignScreenTall, false);
  int tall = ComputeTall(this, _buildModeFlags, inResourceData, alignScreenWide,
                         alignScreenTall, false);
  ...
  _buildModeFlags |=
      ComputePos(this, xstr, x, wide, alignScreenWide, true, OP_SET);
  _buildModeFlags |=
      ComputePos(this, ystr, y, tall, alignScreenTall, false, OP_SET);

  bool bUsesTitleSafeArea = false;
  int titleSafeWide = 0;
  int titleSafeTall = 0;

  Rect_t excludeEdgeFromTitleSafe; // if a side is set to != 0, don't title safe
  ...
  tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s - %s: Action signal", __FUNCTION__,
         GetName());
  // Automatically add an action signal target if one is specified.  This allows
  // for nested child buttons to add their distant parents as action signal
  // targets.
  ...
  m_bForceStereoRenderToFrameBuffer =
      inResourceData->GetBool("ForceStereoRenderToFrameBuffer", false);
  ...
  tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s - %s: Pin Sibling", __FUNCTION__,
         GetName());
  const char *pszSiblingName =
      inResourceData->GetString("pin_to_sibling", NULL);
  PinCorner_e pinOurCornerToSibling = GetPinCornerFromString(
      inResourceData->GetString("pin_corner_to_sibling", NULL));
  PinCorner_e pinSiblingCorner = GetPinCornerFromString(
      inResourceData->GetString("pin_to_sibling_corner", NULL));
  PinToSibling(pszSiblingName, pinOurCornerToSibling, pinSiblingCorner);

  tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s - %s: Color overrides", __FUNCTION__,
         GetName());
  // Allow overriding of colors. Used mostly by HUD elements, where scheme color
  // usage is often undesired.
  IScheme *pScheme = vgui::scheme()->GetIScheme(GetScheme());
  for (int i = 0; i < m_OverridableColorEntries.Count(); i++) {
    // Need to ensure the key exists, so we don't overwrite existing colors when
    // it's not set.
    if (inResourceData->FindKey(m_OverridableColorEntries[i].m_pszScriptName,
                                false)) {
      // Get the color as a string - test whether it is an actual color or a
      // reference to a scheme color
      const char *pColorStr = inResourceData->GetString(
          m_OverridableColorEntries[i].m_pszScriptName);
      ...
      if (pColorStr[0] == '.' || isdigit(pColorStr[0])) {
        ...
        // First character wasn't a digit or a decimal - do a scheme color
        // lookup
        clrDest = pScheme->GetColor(pColorStr, Color(255, 255, 255, 255));
      }

      (*m_OverridableColorEntries[i].m_pColor) =
          m_OverridableColorEntries[i].m_colFromScript;
      m_OverridableColorEntries[i].m_bOverridden = true;
    }
  }

  tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s - %s: Keyboard enabled", __FUNCTION__,
         GetName());
  const char *pKeyboardInputEnabled =
      inResourceData->GetString("keyboardinputenabled", NULL);
...
//			Done post apply settings, so that baseclass settings
//don't stomp 			the script specified override colors.
//-----------------------------------------------------------------------------
...
//saved between sessions 			eg. dialog positions, last directory opened, list
//column width
//-----------------------------------------------------------------------------
          ...
          if ((DATATYPE_INT == pMap->firstParamType) &&
            ...
            (this->*((MessageFunc_ConstCharPtrConstCharPtr_t)pMap->func))(
                param1->GetString(), param2->GetString());
                     ...
                     (DATATYPE_CONSTCHARPTR == pMap->secondParamType)) {
            typedef void (Panel::*MessageFunc_IntConstCharPtr_t)(int,
                                                                 const char *);
            (this->*((MessageFunc_IntConstCharPtr_t)pMap->func))(
                param1->GetInt(), param2->GetString());
          } else if ((DATATYPE_PTR == pMap->firstParamType) &&
                     (DATATYPE_CONSTCHARPTR == pMap->secondParamType)) {
            typedef void (Panel::*MessageFunc_PtrConstCharPtr_t)(void *,
                                                                 const char *);
            ...
            (this->*((MessageFunc_PtrConstCharPtr_t)pMap->func))(
                param1->GetPtr(), param2->GetWString());
                     ...
                     (DATATYPE_CONSTCHARPTR == pMap->secondParamType)) {
            typedef void (Panel::*MessageFunc_HandleConstCharPtr_t)(
                VPANEL, const char *);
            VPANEL vp = ivgui()->HandleToPanel(param1->GetInt());
            (this->*((MessageFunc_HandleConstCharPtr_t)pMap->func))(
                vp, param2->GetString());
          } else if ((DATATYPE_HANDLE == pMap->firstParamType) &&
                     (DATATYPE_CONSTWCHARPTR == pMap->secondParamType)) {
            typedef void (Panel::*MessageFunc_HandleConstCharPtr_t)(
                VPANEL, const wchar_t *);
            VPANEL vp = ivgui()->HandleToPanel(param1->GetInt());
            (this->*((MessageFunc_HandleConstCharPtr_t)pMap->func))(
                vp, param2->GetWString());
          } else if ((DATATYPE_HANDLE == pMap->firstParamType) &&
                     (DATATYPE_HANDLE == pMap->secondParamType)) {
            typedef void (Panel::*MessageFunc_HandleConstCharPtr_t)(VPANEL,
                                                                    VPANEL);
...
void Panel::PostMessageToAllSiblings(KeyValues *msg,
...
class CProportionalIntWithScreenspacePropertyX
    : public vgui::IPanelAnimationPropertyConverter {
public:
  int ExtractValue(Panel *pPanel, const char *pszKey) {
    int nPos = 0;
    ComputePos(pPanel, pszKey, nPos, GetPanelDimension(pPanel),
               GetScreenSize(pPanel), true, OP_ADD);
    return nPos;
  }
...
class CProportionalIntWithScreenspacePropertyY
    : public CProportionalIntWithScreenspacePropertyX {
public:
...
class CProportionalWidthProperty
    : public vgui::IPanelAnimationPropertyConverter {
public:
  int ExtractValue(Panel *pPanel, const char *pszKey) {
    ...
    return Compute(pPanel, nBuildFlags, pszKey, nParentWide, nParentTall,
                   false);
  }
...
private:
  virtual int Compute(Panel *pPanel, unsigned int &nBuildFlags,
                      const char *pszKey, int nParentWide, int nParentTall,
                      bool bComputingOther) {
    KeyValuesAD kv("temp");
    kv->SetString("wide", pszKey);

    return ComputeWide(pPanel, nBuildFlags, kv, nParentWide, nParentTall,
                       false);
  }
};
...
private:
  virtual int Compute(Panel *pPanel, unsigned int &nBuildFlags,
    ...
    return ComputeTall(pPanel, nBuildFlags, kv, nParentWide, nParentTall,
                       false);
  }
};
...
void Panel::DrawBox(int x, int y, int wide, int tall, Color color,
  ...
  //=============================================================================
  // HPE_BEGIN:
  // [tj] We now check each individual corner and decide whether to draw it
  // straight or rounded
  //=============================================================================
  // TOP-LEFT
  ...
  if (ShouldDrawTopRightCornerRounded()) {
    ...
    surface()->DrawFilledRect(x + wide - cornerWide, y, x + wide,
                              y + cornerTall);
  }
  ...
  if (ShouldDrawBottomLeftCornerRounded()) {
    ...
    surface()->DrawFilledRect(x + 0, y + tall - cornerTall, x + cornerWide,
                              y + tall);
  }
  ...
  if (ShouldDrawBottomRightCornerRounded()) {
    ...
    surface()->DrawFilledRect(x + wide - cornerWide, y + tall - cornerTall,
                              x + wide, y + tall);
  }
  //=============================================================================
  // HPE_END
...
//=============================================================================
// HPE_BEGIN:
// [menglish] Draws a hollow box similar to the already existing draw hollow box
// function, but takes the indents as params
//=============================================================================
...
void Panel::OnFinishDragging(bool mousereleased, MouseCode code,
  ...
  if (m_pDragDrop->m_bDragStarted) {
    ...
    if (mousereleased && m_pDragDrop->m_hCurrentDrop != 0 &&
...
void Panel::FillRectSkippingPanel(const Color &clr, int x, int y, int w, int h,
                                  Panel *skipPanel) {
  int sx = 0, sy = 0, sw, sh;
...
public:
  CPanelMessageMapDictionary()
      : m_PanelMessageMapPool(
            sizeof(PanelMessageMap), 32, CUtlMemoryPool::GROW_FAST,
...
//-----------------------------------------------------------------------------
PanelMessageMap *
CPanelMessageMapDictionary::FindPanelMessageMap(char const *className) {
...
//-----------------------------------------------------------------------------
PanelMessageMap *
CPanelMessageMapDictionary::FindOrAddPanelMessageMap(char const *className) {
...
public:
  CPanelKeyBindingMapDictionary()
      : m_PanelKeyBindingMapPool(
            sizeof(PanelKeyBindingMap), 32, CUtlMemoryPool::GROW_FAST,
...
char const *
CPanelKeyBindingMapDictionary::StripNamespace(char const *className) {
  if (!strnicmp(className, "vgui::", 6)) {
    return className + 6;
  }
  return className;
...
//-----------------------------------------------------------------------------
PanelKeyBindingMap *
CPanelKeyBindingMapDictionary::FindPanelKeyBindingMap(char const *className) {
  int lookup = m_MessageMaps.Find(StripNamespace(className));
  if (lookup != m_MessageMaps.InvalidIndex()) {
    return m_MessageMaps[lookup].map;
  }
...
void VguiPanelGetSortedChildPanelList(Panel *pParentPanel,
                                      void *pSortedPanels) {
  CUtlSortVector<SortedPanel_t, CSortedPanelYLess> *pList =
      reinterpret_cast<CUtlSortVector<SortedPanel_t, CSortedPanelYLess> *>(
          pSortedPanels);
...
int ComputeWide(Panel *pPanel, unsigned int &nBuildFlags,
                KeyValues *inResourceData, int nParentWide, int nParentTall,
                bool bComputingOther) {
  ...
  if (wstr) {
    if (wstr[0] == 'f' || wstr[0] == 'F') {
      ...
      if (wstr[0] == 'o' || wstr[0] == 'O') {
        ...
        wide = ComputeTall(pPanel, nBuildFlags, inResourceData, nParentWide,
                           nParentTall, true);
    ...
    if (nBuildFlags & Panel::BUILDMODE_SAVE_WIDE_PROPORTIONAL_TALL) {
      ...
      if (pPanel->IsProportional()) {
        ...
        wide =
            scheme()->GetProportionalScaledValueEx(pPanel->GetScheme(), wide);
      }
...
int ComputeTall(Panel *pPanel, unsigned int &nBuildFlags,
                KeyValues *inResourceData, int nParentWide, int nParentTall,
                bool bComputingOther) {
  int tall = pPanel->GetTall();

  // allow tall to be use the "fill" option, set to the height of the
  // parent/screen
  ...
  if (tstr) {
    if (tstr[0] == 'f' || tstr[0] == 'F') {
      ...
      if (tstr[0] == 'o' || tstr[0] == 'O') {
        ...
        tall = ComputeWide(pPanel, nBuildFlags, inResourceData, nParentWide,
                           nParentTall, true);
        if (pPanel->IsProportional()) {
    ...
    if (nBuildFlags & Panel::BUILDMODE_SAVE_TALL_PROPORTIONAL_WIDE) {
      ...
      if (pPanel->IsProportional()) {
        // scale the height up to our screen co-ords
        tall =
            scheme()->GetProportionalScaledValueEx(pPanel->GetScheme(), tall);
      }
      // now correct the alignment
...
int ComputePos(Panel *pPanel, const char *pszInput, int &nPos, const int &nSize,
               const int &nParentSize, const bool &bX, EOperator eOp) {
  const int nFlagRightAlign = bX ? Panel::BUILDMODE_SAVE_XPOS_RIGHTALIGNED
                                 : Panel::BUILDMODE_SAVE_YPOS_BOTTOMALIGNED;
  const int nFlagCenterAlign = bX ? Panel::BUILDMODE_SAVE_XPOS_CENTERALIGNED
                                  : Panel::BUILDMODE_SAVE_YPOS_CENTERALIGNED;
  const int nFlagProportionalSelf =
      bX ? Panel::BUILDMODE_SAVE_XPOS_PROPORTIONAL_SELF
         : Panel::BUILDMODE_SAVE_YPOS_PROPORTIONAL_SELF;
  const int nFlagProportionalParent =
      bX ? Panel::BUILDMODE_SAVE_XPOS_PROPORTIONAL_PARENT
         : Panel::BUILDMODE_SAVE_YPOS_PROPORTIONAL_PARENT;
  ...
  if (pszInput) {
    ...
    if (pPanel->IsProportional()) {
      int nOldPos = nNewPos;
      nNewPos =
          scheme()->GetProportionalScaledValueEx(pPanel->GetScheme(), nNewPos);
  ...
  if (tf_debug_tabcontainer.GetBool() &&
      !Q_stricmp("TabContainer", pPanel->GetName())) {
    Msg("TabContainer nFlags:%x nPos:%d nParentSize:%d nPosDelta:%d nSize:%d "
        "GetParent:%p (%s) pszInput:'%s'\n",
