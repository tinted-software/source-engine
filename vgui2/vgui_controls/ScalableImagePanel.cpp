...
void ScalableImagePanel::PaintBackground() {
  ...
  surface()->DrawSetColor(m_DrawColor.r(), m_DrawColor.g(), m_DrawColor.b(),
                          GetAlpha());
...
void ScalableImagePanel::ApplySettings(KeyValues *inResourceData) {
  ...
  if (IsProportional()) {
    ...
    m_iCornerHeight =
        scheme()->GetProportionalScaledValueEx(GetScheme(), m_iCornerHeight);
    m_iCornerWidth =
        scheme()->GetProportionalScaledValueEx(GetScheme(), m_iCornerWidth);
  }
...
void ScalableImagePanel::PerformLayout(void) {
  ...
  m_flCornerWidthPercent =
      (wide > 0) ? ((float)m_iSrcCornerWidth / (float)wide) : 0;
  m_flCornerHeightPercent =
      (tall > 0) ? ((float)m_iSrcCornerHeight / (float)tall) : 0;
}
...
const char *ScalableImagePanel::GetDescription() {
  ...
  _snprintf(buf, sizeof(buf),
            "%s string image, int src_corner_height, int src_corner_width, int "
            "draw_corner_height, int draw_corner_width",
