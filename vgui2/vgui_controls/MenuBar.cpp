...
MenuBar::MenuBar(Panel *parent, const char *panelName)
    : Panel(parent, panelName), m_nRightEdge(0) {}
...
void MenuBar::PerformLayout() {
  ...
  for (int i = 0; i < m_pMenuButtons.Count(); ++i) {
    ...
    m_pMenuButtons[i]->SetSize(nWide + Label::Content,
                               nBarHeight - 2 * MENUBARINDENT);
...
//-----------------------------------------------------------------------------
// Purpose: Get the size of the menus in the bar (so other children can be added
// to menu bar) Input  : w -
//			int&h -
//-----------------------------------------------------------------------------
...
// Purpose: Message map
//-----------------------------------------------------------------------------
void MenuBar::OnMenuClose() { RequestFocus(); }
  ...
  for (int i = 0; i < m_pMenuButtons.Count(); i++) {
    ...
    if (m_pMenuButtons[i]->IsDepressed()) {
      ...
      for (int j = 0; j < m_pMenuButtons.Count(); j++) {
        MenuButton *button = static_cast<MenuButton *>(
            ipanel()->GetPanel(menuButton, GetModuleName()));
        // it is one of ours.
