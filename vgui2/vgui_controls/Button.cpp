...
void Button::Init() {
  ...
  REGISTER_COLOR_AS_OVERRIDABLE(_keyboardFocusColor,
...
void Button::ApplySchemeSettings(IScheme *pScheme) {
  ...
  _selectedFgColor =
      GetSchemeColor("Button.SelectedTextColor", _selectedFgColor, pScheme);
  _selectedBgColor =
      GetSchemeColor("Button.SelectedBgColor", _selectedBgColor, pScheme);
...
void Button::ApplySettings(KeyValues *inResourceData) {
  ...
  m_bStaySelectedOnClick =
      inResourceData->GetBool("stayselectedonclick", false);
  ...
  int iButtonActivationType =
      inResourceData->GetInt("button_activation_type", -1);
...
void Button::OnKeyCodePressed(KeyCode code) {
  ...
  if ((localCode == KEY_XBUTTON_A || localCode == STEAMCONTROLLER_A) &&
      IsEnabled()) {
    SetArmed(true);
...
void Button::OnKeyCodeReleased(KeyCode keycode) {
  ...
  if (_buttonFlags.IsFlagSet(BUTTON_KEY_DOWN) &&
      (code == KEY_XBUTTON_A || code == KEY_XBUTTON_START ||
       code == STEAMCONTROLLER_A)) {
    SetArmed(true);
    if (_activationType != ACTIVATE_ONPRESSED) {
      DoClick();
    }
  } else if (_buttonFlags.IsFlagSet(BUTTON_KEY_DOWN) &&
  ...
  if (!(code == KEY_XSTICK1_UP || code == KEY_XSTICK1_DOWN ||
        code == KEY_XSTICK1_LEFT || code == KEY_XSTICK1_RIGHT ||
        code == KEY_XSTICK2_UP || code == KEY_XSTICK2_DOWN ||
        code == KEY_XSTICK2_LEFT || code == KEY_XSTICK2_RIGHT ||
        code == KEY_XBUTTON_UP || code == KEY_XBUTTON_DOWN ||
        code == KEY_XBUTTON_LEFT || code == KEY_XBUTTON_RIGHT ||
