...
public:
  ...
  int GetViewTextIndex() { return _viewIndex; }
...
void RichText::Paint() {
  ...
  for (int i = startIndex; i < m_TextStream.Count() && renderState.y < tall;) {
    // 1.
    // Update our current render state based on the formatting and color
    // streams, this has to happen if it's our very first iteration, or if we
    ...
    // 2.
    // if we've passed a line break go to that
    if (m_LineBreaks.IsValidIndex(lineBreakIndexIndex) &&
      ...
      // Skip white space unless the previous line ended from the hard carriage
      // return
    ...
    if (m_LineBreaks.IsValidIndex(lineBreakIndexIndex) &&
        m_LineBreaks[lineBreakIndexIndex] < iLim)
...
void RichText::RecalculateLineBreaks() {
  ...
  for (int i = startChar; i < m_TextStream.Count(); ++i) {
    ...
    if (bForceBreak || ch == '\r' || ch == '\n') {
      ...
      if (ch == '\r' || ch == '\n') {
      ...
      } else if (bPreviousWordStartedOnNewLine ||
                 iPreviousWordStartIndex <= lineStartIndex) {
        lineStartIndex = i;
        ...
        // If the word is clickable, and that started prior to the beginning of
        // the word, then we must split the click panel
        if (renderStateAtLastWord.textClickable &&
            m_FormatStream[renderStateAtLastWord.formatStreamIndex]
                    .textStreamIndex < i) {
          // need to split the url into two panels
          int oldIndex =
              _clickableTextPanels[clickableTextNum - 1]->GetTextIndex();
...
void RichText::OnKeyCodeTyped(KeyCode code) {
  ...
  if (ctrl || (winkey && IsOSX())) {
    ...
    default: {
      ...
      // and we don't want select[1] changed unless a char was pressed that this
      // fxn handles
      return;
    }
    }
...
int RichText::GetStartDrawIndex(int &lineBreakIndexIndex) {
  ...
  if (startLine >= m_LineBreaks.Count()) // incase the line breaks got reset and
                                         // the scroll bar hasn't
  {
  ...
  while (text[i] != 0) {
    ...
    if (!Q_strnicmp(text + i, "<a href=", 8)) {
      ...
      Q_strncpy(pchURLText, text + i,
                min((int)(pchURLEnd - text) - i + 1, cchURLText));
      i += (pchURLEnd - text - i);
      i += Q_strlen("</a>");
...
void RichText::OnTextClicked(const wchar_t *wszText) {
  // Strip leading/trailing quotes, which may be present on href tags or may
  // not.
  ...
  if (m_hPanelToHandleClickingURLs.Get()) {
    PostMessage(m_hPanelToHandleClickingURLs.Get(),
                new KeyValues("URLClicked", "url", ansi));
  } else {
