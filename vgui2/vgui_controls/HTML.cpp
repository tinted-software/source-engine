...
class HTMLPopup : public vgui::Frame {
  ...
  public:
    PopupHTML(Frame *parent, const char *pchName, bool allowJavaScript,
              bool bPopupWindow)
...
public:
  ...
  enum { vert_inset = 40, horiz_inset = 6 };
  ...
  MESSAGE_FUNC(OnCloseWindow, "OnCloseWindow") { Close(); }
...
HTML::HTML(Panel *parent, const char *name, bool allowJavaScript,
           bool bPopupWindow)
    : Panel(parent, name), m_NeedsPaint(this, &HTML::BrowserNeedsPaint),
  ...
  if (m_SteamAPIContext.SteamHTMLSurface()) {
    ...
    SteamAPICall_t hSteamAPICall =
        m_SteamAPIContext.SteamHTMLSurface()->CreateBrowser(
            surface()->GetWebkitHTMLUserAgentString(), NULL);
    m_SteamCallResultBrowserReady.Set(hSteamAPICall, this,
                                      &HTML::OnBrowserReady);
  } else {
  ...
  m_pContextMenu->AddMenuItem(
      "#vgui_HTMLBack", new KeyValues("Command", "command", "back"), this);
  m_pContextMenu->AddMenuItem("#vgui_HTMLForward",
                              new KeyValues("Command", "command", "forward"),
                              this);
  m_pContextMenu->AddMenuItem(
      "#vgui_HTMLReload", new KeyValues("Command", "command", "reload"), this);
  m_pContextMenu->AddMenuItem(
      "#vgui_HTMLStop", new KeyValues("Command", "command", "stop"), this);
  m_pContextMenu->AddSeparator();
  m_pContextMenu->AddMenuItem("#vgui_HTMLCopyUrl",
                              new KeyValues("Command", "command", "copyurl"),
                              this);
  m_iCopyLinkMenuItemID = m_pContextMenu->AddMenuItem(
      "#vgui_HTMLCopyLink", new KeyValues("Command", "command", "copylink"),
      this);
  m_pContextMenu->AddMenuItem(
      "#TextEntry_Copy", new KeyValues("Command", "command", "copy"), this);
  m_pContextMenu->AddMenuItem(
      "#TextEntry_Paste", new KeyValues("Command", "command", "paste"), this);
  m_pContextMenu->AddSeparator();
  m_nViewSourceAllowedIndex = m_pContextMenu->AddMenuItem(
      "#vgui_HTMLViewSource", new KeyValues("Command", "command", "viewsource"),
      this);
}
...
//-----------------------------------------------------------------------------
void HTML::OnSetCursorVGUI(int cursor) { SetCursor((HCursor)cursor); }
...
void HTML::Paint() {
  ...
  if (m_iScrollBorderX > 0 || m_iScrollBorderY > 0) {
    ...
    if (m_iScrollBorderX) {
      surface()->DrawFilledRect(w - m_iScrollBorderX - right, top, w,
                                h - bottom);
    }
    if (m_iScrollBorderY) {
      surface()->DrawFilledRect(left, h - m_iScrollBorderY - bottom,
                                w - m_iScrollBorderX - right, h);
    }
  }
}
...
void HTML::PerformLayout() {
  ...
  const char *resourceString =
      pClientScheme->GetResourceString("HTML.SearchInsetY");
  if (resourceString) {
  ...
  m_pFindBar->SetBounds(GetWide() - iSearchWide - iSearchInsetX - vbarInset,
                        m_pFindBar->BIsHidden() ? -1 * iSearchTall - 5
                                                : iSearchInsetY,
  ...
  if (IsSteamInOfflineMode() && !force) {
    ...
    if (baseDir) {
      ...
      if (!g_pFullFileSystem->FileExists(baseDir)) {
        Q_snprintf(otherName, sizeof(otherName), "%senglish.html",
                   OFFLINE_FILE);
        baseDir = otherName;
      }
      ...
      if (m_SteamAPIContext.SteamHTMLSurface())
        m_SteamAPIContext.SteamHTMLSurface()->LoadURL(m_unBrowserHandle,
                                                      htmlLocation, NULL);
    } else {
      if (m_SteamAPIContext.SteamHTMLSurface())
        m_SteamAPIContext.SteamHTMLSurface()->LoadURL(m_unBrowserHandle, URL,
                                                      NULL);
    }
  } else {
...
//-----------------------------------------------------------------------------
bool HTML::BCanGoBack() { return m_bCanGoBack; }

//-----------------------------------------------------------------------------
// Purpose: Checks if the browser can go forward further
//-----------------------------------------------------------------------------
bool HTML::BCanGoFoward() { return m_bCanGoForward; }
...
void HTML::RunJavascript(const char *pchScript) {
  if (m_SteamAPIContext.SteamHTMLSurface())
    m_SteamAPIContext.SteamHTMLSurface()->ExecuteJavascript(m_unBrowserHandle,
                                                            pchScript);
}
...
void HTML::OnMousePressed(MouseCode code) {
  ...
  if (code != MOUSE_RIGHT) {
    if (m_SteamAPIContext.SteamHTMLSurface())
      m_SteamAPIContext.SteamHTMLSurface()->MouseDown(
          m_unBrowserHandle, ConvertMouseCodeToCEFCode(code));
  }
...
void HTML::OnMouseReleased(MouseCode code) {
  if (code == MOUSE_LEFT) {
    ...
    if (!m_sDragURL.IsEmpty() && input()->GetMouseOver() != GetVPanel() &&
        input()->GetMouseOver()) {
      ...
      if (ipanel()->RequestInfo(input()->GetMouseOver(), kv) &&
        ...
        ivgui()->PostMessage(
            vpanel, new KeyValues("DragDrop", "text", m_sDragURL.Get()),
            GetVPanel());
      }
    }
    m_sDragURL = NULL;
  ...
  if (m_SteamAPIContext.SteamHTMLSurface())
    m_SteamAPIContext.SteamHTMLSurface()->MouseUp(
        m_unBrowserHandle, ConvertMouseCodeToCEFCode(code));
}
...
void HTML::OnCursorMoved(int x, int y) {
  ...
  if (vgui::input()->GetMouseOver() == GetVPanel()) {
    ...
    if (!input()->GetMouseOver()) {
      // we're not over any vgui window, switch to the OS implementation of
      // drag/drop BR FIXME
  ...
  if (!m_sDragURL.IsEmpty() && !input()->GetCursorOveride()) {
    // if we've dragged far enough (in global coordinates), set to use the drag
    // cursor
    int gx, gy;
...
void HTML::OnMouseDoublePressed(MouseCode code) {
  if (m_SteamAPIContext.SteamHTMLSurface())
    m_SteamAPIContext.SteamHTMLSurface()->MouseDoubleClick(
        m_unBrowserHandle, ConvertMouseCodeToCEFCode(code));
}
...
#ifdef OSX
  // for now pipe through the cmd-key to be like the control key so we get
  // copy/paste
...
void HTML::OnKeyTyped(wchar_t unichar) {
  if (m_SteamAPIContext.SteamHTMLSurface())
    m_SteamAPIContext.SteamHTMLSurface()->KeyChar(
        m_unBrowserHandle, unichar,
...
void HTML::ShowFindDialog() {
  ...
  GetAnimationController()->RunAnimationCommand(
      m_pFindBar, "ypos", iSearchInsetY, 0.0f, flAnimationTime,
...
void HTML::HideFindDialog() {
  ...
  float flAnimationTime = 0.0f;
  const char *resourceString =
      pClientScheme->GetResourceString("HTML.SearchAnimationTime");
  if (resourceString) {
    flAnimationTime = atof(resourceString);
  }

  GetAnimationController()->RunAnimationCommand(
      m_pFindBar, "ypos", -1 * h - 5, 0.0f, flAnimationTime,
      AnimationController::INTERPOLATOR_LINEAR);
...
void HTML::OnKeyCodeTyped(KeyCode code) {
  ...
  if (m_SteamAPIContext.SteamHTMLSurface())
    m_SteamAPIContext.SteamHTMLSurface()->KeyDown(
        m_unBrowserHandle, KeyCode_VGUIToVirtualKey(code),
        (ISteamHTMLSurface::EHTMLKeyModifiers)GetKeyModifiers());
}
...
void HTML::OnKeyCodeReleased(KeyCode code) {
  if (m_SteamAPIContext.SteamHTMLSurface())
    m_SteamAPIContext.SteamHTMLSurface()->KeyUp(
        m_unBrowserHandle, KeyCode_VGUIToVirtualKey(code),
        (ISteamHTMLSurface::EHTMLKeyModifiers)GetKeyModifiers());
}
...
void HTML::OnMouseWheeled(int delta) {
  ...
  if (m_SteamAPIContext.SteamHTMLSurface())
    m_SteamAPIContext.SteamHTMLSurface()->MouseWheel(m_unBrowserHandle,
                                                     delta * 100.0 / 3.0);
}
...
void HTML::AddCustomURLHandler(const char *customProtocolName,
  ...
  Q_strncpy(m_CustomURLHandlers[index].url, customProtocolName,
...
void HTML::BrowserResize() {
  ...
  */
  ...
  if (m_iWideLastHTMLSize != (w - m_iScrollBorderX - right) ||
    ...
    {
      if (m_SteamAPIContext.SteamHTMLSurface())
        m_SteamAPIContext.SteamHTMLSurface()->SetSize(
            m_unBrowserHandle, m_iWideLastHTMLSize, m_iTalLastHTMLSize);
    }

    // webkit forgets the scroll offset when you resize (it saves the scroll in
    // a DC and a resize throws away the DC) so just tell it after the resize
    int scrollV = _vbar->GetValue();
    int scrollH = _hbar->GetValue();

    if (m_SteamAPIContext.SteamHTMLSurface())
      m_SteamAPIContext.SteamHTMLSurface()->SetHorizontalScroll(
          m_unBrowserHandle, scrollH);
    if (m_SteamAPIContext.SteamHTMLSurface())
      m_SteamAPIContext.SteamHTMLSurface()->SetVerticalScroll(m_unBrowserHandle,
                                                              scrollV);
  }
}
...
// Purpose: data accessor
//-----------------------------------------------------------------------------
bool HTML::IsScrollbarVisible() { return _vbar->IsVisible(); }
...
void HTML::PostChildPaint() {
  ...
  // TODO::STYLE
  // m_pInteriorPanel->SetPaintAppearanceEnabled( true ); // turn painting back
  // on so the IE hwnd can render this border
}
...
void HTML::AddHeader(const char *pchHeader, const char *pchValue) {
  if (m_SteamAPIContext.SteamHTMLSurface())
    m_SteamAPIContext.SteamHTMLSurface()->AddHeader(m_unBrowserHandle,
                                                    pchHeader, pchValue);
}
...
void HTML::OnCommand(const char *pchCommand) {
  if (!Q_stricmp(pchCommand, "back")) {
    ...
    if (m_SteamAPIContext.SteamHTMLSurface())
      m_SteamAPIContext.SteamHTMLSurface()->PasteFromClipboard(
          m_unBrowserHandle);
...
void HTML::OnFileSelected(const char *pchSelectedFile) {
  ...
  if (m_SteamAPIContext.SteamHTMLSurface())
    m_SteamAPIContext.SteamHTMLSurface()->FileLoadDialogResponse(
        m_unBrowserHandle, ppchSelectedFiles);
...
void HTML::Find(const char *pchSubStr) {
  m_bInFind = false;
  if (m_sLastSearchString ==
      pchSubStr) // same string as last time, lets fine next
    m_bInFind = true;

  m_sLastSearchString = pchSubStr;

  if (m_SteamAPIContext.SteamHTMLSurface())
    m_SteamAPIContext.SteamHTMLSurface()->Find(m_unBrowserHandle, pchSubStr,
                                               m_bInFind, false);
}
...
void HTML::FindPrevious() {
  if (m_SteamAPIContext.SteamHTMLSurface())
    m_SteamAPIContext.SteamHTMLSurface()->Find(
        m_unBrowserHandle, m_sLastSearchString, m_bInFind, true);
}

//-----------------------------------------------------------------------------
// Purpose: find any text on the html page with this sub string
//-----------------------------------------------------------------------------
void HTML::FindNext() { Find(m_sLastSearchString); }
...
//-----------------------------------------------------------------------------
void HTML::OnEditNewLine(Panel *pPanel) { OnTextChanged(pPanel); }
...
void HTML::BrowserNeedsPaint(HTML_NeedsPaint_t *pCallback) {
  ...
  if (m_iHTMLTextureID != 0 &&
      ((_vbar->IsVisible() && pCallback->unScrollY > 0 &&
  ...
  if (m_bNeedsFullTextureUpload || m_iHTMLTextureID == 0 ||
      tw != (int)pCallback->unWide || tt != (int)pCallback->unTall) {
    m_bNeedsFullTextureUpload = false;
    if (m_iHTMLTextureID != 0)
      surface()->DeleteTextureByID(m_iHTMLTextureID);

    // if the dimensions changed we also need to re-create the texture ID to
    // support the overlay properly (it won't resize a texture on the fly, this
    // is the only control that needs
    ...
    surface()->DrawSetTextureRGBAEx(
        m_iHTMLTextureID, (const unsigned char *)pCallback->pBGRA,
        pCallback->unWide, pCallback->unTall,
        IMAGE_FORMAT_BGRA8888); // BR FIXME - this call seems to shift by some
                                // number of pixels?
    m_allocedTextureWidth = pCallback->unWide;
    m_allocedTextureHeight = pCallback->unTall;
  } else if ((int)pCallback->unUpdateWide > 0 &&
             (int)pCallback->unUpdateTall > 0) {
    // same size texture, just bits changing in it, lets twiddle
    surface()->DrawUpdateRegionTextureRGBA(
        m_iHTMLTextureID, pCallback->unUpdateX, pCallback->unUpdateY,
        (const unsigned char *)pCallback->pBGRA, pCallback->unUpdateWide,
        pCallback->unUpdateTall, IMAGE_FORMAT_BGRA8888);
  } else {
    surface()->DrawSetTextureRGBAEx(
        m_iHTMLTextureID, (const unsigned char *)pCallback->pBGRA,
        pCallback->unWide, pCallback->unTall, IMAGE_FORMAT_BGRA8888);
  }
...
//-----------------------------------------------------------------------------
bool HTML::OnStartRequest(const char *url, const char *target,
                          const char *pchPostData, bool bIsRedirect) {
  if (!url || !Q_stricmp(url, "about:blank"))
    return true; // this is just webkit loading a new frames contents inside an
                 // existing page
  ...
  for (int i = 0; i < m_CustomURLHandlers.Count(); i++) {
    if (!Q_strnicmp(m_CustomURLHandlers[i].url, url,
                    Q_strlen(m_CustomURLHandlers[i].url))) {
      // we have a custom handler
      Panel *targetPanel = m_CustomURLHandlers[i].hPanel;
      if (targetPanel) {
        PostMessage(targetPanel, new KeyValues("CustomURL", "url",
  ...
  if (m_bNewWindowsOnly && bIsRedirect) {
    if (target &&
        (!Q_stricmp(target, "_blank") ||
         !Q_stricmp(target, "_new"))) // only allow NEW windows (_blank ones)
    {
      return true;
    } else {
      return false;
...
void HTML::BrowserStartRequest(HTML_StartRequest_t *pCmd) {
  bool bRes = OnStartRequest(pCmd->pchURL, pCmd->pchTarget, pCmd->pchPostData,
                             pCmd->bIsRedirect);

  if (m_SteamAPIContext.SteamHTMLSurface())
    m_SteamAPIContext.SteamHTMLSurface()->AllowStartRequest(m_unBrowserHandle,
                                                            bRes);
}
...
void HTML::BrowserFinishedRequest(HTML_FinishedRequest_t *pCmd) {
  ...
  if (pCmd->pchPageTitle && pCmd->pchPageTitle[0])
    PostActionSignal(
        new KeyValues("PageTitleChange", "title", pCmd->pchPageTitle));
...
void HTML::BrowserOpenNewTab(HTML_OpenLinkInNewTab_t *pCmd) {
  (pCmd);
  // Not supported by default, if a child class overrides us and knows how to
  // handle tabs, then it can do this.
}
...
void HTML::BrowserSetHTMLTitle(HTML_ChangedTitle_t *pCmd) {
  PostMessage(GetParent(),
              new KeyValues("OnSetHTMLTitle", "title", pCmd->pchTitle));
...
//-----------------------------------------------------------------------------
void HTML::BrowserHorizontalScrollBarSizeResponse(
    HTML_HorizontalScroll_t *pCmd) {
...
void HTML::BrowserLinkAtPositionResponse(HTML_LinkAtPosition_t *pCmd) {
  ...
  m_pContextMenu->SetItemVisible(m_iCopyLinkMenuItemID,
                                 !m_LinkAtPos.m_sURL.IsEmpty() ? true : false);
  ...
  if (m_bRequestingCopyLink) {
    ...
    if (!m_LinkAtPos.m_sURL.IsEmpty())
      system()->SetClipboardText(m_LinkAtPos.m_sURL,
                                 m_LinkAtPos.m_sURL.Length());
    else
...
void HTML::BrowserJSAlert(HTML_JSAlert_t *pCmd) {
  MessageBox *pDlg =
      new MessageBox(m_sCurrentURL, (const char *)pCmd->pchMessage, this);
...
void HTML::BrowserJSConfirm(HTML_JSConfirm_t *pCmd) {
  QueryBox *pDlg =
      new QueryBox(m_sCurrentURL, (const char *)pCmd->pchMessage, this);
  pDlg->AddActionSignalTarget(this);
...
void HTML::DismissJSDialog(int bResult) {
  if (m_SteamAPIContext.SteamHTMLSurface())
    m_SteamAPIContext.SteamHTMLSurface()->JSDialogResponse(m_unBrowserHandle,
                                                           bResult);
};
...
void HTML::GetLinkAtPosition(int x, int y) {
  if (m_SteamAPIContext.SteamHTMLSurface())
    m_SteamAPIContext.SteamHTMLSurface()->GetLinkAtPosition(m_unBrowserHandle,
                                                            x, y);
}
