  ...
  while (1) {
    ...
    if (!_topLevelFocus) {
      ...
      if (_mainPanel->GetVParent() &&
          _mainPanel->GetVParent() != surface()->GetEmbeddedPanel()) {
        // we're not a top level panel, so forward up the request instead of
        // looping
        if (ipanel()->RequestFocusPrev(_mainPanel->GetVParent(),
                                       _mainPanel->GetVPanel())) {
          bFound = true;
  ...
  if (best) {
    ...
    if (!CanButtonBeDefault(best->GetVPanel())) {
      if (_defaultButton) {
        ...
        if (_mainPanel->GetVParent()) {
          ivgui()->PostMessage(_mainPanel->GetVParent(),
                               new KeyValues("FindDefaultButton"), NULL);
        }
      }
    } else {
  ...
  while (1) {
    ...
    if (!_topLevelFocus) {
      if (_mainPanel->GetVParent() &&
        ...
        // looping
        if (stack_depth < 15) {
          if (ipanel()->RequestFocusNext(_mainPanel->GetVParent(),
                                         _mainPanel->GetVPanel())) {
            bFound = true;
            SetCurrentDefaultButton(NULL);
            break;
...
//-----------------------------------------------------------------------------
// Purpose: sets the panel that owns this FocusNavGroup to be the root in the
// focus traversal heirarchy
//-----------------------------------------------------------------------------
void FocusNavGroup::SetFocusTopLevel(bool state) { _topLevelFocus = state; }
...
// Purpose: sets panel which receives input when ENTER is hit
//-----------------------------------------------------------------------------
void FocusNavGroup::SetCurrentDefaultButton(
    VPANEL panel, bool sendCurrentDefaultButtonMessage) {
  if (panel == _currentDefaultButton.Get())
    return;

  if (sendCurrentDefaultButtonMessage && _currentDefaultButton.Get() != 0) {
    ivgui()->PostMessage(_currentDefaultButton,
                         new KeyValues("SetAsCurrentDefaultButton", "state", 0),
                         NULL);
  }
  ...
  if (sendCurrentDefaultButtonMessage && _currentDefaultButton.Get() != 0) {
    ivgui()->PostMessage(_currentDefaultButton,
                         new KeyValues("SetAsCurrentDefaultButton", "state", 1),
                         NULL);
  }
}
...
// Purpose: sets panel which receives input when ENTER is hit
//-----------------------------------------------------------------------------
VPANEL FocusNavGroup::GetDefaultButton() { return _defaultButton; }
