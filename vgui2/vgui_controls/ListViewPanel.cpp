  ...
  void UpdateImage() {
    if (m_pListViewPanel->m_pImageList) {
      ...
      if (m_pListViewPanel->m_pImageList->IsValidIndex(imageIndex)) {
        SetImageAtIndex(0, m_pListViewPanel->m_pImageList->GetImage(imageIndex),
                        0);
      } else {
        // use the default
...
private:
  ...
  // IBorder			  *_keyFocusBorder;		// maybe in the
  // future when I'm the 'active' but not selected item, I'll have a border
...
// Purpose:
//-----------------------------------------------------------------------------
int ListViewPanel::AddItem(const KeyValues *data, bool bScrollToItem,
                           bool bSortOnAdd) {
...
// Purpose:
//-----------------------------------------------------------------------------
int ListViewPanel::GetItemCount() { return m_DataItems.Count(); }
...
void ListViewPanel::DeleteAllItems() {
  FOR_EACH_LL(m_DataItems, index) { m_DataItems[index]->MarkForDeletion(); }
...
// Purpose:
//-----------------------------------------------------------------------------
int ListViewPanel::InvalidItemID() { return m_DataItems.InvalidIndex(); }
...
void ListViewPanel::SortList() {
  ...
  for (int i = m_DataItems.Head(); i != m_DataItems.InvalidIndex();
       i = m_DataItems.Next(i)) {
    // insert the items sorted
    if (m_pSortFunc) {
      int insertionPoint;
      for (insertionPoint = 0; insertionPoint < m_SortedItems.Count();
           insertionPoint++) {
        if (m_pSortFunc(m_DataItems[i]->GetData(),
                        m_DataItems[m_SortedItems[insertionPoint]]->GetData()))
          break;
      }
...
// Purpose:
//-----------------------------------------------------------------------------
void ListViewPanel::SetImageList(ImageList *imageList,
  ...
  FOR_EACH_LL(m_DataItems, i) { m_DataItems[i]->UpdateImage(); }
}
...
// Purpose:
//-----------------------------------------------------------------------------
int ListViewPanel::GetSelectedItemsCount() { return m_SelectedItems.Count(); }
...
void ListViewPanel::PerformLayout() {
  ...
  if (cols > maxColVisible) {
    ...
    cols = (GetItemCount() + (itemsPerCol - 1)) /
           (itemsPerCol > 0 ? itemsPerCol : 1);
...
// Purpose:
//-----------------------------------------------------------------------------
void ListViewPanel::Paint() { BaseClass::Paint(); }
...
void ListViewPanel::OnShiftSelect(int itemID) {
  // if we dont' have a valid selected ItemID - then we just choose the first
  // item
  if (!m_DataItems.IsValidIndex(m_ShiftStartItemID)) {
    m_ShiftStartItemID = m_DataItems.Head();
  }

  // find out if the just pressed item is "earlier" or is the 'last selected
  // item'
  ...
  for (i = lowerPos; i <= upperPos; i++) {
    // do not use AddSelectedItem because we don't want to switch the
    // shiftStartItemID
...
// Purpose:
//-----------------------------------------------------------------------------
void ListViewPanel::OnItemMouseDoublePressed(ListViewItem *pItem,
                                             MouseCode code) {
  if (code == MOUSE_LEFT) {
...
void ListViewPanel::OnKeyTyped(wchar_t unichar) {
  if (!iswcntrl(unichar)) {
    ...
    if (m_SortedItems.IsValidIndex(itemPos)) {
      ...
      // if the after this item we couldn't fine an item with  the same letter,
      // fall through and just start from the beginning of list to the last
      // selected item
    }

    for (i = 0; i < m_SortedItems.Count(); i++) {
      // we've gone all the way around - break - if we had a valid index, this
      // is one more that last selectedItem, if not it's an illegal index
      if (i == itemPos)
        break;
