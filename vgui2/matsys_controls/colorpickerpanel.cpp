...
ColorChannel_t CColorXYPreview::s_pHSVRemapX[3] = {CHANNEL_SATURATION,
                                                   CHANNEL_HUE, CHANNEL_HUE};

ColorChannel_t CColorXYPreview::s_pHSVRemapY[3] = {CHANNEL_VALUE, CHANNEL_VALUE,
                                                   CHANNEL_SATURATION};

ColorChannel_t CColorXYPreview::s_pRGBRemapX[3] = {CHANNEL_BLUE, CHANNEL_BLUE,
                                                   CHANNEL_RED};

ColorChannel_t CColorXYPreview::s_pRGBRemapY[3] = {CHANNEL_GREEN, CHANNEL_RED,
                                                   CHANNEL_GREEN};

//-----------------------------------------------------------------------------
// Constructor
//-----------------------------------------------------------------------------
CColorXYPreview::CColorXYPreview(vgui::Panel *pParent, const char *pName)
...
void CColorXYPreview::Paint(void) {
  ...
  if (m_Type != COLOR_TYPE_RGB) {
    ...
    y = GetImageHeight() - 1 -
        (int)(vecHSVNormalized[s_pHSVRemapY[m_Channel]] + 0.5f);
  } else {
    x = ((unsigned char *)&m_CurrentColor)[s_pRGBRemapX[m_Channel]];
    y = GetImageHeight() - 1 -
        ((unsigned char *)&m_CurrentColor)[s_pRGBRemapY[m_Channel]];
  }
...
// Constructor
//-----------------------------------------------------------------------------
CColorZPreview::CColorZPreview(vgui::Panel *pParent, const char *pName)
    : BaseClass(pParent, pName) {
  Init(8, 256, false);
  m_CurrentColor.r = m_CurrentColor.g = m_CurrentColor.b = 255;
...
void CColorZPreview::ComputeHSVColorForPoint(int y, bool bProceduralTexture,
                                             Vector &vecHSV) {
...
void CColorZPreview::Paint(void) {
  ...
  int py[3] = {MARKER_WIDTH + y - MARKER_WIDTH, MARKER_WIDTH + y + MARKER_WIDTH,
               MARKER_WIDTH + y};
...
void CColorPickerPanel::GetCurrentColor(Color *pColor) {
  pColor->SetColor(m_CurrentColor.r, m_CurrentColor.g, m_CurrentColor.b,
                   m_CurrentAlpha);
}

void CColorPickerPanel::GetInitialColor(Color *pColor) {
  pColor->SetColor(m_InitialColor.r, m_InitialColor.g, m_InitialColor.b,
                   m_InitialAlpha);
}
...
void CColorPickerPanel::OnRadioButtonChecked(KeyValues *pKeyValues) {
  // NOTE: The radio button command strings are defined in the colorpicker.res
  // file in game/platform/resource.
...
void CColorPickerPanel::OnColorChanged(vgui::TextEntry *pChanged) {
  ...
  if (pChanged != m_pSaturationText) {
    Q_snprintf(temp, sizeof(temp), "%d",
               (int)(m_CurrentHSVColor.y * 100 + 0.5f));
    m_pSaturationText->SetText(temp);
  }
  if (pChanged != m_pValueText) {
    Q_snprintf(temp, sizeof(temp), "%d",
               (int)(m_CurrentHSVColor.z * 100 + 0.5f));
...
CColorPickerFrame::~CColorPickerFrame() { CleanUpMessage(); }
...
//
//-----------------------------------------------------------------------------
CColorPickerButton::CColorPickerButton(vgui::Panel *pParent, const char *pName,
...
void CColorPickerButton::DoClick() {
  ...
  CColorPickerFrame *pColorPickerDialog =
      new CColorPickerFrame(this, "Select Color");
