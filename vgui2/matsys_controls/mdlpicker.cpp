...
static bool SaveTgaAndAddToP4(unsigned char *pImage, ImageFormat imageFormat,
  ...
  if (!TGAWriter::WriteToBuffer(pImage, buffer, Width, Height, imageFormat,
  ...
  g_pFullFileSystem->AsyncWrite(szDestFilename, buffer.Base(), buffer.TellPut(),
                                true);
...
// Sort by MDL name
//-----------------------------------------------------------------------------
static int __cdecl MDLBrowserSortFunc(vgui::ListPanel *pPanel,
...
CMDLPicker::CMDLPicker(vgui::Panel *pParent, int nFlags)
  ...
  if (nFlags & PAGE_SCREEN_CAPS) {
    ...
    m_pBackgroundColor =
        new CColorPickerButton(m_pScreenCapsPage, "BackgroundColor", this);

    m_pScreenCapsPage->LoadControlSettingsAndUserConfig(
        "resource/mdlpickerscreencaps.res");
    ...
    //		m_pBackgroundColor = ( CColorPickerButton *
    //)m_pScreenCapsPage->FindChildByName( "BackgroundColor" );
    m_pBackgroundColor->SetColor(255, 0, 0, 255);

    Label *m_pOutputDirectory;
    m_pOutputDirectory =
        (Label *)m_pScreenCapsPage->FindChildByName("OutputDirectory");
    m_pOutputDirectory->SetText("c:\\");

    Button *pSelectProbe =
        (Button *)m_pScreenCapsPage->FindChildByName("Capture");
    pSelectProbe->AddActionSignalTarget(this);
    pSelectProbe =
        (Button *)m_pScreenCapsPage->FindChildByName("OutputDirectorySelect");
    pSelectProbe->AddActionSignalTarget(this);
    ...
    pSelectProbe->AddActionSignalTarget(this);
    pSelectProbe =
        (Button *)m_pScreenCapsPage->FindChildByName("GenerateBackpackIcons");
    pSelectProbe->AddActionSignalTarget(this);
  }
...
void CMDLPicker::RefreshRenderSettings() {
  ...
  // thumbnail safe zone
  pToggle =
      (vgui::CheckButton *)m_pRenderPage->FindChildByName("ThumbnailSafeZone");
  pToggle->AddActionSignalTarget(this);
  ...
  if (!Q_stricmp(pCommand, "ChooseLightProbe")) {
    ...
    if (!m_hDirectorySelectDialog.Get()) {
      m_hDirectorySelectDialog =
          new DirectorySelectDialog(this, "Choose Screen Caps output folder");
    }
    ...
    vgui::CheckButton *pGroundToggle =
        (vgui::CheckButton *)m_pRenderPage->FindChildByName("NoGround");
    bool bOriginalGridState = pGroundToggle->IsSelected();

    vgui::CheckButton *pSafeZoneToggle =
        (vgui::CheckButton *)m_pRenderPage->FindChildByName(
            "ThumbnailSafeZone");
    ...
    if (input()->IsKeyDown(KEY_RCONTROL) || input()->IsKeyDown(KEY_LCONTROL)) {
      ...
      for (int i = 0; i < nCount; ++i) {
        if (m_pAssetBrowser->IsItemVisible(m_AssetList[i].m_nItemId) &&
          ...
          KeyValues *pItemKeyValues =
              m_pAssetBrowser->GetItem(m_AssetList[i].m_nItemId);
          ...
          Label *m_pResults =
              (Label *)m_pScreenCapsPage->FindChildByName("CaptureResults");
...
//-----------------------------------------------------------------------------
// Screen captures the specific model and writes out a .tga.  Assumes the
// MDLPreview panel has been properly adjusted to 0,0 in screen space and that
// width / height have been set.
//-----------------------------------------------------------------------------
const char *CMDLPicker::CaptureModel(int nModIndex, const char *AssetName,
                                     const char *OutputPath, int Width,
                                     int Height, Color BackgroundColor,
                                     bool bSelectedOnly) {
  char pBuf[MAX_PATH];
  Q_snprintf(pBuf, sizeof(pBuf), "%s\\%s\\%s", GetModPath(nModIndex),
             m_pAssetSubDir, AssetName);
  Q_FixSlashes(pBuf);
  ...
  pRenderContext->ReadPixels(0, 0, Width, Height, pImageBlack,
                             IMAGE_FORMAT_BGRA8888);
  ...
  // Get Bits from the material system
  pRenderContext->ReadPixels(0, 0, Width, Height, pImageWhite,
                             IMAGE_FORMAT_BGRA8888);
  ...
  bool bResult = SaveTgaAndAddToP4(pImageBlack, IMAGE_FORMAT_BGRA8888, Width,
                                   Height, szPathedFileName);
...
void CMDLPicker::CaptureScreenCaps(void) {
  ...
  CColorPickerButton *m_pBackgroundColor;
  m_pBackgroundColor = (CColorPickerButton *)m_pScreenCapsPage->FindChildByName(
      "BackgroundColor");
  ...
  for (int i = 0; i < nCount; ++i) {
    if (m_pAssetBrowser->IsItemVisible(m_AssetList[i].m_nItemId) &&
      ...
      CaptureModel(nModIndex, pSelectedAsset, szBathPath, width, height,
                   NewPanelColor, bSelectedOnly);
      nNumItems++;
    }
  }
...
//-----------------------------------------------------------------------------
#if defined(_X360)
const char *getenv(const char *varname) { return NULL; }
#endif
...
void CMDLPicker::WriteBackbackVMTFiles(const char *pAssetName) {
  ...
  Q_snprintf(pVMTFilename, sizeof(pVMTFilename),
             "%s\\materials\\backpack\\%s.vmt", pVProject, pStrippedAssetName);
  Q_FixSlashes(pVMTFilename);

  char pBaseTextureName[MAX_PATH];
  Q_snprintf(pBaseTextureName, sizeof(pBaseTextureName), "backpack\\%s",
             pStrippedAssetName);
  ...
  Q_snprintf(pVMTFilename, sizeof(pVMTFilename),
             "%s\\materials\\backpack\\%s_large", pVProject,
             pStrippedAssetName);
  V_SetExtension(pVMTFilename, ".vmt", sizeof(pVMTFilename));
  Q_FixSlashes(pVMTFilename);

  Q_snprintf(pBaseTextureName, sizeof(pBaseTextureName), "backpack\\%s_large",
             pStrippedAssetName);
  Q_FixSlashes(pBaseTextureName);
...
void CMDLPicker::GenerateBackpackIcons(void) {
  ...
  for (selectedItemIndex = 0; selectedItemIndex < m_AssetList.Count();
       ++selectedItemIndex) {
    if (m_pAssetBrowser->IsItemVisible(
            m_AssetList[selectedItemIndex].m_nItemId) &&
        m_pAssetBrowser->IsItemSelected(
            m_AssetList[selectedItemIndex].m_nItemId)) {
      break;
    }
  }
  ...
  KeyValues *pItemKeyValues =
      m_pAssetBrowser->GetItem(m_AssetList[selectedItemIndex].m_nItemId);
  const char *pSelectedAsset = pItemKeyValues->GetString("asset");
  ...
  Q_snprintf(pOutputPath, sizeof(pOutputPath),
             "%s\\%s\\materialsrc\\backpack\\", pVContent, pVMod);
  Q_FixSlashes(pOutputPath);

  int nModIndex = pItemKeyValues->GetInt("modIndex");
  const char *pLargeTGAName =
      CaptureModel(nModIndex, pLargeAssetName, pOutputPath, width, height,
                   NewPanelColor, true);
  if (!pLargeTGAName)
    return;
...

  {
    ...
    FileHandle_t hVTexOptionsFile =
        g_pFullFileSystem->Open(pVTexOptionsFileName, "w");
    if (hVTexOptionsFile) {
  ...
  // run vtex on the TGA and .txt file to create .VTF and add it to our Perforce
  // changelist
  char *vTexArgv[64];
  int vTexArgc = 0;
  vTexArgv[vTexArgc++] = "";
  ...
  if (!g_pFullFileSystem->ReadFile(pLargeTGAName, NULL, largeTGAFileData) ||
      !TGALoader::GetInfo(largeTGAFileData, &nCheckWidth, &nCheckHeight,
                          &largeFmt, &gamma) ||
  ...
  if (!TGALoader::LoadRGBA8888(largeTGAFileData, largeTGAImageData, nCheckWidth,
                               nCheckHeight) ||
      nCheckWidth != width || nCheckHeight != height) {
  ...
  //
  // Perform a downsample.  This is better than just re-rendering at the smaller
  // size, which essentially just point-samples the image.
  //
  ...
  resampleInfo.m_nFlags =
      ImageLoader::RESAMPLE_CLAMPS | ImageLoader::RESAMPLE_CLAMPT;
  // resampleInfo.m_nFlags |= ImageLoader::RESAMPLE_NICE_FILTER; // Turn this
  // off.  It has some sort of edge enhancement or something.  Causes edges to
  // ring.
  ...
  // Save it
  if (!SaveTgaAndAddToP4(resampleInfo.m_pDest, IMAGE_FORMAT_RGBA8888,
                         resampleInfo.m_nDestWidth, resampleInfo.m_nDestHeight,
                         pSmallTGAName)) {
    return;
  }
...
CUtlString CMDLPicker::GetOutputFileSuffix() {
  char temp[256];
  TextEntry *pTempValue =
      (TextEntry *)m_pScreenCapsPage->FindChildByName("SuffixText");
  if (pTempValue) {
    pTempValue->GetText(temp, sizeof(temp));
  }
  return temp;
...
void CMDLPicker::SaveCaps(const char *szFileName) {
  ...
  for (int i = 1; i < MAX_SELECTED_MODELS; i++) {
    if (m_hSelectedMDL[i] != MDLHANDLE_INVALID) {
      const char *MergedModelName =
          vgui::MDLCache()->GetModelName(m_hSelectedMDL[i]);
...
bool CMDLPicker::RestoreCaps(const char *szFileName) {
  ...
  QAngle angDir;
  Vector vecOffset;
  sscanf(CaptureData->GetString("CameraPosition"), "%g %g %g", &vecPos.x,
         &vecPos.y, &vecPos.z);
  sscanf(CaptureData->GetString("CameraAngles"), "%g %g %g", &angDir.x,
         &angDir.y, &angDir.z);
  sscanf(CaptureData->GetString("CameraOffset"), "%g %g %g", &vecOffset.x,
         &vecOffset.y, &vecOffset.z);
  ...
  m_pBackgroundColor = (CColorPickerButton *)m_pScreenCapsPage->FindChildByName(
      "BackgroundColor");
  sscanf(CaptureData->GetString("BackgroundColor"), "%d %d %d %d", &r, &g, &b,
         &a);
...
//-----------------------------------------------------------------------------
void CMDLPicker::SelectMDL(const char *pRelativePath, bool bDoLookAt,
                           int nSelectSecondary) {
