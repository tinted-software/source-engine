...
//-----------------------------------------------------------------------------
CVMTPanel::CVMTPanel(vgui::Panel *pParent, const char *pName)
...
void CVMTPanel::PerformLayout() {
  ...
  m_iViewableWidth = bVertScrollVisible
                         ? iWidth - SCROLLBAR_SIZE - WINDOW_BORDER_WIDTH
                         : iWidth;
  m_iViewableHeight = bHorizScrollVisible
                          ? iHeight - SCROLLBAR_SIZE - WINDOW_BORDER_WIDTH
                          : iHeight;
...
// Draw it on a sphere
//-----------------------------------------------------------------------------
void CVMTPanel::RenderSphere(const Vector &vCenter, float flRadius, int nTheta,
                             int nPhi) {
  ...
  bool bIsUsingBumpedLightmap =
      m_pMaterial->GetPropertyFlag(MATERIAL_PROPERTY_NEEDS_BUMPED_LIGHTMAPS);
