...
CTranslationManipulator::CTranslationManipulator(matrix3x4_t *pTransform)
    : CTransformManipulator(pTransform) {
  m_lastx = m_lasty = 0;
}

void CTranslationManipulator::OnMousePressed(vgui::MouseCode code, int x,
                                             int y) {
  m_lasty = y;
  m_lastx = x;
}
...
CRotationManipulator::CRotationManipulator(matrix3x4_t *pTransform)
    : CTransformManipulator(pTransform) {
  m_lastx = m_lasty = 0;
  m_altitude = M_PI / 6;
  m_azimuth = -3 * M_PI / 4;
...
CPotteryWheelPanel::CPotteryWheelPanel(vgui::Panel *pParent, const char *pName)
    : BaseClass(pParent, pName), m_pCameraRotate(NULL),
      m_pCameraTranslate(NULL), m_pCameraZoom(NULL), m_pLightManip(NULL),
      m_pCurrentManip(NULL), m_nCaptureMouseCode(vgui::MouseCode(-1)),
      m_xoffset(0), m_yoffset(0), m_bRenderToTexture(true) {
...
void CPotteryWheelPanel::SetLightProbe(CDmxElement *pLightProbe) {
  ...
  pVMTKeyValues->SetInt("$no_fullbright", 1);
  pVMTKeyValues->SetInt("$nocull", 1);
  m_LightProbeHDRBackground.Init("SPWP_LightProbeBackground_HDR",
                                 pVMTKeyValues);
  m_LightProbeHDRBackground->Refresh();

  const CUtlVector<Vector> &ambientCube =
      pLightProbe->GetArray<Vector>("ambientCube");
  ...
  const CUtlVector<CDmxElement *> &localLights =
      pLightProbe->GetArray<CDmxElement *>("localLights");
  ...
  for (int i = 0; i < nLightCount; ++i) {
    ...
    if (!Q_stricmp(pType, "directional")) {
      pDesc->InitDirectional(pLocalLight->GetValue<Vector>("direction"),
                             vecColor);
      ++m_nLightCount;
      continue;
    }
    ...
    if (!Q_stricmp(pType, "spot")) {
      const Vector &vecAtten = pLocalLight->GetValue<Vector>("attenuation");
      pDesc->InitSpot(pLocalLight->GetValue<Vector>("origin"), vecColor,
                      vec3_origin,
...
// Purpose:
//-----------------------------------------------------------------------------
int CPotteryWheelPanel::GetCameraFOV(void) { return m_Camera.m_flFOV; }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CPotteryWheelPanel::SetCameraFOV(float flFOV) { m_Camera.m_flFOV = flFOV; }
...
// Purpose:
//-----------------------------------------------------------------------------
void CPotteryWheelPanel::SetCameraPositionAndAngles(const Vector &vecPos,
                                                    const QAngle &angDir,
                                                    bool syncManipulators) {
  SetIdentityMatrix(m_CameraPivot);
...
// Purpose:
//-----------------------------------------------------------------------------
void CPotteryWheelPanel::GetCameraPositionAndAngles(Vector &vecPos,
                                                    QAngle &angDir) {
...
//-----------------------------------------------------------------------------
void CPotteryWheelPanel::SetupRenderState(int nDisplayWidth,
                                          int nDisplayHeight) {
  ...
  LightDesc_t *pDesc =
      (LightDesc_t *)stackalloc(m_nLightCount * sizeof(LightDesc_t));
  for (int i = 0; i < m_nLightCount; ++i) {
    pDesc[i] = m_Lights[i].m_Desc;
    VectorTransform(m_Lights[i].m_Desc.m_Position, m_Lights[i].m_LightToWorld,
                    pDesc->m_Position);
    VectorRotate(m_Lights[i].m_Desc.m_Direction, m_Lights[i].m_LightToWorld,
                 pDesc->m_Direction);
...
//-----------------------------------------------------------------------------
void CPotteryWheelPanel::ComputePanelPosition(const Vector &vecPosition,
                                              Vector2D *pPanelPos) {
  int w, h;
  GetSize(w, h);
...
void CPotteryWheelPanel::Paint() {
  ...
  pRenderContext->SetIntRenderingParameter(
      INT_RENDERPARM_WRITE_DEPTH_TO_DESTALPHA, false);
  ...
  if (HasLightProbe()) {
    ...
    RenderBox(m_Camera.m_origin, vec3_angle, Vector(-100, -100, -100),
...
void CPotteryWheelPanel::OnMousePressed(vgui::MouseCode code) {
  ...
  } else if (input()->IsKeyDown(KEY_RCONTROL) ||
...
void CPotteryWheelPanel::EnterManipulationMode(
...
void CPotteryWheelPanel::EnableMouseCapture(
...
bool CPotteryWheelPanel::WarpMouse(int &x, int &y) {
  ...
  // around using the mouse.  This control resets the cursor to the center of
  // the window after each mouse move.  Except the input()->SetCursorPos results
  // (after a lot of redirection) to vgui/matsurface/Cursor.cpp function
  // CursorSetPos but it has a (needed) test to not move the cursor if it's
  // currently hidden. Rather than change all the levels between here and there
  // to support a flag, we are just jumping to the chase and directly calling
  // the inputsystem SetCursorPosition on OpenGL platforms
  //
