...
void CTGAPreviewPanel::SetTGA(const char *pFullPath) {
  ...
  if (!TGALoader::Load((unsigned char *)GetImageBuffer(), buf, nWidth, nHeight,
                       IMAGE_FORMAT_BGRA8888, flGamma, false)) {
    Shutdown();
  } else {
    DownloadTexture();
  }
...
//-----------------------------------------------------------------------------
const char *CTGAPreviewPanel::GetTGA() const { return m_TGAName; }
