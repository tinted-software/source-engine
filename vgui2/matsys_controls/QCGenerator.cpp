...
CQCGenerator::CQCGenerator(vgui::Panel *pParent, const char *pszPath,
  ...
  snprintf(szSearchPath, sizeof(szSearchPath), "%s%s", szGamePath,
           pSurfacePropFilename);
  ...
  if (!fp) {
    ...
    // hl2 directory 		that contains the file.  It potentially needs to search the
    //entire drive or prompt for the location
  ...
  if (m_QCInfo_t.bDisableCollision != true) {
    ...
    if (m_QCInfo_t.bReferenceAsPhys == true) {
      ...
      if (Q_strcmp(m_QCInfo_t.pszCollisionPath, "")) {
        g_pFullFileSystem->FPrintf(pSaveFile, "$collisionmodel \"%s\"",
                                   strrchr(m_QCInfo_t.pszCollisionPath, '\\') +
                                       1);
      }
    }
