...
// Sort by asset name
//-----------------------------------------------------------------------------
static int __cdecl PickerBrowserSortFunc(vgui::ListPanel *pPanel,
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CPicker::CPicker(vgui::Panel *pParent, const char *pColumnHeader,
...
void CPicker::OnKeyCodePressed(KeyCode code) {
  if ((code == KEY_UP) || (code == KEY_DOWN) || (code == KEY_PAGEUP) ||
    ...
    vgui::ipanel()->SendMessage(m_pPickerBrowser->GetVPanel(), pMsg,
                                GetVPanel());
    pMsg->deleteThis();
  } else {
...
void CPicker::RefreshChoiceList() {
  ...
  int nTotalCount = 0;
  for (int nItemID = m_pPickerBrowser->FirstItem();
       nItemID != m_pPickerBrowser->InvalidItemID();
    ...
    bool bVisible =
        !m_Filter.Length() || Q_stristr(pPickerName, m_Filter.Get());
  ...
  Q_snprintf(pColumnTitle, sizeof(pColumnTitle), "%s (%d/%d)", m_pPickerType,
             nMatchingCount, nTotalCount);
  m_pPickerBrowser->SetColumnHeaderText(0, pColumnTitle);

  m_pPickerBrowser->SortList();
  if ((m_pPickerBrowser->GetSelectedItemsCount() == 0) &&
      (m_pPickerBrowser->GetItemCount() > 0)) {
...
//
//-----------------------------------------------------------------------------
CPickerFrame::CPickerFrame(vgui::Panel *pParent, const char *pTitle,
                           const char *pPickerType, const char *pTextType)
...
//-----------------------------------------------------------------------------
void CPickerFrame::DoModal(const PickerList_t &list,
