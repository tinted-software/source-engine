...
static const float THUMBNAIL_SAFE_ZONE_HEIGHT_SCALE =
    (float)THUMBNAIL_SAFE_ZONE_HEIGHT / THUMBNAIL_SAFE_ZONE_SIZE;
...
void CMDLPanel::SetMDL(const char *pMDLName, void *pProxyData) {
  ...
  // FindMDL takes a reference and the the CMDL will also hold a reference for
  // as long as it sticks around. Release the FindMDL reference.
  ...
  AssertMsg(hMDL == MDLHANDLE_INVALID || nRef > 0,
            "CMDLPanel::SetMDL referenced a model that has a zero ref count.");
}
...
bool CMDLPanel::GetBoundingBox(Vector &vecBoundsMin, Vector &vecBoundsMax) {
  ...
  GetMDLBoundingBox(&vecBoundsMin, &vecBoundsMax, m_RootMDL.m_MDL.GetMDL(),
                    m_RootMDL.m_MDL.m_nSequence);
...
bool CMDLPanel::GetBoundingSphere(Vector &vecCenter, float &flRadius) {
  ...
  GetMDLBoundingSphere(&vecEngineCenter, &flRadius, m_RootMDL.m_MDL.GetMDL(),
                       m_RootMDL.m_MDL.m_nSequence);
...
// Purpose:
//-----------------------------------------------------------------------------
void CMDLPanel::SetModelAnglesAndPosition(const QAngle &angRot,
                                          const Vector &vecPos) {
  SetIdentityMatrix(m_RootMDL.m_MDLToWorld);
...
void CMDLPanel::DrawCollisionModel() {
  ...
  CStudioHdr studioHdr(g_pMDLCache->GetStudioHdr(m_RootMDL.m_MDL.GetMDL()),
                       g_pMDLCache);

  matrix3x4_t pBoneToWorld[MAXSTUDIOBONES];
  m_RootMDL.m_MDL.SetUpBones(m_RootMDL.m_MDLToWorld, MAXSTUDIOBONES,
                             pBoneToWorld);
...
void CMDLPanel::OnPaint3D() {
  ...
  matrix3x4_t *pBoneToWorld =
      g_pStudioRender->LockBoneMatrices(studioHdr.numbones());
  m_RootMDL.m_MDL.SetUpBones(m_RootMDL.m_MDLToWorld, studioHdr.numbones(),
                             pBoneToWorld, m_PoseParameters, m_SequenceLayers,
                             m_nNumSequenceLayers);
  ...
  for (int iMerge = 0; iMerge < nMergeCount; ++iMerge) {
    ...
    studiohdr_t *pStudioHdr =
        g_pMDLCache->GetStudioHdr(m_aMergeMDLs[iMerge].m_MDL.GetMDL());
    matrix3x4_t *pMergeBoneToWorld = &matMergeBoneToWorld[0];

    // If we have a valid mesh, bonemerge it. If we have an invalid mesh we
    // can't bonemerge because it'll crash trying to pull data from the missing
    // header.
    if (pStudioHdr != NULL) {
      CStudioHdr mergeHdr(pStudioHdr, g_pMDLCache);
      m_aMergeMDLs[iMerge].m_MDL.SetupBonesWithBoneMerge(
          &mergeHdr, pMergeBoneToWorld, &studioHdr, pBoneToWorld,
          m_RootMDL.m_MDLToWorld);

      pOverrideMaterial =
          GetOverrideMaterial(m_aMergeMDLs[iMerge].m_MDL.GetMDL());
      if (pOverrideMaterial != NULL)
        g_pStudioRender->ForcedMaterialOverride(pOverrideMaterial);

      m_aMergeMDLs[iMerge].m_MDL.Draw(m_aMergeMDLs[iMerge].m_MDLToWorld,
                                      pMergeBoneToWorld);
      ...
      RenderingMergedModel(pRenderContext, &mergeHdr,
                           m_aMergeMDLs[iMerge].m_MDL.GetMDL(),
                           pMergeBoneToWorld);
    }
  }

  RenderingRootModel(pRenderContext, &studioHdr, m_RootMDL.m_MDL.GetMDL(),
                     pBoneToWorld);
...
void CMDLPanel::SetPoseParameters(const float *pPoseParameters, int nCount) {
  if (pPoseParameters) {
    ...
    CStudioHdr studioHdr(g_pMDLCache->GetStudioHdr(m_RootMDL.m_MDL.GetMDL()),
                         g_pMDLCache);
    Studio_CalcDefaultPoseParameters(&studioHdr, m_PoseParameters,
                                     MAXSTUDIOPOSEPARAM);
  }
}
...
//-----------------------------------------------------------------------------
void CMDLPanel::SetSequenceLayers(const MDLSquenceLayer_t *pSequenceLayers,
                                  int nCount) {
  if (pSequenceLayers) {
...
//-----------------------------------------------------------------------------
void CMDLPanel::SetSkin(int nSkin) { m_RootMDL.m_MDL.m_nSkin = nSkin; }
...
void CMDLPanel::OnTick() {
  BaseClass::OnTick();
  if (m_RootMDL.m_MDL.GetMDL() != MDLHANDLE_INVALID) {
    m_RootMDL.m_MDL.m_flTime =
        (GetAutoPlayTime() - m_RootMDL.m_flCycleStartTime);
...
void CMDLPanel::Paint() {
  ...
  if (m_bThumbnailSafeZone) {
    ...
    IMaterial *safezone = materials->FindMaterial("vgui/thumbnails_safezone",
                                                  TEXTURE_GROUP_VGUI, true);
    if (safezone) {
    ...
    int iRemappedHeight = RemapVal(iScaledHeight, 0, THUMBNAIL_SAFE_ZONE_HEIGHT,
                                   0, iScaledHeight);

    pRenderContext->DrawScreenSpaceRectangle(
        safezone, screenposx, screenposy, iWidth, iRemappedHeight, 0, 0,
...
void CMDLPanel::DoAnimationEvents() {
  ...
  DoAnimationEvents(&studioHdr, m_RootMDL.m_MDL.m_nSequence,
                    m_RootMDL.m_MDL.m_flTime, false, &m_EventState);

  for (int i = 0; i < m_nNumSequenceLayers; ++i) {
    float flTime =
        m_RootMDL.m_MDL.m_flTime - m_SequenceLayers[i].m_flCycleBeganAt;
    // Plat_DebugString( CFmtStr("Animation: time = %f, started = %f, delta =
    // %f\n",m_RootMDL.m_MDL.m_flTime,m_SequenceLayers[ i
    // ].m_flCycleBeganAt,flTime ) );
    DoAnimationEvents(&studioHdr, m_SequenceLayers[i].m_nSequenceIndex, flTime,
                      m_SequenceLayers[i].m_bNoLoop,
...
// Purpose:
//-----------------------------------------------------------------------------
void CMDLPanel::DoAnimationEvents(CStudioHdr *pStudioHdr, int nSeqNum,
                                  float flTime, bool bNoLoop,
  ...
  float flEventCycle =
      (flTime * m_RootMDL.m_MDL.m_flPlaybackRate) / nFrameCount;
  // Plat_DebugString( CFmtStr("Event cycle: %f, playback rate: %f, frame count:
  ...
  if (pEventState->m_nEventSequence != nSeqNum) {
    ...
    flEventCycle = 0.0f;
    pEventState->m_flPrevEventCycle =
        -0.01f; // back up to get 0'th frame animations
  }
...
void CMDLPanel::FireEvent(const char *pszEventName,
...
// Purpose:
//-----------------------------------------------------------------------------
void CMDLPanel::SetMergeMDL(MDLHandle_t handle, void *pProxyData,
                            int nSkin /*= -1 */) {
...
// Purpose:
//-----------------------------------------------------------------------------
MDLHandle_t CMDLPanel::SetMergeMDL(const char *pMDLName, void *pProxyData,
                                   int nSkin /*= -1 */) {
  MDLHandle_t hMDLFindResult = vgui::MDLCache()->FindMDL(pMDLName);
  MDLHandle_t hMDL = pMDLName ? hMDLFindResult : MDLHANDLE_INVALID;
  if (vgui::MDLCache()->IsErrorModel(hMDL)) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CMDLPanel::ClearMergeMDLs(void) { m_aMergeMDLs.Purge(); }
