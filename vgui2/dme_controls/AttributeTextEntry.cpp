...
CAttributeTextEntry::CAttributeTextEntry(Panel *parent, const char *panelName)
    : BaseClass(parent, panelName), m_bValueStored(false),
...
// Drag + drop
//-----------------------------------------------------------------------------
bool CAttributeTextEntry::GetDropContextMenu(Menu *menu,
...
bool CAttributeTextEntry::IsDroppable(CUtlVector<KeyValues *> &msglist) {
  ...
  case AT_ELEMENT: {
    CDmElement *ptr = reinterpret_cast<CDmElement *>(
        g_pDataModel->GetElement(DmElementHandle_t(msg->GetInt("root"))));
    if (ptr) {
      return true;
    }
    return false;
...
void CAttributeTextEntry::OnPanelDropped(CUtlVector<KeyValues *> &msglist) {
  ...
  if (!Q_stricmp(cmd, "droptext") || !Q_stricmp(cmd, "default")) {
    ...
    case AT_ELEMENT: {
      CDmElement *ptr = reinterpret_cast<CDmElement *>(
          g_pDataModel->GetElement(DmElementHandle_t(data->GetInt("root"))));
      if (!ptr) {
        break;
      }
      pPanel->SetDirty(true);
...
void CAttributeTextEntry::ApplyMouseWheel(T newValue, T originalValue) {
  ...
  if (pPanel->IsAutoApply()) {
    ...
    CElementTreeUndoScopeGuard guard(
        0, pPanel->GetNotify(), "Set Attribute Value", "Set Attribute Value");
