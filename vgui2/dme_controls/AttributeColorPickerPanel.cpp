...
// Constructor
//-----------------------------------------------------------------------------
CAttributeColorPickerPanel::CAttributeColorPickerPanel(
    vgui::Panel *parent, const AttributeWidgetInfo_t &info)
...
void CAttributeColorPickerPanel::OnPreview(KeyValues *data) {
  ...
  if (IsAutoApply()) {
    ...
    CElementTreeNotifyScopeGuard guard("CAttributeColorPickerPanel::OnPreview",
...
void CAttributeColorPickerPanel::OnCancelled() {
  ...
  Refresh();
  CElementTreeNotifyScopeGuard notify("CAttributeColorPickerPanel::OnCancelled",
                                      NOTIFY_CHANGE_ATTRIBUTE_VALUE,
                                      GetNotify());
  SetDirty(false);
}
...
void CAttributeColorPickerPanel::OnPicked(KeyValues *data) {
  ...
  // Kind of an evil hack, but "undo" copies the "old value" off for doing undo,
  // and that value is the new value because
  //  we haven't been tracking undo while manipulating this.  So we'll turn off
  //  undo and set the value to the original value.

  // In effect, all of the wheeling will drop out and it'll look just like we
  // started at the original value and ended up at the
  //  final value...
  {
    CDisableUndoScopeGuard guard;
    SetAttributeValue(m_InitialColor);
  ...
  {
    CElementTreeUndoScopeGuard guard(NOTIFY_SETDIRTYFLAG, GetNotify(),
                                     "Set Attribute Value",
                                     "Set Attribute Value");
    SetAttributeValue(c);
  }
