...
//
//-----------------------------------------------------------------------------
IMPLEMENT_DMEPANEL_FACTORY(CDmeSourceDCCFilePanel, DmeSourceDCCFile,
                           "DmeSourceDCCFileDefault",
                           "Maya/XSI Source File Editor", true);

//-----------------------------------------------------------------------------
// Sort by MDL name
//-----------------------------------------------------------------------------
static int __cdecl DccObjectSortFunc(vgui::ListPanel *pPanel,
...
//-----------------------------------------------------------------------------
CDmeSourceDCCFilePanel::CDmeSourceDCCFilePanel(vgui::Panel *pParent,
                                               const char *pPanelName)
    : BaseClass(pParent, pPanelName) {
  m_pRootDCCObjects = new vgui::ListPanel(this, "DCCObjectList");
  m_pRootDCCObjects->AddColumnHeader(0, "dccobject", "Maya/XSI Object Name",
                                     100, 0);
  ...
  m_pDCCObjectBrowser = new vgui::Button(this, "DCCObjectBrowser", "...", this,
                                         "OnBrowseDCCObject");
  ...
  m_pAddDCCObject = new vgui::Button(this, "AddDCCObjectButton", "Add", this,
                                     "OnAddDCCObject");
  m_pRemoveDCCObject = new vgui::Button(this, "RemoveDCCObjectButton", "Remove",
                                        this, "OnRemoveDCCObject");

  m_pApplyChanges = new vgui::Button(this, "ApplyChangesButton", "Apply", this,
                                     "OnApplyChanges");
...
void CDmeSourceDCCFilePanel::RefreshDCCObjectList() {
  ...
  for (int i = 0; i < nCount; ++i) {
    KeyValues *pItemKeys = new KeyValues(
        "node", "dccobject", m_hSourceDCCFile->m_RootDCCObjects.Get(i));
...
void CDmeSourceDCCFilePanel::SelectDCCObject(int nDCCObjectIndex) {
  ...
  for (; nItemID != m_pRootDCCObjects->InvalidItemID();
       nItemID = m_pRootDCCObjects->NextItem(nItemID)) {
...
//-----------------------------------------------------------------------------
void CDmeSourceDCCFilePanel::OnDCCObjectAdded(const char *pDCCObjectName,
...
//-----------------------------------------------------------------------------
void CDmeSourceDCCFilePanel::ShowDCCObjectBrowser(const char *pTitle,
                                                  const char *pPrompt,
...
void CDmeSourceDCCFilePanel::OnAddDCCObject() {
  if (m_hSourceDCCFile.Get()) {
    ...
    ShowDCCObjectBrowser("Add DCC Object", "Enter DCC object name to add",
                         pDialogKeys);
  }
}
...
void CDmeSourceDCCFilePanel::OnBrowseDCCObject() {
  ...
  KeyValues *pDialogKeys =
      new KeyValues("ChangeDCCObject", "dccObjectIndex", nDCCObjectIndex);
  ShowDCCObjectBrowser("Edit Maya/XSI Object",
                       "Enter new name of Maya/XSI object", pDialogKeys);
}

//-----------------------------------------------------------------------------
// Called when the source file name changes
//-----------------------------------------------------------------------------
bool CDmeSourceDCCFilePanel::CheckForDuplicateNames(const char *pDCCObjectName,
    ...
    for (int i = 0; i < nCount; ++i) {
      ...
      if (!Q_stricmp(pDCCObjectName, m_hSourceDCCFile->m_RootDCCObjects[i])) {
        vgui::MessageBox *pError = new vgui::MessageBox(
            "#DmeSourceDCCFile_DuplicateSourceTitle",
