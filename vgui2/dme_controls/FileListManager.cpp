...
template <int C>
int ListPanelStringSortFunc(vgui::ListPanel *pPanel,
...
const int fixedflags =
    vgui::ListPanel::COLUMN_UNHIDABLE | vgui::ListPanel::COLUMN_FIXEDSIZE;

static ColumnInfo_t g_ColInfo[] = {
    {"filename", "#BxFileManager_Filename", 150, baseflags,
     ListPanelStringSortFunc<CI_FILENAME>, vgui::Label::a_west},
    {"path", "#BxFileManager_Path", 240, baseflags,
     ListPanelStringSortFunc<CI_PATH>, vgui::Label::a_west},
    {"loaded", "#BxFileManager_Loaded", 40, fixedflags,
     ListPanelStringSortFunc<CI_LOADED>, vgui::Label::a_center},
    {"numelements", "#BxFileManager_NumElements", 60, fixedflags,
     ListPanelStringSortFunc<CI_NUMELEMENTS>, vgui::Label::a_east},
    {"changed", "#BxFileManager_Changed", 50, fixedflags,
     ListPanelStringSortFunc<CI_CHANGED>, vgui::Label::a_center},
    {"in_perforce", "#BxFileManager_P4Exists", 35, fixedflags,
     ListPanelStringSortFunc<CI_INPERFORCE>, vgui::Label::a_center},
    {"open_for_edit", "#BxFileManager_P4Edit", 40, fixedflags,
     ListPanelStringSortFunc<CI_OPENFOREDIT>, vgui::Label::a_center},
};

const char *GetKey(ColumnIndex_t ci) { return g_ColInfo[ci].columnName; }
...
void AddColumn(CFileListManager *pFileManager, ColumnIndex_t ci) {
  pFileManager->AddColumnHeader(
      ci, g_ColInfo[ci].columnName, g_ColInfo[ci].columnText,
      g_ColInfo[ci].startingWidth, g_ColInfo[ci].flags);
  pFileManager->SetSortFunc(ci, g_ColInfo[ci].pfnSort);
  pFileManager->SetColumnTextAlignment(ci, g_ColInfo[ci].alignment);
}

CFileListManager::CFileListManager(vgui::Panel *parent)
    : BaseClass(parent, "FileListManager") {
...
int CFileListManager::AddItem(DmFileId_t fileid, const char *pFilename,
                              const char *pPath, bool bLoaded, int nElements,
                              bool bChanged, bool bInPerforce,
                              bool bOpenForEdit) {
  KeyValues *kv =
      new KeyValues("", GetKey(CI_FILENAME), pFilename, GetKey(CI_PATH), pPath);
...
void CFileListManager::SetLoaded(DmFileId_t fileid, bool bLoaded) {
  CNotifyScopeGuard notify("CFileListManager::SetLoaded",
...
int AddMenuItemHelper(vgui::Menu *pMenu, const char *pItemName,
                      const char *pKVName, vgui::Panel *pTarget,
                      bool bEnabled) {
...
void CFileListManager::OnOpenContextMenu(KeyValues *pParams) {
  ...
  if (itemID < 0) {
    ...
    AddMenuItemHelper(m_hContextMenu, "Load", "load", this,
                      nLoaded < nSelected && nOnDisk > 0);
    AddMenuItemHelper(m_hContextMenu, "Unload", "unload", this, nLoaded > 0);
    AddMenuItemHelper(m_hContextMenu, "Save", "save", this,
                      nChanged > 0 && nOnDisk == nSelected);
    AddMenuItemHelper(m_hContextMenu, "Save As...", "saveas", this,
                      nLoaded == 1 && nSelected == 1);
    AddMenuItemHelper(m_hContextMenu, "Add To Perforce", "p4add", this,
                      nInPerforce < nSelected && nOnDisk > 0);
    AddMenuItemHelper(m_hContextMenu, "Open For Edit", "p4edit", this,
                      nOpenForEdit < nSelected && nOnDisk > 0);
  }
...
void CFileListManager::OnLoadFiles(KeyValues *pParams) {
  CNotifyScopeGuard notify("CFileListManager::OnLoadFiles",
                           NOTIFY_SOURCE_FILE_LIST_MANAGER,
                           NOTIFY_SETDIRTYFLAG);
...
void CFileListManager::OnUnloadFiles(KeyValues *pParams) {
  CNotifyScopeGuard notify("CFileListManager::OnUnloadFiles",
                           NOTIFY_SOURCE_FILE_LIST_MANAGER,
                           NOTIFY_SETDIRTYFLAG);
...
void CFileListManager::OnSaveFiles(KeyValues *pParams) {
  ...
  for (int i = 0; i < nSelected; ++i) {
    ...
    CDmElement *pRoot =
        GetElement<CDmElement>(g_pDataModel->GetFileRoot(fileid));
    Assert(pRoot);
    if (!pRoot)
      continue;
...
void CFileListManager::OnOpenFile(KeyValues *pParams) {
  ...
  vgui::FileOpenDialog *pFileOpenDialog = new vgui::FileOpenDialog(
      this, "Save .dmx File As", false, pContextKeyValues);
...
void CFileListManager::OnSaveFileAs(KeyValues *pParams) {
  ...
  vgui::FileOpenDialog *pFileOpenDialog = new vgui::FileOpenDialog(
      this, "Save .dmx File As", false, pContextKeyValues);
  // if this control is moved to vgui_controls, change the default format to
  // "dmx", the generic dmx format
  pFileOpenDialog->AddFilter("*.dmx", "Generic MovieObjects File (*.dmx)",
                             false, "movieobjects");
  if (V_strcmp(pFileFormat, "movieobjects") != 0) {
    char description[256];
    V_snprintf(description, sizeof(description), "%s (*.dmx)",
               g_pDataModel->GetFormatDescription(pFileFormat));
...
void CFileListManager::OnFileSelected(KeyValues *pParams) {
  ...
  if (pSaveAsKey) {
    ...
    g_pDataModel->SaveToFile(pFullPath, NULL,
                             g_pDataModel->GetDefaultEncoding(pFormat), pFormat,
                             pRoot);
...
void CFileListManager::OnAddToPerforce(KeyValues *pParams) {
  ...
  if (!bSuccess) {
    vgui::MessageBox *pError = new vgui::MessageBox(
        "Perforce Error!", p4->GetLastError(), GetParent());
...
void CFileListManager::OnDataChanged(KeyValues *pParams) {
  ...
  if (!bPerformFullRefresh) {
    ...
    for (int i = 0; i < nCount; ++i) {
      ...
      bPerformFullRefresh =
          (fileid != (DmFileId_t)GetItemUserData(i)) ||
          Q_stricmp(pFileName, pKeyValues->GetString(pNameKey));
      if (bPerformFullRefresh)
        break;

      pKeyValues->SetInt(GetKey(CI_NUMELEMENTS),
                         g_pDataModel->NumElementsInFile(fileid));
      pKeyValues->SetString(GetKey(CI_LOADED),
                            g_pDataModel->IsFileLoaded(fileid) ? "Y" : "N");
...
void CFileListManager::Refresh() {
  ...
  for (int i = 0; i < nFiles; ++i) {
    ...
    bool bOpenForEdit =
        bInPerforce && p4->GetFileState(pFileName) != P4FILE_UNOPENED;

    char path[256];
    V_ExtractFilePath(pFileName, path, sizeof(path));

    AddItem(fileid, V_UnqualifiedFileName(pFileName), path, bLoaded, nElements,
            bChanged, bInPerforce, bOpenForEdit);
  }
}
...
//
//-----------------------------------------------------------------------------
CFileManagerFrame::CFileManagerFrame(vgui::Panel *parent)
    : BaseClass(parent, "FileManagerFrame") {
...
void CFileManagerFrame::Refresh() { m_pFileListManager->Refresh(); }
