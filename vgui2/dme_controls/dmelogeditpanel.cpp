...
//-----------------------------------------------------------------------------
CDmeLogEditPanel::CDmeLogEditPanel(vgui::Panel *pParent, const char *pName)
...
//-----------------------------------------------------------------------------
int CDmeLogEditPanel::FindOrAddControlPoint(float flIn, float flTolerance,
                                            float flOut) {
  Assert(m_hLog.Get());
  DmeTime_t time = NormalizedToTime(flIn);
  DmeTime_t tolerance =
      (flTolerance >= 0) ? NormalizedToDuration(flTolerance) : DmeTime_t(0);
  ...
  case AT_BOOL:
    nKeyIndex = CastElement<CDmeBoolLog>(m_hLog)->FindOrAddKey(
        time, tolerance, (bool)(flValue >= 0.5f));
    break;

  case AT_INT:
    nKeyIndex = CastElement<CDmeIntLog>(m_hLog)->FindOrAddKey(
        time, tolerance, (int)(flValue + 0.5f));
    break;

  case AT_FLOAT:
    nKeyIndex = CastElement<CDmeFloatLog>(m_hLog)->FindOrAddKey(time, tolerance,
                                                                flValue);
    break;
  ...
  case AT_COLOR: {
    ...
    nKeyIndex =
        CastElement<CDmeColorLog>(m_hLog)->FindOrAddKey(time, tolerance, c);
  } break;
...
int CDmeLogEditPanel::ModifyControlPoint(int nPoint, float flIn, float flOut) {
  ...
  case AT_BOOL:
    ...
    nKeyIndex = CastElement<CDmeBoolLog>(m_hLog)->FindOrAddKey(
        time, DmeTime_t(0), (bool)(flValue >= 0.5f));
    break;

  case AT_INT:
    RemoveControlPoint(nPoint);
    nKeyIndex = CastElement<CDmeIntLog>(m_hLog)->FindOrAddKey(
        time, DmeTime_t(0), (int)(flValue + 0.5f));
    break;

  case AT_FLOAT:
    RemoveControlPoint(nPoint);
    nKeyIndex = CastElement<CDmeFloatLog>(m_hLog)->FindOrAddKey(
        time, DmeTime_t(0), flValue);
    break;
  ...
  case AT_COLOR: {
    ...
    RemoveControlPoint(nPoint);
    nKeyIndex =
        CastElement<CDmeColorLog>(m_hLog)->FindOrAddKey(time, DmeTime_t(0), c);
  } break;
...
float CDmeLogEditPanel::GetValue(float flIn) {
  ...
  case AT_VECTOR2:
    flValue =
        CastElement<CDmeVector2Log>(m_hLog)->GetValue(time)[m_nFieldIndex];
    break;

  case AT_VECTOR3:
    flValue =
        CastElement<CDmeVector3Log>(m_hLog)->GetValue(time)[m_nFieldIndex];
    break;
  ...
  case AT_QUATERNION:
    flValue =
        CastElement<CDmeQuaternionLog>(m_hLog)->GetValue(time)[m_nFieldIndex];
    break;
  }
...
void CDmeLogEditPanel::GetControlPoint(int nPoint, float *pIn, float *pOut) {
  ...
  *pIn = TimeToNormalized(time);
  ...
  case AT_VECTOR2:
    flValue =
        CastElement<CDmeVector2Log>(m_hLog)->GetKeyValue(nPoint)[m_nFieldIndex];
    break;

  case AT_VECTOR3:
    flValue =
        CastElement<CDmeVector3Log>(m_hLog)->GetKeyValue(nPoint)[m_nFieldIndex];
    break;
  ...
  case AT_QANGLE:
    flValue =
        CastElement<CDmeQAngleLog>(m_hLog)->GetKeyValue(nPoint)[m_nFieldIndex];
    break;

  case AT_QUATERNION:
    flValue = CastElement<CDmeQuaternionLog>(m_hLog)->GetKeyValue(
        nPoint)[m_nFieldIndex];
    break;
  }
...
void CDmeLogEditPanel::SetDmeLog(CDmeLog *pLog) {
  bool bValid = pLog && (pLog->GetDataType() == AT_INT ||
                         pLog->GetDataType() == AT_FLOAT ||
                         pLog->GetDataType() == AT_COLOR);
  if (bValid) {
    m_hLog = pLog;
  } else {
...
CDmeLogEditFrame::CDmeLogEditFrame(vgui::Panel *pParent, const char *pTitle)
  ...
  m_pCancelButton =
      new Button(this, "CancelButton", "#GameUI_Cancel", this, "Cancel");
...
CDmeLogEditFrame::~CDmeLogEditFrame() { CleanUpMessage(); }
...
//-----------------------------------------------------------------------------
void CDmeLogEditFrame::DoModal(CDmeLog *pLog, DmeTime_t startTime,
                               DmeTime_t endTime, KeyValues *pKeyValues) {
  CleanUpMessage();
  ...
  case AT_COLOR:
    m_pFilter->AddItem("RGB Channel",
                       new KeyValues("Mask", "Value",
                                     CDmeLogEditPanel::FIELD_R |
                                         CDmeLogEditPanel::FIELD_G |
                                         CDmeLogEditPanel::FIELD_B));
    m_pFilter->AddItem("Red Channel", new KeyValues("Mask", "Value",
                                                    CDmeLogEditPanel::FIELD_R));
    m_pFilter->AddItem(
        "Green Channel",
        new KeyValues("Mask", "Value", CDmeLogEditPanel::FIELD_G));
    m_pFilter->AddItem(
        "Blue Channel",
        new KeyValues("Mask", "Value", CDmeLogEditPanel::FIELD_B));
    m_pFilter->AddItem(
        "Alpha Channel",
        new KeyValues("Mask", "Value", CDmeLogEditPanel::FIELD_A));
    break;

  case AT_VECTOR2:
    m_pFilter->AddItem(
        "X Channel", new KeyValues("Mask", "Value", CDmeLogEditPanel::FIELD_X));
    m_pFilter->AddItem(
        "Y Channel", new KeyValues("Mask", "Value", CDmeLogEditPanel::FIELD_Y));
    break;
  ...
  case AT_QANGLE:
    ...
    m_pFilter->AddItem(
        "Y Channel", new KeyValues("Mask", "Value", CDmeLogEditPanel::FIELD_Y));
    m_pFilter->AddItem(
        "Z Channel", new KeyValues("Mask", "Value", CDmeLogEditPanel::FIELD_Z));
    break;
  ...
  case AT_QUATERNION:
    ...
    m_pFilter->AddItem(
        "Z Channel", new KeyValues("Mask", "Value", CDmeLogEditPanel::FIELD_Z));
    m_pFilter->AddItem(
        "W Channel", new KeyValues("Mask", "Value", CDmeLogEditPanel::FIELD_W));
    break;
  }
