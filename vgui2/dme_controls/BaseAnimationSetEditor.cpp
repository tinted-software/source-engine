...
static const char *
NameForLayout(CBaseAnimationSetEditor::EAnimSetLayout_t layout, bool menu) {
  ...
  Assert(0);
  return menu ? g_AnimSetLayout[0].contextmenulabel
              : g_AnimSetLayout[0].shortname;
}

static CBaseAnimationSetEditor::EAnimSetLayout_t
LayoutForName(const char *name) {
  int c = ARRAYSIZE(g_AnimSetLayout);
  for (int i = 0; i < c; ++i) {
    const AnimSetLayout_t &data = g_AnimSetLayout[i];
...
void CBaseAnimationSetEditor::OnButtonToggled(KeyValues *params) {
  ...
  {
  // FIXME, could do this with MESSAGE_FUNC_PARAMS and look up "panel" ptr and
  compare to figure out which button was manipulated...
...
void CBaseAnimationSetEditor::ChangeLayout(EAnimSetLayout_t newLayout) {
  ...
  case LAYOUT_SPLIT: {
    ...
    m_hControlGroup->SetAutoResize(Panel::PIN_TOPLEFT,
    ...
    sub->SetAutoResize(Panel::PIN_TOPLEFT, Panel::AUTORESIZE_DOWNANDRIGHT, 0, 0,
                       0, 0);
  ...
  case LAYOUT_VERTICAL: {
    ...
    for (i = 0; i < list.Count(); ++i) {
      ...
      list[i]->SetAutoResize(Panel::PIN_TOPLEFT, Panel::AUTORESIZE_DOWNANDRIGHT,
                             0, 0, 0, 0);
    }
...
//-----------------------------------------------------------------------------
CDmeChannel *
CBaseAnimationSetEditor::FindImportChannel(CDmeChannel *pChannel,
