...
//
//-----------------------------------------------------------------------------
IMPLEMENT_DMEPANEL_FACTORY(CDmeDagEditPanel, DmeDag, "DmeDagPreview2",
                           "DmeDag Previewer 2", false);
IMPLEMENT_DMEPANEL_FACTORY(CDmeDagEditPanel, DmeSourceSkin,
                           "DmeSourceSkinPreview2", "MDL Skin Previewer 2",
                           false);
IMPLEMENT_DMEPANEL_FACTORY(CDmeDagEditPanel, DmeSourceAnimation,
                           "DmeSourceAnimationPreview2",
                           "MDL Animation Previewer 2", false);
IMPLEMENT_DMEPANEL_FACTORY(CDmeDagEditPanel, DmeDCCMakefile,
                           "DmeMakeFileOutputPreview2",
...
static int __cdecl AnimationNameSortFunc(vgui::ListPanel *pPanel,
...
// constructor, destructor
//-----------------------------------------------------------------------------
CDmeAnimationListPanel::CDmeAnimationListPanel(vgui::Panel *pParent,
                                               const char *pName)
...
void CDmeAnimationListPanel::OnItemSelected() {
  ...
  PostActionSignal(
      new KeyValues("AnimationSelected", "animationName", pAnimationName));
}
...
private:
  ...
  // Modify controls in the presets which have had stereo or multilevel settings
  // changed
  ...
  // Sets preset values
  void GenerateProceduralPresetValues(CDmElement *pPreset, CDmElement *pControl,
                                      bool bIdentity, float flForceValue);
  void
  GenerateProceduralPresetValues(CDmElement *pPreset,
...
//-----------------------------------------------------------------------------
void CDmeCombinationOperatorPanel::SetCombinationOperator(
...
// Sets preset values
//-----------------------------------------------------------------------------
void CDmeCombinationOperatorPanel::GenerateProceduralPresetValues(
    CDmElement *pPreset, CDmElement *ctrl, bool bForceValue,
    float flForceValue) {
...
void CDmeCombinationOperatorPanel::GenerateProceduralPresetValues(
    CDmElement *pPreset, const CDmrElementArray<CDmElement> &controls,
    bool bIdentity, float flForceValue /*= -1.0f*/) {
  ...
  for (int i = 0; i < c; ++i) {
    ...
    CDmElement *pControlValue =
        CreateElement<CDmElement>(pControl->GetName(), pPreset->GetFileId());
    GenerateProceduralPresetValues(pControlValue, pControl, bIdentity,
                                   flForceValue);
...
// Removes a controls from all presets
//-----------------------------------------------------------------------------
void CDmeCombinationOperatorPanel::RemoveAnimationControlFromPresets(
...
// Removes a controls from all presets
//-----------------------------------------------------------------------------
void CDmeCombinationOperatorPanel::RemoveAnimationControlFromSelectionGroups(
    const char *pControlName) {
  CDmeAnimationSet *pAnimationSet = m_AnimSet.Get();
...
void CDmeCombinationOperatorPanel::RemoveUnusedAnimationSetControls() {
  ...
  // Remove all controls in the animation set and in presets that don't exist in
  ...
  for (int i = nAnimSetCount; --i >= 0;) {
    ...
    if (m_hCombinationOperator.Get() &&
        m_hCombinationOperator->FindControlIndex(pControlName) >= 0)
      continue;
...
// changed
//-----------------------------------------------------------------------------
void CDmeCombinationOperatorPanel::ModifyExistingAnimationSetControl(
  ...
  bool bIsStereoControl =
      m_hCombinationOperator->IsStereoControl(nControlIndex);
  ...
  if (bIsAnimControlStereo != bIsStereoControl) {
    ...
    if (!bIsStereoControl) {
      ...
      const Vector2D &value =
          m_hCombinationOperator->GetStereoControlValue(nControlIndex);
  ...
  if (bIsAnimMultiControl != bIsMultiControl) {
    ...
    if (!bIsMultiControl) {
      ...
      pControlElement->SetValue<float>(
          "multilevel",
          m_hCombinationOperator->GetMultiControlLevel(nControlIndex));
    }
  }

  float flDefaultValue =
      m_hCombinationOperator->GetRawControlCount(nControlIndex) == 2 ? 0.5f
                                                                     : 0.0f;
...
// changed
//-----------------------------------------------------------------------------
void CDmeCombinationOperatorPanel::ModifyExistingAnimationSetPresets(
    CDmElement *pControlElement) {
  const char *pControlName = pControlElement->GetName();

  CDmeAnimationSet *pAnimationSet = m_AnimSet.Get();
  const CDmrElementArray<CDmePresetGroup> &presetGroupList =
      pAnimationSet->GetPresetGroups();
...
void CDmeCombinationOperatorPanel::ModifyExistingAnimationSetControls() {
  ...
  // Update the main controls; update defaults and add or remove combo + multi
  // data
  int nAnimSetCount = controls.Count();
  for (int i = nAnimSetCount; --i >= 0;) {
    CDmElement *pControlElement = controls[i];
...
void CDmeCombinationOperatorPanel::AddNewAnimationSetControls() {
  ...
  for (int i = 0; i < nCombinationControlCount; ++i) {
    ...
    CDmElement *pControl =
        CreateElement<CDmElement>(pControlName, pAnimationSet->GetFileId());
    Assert(pControl);
    ...
    if (bIsMultiControl) {
      pControl->SetValue("multilevel",
                         m_hCombinationOperator->GetMultiControlLevel(i));
    }
...
void CDmeCombinationOperatorPanel::ComputeProceduralPresets() {
  ...
  GenerateProceduralPresetValues(pPreset, pAnimationSet->GetControls(), true,
                                 0.0f);
...
void CDmeCombinationOperatorPanel::SortAnimationSetControls() {
  ...
  DmElementHandle_t *pElements =
      (DmElementHandle_t *)_alloca(nControlCount * sizeof(DmElementHandle_t));
  for (int i = 0; i < nCombinationControlCount; ++i) {
    const char *pControlName = m_hCombinationOperator->GetControlName(i);
...
//-----------------------------------------------------------------------------
// Called when the animation set has to be made to match the combination
// operator
//-----------------------------------------------------------------------------
void CDmeCombinationOperatorPanel::RefreshAnimationSet() {
  ...
  // Add all controls not in the animation set but which do exist in the
  // combination system
  ...
  if (m_hCombinationOperator) {
    {
      ...
      for (int i = 0; i < c; ++i) {
        ...
        if (m_hCombinationOperator->IsStereoControl(i)) {
          m_hCombinationOperator->SetControlValue(
              i, value.m_pValue[ANIM_CONTROL_VALUE],
        ...
        if (m_hCombinationOperator->IsMultiControl(i)) {
          m_hCombinationOperator->SetMultiControlLevel(
              i, value.m_pValue[ANIM_CONTROL_MULTILEVEL]);
        }
      }
    }
...
// Constructor, destructor
//-----------------------------------------------------------------------------
CDmeDagEditPanel::CDmeDagEditPanel(vgui::Panel *pParent, const char *pName)
    : BaseClass(pParent, pName) {
  m_pPropertiesSplitter = new vgui::Splitter(this, "PropertiesSplitter",
  ...
  m_pDagRenderPanel =
      new CDmeDagRenderPanel(pSplitterRightSide, "DagRenderPanel");
  ...
  m_pCombinationPage =
      new vgui::PropertyPage(m_pEditorSheet, "AnimationSetEditor");
  m_pVertexAnimationPage =
      new vgui::PropertyPage(m_pEditorSheet, "VertexAnimationPage");

  m_pCombinationPanel = new CDmeCombinationOperatorPanel(
      m_pCombinationPage, "AnimationSetEditorPanel");
  m_pCombinationPanel->CreateToolsSubPanels();

  m_pAnimationListPanel =
      new CDmeAnimationListPanel(m_pAnimationPage, "AnimationListPanel");
  m_pAnimationListPanel->AddActionSignalTarget(this);
  m_pVertexAnimationListPanel = new CDmeAnimationListPanel(
      m_pVertexAnimationPage, "VertexAnimationListPanel");
  m_pVertexAnimationListPanel->AddActionSignalTarget(this);

  m_pCombinationPage->LoadControlSettingsAndUserConfig(
      "resource/dmedageditpanel_animationseteditorpage.res");
  m_pAnimationPage->LoadControlSettingsAndUserConfig(
      "resource/dmedageditpanel_animationpage.res");
  m_pVertexAnimationPage->LoadControlSettingsAndUserConfig(
      "resource/dmedageditpanel_vertexanimationpage.res");
...
void CDmeDagEditPanel::SetMakefileRootElement(CDmElement *pRoot) {
  ...
  CDmeAnimationList *pVertexAnimationList =
      pRoot->GetValueElement<CDmeAnimationList>("vertexAnimationList");
...
void CDmeDagEditPanel::SetDmeElement(CDmeSourceAnimation *pAnimation) {
  ...
  //	if ( pAnimationList->FindAnimation( pAnimation->m_SourceAnimationName )
  //< 0 ) 		return;
}
...
// paint it!
//-----------------------------------------------------------------------------
void CDmeDagEditPanel::Paint() { BaseClass::Paint(); }
...
void CDmeDagEditPanel::SetVertexAnimationList(
    CDmeAnimationList *pAnimationList) {
...
void CDmeDagEditPanel::OnPageChanged() {
  ...
  if (m_pEditorSheet->GetActivePage() == m_pAnimationPage) {
    m_pDagRenderPanel->SelectAnimation(
        m_pAnimationListPanel->GetSelectedAnimation());
    m_pDagRenderPanel->SelectVertexAnimation("");
    return;
  }
  ...
  if (m_pEditorSheet->GetActivePage() == m_pVertexAnimationPage) {
    m_pDagRenderPanel->SelectAnimation("");
    m_pDagRenderPanel->SelectVertexAnimation(
        m_pVertexAnimationListPanel->GetSelectedAnimation());
    return;
  }
}
