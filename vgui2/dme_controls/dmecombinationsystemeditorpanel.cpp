...
//
//-----------------------------------------------------------------------------
IMPLEMENT_DMEPANEL_FACTORY(CDmeCombinationSystemEditorPanel,
                           DmeCombinationOperator,
                           "DmeCombinationOperatorEditor",
...
//-----------------------------------------------------------------------------
static void ImportCombinationControls(CDmeCombinationOperator *pDestComboOp,
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    pStatusFrame->AddOperation(
        pControlName, bPartialMatch ? "Partial rule match" : "Successful");
    ...
    ControlIndex_t index =
        pDestComboOp->FindOrCreateControl(pControlName, bIsStereo);
...
//-----------------------------------------------------------------------------
static void ImportDominationRules(CDmeCombinationOperator *pDestComboOp,
                                  CDmeCombinationOperator *pSrcComboOp,
                                  COperationFileListFrame *pStatusFrame) {
  ...
  for (int i = 0; i < nRuleCount; ++i) {
    ...
    for (int j = 0; j < nDominatorCount; ++j) {
      ...
      if (!pDestComboOp->HasRawControl(pDominatorName)) {
        bMismatch = true;
        pStatusFrame->AddOperation(pDominatorName,
                                   "Missing raw control for dominance rule");
        break;
      }
    }
    ...
    for (int j = 0; j < nSuppressedCount; ++j) {
      ...
      if (!pDestComboOp->HasRawControl(pSuppressedName)) {
        bMismatch = true;
        pStatusFrame->AddOperation(pSuppressedName,
                                   "Missing raw control for dominance rule");
        break;
      }
    }
...
//-----------------------------------------------------------------------------
static bool ImportCombinationData(vgui::Panel *pParent,
  ...
  CDmeCombinationOperator *pComboOp =
      CastElement<CDmeCombinationOperator>(pRoot);
  if (!pComboOp) {
    pComboOp =
  ...
  if (pComboOp) {
    ...
    COperationFileListFrame *pStatusFrame = new COperationFileListFrame(
        pParent, "Import Status", "Status", false, true);
...
public:
  ...
  CDmeInputControlListPanel(vgui::Panel *pParent, const char *pName,
...
public:
  // constructor, destructor
  CDmeRawControlListPanel(vgui::Panel *pParent, const char *pName,
                          CDmeCombinationControlsPanel *pComboPanel);
...
private:
  ...
  // Builds a list of selected control + raw control names, returns true if any
  // control is stereo
  void BuildSelectedControlLists(bool bOnlyGroupedControls,
                                 CUtlVector<CUtlString> &controlNames,
                                 CUtlVector<CUtlString> &rawControlNames,
                                 bool *pbStereo = NULL, bool *pbEyelid = NULL);

  // If it finds a duplicate control name, reports an error message and returns
  // it found one
  bool HasDuplicateControlName(const char *pControlName,
...
//-----------------------------------------------------------------------------
static int __cdecl ControlNameSortFunc(vgui::ListPanel *pPanel,
...
static int __cdecl PeakSortFunc(vgui::ListPanel *pPanel,
                                const vgui::ListPanelItem &item1,
                                const vgui::ListPanelItem &item2) {
...
// constructor, destructor
//-----------------------------------------------------------------------------
CDmeCombinationControlsPanel::CDmeCombinationControlsPanel(vgui::Panel *pParent,
                                                           const char *pName)
    : BaseClass(pParent, pName) {
  m_pSplitter = new vgui::Splitter(this, "ControlsSplitter",
  ...
  m_pControlList =
      new CDmeInputControlListPanel(pSplitterLeftSide, "ControlList", this);
  ...
  m_pRawControlList =
      new CDmeRawControlListPanel(pSplitterRightSide, "RawControlList", this);
...
//-----------------------------------------------------------------------------
void CDmeCombinationControlsPanel::SetCombinationOperator(
...
CDmeCombinationOperator *
CDmeCombinationControlsPanel::GetCombinationOperator() {
...
void CDmeCombinationControlsPanel::RefreshCombinationOperator() {
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    float flDefault =
        m_hCombinationOperator->GetRawControlCount(i) == 2 ? 0.5f : 0.0f;
    const char *pName = m_hCombinationOperator->GetControlName(i);
    KeyValues *kv = new KeyValues("node", "name", pName);
    kv->SetString("stereo",
                  m_hCombinationOperator->IsStereoControl(i) ? "On" : "Off");
    kv->SetString("eyelid",
                  m_hCombinationOperator->IsEyelidControl(i) ? "On" : "Off");
    kv->SetFloat("default", flDefault);
    kv->SetColor("cellcolor", bIsMultiControl ? Color(192, 192, 0, 255)
...
void CDmeCombinationControlsPanel::NotifyDataChanged() {
  PostActionSignal(
      new KeyValues("DmeElementChanged", "DmeCombinationControlsPanel", 1));
}
...
void CDmeCombinationControlsPanel::RefreshRawControlNames() {
  ...
  ControlIndex_t nControlIndex =
      m_hCombinationOperator->FindControlIndex(pControlName);
  if (nControlIndex < 0)
    return;
  ...
  for (int i = 0; i < nCount; ++i) {
    KeyValues *kv = new KeyValues(
        "node", "name",
        m_hCombinationOperator->GetRawControlName(nControlIndex, i));
    switch (nCount) {
    case 0:
    case 1:
    ...
    float flWrinkleScale =
        m_hCombinationOperator->GetRawControlWrinkleScale(nControlIndex, i);
    kv->SetString("wrinkletype",
                  (flWrinkleScale < 0.0f) ? "Compress" : "Stretch");
...
// Called to open a context-sensitive menu for a particular menu item
//-----------------------------------------------------------------------------
const char *CDmeCombinationControlsPanel::GetSelectedRawControl(
    ControlIndex_t &nControlIndex) {
  if (!m_hCombinationOperator.Get())
    return NULL;
...
void CDmeCombinationControlsPanel::MoveControlInFrontOf(
    const char *pDragControl, const char *pDropControl) {
...
void CDmeCombinationControlsPanel::OnToggleWrinkleType() {
  ...
  float flWrinkleScale = m_hCombinationOperator->GetRawControlWrinkleScale(
      nControlIndex, pRawControlName);
  m_hCombinationOperator->SetWrinkleScale(nControlIndex, pRawControlName,
                                          -flWrinkleScale);
  RefreshRawControlNames();
  m_hCombinationOperator->GenerateWrinkleDeltas();
}

void CDmeCombinationControlsPanel::SetRawControlWrinkleValue(
...
void CDmeCombinationControlsPanel::OnOpenRawControlsContextMenu() {
  ...
  m_hContextMenu->AddMenuItem("#DmeCombinationSystemEditor_MoveUp",
                              new KeyValues("MoveUp"), this);
  m_hContextMenu->AddMenuItem("#DmeCombinationSystemEditor_MoveDown",
                              new KeyValues("MoveDown"), this);
  m_hContextMenu->AddMenuItem("#DmeCombinationSystemEditor_ToggleWrinkleType",
                              new KeyValues("ToggleWrinkleType"), this);
...
void CDmeCombinationControlsPanel::OnOpenContextMenu(KeyValues *kv) {
  ...
  if (nSelectedItemCount > 1) {
    m_hContextMenu->AddMenuItem("#DmeCombinationSystemEditor_GroupControls",
                                new KeyValues("GroupControls"), this);
  }

  if (bGroupedControls) {
    m_hContextMenu->AddMenuItem("#DmeCombinationSystemEditor_UngroupControls",
                                new KeyValues("UngroupControls"), this);
  }

  if (nSelectedItemCount >= 1) {
    int nMenuItemID = m_hContextMenu->AddCheckableMenuItem(
  ...
  if (nSelectedItemCount == 1) {
    m_hContextMenu->AddMenuItem("#DmeCombinationSystemEditor_RenameControl",
                                new KeyValues("RenameControl"), this);
  }

  if (nSelectedItemCount >= 1) {
    m_hContextMenu->AddSeparator();
    m_hContextMenu->AddMenuItem("#DmeCombinationSystemEditor_MoveUp",
                                new KeyValues("MoveUpInputControl"), this);
    m_hContextMenu->AddMenuItem("#DmeCombinationSystemEditor_MoveDown",
                                new KeyValues("MoveDownInputControl"), this);
  }

  if (nSelectedItemCount >= 1 || bGroupedControls) {
    m_hContextMenu->AddSeparator();
  }
  m_hContextMenu->AddMenuItem("#DmeCombinationSystemEditor_Import",
                              new KeyValues("ImportCombination"), this);
...
void CDmeCombinationControlsPanel::BuildSelectedControlLists(
    bool bOnlyGroupedControls, CUtlVector<CUtlString> &controlNames,
    CUtlVector<CUtlString> &rawControlNames, bool *pbStereo, bool *pbEyelid) {
  ...
  for (int i = 0; i < nSelectedItemCount; ++i) {
    ...
    int nRawControlCount =
        m_hCombinationOperator->GetRawControlCount(nControlIndex);
    ...
    for (int j = 0; j < nRawControlCount; ++j) {
      rawControlNames.AddToTail(
          m_hCombinationOperator->GetRawControlName(nControlIndex, j));
    }
  }
...
// found one
//-----------------------------------------------------------------------------
bool CDmeCombinationControlsPanel::HasDuplicateControlName(
    const char *pControlName, CUtlVector<CUtlString> &retiredControlNames) {
  int i;
  ...
  if (i == nRetiredControlNameCount) {
    ...
    if (m_hCombinationOperator->FindControlIndex(pControlName) >= 0) {
      vgui::MessageBox *pError = new vgui::MessageBox(
          "#DmeCombinationSystemEditor_DuplicateNameTitle",
...
// Called by OnGroupControls and OnRenameControl after we get a new group name
//-----------------------------------------------------------------------------
void CDmeCombinationControlsPanel::PerformGroupControls(
  ...
  // assuming it's not in the list of grouped control names we're going to group
  // together
  ...
  // Create new control
  ControlIndex_t nNewControl = m_hCombinationOperator->FindOrCreateControl(
      pGroupedControlName, bIsStereo);
...
// Called by OnGroupControls after we get a new group name
//-----------------------------------------------------------------------------
void CDmeCombinationControlsPanel::PerformRenameControl(
  ...
  // assuming it's not in the list of grouped control names we're going to group
  // together
  ControlIndex_t nFoundIndex =
      m_hCombinationOperator->FindControlIndex(pNewControlName);
...
void CDmeCombinationControlsPanel::OnGroupControls() {
  vgui::InputDialog *pInput = new vgui::InputDialog(
      this, "Group Controls", "Enter name of grouped control");
...
void CDmeCombinationControlsPanel::OnUngroupControls() {
  ...
  bool bIsStereo = false;
  bool bIsEyelid = false;
  BuildSelectedControlLists(true, controlNames, rawControlNames, &bIsStereo,
                            &bIsEyelid);
  ...
  int nGroupedControlCount = rawControlNames.Count();
  for (int i = 0; i < nGroupedControlCount; ++i) {
    const int nControlIndex = m_hCombinationOperator->FindOrCreateControl(
        rawControlNames[i], bIsStereo, true);
...
void CDmeCombinationControlsPanel::OnToggleStereoControl() {
  // Build lists of selected controls + raw controls
  // Yeah, this isn't super efficient, but this UI is not going to be
  // super-polished
  CUtlVector<CUtlString> controlNames;
  CUtlVector<CUtlString> rawControlNames;
  ...
  for (int i = 0; i < nControlCount; ++i) {
    ControlIndex_t nControlIndex =
        m_hCombinationOperator->FindControlIndex(controlNames[i]);
...
void CDmeCombinationControlsPanel::OnToggleEyelidControl() {
  ...
  bool bIsEyelid = false;
  BuildSelectedControlLists(false, controlNames, rawControlNames, NULL,
                            &bIsEyelid);
...
void CDmeCombinationControlsPanel::OnRenameControl() {
  ...
  vgui::InputDialog *pInput = new vgui::InputDialog(
      this, "Rename Control", "Enter new name of control");
  pInput->SetMultiline(false);
...
void CDmeCombinationControlsPanel::OnImportCombination() {
  ...
  vgui::FileOpenDialog *pDialog = new vgui::FileOpenDialog(
      this, "Select File to Import", true, new KeyValues("ImportControls"));
...
// Constructor, destructor
//-----------------------------------------------------------------------------
CDmeInputControlListPanel::CDmeInputControlListPanel(
...
// Called when a list panel's selection changes
//-----------------------------------------------------------------------------
void CDmeInputControlListPanel::OnPanelDropped(
...
// Constructor, destructor
//-----------------------------------------------------------------------------
CDmeRawControlListPanel::CDmeRawControlListPanel(
    vgui::Panel *pParent, const char *pName,
    CDmeCombinationControlsPanel *pComboPanel)
    : BaseClass(pParent, pName), m_pComboPanel(pComboPanel) {
...
public:
  ...
  void DoModal(CDmeCombinationOperator *pCombinationOperator,
...
private:
  // Refreshes the list of raw controls
  void RefreshRawControlNames(CDmeCombinationOperator *pCombinationOperator,
...
CRawControlPickerFrame::CRawControlPickerFrame(vgui::Panel *pParent,
                                               const char *pTitle)
  ...
  m_pOpenButton =
      new vgui::Button(this, "OkButton", "#MessageBox_OK", this, "Ok");
  m_pCancelButton = new vgui::Button(this, "CancelButton", "#MessageBox_Cancel",
                                     this, "Cancel");
  SetBlockDragChaining(true);

  LoadControlSettingsAndUserConfig(
      "resource/dmecombinationsystemeditor_rawcontrolpickerframe.res");

  SetTitle(pTitle, false);
}

CRawControlPickerFrame::~CRawControlPickerFrame() { CleanUpMessage(); }

//-----------------------------------------------------------------------------
// Refreshes the list of raw controls
//-----------------------------------------------------------------------------
void CRawControlPickerFrame::RefreshRawControlNames(
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    if ((bChooseSuppressed && bIsDominator) ||
        (!bChooseSuppressed && bIsSuppressed))
      continue;

    KeyValues *kv = new KeyValues("node", "name",
                                  pCombinationOperator->GetRawControlName(i));
    int nItemID = m_pRawControlList->AddItem(kv, 0, false, false);
    if ((bChooseSuppressed && bIsSuppressed) ||
        (!bChooseSuppressed && bIsDominator)) {
...
// Sets the current scene + animation list
//-----------------------------------------------------------------------------
void CRawControlPickerFrame::DoModal(
    CDmeCombinationOperator *pCombinationOperator,
    CDmeCombinationDominationRule *pRule, bool bSuppressed,
    KeyValues *pContextKeyValues) {
  CleanUpMessage();
...
class CDmeCombinationDominationRulesPanel : public vgui::EditablePanel {
  DECLARE_CLASS_SIMPLE(CDmeCombinationDominationRulesPanel,
                       vgui::EditablePanel);
...
static int __cdecl DominatorNameSortFunc(vgui::ListPanel *pPanel,
                                         const vgui::ListPanelItem &item1,
                                         const vgui::ListPanelItem &item2) {
...
// Sets the combination operator
//-----------------------------------------------------------------------------
void CDmeCombinationDominationRulesPanel::SetCombinationOperator(
    CDmeCombinationOperator *pOp) {
  if (pOp != m_hCombinationOperator.Get()) {
    m_hCombinationOperator = pOp;
    RefreshCombinationOperator();
...
void CDmeCombinationDominationRulesPanel::RefreshCombinationOperator() {
  ...
  for (int i = 0; i < nCount; ++i) {
    CDmeCombinationDominationRule *pRule =
        m_hCombinationOperator->GetDominationRule(i);
    ...
    const char **ppStrings =
        (const char **)_alloca(nControlCount * sizeof(const char *));
    for (int j = 0; j < nControlCount; ++j) {
      ppStrings[j] = pRule->GetDominator(j);
    }
    qsort(ppStrings, (size_t)nControlCount, (size_t)sizeof(char *),
          ControlNameSortFunc);
    for (int j = 0; j < nControlCount; ++j) {
      nLen +=
          Q_snprintf(&pTemp[nLen], sizeof(pTemp) - nLen, "%s ", ppStrings[j]);
    }
...
void CDmeCombinationDominationRulesPanel::NotifyDataChanged() {
  PostActionSignal(new KeyValues("DmeElementChanged",
                                 "DmeCombinationDominationRulesPanel", 2));
}

//-----------------------------------------------------------------------------
// Returns the currently selected domination rule
//-----------------------------------------------------------------------------
CDmeCombinationDominationRule *
CDmeCombinationDominationRulesPanel::GetSelectedRule() {
  if (!m_hCombinationOperator.Get())
    return NULL;
...
void CDmeCombinationDominationRulesPanel::OnImportDominationRules() {
  ...
  vgui::FileOpenDialog *pDialog =
      new vgui::FileOpenDialog(this, "Select File to Import", true,
                               new KeyValues("ImportDominationRules"));
  pDialog->SetStartDirectoryContext("combination_system_import", pStartingDir);
  pDialog->AddFilter("*.dmx", "Exported model file (*.dmx)", true);
  pDialog->SetDeleteSelfOnClose(true);
  pDialog->AddActionSignalTarget(this);
...
void CDmeCombinationDominationRulesPanel::OnOpenContextMenu(KeyValues *kv) {
  ...
  m_hContextMenu = new vgui::Menu(this, "ActionMenu");
  m_hContextMenu->AddMenuItem("#DmeCombinationSystemEditor_AddDominationRule",
                              new KeyValues("AddDominationRule"), this);
  if (nSelectedItemCount > 0) {
    m_hContextMenu->AddMenuItem(
        "#DmeCombinationSystemEditor_RemoveDominationRule",
  ...
  if (nSelectedItemCount == 1) {
    ...
    m_hContextMenu->AddSeparator();
    m_hContextMenu->AddMenuItem("#DmeCombinationSystemEditor_SelectSuppressed",
                                new KeyValues("SelectSuppressed"), this);
    m_hContextMenu->AddMenuItem("#DmeCombinationSystemEditor_SelectDominators",
                                new KeyValues("SelectDominators"), this);
    m_hContextMenu->AddSeparator();
    m_hContextMenu->AddMenuItem("#DmeCombinationSystemEditor_MoveUp",
                                new KeyValues("MoveUp"), this);
    m_hContextMenu->AddMenuItem("#DmeCombinationSystemEditor_MoveDown",
  ...
  m_hContextMenu->AddSeparator();
  m_hContextMenu->AddMenuItem("#DmeCombinationSystemEditor_ImportDomination",
                              new KeyValues("ImportDominationRules"), this);
...
// Selects a particular domination rule
//-----------------------------------------------------------------------------
void CDmeCombinationDominationRulesPanel::SelectRule(
    CDmeCombinationDominationRule *pRule) {
  for (int nItemID = m_pDominationRulesList->FirstItem();
       nItemID != m_pDominationRulesList->InvalidItemID();
       nItemID = m_pDominationRulesList->NextItem(nItemID)) {
    KeyValues *pKeyValues = m_pDominationRulesList->GetItem(nItemID);
    if (pRule ==
        GetElementKeyValue<CDmeCombinationDominationRule>(pKeyValues, "rule")) {
...
void CDmeCombinationDominationRulesPanel::OnAddDominationRule() {
  CDmeCombinationDominationRule *pRule =
      m_hCombinationOperator->AddDominationRule();
  RefreshCombinationOperator();
  SelectRule(pRule);
  OnSelectSuppressed();
  NotifyDataChanged();
...
//-----------------------------------------------------------------------------
void CDmeCombinationDominationRulesPanel::OnRawControlPicked(
  ...
  if (pContextKeys) {
    CDmeCombinationDominationRule *pRule =
        GetElementKeyValue<CDmeCombinationDominationRule>(pContextKeys, "rule");
    if (!pRule)
      return;

    pRule->RemoveAllDominators();
    for (KeyValues *pKey = pControlList->GetFirstValue(); pKey;
...
void CDmeCombinationDominationRulesPanel::OnSelectDominators() {
  ...
  CRawControlPickerFrame *pPicker =
      new CRawControlPickerFrame(this, "Select Dominator(s)");
...
void CDmeCombinationDominationRulesPanel::OnSelectSuppressed() {
  ...
  CRawControlPickerFrame *pPicker =
      new CRawControlPickerFrame(this, "Select Suppressed");
...
CDmeCombinationSystemEditorPanel::CDmeCombinationSystemEditorPanel(
  ...
  m_pDominationRulesPage =
      new vgui::PropertyPage(m_pEditorSheet, "DominationRulesPage");
  m_pPropertiesPage = new vgui::PropertyPage(m_pEditorSheet, "PropertiesPage");

  m_pControlsPanel = new CDmeCombinationControlsPanel((vgui::Panel *)NULL,
                                                      "CombinationControls");
  m_pControlsPanel->AddActionSignalTarget(this);

  m_pDominationRulesPanel = new CDmeCombinationDominationRulesPanel(
      (vgui::Panel *)NULL, "DominationRules");
  m_pDominationRulesPanel->AddActionSignalTarget(this);

  m_pPropertiesPanel =
      new CDmeElementPanel((vgui::Panel *)NULL, "PropertiesPanel");
  m_pPropertiesPanel->AddActionSignalTarget(this);

  m_pControlsPanel->LoadControlSettingsAndUserConfig(
      "resource/dmecombinationsystemeditorpanel_controlspage.res");
  ...
  LoadControlSettingsAndUserConfig(
      "resource/dmecombinationsystemeditorpanel.res");
...
//-----------------------------------------------------------------------------
void CDmeCombinationSystemEditorPanel::SetDmeElement(
    CDmeCombinationOperator *pComboOp) {
...
CDmeCombinationSystemEditorFrame::CDmeCombinationSystemEditorFrame(
    vgui::Panel *pParent, const char *pTitle)
    : BaseClass(pParent, "DmeCombinationSystemEditorFrame") {
  SetDeleteSelfOnClose(true);
  m_pEditor = new CDmeCombinationSystemEditorPanel(
      this, "DmeCombinationSystemEditorPanel");
  m_pEditor->AddActionSignalTarget(this);
  m_pOpenButton = new vgui::Button(this, "OpenButton", "#FileOpenDialog_Open",
                                   this, "Open");
  m_pCancelButton = new vgui::Button(this, "CancelButton",
                                     "#FileOpenDialog_Cancel", this, "Cancel");
  SetBlockDragChaining(true);

  LoadControlSettingsAndUserConfig(
      "resource/dmecombinationsystemeditorframe.res");
...
// Sets the current scene + animation list
//-----------------------------------------------------------------------------
void CDmeCombinationSystemEditorFrame::SetCombinationOperator(
