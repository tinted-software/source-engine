...
CSoundRecordPanel::CSoundRecordPanel(vgui::Panel *pParent, const char *pTitle)
  ...
  m_pOkButton =
      new Button(this, "OkButton", "#FileOpenDialog_Open", this, "Ok");
...
CSoundRecordPanel::~CSoundRecordPanel() { StopSoundPreview(); }
...
void CSoundRecordPanel::DoModal(const char *pFileName) {
  ...
  g_pFullFileSystem->FullPathToRelativePath(pFileName, pRelativeWAVPath,
  ...
  g_pFullFileSystem->RelativePathToFullPath(pRelativeDir, "MOD", pFoundFullPath,
                                            sizeof(pFoundFullPath));
  ...
  if (bBadDirectory) {
    ...
    Q_snprintf(pBuf, sizeof(pBuf),
               "File %s is in a bad directory!\nAudio must be recorded into "
               "your mod's sound/ directory.\n",
               pFileName);
    vgui::MessageBox *pMessageBox =
        new vgui::MessageBox("Bad Save Directory!\n", pBuf, GetParent());
    pMessageBox->DoModal();
    return;
  }
...
void CSoundRecordPanel::PlaySoundPreview() {
  ...
  m_nPlayingSound = EngineTool()->StartSound(
      0, true, -1, CHAN_STATIC, m_EngineFileName, VOL_NORM, SNDLVL_NONE,
...
void CSoundRecordPanel::OnCommand(const char *pCommand) {
  ...
  if (!Q_stricmp(pCommand, "Ok")) {
    PostActionSignal(
        new KeyValues("SoundRecorded", "relativepath", m_EngineFileName.Get()));
    CloseModal();
    return;
  }
