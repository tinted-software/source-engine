...
// ----------------------------------------------------------------------------
CAttributeElementPickerPanel::CAttributeElementPickerPanel(
...
void CAttributeElementPickerPanel::PostConstructor() { Refresh(); }
...
void CAttributeElementPickerPanel::Refresh() {
  ...
  if (element) {
    ...
    if (m_bShowMemoryUsage) {
      Q_snprintf(elemText, sizeof(elemText), "%s %s %.3fMB",
                 element->GetTypeString(), idstr,
                 element->EstimateMemoryUsage() / float(1 << 20));
    } else {
      Q_snprintf(elemText, sizeof(elemText), "%s %s", element->GetTypeString(),
                 idstr);
    }
  }
...
void CAttributeElementPickerPanel::ShowPickerDialog() {
  CDmeEditorChoicesInfo *pInfo =
      CastElement<CDmeEditorChoicesInfo>(GetEditorInfo());
  if (!pInfo)
    return;
  ...
  if (ElementPropertiesChoices()->GetElementChoiceList(
  ...
  CDmePickerFrame *pDmePickerDialog =
      new CDmePickerFrame(this, "Select DME Element");
