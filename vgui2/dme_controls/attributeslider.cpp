...
CAttributeSlider::CAttributeSlider(CBaseAnimSetAttributeSliderPanel *parent,
    ...
    : BaseClass((Panel *)parent, panelName), m_pParent(parent), m_pWhite(NULL),
      m_bPreviewEnabled(false), m_bSimplePreviewOnly(true),
      m_bCursorInsidePanel(false), m_flPreviewGoalTime(-1.0f), m_bRampUp(false),
      m_bFaderBeingDragged(false), m_flFaderAmount(1.0f),
      m_bIsLogPreviewControl(false), m_bSelected(false), m_pRightTextField(0) {
...
void CAttributeSlider::PaintBackground() {
  ...
  int nMidPoint =
      (int)((float)rect.width * clamp(flMidPoint, 0.0f, 1.0f) + 0.5f);
  ...
  if (viewType == ANIM_CONTROL_VALUE && IsControlActive(ANIM_CONTROL_BALANCE)) {
    ...
    int nLeftValue =
        (int)((float)rect.width * clamp(flLeftValue, 0.0f, 1.0f) + 0.5f);
    int nRightValue =
        (int)((float)rect.width * clamp(flRightValue, 0.0f, 1.0f) + 0.5f);
    ...
    surface()->DrawFilledRect(rect.x + min(nLeftValue, nMidPoint), y0,
                              rect.x + max(nLeftValue, nMidPoint), y1);
    surface()->DrawFilledRect(rect.x + min(nRightValue, nMidPoint), y1,
                              rect.x + max(nRightValue, nMidPoint), y2);
  } else {
    ...
    surface()->DrawSetColor(
        IsDragging() ? s_DraggingBarColor
                     : s_BarColor[m_bIsLogPreviewControl][IsSelected()]);
    surface()->DrawFilledRect(rect.x + min(nValue, nMidPoint), y0,
                              rect.x + max(nValue, nMidPoint), y2);
  }
