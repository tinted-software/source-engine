...
//-----------------------------------------------------------------------------
CDmePanel::CDmePanel(vgui::Panel *pParent, const char *pPanelName,
...
CDmePanel::~CDmePanel() { DeleteCachedPanels(); }
...
void CDmePanel::PopulateEditorNames(const char *pPanelName) {
  ...
  for (; h != DMEFACTORY_HANDLE_INVALID;
       h = DmePanelNextFactory(h, m_hElement.Get())) {
    ...
    if (pPreferredEditor && !bFoundPanelName &&
        !Q_stricmp(pPreferredEditor, pEditorName)) {
      nBestInheritanceDepth = 0;
      nActiveItemID = nItemID;
      continue;
    }
...
void CDmePanel::OnCut() { PostMessageToDmePanel("OnCut"); }

void CDmePanel::OnCopy() { PostMessageToDmePanel("OnCopy"); }

void CDmePanel::OnPaste() { PostMessageToDmePanel("OnPaste"); }

void CDmePanel::OnPasteInsert() { PostMessageToDmePanel("OnPasteInsert"); }

void CDmePanel::OnPasteReference() {
  PostMessageToDmePanel("OnPasteReference");
}

void CDmePanel::OnEditDelete() { PostMessageToDmePanel("OnEditDelete"); }
...
void CDmePanel::OnViewedElementChanged(KeyValues *kv) {
  ...
  // when doing the back/forward searching. Just calling the normal
  // SetDmeElement doesn't work because it reorders the history. What we want is
  // to populate the combo box without causing the OnTextChanged message to get
  // sent.
  ...
  // If the current editor isn't supported by this new element, then just reset.
  // Too bad.
  bool bFound = false;
  if (m_CurrentEditorName.Length() && pElement) {
    DmeFactoryHandle_t h = DmePanelFirstFactory(pElement);
    for (; h != DMEFACTORY_HANDLE_INVALID;
         h = DmePanelNextFactory(h, pElement)) {
      const char *pEditorName = DmePanelFactoryName(h);
...
void CDmePanel::Refresh(bool bValuesOnly) {
  if (m_pDmeEditorPanel) {
    KeyValues *pKeyValues =
        new KeyValues("ElementChangedExternally", "valuesOnly", bValuesOnly);
...
void CDmePanel::SetEditor(const char *pEditorName) {
  ...
  if (!m_pDmeEditorPanel) {
    ...
    if (CreateDmePanel(this, "DmePanelEditor", m_hElement, pEditorName,
                       &entry)) {
...
//-----------------------------------------------------------------------------
void CDmePanel::SetDmeElement(CDmElement *pDmeElement, bool bForce,
...
// Constructor
//-----------------------------------------------------------------------------
CBaseDmePanelFactory::CBaseDmePanelFactory(const char *pElementType,
    ...
    if (!Q_stricmp(pFactory->m_pElementType, pElementType) &&
      ...
      if (!bIsOverride) {
        AssertMsg(pFactory->m_bIsOverride,
                  ("Two DmePanel factories have the same name (\"%s\") + type "
                   "(\"%s\")!\n",
...
DmeFactoryHandle_t DmePanelFirstFactory(CDmElement *pElement) {
  CBaseDmePanelFactory *pFactory =
      CBaseDmePanelFactory::s_pFirstDmePanelFactory;
...
//-----------------------------------------------------------------------------
bool CDmePanel::CreateDmePanel(vgui::Panel *pParent, const char *pPanelName,
  ...
  for (; pFactory; pFactory = pFactory->m_pNext) {
    ...
    int nInheritanceDepth =
        pElement->GetInheritanceDepth(pFactory->m_pElementType);
    Assert(nInheritanceDepth >= 0);
    if (nBestInheritanceDepth >= 0 &&
        (nInheritanceDepth > nBestInheritanceDepth))
      continue;
  ...
  if (pBestFactory) {
    ...
    pMap->m_pEditorPanel =
        pBestFactory->CreateDmePanel(pParent, pPanelName, pElement);
    return true;
  }
  return false;
}
