...
public:
  ...
  void DoModal(CDmeParticleSystemDefinition *pParticleSystem,
               ParticleFunctionType_t type, KeyValues *pContextKeyValues);
...
private:
  ...
  void RefreshParticleFunctions(CDmeParticleSystemDefinition *pDefinition,
...
static int __cdecl ParticleFunctionNameSortFunc(
    vgui::ListPanel *pPanel, const vgui::ListPanelItem &item1,
...
CParticleFunctionPickerFrame::CParticleFunctionPickerFrame(vgui::Panel *pParent,
                                                           const char *pTitle)
...
//-----------------------------------------------------------------------------
void CParticleFunctionPickerFrame::RefreshParticleFunctions(
  ...
  CUtlVector<IParticleOperatorDefinition *> &list =
      g_pParticleSystemMgr->GetAvailableParticleOperatorList(type);
  ...
  for (int i = 0; i < nFunctionCount; ++i) {
    const char *pFunctionName =
        pParticleSystem->GetParticleFunction(type, i)->GetName();
...
// Sets the current scene + animation list
//-----------------------------------------------------------------------------
void CParticleFunctionPickerFrame::DoModal(
    CDmeParticleSystemDefinition *pParticleSystem, ParticleFunctionType_t type,
...
public:
  CParticleChildrenPickerFrame(vgui::Panel *pParent, const char *pTitle,
...
static int __cdecl ParticleChildrenNameSortFunc(
    vgui::ListPanel *pPanel, const vgui::ListPanelItem &item1,
    const vgui::ListPanelItem &item2) {
  const char *string1 = item1.kv->GetString("name");
  const char *string2 = item2.kv->GetString("name");
...
// Refreshes the list of raw controls
//-----------------------------------------------------------------------------
void CParticleChildrenPickerFrame::RefreshChildrenList(
    CDmeParticleSystemDefinition *pCurrentParticleSystem) {
...
// Sets the current scene + animation list
//-----------------------------------------------------------------------------
void CParticleChildrenPickerFrame::DoModal(
    CDmeParticleSystemDefinition *pParticleSystem,
    KeyValues *pContextKeyValues) {
  CleanUpMessage();
...
void CParticleChildrenPickerFrame::OnCommand(const char *pCommand) {
  if (!Q_stricmp(pCommand, "Ok")) {
    ...
    CDmeParticleSystemDefinition *pParticleSystem =
        GetElementKeyValue<CDmeParticleSystemDefinition>(pKeyValues,
                                                         "particleSystem");
...
public:
  ...
  CParticleFunctionBrowser(vgui::Panel *pParent, const char *pName,
                           ParticleFunctionType_t type,
                           IParticleSystemPropertiesPanelQuery *pQuery);
...
//-----------------------------------------------------------------------------
static int __cdecl ParticleFunctionSortFunc(vgui::ListPanel *pPanel,
...
CParticleFunctionBrowser::CParticleFunctionBrowser(
    vgui::Panel *pParent, const char *pName, ParticleFunctionType_t type,
    IParticleSystemPropertiesPanelQuery *pQuery)
...
void CParticleFunctionBrowser::SelectDefaultFunction() {
  if (m_pFunctionList->GetSelectedItemsCount() == 0 &&
      m_pFunctionList->GetItemCount() > 0) {
...
//-----------------------------------------------------------------------------
void CParticleFunctionBrowser::SetParticleFunctionProperties(
...
//-----------------------------------------------------------------------------
void CParticleFunctionBrowser::SetParticleSystem(
    CDmeParticleSystemDefinition *pParticleSystem) {
...
void CParticleFunctionBrowser::RefreshParticleFunctionList() {
  ...
  for (int i = 0; i < nCount; ++i) {
    CDmeParticleFunction *pFunction =
        m_hParticleSystem->GetParticleFunction(m_FuncType, i);
...
CParticleFunctionBrowser::GetSelectedFunction(int nIndex) {
  if (!m_hParticleSystem.Get())
    return NULL;
  ...
  KeyValues *pKeyValues = m_pFunctionList->GetItem(nItemID);
  return GetElementKeyValue<CDmeParticleFunction>(pKeyValues,
                                                  "particleFunction");
}
...
void CParticleFunctionBrowser::OnMoveUp() {
  ...
  {
    CUndoScopeGuard guard(0, NOTIFY_SETDIRTYFLAG, "Move Function Up",
                          "Move Function Up");
...
void CParticleFunctionBrowser::OnMoveDown() {
  ...
  {
    CUndoScopeGuard guard(0, NOTIFY_SETDIRTYFLAG, "Move Function Down",
                          "Move Function Down");
...
// Select a particular particle function
//-----------------------------------------------------------------------------
void CParticleFunctionBrowser::SelectParticleFunction(
    CDmeParticleFunction *pFind) {
  m_pFunctionList->ClearSelectedItems();
  for (int nItemID = m_pFunctionList->FirstItem();
       nItemID != m_pFunctionList->InvalidItemID();
       nItemID = m_pFunctionList->NextItem(nItemID)) {
    KeyValues *kv = m_pFunctionList->GetItem(nItemID);
    CDmeParticleFunction *pFunction =
        GetElementKeyValue<CDmeParticleFunction>(kv, "particleFunction");
    if (pFunction == pFind) {
      m_pFunctionList->AddSelectedItem(nItemID);
      break;
    }
...
void CParticleFunctionBrowser::OnParticleChildPicked(KeyValues *pKeyValues) {
  ...
  CDmeParticleFunction *pFunction;
  {
    CUndoScopeGuard guard(0, NOTIFY_SETDIRTYFLAG, "Add Particle System Child",
                          "Add Particle System Child");
...
void CParticleFunctionBrowser::OnParticleFunctionPicked(KeyValues *pKeyValues) {
  ...
  CDmeParticleFunction *pFunction;
  {
    CUndoScopeGuard guard(0, NOTIFY_SETDIRTYFLAG, "Add Particle Function",
...
void CParticleFunctionBrowser::OnAdd() {
  if (m_FuncType != FUNCTION_CHILDREN) {
    CParticleFunctionPickerFrame *pPicker =
        new CParticleFunctionPickerFrame(this, "Select Particle Function");
    pPicker->DoModal(m_hParticleSystem, m_FuncType, NULL);
  } else {
    CParticleChildrenPickerFrame *pPicker = new CParticleChildrenPickerFrame(
        this, "Select Child Particle Systems", m_pQuery);
...
void CParticleFunctionBrowser::OnRemove() {
  ...
  {
    CUndoScopeGuard guard(0, NOTIFY_SETDIRTYFLAG, "Remove Particle Function",
...
void CParticleFunctionBrowser::OnInputCompleted(KeyValues *pKeyValues) {
  ...
  CDmeParticleFunction *pFunction = GetSelectedFunction();
  {
    CUndoScopeGuard guard(0, NOTIFY_SETDIRTYFLAG, "Rename Particle Function",
                          "Rename Particle Function");
...
void CParticleFunctionBrowser::OnRename() {
  ...
  vgui::InputDialog *pInput = new vgui::InputDialog(
      this, "Rename Particle Function", "Enter new name of particle function");
...
void CParticleFunctionBrowser::RefreshParticleFunctionProperties() {
  if (IsVisible()) {
    CDmeParticleFunction *pFunction = GetSelectedFunction();
    if (pFunction) {
      m_pParticleFunctionProperties->SetTypeDictionary(
          pFunction->GetEditorTypeDictionary());
    }
    m_pParticleFunctionProperties->SetDmeElement(pFunction);

    // Notify the outside world so we can get helpers to render correctly in the
    // preview
...
void CParticleFunctionBrowser::OnOpenContextMenu(KeyValues *kv) {
  ...
  m_hContextMenu->AddMenuItem("#ParticleFunctionBrowser_Add",
                              new KeyValues("Add"), this);
  if (nSelectedItemCount >= 1) {
    m_hContextMenu->AddMenuItem("#ParticleFunctionBrowser_Remove",
                                new KeyValues("Remove"), this);
  }
  if (nSelectedItemCount == 1) {
    m_hContextMenu->AddMenuItem("#ParticleFunctionBrowser_Rename",
                                new KeyValues("Rename"), this);
    m_hContextMenu->AddSeparator();
    m_hContextMenu->AddMenuItem("#ParticleFunctionBrowser_MoveUp",
                                new KeyValues("MoveUp"), this);
    m_hContextMenu->AddMenuItem("#ParticleFunctionBrowser_MoveDown",
...
// Constructor
//-----------------------------------------------------------------------------
CParticleSystemPropertiesPanel::CParticleSystemPropertiesPanel(
    IParticleSystemPropertiesPanelQuery *pQuery, vgui::Panel *pParent)
    : BaseClass(pParent, "ParticleSystemPropertiesPanel"), m_pQuery(pQuery) {
  SetKeyBoardInputEnabled(true);

  m_pSplitter =
      new vgui::Splitter(this, "Splitter", vgui::SPLITTER_MODE_VERTICAL, 1);
  vgui::Panel *pSplitterLeftSide = m_pSplitter->GetChild(0);
  vgui::Panel *pSplitterRightSide = m_pSplitter->GetChild(1);

  m_pFunctionBrowserArea =
      new vgui::EditablePanel(pSplitterLeftSide, "FunctionBrowserArea");
  m_pFunctionTypeCombo =
      new vgui::ComboBox(pSplitterLeftSide, "FunctionTypeCombo",
                         PARTICLE_FUNCTION_COUNT + 1, false);
  m_pFunctionTypeCombo->AddItem("Properties",
                                new KeyValues("choice", "index", -1));
  m_pFunctionTypeCombo->AddActionSignalTarget(this);

  m_pParticleFunctionProperties =
      new CDmeElementPanel(pSplitterRightSide, "FunctionProperties");
  m_pParticleFunctionProperties->SetAutoResize(
      vgui::Panel::PIN_TOPLEFT, vgui::Panel::AUTORESIZE_DOWNANDRIGHT, 6, 6, -6,
      -6);
  m_pParticleFunctionProperties->AddActionSignalTarget(this);

  for (int i = 0; i < PARTICLE_FUNCTION_COUNT; ++i) {
    const char *pTypeName =
        GetParticleFunctionTypeName((ParticleFunctionType_t)i);

    m_pFunctionTypeCombo->AddItem(pTypeName,
                                  new KeyValues("choice", "index", i));
    m_pParticleFunctionBrowser[i] =
        new CParticleFunctionBrowser(m_pFunctionBrowserArea, "FunctionBrowser",
                                     (ParticleFunctionType_t)i, m_pQuery);
    m_pParticleFunctionBrowser[i]->SetParticleFunctionProperties(
        m_pParticleFunctionProperties);
...
//-----------------------------------------------------------------------------
void CParticleSystemPropertiesPanel::SetParticleSystem(
    CDmeParticleSystemDefinition *pParticleSystem) {
  m_hParticleSystem = pParticleSystem;
  for (int i = 0; i < PARTICLE_FUNCTION_COUNT; ++i) {
  ...
  if (nPage < 0) {
    if (m_hParticleSystem.Get()) {
      m_pParticleFunctionProperties->SetTypeDictionary(
          m_hParticleSystem->GetEditorTypeDictionary());
    }
...
//-----------------------------------------------------------------------------
void CParticleSystemPropertiesPanel::OnDmeElementChanged(
    KeyValues *pKeyValues) {
  int nNotifyFlags = pKeyValues->GetInt("notifyFlags");
  OnParticleSystemModified();

  if (nNotifyFlags &
      (NOTIFY_CHANGE_TOPOLOGICAL | NOTIFY_CHANGE_ATTRIBUTE_ARRAY_SIZE)) {
    for (int i = 0; i < PARTICLE_FUNCTION_COUNT; ++i) {
...
void CParticleSystemPropertiesPanel::OnParticleFunctionSelChanged(
    KeyValues *pParams) {
  // Notify the outside world so we can get helpers to render correctly in the
  // preview
  CDmeParticleFunction *pFunction =
      GetElementKeyValue<CDmeParticleFunction>(pParams, "function");
  KeyValues *pMessage = new KeyValues("ParticleFunctionSelChanged");
  SetElementKeyValue(pMessage, "function", pFunction);
  PostActionSignal(pMessage);
}
...
void CParticleSystemPropertiesPanel::Refresh(bool bValuesOnly) {
  ...
  m_pParticleFunctionProperties->Refresh(
      bValuesOnly ? CElementPropertiesTreeInternal::REFRESH_VALUES_ONLY
...
private:
  MESSAGE_FUNC_INT(OnElementChangedExternally, "ElementChangedExternally",
                   valuesOnly);
  MESSAGE_FUNC(OnParticleSystemModified, "ParticleSystemModified");
  MESSAGE_FUNC_PARAMS(OnParticleFunctionSelChanged,
                      "ParticleFunctionSelChanged", params);
...
//-----------------------------------------------------------------------------
IMPLEMENT_DMEPANEL_FACTORY(CParticleSystemDmePanel, DmeParticleSystemDefinition,
                           "DmeParticleSystemDefinitionEditor",
                           "Particle System Editor", true);

//-----------------------------------------------------------------------------
// constructor, destructor
//-----------------------------------------------------------------------------
CParticleSystemDmePanel::CParticleSystemDmePanel(vgui::Panel *pParent,
                                                 const char *pName)
    : BaseClass(pParent, pName) {
  m_Splitter =
      new vgui::Splitter(this, "Splitter", SPLITTER_MODE_HORIZONTAL, 1);
  ...
  m_pProperties->SetAutoResize(PIN_TOPLEFT, AUTORESIZE_DOWNANDRIGHT, 0, 0, 0,
                               0);
...
// Layout
//-----------------------------------------------------------------------------
void CParticleSystemDmePanel::SetDmeElement(
