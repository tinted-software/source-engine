...
public:
  CAnimGroupTree(Panel *parent, const char *panelName,
...
CAnimGroupTree::CAnimGroupTree(Panel *parent, const char *panelName,
                               CBaseAnimSetControlGroupPanel *groupPanel)
...
bool CAnimGroupTree::IsItemDroppable(int itemIndex,
...
void CAnimGroupTree::OnItemDropped(int itemIndex,
                                   CUtlVector<KeyValues *> &msglist) {
...
void CAnimGroupTree::OnImportAnimation() {
  PostMessage(m_pGroupPanel->m_hEditor,
              new KeyValues("ImportAnimation", "visibleOnly", "1"), 0.0f);
}
...
CBaseAnimSetControlGroupPanel::CBaseAnimSetControlGroupPanel(
    ...
    : BaseClass(parent, className), m_bStartItemWasSelected(false),
      m_SliderNames(0, 0, true) {
  m_hEditor = editor;
  m_hGroups = new CAnimGroupTree(this, "AnimSetGroups", this);
  m_hGroups->SetMultipleItemDragEnabled(true);
  m_hGroups->SetAutoResize(Panel::PIN_TOPLEFT, Panel::AUTORESIZE_DOWNANDRIGHT,
                           0, 0, 0, 0);
...
static int AddItemToTree(TreeView *tv, const char *label, int parentIndex,
                         const Color &fg, int groupNumber, int handle) {
...
void CBaseAnimSetControlGroupPanel::OnTreeViewItemSelected(int itemIndex) {
  ...
  CBaseAnimSetAttributeSliderPanel *attSliders =
...
void CBaseAnimSetControlGroupPanel::ChangeAnimationSet(
  ...
  if (bRebuildGroups) {
    m_hGroups->SetFont(scheme()
                           ->GetIScheme(GetScheme())
                           ->GetFont("DefaultBold", IsProportional()));
    ...
    // Create root
    int rootIndex =
        AddItemToTree(m_hGroups, "root", -1, Color(128, 128, 128, 255), -1,
    ...
    for (int i = 0; i < c; ++i) {
      ...
      if (!group->HasAttribute("treeColor")) {
        if (!guard) {
          guard =
              new CAppUndoScopeGuard(NOTIFY_SETDIRTYFLAG, "Set Default Colors");
        }
        group->SetValue<Color>("treeColor", defaultColor);
      }
      int groupIndex = AddItemToTree(m_hGroups, group->GetName(), rootIndex,
                                     group->GetValue<Color>("treeColor"), i,
                                     (int)group->GetHandle());

      const CDmrStringArray array(group, "selectedControls");
      if (array.IsValid()) {
        for (int j = 0; j < array.Count(); ++j) {
          AddItemToTree(m_hGroups, array[j], groupIndex,
                        Color(200, 200, 200, 255), -1,
                        (int)DMELEMENT_HANDLE_INVALID);
        }
      }
  ...
  if (changed) {
    for (int i = 0; i < m_hSelectableIndices.Count(); ++i) {
      ...
      m_hGroups->AddSelectedItem(
          m_hSelectableIndices[i],
...
//-----------------------------------------------------------------------------
void CBaseAnimSetControlGroupPanel::ExpandOpenItems(OpenItemTree_t &tree,
                                                    int nOpenTreeIndex,
                                                    int nItemIndex,
                                                    bool makeVisible) {
...
void CBaseAnimSetControlGroupPanel::FillInDataForItem(TreeItem_t &item,
                                                      int nItemIndex) {
...
//-----------------------------------------------------------------------------
void CBaseAnimSetControlGroupPanel::BuildOpenItemList(OpenItemTree_t &tree,
                                                      int nParent,
                                                      int nItemIndex) {
  KeyValues *data = m_hGroups->GetItemData(nItemIndex);
  if (!data)
...
//-----------------------------------------------------------------------------
int CBaseAnimSetControlGroupPanel::FindTreeItem(int nParentIndex,
