...
public:
  ...
  virtual void OnLabelChanged(int itemIndex, const char *oldString,
                              const char *newString);

  virtual bool IsItemDroppable(int m_ItemIndex,
...
// Constructor
//-----------------------------------------------------------------------------
CElementTree::CElementTree(CElementPropertiesTreeInternal *parent,
...
// Destructor
//-----------------------------------------------------------------------------
CElementTree::~CElementTree() { delete m_pEditMenu; }

ScrollBar *CElementTree::GetScrollBar() { return m_pVertSB; }

bool CElementTree::IsItemDroppable(int itemIndex,
                                   CUtlVector<KeyValues *> &msglist) {
  return m_pParent->IsItemDroppable(itemIndex, msglist);
}

bool CElementTree::GetItemDropContextMenu(int itemIndex, Menu *menu,
                                          CUtlVector<KeyValues *> &msglist) {
  return m_pParent->GetItemDropContextMenu(itemIndex, menu, msglist);
}

void CElementTree::OnItemDropped(int itemIndex,
                                 CUtlVector<KeyValues *> &msglist) {
  m_pParent->OnItemDropped(itemIndex, msglist);
}

HCursor CElementTree::GetItemDropCursor(int itemIndex,
                                        CUtlVector<KeyValues *> &msglist) {
  return m_pParent->GetItemDropCursor(itemIndex, msglist);
}

void CElementTree::OnLabelChanged(int itemIndex, const char *oldString,
                                  const char *newString) {
...
//
//-----------------------------------------------------------------------------
CElementTreeViewListControl::CElementTreeViewListControl(Panel *pParent,
                                                         const char *pName)
...
int CElementTreeViewListControl::AddItem(
    KeyValues *data, bool allowLabelEditing, int parentItemIndex,
...
void CElementTreeViewListControl::ApplySchemeSettings(IScheme *pScheme) {
  ...
  GetTree()->SetFont(
      pScheme->GetFont("DmePropertyVerySmall", IsProportional()));
}
...
bool CElementTreeViewListControl::IsDrawingGrid() { return m_bDrawGrid; }
...
void CElementTreeViewListControl::PerformLayout() {
  ...
  ScrollBar *sb = ((CElementTree *)GetTree())->GetScrollBar();
  if (sb && sb->GetParent()) {
    sb->SetBounds(sb->GetParent()->GetWide() - sb->GetWide(), 0, sb->GetWide(),
                  sb->GetParent()->GetTall());
  }
...
void CElementTreeViewListControl::SetFont(HFont font) {
  ...
  for (int i = m_Panels.FirstInorder(); i != m_Panels.InvalidIndex();
    ...
    for (int j = 0; j < c; ++j) {
      ...
      CBaseAttributePanel *attrPanel =
          dynamic_cast<CBaseAttributePanel *>(panel);
      if (!attrPanel) {
        continue;
      }
...
int CElementTreeViewListControl::GetFontSize() { return m_iFontSize; }
...
public:
  CHistoryMenuButton(Panel *parent, const char *panelName, const char *text,
...
CHistoryMenuButton::CHistoryMenuButton(Panel *parent, const char *panelName,
                                       const char *text,
                                       CElementPropertiesTreeInternal *tree,
                                       int whichMenu)
    : BaseClass(parent, panelName, text), m_pPropertiesTreeInternal(tree),
      m_nWhichMenu(whichMenu) {
...
public:
  CSearchComboBox(CElementPropertiesTreeInternal *tree, vgui::Panel *parent,
                  const char *panelName, int numLines, bool allowEdit);
...
CSearchComboBox::CSearchComboBox(CElementPropertiesTreeInternal *tree,
...
void CSearchComboBox::OnShowMenu(Menu *menu) {
  ...
  Assert(m_pTree);
  if (m_pTree) {
    m_pTree->PopulateHistoryMenu(
        CElementPropertiesTreeInternal::DME_PROPERTIESTREE_MENU_SEARCHHSITORY,
        menu);
  }
}
...
public:
  CPropertiesTreeToolbar(vgui::Panel *parent, const char *panelName,
                         CElementPropertiesTreeInternal *tree);
...
CPropertiesTreeToolbar::CPropertiesTreeToolbar(
    vgui::Panel *parent, const char *panelName,
    CElementPropertiesTreeInternal *tree)
  ...
  m_pBack = new CHistoryMenuButton(
      this, "Nav_Back", "#Dme_NavBack", tree,
  ...
  m_pFwd = new CHistoryMenuButton(
      this, "Nav_Forward", "#Dme_NavForward", tree,
...
void CPropertiesTreeToolbar::UpdateButtonState() {
  m_pBack->SetEnabled(
      m_pTree->GetHistoryMenuItemCount(
          CElementPropertiesTreeInternal::DME_PROPERTIESTREE_MENU_BACKWARD) > 0
          ? true
          : false);
  m_pFwd->SetEnabled(
      m_pTree->GetHistoryMenuItemCount(
          CElementPropertiesTreeInternal::DME_PROPERTIESTREE_MENU_FORWARD) > 0
          ? true
          : false);

  // m_pShowSearchResults->SetEnabled( m_pTree->GetHistoryMenuItemCount(
  // CElementPropertiesTreeInternal::DME_PROPERTIESTREE_MENU_SEARCHHSITORY ) > 0
  // ? true : false );
}
...
void CPropertiesTreeToolbar::OnKeyCodeTyped(KeyCode code) {
  ...
  case KEY_F3: {
    ...
    if (parent) {
      KeyValues *msg =
          new KeyValues("OnNavigateSearchAgain", "direction", shift ? -1 : 1);
      PostMessage(parent, msg);
    }
  } break;
  default:
...
CElementPropertiesTreeInternal::CElementPropertiesTreeInternal(
    vgui::Panel *parent, IDmNotify *pNotify, CDmElement *pObject,
    bool autoApply /* = true */, CDmeEditorTypeDictionary *pDict /* = NULL */)
    : BaseClass(parent, "ElementPropertiesTree"), m_pNotify(pNotify),
      m_hTypeDictionary(pDict), m_bAutoApply(autoApply),
  ...
  {
    CDmElement *pResults =
        CreateElement<CDmElement>("Search Results", DMFILEID_INVALID);
    Assert(pResults);
...
//-----------------------------------------------------------------------------
void CElementPropertiesTreeInternal::OnElementChangedExternally(
    int valuesOnly) {
...
//-----------------------------------------------------------------------------
void CElementPropertiesTreeInternal::SetTypeDictionary(
...
void CElementPropertiesTreeInternal::ApplyChanges() {
  ...
  for (int i = 0; i < nCount; ++i) {
    attributewidgetfactorylist->ApplyChanges(
        m_AttributeWidgets[i].m_pValueWidget, this);
  }
}
...
//-----------------------------------------------------------------------------
void CElementPropertiesTreeInternal::Refresh(
    RefreshType_t rebuild /* = false */,
  ...
  int nCount = m_AttributeWidgets.Count();
  for (int i = 0; i < nCount; ++i) {
    attributewidgetfactorylist->Refresh(m_AttributeWidgets[i].m_pValueWidget,
                                        this);
  }
}
...
void CElementPropertiesTreeInternal::OnRename() {
  ...
  m_pTree->GetTree()->StartEditingLabel(
      m_pTree->GetTree()->GetFirstSelectedItem());
}
...
void CElementPropertiesTreeInternal::GetPathToItem(CUtlVector<TreeItem_t> &path,
                                                   int itemIndex) {
  for (int idx = itemIndex; idx != m_pTree->GetTree()->GetRootItemIndex();
    ...
    TreeItem_t treeitem;
    treeitem.m_pElement =
        GetElementKeyValue<CDmElement>(itemData, "ownerelement");
    treeitem.m_pAttributeName = itemData->GetString("attributeName", "");
    treeitem.m_pArrayElement =
        isArrayElement ? GetElementKeyValue<CDmElement>(itemData, "dmeelement")
                       : NULL;
    path.AddToTail(treeitem);
  }
}

int CElementPropertiesTreeInternal::OpenPath(
    const CUtlVector<TreeItem_t> &path) {
  bool bFound = false;
  ...
  for (int i = 0; i < nPathItems; ++i) {
    ...
    for (int i = 0; i < nChildren; ++i) {
      ...
      CDmElement *pOwnerElement =
          GetElementKeyValue<CDmElement>(childData, "ownerelement");
      ...
      if (isArrayElement) {
        ...
        if (nArrayIndex >= 0 &&
            array[nArrayIndex] == childTreeItem.m_pArrayElement) {
          bFound = true;
...
void CElementPropertiesTreeInternal::OnPaste_(bool reference) {
  ...
  bool isElementAttribute =
      attType == AT_ELEMENT || attType == AT_ELEMENT_ARRAY;
  ...
  if (isArrayElement || attType == AT_ELEMENT_ARRAY) {
    CElementTreeUndoScopeGuard guard(NOTIFY_SETDIRTYFLAG, m_pNotify,
                                     reference ? "Paste Reference" : "Paste");

    CDmrElementArray<> array(pAttribute);
    int nArrayIndex =
        isArrayElement ? itemData->GetInt("arrayIndex") + 1 : array.Count();
    DropItemsIntoArray(array, msglist, list, nArrayIndex,
                       reference ? DO_LINK : DO_COPY);
  }
  ...
  if (isArrayElement) {
    itemIndex = m_pTree->GetTree()->GetItemParent(
        itemIndex); // if we're an array element, start with the array itself
  }
...
void CElementPropertiesTreeInternal::OnDeleteSelected() {
  ...
  bool bChangeOccurred = false;
  {
    ...
    for (int si = 0; si < nSelected; ++si) {
      ...
      CDmAttribute *pAttr =
          pOwner->GetAttribute(item->GetString("attributeName"));
      if (pAttr == NULL)
        continue;
...
void CElementPropertiesTreeInternal::OnClear() {
  ...
  CElementTreeNotifyScopeGuard notify("CElementPropertiesTreeInternal::OnClear",
                                      NOTIFY_SETDIRTYFLAG, m_pNotify);
  ...
  for (int i = 0; i < c; ++i) {
    ...
    if (pOwner && pAttributeName[0]) {
      ...
      DmAttributeType_t attType =
          pAttribute ? pAttribute->GetType() : AT_UNKNOWN;
      switch (attType) {
      default:
        break;
...
// For each owner/attribute have an entry and them for each arrayIndex into any
// array type, need to sort by arrayIndex so we can remove them in reverse order
...
struct OwnerAttribute_t {
  ...
  OwnerAttribute_t(const OwnerAttribute_t &src)
      : sortedData(0, 0, ArrayIndexLessFunc) {
    pOwner = src.pOwner;
    symAttribute = src.symAttribute;
    for (int i = src.sortedData.FirstInorder();
         i != src.sortedData.InvalidIndex();
         i = src.sortedData.NextInorder(i)) {
      sortedData.Insert(src.sortedData[i]);
    }
  }

  static bool LessFunc(const OwnerAttribute_t &lhs,
                       const OwnerAttribute_t &rhs) {
...
bool CElementPropertiesTreeInternal::OnRemoveFromData(KeyValues *item) {
  ...
  if (arrayItem && IsArrayType(attType)) {
    ...
    CElementTreeUndoScopeGuard guard(NOTIFY_SETDIRTYFLAG,
  ...
  if (attType == AT_ELEMENT) {
    if (pOwner->GetValue<DmElementHandle_t>(pAttributeName) !=
...
bool CElementPropertiesTreeInternal::OnRemoveFromData(
    CUtlVector<KeyValues *> &list) {
  ...
  for (i = sorted.m_Sorted.FirstInorder(); i != sorted.m_Sorted.InvalidIndex();
       i = sorted.m_Sorted.NextInorder(i)) {
    OwnerAttribute_t &entry = sorted.m_Sorted[i];

    // Walk it backward by array index...
    for (int j = entry.sortedData.LastInorder();
         j != entry.sortedData.InvalidIndex();
...
void CElementPropertiesTreeInternal::OnRemove() {
  CElementTreeNotifyScopeGuard notify(
      "CElementPropertiesTreeInternal::OnRemove", NOTIFY_SETDIRTYFLAG,
      m_pNotify);
...
void CElementPropertiesTreeInternal::JumpToHistoryItem() {
  if (m_nCurrentHistoryPosition < 0 ||
      m_nCurrentHistoryPosition >= m_hHistory.Count()) {
    m_nCurrentHistoryPosition = 0;
  }
...
bool CElementPropertiesTreeInternal::BuildExpansionListToFindElement_R(
    CUtlRBTree<CDmElement *, int> &visited, int depth, SearchResult_t &sr,
    CDmElement *owner, CDmElement *element, const char *attributeName,
  ...
  for (CDmAttribute *attribute = element->FirstAttribute(); attribute;
    ...
    if (attribute->GetType() == AT_ELEMENT) {
      if (!BuildExpansionListToFindElement_R(
              visited, depth + 1, sr, element,
...
static ConVar
    dme_properties_maxsearchresults("dme_properties_maxsearchresults", "50", 0,
                                    "Max number of search results to track.");

void CElementPropertiesTreeInternal::FindMatchingElements_R(
    CUtlRBTree<CDmElement *, int> &visited, const char *searchstr,
    CDmElement *element, CUtlVector<SearchResult_t> &list) {
  ...
  for (CDmAttribute *attribute = element->FirstAttribute(); attribute;
    ...
    if (attribute->GetType() == AT_ELEMENT) {
      FindMatchingElements_R(visited, searchstr,
                             attribute->GetValueElement<CDmElement>(), list);
    } else if (attribute->GetType() == AT_ELEMENT_ARRAY) {
      // Walk child objects
      const CDmrElementArray<CDmElement> elementArray(attribute);
      int c = elementArray.Count();
...
void CElementPropertiesTreeInternal::NavigateToSearchResult() {
  ...
  BuildExpansionListToFindElement_R(
      visited, 0, m_SearchResults[m_nCurrentSearchResult], m_hObject.Get(),
...
void CElementPropertiesTreeInternal::OnNavSearch(const char *text) {
  ...
  if (changed) {
    ...
    FindMatchingElements_R(visited, m_szSearchStr, m_hObject.Get(),
                           m_SearchResults);
    ...
    if (m_SearchResultsRoot.Get()) {
      ...
      char sz[512];
      Q_snprintf(sz, sizeof(sz), "Search Results [%d] for '%s'", c,
                 m_szSearchStr);
  ...
  m_nCurrentSearchResult =
      clamp(m_nCurrentSearchResult, 0, m_SearchResults.Count() - 1);
...
void CElementPropertiesTreeInternal::PopulateHistoryMenu(int whichMenu,
                                                         Menu *menu) {
  ValidateHistory();
  ...
  case DME_PROPERTIESTREE_MENU_BACKWARD: {
    for (int i = m_nCurrentHistoryPosition + 1; i < c; ++i) {
      ...
      char sz[256];
      Q_snprintf(sz, sizeof(sz), "%s < %s >", element->GetName(),
                 element->GetTypeString());
      menu->AddMenuItem("backitem", sz,
                        new KeyValues("OnNavigateBack", "item", i), this);
    }
  } break;
  case DME_PROPERTIESTREE_MENU_FORWARD: {
    for (int i = 0; i < m_nCurrentHistoryPosition; ++i) {
      ...
      Q_snprintf(sz, sizeof(sz), "%s < %s >", element->GetName(),
                 element->GetTypeString());
      menu->AddMenuItem("fwditem", sz,
                        new KeyValues("OnNavigateForward", "item", i), this);
    }
  } break;
  case DME_PROPERTIESTREE_MENU_SEARCHHSITORY: {
    ...
    for (int i = 0; i < c; ++i) {
      ...
      menu->AddMenuItem("search", str.Get(),
                        new KeyValues("OnNavSearch", "text", str.Get()), this);
    }
  } break;
  }
}
...
void CElementPropertiesTreeInternal::SpewHistory() {
  ...
  for (i = 0; i < c; ++i) {
    ...
    Msg("%s:  [%02d] %s <%s>\n",
        ((i < m_nCurrentHistoryPosition)
             ? "Fwd"
             : (i == m_nCurrentHistoryPosition ? "Current" : "Backward")),
        i, element->GetName(), element->GetTypeString());
  }
}

void CElementPropertiesTreeInternal::AddAttribute(const char *pAttributeName,
                                                  KeyValues *pContext) {
  ...
  DmAttributeType_t attributeType =
  ...
  if (pElement->HasAttribute(pAttributeName)) {
    Warning(
        "Can't add attribute '%s', attribute with that name already exists\n",
        pAttributeName);
    return;
  }
  ...
  CDmAttribute *pAttribute =
      pElement->AddAttribute(pAttributeName, attributeType);
  if (pAttribute) {
...
void CElementPropertiesTreeInternal::SetElementAttribute(
    const char *pElementName, KeyValues *pContext) {
...

  {
    CElementTreeUndoScopeGuard guard(NOTIFY_SETDIRTYFLAG, m_pNotify,
                                     "Set Element");
    DmElementHandle_t newElement = g_pDataModel->CreateElement(
        pElementType, pElementName, pElement->GetFileId());
...
//-----------------------------------------------------------------------------
bool CElementPropertiesTreeInternal::ShowSetElementAttributeDialog(
    CDmElement *pOwner, const char *pAttributeName, int nArrayIndex,
    const char *pElementType) {
  if (!pOwner || !pAttributeName || !pAttributeName[0] || !pElementType ||
      !pElementType[0])
    return false;
  ...
  KeyValues *kv =
      new KeyValues("OnSetElement", "attributeName", pAttributeName);
  SetElementKeyValue(kv, "element", pOwner);
  kv->SetInt("index", nArrayIndex);
  kv->SetString("elementType", pElementType);

  InputDialog *pSetAttributeDialog =
      new InputDialog(this, "Set Element", "Element Name:", elemName);
...
bool CElementPropertiesTreeInternal::ShowAddAttributeDialog(
    CDmElement *pElement, const char *pAttributeType) {
  ...
  KeyValues *kv =
      new KeyValues("OnAddAttribute", "attributeType", pAttributeType);
  SetElementKeyValue(kv, "element", pElement);

  InputDialog *pAddDialog =
      new InputDialog(this, "Add Attribute", "Attribute Name:", attrName);
...
void CElementPropertiesTreeInternal::OnCommand(const char *cmd) {
  ...
  for (int i = 0; i < c; ++i) {
    ...
    if (pElementType) {
      ...
      bool arrayItem = !item->IsEmpty("arrayIndex");
      int arrayIndex = item->GetInt("arrayIndex");
      if (ShowSetElementAttributeDialog(pOwner, pAttributeName,
                                        arrayItem ? arrayIndex : -1,
                                        pElementType))
        return;
      continue;
    }
  ...
  bool bRefreshRequired = false;
  {
    ...
    for (int i = 0; i < c; ++i) {
      ...
      if (attType == AT_ELEMENT_ARRAY) {
        ...
        if (!pElementTypeName) {
          Warning("CElementPropertiesTreeInternal::OnAddItem: Unknown Element "
                  "Type %s\n",
                  pElementType);
          continue;
        }
        ...
        if (elementNum++ == 0) {
          Q_snprintf(elementName, sizeof(elementName), "new%s",
                     pElementTypeName);
        } else {
          Q_snprintf(elementName, sizeof(elementName), "new%s%i",
                     pElementTypeName, elementNum);
        }

        DmElementHandle_t newElement = g_pDataModel->CreateElement(
            pElementType, elementName, pOwner->GetFileId());
        if (newElement != DMELEMENT_HANDLE_INVALID) {
          array.AddToTail(newElement);
          bRefreshRequired = true;
        }
...
void CElementPropertiesTreeInternal::OnSetShared(KeyValues *params) {
  ...
  CElementTreeUndoScopeGuard guard(NOTIFY_SETDIRTYFLAG, m_pNotify,
                                   bShared ? "Mark Shared" : "Mark Not Shared");
...
void CElementPropertiesTreeInternal::OnChangeFile(KeyValues *params) {
  ...
  CElementTreeNotifyScopeGuard notify(
      "CElementPropertiesTreeInternal::OnChangeFile", NOTIFY_SETDIRTYFLAG,
      m_pNotify);
...
void CElementPropertiesTreeInternal::OnShowFileDialog(KeyValues *params) {
  ...
  FileOpenDialog *pDialog =
      new FileOpenDialog(this, pTitle, bOpenOnly, pContext->MakeCopy());
...
void CElementPropertiesTreeInternal::OnImportElement(const char *pFullPath,
                                                     KeyValues *pContext) {
  CDmElement *pRoot = NULL;
  DmFileId_t tempFileid;
  {
    CDisableUndoScopeGuard guard;
    tempFileid = g_pDataModel->RestoreFromFile(pFullPath, NULL, NULL, &pRoot,
                                               CR_FORCE_COPY);
  }
  if (!pRoot)
    return;
...
void CElementPropertiesTreeInternal::OnExportElement(const char *pFullPath,
                                                     KeyValues *pContext) {
  CDmElement *pRoot = NULL;
  ...
  if (nSelected <= 1) {
    ...
    // HACK - this is just a temporary hack - we should really force
    // serialization to traverse past fileid changes in this case
  ...
  // if this control is ever moved to vgui_controls, change the default format
  // to "dmx", the generic dmx format
...
//-----------------------------------------------------------------------------
vgui::Panel *CElementPropertiesTreeInternal::CreateAttributeDataWidget(
    CDmElement *pElement, const char *pWidgetName, CDmElement *obj,
    CDmAttribute *pAttribute, int nArrayIndex) {
  AttributeWidgetInfo_t info;
  SetupWidgetInfo(&info, pElement, pAttribute, nArrayIndex);
  IAttributeWidgetFactory *pFactory =
      attributewidgetfactorylist->GetWidgetFactory(pWidgetName);
  if (!pFactory)
    return NULL;
...
void CElementPropertiesTreeInternal::UpdateTree() {
  ...
  if (m_hObject.Get()) {
    ...
    kv->SetInt("Expand", 1);
    kv->SetInt("dmeelement", m_hObject.Get() ? m_hObject.Get()->GetHandle()
                                             : DMELEMENT_HANDLE_INVALID);
    kv->SetInt("ownerelement", m_hObject.Get() ? m_hObject.Get()->GetHandle()
                                               : DMELEMENT_HANDLE_INVALID);
    kv->SetString("attributeName", "name");
    kv->SetInt("root", m_hObject.Get() ? m_hObject.Get()->GetHandle()
                                       : DMELEMENT_HANDLE_INVALID);
    kv->SetInt("editablelabel", editableLabel ? 1 : 0);

    CDmElement *pElement = m_hObject.Get();
    vgui::Panel *widget =
        CreateAttributeDataWidget(pElement, "element", pElement, NULL);
    ...
    m_pTree->GetTree()->SetItemSelectionUnfocusedBgColor(
        rootIndex, Color(255, 153, 35, 255));
...
void CElementPropertiesTreeInternal::GenerateDragDataForItem(int itemIndex,
                                                             KeyValues *msg) {
...
struct DataModelFilenameArray {
  int Count() const { return g_pDataModel->NumFileIds(); }
  const char *operator[](int i) const {
    return g_pDataModel->GetFileName(g_pDataModel->GetFileId(i));
  }
};

void CElementPropertiesTreeInternal::GenerateContextMenu(int itemIndex, int x,
                                                         int y) {
  KeyValues *data = m_pTree->GetItemData(itemIndex);
  if (!data) {
    Assert(data);
  ...
  bool bIsArrayAttribute =
      !bIsArrayItem && (attributeType >= AT_FIRST_ARRAY_TYPE);
  ...
  bool bIsElementArrayAttribute =
      bIsArrayAttribute && (attributeType == AT_ELEMENT_ARRAY);
  bool bIsElementArrayItem =
      bIsArrayItem && (attributeType == AT_ELEMENT_ARRAY);
  ...
  if (bIsElementArrayItem) {
    ...
    m_hContextMenu->AddCheckableMenuItem("* Element Attribute Operations *",
                                         this);
  ...
  // cut / copy / paste
  m_hContextMenu->AddMenuItem("#DmeElementPropertiesCut",
                              new KeyValues("OnCut"), this);
  m_hContextMenu->AddMenuItem("#DmeElementPropertiesCopy",
                              new KeyValues("OnCopy"), this);
  id = m_hContextMenu->AddMenuItem("#DmeElementPropertiesPaste",
                                   new KeyValues("OnPaste"), this);
  m_hContextMenu->SetItemEnabled(id,
                                 vgui::system()->GetClipboardTextCount() > 0);

  // paste special
  // Would have to get the clipboard contents and examine to enable a cascading
  // "Paste Special" menu here
  Menu *pasteSpecial = new Menu(this, "Paste Special");
  pasteSpecial->SetFont(m_pTree->GetTree()->GetFont());
  id = m_hContextMenu->AddCascadingMenuItem("#DmeElementPropertiesPasteSpecial",
                                            this, pasteSpecial);
  m_hContextMenu->SetItemEnabled(id,
                                 vgui::system()->GetClipboardTextCount() > 0);
  ...
  if (bIsArrayAttribute && !bIsArrayAttributeEmpty) {
    removeItemID = m_hContextMenu->AddMenuItem("#DmeElementPropertiesClear",
                                               new KeyValues("OnRemove"), this);
  } else {
    removeItemID = m_hContextMenu->AddMenuItem("#DmeElementPropertiesRemove",
                                               new KeyValues("OnRemove"), this);
  }
  ...
  if (data->GetInt("editablelabel")) {
    if (bIsArrayItem) {
      m_hContextMenu->AddMenuItem("Rename Element...",
                                  new KeyValues("OnRename"), this);
    } else {
      m_hContextMenu->AddMenuItem("Rename Attribute...",
                                  new KeyValues("OnRename"), this);
    }
  }
  ...
  if (bIsElementArrayAttribute && !bIsArrayAttributeEmpty) {
    m_hContextMenu->AddMenuItem("#DmeElementPropertiesSortByName",
                                new KeyValues("OnSortByName"), this);
  }
  ...
  if (bIsArrayAttribute && !bIsElementArrayAttribute) {
    m_hContextMenu->AddMenuItem("#DmeElementPropertiesAddItem",
                                new KeyValues("OnAddItem"), this);
  }

  // Add Attribute
  if ((bIsElementAttribute && !bIsElementAttributeNull) ||
      m_pTree->GetTree()->GetRootItemIndex() == itemIndex ||
      bIsElementArrayItem) {
    Menu *addMenu = new Menu(this, "AddAttribute");
    addMenu->SetFont(m_pTree->GetTree()->GetFont());
    m_hContextMenu->AddCascadingMenuItem("#DmeElementPropertiesAddAttribute",
                                         this, addMenu);
    {
      for (int i = AT_FIRST_VALUE_TYPE; i < AT_TYPE_COUNT; ++i) {
        const char *typeName =
            g_pDataModel->GetAttributeNameForType((DmAttributeType_t)i);
        if (typeName && typeName[0]) {
          char add_attribute[256];
          Q_snprintf(add_attribute, sizeof(add_attribute), "attribute_%s",
                     typeName);
          id = addMenu->AddMenuItem(
              typeName, new KeyValues("Command", "command", add_attribute),
              this);
  ...
  if (bIsElementAttribute || bIsElementArrayAttribute || bIsElementArrayItem) {
    ...
    if (bIsElementArrayAttribute) {
      m_hContextMenu->AddCascadingMenuItem("#DmeElementPropertiesAddElement",
                                           this, addMenu);
    } else if (bIsElementAttributeNull) {
      m_hContextMenu->AddCascadingMenuItem("#DmeElementPropertiesNewElement",
                                           this, addMenu);
    } else if (bIsElementAttribute || bIsElementArrayItem) {
      m_hContextMenu->AddCascadingMenuItem(
          "#DmeElementPropertiesReplaceElement", this, addMenu);
    }

    // Populate from factories
    for (int i = g_pDataModel->GetFirstFactory();
         g_pDataModel->IsValidFactory(i); i = g_pDataModel->GetNextFactory(i)) {
      ...
      id = addMenu->AddMenuItem(
          elementType, new KeyValues("Command", "command", add_element), this);
      addMenu->GetMenuItem(id)->SetContentAlignment(Label::a_center);
    }
  }
  ...
  if ((bIsElementAttribute && !bIsElementAttributeNull) ||
    ...
    if (nShared < nElements) {
      m_hContextMenu->AddMenuItem(
          "Mark Shared", new KeyValues("OnSetShared", "shared", 1), this);
    }
    if (nShared > 0) {
      m_hContextMenu->AddMenuItem(
          "Mark Not Shared", new KeyValues("OnSetShared", "shared", 0), this);
    }
  }
  ...
  if ((bIsElementAttribute && !bIsElementAttributeNull) ||
      ...
      bIsElementArrayItem) {
    KeyValues *pContext =
        new KeyValues("context", "command", "OnExportElement");
    pContext->SetInt("element", (int)pElement->GetHandle());

    KeyValues *kv =
        new KeyValues("OnShowFileDialog", "title", "Export Element");
  ...
  if (pElement) {
    ...
    m_hContextMenu->AddCascadingMenuItem(
        "#DmeElementPropertiesChangeFileAssociation", this, menu);
    ...
    for (int i = 0; i < nFiles; ++i) {
      ...
      menu->AddMenuItem(
          pText, new KeyValues("OnChangeFile", "filename", pFileName), this);
    }

    char filename[MAX_PATH];
    V_GenerateUniqueName(filename, sizeof(filename), "unnamed",
                         DataModelFilenameArray());

    menu->AddMenuItem("<new file>",
                      new KeyValues("OnChangeFile", "filename", filename),
                      this);
  }
...
void CElementPropertiesTreeInternal::OnLabelChanged(int itemIndex,
  ...
  if (nNotifyFlags) {
    Refresh((nNotifyFlags == NOTIFY_CHANGE_ATTRIBUTE_VALUE)
                ? REFRESH_VALUES_ONLY
                : REFRESH_TREE_VIEW);
  }
}

bool CElementPropertiesTreeInternal::IsItemDroppable(
...
HCursor CElementPropertiesTreeInternal::GetItemDropCursor(
    int itemIndex, CUtlVector<KeyValues *> &msglist) {
...
struct ArrayItem_t {
  ArrayItem_t(CDmAttribute *pAttr = NULL, int nIndex = -1)
      : m_pAttr(pAttr), m_nIndex(nIndex) {}
...
void CElementPropertiesTreeInternal::DropItemsIntoArray(
    CDmrElementArray<> &array, CUtlVector<KeyValues *> &msglist,
    CUtlVector<CDmElement *> &list, int nArrayIndex, DropOperation_t op) {
  ...
  if (op == DO_COPY) {
    ...
    CUtlRBTree<ArrayItem_t> arrayItemSorter(0, msglist.Count(),
                                            ArrayItem_t::LessFunc);
    ...
    for (int i = 0; i < nMsgs; ++i) {
      ...
      bool isArrayElement = !itemData->IsEmpty("arrayIndex");
      CDmAttribute *pAttribute = pOwner->GetAttribute(
          pAttributeName, isArrayElement ? AT_ELEMENT_ARRAY : AT_ELEMENT);
      if (!pAttribute)
        continue;
    ...
    // walk through all array items, back to front, so that removing won't mess
    // up the indices
    for (int i = arrayItemSorter.LastInorder();
         i != arrayItemSorter.InvalidIndex();
      ...
      if (arrayItem.m_pAttr == array.GetAttribute() &&
          arrayItem.m_nIndex < nArrayIndex) {
  ...
  for (int i = 0; i < nElements; ++i) {
    ...
    if (array[base + i] != list[i]) {
      // if couldn't be dropped into array, skip it and merge remaining items
      // down by one
...
CElementPropertiesTreeInternal::DropOperation_t
CElementPropertiesTreeInternal::GetDropOperation(
    int itemIndex, CUtlVector<KeyValues *> &msglist) {
  bool bCtrlDown =
      input()->IsKeyDown(KEY_LCONTROL) || input()->IsKeyDown(KEY_RCONTROL);
  ...
  if (!ElementTree_IsArrayItem(itemData) &&
      ElementTree_GetAttributeType(itemData) != AT_ELEMENT_ARRAY)
  ...
  for (int i = 0; i < nMsgs; ++i) {
    ...
    if (!ElementTree_IsArrayItem(pMsg) ||
        ElementTree_GetAttributeType(pMsg) != AT_ELEMENT_ARRAY)
      return DO_LINK; // dragging from a non-array attribute

    op = DO_MOVE; // basically, op will only stay DO_MOVE if *every* item is a
                  // non-element or is an array (or array item)
  }
...
void CElementPropertiesTreeInternal::OnItemDropped(
    int itemIndex, CUtlVector<KeyValues *> &msglist) {
  if (!msglist.Count())
    return;
  ...
  if (isArrayElement) {
    ...
    bool bAfter =
        Q_stricmp(cmd, "after") == 0 || Q_stricmp(cmd, "default") == 0;
  ...
  // Mouse is over an element attribute or element array attribute
    ...
    if (attType == AT_ELEMENT) {
      ...
      bool bTail =
          !cmd[0] || !Q_stricmp(cmd, "default") || !Q_stricmp(cmd, "tail");
    ...
    if (isArrayElement || attType == AT_ELEMENT_ARRAY) {
      ...
      for (int i = 0; i < nElements; ++i) {
        ...
        for (int ci = 0; ci < nChildren; ++ci) {
          ...
          if (list[i] ==
              GetElementKeyValue<CDmElement>(pChildData, "dmeelement")) {
...
bool CElementPropertiesTreeInternal::GetItemDropContextMenu(
    int itemIndex, Menu *menu, CUtlVector<KeyValues *> &msglist) {
  KeyValues *itemData = m_pTree->GetItemData(itemIndex);
...
// Gets tree view text
//-----------------------------------------------------------------------------
void CElementPropertiesTreeInternal::GetTreeViewText(CDmElement *obj,
                                                     CDmAttribute *pAttribute,
                                                     int nArrayIndex,
  ...
  if (nArrayIndex < 0) {
    ...
    editableText = !pAttribute->IsFlagSet(FATTRIB_EXTERNAL) &&
                   !pAttribute->IsFlagSet(FATTRIB_READONLY);
  } else {
    // array types
    ...
    if (type == AT_ELEMENT_ARRAY) {
      ...
      if (pEntryElement) {
        Q_snprintf(pBuffer, nMaxLen, "%s",
                   pEntryElement->GetValueString("name"));
        editableText = true;
      }
    } else {
...
//-----------------------------------------------------------------------------
int CElementPropertiesTreeInternal::FindTreeItem(int nParentIndex,
                                                 const TreeItem_t &info) {
  // Look for a match
  ...
  for (int i = nCount; --i >= 0;) {
    ...
    if ((pElement == info.m_pElement) &&
        (pArrayElement == info.m_pArrayElement) &&
...
void CElementPropertiesTreeInternal::SpewOpenItems(int depth,
                                                   OpenItemTree_t &tree,
                                                   int nOpenTreeIndex,
                                                   int nItemIndex) {
  ...
  if (nOpenTreeIndex != tree.InvalidIndex()) {
    ...
    if (info.m_nFlags & EP_EXPANDED) {
      Msg("[%d] Marking %s <%s> %s array(%s) [expanded %i]\n", depth,
          ...
          info.m_Item.m_pArrayElement ? info.m_Item.m_pArrayElement->GetName()
                                      : "NULL",
...
//-----------------------------------------------------------------------------
void CElementPropertiesTreeInternal::ExpandOpenItems(OpenItemTree_t &tree,
                                                     int nOpenTreeIndex,
                                                     int nItemIndex,
                                                     bool makeVisible) {
  int i = tree.FirstChild(nOpenTreeIndex);
...
void CElementPropertiesTreeInternal::FillInDataForItem(TreeItem_t &item,
                                                       int nItemIndex) {
...
//-----------------------------------------------------------------------------
void CElementPropertiesTreeInternal::BuildOpenItemList(
    OpenItemTree_t &tree, int nParent, int nItemIndex,
...
// Builds a list of open items
//-----------------------------------------------------------------------------
void CElementPropertiesTreeInternal::RefreshTreeView(
    bool preservePrevSelectedItem /*= false*/) {
  int nIndex = m_pTree->GetTree()->GetRootItemIndex();
  if (nIndex >= 0) {
    ...
    BuildOpenItemList(openItems, openItems.InvalidIndex(), nIndex,
                      preservePrevSelectedItem);
...
//-----------------------------------------------------------------------------
void CElementPropertiesTreeInternal::SetTreeItemColor(int nItemID,
                                                      CDmElement *pEntryElement,
                                                      bool bIsElementArrayItem,
                                                      bool bEditableLabel) {
  ...
  if (pEntryElement != NULL) {
    if ((pEntryElement->HasAttribute("visible") &&
         !pEntryElement->GetValue<bool>("visible")) ||
        (pEntryElement->HasAttribute("mute") &&
         pEntryElement->GetValue<bool>("mute"))) {
      bIsDim = true;
    }
  }
  ...
  m_pTree->GetTree()->SetItemSelectionBgColor(
      nItemID, Color(255, 153, 35, bIsDim ? dimAlpha : 255));
  m_pTree->GetTree()->SetItemSelectionUnfocusedBgColor(
      nItemID, Color(255, 153, 35, bIsDim ? dimAlpha : 255));

  if (bIsElementArrayItem) {
    // element array items are green
    m_pTree->GetTree()->SetItemFgColor(
        nItemID, Color(66, 196, 66, bIsDim ? dimAlpha : 255));
  } else if (bEditableLabel) {
    // custom attributes are light yellow
    m_pTree->GetTree()->SetItemFgColor(
        nItemID, Color(190, 190, 105, bIsDim ? dimAlpha : 255));
  } else {
    // otherwise it's just light grey
    m_pTree->GetTree()->SetItemFgColor(
        nItemID, Color(160, 160, 160, bIsDim ? dimAlpha : 255));
  }
}
...
// Adds a single entry into the tree
//-----------------------------------------------------------------------------
void CElementPropertiesTreeInternal::CreateTreeEntry(
    int parentNodeIndex, CDmElement *obj, CDmAttribute *pAttribute,
  ...
  GetTreeViewText(obj, pAttribute, nArrayIndex, pText, sizeof(pText),
                  bEditableLabel);
  ...
  bool bIsDroppable =
      bIsElementArrayItem || bIsElementAttribute || bIsElementArrayAttribute;
  ...
  if (bIsDroppable) {
    ...
    kv->SetString(
        "droppableelementtype",
        "dmeelement"); // FIXME: Should be able to restrict to certain types!!!
  }
  ...
  int itemIndex =
      m_pTree->AddItem(kv, bEditableLabel, parentNodeIndex, columns);
  SetTreeItemColor(itemIndex, pEntryElement, bIsElementArrayItem,
                   bEditableLabel);
  kv->deleteThis();
}
...
void CElementPropertiesTreeInternal::SetupWidgetInfo(
    AttributeWidgetInfo_t *pInfo, CDmElement *obj, CDmAttribute *pAttribute,
    int nArrayIndex) {
  ...
  if (m_hTypeDictionary && pAttributeName) {
    if (nArrayIndex < 0) {
      pInfo->m_pEditorInfo =
          m_hTypeDictionary->GetAttributeInfo(obj, pAttributeName);
    } else {
      pInfo->m_pEditorInfo =
          m_hTypeDictionary->GetAttributeArrayInfo(obj, pAttributeName);
    }
  }
}
...
//-----------------------------------------------------------------------------
void CElementPropertiesTreeInternal::InsertSingleAttribute(
    int parentNodeIndex, CDmElement *obj, CDmAttribute *pAttribute,
    int nArrayIndex) {
  ...
  if (nArrayIndex >= 0) {
    pFactory = attributewidgetfactorylist->GetArrayWidgetFactory(
        obj, pAttribute, m_hTypeDictionary);
  } else {
    pFactory = attributewidgetfactorylist->GetWidgetFactory(obj, pAttribute,
                                                            m_hTypeDictionary);
  }
  if (!pFactory)
    return;
  ...
  CBaseAttributePanel *attrPanel =
      dynamic_cast<CBaseAttributePanel *>(attributeWidget.m_pValueWidget);
  if (attrPanel) {
    attrPanel->SetFont(m_pTree->GetFont(m_pTree->GetFontSize()));
  }

  // Now create the tree-view entry
  CreateTreeEntry(parentNodeIndex, obj, pAttribute, nArrayIndex,
                  attributeWidget);
...
//-----------------------------------------------------------------------------
void CElementPropertiesTreeInternal::InsertAttributes(int parentNodeIndex,
                                                      CDmElement *obj) {
  Assert(obj);

  // Build a list of attributes for sorting
  AttributeInfo_t *pInfo = (AttributeInfo_t *)_alloca(obj->AttributeCount() *
                                                      sizeof(AttributeInfo_t));
  int nCount = 0;
  for (CDmAttribute *pAttribute = obj->FirstAttribute(); pAttribute;
...
void CElementPropertiesTreeInternal::RemoveItem_R(int nItemIndex) {
  ...
  if (data) {
    ...
    search.m_pValueWidget =
        static_cast<vgui::Panel *>(data->GetPtr("widget", NULL));
...
//-----------------------------------------------------------------------------
void CElementPropertiesTreeInternal::InsertAttributeArrayMembers(
    int parentNodeIndex, CDmElement *obj, CDmAttribute *pAttribute) {
  Assert(obj);
...
void CElementPropertiesTreeInternal::OnKeyDelete() { RemoveSelected(false); }

void CElementPropertiesTreeInternal::OnKeyBackspace() { RemoveSelected(true); }
...
void CElementPropertiesTreeInternal::OnRefresh() {
  ...
  CElementTreeNotifyScopeGuard notify(
      "CElementPropertiesTreeInternal::OnRefresh",
...
//
//-----------------------------------------------------------------------------
CElementPropertiesTree::CElementPropertiesTree(vgui::Panel *parent,
                                               IDmNotify *pNotify,
                                               CDmElement *pObject,
                                               CDmeEditorTypeDictionary *pDict)
  ...
  m_pProperties =
      new CElementPropertiesTreeInternal(this, pNotify, pObject, false, pDict);
...
void CElementPropertiesTree::Init() { m_pProperties->Init(); }
...
void CElementPropertiesTree::Refresh(
    CElementPropertiesTreeInternal::RefreshType_t
        rebuild /* = REFRESH_REBUILD */,
    bool preservePrevSelectedItem /*= false*/) {
...
//
//-----------------------------------------------------------------------------
IMPLEMENT_DMEPANEL_FACTORY(CDmeElementPanel, DmElement, "DmeElementDefault",
                           "Dme Element Editor", true);
...
//-----------------------------------------------------------------------------
void CDmeElementPanel::NotifyDataChanged(const char *pReason, int nNotifySource,
                                         int nNotifyFlags) {
  if (nNotifyFlags &
      (NOTIFY_CHANGE_TOPOLOGICAL | NOTIFY_CHANGE_ATTRIBUTE_VALUE |
       NOTIFY_CHANGE_ATTRIBUTE_ARRAY_SIZE)) {
    KeyValues *pKeyValues =
        new KeyValues("DmeElementChanged", "notifyFlags", nNotifyFlags);
