...
static ConVar ifm_attributesliderbalance_sensitivity(
    "ifm_attributesliderbalance_sensitivity", "1.0", 0);

extern float ifm_fader_timescale;

enum {
  // Just some arbitrary number that we're not likely to see in another .lib or
  // the main app's code
  UNDO_CHAIN_MOUSEWHEEL_ATTRIBUTE_SLIDER = 9876,
};

bool CBaseAnimSetAttributeSliderPanel::ChannelToSliderLookup_t::Less(
    const ChannelToSliderLookup_t &lhs, const ChannelToSliderLookup_t &rhs) {
...
public:
  CPresetSideFilterSlider(CBaseAnimSetAttributeSliderPanel *pParent,
...
CPresetSideFilterSlider::CPresetSideFilterSlider(
    CBaseAnimSetAttributeSliderPanel *parent, const char *panelName)
...
//
//-----------------------------------------------------------------------------
CBaseAnimSetAttributeSliderPanel::CBaseAnimSetAttributeSliderPanel(
    vgui::Panel *parent, const char *className, CBaseAnimationSetEditor *editor)
    : BaseClass(parent, className), m_flEstimatedValue(0.0f),
      m_ChannelToSliderLookup(0, 0, ChannelToSliderLookup_t::Less),
      m_flRecomputePreviewTime(-1.0f), m_bRequestedNewPreview(false),
      m_flPrevTime(0.0f), m_nFaderChangeFlags(0) {
  m_hEditor = editor;

  m_pLeftRightBoth[0] =
      new Button(this, "AttributeSliderLeftOnly", "", this, "OnLeftOnly");
  m_pLeftRightBoth[1] =
      new Button(this, "AttributeSliderRightOnly", "", this, "OnRightOnly");
  ...
  m_Sliders->SetAutoResize(
      Panel::PIN_TOPLEFT, Panel::AUTORESIZE_DOWNANDRIGHT, 0,
...
void CBaseAnimSetAttributeSliderPanel::StampValueIntoLogs(
    CDmElement *control, AnimationControlType_t type, float flValue) {}
...
void CBaseAnimSetAttributeSliderPanel::PerformLayout() {
  ...
  m_pLeftRightBoth[1]->SetBounds(w - 15, (availH - btnSize) / 2, btnSize,
                                 btnSize);
  m_pPresetSideFilter->SetBounds(23 + btnSize, 4, w - 38 - 2 * btnSize,
                                 availH - 8);
}
...
//     figures out the crossfade amount of each preset slider with non-zero
//     influence
//  b) not holding control, then just see if we are previewing whichever preset
//  the mouse is over
// Input  :  -
//-----------------------------------------------------------------------------
void CBaseAnimSetAttributeSliderPanel::OnThink() {
  ...
  bool nameChanged =
      (fader.name && (m_PreviousPreviewFader.IsEmpty() ||
                      Q_stricmp(m_PreviousPreviewFader.Get(), fader.name)))
          ? true
          : false;
  bool beingDraggedChanged =
      fader.isbeingdragged != m_Previous.isbeingdragged ? true : false;
  bool ctrlKeyChanged =
      fader.holdingctrl != m_Previous.holdingctrl ? true : false;
  ...
  for (int i = 0; i < c; ++i) {
    ...
    if (m_CtrlKeyPreviewSlider.Get() == slider) {
      // The preset stuff shouldn't be active when we're holding ctrl over the
      // raw attribute sliders!!!
      ...
      if (!slider->IsTransform()) {
        ...
        dest.m_pValue[ANIM_CONTROL_BALANCE] =
            slider->GetValue(ANIM_CONTROL_BALANCE);
        dest.m_pValue[ANIM_CONTROL_MULTILEVEL] =
            slider->GetValue(ANIM_CONTROL_MULTILEVEL);
      } else {
      ...
      // If we aren't over any of the preset sliders, then we need to be able to
      // ramp down to the current value, too
    ...
    preview.m_pValue[ANIM_CONTROL_VALUE] =
        slider->IsTransform()
            ? 0.0f
            : slider->GetControlDefaultValue(ANIM_CONTROL_VALUE);
    ...
    BlendFlexValues(&preview, current, preview, 1.0f,
                    slider->IsControlActive(ANIM_CONTROL_BALANCE)
                        ? m_pPresetSideFilter->GetPos()
                        : 0.5f);

    bool simple = fader.isbeingdragged ||
                  (!fader.holdingctrl && !slider->IsRampingTowardPreview() &&
                   !ctrlKeyChanged);
    ...
    if (bFaderChanged || fader.isbeingdragged) {
      ...
      if (simple) {
        ...
        // For the "ramp down" case (just released ctrl key), we need the
        // original values instead of the preview valuees
        if (ctrlKeyChanged && !fader.holdingctrl &&
            slider->IsRampingTowardPreview()) {
          Assert(!fader.isbeingdragged);
          slider->RampDown();
        } else {
          // Apply the left-right balance to the target
...
void CBaseAnimSetAttributeSliderPanel::ChangeAnimationSet(
    ...
    if (m_SliderList.Count() == controlCount) {
      ...
      for (i = 0; i < controlCount; ++i) {
        ...
        if (m_SliderList[i]->IsControlActive(ANIM_CONTROL_BALANCE) != bStereo ||
            m_SliderList[i]->IsControlActive(ANIM_CONTROL_MULTILEVEL) !=
                bIsMulti) {
          rebuild = true;
          break;
        }
  ...
  for (i = 0; i < controlCount; ++i) {
    ...
    CAttributeSlider *slider =
        new CAttributeSlider(this, control->GetName(), control);
    ...
    if (bStereo) {
      slider->SetValue(ANIM_CONTROL_BALANCE,
                       control->GetValue<float>("balance"));
    }

    bool bMulti = control->GetValue<bool>("multi");
    slider->ActivateControl(ANIM_CONTROL_MULTILEVEL, bMulti);
    if (bMulti) {
      slider->SetValue(ANIM_CONTROL_MULTILEVEL,
                       control->GetValue<float>("multilevel"));
    }
    ...
    for (int j = 0; j < LOG_PREVIEW_MAX_CHANNEL_COUNT; ++j) {
      ...
      if (lookup.ch) {
        Assert(m_ChannelToSliderLookup.Find(lookup) ==
               m_ChannelToSliderLookup.InvalidIndex());
...
void CBaseAnimSetAttributeSliderPanel::SetVisibleControlsForSelectionGroup(
    CUtlSymbolTable &visible) {
  int i, c;
  ...
  for (i = 0; i < c; ++i) {
    ...
    bool showSlider =
        visible.Find(sliderName) != UTL_INVAL_SYMBOL ? true : false;
...
void CBaseAnimSetAttributeSliderPanel::ApplyPreset(float flScale,
  ...
  for (int i = 0; i < c; ++i) {
    ...
    target.m_pValue[ANIM_CONTROL_VALUE] =
        slider->GetControlDefaultValue(ANIM_CONTROL_VALUE);
...
bool CBaseAnimSetAttributeSliderPanel::ApplySliderValues(bool bForce) {
  ...
  for (int i = 0; i < c; ++i) {
    ...
    bool bUsePreviewValue =
        pSlider->IsPreviewEnabled() && (!pSlider->IsSimplePreview() || bForce);
    ...
    for (int j = 0; j < LOG_PREVIEW_MAX_CHANNEL_COUNT; ++j) {
      ...
      if (bPullSlidersFromScene) {
        ...
        // If we're dragging value, we're now going to be changing balance as
        // well
        if (pSlider->GetDragControl() == ANIM_CONTROL_VALUE &&
            type == ANIM_CONTROL_BALANCE)
          continue;
        ...
        float flValue = bUsePreviewValue ? pSlider->GetPreview(type)
                                         : pSlider->GetValue(type);
        if (pControl->GetValue<float>(s_pAnimControlAttribute[type]) !=
                flValue ||
            bForce) {
          valuesChanged = true;
          pControl->SetValue<float>(s_pAnimControlAttribute[type], flValue);
...
void CBaseAnimSetAttributeSliderPanel::UpdatePreviewSliderTimes() {
  ...
  for (int i = 0; i < c; ++i) {
    ...
    // If a fader is being dragged, or we're directly manipulating the slider,
  ...
  case AS_RECORD: {
    // Put one or all things into record mode (if dragging a single slider,
    // other channels should be in playback mode)
    if (changingvalues || previewing) {
      ActivateControlSetInMode(changingvalues ? CM_RECORD : CM_PASS, CM_PLAY,
                               CM_PLAY, dragSlider);
    } else {
...
bool CBaseAnimSetAttributeSliderPanel::GetAttributeSliderValue(
    AttributeValue_t *pValue, const char *name) {
  int c = m_SliderList.Count();
  for (int i = 0; i < c; ++i) {
    CAttributeSlider *slider = m_SliderList[i];
...
void CBaseAnimSetAttributeSliderPanel::GetChannelsForControl(
    CDmElement *control, CDmeChannel *channels[LOG_PREVIEW_MAX_CHANNEL_COUNT]) {
  ...
  if (control->GetValue<bool>("combo")) {
    channels[LOG_PREVIEW_VALUE] =
        control->GetValueElement<CDmeChannel>("valuechannel");
    channels[LOG_PREVIEW_BALANCE] =
        control->GetValueElement<CDmeChannel>("balancechannel");
  } else {
    channels[LOG_PREVIEW_VALUE] =
        control->GetValueElement<CDmeChannel>("channel");
    channels[LOG_PREVIEW_BALANCE] = 0;
  }

  if (control->GetValue<bool>("multi")) {
    channels[LOG_PREVIEW_MULTILEVEL] =
        control->GetValueElement<CDmeChannel>("multilevelchannel");
  } else {
...
void CBaseAnimSetAttributeSliderPanel::GetActiveTimeSelectionParams(
    DmeLog_TimeSelection_t &params) {
  Assert(0);
}

void CBaseAnimSetAttributeSliderPanel::ActivateControlSetInMode(
    int mode, int otherChannelsMode, int hiddenChannelsMode,
  ...
  bool updateChannelsModeChanged =
      m_nActiveControlSetMode != mode ? true : false;
    ...
    for (j = 0; j < LOG_PREVIEW_MAX_CHANNEL_COUNT; ++j) {
      ...
      if (hidden) {
        ...
        if (!whichSlider) {
          ...
          // this causes value and balance to get lumped together, since we're
          // now dragging left/right values
          if (slider->GetDragControl() == ANIM_CONTROL_MULTILEVEL) {
            useArray = (j == LOG_PREVIEW_MULTILEVEL) ? &updateChannels
                                                     : &otherChannels;
          } else {
            useArray = (j != LOG_PREVIEW_MULTILEVEL) ? &updateChannels
                                                     : &otherChannels;
  ...
  if (c > 0) {
    if (channelMode == CM_RECORD) {
      DmeLog_TimeSelection_t params;
      GetActiveTimeSelectionParams(params);
      params.SetRecordingMode((NULL == whichSlider) ? RECORD_PRESET
                                                    : RECORD_ATTRIBUTESLIDER);

      g_pChannelRecordingMgr->StartLayerRecording("Dragging Sliders", &params);

      for (i = 0; i < c; ++i) {
        // THIS PUTS THEM INTO CM_RECORD
        g_pChannelRecordingMgr->AddChannelToRecordingLayer(
            updateChannels[i], GetCurrentMovie(), GetCurrentShot());
      }

      SetTimeSelectionParametersForRecordingChannels(
          (NULL == whichSlider) ? m_Previous.amount : 1.0f);
    } else {
...
void CBaseAnimSetAttributeSliderPanel::
    SetTimeSelectionParametersForRecordingChannels(float flIntensity) {
...
void CBaseAnimSetAttributeSliderPanel::MaybeAddPreviewLog(
    CDmeFilmClip *shot, CUtlVector<LogPreview_t> &list, CDmElement *control,
    bool bDragging, bool isActiveLog, bool bSelected) {
  CDmeChannel *ctrlChannels[LOG_PREVIEW_MAX_CHANNEL_COUNT];
  GetChannelsForControl(control, ctrlChannels);
...
void CBaseAnimSetAttributeSliderPanel::PerformRecomputePreview() {
  ...
  for (i = 0; i < c; ++i) {
    ...
    MaybeAddPreviewLog(shot, m_ActiveTransforms, control, slider->IsDragging(),
                       false, slider->IsSelected());
    MaybeAddPreviewLog(shot, m_CurrentPreview, control, slider->IsDragging(),
                       (control == previewControl), slider->IsSelected());
  }
}

CUtlVector<LogPreview_t> *
CBaseAnimSetAttributeSliderPanel::GetActiveTransforms() {
  return &m_ActiveTransforms;
}

CDmElement *CBaseAnimSetAttributeSliderPanel::GetElementFromSlider(
...
void CBaseAnimSetAttributeSliderPanel::SetLogPreviewControlFromSlider(
    CAttributeSlider *pSlider) {
...
void CBaseAnimSetAttributeSliderPanel::SetLogPreviewControl(
    CDmElement *control) {
  if (!m_hEditor.Get())
    return;
...
void CBaseAnimSetAttributeSliderPanel::GetVisibleControls(
    CUtlVector<VisItem_t> &list) {
  const CDmaElementArray<CDmElement> &controls = m_AnimSet->GetControls();
...
int CBaseAnimSetAttributeSliderPanel::BuildVisibleControlList(
  ...
  for (i = 0; i < c; ++i) {
    ...
    MaybeAddPreviewLog(NULL, list, ctrl, slider->IsDragging(), false,
                       slider->IsSelected());
  }
...
int CBaseAnimSetAttributeSliderPanel::BuildFullControlList(
    CUtlVector<LogPreview_t> &list) {
  if (!m_AnimSet.Get())
    return 0;
...
void CBaseAnimSetAttributeSliderPanel::MoveToSlider(
    CAttributeSlider *pCurrentSlider, int nDirection) {
...
void CBaseAnimSetAttributeSliderPanel::SetControlSelected(
    CAttributeSlider *slider, bool state) {
  slider->SetSelected(state);
  RecomputePreview();
}

void CBaseAnimSetAttributeSliderPanel::SetControlSelected(CDmElement *control,
                                                          bool state) {
...
CAttributeSlider *
CBaseAnimSetAttributeSliderPanel::FindSliderForControl(CDmElement *control) {
  int c = m_SliderList.Count();
  int i;
  for (i = 0; i < c; ++i) {
...
bool CBaseAnimSetAttributeSliderPanel::GetSliderValues(AttributeValue_t *pValue,
                                                       int nIndex) {
  Assert(pValue);
