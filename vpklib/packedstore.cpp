...
static int s_FileHeaderSize(char const *pName, int nNumDataParts,
                            int nNumMetaDataBytes) {
...
// hash chain for accelerating file lookups. We can find an extension by hash,
// and find the directories containing files with this extension by another hash
...
public:
  ...
  CUtlIntrusiveList<CFileDirectoryData>
      m_pDirectoryHashTable[PACKEDFILE_DIR_HASH_SIZE];
...
static int SkipFile(char const *&pData) // returns highest file index
{
  ...
  pData += sizeof(uint16);
  while (Q_memcmp(pData, &packedfileindex_end, sizeof(packedfileindex_end)) !=
         0) {
...
CFileHeaderFixedData *CPackedStore::FindFileEntry(char const *pDirname,
  ...
  CFileExtensionData const *pExt =
      m_pExtensionData[nExtensionHash].FindNamedNodeCaseSensitive(pExtension);
  if (pExt) {
    int nDirHash = HashString(pDirname) % PACKEDFILE_DIR_HASH_SIZE;
    CFileDirectoryData const *pDir =
        pExt->m_pDirectoryHashTable[nDirHash].FindNamedNodeCaseSensitive(
            pDirname);
    if (pDir) {
      if (pExtBaseOut)
        *pExtBaseOut = (uint8 *)pDir;
      // we found the right directory. now, sequential search. data is heavily
      // packed, so this is a little awkward. See fileformat.txt
      ...
      while (*pData) // until we're out of files to look at
      {
        if (!V_strcmp(pData, pBaseName)) // found it?
        {
          ...
          return (CFileHeaderFixedData *)(pData + 1 +
                                          V_strlen(pData)); // return header
        }
...
CPackedStore::CPackedStore(char const *pFileBasename, char *pszFName,
                           IBaseFileSystem *pFS, bool bOpenForWrite)
  ...
  if (pFileBasename) {
    ...
    // Try to load the VPK as a standalone (probably an addon) even if the
    // standard _dir name is not present
    if (dirFile.IsOk()) {
      m_bUseDirFile = true;
    } else {
      m_bUseDirFile = false;
    ...
    if (dirFile.IsOk()) {
      ...
      if ((dirFile.Read(&dirHeader, sizeof(dirHeader)) == sizeof(dirHeader)) &&
        ...
        if (dirHeader.m_nVersion == VPK_PREVIOUS_VERSION) {
          ...
          dirHeader.m_nEmbeddedChunkSize = dirFile.Size() -
                                           dirHeader.m_nDirectorySize -
                                           sizeof(VPKDirHeaderOld_t);
          ...
          Error("Unknown version %d for vpk %s", dirHeader.m_nVersion,
                pFileBasename);
        }
        bNewFileFormat = true;
      ...
      // now, if we are opening for write, read the entire contents of the
      // embedded data chunk in the dir into ram
      if (bOpenForWrite && bNewFileFormat) {
        if (dirHeader.m_nEmbeddedChunkSize) {
          ...
          // We'll allocate around half a meg of contiguous memory for the read.
          // Any more and the SDK's VPK utility has a higher chance of choking
          // on low-end machines.
      ...
      if (dirHeader.m_nSelfHashesSize == 3 * sizeof(m_DirectoryMD5.bits)) {
        ...
        // at this point the filesystem has calculated an MD5 of everything in
        // the file up to this point. we could ask it for a snapshot of that MD5
        // value and then be able to compare it to m_TotalFileMD5 but we would
        // have to do it *before* we read it
      ...
      if (dirHeader.m_nSignatureSize != 0) {
        ...
        uint32 nExpectedSignedSize =
            nSizeOfHeader + dirHeader.ComputeSizeofSignedDataAfterHeader();
        if (m_nSizeOfSignedData != nExpectedSignedSize) {
          Error(
              "Size mismatch determining size of signed data block (%d vs %d)",
              m_nSizeOfSignedData, nExpectedSignedSize);
        }
    ...
    Q_MakeAbsolutePath(m_pszFullPathName, sizeof(m_pszFullPathName),
                       m_pszFileBaseName);
...
void CPackedStore::GetDataFileName(char *pchFileNameOut, int cchFileNameOut,
                                   int nFileNumber) const {
  if (nFileNumber == VPKFILENUMBER_EMBEDDED_IN_DIR_FILE) {
    if (m_bUseDirFile) {
      V_snprintf(pchFileNameOut, cchFileNameOut, "%s_dir.vpk",
                 m_pszFileBaseName);
    } else {
      V_snprintf(pchFileNameOut, cchFileNameOut, "%s.vpk", m_pszFileBaseName);
    }
  } else {
    V_snprintf(pchFileNameOut, cchFileNameOut, "%s_%03d.vpk", m_pszFileBaseName,
               nFileNumber);
  }
}
...
void SplitFileComponents(char const *pFileName, char *pDirOut, char *pBaseOut,
                         char *pExtOut) {
  char pTmpDirOut[MAX_PATH];
  V_ExtractFilePath(pFileName, pTmpDirOut, MAX_PATH);
  // now, pTmpDirOut to pDirOut, except when we find more then one '\' in a row,
  // only output one
  ...
  *(pOutDirPtr) = 0; // null terminate
  ...
  // the game sometimes asks for paths like dir1/../dir2/ we will replace this
  // with dir2/. This one line of perl code sucks in c++.
  for (;;) {
    char *pDotDot =
        V_strstr(pDirOut + 1,
                 "/../"); // start at second char. we don't want a beginning /
    if (!pDotDot) {
      break;
    }
...
CPackedStoreFileHandle CPackedStore::OpenFile(char const *pFileName) {
  ...
  if (pHeader) {
    ...
    ret.m_nFileSize = pHeader->m_PartDescriptors[0].m_nFileDataSize +
                      pHeader->m_nMetaDataSize;
...
void CPackedStore::Write(void) {
  ...
  if (m_SignaturePrivateKey.Count() > 0 && m_SignaturePublicKey.Count() > 0) {
    ...
    headerOut.m_nSignatureSize = sizeof(uint32) + m_SignaturePublicKey.Count() +
                                 sizeof(uint32) + nExpectedSignatureSize;
#else
  ...
  if (m_EmbeddedChunkData.Count()) {
    ...
    while (nRemainingSize > 0) {
      // We'll write around half a meg of contiguous memory at once. Any more
      // and the SDK's VPK utility has a higher chance of choking on low-end
      // machines.
  ...
  bufDirFile.Put(m_vecChunkHashFraction.Base(),
                 m_vecChunkHashFraction.Count() *
                     sizeof(m_vecChunkHashFraction[0]));
  ...
  MD5_ProcessSingleBuffer(bufDirFile.Base(), bufDirFile.TellPut(),
                          m_TotalFileMD5);
  ...
  if (headerOut.m_nSignatureSize > 0) {
    ...
    uint32 nExpectedSignedSize =
        sizeof(headerOut) + headerOut.ComputeSizeofSignedDataAfterHeader();
    if (m_nSizeOfSignedData != nExpectedSignedSize) {
      Error("Size mismatch determining size of signed data block (%d vs %d)",
            m_nSizeOfSignedData, nExpectedSignedSize);
    }
    ...
    if (!CCrypto::RSASignSHA256((const uint8 *)bufDirFile.Base(),
                                bufDirFile.TellPut(),
                                (uint8 *)m_Signature.Base(), &cubSignature,
                                (const uint8 *)m_SignaturePrivateKey.Base(),
    ...
    if (cubSignature != nExpectedSignatureSize) {
      Error(
          "VPK signing produced %d byte signature.  Expected size was %d bytes",
          cubSignature, nExpectedSignatureSize);
    }
    ...
    if (!CCrypto::RSAVerifySignatureSHA256(
            (const uint8 *)bufDirFile.Base(), bufDirFile.TellPut(),
            (const uint8 *)m_Signature.Base(), cubSignature,
            (const uint8 *)m_SignaturePublicKey.Base(),
            m_SignaturePublicKey.Count())) {
      Error("VPK signature verification failed immediately after signing.  The "
            "public key might be invalid, or might not match the private key "
  ...
  GetDataFileName(szOutFileName, sizeof(szOutFileName),
...
CPackedStore::ESignatureCheckResult
CPackedStore::CheckSignature(int nSignatureSize, const void *pSignature) const {
  ...
  if (nSignatureSize > 0 && pSignature != NULL) {
    if (m_SignaturePublicKey.Count() != nSignatureSize ||
        V_memcmp(pSignature, m_SignaturePublicKey.Base(), nSignatureSize) !=
            0) {
  ...
  if (!g_pFullFileSystem->ReadFile(szFilename, NULL, bufSignedData,
                                   m_nSizeOfSignedData))
  ...
  // Check the signature
  if (!CCrypto::RSAVerifySignatureSHA256(
          (const uint8 *)bufSignedData.Base(), m_nSizeOfSignedData,
...
CPackedStoreReadCache::CPackedStoreReadCache(IBaseFileSystem *pFS)
    : m_treeCachedVPKRead(CachedVPKRead_t::Less) {
  m_pPackedStore = NULL;
  m_cItemsInCache = 0;
  m_pFileSystem = pFS;
...
// check if the read request can be satisfied from the read cache we have in 1MB
// chunks
bool CPackedStoreReadCache::BCanSatisfyFromReadCache(
    uint8 *pOutData, CPackedStoreFileHandle &handle,
    FileHandleTracker_t &fHandle, int nDesiredPos, int nNumBytes, int &nRead) {
  nRead = 0;
  ...
  // the request might straddle multiple chunks - we make sure we have all of
  // the data, if we are missing any, we fail
  while (nNumBytes) {
    int nReadChunk = 0;
    if (!BCanSatisfyFromReadCacheInternal(
            pOutData, handle, fHandle, nDesiredPos, cubReadChunk, nReadChunk)) {
      return false;
    }
...
bool CPackedStoreReadCache::ReadCacheLine(FileHandleTracker_t &fHandle,
                                          CachedVPKRead_t &cachedVPKRead) {
  cachedVPKRead.m_cubBuffer = 0;
#ifdef IS_WINDOWS_PC
  if (cachedVPKRead.m_nFileFraction != fHandle.m_nCurOfs)
    SetFilePointer(fHandle.m_hFileHandle, cachedVPKRead.m_nFileFraction, NULL,
                   FILE_BEGIN);
  ReadFile(fHandle.m_hFileHandle, cachedVPKRead.m_pubBuffer,
           k_cubCacheBufferSize, (LPDWORD)&cachedVPKRead.m_cubBuffer, NULL);
  SetFilePointer(fHandle.m_hFileHandle, fHandle.m_nCurOfs, NULL, FILE_BEGIN);
#else
  m_pFileSystem->Seek(fHandle.m_hFileHandle, cachedVPKRead.m_nFileFraction,
                      FILESYSTEM_SEEK_HEAD);
  cachedVPKRead.m_cubBuffer = m_pFileSystem->Read(
      cachedVPKRead.m_pubBuffer, k_cubCacheBufferSize, fHandle.m_hFileHandle);
  m_pFileSystem->Seek(fHandle.m_hFileHandle, fHandle.m_nCurOfs,
                      FILESYSTEM_SEEK_HEAD);
#endif
  Assert(cachedVPKRead.m_hMD5RequestHandle == 0);
  if (m_pFileTracker) // file tracker doesn't exist in the VPK command line tool
  {
    cachedVPKRead.m_hMD5RequestHandle =
        m_pFileTracker->SubmitThreadedMD5Request(
            cachedVPKRead.m_pubBuffer, cachedVPKRead.m_cubBuffer,
            m_pPackedStore->m_PackFileID, cachedVPKRead.m_nPackFileNumber,
            cachedVPKRead.m_nFileFraction);
  }
...
bool CPackedStoreReadCache::CheckMd5Result(CachedVPKRead_t &cachedVPKRead) {
  ...
  if (!m_pPackedStore->FindFileHashFraction(cachedVPKRead.m_nPackFileNumber,
                                            cachedVPKRead.m_nFileFraction,
                                            chunkHashFraction))
    return true;

  if (Q_memcmp(&cachedVPKRead.m_md5Value, &chunkHashFraction.m_md5contents,
               sizeof(MD5Value_t)) != 0) {
    char szFilename[512];
    m_pPackedStore->GetDataFileName(szFilename, sizeof(szFilename),
                                    cachedVPKRead.m_nPackFileNumber);

    char szCalculated[MD5_DIGEST_LENGTH * 2 + 4];
    char szExpected[MD5_DIGEST_LENGTH * 2 + 4];
    V_binarytohex(cachedVPKRead.m_md5Value.bits, MD5_DIGEST_LENGTH,
                  szCalculated, sizeof(szCalculated));
    V_binarytohex(chunkHashFraction.m_md5contents.bits, MD5_DIGEST_LENGTH,
                  szExpected, sizeof(szExpected));
    ...
    Warning("Corruption detected in %s\n"
            ...
            szFilename, cachedVPKRead.m_nFileFraction, szExpected,
            szCalculated);
...
int CPackedStoreReadCache::FindBufferToUse() {
  ...
  for (int i = 0; i < m_cItemsInCache; i++) {
    ...
    if (m_treeCachedVPKRead[idxCurrent].m_hMD5RequestHandle) {
      ...
      if (m_pFileTracker->IsMD5RequestComplete(
              cachedVPKRead.m_hMD5RequestHandle, &cachedVPKRead.m_md5Value)) {
  ...
  if (m_treeCachedVPKRead[idxToRemove].m_hMD5RequestHandle) {
    ...
    m_pFileTracker->BlockUntilMD5RequestComplete(
        cachedVPKRead.m_hMD5RequestHandle, &cachedVPKRead.m_md5Value);
...
// manage the cache
bool CPackedStoreReadCache::BCanSatisfyFromReadCacheInternal(
    uint8 *pOutData, CPackedStoreFileHandle &handle,
    FileHandleTracker_t &fHandle, int nDesiredPos, int nNumBytes, int &nRead) {
  ...
  if (idxTrackedVPKFile == m_treeCachedVPKRead.InvalidIndex() ||
    ...
    // if we didnt find it, we had to grab the write lock, it may have been
    // added while we waited
  ...
  if (cachedVPKRead.m_pubBuffer == NULL) {
    ...
    // We need to have it locked for write, because we're about to muck with
    // these structures.
    if (!bLockedForWrite) {
  ...
  if (cachedVPKRead.m_pubBuffer != NULL && nBufferEnd > nDesiredPos) {
    ...
    m_cReadFromCache++;
    bSuccess = true;
    m_rgLastUsedTime[m_treeCachedVPKRead[idxTrackedVPKFile].m_idxLRU] =
        Plat_MSTime();
  }
  if (bLockedForWrite)
...
void CPackedStoreReadCache::RetryBadCacheLine(CachedVPKRead_t &cachedVPKRead) {
  ChunkHashFraction_t chunkHashFraction;
  m_pPackedStore->FindFileHashFraction(cachedVPKRead.m_nPackFileNumber,
                                       cachedVPKRead.m_nFileFraction,
                                       chunkHashFraction);

  cachedVPKRead.m_pubBuffer = (uint8 *)malloc(k_cubCacheBufferSize);
  FileHandleTracker_t &fHandle =
      m_pPackedStore->GetFileHandle(cachedVPKRead.m_nPackFileNumber);
...
// this is currently only for gathering information, doesnt do anything to
// repair the cache
void CPackedStoreReadCache::RetryAllBadCacheLines() {
  ...
  //			// m_listCachedVPKReadsFailed contains all the data
  //about failed reads - for error or OGS reporting
  //		}
  //		m_rwlock.UnlockWrite();
  //	}
}
...
void CPackedStore::GetPackFileLoadErrorSummary(CUtlString &sErrors) {
  FOR_EACH_LL(m_PackedStoreReadCache.m_listCachedVPKReadsFailed, i) {
    ...
    fhandle.m_nFileNumber =
        m_PackedStoreReadCache.m_listCachedVPKReadsFailed[i].m_nPackFileNumber;
    ...
    Q_binarytohex(
        m_PackedStoreReadCache.m_listCachedVPKReadsFailed[i].m_md5Value.bits,
        sizeof(MD5Value_t), hex, sizeof(hex));

    ChunkHashFraction_t chunkHashFraction;
    FindFileHashFraction(
        m_PackedStoreReadCache.m_listCachedVPKReadsFailed[i].m_nPackFileNumber,
        m_PackedStoreReadCache.m_listCachedVPKReadsFailed[i].m_nFileFraction,
        chunkHashFraction);

    char hex2[sizeof(MD5Value_t) * 2 + 1];
    Q_binarytohex(chunkHashFraction.m_md5contents.bits, sizeof(MD5Value_t),
                  hex2, sizeof(hex2));
...
int CPackedStore::ReadData(CPackedStoreFileHandle &handle, void *pOutData,
                           int nNumBytes) {
  int nRet = 0;
  ...
  if (nNumBytes > 0) {
    ...
    int nNumMetaDataBytes =
        MIN(nNumBytes, handle.m_nMetaDataSize - handle.m_nCurrentFileOffset);
    if (nNumMetaDataBytes > 0) {
      memcpy(pOutData,
             reinterpret_cast<uint8 const *>(handle.m_pMetaData) +
                 handle.m_nCurrentFileOffset,
             nNumMetaDataBytes);
    ...
    if (nNumBytes > 0) {
      ...
      int nDesiredPos = handle.m_nFileOffset + handle.m_nCurrentFileOffset -
                        handle.m_nMetaDataSize;
      int nRead;
      fHandle.m_Mutex.Lock();
      if (handle.m_nFileNumber == VPKFILENUMBER_EMBEDDED_IN_DIR_FILE) {
        // for file data in the directory header, all offsets are relative to
        // the size of the dir header.
        nDesiredPos += m_nDirectoryDataSize + sizeof(VPKDirHeader_t);
      }

      if (m_PackedStoreReadCache.BCanSatisfyFromReadCache(
              (uint8 *)pOutData, handle, fHandle, nDesiredPos, nNumBytes,
              nRead)) {
        ...
        ReadFile(fHandle.m_hFileHandle, pOutData, nNumBytes, (LPDWORD)&nRead,
                 NULL);
#else
        m_pFileSystem->Seek(fHandle.m_hFileHandle, nDesiredPos,
                            FILESYSTEM_SEEK_HEAD);
...
bool CPackedStore::HashEntirePackFile(CPackedStoreFileHandle &handle,
                                      int64 &nFileSize, int nFileFraction,
...
#define CRC_CHUNK_SIZE (32 * 1024)
  ...
  unsigned int fileLength =
      GetFileSize(fHandle.m_hFileHandle, (LPDWORD)&fileSizeHigh);
#else
...
void CPackedStore::HashChunkFile(int iChunkFileIndex) {
  ...
  while (1) {
    ...
    HashEntirePackFile(VPKHandle, fileSize, nFileFraction, k_nFileFractionSize,
                       filehash);
...
void CPackedStore::HashAllChunkFiles() {
  ...
  for (int iChunkFileIndex = 0; iChunkFileIndex <= GetHighestChunkFileIndex();
       ++iChunkFileIndex)
...
bool CPackedStore::BTestDirectoryHash() {
  ...
  return Q_memcmp(m_DirectoryMD5.bits, md5Directory.bits,
                  sizeof(md5Directory.bits)) == 0;
}
...
bool CPackedStore::BTestMasterChunkHash() {
  ...
  return Q_memcmp(m_ChunkHashesMD5.bits, md5ChunkHashes.bits,
                  sizeof(md5ChunkHashes.bits)) == 0;
}
...
bool CPackedStore::FindFileHashFraction(int nPackFileNumber, int nFileFraction,
                                        ChunkHashFraction_t &fileHashFraction) {
...
void CPackedStore::GetPackFileName(CPackedStoreFileHandle &handle,
                                   char *pchFileNameOut,
                                   int cchFileNameOut) const {
...
#else
    m_FileHandles[nFileHandleIdx].m_hFileHandle =
        m_pFileSystem->Open(pszDataFileName, "rb");
    if (m_FileHandles[nFileHandleIdx].m_hFileHandle !=
...
bool CPackedStore::InternalRemoveFileFromDirectory(const char *pszName) {
  ...
  int nBytesToRemove = (int)(V_strlen((char *)pData.m_pDirFileNamePtr) + 1 +
                             pHeader->HeaderSizeIncludingMetaData());
  m_DirectoryData.RemoveMultiple(
      pData.m_pDirFileNamePtr - m_DirectoryData.Base(), nBytesToRemove);
  return true;
}

void CPackedStore::AddFileToDirectory(const VPKContentFileInfo_t &info) {
  // this method is fairly complicated because it has to do inserts into the
  // packed directory data Our strategy is to build out the whole ext _ dir _
  // file record. if none of this is already present, we will just insert it in
  // the head of the file. If the extension is present, we'll insert the
  // dir+file part. If the extension + dir is present, we just insert the file
  // part at the right place. If everything is present, we just need to return
  // the current record
  ...
  int nNumDataParts = 1;
  int nFileDataSize =
      s_FileHeaderSize(pszBase, nNumDataParts, info.m_iPreloadSize);
  int nTotalHeaderSize =
      (int)(nFileDataSize + (2 + strlen(pszExt)) + (2 + strlen(pszDir)));
  ...
  *(pOut++) = 0;
  ...
  newPart.m_nFileNumber = (info.m_idxChunk < 0)
                              ? VPKFILENUMBER_EMBEDDED_IN_DIR_FILE
                              : info.m_idxChunk;
  ...
  *(pOut++) = 0; // mark no more dirs in extension
  ...
  // now, we need to insert our header, figuring out how many of the fields are
  // already there
  int nExtensionHash = HashString(pszExt) % PACKEDFILE_EXT_HASH_SIZE;
  int nInsertOffset = 0;
  CFileExtensionData const *pExt =
      m_pExtensionData[nExtensionHash].FindNamedNodeCaseSensitive(pszExt);
  ...
  if (pExt) {
    ...
    CFileDirectoryData const *pDir =
        pExt->m_pDirectoryHashTable[nDirHash].FindNamedNodeCaseSensitive(
            pszDir);
    if (pDir) {
      // dir and extension found. all we need to do is insert the file data
      // itself
      ...
      nInsertOffset =
          pStartOfDirFileData - (char const *)(m_DirectoryData.Base());
    } else {
      char const *pStartOfExtFileData = pExt->m_Name + 1 + strlen(pExt->m_Name);
      nInsertOffset =
          pStartOfExtFileData - (char const *)(m_DirectoryData.Base());
    }
  }
...
ePackedStoreAddResultCode
CPackedStore::AddFile(char const *pFile, uint16 nMetaDataSize,
                      const void *pFileData, uint32 nFileTotalSize,
                      bool bMultiChunk, uint32 const *pCrcValue) {
  ...
  {
    CFileHeaderFixedData *pHeader = pData.m_pHeaderData;
    if ((nFileTotalSize == pHeader->TotalDataSize()) &&
        (pHeader->m_nFileCRC == nCRC) &&
  ...
  const unsigned char *pDataStart =
      (const unsigned char *)pFileData + dirEntry.m_iPreloadSize;
  ...
  if (bMultiChunk && nBytesInChunk > 0) {
    ...
    if (m_nHighestChunkFileIndex < 0) {
      ...
      GetDataFileName(szDataFileName, sizeof(szDataFileName),
                      m_nHighestChunkFileIndex);
      dirEntry.m_iOffsetInChunk = g_pFullFileSystem->Size(szDataFileName);
      if ((int)dirEntry.m_iOffsetInChunk <=
          0) // technical wrong, but we shouldn't have 2GB chunks.  (Sort of
             // defeats the whole purpose.)
      {
        // Note, there is one possible failure case.  if we have a file whose
        // data is actually all in the preload section, but it is marked as
        // being in a chunk, then we might have a zero byte "chunk."  We really
        // should not be assigning any files to "chunks" if they are entirely in
        // the preload area.
    ...
    m_nHighestChunkFileIndex =
        MAX(m_nHighestChunkFileIndex, dirEntry.m_idxChunk);
    ...
    m_pFileSystem->Seek(fHandle, dirEntry.m_iOffsetInChunk,
                        FILESYSTEM_SEEK_HEAD);
...
int CPackedStore::GetFileList(CUtlStringList &outFilenames,
                              bool bFormattedOutput, bool bSortedOutput) {
  return GetFileList(NULL, outFilenames, bFormattedOutput, bSortedOutput);
}

int CPackedStore::GetFileList(const char *pWildCard,
  ...
  // Parse the wildcard string into a base and extension used for string
  // comparisons
  if (pWildCard) {
    ...
    // Remove '*' from the base and extension strings so that the string
    // comparison calls will match
      ...
      while (*pData) // until we're out of files to look at
      {
        ...
        if (bFormattedOutput) {
          CFileHeaderFixedData const *pHeader =
              reinterpret_cast<CFileHeaderFixedData const *>(pData + 1 +
                                                             strlen(pData));
          sprintf(pszFNameOut, "%s%s%s crc=0x%x metadatasz=%d", pszCurDir,
                  pData, pszCurExtension, pHeader->m_nFileCRC,
                  pHeader->m_nMetaDataSize);
          CFilePartDescr const *pPart = &(pHeader->m_PartDescriptors[0]);
          while (pPart->m_nFileNumber != PACKFILEINDEX_END) {
            sprintf(pszFNameOut + strlen(pszFNameOut),
                    " fnumber=%d ofs=0x%x sz=%d", pPart->m_nFileNumber,
        ...
        if (pWildCard) {
          ...
          V_ExtractFilePath(pszFNameOut, szFNameOutPath,
                            sizeof(szFNameOutPath));
          V_FileBase(pszFNameOut, szFNameOutBase, sizeof(szFNameOutBase));
          V_ExtractFileExtension(pszFNameOut, szFNameOutExt,
                                 sizeof(szFNameOutExt));

          matches = !V_strnicmp(szFNameOutPath, szWildCardPath,
                                sizeof(szWildCardPath));
          matches =
              matches && (!V_strlen(szWildCardExt) || bNoExtWildcard
                              ? 0 == V_strnicmp(szFNameOutExt, szWildCardExt,
                                                strlen(szWildCardExt))
                              : 0 != V_stristr(szFNameOutExt, szWildCardExt));
          matches =
              matches && (!V_strlen(szWildCardBase) || bNoBaseWildcard
                              ? 0 == V_strnicmp(szFNameOutBase, szWildCardBase,
                                                strlen(szWildCardBase))
...
void CPackedStore::GetFileList(
    const char *pWildcard, CUtlVector<VPKContentFileInfo_t> &outVecResults) {
  ...
  FOR_EACH_VEC(vecFilenames, i) {
    ...
    if (!h)
      Error("File '%s' was returned by GetFileList, but OpenFile() fails?!",
            vecFilenames[i]);

    // Convert to output structure
    VPKContentFileInfo_t &f = outVecResults[outVecResults.AddToTail()];
    f.m_sName = vecFilenames[i];
    f.m_idxChunk = (h.m_nFileNumber == VPKFILENUMBER_EMBEDDED_IN_DIR_FILE)
                       ? -1
                       : h.m_nFileNumber;
...
int CPackedStore::GetFileAndDirLists(CUtlStringList &outDirnames,
                                     CUtlStringList &outFilenames,
                                     bool bSortedOutput) {
...
void CPackedStore::BuildFindFirstCache() {
  ...
  // Init
  V_strncpy(szLastDirFound,
            "$$$$$$$HighlyUnlikelyPathForInitializationPurposes#######",
            sizeof(szLastDirFound));
  ...
  FOR_EACH_VEC(allVPKFiles, i) {
    ...
    if (V_strnicmp(szFilePath, szLastDirFound, sizeof(szLastDirFound))) {
      ...
      m_dirContents.Insert(m_directoryList.Count(),
                           new CUtlStringList()); // Freed in destructor
    }
...
int CPackedStore::GetFileAndDirLists(const char *pWildCard,
                                     CUtlStringList &outDirnames,
                                     CUtlStringList &outFilenames,
                                     bool bSortedOutput) {

  // If this is the first time we've called FindFirst on this CPackedStore then
  // let's build the caches
  if (!m_directoryList.Count()) {
    BuildFindFirstCache();
#ifdef NEVER
    printf("CPackedStore::GetFileAndDirLists - list of directories in VPK "
           "files\n");
  ...
  if (pWildCard) {
    ...
    // From the pattern, we now have the directory path up to the file pattern,
    // the filename base, and the filename extension.
    ...
    FOR_EACH_VEC(m_directoryList, i) {
      ...
      if ((nLenWildcardPath &&
        ...
        if (szSubDirExtension) {
          // Null out the . and move the szSubDirExtension to point to the
          // extension
          *szSubDirExtension = '\0';
          szSubDirExtension++;
        }

        // If we have a base dir name, and we have a szWildCardBase to match
        // against
        if (bBaseWildcard)
          bBaseMatch = true; // The base is the wildCard ("*"), so whatever we
                             // have as the base matches
        else
          bBaseMatch =
              (0 == V_strnicmp(szSubDir, szWildCardBase, nLenWildcardBase));

        // If we have an extension and we have a szWildCardExtension to mach
        // against
        if (bExtWildcard)
          bExtMatch = true; // The extension is the wildcard ("*"), so whatever
                            // we have as the extension matches
        else
          bExtMatch = (NULL == szSubDirExtension && '\0' == *szWildCardExt) ||
                      ((NULL != szSubDirExtension) &&
                       (0 == V_strnicmp(szSubDirExtension, szWildCardExt,
                                        nLenWildcardExt)));

        // If both parts match, then add it to the list of directories that
        // match
    ...
    FOR_EACH_VEC(m_directoryList, i) {
      ...
      if (!V_strnicmp(szWildCardPath, m_directoryList[i],
                      sizeof(szWildCardPath))) {
        ...
        FOR_EACH_VEC(filesInDirectory, iFile) {
          ...
          V_FileBase(filesInDirectory[iFile], szFNameOutBase,
                     sizeof(szFNameOutBase));
          V_ExtractFileExtension(filesInDirectory[iFile], szFNameOutExt,
                                 sizeof(szFNameOutExt));

          // Since we have a sorted list we can optimize using the return code
          // of the compare
          ...
          if (matches) {
            ...
            FOR_EACH_VEC(outFilenames, j) {
              if (!V_strncmp(outFilenames[j], filesInDirectory[iFile],
                             V_strlen(filesInDirectory[iFile]))) {
                bFound = true;
                break;
              }
            }
