...
#define VPKFILENUMBER_EMBEDDED_IN_DIR_FILE                                     \
  0x7fff // if a chunk refers to this file number, it is data embedded in the
         // same file as the directory block.

#define VPK_HEADER_MARKER                                                      \
  0x55aa1234 // significes that this is a new vpk header format
...
struct VPKDirHeader_t {
  ...
  uint32 ComputeSizeofSignedDataAfterHeader() const {
    return m_nDirectorySize + m_nEmbeddedChunkSize + m_nChunkHashesSize +
           m_nSelfHashesSize;
  }
};
