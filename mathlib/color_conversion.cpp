...
//-----------------------------------------------------------------------------
static byte texgammatable[256]; // palette is sent through this to convert to
                                // screen gamma
...
static void CheckExponentTable() {
  for (int i = 0; i < 256; i++) {
    ...
    Assert(testAgainst == 0 ? power2_n[i] < 1.16E-041
...
inline static int VectorToColorRGBExp32_CalcExponent(const float *pin) {
  ...
  expComponent -= biasedSeven; // now the difference from seven (positive if was
                               // less than, etc)
  return expComponent;
}
...
/// code, or better yet moving it onto the cell.
/// \warning: Assumes an IEEE 754 single-precision float representation! Those
/// of you porting to an 8080 are out of luck.
void VectorToColorRGBExp32(const Vector &vin, ColorRGBExp32 &c) {
  Assert(s_bMathlibInitialized);
  ...
  // We can totally wind up above 255 and that's okay--but above 256 would be
  // right out.
  Assert(vin.x * scalar < 256.0f && vin.y * scalar < 256.0f &&
         vin.z * scalar < 256.0f);
