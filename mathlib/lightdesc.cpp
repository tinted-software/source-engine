...
void LightDesc_t::ComputeLightAtPoints(const FourVectors &pos,
  ...
  case MATERIAL_LIGHT_SPOT: {
    fltx4 dot2 = SubSIMD(
        Four_Zeros,
        delta *
            m_Direction); // dot position with spot light dir for cone falloff
    ...
    if ((m_Falloff != 0.0) && (m_Falloff != 1.0)) {
      // !!speed!! could compute integer exponent needed by powsimd and store in
      // light
      cone_falloff_scale = PowSIMD(cone_falloff_scale, m_Falloff);
    }
    strength = MulSIMD(cone_falloff_scale, strength);

    // now, zero out lighting where dot2<phidot. This will mask out any invalid
    // results from pow function, etc
    fltx4 OutsideMask =
...
void LightDesc_t::SetupNewStyleAttenuation(float fFiftyPercentDistance,
  ...
  if (d0 < d50) {
    // !!warning in lib code???!!!
    Warning("light has _fifty_percent_distance of %f but no "
