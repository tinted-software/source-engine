...
void ColorQuantize(uint8 const *Image, int Width, int Height, int flags,
                   int ncolors, uint8 *out_pixels, uint8 *out_palette,
                   int firstcolor) {
    ...
    for (x = 0; x < Width; x++) {
      ...
      for (int i = 0; i < N_EXTRAVALUES; i++) {
        ...
        NthSample(s, y * Width + x, N_DIMENSIONS)->Value[c] =
            (uint8)(min(255, max(0, val1)));
      }
    }
  struct QuantizedValue *q =
      Quantize(s, Width * Height, N_DIMENSIONS, ncolors, Weights, firstcolor);
  delete[] s;
