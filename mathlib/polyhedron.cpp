...
CPolyhedron *
ClipLinkedGeometry(GeneratePolyhedronFromPlanes_UnorderedPolygonLL *pPolygons,
...
// #define ENABLE_DEBUG_POLYHEDRON_DUMPS //Dumps debug information to disk for
// use with glview. Requires that tier2 also be in all projects using debug
// mathlib #define DEBUG_DUMP_POLYHEDRONS_TO_NUMBERED_GLVIEWS //dumps
// successfully generated polyhedrons
...
void DumpAABBToGLView(const Vector &vCenter, const Vector &vExtents,
                      const Vector &vColor, FILE *pFile);
...
CPolyhedron_AllocByNew *CPolyhedron_AllocByNew::Allocate(
    ...
    unsigned short iPolygons) // creates the polyhedron along with enough memory
                              // to hold all it's data in a single allocation
{
  void *pMemory =
...
#include "tier0/memdbgon.h"
  ...
  pAllocated->pVertices =
      (Vector *)(pAllocated + 1); // start vertex memory at the end of the class
  pAllocated->pLines =
...
CPolyhedron *GetTempPolyhedron(
    unsigned short iVertices, unsigned short iLines, unsigned short iIndices,
    unsigned short
        iPolygons) // grab the temporary polyhedron. Avoids new/delete for quick
...
struct GeneratePolyhedronFromPlanes_Line {
  ...
  GeneratePolyhedronFromPlanes_Polygon
      *pPolygons[2]; // viewing from the outside with the point connections
                     // going up, 0 is the left polygon, 1 is the right
  int iSaveIndices;
  bool bAlive; // connected to at least one living point
  bool bCut;   // connected to at least one dead point

  GeneratePolyhedronFromPlanes_LineLL
      *pPointLineLinks[2]; // rather than going into a point and searching for
                           // its link to this line, lets just cache it to
                           // eliminate searching
  GeneratePolyhedronFromPlanes_LineLL
      *pPolygonLineLinks[2]; // rather than going into a polygon and searching
                             // for its link to this line, lets just cache it to
                             // eliminate searching
...
struct GeneratePolyhedronFromPlanes_LineLL {
  ...
  int iReferenceIndex; // whatever is referencing the line should know which
                       ...
                       // polygons, it's which point to follow to continue going
                       // clockwise, which makes polygon 0 the one on the left
                       // side of an upward facing line vector, for points, it's
                       // the OTHER point's index
...
CPolyhedron *ClipPolyhedron(const CPolyhedron *pExistingPolyhedron,
  ...
  if (iUsefulPlaneCount == 0) {
    ...
    memutils::copy(pReturn->pVertices, pExistingPolyhedron->pVertices,
                   pReturn->iVertexCount);
    memutils::copy(pReturn->pLines, pExistingPolyhedron->pLines,
                   pReturn->iLineCount);
    memutils::copy(pReturn->pIndices, pExistingPolyhedron->pIndices,
                   pReturn->iIndexCount);
    memutils::copy(pReturn->pPolygons, pExistingPolyhedron->pPolygons,
                   pReturn->iPolygonCount);
...
bool FindConvexShapeLooseAABB(
    const float *pInwardFacingPlanes, int iPlaneCount, Vector *pAABBMins,
    Vector *pAABBMaxs) // bounding box of the convex shape (subject to floating
                       // point error)
{
  ...
  float *vertsOut = (float *)stackalloc(
      (iPlaneCount + 4) *
      (sizeof(float) * 3)); // each plane will initially have 4 points in its
                            // polygon representation, and each plane clip has
                            // the possibility to add 1 point to the polygon
  float *vertsSwap;
...
#if defined(_DEBUG) && defined(ENABLE_DEBUG_POLYHEDRON_DUMPS) &&               \
    defined(DEBUG_DUMP_POLYHEDRONS_TO_NUMBERED_GLVIEWS)
...
const char *
DumpPolyhedronCutHistory(const CUtlVector<CPolyhedron *> &DumpedHistory,
                         const CUtlVector<const float *> &CutHistory,
                         const VMatrix *pTransform) {
#ifdef ENABLE_DEBUG_POLYHEDRON_DUMPS
  if (pTransform == NULL)
    pTransform = &s_matIdentity;
    ...
    //===================================================================================================
    {
      ...
      do {
        ...
        if (fPointDist > fOnPlaneEpsilon) {
          ...
          pPoint->planarity =
              POINT_ONPLANE; // point is on the plane, he's everyone's buddy

          // Project on-plane points leaning towards death closer to the plane.
          // This battles floating point precision decay.
          //  Consider the case of a large on-plane epsilon leaving protrusions
          //  over time
          /*if( fPointDist < 0.0f )
          {
                  ...
                  pPoint->ptPosition.x = vPositionAsDouble[0] - (distAsDouble *
vNormalAsDouble[0]); pPoint->ptPosition.y = vPositionAsDouble[1] - (distAsDouble
* vNormalAsDouble[1]); pPoint->ptPosition.z = vPositionAsDouble[2] -
(distAsDouble * vNormalAsDouble[2]);

#if ( 0 && defined( _DEBUG ) )
                  float fDebugDist = vNormal.Dot( pPoint->ptPosition ) -
fPlaneDist; //just for looking at in watch windows AssertMsg( fabs( fDebugDist )
< fabs(fPointDist), "Projected point is further from plane than unprojected." );
#endif
                  fPointDist = vNormal.Dot( pPoint->ptPosition ) - fPlaneDist;
//recompute dist (not guaranteed to be 0.0 like we want)
          }*/
        }
            ...
            do {
              if (pOnPlaneLineWalk->pLine->bAlive) {
                ...
                if (pOnPlaneLineWalk->pNext->pLine->bCut ||
                  ...
                  // polygon of the polyhedron. 	We have to downgrade this point
                  //to dead to avoid situations where float imprecision 	turns
                  //the polyhedron into a *slightly* concave shape. Concave
                  //shapes might break this algorithm, even falsely concave
                  //shapes.
                  bDead = true;
                  break;
                }
    ...
    // surrounded by higher status 	It indicates a concave shape. It's impossible
    //to have it occur in theoretical space. But floating point numbers
    //introduce error.
    {
      ...
      do {
        ...
        AssertMsg_DumpPolyhedron(
            iSurroundedCount <= 1,
            "Concave polygon, cutting process might break. Consider adjusting "
            "the on-plane epsilon to better compensate for floating point "
            "precision.");
                                   ...
                                   // connected to a dead one. Dead line
        {
          ...
          for (int i = 0; i != 2; ++i) {
            ...
            if (pLineLink->pNext == pLineLink) {
              ...
              pPolygon->pLines =
                  pLineLink->pPrev; // Always have the polygon's head line be
                                    // just before the gap in the polygon
                  ...
                  sizeof(GeneratePolyhedronFromPlanes_Point));
          {
            ...
            float fInvTotalDist =
                ...
                (pDeadPoint->fPlaneDist -
                 pLivingPoint->fPlaneDist); // subtraction because the living
                                            // index is known to be negative
    ...
    //===================================================================================================
    {
      ...
      if (pActivePolygonWalk) {
        ...
        while (pLines->pLine->bAlive) // now seek counterclockwise until we find
                                      // a line on the plane (in case we started
                                      // on an alive line last seek)
          pLines = pLines->pPrev;
      ...
      //(pPrev) 			If the line is on-plane. Skip the bridge line making, but set
      //links to the new polygon as if we'd just created the bridge
      ...
      do {
        if (pWorkPolygon->bMissingASide) {
          ...
          pGapLines[1] =
              pTestLine->pLine->pPolygonLineLinks
                  [pTestLine->iReferenceIndex]; // get the same line, but in the
                                                // polygons linked list.
          ...
          Assert_DumpPolyhedron(
              ...
              (pWorkPolygon->bMissingASide ==
               true)); // if we're not done fixing, and if the shared line was
    ...
    // verify that repairs are complete
    {
      GeneratePolyhedronFromPlanes_UnorderedPolygonLL *pDebugPolygonWalk =
          pAllPolygons;
      do {
        AssertMsg_DumpPolyhedron(pDebugPolygonWalk->pPolygon->bMissingASide ==
                                     false,
...
CPolyhedron *GeneratePolyhedronFromPlanes(const float *pOutwardFacingPlanes,
  ...
  // therefore isn't a solid method this version will start with a cube and hack
  // away at it (retaining point connection information) to produce a polyhedron
  // with no guesswork involved, this method should be rock solid
  ...
  GeneratePolyhedronFromPlanes_UnorderedPointLL StartingPointList[8]; // 8
                                                                      // points
...
void DumpAABBToGLView(const Vector &vCenter, const Vector &vExtents,
  ...
  fprintf(pFile, "%6.3f %6.3f %6.3f %.2f %.2f %.2f\n", vMins.x, vMins.y,
          vMins.z, vColor.x, vColor.y, vColor.z);
  fprintf(pFile, "%6.3f %6.3f %6.3f %.2f %.2f %.2f\n", vMins.x, vMins.y,
          vMaxs.z, vColor.x, vColor.y, vColor.z);
  fprintf(pFile, "%6.3f %6.3f %6.3f %.2f %.2f %.2f\n", vMins.x, vMaxs.y,
          vMaxs.z, vColor.x, vColor.y, vColor.z);
  fprintf(pFile, "%6.3f %6.3f %6.3f %.2f %.2f %.2f\n", vMins.x, vMaxs.y,
          vMins.z, vColor.x, vColor.y, vColor.z);
...
#define DPTGLV_LINE_WRITEPOINT(index)                                          \
  fprintf(pFile, "%6.3f %6.3f %6.3f %.2f %.2f %.2f\n", vLinePoints[index].x,   \
          vLinePoints[index].y, vLinePoints[index].z, pLineColors[index]->x,   \
    ...
    for (counter2 = 0; counter2 != pPolyhedron->pPolygons[counter].iIndexCount;
      ...
      Vector *pColor =
          &pColors[pPolyhedron->pLines[pLineReference->iLineIndex]
                       .iPointIndices[pLineReference->iEndPointIndex]];
      fprintf(pFile, "%6.3f %6.3f %6.3f %.2f %.2f %.2f\n", pVertex->x,
              pVertex->y, pVertex->z, pColor->x, pColor->y, pColor->z);
    }
  }
...
void DumpPlaneToGlView(const float *pPlane, float fGrayScale,
  ...
  fprintf(pFile, "%6.3f %6.3f %6.3f %.2f %.2f %.2f\n", vPlaneVerts[0].x,
          vPlaneVerts[0].y, vPlaneVerts[0].z, fGrayScale, fGrayScale,
          fGrayScale);
  fprintf(pFile, "%6.3f %6.3f %6.3f %.2f %.2f %.2f\n", vPlaneVerts[1].x,
          vPlaneVerts[1].y, vPlaneVerts[1].z, fGrayScale, fGrayScale,
          fGrayScale);
  fprintf(pFile, "%6.3f %6.3f %6.3f %.2f %.2f %.2f\n", vPlaneVerts[2].x,
          vPlaneVerts[2].y, vPlaneVerts[2].z, fGrayScale, fGrayScale,
          fGrayScale);
  fprintf(pFile, "%6.3f %6.3f %6.3f %.2f %.2f %.2f\n", vPlaneVerts[3].x,
          vPlaneVerts[3].y, vPlaneVerts[3].z, fGrayScale, fGrayScale,
          fGrayScale);
