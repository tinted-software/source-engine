...
const uint32 ALIGN16 g_SIMD_signmask[4] ALIGN16_POST = {0x80000000, 0x80000000,
                                                        0x80000000, 0x80000000};
const uint32 ALIGN16 g_SIMD_lsbmask[4] ALIGN16_POST = {0xfffffffe, 0xfffffffe,
                                                       0xfffffffe, 0xfffffffe};
const uint32 ALIGN16 g_SIMD_clear_wmask[4] ALIGN16_POST = {
    0xffffffff, 0xffffffff, 0xffffffff, 0};
const uint32 ALIGN16 g_SIMD_AllOnesMask[4] ALIGN16_POST = {
    0xffffffff, 0xffffffff, 0xffffffff, 0xffffffff}; // ~0,~0,~0,~0
const uint32 ALIGN16 g_SIMD_Low16BitsMask[4] ALIGN16_POST = {
    0xffff, 0xffff, 0xffff, 0xffff}; // 0xffff x 4
...
void FourVectors::RotateManyBy(FourVectors *RESTRICT pVectors,
...
#ifdef _X360
  // Same algorithm as above, but the loop is unrolled to eliminate data hazard
  // latencies and simplify prefetching. Named variables are deliberately used
  // instead of arrays to ensure that the variables live on the registers
  // instead of the stack (stack load/store is a serious penalty on 360).  Nb:
  // for prefetching to be most efficient here, the loop should be unrolled to 8
  // FourVectors per iteration; because each FourVectors is 48 bytes long, 48 *
  // 8 = 384, its least common multiple with the 128-byte cache line. That way
  // you can fetch the next 3 cache lines while you work on these three. If you
  // do go this route, be sure to dissassemble and make sure it doesn't spill
  // registers to stack as you do this; the cost of that will be excessive.
  // Unroll the loop a little and just live with the fact that you'll be doing a
  // couple of redundant dbcts (they don't cost you anything). Be aware that all
  // three cores share L2 and it can only have eight cache lines fetching at a
  // time.
  ...
  if (numVectors & 0x01) {
    ...
    pVectNext = pVectors++;
    outX1 = AddSIMD(AddSIMD(MulSIMD(pVectNext->x, matSplat00),
                            MulSIMD(pVectNext->y, matSplat01)),
                    MulSIMD(pVectNext->z, matSplat02));
    outY1 = AddSIMD(AddSIMD(MulSIMD(pVectNext->x, matSplat10),
                            MulSIMD(pVectNext->y, matSplat11)),
                    MulSIMD(pVectNext->z, matSplat12));
    outZ1 = AddSIMD(AddSIMD(MulSIMD(pVectNext->x, matSplat20),
                            MulSIMD(pVectNext->y, matSplat21)),
    ...
    outX0 = AddSIMD(AddSIMD(MulSIMD(pVectors->x, matSplat00),
                            MulSIMD(pVectors->y, matSplat01)),
                    MulSIMD(pVectors->z, matSplat02));
    outY0 = AddSIMD(AddSIMD(MulSIMD(pVectors->x, matSplat10),
                            MulSIMD(pVectors->y, matSplat11)),
                    MulSIMD(pVectors->z, matSplat12));
    outZ0 = AddSIMD(AddSIMD(MulSIMD(pVectors->x, matSplat20),
  ...
  EVEN_CASE:
    ...
    outX1 = MaddSIMD(pVectNext->z, matSplat02,
                     AddSIMD(MulSIMD(pVectNext->x, matSplat00),
                             MulSIMD(pVectNext->y, matSplat01)));
    outY1 = MaddSIMD(pVectNext->z, matSplat12,
                     AddSIMD(MulSIMD(pVectNext->x, matSplat10),
                             MulSIMD(pVectNext->y, matSplat11)));
    outZ1 = MaddSIMD(pVectNext->z, matSplat22,
                     AddSIMD(MulSIMD(pVectNext->x, matSplat20),
                             MulSIMD(pVectNext->y, matSplat21)));
...
#ifdef _X360
// Loop-scheduled code to process FourVectors in groups of eight quite
// efficiently.
void FourVectors_TransformManyGroupsOfEightBy(FourVectors *RESTRICT pVectors,
                                              unsigned int numVectors,
                                              const matrix3x4_t &rotationMatrix,
  ...
  AssertMsg(
      (pOut < pVectors && pOut + numVectors <= pVectors) ||
  ...
  fltx4 matSplat00, matSplat01, matSplat02, matSplat03, // TWELVE REGISTERS
      matSplat10, matSplat11, matSplat12, matSplat13, matSplat20, matSplat21,
      matSplat22, matSplat23;
  ...
  // this macro defines how to compute a specific row from an input and certain
  // splat columns
#define COMPUTE(res, invec, xterm, yterm, zterm, transterm)                    \
  res = AddSIMD(AddSIMD(MulSIMD((invec)->z, zterm),                            \
                        AddSIMD(MulSIMD((invec)->x, xterm),                    \
                                MulSIMD((invec)->y, yterm))),                  \
                transterm)
...
// #define FROZ(out,in,offset) COMPUTE((out+offset)->x, (in + offset),
// matSplat00, matSplat01, matSplat02, matSplat03); COMPUTE((out + offset )->y,
// (in + offset), matSplat10, matSplat11, matSplat12, matSplat13); COMPUTE((out
// + offset)->z, (in + offset), matSplat20, matSplat21, matSplat22, matSplat23)
#define COMPUTE_GROUP(resgroup, dataptr)                                       \
  COMPUTE(resgroup##X, (dataptr), matSplat00, matSplat01, matSplat02,          \
          matSplat03);                                                         \
  COMPUTE(resgroup##Y, (dataptr), matSplat10, matSplat11, matSplat12,          \
          matSplat13);                                                         \
  COMPUTE(resgroup##Z, (dataptr), matSplat20, matSplat21, matSplat22,          \
          matSplat23)
#define WRITE_GROUP(ptr, resgroup)                                             \
  ...
  */
#define COMPUTE_STAGE1_ROW(res, tempvar, invec, xsplat, ysplat, zsplat,        \
                           transplat)                                          \
  res = MulSIMD((invec)->y, ysplat);                                           \
  tempvar = MaddSIMD((invec)->z, zsplat, transplat)
#define COMPUTE_STAGE2_ROW(res, tempvar, invec, xsplat, ysplat, zsplat,        \
                           transplat)                                          \
  res = MaddSIMD((invec)->x, xsplat, res)
#define COMPUTE_STAGE3_ROW(res, tempvar, invec, xsplat, ysplat, zsplat,        \
                           transplat)                                          \
  res = AddSIMD(res, tempvar) // frees up the tempvar

#define COMPUTE_STAGE1_GROUP(resgroup, invec)                                  \
  COMPUTE_STAGE1_ROW(resgroup##X, resgroup##X##Temp, invec, matSplat00,        \
                     matSplat01, matSplat02, matSplat03);                      \
  COMPUTE_STAGE1_ROW(resgroup##Y, resgroup##Y##Temp, invec, matSplat10,        \
                     matSplat11, matSplat12, matSplat13);                      \
  COMPUTE_STAGE1_ROW(resgroup##Z, resgroup##Z##Temp, invec, matSplat20,        \
                     matSplat21, matSplat22, matSplat23)

#define COMPUTE_STAGE2_GROUP(resgroup, invec)                                  \
  COMPUTE_STAGE2_ROW(resgroup##X, resgroup##X##Temp, invec, matSplat00,        \
                     matSplat01, matSplat02, matSplat03);                      \
  COMPUTE_STAGE2_ROW(resgroup##Y, resgroup##Y##Temp, invec, matSplat10,        \
                     matSplat11, matSplat12, matSplat13);                      \
  COMPUTE_STAGE2_ROW(resgroup##Z, resgroup##Z##Temp, invec, matSplat20,        \
                     matSplat21, matSplat22, matSplat23)

#define COMPUTE_STAGE3_GROUP(resgroup, invec)                                  \
  COMPUTE_STAGE3_ROW(resgroup##X, resgroup##X##Temp, invec, matSplat00,        \
                     matSplat01, matSplat02, matSplat03);                      \
  COMPUTE_STAGE3_ROW(resgroup##Y, resgroup##Y##Temp, invec, matSplat10,        \
                     matSplat11, matSplat12, matSplat13);                      \
  COMPUTE_STAGE3_ROW(resgroup##Z, resgroup##Z##Temp, invec, matSplat20,        \
                     matSplat21, matSplat22, matSplat23)
  ...
  // out. If made truly cyclic, such that every line interleaved a stage 1,
  // stage 2, stage 3, and write, then throughput could be higher (probably by
  // about 50%).
  while (inData < STOP) {
...
#ifdef _X360
// Loop-scheduled code to process FourVectors in groups of eight quite
// efficiently. This is the version to call when starting on a 128-byte-aligned
// address.
void FourVectors_TransformManyGroupsOfEightBy_128byteAligned(
    FourVectors *RESTRICT pVectors, unsigned int numVectors,
  ...
  AssertMsg((numVectors & 0x07) == 0, "FourVectors_TransformManyGroupsOfEight "
                                      "called with numVectors % 8 != 0!");

  // Assert alignment
  AssertMsg(((reinterpret_cast<uint32>(pVectors) & 127) == 0) &&
                ((reinterpret_cast<uint32>(pOut) & 127) == 0),
            "FourVectors_Transform..aligned called with non-128-byte-aligned "
            "buffers.");
  ...
  // Here's the plan. 8 four-vecs = 3 cache lines exactly. It takes about 400
  // cycles to process a group of eight, and cache latency is 600 cycles, so we
  // try to prefetch two iterations ahead (eg fetch iteration 3 while working on
  // iteration 1). In the case of the output, we can simply zero-flush the cache
  // lines since we are sure to write into them. Because we're reading and
  // fetching two ahead, we want to stop two away from the last iteration.

  // No matter what, we will need to prefetch the first two groups of eight of
  // input (that's the first six cache lines)
  __dcbt(0, pVectors);
  __dcbt(128, pVectors);
  __dcbt(256, pVectors);
  __dcbt(384, pVectors);
...
#define WRITE(term, reg, toptr) toptr->term = reg
  ...
  // #define FROZ(out,in,offset) COMPUTE((out+offset)->x, (in + offset),
  // matSplat00, matSplat01, matSplat02, matSplat03); COMPUTE((out + offset
  // )->y, (in + offset), matSplat10, matSplat11, matSplat12, matSplat13);
  // COMPUTE((out + offset)->z, (in + offset), matSplat20, matSplat21,
  // matSplat22, matSplat23)
#define COMPUTE_GROUP(resgroup, dataptr)                                       \
  COMPUTE(resgroup##X, (dataptr), matSplat00, matSplat01, matSplat02,          \
          matSplat03);                                                         \
...
#define COMPUTE_STAGE3_GROUP(resgroup, invec)                                  \
  ...
  // Okay. First do all but the last two turns of the crank; we don't want to
  // overshoot with the flush-to-zero.
  FourVectors *RESTRICT inData = pVectors;
  FourVectors *RESTRICT outData = pOut;
...
void FourVectors::TransformManyBy(FourVectors *RESTRICT pVectors,
                                  unsigned int numVectors,
                                  const matrix3x4_t &rotationMatrix,
                                  FourVectors *RESTRICT pOut) {
  Assert(numVectors > 0);
...
#ifdef _X360
  // The really fast version of this function likes to operate on blocks of
  // eight. So, chug through groups of eight, then deal with any leftovers.
  int numVectorsRoundedToNearestEight = numVectors & (~0x07);
  if (numVectors >= 8) {
    // aligned?
    if ((reinterpret_cast<unsigned int>(pVectors) & 127) == 0 &&
        (reinterpret_cast<unsigned int>(pOut) & 127) == 0) {
      FourVectors_TransformManyGroupsOfEightBy_128byteAligned(
          pVectors, numVectorsRoundedToNearestEight, rotationMatrix, pOut);
    } else {
      FourVectors_TransformManyGroupsOfEightBy(
          pVectors, numVectorsRoundedToNearestEight, rotationMatrix, pOut);
    }
  ...
  if (numVectors > 0) {
    ...
    do {
      ...
      pOut->x =
          MaddSIMD(pVectors->z, matSplat02,
                   MaddSIMD(pVectors->y, matSplat01,
                            MaddSIMD(pVectors->x, matSplat00, matSplat03)));
      pOut->y =
          MaddSIMD(pVectors->z, matSplat12,
                   MaddSIMD(pVectors->y, matSplat11,
                            MaddSIMD(pVectors->x, matSplat00, matSplat13)));
      pOut->z =
          MaddSIMD(pVectors->z, matSplat22,
                   MaddSIMD(pVectors->y, matSplat21,
                            MaddSIMD(pVectors->x, matSplat00, matSplat23)));
...
// Loop-scheduled code to process FourVectors in groups of eight quite
// efficiently.
static void FourVectors_TransformManyGroupsOfEightBy_InPlace(
    FourVectors *RESTRICT pVectors, unsigned int numVectors,
    const matrix3x4_t &rotationMatrix) {
  Assert(numVectors > 0);
  if (numVectors == 0)
...
// In-place version of above. It's necessary to have this, rather than just
// allowing pOut and pVectors to equal each other, because of the semantics of
// RESTRICT: pVectors and pOut must not be allowed to alias. (Simply
// un-restricting the pointers results in very poor scheduling.)
void FourVectors::TransformManyBy(FourVectors *RESTRICT pVectors,
                                  unsigned int numVectors,
                                  const matrix3x4_t &rotationMatrix) {
  Assert(numVectors > 0);
  ...
  int numVectorsRoundedToNearestEight = numVectors & (~0x07);
  if (numVectors >= 8) {
    FourVectors_TransformManyGroupsOfEightBy_InPlace(
        pVectors, numVectorsRoundedToNearestEight, rotationMatrix);
    numVectors -= numVectorsRoundedToNearestEight;
    pVectors += numVectorsRoundedToNearestEight;
  }
#endif
...
void TransformManyPointsBy(VectorAligned *RESTRICT pVectors,
                           unsigned int numVectors, FLTX4 mRow0, FLTX4 mRow1,
                           FLTX4 mRow2) {
  /**************************************************
  ...
  COMPILE_TIME_ASSERT(sizeof(VectorAligned) ==
                      sizeof(XMFLOAT4)); // VectorAligned's need to be 16 bytes
  ...
  {
    // cook up the pointers from integer math. Necessary because otherwise we
    // LHS all over the place. (Odd that this doesn't happen to the xbox math.)
    ...
    UINT_PTR InputStreamCGroupBase =
        XMMin(InputVector + (XM_CACHE_LINE_SIZE - 1), InputStreamEnd) &
        ~(XM_CACHE_LINE_SIZE - 1);
    UINT_PTR InputStreamCGroupEnd =
        InputStreamCGroupBase + ((InputStreamEnd - InputStreamCGroupBase) &
                                 ~(4 * XM_CACHE_LINE_SIZE - 1));
