...
#pragma warning(disable : 4244) // "conversion from 'const int' to 'float',
                                // possible loss of data"
#pragma warning(disable : 4730) // "mixing _m64 and floating point expressions
...
//counterclockwise 				rotations in degrees around Y, Z, and X respectively.
//-----------------------------------------------------------------------------
...
void ConcatTransforms_Aligned(const matrix3x4_t &m0, const matrix3x4_t &m1,
                              matrix3x4_t &out) {
...
void VectorAngles(const Vector &forward, const Vector &pseudoup,
  ...
  } else // forward is mostly Z, gimbal lock-
  {
    ...
    angles[1] = RAD2DEG(
        atan2f(-left[0],
               left[1])); // This was originally copied from the "void
                          // MatrixAngles( const matrix3x4_t& matrix, float
                          // *angles )" code, and it's 180 degrees off, negated
                          // the values and it all works now (Dave Kircher)
...
//columns as follows: 			matrix[][0] is forward 			matrix[][1] is left 			matrix[][2] is
//up
//-----------------------------------------------------------------------------
...
float QuaternionAngleDiff(const Quaternion &p, const Quaternion &q) {
  ...
  // Note if the quaternion is slightly non-normalized the square root below may
  // be more than 1, the value is clamped to one otherwise it may result in
  // asin() returning an undefined result.
  float sinang =
      MIN(1.0f, sqrt(diff.x * diff.x + diff.y * diff.y + diff.z * diff.z));
...
//axis. 				[1]: YAW:	Counterclockwise rotation around the Z axis. 				[2]: ROLL:
//Counterclockwise rotation around the X axis. 			*outQuat - quaternion of form
//(i,j,k,real)
//-----------------------------------------------------------------------------
