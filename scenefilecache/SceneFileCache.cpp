...
private:
  ...
  bool GetSceneDataFromImage(const char *pSceneName, int iIndex, byte *pData,
                             size_t *pLength);
...
InitReturnVal_t CSceneFileCache::Init() {
  const char *pSceneImageName =
      IsX360() ? "scenes/scenes.360.image" : "scenes/scenes.image";
    ...
    if (filesystem->ReadFile(pSceneImageName, "GAME", m_SceneImageFile)) {
      ...
      if (IsX360()) {
        if (filesystem->GetDVDMode() == DVDMODE_STRICT) {
          ...
          // relaxed
          Warning(
              "CSceneFileCache: Failed to load %s, scene playback disabled.\n",
              pSceneImageName);
          return INIT_OK;
        }
      }
...
bool CSceneFileCache::GetSceneData(char const *pFilename, byte *buf,
                                   size_t bufsize) {
  Assert(pFilename);
  Assert(buf);
  Assert(bufsize > 0);
...
bool CSceneFileCache::GetSceneCachedData(char const *pFilename,
  ...
  SceneImageEntry_t *pEntries =
      (SceneImageEntry_t *)((byte *)pHeader + pHeader->nSceneEntryOffset);
  SceneImageSummary_t *pSummary =
      (SceneImageSummary_t *)((byte *)pHeader +
...
int CSceneFileCache::FindSceneInImage(const char *pSceneName) {
  ...
  CRC32_t crcFilename =
      CRC32_ProcessSingleBuffer(szCleanName, strlen(szCleanName));
