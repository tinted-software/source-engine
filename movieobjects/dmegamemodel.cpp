...
//-----------------------------------------------------------------------------
IMPLEMENT_ELEMENT_FACTORY(DmeGlobalFlexControllerOperator,
...
void CDmeGlobalFlexControllerOperator::OnAttributeChanged(
...
void CDmeGlobalFlexControllerOperator::GetInputAttributes(
    CUtlVector<CDmAttribute *> &attrs) {
  attrs.AddToTail(m_flexWeight.GetAttribute());
}

void CDmeGlobalFlexControllerOperator::GetOutputAttributes(
    CUtlVector<CDmAttribute *> &attrs) {
...
int CDmeGlobalFlexControllerOperator::FindGlobalFlexControllerIndex() const {
  ...
  MDLHandle_t h = pModelName && pModelName[0] ? g_pMDLCache->FindMDL(pModelName)
                                              : MDLHANDLE_INVALID;
    ...
    if (hdr) {
      ...
      for (LocalFlexController_t i = LocalFlexController_t(0); i < fc; ++i) {
        ...
        if (flex->localToGlobal == -1) {
          flex->localToGlobal =
              g_pGlobalFlexController->FindGlobalFlexController(
                  flex->pszName());
        }
...
CDmeGlobalFlexControllerOperator *
CDmeGameModel::AddGlobalFlexController(const char *controllerName,
                                       int globalIndex) {
  int i, c;
  ...
  if (i >= c) {
    CDmeGlobalFlexControllerOperator *newOperator =
        CreateElement<CDmeGlobalFlexControllerOperator>(controllerName,
                                                        GetFileId());
    Assert(newOperator);
    if (!newOperator)
      return NULL;
...
//-----------------------------------------------------------------------------
CDmeGlobalFlexControllerOperator *
CDmeGameModel::FindGlobalFlexController(int nGlobalIndex) {
  int i, c;
  c = m_globalFlexControllers.Count();
  for (i = 0; i < c; ++i) {
    CDmeGlobalFlexControllerOperator *op = m_globalFlexControllers.Get(i);
...
// example) into post-compiled data (.mdl or .ani files)
bool CDmeGameModel::GetSrcBoneTransforms(matrix3x4_t *pPreTransform,
                                         matrix3x4_t *pPostTransform,
                                         int nBoneIndex) const {
  ...
  for (int i = 0; i < nCount; ++i) {
    const mstudiosrcbonetransform_t *pSrcTransform =
        pStudioHdr->SrcBoneTransform(i);
...
CDmeGlobalFlexControllerOperator *
CDmeGameModel::GetGlobalFlexController(int localIndex) {
  return m_globalFlexControllers.Get(localIndex);
}

void CDmeGameModel::RemoveGlobalFlexController(
...
void CDmeGameModel::AppendGlobalFlexControllerOperators(
    CUtlVector<IDmeOperator *> &list) {
  int c = m_globalFlexControllers.Count();
  for (int i = 0; i < c; ++i) {
...
static bool IsDagUnderGameModel(CDmeDag *pDag, CDmeGameModel *pGameModel) {
  ...
  DmAttributeReferenceIterator_t i =
      g_pDataModel->FirstAttributeReferencingElement(pDag->GetHandle());
...
// Is this dag under the game model?
//-----------------------------------------------------------------------------
static CDmeDag *GetDagForTransform(CDmeTransform *pTransform,
                                   CDmeGameModel *pGameModel) {
  DmAttributeReferenceIterator_t i =
      g_pDataModel->FirstAttributeReferencingElement(pTransform->GetHandle());
  while (i != DMATTRIBUTE_REFERENCE_ITERATOR_INVALID) {
    CDmAttribute *pAttribute = g_pDataModel->GetAttribute(i);
    CDmElement *pDmeParent = pAttribute->GetOwner();
...
//-----------------------------------------------------------------------------
void CDmeGameModel::AddBones(studiohdr_t *pStudioHdr, const char *pBaseName,
                             int nFirstBone, int nCount) {
  ...
  // make room for bones
  CDmeDag **pDags =
      (CDmeDag **)_alloca(pStudioHdr->numbones * sizeof(CDmeDag *));
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    Q_snprintf(name, sizeof(name), "%s_bone %d (%s)", pBaseName, bi,
               pBone->pszName());
...
void CDmeGameModel::SetBone(uint index, const Vector &pos,
                            const Quaternion &rot) {
  m_bones[index]->SetPosition(pos);
  m_bones[index]->SetOrientation(rot);
}

void CDmeGameModel::RemoveAllBones() { m_bones.RemoveAll(); }

uint CDmeGameModel::NumBones() const { return m_bones.Count(); }
...
uint CDmeGameModel::NumFlexWeights() const { return m_flexWeights.Count(); }
...
void CDmeGameModel::SetFlexWeights(uint nFlexWeights,
                                   const float *flexWeights) {
...
void CDmeGameModel::SetFlags(int nFlags) { m_flags = nFlags; }

void CDmeGameModel::SetSkin(int nSkin) { m_skin = nSkin; }

void CDmeGameModel::SetBody(int nBody) { m_body = nBody; }

void CDmeGameModel::SetSequence(int nSequence) { m_sequence = nSequence; }

int CDmeGameModel::GetSkin() const { return m_skin; }

int CDmeGameModel::GetBody() const { return m_body; }

int CDmeGameModel::GetSequence() const { return m_sequence; }

const char *CDmeGameModel::GetModelName() const { return m_modelName.Get(); }
...
// accessors
//-----------------------------------------------------------------------------
const char *CDmeGameSprite::GetModelName() const { return m_modelName.Get(); }

float CDmeGameSprite::GetScale() const { return m_renderscale; }

float CDmeGameSprite::GetFrame() const { return m_frame; }

int CDmeGameSprite::GetRenderMode() const { return m_rendermode; }

int CDmeGameSprite::GetRenderFX() const { return m_renderfx; }

const Color &CDmeGameSprite::GetColor() const { return m_color; }

float CDmeGameSprite::GetProxyRadius() const { return m_proxyRadius; }

void CDmeGameSprite::SetState(bool bVisible, float nFrame, int nRenderMode,
                              int nRenderFX, float flRenderScale,
                              float flProxyRadius, const Vector &pos,
