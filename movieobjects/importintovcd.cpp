...
//-----------------------------------------------------------------------------
// Helper wrapper class for log layers (necessary to avoid movieobjects
// dependence)
//-----------------------------------------------------------------------------
...
private:
  void CurveSimplify_R(float flThreshold, int nStartPoint, int nEndPoint,
                       CDmeLogLayerHelper *pDest);

  // Computes the total error
  float ComputeTotalError(CDmeLogLayerHelper *pDest, int nStartPoint,
                          int nEndPoint);

  // Select the best fit curve type
  void ChooseBestCurveType(int nKey, int nStartPoint, int nEndPoint,
                           CDmeLogLayerHelper *pDest);

  // Compute first + second derivatives of data
  void ComputeDerivates(float *pSlope, float *pAccel, int nPoint,
                        CDmeLogLayerHelper *pDest);
...
int CDmeLogLayerHelper::TotalRemovedPoints() { return s_nTotalRemovedPoints; }

//-----------------------------------------------------------------------------
// Constructor
//-----------------------------------------------------------------------------
CDmeLogLayerHelper::CDmeLogLayerHelper(CDmElement *pLogLayer,
...
void CDmeLogLayerHelper::InsertAfter(int nAfter, int nTime, float flValue,
                                     int nCurveType) {
...
float CDmeLogLayerHelper::GetValue(int nTime) const {
  ...
  int nCurveType =
      m_curvetypes.Count() ? m_curvetypes[ti] : m_nDefaultCurveType;
  ...
  float t =
      (float)(nTime - m_times[ti]) / (float)(m_times[ti + 1] - m_times[ti]);
  Interpolator_CurveInterpolate(nInterpolationType, vecArg1, vecArg1, vecArg2,
                                vecArg2, t, vecOutput);
  return vecOutput.y;
}
...
// Computes the total error
//-----------------------------------------------------------------------------
float CDmeLogLayerHelper::ComputeTotalError(CDmeLogLayerHelper *pDest,
...
void CDmeLogLayerHelper::ChooseBestCurveType(int nKey, int nStartPoint,
                                             int nEndPoint,
                                             CDmeLogLayerHelper *pDest) {
  return;
  ...
  for (int i = 0; s_nInterpTypes[i] >= 0; ++i) {
    pDest->SetCurveType(nKey,
                        MAKE_CURVE_TYPE(s_nInterpTypes[i], s_nInterpTypes[i]));
...
// Compute first + second derivatives of data
//-----------------------------------------------------------------------------
void CDmeLogLayerHelper::ComputeDerivates(float *pSlope, float *pAccel,
                                          int nPoint,
                                          CDmeLogLayerHelper *pDest) {
  ...
  *pAccel = (flNextPoint - 2 * flCurrPoint + flPrevPoint) /
            (flDeltaTime * flDeltaTime);
}
...
//-----------------------------------------------------------------------------
void CDmeLogLayerHelper::CurveSimplify_R(float flThreshold, int nStartPoint,
                                         int nEndPoint,
                                         CDmeLogLayerHelper *pDest) {
  ...
  */
  ...
  if (flMaxDistance > flThreshold) {
    int nKey = pDest->InsertKey(m_times[nMaxPoint], m_values[nMaxPoint],
                                m_nDefaultCurveType);
    Assert(nKey != 0);
...
void CDmeLogLayerHelper::Simplify(float flThreshhold) {
  ...
  destLayer.AddToTail(m_times[nFirstKey], m_values[nFirstKey],
                      m_nDefaultCurveType);
  destLayer.AddToTail(m_times[nLastKey], m_values[nLastKey],
                      m_nDefaultCurveType);

  // Recursively finds the point with the largest error from the "simplified
  // curve" and subdivides the problem on both sides until the largest delta
  // from the simplified curve is less than the tolerance
  ...
  m_curvetypes.CopyArray(destLayer.m_curvetypes.Base(),
                         destLayer.m_curvetypes.Count());
...
//-----------------------------------------------------------------------------
static CChoreoActor *FindOrAddActor(CChoreoScene *pScene,
                                    const char *pActorName,
                                    const char *pActorModel) {
...
//-----------------------------------------------------------------------------
static CChoreoEvent *FindOrAddAnimationEvent(CChoreoScene *pScene,
                                             CChoreoActor *pActor) {
...
// Finds sound events
//-----------------------------------------------------------------------------
static CChoreoEvent *FindOrAddSoundEvent(CChoreoScene *pScene,
                                         CChoreoActor *pActor,
...
static CFlexAnimationTrack *FindOrCreateTrack(CChoreoEvent *pEvent,
                                              const char *pFlexControllerName) {
...
//-----------------------------------------------------------------------------
void GetStereoFlexControllerRange(float *pMin, float *pMax,
                                  studiohdr_t *pStudioHdr,
                                  const char *pFlexName) {
  ...
  for (LocalFlexController_t i = LocalFlexController_t(0);
       i < pStudioHdr->numflexcontrollers; ++i) {
    ...
    // FIXME: Probably want to get the left + right controller + find the min
    // and max of each, but this is unnecessary.
...
void GetFlexControllerRange(float *pMin, float *pMax, studiohdr_t *pStudioHdr,
                            const char *pFlexName) {
  for (LocalFlexController_t i = LocalFlexController_t(0);
       i < pStudioHdr->numflexcontrollers; ++i) {
    mstudioflexcontroller_t *pFlex = pStudioHdr->pFlexcontroller(i);
...
//-----------------------------------------------------------------------------
void ImportSamplesIntoTrack(CFlexAnimationTrack *pTrack, CDmElement *pLog,
                            int nSampleType, int nTimeOffset,
  ...
  int nDefaultCurveType =
      MAKE_CURVE_TYPE(info.m_nInterpolationType, info.m_nInterpolationType);
  ...
  for (int j = 0; j < nSampleCount; ++j) {
    ...
    CExpressionSample *pSample =
        pTrack->AddSample(flTime, flValue, nSampleType);
...
//-----------------------------------------------------------------------------
void ImportMonoLogDataIntoEvent(studiohdr_t *pStudioHdr, CChoreoEvent *pEvent,
                                const char *pTrackName, CDmElement *pLog,
...
//-----------------------------------------------------------------------------
void ImportStereoLogDataIntoEvent(studiohdr_t *pStudioHdr, CChoreoEvent *pEvent,
                                  const char *pTrackName, CDmElement *pValueLog,
                                  CDmElement *pBalanceLog, int nTimeOffset,
                                  const ImportVCDInfo_t &info) {
...
static int ComputeEventTime(CDmElement *pRoot, CChoreoEvent *pEvent) {
  ...
  // Iterate over all elements in the animations attribute; each one refers to a
  // log.
...
//-----------------------------------------------------------------------------
void ImportAnimations(CDmElement *pRoot, CChoreoScene *pChoreoScene,
                      CChoreoActor *pActor, studiohdr_t *pStudioHdr,
                      const ImportVCDInfo_t &info) {
  CChoreoEvent *pEvent = FindOrAddAnimationEvent(pChoreoScene, pActor);
  pEvent->SetDefaultCurveType(
      MAKE_CURVE_TYPE(info.m_nInterpolationType, info.m_nInterpolationType));
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    if (StringHasPrefix(pLogName, "value_")) {
      ...
      Q_snprintf(pBalanceName, sizeof(pBalanceName), "balance_%s",
                 pLogName + 6);
      ...
      if (pBalanceLog) {
        ImportStereoLogDataIntoEvent(pStudioHdr, pEvent, pLogName + 6, pLog,
                                     pBalanceLog, nTimeOffset, info);
      }
    } else {
      ImportMonoLogDataIntoEvent(pStudioHdr, pEvent, pLogName, pLog,
                                 nTimeOffset, info);
    }
  }
}
...
//-----------------------------------------------------------------------------
void ImportSounds(CDmElement *pRoot, CChoreoScene *pChoreoScene,
                  CChoreoActor *pActor, const ImportVCDInfo_t &info) {
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    CChoreoEvent *pEvent =
        FindOrAddSoundEvent(pChoreoScene, pActor, pEventName);
...
//-----------------------------------------------------------------------------
bool ImportLogsIntoVCD(const char *pFacFullPath, CChoreoScene *pChoreoScene,
                       const ImportVCDInfo_t &info) {
  CDmElement *pRoot;
  DmFileId_t id = g_pDataModel->RestoreFromFile(pFacFullPath, NULL, NULL,
                                                &pRoot, CR_FORCE_COPY);
...
// Main entry point for importing a .fac file into a .vcd file
//-----------------------------------------------------------------------------
bool ImportLogsIntoVCD(const char *pFacFullPath, const char *pVCDInFullPath,
                       const char *pVCDOutPath, const ImportVCDInfo_t &info) {
  CUtlBuffer buf;
  ...
  CChoreoScene *pScene =
      ChoreoLoadScene(pVCDInFullPath, NULL, GetTokenProcessor(), NULL);
  if (!pScene) {
  ...
  if (!Q_IsAbsolutePath(pVCDOutPath)) {
    g_pFullFileSystem->RelativePathToFullPath(pVCDOutPath, NULL, pTemp,
                                              sizeof(pTemp));
