...
bool ConvertEventToDmx(CChoreoEvent *event, CDmeChannelsClip *clip) {
  ...
  if (array.IsValid()) {
    CDmeChannel *channel =
        CreateElement<CDmeChannel>("event_ramp", clip->GetFileId());
    array.AddToTail(channel);

    // Fill in values..., just log for now
    channel->CreateLog(AT_FLOAT);
    CDmeTypedLog<float> *ramp =
        static_cast<CDmeTypedLog<float> *>(channel->GetLog());
    if (ramp) {
      CDmeFloatCurveInfo *pCurveInfo = CreateElement<CDmeFloatCurveInfo>(
          "floatcurveinfo", clip->GetFileId());

      pCurveInfo->SetDefaultCurveType(
          MAKE_CURVE_TYPE(INTERPOLATE_CATMULL_ROM_NORMALIZE,
      ...
      // Left edge
      pCurveInfo->SetEdgeInfo(0, event->GetRamp()->IsEdgeActive(true),
                              event->GetRamp()->GetEdgeZeroValue(true),
                              event->GetRamp()->GetEdgeCurveType(true));
      // Right edge
      pCurveInfo->SetEdgeInfo(1, event->GetRamp()->IsEdgeActive(false),
      ...
      for (int j = 0; j < rampCount; ++j) {
        ...
        ramp->SetKey(DmeTime_t(sample->time), sample->value,
                     sample->GetCurveType());
      }
    }
  }
  ...
  if (event->GetNumRelativeTags() > 0) {
    CDmElement *tags =
        CreateElement<CDmElement>("relative_tags", clip->GetFileId());
    if (tags) {
  ...
  if (event->GetNumTimingTags() > 0) {
    CDmElement *tags =
        CreateElement<CDmElement>("timing_tags", clip->GetFileId());
    if (tags) {
  ...
  for (tagtype = 0; tagtype < CChoreoEvent::NUM_ABS_TAG_TYPES; tagtype++) {
    if (event->GetNumAbsoluteTags((CChoreoEvent::AbsTagType)tagtype) > 0) {
      ...
      if (tags) {
        ...
        Assert(names.IsValid() && percentages.IsValid());
        for (int t = 0;
             t < event->GetNumAbsoluteTags((CChoreoEvent::AbsTagType)tagtype);
             t++) {
          CEventAbsoluteTag *abstag =
              event->GetAbsoluteTag((CChoreoEvent::AbsTagType)tagtype, t);
          Assert(abstag);
  ...
  if (event->IsUsingRelativeTag()) {
    CDmElement *relativeTag =
        CreateElement<CDmElement>("relative_tag", clip->GetFileId());
    if (relativeTag) {
  ...
  case CChoreoEvent::SPEAK: {
    CDmElement *speakProperties =
        CreateElement<CDmElement>("SPEAK", clip->GetFileId());
    if (speakProperties) {
      clip->SetValue<CDmElement>("SPEAK", speakProperties);

      speakProperties->SetValue(
          "closedcaption_type",
          CChoreoEvent::NameForCCType(event->GetCloseCaptionType()));
      speakProperties->SetValue("closedcaption_token",
                                event->GetCloseCaptionToken());
      bool usingCombined =
          (event->GetCloseCaptionType() != CChoreoEvent::CC_DISABLED &&
           event->IsUsingCombinedFile());
      speakProperties->SetValue<bool>("closedcaption_usingcombinedfile",
                                      usingCombined);
      speakProperties->SetValue<bool>("closedcaption_combinedusesgender",
                                      event->IsCombinedUsingGenderToken());
      speakProperties->SetValue<bool>("closedcaption_noattenuate",
                                      event->IsSuppressingCaptionAttenuation());
    }
  } break;
  case CChoreoEvent::GESTURE: {
    CDmElement *gestureProperties =
        CreateElement<CDmElement>("GESTURE", clip->GetFileId());
  ...
  case CChoreoEvent::FLEXANIMATION: {
    ...
    if (array.IsValid()) {
      ...
      for (int i = 0; i < numTracks; ++i) {
        ...
        CDmeChannel *channel = CreateElement<CDmeChannel>(
            track->GetFlexControllerName(), clip->GetFileId());
        array.AddToTail(channel);
        ...
        if (log) {
          ...
          pCurveInfo->SetDefaultEdgeZeroValue(0.0f);
          // Left edge
          pCurveInfo->SetEdgeInfo(0, track->IsEdgeActive(true),
                                  track->GetEdgeZeroValue(true),
                                  track->GetEdgeCurveType(true));
          // Right edge
          pCurveInfo->SetEdgeInfo(1, track->IsEdgeActive(false),
          ...
          for (int j = 0; j < sampleCount; ++j) {
            ...
            log->SetKey(DmeTime_t(sample->time), sample->value,
                        sample->GetCurveType());
          }
        }
        ...
        if (track->IsComboType()) {
          ...
          Q_snprintf(balanceChannelName, sizeof(balanceChannelName),
                     "%s_balance", track->GetFlexControllerName());

          CDmeChannel *balanceChannel =
              CreateElement<CDmeChannel>(balanceChannelName, clip->GetFileId());
          ...
          CDmeTypedLog<float> *balance =
              static_cast<CDmeTypedLog<float> *>(balanceChannel->GetLog());
          if (balance) {
            CDmeFloatCurveInfo *pCurveInfo = CreateElement<CDmeFloatCurveInfo>(
                "floatcurveinfo", clip->GetFileId());
            ...
            */
            ...
            for (int j = 0; j < sampleCount; ++j) {
              ...
              balance->SetKey(DmeTime_t(sample->time), sample->value,
                              sample->GetCurveType());
            }
          }
        }
  ...
  case CChoreoEvent::LOOP: {
    CDmElement *loopProperties =
        CreateElement<CDmElement>("LOOP", clip->GetFileId());
    if (loopProperties) {
...
bool ConvertSceneToDmx(CChoreoScene *scene, CDmeFilmClip *dmx) {
  ...
  CDmElement *scaleSettings =
      CreateElement<CDmElement>("scalesettings", dmx->GetFileId());
  Assert(scaleSettings);
  CDmAttribute *scaleAttribute =
      dmx->AddAttributeElement<CDmElement>("scalesettings");
  Assert(scaleAttribute);
  scaleAttribute->SetValue(scaleSettings->GetHandle());
  if (scaleSettings) {
    for (int i = scene->TimeZoomFirst(); i != scene->TimeZoomInvalid();
         i = scene->TimeZoomNext(i)) {
  ...
  CDmeTrackGroup *pTrackGroup =
      dmx->FindOrAddTrackGroup(VCD_SCENE_RAMP_TRACK_GROUP_NAME);
  CDmeTrack *track = pTrackGroup->FindOrAddTrack(
      VCD_SCENE_RAMP_TRACK_GROUP_NAME, DMECLIP_CHANNEL);
  Assert(track);

  // Set a CDmeChannel for the scene_ramp
  CDmeChannelsClip *pClip = CreateElement<CDmeChannelsClip>(
      VCD_SCENE_RAMP_TRACK_GROUP_NAME, dmx->GetFileId());
  Assert(pClip);
  track->AddClip(pClip);
  ...
  for (int actor = 0; actor < numActors; ++actor) {
    ...
    for (int channel = 0; channel < numChannels; ++channel) {
      ...
      CDmeTrack *track =
          pTrackGroup->FindOrAddTrack(channelName, DMECLIP_CHANNEL);
      Assert(track);
      if (!track)
        continue;
      ...
      for (int event = 0; event < numEvents; ++event) {
        ...
        CDmeChannelsClip *pClip =
            CreateElement<CDmeChannelsClip>("", dmx->GetFileId());
        Assert(pClip);
        track->AddClip(pClip);
  ...
  track = pTrackGroup->FindOrAddTrack(VCD_GLOBAL_EVENTS_TRACK_GROUP_NAME,
                                      DMECLIP_CHANNEL);
  Assert(track);
...
void EnsureActorAndChannelForTrack(CChoreoScene *scene, CDmeTrackGroup *pActor,
                                   CDmeTrack *pChannel) {
...
template <class T>
T *FindAttributeInArray(const CDmrElementArray<> &array,
...
bool ConvertDmxToEvent(CChoreoScene *scene, CDmeTrackGroup *pActor,
                       CDmeTrack *pChannel, CDmeChannelsClip *clip,
                       bool globalEvent) {
  bool bret = true;
  ...
  if (array.IsValid()) {
    ...
    for (int i = 0; i < c; ++i) {
      ...
      for (int j = 0; j < rampCount; ++j) {
        CExpressionSample *sample = event->AddRamp(
            ramp->GetKeyTime(j).GetSeconds(), ramp->GetKeyValue(j), false);
  ...
  for (tagtype = 0; tagtype < CChoreoEvent::NUM_ABS_TAG_TYPES; tagtype++) {
    ...
    if (clip->HasAttribute(sz)) {
      ...
      if (tags) {
        ...
        for (int t = 0; t < names.Count(); t++) {
          event->AddAbsoluteTag((CChoreoEvent::AbsTagType)tagtype, names[t],
                                percentages[t]);
        }
      }
    }
  ...
  case CChoreoEvent::SPEAK: {
    ...
    if (speakProperties) {
      event->SetCloseCaptionType(CChoreoEvent::CCTypeForName(
          speakProperties->GetValueString("closedcaption_type")));
      event->SetCloseCaptionToken(
          speakProperties->GetValueString("closedcaption_token"));
      event->SetUsingCombinedFile(
          speakProperties->GetValue<bool>("closedcaption_usingcombinedfile"));
      event->SetCombinedUsingGenderToken(
          speakProperties->GetValue<bool>("closedcaption_combinedusesgender"));
      event->SetSuppressingCaptionAttenuation(
          speakProperties->GetValue<bool>("closedcaption_noattenuate"));
    }
  } break;
  case CChoreoEvent::GESTURE: {
    ...
    if (gestureProperties) {
      if (Q_stricmp(clip->GetName(), "NULL")) {
        event->SetGestureSequenceDuration(
            gestureProperties->GetValue<float>("sequenceduration"));
      }
    }
  } break;
  case CChoreoEvent::FLEXANIMATION: {
    ...
    if (array.IsValid()) {
      ...
      for (int i = 0; i < numTracks; ++i) {
        ...
        if (log) {
          ...
          for (int j = 0; j < sampleCount; ++j) {
            ...
            CExpressionSample *sample =
                track->AddSample(time.GetSeconds(), value, 0);
        ...
        if (track->IsComboType()) {
          ...
          CDmeChannel *balanceChannel =
              FindAttributeInArray<CDmeChannel>(array, balanceChannelName);
          if (balanceChannel) {
            CDmeTypedLog<float> *balance =
                static_cast<CDmeTypedLog<float> *>(balanceChannel->GetLog());
            if (balance) {
              ...
              for (int j = 0; j < sampleCount; ++j) {
                ...
                CExpressionSample *sample =
                    track->AddSample(time.GetSeconds(), value, 1);
                sample->SetCurveType(curveType);
              }
            } else {
              char msg[512];
              Q_snprintf(msg, sizeof(msg),
                         "Error:  Missing balance channel for combo flex track "
                         "(%s) in (%s)\n",
...
bool ConvertDmxToScene(CDmeFilmClip *dmx, CChoreoScene *scene) {
  ...
  CDmeTrackGroup *pTrackGroup =
      dmx->FindTrackGroup(VCD_SCENE_RAMP_TRACK_GROUP_NAME);
  if (pTrackGroup) {
    CDmeTrack *track = pTrackGroup->FindTrack(VCD_SCENE_RAMP_TRACK_GROUP_NAME);
    if (track) {
      CDmeChannelsClip *pClip = CastElement<CDmeChannelsClip>(
          track->FindNamedClip(VCD_SCENE_RAMP_TRACK_GROUP_NAME));
      if (pClip) {
        ...
        if (array.IsValid() && (array.Count() > 0)) {
          ...
          if (channel) {
            ...
            if (ramp) {
              ...
              for (int j = 0; j < rampCount; ++j) {
                CExpressionSample *sample =
                    scene->AddSceneRamp(ramp->GetKeyTime(j).GetSeconds(),
                                        ramp->GetKeyValue(j), false);
                sample->SetCurveType(ramp->GetKeyCurveType(j));
              }
            }
          }
  ...
  if (pTrackGroup) {
    CDmeTrack *pTrack =
        pTrackGroup->FindTrack(VCD_GLOBAL_EVENTS_TRACK_GROUP_NAME);
    if (pTrack) {
      // Now add global events
      int numEvents = pTrack->GetClipCount();
      for (int event = 0; event < numEvents; ++event) {
        CDmeChannelsClip *pClip =
            CastElement<CDmeChannelsClip>(pTrack->GetClip(event));
        if (!pClip)
          continue;

        bool success =
            ConvertDmxToEvent(scene, pTrackGroup, pTrack, pClip, true);
        if (!success) {
          bret = false;
          Assert(0);
          break;
  ...
  for (int i = 0; i < nNumTrackGroups; ++i) {
    ...
    for (int j = 0; j < nNumTracks; ++j) {
      ...
      for (int clip = 0; clip < numEvents; ++clip) {
        CDmeChannelsClip *pClip =
            CastElement<CDmeChannelsClip>(pTrack->GetClip(clip));
        if (!pClip)
          continue;

        bool success =
            ConvertDmxToEvent(scene, pTrackGroup, pTrack, pClip, false);
        if (!success) {
          bret = false;
          Assert(0);
          break;
