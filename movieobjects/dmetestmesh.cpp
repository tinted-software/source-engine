...
void CDmeTestMesh::OnConstruction() {
  ...
  m_pControlCage = NULL;
  SetValue("transform",
           g_pDataModel->IsUnserializing()
               ? NULL
               : CreateElement<CDmeTransform>("transform", GetFileId()));
...
void CDmeTestMesh::CreateMesh() {
  ...
  m_pMaterial = g_pMaterialSystem->FindMaterial("shadertest/vertextexturetest",
                                                NULL, false);
  ...
  static Vector s_pPositions[8] = {
      Vector(-10, -10, -10), Vector(10, -10, -10), Vector(-10, 10, -10),
      Vector(10, 10, -10),   Vector(-10, -10, 10), Vector(10, -10, 10),
      Vector(-10, 10, 10),   Vector(10, 10, 10),
  };

  static Vector2D s_pTexCoords[8] = {
      Vector2D(0, 0),     Vector2D(0.5, 0),   Vector2D(0, 0.5),
      Vector2D(0.5, 0.5), Vector2D(0.5, 0.5), Vector2D(1, 0.5),
      Vector2D(0.5, 1),   Vector2D(1, 1),
  };
...
// that make up the face. A face is assumed to traverse its vertices in CCW
// order. We define the "relative edge index" for an edge within a face as the
...
// a right-handed, or CCW direction. Face 0 associated with the edge, to
// maintain a CCW ordering, must traverse the edge in a *reverse* direction,
// heading from vertex 1 to vertex 0. Face 1 associated with the edge traverses
// the edge in a forward direction, from vertex 0 to vertex 1.
//
...
// per face, a new vertex per edge, and adjust all existing vertices. When
// creating these vertices in the subdivided mesh, we first add the face
// midpoint vertices, then the edge midpoint vertices, then the vertices from
// the un-subdivided mesh, to the m_Vertices array of the subdivided mesh.
//
// Edge subdivision always works in a uniform way: For each edge in the
// unsubdivided mesh, 4 edges are created from the edge midpoint, connecting to
// the two face midpoint vertices and the two edge endpoints. In order to
// maintain the specific ordering of the edges described above, we define the
// edges in the following manner:
//		* Subdivided edge 0 : Starts at face 0 midpoint, ends at edge
//midpoint
//		* Subdivided edge 1 : Starts at edge midpoint, ends at face 1
//midpoint
//		* Subdivided edge 2 : Starts at original edge's vertex 0, ends
//at edge midpoint
//		* Subdivided edge 3 : Starts at edge midpoint, ends at original
//edge's vertex 1
//
// Face subdivision *also* always works in a uniform way: For each face in the
// unsubdivided mesh, N new faces are created, one for each edge in the
// unsubdivided face. The faces are ordered in a very specific way:
//	    * Subdivided face 0 : Starts at the face midpoint, goes to
//unsubdivided edge 0's midpoint, 								winds around the edge until it hits
//unsubdivided edge 1's midpoint, 								then heads back to the face midpoint.
//	    * Subdivided face 1 : Starts at the face midpoint, goes to
//unsubdivided edge 1's midpoint, 								winds around the edge until it hits
//unsubdivided edge 2's midpoint, 								then heads back to the face midpoint. 		etc.
//-----------------------------------------------------------------------------
...
// NOTE: The edge is always defined such that the edge going from vertex[0] to
// vertex[1] is counter-clockwise when seen from face[1] and and clockwise when
// seen from face[0].
struct Edge_t {
  int m_pFace[2];
  int m_pRelativeEdgeIndex[2]; // Goes from 0-N always, specifies the Nth edge
                               // of the polygon it's part of for each of the
                               // two faces
  int m_pVertex[2];
};
...
struct Face_t {
  ...
  // isn't actually a part of the mesh data, but I'm storing it here to reduce
...
struct SubdivMesh_t {
  ...
  // Positive values mean read from m_Edges[x], use m_pVertex[0] for leading
  // vertex Negative values mean read from m_Edges[-1-x], use m_pVertex[1] for
  // leading vertex
...
//-----------------------------------------------------------------------------
static inline int GetLeadingEdgeVertexIndex(const SubdivMesh_t &src,
                                            int nEdge) {
  if (nEdge >= 0) {
...
static inline const SubdivVertex_t &
GetLeadingEdgeVertex(const SubdivMesh_t &src, int nEdge) {
...
//-----------------------------------------------------------------------------
static void AddFaceMidpointsToMesh(const SubdivMesh_t &src,
                                   SubdivMesh_t &dest) {
  ...
  for (int i = 0; i < nSrcFaceCount; ++i) {
    ...
    // Store off the face index in the dest mesh of the first subdivided face
    // for this guy.
...
//-----------------------------------------------------------------------------
static void AddEdgeMidpointsToMesh(const SubdivMesh_t &src,
                                   SubdivMesh_t &dest) {
  ...
  for (int i = 0; i < nSrcEdgeCount; ++i) {
    ...
    // NOTE: We know that the first n vertices added to dest correspond to the
    // src face midpoints
...
// Adds edge midpoints to a mesh
//-----------------------------------------------------------------------------
static void AddModifiedVerticesToMesh(const SubdivMesh_t &src,
                                      SubdivMesh_t &dest) {
  int nSrcVertexCount = src.m_Vertices.Count();

  // This computes the equation v(i+1) = ((N-2)/N) * v(i) + (1/N^2) * sum( ei +
  // fi )
  ...
  for (int i = 0; i < nSrcEdgeCount; ++i) {
    ...
    for (int j = 0; j < 2; ++j) {
      ...
      // This adds in the contribution from the source vertex at the opposite
      // edge
      const SubdivVertex_t &srcOtherVert =
          src.m_Vertices[edge.m_pVertex[1 - j]];
      VectorMA(destVertex.m_vecPosition, ooValenceSq,
               srcOtherVert.m_vecPosition, destVertex.m_vecPosition);
      VectorMA(destVertex.m_vecTexCoord, ooValenceSq,
               srcOtherVert.m_vecTexCoord, destVertex.m_vecTexCoord);
      ...
      VectorMA(destVertex.m_vecPosition, 0.5f * ooValenceSq,
               pSrcFace->m_vecPosition, destVertex.m_vecPosition);
      VectorMA(destVertex.m_vecTexCoord, 0.5f * ooValenceSq,
               pSrcFace->m_vecTexCoord, destVertex.m_vecTexCoord);
...
static void AddSubdividedEdges(const SubdivMesh_t &src, SubdivMesh_t &dest) {
  ...
  // The vertex index for the vert at the center of original edge N is
  // nSrcFaceCount + N; The vertex index for the vert at original vertex N is
  // nSrcFaceCount + nSrcEdgeCount + N;
  int nSrcFaceCount = src.m_Faces.Count();
  int nSrcEdgeCount = src.m_Edges.Count();
  ...
  for (int i = 0; i < nSrcEdgeCount; ++i) {
    ...
    pDstEdge[0].m_pFace[0] =
        pSubdividedFaceIndex[0] + pPrevRelativeEdgeIndex[0];
...
static void AddSubdividedFaces(const SubdivMesh_t &src, SubdivMesh_t &dest) {
  ...
  for (int i = 0; i < nSrcFaceCount; ++i) {
    int nEdgeCount = src.m_Faces[i].m_nEdgeCount;
    const int *pSrcEdgeIndex =
        &src.m_EdgeIndices[src.m_Faces[i].m_nFirstEdgeIndex];
    ...
    for (int j = 0; j < nEdgeCount; ++j) {
      ...
      // Now add in the edge indices to refer to the edges created in
      // AddSubdividedEdges. Note that the new edge index == the old edge index
      // * 4, since we always create 4 edges for every edge in the source list.
      int *pCurrDestEdgeIndex = &pDestEdgeIndex[j * 4];
      int nSrcEdgeIndex = pSrcEdgeIndex[j];
      if (nSrcEdgeIndex >= 0) {
        // This means this polygon is the '1' index in the edge; it's following
        // this edge CCW.
        int nDestEdgeIndex = nSrcEdgeIndex * 4;
        pCurrDestEdgeIndex[0] =
            -1 - (nDestEdgeIndex + 1); // We're following this edge backwards
        pCurrDestEdgeIndex[1] = nDestEdgeIndex + 3;
        pPrevDestEdgeIndex[2] = nDestEdgeIndex + 2;
        pPrevDestEdgeIndex[3] = nDestEdgeIndex + 1;
      } else {
        // This means this polygon is the '0' index in the edge; it's following
        // this edge CW.
        int nDestEdgeIndex = (-1 - nSrcEdgeIndex) * 4;
        pCurrDestEdgeIndex[0] = nDestEdgeIndex;
        pCurrDestEdgeIndex[1] =
            -1 - (nDestEdgeIndex + 2); // We're following this edge backwards
        pPrevDestEdgeIndex[2] =
            -1 - (nDestEdgeIndex + 3); // We're following this edge backwards
        pPrevDestEdgeIndex[3] =
            -1 - (nDestEdgeIndex); // We're following this edge backwards
      }
...
static void SubdivideMesh(const SubdivMesh_t &src, SubdivMesh_t &dest) {
  ...
  int nSrcFaceCount = src.m_Faces.Count();
  int nSrcEdgeCount = src.m_Edges.Count();
  dest.m_Vertices.EnsureCapacity(nSrcFaceCount + nSrcEdgeCount +
                                 src.m_Vertices.Count());
  dest.m_Edges.EnsureCapacity(nSrcEdgeCount * 4);
  dest.m_EdgeIndices.EnsureCapacity(nSrcFaceCount * 16);
  dest.m_Faces.EnsureCapacity(
      nSrcFaceCount * 4); // This is only true if we have valence 4 everywhere.

  // First, compute midpoints of each face, add them to the mesh
  AddFaceMidpointsToMesh(src, dest);

  // Next, for each edge, compute a new point which is the average of the edge
  // points and the face midpoints
  AddEdgeMidpointsToMesh(src, dest);

  // Add modified versions of the vertices in the src mesh based on the new
  // computed points and add them to the dest mesh
...
void CDmeTestMesh::CreateControlCage() {
  ...
  // Draw a simple cube
  static Vector s_pPositions[8] = {
      Vector(-30, -30, -30), Vector(30, -30, -30), Vector(-30, 30, -30),
      Vector(30, 30, -30),   Vector(-30, -30, 30), Vector(30, -30, 30),
  ...
  // In what order does edge s_pEdges[i] appear on faces s_pEdgeFaces[i][0] and
  // s_pEdgeFaces[i][1] in the list s_pIndices[s_pEdgeFaces[i][j]] below? Note
  // the #s 0, 1, 2, and 3 should appear 6 times each in this array representing
  // the fact that each face has a 0th,1st,2nd, and 3rd edge.
  ...
  for (i = 0; i < 6; ++i) {
    ...
    for (int j = 0; j < 4; ++j) {
      int k;
      for (k = 0; k < 12; ++k) {
        if ((s_pIndices[i][j] == s_pEdges[k][0]) &&
            (s_pIndices[i][j + 1] == s_pEdges[k][1])) {
          m_pControlCage->m_EdgeIndices.AddToTail(k);
          break;
        }
        if ((s_pIndices[i][j] == s_pEdges[k][1]) &&
            (s_pIndices[i][j + 1] == s_pEdges[k][0])) {
...
void CDmeTestMesh::DrawSubdivMesh(const SubdivMesh_t &mesh) {
  ...
  IMaterial *pMaterial =
      g_pMaterialSystem->FindMaterial("debug/debugwireframe", NULL, false);
  ...
  //	meshBuilder.Begin( pMesh, MATERIAL_TRIANGLES, nVertexCount,
  //mesh.m_nTotalIndexCount );
  meshBuilder.Begin(pMesh, MATERIAL_LINES, nVertexCount,
                    mesh.m_nTotalLineCount * 2);
  ...
  for (int i = 0; i < nFaceCount; ++i) {
    ...
    const int *pEdgeIndex =
        &mesh.m_EdgeIndices[mesh.m_Faces[i].m_nFirstEdgeIndex];
    int nPrevIndex =
        GetLeadingEdgeVertexIndex(mesh, pEdgeIndex[nEdgeCount - 1]);
    for (int j = 0; j < nEdgeCount; ++j) {
  ...
  {
          int nEdgeCount = mesh.m_Faces[i].m_nEdgeCount;
      const int *pEdgeIndex = &mesh.m_EdgeIndices[
  mesh.m_Faces[i].m_nFirstEdgeIndex ]; int nRootIndex =
  GetLeadingEdgeVertexIndex( mesh, pEdgeIndex[0] ); int nPrevIndex =
  GetLeadingEdgeVertexIndex( mesh, pEdgeIndex[1] ); for ( int j = 0; j <
  nEdgeCount - 2; ++j )
          {
                  int nCurrIndex = GetLeadingEdgeVertexIndex( mesh,
  pEdgeIndex[j+2] ); meshBuilder.FastIndex( nRootIndex ); meshBuilder.FastIndex(
  nPrevIndex ); meshBuilder.FastIndex( nCurrIndex ); nPrevIndex = nCurrIndex;
          }
  }
  */
...
void CDmeTestMesh::DrawBox(CDmeTransform *pTransform) {
  ...
  for (int i = 0; i < 32; ++i) {
    pMorphFactors[i] =
        0.5f +
        0.5f * sin(2 * 3.14 * (Plat_FloatTime() / 5.0f + (float)i / 32.0f));
  }
...
// Draws the mesh
//-----------------------------------------------------------------------------
void CDmeTestMesh::Draw(const matrix3x4_t &shapeToWorld,
...
//	DrawSubdividedBox( pTransform );
  ...
  g_pStudioRender->DrawModel(NULL, info, pBoneToWorld, vecModelOrigin,
  ...
  IMaterial *pMaterial = g_pMaterialSystem->FindMaterial(
      "Models/shadertest/unlitgenericmodel", NULL, false);
  //	IMaterial *pMaterial = g_pMaterialSystem->FindMaterial(
  //"debug/debugwireframevertexcolor", NULL, false ); 	IMaterial *pMaterial =
  //g_pMaterialSystem->FindMaterial( "debug/debugwireframe", NULL, false );
  pRenderContext->Bind(pMaterial);
  ...
  for (int mi = 0; mi < mn; ++mi) {
    ...
    meshBuilder.Begin(pMesh, MATERIAL_TRIANGLES, vertices.size(),
                      indices.size());
...
void CDmeTestMesh::SetUpBones(CDmeTransform *pTransform, int nMaxBoneCount,
                              matrix3x4_t *pBoneToWorld) {
  ...
  //	CalcAutoplaySequences( pStudioHdr, NULL, pos, q, pPoseParameter,
  //BoneMask( ), flTime );
  ...
  for (int i = 0; i < nMaxBoneCount; i++) {
    ...
    if (studioHdr.pBone(i)->parent == -1) {
      ...
      ConcatTransforms(pBoneToWorld[studioHdr.pBone(i)->parent], boneMatrix,
                       pBoneToWorld[i]);
    }
  }
}
...
//
// 'nodes'						// bone naming and
// hierarchy #bone "$bonename" #parent	// one of these per bone - can be in any
// order, but generally sequential 'end'
//
// 'skeleton'					// joint animation (and begin
// pose) 'time' #time					// repeat time + joints
// block once per frame #bone .x .y .z .rx .ry .rz	//
// bone/translation/rotation - can traverse bones in any order, and even skip
// them 'end'
//
// 'triangles'					// actual vertex data - as
// non-indexed triangle lists $texturefilename				//
// repeat texture + 3 vertex lines for each triangle #bone .x .y .z .nx .ny .nz
// .tu .tv #count #bone0 .weight0	// boneN & weightN may or may not exist
// for N={0..511} #bone .x .y .z .nx .ny .nz .tu .tv #count #bone0 .weight0
// // boneN & weightN may or may not exist for N={0..511} #bone .x .y .z .nx .ny
// .nz .tu .tv #count #bone0 .weight0	// boneN & weightN may or may not exist
// for N={0..511} 'end'
//
// 'vertexanimation'			// morph targets
// 'time' #time					// repeat time + vertices block
// once per vertex #vertex .x .y .z .nx .ny .nz	// vertex/position/normal 'end'
//
//-----------------------------------------------------------------------------

// TODO - check out lookup_index for whether it's looking for exact vertex
// matches, or within a float tolerance DONE - lookup_index checks materiaks,
// coords and texcoords for exact match, and normals for within 2 degrees
...
void ReadBonesFromSMD(std::vector<CDmeTransform *> &bones, std::istream &is,
                      DmFileId_t fileid) {
  uint index;
  int parent;
  char name[MAXNAME];
      ...
      if (parent > 0) {
        if ((uint(parent) >= bones.size()) || (bones[parent] == NULL)) {
          ...
          Assert(0); // this code is so badly bit-rotten...
                     //					bones[parent]->AddChild(
                     //bones[index]->GetHandle() );
        }
      }
    } else {
...
void ReadSkeletalAnimationFromSMD(std::vector<CDmeTransform *> &bones,
                                  std::istream &is) {
  char line[MAXLINE];
...
//				ConcatTransforms( rootToWorld, boneMatrix, newMatrix );
      ...
      if (strcmp(cmd, "time") == 0) {
        ...
        if ( time != anim.size() )
        {
                Warning( "ReadSkeletalAnimationFromSMD: reading keyframe %d out
        of order\n", time );
        }
        if ( time >= anim.size() )
        {
        ...
        if ( time > 0 )
        {
                if ( anim[time-1] )
                {
                        std::copy( anim[time-1], anim[time-1] + nodes.size(),
        anim[time] );
                }
                else
                {
                        Warning( "ReadSkeletalAnimationFromSMD: missing skeletal
        keyframe %d\n", time-1 );
                }
        }
        */
      } else if (strcmp(cmd, "end") == 0) {
        //				Build_Reference( nodes, anim, matrices
        //); // skip - leave this for dmemesh generation
        return;
      } else {
        Warning("ReadSkeletalAnimationFromSMD: expected bone, time or end, "
                "found %s\n",
                line);
      }
    } else {
      Warning("ReadSkeletalAnimationFromSMD: expected bone, time or end, found "
              "%s\n",
              line);
    }
  }
...
void SortAndBalanceBones(std::vector<skinning_info_t> &skinning) {
  // TODO - studiomdl collapses (sums) duplicate bone weights - is this
  // necessary?!?!
...
int ReadVertexFromSMD(std::vector<vertex_t> &vertices, int numbones,
                      std::istream &is) {
  int boneIndex;
  is >> boneIndex;
  ...
  std::vector<vertex_t>::iterator vi =
      std::find(vertices.begin(), vertices.end(), vert);
  if (vi != vertices.end())
...
void ReadTrianglesFromSMD(std::vector<submesh_t *> &meshes, int numbones,
                          std::istream &is) {
  ...
  while (is.getline(line, MAXLINE)) {
    ...
    // the studiomdl comment here is "strip off trailing smag" whatever smag
    // is...
    ...
    if (texname[0] == '\0' || stricmp(texname, "null.bmp") == 0 ||
...
void RemapBonesOnSubmesh(submesh_t *pMesh,
...
CDmeTestMesh *CDmeTestMesh::ReadMeshFromSMD(char *pFilename,
                                            DmFileId_t fileid) {
