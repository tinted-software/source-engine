...
//-----------------------------------------------------------------------------
void CDmeCamera::SetupRenderState(
    int nDisplayWidth, int nDisplayHeight,
    bool bUseEngineCoordinateSystem /* = false */) {
...
//-----------------------------------------------------------------------------
void CDmeCamera::GetViewMatrix(VMatrix &view,
                               bool bUseEngineCoordinateSystem /* = false */) {
...
void CDmeCamera::GetProjectionMatrix(VMatrix &proj, int width, int height) {
  ...
  //	MatrixBuildPerspective( proj, flFOV, flFOV * flApsectRatio, flZNear,
  //flZFar );
...
void CDmeCamera::GetViewProjectionInverse(VMatrix &viewprojinv, int width,
                                          int height) {
  VMatrix view, proj;
  GetViewMatrix(view);
...
//-----------------------------------------------------------------------------
void CDmeCamera::ComputeScreenSpacePosition(const Vector &vecWorldPosition,
