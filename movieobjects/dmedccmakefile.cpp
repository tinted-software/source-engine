...
IMPLEMENT_ELEMENT_FACTORY(DmeSourceMayaAnimationFile,
                          CDmeSourceMayaAnimationFile);
...
void CDmeSourceMayaModelFile::OnConstruction() { m_ExportType = 0; }

void CDmeSourceMayaModelFile::OnDestruction() {}

void CDmeSourceMayaAnimationFile::OnConstruction() { m_ExportType = 1; }
...
// Purpose:
//-----------------------------------------------------------------------------
void CDmeDCCMakefile::OnConstruction() { m_bFlushFile = false; }
...
void CDmeDCCMakefile::GetOutputs(CUtlVector<CUtlString> &fullPaths) {
  ...
  // FIXME: We need to come up with an appropriate directory structure for
  // export
  ...
  Q_snprintf(pFullPath, sizeof(pFullPath), "%s\\%s.dmx", pOutputDir,
             pOutputName);
...
CDmElement *CDmeDCCMakefile::CreateOutputElement() {
  if (m_bFlushFile) {
    ...
    if (GetFileId() != DMFILEID_INVALID) {
      // NOTE: CDmeHandles will correctly re-hook up to the new makefile after
      // load If the file fails to load, we have the copy. If the file correctly
      // has the make in it it will replace this copy I made
      ...
      if (g_pDataModel->RestoreFromFile(g_pDataModel->GetFileName(fileId), NULL,
                                        NULL, &pRoot,
                                        CR_DELETE_OLD) != DMFILEID_INVALID) {
      ...
      // NOTE: We expect file backup prior to compile to avoid really fatal
      // errors This case happens if the file failed to load. In this case, we
      // must use the copy of the makefile
      Assert(0);
      return NULL;
    }
  }
...
static DmeMakefileType_t s_pMayaModelSourceTypes[] = {
    {"DmeSourceMayaModelFile", "Maya Model File", true, "makefiledir:../maya",
     "*.ma;*.mb", "Maya File (*.ma,*.mb)"},
...
static DmeMakefileType_t s_pMayaAnimationSourceTypes[] = {
    {"DmeSourceMayaAnimationFile", "Maya Animation File", true,
     "makefiledir:../maya", "*.ma;*.mb", "Maya File (*.ma,*.mb)"},
    {NULL, NULL, false, NULL, NULL, NULL},
};
...
// Makefile type
//-----------------------------------------------------------------------------
static DmeMakefileType_t s_MayaModelMakefileType = {"DmeMayaModelMakefile",
                                                    "Maya Model Component",
                                                    true,
                                                    "contentdir:models",
                                                    "*.dmx",
                                                    "DMX File (*.dmx)"};
...
static DmeMakefileType_t s_pXSISourceTypes[] = {
    {"DmeSourceXSIFile", "XSI File", true, "makefiledir:../xsi", "*.xsi",
     "XSI File (*.xsi)"},
    {NULL, NULL, false, NULL, NULL, NULL},
};
...
static DmeMakefileType_t s_XSIMakefileType = {
    "DmeXSIMakefile", "XSI Model Component", true, "contentdir:models",
    "*.dmx",          "DMX File (*.dmx)",
};
