...
bool CDmeUnpackColorOperator::IsDirty() {
  ...
  //	float s = 255.999f;
  //	return c.r() != s*m_red.Get() || c.g() != s*m_green.Get() || c.b() !=
  //s*m_blue.Get() || c.a() != s*m_alpha.Get();
  return c.r() != m_red.Get() || c.g() != m_green.Get() ||
         c.b() != m_blue.Get() || c.a() != m_alpha.Get();
}

void CDmeUnpackColorOperator::Operate() {
  //	float s = 255.999f;
  //	m_color.Set( Color( s*m_red.Get(), s*m_green.Get(), s*m_blue.Get(),
  //s*m_alpha.Get() ) );
...
void CDmeUnpackColorOperator::GetInputAttributes(
    CUtlVector<CDmAttribute *> &attrs) {
  attrs.AddToTail(m_color.GetAttribute());
}

void CDmeUnpackColorOperator::GetOutputAttributes(
    CUtlVector<CDmAttribute *> &attrs) {
...
void CDmeUnpackVector2Operator::GetInputAttributes(
    CUtlVector<CDmAttribute *> &attrs) {
  attrs.AddToTail(m_vector.GetAttribute());
}

void CDmeUnpackVector2Operator::GetOutputAttributes(
    CUtlVector<CDmAttribute *> &attrs) {
...
void CDmeUnpackVector3Operator::GetInputAttributes(
    CUtlVector<CDmAttribute *> &attrs) {
  attrs.AddToTail(m_vector.GetAttribute());
}

void CDmeUnpackVector3Operator::GetOutputAttributes(
    CUtlVector<CDmAttribute *> &attrs) {
  attrs.AddToTail(m_x.GetAttribute());
  attrs.AddToTail(m_y.GetAttribute());
...
void CDmeUnpackVector4Operator::GetInputAttributes(
    CUtlVector<CDmAttribute *> &attrs) {
  attrs.AddToTail(m_vector.GetAttribute());
}

void CDmeUnpackVector4Operator::GetOutputAttributes(
    CUtlVector<CDmAttribute *> &attrs) {
  attrs.AddToTail(m_x.GetAttribute());
  attrs.AddToTail(m_y.GetAttribute());
  attrs.AddToTail(m_z.GetAttribute());
...
void CDmeUnpackQAngleOperator::GetInputAttributes(
    CUtlVector<CDmAttribute *> &attrs) {
  attrs.AddToTail(m_qangle.GetAttribute());
}

void CDmeUnpackQAngleOperator::GetOutputAttributes(
    CUtlVector<CDmAttribute *> &attrs) {
  attrs.AddToTail(m_x.GetAttribute());
  attrs.AddToTail(m_y.GetAttribute());
  attrs.AddToTail(m_z.GetAttribute());
...
//-----------------------------------------------------------------------------
IMPLEMENT_ELEMENT_FACTORY(DmeUnpackQuaternionOperator,
                          CDmeUnpackQuaternionOperator);
...
void CDmeUnpackQuaternionOperator::GetInputAttributes(
    CUtlVector<CDmAttribute *> &attrs) {
  attrs.AddToTail(m_quaternion.GetAttribute());
}

void CDmeUnpackQuaternionOperator::GetOutputAttributes(
    CUtlVector<CDmAttribute *> &attrs) {
  attrs.AddToTail(m_x.GetAttribute());
  attrs.AddToTail(m_y.GetAttribute());
  attrs.AddToTail(m_z.GetAttribute());
...
void CDmeUnpackVMatrixOperator::GetInputAttributes(
    CUtlVector<CDmAttribute *> &attrs) {
  attrs.AddToTail(m_vmatrix.GetAttribute());
}

void CDmeUnpackVMatrixOperator::GetOutputAttributes(
    CUtlVector<CDmAttribute *> &attrs) {
  for (uint i = 0; i < 16; ++i) {
