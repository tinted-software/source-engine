...
//-----------------------------------------------------------------------------
void CDmeSource::OnConstruction() { m_DependentMakefile = NULL; }
...
void CDmeMakefile::PreCompile() {
  ...
  // NOTE: Don't check for m_hOutput == 0; we always need to call
  // DestroyOutputElement Sometimes makefiles have to do stuff even if m_hOutput
  // == NULL
...
void CDmeMakefile::GetMakefilePath(char *pFullPath, int nBufLen) {
  ...
  const char *pFileName =
      (fileId != DMFILEID_INVALID) ? g_pDataModel->GetFileName(fileId) : "";
...
bool CDmeMakefile::GetOutputName(char *pFullPath, int nBufLen) {
  ...
  if (!GetDefaultDirectory(GetOutputDirectoryID(), pOutputPath,
                           sizeof(pOutputPath)))
    return false;
  ...
  Q_ComposeFileName(pOutputPath, Q_UnqualifiedFileName(pFileName), pFullPath,
                    nBufLen);
...
//-----------------------------------------------------------------------------
// Converts the m_pDefaultDirectoryID field of the DmeMakefileType_t to a full
// path
//-----------------------------------------------------------------------------
bool CDmeMakefile::GetDefaultDirectory(const char *pDefaultDirectoryID,
                                       char *pFullPath, int nBufLen) {
  ...
  if (StringHasPrefix(pDefaultDirectoryID, "makefilegamedir:")) {
    ...
    GetModContentSubdirectory(NULL, pModContentDirectory,
                              sizeof(pModContentDirectory));

    char pRelativePath[MAX_PATH];
    if (!Q_MakeRelativePath(pMakefilePath, pModContentDirectory, pRelativePath,
    ...
    Q_ComposeFileName(pModDirectory, pRelativePath, pMakefileGamePath,
                      sizeof(pMakefileGamePath));

    pDefaultDirectoryID += 16;
    Q_ComposeFileName(pMakefileGamePath, pDefaultDirectoryID, pFullPath,
                      nBufLen);
    Q_RemoveDotSlashes(pFullPath);
    return true;
  }
...
//-----------------------------------------------------------------------------
void CDmeMakefile::RelativePathToFullPath(const char *pRelativePath,
                                          char *pFullPath, int nBufLen) {
  if (!pRelativePath[0]) {
    pFullPath[0] = 0;
    return;
...
//-----------------------------------------------------------------------------
void CDmeMakefile::FullPathToRelativePath(const char *pFullPath,
...
//-----------------------------------------------------------------------------
CDmeSource *CDmeMakefile::AddSource(const char *pSourceType,
  ...
  CDmElement *pElement = GetElement<CDmElement>(
      g_pDataModel->CreateElement(pSourceType, "", GetFileId()));
...
//-----------------------------------------------------------------------------
CDmeSource *CDmeMakefile::FindSource(const char *pSourceType,
                                     const char *pFullPath) {
  char pRelativePath[MAX_PATH];
  FullPathToRelativePath(pFullPath, pRelativePath, sizeof(pRelativePath));
...
//-----------------------------------------------------------------------------
CDmeSource *CDmeMakefile::SetSingleSource(const char *pSourceType,
                                          const char *pFullPath) {
  // FIXME: we maybe shouldn't remove everything if the source can't be created
  // for some reason?
...
// Changes a source
//-----------------------------------------------------------------------------
void CDmeMakefile::SetSourceFullPath(CDmeSource *pSource,
                                     const char *pFullPath) {
  char pRelativePath[MAX_PATH];
  FullPathToRelativePath(pFullPath, pRelativePath, sizeof(pRelativePath));
...
//-----------------------------------------------------------------------------
void CDmeMakefile::GetSourceFullPath(CDmeSource *pSource, char *pFullPath,
                                     int nBufLen) {
...
//-----------------------------------------------------------------------------
void CDmeMakefile::GetSources(const char *pSourceType,
...
//-----------------------------------------------------------------------------
int CDmeMakefile::GetSourceCount() { return m_Sources.Count(); }

CDmeSource *CDmeMakefile::GetSource(int nIndex) { return m_Sources[nIndex]; }
...
void CDmeMakefile::RemoveSource(const char *pSourceType,
                                const char *pFullPath) {
  char pRelativePath[MAX_PATH];
  FullPathToRelativePath(pFullPath, pRelativePath, sizeof(pRelativePath));
  int nCount = m_Sources.Count();
...
//-----------------------------------------------------------------------------
bool CDmeMakefile::UpdateSourceNames(const char *pOldRootDir,
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    if (pOldRelativePath[0]) {
      Q_ComposeFileName(pOldRootDir, pOldRelativePath, pOldSourcePath,
                        sizeof(pOldSourcePath));
      Q_RemoveDotSlashes(pOldSourcePath);
      if (!Q_MakeRelativePath(pOldSourcePath, pNewRootDir, pNewSourcePath,
                              sizeof(pNewSourcePath))) {
...
//-----------------------------------------------------------------------------
void CDmeMakefile::SetAssociation(CDmeSource *pSource,
