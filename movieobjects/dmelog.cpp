...
template <class T> T Average(const T *pValues, int nValues) {
...
// float version
template <> float Average(const float *pValues, int nValues) {
  Assert(nValues > 0);
  if (nValues <= 0)
    return 0.0f;
...
// Color version
template <> Color Average(const Color *pValues, int nValues) {
  Assert(nValues > 0);
  if (nValues <= 0)
...
// Vector2 version
template <> Vector2D Average(const Vector2D *pValues, int nValues) {
  Assert(nValues > 0);
  if (nValues <= 0)
...
// Vector3 version
template <> Vector Average(const Vector *pValues, int nValues) {
  Assert(nValues > 0);
  if (nValues <= 0)
...
// QAngle version
template <> QAngle Average(const QAngle *pValues, int nValues) {
  Assert(nValues > 0);
  if (nValues <= 0)
...
// Quaternion version
template <> Quaternion Average(const Quaternion *pValues, int nValues) {
  Assert(nValues > 0);
  if (nValues <= 0)
...
// catch-all for non-interpolable types - just holds first value
template <class T> T Interpolate(float t, const T &ti, const T &tj) {
  if (IsInterpolableType(CDmAttributeInfo<T>::AttributeType())) {
    static bool first = true;
    if (first) {
      first = false;
      Warning("CDmeLog: interpolable type %s doesn't have an interpolation "
              "function!",
              CDmAttributeInfo<T>::AttributeTypeName());
    }
  }
...
// float version
template <> float Interpolate(float t, const float &ti, const float &tj) {
  return t * tj + (1.0f - t) * ti;
}

// Color version
template <> Color Interpolate(float t, const Color &ti, const Color &tj) {
  int ri, gi, bi, ai;
  int rj, gj, bj, aj;
...
// Vector3 version
template <> Vector Interpolate(float t, const Vector &ti, const Vector &tj) {
  return t * tj + (1.0f - t) * ti;
}

// QAngle version
template <> QAngle Interpolate(float t, const QAngle &ti, const QAngle &tj) {
  QAngle qaResult;
...
template <class T> T ScaleValue(const T &value, float scale) {
  return value * scale;
}
template <> bool ScaleValue(const bool &value, float scale) {
  Assert(0);
  return value;
}
template <> Color ScaleValue(const Color &value, float scale) {
  Assert(0);
  return value;
}
template <> Vector4D ScaleValue(const Vector4D &value, float scale) {
  return Vector4D(value.x * scale, value.y * scale, value.z * scale,
                  value.w * scale);
}

template <> Quaternion ScaleValue(const Quaternion &value, float scale) {
  return Quaternion(value.x * scale, value.y * scale, value.z * scale,
                    value.w * scale);
}

template <> VMatrix ScaleValue(const VMatrix &value, float scale) {
  Assert(0);
  return value;
}

template <> CUtlString ScaleValue(const CUtlString &value, float scale) {
  Assert(0);
  return value;
}
...
template <> CUtlString Subtract(const CUtlString &v1, const CUtlString &v2) {
  return v1;
}

template <> Color Subtract(const Color &v1, const Color &v2) {
  Color ret;
  for (int i = 0; i < 4; ++i) {
...
template <> Vector4D Subtract(const Vector4D &v1, const Vector4D &v2) {
  Vector4D ret;
  for (int i = 0; i < 4; ++i) {
    ret[i] = v1[i] - v2[i];
  }
...
template <> Quaternion Subtract(const Quaternion &v1, const Quaternion &v2) {
  Quaternion ret;
  for (int i = 0; i < 4; ++i) {
    ret[i] = v1[i];
  }
...
template <> CUtlString Add(const CUtlString &v1, const CUtlString &v2) {
  return v1;
}

template <> Color Add(const Color &v1, const Color &v2) {
  Color ret;
  for (int i = 0; i < 4; ++i) {
    ret[i] = clamp(v1[i] + v2[i], 0, 255);
  }
...
template <> Vector4D Add(const Vector4D &v1, const Vector4D &v2) {
  Vector4D ret;
  for (int i = 0; i < 4; ++i) {
    ret[i] = v1[i] + v2[i];
  }
...
template <> Quaternion Add(const Quaternion &v1, const Quaternion &v2) {
  return v1;
}
...
bool CDmeLogLayer::ValidateKeys() const {
  ...
  for (int i = 1; i < nCount; ++i) {
    if (m_times[i] <= m_times[i - 1]) {
      Warning("Error in log %s! Key times are out of order [keys %d->%d: "
              "%d->%d]!\n",
...
template <class T> class CUndoLayerAdded : public CUndoElement {
...
template <class T> class CUndoFlattenLayers : public CUndoElement {
  typedef CUndoElement BaseClass;

public:
  CUndoFlattenLayers(const char *desc, CDmeTypedLog<T> *pLog, float threshold,
                     int flags)
      : BaseClass(desc), m_bNeedsCleanup(true), m_hLog(pLog), m_nFlags(flags),
...
//-----------------------------------------------------------------------------
template <class T> void CDmeTypedLogLayer<T>::OnConstruction() {
  //	m_times.Init( this, "times" );
  //	m_CurveTypes.Init( this, "curvetypes" );
  m_values.Init(this, "values");
}

template <class T> void CDmeTypedLogLayer<T>::SetOwnerLog(CDmeLog *owner) {
  Assert(owner);
  Assert(assert_cast<CDmeTypedLog<T> *>(owner));
  m_pOwnerLog = owner;
}

template <class T> CDmeTypedLog<T> *CDmeTypedLogLayer<T>::GetTypedOwnerLog() {
...
template <class T> void CDmeTypedLogLayer<T>::RemoveKeys(DmeTime_t starttime) {
...
template <class T> void CDmeTypedLogLayer<T>::ClearKeys() {
  m_times.RemoveAll();
  m_values.RemoveAll();
  m_CurveTypes.RemoveAll();
  m_lastKey = 0;
...
template <class T> void CDmeTypedLogLayer<T>::Filter(int nSampleRadius) {
...
template <class T> void CDmeTypedLogLayer<T>::Filter2(DmeTime_t sampleRadius) {
  // Doesn't mess with curvetypes!!!
...
template <class T> void CDmeTypedLog<T>::RemoveRedundantKeys(float threshold) {
...
template <class T> void CDmeTypedLogLayer<T>::RemoveRedundantKeys() {
...
template <class T> bool CDmeTypedLogLayer<T>::IsUsingEdgeInfo() const {
...
template <class T> DmeTime_t CDmeTypedLogLayer<T>::GetRightEdgeTime() const {
...
template <class T> int CDmeTypedLogLayer<T>::GetEdgeCurveType(int edge) const {
...
//-----------------------------------------------------------------------------
template <class T> CDmeLogLayer *CreateLayer<T>(CDmeTypedLog<T> *pOwnerLog) {
...
//
//-----------------------------------------------------------------------------
template <class T> void CDmeTypedCurveInfo<T>::OnConstruction() {
...
template <class T> void CDmeTypedCurveInfo<T>::SetUseEdgeInfo(bool state) {
  m_bUseEdgeInfo = state;
}

template <class T> bool CDmeTypedCurveInfo<T>::IsUsingEdgeInfo() const {
...
template <class T> int CDmeTypedCurveInfo<T>::GetEdgeCurveType(int edge) const {
  Assert(edge == 0 || edge == 1);
...
int CDmeLog::FindLayerForTimeSkippingTopmost(DmeTime_t time) const {
  int c =
      m_Layers.Count() - 1; // This makes it never consider the topmost layer!!!
  for (int i = c - 1; i >= 0; --i) {
    CDmeLogLayer *layer = m_Layers[i];
    DmeTime_t layerStart = layer->GetBeginTime();
    if (layerStart == DmeTime_t::MinTime())
...
//-----------------------------------------------------------------------------
template <class T> void CDmeTypedLog<T>::OnConstruction() {
  if (!g_pDataModel->IsUnserializing()) {
...
template <class T> void CDmeTypedLog<T>::SetDefaultValue(const T &value) {
  m_UseDefaultValue = true;
  m_DefaultValue.Set(value);
}

template <class T> const T &CDmeTypedLog<T>::GetDefaultValue() const {
  Assert((bool)m_UseDefaultValue);
  return m_DefaultValue;
}

template <class T> bool CDmeTypedLog<T>::HasDefaultValue() const {
  return m_UseDefaultValue;
}

template <class T> void CDmeTypedLog<T>::ClearDefaultValue() {
...
template <class T> void CDmeTypedLog<T>::AddLayerToTail(CDmeLogLayer *layer) {
  Assert(layer);
...
template <class T> CDmeLogLayer *CDmeTypedLog<T>::RemoveLayerFromTail() {
  Assert(m_Layers.Count() >= 1);
...
template <class T> CDmeLogLayer *CDmeTypedLog<T>::RemoveLayer(int iLayer) {
...
template <class T> CDmeLogLayer *CDmeTypedLog<T>::AddNewLayer() {
...
template <class T> CDmeTypedCurveInfo<T> *CDmeTypedLog<T>::GetTypedCurveInfo() {
  Assert(!m_CurveInfo.GetElement() ||
...
template <class T> CDmeCurveInfo *CDmeTypedLog<T>::GetOrCreateCurveInfo() {
...
template <class T> struct ActiveLayer_t {
  ActiveLayer_t()
...
template <class T> struct LayerEvent_t {
...
template <class T> static void SpewEvents(CUtlRBTree<LayerEvent_t<T>> &events) {
  for (unsigned short idx = events.FirstInorder(); idx != events.InvalidIndex();
    ...
    Msg("Event %u layer %i at time %i type %s\n", (unsigned)idx,
        pEvent->m_nLayer, pEvent->m_Time.GetTenthsOfMS(),
...
template <> inline void SpewKey<Vector2D>(const Vector2D &val) {
  Msg("%f,%f", val.x, val.y);
}

template <> inline void SpewKey<Vector4D>(const Vector4D &val) {
  Msg("%f,%f,%f,%f", val.x, val.y, val.z, val.w);
}

template <> inline void SpewKey<DmeTime_t>(const DmeTime_t &val) {
  Msg("%d", val.GetTenthsOfMS());
}

template <> inline void SpewKey<bool>(const bool &val) {
  Msg("%s", val ? "true" : "false");
}

template <> inline void SpewKey<Color>(const Color &val) {
  Msg("%08x", val.GetRawColor());
}

template <> inline void SpewKey(const Vector &val) {
  Msg("[%f %f %f]", val.x, val.y, val.z);
}

template <> inline void SpewKey(const Quaternion &val) {
  Msg("[%f %f %f %f]", val.x, val.y, val.z, val.w);
}
...
static void SpewFlattenedKey(CDmeTypedLogLayer<T> *pLogLayer,
  ...
  Msg("Layer %d:  adding key at time %f [%d -> %d], value ",
      pActiveLayer->priority, t.GetSeconds(),
      pActiveLayer->firstTime.GetTenthsOfMS(),
      pActiveLayer->lastTime.GetTenthsOfMS());
  SpewKey(val);
  Msg("\n");
}
...
static void AddDiscontinitySample(CDmeTypedLogLayer<T> *pTargetLayer,
  ...
  if (pLog->IsUsingCurveTypes()) {
    if (pSpewLabel) {
      Msg("Adding %s helper key at %d value ", pSpewLabel,
          tKeyTime.GetTenthsOfMS());
      SpewKey(val);
      Msg(" [curvetype %s]\n",
...
template <class T>
static DmeTime_t
ProcessStartLayerStartEvent(bool bSpew, bool bFixupDiscontinuities,
                            CDmeTypedLog<T> *pLog, LayerEvent_t<T> *pEvent,
                            CUtlVector<ActiveLayer_t<T>> &layerlist,
...
void CDmeTypedLog<T>::FlattenLayers(float threshold, int flags) {
  ...
  while (1) {
    ...
    case LayerEvent_t<T>::LE_END:
      iCurrentKeyTime =
          ProcessStartLayerEndEvent(bSpew, bFixupDiscontinuities, this, pEvent,
                                    layerlist, active, flattenedlayer);
      break;
    }
...
void CDmeTypedLog<T>::FinishTimeSelection(DmeTime_t tHeadPosition,
  ...
  int nBestLayer = GetTopmostLayer(); // Topmost should be at least layer # 1 (0
                                      // is the base layer)
  if (nBestLayer < 1)
    return;
...
//-----------------------------------------------------------------------------
template <class T> struct LogClampHelper_t {
public:
...
template <class T> class CLogFalloffBlend {
public:
...
template <class T> const T &CLogFalloffBlend<T>::GetDelta() const {
  return m_Delta;
}
...
// selection
//-----------------------------------------------------------------------------
static inline DmeTime_t
ComputeResampleStartTime(const DmeLog_TimeSelection_t &params, int nSide) {
...
template <class T> void CDmeTypedLog<T>::RemoveKeys(DmeTime_t starttime) {
  int bestLayer = GetTopmostLayer();
  if (bestLayer < 0)
    return;
...
template <class T> void CDmeTypedLog<T>::ClearKeys() {
  int bestLayer = GetTopmostLayer();
  if (bestLayer < 0)
    return;
...
// Returns a specific key's value
//-----------------------------------------------------------------------------
template <class T> DmeTime_t CDmeTypedLog<T>::GetKeyTime(int nKeyIndex) const {
  int bestLayer = GetTopmostLayer();
  if (bestLayer < 0)
    return DmeTime_t::MinTime();
...
template <class T> CDmeTypedLogLayer<T> *CDmeTypedLog<T>::GetLayer(int index) {
  if (index < 0)
    return NULL;
...
template <class T> const T &CDmeTypedLog<T>::GetValue(DmeTime_t time) const {
...
void CDmeTypedLog<T>::SetKey(DmeTime_t time, const CDmAttribute *pAttr,
                             uint index, int curveType /*= CURVE_DEFAULT*/) {
  int bestLayer = GetTopmostLayer();
  if (bestLayer == -1)
    return;
...
template <class T> bool CDmeTypedLog<T>::SetDuplicateKeyAtTime(DmeTime_t time) {
  int bestLayer = GetTopmostLayer();
  if (bestLayer == -1)
    return false;
...
// Returns a specific key's value
//-----------------------------------------------------------------------------
template <class T> const T &CDmeTypedLog<T>::GetKeyValue(int nKeyIndex) const {
  int bestLayer = GetTopmostLayer();
  if (bestLayer == -1) {
    static T s_value;
    CDmAttributeInfo<T>::SetDefaultValue(
...
// resampling and filtering
//-----------------------------------------------------------------------------
template <class T> void CDmeTypedLog<T>::Resample(DmeFramerate_t samplerate) {
  int c = m_Layers.Count();
  for (int i = 0; i < c; ++i) {
...
template <class T> void CDmeTypedLog<T>::Filter(int nSampleRadius) {
  int c = m_Layers.Count();
  for (int i = 0; i < c; ++i) {
...
template <class T> void CDmeTypedLog<T>::Filter2(DmeTime_t sampleRadius) {
  int c = m_Layers.Count();
  for (int i = 0; i < c; ++i) {
...
template <class T> void CDmeTypedLog<T>::SetUseEdgeInfo(bool state) {
  Assert(IsUsingCurveTypes());
  GetTypedCurveInfo()->SetUseEdgeInfo(state);
}

template <class T> bool CDmeTypedLog<T>::IsUsingEdgeInfo() const {
  Assert(IsUsingCurveTypes());
...
template <class T> void CDmeTypedLog<T>::SetDefaultEdgeZeroValue(const T &val) {
  Assert(IsUsingCurveTypes());
  GetTypedCurveInfo()->SetDefaultEdgeZeroValue(val);
}

template <class T> const T &CDmeTypedLog<T>::GetDefaultEdgeZeroValue() const {
  Assert(IsUsingCurveTypes());
  return GetTypedCurveInfo()->GetDefaultEdgeZeroValue();
}

template <class T> void CDmeTypedLog<T>::SetRightEdgeTime(DmeTime_t time) {
  Assert(IsUsingCurveTypes());
  GetTypedCurveInfo()->SetRightEdgeTime(time);
}

template <class T> DmeTime_t CDmeTypedLog<T>::GetRightEdgeTime() const {
  Assert(IsUsingCurveTypes());
...
template <class T> int CDmeTypedLog<T>::GetEdgeCurveType(int edge) const {
  Assert(IsUsingCurveTypes());
  return GetTypedCurveInfo()->GetEdgeCurveType(edge);
}

template <class T> void CDmeTypedLog<T>::GetZeroValue(int side, T &val) const {
  Assert(IsUsingCurveTypes());
  GetTypedCurveInfo()->GetZeroValue(side, val);
}

template <class T> bool CDmeTypedLog<T>::IsEdgeActive(int edge) const {
  Assert(IsUsingCurveTypes());
  return GetTypedCurveInfo()->IsEdgeActive(edge);
}

template <class T> void CDmeTypedLog<T>::GetEdgeValue(int edge, T &val) const {
  Assert(IsUsingCurveTypes());
...
void CDmeTypedLog<T>::RevealUsingTimeSelection(
  ...
  for (int t = params.m_nTimes[TS_LEFT_FALLOFF].GetTenthsOfMS();
    ...
    writeLayer->InsertKey(curtime, newValue,
                          IsUsingCurveTypes() ? GetDefaultCurveType()
                                              : CURVE_DEFAULT);
  }
  ...
  case FILTER_SMOOTH: {
    ...
    if (bResample) {
      for (t = params.m_nTimes[TS_LEFT_FALLOFF].GetTenthsOfMS();
        ...
        writeLayer->SetKey(curtime, curValue,
                           IsUsingCurveTypes() ? GetDefaultCurveType()
                                               : CURVE_DEFAULT);
      }
    } else {
      // Do a second pass where we bias the keys in the falloff area back toward
      // the original value
      int kc = baseLayer->GetKeyCount();
      for (int i = 0; i < kc; ++i) {
        DmeTime_t curtime = baseLayer->GetKeyTime(i);
  ...
  case FILTER_JITTER: {
    ...
    if (bResample) {
      ...
      for (t = params.m_nTimes[TS_LEFT_FALLOFF].GetTenthsOfMS();
        ...
        writeLayer->SetKey(curtime, newValue,
                           IsUsingCurveTypes() ? GetDefaultCurveType()
                                               : CURVE_DEFAULT);
      }
  ...
  case FILTER_SOFTEN: {
    ...
    for (int i = 0; i < kc; ++i) {
      ...
      if (frac != 1.0f) {
        ...
        if (curtime <= params.m_nTimes[TS_LEFT_HOLD]) {
          // Get the value at the crossing point (either green edge for sharpen,
          // or left edge for soften...)
          crossingValue[0] =
...
void CDmeTypedLog<T>::PasteAndRescaleSamples(
  ...
  CDmeTypedLogLayer<T> *pClipboard =
      CastElement<CDmeTypedLogLayer<T>>(const_cast<CDmeLogLayer *>(pDataLayer));
  ...
  for (int j = 0; j < nKeyCount; ++j) {
    ...
    if (bBlendAreaInFalloffRegion && (nState != PASTE_STATE_HOLD)) {
      ...
      flFactorBlend = destParams.AdjustFactorForInterpolatorType(flFactorBlend,
                                                                 nBlendIndex);
...
// DmeLog_TimeSelection_t& params, const bool& value ) { Assert( 0 ); }
// template<> void CDmeTypedLog< bool >::StampKeyAtHead( const
// DmeLog_TimeSelection_t& params, const CDmAttribute *pAttr, uint index /*= 0*/
