...
// Expose this class to the scene database
//-----------------------------------------------------------------------------
IMPLEMENT_ELEMENT_FACTORY(DmeSingleIndexedComponent,
...
//
//-----------------------------------------------------------------------------
void CDmeSingleIndexedComponent::AddComponent(int component,
                                              float weight /*= 1.0f */) {
  if (IsComplete())
    return;
...
void CDmeSingleIndexedComponent::AddComponents(
    const CUtlVector<int> &components) {
...
void CDmeSingleIndexedComponent::RemoveComponent(int component) {
  ...
  if (cIndex >= m_Components.Count() ||
      m_Components[cIndex] != component) // Component not in selection
    return;
...
//
//-----------------------------------------------------------------------------
bool CDmeSingleIndexedComponent::GetComponent(int index, int &component,
                                              float &weight) const {
  if (index < Count()) {
    if (IsComplete()) {
      component = index;
...
//
//-----------------------------------------------------------------------------
void CDmeSingleIndexedComponent::GetComponents(
    CUtlVector<int> &components, CUtlVector<float> &weights) const {
  if (IsComplete()) {
    const int nComponents = Count();

    int *pComponents =
        reinterpret_cast<int *>(alloca(nComponents * sizeof(int)));
    float *pWeights =
        reinterpret_cast<float *>(alloca(nComponents * sizeof(float)));
...
//-----------------------------------------------------------------------------
void CDmeSingleIndexedComponent::GetComponents(
    CUtlVector<int> &components) const {
  if (IsComplete()) {
    const int nComponents = Count();
...
//
//-----------------------------------------------------------------------------
void CDmeSingleIndexedComponent::Subtract(
    const CDmeSingleIndexedComponent &rhs) {
...
void CDmeSingleIndexedComponent::Add(const CDmeSingleIndexedComponent &rhs) {
  ...
  if (IsComplete()) {
    ...
    newComponents.EnsureCapacity(
        nLhs + nRhs * 0.5); // Just an estimate assuming 50% of the components
                            // in rhs aren't in lhs
    CUtlVector<float> newWeights;
...
//
//-----------------------------------------------------------------------------
void CDmeSingleIndexedComponent::Intersection(
    const CDmeSingleIndexedComponent &rhs) {
  const int nLhs = Count();
  const int nRhs = rhs.Count();
