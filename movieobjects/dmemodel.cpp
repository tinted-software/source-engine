...
void CDmeModel::CaptureJointsToBaseState(const char *pBaseStateName) {
  ...
  if (!pTransformList) {
    pTransformList =
        CreateElement<CDmeTransformList>(pBaseStateName, GetFileId());
  ...
  if (nJointCount > nCurrentCount) {
    for (int i = nCurrentCount; i < nJointCount; ++i) {
      CDmeTransform *pTransform = CreateElement<CDmeTransform>(
          m_JointTransforms[i]->GetName(), pTransformList->GetFileId());
      pTransformList->m_Transforms.AddToTail(pTransform);
    }
  } else if (nJointCount < nCurrentCount) {
    pTransformList->m_Transforms.RemoveMultiple(nJointCount,
                                                nCurrentCount - nJointCount);
  }
...
//-----------------------------------------------------------------------------
void CDmeModel::LoadJointTransform(CDmeDag *pJoint,
  ...
  // Determines joint transform index; no index, no traversing lower in the
  // hierarchy
  ...
  matrix3x4_t bindJointToParent, bindPoseToJoint, bindPoseToWorld,
      jointToBindPose;
  if (pBindPose) {
    if (nJointIndex >= pBindPose->GetTransformCount()) {
      Warning("Model is in an invalid state! There are different numbers of "
              "bones in the bind pose and joint transform list!\n");
      return;
    }
  ...
  for (int i = 0; i < nChildCount; ++i) {
    ...
    LoadJointTransform(pChildJoint, pBindPose, jointToWorld, jointToBindPose,
                       bSetHardwareState);
  }
}
...
//-----------------------------------------------------------------------------
CDmeModel::SetupBoneRetval_t
CDmeModel::SetupBoneMatrixState(const matrix3x4_t &shapeToWorld,
  ...
  bool bSetHardwareState =
      (nJointCount <= nBoneBatchCount) && !bForceSoftwareSkin;
  ...
  for (int i = 0; i < nChildCount; ++i) {
    ...
    LoadJointTransform(pChildJoint, pBindPose, shapeToWorld, parentToBindPose,
                       bSetHardwareState);
  }
...
matrix3x4_t *CDmeModel::SetupModelRenderState(const matrix3x4_t &shapeToWorld,
                                              bool bHasSkinningData,
                                              bool bForceSoftwareSkin) {
  CMatRenderContextPtr pRenderContext(g_pMaterialSystem);
  if (bHasSkinningData && (s_ModelStack.Count() > 0)) {
    SetupBoneRetval_t retVal = s_ModelStack.Top()->SetupBoneMatrixState(
        shapeToWorld, bForceSoftwareSkin);
...
//-----------------------------------------------------------------------------
void CDmeModel::ZUp(bool bZUp) { SetValue("upAxis", bZUp ? "Z" : "Y"); }
