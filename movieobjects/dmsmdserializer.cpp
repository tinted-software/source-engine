...
bool CDmSmdSerializer::Unserialize(
    CUtlBuffer &utlBuf, const char * /* pszEncodingName */,
    int /* nEncodingVersion */, const char * /* pszSourceFormatName */,
    int /* nSourceFormatVersion */, DmFileId_t nDmFileId,
...
//-----------------------------------------------------------------------------
CDmElement *
CDmSmdSerializer::ReadSMD(const char *pszFilename,
...
//
//-----------------------------------------------------------------------------
static bool ParserHandleSectionStart(const char *pszBuf,
                                     const char *pszSectionId) {
  pszBuf = ParserSkipSpace(pszBuf);
...
//
//-----------------------------------------------------------------------------
void CDmSmdSerializer::ParserGetNodeName(const char *pszBuf,
                                         CUtlString &sName) const {
...
//
//-----------------------------------------------------------------------------
bool CDmSmdSerializer::ParserHandleSkeletonLine(const char *pszBuf,
                                                CUtlString &sName, int &nId,
                                                int &nParentId) const {
...
bool CQcData::ParseQc(const CUtlString &smdPath, const CUtlString &qcPath) {
    ...
    try {
      ...
      while (std::getline(ifs, buf)) {
        ...
        if (!V_stricmp(tokens[0], "$upaxis")) {
          ...
          } else if (strchr(tokens[1].Get(), 'x') ||
                     strchr(tokens[1].Get(), 'X')) {
  ...
  if (m_cdmaterials.empty()) {
    // If m_cdmaterials is empty, then put the relative smd path onto the
    // cdmaterials path
...
bool CQcData::GetQcData(const CUtlString &smdPath) {
  try {
    ...
    // Look for any *.qc file in the same directory as the smd that contains the
    // smd pathname
    V_strncpy(szBuf0, smdPath.Get(), ARRAYSIZE(szBuf0));
    V_FixSlashes(szBuf0);
    ...
    if (sFileBase0.Length() > 0 && sFilePath.Length() > 0) {
      ...
      if ((hFile = _findfirst(sQcGlob.Get(), &qcFile)) != -1L) {
        ...
        do {
          ...
          while (std::getline(ifs, buf)) {
            if (V_stristr(buf.c_str(), sFileBase0.Get()) ||
                V_stristr(buf.c_str(), sFileBase1.Get())) {
              _findclose(hFile);
...
public:
  ...
  inline bool operator==(const CVertex &rhs) const {
    if (m_vPosition != rhs.m_vPosition || m_vNormal != rhs.m_vNormal ||
        m_vUV != rhs.m_vUV || m_nWeights != rhs.m_nWeights)
      return false;
...
bool CTriangle::Valid() const {
  // A triangle is valid it it has three vertices and all three vertices are
  // unique
...
//
//-----------------------------------------------------------------------------
static bool ParserAddJoint(CDmSmdSerializer::SmdJointMap_t &smdJointMap,
                           int nId, const char *pszName, int nParentId,
                           const char *pszFilename, int nLineNumber) {
  if (nId < 0) {
    Error("Error! %s(%d) : node error : invalid node id %d, must be >= 0: Line "
          "'%d \"%s\" %d'\n",
  ...
  CDmSmdSerializer::SmdJoint_t &smdJoint =
      smdJointMap.Element(smdJointMap.Insert(nId));
...
//-----------------------------------------------------------------------------
static bool
ParserCreateJoint(const CDmSmdSerializer::SmdJointMap_t &smdJointMap,
                  CDmeModel *pDmeModel, CDmeChannelsClip *pDmeChannelsClip,
  ...
  CDmeJoint *pDmeJoint =
      CreateElement<CDmeJoint>(sName.Get(), pDmeModel->GetFileId());

  if (!pDmeJoint) {
    Error("%s(%d) : node error : can't create joint %d(%s)\n", pszFilename,
          nLineNumber, nId, sName.Get());
    return false;
  }
  ...
  if (nParentId < 0) {
    ...
    CDmAttribute *pRootJointAttr =
        pDmeJoint->AddAttribute("__rootJoint", AT_BOOL);
  ...
  if (pDmeChannelsClip) {
    ...
    CDmeChannel *pDmePosChannel = CreateElement<CDmeChannel>(
        CFmtStr("%s_p", pDmeTransform->GetName()).Access(),
    ...
    CDmeChannel *pDmeRotChannel = CreateElement<CDmeChannel>(
        CFmtStr("%s_o", pDmeTransform->GetName()).Access(),
        pDmeChannelsClip->GetFileId());
...
//-----------------------------------------------------------------------------
static bool SmdJointLessFunc(const CDmSmdSerializer::SmdJoint_t *pLhs,
                             const CDmSmdSerializer::SmdJoint_t *pRhs) {
  // try to preserve joints without parents in their original order as much as
  // possible
...
//
//-----------------------------------------------------------------------------
static void ParserCreateJoints(CDmSmdSerializer::SmdJointMap_t &smdJointMap,
                               CDmeModel *pDmeModel,
  ...
  std::stable_sort(smdJointList.Base(),
                   smdJointList.Base() + smdJointList.Count(),
                   SmdJointLessFunc);
  ...
  for (int i = 0; i < smdJointList.Count(); ++i) {
    ...
    ParserCreateJoint(smdJointMap, pDmeModel, pDmeChannelsClip, pSmdJoint,
                      pszFilename);
  }
}
...
//
//-----------------------------------------------------------------------------
void CDmSmdSerializer::ParserSetJoint(const SmdJointMap_t &smdJointMap,
                                      int nFrame, int nId,
  ...
  if (!pDmeDag) {
    Error("%s(%d) : skeleton error : no dmedag created for can't find joint %d "
          "(%s)\n",
          pszFilename, nLineNumber, nId,
          smdJointMap.Element(nIdIndex).m_sName.Get());
    return;
  }

  const Quaternion qOrientation = eRadianEulerXYZ;

  if (pDmeDag->GetValue("__rootJoint", false) &&
      GetUpAxis() != CDmSmdSerializer::Y_AXIS) {
    matrix3x4_t mPre;
    matrix3x4_t mPost;
  ...
  if (m_bOptAnimation) {
    const DmeTime_t tCurrent(static_cast<float>(nFrame),
                             DmeFramerate_t(m_flFrameRate));

    CDmeTransform *pDmeTransform = pDmeDag->GetTransform();

    CDmeVector3Log *pDmePosLog =
        pDmeDag->GetValueElement<CDmeVector3Log>("__posLog");
    pDmePosLog->SetKey(tCurrent, pDmeTransform->GetPosition());

    CDmeQuaternionLog *pDmeRotLog =
        pDmeDag->GetValueElement<CDmeQuaternionLog>("__rotLog");
...
//-----------------------------------------------------------------------------
static int GetActualId(const CDmSmdSerializer::SmdJointMap_t &smdJointMap,
                       int nId) {
  const CDmSmdSerializer::SmdJointMap_t::IndexType_t nIdIndex =
      smdJointMap.Find(nId);
...
static int VertexWeightLessFunc(const void *pLhs, const void *pRhs) {
  const CVertexWeight *pVertexWeightL =
      reinterpret_cast<const CVertexWeight *>(pLhs);
  const CVertexWeight *pVertexWeightR =
      reinterpret_cast<const CVertexWeight *>(pRhs);
...
//-----------------------------------------------------------------------------
static void
HandleVertexWeights(const CDmSmdSerializer::SmdJointMap_t &smdJointMap,
                    int nFallbackBoneIndexId, CVertex &vertex,
                    const char *pszLine, const char *pszFilename,
                    int nLineNumber) {
  ...
  if (nTokenEnd > 10) {
    ...
    for (uint i = 10; i < nTokenEnd; ++i) {
      ...
      if (nActualId < 0) {
        Error("%s(%d) : triangle error : ignoring unknown joint id(%d) with "
              "actual id(%d) for vertex weight\n",
              pszFilename, nLineNumber, nId, nActualId);
        continue;
      }

      CVertexWeight &tmpVertexWeight =
          tmpVertexWeights[tmpVertexWeights.AddToTail()];
      tmpVertexWeight.m_nBoneIndex = nActualId;
      tmpVertexWeight.m_flWeight = flWeight;
    }

    // Sort vertex weights inversely by weight
    qsort(tmpVertexWeights.Base(), tmpVertexWeights.Count(),
          sizeof(CVertexWeight), VertexWeightLessFunc);

    // Take at most the top ARRAYSIZE( vertex.m_vertexWeights ) from
    // tmpVertexWeights Figure out actual weight count and total weight

    float flTotalWeight = 0.0f;
    vertex.m_nWeights = 0;

    for (int i = 0;
         i < tmpVertexWeights.Count() && i < ARRAYSIZE(vertex.m_vertexWeights);
         ++i) {
    ...
    if (vertex.m_nWeights > 0 && flTotalWeight > flEps) {
      ...
      vertex.m_vertexWeights[0].m_nBoneIndex =
          GetActualId(smdJointMap, nFallbackBoneIndexId);
...
//
//-----------------------------------------------------------------------------
static CDmeMesh *CreateDmeMesh(CDmeModel *pDmeModel, const char *pszFilename,
                               int nLineNumber) {
  ...
  CDmeDag *pDmeDag =
      CreateElement<CDmeDag>(pszMeshName, pDmeModel->GetFileId());
  if (!pDmeDag)
    return NULL;
  ...
  CDmeMesh *pDmeMesh =
      CreateElement<CDmeMesh>(sMeshName.Get(), pDmeDag->GetFileId());
  if (!pDmeMesh)
    return NULL;
  ...
  pDmeVertexData->CreateJointWeightsAndIndices(
      MAX_WEIGHTS_PER_VERTEX, &nJointWeightField, &nJointIndexField);
...
//
//-----------------------------------------------------------------------------
static CDmeFaceSet *FindOrCreateFaceSet(CDmeMesh *pDmeMesh,
  ...
  CDmeFaceSet *pDmeFaceSet =
      CreateElement<CDmeFaceSet>(szFaceSetName, pDmeMesh->GetFileId());
  Assert(pDmeFaceSet);

  CDmeMaterial *pDmeMaterial =
      CreateElement<CDmeMaterial>(szFaceSetName, pDmeMesh->GetFileId());
  Assert(pDmeMaterial);
...
//
//-----------------------------------------------------------------------------
static void CreatePolygon(CDmeMesh *pDmeMesh, const CUtlString &sMaterial,
                          CTriangle &triangle, const char *pszFilename,
                          int nLineNumber) {
  ...
  FieldIndex_t nPositionField =
      pDmeVertexData->FindFieldIndex(CDmeVertexData::FIELD_POSITION);
  CDmrArray<Vector> positionData =
      pDmeVertexData->GetVertexData(nPositionField);

  FieldIndex_t nNormalField =
      pDmeVertexData->FindFieldIndex(CDmeVertexData::FIELD_NORMAL);
  CDmrArray<Vector> normalData = pDmeVertexData->GetVertexData(nNormalField);

  FieldIndex_t nUvField =
      pDmeVertexData->FindFieldIndex(CDmeVertexData::FIELD_TEXCOORD);
  CDmrArray<Vector2D> uvData = pDmeVertexData->GetVertexData(nUvField);

  FieldIndex_t nJointWeightField =
      pDmeVertexData->FindFieldIndex(CDmeVertexData::FIELD_JOINT_WEIGHTS);
  CDmrArray<float> jointWeightData =
      pDmeVertexData->GetVertexData(nJointWeightField);

  FieldIndex_t nJointIndexField =
      pDmeVertexData->FindFieldIndex(CDmeVertexData::FIELD_JOINT_INDICES);
  CDmrArray<int> jointIndexData =
      pDmeVertexData->GetVertexData(nJointIndexField);

  CDmeFaceSet *pDmeFaceSet =
      FindOrCreateFaceSet(pDmeMesh, sMaterial, pszFilename, nLineNumber);
  if (!pDmeFaceSet) {
    Error("%s(%d) : mesh error : couldn't find or create DmeFace set for "
          "material \"%s\" on mesh \"%s\"\n",
  ...
  for (uint i = 0; i < 3; ++i) {
...

    {
      ...
      pDmeVertexData->SetVertexIndices(nPositionField, nNewVertexIndex, 1,
                                       &nPositionIndex);

      const int nFaceSetIndex = pDmeFaceSet->AddIndices(1);
      pDmeFaceSet->SetIndices(nFaceSetIndex, 1,
                              const_cast<int *>(&nNewVertexIndex));
    }

    {
      ...
      pDmeVertexData->SetVertexIndices(nNormalField, nNewVertexIndex, 1,
                                       &nNormalIndex);
    }
...
//-----------------------------------------------------------------------------
static CDmeFaceSet *
FindOrAddFaceSet(CDmeMesh *pDmeMesh,
                 CUtlMap<CUtlString, CDmeFaceSet *> &faceListMap,
                 const char *pszMaterialPath) {
  CUtlMap<CUtlString, CDmeFaceSet *>::IndexType_t nIndex =
      faceListMap.Find(CUtlString(pszMaterialPath));

  if (faceListMap.IsValidIndex(nIndex))
    return faceListMap.Element(nIndex);

  CDmeFaceSet *pDmeFaceSet =
      CreateElement<CDmeFaceSet>(pszMaterialPath, pDmeMesh->GetFileId());
  CDmeMaterial *pDmeMaterial =
      CreateElement<CDmeMaterial>(pszMaterialPath, pDmeMesh->GetFileId());
  Assert(pDmeFaceSet && pDmeMaterial);
  pDmeMaterial->SetMaterial(pszMaterialPath);
  pDmeMesh->AddFaceSet(pDmeFaceSet);

  return faceListMap.Element(
      faceListMap.Insert(CUtlString(pszMaterialPath), pDmeFaceSet));
}

//-----------------------------------------------------------------------------
//
//-----------------------------------------------------------------------------
CDmeChannelsClip *FindOrCreateChannelsClip(CDmElement *pDmeRoot,
                                           const char *pszAnimationName) {
  CDmeAnimationList *pDmeAnimationList =
      pDmeRoot->GetValueElement<CDmeAnimationList>("animationList");
  ...
  if (pDmeAnimationList) {
    ...
    pDmeAnimationList = CreateElement<CDmeAnimationList>(pszAnimationName,
                                                         pDmeRoot->GetFileId());
    pDmeChannelsClip = CreateElement<CDmeChannelsClip>(pszAnimationName,
                                                       pDmeRoot->GetFileId());
...
//-----------------------------------------------------------------------------
CDmElement *CDmSmdSerializer::ReadSMD(CUtlBuffer &utlBuf, DmFileId_t nDmFileId,
                                      const char *pszFilename,
    ...
    if (nParserState == kPreamble) {
      ...
      if (ParserHandleVersion(pszLine)) {
        if (qcData.m_nUpAxis != m_nUpAxis) {
          Warning("Importer UpAxis (%d) is different from UpAxis in SMD data "
                  "(%d), using value found in SMD/QC\n",
      ...
      if (nBadPreambleCount > 10) {
        Error("%s(%d) : preamble error : too many errors, not an SMD file, "
              "aborting\n",
      ...
      if (ParserHandleSectionStart(pszLine, "end")) {
        ParserCreateJoints(smdJointMap, pDmeModel, pDmeChannelsClip,
                           pszFilename);
      ...
      if (!m_bOptImportSkeleton ||
          !ParserHandleSkeletonLine(pszLine, sName, nId, nParentId))
        continue;

      if (!ParserAddJoint(smdJointMap, nId, sName, nParentId, pszFilename,
                          nLineNumber))
        continue;
      ...
      if (m_bOptAnimation && ParserHandleTime(pszLine, nFrame)) {
        if (!bStartTimeSet) {
          pDmeChannelsClip->SetStartTime(DmeTime_t(
              static_cast<float>(nFrame), DmeFramerate_t(m_flFrameRate)));
          bStartTimeSet = true;
        }
        continue;
      }
      ...
      if (sscanf(pszLine, "%d %f %f %f %f %f %f", &nId, &vPosition.x,
                 &vPosition.y, &vPosition.z, &eRadianEulerXYZ.x,
                 &eRadianEulerXYZ.y, &eRadianEulerXYZ.z) == 7) {
        if (!m_bOptImportSkeleton)
          continue;

        ParserSetJoint(smdJointMap, nFrame, nId, vPosition, eRadianEulerXYZ,
                       pszFilename, nLineNumber);
      }
      ...
      if (sscanf(pszLine, "%d %f %f %f %f %f %f %f %f", &nFallbackSkinJoint,
                 &vPosition.x, &vPosition.y, &vPosition.z, &vNormal.x,
                 &vNormal.y, &vNormal.z, &vUV.x, &vUV.y) == 9) {
        ...
        if (m_bOptImportSkeleton) {
          HandleVertexWeights(smdJointMap, nFallbackSkinJoint, vertex, pszLine,
                              pszFilename, nLineNumber);
        }
        ...
        V_StripExtension(szShadingGroup, pszMaterialPath,
                         ARRAYSIZE(pszMaterialPath));
  ...
  if (pDmeChannelsClip) {
    ...
    pDmeChannelsClip->SetDuration(
        DmeTime_t(nFrame, DmeFramerate_t(m_flFrameRate)) -
...
void CDmSmdSerializer::FixNodeName(CUtlString &sName) const {
  ...
  if (!m_sNodeDelPrefix.IsEmpty() &&
      StringHasPrefix(pszName, m_sNodeDelPrefix.Get())) {
    pszName =
        const_cast<char *>(StringAfterPrefix(pszName, m_sNodeDelPrefix.Get()));
  }
