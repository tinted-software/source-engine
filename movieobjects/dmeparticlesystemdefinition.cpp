...
//-----------------------------------------------------------------------------
IMPLEMENT_ELEMENT_FACTORY_INSTALL_EXPLICITLY(DmeParticleFunction,
                                             CDmeParticleFunction);

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CDmeParticleFunction::OnConstruction() { m_bSkipNextResolve = false; }
...
//-----------------------------------------------------------------------------
static void CreateEditorAttributeInfo(CDmeEditorType *pEditorType,
                                      const char *pAttributeName,
                                      const char *pWidgetInfo) {
  if (!pWidgetInfo)
    return;
  ...
  if (parse.ArgC() == 1) {
    CDmeEditorAttributeInfo *pInfo =
        CreateElement<CDmeEditorAttributeInfo>("field info");
...
//-----------------------------------------------------------------------------
void CDmeParticleFunction::AddMissingFields(
    const DmxElementUnpackStructure_t *pUnpack) {
  DestroyElement(m_hTypeDictionary, TD_DEEP);
  m_hTypeDictionary =
      CreateElement<CDmeEditorTypeDictionary>("particleFunctionDict");
  CDmeEditorType *pEditorType = CreateElement<CDmeEditorType>(GetTypeString());

  for (; pUnpack->m_pAttributeName; ++pUnpack) {
    CreateEditorAttributeInfo(pEditorType, pUnpack->m_pAttributeName,
                              (const char *)pUnpack->m_pUserData);
    ...
    CDmAttribute *pAttribute =
        AddAttribute(pUnpack->m_pAttributeName, pUnpack->m_AttributeType);
    if (pUnpack->m_pDefaultString) {
      int nLen = Q_strlen(pUnpack->m_pDefaultString);
      CUtlBuffer bufParse(pUnpack->m_pDefaultString, nLen,
...
//-----------------------------------------------------------------------------
void CDmeParticleFunction::UpdateAttributes(
    const DmxElementUnpackStructure_t *pUnpack) {
...
//-----------------------------------------------------------------------------
void CDmeParticleFunction::MarkNewInstance() { m_bSkipNextResolve = true; }
...
void CDmeParticleFunction::Resolve() {
  ...
  for (CDmAttribute *pAttr = FirstAttribute(); pAttr;
    ...
    while (i != DMATTRIBUTE_REFERENCE_ITERATOR_INVALID) {
      ...
      CDmeParticleSystemDefinition *pDef =
          CastElement<CDmeParticleSystemDefinition>(pAttribute->GetOwner());
...
// Expose this class to the scene database
//-----------------------------------------------------------------------------
IMPLEMENT_ELEMENT_FACTORY_INSTALL_EXPLICITLY(DmeParticleOperator,
                                             CDmeParticleOperator);
...
// Expose this class to the scene database
//-----------------------------------------------------------------------------
IMPLEMENT_ELEMENT_FACTORY_INSTALL_EXPLICITLY(DmeParticleChild,
                                             CDmeParticleChild);
...
//-----------------------------------------------------------------------------
void CDmeParticleChild::SetChildParticleSystem(
    CDmeParticleSystemDefinition *pDef,
    IParticleOperatorDefinition *pDefinition) {
...
// Expose this class to the scene database
//-----------------------------------------------------------------------------
IMPLEMENT_ELEMENT_FACTORY_INSTALL_EXPLICITLY(DmeParticleSystemDefinition,
...
void CDmeParticleSystemDefinition::OnConstruction() {
  ...
  m_hTypeDictionary =
      CreateElement<CDmeEditorTypeDictionary>("particleSystemDefinitionDict");
  CDmeEditorType *pEditorType =
      CreateElement<CDmeEditorType>("DmeParticleSystemDefinition");

  const DmxElementUnpackStructure_t *pUnpack =
      g_pParticleSystemMgr->GetParticleSystemDefinitionUnpackStructure();
  for (; pUnpack->m_pAttributeName; ++pUnpack) {
    CreateEditorAttributeInfo(pEditorType, pUnpack->m_pAttributeName,
                              (const char *)pUnpack->m_pUserData);
...
// Returns the editor type dictionary
//-----------------------------------------------------------------------------
CDmeEditorTypeDictionary *
CDmeParticleSystemDefinition::GetEditorTypeDictionary() {
  return m_hTypeDictionary;
}
...
//-----------------------------------------------------------------------------
static void
RemoveObsoleteAttributes(CDmElement *pElement,
                         const DmxElementUnpackStructure_t *pUnpack) {
  ...
  for (CDmAttribute *pAttr = pElement->FirstAttribute(); pAttr; pAttr = pNext) {
    ...
    bool bFound = false;
    for (const DmxElementUnpackStructure_t *pTrav = pUnpack;
         pTrav->m_pAttributeName; ++pTrav) {
...
void CDmeParticleSystemDefinition::OnElementUnserialized() {
  ...
  RemoveObsoleteAttributes(
      this, g_pParticleSystemMgr->GetParticleSystemDefinitionUnpackStructure());
...
//-----------------------------------------------------------------------------
CDmeParticleFunction *
CDmeParticleSystemDefinition::AddOperator(ParticleFunctionType_t type,
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    CDmeParticleOperator *pFunction =
        CreateElement<CDmeParticleOperator>(pFunctionName, GetFileId());
...
CDmeParticleFunction *
CDmeParticleSystemDefinition::AddChild(CDmeParticleSystemDefinition *pChild) {
  Assert(pChild);

  CUtlVector<IParticleOperatorDefinition *> &list =
      g_pParticleSystemMgr->GetAvailableParticleOperatorList(FUNCTION_CHILDREN);
  Assert(list.Count() == 1);
  CDmeParticleChild *pFunction =
      CreateElement<CDmeParticleChild>(pChild->GetName(), GetFileId());
...
//-----------------------------------------------------------------------------
// Remove
void CDmeParticleSystemDefinition::RemoveFunction(
    ParticleFunctionType_t type, CDmeParticleFunction *pFunction) {
  int nIndex = FindFunction(type, pFunction);
  RemoveFunction(type, nIndex);
}

void CDmeParticleSystemDefinition::RemoveFunction(ParticleFunctionType_t type,
                                                  int nIndex) {
  if (nIndex >= 0) {
...
// Find
//-----------------------------------------------------------------------------
int CDmeParticleSystemDefinition::FindFunction(
    ParticleFunctionType_t type, CDmeParticleFunction *pParticleFunction) {
...
int CDmeParticleSystemDefinition::FindFunction(ParticleFunctionType_t type,
                                               const char *pFunctionName) {
  int nCount = m_ParticleFunction[type].Count();
  for (int i = 0; i < nCount; ++i) {
    if (!Q_stricmp(pFunctionName,
...
// Iteration
//-----------------------------------------------------------------------------
int CDmeParticleSystemDefinition::GetParticleFunctionCount(
    ParticleFunctionType_t type) const {
  return m_ParticleFunction[type].Count();
}

CDmeParticleFunction *
CDmeParticleSystemDefinition::GetParticleFunction(ParticleFunctionType_t type,
                                                  int nIndex) {
...
// Reordering
//-----------------------------------------------------------------------------
void CDmeParticleSystemDefinition::MoveFunctionUp(
    ParticleFunctionType_t type, CDmeParticleFunction *pElement) {
...
void CDmeParticleSystemDefinition::MoveFunctionDown(
    ParticleFunctionType_t type, CDmeParticleFunction *pElement) {
  int nIndex = FindFunction(type, pElement);
...
void CDmeParticleSystemDefinition::RecompileParticleSystem() {
  ...
  int nFlags =
      g_pDataModel->IsEncodingBinary(pEncoding) ? 0 : CUtlBuffer::TEXT_BUFFER;
