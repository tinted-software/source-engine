...
void CDmeTimeFrame::OnAttributeChanged(CDmAttribute *pAttribute) {
  ...
  if (pAttribute == m_Start.GetAttribute() ||
      pAttribute == m_Duration.GetAttribute()) {
...
void CDmeTimeFrame::SetTimeScale(float flScale, DmeTime_t scaleCenter,
                                 bool bChangeDuration) {
#ifdef _DEBUG
  ...
  if (bChangeDuration) {
    ...
    if (scaleCenter.GetTenthsOfMS() != m_Start) {
      int newStart = int((m_Start - scaleCenter.GetTenthsOfMS()) * ratio +
                         scaleCenter.GetTenthsOfMS());
      SetStartTime(DmeTime_t(newStart));
    }

    int newStart = m_Start;
    int newOffset =
        int((t + m_Offset) * ratio + newStart - scaleCenter.GetTenthsOfMS());
