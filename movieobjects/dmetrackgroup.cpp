...
void CDmeTrackGroup::OnDestruction() {
  // NOTE: The track owner handles may still be pointing to us when we get
  // destructed, but their handles will be invalid, so GetTrackGroup on a track
...
// Volume
//-----------------------------------------------------------------------------
void CDmeTrackGroup::SetVolume(float state) { m_Volume = state; }
float CDmeTrackGroup::GetVolume() const { return m_Volume.Get(); }
...
CDmeTrack *CDmeTrackGroup::AddTrack(const char *pTrackName,
...
CDmeTrack *CDmeTrackGroup::FindOrAddTrack(const char *pTrackName,
                                          DmeClipType_t trackType) {
...
bool CDmeTrackGroup::FindTrackForClip(CDmeClip *pClip, int *pTrackIndex,
...
//-----------------------------------------------------------------------------
void CDmeTrackGroup::FindClipsAtTime(DmeClipType_t clipType, DmeTime_t time,
...
void CDmeTrackGroup::FindClipsIntersectingTime(
    DmeClipType_t clipType, DmeTime_t startTime, DmeTime_t endTime,
...
void CDmeTrackGroup::FindClipsWithinTime(DmeClipType_t clipType,
...
CDmeFilmClip *GetParentClip(CDmeTrackGroup *pTrackGroup) {
  DmAttributeReferenceIterator_t hAttr =
      g_pDataModel->FirstAttributeReferencingElement(pTrackGroup->GetHandle());
