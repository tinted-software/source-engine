...
//-----------------------------------------------------------------------------
void CDmeChannelRecordingMgr::StartLayerRecording(
    const char *pUndoRedoDesc, const DmeLog_TimeSelection_t *pTimeSelection) {
...
void CDmeChannelRecordingMgr::FinishLayerRecording(
    float flThreshhold, bool bFlattenLayers /*=true*/) {
  Assert(m_bActive);
...
//-----------------------------------------------------------------------------
void CDmeChannelRecordingMgr::AddChannelToRecordingLayer(CDmeChannel *pChannel,
                                                         CDmeClip *pRoot,
                                                         CDmeClip *pShot) {
...
//-----------------------------------------------------------------------------
void CDmeChannelRecordingMgr::GetLocalTimeSelection(
    DmeLog_TimeSelection_t &selection, int nIndex) {
  ...
  for (int i = 0; i < TS_TIME_COUNT; ++i) {
    selection.m_nTimes[i] = CDmeClip::ToChildMediaTime(
        info.m_ClipStack, selection.m_nTimes[i], false);
  }
...
//-----------------------------------------------------------------------------
void CDmeChannelRecordingMgr::UpdateTimeAdvancing(bool bPaused,
                                                  DmeTime_t tCurTime) {
  ...
  if (!bPaused && !m_TimeSelection.IsTimeAdvancing()) {
    ...
    for (int i = 0; i < nCount; ++i) {
      ...
      DmeTime_t t =
          CDmeClip::ToChildMediaTime(info.m_ClipStack, tCurTime, false);
      info.m_Channel->GetLog()->RemoveKeys(t, DMETIME_MAXTIME);
    }
  }
}

void CDmeChannelRecordingMgr::UpdateRecordingTimeSelectionTimes(
    const DmeLog_TimeSelection_t &timeSelection) {
  Assert(m_bActive);
  for (int i = 0; i < TS_TIME_COUNT; ++i) {
...
void CDmeChannelRecordingMgr::SetPresetValue(CDmeChannel *pChannel,
                                             CDmAttribute *pPresetValue) {
  Assert(pChannel->m_nRecordLayerIndex != -1);
...
void CDmeChannelRecordingMgr::SetProceduralTarget(int nProceduralMode,
                                                  const CDmAttribute *pTarget) {
  m_nRevealType = nProceduralMode;
  m_pRevealTarget = pTarget;
  m_PasteTarget.RemoveAll();
}

void CDmeChannelRecordingMgr::SetProceduralTarget(
    int nProceduralMode, const CUtlVector<KeyValues *> &list) {
  m_nRevealType = nProceduralMode;
  m_pRevealTarget = NULL;
  m_PasteTarget.RemoveAll();
...
int CDmeChannelRecordingMgr::GetProceduralType() const { return m_nRevealType; }
...
void CDmeChannel::OnConstruction() {
  ...
  m_fromElement.Init(this, "fromElement",
                     FATTRIB_HAS_CALLBACK | FATTRIB_NEVERCOPY);
  m_fromAttribute.Init(this, "fromAttribute",
                       FATTRIB_TOPOLOGICAL | FATTRIB_HAS_CALLBACK);
  ...
  m_toAttribute.Init(this, "toAttribute",
                     FATTRIB_TOPOLOGICAL | FATTRIB_HAS_CALLBACK);
...
void CDmeChannel::Play() {
  ...
  // We might not want to do it this way, but this makes empty logs not get in
  // the way if there is a valid pFromAttr
#if 1
...
void CDmeChannel::GetOutputAttributes(CUtlVector<CDmAttribute *> &attrs) {
  ...
  if (mode == CM_RECORD || mode == CM_PASS) {
    if (GetFromAttribute() == GetToAttribute())
      return; // record/pass from and to the same attribute doesn't write
              // anything
  }
...
// HACK:  This is an evil hack since the element and attribute change
// sequentially, but they really need to change in lockstep or else you're
// looking
//  up an attribute from some other element or vice versa.

void CDmeChannel::SetInput(CDmElement *pElement, const char *pAttribute,
                           int index) {
...
void CDmeChannel::SetOutput(CDmElement *pElement, const char *pAttribute,
                            int index) {
...
DmeTime_t CDmeChannel::GetCurrentTime() const { return m_tCurrentTime; }
...
// choosing the time closest to (and after) a timeframe if multiple sets in a
// frame
//-----------------------------------------------------------------------------
void CDmeChannel::SetCurrentTime(DmeTime_t time, DmeTime_t start,
                                 DmeTime_t end) {
  m_tPreviousTime = m_tCurrentTime;
  ...
  if ((dt < t0 && totf < t0 &&
       dt < totf) ||             // both prior to clip, old totf closer
...
//-----------------------------------------------------------------------------
// ClearTimeMetric marks m_timeOutsideTimeframe invalid (-inf is the worst
// possible match)
//-----------------------------------------------------------------------------
...
void CDmeChannel::SetChannelToPlayToSelf(const char *outputAttributeName,
                                         float defaultValue,
                                         bool force /*= false*/) {
  ...
  // Usually we won't put it into playback if it's empty, we'll just read the
  // default value continously
...
CDmeLogLayer *
FindLayerInSnapshot(const CDmrElementArray<CDmElement> &snapshotArray,
                    CDmeLog *origLog) {
...
KeyValues *FindLayerInPasteData(const CUtlVector<KeyValues *> &list,
                                CDmeLog *log) {
  int c = list.Count();
  for (int i = 0; i < c; ++i) {
    ...
    LayerSelectionData_t *data =
        reinterpret_cast<LayerSelectionData_t *>(kv->GetPtr("LayerData"));
    if (!data)
      continue;
...
static int FindSpanningLayerAndSetIntensity(DmeLog_TimeSelection_t &ts,
                                            LayerSelectionData_t *data) {
  ...
  for (; i < data->m_vecData.Count() - 1; ++i) {
    ...
    if (frac >= current->m_flStartFraction && frac <= next->m_flStartFraction) {
      frac = RemapVal(frac, current->m_flStartFraction, next->m_flStartFraction,
                      0.0f, 1.0f);
...
void CDmeChannel::Record() {
  ...
  if (g_pChannelRecordingMgr->ShouldRecordUsingTimeSelection()) {
    ...
    // Computes local times for the time selection
    DmeLog_TimeSelection_t timeSelection;
    g_pChannelRecordingMgr->GetLocalTimeSelection(timeSelection,
                                                  m_nRecordLayerIndex);
    ...
    case PROCEDURAL_PRESET_NOT: {
      pLog->StampKeyAtHead(time, m_tPreviousTime, timeSelection, pFromAttr,
                           m_fromIndex.Get());
    } break;
    case PROCEDURAL_PRESET_REVEAL: {
      ...
      const CDmrElementArray<CDmElement> snapshotArray =
          const_cast<CDmAttribute *>(
              g_pChannelRecordingMgr->GetProceduralTarget());
    ...
    case PROCEDURAL_PRESET_PASTE: {
      const CUtlVector<KeyValues *> &pasteTarget =
          g_pChannelRecordingMgr->GetPasteTarget();
      KeyValues *layer = FindLayerInPasteData(pasteTarget, pLog);
      if (layer) {
        LayerSelectionData_t *data = reinterpret_cast<LayerSelectionData_t *>(
            layer->GetPtr("LayerData"));
        Assert(data);

        int iSourceLayer =
            FindSpanningLayerAndSetIntensity(timeSelection, data);

        CDmeLogLayer *sourceLayer = data->m_vecData[iSourceLayer].m_hData.Get();
        CDmeLogLayer *targetLayer =
            data->m_vecData[iSourceLayer + 1].m_hData.Get();
        if (sourceLayer && sourceLayer->GetKeyCount() > 0 && targetLayer &&
            targetLayer->GetKeyCount() > 0 &&
          ...
          if (nType == PROCEDURAL_PRESET_STAGGER) {
            pLog->BlendTimesUsingTimeSelection(sourceLayer, targetLayer,
                                               outputLayer, timeSelection,
                                               data->m_tStartOffset);
          } else {
            pLog->BlendLayersUsingTimeSelection(sourceLayer, targetLayer,
                                                outputLayer, timeSelection,
...
//-----------------------------------------------------------------------------
bool CDmeChannel::BuildClipStack(DmeClipStack_t *pClipStack, CDmeClip *pRoot,
                                 CDmeClip *pShot) {
