...
//-----------------------------------------------------------------------------
CDmeVertexDeltaData *
CDmeMesh::FindOrCreateDeltaState(const char *pInDeltaName) {
...
CDmeVertexDeltaData *CDmeMesh::ModifyOrCreateDeltaStateFromBaseState(
  ...
  for (int i = 0; i < sizeof(deltaFields) / sizeof(deltaFields[0]); ++i) {
    ...
    case AT_COLOR_ARRAY:
      CreateDeltaFieldFromBaseField(standardFieldIndex,
                                    CDmrArrayConst<Vector>(pBaseData),
...
CDmeMesh::FeatherSelection(float fDistance, Distance_t distanceType,
                           CDmeSingleIndexedComponent *pSelection,
                           CDmMeshComp *pPassedMeshComp) {
...
bool CDmeMesh::AddMaskedDelta(
  ...
  for (int j(0); j < nBaseField; ++j) {
    ...
    for (int k(0); k < nDeltaField; ++k) {
      ...
      switch (pBaseData->GetType()) {
      case AT_FLOAT_ARRAY:
        AddRawDelta(pDelta, CDmrArray<float>(pBaseData), baseFieldIndex, weight,
                    pMask);
        break;
      case AT_COLOR_ARRAY:
        // TODO: Color is missing some algebraic operators
        //				AddRawDelta( pDelta, CDmrArray< Color >(
        //pBaseData ), baseFieldIndex, weight, pMask );
        break;
      case AT_VECTOR2_ARRAY:
  ...
  for (int i = 0; i < nDeltas; ++i) {
    ...
    for (int j(0); j < nBaseField; ++j) {
      ...
      for (int k(0); k < nDeltaField; ++k) {
        ...
        case AT_FLOAT_ARRAY:
          AddCorrectedDelta(CDmrArray<float>(pBaseData), baseIndices,
                            compList[i], baseFieldName, weight, pMask);
          break;
        case AT_COLOR_ARRAY:
          AddCorrectedDelta(CDmrArray<Vector>(pBaseData), baseIndices,
                            compList[i], baseFieldName, weight, pMask);
...
bool CDmeMesh::InterpMaskedData(CDmeVertexData *paData,
  ...
  for (int i = 0; i < naField; ++i) {
    ...
    for (int j = 0; j < nbField; ++j) {
      ...
      case AT_FLOAT_ARRAY:
        InterpMaskedData(CDmrArray<float>(paAttr),
                         CDmrArrayConst<float>(pbAttr).Get(), weight, pMask);
        break;
      case AT_COLOR_ARRAY:
        InterpMaskedData(CDmrArray<Vector4D>(paAttr),
                         CDmrArrayConst<Vector4D>(pbAttr).Get(), weight, pMask);
        break;
      case AT_VECTOR2_ARRAY:
        InterpMaskedData(CDmrArray<Vector2D>(paAttr),
                         CDmrArrayConst<Vector2D>(pbAttr).Get(), weight, pMask);
        break;
      case AT_VECTOR3_ARRAY:
        InterpMaskedData(CDmrArray<Vector>(paAttr),
                         CDmrArrayConst<Vector>(pbAttr).Get(), weight, pMask);
        break;
      default:
        break;
      }
        ...
        for (int l = 0; l < nDeltaField; ++l) {
          ...
          case AT_FLOAT_ARRAY:
            ...
            AddCorrectedDelta(floatData, bindIndices, compList[i],
                              baseFieldName);
            InterpMaskedData(CDmrArray<float>(pDstBaseData), floatData, weight,
                             pMask);
            break;
          case AT_COLOR_ARRAY:
...
//-----------------------------------------------------------------------------
void CDmeMesh::ReplaceMaterial(const char *pOldMaterialName,
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    if (!pReplacementMaterial) {
      pReplacementMaterial = CreateElement<CDmeMaterial>(
          pMaterial->GetName(), pMaterial->GetFileId());
...
//-----------------------------------------------------------------------------
static void CollapseRedundantDeltaNormals(CDmeVertexDeltaData *pDmeDelta,
                                          const CUtlVector<int> &normalMap) {
  if (!pDmeDelta)
    return;

  FieldIndex_t nNormalFieldIndex =
      pDmeDelta->FindFieldIndex(CDmeVertexData::FIELD_NORMAL);
  if (nNormalFieldIndex < 0)
    return; // No normal deltas

  const CUtlVector<Vector> &oldNormalData = pDmeDelta->GetNormalData();
  const CUtlVector<int> &oldNormalIndices =
      pDmeDelta->GetVertexIndexData(nNormalFieldIndex);
  ...
  pDmeDelta->SetVertexData(nNormalFieldIndex, 0, newNormalData.Count(),
                           AT_VECTOR3, newNormalData.Base());
  pDmeDelta->SetVertexIndices(nNormalFieldIndex, 0, newNormalIndices.Count(),
                              newNormalIndices.Base());
}
...
// return false.
//-----------------------------------------------------------------------------
static bool CollapseRedundantBaseNormals(CDmeVertexData *pDmeVertexData,
                                         CUtlVector<int> &normalMap,
                                         float flNormalBlend) {
  if (!pDmeVertexData)
    return false;

  FieldIndex_t nPositionFieldIndex =
  ...
  const CUtlVector<int> &oldNormalIndices =
      pDmeVertexData->GetVertexIndexData(nNormalFieldIndex);
  ...
  for (int i = 0; i < nPositionDataCount; ++i) {
    ...
    const CUtlVector<int> &vertexIndices =
        pDmeVertexData->FindVertexIndicesFromDataIndex(
            CDmeVertexData::FIELD_POSITION, i);
  ...
  for (int i = 0; i < newNormalIndices.Count(); ++i) {
    if (newNormalIndices[i] == -1) {
      newNormalIndices[i] =
          newNormalData.AddToTail(oldNormalData[oldNormalIndices[i]]);
    }
  }
  ...
  nNormalFieldIndex =
      pDmeVertexData->CreateField(CDmeVertexDeltaData::FIELD_NORMAL);
  pDmeVertexData->AddVertexData(nNormalFieldIndex, newNormalData.Count());
  pDmeVertexData->SetVertexData(nNormalFieldIndex, 0, newNormalData.Count(),
                                AT_VECTOR3, newNormalData.Base());
...
//-----------------------------------------------------------------------------
static bool
CollapseRedundantBaseNormalsAggressive(CDmeVertexData *pDmeVertexData,
                                       float flNormalBlend) {
  if (!pDmeVertexData)
    return false;
