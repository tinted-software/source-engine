...
void CDmeTrack::OnConstruction() {
  ...
  m_ClipType.InitAndSet(this, "clipType", DMECLIP_UNKNOWN,
...
void CDmeTrack::OnAttributeChanged(CDmAttribute *pAttribute) {
  ...
  // This gets called when start/end time of children change, or if the array
  // changes This is a hack, since any OnAttributeChanged call that gets chained
  // here from another element will trigger this At some point, we'll probably
  // have to start sending more data through OnAttributeChanged, (like an event
  // string or chain path) or perhaps add a new callback OnElementChanged() with
  // this data
  if (pAttribute == m_Clips.GetAttribute() ||
      (pAttribute->GetOwner() != this)) {
...
void CDmeTrack::SetClipType(DmeClipType_t type) { m_ClipType = type; }

void CDmeTrack::SetCollapsed(bool state) { m_Collapsed = state; }
...
// Volume
//-----------------------------------------------------------------------------
void CDmeTrack::SetVolume(float state) { m_Volume = state; }
float CDmeTrack::GetVolume() const { return m_Volume.Get(); }
...
void CDmeTrack::RemoveClip(int i) {
  ...
  CSuppressAutoFixup suppress(this,
                              SUPPRESS_OVERLAP_FIXUP | SUPPRESS_DIRTY_ORDERING);
  m_Clips.Remove(i);
}
...
//-----------------------------------------------------------------------------
void CDmeTrack::FindClipsAtTime(DmeTime_t time, DmeClipSkipFlag_t flags,
...
void CDmeTrack::FindClipsIntersectingTime(DmeTime_t startTime,
                                          DmeTime_t endTime,
...
void CDmeTrack::FindClipsWithinTime(DmeTime_t startTime, DmeTime_t endTime,
                                    DmeClipSkipFlag_t flags,
                                    CUtlVector<CDmeClip *> &clips) const {
  if ((flags & DMESKIP_INVISIBLE) && IsCollapsed())
    return;
...
void CDmeTrack::ShiftAllClipsAfter(DmeTime_t startTime, DmeTime_t dt,
                                   bool bTestStartingTime) {
  if (dt == DmeTime_t(0))
    return;
  ...
  for (int i = 0; i < c; ++i) {
    ...
    DmeTime_t testTime =
        bTestStartingTime ? pSubClip->GetStartTime() : pSubClip->GetEndTime();
...
void CDmeTrack::ShiftAllClipsBefore(DmeTime_t endTime, DmeTime_t dt,
                                    bool bTestEndingTime) {
  if (dt == DmeTime_t(0))
    return;
  ...
  for (int i = 0; i < c; ++i) {
    CDmeClip *pSubClip = GetClip(i);
    DmeTime_t testTime =
        bTestEndingTime ? pSubClip->GetEndTime() : pSubClip->GetStartTime();
    if (endTime > testTime) {
...
//-----------------------------------------------------------------------------
void CDmeTrack::ShiftAllFilmClipsAfter(CDmeClip *pClip, DmeTime_t dt,
                                       bool bShiftClip) {
  Assert(IsFilmTrack());
...
void CDmeTrack::ShiftAllFilmClipsBefore(CDmeClip *pClip, DmeTime_t dt,
                                        bool bShiftClip) {
  Assert(IsFilmTrack());
  if (!IsFilmTrack() || (m_Clips.Count() == 0) || (dt == DmeTime_t(0)))
    return;
...
void CDmeTrack::SortClipsByStartTime() {
  // If we're not a film clip, then we haven't installed callbacks to make
  // sorting fast. The IS_SORTED flag is some random state
  ...
  for (int i = 0; i < c; ++i) {
    ...
    pSortInfo[i].m_startTime =
        pSubClip ? pSubClip->GetStartTime() : DmeTime_t::InvalidTime();
...
CDmeClip *CDmeTrack::FindFilmClipAtTime(DmeTime_t localTime) {
  ...
  for (int i = 0; i < c; ++i) {
    CDmeClip *pSubClip = GetClip(i);
    if (pSubClip && pSubClip->GetStartTime() <= localTime &&
        pSubClip->GetEndTime() > localTime)
      return pSubClip;
  }
...
//-----------------------------------------------------------------------------
CDmeClip *CDmeTrack::FindFirstFilmClipIntesectingTime(DmeTime_t localStartTime,
                                                      DmeTime_t localEndTime) {
  if (!IsFilmTrack() || (m_Clips.Count() == 0))
    return NULL;
  ...
  for (int i = 0; i < c; ++i) {
    ...
    if (!pSubClip)
      continue;
    if ((localStartTime < pSubClip->GetEndTime()) &&
        (localEndTime >= pSubClip->GetStartTime()))
...
//-----------------------------------------------------------------------------
void CDmeTrack::InsertSpaceInFilmTrack(DmeTime_t localStartTime,
                                       DmeTime_t localEndTime) {
  if (!IsFilmTrack() || (m_Clips.Count() == 0))
    return;
  ...
  CDmeClip *pClip =
      FindFirstFilmClipIntesectingTime(localStartTime, localEndTime);
  if (pClip) {
...
CDmeClip *CDmeTrack::FindPrevFilmClip(CDmeClip *pClip) {
  ...
  // Probably doesn't matter though, since there will usually not be a ton of
  // tracks
...
void CDmeTrack::FindAdjacentFilmClips(CDmeClip *pClip, CDmeClip *&pPrevClip,
                                      CDmeClip *&pNextClip) {
...
//-----------------------------------------------------------------------------
void CDmeTrack::FindAdjacentFilmClips(DmeTime_t localTime, CDmeClip *&pPrevClip,
                                      CDmeClip *&pNextClip) {
  pPrevClip = pNextClip = NULL;
...
//-----------------------------------------------------------------------------
void CDmeTrack::FillAllGapsWithSlugs(const char *pSlugName, DmeTime_t startTime,
                                     DmeTime_t endTime) {
  if (!IsFilmTrack())
    return;
  ...
  for (int i = 0; i < c; ++i) {
    ...
    if (clipStartTime > startTime) {
      ...
      CDmeFilmClip *pSlug =
          CreateSlugClip(pSlugName, startTime, clipStartTime, GetFileId());
  ...
  if (endTime > startTime) {
    ...
    CDmeFilmClip *pSlug =
        CreateSlugClip(pSlugName, startTime, endTime, GetFileId());
...
CDmeTrackGroup *GetParentTrackGroup(CDmeTrack *pTrack) {
  DmAttributeReferenceIterator_t hAttr =
      g_pDataModel->FirstAttributeReferencingElement(pTrack->GetHandle());
  for (; hAttr != DMATTRIBUTE_REFERENCE_ITERATOR_INVALID;
    ...
    CDmeTrackGroup *pTrackGroup =
        CastElement<CDmeTrackGroup>(pAttr->GetOwner());
    if (pTrackGroup)
      return pTrackGroup;
  }
  return NULL;
