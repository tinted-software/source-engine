...
//-----------------------------------------------------------------------------
// For sounds that are relative paths (instead of GameSound names), get full
// path
//-----------------------------------------------------------------------------
bool CDmeSound::ComputeSoundFullPath(char *pBuf, int nBufLen) {
  ...
  Q_snprintf(pRelativePath, sizeof(pRelativePath), "sound\\%s",
             m_SoundName.Get());
  return g_pFullFileSystem->RelativePathToFullPath(pRelativePath, "GAME", pBuf,
                                                   nBufLen) != NULL;
}
...
CDmElement *CDmeGameSound::FindOrAddPhonemeExtractionSettings() {
  ...
  CDmElement *settings =
      CreateElement<CDmElement>("PhonemeExtractionSettings", GetFileId());
  if (!settings)
    return NULL;
