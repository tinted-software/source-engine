...
CDmeMesh *CDmeMorphOperator::GetMesh() { return m_mesh.GetElement(); }
...
void CDmeMorphOperator::OnAttributeChanged(CDmAttribute *pAttribute) {
  if (pAttribute == m_mesh.GetAttribute()) {
    ...
    if (pMesh) {
#if 0 // right now, the file already contains these weights, and re-creating
      // them breaks the channel connections
...
void AddVertexAttributes(CUtlVector<CDmAttribute *> &attrs,
                         CDmElement *pObject) {
