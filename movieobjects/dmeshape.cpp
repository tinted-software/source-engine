...
void CDmeShape::Draw(const matrix3x4_t &shapeToWorld,
...
int CDmeShape::GetParentCount() const {
  ...
  while (i != DMATTRIBUTE_REFERENCE_ITERATOR_INVALID) {
    ...
    if (pDag && pAttribute->GetNameSymbol() == symShape &&
        pDag->GetFileId() == GetFileId()) {
      ++nReferringDags;
    }
...
//
//-----------------------------------------------------------------------------
void CDmeShape::GetShapeToWorldTransform(matrix3x4_t &mat,
                                         int nParentIndex /*= 0 */) const {
