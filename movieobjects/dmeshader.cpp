...
const char *CDmeShader::GetShaderName() const { return m_ShaderName; }
...
//-----------------------------------------------------------------------------
IDmAttribute *CDmeShader::AddAttributeForShaderParameter(IShader *pShader,
                                                         int nIndex) {
...
void CDmeShader::AddNewShaderParams(IShader *pShader) {
  ...
  for (i = 0; i < nCount; ++i) {
    ...
    IDmAttribute *pAttribute = NULL;
    for (pAttribute = FirstAttribute(); pAttribute;
