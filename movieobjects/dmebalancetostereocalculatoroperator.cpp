...
//-----------------------------------------------------------------------------
IMPLEMENT_ELEMENT_FACTORY(DmeBalanceToStereoCalculatorOperator,
                          CDmeBalanceToStereoCalculatorOperator);
...
void CDmeBalanceToStereoCalculatorOperator::GetInputAttributes(
...
void CDmeBalanceToStereoCalculatorOperator::GetOutputAttributes(
    CUtlVector<CDmAttribute *> &attrs) {
...
float CDmeBalanceToStereoCalculatorOperator::ComputeDefaultValue() {
  // NOTE: This is a total hack, which we expect to remove soon, when the
  // balance work is done.
...
void CDmeBalanceToStereoCalculatorOperator::Operate() {
  ...
  if (m_bSpewResult) {
    Msg("%s = l('%f') r('%f') m('%f')\n", GetName(), (float)m_result_left,
        (float)m_result_right, (float)m_result_multi);
  }
}
