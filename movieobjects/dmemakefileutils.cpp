...
IDmeMakefileUtils *GetDefaultDmeMakefileUtils() { return &s_MakefileUtils; }
...
CDmeMakefileUtils::DetermineCompileAdapter(CDmElement *pElement) {
  ...
  while (pTree) {
    ...
    for (; pCurr; pCurr = pCurr->m_pNext) {
      ...
      int nInheritanceDepth =
          pElement->GetInheritanceDepth(pCurr->m_ElementType);
      ...
      if (nBestInheritanceDepth >= 0 &&
          (nInheritanceDepth >= nBestInheritanceDepth))
        continue;
...
CDmeMakefileUtils::DetermineOpenEditorAdapter(CDmElement *pElement) {
  int nBestInheritanceDepth = -1;
...
void CDmeMakefileUtils::AddCompilationTask(CDmElement *pElement,
                                           CCompileFuncAdapterBase *pAdapter) {
  Assert(m_CompilationStep == BUILDING_STANDARD_DEPENDENCIES ||
         m_CompilationStep == BUILDING_ALL_DEPENDENCIES);
...
//-----------------------------------------------------------------------------
bool CDmeMakefileUtils::AddCompileDependencies(CDmeMakefile *pMakefile,
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    CCompileFuncAdapterBase *pAdapter =
        DetermineCompileAdapter(pDependentMakefile);
    if (pAdapter) {
      // Add dependent makefiles first
      if (!pAdapter->PerformCompilationStep(
              pDependentMakefile, bBuildAllDependencies
...
//-----------------------------------------------------------------------------
bool CDmeMakefileUtils::PerformCompilationStep(CDmElement *pElement,
                                               CompilationStep_t step) {
  // Do nothing
  return true;
}

bool CDmeMakefileUtils::PerformCompilationStep(CDmeMakefile *pMakefile,
                                               CompilationStep_t step) {
  switch (step) {
...
void CDmeMakefileUtils::StartNextCompileTask() {
  ...
  // NOTE: PerformCompilationStep is expected to call SetCompileProcess to set
  // m_hCompileProcess
  CompileInfo_t &info = m_CompileTasks[m_nCurrentCompileTask];
  bool bOk = info.m_pAdapter->PerformCompilationStep(info.m_hElement,
...
//-----------------------------------------------------------------------------
void CDmeMakefileUtils::PerformCompile(CDmElement *pElement,
                                       bool bBuildAllDependencies) {
  ...
  // Add dependent makefiles first
  m_CompilationStep = bBuildAllDependencies ? BUILDING_ALL_DEPENDENCIES
                                            : BUILDING_STANDARD_DEPENDENCIES;
...
// occurred)
//-----------------------------------------------------------------------------
int CDmeMakefileUtils::GetExitCode() { return m_nExitCode; }
...
CompilationState_t CDmeMakefileUtils::UpdateCompilation(char *pOutputBuf,
                                                        int nBufLen) {
  ...
  CompilationState_t retVal = (m_CompilationStep == AFTER_COMPILATION_SUCCEEDED)
...
//-----------------------------------------------------------------------------
bool CDmeMakefileUtils::PerformCompilationStep(CDmeMDLMakefile *pMakeFile,
                                               CompilationStep_t step) {
...
#ifdef _DEBUG
  Q_snprintf(pStudioMDLCmd, sizeof(pStudioMDLCmd),
             "%s\\studiomdl.exe -allowdebug %s", pBinDirectory,
             pMakeFile->GetFileName());
#else
  Q_snprintf(pStudioMDLCmd, sizeof(pStudioMDLCmd), "%s\\studiomdl.exe %s",
             pBinDirectory, pMakeFile->GetFileName());
#endif
...
//-----------------------------------------------------------------------------
bool CDmeMakefileUtils::PerformCompilationStep(CDmeMayaMakefile *pMakeFile,
                                               CompilationStep_t step) {
  if (step != PERFORMING_COMPILATION)
    return PerformCompilationStep(static_cast<CDmeMakefile *>(pMakeFile), step);
  ...
  case 1: // skeletal animation
    ...
    mayaCommand += " -";
    mayaCommand +=
        pDmeSourceDCCFile->m_FrameIncrement.GetAttribute()->GetName();
    mayaCommand += " ";
  ...
  for (int rootObjectIndex(0); rootObjectIndex < rootObjectCount;
       ++rootObjectIndex) {
    mayaCommand += " ";
  ...
  pMakeFile->GetSourceFullPath(pDmeSourceDCCFile, pSourcePath,
                               sizeof(pSourcePath));
  ...
  Q_snprintf(pMayaCommand, sizeof(pMayaCommand),
             "mayabatch.exe -batch -file \"%s\" -command \"%s\"", pSourcePath,
             mayaCommand.Get());
...
void CDmeMakefileUtils::OpenEditor(CDmeSourceMayaFile *pDmeSourceDCCFile) {
  CDmeMayaMakefile *pMakefile =
      FindReferringElement<CDmeMayaMakefile>(pDmeSourceDCCFile, "sources");
  if (!pMakefile)
    return;
  ...
  char pMayaCommand[1024];
  Q_snprintf(pMayaCommand, sizeof(pMayaCommand), "maya.exe -file \"%s\"",
             pSourcePath);
