...
//-----------------------------------------------------------------------------
const int *
CDmMeshUtils::BuildDataMirrorMap(CDmeVertexData *pBase, int axis,
...
//
//-----------------------------------------------------------------------------
void MergeDeltaState(CDmeMesh *pDmeMesh, CDmeVertexDeltaData *pSrcDelta,
  ...
  CDmeCombinationOperator *pDmeCombo =
      FindReferringElement<CDmeCombinationOperator>(pDmeMesh, "targets");
  if (!pDmeCombo)
    return;
...
bool CDmMeshUtils::Merge(CDmeMesh *pSrcMesh, CDmeMesh *pDstMesh,
  ...
  for (int i = 0; i < pSrcMesh->DeltaStateCount(); ++i) {
    ...
    CDmeVertexDeltaData *pDstDelta =
        pDstMesh->FindDeltaState(pSrcDelta->GetName());
    MergeDeltaState(pDstMesh, pSrcDelta, pDstDelta, nPositionOffset,
                    nNormalOffset, nWrinkleOffset);
  }
...
bool CopyField(CDmeVertexData::StandardFields_t field, CDmeVertexData *pSrcData,
  ...
  case AT_VECTOR2_ARRAY:
    ...
    break;
  case AT_VECTOR3_ARRAY:
    CopyFieldData(CDmrArrayConst<Vector>(pSrcVertexData), srcIndices, pDstData,
                  dstFieldIndex, vertexWeightMap);
    break;
  default:
    break;
...
bool CDmMeshUtils::Merge(CDmMeshComp &srcComp,
  ...
  for (int i = 0; i < nSrcCount; ++i) {
    ...
    if (vertexWeight.m_nVertexWeights == 0) {
      Warning("Warning: Merge() - No Match For Src Vertex: %f %f %f, Using "
              "Closest: %f %f %f\n",
...
bool CDmMeshUtils::CreateWrinkleDeltaFromBaseState(
    ...
    WrinkleOp wrinkleOp /* = kReplace */, CDmeMesh *pPassedMesh /* = NULL */,
    CDmeVertexData *pPassedBind /* = NULL */,
    CDmeVertexData *pPassedCurrent /* = NULL */) {
  CDmeVertexData *pBind = pPassedBind   ? pPassedBind
                          : pPassedMesh ? pPassedMesh->GetBindBaseState()
                                        : NULL;
  CDmeVertexData *pCurr = pPassedCurrent ? pPassedCurrent
                          : pPassedMesh  ? pPassedMesh->GetCurrentBaseState()
                                         : NULL;

  const CDmeMesh *pMesh =
      pPassedMesh ? pPassedMesh
  ...
  const CDmeMesh *pCurrMesh =
      pCurr ? FindReferringElement<CDmeMesh>(pCurr, "baseStates", false) : NULL;
  ...
  if (wrinkleOp == kAdd) {
    ...
    if (pWrinkleDeltaAttr) {
      ...
      if (wrinkleDeltaArray.Count()) {
        const CUtlVector<int> &wrinkleDeltaIndices =
            pDelta->GetVertexIndexData(nWrinkleIndex);
        Assert(wrinkleDeltaIndices.Count() == wrinkleDeltaArray.Count());

        pOldWrinkleData = reinterpret_cast<float *>(
            alloca(nBaseTexCoordCount * sizeof(float)));
  ...
  if (pOldWrinkleData) {
    for (int i = 0; i < nPosCount; ++i) {
      ...
      for (int j = 0; j < baseVerts.Count(); ++j) {
        ...
        if (fabs(pOldWrinkleData[nTexCoordIndex]) > 0.0001 ||
            fabs(v.x) >= (1 / 4096.0f) || fabs(v.y) >= (1 / 4096.0f) ||
            fabs(v.z) >= (1 / 4096.0f)) {
          flDelta = v.Length();
...
bool CDmMeshFaceIt::Reset(const CDmeMesh *pMesh,
  ...
  if (pMesh) {
    m_pMesh = pMesh;
    m_pVertexData = pVertexData     ? pVertexData
                    : m_pVertexData ? m_pVertexData
                                    : m_pMesh->GetCurrentBaseState();
...
//-----------------------------------------------------------------------------
ControlIndex_t
FindComboOpControlIndexForAnimSetControl(CDmeCombinationOperator *pComboOp,
                                         const char *pControlName,
                                         bool *pIsMulti /*= NULL*/) {
  const char *pMultiControlBaseName =
      pControlName ? StringAfterPrefix(pControlName, "multi_") : NULL;
  if (pIsMulti) {
