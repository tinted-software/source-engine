...
//-----------------------------------------------------------------------------
CDmeTrack *
CDmeClip::FindTrackForClip(CDmeClip *pClip,
                           CDmeTrackGroup **ppTrackGroup /*= NULL*/) const {
...
//-----------------------------------------------------------------------------
CDmeTrack *
CDmeFilmClip::FindTrackForClip(CDmeClip *pClip,
                               CDmeTrackGroup **ppTrackGroup /*= NULL*/) const {
...
void CDmeFilmClip::OnElementUnserialized() {
  ...
  if (HasAttribute("overlay") || HasAttribute("overlayalpha")) {
    Warning("CDmeFilmClip %s is an old version that has overlay and/or "
...
IVideoMaterial *CDmeFilmClip::GetCachedVideoMaterial() {
  if (m_bReloadCachedVersion) {
    if (g_pVideo) {
      ...
      if (m_AVIFile[0]) {
        m_pCachedVersion =
            g_pVideo->CreateVideoMaterial(m_AVIFile, m_AVIFile, "MOD");
      }
    }
    m_bReloadCachedVersion = false;
  }
