...
//-----------------------------------------------------------------------------
IMPLEMENT_ELEMENT_FACTORY(DmeCombinationInputControl,
...
void CDmeCombinationInputControl::OnElementUnserialized() {
  ...
  if (nWrinkleCount < nControlCount) {
    ...
    m_WrinkleScales.RemoveMultiple(nControlCount,
                                   nWrinkleCount - nControlCount);
  }
}
...
// Removes controls
//-----------------------------------------------------------------------------
bool CDmeCombinationInputControl::RemoveRawControl(
...
//-----------------------------------------------------------------------------
bool CDmeCombinationInputControl::IsStereo() const { return m_bIsStereo; }
...
//-----------------------------------------------------------------------------
bool CDmeCombinationInputControl::IsEyelid() const { return m_bIsEyelid; }
...
// Reordering controls
//-----------------------------------------------------------------------------
void CDmeCombinationInputControl::MoveRawControlUp(
    const char *pRawControlName) {
...
void CDmeCombinationInputControl::MoveRawControlDown(
    const char *pRawControlName) {
  int nIndex = FindRawControl(pRawControlName);
...
void CDmeCombinationInputControl::SetWrinkleScale(const char *pRawControlName,
...
const char *CDmeCombinationInputControl::GetEyesUpDownFlexName() const {
  const CDmAttribute *pEyesUpDownFlexAttr =
      GetAttribute("eyesUpDownFlex", AT_STRING);
...
// Expose this class to the scene database
//-----------------------------------------------------------------------------
IMPLEMENT_ELEMENT_FACTORY(DmeCombinationDominationRule,
...
void CDmeCombinationDominationRule::OnConstruction() {
  m_Dominators.Init(this, "dominators",
                    FATTRIB_HAS_CALLBACK | FATTRIB_HAS_ARRAY_CALLBACK);
  m_Suppressed.Init(this, "suppressed",
                    FATTRIB_HAS_CALLBACK | FATTRIB_HAS_ARRAY_CALLBACK);
}
...
//-----------------------------------------------------------------------------
void CDmeCombinationDominationRule::OnAttributeChanged(
    CDmAttribute *pAttribute) {
  BaseClass::OnAttributeChanged(pAttribute);

  if (pAttribute == m_Dominators.GetAttribute() ||
      pAttribute == m_Suppressed.GetAttribute()) {
...
//-----------------------------------------------------------------------------
bool CDmeCombinationDominationRule::HasString(const char *pString,
                                              const CDmaStringArray &attr) {
...
//-----------------------------------------------------------------------------
void CDmeCombinationDominationRule::AddDominator(
    const char *pDominatorControl) {
...
//-----------------------------------------------------------------------------
void CDmeCombinationDominationRule::AddSuppressed(
    const char *pSuppressedControl) {
...
// Search
//-----------------------------------------------------------------------------
bool CDmeCombinationDominationRule::HasDominatorControl(
    const char *pDominatorControl) const {
...
bool CDmeCombinationDominationRule::HasSuppressedControl(
    const char *pSuppressedControl) const {
...
//-----------------------------------------------------------------------------
// Finds the index of the input control with the specified name (and creates one
// if necessary)
//-----------------------------------------------------------------------------
void CDmeCombinationOperator::OnElementUnserialized() {
  BaseClass::OnElementUnserialized();

  // Mark all of the input as not being in their default state since we read it
  // from a file
...
// if necessary)
//-----------------------------------------------------------------------------
ControlIndex_t CDmeCombinationOperator::FindOrCreateControl(
    const char *pControlName, bool bStereo, bool bAutoAddRawControl) {
  ...
  // NOTE: the y coordinate of the control value is -1 if it's not a stereo
  // control
  CDmeCombinationInputControl *pInputControl =
      CreateElement<CDmeCombinationInputControl>(pControlName, GetFileId());
  ...
  Assert(m_InputControls.Count() ==
         m_ControlValues[COMBO_CONTROL_NORMAL].Count());
  Assert(m_InputControls.Count() ==
         m_ControlValues[COMBO_CONTROL_LAGGED].Count());
...
//-----------------------------------------------------------------------------
ControlIndex_t
CDmeCombinationOperator::FindControlIndex(const char *pControlName) {
  int nCount = m_InputControls.Count();
  for (int i = 0; i < nCount; ++i) {
    if (!m_InputControls[i])
      continue;
...
//-----------------------------------------------------------------------------
void CDmeCombinationOperator::SetControlName(ControlIndex_t nControl,
...
void CDmeCombinationOperator::UpdateDefaultValue(ControlIndex_t nControlIndex) {
  if (m_IsDefaultValue[nControlIndex]) {
    ...
    m_ControlValues[COMBO_CONTROL_NORMAL].Set(
        nControlIndex, Vector(flDefaultValue, vec.y, vec.z));
    const Vector &vec2 = m_ControlValues[COMBO_CONTROL_LAGGED][nControlIndex];
    m_ControlValues[COMBO_CONTROL_LAGGED].Set(
        nControlIndex, Vector(flDefaultValue, vec2.y, vec2.z));
  }
}
...
//-----------------------------------------------------------------------------
void CDmeCombinationOperator::AddRawControl(ControlIndex_t nControl,
                                            const char *pRawControlName) {
...
//-----------------------------------------------------------------------------
void CDmeCombinationOperator::RemoveRawControl(ControlIndex_t nControl,
                                               const char *pRawControlName) {
  CDmeCombinationInputControl *pInputControl = m_InputControls[nControl];
  Assert(pInputControl);
...
const char *CDmeCombinationOperator::GetRawControlName(ControlIndex_t nControl,
                                                       int nIndex) const {
  CDmeCombinationInputControl *pInputControl = m_InputControls[nControl];
  Assert(pInputControl);
...
float CDmeCombinationOperator::GetRawControlWrinkleScale(
    ControlIndex_t nControl, int nIndex) const {
  CDmeCombinationInputControl *pInputControl = m_InputControls[nControl];
  Assert(pInputControl);
...
float CDmeCombinationOperator::GetRawControlWrinkleScale(
    ControlIndex_t nControl, const char *pRawControlName) const {
  CDmeCombinationInputControl *pInputControl = m_InputControls[nControl];
  Assert(pInputControl);
...
//
//-----------------------------------------------------------------------------
float CDmeCombinationOperator::GetControlDefaultValue(
    ControlIndex_t nControl) const {
  CDmeCombinationInputControl *pInputControl = m_InputControls[nControl];
  Assert(pInputControl);
...
//
//-----------------------------------------------------------------------------
float CDmeCombinationOperator::GetControlBaseValue(
    ControlIndex_t nControl) const {
  CDmeCombinationInputControl *pInputControl = m_InputControls[nControl];
  Assert(pInputControl);
...
//-----------------------------------------------------------------------------
void CDmeCombinationOperator::SetWrinkleScale(ControlIndex_t nControl,
                                              const char *pRawControlName,
                                              float flWrinkleScale) {
  CDmeCombinationInputControl *pInputControl = m_InputControls[nControl];
  Assert(pInputControl);
...
// Reordering controls
//-----------------------------------------------------------------------------
void CDmeCombinationOperator::MoveControlBefore(const char *pDragControlName,
  ...
  // Have to copy because InsertAfter may reallocate memory before doing the
  // copy and therefore might be referencing garabage
...
// Reordering controls
//-----------------------------------------------------------------------------
void CDmeCombinationOperator::MoveControlAfter(const char *pDragControlName,
                                               const char *pDropControlName) {
...
// Reordering controls
//-----------------------------------------------------------------------------
void CDmeCombinationOperator::MoveRawControlUp(ControlIndex_t nControlIndex,
                                               const char *pRawControlName) {
...
// Reordering controls
//-----------------------------------------------------------------------------
void CDmeCombinationOperator::MoveRawControlDown(ControlIndex_t nControlIndex,
                                                 const char *pRawControlName) {
...
//-----------------------------------------------------------------------------
void CDmeCombinationOperator::SetStereoControl(ControlIndex_t nControlIndex,
                                               bool bIsStereo) {
  m_InputControls[nControlIndex]->SetStereo(bIsStereo);
}

bool CDmeCombinationOperator::IsStereoControl(
    ControlIndex_t nControlIndex) const {
...
void CDmeCombinationOperator::SetEyelidControl(ControlIndex_t nControlIndex,
                                               bool bIsEyelid) {
  m_InputControls[nControlIndex]->SetEyelid(bIsEyelid);
}

bool CDmeCombinationOperator::IsEyelidControl(
    ControlIndex_t nControlIndex) const {
...
//
//-----------------------------------------------------------------------------
const char *CDmeCombinationOperator::GetEyesUpDownFlexName(
    ControlIndex_t nControlIndex) const {
...
//-----------------------------------------------------------------------------
void CDmeCombinationOperator::SetControlValue(ControlIndex_t nControlIndex,
                                              float flValue,
                                              CombinationControlType_t type) {
  m_IsDefaultValue[nControlIndex] = false;
  float flMultiLevel = m_ControlValues[type][nControlIndex].z;
  m_ControlValues[type].Set(nControlIndex,
                            Vector(flValue, flValue, flMultiLevel));
}

void CDmeCombinationOperator::SetControlValue(ControlIndex_t nControlIndex,
                                              float flLevel, float flBalance,
                                              CombinationControlType_t type) {
  Assert(IsStereoControl(nControlIndex));
  m_IsDefaultValue[nControlIndex] = false;
  float flMultiLevel = m_ControlValues[type][nControlIndex].z;
  m_ControlValues[type].Set(nControlIndex,
                            Vector(flLevel, flBalance, flMultiLevel));
}
...
// Sets the value of a control
//-----------------------------------------------------------------------------
float CDmeCombinationOperator::GetControlValue(
    ControlIndex_t nControlIndex, CombinationControlType_t type) const {
  Assert(!IsStereoControl(nControlIndex));
...
// Sets the value of a control
//-----------------------------------------------------------------------------
const Vector2D &CDmeCombinationOperator::GetStereoControlValue(
    ControlIndex_t nControlIndex, CombinationControlType_t type) const {
...
//-----------------------------------------------------------------------------
void CDmeCombinationOperator::SetMultiControlLevel(
    ControlIndex_t nControlIndex, float flMultiLevel,
    CombinationControlType_t type) {
  m_IsDefaultValue[nControlIndex] = false;
  const Vector2D &value = m_ControlValues[type][nControlIndex].AsVector2D();
  m_ControlValues[type].Set(nControlIndex,
                            Vector(value.x, value.y, flMultiLevel));
}

float CDmeCombinationOperator::GetMultiControlLevel(
    ControlIndex_t nControlIndex, CombinationControlType_t type) const {
...
// remappings)
//-----------------------------------------------------------------------------
bool CDmeCombinationOperator::IsMultiControl(
    ControlIndex_t nControlIndex) const {
  return m_InputControls[nControlIndex]->RawControlCount() >= 3 ||
         IsEyelidControl(nControlIndex);
}
...
//-----------------------------------------------------------------------------
CDmeCombinationOperator::RawControlIndex_t
CDmeCombinationOperator::FindRawControlIndex(
...
bool CDmeCombinationOperator::IsRawControlStereo(const char *pRawControlName) {
  ...
  CDmeCombinationInputControl *pInputControl =
      m_InputControls[m_RawControlInfo[nIndex].m_InputControl];
...
bool CDmeCombinationOperator::IsDeltaStateStereo(const char *pDeltaStateName) {
  ...
  for (int i = 0; i < nCount; ++i) {
    CDmeCombinationInputControl *pInputControl =
        m_InputControls[m_RawControlInfo[pTemp[i]].m_InputControl];
...
//-----------------------------------------------------------------------------
bool CDmeCombinationOperator::DoesTargetContainDeltaState(
    ...
    for (int j = 0; j < nRemapCount; ++j) {
      ...
      default: {
        if (j == 0) {
          ...
          info.m_FilterRamp.Init(flPeak - flStep, flPeak, flPeak,
                                 flPeak + flStep);
        }
      } break;
      }
...
void CDmeCombinationOperator::RebuildDominatorInfo() {
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    m_DominatorInfo[k].m_DominantIndices.AddMultipleToTail(nDominatorCount,
                                                           pDominators);
    m_DominatorInfo[k].m_SuppressedIndices.AddMultipleToTail(nSuppressedCount,
                                                             pSuppressed);
  }
}
...
CDmeCombinationDominationRule *CDmeCombinationOperator::AddDominationRule() {
  CDmeCombinationDominationRule *pDominationRule =
      CreateElement<CDmeCombinationDominationRule>("rule", GetFileId());
...
//-----------------------------------------------------------------------------
CDmeCombinationDominationRule *CDmeCombinationOperator::AddDominationRule(
    int nDominatorCount, const char **ppDominatorControlNames,
    int nSuppressedCount, const char **ppSuppressedControlNames) {
...
CDmeCombinationDominationRule *CDmeCombinationOperator::AddDominationRule(
  ...
  return AddDominationRule(dominators.Count(), (const char **)dominators.Base(),
                           suppressed.Count(),
...
void CDmeCombinationOperator::RemoveDominationRule(
...
CDmeCombinationDominationRule *
CDmeCombinationOperator::GetDominationRule(int i) {
  return m_Dominators[i];
}
...
//-----------------------------------------------------------------------------
int CDmeCombinationOperator::FindDominationRule(
    CDmeCombinationDominationRule *pRule) {
  int nCount = m_Dominators.Count();
  for (int i = 0; i < nCount; ++i) {
    if (m_Dominators[i] == pRule)
...
// Rule reordering
//-----------------------------------------------------------------------------
void CDmeCombinationOperator::MoveDominationRuleUp(
    CDmeCombinationDominationRule *pRule) {
...
void CDmeCombinationOperator::MoveDominationRuleDown(
    CDmeCombinationDominationRule *pRule) {
  int nIndex = FindDominationRule(pRule);
...
//-----------------------------------------------------------------------------
void CDmeCombinationOperator::AttachChannelToControlValue(
    ControlIndex_t nControlIndex, CombinationControlType_t type,
...
//-----------------------------------------------------------------------------
int CDmeCombinationOperator::FindDeltaStateIndex(CDmAttribute *pDeltaArray,
...
//-----------------------------------------------------------------------------
int CDmeCombinationOperator::ParseDeltaName(const char *pDeltaStateName,
                                            int *pControlIndices) {
  char pBuf[512];
...
void CDmeCombinationOperator::ComputeCombinationInfo(int nIndex) {
  ...
  info.m_hDestAttribute[COMBO_CONTROL_NORMAL] =
      weights.GetAttribute()->GetHandle();
  info.m_hDestAttribute[COMBO_CONTROL_LAGGED] =
      weightsLagged.IsValid() ? weightsLagged.GetAttribute()->GetHandle()
                              : DMATTRIBUTE_HANDLE_INVALID;
...
//-----------------------------------------------------------------------------
// Remaps non-stereo -> stereo, stereo ->left/right, also adds multilevel +
// filter
//-----------------------------------------------------------------------------
void CDmeCombinationOperator::ComputeInternalControlValue(
    RawControlIndex_t nRawControlIndex, CombinationControlType_t type,
    Vector2D &value) {
  ...
  if (flValue <= info.m_FilterRamp.x || flValue >= info.m_FilterRamp.w) {
    ...
    flValue =
        RemapVal(flValue, info.m_FilterRamp.x, info.m_FilterRamp.y, 0.0f, 1.0f);
  } else if (flValue > info.m_FilterRamp.z) {
    flValue =
        RemapVal(flValue, info.m_FilterRamp.z, info.m_FilterRamp.w, 1.0f, 0.0f);
  } else {
    flValue = 1.0f;
  }
...
void CDmeCombinationOperator::ComputeLaggedInputValues() {
  ...
  for (int i = 0; i < nCount; ++i) {
    Vector vecLerp;
    VectorLerp(m_ControlValues[COMBO_CONTROL_NORMAL][i],
               m_ControlValues[COMBO_CONTROL_LAGGED][i], flFactor, vecLerp);
...
void CDmeCombinationOperator::Operate() {
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    for (CombinationControlType_t type = COMBO_CONTROL_FIRST;
         type < COMBO_CONTROL_TYPE_COUNT;
      ...
      CDmAttribute *pAttribute =
          g_pDataModel->GetAttribute(info.m_hDestAttribute[type]);
      if (!pAttribute || pAttribute->GetType() != AT_VECTOR2_ARRAY)
        continue;

      CDmrArray<Vector2D> vec2D(pAttribute);

      CombinationControlType_t useType =
          m_bSpecifyingLaggedData ? type : COMBO_CONTROL_NORMAL;
...
void CDmeCombinationOperator::GetInputAttributes(
...
void CDmeCombinationOperator::GetOutputAttributes(
    CUtlVector<CDmAttribute *> &attrs) {
  int nCount = m_CombinationInfo.Count();
  for (int i = 0; i < nCount; ++i) {
    CombinationInfo_t &info = m_CombinationInfo[i];
    for (int j = 0; j < COMBO_CONTROL_TYPE_COUNT; ++j) {
      ...
      CDmAttribute *pAttribute =
          g_pDataModel->GetAttribute(info.m_hDestAttribute[j]);
      if (!pAttribute || pAttribute->GetType() != AT_VECTOR2_ARRAY)
        continue;
...
CDmElement *CDmeCombinationOperator::GetOperationDeltaState(int nTargetIndex,
                                                            int nOpIndex) {
  ...
  int nDeltaStateIndex =
      m_CombinationInfo[nTargetIndex].m_Outputs[nOpIndex].m_nDeltaStateIndex;
  return deltaArray[nDeltaStateIndex];
}

const CUtlVector<int> &
CDmeCombinationOperator::GetOperationControls(int nTargetIndex,
                                              int nOpIndex) const {
  return m_CombinationInfo[nTargetIndex].m_Outputs[nOpIndex].m_ControlIndices;
}

int CDmeCombinationOperator::GetOperationDominatorCount(int nTargetIndex,
                                                        int nOpIndex) const {
  return m_CombinationInfo[nTargetIndex]
      .m_Outputs[nOpIndex]
      .m_DominatorIndices.Count();
}

const CUtlVector<int> &
CDmeCombinationOperator::GetOperationDominator(int nTargetIndex, int nOpIndex,
                                               int nDominatorIndex) const {
  int nIndex = m_CombinationInfo[nTargetIndex]
                   .m_Outputs[nOpIndex]
                   .m_DominatorIndices[nDominatorIndex];
...
// NOTE: This is only being used because we have no authoring path for wrinkle
// data yet
//-----------------------------------------------------------------------------
void CDmeCombinationOperator::GenerateWrinkleDeltas(CDmeShape *pShape,
                                                    bool bOverwrite) {
  ...
  if (pBindState) {
    ...
    for (int i = 0; i < nRawControlCount; ++i) {
      CDmeVertexDeltaData *pDelta =
          pMesh->FindDeltaState(m_RawControlInfo[i].m_Name);
      if (pDelta) {
        pDelta->GenerateWrinkleDelta(
            pBindState, m_RawControlInfo[i].m_flWrinkleScale, bOverwrite);
      }
    }
  }
}
...
//-----------------------------------------------------------------------------
static void CreateLaggedLog(CDmeVector2Log *pLog, CDmeVector2Log *pLaggedLog,
                            int nSamplesPerSec) {}
...
void CreateLaggedVertexAnimation(CDmeChannelsClip *pClip, int nSamplesPerSec) {
  ...
  for (int i = 0; i < nChannelCount; ++i) {
    ...
    CDmeChannel *pNewChannel =
        CreateElement<CDmeChannel>(pNewChannelName, pClip->GetFileId());
    ...
    CreateLaggedLog(static_cast<CDmeVector2Log *>(pChannel->GetLog()), pNewLog,
                    nSamplesPerSec);
  }
}
...
//
//-----------------------------------------------------------------------------
IMPLEMENT_ELEMENT_FACTORY(DmeMayaCombinationOperator,
                          CDmeMayaCombinationOperator);
...
void CDmeMayaCombinationOperator::OnConstruction() {
  ...
  m_DeltaStateWeights[COMBO_CONTROL_LAGGED].Init(this,
...
void CDmeMayaCombinationOperator::RemoveDeltaState(
    const char *pDeltaStateName) {
...
const Vector2D &CDmeMayaCombinationOperator::GetDeltaStateWeight(
    int nIndex, CombinationControlType_t type) const {
