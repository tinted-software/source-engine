...
void Bitmap_t::Init(int xs, int ys, ImageFormat imageFormat, int nStride) {
  ...
  // Check for NOP
  if (m_pBits && m_bOwnsBuffer && m_nWidth == xs && m_nHeight == ys &&
      nStride == m_nStride && nPixSize == m_nPixelSize) {
...
void Bitmap_t::SetBuffer(int nWidth, int nHeight, ImageFormat imageFormat,
...
// }
//
// bool LoadVTFFromBuffer( CUtlBuffer fileData, const char *pszDebugName =
// "buffer" )
//{
//
...
//	{
//		memcpy( PixPtr(0, y), pVTFTexture->ImageData(0, 0, 0, 0, y),
//m_nWidth*4 );
//	}
//
//	// Clean up
...
//	IVTFTexture *pVTFTexture( CreateVTFTexture() );
//	const int nFlags = TEXTUREFLAGS_NOMIP | TEXTUREFLAGS_NOLOD |
//TEXTUREFLAGS_SRGB; 	if ( !pVTFTexture->Init( m_nWidth, m_nHeight, 1,
//IMAGE_FORMAT_RGBA8888, nFlags, 1, 1 ) )
//	{
//		DestroyVTFTexture( pVTFTexture );
//		return false;
//	}
...
//	ImgUtl_StretchRGBAImage( pImgSrc->m_pBits, pImgSrc->m_nWidth,
//pImgSrc->m_nHeight, pNewData, nNewSizeX, nNewSizeY ); 	Clear(); 	m_pBits =
//pNewData; 	m_nWidth = nNewSizeX; 	m_nHeight = nNewSizeY;
// }
//
...
//	{
//		memcpy( pNewData + y*nRowSize, pImgSrc->PixPtr(x0, y0+y),
//nRowSize );
//	}
//
...
void Bitmap_t::Crop(int x0, int y0, int nWidth, int nHeight,
...
void Bitmap_t::SetPixelData(const Bitmap_t &src, int nSrcX1, int nSrcY1,
                            int nCopySizeX, int nCopySizeY, int nDestX1,
                            int nDestY1) {
  // Safety
  if (!src.IsValid()) {
    Assert(src.IsValid());
  ...
  if (nSrcX1 < 0 || nSrcY1 < 0 || nSrcX1 + nCopySizeX > src.Width() ||
      nSrcY1 + nCopySizeY > src.Height()) {
    Assert(nSrcX1 >= 0);
    Assert(nSrcY1 >= 0);
  ...
  // But we can clip the rectangle if it extends outside the destination image
