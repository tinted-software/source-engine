...
//-----------------------------------------------------------------------------
template <int type, bool bNiceFilter> class CKernelWrapper {
public:
          ...
          for (int m = 0, srcZ = startZ; m < dratio; ++m, ++srcZ) {
            ...
            for (int k = 0, srcY = startY; k < hratio; ++k, ++srcY) {
              ...
              for (int l = 0, srcX = startX; l < wratio; ++l, ++srcX) {
                // HACK: This temp variable fixes an internal compiler error in
                // vs2005
                int temp = srcX;
      ...
      for (int i = 0; i < info.m_nDestHeight; ++i) {
        ...
        for (int j = 0; j < info.m_nDestWidth; ++j, dstPixel += 4) {
          ...
          if (type == KERNEL_NORMALMAP) {
            ...
            AddAlphaToAlphaResult(kernel, info, startX, startY, startZ, flAlpha,
                                  pAlphaResult);
          } else {
            for (int ch = 0; ch < 3; ++ch)
              info.m_pDest[dstPixel + ch] =
            ...
            info.m_pDest[dstPixel + 3] = Clamp(
                ...
                (info.m_flColorScale[3] * (total[3] - info.m_flColorGoal[3])));
          }
        }
      }
...
bool ResampleRGBA32323232F(const ResampleInfo_t &info) {
  ...
  // Make sure that we aren't upscaling the image. . .we don't support that very
  // well.
  Assert(info.m_nSrcWidth >= info.m_nDestWidth);
  Assert(info.m_nSrcHeight >= info.m_nDestHeight);
        ...
        for (int nSampleX = 0; nSampleX < nSampleWidth; nSampleX++) {
          ...
          accum[2] += pSrc[((x * nSampleWidth + nSampleX) +
                            (y * nSampleHeight + nSampleY) * info.m_nSrcWidth) *
                               4 +
                           2];
          accum[3] += pSrc[((x * nSampleWidth + nSampleX) +
                            (y * nSampleHeight + nSampleY) * info.m_nSrcWidth) *
                               4 +
                           3];
        }
...
void GenerateMipmapLevelsLQ(unsigned char *pSrc, unsigned char *pDst, int width,
                            int height, ImageFormat imageFormat,
                            int numLevels) {
  CUtlMemory<unsigned char> tmpImage;
  ...
  int mipmap0Size =
      GetMemRequired(width, height, 1, IMAGE_FORMAT_RGBA8888, false);
  ...
  if (imageFormat != IMAGE_FORMAT_RGBA8888) {
    ...
    ConvertImageFormat(tmpImage.Base(), IMAGE_FORMAT_RGBA8888, pSrc,
                       imageFormat, width, height, 0, 0);
  ...
  do {
    ...
    for (int j = 0; j < dstHeight; ++j) {
      for (int i = 0; i < dstWidth; ++i) {
        ...
        pDstPixel[0] = ((unsigned int)pSrcPixel[0] +
                        (unsigned int)pSrcPixel[0 + cSrcStride] +
                        (unsigned int)pSrcPixel[0 + cSrcPitch] +
                        (unsigned int)pSrcPixel[0 + cSrcPitch + cSrcStride]) >>
                       2;
        pDstPixel[1] = ((unsigned int)pSrcPixel[1] +
                        (unsigned int)pSrcPixel[1 + cSrcStride] +
                        (unsigned int)pSrcPixel[1 + cSrcPitch] +
                        (unsigned int)pSrcPixel[1 + cSrcPitch + cSrcStride]) >>
                       2;
        pDstPixel[2] = ((unsigned int)pSrcPixel[2] +
                        (unsigned int)pSrcPixel[2 + cSrcStride] +
                        (unsigned int)pSrcPixel[2 + cSrcPitch] +
                        (unsigned int)pSrcPixel[2 + cSrcPitch + cSrcStride]) >>
                       2;
        pDstPixel[3] = ((unsigned int)pSrcPixel[3] +
                        (unsigned int)pSrcPixel[3 + cSrcStride] +
    ...
    pSrcLevel = pDstLevel;
    pDstLevel +=
        GetMemRequired(dstWidth, dstHeight, 1, IMAGE_FORMAT_RGBA8888, false);
