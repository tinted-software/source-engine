...
FloatBitMap_t::FloatBitMap_t(char const *tgafilename) {
  ...
  uint8 *pImage1Tmp = new uint8[ImageLoader::GetMemRequired(
  ...
  uint8 *pImage1 = new uint8[ImageLoader::GetMemRequired(
      ...
      for (int pass = 0; pass < 10; pass++) {
        ...
        for (int x = 1; x < Width - 1; x++) {
          for (int y = 1; y < Height - 1; y++) {
            for (int c = 0; c < 3; c++) {
              float desiredx =
                  DiffMapX.Pixel(x, y, c) + cursrc->Pixel(x + 1, y, c);
              float desiredy =
                  DiffMapY.Pixel(x, y, c) + cursrc->Pixel(x, y + 1, c);
