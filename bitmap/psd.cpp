...
//numEntries = numBytesPalette/3; 	unsigned int	numBytesImgResources; 	byte
//imgresources[ numBytesImgResources ]; = { sequence of PSDImgResEntry_t }
...
bool PSDReadFileRGBA8888(CUtlBuffer &buf, Bitmap_t &bitmap) {
  ...
  if (nCompressionType == 0) {
    ...
    unsigned int nLineLengthData =
        sizeof(unsigned short) * bitmap.Height() * nChannelsCount;
...
PSDImageResources::ResElement
PSDImageResources::FindElement(Resource eType) const {
  ResElement res;
...
PSDResFileInfo::ResFileInfoElement
PSDResFileInfo::FindElement(ResFileInfo eType) const {
