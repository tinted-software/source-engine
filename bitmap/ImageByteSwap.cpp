...
#if defined(_WIN32) && !defined(_X360) && !defined(NO_X360_XDK) &&             \
...
#pragma warning(push)
#pragma warning(                                                               \
    disable : 4458) // warning C4530: C++ exception handler used, but unwind
...
void PreConvertSwapImageData(unsigned char *pImageData, int nImageSize,
  ...
  if (!stride) {
    ...
    for (int i = 0; i < nRows; i++) {
      XGEndianSwapMemory(pImageData, pImageData, xEndian,
                         XGENDIANTYPE_GET_DATA_SIZE(xEndian), width);
      pImageData += stride;
    }
  }
#endif
...
#ifdef _X360
  // It would have been nice to use the 360 D3DFORMAT bit encodings, but the
  // codes are different for win32, and this routine is used by a win32 library
  // to manipulate 360 data, so there can be no reliance on D3DFORMAT bits
  XGENDIANTYPE xEndian;
  switch (imageFormat) {
  default:
    return;
