...
typedef void (*UserFormatToRGBA8888Func_t)(const uint8 *src, uint8 *dst,
                                           int numPixels);
typedef void (*RGBA8888ToUserFormatFunc_t)(const uint8 *src, uint8 *dst,
                                           int numPixels);
...
static void RGBA8888ToRGB888_BLUESCREEN(const uint8 *src, uint8 *dst,
                                        int numPixels);
static void RGBA8888ToBGR888_BLUESCREEN(const uint8 *src, uint8 *dst,
                                        int numPixels);
...
// static void RGBA8888ToRGBA16161616F( const uint8 *src, uint8 *dst, int
// numPixels );
...
static void RGB888_BLUESCREENToRGBA8888(const uint8 *src, uint8 *dst,
                                        int numPixels);
static void BGR888_BLUESCREENToRGBA8888(const uint8 *src, uint8 *dst,
                                        int numPixels);
...
// static void RGBA16161616FToRGBA8888( const uint8 *src, uint8 *dst, int
// numPixels );

static UserFormatToRGBA8888Func_t
GetUserFormatToRGBA8888Func_t(ImageFormat srcImageFormat) {
...
static RGBA8888ToUserFormatFunc_t
GetRGBA8888ToUserFormatFunc_t(ImageFormat dstImageFormat) {
...
static inline void GetColorBlockColorsBGRA8888(DXTColBlock *pBlock,
  ...
  // If this was a perf problem, we could optimize it. But this isn't used in
  // any hotpaths (now) so let's just do the correct but slow fp math.
  col_0->a = 0xff;
...
template <class CDestPixel>
static void ConvertFromDXT1(const uint8 *src, CDestPixel *dst, int width,
                            int height) {
...
template <class CDestPixel>
static void ConvertFromDXT5(const uint8 *src, CDestPixel *dst, int width,
                            int height) {
  int realWidth = 0;
  int realHeight = 0;
  CDestPixel *realDst = NULL;
...
template <class CDestPixel>
static void ConvertFromDXT5IgnoreAlpha(const uint8 *src, CDestPixel *dst,
...
template <class CDestPixel>
static void ConvertFromATIxN(const uint8 *src, CDestPixel *dst, int width,
  ...
  for (j = 0; j < yblocks; j++) {
    ...
    pBlock = (DXTAlphaBlock3BitLinear *)((uint8 *)pSrcScan +
...
bool ConvertToATIxN(const uint8 *src, ImageFormat srcImageFormat, uint8 *dst,
...
bool ConvertToDXTLegacy(const uint8 *src, ImageFormat srcImageFormat,
                        uint8 *dst, ImageFormat dstImageFormat, int width,
...
void CompressSTB(uint8 *pDstBytes, ImageFormat dstFmt, const uint8 *pSrcBytes,
  ...
  for (uint32 y = 0; y < cPixY; y += 4) {
    ...
    for (uint x = 0; x < cPixX; x += 4) {
      ...
      stb_compress_dxt_block(pDstBytes, (const uint8 *)srcBlock, cbWriteAlpha,
                             STB_DXT_NORMAL);
...
bool ConvertToDXT(const uint8 *src, ImageFormat srcImageFormat, uint8 *dst,
                  ImageFormat dstImageFormat, int width, int height,
                  int srcStride, int dstStride) {
  // The STB compressor (the new compressor) is faster and higher quality in
  // most cases, and has less error overall than the S3TC compressor. So use it
  // by default, unless we're working with a format that STB doesn't support.
  ...
  if (bUseNewCompressor)
    return ConvertToDXTRuntime(src, srcImageFormat, dst, dstImageFormat, width,
                               height, srcStride, dstStride);

  return ConvertToDXTLegacy(src, srcImageFormat, dst, dstImageFormat, width,
                            height, srcStride, dstStride);
}
...
void ConvertImageFormat_RGB323232F_To_RGBA8888(float *pSrcImage, uint8 *dst,
                                               int width, int height) {
  FloatBitMap_t flbm;
...
void ConvertImageFormat_RGB323232F_To_BGRA8888(float *pSrcImage, uint8 *dst,
                                               int width, int height) {
  FloatBitMap_t flbm;
  flbm.AllocateRGB(width, height);
...
bool ConvertImageFormat(const uint8 *src, ImageFormat srcImageFormat,
  ...
  if (((dstImageFormat == IMAGE_FORMAT_DXT1 ||
      ...
      (dstImageFormat == IMAGE_FORMAT_DXT5 &&
       srcImageFormat == IMAGE_FORMAT_DXT5_RUNTIME) ||
      (dstImageFormat == IMAGE_FORMAT_DXT1 &&
       srcImageFormat == IMAGE_FORMAT_DXT1_RUNTIME) ||
      (dstImageFormat == IMAGE_FORMAT_DXT5_RUNTIME &&
              ...
              dstImageFormat == IMAGE_FORMAT_ATI2N)) // ATI compressed dest
  {
    ...
    if ((srcImageFormat == dstImageFormat) ||
        ((srcImageFormat == IMAGE_FORMAT_BGRA8888) &&
         (dstImageFormat == IMAGE_FORMAT_BGRX8888))) {
      if (IsX360() && (srcStride == dstStride) &&
          (width * srcPixelSize == srcStride)) {
        // fastest path
...
void ConvertNormalMapRGBA8888ToDUDVMapUVWQ8888(const uint8 *src, int width,
                                               int height, uint8 *dst_) {
...
void ConvertNormalMapRGBA8888ToDUDVMapUVLX8888(const uint8 *src, int width,
                                               int height, uint8 *dst_) {
  unsigned const char *lastPixel = src + width * height * 4;
  char *dst = (char *)dst_; // NOTE: this is signed!!!!
...
void ConvertNormalMapRGBA8888ToDUDVMapUV88(const uint8 *src, int width,
                                           int height, uint8 *dst_) {
  unsigned const char *lastPixel = src + width * height * 4;
  char *dst = (char *)dst_; // NOTE: this is signed!!!!
...
// Image rotation
//-----------------------------------------------------------------------------
bool RotateImageLeft(const uint8 *src, uint8 *dst, int widthHeight,
...
bool RotateImage180(const uint8 *src, uint8 *dst, int widthHeight,
                    ImageFormat imageFormat) {
...
void RGBA8888ToP8(const uint8 *src, uint8 *dst, int numPixels) { Assert(0); }
...
void P8ToRGBA8888(const uint8 *src, uint8 *dst, int numPixels) { Assert(0); }
