...
SpewRetval_t DedicatedSpewOutputFunc(SpewType_t spewType,
                                     char const *pMsg); // in sys_common.cpp

//-----------------------------------------------------------------------------
// Run a single VGUI frame. if bFinished is true, run VGUIFinishedConfig()
// first.
//-----------------------------------------------------------------------------
...
bool CDedicatedAppSystemGroup::Create() {
  ...
  if (sys->LoadModules(this)) {
    ...
    // "playing" the game.
    auto inputsystem =
        (IInputSystem *)FindSystem(INPUTSYSTEM_INTERFACE_VERSION);
    if (inputsystem) {
...
bool CDedicatedAppSystemGroup::PreInit() {
  ...
  IInputSystem *inputsystem =
      (IInputSystem *)factory(INPUTSYSTEM_INTERFACE_VERSION, NULL);
  if (inputsystem) {
