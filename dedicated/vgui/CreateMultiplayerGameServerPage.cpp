...
void CCreateMultiplayerGameServerPage::OnCommand(const char *cmd) {
  ...
  if (!stricmp(cmd, "cancel")) {
    ...
    if (strlen(m_szPassword) < 3 || BadRconChars(m_szPassword)) {
      ...
      //gameData->FindKey( "FileSystem" ); 					if ( !pFileSystem ) 						Error( "Game %s
      //missing FileSystem key.", gameData->GetString( "game" ) );
      //
      //					// Mods just specify their app
      //ID (CS, HL2, HL2MP, etc), and it mounts all the necessary caches. 					int
      //iAppId = pFileSystem->GetInt( "SteamAppId" ); 					if ( iAppId )
      //					{
      //						CUtlVector<unsigned int>
      //depList; 						MountDependencies( iAppId, depList );
      //
      //						char
      //gameinfoFilename[MAX_PATH]; 						Q_snprintf( gameinfoFilename, sizeof(
      //gameinfoFilename ), "%s\\gameinfo.txt", m_iServer.gameDir );
