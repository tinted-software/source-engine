...
static volatile bool bSpewAllocations =
    false; // TODO: Register CMemoryStacks with g_pMemAlloc, so it can spew a
           // summary
...
CMemoryStack::CMemoryStack()
    : m_pBase(NULL), m_pNextAlloc(NULL), m_pAllocLimit(NULL),
      m_pCommitLimit(NULL), m_alignment(16),
#ifdef MEMSTACK_VIRTUAL_MEMORY_AVAILABLE
      m_commitSize(0), m_minCommit(0),
#ifdef _PS3
      m_pVirtualMemorySection(NULL),
#endif
#endif
      m_maxSize(0), m_bRegisteredAllocation(false) {
...
void CMemoryStack::RegisterAllocation() {
  ...
  if (GetSize()) {
    if (m_bRegisteredAllocation)
      Warning("CMemoryStack: ERROR - mismatched "
...
CPhysicalMemoryStack::CPhysicalMemoryStack()
    : m_nAlignment(16), m_nAdditionalFlags(0), m_nUsage(0), m_nPeakUsage(0),
      m_pLastAllocedChunk(NULL), m_nFirstAvailableChunk(0),
      m_nChunkSizeInBytes(0), m_ExtraChunks(32, 32), m_nFramePeakUsage(0) {
