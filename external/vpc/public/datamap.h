...
typedef enum _fieldtypes {
  ...
  FIELD_BOOLEAN, // boolean, implemented as an int, I may use this as a hint for
                 // compression
  ...
  FIELD_VMATRIX_WORLDSPACE,   // A VMatrix that maps some local space to world
                              ...
                              // transitions)
  FIELD_MATRIX3X4_WORLDSPACE, // matrix3x4_t that maps some local space to world
                              // space (translation is fixed up on level
                              // transitions)
...
// Field sizes...
//-----------------------------------------------------------------------------
template <int FIELD_TYPE> class CDatamapFieldSizeDeducer {
public:
  enum { SIZE = 0 };
...
DECLARE_FIELD_SIZE(FIELD_INTERVAL,
...
#define _FIELD(name, fieldtype, count, flags, mapname, tolerance)              \
  {fieldtype,                                                                  \
   #name,                                                                      \
   offsetof(classNameTypedef, name),                                           \
   count,                                                                      \
   flags,                                                                      \
   mapname,                                                                    \
   NULL,                                                                       \
...
#define DEFINE_EMBEDDED_ARRAY(name, count)                                     \
  {FIELD_EMBEDDED,                                                             \
   ...
   NULL,                                                                       \
   &(((classNameTypedef *)0)->name->m_DataMap),                                \
   sizeof(((classNameTypedef *)0)->name[0]),                                   \
   NULL,                                                                       \
   0,                                                                          \
   0.0f}
...
#define DEFINE_EMBEDDED_AUTO_ARRAY(name)                                       \
  {FIELD_EMBEDDED,                                                             \
   #name,                                                                      \
   offsetof(classNameTypedef, name),                                           \
   SIZE_OF_ARRAY(((classNameTypedef *)0)->name),                               \
   FTYPEDESC_SAVE,                                                             \
   NULL,                                                                       \
   NULL,                                                                       \
   NULL,                                                                       \
...
#define DEFINE_PRED_TYPEDESCRIPTION_PTR(name, fieldtype)                       \
  {FIELD_EMBEDDED,                                                             \
   ...
   offsetof(classNameTypedef, name),                                           \
   1,                                                                          \
   FTYPEDESC_SAVE | FTYPEDESC_PTR | FTYPEDESC_KEY,                             \
   NULL,                                                                       \
   NULL,                                                                       \
   NULL,                                                                       \
   &fieldtype::m_PredMap}
...
#define DEFINE_INPUT(name, fieldtype, inputname)                               \
  {fieldtype,                                                                  \
   ...
   offsetof(classNameTypedef, name),                                           \
   1,                                                                          \
   FTYPEDESC_INPUT | FTYPEDESC_SAVE | FTYPEDESC_KEY,                           \
   inputname,                                                                  \
   NULL,                                                                       \
   NULL,                                                                       \
   NULL,                                                                       \
...
#define DEFINE_FUNCTION_RAW(function, func_type)                               \
  {FIELD_VOID,                                                                 \
   nameHolder.GenerateName(#function),                                         \
   NULL,                                                                       \
   1,                                                                          \
...
#define FTYPEDESC_OUTPUT                                                       \
  0x0010 // This field propagates its value to all targets whenever it changes
...
#define FTYPEDESC_NOERRORCHECK                                                 \
  0x0400 // The field is part of the prediction typedescription, but doesn't get
         // compared when checking for errors
...
template <typename T> inline void DataMapAccess(T *ignored, datamap_t **p) {
  *p = &T::m_DataMap;
}
