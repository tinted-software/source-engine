...
// sequentially
//-----------------------------------------------------------------------------
template <class T> class CUtlFixedMemory {
public:
...
template <class T> CUtlFixedMemory<T>::~CUtlFixedMemory() { Purge(); }

//-----------------------------------------------------------------------------
// Fast swap - WARNING: Swap invalidates all ptr-based indices!!!
//-----------------------------------------------------------------------------
template <class T> void CUtlFixedMemory<T>::Swap(CUtlFixedMemory<T> &mem) {
...
// element access
//-----------------------------------------------------------------------------
template <class T> inline T &CUtlFixedMemory<T>::operator[](int i) {
  Assert(IsIdxValid(i));
  return *(T *)i;
}

template <class T> inline const T &CUtlFixedMemory<T>::operator[](int i) const {
  Assert(IsIdxValid(i));
  return *(T *)i;
}

template <class T> inline T &CUtlFixedMemory<T>::Element(int i) {
  Assert(IsIdxValid(i));
  return *(T *)i;
}

template <class T> inline const T &CUtlFixedMemory<T>::Element(int i) const {
  Assert(IsIdxValid(i));
  return *(T *)i;
}
...
// Size
//-----------------------------------------------------------------------------
template <class T> inline int CUtlFixedMemory<T>::NumAllocated() const {
...
//-----------------------------------------------------------------------------
template <class T> inline bool CUtlFixedMemory<T>::IsIdxValid(int i) const {
#ifdef _DEBUG
...
template <class T> void CUtlFixedMemory<T>::Grow(int num) {
  if (num <= 0)
    return;
...
// Makes sure we've got at least this much memory
//-----------------------------------------------------------------------------
template <class T> inline void CUtlFixedMemory<T>::EnsureCapacity(int num) {
...
// Memory deallocation
//-----------------------------------------------------------------------------
template <class T> void CUtlFixedMemory<T>::Purge() {
  if (!m_pBlocks)
    return;
