...
public:
  ...
  // can later assign the keyvalues to be deleted with Assign(pKeyValues).
  //
  class AutoDelete {
  public:
  ...
  void InsertSubKey(int nIndex,
                    KeyValues *pSubKey); // Inserts the given sub-key before the
                                         // Nth child location
  bool ContainsSubKey(
  ...
  void
  ElideSubKey(KeyValues *pSubKey); // Removes a subkey but inserts all of its
                                   // children in its place, in-order (flattens
                                   // a tree, like firing a manager!)
  ...
  enum MergeKeyValuesOp_t {
    ...
    MERGE_KV_BORROW, // update values only update existing keys in storage, keys
                     // in update that do not exist in storage are discarded
  };
