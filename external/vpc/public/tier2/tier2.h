...
struct FadeData_t {
  float m_flPixelMin; // Size (height in pixels) above which objects start to
                      // fade in
  float m_flPixelMax; // Size (height in pixels) above which objects are fully
                      // faded in
  float m_flWidth;    // Reference screen res w.r.t which the above pixel values
                      // were chosen
  float m_flFadeDistScale; // Scale factor applied before entity distance-based
                           // fade is calculated
};
