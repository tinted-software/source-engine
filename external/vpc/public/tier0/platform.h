...
template <typename FUNCPTR_TYPE> class CDynamicFunction {
public:
...
//  if you don't call
// EndWatchdogTimer within nn seconds, the program will kick off an exception.
// This is for making sure that hung dedicated servers abort (and restart)
// instead of staying hung. Calling EndWatchdogTimer more than once or when
// there is no active watchdog is fine. Only does anything under linux right
// now. It should be possible to implement this functionality in windows via a
// thread, if desired.
...
ResetBaseTime(void); // reset plat_floattime to 0 for a subprocess
#else
...
#endif
PLATFORM_INTERFACE const char *
GetPlatformSpecificFileName(const char *FileName);
