...
#else
template <class _Ty> class CNullStatsGathererAllocator {
...
public:
  ...
  // helper code
  void
  PushSubTree(const CCallStackStatsGatherer_Standardized_t &SubTree,
...
template <class STATSTRUCT> class CCallStackStatsGatherer_StructAccessor_Base {
public:
...
protected:
  uint32 m_iEntryIndex; // index of the stat entry we want in the vector. Stored
...
// a handler here. 						You'll need to not only call GetEntry(), but lock/unlock the
...
#pragma pack(pop)
  ...
  void KeepSubTree(CCallStackStatsGatherer_Standardized_t &SubTree) {
    ...
    for (StoredSubTreeVector_t::iterator treeIter = m_StoredSubTrees.begin();
  ...
  uint32 PatchInSubTrees(void *const CallStackIn[CAPTUREDCALLSTACKLENGTH],
    ...
    // trees:\n=======================ONE=======================\n\t%s\n",
    // szTemp ); TranslateStackInfo( treeIter->Stack, CAPTUREDCALLSTACKLENGTH,
...
CCallStackStatsGatherer<STATSTRUCT, CAPTUREDCALLSTACKLENGTH,
                        STACKACQUISITIONFUNCTION, STATMUTEXHANDLER,
                        TEMPLATIZEDMEMORYALLOCATOR>::GetEntry(uint32
                                                                  iEntryIndex) {
#if defined(ENABLE_STACK_STATS_GATHERING)
  return CCallStackStatsGatherer_StructAccessor_Base<STATSTRUCT>(Standardized(),
                                                                 iEntryIndex);
...
template <class STATSTRUCT, size_t CAPTUREDCALLSTACKLENGTH,
          FN_GetCallStack STACKACQUISITIONFUNCTION, typename STATMUTEXHANDLER,
          template <typename T> class TEMPLATIZEDMEMORYALLOCATOR>
const CCallStackStatsGatherer_FunctionTable_t &
CCallStackStatsGatherer<STATSTRUCT, CAPTUREDCALLSTACKLENGTH,
                        STACKACQUISITIONFUNCTION, STATMUTEXHANDLER,
...
void CCallStackStatsGatherer<STATSTRUCT, CAPTUREDCALLSTACKLENGTH,
  ...
  memcpy(pushVal.Stack, PushStack.pStack,
         MIN(CAPTUREDCALLSTACKLENGTH, PushStack.iValidEntries) *
             sizeof(void *));
...
void CCallStackStatsGatherer<
  ...
  for (PushedSubTreeVector_t::reverse_iterator treeIter =
    ...
    SyncMutexes(void *pParent, bool bLock) // true for lock, false for unlock
{
  ...
  if (bLock) {
    ...
    for (StoredSubTreeVector_t::iterator treeIter =
...
public:
  ...
  STATSTRUCT *
  GetStruct(void) // do not hold this pointer outside the lock period
  {
