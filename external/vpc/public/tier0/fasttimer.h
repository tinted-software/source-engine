...
public:
  ...
  const CCycleCount &
  GetDuration() const; // Get the elapsed time between Start and End calls.
  CCycleCount
  GetDurationInProgress() const; // Call without ending. Not that cheap.
...
#if !COMPILER_GCC
#pragma warning(push)
#pragma warning(disable : 4189) // warning C4189: local variable is initialized
                                // but not referenced
#endif
