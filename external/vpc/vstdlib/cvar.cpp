...
void CCvar::RegisterConCommand(ConCommandBase *variable) {
  ...
  if (pOther) {
    if (variable->IsCommand() || pOther->IsCommand()) {
      Warning("WARNING: unable to link %s and %s because one or more is a "
              "ConCommand.\n",
      ...
      if (s_pCVarQuery->AreConVarsLinkable(pChildVar, pParentVar)) {
        ...
        if (pChildVar->m_pszDefaultValue && pParentVar->m_pszDefaultValue &&
          ...
          if (V_stricmp(pChildVar->m_pszDefaultValue,
            ...
            Warning("Parent and child ConVars with different default values! "
        ...
        for (int k = 0; k < ARRAYSIZE(nFlags); ++k) {
          if ((pChildVar->m_nFlags & nFlags[k]) !=
            ...
            Warning("Convar %s has conflicting %s flags (child: %s%s, parent: "
                    "%s%s, parent wins)\n",
                    variable->GetName(), szFlags[k],
                    (pChildVar->m_nFlags & nFlags[k]) ? "has " : "no ",
                    szFlags[k],
...
//			WITH a check to see if the element already exists within
//the hash.
//-----------------------------------------------------------------------------
CConCommandHash::CCommandHashHandle_t
CConCommandHash::Insert(ConCommandBase *cmd) {
...
//-----------------------------------------------------------------------------
CConCommandHash::CCommandHashHandle_t
CConCommandHash::FastInsert(ConCommandBase *cmd) {
...
//-----------------------------------------------------------------------------
CConCommandHash::CCommandHashHandle_t
CConCommandHash::Find(const char *name, HashKey_t hashkey) const {
...
//-----------------------------------------------------------------------------
CConCommandHash::CCommandHashHandle_t
CConCommandHash::Find(const ConCommandBase *cmd) const {
