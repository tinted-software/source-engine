...
public:
  ...
  inline ConCommandBase *const &
  operator[](const CCommandHashIterator_t &iter) const {
    return (*this)[iter.handle];
  }
...
inline CConCommandHash::CCommandHashHandle_t
CConCommandHash::Find(const char *name) const {
  return Find(name, HashStringCaseless(name));
}

inline ConCommandBase *&
CConCommandHash::operator[](CCommandHashHandle_t hHash) {
  return (m_aDataPool[hHash].m_Data);
}

inline ConCommandBase *const &
CConCommandHash::operator[](CCommandHashHandle_t hHash) const {
  return (m_aDataPool[hHash].m_Data);
}
...
inline CConCommandHash::HashKey_t
CConCommandHash::Hash(const ConCommandBase *cmd) {
