...
static bool HTSupported(void) {
  ...
  enum {
    ...
    FAMILY_ID_PENTIUM_PRO =
        0x0600, //            P Pro, P II, P III, P M, Celeron M, Core Duo, Core
                //            Solo, Core2 Duo, Core2 Extreme, P D, Xeon model F,
                //            also 45-nm : Intel Atom, Core i7, Xeon MP ; see
                //            Intel Processor Identification and the CPUID
                //            instruction pg 20,21
...
int LogicalProcessorsPerCore() {
  ...
  if (nFn1_Edx & HTT) {
        ...
        1) // Has Hyper-Threading OR Core Multi-Processing not been enabled ?
    {
      ...
      return 1; // HT was turned off, for all intents and purposes in our engine
                // it means one logical CPU per core
    }
...
const CPUInformation &GetCPUInformation() {
  ...
  if (pi.m_nPhysicalProcessors == 0 && pi.m_nLogicalProcessors == 0) {
    Assert(!"Sergiy: apparently I didn't fix some CPU detection code "
            "completely. Let me know and I'll do my best to fix it soon.");
    pi.m_nPhysicalProcessors = 1;
