...
void *g_AllocRegions[] = {
    ...
    create_mspace(100 * 1024 * 1024, 1),
      ...
      // too much overhead. need to try lifetime-based management [6/9/2009 tom]
...
inline void heapstats_internal(FILE *pFile) {
  ...
  for (int i = 0; i < ARRAYSIZE(g_AllocRegions); i++) {
    ...
    _snprintf(
        ...
        "     %d:arena         -%10d (non-mmapped space allocated from "
        "system)\n"
...
//
//-----------------------------------------------------------------------------
template <typename T> inline T MemAlign(T val, unsigned alignment) {
...
template <typename CAllocator> void *CSmallBlockPool<CAllocator>::Alloc() {
  SharedData_t *pSharedData = GetSharedData();
...
template <typename CAllocator> void CSmallBlockPool<CAllocator>::Free(void *p) {
  SharedData_t *pSharedData = GetSharedData();
...
size_t CSmallBlockPool<CAllocator>::Compact(bool bIncremental) {
  ...
  pSharedData->m_Lock.UnlockWrite();
  if (bIsCorrupt) {
    Warning("***** HEAP IS CORRUPT (free compromised for block size %d,in %s "
            "heap, possible write after free *****)\n",
            m_nBlockSize,
...
template <typename CAllocator> bool CSmallBlockPool<CAllocator>::Validate() {
#ifdef NO_SBH
  return true;
#else
  int invalid = 0;
...
//
//-----------------------------------------------------------------------------
template <typename CAllocator> CSmallBlockHeap<CAllocator>::CSmallBlockHeap() {
...
void *CSmallBlockHeap<CAllocator>::Realloc(void *p, size_t nBytes) {
  ...
  if (!pNewBlock) {
    pNewBlock = MemAlloc_Alloc(nBytes); // Call back out so blocks can move from
                                        // the secondary to the primary pools
  }
...
template <typename CAllocator> void CSmallBlockHeap<CAllocator>::Free(void *p) {
...
template <typename CAllocator>
CSmallBlockPool<CAllocator> *
CSmallBlockHeap<CAllocator>::FindPool(size_t nBytes) {
...
template <typename CAllocator> bool CSmallBlockHeap<CAllocator>::Validate() {
  bool valid = true;
  for (int i = 0; i < NUM_POOLS; i++) {
...
size_t LMDComputeHeaderSize(size_t align = 0) {
  ...
  if (align > LMD_MAX_ALIGN)
    s_StdMemAlloc.SetCRTAllocFailed(
        align); // TODO: could convert alignment to exponent to get around this,
                // or use a flag for alignments over 1KB or 1MB...
...
#ifdef MEMALLOC_SUPPORTS_ALIGNED_ALLOCATIONS
INTERNAL_INLINE void *
CStdMemAlloc::InternalAllocAligned(int region, size_t nSize, size_t align) {
...
#ifdef MEMALLOC_SUPPORTS_ALIGNED_ALLOCATIONS
INTERNAL_INLINE void *
CStdMemAlloc::InternalReallocAligned(void *pMem, size_t nSize, size_t align) {
  if (!pMem) {
...
IVirtualMemorySection *
CStdMemAlloc::AllocateVirtualMemorySection(size_t numMaxBytes) {
