...
void InitTime(struct timeval &tp) { secbase = tp.tv_sec; }
...
PLATFORM_INTERFACE const tchar *Plat_GetCommandLine() { return g_CmdLine; }
...
void Plat_ExitProcess(int nCode) { _exit(nCode); }
...
void EndWatchdogTimer(void) { alarm(0); }
...
size_t ApproximateProcessMemoryUsage(void) {
  ...
  if (pFile) {
    int nSize, nTotalProgramSize, nResident, nResidentSetSize, nShare,
        nSharedPagesTotal, nDummy0;
    if (fscanf(pFile, "%d %d %d %d %d %d %d", &nSize, &nTotalProgramSize,
               &nResident, &nResidentSetSize, &nShare, &nSharedPagesTotal,
               &nDummy0)) {
      nRet = 4096 * nSize;
    }
    fclose(pFile);
...
size_t ApproximateProcessMemoryUsage(void) { return 0; }
