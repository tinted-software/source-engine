...
CLoggingSystem::CLoggingSystem()
    : m_nChannelCount(0), m_nChannelTagCount(0), m_nTagNamePoolIndex(0),
...
LoggingResponse_t CLoggingSystem::LogDirect(LoggingChannelID_t channelID,
  ...
  switch (response) {
  case LR_DEBUGGER:
    // Asserts put the debug break in the macro itself so the code breaks at the
    // failure point.
...
CLoggingSystem::LoggingChannel_t *
CLoggingSystem::GetChannel(LoggingChannelID_t channelID) {
  Assert(IsValidChannelID(channelID));
  return &m_RegisteredChannels[channelID];
}

const CLoggingSystem::LoggingChannel_t *
CLoggingSystem::GetChannel(LoggingChannelID_t channelID) const {
  Assert(IsValidChannelID(channelID));
  return &m_RegisteredChannels[channelID];
}
...
const CLoggingSystem::LoggingChannel_t *
LoggingSystem_GetChannel(LoggingChannelID_t channelIndex) {
