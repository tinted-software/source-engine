...
template <typename CAllocator> class CSmallBlockPool {
public:
  CSmallBlockPool() {
    m_nBlockSize = 0;
    m_nCommittedPages = 0;
...
public:
  ...
  virtual IVirtualMemorySection *
  AllocateVirtualMemorySection(size_t numMaxBytes);
...
#error
  ...
  template <size_t SIZE_MB, bool bPhysical> class CFixedAllocator {
  public:
    enum {
      BYTES_PAGE = (16 * 1024),
...
#error
  ...
  typedef CSmallBlockHeap<CFixedAllocator<MBYTES_SECONDARY_SBH, false>>
      CFixedVirtualSmallBlockHeap; // @TODO: move back into above heap if number
                                   // stays at 16 [7/15/2009 tom]
#endif
