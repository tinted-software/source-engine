...
bool CPropertyStates::SetProperty(ToolProperty_t *pToolProperty,
  ...
  case PT_DEPRECATED:
    g_pVPC->VPCError("SetProperty: Property %s has been deprecated and is no "
                     "longer supported!",
                     pToolProperty->m_ParseString.Get());
    break;
...
void CVCProjGenerator::FileExcludedFromBuild(bool bExcluded) {
  if (!m_pFileConfig) {
    g_pVPC->VPCSyntaxError("Cannot set $ExcludedFromBuild unless in a $File "
...
void CVCProjGenerator::HandleProperty(const char *pPropertyName,
  ...
  default:
    g_pVPC->VPCError("HandleProperty: No support for Tool:%s Property:%s - "
                     "requires implementation",
                     g_pVPC->KeywordToName(m_nActivePropertySection),
                     pPropertyName);
  }
...
configKeyword_e
CVCProjGenerator::SetPS3VisualStudioIntegrationType(configKeyword_e eKeyword) {
