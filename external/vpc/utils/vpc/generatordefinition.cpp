...
CGeneratorDefinition::CGeneratorDefinition() { Clear(); }
...
void CGeneratorDefinition::IterateAttributesKey(ToolProperty_t *pProperty,
  ...
  if (!V_stricmp(pAttributeName, "type")) {
    ...
    } else if (!V_stricmp(pValue, "deprecated") ||
               !V_stricmp(pValue, "donotuse")) {
      pProperty->m_nType = PT_DEPRECATED;
    } else {
      // unknown
      g_pVPC->VPCError("Unknown type '%s' in '%s'", pValue,
    ...
    for (KeyValues *pKV = pAttributesKV->GetFirstSubKey(); pKV;
         pKV = pKV->GetNextKey()) {
      const char *pOrdinalName = pKV->GetName();
      const char *pOrdinalValue = pKV->GetString();
      if (!pOrdinalValue[0]) {
        g_pVPC->VPCError("Unknown ordinal value for name '%s' in '%s'",
                         pOrdinalName, pProperty->m_ParseString.Get());
      }
    ...
    g_pVPC->VPCError("Unknown attribute '%s' in '%s'", pAttributeName,
                     pProperty->m_ParseString.Get());
  }
}

void CGeneratorDefinition::IteratePropertyKey(GeneratorTool_t *pTool,
                                              KeyValues *pPropertyKV) {
...
void CGeneratorDefinition::AssignIdentifiers() {
  ...
  for (int i = 0; i < m_Tools.Count(); i++) {
    ...
    if (keyword == KEYWORD_UNKNOWN) {
      g_pVPC->VPCError("Unknown Tool Keyword '%s' in '%s'",
                       pTool->m_ParseString.Get(), m_ScriptName.Get());
    }
    ...
    for (int j = 0; j < pTool->m_Properties.Count(); j++) {
      ...
      if (pProperty->m_nType == PT_IGNORE ||
          pProperty->m_nType == PT_DEPRECATED) {
        continue;
      }
      ...
      if (!bFound) {
        g_pVPC->VPCError("Could not find PROPERTYNAME( %s, %s ) for %s",
                         prefixString.Get(), pPropertyName, m_ScriptName.Get());
      }
    }
  }
  ...
  if (g_pVPC->IsVerbose()) {
    for (int i = 0; i < usedPropertyNames.Count(); i++) {
      if (!usedPropertyNames[i]) {
        g_pVPC->VPCWarning("Unused PROPERTYNAME( %s, %s ) in %s",
                           m_pPropertyNames[i].m_pPrefixName,
                           m_pPropertyNames[i].m_pPropertyName,
                           m_ScriptName.Get());
      }
    }
  }
...
void CGeneratorDefinition::LoadDefinition(const char *pDefnitionName,
                                          PropertyName_t *pPropertyNames) {
  Clear();

  m_pPropertyNames = pPropertyNames;
  g_pVPC->GetScript().PushScript(
      CFmtStr("vpc_scripts\\definitions\\%s", pDefnitionName));

  // project definitions are KV format
  KeyValues *pScriptKV = new KeyValues(g_pVPC->GetScript().GetName());

  pScriptKV->LoadFromBuffer(g_pVPC->GetScript().GetName(),
                            g_pVPC->GetScript().GetData());

  m_ScriptName = g_pVPC->GetScript().GetName();
  m_ScriptCRC = CRC32_ProcessSingleBuffer(
      g_pVPC->GetScript().GetData(), strlen(g_pVPC->GetScript().GetData()));
  ...
  g_pVPC->VPCStatus(false, "Definition: '%s' Version: %s", m_NameString.Get(),
                    m_VersionString.Get());
...
ToolProperty_t *CGeneratorDefinition::GetProperty(configKeyword_e keyword,
