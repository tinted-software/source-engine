...
public:
  ...
  GenerateSolutionFile(const char *pSolutionFilename,
    ...
    fprintf(fp, "# Disable built-in rules/variables. We don't depend on them, "
                "and they slow down make processing.\n");
    ...
    if (V_stristr(pTargetPlatformName, "64")) {
      fprintf(fp, "    export CHROOT_NAME ?= $(subst /,_,$(dir $(abspath "
      ...
      g_pVPC->VPCError("TargetPlatform (%s) doesn't seem to be 32 or 64 bit, "
                       "can't configure chroot parameters",
                       pTargetPlatformName);
    }
    fprintf(fp,
            "    CHROOT_CONF := /etc/schroot/chroot.d/$(CHROOT_NAME).conf\n");
    fprintf(fp, "    CHROOT_DIR := $(abspath $(dir $(lastword "
    ...
    fprintf(fp, "        CPUS := $(shell $(TOOL_PATH)grep processor "
                "/proc/cpuinfo | $(TOOL_PATH)wc -l)\n");
    fprintf(fp, "    endif\n");
    ...
    fprintf(fp, "# make VALVE_NO_PROJECT_DEPS 1 or empty (so "
    ...
    fprintf(fp, "\t$(MAKE) -f $(lastword $(MAKEFILE_LIST)) -j$(MAKE_JOBS) "
                "all-targets\n\n");
    ...
    fprintf(fp, "# this is a bit over-inclusive, but the alternative (actually "
                "adding each referenced c/cpp/h file to\n");
    fprintf(fp, "# the tags file) seems like more work than it's worth.  feel "
                "free to fix that up if it bugs you. \n");
    fprintf(fp, "TAGS:\n");
    ...
    fprintf(fp, ".PHONY: TAGS showtargets regen showregen clean cleantargets "
