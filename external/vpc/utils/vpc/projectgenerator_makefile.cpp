...
public:
      ...
      for (int iSpecific = pFile->m_Configurations.First();
        ...
        if (pPrecompiledHeaderOption &&
          ...
          if (!pUsePCHThroughFile) {
            g_pVPC->VPCError("File %s creates a precompiled header in config "
                             "%s but no UsePCHThroughFile option specified.",
                             pFile->m_Filename.String(),
                             pSpecific->GetConfigName());
          }
          ...
          if (m_Lookup.Find(sLookup) != m_Lookup.InvalidIndex()) {
            g_pVPC->VPCError("File %s has UsePCHThroughFile of %s but another "
                             "file already does.",
  ...
  CFileConfig *
  FindFileThatCreatesPrecompiledHeader(const char *pConfigName,
                                       const char *pUsePCHThroughFile) {
    char sLookup[1024];
      ...
      if (IsLibraryFile(pFilename)) {
        ...
        if ((!sImportLibraryFile[0] ||
             V_stricmp(sImportLibraryFile,
                       pFilename)) && // only link this as a library if it isn't
                                      // our own output!
