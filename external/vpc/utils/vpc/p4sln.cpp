...
static const char *FixPerforceFilename(const char *filename) {
  if (filename && V_strlen(filename) > 2 &&
      !V_strnicmp(filename + 1, ":\\\\", 3)) {
    // strip out the first backslash
    static char newFilename[MAX_PATH];
    V_snprintf(newFilename, sizeof(newFilename), "%c:%s", filename[0],
               &filename[3]);
...
static void
GetChangelistFilenames(CUtlVector<int> &changelists,
                       CUtlVector<CUtlString> &changelistFilenames) {
  // P4 interface didn't initalize in main - abort
  if (!p4) {
    g_pVPC->VPCWarning("P4SLN: Perforce interface not available. Unable to "
                       "generate solution from the given Perforce changelist.");
    return;
  }
  ...
  bool bIncludeAllChangelists =
      (changelists.Find(-1) != changelists.InvalidIndex());

  for (int i = 0; i < fileList.Count(); i++) {
    if (bIncludeAllChangelists || changelists.Find(fileList[i].m_iChangelist) !=
                                      changelists.InvalidIndex()) {
...
static void
AddAdditionalDependencies(CUtlVector<CDependency_Project *> &projects,
  ...
  for (int nProject = 0; nProject < projects.Count(); nProject++) {
    ...
    for (int nDependency = 0;
         nDependency < pCurProject->m_AdditionalProjectDependencies.Count();
         nDependency++) {
      const char *pLookingFor =
          pCurProject->m_AdditionalProjectDependencies[nDependency].String();

      // Search for a match in allProjects.
      int nFound =
          CDependency_Project::FindByProjectName(allProjects, pLookingFor);
      if (nFound == -1) {
        g_pVPC->VPCError("P4SLN: Project %s lists '%s' in its "
                         "$AdditionalProjectDependencies, but there is no "
...
static void
GetProjectsDependingOnFiles(CProjectDependencyGraph &dependencyGraph,
  ...
  for (int iFile = 0; iFile < filenames.Count(); iFile++) {
    CDependency *pFile =
        dependencyGraph.FindDependency(filenames[iFile].String());
    if (!pFile) {
      char szRelative[MAX_PATH];
      if (!V_MakeRelativePath(filenames[iFile].String(),
                              g_pVPC->GetSourcePath(), szRelative,
                              sizeof(szRelative))) {
        V_strncpy(szRelative, filenames[iFile].String(), sizeof(szRelative));
      }

      // This probably means their build commands on the command line didn't
      // include any projects that included this file.
      g_pVPC->VPCWarning("%s is not found in the projects searched.",
                         szRelative);
      continue;
    }

    // Now see which projects depend on this file.
    for (int iProject = 0; iProject < dependencyGraph.m_Projects.Count();
         iProject++) {
      CDependency_Project *pProject = dependencyGraph.m_Projects[iProject];

      if (pProject->DependsOn(
              pFile, k_EDependsOnFlagCheckNormalDependencies |
  ...
  // the user can explicitly provide a set of groups to narrow the wide
  // dependency target
  ...
  if (groupRestrictions.Count()) {
    ...
    for (int i = 0; i < groupRestrictions.Count(); i++) {
      ...
      if (!g_pVPC->GetProjectsInGroup(projectIndices,
                                      groupRestrictions[i].Get())) {
        g_pVPC->VPCError("No projects found in group '%s'.",
                         groupRestrictions[i].Get());
      }
  ...
  // Make sure that each of the dependent projects are members of the restricted
  // groups, otherwise prevent their inclusion.
  CUtlVector<int> doomedProjectIndices;
  if (allowedProjectIndices.Count()) {
    for (int j = 0; j < projects.Count(); j++) {
      // find the target project in the allowed set
      if (allowedProjectIndices.Find(projects[j]->m_iProjectIndex) ==
          allowedProjectIndices.InvalidIndex()) {
        // the target project is not in the allowed set
        // add in descending order so indices can be properly removed below from
        // largest index to smallest
        doomedProjectIndices.AddToHead(j);
      }
    }

    // Remove the projects that are not part of the restrict-to-groups
    // Indexes were added in descending order, so removal is actually from the
    // end, truncating the set
...
static void UpdateProjects(CUtlVector<CDependency_Project *> &projects) {
  for (int iProject = 0; iProject < projects.Count(); iProject++) {
    ...
    if (g_pVPC->IsForceGenerate() ||
      ...
      g_pVPC->ParseProjectScript(pDependency->m_szStoredScriptName, 0, false,
                                 true);
    }
  }
}
...
void GenerateSolutionForPerforceChangelist(
  ...
  int nDepFlags =
      BUILDPROJDEPS_FULL_DEPENDENCY_SET | BUILDPROJDEPS_CHECK_ALL_PROJECTS;
  ...
  // Add g_targetProjects, which will include any other projects that they added
  // on the command line with +tier0 *engine syntax.
  CUtlVector<CDependency_Project *> commandLineProjects;
  dependencyGraph.TranslateProjectIndicesToDependencyProjects(
      g_pVPC->m_TargetProjects, commandLineProjects);
