...
void CProjectGenerator_CodeLite::GenerateCodeLiteProject(
        ...
        pCollector->GetProjectName().String());
  {
    ...
    Write("<Settings Type=\"Dynamic Library\">\n");
    {
      ...
      --m_nIndent;
      Write("</GlobalSettings>\n");
      Write("<Configuration Name=\"Debug\" CompilerType=\"gnu g++\" "
            ...
            "BuildResWithGlobalSettings=\"append\">\n");
      {
        ...
        {
          ++m_nIndent;
          Write("<RebuildCommand>make CFG=debug -f %s clean "
                "all</RebuildCommand>\n",
                pMakefileFilename);
          Write("<CleanCommand>make CFG=debug -f %s clean</CleanCommand>\n",
                pMakefileFilename);
          Write("<BuildCommand>make CFG=debug -f %s -j `getconf "
      ...
      Write("<Configuration Name=\"Release\" CompilerType=\"gnu g++\" "
            "DebuggerType=\"GNU gdb debugger\" Type=\"Dynamic Library\" "
            "BuildCmpWithGlobalSettings=\"append\" "
            "BuildLnkWithGlobalSettings=\"append\" "
            "BuildResWithGlobalSettings=\"append\">\n");
      {
        ...
        Write("<CustomBuild Enabled=\"yes\">\n");
        {
          ...
          Write("<CleanCommand>make -f %s clean</CleanCommand>\n",
                pMakefileFilename);
          Write("<BuildCommand>make -f %s -j `getconf "
                "_NPROCESSORS_ONLN`</BuildCommand>\n",
                pMakefileFilename);
          Write("<WorkingDirectory>$(ProjectPath)</WorkingDirectory>\n");
          --m_nIndent;
