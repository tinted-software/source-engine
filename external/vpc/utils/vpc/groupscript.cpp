...
//
//-----------------------------------------------------------------------------
groupTagIndex_t VPC_Group_FindOrCreateGroupTag(const char *pName,
                                               bool bCreate) {
...
void VPC_GroupKeyword_Group() {
  ...
  while (1) {
    ...
    groupTagIndex_t groupTagIndex =
        VPC_Group_FindOrCreateGroupTag(pToken, true);
...
void VPC_GroupKeyword_Project() {
  ...
  while (1) {
    ...
    if (!V_stricmp(pToken, "}")) {
      ...
      scriptIndex_t scriptIndex =
          g_pVPC->m_Projects[projectIndex].scripts.AddToTail();
      g_pVPC->m_Projects[projectIndex].scripts[scriptIndex].name = pToken;

      pToken = g_pVPC->GetScript().PeekNextToken(false);
      if (pToken && pToken[0]) {
        pToken = g_pVPC->GetScript().GetToken(false);
        g_pVPC->m_Projects[projectIndex].scripts[scriptIndex].m_condition =
            pToken;
      }
    }
  }
...
void CVPC::GenerateBuildSet(CProjectDependencyGraph &dependencyGraph) {
  ...
  for (int i = 0; i < m_BuildCommands.Count(); i++) {
    ...
    groupTagIndex_t groupTagIndex =
        VPC_Group_FindOrCreateGroupTag(pCommand + 1, false);
      ...
      for (int k = 0; k < pGroup->projects.Count(); k++) {
        ...
        if (pCommand[0] == '*') {
          ...
          if (!dependencyGraph.HasGeneratedDependencies())
            dependencyGraph.BuildProjectDependencies(
                BUILDPROJDEPS_CHECK_ALL_PROJECTS, m_pPhase1Projects);

          dependencyGraph.GetProjectDependencyTree(targetProject, projectsToAdd,
                                                   false);
        } else if (pCommand[0] == '@') {
