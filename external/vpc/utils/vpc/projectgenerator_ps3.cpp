...
bool CProjectGenerator_PS3::WriteFolder(CProjectFolder *pFolder) {
  ...
  m_XMLWriter.Write(CFmtStrMax(
      "Name=\"%s\"", m_XMLWriter.FixupXMLString(pFolder->m_Name.Get())));
  m_XMLWriter.Write(">");
...
bool CProjectGenerator_PS3::WriteSNCLinkerTool(CLinkerTool *pLinkerTool) {
  ...
  for (int i = 0; i < pLinkerTool->m_PropertyStates.m_Properties.Count(); i++) {
    ...
    case PS3_SNCLINKER_GenerateMapFile:
      if (nOrdinalValue == 1) {
        additionalOptions += CFmtStrMax(
            "-Map=&quot;%s&quot; ", pLinkerTool->m_PropertyStates
...
bool CProjectGenerator_PS3::WriteLibrarianTool(CLibrarianTool *pLibrarianTool) {
  ...
  for (int i = 0; i < pLibrarianTool->m_PropertyStates.m_Properties.Count();
    ...
    case PS3_LIBRARIAN_OutputFile:
      m_XMLWriter.Write(CFmtStrMax(
          "OutputFile=\"%s\"", pLibrarianTool->m_PropertyStates.m_Properties[i]
                                   .m_StringValue.Get()));
      break;
...
const char *
CProjectGenerator_PS3::BoolStringToTrueFalseString(const char *pValue) {
...
bool CProjectGenerator_PS3::WriteToXML() {
  ...
  if (g_pVPC->BUseP4SCC())
    m_XMLWriter.Write("SccProjectName=\"Perforce "
                      "Project\"\nSccLocalPath=\"..\"\nSccProvider=\"MSSCCI:"
                      "Perforce SCM\"\n");
  m_XMLWriter.Write(">");
