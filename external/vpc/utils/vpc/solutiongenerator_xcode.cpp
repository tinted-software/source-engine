...
public:
  virtual void
  GenerateSolutionFile(const char *pSolutionFilename,
...
static CUtlString OutputFileWithDirectoryFromConfig(KeyValues *pConfigKV) {
  ...
  if (V_strstr(szOutputDir, "$") || V_strstr(szOutputFile, "$")) {
    g_pVPC->VPCWarning("$OutputDirectory '%s' or $OutputFile directive '%s' "
                       "contains what looks like a shell variable reference -- "
                       "this will be treated literally in most circumstances "
                       "by the Xcode build system and is likely not intended.",
...
static CUtlString GameOutputFileFromConfig(KeyValues *pConfigKV) {
  ...
  if (V_strstr(szGameOutputFile, "$")) {
    g_pVPC->VPCWarning("$GameOutputFile '%s' contains what looks like a shell "
                       "variable reference -- "
                       "this will be treated literally in most circumstances "
                       "by the Xcode build system and is likely not intended.",
                       szGameOutputFile);
...
static bool
ProjectProducesBinary(const CBaseProjectDataCollector *pProjectDataCollector) {
  return (
    ...
    Write("objects = {\n");
    {
        ...
        FOR_EACH_VEC(g_vecPGenerators, iGenerator) {
          ...
          for (int i = g_vecPGenerators[iGenerator]->m_Files.First();
            ...
            if ((!bIsDynamicFile ||
              ...
              Write("%024llX /* %s in Sources */ = {isa = PBXBuildFile; "
                    "fileRef = %024llX /* %s */; ",
            ...
            if (IsDynamicLibrary(pFileName)) {
              Write("\n");
              Write("%024llX /* %s in Frameworks */ = {isa = PBXBuildFile; "
                    "fileRef = %024llX /* %s */; };",
                    makeoid2(g_vecPGenerators[iGenerator]->GetProjectName(),
                             pFileName, EOIDTypeBuildFile),
                    V_UnqualifiedFileName(pFileName),
          ...
          for (int i = 0; i < libs.Count(); i++) {
            Write("\n");
            Write("%024llX /* lib%s.dylib in Frameworks */ = {isa = "
          ...
          for (int i = 0; i < sysFrameworks.Count(); i++) {
            Write("\n");
            Write("%024llX /* %s.framework in Frameworks */ = {isa = "
          ...
          if (sOutputFile.Length() &&
            ...
            for (int iTestProject = 0; iTestProject < projects.Count();
              ...
              if (pTestProject->DependsOn(pCurProject, dependsOnFlags) ||
                ...
                Write("\n");
                Write("%024llX /* (lib)%s */ = {isa = PBXBuildFile; fileRef = "
          ...
          if (sGameOutputFile.Length()) {
            Write("\n");
            Write("%024llX /* %s */ = {isa = PBXBuildFile; fileRef = %024llX "
                  "/* %s */; };",
                  makeoid2(g_vecPGenerators[iGenerator]->GetProjectName(),
          ...
          if (sOutputFile.Length()) {
            Write("\n");
            Write("%024llX /* %s in Products */ = {isa = PBXBuildFile; fileRef "
                  "= %024llX /* %s */; };",
                  makeoid2(g_vecPGenerators[iGenerator]->GetProjectName(),
      ...
      ++m_nIndent;
      {
        ...
        for (int iConfig = 0; iConfig < V_ARRAYSIZE(k_rgchXCConfigFiles);
          ...
          Write("\n");
          Write("%024llX /* %s */ = {isa = PBXFileReference; fileEncoding = 4; "
        ...
        FOR_EACH_VEC(g_vecPGenerators, iGenerator) {
          ...
          for (int i = g_vecPGenerators[iGenerator]->m_Files.First();
            ...
            Write("\n");
            Write("%024llX /* %s */ = {isa = PBXFileReference; fileEncoding = "
                  "4; explicitFileType = \"%s\"; name = \"%s\"; path = \"%s\"; "
                  "sourceTree = \"<absolute>\"; };",
                  makeoid2(g_vecPGenerators[iGenerator]->GetProjectName(),
          ...
          if (sOutputFile.Length()) {
            ...
            Write("%024llX /* %s */ = {isa = PBXFileReference; "
                  "explicitFileType = \"%s\"; includeInIndex = 0; path = "
                  "\"%s\"; sourceTree = BUILT_PRODUCTS_DIR; };",
                  makeoid2(g_vecPGenerators[iGenerator]->GetProjectName(),
                           sOutputFile, EOIDTypeFileReference),
...
#define TELL_XCODE_ABOUT_OUTPUT_FILES 1
                ...
                sConditionalBlock += "if [ $EARLY_OUT -eq 1 ]; then\\n"
                                     "    echo \\\"outputs are newer than "
                                     "input, skipping execution...\\\"\\n"
                                     "    exit 0\\n"
                                     "fi\\n";
                Write("shellScript = \"cd %s\\n"
                      "%s"
      ...
      FOR_EACH_VEC(projects, iProject) {
        ...
        ++m_nIndent;
        {
          ...
          Write("buildConfigurationList = %024llX /* Build configuration list "
        ...
        FOR_EACH_VEC(projects, iProject) {
          ...
          for (int i = 0; i < cSubAggregateTargets; i++) {
            ...
            ++m_nIndent;
            {
              ...
              Write("buildConfigurationList = %024llX /* Build configuration "
      ...
      ++m_nIndent;
      {
        ...
        Write("%024llX /* Build configuration list for PBXAggregateTarget "
              "\"All\" */ = {\n",
              makeoid(oidStrSolutionRoot, EOIDTypeConfigurationList, 1));
        ++m_nIndent;
        {
        ...
        FOR_EACH_VEC(projects, iProject) {
          Write("\n");
          Write("%024llX /* Build configuration list for PBXNativeTarget "
                "\"%s\" */ = {\n",
                makeoid(projects[iProject]->m_ProjectName,
                        EOIDTypeConfigurationList),
                projects[iProject]->m_ProjectName.String());
