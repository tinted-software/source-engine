...
public:
  ...
  void SetupImportLibrary(CProjectDependencyGraph *pGraph,
                          CSpecificConfig *pConfig, const char *szScriptName) {
    ...
    // that is the final product.  This was
    //             kind of hackily added originally -- the $OutputFile directive
    //             was relative to the base directory, but some generators
    //             (XCode) put all their outputs into a object directory, then
    //             use $GameOutputFile to *actually* output.
    m_LinkerOutputFile =
...
CDependency *
CProjectDependencyGraph::FindOrCreateDependency(const char *pFilename) {
...
bool CProjectDependencyGraph::SaveCache(const char *pFilename) {
  ...
  for (int i = m_AllFiles.First(); i != m_AllFiles.InvalidIndex();
    ...
    for (int iDependency = 0; iDependency < pDep->m_Dependencies.Count();
         iDependency++) {
...
public:
  virtual bool VisitProject(projectIndex_t iProject,
    ...
    if (!bAdded) {
      g_pVPC->VPCError("CGameFilterProjectIterator::VisitProject( %s ) - no "
                       "project found by that name.",
                       szProjectName);
      return false;
    }
