...
//otherwise 		vecBonusFiles will contain a list of valid OS files that are on disk
//but did not 		match the current target platform.
//-----------------------------------------------------------------------------
static char *ResolveFilename(const char *pszFile,
  ...
  if (nPlatformColumn < 0) {
    g_pVPC->VPCWarning("Internal Error: Target Platform: '%s' unrecognized "
                       "while expanding $os!",
                       szPlatform);
    return NULL;
  }
...
void VPC_Keyword_AddFilesByPattern() {
  ...
  while (1) {
    ...
    if (pToken[0] == '[') {
      if (files.Count() == 0) {
        g_pVPC->VPCSyntaxError("Conditional specified on a $FilePattern "
                               "without any pattern preceding it.");
      }
  ...
  if (g_pVPC->IsShowCaseIssues() && !bDynamicFile) {
    for (int i = 0; i < files.Count(); i++) {
      ...
      if (!Sys_IsFilenameCaseConsistent(pFilename, actualFilename,
        ...
        g_pVPC->VPCWarning("Case Consistency Issue! File '%s' specified in "
                           "'%s' is inconsistent with OS version '%s'.",
...
static void VPC_ParseFileList(CUtlStringList &files) {
  ...
  while (1) {
    ...
    if (!pNextToken || !pNextToken[0]) {
      ...
      if (pToken && pToken[0] == '[') {
        if (files.Count() == 0) {
          g_pVPC->VPCSyntaxError("Conditional specified on a file list without "
                                 "any file preceding it.");
        }
...
//
//	Specialized instruction to populate the load address macro based on a
//project 	name. Assumes the contained list is minimally packed and has free
//reign of 	space up to the limit. Finds unused space spreads it out evenly
  ...
  while (1) {
    ...
    if (pToken[0] == '[') {
      if (extensions.Count() == 0) {
        g_pVPC->VPCSyntaxError("Conditional specified on a $CustomBuildStep "
                               "without any extensions preceding it.");
      }
...
bool CVPC::ParseProjectScript(const char *pScriptName, int depth, bool bQuiet,
  ...
  if (!depth) {
    ...
    // $(ProjectName) can be auto-decorated, making it unuseable by scripts
  ...
  if (!depth) {
    ...
    g_pVPC->RemoveScriptCreatedMacros(); // Remove any macros that came from the
                                         // script file.
  }
