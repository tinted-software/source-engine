...
bool VPC_Config_General_OutputDirectory(const char *pPropertyName) {
  SET_STRING_PROPERTY(pPropertyName, g_spConfig, get_OutputDirectory,
                      put_OutputDirectory);
}

bool VPC_Config_General_IntermediateDirectory(const char *pPropertyName) {
  SET_STRING_PROPERTY(pPropertyName, g_spConfig, get_IntermediateDirectory,
                      put_IntermediateDirectory);
}

bool VPC_Config_General_ExtensionsToDeleteOnClean(const char *pPropertyName) {
  SET_STRING_PROPERTY(pPropertyName, g_spConfig, get_DeleteExtensionsOnClean,
                      put_DeleteExtensionsOnClean);
}

bool VPC_Config_General_BuildLogFile(const char *pPropertyName) {
  SET_STRING_PROPERTY(pPropertyName, g_spConfig, get_BuildLogFile,
                      put_BuildLogFile);
}

bool VPC_Config_General_InheritedProjectPropertySheets(
    const char *pPropertyName) {
  SET_STRING_PROPERTY(pPropertyName, g_spConfig, get_InheritedPropertySheets,
                      put_InheritedPropertySheets);
}

bool VPC_Config_General_ConfigurationType(const char *pPropertyName) {
  char buff[MAX_SYSTOKENCHARS];

  if (!ParsePropertyValue(&g_pScriptData, g_pScriptLine, NULL, buff,
                          sizeof(buff)))
    return true;
  ...
  if (!V_stricmp(buff, "Utility"))
    option = typeUnknown;
  else if (!V_stricmp(buff, "Application (.exe)") ||
           !V_stricmp(buff, "Title (.xex)"))
    option = typeApplication;
  else if (!V_stricmp(buff, "Dynamic Library (.dll)") ||
           !V_stricmp(buff, "Dynamic Library (.xex)"))
  ...
  SET_LIST_PROPERTY(pPropertyName, g_spConfig, get_ConfigurationType,
                    put_ConfigurationType, ConfigurationTypes, option);
}
...
bool VPC_Config_General_UseOfMFC(const char *pPropertyName) {
  ...
  SET_LIST_PROPERTY(pPropertyName, g_spConfig, get_useOfMfc, put_useOfMfc,
                    useOfMfc, option);
}
...
bool VPC_Config_General_UseOfATL(const char *pPropertyName) {
  ...
  SET_LIST_PROPERTY(pPropertyName, g_spConfig, get_useOfATL, put_useOfATL,
                    useOfATL, option);
}

bool VPC_Config_General_MinimizeCRTUseInATL(const char *pPropertyName) {
  SET_BOOL_PROPERTY(pPropertyName, g_spConfig,
                    get_ATLMinimizesCRunTimeLibraryUsage,
...
bool VPC_Config_General_CharacterSet(const char *pPropertyName) {
  ...
  SET_LIST_PROPERTY(pPropertyName, g_spConfig, get_CharacterSet,
                    put_CharacterSet, charSet, option);
}

bool VPC_Config_General_CommonLanguageRuntimeSupport(
    const char *pPropertyName) {
...
bool VPC_Config_General_WholeProgramOptimization(const char *pPropertyName) {
  ...
  SET_LIST_PROPERTY(pPropertyName, g_spConfig, get_WholeProgramOptimization,
                    put_WholeProgramOptimization, WholeProgramOptimizationTypes,
                    option);
}
...
property_t g_generalProperties[] = {
    ...
    {"$ExtensionsToDeleteOnClean",
     VPC_Config_General_ExtensionsToDeleteOnClean},
    {"$BuildLogFile", VPC_Config_General_BuildLogFile},
    {"$InheritedProjectPropertySheets",
     VPC_Config_General_InheritedProjectPropertySheets},
    ...
    {"$MinimizeCRTUseInATL", VPC_Config_General_MinimizeCRTUseInATL,
     PLATFORM_WINDOWS},
    {"$CharacterSet", VPC_Config_General_CharacterSet},
    {"$CommonLanguageRuntimeSupport",
     VPC_Config_General_CommonLanguageRuntimeSupport, PLATFORM_WINDOWS},
