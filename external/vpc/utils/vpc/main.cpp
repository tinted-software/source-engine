...
bool CVPC::CheckBinPath(char *pOutBinPath, int outBinPathSize) {
  ...
  if (!bFound) {
    VPCError("Failed to determine source directory from current path. "
             "Expecting 'vpc_scripts' in source path.");
  }
...
void CVPC::SpewUsage(void) {
  ...
  if (!bNoHeader) {
    ...
    if (!m_bHelp) {
      ...
      Log_Msg(LOG_VPC, "    vpc +client /hl2     <-- Creates a Win32 .vcproj "
                       "for the HL2 client.\n");
      Log_Msg(LOG_VPC, "    vpc +shaderapi /x360 <-- Creates a Xbox360 .vcproj "
                       "for the shaderapi.\n");

      Log_Msg(LOG_VPC, "\n");
      Log_Msg(LOG_VPC, "  Multiple .vcproj generation - Multiple Projects for "
                       "Games and Platforms:\n");
      Log_Msg(LOG_VPC, "    vpc +client /hl2 /tf           <-- Creates ALL the "
                       "Win32 .vcprojs for the HL2 and TF client.\n");
      Log_Msg(LOG_VPC, "    vpc +gamedlls /allgames        <-- Creates ALL the "
                       "Win32 .vcprojs for client and server for all GAMES.\n");
      Log_Msg(LOG_VPC, "    vpc +tools -tier0 /win32 /x360 <-- Creates ALL the "
                       "Win32 and Xbox360 .vcprojs for the tool projects but "
                       "not the tier0 project.\n");
      ...
      Log_Msg(LOG_VPC, "\n--- OPTIONS ---\n");
      Log_Msg(LOG_VPC, "[/q]:          Quiet mode (quiet mode is automatically "
                       "on if the VPC_QUIET environment variable is set)\n");
      ...
      Log_Msg(LOG_VPC, "[/testmode]:   Override output .vcproj file to be "
                       "named 'test.vcproj'\n");
#ifdef VPC_SCC_INTEGRATION
      Log_Msg(LOG_VPC,
...
#else
      Log_Msg(LOG_VPC, "[/srcctl]:     Enable P4SCC source control integration "
                       "- can also set environment variable VPC_SRCCTL to 1\n");
#endif
      Log_Msg(LOG_VPC, "[/mirror]:     <path> - Mirror output files to "
                       "specified path. Used for A:B testing.\n");
      Log_Msg(LOG_VPC, "[/2019]:       Generate projects and solutions for "
                       "Visual Studio 2019 [default]\n");
      Log_Msg(LOG_VPC, "[/2015]:       Generate projects and solutions for "
                       "Visual Studio 2015\n");
      Log_Msg(LOG_VPC, "[/2013]:       Generate projects and solutions for "
                       "Visual Studio 2013\n");
      Log_Msg(LOG_VPC, "[/2012]:       Generate projects and solutions for "
                       "Visual Studio 2012\n");
      Log_Msg(LOG_VPC, "[/2010]:       Generate projects and solutions for "
                       "Visual Studio 2010\n");
      Log_Msg(LOG_VPC, "[/2005]:       Generate projects and solutions for "
                       "Visual Studio 2005\n");
      Log_Msg(LOG_VPC, "[/2008]:       Generate projects and solutions for "
                       "Visual Studio 2008\n");
      Log_Msg(LOG_VPC,
              "[/windows]:    Generate projects for both Win32 and Win64\n");
      Log_Msg(LOG_VPC, "[/unity]:      Enable unity file generation\n");
      Log_Msg(LOG_VPC, "[/32bittools]: Specify 32-bit toolchain in VC++ even "
                       "when compiling 64 bit target\n");
      ...
      Log_Msg(LOG_VPC, "[/p4sln]:      <.sln filename> <changelists...> - make "
                       "a solution file based on\n");
      Log_Msg(LOG_VPC, "               the changelist. Changelists can be "
                       "specific numbers, 0 or \"default\"\n");
      Log_Msg(LOG_VPC, "               for the default changelist, or \"all\" "
                       "for all active changelists.\n");
      Log_Msg(
          LOG_VPC,
          "[/nop4add]:    Don't automatically add project files to Perforce\n");
      Log_Msg(LOG_VPC, "[/slnitems]:   <filename> - adds all files listed in "
                       "<filename> to generated\n");
      Log_Msg(LOG_VPC, "               solutions\n");
      Log_Msg(LOG_VPC, "[/showdeps]:   Show an example dependency chain for "
                       "each project that depends\n");
      Log_Msg(LOG_VPC,
              "               on your p4 change list(s).  Use with /p4sln.\n");
      Log_Msg(LOG_VPC, "[/checkfiles]: Check for the existence of files in "
                       "$file commands. For debugging vpc files.\n");
      Log_Msg(LOG_VPC, "               Only works if the currrent directory is "
      ...
      Log_Msg(LOG_VPC, "[/define:xxx]: Enable a custom conditional $XXX to use "
                       "for quick testing in VPC files.\n");
    }
  }
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (!V_stricmp(pArg, "/mksln")) {
      ...
      if (i >= argc || argv[i][0] == '+' || argv[i][0] == '-' ||
        ...
        VPCError("%s <solution filename> <changelist number> [ "
                 "[restrict_to_group] ].",
                 pArg);
      }
      ...
      if (m_iP4Changelists.Count() == 0) {
        VPCError("%s <solution filename> <changelist number> [additional "
                 "changelist numbers] [ [restrict_to_group] ].",
                 pArg);
      }
...
void CVPC::CheckForInstalledXDK() {
  ...
  if (!bHasXDK) {
    VPCError("Cannot Build For Xbox 360, XDK is missing or damaged. Remove "
             "/x360 from command line.");
  }
#endif
}
...
bool CVPC::BuildTargetProjects() {
  ...
  if (!m_TargetProjects.Count()) {
    VPCError("No recognized project(s) to build.  Use /h or /projects or "
             "/groups to spew more info.");
  }
...
void CVPC::FindProjectFromVCPROJ(const char *pScriptNameVCProj) {
  ...
  // re-gen parameters mod and platform will be seperated by '_' after the
