...
class UniqueId {
public:
  explicit UniqueId(intptr_t data) : data_(data) {}

  bool operator==(const UniqueId &other) const { return data_ == other.data_; }
...
public:
   ...
   */
  template <class S>
  V8_INLINE Handle(Handle<S> that) : val_(reinterpret_cast<T *>(*that)) {
    /**
  ...
  template <class S> V8_INLINE Handle<S> As() { return Handle<S>::Cast(*this); }
...
public:
  V8_INLINE Local();
  template <class S>
  V8_INLINE Local(Local<S> that) : Handle<T>(reinterpret_cast<T *>(*that)) {
    /**
     * This check fails when trying to convert between incompatible
     * handles. For example, converting from a Handle<String> to a
     * Handle<Number>.
  ...
  template <class S>
  V8_INLINE Local(Handle<S> that) : Handle<T>(reinterpret_cast<T *>(*that)) {
    TYPE_CHECK(T, S);
  }

  template <class S> V8_INLINE Local<S> As() { return Local<S>::Cast(*this); }
...
// Convenience wrapper.
template <class T> inline Maybe<T> maybe(T t) { return Maybe<T>(t); }
...
public:
  ...
  CompileUnbound(Isolate *isolate, Source *source,
   ...
   *   context.
   */
  static Local<Script> Compile(Isolate *isolate, Source *source,
                               CompileOptions options = kNoCompileOptions);
  ...
  StartStreamingScript(Isolate *isolate, StreamedSource *source,
                       CompileOptions options = kNoCompileOptions);
...
public:
   ...
   *   StackFrame.
   */
  static Local<StackTrace>
  CurrentStackTrace(Isolate *isolate, int frame_limit,
...
public:
   ...
   */
  V8_INLINE ExternalStringResourceBase *
  ...
  static Local<String> NewFromOneByte(Isolate *isolate, const uint8_t *data,
                                      NewStringType type = kNormalString,
                                      int length = -1);

  /** Allocates a new string from UTF-16 data.*/
  static Local<String> NewFromTwoByte(Isolate *isolate, const uint16_t *data,
                                      NewStringType type = kNormalString,
                                      int length = -1);
...
public:
  ...
  bool SetAccessor(Handle<String> name, AccessorGetterCallback getter,
...
public:
  ...
  void SetNativeDataProperty(
      ...
      Handle<Value> data = Handle<Value>(), PropertyAttribute attribute = None,
  ...
  // This function is not yet stable and should not be used at this time.
  bool SetDeclaredAccessor(
      Local<Name> name, Local<DeclaredAccessorDescriptor> descriptor,
      PropertyAttribute attribute = None,
      Local<AccessorSignature> signature = Local<AccessorSignature>(),
      AccessControl settings = DEFAULT);
...
public:
  ...
  New(Isolate *isolate, FunctionCallback callback = 0,
      Handle<Value> data = Handle<Value>(),
...
public:
   ...
   */
  void
  SetIndexedPropertyHandler(IndexedPropertyGetterCallback getter,
...
public:
  static Local<Signature>
  New(Isolate *isolate,
      Handle<FunctionTemplate> receiver = Handle<FunctionTemplate>(),
      int argc = 0, Handle<FunctionTemplate> argv[] = 0);
...
public:
  static Local<AccessorSignature>
  New(Isolate *isolate,
      Handle<FunctionTemplate> receiver = Handle<FunctionTemplate>());
...
public:
  // This function is not yet stable and should not be used at this time.
  static Local<RawOperationDescriptor>
  NewInternalFieldDereference(Isolate *isolate, int internal_field);
...
public:
  ...
  NewPrimitiveValue(Isolate *isolate,
                    DeclaredAccessorDescriptorDataType data_type,
                    uint8_t bool_offset = 0);
  Local<DeclaredAccessorDescriptor>
  NewBitmaskCompare8(Isolate *isolate, uint8_t bitmask, uint8_t compare_value);
...
public:
  ...
  Extension(const char *name, const char *source = 0, int dep_count = 0,
            const char **deps = 0, int source_length = -1);
  virtual ~Extension() {}
...
public:
  ...
  void set_max_available_threads(int value) { max_available_threads_ = value; }
...
public:
  ...
  struct CreateParams {
    CreateParams()
        : entry_hook(NULL), code_event_handler(NULL), enable_serializer(false) {
    }
  ...
  private:
    ...
    AllowJavascriptExecutionScope &
    operator=(const AllowJavascriptExecutionScope &);
  };
  ...
  private:
    ...
    SuppressMicrotaskExecutionScope &
    operator=(const SuppressMicrotaskExecutionScope &);
  };
...
protected:
  virtual int DecompressData(char *raw_data, int *raw_data_size,
...
class V8_EXPORT ExternalResourceVisitor{
  // NOLINT
  public :
      virtual ~ExternalResourceVisitor(){} virtual void VisitExternalString(
...
class V8_EXPORT PersistentHandleVisitor{
  // NOLINT
  public :
      virtual ~PersistentHandleVisitor(){} virtual void VisitPersistentHandle(
...
public:
   ...
   */
  static void
  VisitHandlesForPartialDependence(Isolate *isolate,
...
public:
  ...
  New(Isolate *isolate, ExtensionConfiguration *extensions = NULL,
...
template <int kSmiShiftSize> V8_INLINE internal::Object *IntToSmi(int value) {
...
public:
  ...
  static const int kAmountOfExternalAllocatedMemoryOffset = 4 * kApiPointerSize;
  ...
  V8_INLINE static internal::Object **GetRoot(v8::Isolate *isolate, int index) {
    ...
    return reinterpret_cast<internal::Object **>(addr +
                                                 index * kApiPointerSize);
  }
...
template <class T> Local<T>::Local() : Handle<T>() {}

template <class T> Local<T> Local<T>::New(Isolate *isolate, Handle<T> that) {
  return New(isolate, that.val_);
}
...
template <class T> Handle<T> Handle<T>::New(Isolate *isolate, T *that) {
  if (that == NULL)
    return Handle<T>();
  T *that_ptr = that;
...
template <class T> Local<T> Local<T>::New(Isolate *isolate, T *that) {
  if (that == NULL)
    return Local<T>();
  T *that_ptr = that;
  internal::Object **p = reinterpret_cast<internal::Object **>(that_ptr);
...
template <class T> Local<T> Eternal<T>::Get(Isolate *isolate) {
...
template <class T> bool PersistentBase<T>::IsIndependent() const {
  typedef internal::Internals I;
  if (this->IsEmpty())
    return false;
...
template <class T> bool PersistentBase<T>::IsNearDeath() const {
  typedef internal::Internals I;
  if (this->IsEmpty())
    return false;
  uint8_t node_state =
...
template <class T> void PersistentBase<T>::MarkIndependent() {
  typedef internal::Internals I;
  if (this->IsEmpty())
    return;
...
template <class T> void PersistentBase<T>::MarkPartiallyDependent() {
  typedef internal::Internals I;
  if (this->IsEmpty())
    return;
  I::UpdateNodeFlag(reinterpret_cast<internal::Object **>(this->val_), true,
...
template <typename T> Isolate *ReturnValue<T>::GetIsolate() {
...
template <typename T> internal::Object *ReturnValue<T>::GetDefaultValue() {
...
template <typename T> Local<Function> FunctionCallbackInfo<T>::Callee() const {
  return Local<Function>(
      reinterpret_cast<Function *>(&implicit_args_[kCalleeIndex]));
}
...
Handle<Value> ScriptOrigin::ResourceName() const { return resource_name_; }
...
Handle<Integer> ScriptOrigin::ScriptID() const { return script_id_; }

ScriptCompiler::Source::Source(Local<String> string, const ScriptOrigin &origin,
                               CachedData *data)
    : source_string(string), resource_name(origin.ResourceName()),
...
template <typename T>
void Isolate::SetObjectGroupId(const Persistent<T> &object, UniqueId id) {
  TYPE_CHECK(Value, T);
  SetObjectGroupId(reinterpret_cast<v8::internal::Object **>(object.val_), id);
}

template <typename T>
void Isolate::SetReferenceFromGroup(UniqueId id, const Persistent<T> &object) {
  TYPE_CHECK(Value, T);
