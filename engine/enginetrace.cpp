...
//-----------------------------------------------------------------------------
inline bool
CEngineTrace::ShouldPerformCustomRayTest(const Ray_t &ray,
                                         ICollideable *pCollideable) const {
...
//-----------------------------------------------------------------------------
CPhysCollide *
CEngineTrace::GetCollidableFromDisplacementsInAABB(const Vector &vMins,
                                                   const Vector &vMaxs) {
  CCollisionBSPData *pBSPData = GetCollisionBSPData();
  ...
  for (int i = 0; i < iLeafCount; ++i) {
    ...
    for (int k = 0; k < curLeaf.dispCount; k++) {
      ...
      if (iTriCount > 65535) {
        AssertMsg(0, "Displacement surfaces have too many triangles to "
                     "duplicate in GetCollidableFromDisplacementsInBox.");
        EndTrace(pTraceInfo);
        return NULL;
      }
  ...
  if (pPlanesOut) {
    ...
    if (pBrush->IsBox()) {
      ...
      for (cbrushside_t *side =
               &pBSPData->map_brushsides[pBrush->firstbrushside];
           side != stopside; ++side) {
        Vector4D pVec(side->plane->normal.x, side->plane->normal.y,
