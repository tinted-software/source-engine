...
public:
  ...
  virtual void
  SetClientFrameTime(float frametime); // gpGlobals->frametime on client
...
bool CEngineTool::PrecacheSound(const char *pName, bool bPreload) {
  tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s(%s, %s)", __FUNCTION__,
         tmDynamicString(TELEMETRY_LEVEL0, pName), bPreload ? "true" : "false");
...
void CEngineTool::StartMovieRecording(KeyValues *pMovieParams) {
  ...
  if (pMovieParams->GetInt("outputavi", 0)) {
    Warning("Got a request to record a movie using AVI, but AVI is deprecated. "
            "Using QuickTime/H264 instead.\n");
  ...
  int nHeight =
      pMovieParams->GetInt("height", videomode->GetModeStereoHeight());
...
CEngineTool::CreatePartitionHandle(IHandleEntity *pEntity,
