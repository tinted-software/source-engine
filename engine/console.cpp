...
void ConsoleLogFileCallback(IConVar *var, const char *pOldValue,
  ...
  if (!COM_IsValidPath(logFile) || !COM_IsValidLogFilename(logFile)) {
    ...
    if (!extension ||
        (Q_strcasecmp(extension, "log") && Q_strcasecmp(extension, "txt"))) {
...
static ConVar con_trace("con_trace", "0", FCVAR_MATERIAL_SYSTEM_THREAD,
                        "Print console text to low level printout.");
static ConVar con_notifytime("con_notifytime", "8",
...
static ConVar con_filter_enable(
    "con_filter_enable", "0", FCVAR_MATERIAL_SYSTEM_THREAD,
    "Filters console output based on the setting of con_filter_text. 1 filters "
    "completely, 2 displays filtered text brighter than other text.");
static ConVar con_filter_text("con_filter_text", "",
                              FCVAR_MATERIAL_SYSTEM_THREAD,
                              "Text with which to filter console spew. Set "
                              "con_filter_enable 1 or 2 to activate.");
static ConVar
    con_filter_text_out("con_filter_text_out", "", FCVAR_MATERIAL_SYSTEM_THREAD,
                        "Text with which to filter OUT of console spew. Set "
                        "con_filter_enable 1 or 2 to activate.");
...
void Con_Init(void) {
  ...
  // Check -consolelog arg and set con_logfile if it's present. This gets some
  // messages logged
  //  that we would otherwise miss due to con_logfile being set in the .cfg
  //  file.
  const char *filename = NULL;
  if (CommandLine()->CheckParm("-consolelog", &filename) && filename &&
      filename[0]) {
...
================
*/
void Con_Shutdown(void) { con_initialized = false; }
...
void Con_ColorPrint(const Color &clr, char const *msg) {
  if (IsPC()) {
    ...
    if (Plat_IsInDebugSession() && !con_trace.GetInt() &&
        !spew_consolelog_to_debugstring.GetBool()) {
...
// Purpose:
//-----------------------------------------------------------------------------
CConPanel::~CConPanel(void) { g_pConPanel = NULL; }
...
void CConPanel::DrawNotify(void) {
  ...
  // Protect against background modifications to m_NotifyText.
  // DEADLOCK WARNING: Cannot call DrawColoredText while holding
  // g_AsyncNotifyTextMutex or deadlock can occur while MatQueue0 holds material
  // system lock and attempts to add text to m_NotifyText.
...
void CConPanel::PaintBackground() {
  ...
  int wide = GetWide();
  char ver[100];
  Q_snprintf(ver, sizeof(ver), "Source Engine %i (build %d)", PROTOCOL_VERSION,
             build_number());
  wchar_t unicode[200];
  ...
  if (cl.IsActive()) {
    if (cl.m_NetChannel->IsLoopback()) {
      ...
      Q_snprintf(ver, sizeof(ver), "Server '%s' Map '%s'",
