...
static ConVar r_fastzreject(
    "r_fastzreject", "0", FCVAR_ALLOWED_IN_COMPETITIVE,
...
static ConVar
    r_fastzrejectdisp("r_fastzrejectdisp", "0", 0,
                      "Activates/deactivates fast z rejection on displacements "
                      "(360 only). Only active when r_fastzreject is on.");
...
void Shader_DrawSurfaceDynamic(IMatRenderContext *pRenderContext,
  ...
  tmZoneFiltered(TELEMETRY_LEVEL0, 50, TMZF_NONE, "%s %d", __FUNCTION__,
                 surfID);
...
//-----------------------------------------------------------------------------
static inline void
Shader_SetChainLightmapState(IMatRenderContext *pRenderContext,
                             SurfaceHandle_t surfID) {
...
void Shader_SetChainTextureState(IMatRenderContext *pRenderContext,
  ...
  if (bShadowDepth) {
    ...
    pRenderContext->Bind(MSurf_TexInfo(surfID)->material,
                         pBaseEntity ? pBaseEntity->GetClientRenderable()
                                     : NULL);
  ...
  while (listIndex < count) {
...
#else
    IMesh *pBuildMesh =
        pRenderContext->GetDynamicMesh(false, g_WorldStaticMeshes[sortIDBase]);
    ...
    for (; listIndex < count; listIndex++) {
...
#endif
      {
        if (meshList.Count() < MAX_VERTEX_FORMAT_CHANGES - 1) {
          ...
          if (bWarn) {
            Warning("Too many vertex format changes in frame, whole world not "
                    "rendered\n");
            bWarn = false;
          }
          continue;
      ...
      MSL_FOREACH_SURFACE_IN_GROUP_BEGIN(sortList, group, surfIDList) {
        tmZoneFiltered(TELEMETRY_LEVEL0, 50, TMZF_NONE,
                       "BuildIndicesForWorldSurface");
#ifdef NEWMESH
        BuildIndicesForWorldSurface(indexBufferBuilder, surfIDList,
                                    host_state.worldbrush);
#else
        Assert(meshBuilder.m_nFirstVertex == 0);
        BuildIndicesForWorldSurface(meshBuilder, surfIDList,
                                    host_state.worldbrush);
#endif
      }
      MSL_FOREACH_SURFACE_IN_GROUP_END()
    ...
    for (int m = 0; m < meshTotal; m++) {
      ...
      for (int b = 0; b < mesh.numbatches; b++) {
        ...
        Warning("pRenderContext->Draw( MATERIAL_TRIANGLES, batch.firstIndex = "
                "%d, batch.numIndex = %d )\n",
...
//-----------------------------------------------------------------------------
static void
Shader_DrawChainsWireframe(const CUtlVector<msurface2_t *> &surfaceList) {
...
//-----------------------------------------------------------------------------
static void
Shader_DrawChainNormals(const CUtlVector<msurface2_t *> &surfaceList) {
...
static void
Shader_DrawChainBumpBasis(const CUtlVector<msurface2_t *> &surfaceList) {
  Vector p, tVect, tangentS, tangentT;
  ...
  for (int i = 0; i < surfaceList.Count(); i++) {
    ...
    for (vertID = 0; vertID < MSurf_VertCount(surfID); ++vertID) {
      ...
      for (int j = 0; j < 3; j++) {
        ...
        worldSpaceBumpBasis[j][2] = g_localBumpBasis[j][0] * norm[0] +
...
void Shader_DrawDispChain(int nSortGroup, const CMSurfaceSortList &list,
                          unsigned long flags, ERenderDepthMode DepthMode) {
  ...
  if (count) {
    ...
    int i = 0;
    MSL_FOREACH_GROUP_BEGIN(list, nSortGroup, group){
        MSL_FOREACH_SURFACE_IN_GROUP_BEGIN(list, group,
                                           surfID){pList[i] = surfID;
    ++i;
  }
  MSL_FOREACH_SURFACE_IN_GROUP_END()
}
...
static void Shader_WorldShadowDepthFill(CWorldRenderList *pRenderList,
  ...
  for (g = 0; g < MAX_MAT_SORT_GROUPS; ++g) {
    ...
    // flashlights, etc.
    Shader_DrawDispChain(nSortGroup, pRenderList->m_DispSortList, flags,
                         DepthMode);
  }
  if (nVertexCount == 0)
    return;
...
#ifdef _X360
  bool bFastZRejectDisplacements =
      s_bFastZRejectDisplacements || (r_fastzrejectdisp.GetInt() != 0);
#endif
...
#ifdef _X360
    // Draws opaque displacement surfaces along with shadows, overlays,
    // flashlights, etc. NOTE: This only makes sense on the 360, since the extra
    // batches aren't worth it on the PC (I think!)
...
void Shader_DrawTranslucentSurfaces(IWorldRenderList *pRenderListIn,
  ...
  for (i = 0; i < MAX_MAT_SORT_GROUPS; ++i) {
    ...
    for (int listIndex = surfaceList.Count(); --listIndex >= 0;) {
      ...
      //buffer, we shouldn't use this 			since it causes a lock and
      //drawindexedprimitive per surface! (gary) 			Shader_DrawSurfaceStatic(
      //surfID );
      Shader_DrawSurfaceDynamic(pRenderContext, surfID, false);
...
void R_SetFogVolumeState(int fogVolume, bool useHeightFog) {
  ...
  if (pMaterial && pFogEnableVar->GetIntValueFast() &&
      fog_enable_water_fog.GetBool()) {
...
void Shader_BrushSurface(SurfaceHandle_t surfID, model_t *model,
  ...
  if (!s_pBrushRenderOverride) {
    ...
    //shouldn't use this 		since it causes a lock and drawindexedprimitive per
    //surface! (gary) 		Shader_DrawSurfaceStatic( surfID );
    Shader_DrawSurfaceDynamic(pRenderContext, surfID, false);
...
public:
  ...
  void DrawTranslucentBrushModel(model_t *model, IClientEntity *baseentity) {
    ...
    BuildTransLists_r(renderT, model,
  ...
  void AddSurfaceToBatch(transrender_t &renderT, transnode_t *pNode,
  ...
  void AddTransBatch(transrender_t &renderT, SurfaceHandle_t surfID) {
    transbatch_t &batch = renderT.batches[renderT.pLastNode->firstBatch +
  ...
  // build node lists
  void BuildTransLists_r(transrender_t &renderT, model_t *model,
                         mnode_t *node) {
    float dot;
    ...
    for (int i = 0; i < node->numsurfaces; i++, surfID++) {
      ...
      if (MSurf_Flags(surfID) & SURFDRAW_TRANS) {
        ...
        if (renderT.pLastBatch && renderT.pLastBatch->sortID == sortID) {
          AddSurfaceToBatch(renderT, renderT.pLastNode, renderT.pLastBatch,
                            surfID);
        } else {
    ...
    for (int i = 0; i < renderT.nodeCount; i++) {
      ...
      if (node.decalSurfaceCount) {
        for (j = 0; j < node.decalSurfaceCount; j++) {
          SurfaceHandle_t surfID =
              renderT.decalSurfaces[node.firstDecalSurface + j];
          Assert(!(MSurf_Flags(surfID) & SURFDRAW_NODRAW));
          if (SurfaceHasDecals(surfID)) {
            DecalSurfaceAdd(surfID, BRUSHMODEL_DECAL_SORT_GROUP);
          }
  ...
  void DrawOpaqueBrushModel(IClientEntity *baseentity, model_t *model,
                            const Vector &origin, ERenderDepthMode DepthMode);
...
// transrender_t pass!
CBrushBatchRender::brushrender_t *
CBrushBatchRender::FindOrCreateRenderBatch(model_t *pModel) {
  ...
  if (planeList.Base())
    memcpy(renderT.pPlanes, planeList.Base(),
...
void CBrushBatchRender::DrawOpaqueBrushModel(IClientEntity *baseentity,
  ...
  for (i = 0; i < pRender->planeCount; i++) {
    ...
    backface[i] =
        (DepthMode == DEPTH_MODE_NORMAL && (dot < BACKFACE_EPSILON))
            ? true
...
//-----------------------------------------------------------------------------
void CBrushBatchRender::DrawTranslucentBrushModel(
    IClientEntity *baseentity, model_t *model, const Vector &origin,
    bool bShadowDepth, bool bDrawOpaque, bool bDrawTranslucent) {
  if (bDrawOpaque) {
    DrawOpaqueBrushModel(baseentity, model, origin,
                         bShadowDepth ? DEPTH_MODE_SHADOW : DEPTH_MODE_NORMAL);
  }
...
void R_DrawBrushModel(IClientEntity *baseentity, model_t *model,
                      ...
                      ERenderDepthMode DepthMode, bool bDrawOpaque,
  ...
  if (model->flags & MODELFLAG_FRAMEBUFFER_TEXTURE) {
    ...
    pRenderContext->CopyRenderTargetToTexture(
        pRenderContextMat->GetFrameBufferCopyTexture(0));
  }
  ...
  if (s_pBrushRenderOverride) {
    ...
    if (model->flags & MODELFLAG_TRANSLUCENT) {
      if (DepthMode == DEPTH_MODE_NORMAL) {
        g_BrushBatchRenderer.DrawTranslucentBrushModel(
            baseentity, model, origin, false, bDrawOpaque, bDrawTranslucent);
      }
    } else if (bDrawOpaque) {
  ...
  Shader_BrushEnd(pRenderContext, brushTransform.GetNonIdentityMatrix(), model,
                  DepthMode != DEPTH_MODE_NORMAL, baseentity);
...
// Finds all leaves of the BSP tree within a particular volume
//-----------------------------------------------------------------------------
static bool EnumerateLeafInBox_R(mnode_t *RESTRICT node,
                                 const EnumLeafBoxInfo_t *RESTRICT pInfo) {
  // no polygons in solid nodes (don't report these leaves either)
  if (node->contents == CONTENTS_SOLID)
    return true; // solid
  ...
  Assert(IsBoxIntersectingBoxExtents(
             AlignThatVector(node->m_vecCenter),
             AlignThatVector(node->m_vecHalfDiagonal),
  ...
  // rough cull...
  if (!IsBoxIntersectingBoxExtents(LoadAlignedSIMD(node->m_vecCenter),
                                   LoadAlignedSIMD(node->m_vecHalfDiagonal),
  ...
  if (node->contents >= 0) {
    // if a leaf node, report it to the iterator...
    return pInfo->m_pIterator->EnumerateLeaf(LeafToIndex((mleaf_t *)node),
                                             pInfo->m_nContext);
  }
  ...
  */
  ...
  cornermin = XMVectorSelect(
      vecBoxMax, vecBoxMin,
      control); // cornermin[i] = control[i] ? vecBoxMin[i] : vecBoxMax[i]
  cornermax = XMVectorSelect(vecBoxMin, vecBoxMax, control);

  // compute dot products
  fltx4 dotCornerMax =
      __vmsum3fp(planeNormal, cornermax); // vsumfp ignores w component
  ...
  if (XMComparisonAllTrue(
          conditionRegister)) // plane->normal . cornermax <= plane->dist
    return EnumerateLeafInBox_R(node->children[1], pInfo);
  ...
  if (DotProduct( plane->normal, cornermax ) <= plane->dist)
  {
          return EnumerateLeafInBox_R( node->children[1], info, infoBoxCenter,
  infoBoxHalfDiagonal );
  }
  else if (DotProduct( plane->normal, cornermin ) >= plane->dist)
  {
          return EnumerateLeafInBox_R( node->children[0], info, infoBoxCenter,
  infoBoxHalfDiagonal );
  }
  else
  {
          // Here the box is split by the node
          bool ret = EnumerateLeafInBox_R( node->children[0], info,
  infoBoxCenter, infoBoxHalfDiagonal ); if (!ret) return false;
