...
// These counters are for debugging in dumps.  If these are non-zero it may
// indicate some kind of heap problem caused by the setjmp/longjmp error
// handling
...
static ConVar
    host_competitive_ever_enabled("host_competitive_ever_enabled", "0",
                                  FCVAR_HIDDEN,
                                  "Has competitive ever been enabled this run?",
                                  true, 0, true, 1, true, 1, false, 1, NULL);

static ConVar mem_test_each_frame("mem_test_each_frame", "0", 0,
                                  "Run heap check at end of every frame\n");
static ConVar
    mem_test_every_n_seconds("mem_test_every_n_seconds", "0", 0,
...
ConVar host_showcachemiss(
    ...
    "Print a debug message when the client or server cache is missed.");
static ConVar
    mem_dumpstats("mem_dumpstats", "0", 0,
                  "Dump current and max heap usage info to console at end of "
                  "frame ( set to 2 for continuous output )\n");
static ConVar host_ShowIPCCallCount(
    "host_ShowIPCCallCount", "0", 0,
    "Print # of IPC calls this number of times per second. If set to -1, the # "
    "of IPC calls is shown every frame.");

#if defined(RAD_TELEMETRY_ENABLED)
static void OnChangeTelemetryPause(IConVar *var, const char *pOldValue,
                                   float flOldValue) {
  tmPause(TELEMETRY_LEVEL0, 1);
}

static void OnChangeTelemetryResume(IConVar *var, const char *pOldValue,
                                    float flOldValue) {
  tmPause(TELEMETRY_LEVEL0, 0);
}

static void OnChangeTelemetryLevel(IConVar *var, const char *pOldValue,
                                   float flOldValue) {
  char *pIEnd;
...
static void OnChangeTelemetryFrameCount(IConVar *var, const char *pOldValue,
                                        float flOldValue) {
  char *pIEnd;
...
static void OnChangeTelemetryServer(IConVar *var, const char *pOldValue,
                                    float flOldValue) {
  const char *pServerAddress = ((ConVar *)var)->GetString();

  Q_strncpy(g_Telemetry.ServerAddress, pServerAddress,
            ARRAYSIZE(g_Telemetry.ServerAddress));
  Msg(" TELEMETRY: Setting Telemetry server: '%s'\n", pServerAddress);
}

static void OnChangeTelemetryDemoStart(IConVar *var, const char *pOldValue,
                                       float flOldValue) {
  char *pIEnd;
  ...
  if (g_Telemetry.DemoTickStart > 2000) {
    ...
    // If we're far away from the start of the demo file, then jump to ~1000
    // ticks before.
    Q_snprintf(cmd, sizeof(cmd), "demo_gototick %d",
               g_Telemetry.DemoTickStart - 1000);
    Cbuf_AddText(cmd);
  }
...
static void OnChangeTelemetryDemoEnd(IConVar *var, const char *pOldValue,
                                     float flOldValue) {
  char *pIEnd;
  const char *pVal = ((ConVar *)var)->GetString();
...
ConVar telemetry_pause("telemetry_pause", "0", 0, "Pause Telemetry",
                       OnChangeTelemetryPause);
ConVar telemetry_resume("telemetry_resume", "0", 0, "Resume Telemetry",
                        OnChangeTelemetryResume);
ConVar telemetry_framecount("telemetry_framecount", "0", 0,
                            "Set Telemetry count of frames to capture",
                            OnChangeTelemetryFrameCount);
ConVar telemetry_level("telemetry_level", "0", 0,
                       "Set Telemetry profile level: 0 being off.",
                       OnChangeTelemetryLevel);
ConVar telemetry_server("telemetry_server", "localhost", 0,
                        "Set Telemetry server", OnChangeTelemetryServer);
ConVar telemetry_demostart("telemetry_demostart", "0", 0,
                           "When playing demo, start telemetry on tick #",
                           OnChangeTelemetryDemoStart);
ConVar telemetry_demoend("telemetry_demoend", "0", 0,
                         "When playing demo, stop telemetry on tick #",
...
void HostTimerSpinMsChangedCallback(IConVar *var, const char *pOldString,
                                    float flOldValue);
ConVar host_timer_spin_ms(
    "host_timer_spin_ms", "0", FCVAR_NONE,
...
void HostTimerSpinMsChangedCallback(IConVar *var, const char *pOldString,
  ...
  if (pForcedValue != NULL) {
    if (V_strcmp(host_timer_spin_ms.GetString(), pForcedValue)) {
      Msg("Value for host_timer_spin_ms is locked to %s by command line "
          "parameter.\n",
          pForcedValue);
...
CON_COMMAND(host_timer_report, "Spew CPU timer jitter for the last 128 frames "
                               "in microseconds (dedicated only)") {
  if (sv.IsDedicated()) {
    for (int i = 1; i <= ARRAYSIZE(host_jitterhistory); ++i) {
      unsigned int slot =
          (i + host_jitterhistorypos) % ARRAYSIZE(host_jitterhistory);
...
ConVar deathmatch("deathmatch", "0", FCVAR_NOTIFY | FCVAR_INTERNAL_USE,
...
bool GetFileFromRemoteStorage(ISteamRemoteStorage *pRemoteStorage,
  ...
  if (nFileSize > 0) {
    ...
    if (pRemoteStorage->FileRead(pszRemoteFileName, buf.Base(), nFileSize) ==
        nFileSize) {
      ...
      if (hFile) {
        ...
        bSuccess =
            g_pFileSystem->Write(buf.Base(), nFileSize, hFile) == nFileSize;
...
void Host_DefaultMapFileName(const char *pFullMapName,
                             /* out */ char *pDiskName,
...
void CheckForFlushMemory(const char *pCurrentMapName,
  ...
  //   Case 2: loading any map (inc. A to A) and free memory is below
  //   host_flush_threshold MB
  //          -> flush everything
  //   Case 3: loading a 'blacklisted' map (the known biggest memory users, or
  //   where texture sets change)
  //          -> flush everything
  static const char *mapBlackList[] = {
      // --hl2--
      "d1_canals_01", "d1_canals_05", "d1_eli_01", "d1_town_01", "d2_coast_01",
      "d2_prison_01", "d3_c17_01", "d3_c17_05", "d3_c17_09", "d3_citadel_01",
      "d3_breen_01",
      // --ep1--
      "ep1_c17_02", "ep1_c17_02b", "ep1_c17_05", "ep1_c17_06",
      // --ep2--
      "ep2_outland_06a", "ep2_outland_09", "ep2_outland_11", "ep2_outland_12",
      "ep2_outland_12a",
      // --tf--
      "tc_hydro"};
  ...
  bool bIsMapChanging =
      pCurrentMapName[0] && V_stricmp(pCurrentMapName, pDestMapName);
...
void SetupNewBindings() {
  ...
  FOR_EACH_TRUE_SUBKEY(pNewBindingsData, pSubKey) {
    ...
    // Force the key binding if the ideal key is bound to an irrelevant command,
    // which is the case for CS:S, which binds F6 to "quick save," which has no
    // used in the game at all.
    const char *pOverrideIfCmd = pSubKey->GetString("override_if", NULL);
    if (pOverrideIfCmd) {
      const char *pCurrentBindingForKey =
          ::Key_BindingForKey(g_pInputSystem->StringToButtonCode(pIdealKey));
      if (!pCurrentBindingForKey ||
          !V_stricmp(pOverrideIfCmd, pCurrentBindingForKey)) {
    ...
    if (!pCurrentBindingForIdealKey) {
      ...
      V_snprintf(szBindCmd, sizeof(szBindCmd), "bind \"%s\" \"%s\"", pIdealKey,
                 pBinding);
      Cbuf_AddText(szBindCmd);
      continue;
    }
    ...
    bool bFound = false;
    int nNumAttempts = 1;
    for (int nCurButton = (int)KEY_0; nCurButton <= (int)KEY_LAST;
         ++nCurButton, ++nNumAttempts) {
      // Don't consider numpad, windows keys, etc
      bool bFoundInvalidKey = false;
      for (int iKeyIndex = 0; iKeyIndex < sizeof(nInvalidKeyBindings) /
                                              sizeof(nInvalidKeyBindings[0]);
           iKeyIndex++) {
...
void Host_WriteConfiguration_360(void) {
  ...
  Q_snprintf(strFilename, sizeof(strFilename), "cfg:/%s_config.cfg",
             GetCurrentMod());
  ...
  Q_snprintf(strVideoFilename, sizeof(strVideoFilename),
...
void Host_WriteConfiguration(const char *filename, bool bAllVars) {
  ...
  // Don't write config when in default--most of the values are defaults which
  // is not what the player wants. If bAllVars is set, go ahead and write out
  // the file anyways, since it was requested explicitly.
  if (!cbIsUserRequested && (CommandLine()->CheckParm("-default") ||
                             host_competitive_ever_enabled.GetBool()))
    return;
...
#if defined(NO_STEAM)
  AssertMsg(false,
            "SteamCloud not available on Xbox 360. Badger Martin to fix this.");
#else
    ...
    if (pRemoteStorage->GetQuota(&totalBytes, &availableBytes)) {
      if (totalBytes > 0) {
        if (cl_cloud_settings.GetInt() == STEAMREMOTESTORAGE_CLOUD_ON) {
          ...
          if (pRemoteStorage->FileWrite(szFileName, configBuff.Base(),
                                        configBuff.TellMaxPut())) {
          ...
          Q_strncpy(szLogoFileName, cl_logofile.GetString(),
                    sizeof(szLogoFileName)); // .vtf file

          if (g_pFileSystem->FileExists(szLogoFileName, "MOD")) {
            // store logo .VTF file
            FileHandle_t hFile =
                g_pFileSystem->Open(szLogoFileName, "rb", "MOD");
            if (FILESYSTEM_INVALID_HANDLE != hFile) {
              ...
              if (g_pFileSystem->Read(pBuffer, unSize, hFile) == unSize) {
                Q_SetExtension(g_szDefaultLogoFileName, ".vtf",
                               sizeof(g_szDefaultLogoFileName));
                if (pRemoteStorage->FileWrite(g_szDefaultLogoFileName, pBuffer,
                                              unSize)) {
                  DevMsg("[Cloud]: SUCCEESS saving %s in remote storage\n",
...
bool XBX_SetProfileDefaultSettings(void) {
  // These defined values can't play nicely with the PC, so we need to ignore
  // them for that build target
#ifdef _X360
  ...
  // These are the values we're interested in having returned (must match the
  // indices above)
  const DWORD dwSettingIds[NUM_PROFILE_SETTINGS] = {
      XPROFILE_GAMER_DIFFICULTY, XPROFILE_GAMER_ACTION_MOVEMENT_CONTROL,
      XPROFILE_GAMER_YAXIS_INVERSION, XPROFILE_OPTION_CONTROLLER_VIBRATION};

  // Must have a valid primary user by this point
  int nPrimaryID = XBX_GetPrimaryUserId();

  // First, we call with a NULL pointer and zero size to retrieve the buffer
  // size we'll get back
  ...
  DWORD dwError = XUserReadProfileSettings(0, // Family ID (current title)
                                           nPrimaryID, // User ID
                                           NUM_PROFILE_SETTINGS, dwSettingIds,
                                           &dwResultSize, pResults, NULL);
  ...
  dwError = XUserReadProfileSettings(
      0,          // Family ID (current title)
      nPrimaryID, // User ID
  ...
  Assert(pSetting->data.type == XUSER_DATA_TYPE_INT32);
  Q_snprintf(
      szScratch, sizeof(szScratch), "joy_movement_stick %d",
      (pSetting->data.nData == XPROFILE_ACTION_MOVEMENT_CONTROL_L_THUMBSTICK)
          ? 0
          : 1);
  Cbuf_AddText(szScratch);
  Q_snprintf(
      szScratch, sizeof(szScratch), "joy_movement_stick_default %d",
      (pSetting->data.nData == XPROFILE_ACTION_MOVEMENT_CONTROL_L_THUMBSTICK)
          ? 0
          : 1);
  Cbuf_AddText(szScratch);
  ...
  Assert(pSetting->data.type == XUSER_DATA_TYPE_INT32);
  Q_snprintf(szScratch, sizeof(szScratch), "joy_inverty %d",
             pSetting->data.nData);
  Cbuf_AddText(szScratch);
  Q_snprintf(szScratch, sizeof(szScratch), "joy_inverty_default %d",
             pSetting->data.nData);
  Cbuf_AddText(szScratch);
  ...
  Assert(pSetting->data.type == XUSER_DATA_TYPE_INT32);
  Q_snprintf(szScratch, sizeof(szScratch), "cl_rumblescale %d",
             (pSetting->data.nData != 0) ? 1 : 0);
  Cbuf_AddText(szScratch);
...
//-----------------------------------------------------------------------------
// Purpose: Read our configuration from the 360, filling in defaults on our
// first run
//-----------------------------------------------------------------------------
void Host_ReadConfiguration_360(void) {
  ...
  bool bStorageDeviceValid =
      (XBX_GetStorageDeviceId() != XBX_INVALID_STORAGE_ID &&
       XBX_GetStorageDeviceId() != XBX_STORAGE_DECLINED);
  bool bSaveConfig = false;

  // Call through normal API function once the content container is opened
  if (CommandLine()->CheckParm("-forcexboxreconfig") ||
      bStorageDeviceValid == false ||
      g_pFileSystem->FileExists(strFileName) == false) {
    // If we've already done this in this session, never do it again (we don't
    // want to stomp their settings under any circumstances)
    if (g_bConfigCfgExecuted == false) {
...
void Host_ReadConfiguration() {
  ...
  if (pRemoteStorage) {
    // if cloud settings is default but remote storage does not exist yet, set
    // it to sync all because this is the first computer the game is run
    // on--default to copying everything to the cloud
    ...
    if (cl_cloud_settings.GetInt() == STEAMREMOTESTORAGE_CLOUD_ON) {
      // config files are run through the exec command which got pretty
      // complicated with all the splitscreen stuff. Steam UFS doens't support
      // split screen well (2 users ?)
      GetFileFromRemoteStorage(pRemoteStorage, "cfg/config.cfg",
                               "cfg/config.cfg");
    }
  }
  ...
  if (pRemoteStorage) {
    if (cl_cloud_settings.GetInt() == STEAMREMOTESTORAGE_CLOUD_ON) {
      ...
      Q_SetExtension(g_szDefaultLogoFileName, ".vtf",
                     sizeof(g_szDefaultLogoFileName));
      GetFileFromRemoteStorage(pRemoteStorage, g_szDefaultLogoFileName,
                               g_szDefaultLogoFileName);
  ...
  if (nNumBinds == 0) {
    ...
    // Setup new bindings - if we ship an update that requires that every user
    // has a key bound for X concommand, SetupNewBindings() will do its best to
    // ensure that a key is bound.  We assume that kb_def.lst includes any
    // bindings that SetupNewBindings() might include in the case that nNumBinds
    // == 0 above.
    SetupNewBindings();
  }
...
CON_COMMAND(host_writeconfig,
  ...
  if (args.ArgC() > 3) {
    ...
    ConMsg("<filename.cfg> is required, optionally specify \"full\" to write "
           "out all archived convars.\n");
  ...
  if (args.ArgC() >= 2) {
    ...
    if (!bWriteAll)
      ConMsg("Wrote partial config file \"%s\" out, to write full file use "
             "host_writeconfig \"%s\" full\n",
             outfile, outfile);
  } else {
...
static ConCommand
    recompute_speed("recompute_speed", Host_RecomputeSpeed_f,
...
void Host_BuildConVarUpdateMessage(NET_SetConVar *cvarMsg, int flags,
  ...
  for (var = g_pCVar->GetCommands(); var; var = var->GetNext()) {
    ...
    Q_strncpy(acvar.value, Host_CleanupConVarStringValue(pCvar->GetString()),
              MAX_OSPATH);
...
void CFrameTimer::MarkFrame() {
  ...
  if (host_speeds.GetInt()) {
    ...
    Q_snprintf(sz, sizeof(sz),
               "%3i fps -- inp(%.2f) sv(%.2f) cl(%.2f) render(%.2f) snd(%.2f) "
               "cl_dll(%.2f) exec(%.2f) ents(%d) ticks(%d)",
...
static ConVar
    mem_periodicdumps("mem_periodicdumps", "0", 0,
...
void _Host_RunFrame_Client(bool framefinished) {
  ...
  if (CL_IsHL2Demo() || CL_IsPortalDemo()) // don't need sv.IsDedicated()
                                           // because ded servers don't run this
  {
...
bool CheckVarRange_Generic(ConVar *pVar, int minVal, int maxVal) {
  if (!CanCheat() && !Host_IsSinglePlayerGame()) {
    ...
    if (clampedValue != pVar->GetInt()) {
      Warning("sv_cheats=0 prevented changing %s outside of the range [%d,%d] "
              "(was %d).\n",
...
void _Host_RunFrame_Render() {
  ...
  }
  {
    ...
    tmZone(TELEMETRY_LEVEL0, TMZF_NONE,
...
// threaded path only supported in listen server
...
#endif
    {
      ...
      for (int tick = 0; tick < numticks; tick++) {
        ...
        // Track jitter (delta between ideal time and actual tick execution
        // time)
        host_jitterhistory[host_jitterhistorypos] = jitter;
        host_jitterhistorypos =
            (host_jitterhistorypos + 1) % ARRAYSIZE(host_jitterhistory);

        // Very slowly decay "ideal" towards current wall clock unless delta is
        // large
        if (fabs(jitter) > 1.0f) {
          host_idealtime = now;
        } else {
      ...
      if (sv.IsDedicated() && g_pServerReplayContext &&
          g_pServerReplayContext->IsInitialized()) {
...
#if defined(REPLAY_ENABLED)
        // Update client-side replay history manager - called here since
        // interpolation_amount is set
...
#endif
  //
  // Next check the new method of enabling low violence based on country of
  // purchase and other means that are inaccessible by the user.
  //
...
void Host_PostInit() {
  ...
  if (Q_stricmp(CurrentLocale, en_US)) {
    ...
    SDL_ShowSimpleMessageBox(0, "Warning", MessageText,
...
// Check with steam to see if the requested file (requires full path) is a
// valid, signed binary
bool DLL_LOCAL Host_IsValidSignature(const char *pFilename,
                                     bool bAllowUnknown) {
  ...
  if (sv.IsDedicated() || IsOSX() || IsLinux() || IsBSD()) {
    ...
    if (Steam3Client().SteamUtils()) {
      SteamAPICall_t hAPICall =
          Steam3Client().SteamUtils()->CheckFileSignature(pFilename);
      bool bAPICallFailed = true;
      while (!Steam3Client().SteamUtils()->IsAPICallCompleted(
          hAPICall, &bAPICallFailed)) {
      ...
      if (bAPICallFailed) {
        ...
        Steam3Client().SteamUtils()->GetAPICallResult(
            hAPICall, &result, sizeof(result), result.k_iCallback,
            &bAPICallFailed);
        if (bAPICallFailed) {
          Warning("CheckFileSignature API call on %s failed\n", pFilename);
        } else {
          if (result.m_eCheckFileSignature ==
...
// Ask steam if it is ok to load this DLL.  Unsigned DLLs should not be loaded
// unless the client is running -insecure (testing a plugin for example) This
// keeps legitimate users with modified binaries from getting VAC banned because
// of them
bool DLL_LOCAL Host_AllowLoadModule(const char *pFilename, const char *pPathID,
                                    bool bAllowUnknown,
  ...
  if (sv.IsDedicated() || bIsServerOnly) {
    ...
    // Do we need to do the signature checking? If secure servers are disabled,
    // just skip it.
    if (Host_IsSecureServerAllowed()) {
      if (Steam3Client().SteamUtils()) {
        ...
        if (pPathID) {
          char szFullPath[512];
          const char *pFullPath = g_pFileSystem->RelativePathToFullPath(
              szDllname, pPathID, szFullPath, sizeof(szFullPath));
          if (!pFullPath) {
...
void Host_Init(bool bDedicated) {
  ...
  if (Host_IsSecureServerAllowed()) {
    ...
    if (!Host_AllowLoadModule("engine" DLL_EXT_STRING, "EXECUTABLE_PATH", false,
                              bDedicated)) {
  ...
  if (CommandLine()->FindParm("-nocrashdialog")) {
    // stop the various windows error message boxes from showing up (used by the
    // auto-builder so it doesn't block on error)
    Sys_NoCrashDialog();
  }
...
void AddTransitionResources(CSaveRestoreData *pSaveData, const char *pLevelName,
  ...
  for (int i = 0; i < pSaveData->levelInfo.connectionCount; i++) {
    if (!Q_stricmp(pLevelName, pSaveData->levelInfo.levelList[i].mapName) &&
        !Q_stricmp(pLandmarkName,
                   pSaveData->levelInfo.levelList[i].landmarkName)) {
      transitionMask = 1 << i;
      break;
    }
  }
  ...
  for (int i = 0; i < pSaveData->NumEntities(); i++) {
    if (pSaveData->GetEntityInfo(i)->flags & transitionMask) {
      ...
      // add to the next map's resource list which effectively keeps it from
      // being purged only care about the actual mdl and not any of its
      // dependants
  ...
  if (bHasHumans) {
    // the presence of any human entity in the transition needs to ensure all
    // the human mdls stay
...
bool Host_Changelevel(bool loadfromsavedgame, const char *mapname,
                      const char *start) {
  ...
  if (g_iServerGameDLLVersion >= 10) {
    serverGameDLL->PrepareLevelResources(szMapName, sizeof(szMapName),
                                         szMapFile, sizeof(szMapFile));
  }
  ...
  if (!modelloader->Map_IsValid(szMapFile)) {
    ...
    SCR_EndLoadingPlaque();
#endif
    // We have already inactivated clients at this point due to
    // PrepareLevelResources being blocking, false alarm, tell them to reconnect
    // (which doesn't mean full reconnect, just start rejoining the map)
    //
    // In the likely case that the game DLL tries another map this is harmless,
    // they'll wait on the game server in the connect process if its in another
    // level change by time they get there.
    sv.ReconnectClients();
    return false;
  }

  // If changing from the same map to the same map, optimize by not closing and
  // reopening the packfile which is embedded in the .bsp; we do this by
  // incrementing the packfile's refcount via BeginMapAccess()/EndMapAccess()
  // through the base filesystem API.
  ...
  if (bTransitionBySave) {
    ...
    if (!saverestore->SaveGameSlot("_transition", comment, false, true,
                                   szMapName, startspot)) {
  ...
  if (loadfromsavedgame) {
    ...
    g_pServerPluginHandler->LevelInit(szMapName, CM_EntityString(), oldlevel,
...
#endif
  {
    ...
    audiosourcecache->LevelInit(szMapName);
#endif
    g_pServerPluginHandler->LevelInit(szMapName, CM_EntityString(), NULL, NULL,
                                      false, false);
  }
...
bool Host_NewGame(char *mapName, bool loadGame, bool bBackgroundLevel,
  ...
  // Steam may not have been started yet, ensure it is available to the game DLL
  // before we ask it to prepare level resources
  ...
  g_pServerPluginHandler->LevelInit(szMapName, CM_EntityString(), pszOldMap,
                                    pszLandmark, loadGame && !bOldSave,
                                    bBackgroundLevel);
  ...
  if (!sv.IsDedicated()) {
    ...
    char str[512];
    Q_snprintf(str, sizeof(str), "connect localhost:%d listenserver",
               sv.GetUDPPort());
    Cbuf_AddText(str);
  } else {
