...
bool MD5_MapFile(MD5Value_t *md5value, const char *pszFileName) {
  ...
  if (IsX360()) {
    ...
    V_snprintf(versionString, ARRAYSIZE(versionString), "%d",
               header.lumps[LUMP_MAP_FLAGS].version);
...
// Purpose:
//-----------------------------------------------------------------------------
bool MD5_Hash_Buffer(unsigned char pDigest[16], const unsigned char *pBuffer,
                     int nSize, bool bSeed /* = FALSE */,
                     unsigned int seed[4] /* = NULL */) {
  MD5Context_t ctx;
