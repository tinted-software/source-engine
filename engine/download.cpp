...
ConVar download_debug("download_debug", "0",
                      FCVAR_DONTRECORD); // For debug printouts
...
public:
  ...
  void PersistToDisk(
      const RequestContext_t *rc); ///< Writes out a completed download to disk
  void PersistToCache(
      const RequestContext_t *rc); ///< Writes out a partial download (lost
                                   ///< connection, user abort, etc) to cache

private:
  KeyValues *m_cache;

  void GetCacheFilename(const RequestContext_t *rc, char cachePath[_MAX_PATH]);
  void GenerateCacheFilename(const RequestContext_t *rc,
      ...
      if (bytesWritten != bytesRead) {
        ...
        if (!bMapFile) {
          if (totalBytes > MAX_FILE_SIZE) {
            ConDColorMsg(DownloadErrorColor,
                         "DecompressBZipToDisk: '%s' too big (max %i bytes).\n",
...
void DownloadCache::PersistToDisk(const RequestContext_t *rc) {
  ...
  if (rc && rc->data && rc->nBytesTotal) {
    ...
    if (!g_pFileSystem->FileExists(absPath)) {
      ...
      bool success = false;
      if (rc->bIsBZ2) {
        success = DecompressBZipToDisk(absPath, rc->absLocalPath,
...
//--------------------------------------------------------------------------------------------------------------
void DownloadCache::GetCacheFilename(const RequestContext_t *rc,
                                     char cachePath[_MAX_PATH]) {
  BuildKeyNames(rc->gamePath);
...
//--------------------------------------------------------------------------------------------------------------
void DownloadCache::GenerateCacheFilename(const RequestContext_t *rc,
                                          char cachePath[_MAX_PATH]) {
  GetCacheFilename(rc, cachePath);
  BuildKeyNames(rc->gamePath);
...
public:
  ...
  virtual bool Update(); ///< Monitors download thread, starts new downloads,
...
private:
  void QueueInternal(const char *pBaseURL, const char *pURLPath,
                     const char *pGamePath, bool bAsHttp, bool bCompressed);

protected:
  virtual void UpdateProgressBar();

  virtual RequestContext_t *
  NewRequestContext(); ///< Call this to allocate a RequestContext_t - calls
                       ///< setup functions for derived classes
  virtual bool ShouldAttemptCompressedFileDownload() { return true; }
  virtual void SetupURLPath(RequestContext_t *pRequestContext,
                            const char *pURLPath);
  ...
  void
  PruneCompletedRequests(); ///< Check download requests that have been
                            ///< completed to see if their threads have exited
  void
  ...
  RequestContext_t *m_activeRequest; ///< this is the active request being
...
//--------------------------------------------------------------------------------------------------------------
void CDownloadManager::SetupURLPath(RequestContext_t *pRequestContext,
                                    const char *pURLPath) {
...
void CDownloadManager::SetupServerURL(RequestContext_t *pRequestContext) {
  Q_strncpy(pRequestContext->serverURL,
            cl.m_NetChannel->GetRemoteAddress().ToString(), BufferSize);
}
//--------------------------------------------------------------------------------------------------------------
bool CDownloadManager::HasMapBeenDownloadedFromServer(
...
bool CDownloadManager::FileDenied(const char *filename,
...
bool CDownloadManager::FileReceived(const char *filename,
                                    unsigned int requestID) {
  if (!m_activeRequest)
    return false;
...
//--------------------------------------------------------------------------------------------------------------
void CDownloadManager::MarkMapAsDownloadedFromServer(
    const char *serverMapName) {
  if (!serverMapName)
    return;
...
//--------------------------------------------------------------------------------------------------------------
void CDownloadManager::QueueInternal(const char *pBaseURL, const char *pURLPath,
                                     const char *pGamePath, bool bAsHttp,
                                     bool bCompressed) {
  // NOTE: Assumes valid game path (i.e. IsGamePathValidAndSafe() has been
  // called already)
  ...
  // Setup base path.  We put it in the "download" search path, if they have set
  // one
  char szBasePath[MAX_PATH];
  if (g_pFileSystem->GetSearchPath(k_szDownloadPathID, false, szBasePath,
                                   sizeof(szBasePath)) > 0) {
  ...
  // NOTE: Loose files on disk must always be lowercase!  At least on Linux they
  // HAVE to be, but we do the same thing on Windows to keep things consistent.
  char szGamePathLower[MAX_PATH];
  V_strcpy_safe(szGamePathLower, rc->gamePath);
  V_strlower(szGamePathLower);

  // Now set the full absolute path.  Why does the file system not provide a
  // convenient method to do stuff like this?
...
void CDownloadManager::Queue(const char *baseURL, const char *urlPath,
                             const char *gamePath) {
  ...
  // HTTP or NetChan?
  bool bAsHTTP = baseURL && (!Q_strnicmp(baseURL, "http://", 7) ||
  ...
  // file will abort.
  if (bAsHTTP && ShouldAttemptCompressedFileDownload() &&
      !g_pFileSystem->FileExists(va("%s.bz2", gamePath))) {
  ...
  case HTTP_FETCH:
    ...
    if (m_activeRequest->nBytesTotal) {
      ...
      if (percent != m_lastPercent) {
        if (download_debug.GetBool()) {
          ConDColorMsg(
              DownloadColor, "Downloading %s%s: %3.3d%% - %d of %d bytes\n",
...
//--------------------------------------------------------------------------------------------------------------
bool CL_DownloadUpdate(void) { return s_DownloadManager.Update(); }

//--------------------------------------------------------------------------------------------------------------
void CL_HTTPStop_f(void) { s_DownloadManager.Stop(); }
...
int CL_GetDownloadQueueSize(void) { return s_DownloadManager.GetQueueSize(); }
...
int CL_CanUseHTTPDownload(void) {
  if (sv_downloadurl.GetString()[0]) {
    const char *serverMapName =
        va("%s:%s", sv_downloadurl.GetString(), cl.m_szLevelFileName);
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CDownloadSystem, IDownloadSystem,
                                  INTERFACEVERSION_DOWNLOADSYSTEM,
                                  s_DownloadSystem);
