...
void CPureServerWhitelist::Load(int iPureMode) {
  ...
  // Load base trusted keys
  {
    ...
    bool bLoaded =
        kv->LoadFromFile(g_pFileSystem, "cfg/trusted_keys_base.txt", "game");
    if (bLoaded)
  ...
  if (iPureMode == 0) {
    KeyValues *kv = new KeyValues("");
    bool bLoaded =
        kv->LoadFromFile(g_pFileSystem, "cfg/pure_server_minimal.txt", "game");
    if (bLoaded)
  ...
  // Load up full pure rules
  {
    KeyValues *kv = new KeyValues("");
    bool bLoaded =
        kv->LoadFromFile(g_pFileSystem, "cfg/pure_server_full.txt", "game");
    if (bLoaded)
      bLoaded = LoadCommandsFromKeyValues(kv);
    else
  ...
  if (iPureMode == 1) {
    ...
    KeyValues *kv = new KeyValues("");
    bool bLoaded = kv->LoadFromFile(g_pFileSystem,
                                    "cfg/pure_server_whitelist.txt", "game");
    if (!bLoaded)
    ...
    if (bLoaded)
      ...
      Msg("pure_server_whitelist.txt not present; pure server using only base "
          "file rules\n");
    kv->deleteThis();
    ...
    if (bLoaded)
      ...
      Msg("trusted_keys.txt not present; pure server using only base trusted "
          "key list\n");
    kv->deleteThis();
  }
...
bool operator==(const PureServerPublicKey_t &a,
                const PureServerPublicKey_t &b) {
  return a.Count() == b.Count() && V_memcmp(a.Base(), b.Base(), a.Count()) == 0;
}

bool CPureServerWhitelist::CommandDictDifferent(
    const CUtlDict<CCommand *, int> &a, const CUtlDict<CCommand *, int> &b) {
...
bool CPureServerWhitelist::operator==(const CPureServerWhitelist &x) const {
  ...
  if (CommandDictDifferent(m_FileCommands, x.m_FileCommands) ||
      ...
      CommandDictDifferent(m_NonRecursiveDirCommands,
                           x.m_NonRecursiveDirCommands) ||
      CommandDictDifferent(x.m_NonRecursiveDirCommands,
                           m_NonRecursiveDirCommands))
    return false;
...
bool CPureServerWhitelist::LoadCommandsFromKeyValues(KeyValues *kv) {
  for (KeyValues *pCurItem = kv->GetFirstValue(); pCurItem;
    ...
    bool bFromTrustedSource = false, bAllowFromDisk = false, bCheckCRC = false,
         bAny = false;
    ...
    if ((bFromTrustedSource && (bAllowFromDisk || bCheckCRC || bAny)) ||
        (bAny && bCheckCRC)) {
...
void CPureServerWhitelist::AddHardcodedFileCommands() {
  AddFileCommand("materials/vgui/replay/thumbnails/...",
                 ePureServerFileClass_Any, kLoadOrder_HardcodedOverride);
  AddFileCommand("sound/ui/hitsound.wav", ePureServerFileClass_Any,
                 kLoadOrder_HardcodedOverride);
  AddFileCommand("sound/ui/killsound.wav", ePureServerFileClass_Any,
                 kLoadOrder_HardcodedOverride);
  AddFileCommand("materials/vgui/logos/...", ePureServerFileClass_Any,
                 kLoadOrder_HardcodedOverride);
}

void CPureServerWhitelist::AddFileCommand(const char *pszFilePath,
                                          EPureServerFileClass eFileClass,
...
bool CPureServerWhitelist::LoadTrustedKeysFromKeyValues(KeyValues *kv) {
  for (KeyValues *pCurItem = kv->GetFirstTrueSubKey(); pCurItem;
       pCurItem = pCurItem->GetNextTrueSubKey()) {
    if (V_stricmp(pCurItem->GetName(), "public_key") != 0) {
      Warning("Trusted key list has unexpected block '%s'; expected only "
              "'public_key' blocks\n",
              pCurItem->GetName());
      continue;
    }
    ...
    // Because nobody could *ever* possible attempt to parse bad data.  It could
...
void CPureServerWhitelist::InternalCacheFileCRCs(
  ...
  //	for ( int i=theList.First(); i != theList.InvalidIndex(); i =
  //theList.Next( i ) )
  //	{
  ...
  //			m_pFileSystem->CacheFileCRCs( pPathname, eType,
  //&m_ForceMatchList );
  //		}
  //	}
}
...
void CPureServerWhitelist::DecodeCommandList(
  ...
  for (int i = 0; i < nCommands; i++) {
    ...
    if (nFormatVersion == 0) {
      pCommand->m_eFileClass = (val & 1) ? ePureServerFileClass_Any
...
CPureServerWhitelist::GetBestEntry(const char *pFilename) {
...
void CPureServerWhitelist::AddRef() { ThreadInterlockedIncrement(&m_RefCount); }
...
const byte *CPureServerWhitelist::GetTrustedKey(int iKeyIndex,
                                                int *nKeySize) const {
...
CPureServerWhitelist::GetFileClass(const char *pszFilename) {
  CCommand *pCommand = GetBestEntry(pszFilename);
  if (pCommand)
    return pCommand->m_eFileClass;

  // Default action is to be permissive.  (The default whitelist protects
  // certain directories and files at a root level.)
  return ePureServerFileClass_Any;
}

void CPureFileTracker::AddUserReportedFileHash(int idxFile,
  ...
  if (idxUserReported == m_treeUserReportedFileHash.InvalidIndex()) {
    ...
    if (bAddMasterRecord) {
      ...
      while (idx != m_treeUserReportedFileHash.InvalidIndex() &&
             m_treeUserReportedFileHash[idx].m_idxFile ==
                 m_treeUserReportedFileHash[idxUserReported].m_idxFile) {
        if (m_treeUserReportedFileHash[idx].m_FileHash ==
      ...
      // if ctTotalFiles >> ctMatches then that means clients are reading
      // different bits from the file. in order to get this right we need to ask
      // them to read the entire thing
      if (ctMatches >= sv_pure_consensus.GetInt()) {
        MasterFileHash_t masterFileHashNew;
        masterFileHashNew.m_idxFile =
            m_treeUserReportedFileHash[idxUserReported].m_idxFile;
        masterFileHashNew.m_cMatches = ctMatches;
        masterFileHashNew.m_FileHash =
            m_treeUserReportedFileHash[idxUserReported].m_FileHash;
        m_treeMasterFileHashes.Insert(masterFileHashNew);
        // remove all the individual records that matched the new master, we
  ...
  // we dont have enough data to decide if you match or not yet - so we call it
  // a match
}

void FileRenderHelper(USERID_t userID, const char *pchMessage,
                      const char *pchPath, const char *pchFileName,
                      FileHash_t *pFileHash, int nFileFraction,
                      FileHash_t *pFileHashLocal) {
  char rgch[256];
  char hex[34];
  Q_memset(hex, 0, sizeof(hex));
  Q_binarytohex((const byte *)&pFileHash->m_md5contents.bits,
                sizeof(pFileHash->m_md5contents.bits), hex, sizeof(hex));

  char hex2[34];
  Q_memset(hex2, 0, sizeof(hex2));
  if (pFileHashLocal)
    Q_binarytohex((const byte *)&pFileHashLocal->m_md5contents.bits,
                  sizeof(pFileHashLocal->m_md5contents.bits), hex2,
                  sizeof(hex2));

  if (pFileHash->m_PackFileID) {
    Q_snprintf(rgch, 256,
               "Pure server: file: %s\\%s ( %d %d %8.8x %6.6x ) %s : %s : %s\n",
               pchPath, pchFileName, pFileHash->m_PackFileID,
               pFileHash->m_nPackFileNumber, nFileFraction,
               pFileHash->m_cbFileLen, pchMessage, hex, hex2);
  } else {
    Q_snprintf(rgch, 256,
               "Pure server: file: %s\\%s ( %d %d %x ) %s : %s : %s\n", pchPath,
               pchFileName, pFileHash->m_eFileHashType, pFileHash->m_cbFileLen,
               pFileHash->m_eFileHashType ? pFileHash->m_crcIOSequence : 0,
               pchMessage, hex, hex2);
  }
  if (userID.idtype != 0)
...
bool CPureFileTracker::DoesFileMatch(const char *pPathID,
                                     const char *pRelativeFilename,
                                     int nFileFraction, FileHash_t *pFileHash,
                                     USERID_t userID) {
  //	// if the server has been idle for more than 15 minutes, discard all
  //this data 	const float flRetireTime = sv_pure_retiretime.GetFloat(); 	float
  //flCurTime = Plat_FloatTime(); 	if ( ( flCurTime - m_flLastFileReceivedTime )
  //> flRetireTime )
  //	{
  ...
  //
  //	// The clients must send us all files. We decide if it is whitelisted or
  //not
  //	// That way the clients can not hide modified files in a whitelisted
  //directory 	if ( pFileHash->m_PackFileID == 0 &&
  //		sv.GetPureServerWhitelist()->GetFileClass( pRelativeFilename )
  ...
  //			V_snprintf( warningStr, sizeof( warningStr ), "Pure
  //server: file [%s]\\%s ignored by whitelist.", pPathID, pRelativeFilename );
  ...
  //	Q_strncpy( rgchFilenameFixed, pRelativeFilename, sizeof(
  //rgchFilenameFixed ) ); 	Q_FixSlashes( rgchFilenameFixed );
  //
  //	// first look up the file and see if we have ever seen it before
  //	CRC32_t crcFilename;
  //	CRC32_Init( &crcFilename );
  //	CRC32_ProcessBuffer( &crcFilename, rgchFilenameFixed, Q_strlen(
  //rgchFilenameFixed ) ); 	CRC32_ProcessBuffer( &crcFilename, pPathID, Q_strlen(
  //pPathID ) ); 	CRC32_Final( &crcFilename ); 	UserReportedFile_t ufile;
  ...
  //	EFileCRCStatus eStatus = g_pFileSystem->CheckCachedFileHash( pPathID,
  //rgchFilenameFixed, nFileFraction, &filehashLocal ); 	if ( eStatus ==
  //k_eFileCRCStatus_FileInVPK)
  //	{
  //		// you managed to load a file outside a VPK that the server has
  //in the VPK
  //		// this is possible if the user explodes the VPKs into
  //individual files and then deletes the VPKs 		FileRenderHelper( userID, "file
  //should be in VPK", pPathID, rgchFilenameFixed, pFileHash, nFileFraction,
  //NULL ); 		return false;
  //	}
  //	// if the user sent us a full file hash, but we dont have one, hash it
  //now 	if ( pFileHash->m_eFileHashType == FileHash_t::k_EFileHashTypeEntireFile
  //&& 		( eStatus != k_eFileCRCStatus_GotCRC || filehashLocal.m_eFileHashType !=
  //FileHash_t::k_EFileHashTypeEntireFile ) )
  //	{
  //		// lets actually read the file so we get a complete file hash
  //		FileHandle_t f = g_pFileSystem->Open( rgchFilenameFixed, "rb",
  //pPathID);
  //		// try to load the file and really compute the hash - should
  //only have to do this once ever 		if ( f )
  //		{
  //			// load file into a null-terminated buffer
  //			int fileSize = g_pFileSystem->Size( f );
  //			unsigned bufSize = g_pFileSystem->GetOptimalReadSize( f,
  //fileSize );
  //
  //			char *buffer =
  //(char*)g_pFileSystem->AllocOptimalReadBuffer( f, bufSize ); 			Assert( buffer
  //);
  //
  //			// read into local buffer
  //			bool bRetOK = ( g_pFileSystem->ReadEx( buffer, bufSize,
  //fileSize, f ) != 0 ); 			bRetOK; 			g_pFileSystem->FreeOptimalReadBuffer( buffer
  //);
  //
  //			g_pFileSystem->Close( f );	// close file after
  //reading
  //
  //			eStatus = g_pFileSystem->CheckCachedFileHash( pPathID,
  //rgchFilenameFixed, nFileFraction, &filehashLocal );
  //		}
  //		else
  //		{
  //			// what should we do if we couldn't open the file?
  //should probably kick 			FileRenderHelper( userID, "could not open file to hash
  //( benign for now )", pPathID, rgchFilenameFixed, pFileHash, nFileFraction,
  //NULL );
  //		}
  //	}
  //	if ( eStatus == k_eFileCRCStatus_GotCRC )
  //	{
  //		if ( filehashLocal.m_eFileHashType ==
  //FileHash_t::k_EFileHashTypeEntireFile && 			pFileHash->m_eFileHashType ==
  //FileHash_t::k_EFileHashTypeEntireFile )
  //		{
  ...
  //				// did not match - record so that we have a
  //record of the file that did not match ( just for reporting )
  //				AddUserReportedFileHash( idxFile, pFileHash,
  //userID, false ); 				FileRenderHelper( userID, "file does not match", pPathID,
  //rgchFilenameFixed, pFileHash, nFileFraction, &filehashLocal ); 				return false;
  //			}
  //		}
  //	}
  //
  //	// if this is a VPK file, we have completely cataloged all the VPK
  //files, so no suprises are allowed 	if ( pFileHash->m_PackFileID )
  //	{
  //		AddUserReportedFileHash( idxFile, pFileHash, userID, false );
  //		FileRenderHelper( userID, "unrecognized vpk file", pPathID,
  //rgchFilenameFixed, pFileHash, nFileFraction, NULL ); 		return false;
  //	}
  //
  //
  ...
  //
  //		FileHash_t *pFileHashLocal =
  //&m_treeMasterFileHashes[idxMaster].m_FileHash; 		if ( *pFileHashLocal ==
  //*pFileHash )
  //		{
  ...
  //		else
  //		{
  //			// did not match - record so that we have a record of
  //the file that did not match ( just for reporting ) 			AddUserReportedFileHash(
  //idxFile, pFileHash, userID, false );
  //			// and then return failure
  //			FileRenderHelper( userID, "file does not match server
  //master file", pPathID, rgchFilenameFixed, pFileHash, nFileFraction,
  //pFileHashLocal ); 			return false;
  //		}
  //	}
  //
  //	// no master record, accumulate individual record so we can get a
  //consensus 	if ( sv_pure_trace.GetInt() == 3 )
  //	{
  //		FileRenderHelper( userID, "server does not have hash for this
  //file. Waiting for consensus", pPathID, rgchFilenameFixed, pFileHash,
  //nFileFraction, NULL );
  //	}
  //
...
class CStupidLess {
public:
  bool Less(const FindFileIndex_t &src1, const FindFileIndex_t &src2,
            void *pCtx) {
...
int CPureFileTracker::ListUserFiles(bool bListAll,
                                    const char *pchFilenameFind) {
  ...
  while (idxFindFile != m_treeAllReportedFiles.InvalidIndex()) {
    ...
    if (pchFilenameFind && Q_stristr(ufile.m_filename, pchFilenameFind)) {
      FileHash_t filehashLocal;
      EFileCRCStatus eStatus = g_pFileSystem->CheckCachedFileHash(
          ufile.m_path.String(), ufile.m_filename.String(), 0, &filehashLocal);

      if (eStatus == k_eFileCRCStatus_GotCRC) {
        USERID_t useridFake;
        useridFake.idtype = IDTYPE_STEAM;
        FileRenderHelper(useridFake, "Found: ", ufile.m_path.String(),
                         ufile.m_filename.String(), &filehashLocal, 0, NULL);
        FindFileIndex_t ffi;
        ffi.idxFindFile = idxFindFile;
        m_vecReportedFiles.Insert(ffi);
      } else {
        Msg("File not found %s %s %x\n", ufile.m_filename.String(),
            ufile.m_path.String(), idxFindFile);
      }
    }
  ...
  while (idx != m_treeUserReportedFileHash.InvalidIndex()) {
    ...
    while (idxNext != m_treeUserReportedFileHash.InvalidIndex() &&
           m_treeUserReportedFileHash[idx].m_idxFile ==
               m_treeUserReportedFileHash[idxNext].m_idxFile) {
      if (m_treeUserReportedFileHash[idx].m_FileHash ==
          m_treeUserReportedFileHash[idxNext].m_FileHash) {
        ctMatches++;
        cTotalMatches++;
      }
      ctFiles++;
    ...
    if (Q_stristr(ufile.m_filename.String(), "bin\\pak01") != NULL ||
      ...
      if (eStatus == k_eFileCRCStatus_GotCRC) {
        if (filehashLocal.m_eFileHashType ==
                FileHash_t::k_EFileHashTypeEntireFile &&
            file.m_FileHash.m_eFileHashType ==
                FileHash_t::k_EFileHashTypeEntireFile &&
    ...
    if (ctMatches != ctFiles ||
        idxMaster != m_treeMasterFileHashes.InvalidIndex() || bListAll ||
        (pchFilenameFind && m_vecReportedFiles.Find(ffi) != -1) || bOutput) {
      char rgch[256];
      Q_snprintf(rgch, 256, "reports=%d matches=%d Hash details:", ctFiles,
                 ctMatches);
      FileHash_t *pFileHashMaster = NULL;
      if (idxMaster != m_treeMasterFileHashes.InvalidIndex())
        pFileHashMaster = &m_treeMasterFileHashes[idxMaster].m_FileHash;
      FileRenderHelper(file.m_userID, rgch, ufile.m_path.String(),
                       ufile.m_filename.String(), &file.m_FileHash, 0,
                       pFileHashMaster);
    }
  }
  Msg("Total user files %d %d %d \n", m_treeUserReportedFileHash.Count(),
      cTotalFiles, cTotalMatches);
  Msg("Total files %d, total with authoritative hashes %d \n",
      m_treeAllReportedFiles.Count(), m_treeMasterFileHashes.Count());
  Msg("Matching files %d %d %d \n", m_cMatchedFile, m_cMatchedMasterFile,
      m_cMatchedMasterFileHash);

  return 0;
}

int CPureFileTracker::ListAllTrackedFiles(bool bListAll,
                                          const char *pchFilenameFind,
                                          int nFileFractionMin,
  ...
  do {
    ...
    count = g_pFileSystem->GetUnverifiedFileHashes(
        rgUnverifiedFiles, ARRAYSIZE(rgUnverifiedFiles));

    if (count &&
        (bListAll ||
         (pchFilenameFind &&
          Q_stristr(rgUnverifiedFiles[0].m_Filename, pchFilenameFind) &&
          rgUnverifiedFiles[0].m_nFileFraction >= nFileFractionMin &&
          rgUnverifiedFiles[0].m_nFileFraction <= nFileFractionMax))) {
      USERID_t useridFake;
      useridFake.idtype = IDTYPE_STEAM;
      FileRenderHelper(useridFake, "", rgUnverifiedFiles[0].m_PathID,
                       rgUnverifiedFiles[0].m_Filename,
                       &rgUnverifiedFiles[0].m_FileHash,
                       rgUnverifiedFiles[0].m_nFileFraction, NULL);
      if (rgUnverifiedFiles[0].m_FileHash.m_PackFileID) {
        g_pFileSystem->CheckVPKFileHash(
            rgUnverifiedFiles[0].m_FileHash.m_PackFileID,
            rgUnverifiedFiles[0].m_FileHash.m_nPackFileNumber,
            rgUnverifiedFiles[0].m_nFileFraction,
            rgUnverifiedFiles[0].m_FileHash.m_md5contents);
      }
      cTotalMatch++;
    }
    if (count)
...
static ConCommand svpurelistuserfiles("sv_pure_listuserfiles",
                                      CC_ListPureServerFiles,
                                      "ListPureServerFiles");
...
static ConCommand svpurefinduserfiles("sv_pure_finduserfiles",
                                      CC_PureServerFindFile,
                                      "ListPureServerFiles");

void CC_PureServerListTrackedFiles(const CCommand &args) {
  // BUGBUG! Because this code is in engine instead of server, it exists in the
  // client - ugh! Remove this command from client before shipping for realz.
  // if ( !sv.IsDedicated() )
  //	return;
  ...
  g_PureFileTracker.ListAllTrackedFiles(args.ArgC() <= 1,
                                        args.ArgC() >= 2 ? args[1] : NULL,
                                        nFileFractionMin, nFileFractionMax);
}

static ConCommand svpurelistfiles("sv_pure_listfiles",
                                  CC_PureServerListTrackedFiles,
                                  "ListPureServerFiles");

void CC_PureServerCheckVPKFiles(const CCommand &args) {
  if (sv.IsDedicated())
    Plat_BeginWatchdogTimer(
        5 * 60); // reset watchdog timer to allow 5 minutes for the VPK check
  g_pFileSystem->CacheAllVPKFileHashes(false, true);
  if (sv.IsDedicated())
    Plat_EndWatchdogTimer();
}

static ConCommand svpurecheckvpks("sv_pure_checkvpk",
                                  CC_PureServerCheckVPKFiles,
