...
csurface_t CCollisionBSPData::nullsurface = {
    "**empty**", 0, 0}; // generic null collision model surface
...
static int FASTCALL CM_BrushBoxContents(CCollisionBSPData *pBSPData,
                                        const Vector &vMins,
                                        const Vector &vMaxs, cbrush_t *pBrush) {
  if (pBrush->IsBox()) {
...
static int FASTCALL CM_BrushPointContents(CCollisionBSPData *pBSPData,
                                          const Vector &vPos,
                                          cbrush_t *pBrush) {
  if (pBrush->IsBox()) {
    cboxbrush_t *pBox = &pBSPData->map_boxbrushes[pBrush->GetBox()];
...
void FASTCALL CM_TraceToLeaf(TraceInfo_t *RESTRICT pTraceInfo, int ndxLeaf,
  ...
  if (pLeaf->dispCount) {
    ...
    if (IsX360()) {
      ...
      static const fltx4 vecEpsilon = {
          DISPCOLL_DIST_EPSILON, DISPCOLL_DIST_EPSILON, DISPCOLL_DIST_EPSILON,
      ...
      // TODO: this loop probably ought to be unrolled so that we can make a
      // more efficient job of intersecting rays against boxes. The simple SIMD
      // version used here, though about 6x faster than the fpu version, is
      // slower still than intersecting against four boxes simultaneously.
      for (int i = 0; i < pLeaf->dispCount; i++) {
        ...
        if (IS_POINT) {
          if (!IsBoxIntersectingRay(LoadAlignedSIMD(pDispBounds->mins.Base()),
                                    LoadAlignedSIMD(pDispBounds->maxs.Base()),
                                    traceStart, traceDelta, traceInvDelta,
                                    vecEpsilon))
            continue;
        } else {
          fltx4 mins =
              SubSIMD(LoadAlignedSIMD(pDispBounds->mins.Base()), extents);
          fltx4 maxs =
              AddSIMD(LoadAlignedSIMD(pDispBounds->maxs.Base()), extents);
          if (!IsBoxIntersectingRay(mins, maxs, traceStart, traceDelta,
...
void CM_DecompressVis(CCollisionBSPData *pBSPData, int cluster, int visType,
  ...
  byte *in =
      ((byte *)pBSPData->map_vis) + pBSPData->map_vis->bitofs[cluster][visType];
...
bool CM_AreasConnected(int area1, int area2) {
  ...
  if (area1 >= pBSPData->numareas || area2 >= pBSPData->numareas) {
    Sys_Error("area(1==%i, 2==%i) >= numareas (%i):  Check if "
              "engine->ResetPVS() was called from ClientSetupVisibility",
...
int CM_BoxVisible(const Vector &mins, const Vector &maxs, const byte *visbits,
                  int vissize) {
