...
public:
  ...
  unsigned int Count() { return m_Lookup.Count(); }

  void Purge() { m_Lookup.Purge(); }

  const char *String(int index) { return m_Lookup.Key(index).Get(); }

  bool IsValidIndex(int index) { return m_Lookup.IsValidHandle(index); }

  int Insert(const char *pString) { return m_Lookup.Insert(pString); }
...
private:
  CUtlStableHashtable<CUtlConstString, CNetworkStringTableItem,
...
CNetworkStringTable::CNetworkStringTable(TABLEID id, const char *tableName,
  ...
  if (m_nUserDataSizeBits > CNetworkStringTableItem::MAX_USERDATA_BITS) {
    Host_Error("String tables user data bits restricted to %i bits, requested "
               "%i is too large\n",
  ...
  if (m_nUserDataSize > CNetworkStringTableItem::MAX_USERDATA_SIZE) {
    Host_Error("String tables user data size restricted to %i bytes, requested "
               "%i is too large\n",
  ...
  if ((1 << m_nEntryBits) != maxentries) {
    Host_Error("String tables must be powers of two in size!, %i is not a "
               "power of 2\n",
               maxentries);
  }
...
void CNetworkStringTable::SetAllowClientSideAddString(bool state) {
  ...
  if (m_bAllowClientSideAddString) {
    ...
    m_pItemsClientSide->Insert(
        "___clientsideitemsplaceholder1___"); // -1 can't be used since it looks
                                              // like the "invalid" index from
                                              // other string lookups
  }
}
...
int CNetworkStringTable::WriteUpdate(CBaseClient *client, bf_write &buf,
  ...
  ETWMark2I(GetTableName(), entriesUpdated,
            buf.GetNumBitsWritten() - nTableStartBit);
  ...
  for (int i = 0; i < entries; i++) {
    ...
    if (buf.ReadOneBit()) {
      ...
      if (substringcheck) {
        ...
        Q_strncpy(entry, history[index].string,
                  Min(sizeof(entry), (size_t)bytestocopy + 1));
...
#ifdef _DEBUG
  if (m_bLocked) {
    DevMsg("Warning! CNetworkStringTable::SetStringUserData (%s): changing "
           "entry %i while locked.\n",
  ...
  if (m_pItemsClientSide) {
    ...
    for (int i = 0; i < numstrings; i++) {
      ...
      const void *pUserData =
          m_pItemsClientSide->Element(i).GetUserData(&userDataSize);
      if (userDataSize > 0) {
        buf.WriteOneBit(1);
        buf.WriteWord((short)userDataSize);
        buf.WriteBytes(pUserData, userDataSize);
...
bool CNetworkStringTable::WriteBaselines(SVC_CreateStringTable &msg,
                                         char *msg_buffer,
                                         int msg_buffer_size) {
  VPROF_BUDGET("CNetworkStringTable::WriteBaselines",
...
//-----------------------------------------------------------------------------
INetworkStringTable *
CNetworkStringTableContainer::FindTable(const char *tableName) const {
  for (int i = 0; i < m_Tables.Count(); i++) {
...
//-----------------------------------------------------------------------------
INetworkStringTable *
CNetworkStringTableContainer::GetTable(TABLEID stringTable) const {
...
void CNetworkStringTableContainer::WriteBaselines(bf_write &buf) {
  VPROF_BUDGET("CNetworkStringTableContainer::WriteBaselines",
               VPROF_BUDGETGROUP_OTHER_NETWORKING);
  ...
  if (!msg_buffer) {
    Host_Error("Failed to allocate %llu bytes of memory in "
  ...
  for (int i = 0; i < m_Tables.Count(); i++) {
    ...
    if (msg.m_DataOut.IsOverflowed()) {
      Warning("Warning:  Overflowed writing uncompressed string table data for "
              "%s\n",
              table->GetTableName());
    }

    msg.m_bDataCompressed = false;
    if (msg.m_DataOut.GetNumBytesWritten() >=
        sv_compressstringtablebaselines_threshhold.GetInt()) {
      CFastTimer compressTimer;
      compressTimer.Start();

      // TERROR: bzip-compress the stringtable before adding it to the packet.
      // Yes, the whole packet will be bzip'd, but the uncompressed data also
      // has to be under the NET_MAX_PAYLOAD limit.
      ...
      if (COM_BufferToBufferCompress_Snappy(compressedData, &compressedSize,
        ...
        msg.m_DataOut.WriteBits(compressedData,
                                compressedSize * 8); // compressed data

        // if ( compressstringtablbaselines > 1 )
        {
          compressTimer.End();
          DevMsg("Stringtable %s compression: %d -> %d bytes: %.2fms\n",
                 table->GetTableName(), numBytes, compressedSize,
    ...
    if (sv_dumpstringtables.GetBool()) {
      DevMsg("CNetworkStringTableContainer::WriteBaselines wrote %d bytes for "
             "table %s [space remaining %d bytes]\n",
