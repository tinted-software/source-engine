...
CDecalVert *R_DoDecalSHClip(CDecalVert *pInVerts, CDecalVert *pOutVerts,
  ...
  if (outCount && pDecal->material->InMaterialPage()) {
    ...
    for (int i = 0; i < outCount; ++i) {
      pOutVerts[i].m_ctCoords.x =
          offset[0] + (pOutVerts[i].m_ctCoords.x * scale[0]);
      pOutVerts[i].m_ctCoords.y =
          offset[1] + (pOutVerts[i].m_ctCoords.y * scale[1]);
    }
  }
...
// array, 'verts'.
void R_SetupDecalVertsForMSurface(decal_t *RESTRICT pDecal,
                                  SurfaceHandle_t surfID,
                                  Vector *RESTRICT pTextureSpaceBasis,
                                  CDecalVert *RESTRICT pVerts) {
  unsigned short *RESTRICT pIndices =
  ...
  for (int j = 0; j < count; j++) {
    ...
    pVerts[j].m_vPos = host_state.worldbrush->vertexes[vertIndex]
                           .position; // Copy model space coordinates
    // garymcthack - what about m_ParentTexCoords?
    pVerts[j].m_ctCoords.x =
        DotProduct(pVerts[j].m_vPos, pTextureSpaceBasis[0]) + uOffset;
    pVerts[j].m_ctCoords.y =
        DotProduct(pVerts[j].m_vPos, pTextureSpaceBasis[1]) + vOffset;
