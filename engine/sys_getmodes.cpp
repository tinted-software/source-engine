...
ConVar cl_savescreenshotstosteam(
    "cl_savescreenshotstosteam", "0", FCVAR_HIDDEN,
    "Saves screenshots to the Steam's screenshot library");
ConVar cl_screenshotusertag("cl_screenshotusertag", "", FCVAR_HIDDEN,
                            "User to tag in the screenshot");
ConVar cl_screenshotlocation("cl_screenshotlocation", "", FCVAR_HIDDEN,
...
protected:
  ...
  void WriteScreenshotToSteam(uint8 *pImage, int cubImage, int width,
                              int height);
...
private:
  ...
  void BlitGraphicToHDC(HDC hdc, byte *rgba, int imageWidth, int imageHeight,
                        int x0, int y0, int x1, int y1);
  void BlitGraphicToHDCWithAlpha(HDC hdc, byte *rgba, int imageWidth,
...
//-----------------------------------------------------------------------------
int CVideoMode_Common::GetModeStereoWidth(void) const { return m_nStereoWidth; }
...
int CVideoMode_Common::GetModeUIWidth(void) const { return m_nUIWidth; }

int CVideoMode_Common::GetModeUIHeight(void) const { return m_nUIHeight; }
...
int CVideoMode_Common::FindVideoMode(int nDesiredWidth, int nDesiredHeight,
  ...
  // If we want to scale the 3D portion of the game and leave the UI at the same
  // res, then
  //	re-enable this code. Not that on retina displays the UI will be super
  //small and that 	should probably be fixed.
#if 0
...
void CVideoMode_Common::ResetCurrentModeForNewResolution(int nWidth,
  ...
  if (UseVR() || ShouldForceVRActive()) {
    g_pSourceVR->GetViewportBounds(ISourceVirtualReality::VREye_Left, NULL,
                                   NULL, &m_nStereoWidth, &m_nStereoHeight);
    VRRect_t vrBounds;
  ...
  } else if (materials->GetCurrentConfigForVideoCard().m_nVRModeAdapter ==
             materials->GetCurrentAdapter()) {
    // if we aren't in VR mode but we do have a VR mode adapter set, we must not
    // be full screen because that would show up on the HMD
    m_bWindowed = true;
  }
}
...
bool CVideoMode_Common::CreateGameWindow(int nWidth, int nHeight,
  ...
  if (!InEditMode()) {
    ...
    // Play our videos for the background after the render device has been
    // initialized
    DrawStartupVideo();
...
//-----------------------------------------------------------------------------
void CVideoMode_Common::WriteScreenshotToSteam(uint8 *pImage, int cubImage,
                                               int width, int height) {
#if !defined(NO_STEAM)
  if (cl_savescreenshotstosteam.GetBool()) {
    if (Steam3Client().SteamScreenshots()) {
      ScreenshotHandle hScreenshot =
          Steam3Client().SteamScreenshots()->WriteScreenshot(pImage, cubImage,
                                                             width, height);
...
// and applies any tags that have been set for it
//-----------------------------------------------------------------------------
void CVideoMode_Common::AddScreenshotToSteam(const char *pchFilename, int width,
                                             int height) {
#if !defined(NO_STEAM)
  if (cl_savescreenshotstosteam.GetBool()) {
    if (Steam3Client().SteamScreenshots()) {
      ScreenshotHandle hScreenshot =
          Steam3Client().SteamScreenshots()->AddScreenshotToLibrary(
              pchFilename, NULL, width, height);
      ApplySteamScreenshotTags(hScreenshot);
    }
  }
  cl_screenshotusertag.SetValue(0);
  ...
  if (hScreenshot != INVALID_SCREENSHOT_HANDLE) {
    if (cl_screenshotusertag.GetBool()) {
      if (Steam3Client().SteamUtils()) {
        CSteamID steamID(cl_screenshotusertag.GetInt(),
                         Steam3Client().SteamUtils()->GetConnectedUniverse(),
...
void CVideoMode_Common::SetupStartupGraphic() {
  ...
  // loading.vtf
  buf.Clear(); // added this Clear() because we saw cases where LoadVTF was not
               // emptying the buf fully in the above section
  ...
  if (debugstartup) {
    for (int repeat = 0; repeat < 100000; repeat++) {
      ...
      if (1) // draw normal BK
      {
        ...
        DrawScreenSpaceRectangle(pMaterial, 0, 0 + slide, w, h - 50, 0, 0,
                                 tw - 1, th - 1, tw, th, NULL, 1, 1, depth);
        if (!IsSteamDeck())
          DrawScreenSpaceRectangle(pLoadingMaterial, w - lw, h - lh + slide / 2,
                                   lw, lh, 0, 0, lw - 1, lh - 1, lw, lh, NULL,
                                   1, 1, depth - 0.1);
        else
          // TODO: Steam Deck
          DrawScreenSpaceRectangle(pLoadingMaterial, w - lw, h - lh + slide / 2,
      ...
      if (0) {
        ...
        for (int x = 0; x < grid_size; x++) {
          ...
          for (int y = 0; y < grid_size; y++) {
            ...
            // if (! ((x^y) & 1) )
            {
              DrawScreenSpaceRectangle(pMaterial, 10.0f + cornerx,
                                       10.0f + cornery, 15, 15, 0, 0, tw - 1,
    ...
    for (int i = 0; i < 2; ++i) {
      ...
      DrawScreenSpaceRectangle(pLoadingMaterial, w - lw, h - lh, lw, lh, 0, 0,
                               lw - 1, lh - 1, lw, lh, NULL, 1, 1, depth);
      g_pMaterialSystem->SwapBuffers();
    }
  }
...
//-----------------------------------------------------------------------------
void CVideoMode_Common::BlitGraphicToHDCWithAlpha(HDC hdc, byte *rgba,
                                                  int imageWidth,
...
void CVideoMode_Common::InvalidateWindow() {
  if (CommandLine()->FindParm("-noshaderapi")) {
    ...
    fake.window.windowID =
        SDL_GetWindowID((SDL_Window *)g_pLauncherMgr->GetWindowRef());
...
// Purpose: Blits an image to the loading window hdc
//-----------------------------------------------------------------------------
void CVideoMode_Common::BlitGraphicToHDC(HDC hdc, byte *rgba, int imageWidth,
...
#define FIXED_BLEND(a, b, out, frac)                                           \
  ...
  for (int v = 0; v < tall; v++) {
    ...
    for (int u = 0; u < dibwide; u++, dest += 3) {
      ...
      byte *xsrc[4] = {srcTopLine + (iBilinearU + 0) * 4,
...
void CVideoMode_Common::AdjustWindow(int nWidth, int nHeight, int nBPP,
  ...
  if (bWindowed) {
    ...
    // Remove window border going into fullscreen mode to avoid Vista sizing
    // issues when DWM is enabled
    style &= ~WS_OVERLAPPEDWINDOW;
  }
  ...
  // Now center
  CenterEngineWindow(game->GetMainWindow(), WindowRect.right - WindowRect.left,
                     WindowRect.bottom - WindowRect.top);
#if defined(USE_SDL)
  g_pLauncherMgr->SetWindowFullScreen(!bWindowed,
                                      WindowRect.right - WindowRect.left,
                                      WindowRect.bottom - WindowRect.top);
  ...
  g_pLauncherMgr->SizeWindow(WindowRect.right - WindowRect.left,
                             WindowRect.bottom - WindowRect.top);
...
// Output : static void
//-----------------------------------------------------------------------------
void CVideoMode_Common::CenterEngineWindow(void *hWndCenter, int width,
                                           int height) {
  int CenterX, CenterY;
...
void CVideoMode_Common::TakeSnapshotTGA(const char *pFilename) {
  ...
  ReadScreenPixels(0, 0, GetModeStereoWidth(), GetModeStereoHeight(), pImage,
                   IMAGE_FORMAT_RGB888);

  CUtlBuffer outBuf;
  if (TGAWriter::WriteToBuffer(pImage, outBuf, GetModeStereoWidth(),
                               GetModeStereoHeight(), IMAGE_FORMAT_RGB888,
                               IMAGE_FORMAT_RGB888)) {
    if (!g_pFileSystem->WriteFile(pFilename, NULL, outBuf)) {
      ...
      if (g_pFileSystem->GetLocalPath(pFilename, szPath, sizeof(szPath))) {
        AddScreenshotToSteam(szPath, GetModeStereoWidth(),
                             GetModeStereoHeight());
      }
    }
  }
...
void CVideoMode_Common::BlitHiLoScreenBuffersTo16Bit(void) {
  if (IsX360()) {
    // FIXME: this breaks in 480p due to (at least) the multisampled depth
    // buffer (need to cache, clear and restore the depth target)
    Assert(0);
    return;
  }
...
void CVideoMode_Common::WriteMovieFrame(const MovieInfo_t &info) {
  ...
  ReadScreenPixels(0, 0, GetModeStereoWidth(), GetModeStereoHeight(), hp,
                   IMAGE_FORMAT_BGR888);

  // Store frame to disk
  if (info.DoTga()) {
    VID_ProcessMovieFrame(
        info, false, va("%s%04d.tga", pMovieName, nMovieFrame),
  ...
  if (info.DoJpg()) {
    VID_ProcessMovieFrame(info, true, va("%s%04d.jpg", pMovieName, nMovieFrame),
                          GetModeStereoWidth(), GetModeStereoHeight(),
                          (unsigned char *)hp);
  }
...
void CVideoMode_Common::TakeSnapshotJPEG(const char *pFilename, int quality) {
  ...
  char final[64];
  Q_strncpy(orig,
            Q_pretifymem(GetModeStereoWidth() * 3 * GetModeStereoHeight(), 2),
            sizeof(orig));
  Q_strncpy(final, Q_pretifymem(finalSize, 2), sizeof(final));

  Msg("Wrote '%s':  %s (%dx%d) compresssed (quality %i) to %s\n", pFilename,
      orig, GetModeStereoWidth(), GetModeStereoHeight(), quality, final);
  ...
  for (int i = 0; i < nModeCount; i++) {
    ...
    for (int j = 0; j < m_nNumModes; j++) {
      if (info.m_Width == m_rgModeList[j].width &&
        ...
        // in VR mode we want the highest refresh rate, without regard for the
...
bool CVideoMode_MaterialSystem::SetMode(int nWidth, int nHeight,
  ...
  config.SetFlag(MATSYS_VIDCFG_FLAGS_SCALE_TO_OUTPUT_RESOLUTION,
                 (DWORD)nWidth != videoMode.dwDisplayWidth ||
                     (DWORD)nHeight != videoMode.dwDisplayHeight);
  if (nHeight == 480 || nWidth == 576) {
    // Use 2xMSAA for standard def (see mat_software_aa_strength for fake hi-def
    // aa)
    // FIXME: shuffle the EDRAM surfaces to allow 4xMSAA for standard def
    //        (they would overlap & trash each other with the current
    //        arrangement)
    // NOTE: This should affect 640x480 and 848x480 (which is also used for
    // 640x480 widescreen), and PAL 640x576
...
void CVideoMode_MaterialSystem::ChangeDisplaySettingsToFullscreen(int nWidth,
  ...
  DEVMODE dmCurrent;
  if (EnumDisplaySettings(materials->GetDisplayDeviceName(),
      ...
      ((dm.dmFields & DM_DISPLAYFREQUENCY) == 0 ||
       dmCurrent.dmDisplayFrequency == dm.dmDisplayFrequency)) {
    return;
  }
#endif

  if (nWidth == m_nLastCDSWidth && nHeight == m_nLastCDSHeight &&
      nBPP == m_nLastCDSBPP && freq == m_nLastCDSFreq)
    return;
  ...
  ChangeDisplaySettingsEx(materials->GetDisplayDeviceName(), &dm, NULL,
                          CDS_FULLSCREEN, NULL);
#elif defined(USE_SDL)
