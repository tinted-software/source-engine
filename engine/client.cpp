...
ConVar cl_logofile("cl_logofile", "materials/decals/spraylogo.vtf",
                   FCVAR_ARCHIVE,
...
static ConVar cl_downloadfilter("cl_downloadfilter", "all", FCVAR_ARCHIVE,
                                ...
                                "the server (all, none, nosounds, mapsonly)");

#ifdef OSX
// OS X is barely making it due to virtual memory pressure on 32bit, our
// behavior of load new models -> unload unused is far too abusive for its
// estimated margin of maybe two or three bytes before crashing.
#define CONVAR_DEFAULT_ALWAYS_FLUSH_MODELS "1"
#else
#define CONVAR_DEFAULT_ALWAYS_FLUSH_MODELS "0"
#endif
static ConVar
    cl_always_flush_models("cl_always_flush_models",
                           CONVAR_DEFAULT_ALWAYS_FLUSH_MODELS,
                           FCVAR_INTERNAL_USE,
                           "If set, always flush models between map loads.  "
                           "Useful on systems under memory pressure.");

extern ConVar sv_downloadurl;

#if defined(_DEBUG) || defined(STAGING_ONLY)
static ConVar
    debug_clientstate_fake_hltv("debug_clientstate_fake_hltv", "0", 0,
                                "If set, spoof as HLTV for testing purposes.");
#if defined(REPLAY_ENABLED)
static ConVar debug_clientstate_fake_replay(
    "debug_clientstate_fake_replay", "0", 0,
...
#if !defined(NO_STEAM)
  info.m_nFriendsID =
      Steam3Client().SteamUser()
          ? Steam3Client().SteamUser()->GetSteamID().GetAccountID()
          : 0;
#else
  info.m_nFriendsID = 0;
#endif
...
float CClientState::GetClientInterpAmount() {
  ...
  const ConVar_ServerBounded *pBounded =
      static_cast<const ConVar_ServerBounded *>(s_cl_interp_ratio);
  if (pBounded)
...
void CClientState::SetModel(int tableIndex) {
  ...
  if (tableIndex == 1) {
    // The world model must match the LevelFileName -- it is the path we just
    // checked the CRC for, and paths may differ from what the server is using
    // based on what the gameDLL override does
...
int CClientState::LookupGenericIndex(char const *name) {
  if (!m_pGenericPrecacheTable) {
    Warning("Can't LookupGenericIndex( %s ), no precache table [no level "
            "loaded?]\n",
            name);
    return -1;
  }
...
void CClientState::AddCustomFile(int slot, const char *resourceFile) {
  ...
  if (g_pFileSystem->RelativePathToFullPath(
          filehex.m_Filename, "game", szAbsFilename, sizeof(szAbsFilename),
          FILTER_CULLPACK)) {
...
void CClientState::ReadPreserveEnt(CEntityReadInfo &u) {
  ...
  // XXX(JohnS): This was historically checking for NewEntity overflow, though
  // this path does not care (and new entity
  //             may be -1).  The old entity bounds check here seems like what
  //             was intended, but since nNewEntity should not be overflowed
  //             either, I've left that check in case it was guarding against a
  //             case I am overlooking.
  if (u.m_nOldEntity >= MAX_EDICTS || u.m_nOldEntity < 0 ||
      u.m_nNewEntity >= MAX_EDICTS) {
...
void CClientState::StartUpdatingSteamResources() {
  ...
  m_hWaitForResourcesHandle =
      g_pFileSystem->WaitForResources(m_szLevelBaseName);
  m_bUpdateSteamResources = false;
  m_bShownSteamResourceUpdateProgress = false;
  m_bDownloadResources = false;
...
void CClientState::CheckUpdatingSteamResources() {
  ...
  if (m_bPrepareClientDLL) {
    ...
    V_strncpy(szMutableLevelName, m_szLevelBaseName,
              sizeof(szMutableLevelName));

    // if the game .dll doesn't support this call assume everything is prepared
    IServerGameDLL::ePrepareLevelResourcesResult eResult =
        IServerGameDLL::ePrepareLevelResources_Prepared;
    if (g_iServerGameDLLVersion >= 10) {
      eResult = serverGameDLL->AsyncPrepareLevelResources(
          szMutableLevelName, sizeof(szMutableLevelName), m_szLevelFileName,
          sizeof(m_szLevelFileName), &flPrepareProgress);
    }
...
void CClientState::CheckFileCRCsWithServer() {
  ...
  //!
  //! 1.) Removed the CRC functionality (because it was broken when we switched
  //! to use MD5's for hashes of
  //!     loose files, but the server only has CRC's of some files in the VPK
  //!     headers.).  Currently the only supported pure server mode is "trusted
  //!     source."
  //! 2.) Sending MD5's of VPK's is a bit too restrictive for most use cases.
  //! For example, if a client
  //!     has an extra VPK for custom content, the server doesn't know what to
  //!     do with it.  Or if we release an optional update, the VPK's might
  //!     legitimately differ.
  //!
  //! Rich has pointed out that we really need pure server client work to be
  //! something that the client cannot easily bypass.  Currently that is the
  //! case.  But I need to ship the SteamPipe conversion now. We can revisit
  //! pure server security after that has shipped.
  //
  //	VPROF_( "CheckFileCRCsWithServer", 1,
  //VPROF_BUDGETGROUP_OTHER_NETWORKING, false, BUDGETFLAG_CLIENT ); 	const float
  //flBatchInterval = 1.0f / 5.0f; 	const int nBatchSize = 5;
  //
  //	// Don't do this yet..
  ...
  //	int count = g_pFileSystem->GetUnverifiedFileHashes( rgUnverifiedFiles,
  //ARRAYSIZE( rgUnverifiedFiles ) ); 	if ( count == 0 ) 		return;
  //
  //	// Send the messages to the server.
  //	for ( int i=0; i < count; i++ )
  //	{
  //		CLC_FileCRCCheck crcCheck;
  //		V_strncpy( crcCheck.m_szPathID, rgUnverifiedFiles[i].m_PathID,
  //sizeof( crcCheck.m_szPathID ) ); 		V_strncpy( crcCheck.m_szFilename,
  //rgUnverifiedFiles[i].m_Filename, sizeof( crcCheck.m_szFilename ) );
  //		crcCheck.m_nFileFraction = rgUnverifiedFiles[i].m_nFileFraction;
  //		crcCheck.m_MD5 = rgUnverifiedFiles[i].m_FileHash.m_md5contents;
  //		crcCheck.m_CRCIOs =
  //rgUnverifiedFiles[i].m_FileHash.m_crcIOSequence; 		crcCheck.m_eFileHashType =
  //rgUnverifiedFiles[i].m_FileHash.m_eFileHashType; 		crcCheck.m_cbFileLen =
  //rgUnverifiedFiles[i].m_FileHash.m_cbFileLen; 		crcCheck.m_nPackFileNumber =
  //rgUnverifiedFiles[i].m_FileHash.m_nPackFileNumber; 		crcCheck.m_PackFileID =
  //rgUnverifiedFiles[i].m_FileHash.m_PackFileID;
  //
...
void CClientState::FinishSignonState_New() {
  ...
  if (!CL_CheckCRCs(m_szLevelFileName)) {
    Host_Error("Unable to verify map %s",
               (m_szLevelFileName && m_szLevelFileName[0]) ? m_szLevelFileName
                                                           : "unknown");
    return;
  }

  // We're going to force-touch a lot of textures and resources below, we don't
  // want the streaming system to try and pull these in as if they were being
  // used for normal rendering.
  materials->SuspendTextureStreaming();

  // Only do this if our server is shut down and we're acting as a client.
  // Otherwise the server handles this when it starts the load.
  if (sv.m_State < ss_loading) {
    // Reset the last used count on all models before beginning the new load --
    // The nServerCount value on models should always resolve as different from
    // values from previous servers.
  ...
  // Our load process is causing some very bad allocation & performance impact
  // in drivers, due to the enormous amount of resources being used (and, in the
  // case of EnableHDR(), released and re-uploaded) in a single frame.  Ensuring
  // that we push a few frames through before and after the main model loading
  // spree results in dramatically better results.  Without these calls, for
  // instance, OS X on high texture quality cannot survive map load.
  //
  // This is pretty janky, but doesn't really have any cost (and even makes our
  // one-frozen-frame load screen slightly less likely to trigger OS "not
  // responding" warnings)
  extern void V_RenderSwapBuffers();
  V_RenderSwapBuffers();

  // Before we do anything with the whitelist, make sure we have the proper map
  // pack mounted this will load the .bsp by setting the world model the string
  // list at the hardcoded index 1.
  cl.SetModel(1);
  ...
  m_bCheckCRCsWithServer =
      false; // Don't check CRCs yet.. wait until we got a whitelist and cleaned
             // out our files based on it to send CRCs.

  // We check the new whitelist now so loads that happen between here and
  // FullyConnected are checked against it, but don't trigger a reload of dirty
  // files until after we flush unused resources from the last map in
  // CL_FullyConnected. This fixes reloading materials that are unused, and
  // trying to reload materials that are no longer pure, but would be unloaded
  // in fully connected anyway.
...
void CClientState::ConsistencyCheck(bool bChanged) {
  ...
  for (int i = 0; i < m_pDownloadableFileTable->GetNumStrings(); ++i) {
    ...
    if (userData && userData[0] == CONSISTENCY_EXACT &&
      ...
      // Hm, this isn't supported anymore.  (Use sv_pure instead.)  Under
      // ordinary circumstances, servers shouldn't be asking for it.
      Assert(false);
      continue;
    }
...
void CClientState::RunFrame() {
  ...
  if ((SYSCALL_SUCCESS == Plat_GetPagedPoolInfo(&ppi)) &&
    ...
    if (!s_bLowPagedPoolMemoryWarning ||
        (Plat_FloatTime() - s_flLastWarningTime >
         3.0f)) // print a warning no faster than once every 3 sec
    {
      s_bLowPagedPoolMemoryWarning = true;
      s_flLastWarningTime = Plat_FloatTime();
      Warning("OS Paged Pool Memory Low!\n");
      Warning("  Currently using %lu pages (%lu Kb) of total %lu pages (%lu Kb "
              "total)\n",
              ppi.numPagesUsed, ppi.numPagesUsed * Plat_GetMemPageSize(),
              (ppi.numPagesFree + ppi.numPagesUsed),
    ...
    Msg("Info: OS Paged Pool Memory restored - currently %lu pages free (%lu "
        "Kb) of total %lu pages (%lu Kb total).\n",
