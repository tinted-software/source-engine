...
void Host_Stats_f(void) {
  ...
  ConMsg("CPU    In_(KB/s)  Out_(KB/s)  Uptime  Map_changes  FPS      Players  "
         "Connects\n%s\n",
         stats);
}
...
void CServerRemoteAccess::LogCommand(ra_listener_id listener, const char *msg) {
  ...
  if (listener < (ra_listener_id)m_ListenerIDs.Count() &&
...
void CServerRemoteAccess::GetStatsString(char *buf, int bufSize) {
  ...
  _snprintf(buf, bufSize - 1,
            "%-6.2f %-10.2f %-11.2f %-7i %-12i %-8.2f %-8i %-8i",
            sv.GetCPUUsage() * 100, avgIn / 1024.0f, avgOut / 1024.0f,
            (int)(Sys_FloatTime()) / 60, sv.GetSpawnCount() - 1,
...
void CServerRemoteAccess::GetPlayerList(CUtlBuffer &value) {
  ...
  for (int i = 0; i < sv.GetClientCount(); i++) {
    ...
    if (client->IsFakeClient()) {
      ...
      value.Printf(
          "\"%s\" %s %s %d %d %d %d\n", client->GetClientName(),
          client->GetNetworkIDString(), client->GetNetChannel()->GetAddress(),
          (int)(client->GetNetChannel()->GetAvgLatency(FLOW_OUTGOING) *
                1000.0f),
...
void CServerRemoteAccess::SendMessageToAdminUI(ra_listener_id listenerID,
  ...
  if (listenerID != m_AdminUIID) {
    Warning("ServerRemoteAccess: Sending AdminUI message to non-AdminUI "
            "listener\n");
  }
