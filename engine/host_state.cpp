...
void HostState_Frame(float time) { g_HostState.FrameUpdate(time); }
...
// The external API queues up state changes to happen when the state machine is
// processed.
...
void CHostState::State_ChangeLevelMP() {
  ...
  IGameEvent *event =
      g_GameEventManager.CreateEvent("server_changelevel_failed");
  if (event) {
...
void CHostState::State_Run(float frameTime) {
  ...
  // Nice big timeout to play it safe while still ensuring that we don't get
