...
public:
  ...
  virtual void EmitSound(IRecipientFilter &filter, int iEntIndex, int iChannel,
                         ...
                         int iSpecialDSP, const Vector *pOrigin,
...
void CEngineSoundServer::EmitSoundInternal(
  ...
  if ((iSoundLevel < soundlevel_t(MIN_SNDLVL_VALUE)) ||
      (iSoundLevel > soundlevel_t(MAX_SNDLVL_VALUE))) {
  ...
  SV_StartSound(filter, pEdict, iChannel, pSample, flVolume, iSoundLevel,
                iFlags, iPitch, iSpecialDSP, pOrigin, soundtime, speakerentity,
                pUtlVecOrigins);
}
...
void CEngineSoundServer::EmitSentenceByIndex(
  ...
  if (iSentenceIndex >= 0) {
    ...
    EmitSoundInternal(filter, iEntIndex, iChannel, pName, flVolume, iSoundLevel,
                      iFlags, iPitch, iSpecialDSP, pOrigin, pDirection,
...
void CEngineSoundServer::EmitSound(
  ...
  EmitSound(filter, iEntIndex, iChannel, pSample, flVolume,
            ATTN_TO_SNDLVL(flAttenuation), iFlags, iPitch, iSpecialDSP, pOrigin,
  ...
  if (pSample && TestSoundChar(pSample, CHAR_SENTENCE)) {
    ...
    if (iSentenceIndex >= 0) {
      EmitSentenceByIndex(filter, iEntIndex, iChannel, iSentenceIndex, flVolume,
                          iSoundLevel, iFlags, iPitch, iSpecialDSP, pOrigin,
                          pDirection, pUtlVecOrigins, bUpdatePositions,
...
void CEngineSoundServer::StopSound(int iEntIndex, int iChannel,
  ...
  EmitSound(filter, iEntIndex, iChannel, pSample, 0, SNDLVL_NONE, SND_STOP,
            PITCH_NORM, 0, NULL, NULL, NULL, true);
}
...
void Host_RestartAmbientSounds()
{
        ...
        int nSounds = S_GetCurrentStaticSounds( soundInfo, NUM_INFOS,
CHAN_STATIC );
        ...
        for ( int i = 0; i < nSounds; i++)
        {
                        ...
                        soundInfo[i].entity != -1 )
                {
                        ...
                        SV_StartSound( filter, EDICT_NUM(soundInfo[i].entity),
                                                   ...
                                                   0,                    // @Q
(toml 05-09-02): Is this correct, or will I need to squirrel away the original
flags? soundInfo[i].pitch );
                }
        }
#endif
} */
