...
static inline void EncodeFloat(const SendProp *pProp, float fVal,
  ...
  if (fVal < pProp->m_fLowValue) {
    ...
    if (!(pProp->GetFlags() & SPROP_ROUNDUP)) {
      DataTable_Warning("(class %s): Out-of-range value (%f) in SendPropFloat "
                        "'%s', clamping.\n",
...
void Int_Encode(const unsigned char *pStruct, DVariant *pVar,
  ...
  if (pProp->GetFlags() & SPROP_VARINT) {
    ...
    if (pProp->m_nBits < 32) {
      if (pProp->GetFlags() & SPROP_UNSIGNED) {
        AssertMsg3(nValue == pVar->m_Int,
                   "Unsigned prop %s needs more bits? Expected %i == %i",
                   pProp->GetName(), nValue, pVar->m_Int);
      } else {
        AssertMsg3(nValue == pVar->m_Int,
                   "Signed prop %s needs more bits? Expected %i == %i",
                   pProp->GetName(), nValue, pVar->m_Int);
      }
    } else {
  ...
  if (flags & SPROP_VARINT) {
    ...
    if (bits != 32 && (flags & SPROP_UNSIGNED) == 0) {
      ...
      if (pInfo->m_Value.m_Int & highbit) {
        ...
        pInfo->m_Value.m_Int -=
            highbit; // ... then put it back with sign extension
      }
    }
  }
...
void VectorXY_Encode(const unsigned char *pStruct, DVariant *pVar,
                     const SendProp *pProp, bf_write *pOut, int objectID) {
  EncodeFloat(pProp, pVar->m_Vector[0], pOut, objectID);
  EncodeFloat(pProp, pVar->m_Vector[1], pOut, objectID);
}
...
bool VectorXY_IsZero(const unsigned char *pStruct, DVariant *pVar,
                     const SendProp *pProp) {
  return (pVar->m_Vector[0] == 0) && (pVar->m_Vector[1] == 0);
}
...
void Array_Encode(const unsigned char *pStruct, DVariant *pVar,
  ...
  AssertMsg(pArrayProp, "Array_Encode: missing m_pArrayProp for SendProp '%s'.",
            pProp->m_pVarName);
...
void Int64_Encode(const unsigned char *pStruct, DVariant *pVar,
                  const SendProp *pProp, bf_write *pOut, int objectID) {
  ...
  if (pProp->GetFlags() & SPROP_VARINT) {
    ...
    if (pProp->IsSigned()) {
      ...
      pOut->WriteUBitLong((unsigned int)highInt,
                          pProp->m_nBits - 32 - 1); // For the sign bit
    } else {
      pOut->WriteUBitLong((unsigned int)lowInt, 32);
...
bool Int64_IsZero(const unsigned char *pStruct, DVariant *pVar,
                  const SendProp *pProp) {
#ifdef SUPPORTS_INT64
