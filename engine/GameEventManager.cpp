...
static ConVar
    net_showevents("net_showevents", "0", FCVAR_CHEAT,
...
bool CGameEventManager::FireEventIntern(IGameEvent *event, bool bServerOnly,
  ...
  tmZoneFiltered(TELEMETRY_LEVEL0, 50, TMZF_NONE, "%s (name: %s listeners: %d)",
                 __FUNCTION__,
                 tmDynamicString(TELEMETRY_LEVEL0, event->GetName()),
                 descriptor->listeners.Count());
  ...
  for (int i = 0; i < descriptor->listeners.Count(); i++) {
    ...
    if (listener->m_nListenerType == CLIENTSIDE_OLD ||
        listener->m_nListenerType == SERVERSIDE_OLD) {
      tmZone(TELEMETRY_LEVEL1, TMZF_NONE,
             "FireGameEvent (i: %d, listenertype: %d (old))", i,
      ...
      tmZone(TELEMETRY_LEVEL1, TMZF_NONE,
             "FireGameEvent (i: %d, listenertype: %d (new))", i,
             listener->m_nListenerType);
...
bool CGameEventManager::RegisterEvent(KeyValues *event) {
  ...
  if (m_GameEvents.Count() == MAX_EVENT_NUMBER) {
    DevMsg("CGameEventManager: couldn't register event '%s', limit reached "
           "(%i).\n",
  ...
  if (!descriptor) {
    ...
    AssertMsg2(V_strlen(event->GetName()) <= MAX_EVENT_NAME_LENGTH,
               "Event named '%s' exceeds maximum name length %d",
               event->GetName(), MAX_EVENT_NAME_LENGTH);
...
CGameEventManager::GetEventDescriptor(int eventid) // returns event name or NULL
{
  if (eventid < 0)
    return NULL;
...
bool CGameEventManager::AddListenerAll(void *listener, int nListenerType) {
  ...
  DevMsg("Warning! Game event listener registerd for all events. Use newer "
