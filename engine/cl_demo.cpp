...
static ConVar
    demo_recordcommands("demo_recordcommands", "1", FCVAR_CHEAT,
...
static ConVar
    demo_interpolateview("demo_interpolateview", "1", 0,
...
static ConVar timedemo_runcount("timedemo_runcount", "0", 0,
...
static ConVar
    demo_interplimit("demo_interplimit", "4000", 0,
                     "How much origin velocity before it's considered to have "
                     "'teleported' causing interpolation to reset.");
...
static ConVar
    demo_fastforwardstartspeed("demo_fastforwardstartspeed", "2", 0,
                               "Go this fast when starting to hold FF button.");
static ConVar
    demo_fastforwardfinalspeed("demo_fastforwardfinalspeed", "20", 0,
                               "Go this fast when starting to hold FF button.");
...
static bool IsControlCommand(unsigned char cmd) {
  return ((cmd == dem_signon) || (cmd == dem_stop) || (cmd == dem_synctick) ||
          (cmd == dem_datatables) || (cmd == dem_stringtables));
}

// Puts a flashing overlay on the screen during demo recording/playback
static ConVar
    cl_showdemooverlay("cl_showdemooverlay", "0", 0,
                       "How often to flash demo recording/playback overlay (0 "
                       "- disable overlay, -1 - show always)");
...
DemoOverlay::DemoOverlay()
    : m_fLastTickTime(0.f), m_fLastTickOverlay(0.f), m_bTick(false),
...
void CDemoRecorder::RecordStringTables() {
  ...
  while (dataLen <= DEMO_FILE_MAX_STRINGTABLE_SIZE) {
    ...
    buf.SetAssertOnOverflow(
        false); // Doesn't turn off all the spew / asserts, but turns off one
  ...
  if (dataLen > DEMO_FILE_MAX_STRINGTABLE_SIZE) {
    Warning("Failed to RecordStringTables. Trying to record string table "
            "that's bigger than max string table size\n");
  }
...
void CDemoRecorder::RecordUserInput(int cmdnumber) {
  ...
  m_DemoFile.WriteUserCmd(cmdnumber, buffer, msg.GetNumBytesWritten(),
                          GetRecordingTick());
}
...
void CDemoPlayer::StopPlayback(void) {
  ...
  if (m_bTimeDemo) {
    ...
    if (demotime > 0.0f) {
      DevMsg("Demo playback finished ( %.1f seconds, %i render frames, %.2f "
             "fps).\n",
...
bool CDemoPlayer::IsLoading(void) { return m_bLoading; }
...
//-----------------------------------------------------------------------------
// Purpose: Read in next demo message and send to local client over network
// channel, if it's time. Output : netpacket_t* -- NULL if there is no packet
...
netpacket_t *CDemoPlayer::ReadPacket(void) {
  ...
  if (IsSkipping()) {
    // Every nMaxConsecutiveSkipPackets frames return NULL so that we don't
    // build up an endless supply of unprocessed packets. This avoids causing
    // overflows and excessive "highwater marks" in various Dota subsystems.
  ...
  while (!bStopReading) {
    ...
    case dem_datatables: {
      ...
      void *data =
          malloc(256 * 1024); // X360TBD: How much memory is really needed here?
    ...
    case dem_stringtables: {
      ...
      if (dataLen > DEMO_FILE_MAX_STRINGTABLE_SIZE) {
        Warning("ReadPacket failed to read string tables. Trying to read "
                "string tables that's bigger than max string table size\n");
      }
...
bool CDemoPlayer::StartPlayback(const char *filename, bool bAsTimeDemo) {
  ...
  cl.m_NetChannel = NET_CreateNetChannel(NS_CLIENT, NULL, "DEMO", &cl, false,
                                         dh->networkprotocol);
...
void CDemoPlayer::WriteTimeDemoResults(void) {
  ...
  //	g_pFileSystem->FPrintf( fileHandle, "%s,", CommandLine()->CheckParm(
  //"-nosound" ) ? "off" : "on" );
  g_pFileSystem->FPrintf(fileHandle, "%s,",
                         mat_reducefillrate.GetBool() ? "on" : "off");
  g_pFileSystem->FPrintf(fileHandle, "%s,",
                         r_waterforcereflectentities.GetBool() ? "on" : "off");
  g_pFileSystem->FPrintf(fileHandle, "%s,",
                         mat_motion_blur_enabled.GetBool() ? "on" : "off");
  g_pFileSystem->FPrintf(fileHandle, "%s,",
                         r_flashlightdepthtexture.GetBool() ? "on" : "off");
  g_pFileSystem->FPrintf(fileHandle, "%s,",
                         mat_reduceparticles.GetBool() ? "on" : "off");
  g_pFileSystem->FPrintf(fileHandle, "%s,",
                         r_dopixelvisibility.GetBool() ? "on" : "off");
  g_pFileSystem->FPrintf(fileHandle, "%s,",
                         CommandLine()->CheckParm("-nulldevice") ? "yes"
                                                                 : "no");

  int itimedemo_comment = CommandLine()->FindParm("-timedemo_comment");
  const char *timedemo_comment =
      itimedemo_comment ? CommandLine()->GetParm(itimedemo_comment + 1) : "";
...
static bool ComputeNextIncrementalDemoFilename(char *name, int namesize) {
  ...
  ConMsg("Unable to find a valid incremental demo filename for %s, try "
         "clearing the directory of %snnn.dem\n",
         name, basename);
  return false;
}
...
void CL_TimeDemo_f(const CCommand &args) {
  ...
  if (args.ArgC() < 2 || args.ArgC() > 4) {
    ConMsg("timedemo <demoname> <optional stats.txt> : gets demo speeds, "
           "starting from optional frame\n");
    return;
  }
...
void CL_BenchFrame_f(const CCommand &args) {
  ...
  if (args.ArgC() != 4) {
    ConMsg("benchframe <demoname> <frame> <tgafilename>: takes a snapshot of a "
...
void CDemoPlayer::OnStopCommand() { cl.Disconnect("Demo stopped", true); }
