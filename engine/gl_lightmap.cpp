...
//-----------------------------------------------------------------------------
static void
AddSingleDynamicLightToBumpLighting(dlight_t &dl, SurfaceHandle_t surfID,
                                    const Vector &lightOrigin, float perpDistSq,
                                    float lightRadiusSq, Vector *pBumpBasis,
...
static const int32 ALIGN16 g_SIMD_HalfWordMask[4] = {0x0000000, 0x0000FFFF,
...
static void AccumulateLightstyles_EightAtAtime(
    ...
    Vector4D *RESTRICT
        bLights // pointer to the blocklights row we'll be writing into --
                // should be cache aligned, but only hurts perf if it's not
) {
  ...
  for (int i = 0; i < lightmapSize; i += 8) {
    ...
    fltx4 signedUnpackHi0 =
        __vupkhsb(bytePackedLightMap0); // signed unpack of words 0 and 1, like
                                        // the unsigned unpack but replaces 0x00
                                        // w/ sign extension
...
static void AccumulateBumpedLightstyles(ColorRGBExp32 *RESTRICT pLightmap,
  ...
  COMPILE_TIME_ASSERT(
      ...
      4); // This function is carefully scheduled around four-byte colors
...
static void ComputeLightmapFromLightstyle(msurfacelighting_t *pLighting,
  ...
  for (int maps = minmap; maps < maxmap && pLighting->m_nStyles[maps] != 255;
    ...
    if (fscalar > 0.0f) {
#ifdef _X360
      fltx4 scalar = ReplicateX4(
          fscalar); // we use SIMD versions of these functions on 360
#else
...
static void SortSurfacesByLightmapID(SurfaceHandle_t *pToSort,
                                     int iSurfaceCount) {
  SurfaceHandle_t *pSortTemp =
      (SurfaceHandle_t *)stackalloc(sizeof(SurfaceHandle_t) * iSurfaceCount);
  ...
  for (int radix = 0; radix != 4; ++radix) {
    ...
    for (int i = 0; i != iSurfaceCount; ++i) {
      uint8 val = (materialSortInfoArray[MSurf_MaterialSortID(pSortTemp[i])]
                       .lightmapPageID >>
                   iBitOffset) &
                  0xFF;
      ++iCounts[val];
...
void R_RedownloadAllLightmaps() {
  ...
  if (!host_state.worldbrush->unloadedlightmaps) {
    ...
    SurfaceHandle_t *pSortedSurfaces =
        (SurfaceHandle_t *)stackalloc(sizeof(SurfaceHandle_t) * iSurfaceCount);
    ...
    SortSurfacesByLightmapID(
        pSortedSurfaces,
        iSurfaceCount); // sorts in place, so now the array really is sorted
