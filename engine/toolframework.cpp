...
void CToolFrameworkInternal::LoadToolsFromLibrary(const char *dllname) {
  ...
  if (!factory) {
    ...
    Warning("CToolFrameworkInternal::LoadToolsFromLibrary:  Dll '%s' has no "
            "factory\n",
            dllname);
    return;
  }
  ...
  if (!dictionary) {
    Sys_UnloadModule(module);
    Warning("CToolFrameworkInternal::LoadToolsFromLibrary:  Dll '%s' doesn't "
            "support '%s'\n",
  ...
  if (!dictionary->Connect(g_AppSystemFactory)) {
    Sys_UnloadModule(module);
    Warning("CToolFrameworkInternal::LoadToolsFromLibrary:  Dll '%s' "
...
void CToolFrameworkInternal::LoadTools() {
  ...
  // We don't ship enginetools.txt to Steam public, so we'll need to load
  // sdkenginetools.txt if enginetools.txt isn't present
  bool bLoadSDKFile =
      !g_pFileSystem->FileExists("enginetools.txt", "EXECUTABLE_PATH");

  if (kv &&
      kv->LoadFromFile(g_pFileSystem,
                       bLoadSDKFile ? "sdkenginetools.txt" : "enginetools.txt",
                       "EXECUTABLE_PATH")) {
...
//-----------------------------------------------------------------------------
const char *
CToolFrameworkInternal::GetEntityData(const char *pActualEntityData) {
  if (m_nActiveToolIndex >= 0) {
    IToolSystem *tool = m_ToolSystems[m_nActiveToolIndex];
    Assert(tool);
