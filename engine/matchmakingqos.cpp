...
bool CQosDetector::WaitForResults() {
  ...
  //
  const int numSecQosTimeout =
      30; // Consider QOS query timed out if 30 seconds elapsed
  float flTimeStart = Plat_FloatTime(),
        flTimeEndTimeout = flTimeStart + numSecQosTimeout;
  for (;;) {
...
void CQosDetector::ProcessResults() {
  ...
  Results.flLoss = m_pQos->axnqosinfo->cProbesXmit
                       ? (float(m_pQos->axnqosinfo->cProbesXmit -
                                m_pQos->axnqosinfo->cProbesRecv) *
                          100.f / m_pQos->axnqosinfo->cProbesXmit)
                       : 0.f;
  ...
  Msg("[QOS] Fresh QOS results available:\n"
      ...
      Results.nPingMsMin, Results.nPingMsMed, Results.flBwUpKbs,
      Results.flBwDnKbs, Results.flLoss);
}
...
void CQosThread::Run() {
  ...
  // Every request of QOS data is instantaneous and returns the last successful
  // QOS query result.
  //
