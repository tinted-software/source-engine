...
CDemoFile::CDemoFile()
    : m_pBuffer(NULL), m_bAllowHeaderWrite(true), m_bIsStreamBuffer(false) {}
...
void CDemoFile::WriteCmdHeader(unsigned char cmd, int tick) {
  ...
  static const char *cmdname[] = {
      "dem_unknown",    "dem_signon",     "dem_packet",
      "dem_synctick",   "dem_consolecmd", "dem_usercmd",
      "dem_datatables", "dem_stop",       "dem_stringtables"};
...
//
// Purpose: Rewind from the current spot by the time stamp, byte code and frame
// counter offsets
//-----------------------------------------------------------------------------
...
int CDemoFile::ReadRawData(char *buffer, int length) {
  ...
  if (length < size) {
    ...
    m_pBuffer->SeekGet(CUtlBuffer::SEEK_CURRENT,
                       -(int)sizeof(int)); // rewind our get pointer
    return -1;
  }
...
bool CDemoFile::Open(const char *name, bool bReadOnly, bool bMemoryBuffer,
                     int nBufferSize /*=0*/, bool bAllowHeaderWrite /*=true*/) {
  if (m_pBuffer && m_pBuffer->IsValid()) {
  ...
  if (bMemoryBuffer) {
    ...
    m_pBuffer = new CUtlStreamBuffer(
        name, NULL, bReadOnly ? CUtlBuffer::READ_ONLY : 0, false);
