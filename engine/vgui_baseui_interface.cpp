...
public:
  ...
  virtual void UpdatePlayerInfo(uint64 nPlayerId, const char *pName, int nTeam,
...
void CEngineVGui::Init() {
  ...
  if (lang[0])
    vgui::system()->SetRegistryString(
        "HKEY_CURRENT_USER\\Software\\Valve\\Source\\Language", lang);
  ...
  if (IsX360() || IsSteamDeck()) {
    ...
    if (CL_ShouldLoadBackgroundLevel(ccommand)) {
      ...
      // This is a hint to avoid the menu pop due to the impending background
      // map that the game ui is not aware of until 1 frame later.
  ...
  staticPanel->SetBounds(0, 0, videomode->GetModeUIWidth(),
                         videomode->GetModeUIHeight());
  ...
  staticClientDLLPanel->SetBounds(0, 0, videomode->GetModeUIWidth(),
                                  videomode->GetModeUIHeight());
  ...
  staticClientDLLToolsPanel->SetBounds(0, 0, videomode->GetModeUIWidth(),
                                       videomode->GetModeUIHeight());
  ...
  staticEngineToolsPanel->SetBounds(0, 0, videomode->GetModeUIWidth(),
                                    videomode->GetModeUIHeight());
  ...
  staticGameUIPanel->SetBounds(0, 0, videomode->GetModeUIWidth(),
                               videomode->GetModeUIHeight());
  ...
  staticGameDLLPanel->SetBounds(0, 0, videomode->GetModeUIWidth(),
                                videomode->GetModeUIHeight());
  ...
  staticFocusOverlayPanel->SetBounds(0, 0, videomode->GetModeUIWidth(),
                                     videomode->GetModeUIHeight());
...
void CEngineVGui::Shutdown() {
  ...
  if (IsPC() && CL_IsPortalDemo()) // if they are playing the demo then open the
                                   // storefront on shutdown
  {
...
bool CEngineVGui::Key_Event(const InputEvent_t &event) {
  ...
  if (bDown &&
      (code == KEY_ESCAPE || code == KEY_XBUTTON_START ||
    ...
    if (IsX360() && !IsGameUIVisible()) {
      // 360 UI does not toggle, engine does "show", but UI needs to handle
      // "hide"
      Cbuf_AddText("gameui_activate");
      return true;
    }
...
void CEngineVGui::Paint(PaintMode_t mode) {
  ...
  // Force engine's root panel (staticPanel) to be full screen size
  {
    ...
    panel->SetBounds(
        0, 0, w,
        h); // ignore x and y here because the viewport takes care of that
  }
...
//-----------------------------------------------------------------------------
void CEngineVGui::NotifyOfServerConnect(const char *pchGame, int IP,
                                        int connectionPort, int queryPort) {
  if (!staticGameUIFuncs)
    return;
...
void CEngineVGui::UpdatePlayerInfo(uint64 nPlayerId, const char *pName,
