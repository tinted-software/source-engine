...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CReplayDemoPlayer, IReplayDemoPlayer,
                                  INTERFACEVERSION_REPLAYDEMOPLAYER,
                                  s_ReplayDemoPlayer);

//----------------------------------------------------------------------------------------

CReplayDemoPlayer::CReplayDemoPlayer()
    : m_pMovie(NULL), m_nCurReplayIndex(0), m_flStartRenderTime(0.0f),
      m_bInStartPlayback(false), m_bStopCommandEncountered(false),
...
void CReplayDemoPlayer::AddReplayToList(ReplayHandle_t hReplay,
                                        int iPerformance) {
  ...
  if (iPerformance >= 0) {
    ...
    const CReplayPerformance *pPerformance =
        pReplay->GetPerformance(iPerformance);
...
const CReplayDemoPlayer::PlaybackInfo_t *
CReplayDemoPlayer::GetCurrentPlaybackInfo() const {
...
bool CReplayDemoPlayer::StartPlayback(const char *pFilename, bool bAsTimeDemo) {
  ...
  // Set this flag so we can detect whether the replay made it all the way to
  // full signon state, otherwise we'll display a message StopPlayback() is
  // called.
  m_bFullSignonStateReached = false;
  ...
  if (g_pReplayMovieManager->IsRendering()) {
    ...
    V_strcat(szIdealFilename, va("_%i", pReplay->m_nSpawnTick),
    ...
    V_snprintf(szRenderPath, sizeof(szRenderPath), "%s%c%s%c%s%c%s",
               com_gamedir, CORRECT_PATH_SEPARATOR, SUBDIR_REPLAY,
               CORRECT_PATH_SEPARATOR, SUBDIR_CLIENT, CORRECT_PATH_SEPARATOR,
               SUBDIR_RENDERED);

    char szActualFilename[MAX_OSPATH];
    Replay_GetFirstAvailableFilename(szActualFilename, sizeof(szActualFilename),
                                     szIdealFilename, pExtension, szRenderPath,
                                     0);
    ...
    if (pPlaybackInfo->m_iPerformance < 0) {
      g_pReplayMovieManager->GetCachedMovieTitleAndClear(
          wszMovieTitle, MAX_REPLAY_TITLE_LENGTH);
    } else {
      const CReplayPerformance *pPerformance =
          pReplay->GetPerformance(pPlaybackInfo->m_iPerformance);
      AssertMsg(pPerformance, "Performance should always be valid!");
      if (pPerformance) {
        V_wcsncpy(wszMovieTitle, pPerformance->m_wszTitle,
                  sizeof(wszMovieTitle));
      }
    }
...
void CReplayDemoPlayer::PlayNextReplay() {
  ...
  // Play the replay using the 'playreplay' command - pass in the performance as
  // well, -1 meaning play without a performance.
...
float CReplayDemoPlayer::CalcMovieLength() const {
  ...
  const int nStartTick = pPlaybackInfo->m_nStartTick >= 0
                             ? pPlaybackInfo->m_nStartTick
                             : pReplay->m_nSpawnTick;
  const int nEndTick =
      pPlaybackInfo->m_nEndTick >= 0
          ? pPlaybackInfo->m_nEndTick
...
void CReplayDemoPlayer::StopPlayback() {
  ...
  } else if (!bRenderCancelled) // Without this check, batch rendering will
                                // continue to try and render after cancel
  {
    CReplay *pReplay = GetCurrentReplay();
    if (!pReplay)
      return;

    // Get the 'saved' performance from the performance controller, since the
    // performance we initiated playback with may not be the one we want to
    // select in the replay browser.  The user may have save as a new
    // performance, in which case we'll want to highlight that one.
    CReplayPerformance *pSavedPerformance =
        g_pReplayPerformanceController->GetSavedPerformance();

    // Get the index - FindPerformance() will set the output index to -1 if it
    // can't find the performance
    ...
    g_pClientReplay->OnPlaybackComplete(pReplay->GetHandle(),
                                        iHighlightPerformance);
  ...
  if (!bDoneWithBatch && !bRenderCancelled) {
    ...
    if (!m_bFullSignonStateReached) {
      DisplayFailedToPlayMsg(pPlaybackInfo ? pPlaybackInfo->m_iPerformance
                                           : -1);
    }
  }
}
...
void CReplayDemoPlayer::DisplayFailedToPlayMsg(int iPerformance) {
  g_pClientReplay->DisplayReplayMessage(
      iPerformance < 0 ? "#Replay_Err_User_FailedToPlayReplay"
