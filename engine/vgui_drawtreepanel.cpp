...
static ConVar vgui_drawtree_popupsonly(
    ...
    ChangeCallback_RefreshDrawTree);
static ConVar
    vgui_drawtree_freeze("vgui_drawtree_freeze", "0", 0,
...
static ConVar vgui_drawtree_panelalpha(
    ...
    ChangeCallback_RefreshDrawTree);
static ConVar
    vgui_drawtree_render_order("vgui_drawtree_render_order", "0", 0,
                               "List the vgui_drawtree panels in render order.",
                               ChangeCallback_RefreshDrawTree);
static ConVar vgui_drawtree_bounds("vgui_drawtree_bounds", "0", 0,
                                   "Show panel bounds.",
                                   ChangeCallback_RefreshDrawTree);
static ConVar vgui_drawtree_draw_selected("vgui_drawtree_draw_selected", "0", 0,
...
public:
  ...
  CDrawTreeFrame(vgui::Panel *parent, const char *pName)
    ...
    m_pShowBounds = vgui::SETUP_PANEL(
        new CConVarCheckButton(this, "show panel bounds", "Show Panel Bounds"));
    m_pShowBounds->SetVisible(true);
    m_pShowBounds->SetConVar(&vgui_drawtree_bounds);

    m_pHighlightSelected = vgui::SETUP_PANEL(new CConVarCheckButton(
        this, "highlight selected", "Highlight Selected"));
    ...
    m_pPerformLayoutBtn = vgui::SETUP_PANEL(
        new vgui::Button(this, "performlayout", "Perform Layout (Highlighted)",
                         this, "performlayout"));
    m_pPerformLayoutBtn->SetVisible(true);

    m_pReloadSchemeBtn = vgui::SETUP_PANEL(
        new vgui::Button(this, "reloadscheme", "Reload Scheme (Highlighted)",
                         this, "reloadscheme"));
  ...
  void RecalculateSelectedHighlight(void) {
    ...
    if (!vgui_drawtree_draw_selected.GetBool() ||
        m_pTree->GetSelectedItemCount() != 1) {
      // clear the selection
      ...
      if (data) {
        g_DrawTreeSelectedPanel =
            (data) ? (vgui::VPANEL)data->GetPtr("PanelPtr", 0) : 0;
      } else {
        g_DrawTreeSelectedPanel = 0;
      }
    }
  ...
  virtual void OnClose() {
    ...
    // fixme - g_DrawTreeSelectedPanel has a potential crash if you hilight a
    // panel, and then spam hud_reloadscheme you will sometimes end up on a
    // different panel or on garbage.
  }
