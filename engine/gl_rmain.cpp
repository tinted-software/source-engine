...
public:
  ...
  virtual bool ClipTransformWithProjection(const VMatrix &worldToScreen,
                                           const Vector &point, Vector *pClip);
...
float ComputeViewMatrices(VMatrix *pWorldToView, VMatrix *pViewToProjection,
                                     ...
                                     // jitter and tiled output of posters
  {
    ...
    *pViewToProjection = viewSetup.m_ViewToProjection;
    // ...but then override the Z range (needed for correct skybox rendering,
    // etc).
    MatrixBuildPerspectiveZRange(*pViewToProjection, viewSetup.zNear,
                                 viewSetup.zFar);
  } else {
...
void CRender::Push3DView(const CViewSetup &view, int nFlags,
                         ...
                         ITexture *pDepthTexture) {
  Assert(!IsX360() ||
         (pDepthTexture == NULL)); // Don't render to a depth texture on the
...
void CRender::UpdateBrushModelLightmap(model_t *model,
  ...
  if (model->flags & MODELFLAG_HAS_DLIGHT) {
    ...
    if (!bLight) {
      model->flags &= ~MODELFLAG_HAS_DLIGHT; // don't need to check again unless
                                             // a dlight hits us
    }
  }
}
  ...
  if (--m_iLightmapUpdateDepth == 0) {
    ...
    if (g_LightmapUpdateList.Count() && r_dynamiclighting.GetBool() &&
      ...
      qsort(g_LightmapUpdateList.Base(), g_LightmapUpdateList.Count(),
            sizeof(g_LightmapUpdateList.Element(0)), LightmapPageCompareFunc);
      int i;
...
//-----------------------------------------------------------------------------
bool CRender::ClipTransformWithProjection(const VMatrix &worldToScreen,
                                          const Vector &point, Vector *pClip) {
...
//-----------------------------------------------------------------------------
// Compute the scene coordinates of a point in 3D using the current engine's
// projection
//-----------------------------------------------------------------------------
