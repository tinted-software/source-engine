...
public:
  ...
  inline unsigned char *CallPropProxy(CSendNode *pNode, int iProp,
    ...
    if (pNode->GetDataTableProxyIndex() == DATATABLE_PROXY_INDEX_NOPROXY) {
      ...
      bool bCur =
          m_pNewStateProxies[pNode->GetDataTableProxyIndex()].m_Bits.Get(
              m_iClient) != 0;
          ...
          if (bPrev) {
            ...
            for (int i = 0; i < pNode->m_nRecursiveProps; i++) {
              if (m_nNewProxyProps < ARRAYSIZE(m_NewProxyProps)) {
                ...
                Error("CPropCullStack::CallPropProxy - overflowed "
                      "m_nNewProxyProps");
              }
...
static FORCEINLINE void SendTable_EncodeProp(CEncodeInfo *pInfo,
                                             unsigned long iProp) {
  ...
  pProp->GetProxyFn()(pProp, pStructBase, pStructBase + pProp->GetOffset(),
                      &var,
                      0, // iElement
                      pInfo->GetObjectID());
  ...
  g_PropTypeFns[pProp->m_Type].Encode(pStructBase, &var, pProp,
...
bool SendTable_Encode(const SendTable *pTable, const void *pStruct,
  ...
  if (pRecipients) {
    ErrorIfNot(pRecipients->NumAllocated() >=
                   pPrecalc->GetNumDataTableProxies(),
...
int SendTable_CalcDelta(const SendTable *pTable,
  ...
  bf_read toBits("SendTable_CalcDelta/toBits", pToState, BitByte(nToBits),
                 nToBits);
  CDeltaBitsReader toBitsReader(&toBits);
  unsigned int iToProp = toBitsReader.ReadNextPropIndex();

  if (pFromState) {
    bf_read fromBits("SendTable_CalcDelta/fromBits", pFromState,
                     BitByte(nFromBits), nFromBits);
    CDeltaBitsReader fromBitsReader(&fromBits);
    unsigned int iFromProp = fromBitsReader.ReadNextPropIndex();

    for (; iToProp < MAX_DATATABLE_PROPS;
         iToProp = toBitsReader.ReadNextPropIndex()) {
      ...
      if (iFromProp == iToProp) {
        ...
        if (fromBitsReader.ComparePropData(&toBitsReader,
                                           pPrecalc->GetProp(iToProp))) {
          *pDeltaProps++ = iToProp;
        ...
        // Only the 'to' state has this property, so just skip its data and
        // register a change.
  ...
  for (int i = 0; i < pTable->m_nProps; i++) {
    ...
    if (pProp->GetArrayProp()) {
      if (pProp->GetArrayProp()->GetType() == DPT_DataTable) {
        Error("Invalid property: %s/%s (array of datatables) [on prop %d of %d "
              "(%s)].",
