...
struct model_t {
  ...
  IMaterial **ppMaterials; // null-terminated runtime material cache;
                           // ((intptr_t*)(ppMaterials))[-1] == nMaterials
...
inline class IDispInfo *
MLeaf_Disaplcement(mleaf_t *pLeaf, int index,
...
struct msurface1_t {
  ...
  short
      textureExtents[2]; // ?? s/t texture size, 1..512 for all non-sky surfaces
...
struct msurface2_t {
  ...
  // unsigned char			vertCount;		// number of
  // verts for this surface unsigned char			sortGroup;
  // // only uses 2 bits, subdivide?
  ...
  unsigned short
      m_bDynamicShadowsEnabled : 1; // Can this surface receive dynamic shadows?
...
inline const SurfaceHandle_t
SurfaceHandleFromIndex(int surfaceIndex, const worldbrushdata_t *pData) {
  return &pData->surfaces2[surfaceIndex];
}

inline SurfaceHandle_t
SurfaceHandleFromIndex(int surfaceIndex,
                       worldbrushdata_t *pData = host_state.worldbrush) {
  return &pData->surfaces2[surfaceIndex];
}
...
inline short *
MSurf_TextureExtents(SurfaceHandle_t surfID,
                     worldbrushdata_t *pData = host_state.worldbrush) {
  int surfaceIndex = MSurf_Index(surfID, pData);
  //	ASSERT_SURF_VALID( surfID );
  Assert(pData);
...
inline short *
MSurf_LightmapMins(SurfaceHandle_t surfID,
                   worldbrushdata_t *pData = host_state.worldbrush) {
  int surfaceIndex = MSurf_Index(surfID, pData);
  //	ASSERT_SURF_VALID( surfID );
  Assert(pData);
...
inline short *
MSurf_LightmapExtents(SurfaceHandle_t surfID,
                      worldbrushdata_t *pData = host_state.worldbrush) {
  int surfaceIndex = MSurf_Index(surfID, pData);
  //	ASSERT_SURF_VALID( surfID );
  Assert(pData);
...
inline mtexinfo_t *
MSurf_TexInfo(SurfaceHandle_t surfID,
              worldbrushdata_t *pData = host_state.worldbrush) {
...
MSurf_AvgLightColor(SurfaceHandle_t surfID, int nIndex,
                    worldbrushdata_t *pData = host_state.worldbrush) {
  int surfaceIndex = MSurf_Index(surfID, pData);
  //	ASSERT_SURF_VALID( surfID );
  Assert(pData);
...
MSurf_NumPrims(SurfaceHandle_t surfID,
               worldbrushdata_t *pData = host_state.worldbrush) {
...
inline unsigned short
MSurf_FirstPrimID(SurfaceHandle_t surfID,
                  worldbrushdata_t *pData = host_state.worldbrush) {
...
inline ColorRGBExp32 *
MSurf_Samples(SurfaceHandle_t surfID,
              worldbrushdata_t *pData = host_state.worldbrush) {
  int surfaceIndex = MSurf_Index(surfID, pData);
  //	ASSERT_SURF_VALID( surfID );
  Assert(pData);
...
inline IDispInfo *
MSurf_DispInfo(SurfaceHandle_t surfID,
               worldbrushdata_t *pData = host_state.worldbrush) {
...
// worldbrushdata_t *pData = host_state.worldbrush )
inline unsigned int &
MSurf_FirstVertNormal(SurfaceHandle_t surfID,
                      worldbrushdata_t *pData = host_state.worldbrush) {
  int surfaceIndex = MSurf_Index(surfID, pData);
  //	ASSERT_SURF_VALID( surfID );
  Assert(pData);
...
inline short &
MSurf_MaterialSortID(SurfaceHandle_t surfID,
                     worldbrushdata_t *pData = host_state.worldbrush) {
  return surfID->materialSortID;
}

inline short *
MSurf_OffsetIntoLightmapPage(SurfaceHandle_t surfID,
                             worldbrushdata_t *pData = host_state.worldbrush) {
  int surfaceIndex = MSurf_Index(surfID, pData);
  //	ASSERT_SURF_VALID( surfID );
  Assert(pData);
...
inline VPlane
MSurf_GetForwardFacingPlane(SurfaceHandle_t surfID,
                            worldbrushdata_t *pData = host_state.worldbrush) {
  //	ASSERT_SURF_VALID( surfID );
  Assert(pData);
...
inline OverlayFragmentHandle_t &
MSurf_OverlayFragmentList(SurfaceHandle_t surfID,
                          worldbrushdata_t *pData = host_state.worldbrush) {
  return surfID->m_nFirstOverlayFragment;
}

inline msurfacelighting_t *
SurfaceLighting(SurfaceHandle_t surfID,
                worldbrushdata_t *pData = host_state.worldbrush) {
  int surfaceIndex = MSurf_Index(surfID, pData);
  Assert(pData);
