...
public:
  ...
  int m_DLightMarkFrame; // last frame in which a dlight was marked on this prop
                         // (helps detect lights that are marked but have moved
                         // away from this prop)
  CUtlVector<short>
      m_LightStyleWorldLights; // This is a list of lights that affect this
                               // static prop cache entry.
...
static CClassMemoryPool<PropLightcache_t>
    s_PropCache(256, CClassMemoryPool<lightcache_t>::GROW_SLOW);
  ...
  if (!r_oldlightselection.GetBool()) {
    ...
    case emit_spotlight: // directional & positional
    {
      ...
      if (!IsSphereIntersectingCone(mid, sphereRadius, pLight->origin,
                                    pLight->normal, sinAngle, pLight->stopdot2))
        return 0;
    }
...
//-----------------------------------------------------------------------------
static const byte *AddWorldLightToLightingState(
...
//-----------------------------------------------------------------------------
static const byte *ComputeLightStyles(lightcache_t *pCache,
...
//-----------------------------------------------------------------------------
static const byte *ComputeDynamicLighting(lightcache_t *pCache,
                                          LightingState_t &lightingState,
                                          const Vector &origin, int leaf,
                                          const byte *pVis = 0) {
...
//-----------------------------------------------------------------------------
static const byte *
ComputeStaticLightingForCacheEntry(CBaseLightCache *pcache,
                                   const Vector &origin, int leaf,
...
inline const byte *
AddLightingState(LightingState_t &dst, const LightingState_t &src,
                 LightingStateInfo_t &info, const Vector &bucketOrigin,
                 const byte *pVis, bool bDynamic, bool bIgnoreVis) {
  int i;
