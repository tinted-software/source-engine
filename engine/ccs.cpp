...
// Not to be confused with the game's server - this is a completely different
// thing that allows multiple game clients to connect to a single client, so con
// commands can be sent simultaneously to multiple running game clients, screen
// captures can be sent in real-time to a single client for comparison purposes,
// and all convar's can be diff'd between all connected clients and the server.
// The server portion of this code needs socketlib, which hasn't been ported to
// Linux yet, and shouldn't be shipped because it could be a security risk (it's
// only intended for primarily graphics debugging and detailed comparisons of GL
// vs. D3D9).
...
class frame_buf_window {
  friend LRESULT CALLBACK static_window_proc(HWND hWnd, UINT message,
                                             WPARAM wParam, LPARAM lParam);

  static LRESULT CALLBACK static_window_proc(HWND hWnd, UINT message,
                                             WPARAM wParam, LPARAM lParam) {
    frame_buf_window *pApp =
        reinterpret_cast<frame_buf_window *>(GetWindowLong(hWnd, 0));
    if (!pApp)
...
public:
  frame_buf_window(const char *pTitle = "frame_buf_window", int width = 640,
                   int height = 480, int scaleX = 1, int scaleY = 1);

  virtual ~frame_buf_window();

  // Return true if you handle the window message.
  typedef bool (*user_window_proc_ptr_t)(LRESULT &hres, HWND hWnd, UINT message,
                                         WPARAM wParam, LPARAM lParam,
                                         void *pData);
  void set_window_proc_callback(user_window_proc_ptr_t windowProcPtr,
                                void *pData) {
...
frame_buf_window::frame_buf_window(const char *pTitle, int width, int height,
                                   int scaleX, int scaleY)
    : m_width(width), m_height(height), m_orig_width(0), m_orig_height(0),
      m_orig_x(0), m_orig_y(0), m_window(0), m_pBitmap_hdr(NULL),
      m_windowHDC(0), m_pWindow_proc(NULL), m_pWindow_proc_data(NULL) {
  m_scale_x = scaleX;
  m_scale_y = scaleY;
...
frame_buf_window::~frame_buf_window() { close(); }
...
void frame_buf_window::create_window(const char *pTitle) {
  ...
  m_window_class.hbrBackground =
      (HBRUSH)GetStockObject(NULL_BRUSH); // GetStockObject(NULL_BRUSH);
  ...
  m_window = CreateWindowEx(0, pTitle, pTitle, WS_OVERLAPPEDWINDOW, m_orig_x,
                            m_orig_y, rect.right, rect.bottom, 0, 0, 0, 0);
...
void frame_buf_window::create_bitmap(void) {
  ...
  m_pBitmap_hdr->bmiHeader.biCompression =
      BI_RGB; // BI_BITFIELDS;
...
LRESULT frame_buf_window::window_proc(HWND hWnd, UINT message, WPARAM wParam,
                                      LPARAM lParam) {
  if (m_pWindow_proc) {
    LRESULT lres;
    bool status = m_pWindow_proc(lres, hWnd, message, wParam, lParam,
                                 m_pWindow_proc_data);
    if (status)
      return lres;
  }
  ...
  case WM_PAINT: {
    if (m_frame_buffer.is_valid()) {
      ...
      StretchDIBits(m_windowHDC, 0, 0, window_size.right, window_size.bottom, 0,
                    0, m_width, m_height, m_frame_buffer.get_ptr(),
                    m_pBitmap_hdr, DIB_RGB_COLORS, SRCCOPY);
...
public:
  enum { cProtocolPort = 3779, cMaxClients = 4 };
...
public:
  CConCommandConnection()
      : m_pSocket(NULL), m_bDeleteSocket(false), m_nEndpointIndex(-1),
        m_bClientFlag(false), m_bReceivedNewCameraPos(false), m_nSendBufOfs(0),
        m_bHasNewScreenshot(false), m_nScreenshotID(0) {
    memset(&m_NewCameraPos, 0, sizeof(m_NewCameraPos));
  }

  ~CConCommandConnection() { Deinit(); }
  ...
  bool Init(const char *pAddress) {
    ...
    uint n = 0;
    while (m_pSocket->GetEndpointSocketState(0) ==
           SSTATE_CONNECTION_IN_PROGRESS) {
  ...
  bool Init(CSocketConnection *pSocket, int nEndpointIndex, bool bClientFlag) {
    ...
    int nFlag = 1;
    pSocket->SetSocketOpt(nEndpointIndex, IPPROTO_TCP, TCP_NODELAY, &nFlag,
                          sizeof(int));
  ...
  bool TickConnectionRecv() {
    for (;;) {
      uint8 buf[4096];
      int nBytesRead = 0;
      SocketErrorCode_t nReadErr = m_pSocket->ReadFromEndpoint(
          m_nEndpointIndex, buf, sizeof(buf), &nBytesRead);
  ...
  bool TickConnectionSend() {
    while (m_nSendBufOfs < m_SendBuf.Count()) {
      int nBytesWritten = 0;
      SocketErrorCode_t nWriteErr = m_pSocket->WriteToEndpoint(
          m_nEndpointIndex, &m_SendBuf[m_nSendBufOfs],
  ...
  bool TickConnection() {
    ...
    if (m_bReceivedNewCameraPos) {
      ...
      char buf[256];
      // V_snprintf( buf, sizeof( buf ), "setpos_exact %f %f %f;setang_exact %f
      // %f %f\n", m_NewCameraPos.m_Pos.x, m_NewCameraPos.m_Pos.y,
      // m_NewCameraPos.m_Pos.z, m_NewCameraPos.m_Angle.x,
      // m_NewCameraPos.m_Angle.y, m_NewCameraPos.m_Angle.z );
      V_snprintf(buf, sizeof(buf),
                 "setpos_exact 0x%X 0x%X 0x%X;setang_exact 0x%X 0x%X 0x%X\n",
                 *(DWORD *)&m_NewCameraPos.m_Pos.x,
                 *(DWORD *)&m_NewCameraPos.m_Pos.y,
                 *(DWORD *)&m_NewCameraPos.m_Pos.z,
                 *(DWORD *)&m_NewCameraPos.m_Angle.x,
                 *(DWORD *)&m_NewCameraPos.m_Angle.y,
                 *(DWORD *)&m_NewCameraPos.m_Angle.z);
...
private:
  ...
  bool ProcessMessages() {
    while (m_RecvBuf.Count() >= cPacketHeaderSize) {
      ...
      const PacketHeader_t header(
          *reinterpret_cast<const PacketHeader_t *>(&m_RecvBuf[0]));
      ...
      const uint8 *pMsg =
          nNumMessageBytes ? &m_RecvBuf[cPacketHeaderSize] : NULL;
      ...
      case cPacketTypeScreenshotRequest: {
        ...
        videomode->ReadScreenPixels(0, 0, nWidth, nHeight, s_pBuf,
                                    IMAGE_FORMAT_RGB888);

        size_t nPNGSize = 0;
        void *pPNGData = tdefl_write_image_to_png_file_in_memory(
            s_pBuf, nWidth, nHeight, 3, &nPNGSize, true);
      ...
      case cPacketTypeScreenshotReply: {
        ...
        if (!replyPacket.m_nScreenshotID) {
          char szFilename[512];
          V_snprintf(szFilename, sizeof(szFilename), "%s_%u.png",
                     replyPacket.m_szFilename, m_nEndpointIndex);
          ...
          unsigned char *pImageData =
              stbi_load_from_memory(reinterpret_cast<stbi_uc const *>(pPNGData),
          ...
          if (((int)m_screenshot.width() != nWidth) ||
              ((int)m_screenshot.height() != nHeight)) {
      ...
      case cPacketTypeCommand: {
        if (nNumMessageBytes) {
          // ConMsg( "CONCMDSRV: Console command from client %i: ",
          // m_nEndpointIndex );
      ...
      case cPacketTypeConVarDumpRequest: {
        ...
        for (int i = 0; i < conVars.Count(); i++) {
          ...
          conVarData.AddMultipleToTail(V_strlen(pName) + 1,
                                       reinterpret_cast<const uint8 *>(pName));
          conVarData.AddMultipleToTail(V_strlen(pVal) + 1,
                                       reinterpret_cast<const uint8 *>(pVal));
        }
      ...
      case cPacketTypeConVarDumpReply: {
        ...
        const char *pEnd =
            reinterpret_cast<const char *>(pMsg) + nNumMessageBytes;
        ...
        ConMsg(
            "Received convar dump reply from endpoint %i, %u total convars\n",
      ...
      if (nNumBytesRemaining >= 0) {
        memmove(&m_RecvBuf[0], &m_RecvBuf[0] + header.m_nTotalSize,
                nNumBytesRemaining);
...
ConVar ccs_remote_screenshots_delta("ccs_remote_screenshots_delta", "1",
                                    FCVAR_CHEAT);

class CConCommandServer : public CConCommandServerProtocolTypes {
  static bool UserWindowProc(LRESULT &hres, HWND hWnd, UINT message,
                             WPARAM wParam, LPARAM lParam, void *pData) {
...
public:
  CConCommandServer()
      : m_bInitialized(false), m_pFrameBufWindow(NULL),
        m_nViewEndpointIndex(0) {}

  ~CConCommandServer() { Deinit(); }
  ...
  bool Init() {
    ...
    if (m_Socket.Listen(cProtocolPort, cMaxClients) != SOCKET_SUCCESS) {
      Warning("CONCMDSRV: CConCommandServer:Init: Failed to create listen "
              "socket!\n");
      return false;
    }
  ...
  void TickFrame(float flTime) {
    ...
    if (ccs_broadcast_camera.GetBool()) {
      ...
      if (ccs_camera_sync.GetBool()) {
        ...
        V_snprintf(buf, sizeof(buf),
                   "setpos_exact 0x%X 0x%X 0x%X;setang_exact 0x%X 0x%X 0x%X\n",
                   *(DWORD *)&setCameraPacket.m_Pos.x,
                   *(DWORD *)&setCameraPacket.m_Pos.y,
                   *(DWORD *)&setCameraPacket.m_Pos.z,
                   *(DWORD *)&setCameraPacket.m_Angle.x,
                   *(DWORD *)&setCameraPacket.m_Angle.y,
                   *(DWORD *)&setCameraPacket.m_Angle.z);
        Cbuf_AddText(buf);
        Cbuf_Execute();
      }
    }
        ...
        if (client.HasNewScreenshotFlag()) {
          ...
          if (ccs_remote_screenshots_delta.GetBool()) {
            if (videomode && !bHasUpdatedScreenshot) {
              ...
              if ((m_screenshot.width() != nScreenWidth) ||
                  (m_screenshot.height() != nScreenHeight)) {
                m_screenshot.init(nScreenWidth, nScreenHeight);
              }

              videomode->ReadScreenPixels(0, 0, nScreenWidth, nScreenHeight,
                                          m_screenshot.get_ptr(),
                                          IMAGE_FORMAT_RGB888);
            }
...
private:
  ...
  void AcceptNewConnections() {
    for (;;) {
      ...
      SocketErrorCode_t err =
          m_Socket.TryAcceptIncomingConnection(&nNewEndpointIndex);
  ...
  static int FindConVar(const DumpedConVarVector_t &sortedConVars,
                        const char *pName) {
  ...
  static void DiffConVars(DumpedConVarVector_t &serverConVars,
                          DumpedConVarVector_t &clientConVars) {
    ...
    memset(&clientConVarFoundFlags.Head(), 0,
           sizeof(bool) * clientConVars.Count());
    ...
    for (int i = 0; i < serverConVars.Count(); i++) {
      ...
      if (nClientConVarIndex < 0) {
        ...
        if (V_strcmp(pServerConVarValue,
                     clientConVars[nClientConVarIndex].m_Value.Get()) == 0) {
          nTotalMatches++;
        } else {
          nTotalMismatches++;
  ...
  void diffClientsDumpedConVars(int nClientIndex,
                                DumpedConVarVector_t &clientConVars) {
...
CON_COMMAND_F(ccs_msg,
              "Send a message to any connected con command server clients.",
              FCVAR_CHEAT) {
  if (g_ConCommandServer.IsInitialized()) {
    if (args.ArgC() < 2) {
...
CON_COMMAND_F(
    ccs_cmd,
    "Send a console message to any connected con command server clients.",
    FCVAR_CHEAT) {
  if (g_ConCommandServer.IsInitialized()) {
    if (args.ArgC() < 2) {
...
CON_COMMAND_F(ccs_cmd_local,
              "Send a console message to any connected con command server "
              "clients, and also issue the command locally.",
              FCVAR_CHEAT) {
  if (g_ConCommandServer.IsInitialized()) {
    if (args.ArgC() < 2) {
...
CON_COMMAND_F(ccs_screenshot, "Request screenshots from connected clients",
              FCVAR_CHEAT) {
  if (g_ConCommandServer.IsInitialized()) {
...
CON_COMMAND_F(ccs_diff_convars,
              "Diffs server's convars vs. all connected clients", FCVAR_CHEAT) {
  if (g_ConCommandServer.IsInitialized()) {
...
CON_COMMAND_F(ccs_disconnect, "Disconnect from a con cmd server.",
              FCVAR_CHEAT) {
  if (!g_ConCommandConnection.IsConnected()) {
