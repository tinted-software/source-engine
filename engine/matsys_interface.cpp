...
static int ReadVideoConfigInt(const char *pName, int nDefault) {
  ...
  bool bFileExists = pVideoConfig->LoadFromFile(
      g_pFullFileSystem, MOD_VIDEO_CONFIG_SETTINGS, "MOD");

  // We probably didn't have one on disk yet, just bail.  It'll get created
  // soon.
  if (!bFileExists)
    return nDefault;
...
static const char *ReadVideoConfigString(const char *pName,
                                         const char *pDefault) {
#if USE_VIDEOCONFIG_FILE
  AUTO_LOCK(g_VideoConfigMutex);
...
static void WriteVideoConfigInt(const char *pName, int nEntry) {
  ...
  // Try to make a keyvalues from the cfg file
  KeyValues *pVideoConfig = new KeyValues("videoconfig");
  pVideoConfig->LoadFromFile(g_pFullFileSystem, MOD_VIDEO_CONFIG_SETTINGS,
                             "MOD");

  pVideoConfig->SetInt(pName, nEntry);

  pVideoConfig->SaveToFile(g_pFullFileSystem, MOD_VIDEO_CONFIG_SETTINGS, "MOD",
                           false, false, true);
  pVideoConfig->deleteThis();
#else
...
//-----------------------------------------------------------------------------
static void
ReadMaterialSystemConfigFromRegistry(MaterialSystem_Config_t &config) {
#ifndef SWDS
  if (IsX360())
    return;
  ...
  config.SetFlag(MATSYS_VIDCFG_FLAGS_WINDOWED,
                 ReadVideoConfigInt("ScreenWindowed", 0) != 0);
  ...
  if (conVar.IsValid()) {
    ...
    if (!SDL_GetDisplayBounds(displayIndex, &rect)) {
      if ((config.m_VideoMode.m_Width > rect.w) ||
  ...
  const char *szMonitorGamma =
      ReadVideoConfigString("ScreenMonitorGamma", "2.2");
  if (szMonitorGamma && strlen(szMonitorGamma) > 0) {
    ...
    flMonitorGamma =
        OverrideVideoConfigFromCommandLine("mat_monitorgamma", flMonitorGamma);
  ...
  if (nValue != -1) {
    nValue =
        OverrideVideoConfigFromCommandLine("mat_motion_blur_enabled", nValue);
  ...
  if (nValue != -1) {
    nValue =
        OverrideVideoConfigFromCommandLine("r_flashlightdepthtexture", nValue);

    ConVarRef conVar("r_flashlightdepthtexture");
    if (conVar.IsValid()) {
      conVar.SetValue(nValue);
      config.m_bShadowDepthTexture =
          ReadVideoConfigInt("ShadowDepthTexture", 0) != 0;
    }
  }
...
//-----------------------------------------------------------------------------
static void
WriteMaterialSystemConfigToRegistry(const MaterialSystem_Config_t &config) {
#ifndef SWDS
  if (IsX360())
    return;
  ...
  WriteVideoConfigInt("ShadowDepthTexture",
...
//-----------------------------------------------------------------------------
static void
OverrideMaterialSystemConfigFromCommandLine(MaterialSystem_Config_t &config) {
  if (IsX360()) {
...
#if defined(USE_SDL) && !defined(SWDS)
  // If -displayindex was specified on the command line, then set
  // sdl_displayindex.
  if (CommandLine()->FindParm("-displayindex")) {
    ...
    if (conVar.IsValid()) {
      int displayIndex =
          CommandLine()->ParmValue("-displayindex", conVar.GetInt());
  ...
  config.m_VideoMode.m_Width =
      MIN(videoMode.m_Width, config.m_VideoMode.m_Width);
  config.m_VideoMode.m_Height =
...
void HandleServerAllowColorCorrection() {
  ...
  if (!sv_allow_color_correction.GetBool() && mat_colorcorrection.GetInt()) {
    Warning("mat_colorcorrection being forced to 0 due to "
...
// auto config version to store in the registry so we can force reconfigs if
// needed
...
void InitMaterialSystemConfig(bool bInEditMode) {
  ...
  // Don't smack registry if dxlevel is overridden, or if the video config was
...
void GetMaterialSystemConfigForBenchmarkUpload(KeyValues *dataToUpload) {
  ...
  dataToUpload->SetInt("Windowed", (g_pMaterialSystemConfig->m_Flags &
                                    MATSYS_VIDCFG_FLAGS_WINDOWED)
                                       ? 1
                                       : 0);
...
void PrintMaterialSystemConfig(const MaterialSystem_Config_t &config) {
  ...
  Warning("MATSYS_VIDCFG_FLAGS_FORCE_HWSYNC: %s\n",
          (config.m_Flags & MATSYS_VIDCFG_FLAGS_FORCE_HWSYNC) ? "true"
                                                              : "false");
  ...
  Warning("MATSYS_VIDCFG_FLAGS_USE_Z_PREFILL: %s\n",
          (config.m_Flags & MATSYS_VIDCFG_FLAGS_USE_Z_PREFILL) ? "true"
                                                               : "false");
...
CON_COMMAND(mat_enable_vrmode,
  ...
  if (bVRMode) {
    ...
    if (nVRModeAdapter == -1) {
      Warning("Unable to get VRModeAdapter from OpenVR. VR mode will not be "
              "enabled. Try restarting and then enabling VR again.\n");
    }
    mat_vrmode_adapter.SetValue(nVRModeAdapter);
#else
    mat_vrmode_adapter.SetValue(
        0); // This convar isn't actually used on other platforms so just use 0
...
static ITexture *CreateFullFrameDepthTexture(void) {
  if (IsX360()) {
    return materials->CreateNamedRenderTargetTextureEx2(
        "_rt_FullFrameDepth", 1, 1, RT_SIZE_FULL_FRAME_BUFFER,
        materials->GetShadowDepthTextureFormat(), MATERIAL_RT_DEPTH_NONE,
...
static ITexture *CreateResolvedFullFrameDepthTexture(void) {
  if (IsPC()) {
    return materials->CreateNamedRenderTargetTextureEx2(
        "_rt_ResolvedFullFrameDepth", 1, 1, RT_SIZE_FULL_FRAME_BUFFER,
...
void InitWellKnownRenderTargets(void) {
  ...
  // before we create anything, see if VR mode wants to override the
  // "framebuffer" size
  if (UseVR()) {
    int nWidth, nHeight;
    ...
    if (IsX360())
      materials->AddTextureAlias(
          ...
          "_rt_SmallFB0"); // an alias is good enough on the 360 since we don't
                           // have a texture lock problem during post processing
    else
  ...
  if (IsX360()) {
    g_FullFrameFBTexture2.Init(
        CreateFullFrameFBTexture(2, CREATERENDERTARGETFLAGS_TEMP));
  }
...
static void InitDebugMaterials(void) {
  ...
  if (g_pMaterialSystemHardwareConfig->GetDXSupportLevel() >= 90) {
    ...
    g_pMaterialSSAODepthWrite[0][0] = g_pMaterialSystem->FindProceduralMaterial(
        "__ColorDepthWrite00", TEXTURE_GROUP_OTHER, pVMTKeyValues);
    ...
    pVMTKeyValues->SetInt("$nocull", 1);
    pVMTKeyValues->SetInt("$color_depth", 1);
    g_pMaterialSSAODepthWrite[0][1] = g_pMaterialSystem->FindProceduralMaterial(
        "__ColorDepthWrite01", TEXTURE_GROUP_OTHER, pVMTKeyValues);
    ...
    pVMTKeyValues->SetInt("$nocull", 0);
    pVMTKeyValues->SetInt("$color_depth", 1);
    g_pMaterialSSAODepthWrite[1][0] = g_pMaterialSystem->FindProceduralMaterial(
        "__ColorDepthWrite10", TEXTURE_GROUP_OTHER, pVMTKeyValues);
    ...
    pVMTKeyValues->SetInt("$nocull", 1);
    pVMTKeyValues->SetInt("$color_depth", 1);
    g_pMaterialSSAODepthWrite[1][1] = g_pMaterialSystem->FindProceduralMaterial(
        "__ColorDepthWrite11", TEXTURE_GROUP_OTHER, pVMTKeyValues);
    ...
    g_pMaterialSSAODepthWrite[0][0] = g_pMaterialSSAODepthWrite[0][1] =
        g_pMaterialSSAODepthWrite[1][0] = g_pMaterialSSAODepthWrite[1][1] =
            NULL;
  }
#endif
}
...
void InitStartupScreen() {
  ...
  CMatRenderContextPtr pRenderContext(g_pMaterialSystem);
  pRenderContext->SetNonInteractiveTempFullscreenBuffer(
      pTexture, MATERIAL_NON_INTERACTIVE_MODE_STARTUP);

  pTexture = materials->FindTexture("//platform/materials/engine/box",
                                    TEXTURE_GROUP_OTHER);
  ...
  if (modinfo->LoadFromFile(g_pFileSystem, "gameinfo.txt")) {
    if (V_stricmp(modinfo->GetString("type", "singleplayer_only"),
                  "multiplayer_only") == 0) {
      pRenderContext->SetNonInteractivePacifierTexture(pTexture, 0.5f, 0.9f,
                                                       0.1f);
    } else {
      pRenderContext->SetNonInteractivePacifierTexture(pTexture, 0.5f, 0.86f,
                                                       0.1f);
    }
  }
  modinfo->deleteThis();
  ...
  if (!SurfaceHasDispInfo(surfID) &&
    ...
    if (pMaterialName) {
      ...
      if (bShowIt) {
        ...
        Warning("Warning: using WorldTwoTextureBlend on a non-displacement "
                "surface.\n"
  ...
  for (int i = 0; i < MSurf_VertCount(surfID); i++) {
    ...
    if (!SurfaceHasDispInfo(surfID) &&
      ...
      if (!bWarned) {
        const char *pMaterialName = MSurf_TexInfo(surfID)->material->GetName();
        bWarned = true;
        Warning("Warning: WorldTwoTextureBlend found on a non-displacement "
                "surface (material: %s). This wastes perf for no benefit.\n",
                pMaterialName);
      }
