...
static ConVar
    sv_lowedict_threshold("sv_lowedict_threshold", "8", FCVAR_NONE,
                          "When only this many edicts are free, take the "
                          "action specified by sv_lowedict_action.",
                          true, 0, true, MAX_EDICTS);
static ConVar sv_lowedict_action(
    "sv_lowedict_action", "0", FCVAR_NONE,
    "0 - no action, 1 - warn to log file, 2 - attempt to restart the game, if "
    "applicable, 3 - restart the map, 4 - go to the next map in the map cycle, "
    "5 - spew all edicts.",
    true, 0, true, 5);
...
static void SpewEdicts() {
  ...
  struct EdictCount_t {
    EdictCount_t(const char *pszClassName, int nCount)
        : m_pszClassName(pszClassName), m_nCount(nCount) {}
  ...
  FOR_EACH_VEC(vecEnts, i) {
    g_Log.Printf("(%3.2f%%) %d\t%s\n",
                 vecEnts[i]->m_nCount / (float)nEdictNum * 100.f,
                 vecEnts[i]->m_nCount, vecEnts[i]->m_pszClassName);
  }
...
edict_t *ED_Alloc(int iForceEdictIndex) {
  ...
  for (;;) {
    ...
    // If this assert goes off, someone most likely called pedict->ClearFree()
    // and not ED_ClearFreeFlag()?
    Assert(pEdict->IsFree());
    Assert(iBit == pEdict->m_EdictIndex);
    if ((pEdict->freetime < 2) ||
  ...
  if (sv_lowedict_action.GetInt() > 0 &&
      sv.num_edicts >= sv.max_edicts - sv_lowedict_threshold.GetInt()) {
    int nEdictsRemaining = sv.max_edicts - sv.num_edicts;
    g_Log.Printf(
        "Warning: free edicts below threshold. %i free edict%s remaining.\n",
...
int NUM_FOR_EDICT(const edict_t *e) {
  if (&sv.edicts[e->m_EdictIndex] ==
      e) // NOTE: old server.dll may stomp m_EdictIndex
