...
public:
  ...
  virtual void Connect(
      const char *adr,
      const char *pszSourceTag); // start a connection challenge
  virtual bool SetSignonState(int state, int count);
  virtual void Disconnect(const char *pszReason, bool bShowMainMenu);
  virtual void SendConnectPacket(int challengeNr, int authProtocol,
...
private:
  bool PrepareSteamConnectResponse(uint64 unGSSteamID, bool bGSSecure,
...
public:
  ...
  CUtlString m_sRetrySourceTag; // string that describes why we decided to
                                // connect to this server (empty for command
  ...
  int m_nServerCount; // server identification for prespawns, must match the
                      // svs.spawncount which is incremented on server spawning.
                      // This supercedes svs.spawn_issued, in that we can now
                      // spend a fair amount of time sitting connected to the
                      // server but downloading models, sounds, etc.  So much
                      // time that it is possible that the server might change
                      // levels again and, if so, we need to know that.
  uint64 m_ulGameServerSteamID; // Steam ID of the game server we are trying to
                                // connect to, or are connected to.  Zero if
                                // unknown
  int m_nCurrentSequence; // this is the sequence number of the current incoming
                          // packet
  ...
  bool m_bRestrictServerCommands; // If true, then the server is only allowed to
                                  // execute commands marked with
                                  // FCVAR_SERVER_CAN_EXECUTE on the client.
  bool
      m_bRestrictClientCommands; // If true, then IVEngineClient::ClientCmd is
                                 // only allowed to execute commands marked with
                                 // FCVAR_CLIENTCMD_CAN_EXECUTE on the client.
};
