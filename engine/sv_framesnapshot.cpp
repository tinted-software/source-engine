...
CFrameSnapshotManager::~CFrameSnapshotManager(void) {
  AssertMsg1(m_FrameSnapshots.Count() == 0 || IsInErrorExit(),
             "Expected m_FrameSnapshots to be empty. It had %i items.",
             m_FrameSnapshots.Count());

  // TODO: This assert has been failing. HenryG says it's a valid assert and
  // that we're probably leaking memory.
  AssertMsg1(m_PackedEntitiesPool.Count() == 0 || IsInErrorExit(),
             "Expected m_PackedEntitiesPool to be empty. It had %i items.",
             m_PackedEntitiesPool.Count());
}
...
CFrameSnapshot *
CFrameSnapshotManager::NextSnapshot(const CFrameSnapshot *pSnapshot) {
...
CFrameSnapshot *CFrameSnapshotManager::TakeTickSnapshot(int tickcount) {
  ...
  if (replay && replay->IsActive()) {
    ...
    Q_memset(snap->m_pReplayEntityData, 0,
             snap->m_nValidEntities * sizeof(CReplayEntityData));
  }
#endif
...
PackedEntity *
CFrameSnapshotManager::GetPreviouslySentPacket(int iEntity, int iSerialNumber) {
...
PackedEntity *
CFrameSnapshotManager::CreatePackedEntity(CFrameSnapshot *pSnapshot,
                                          int entity) {
  m_WriteMutex.Lock();
  PackedEntity *packedEntity = m_PackedEntitiesPool.Alloc();
  PackedEntityHandle_t handle =
      reinterpret_cast<PackedEntityHandle_t>(packedEntity);
  m_WriteMutex.Unlock();
...
// //
UnpackedDataCache_t *
CFrameSnapshotManager::GetCachedUncompressedEntity(PackedEntity *packedEntity) {
  ...
  pdcOldest->bits =
      -1; // important, this is the signal for the caller to fill this structure
