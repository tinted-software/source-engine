...
void ReadCheatCommandsFromFile(char *pchFileName) {
  ...
  for (pKey = pCheatCodeKeys->GetFirstTrueSubKey(); pKey;
    ...
    Q_strncpy(pNewCheatCode->szCommand,
    ...
    if (pNewCheatCode->iCodeLength < CHEAT_NAME_MAX_LEN) {
      // If it's activation is a subsequence of another cheat, the longer cheat
      // can't be activated!
...
void CheckCheatCodes() {
  ...
  for (int iCheatCode = 0; iCheatCode < iNumCheatCodes; ++iCheatCode) {
    ...
    int iLogIndex =
        s_nKeyLogIndex -
        pCheatCode->iCodeLength; // Check code against the back chunk of the log
