...
public:
  ...
  virtual void
  GetAllSoundFilesInManifest(CUtlRBTree<FileNameHandle_t, int> &list,
                             char const *manifestfile) {
    list.RemoveAll();
  ...
  virtual void CacheBuildingUpdateProgress(float percent,
    ...
    if (format) {
      ...
      g_pVGuiLocalize->ConstructString_safe(constructed, (wchar_t *)format, 1,
                                            file);
