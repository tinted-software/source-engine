...
static ConVar net_showudp_wire("net_showudp_wire", "0", 0,
                               "Show incoming packet information");

#define UDP_SO_RCVBUF_SIZE 131072

static ConVar net_udp_rcvbuf("net_udp_rcvbuf", NETSTRING(UDP_SO_RCVBUF_SIZE),
                             FCVAR_ALLOWED_IN_COMPETITIVE,
                             "Default UDP receive buffer size", true, 8192,
                             true, 128 * 1024);

static ConVar net_showsplits("net_showsplits", "0", 0,
                             "Show info about packet splits");

static ConVar net_splitrate(
    "net_splitrate", "1", 0,
    "Number of fragments for a splitpacket that can be sent per frame");

static ConVar ipname("ip", "localhost", FCVAR_ALLOWED_IN_COMPETITIVE,
                     "Overrides IP for multihomed hosts");
static ConVar hostport("hostport", NETSTRING(PORT_SERVER),
                       FCVAR_ALLOWED_IN_COMPETITIVE, "Host game server port");
static ConVar hostip("hostip", "", FCVAR_ALLOWED_IN_COMPETITIVE,
                     "Host game server ip");

static ConVar clientport("clientport", NETSTRING(PORT_CLIENT),
                         FCVAR_ALLOWED_IN_COMPETITIVE, "Host game client port");
static ConVar hltvport("tv_port", NETSTRING(PORT_HLTV),
                       FCVAR_ALLOWED_IN_COMPETITIVE, "Host SourceTV port");
static ConVar matchmakingport("matchmakingport", NETSTRING(PORT_MATCHMAKING),
                              FCVAR_ALLOWED_IN_COMPETITIVE,
                              "Host Matchmaking port");
static ConVar systemlinkport("systemlinkport", NETSTRING(PORT_SYSTEMLINK),
                             FCVAR_ALLOWED_IN_COMPETITIVE, "System Link port");
...
ConVar net_usesocketsforloopback(
    ...
    "Use network sockets layer even for listen server local player's packets "
    "(multiplayer only).");
...
static ConVar
    fakeshuffle("net_fakeshuffle", "0", FCVAR_CHEAT,
...
static ConVar net_savelargesplits(
    ...
    "If not -1, then if a split has this many or more split parts, save the "
    "entire packet to disc for analysis.");
#endif

#ifdef _X360
static void NET_LogServerCallback(IConVar *var, const char *pOldString,
                                  float flOldValue);
static ConVar net_logserver("net_logserver", "0", 0,
                            "Dump server stats to a file",
                            NET_LogServerCallback);
static ConVar net_loginterval("net_loginterval", "1", 0,
                              "Time in seconds between server logs");
#endif
...
typedef struct {
  ...
  char buffer[NET_MAX_MESSAGE]; // This has to be big enough to hold the largest
                                // message
} LONGPACKET;
...
// For metering out splitpackets, don't do them too fast as remote UDP socket
// will drop some payloads causing them to always fail to be reconstituted This
// problem is largely solved by increasing the buffer sizes for UDP sockets on
// Windows
...
void NET_ClearLastError(void) { net_error = 0; }
...
int NET_OpenSocket(const char *net_interface, int &port, int protocol) {
  ...
  if (protocol == IPPROTO_TCP) {
    ...
    opt =
        1; // set TCP options: Disables the Nagle algorithm for send coalescing.
  ...
  VCR_NONPLAYBACKFN(
      getsockopt(newsocket, SOL_SOCKET, SO_RCVBUF, (char *)&opt, &len), ret,
      "getsockopt");
  if (ret == -1) {
    NET_GetLastError();
  ...
  int port_offset; // try binding socket to port, try next 10 is port is already
                   // used
  ...
  for (port_offset = 0; port_offset < PORT_TRY_MAX; port_offset++) {
    ...
    if (ret != -1) {
      if (port != PORT_ANY && port_offset != 0) {
        ...
        ConDMsg("Socket bound to non-default port %i because original port was "
                "already in use.\n",
                port);
      }
      break;
  ...
  if (port_offset > 0) {
    if (bStrictBind) {
      ...
      Sys_Exit("ERROR: Port %i was unavailable - quitting due to "
               "\"-strictportbind\" command-line flag!\n",
               port - port_offset);
    } else {
...
#define SPLIT_PACKET_TRACKING_MAX                                              \
  256 // most number of outstanding split packets to allow
  ...
  if (splitPacketEntries.Count() > SPLIT_PACKET_TRACKING_MAX) {
    while (splitPacketEntries.Count() > SPLIT_PACKET_TRACKING_MAX) {
      ...
      if (net_time != entry->lastactivetime)
        splitPacketEntries.Remove(0); // we add to tail each time, so head is
                                      // the oldest entry, kill them first
    }
  }
}
...
bool NET_GetLong(const int sock, netpacket_t *packet) {
  ...
  if (nSplitSizeMinusHeader < MIN_SPLIT_SIZE ||
    ...
    Msg("NET_GetLong:  Split packet from %s with invalid split size (number "
        "%i/ count %i) where size %i is out of valid range [%llu - %llu]\n",
        packet->from.ToString(), packetNumber, packetCount,
        nSplitSizeMinusHeader, (uint64)MIN_SPLIT_SIZE, (uint64)MAX_SPLIT_SIZE);
    return false;
  }
  ...
  if (packetNumber >= MAX_SPLITPACKET_SPLITS ||
    ...
    Msg("NET_GetLong:  Split packet from %s with too many split parts (number "
        "%i/ count %i) where %llu is max count allowed\n",
        packet->from.ToString(), packetNumber, packetCount,
  ...
  if (entry->splitflags[packetNumber] != sequenceNumber) {
    ...
    if (net_showsplits.GetInt() && net_showsplits.GetInt() != 3) {
      Msg("<-- [%s] Split packet %4i/%4i seq %5i size %4i mtu %4llu from %s\n",
          ...
          size, (uint64)(nSplitSizeMinusHeader + sizeof(SPLITPACKET)),
...
bool NET_ReceiveDatagram(const int sock, netpacket_t *packet) {
  ...
  int ret = 0;
  {
    ...
    ret = VCRHook_recvfrom(net_socket, (char *)packet->data, NET_MAX_MESSAGE, 0,
  ...
  if (ret >= NET_MIN_MESSAGE) {
    ...
    if (net_showudp_wire.GetBool()) {
      Msg("WIRE:  UDP sz=%d tm=%f rt %f from %s\n", ret, net_time,
          Plat_FloatTime(), packet->from.ToString());
    }
    ...
    if (X360SecureNetwork()) {
      ...
      if (nVoiceBytes > 0) {
        ...
        int nDecompressedVoice =
            COM_GetUncompressedSize(pVoice, nCompressedSize);
        if (nDecompressedVoice >= 0) {
          ...
          if (!COM_BufferToBufferDecompress((char *)bufVoice.Base(),
                                            &unActualDecompressedSize, pVoice,
                                            nCompressedSize))
            return false;
    ...
    if (ret < NET_MAX_MESSAGE) {
      ...
      if (LittleLong(*(int *)packet->data) ==
        ...
        // Decompress
        int actualSize =
            COM_GetUncompressedSize(pCompressedData, nCompressedDataSize);
        ...
        COM_BufferToBufferDecompress((char *)memDecompressed.Base(),
                                     &uDecompressedSize, pCompressedData,
                                     nCompressedDataSize);
        if (uDecompressedSize == 0 ||
            ((unsigned int)actualSize) != uDecompressedSize) {
          if (net_showudp.GetBool()) {
            Msg("UDP:  discarding %d bytes from %s due to decompression error "
                "[%d decomp, actual %d] at tm=%f rt=%f\n",
      ...
      if (nVoiceBits > 0) {
        ...
        byte flagByte = *((byte *)packet->data + sizeof(unsigned int) +
                          sizeof(unsigned int));
...
bool NET_ReceiveValidDatagram(const int sock, netpacket_t *packet) {
  ...
  // invalid packets per frame will effectively DOS the server, but at that
  // point you're basically flooding the network and you need to solve this at a
  // higher firewall or router level instead which is beyond the scope of our
  // netcode.
  // --henryg 10/12/2011
...
int NET_SendToImpl(SOCKET s, const char FAR *buf, int len,
  ...
  if (X360SecureNetwork()) {
    ...
    // NOTE: The header bytes *should* be swapped to network endian, however
    // when communicating with XLSP servers (the only cross-platform
    // communication possible with a secure network) the server's network stack
    // swaps the header at the receiving end.
    const int nVDPHeaderBytes = 2;
    Assert(len < (unsigned short)-1);

    const unsigned short nDataBytes =
        iGameDataLength == -1 ? len : iGameDataLength;
...
static ConVar net_splitpacket_maxrate(
    "net_splitpacket_maxrate", SPLITPACKET_MAX_DATA_BYTES_PER_SECOND, 0,
    "Max bytes per second when queueing splitpacket chunks", true, MIN_RATE,
    true, MAX_RATE);
...
int NET_SendLong(INetChannel *chan, int sock, SOCKET s, const char FAR *buf,
  ...
  while (nBytesLeft > 0) {
    ...
    // flush itself well). The behavior the queue fixes is that if you send too
    // many DP packets without giving up your timeslice, it'll just discard the
    // 7th and later packets until you Sleep() (issue might be on client
    // recipient side, need to snif packets to double check)

    if (netchan && (nFragmentsSent >= net_splitrate.GetInt() ||
                    net_queued_packet_thread.GetInt() ==
                        NET_QUEUED_PACKET_THREAD_DEBUG_VALUE)) {
      // Don't let this rate get too high (SPLITPACKET_MAX_DATA_BYTES_PER_SECOND
      // == 15000 bytes/sec) or user's won't be able to receive all of the parts
      // since they'll be too close together.
      /// XXX(JohnS): (float)cv.GetInt() is just preserving what this was doing
      /// before to avoid changing the
      ///             semantics of this convar
      float flMaxSplitpacketDataRateBytesPerSecond =
          min((float)netchan->GetDataRate(),
              (float)net_splitpacket_maxrate.GetInt());

      // Calculate the delay (measured from now) for when this packet should be
      // sent.
      uint32 delay =
    ...
    if (net_showsplits.GetInt() && net_showsplits.GetInt() != 2) {
      ...
      Msg("--> [%s] Split packet %4i/%4i seq %5i size %4i mtu %4i to %s [ "
          "total %4i ]\n",
          DescribeSocket(sock), nPacketNumber, nPacketCount, nSequenceNumber,
          size, nMaxRoutableSize, adr.ToString(), sendlen);
    }
  }
...
int NET_SendPacket(INetChannel *chan, int sock, const netadr_t &to,
  ...
  if (pVoicePayload) {
    VPROF_BUDGET("NET_SendPacket_CompressVoice",
                 VPROF_BUDGETGROUP_OTHER_NETWORKING);
    unsigned int nCompressedLength =
        COM_GetIdealDestinationCompressionBufferSize_Snappy(
            pVoicePayload->GetNumBytesWritten());
    memCompressedVoice.EnsureCapacity(nCompressedLength +
    ...
    *(unsigned short *)pVoice = LittleShort(usVoiceBits);
    ...
    if (net_compressvoice.GetBool()) {
      if (COM_BufferToBufferCompress_Snappy(
              pVoice, &nCompressedLength, pVoicePayload->GetData(),
              pVoicePayload->GetNumBytesWritten() &&
  ...
  if (bUseCompression) {
    ...
    unsigned int nCompressedLength =
        COM_GetIdealDestinationCompressionBufferSize_Snappy(length);

    memCompressed.EnsureCapacity(nCompressedLength + nVoiceBytes +
                                 sizeof(unsigned int));

    *(int *)memCompressed.Base() = LittleLong(NET_HEADER_FLAG_COMPRESSEDPACKET);

    if (COM_BufferToBufferCompress_Snappy(memCompressed.Base() +
                                              sizeof(unsigned int),
                                          &nCompressedLength, data, length) &&
...
static bool OpenSocketInternal(int nModule, int nSetPort, int nDefaultPort,
                               const char *pName, int nProtocol, bool bTryAny,
                               int flags = (OSOCKET_FLAG_USE_IPNAME |
                                            OSOCKET_FLAG_FAIL)) {
  ...
  if (!net_sockets[nModule].nPort) {
    const char *netinterface =
        (flags & OSOCKET_FLAG_USE_IPNAME) ? ipname.GetString() : NULL;
...
void NET_OpenSockets(void) {
  // Xbox 360 uses VDP protocol to combine encrypted game data with clear voice
  // data
  ...
  if (IsX360()) {
    OpenSocketInternal(NS_MATCHMAKING, matchmakingport.GetInt(),
                       PORT_MATCHMAKING, "matchmaking", nProtocol, false);
    OpenSocketInternal(NS_SYSTEMLINK, systemlinkport.GetInt(), PORT_SYSTEMLINK,
                       "systemlink", IPPROTO_UDP, false);
  }

#ifdef LINUX
  // On Linux, if you bind to a specific address then you will NOT receive
  // broadcast messages. This means that if you do a +ip X.X.X.X, your game will
  // not show up on the LAN server browser page. To workaround this, if the user
  // has specified sv_lan and an IP address, we open an INADDR_ANY port. See
  // http://developerweb.net/viewtopic.php?id=5722 for more information.
  ...
  if (sv_lan.GetBool()) {
    ...
    if (net_interface[0] && (Q_strcmp(net_interface, "localhost") != 0)) {
      ...
      static int s_ports[] = {26900, 26901, 26902, 26903, 26904, 26905,
      ...
      for (size_t iport = 0; iport < ARRAYSIZE(s_ports); iport++) {
        ...
        if (!bPortUsed) {
          ...
          if (OpenSocketInternal(NS_SVLAN, s_ports[iport], PORT_SERVER, "lan",
                                 nProtocol, false, 0))
            break;
        }
      }
...
void NET_LogServerStatus(void) {
  ...
  Q_snprintf(msg, size, "%.0f,%d,%.0f,%.0f,%.0f,%.1f,%.1f,%.1f,%.1f,%.1f\n",
             fCurrentTime, numChannels, fStats[NET_LATENCY], fStats[NET_LOSS],
             fStats[NET_PACKETS_IN], fStats[NET_PACKETS_OUT],
             fStats[NET_FLOW_IN] / 1024.0f, fStats[NET_FLOW_OUT] / 1024.0f,
             fStats[NET_CHOKE_IN], fStats[NET_CHOKE_OUT]);

  g_pFileSystem->Write(msg, Q_strlen(msg), g_fh);
}

void NET_LogServerCallback(IConVar *pConVar, const char *pOldString,
                           float flOldValue) {
  ...
  if (var.GetBool()) {
    ...
    char msg[128];
    Q_snprintf(msg, 128,
               "Time,Channels,Latency,Loss,Packets In,Packets Out,Flow "
               "In(kB/s),Flow Out(kB/s),Choke In,Choke Out\n");
    g_pFileSystem->Write(msg, Q_strlen(msg), g_fh);
  } else {
    if (g_fh) {
      g_pFileSystem->Close(g_fh);
...
void NET_Init(bool bIsDedicated) {
  ...
  if (CommandLine()->FindParm("-NoQueuedPacketThread"))
    Warning("Found -NoQueuedPacketThread, so no queued packet thread will be "
            "created.\n");
  else
  ...
  if (hPort == -1) {
    hPort = CommandLine()->ParmValue("+port",
                                     -1); // check if they used +port by mistake
  }
  ...
  {
    ipname.SetValue(ip); // update the cvar right now, this will get overwritten
                         // by "stuffcmds" later
  }
...
CON_COMMAND(net_status, "Shows current network status") {
  ...
  ConMsg("- Ports: Client %u, Server %u, HLTV %u, Matchmaking %u, Systemlink "
         "%u%s\n",
