...
#ifndef DEDICATED // have to test this because VPC is forcing us to compile this
                  // file.

// How many bytes initial data bytes of the mp3 should be saved in the
// soundcache, in addition to the small amount of metadata (playbackrate, etc).
// This will increase memory usage by ( N *
// number-of-precached-mp3-sounds-in-the-whole-game ) at all times, as the
// soundcache is held in memory.
//
// Right now we're setting this to zero. The IsReadyToMix() logic at the data
// layer will delay mixing of the sound until it arrives. Setting this to
// anything above zero, however, will allow the sound to start, so it needs to
// either be enough to cover SND_ASYNC_LOOKAHEAD_SECONDS or none at all.
...
CAudioSourceMP3::CAudioSourceMP3(CSfxTable *pSfx) {
  ...
  if (file != -1) {
    ...
    // No sound cache, the file isn't here, print this so that the relatively
    // deep failure points that are about to spew make a little more sense
    Warning("MP3 is completely missing, sound system will be upset to learn of "
            "this [ %s ]\n",
            pSfx->GetFileName());
    m_dataSize = 0;
  }
...
int CAudioSourceMP3::SampleRate() {
  if (!m_sampleRate) {
    // This should've come from the sound cache. We can avoid sync I/O jank if
    // and only if we've started streaming data already for some other reason.
    // (Despite the name, CreateWaveDataMemory is just creating a wrapper class
    // that manages access to the wave data cache)
    IWaveData *pData = CreateWaveDataMemory(*this);
    if (!pData->IsReadyToMix() && SND_IsInGame()) {
      // If you hit this, you're creating a sound source that isn't in the sound
      // cache, and asking for its sample rate before it has streamed enough
      // data in to read it from the underlying file. Your options are:
      // - Rebuild sound cache or figure out why this sound wasn't included.
      // - Precache this sound at level load so this doesn't happen during
      // gameplay.
      // - Somehow call CacheLoad() on this source earlier so it has time to get
      // data into memory so the data
      //   shows up as IsReadyToMix here, and this crutch won't jank.
      Warning("MP3 initialized with no sound cache, this may cause janking. [ "
              "%s ]\n",
              GetFileName());
      // The code below will still go fine, but the mixer will emit a jank
      // warning that the data wasn't ready and do sync I/O
    }
...
void CAudioSourceMP3::GetCacheData(CAudioSourceCachedInfo *info) {
  // Don't want to replicate our cached sample rate back into the new cache,
  // ensure we recompute it.
  CAudioMixerWaveMP3 *pTempMixer =
      new CAudioMixerWaveMP3(CreateWaveDataMemory(*this));
  m_sampleRate = pTempMixer->GetStreamOutputRate();
  delete pTempMixer;

  AssertMsg(m_sampleRate, "Creating cache with invalid sample rate data");
  if (!m_sampleRate) {
    Warning("Failed to find sample rate creating cache data for MP3, cache "
            "will be invalid [ %s ]\n",
            GetFileName());
  }
...
#if MP3_STARTUP_DATA_SIZE_BYTES >                                              \
    0 // We may have defined the startup data to nothingness
  if (info->s_bIsPrecacheSound && m_dataSize > 0) {
    // Ideally this would mimic the wave startup data code and figure out this
    // calculation:
    //   int bytesNeeded = m_channels * ( m_bits >> 3 ) * m_rate *
    //   SND_ASYNC_LOOKAHEAD_SECONDS;
    // (plus header)
    ...
    if (readSize != dataSize) {
      Warning(
          "Building soundcache, expected %i bytes of data but got %i [ %s ]\n",
