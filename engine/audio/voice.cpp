...
#define VOICE_RECEIVE_BUFFER_SIZE                                              \
  (VOICE_OUTPUT_SAMPLE_RATE_MAX * BYTES_PER_SAMPLE)
...
ConVar voice_fadeouttime("voice_fadeouttime",
                         "0.1"); // It fades to no sound at the tail end of your
                                 // voice data when you release the key.
...
// This should not be lower than the maximum difference between clients' frame
// durations (due to cmdrate/updaterate), plus some jitter allowance.
ConVar voice_buffer_ms("voice_buffer_ms", "100", FCVAR_INTERNAL_USE,
                       "How many milliseconds of voice to buffer to avoid "
                       "dropouts due to jitter and frame time differences.");
...
// The codec and sample rate passed to Voice_Init. "" and -1 if voice is not
// initialized
...
public:
  ...
  CVoiceWriterData(const CVoiceWriterData &other)
      : m_pChannel(other.m_pChannel), m_nCount(other.m_nCount), m_Buffer() {
...
public:
  ...
  void AddDecompressedData(CVoiceChannel *ch, const byte *data,
                           size_t datalen) {
...
// Helper for doing a default-init with some codec if we weren't passed specific
// parameters
bool Voice_InitWithDefault(const char *pCodecName) {
  ...
  if (nRate < 0) {
    Msg("Voice_InitWithDefault: Unable to determine defaults for codec "
        "\"%s\"\n",
        pCodecName);
    return false;
  }
...
bool Voice_Init(const char *pCodecName, int nSampleRate) {
  ...
  // Miles has not been in use for voice in a long long time.  Not worth the
  // surface to support ancient demos that may use it (and probably do not work
  // for other reasons) "vaudio_miles"
  ...
  if (g_bUsingSteamVoice) {
    if (!steamapicontext->SteamFriends() || !steamapicontext->SteamUser()) {
      Msg("Voice_Init: Requested Steam voice, but cannot access API.  Voice "
          "will not function\n");
      return false;
    }
  }
  ...
  if (bSteam && nSampleRate == 0) {
    ...
    // Steam's sample rate may change and not be supported by our rather
    // unflexible sound engine. However, steam will resample as necessary in
    // DecompressVoice, so we can pretend we're outputting at native rates.
    //
    // Behind the scenes, we'll request steam give us the encoded stream at its
    // "optimal" rate, then we'll try to decompress the output at this rate,
    // making it transparent to us that the encoded stream is not at our output
    // rate.
  ...
  if (!g_pVoiceRecord) {
    Msg("Unable to initialize sound capture. You won't be able to speak to "
        "other players.");
  }

  // Init codec DLL for non-steam
  if (!bSteam) {
    // CELT's qualities are 0-3, we historically just passed 4 to the other two
    // even though they don't really map to the same thing.
    //
    // Changing the quality level we use here will require either extending
    // SVC_VoiceInit to pass down which quality is in use or using a different
    // codec name (vaudio_celtHD!) for backwards compatibility
    ...
    if (!g_hVoiceCodecDLL ||
      ...
      Msg("Unable to load voice codec '%s'. Voice disabled. (module %i, iface "
          "%i, codec %i)\n",
  ...
  // XXX(JohnS): These don't do much in Steam codec mode, but code below uses
  // their presence to mean 'voice fully
  //             initialized' and other things assume they will succeed.
  InitMixerControls();

  // Steam mode uses steam for raw input so this isn't meaningful and could have
  // side-effects
...
void Voice_Idle(float frametime) {
  ...
  g_nVoiceFadeSamples = max(
      (int)(voice_fadeouttime.GetFloat() * g_VoiceSampleFormat.nSamplesPerSec),
      2);
  ...
  if (g_pVoiceRecord) {
    ...
    if (g_bVoiceRecording) {
      if (steamapicontext && steamapicontext->SteamFriends() &&
        ...
        steamapicontext->SteamFriends()->SetInGameVoiceSpeaking(
            steamapicontext->SteamUser()->GetSteamID(), true);
      }
...
bool Voice_RecordStop() {
  ...
  if (g_bVoiceRecording) {
    if (steamapicontext->SteamFriends() && steamapicontext->SteamUser()) {
      ...
      steamapicontext->SteamFriends()->SetInGameVoiceSpeaking(
          steamapicontext->SteamUser()->GetSteamID(), false);
    }
  }
...
int Voice_GetCompressedData(char *pchDest, int nCount, bool bFinal) {
  ...
  if (g_bUsingSteamVoice && g_bVoiceRecordStopping) {
    ...
    // We're going to always request steam give us the encoded stream at the
    // optimal rate, unless our final output rate is lower than it.  We'll pass
    // our output rate when we actually extract the data, which Steam will
    // happily upsample from its optimal rate for us.
    int nEncodeRate =
        min((int)steamapicontext->SteamUser()->GetVoiceOptimalSampleRate(),
            Voice_SamplesPerSec());
    EVoiceResult result = steamapicontext->SteamUser()->GetAvailableVoice(
        &cbCompressed, &cbUncompressed, nEncodeRate);
    if (result == k_EVoiceResultOK) {
      result = steamapicontext->SteamUser()->GetVoice(
          true, pchDest, nCount, &cbCompressedWritten,
  ...
  if (g_pVoiceRecord && pCodec) {
    ...
    short tempData[8192];
    int samplesWanted = min(nCount / BYTES_PER_SAMPLE,
    ...
    if (g_pMicInputFileData) {
      ...
      gotten = min(sizeof(tempData) / BYTES_PER_SAMPLE,
                   (size_t)min(nShouldGet,
                               (g_nMicInputFileBytes - g_CurMicInputFileByte) /
                                   BYTES_PER_SAMPLE));
      ...
      if (deltaData < voice_threshold.GetFloat() &&
          maxData < voice_threshold.GetFloat()) {
...
//recently used 			channel. If the LRU channel is picked, it is flushed and all
//other 			channels are aged.
  ...
  for (int i = 0; i < VOICE_NUM_CHANNELS; i++) {
    ...
    if (pChannel->m_iEntity == nEntity) {
      return i;
               ...
               (pChannel->m_pVoiceCodec || g_bUsingSteamVoice)) {
...
int Voice_AddIncomingData(int nChannel, const char *pchData, int nCount,
  ...
  if ((pChannel = GetVoiceChannel(nChannel)) == NULL ||
      (!g_bUsingSteamVoice && !pChannel->m_pVoiceCodec)) {
    return (0);
  }
  ...
  if (g_bUsingSteamVoice) {
    ...
    EVoiceResult result = steamapicontext->SteamUser()->DecompressVoice(
        pchData, nCount, decompressed, sizeof(decompressed), &nBytesWritten,
        Voice_SamplesPerSec());
    if (result == k_EVoiceResultOK) {
      nDecompressed = nBytesWritten / BYTES_PER_SAMPLE;
    }
  } else {
    nDecompressed = pChannel->m_pVoiceCodec->Decompress(
  ...
  if (g_bInTweakMode) {
    ...
    for (int i = 0; i < nDecompressed; ++i) {
      g_VoiceTweakSpeakingVolume =
          max((int)abs(data[i]), g_VoiceTweakSpeakingVolume);
    }
  ...
  g_VoiceWriter.AddDecompressedData(pChannel, (const byte *)decompressed,
                                    nDecompressed * 2);
...
void Voice_ForceInit() {
  ...
  // Lacking a better default, just peak at what the server's sv_voicecodec is
  // set to
