...
//the sentence. 		Sentences may be separated by one or more lines 		There may be
//tabs or spaces preceding the sentence name 		The sentence must end in a /n or /r
...
static ConCommand vox_reload("vox_reload", VOX_Reload,
                             "Reload sentences.txt file", FCVAR_CHEAT);
...
typedef struct _vox_entname {
  ...
  char *pszglobal[CVOXGLOBMAX]; // 1 global word, shared by this type of entity,
                                // picked randomly, expires after 5min
...
ConVar
    snd_vox_globaltimeout("snd_vox_globaltimeout",
...
void VOX_LoadSound(channel_t *pchan, const char *pszin) {
  ...
  if (cword) {
    ...
    // if no words, then pchan->pMixer is null; chan will be released right
    // away.
  ...
  if (spewsentences) {
    ...
    for (i = 0; i < rep.Count(); ++i) {
      ...
      if ( !Q_stricmp( rep[ i ].word, "_comma" ) )
      {
              ...
              if ( i != rep.Count() - 1 )
              {
                      Q_strncat( outbuf, ", ", sizeof( outbuf ),
      COPY_ALL_CHARACTERS );
              }
              continue;
      }
