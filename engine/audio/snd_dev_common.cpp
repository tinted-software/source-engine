...
static float XfadeSpeakerVolToMono(float scale, float xfade, float ispeaker,
  ...
  if (cspeaker == 4) {
    ...
    if (fmix2channels)
      scale_target =
          scale_targets2ch[clamp(FastFloatToSmallInt(ispeaker), 0, 3)];
    else
  ...
  if (cspeaker == 5) {
    ...
    scale_target =
        scale_targets[(int)clamp(FastFloatToSmallInt(ispeaker), 0, 4)];
    goto XfadeExit;
  }

  // if (cspeaker == 2 )
  scale_target = 0.9; // front 2 speakers in stereo each get 50% of total volume
                      // in mono case
  ...
  if (pPaint->fsurround) {
    if (FVolumeRearNonZero(volume)) {
      ...
      Mix8MonoWavtype(pChannel, pPaint->pbufrear + outputOffset,
                      &volume[IREAR_LEFT], (byte *)pData, inputOffset,
    ...
    if (pPaint->fsurround_center && FVolumeCenterNonZero(volume)) {
      ...
      Mix8MonoWavtype(pChannel, pPaint->pbufcenter + outputOffset,
                      &volume[IFRONT_CENTER], (byte *)pData, inputOffset,
                      rateScaleFix, outCount);
    }
  }
}
