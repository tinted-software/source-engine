...
#define MSEC_TO_SAMPS(a)                                                       \
  (((a) * SOUND_DMA_SPEED) /                                                   \
   1000) // convert milliseconds to # samples in equivalent time
#define SEC_TO_SAMPS(a)                                                        \
  ((a) * SOUND_DMA_SPEED) // convert seconds to # samples in equivalent time
...
// first call fbgain:		feedback value, 0-PMAX (normalized to 0.0-1.0)
// outgain:		gain
// in:	input sample
...
// call fbgain:		feedback gain (built into filter gain) outgain:
// output gain cnumer:		filter order numer:		filter
// numerator, 0-PMAX (normalized to 0.0-1.0), cnumer+1 dimensional denom:
// filter denominator, 0-PMAX (normalized to 0.0-1.0), cnumer+1 dimensional
...
// cnumer+1 dimensional denom:		filter denominator, 0-PMAX (normalized
// to 0.0-1.0), cnumer+1 dimensional pfsamps:		filter state, cnumer+1
// dimensional in:			input sample
...
void FLT_Design_3db_IIR(float cutoff, float ftype, int *pM, int *pL, int *a,
  ...
  // calculations:
  // Wc = 2pi * fc/44100
  // convert to radians Oc = tan (Wc/2) * Gc / sqt ( 1 - Gc^2)
  // get analog version, low pass Oc = tan (Wc/2) * (sqt (1 - Gc^2)) / Gc
  // analog version, high pass Gc = 10 ^ (-Ac/20)
  // gain at cutoff.  Ac = 3db, so Gc^2 = 0.5 a = ( 1 - Oc ) / ( 1 + Oc ) b = (
  // 1 - a ) / 2
...
void DLY_SetNormalizingGain(dly_t *pdly, int feedback) {
  ...
  gain =
      gain * 4; // compensate for fact that gain calculation is for +/- 32767
                // amplitude wavs ie: ok to allow a bit more gain because most
                // wavs are not at theoretical peak amplitude at all times

  // limit gain reduction to N% PMAX

  gain =
      clamp(gain, (float)(PMAX * DLY_NORMALIZING_REDUCTION_MAX), (float)PMAX);
...
inline int DLY_GetNext(dly_t *pdly, int x) {
  ...
  case DLY_ALLPASS:
    return DelayAllpass(pdly->D, pdly->t, pdly->w, &pdly->p, pdly->a, pdly->b,
                        x);
  case DLY_LOWPASS:
    return DelayLowpass(pdly->D, pdly->t, pdly->w, &(pdly->p), pdly->a, pdly->b,
                        pdly->pflt->a, pdly->pflt->L, pdly->pflt->b,
                        pdly->pflt->w, x);
  case DLY_LINEAR:
  ...
  case DLY_PLAIN_4TAP:
    return ReverbSimple_multitap(pdly->D, pdly->t, pdly->t1, pdly->t2, pdly->t3,
                                 pdly->w, &pdly->p, pdly->a, pdly->b, x);
  case DLY_LOWPASS_4TAP:
    return DelayLowpass_multitap(pdly->D, pdly->t, pdly->t1, pdly->t2, pdly->t3,
...
inline int DLY_GetNextXfade(dly_t *pdly, int x) {
  ...
  case DLY_LINEAR:
    return DelayLinear_xfade(pdly->D, pdly->t, pdly->tnew, pdly->xf, pdly->w,
                             &pdly->p, x);
  case DLY_FLINEAR:
    return DelayLinear_lowpass_xfade(
        pdly->D, pdly->t, pdly->tnew, pdly->xf, pdly->w, &(pdly->p), pdly->a,
...
//PMAX) 		b - array of M gain values for plain reverb outputs (1, .9, .8, .7) 		xin
//- input value 		if fparallel, filters are built into delays, 		otherwise, filter
//is in feedback loop
...
prm_rng_t rva_rng[] = {
    ...
    {rva_height, -1000.0,
     1000.0}, // 0-1000.0 room height in feet - used instead of size if non-zero
    ...
    {rva_fbdepth, -1.0, 1.0}, // 0-1.0 material reflectivity - used as feedback
                              // param instead of decay if non-zero
    {rva_fbheight, -1.0,
     1.0}, // 0-1.0 material reflectivity - used as feedback param instead of
           // decay if non-zero if < 0, a predelay is allocated, then feedback
           // is -1*param given
...
void RVA_ConstructDelays(float *rgd, float *rgf, int m, int *D, int *a, int *b,
  ...
  for (i = 0; i < m; i++) {
    ...
    if (i < 3)
      ...
      dm = max((double)(RVA_MIN_SEPARATION * (i / 3)),
               ((i / 3) * ((float)d * 0.18)));
...
rva_t *RVA_Params(prc_t *pprc) {
  ...
  float width =
      fabs(pprc->prm[rva_width]); // 0-1000 controls size of 1/3 of delays -
                                  // used instead of size if non-zero
  float depth =
      fabs(pprc->prm[rva_depth]); // 0-1000 controls size of 1/3 of delays -
                                  // used instead of size if non-zero
  float height =
      fabs(pprc->prm[rva_height]); // 0-1000 controls size of 1/3 of delays -
                                   // used instead of size if non-zero
  ...
  float ftaps =
      pprc->prm[rva_iftaps]; // if > 0 increase reverb density using 3 extra
                             // taps d = (1.0 - ftaps * n) n = 0,1,2,3
  ...
  if (int(width) || int(height) || int(depth)) {
    ...
    for (int i = 0; i < m; i++) {
      ...
      if (i == 0) {
        ...
        a[i] = (int)min(0.999 * PMAX, (double)PMAX * DLY_NormalizeFeedback(
                                                         D[0], feedback, D[i]));
      }
...
void LFOWAV_Fill(int *w, int count, int type) {
  ...
  case LFO_SIN: // sine wav, all values 0 <= x <= LFOAMP, initial value = 0
    for (i = 0; i < count; i++) {
      x = (int)((float)(LFOAMP)*sinf((2.0 * M_PI_F * (float)i / (float)count) +
                                     (M_PI_F * 1.5)));
      w[i] = (x + LFOAMP) / 2;
    }
    break;
  ...
  case LFO_SQR: // square wave, 50% duty cycle, all values 0 <= x <= LFOAMP,
    ...
    break;
  case LFO_SAW: // forward saw wav, aall values 0 <= x <= LFOAMP, initial value
                // = 0
    for (i = 0; i < count; i++)
  ...
  case LFO_RND: // random wav, all values 0 <= x <= LFOAMP
    ...
    break;
  case LFO_LOG_IN: // logarithmic fade in, all values 0 <= x <= LFOAMP, initial
                   // value = 0
    for (i = 0; i < count; i++)
  ...
  case LFO_LOG_OUT: // logarithmic fade out, all values 0 <= x <= LFOAMP,
    ...
    break;
  case LFO_LIN_IN: // linear fade in, all values 0 <= x <= LFOAMP, initial value
                   // = 0
    for (i = 0; i < count; i++)
      w[i] = (int)((float)(LFOAMP) * (float)i / (float)(count));
    break;
...
lfo_t *LFO_Alloc(int wtype, float freqHz, bool foneshot, float gain) {
  ...
  for (i = 0; i < CLFO; i++)
    if (!lfos[i].fused) {
      ...
      plfo->fused = true;
      plfo->pdly =
          lfowavs[type].pdly; // pdly in lfo points to wav table data in lfowavs
      plfo->f = freqHz;
...
// [0...      tslice              ...D]
// input samples 0...D (D is NEWEST sample) [0...          ...n][m... tseg ...D]
// region to be cut or duplicated m...D

// [0...   [p..txf1..n][m... tseg ...D]
// fade in  region 1 txf1 p...n [0...          ...n][m..[q..txf2..D]
...
// ...n][m..[q...xf2..D][m... tdup ...D] [0... ...n][m..[.XFADE...n][m... tdup
// ...D]		final duplicated output buffer - resample at fstep
...
crs_t *CRS_Alloc(int lfotype, float fHz, float fdepth, float mix) {
  ...
  ramptime =
      (float)CRS_RAMPTIME / 1000.0; // # milliseconds to ramp between new values
...
amp_t *AMP_Alloc(float gain, float vthresh, float distmix, float vfeed,
  ...
  // 0.0, 1.0},			// how much amplitude changes (decreases) from
  // current value (0-1.0) modglide,	0.01, 100.0},		// glide time
  // between mapcur and ampnew in milliseconds
...
prm_rng_t amp_rng[] = {
    ...
    {amp_imodrate, 0.0, 200.0}, // frequency at which amplitude values change to
                                // new random value. 0 is no self-modulation
    {amp_imoddepth, 0.0,
...
amp_t *AMP_Params(prc_t *pprc) {
  ...
  if (pprc->prm[amp_imodrate] > 0.0) {
    ...
    modtime = 1.0 / max((double)pprc->prm[amp_imodrate],
...
struct pset_t {
  ...
  float db_min;  // if sndlvl of a new sound is < db_min, reduce mix_min/max by
                 // db_mixdrop
  float db_mixdrop; // reduce mix_min/max by n% if sndlvl of new sound less than
                    // db_min
  float
      duration; // if > 0, duration of preset in seconds (duration 0 = infinite)
...
//sounds pass through this dsp (such as low pass filter) 	dsp_player	- set
//this cvar to a preset # to cause all sounds to run through the effect (serial,
//in-line). 					use: player is deafened, player fires special weapon, player is hit
//by special weapon.
...
ConVar dsp_player(
    "dsp_player", "0",
    FCVAR_DEMO | FCVAR_SERVER_CAN_EXECUTE); // dsp on player - sound when player
                                            // hit by special device (1-2ch)
ConVar dsp_facingaway("dsp_facingaway", "0",
                      FCVAR_DEMO); // "30" sounds that face away from player
                                   // (weapons, voice) (1-4ch)
...
ConVar dsp_automatic("dsp_automatic", "0",
                     FCVAR_DEMO); // automatic room type detection. if non zero,
                                  // replaces dsp_room
...
ConVar dsp_off(
    "dsp_off", "0",
    FCVAR_CHEAT |
        FCVAR_ALLOWED_IN_COMPETITIVE); // set to 1 to disable all dsp processing
ConVar dsp_slow_cpu(
    "dsp_slow_cpu", "0",
    FCVAR_ARCHIVE |
        FCVAR_DEMO); // set to 1 if cpu bound - ie: does not process dsp_room fx
ConVar snd_profile(
    "snd_profile", "0",
...
inline void DSP_FreePrevPreset(dsp_t *pdsp) {
  // free previous presets if non-null - ie: rapid change of preset just kills
  // old without xfade
...
void DSP_SetPreset(int idsp, int ipsetnew) {
  ...
  if (idsp == idsp_room || idsp == idsp_automatic) {
    ...
    // dspmix param NOTE: so - no crossfade is needed between dspmix and dspmix
...
struct auto_params_t {
  ...
  float surface_refl[6]; // reflectivity for left,right,front,back,ceiling,floor
                         // surfaces 0.0 for open surface (sky or no hit)
...
// tile									0.3
// * 3.3 = 0.99 metal
// 0.25 * 3.3 = 0.83 concrete,rock,brick,glass,gravel		0.2  * 3.3 =
// 0.66 metal panel/vent, wood, water		0.1	 * 3.3 = 0.33
...
#define CDFRTEMPLATES                                                          \
  (sizeof(g_prc_dfr_auto) / sizeof(pset_t)) // number of diffusor templates
...
// both non-null, the channels map 1:1 through the processors.
...
// given ptr to null term. string, return integer or float value from
// g_dsp_stringmap
...
bool DSP_LoadPresetFile(void) {
  ...
  for (j = 0; j < cpresets; j++) {
    ...
    while (1) {
      ...
      cproc++;
      if (cproc > CPSET_PRCS)
        DevMsg("DSP PARSE ERROR!!! dsp_presets.txt: missing } or too many "
               "processors in preset #: %d \n",
               ipreset);
...
void CheckNewDspPresets(void) {
  ...
  // previous preset (ie; previous is null)

  if (iplayer == ipset_player_prev && !DSP_IsCrossfading(idsp_player)) {
    if (DSP_HasExpired(idsp_player)) {
      iplayer =
          DSP_OneShotPrevious(idsp_player); // preset has expired - revert to
                                            // previous preset before one-shot
  ...
  for (int i = SOUND_BUFFER_SPECIAL_START; i < g_paintBuffers.Count(); ++i) {
    ...
    if (pSpecialBuffer->nSpecialDSP != pSpecialBuffer->nPrevSpecialDSP &&
      ...
      DSP_SetPreset(pSpecialBuffer->idsp_specialdsp,
                    pSpecialBuffer->nSpecialDSP);
...
inline int S_Compress(int xin) {
  ...
  // threshold f(Cn) = 1				for C <= C0	// unity
  // gain below threshold Yn = f(Cn) * Xn
  // // compressor output
