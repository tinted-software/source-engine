...
// The outer code mixes in PAINTBUFFER_SIZE (# of samples) chunks (see
...
bool CAudioXAudio::Init(void) {
  ...
  // create the mastering voice, this will upsample to the devices target hw
  // output rate
  ...
  m_pSourceVoice = NULL;
  hr = m_pXAudio2->CreateSourceVoice(&m_pSourceVoice, &waveFormatEx, 0,
  ...
  m_bufferSizeBytes =
      XAUDIO2_BUFFER_SAMPLES * (m_deviceSampleBits / 8) * m_deviceChannels;
  ...
  m_deviceSampleCount =
      MAX_XAUDIO2_BUFFERS * (m_bufferSizeBytes / (DeviceSampleBytes()));

  // NOTE: This really shouldn't be tied to the # of bufferable samples.
  // This just needs to be large enough so that it doesn't fake out the sampling
  // in GetSoundTime().  Basically GetSoundTime() assumes a cyclical time stamp
  // and finds wraparound cases but that means it needs to get called much more
  ...
  // not really part of XAudio2, but mixer xma lacks one-time init, so doing it
  // here
...
//-----------------------------------------------------------------------------
int CAudioXAudio::PaintBegin(float mixAheadTime, int soundtime,
                             int paintedtime) {
...
//-----------------------------------------------------------------------------
int CAudioXAudio::TransferStereo(const portable_samplepair_t *pFrontBuffer,
                                 int paintedTime, int endTime,
                                 char *pOutputBuffer) {
  int linearCount;
  int i;
...
//-----------------------------------------------------------------------------
int CAudioXAudio::TransferSurroundInterleaved(
...
void CAudioXAudio::TransferSamples(int endTime) {
  ...
  if (!m_bSurround) {
    pBuffer->AudioBytes = TransferStereo(PAINTBUFFER, g_paintedtime, endTime,
                                         (char *)pBuffer->pAudioData);
  } else {
    pBuffer->AudioBytes = TransferSurroundInterleaved(
