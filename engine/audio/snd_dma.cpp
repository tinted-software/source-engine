...
int MXR_GetMixGroupListFromDirName(const char *pDirname, byte *pList,
                                   int listMax);
...
const float g_DashboardMusicFadeRate =
    0.5f; // Fades one half full-scale volume per second (two seconds for
          // complete fadeout)
...
static CUtlMap<FileNameHandle_t, SfxDictEntry>
    s_Sounds(0, 0, DefLessFunc(FileNameHandle_t));
...
const char *CSfxTable::GetFileName() {
  if (IsX360() && m_bUseErrorFilename) {
    // Redirecting error sounds to a valid empty wave, prevents a bad loading
    // retry pattern during gameplay which may event sounds skipped by preload,
...
static ConCommand
    stopsound("stopsound", S_StopAllSoundsC, 0,
              FCVAR_CHEAT); // Marked cheat because it gives an advantage to
                            // players minimising ambient noise.
...
void S_Init(void) {
  ...
  if (IsX360()) {
    g_pQueuedLoader->InstallLoader(RESOURCEPRELOAD_SOUND,
  ...
  if (g_pVideo != NULL) {
    if (g_pVideo->SoundDeviceCommand(VideoSoundDeviceOperation::HOOK_X_AUDIO,
                                     NULL) != VideoResult::SUCCESS) {
      Assert(0);
    }
  }
#endif
  ...
  if (!pSfx->pSource) {
    if (IsX360()) {
      if (SND_IsInGame() && !g_pQueuedLoader->IsMapLoading()) {
        // sound should be present (due to reslists), but NOT allowing a load
        // hitch during gameplay loading a sound during gameplay is a bad
        // experience, causes a very expensive sync i/o to fetch the header and
        // in the case of a memory wave, the actual audio data
        bool bFound = false;
        if (!pSfx->m_bIsLateLoad) {
          if (pSfx->getname() != PSkipSoundChars(pSfx->getname())) {
            ...
            FileNameHandle_t fnHandle =
                g_pFileSystem->FindOrAddFileName(pSfx->GetFileName());
            ...
            if (i != s_Sounds.InvalidIndex()) {
              ...
              if (pOtherSource && pOtherSource->IsCached()) {
                // Can safely let the "load" continue because the headers are
                // expected to be in the preload that are now persisted and the
                // wave data cache will find an existing audio buffer match, so
                // no sync i/o should occur from either.
                bFound = true;
              }
            }
          }
...
void S_ReloadFilesInList(IFileList *pFilesToReload) {
  ...
  wavedatacache->Flush();
  audiosourcecache
      ->ForceRecheckDiskInfo(); // Force all cached audio data to recheck
                                // size/date info next time it's accessed.
...
channel_t *SND_StealDynamicChannel(SoundSource soundsource, int entchannel,
      ...
      if (entchannel != CHAN_AUTO) {
        ...
        if (checkChannel == -1) {
          if (ch->entchannel != CHAN_STREAM && ch->entchannel != CHAN_VOICE &&
              ch->entchannel != CHAN_VOICE2) {
            checkChannel = ch->entchannel;
          }
        }
        if (ch->soundsource == soundsource && (soundsource != -1) &&
            ch->entchannel == checkChannel) {
          // we found an exact match for this entity and this channel, but the
          // sound we want to play is considered low priority so instead of
          // stomping this entry pretend we couldn't find a free slot to play
          // and let the existing sound keep going
  ...
  if (nExactCount > 0) {
    ...
    // otherwise mark all others to be freed and use the closest one to being
    // done
    if (nExactCount > 1) {
      return pReturn;
    }
...
channel_t *SND_PickDynamicChannel(SoundSource soundsource, int entchannel,
  ...
  channel_t *pChannel = SND_StealDynamicChannel(
      soundsource, entchannel, origin, sfx, flDelay, bDoNotOverwriteExisting);
  if (!pChannel)
    return NULL;
...
void S_SpatializeChannel(int pVolume[CCHANVOLUMES / 2], int master_vol,
...
ConVar dsp_dist_min(
    "dsp_dist_min", "0.0",
    FCVAR_DEMO | FCVAR_CHEAT); // range at which sounds are mixed at dsp_mix_min
ConVar dsp_dist_max(
    "dsp_dist_max", "1440.0",
...
ConVar dsp_db_min("dsp_db_min", "80",
                  FCVAR_DEMO); // sounds with sndlvl below this get
                               // dsp_db_mixdrop % less dsp mix
...
float SND_GetDistanceMix(channel_t *pchannel, int idist) {
  ...
  dist = clamp(dist, (float)DVAR_DIST_MIN, (float)DVAR_DIST_MAX) -
         (float)DVAR_DIST_MIN;
...
ConVar
    snd_obscured_gain_db("snd_obscured_gain_dB", "-2.70",
  ...
  if (tr.DidHit() && tr.fraction < 0.99) {
    ...
    for (count = 0, i = 0; i < 4; i++) {
      ...
      if (tr.DidHit() && tr.fraction < 0.99 && !tr.startsolid) {
        count++; // skip first obscured point: at least 2 points + center should
                 // be obscured to hear db loss
        if (count > 1)
        ...
        if (chan & 0x1) {
          ...
          /* UNDONE: not needed, now that this applies only to dsp 'room' buffer
             ...
             average distance to walls
                                                  ...
                                                  float dmin = (vl + vr) / 2.0;
             // average distance to walls float d = vl - vr;
             // l/r separation

                                                  // if separation is less than
             average, increase min
...
void DAS_StoreNode(das_room_t *proom, int dsp_preset) {
  ...
  if (!proom->bskyabove) {
    ...
    g_das_nodes[i].range_max =
        fpmin((int)DAS_DIST_MAX, min(proom->width_max * 5, proom->length_max));
  ...
  for (i = 0; i < DAS_CNODES; i++) {
    if (g_das_nodes[i].fused && g_das_nodes[i].fseesplayer) {
      ...
      /*
                              ...
                              {
                                      if ( g_das_nodes[i].room.bskyabove ==
         foutside )
                                      {
                                              dist_min = g_das_nodes[i].dist;
                                              dsp_preset =
         g_das_nodes[i].dsp_preset;

                                              // save pointer to last node that
         saw player
...
void DAS_SetTraceHeight(das_room_t *proom, trace_t *ptrU, trace_t *ptrD) {
  ...
  if (h > low_ceiling && h <= wall_height) {
    ...
    if (h > door_height)
      proom->vplayer.z =
          fpmin(ptrD->endpos.z, ptrD->startpos.z) + door_height + 1;
    else
    ...
    proom->vplayer.z =
        fpmin(ptrD->endpos.z, ptrD->startpos.z) + wall_height + 1;
  } else {
...
bool DAS_UpdateRoomSize(das_room_t *proom) {
  ...
  if (das_debug.GetInt() > 2) {
    ...
    if (DAS_HitSolid(&tr))
      ...
      DAS_DebugDrawTrace(&tr, 255, 0, 0, 10,
                         DAS_ScanningForHeight(proom) +
                             3); // red lines if sky hit or no hit
  }
...
void DAS_SetDiffusion(das_room_t *proom) {
  ...
  SpatialPartitionListMask_t mask = PARTITION_CLIENT_SOLID_EDICTS;	// count
  only solid ents in room int count; float vol; float volroom; float dfn;

  enumerator.Reset();

  SpatialPartition()->EnumerateElementsInBox(mask, proom->room_mins,
  proom->room_maxs, true, &enumerator );
  ...
  volroom = (proom->room_maxs.x - proom->room_mins.x) * (proom->room_maxs.y -
  proom->room_mins.y) * (proom->room_maxs.z - proom->room_mins.z); volroom =
  fabs(volroom);
...
void DAS_DisplayRoomDEBUG(das_room_t *proom, bool fnew, float preset) {
  ...
  if (fnew)
    DevMsg("NEW DSP NODE: size:(%.0f,%.0f) height:(%.0f) dif %.4f : refl %.4f "
           ": cobj: %.0f : sky %.0f \n",
  ...
  if (preset >= 0.0) {
    ...
    DevMsg("DSP PRESET: %.0f size:(%.0f,%.0f) height:(%.0f) dif %.4f : refl "
           "%.4f : cobj: %.0f : sky %.0f \n",
...
void RemapPlayerOrMusicVols(channel_t *ch, int volumes[CCHANVOLUMES / 2],
                    ...
                    // ch->wavtype != CHAR_DISTVARIANT) )
  {
    ...
    // = {0.29, 0.29, 0.09, 0.09, 0.63};	// FL, FR, RL, RR, FC - 5
    // channel (stereo source) volume distribution

    float vol_dist5[] = {0.30, 0.30, 0.09, 0.09,
                         0.59}; // FL, FR, RL, RR, FC - 5 channel (mono source)
                                // volume distribution
    ...
    float vol_dist4[] = {0.50, 0.50, 0.15, 0.15,
                         0.00}; // FL, FR, RL, RR, 0  - 4 channel (mono source)
                                // volume distribution
...
void SND_Spatialize(channel_t *ch) {
  ...
  if (IsSoundSourceLocalPlayer(ch->soundsource) &&
...
int S_StartDynamicSound(StartSoundParams_t &params) {
  ...
  target_chan = SND_PickDynamicChannel(
      params.soundsource, params.entchannel, params.origin, params.pSfx,
      params.delay,
      (params.flags & SND_DO_NOT_OVERWRITE_EXISTING_ON_CHANNEL) != 0);
  if (!target_chan)
    return 0;
  ...
  if (bIsSentence) {
    ...
    if (!pSource && !params.pSfx->m_bIsLateLoad) {
      Warning("Failed to load sound \"%s\", file probably missing from "
              "disk/repository\n",
              sndname);
    }
  }
  ...
  if (nSndShowStart > 0 && nSndShowStart < 7 && nSndShowStart != 4) {
    ...
    DevMsg("DynamicSound %s : src %d : channel %d : %d dB : vol %.2f : time "
           "%.3f\n",
    ...
    if (nSndShowStart == 2 || nSndShowStart == 5)
      DevMsg("\t dspmix %1.2f : distmix %1.2f : dspface %1.2f : lvol %1.2f : "
             "cvol %1.2f : rvol %1.2f : rlvol %1.2f : rrvol %1.2f\n",
             pTargetChan->dspmix, pTargetChan->distmix, pTargetChan->dspface,
             pTargetChan->fvolume[IFRONT_LEFT],
             pTargetChan->fvolume[IFRONT_CENTER],
             pTargetChan->fvolume[IFRONT_RIGHT],
             pTargetChan->fvolume[IREAR_LEFT],
             pTargetChan->fvolume[IREAR_RIGHT]);
    if (nSndShowStart == 3)
      DevMsg("\t x: %4f y: %4f z: %4f\n", pTargetChan->origin.x,
             pTargetChan->origin.y, pTargetChan->origin.z);
  ...
  if (params.delay != 0.0f) {
    ...
    // delay count is computed at the sampling rate of the source because the
    // output rate will match the source rate when the sound is mixed
...
int S_StartStaticSound(StartSoundParams_t &params) {
  ...
  */
  ...
  if (nSndShowStart > 0 && nSndShowStart < 7 && nSndShowStart != 4) {
    DevMsg("StaticSound %s : src %d : channel %d : %d dB : vol %.2f : radius "
           "%.0f : time %.3f\n",
           sndname, params.soundsource, params.entchannel, params.soundlevel,
...
#ifdef STAGING_ONLY
  if (snd_filter.GetString()[0] &&
      !Q_stristr(params.pSfx->getname(), snd_filter.GetString())) {
    return 0;
  }
#endif // STAGING_ONLY
...
ConVar snd_spatialize_roundrobin(
    "snd_spatialize_roundrobin", "0", FCVAR_ALLOWED_IN_COMPETITIVE,
    "Lowend optimization: if nonzero, spatialize only a fraction of sound "
    "channels each frame. 1/2^x of channels will be spatialized per frame.");
/*
============
S_Update
...
void S_Update(const AudioState_t *pAudioState) {
  ...
  if (snd_show.GetInt()) {
    ...
    for (int i = 0; i < activeChannels.Count(); i++) {
      ...
      float timeleft =
          (float)sampleCount / (float)channel->sfx->pSource->SampleRate();
      ...
      if (snd_surround.GetInt() < 4) {
        Con_NXPrintf(&np,
                     ...
                     total + 1, (int)channel->fvolume[IFRONT_LEFT],
                     (int)channel->fvolume[IFRONT_RIGHT], channel->master_vol,
                     channel->soundsource, (int)channel->origin[0],
                     (int)channel->origin[1], (int)channel->origin[2], timeleft,
        ...
        Con_NXPrintf(&np,
                     ...
                     (int)channel->fvolume[IREAR_RIGHT], channel->master_vol,
                     channel->soundsource, (int)channel->origin[0],
                     (int)channel->origin[1], (int)channel->origin[2], timeleft,
                     bLooping, channel->sfx->getname());
      }
      ...
      if (snd_visualize.GetInt()) {
        CDebugOverlay::AddTextOverlay(channel->origin, 0.05f,
                                      channel->sfx->getname());
      }
...
void GetSoundTime(void) {
  ...
  if (cl_movieinfo.IsRecording() || IsReplayRendering()) {
    // when recording a replay, we look at the record frame rate, not the engine
    // frame rate
    ...
    if (IsReplayRendering()) {
      ...
      if (pMovieRenderer && pMovieRenderer->IsAudioSyncFrame()) {
        ...
        if (s_lastsoundtime != t) {
          ...
          float fSamples = frameTime * (float)g_AudioDevice->DeviceDmaSpeed() +
                           g_ReplaySoundTimeFracAccumulator;
...
static void S_PlayDelay(const CCommand &args) {
  if (args.ArgC() != 3) {
    Msg("Usage:  sndplaydelay delay_in_sec (negative to skip ahead) "
        "soundname\n");
    return;
  }
...
static ConCommand sndplaydelay(
    "sndplaydelay", S_PlayDelay,
...
struct classlistelem_t {
  char szclassname[CMXRNAMEMAX]; // name of entities' class, such as CAI_BaseNPC
                                 // or CHL2_Player
};
...
struct grouprule_t {
  ...
  int priority; // 0..100 higher priority sound groups duck all lower pri groups
                // if enabled
  ...
  int causes_ducking;    // if 1, sound group ducks other 'is_ducked' sounds of
                         // lower priority
  float duck_target_pct; // if sound group is ducked, target percent of original
                         // volume
...
int g_mapMixgroupidToGrouprulesid
    [CMXRGROUPMAX]; // map mixgroupid (one per unique group name)
...
int MXR_AddClassname(const char *pName) {
  ...
  Q_memcpy(g_groupclasslist[g_cgroupclass].szclassname, pName,
           min((size_t)CMXRNAMEMAX - 1, strlen(pName)));
  g_cgroupclass++;
  ...
  while (1) {
    ...
    if (com_token[0])
      Q_memcpy(pgroup->szmixgroup, com_token,
               min((size_t)CMXRNAMEMAX - 1, strlen(com_token)));
