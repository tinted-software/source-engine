...
typedef HRESULT(WINAPI *DirectSoundCaptureCreateFn)(
    const GUID FAR *lpGUID, LPDIRECTSOUNDCAPTURE *pCapture,
    LPUNKNOWN pUnkOuter);
...
bool VoiceRecord_DSound::Init(int sampleRate) {
  ...
  createFn = (DirectSoundCaptureCreateFn)GetProcAddress(
      m_hInstDS, "DirectSoundCaptureCreate");
  if (!createFn)
    goto HandleError;
