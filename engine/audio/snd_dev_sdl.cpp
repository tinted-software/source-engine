...
extern void S_SpatializeChannel(/*int nSlot,*/ int volume[6], int master_vol,
...
//-----------------------------------------------------------------------------
//
// NOTE: This only allows 16-bit, stereo wave out  (!!! FIXME: but SDL
// supports 7.1, etc, too!)
//
//-----------------------------------------------------------------------------
...
public:
  ...
  void SpatializeChannel(int nSlot, int volume[CCHANVOLUMES / 2],
                         int master_vol, const Vector &sourceDir, float gain,
                         float mono);
...
private:
  ...
  static void SDLCALL AudioCallbackEntry(void *userdata, Uint8 *stream,
                                         int len);
...
//-----------------------------------------------------------------------------
CAudioDeviceSDLAudio::CAudioDeviceSDLAudio() { m_devId = 0; }

//-----------------------------------------------------------------------------
// Destructor. Make sure our global pointer gets set to NULL.
//-----------------------------------------------------------------------------
CAudioDeviceSDLAudio::~CAudioDeviceSDLAudio() { g_wave = NULL; }
...
bool CAudioDeviceSDLAudio::Init(void) {
  ...
  //	Audio_CreateSDLAudioDevice() multiple times. CloseWaveOut() will free
  //the
...
void CAudioDeviceSDLAudio::Shutdown(void) { CloseWaveOut(); }
...
// #define SDLAUDIO_FAIL(fnstr) do { DevWarning(fnstr " failed");
// CloseWaveOut(); return; } while (false) #define SDLAUDIO_FAIL(fnstr) do {
...
#undef SDLAUDIO_FAIL
  ...
  if (g_pVideo != NULL) {
    g_pVideo->SoundDeviceCommand(VideoSoundDeviceOperation::SET_SDL_PARAMS,
                                 NULL, (void *)&obtained);
  }
#endif
}
...
void CAudioDeviceSDLAudio::AudioCallbackEntry(void *userdata, Uint8 *stream,
                                              int len) {
...
void CAudioDeviceSDLAudio::AudioCallback(Uint8 *stream, int len) {
  ...
  while (len > 0) {
    ...
    m_readPos =
        len ? 0
            : (m_readPos + writeLen); // if still bytes to write to stream,
                                      // we're rolling around the ring buffer.
  }

#if defined(BINK_VIDEO) && defined(LINUX)
  // Mix in Bink movie audio if that stuff is playing.
  if (g_pVideo != NULL) {
    g_pVideo->SoundDeviceCommand(VideoSoundDeviceOperation::SDLMIXER_CALLBACK,
                                 (void *)stream_orig, (void *)&totalWriteable);
  }
#endif
...
bool CAudioDeviceSDLAudio::IsActive(void) { return (m_pauseCount == 0); }

float CAudioDeviceSDLAudio::MixDryVolume(void) { return 0; }

bool CAudioDeviceSDLAudio::Should3DMix(void) { return false; }
...
void CAudioDeviceSDLAudio::Mix8Mono(channel_t *pChannel, char *pData,
  ...
  Mix8MonoWavtype(pChannel, ppaint->pbuf + outputOffset, volume, (byte *)pData,
                  inputOffset, rateScaleFix, outCount);
}

void CAudioDeviceSDLAudio::Mix8Stereo(channel_t *pChannel, char *pData,
                                      int outputOffset, int inputOffset,
                                      fixedint rateScaleFix, int outCount,
                                      int timecompress) {
  int volume[CCHANVOLUMES];
  ...
  Mix8StereoWavtype(pChannel, ppaint->pbuf + outputOffset, volume,
                    (byte *)pData, inputOffset, rateScaleFix, outCount);
}

void CAudioDeviceSDLAudio::Mix16Mono(channel_t *pChannel, short *pData,
                                     int outputOffset, int inputOffset,
                                     fixedint rateScaleFix, int outCount,
                                     int timecompress) {
  int volume[CCHANVOLUMES];
...
void CAudioDeviceSDLAudio::Mix16Stereo(channel_t *pChannel, short *pData,
                                       int outputOffset, int inputOffset,
                                       fixedint rateScaleFix, int outCount,
                                       int timecompress) {
  int volume[CCHANVOLUMES];
...
void CAudioDeviceSDLAudio::ChannelReset(int entnum, int channelIndex,
                                        float distanceMod) {}
...
void CAudioDeviceSDLAudio::SpatializeChannel(int nSlot,
                                             int volume[CCHANVOLUMES / 2],
                                             int master_vol,
...
void CAudioDeviceSDLAudio::ApplyDSPEffects(int idsp,
