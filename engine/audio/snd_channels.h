...
#define IFRONT_CENTERD0                                                        \
  11 // dummy slot - center channel is mono, but mixers reference volume[1] slot
...
struct channel_t {
  ...
  float
      fvolume_inc[CCHANVOLUMES]; // volume increment, per frame, moves volume[i]
                                 // to vol_target[i] (per spatialization)
  ...
  int speakerentity; // if a sound is being played through a speaker entity
                     ...
                     //  entity upon which to show the lips moving, if the sound
                     //  has sentence data
  ...
  union {
    ...
    struct {
      ...
      bool delayed_start : 1; // If true, sound had a delay and so same sound on
                              // same channel won't channel steal from it
      ...
      bool m_bIsFreeingChannel : 1; // true when inside S_FreeChannel - prevents
                                    // reentrance
