...
// Dev builds will be missing soundcaches and hitch sometimes, we only care if
// its being properly launched from steam where sound caches should be complete.
ConVar snd_async_spew_blocking(
    ...
    "( 0=Off, 1=With -steam only, 2=Always");
ConVar snd_async_spew("snd_async_spew", "0", 0,
...
ConVar
    snd_async_stream_spew("snd_async_stream_spew", "0", 0,
...
CAsyncWaveData::CAsyncWaveData()
    : m_nDataSize(0), m_nReadSize(0), m_pvData(0), m_pAlloc(0),
      m_hBuffer(INVALID_BUFFER_HANDLE), m_nBufferBytes(0),
      m_hAsyncControl(NULL), m_bLoaded(false), m_bMissing(false), m_start(0.0),
      m_arrival(0.0), m_bPostProcessed(false), m_hFileNameHandle(0) {}
...
//-----------------------------------------------------------------------------
CAsyncWaveData *
CAsyncWaveData::CreateResource(const asyncwaveparams_t &params) {
...
bool CAsyncWaveData::BlockingGetDataPointer(void **ppData) {
  ...
  if (!m_bLoaded) {
    ...
    if (SndAsyncSpewBlocking()) {
      ...
      g_pFileSystem->AsyncFinish(m_hAsyncControl, true);
      float ed = (float)Plat_FloatTime();
      Warning("%f BlockingGetDataPointer:  Async I/O Force %s (%8.2f msec / "
              "%8.2f msec total )\n",
              realtime, GetFileName(), 1000.0f * (float)(ed - st),
              1000.0f * (float)(m_arrival - m_start));
    } else {
...
void CAsyncWaveData::StartAsyncLoading(const asyncwaveparams_t &params) {
  ...
  if (IsX360() && (g_pFileSystem->GetDVDMode() == DVDMODE_STRICT)) {
    ...
    // resolve to absolute name now, where path can be filtered to just the zips
    // (fast find, no real i/o) otherwise the dvd will do a costly seek for each
    // zip miss due to search path fall through
    PathTypeQuery_t pathType;
    if (!g_pFileSystem->RelativePathToFullPath(
            m_async.pszFilename, m_async.pszPathID, szFullName,
...
CAsyncWavDataCache::CAsyncWavDataCache()
    : m_CacheHandles(0, 0, CacheHandleLessFunc),
      m_BufferList(0, 0, BufferHandleLessFunc), m_bInitialized(false),
...
bool CAsyncWavDataCache::Init(unsigned int memSize) {
  ...
  Msg("WARNING CAsyncWavDataCache::Init() forcing small memory cache size: "
      "%u\n",
      memSize);
#endif
  ...
  *pData = NULL;
  ...
  if (data->m_nDataSize != 0) {
    if (datasize != data->m_nDataSize) {
      // We've had issues where we are called with datasize larger than what we
      // read on disk.
      //  Ie: datasize is 277,180, data->m_nDataSize is 263,168
      // This can happen due to a corrupted audio cache, but it's more likely
      // that somehow
      //  we wound up reading the cache data from one language and the file from
      //  another.
      DevMsg("Cached datasize != sound datasize %d - %d.\n", datasize,
             data->m_nDataSize);
#ifdef STAGING_ONLY
      // Adding a STAGING_ONLY debugger break to try and help track this down.
      // Hopefully we'll
      //  get this crash internally with full debug information instead of just
      //  minidump files.
      DebuggerBreak();
#endif
...
void CAsyncWavDataCache::SpewMemoryUsage(int level) {
  ...
  if (IsX360()) {
    ...
    if (level >= 1) {
      ...
      for (h = m_BufferList.FirstInorder(); h != m_BufferList.InvalidIndex();
           h = m_BufferList.NextInorder(h)) {
        ...
        CacheLockMutex();
        if (pData) {
          ConMsg("Start:%7d Length:%7d Lock:%3d %s\n", pData->m_async.nOffset,
                 pData->m_nDataSize, lockCount, pData->GetFileName());
        }
        CacheUnlockMutex();
      }
    }
    ...
    ConMsg("CAsyncWavDataCache: %.2f MB used of %.2f MB, %.2f%% of capacity",
           (float)bytesUsed / (1024.0f * 1024.0f),
           (float)bytesTotal / (1024.0f * 1024.0f), percent);
  }
}
...
//IWaveData 			abstracts the data access.  The mixer abstracts data encoding/format
//-----------------------------------------------------------------------------
...
CWaveDataStreamAsync::CWaveDataStreamAsync(CAudioSource &source,
                                           ...
                                           int startOffset)
    : m_source(source), m_dataStart(fileStart), m_dataSize(fileSize),
      m_pStreamSource(pStreamSource), m_bValid(false), m_hCache(0),
      m_hStream(INVALID_STREAM_HANDLE), m_hFileName(0), m_pSfx(sfx) {
  ...
  if (IsX360()) {
    ...
    if (!Q_strnicmp(pFileName, "music", 5) &&
        (pFileName[5] == '\\' || pFileName[5] == '/')) {
      // music discards and cycles its buffers
      flags |= STREAMED_SINGLEPLAY;
    } else if (!Q_strnicmp(pFileName, "vo", 2) &&
               (pFileName[2] == '\\' || pFileName[2] == '/') &&
               !source.IsSentenceWord()) {
      // vo discards and cycles its buffers, except for sentence sources, which
      // do recur
      flags |= STREAMED_SINGLEPLAY;
    }
    ...
    if (source.Format() == WAVE_FORMAT_XMA) {
      ...
      // the buffer must be large enough to cover worst case delivery i/o
      // latency the xma mixer expects quantum xma blocks
    ...
    if (m_dataSize <= STREAM_BUFFER_DATASIZE ||
        m_dataSize <= numBuffers * bufferSize) {
    ...
    // ensures that buffers can be filled and then consumed/requeued at the same
    // time
    ...
    if (source.IsLooped()) {
      int loopBlock;
      loopStart = m_pStreamSource->GetLoopingInfo(&loopBlock, NULL, NULL) *
                  m_sampleSize;
      if (source.Format() == WAVE_FORMAT_XMA) {
        // xma works in blocks, mixer handles inter-block accurate loop
        // positioning block streaming will cycle from the block where the loop
        // occurs
    ...
    m_hStream = wavedatacache->OpenStreamedLoad(
        pFileName, m_dataSize, m_dataStart, startOffset, loopStart, bufferSize,
        numBuffers, flags);
  }
    ...
    if (IsX360()) {
      if (m_hStream != INVALID_STREAM_HANDLE) {
        ...
        m_bufferCount = wavedatacache->CopyStreamedDataIntoMemory(
            m_hStream, m_buffer, m_bufferSize * m_sampleSize, seekpos,
      ...
      // do any conversion now the source needs (mixer will decode/decompress)
      // on this buffer
      m_pStreamSource->UpdateSamples(m_buffer, m_bufferCount);
    }
  }
