...
static void AudioCallback(void *pContext, AudioQueueRef pQueue,
...
void OnSndSurroundCvarChanged2(IConVar *pVar, const char *pOldString,
                               float flOldValue);
void OnSndSurroundLegacyChanged2(IConVar *pVar, const char *pOldString,
                                 float flOldValue);
...
void AudioQueueIsRunningCallback(void *inClientData, AudioQueueRef inAQ,
  ...
  UInt32 running = 0;
  UInt32 size;
  OSStatus err = AudioQueueGetProperty(inAQ, kAudioQueueProperty_IsRunning,
                                       &running, &size);
...
void CAudioDeviceAudioQueue::OpenWaveOut(void) {
  ...
  OSStatus err = AudioQueueNewOutput(&m_DataFormat, AudioCallback, this, NULL,
                                     NULL, 0, &m_Queue);
  if (err != noErr) {
  ...
  err = AudioQueueAddPropertyListener(m_Queue, kAudioQueueProperty_IsRunning,
                                      AudioQueueIsRunningCallback, this);
  if (err != noErr) {
    DevMsg("Failed to create AudioQueue output %d\n", (int)err);
    m_bFailed = true;
    return;
...
void CAudioDeviceAudioQueue::CloseWaveOut(void) {
  if (ValidWaveOut()) {
    ...
    AudioQueueRemovePropertyListener(m_Queue, kAudioQueueProperty_IsRunning,
                                     AudioQueueIsRunningCallback, this);
...
void CAudioDeviceAudioQueue::PaintEnd(void) {
  ...
  while (((m_buffersSent - m_buffersCompleted) >> SAMPLE_16BIT_SHIFT) <
    ...
    Q_memcpy(m_Buffers[iBuf]->mAudioData,
             (char *)m_sndBuffers + iBuf * BUFFER_SIZE, BUFFER_SIZE);
...
float CAudioDeviceAudioQueue::MixDryVolume(void) { return 0; }

bool CAudioDeviceAudioQueue::Should3DMix(void) { return false; }
...
void CAudioDeviceAudioQueue::UpdateListener(const Vector &position,
...
bool CAudioDeviceAudioQueue::BIsPlaying() {
  ...
  OSStatus result = AudioQueueGetProperty(
      m_Queue, kAudioQueueProperty_IsRunning, &isRunning, &propSize);
  return isRunning != 0;
}
...
void CAudioDeviceAudioQueue::Mix8Mono(channel_t *pChannel, char *pData,
...
void CAudioDeviceAudioQueue::Mix8Stereo(channel_t *pChannel, char *pData,
                                        int outputOffset, int inputOffset,
                                        fixedint rateScaleFix, int outCount,
                                        int timecompress) {
  int volume[CCHANVOLUMES];
...
void CAudioDeviceAudioQueue::Mix16Mono(channel_t *pChannel, short *pData,
                                       int outputOffset, int inputOffset,
                                       fixedint rateScaleFix, int outCount,
                                       int timecompress) {
  int volume[CCHANVOLUMES];
...
void CAudioDeviceAudioQueue::Mix16Stereo(channel_t *pChannel, short *pData,
                                         int outputOffset, int inputOffset,
                                         fixedint rateScaleFix, int outCount,
                                         int timecompress) {
  int volume[CCHANVOLUMES];
...
void CAudioDeviceAudioQueue::ChannelReset(int entnum, int channelIndex,
                                          float distanceMod) {}
...
void CAudioDeviceAudioQueue::SpatializeChannel(int volume[CCHANVOLUMES / 2],
                                               int master_vol,
...
void CAudioDeviceAudioQueue::ApplyDSPEffects(int idsp,
...
static uint32 GetSpeakerConfigForSurroundMode(int surroundMode,
                                              const char **pConfigDesc) {
...
void OnSndSurroundCvarChanged2(IConVar *pVar, const char *pOldString,
  ...
  ConVarRef var(pVar);
  newSpeakerConfig =
      GetSpeakerConfigForSurroundMode(var.GetInt(), &speakerConfigDesc);
