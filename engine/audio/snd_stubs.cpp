...
#if !defined(DEDICATED) && (defined(OSX) || defined(_WIN32)) &&                \
    !defined(NO_STEAM)
...
bool CEngineVoiceSteam::IsLocalPlayerTalking(int iController) {
  uint32 nBytes = 0;
  EVoiceResult res =
      Steam3Client().SteamUser()->GetAvailableVoice(&nBytes, NULL, 0);
  switch (res) {
...
void CEngineVoiceSteam::RemovePlayerFromVoiceList(XUID xPlayer,
                                                  int iController) {
  if (!xPlayer && iController >= 0 && iController < XUSER_MAX_COUNT) {
    // Remove local player
...
void CEngineVoiceSteam::GetRemoteTalkers(int *pNumTalkers,
                                         XUID *pRemoteTalkers) {
  if (pNumTalkers)
...
void CEngineVoiceSteam::GetVoiceData(int iController,
                                     const byte **ppvVoiceDataBuffer,
                                     unsigned int *pnumVoiceDataBytes) {
  const int size = ARRAYSIZE(m_pbVoiceData) / XUSER_MAX_COUNT;
  byte *pbVoiceData =
      m_pbVoiceData + iController * ARRAYSIZE(m_pbVoiceData) / XUSER_MAX_COUNT;
  *ppvVoiceDataBuffer = pbVoiceData;

  EVoiceResult res = Steam3Client().SteamUser()->GetVoice(
      true, pbVoiceData, size, pnumVoiceDataBytes, false, NULL, 0, NULL, 0);
  switch (res) {
  case k_EVoiceResultNoData:
  case k_EVoiceResultOK:
    return;
...
void CEngineVoiceSteam::SetPlaybackPriority(XUID remoteTalker, int iController,
                                            int iAllowPlayback) {
  ;
}

void CEngineVoiceSteam::PlayIncomingVoiceData(
    XUID xuid, const byte *pbData, unsigned int dwDataSize,
    const bool *bAudiblePlayers /* = NULL */) {
  for (DWORD dwSlot = 0; dwSlot < XBX_GetNumGameUsers(); ++dwSlot) {
    IPlayerLocal *pPlayer =
        g_pMatchFramework->GetMatchSystem()->GetPlayerManager()->GetLocalPlayer(
            dwSlot);
  ...
  if (!g_pMatchFramework->GetMatchSystem()->GetMatchVoice()->CanPlaybackTalker(
          xuid))
    return;
  ...
  EVoiceResult res = Steam3Client().SteamUser()->DecompressVoice(
      const_cast<byte *>(pbData), dwDataSize, pbUncompressedVoice,
  ...
  // sound engine).
  if (nChannel != VOICE_CHANNEL_ERROR) {
    Voice_AddIncomingData(nChannel, pbUncompressedVoice, numUncompressedBytes,
                          0, false);
  }
}
