...
CMixerControls::CMixerControls() {
  ...
  theStatus = AudioDeviceGetPropertyInfo(m_theDefaultDeviceID, 0, TRUE,
                                         kAudioDevicePropertyDeviceName,
                                         &outSize, NULL);
  if (theStatus == noErr) {
    m_szMixerName = (char *)malloc(outSize * sizeof(char));

    theStatus = AudioDeviceGetProperty(m_theDefaultDeviceID, 0, TRUE,
                                       kAudioDevicePropertyDeviceName, &outSize,
                                       m_szMixerName);
...
const char *CMixerControls::GetMixerName() { return m_szMixerName; }
...
bool CMixerControls::GetValue_Float(Control iControl, float &value) {
  ...
  case MicVolume: {
    ...
    for (uint iChannel = 0; iChannel < 3; iChannel++) {
      // scan the channel list until you find a channel set to non-zero, then
      // use that
      Float32 theVolume = 0;
      ...
      theError = AudioObjectGetPropertyData(m_theDefaultDeviceID, &theAddress,
                                            0, NULL, &theSize, &theVolume);
      value = theVolume;
  ...
  case MicMute:
    ...
    // sound card echoes whatever you say into the mic.
    {
      ...
      AudioObjectPropertyAddress theAddress = {
          kAudioDevicePropertyMute, kAudioDevicePropertyScopeInput, 1};

      OSStatus theError = AudioObjectGetPropertyData(
          m_theDefaultDeviceID, &theAddress, 0, NULL, &theSize, &theMute);
      value = theMute;
      return theError == noErr;
    }
  default:
...
bool CMixerControls::SetValue_Float(Control iControl, float value) {
  ...
  case MicVolume: {
    ...
    size = sizeof(canset);
    OSStatus err = AudioDeviceGetPropertyInfo(defaultInputDevice, 0, true,
                                              kAudioDevicePropertyVolumeScalar,
                                              &size, &canset);
    if (err == noErr && canset == true) {
      size = sizeof(theVolume);
      err = AudioDeviceSetProperty(defaultInputDevice, NULL, 0, true,
                                   kAudioDevicePropertyVolumeScalar, size,
                                   &theVolume);
      return err == noErr;
    }
    ...
    err = AudioDeviceGetProperty(defaultInputDevice, 0, true,
    ...
    // set volume
    size = sizeof(float);
    err = AudioDeviceSetProperty(defaultInputDevice, 0, channels[0], true,
                                 kAudioDevicePropertyVolumeScalar, size,
                                 &theVolume);
    // AssertMsg1( noErr==err, "error setting volume of channel
    // %d\n",(int)channels[0]);
    err = AudioDeviceSetProperty(defaultInputDevice, 0, channels[1], true,
                                 kAudioDevicePropertyVolumeScalar, size,
                                 &theVolume);
    // AssertMsg1( noErr==err, "error setting volume of channel
    // %d\n",(int)channels[1]);
  ...
  case MicMute:
    ...
    // sound card echoes whatever you say into the mic.
    {
      ...
      theError = AudioDeviceSetProperty(m_theDefaultDeviceID, NULL, 0, TRUE,
                                        kAudioDevicePropertyMute, theMuteSize,
                                        &theMute);
      return theError == noErr;
    }
  default:
