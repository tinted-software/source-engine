...
// exception class
CMPAException::CMPAException(ErrorIDs ErrorID, const char *szFile,
                             const char *szFunction, bool bGetLastError)
...
CMPAFile::CMPAFile(const char *szFile, uint32 dwFileOffset, FileHandle_t hFile)
    : m_pBuffer(NULL), m_dwBufferSize(0), m_dwBegin(dwFileOffset), m_dwEnd(0),
      m_dwNumTimesRead(0), m_bVBRFile(false), m_pVBRHeader(NULL),
      m_bMustReleaseFile(false), m_pMPAHeader(NULL), m_hFile(hFile),
      m_szFile(NULL), m_dwFrameNo(1) {
...
void CMPAFile::Open(const char *szFilename) {
  ...
  m_hFile = g_pFullFileSystem->Open(
      szFilename, "rb",
      "GAME"); //::CreateFile( szFilename, GENERIC_READ, FILE_SHARE_READ, NULL,
               //:OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL );
  if (m_hFile == FILESYSTEM_INVALID_HANDLE) {
    // throw error
    throw CMPAException(CMPAException::ErrOpenFile, szFilename,
                        _T("CreateFile"), true);
  }
}
...
void CMPAFile::SetPosition(int offset) {
  ...
  dwMoveMethod ); if (liOff.LowPart  == INVALID_SET_FILE_POINTER &&
  GetLastError() != NO_ERROR )
  {
      // throw error
      throw CMPAException( CMPAException::ErrSetPosition, m_szFile,
...
// uint32 (32bit)
uint32 CMPAFile::ExtractBytes(uint32 &dwOffset, uint32 dwNumBytes,
                              bool bMoveOffset) {
  Assert(dwNumBytes > 0);
  ...
  for (uint32 n = dwOffset; n < dwOffset + dwNumBytes; n++) {
    dwResult |= ((byte)m_pBuffer[n])
                << (8 * dwNumByteShifts); // the bit shift will do the correct
                                          // byte order for you
    dwNumByteShifts--;
  }
...
void CMPAFile::FillBuffer(uint32 dwOffsetToRead) {
  ...
  uint32 dwBytesRead = Read(m_pBuffer + m_dwBufferSize,
