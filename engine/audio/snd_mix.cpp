...
int g_snd_profile_type =
    0; // type 1 dsp, type 2 mixer, type 3 load sound, type 4 all sound
...
void MIX_InitializePaintbuffer(paintbuffer_t *pPaintBuffer, bool bSurround,
                               bool bSurroundCenter) {
  V_memset(pPaintBuffer, 0, sizeof(*pPaintBuffer));

  pPaintBuffer->pbuf = (portable_samplepair_t *)_aligned_malloc(
      PAINTBUFFER_MEM_SIZE * sizeof(portable_samplepair_t), 16);
  V_memset(pPaintBuffer->pbuf, 0,
           PAINTBUFFER_MEM_SIZE * sizeof(portable_samplepair_t));

  if (bSurround) {
    pPaintBuffer->pbufrear = (portable_samplepair_t *)_aligned_malloc(
        PAINTBUFFER_MEM_SIZE * sizeof(portable_samplepair_t), 16);
    V_memset(pPaintBuffer->pbufrear, 0,
             PAINTBUFFER_MEM_SIZE * sizeof(portable_samplepair_t));
  }
  if (bSurroundCenter) {
    pPaintBuffer->pbufcenter = (portable_samplepair_t *)_aligned_malloc(
        PAINTBUFFER_MEM_SIZE * sizeof(portable_samplepair_t), 16);
    V_memset(pPaintBuffer->pbufcenter, 0,
             PAINTBUFFER_MEM_SIZE * sizeof(portable_samplepair_t));
  }
}
...
bool MIX_InitAllPaintbuffers(void) {
  ...
  while (g_paintBuffers.Count() < SOUND_BUFFER_BASETOTAL) {
    ...
    MIX_InitializePaintbuffer(&(g_paintBuffers[nIndex]), bSurround,
                              bSurroundCenter);
  }
...
//fewer samples are mixed. 		 if (endtime - paintedtime) is not aligned on
//boundaries of 4, 		 we'll miss data if outputRate < SOUND_DMA_SPEED!
void MIX_MixChannelsToPaintbuffer(CChannelList &list, int endtime, int flags,
  ...
  tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s c:%d %d/%d", __FUNCTION__,
         list.Count(), rate, outputRate);
  ...
  for (i = list.Count(); --i >= 0;) {
    ...
    Assert(!((ch->flags.bdry && ch->flags.bSpeaker) ||
             (ch->flags.bdry && ch->special_dsp != 0)));
    ...
    if (bIsMouth) {
      if ((ch->soundsource == SOUND_FROM_UI_PANEL) ||
          entitylist->GetClientEntity(ch->soundsource) ||
          (ch->flags.bSpeaker &&
    ...
    if ((ch->nFreeChannelAtSampleTime > 0 &&
         (int)ch->nFreeChannelAtSampleTime <= endtime)) {
      S_FreeChannel(ch);
      list.RemoveChannelFromList(i);
    }
  }
...
inline portable_samplepair_t *
S_GetNextpFilter(int i, portable_samplepair_t *pbuffer,
...
// paintbuffers: SOUND_BUFFER_ROOM, SOUND_BUFFER_FACING, IFACINGAWAY,
// SOUND_BUFFER_DRY, SOUND_BUFFER_SPEAKER, SOUND_BUFFER_SPECIALs dsp fx are then
// applied to these buffers by the caller. caller also remixes all into final
// SOUND_BUFFER_PAINT output.
...
void MIX_UpsampleAllPaintbuffers(CChannelList &list, int end, int count) {
  ...
  // mix and upsample all 'dry' sounds (channels) to 44khz SOUND_BUFFER_DRY
  // paintbuffer

  if (list.m_hasDryChannels)
    MIX_MixUpsampleBuffer(list, SOUND_BUFFER_DRY, end, count, SOUND_MIX_DRY);

  // mix and upsample all 'speaker' sounds (channels) to 44khz
  // SOUND_BUFFER_SPEAKER paintbuffer

  if (list.m_hasSpeakerChannels)
    MIX_MixUpsampleBuffer(list, SOUND_BUFFER_SPEAKER, end, count,
                          SOUND_MIX_SPEAKER);

  // mix and upsample all 'special dsp' sounds (channels) to 44khz
  // SOUND_BUFFER_SPECIALs paintbuffer
  ...
  if (!g_bDspOff) {
    ...
    MIX_ActivatePaintbuffer(
        SOUND_BUFFER_ROOM); // operates on MIX_MixChannelsToPaintbuffer
  }
...
ConVar snd_cull_duplicates(
    "snd_cull_duplicates", "0", FCVAR_ALLOWED_IN_COMPETITIVE,
...
#ifdef POSIX
      // If we aren't the active app and the option for background audio isn't
      // on, mute the audio Windows has it's own system for background muting
...
// A mix pass is performed, resulting in mixed sounds in SOUND_BUFFER_ROOM,
// SOUND_BUFFER_FACING, SOUND_BUFFER_FACINGAWAY, SOUND_BUFFER_DRY,
// SOUND_BUFFER_SPEAKER, SOUND_BUFFER_SPECIALs

// directional sounds are panned and mixed between SOUND_BUFFER_FACING and
// SOUND_BUFFER_FACINGAWAY omnidirectional sounds are panned 100% into
// SOUND_BUFFER_FACING sound sources far from player (ie: near back of room )
// are mixed in proportion to this distance into SOUND_BUFFER_ROOM sounds with
// ch->bSpeaker set are mixed in mono into SOUND_BUFFER_SPEAKER sounds with
// ch->bSpecialDSP set are mixed in mono into SOUND_BUFFER_SPECIALs

// dsp_facingaway fx (2 or 4ch filtering) are then applied to the
// SOUND_BUFFER_FACINGAWAY dsp_speaker fx (1ch) are then applied to the
// SOUND_BUFFER_SPEAKER dsp_specialdsp fx (1ch) are then applied to the
// SOUND_BUFFER_SPECIALs dsp_room fx (1ch reverb) are then applied to the
// SOUND_BUFFER_ROOM

// All buffers are recombined into the SOUND_BUFFER_PAINT

// The dsp_water and dsp_player fx are applied in series to the
// SOUND_BUFFER_PAINT
...
void MIX_PaintChannels(int endtime, bool bIsUnderwater) {
  ...
  // the cache needs to hold the audio in memory during mixing, so tell it that
  // mixing is starting
  ...
  while (g_paintedtime < endtime) {
    ...
    // SOUND_BUFFER_ROOM, SOUND_BUFFER_FACING, SOUND_BUFFER_FACINGAWAY,
    // SOUND_BUFFER_DRY, SOUND_BUFFER_SPEAKER, SOUND_BUFFER_SPECIALs
    ...
    if (g_bdirectionalfx) {
      g_AudioDevice->ApplyDSPEffects(
          idsp_facingaway, MIX_GetPFrontFromIPaint(SOUND_BUFFER_FACINGAWAY),
    ...
    if (!g_bDspOff && list.m_hasSpeakerChannels) {
      ...
      g_AudioDevice->ApplyDSPEffects(
          idsp_speaker, MIX_GetPFrontFromIPaint(SOUND_BUFFER_SPEAKER),
      ...
      MIX_MixPaintbuffers(SOUND_BUFFER_SPEAKER, SOUND_BUFFER_FACING,
                          SOUND_BUFFER_FACING, count, 1.0); // +70% dry speaker

      MIX_ScalePaintBuffer(SOUND_BUFFER_SPEAKER, count, 0.43);

      MIX_MixPaintbuffers(SOUND_BUFFER_SPEAKER, SOUND_BUFFER_ROOM,
                          SOUND_BUFFER_ROOM, count, 1.0); // +30% wet speaker
    }
    ...
    if (!g_bDspOff) {
      ...
      for (int iDSP = 0; iDSP < list.m_nSpecialDSPs.Count(); ++iDSP) {
        ...
        for (int i = SOUND_BUFFER_SPECIAL_START; i < g_paintBuffers.Count();
             ++i) {
          paintbuffer_t *pSpecialBuffer = MIX_GetPPaintFromIPaint(i);
          if (pSpecialBuffer->nSpecialDSP == list.m_nSpecialDSPs[iDSP] &&
              pSpecialBuffer->idsp_specialdsp != -1) {
            g_AudioDevice->ApplyDSPEffects(
                pSpecialBuffer->idsp_specialdsp, MIX_GetPFrontFromIPaint(i),
                MIX_GetPRearFromIPaint(i), MIX_GetPCenterFromIPaint(i), count);

            // mix SOUND_BUFFER_SPECIALs with SOUND_BUFFER_ROOM and
            // SOUND_BUFFER_FACING
            MIX_ScalePaintBuffer(i, count, 0.7);

            MIX_MixPaintbuffers(i, SOUND_BUFFER_FACING, SOUND_BUFFER_FACING,
                                count, 1.0); // +70% dry speaker

            MIX_ScalePaintBuffer(i, count, 0.43);

            MIX_MixPaintbuffers(i, SOUND_BUFFER_ROOM, SOUND_BUFFER_ROOM, count,
                                1.0); // +30% wet speaker
        ...
        if (!bFoundMixer) {
          ...
          g_paintBuffers[nIndex].nPrevSpecialDSP =
              g_paintBuffers[nIndex].nSpecialDSP;
          g_paintBuffers[nIndex].idsp_specialdsp =
              DSP_Alloc(g_paintBuffers[nIndex].nSpecialDSP, 300, 1);
        }
      }
    }
    ...
    g_AudioDevice->ApplyDSPEffects(
        Get_idsp_room(), MIX_GetPFrontFromIPaint(SOUND_BUFFER_ROOM),
    ...
    if (b_spatial_delays && !g_bDspOff && !DSP_RoomDSPIsOff()) {
      ...
      g_AudioDevice->ApplyDSPEffects(
          idsp_spatial, MIX_GetPFrontFromIPaint(SOUND_BUFFER_ROOM),
          MIX_GetPRearFromIPaint(SOUND_BUFFER_ROOM),
          MIX_GetPCenterFromIPaint(SOUND_BUFFER_ROOM), count);
    }
    ...
    if (g_bdirectionalfx) // KDB: perf
    {
      ...
      MIX_MixPaintbuffers(SOUND_BUFFER_FACING, SOUND_BUFFER_FACINGAWAY,
                          SOUND_BUFFER_PAINT, count, DSP_NOROOM_MIX);

      // Add in dsp room fx to paintbuffer, mix at 75%
      MIX_MixPaintbuffers(SOUND_BUFFER_ROOM, SOUND_BUFFER_PAINT,
                          SOUND_BUFFER_PAINT, count, DSP_ROOM_MIX);
    } else {
      // Mix IFACING buffer with SOUND_BUFFER_ROOM
      // (SOUND_BUFFER_FACINGAWAY contains no data, IFACINGBBUFFER has full dry
      // mix based on distance from listener) if dsp disabled, mix 100%
      // facingbuffer, otherwise, mix 75% facingbuffer + roombuffer
      float mix = g_bDspOff ? 1.0 : DSP_ROOM_MIX;
      MIX_MixPaintbuffers(SOUND_BUFFER_ROOM, SOUND_BUFFER_FACING,
                          SOUND_BUFFER_PAINT, count, mix);
    }
    ...
    if (bIsUnderwater) {
      ...
      // buffers.
      g_AudioDevice->ApplyDSPEffects(
          idsp_water, MIX_GetPFrontFromIPaint(SOUND_BUFFER_PAINT),
    ...
    // disabled
    g_AudioDevice->ApplyDSPEffects(
        idsp_player, MIX_GetPFrontFromIPaint(SOUND_BUFFER_PAINT),
        MIX_GetPRearFromIPaint(SOUND_BUFFER_PAINT),
        MIX_GetPCenterFromIPaint(SOUND_BUFFER_PAINT), count);
    ...
    /*
       ...
       MIX_GetPFrontFromIPaint(SOUND_BUFFER_PAINT),
       MIX_GetPRearFromIPaint(SOUND_BUFFER_PAINT),
       MIX_GetPCenterFromIPaint(SOUND_BUFFER_PAINT), count );
    */
    ...
    MIX_MixPaintbuffers(SOUND_BUFFER_PAINT, SOUND_BUFFER_DRY,
                        SOUND_BUFFER_PAINT, count, 1.0);
...
// down. b) if mixing to SOUND_BUFFER_ROOM, scale all volumes by ch.dspmix and
// dsp_room gain c) if mixing to SOUND_BUFFER_FACINGAWAY, scale all volumes by
...
bool MIX_ScaleChannelVolume(paintbuffer_t *ppaint, channel_t *pChannel,
  ...
  if (mixflag & SOUND_BUSS_ROOM) {
    ...
    // if dspmix is 1.0, 100% of sound goes to SOUND_BUFFER_ROOM and 0% to
    // SOUND_BUFFER_FACING
...
#define psp_size 8
  ...
  __asm
  {
    // prologue
        ...
        // must multiply by the table size.  There are 256 4-byte values in the
        // table at each level. So each index must be shifted left by 10, but
        // since the bits we use are in the MSB rather than LSB they must be
...
void SW_Mix8StereoDirectional(float soundfacing, portable_samplepair_t *pOutput,
                              int *volume, byte *pData, int inputOffset,
                              fixedint rateScaleFix, int outCount) {
  int sampleIndex = 0;
  fixedint sampleFrac = inputOffset;
  int x;
...
void SW_Mix8Mono(portable_samplepair_t *pOutput, int *volume, byte *pData,
...
void SW_Mix8Mono_Interp(portable_samplepair_t *pOutput, int *volume,
                        byte *pData, int inputOffset, fixedint rateScaleFix,
                        int outCount) {
  fixedint sampleIndex = 0;
  fixedint rateScaleFix14 = FIX_28TO14(
      rateScaleFix); // convert 28 bit fixed point to 14 bit fixed point
...
void SW_Mix8Stereo(portable_samplepair_t *pOutput, int *volume, byte *pData,
                   int inputOffset, fixedint rateScaleFix, int outCount) {
  int sampleIndex = 0;
  fixedint sampleFrac = inputOffset;
  int *lscale, *rscale;
...
void Mix8StereoWavtype(channel_t *pChannel, portable_samplepair_t *pOutput,
  ...
  case CHAR_DOPPLER:
    SW_Mix8StereoDopplerLeft(pOutput, volume, pData, inputOffset, rateScaleFix,
                             outCount);
  ...
  case CHAR_DIRECTIONAL:
    if (FUseHighQualityPitch(pChannel))
      SW_Mix8StereoDirectional_Interp(pChannel->dspface, pOutput, volume, pData,
                                      inputOffset, rateScaleFix, outCount);
    else
...
void Mix16StereoWavtype(channel_t *pChannel, portable_samplepair_t *pOutput,
  ...
  switch (pChannel->wavtype) {
  case CHAR_DOPPLER:
    SW_Mix16StereoDopplerLeft(pOutput, volume, pData, inputOffset, rateScaleFix,
                              outCount);
...
void SND_MoveMouth8(channel_t *ch, CAudioSource *pSource, int count) {
  ...
  if (IsX360()) {
    // not supporting because data is assumed to be 8 bit and bypasses mixer
    // (decoding)
    return;
  }
...
bool SND_IsMouth(channel_t *pChannel) {
  ...
  if (pChannel->entchannel == CHAN_VOICE ||
      pChannel->entchannel == CHAN_VOICE2) {
    return true;
  }
...
bool SND_IsRecording() {
  return ((IsReplayRendering() || cl_movieinfo.IsRecording()) &&
          !Con_IsVisible());
}
...
void SND_RecordBuffer(void) {
  ...
  if (IsReplayRendering()) {
    ...
    IReplayMovieRenderer *pMovieRenderer =
        g_pClientReplayContext->GetMovieRenderer();
    if (IsReplayRendering() && pMovieRenderer &&
        pMovieRenderer->IsAudioSyncFrame()) {
      pMovieRenderer->RenderAudio((unsigned char *)tmp, bufferSize,
                                  snd_linear_count);
    }
#endif
  } else {
