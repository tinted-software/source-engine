...
void CAudioSourceWave::GetCacheData(CAudioSourceCachedInfo *info) {
  ...
  if (GetStartupData(tempbuf, sizeof(tempbuf), datalen) &&
      info->s_bIsPrecacheSound && datalen > 0) {
...
CAudioSourceMemWave::CAudioSourceMemWave(CSfxTable *pSfx)
  ...
  if (IsX360()) {
    ...
    if (bValid) {
      // a 360 memory wave is a critical resource kept locked in memory, load
      // its data now
      CacheLoad();
    }
  }
    ...
    if (samplePosition >= m_nCachedDataSize) {
      *pData = GetDataPointer();
      ...
      if (IsPC() || !IsX360()) {
        ...
        if (samplePosition + (sampleCount * m_sampleSize) >=
            m_nCachedDataSize) {
        ...
        *pData = GetCachedDataPointer();
        ...
        // for 360, memory wave data should have already been loaded and locked
        // in cache
        Assert(0);
      }
    }
...
int CAudioSourceMemWave::GetCacheStatus(void) {
  ...
  if (IsPC() || !IsX360()) {
    ...
    return wavedatacache->IsStreamedDataReady(m_hStream) ? AUDIO_IS_LOADED
                                                         : AUDIO_NOT_LOADED;
  }
  ...
  if (IsPC() || !IsX360()) {
    ...
    if (m_hStream == INVALID_STREAM_HANDLE) {
      ...
      if (m_format == WAVE_FORMAT_XMA || m_format == WAVE_FORMAT_PCM) {
        ...
        // restricting to XMA due to not correctly running a post
        // ConvertSamples, which is not an issue for XMA
      ...
      m_hStream =
          wavedatacache->OpenStreamedLoad(pFilename, m_dataSize, m_dataStart, 0,
                                          -1, m_dataSize, 1, streamFlags);
      if (m_hStream != INVALID_STREAM_HANDLE &&
          !(streamFlags & STREAMED_QUEUEDLOAD)) {
        // block and finish load, convert data once right now
        char *pWaveData =
            (char *)wavedatacache->GetStreamedDataPointer(m_hStream, true);
        if (pWaveData) {
...
CAudioMixer *CAudioSourceStreamWave::CreateMixer(int initialStreamPosition) {
  ...
  const char *pFileName = m_pSfx->GetFileName();
  if (IsX360() &&
      (m_format == WAVE_FORMAT_XMA || m_format == WAVE_FORMAT_PCM)) {
...
void CAudioSourceStreamWave::ParseChunk(IterateRIFF &walk, int chunkName) {
  ...
  //needed chunks must occur 		before the DATA chunk.  But, that is not standard
  //and breaks most other wav parsers.
...
CAudioSource *Audio_CreateStreamedWave(CSfxTable *pSfx) {
  ...
  if (Audio_IsMP3(pSfx->GetFileName())) {
    return Audio_CreateMemoryMP3(
        pSfx); // TOSUCK: Dont work with streamed mp3, idk why
  }
#endif
...
CAudioSourceCachedInfo::CAudioSourceCachedInfo()
    : infolong(0), flagsbyte(0), m_dataStart(0), m_dataSize(0), m_loopStart(0),
      m_sampleCount(0), m_usCachedDataSize(0), m_pCachedData(0),
      m_usHeaderSize(0), m_pHeader(0), m_pSentence(0) {}
...
// Versions
//   3: The before time
//   4: Changed MP3 caching to ensure we store proper sample rate, removed hack
//   to not cache vo/ 5: Fixed bug that could result in incorrect mp3 datasizes
//   in the sound cache
...
public:
  struct SearchPathCache : CUtlCachedFileData<CAudioSourceCachedInfo> {
    SearchPathCache(const char *pszRepositoryFilename,
                    const char *pszSearchPath,
                    UtlCachedFileDataType_t eOutOfDateMethod)
        : CUtlCachedFileData(pszRepositoryFilename, AUDIOSOURCE_CACHE_VERSION,
                             AsyncLookaheadMetaChecksum, eOutOfDateMethod) {
    ...
    virtual ~SearchPathCache() { Shutdown(); }
  };
  ...
  GetInfo(int audiosourcetype, bool soundisprecached, CSfxTable *sfx);
  virtual void RebuildCacheEntry(int audiosourcetype, bool soundisprecached,
                                 CSfxTable *sfx);
  virtual void ForceRecheckDiskInfo();

private:
  SearchPathCache *LookUpCacheEntry(const char *szCleanedFilename,
                                    int audiosourcetype, bool soundisprecached,
                                    CSfxTable *sfx);
  ...
  static void GetSoundFilename(char *szResult, int nResultSize,
...
CAudioSourceCache::FindCacheForSearchPath(const char *pszSearchPath) {
...
CAudioSourceCache::SearchPathCache *
CAudioSourceCache::CreateCacheForSearchPath(const char *pszSearchPath) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CAudioSourceCache::LevelShutdown() { CheckSaveDirtyCaches(); }

//-----------------------------------------------------------------------------
void CAudioSourceCache::GetSoundFilename(char *szResult, int nResultSize,
                                         const char *pszInputFilename) {
...
//-----------------------------------------------------------------------------
CAudioSourceCache::SearchPathCache *
CAudioSourceCache::LookUpCacheEntry(const char *fn, int audiosourcetype,
  ...
  // Get absolute filename.  This thing had better exist in the filesystem
  // somewhere
  char szAbsFilename[1024];
  if (!g_pFullFileSystem->RelativePathToFullPath(
          szRelFilename, "game", szAbsFilename, sizeof(szAbsFilename))) {
    return NULL;
  }
  ...
  FOR_EACH_VEC(m_vecCaches, idx) {
    SearchPathCache *pCache = m_vecCaches[idx];
    if (V_strncmp(pCache->m_szSearchPath, szAbsFilename,
                  V_strlen(pCache->m_szSearchPath)) == 0) {
      return pCache;
    }
  }
  Warning("Cannot figure out which search path %s came from.  Absolute path is "
          "%s\n",
...
void CAudioSourceCache::ForceRecheckDiskInfo() {
  FOR_EACH_VEC(m_vecCaches, idx) { m_vecCaches[idx]->ForceRecheckDiskInfo(); }
}
...
void CAudioSourceCache::BuildCache(char const *pszSearchPath) {
  ...
  // Add a search path to the filesystem.  We'll add one search path as a kludge
  // so we can use the existing file finder system easily
  g_pFullFileSystem->AddSearchPath(szAbsPath, "soundcache_kludge",
                                   PATH_ADD_TO_HEAD);
  ...
  // FindCacheForSearchPath expects an absolute search path, but if we're
  // working with a VPK we'll have the path to the file, wherein the proper path
  // to the file is /foo/bar.vpk, but the *search path* should be /foo/bar.vpk/
  ...
  FOR_EACH_VEC(vecFilenames, idxFilename) {
    ...
    if (V_strnicmp(pszRelName, "sound" CORRECT_PATH_SEPARATOR_S, 6) != 0) {
      Warning(
          "Relative name %s doesn't begin with leading 'sound' directory?\n",
          pszRelName);
      Assert(false);
      continue;
    }
...
CON_COMMAND(snd_buildcache, "<directory or VPK filename>  Rebulds sound cache "
                            "for a given search path.\n") {
  if (args.ArgC() < 2) {
  ...
  for (int idxArg = 1; idxArg < args.ArgC(); ++idxArg) {
    ...
    FOR_EACH_VEC(vecPaths, idxPath) { g_ASCache.BuildCache(vecPaths[idxPath]); }
  }

  // And now quit the game, because we mucked with search paths and the game is
  // almost certainly not going to work anymore
