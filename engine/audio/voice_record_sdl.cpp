...
VoiceRecord_SDL::VoiceRecord_SDL() : m_nSampleRate(0), m_Device(0) {
...
bool VoiceRecord_SDL::InitalizeInterfaces() {
  ...
  m_Device = SDL_OpenAudioDevice(NULL, SDL_TRUE, &desiredRecordingSpec,
                                 &m_ReceivedRecordingSpec, 0);

  if (m_Device != 0) {
    // Calculate per sample bytes
    m_BytesPerSample = m_ReceivedRecordingSpec.channels *
    ...
    m_AudioBuffer.writePtr =
        m_AudioBuffer.data + SAMPLE_COUNT * m_BytesPerSample * 2;
