...
IAudioStream *CAudioMixerWaveMP3::GetStream() {
  if (!m_bStreamInit) {
    ...
    if (vaudio) {
      m_pStream = vaudio->CreateMP3StreamDecoder(
          static_cast<IAudioStreamEvent *>(this));
    } else {
...
int CAudioMixerWaveMP3::GetOutputData(void **pData, int sampleCount,
  ...
  IAudioStream *pStream = GetStream();
  if (!pStream) {
    // Needed for channel count, and with a failed stream init we probably
    // should fail to return data anyway.
    return 0;
  }
