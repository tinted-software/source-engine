...
#ifdef OSX
ALvoid alBufferDataStaticProc(const ALint bid, ALenum format, ALvoid *data,
                              ALsizei size, ALsizei freq) {
  static alBufferDataStaticProcPtr proc = NULL;

  if (proc == NULL) {
    proc = (alBufferDataStaticProcPtr)alGetProcAddress(
...
void CAudioDeviceOpenAL::PaintEnd(void) {
  ...
  int nProcessedLoop = 200; // spin for a max of 200 times de-queing buffers,
                            // fixes a hang on exit
...
#ifdef OSX
    alBufferDataStaticProc(m_Buffer[iBuf], AL_FORMAT_STEREO16,
                           (char *)m_sndBuffers + iBuf * BUFFER_SIZE,
                           BUFFER_SIZE, DeviceDmaSpeed());
#else
    alBufferData(m_Buffer[iBuf], AL_FORMAT_STEREO16,
...
void CAudioDeviceOpenAL::UpdateListener(const Vector &position,
...
void CAudioDeviceOpenAL::Mix8Mono(channel_t *pChannel, char *pData,
...
void CAudioDeviceOpenAL::Mix8Stereo(channel_t *pChannel, char *pData,
                                    int outputOffset, int inputOffset,
                                    fixedint rateScaleFix, int outCount,
                                    int timecompress) {
  int volume[CCHANVOLUMES];
...
void CAudioDeviceOpenAL::Mix16Mono(channel_t *pChannel, short *pData,
                                   int outputOffset, int inputOffset,
                                   fixedint rateScaleFix, int outCount,
                                   int timecompress) {
  int volume[CCHANVOLUMES];
...
void CAudioDeviceOpenAL::Mix16Stereo(channel_t *pChannel, short *pData,
                                     int outputOffset, int inputOffset,
                                     fixedint rateScaleFix, int outCount,
                                     int timecompress) {
  int volume[CCHANVOLUMES];
...
void CAudioDeviceOpenAL::ChannelReset(int entnum, int channelIndex,
                                      float distanceMod) {}
...
void CAudioDeviceOpenAL::SpatializeChannel(int volume[CCHANVOLUMES / 2],
                                           int master_vol,
...
void CAudioDeviceOpenAL::ApplyDSPEffects(int idsp,
