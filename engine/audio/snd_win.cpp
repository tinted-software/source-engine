...
IAudioDevice *IAudioDevice::AutoDetectInit(bool waveOnly) {
  ...
  if (IsPC()) {
    ...
    if (!pDevice) {
      ...
      pDevice = Audio_CreateOpenALDevice(); // fall back to openAL if the audio
                                            // queue fails
    }
...
#ifdef NEVER
    // Jul 2012. mikesart. E-mail exchange with Ryan Gordon after figuring out
    // that Audio_CreatePulseAudioDevice() wasn't working on Ubuntu 12.04 (lots
    // of stuttering).
    //
    ...
    if (!pDevice) {
      ...
      pDevice = Audio_CreatePulseAudioDevice(); // fall back to PulseAudio if
                                                // SDL fails
    }
#endif // NEVER
