...
private:
  bool InitalizeInterfaces(); // Initialize the openal capture buffers and other
                              // interfaces
...
VoiceRecord_AudioQueue::VoiceRecord_AudioQueue()
    : m_nSampleRate(0), m_AudioUnit(NULL), m_SampleBufferSize(0),
      m_SampleBuffer(NULL), m_SampleBufferReadPos(0), m_SampleBufferWritePos(0),
      m_bRecordingAudio(false), m_hThread(NULL), m_bFirstInit(true) {
  ClearInterfaces();
}
...
void VoiceRecord_AudioQueue::Release() { ReleaseInterfaces(); }
...
OSStatus
ComplexBufferFillPlayback(AudioConverterRef inAudioConverter,
  ...
  if (vr->m_nMicInputSamplesAvaialble) {
    ...
    int nBytesAvailable =
        vr->m_nMicInputSamplesAvaialble * vr->m_InputBytesPerPacket;
    ...
    if (nBytesRequired < nBytesAvailable) {
      ...
      vr->m_MicInputBuffer.mBuffers[0].mData =
          (char *)vr->m_MicInputBuffer.mBuffers[0].mData + nBytesRequired;
      vr->m_MicInputBuffer.mBuffers[0].mDataByteSize =
          nBytesAvailable - nBytesRequired;
    } else {
      ioData->mBuffers[0].mData = vr->m_MicInputBuffer.mBuffers[0].mData;
    ...
    *ioNumberDataPackets =
        ioData->mBuffers[0].mDataByteSize / vr->m_InputBytesPerPacket;
    vr->m_nMicInputSamplesAvaialble =
        nBytesAvailable / vr->m_InputBytesPerPacket - *ioNumberDataPackets;
  } else {
...
static OSStatus recordingCallback(void *inRefCon,
  ...
  if (vr->m_nMicInputSamplesAvaialble == 0) {
    err = AudioUnitRender(vr->GetAudioUnit(), ioActionFlags, inTimeStamp, 1,
                          inNumberFrames, &vr->m_MicInputBuffer);
    if (err == noErr)
      vr->m_nMicInputSamplesAvaialble =
          vr->m_MicInputBuffer.mBuffers[0].mDataByteSize /
          vr->m_InputBytesPerPacket;
  }

  if (vr->m_nMicInputSamplesAvaialble > 0) {
    UInt32 nConverterSamples =
        ceil(vr->m_nMicInputSamplesAvaialble / vr->m_flSampleRateConversion);
    vr->m_ConverterBuffer.mBuffers[0].mDataByteSize = vr->m_ConverterBufferSize;
    OSStatus err = AudioConverterFillComplexBuffer(
        vr->GetConverter(), ComplexBufferFillPlayback, vr, &nConverterSamples,
        &vr->m_ConverterBuffer, NULL);
    if (err == noErr || err == -1)
      vr->RenderBuffer((short *)vr->m_ConverterBuffer.mBuffers[0].mData,
                       vr->m_ConverterBuffer.mBuffers[0].mDataByteSize /
                           sizeof(short));
  }
...
void VoiceRecord_AudioQueue::RenderBuffer(const short *pszBuf, int nSamples) {
  ...
  int nFirstCopy =
      MIN(nSamples * sizeof(short), m_SampleBufferSize - samplePos);
  ...
  if ((bBeforeRead && samplePos > readPos)) {
    m_SampleBufferReadPos = (readPos + m_SampleBufferSize / 2) %
                            m_SampleBufferSize; // if we crossed the read
                                                // pointer then bump it forward
    // printf( "Crossed %d %d (%d)\n", (int)samplePosBefore, (int)samplePos,
    // readPos );
  }
}
...
bool VoiceRecord_AudioQueue::InitalizeInterfaces() {
  ...
  status = AudioUnitSetProperty(m_AudioUnit, kAudioUnitProperty_StreamFormat,
                                kAudioUnitScope_Output, 1, &audioDeviceFormat,
                                sizeof(audioDeviceFormat));
  if (status != noErr)
    return false;
  ...
  m_flSampleRateConversion =
      audioDeviceFormat.mSampleRate / audioOutputFormat.mSampleRate;

  // setup sample rate conversion
  status =
      AudioConverterNew(&audioDeviceFormat, &audioOutputFormat, &m_Converter);
  if (status != noErr)
    return false;

  UInt32 primeMethod = kConverterPrimeMethod_None;
  status = AudioConverterSetProperty(m_Converter, kAudioConverterPrimeMethod,
                                     sizeof(UInt32), &primeMethod);
  if (status != noErr)
    return false;

  UInt32 quality = kAudioConverterQuality_Medium;
  status = AudioConverterSetProperty(m_Converter,
  ...
  status = AudioUnitSetProperty(
      m_AudioUnit, kAudioOutputUnitProperty_SetInputCallback,
  ...
  status = AudioDeviceGetProperty(inputDevice, 1, 1,
  ...
  m_MicInputBuffer.mBuffers[0].mDataByteSize =
      m_nBufferFrameSize * audioDeviceFormat.mBitsPerChannel / 8 *
      audioDeviceFormat.mChannelsPerFrame;
  m_MicInputBuffer.mBuffers[0].mData =
      malloc(m_MicInputBuffer.mBuffers[0].mDataByteSize);
  m_MicInputBuffer.mBuffers[0].mNumberChannels =
      audioDeviceFormat.mChannelsPerFrame;
  ...
  m_ConverterBuffer.mBuffers[0].mDataByteSize =
      m_nBufferFrameSize * audioOutputFormat.mBitsPerChannel / 8 *
      audioOutputFormat.mChannelsPerFrame;
  m_ConverterBuffer.mBuffers[0].mData =
      malloc(m_MicInputBuffer.mBuffers[0].mDataByteSize);
  ...
  m_SampleBufferReadPos = m_SampleBufferWritePos = 0;
  m_SampleBufferSize = ceil(kNumSecAudioBuffer * m_nSampleRate *
...
int VoiceRecord_AudioQueue::GetRecordedData(short *pOut, int nSamples) {
  ...
  if (nOutstandingSamples < cbSamples)
    cbSamples =
        nOutstandingSamples; // clamp to the number of samples we have available

  memcpy((char *)pOut, m_SampleBuffer + readPos,
         MIN(cbSamples, m_SampleBufferSize - readPos));
  ...
  // (int)m_SampleBufferReadPos, (int)m_SampleBufferWritePos, m_SampleBufferSize
  // );
  return cbSamples / 2;
}
