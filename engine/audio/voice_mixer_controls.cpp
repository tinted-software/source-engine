...
// The correct method is to use the AudioEndpoint COM objects to manage controls
// like this The interface is not 1:1 so for now we'll just back the state with
// convars and reapply it on init of the mixer controls.  In the future when XP
// is no longer the majority of our user base we should revisit this and move to
// the new API. http://msdn.microsoft.com/en-us/library/aa964574(VS.85).aspx
...
bool CMixerControls::Init() {
  ...
  // This is done in reverse so the 0th device is our fallback if none of them
  // had the correct MicVolume control
  for (int iDevice = static_cast<int>(mixerGetNumDevs()) - 1;
       iDevice >= 0 && !bFoundConnectionWithMicVolume; --iDevice) {
    // Open the mixer.
    ...
    for (UINT u = 0; u < mxcaps.cDestinations; u++) {
      ...
      for (UINT v = 0; v < cConnections; v++) {
        ...
        if (!GetLineControls(micLine.dwLineID, controls.Base(),
                             micLine.cControls))
          continue;
        ...
        for (UINT i = 0; i < micLine.cControls; i++) {
          ...
          if (micLine.dwComponentType ==
            ...
            if (recordLine.dwComponentType ==
              ...
              m_ControlInfos[MicMute].m_cMultipleItems =
                  pControl->cMultipleItems;
            }
            ...
            if (recordLine.dwComponentType ==
              ...
              // We found a good recording device and can stop looking throught
