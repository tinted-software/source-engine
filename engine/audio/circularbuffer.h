...
//			and read from these buffers as though from a file. When
//it is 			write-overflowed (you write more data in than the buffer can hold), 			the
//read pointer is advanced to allow the new data to be written. 			This means old
//data will be discarded if you write too much data 			into the buffer.
//
...
public:
  ...
  int GetSize(); // Get the size of the buffer (how much can you write without
                 // reading before losing data.

  int GetWriteAvailable(); // Get the amount available to write without
                           // overflowing. Note: you can write however much you
                           // want, but it may overflow, in which case the
                           // newest data is kept and the oldest is discarded.
...
template <int size> class CSizedCircularBuffer : public CCircularBuffer {
public:
