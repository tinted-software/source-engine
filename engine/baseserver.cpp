...
static ConVar sv_max_queries_sec_global(
    "sv_max_queries_sec_global", "3000", 0,
    "Maximum queries per second to respond to from anywhere.");

static ConVar sv_max_connects_sec(
    "sv_max_connects_sec", "2.0", 0,
    "Maximum connections per second to respond to from a single IP address.");
static ConVar sv_max_connects_window(
    "sv_max_connects_window", "4", 0,
    "Window over which to average connections per second averages.");
// This defaults to zero so that somebody spamming the server with packets
// cannot lock out other clients.
static ConVar sv_max_connects_sec_global(
    "sv_max_connects_sec_global", "0", 0,
    "Maximum connections per second to respond to from anywhere.");

static CIPRateLimit s_queryRateChecker(&sv_max_queries_sec,
                                       &sv_max_queries_window,
                                       &sv_max_queries_sec_global);
static CIPRateLimit s_connectRateChecker(&sv_max_connects_sec,
...
static ConVar
    sv_instancebaselines("sv_instancebaselines", "1", FCVAR_DEVELOPMENTONLY,
                         "Enable instanced baselines. Saves network overhead.");
static ConVar sv_stats("sv_stats", "1", 0, "Collect CPU usage stats");
static ConVar
    sv_enableoldqueries("sv_enableoldqueries", "0", 0,
...
static void SetMasterServerKeyValue(ISteamGameServer *pUpdater,
                                    IConVar *pConVar) {
  ConVarRef var(pConVar);
...
bool CBaseServer::CheckChallengeNr(netadr_t &adr, int nChallengeValue) {
  ...
  uint64 challenge =
      ((uint64)adr.GetIPNetworkByteOrder() << 32) + m_CurrentRandomNonce;
  CRC32_t hash;
  CRC32_Init(&hash);
...
IClient *CBaseServer::ConnectClient(netadr_t &adr, int protocol, int challenge,
  ...
  if (!CheckChallengeType(
          client, nNextUserID, adr, authProtocol, hashedCDkey, cdKeyLen,
  ...
  if (!pSteamGameServer && authProtocol == PROTOCOL_STEAM) {
    Warning("NULL ISteamGameServer in ConnectClient. Steam authentication may "
            "fail.\n");
  }

  if (Filter_IsUserBanned(client->GetNetworkID())) {
    // Need to make sure the master server is updated with the rejected
    // connection because we called Steam3Server().NotifyClientConnect() in
    // CheckChallengeType() above.
    if (pSteamGameServer && authProtocol == PROTOCOL_STEAM)
  ...
  // Tell client connection worked, now use netchannels
  {
    ...
    NET_SendPacket(NULL, m_Socket, adr, msg.GetData(),
...
bool CBaseServer::ProcessConnectionlessPacket(netpacket_t *packet) {
  ...
  case C2S_CONNECT: {
    ...
    if (!CheckChallengeNr(packet->from, challengeNr)) {
      RejectConnection(packet->from, clientChallenge,
    ...
    // There's a magic number we use in the steam.inf in P4 that we don't
    // update. We can use this to detect if they are running out of P4, and if
    // so, don't do any version checking.
    const char *pszVersionInP4 = "2000";
    const char *pszVersionString = GetSteamInfIDVersionInfo().szVersionString;
    if (V_strcmp(pszVersionString, pszVersionInP4) &&
        V_strcmp(productVersion, pszVersionInP4)) {
      int nVersionCheck = Q_strncmp(pszVersionString, productVersion,
                                    V_strlen(pszVersionString));
      if (nVersionCheck < 0) {
        RejectConnection(packet->from, clientChallenge,
    ...
    // )
    // 				{
    // 					RejectConnection( packet->from, "Cannot
    // connect to a secure server while plug-ins are\nloaded on your client\n"
    // );
    //					break;
    // 				}
    ...
    /*				if ( authProtocol == PROTOCOL_STEAM )
       ...
       sizeof(cdkey) )
                                            {
                                                    RejectConnection(
       packet->from, clientChallenge, "#GameUI_ServerRejectBadSteamKey" );
                                                    break;
                                            }
                                            msg.ReadBytes( cdkey, keyLen );

                                            ConnectClient( packet->from,
       protocol, challengeNr, clientChallenge, authProtocol, name, password,
       cdkey, keyLen );	// cd key is actually a raw encrypted key
                                    }
                                    else*/
    {
  ...
  default: {
    ...
    if (Steam3Server().SteamGameServer() &&
      ...
      // This is where it will usually want to respond to something immediately
      // by sending some packets, so check for that immediately.
...
Counts number of HLTV and Replay connections.  Clients includes regular
connections
==================
*/
int CBaseServer::GetNumProxies(void) const {
  ...
  for (int i = 0; i < m_Clients.Count(); i++) {
    ...
    if (m_Clients[i]->IsConnected() &&
        (m_Clients[i]->IsHLTV() || m_Clients[i]->IsReplay()))
#else
...
bool CBaseServer::GetPlayerInfo(int nClientIndex, player_info_t *pinfo) {
  ...
  if (nClientIndex < 0 || !GetUserInfoTable() ||
      nClientIndex >= GetUserInfoTable()->GetNumStrings()) {
...
void CBaseServer::ReplyChallenge(netadr_t &adr, int clientChallenge) {
  ...
  msg.WriteLong(S2C_MAGICVERSION); // This makes it so we can detect that this
                                   // server is correct
  msg.WriteLong(challengeNr);      // Server to client challenge
  msg.WriteLong(clientChallenge);  // Client to server challenge to ensure our
                                   // reply is what they asked
...
int CBaseServer::GetChallengeType(netadr_t &adr) {
  ...
  if (Host_IsSinglePlayerGame() ||
      (!Steam3Client().SteamUser() && !IsDedicated())) {
    return PROTOCOL_HASHEDCDKEY;
  } else
#endif
  {
...
void CBaseServer::CalculateCPUUsage(void) {
  ...
  // only do this every 1 second
  {
    ...
    if (getrusage(RUSAGE_SELF, &currentUsage) == 0) {
      double flTimeDiff =
          (double)(currentUsage.ru_utime.tv_sec - s_lastUsage.ru_utime.tv_sec) +
          (double)((currentUsage.ru_utime.tv_usec -
                    s_lastUsage.ru_utime.tv_usec) /
                   1000000);
...
void CBaseServer::CheckTimeouts(void) {
  VPROF_BUDGET("CBaseServer::CheckTimeouts",
...
void CBaseServer::UpdateUserSettings(void) {
  VPROF_BUDGET("CBaseServer::UpdateUserSettings",
               VPROF_BUDGETGROUP_OTHER_NETWORKING);
  for (int i = 0; i < m_Clients.Count(); i++) {
    CBaseClient *cl = m_Clients[i];
...
void CBaseServer::SendPendingServerInfo() {
  VPROF_BUDGET("CBaseServer::SendPendingServerInfo",
               VPROF_BUDGETGROUP_OTHER_NETWORKING);
  for (int i = 0; i < m_Clients.Count(); i++) {
    CBaseClient *cl = m_Clients[i];
...
bool CBaseServer::CheckChallengeType(CBaseClient *client, int nNewUserID,
  ...
  if (IsHLTV()) {
    ...
    /*		if ( cbCookie <= 0 || cbCookie >= STEAM_KEYSIZE )
                    {
                            RejectConnection( adr, clientChallenge,
       "#GameUI_ServerRejectInvalidSteamCertLen" ); return false;
                    }*/
    ...
    // Now that we have auth'd with steam, client->GetSteamID() is now valid and
    // we can verify against the GC lobby
    bool bHasGCLobby =
        g_iServerGameDLLVersion >= 8 && serverGameDLL->GetServerGCLobby();
    if (bHasGCLobby) {
      if (!serverGameDLL->GetServerGCLobby()->SteamIDAllowedToConnect(
              client->m_SteamID)) {
        ISteamGameServer *pSteamGameServer = Steam3Server().SteamGameServer();
        if (pSteamGameServer)
          pSteamGameServer->SendUserDisconnect(client->m_SteamID);
    ...
    if (!Steam3Server().NotifyLocalClientConnect(
            client)) // the userID isn't alloc'd yet so we need to fill it in
                     // manually
    {
...
================
*/
void CBaseServer::RejectConnection(const netadr_t &adr, int clientChallenge,
                                   const char *s) {
  ALIGN4 char msg_buffer[MAX_ROUTABLE_PAYLOAD] ALIGN4_POST;
  bf_write msg(msg_buffer, sizeof(msg_buffer));
...
void CBaseServer::RunFrame(void) {
  ...
  if (m_flLastRandomNumberGenerationTime < 0 ||
      (m_flLastRandomNumberGenerationTime + CHALLENGE_NONCE_LIFETIME) <
          g_ServerGlobalVariables.realtime) {
    m_LastRandomNonce = m_CurrentRandomNonce;

    // RandomInt maps a uniform distribution on the interval [0,INT_MAX], so
    // make two calls to get the random number. RandomInt will always return the
    // minimum value if the difference in min and max is greater than or equal
    // to INT_MAX.
    m_CurrentRandomNonce =
        (((uint32)RandomInt(0, 0xFFFF)) << 16) | RandomInt(0, 0xFFFF);
    m_flLastRandomNumberGenerationTime = g_ServerGlobalVariables.realtime;
  }

  // Timed pause - resume game when time expires
  if (m_flPausedTimeEnd >= 0.f && m_State == ss_paused &&
      Sys_FloatTime() >= m_flPausedTimeEnd) {
...
CBaseClient *CBaseServer::CreateFakeClient(const char *name) {
  ...
  if (sv_stressbots.GetBool()) {
    netadr_t adrNull(0, 0); // 0.0.0.0:0 signifies a bot. It'll plumb all the
                            // way down to winsock calls but it won't make them.
    netchan = NET_CreateNetChannel(m_Socket, &adrNull, adrNull.ToString(),
                                   fakeclient, true);
  }
...
void CBaseServer::Shutdown(void) {
  ...
  for (int i = m_Clients.Count() - 1; i >= 0; i--) {
    ...
    if (cl->IsConnected()) {
      ...
      // shutting down is because the listen server client typed disconnect, in
      // which case we won't call cl->DropClient, but the client might have some
      // frame snapshot references left over, etc.
      cl->Clear();
    }
  ...
  for (int i = 0; i < m_Clients.Count(); i++) {
    ...
    if (!cl->SendNetMsg(msg, reliable)) {
      if (msg.IsReliable() || reliable) {
        DevMsg("BroadcastMessage: Reliable broadcast message overflow for "
               "client %s",
               cl->GetClientName());
      }
    }
...
void CBaseServer::BroadcastMessage(INetMessage &msg, IRecipientFilter &filter) {
  if (filter.IsInitMessage()) {
    ...
    if (IsActive()) {
      ConDMsg("SV_BroadcastMessage: Init message being created after signon "
    ...
    for (int i = 0; i < num; i++) {
      ...
      if (!cl->SendNetMsg(msg)) {
        if (msg.IsReliable()) {
          DevMsg("BroadcastMessage: Reliable filter message overflow for "
                 "client %s",
                 cl->GetClientName());
        }
      }
...
void CBaseServer::WriteTempEntities(CBaseClient *client,
  ...
  VPROF_BUDGET("CBaseServer::WriteTempEntities",
               VPROF_BUDGETGROUP_OTHER_NETWORKING);
...
void CBaseServer::RecalculateTags(void) {
  ...
  // Games without this interface will have no tagged cvars besides
  ...
  if (sv_visiblemaxplayers.GetInt() > 0 &&
      sv_visiblemaxplayers.GetInt() < nMaxReportedClients) {
...
void CBaseServer::SetPausedForced(bool bPaused, float flDuration /*= -1.f*/) {
  ...
  m_flPausedTimeEnd =
      (bPaused && flDuration > 0.f) ? Sys_FloatTime() + flDuration : -1.f;
