...
bool CDispInfo::Render(CGroupMesh *pGroup, bool bAllowDebugModes) {
  ...
  if (bAllowDebugModes) {
    ...
    if (mat_surfaceid.GetInt()) {
      ...
      intp nInt = (mat_surfaceid.GetInt() != 2)
                      ? (intp)m_ParentSurfID
...
typedef void
ProcessLightmapSampleFunc_t(const ProcessLightmapSampleData_t &data,
                            const Vector &vPos, const Vector &vNormal,
                            const Vector &vTangentS, const Vector &vTangentT,
