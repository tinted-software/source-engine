...
static ConVar
    vprof_dump_oninterval("vprof_dump_oninterval", "0", 0,
                          "Interval (in seconds) at which vprof will batch up "
                          "data and dump it to the console.");
// vprof_report_oninterval gives more detail. If both vprof_report_oninterval
// and vprof_dump_oninterval are set then vprof_report_oninterval wins.
static ConVar vprof_report_oninterval(
    "vprof_report_oninterval", "0", 0,
    "Interval (in seconds) at which vprof will batch up a full report to the "
    "console -- more detailed than vprof_dump_oninterval.");
  ...
  if (g_VProfCurrentProfile.IsEnabled()) {
    ...
    if ((s_flIntervalStartTime + flIntervalTime) < flCurrentTime) {
      if (vprof_report_oninterval.GetFloat() != 0.0f) {
        ...
        // called on a timer. Each report can take about 1.5 ms on a fast
        // machine.
  ...
  if (g_VProfCurrentProfile.IsEnabled() && vprof_dump_spikes.GetFloat()) {
    ...
    if (g_VProfSignalSpike || eng->GetFrameTime() > (1.f / spikeThreash)) {
      if (g_VProfSignalSpike ||
        ...
        g_VProfCurrentProfile.OutputReport(
            VPRT_SUMMARY | VPRT_LIST_BY_TIME | VPRT_LIST_BY_TIME_LESS_CHILDREN |
...
// Con commands that are defined with DEFERRED_CON_COMMAND are called by
// PreUpdateProfile() which calls ExecuteDeferredOp(). This ensures that vprof
// operations are done at the appropriate time in the frame loop. Note that only
// one deferred command can be set at a time, so only one deferred command can
...
DEFERRED_CON_COMMAND(vprof_novsync_off,
                     "Leaves vsync on when vxconsole brings up showbudget.") {
...
DEFERRED_CON_COMMAND(vprof_show_cachemiss, "Shows cachemisses in vprof") {
  ...
  g_VProfCurrentProfile.VXConsoleReportMode(
      CVProfile::VXCONSOLE_REPORT_L2CACHE_MISSES);
}

DEFERRED_CON_COMMAND(vprof_show_loadhitstore,
                     "Shows load-hit-stores in vprof") {
  if (!g_fVprofCacheMissOnByUI) {
    Msg("VProf cache miss enabled.\n");
    g_VProfCurrentProfile.PMEEnable(true);
    g_fVprofCacheMissOnByUI = true;
  ...
  g_VProfCurrentProfile.VXConsoleReportMode(
      CVProfile::VXCONSOLE_REPORT_LOAD_HIT_STORE);
}

DEFERRED_CON_COMMAND(vprof_time_scale,
                     "Scale used when displaying time (0 = use default)") {
  float flScale = atof(g_szDefferedArg1);
  if (flScale <= 0.0f) {
    flScale = 1000.0f;
  }
  g_VProfCurrentProfile.VXConsoleReportScale(CVProfile::VXCONSOLE_REPORT_TIME,
                                             flScale);
}
...
DEFERRED_CON_COMMAND(
  ...
  g_VProfCurrentProfile.VXConsoleReportScale(
      CVProfile::VXCONSOLE_REPORT_L2CACHE_MISSES, flScale);
}
...
DEFERRED_CON_COMMAND(
  ...
  g_VProfCurrentProfile.VXConsoleReportScale(
      CVProfile::VXCONSOLE_REPORT_LOAD_HIT_STORE, flScale);
}
  ...
  if (!g_fVprofOnByUI) {
    ...
    if (IsX360() && !g_bVProfNoVSyncOff) {
      ...
      if (mat_vsyncref.GetBool()) {
        Warning("Disabling vsync (via mat_vsync) to increase profiling "
                "accuracy.\n");
...
DEFERRED_CON_COMMAND(vprof_generate_report,
  ...
  g_VProfCurrentProfile.Pause();
  ConsoleLogger consoleLog;
  // This used to generate six different reports, which is expensive and hard to
  // read. Default to two to save time and space.
  g_VProfCurrentProfile.OutputReport(
...
#ifdef _X360
DEFERRED_CON_COMMAND(vprof_360_enable_counters,
                     "Enable 360 L2 and LHS counters for a node") {
  g_VProfCurrentProfile.Pause();
...
DEFERRED_CON_COMMAND(
    vprof_360_enable_counters_recursive,
    "Enable 360 L2 and LHS counters for a node and all subnodes") {
  g_VProfCurrentProfile.Pause();
...
DEFERRED_CON_COMMAND(vprof_360_disable_counters,
                     "Disable 360 L2 and LHS counters for a node. Specify "
                     "'all' to mean all nodes.") {
  g_VProfCurrentProfile.Pause();
...
DEFERRED_CON_COMMAND(
    vprof_360_disable_counters_recursive,
...
DEFERRED_CON_COMMAND(vprof_360_report_counters,
                     "Report L2/LHS info for specified node") {
  g_VProfCurrentProfile.Pause();
  ConsoleLogger consoleLog;

  CVProfNode *pNode = g_VProfCurrentProfile.FindNode(
      g_VProfCurrentProfile.GetRoot(), g_szDefferedArg1);
  if (pNode) {
    Msg("NODE %s\n\tL2 misses: %d\n\tLHS misses: %d\n", g_szDefferedArg1,
        pNode->GetL2CacheMisses(), pNode->GetLoadHitStores());
  } else {
...
DEFERRED_CON_COMMAND(
    vprof_360_cpu_trace_enable,
    "Enable CPU tracing: it will begin when specified node starts, and end "
    "when it stops. Do this before calling vprof_360_cpu_trace_go.") {
  CVProfNode *RESTRICT upon =
      g_VProfCurrentProfile.CPUTraceEnableForNode(g_szDefferedArg1);
  if (upon != NULL) {
    Msg("%s will be traced from start to end. Make sure vprof is enabled, and "
        "enter \nvprof_360_cpu_trace_go <filename> to engage!\n",
        upon->GetName());
  } else {
    Msg("Could not find node %s. Maybe you need to run vprof for a bit so I "
        "can know about that node? Or, you might need to wrap it in "
        "\"double-quotes\". \n",
        g_szDefferedArg1);
  }
}

DEFERRED_CON_COMMAND(vprof_360_cpu_trace_disable,
                     "Disable CPU tracing on all nodes.") {
...
DEFERRED_CON_COMMAND(
    ...
    "syntax: vprof_360_cpu_trace_go <filename>. Will record one CPU trace of "
    "the node specified in vprof_360_cpu_trace_enable, dumping it to "
    "e:/filename.pix2.") {
  if (!g_fVprofOnByUI) {
    Msg("VProf enabled.\n");
    g_VProfCurrentProfile.Start();
  ...
  if (g_VProfCurrentProfile.CPUTraceGetEnabledNode() == NULL ||
      g_VProfCurrentProfile.CPUTraceGetEnabledNode() ==
          g_VProfCurrentProfile.GetRoot()) {
  ...
  if (g_VProfCurrentProfile.CPUTraceGetEnabledNode() != NULL) {
    if (!g_szDefferedArg1[0]) {
      ...
      V_snprintf(g_szDefferedArg1, ARRAYSIZE(g_szDefferedArg1),
                 "vprof_%d_%d_%d_%d_%d_%d", systemTime.wMonth, systemTime.wDay,
                 systemTime.wHour, systemTime.wMinute, systemTime.wSecond,
                 systemTime.wMilliseconds);
    }
    const char *filename =
        g_VProfCurrentProfile.SetCPUTraceFilename(g_szDefferedArg1);
    g_VProfCurrentProfile.SetCPUTraceEnabled(CVProfile::kFirstHitNode);
    Msg("Trace will be written to %s\n", filename);
  } else {
    Msg("Please specify a node to profile with vprof_360_cpu_trace_enable "
        "<node>.\n");
  }
}
...
DEFERRED_CON_COMMAND(
    ...
    "syntax: vprof_360_cpu_trace_go_repeat <filename>. For each time the node "
    "specified in vprof_360_cpu_trace_enable is hit during the next frame, "
    "dump a CPU trace to e:/filenameXXXX.pix2.") {
  if (g_VProfCurrentProfile.CPUTraceGetEnabledNode() != NULL) {
    const char *filename =
        g_VProfCurrentProfile.SetCPUTraceFilename(g_szDefferedArg1);
    g_VProfCurrentProfile.SetCPUTraceEnabled(
        CVProfile::kAllNodesInFrame_WaitingForMark);
    Msg("Trace will be written to %s%.4d ... \n", filename,
        g_VProfCurrentProfile.GetMultiTraceIndex());
  } else {
    Msg("Please specify a node to profile with vprof_360_cpu_trace_enable "
        "<node>.\n");
  }
...
public:
  ...
  virtual void
  GetBudgetGroupTimes(float times[IVProfExport::MAX_BUDGETGROUP_TIMES]) {
    int nTotalGroups =
        min(m_Times.Count(), GetActiveVProfile()->GetNumBudgetGroups());
...
public:
  ...
  void CalculateBudgetGroupTimes_Recursive(CVProfNode *pNode) {
    ...
    if (groupID >= 0 &&
        groupID <
            min(m_Times.Count(), (int)IVProfExport::MAX_BUDGETGROUP_TIMES)) {
...
private:
  ...
  int m_nListeners;
  int m_BudgetFlagsFilter; // We can only capture one type of filtered data at a
                           // time.
  bool m_bStart;
  bool m_bStop;
};
