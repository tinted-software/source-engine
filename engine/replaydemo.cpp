...
CReplayDemoRecorder::~CReplayDemoRecorder() { StopRecording(); }
...
void CReplayDemoRecorder::StartRecording(const char *pFilename,
                                         bool bContinuously) {
  ...
  const int nBufferSize =
      1024 * 1024 *
      (replay_buffersize.IsValid() ? replay_buffersize.GetInt() : 16);
  ...
  // Using this tickcount allows us to sync up client-side recorded ragdolls
  // later with replay demos on clients
  ...
  g_pServerReplayContext->GetSessionRecorder()->SetCurrentRecordingStartTick(
      m_nStartTick);
}

bool CReplayDemoRecorder::IsRecording() { return m_bIsRecording; }
...
void CReplayDemoRecorder::StopRecording() {
  ...
  // replay_stoprecording gets set to 0 from within the replay session recorder,
  // but only if we aren't starting to record a new round
}

CDemoFile *CReplayDemoRecorder::GetDemoFile() { return &m_DemoFile; }
...
int CReplayDemoRecorder::WriteSignonData() {
  ...
  msg.WriteBits(m_pReplayServer->m_Signon.GetData(),
                m_pReplayServer->m_Signon.GetNumBitsWritten());
...
void CReplayDemoRecorder::WriteFrame(CReplayFrame *pFrame) {
  ...
  // get delta frame
  CClientFrame *deltaFrame = m_pReplayServer->GetClientFrame(
      m_nDeltaTick); // NULL if m_nDeltaTick is not found or -1

  // send entity update, delta compressed if deltaFrame != NULL
  sv.WriteDeltaEntities(m_pReplayServer->m_MasterClient, pFrame, deltaFrame,
                        msg);
  ...
  sv.WriteTempEntities(m_pReplayServer->m_MasterClient, pFrame->GetSnapshot(),
