...
static ConVar
    tv_nochat("tv_nochat", "0", FCVAR_ARCHIVE | FCVAR_USERINFO,
...
static ConVar
    cl_ignorepackets("cl_ignorepackets", "0", FCVAR_CHEAT,
...
static ConVar cl_block_usercommand(
    "cl_block_usercommand", "0", FCVAR_CHEAT,
...
ConVar dev_loadtime_map_elapsed("dev_loadtime_map_elapsed", "0.0",
                                FCVAR_HIDDEN);
...
void CL_ReloadFilesInList(IFileList *pFilesToReload) {
  ...
  // Handle models.. NOTE: this MUST come after materials->ReloadFilesInList
  // because the models need to know which materials got flushed.
...
void PrintSvPureWhitelistClassification(
    const CPureServerWhitelist *pWhiteList) {
  if (pWhiteList == NULL) {
  ...
  CPureServerWhitelist *pStandardList =
      CPureServerWhitelist::Create(g_pFullFileSystem);
  pStandardList->Load(0);
  if (*pStandardList == *pWhiteList) {
    Msg("The server is using sv_pure 0.  (Enforcing consistency for select "
        "files only)\n");
  } else {
...
const CPrecacheUserData *CL_GetPrecacheUserData(INetworkStringTable *table,
  ...
  const CPrecacheUserData *data =
      (CPrecacheUserData *)table->GetStringUserData(index, &testLength);
  if (data) {
    ErrorIfNot(testLength == sizeof(*data),
               ("CL_GetPrecacheUserData(%d,%d) - length (%d) invalid.",
                table->GetTableId(), index, testLength));
  }
  return data;
}
  ...
  if (!initialized) {
    if (Steam3Client().SteamApps() &&
      ...
      if (VCRGetMode() != VCR_Playback) {
        s_bIsHL2Demo = !Steam3Client().SteamApps()->BIsSubscribedApp(
...
void CL_InitPortalDemoFlag() {
  ...
  if (!initialized) {
    if (Steam3Client().SteamApps() &&
      ...
      if (VCRGetMode() != VCR_Playback) {
        s_bIsPortalDemo = !Steam3Client().SteamApps()->BIsSubscribedApp(
...
void CL_DispatchSound(const SoundInfo_t &sound) {
  ...
  if (sound.bIsSentence) {
    ...
    const char *pchTranslatedName =
        g_ClientDLL->TranslateEffectForVisionFilter("sounds", name);
  ...
  if (snd_show.GetInt() >= 2) {
    DevMsg("%i (seq %i) %s : src %d : ch %d : %d dB : vol %.2f : time %.3f "
           "(%.4f delay) @%.1f %.1f %.1f\n",
...
#endif
      if (params.delay <= 0) {
        // leave a little delay to flag the channel in the low-level sound
        // system
        params.delay = 1e-6f;
      }
    } else {
  ...
  // Give the client DLL a chance to run arbitrary code to affect the sound
  // parameters before we play.
...
void CL_Retry() {
  ...
  // We need to temporarily disable this execution marker so the connect command
  // succeeds if it was executed by the server. We would still need this even if
  // we called CL_Connect directly because the connect process may execute
  ...
  if (cl.m_bRestrictServerCommands && bCanAddExecutionMarkers)
    Cbuf_AddTextWithMarkers(
        eCmdExecutionMarker_Disable_FCVAR_SERVER_CAN_EXECUTE, pszCommand,
...
void CL_Connect(const char *address, const char *pszSourceTag) {
  ...
  // This happens as part of the load process anyway, but on slower systems it
  // causes the server to timeout the connection.  Use the opportunity to flush
  // anything before starting a new connection.
...
CON_COMMAND_F(connect, "Connect to specified server.", FCVAR_DONTRECORD) {
  ...
  // and we donly want spaces to count.  So we'll need to re-split the arg
  // string
...
CON_COMMAND_F(redirect, "Redirect client to specified server.",
              FCVAR_DONTRECORD | FCVAR_SERVER_CAN_EXECUTE) {
  if (!CBaseClientState::ConnectMethodAllowsRedirects()) {
    ConMsg("redirect: Current connection method does not allow silent "
           "redirects.\n");
    return;
  }
...
void CL_RegisterResources(void) {
  ...
  if (!host_state.worldmodel) {
    Host_Error("CL_RegisterResources:  host_state.worldmodel/cl.GetModel( 1 "
               ")==NULL\n");
  }
...
void CL_FullyConnected(void) {
  ...
  if (IsX360()) {
    // Notify the loader the end of the loading context, preloads are about to
    // be purged
  ...
  // We initialize this list before the load, but don't perform reloads until
  // after flushes have happened to avoid unnecessary reloads of items that wont
  // be used on this map.
  ...
  if (IsX360()) {
    // Reset material system temporary memory (once loading is complete), ready
    // for in-map use
...
#ifdef _X360
  // At this point, check for a valid controller connection.  If it's been lost,
  // then we need to pop our game UI up
  XINPUT_CAPABILITIES caps;
  if (XInputGetCapabilities(XBX_GetPrimaryUserId(), XINPUT_FLAG_GAMEPAD,
...
void CL_NextDemo(void) {
  ...
  Q_snprintf(str, sizeof(str), "%s %s",
             CommandLine()->FindParm("-timedemoloop") ? "timedemo" : "playdemo",
...
CON_COMMAND_F(
    devshots_screenshot,
    "Used by the -makedevshots system to take a screenshot. For taking your "
    "own screenshots, use the 'screenshot' command instead.",
    FCVAR_DONTRECORD) {
  CL_TakeScreenshot(NULL);
...
ConCommand
    screenshot_internal_command("__screenshot_internal", screenshot_internal,
                                "Internal command to take a screenshot without "
                                "renumbering or notifying Steam.",
...
void CL_TakeSnapshotAndSwap() {
  ...
  if (cl_takesnapshot) {
    // Disable threading for the duration of the screenshots, because we need to
    // get pointers to the (complete) back buffer right now.
    ...
    if (cl_takesnapshot_internal) {
      ...
      Q_snprintf(filename, sizeof(filename), "screenshots/%s.jpg",
                 cl_snapshotname);
    ...
    // Restore threading if it was previously enabled (if it wasn't this will do
    // nothing).
...
void CL_StartMovie(const char *filename, int flags, int nWidth, int nHeight,
                   float flFrameRate, int nJpegQuality,
...
#define SOUND_DMA_SPEED 44100 // hardware playback rate
    ...
    if (videoSystem == VideoSystem::NONE && g_pVideo) {
      // Find a video system based on features if they didn't specify a specific
      // one.
    ...
    if (!cl_simulate_no_quicktime.GetBool() && g_pVideo &&
        videoSystem != VideoSystem::NONE) {
      ...
      if (g_pVideoRecorder != NULL) {
        ...
        const int nSize = 256;
        CFmtStrN<nSize> fmtFullFilename("%s%c%s", com_gamedir,
                                        CORRECT_PATH_SEPARATOR, filename);
        ...
        g_pVideoRecorder->CreateNewMovieFile(szFullFilename,
                                             cl_movieinfo.DoVideoSound());
#ifdef USE_WEBM_FOR_REPLAY
        g_pVideoRecorder->SetMovieVideoParameters(VideoEncodeCodec::WEBM_CODEC,
                                                  nJpegQuality, nWidth, nHeight,
                                                  theFps);
#else
        g_pVideoRecorder->SetMovieVideoParameters(
            VideoEncodeCodec::DEFAULT_CODEC, nJpegQuality, nWidth, nHeight,
            theFps);
#endif

        if (cl_movieinfo.DoVideo()) {
          g_pVideoRecorder->SetMovieSourceImageParameters(
              VideoEncodeSourceFormat::BGR_24BIT, nWidth, nHeight);
        }
...
#else
    Warning("Failed to launch startmovie!  If you are trying to use h264, "
            "please make sure you have QuickTime installed.\n");
#endif
    CL_EndMovie();
  }
}
...
void CL_EndMovie() {
  ...
  if (g_pVideoRecorder &&
      (cl_movieinfo.DoVideo() || cl_movieinfo.DoVideoSound())) {
...
CON_COMMAND_F(startmovie, "Start recording movie frames.", FCVAR_DONTRECORD) {
  ...
  if (args.ArgC() < 2) {
...
#else
    ConMsg(" h264 = H.264-encoded audio and video (must have QuickTime "
           "installed!)\n");
#endif
    ...
    ConMsg(" jpeg_quality nnn = set jpeq quality to nnn (range 1 to 100), "
           "default %d\n",
  ...
  if (args.ArgC() > 2) {
    ...
    for (int i = 2; i < args.ArgC(); ++i) {
      if (!Q_stricmp(args[i], "avi")) {
...
#else
        Warning("AVI is no longer supported!  Make sure QuickTime is installed "
                "and use \"h264\" - if you install QuickTime, you will need to "
                "reboot before using startmovie.\n");
#endif
        return;
      }
#ifdef USE_WEBM_FOR_REPLAY
        ...
        Warning("WebM is not supported on this platform!  Make sure QuickTime "
                "is installed and use \"h264\" - if you install QuickTime, you "
                "will need to reboot before using startmovie.\n");
        return;
      }
#endif
  ...
  if (flags == 0) {
#ifdef USE_WEBM_FOR_REPLAY
    Warning("Missing or unknown recording types, must specify one or both of "
            "'webm' or 'raw'\n");
#else
    Warning("Missing or unknown recording types, must specify one or both of "
            "'h264' or 'raw'\n");
  ...
  CL_StartMovie(args[1], flags, videomode->GetModeStereoWidth(),
                videomode->GetModeStereoHeight(), flFrameRate, nJpegQuality,
                videoSystem);
  ConMsg("Started recording movie, frames will record after console is "
         "cleared...\n");
}
...
bool CL_ShouldLoadBackgroundLevel(const CCommand &args) {
  ...
  if (args.ArgC() == 2) {
    ...
    if (IsX360()) {
      // 360 needs to get UI in the correct state to transition to the
      // Background level from the credits.
...
void CL_SetSteamCrashComment() {
  ...
  Q_snprintf(
      ...
      info.m_SubSysID, info.m_Revision, dxlevel ? dxlevel : "Unk", nDxLevel,
  ...
  Q_snprintf(
      ...
      "waterforcereflectentities: %i mat_motion_blur_enabled: %i",
      mat_picmip.GetInt(), mat_forceaniso.GetInt(), mat_trilinear.GetInt(),
      mat_antialias.GetInt(), mat_aaquality.GetInt(), mat_vsync.GetInt(),
      r_rootlod.GetInt(), mat_reducefillrate.GetInt(),
  ...
  Q_snprintf(videoinfo, sizeof(videoinfo),
             ...
             mat_trilinear.GetInt(), mat_antialias.GetInt(), mat_vsync.GetInt(),
             r_rootlod.GetInt(), mat_reducefillrate.GetInt(),
             r_shadowrendertotexture.GetInt(),
...
// register commands
//
static ConCommand startupmenu(
    "startupmenu", &CL_CheckToDisplayStartupMenus,
    "Opens initial menu screen and loads the background bsp, but only if no "
    "other level is being loaded, and we're not in developer mode.");
...
void CL_ChangeCloudSettingsCvar(IConVar *var, const char *pOldValue,
                                float flOldValue);
ConVar cl_cloud_settings("cl_cloud_settings", "1", FCVAR_HIDDEN,
                         "Cloud enabled from (from "
                         "HKCU\\Software\\Valve\\Steam\\Apps\\appid\\Cloud)",
...
void CL_ChangeCloudSettingsCvar(IConVar *var, const char *pOldValue,
  ...
  if (IsPC() && Steam3Client().SteamRemoteStorage()) {
    ...
    if (cl_cloud_settings.GetInt() == STEAMREMOTESTORAGE_CLOUD_ON &&
        flOldValue == STEAMREMOTESTORAGE_CLOUD_OFF) {
...
void CL_InitCloudSettingsCvar() {
  if (IsPC() && Steam3Client().SteamRemoteStorage()) {
    ...
    // If not on PC or steam not available, set to 0 to make sure no replication
...
CON_COMMAND_F(setinfo, "Adds a new user info value",
              FCVAR_CLIENTCMD_CAN_EXECUTE) {
  if (args.ArgC() != 3) {
  ...
  // Prevent players manually changing their name (their Steam account provides
  // it now)
  ...
  if (bFunky) {
    Msg("Ignoring convar change request for variable '%s', which contains "
        "invalid character(s)\n",
        name);
    return;
  }
    ...
    if (pCommand->IsFlagSet(FCVAR_NOT_CONNECTED)) {
      ...
      if (cl.IsConnected()) {
        ...
        if (pCommand->IsFlagSet(FCVAR_USERINFO) && g_ClientDLL &&
          ...
          ConMsg("Can't change %s when playing, disconnect from the server or "
                 "switch team to spectators\n",
                 pCommand->GetName());
          return;
        }
      }
...
void Callback_DynamicModelsChanged(void *object,
                                   INetworkStringTable *stringTable,
                                   int stringNumber, const char *newString,
                                   const void *newData) {
#ifndef SWDS
  ...
  if (modelinfoclient) {
    modelinfoclient->OnDynamicModelsStringTableChange(stringNumber, newString,
                                                      newData);
  }
#endif
}
