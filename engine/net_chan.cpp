...
ConVar net_blocksize("net_maxfragments", NETSTRING(MAX_ROUTABLE_PAYLOAD), 0,
...
static ConVar net_compresspackets_minsize(
    "net_compresspackets_minsize", "1024", 0,
    "Don't bother compressing packets below this size.");
static ConVar
    net_maxcleartime("net_maxcleartime", "4.0", 0,
                     "Max # of seconds we can wait for next packets to be sent "
                     "based on rate setting (0 == no limit).");
static ConVar net_maxpacketdrop("net_maxpacketdrop", "5000", 0,
                                "Ignore any packets with the sequence number "
                                "more than this ahead (0 == no limit)");
...
static bool ShouldChecksumPackets() { return NET_IsMultiplayer(); }
...
void CNetChan::Clear() {
  ...
  if (m_bProcessingMessages) {
    // ProcessMessages() needs to know we just nuked the receive list from under
    // it or bad things ensue.
...
void CNetChan::CompressFragments() {
  ...
  // if it uses only the first couple bits of the last byte in the message). If
  // the last few bits are different, it can produce a different compressed
  // size.
  ...
  VPROF_BUDGET("CNetChan::CompressFragments",
  ...
  for (int i = 0; i < MAX_STREAMS; i++) {
    ...
    if (data->isCompressed ||
        (int)data->bytes < net_compresspackets_minsize.GetInt())
      continue;
    ...
    if (data->buffer) {
      ...
      unsigned int compressedSize =
          COM_GetIdealDestinationCompressionBufferSize_Snappy(data->bytes);
      char *compressedData = new char[compressedSize];

      if (COM_BufferToBufferCompress_Snappy(compressedData, &compressedSize,
    ...
    } else                     // it's a file
    {
      ...
      if (hZipFile != FILESYSTEM_INVALID_HANDLE) {
        ...
        if (COM_BufferToBufferCompress_Snappy(compressed, &compressedSize,
...
void CNetChan::UncompressFragments(dataFragments_t *data) {
  ...
  // uncompress data
  COM_BufferToBufferDecompress(newbuffer, &uncompressedSize, data->buffer,
                               data->bytes);
...
void CNetChan::Setup(int sock, netadr_t *adr, const char *name,
                     INetChannelHandler *handler, int nProtocolVersion) {
  Assert(name);
  Assert(handler);
  ...
  SetMaxBufferSize(false, NET_MAX_DATAGRAM_PAYLOAD, true); // Set up voice
                                                           // buffer
...
bool CNetChan::CreateFragmentsFromBuffer(bf_write *buffer, int stream) {
  VPROF_BUDGET("CNetChan::CreateFragmentsFromBuffer",
               VPROF_BUDGETGROUP_OTHER_NETWORKING);
...
bool CNetChan::CreateFragmentsFromFile(const char *filename, int stream,
  ...
  if (totalBytes >= (net_maxfilesize.GetInt() * 1024 * 1024)) {
    ConMsg("CreateFragmentsFromFile: '%s' size exceeds net_maxfilesize limit "
           "(%i MB).\n",
...
bool CNetChan::SendSubChannelData(bf_write &buf) {
  VPROF_BUDGET("CNetChan::SendSubChannelData",
               VPROF_BUDGETGROUP_OTHER_NETWORKING);
...
bool CNetChan::ReadSubChannelData(bf_read &buf, int stream) {
  ...
  if (offset == 0) // first fragment, read header info
  {
    ...
    if (data->bytes > MAX_FILE_SIZE) {
      // This can happen with the compressed path above, which uses VarInt32
      // rather than MAX_FILE_SIZE_BITS
      Warning("Net message exceeds max size (%u / %u)\n", MAX_FILE_SIZE,
              data->bytes);
      // Subsequent packets for this transfer will treated as invalid since we
      // never setup a buffer.
      return false;
    }
  ...
  if ((startFragment + numFragments) == data->numFragments) {
    ...
    // a malicious client can send a fragment beyond what was arranged in
    // fragment#0 header old code will overrun the allocated buffer and likely
    // cause a server crash it could also cause a client memory overrun because
    // the offset can be anywhere from 0 to 16MB range drop the packet and wait
    // for client to retry
  ...
  if (length == 0 || (offset + length > data->bytes)) {
    ...
    ConMsg("Malformed fragment ofs %i len %d, buffer size %d from %s\n", offset,
           length, PAD_NUMBER(data->bytes, 4), remote_address.ToString());
    return false;
  }
...
bool CNetChan::ProcessMessages(bf_read &buf) {
  ...
  bf_read democopy =
      buf; // create a copy of reading buffer state for demo recording
  ...
  while (true) {
    ...
    if (netmsg) {
      ...
      UpdateMessageStats(netmsg->GetGroup(),
                         buf.GetNumBitsRead() - nMsgStartBit);
...
CON_COMMAND(netchan_test_upload, "[filename]: Uploads a file to server.") {
  ...
  //$ TODO: the con command system is truncating the filenames we're passing in.
  //Need to workaround this...
  const char *filename =
      args.GetCommandString() + V_strlen("netchan_test_upload ");
...
bool CNetChan::TestUpload(const char *filename) {
  ...
  data.pendingFragments =
      0; // number of fragments send, but not acknowledged yet
...
bool CNetChan::HandleUpload(dataFragments_t *data,
  ...
  if (!s_pAllowUpload || !s_pAllowUpload->GetBool()) {
    ...
    // Make sure that this file is not being written to a location above the
    // current directory, isn't in writing to any locations we don't want, isn't
    // an unsupported
...
int CNetChan::ProcessPacketHeader(netpacket_t *packet) {
  ...
  if (net_maxpacketdrop.GetInt() > 0 &&
      m_PacketDrop > net_maxpacketdrop.GetInt()) {
    if (net_showdrop.GetInt()) {
  ...
  ETWReadPacket(packet->from.ToString(), packet->wiresize, m_nInSequenceNr,
                m_nOutSequenceNr);
...
void CNetChan::ProcessPacket(netpacket_t *packet, bool bHasHeader) {
  ...
  if (net_showudp.GetInt() && net_showudp.GetInt() != 3) {
    ConMsg("UDP <- %s: sz=%i seq=%i ack=%i rel=%i ch=%d, tm=%f rt=%f wire=%i\n",
           ...
           m_nOutSequenceNrAck & 63, flags & PACKET_FLAG_RELIABLE ? 1 : 0,
           flags & PACKET_FLAG_CHALLENGE ? 1 : 0, net_time,
           (float)Plat_FloatTime(), packet->wiresize);
  }
...
bool CNetChan::ProcessStream(void) {
  ...
  if ((m_SteamType == STREAM_CMD_DATA) && (m_StreamLength == 0)) {
    ...
    const int cMaxPayload = GetProtocolVersion() > PROTOCOL_VERSION_23
                                ? NET_MAX_PAYLOAD
                                : NET_MAX_PAYLOAD_V23;
...
bool CNetChan::IsValidFileForTransfer(const char *pszFilename) {
  ...
  if (V_stristr(szTemp, "lua/") || V_stristr(szTemp, "gamemodes/") ||
      V_stristr(szTemp, "addons/") || V_stristr(szTemp, "~/") ||
      // V_stristr( szTemp, "//" ) || 		// Don't allow '//'. TODO: Is
      // this check ok?
      V_stristr(szTemp, "./././") || // Don't allow folks to make crazy long
                                     // paths with ././././ stuff.
      V_stristr(szTemp, "   ") ||    // Don't allow multiple spaces or tab (was
                                     // being used for an exploit).
      V_stristr(szTemp, "\t")) {
    return false;
  }

  // If .exe or .EXE or these other strings exist _anywhere_ in the filename,
  // reject it.
  if (V_stristr(szTemp, ".cfg") || V_stristr(szTemp, ".lst") ||
      V_stristr(szTemp, ".exe") || V_stristr(szTemp, ".vbs") ||
      V_stristr(szTemp, ".com") || V_stristr(szTemp, ".bat") ||
      V_stristr(szTemp, ".cmd") || V_stristr(szTemp, ".dll") ||
      V_stristr(szTemp, ".so") || V_stristr(szTemp, ".dylib") ||
      V_stristr(szTemp, ".ini") || V_stristr(szTemp, ".log") ||
      V_stristr(szTemp, ".lua") || V_stristr(szTemp, ".vdf") ||
      V_stristr(szTemp, ".smx") || V_stristr(szTemp, ".gcf") ||
      V_stristr(szTemp, ".lmp") || V_stristr(szTemp, ".sys")) {
    return false;
  }
  ...
  if ((extension_len != 3) && (extension_len != 4) &&
      V_stricmp(extension, ".bsp.bz2") && V_stricmp(extension, ".xbox.vtx") &&
      V_stricmp(extension, ".dx80.vtx") && V_stricmp(extension, ".dx90.vtx") &&
