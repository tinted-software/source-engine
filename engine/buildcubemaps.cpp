...
static void TakeCubemapSnapshot(const Vector &origin, const char *pFileNameBase,
  ...
  if (bPFM) {
    ...
    if (g_pMaterialSystemHardwareConfig->GetHDRType() == HDR_TYPE_INTEGER) {
      ...
      // 			Q_snprintf( name, sizeof( name ), "%s_filtered",
      // pFileNameBase); 			OutEnvmap.WritePFMs( name );
    }
...
CON_COMMAND(envmap, "") {
  ...
  TakeCubemapSnapshot(MainViewOrigin(), str, mat_envmapsize.GetInt(),
...
CON_COMMAND(lightprobe,
  ...
  if (args.ArgC() < 2) {
    ConMsg("sample_lighting usage: lightprobe <base file name> [cubemap "
           "dimension]\n");
    return;
  }
...
static bool LoadSrcVTFFiles(IVTFTexture *pSrcVTFTextures[6],
  ...
  for (i = 0; i < 6; i++) {
    ...
    if (((pSrcVTFTextures[i]->Width() != pSrcVTFTextures[0]->Width()) &&
      ...
      Warning("*** Error: Skybox vtf files for %s weren't compiled with the "
      ...
      for (int iMip = 0; iMip < pDstCubemap->MipCount(); ++iMip) {
                   ...
                   pSrcVTFTextures[iFace]->Height()) // If texture is square
        {
          if (iSrcMipSize != iSize) {
            Warning("%s - ERROR! Cannot copy square face for default cubemap! "
                       ...
                       2) // If texture is rectangle 2x wide
        {
          ...
          if ((iMipHeight > 1) && (iSrcMipSize * 2 != iSize)) {
            ...
            if (nSrcRowSize != nDstRowSize) {
              Warning("%s - ERROR building default cube map! nSrcRowSize(%d) "
                      "!= nDstRowSize(%d)!\n",
          ...
          Warning("%s - Couldn't create default cubemap because texture res is "
                  "%dx%d\n",
                  pSkyboxBaseName, pSrcVTFTextures[iFace]->Width(),
                  pSrcVTFTextures[iFace]->Height());
          memset(pDstBits, 0, iSize);
          return;
        }
...
static void AddSampleToBSPFile(bool bHDR, mcubemapsample_t *pSample,
  ...
  if (!g_pFileSystem->RelativePathToFullPath_safe(
          textureName, "DEFAULT_WRITE_PATH", localPath) ||
      !*localPath) {
...
// Added these to seperate from R_BuildCubemapSamples to a) clean it up abit and
// b) fix the issue where if it fails, mouse is disabled.
...
void R_BuildCubemapSamples_PreBuild() {
  ...
  //"r_drawtranslucentworld" ); 	ConVar *r_drawtranslucentrenderables = ( ConVar
  //* )cv->FindVar( "r_drawtranslucentrenderables" );
...
void R_BuildCubemapSamples(int numIterations) {
  ...
  for (bounce = 0; bounce < numIterations; bounce++) {
    ...
    Q_snprintf(matDir, sizeof(matDir), "materials/maps/%s",
               cl.m_szLevelBaseName);
    g_pFileSystem->CreateDirHierarchy(matDir, "DEFAULT_WRITE_PATH");

    char pTemp[MAX_PATH];
    Q_snprintf(pTemp, sizeof(pTemp), "materialsrc/maps/%s",
               cl.m_szLevelBaseName);
    ...
    bool bSupportsHDR =
        true; // g_pMaterialSystemHardwareConfig->GetHDRType() != HDR_TYPE_NONE;
    ...
    // Resolve levelfilename to absolute to ensure we are writing the exact file
    // we loaded and not preferentially to DEFAULT_WRITE_PATH
    char szAbsFile[MAX_PATH] = {0};
    g_pFullFileSystem->RelativePathToFullPath(cl.m_szLevelFileName, NULL,
                                              szAbsFile, sizeof(szAbsFile));
    if (!*szAbsFile) {
...
CON_COMMAND(buildcubemaps, "Rebuild cubemaps.") {
  ...
  // do this to force a frame to render so the material system syncs up to
  // single thread mode
  V_RenderVGuiOnly();
  if (args.ArgC() == 1) {
