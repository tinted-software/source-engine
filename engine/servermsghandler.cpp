...
bool CClientState::ProcessVoiceData(SVC_VoiceData *msg) {
  ...
  // sound engine).
  Voice_AddIncomingData(nChannel, chReceived, Bits2Bytes(bitsRead),
                        m_nCurrentSequence);
#endif
  return true;
};
...
void CClientState::ProcessSoundsWithProtoVersion(
    SVC_Sounds *msg, CUtlVector<SoundInfo_t> &sounds, int nProtoVersion) {
  ...
  // Reserve this memory up front so it doesn't realloc under pDeltaSound
  // pointing at it
...
bool CClientState::ProcessSounds(SVC_Sounds *msg) {
  if (msg->m_DataIn.IsOverflowed()) {
    // Overflowed before we even started! There's nothing we can do with this
    // buffer.
    return false;
  }
  ...
  ProcessSoundsWithProtoVersion(msg, sounds,
                                g_ClientGlobalVariables.network_protocol);
  ...
  if (msg->m_nLength != nRelativeBitsRead || msg->m_DataIn.IsOverflowed()) {
    ...
    // If the demo file thinks it's version 18 or 19, it might actually be the
    // other. This is a work around for when we broke compatibility Halloween
    // 2011. -Jeep
    if (g_ClientGlobalVariables.network_protocol == PROTOCOL_VERSION_18) {
      nFallbackProtocol = PROTOCOL_VERSION_19;
    } else if (g_ClientGlobalVariables.network_protocol ==
               PROTOCOL_VERSION_19) {
      nFallbackProtocol = PROTOCOL_VERSION_18;
    }

    if (nFallbackProtocol != 0) {
      // Roll back our buffer to before we read those bits and wipe the overflow
      // flag
      msg->m_DataIn.Reset();
...
bool CClientState::ProcessBSPDecal(SVC_BSPDecal *msg) {
  ...
  if (msg->m_nEntityIndex) {
    ...
    if (!model) {
      Warning("ProcessBSPDecal:  Trying to project on world before "
  ...
  if (model == NULL) {
    ...
    Warning("Warning! Static BSP decal (%s), on NULL model index %i for entity "
            "index %i.\n",
...
bool CClientState::ProcessPacketEntities(SVC_PacketEntities *msg) {
  if (!msg->m_bIsDelta) {
    ...
    // Preprocessing primarily does client prediction. So if we're processing
    // deltas--do it otherwise, we're about to be told exactly what the state of
    // everything is--so skip it.
...
bool CClientState::ProcessTempEntities(SVC_TempEntities *msg) {
  ...
  for (int i = 0; i < msg->m_nNumEntries; i++) {
    ...
    // deltaBitsReader.ReadNextPropIndex reads uint32s, so make sure we alloc in
    // 4-byte chunks.
