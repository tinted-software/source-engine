...
ConVar mem_max_heapsize_dedicated(
    ...
    "Maximum amount of memory to dedicate to engine hunk and datacache, for "
    "dedicated server (in mb)");
...
void Con_ColorPrintf(const Color &clr, PRINTF_FORMAT_STRING const char *fmt,
                     ...) FMTFUNCTION(2, 3);
...
void Sys_Printf(char *fmt, ...) {
  ...
  if (developer.GetInt()) {
    ...
    ::MultiByteToWideChar(CP_UTF8, 0, text, -1, unicode,
                          sizeof(unicode) / sizeof(wchar_t));
...
bool Sys_MessageBox(const char *title, const char *info,
  ...
  if (IDOK == ::MessageBox(NULL, title, info,
                           MB_ICONEXCLAMATION |
...
void Sys_Error_Internal(bool bMinidump, const char *error, va_list argsList) {
  ...
  if (IsPC() && !CommandLine()->FindParm("-makereslists") &&
  ...
  if (bMinidump && !Plat_IsInDebugSession() &&
      !CommandLine()->FindParm("-nominidumps")) {
#if defined(WIN32)
...
#elif defined(POSIX)
    // Doing this doesn't quite work the way we want because there is no
    // "crashing" thread and we see "No thread was identified as the cause of
    // the crash; No signature could be created because we do not know which
    // thread crashed" on the back end
...
static ConVar
    sys_minidumpspewlines("sys_minidumpspewlines", "500", 0,
...
static void AddSpewRecord(char const *pMsg) {
  ...
  g_SpewHistory[i].Format("%d(%f):  %s", g_nSpewLines++, Plat_FloatTime(),
                          pMsg);
...
void GetSpew(char *buf, size_t buflen) {
  ...
  // Walk backward(
  for (int i = g_SpewHistory.Tail(); i != g_SpewHistory.InvalidIndex();
       i = g_SpewHistory.Previous(i)) {
...
//-----------------------------------------------------------------------------
// Purpose: factory comglomerator, gets the client, server, and gameui dlls
// together
//-----------------------------------------------------------------------------
...
int Sys_InitGame(CreateInterfaceFn appSystemFactory, const char *pBaseDir,
  ...
  if (CommandLine()->FindParm("-pidfile")) {
    FileHandle_t pidFile = g_pFileSystem->Open(
...
static bool LoadThisDll(char *szDllFilename, bool bIsServerOnly) {
  ...
  // check signature, don't let users with modified binaries connect to secure
  // servers, they will get VAC banned
  ...
  if ((pDLL = g_pFileSystem->LoadModule(szDllFilename, "GAMEBIN", false)) ==
      NULL) {
  ...
  if (g_ServerFactory) {
    ...
    for (;;) {
      ...
      if (g_iServerGameDLLVersion < 4) {
        g_iServerGameDLLVersion = 0;
        Msg("Could not get IServerGameDLL interface from library %s",
            szDllFilename);
        goto IgnoreThisDLL;
      }
    }
    ...
    serverGameTags = (IServerGameTags *)g_ServerFactory(
        INTERFACEVERSION_SERVERGAMETAGS, NULL);
...
void Sys_GetRegKeyValueUnderRoot(HKEY rootKey, const char *pszSubKey,
                                 ...
                                 OUT_Z_CAP(nReturnLength) char *pszReturnString,
                                 int nReturnLength,
...
void Sys_GetRegKeyValueInt(const char *pszSubKey, const char *pszElement,
