...
CON_COMMAND(mat_get_textures, "VXConsole command") {
  ...
  g_pMaterialSystemDebugTextureInfo->EnableGetAllTextures(
      args.ArgC() >= 2 && (Q_stricmp(args[1], "all") == 0));
}
#endif

static ConVar
    mat_texture_list("mat_texture_list", "0", FCVAR_CHEAT,
...
ConVar mat_texture_list_all("mat_texture_list_all", "0",
...
static IMaterial *UseDebugMaterial(char const *szMaterial,
  ...
  if (foundVar && FrameVar) {
    ...
    if (pMatTexture->IsRenderTarget() || pMatTexture->IsProcedural())
      numAnimFrames =
          0; // Cannot use the stencil parts of the render targets and shouldn't
             // use the other frames of procedural textures
...
CON_COMMAND_F(mat_texture_list_txlod,
  ...
  Warning("Usage: 'mat_texture_list_txlod +1' to inc lod | "
          "'mat_texture_list_txlod -1' to dec lod\n");
}
...
struct ViewParamsLast {
  ViewParamsLast()
      : lstMaterials(DefLessFunc(UtlSymId_t)), flTime(0.f),
        bHighlighted(false) {}
    ...
    if (idx == s_viewParamsLast.lstMaterials.InvalidIndex()) {
      ...
      if (pMat) {
        ...
        for (int idxParam = 0; idxParam < numParams; ++idxParam) {
          ...
          // arrVars[ idxParam ]->GetIntValue() ? true : false; 						arrVars[
          // idxParam ]->SetIntValue( 1 ); 						continue;
          // 					}
  ...
  for (MapIdx idx = s_viewParamsLast.lstMaterials.FirstInorder();
    ...
    if (pMat) {
      ...
      for (int idxParam = 0; idxParam < numParams; ++idxParam) {
        ...
        // 				{
        // 					arrVars[ idxParam
        // ]->SetIntValue( vars.m_matInfo.ignorez ? 1 : 0 ); 					continue;
        // 				}
...
#ifdef IS_WINDOWS_PC
bool IsSpaceOrQuote(char val) { return (isspace(val) || val == '\"'); }
...
CRenderTextureEditor::CRenderTextureEditor(vgui::Panel *parent,
                                           char const *szName)
    : BaseClass(parent, szName), m_pInfo(NULL), m_iInfoHint(0),
  ...
  m_pRebuild = vgui::SETUP_PANEL(
      new vgui::Button(this, "RebuildVTF", "Rebuild VTF", this, "RebuildVTF"));
  m_pRebuild->SetVisible(true);

  m_pToggleNoMip = vgui::SETUP_PANEL(new vgui::Button(
      this, "ToggleNoMip", "ToggleNoMip", this, "ToggleNoMip"));
  ...
  m_pSaveImg = vgui::SETUP_PANEL(
      new vgui::Button(this, "SaveImg", "Save Image", this, "SaveImg"));
...
void CRenderTextureEditor::SetDispInfo(KeyValues *kv, int iHint) {
  ...
  if (!(m_pInfo && !arrMaterials.GetNumStrings())) {
    if (kv) {
      ...
      // We support saving the 8888 formats right now: "RGBA8888", "ABGR8888",
      // "ARGB8888", "BGRA8888", "BGRX8888"
...
void CRenderTextureEditor::PerformLayout() {
  ...
  m_pRebuild->SetPos(2 * TILE_BORDER + TILE_SIZE - 50 - 65 - 95,
                     2 * TILE_BORDER);
  m_pRebuild->SetWide(90);
  m_pRebuild->SetVisible(m_lstMaterials.Count() > 0);

  m_pToggleNoMip->SetPos(2 * TILE_BORDER + TILE_SIZE - 50 - 95,
                         (2 * TILE_BORDER) + m_pReload->GetTall() + 1);
...
#ifdef IS_WINDOWS_PC
bool GetTextureContentPath(const char *pszVTF, char *szTextureContentPath,
                           int nBufSize) {
  ...
  if (!mat_texture_list_content_path.GetString()[0]) {
    szTextureContentPath =
        const_cast<char *>(g_pFileSystem->RelativePathToFullPath(
            vhRelVTF, "game", szTextureContentPath, nBufSize - 1));
    ...
    V_strncpy(szTextureContentPath, mat_texture_list_content_path.GetString(),
              MAX_PATH);
...
void CRenderTextureEditor::OnCommand(const char *command) {
  ...
  if (!stricmp(command, "ToggleNoMip")) {
    ...
    if (*szTextureFile)
      pMatTexture =
          materials->FindTexture(szTextureFile, szTextureGroup, false);
    if (!pMatTexture)
    ...
    // 1. look for TGA - if found, get the txt file (if txt file missing, create
    // one)
    ...
    char *pExtPut =
        ...
        strlen(
            ".vtf"); // compensating the TEXTURE_FNAME_EXTENSION(.vtf) extension
    ...
    if (g_pFullFileSystem->FileExists(szFileName)) {
      ...
      SetBufferValue((char *)bufTxtFileBuffer.Base(), "nomip",
                     bNewNoMip ? "1" : "0");
      ...
      if (g_pFullFileSystem->FileExists(szFileName)) {
        ...
        sprintf(chCommand, "/C psdinfo \"%s\" > \"%s\"", szFileName,
                szTxtFileName);
        ...
        if (g_pFullFileSystem->WriteFile(szTxtFileName, 0, bufTxtFileBuffer)) {
          ...
          sprintf(chCommand, "/C psdinfo -write \"%s\" < \"%s\"", szFileName,
                  szTxtFileName);
          Sys_Sleep(200);
          vgui::system()->ShellExecuteEx("open", "cmd.exe", chCommand);
          Sys_Sleep(200);
  ...
  if (!stricmp(command, "RebuildVTF")) {
    ...
    Q_snprintf(szVTFFilename, sizeof(szVTFFilename) - 1, "%s/materials/%s.vtf",
               pGameDir, szTextureFile);
    g_p4factory->SetOpenFileChangeList("Texture LOD Autocheckout");
    ...
    argv[iArg++] = "-quiet";
    argv[iArg++] =
        "-UseStandardError"; // These are only here for the -currently released-
                             // version of vtex.dll.
...
void CRenderTextureEditor::Paint() {
  ...
  if (!m_bufInfoText.TellPut()) {
    m_bufInfoText.Printf("%s%s\r\n"
                         "%s Kb    %dx%d    %s",
                         szPrintFilePrefix, szPrintFileName, chSizeBuf,
                         iTxWidth, iTxHeight, szTxFormat);
  }
  ...
  if (IMaterial *pMaterial = UseDebugMaterial(
    ...
    if (bHasAlpha) {
      orgTxX += orgTxXA;
      orgTxY += orgTxYA;
      if (IMaterial *pMaterialDebug = UseDebugMaterial(
...
CRenderTexturesListViewPanel::CRenderTexturesListViewPanel(vgui::Panel *parent,
                                                           char const *szName)
    : vgui::TileViewPanelEx(parent, szName), m_pListPanel(NULL),
      m_bPaintAlpha(false) {
...
CRenderTexturesListViewPanel::~CRenderTexturesListViewPanel() { NULL; }
...
CTextureListPanel::CTextureListPanel(vgui::Panel *parent)
  ...
  SetSize(videomode->GetModeStereoWidth() - 20,
          videomode->GetModeStereoHeight() - 20);
  SetPos(10, 10);
  SetVisible(true);
  SetCursor(null);
  ...
  m_pResolveTexturePath = vgui::SETUP_PANEL(
      new vgui::CheckButton(this, "resolvepath", "Resolve Full Texture Path"));
  ...
  // Filtering
  m_pHideMipped = vgui::SETUP_PANEL(
      new vgui::CheckButton(this, "HideMipped", "Hide Mipped"));
  ...
  m_pReloadAllMaterialsButton = vgui::SETUP_PANEL(
      new vgui::Button(this, "ReloadAllMaterials", "Reload All Materials"));
  ...
  m_pCommitChangesButton = vgui::SETUP_PANEL(
      new vgui::Button(this, "CommitChanges", "Commit Changes"));
  ...
  m_pDiscardChangesButton = vgui::SETUP_PANEL(
      new vgui::Button(this, "DiscardChanges", "Discard Changes"));
  ...
  int col = -1;
  m_pListPanel->AddColumnHeader(++col, KEYNAME_NAME, "Texture Name", 200, 100,
                                700, vgui::ListPanel::COLUMN_RESIZEWITHWINDOW);
  m_pListPanel->AddColumnHeader(++col, KEYNAME_PATH, "Path", 50, 50, 300, 0);
  m_pListPanel->AddColumnHeader(++col, KEYNAME_SIZE, "Kilobytes", 50, 50, 50,
                                0);
  m_pListPanel->SetSortFunc(col, KilobytesSortFunc);
  m_pListPanel->SetSortColumnEx(col, 0, true); // advanced sorting setup
  m_pListPanel->AddColumnHeader(++col, KEYNAME_TEXTURE_GROUP, "Group", 100, 100,
                                300, 0);
  m_pListPanel->AddColumnHeader(++col, KEYNAME_FORMAT, "Format", 250, 50, 300,
                                0);
  ...
  m_pListPanel->AddColumnHeader(++col, KEYNAME_BINDS_FRAME, "# Binds", 50, 50,
                                50, 0);
  m_pListPanel->AddColumnHeader(++col, KEYNAME_BINDS_MAX, "BindsMax", 50, 50,
                                50, 0);
...
void CTextureListPanel::PerformLayout() {
  ...
  m_pCVarListLabel->SetVisible(
      false); // m_pCVarListLabel->SetVisible( !bCollapsed );
  ...
  vgui::Panel *buttons[] = {
      m_pSpecialTexs,        m_pShowTextureMemoryUsageOption,
      m_pAllTextures,        m_pViewTextures,
      m_pFilteringChk,       m_pHideMipped,
      m_pResolveTexturePath, m_pCopyToClipboardButton};
...
bool CTextureListPanel::UpdateDisplayedItem(KeyValues *pDispData,
  ...
  if (pDispData->GetInt(KEYNAME_SIZE) != kv->GetInt(KEYNAME_SIZE) ||
      ...
      Q_stricmp(pDispData->GetString(KEYNAME_PATH),
                kv->GetString(KEYNAME_PATH)) != 0 ||
...
void CTextureListPanel::Paint() {
  ...
  for (KeyValues *pCur = textureList.Get()->GetFirstSubKey(); pCur;
    ...
    if (m_pResolveTexturePath->IsSelected()) {
      char chResolveName[256] = {0}, chResolveNameArg[256] = {0};
      Q_snprintf(chResolveNameArg, sizeof(chResolveNameArg) - 1,
                 "materials/%s.vtf", pCur->GetString(KEYNAME_NAME));
      char const *szResolvedName = g_pFileSystem->RelativePathToFullPath(
          chResolveNameArg, "game", chResolveName, sizeof(chResolveName) - 1);
      if (szResolvedName) {
...
CON_COMMAND(mat_texture_save_fonts, "Save all font textures") {
  for (int i = 0; i < 8192; i++) {
    ...
    Q_snprintf(szTextureName, ARRAYSIZE(szTextureName), "__font_page_%d.tga",
               i);
...
void mat_texture_list_on_f() {
  ...
  // On Linux, the mouse gets recentered when it's hidden. So if you bring up
  // the texture list
  //	dialog while the game is running, we need to make sure the mouse is
  //shown. Otherwise it's 	very tough to use when your mouse keeps getting
  //recentered.
