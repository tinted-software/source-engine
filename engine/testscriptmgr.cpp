...
CON_COMMAND_EXTERN(Test_WaitForCheckPoint, Test_WaitForCheckPoint, "") {
  if (args.ArgC() < 2) {
    Error("Test_WaitForCheckPoint <checkpoint name> [once]: requires "
          "checkpoint name.");
    return;
  }
...
CON_COMMAND_EXTERN(
  ...
  if (args.ArgC() < 3) {
    Error("Test_LoopLoopForNumSeconds: requires a loop name and number of "
          "seconds to loop.");
    return;
  }
...
CON_COMMAND(
  ...
  if (args.ArgC() < 3) {
    Error("Test_RandomChance: requires percentage chance parameter (0-100) "
...
CON_COMMAND(Test_StartScript, "Start a test script running..") {
  if (args.ArgC() < 2) {
    Warning("Test_StartScript: requires filename of script to start (file must "
            "be under testscripts directory).\n");
  }
...
void CTestScriptMgr::LoopForNumSeconds(const char *pLoopName, double nSeconds) {
  ...
  CLoopInfo *pLoop = FindLoop(pLoopName);
  if (!pLoop) {
    Error("CTestScriptMgr::LoopForNumSeconds( %s ): no loop with this name "
          "exists.",
          pLoopName);
  }
