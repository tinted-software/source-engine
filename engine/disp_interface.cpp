...
//-----------------------------------------------------------------------------
CDispShadowFragment *
CDispInfo::AllocateShadowDecalFragment(DispShadowHandle_t h, int nCount) {
        ...
        while (hFrag != DISP_DECAL_FRAGMENT_HANDLE_INVALID) {
          ...
          for (int iVert = 0; iVert < fragment.m_nVerts; ++iVert) {
            ...
            meshBuilder.Color4ub(
                fragment.m_pDecal->color.r, fragment.m_pDecal->color.g,
...
int DispInfo_ComputeIndex(HDISPINFOARRAY hArray, IDispInfo *pInfo) {
  ...
  intp iElement =
      ((intp)pInfo - (intp)(pArray->m_pDispInfos)) / sizeof(CDispInfo);
...
void DispInfo_RenderList(int nSortGroup, SurfaceHandle_t *pList, int listCount,
                         bool bOrtho, unsigned long flags,
  ...
  DispInfo_BuildPrimLists(nSortGroup, pList, listCount,
                          (DepthMode != DEPTH_MODE_NORMAL), visibleDisps,
                          nVisibleDisps);
