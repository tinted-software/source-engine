...
public:
  ...
  virtual const byte *GetTrustedKey(int iKeyIndex,
                                    int *nKeySize) const OVERRIDE;
...
private:
  ...
  public:
    ...
    unsigned short
        m_LoadOrder; // What order this thing was specified in the whitelist
                     // file?  Used to resolve rule conflicts.
  };

  void
  UpdateCommandStats(CUtlDict<CPureServerWhitelist::CCommand *, int> &commands,
                     int *pHighest, int *pLongestPathName);
  ...
  void
  EncodeCommandList(CUtlDict<CPureServerWhitelist::CCommand *, int> &theList,
                    CUtlBuffer &buf);
  void
  DecodeCommandList(CUtlDict<CPureServerWhitelist::CCommand *, int> &theList,
                    CUtlBuffer &buf, uint32 nFormatVersion);

  void AddHardcodedFileCommands();
  void AddFileCommand(const char *pszFilePath, EPureServerFileClass eFileClass,
                      unsigned short nLoadOrder);

  CPureServerWhitelist::CCommand *
  CheckEntry(CUtlDict<CPureServerWhitelist::CCommand *, int> &dict,
  ...
  static bool CommandDictDifferent(const CUtlDict<CCommand *, int> &a,
                                   const CUtlDict<CCommand *, int> &b);
...
struct UserReportedFileHash_t {
  ...
  static bool Less(const UserReportedFileHash_t &lhs,
                   const UserReportedFileHash_t &rhs) {
...
struct UserReportedFile_t {
  ...
  static bool Less(const UserReportedFile_t &lhs,
                   const UserReportedFile_t &rhs) {
...
public:
  ...
  void AddUserReportedFileHash(int idxFile, FileHash_t *pFileHash,
                               USERID_t userID, bool bAddMasterRecord);
  bool DoesFileMatch(const char *pPathID, const char *pRelativeFilename,
                     int nFileFraction, FileHash_t *pFileHash, USERID_t userID);
