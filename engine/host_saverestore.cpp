...
// so use
//			g_pSaveRestoreFileSystem when accessing them. The .sav
//file is always stored 			on disk, so use g_pFileSystem when accessing it.
//
// $Workfile:     $
// $Date:         $
// $NoKeywords: $
...
static bool HaveExactMap(const char *pszMapName) {
  ...
  IVEngineServer::eFindMapResult eResult =
      g_pVEngineServer->FindMap(szCanonName, sizeof(szCanonName));
...
struct SAVE_HEADER {
  ...
  int lightStyleCount;
  int mapVersion;
  float time__USE_VCR_MODE; // This is renamed to include the __USE_VCR_MODE
                            // prefix due to a #define on win32 from the VCR
                            // mode changes The actual save games have the
                            // string "time__USE_VCR_MODE" in them
  char mapName[32];
  char skyName[32];
};
...
private:
  ...
  int SaveReadHeader(FileHandle_t pFile, GAME_HEADER *pHeader,
                     int readGlobalState, bool *pbOldSave);
  ...
  int SaveReadNameAndComment(FileHandle_t f, OUT_Z_CAP(nameSize) char *name,
                             int nameSize, OUT_Z_CAP(commentSize) char *comment,
                             int commentSize) OVERRIDE;
  ...
  if (!IsXSave()) {
    if (count == 1) {
      ...
      Q_snprintf(oldName, sizeof(oldName), "//%s/%s%s%02d.%s", MOD_DIR,
                 GetSaveDir(), pName, count - 1,
                 ext); // quick04.sav, etc. DON'T FixSlashes on this, it needs
                       // to be //MOD
    }
...
int CSaveRestore::SaveGameSlot(const char *pSaveName, const char *pSaveComment,
  ...
  gameHeader.mapCount =
      0; // No longer used. The map packer will place the map count at the head
         // of the compound files (toml 7/18/2007)
...
int CSaveRestore::SaveReadHeader(FileHandle_t pFile, GAME_HEADER *pHeader,
  ...
  if (readGlobalState &&
      pHeader->mapCount == 0) // Alfred: Only load save games from the OB era
                              // engine where mapCount is forced to zero
  {
...
bool CSaveRestore::LoadGame(const char *pName) {
  ...
  if (pFile) {
    ...
    if (SaveReadNameAndComment(pFile, szDummyName, sizeof(szDummyName),
...
void CSaveRestore::SaveGameStateGlobals(CSaveRestoreData *pSaveData) {
  ...
  pSaveData->levelInfo.time =
      0; // prohibits rebase of header.time (why not just save time as a
         // field_float and ditch this hack?)
...
bool CSaveRestore::SaveGameState(bool bTransition,
  ...
  if (!IsXSave()) {
    Q_snprintf(
        name, 256, "//%s/%s%s.HL1", MOD_DIR, GetSaveDir(),
        GetSaveGameMapName(
            sv.GetMapName())); // DON'T FixSlashes on this, it needs to be //MOD
    SaveMsg("Queue COM_CreatePath\n");
    g_AsyncSaveCallQueue.QueueCall(&COM_CreatePath,
                                   CUtlEnvelope<const char *>(name));
  } else {
    Q_snprintf(
        name, 256, "%s:/%s.HL1", GetCurrentMod(),
        GetSaveGameMapName(
            sv.GetMapName())); // DON'T FixSlashes on this, it needs to be //MOD
  }
  ...
  if (!IsXSave()) {
    Q_snprintf(
        name, sizeof(name), "//%s/%s%s.HL2", MOD_DIR, GetSaveDir(),
        GetSaveGameMapName(
            sv.GetMapName())); // DON'T FixSlashes on this, it needs to be //MOD
  } else {
    Q_snprintf(
        name, sizeof(name), "%s:/%s.HL2", GetCurrentMod(),
        GetSaveGameMapName(
            sv.GetMapName())); // DON'T FixSlashes on this, it needs to be //MOD
  }
...
void CSaveRestore::ReapplyDecal(bool adjacent, RestoreLookupTable *table,
  ...
  if (adjacent) {
    ...
    int entityToHit =
        entry->entityIndex != 0
            ? LookupRestoreSpotSaveIndex(table, entry->entityIndex)
            : entry->entityIndex;
    if (entityToHit >= 0) {
      // NOTE: I re-initialized the origin and angles as the decals pos/angle
      // are saved in local space (ie. relative to
...
//-----------------------------------------------------------------------------
int CSaveRestore::SaveReadNameAndComment(FileHandle_t f,
                                         OUT_Z_CAP(nameSize) char *name,
                                         int nameSize,
                                         OUT_Z_CAP(commentSize) char *comment,
                                         int commentSize) {
...
int CSaveRestore::LoadGameState(char const *level, bool createPlayers) {
  ...
  sv.m_nTickCount =
      (int)(header.time__USE_VCR_MODE / host_state.interval_per_tick);
  // SUCCESS!
  return 1;
}

CSaveRestore::RestoreLookupTable *
CSaveRestore::FindOrAddRestoreLookupTable(char const *mapname) {
...
CON_COMMAND_F(save, "Saves current game.", FCVAR_DONTRECORD) {
  ...
  if (strstr(sv.m_szMapname, "background")) {
    ConDMsg("\"background\" is a reserved map name and cannot be saved or "
            "loaded.\n");
    return;
  }
...
bool CSaveRestore::StorageDeviceValid(void) {
  ...
  return (nStorageDeviceID != XBX_INVALID_STORAGE_ID &&
          nStorageDeviceID != XBX_STORAGE_DECLINED);
#endif
