...
// OS X and Linux are blowing up right now due to this.  Benefits vs possible
// regressions on DX less clear.
...
#endif
static ConVar mod_offline_hdr_switch(
...
static ConVar
    mod_forcetouchdata("mod_forcetouchdata", "1", 0,
...
static ConVar mod_dynamicunloadtime("mod_dynamicunloadtime", "150",
                                    FCVAR_HIDDEN | FCVAR_DONTRECORD);
static ConVar mod_dynamicunloadtextures("mod_dynamicunloadtex", "1",
                                        FCVAR_HIDDEN | FCVAR_DONTRECORD);
static ConVar mod_dynamicloadpause("mod_dynamicloadpause", "0",
                                   FCVAR_CHEAT | FCVAR_HIDDEN |
                                       FCVAR_DONTRECORD);
static ConVar mod_dynamicloadthrottle("mod_dynamicloadthrottle", "0",
                                      FCVAR_CHEAT | FCVAR_HIDDEN |
                                          FCVAR_DONTRECORD);
static ConVar mod_dynamicloadspew("mod_dynamicloadspew", "0",
                                  FCVAR_HIDDEN | FCVAR_DONTRECORD);

#define DynamicModelDebugMsg(...)                                              \
  (mod_dynamicloadspew.GetBool() ? Msg(__VA_ARGS__) : (void)0)
...
// Extended from the on-disk struct to include uncompressed size and stop
// propagation of bogus signed values
struct dgamelump_internal_t {
  dgamelump_internal_t(dgamelump_t &other, unsigned int nCompressedSize)
      : id(other.id), flags(other.flags), version(other.version),
        offset(Max(other.fileofs, 0)), uncompressedSize(Max(other.filelen, 0)),
...
DEFINE_FIELD(fileofs, FIELD_INTEGER), DEFINE_FIELD(filelen, FIELD_INTEGER),
    ...
    END_BYTESWAP_DATADESC()
        ...
        BEGIN_BYTESWAP_DATADESC(dheader_t) DEFINE_FIELD(ident, FIELD_INTEGER),
    DEFINE_FIELD(version, FIELD_INTEGER),
    DEFINE_EMBEDDED_ARRAY(lumps, HEADER_LUMPS),
    DEFINE_FIELD(mapRevision, FIELD_INTEGER),
    END_BYTESWAP_DATADESC()

        bool Model_LessFunc(FileNameHandle_t const &a,
...
public:
  CModelLoader()
      : m_ModelPool(sizeof(model_t), MAX_KNOWN_MODELS,
                    CUtlMemoryPool::GROW_FAST, "CModelLoader::m_ModelPool"),
  ...
  // Called by server and client engine code to flush unreferenced dynamic
  // models
  virtual void FlushDynamicModels() { InternalUpdateDynamicModels(true); }

  // Called by server and client to force-unload dynamic models regardless of
  // refcount!
  ...
  virtual bool RegisterModelLoadCallback(model_t *pModel, bool bClientOnly,
                                         IModelLoadCallback *pCallback,
                                         bool bCallImmediatelyIfLoaded);

  // Called by client code or IModelLoadCallback destructor
  virtual void UnregisterModelLoadCallback(model_t *pModel, bool bClientOnly,
                                           IModelLoadCallback *pCallback);

  virtual void Client_OnServerModelStateChanged(model_t *pModel,
                                                bool bServerLoaded);
...
private:
  ...
  public:
    ...
    CDynamicModelInfo()
        : m_iRefCount(0), m_iClientRefCount(0), m_nLoadFlags(INVALIDFLAG),
          m_uLastTouchedMS_Div256(0) {}
    int16 m_iRefCount;
...
void CMapLoadHelper::Init(model_t *pMapModel, const char *loadname) {
  ...
  s_MapFileHandle = g_pFileSystem->OpenEx(s_szMapName, "rb",
  ...
  if (s_MapHeader.version < MINBSPVERSION || s_MapHeader.version > BSPVERSION) {
    g_pFileSystem->Close(s_MapFileHandle);
    s_MapFileHandle = FILESYSTEM_INVALID_HANDLE;
    Host_Error("CMapLoadHelper::Init, map %s has wrong version (%i when "
               "expecting %i)\n",
  ...
  // Store map version, but only do it once so that the communication between
  // the engine and Hammer isn't broken. The map version is incremented whenever
  // a Hammer to Engine session is established so resetting the global map
  // version each time causes a problem.
  ...
  // XXX(johns): There are security issues with this system currently. sv_pure
  // doesn't handle unexpected/mismatched
  //             lumps, so players can create lumps for maps not using them to
  //             wallhack/etc.. Currently unused, disabling until we have time
  //             to make a proper security pass.
  if (IsPC()) {
...
void CMapLoadHelper::FreeLightingLump(void) {
  if (IsX360() && (s_MapFileHandle == FILESYSTEM_INVALID_HANDLE) &&
      s_MapBuffer.Base()) {
    int lightingLump =
        LumpSize(LUMP_LIGHTING_HDR) ? LUMP_LIGHTING_HDR : LUMP_LIGHTING;
    ...
    if (LumpSize(lightingLump)) {
      ...
      for (int i = 0; i < HEADER_LUMPS; i++) {
        if ((LumpOffset(i) > lightingOffset) && (i != LUMP_PAKFILE)) {
          Warning("CMapLoadHelper: Cannot free lighting lump (should be last "
                  "before the PAK lump). Regenerate the .360.bsp file with the "
                  "latest version of makegamedata.");
          return;
        }
      }

      // Flag the lighting chunk as gone from the BSP (principally, this sets
      // 'filelen' to 0)
      V_memset(&s_MapHeader.lumps[lightingLump], 0, sizeof(lump_t));

      // Shrink the buffer to free up the space that was used by the lighting
      // lump
      void *shrunkBuffer = realloc(s_MapBuffer.Base(), lightingOffset);
      Assert(shrunkBuffer ==
             s_MapBuffer.Base()); // A shrink would surely never move!!!
      s_MapBuffer.SetExternalBuffer(shrunkBuffer, lightingOffset,
                                    lightingOffset);
    }
  }
}
...
int CMapLoadHelper::LumpSize(int lumpId) {
  ...
  // all knowledge of compression is private, they expect and get the original
  // size
...
CMapLoadHelper::CMapLoadHelper(int lumpToLoad) {
  ...
  if (s_MapBuffer.Base()) {
    ...
    m_pRawData = (byte *)g_pFileSystem->AllocOptimalReadBuffer(
...
// Output : byte
//-----------------------------------------------------------------------------
byte *CMapLoadHelper::LumpBase(void) { return m_pData; }
...
void EnableHDR(bool bEnable) {
  ...
  if (IsX360()) {
    ...
    // can safely do the state update only, knowing that the state change won't
    // affect 360 resources
    ((MaterialSystem_Config_t *)g_pMaterialSystemConfig)
        ->SetFlag(MATSYS_VIDCFG_FLAGS_ENABLE_HDR, bEnable);
    return;
  }
  ...
  /// XXX(JohnS): This works around part of the terribleness the comments below
  /// discuss by performing
  ///             UpdateMaterialSystemConfig with the device offline, removing
  ///             its need to do multiple re-uploads of things.  I am not
  ///             positive my changes to allow that won't introduce terrible
  ///             regressions or awaken ancient bugs, hence the kill switch.
...
bool Map_CheckForHDR(model_t *pModel, const char *pLoadName) {
  ...
  bool bHasHDR = false;
  if (IsX360()) {
    // If this is true, the 360 MUST use HDR, because the LDR data gets stripped
    // out.
  ...
  bool bEnableHDR =
      (IsX360() && bHasHDR) ||
      (bHasHDR && (g_pMaterialSystemHardwareConfig->GetDXSupportLevel() >= 90));
...
static int ComputeLightmapSize(dface_t *pFace, mtexinfo_t *pTexInfo) {
  ...
  int nLuxels = (pFace->m_LightmapTextureSizeInLuxels[0] + 1) *
                (pFace->m_LightmapTextureSizeInLuxels[1] + 1);
  if (bNeedsBumpmap) {
...
void Mod_LoadLighting(CMapLoadHelper &lh) {
  ...
  if (IsX360()) {
    // Free the lighting lump, to increase the amount of memory free during the
    // rest of loading
...
void Mod_LoadWorldlights(CMapLoadHelper &lh, bool bIsHDR) {
  ...
  case 0: {
    ...
    lh.GetMap()->worldlights = (dworldlight_t *)Hunk_AllocName(
        nNumWorldLights * sizeof(dworldlight_t),
        va("%s [%s]", lh.GetLoadName(), "worldlights"));
    dworldlight_version0_t *RESTRICT pOldWorldLight =
        reinterpret_cast<dworldlight_version0_t *>(lh.LumpBase());
  ...
  for (int i = 0; i < lh.GetMap()->numworldlights; i++) {
    ...
    if (lh.GetMap()->worldlights[i].radius < 1) {
      lh.GetMap()->worldlights[i].radius =
          ComputeLightRadius(&lh.GetMap()->worldlights[i], bIsHDR);
    }
  }
}
...
void Mod_LoadLump(model_t *loadmodel, int iLump, char *loadname,
  ...
  if (lh.LumpSize() % elementSize) {
    Host_Error("Mod_LoadLump: funny lump size in %s",
               loadmodel->strName.String());
  }
...
void Mod_LoadCubemapSamples(void) {
  ...
  bool bHDR =
      g_pMaterialSystemHardwareConfig->GetHDREnabled(); // g_pMaterialSystemHardwareConfig->GetHDRType()
                                                        // != HDR_TYPE_NONE;
  ...
  for (i = 0; i < count; i++, in++, out++) {
    ...
    out->pTexture = materials->FindTexture(textureName, TEXTURE_GROUP_CUBE_MAP,
                                           true, nCreateFlags);
    if (IsErrorTexture(out->pTexture)) {
      ...
      if (IsErrorTexture(out->pTexture)) {
        Q_snprintf(textureName, sizeof(textureName), "maps/%s/cubemapdefault",
                   loadName);
        out->pTexture = materials->FindTexture(
            textureName, TEXTURE_GROUP_CUBE_MAP, true, nCreateFlags);
        if (IsErrorTexture(out->pTexture)) {
  ...
  if (count) {
    ...
    if (IsErrorTexture(pTexture)) {
      pTexture = materials->FindTexture(
          "engine/defaultcubemap", TEXTURE_GROUP_CUBE_MAP, true, nCreateFlags);
    }
...
bool Mod_LoadGameLump(int lumpId, void *pOutBuffer, int size) {
  ...
  if (s_MapBuffer.Base()) {
    ...
    if (g_GameLumpDict[i].offset + dataLength >
        (unsigned int)s_MapBuffer.TellMaxPut()) {
      // out of range
      Assert(0);
      return false;
    }
    ...
    g_pFileSystem->Seek(fileHandle, g_GameLumpDict[i].offset,
  ...
  if (!CLZMA::IsCompressed(pData) ||
      CLZMA::GetActualSize((unsigned char *)pData) !=
          g_GameLumpDict[i].uncompressedSize) {
    Warning("Failed loading game lump %i: lump claims to be compressed but "
            "metadata does not match\n",
            lumpId);
  } else {
    // uncompress directly into caller's buffer
    int outputLength = CLZMA::Uncompress(pData, (unsigned char *)pOutBuffer);
    bResult = (outputLength > 0 && (unsigned int)outputLength ==
                                       g_GameLumpDict[i].uncompressedSize);
  }
...
void Mod_LoadGameLumpDict(void) {
  ...
  if (lhSize >= sizeof(dgamelumpheader_t)) {
    ...
    const int nMaxGameLumps =
        (INT_MAX - sizeof(dgamelumpheader_t)) / sizeof(dgamelump_t);
    if (pGameLumpHeader->lumpCount < 0 ||
        pGameLumpHeader->lumpCount > nMaxGameLumps ||
        sizeof(dgamelumpheader_t) +
                sizeof(dgamelump_t) * pGameLumpHeader->lumpCount >
            lhSize) {
      ...
      for (int i = 0; i < pGameLumpHeader->lumpCount; ++i) {
        if (pGameLump[i].fileofs >= 0 &&
            (unsigned int)pGameLump[i].fileofs >=
                (unsigned int)lh.LumpOffset() &&
            (unsigned int)pGameLump[i].fileofs <
                (unsigned int)lh.LumpOffset() + lhSize &&
          ...
          if (i + 1 < pGameLumpHeader->lumpCount &&
              ...
              (unsigned int)pGameLump[i + 1].fileofs <=
                  (unsigned int)lh.LumpOffset() + lhSize) {
            compressedSize = (unsigned int)pGameLump[i + 1].fileofs -
                             (unsigned int)pGameLump[i].fileofs;
          } else {
            compressedSize = (unsigned int)lh.LumpOffset() + lhSize -
                             (unsigned int)pGameLump[i].fileofs;
          }
  ...
  if (pVirtualModel && nServerCount >= 1) {
    ...
    for (int i = 1; i < pVirtualModel->m_group.Count(); ++i) {
      MDLHandle_t childHandle =
          (MDLHandle_t)(intp)pVirtualModel->m_group[i].cache & 0xffff;
...
void CMDLCacheNotify::ComputeModelFlags(model_t *pModel, MDLHandle_t handle) {
  ...
  int materialCount =
      Mod_GetModelMaterials(pModel, ARRAYSIZE(pMaterials), pMaterials);
  ...
  for (int i = 0; i < materialCount; ++i) {
    ...
    if (pMaterial->NeedsPowerOfTwoFrameBufferTexture(
            false)) // The false checks if it will ever need the frame buffer,
                    // not just this frame
    {
...
void CMDLCacheNotify::OnDataLoaded(MDLCacheDataType_t type,
  ...
  case MDLCACHE_VCOLLIDE: {
    ...
    // Expand the model bounds to enclose the collision model (should be done in
    // studiomdl)
  ...
  virtual bool CreateResource(const char *pName) {
    ...
    if (modType == mod_brush) {
      ...
      if (pMapModel) {
        ...
        V_snprintf(szAINName, sizeof(szAINName), "maps/graphs/%s.360.ain",
                   szLoadName);
...
void CModelLoader::Init(void) {
  ...
  g_pQueuedLoader->InstallLoader(RESOURCEPRELOAD_MODEL,
...
model_t *CModelLoader::GetModelForName(const char *name,
  ...
  AssertMsg(!(referencetype & FMODELLOADER_DYNAMIC),
            "GetModelForName: dynamic models must use GetDynamicModel");
...
model_t *CModelLoader::ReferenceModel(const char *name,
                                      REFERENCETYPE referencetype) {
  AssertMsg(!(referencetype & FMODELLOADER_DYNAMIC),
            "ReferenceModel: do not use for dynamic models");
...
model_t *CModelLoader::LoadModel(model_t *mod, REFERENCETYPE *pReferencetype) {
  ...
  case mod_brush: {
    ...
    // it's called twice. The second invocation is harmless. Add to file system
    // before loading so referenced objects in map can use the filename.
    ...
    if (IsX360()) {
      // must establish the bsp feature set first to ensure proper state during
      // queued loading
      Map_CheckForHDR(mod, m_szLoadName);

      // Do not optimize map-to-same-map loading in TF
      // FIXME/HACK: this fixes a bug (when shipping Orange Box) where static
      // props would sometimes
      //             disappear when a client disconnects and reconnects to the
      //             same map+server (static prop lighting data persists when
      //             loading map A after map A)
      ...
      bQueuedLoader = g_pQueuedLoader->BeginMapLoading(
          mod->strName, g_pMaterialSystemHardwareConfig->GetHDREnabled(),
          bOptimizeMapReload);
    }
  ...
  COM_TimestampedLog("Load of %s took %.3f msec", mod->strName.String(),
                     1000.0f * dt);
...
static void BuildSpriteLoadName(const char *pName, char *pOut, int outLen,
  ...
  if (pExt != NULL) {
    ...
    if (!bIsVMT) {
      if (g_pVideo) {
        bIsVideo = (g_pVideo->LocateVideoSystemForPlayingFile(pName) !=
                    VideoSystem::NONE);
      }
    }
  }
...
void CModelLoader::UnreferenceModel(model_t *model,
                                    REFERENCETYPE referencetype) {
  AssertMsg(!(referencetype & FMODELLOADER_DYNAMIC),
            "UnreferenceModel: do not use for dynamic models");
  model->nLoadFlags &= ~referencetype;
}
...
void CModelLoader::UnreferenceAllModels(REFERENCETYPE referencetype) {
  AssertMsg(!(referencetype & FMODELLOADER_DYNAMIC),
            "UnreferenceAllModels: do not use for dynamic models");
      ...
      if (g_pMDLCache->IsDataLoaded(pModel->studio, MDLCACHE_STUDIOHWDATA)) {
        ...
        if (pStudioHdr) {
          ...
          for (int iMat = 0; iMat < nMaterials; iMat++) {
            if (pMaterials[iMat] &&
                pMaterials[iMat]->WasReloadedFromWhitelist()) {
#ifdef PURE_SERVER_DEBUG_SPEW
...
void CModelLoader::UnloadAllModels(bool bCheckReference) {
  ...
  FOR_EACH_MAP_FAST(m_Models, i) {
    ...
    if (IsX360() && g_pQueuedLoader->IsMapLoading() &&
      ...
      // models preloaded by the queued loader are not initially claimed and
      // MUST remain until the end of the load process unclaimed models get
      // unloaded during the post load purge
      continue;
    }
...
static void Mod_ComputeBrushModelFlags(model_t *mod) {
  ...
  mod->flags &=
      ~(MODELFLAG_MATERIALPROXY | MODELFLAG_TRANSLUCENT |
        MODELFLAG_FRAMEBUFFER_TEXTURE | MODELFLAG_TRANSLUCENT_TWOPASS);
...
static int SurfFlagsToSortGroup(SurfaceHandle_t surfID, int flags) {
  ...
  if (++warningcount < 10) {
    ...
    DevWarning("- This implies you have a surface (usually a displacement) "
               "embedded in solid.\n");
...
void CModelLoader::Map_LoadModel(model_t *mod) {
  ...
  if (IsX360() && !m_bMapHasHDRLighting) {
    Warning("Map '%s' lacks exepected HDR data! 360 does not support accurate "
            "LDR visuals.",
            m_szLoadName);
  }
...
//-----------------------------------------------------------------------------
static void GetSpriteInfo(const char *pName, bool bIsVideo, int &nWidth,
                          int &nHeight, int &nFrameCount) {
  nFrameCount = 1;
  nWidth = nHeight = 1;
  ...
  if (bIsVideo && g_pVideo != NULL) {
    pVideoMaterial = g_pVideo->CreateVideoMaterial(
        pName, pName, "GAME", VideoPlaybackFlags::DEFAULT_MATERIAL_OPTIONS,
...
void CModelLoader::Sprite_LoadModel(model_t *mod) {
  ...
  GetSpriteInfo(loadName, bIsVideo, mod->sprite.width, mod->sprite.height,
...
void CModelLoader::Studio_LoadModel(model_t *pModel, bool bTouchAllData) {
  ...
  if (pModel->nLoadFlags & FMODELLOADER_DYNAMIC) {
    // Cache the material pointers so that we don't re-scan all the VMTs on
    // dynamic unload
    COMPILE_TIME_ASSERT(sizeof(intptr_t) == sizeof(IMaterial *));
    IMaterial **pMem =
        (IMaterial **)malloc((1 + nMaterials) * sizeof(IMaterial *));
  ...
  FOR_EACH_MAP_FAST(m_Models, i) {
    ...
    if (pModel->type == mod_studio || pModel->type == mod_bad) {
      ...
      ConMsg("%4d: Flags:0x%8.8x RefCount:%2d %s\n", i, pModel->nLoadFlags,
             refCount, pModel->strName.String());
    } else {
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CModelLoader::Map_IsValid(char const *pMapFile,
                               bool bQuiet /* = false */) {
  ...
  // - Don't allow characters not allowed on all supported platforms for
  // consistency
  // - Don't allow quotes or ;"' as defense-in-depth against script abuses (and,
  // no real reason for mapnames to use these)
  ...
  for (; pBaseFileName && *pBaseFileName; pBaseFileName++) {
    ...
    case '*':
      bIllegalChar = true;
    // Additional special characters in source engine commands, defense-in-depth
    // against things that might be composing commands with map names (though
  ...
  mapfile = g_pFileSystem->OpenEx(szMapFile, "rb",
                                  IsX360() ? FSOPEN_NEVERINPACK : 0, "GAME");
  if (mapfile != FILESYSTEM_INVALID_HANDLE) {
    ...
    if (header.ident == IDBSPHEADER) {
      if (header.version >= MINBSPVERSION && header.version <= BSPVERSION) {
        ...
        if (!bQuiet) {
          Warning("CModelLoader::Map_IsValid:  Map '%s' bsp version %i, "
                  "expecting %i\n",
  ...
  if (pExt) {
    if (!V_stricmp(pExt, "spr") || !V_stricmp(pExt, "vmt")) {
      ...
      return mod_studio;
               ...
               g_pVideo->LocateVideoSystemForPlayingFile(pModelName) !=
                   VideoSystem::NONE) // video sprite
    {
      return mod_sprite;
    }
  }
...
model_t *CModelLoader::GetDynamicModel(const char *name, bool bClientOnly) {
  ...
  CDynamicModelInfo &dyn = m_DynamicModels[m_DynamicModels.Insert(
      pModel)]; // Insert returns existing if key is already set
...
void CModelLoader::UpdateDynamicModelLoadQueue() {
  ...
  if (mod_dynamicloadthrottle.GetFloat() > 0 &&
      Plat_FloatTime() <
          s_LastDynamicLoadTime + mod_dynamicloadthrottle.GetFloat())
    return;

  if (m_bDynamicLoadQueueHeadActive) {
    Assert(m_DynamicModelLoadQueue.Count() >= 1);
    MaterialLock_t matLock =
        g_pMaterialSystem->Lock(); // ASDFADFASFASEGAafliejsfjaslaslgsaigas
    ...
    if (bComplete) {
      ...
      if (hDyn != m_DynamicModels.InvalidHandle()) {
        ...
        dyn.m_nLoadFlags &=
            ~(CDynamicModelInfo::QUEUED | CDynamicModelInfo::LOADING);
      ...
      // we keep some file cache around to make sure that LoadModel doesn't do
      // blocking load
  ...
  // If we're not working, and we have work to do, and the queued loader is open
  // for business...
  if (!m_bDynamicLoadQueueHeadActive && m_DynamicModelLoadQueue.Count() > 0 &&
      g_pQueuedLoader->IsFinished()) {
    model_t *pModel = m_DynamicModelLoadQueue[0];
    UtlHashHandle_t hDyn = m_DynamicModels.Find(pModel);
    Assert(hDyn != m_DynamicModels.InvalidHandle());
    if (hDyn != m_DynamicModels.InvalidHandle()) {
      ...
      // the queued loader is very ... particular about path names. it doesn't
      // like leading "models/"
...
void CModelLoader::FinishDynamicModelLoadIfReady(CDynamicModelInfo *pDyn,
                                                 model_t *pModel) {
  ...
  if ((dyn.m_nLoadFlags & CDynamicModelInfo::CLIENTREADY)) {
    if (!(dyn.m_nLoadFlags & CDynamicModelInfo::SERVERLOADING)) {
      // There ought to be a better way to plumb this through, but this should
      // be ok...
      if (sv.GetDynamicModelsTable()) {
        int netidx =
            sv.GetDynamicModelsTable()->FindStringIndex(pModel->strName);
      ...
      // Reverse order; UnregisterModelLoadCallback does a FastRemove that swaps
      // from back
...
bool CModelLoader::RegisterModelLoadCallback(model_t *pModel, bool bClientOnly,
                                             IModelLoadCallback *pCallback,
                                             bool bCallImmediatelyIfLoaded) {
  UtlHashHandle_t hDyn = m_DynamicModels.Find(pModel);
  Assert(hDyn != m_DynamicModels.InvalidHandle());
  ...
  int readyFlag = bClientOnly ? CDynamicModelInfo::CLIENTREADY
                              : CDynamicModelInfo::ALLREADY;
  CDynamicModelInfo &dyn = m_DynamicModels[hDyn];
  AssertMsg(dyn.m_iRefCount > 0,
            "RegisterModelLoadCallback requires non-zero model refcount");
  if (dyn.m_nLoadFlags & readyFlag) {
    ...
    if (!dyn.m_Callbacks.HasElement(callbackID)) {
      ...
      m_RegisteredDynamicCallbacks[m_RegisteredDynamicCallbacks.Insert(
          callbackID, 0)]++;
    }
  }
...
bool CModelLoader::IsDynamicModelLoading(model_t *pModel, bool bClientOnly) {
  ...
  if (hDyn != m_DynamicModels.InvalidHandle()) {
    CDynamicModelInfo &dyn = m_DynamicModels[hDyn];
    AssertMsg(dyn.m_iRefCount > 0,
              "dynamic model state cannot be queried with zero refcount");
...
void CModelLoader::AddRefDynamicModel(model_t *pModel, bool bClientSideRef) {
  ...
  DynamicModelDebugMsg("model %p [%s] addref %d (%d)\n", pModel,
                       pModel->strName.String(), dyn.m_iRefCount,
                       dyn.m_iClientRefCount);

  if (!(dyn.m_nLoadFlags &
        (CDynamicModelInfo::QUEUED | CDynamicModelInfo::CLIENTREADY))) {
...
void CModelLoader::UnregisterModelLoadCallback(model_t *pModel,
                                               bool bClientOnly,
                                               IModelLoadCallback *pCallback) {
  Assert(((uintptr_t)pCallback & 1) == 0);
  uintptr_t callbackID = (uintptr_t)pCallback | (uintptr_t)bClientOnly;
  if (int *pCallbackRegistrationCount =
    ...
    if (pModel) {
      ...
      for (UtlHashHandle_t i = m_DynamicModels.FirstHandle();
           i != m_DynamicModels.InvalidHandle();
...
void CModelLoader::QueueDynamicModelLoad(CDynamicModelInfo *dyn, model_t *mod) {
  ...
  mod->nLoadFlags |= (dyn->m_iClientRefCount > 0 ? FMODELLOADER_DYNCLIENT
                                                 : FMODELLOADER_DYNSERVER);
}

bool CModelLoader::CancelDynamicModelLoad(CDynamicModelInfo *dyn,
                                          model_t *mod) {
...
void CModelLoader::InternalUpdateDynamicModels(bool bIgnoreTime) {
  ...
  const uint delay =
      bIgnoreTime
          ? 0
          : (int)(clamp(mod_dynamicunloadtime.GetFloat(), 1.f, 600.f) * 1000);

  UpdateDynamicModelLoadQueue();

#ifdef _DEBUG
  extern CNetworkStringTableContainer *networkStringTableContainerServer;
  bool bPrevStringTableLockState =
      networkStringTableContainerServer->Lock(false);
#endif

  // Scan for models to unload. TODO: accelerate with a "models to potentially
  // unload" list?
  ...
  while (i != m_DynamicModels.InvalidHandle()) {
    ...
    // UNLOAD THIS MODEL if zero refcount and not currently loading, and either
    // timed out or never loaded
    if (dyn.m_iRefCount <= 0 &&
        !(dyn.m_nLoadFlags & CDynamicModelInfo::LOADING) &&
...
void CModelLoader::Client_OnServerModelStateChanged(model_t *pModel,
                                                    bool bServerLoaded) {
#ifndef SWDS
  // Listen server don't distinguish between server and client ready, never use
  // SERVERLOADING flag
  if (sv.IsActive())
    return;
  ...
  if (i != m_DynamicModels.InvalidHandle()) {
    ...
    if (!bServerLoaded) {
      if (dyn.m_nLoadFlags & CDynamicModelInfo::ALLREADY)
        DynamicModelDebugMsg("dynamic model [%s] loaded on client but not "
                             "server! is this bad? unknown...",
                             pModel->strName.String());
      dyn.m_nLoadFlags &= ~CDynamicModelInfo::ALLREADY;
...
#if defined(WIN32)
int ComputeSize(studiohwdata_t *hwData, int *numVerts, int *pTriCount,
                bool onlyTopLod = false) {
  unsigned size = 0;
  ...
  for (int i = 0; i < max_lod; i++) {
    ...
    for (int j = 0; j < hwData->m_NumStudioMeshes; j++) {
      ...
      for (int k = 0; k < pMeshData->m_NumGroup; k++) {
        ...
        size +=
            2 * pMeshGroup
                    ->m_NumVertices; // Size of m_pGroupIndexToMeshIndex[] array
...
// APSFIXME: needs to only do models that are resident, sizes might be wrong,
// i.e lacking compressed vert state?
CON_COMMAND_F(model_list, "Dump model list to file",
  ...
  if (g_pFileSystem) {
    FileHandle_t fileHandle =
        g_pFileSystem->Open("model_list.csv", "wt", "GAME");
    ...
    if (fileHandle) {
      ...
      g_pFileSystem->FPrintf(
          fileHandle,
          "name,dataSize,numVerts,nTriCount,dataSizeLod0,numVertsLod0,"
      ...
      for (int i = 0; i < modelloader->GetCount(); i++) {
        ...
        if (model) {
          ...
          if (hwData) {
            ...
            dataSize = ComputeSize(hwData, &numVerts, &nTriCount,
                                   false); // Size of vertex data
            dataSizeLod0 =
                ComputeSize(hwData, &numVertsLod0, &nTriCountLod0, true);
          }

          studiohdr_t *pStudioHdr =
              (studiohdr_t *)modelloader->GetExtraData(model);
          dataSize += pStudioHdr->length; // Size of MDL file
          numBones = pStudioHdr->numbones;
          numParts = pStudioHdr->numbodyparts;

          g_pFileSystem->FPrintf(
              fileHandle, "%s,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n", name, dataSize,
              numVerts, nTriCount, dataSizeLod0, numVertsLod0, nTriCountLod0,
              numBones, numParts, numLODs, numMeshes);
        }
      }
...
CON_COMMAND_F(mod_dynamicmodeldebug, "debug spew for dynamic model loading",
              FCVAR_HIDDEN | FCVAR_DONTRECORD) {
...
void CModelLoader::DebugPrintDynamicModels() {
  ...
  if (sv.GetDynamicModelsTable()) {
    for (int i = 0; i < sv.GetDynamicModelsTable()->GetNumStrings(); ++i) {
      int dummy = 0;
      char *data =
          (char *)sv.GetDynamicModelsTable()->GetStringUserData(i, &dummy);
      bool bLoadedOnServer = !(data && dummy && data[0] == 0);
      Msg("%3i: %c %s\n", i, bLoadedOnServer ? '*' : ' ',
          sv.GetDynamicModelsTable()->GetString(i));
    }
  }
  ...
  if (cl.m_pDynamicModelsTable) {
    for (int i = 0; i < cl.m_pDynamicModelsTable->GetNumStrings(); ++i) {
      int dummy = 0;
      char *data =
          (char *)cl.m_pDynamicModelsTable->GetStringUserData(i, &dummy);
      bool bLoadedOnServer = !(data && dummy && data[0] == 0);
      Msg("%3i: %c %s\n", i, bLoadedOnServer ? '*' : ' ',
          cl.m_pDynamicModelsTable->GetString(i));
    }
  }
#endif
  ...
  Msg("\ndynamic models:\n");
  for (UtlHashHandle_t h = m_DynamicModels.FirstHandle();
       h != m_DynamicModels.InvalidHandle();
    ...
    Msg("%d (%d%c): %s [ref: %d (%dc)] %s%s%s%s\n", idx, ((-2 - idx) >> 1),
        (idx & 1) ? 'c' : 's', m_DynamicModels.Key(h)->strName.String(),
        dyn.m_iRefCount, dyn.m_iClientRefCount,
