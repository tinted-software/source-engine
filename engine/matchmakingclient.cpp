...
// multiplayer
//			game, tracking new players and dropped players during
//the game, and reporting 			game results and stats after the game is complete.
//
//=============================================================================//
...
void CMatchmaking::SelectSession(uint idx) {
  ...
  m_Session.SetSessionSlots(SLOTS_TOTALPUBLIC,
                            pResult->dwOpenPublicSlots +
                                pResult->dwFilledPublicSlots);
  m_Session.SetSessionSlots(SLOTS_TOTALPRIVATE,
                            pResult->dwOpenPrivateSlots +
                                pResult->dwFilledPrivateSlots);
...
void CMatchmaking::JoinInviteSession(XSESSION_INFO *pHostInfo) {
  ...
  case INVITE_VALIDATING:
    ...
    Msg("[JoinInviteSession:INVITE_VALIDATING] Validating user storage before "
        "accepting invite.\n");
    ...
    if ((m_InviteWaitingInfo.m_SignInState !=
      ...
      Msg("[JoinInviteSession:INVITE_VALIDATING] Failed to get sign in to LIVE "
          "info, aborting.\n");
      InviteCancel();
      return;
    }
  ...
  case INVITE_AWAITING_STORAGE:
    // Wait for user to select storage, but keep an eye on user change or logout
    // or etc
    {
    ...
    // Otherwise user has selected the storage device, try to accept the invite
    // once again
...
void CMatchmaking::JoinInviteSessionByID(XNKID nSessionID) {
  ...
  DWORD dwError = XSessionSearchByID(nSessionID, XBX_GetPrimaryUserId(),
  ...
  dwError = XSessionSearchByID(nSessionID, XBX_GetPrimaryUserId(),
                               &dwResultSize, pSearchResults, NULL);
