...
#ifdef _X360
ConVar save_in_memory("save_in_memory", "1", 1,
...
void CSaveRestoreFileSystem::Compress(SaveFile_t *pFile) {
  ...
  if (pCompressedBuffer == NULL) {
    ...
    pFile->pCompressedBuffer->AssumeMemory(
        pCompressedBuffer, pFile->nCompressedSize, pFile->nCompressedSize); // ?
  }
  // end compression
...
FileHandle_t CSaveRestoreFileSystem::Open(const char *pFullName,
  ...
  if (!Q_stricmp(pOptions, "rb")) {
    ...
    Assert(0);
    Warning("CSaveRestoreFileSystem: Attempted to open %s with unsupported "
            "option %s\n",
            pFullName, pOptions);
    return (void *)0;
  }
...
//-----------------------------------------------------------------------------
FSAsyncStatus_t
CSaveRestoreFileSystem::AsyncWrite(const char *pFileName, const void *pSrc,
                                   int nSrcBytes, bool bFreeMemory,
...
//-----------------------------------------------------------------------------
FSAsyncStatus_t
CSaveRestoreFileSystem::AsyncAppend(const char *pFileName, const void *pSrc,
                                    int nSrcBytes, bool bFreeMemory,
...
//-----------------------------------------------------------------------------
FSAsyncStatus_t
CSaveRestoreFileSystem::AsyncAppendFile(const char *pDestFileName,
                                        const char *pSrcFileName,
                                        FSAsyncControl_t *pControl) {
...
//does NOT 			currently use pPath to filter the filename search.
//-----------------------------------------------------------------------------
...
void CSaveRestoreFileSystem::AuditFiles(void) {
  ...
  Msg("SIM: ------------------------------------------------------------");
  Msg("SIM: Total files: %d [c: %.02f KB / c: %.02f KB] : Total Size: %.02f "
      "KB\n",
  ...
  void DirectoryCopy(const char *pPath, const char *pDestFileName,
    ...
    for (int i = 0; i < list.Count(); i++) {
      ...
      if (fileSize) {
        ...
        g_pFileSystem->AsyncAppend(
            pDestFileName,
            memcpy(new char[sizeof(int)], &fileSize, sizeof(int)), sizeof(int),
            true);
