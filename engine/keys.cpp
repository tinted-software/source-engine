...
CON_COMMAND_F(bind_mac, "Bind this key but only on Mac, not win32",
              FCVAR_DONTRECORD) {
  if (IsOSX()) {
    int i, c;
    char cmd[1024];
...
static bool HandleEngineKey(const InputEvent_t &event) {
  ...
  if (IsPC() && bDown) {
    if (IsJoystickCode(code) && !IsJoystickAxisCode(code) &&
        !IsSteamControllerCode(code) && !s_pKeyInfo[code].m_pKeyBinding) {
      ConDMsg("%s is unbound.\n", g_pInputSystem->ButtonCodeToString(code));
    }
  }

  // Allow the client to handle mouse wheel events while the game has focus,
  // without having to bind keys.
#if !defined(SWDS)
