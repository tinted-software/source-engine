...
#define GAMESTATSUPLOADER_CONNECT_RETRY_TIME 1.0
  ...
  virtual bool UploadGameStats(char const *mapname, unsigned int blobversion,
    ...
    return true;
    // return UploadGameStatsInternal( mapname, blobversion, blobsize,
    // pvBlobData );
  }
...
#error
      ...
      if (Steam3Client().SteamUtils() &&
          (Steam3Client().SteamUser()->BLoggedOn()) &&
          (k_EUniverseBeta ==
    ...
    if ((buf[0] == 0) && sv.IsDedicated()) {
      // For Linux dedicated servers, where we won't get a unique ID: set the ID
      // to "unknown" so we have something.  (If there's no ID, stats don't get
      // sent.)  This will later get altered to be a hash of IP&port, but this
      // gets called early before IP is determined so we can't make the hash
      // now.
  ...
  bool UploadGameStatsInternal(char const *mapname, unsigned int blobversion,
                               unsigned int blobsize, const void *pvBlobData) {
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CUploadGameStats, IUploadGameStats,
...
CWin32UploadGameStats::CWin32UploadGameStats(
    ...
    u32 contextid)
    : m_States(), m_uCurrentState(eCreateTCPSocket), m_HarvesterSockAddr(),
      m_SocketTCP(0), m_rCrashParameters(rGameStatsParameters),
...
//
//-----------------------------------------------------------------------------
bool CWin32UploadGameStats::DoBlockingReceive(uint bytesExpected,
                                              CUtlBuffer &buf) {
...
void CWin32UploadGameStats::AddState(uint StateIndex,
...
EGameStatsUploadStatus
Win32UploadGameStatsBlocking(const TGameStatsParameters &rGameStatsParameters) {
...
public:
  CAsyncUploaderThread()
      : m_hThread(NULL), m_bRunning(false), m_eventQueue(false),
        m_eventInitShutdown(false) {}
...
protected:
  ...
  enum { SLEEP_ENTRY_UPLOADED = 10 * 1000 };
...
public:
  ...
  void QueueData(char const *szMapName, uint uiBlobVersion, uint uiBlobSize,
                 const void *pvBlob);
...
CAsyncUploaderThread::DataEntry *
CAsyncUploaderThread::DataEntry::AllocCopy() const {
...
void CAsyncUploaderThread::QueueData(char const *szMapName, uint uiBlobVersion,
                                     uint uiBlobSize, const void *pvBlob) {
...
void CAsyncUploaderThread::TerminateAndSelfDelete() {
  ...
  if (hThread) {
    ...
    m_eventQueue.Set();
    m_eventInitShutdown
        .Set(); // MUST BE LAST MEMBER ACCESS, other thread may now delete this
...
void CAsyncUploaderThread::ThreadProc() {
  ...
  while (m_bRunning) {
    ...
    if (m_bRunning && pUpload) {
      ...
      bool bSuccess = g_UploadGameStats.UploadGameStatsInternal(
          pUpload->szMapName, pUpload->uiBlobVersion, pUpload->uiBlobSize,
          pUpload->pvBlob);
      bSuccess;
      ...
      // DevMsg( 3, "AsyncUploaderThread: Upload finished (status=%d) for data
      // [%.*s]\n", bSuccess, pUpload->uiBlobSize, pUpload->pvBlob );

      // Using an event as an interruptable sleep; signaled if m_bRunning is
      // cleared
...
void AsyncUpload_QueueData(char const *szMapName, uint uiBlobVersion,
                           uint uiBlobSize, const void *pvBlob) {
  if (!g_pAsyncUploader) {
