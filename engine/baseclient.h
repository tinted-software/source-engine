...
public:
  ...
  // Is an actual human player or splitscreen player (not a bot and not a HLTV
  // slot)
...
public:
  ...
  int m_nClientSlot;
  // entity index of this client (different from clientSlot+1 in HLTV and Replay
  // mode):
  int m_nEntityIndex;
  ...
  bool m_bSendServerInfo; // true if we need to send server info packet to start
                          // connect
...
#endif
  int m_clientChallenge; // client's challenge he sent us, we use to auth
                         // replies
  ...
  int m_nDeltaTick; // -1 = no compression.  This is where the server is
                    // creating the compressed info from.
  int m_nStringTableAckTick; // Highest tick acked for string tables (usually
                             // m_nDeltaTick, except when it's -1)
...
private:
  ...
  int m_iTracing; // 0 = not active, 1 = active for this frame, 2 = forced
                  // active
