  ...
  FOR_EACH_LL(g_ClientSendTables, iClientTable) {
    ...
    for (int iProp = 0; iProp < pTable->GetNumProps(); iProp++) {
      ...
      if (pProp->m_Type == DPT_DataTable) {
        ...
        if (!pChild) {
          DataTable_Warning("SetupClientSendTableHierarchy: missing SendTable "
                            "'%s' (referenced by '%s').\n",
...
static bool
MatchRecvPropsToSendProps_R(CUtlRBTree<MatchingProp_t, unsigned short> &lookup,
                            char const *sendTableName, SendTable *pSendTable,
                            RecvTable *pRecvTable, bool bAllowMismatches,
                            bool *pAnyMismatches) {
  for (int i = 0; i < pSendTable->m_nProps; i++) {
    ...
    if (pSendProp->GetType() == DPT_DataTable) {
      if (!MatchRecvPropsToSendProps_R(
              ...
              FindRecvTable(pSendProp->GetDataTable()->m_pNetTableName),
              bAllowMismatches, pAnyMismatches))
        return false;
    }
  }
...
static void
CopySendPropsToRecvProps(CUtlRBTree<MatchingProp_t, unsigned short> &lookup,
...
bool RecvTable_CreateDecoders(const CStandardSendProxies *pSendProxies,
                              bool bAllowMismatches, bool *pAnyMismatches) {
  DTI_Init();
  ...
  FOR_EACH_LL(g_RecvDecoders, i) {
    ...
    if (!MatchRecvPropsToSendProps_R(
            ...
            pDecoder->GetSendTable(),
            FindRecvTable(pDecoder->GetSendTable()->m_pNetTableName),
            bAllowMismatches, pAnyMismatches))
      return false;
...
bool RecvTable_Decode(RecvTable *pTable, void *pStruct, bf_read *pIn,
  ...
  while ((iProp = deltaBitsReader.ReadNextPropIndex()) < MAX_DATATABLE_PROPS) {
    ...
    if (pProp) {
      ...
      // They're allowed to be missing props here if they're playing back a
      // demo. This allows us to change the datatables and still preserve old
      // demos.
...
int RecvTable_MergeDeltas(RecvTable *pTable,
  ...
  while (1) {
    ...
    while (iOldProp < iNewProp) {
      ...
      oldStateReader.CopyPropData(deltaBitsWriter.GetBitBuf(),
                                  pDecoder->GetSendProp(iOldProp));
      iOldProp = oldStateReader.ReadNextPropIndex();
    }
    ...
    newStateReader.CopyPropData(deltaBitsWriter.GetBitBuf(),
                                pDecoder->GetSendProp(iNewProp));
