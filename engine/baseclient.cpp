...
ConVar sv_namechange_cooldown_seconds(
    ...
    "When a client name change is received, wait N seconds before allowing "
    "another name change");
ConVar sv_netspike_on_reliable_snapshot_overflow(
    "sv_netspike_on_reliable_snapshot_overflow", "0", FCVAR_NONE,
    "If nonzero, the server will dump a netspike trace if a client is dropped "
    "due to reliable snapshot overflow");
ConVar sv_netspike_sendtime_ms(
    "sv_netspike_sendtime_ms", "0", FCVAR_NONE,
    "If nonzero, the server will dump a netspike trace if it takes more than N "
    "ms to prepare a snapshot to a single client.  This feature does take some "
    "CPU cycles, so it should be left off when not in use.");
ConVar sv_netspike_output(
    "sv_netspike_output", "1", FCVAR_NONE,
    "Where the netspike data be written?  Sum of the following values: "
    "1=netspike.txt, 2=ordinary server log");
...
bool CBaseClient::SetSignonState(int state, int spawncount) {
  ...
  case SIGNONSTATE_CONNECTED: // client is connected, leave client in this state
                              // and let SendPendingSignonData do the rest
...
//---------------------------------------------------------------------------
// Purpose: Determine whether or not a character should be ignored in a player's
// name.
//---------------------------------------------------------------------------
inline bool BIgnoreCharInName(unsigned char cChar, bool bIsFirstCharacter) {
  // Don't copy '%' or '~' chars across
  // Don't copy '#' chars across if they would go into the first position in the
  // name Don't allow color codes ( less than COLOR_MAX )
  return cChar == '%' || cChar == '~' || cChar < 0x09 ||
         (bIsFirstCharacter && cChar == '#');
}
...
void ValidateName(char *pszName, int nBuffSize) {
  ...
  if (Q_strlen(pszName) <= 0) {
    ...
    // did we get all the way to the end of the name without a non-whitespace
    // character?
    if (*pChar == '\0') {
      Q_snprintf(pszName, nBuffSize, "unnamed");
    }
...
void CBaseClient::SetName(const char *playerName) {
  ...
  while (*pFrom && pTo < pLimit) {
    ...
    // the name Don't allow color codes ( less than COLOR_MAX )
  ...
  if (!IsX360()) {
    ...
    while (true) {
      for (i = 0; i < m_Server->GetClientCount(); i++) {
        ...
        // If it's 2 bots they're allowed to have matching names, otherwise
        // there's a conflict
        if (!Q_stricmp(client->GetClientName(), val) &&
            !(IsFakeClient() && client->IsFakeClient())) {
      ...
      Q_snprintf(newname, sizeof(newname), "(%d)%-.*s", dupc++,
...
void CBaseClient::Connect(const char *szName, int nUserID,
  ...
  if (bFakePlayer) {
    // Hidden fake players and the HLTV/Replay bot will get removed by
...
bool CBaseClient::ProcessSetConVar(NET_SetConVar *msg) {
  for (int i = 0; i < msg->m_ConVars.Count(); i++) {
    ...
    if (bFunky) {
      Msg("Ignoring convar change request for variable '%s' from client %s; "
    ...
    // flagged userinfo. This is a simple fix to exploits that send bogus data
    // later, and catches bugs (why are new userinfo convars appearing later?)
...
void CBaseClient::StartTrace(bf_write &msg) {
  ...
  if (m_iTracing < 2) {
    if (m_Trace.m_nMinWarningBytes <= 0 &&
...
void CBaseClient::EndTrace(bf_write &msg) {
  ...
  if (m_iTracing < 2 // not forced
      && (nBitThreshold <= 0 ||
          bits < nBitThreshold) // didn't exceed data threshold
      &&
      (sv_netspike_sendtime_ms.GetFloat() <= 0.0f ||
       flElapsedMs <
           sv_netspike_sendtime_ms.GetFloat())) // didn't exceed time threshold
  {
  ...
  logData.Printf("%f/%d Player [%s][%d][adr:%s] was sent a datagram %d bits "
                 "(%8.3f bytes), took %.2fms\n",
...
void CBaseClient::TraceNetworkData(bf_write &msg, char const *fmt, ...) {
  if (!IsTracing())
    return;
  VPROF_BUDGET("CBaseClient::TraceNetworkData",
...
void CBaseClient::TraceNetworkMsg(int nBits, char const *fmt, ...) {
  if (!IsTracing())
    return;
  VPROF_BUDGET("CBaseClient::TraceNetworkMsg",
               VPROF_BUDGETGROUP_OTHER_NETWORKING);
  char buf[64];
  va_list argptr;
  va_start(argptr, fmt);
...
write_again:
    ...
    if (!deltaFrame) {
      ...
      if (!bWasTracing) {
        ...
        if (sv_netspike_on_reliable_snapshot_overflow.GetBool()) {
          ...
          {
            Warning(" RELIABLE SNAPSHOT OVERFLOW!  Triggering trace to see "
                    "what is so large\n");
            bFailedOnce = true;
            m_iTracing = 2;
            goto write_again;
  ...
  // will make m_NetChannel non-null).
  ...
  if (!deltaFrame) {
    VPROF_BUDGET("SendSnapshot Transmit Full",
                 VPROF_BUDGETGROUP_OTHER_NETWORKING);
    ...
    VPROF_BUDGET("SendSnapshot Transmit Delta",
                 VPROF_BUDGETGROUP_OTHER_NETWORKING);
...
bool CBaseClient::IsNameChangeOnCooldown(bool bShowStatusMessage /*= false*/) {
  ...
  if (m_fTimeLastNameChange > 0.0) {
    ...
    double dNextChangeTime =
        m_fTimeLastNameChange + sv_namechange_cooldown_seconds.GetFloat();
    if (timeNow < dNextChangeTime) {
      // Cooldown period still active; throttle the name change
      if (bShowStatusMessage) {
        ClientPrintf(
            "You have changed your name recently, and must wait %i seconds.\n",
