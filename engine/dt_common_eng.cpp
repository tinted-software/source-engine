...
RecvTable *DataTable_FindRenamedTable(const char *pOldTableName) {
  ...
  // Get the renamed receive table list from the client DLL and see if we can
  // find a new name (assuming it was renamed at all).
  const CRenamedRecvTableInfo *pCur = g_ClientDLL->GetRenamedRecvTableInfos();

  // This should be a very short list, so we'll do string compares until 2020
  // when someone finds this code and the list has grown to 10,000.
...
bool DataTable_SetupReceiveTableFromSendTable(SendTable *sendTable,
  ...
  for (int iProp = 0; iProp < pTable->m_nProps; iProp++) {
    ...
    if (CommandLine()->FindParm("-dti") &&
        pSendTableProp->GetParentArrayPropName()) {
      pProp->m_pParentArrayPropName =
          COM_StringCopy(pSendTableProp->GetParentArrayPropName());
    }
...
void DataTable_CreateClientTablesFromServerTables() {
  if (!serverGameDLL) {
    Sys_Error("DataTable_CreateClientTablesFromServerTables:  No serverGameDLL "
              "loaded!");
  }
...
void DataTable_CreateClientClassInfosFromServerClasses(
  ...
  if (!serverGameDLL) {
    Sys_Error("DataTable_CreateClientClassInfosFromServerClasses:  No "
              "serverGameDLL loaded!");
  }
