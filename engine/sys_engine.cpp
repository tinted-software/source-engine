...
// Sleep time when not focus. Set to 0 to not sleep even if app doesn't have
// focus.
...
//-----------------------------------------------------------------------------
static void fps_max_callback(IConVar *var, const char *pOldValue,
                             float flOldValue) {
  // Only update s_nDesiredFPSMax when not driven by the mat_powersavingsmode
  // ConVar (see below)
...
// When set, this ConVar (typically driven from the advanced video settings)
// will drive fps_max (see above) to half of the refresh rate, if the user
// hasn't otherwise set fps_max (via console, commandline etc)
static void mat_powersavingsmode_callback(IConVar *var, const char *pOldValue,
                                          float flOldValue) {
  ...
  if (((ConVar *)var)->GetBool()) {
    ...
    nRefresh = MAX(30, (mode.m_RefreshRate + 1) >>
                           1); // Half of display refresh rate (min of 30Hz)
  }

  fps_max.SetValue(nRefresh);
  s_bFPSMaxDrivenByPowerSavings = false;
}
static ConVar mat_powersavingsmode("mat_powersavingsmode", "0", FCVAR_ARCHIVE,
                                   "Power Savings Mode",
...
#ifdef WIN32
static void cpu_frequency_monitoring_callback(IConVar *var,
...
ConVar cpu_frequency_monitoring(
    "cpu_frequency_monitoring", "0", 0,
    "Set CPU frequency monitoring interval in seconds. Zero means disabled.",
    true, 0.0f, true, 10.0f, cpu_frequency_monitoring_callback);
#endif
...
private:
  ...
  float m_flMinFrameTime;  // Expected duration of a frame, or zero if it is
                           // unlimited.
...
bool CEngine::Load(bool bDedicated, const char *rootdir) {
  ...
  if (Sys_InitGame(g_AppSystemFactory, rootdir, game->GetMainWindowAddress(),
                   bDedicated)) {
    success = true;
...
bool CEngine::FilterTime(float dt) {
  ...
  if (IsPC() && !sv.IsDedicated() && !CanCheat() && fps_max.GetFloat() < 30) {
    ...
    if (fps_max.GetFloat() != 0.0f) {
      Warning("sv_cheats is 0 and fps_max is being limited to a minimum of 30 "
              "(or set to 0).\n");
...
void CEngine::Frame(void) {
  ...
  // Watch for data from the CPU frequency monitoring system and print it to the
  // console.
  ...
  if (frequency.m_timeStamp > s_lastFrequencyTimestamp) {
    ...
    Msg("~CPU Freq: %1.3f GHz    Percent of requested: %3.1f%%    Minimum "
        "percent seen: %3.1f%%\n",
        frequency.m_GHz, frequency.m_percentage, frequency.m_lowestPercentage);
  }

  // Loop until it is time for our frame. Don't return early because pumping
  // messages and processing console input is expensive (0.1 ms for each call to
  // ProcessConsoleInput).
  for (;;) {
    // Get current time
    ...
    if (IsPC() && (!sv.IsDedicated() || host_timer_spin_ms.GetFloat() != 0)) {
      ...
      int nSleepMS =
          (int)((m_flMinFrameTime - m_flFrameTime) * 1000 - fBusyWaitMS);
      if (nSleepMS > 0)
        ThreadSleep(nSleepMS);

      // Go back to the top of the loop and see if it is time yet.
    } else {
      int nSleepMicrosecs = (int)ceilf(clamp(
          (m_flMinFrameTime - m_flFrameTime) * 1000000.f, 1.f, 1000000.f));
#ifdef POSIX
