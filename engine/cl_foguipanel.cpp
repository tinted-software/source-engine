...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CFogUIPanel::CFogUIPanel(vgui::Panel *parent)
    : vgui::Frame(parent, "FogUIPanel") {
  SetTitle("FogUI", true);
...
void CFogUIPanel::OnTick() {
  ...
  bool bFogEnabled =
      (m_pFogEnable->IsSelected() && m_pFogOverride->IsSelected());
  ...
  bool bFogSkyEnabled =
      (m_pFogEnableSky->IsSelected() && m_pFogOverride->IsSelected());
  ...
  bool bFarZEnabled =
      (m_pFarZOverride->IsSelected() && m_pFogOverride->IsSelected());
...
void CFogUIPanel::OnTextNewLine(KeyValues *data) {
  ...
  if (pTextEntry == m_pFogColorGreenText ||
      pTextEntry == m_pFogColorGreenTextSky) {
    UpdateFogColorGreenSlider();
    return;
  }
  if (pTextEntry == m_pFogColorBlueText ||
      pTextEntry == m_pFogColorBlueTextSky) {
...
void CFogUIPanel::OnMessage(const KeyValues *params, VPANEL fromPanel) {
  ...
  if (!Q_strcmp("SliderMoved", params->GetName())) {
    ...
    if (fromPanel == m_pFogColorGreenSky->GetVPanel()) {
      m_pFogColorGreenTextSky->SetText(
          va("%i", m_pFogColorGreenSky->GetValue()));
      UpdateFogColors();
    }
...
static ConCommand fogui("fogui", FogUI_f, "Show/hide fog control UI.",
                        FCVAR_DONTRECORD);