...
void CColorSlider::SetValue(int nKnobIndex, int value) {
  Assert(m_nKnobCount > nKnobIndex);
  SetNormalizedValue(nKnobIndex, (float)(value - m_nMinValue) /
...
float CSelectedHSVOperation::GetSelectionAmount(const Vector &rgb) const {
  ...
  for (int i = 0; i < m_SelectedRGBs.Count(); i++) {
    ...
    case SELECT_NEARBY_RGB:
      flCurSelAmount = FuzzyLessThan(rgb.DistTo(m_SelectedRGBs[i]), m_Tolerance,
                                     m_Fuzziness * m_Tolerance);
      break;
...
static ConCommand
    print_colorcorrection("print_colorcorrection", PrintColorCorrection,
