...
struct MovieInfo_t {
  ...
  bool DoVideo() const { return (type & FMOVIE_VID) ? true : false; }

  bool DoVideoSound() const { return (type & FMOVIE_VIDSOUND) ? true : false; }
...
const struct CPrecacheUserData *
CL_GetPrecacheUserData(INetworkStringTable *table, int index);
