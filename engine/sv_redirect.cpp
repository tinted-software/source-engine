...
static char
    sv_redirect_buffer[4096]; // can't be any bigger because then we hit other
                              // static limits in the engine print funcs
...
void SV_RedirectFlush(void) {
  ...
  {
    NET_OutOfBandPrintf(sv.m_Socket, sv_redirectto, "%c%s", A2A_PRINT,
                        sv_redirect_buffer);
...
void SV_RedirectAddText(const char *txt) {
  ...
  Q_strncat(sv_redirect_buffer, (char *)txt, sizeof(sv_redirect_buffer),
            COPY_ALL_CHARACTERS);
}