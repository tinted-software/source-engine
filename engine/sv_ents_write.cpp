...
void PrintChangeTracks() {
  ...
  ConMsg("---------------------------------------------------------------------"
         "---\n");
  ConMsg("CalcDelta MS / %% time / Encode MS / # Changed / # Unchanged / Class "
         "Name\n");
  ConMsg("---------------------------------------------------------------------"
         "---\n");
...
static inline bool SV_NeedsExplicitDestroy(int entnum, CFrameSnapshot *from,
  ...
  if (entnum >= to->m_nNumEntities ||
      to->m_pEntities[entnum].m_pClass == NULL) // doesn't exits in new
  {
...
// Calculates the delta between the two states and writes the delta and the new
// properties into u.m_pBuf. Returns false if the states are the same.
//
// Also uses the IFrameChangeList in pTo to come up with a smaller set of
// properties to delta against. It deltas against any properties that have
// changed since iFromFrame. If iFromFrame is -1, then it deltas all properties.
static int SV_CalcDeltaAndWriteProps(CEntityWriteInfo &u,
  ...
  if (nDeltaProps) {
    nCulledProps = SendTable_CullPropsFromProxies(
        pToTable, deltaProps, nDeltaProps, u.m_nClientEntity - 1, NULL, -1,
...
static inline void SV_DetermineUpdateType(CEntityWriteInfo &u) {
  ...
  if (!u.m_bCullProps && (hltv || replay)) {
    unsigned char *pBuffer =
        hltv ? hltv->m_DeltaCache.FindDeltaBits(
                   u.m_nNewEntity, u.m_pFromSnapshot->m_nTickCount, nBits)
             : replay->m_DeltaCache.FindDeltaBits(
                   u.m_nNewEntity, u.m_pFromSnapshot->m_nTickCount, nBits);
#else
  if (!u.m_bCullProps && hltv) {
  ...
  if (nCheckProps == -1) {
    ...
    // that should happen only in HLTV/Replay demo playback mode, this code is
    // really expensive
  ...
  if (nCheckProps > 0) {
    ...
    if (!u.m_bCullProps) {
      if (hltv) {
        ...
        hltv->m_DeltaCache.AddDeltaBits(
            u.m_nNewEntity, u.m_pFromSnapshot->m_nTickCount, 0, NULL);
      }

#if defined(REPLAY_ENABLED)
      if (replay) {
        // no bits changed, PreserveEnt
        replay->m_DeltaCache.AddDeltaBits(
            u.m_nNewEntity, u.m_pFromSnapshot->m_nTickCount, 0, NULL);
      }
#endif
    }
...
static inline void SV_WriteEnterPVS(CEntityWriteInfo &u) {
  ...
  {*/
  // send all changed properties when entering PVS (no SendProxy culling since
  // we may use it as baseline
  ...
  else
  {
          ...
          u.m_nFullProps += SV_CalcDeltaAndWriteProps( u, pFromData, nFromBits,
  u.m_pNewPack );
  }*/
...
static inline int SV_WriteDeletions(CEntityWriteInfo &u) {
  ...
  for (int i = 0; i < nLast; i++) {
    ...
    // If the entity is marked to transmit in the u.m_pTo, then it can never be
    // destroyed by the m_iExplicitDeleteSlots Another possible fix would be to
    // clear any slots in the explicit deletes list that were actually occupied
    ...
    bool bNeedsExplicitDelete =
        SV_NeedsExplicitDestroy(i, pFromSnapShot, pToSnapShot);
    if (!bNeedsExplicitDelete && u.m_pTo) {
      bNeedsExplicitDelete =
          (pToSnapShot->m_iExplicitDeleteSlots.Find(i) !=
           pToSnapShot->m_iExplicitDeleteSlots.InvalidIndex());
      // We used to do more stuff here as a sanity check, but I don't think it
      // was necessary since the only thing that would unset the bould would be
      // a "recreate" in the same slot which is already implied by the
...
void CBaseServer::WriteDeltaEntities(CBaseClient *client, CClientFrame *to,
  ...
  VPROF_BUDGET("CBaseServer::WriteDeltaEntities",
