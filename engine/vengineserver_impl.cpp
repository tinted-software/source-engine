...
int MapList_ListMaps(const char *pszSubString, bool listobsolete, bool verbose,
                     int maxcount, int maxitemlength, char maplist[][64]);
...
static void SurfaceToVerts(model_t *model, SurfaceHandle_t surfID,
                           Vector verts[], int *vertCount) {
  ...
  for (int i = 0; i < *vertCount; i++) {
    int vertIndex =
        model->brush.pShared->vertindices[MSurf_FirstVertIndex(surfID) + i];
...
// Calculate the surface area of an arbitrary msurface_t polygon (convex with
// collinear verts)
static float SurfaceArea(model_t *model, SurfaceHandle_t surfID) {
  ...
  for (i = 0; i < vertCount; i++) {
    ...
    VectorSubtract(
        verts[i], center,
        edge0); // 0.5 * edge cross edge (0.5 is done once at the end)
...
static void SurfaceCenter(model_t *model, SurfaceHandle_t surfID,
                          Vector &center) {
...
public:
  ...
  virtual const CCheckTransmitInfo *
  GetPrevCheckTransmitInfo(edict_t *pPlayerEdict) {
  ...
  //			*outputpvs - If null, then return value is the needed
  //length
  ...
  virtual int GetPVSForCluster(int clusterIndex, int outputpvslength,
    ...
    if (outputpvs) {
      if (outputpvslength < length) {
        Sys_Error("GetPVSForOrigin called with inusfficient sized pvs array, "
                  "need %i bytes!",
                  length);
        return length;
      }
  ...
  virtual bool CheckOriginInPVS(const Vector &org,
    ...
    if (offset > checkpvssize) {
      Sys_Error("CheckOriginInPVS:  cluster would read past end of pvs data "
                "(%i:%i)\n",
                offset, checkpvssize);
      return false;
    }
  ...
  virtual bool CanPlayerChangeName(const edict_t *pEdict) {
    ...
    for (int i = 0; i < sv.GetClientCount(); i++) {
      ...
      if (pClient->edict == pEdict) {
        return (!pClient->IsPlayerNameLocked() &&
                !pClient->IsNameChangeOnCooldown());
      }
    }
  ...
  // See header comment. This is the canonical map lookup spot, and a superset
  // of the server gameDLL's CanProvideLevel/PrepareLevelResources
  virtual eFindMapResult FindMap(/* in/out */ char *pMapName, int nMapNameMax) {
    ...
    IServerGameDLL::eCanProvideLevelResult eCanGameDLLProvide =
        IServerGameDLL::eCanProvideLevel_CannotProvide;
    if (g_iServerGameDLLVersion >= 10) {
      eCanGameDLLProvide =
          serverGameDLL->CanProvideLevel(pMapName, nMapNameMax);
    }

    if (eCanGameDLLProvide == IServerGameDLL::eCanProvideLevel_Possibly) {
      return eFindMap_PossiblyAvailable;
    } else if (eCanGameDLLProvide ==
               IServerGameDLL::eCanProvideLevel_CanProvide) {
      // See if the game dll fixed up the map name
      return (V_strcmp(szOriginalName, pMapName) == 0) ? eFindMap_Found
                                                       : eFindMap_NonCanonical;
    }

    AssertMsg(eCanGameDLLProvide ==
                  IServerGameDLL::eCanProvideLevel_CannotProvide,
    ...
    char match[1][64] = {{0}};
    if (MapList_ListMaps(pMapName, false, false, 1, sizeof(match[0]), match) &&
        *(match[0])) {
  ...
  virtual int GetEntityCount(void) { return sv.num_edicts - sv.free_edicts; }
  ...
  */
  virtual void EmitAmbientSound(int entindex, const Vector &pos,
    ...
    if ((fFlags & SND_SPAWNING) && sv.allowsignonwrites) {
      ...
      if (fFlags & SND_SPAWNING) {
        DevMsg("EmitAmbientSound: warning, broadcasting sound labled as "
               "SND_SPAWNING.\n");
      }
  ...
  virtual int CheckHeadnodeVisible(int nodenum, const byte *visbits,
                                   int vissize) {
  ...
  */
  ...
  virtual void ClientCommandKeyValues(edict_t *pEdict, KeyValues *pCommand) {
    ...
    if ((entnum < 1) || (entnum > sv.GetClientCount())) {
      ConMsg("\n!!!\n\nClientCommandKeyValues:  Some entity tried to stuff "
             "'%s' to console buffer of entity %i when maxclients was set to "
             "%i, ignoring\n\n",
  ...
  */
  ...
  virtual bf_write *EntityMessageBegin(int ent_index, ServerClass *ent_class,
    ...
    if (s_MsgData.started) {
      Sys_Error("EntityMessageBegin:  New message started before matching call "
                "to EndMessage.\n ");
      return NULL;
    }
  ...
  virtual bf_write *UserMessageBegin(IRecipientFilter *filter, int msg_index) {
    if (s_MsgData.started) {
      Sys_Error("UserMessageBegin:  New message started before matching call "
                "to EndMessage.\n ");
      return NULL;
    }
    ...
    if (s_MsgData.currentMsg == &s_MsgData.userMsg) {
      ...
      if (msgsize == -1) {
        if (bytesWritten > MAX_USER_MSG_DATA) {
          Warning("DLL_MessageEnd:  Refusing to send user message %s of %i "
  ...
  virtual void SetFakeClientConVarValue(edict_t *pEntity, const char *pCvarName,
                                        const char *value) {
  ...
  virtual int GetAppID() { return GetSteamAppID(); }
  ...
  */
  ...
  CGamestatsData *GetGamestatsData() { return g_pGamestatsData; }

  virtual IReplaySystem *GetReplay() { return g_pReplay; }
  ...
  virtual int GetAllClusterBounds(bbox_t *pBBoxList, int maxBBox) {
    ...
    if (pBSPData && pBSPData->map_vis && host_state.worldbrush) {
      ...
      for (int i = 0; i < host_state.worldbrush->numleafs; i++) {
        ...
        if (!(pLeaf->contents & CONTENTS_SOLID) && pLeaf->cluster >= 0 &&
            pLeaf->cluster < maxBBox) {
          Vector mins, maxs;
          mins = pLeaf->m_vecCenter - pLeaf->m_vecHalfDiagonal;
          maxs = pLeaf->m_vecCenter + pLeaf->m_vecHalfDiagonal;
          AddPointToBounds(mins, pBBoxList[pLeaf->cluster].mins,
                           pBBoxList[pLeaf->cluster].maxs);
          AddPointToBounds(maxs, pBBoxList[pLeaf->cluster].mins,
                           pBBoxList[pLeaf->cluster].maxs);
        }
      }
  ...
  virtual float GetServerTime() const OVERRIDE { return sv.GetTime(); }

  virtual IServer *GetIServer() OVERRIDE { return (IServer *)&sv; }

  virtual void SetPausedForced(bool bPaused,
                               float flDuration /*= -1.f*/) OVERRIDE {
...
// Backwards-compat shim that inherits newest then provides overrides for the
// legacy behavior
class CVEngineServer22 : public CVEngineServer {
  virtual int IsMapValid(const char *filename) OVERRIDE {
    // For users of the older interface, preserve here the old modelloader
    // behavior of wrapping maps/%.bsp around the filename. This went away in
    // newer interfaces since maps can now live in other places.
...
// INTERFACEVERSION_VENGINESERVER_VERSION_21 is compatible with 22 latest since
// we only added virtuals to the end, so expose that as well.
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CVEngineServer, IVEngineServer021,
                                  INTERFACEVERSION_VENGINESERVER_VERSION_21,
                                  g_VEngineServer22);
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CVEngineServer, IVEngineServer022,
...
//-----------------------------------------------------------------------------
static CUtlMemoryPool s_PVSInfoAllocator(128, 128 * 64,
...
void CVEngineServer::PlaybackTempEntity(IRecipientFilter &filter, float delay,
  ...
  if (!SendTable_Encode(pST, pSender, &buffer, classID, NULL, false)) {
    Host_Error("PlaybackTempEntity: SendTable_Encode returned false (ent %d), "
               "overflow? %i\n",
