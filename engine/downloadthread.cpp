...
void ReadData(RequestContext_t &rc) {
  ...
  while (!rc.shouldStop) {
    ...
    // this thread to finish, but will clean up the RequestContext_t at some
    // later point when InternetReadFile() has returned and this thread has
    // finished.
 ...
 * to quit.
 */
void CleanUpDownload(RequestContext_t &rc, HTTPStatus_t status,
                     HTTPError_t error = HTTP_ERROR_NONE) {
  if (status != HTTP_DONE || error != HTTP_ERROR_NONE) {
    // Thread_DPrintf( "CleanUpDownload() - http status is %s, error state is
...
DWORD __stdcall DownloadThread(void *voidPtr) {
  ...
  flags |=
      INTERNET_FLAG_KEEP_CONNECTION; // Use keep-alive semantics.  Since each
                                     // file downloaded is a separate connection
  ...
  if (url.nScheme == INTERNET_SCHEME_HTTPS) {
    ...
    // possible.
    flags |=
        INTERNET_FLAG_SECURE; // Use SSL, etc.  Kinda need this for HTTPS URLs.
    flags |= INTERNET_FLAG_IGNORE_CERT_CN_INVALID;   // Don't check hostname on
                                                     // the SSL cert.
    flags |= INTERNET_FLAG_IGNORE_CERT_DATE_INVALID; // Don't check for expired
                                                     // SSL certs.
  }
  ...
  if (HttpQueryInfo(rc.hDataResource,
    ...
    if (code > 0) {
      ...
      if (!rc.data) {
        // We're seeing crazy large numbers being returned in code (0x48e1e22),
        // the new fails, and we crash. This should probably be a different
        // error?
...
static size_t curlWriteFn(void *ptr, size_t size, size_t nmemb, void *stream) {
  ...
  if (pRC->nBytesTotal &&
      pRC->nBytesCurrent + (size * nmemb) <= pRC->nBytesTotal) {
...
void SetProxiesForURL(CURL *hMasterCURL, const char *pszURL) {
  ...
  CFURLRef url =
      CFURLCreateWithBytes(NULL, (const UInt8 *)pszURL, strlen(pszURL),
                           kCFStringEncodingASCII, NULL);
  // copy the proxies dictionary
  CFDictionaryRef proxyDict = SCDynamicStoreCopyProxies(NULL);
  // and ask the system what proxies it thinks I should consider for the given
  // URL
  CFArrayRef proxies = CFNetworkCopyProxiesForURL(url, proxyDict);

  CFIndex iProxy;
  // walk through the returned set, looking for any types we (and lib curl) can
  // handle the list is returned in "preference order", but we can only handle
  // http, and pac urls
  for (iProxy = 0; iProxy < CFArrayGetCount(proxies); iProxy++) {
    CFDictionaryRef proxy =
        (CFDictionaryRef)CFArrayGetValueAtIndex(proxies, iProxy);
    ...
    if (CFEqual(proxyType, kCFProxyTypeNone)) {
      ...
      if (val == NULL ||
          !CFNumberGetValue((CFNumberRef)val, kCFNumberIntType, &uProxyPort))
      ...
      if (val != NULL &&
          CFStringGetCString((CFStringRef)val, rgchProxyHost + cbOffset,
                             sizeof(rgchProxyHost) - cbOffset,
      ...
      CFURLRef cfUrl = (CFURLRef)CFDictionaryGetValue(
          proxy, kCFProxyAutoConfigurationURLKey);
      CFStringGetCString(
          (CFStringRef)CFStringCreateWithFormat(NULL, NULL, CFSTR("%@"), cfUrl),
      ...
      if (!g_pacRequestCtx.cacheData ||
          Q_strcmp((const char *)g_pacRequestCtx.cacheData, rgchPacURL)) {
        if (g_pacRequestCtx.cacheData) {
        ...
        // grab the data, using the same request context structure (and
        // callbacks) we use for real downloads
        ...
        curl_easy_setopt(hCURL, CURLOPT_FOLLOWLOCATION,
                         1); // follow 30x redirections from the web server
      ...
      if (res == CURLE_OK) {
        ...
        if (!g_pacRequestCtx.cacheData) {
          g_pacRequestCtx.cacheData =
              (unsigned char *)malloc(Q_strlen(rgchPacURL) + 1);
          Q_memcpy(g_pacRequestCtx.cacheData, rgchPacURL, Q_strlen(rgchPacURL));
        }

        if (!g_pacRequestCtx
                 .data) // no data in the proxy.pac they have, so just ignore it
          return;

        // wrap the data (the pac contents) into a cfstring
        CFStringRef cfPacStr = CFStringCreateWithCString(
            kCFAllocatorDefault, (const char *)g_pacRequestCtx.data,
            kCFStringEncodingASCII);

        // and ask the system, given this proxy pac, what (list of) proxies
        // should I consider for this URL?
        CFErrorRef err;
        CFArrayRef proxiesForUrl =
            CFNetworkCopyProxiesForAutoConfigurationScript(cfPacStr, cfUrl,
                                                           &err);
        if (proxiesForUrl) {
          // we're re-assigning the value that the loop is iterating over, the
          // postincrement will fire after we do this, hence the -1 (rather than
          // 0) assignment to iProxy
...
#warning "CHTTPDownloadThread doesn't know how to set proxy config"
  ...
  if (pszProxyExceptionList) {
    curl_easy_setopt(
        hMasterCURL,
        (CURLoption)(CURLOPTTYPE_OBJECTPOINT + 177) /*CURLOPT_NOPROXY*/,
...
void DownloadThread(void *voidPtr) {
  ...
  // turn off certificate verification similar to how we set
  ...
  uint32 cubURL = Q_strlen(rc.baseURL) + Q_strlen(rc.urlPath) +
                  2 /*one for the /, one for the null*/;
  ...
  if (res == CURLE_OK) {
    ...
    if (rc.status == HTTPStatus_t(200) || rc.status == HTTPStatus_t(206)) {
      ...
      // will finish before the main thread goes on and starts messing with the
      // file
      WriteFileFromRequestContext(rc);
