  ...
  if (!Q_stricmp(cmd, "close")) {
    if (Q_strlen(m_szEntryCommand) > 0) {
      ...
      if (!Cbuf_HasRoomForExecutionMarkers(2)) {
        AssertMsg(false,
                  "CPluginGameUIDialog::OnCommand called but there is no room "
                  "for the execution markers. Ignoring command.");
        return;
      }
      ...
      Cbuf_AddTextWithMarkers(
          eCmdExecutionMarker_Enable_FCVAR_CLIENTCMD_CAN_EXECUTE, userCMD,
