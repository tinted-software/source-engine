...
static void OnHibernateWhenEmptyChanged(IConVar *var, const char *pOldValue,
                                        float flOldValue);
ConVar sv_hibernate_when_empty("sv_hibernate_when_empty", "1", 0,
                               "Puts the server into extremely low CPU usage "
                               "mode when no clients connected",
                               OnHibernateWhenEmptyChanged);
// ConVar sv_hibernate_ms( "sv_hibernate_ms", "20", 0, "# of milliseconds to
// sleep per frame while hibernating" ); ConVar sv_hibernate_ms_vgui(
// "sv_hibernate_ms_vgui", "20", 0, "# of milliseconds to sleep per frame while
// hibernating but running the vgui dedicated server frontend" ); static ConVar
// sv_hibernate_postgame_delay( "sv_hibernate_postgame_delay", "5", 0, "# of
// seconds to wait after final client leaves before hibernating.");
ConVar sv_shutdown_timeout_minutes(
    "sv_shutdown_timeout_minutes", "360", FCVAR_REPLICATED,
    "If sv_shutdown is pending, wait at most N minutes for server to drain "
...
int GetSvPureMode() { return g_sv_pure_mode; }
...
static void SV_Pure_f(const CCommand &args) {
  ...
  if (pure_mode >= -1 && pure_mode <= 2) {
    ...
    Msg("sv_pure: Only allow client to use certain files.\n"
        "\n"
        "  -1 - Do not apply any rules or restrict which files the client may "
        "load.\n"
...
ConVar sv_pure_kick_clients(
    "sv_pure_kick_clients", "1", 0,
...
ConVar sv_pure_consensus(
    "sv_pure_consensus", "5", 0,
    "Minimum number of file hashes to agree to form a consensus.");
ConVar sv_pure_retiretime(
    "sv_pure_retiretime", "900", 0,
...
static ConVar
    sv_consistency("sv_consistency", "1", FCVAR_REPLICATED,
                   "Legacy variable with no effect!  This was deleted and then "
                   "added as a temporary kludge to prevent players from being "
                   "banned by servers running old versions of SMAC");

/// XXX(JohnS): When steam voice gets ugpraded to Opus we will probably default
/// back to steam.  At that time we should
///             note that Steam voice is the highest quality codec below.
static ConVar sv_voicecodec(
    "sv_voicecodec", "vaudio_opus", 0,
...
ConVar sv_restrict_aspect_ratio_fov(
    "sv_restrict_aspect_ratio_fov", "1", FCVAR_REPLICATED,
    "This can be used to limit the effective FOV of users using wide-screen\n"
    "resolutions with aspect ratios wider than 1.85:1 (slightly wider than "
    "16:9).\n"
    "    0 = do not cap effective FOV\n"
    "    1 = limit the effective FOV on windowed mode users using resolutions\n"
    "        greater than 1.85:1\n"
    "    2 = limit the effective FOV on both windowed mode and full-screen "
    "users\n",
    true, 0, true, 2);

void OnTVEnablehanged(IConVar *pConVar, const char *pOldString,
                      float flOldValue) {
  ConVarRef var(pConVar);

  /*
          ConVarRef replay_enable( "replay_enable" );
          if ( var.GetBool() && replay_enable.IsValid() &&
     replay_enable.GetBool() )
          {
                  var.SetValue( 0 );
                  Warning( "Error: Replay is enabled.  Please disable Replay if
     you wish to enable SourceTV.\n" ); return;
          }
  */
...
ConVar tv_enable("tv_enable", "0", FCVAR_NOTIFY,
                 "Activates SourceTV on server.", OnTVEnablehanged);
...
void CGameServer::CreateEngineStringTables(void) {
  ...
  m_pDynamicModelsTable =
      m_StringTables->CreateStringTable("DynamicModels", 2048, true, 1);
...
CON_COMMAND(user, "Show user data.") {
  ...
  for (i = 0; i < sv.GetClientCount(); i++) {
    ...
    if ((pClient->GetPlayerSlot() == uid) ||
        !Q_strcmp(pClient->GetClientName(), args[1])) {
...
CON_COMMAND(users, "Show user info for players on server.") {
  ...
  for (int i = 0; i < sv.GetClientCount(); i++) {
    ...
    if (pClient->IsConnected()) {
      ConMsg("%i:%i:\"%s\"\n", pClient->GetPlayerSlot(), pClient->GetUserID(),
...
#if !defined(_X360)
// Put this function declaration at global scope to avoid the ambiguity of the
// most vexing parse.
bool CL_IsHL2Demo();
#endif
#endif
...
void SV_InitGameDLL(void) {
  ...
  if (Steam3Client().SteamApps() && !CL_IsHL2Demo() && !CL_IsPortalDemo()) {
    ...
    for (int i = 0; i < ARRAYSIZE(g_ModDirPermissions); i++) {
      if (!Q_stricmp(COM_GetModDirectory(),
        ...
        if (Steam3Client().SteamApps()->BIsSubscribedApp(
                g_ModDirPermissions[i].m_iAppID)) {
          bVerifiedMod = true;
        } else {
  ...
  if (host_state.interval_per_tick < MINIMUM_TICK_INTERVAL ||
    ...
    Sys_Error("GetTickInterval returned bogus tick interval (%f)[%f to %f is "
              "valid range]",
...
void SV_StartSound(IRecipientFilter &filter, edict_t *pSoundEmittingEntity,
                   ...
                   int iSpecialDSP, const Vector *pOrigin, float soundtime,
                   int speakerentity, CUtlVector<Vector> *pUtlVecOrigins) {
...
int GetNetSpikeValue() {
  ...
  if (s_iNetSpikeValue < 0)
    s_iNetSpikeValue =
        Max(V_atoi(CommandLine()->ParmValue("-netspike", "0")), 0);
  return s_iNetSpikeValue;
}

const char szSvNetSpikeUsageText[] =
    "Write network trace if amount of data sent to client exceeds N bytes.  "
    "Use zero to disable tracing.\n"
    "Note that having this enabled, even if never triggered, impacts "
    "performance.  Set to zero when not in use.\n"
    "For compatibility reasons, this command can be initialized on the command "
    "line with the -netspike option.";
...
static ConCommand sv_netspike("sv_netspike", sv_netspike_f,
                              szSvNetSpikeUsageText);
...
bool CGameServer::FinishCertificateCheck(netadr_t &adr, int nAuthProtocol,
  ...
  case PROTOCOL_HASHEDCDKEY:
    ...
    /*
                    if ( !Host_IsSinglePlayerGame() || sv.IsDedicated()) //
       PROTOCOL_HASHEDCDKEY isn't allowed for multiplayer servers
                    {
                            RejectConnection( adr, clientChallenge,
       "#GameUI_ServerCDKeyAuthInvalid" ); return false;
                    }

                    if ( Q_strlen( szRawCertificate ) != 32 )
                    {
                            RejectConnection( adr, clientChallenge,
       "#GameUI_ServerInvalidCDKey" ); return false;
                    }*/
...
static ConVar sv_memlimit(
    "sv_memlimit", "0", 0,
    "If set, whenever a game ends, if the total memory used by the server is "
    "greater than this # of megabytes, the server will exit.");
static ConVar sv_minuptimelimit(
    "sv_minuptimelimit", "0", 0,
    "If set, whenever a game ends, if the server uptime is less than "
    "this number of hours, the server will continue running regardless of "
    "sv_memlimit.");
static ConVar sv_maxuptimelimit(
    "sv_maxuptimelimit", "0", 0,
...
static ConCommand sv_shutdown_cancel("sv_shutdown_cancel", sv_ShutDownCancel,
...
static void sv_ShutDown(void) {
  ...
  if (sv.IsHibernating() || !sv.IsActive()) {
    ...
    if (s_timeForceShutdown == 0.0 && sv_shutdown_timeout_minutes.GetInt() > 0)
      s_timeForceShutdown =
          timeCurrent + sv_shutdown_timeout_minutes.GetInt() * 60.0;

    // Print appropriate message
    if (s_timeForceShutdown > 0.0) {
      Warning(
          "Server will shut down in %d seconds, or when it becomes empty.\n",
...
static ConCommand
    sv_shutdown("sv_shutdown", sv_ShutDown,
                "Sets the server to shutdown next time it's empty");

bool CGameServer::IsHibernating() const { return m_bHibernating; }
    ...
    if (m_bHibernating) {
      ...
      for (int iClient = 0; iClient < m_Clients.Count(); iClient++) {
        ...
        if (pClient->IsFakeClient() && pClient->IsConnected() &&
            !pClient->IsSplitScreenUser() && !pClient->IsReplay() &&
            !pClient->IsHLTV()) {
          pClient->Disconnect("Punting bot, server is hibernating");
        }
      }

      // A solo player using the game menu to return to lobby can leave the
      // server paused
      SetPaused(false);
      ...
      if (s_bExitWhenEmpty) {
        bExit = true;
        Warning("Server shutting down because sv_shutdown was requested and a "
                "server is empty.\n");
      } else {
        if (sv_memlimit.GetInt()) {
          if (ApproximateProcessMemoryUsage() >
              1024 * 1024 * sv_memlimit.GetInt()) {
            if ((sv_minuptimelimit.GetFloat() > 0) &&
                ((Plat_FloatTime() - s_flPlatFloatTimeBeginUptime) / 3600.0 <
                 sv_minuptimelimit.GetFloat())) {
              Warning("Server is using %dMB with an sv_memory_limit of %dMB, "
                      "but will not shutdown because sv_minuptimelimit is %.3f "
                      "hr while current uptime is %.3f\n",
                      ApproximateProcessMemoryUsage() / (1024 * 1024),
                      sv_memlimit.GetInt(), sv_minuptimelimit.GetFloat(),
                      (Plat_FloatTime() - s_flPlatFloatTimeBeginUptime) /
                          3600.0);
            } else {
              Warning("Server shutting down because of using %dMB with an "
                      "sv_memory_limit of %dMB\n",
                      ApproximateProcessMemoryUsage() / (1024 * 1024),
                      sv_memlimit.GetInt());
              bExit = true;
            }
        ...
        if ((sv_maxuptimelimit.GetFloat() > 0) &&
            ((Plat_FloatTime() - s_flPlatFloatTimeBeginUptime) / 3600.0 >
             sv_maxuptimelimit.GetFloat())) {
          Warning("Server will shutdown because sv_maxuptimelimit is %.3f hr "
                  "while current uptime is %.3f, using %dMB with an "
                  "sv_memory_limit of %dMB\n",
                  sv_maxuptimelimit.GetFloat(),
                  (Plat_FloatTime() - s_flPlatFloatTimeBeginUptime) / 3600.0,
                  ApproximateProcessMemoryUsage() / (1024 * 1024),
      ...
      // #ifdef _LINUX
      //			// if we are a child process running forked, we
      //want to exit now. We want to "really" exit. no destructors, no nothing
      //			if ( IsChildProcess() )
      //// are we a subprocess?
      //			{
      //				syscall( SYS_exit_group, 0 );	// we
      //are not going to perform a normal c++ exit. We _dont_ want to run
      //destructors, etc.
      //			}
      // #endif
      if (bExit) {
...
void CGameServer::UpdateHibernationState() {
  ...
  for (int iClient = 0; iClient < m_Clients.Count(); iClient++) {
    CBaseClient *pClient = m_Clients[iClient];
    // don't consider the client being removed, it still shows as connected but
    // won't be in a moment
    if (pClient->IsConnected() &&
        (pClient->IsSplitScreenUser() || !pClient->IsFakeClient())) {
      bHaveAnyClients = true;
      break;
    }
  }

  bool hibernateFromGCServer =
      (g_iServerGameDLLVersion < 8) || !serverGameDLL->GetServerGCLobby() ||
      serverGameDLL->GetServerGCLobby()->ShouldHibernate();

  // If a restart was requested and we're supposed to reboot after XX amount of
  // time, reboot the server.
  ...
  SetHibernating(sv_hibernate_when_empty.GetBool() && hibernateFromGCServer &&
                 !bHaveAnyClients);
}
...
// If enabled, random crashes start to appear in WriteTempEntities, etc. It
// looks like one thread can be in WriteDeltaEntities while another is in
// WriteTempEntities, and both are partying on
// g_FrameSnapshotManager.m_FrameSnapshots. Bruce sent e-mail from a customer
// that stated these crashes don't occur when parallel_sendsnapshot is disabled.
// Zoid said:
//
//   Easiest is just turn off parallel snapshots, it's not much of a win on
//   servers where we are running many instances anyway. It's off in Dota and
//   CSGO dedicated servers.
//
// Bruce also had a patch to disable this in
// //ValveGames/staging/game/tf/cfg/unencrypted/print_instance_config.py
...
void CGameServer::SendClientMessages(bool bSendSnapshots) {
  ...
  if (receivingClientCount) {
    ...
    if (receivingClientCount > 1 && sv_parallel_sendsnapshot.GetBool()) {
      ...
      // must be run on the main thread due to un-threadsafe global state
      // access. It will replace anything that it does process with a NULL
      // pointer.
      ParallelProcess("SV_ParallelSendSnapshot", pReceivingClients,
                      receivingClientCount, &SV_ParallelSendSnapshot);
    }
...
void SV_BroadcastVoiceData(IClient *pClient, int nBytes, char *data,
                           int64 xuid) {
  // Disable voice?
...
void SV_CreateBaseline(void) {
  ...
  if (!g_pLocalNetworkBackdoor) {
    ...
    for (int entnum = 0; entnum < sv.num_edicts; entnum++) {
      ...
      SV_EnsureInstanceBaseline(pClass, entnum, packedData,
...
//-----------------------------------------------------------------------------
// Purpose: Ensure steam context is initialized for multiplayer gameservers.
// Idempotent.
//-----------------------------------------------------------------------------
...
bool SV_ActivateServer() {
  ...
  if (g_pReplay && g_pReplay->IsReplayEnabled()) {
    ...
    if (replay->IsActive()) {
      ...
      CGameClient *pClient =
          (CGameClient *)sv.CreateFakeClient(replay_name.GetString());
  ...
  if (tv_enable.GetBool()) {
    if (CommandLine()->FindParm("-nohltv")) {
      ...
      if (hltv->IsActive() && hltv->IsMasterProxy()) {
        ...
        CGameClient *pClient =
            (CGameClient *)sv.CreateFakeClient(tv_name.GetString());
...
static void SV_AllocateEdicts() {
  ...
  COMPILE_TIME_ASSERT(MAX_EDICT_BITS + 1 <=
                      8 * sizeof(sv.edicts[0].m_EdictIndex));
...
void CGameServer::ReloadWhitelist(const char *pMapName) {
  ...
  // We can use this to detect if they are running out of P4, and if so, don't
  // use the whitelist
...
================
*/
bool CGameServer::SpawnServer(const char *szMapName, const char *szMapFile,
  ...
  // Reset the last used count on all models before beginning the new load --
  // The nServerCount value on models could be from a client load connecting to
  // a different server, and we know we're at the beginning of a new load now.
  ...
  // Clear out the mapversion so it's reset when the next level loads. Needed
  // for changelevels.
  ...
  if (!CommandLine()->FindParm("-allowstalezip")) {
    if (g_pFileSystem->FileExists("stale.txt", "GAME")) {
      Warning("This map is not final!!  Needs to be rebuilt without "
              "-keepstalezip and without -onlyents\n");
    }
  }
  ...
  host_state.SetWorldModel(modelloader->GetModelForName(
      szMapFile, IModelLoader::FMODELLOADER_SERVER));
...
#endif
  // Add in world
  PrecacheModel(szMapFile, RES_FATALIFMISSING | RES_PRELOAD,
...
void CGameServer::UpdateMasterServerPlayers() {
  ...
  for (int i = 0; i < GetClientCount(); i++) {
    ...
    Steam3Server().SteamGameServer()->BUpdateUserData(
        client->m_SteamID, client->GetClientName(), pl->frags);
  }
}
...
void SV_Think(bool bIsSimulating) {
  VPROF("SV_Physics");
  tmZone(TELEMETRY_LEVEL1, TMZF_NONE, "SV_Think(%s)",
         bIsSimulating ? "simulating" : "not simulating");
  ...
  if (s_timeForceShutdown > 0.0) {
    if (s_timeForceShutdown < Plat_FloatTime()) {
      Warning("Server shutting down because sv_shutdown was requested and "
              "timeout has expired.\n");
      HostState_Shutdown();
    }
  }
  ...
  //		if ( sv.IsHibernating() )
  //		{
  //			// if we're hibernating, just sleep for a while and do
  //not call server.dll to run a frame 			int nMilliseconds =
  //sv_hibernate_ms.GetInt(); #ifndef DEDICATED // Non-Linux 			if (
  ...
  //			}
  // #endif
  //			g_pNetworkSystem->SleepUntilMessages( NS_SERVER,
...
void SV_Frame(bool finalTick) {
  ...
  // This value is read on another thread, so this needs to only happen once per
  // frame and be atomic.
