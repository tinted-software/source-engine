...
ConVar cl_entityreport_sorted(
    ...
    "For debugging, draw entity states to console in sorted order. [0 = "
    "disabled, 1 = average, 2 = current, 3 = peak");
...
// Purpose:
//-----------------------------------------------------------------------------
bool CEntityReportPanel::DrawEntry(int row, int col, int rowheight,
                                   int colwidth, int entityIdx) {
  ...
  if (pNet && ((pClientClass = pNet->GetClientClass())) != NULL) {
    ...
    Q_snprintf(text, sizeof(text), "(%i) %s", entityIdx,
               pClientClass->m_pNetworkName);

    g_pVGuiLocalize->ConvertANSIToUnicode(text, unicode, sizeof(unicode));

    DrawColoredText(m_hFont, left + col * colwidth, top + row * rowheight, r, g,
                    b, a, unicode);
...
void CEntityReportPanel::Paint() {
  ...
  if (cl_entityreport_sorted.GetInt() != ENTITYSORT_NONE) {
    ...
    for (int i = 0, entityIdx = start; entityIdx <= lastused;
         ++i, ++entityIdx) {
    ...
    for (int i = 0; i < count; ++i) {
      ...
      if (DrawEntry(row, col, rowheight, colwidth, entityIdx)) {
        row++;
        if (top + row * rowheight >
            videomode->GetModeStereoHeight() - rowheight) {
          row = 0;
          col++;
