...
private:
  ...
  //	~CNetworkStringTable_LockOverride(); // no impl
  CNetworkStringTable_LockOverride(
      const CNetworkStringTable_LockOverride &); // no impl
  CNetworkStringTable_LockOverride &
  operator=(const CNetworkStringTable_LockOverride &); // no impl
public:
...
public:
  // GetModel, RegisterDynamicModel(name) are in
  // CModelInfoClient/CModelInfoServer
  virtual int GetModelIndex(const char *name) const;
  virtual int GetModelClientSideIndex(const char *name) const;

  virtual bool RegisterModelLoadCallback(int modelindex,
  ...
  virtual void GetModelBounds(const model_t *model, Vector &mins, Vector &maxs)
      const;
  ...
  virtual void GetIlluminationPoint(
      ...
      const Vector &origin, const QAngle &angles, Vector *pLightingOrigin);
  ...
  virtual const studiohdr_t *FindModel(
      const studiohdr_t *pStudioHdr, void **cache, char const *modelname) const;
  ...
  byte *LoadAnimBlock(model_t * model, const studiohdr_t *pStudioHdr,
...
public:
  struct ModelFileHandleHash {
    uint operator()(model_t *p) const {
      return PointerHashFunctor()(p->fnHandle);
    }
    uint operator()(FileNameHandle_t fn) const {
      return PointerHashFunctor()(fn);
    }
  };
...
protected:
  // Client-only dynamic model indices are iterators into this struct (only
  // populated by CModelInfoClient subclass)
  CUtlStableHashtable<model_t *, empty_t, ModelFileHandleHash,
                      ModelFileHandleEq, int16, FileNameHandle_t>
...
int CModelInfo::GetModelIndex(const char *name) const {
  ...
  if (pTable) {
    ...
    // dynamic model tables in old system did not have a full path with
    // "models/" prefix
    if (V_strnicmp(name, "models/", 7) == 0 && demoplayer &&
...
model_t *CModelInfo::LookupDynamicModel(int i) {
  ...
  if (IsClientOnlyModelIndex(i)) {
    ...
    return m_ClientDynamicModels.IsValidHandle(h) ? m_ClientDynamicModels[h]
                                                  : NULL;
  } else {
    int netidx = MODEL_TO_NETDYNAMIC(i);
    if (m_NetworkedDynamicModels.IsValidIndex(netidx) &&
        m_NetworkedDynamicModels[netidx])
...
bool CModelInfo::RegisterModelLoadCallback(int modelIndex,
                                           IModelLoadCallback *pCallback,
                                           bool bCallImmediatelyIfLoaded) {
  const model_t *pModel = GetModel(modelIndex);
  Assert(pModel);
  if (pModel && IsDynamicModelIndex(modelIndex)) {
    return modelloader->RegisterModelLoadCallback(
        const_cast<model_t *>(pModel), IsClientOnlyModelIndex(modelIndex),
...
void CModelInfo::UnregisterModelLoadCallback(int modelIndex,
                                             IModelLoadCallback *pCallback) {
  if (modelIndex == -1) {
    ...
    Assert(pModel);
    if (pModel) {
      modelloader->UnregisterModelLoadCallback(
          const_cast<model_t *>(pModel), IsClientOnlyModelIndex(modelIndex),
          pCallback);
    }
  }
...
bool CModelInfo::IsDynamicModelLoading(int modelIndex) {
  ...
  return pModel && modelloader->IsDynamicModelLoading(
                       pModel, IsClientOnlyModelIndex(modelIndex));
}
...
void CModelInfo::AddRefDynamicModel(int modelIndex) {
  if (IsDynamicModelIndex(modelIndex)) {
    ...
    Assert(pModel);
    if (pModel) {
      modelloader->AddRefDynamicModel(pModel,
                                      IsClientOnlyModelIndex(modelIndex));
    }
  }
}
...
void CModelInfo::ReleaseDynamicModel(int modelIndex) {
  if (IsDynamicModelIndex(modelIndex)) {
    ...
    Assert(pModel);
    if (pModel) {
      modelloader->ReleaseDynamicModel(pModel,
                                       IsClientOnlyModelIndex(modelIndex));
    }
  }
}
...
int CModelInfo::GetModelType(const model_t *model) const {
  ...
  if (model->type == mod_bad) {
    if (m_ClientDynamicModels.Find((model_t *)model) !=
        m_ClientDynamicModels.InvalidHandle())
      return mod_studio;
    INetworkStringTable *pTable = GetDynamicModelStringTable();
    if (pTable &&
        pTable->FindStringIndex(model->strName) != INVALID_STRING_INDEX)
      return mod_studio;
    ...
    if (pTable && demoplayer && demoplayer->IsPlayingBack() &&
        demoplayer->GetProtocolVersion() < PROTOCOL_VERSION_20 &&
        V_strnicmp(model->strName, "models/", 7) == 0 &&
...
//-----------------------------------------------------------------------------
virtualmodel_t *
CModelInfo::GetVirtualModel(const studiohdr_t *pStudioHdr) const {
...
// Purpose:
//-----------------------------------------------------------------------------
byte *CModelInfo::GetAnimBlock(const studiohdr_t *pStudioHdr,
                               int nBlock) const {
  MDLHandle_t handle = VoidPtrToMDLHandle(pStudioHdr->VirtualModel());
...
public:
  ...
  virtual void OnDynamicModelsStringTableChange(int nStringIndex,
                                                const char *pString,
                                                const void *pData);

  virtual void
  GetModelMaterialColorAndLighting(const model_t *model, const Vector &origin,
                                   const QAngle &angles, trace_t *pTrace,
                                   Vector &lighting, Vector &matColor);
...
int CModelInfoServer::RegisterDynamicModel(const char *name, bool bClientSide) {
  ...
  bool bWasLocked =
      static_cast<CNetworkStringTable_LockOverride *>(pTable)->LockWithRetVal(
          false);
  char nIsLoaded = 0;
  ...
  GrowNetworkedDynamicModels(netidx);
  m_NetworkedDynamicModels[netidx] =
      modelloader->GetDynamicModel(name, bClientSide);
...
void CModelInfoServer::OnDynamicModelsStringTableChange(int nStringIndex,
                                                        const char *pString,
                                                        const void *pData) {
  AssertMsg(false, "CModelInfoServer::OnDynamicModelsStringTableChange should "
                   "never be called");
}
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CModelInfoServer, IVModelInfo003,
                                  VMODELINFO_SERVER_INTERFACE_VERSION_3,
                                  g_ModelInfoServer);
...
int CModelInfoClient::RegisterDynamicModel(const char *name, bool bClientSide) {
  ...
  Assert(MODEL_TO_CLIENTSIDE((short)CLIENTSIDE_TO_MODEL(localidx)) ==
         (int)localidx);
...
void CModelInfoClient::OnDynamicModelsStringTableChange(int nStringIndex,
                                                        const char *pString,
                                                        const void *pData) {
  ...
  bool bServerLoaded =
      pData ? (*(char *)pData != 0)
            : (g_ClientGlobalVariables.network_protocol <= PROTOCOL_VERSION_20);
...
CModelInfoClient::ComputeScreenFade(const Vector &vecAbsOrigin, float flRadius,
                                    float flFadeScale,
                                    const ScreenFadeInfo_t &fade) const {
