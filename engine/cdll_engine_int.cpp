...
public:
  ...
  virtual bool StartDemoRecording(const char *pszFilename,
                                  const char *pszFolder = NULL);
  virtual void StopDemoRecording(void);
  virtual void TakeScreenshot(const char *pszFilename,
                              const char *pszFolder = NULL);
};
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CEngineClient, IVEngineClient,
                                  ...
                                  s_VEngineClient);
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CEngineClient, IVEngineClient013,
                                  VENGINE_CLIENT_INTERFACE_VERSION_13,
                                  s_VEngineClient);
...
void CEngineClient::ClientCmd(const char *szCmdString) {
  ...
  if (cl.m_bRestrictClientCommands && !Cbuf_HasRoomForExecutionMarkers(2)) {
    AssertMsg(false, "CEngineClient::ClientCmd called but there is no room for "
                     "the execution markers. Ignoring command.");
    return;
  }
  ...
  if (cl.m_bRestrictClientCommands)
    Cbuf_AddTextWithMarkers(
        eCmdExecutionMarker_Enable_FCVAR_CLIENTCMD_CAN_EXECUTE, szCmdString,
...
bool CEngineClient::GetPlayerInfo(int ent_num, player_info_t *pinfo) {
  ...
  int cubPlayerInfo;
  player_info_t *pi = (player_info_t *)cl.m_pUserInfoTable->GetStringUserData(
      ent_num, &cubPlayerInfo);
...
int CEngineClient::GetPlayerForUserID(int userID) {
  ...
  // We are occasionally getting crashes here where it looks like
  // cl.m_nMaxClients
...
//-----------------------------------------------------------------------------
SkyboxVisibility_t
CEngineClient::IsSkyboxVisibleFromPoint(const Vector &vecPoint) {
...
bool CEngineClient::IsPlayingDemoALocallyRecordedDemo() {
  return IsPlayingDemo() && demoplayer && demoplayer->GetDemoFile() &&
         !V_strnicmp(demoplayer->GetDemoFile()->m_DemoHeader.servername,
                     "localhost", 9);
}
...
bool ClientDLL_Load() {
  ...
  // Check the signature on the client dll.  If this fails we load it anyway but
  // put this client into insecure mode so it won't connect to secure servers
  // and get VAC banned
  ...
  if (g_ClientDLLModule) {
    ...
    if (g_ClientFactory) {
      ...
      if (g_pSourceVR) {
        g_pClientVR = (IClientVirtualReality *)g_ClientFactory(
            CLIENTVIRTUALREALITY_INTERFACE_VERSION, NULL);
        if (!g_pClientVR) {
    ...
    // tell Steam to do a quick verify of the install. Sys_Error doesn't return,
    // so do this first.
    if (Steam3Client().SteamApps())
      Steam3Client().SteamApps()->MarkContentCorrupt(true);

    // library failed to load
    Sys_Error("Could not load library client. Try restarting. If that doesn't "
              "work, verify the cache.");
  }
  ...
  if (g_ClientDLL) {
    ...
    if (g_ClientFactory) {
      ...
      if (Replay_IsSupportedModAndPlatform()) {
        ...
        g_pClientReplay = (IClientReplay *)g_ClientFactory(
            CLIENT_REPLAY_INTERFACE_VERSION, NULL);
        if (!g_pClientReplay) {
        ...
        if (!sv.IsDedicated()) {
          ...
          g_pReplayPerformanceManager =
              g_pClientReplayContext->GetPerformanceManager();
          g_pReplayPerformanceController =
              g_pClientReplayContext->GetPerformanceController();
    ...
    if (g_pMaterialSystemHardwareConfig &&
      ...
      if ((Q_stricmp(COM_GetModDirectory(), "tf") == 0) ||
        ...
        Sys_Error("Your graphics hardware must support at least pixel shader "
                  "version 1.1 to run this game!");
      }
    }
  }
...
// Purpose:
//-----------------------------------------------------------------------------
bool CEngineClient::StartDemoRecording(const char *pszFilename,
                                       const char *pszFolder /* = NULL */) {
  ...
  if (!g_ClientDLL->CanRecordDemo(szTemp, sizeof(szTemp))) {
    ConMsg("%s\n", szTemp); // re-use szTemp as the error string if the client
                            // prevents us from starting a demo
    return false;
  }

  if (bUseFolder) {
    V_sprintf_safe(szTemp, "%s%c%s", pszFolder, CORRECT_PATH_SEPARATOR,
                   pszFilename);
  } else {
...
// Purpose:
//-----------------------------------------------------------------------------
void CEngineClient::TakeScreenshot(const char *pszFilename,
                                   const char *pszFolder /* = NULL */) {
  bool bUseFolder = (pszFolder && pszFolder[0]);
  ...
  if (bUseFolder) {
    V_sprintf_safe(szFinal, "%s%c%s", pszFolder, CORRECT_PATH_SEPARATOR,
                   pszFilename);
  } else {
