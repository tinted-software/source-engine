...
void UpdateNameFromSteamID(IConVar *pConVar, CSteamID *pSteamID) {
  ...
  const char *pszName =
      Steam3Client().SteamFriends()->GetFriendPersonaName(*pSteamID);
...
#ifndef SWDS
  if (Steam3Client().SteamUtils() && Steam3Client().SteamFriends() &&
...
// ----------------------------------------------------------------------------------------
// //
void CL_ClanIdChanged(IConVar *pConVar, const char *pOldString,
                      float flOldValue) {
#ifndef SWDS
...
ConVar cl_clanid("cl_clanid", "0",
                 FCVAR_ARCHIVE | FCVAR_USERINFO | FCVAR_HIDDEN,
                 "Current clan ID for name decoration", CL_ClanIdChanged);
ConVar cl_show_connectionless_packet_warnings(
    "cl_show_connectionless_packet_warnings", "0", FCVAR_NONE,
    "Show console messages about ignored connectionless packets on the "
    "client.");
...
void CBaseClientState::SendConnectPacket(int challengeNr, int authProtocol,
                                         uint64 unGSSteamID, bool bGSSecure) {
  ...
  msg.WriteString(
      GetSteamInfIDVersionInfo().szVersionString); // product version
  //	msg.WriteByte( ( g_pServerPluginHandler->GetNumLoadedPlugins() > 0 ) ? 1
  //: 0 ); // have any client-side server plug-ins been loaded?
...
//-----------------------------------------------------------------------------
bool CBaseClientState::PrepareSteamConnectResponse(uint64 unGSSteamID,
                                                   bool bGSSecure,
                                                   const netadr_t &adr,
                                                   bf_write &msg) {
...
static ConVar
    cl_connectmethod("cl_connectmethod", "", FCVAR_USERINFO | FCVAR_HIDDEN,
                     "Method by which we connected to the current server.");

/* static */ bool CBaseClientState::ConnectMethodAllowsRedirects() {
  // Only HLTV should be allowed to redirect clients, but malicious servers can
  // answer a connect attempt as HLTV and then redirect elsewhere. A
  // somewhat-more complete fix for this would involve tracking our redirected
  // status and refusing to interact with non-HLTV servers. For now, however, we
  // just blacklist server browser / matchmaking connect methods from allowing
...
void CBaseClientState::FullConnect(netadr_t &adr) {
  ...
  if (event) {
    ...
    event->SetInt("ip",
                  m_NetChannel->GetRemoteAddress()
                      .GetIPNetworkByteOrder()); // <<< Network byte order?
...
void CBaseClientState::Disconnect(const char *pszReason, bool bShowMainMenu) {
  ...
  if (m_NetChannel) {
    m_NetChannel->Shutdown((pszReason && *pszReason) ? pszReason
                                                     : "Disconnect by user.");
    m_NetChannel = NULL;
  }
}
...
void CBaseClientState::CheckForResend(void) {
  ...
  if (m_nRetryNumber == 0) {
    ...
    if (event) {
      ...
      event->SetInt("ip",
                    adr.GetIPNetworkByteOrder()); // <<< Network byte order?
...
bool CBaseClientState::ProcessConnectionlessPacket(netpacket_t *packet) {
  ...
  if ((packet->from.GetType() != NA_LOOPBACK) &&
      (packet->from.GetIPNetworkByteOrder() !=
       adrServerConnectingTo.GetIPNetworkByteOrder())) {
  ...
  case S2C_CONNECTION:
    if (m_nSignonState == SIGNONSTATE_CHALLENGE) {
      ...
      if (myChallenge != m_retryChallenge) {
        Msg("Server connection did not have the correct challenge, "
            "ignoring.\n");
        return false;
      }
  ...
  case S2C_CHALLENGE: // Response from getchallenge we sent to the server we are
                      // connecting to
  ...
  case S2C_CONNREJECT:
    if (m_nSignonState == SIGNONSTATE_CHALLENGE) // Spoofed?
    {
      int myChallenge = msg.ReadLong();
      if (myChallenge != m_retryChallenge) {
        Msg("Received connection rejection that didn't match my challenge, "
            "ignoring.\n");
        return false;
      }
...
bool CBaseClientState::ProcessStringCmd(NET_StringCmd *msg) {
  ...
  if (!Cbuf_HasRoomForExecutionMarkers(2)) {
    AssertMsg(false, "CBaseClientState::ProcessStringCmd called but there is "
                     "no room for the execution markers. Ignoring command.");
    return true;
  }

  // Run the command, but make sure the command parser knows to only execute
  // commands marked with FCVAR_SERVER_CAN_EXECUTE.
  Cbuf_AddTextWithMarkers(eCmdExecutionMarker_Enable_FCVAR_SERVER_CAN_EXECUTE,
                          msg->m_szCommand,
...
bool CBaseClientState::ProcessSetConVar(NET_SetConVar *msg) {
  ...
  for (int i = 0; i < msg->m_ConVars.Count(); i++) {
    ...
    if (!var.IsFlagSet(FCVAR_REPLICATED)) {
      ConMsg("SetConVar: Can't set server cvar %s to %s, not marked as "
...
bool CBaseClientState::ProcessServerInfo(SVC_ServerInfo *msg) {
  ...
  if (!sv.IsActive() &&
    ...
    // if you are joing a remote server it MUST be multipler and have maxplayer
    // set to more than 1 this prevents a spoofed ServerInfo packet from making
    // the client think its in singleplayer and turning off a bunch of security
    // checks
    if (m_nMaxClients <= 1) {
      ConMsg("Bad maxclients (%u) from server.\n", m_nMaxClients);
      return false;
  ...
  // Set default filename, but this is finalized by ClientState later, so it
  // should not be depended on yet. See PrepareLevelResources call
  Host_DefaultMapFileName(msg->m_szMapName, m_szLevelFileName,
                          sizeof(m_szLevelFileName));
...
bool CBaseClientState::ProcessCreateStringTable(SVC_CreateStringTable *msg) {
  ...
  if (msg->m_bDataCompressed) {
    ...
    /// XXX(JohnS): 11/08/2016 - The PAD_NUMBER() call below was overflowing on
    /// UINT32_MAX-3 values.  Enforcing
    //              resource-usage limits at this level is a lost cause without
    //              a massive overhaul, but clamp these to somewhat reasonable
    //              ranges to prevent overflows with less-audited code in the
    //              engine.
    bool bSuccess = false;
    if (msg->m_DataIn.TotalBytesAvailable() > 0 &&
        msgCompressedSize <=
            (unsigned int)msg->m_DataIn.TotalBytesAvailable() &&
      ...
      // uncompress data
      bSuccess =
          COM_BufferToBufferDecompress(uncompressedBuffer, &uncompressedSize,
                                       compressedBuffer, msgCompressedSize);
...
bool CBaseClientState::ProcessUpdateStringTable(SVC_UpdateStringTable *msg) {
  ...
  // m_StringTableContainer is NULL on level transitions, Seems to be caused by
  // a UpdateStringTable packet comming in before the ServerInfo packet
  //   I'm not sure this is safe, but at least we won't crash. The realy odd
  //   thing is this can happen on the server as well.//tmauer
  if (m_StringTableContainer != NULL) {
    CNetworkStringTable *table =
        (CNetworkStringTable *)m_StringTableContainer->GetTable(
            msg->m_nTableID);

    table->ParseUpdate(msg->m_DataIn, msg->m_nChangedEntries);
  } else {
    Warning("m_StringTableContainer is NULL in "
...
bool CBaseClientState::LinkClasses() {
  ...
  //		{
  //			Host_EndGame(true, "CL_ParseClassInfo_EndClasses: class
  //%d not initialized.\n", i); 			return false;
  //		}
  //	}
...
bool CBaseClientState::GetClassBaseline(int iClass, void const **pData,
  ...
  if (pInfo->m_InstanceBaselineIndex == INVALID_STRING_INDEX) {
    ...
    ErrorIfNot(pInfo->m_InstanceBaselineIndex != INVALID_STRING_INDEX,
               ("GetDynamicBaseline: FindStringIndex(%s-%s) failed.", str,
                pInfo->m_ClassName););
  }
...
// Returns dem file protocol version, or, if not playing a demo, just returns
// PROTOCOL_VERSION
int CBaseClientState::GetDemoProtocolVersion() const {
#ifndef SWDS // why is demo play undefined? it shuold be fine on a dedicated
             // server
