  ...
  if (pText && pText[0] == '$') {
    ...
    if (IsToken(pText, "position")) {
      ...
      if (ParseFloats(pText, tempFloat, 4)) {
        // that's original code, msvc2015 generates illegal instruction on amd64
        // architecture
        /*for ( int i = 0; i < 4; ++i )
        {
...
void TextMessageParse(byte *pMemFile, int fileSize) {
  ...
  while (memfgets(pMemFile, fileSize, &filePos, buf, 512) != NULL) {
    ...
    case MSGFILE_NAME:
      ...
      if (ParseDirective(trim)) // Is this a directive "$command"?, if so parse
                                // it and break
        break;
    ...
    case MSGFILE_TEXT:
      if (IsEndOfText(trim)) {
        ...
        // the Find() lookup will fail (so subtract 2)
