...
CBaseActionEditDialog::CBaseActionEditDialog(CDemoEditorPanel *parent,
    ...
    : vgui::Frame(parent, CBaseDemoAction::NameForType(action->GetType())),
      m_pEditor(parent), m_pAction(action), m_bNewAction(newaction) {
  if (m_bNewAction) {
    SetTitle(
...
private:
  ...
  bool SaveDifferingColor(vgui::TextEntry *control, byte *r, byte *g, byte *b,
                          byte *a);
...
bool CBaseActionTextMessageStartDialog::SaveDifferingColor(
    vgui::TextEntry *control, byte *r, byte *g, byte *b, byte *a) {
  bool bret = false;
