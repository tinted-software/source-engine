...
//client 		of this system should know whether the shadows are always changing or
//not
//
...
ConVar r_shadows_gamecontrol("r_shadows_gamecontrol", "-1",
                             FCVAR_CHEAT); // Shadow override controlled by game
                                           // entities (shadow_controller)
...
private:
  ...
  bool AddDecalToShadowList(ShadowHandle_t handle,
                            ShadowDecalHandle_t decalHandle);
...
ShadowHandle_t CShadowMgr::CreateShadowEx(IMaterial *pMaterial,
  ...
  if (h == m_Shadows.InvalidIndex()) {
    ExecuteNTimes(10, Warning("CShadowMgr::CreateShadowEx - overflowed "
                              "m_Shadows linked list!\n"));
    return h;
  }
  //=============================================================================
  // HPE_END
...
// Adds the surface to the list for this shadow
//-----------------------------------------------------------------------------
bool CShadowMgr::AddDecalToShadowList(ShadowHandle_t handle,
                                      ShadowDecalHandle_t decalHandle) {
  ...
  if (idx == m_ShadowSurfaces.InvalidIndex()) {
    ExecuteNTimes(10, Warning("CShadowMgr::AddDecalToShadowList - overflowed "
                              "m_ShadowSurfaces linked list!\n"));
    return false;
  }
...
//-----------------------------------------------------------------------------
const CShadowMgr::SurfaceBounds_t *
CShadowMgr::GetSurfaceBounds(SurfaceHandle_t surfID) {
...
//-----------------------------------------------------------------------------
inline ShadowDecalHandle_t
CShadowMgr::AddShadowDecalToSurface(SurfaceHandle_t surfID,
                                    ShadowHandle_t handle) {
  ShadowDecalHandle_t decalHandle = m_ShadowDecals.Alloc(true);
  if (decalHandle == m_ShadowDecals.InvalidIndex()) {
    ExecuteNTimes(10, Warning("CShadowMgr::AddShadowDecalToSurface - "
                              "overflowed m_ShadowDecals linked list!\n"));
    return decalHandle;
  }
...
inline void
CShadowMgr::RemoveShadowDecalFromSurface(SurfaceHandle_t surfID,
                                         ShadowDecalHandle_t decalHandle) {
...
void CShadowMgr::ApplyShadowToDisplacement(ShadowBuildInfo_t &build,
  ...
  if (!bIsFlashlight &&
      (MSurf_Flags(pDispInfo->GetParent()) & SURFDRAW_NOSHADOWS))
    return;
...
// &endShadowSpace, 						   const VMatrix &shadowToWorld, unsigned char r, unsigned char
//g, 						   unsigned char b, bool ignoreZ )
//{
...
void CShadowMgr::ApplyFlashlightToLeaf(const Shadow_t &shadow, mleaf_t *pLeaf,
  ...
  // The flashlight frustum didn't intersect the bounding box for this leaf! Get
  // outta here!
...
//-----------------------------------------------------------------------------
inline bool
CShadowMgr::GenerateDispShadowRenderInfo(IMatRenderContext *pRenderContext,
                                         ShadowDecal_t &decal,
                                         ShadowRenderInfo_t &info) {
  //=============================================================================
  // HPE_BEGIN:
...
//-----------------------------------------------------------------------------
inline bool
CShadowMgr::GenerateNormalShadowRenderInfo(IMatRenderContext *pRenderContext,
                                           ShadowDecal_t &decal,
                                           ShadowRenderInfo_t &info) {
  //=============================================================================
  // HPE_BEGIN:
...
void CShadowMgr::RenderShadowList(IMatRenderContext *pRenderContext,
  ...
  if (m_DecalsToRender > m_ShadowDecalCache.Count()) {
    ...
    int diff = min(m_DecalsToRender, (int)MAX_SHADOW_DECAL_CACHE_COUNT) -
               m_ShadowDecalCache.Count();
    if (diff > 0) {
      // Grow the cache.
      m_ShadowDecalCache.Grow(diff);
      DevMsg("[CShadowMgr::RenderShadowList] growing shadow decal cache "
             "(decals: %d, cache: %d, diff: %d).\n",
  ...
  if (m_DecalsToRender > m_DispShadowDecalCache.Count()) {
    // Don't grow past the MAX_SHADOW_DECAL_CACHE_COUNT cap.
    int diff = min(m_DecalsToRender, (int)MAX_SHADOW_DECAL_CACHE_COUNT) -
               m_DispShadowDecalCache.Count();
    if (diff > 0) {
      // Grow the cache.
      m_DispShadowDecalCache.Grow(diff);
      DevMsg("[CShadowMgr::RenderShadowList] growing disp shadow decal cache "
             "(decals: %d, cache: %d, diff: %d).\n",
...
void CShadowMgr::SetStencilAndScissor(IMatRenderContext *pRenderContext,
  ...
  TODO: do we even need to do the far plane?
                  ...
                  // Clip near plane to flashlight frustum and tack on to list
                  nClipCount = ClipPlaneToFrustum( vFarPlane,
  vClippedPolygons[nNumPolygons], vWorldFrustumPoints ); if ( nClipCount > 2 )
  // If the near plane clipped and resulted in a polygon, take note in the
  polygon list
                  {
                          nNumVertices[nNumPolygons] = nClipCount;
                          nNumPolygons++;
  ...
  */
  ...
  if (r_flashlightclip.GetBool() && bUseStencil) {
    /*
       ...
       STENCILOPERATION_KEEP );						//
       Stencil passes but depth fails pRenderContext->SetStencilPassOperation(
    ...
    */
    ...
    pRenderContext->SetStencilTestMask(
        m_Shadows[flashlightInfo.m_Shadow].m_ucShadowStencilBit);
    pRenderContext->SetStencilWriteMask(
        m_Shadows[flashlightInfo.m_Shadow]
            .m_ucShadowStencilBit); // Bit mask which is specific to this shadow
      ...
      if (numIndices > 0) {
        ...
        pRenderContext->BindVertexBuffer(
            ...
            materialSortInfoArray[sortID]
                .material
                ->GetVertexFormat()); // hack fixme. . . use currently bound
                                      // material format instead of passing in?
         ...
         m_FlashlightStates.InvalidIndex()) // Count up the shadows
  {
    ...
    if (m_Shadows[flashlightInfo.m_Shadow].m_pFlashlightDepthTexture) {
      ...
      meshBuilder.TexCoord2f(
          0, 0.0f,
          1.0f); // Posix is rotated due to render target origin differences
#else
