...
//-----------------------------------------------------------------------------
void CSession::SetContext(const uint nContextId, const uint nContextValue,
                          const bool bAsync) {
  g_pXboxSystem->UserSetContext(XBX_GetPrimaryUserId(), nContextId,
                                nContextValue, bAsync);
}
void CSession::SetProperty(const uint nPropertyId, const uint cbValue,
...
void CSession::UpdateSlots(const CClientInfo *pClient, bool bAddPlayers) {
  ...
  if (bAddPlayers) {
    ...
    // The cast to 'int' is needed since otherwise underflow will wrap around to
    // very large numbers and the 'max' macro will do nothing.
    m_nPlayerSlots[SLOTS_FILLEDPRIVATE] =
        max(0, (int)(m_nPlayerSlots[SLOTS_FILLEDPRIVATE] -
                     pClient->m_numPrivateSlotsUsed));
    m_nPlayerSlots[SLOTS_FILLEDPUBLIC] =
        max(0, (int)(m_nPlayerSlots[SLOTS_FILLEDPUBLIC] -
...
void CSession::JoinLocal(const CClientInfo *pClient) {
  ...
  // X360TBD: Make async?
  uint ret = g_pXboxSystem->SessionJoinLocal(m_hSession, pClient->m_cPlayers,
                                             nUserIndex, bPrivate, false);
...
void CSession::JoinRemote(const CClientInfo *pClient) {
  ...
  // X360TBD: Make async?
  uint ret = g_pXboxSystem->SessionJoinRemote(m_hSession, pClient->m_cPlayers,
                                              xuids, bPrivate, false);
  if (ret != ERROR_SUCCESS) {
    // Handle error
...
void CSession::RemoveLocal(const CClientInfo *pClient) {
  ...
  // X360TBD: Make async?
  uint ret = g_pXboxSystem->SessionLeaveLocal(m_hSession, pClient->m_cPlayers,
                                              nUserIndex, false);
  if (ret != ERROR_SUCCESS) {
    // Handle error
...
void CSession::RemoveRemote(const CClientInfo *pClient) {
  ...
  // X360TBD: Make async?
  uint ret = g_pXboxSystem->SessionLeaveRemote(m_hSession, pClient->m_cPlayers,
                                               xuids, false);
  if (ret != ERROR_SUCCESS) {
    // Handle error
...
bool CSession::CreateSession() {
  ...
  // Create the session
  uint ret = g_pXboxSystem->CreateSession(
...
void CSession::SetSessionNonce(int64 nonce) { m_SessionNonce = nonce; }
uint64 CSession::GetSessionNonce() { return m_SessionNonce; }
...
void CSession::SetSessionFlags(uint flags) { m_nSessionFlags = flags; }
uint CSession::GetSessionFlags() { return m_nSessionFlags; }
int CSession::GetPlayerCount() {
  return m_nPlayerSlots[SLOTS_FILLEDPRIVATE] +
         m_nPlayerSlots[SLOTS_FILLEDPUBLIC];
}
void CSession::SetFlag(uint nFlag) { m_nSessionFlags |= nFlag; }
...
void CSession::SetOwnerId(uint id) { m_nOwnerId = id; }
