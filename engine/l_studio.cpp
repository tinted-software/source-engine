...
static ConVar
    r_eyeglintlodpixels("r_eyeglintlodpixels", "20.0", FCVAR_CHEAT,
                        "The number of pixels wide an eyeball has to be before "
                        "rendering an eyeglint.  Is a floating point value.");
ConVar r_rootlod("r_rootlod", "0", FCVAR_MATERIAL_SYSTEM_THREAD | FCVAR_ARCHIVE,
                 "Root LOD", true, 0, true, MAX_NUM_LODS - 1, SetRootLOD_f);
...
ConVar r_debugrandomstaticlighting(
    ...
    "Set to 1 to randomize static lighting for debugging.  Must restart for "
    "change to take affect.");
ConVar r_proplightingfromdisk("r_proplightingfromdisk", "1", FCVAR_CHEAT,
...
static ConVar r_proplightingpooling(
    "r_proplightingpooling", "-1.0", FCVAR_CHEAT,
    "0 - off, 1 - static prop color meshes are allocated from a single shared "
    "vertex buffer (on hardware that supports stream offset)");
...
static void
R_SetNonAmbientLightingState(int numLights,
...
public:
  ...
  unsigned int Size() {
    // TODO: This is wrong because we don't currently account for the size of
    // the textures we create. However, that data isn't available until way
    // after this query is made, so just live with this for now I guess?
    return m_nTotalSize;
  }
  ...
  unsigned int m_bColorTextureValid : 1;   // Whether the texture data is valid,
                                           // but not necessarily created
  unsigned int m_bColorTextureCreated : 1; // Whether the texture data has
...
public:
  ...
  virtual void
  ForcedMaterialOverride(IMaterial *newMaterial,
                         OverrideType_t nOverrideType = OVERRIDE_NORMAL);
  ...
  virtual void AddColoredDecal(ModelInstanceHandle_t handle, Ray_t const &ray,
                               const Vector &decalUp, int decalIndex, int body,
                               Color cColor, bool noPokethru = false,
                               int maxLODToDecal = ADDDECAL_TO_ALL_LODS);

  virtual void GetMaterialOverride(IMaterial **ppOutForcedMaterial,
                                   OverrideType_t *pOutOverrideType);
  ...
  inline vertexFileHeader_t *CacheVertexData() {
    return g_pMDLCache->GetVertexData(
        VoidPtrToMDLHandle(m_pStudioHdr->VirtualModel()));
  }
  ...
  void StaticPropColorTexelCallback(void *pContext, const void *pData,
                                    int numReadBytes,
                                    FSAsyncStatus_t asyncStatus);
...
private:
  ...
  // Internal Decal
  void AddDecalInternal(ModelInstanceHandle_t handle, Ray_t const &ray,
                        const Vector &decalUp, int decalIndex, int body,
                        bool bUseColor, Color cColor, bool noPokeThru,
                        int maxLODToDecal);
...
bool CModelRender::Init() {
  ...
  if (IsX360()) {
    g_pQueuedLoader->InstallLoader(RESOURCEPRELOAD_STATICPROPLIGHTING,
                                   &s_ResourcePreloadPropLighting);
  }
...
void CModelRender::TimeAverageAmbientLight(LightingState_t &actualLightingState,
                                           ...
                                           const Vector *pLightingOrigin) {
  flAttenFactor =
      clamp(flAttenFactor, 0.f, 1.f); // don't need this but alex is a coward
  Vector vecDelta;
  for (int i = 0; i < 6; ++i) {
...
static ConVar
    r_drawlightcache("r_drawlightcache", "0", FCVAR_CHEAT,
    ...
    if (!bStaticLighting) {
      ...
      if (r_drawlightcache.GetInt() == 5) {
        if (pModelInst && pModelInst->m_pModel &&
            !pModelInst->m_pModel->strName.IsEmpty()) {
          const char *szModelName = r_lightcachemodel.GetString();
          bDebugModel =
              V_stristr(pModelInst->m_pModel->strName, szModelName) != NULL;
        }
      }
    ...
    if (drawFlags & STUDIORENDER_DRAW_ITEM_BLINK) {
      ...
      if ((pState->numlights > 0) &&
        ...
        for (int i = 0; i < 6; i++) {
          ...
          minCubeLuminance =
              fpmin(minCubeLuminance, luminance); // min luminance
          maxCubeLuminance =
              fpmax(maxCubeLuminance, luminance); // max luminance
...
static ConVar r_drawmodelstatsoverlaymin(
    ...
    "time in milliseconds that a model must take to render before showing an "
    "overlay in r_drawmodelstatsoverlay 2");
static ConVar r_drawmodelstatsoverlaymax(
    "r_drawmodelstatsoverlaymax", "1.5", FCVAR_ARCHIVE,
    "time in milliseconds beyond which a model overlay is fully red in "
...
void CModelRender::DrawModelExecute(const DrawModelState_t &state,
  ...
  if (nFlexCount > 0) {
    ...
    g_pStudioRender->LockFlexWeights(nFlexCount, &pFlexWeights,
                                     bUsesDelayedWeights ? &pFlexDelayedWeights
                                                         : NULL);
    state.m_pRenderable->SetupWeights(pBoneToWorld, nFlexCount, pFlexWeights,
                                      pFlexDelayedWeights);
  ...
  info.m_Decals = (bShadowDepth || bSSAODepth) ? STUDIORENDER_DECAL_INVALID
                                               : state.m_decals;
...
int CModelRender::DrawStaticPropArrayFast(StaticPropRenderInfo_t *pProps,
  ...
  for (int i = 0; i < count; i++) {
    ...
    if (modelIndex < 0) {
      ...
      modelList[modelIndex].pStudioHWData =
          g_pMDLCache->GetHardwareData(modelList[modelIndex].pModel->studio);
    }
  ...
  for (int i = 0; i < modelList.Count(); i++) {
    ...
    modelList[i].lodCount =
        modelList[i].pStudioHWData ? modelList[i].pStudioHWData->m_NumLODs : 0;
    ...
    for (int i = 0; i < objectList.Count(); i++) {
      ...
      if (bStaticLighting) {
        ...
        if (pColorMeshData && (pColorMeshData->m_bColorMeshValid ||
          ...
          if (pCallQueue) {
            if (CacheLock(
                    hColorMeshData)) // CacheCreate above will call functions
                                     // that won't take place until later. If
                                     // color mesh isn't used right away, it
                                     // could get dumped
            {
        ...
        } else // !pLightcache
        {
          ...
          if (bHasDecals) {
            ...
            LightcacheGetDynamic(
                vLightingOrigin, *pDecalLightState, tempStats,
                LIGHTCACHEFLAGS_STATIC | LIGHTCACHEFLAGS_DYNAMIC |
                    LIGHTCACHEFLAGS_LIGHTSTYLE | LIGHTCACHEFLAGS_ALLOWFAST);
          }
        }
...
//-----------------------------------------------------------------------------
matrix3x4_t *
CModelRender::DrawModelShadowSetup(IClientRenderable *pRenderable, int body,
                                   int skin, DrawModelInfo_t *pInfo,
                                   matrix3x4_t *pCustomBoneToWorld) {
#ifndef SWDS
...
CColorMeshData *
CModelRender::FindOrCreateStaticPropColorData(ModelInstanceHandle_t handle) {
  if (handle == MODEL_INSTANCE_INVALID ||
      !g_pMaterialSystemHardwareConfig->SupportsColorOnSecondStream()) {
...
void CModelRender::StaticPropColorMeshCallback(void *pContext,
  ...
  if (IsX360()) {
    ...
    if (CLZMA::IsCompressed(pCompressedData)) {
      ...
      pOriginalData =
          (byte *)malloc(sizeof(HardwareVerts::FileHeader_t) + actualSize);

      // place the header, then uncompress directly after it
      V_memcpy(pOriginalData, pData, sizeof(HardwareVerts::FileHeader_t));
      int outputLength = CLZMA::Uncompress(
          pCompressedData, pOriginalData + sizeof(HardwareVerts::FileHeader_t));
  ...
  for (meshID = startMesh; meshID < pVhvHdr->m_nMeshes; meshID++) {
    ...
    CMeshBuilder meshBuilder;
    meshBuilder.Begin(
        pStaticPropContext->m_pColorMeshData->m_pMeshInfos[nID].m_pMesh,
        MATERIAL_HETEROGENOUS, numVertexes, 0);
    if (numLightingComponents > 1) {
      pOut = reinterpret_cast<unsigned char *>(
          const_cast<float *>(meshBuilder.Normal()));
    } else {
...
cleanUp:
  if (IsX360()) {
    ...
    // track the color mesh's datacache handle so that we can find it long after
    // the model instance's are gone the static prop filenames are guaranteed
    // uniquely decorated
...
// Called from async i/o thread - must spend minimal cycles in this context
//-----------------------------------------------------------------------------
void CModelRender::StaticPropColorTexelCallback(void *pContext,
                                                const void *pData,
                                                int numReadBytes,
                                                FSAsyncStatus_t asyncStatus) {
  // get our preserved data
  ...
  // This needs to be above the goto or clang complains "goto into protected
  // scope."
  ...
  for (meshID = startMesh; meshID < pVhtHdr->m_nMeshes; meshID++) {
    ...
    // We can't create the real texture here because that's just how the
    // material system works. So instead, squirrel away what we need for later.
    ColorTexelsInfo_t *newCTI = new ColorTexelsInfo_t;
    newCTI->m_nWidth = pMeshData->m_nWidth;
    newCTI->m_nHeight = pMeshData->m_nHeight;
    newCTI->m_nMipmapCount =
        ImageLoader::GetNumMipMapLevels(newCTI->m_nWidth, newCTI->m_nHeight);
    ...
    Q_memcpy(newCTI->m_pTexelData, pVhtHdr->pTexelBase(meshID),
             newCTI->m_nByteCount);

    pStaticPropContext->m_pColorMeshData->m_pMeshInfos[meshID - startMesh]
        .m_pLightmapData = newCTI;
    Assert(
        pStaticPropContext->m_pColorMeshData->m_pMeshInfos[meshID - startMesh]
            .m_pLightmap == NULL);
    anyTextures = true;
  ...
  // This only gets set if we actually have texel data. Otherwise, it remains
  // false.
...
// Called from async i/o thread - must spend minimal cycles in this context
//-----------------------------------------------------------------------------
static void StaticPropColorTexelCallback(const FileAsyncRequest_t &request,
                                         int numReadBytes,
                                         FSAsyncStatus_t asyncStatus) {
  s_ModelRender.StaticPropColorTexelCallback(request.pContext, request.pData,
                                             numReadBytes, asyncStatus);
}
...
// Called from async i/o thread - must spend minimal cycles in this context
//-----------------------------------------------------------------------------
static void QueuedLoaderCallback_PropLighting(void *pContext, void *pContext2,
                                              const void *pData, int nSize,
                                              LoaderError_t loaderError) {
  // translate error
  FSAsyncStatus_t asyncStatus =
      (loaderError == LOADERERROR_NONE ? FSASYNC_OK : FSASYNC_ERR_READING);

  // mimic async i/o completion
  s_ModelRender.StaticPropColorMeshCallback(pContext, pData, nSize,
                                            asyncStatus);
}
...
bool CModelRender::LoadStaticPropColorData(IHandleEntity *pProp,
  ...
  if (pColorMeshData->m_hAsyncControlVertex ||
      pColorMeshData->m_hAsyncControlTexel) {
  ...
  V_strncpy(pContextVertex->m_szFilenameVertex, fileName,
            sizeof(pContextVertex->m_szFilenameVertex));

  if (IsX360() && g_pQueuedLoader->IsMapLoading()) {
    if (!g_pQueuedLoader->ClaimAnonymousJob(fileName,
                                            QueuedLoaderCallback_PropLighting,
                                            (void *)pContextVertex)) {
      // not there as expected
      // as a less optimal fallback during loading, issue as a standard queued
      // loader job
      LoaderJob_t loaderJob;
  ...
  staticPropAsyncContext_t *pContextTexel =
      new staticPropAsyncContext_t(*pContextVertex);

  // queue vertex data for async load
  {
    MEM_ALLOC_CREDIT();
    g_pFileSystem->AsyncRead(fileRequest,
                             &pColorMeshData->m_hAsyncControlVertex);
  }

  Q_snprintf(fileName, sizeof(fileName), "texelslighting_%d.ppl",
             StaticPropMgr()->GetStaticPropIndex(pProp));
  V_strncpy(pContextTexel->m_szFilenameTexel, fileName,
            sizeof(pContextTexel->m_szFilenameTexel));

  // We are already locked, but we will unlock twice--so lock once more for the
  // texel processing.
  ...
  fileRequest.pData = NULL;
  fileRequest.pszFilename =
      fileName; // This doesn't need to happen, but included for clarity.
...
bool CModelRender::UpdateStaticPropColorData(IHandleEntity *pProp,
  ...
  if (!bDebugColor) {
    ...
    vertexFileHeader_t *pVertexHdr = g_pMDLCache->GetVertexData(
        VoidPtrToMDLHandle(pStudioHdr->VirtualModel()));
    if (!pVertexHdr) {
...
CModelRender::CreateInstance(IClientRenderable *pRenderable,
                             LightCacheHandle_t *pCache) {
  Assert(pRenderable);
  ...
  if (pCache) {
    ...
    if (r_decalstaticprops.GetBool() && instance.m_LightCacheHandle)
      instance.m_AmbientLightingState =
          *(LightcacheGetStatic(*pCache, NULL, LIGHTCACHEFLAGS_STATIC));
#else
...
LightCacheHandle_t
CModelRender::GetStaticLighting(ModelInstanceHandle_t handle) {
  if (handle != MODEL_INSTANCE_INVALID) {
    ModelInstance_t &instance = m_ModelInstances[handle];
...
DataCacheHandle_t
CModelRender::GetCachedStaticPropColorData(const char *pName) {
  // valid for 360 only
  Assert(IsX360());
  if (IsPC()) {
...
void CModelRender::AddDecal(ModelInstanceHandle_t handle, Ray_t const &ray,
  ...
  Color cColorTemp;
  AddDecalInternal(handle, ray, decalUp, decalIndex, body, false, cColorTemp,
                   noPokeThru, maxLODToDecal);
}

//-----------------------------------------------------------------------------
void CModelRender::AddColoredDecal(ModelInstanceHandle_t handle,
                                   Ray_t const &ray, const Vector &decalUp,
                                   int decalIndex, int body, Color cColor,
                                   bool noPokeThru, int maxLODToDecal) {
  AddDecalInternal(handle, ray, decalUp, decalIndex, body, true, cColor,
                   noPokeThru, maxLODToDecal);
}

//-----------------------------------------------------------------------------
void CModelRender::GetMaterialOverride(IMaterial **ppOutForcedMaterial,
                                       OverrideType_t *pOutOverrideType) {
  g_pStudioRender->GetMaterialOverride(ppOutForcedMaterial, pOutOverrideType);
}

//-----------------------------------------------------------------------------
void CModelRender::AddDecalInternal(ModelInstanceHandle_t handle,
                                    Ray_t const &ray, const Vector &decalUp,
                                    int decalIndex, int body, bool bUseColor,
                                    Color cColor, bool noPokeThru,
                                    int maxLODToDecal) {
  if (handle == MODEL_INSTANCE_INVALID)
    return;
  ...
  if (bUseColor) {
    ...
    if (found) {
      ...
      pColor->SetVecValue(cColor.r() / 255.0f, cColor.g() / 255.0f,
                          cColor.b() / 255.0f);
    }
  }
...
void CPooledVBAllocator_ColorMesh::Clear(void) {
  if (m_pMesh != NULL) {
    if (m_numAllocations > 0) {
      Warning("ERROR: CPooledVBAllocator_ColorMesh::Clear should not be called "
...
bool CPooledVBAllocator_ColorMesh::CheckIsClear(void) {
  if (m_pMesh) {
    Warning("ERROR: CPooledVBAllocator_ColorMesh's internal mesh (vertex "
...
int CPooledVBAllocator_ColorMesh::Allocate(int numVerts) {
  if (m_pMesh == NULL) {
    Warning("ERROR: CPooledVBAllocator_ColorMesh::Allocate cannot be called "
            "before Init (expect a crash)");
    Assert(m_pMesh);
    return -1;
  }
  ...
  if (m_bStartedDeallocation) {
    Warning("ERROR: CPooledVBAllocator_ColorMesh::Allocate being called after "
            "some (but not all) calls to Deallocate have been called - "
            "invalid! (expect visual artifacts)");
    Assert(!m_bStartedDeallocation);
    return -1;
  }

  if (numVerts > (m_totalVerts - m_numVertsAllocated)) {
    Warning("ERROR: CPooledVBAllocator_ColorMesh::Allocate failing - not "
            "enough space left in the vertex buffer!");
...
void CPooledVBAllocator_ColorMesh::Deallocate(int offset, int numVerts) {
  if (m_pMesh == NULL) {
    Warning("ERROR: CPooledVBAllocator_ColorMesh::Deallocate cannot be called "
            "before Init");
    Assert(m_pMesh != NULL);
    return;
  }
  ...
  if (m_numAllocations == 0) {
    Warning("ERROR: CPooledVBAllocator_ColorMesh::Deallocate called too many "
  ...
  if (m_numAllocations == 0) {
    if (m_numVertsAllocated != 0) {
      Warning("ERROR: CPooledVBAllocator_ColorMesh::Deallocate, after all "
...
//  - Creates Lightmap Textures from data that was squirreled away during ASYNC
//  load. This is necessary because the material system doesn't like us creating
//  things from ASYNC loaders.
//-----------------------------------------------------------------------------
static void CreateLightmapsFromData(CColorMeshData *_colorMeshData) {
  ...
  for (int mesh = 0; mesh < _colorMeshData->m_nMeshes; ++mesh) {
    ...
    meshInfo->m_pLightmap = g_pMaterialSystem->CreateTextureFromBits(
        cti->m_nWidth, cti->m_nHeight, cti->m_nMipmapCount, cti->m_ImageFormat,
        cti->m_nByteCount, cti->m_pTexelData);

    // If this triggers, we need to figure out if it's reasonable to fail. If it
    // is, then we should figure out how to signal back that we shouldn't try to
    // create this again (probably by clearing
