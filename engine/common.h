...
void COM_Log(const char *pszFile, PRINTF_FORMAT_STRING const char *fmt, ...)
    FMTFUNCTION(2, 3); // Log a debug message to specified file ( if pszFile ==
                       // NULL uses c:\\hllog.txt )
void COM_LogString(char const *pchFile, char const *pchString);

const char *
COM_FormatSeconds(int seconds); // returns seconds as hh:mm:ss string
...
unsigned int COM_GetIdealDestinationCompressionBufferSize_LZSS(
...
/// Fetch ideal working buffer size.  You should allocate the buffer you wish to
/// compress into at least this big, in order to get the best performance when
/// using COM_BufferToBufferCompress
inline unsigned int
COM_GetIdealDestinationCompressionBufferSize(unsigned int uncompressedSize) {
  return COM_GetIdealDestinationCompressionBufferSize_LZSS(uncompressedSize);
}

/// Compress the source data into a newly allocated buffer.  Returns the buffer
/// and its size.  Note that the buffer may have been allocated to a larger size
/// than necessary, and the compressed size may be larger than the size of the
/// input!
///
/// If maxCompressedLen is nonzero, then we will fail compression if the
/// compressed data exceeds this size.  Depending on the compressor used, we
/// might be able to terminate early in this case
inline void *COM_CompressBuffer(const void *source, unsigned int sourceLen,
                                unsigned int *compressedLen,
                                unsigned int maxCompressedLen = 0) {
  return COM_CompressBuffer_LZSS(source, sourceLen, compressedLen,
                                 maxCompressedLen);
}

/// Compress data to the specified buffer.  Returns false if compression fails
/// or the data cannot fit into the specified buffer.  If false is returned, the
/// destination buffer and size field are not modified. (Note that this differs
/// from previous behaviour.)
inline bool COM_BufferToBufferCompress(void *dest, unsigned int *destLen,
                                       const void *source,
                                       unsigned int sourceLen) {
...
/// Fetch size of the decompressed data in a buffer that was created using
/// COM_BufferToBufferCompress. Returns -1 if buffer does not appear to be
/// compressed.
