...
static ConVar sv_logbans(
    ...
    "Log server bans in the server logs."); // should sv_banid() calls be logged
                                            // in the server logs?
static ConVar
    sv_logsecret("sv_logsecret", "0", 0,
                 "If set then include this secret when doing UDP logging (will "
                 "use 0x53 as packet type, not usual 0x52)");

static ConVar sv_logfilename_format(
    "sv_logfilename_format", "", FCVAR_ARCHIVE,
    "Log filename format. See strftime for formatting codes.");
static ConVar sv_logfilecompress(
    "sv_logfilecompress", "0", FCVAR_ARCHIVE,
  ...
  if (m_LogAddresses.Count() > 0) {
    ...
    for (int i = 0; i < m_LogAddresses.Count(); i++) {
      if (sv_logsecret.GetInt() != 0)
        NET_OutOfBandPrintf(NS_SERVER, m_LogAddresses.Element(i), "%c%s%s",
                            S2A_LOGSTRING2, sv_logsecret.GetString(), string);
      else
...
void CLog::FireGameEvent(IGameEvent *event) {
  ...
  if (Q_strcmp(name, "server_spawn") == 0) {
    Printf("Started map \"%s\" (CRC \"%s\")\n", sv.GetMapName(),
           MD5_Print(sv.worldmapMD5.bits, MD5_DIGEST_LENGTH));
  }
...
// Given a base filename and an extension, try to find a file that doesn't exist
// which we can use. This is
//  accomplished by appending 000, 001, etc. Set IsGzip to use gzopen instead of
//  filesystem open.
static bool CreateTempFilename(TempFilename_t &info, const char *filenameBase,
                               const char *ext, bool IsGzip) {
  // Check if a logfilename format has been specified - if it has, kick in new
  // behavior.
  ...
  for (int i = 0; i < 1000; i++) {
    ...
    if (!g_pFileSystem->FileExists(info.Filename, "LOGDIR")) {
      // If the path doesn't exist, try opening the file. If that succeeded,
      // return our filehandle and filename.
      if (!IsGzip) {
