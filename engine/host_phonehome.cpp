...
private:
  ...
  void BuildMessage(bf_write &buf, byte msgtype, char const *mapname,
  ...
  void SendSessionMessage(byte msgtype, char const *mapname) {
    ...
    m_pSocket->SendSocketMessage(sa, (const byte *)buf.GetData(),
    ...
    if (m_pSocket->WaitForMessage(PHONE_HOME_TIMEOUT)) {
      ...
      uint bytesReceived = m_pSocket->ReceiveSocketMessage(
          &replyaddress, (byte *)readbuf, sizeof(readbuf));
      if (bytesReceived > 0) {
        // Fixup actual size
