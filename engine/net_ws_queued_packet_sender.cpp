...
ConVar
    net_queued_packet_thread("net_queued_packet_thread", "1", 0,
                             "Use a high priority thread to send queued "
                             "packets out instead of sending them each frame.");
...
void CQueuedPacketSender::QueuePacket(INetChannel *pChan, SOCKET s,
  ...
  if (m_QueuedPackets.Count() < nMaxQueuedPackets) {
    ...
    if (--nWarnings > 0) {
      Warning("CQueuedPacketSender: num queued packets >= nMaxQueuedPackets. "
              "Not queueing anymore.\n");
    }
  }
...
int CQueuedPacketSender::Run() {
  ...
  while (1) {
    ...
    // OK, now send a packet.
    {
      ...
      // We'll pull all packets we should have sent by now and send them out
      // right away
      uint32 msNow = Plat_MSTime();
      ...
      while (m_QueuedPackets.Count() > 0) {
        ...
        if (pPacket->m_unSendTime > msNow) {
          ...
          // Emit ETW events to help with diagnosing network throttling issues
          // as these often have a severe effect on load times in Dota.
        ...
        // If it's a bot, don't do anything. Note: we DO want this code deep
        // here because bots only try to send packets when sv_stressbots is set,
        // in which case we want it to act as closely as a real player as
        // possible.
