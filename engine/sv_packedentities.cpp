...
void SV_EnsureInstanceBaseline(ServerClass *pServerClass, int iEdict,
  ...
  if (pClass->m_InstanceBaselineIndex == INVALID_STRING_INDEX) {
    ...
    // We need this second check in case multiple instances of the same class
    ...
    if (pClass->m_InstanceBaselineIndex == INVALID_STRING_INDEX) {
      ...
      int temp = sv.GetInstanceBaselineTable()->AddString(
          true,
          idString, // Note we're sending a string with the ID number, not the
                    // class name.
          nBytes, pData);

      // Insert a compiler and/or CPU memory barrier to ensure that all
      // side-effects have been published before the index is published.
      // Otherwise the string index may be visible before its initialization has
      // finished. This potential problem is caused by the use of double-checked
      // locking -- the problem is that the code outside of the lock is looking
      // at the variable that is protected by the lock. See this article for
      // details: http://en.wikipedia.org/wiki/Double-checked_locking
...
static inline void SV_PackEntity(int edictIdx, edict_t *edict,
  ...
  tmZoneFiltered(TELEMETRY_LEVEL0, 50, TMZF_NONE, "PackEntities_Normal%s",
                 __FUNCTION__);
...
#endif
    {
      // in HLTV or Replay mode every PackedEntity keeps it's own
      // ChangeFrameList we just copy the ChangeFrameList from prev frame and
      // update it
...
// in Replay mode we ALWAYS have to store position and PVS info, even if entity
// didnt change
void SV_FillReplayData(CFrameSnapshot *pSnapshot, edict_t *edict,
                       int iValidEdict) {
#if !defined(_XBOX)
  if (pSnapshot->m_pReplayEntityData && edict) {
    CReplayEntityData *pReplayData =
        &pSnapshot->m_pReplayEntityData[iValidEdict];
...
void PackEntities_Normal(int clientCount, CGameClient **clients,
                         CFrameSnapshot *snapshot) {
  Assert(snapshot->m_nValidEntities >= 0 &&
         snapshot->m_nValidEntities <= MAX_EDICTS);
  ...
  if (sv_parallel_packentities.GetBool()) {
    ParallelProcess("PackWork_t::Process", workItems.Base(), workItems.Count(),
                    &PackWork_t::Process);
  } else {
