...
#define S2A_EXTRA_DATA_HAS_GAMETAG_DATA                                        \
  0x01                              // Next bytes are the game tag string
...
static char g_MasterServers[][64] = {
    "185.192.97.130:27010", "168.138.92.21:27016", "135.125.188.162:27010"};
...
public:
  ...
  // SeversInfo
  void RequestInternetServerList(const char *gamedir,
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CMaster, IServersInfo,
                                  SERVERLIST_INTERFACE_VERSION, s_MasterServer);
...
void CMaster::RunFrame() {
  ...
  if (m_serverListResponse &&
    ...
    StopRefresh();
    m_serverListResponse->RefreshComplete(
        NServerResponse::nServerFailedToRespond);
    return;
  }
...
void CMaster::ReplyInfo(const netadr_t &adr, uint sequence) {
  ...
  NET_SendPacket(NULL, NS_SERVER, adr, (unsigned char *)buf.Base(),
                 buf.TellPut());
}
...
void CMaster::SendHeartbeat(adrlist_t *p) {
  ...
  NET_SendPacket(NULL, NS_SERVER, p->adr, buf.GetData(),
...
void CMaster::CheckHeartbeat(void) {
  ...
  while (p) {
    ...
    if ((realtime - p->last_heartbeat) <
        HEARTBEAT_SECONDS) // not time to send yet
    {
      p = p->next;
      continue;
    }
    ...
    p->last_heartbeat =
        realtime; // Flag at start so we don't just keep trying for hb's when
    ...
    NET_SendPacket(NULL, NS_SERVER, p->adr, msg.GetData(),
...
void CMaster::AddServer(netadr_t *adr) {
  ...
  if (!n)
    Sys_Error("Error allocating %zd bytes for master address.",
              sizeof(adrlist_t));
...
// Purpose:
//-----------------------------------------------------------------------------
void AddMaster_f(const CCommand &args) { master->AddMaster_f(args); }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void Heartbeat1_f(void) { master->Heartbeat_f(); }

static ConCommand setmaster("addmaster", AddMaster_f);
static ConCommand heartbeat("heartbeat", Heartbeat1_f,
...
// ServersInfo
void CMaster::RequestInternetServerList(const char *gamedir,
                                        IServerListResponse *response) {
  if (m_bNoMasters)
    return;
  ...
  if (response) {
    ...
    m_flRetryRequestTime = m_flStartRequestTime = m_flMasterRequestTime =
        Plat_FloatTime();
  }
  ...
  p = m_pMasterAddresses;
  while (p) {
    NET_SendPacket(NULL, NS_CLIENT, p->adr, msg.GetData(),
                   msg.GetNumBytesWritten());
    p = p->next;
  }
}

void CMaster::RequestLANServerList(const char *gamedir,
                                   IServerListResponse *response) {}
