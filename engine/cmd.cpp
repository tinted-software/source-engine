...
int g_iFilterCommandsByServerCanExecute =
    0; // If this is nonzero, then they want us to only run commands marked with
...
static int CreateExecutionMarker() {
  ...
  if (g_ExecutionMarkers.Count() >= MAX_EXECUTION_MARKERS) {
    // Callers to this function should call Cbuf_HasRoomForExecutionMarkers
    // first.
    Host_Error(
  ...
  do {
    ...
    // if ( CCrypto::GenerateRandomBlock( (uint8 *)&nRandomNumber,
    // sizeof(nRandomNumber) ) )
    // {
...
CON_COMMAND_F(PerfMark,
              "inserts a telemetry marker into the stream. If args are "
              "provided, they will be included.",
              FCVAR_NONE) {
...
bool Cbuf_EscapeCommandArg(const char *pText, char *pOut, unsigned int nOut) {
  // Okay, so, to be honest, all we can do with the super limited syntax is
  // ensure we don't have quotes or control characters, and then wrap the string
  // in quotes. Hence escape *argument*. Anything more advanced than that is
  // just not allowable.
  if (!pText || !*pText)
    return false;

  for (const char *pChar = pText; pChar && *pChar; pChar++) {
    // ASCII control characters (codepoints <= 31) are just not sane to pass to
    // this system. This includes \n and
    // \r.
    if (*pChar <= (char)31)
      return false;
...
bool Cbuf_AddExecutionMarker(ECmdExecutionMarker marker,
                             const char *pszMarkerCode) {
...
bool Cbuf_AddTextWithMarkers(ECmdExecutionMarker markerLeft, const char *text,
                             ECmdExecutionMarker markerRight) {
  ...
  V_sprintf_safe(szMarkerLeft, ";%s %c %d;", CMDSTR_ADD_EXECUTION_MARKER,
                 (char)markerLeft, iMarkerCodeLeft);
  V_sprintf_safe(szMarkerRight, ";%s %c %d;", CMDSTR_ADD_EXECUTION_MARKER,
                 (char)markerRight, iMarkerCodeRight);

  // Due to the behavior of the command buffer, we may be over-estimating the
  // amount of space required.
  int cTextToBeAdded =
      strlen(szMarkerLeft) + strlen(szMarkerRight) + strlen(text) + 3;

  LOCK_COMMAND_BUFFER();
  if (s_CommandBuffer.GetArgumentBufferSize() + cTextToBeAdded + 1 >
      s_CommandBuffer.GetMaxArgumentBufferSize()) {
    // cleanup
  ...
  if (!bSuccess) {
    // If we screwed up the validation, don't allow us to get stuck in a bad
    // state.
...
bool Cbuf_HasRoomForExecutionMarkers(int cExecutionMarkers) {
  return (g_ExecutionMarkers.Count() + cExecutionMarkers) <
...
static void Cbuf_ExecuteCommand(const CCommand &args, cmd_source_t source) {
  // Note: If you remove this, PerfMark needs to do the same logic--so don't do
  // that.
  tmMessage(TELEMETRY_LEVEL0, TMMF_SEVERITY_LOG | TMMF_ICON_NOTE,
            "(source/command) %s",
...
void Cmd_Exec_f(const CCommand &args) {
  ...
  if (IsPC()) {
    if (g_pFileSystem->FileExists(fileName)) {
      ...
      if (!V_stristr(szFile, "autoexec.cfg") &&
          !V_stristr(szFile, "joystick.cfg") &&
...
// Users can alias these commands to remove their functionality because the game
// systems use convars to implement these features The blacklist prevents that
// exploit
      ...
      if (serverGameClients) {
        ...
        if (sv.IsActive()) {
          g_pServerPluginHandler->SetCommandClient(
              cmd_source == src_client ? nClientSlot : -1);
