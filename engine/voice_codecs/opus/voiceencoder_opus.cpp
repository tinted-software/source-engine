...
VoiceEncoder_Opus::~VoiceEncoder_Opus() { TermStates(); }

bool VoiceEncoder_Opus::Init(int quality, int &rawFrameSize,
  ...
  m_Mode =
      opus_custom_mode_create(g_OpusOpts[m_iVersion].iSampleRate,
                              g_OpusOpts[m_iVersion].iRawFrameSize, &iError);
...
void VoiceEncoder_Opus::Release() { delete this; }

void VoiceEncoder_Opus::EncodeFrame(const char *pUncompressedBytes,
                                    char *pCompressed) {
  unsigned char output[1024];

  opus_custom_encode(m_EncoderState, (opus_int16 *)pUncompressedBytes,
                     g_OpusOpts[m_iVersion].iRawFrameSize, output,
                     g_OpusOpts[m_iVersion].iPacketSize);
...
void VoiceEncoder_Opus::DecodeFrame(const char *pCompressed,
                                    char *pDecompressedBytes) {
  unsigned char output[1024];
  char *out = (char *)pCompressed;

  if (!pCompressed) {
    int decoded = opus_custom_decode(
        m_DecoderState, NULL, g_OpusOpts[m_iVersion].iPacketSize,
        (opus_int16 *)pDecompressedBytes, g_OpusOpts[m_iVersion].iRawFrameSize);
    return;
  }
  ...
  int decoded = opus_custom_decode(
      m_DecoderState, output, g_OpusOpts[m_iVersion].iPacketSize,
      (opus_int16 *)pDecompressedBytes, g_OpusOpts[m_iVersion].iRawFrameSize);
}
