...
size_t mp3dec_read_callback(void *buf, size_t size, void *user_data) {
  ...
  int ret_size =
      stream->stream_event->StreamRequestData(buf, size, stream->offset);
...
int mp3dec_seek_callback(uint64_t position, void *user_data) {
  struct AudioStream_s *stream =
      static_cast<AudioStream_s *>((void *)user_data);
...
int CMiniMP3::Decode(void *pBuffer, unsigned int bufferSize) {
  size_t readed =
      mp3dec_ex_read(&mp3d, (mp3d_sample_t *)pBuffer, bufferSize / 2);
  return readed * 2;
}

int CMiniMP3::GetOutputBits() { return 16; }

int CMiniMP3::GetOutputRate() { return mp3d.info.hz; }

int CMiniMP3::GetOutputChannels() { return mp3d.info.channels; }

unsigned int CMiniMP3::GetPosition() { return audio_stream.offset; }
