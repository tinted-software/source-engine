...
public:
  ...
  // This will be the HWND under D3D + Windows (both with and without SDL),
  // SDL_Window* everywhere else.
  void *GetMainDeviceWindow(void);
  // This will be the HWND under Windows, the WindowRef under Mac, and (for now)
  // NULL on Linux
...
private:
  ...
  void
  PlayVideoAndWait(const char *filename,
                   bool bNeedHealthWarning =
                       false); // plays a video file and waits till it's done to
                               // return. Can be interrupted by user.
...
void CGame::DispatchInputEvent(const InputEvent_t &event) {
  ...
  case IE_FingerMotion:
    if (g_ClientDLL)
      g_ClientDLL->IN_TouchEvent(event.m_nType, event.m_nData, event.m_nData2,
                                 event.m_nData3);
  default:
...
void VCR_EnterPausedState() {
  ...
  static GetAsyncKeyStateFn pfn =
      (GetAsyncKeyStateFn)GetProcAddress(hInst, "GetAsyncKeyState");
  if (!pfn)
    return;
...
//-----------------------------------------------------------------------------
static LRESULT WINAPI CallDefaultWindowProc(HWND hWnd, UINT uMsg, WPARAM wParam,
                                            LPARAM lParam) {
  if (unicode)
...
void XBX_HandleInvite(DWORD nUserId) {
  ...
  if (!Q_stricmp(GetCurrentMod(), "tf")) {
    ...
    // NOTE: We may need to actually save off the inviter's XID and search for
    // them later on if we took too long or the
...

{
  ...
  case WM_POWERBROADCAST:
    // Don't go into Sleep mode when running engine, we crash on resume for some
    // reason (as
  ...
  case WM_SYS_SIGNINCHANGED:
    if (XUserGetSigninState(XBX_GetPrimaryUserId()) ==
        eXUserSigninState_NotSignedIn) {
      // X360TBD: User signed out - pause the game?
    }
    EngineVGui()->SystemNotification(lParam ? SYSTEMNOTIFY_USER_SIGNEDIN
  ...
  case WM_SYS_UI:
    ...
    EngineVGui()->SystemNotification(lParam ? SYSTEMNOTIFY_XUIOPENING
...
//-----------------------------------------------------------------------------
static LRESULT WINAPI HLEngineWindowProc(HWND hWnd, UINT uMsg, WPARAM wParam,
                                         LPARAM lParam) {
  ...
  if (!WSAStartup(0x0101, &wsaData)) {
    ...
    if (tmpSocket != INVALID_SOCKET) {
      ...
      if (hInfo) {
        ...
        if (bind(tmpSocket, (sockaddr *)&myIpAddress, sizeof(myIpAddress)) !=
            -1) {
          if (sendto(tmpSocket, pszHostName, 1, 0, (sockaddr *)&myIpAddress,
...
bool CGame::CreateGameWindow(void) {
  ...
  // checked in, but sometimes is by accident), so add this to the Window title
  // too.
  ...
  if (IsPC()) {
    ...
    if (g_pFileSystem->GetLocalPath("resource/game.ico", localPath,
      ...
      wc.hIcon = (HICON)::LoadImage(NULL, localPath, IMAGE_ICON, 0, 0,
                                    LR_LOADFROMFILE | LR_DEFAULTSIZE);
    } else {
      wc.hIcon = (HICON)LoadIcon(GetModuleHandle(0),
  ...
  wchar_t uc[512];
  if (IsPC()) {
    ::MultiByteToWideChar(CP_UTF8, 0, windowName, -1, uc,
                          sizeof(uc) / sizeof(wchar_t));
  }
...
#if !defined(_X360)
  HWND hwnd = unicode->CreateWindowExW(exFlags, CLASSNAME, uc, style, 0, 0, w,
                                       h, NULL, NULL, m_hInstance, NULL);
  // NOTE: On some cards, CreateWindowExW slams the FPU control word
  SetupFPUControlWord();
#else
  HWND hwnd = CreateWindowEx(exFlags, CLASSNAME, windowName, style, 0, 0, w, h,
  ...
  char localPath[MAX_PATH];
  if (g_pFileSystem->GetLocalPath("resource/game-icon.bmp", localPath,
                                  sizeof(localPath))) {
    g_pFileSystem->GetLocalCopy(localPath);
...
#ifndef DEDICATED
  vgui::surface()->OnScreenSizeChanged(videomode->GetModeStereoWidth(),
                                       videomode->GetModeStereoHeight());
#endif
...
void CGame::PlayStartupVideos(void) {
  ...
  bool bRecap = CommandLine()->CheckParm(
      "-recapvid"); // FIXME: This is a temp addition until the movie playback
                    // is centralized -- jdw
  ...
  // There is no access to steam at this point so we are checking for the
  // presence of an empty file that will only exist in the chinese depot
  ...
  while (true) {
    ...
    localPath[0] = 0; // just to make sure we don't play the same avi file twice
                      // in the case that one movie is there but another isn't.
  }
...
void CGame::PlayVideoAndWait(const char *filename, bool bNeedHealthWarning) {
  ...
  // Chinese health messages appears for 11 seconds, so we force a minimum delay
  // time for those
  ...
  VideoResult_t status = g_pVideo->PlayVideoFileFullScreen(
      filename, "GAME", GetMainWindowPlatformSpecificHandle(), m_width,
      m_height, m_iDesktopWidth, m_iDesktopHeight, videomode->IsWindowedMode(),
      forcedMinTime,
      VideoPlaybackFlags::DEFAULT_FULLSCREEN_OPTIONS |
          VideoPlaybackFlags::FILL_WINDOW);
  ...
  // just send a message to the spew
#ifdef _DEBUG
...
bool CGame::LoadUnicode(void) {
  ...
  if (!unicode) {
    Error("Unable to load interface '%s' from unicode.dll",
          VENGINE_UNICODEINTERFACE_VERSION);
    return false;
  }
#endif
...
void CGame::SetMainWindow(SDL_Window *window) {
  ...
  // We also can't do this in GetMainDeviceWindow and just use that, because for
  // some reason people use GetMainWindowAddress and store that pointer to our
  // member.
  SDL_SysWMinfo pInfo;
  SDL_VERSION(&pInfo.version);
  if (!SDL_GetWindowWMInfo((SDL_Window *)g_pLauncherMgr->GetWindowRef(),
                           &pInfo)) {
    Error("Fatal Error: Unable to get window info from SDL.");
    return;
  }
