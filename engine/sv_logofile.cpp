...
public:
  ...
  virtual void OnFileReceived(INetChannel *pChan, const void *pUserData,
    ...
    if (!LogoFile_IsValidVTFFile(pFileData, fileLength)) {
      Warning("CServerFileTransferMgr::OnFileReceived: received an invalid "
              "logo file from a client.\n");
      return;
    }

    if (userDataLength < sizeof(CRC32_t)) {
      Warning("CServerFileTransferMgr::OnFileReceived: invalid userDataLength "
              "(%d).\n",
              userDataLength);
    }
  ...
  if (bValid) {
    ...
    if (!SV_LogoFile_HasLogoFile(crcValue) &&
      ...
      if (!pSenderClient->SendNetMsg(fileRequest, true)) {
        Host_Error("SV_LogoFile_OnConnect: Reliable broadcast message would "
                   "overflow client");
        return;
      }
    }
