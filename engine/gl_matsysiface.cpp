...
static bool LightmapLess(const SurfaceHandle_t &surfID1,
  ...
  // NOTE: Don't care about bumpmap increasing area here because it is a linear
  // factor (all surfs with the same material have the same bumpmapping cost)
#if 1
  int area1 =
...
msurface2_t *
CMSurfaceSortList::GetSurfaceAtHead(const surfacesortgroup_t &group) const {
...
void CMSurfaceSortList::GetSurfaceListForGroup(
    ...
    MSL_FOREACH_SURFACE_IN_GROUP_BEGIN(*this, group,
                                       surfID){list.AddToTail(surfID);
}
