...
//-----------------------------------------------------------------------------
template <class T> class CRangeValidatedArray {
public:
...
template <class T> inline T &CRangeValidatedArray<T>::operator[](int i) {
...
template <class T> inline void CRangeValidatedArray<T>::Detach() {
  m_pArray = NULL;
...
template <class T> inline T *CRangeValidatedArray<T>::Base() {
  return m_pArray;
}
...
template <class T> class CDiscardableArray {
public:
  CDiscardableArray() {
    m_nCount = 0;
    m_nOffset = -1;
  ...
  void Discard() {
    // TODO(johns): Neutered -- Tear this class out. This can no longer be
    // discarded with transparently compressed
    //              BSPs. This is on the range of 500k of memory, and is
    //              probably overly complex for the savings in the current era.
