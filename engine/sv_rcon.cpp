...
ConVar sv_rcon_maxfailures("sv_rcon_maxfailures", "10", 0,
                           ...
                           true, 1, true, 20);
ConVar
    sv_rcon_minfailures("sv_rcon_minfailures", "5", 0,
...
ConVar sv_rcon_whitelist_address("sv_rcon_whitelist_address", "", 0,
                                 "When set, rcon failed authentications will "
                                 "never ban this address, e.g. '127.0.0.1'");

ConVar sv_rcon_maxpacketsize(
    "sv_rcon_maxpacketsize", "1024", 0,
    "The maximum number of bytes to allow in a command packet", true, 0, false,
    0);
ConVar sv_rcon_maxpacketbans("sv_rcon_maxpacketbans", "1", 0,
                             "Ban IPs for sending RCON packets exceeding the "
                             "value specified in sv_rcon_maxpacketsize",
                             true, 0, true, 1);
...
void CRConServer::RunFrame() {
  ...
  for (int i = nCount - 1; i >= 0; --i) {
      ...
      if (bSent) // all this packet was sent, remove it
      {
        pData->m_OutstandingSends.Remove(
            pData->m_OutstandingSends
                .Head()); // delete this entry no matter what,
                          // SendRCONResponse() will re-queue if needed
    ...
    if (readLen > sizeof(int)) // we have a command to process
    {
      ...
      char *recvBuf =
          (char *)_alloca(min(1024ul, readLen)); // a buffer used for recv()
      unsigned int len = 0;
      while (len < readLen) {
      ...
      if (sv_rcon_maxpacketsize.GetInt() > 0 &&
          size > sv_rcon_maxpacketsize.GetInt()) {
      ...
      // Check and see if socket was closed as a result of processing - this can
      // happen if the user has entered too many passwords
...
void CRConServer::FinishRedirect(const char *msg, const netadr_t &adr) {
  ...
  for (int i = nCount - 1; i >= 0; --i) {
    ...
    //Q_strlen(msg)) ); // can't use DevMsg(), we are potentially inside the
    //RedirectFlush() function 		printf("RCON: String is %i long, packet size
    //%i\n", Q_strlen(msg), size );
