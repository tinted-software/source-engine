...
//
// Purpose: Windows Only, does a check against all other processes to see how
// many other instances of this process is running concurrently
//
//=============================================================================//
...
typedef NTSTATUS(__stdcall *NtQuerySystemInformation1)(
    IN ULONG SysInfoClass, IN OUT PVOID SystemInformation,
    ...
    if (Process32First(hSnapshot, &pe32)) {
      do {
        ...
        if (V_strcmp(thisProcessNameShort, otherProcessNameShort) == 0) {
          //					DevMsg(
          //"CreateToolhelp32Snapshot - Process Name [ %s ] - OtherName [ %s ]
...
int CheckOtherInstancesWithEnumProcess(const char *thisProcessNameShort) {
  ...
  //  Query all of the handles in the system.  We have to do this in a loop,
  //  since we do not know how large of a buffer we need.
  nBufferSize = 0;
  ...
  while (TRUE) {
    ...
    status = NtQuerySystemInformation(SystemHandleInformation, pHandleInfo,
                                      nBufferSize, NULL);
  ...
  //  Note that this code assumes that handles will be grouped by process, which
  //  is what Windows does.  If that assumption ever turns out to be false, this
  //  code will have to be altered to keep a list of process IDs already seen.
  //
  ...
  GetProcessImageFileNameFn fn =
      (GetProcessImageFileNameFn)GetProcAddress(hInst,
#ifdef UNICODE
  ...
  for (i = 0; i < pHandleInfo->HandleCount; i++) {
    if (pHandleInfo->HandleInfoArray[i].ProcessId != nLastProcess) {
      ...
      if (process != NULL) {
        //  Query the name of the executable for the process we opened.  If the
        //  query fails, we ignore this process.
        nLength = MAX_PATH;
        ...
        if (bStatus) {
          //
          //  We have the process name.  See if it is the same name as our
          //  process.
          //
          ...
          if (V_strcmp(thisProcessNameShort, otherProcessNameShort) == 0) {
            //  We found an instance of this executable.
            //						DevMsg( "EnumProcess -
            //Process Name [ %s ] - OtherName [ %s ] \n", thisProcessNameShort,
            //otherProcessName );
            iProcessCount++;
          }
        }
...
int CheckOtherInstancesRunning(void) {
  ...
  int iSnapShotCount =
      CheckOtherInstancesRunningWithSnapShot(thisProcessNameShort);
  if (iSnapShotCount > 1) {
    return iSnapShotCount;
  }
