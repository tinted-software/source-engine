...
struct EntityInfo_t {
  ...
  intp m_iLeafList[NUM_TREES]; // Index into the leaf pool - leaf list for
                               // entity (m_aLeafList).
};
...
public:
  ...
  void InsertIntoTree(SpatialPartitionHandle_t hPartition, Voxel_t voxelMin,
                      Voxel_t voxelMax);
...
public:
  ...
  void InsertIntoTree(SpatialPartitionHandle_t hPartition, const Vector &mins,
                      const Vector &maxs, bool bReinsert);
  ...
  bool EnumerateRayStartVoxels(SpatialPartitionListMask_t listMask,
                               IPartitionEnumerator *pIterator,
...
public:
  ...
  virtual SpatialPartitionHandle_t
  CreateHandle(IHandleEntity *pHandleEntity,
  ...
  virtual void InstallQueryCallback_V1(IPartitionQueryCallback *pCallback) {
    Error("Use InstallQueryCallback instead of InstallQueryCallback_V1\n");
  }
...
// Gets entity info (for enumerations).
inline EntityInfo_t &
CSpatialPartition::EntityInfo(SpatialPartitionHandle_t hPartition) {
  return m_aHandles[hPartition];
}

inline EntityInfo_t &
CVoxelTree::EntityInfo(SpatialPartitionHandle_t hPartition) {
...
inline CVoxelTree *
CSpatialPartition::VoxelTree(SpatialPartitionListMask_t listMask) {
...
inline CVoxelTree *
CSpatialPartition::VoxelTreeForHandle(SpatialPartitionHandle_t handle) {
...
// SIMD Versions - need more code changes to fully support this but there are
// enough benefits to use it on some of the code
inline void CVoxelHash::VoxelIndexFromPoint(const fltx4 &fl4WorldPoint,
                                            int pPoint[3]) {
...
inline Voxel_t CVoxelHash::VoxelIndexFromPoint(const Vector &vecWorldPoint) {
  ...
  cnvX.m_flConverted =
      FloatConvertToIntegerFormat(vecWorldPoint.x - m_vecVoxelOrigin.x);
  cnvY.m_flConverted =
      FloatConvertToIntegerFormat(vecWorldPoint.y - m_vecVoxelOrigin.y);
  cnvZ.m_flConverted =
      FloatConvertToIntegerFormat(vecWorldPoint.z - m_vecVoxelOrigin.z);
  // now we load that value back into an integer register.  This will LHS if
  // there aren't enough cycles between the stores and the loads but this will
  // allow the compiler to reorder the operations and when the conversions are
  // implicit they don't get reordered (load instruction immediately follows the
  // store)
...
inline void CVoxelHash::VoxelIndexFromPoint(const Vector &vecWorldPoint,
                                            int pPoint[3]) {
  pPoint[0] =
      static_cast<int>(vecWorldPoint.x - m_vecVoxelOrigin.x) >> m_nLevelShift;
  pPoint[1] =
      static_cast<int>(vecWorldPoint.y - m_vecVoxelOrigin.y) >> m_nLevelShift;
  pPoint[2] =
      static_cast<int>(vecWorldPoint.z - m_vecVoxelOrigin.z) >> m_nLevelShift;
}
...
void CVoxelHash::InsertIntoTree(SpatialPartitionHandle_t hPartition,
                                Voxel_t voxelMin, Voxel_t voxelMax) {
...
void CVoxelHash::UpdateListMask(SpatialPartitionHandle_t hPartition) {
  ...
  if (vmin.uiVoxel == vmax.uiVoxel) {
    ...
    if (hHash != m_aVoxelHash.InvalidHandle()) {
      for (intp i = m_aVoxelHash.Element(hHash);
           i != m_aEntityList.InvalidIndex(); i = m_aEntityList.Next(i)) {
...
CPartitionVisitor( pPartition )
        ...
        bool Intersects( const float *pMins, const float *pMaxs ) const
        {
                ...
                return IsPointInBox( m_f4Point, LoadUnaligned3SIMD( pMins ),
LoadUnaligned3SIMD(pMaxs) );
        }
...
public:
  ...
  bool Intersects(const float *pMins, const float *pMaxs) const {
    ...
    return IsBoxIntersectingRay(f4Mins, f4Maxs, m_f4Start, m_f4Delta,
                                m_f4InvDelta);
  }
...
public:
  ...
  bool Intersects(const float *pMins, const float *pMaxs) const {
    ...
    return IsBoxIntersectingRay(SubSIMD(f4Mins, m_f4Extents),
                                AddSIMD(f4Maxs, m_f4Extents), m_f4Start,
                                m_f4Delta, m_f4InvDelta);
  }
...
bool CVoxelHash::EnumerateElementsInVoxel(Voxel_t voxel, const T &intersectTest,
  ...
  for (intp i = m_aVoxelHash.Element(hHash); i != m_aEntityList.InvalidIndex();
...
bool CVoxelHash::EnumerateElementsInSingleVoxel(
  ...
  if (hHash != m_aVoxelHash.InvalidHandle()) {
    ...
    while (iEntityList != m_aEntityList.InvalidIndex()) {
      ...
      SpatialPartitionListMask_t nListMask =
          m_aEntityList[iEntityList].m_nListMask;
      ...
      if (!intersectTest.Intersects(hInfo.m_vecMin.Base(),
                                    hInfo.m_vecMax.Base()))
        continue;
...
void CVoxelTree::InsertIntoTree(SpatialPartitionHandle_t hPartition,
  ...
  for (nLevel = 0; nLevel < m_nLevelCount - 1; ++nLevel) {
    ...
    if ((flVoxelSize > vecSize.x) && (flVoxelSize > vecSize.y) &&
        (flVoxelSize > vecSize.z))
      break;
  }
  ...
  if (bReinsert) {
    ...
    // if the entity spans the same bounding box of voxels no remove/insert is
    // necessary
    if (info.m_voxelMin.uiVoxel == voxelMin.uiVoxel &&
        info.m_voxelMax.uiVoxel == voxelMax.uiVoxel) {
      bDoInsert = false;
    } else {
...
void CVoxelTree::EnumerateElementsInBox(SpatialPartitionListMask_t listMask,
                                        ...
                                        const Vector &vecMaxs, bool coarseTest,
                                        IPartitionEnumerator *pIterator) {
  VPROF("BoxTest/SphereTest");
...
bool CVoxelTree::EnumerateRayStartVoxels(SpatialPartitionListMask_t listMask,
                                         IPartitionEnumerator *pIterator,
                                         CIntersectSweptBox &intersectSweptBox,
                                         int voxelBounds[4][2][3]) {
  // Iterate over all voxels that intersect the box around the starting ray
...
bool CVoxelTree::EnumerateElementsAlongRay_ExtrudedRay(
  ...
  if (!EnumerateRayStartVoxels(listMask, pIterator, intersectSweptBox,
                               voxelBounds))
    return false;
  ...
  float tMax[3] = {
      0.f, 0.f,
      0.f}; // amount of change in t along ray until we hit the next new voxel
  float tMin[3] = {
      0.f, 0.f,
      0.f}; // amount of change in t along ray until we leave the last voxel
...
ConVar think_trace_limit(
    "think_trace_limit", "0", FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY,
    "Break into the debugger if this many or more traces are performed in a "
    "single think function. Negative numbers mean that the same think function "
    "may be broken into many times (once per [x] may traces), positive numbers "
    "mean each think will break only once.");
...
#ifdef THINK_TRACE_COUNTER_COMPILED
  if (DEBUG_THINK_TRACE_COUNTER_ALLOWED() && think_trace_limit.GetInt() != 0 &&
      g_DebugTracesRemainingBeforeTrap > 0) {
...
CSpatialPartition::CreateHandle(IHandleEntity *pHandleEntity) {
  m_HandlesMutex.Lock();
...
void CSpatialPartition::UpdateListMask(SpatialPartitionHandle_t hPartition,
                                       uint16 nListMask) {
...
//-----------------------------------------------------------------------------
SpatialTempHandle_t
CSpatialPartition::HideElement(SpatialPartitionHandle_t handle) {
  Assert(m_aHandles.IsValidIndex(handle));
  m_HandlesMutex.Lock();
  m_aHandles[handle].m_flags |= ENTITY_HIDDEN;
  m_HandlesMutex.Unlock();
