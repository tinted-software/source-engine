  ...
  virtual bool AreConVarsLinkable(const ConVar *child, const ConVar *parent) {
    ...
    if (repchild && repparent) {
      ...
      if (child->IsFlagSet(FCVAR_GAMEDLL) &&
        ...
        ConMsg("For FCVAR_REPLICATED, ConVar must be defined in client and "
               "game .dlls (%s)\n",
               child->GetName());
        return false;
      }
    ...
    if (repchild || repparent) {
      ConMsg("Both ConVars must be marked FCVAR_REPLICATED for linkage to work "
             "(%s)\n",
             child->GetName());
      return false;
    }
...
bool CCvarUtilities::IsCommand(const CCommand &args) {
  ...
  if (v->IsFlagSet(FCVAR_NOT_CONNECTED)) {
    ...
    if (cl.IsConnected()) {
      ...
      if (v->IsFlagSet(FCVAR_USERINFO) && g_ClientDLL &&
          g_ClientDLL->IsConnectedUserInfoChangeAllowed(v)) {
  ...
  if (v->IsFlagSet(FCVAR_CHEAT)) {
    if (!Host_IsSinglePlayerGame() && !CanCheat()
      ...
      ConMsg("Can't use cheat cvar %s in multiplayer, unless the server has "
             "sv_cheats set to 1.\n",
             v->GetName());
      return true;
    }
  ...
  if (v->IsFlagSet(FCVAR_REPLICATED)
    ...
    if (!sv.IsActive() && !sv.IsLoading() && (cmd_source == src_command) &&
        cl.IsConnected()) {
      ConMsg("Can't change replicated ConVar %s from console of client, only "
...
void CCvarUtilities::WriteVariables(CUtlBuffer &buff, bool bAllVars) {
  ...
  for (var = g_pCVar->GetCommands(); var; var = var->GetNext()) {
    ...
    if (archive) {
      ...
      if (bAllVars ||
          Q_strcmp(pConvar->GetString(), pConvar->GetDefault()) != 0) {
        buff.Printf("%s \"%s\"\n", var->GetName(),
