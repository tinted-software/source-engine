...
#if !defined(NO_STEAM)
  SteamAPI_InitSafe(); // ignore failure, that will fall out later when they
                       // don't get a valid logon cookie
...
//-----------------------------------------------------------------------------
void CSteam3Client::GetAuthSessionTicket(void *pTicket, int cbMaxTicket,
                                         uint32 *pcbTicket, uint32 unIP,
                                         uint16 usPort, uint64 unGSSteamID,
                                         bool bSecure) {
#ifdef NO_STEAM
  m_bGSSecure = bSecure;
#else
  ...
  *pcbTicket = 0;
  ...
  if (!steamID.IsValid()) {
    Warning("Our steam ID %s is not valid.  Steam must be running and you must "
            "be logged in\n",
            steamID.Render());
    return;
  }
  ...
  if (m_hAuthTicket == k_HAuthTicketInvalid || m_unIP != unIP ||
      m_usPort != usPort || m_bGSSecure != bSecure ||
    ...
    *(uint64 *)m_arbTicketData = SteamUser()->GetSteamID().ConvertToUint64();
    ...
    if (m_hAuthTicket == k_HAuthTicketInvalid || m_nTicketSize <= 0) {
      ...
      m_nTicketSize = 0;
      Warning(
          "ISteamUser::GetAuthSessionTicket failed to return a valid ticket\n");
    } else {
...
// Tell the steam backend about the server we are playing on - so that it can
// broadcast this to our friends and they can join. This may be something that
// you should NOT do if you are on a listen server - or you know that the game
// is not joinable for some reason.
#ifdef _DEBUG
  Msg("Sending AdvertiseGame %s %s (%s)\n", steamIDGS.Render(),
      netadr_t(unIP, usPort).ToString(), m_bGSSecure ? "secure" : "insecure");
#endif
