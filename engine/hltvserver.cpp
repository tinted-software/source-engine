...
bool CHLTVServer::ProcessConnectionlessPacket(netpacket_t *packet) {
  bf_read msg =
      packet->message; // We're copying the message, so we don't need to seek
                       // back when passing packet to the base class.
  ...
  while (true) {
    ...
    case dem_stringtables: {
      ...
      if (dataLen > DEMO_FILE_MAX_STRINGTABLE_SIZE) {
        Warning(
            "ReadCompleteDemoFile failed to read string tables. Trying to read "
...
IClient *CHLTVServer::ConnectClient(netadr_t &adr, int protocol, int challenge,
  ...
  IClient *client = (CHLTVClient *)CBaseServer::ConnectClient(
      adr, protocol, challenge, clientChallenge, authProtocol, name, password,
      hashedCDkey, cdKeyLen);
...
void CHLTVServer::ReplyInfo(const netadr_t &adr) {
  ...
  buf.PutUnsignedChar(
      GetProtocolVersion()); // Hardcoded protocol version number
  ...
  // NOTE: This key's meaning is changed in the new version. Since we send
  // gameport and specport, it knows whether we're running SourceTV or not. Then
  // it only needs to know if we're a dedicated or listen server.
  if (IsDedicated())
  ...
  NET_SendPacket(NULL, m_Socket, adr, (unsigned char *)buf.Base(),
                 buf.TellPut());
}
#endif // #ifndef NO_STEAM
