...
private:
  ...
  int m_HostNameLabelRightSidePadding; // Grow the whole panel to make sure
                                       // there's this much padding on the right
                                       // of the hostname label.
...
void CAskConnectPanel::SetHostName(const char *pHostName) {
  ...
  g_pVGuiLocalize->ConstructString_safe(
      wcMessage, g_pVGuiLocalize->Find("#Valve_ServerOfferingToConnect"), 0);
...
void CAskConnectPanel::StartSlideAnimation(float flDuration) {
  ...
  if (pKeyName) {
    ...
    g_pVGuiLocalize->ConvertANSIToUnicode(pKeyName, wcKeyName,
                                          sizeof(wcKeyName));
    g_pVGuiLocalize->ConstructString_safe(
        wcMessage, g_pVGuiLocalize->Find("#Valve_PressKeyToAccept"), 1,
        wcKeyName);
...
void ShowAskConnectPanel(const char *pHostName, float flDuration) {
  ...
  // Hostname is not allowed to contain semicolon, whitespace, or control
  // characters
