...
// if a new decal covers more than this many old decals, retire until this count
// remains
...
class ALIGN16 CDecalVertCache {
  enum decalindex_ordinal {
    DECAL_INDEX =
        0, // set this and use this to free the whole decal's list on compact
...
private:
  ...
  inline void SetIndex(decalcache_t *pBlock, decalindex_ordinal index,
                       int value) {
...
void R_DecalShoot(int textureIndex, int entity, const model_t *model,
                  ...
                  const color32 &rgbaColor, const Vector *pNormal);
...
void CDecalVertCache::StoreVertsInCache(decal_t *pDecal, CDecalVert *pList) {
  ...
  while (blockCount) {
    ...
    // don't memcpy here it overwrites the indices we're storing in the
    // m_decalIndex data
    for (int i = 0; i < 4; i++) {
...
void CDecalVertCache::FreeCachedVerts(decal_t *pDecal) {
  ...
  for (int cacheHandle = pDecal->cacheHandle;
       cacheHandle != INVALID_CACHE_ENTRY; cacheHandle = nextIndex) {
    decalcache_t *pCache = m_cache + cacheHandle;
...
CDecalVert *CDecalVertCache::GetCachedVerts(decal_t *pDecal) {
  ...
  if (cacheHandle == INVALID_CACHE_ENTRY)
    return NULL;
  decalcache_t *pCache = &m_cache[cacheHandle];
  for (int i = cacheHandle; i != INVALID_CACHE_ENTRY;
       i = GetIndex(&m_cache[i], NEXT_VERT_BLOCK_INDEX)) {
    SetIndex(pCache, FRAME_COUNT_INDEX, r_framecount);
    Assert(GetIndex(pCache, DECAL_INDEX) == pDecal->m_iDecalPool);
  }
...
void CDecalVertCache::FindFreeBlocks(int blockCount) {
  ...
  for (; m_freeTestIndex != lastTest;
       m_freeTestIndex = (m_freeTestIndex + 1) & (DECALCACHE_ENTRY_COUNT - 1)) {
...
void Shader_DecalDrawPoly(CDecalVert *v, IMaterial *pMaterial,
  ...
  byte color[4] = {pdecal->color.r, pdecal->color.g, pdecal->color.b,
                   pdecal->color.a};
  if (flFade != 1.0f) {
...
struct decalcontext_t {
  ...
  void InitSurface(SurfaceHandle_t surfID) {
    ...
    sOffset = pTexInfo->lightmapVecsLuxelsPerWorldUnits[0][3] -
              pSurfacelighting->m_LightmapMins[0] +
              pSurfacelighting->m_OffsetIntoLightmapPage[0] + 0.5f;
    tOffset = pTexInfo->lightmapVecsLuxelsPerWorldUnits[1][3] -
              pSurfacelighting->m_LightmapMins[1] +
...
// surface psurf
static void R_DecalVertsLight(CDecalVert *v, const decalcontext_t &context,
                              SurfaceHandle_t surfID, int vertCount) {
...
static decal_t *R_DecalFindOverlappingDecals(decalinfo_t *decalinfo,
  ...
  while (pDecal) {
    ...
    if (!(pDecal->flags & FDECAL_PERMANENT) &&
      ...
      // if the decal is less than half the width of the one we're applying,
      // don't test for overlap test for complete coverage
      float projectWidthTestedDecal =
          pDecal->material->GetMappingWidth() / pDecal->scale;
      ...
      if (sizex >= 0 && sizey >= 0) {
        ...
        if (projectWidthTestedDecal < minProjectedWidth) {
          ...
          if (flArea > areaThreshold) {
            // once you pass the threshold, scale the area by the decal size to
            // select the largest decal above the threshold
            float flAreaScaled = flArea * projectWidthTestedDecal;
            count++;
            if (!plast || flAreaScaled > lastArea) {
              plast = pDecal;
              lastArea = flAreaScaled;
              // go ahead and remove even if you're not at the max overlap count
              // yet because this is a very similar decal
...
CDecalVert *R_DecalSetupVerts(decalcontext_t &context, decal_t *pDecal,
  ...
  if (pDecal->flags & FDECAL_DISTANCESCALE) {
    if (!(pDecal->flags & FDECAL_PLAYERSPRAY)) {
      ...
      if (pDecal->entityIndex == 0) {
        ...
        Vector3DMultiplyPosition(g_BrushToWorldMatrix, pDecal->position,
                                 worldSpaceCenter);
...
void DecalUpdateAndDrawSingle(decalcontext_t &context, SurfaceHandle_t surfID,
                              decal_t *pDecal) {
  if (!pDecal->material)
    return;
  ...
  if (SurfaceHasDispInfo(surfID)) {
    ...
    CDecalVert *v =
        R_DecalSetupVerts(context, pDecal, surfID, pDecal->material);

    if (v)
      Shader_DecalDrawPoly(v, pDecal->material, surfID,
                           pDecal->clippedVertCount, pDecal, 1.0f);
  }
...
void R_DrawDecalsAllImmediate_GatherDecals(IMatRenderContext *pRenderContext,
  ...
  for (int iSortTree = 0; iSortTree < nSortTreeCount; ++iSortTree) {
    ...
    for (int iBucket = 0; iBucket < nBucketCount; ++iBucket) {
      ...
      intp iHead = g_aDecalSortTrees[iSortTree]
                       .m_aDecalSortBuckets[iGroup][iTreeType]
                       .Element(iBucket)
                       .m_iHead;
...
void R_DrawDecalsAllImmediate_Gathered(IMatRenderContext *pRenderContext,
  ...
  for (int i = 0; i != iDecalCount; ++i) {
    ...
    CDecalVert *pVerts =
        R_DecalSetupVerts(context, pDecal, pDecal->surfID, pDecal->material);
    if (!pVerts)
      continue;
    ...
    for (int iVert = 0; iVert < nCount; ++iVert, ++pVerts) {
      ...
      meshBuilder
          .AdvanceVertexF<VTX_HAVEPOS | VTX_HAVENORMAL | VTX_HAVECOLOR, 3>();
    }
  ...
  for (int iSortTree = 0; iSortTree < nSortTreeCount; ++iSortTree) {
    ...
    for (int iBucket = 0; iBucket < nBucketCount; ++iBucket) {
      ...
      while (iElement != g_aDecalSortPool.InvalidIndex()) {
        ...
        CDecalVert *pVerts = R_DecalSetupVerts(context, pDecal, pDecal->surfID,
                                               pDecal->material);
        if (!pVerts)
          continue;
        ...
        for (int iVert = 0; iVert < nCount; ++iVert, ++pVerts) {
          ...
          meshBuilder.AdvanceVertexF<
              VTX_HAVEPOS | VTX_HAVENORMAL | VTX_HAVECOLOR, 3>();
        }
...
inline void R_DrawDecalMeshList(DecalMeshList_t &meshList) {
  ...
  for (int iBatch = 0; iBatch < nBatchCount; ++iBatch) {
    if (g_pMaterialSystemConfig->nFullbright == 1) {
      ...
      pRenderContext->BindLightmapPage(
          meshList.m_aBatches[iBatch].m_iLightmapPage);
    }

    pRenderContext->Bind(meshList.m_aBatches[iBatch].m_pMaterial,
                         meshList.m_aBatches[iBatch].m_pProxy);
    meshList.m_pMesh->Draw(meshList.m_aBatches[iBatch].m_iStartIndex,
                           meshList.m_aBatches[iBatch].m_nIndexCount);
  }
}
...
void R_DecalsGetMaxMesh(IMatRenderContext *pRenderContext,
                        int &nDecalSortMaxVerts, int &nDecalSortMaxIndices) {
  ...
  nDecalSortMaxVerts =
      MIN(nDecalSortMaxVerts,
          8192); // just a guess, you should be able to do 8K dynamic verts in
  ...
  for (int i = 0; i != iDecalCount; ++i) {
    ...
    if (bBatchInit) {
      ...
      if (meshList.m_aBatches.Size() + 1 > meshList.m_aBatches.NumAllocated()) {
        Warning("R_DrawDecalsAll: overflowing m_aBatches. Reduce %d decals in "
                "the scene.\n",
      ...
      while (iElement != g_aDecalSortPool.InvalidIndex()) {
        ...
        if (pDecal->surfID != lastSurf) {
          ...
          if (vertexFormat & (VERTEX_NORMAL | VERTEX_TANGENT_SPACE)) {
            ...
            if (vertexFormat & VERTEX_TANGENT_SPACE) {
              Vector tVect;
              bool bNegate = TangentSpaceSurfaceSetup(pDecal->surfID, tVect);
              TangentSpaceComputeBasis(vecTangentS, vecTangentT, vecNormal,
                                       tVect, bNegate);
            }
          }
        }
        ...
        if (bBatchInit) {
          ...
          if (meshList.m_aBatches.Size() + 1 >
            ...
            Warning("R_DrawDecalsAll: overflowing m_aBatches. Reduce %d decals "
                    "in the scene.\n",
                    nDecalSortMaxVerts * meshList.m_aBatches.NumAllocated());
            meshBuilder.End();
            R_DrawDecalMeshList(meshList);
        ...
        if (pDecal->flags & FDECAL_DYNAMIC) {
          ...
          // Negative fadeDuration value means to fade in
          if (pDecal->fadeDuration < 0) {
            flFadeValue = -(localClientTime - pDecal->fadeStartTime) /
                          pDecal->fadeDuration;
          } else {
            flFadeValue = 1.0 - (localClientTime - pDecal->fadeStartTime) /
                                    pDecal->fadeDuration;
          }
...
void DecalSurfaceDraw_QueueHelper(bool bBatched, int renderGroup,
                                  Vector vModelOrg, int nCheckCount,
  ...
  if (bBatched) {
    ...
    R_DrawDecalsAllImmediate_Gathered(pRenderContext, ppDecals,
                                      iPermanentLightmap, vModelOrg, flFade);
    ppDecals += iPermanentLightmap;
...
void DecalSurfaceDraw(IMatRenderContext *pRenderContext, int renderGroup,
  ...
  if (r_queued_decals.GetBool() &&
    ...
    if (DrawDecals.Count()) {
      ...
      memcpy(rd.Base(), DrawDecals.Base(),
             DrawDecals.Count() * sizeof(decal_t *));
      pCallQueue->QueueCall(DecalSurfaceDraw_QueueHelper, bBatched, renderGroup,
                            modelorg, nCheckCount, (decal_t **)rd.Base(),
                            iPermanentLightmap, iLightmap, iNonLightmap,
                            flFade);
