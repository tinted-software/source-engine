...
void CL_CopyNewEntity(CEntityReadInfo &u, int iClass, int iSerialNum) {
  ...
  if (u.m_bUpdateBaselines) {
    ...
    RecvTable_MergeDeltas(pRecvTable, &fromBuf, u.m_pBuf, &writeBuf, -1, NULL,
                          true);
...
void CL_PreserveExistingEntity(int nOldEntity) {
  ...
  if (!pEnt) {
    // If you hit this, this is because there's a networked client entity that
    // got released by some method other than a server update.  This can happen
    // if client code calls release on a networked entity.

#if defined(STAGING_ONLY)
    // Try to use the cl_removeentity_backtrace_capture code in
...
bool CL_ProcessPacketEntities(SVC_PacketEntities *entmsg) {
  ...
  if (entmsg->m_bIsDelta) {
    ...
    // If we have cl_debug_player_perf set and we see a huge delta between what
    // we've ack'd to the server and where it's at
    //  ask it for an instantaneous perf snapshot
    if (cl_debug_player_perf.GetBool() &&
        (flDeltaSeconds > 0.5f) && // delta is pretty out of date
        ((realtime - g_flLastPerfRequest) >
         5.0f)) // haven't requested in a while
    {
  ...
  if (entmsg->m_bUpdateBaseline) {
    ...
    cl.m_NetChannel->SendNetMsg(
        CLC_BaselineAck(cl.GetServerTickCount(), entmsg->m_nBaseline), true);
  }
