...
void DTI_Init() {
  ...
  if (CommandLine()->FindParm("-dti") && !g_bDTIEnabled) {
    ...
    V_FileBase(engineClient->GetLevelName(), dtiLevelName,
               ARRAYSIZE(dtiLevelName));
    V_snprintf(dtiFileName, ARRAYSIZE(dtiFileName),
               "dti_client_%s_%02d%02d%02d-%02d%02d%02d.csv", dtiLevelName,
    ...
    FOR_EACH_LL(g_DTIRecvTables, iTable) {
      ...
      for (int iProp = 0; iProp < pTable->m_Props.Count(); iProp++) {
        ...
        g_pFileSystem->FPrintf(
            ...
            // Total/Avg index bits
            pProp->m_nIndexBits, (float)pProp->m_nIndexBits / pProp->m_nDecodes,
            row++);
      }
    }
...
void DTI_HookRecvDecoder(CRecvDecoder *pDecoder) {
  ...
  for (int i = 0; i < pTable->m_Props.Count(); i++) {
    ...
    if (!dtiEnabled) {
      ...
      char *parentArrayPropName = const_cast<char *>(
          const_cast<SendProp *>(pSendProp)->GetParentArrayPropName());
      if (parentArrayPropName) {
        char temp[256];
        V_snprintf(temp, sizeof(temp), "%s:%s", parentArrayPropName,
                   pSendProp->GetName());
