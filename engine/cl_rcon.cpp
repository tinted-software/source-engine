...
// If we are going to include winsock.h then we need to disable
...
void CRConClient::SetAddress(const netadr_t &netAdr) {
  ...
  if (m_Address.GetPort() == 0) {
    m_Address.SetPort(
        PORT_SERVER); // override the port setting, by default rcon tries to
...
void CRConClient::ParseReceivedData() {
  ...
  while (size && size <= m_RecvBuffer.TellPut() - m_RecvBuffer.TellGet()) {
    ...
    default: {
      ...
      m_RecvBuffer.GetStringManualCharCount(
          msg.Base(), msg.Count()); // ignore the second string
    } break;
    }
