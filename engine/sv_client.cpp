...
static ConVar
    sv_maxrate("sv_maxrate", "0", FCVAR_REPLICATED,
               "Max bandwidth rate allowed on server, 0 == unlimited");
static ConVar
    sv_minrate("sv_minrate", "3500", FCVAR_REPLICATED,
               "Min bandwidth rate allowed on server, 0 == unlimited");

ConVar
    sv_maxupdaterate("sv_maxupdaterate", "100", FCVAR_REPLICATED,
                     "Maximum updates per second that the server will allow");
ConVar
    sv_minupdaterate("sv_minupdaterate", "20", FCVAR_REPLICATED,
...
bool CGameClient::ProcessClientInfo(CLC_ClientInfo *msg) {
  ...
  if (m_bIsHLTV) {
    // Likely spoofing, or misconfiguration. Don't let Disconnect pathway
    // believe this is a replay bot, it will asplode.
    m_bIsHLTV = false;
    Disconnect(
  ...
  if (m_bIsReplay) {
    // Likely spoofing, or misconfiguration. Don't let Disconnect pathway
    // believe this is an hltv bot, it will asplode.
    m_bIsReplay = false;
...
bool CGameClient::ProcessMove(CLC_Move *msg) {
  ...
  //with command %i(%i)\n", 		numcmds, 		m_Client->m_NetChan->incoming_sequence,
  ...
  if (msg->m_DataIn.IsOverflowed()) {
    Disconnect("ProcessUsercmds:  Overflowed reading usercmd data (check "
               "sending and receiving code for mismatches)!\n");
    return false;
  }
  ...
  if (msg->m_nLength != (endbit - startbit)) {
    Disconnect("ProcessUsercmds:  Incorrect reading frame (check sending and "
...
bool CGameClient::ProcessVoiceData(CLC_VoiceData *msg) {
  ...
  SV_BroadcastVoiceData(this, Bits2Bytes(bitsRead), voiceDataBuffer,
                        msg->m_xuid);
...
bool CGameClient::ProcessFileCRCCheck(CLC_FileCRCCheck *msg) {
  ...
  //	if ( !sv.GetPureServerWhitelist()->GetForceMatchList()->IsFileInList(
  //msg->m_szFilename ) ) 		return true;
  ...
  if (g_PureFileTracker.DoesFileMatch(path, fileName, msg->m_nFileFraction,
                                      &filehash, GetNetworkID())) {
...
void CGameClient::SetupPackInfo(CFrameSnapshot *pSnapshot) {
  ...
  if (sv_maxreplay.GetFloat() > 0) {
    ...
    // buffer
    nMaxFrames = max((float)nMaxFrames,
                     sv_maxreplay.GetFloat() / m_Server->GetTickInterval());
  }
...
void CGameClient::SendSnapshot(CClientFrame *pFrame) {
  ...
  if (m_bIsReplay) {
    ...
    int maxEnts = 255;
    replay->WriteTempEntities(
        this, pFrame->GetSnapshot(), m_pLastSnapshot.GetObject(),
...
bool CGameClient::ShouldSendMessages(void) {
  ...
  if (m_bIsReplay) {
    ...
    // calc snapshot interval
    int nSnapshotInterval =
        1.0f / (m_Server->GetTickInterval() * replay_snapshotrate);
