...
ConVar r_occluderminarea(
    "r_occluderminarea", "0", 0,
    "Prevents this occluder from being used if it takes up less than X% of the "
    "screen. 0 means use whatever the level said to use.");
ConVar r_occludeemaxarea(
    "r_occludeemaxarea", "0", 0,
    "Prevents occlusion testing for entities that take up more than X% of the "
    "screen. 0 means use whatever the level said to use.");
...
// Advance the active edge list until a particular X value is reached.
//-----------------------------------------------------------------------------
inline CWingedEdgeList::WingedEdge_t *
CWingedEdgeList::AdvanceActiveEdgeListToX(WingedEdge_t *pEdge, float x) {
...
inline const CEdgeList::Edge_t &
CEdgeList::EdgeFromSortIndex(int nSortIndex) const {
...
//-----------------------------------------------------------------------------
inline bool
COcclusionSystem::IsPointInsideNearPlane(const Vector &vecPos) const {
...
//-----------------------------------------------------------------------------
inline bool
COcclusionSystem::IsPointInsideAAPlane(const Vector &vecPos,
                                       const AxisAlignedPlane_t &plane) const {
...
void COcclusionSystem::RecomputeOccluderEdgeList() {
  ...
  for (i = host_state.worldbrush->numoccluders; --i >= 0;) {
    ...
    if (cl.m_chAreaBits && (cl.m_chAreaBits[pOccluders[i].area >> 3] &
...
bool COcclusionSystem::IsOccluded(const Vector &vecAbsMins,
  ...
  //vecBoxVertex, pVecProjectedVertex[ i ] ); 		if ( pVecProjectedVertex[ i ].z <=
  //0.0f ) 			return false;
  //	}
