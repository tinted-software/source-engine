...
bool CPlugin::Load(const char *fileName) {
  ...
  // Linux doesn't check signatures, so in that case disable plugins on the
  // client completely unless -insecure is specified
  ...
  if (m_pPluginModule) {
    ...
    if (pluginFactory) {
      ...
      if (!m_pPlugin) {
        ...
        m_pPlugin = (IServerPluginCallbacks *)pluginFactory(
            INTERFACEVERSION_ISERVERPLUGINCALLBACKS_VERSION_2, NULL);
        if (!m_pPlugin) {
          m_iPluginInterfaceVersion = 1;
          m_pPlugin = (IServerPluginCallbacks *)pluginFactory(
              INTERFACEVERSION_ISERVERPLUGINCALLBACKS_VERSION_1, NULL);
          if (!m_pPlugin) {
            Warning("Could not get IServerPluginCallbacks interface from "
                    "plugin \"%s\"",
                    fileName);
            return false;
          }
...
void CServerPlugin::LevelInit(char const *pMapName, char const *pMapEntities,
  ...
  FORALL_PLUGINS { CALL_PLUGIN_IF_ENABLED(LevelInit(pMapName)); }
...
void CServerPlugin::GameFrame(bool simulating) {
  FORALL_PLUGINS { CALL_PLUGIN_IF_ENABLED(GameFrame(simulating)); }
...
void CServerPlugin::SetCommandClient(int index) {
  FORALL_PLUGINS { CALL_PLUGIN_IF_ENABLED(SetCommandClient(index)); }

  serverGameClients->SetCommandClient(index);
}

void CServerPlugin::ClientSettingsChanged(edict_t *pEdict) {
  FORALL_PLUGINS { CALL_PLUGIN_IF_ENABLED(ClientSettingsChanged(pEdict)); }
