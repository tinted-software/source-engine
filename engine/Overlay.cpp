...
struct overlayvert_t {
  Vector pos;
  Vector normal;
  Vector2D texCoord[NUM_OVERLAY_TEXCOORDS]; // texcoord 0 = the mapped tex coord
                                            // from worldcraft texcoord 1 is
                                            // used for alpha and maps the whole
                                            // texture into the whole overlay
  float lightCoord[2];
  float flAlpha;
...
//-----------------------------------------------------------------------------
inline moverlayfragment_t *
COverlayMgr::GetOverlayFragment(OverlayFragmentHandle_t iFragment) {
  ...
  while (iCurrentRenderOrder <= iHighestRenderOrder) {
    ...
    for (int i = m_nFirstRenderQueue[nSortGroup]; i != RENDER_QUEUE_INVALID;
      ...
      int nMaxVertices = pRenderContext->GetMaxVerticesToRender(
          !bWireframeFragments ? renderQueueHead.m_pMaterial
        ...
        for (; hFragment != OVERLAY_FRAGMENT_INVALID;
          ...
          if (pMesh) {
            ...
            if (((nCurrVertexCount + nVertCount) > nMaxVertices) ||
              ...
              indexBuilder.FastPolygonList(0, polyList.Base(),
                                           polyList.Count());
              meshBuilder.End();
              pMesh->Draw();
              pMesh = NULL;
                      ...
                      // material and meshbuilder
          {
            ...
            meshBuilder.Begin(pMesh, MATERIAL_TRIANGLES,
                              MIN(renderQueue.m_nVertexCount, nMaxVertices),
...
void COverlayMgr::Surf_PostClipFragment(moverlay_t *pOverlay,
  ...
  for (int iVert = 0; iVert < nVertCount; ++iVert) {
    ...
    Vector2D uv;
    SurfComputeLightmapCoordinate(ctx, surfID,
                                  pFragment->m_aPrimVerts[iVert].pos, uv);
...
void COverlayMgr::Surf_ClipFragment(moverlay_t *pOverlay,
  ...
  if (pClippedFrag) {
    ...
    for (int iVert = 0; iVert < pClippedFrag->m_aPrimVerts.Count(); ++iVert) {
      overlayFrag.m_aPrimVerts[iVert].pos =
          pClippedFrag->m_aPrimVerts[iVert].pos;
      memcpy(overlayFrag.m_aPrimVerts[iVert].texCoord,
      ...
      if (pFrag) {
        ...
        for (int iVert = 0; iVert < nVertCount; ++iVert) {
          ...
          SurfComputeLightmapCoordinate(ctx, pFrag->m_SurfId,
                                        pFrag->m_aPrimVerts[iVert].pos, uv);
...
bool COverlayMgr::LoadOverlays() {
  ...
  for (int iOverlay = 0; iOverlay < nOverlayCount; ++iOverlay, ++pOverlayIn) {
    ...
    if (pOverlayOut->m_nRenderOrder >= OVERLAY_NUM_RENDER_ORDERS)
      Error("COverlayMgr::LoadOverlays: invalid render order (%d) for an "
            "overlay.",
...
COverlayMgr::AddFragmentToFragmentList(moverlayfragment_t *pSrc) {
  MEM_ALLOC_CREDIT();
