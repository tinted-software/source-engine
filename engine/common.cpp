...
void COM_ExplainDisconnection(bool bPrint, const char *fmt, ...) {
  ...
  if (bPrint) {
    if (gszDisconnectReason[0] == '#') {
      ...
      const wchar_t *wpchReason =
          g_pVGuiLocalize ? g_pVGuiLocalize->Find(gszDisconnectReason) : NULL;
      if (wpchReason) {
...
byte *COM_LoadFile(const char *path, int usehunk, int *pLength) {
  ...
  if (IsX360()) {
    bufSize =
        g_pFileSystem->GetOptimalReadSize(hFile, bufSize); // align to sector
  }
...
void COM_InitFilesystem(const char *pFullModPath) {
  ...
  if (IsPC()) {
    ...
    // There are two language at play here.  The Audio language which is
    // controled by the properties on the game itself in Steam (at least for
    // now).  And the language Steam is set to.
    // Under Windows the text in the game is controled by the language Steam is
    // set in, but the audio is controled by the language set in the game's
    // properties which we can get from Steam3Client

    // A command line override for audio language has also been added.
    // -audiolanguage <language>
    // User must have the .vpk files for the language installed though in order
    // to use the command line switch

    if (Steam3Client().SteamApps()) {
      // use -audiolanguage command line to override audio language, otherwise
      // take language from steam
      Q_strncpy(language,
                CommandLine()->ParmValue(
                    "-audiolanguage",
                    Steam3Client().SteamApps()->GetCurrentGameLanguage()),
                sizeof(language) - 1);
    } else {
      ...
      if (CommandLine()->CheckParm("-audiolanguage")) {
        Q_strncpy(language,
                  CommandLine()->ParmValue("-audiolanguage", "english"),
                  sizeof(language) - 1);
      } else if (szLang) {
  ...
  initInfo.m_bMountHDContent =
      BLoadHDContent(initInfo.m_pDirectoryName, GetBaseDirectory());
...
bool COM_IsValidPath(const char *pszFilename) {
  ...
  if (Q_strlen(pszFilename) <= 0 ||
      ...
      Q_strstr(pszFilename,
               "\r")) // CFileSystem_Stdio::FS_fopen doesn't allow this
  {
    return false;
  }
...
bool COM_IsValidLogFilename(const char *pszFilename) {
  ...
  if (V_stristr(pszFilename, "   ") ||
      V_stristr(pszFilename, "\t")) // don't multiple spaces or tab
    return false;
  ...
  if (extension) {
    if (Q_stricmp(extension, ".log") &&
        Q_stricmp(extension,
                  ".txt")) // must use .log or .txt if an extension is specified
      return false;
...
//-----------------------------------------------------------------------------
unsigned int COM_GetIdealDestinationCompressionBufferSize_Snappy(
...
//-----------------------------------------------------------------------------
void *COM_CompressBuffer_Snappy(const void *source, unsigned int sourceLen,
  ...
  unsigned nMaxCompressedSize =
      COM_GetIdealDestinationCompressionBufferSize_Snappy(sourceLen);
  ...
  *(uint32 *)pCompressed = SNAPPY_ID;
  ...
  snappy::RawCompress((const char *)source, sourceLen,
                      pCompressed + sizeof(uint32), &compressed_length);
  compressed_length += 4;
...
//-----------------------------------------------------------------------------
bool COM_BufferToBufferCompress_Snappy(void *dest, unsigned int *destLen,
                                       const void *source,
  ...
  if (compressedLen < nMaxCompressedSize) {
    ...
    // Yep.  Use the other function to allocate the buffer of the right size and
    // comrpess into it
    void *temp = COM_CompressBuffer_Snappy(source, sourceLen, &compressedLen,
                                           compressedLen);
    if (temp == NULL)
      return false;
  ...
  *(uint32 *)dest = SNAPPY_ID;
  size_t compressed_length;
  snappy::RawCompress((const char *)source, sourceLen,
                      (char *)dest + sizeof(uint32), &compressed_length);
  compressed_length += 4;
  Assert(compressed_length <= nMaxCompressedSize);
...
//-----------------------------------------------------------------------------
unsigned COM_GetIdealDestinationCompressionBufferSize_LZSS(
    unsigned int uncompressedSize) {
  // Our LZSS compressor doesn't need any extra space because it will stop and
  // fail as soon as it figures out it's unable to reduce the size of the data
  // by more than 32 bytes
  return uncompressedSize;
}

//-----------------------------------------------------------------------------
void *COM_CompressBuffer_LZSS(const void *source, unsigned int sourceLen,
                              unsigned int *compressedLen,
                              unsigned int maxCompressedLen) {
  Assert(source);
  Assert(compressedLen);
  ...
  if (pbOut && uCompressedLen > 0 &&
      (uCompressedLen <= maxCompressedLen || maxCompressedLen == 0)) {
...
//-----------------------------------------------------------------------------
bool COM_BufferToBufferCompress_LZSS(void *dest, unsigned int *destLen,
                                     const void *source,
                                     unsigned int sourceLen) {
  Assert(dest);
  Assert(destLen);
  ...
  CLZSS s;
  unsigned int uCompressedLen = 0;
  if (!s.CompressNoAlloc((const byte *)source, sourceLen, (unsigned char *)dest,
                         &uCompressedLen))
    return false;
...
//-----------------------------------------------------------------------------
int COM_GetUncompressedSize(const void *compressed,
                            unsigned int compressedLen) {
  ...
  if (compressedLen > sizeof(pHeader->id) && pHeader->id == SNAPPY_ID) {
    ...
    if (snappy::GetUncompressedLength(
            (const char *)compressed + sizeof(pHeader->id),
...
//-----------------------------------------------------------------------------
bool COM_BufferToBufferDecompress(void *dest, unsigned int *destLen,
  ...
  if (nDecompressedSize >= 0) {
    ...
    if ((unsigned)nDecompressedSize > *destLen) {
      Warning("NET_BufferToBufferDecompress with improperly sized dest buffer "
              "(%u in, %u needed)\n",
    ...
    if (pHeader->id == LZSS_ID) {
      CLZSS s;
      int nActualDecompressedSize =
          s.SafeUncompress((byte *)source, sourceLen, (byte *)dest, *destLen);
      if (nActualDecompressedSize != nDecompressedSize) {
        Warning("NET_BufferToBufferDecompress: header said %d bytes would be "
                "decompressed, but we LZSS decompressed %d\n",
    ...
    if (pHeader->id == SNAPPY_ID) {
      if (!snappy::RawUncompress((const char *)source + 4, sourceLen - 4,
                                 (char *)dest)) {
