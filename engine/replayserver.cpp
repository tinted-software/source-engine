...
CReplayDeltaEntityCache::~CReplayDeltaEntityCache() { Flush(); }
...
unsigned char *CReplayDeltaEntityCache::FindDeltaBits(int nEntityIndex,
                                                      int nDeltaTick,
                                                      int &nBits) {
  nBits = -1;
...
void CReplayDeltaEntityCache::AddDeltaBits(int nEntityIndex, int nDeltaTick,
...
CReplayFrame::~CReplayFrame() { FreeBuffers(); }
...
void CReplayFrame::CopyReplayData(CReplayFrame &frame) {
  ...
  if (bits > 0) {
    ...
    m_Messages[REPLAY_BUFFER_RELIABLE].StartWriting(new char[bytes], bytes,
                                                    bits);
  ...
  if (bits > 0) {
    ...
    m_Messages[REPLAY_BUFFER_UNRELIABLE].WriteBits(
        frame.m_Messages[REPLAY_BUFFER_UNRELIABLE].GetData(),
        frame.m_Messages[REPLAY_BUFFER_UNRELIABLE].GetNumBitsWritten());
    m_Messages[REPLAY_BUFFER_UNRELIABLE].WriteBits(
        frame.m_Messages[REPLAY_BUFFER_TEMPENTS].GetData(),
        frame.m_Messages[REPLAY_BUFFER_TEMPENTS].GetNumBitsWritten());
    m_Messages[REPLAY_BUFFER_UNRELIABLE].WriteBits(
        frame.m_Messages[REPLAY_BUFFER_SOUNDS].GetData(),
        frame.m_Messages[REPLAY_BUFFER_SOUNDS].GetNumBitsWritten());
    m_Messages[REPLAY_BUFFER_UNRELIABLE].WriteBits(
        frame.m_Messages[REPLAY_BUFFER_VOICE].GetData(),
...
int CReplayServer::GetReplaySlot(void) { return m_nPlayerSlot; }
...
int CReplayServer::GetEventDebugID() { return m_nDebugID; }
...
void CReplayServer::InstallStringTables(void) {
  ...
  for (int i = 0; i < numTables; i++) {
    ...
    if (!replayTable) {
      DevMsg(
          "SV_InstallReplayStringTableMirrors! Missing client table \"%s\".\n ",
...
CReplayEntityData *FindReplayDataInSnapshot(CFrameSnapshot *pSnapshot,
                                            int iEntIndex) {
  int a = 0;
...
void CReplayServer::EntityPVSCheck(CClientFrame *pFrame) {
  ...
  while (true) {
    ...
    CReplayEntityData *pReplayData =
        FindReplayDataInSnapshot(pSnapshot, entindex);
...
IServer *CReplayServer::GetBaseServer() { return (IServer *)this; }
...
void CReplayServer::UpdateTick(void) {
  ...
  int removeTick =
      m_nTickCount - 16.0f / m_flTickInterval; // keep 16 second buffer
...
void CReplayServer::FillServerInfo(SVC_ServerInfo &serverinfo) {
  ...
  serverinfo.m_nPlayerSlot =
      m_nPlayerSlot; // all spectators think they're the Replay client
...
const char *CReplayServer::GetPassword() const { return NULL; }

IClient *CReplayServer::ConnectClient(netadr_t &adr, int protocol,
                                      int challenge, int clientChallenge,
...
void CReplayServer::ReplyServerChallenge(netadr_t &adr) { return; }

void CReplayServer::RejectConnection(const netadr_t &adr, int clientChallenge,
                                     const char *s) {
  return;
}

CBaseClient *CReplayServer::CreateFakeClient(const char *name) { return NULL; }
...
void CReplayServer::SendPendingEvents() {
  ...
  if (m_flStartRecordTime != 0.0f) {
    ...
    IGameEvent *pSessionInfoEvent =
        g_pServerReplayContext->CreateReplaySessionInfoEvent();
