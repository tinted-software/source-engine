...
public:
  ...
  virtual bool IsPropInPVS(IHandleEntity *pHandleEntity,
                           const byte *pVis) const;
  virtual ICollideable *GetStaticProp(IHandleEntity *pHandleEntity);
  virtual void RecomputeStaticLighting();
  virtual LightCacheHandle_t
  GetLightCacheHandleForStaticProp(IHandleEntity *pHandleEntity);
  ...
  virtual void AddColorDecalToStaticProp(Vector const &rayStart,
                                         Vector const &rayEnd,
                                         int staticPropIndex, int decalIndex,
                                         bool doTrace, trace_t &tr,
                                         bool bUseColor, Color cColor);
...
bool CStaticProp::Init(int index, StaticPropLump_t &lump, model_t *pModel) {
  ...
  if (pStudioHdr) {
    if (!(pStudioHdr->flags & STUDIOHDR_FLAGS_STATIC_PROP)) {
      ...
      if (nBitchCount < 100) {
        Warning("model %s used as a static prop, but not compiled as a static "
                "prop\n",
                pStudioHdr->pszName());
        nBitchCount++;
      }
  ...
  default: {
    ...
    Q_strncpy(szModel,
              ...
              sizeof(szModel));
    Warning("CStaticProp::Init:  Map error, static_prop with bogus SOLID_ flag "
            "(%d)! (%s)\n",
  ...
  // If we do Mod_SetMaterialVarFlag() while running with the dedicated server,
  // we crash.
  //  RJ said he'd save my butt and look into this. (Hip hip horray! We love
  //  RJ!)
...
void CStaticProp::DisplayStaticPropInfo(int nInfoType) {
  ...
  case 4: {
    ...
    float flPixelWidth =
        pRenderContext->ComputePixelWidthOfSphere(GetRenderOrigin(), Radius());
...
CStaticPropMgr::~CStaticPropMgr() { Assert(!m_bLevelInitialized); }
...
template <>
void UnserializeLump<StaticPropLump_t>(StaticPropLump_t *_output,
                                       CUtlBuffer &buf) {
  Assert(_output != NULL);
...
void CStaticPropMgr::UnserializeModels(CUtlBuffer &buf) {
  ...
  for (int i = 0; i < count; ++i) {
    ...
    case 6:
      ...
      break;
    case 7: // Falls down to version 10. We promoted TF to version 10 to deal
            // with SFM.
    case 10: {
      if (s_MapVersion == 21)
    ...
    case 9:
      UnserializeLump<StaticPropLumpV9_t>(&lump, buf);
      break;
    case 11:
...
//-----------------------------------------------------------------------------
inline int
CStaticPropMgr::HandleEntityToIndex(IHandleEntity *pHandleEntity) const {
...
void CStaticPropMgr::GetAllStaticPropsInOBB(
  ...
  for (counter = 0; counter != iPropVectorSize; ++counter) {
    ...
    for (counter2 = 0; counter2 != 8; ++counter2) {
      /*ptPropExtents[counter2].x = (counter2 &
      (1<<0))?(vPropMaxs.x):(vPropMins.x); ptPropExtents[counter2].y = (counter2
      & (1<<1))?(vPropMaxs.y):(vPropMins.y); ptPropExtents[counter2].z =
      (counter2 & (1<<2))?(vPropMaxs.z):(vPropMins.z);*/
...
//-----------------------------------------------------------------------------
LightCacheHandle_t
CStaticPropMgr::GetLightCacheHandleForStaticProp(IHandleEntity *pHandleEntity) {
  int nIndex = HandleEntityToIndex(pHandleEntity);
...
void CStaticPropMgr::ComputePropOpacity(CStaticProp &prop) {
  ...
  if (modelinfoclient->ModelHasMaterialProxy(prop.GetModel())) {
    modelinfoclient->RecomputeTranslucency(
        prop.GetModel(), prop.GetSkin(), prop.GetBody(),
...
void CStaticPropMgr::AddDecalToStaticProp(Vector const &rayStart,
  ...
  AddColorDecalToStaticProp(rayStart, rayEnd, staticPropIndex, decalIndex,
                            doTrace, tr, false, tempColor);
}

//-----------------------------------------------------------------------------
void CStaticPropMgr::AddColorDecalToStaticProp(
  ...
  Vector up(0, 0, 1);
  if (bUseColor) {
    modelrender->AddColoredDecal(prop.GetModelInstance(), ray, up, decalIndex,
                                 0, cColor, noPokethru);
  } else {
