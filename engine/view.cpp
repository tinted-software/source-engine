...
void FullViewColorAdjustment() {
  ...
  IMaterial *color_correction = materials->FindMaterial(
      "dev/red_green_projection", TEXTURE_GROUP_OTHER, true);
  if (color_correction) {
  ...
  pRenderContext->DrawScreenSpaceRectangle(
      color_correction, 0, 0, nViewportWidth, nViewportHeight, nViewportX,
...
void V_RenderView(void) {
  ...
  if (!bCanRenderWorld) {
    ...
    // since we know we're going to render the world, check for lightmap updates
    // while it is easy to tear down and rebuild
...
public:
  ...
  void DrawBrushModel(IClientEntity *baseentity, model_t *model,
                      ...
                      bool bUnused) {
    R_DrawBrushModel(baseentity, model, origin, angles, DEPTH_MODE_NORMAL, true,
                     true);
  }
  ...
  virtual void DrawBrushModelEx(IClientEntity *baseentity, model_t *model,
    ...
    R_DrawBrushModel(baseentity, model, origin, angles, DEPTH_MODE_NORMAL,
  ...
  virtual void
  SetAreaState(unsigned char chAreaBits[MAX_AREA_STATE_BYTES],
