...
// If you modify this list, you will also need to add the following line to the
// game's client_*.vpc and server_*.vpc files:
//
...
void ReplaySystem_Init(bool bDedicated) {
  ...
  g_pReplay =
      (IReplaySystem *)g_fnReplayFactory(REPLAY_INTERFACE_VERSION, NULL);
  if (!g_pReplay) {
