...
static void sv_setsteamblockingcheck_f(IConVar *pConVar, const char *pOldString,
                                       float flOldValue);

ConVar sv_steamblockingcheck(
    "sv_steamblockingcheck", "0", 0,
    "Check each new player for Steam blocking compatibility, 1 = message only, "
    "2 >= drop if any member of owning clan blocks,"
    "3 >= drop if any player has blocked, 4 >= drop if player has blocked "
    "anyone on server",
...
CSteam3Server &Steam3Server() { return s_Steam3Server; }
...
CSteam3Server::CSteam3Server()
    ...
    : m_CallbackLogonSuccess(this, &CSteam3Server::OnLogonSuccess),
      ...
      m_CallbackValidateAuthTicketResponse(
          this, &CSteam3Server::OnValidateAuthTicketResponse),
      m_CallbackPlayerCompatibilityResponse(
          this, &CSteam3Server::OnComputeNewPlayerCompatibilityResponse),
...
// Purpose: Destructor
//-----------------------------------------------------------------------------
CSteam3Server::~CSteam3Server() { Shutdown(); }
...
void CSteam3Server::Activate(EServerType serverType) {
  ...
  case eServerModeAuthentication:
    MsgAndLog("Initializing Steam libraries for INSECURE Internet server.  "
              "Authentication and VAC not requested.\n");
    break;
  ...
  SteamAPI_SetTryCatchCallbacks(
      false); // We don't use exceptions, so tell steam not to use try/catch in
              // callback handlers
  if (CommandLine()->FindParm("-hushsteam") ||
      !SteamGameServer_InitSafe(
          ...
          m_usPort + 1, // Steam lives on -steamport + 1, master server updater
                        // lives on -steamport.
          usGamePort, usMasterServerUpdaterPort, m_eServerMode,
  ...
  // Note that SteamGameServer_InitSafe() calls SteamAPI_SetBreakpadAppID() for
  // you, which is what we don't want if we wish to report crashes under a
  // different AppId. Reset it back to our crashing one now.
  if (sv.IsDedicated()) {
  ...
  if (m_sAccountToken.IsEmpty()) {
    m_bWantsPersistentAccountLogon = false;
    MsgAndLog("No account token specified; logging into anonymous game server "
              "account.  (Use sv_setsteamaccount to login to a persistent "
              "account.)\n");
...
//-----------------------------------------------------------------------------
const CSteamID &CSteam3Server::GetGSSteamID() { return m_SteamIDGS; }
...
void CSteam3Server::OnLogonFailure(SteamServerConnectFailure_t *pLogonFailure) {
  ...
  if (!m_bLogOnResult) {
    if (pLogonFailure->m_eResult == k_EResultServiceUnavailable) {
      ...
      if (!BLanOnly()) {
        WarningAndLog(
            "Could not establish connection to Steam servers.  (Result = %d)\n",
            pLogonFailure->m_eResult);

        // If this was a permanent failure, switch to anonymous
        // TODO: Requires SDK update
        /*if ( m_bWantsPersistentAccountLogon && ( pLogonFailure->m_eResult ==
        k_EResultInvalidParam || pLogonFailure->m_eResult ==
        k_EResultAccountNotFound ) )
        {
                WarningAndLog( "Invalid game server account token. Retrying
        Steam connection with anonymous logon\n" );
...
void CSteam3Server::OnComputeNewPlayerCompatibilityResponse(
    ComputeNewPlayerCompatibilityResult_t *pCompatibilityResult) {
  CBaseClient *client =
      ClientFindFromSteamID(pCompatibilityResult->m_SteamIDCandidate);
  if (!client)
    return;
  if (sv_steamblockingcheck.GetInt()) {
    if (sv_steamblockingcheck.GetInt() >= 2) {
      if (pCompatibilityResult->m_cClanPlayersThatDontLikeCandidate > 0) {
        client->Disconnect("Another player on this server ( member of owning "
                           "clan ) does not want to play with this player.");
        return;
      }
    }
    if (sv_steamblockingcheck.GetInt() >= 3) {
      if (pCompatibilityResult->m_cPlayersThatDontLikeCandidate > 0) {
        client->Disconnect("Another player on this server does not want to "
    ...
    if (pCompatibilityResult->m_cClanPlayersThatDontLikeCandidate > 0 ||
      ...
      MsgAndLog("Player %s is blocked by %d players and %d clan members and "
                "has blocked %d players on server\n",
...
// Purpose:
//-----------------------------------------------------------------------------
void CSteam3Server::OnValidateAuthTicketResponse(
    ValidateAuthTicketResponse_t *pValidateAuthTicketResponse) {
  ...
  CBaseClient *client =
      ClientFindFromSteamID(pValidateAuthTicketResponse->m_SteamID);
  if (!client)
    return;

  if (pValidateAuthTicketResponse->m_eAuthSessionResponse !=
      k_EAuthSessionResponseOK) {
    OnValidateAuthTicketResponseHelper(
        client, pValidateAuthTicketResponse->m_eAuthSessionResponse);
    return;
  }

  if (Filter_IsUserBanned(client->GetNetworkID())) {
    sv.RejectConnection(client->GetNetChannel()->GetRemoteAddress(),
                        client->GetClientChallenge(),
  ...
  if (sv_steamblockingcheck.GetInt() >= 1) {
    SteamGameServer()->ComputeNewPlayerCompatibility(
...
//them
//-----------------------------------------------------------------------------
void CSteam3Server::OnValidateAuthTicketResponseHelper(
    CBaseClient *cl, EAuthSessionResponse eAuthSessionResponse) {
  INetChannel *netchan = cl->GetNetChannel();

  // If the client is timing out, the Steam failure is probably related (e.g.
  // game crashed). Let's just print that the client timed out.
  ...
  case k_EAuthSessionResponseVACCheckTimedOut:
    cl->Disconnect("An issue with your computer is blocking the VAC system. "
                   "You cannot play on secure "
                   "servers.\n\nhttps://support.steampowered.com/"
                   "kb_article.php?ref=2117-ILZV-2837");
    break;
  default:
...
//-----------------------------------------------------------------------------
bool CSteam3Server::NotifyClientConnect(CBaseClient *client, uint32 unUserID,
  ...
  if (steamID.GetEUniverse() !=
      SteamGameServer()->GetSteamID().GetEUniverse()) {
    WarningAndLog("Client %d %s connected to universe %d, but game server %s "
                  "is running in universe %d\n",
                  unUserID, steamID.Render(), steamID.GetEUniverse(),
                  SteamGameServer()->GetSteamID().Render(),
                  SteamGameServer()->GetSteamID().GetEUniverse());
    return false;
  }
  ...
  EBeginAuthSessionResult eResult =
      SteamGameServer()->BeginAuthSession(pvCookie, ucbCookie, steamID);
  switch (eResult) {
  ...
  case k_EBeginAuthSessionResultInvalidVersion:
    WarningAndLog("S3: Client connected with invalid ticket ( old version ): "
                  "UserID: %x\n",
                  unUserID);
    return false;
  ...
  default:
    WarningAndLog(
        "S3: Client failed auth session for unknown reason. UserID: %x\n",
        unUserID);
    return false;
  }
...
// Purpose:
//-----------------------------------------------------------------------------
void CSteam3Server::NotifyOfServerNameChange() { SendUpdatedServerDetails(); }
...
void CSteam3Server::SendUpdatedServerDetails() {
  ...
  // should not be propagated to the Steam layer.  Steam should be able to
  // report exactly what we give it to the master server, etc.
  for (int i = 0; i < sv.GetClientCount(); ++i) {
    CBaseClient *cl = (CBaseClient *)sv.GetClient(i);
    if (!cl->IsConnected())
      continue;
  ...
  if (sv_visiblemaxplayers.GetInt() > 0 &&
      sv_visiblemaxplayers.GetInt() < nMaxClients)
  ...
  if (hltv && hltv->IsActive()) {
    // This is also the case when we're a relay, in which case we never set a
    // game port, so we'll only have a spectator port
  ...
  //	Msg( "CSteam3Server::SendUpdatedServerDetails: nNumClients=%d,
  //nMaxClients=%d, nFakeClients=%d:\n", nNumClients, nMaxClients, nFakeClients
  //); 	for ( int i = 0 ; i < sv.GetClientCount() ; ++i )
  //	{
  //		IClient	*c = sv.GetClient( i );
  //		Msg("  %d: %s, connected=%d, replay=%d, fake=%d\n", i,
  //c->GetClientName(), c->IsConnected() ? 1 : 0, c->IsReplay() ? 1 : 0,
  //c->IsFakeClient() ? 1 : 0 );
  //	}
}
...
// static ConCommand heartbeat( "heartbeat", Heartbeat_f, "Force heartbeat of
// master servers", 0 );
...
void sv_setsteamaccount_f(const CCommand &args) {
  if (Steam3Server().SteamGameServer() &&
      Steam3Server().SteamGameServer()->BLoggedOn()) {
    Warning("Warning: Game server already logged into steam.  You need to use "
            "the sv_setsteamaccount command earlier.\n");
    return;
  }
...
static ConCommand
    sv_setsteamaccount("sv_setsteamaccount", sv_setsteamaccount_f,
                       "token\nSet game server account token to use for "
                       "logging in to a persistent game server account",
                       0);

static void sv_setsteamgroup_f(IConVar *pConVar, const char *pOldString,
                               float flOldValue);

ConVar sv_steamgroup(
    "sv_steamgroup", "", FCVAR_NOTIFY,
...
void CSteam3Server::UpdateGroupSteamID(bool bForce) {
  ...
  m_SteamIDGroupForBlocking.Set(unAccountID, m_SteamIDGS.GetEUniverse(),
                                k_EAccountTypeClan);
  if (SteamGameServer())
