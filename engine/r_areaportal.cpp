...
static inline bool GetPortalScreenExtents(dareaportal_t *pPortal,
  ...
  for (int j = 0; j < nPassCount; ++j) {
    ...
    for (i = 0; i < nStartVerts; i++) {
      ...
      g_EngineRenderer->ClipTransformWithProjection(g_ScreenFromWorldProjection,
                                                    point, &screenPos);
...
static void R_SetupGlobalFrustum() {
  ...
  if (g_viewSetup.m_bOrtho) {
    ...
    // Rather than try to deal with crazy projection matrices (shear, trapezoid,
    // etc), take whatever FOV we're given, assume it's conservative, and then
    // construct a matching projection matrix for it. Then use that consistent
    // hallucination throughout, rather than refer back to the engine's actual
    // proj. matrix for anything.
    VMatrix matrixView;
    VMatrix matrixProjection;
    VMatrix matrixWorldToScreen;
    g_viewSetup.m_bViewToProjectionOverride = false;

    ComputeViewMatrices(&matrixView, &matrixProjection, &matrixWorldToScreen,
                        g_viewSetup);
...
static void R_SetupVisibleAreaFrustums() {
  ...
  for (int i = 0; i < g_nVisibleAreas; i++) {
    ...
    portalWindow.left = RemapVal(pInfo->m_Rect.left, -1, 1, g_viewWindow.left,
                                 g_viewWindow.right);
    portalWindow.right = RemapVal(pInfo->m_Rect.right, -1, 1, g_viewWindow.left,
                                  g_viewWindow.right);
    portalWindow.top = RemapVal(pInfo->m_Rect.top, -1, 1, g_viewWindow.bottom,
                                g_viewWindow.top);
    ...
    if (g_viewSetup.m_bOrtho) {
      ...
      pInfo->m_Frustum.SetPlane(
          ...
          DotProduct(-CurrentViewForward(),
                     CurrentViewOrigin() +
                         CurrentViewForward() * g_viewSetup.zFar));
    }
...
void R_SetupAreaBits(int iForceViewLeaf /* = -1 */,
  ...
  if (host_state.worldbrush->leafs[leaf].contents & CONTENTS_SOLID ||
      cl.ishltv || cl.isreplay || !cl.m_bAreaBitsValid ||
