...
void CLocalNetworkBackdoor::InitFastCopy() {
  ...
  for (int iClass = 0; iClass < cl.m_nServerClasses; iClass++) {
    ...
    if (!pClientClass)
      Error("InitFastCopy - missing client class %d (Should be equivelent of "
            "server class: %s)",
            iClass, cl.m_pServerClasses[iClass].m_ClassName);
        ...
        if (toDelete & (1 << iBit)) {
          ...
          if (iEdict >= 0 && iEdict < MAX_EDICTS) {
            if (m_CachedEntState[iEdict].m_pNetworkable) {
              ...
              AssertOnce(!"EndEntityStateUpdate:  Would have crashed with NULL "
                          "m_pNetworkable\n");
            }
          } else {
            AssertOnce(!"EndEntityStateUpdate:  Would have crashed with entity "
                        "out of range\n");
          }
        }
      }
