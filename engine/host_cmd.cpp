...
static void voiceconvar_file_changed_f(IConVar *pConVar, const char *pOldValue,
                                       float flOldValue) {
#ifndef SWDS
  ConVarRef var(pConVar);
  if (var.GetInt() == 0) {
    // Force voice recording to stop if they turn off voice_inputfromfile or if
    // sv_allow_voice_from_file is set to 0. Prevents an exploit where clients
    // turn it on, start voice sending a long file, and then turn it off
    // immediately.
...
ConVar sv_allow_voice_from_file(
    "sv_allow_voice_from_file", "1", FCVAR_REPLICATED,
...
uint GetSteamAppID() {
  ...
  if (!bHaveValidSteamInterface) {
    ...
    if (!sunAppID)
      sunAppID = 215; // defaults to Source SDK Base (215) if no steam.inf can
                      // be found.
  }
...
void Host_Status_PrintClient(IClient *client, bool bShowAddress,
  ...
  builder.AddColumnText(
      va("%" STATUS_COLUMN_LENGTH_USERID_STR "i", client->GetUserID()),
      STATUS_COLUMN_LENGTH_USERID);
  builder.AddColumnText(va("\"%s\"", client->GetClientName()),
                        STATUS_COLUMN_LENGTH_NAME);
  builder.AddColumnText(client->GetNetworkIDString(),
                        STATUS_COLUMN_LENGTH_STEAMID);

  if (nci != NULL) {
    builder.AddColumnText(COM_FormatSeconds(nci->GetTimeConnected()),
                          STATUS_COLUMN_LENGTH_TIME);
    builder.AddColumnText(
        va("%" STATUS_COLUMN_LENGTH_PING_STR "i",
           (int)(1000.0f * nci->GetAvgLatency(FLOW_OUTGOING))),
        STATUS_COLUMN_LENGTH_PING);
    builder.AddColumnText(va("%" STATUS_COLUMN_LENGTH_LOSS_STR "i",
                             (int)(100.0f * nci->GetAvgLoss(FLOW_INCOMING))),
...
#define LIMIT_PER_CLIENT_COMMAND_EXECUTION_ONCE_PER_INTERVAL(seconds)          \
  {                                                                            \
    static float g_flLastTime__Limit[ABSOLUTE_PLAYER_LIMIT] = {                \
        0.0f}; /* we don't have access to any of the three MAX_PLAYERS         \
                  #define's here unfortunately */                              \
...
CON_COMMAND(status, "Display map and connection status.") {
  ...
  print("version : %s/%d %d %s%s%s\n",
        GetSteamInfIDVersionInfo().szVersionString, PROTOCOL_VERSION,
        build_number(), bGSSecure ? "secure" : "insecure",
        pchSecureReasonString, pchUniverse);
  ...
  // "uniqueid", cmd_source == src_command ? "  adr" : "" );
...
int _Host_Map_f_CompletionFunc(
    char const *cmdname, char const *partial,
    char commands[COMMAND_COMPLETION_MAXITEMS][COMMAND_COMPLETION_ITEM_LENGTH]);

// Note, leaves name alone if no match possible
static bool Host_Map_Helper_FuzzyName(const CCommand &args, char *name,
                                      size_t bufsize) {
  char commands[COMMAND_COMPLETION_MAXITEMS][COMMAND_COMPLETION_ITEM_LENGTH];
  CUtlString argv0;
  argv0 = args.Arg(0);
...
void Host_Map_Helper(const CCommand &args, bool bEditmode, bool bBackground,
  ...
  if ((g_iServerGameDLLVersion >= 10) &&
      !serverGameDLL->IsManualMapChangeOkay(&pszReason)) {
  ...
  IVEngineServer::eFindMapResult eResult =
      g_pVEngineServer->FindMap(szMapName, sizeof(szMapName));
  ...
  // In our case, we don't care because we're looking for anything on the order
  // of second precision, which covers runtime up to around 4 months.
...
#if defined(REPLAY_ENABLED)
  // If we're recording the game, finalize the replay so players can download
  // it.
...
//-----------------------------------------------------------------------------
CON_COMMAND(restart, "Restart the game on the same level (add setpos to jump "
                     "to current view position on restart).") {
  if (
#if !defined(SWDS)
...
// Restarts the current server for a dead player
//-----------------------------------------------------------------------------
CON_COMMAND(reload, "Reload the most recent saved game (add setpos to jump to "
                    "current view position on reload).") {
#ifndef SWDS
...
void Host_Changelevel_f(const CCommand &args) {
  ...
  IVEngineServer::eFindMapResult eResult =
      g_pVEngineServer->FindMap(szName, sizeof(szName));
  if (eResult == IVEngineServer::eFindMap_NotFound) {
    // Warn, but but proceed even if the map is not found, such that we hit the
    // proper server_levelchange_failed codepath and event later on.
...
void Host_Changelevel2_f(const CCommand &args) {
  if (args.ArgC() < 2) {
    ConMsg("changelevel2 <levelname> : continue game on a new level in the "
           "unit\n");
    return;
  }
  ...
  if (eResult == IVEngineServer::eFindMap_NotFound) {
    if (!CL_IsHL2Demo() ||
        (CL_IsHL2Demo() && !(!Q_stricmp(szName, "d1_trainstation_03") ||
                             !Q_stricmp(szName, "d1_town_02a")))) {
  ...
  // a valid map
  if (IsPC() && CL_IsHL2Demo() && !sv.IsDedicated() &&
      !Q_stricmp(szName, "d1_trainstation_03")) {
  ...
  if (IsPC() && CL_IsPortalDemo() && !sv.IsDedicated() &&
      !Q_stricmp(szName, "testchmb_a_07")) {
...
#if !defined(SWDS)
  // Just run the regular Disconnect function if we're not the client or the
  // client didn't handle it for us
...
#ifdef _WIN32
// manually pull in the GetEnvironmentVariableA defn so we don't need to include
// windows.h
extern "C" {
DWORD __declspec(dllimport) __stdcall GetEnvironmentVariableA(const char *,
                                                              char *, DWORD);
}
#endif // _WIN32
...
CON_COMMAND(version, "Print version info string.") {
  ...
  ConMsg("Network PatchVersion: %8s   # Determines client and server "
         "compatibility\n",
...
// No non-testing use for this at the moment, though server mods in public will
// expose similar functionality
...
CON_COMMAND(kickid, "Kick a player by userid or uniqueid, with a message.") {
  ...
  if (i < sv.GetClientCount()) {
    ...
    // can't kick yourself!
    if (cmd_source != src_command && host_client == client &&
        !sv.IsDedicated()) {
      return;
    }
...
public:
  ...
  PureCallBase() { NonPureFunction(); }

  void NonPureFunction() { PureFunction(); }
};
...
//-----------------------------------------------------------------------------
CON_COMMAND(crash, "[ sys_error | hang | purecall | segfault | minidump ]: "
                   "Cause the engine to crash.") {
  if (cmd_source != src_command)
    return;
...
CON_COMMAND(cache_print_summary, "cache_print_summary [section]\nPrint out a "
                                 "summary contents of cache memory.") {
  const char *pszSection = NULL;
  if (args.ArgC() == 2) {
    pszSection = args[1];
  }
...
CON_COMMAND(sv_dump_edicts,
  ...
  for (int i = 0; i < sv.num_edicts; ++i) {
    CUtlMap<CUtlString, int>::IndexType_t index =
        classNameCountMap.Find(sv.edicts[i].GetClassName());
...
// make valve_ds only?
CON_COMMAND_F(memory_diff, "show memory stats relative to snapshot",
              FCVAR_CHEAT) {
...
CON_COMMAND(fs_find, "Run virtual filesystem find") {
  ...
  for (pFile = g_pFullFileSystem->FindFirstEx(pWildcard, pPathID, &findhandle);
       pFile; pFile = g_pFullFileSystem->FindNext(findhandle)) {
    ConMsg("%s\n", pFile);
    matches++;
  }
