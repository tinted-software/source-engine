...
public: // CBaseServer interface:
  ...
  void RejectConnection(const netadr_t &adr, int clientChallenge,
                        const char *s);
...
public:
  ...
  void Changelevel();
  CClientFrame *
  AddNewFrame(CClientFrame *pFrame); // add new frame, returns Replay's copy
...
public:
  ...
  CReplayDemoRecorder
      m_DemoRecorder;           // Replay demo object for recording and playback
  ...
  CReplayFrame
      m_ReplayFrame; // all incoming messages go here until Snapshot is made
...
private:
  void SendPendingEvents(); // Send events to clients regarding
                            // start/stop/replays available
