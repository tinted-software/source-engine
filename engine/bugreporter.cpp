...
static ConVar
    bugreporter_includebsp("bugreporter_includebsp", "1", 0,
...
CBugUIPanel::CBugUIPanel(bool bIsPublic, vgui::Panel *parent)
    : BaseClass(parent, "BugUIPanel"), m_bIsPublic(bIsPublic),
      m_bAddVMF(false) {
  m_sDllName =
...
bool CBugUIPanel::Init() {
  ...
  if (m_pBugReporter && m_pBugReporter->IsPublicUI()) {
    ...
    if (Steam3Client().SteamUser()) {
      m_pSubmitterLabel->SetText(
          Steam3Client().SteamUser()->GetSteamID().Render());
    } else {
...
void CBugUIPanel::OnTick() {
  ...
  if (m_flPauseTime > 0.0f) {
    if (m_flPauseTime <= system()->GetFrameTime()) {
      ...
      if (m_pProgressDialog) {
        float percent =
            clamp(1.0f - (float)(m_flPauseTime - system()->GetFrameTime()) /
...
void CBugUIPanel::OnSubmit() {
  ...
  Q_snprintf(driverinfo, sizeof(driverinfo),
             ...
             "DXLevel:  %s\nVid:  %i x %i\n"
             "Framerate:  %.3f\n",
             osversion, info.m_pDriverName, info.m_VendorID, info.m_DeviceID,
  ...
  if (cl.IsActive() && g_ServerGlobalVariables.mapversion != 0 &&
    ...
    time_t mapfiletime = g_pFileSystem->GetFileTime(
...
#define BSIZE 65535
int copyfile(const char *local, const char *remote, void *ignored,
             int ignoredFlags) {
  ssize_t bytes;
  int fps, fpd;
  char buffer[BSIZE];
...
#error
    ...
    Assert(pCopyBuf);
    if (!pCopyBuf) {
      Warning("CBugUIPanel::UploadFile:  Unable to allocate copy buffer of %d "
              "bytes\n",
              nCopyBufferSize);
      fclose(r);
      g_pFileSystem->Close(hLocal);
...
void CEngineBugReporter::Init(void) { m_ParentPanel = 0; }
...
void CEngineBugReporter::InstallBugReportingUI(
  ...
  Q_snprintf(fn, sizeof(fn), "%s%s", GetInternalBugReporterDLL(),
             DLL_EXT_STRING);
