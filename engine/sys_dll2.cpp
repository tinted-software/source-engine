...
void EXPORT F(IEngineAPI **api) {
  CreateInterfaceFn factory =
      Sys_GetFactoryThis(); // This silly construction is necessary to prevent
                            // the LTCG compiler from crashing.
...
CON_COMMAND(bigalloc, "huge alloc crash") {
  ...
  // Alloc a bit less than UINT_MAX so there is room for heap headers in the
  // malloc functions.
...
extern "C" void __cdecl FailSafe(unsigned int uStructuredExceptionCode,
...
static void posix_signal_handler(int i) { siglongjmp(g_mark, -1); }
...
static bool IsSourceModLoaded() {
#if defined(_WIN32)
  static const char *s_pFileNames[] = {
      "metamod.2.tf2.dll", "sourcemod.2.tf2.dll", "sdkhooks.ext.2.ep2v.dll",
  ...
  if (fh) {
    char buf[1024];
    static const char *s_pFileNames[] = {
        "metamod.2.tf2.so", "sourcemod.2.tf2.so", "sdkhooks.ext.2.ep2v.so",
...
template <int _SIZE> class CErrorText {
public:
  ...
  void BuildComment(char const *pchSysErrorText, bool bRealCrash) {
    ...
    if (m_bIsDedicatedServer) {
      ...
      if (bSourceModLoaded) {
        ...
        //  like 232251 (instead of 232250). 232251 is for the TF2 Windows
        //  client, but we actually report those crashes under ID 440, so this
        //  should be ok.
...
#ifdef IS_WINDOWS_PC
// This warning only applies if you want to catch structured exceptions
// (crashes) using C++ exceptions. We do not want to do that so we can build
// with C++ exceptions completely disabled, and just suppress this warning.
// warning C4535: calling _set_se_translator() requires /EHa
#pragma warning(suppress : 4535)
    _se_translator_function curfilter = _set_se_translator(&FailSafe);
#elif defined(POSIX)
    // Only need to worry about this function crashing when we're dealing with a
    // real crash.
    ...
    DO_TRY {
      ...
      CommentPrintf("SourceMod:%d,DS:%d,Crash:%d\n\n", bSourceModLoaded,
                    m_bIsDedicatedServer, bRealCrash);
      ...
      if (Plat_GetPagedPoolInfo(&ppi_info) != SYSCALL_UNSUPPORTED) {
        CommentPrintf("\nPaged Pool\nprev PP PAGES: used: %lu, free %lu\nfinal "
                      "PP PAGES: used: %lu, free %lu\n",
                      g_pagedpoolinfo.numPagesUsed,
                      g_pagedpoolinfo.numPagesFree, ppi_info.numPagesUsed,
                      ppi_info.numPagesFree);
      }

      CommentPrintf("memallocfail? = %u\nActive: %s\nSpawnCount %d MapLoad "
                    "Count %d\nError count %d, end demo %d, abort count %d\n",
                    MemAlloc_MemoryAllocFailed(),
                    (game && game->IsActiveApp()) ? "active" : "inactive",
                    gHostSpawnCount, g_nMapLoadCount, g_HostErrorCount,
                    g_HostEndDemo, g_HostServerAbortCount);
...
#if defined(LINUX)
        if (bRealCrash) {
          // bRealCrash is set when we're actually making a comment for a dump
          // or error.
  ...
  void BuildCommentMemStatus() {
    ...
    if (hInst) {
      ...
      GetProcessMemoryInfoFn fn =
          (GetProcessMemoryInfoFn)GetProcAddress(hInst, "GetProcessMemoryInfo");
      if (fn) {
        ...
        if (fn(GetCurrentProcess(), &counters,
...
#undef _XTAG
  ...
  void BuildCommentExtended() {
    try {
      ...
      for (const ConCommandBase *var = g_pCVar->GetCommands(); var;
           var = var->GetNext()) {
        if (var->IsCommand())
          continue;
        ...
        if (!(pCvar->IsFlagSet(FCVAR_NEVER_AS_STRING))) {
          ...
          Q_strncpy(var1, Host_CleanupConVarStringValue(pCvar->GetString()),
                    sizeof(var1));
          Q_strncpy(var2, Host_CleanupConVarStringValue(pCvar->GetDefault()),
                    sizeof(var2));
        ...
        if (!(pCvar->IsFlagSet(FCVAR_NEVER_AS_STRING)))
          ...
          CommentPrintf("%s '%f' '%f'\n", pCvar->GetName(), pCvar->GetFloat(),
                        Q_atof(pCvar->GetDefault()));
      }
...
//-----------------------------------------------------------------------------
// Purpose: Attempt to initialize appid/steam.inf/minidump information. May only
// return partial information if called
//          before Filesystem is ready.
//
//          The desire is to be able to call this ASAP to init basic minidump
//          and AppID info, then re-call later on when the filesystem is setup,
//          so full version # information and such can be propagated to the
//          minidump system. (Currently, SDK mods will generally only have
//          partial information prior to filesystem init)
//-----------------------------------------------------------------------------
// steam.inf keys.
...
static eSteamInfoInit Sys_TryInitSteamInfo(void *pvAPI,
                                           SteamInfVersionInfo_t &VerInfo,
                                           const char *pchMod,
                                           const char *pchBaseDir,
                                           bool bDedicated) {
  ...
  if (!bFoundInf) {
    // We may try to load the steam.inf BEFORE we turn on the filesystem, so use
    // raw filesystem API's here.
    char szFullPath[MAX_PATH] = {0};
    char szModSteamInfPath[MAX_PATH] = {0};
    V_ComposeFileName(pchMod, "steam.inf", szModSteamInfPath,
                      sizeof(szModSteamInfPath));
    V_MakeAbsolutePath(szFullPath, sizeof(szFullPath), szModSteamInfPath,
                       pchBaseDir);
  ...
  if (bFoundInf) {
    ...
    while (1) {
      ...
      if (!Q_strnicmp(com_token, VERSION_KEY, Q_strlen(VERSION_KEY))) {
        V_strcpy_safe(VerInfo.szVersionString,
                      com_token + Q_strlen(VERSION_KEY));
        ...
        V_strcpy_safe(VerInfo.szProductString,
                      com_token + Q_strlen(PRODUCT_KEY));
      } else if (!Q_strnicmp(com_token, SERVER_VERSION_KEY,
                             Q_strlen(SERVER_VERSION_KEY))) {
      ...
      } else if (!Q_strnicmp(com_token, SERVER_APPID_KEY,
                             Q_strlen(SERVER_APPID_KEY))) {
        VerInfo.ServerAppID = atoi(com_token + Q_strlen(SERVER_APPID_KEY));
      }
    }

    // If we found a steam.inf we're as good as we're going to get, but don't
    // tell callers we're fully initialized if it doesn't at least have an AppID
    initState = (VerInfo.AppID != k_uAppIdInvalid) ? eSteamInfo_Initialized
                                                   : eSteamInfo_Partial;
  } else if (!bDedicated) {
    // Opening steam.inf failed - try to open gameinfo.txt and read in just
    // SteamAppId from that. (gameinfo.txt lacks the dedicated server steamid,
    // so we'll just have to live until filesystem init to setup breakpad there
    // when we hit this case)
    char szModGameinfoPath[MAX_PATH] = {0};
    char szFullPath[MAX_PATH] = {0};
    V_ComposeFileName(pchMod, "gameinfo.txt", szModGameinfoPath,
                      sizeof(szModGameinfoPath));
    V_MakeAbsolutePath(szFullPath, sizeof(szFullPath), szModGameinfoPath,
                       pchBaseDir);
    ...
    if (fp) {
      ...
      if (pkvGameInfo->LoadFromBuffer("gameinfo.txt", buffer)) {
        VerInfo.AppID = (AppId_t)pkvGameInfo->GetInt("FileSystem/SteamAppId",
                                                     k_uAppIdInvalid);
      }
    }
  ...
  // In partial state the ServerAppID might be unknown, but if we found the full
  // steam.inf and it's not set, it shares AppID.
  if (initState == eSteamInfo_Initialized &&
      VerInfo.ServerAppID == k_uAppIdInvalid)
...
#ifndef NO_STEAM
  // If -nobreakpad was specified or we found metamod or sourcemod, don't
  // register breakpad.
  bool bUseBreakpad = !CommandLine()->FindParm("-nobreakpad") &&
                      (!bDedicated || !IsSourceModLoaded());
  AppId_t BreakpadAppId = bDedicated ? VerInfo.ServerAppID : VerInfo.AppID;
  Assert(BreakpadAppId != k_uAppIdInvalid ||
         initState < eSteamInfo_Initialized);
  if (BreakpadAppId != k_uAppIdInvalid && initState > previousInitState &&
      bUseBreakpad) {
    void *pvMiniDumpContext = NULL;
    PFNPreMinidumpCallback pfnPreMinidumpCallback = NULL;
    bool bFullMemoryDump = !bDedicated && IsWindows() &&
                           CommandLine()->FindParm("-full_memory_dumps");

#if defined(POSIX)
    // On Windows we're relying on the try/except to build the minidump comment.
    // On Linux, we don't have that
    ...
    // We can filter various crash dumps differently in the Socorro backend
    // code:
    //    Steam/min/web/crash_reporter/socorro/scripts/config/collectorconfig.py
    SteamAPI_SetBreakpadAppID(BreakpadAppId);
    SteamAPI_UseBreakpadCrashHandler(pchVersion, __DATE__, __TIME__,
                                     bFullMemoryDump, pvMiniDumpContext,
...
bool CEngineAPI::Connect(CreateInterfaceFn factory) {
  ...
  if (!g_pStudioRender || !g_pDataCache || !g_pPhysics || !g_pMDLCache ||
      !g_pMatSystemSurface || !g_pInputSystem /* || !g_pVideo */) {
...
void CEngineAPI::SetStartupInfo(StartupInfo_t &info) {
  ...
  eSteamInfoInit steamInfo =
      Sys_TryInitSteamInfo(this, g_SteamInfIDVersionInfo, info.m_pInitialMod,
                           info.m_pBaseDirectory, bDedicated);
  ...
  if (steamInfo != eSteamInfo_Initialized) {
    // Try again with filesystem available. This is commonly needed for SDK mods
    // which need the filesystem to find their steam.inf, due to mounting SDK
    // search paths.
    steamInfo =
        Sys_TryInitSteamInfo(this, g_SteamInfIDVersionInfo, info.m_pInitialMod,
                             info.m_pBaseDirectory, bDedicated);
    ...
    if (steamInfo != eSteamInfo_Initialized) {
      Warning("Failed to find steam.inf or equivalent steam info. May not have "
              "proper information to connect to Steam.\n");
    }
  }
  ...
  if (IsPC()) {
    ...
    if (modinfo->LoadFromFile(g_pFileSystem, "gameinfo.txt")) {
      // Enable file tracking - client always does this in case it connects to a
      // pure server. server only does this if sv_pure is set If it's not
      // singleplayer_only
      ...
      m_bSupportsVR =
          modinfo->GetInt("supportsvr") > 0 && CommandLine()->CheckParm("-vr");
      if (m_bSupportsVR) {
        ...
        if (InitVR()) {
          if (Steam3Client().SteamUtils()) {
            if (Steam3Client().SteamUtils()->IsSteamRunningInVR() &&
                g_pSourceVR->IsHmdConnected()) {
...
InitReturnVal_t CEngineAPI::Init() {
  ...
  // Initialize the FPU control word
    ...
    !defined(__arm__) && !defined(PLATFORM_WINDOWS_PC64)
  _asm
      {
		fninit
      }
...
static void MoveConsoleWindowToFront() {
  ...
  if (hInst) {
    ...
    GetConsoleWindowFn fn =
        (GetConsoleWindowFn)GetProcAddress(hInst, "GetConsoleWindow");
    if (fn) {
      HWND hwnd = fn();
...
CON_COMMAND(gprofilerstart,
            "Starts the gperftools profiler recording to the specified file.") {
  if (g_gprofiling) {
  ...
  if (args.ArgC() < 2) {
    ...
    // Use the current date and time to create a unique file name.time_t t =
    // time(NULL);
    time_t t = time(NULL);
    struct tm tm = *localtime(&t);

    V_sprintf_safe(buffer, "%s/valveprofile_%4d_%02d_%02d_%02d.%02d.%02d.prof",
                   s_pszHomeDir, tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday,
  ...
  if (result) {
    Msg("Profiling started successfully. Recording to %s. Stop profiling with "
        "gprofilerstop.\n",
        profname);
    g_gprofiling = true;
  } else {
...
bool CEngineAPI::InitVR() {
  if (m_bSupportsVR) {
    ...
    if (g_pSourceVR) {
      ...
      if (!Host_AllowLoadModule("sourcevr" DLL_EXT_STRING, "EXECUTABLE_PATH",
                                false)) {
        Warning("Preventing connections to secure servers because sourcevr.dll "
                "is not signed.\n");
...
InitReturnVal_t CEngineAPI::HandleSetModeError() {
  ...
  if (CommandLine()->FindParm("-safe")) {
    Sys_MessageBox("Failed to set video mode.\n\nThis game has a minimum "
                   "requirement of DirectX 7.0 compatible hardware.\n",
  ...
  if (Sys_MessageBox(
          ...
          "restart with the new video settings.",
...
// This function is set as the crash handler for unhandled exceptions and as the
// minidump handler for to be used by all of tier0's crash recording. This
// function adds a game-specific minidump comment and ensures that the SteamAPI
// function is used to save the minidump so that crashes are uploaded. SteamAPI
// has previously been configured to use breakpad by calling
// SteamAPI_UseBreakpadCrashHandler.
extern "C" void __cdecl WriteSteamMiniDumpWithComment(
    unsigned int uStructuredExceptionCode,
    struct _EXCEPTION_POINTERS *pExceptionInfo, const char *pszFilenameSuffix) {
...
int CEngineAPI::Run() {
...
#ifdef _X360
  return RunListenServer(); // don't handle exceptions on 360 (because if we do
                            // then minidumps won't work at all)
#elif defined(_WIN32)
  ...
  // callback that is called from kernel mode because 64-bit Windows cannot
  // handle throwing exceptions across that boundary. See this article for
  // details:
...
bool CModAppSystemGroup::Create() {
  ...
  if (!serverSharedSystems) {
    Assert(!"Expected both game and client .dlls to have or not have shared "
            "app systems interfaces!!!");
    return AddLegacySystems();
  }
  ...
  for (int i = 0; i < list.Count(); ++i) {
    if (!Q_stricmp(list[i].m_pModuleName, moduleName)) {
      if (Q_stricmp(list[i].m_pInterfaceName, interfaceName)) {
        Error("Game and client .dlls requesting different versions '%s' vs. "
              "'%s' from '%s'\n",
...
DWORD __stdcall LongTickWatcherThread(void *voidPtr) {
  ...
  double flNextPossibleDumpTime =
      Plat_FloatTime() + MIN_TIME_BETWEEN_DUMPED_TICKS;
  int nNumDumpsThisTick = 0;

  while (eng->GetQuitting() == IEngine::QUIT_NOTQUITTING &&
    ...
    if (sv.m_State == ss_active && sv.m_bSimulatingTicks) {
      ...
      if (nLastTick > 0 && nLastTick == curTick) {
        if (curTime > flNextPossibleDumpTime && curTime > flWarnTickTime &&
          ...
          // If you're debugging a minidump and you ended up here, you probably
          // want to switch to the main thread.
...
bool EnableLongTickWatcher() {
  ...
  if (!g_bLongTickWatcherThreadEnabled) {
    ...
    VCRHook_CreateThread(NULL, 0,
                         ...
                         NULL, 0, (uintp *)&nThreadID);

    bRet = true;
  } else if (g_bQuitLongTickWatcherThread) {
    Msg("Cannot create a new long tick watcher while waiting for an old one to "
        "terminate.\n");
  } else {
...
bool CDedicatedServerAPI::Connect(CreateInterfaceFn factory) {
  ...
  if (CommandLine()->FindParm("-dumplongticks")) {
    Msg("-dumplongticks found on command line. Activating long tick watcher "
        "thread.\n");
...
bool CDedicatedServerAPI::ModInit(ModInfo_t &info) {
  ...
  // set this up as early as possible, if the server isn't going to run pure,
  // stop CRCing bits as we load them this happens even before the ConCommand's
  // are processed, but we need to be sure to either CRC every file that is
  // loaded, or not bother doing any Note that this mirrors g_sv_pure_mode from
  // sv_main.cpp
  ...
  if (pure_mode)
    g_pFullFileSystem->EnableWhitelistFileTracking(
        true, true,
        CommandLine()->FindParm("-sv_pure_verify_hashes") ? true : false);
  else
...
CON_COMMAND(dumplongticks, "Enables generating minidumps on long ticks.") {
  ...
  if (args.ArgC() == 1 || enable) {
    if (EnableLongTickWatcher()) {
      Msg("Long tick watcher thread created. Use \"dumplongticks 0\" to "
          "disable.\n");
    }
  } else {
