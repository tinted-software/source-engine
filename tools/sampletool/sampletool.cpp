...
const char *GetVGuiControlsModuleName() { return "SampleTool"; }
...
void CreateTools() { g_pSampleTool = new CSampleTool(); }
...
vgui::MenuBar *CSampleTool::CreateMenuBar(CBaseToolSystem *pParent) {
  ...
  CToolMenuButton *pFileButton = CreateToolFileMenuButton(
      pMenuBar, "File", "&File", GetActionTarget(), this);
  CToolMenuButton *pSwitchButton = CreateToolSwitchMenuButton(
      pMenuBar, "Switcher", "&Tools", GetActionTarget());
...
// Input  :  -
//-----------------------------------------------------------------------------
void CSampleTool::OnExit() { enginetools->Command("quit\n"); }
...
void CSampleTool::OnOpen() { OpenFile("txt"); }

bool CSampleTool::OnReadFileFromDisk(const char *pFileName,
                                     const char *pFileFormat,
                                     KeyValues *pContextKeyValues) {
  // FIXME: Implement
...
void CSampleTool::OnSaveAs() { SaveFile(NULL, NULL, 0); }

bool CSampleTool::OnWriteFileToDisk(const char *pFileName,
                                    const char *pFileFormat,
                                    KeyValues *pContextKeyValues) {
  // FIXME: Implement
...
//-----------------------------------------------------------------------------
void CSampleTool::OnFileOperationCompleted(
...
//-----------------------------------------------------------------------------
void CSampleTool::SetupFileOpenDialog(vgui::FileOpenDialog *pDialog,
                                      bool bOpenFile, const char *pFileFormat,
                                      KeyValues *pContextKeyValues) {
