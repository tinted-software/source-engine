...
// Constructor
//-----------------------------------------------------------------------------
CCommEditDoc::CCommEditDoc(ICommEditDocCallback *pCallback)
...
//-----------------------------------------------------------------------------
void CCommEditDoc::NotifyDataChanged(const char *pReason, int nNotifySource,
                                     int nNotifyFlags) {
...
bool CCommEditDoc::LoadFromFile(const char *pFileName) {
  ...
  if (!V_stricmp(Q_GetFileExtension(pFileName), "bsp")) {
    ...
    Q_StripExtension(pFileName, m_pTXTFileName,
                     sizeof(m_pTXTFileName) - strlen(pCommentaryAppend) - 1);
    ...
    CDmrElementArray<> subkeys(
        m_hRoot->AddAttribute("subkeys", AT_ELEMENT_ARRAY));
    ...
    if (!pComm) {
      ...
      vgui::MessageBox *pMessageBox =
          new vgui::MessageBox("Bad file name!\n", pBuf, g_pCommEditTool);
      pMessageBox->DoModal();
      return false;
    }
    ...
    *pComm = '\0';
    ...
    DmFileId_t fileid =
        g_pDataModel->RestoreFromFile(pFileName, NULL, "keyvalues", &pTXT);
...
void CCommEditDoc::SaveToFile() {
  if (m_hRoot.Get() && m_pTXTFileName && m_pTXTFileName[0]) {
    g_pDataModel->SaveToFile(m_pTXTFileName, NULL, "keyvalues", "keyvalues",
                             m_hRoot);
  }
...
CDmAttribute *CCommEditDoc::GetEntityList() {
  ...
  return hEntityList ? hEntityList->GetAttribute("subkeys", AT_ELEMENT_ARRAY)
                     : NULL;
}
...
// Purpose:
//-----------------------------------------------------------------------------
void CCommEditDoc::AddNewInfoTarget(const Vector &vecOrigin,
  ...
  CDmeCommentaryNodeEntity *pTarget;
  {
    ...
    pTarget = CreateElement<CDmeCommentaryNodeEntity>(
        "target", entities.GetOwner()->GetFileId());
...
// Purpose:
//-----------------------------------------------------------------------------
void CCommEditDoc::AddNewCommentaryNode(const Vector &vecOrigin,
                                        const QAngle &angAngles) {
  CDmrCommentaryNodeEntityList entities = GetEntityList();

  CDmeCommentaryNodeEntity *pNode;
  {
    CAppUndoScopeGuard guard(NOTIFY_SETDIRTYFLAG, "Add Commentary Node",
                             "Add Commentary Node");

    pNode = CreateElement<CDmeCommentaryNodeEntity>(
        "node", entities.GetOwner()->GetFileId());
...
void CCommEditDoc::DeleteCommentaryNode(CDmElement *pRemoveNode) {
  ...
  for (int i = 0; i < nCount; ++i) {
    if (pRemoveNode == entities[i]) {
      CAppUndoScopeGuard guard(NOTIFY_SETDIRTYFLAG, "Delete Commentary Node",
                               "Delete Commentary Node");
...
//-----------------------------------------------------------------------------
CDmeCommentaryNodeEntity *
CCommEditDoc::GetCommentaryNodeForLocation(Vector &vecOrigin,
                                           QAngle &angAbsAngles) {
  CDmrCommentaryNodeEntityList entities = GetEntityList();
  int nCount = entities.Count();
  for (int i = 0; i < nCount; ++i) {
...
//-----------------------------------------------------------------------------
bool CCommEditDoc::GetStringChoiceList(const char *pChoiceListType,
                                       CDmElement *pElement,
...
//-----------------------------------------------------------------------------
bool CCommEditDoc::GetElementChoiceList(const char *pChoiceListType,
                                        CDmElement *pElement,
                                        const char *pAttributeName,
                                        bool bArrayElement,
...
//-----------------------------------------------------------------------------
void CCommEditDoc::OnDataChanged(const char *pReason, int nNotifySource,
                                 int nNotifyFlags) {
