...
// Constructor
//-----------------------------------------------------------------------------
CCommentaryPropertiesPanel::CCommentaryPropertiesPanel(CCommEditDoc *pDoc,
                                                       vgui::Panel *pParent)
  ...
  m_pViewPositionPicker =
      new vgui::Button(m_pCommentaryNode, "ViewPositionPickerButton", "", this,
                       "PickViewPosition");
  m_pViewTargetPicker = new vgui::Button(
      m_pCommentaryNode, "ViewTargetPickerButton", "", this, "PickViewTarget");
  ...
  m_pPreventMovement =
      new vgui::CheckButton(m_pCommentaryNode, "PreventMovement", "");
  ...
  m_pCommentaryNode->LoadControlSettings(
  ...
  m_pInfoTarget->LoadControlSettings(
      "resource/commentarypropertiessubpanel_target.res");

  m_pCommentaryNodeScroll = new vgui::ScrollableEditablePanel(
      this, m_pCommentaryNode, "CommentaryNodeScroll");
  m_pInfoTargetScroll = new vgui::ScrollableEditablePanel(this, m_pInfoTarget,
                                                          "InfoTargetScroll");
...
// Text to attribute...
//-----------------------------------------------------------------------------
void CCommentaryPropertiesPanel::TextEntryToAttribute(
...
void CCommentaryPropertiesPanel::TextEntriesToVector(
...
void CCommentaryPropertiesPanel::UpdateCommentaryNode() {
  ...
  CAppUndoScopeGuard guard(NOTIFY_SETDIRTYFLAG, "Commentary Node Change",
                           "Commentary Node Change");
  ...
  m_hEntity->SetValue<int>("prevent_movement",
                           m_pPreventMovement->IsSelected() ? 1 : 0);
  m_hEntity->MarkDirty();
}
...
void CCommentaryPropertiesPanel::UpdateInfoTarget() {
  ...
  CAppUndoScopeGuard guard(NOTIFY_SETDIRTYFLAG, "Info Target Change",
                           "Info Target Change");
...
void CCommentaryPropertiesPanel::PopulateCommentaryNodeFields() {
  ...
  m_pPreventMovement->SetSelected(
      m_hEntity->GetValue<int>("prevent_movement") != 0);
}
...
void CCommentaryPropertiesPanel::PickSound() {
  CSoundPickerFrame *pSoundPickerDialog = new CSoundPickerFrame(
      g_pCommEditTool->GetRootPanel(), "Select commentary audio file",
      CSoundPicker::PICK_WAVFILES);
...
void CCommentaryPropertiesPanel::OnPicked(KeyValues *pParams) {
  ...
  vgui::TextEntry *pTextEntry =
      (vgui::TextEntry *)pContextKeyValues->GetPtr("widget");
...
void CCommentaryPropertiesPanel::PickInfoTarget(vgui::TextEntry *pControl) {
  ...
  CPickerFrame *pInfoTargetPickerDialog = new CPickerFrame(
      g_pCommEditTool->GetRootPanel(), "Select Target", "InfoTarget", NULL);
...
//-----------------------------------------------------------------------------
// Used to open a particular file in perforce, and deal with all the lovely
// dialogs
//-----------------------------------------------------------------------------
void CCommentaryPropertiesPanel::OnFileSelected(KeyValues *pKeyValues) {
  ...
  if (g_pFileSystem->FileExists(pFileName)) {
    ...
    Q_snprintf(pBuf, sizeof(pBuf),
               "File %s already exists!\nRecording audio will not overwrite "
               "existing files.",
               pFileName);
    vgui::MessageBox *pMessageBox =
        new vgui::MessageBox("File already Exists!\n", pBuf, g_pCommEditTool);
    pMessageBox->DoModal();
    return;
  }

  CSoundRecordPanel *pSoundRecordPanel = new CSoundRecordPanel(
      g_pCommEditTool->GetRootPanel(), "Record Commentary");
...
void CCommentaryPropertiesPanel::RecordSound() {
  ...
  vgui::FileOpenDialog *pDialog =
      new vgui::FileOpenDialog(this, "Save As", false);
