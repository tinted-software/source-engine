...
const char *GetVGuiControlsModuleName() { return "CommEditTool"; }
...
void CreateTools() { g_pCommEditTool = new CCommEditTool(); }
...
bool CCommEditTool::Init() {
  ...
  {
    m_hPreviewNode = CreateElement<CDmeCommentaryNodeEntity>("preview node",
                                                             DMFILEID_INVALID);
    m_hPreviewNode->SetValue("classname", "point_commentary_node");
    m_hPreviewTarget = CreateElement<CDmeCommentaryNodeEntity>(
        "preview target", DMFILEID_INVALID);
...
//-----------------------------------------------------------------------------
inline CCommEditDoc *CCommEditTool::GetDocument() { return m_pDoc; }
...
bool CCommEditTool::TrapKey(ButtonCode_t key, bool down) {
  ...
  if (!m_bInNodeDropMode) {
    if (!IsGameInputEnabled() && !IsFullscreen() && (key == KEY_BACKQUOTE) &&
        down) {
      BringConsoleToFront();
      return true;
    }
...
public:
  CCommEditViewMenuButton(CCommEditTool *parent, const char *panelName,
...
CCommEditViewMenuButton::CCommEditViewMenuButton(
    CCommEditTool *parent, const char *panelName, const char *text,
    vgui::Panel *pActionSignalTarget)
    : BaseClass(parent, panelName, text, pActionSignalTarget) {
  m_pTool = parent;

  AddCheckableMenuItem("properties", "#CommEditProperties",
                       new KeyValues("OnToggleProperties"),
                       pActionSignalTarget);
  AddCheckableMenuItem("commentarynodebrowser", "#CommEditEntityReport",
                       new KeyValues("OnToggleEntityReport"),
                       pActionSignalTarget);
  AddCheckableMenuItem("console", "#BxConsole", new KeyValues("ToggleConsole"),
                       pActionSignalTarget);

  AddSeparator();

  AddMenuItem("defaultlayout", "#CommEditViewDefault",
...
public:
  CCommEditToolMenuButton(CCommEditTool *parent, const char *panelName,
                          const char *text, vgui::Panel *pActionSignalTarget);
  virtual void OnShowMenu(vgui::Menu *menu);
...
CCommEditToolMenuButton::CCommEditToolMenuButton(
    CCommEditTool *parent, const char *panelName, const char *text,
    vgui::Panel *pActionSignalTarget)
    : BaseClass(parent, panelName, text, pActionSignalTarget) {
  m_pTool = parent;

  AddMenuItem("addnewnodes", "#CommEditAddNewNodes",
...
vgui::MenuBar *CCommEditTool::CreateMenuBar(CBaseToolSystem *pParent) {
  ...
  CCommEditToolMenuButton *pToolButton = new CCommEditToolMenuButton(
      this, "CommEdit", "&CommEdit", GetActionTarget());
  CCommEditViewMenuButton *pViewButton =
      new CCommEditViewMenuButton(this, "View", "&View", GetActionTarget());
...
CConsolePage *CCommEditTool::GetConsole() { return m_hConsole; }
...
//-----------------------------------------------------------------------------
void CCommEditTool::ShowEntityInEntityProperties(
...
void CCommEditTool::OnDefaultLayout() {
  ...
  ToolWindow *pPropertyWindow = m_ToolWindowFactory.InstanceToolWindow(
      GetClientArea(), false, properties, "#CommEditProperties", false);
  ToolWindow *pEntityReportWindow = m_ToolWindowFactory.InstanceToolWindow(
      GetClientArea(), false, pEntityReport, "#CommEditEntityReport", false);
...
void CCommEditTool::CreateTools(CCommEditDoc *doc) {
  ...
  if (!m_hCommentaryNodeBrowser.Get()) {
    m_hCommentaryNodeBrowser = new CCommentaryNodeBrowserPanel(
        m_pDoc, this, "CommentaryNodeBrowserPanel");
  }
...
void CCommEditTool::InitTools() {
  ...
  windowposmgr->RegisterPanel("commentarynodebrowser", m_hCommentaryNodeBrowser,
                              false);
  windowposmgr->RegisterPanel("Console", m_hConsole, false); // No context menu

  if (!windowposmgr->LoadPositions("cfg/commedit.txt", this,
                                   &m_ToolWindowFactory, "CommEdit")) {
    OnDefaultLayout();
  }
}
...
void CCommEditTool::ShowToolWindow(Panel *tool, char const *toolName,
                                   bool visible) {
  Assert(tool);
...
bool CCommEditTool::OnReadFileFromDisk(const char *pFileName,
...
bool CCommEditTool::OnWriteFileToDisk(const char *pFileName,
                                      const char *pFileFormat,
                                      KeyValues *pContextKeyValues) {
  if (!m_pDoc)
    return false;
...
void CCommEditTool::OnClose() {
  if (m_pDoc && m_pDoc->IsDirty()) {
    SaveFile(m_pDoc->GetTXTFileName(), "txt",
...
void CCommEditTool::CenterView(CDmeCommentaryNodeEntity *pEntity) {
  ...
  servertools->SnapPlayerToPosition(vecOrigin, angles,
...
void CCommEditTool::AttachAllEngineEntities() {
  ...
  for (EntitySearchResult sr = clienttools->FirstEntity(); sr != NULL;
       sr = clienttools->NextEntity(sr)) {
    if (!sr)
      continue;
    ...
    if (Q_strcmp(pClassName, "class C_PointCommentaryNode") == 0) {
      ...
      CDmeCommentaryNodeEntity *pNode =
          m_pDoc->GetCommentaryNodeForLocation(vecOrigin, angAngles);
      if (pNode) {
...
// saved/loaded
//-----------------------------------------------------------------------------
void CCommEditTool::OnFileOperationCompleted(
...
//-----------------------------------------------------------------------------
void CCommEditTool::SetupFileOpenDialog(vgui::FileOpenDialog *pDialog,
...
// Background
//-----------------------------------------------------------------------------
const char *CCommEditTool::GetLogoTextureName() { return NULL; }

//-----------------------------------------------------------------------------
// Inherited from ICommEditDocCallback
//-----------------------------------------------------------------------------
void CCommEditTool::OnDocChanged(const char *pReason, int nNotifySource,
                                 int nNotifyFlags) {
