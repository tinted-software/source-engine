...
// Constructor
//-----------------------------------------------------------------------------
CGameEventEditPanel::CGameEventEditPanel(CGameEventEditDoc *pDoc,
                                         vgui::Panel *pParent)
  ...
  for (int i = 0; i < MAX_GAME_EVENT_PARAMS; i++) {
    m_pParamLabels[i] = new vgui::Label(this, VarArgs("ParamLabel%d", i + 1),
                                        VarArgs("event param %d:", i + 1));
  ...
  m_pSendEventButton =
      new vgui::Button(this, "SendEvent", "", this, "SendEvent");
...
CGameEventEditPanel::~CGameEventEditPanel() { m_pEvents->deleteThis(); }
  ...
  if (panel == m_pEventCombo) {
    ...
    if (kv) {
      ...
      while (subkey && i < MAX_GAME_EVENT_PARAMS) {
        ...
        char buf[128];
        Q_snprintf(buf, sizeof(buf), "%s (%s)", subkey->GetName(),
                   subkey->GetString());
