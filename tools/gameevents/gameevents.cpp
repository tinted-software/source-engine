...
const char *GetVGuiControlsModuleName() { return "GameEvents"; }
...
bool ConnectTools(CreateInterfaceFn factory) {
  gameeventmanager =
      (IGameEventManager2 *)factory(INTERFACEVERSION_GAMEEVENTSMANAGER2, NULL);
...
void CreateTools() { g_pSampleTool = new CGameEventTool(); }
...
// Input  :  -
//-----------------------------------------------------------------------------
void CGameEventTool::OnExit() { enginetools->Command("quit\n"); }
...
void CGameEventTool::OnNew() {
  if (m_pDoc) {
    if (m_pDoc->IsDirty()) {
      SaveFile(m_pDoc->GetTXTFileName(), "xt",
...
bool CGameEventTool::OnReadFileFromDisk(const char *pFileName,
                                        const char *pFileFormat,
                                        KeyValues *pContextKeyValues) {
  OnCloseNoSave();
...
void CGameEventTool::OnSaveAs() { SaveFile(NULL, NULL, 0); }

bool CGameEventTool::OnWriteFileToDisk(const char *pFileName,
                                       const char *pFileFormat,
                                       KeyValues *pContextKeyValues) {
  // FIXME: Implement
...
//-----------------------------------------------------------------------------
void CGameEventTool::OnFileOperationCompleted(
...
//-----------------------------------------------------------------------------
void CGameEventTool::SetupFileOpenDialog(vgui::FileOpenDialog *pDialog,
                                         bool bOpenFile,
                                         const char *pFileFormat,
                                         KeyValues *pContextKeyValues) {
...
CConsolePage *CGameEventTool::GetConsole() { return m_hConsole; }

void CGameEventTool::ShowToolWindow(Panel *tool, char const *toolName,
                                    bool visible) {
  Assert(tool);
...
void CGameEventTool::OnDefaultLayout() {
  ...
  ToolWindow *pEditPanelWindow = m_ToolWindowFactory.InstanceToolWindow(
      GetClientArea(), false, pEditPanel, "#CommEditProperties", false);
