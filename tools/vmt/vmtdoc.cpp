...
void CVMTDoc::SetupPreviewMaterial() {
  ...
  IMaterial *pMaterial = g_pMaterialSystem->FindMaterial(
      pMaterialName, "Editable material", false);
...
CDmElement *CVMTDoc::ExtractDefaultParameters() {
  ...
  for (; pAttribute; pAttribute = pNextAttribute) {
    ...
    // FIXME: We can't do this.. the defaults in the strings need to be changed
    // to make it so they actually match the true defaults
    continue;
  ...
  for (; pAttribute; pAttribute = pNextAttribute) {
    ...
    if (pOldShader) {
      ...
      for (i = 0; i < nCount; ++i) {
        ...
        char temp1[512];
        char temp2[512];
        CUtlBuffer buf1(temp1, sizeof(temp1),
...
//-----------------------------------------------------------------------------
CDmAttribute *
CVMTDoc::AddAttributeForShaderParameter(CDmElement *pMaterial,
