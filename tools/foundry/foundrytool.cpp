...
//-----------------------------------------------------------------------------
class CFoundryTool : public CBaseToolSystem,
                     public IFileMenuCallbacks,
                     public IFoundryDocCallback,
                     public IFoundryTool {
...
void CFoundryTool::InitEditorDict() {
  ...
  CDmeEditorType *pEditorType =
      CreateElement<CDmeEditorType>("vmfEntity", DMFILEID_INVALID);
  ...
  pInfo =
      CreateElement<CDmeEditorAttributeInfo>("editor info", DMFILEID_INVALID);
  pInfo->m_bIsVisible = false;
  pEditorType->AddAttributeInfo("editor", pInfo);
  m_toolElements.AddToTail(pInfo->GetHandle());

  pInfo =
      CreateElement<CDmeEditorAttributeInfo>("other info", DMFILEID_INVALID);
  pInfo->m_bIsVisible = false;
  pEditorType->AddAttributeInfo("other", pInfo);
  m_toolElements.AddToTail(pInfo->GetHandle());

  pInfo =
      CreateElement<CDmeEditorAttributeInfo>("_visible info", DMFILEID_INVALID);
  pInfo->m_bIsVisible = false;
  pEditorType->AddAttributeInfo("_visible", pInfo);
  m_toolElements.AddToTail(pInfo->GetHandle());

  pInfo = CreateElement<CDmeEditorAttributeInfo>("_placeholder info",
                                                 DMFILEID_INVALID);
  pInfo->m_bIsVisible = false;
...
// returns the document
//-----------------------------------------------------------------------------
inline CFoundryDoc *CFoundryTool::GetDocument() { return m_pDoc; }
...
public:
  CFoundryViewMenuButton(CFoundryTool *parent, const char *panelName,
...
CFoundryViewMenuButton::CFoundryViewMenuButton(CFoundryTool *parent,
  ...
  AddCheckableMenuItem("properties", "#FoundryProperties",
                       new KeyValues("OnToggleProperties"),
                       pActionSignalTarget);
  AddCheckableMenuItem("entityreport", "#FoundryEntityReport",
                       new KeyValues("OnToggleEntityReport"),
                       pActionSignalTarget);

  AddSeparator();

  AddMenuItem("defaultlayout", "#FoundryViewDefault",
...
public:
  CFoundryToolMenuButton(CFoundryTool *parent, const char *panelName,
                         const char *text, vgui::Panel *pActionSignalTarget);
  virtual void OnShowMenu(vgui::Menu *menu);
...
CFoundryToolMenuButton::CFoundryToolMenuButton(CFoundryTool *parent,
                                               const char *panelName,
                                               const char *text,
                                               vgui::Panel *pActionSignalTarget)
    : BaseClass(parent, panelName, text, pActionSignalTarget) {
  ...
  AddMenuItem("reload", "#FoundryReload", new KeyValues("ReloadMap"),
              pActionSignalTarget);
  AddMenuItem("reloadsave", "#FoundryReloadFromSave",
              new KeyValues("ReloadFromSave"), pActionSignalTarget);
...
vgui::MenuBar *CFoundryTool::CreateMenuBar(CBaseToolSystem *pParent) {
  ...
  CFoundryToolMenuButton *pToolButton = new CFoundryToolMenuButton(
      this, "Foundry", "F&oundry", GetActionTarget());
  CFoundryViewMenuButton *pViewButton =
      new CFoundryViewMenuButton(this, "View", "&View", GetActionTarget());
...
void CFoundryTool::OnDefaultLayout() {
  ...
  ToolWindow *pPropertyWindow = m_ToolWindowFactory.InstanceToolWindow(
      GetClientArea(), false, properties, "#FoundryProperties", false);
  ToolWindow *pEntityReportWindow = m_ToolWindowFactory.InstanceToolWindow(
      GetClientArea(), false, pEntityReport, "#FoundryEntityReport", false);
...
void CFoundryTool::InitTools() {
  ...
  if (!windowposmgr->LoadPositions("cfg/foundry.txt", this,
                                   &m_ToolWindowFactory, "Foundry")) {
    OnDefaultLayout();
  }
}
...
void CFoundryTool::ShowToolWindow(Panel *tool, char const *toolName,
                                  bool visible) {
  Assert(tool);
...
void CFoundryTool::OnNew() {
  if (m_pDoc) {
    if (m_pDoc->IsDirty()) {
      SaveFile(m_pDoc->GetVMFFileName(), "vmf",
...
bool CFoundryTool::OnReadFileFromDisk(const char *pFileName,
                                      const char *pFileFormat,
                                      KeyValues *pContextKeyValues) {
  OnCloseNoSave();
...
bool CFoundryTool::OnWriteFileToDisk(const char *pFileName,
                                     const char *pFileFormat,
                                     KeyValues *pContextKeyValues) {
  if (!m_pDoc)
    return true;
...
// saved/loaded
//-----------------------------------------------------------------------------
void CFoundryTool::OnFileOperationCompleted(
...
//-----------------------------------------------------------------------------
void CFoundryTool::SetupFileOpenDialog(vgui::FileOpenDialog *pDialog,
                                       bool bOpenFile, const char *pFileFormat,
                                       KeyValues *pContextKeyValues) {
...
// Inherited from IFoundryDocCallback
//-----------------------------------------------------------------------------
void CFoundryTool::OnDocChanged(const char *pReason, int nNotifySource,
                                int nNotifyFlags) {
  UpdateMenuBar();
