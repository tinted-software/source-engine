...
// Constructor
//-----------------------------------------------------------------------------
CFoundryDoc::CFoundryDoc(IFoundryDocCallback *pCallback)
...
//-----------------------------------------------------------------------------
void CFoundryDoc::NotifyDataChanged(const char *pReason, int nNotifySource,
                                    int nNotifyFlags) {
...
//-----------------------------------------------------------------------------
void CFoundryDoc::OnDataChanged(const char *pReason, int nNotifySource,
                                int nNotifyFlags) {
...
//-----------------------------------------------------------------------------
// Always copy the worldspawn and other entities that had data built into them
// by VBSP out
//-----------------------------------------------------------------------------
void CFoundryDoc::AddOriginalEntities(CUtlBuffer &entityBuf,
...
//-----------------------------------------------------------------------------
void CFoundryDoc::AddVMFEntities(CUtlBuffer &entityBuf,
                                 const char *pActualEntityData) {
  ...
  for (int iEntity = 0; iEntity < nCount; ++iEntity) {
    ...
    for (CDmAttribute *pAttribute = pEntity->FirstAttribute(); pAttribute;
      ...
      if (!Q_stricmp(pAttribute->GetName(), "editorType") ||
          !Q_stricmp(pAttribute->GetName(), "editor"))
        continue;
...
const char *CFoundryDoc::GenerateEntityData(const char *pActualEntityData) {
  ...
  // Contains the text block the engine can parse containing the entity data to
  // spawn
