...
// Sort by target name
//-----------------------------------------------------------------------------
static int __cdecl TargetNameSortFunc(vgui::ListPanel *pPanel,
...
// Sort by class name
//-----------------------------------------------------------------------------
static int __cdecl ClassNameSortFunc(vgui::ListPanel *pPanel,
                                     const ListPanelItem &item1,
                                     const ListPanelItem &item2) {
...
// Constructor
//-----------------------------------------------------------------------------
CEntityReportPanel::CEntityReportPanel(CFoundryDoc *pDoc, vgui::Panel *pParent,
                                       const char *pName)
  ...
  m_pEntities->AddColumnHeader(0, "targetname", "Name", 52,
                               ListPanel::COLUMN_RESIZEWITHWINDOW);
  m_pEntities->AddColumnHeader(1, "classname", "Class Name", 52,
                               ListPanel::COLUMN_RESIZEWITHWINDOW);
...
void CEntityReportPanel::ReadSettingsFromRegistry() {
  ...
  m_pFilterByKeyvalue->SetSelected(
      registry->ReadInt(pKeyBase, "FilterByKeyvalue", 0));
  m_pFilterByClass->SetSelected(
      registry->ReadInt(pKeyBase, "FilterByClass", 0));
  m_pFilterByHidden->SetSelected(
      registry->ReadInt(pKeyBase, "FilterByHidden", 1));
  m_pExact->SetSelected(registry->ReadInt(pKeyBase, "Exact", 0));

  m_iFilterByType = (FilterType_t)registry->ReadInt(pKeyBase, "FilterByType",
                                                    FILTER_SHOW_EVERYTHING);
  m_pFilterEverything->SetSelected(m_iFilterByType == FILTER_SHOW_EVERYTHING);
  m_pFilterPointEntities->SetSelected(m_iFilterByType ==
                                      FILTER_SHOW_POINT_ENTITIES);
  m_pFilterBrushModels->SetSelected(m_iFilterByType ==
                                    FILTER_SHOW_BRUSH_ENTITIES);

  // Gotta call change functions manually since SetText doesn't post an action
  // signal
...
void CEntityReportPanel::OnDeleteEntities(void) {
  // This is undoable
  CAppUndoScopeGuard guard(NOTIFY_SETDIRTYFLAG, "Delete Entities",
                           "Delete Entities");
...
void CEntityReportPanel::OnCommand(const char *pCommand) {
  if (!Q_stricmp(pCommand, "delete")) {
    ...
    MessageBox *pConfirm =
        new MessageBox("#FoundryDeleteObjects", "#FoundryDeleteObjectsMsg",
                       g_pFoundryTool->GetRootPanel());
...
//-----------------------------------------------------------------------------
// FIXME: Necessary because SetSelected doesn't cause a ButtonToggled message to
// trigger
//-----------------------------------------------------------------------------
...
bool CEntityReportPanel::ShouldAddEntityToList(CDmeVMFEntity *pEntity) {
  ...
  */
  ...
  if (m_iFilterByType == FILTER_SHOW_BRUSH_ENTITIES &&
      !pEntity->IsPlaceholder())
    return false;
  ...
  for (CDmAttribute *pKey = pEntity->FirstEntityKey(); pKey;
       pKey = pEntity->NextEntityKey(pKey)) {
    // first, check key
    if (m_szFilterKey.IsEmpty() || !Q_stricmp(m_szFilterKey, pKey->GetName())) {
      ...
      if ((!m_bExact && Q_stristr(pValue, m_szFilterValue)) ||
          !Q_stricmp(pValue, m_szFilterValue))
        return true;
    }
  }
