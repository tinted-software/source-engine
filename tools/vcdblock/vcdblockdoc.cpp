...
// Constructor
//-----------------------------------------------------------------------------
CVcdBlockDoc::CVcdBlockDoc(IVcdBlockDocCallback *pCallback)
...
//-----------------------------------------------------------------------------
void CVcdBlockDoc::NotifyDataChanged(const char *pReason, int nNotifySource,
                                     int nNotifyFlags) {
...
bool CVcdBlockDoc::LoadFromFile(const char *pFileName) {
  ...
  CAppDisableUndoScopeGuard guard("CVcdBlockDoc::LoadFromFile",
                                  NOTIFY_CHANGE_OTHER);
  SetDirty(false);
  ...
  if (!V_stricmp(Q_GetFileExtension(pFileName), "vle")) {
    if (g_pDataModel->RestoreFromFile(m_pEditFileName, NULL, "vmf", &pEdit) !=
        DMFILEID_INVALID) {
...
void CVcdBlockDoc::SaveToFile() {
  if (m_hEditRoot.Get() && m_pEditFileName && m_pEditFileName[0]) {
    g_pDataModel->SaveToFile(m_pEditFileName, NULL, "keyvalues", "vmf",
                             m_hEditRoot);
  }
...
CDmAttribute *CVcdBlockDoc::GetEntityList() {
  return m_hEditRoot ? m_hEditRoot->GetAttribute("entities", AT_ELEMENT_ARRAY)
                     : NULL;
}
...
// Purpose:
//-----------------------------------------------------------------------------
void CVcdBlockDoc::AddNewInfoTarget(const Vector &vecOrigin,
                                    const QAngle &angAngles) {
  CDmrElementArray<> entities = GetEntityList();

  CDmeVMFEntity *pTarget;
  {
    CAppUndoScopeGuard guard(NOTIFY_SETDIRTYFLAG, "Add Info Target",
                             "Add Info Target");

    pTarget =
        CreateElement<CDmeVMFEntity>("", entities.GetOwner()->GetFileId());
...
void CVcdBlockDoc::DeleteInfoTarget(CDmeVMFEntity *pNode) {
  ...
  for (int i = 0; i < nCount; ++i) {
    if (pNode == entities[i]) {
      CAppUndoScopeGuard guard(NOTIFY_SETDIRTYFLAG, "Delete Info Target",
                               "Delete Info Target");
...
//-----------------------------------------------------------------------------
CDmeVMFEntity *CVcdBlockDoc::GetInfoTargetForLocation(Vector &vecOrigin,
                                                      QAngle &angAbsAngles) {
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    if (pNode->GetRenderOrigin().DistTo(vecOrigin) < 1e-3 &&
        vecAngles.DistTo(*(Vector *)&angAbsAngles) < 1e-1)
      return pNode;
  }
...
// Output : CDmeVMFEntity
//-----------------------------------------------------------------------------
CDmeVMFEntity *CVcdBlockDoc::GetInfoTargetForLocation(Vector &vecStart,
                                                      Vector &vecEnd) {
  Vector vecDelta;
  float flEndDist;
  ...
  for (int i = 0; i < nCount; ++i) {
    CDmeVMFEntity *pNode = CastElement<CDmeVMFEntity>(entities[i]);
    float flDistAway =
        DotProduct(pNode->GetRenderOrigin() - vecStart, vecDelta);

    if (flDistAway > 0.0 && flDistAway < flEndDist) {
      float flDistFromLine =
          (pNode->GetRenderOrigin() - vecStart - vecDelta * flDistAway)
              .Length();
...
//-----------------------------------------------------------------------------
bool CVcdBlockDoc::GetStringChoiceList(const char *pChoiceListType,
                                       CDmElement *pElement,
                                       const char *pAttributeName,
                                       bool bArrayElement,
                                       StringChoiceList_t &list) {
...
//-----------------------------------------------------------------------------
bool CVcdBlockDoc::GetElementChoiceList(const char *pChoiceListType,
                                        CDmElement *pElement,
                                        const char *pAttributeName,
                                        bool bArrayElement,
                                        ElementChoiceList_t &list) {
...
//-----------------------------------------------------------------------------
void CVcdBlockDoc::OnDataChanged(const char *pReason, int nNotifySource,
                                 int nNotifyFlags) {
...
//-----------------------------------------------------------------------------
static const char *s_pUseOriginalClasses[] = {"worldspawn", "func_occluder",
                                              NULL};
...
void CVcdBlockDoc::InitializeFromServer(CDmrElementArray<> &entityList) {
  ...
  while (pServerEnt) {
    ...
    if (servertools->GetKeyValue(pServerEnt, "classname", classname,
                                 sizeof(classname))) {
      if (!Q_stricmp(classname, "info_target")) {
        char hammerid[256];
        if (servertools->GetKeyValue(pServerEnt, "hammerid", hammerid,
                                     sizeof(hammerid))) {
          int nextId = CDmeVMFEntity::GetNextEntityId();
          CDmeVMFEntity::SetNextEntityId(atoi(hammerid));

          CDmeVMFEntity *pTarget = CreateElement<CDmeVMFEntity>(
              "", entityList.GetOwner()->GetFileId());
...
//-----------------------------------------------------------------------------
// Check the list of entities on the server against the edits that are already
// made
//-----------------------------------------------------------------------------
void CVcdBlockDoc::VerifyAllEdits(const CDmrElementArray<> &entityList) {
  ...
  for (int i = 0; i < entityList.Count(); i++) {
    ...
    CBaseEntity *pServerEntity =
        servertools->FindEntityByHammerID(pEntity->GetEntityId());
...
bool CVcdBlockDoc::CopyEditsToVMF() {
  ...
  if (g_pDataModel->RestoreFromFile(m_pVMFFileName, NULL, "vmf", &pVMF) ==
      DMFILEID_INVALID) {
  ...
  for (int j = 0; j < nCount; j++) {
    ...
    if (pEntity->IsDirty()) {
      CDmElement *pVMFEntity =
          CreateElement<CDmElement>(pEntity->GetName(), fileid);
  ...
  // currently, don't overwrite the vmf, not sure if this is serializing
  // correctly yet
  char tmpname[256];
