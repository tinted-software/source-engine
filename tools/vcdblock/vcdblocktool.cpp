...
const char *GetVGuiControlsModuleName() { return "VcdBlockTool"; }
...
bool ConnectTools(CreateInterfaceFn factory) {
  return (materials != NULL) && (g_pMatSystemSurface != NULL) &&
         (mdlcache != NULL) && (studiorender != NULL) &&
...
void CreateTools() { g_pVcdBlockTool = new CVcdBlockTool(); }
...
bool CVcdBlockTool::Init() {
  ...
  {
    m_hPreviewTarget =
        CreateElement<CDmeVMFEntity>("preview target", DMFILEID_INVALID);
...
//-----------------------------------------------------------------------------
inline CVcdBlockDoc *CVcdBlockTool::GetDocument() { return m_pDoc; }
...
//-----------------------------------------------------------------------------
bool CVcdBlockTool::IsMiniViewportCursor(int x, int y, Vector &org,
                                         Vector &forward) {
...
void CVcdBlockTool::QuickLoad(void) {
  ...
  float flFov;
  clienttools->GetLocalPlayerEyePosition(m_vecPlayerOrigin, m_vecPlayerAngles,
                                         flFov);
...
//
//-----------------------------------------------------------------------------
void CVcdBlockTool::QuickSave(void) { enginetools->Command("save quick\n"); }
...
#if 1
          CDmeVMFEntity *pSelection =
              m_pDoc->GetInfoTargetForLocation(tr.startpos, tr.endpos);
          if (pSelection) {
...
public:
  CVcdBlockViewMenuButton(CVcdBlockTool *parent, const char *panelName,
...
CVcdBlockViewMenuButton::CVcdBlockViewMenuButton(
    CVcdBlockTool *parent, const char *panelName, const char *text,
    vgui::Panel *pActionSignalTarget)
    : BaseClass(parent, panelName, text, pActionSignalTarget) {
  m_pTool = parent;

  AddCheckableMenuItem("properties", "#VcdBlockProperties",
                       new KeyValues("OnToggleProperties"),
                       pActionSignalTarget);
  AddCheckableMenuItem("entityreport", "#VcdBlockEntityReport",
                       new KeyValues("O|ÔglaEq”ípyOeport"),
                       pActionSignalTarget);

  AddSeparator();

  AddMenuItem("defaultlayout", "#VcdBlockViewDefault",
...
public:
  CVcdBlockToolMenuButton(CVcdBlockTool *parent, const char *panelName,
                          const char *text, vgui::Panel *pActionSignalTarget);
  virtual void OnShowMenu(vgui::Menu *menu);
...
CVcdBlockToolMenuButton::CVcdBlockToolMenuButton(
    CVcdBlockTool *parent, const char *panelName, const char *text,
    vgui::Panel *pActionSignalTarget)
    : BaseClass(parent, panelName, text, pActionSignalTarget) {
  m_pTool = parent;

  AddMenuItem("addnewnodes", "#VcdBlockAddNewNodes",
              new KeyValues("AddNewNodes"), pActionSignalTarget, NULL,
              "VcdBlockAddNewNodes");
  AddMenuItem("copyeditstovmf", "#VcdBlockCopyEditsToVMF",
              new KeyValues("CopyEditsToVMF"), pActionSignalTarget, NULL,
              "VcdBlockCopyEditsToVMF");

  AddSeparator();

  AddCheckableMenuItem("rememberposition", "#VcdBlockRememberPosition",
                       new KeyValues("RememberPosition"), pActionSignalTarget,
                       NULL, "VcdBlockRememberPosition");
...
vgui::MenuBar *CVcdBlockTool::CreateMenuBar(CBaseToolSystem *pParent) {
  ...
  CVcdBlockToolMenuButton *pToolButton = new CVcdBlockToolMenuButton(
      this, "VcdBlock", "&VcdBlock", GetActionTarget());
  CVcdBlockViewMenuButton *pViewButton =
      new CVcdBlockViewMenuButton(this, "View", "&View", GetActionTarget());
...
void CVcdBlockTool::OnDefaultLayout() {
  ...
  ToolWindow *pPropertyWindow = m_ToolWindowFactory.InstanceToolWindow(
      GetClientArea(), false, properties, "#VcdBlockProperties", false);
  ToolWindow *pEntityReportWindow = m_ToolWindowFactory.InstanceToolWindow(
      GetClientArea(), false, pEntityReport, "#VcdBlockEntityReport", false);
...
void CVcdBlockTool::CreateTools(CVcdBlockDoc *doc) {
  ...
  if (!m_hInfoTargetBrowser.Get()) {
    m_hInfoTargetBrowser =
        new CInfoTargetBrowserPanel(m_pDoc, this, "InfoTargetBrowserPanel");
  }
...
void CVcdBlockTool::InitTools() {
  ...
  if (!windowposmgr->LoadPositions("cfg/vcdblock.txt", this,
                                   &m_ToolWindowFactory, "VcdBlock")) {
    OnDefaultLayout();
  }
}
...
void CVcdBlockTool::ShowToolWindow(Panel *tool, char const *toolName,
                                   bool visible) {
  Assert(tool);
...
// Input  :  -
//-----------------------------------------------------------------------------
void CVcdBlockTool::OnExit() { enginetools->Command("quit\n"); }
...
bool CVcdBlockTool::OnReadFileFromDisk(const char *pFileName,
...
void CVcdBlockTool::RestartMap() { OnRestartLevel(); }

bool CVcdBlockTool::OnWriteFileToDisk(const char *pFileName,
                                      const char *pFileFormat,
                                      KeyValues *pContextKeyValues) {
  if (!m_pDoc)
    return true;
...
void CVcdBlockTool::OnClose() {
  if (m_pDoc && m_pDoc->IsDirty()) {
    SaveFile(m_pDoc->GetEditFileName(), "vle",
...
void CVcdBlockTool::OnCloseNoSave() {
  ...
  if (m_pDoc) {
    CAppNotifyScopeGuard sg("CVcdBlockTool::OnCloseNoSave",
                            NOTIFY_CHANGE_OTHER);
...
void CVcdBlockTool::AttachAllEngineEntities() {
  ...
  for ( EntitySearchResult sr = clienttools->FirstEntity(); sr != NULL; sr =
  clienttools->NextEntity( sr ) )
  {
          if ( !sr )
                  continue;
          ...
          if ( Q_strcmp( pClassName, "class C_InfoTarget" ) == 0 )
          {
                  ...
                  CDmeVMFEntity *pNode = m_pDoc->GetInfoTargetForLocation(
  vecOrigin, angAngles ); if ( pNode )
                  {
...
// saved/loaded
//-----------------------------------------------------------------------------
void CVcdBlockTool::OnFileOperationCompleted(
...
//-----------------------------------------------------------------------------
void CVcdBlockTool::SetupFileOpenDialog(vgui::FileOpenDialog *pDialog,
...
// Background
//-----------------------------------------------------------------------------
const char *CVcdBlockTool::GetLogoTextureName() { return NULL; }

//-----------------------------------------------------------------------------
// Inherited from IVcdBlockDocCallback
//-----------------------------------------------------------------------------
void CVcdBlockTool::OnDocChanged(const char *pReason, int nNotifySource,
                                 int nNotifyFlags) {
