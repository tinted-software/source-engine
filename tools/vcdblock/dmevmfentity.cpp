...
void CDmeVMFEntity::OnConstruction() {
  ...
  // m_TargetName.GetAttribute()->AddFlag( FATTRIB_DONTSAVE |
  // FATTRIB_HAS_CALLBACK ); m_vecLocalAngles.GetAttribute()->AddFlag(
...
//-----------------------------------------------------------------------------
int CDmeVMFEntity::GetEntityId() const { return atoi(GetName()); }

//-----------------------------------------------------------------------------
// Returns the next available entity id
//-----------------------------------------------------------------------------
int CDmeVMFEntity::GetNextEntityId() { return s_nNextEntityId; }
...
CDmAttribute *CDmeVMFEntity::NextEntityKey(CDmAttribute *pEntityKey) {
  ...
  for (CDmAttribute *pAttribute = pEntityKey->NextAttribute(); pAttribute;
       pAttribute = pAttribute->NextAttribute()) {
    if (IsEntityKey(pAttribute))
      return pAttribute;
  }
...
//-----------------------------------------------------------------------------
const Vector &CDmeVMFEntity::GetRenderOrigin(void) { return m_vecLocalOrigin; }
...
void CDmeVMFEntity::SetRenderAngles(const QAngle &angles) {
  m_vecLocalAngles.Set(
      Vector(angles.x, angles.y,
             angles.z)); // FIXME: angles is a vector due to the vmf "angles"
...
void CDmeVMFEntity::DrawFloorTarget(IMaterial *pMaterial) {
  ...
  CBaseTrace tr;
  ray.Init(m_vecLocalOrigin.Get() + Vector(0, 0, 10),
           m_vecLocalOrigin.Get() + Vector(0, 0, -128), Vector(-13, -13, 0),
           Vector(13, 13, 10));
...
bool CDmeVMFEntity::CopyFromServer(CBaseEntity *pServerEnt,
                                   const char *szSrcField,
                                   const char *szDstField) {
  char text[256];
...
bool CDmeVMFEntity::CopyToServer(void) {
  if (GetEntityId() != 0) {
    CBaseEntity *pServerEntity =
        servertools->FindEntityByHammerID(GetEntityId());
    if (pServerEntity != NULL) {
      ...
      Vector tmp(m_vecLocalAngles.Get().x, m_vecLocalAngles.Get().y,
                 m_vecLocalAngles.Get().z);
...
bool CDmeVMFEntity::IsSameOnServer(CBaseEntity *pServerEntity) {
  ...
  return (mapOrigin == m_vecLocalOrigin.Get() &&
          mapAngles == m_vecLocalAngles.Get());
}

bool CDmeVMFEntity::CreateOnServer(void) {
  CBaseEntity *pServerEntity =
      servertools->CreateEntityByName(m_ClassName.Get());
  ...
  if (pServerEntity) {
    ...
    CBaseTrace tr;
    ray.Init(m_vecLocalOrigin.Get() + Vector(0, 0, 10),
             m_vecLocalOrigin.Get() + Vector(0, 0, -1000));
    enginetools->TraceRayServer(ray, MASK_OPAQUE, &traceFilter, &tr);
