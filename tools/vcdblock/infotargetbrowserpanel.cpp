...
// Constructor
//-----------------------------------------------------------------------------
CInfoTargetBrowserPanel::CInfoTargetBrowserPanel(CVcdBlockDoc *pDoc,
                                                 vgui::Panel *pParent,
                                                 const char *pName)
...
CInfoTargetBrowserPanel::~CInfoTargetBrowserPanel() { SaveUserConfig(); }
...
void CInfoTargetBrowserPanel::OnProperties(void) {
  ...
  CDmeVMFEntity *pEntity =
      CastElement<CDmeVMFEntity>((CDmElement *)kv->GetPtr("entity"));
...
// Purpose:
//-----------------------------------------------------------------------------
void CInfoTargetBrowserPanel::OnItemSelected(void) { OnProperties(); }
...
void CInfoTargetBrowserPanel::SelectNode(CDmeVMFEntity *pNode) {
  ...
  for (int nItemID = m_pEntities->FirstItem();
       nItemID != m_pEntities->InvalidItemID();
...
void CInfoTargetBrowserPanel::OnCommand(const char *pCommand) {
  if (!Q_stricmp(pCommand, "delete")) {
    ...
    MessageBox *pConfirm =
        new MessageBox("#VcdBlockDeleteObjects", "#VcdBlockDeleteObjectsMsg",
                       g_pVcdBlockTool->GetRootPanel());
