...
// Constructor
//-----------------------------------------------------------------------------
CActBusyDoc::CActBusyDoc(IActBusyDocCallback *pCallback)
...
CActBusyDoc::~CActBusyDoc() { g_pDataModel->RemoveNotificationCallback(this); }

//-----------------------------------------------------------------------------
// Inherited from INotifyUI
//-----------------------------------------------------------------------------
void CActBusyDoc::NotifyDataChanged(const char *pReason, int nNotifySource,
                                    int nNotifyFlags) {
...
// Gets the file name
//-----------------------------------------------------------------------------
const char *CActBusyDoc::GetFileName() { return m_pFileName; }
...
// Dirty bits
//-----------------------------------------------------------------------------
void CActBusyDoc::SetDirty(bool bDirty) { m_bDirty = bDirty; }

bool CActBusyDoc::IsDirty() const { return m_bDirty; }
...
bool CActBusyDoc::LoadFromFile(const char *pFileName) {
  ...
  m_hRoot = root;
  OnDataChanged("CActBusyDoc::LoadFromFile", NOTIFY_SOURCE_APPLICATION,
...
void CActBusyDoc::SaveToFile() {
  if (m_hRoot.Get() && m_pFileName && m_pFileName[0]) {
    g_pDataModel->SaveToFile(m_pFileName, NULL, "keyvalues", "actbusy",
                             m_hRoot);
  }
...
void CActBusyDoc::CreateActBusy() {
  ...
  // Create the main element
  CDmeHandle<CDmElement> hActBusy =
      g_pDataModel->CreateElement("DmElement", "ActBusy", fileid);
...
//-----------------------------------------------------------------------------
CDmElement *CActBusyDoc::GetRootObject() { return m_hRoot; }

//-----------------------------------------------------------------------------
// Called when data changes
//-----------------------------------------------------------------------------
void CActBusyDoc::OnDataChanged(const char *pReason, int nNotifySource,
                                int nNotifyFlags) {
