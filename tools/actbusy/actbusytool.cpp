...
//-----------------------------------------------------------------------------
const char *GetVGuiControlsModuleName() { return "ActBusyTool"; }
...
//-----------------------------------------------------------------------------
class CActBusyTool : public CBaseToolSystem,
                     public IFileMenuCallbacks,
                     public IActBusyDocCallback {
...
public:
  ...
  virtual bool OnWriteFileToDisk(const char *pFileName, const char *pFileFormat,
                                 KeyValues *pContextKeyValues);
  virtual void
  OnFileOperationCompleted(const char *pFileType, bool bWroteFile,
...
void CreateTools() { g_pActBusyTool = new CActBusyTool(); }
...
//-----------------------------------------------------------------------------
void CActBusyTool::OnToolActivate() { BaseClass::OnToolActivate(); }

void CActBusyTool::OnToolDeactivate() { BaseClass::OnToolDeactivate(); }

//-----------------------------------------------------------------------------
// Hides standard fields
//-----------------------------------------------------------------------------
void CActBusyTool::HideStandardFields(CDmeEditorType *pEditorType,
                                      int nFieldFlags) {
  CDmeEditorAttributeInfo *pInfo;

  if (nFieldFlags & EDITOR_FIELD_NAME) {
    pInfo =
        CreateElement<CDmeEditorAttributeInfo>("name info", DMFILEID_INVALID);
  ...
  if (nFieldFlags & EDITOR_FIELD_TYPE) {
    pInfo =
        CreateElement<CDmeEditorAttributeInfo>("type info", DMFILEID_INVALID);
    pInfo->m_bIsVisible = false;
...
void CActBusyTool::InitEditorDict() {
  ...
  CDmeEditorType *pActBusyList =
      CreateElement<CDmeEditorType>("actBusyList", DMFILEID_INVALID);
  HideStandardFields(pActBusyList, EDITOR_FIELD_NAME | EDITOR_FIELD_TYPE |
                                       EDITOR_FIELD_ID |
                                       EDITOR_FIELD_EDITORTYPE);
  g_pEditorTypeDict->AddEditorType(pActBusyList);
  m_toolElements.AddToTail(pActBusyList->GetHandle());

  pInfo =
      CreateElement<CDmeEditorAttributeInfo>("children info", DMFILEID_INVALID);
  pActBusyList->AddAttributeInfo("children", pInfo);
  m_toolElements.AddToTail(pInfo->GetHandle());
  pArrayInfo = CreateElement<CDmeEditorAttributeInfo>("hide text info",
                                                      DMFILEID_INVALID);
  pInfo->SetArrayInfo(pArrayInfo);
  pArrayInfo->SetValue("hideText", true);
  m_toolElements.AddToTail(pArrayInfo->GetHandle());

  CDmeEditorType *pActBusyType =
      CreateElement<CDmeEditorType>("actBusy", DMFILEID_INVALID);
  HideStandardFields(pActBusyType, EDITOR_FIELD_TYPE | EDITOR_FIELD_ID |
                                       EDITOR_FIELD_EDITORTYPE);
  m_toolElements.AddToTail(pActBusyType->GetHandle());

  // anims only accept activity names
  pInfo = CreateElement<CDmeEditorAttributeInfo>("busy anim info",
                                                 DMFILEID_INVALID);
  ...
  pInfo = CreateElement<CDmeEditorAttributeInfo>("entry anim info",
                                                 DMFILEID_INVALID);
  ...
  pInfo = CreateElement<CDmeEditorAttributeInfo>("exit anim info",
                                                 DMFILEID_INVALID);
  ...
  pInfo = CreateElement<CDmeEditorAttributeInfo>("busy sequence info",
                                                 DMFILEID_INVALID);
  ...
  pInfo = CreateElement<CDmeEditorAttributeInfo>("entry sequence info",
                                                 DMFILEID_INVALID);
  ...
  pInfo = CreateElement<CDmeEditorAttributeInfo>("exit sequence info",
                                                 DMFILEID_INVALID);
  ...
  CDmeEditorStringChoicesInfo *pChoicesInfo =
      CreateElement<CDmeEditorStringChoicesInfo>("interrupts info",
                                                 DMFILEID_INVALID);
  pActBusyType->AddAttributeInfo("interrupts", pChoicesInfo);
  pChoicesInfo->m_Widget = "stringchoice";
  m_toolElements.AddToTail(
      pChoicesInfo->AddChoice("BA_INT_NONE", "No Interrupts")->GetHandle());
  m_toolElements.AddToTail(
      pChoicesInfo->AddChoice("BA_INT_DANGER", "Danger")->GetHandle());
  m_toolElements.AddToTail(
      pChoicesInfo->AddChoice("BA_INT_PLAYER", "Player")->GetHandle());
  m_toolElements.AddToTail(
      pChoicesInfo->AddChoice("BA_INT_AMBUSH", "Ambush")->GetHandle());
  m_toolElements.AddToTail(
      pChoicesInfo->AddChoice("BA_INT_COMBAT", "Combat")->GetHandle());
  m_toolElements.AddToTail(
      pChoicesInfo->AddChoice("BA_INT_ZOMBIESLUMP", "Zombie Slump")
          ->GetHandle());
...
public:
  CActBusyViewMenuButton(CActBusyTool *parent, const char *panelName,
...
CActBusyViewMenuButton::CActBusyViewMenuButton(CActBusyTool *parent,
  ...
  AddCheckableMenuItem(
      "properties", "#ActBusyProperties",
      new KeyValues("Command", "command", "OnToggleProperties"),
      pActionSignalTarget);
  AddCheckableMenuItem(
      "picker", "#ActBusyViewSequencePicker",
      new KeyValues("Command", "command", "OnToggleSequencePicker"),
      pActionSignalTarget);

  AddSeparator();
  AddMenuItem("defaultlayout", "#ActBusyViewDefault",
              new KeyValues("Command", "command", "OnDefaultLayout"),
              pActionSignalTarget);
...
public:
  CActBusyMenuButton(CActBusyTool *parent, const char *panelName,
                     const char *text, vgui::Panel *pActionSignalTarget);
  virtual void OnShowMenu(vgui::Menu *menu);
...
CActBusyMenuButton::CActBusyMenuButton(CActBusyTool *parent,
                                       const char *panelName, const char *text,
                                       vgui::Panel *pActionSignalTarget)
    : BaseClass(parent, panelName, text, pActionSignalTarget) {
  m_pTool = parent;

  AddMenuItem("NewActBusy", "#ActBusyNewActBusy",
              new KeyValues("Command", "command", "OnNewActBusy"),
              pActionSignalTarget);
  AddMenuItem("DeleteActBusy", "#ActBusyDeleteActBusy",
              new KeyValues("Command", "command", "OnDeleteActBusy"),
              pActionSignalTarget);
...
vgui::MenuBar *CActBusyTool::CreateMenuBar(CBaseToolSystem *pParent) {
  ...
  CToolMenuButton *pEditButton =
      CreateToolEditMenuButton(this, "Edit", "&Edit", GetActionTarget());
  CActBusyMenuButton *pActBusyButton =
      new CActBusyMenuButton(this, "ActBusy", "&ActBusy", GetActionTarget());
  CToolMenuButton *pSwitchButton = CreateToolSwitchMenuButton(
      m_pMenuBar, "Switcher", "&Tools", GetActionTarget());
  CActBusyViewMenuButton *pViewButton =
      new CActBusyViewMenuButton(this, "View", "&View", GetActionTarget());
...
// saved/loaded
//-----------------------------------------------------------------------------
void CActBusyTool::OnFileOperationCompleted(
...
//-----------------------------------------------------------------------------
void CActBusyTool::SetupFileOpenDialog(vgui::FileOpenDialog *pDialog,
                                       bool bOpenFile, const char *pFileFormat,
                                       KeyValues *pContextKeyValues) {
...
//-----------------------------------------------------------------------------
bool CActBusyTool::OnReadFileFromDisk(const char *pFileName,
                                      const char *pFileFormat,
                                      KeyValues *pContextKeyValues) {
  OnCloseNoSave();
...
// Called by SaveFile to allow clients to actually write the file out
//-----------------------------------------------------------------------------
bool CActBusyTool::OnWriteFileToDisk(const char *pFileName,
                                     const char *pFileFormat,
                                     KeyValues *pContextKeyValues) {
  if (!m_pDoc)
    return true;
...
void CActBusyTool::OnNew() {
  if (m_pDoc) {
    if (m_pDoc->IsDirty()) {
      SaveFile(m_pDoc->GetFileName(), "actbusy",
...
// Inherited from IActBusyDocCallback
//-----------------------------------------------------------------------------
void CActBusyTool::OnDocChanged(const char *pReason, int nNotifySource,
                                int nNotifyFlags) {
  UpdateMenuBar();
...
// Output : CActBusyDoc
//-----------------------------------------------------------------------------
CActBusyDoc *CActBusyTool::GetDocument() { return m_pDoc; }
...
void CActBusyTool::InitTools() {
  ...
  if (!windowposmgr->LoadPositions("cfg/actbusy.txt", this,
                                   &m_ToolWindowFactory, "ActBusy")) {
    OnDefaultLayout();
  }
}
...
void CActBusyTool::ShowToolWindow(Panel *tool, const char *toolName,
                                  bool visible) {
  Assert(tool);
...
void CActBusyTool::OnDefaultLayout() {
  ...
  ToolWindow *propertyWindow = m_ToolWindowFactory.InstanceToolWindow(
      GetClientArea(), false, properties, "#ActBusyProperties", false);
  ToolWindow *pickerWindow = m_ToolWindowFactory.InstanceToolWindow(
      GetClientArea(), false, picker, "#ActBusyViewSequencePicker", false);
