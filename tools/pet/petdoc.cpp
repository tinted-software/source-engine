...
//-----------------------------------------------------------------------------
void CPetDoc::NotifyDataChanged(const char *pReason, int nNotifySource,
                                int nNotifyFlags) {
  OnDataChanged(pReason, nNotifySource, nNotifyFlags);
}

bool CPetDoc::GetIntChoiceList(const char *pChoiceListType,
...
// Gets the file name
//-----------------------------------------------------------------------------
const char *CPetDoc::GetFileName() { return m_pFileName; }
...
// Dirty bits
//-----------------------------------------------------------------------------
void CPetDoc::SetDirty(bool bDirty) { m_bDirty = bDirty; }

bool CPetDoc::IsDirty() const { return m_bDirty; }
...
bool CPetDoc::CreateRootElement() {
  ...
  // We need to create an element array attribute storing particle system
  // definitions
...
void CPetDoc::SaveToFile() {
  if (m_hRoot.Get() && m_pFileName && m_pFileName[0]) {
    g_pDataModel->SaveToFile(m_pFileName, NULL, "binary", PET_FILE_FORMAT,
                             m_hRoot);
  }
...
//-----------------------------------------------------------------------------
CDmElement *CPetDoc::GetRootObject() { return m_hRoot; }
...
void CPetDoc::AddNewParticleSystemDefinition(CDmeParticleSystemDefinition *pNew,
...
CPetDoc::AddNewParticleSystemDefinition(const char *pName) {
  if (!pName || !pName[0]) {
  ...
  {
    CAppUndoScopeGuard guard(NOTIFY_SETDIRTYFLAG, "Add Particle System",
                             "Add Particle System");

    pParticleSystem =
        CreateElement<CDmeParticleSystemDefinition>(pName, GetFileId());
...
//-----------------------------------------------------------------------------
void CPetDoc::DeleteParticleSystemDefinition(
    CDmeParticleSystemDefinition *pParticleSystem) {
  if (!pParticleSystem)
    return;
  ...
  for (int i = 0; i < nCount; ++i) {
    if (pParticleSystem == particleSystemList[i]) {
      CAppUndoScopeGuard guard(NOTIFY_SETDIRTYFLAG, "Delete Particle System",
  ...
  for (int i = 0; i < nChildCount; ++i) {
    ...
    CDmeParticleSystemDefinition *pParent =
        FindReferringElement<CDmeParticleSystemDefinition>(pChildReference,
                                                           "children");
    if (!pParent)
      continue;
...
CDmeParticleSystemDefinition *
CPetDoc::FindParticleSystemDefinition(const char *pName) {
  CDmrParticleSystemList particleSystemList(GetParticleSystemDefinitionList());
  int nCount = particleSystemList.Count();
  for (int i = 0; i < nCount; ++i) {
...
// Deletes a particle system definition
//-----------------------------------------------------------------------------
void CPetDoc::ReplaceParticleSystemDefinition(
    CDmeParticleSystemDefinition *pParticleSystem) {
  if (!pParticleSystem)
    return;
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    if (!Q_stricmp(particleSystemList[i]->GetName(),
                   pParticleSystem->GetName())) {
      nFoundIndex = i;
      break;
    }
  }
  ...
  if (nFoundIndex < 0) {
    CAppUndoScopeGuard guard(NOTIFY_SETDIRTYFLAG, "Replace Particle System",
                             "Replace Particle System");
    CDmrParticleSystemList particleSystemList(
        GetParticleSystemDefinitionList());
    pParticleSystem->SetFileId(m_hRoot->GetFileId(), TD_ALL);
    particleSystemList.AddToTail(pParticleSystem);
    return;
  }

  CDmeParticleSystemDefinition *pOldParticleSystem =
      particleSystemList[nFoundIndex];
...
void CPetDoc::UpdateParticleDefinition(CDmeParticleSystemDefinition *pDef) {
  ...
  if (clienttools) {
    clienttools->ReloadParticleDefintions(GetFileName(), buf.Base(),
                                          buf.TellMaxPut());
  }
  if (servertools) {
    servertools->ReloadParticleDefintions(GetFileName(), buf.Base(),
                                          buf.TellMaxPut());
  }
...
//-----------------------------------------------------------------------------
bool CPetDoc::GetStringChoiceList(const char *pChoiceListType,
                                  CDmElement *pElement,
...
//-----------------------------------------------------------------------------
bool CPetDoc::GetElementChoiceList(const char *pChoiceListType,
                                   CDmElement *pElement,
                                   const char *pAttributeName,
                                   bool bArrayElement,
...
//-----------------------------------------------------------------------------
void CPetDoc::OnDataChanged(const char *pReason, int nNotifySource,
                            int nNotifyFlags) {
