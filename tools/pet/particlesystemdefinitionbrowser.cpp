...
//-----------------------------------------------------------------------------
static int __cdecl ParticleSystemNameSortFunc(vgui::ListPanel *pPanel,
...
CParticleSystemDefinitionBrowser::CParticleSystemDefinitionBrowser(
    CPetDoc *pDoc, vgui::Panel *pParent, const char *pName)
    : BaseClass(pParent, pName), m_pDoc(pDoc) {
  SetKeyBoardInputEnabled(true);
  SetPaintBackgroundEnabled(true);

  m_pParticleSystemsDefinitions = new vgui::ListPanel(this, "ParticleSystems");
  m_pParticleSystemsDefinitions->AddColumnHeader(
      0, "name", "Name", 52, ListPanel::COLUMN_RESIZEWITHWINDOW);
  m_pParticleSystemsDefinitions->SetColumnSortable(0, true);
  m_pParticleSystemsDefinitions->SetEmptyListText(
      "No Particle System Definitions");
...
CParticleSystemDefinitionBrowser::GetSelectedParticleSystem(int i) {
...
void CParticleSystemDefinitionBrowser::DeleteParticleSystems() {
  ...
  {
    // This is undoable
    CAppUndoScopeGuard guard(NOTIFY_SETDIRTYFLAG, "Delete Particle Systems",
...
//-----------------------------------------------------------------------------
void CParticleSystemDefinitionBrowser::LoadKVSection(
    CDmeParticleSystemDefinition *pNew, KeyValues *pOverridesKv,
    ParticleFunctionType_t eType) {
  // Operator KV
  KeyValues *pOperator =
      pOverridesKv->FindKey(GetParticleFunctionTypeName(eType), NULL);
  if (!pOperator)
    return;
  ...
  FOR_EACH_TRUE_SUBKEY(pOperator, pFunctionBlock) {
    ...
    if (iFunction >= 0) {
      CDmeParticleFunction *pDmeFunction =
          pNew->GetParticleFunction(eType, iFunction);
      // Elements
      FOR_EACH_SUBKEY(pFunctionBlock, pAttributeItem) {
        CDmAttribute *pAttribute =
            pDmeFunction->GetAttribute(pAttributeItem->GetName());
        if (!pAttribute) {
          Warning("Unable to Find Attribute [%s] in Function [%s] in Operator "
                  "[%s] for Definition [%s]\n",
                  pAttributeItem->GetName(), pFunctionBlock->GetName(),
      ...
      Warning(
          "Function [%s] not found under Operator [%s] for Definition [%s]\n",
          pFunctionBlock->GetName(), GetParticleFunctionTypeName(eType),
          pNew->GetName());
    }
  }
}
...
//-----------------------------------------------------------------------------
CDmeParticleSystemDefinition *
CParticleSystemDefinitionBrowser::CreateParticleFromKV(KeyValues *pKeyValue) {
  ...
  for (int i = 0; i < m_pParticleSystemsDefinitions->GetItemCount(); ++i) {
    ...
    if (!V_strcmp(kv->GetString("name", ""), pBaseParticleName)) {
      //
      pBaseParticleDef = GetElementKeyValue<CDmeParticleSystemDefinition>(
          kv, "particleSystem");
      break;
    }
  }
  ...
  CDmeParticleSystemDefinition *pNew =
      CastElement<CDmeParticleSystemDefinition>(pBaseParticleDef->Copy());
...
//-----------------------------------------------------------------------------
// Create from KV
void CParticleSystemDefinitionBrowser::CreateParticleSystemsFromKV(
  ...
  CUtlBuffer bufText(bufRawData.Base(), bufRawData.TellPut(),
...
//-----------------------------------------------------------------------------
void CParticleSystemDefinitionBrowser::SelectParticleSystem(
    CDmeParticleSystemDefinition *pFind) {
  m_pParticleSystemsDefinitions->ClearSelectedItems();
...
void CParticleSystemDefinitionBrowser::OnInputCompleted(KeyValues *pKeyValues) {
  ...
  if (m_pDoc->IsParticleSystemDefined(pText)) {
    ...
    vgui::MessageBox *pMessageBox = new vgui::MessageBox(
        "Duplicate Particle System Name!\n", pBuf, g_pPetTool->GetRootPanel());
    pMessageBox->DoModal();
    return;
  }
  ...
  if (pKeyValues->FindKey("create")) {
    CDmeParticleSystemDefinition *pParticleSystem =
        m_pDoc->AddNewParticleSystemDefinition(pText);
...
void CParticleSystemDefinitionBrowser::CopyToClipboard() {
  ...
  CUtlRBTree<CDmeParticleSystemDefinition *> defs(
      0, 0, DefLessFunc(CDmeParticleSystemDefinition *));
  for (int i = 0; i < nCount; ++i) {
    CDmeParticleSystemDefinition *pParticleSystem =
        GetSelectedParticleSystem(i);

    CUtlBuffer buf(0, 0, CUtlBuffer::TEXT_BUFFER);
    if (g_pDataModel->Serialize(buf, "keyvalues2", "pcf",
                                pParticleSystem->GetHandle())) {
...
//-----------------------------------------------------------------------------
void CParticleSystemDefinitionBrowser::ReplaceDef_r(
    CUndoScopeGuard &guard, CDmeParticleSystemDefinition *pDef) {
  if (!pDef)
    return;
  ...
  for (int i = 0; i < nChildCount; ++i) {
    CDmeParticleChild *pChildFunction = static_cast<CDmeParticleChild *>(
        pDef->GetParticleFunction(FUNCTION_CHILDREN, i));
...
void CParticleSystemDefinitionBrowser::PasteFromClipboard() {
  // This is undoable
  CAppUndoScopeGuard guard(NOTIFY_SETDIRTYFLAG, "Paste From Clipboard",
                           "Paste From Clipboard");
  ...
  for (int i = 0; i < nItems; ++i) {
    ...
    CUtlBuffer buf(pData, nLen,
                   CUtlBuffer::TEXT_BUFFER | CUtlBuffer::READ_ONLY);

    DmElementHandle_t hRoot;
    if (!g_pDataModel->Unserialize(buf, "keyvalues2", "pcf", NULL, "paste",
                                   CR_FORCE_COPY, hRoot))
      continue;

    CDmeParticleSystemDefinition *pDef =
        GetElement<CDmeParticleSystemDefinition>(hRoot);
    if (!pDef)
      continue;
...
void CParticleSystemDefinitionBrowser::OnCommand(const char *pCommand) {
  if (!Q_stricmp(pCommand, "create")) {
    vgui::InputDialog *pInputDialog = new vgui::InputDialog(
        g_pPetTool->GetRootPanel(), "Enter Particle System Name", "Name:", "");
  ...
  if (!Q_stricmp(pCommand, "Create From KeyValue")) {
    vgui::FileOpenDialog *pDialog = new vgui::FileOpenDialog(
        g_pPetTool->GetRootPanel(), "Select KV File", vgui::FOD_OPEN);
    pDialog->SetTitle("Choose KeyValue File", true);
    pDialog->AddFilter("*.txt", "KeyValue File (*.txt)", true);
    pDialog->AddActionSignalTarget(this);

    char szParticlesDir[MAX_PATH];
    pDialog->SetStartDirectory(g_pFullFileSystem->RelativePathToFullPath(
        "particles", "MOD", szParticlesDir, sizeof(szParticlesDir)));
...
void CParticleSystemDefinitionBrowser::UpdateParticleSystemList(void) {
  const CDmrParticleSystemList particleSystemList =
      m_pDoc->GetParticleSystemDefinitionList();
