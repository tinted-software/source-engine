...
bool CBaseToolSystem::ServerInit(CreateInterfaceFn serverFactory) {
  ...
  if (!servertools) {
    Error("CBaseToolSystem::PostInit:  Unable to get '%s' interface from game "
          ".dll\n",
...
bool CBaseToolSystem::ClientInit(CreateInterfaceFn clientFactory) {
  ...
  if (!clienttools) {
    Error("CBaseToolSystem::PostInit:  Unable to get '%s' interface from "
          "client .dll\n",
          VCLIENTTOOLS_INTERFACE_VERSION);
  } else {
    g_pGlobalFlexController =
        &g_GlobalFlexController; // don't set this until clienttools is
                                 // connected
  }
...
void CBaseToolSystem::OnToolActivate() {
  ...
  // FIXME: Note that this is necessary because IsGameInputEnabled depends on
  // m_bIsActive at the moment
  OnModeChanged();
