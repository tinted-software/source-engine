...
void GetMDLBoundingBox(Vector *pMins, Vector *pMaxs, MDLHandle_t h,
  ...
  } else if (!VectorCompare(vec3_origin, pStudioHdr->hull_min) ||
             !VectorCompare(vec3_origin, pStudioHdr->hull_max)) {
    // look for hull
...
void CMDL::UnreferenceMDL() {
  ...
  if (m_MDLHandle != MDLHANDLE_INVALID) {
    // XXX need to figure out where it is safe to flush the queue during map
    // change to not crash
#if 0
...
void CMDL::Draw(const matrix3x4_t &rootToWorld,
  ...
  Vector white(m_Color.r() / 255.0f, m_Color.g() / 255.0f,
               m_Color.b() / 255.0f);
  ...
  if (nFlexDescCount) {
    ...
    g_pStudioRender->LockFlexWeights(info.m_pStudioHdr->numflexdesc,
                                     &pFlexWeights);
  ...
  g_pStudioRender->DrawModel(
      NULL, info, const_cast<matrix3x4_t *>(pBoneToWorld), pFlexWeights, NULL,
...
void CMDL::SetUpBones(const matrix3x4_t &rootToWorld, int nMaxBoneCount,
                      matrix3x4_t *pBoneToWorld, const float *pPoseParameters,
                      MDLSquenceLayer_t *pSequenceLayers,
                      int nNumSequenceLayers) {
  ...
  IBoneSetup boneSetup(&studioHdr, BONE_USED_BY_ANYTHING_AT_LOD(m_nLOD),
                       pPoseParameter, NULL);
  ...
  if (pSequenceLayers) {
    ...
    for (int i = 0; i < nNumSequenceLayers; ++i) {
      ...
      if ((nSeqIndex >= 0) && (nSeqIndex < nNumSeq)) {
        ...
        float flLayerCycle;
        int nLayerFrameCount =
            MAX(1, Studio_MaxFrame(&studioHdr, nSeqIndex, pPoseParameter));
        ...
        if (pSequenceLayers[i].m_bNoLoop) {
          ...
          // FIXME: We're always wrapping; may want to determing if we should
          // clamp
          flLayerCycle -= (int)(flLayerCycle);
        }

        boneSetup.AccumulatePose(pos, q, nSeqIndex, flLayerCycle, flWeight,
                                 m_flTime, NULL);
      }
    }
  }
  ...
  //BONE_USED_BY_VERTEX_AT_LOD( m_nLOD ), flTime );
  ...
  Studio_RunBoneFlexDrivers(m_pFlexControls, &studioHdr, pos, pBoneToWorld,
                            rootToWorld);
}
...
// Purpose:
//-----------------------------------------------------------------------------
void CMDL::SetupBonesWithBoneMerge(const CStudioHdr *pMergeHdr,
  ...
  for (int i = 0; i < MAXSTUDIOPOSEPARAM; ++i) {
    ...
    if (i < nPoseCount) {
      const mstudioposeparamdesc_t &Pose =
          ((CStudioHdr *)pMergeHdr)->pPoseParameter(i);
  ...
  IBoneSetup boneSetup(pMergeHdr, BONE_USED_BY_ANYTHING_AT_LOD(m_nLOD),
                       pPoseParameter);
  boneSetup.InitPose(pos, q);
  boneSetup.AccumulatePose(pos, q, m_nSequence, flCycle, 1.0f, m_flTime, NULL);
  ...
  for (int iMergeBone = 0; iMergeBone < pMergeHdr->numbones(); ++iMergeBone) {
    ...
    int iParentBoneIndex =
        Studio_BoneIndexByName(pFollow, pMergeBones[iMergeBone].pszName());
    if (iParentBoneIndex >= 0) {
      MatrixCopy(pFollowBoneToWorld[iParentBoneIndex],
    ...
    if (!bMerged) {
      ...
      if (pMergeBones[iMergeBone].parent == -1) {
        ConcatTransforms(matModelToWorld, matBone,
                         pMergeBoneToWorld[iMergeBone]);
      } else {
        ConcatTransforms(pMergeBoneToWorld[pMergeBones[iMergeBone].parent],
                         matBone, pMergeBoneToWorld[iMergeBone]);
      }
    }
  }
}
