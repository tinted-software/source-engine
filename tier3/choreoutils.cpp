...
bool AutoAddGestureKeys(CChoreoEvent *e, CStudioHdr *pStudioHdr,
  ...
  if (!pSeqKeyValues->LoadFromBuffer(
          pStudioHdr->pszName(),
          Studio_GetKeyValueText(pStudioHdr, iSequence))) {
...
bool UpdateSequenceLength(CChoreoEvent *e, CStudioHdr *pStudioHdr,
  ...
  if (bLooping) {
    if (e->IsFixedLength()) {
      ...
      if (bVerbose) {
        ConMsg("UpdateSequenceLength:  %s is looping, removing fixed length "
               "flag\n",
               e->GetName());
      }
      bChanged = true;
  ...
  if (e->HasEndTime()) {
    ...
    if (fabs(dt - flSeqDuration) > 0.01f) {
      if (bCheckOnly)
        return true;
      if (bVerbose) {
        ConMsg("CheckSequenceLength:  %s has wrong duration, changing length "
               "from %f to %f seconds\n",
    ...
    if (bCheckOnly)
      return true;
    if (bVerbose) {
      ConMsg("CheckSequenceLength:  %s has wrong duration, changing length to "
             "%f seconds\n",
...
const char *GetSoundForEvent(CChoreoEvent *pEvent, CStudioHdr *pStudioHdr) {
  ...
  const char *pFileName = g_pSoundEmitterSystem->GetWavFileForSound(
      pSoundName,
      (pStudioHdr && pStudioHdr->IsValid()) ? pStudioHdr->pszName() : NULL);
