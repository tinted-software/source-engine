...
static mach_error_t allocateBranchIsland(BranchIsland **island,
                                         void *originalFunctionAddress);

mach_error_t freeBranchIsland(BranchIsland *island);

#if defined(__ppc__) || defined(__POWERPC__)
mach_error_t setBranchIslandTarget(BranchIsland *island, const void *branchTo,
                                   long instruction);
#endif

#if defined(__i386__) || defined(__x86_64__)
mach_error_t setBranchIslandTarget_i386(BranchIsland *island,
                                        const void *branchTo,
                                        char *instructions);
void atomic_mov64(uint64_t *targetAddress, uint64_t value);

static Boolean eatKnownInstructions(unsigned char *code,
                                    uint64_t *newInstruction, int *howManyEaten,
                                    char *originalInstructions,
                                    int *originalInstructionCount,
                                    uint8_t *originalInstructionSizes);

static void fixupInstructions(uint32_t offset, void *instructionsToFix,
                              int instructionCount, uint8_t *instructionSizes);
#endif
...
#define kMFCTRInstruction 0x7c0903a6
  ...
  Boolean overridePossible = eatKnownInstructions(
      (unsigned char *)originalFunctionPtr, &jumpRelativeInstruction,
      &eatenCount, originalInstructions, &originalInstructionCount,
  ...
  if (!err) {
    err = vm_protect(mach_task_self(), (vm_address_t)originalFunctionPtr, 8,
                     false, (VM_PROT_ALL | VM_PROT_COPY));
    if (err)
      err = vm_protect(mach_task_self(), (vm_address_t)originalFunctionPtr, 8,
                       false, (VM_PROT_DEFAULT | VM_PROT_COPY));
  }
  if (err)
    fprintf(stderr, "err = %x %s:%d\n", err, __FILE__, __LINE__);
  ...
  //	Optionally allocate & return the reentry island. This may contain
  //relocated
  //  jmp instructions and so has all the same addressing reachability
  //  requirements the escape island has to the original function, except the
  //  escape island is technically our original function.
  ...
  if (!err) {
    ...
    do {
      ...
      if (!err) {
        escapeIslandEngaged =
            CompareAndSwap(originalInstruction, branchAbsoluteInstruction,
  ...
  //
  // Note that on i386, we do not support someone else changing the code under
  // our feet
  if (!err) {
    uint32_t offset = (uintptr_t)originalFunctionPtr - (uintptr_t)reentryIsland;
    fixupInstructions(offset, originalInstructions, originalInstructionCount,
                      originalInstructionSizes);

    if (reentryIsland)
      err = setBranchIslandTarget_i386(
          reentryIsland, (void *)((char *)originalFunctionPtr + eatenCount),
...
static mach_error_t allocateBranchIslandAux(BranchIsland **island,
                                            void *originalFunctionAddress,
                                            bool forward) {
  assert(island);
...
/*******************************************************************************
        ...
        @param	instruction	->	Optional instruction to execute prior to
   branch. Set to zero for nop.
        @result				<-	mach_error_t
...
static Boolean eatKnownInstructions(unsigned char *code,
  ...
  while (remainsToEat > 0) {
    ...
    // if all instruction matches failed, we don't know current instruction
    // then, stop here
    if (!curInstructionKnown) {
      allInstructionsKnown = false;
      fprintf(stderr, "mach_override: some instructions unknown! Need to "
                      "update mach_override.c\n");
      break;
    }
  ...
  if (originalInstructions) {
    ...
    if (enoughSpaceForOriginalInstructions) {
      ...
      // printf ("Not enough space in island to store original instructions.
      // Adapt the island definition and kOriginalInstructionsSize\n");
      return false;
    }
  }
  ...
  if (allInstructionsKnown) {
    ...
    // keep only last 3 instructions bytes, first 5 will be replaced by JMP
    // instr
...
void atomic_mov64(uint64_t *targetAddress, uint64_t value) {
  while (true) {
    ...
    if (OSAtomicCompareAndSwap64(old_value, value, (int64_t *)targetAddress))
      return;
  }
}
#elif defined(__x86_64__)
