...
static ConVar
    mod_forcedata("mod_forcedata", (AsyncMdlCache()) ? "0" : "1", 0,
...
struct AsyncInfo_t {
  AsyncInfo_t()
      : hControl(NULL), hModel(MDLHANDLE_INVALID), type(MDLCACHE_NONE),
        iAnimBlock(0) {}
...
inline intp GetAsyncInfoIndex(MDLHandle_t hModel, MDLCacheDataType_t type,
                              int iAnimBlock = 0) {
...
inline intp SetAsyncInfoIndex(MDLHandle_t hModel, MDLCacheDataType_t type,
                              int iAnimBlock, intp index) {
  AUTO_LOCK(g_AsyncInfoMapMutex);
...
private:
  ...
  void MakeFilename(MDLHandle_t handle, const char *pszExtension,
                    char *pszFileName, int nMaxLength);
  ...
  virtual bool
  HandleCacheNotification(const DataCacheNotification_t &notification);
  ...
  bool ReadFileNative(char *pFileName, const char *pPath, CUtlBuffer &buf,
                      int nMaxBytes = 0,
                      MDLCacheDataType_t type = MDLCACHE_NONE);
...
// Constructs a filename based on a model handle
//-----------------------------------------------------------------------------
void CMDLCache::MakeFilename(MDLHandle_t handle, const char *pszExtension,
                             char *pszFileName, int nMaxLength) {
...
//-----------------------------------------------------------------------------
void CMDLCache::NotifyFileUnloaded(MDLHandle_t handle,
...
void CMDLCache::UnserializeVCollide(MDLHandle_t handle, bool synchronousLoad) {
  ...
  if (iAsync == NO_ASYNC) {
      ...
      if (pVirtualModel) {
        for (int i = 1; i < pVirtualModel->m_group.Count(); i++) {
          MDLHandle_t sharedHandle =
              VoidPtrToMDLHandle(pVirtualModel->m_group[i].cache);
    ...
    if (IsX360()) {
      ...
      UpdateOrCreate(NULL, pFileName, pX360Filename, sizeof(pX360Filename),
                     "GAME");
...
void CMDLCache::AllocateAnimBlocks(studiodata_t *pStudioData, int nCount) {
  ...
  pStudioData->m_iFakeAnimBlockStall =
      new unsigned int[pStudioData->m_nAnimBlockCount];
  memset(pStudioData->m_iFakeAnimBlockStall, 0,
         sizeof(unsigned int) * pStudioData->m_nAnimBlockCount);
}
...
unsigned char *CMDLCache::UnserializeAnimBlock(MDLHandle_t handle, int nBlock) {
  ...
  if (iAsync == NO_ASYNC) {
    ...
    if (IsX360()) {
      char pX360Filename[MAX_PATH];
      UpdateOrCreate(pStudioHdr, pFileName, pX360Filename,
                     sizeof(pX360Filename), "GAME");
      Q_strncpy(pFileName, pX360Filename, sizeof(pX360Filename));
    }
...
void CMDLCache::FreeVirtualModel(MDLHandle_t handle) {
  ...
  if (pStudioData && pStudioData->m_pVirtualModel) {
    ...
    Assert((nGroupCount >= 1) &&
           pStudioData->m_pVirtualModel->m_group[0].cache ==
               MDLHandleToVirtual(handle));

    // NOTE: Start at *1* here because the 0th element contains a reference to
    // *this* handle
    for (int i = 1; i < nGroupCount; ++i) {
      MDLHandle_t h =
          VoidPtrToMDLHandle(pStudioData->m_pVirtualModel->m_group[i].cache);
      FreeVirtualModel(h);
      Release(h);
    }
...
virtualmodel_t *CMDLCache::GetVirtualModelFast(const studiohdr_t *pStudioHdr,
  ...
  if (!pStudioData->m_pVirtualModel) {
    ...
    Assert(nGroup == 0);
    pStudioData->m_pVirtualModel->m_group[nGroup].cache =
        MDLHandleToVirtual(handle);
...
bool CMDLCache::BuildHardwareData(MDLHandle_t handle, studiodata_t *pStudioData,
  ...
  if (IsX360()) {
    unsigned char *pInputData =
        (unsigned char *)pVtxHdr + sizeof(OptimizedModel::FileHeader_t);
    if (CLZMA::IsCompressed(pInputData)) {
      ...
      V_memcpy(pOriginalData.Get(), pVtxHdr,
               sizeof(OptimizedModel::FileHeader_t));
      unsigned int nOutputSize = CLZMA::Uncompress(
          pInputData, sizeof(OptimizedModel::FileHeader_t) +
  ...
  if (IsX360()) {
    ...
    if (bOk && type == MDLCACHE_STUDIOHDR) {
      ...
      if (pStudioHdr->studiohdr2index == 0) {
        ...
        int sizeIncrease =
            (offsetStudiohdr2 - bufferContentsEnd) + sizeof(studiohdr2_t);
        ...
        studiohdr2_t *pStudioHdr2 =
            (studiohdr2_t *)((byte *)pStudioHdr + offsetStudiohdr2);
...
studiohdr_t *CMDLCache::UnserializeMDL(MDLHandle_t handle, void *pData,
  ...
  if (IsX360()) {
    if (CLZMA::IsCompressed((unsigned char *)pData)) {
      ...
      unsigned int nOutputSize = CLZMA::Uncompress(
          (unsigned char *)pData, (unsigned char *)pOriginalData.Get());
      if (nOutputSize != nOriginalSize) {
        // decoder failure
        return NULL;
      }
  ...
  if (IsX360()) {
    if (CLZMA::IsCompressed((unsigned char *)buf.PeekGet())) {
      ...
      unsigned int nOriginalSize =
          CLZMA::GetActualSize((unsigned char *)buf.PeekGet());
      void *pOriginalData = malloc(nOriginalSize);
      unsigned int nOutputSize = CLZMA::Uncompress(
          (unsigned char *)buf.PeekGet(), (unsigned char *)pOriginalData);
      if (nOutputSize != nOriginalSize) {
        // decoder failure
        free(pOriginalData);
        return false;
...
studiohdr_t *CMDLCache::GetStudioHdr(MDLHandle_t handle) {
  ...
  if (!pHdr) {
    ...
    if (!ReadMDLFile(handle, pModelName, buf)) {
      ...
      if (!bOk) {
        if (IsOSX()) {
          // rbarris wants this to go somewhere like the console.log prior to
          // crashing, which is what the Error call will do next
          printf("\n ##### Model %s not found and %s couldn't be loaded",
                 pModelName, ERROR_MODEL);
          fflush(stdout);
        }
...
bool CMDLCache::HandleCacheNotification(
  ...
  case DC_REMOVED: {
    ...
    if ((DataCacheClientID_t)(intp)notification.pItemData ==
...
//-----------------------------------------------------------------------------
vertexFileHeader_t *
CMDLCache::CreateThinVertexes(vertexFileHeader_t *originalData,
                              const studiohdr_t *pStudioHdr, int *cacheLength) {
...
bool CMDLCache::ProcessDataIntoCache(MDLHandle_t handle,
  ...
  case MDLCACHE_STUDIOHWDATA: {
    ...
    if (IsX360() &&
        !(pStudioDataCurrent->m_nFlags & STUDIODATA_FLAGS_NO_STUDIOMESH) &&
        bCompressedVVDs) {
      // Replace the cached vertex data with a thin version (used for model
      // decals). Flexed meshes require the fat data to remain, for CPU mesh
      // anim.
      if (pStudioHdrCurrent->numflexdesc == 0) {
        ...
        if (originalVertexData) {
          ...
          vertexFileHeader_t *thinVertexData = CreateThinVertexes(
              originalVertexData, pStudioHdrCurrent, &thinVertexDataSize);
          ...
          if (thinVertexData && (thinVertexDataSize > 0)) {
            ...
            // Add the new one
            CacheData(&pStudioDataCurrent->m_VertexCache, thinVertexData,
                      thinVertexDataSize, pStudioHdrCurrent->pszName(),
                      MDLCACHE_VERTEXES,
    ...
    if (bDataValid) {
      ...
      if (IsX360()) {
        if (CLZMA::IsCompressed((unsigned char *)pData)) {
          ...
          // get a "fake" (not really aligned) optimal read buffer, as expected
          // by the free logic
          void *pOriginalData = g_pFullFileSystem->AllocOptimalReadBuffer(
              FILESYSTEM_INVALID_HANDLE, nOriginalSize, 0);
          unsigned int nOutputSize = CLZMA::Uncompress(
              (unsigned char *)pData, (unsigned char *)pOriginalData);
          if (nOutputSize != nOriginalSize) {
            // decoder failure
      ...
      CacheData(&pStudioDataCurrent->m_pAnimBlock[iAnimBlock], pData, nDataSize,
                pCacheName, MDLCACHE_ANIMBLOCK,
    ...
    if (bDataValid) {
      ...
      if ((header.size == sizeof(header)) && header.solidCount > 0) {
        ...
        g_pPhysicsCollision->VCollideLoad(
            pCollide, header.solidCount, (const char *)buf.PeekGet(), nBufSize);
        if (m_pCacheNotify) {
...
//-----------------------------------------------------------------------------
vertexFileHeader_t *
CMDLCache::BuildAndCacheVertexData(studiohdr_t *pStudioHdr,
                                   vertexFileHeader_t *pRawVvdHdr) {
  MDLHandle_t handle = VoidPtrToMDLHandle(pStudioHdr->VirtualModel());
  vertexFileHeader_t *pVvdHdr;
  ...
  if (IsX360()) {
    unsigned char *pInput =
        (unsigned char *)pRawVvdHdr + sizeof(vertexFileHeader_t);
    if (CLZMA::IsCompressed(pInput)) {
      ...
      unsigned int nOutputSize =
          CLZMA::Uncompress(pInput, sizeof(vertexFileHeader_t) +
                                        (unsigned char *)pOriginalData.Get());
      if (nOutputSize != nOriginalSize) {
        // decoder failure
        return NULL;
  ...
  bool bNeedsTangentS =
      IsX360() || (g_pMaterialSystemHardwareConfig->GetDXSupportLevel() >= 80);
...
void CMDLCache::ProcessQueuedData(ModelParts_t *pModelParts, bool bHeaderOnly) {
  ...
  if (!pModelParts->bHeaderLoaded &&
      (pModelParts->nLoadedParts & (1 << ModelParts_t::BUFFER_MDL))) {
...
void CMDLCache::QueuedLoaderCallback_MDL(void *pContext, void *pContext2,
  ...
  ModelParts_t::BufferType_t bufferType =
      static_cast<ModelParts_t::BufferType_t>((intp)pContext2);
  ...
  if (pModelParts->DoFinalProcessing()) {
    if (!IsPC()) {
        ...
        if (studiohdr_t *pHdr = g_MDLCache.GetStudioHdr(pModelParts->hMDL)) {
          if (!(pHdr->flags & STUDIOHDR_FLAGS_OBSOLETE)) {
            ...
            for (int t = 0; t < pHdr->numtextures; ++t) {
              ...
              pTexture += (pTexture[0] == CORRECT_PATH_SEPARATOR ||
                           pTexture[0] == INCORRECT_PATH_SEPARATOR);
              for (int cd = 0; cd < pHdr->numcdtextures; ++cd) {
                ...
                V_ComposeFileName(pCdTexture, pTexture, buf + prefixLen,
                                  MAX_PATH - prefixLen);
                V_strncat(buf, ".vmt", MAX_PATH, COPY_ALL_CHARACTERS);
                pModelParts->bMaterialsPending = true;
                const char *pbuf = buf;
                g_pFullFileSystem->AddFilesToFileCache(pModelParts->hFileCache,
                                                       &pbuf, 1, "GAME");
      ...
      g_pQueuedLoader->QueueDynamicLoadFunctor(
          CreateFunctor(ProcessDynamicLoad, pModelParts));
    }
  }
}
...
void CMDLCache::ProcessDynamicLoad(ModelParts_t *pModelParts) {
      ...
      for (int t = 0; t < pHdr->numtextures; ++t) {
        ...
        for (int cd = 0; cd < pHdr->numcdtextures; ++cd) {
          ...
          IMaterial *pMaterial = materials->FindMaterial(
              buf + prefixLen, TEXTURE_GROUP_MODEL, false);
  ...
  g_pQueuedLoader->QueueCleanupDynamicLoadFunctor(
      CreateFunctor(CleanupDynamicLoad, pCleanup));
...
virtualmodel_t *studiohdr_t::GetVirtualModel(void) const {
  ...
  return g_MDLCache.GetVirtualModelFast(this,
                                        VoidPtrToMDLHandle(VirtualModel()));
}
