...
static ConVar
    mem_force_flush("mem_force_flush", "0", FCVAR_CHEAT,
...
CDataCacheSection::CDataCacheSection(CDataCache *pSharedCache,
                                     ...
                                     const char *pszName)
    : m_pClient(pClient), m_LRU(pSharedCache->m_LRU),
      m_mutex(pSharedCache->m_mutex), m_pSharedCache(pSharedCache),
      m_nFrameUnlockCounter(0), m_options(0) {
...
//-----------------------------------------------------------------------------
DataCacheHandle_t
CDataCacheSectionFastFind::DoFind(DataCacheClientID_t clientId) {
  AUTO_LOCK(m_mutex);
...
ConVar datacachesize("datacachesize", "64", FCVAR_INTERNAL_USE, "Size in MB.",
                     true, 32, true, 512, DataCacheSize_f);
