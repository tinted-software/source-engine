...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
  ...
  int len =
#endif
      _snprintf(szBuffer, sizeof(szBuffer) - 1, "PATH=%s\\bin\\;%s", pRootDir,
                pPath);
  szBuffer[ARRAYSIZE(szBuffer) - 1] = 0;
  assert(len < 4096);
  _putenv(szBuffer);

  HINSTANCE launcher = LoadLibrary(
      "bin\\dedicated.dll"); // STEAM OK ... filesystem not mounted yet
  if (!launcher) {
    char *pszError;
    FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM |
                      FORMAT_MESSAGE_IGNORE_INSERTS,
                  NULL, GetLastError(),
                  MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), (LPTSTR)&pszError,
                  0, NULL);
  ...
  DedicatedMain_t main =
      (DedicatedMain_t)GetProcAddress(launcher, "DedicatedMain");
...
int main(int argc, char *argv[]) {
  ...
  snprintf(szBuffer, sizeof(szBuffer) - 1, "LD_LIBRARY_PATH=%s/bin:%s", cwd,
           pPath);
  ...
  DedicatedMain_t dedicated_main =
      (DedicatedMain_t)dlsym(dedicated, "DedicatedMain");
  if (!dedicated_main) {
