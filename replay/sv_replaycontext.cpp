...
CServerReplayContext::CServerReplayContext()
    : m_pSessionRecorder(NULL), m_pFileserverCleaner(NULL),
      m_bShouldAbortRecording(false), m_flConVarSanityCheckTime(0.0f) {}
...
bool CServerReplayContext::Init(CreateInterfaceFn fnFactory) {
  ...
  m_pShared->m_pRecordingSessionManager =
      new CServerRecordingSessionManager(this);
  m_pShared->m_pRecordingSessionBlockManager =
      new CServerRecordingSessionBlockManager(this);
...
void CServerReplayContext::Think() {
  ...
  if (m_bShouldAbortRecording) {
    ...
    g_pBlockSpewer->PrintMsg("Replay recording shutting down due to publishing "
                             "error!  Recording will begin");
...
void CServerReplayContext::UpdateFileserverIPFromHostname(
    const char *pHostname) {
  if (!g_pEngine->NET_GetHostnameAsIP(pHostname, m_szFileserverIP,
                                      sizeof(m_szFileserverIP))) {
...
void CServerReplayContext::UpdateFileserverProxyIPFromHostname(
    const char *pHostname) {
  if (!g_pEngine->NET_GetHostnameAsIP(pHostname, m_szFileserverProxyIP,
                                      sizeof(m_szFileserverProxyIP))) {
  ...
  Log("Cached resolved fileserver proxy hostname to IP address: \"%s\" -> "
      "\"%s\"\n",
...
IGameEvent *CServerReplayContext::CreateReplaySessionInfoEvent() {
  IGameEvent *pEvent =
      g_pGameEventManager->CreateEvent("replay_sessioninfo", true);
  if (!pEvent)
    return NULL;
  ...
  // Fill event
  pEvent->SetString(
      "sn", m_pShared->m_pRecordingSessionManager->GetCurrentSessionName());
  pEvent->SetInt("di", replay_block_dump_interval.GetInt());
  pEvent->SetInt(
      "cb",
      m_pShared->m_pRecordingSessionManager->GetCurrentSessionBlockIndex());
...
const char *CServerReplayContext::GetLocalFileServerPath() const {
  ...
  // Fix up the path name - NOTE: We intentionally avoid calling
  // V_FixupPathName(), which pushes the entire output string to lower case.
...
const char *CServerReplayContext::GetServerSubDirName() const {
  ...
  if (!pSubDirName || !pSubDirName[0]) {
    Msg("No '-replayserverdir' parameter found - using default replay "
        "folder.\n");
    return SUBDIR_SERVER;
  }

  Msg("\n** Using custom replay dir name: \"%s%c%s\"\n\n", SUBDIR_REPLAY,
      CORRECT_PATH_SEPARATOR, pSubDirName);
...
void CServerReplayContext::OnPublishFailed() {
  // Don't report publish failure and shutdown publishing more than once per
  // session.
...
CServerRecordingSession *SV_GetRecordingSessionInProgress() {
  return SV_CastSession(
      SV_GetRecordingSessionManager()->GetRecordingSessionInProgress());
}

const char *SV_GetTmpDir() {
  return Replay_va("%s%s%c", SV_GetBasePath(), SUBDIR_TMP,
                   CORRECT_PATH_SEPARATOR);
}

bool SV_IsOffloadingEnabled() { return false; }
