...
CBaseRecordingSession::CBaseRecordingSession(IReplayContext *pContext)
    : m_pContext(pContext), m_bRecording(false), m_bAutoDelete(false),
      m_bBlocksLoaded(false), m_flStartTime(0.0f) {}
...
bool CBaseRecordingSession::Read(KeyValues *pIn) {
  ...
  if (m_strName.IsEmpty()) {
    ...
    IF_REPLAY_DBG(Warning("Session with no session name found - aborting load "
...
const char *CBaseRecordingSession::GetPath() const {
  return Replay_va("%s%s%c", m_pContext->GetBaseDir(), SUBDIR_SESSIONS,
                   CORRECT_PATH_SEPARATOR);
}

const char *CBaseRecordingSession::GetSessionInfoURL() const {
  return Replay_va("%s%s.%s", m_strBaseDownloadURL.Get(), m_strName.Get(),
...
void CBaseRecordingSession::LoadBlocksForSession() {
  ...
  IRecordingSessionBlockManager *pBlockManager =
      m_pContext->GetRecordingSessionBlockManager();

  // Peek in directory and load files based on what's there
  FileFindHandle_t hFind;
  CFmtStr fmtPath("%s%s*.%s", pBlockManager->GetBlockPath(), m_strName.Get(),
                  GENERIC_FILE_EXTENSION);
...
void CBaseRecordingSession::OnDelete() {
  ...
  // Dynamically load blocks if necessary, then delete from the block manager
  // and from disk
  DeleteBlocks();
}
...
void CBaseRecordingSession::PopulateWithRecordingData(
...
bool CBaseRecordingSession::ShouldDitchSession() const { return m_bAutoDelete; }

//----------------------------------------------------------------------------------------

bool CBaseRecordingSession::CLessFunctor::Less(
    const CBaseRecordingSessionBlock *pSrc1,
