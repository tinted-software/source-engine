...
//
// Manages and serializes all replay recording session data - instanced on both
// the client and server via CClientRecordingSessionManager and
// CServerRecordingSessionManager.
//
class CBaseRecordingSessionManager
    : public CGenericPersistentManager<CBaseRecordingSession>,
...
public:
  ...
  virtual CBaseRecordingSession *OnSessionStart(int nCurrentRecordingStartTick,
  ...
  // This is here so that server-side cleanup can be done for a ditched session.
  // See calling code for details.
  ...
  virtual const CBaseRecordingSession *
  FindSession(ReplayHandle_t hSession) const;
...
protected:
  ...
  CBaseRecordingSession *m_pRecordingSession; // Currently recording session, or
