...
void CReplayPerformanceManager::Init() {
  g_pFullFileSystem->CreateDirHierarchy(
      Replay_va("%s%s", CL_GetBasePath(), SUBDIR_PERFORMANCES));
}
...
void CReplayPerformanceManager::DeletePerformance(
  ...
  pOwnerReplay->m_vecPerformances.FindAndRemove(
      pPerformance); // This can fail if the replay doesn't own the performance
...
const char *CReplayPerformanceManager::GetRelativePath() const {
  return Replay_va("%s%s%c", CL_GetRelativeBasePath(), SUBDIR_PERFORMANCES,
                   CORRECT_PATH_SEPARATOR);
}

const char *CReplayPerformanceManager::GetFullPath() const {
  return Replay_va("%s%c%s", g_pEngine->GetGameDir(), CORRECT_PATH_SEPARATOR,
                   GetRelativePath());
}

CReplayPerformance *
CReplayPerformanceManager::CreatePerformance(CReplay *pReplay) {
  return new CReplayPerformance(pReplay);
}

const char *
CReplayPerformanceManager::GeneratePerformanceFilename(CReplay *pReplay) {
  static char s_szBaseFilename[MAX_OSPATH];
  char szIdealBaseFilename[MAX_OSPATH];
  V_strcpy_safe(szIdealBaseFilename,
                Replay_va("replay_%i_edit", (int)pReplay->GetHandle()));
  Replay_GetFirstAvailableFilename(
      s_szBaseFilename, sizeof(s_szBaseFilename), szIdealBaseFilename,
