...
public:
  ...
  enum Error_t {
    ERROR_NONE,
    ERROR_NOTONDISK,   // The replay was lost somehow - eg a server crash before
                       // the replay had a chance to write to disk
  ...
  RemoteStatus_t
      m_nRemoteStatus; // This represents the block's status on the server
  Error_t m_nHttpError;
  int32 m_iReconstruction;   // For client-side reconstruction of sessions, this
                             // represents the index of the given block
  ReplayHandle_t m_hSession; // What session is this partial replay a part of?
  uint32 m_uFileSize; // Size in bytes of the binary block file (if compressed,
                      // this represents the compressed file size)
  uint32 m_uUncompressedSize; // If compressed, this represents the uncompressed
                              // file size
  char m_szFullFilename[512]; // Filename for the .block file itself.
                              // NOTE: On the server, full path info is written
                              // - on client, only filename is written
  CompressorType_t
      m_nCompressorType; // What type of compressor/decompressor was/should be
                         // used, if any?  Can be COMPRESSORTYPE_INVALID if not
                         // compressed.
  uint8 m_aHash[16];     // Server sets this and client compares to validate
