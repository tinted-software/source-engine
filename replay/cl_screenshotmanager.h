...
private:
  ...
  CaptureScreenshotParams_t
      m_screenshotParams;        // Params for next scheduled screenshot
  int m_aScreenshotWidths[3][2]; // [ 16:9, 16:10, 4:3 ][ lo res, hi res ]

  ReplayHandle_t
      m_hScreenshotReplay; // Destination replay for any screenshots taken - we
                           // always write to the "pending" replay for the most
                           // part, but if we want to take a screenshot after
                           // the local player is dead, we need to use a handle
                           // rather than using m_pPendingReplay directly, since
                           // it becomes NULL when the player dies.
  float m_flLastScreenshotTime;
  int m_nPrevScreenDims[2]; // Screenshot dimensions, used to determine if we
                            // should update the screenshot cache on the client
};
