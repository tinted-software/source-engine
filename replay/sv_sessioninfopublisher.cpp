...
CSessionInfoPublisher::CSessionInfoPublisher(CServerRecordingSession *pSession)
    : m_pSession(pSession), m_flSessionInfoPublishTime(0.0f),
      m_itLastCompletedBlockWrittenToBuffer(~0), m_pFilePublisher(NULL),
      m_bShouldPublish(false) {}

CSessionInfoPublisher::~CSessionInfoPublisher() {}

void CSessionInfoPublisher::Publish() { m_bShouldPublish = true; }
...
void CSessionInfoPublisher::RefreshSessionInfoBlockData(CUtlBuffer &buf) {
  ...
  for (int i = itStart; i < vecBlocks.Count(); ++i) {
    ...
    IF_REPLAY_DBG(
        Warning("Writing block w/ recon index %i to session info buffer\n",
...
void CSessionInfoPublisher::Think() {
  ...
  // Create as much of the header as possible now - the rest will be written in
  // AdjustHeader() once the publisher knows the md5 digest and the compression
  // result.
  Assert(m_pSession->m_strName.Length() <
         MAX_SESSIONNAME_LENGTH); // The only way this name is going to get very
                                  // long is if
  V_strcpy_safe(m_SessionInfoHeader.m_szSessionName,
                m_pSession->m_strName.Get());
  ...
  // Format a path & filename that points to the tmp dir, with <session
  // name>.dmx on the end
  CFmtStr fmtTmpSessionInfoFile("%s%s.%s", SV_GetTmpDir(),
                                m_pSession->m_strName.Get(),
...
void CSessionInfoPublisher::OnPublishComplete(const IFilePublisher *pPublisher,
                                              void *pUserData) {
  Assert(!pUserData);
...
void CSessionInfoPublisher::AdjustHeader(const IFilePublisher *pPublisher,
                                         void *pHeaderData) {
  SessionInfoHeader_t *pHeader =
      static_cast<SessionInfoHeader_t *>(pHeaderData);

  // Set compressor type - will return COMPRESSORTYPE_INVALID if compression
  // failed.
