...
ConVar replay_publish_simulate_delay_local_http(
    ...
    true, 0.0f, true, 60.0f);
ConVar replay_publish_simulate_rename_fail(
    "replay_publish_simulate_rename_fail", "0", FCVAR_DONTRECORD,
    "Simulate a rename failure during local HTTP publishing, which will force "
    "a manual copy & delete.",
    true, 0.0f, true, 1.0f);
...
JobStatus_t CLocalPublishJob::DoExecute() {
  ...
  const char *pFileserverPath =
      g_pServerReplayContext->GetLocalFileServerPath();
  ...
  // Format a path & filename that points to the fileserver's download
  // directory, with <session name>.dmx on the end
...
#endif
      PrintEventResult(false);
      CFmtStr fmtError("Target already existed and could not be removed: '%s'",
                       pTargetFilename);
  ...
  SimulateDelay(replay_publish_simulate_delay_local_http.GetInt(),
                "Local HTTP");

  // Rename the file - RenameFile() still returns true, even if the destination
  // pathname is nonsense.  If the *source* is invalid, it fails as expected,
  // though.  Adding a FileExists() does not help.
  PrintEventStartMsg("Renaming to target");
  const bool bSimulateRenameFail =
      replay_publish_simulate_rename_fail.GetBool();
  if (bSimulateRenameFail ||
      !g_pFullFileSystem->RenameFile(m_szLocalFilename, pTargetFilename)) {
...
CCompressionJob::CCompressionJob(const uint8 *pSrcData, uint32 nSrcSize,
                                 CompressorType_t nType, bool *pOutResult,
                                 uint32 *pResultSize)
    : m_pSrcData(pSrcData), m_nSrcSize(nSrcSize),
      m_pCompressionResult(pOutResult), m_pResultSize(pResultSize) {
...
JobStatus_t CCompressionJob::DoExecute() {
  ...
  if (m_nSrcSize == 0) {
    SetError(ERROR_FAILED_ZERO_LENGTH_DATA,
  ...
  const int nMaxCompressedSize =
      ceil(m_nSrcSize * 1.1f) +
      600; // see "destLen" - http://www.bzip.org/1.0.3/html/util-fns.html
  uint8 *pCompressed = new uint8[nMaxCompressedSize];

  // Compress
  unsigned int nCompressedSize;
  PrintEventStartMsg("Compressing");
  if (!m_pCompressor->Compress((char *)pCompressed, &nCompressedSize,
                               (const char *)m_pSrcData, m_nSrcSize)) {
    // Compression failed?
...
CMd5Job::CMd5Job(const void *pSrcData, int nSrcSize, bool *pOutHashed,
                 uint8 *pOutHash, unsigned int pSeed[4] /*=NULL*/)
    : m_pSrcData(pSrcData), m_nSrcSize(nSrcSize), m_pHashed(pOutHashed),
      m_pHash(pOutHash), m_pSeed(pSeed) {
  *m_pHashed = false;
...
JobStatus_t CMd5Job::DoExecute() {
  ...
  bool bResult = g_pEngine->MD5_HashBuffer(m_pHash, (const uint8 *)m_pSrcData,
                                           m_nSrcSize, m_pSeed);
...
public:
  ...
  CBaseFilePublisher()
      : m_pCallbackHandler(NULL), m_pUserData(NULL), m_pCurrentJob(NULL),
        m_pInData(NULL), m_pHeaderData(NULL), m_nStatus(PUBLISHSTATUS_INVALID),
        m_nPhase(PHASE_INVALID), m_bCompressedOk(false), m_bHashedOk(false),
        m_nHeaderSize(0), m_nCompressedSize(0), m_nInSize(0),
        m_nInType(IO_INVALID) {
    m_szOutFilename[0] = 0;
    V_memset(m_aHash, 0, sizeof(m_aHash));
  }

  virtual PublishStatus_t GetStatus() const { return m_nStatus; }

  void SetStatus(PublishStatus_t nStatus) { m_nStatus = nStatus; }

  virtual bool IsDone() const { return m_nStatus != PUBLISHSTATUS_INVALID; }

  virtual bool Compressed() const { return m_bCompressedOk; }

  virtual bool Hashed() const { return m_bHashedOk; }
  ...
  virtual int GetCompressedSize() const { return m_nCompressedSize; }
  ...
  virtual void Publish(const PublishFileParams_t &params) {
    ...
    m_pSrcData = params.m_pSrcData; // Cache src data so we can determine
                                    // whether free'ing is OK
  ...
  void PrintErrors() {
    // If we don't print out any error now, it'll be lost once the job is
    // released.  Kind of a hack.
    if (m_pCurrentJob->GetStatus() == JOB_FAILED && !IsFailureOkForPhase()) {
      CBasePublishJob *pCurrentJob =
          dynamic_cast<CBasePublishJob *>(m_pCurrentJob);
      if (pCurrentJob) {
  ...
  virtual void Think() {
    ...
    if (flCurTime >
        m_flStartTime + replay_fileserver_offload_aborttime.GetFloat()) {
      g_pBlockSpewer->PrintMsg(
          Replay_va("ERROR: Publish timed out after %i seconds.",
                    replay_fileserver_offload_aborttime.GetInt()));
      Abort();
      return;
    }
    ...
    if (nJobStatus <= JOB_OK) {
      ...
      // Otherwise, publish isn't complete yet - go to next phase and spawn job
      // thread
      SetupNextJob(false);
    }
  }
...
private:
  enum IO_t {
    ...
    IO_BUFFER,
    IO_FILE,
    IO_DONTCARE, // As an input, this means the job doesn't care about the main
                 // pipeline stream (e.g. adjust header gets its inputs
                 // elsewhere) phase.  As an output, this should only be used
                 // for the final phase (publish).
  };
  ...
  void CacheOutputsOfCurrentJobForInputsOfNextJob() {
    ...
    if (m_nPhase == PHASE_WRITETODISK) {
      ...
      // Compare output data against input data - if different, free input and
      // replace with output.  In the case of hashing, for example, the input
      // buffer is used to do some computation, but the buffer itself goes
      // untouched.
      if (pJobOutData &&
          (m_pInData != pJobOutData || m_nInSize != nJobOutDataSize)) {
        m_pInData = pJobOutData;
        m_nInSize = nJobOutDataSize;
        bFreeOldInData = true;
      }
    } else if (nOutputType == IO_DONTCARE) {
      // This should have been cleaned up in write-to-disk phase if we're in
      // publish phase
  ...
  // NOTE: This needs to return a CJob ptr (i.e. and not a CBaseJob) since the
  // job may be an AsyncWrite
  CJob *GetJobForPhase(Phase_t nPhase) {
    CJob *pResult = NULL;

    switch (nPhase) {
    case PHASE_COMPRESSION:
      pResult = new CCompressionJob(m_pInData, m_nInSize, m_nCompressorType,
                                    &m_bCompressedOk, &m_nCompressedSize);
      break;
    ...
    case PHASE_ADJUSTHEADER: {
      // Let the callback handler make any adjustments to the header (add md5
      // digest, etc.)
      ...
      if (m_pHeaderData && m_nHeaderSize) {
        ...
        g_pFullFileSystem->AsyncWrite(m_szOutFilename, m_pHeaderData,
                                      m_nHeaderSize, bFreeMemory, false,
                                      &hFileJob);
    ...
    case PHASE_WRITETODISK:
      if (m_pInData && m_nInSize) {
        // Create an asynchronous write job - if a header already exists in the
        // file, append.
        FSAsyncControl_t hFileJob;
        const bool bAppend = m_pHeaderData != NULL;
        g_pFullFileSystem->AsyncWrite(m_szOutFilename, m_pInData, m_nInSize,
                                      false, bAppend, &hFileJob);
        pResult = (CJob *)hFileJob;
      }
      break;
    ...
    Assert(GetCurrentPhaseInputType() == IO_DONTCARE ||
           m_nInType == IO_DONTCARE || GetCurrentPhaseInputType() == m_nInType);

    return pResult;
  }

  bool IsFailureOkForPhase() const {
    // Compression will fail (e.g. due to small buffer size), which shouldn't
    // bring down the house.
    return m_nPhase == PHASE_COMPRESSION || m_nPhase == PHASE_DELETEFILE;
  }

  bool IsLastPhase() const { return m_PhaseQueue.Count() == 0; }
...
CBaseFilePublisher::IoInfo_t CBaseFilePublisher::sm_aPhaseIOTypes[NUM_PHASES] =
    {
        ...
        {IO_DONTCARE,
         IO_DONTCARE},          // PHASE_ADJUSTHEADER - this phase can operate
                                // independent of the pipeline, so
                                //						long as
                                //any compression/hashing is taken care of.
...
IFilePublisher *SV_PublishFile(const PublishFileParams_t &params) {
  Assert(!params.m_pHeaderData ||
         (params.m_pHeaderData && params.m_pCallbackHandler));
