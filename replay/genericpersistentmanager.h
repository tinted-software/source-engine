...
template <class T> class CGenericPersistentManager : public CBaseThinker {
public:
  ...
  T *CreateAndGenerateHandle(); // Creates a new object and generates a unique
                                // handle

  void Add(T *pNewObj); // Commit the object - NOTE: The Create*() functions
                        // don't call Add() for you
  void Remove(
      ReplayHandle_t hObj); // Remove() will remove the object, remove any .dmx
                            // associated with the object on disk, and usually
                            // delete attached files (like .dems or movies, etc,
                            // depending on what the manager implementation is)
  void Remove(T *pObj);
  void RemoveFromIndex(int it);
  void Clear(); // Remove all objects - NOTE: Doesn't save right away

  bool WriteObjToFile(
      T *pObj, const char *pFilename); // Write object data to an arbitrary file
  ...
  virtual void Think(); // IReplayThinker implementation - NOTE: not meant to be
                        // called directly - called from think manager
  virtual const char *
  GetIndexPath() const; // Should return path where index file lives
...
protected:
  ...
  virtual const char *
  GetRelativeIndexPath() const = 0; // Should return relative (to replay/client
                                    // or replay/server) path where index file
                                    // lives - NOTE: Last char should be a slash
  virtual const char *GetIndexFilename()
      const = 0; // Should return just the name of the file, e.g. "replays.dmx"
  ...
  const char *GetIndexFullFilename()
...
template <class T> void CGenericPersistentManager<T>::Shutdown() { Save(); }

template <class T>
CGenericPersistentManager<T>::CGenericPersistentManager()
    : m_nHandleSeed(0), m_nVersion(-1), m_bIndexDirty(false),
      m_flNextFlushTime(0.0f), m_flNextUnloadTime(0.0f) {}

template <class T> CGenericPersistentManager<T>::~CGenericPersistentManager() {
  Clear();
}

template <class T> void CGenericPersistentManager<T>::Clear() {
  ...
  // NOTE: This list contains pointers to objects in m_vecObjs, so no
  // destruction of elements here
  m_lstDirtyObjs.RemoveAll();
  m_lstObjsToUnload.RemoveAll();
}

template <class T> int CGenericPersistentManager<T>::Count() const {
  return m_vecObjs.Count();
}

template <class T> void CGenericPersistentManager<T>::FlagIndexForFlush() {
  m_bIndexDirty = true;

  IF_REPLAY_DBG2(Warning("%f %s: Index flagged\n", g_pEngine->GetHostTime(),
                         GetDebugName()));
}
...
template <class T> void CGenericPersistentManager<T>::FlagForUnload(T *pObj) {
  AssertMsg(ShouldSerializeToIndividualFiles(),
            "This functionality should only be used for managers that write to "
            "individual files, i.e. NOT managers that maintain one monolithic "
            "index.");
...
template <class T> bool CGenericPersistentManager<T>::IsDirty(T *pNewObj) {
  return m_lstDirtyObjs.Find(pNewObj) != m_lstDirtyObjs.InvalidIndex();
}

template <class T> void CGenericPersistentManager<T>::Add(T *pNewObj) {
  IF_REPLAY_DBG2(
      Warning("Adding object with handle %i\n", pNewObj->GetHandle()));
...
template <class T> void CGenericPersistentManager<T>::Remove(T *pObj) {
...
template <class T> void CGenericPersistentManager<T>::RemoveFromIndex(int it) {
...
#endif
    if (g_pFullFileSystem->FileExists(strFullFilename.Get()) &&
        !bSimulateDelete) {
  ...
  AssertMsg(
      m_lstObjsToUnload.Find(pObj) == m_lstObjsToUnload.InvalidIndex(),
      "The object being removed was also in the unload list - is this OK?  If "
      "so, code should be added to remove from that list as well.");
...
template <class T>
int CGenericPersistentManager<T>::FindIteratorFromHandle(
    ReplayHandle_t hHandle) {
  FOR_EACH_VEC(m_vecObjs, i) {
    T *pCurObj = m_vecObjs[i];
    if (hHandle == pCurObj->GetHandle()) {
      return i;
...
template <class T> bool CGenericPersistentManager<T>::Load() {
  bool bResult = true;
  ...
  if (pRoot->LoadFromFile(g_pFullFileSystem, pFullFilename)) {
    ...
    if (ShouldSerializeToIndividualFiles()) {
      ...
      if (pFileIndex) {
        ...
        const char *pFilename =
            g_pFullFileSystem->FindFirst(fmtPath.Access(), &hFind);
        while (pFilename) {
          // Ignore index file
    ...
    FOR_EACH_VEC(m_vecObjs, i) { OnObjLoaded(m_vecObjs[i]); }
  }
...
template <class T>
bool CGenericPersistentManager<T>::WriteObjToFile(T *pObj,
                                                  const char *pFilename) {
...
template <class T> bool CGenericPersistentManager<T>::Save() {
  ...
  // Write all objects to one monolithic file - writes all objects (ignores
  // "dirtyness")
  else {
    FOR_EACH_VEC(m_vecObjs, i) {
      T *pCurObj = m_vecObjs[i];
  ...
  if (m_bIndexDirty) {
    return bResult &&
           pRoot->SaveToFile(g_pFullFileSystem, GetIndexFullFilename());
  }

  return bResult;
}

template <class T> T *CGenericPersistentManager<T>::CreateAndGenerateHandle() {
  T *pNewObj = Create();
...
template <class T> void CGenericPersistentManager<T>::Think() {
...
template <class T> void CGenericPersistentManager<T>::FlushThink() {
...
template <class T> void CGenericPersistentManager<T>::UnloadThink() {
  const float flHostTime = g_pEngine->GetHostTime();
...
const char *CGenericPersistentManager<T>::GetIndexPath() const {
  return Replay_va("%s%s", GetReplayContext()->GetBaseDir(),
...
template <class T> bool CGenericPersistentManager<T>::HaveDirtyObjects() const {
  return m_lstDirtyObjs.Count() > 0;
}

template <class T> bool CGenericPersistentManager<T>::HaveObjsToUnload() const {
  return m_lstObjsToUnload.Count() > 0;
}

template <class T> void CGenericPersistentManager<T>::CreateIndexDir() {
  g_pFullFileSystem->CreateDirHierarchy(GetIndexPath(), "DEFAULT_WRITE_PATH");
}

template <class T>
bool CGenericPersistentManager<T>::ReadObjFromFile(const char *pFile, T *&pOut,
                                                   bool bForceLoad) {
...
template <class T>
T *CGenericPersistentManager<T>::ReadObjFromKeyValues(KeyValues *pObjData,
                                                      bool bForceLoad) {
  T *pNewObj = Create();
  Assert(pNewObj);
  if (!pNewObj)
  ...
  // Attempt to read data for the object, and fail to load this particular
  // object if the reader says we should.
...
void CGenericPersistentManager<T>::UpdateHandleSeed(ReplayHandle_t hNewHandle) {
  m_nHandleSeed =
      (ReplayHandle_t)(GetHandleBase() +
                       MAX((uint32)m_nHandleSeed, (uint32)hNewHandle) + 1);

#ifdef _DEBUG
  FOR_EACH_VEC(m_vecObjs, i) {
    AssertMsg(m_nHandleSeed != m_vecObjs[i]->GetHandle(),
