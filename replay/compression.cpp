...
public:
  virtual bool Compress(char *pDest, unsigned int *pDestLen,
                        const char *pSource, unsigned int nSourceLen) {
    return g_pEngine->LZSS_Compress(pDest, pDestLen, pSource, nSourceLen);
  }

  virtual bool Decompress(char *pDest, unsigned int *pDestLen,
                          const char *pSource, unsigned int nSourceLen) {
...
#define BZ2_DEFAULT_BLOCKSIZE100k                                              \
  9 // Highest compression rate, but uses the most memory
...
public:
  CCompressor_Bz2(int nBlockSize100k = BZ2_DEFAULT_BLOCKSIZE100k,
                  int nWorkFactor = BZ2_DEFAULT_WORKFACTOR)
      : m_nBlockSize100k(nBlockSize100k), m_nWorkFactor(nWorkFactor) {}

  virtual bool Compress(char *pDest, unsigned int *pDestLen,
                        const char *pSource, unsigned int nSourceLen) {
    return BZ_OK == BZ2_bzBuffToBuffCompress(pDest, pDestLen,
                                             const_cast<char *>(pSource),
                                             nSourceLen, m_nBlockSize100k,
                                             0, // Silent verbosity
                                             m_nWorkFactor);
  }
  ...
  virtual bool Decompress(char *pDest, unsigned int *pDestLen,
    ...
    return BZ_OK == BZ2_bzBuffToBuffDecompress(
                        pDest, pDestLen, const_cast<char *>(pSource),
                        nSourceLen,
...
CON_COMMAND(replay_testcompress, "Test compression") {
  ...
  if (g_pFullFileSystem->Read(pUncompressed, nInFileSize, hInFile) !=
    ...
    // Compress
    if (!pCompressor->Compress(pCompressed, &nCompressedSize, pUncompressed,
                               nInFileSize)) {
      Warning("Compression failed.\n");
    } else {
      CFmtStr fmtOutFilename("%s.%s", pInFilename, pCompressionTypeName);
      FileHandle_t hOutFile =
          g_pFullFileSystem->Open(fmtOutFilename.Access(), "wb+");
      if (!hOutFile) {
        Warning("Failed to open out file, %s\n", fmtOutFilename.Access());
      } else {
        if (g_pFullFileSystem->Write(pCompressed, nCompressedSize, hOutFile) !=
            (int)nCompressedSize) {
          ...
          Warning(
              "Wrote compressed file to successfully (%s) - ratio: %.2f:1\n",
