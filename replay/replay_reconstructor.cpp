...
public:
  bool Less(const CClientRecordingSessionBlock *pBlock1,
            const CClientRecordingSessionBlock *pBlock2, void *pCtx) {
...
bool Replay_Reconstruct(CReplay *pReplay, bool bDeleteBlocks /*=true*/) {
  ...
  const CBaseRecordingSession::BlockContainer_t &vecAllBlocks =
      pSession->GetBlocks();
  ...
  CUtlSortVector<const CClientRecordingSessionBlock *, CReplay_LessFunc>
      vecReplayBlocks;
  FOR_EACH_VEC(vecAllBlocks, i) {
    const CClientRecordingSessionBlock *pCurBlock =
        CL_CastBlock(vecAllBlocks[i]);
  ...
  int iLastReconstructionIndex =
      0; // All replay reconstruction will start with block 0
  ...
  strReconstructedFileFilename.Format(
      "%s%s_%i.dem", CL_GetReplayManager()->GetIndexPath(),
      pSession->m_strName.Get(), pReplay->GetHandle());
  FileHandle_t hReconstructedFile =
      g_pFullFileSystem->Open(strReconstructedFileFilename.Get(), "wb");
  if (hReconstructedFile == FILESYSTEM_INVALID_HANDLE) {
    CL_GetErrorSystem()->AddErrorFromTokenName("#Replay_Err_Recon_OpenOutFile");
    return false;
  }

  // Now that we have an ordered list of replays to reconstruct, create the
  // mother file
  bool bFailed = false;
  FOR_EACH_VEC(vecReplayBlocks, i) {
    const CClientRecordingSessionBlock *pCurBlock = vecReplayBlocks[i];
    ...
    if (!pBuffer) {
      ...
      if (nSize != g_pFullFileSystem->Read(pBuffer, nSize, hBlockFile)) {
        ...
        CompressorType_t nCompressorType =
            (CompressorType_t)pCurBlock->m_nCompressorType;
          ...
          if (!pCompressor) {
            ...
            if (!pUncompressedBuffer) {
              ...
              if (!nUncompressedSize) {
                ...
                bFailed = true;
              } else if (!pCompressor->Decompress(pUncompressedBuffer,
                                                  &nUncompressedSize, pBuffer,
                                                  nCompressedSize)) {
                CL_GetErrorSystem()->AddErrorFromTokenName(
        ...
        if (g_pFullFileSystem->Write(pBuffer, nSize, hReconstructedFile) ==
            nSize) {
  ...
  if (!bFailed) {
    ...
    *((int32 *)szEndTickBuf) = nLastTick;
    unsigned char szStopBuf[] = {dem_stop,        szEndTickBuf[0],
                                 szEndTickBuf[1], szEndTickBuf[2],
                                 szEndTickBuf[3], 0};
    int nStopSize = sizeof(szStopBuf);
    if (g_pFullFileSystem->Write(szStopBuf, nStopSize, hReconstructedFile) !=
        nStopSize) {
      Warning("Replay: Failed to write stop bits to reconstructed replay file "
              "\"%s\"\n",
    ...
    // Delete blocks - removes from session, session block manager, and from
    // disk if no other replays are depending on them.
    if (bDeleteBlocks) {
