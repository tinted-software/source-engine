...
CReplayMovieManager::CReplayMovieManager()
    : m_pPendingMovie(NULL), m_pVidModeSettings(NULL),
      m_pRenderMovieSettings(NULL), m_bIsRendering(false),
...
CReplayMovieManager::~CReplayMovieManager() { delete m_pVidModeSettings; }

bool CReplayMovieManager::Init() {
  // Create "rendered" dir
  const char *pRenderedDir = Replay_va("%s%s%c", CL_GetReplayBaseDir(),
                                       SUBDIR_RENDERED, CORRECT_PATH_SEPARATOR);
  g_pFullFileSystem->CreateDirHierarchy(pRenderedDir);

  return BaseClass::Init();
}

int CReplayMovieManager::GetMovieCount() { return Count(); }

void CReplayMovieManager::GetMovieList(CUtlLinkedList<IReplayMovie *> &list) {
  FOR_EACH_OBJ(this, i) { list.AddToTail(ToMovie(m_vecObjs[i])); }
}
...
CReplayMovie *CReplayMovieManager::Create() { return new CReplayMovie(); }
...
IReplayMovie *CReplayMovieManager::CreateAndAddMovie(ReplayHandle_t hReplay) {
  CReplayMovie *pNewMovie =
      CreateAndGenerateHandle(); // Sets m_hThis (which is accessed via
                                 // GetHandle())
  ...
  V_memcpy(&pNewMovie->m_RenderSettings, &m_pRenderMovieSettings->m_Settings,
           sizeof(ReplayRenderSettings_t));
...
void CReplayMovieManager::DeleteMovie(ReplayHandle_t hMovie) {
  ...
  CReplay *pOwnerReplay =
      pMovie ? CL_GetReplayManager()->GetReplay(pMovie->m_hReplay) : NULL;
...
IReplayMovie *CReplayMovieManager::GetPendingMovie() { return m_pPendingMovie; }

void CReplayMovieManager::FlagMovieForFlush(IReplayMovie *pMovie,
                                            bool bImmediate) {
...
int CReplayMovieManager::GetVersion() const { return MOVIE_MANAGER_VERSION; }

IReplayContext *CReplayMovieManager::GetReplayContext() const {
  return g_pClientReplayContextInternal;
}

float CReplayMovieManager::GetNextThinkTime() const { return 0.1f; }
...
void CReplayMovieManager::GetCachedMovieTitleAndClear(wchar_t *pOut,
                                                      int nOutBufLength) {
...
void CReplayMovieManager::AddReplayForRender(CReplay *pReplay,
                                             int iPerformance) {
...
void CReplayMovieManager::RenderMovie(RenderMovieParams_t const &params) {
  ...
  if (params.m_hReplay == REPLAY_HANDLE_INVALID) {
    ...
    AddReplayForRender(CL_GetReplayManager()->GetReplay(params.m_hReplay),
...
void CReplayMovieManager::CompleteRender(bool bSuccess, bool bShowBrowser) {
  ...
  g_pClient->OnRenderComplete(*m_pRenderMovieSettings, m_bRenderingCancelled,
                              bSuccess, bShowBrowser);
...
void CReplayMovieManager::CancelRender() {
  ...
  g_pEngine->Host_Disconnect(
      false); // CReplayDemoPlayer::StopPlayback() will be called
}

void CReplayMovieManager::GetMoviesAsQueryableItems(
    CUtlLinkedList<IQueryableReplayItem *, int> &lstMovies) {
  lstMovies.RemoveAll();
  FOR_EACH_OBJ(this, i) { lstMovies.AddToHead(ToMovie(m_vecObjs[i])); }
}
...
const char *CReplayMovieManager::GetRawExportDir() const {
  ...
  s_fmtExportDir.sprintf("%smovie_%02i%02i%04i_%02i%02i%02i%c", GetRenderDir(),
