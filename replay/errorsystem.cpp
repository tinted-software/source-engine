...
CErrorSystem::~CErrorSystem() { Clear(); }
...
void CErrorSystem::AddFormattedErrorFromTokenName(
    const char *pFormatToken /*=NULL*/, KeyValues *pFormatArgs /*=NULL*/) {
  if (!pFormatToken) {
  ...
  if (g_pVGuiLocalize) {
    g_pVGuiLocalize->ConstructString(wszErrorStr, sizeof(wszErrorStr),
                                     pFormatToken, pFormatArgs);
  } else {
...
void CErrorSystem::OGS_ReportSessionBlockDownloadError(
    const CHttpDownloader *pDownloader,
...

{
  ...
  KeyValues *pBlockDownloadError =
      new KeyValues("TF2ReplayBlockDownloadErrors");
  pBlockDownloadError->SetInt("ErrorCounter", g_nGenericErrorCounter);
  pBlockDownloadError->SetInt("NumCurrentDownloads",
  ...
  pBlockDownloadError->SetInt("NumDownloadAttempts",
                              pBlock->GetNumDownloadAttempts());

  // Only include these if appropriate - otherwise, let them be NULL for the
  // given row
  if (pSizesDiffer) {
  ...
  if (pHashFail) {
    ...
    V_binarytohex(pBlock->m_aHash, sizeof(pBlock->m_aHash), szRemoteHash,
                  sizeof(szRemoteHash));
    V_binarytohex(pLocalHash, sizeof(pBlock->m_aHash), szLocalHash,
                  sizeof(szLocalHash));
...
void CErrorSystem::OGS_ReportSessioInfoDownloadError(
    const CHttpDownloader *pDownloader, const char *pErrorToken) {
  // Create a download error and queue for upload
  KeyValues *pDownloadError = pDownloader->GetOgsRow(g_nGenericErrorCounter);
  g_pClient->UploadOgsData(pDownloadError, false);

  // Create session info download error
  KeyValues *pSessionInfoDownloadError =
      new KeyValues("TF2ReplaySessionInfoDownloadErrors");
  pSessionInfoDownloadError->SetInt("ErrorCounter", g_nGenericErrorCounter++);
  pSessionInfoDownloadError->SetString("SessionInfoDownloadErrorID",
                                       pErrorToken);
...
// Note: we use the ErrorCounter as part of the key and so it must be unique.
// This means that all special error tables (ie., session info download errors)
// must call back into this base function to write out the base error and
