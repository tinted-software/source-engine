...
IClientEntityList *entitylist = NULL; // icliententitylist.h forces the use of
                                      // this name by externing in the header
...
public:
  virtual bool Connect(CreateInterfaceFn fnFactory) {
    ...
    g_pEngine =
        (IEngineReplay *)fnFactory(ENGINE_REPLAY_INTERFACE_VERSION, NULL);
    REPLAY_INIT(g_pEngine);

    REPLAY_INIT(g_pEngine->IsSupportedModAndPlatform());

#if !defined(DEDICATED)
    g_pEngineClient = (IEngineClientReplay *)fnFactory(
        ENGINE_REPLAY_CLIENT_INTERFACE_VERSION, NULL);
    REPLAY_INIT(g_pEngineClient);

    g_pEngineTraceClient =
        (IEngineTrace *)fnFactory(INTERFACEVERSION_ENGINETRACE_CLIENT, NULL);
    REPLAY_INIT(g_pEngineTraceClient);

    g_pReplayDemoPlayer =
        (IReplayDemoPlayer *)fnFactory(INTERFACEVERSION_REPLAYDEMOPLAYER, NULL);
    REPLAY_INIT(g_pReplayDemoPlayer);

    g_pVGuiLocalize =
        (vgui::ILocalize *)fnFactory(VGUI_LOCALIZE_INTERFACE_VERSION, NULL);
    REPLAY_INIT(g_pVGuiLocalize);

    g_pDebugOverlay =
        (IVDebugOverlay *)fnFactory(VDEBUG_OVERLAY_INTERFACE_VERSION, NULL);
    REPLAY_INIT(g_pDebugOverlay);
#endif

    g_pGameEventManager = (IGameEventManager2 *)fnFactory(
        INTERFACEVERSION_GAMEEVENTSMANAGER2, NULL);
    REPLAY_INIT(g_pGameEventManager);

#if !defined(DEDICATED)
    g_pDownloadSystem =
        (IDownloadSystem *)fnFactory(INTERFACEVERSION_DOWNLOADSYSTEM, NULL);
...
#if defined(DEDICATED)
    REPLAY_INIT(ReplayLib_Init(g_pEngine->GetGameDir(),
                               NULL)) // Init without the client replay context
#else
    REPLAY_INIT(ReplayLib_Init(g_pEngine->GetGameDir(),
                               g_pClientReplayContextInternal));
#endif
  ...
  virtual bool IsRecording() {
    // NOTE: demoplayer->IsPlayingBack() needs to be checked here, as
    // "replay_enable" and "replay_recording" will inevitably get stored with
    // signon data in any playing demo. If the !demoplayer->IsPlayingBack() line
    // is omitted below, Replay_IsRecording() becomes useless during demo
    // playback and will always return true.
...
#if !defined(DEDICATED)
    g_pClient =
        (IClientReplay *)fnClientFactory(CLIENT_REPLAY_INTERFACE_VERSION, NULL);
    if (!g_pClient)
      return false;

    entitylist = (IClientEntityList *)fnClientFactory(
...
#if !defined(DEDICATED)
    if (g_pClientReplayContextInternal &&
        g_pClientReplayContextInternal->IsInitialized()) {
  ...
  virtual void CL_Render() {
    ...
    if (g_pClientReplayContextInternal->m_pScreenshotManager
            ->ShouldCaptureScreenshot()) {
      g_pClientReplayContextInternal->m_pScreenshotManager
          ->DoCaptureScreenshot();
      return;
    }

    // Currently rendering?  NOTE: GetMovieRenderer() only returns a valid ptr
    // during rendering
    IReplayMovieRenderer *pReplayMovieRenderer =
        g_pClientReplayContextInternal->GetMovieRenderer();
  ...
  virtual bool SV_Init(CreateInterfaceFn fnServerFactory) {
    ...
    g_pServer =
        (IServerReplay *)fnServerFactory(SERVER_REPLAY_INTERFACE_VERSION, NULL);
    if (!g_pServer)
      return false;
  ...
  virtual void SV_NotifyReplayRequested() {
    ...
    // A replay was requested - notify the session so we don't throw it away at
  ...
  virtual void
  SV_EndRecordingSession(bool bForceSynchronousPublish /*=false*/) {
    if (!g_pEngine->IsSupportedModAndPlatform())
      return;
    ...
    if (bForceSynchronousPublish) {
      ...
      // Let the session manager do any cleanup - this will remove files
      // associated with a ditched session.  For example, if the server was shut
      // down in the middle of a round where no one saved a replay, the files
      // will be published synchronously above and then cleaned up synchronously
      // here.
      SV_GetRecordingSessionManager()->DoSessionCleanup();

      // Since the recording session manager will kick off a cleanup job, we
      // need to wait for it here since we're shutting down.
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CReplaySystem, IReplaySystem,
                                  REPLAY_INTERFACE_VERSION, s_Replay);
...
const char *Replay_GetDownloadURL() {
  ...
  V_snprintf(s_szFileURL, sizeof(s_szFileURL), "%s://%s:%i/%s/",
             replay_fileserver_protocol.GetString(),
             replay_fileserver_host.GetString(),
             replay_fileserver_port.GetInt(), Replay_GetDownloadURLPath());
...
fail:
  AssertMsg(0, "Replay_CrackURL() was passed an invalid URL and has failed.  "
