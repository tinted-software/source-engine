...
CClientRecordingSession::CClientRecordingSession(IReplayContext *pContext)
    : CBaseRecordingSession(pContext), m_iLastBlockToDownload(-1),
      m_iGreatestConsecutiveBlockDownloaded(-1),
      m_nSessionInfoDownloadAttempts(0), m_flLastUpdateTime(-1.0f),
      m_pSessionInfoDownloader(NULL), m_bTimedOut(false),
...
void CClientRecordingSession::SyncSessionBlocks() {
  // If the last update time hasn't been initialized yet, initialize it now
  // since this will be the first time we are attempting to download the session
  // info file.
...
void CClientRecordingSession::OnReplayDeleted(CReplay *pReplay) {
  ...
  // This will load session blocks and delete them from disk if possible.  In
  // the case that all other replays for a session have already been
  // reconstructed and pReplay was the last replay that was never reconstructed,
  // we should delete session's blocks now. Note that these calls will only (a)
  // load blocks if they aren't loaded already, and (b) Delete blocks if all
  // associated replays have been reconstructed.
...
bool CClientRecordingSession::Read(KeyValues *pIn) {
  ...
  m_iGreatestConsecutiveBlockDownloaded =
      pIn->GetInt("last_consec_block_downloaded", -1);
...
void CClientRecordingSession::AdjustLastBlockToDownload(
    int iNewLastBlockToDownload) {
...
int CClientRecordingSession::UpdateLastBlockToDownload() {
  // Here we calculate the block we'll need in order to reconstruct the replay
  // at the post-death time, based on replay_postdeathrecordtime, NOT the
  // current time.  The index calculated here may be greater than the actual
  // last block the server writes, since the round may end or the map may
  // change.  This is adjusted for when we actually download the blocks.
  ...
  const int nAddedBlocks = (int)ceil(replay_postdeathrecordtime.GetFloat() /
                                     nDumpInterval); // Round up
  ...
  IF_REPLAY_DBG(ConColorMsg(0, Color(0, 255, 0),
...
void CClientRecordingSession::Think() {
  ...
  if (m_pSessionInfoDownloader && m_pSessionInfoDownloader->IsDone() &&
      m_pSessionInfoDownloader->CanDelete()) {
    // Failure?
    if (m_pSessionInfoDownloader->m_nError !=
        CSessionInfoDownloader::ERROR_NONE) {
      // If there was an error, increment the error count and update the
      // appropriate replays if we've tried a sufficient number of times.
      ++m_nSessionInfoDownloadAttempts;
      if (m_nSessionInfoDownloadAttempts >=
          MAX_SESSION_INFO_DOWNLOAD_ATTEMPTS) {
        FOR_EACH_LL(m_lstReplays, i) {
          CReplay *pCurReplay = m_lstReplays[i];

          // If this replay has already been set to "ready to convert" state (or
          // beyond), skip.
...
void CClientRecordingSession::UpdateAllBlocksDownloaded() {
  // We're only "done" if this session is no longer recording and all blocks are
  // downloaded.
  const bool bOld = m_bAllBlocksDownloaded;
  m_bAllBlocksDownloaded =
      !m_bRecording &&
      (m_iGreatestConsecutiveBlockDownloaded >= m_iLastBlockToDownload);
...
void CClientRecordingSession::UpdateGreatestConsecutiveBlockDownloaded() {
  // Assumes m_vecBlocks is sorted in ascending order (for both reconstruction
  // indices and handle, which should be parallel)
  int j = 0;
  int iGreatestConsecutiveBlockDownloaded = 0;
  FOR_EACH_VEC(m_vecBlocks, i) {
    CClientRecordingSessionBlock *pCurBlock = CL_CastBlock(m_vecBlocks[i]);

    AssertMsg(pCurBlock->m_iReconstruction == j,
              "Session blocks must be sorted!");

    // If the block hasn't been downloaded, stop here
    if (pCurBlock->m_nDownloadStatus !=
        CClientRecordingSessionBlock::DOWNLOADSTATUS_DOWNLOADED)
      break;

    // Block has been downloaded - update the counter
    iGreatestConsecutiveBlockDownloaded =
        MAX(iGreatestConsecutiveBlockDownloaded, pCurBlock->m_iReconstruction);
...
void CClientRecordingSession::UpdateReplayStatuses(
  ...
  // If the download was successful, update the greatest consecutive block
  // downloaded index
  ...
  const bool bFailed = pBlock->m_nDownloadStatus ==
                       CClientRecordingSessionBlock::DOWNLOADSTATUS_ERROR;

  // Go through all replays that refer to this session and update their status
  // if necessary
  FOR_EACH_LL(m_lstReplays, i) {
    CReplay *pCurReplay = m_lstReplays[i];
    ...
    // If the download failed and the block is required for this replay, mark as
    // such
    if (bFailed &&
        pCurReplay->m_iMaxSessionBlockRequired >= pBlock->m_iReconstruction) {
      pCurReplay->m_nStatus = CReplay::REPLAYSTATUS_ERROR;
      bFlush = true;
    ...
    // Have we downloaded all blocks required for the given replay?
    else if (!bFailed && pCurReplay->m_iMaxSessionBlockRequired <=
                             m_iGreatestConsecutiveBlockDownloaded) {
      ...
      g_pClient->DisplayReplayMessage("#Replay_DownloadComplete", false, false,
...
void CClientRecordingSession::OnDownloadTimeout() {
  ...
  FOR_EACH_LL(m_lstReplays, i) {
    ...
    if (m_iGreatestConsecutiveBlockDownloaded >=
...
void CClientRecordingSession::ShowDownloadFailedMessage(
    const CReplay *pReplay) {
  // Don't show the download failed message for replays that were saved during
  // this run of the game.
  if (!pReplay || !pReplay->m_bSavedDuringThisSession)
    return;

  // Display an error message
  g_pClient->DisplayReplayMessage("#Replay_DownloadFailed", true, false,
...
void CClientRecordingSession::CacheReplay(CReplay *pReplay) {
  ...
  // We should no longer auto-delete this session if CacheReplay() is being
  // called.  This can happen if the user connects to a server, saves a replay,
  // deletes the replay (at which point auto-delete is flagged for the recording
  // session), and then saves another replay.  In this situation, we obviously
  // don't want to delete the session anymore.
  if (m_bAutoDelete) {
    m_bAutoDelete = false;
  }
}
...
bool CClientRecordingSession::ShouldSyncBlocksWithServer() const {
  ...
  // If block count is out of sync with the m_iLastBlockDownloaded we need to
  // sync up
  const bool bReachedMaxDownloadAttempts =
  ...
  //	const bool bAlreadyDownloadedAllNeededBlocks = m_iLastBlockToDownload <=
  //m_iGreatestConsecutiveBlockDownloaded;
  const bool bAlreadyDownloadedAllNeededBlocks =
      m_iLastBlockToDownload <
      m_vecBlocks
          .Count(); // NOTE/TODO: Shouldn't this look at
                    // m_iGreatestConsecutiveBlockDownloaded?  Tried for a week,
                    // but it caused bugs.  Reverting for now.  TODO
  const bool bTimedOut = false; // TimedOut();

  const bool bResult = !bReachedMaxDownloadAttempts && bNeedToDownloadBlocks &&
                       !bAlreadyDownloadedAllNeededBlocks && !bTimedOut;
  ...
  if (bResult) {
    ...
    if (bTimedOut)
      DBG3("   - Download timed out (session info file didn't change after 90 "
           "seconds)\n");
  }
...
void CClientRecordingSession::PopulateWithRecordingData(
    int nCurrentRecordingStartTick) {
