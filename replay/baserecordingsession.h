...
public:
  ...
  class CLessFunctor {
  public:
    bool Less(const CBaseRecordingSessionBlock *pSrc1,
              const CBaseRecordingSessionBlock *pSrc2, void *pContext);
  };

  typedef CUtlSortVector<CBaseRecordingSessionBlock *, CLessFunctor>
      BlockContainer_t;
  ...
  // Determines whether or not a session gets nuked at the end of a round or
  // not.
  ...
  CUtlString m_strName; // A unique session name, given by the server at the
                        // start of recording based on time/date
  CUtlString m_strBaseDownloadURL; // The download URL, with no filename, e.g.,
                                   // "http://someserver:80/somepath/"
  int m_nServerStartRecordTick; // The tick at which the server began recording
                                // the given session (based on
                                // g_ServerGlobalVariables.tickcount) - which is
                                // the tick client spawn/death ticks are
                                // relative to
  float m_flStartTime;

  // Non-persistent:
  IReplayContext *m_pContext;
  bool m_bAutoDelete; // Set to true if a session is removed while it's
                      // recording - if flagged, session will auto-delete once
                      // recording ends
  bool m_bBlocksLoaded;

protected:
  BlockContainer_t m_vecBlocks; // A list of session blocks for the given
                                // session - NOTE: Blocks should not be free'd
};
