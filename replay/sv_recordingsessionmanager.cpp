...
CServerRecordingSessionManager::CServerRecordingSessionManager(
    ...
    : CBaseRecordingSessionManager(pContext), m_flNextScheduledCleanup(0.0f),
      m_bOffload(false) {}
...
const char *CServerRecordingSessionManager::GetNewSessionName() const {
  ...
  return Replay_va("%04i%02i%02i-%02i%02i%02i-%s", 1900 + today.tm_year,
                   today.tm_mon + 1, today.tm_mday, today.tm_hour, today.tm_min,
                   today.tm_sec, g_pEngine->GetGameServer()->GetMapName());
}

void CServerRecordingSessionManager::Think() {
  VPROF_BUDGET("CServerRecordingSessionManager::Think",
...
bool CServerRecordingSessionManager::CanDeleteSession(
    ReplayHandle_t hSession) const {
...
CServerRecordingSessionManager::OnSessionStart(int nCurrentRecordingStartTick,
                                               const char *pSessionName) {
  CBaseRecordingSession *pResult =
      BaseClass::OnSessionStart(nCurrentRecordingStartTick, pSessionName);
