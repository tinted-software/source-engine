...
CSessionInfoDownloader::CSessionInfoDownloader()
    : m_pDownloader(NULL), m_pSession(NULL), m_flLastDownloadTime(0.0f),
      m_nError(ERROR_NONE), m_nHttpError(HTTP_ERROR_NONE), m_bDone(false) {}

CSessionInfoDownloader::~CSessionInfoDownloader() {
  Assert(m_pDownloader ==
         NULL); // We should have deleted the downloader already
}
...
void CSessionInfoDownloader::DownloadSessionInfoAndUpdateBlocks(
...
void CSessionInfoDownloader::OnDownloadComplete(CHttpDownloader *pDownloader,
  ...
  if (pDownloader->GetStatus() != HTTP_DONE || bForceError) {
    ...
    if (!ReadSessionInfoHeader(pData, pDownloader->GetSize(), header)) {
      ...
      IF_REPLAY_DBG(
          Warning("Session info downloaded successfully for session %s\n",
      ...
      if (nNumBlocks <= 0) {
        ...
        if (!pSessionName || !pSessionName[0]) {
          ...
          // Now that we have the session name, find it in the client session
          // manager
          CClientRecordingSession *pSession = CL_CastSession(
              CL_GetRecordingSessionManager()->FindSessionByName(pSessionName));
          Assert(pSession);
          if (!pSession) {
            ...
            const CompressorType_t nHeaderCompressorType =
                header.m_nCompressorType;
            ...
            bool bPayloadValid = g_pEngine->MD5_HashBuffer(
                header.m_aHash, (const uint8 *)pPayload, header.m_uPayloadSize,
                NULL);
            ...
            if (!bPayloadValid) {
              ...
              if (bPayloadValid) {
                AssertMsg(pUncompressedPayload,
                          "This should never be NULL here.");
                AssertMsg(uUncompressedPayloadSize >=
                              MIN_SESSION_INFO_PAYLOAD_SIZE,
                          "This size should always be valid here.");

                RecordingSessionBlockSpec_t DummyBlock;
                CUtlBuffer buf(pUncompressedPayload, uUncompressedPayloadSize,
                               CUtlBuffer::READ_ONLY);

                // Optimization: start the read at the first block we care
                // about, which is one block after the last consecutive,
                // downloaded block. This optimization should come in handy on
                // servers that run lengthy rounds, so we're not reiterating
                // over inconsequential blocks (i.e. they were already
                // downloaded).
                int iStartBlock =
                    pSession->GetGreatestConsecutiveBlockDownloaded() + 1;
                if (iStartBlock > 0) {
                  buf.SeekGet(CUtlBuffer::SEEK_HEAD,
                              iStartBlock *
                                  sizeof(RecordingSessionBlockSpec_t));
                }

                // If for some reason the seek caused a 'get' overflow, try
                ...
                for (int i = iStartBlock; i < header.m_nNumBlocks; ++i) {
                  ...
                  IF_REPLAY_DBG(
                      Warning("processing block with recon index: %i\n",
                              DummyBlock.m_iReconstruction));

                  // Get reconstruction index
                  const int iBlockReconstruction =
                      (ReplayHandle_t)DummyBlock.m_iReconstruction;
                  ...
                  if (nRemoteStatus < 0 ||
                      nRemoteStatus >= CBaseRecordingSessionBlock::MAX_STATUS) {
                  ...
                  const uint32 uUncompressedSize =
                      (uint32)DummyBlock.m_uUncompressedSize;

                  // Get the compressor type
                  const int nCompressorType =
                      (uint32)DummyBlock.m_nCompressorType;

                  // Attempt to find the block in the session
                  CClientRecordingSessionBlock *pBlock = CL_CastBlock(
                      CL_GetRecordingSessionBlockManager()->FindBlockForSession(
                          m_pSession->GetHandle(), iBlockReconstruction));

                  // If the block exists and has already been downloaded, we
                  // have nothing more to update
                  if (pBlock && !pBlock->NeedsUpdate())
                    continue;

                  bool bBlockDataChanged = false;

                  // If the block doesn't exist in the session block manager,
                  // create it now
                  if (!pBlock) {
                    CClientRecordingSessionBlock *pNewBlock =
                        CL_CastBlock(CL_GetRecordingSessionBlockManager()
                                         ->CreateAndGenerateHandle());
                    pNewBlock->m_iReconstruction = iBlockReconstruction;
                    //				pNewBlock->m_strFullFilename =
                    //Replay_va(
                    const char *pFullFilename = Replay_va(
                        "%s%s_part_%i.%s", pNewBlock->GetPath(),
                  ...
                  IF_REPLAY_DBG2(Warning("   Block %i status=%s\n",
                                         ...
                                         pBlock->GetRemoteStatusStringSafe(
                                             pBlock->m_nRemoteStatus)));

                  // Now that we've got a block, replicate the server data/fill
                  // it in
                  if (pBlock->m_nRemoteStatus != nRemoteStatus) {
                    pBlock->m_nRemoteStatus =
                        (CBaseRecordingSessionBlock::RemoteStatus_t)
                            nRemoteStatus;
                    bBlockDataChanged = true;
                  }
                  ...
                  if (pBlock->m_nCompressorType !=
                      (CompressorType_t)nCompressorType) {
                    pBlock->m_nCompressorType =
                        (CompressorType_t)nCompressorType;
                    bBlockDataChanged = true;
                  }
                  ...
                  if (!pBlock->HasValidHash()) {
                    V_memcpy(pBlock->m_aHash, DummyBlock.m_aHash,
                             sizeof(pBlock->m_aHash));
                    bBlockDataChanged = true;
                  }

                  // Shift the block's state from waiting to ready-to-download
                  // if the block is ready on the server.
                  if (pBlock->m_nDownloadStatus ==
                          CClientRecordingSessionBlock::
                              DOWNLOADSTATUS_WAITING &&
                      nRemoteStatus ==
                          CBaseRecordingSessionBlock::STATUS_READYFORDOWNLOAD) {
                    pBlock->m_nDownloadStatus = CClientRecordingSessionBlock::
                        DOWNLOADSTATUS_READYTODOWNLOAD;
                  }

                  // Save
                  if (bBlockDataChanged) {
                    CL_GetRecordingSessionBlockManager()->FlagForFlush(pBlock,
                                                                       false);
                ...
                // If this session is not recording, make sure the max number of
                // blocks in the session is in check.
                if (!pSession->m_bRecording &&
                    pSession->GetLastBlockToDownload() >= nNumBlocks) {
                  // This will adjust all replay max blocks
                  pSession->AdjustLastBlockToDownload(nNumBlocks - 1);
                }

                // If we've updated something, set cache the current time in the
                // session
                if (bUpdatedSomething) {
                  pSession->RefreshLastUpdateTime();
                } else if (pSession->GetLastUpdateTime() >= 0.0f &&
                           (g_pEngine->GetHostTime() -
                                pSession->GetLastUpdateTime() >
  ...
  if (m_nError != ERROR_NONE) {
    ...
    // Report error to OGS
    CL_GetErrorSystem()->OGS_ReportSessioInfoDownloadError(pDownloader,
                                                           pErrorToken);
  }
...
const char *
CSessionInfoDownloader::GetErrorString(int nError,
                                       HTTPError_t nHttpError) const {
  switch (nError) {
