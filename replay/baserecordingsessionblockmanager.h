...
class CBaseRecordingSessionBlockManager
    : public CGenericPersistentManager<CBaseRecordingSessionBlock>,
...
public:
  ...
  virtual void LoadBlockFromFileName(
      ...
      IRecordingSession
          *pSession); // NOTE: This will not actually add the block to the
                      // session block manager - this is for loading blocks and
                      // adding them to recording sessions so they can be
                      // deleted from disk and cleaned up from the fileserver if
                      // necessary.

  // Find the block for with the given reconstruction index for the given
  // session
  CBaseRecordingSessionBlock *FindBlockForSession(ReplayHandle_t hSession,
                                                  int iReconstruction);
