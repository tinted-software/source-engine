...
CScreenshotManager::CScreenshotManager()
    : m_flLastScreenshotTime(0.0f), m_hScreenshotReplay(REPLAY_HANDLE_INVALID) {
}
...
bool CScreenshotManager::Init() {
  ...
  CFmtStr fmtThumbnailsPath("%s%cmaterials%cvgui%creplay%cthumbnails",
                            g_pEngine->GetGameDir(), CORRECT_PATH_SEPARATOR,
  ...
  // Set current screenshot dims to 0 - screenshot cache will be created first
  // time we see proper screen dimensions
...
float CScreenshotManager::GetNextThinkTime() const { return 0.0f; }
...
void CScreenshotManager::Think() {
  ...
  IReplayScreenshotSystem *pScreenshotSystem =
      g_pClient->GetReplayScreenshotSystem();
  Assert(pScreenshotSystem);

  // Check to see if screen resolution changed, and if so, update the
  // client-side screenshot cache.
...
bool CScreenshotManager::ShouldCaptureScreenshot() {
  ...
  return (m_flScreenshotCaptureTime >= 0.0f &&
          m_flScreenshotCaptureTime <= g_pEngine->GetHostTime());
}
...
void CScreenshotManager::DoCaptureScreenshot() {
  // Reset screenshot capture schedule time, even if we don't end up actually
  ...
  if (!m_screenshotParams.m_bIgnoreMinTimeBetweenScreenshots &&
      (g_pEngine->GetHostTime() - m_flLastScreenshotTime <
       replay_mintimebetweenscreenshots.GetInt()))
    return;
  ...
  V_strcpy_safe(szIdealBaseFilename,
                CL_GetRecordingSessionManager()
                    ->m_ServerRecordingState.m_strSessionName.Get());
  Replay_GetFirstAvailableFilename(
      szBaseFilename, sizeof(szBaseFilename), szIdealBaseFilename, ".vtf",
  ...
  V_snprintf(szScreenshotPath, sizeof(szScreenshotPath),
             "materials\\vgui\\replay\\thumbnails\\%s.vtf", szBaseFilename);
  ...
  if (m_screenshotParams.m_nEntity > 0) {
    IClientEntity *pEntity =
        entitylist->GetClientEntity(m_screenshotParams.m_nEntity);
    if (pEntity) {
      ...
      ray.Init(vStartPos,
               pEntity->GetAbsOrigin() + m_screenshotParams.m_posCamera);
      g_pEngineTraceClient->TraceRay(ray, MASK_PLAYERSOLID, &traceFilter,
                                     &trace);
  ...
  V_snprintf(szVTFFullPath, sizeof(szVTFFullPath),
  ...
  pReplay->AddScreenshot(nScreenshotDims[0], nScreenshotDims[1],
                         szBaseFilename);
}
...
void CScreenshotManager::GetUnpaddedScreenshotSize(int &nOutWidth,
                                                   int &nOutHeight) {
