...
public:
  ...
  virtual void OnPublishComplete(const IFilePublisher *pPublisher,
                                 void *pUserData) = 0;
  virtual void OnPublishAborted(const IFilePublisher *pPublisher) = 0;
  virtual void AdjustHeader(const IFilePublisher *pPublisher,
                            void *pHeaderData) = 0;
};
...
//
// NOTE: You can force an IFilePublisher to delete itself by calling
// EnableAutoDelete().
//
class IFilePublisher {
public:
  ...
  //
  // NOTE: Call Compressed() and Hashed() when IsDone() is true or in
  // OnPublishComplete() to find out if compression/hashing succeeded.
  //
  // Setting bFreeSrcData to false will keep the publisher from deleting the
  // initial source data. Otherwise, the process is that buffers will be cleaned
  // up as they are no longer needed - for example, if compression is enabled,
  // the initial buffer will be free'd if bFreeSrcData is true.
  //
  ...
  virtual bool
  Compressed() const = 0; // If compression was requested, did it succeed?
...
public:
  CBasePublishJob(JobPriority_t nPriority = JP_NORMAL,
...
public:
  CCompressionJob(const uint8 *pSrcData, uint32 nSrcSize,
                  CompressorType_t nType, bool *pOutCompressed,
...
public:
  CMd5Job(const void *pSrcData, int nSrcSize, bool *pOutHashed,
          uint8 pOutHash[16], unsigned int pSeed[4] = NULL);
