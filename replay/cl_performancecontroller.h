...
private:
  ...
  CReplayPerformance
      *m_pSavedPerformance; // Points to the saved performance - scratch copies
                            // to saved - should not be modified directly
  CReplayPerformance
      *m_pScratchPerformance; // The working performance, ie the temporary
                              // performance we muck with until the user saves
                              // or discards
  bool m_bRewinding;
  bool
      m_bPaused; // Maintain our own state for paused/playing for event queueing
  CUtlLinkedList<KeyValues *>
      m_EventQueue; // If user pauses and changes camera, etc, it gets queued
                    // here - if multiple camera changes occur, previous is
                    // stomped
  ...
  //
  void PlaybackThink();
  void ReadSetViewEvent(KeyValues *pEventSubKey, Vector &origin, QAngle &angles,
                        float &fov, float *pAccel, float *pSpeed,
                        float *pRotFilter);
  void DebugRender();
  bool SetupPlaybackHandler();
  void SetupPlaybackFromPerformance(CReplayPerformance *pPerformance);
  void
  SetupPlaybackExistingStream(); // Don't load anything from disk - use what's
                                 // already in memory - used for rewind
