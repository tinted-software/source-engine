...
//
// Generic downloader class - downloads a single file on its own thread, and
// maintains state for that data.
//
// TODO: Derive from CBaseThinker and remove explicit calls to Think() - will
// make this class less bug-prone (easy to forget to call Think()).
//
class CHttpDownloader {
public:
  // Pass in a callback
  ...
  // pGamePath - Game path where we should put the file - can be NULL if we
  // don't
  //    want to save to the file to disk
  // pUserData - Passed back to IDownloadHandler, if one has been set - can be
  // NULL pBytesDownloaded - If non-NULL, # of bytes downloaded written. Returns
  // true on success.
  //
  ...
  inline uint32 GetSize() const {
    return m_uSize;
  } // File size in bytes - NOTE: Not valid until the download is complete,
...
private:
  ...
  // Use this to make sure that AbortDownloadAndCleanup isn't executed
  // simultaneously by two threads. This was causing use-after-free crashes
  // during shutdown.
  CThreadMutex m_lock;
};
