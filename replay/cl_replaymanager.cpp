...
CReplayManager::CReplayManager()
    : m_pPendingReplay(NULL), m_pReplayLastLife(NULL), m_pReplayThisLife(NULL),
...
bool CReplayManager::Init(CreateInterfaceFn fnCreateFactory) {
  ...
  // Session manager init'd by this point - go through and link up replays to
  // sessions
...
IReplayFactory *
CReplayManager::GetReplayFactory(CreateInterfaceFn fnCreateFactory) {
  return (IReplayFactory *)fnCreateFactory(INTERFACE_VERSION_REPLAY_FACTORY,
                                           NULL);
}

void CReplayManager::OnSessionStart() {
  // The pending replay doesn't exist yet at this point as far as I've seen,
  // since the "replay_sessioninfo" event comes down a frame or more after the
  // "replay_recording" replicated cvar is set to 1, which is what triggers
  ...
  if (m_pPendingReplay) {
    ...
    if (m_pPendingReplay->m_hSession == REPLAY_HANDLE_INVALID) {
      ReplayHandle_t hSessionInProgress = CL_GetRecordingSessionManager()
    ...
    if (m_pPendingReplay->m_nSpawnTick < 0) {
      const int nServerStartTick =
          CL_GetRecordingSessionManager()->m_ServerRecordingState.m_nStartTick;
      Assert(nServerStartTick > 0);
      m_pPendingReplay->m_nSpawnTick =
          MAX(0, -m_pPendingReplay->m_nSpawnTick - nServerStartTick);
    }
  }
}
...
void CReplayManager::CommitPendingReplayAndBeginDownload() {
  ...
  CClientRecordingSession *pSession =
      CL_CastSession(CL_GetRecordingSessionManager()->FindSession(
          m_pReplayThisLife->m_hSession));
...
void CReplayManager::CompletePendingReplay() {
  ...
  CBaseRecordingSession *pSession =
      CL_GetRecordingSessionManager()->FindSession(
          m_pPendingReplay->m_hSession);

  // Sometimes the session isn't valid here, like when we're first joining a
  // server
  if (!pSession)
    return;
  ...
  // Cache death tick
  m_pPendingReplay->m_nDeathTick = g_pEngineClient->GetLastServerTickTime() -
                                   pSession->m_nServerStartRecordTick;

  SanityCheckReplay(m_pPendingReplay);

  // Calc replay length
  m_pPendingReplay->m_flLength = g_pEngine->TicksToTime(
      m_pPendingReplay->m_nDeathTick - m_pPendingReplay->m_nSpawnTick +
      g_pEngine->TimeToTicks(replay_postdeathrecordtime.GetFloat()));

  // Cache player slot so we can start playback of replays from recorder
  ...
  // If the replay was requested by the user already, update the # of blocks we
  // should download & commit the replay
  ...
  // Before we copy the pointer to "this life," end recording so the replay can
  // do any cleanup (eg listening for game events)
...
void CReplayManager::DeleteReplay(ReplayHandle_t hReplay, bool bNotifyUI) {
  ...
  // The session manager will delete the .dem, the session .dmx and remove the
  // session item itself if this is the last replay associated with it.
...
void CReplayManager::FlagReplayForFlush(CReplay *pReplay,
...
void CReplayManager::InitReplay(CReplay *pReplay) {
  ...
  V_FileBase(g_pEngineClient->GetLevelName(), m_pPendingReplay->m_szMapName,
             sizeof(m_pPendingReplay->m_szMapName));
...
CReplay *CReplayManager::CreatePendingReplay() {
  ...
  // If we've already begun recording, link to the session now, otherwise link
  // once we start recording.
  CBaseRecordingSession *pSessionInProgress =
...
void CReplayManager::AttemptToSetupNewReplay() {
  ...
  if (!m_pPendingReplay) {
    ...
    const int nServerStartTick =
        CL_GetRecordingSessionManager()->m_ServerRecordingState.m_nStartTick;
    pPendingReplay->m_nSpawnTick =
        g_pEngineClient->GetLastServerTickTime() - nServerStartTick;
    if (nServerStartTick == 0) {
      // Didn't receive the replay_sessioninfo event yet - make spawn tick
      // negative so when the event IS received, we can detect that the server
      // start tick still needs to be subtracted.
      pPendingReplay->m_nSpawnTick *= -1;
    }

    // Setup post-death record time
    extern ConVar replay_postdeathrecordtime;
    pPendingReplay->m_nPostDeathRecordTime =
        replay_postdeathrecordtime.GetFloat();
...
void CReplayManager::Think() {
  ...
  // NOTE: we use Sys_FloatTime() here, since the client sets the next update
  // time with engine->Time(), which also uses Sys_FloatTime().
  if (m_pPendingReplay &&
      m_pPendingReplay->m_flNextUpdateTime <= Sys_FloatTime()) {
    m_pPendingReplay->Update(); // Allow the replay's Update() function to set
...
void CReplayManager::DebugThink() {
  ...
  if (replay_debug.GetBool()) {
    ...
    CReplay *pReplays[] = {m_pPendingReplay, m_pReplayThisLife,
                           m_pReplayLastLife};
    for (int i = 0; i < 3; ++i) {
      ...
      g_pEngineClient->Con_NPrintf(
          i, "%s:  handle=%i  [%i, %i]  C? %s R? %s  MaxBlock: %i",
          pReplayNames[i], pCurReplay->GetHandle(), pCurReplay->m_nSpawnTick,
      ...
      int nCurLine = 5;
      g_pEngineClient->Con_NPrintf(
          nCurLine, "Screenshot replay: handle=%i",
      ...
      FOR_EACH_REPLAY(j) {
        ...
        g_pEngineClient->Con_NPrintf(
            nCurLine++, "%i: handle=%i  ticks=[%i %i]", i, pReplay->GetHandle(),
            pReplay->m_nSpawnTick, pReplay->m_nDeathTick);
      }

      // Current tick:
      g_pEngineClient->Con_NPrintf(++nCurLine, "MAIN tick: %f",
                                   g_pEngineClient->GetLastServerTickTime());
      g_pEngineClient->Con_NPrintf(++nCurLine, "   server tick: %f",
                                   g_pEngineClient->GetLastServerTickTime());
      nCurLine += 2;
    }
  }
...
void CReplayManager::GetReplays(CUtlLinkedList<CReplay *, int> &lstReplays) {
  ...
  FOR_EACH_REPLAY(i) { lstReplays.AddToTail(GET_REPLAY_AT(i)); }
}

void CReplayManager::GetReplaysAsQueryableItems(
    CUtlLinkedList<IQueryableReplayItem *, int> &lstReplays) {
  lstReplays.RemoveAll();
  FOR_EACH_REPLAY(i) {
    lstReplays.AddToHead(
        dynamic_cast<IQueryableReplayItem *>(GET_REPLAY_AT(i)));
  }

  if (m_pPendingReplay && !m_pPendingReplay->m_bComplete &&
...
const char *CReplayManager::GetReplaysDir() const { return GetIndexPath(); }

float CReplayManager::GetDownloadProgress(const CReplay *pReplay) {
  // Give each downloadable session block equal weight since we won't know the
  // size of blocks that have not been created/written yet on the server.

  // Go through all blocks in the replay and figure out how many bytes have been
  // downloaded
  float flSum = 0.0f;

  CClientRecordingSession *pSession = CL_CastSession(
      CL_GetRecordingSessionManager()->FindSession(pReplay->m_hSession));
  Assert(pSession);
  if (!pSession)
    return 0.0f;

  const CBaseRecordingSession::BlockContainer_t &vecBlocks =
      pSession->GetBlocks();
  FOR_EACH_VEC(vecBlocks, i) {
    ...
    const float flSubProgress =
        pCurBlock->m_uFileSize == 0
            ? 0.0f
            : clamp((float)pCurBlock->m_uBytesDownloaded /
                        pCurBlock->m_uFileSize,
                    0.0f, 1.0f);
  ...
  // NOTE: This will cause a bug in download progress if the round ends and cuts
  // the number of expected blocks down - but that situation is probably less
  // likely to occur than the situation where a client is expecting more blocks
  // that *will* be created.  To avoid pops in the latter situation, we account
  // for those blocks here.
