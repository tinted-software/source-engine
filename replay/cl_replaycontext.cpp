...
CClientReplayContext::CClientReplayContext()
    : m_pReplayManager(NULL), m_pScreenshotManager(NULL),
      m_pMovieRenderer(NULL), m_pMovieManager(NULL),
      m_pPerformanceManager(NULL), m_pPerformanceController(NULL),
      m_pTestDownloader(NULL), m_pRenderQueue(NULL),
...
bool CClientReplayContext::Init(CreateInterfaceFn fnFactory) {
  ...
  m_pShared->m_pRecordingSessionManager =
      new CClientRecordingSessionManager(this);
  m_pShared->m_pRecordingSessionBlockManager =
      new CClientRecordingSessionBlockManager(this);
  ...
  // Cleanup any unneeded block data from disk - cleanup is done on the fly, but
  // this will clean up blocks from the olden days, when block data was not
  // cleaned up properly.
...
void CClientReplayContext::Shutdown() {
  ...
  // status changes in replays, etc, which will need to be saved in
  // CReplayManager::Shutdown(), etc.
...
void CClientReplayContext::DebugThink() {
  ...
  CClientRecordingSession *pRecordingSession =
      CL_GetRecordingSessionInProgress();
  if (pRecordingSession) {
    g_pEngineClient->Con_NPrintf(iLine++, "SESSION IN PROGRESS:");
    g_pEngineClient->Con_NPrintf(iLine++, "  BLOCKS: %i",
                                 pRecordingSession->GetNumBlocks());
    g_pEngineClient->Con_NPrintf(iLine++, "  NAME: %s",
                                 pRecordingSession->m_strName.Get());
    g_pEngineClient->Con_NPrintf(iLine++, "  URL: %s",
                                 pRecordingSession->m_strBaseDownloadURL.Get());
    g_pEngineClient->Con_NPrintf(
        iLine++, "  LAST CONSECUTIVE BLOCK DOWNLOADED: %i",
...
void CClientReplayContext::SetMovieRenderer(
...
void CClientReplayContext::OnSignonStateFull() {
  ...
  // Play a performance?  This will play a performance from the beginning, if
  // we're loading one (ie the 'watch' button in the details panel of the replay
  // browser), or will continue playback if the user rewound while watching or
  ...
  if (CL_GetMovieManager()->IsRendering()) {
    ...
    if (!m_pMovieRenderer->SetupRenderer(
            CL_GetMovieManager()->GetRenderMovieSettings(), pMovie)) {
      Warning("Render failed!\n");
      CL_GetMovieManager()->CancelRender();
    }
  }

  // If we're not rendering and are playing back a replay, initialize the
  // performance editor - won't actually show until the user presses space, if
  // they do at all.
...
void CClientReplayContext::OnClientSideDisconnect() {
  ...
  if (!g_pEngineClient->IsPlayingReplayDemo()) {
    ...
    // NOTE: We need to let the replay manager deal before we end the session,
    // otherwise the state of the session will be cleared.
    ...
    // Sets recording flag to false in session in progress, clears session in
    // progress, and clears server state
    CL_GetRecordingSessionManager()->OnSessionEnd();
  }
}

void CClientReplayContext::PlayReplay(ReplayHandle_t hReplay, int iPerformance,
                                      bool bPlaySound) {
  ...
  if (!ReconstructReplayIfNecessary(pReplay)) {
    Replay_MsgBox(iPerformance < 0 ? "#Replay_Err_User_FailedToPlayReplay"
  ...
  // Play a sound?
  if (bPlaySound) {
    g_pClient->PlaySound(iPerformance >= 0 ? "replay\\playperformance.wav"
...
bool CClientReplayContext::ReconstructReplayIfNecessary(CReplay *pReplay) {
  ...
  if (!pReplay->HasReconstructedReplay() || replay_forcereconstruct.GetBool()) {
    if (!Replay_Reconstruct(pReplay)) {
      CL_GetErrorSystem()->AddErrorFromTokenName(
...
void CClientReplayContext::GetPlaybackTimes(
  ...
  const int nServerRecordStartTick =
      CL_GetRecordingSessionManager()->GetServerStartTickForSession(
          pReplay->m_hSession);
  ...
  // Calculate length
  const int nReplayEndTick =
      pReplay->m_nSpawnTick + g_pEngine->TimeToTicks(pReplay->m_flLength);
  const int nEndTick = (pPerformance && pPerformance->m_nTickOut > 0)
                           ? pPerformance->m_nTickOut
                           : nReplayEndTick;
  flOutLength = pPerformance ? g_pEngine->TicksToTime(nEndTick - nStartTick)
                             : pReplay->m_flLength;

  // Calculate current time
  const int nCurTick = MAX(g_pEngineClient->GetClientGlobalVars()->tickcount -
                               nStartTick - nServerRecordStartTick,
                           0);
...
void CClientReplayContext::DisableReplayOnClient(bool bDisable) {
  ...
  Replay_HudMsg(bDisable ? "#Replay_ClientSideDisabled"
...
CClientRecordingSessionManager *CL_GetRecordingSessionManager() {
  return static_cast<CClientRecordingSessionManager *>(
      g_pClientReplayContextInternal->GetRecordingSessionManager());
}

CClientRecordingSession *CL_GetRecordingSessionInProgress() {
  return CL_CastSession(
      CL_GetRecordingSessionManager()->GetRecordingSessionInProgress());
}
