...
#if _DEBUG
ConVar replay_fileserver_simulate_delete(
    "replay_fileserver_simulate_delete", "0", FCVAR_GAMEDLL,
...
IFileserverCleanerJob *SV_CastJobToIFileserverCleanerJob(CBaseJob *pJob) {
  ...
  AssertMsg(pResult != NULL,
            "Cast failed!  Are you sure this job is an IFileserverCleanerJob?");
  return pResult;
}
...
CFileserverCleaner::CFileserverCleaner()
    : m_bRunning(false), m_bPrintResult(false), m_pCleanerJob(NULL),
      m_pSpewer(NULL), m_nNumFilesDeleted(0) {}
...
void CFileserverCleaner::DoCleanAsynchronous(
    bool bPrintResult /*=false*/, ISpewer *pSpewer /*=g_pDefaultSpewer*/) {
  if (m_bRunning)
    return;
...
void CFileserverCleaner::PrintResult() {
  ...
  m_pSpewer->PrintValue("Number of files removed",
                        Replay_va("%i", nNumFilesRemoved));
...
void CLocalFileDeleterJob::AddFileForDelete(const char *pFilename) {
  CFmtStr fmtFullFilename(
      "%s%s", g_pServerReplayContext->GetLocalFileServerPath(), pFilename);
  ...
  FOR_EACH_VEC(m_vecFiles, i) {
    ...
    if (!g_pFullFileSystem->FileExists(pCurFilename)) {
      ...
      SetError(
          ...
          fmtError.Access()); // TODO: This will only catch the last filename
...
bool SV_DoFileserverCleanup(bool bForceCleanAll, ISpewer *pSpewer) {
  CServerRecordingSessionManager *pSessionManager =
      SV_GetRecordingSessionManager();
  CBaseRecordingSession *pRecordingSession = SV_GetRecordingSessionInProgress();

  for (int i = 0; i < pSessionManager->Count();) {
    CServerRecordingSession *pCurSession =
        SV_CastSession(SV_GetRecordingSessionManager()->m_vecObjs[i]);
    ...
    if (pCurSession != NULL && pCurSession != pRecordingSession) {
      ...
      if (bForceCleanAll || pCurSession->SessionExpired()) {
        // The session's OnDelete() will add the session file to the cleanup
        // system, and also delete all associated blocks, whose OnDelete() will
        // also add their associated .block files to the cleanup system.
