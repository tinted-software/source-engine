...
//
// CSessionPublishManager takes care of all the publishing for a particular
// session. For asynchronous publishing of block and session info data,
// publishing can sometimes overlap between rounds, as is sometimes the case for
// FTP.
//
// A CSessionPublishManager instance are created by passing in the in-progress
// recording session into the constructor.
//
// CSessionRecorder maintains a list of CSessionPublishManager instances and
// cleans them up once all publishing for their corresponding session is
// completed.
//
...
public:
  ...
  // Unlock the associated session - this should only be called if IsDone()
  // returns true.
  void UnlockSession();
