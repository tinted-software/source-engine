...
public:
  ...
  struct ServerRecordingState_t {
    ...
    int m_nDumpInterval; // The interval at which the server is dumping partial
                         // replays
    int m_nCurrentBlock; // Current session block being written to on the server
                         // (approximation - may be ahead but never behind)
    int m_nStartTick; // The tick on the server when the session began - used to
                      // calculate an adjusted spawn tick on the client
...
private:
  ...
  int m_nNumSessionBlockDownloaders; // TODO: Manage the number of session block
                                     // downloaders
};
