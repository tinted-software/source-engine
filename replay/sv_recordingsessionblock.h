...
public:
  ...
  WriteStatus_t
      m_nWriteStatus; // SERVER: initially set to WRITESTATUS_INVALID, then set
                      // to STATUS_WORKING, STATUS_SUCCESS, or STATUS_FAILED,
                      // depending on the state of the write process (which runs
...
inline CServerRecordingSessionBlock *
SV_CastBlock(IReplaySerializeable *pBlock) {
  return static_cast<CServerRecordingSessionBlock *>(pBlock);
}

inline const CServerRecordingSessionBlock *
SV_CastBlock(const IReplaySerializeable *pBlock) {
