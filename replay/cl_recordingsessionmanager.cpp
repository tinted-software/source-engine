...
CClientRecordingSessionManager::CClientRecordingSessionManager(
    IReplayContext *pContext)
    : CBaseRecordingSessionManager(pContext), m_nNumSessionBlockDownloaders(0),
      m_flNextBlockUpdateTime(0.0f), m_flNextPossibleDownloadTime(0.0f) {}
...
CClientRecordingSessionManager::OnSessionStart(int nCurrentRecordingStartTick,
...
void CClientRecordingSessionManager::OnSessionEnd() {
  if (m_pRecordingSession) {
    ...
    AssertMsg(!m_pRecordingSession->m_bRecording,
              "This flag should have been cleared already!  See "
              "CBaseRecordingSession::OnStopRecording()");
...
void CClientRecordingSessionManager::FireGameEvent(IGameEvent *pEvent) {
  ...
  if (!V_stricmp("replay_sessioninfo", pEventName)) {
    ...
    m_ServerRecordingState.m_nCurrentBlock =
        nCurrentBlock +
        1; // This will account for any different between when the server
           // actually dumps a block and the client predicts a dump
    m_ServerRecordingState.m_nStartTick = nStartTick;

    // If the server's in a weird state, disable replay on the client so they
    // don't create any replays that don't play, etc.
    g_pClientReplayContextInternal->DisableReplayOnClient(
  ...
  // When the player dies, we fill out the rest of the data here
           ...
           pEvent->GetInt("victim_entindex") ==
               g_pEngineClient->GetPlayerSlot() + 1 &&
...
void CClientRecordingSessionManager::Think() {
  VPROF_BUDGET("CClientRecordingSessionManager::Think",
  ...
  if (replay_debug.GetBool()) {
    ...
    g_pEngineClient->Con_NPrintf(100, "Time until block dump: ~%f",
                                 m_flNextBlockUpdateTime - flHostTime);
    g_pEngineClient->Con_NPrintf(101, "Post-death record time: %f",
  ...
  if (m_flNextBlockUpdateTime <= flHostTime) {
    ...
    // NOTE: Now the number of blocks in the recording session in progress
    // should be different from the number of blocks in its list - so it should
    // spawn a download thread to grab the session info and create the new
    // block.
    ...
    // Setup next think
    m_flNextBlockUpdateTime =
        flHostTime + m_ServerRecordingState.m_nDumpInterval;
  }
}
...
void CClientRecordingSessionManager::DownloadThink() {
  ...
  // For session in progress - check predicted # of blocks on the server based
  // on current number of blocks in our list - if different, download the
  // session info and create any outstanding blocks on the client.
  bool bEnoughTimeHasPassed = flHostTime >= m_flNextPossibleDownloadTime;
  if (!bEnoughTimeHasPassed)
    return;

  // Go through all sessions to see if we need to create session block
  // downloaders
  FOR_EACH_OBJ(this, i) {
    CClientRecordingSession *pCurSession = CL_CastSession(m_vecObjs[i]);

    // Already have a session block downloader?  NOTE: The think manager calls
    // its Think().
    if (pCurSession->HasSessionInfoDownloader())
      continue;

    // If the # of blocks on the client is out of sync with the number of blocks
    // we need to eventually download, sync with the server, i.e. download the
    // session info and create blocks/sync block data as needed.
  ...
  if (bKickedOffDownload) {
    m_flNextPossibleDownloadTime =
        flHostTime + MAX(MIN_SERVER_DUMP_INTERVAL,
                         CL_GetRecordingSessionManager()
                             ->m_ServerRecordingState.m_nDumpInterval);
  }
}
...
void CClientRecordingSessionManager::OnObjLoaded(
...
void CClientRecordingSessionManager::OnReplayDeleted(CReplay *pReplay) {
  // Notify the session that a replay has been deleted, in case it needs to do
  // any cleanup.
  CClientRecordingSession *pSession =
      CL_CastSession(FindSession(pReplay->m_hSession));
  if (pSession) {
    pSession->OnReplayDeleted(pReplay);
  }

  // Get the # of replays that depend on the given session
  int nNumDependentReplays =
      CL_GetReplayManager()->GetNumReplaysDependentOnSession(
          pReplay->m_hSession);
...
void CClientRecordingSessionManager::OnReplaysLoaded() {
  ...
  FOR_EACH_REPLAY(i) {
    ...
    CClientRecordingSession *pOwnerSession = CL_CastSession(
        CL_GetRecordingSessionManager()->FindSession(pCurReplay->m_hSession));
