...
CSharedReplayContext::CSharedReplayContext(IReplayContext *pOwnerContext)
    : m_pOwnerContext(pOwnerContext), m_pRecordingSessionManager(NULL),
      m_pRecordingSessionBlockManager(NULL), m_pErrorSystem(NULL),
      m_pThreadPool(NULL), m_bInit(false) {}
...
bool CSharedReplayContext::Init(CreateInterfaceFn fnFactory) {
  m_strRelativeBasePath.Format("%s%c%s%c", SUBDIR_REPLAY,
                               CORRECT_PATH_SEPARATOR, m_strSubDir.Get(),
                               CORRECT_PATH_SEPARATOR);

  m_strBasePath.Format("%s%c%s", g_pEngine->GetGameDir(),
                       CORRECT_PATH_SEPARATOR, m_strRelativeBasePath.Get());

  // Owning context should have initialized these by now
  // NOTE: Session manager init must come after block manager init since session
  // manager assumes all blocks have been loaded.
  //
...
bool CSharedReplayContext::InitThreadPool() {
  ...
  // Jon says: The client only really needs a single "ReplayContext" thread, so
  // that the replay editor can write
  //  data asynchronously. The game server does in fact require 4 threads, and
  //  can be configured to use more
  ...
  if (!pThreadPool->Start(ThreadPoolStartParams_t(true, nMaxThreads),
                          "ReplayContext")) {
    Log("failed!\n");
    return false;
  }
