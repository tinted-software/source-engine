...
private:
  ...
  int m_itLastCompletedBlockWrittenToBuffer; // The last block written to the
                                             // buffer that isn't going to
                                             // change state
  CUtlBuffer m_bufSessionInfo; // Growable buffer for the session info block
                               // data (does not include header)

  CServerRecordingSession *m_pSession;

  bool
      m_bShouldPublish; // True if we should publish - can be true while already
                        // publishing.  Means another publish is needed.
};
