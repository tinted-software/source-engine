...
ConVar replay_simulate_expired_sessions(
    ...
    "Simulate expired replay session data - the value of this cvar should be "
    "between 0 and 100 and is a probability - any cleanup done (via end of "
    "round cleanup or explicit replay_docleanup) will use this value to "
    "determine whether data is expired.  E.g, use a value of 100 to delete all "
    "sessions, or 50 for a 50 chance of a given session being considered "
    "expired.",
...
CServerRecordingSession::CServerRecordingSession(IReplayContext *pContext)
    : CBaseRecordingSession(pContext), m_bReplaysRequested(false),
      m_nLifeSpan(0) {}
...
void CServerRecordingSession::SetLocked(bool bLocked) {
  ...
  FOR_EACH_VEC(m_vecBlocks, i) { m_vecBlocks[i]->SetLocked(bLocked); }
}

void CServerRecordingSession::PopulateWithRecordingData(
...
void CServerRecordingSession::VerifyLocks() {
  ...
  FOR_EACH_VEC(m_vecBlocks, i) {
    AssertMsg(m_vecBlocks[i]->IsLocked() == bLocked,
              "Parent/child locks out of sync.  The block probably needs to "
