...
#define SERVER_REPLAY_ERROR_LOST                                               \
  "The server crashed before the replay could be finalized.  Replay lost."

//----------------------------------------------------------------------------------------

CSessionRecorder::CSessionRecorder()
    : m_bRecordingAborted(false), m_nCurrentRecordingStartTick(-1) {}

CSessionRecorder::~CSessionRecorder() {}

bool CSessionRecorder::Init() {
  g_pFullFileSystem->CreateDirHierarchy(
      Replay_va("%s%s", SV_GetBasePath(), SUBDIR_SESSIONS));
  return true;
}
...
void CSessionRecorder::StartRecording() {
  ...
  // We only care about local fileserver path in the case that we aren't
  // offloading files to an external sfileserver
  const char *pWritePath = g_pServerReplayContext->GetLocalFileServerPath();
  if ((!pWritePath || !pWritePath[0])) {
    ConMsg("\n*\n* ERROR: Failed to begin record: make sure "
           "\"replay_local_fileserver_path\" refers to a valid path!\n** "
  ...
  CBaseRecordingSession *pSession =
      SV_GetRecordingSessionManager()->OnSessionStart(
          m_nCurrentRecordingStartTick, NULL);

  // Create a new publish manager and add it.  The dump interval and any
  // additional setup is done there.
  CreateAndAddNewPublishManager(
      static_cast<CServerRecordingSession *>(pSession));
}

void CSessionRecorder::CreateAndAddNewPublishManager(
    CServerRecordingSession *pSession) {
  CSessionPublishManager *pNewPublishManager =
      new CSessionPublishManager(pSession);

  // Let the publish manager know that it is the 'current' publish manager.
  pNewPublishManager->OnStartRecording();

  // Add to the head of the list, since the desired convention is for the list
  // to be sorted from newest to oldest.
...
void CSessionRecorder::UpdateSessionLocks() {
  for (int i = m_lstPublishManagers.Head();
       i != m_lstPublishManagers.InvalidIndex();) {
    ...
    if (pCurManager->IsDone()) {
      ...
      IF_REPLAY_DBG(Warning("\n---\n*\n* All publishing done for session.  %i "
                            "still publishing.\n*\n---\n",
...
void CSessionRecorder::StopRecording(bool bAborting) {
  ...
  if (pSession) {
    ...
    // Notify session manager - the session will be flagged for unload or
    // deletion, but will not actually be free'd until it is "unlocked" by the
    // publish manager.
