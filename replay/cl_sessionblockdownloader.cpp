...
void CSessionBlockDownloader::AbortDownloadsAndCleanup(
    CClientRecordingSession *pSession) {
  // NOTE: sm_nNumCurrentDownloads will be decremented in OnDownloadComplete(),
  // which is invoked by CHttpDownloader::AbortDownloadAndCleanup()
  ...
  FOR_EACH_LL(m_lstDownloaders, i) {
    ...
    CClientRecordingSessionBlock *pBlock =
        (CClientRecordingSessionBlock *)pCurDownloader->GetUserData();
...
void CSessionBlockDownloader::Think() {
  ...
  if (!AtMaxConcurrentDownloads()) {
    // Go through all blocks and begin downloading any that the server index
    // downloader has determined are ready
    CClientRecordingSessionBlockManager *pBlockManager =
        CL_GetRecordingSessionBlockManager();
    FOR_EACH_OBJ(pBlockManager, i) {
      CClientRecordingSessionBlock *pBlock =
          CL_CastBlock(pBlockManager->m_vecObjs[i]);
      ...
      CClientRecordingSession *pSession = CL_CastSession(
          CL_GetRecordingSessionManager()->Find(pBlock->m_hSession));
      if (!pSession) {
        AssertMsg(0, "Session for block not found!  This should never happen!");
        continue;
      }

      // Do we need any blocks at all from this session?  Is this block within
      // range?
      int iLastBlockToDownload = pSession->GetLastBlockToDownload();
      if (iLastBlockToDownload < 0 ||
          pBlock->m_iReconstruction > iLastBlockToDownload) {
        continue;
      }
      ...
      const char *pURL =
          pForceURL[0] ? pForceURL
                       : Replay_va("%s%s", pSession->m_strBaseDownloadURL.Get(),
                                   pFilename);
#else
      const char *pURL =
          Replay_va("%s%s", pSession->m_strBaseDownloadURL.Get(), pFilename);
#endif
      const char *pGamePath =
          Replay_va("%s%s", CL_GetRecordingSessionBlockManager()->GetSavePath(),
                    pFilename);
      pDownloader->BeginDownload(pURL, pGamePath, (void *)pBlock,
                                 &pBlock->m_uBytesDownloaded);

      IF_REPLAY_DBG(Warning("%s block %i from %s to path %s...\n",
                            pBlock->GetNumDownloadAttempts()
      ...
      pBlock->m_nDownloadStatus =
          CClientRecordingSessionBlock::DOWNLOADSTATUS_DOWNLOADING;
...
void CSessionBlockDownloader::OnConnecting(CHttpDownloader *pDownloader) {
  CClientRecordingSessionBlock *pBlock =
      (CClientRecordingSessionBlock *)pDownloader->GetUserData();
  AssertValidReadPtr(pBlock);
  pBlock->m_nDownloadStatus =
      CClientRecordingSessionBlock::DOWNLOADSTATUS_CONNECTING;
}
...
void CSessionBlockDownloader::OnDownloadComplete(CHttpDownloader *pDownloader,
                                                 const unsigned char *pData) {
  // TODO: Compare downloaded byte size (pDownloader->GetBytesDownloaded()) to
  // size in block Write block size into session info on server

  int it = m_lstDownloaders.Find(pDownloader);
  if (it == m_lstDownloaders.InvalidIndex()) {
    AssertMsg(0, "Downloader now found in session block downloader list!  This "
                 "should never happen!");
    return;
  }
  ...
  case HTTP_ABORTED:
    ...
    pBlock->m_nDownloadStatus =
        CClientRecordingSessionBlock::DOWNLOADSTATUS_ABORTED;
    break;
...

  {
    ...
    const bool bSizesDiffer =
        replay_simulate_size_discrepancy.GetBool() ||
        pBlock->m_uFileSize != pDownloader->GetBytesDownloaded();
    const bool bHashFail = replay_simulate_bad_hash.GetBool() ||
                           !pBlock->ValidateData(pData, nSize, aLocalHash);
#else
    const bool bSizesDiffer =
        pBlock->m_uFileSize != pDownloader->GetBytesDownloaded();
    ...
    if (bSizesDiffer) {
      AssertMsg(0, "Number of bytes downloaded differs from size specified in "
                   "session info file.");
      bTryAgain = true;
    } else if (bHashFail) {
    ...
    if (bTryAgain) {
      ...
      CL_GetErrorSystem()->OGS_ReportSessionBlockDownloadError(
          pDownloader, pBlock, pDownloader->GetBytesDownloaded(), m_nMaxBlock,
  ...
  case HTTP_ERROR:
    ...
    // If we've attempted and failed to download the block 3 times, report the
    // error and put the block in error state.
    ...
    // Now that the block is in the error state, the replay's status will be
    // updated to the error state as well (see pSession->UpdateReplayStatuses()
    // below).
    ...
    // Report the error to user & OGS
    {
      ...
      const char *pToken =
          CHttpDownloader::GetHttpErrorToken(pDownloader->GetError());
      CL_GetErrorSystem()->AddFormattedErrorFromTokenName(
          "#Replay_DL_Err_HTTP_Prefix", new KeyValues("args", "err", pToken));
    }

    break;

  default:
    AssertMsg(0, "Invalid download state in "
                 "CSessionBlockDownloader::OnDownloadComplete()");
  }
  ...
  CClientRecordingSession *pSession = CL_CastSession(
      CL_GetRecordingSessionManager()->FindSession(pBlock->m_hSession));
  Assert(pSession);
