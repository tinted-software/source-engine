...
#ifdef _DEBUG
ConVar replay_simulate_long_save("replay_simulate_long_save", "0",
                                 FCVAR_DONTRECORD,
                                 "Simulate a long save.  Seconds.");
#endif
...
public:
  ...
  virtual JobStatus_t DoExecute() {
    ...
    return m_pInData->SaveToFile(g_pFullFileSystem, m_szFullFilename, "MOD")
               ? JOB_OK
               : JOB_FAILED;
  }
...
CPerformanceController::CPerformanceController()
    : m_pRoot(NULL), m_pCurEvent(NULL), m_pDbgRoot(NULL),
      m_pPlaybackHandler(NULL), m_pSetViewEvent(NULL), m_pSaveJob(NULL),
      m_bViewOverrideMode(false), m_bDirty(false), m_bLastSaveStatus(false),
      m_bRewinding(false), m_pSavedPerformance(NULL),
      m_pScratchPerformance(NULL), m_hReplay(REPLAY_HANDLE_INVALID),
      m_nState(STATE_DORMANT), m_pEditor(NULL), m_flLastCamSetViewTime(0.0f),
      m_flTimeScale(1.0f) {}

CPerformanceController::~CPerformanceController() { Cleanup(); }

void CPerformanceController::Cleanup() {
  AssertMsg(!m_pScratchPerformance ||
                m_pScratchPerformance != m_pSavedPerformance,
            "Sanity check failed.  We should never be assigning saved to "
  ...
  FOR_EACH_LL(m_EventQueue, i) { m_EventQueue[i]->deleteThis(); }
...
void CPerformanceController::StartRecording(CReplay *pReplay, bool bSnip) {
  ...
  AssertMsg(m_nState == STATE_PLAYING || !m_pRoot,
  ...
  if (m_nState == STATE_DORMANT) {
    ...
    // Nuke everything after the current event, or does nothing if we've past
    // the end of the playback stream
    if (bSnip) {
      Snip();
    }
...
bool CPerformanceController::DumpStreamToFileAsync(const char *pFullFilename) {
  // m_pRoot can be NULL if the user only set an in and/or out point, and wants
  // to save.
  if (!m_pRoot)
    return true;
...
bool CPerformanceController::SaveAsync() {
  ...
  // NOTE: m_pSavedPerformance should always be valid here, as 'save' is
  // disabled until it has an actual performance to save to.

  // Copy the relevant data from scratch -> saved - we want to preserve the
  // filename the saved performance, and have no reason to copy over duplicate
  // data (eg the replay handle).
...
bool CPerformanceController::SaveAsAsync(const wchar_t *pTitle) {
  ...
  //
  //   * We've already dealt with checking the given title versus existing
  //   performances
  ...
  CReplayPerformance *pExistingPerformance =
      pReplay->GetPerformanceWithTitle(pTitle);
  if (!pExistingPerformance) {
    // Create and add a new performance to the replay with a unique filename -
    // do not generate a title since we will use the incoming title.
    ...
    // If we didn't spawn a thread, we want this to be true here, since the
    // replay flushed and DumpStreamToFileAsync() succeeded.
  ...
  // Overwriting an existing performance?
    ...
    // Performance with the given name already exists - overwrite it (again,
    // this function assumes that any UI around asking the user if they're sure
    // they want to replace has already been navigated, and the user has
    // selected to overwrite).
    m_pSavedPerformance = pExistingPerformance;
  }

  // Copy the title to the scratch
  V_wcsncpy(m_pScratchPerformance->m_wszTitle, pTitle,
            MAX_TAKE_TITLE_LENGTH * sizeof(wchar_t));
...
void CPerformanceController::OnSignonStateFull() {
  ...
  if (m_bRewinding) {
    ...
    AssertMsg(!m_pScratchPerformance,
              "Scratch replay should not be valid yet.");

    // If we've gotten this far and the replay is invalid, we're likely playing
    // back a regular demo and didn't early out somewhere up the chain.
    ...
    CReplayPerformance *pPerformance =
        g_pReplayDemoPlayer->GetCurrentPerformance();
    if (pPerformance) {
      SetupPlaybackFromPerformance(pPerformance);

      // Make a copy of the performance we're playing back so the user can make
      // changes w/o fucking up the original.
...
void CPerformanceController::CreateNewScratchPerformance(CReplay *pReplay) {
  ...
  m_pScratchPerformance =
      CL_GetPerformanceManager()->CreatePerformance(pReplay);

  // Give it a default name
  m_pScratchPerformance->AutoNameIfHasNoTitle(pReplay->m_szMapName);

  // Generate a filename for the new performance
  m_pScratchPerformance->SetFilename(
      CL_GetPerformanceManager()->GeneratePerformanceFilename(pReplay));
}
...
void CPerformanceController::NotifyPauseState(bool bPaused) {
  ...
  if (!bPaused) {
    // Add queued events
    for (int i = m_EventQueue.Tail(); i != m_EventQueue.InvalidIndex();
         i = m_EventQueue.Previous(i)) {
...
void CPerformanceController::Snip() {
  ...
  for (KeyValues *pCurEvent = m_pRoot->GetFirstTrueSubKey();
       pCurEvent != NULL;) {
...
bool CPerformanceController::IsCameraChangeEvent(int nType) const {
  return nType >= EVENTTYPE_CAMERA_CHANGE_BEGIN &&
         nType <= EVENTTYPE_CAMERA_CHANGE_END;
}
...
void CPerformanceController::RemoveDuplicateEventsFromQueue() {
  // Add queued events - only add the most recent camera change event, and the
  // most recent player change event.
...
void CPerformanceController::AddEvent_Camera_Change_FirstPerson(
    float flTime, int nEntityIndex) {
  QUEUE_OR_RECORD_EVENT(pEvent, flTime, EVENTTYPE_CAMERA_CHANGE_FIRSTPERSON);
  pEvent->SetInt("ent", nEntityIndex);
}

void CPerformanceController::AddEvent_Camera_Change_ThirdPerson(
    float flTime, int nEntityIndex) {
...
void CPerformanceController::AddEvent_Camera_ChangePlayer(float flTime,
                                                          int nEntIndex) {
  QUEUE_OR_RECORD_EVENT(pEvent, flTime, EVENTTYPE_CHANGEPLAYER);
  pEvent->SetInt("ent", nEntIndex);
}

void CPerformanceController::AddEvent_Camera_SetView(
    const SetViewParams_t &params) {
  QUEUE_OR_RECORD_EVENT(pEvent, params.m_flTime, EVENTTYPE_CAMERA_SETVIEW);
  pEvent->SetString("pos", Replay_va("%f %f %f", params.m_pOrigin->x,
                                     params.m_pOrigin->y, params.m_pOrigin->z));
  pEvent->SetString("ang", Replay_va("%f %f %f", params.m_pAngles->x,
                                     params.m_pAngles->y, params.m_pAngles->z));
...
bool CPerformanceController::SetupPlaybackHandler() {
  IReplayPerformancePlaybackHandler *pHandler =
      g_pClient->GetPerformancePlaybackHandler();
  if (!pHandler)
    return false;
...
void CPerformanceController::SetupPlaybackExistingStream() {
  // m_pRoot can be NULL here if the user is watching the original replay and
  // has rewound without changing anything.
  if (!m_pRoot)
    return;
...
void CPerformanceController::SetupPlaybackFromPerformance(
    CReplayPerformance *pPerformance) {
  AssertMsg(!m_pSavedPerformance,
            "This probably hit because either SaveNow() or Discard() were not "
            "called.  One of those should always be called on disconnect after "
            "watching or editing a replay.");
  ...
  if (!pPerformance->m_pReplay) {
    AssertMsg(
        0, "Performance passed in with an invalid replay pointer!  This bad!");
    return;
  }
...
void CPerformanceController::ReadSetViewEvent(KeyValues *pEventSubKey,
                                              Vector &origin, QAngle &angles,
                                              float &fov, float *pAccel,
                                              float *pSpeed,
                                              float *pRotFilter) {
...
void CPerformanceController::PlaybackThink() {
  ...
  const CGlobalVarsBase *g_pClientGlobalVariables =
      g_pEngineClient->GetClientGlobalVars();

  const int nReplaySpawnTick = pReplay->m_nSpawnTick;
  Assert(nReplaySpawnTick >= 0);
  const float flCurTime = g_pClientGlobalVariables->curtime -
                          g_pEngine->TicksToTime(nReplaySpawnTick);
  ...
  while (1) {
    ...
    case EVENTTYPE_CAMERA_CHANGE_FIRSTPERSON:
      m_bViewOverrideMode = false;
      m_pPlaybackHandler->OnEvent_Camera_Change_FirstPerson(
          flEventTime, m_pCurEvent->GetInt("ent"));
      break;

    case EVENTTYPE_CAMERA_CHANGE_THIRDPERSON:
      m_bViewOverrideMode = true;
      m_pPlaybackHandler->OnEvent_Camera_Change_ThirdPerson(
          flEventTime, m_pCurEvent->GetInt("ent"));
      break;
    ...
    case EVENTTYPE_CHANGEPLAYER:
      m_pPlaybackHandler->OnEvent_Camera_ChangePlayer(
          flEventTime, m_pCurEvent->GetInt("ent"));
      break;
    ...
    case EVENTTYPE_CAMERA_SETVIEW:
      AssertMsg(
          m_bViewOverrideMode,
          "Camera mode needs to be set before a setview can take effect.");

      if (m_bViewOverrideMode) {
        // Get sample for current time
        ReadSetViewEvent(m_pCurEvent, aOrigin[0], aAngles[0], aFov[0], &flAccel,
                         &flSpeed, &flRotFilter);
        //				g_pEngineClient->Con_NPrintf( 0, "sample
        //0 time: %f", flEventTime );
        m_flLastCamSetViewTime = flEventTime;
        m_pSetViewEvent =
            m_pCurEvent; // Stomp any previous set view - we want the last one
        bShouldCut =
            bShouldCut ||
            m_pCurEvent->GetBool("cut"); // We cut if any set-view event cuts,
                                         // otherwise we will interpolate
      }
      break;
  ...
  if (m_bViewOverrideMode && m_pSetViewEvent) {
    if (bShouldCut) {
      ...
      // Default second sample to first, in case we don't find a sample to
      // interpolate with
      aOrigin[1] = aOrigin[0];
      aAngles[1] = aAngles[0];
      aFov[1] = aFov[0];
      ...
      while (pSearch) {
        ...
        if (pSearch->GetInt("type", EVENTTYPE_INVALID) ==
            EVENTTYPE_CAMERA_SETVIEW) {
          // Found next sample within half a second - calc interpolation
          // parameter & get data
          float flDiff = flSearchTime - m_flLastCamSetViewTime;
          Assert(flDiff > 0.0f);
          if (flDiff > 0.0f) {
            t = clamp((flCurTime - m_flLastCamSetViewTime) / flDiff, 0.0f,
                      1.0f);
            ...
            if (pSearch->GetBool("cut")) {
              const int iSrc =
                  clamp((int)(.5f + t), 0,
                        1); // Round t to 0 or 1, so we set the camera to the
                            // current frame if t < 0.5f, and we set the camera
                            // to the 'cut'/next frame if t >= 0.5.
              aOrigin[2] = aOrigin[iSrc];
              aAngles[2] = aAngles[iSrc];
              aFov[2] = aFov[iSrc];
            } else {
              ReadSetViewEvent(pSearch, aOrigin[1], aAngles[1], aFov[1],
                               &flAccel, &flSpeed, &flRotFilter);
            }
          }
          break;
        }
      ...
      aAngles[2] = Lerp(t, aAngles[0],
                        aAngles[1]); // NOTE: Calls QuaternionSlerp() internally
      aFov[2] = Lerp(t, aFov[0], aFov[1]);
    }

    // Setup current view
    SetViewParams_t params(flEventTime, &aOrigin[2], &aAngles[2], aFov[2],
                           flAccel, flSpeed, flRotFilter);
...
float CPerformanceController::GetNextThinkTime() const { return 0.0f; }
