...
CON_COMMAND(save_replay, "Save a replay of the current life if possible.") {
  ...
  if (!pReplayForCurrentLife || pReplayForCurrentLife->m_bRequestedByUser ||
      pReplayForCurrentLife->m_bSaved) {
  ...
  // Send a message to the server, regardless of whether the player is alive or
  // dead, requesting that a demo be written.  Format a file name with the
  // client's steam id and a timestamp (gpGlobals->tickcount).
  ...
  CClientRecordingSession *pSession = CL_CastSession(
      CL_GetRecordingSessionManager()->Find(pReplayForCurrentLife->m_hSession));
  if (!pSession) {
    AssertMsg(0,
              "Replay points to a non-existent session - should never happen!");
    CL_GetErrorSystem()->AddErrorFromTokenName("#Replay_ReplayBadSession");
    return;
  }

  // Replay for current life is complete (ie, player is dead and replay is ready
  // to be committed)
  if (pReplayForCurrentLife->m_bComplete) {
    CL_GetReplayManager()->CommitPendingReplayAndBeginDownload();
  } else {
    // Mark the replay as requested by the user, so we can commit automatically
    // as soon as the replay is complete (ie when the player dies, etc.).
...
CON_COMMAND(replay_add_fake_replays, "Adds a set of fake replays") {
  if (args.ArgC() < 2) {
    DevMsg("Use \'replay_add_fake_replays\' <num fake replays to add> <today "
           "only>\n");
    return;
  }
...
CON_COMMAND_F(replay_confirmquit,
              "Make sure all replays are rendered before quitting",
              FCVAR_HIDDEN | FCVAR_DONTRECORD) {
  // TODO: Check to see if any replays are downloading - warn user.  If user
  // wants to quit anyway, make sure to set any blocks to not downloaded, save,
  // and delete any files that were only partially downloaded.

  // Unrendered replays? Display the quit confirmation dialog with the option to
  // render all and quit
  if (CL_GetReplayManager()->GetUnrenderedReplayCount() > 0 &&
      g_pClient->OnConfirmQuit()) {
    // Play a sound.
...
CON_COMMAND_F(replay_deleteclientreplays,
              "Deletes all replays from client replay history, as well as all "
              "files associated with each replay.",
              FCVAR_DONTRECORD) {
...
CON_COMMAND_F(replay_removeclientreplay,
              "Remove the replay at the given index.", FCVAR_DONTRECORD) {
  if (args.ArgC() != 2) {
...
CON_COMMAND_F(replay_printclientreplays, "Prints out all client replay info",
              FCVAR_DONTRECORD) {
  FOR_EACH_REPLAY(i) {
    ...
    // TODO: All of this should go into a virtual function in CReplay, rather
    // than some here and some in DumpGameSpecificData()
    ...
    for (int i = 0; i < nScreenshotCount; ++i) {
      ...
      Msg("    screenshot %i:\n", i);
      Msg("      dimensions: w=%i, h=%i\n", pScreenshot->m_nWidth,
          pScreenshot->m_nHeight);
    ...
    for (int i = 0; i < nPerfCount; ++i) {
      ...
      g_pVGuiLocalize->ConvertUnicodeToANSI(pCurPerformance->m_wszTitle,
                                            szTitle, sizeof(szTitle));
      Msg("    performance %i:\n", i);
      Msg("      title: %s\n", szTitle);
      Msg("      ticks: in=%i  out=%i\n", pCurPerformance->m_nTickIn,
          pCurPerformance->m_nTickOut);
...
CON_COMMAND_F(replay_renderunpause, "Unpause Replay rendering.",
              FCVAR_DONTRECORD) {
  if (!CL_GetMovieManager()->IsRendering())
    return;
...
CON_COMMAND_F(replay_printqueuedtakes,
              "Print a list of takes queued for rendering.", FCVAR_DONTRECORD) {
...
CON_COMMAND_F(replay_clearqueuedtakes, "Clear takes from render queue.",
              FCVAR_DONTRECORD) {
