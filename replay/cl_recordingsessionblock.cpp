...
CClientRecordingSessionBlock::CClientRecordingSessionBlock(
    ...
    : CBaseRecordingSessionBlock(pContext), m_uBytesDownloaded(0),
      m_nDownloadStatus(DOWNLOADSTATUS_WAITING), m_nHttpError(HTTP_ERROR_NONE),
      m_bDataInvalid(false), m_nDownloadAttempts(0) {}
...
bool CClientRecordingSessionBlock::Read(KeyValues *pIn) {
  ...
  V_snprintf(m_szFullFilename, sizeof(m_szFullFilename), "%s%s", GetPath(),
             pBlockFile);
...
bool CClientRecordingSessionBlock::ValidateData(
    const void *pData, int nSize, unsigned char *pOutHash /*=NULL*/) const {
  unsigned char aDigest[16];
  if (!g_pEngine->MD5_HashBuffer(aDigest, (const unsigned char *)pData, nSize,
                                 NULL))
    return false;
