...
CHttpDownloader::CHttpDownloader(IDownloadHandler *pHandler)
    : m_pHandler(pHandler), m_flNextThinkTime(0.0f), m_uBytesDownloaded(0),
      m_uSize(0), m_pThreadState(NULL), m_bDone(false),
      m_nHttpError(HTTP_ERROR_NONE), m_nHttpStatus(HTTP_INVALID),
      m_pBytesDownloaded(NULL), m_pUserData(NULL) {}

CHttpDownloader::~CHttpDownloader() { CleanupThreadIfDone(); }
...
bool CHttpDownloader::BeginDownload(const char *pURL, const char *pGamePath,
                                    void *pUserData, uint32 *pBytesDownloaded) {
  if (!pURL || !pURL[0])
    return false;
  ...
  if (pGamePath) {
    ...
    V_AppendSlash(m_pThreadState->absLocalPath,
                  sizeof(m_pThreadState->absLocalPath));
...
void CHttpDownloader::Think() {
  ...
  if (!m_pThreadState->shouldStop) {
    ...
    case HTTP_ERROR:
      ...
      m_uBytesDownloaded = (uint32)m_pThreadState->nBytesCurrent;
      m_uSize =
          m_pThreadState->nBytesTotal; // NOTE: Need to do this here in the case
                                       // that a file is small enough that we
                                       // never hit HTTP_FETCH
      m_bDone = true;
...
// This does not increment the "ErrorCounter" field and should only be called
// from code that eventually calls into OGS_ReportGenericError().
