...
CBaseThinker::~CBaseThinker() { g_pThinkManager->RemoveThinker(this); }

void CBaseThinker::Think() {
  AssertMsg(ShouldThink(), "Thinking before ready - Think() being called "
                           "explicitly?  Let the think manager call Think().");
}
...
void CBaseThinker::PostThink() { m_flNextThinkTime = GetNextThinkTime(); }
