...
const char *Test_GetPath() {
  return Replay_va("%s%c%s%c%s%c", g_pEngine->GetGameDir(),
                   CORRECT_PATH_SEPARATOR, SUBDIR_REPLAY,
...
CTestObj::~CTestObj() { delete m_pTest; }

const char *CTestObj::GetSubKeyTitle() const {
  return Replay_va("test_%i", GetHandle());
}

const char *CTestObj::GetPath() const { return Test_GetPath(); }
...
const char *CTestManager::GetIndexPath() const { return Test_GetPath(); }

int CTestManager::GetVersion() const { return TESTMANAGER_VERSION; }

CTestObj *CTestManager::Create() { return new CTestObj(); }
