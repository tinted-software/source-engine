...
CSessionBlockPublisher::CSessionBlockPublisher(
    CServerRecordingSession *pSession,
    CSessionInfoPublisher *pSessionInfoPublisher)
    : m_pSession(pSession), m_pSessionInfoPublisher(pSessionInfoPublisher) {
  // Cache the dump interval so it can't be modified during a round - doing so
  // would require an update on all clients.
  extern ConVar replay_block_dump_interval;
  m_nDumpInterval =
      MAX(MIN_SERVER_DUMP_INTERVAL, replay_block_dump_interval.GetInt());
...
void CSessionBlockPublisher::WriteAndPublishSessionBlock() {
  ...
  g_pEngine->GetSessionRecordBuffer(
      &pSessionBuffer,
      &nSessionBufferSize); // This will get called the last client disconnects
                            // from the server - but in waiting for players
                            // state we won't have a demo buffer
  if (!pSessionBuffer || nSessionBufferSize == 0)
    return;

  // Create a new block
  CServerRecordingSessionBlock *pNewBlock = SV_CastBlock(
      SV_GetRecordingSessionBlockManager()->CreateAndGenerateHandle());
  if (!pNewBlock) {
  ...
  if (m_pSession->m_nServerStartRecordTick < 0) {
    Warning("Error: Current recording start tick was not properly setup.  "
            "Aborting block write.\n");
    delete pNewBlock;
    return;
  }
  ...
  const char *pFullFilename =
      Replay_va("%s%s_part_%u.%s", SV_GetTmpDir(),
                SV_GetRecordingSessionManager()->GetCurrentSessionName(),
                iCurrentSessionBlock, BLOCK_FILE_EXTENSION);
  V_strcpy(pNewBlock->m_szFullFilename, pFullFilename);
  pNewBlock->m_nWriteStatus =
      CServerRecordingSessionBlock::WRITESTATUS_INVALID; // Must be set here to
                                                         // trigger write
  ...
  // Match the session's lock - the block will be unlocked once recording has
  // stopped and all publishing is complete.
  pNewBlock->SetLocked(true);

  // Commit the replay to the history manager's list
  SV_GetRecordingSessionBlockManager()->Add(pNewBlock);

  // Also store a pointer to the block in the session - NOTE: session will not
  // attempt to free this pointer
  m_pSession->AddBlock(pNewBlock, false);

  // Cache the block temporarily while the binary block itself writes to disk -
  // NOTE: will not attempt to free
...
void CSessionBlockPublisher::GatherBlockData(
    uint8 *pSessionBuffer, int nSessionBufferSize,
  ...
  // If this is the first block, pass in a header to be written.  Otherwise,
  // just write the block.
  ...
  // Note that pBlockCopy is based on pBuffer, which was allocated with
  // nBlockSize PLUS header size - this will not overflow.
...
void CSessionBlockPublisher::PublishBlock(
    CServerRecordingSessionBlock *pBlock) {
  uint8 *pSessionBuffer;
  int nSessionBufferSize;
  if (!g_pEngine->GetSessionRecordBuffer(&pSessionBuffer,
                                         &nSessionBufferSize)) {
  ...
  int nBlockSize;
  GatherBlockData(pSessionBuffer, nSessionBufferSize, pBlock, &pSafeBlockData,
                  &nBlockSize);

  // We've got what we need and can reset the put ptr
  g_pEngine->ResetReplayRecordBuffer();

  AssertMsg(!pBlock->m_pPublisher,
            "No publisher should exist for this block yet!");
  ...
  // Make sure the main thread doesn't unload the block while it's being
  // published
...
void CSessionBlockPublisher::OnPublishComplete(const IFilePublisher *pPublisher,
                                               void *pUserData) {
  CServerRecordingSessionBlock *pBlock =
      (CServerRecordingSessionBlock *)pUserData;
  Assert(pBlock);
  ...
  // Now that m_nWriteStatus has been set in the block, the session info will be
  // updated accordingly the next time PublishThink() is run.

  // Mark the block as dirty since it was modified
  Assert(pBlock->m_nWriteStatus !=
         CServerRecordingSessionBlock::WRITESTATUS_INVALID);
  SV_GetRecordingSessionBlockManager()->FlagForFlush(pBlock, false);

  IF_REPLAY_DBG(
      Warning("Publish complete for block %s\n", pBlock->GetDebugName()));
}

void CSessionBlockPublisher::OnPublishAborted(
    const IFilePublisher *pPublisher) {
...
CServerRecordingSessionBlock *CSessionBlockPublisher::FindBlockFromPublisher(
    const IFilePublisher *pPublisher) {
...
void CSessionBlockPublisher::Think() {
  // NOTE: This member function gets called even if replay is disabled.  This is
  // intentional.
...
void CSessionBlockPublisher::PublishThink() {
  AssertMsg(m_pSession->IsLocked(),
            "The session isn't locked, which means blocks can be being deleted "
  ...
  for (int it = m_lstPublishingBlocks.Head();
       it != m_lstPublishingBlocks.InvalidIndex();) {
    CServerRecordingSessionBlock *pCurBlock = m_lstPublishingBlocks[it];
    ...
    // If set to anything besides InvalidIndex(), it will be removed from the
    // list
    ...
    case CServerRecordingSessionBlock::WRITESTATUS_WORKING: // Do nothing if
                                                            // still writing
      break;
    ...
    if (bWriteBlockInfoToDisk) {
      ...
      Assert(pCurBlock->m_nWriteStatus !=
             CServerRecordingSessionBlock::WRITESTATUS_INVALID);
    ...
    if (itRemove != m_lstPublishingBlocks.InvalidIndex()) {
      ...
      // Removes from the list but doesn't free, since any pointer here points
      // to a block somewhere
...
void CSessionBlockPublisher::Validate() {
  FOR_EACH_LL(m_lstPublishingBlocks, i) {
    CServerRecordingSessionBlock *pCurBlock = m_lstPublishingBlocks[i];
    Assert(pCurBlock->m_nRemoteStatus ==
           CBaseRecordingSessionBlock::STATUS_READYFORDOWNLOAD);
  }
}
#endif
