...
CON_COMMAND(replay_docleanup,
            "Deletes stale session data from the fileserver.  "
            "\"replay_docleanup force\" will remove all replay session data.") {
  ENSURE_DEDICATED();
  ...
  if (args.ArgC() == 2) {
    if (!V_stricmp(args[1], "force")) {
      ...
      ConMsg("\n   ** ERROR: '%s' is not a valid paramter - use 'force' to "
             "force clean all replay session data.\n\n",
             args[1]);
      return;
    }
  }
...
CON_COMMAND_F(replay_dopublishtest,
...
void PrintSessionInfo(const char *pFilename) {
  ...
  char szDigestStr[33];
  V_binarytohex(header.m_aHash, sizeof(header.m_aHash), szDigestStr,
                sizeof(szDigestStr));
  ...
  Msg("\t%27s: %s\n", "currently recording?",
      header.m_bRecording ? "yes" : "no");
  Msg("\t%27s: %i\n", "# blocks", header.m_nNumBlocks);
  Msg("\t%27s: %s\n", "compressor",
      GetCompressorNameSafe(header.m_nCompressorType));
  ...
  if (!g_pEngine->MD5_HashBuffer(header.m_aHash, (const uint8 *)pPayload,
  ...
  if (header.m_nCompressorType == COMPRESSORTYPE_INVALID) {
    // The payload is already uncompressed - don't free, since this buffer was
    // allocated by the CUtlBuffer "buf"
    pUncompressedPayload = pPayload;
    bFreeUncompressedPayload = false;
  } else {
    if (uUncompressedPayloadSize != header.m_uPayloadSizeUC) {
      Msg("Decompressed to a different size (%u) than specified by header "
          "(%u)\n",
    ...
    pCompressor->Decompress((char *)pUncompressedPayload,
  ...
  if (uUncompressedPayloadSize <= MIN_SESSION_INFO_PAYLOAD_SIZE) {
    ...
    CUtlBuffer bufPayload(pUncompressedPayload, uUncompressedPayloadSize,
                          CUtlBuffer::READ_ONLY);

    Msg("\n\tblocks:\n\n");
    Msg("\t   index        status                        MD5               "
        "compressor    size (uncompressed)  size (compressed)\n");
    ...
    for (int i = 0; i < header.m_nNumBlocks; ++i) {
      ...
      V_binarytohex(DummyBlock.m_aHash, sizeof(DummyBlock.m_aHash), szDigestStr,
                    sizeof(szDigestStr));
      ...
      Msg("%20s", CBaseRecordingSessionBlock::GetRemoteStatusStringSafe(
                      (CBaseRecordingSessionBlock::RemoteStatus_t)
                          DummyBlock.m_uRemoteStatus));
      Msg("%35s", szDigestStr);
      Msg("%8s", GetCompressorNameSafe(
                     (CompressorType_t)DummyBlock.m_nCompressorType));
