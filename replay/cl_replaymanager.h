...
public:
  ...
  CReplay *m_pReplayThisLife; // Valid only between replay completion (ie player
                              // death) and player respawn, otherwise NULL
...
private:
  ...
  CReplay *m_pPendingReplay; // This is the replay we're currently recording -
                             // one which will either be committed (via
                             // Commit()) or not, depending on whether the
                             // player chooses to save the replay.

  CReplay *m_pReplayLastLife; // The previous life (ie between the player's
                              // previous spawn and the current spawn), if any
                              // (otherwise NULL)
