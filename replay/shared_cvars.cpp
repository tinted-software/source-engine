...
void OnReplayEnableChanged(IConVar *pVar, const char *pOldValue,
                           float flOldValue);
void OnReplayRecordingChanged(IConVar *pVar, const char *pOldValue,
                              float flOldValue);

//----------------------------------------------------------------------------------------

// Replicated
ConVar replay_enable("replay_enable", "0", FCVAR_REPLICATED | FCVAR_DONTRECORD,
                     "Enable Replay recording on server", true, 0, true, 1,
                     OnReplayEnableChanged);
ConVar replay_recording("replay_recording", "0",
                        FCVAR_REPLICATED | FCVAR_DONTRECORD | FCVAR_HIDDEN, "",
                        true, 0, true, 1, OnReplayRecordingChanged);

ConVar replay_flushinterval("replay_flushinterval", "15",
                            FCVAR_DONTRECORD | FCVAR_ARCHIVE,
                            "Replay system will flush to disk a maximum of "
                            "every replay_flushinterval seconds.",
                            true, 1.0f, true, 60.0f);
...
void OnReplayEnableChanged(IConVar *pVar, const char *pOldValue,
  ...
  {
          var.SetValue( 0 );
          Warning( "Error: SourceTV is enabled.  Please disable SourceTV if you
  wish to enable Replay.\n" ); return;
  }
  */
  ...
  if (nNewValue) {
    ...
    // Reset value - note that the recursion depth counter will keep this from
    // being dumb.
    var.SetValue(0);
