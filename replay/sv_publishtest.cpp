...
private:
  bool Test_Emptyness(const char *pDescription, const char *pStr,
                      bool bPrintResult);
  bool Test_Hostname(const char *pHostname, const char *pProtocolExample);
  bool Test_Protocol(const char *pDescription, const char *pProtocol,
                     const char **pAcceptableProtocols);
  bool Test_Port(int nPort);
  bool Test_Path(const char *pDescription, const char *pPath,
                 bool bForwardSlashesAllowed, bool bBackslashesAllowed);
  bool Test_LocalWebServerCVars();
  bool Test_IO(const char *pFilename);
  bool Test_FilePublish(const char *pFilename, bool bOffload);
  bool Test_PublishedFileDelete(const char *pFullFilename,
                                const char *pFilename, bool bOffload);
...
CPublishTester::CPublishTester()
    : m_pGarbageBuffer(NULL), m_pJob(NULL), m_pCleanerJob(NULL) {
...
bool CPublishTester::Test_Hostname(const char *pHostname,
                                   const char *pProtocolExample) {
  if (!Test_Emptyness("Hostname", pHostname, false))
    return false;

  if (V_strstr(pHostname, "://")) {
    g_pBlockSpewer->PrintEventResult(false);
    CFmtStr fmtError("Should not contain a protocol (e.g: %s)!",
                     pProtocolExample);
...
bool CPublishTester::Test_Emptyness(const char *pDescription, const char *pStr,
                                    bool bPrintResult) {
...
bool CPublishTester::Test_Path(const char *pDescription, const char *pPath,
                               bool bForwardSlashesAllowed,
...
bool CPublishTester::Test_Protocol(const char *pDescription,
...
bool CPublishTester::Test_LocalWebServerCVars() {
  // NOTE: We use the raw cvar here as opposed to
  // CServerReplayContext::GetLocalFileServerPath(), which actually fixes
  // slashes.  If the cvar is using incorrect slashes here for the given OS,
  // this test will fail with a specific error message.
  extern ConVar replay_local_fileserver_path;
  if (!Test_Path("Path", replay_local_fileserver_path.GetString(), IsPosix(),
                 IsWindows()))
    return false;
...
bool CPublishTester::Test_IO(const char *pFilename) {
  ...
  if (g_pFullFileSystem->Write(m_pGarbageBuffer, GARBAGE_BUFFER_SIZE,
                               hTmpFile) != GARBAGE_BUFFER_SIZE) {
    g_pBlockSpewer->PrintEventResult(false);
    return false;
  }
  g_pBlockSpewer->PrintEventResult(true);
...
bool CPublishTester::Test_PublishedFileDelete(const char *pFullFilename,
  ...
  IFileserverCleanerJob *pCleanerJobImp =
      SV_CastJobToIFileserverCleanerJob(m_pCleanerJob);
...
bool CPublishTester::Test_WaitingForPlayersCVar() {
  ...
  if (mp_waitingforplayers_cancel.IsValid() &&
      mp_waitingforplayers_cancel.GetBool()) {
    g_pBlockSpewer->PrintEventError(
...
bool CPublishTester::Go() {
  ...
  // Fileserver convars
  g_pBlockSpewer->PrintTestHeader(
      "Testing Fileserver ConVars (replay_fileserver_*)");

  // Test replay_fileserver_protocol
  extern ConVar replay_fileserver_protocol;
  if (!Test_Protocol("Protocol", replay_fileserver_protocol.GetString(),
                     g_pAcceptableFileserverProtocols))
    return false;

  extern ConVar replay_fileserver_host;
  if (!Test_Hostname(replay_fileserver_host.GetString(),
                     "\"http\" or \"https\""))
    return false;
  ...
  CFmtStr fmtTmpFilenameFullPath("%s%s", SV_GetTmpDir(),
                                 fmtTmpFilename.Access());
