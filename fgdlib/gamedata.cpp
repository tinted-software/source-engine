...
//allocated buffer 				pointer to non-NULL buffer - token will be placed in buffer
//			ttexpecting -
//			pszExpecting -
// Output :
//-----------------------------------------------------------------------------
...
//discard the token. 			ttexpecting - The token type that we are expecting. If this
//is not TOKENNONE 				and token type read is different, the operation will fail.
//			pszExpecting - The token string that we are expecting.
//If this string 				is not NULL and the token string read is different, the
//operation will fail.
      ...
      if (!pNewClass->InitFromTokens(tr, this)) {
                       ...
                       "npcclass")) // NPC class - can be spawned by npc_maker.
        {
          ...
          pNewClass->SetNPCClass(true);
                       ...
                       szToken,
                       "filterclass")) // Filter class - can be used as a filter
        {
          pNewClass->SetPointClass(true);
...
//fields 			Origin - the origin offset of the instance 			Angles - the angle rotation
//of the instance
...
//parameter is true. 				this is generally only false on the instance level.
