...
/**
 * Utility class for providing some basic capabilities for enumerating and
 * specifying MOD directories.
 *
 * \version 1.0
 *
 * \date 07-18-2006
...
void ModConfigsHelper::setSourceModBaseDir() {
  ...
  Q_StripLastDir(
      m_sourceModBaseDir,
      sizeof(m_sourceModBaseDir)); // Get rid of the 'sourcesdk' directory.
  Q_StripLastDir(
      m_sourceModBaseDir,
      sizeof(m_sourceModBaseDir)); // Get rid of the '%USER%' directory.
  Q_strncat(m_sourceModBaseDir, "SourceMods", sizeof(m_sourceModBaseDir),
            COPY_ALL_CHARACTERS); // Add 'SourceMods'
}

/**
 * Searches the parent MOD directory for child MODs and puts their names in the
 * member vector
 */
  ...
  do {
    if (wfd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
      if (wfd.cFileName[0] == '.') {
        ...
        // They are directories not named '.' or '..' so add them to the list of
        // mod directories
