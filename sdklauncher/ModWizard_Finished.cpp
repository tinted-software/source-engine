...
CModWizardSubPanel_Finished::CModWizardSubPanel_Finished(Panel *parent,
...
void CModWizardSubPanel_Finished::GetReady(const char *pOutputDirName) {
  ...
  if (formatStr) {
    ...
    g_pVGuiLocalize->ConvertANSIToUnicode(m_OutputDirName, tempStr,
                                          sizeof(tempStr));
    g_pVGuiLocalize->ConstructString(labelStr, sizeof(labelStr), formatStr, 1,
                                     tempStr);
    m_pFinishedText->SetText(labelStr);
  }
}

WizardSubPanel *CModWizardSubPanel_Finished::GetNextSubPanel() { return NULL; }
