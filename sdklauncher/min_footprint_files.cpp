  ...
  if (fpSrc) {
    ...
    if (fpDest) {
      while (1) {
        char tempData[4096];
        int nBytesRead =
            g_pFullFileSystem->Read(tempData, sizeof(tempData), fpSrc);
        if (nBytesRead)
...
public:
  ...
  void OnTick() {
    ...
    while (nCopied < 10) {
      ...
      // If the dest file doesn't exist or it's read-only, copy our file over
      // it.
      if ((pFilename->m_Flags & MF_ALWAYS_COPY) ||
          (_access(pFilename->m_DestFilename, 2) != 0)) {
  ...
  void ShowDeprecatedAppIDNotice() {
    // Try to open the file that indicates that we've already been through this
    // check
    FileHandle_t fp =
        g_pFullFileSystem->Open(SHOW_DEPRECATED_APP_ID_MARKER, "rb");
    ...
    // We have not checked for outdated mods yet...
      ...
      //
      // Look for all of the mods under 'SourceMods', check if the SteamAppId is
      // out of date, and warn the user if this is the case
      //
      ...
      Q_strncpy(szProblemMods,
                "The SteamAppId values for the following mods should be "
                "changed from 220 to 215:\r\n\r\n",
                5 * MAX_PATH);
      ...
      for (int i = 0; i < modDirs.Count(); i++) {
        ...
        Q_strncat(szGameConfigPath, "\\gameinfo.txt", MAX_PATH,
                  COPY_ALL_CHARACTERS);
        FileHandle_t fpGameInfo =
            g_pFullFileSystem->Open(szGameConfigPath, "rb");
        ...
        if (fpGameInfo) {
          ...
          if (NULL != pFileSystemInfo) {
            ...
            // This is the one that needs replacing add this mod to the list of
            // suspect mods
            if (GetAppSteamAppId(k_App_HL2) == iAppId) {
              bProblemModExists = true;
              Q_strncat(szProblemMods, modDirs[i], MAX_PATH,
                        COPY_ALL_CHARACTERS);
      ...
      if (bProblemModExists) {
        ...
        Q_strncat(szProblemMods,
                  "\r\nIf you did not author any of the above mods you can "
                  "ignore this message.\r\nIf you did author any of the above "
                  "mods you should change the SteamAppId\nvalue in the "
                  "gameinfo.txt file for each mod listed above.\r\n",
...
void AddMinFootprintFile(CUtlVector<CMinFootprintFilename> &filenames,
                         const char *pSrcFilename, const char *pDestFilename,
                         int flags) {
  // Just copy this one file.
  CMinFootprintFilename *pOutputFilename = &filenames[filenames.AddToTail()];
  Q_strncpy(pOutputFilename->m_SrcFilename, pSrcFilename,
            sizeof(pOutputFilename->m_SrcFilename));
  Q_strncpy(pOutputFilename->m_DestFilename, pDestFilename,
            sizeof(pOutputFilename->m_DestFilename));
  pOutputFilename->m_Flags = flags;
}

void GetMinFootprintFiles_R(CUtlVector<CMinFootprintFilename> &filenames,
                            const char *pSrcDirName, const char *pDestDirName) {
  // pDirName\*.*
  ...
  const char *pFilename = g_pFullFileSystem->FindFirstEx(
      wildcard, SDKLAUNCHER_MAIN_PATH_ID, &findHandle);
  while (pFilename) {
    if (Q_stricmp(pFilename, ".") != 0 && Q_stricmp(pFilename, "..") != 0) {
      char fullSrcFilename[MAX_PATH], fullDestFilename[MAX_PATH];
      Q_snprintf(fullSrcFilename, sizeof(fullSrcFilename), "%s%c%s",
                 pSrcDirName, CORRECT_PATH_SEPARATOR, pFilename);
      Q_snprintf(fullDestFilename, sizeof(fullDestFilename), "%s%c%s",
                 pDestDirName, CORRECT_PATH_SEPARATOR, pFilename);

      if (g_pFullFileSystem->FindIsDirectory(findHandle)) {
        CTempDirName *pOut = &subDirs[subDirs.AddToTail()];
        Q_strncpy(pOut->m_SrcDirName, fullSrcFilename,
                  sizeof(pOut->m_SrcDirName));
        Q_strncpy(pOut->m_DestDirName, fullDestFilename,
                  sizeof(pOut->m_DestDirName));
      } else {
  ...
  for (int i = 0; i < subDirs.Count(); i++) {
    GetMinFootprintFiles_R(filenames, subDirs[i].m_SrcDirName,
                           subDirs[i].m_DestDirName);
  }
}
...
void DumpMinFootprintFiles(bool bForceRefresh) {
  ...
  if (!bValidFile) {
    VGUIMessageBox((vgui::Frame *)g_pMainFrame, "#Warning",
                   "#CantLoadMinFootprintFiles");
    kv->deleteThis();
    return;
  }
  ...
  CMinFootprintFilesPanel *pDisplay = new CMinFootprintFilesPanel(
      (vgui::Frame *)g_pMainFrame, "MinFootprintFilesPanel");
  pDisplay->m_iOutputVersionNumber = latestVersion;

  // Our version is out of date, let's copy out all the min footprint files.
  // NOTE: copy files that are set to always_copy whether the version changed or
  // not.
  for (KeyValues *pCur = pFileList->GetFirstSubKey(); pCur;
    ...
    if ((Q_stricmp(pCur->GetName(), "mapping") == 0) && bVersionChanged) {
      ...
      Q_snprintf(destDirTemp, sizeof(destDirTemp), "%s%c%s",
                 GetSDKLauncherBaseDirectory(), CORRECT_PATH_SEPARATOR,
                 pDestMapping);
      ...
      Q_snprintf(destFileTemp, sizeof(destFileTemp), "%s%c%s",
                 GetSDKLauncherBaseDirectory(), CORRECT_PATH_SEPARATOR,
                 pDestMapping);
      Q_MakeAbsolutePath(destFile, sizeof(destFile), destFileTemp);

      if (bVersionChanged || (flags & MF_ALWAYS_COPY)) {
        AddMinFootprintFile(pDisplay->m_Filenames, pDestMapping, destFile,
                            flags);
      }
    } else if ((Q_stricmp(pCur->GetName(), "create_directory") == 0) &&
               bVersionChanged) {
