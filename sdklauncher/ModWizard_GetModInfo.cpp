...
// ----------------------------------------------------------------------------------------
// // Directory select dialog that preserves the modalness of the previous
// dialog.
// ----------------------------------------------------------------------------------------
// //
...
CModWizardSubPanel_GetModInfo::CModWizardSubPanel_GetModInfo(
...
WizardSubPanel *CModWizardSubPanel_GetModInfo::GetNextSubPanel() {
  ...
  if (m_ModType == ModType_FromScratch && !V_strcmp(g_engineDir, "source2007"))
    return dynamic_cast<WizardSubPanel *>(GetWizardPanel()->FindChildByName(
        "CModWizardSubPanel_TemplateOptions"));
  else
    return dynamic_cast<WizardSubPanel *>(
        GetWizardPanel()->FindChildByName("CModWizardSubPanel_CopyFiles"));
}
...
bool CModWizardSubPanel_GetModInfo::OnNextButton() {
  ...
  if (modPath[0] == 0) {
    ...
    VGUIMessageBox(
        this, "Error",
        "Please enter a full path (like C:\\MyMod) to install the mod.");
    return false;
  }
  ...
  if (m_ModType == ModType_SourceCodeOnly) {
    ...
    for (int i = 0; i < modNameLen; i++) {
      if (!isalnum(modName[i]) && modName[i] != '-' && modName[i] != '_' &&
          modName[i] != ' ') {
    ...
    // Tony; after the other check for invalid characters, make the actual
    // outputModGamedirName lowercase with spaces stripped out.
    char strippedModName[512]; // this is what gets thrown on below!
    // Tony; reuse modNameLen when I copy it to the new string.
    // I could have sworn there was an easier way to do this, but it completely
    // escapes me.
    int i, j;
    ...
    // Build the name of the content directory
    // (c:\steam\steamapps\sourcemods\modname).
    Q_strncpy(outputModGamedirName, GetSDKLauncherBaseDirectory(),
              sizeof(outputModGamedirName)); // steamapps\username\sourcesdk

    Q_StripLastDir(outputModGamedirName,
                   sizeof(outputModGamedirName)); // steamapps\username
    Q_StripLastDir(outputModGamedirName,
                   sizeof(outputModGamedirName)); // steamapps
    ...
    Q_AppendSlash(outputModGamedirName, sizeof(outputModGamedirName));
    Q_strncat(outputModGamedirName, strippedModName,
              sizeof(outputModGamedirName),
    ...
    if (!CreateFullDirectory(outputModGamedirName) ||
      ...
      VGUIMessageBox(
          this, "Error",
          "Unable to create directory '%s' or one if its subdirectories.",
          modPath);
      return false;
    }
  }
  ...
  CModWizardSubPanel_CopyFiles *pCopyPanel =
      dynamic_cast<CModWizardSubPanel_CopyFiles *>(
          GetWizardPanel()->FindChildByName("CModWizardSubPanel_CopyFiles"));
  if (!pCopyPanel) {
...
void CModWizardSubPanel_GetModInfo::OnCommand(const char *command) {
  if (Q_stricmp(command, "SearchButton") == 0) {
    CModalPreserveDirectorySelectDialog *pDlg =
        vgui::SETUP_PANEL(new CModalPreserveDirectorySelectDialog(
