...
//-----------------------------------------------------------------------------
// Purpose: Retrieves a URL out of the external localization file and opens it
// in a
//			web browser.
// Input  : *lpszLocalName - Localized name of the URL to open via shell
// execution
//-----------------------------------------------------------------------------
    ...
    if (Q_stricmp(command, "ResetConfigs") == 0) {
      ...
      if (GetVParent()) {
        PostMessage(GetVParent(),
                    new KeyValues("Command", "command", "ResetConfigs"));
      }
...
public:
  ...
  void OnCommand(const char *command) {
    ...
    if (Q_stricmp(command, "Continue") == 0) {
      PostMessage(g_pSDKLauncherDialog,
                  new KeyValues("Command", "command", "RefreshMinFootprint"));
      MarkForDeletion();
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CSDKLauncherDialog::CSDKLauncherDialog(vgui::Panel *parent, const char *name)
...
void CSDKLauncherDialog::PopulateMediaList() {
  ...
  if (dataFile->LoadFromFile(g_pFullFileSystem, "MediaList.txt", NULL)) {
    ...
    for (KeyValues *pCur = pImages->GetFirstTrueSubKey(); pCur;
         pCur = pCur->GetNextTrueSubKey()) {
    ...
    for (KeyValues *pSection = pSections->GetFirstTrueSubKey(); pSection;
         pSection = pSection->GetNextTrueSubKey()) {
      int id = pSection->GetInt("id");
      const char *pName = pSection->GetString("Name");
      m_pMediaList->AddSection(id, pName);
      m_pMediaList->AddColumnToSection(id, "Image", "",
                                       SectionedListPanel::COLUMN_IMAGE, 20);
      m_pMediaList->AddColumnToSection(id, "Text", pName, 0, 200);

      // Set all the rows.
      for (KeyValues *kv = pSection->GetFirstTrueSubKey(); kv != NULL;
...
void CSDKLauncherDialog::Launch(int hActiveListItem, bool bForce) {
  ...
  if (pStr) {
    if (Q_stricmp(pStr, "CreateMod") == 0) {
      if (!V_stricmp(g_engineDir, "ep1") ||
          !V_stricmp(g_engineDir, "source2007")) {
        RunCreateModWizard(false);
      } else {
        VGUIMessageBox(this, "Unable to Run 'Create A Mod' Wizard",
                       "Support for creating total conversions are not "
                       "available using this engine versions.");
      }
      ...
      CResetConfirmationMessageBox *pDlg =
          new CResetConfirmationMessageBox(this, "ResetConfirmation");
  ...
  if (pStr) {
    ...
    if (!bForce && Q_stristr(pStr, "%gamedir%")) {
      // Make sure the currently-selected gamedir is valid and has a
      // gameinfo.txt in it.
      ...
      if (_access(testStr, 0) != 0) {
        CGameInfoMessageBox *dlg =
            new CGameInfoMessageBox(this, "GameInfoMessageBox");
    ...
    if (bRun) {
      ...
      V_StrSubst(programNameTemp, "%gamedir%", pModDir, programName,
                 sizeof(programName));
      V_strncpy(programNameTemp, programName, sizeof(programNameTemp));
      V_StrSubst(programNameTemp, "%enginedir%", g_engineDir, programName,
                 sizeof(programName));

      V_strncpy(launchDirectory, GetSDKLauncherBaseDirectory(),
                sizeof(launchDirectory));
      ...
      if (NULL != V_strstr(programName, "-tools")) {
        ...
        if (!V_strcmp(g_engineDir, "ep1")) {
          VGUIMessageBox(this, "Error",
                         "Source Engine Tools is not compatible with the "
                         "selected engine version.");
          return;
        }

        // If we are running the engine tools then change our launch directory
...
bool CSDKLauncherDialog::ParseConfigs(CUtlVector<CGameConfig *> &configs) {
  ...
  for (KeyValues *pGame = gameBlock->GetFirstTrueSubKey(); pGame;
...
//-----------------------------------------------------------------------------
// Purpose: Selection has changed in the active config drop-down, set the
// environment
//-----------------------------------------------------------------------------
...
void CSDKLauncherDialog::RefreshConfigs(void) {
  ...
  if (g_ConfigManager.LoadConfigs(szGameConfigDir) == false) {
    ...
    if (g_ConfigManager.WasConvertedOnLoad()) {
      ...
      CConversionInfoMessageBox *pDlg =
          new CConversionInfoMessageBox(this, "ConversionInfo");
...
void CSDKLauncherDialog::ResetConfigs(void) {
  ...
  // Notify the user
  VGUIMessageBox(this, "Complete",
                 "Your game configurations have successfully been reset to "
