...
char const *CTestCase::GetName() { return m_pName; }
...
char const *CTestSuite::GetName() { return m_pName; }
...
void CTestSuite::AddTest(ITestCase *pTest) {
  ...
  if (!m_ppTestCases) {
    ...
    m_ppTestCases = (ITestCase **)realloc(
        m_ppTestCases, (m_TestCount + 1) * sizeof(ITestCase **));
  }
...
void UnitTestInstallTestCase(ITestCase *pTest) {
  ...
  if (s_TestCount == s_TestAllocated) {
    if (!s_ppTestCases) {
      ...
      s_ppTestCases = (ITestCase **)realloc(
          s_ppTestCases, s_TestAllocated * 2 * sizeof(ITestCase **));
      s_TestAllocated *= 2;
    }
  }
...
int UnitTestCount() { return s_TestCount; }
