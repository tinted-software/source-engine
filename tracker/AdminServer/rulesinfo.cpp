...
CRulesInfo::CRulesInfo(IResponse *target, serveritem_t &server) {
  ...
  m_pQuery->AddMessageHandler(new CRulesInfoMsgHandlerDetails(
      this, CMsgHandler::MSGHANDLER_ALL, &bytecode));
}

CRulesInfo::~CRulesInfo() { delete m_pQuery; }
...
void CRulesInfo::UpdateServer(netadr_t *adr,
...
void CRulesInfo::Refresh() { Query(); }

bool CRulesInfo::IsRefreshing() { return m_bIsRefreshing; }

serveritem_t &CRulesInfo::GetServer() { return m_Server; }
...
CUtlVector<vgui::KeyValues *> *CRulesInfo::Rules() { return m_vRules; }