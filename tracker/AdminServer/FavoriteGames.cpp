...
void CFavoriteGames::LoadFavoritesList(KeyValues *favoritesData,
                                       bool loadrcon) {
  // load in favorites
  for (KeyValues *dat = favoritesData->GetFirstSubKey(); dat != NULL;
       dat = dat->GetNextKey()) {
    serveritem_t server;
    ...
    v_strncpy(server.gameDir, dat->GetString("gamedir"),
              sizeof(server.gameDir));
    server.players = dat->GetInt("players");
    server.maxPlayers = dat->GetInt("maxplayers");

    if (loadrcon) {
      v_strncpy(server.rconPassword, dat->GetString("rconpassword"),
                sizeof(server.rconPassword));
    }
...
void CFavoriteGames::UpdateServer(serveritem_t &serverIn) {
  for (int i = 0; i < m_Servers.ServerCount(); i++) {
    ...
    if (!memcmp(serverOut.ip, serverIn.ip, sizeof(serverIn.ip)) &&
      ...
      v_strncpy(serverOut.rconPassword, serverIn.rconPassword,
                sizeof(serverOut.rconPassword));
    }
  }
}

//-----------------------------------------------------------------------------
// Purpose: saves the current list of servers to the favorites section of the
// data file
//-----------------------------------------------------------------------------
...
void CFavoriteGames::OnBeginConnect() {
  ...
  // get the server
  int serverIndex =
      m_pGameList->GetDataItem(m_pGameList->GetSelectedRow(0))->userData;
...
//-----------------------------------------------------------------------------
// Purpose: called when a server response has timed out, treated just like a
// normal server
//-----------------------------------------------------------------------------
...
void CFavoriteGames::OnOpenContextMenu(int row) {
  ...
  if (m_pGameList->GetNumSelectedRows()) {
    ...
    menu->AddMenuItem("RemoveServer", "R&emove server from favorites",
  ...
  menu->AddMenuItem("AddServerByName", "&Add server by IP address",
...
void CFavoriteGames::ImportFavorites() {
  ...
  if (vgui::system()->GetRegistryString(
          "HKEY_LOCAL_MACHINE\\Software\\Valve\\Half-life\\InstallPath", name,
          sizeof(name))) {
    // attach name of favorites file
    strcat(name, "\\favsvrs.dat");
  }
  // check if counterstrike is installed
  else if (vgui::system()->GetRegistryString(
               "HKEY_LOCAL_MACHINE\\Software\\Sierra "
               "OnLine\\Setup\\CSTRIKE\\Directory",
               name, sizeof(name))) {
    // attach name of favorites file
    strcat(name, "\\favsvrs.dat");
  ...
  m_pImportFavoritesdlg = new MessageBox(
      "Updating Favorites",
      "Transferring your Favorites. This may take a minute...", this);
  ...
  // post the message with a delay so that the box will display before the
  // command is executed
...
void CFavoriteGames::OnImportFavoritesFile() {
  ...
  if (fileSize == 0) {
    MessageBox *dlg =
        new MessageBox("Unable to load favorites", "Error loading file.", this);
    dlg->DoModal();
...
bool CFavoriteGames::CheckForCorruption(char *data, const char *checkString) {
  if (strcmp(data, checkString) != 0) {
    MessageBox *dlg = new MessageBox("Unable to load favorites",
                                     "Error loading. File may be corrupt.");
    dlg->DoModal();
    return true;
  }
  return false;
...
//-----------------------------------------------------------------------------
// Purpose: Parse throught the details of a server and populate a serveritem_t
// structure
...
bool CFavoriteGames::LoadAFavoriteServer(CUtlBuffer &buf) {
  ...
  fav.gameDescription[strlen(fav.gameDescription) - 1] =
...
MessageMapItem_t CFavoriteGames::m_MessageMap[] = {
    ...
    MAP_MESSAGE_INT(CFavoriteGames, "RefreshServer", OnRefreshServer,
                    "serverID"),
    MAP_MESSAGE_INT(CFavoriteGames, "OpenContextMenu", OnOpenContextMenu,
                    "row"),
};
