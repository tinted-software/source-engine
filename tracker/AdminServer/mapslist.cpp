...
typedef enum { FS, PAK } MAP_TYPES;

CMapsList::CMapsList(IResponse *target, serveritem_t &server,
                     const char *rconPassword, const char *mod) {
...
CMapsList::~CMapsList() { delete m_pRcon; }
...
void CMapsList::ServerResponded() {
  ...
  if (strstr(store, check)) { // if the name of the mod dir is in the return
                              // list its old style format
    checkDir = true;
  }
  ...
  while (cur != NULL) {
    ...
    if (strncmp(cur, "Dir:", 4) && strncmp(cur, "-------------", 13)) {
      ...
      if (mapsLine.CountToken() >= 2) {
        ...
        for (k = 0; k < m_MapsList.Count();
             k++) // now check it doesn't already exist inside the map list...
        {
...
bool CMapsList::IsRefreshing() { return m_bIsRefreshing; }
