...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CServerList::CServerList(IServerRefreshResponse *target)
    : m_Queries(0, MAX_QUERY_SOCKETS, QueryLessFunc) {
  ...
  if (!vgui::system()->GetRegistryString(
          "HKEY_CURRENT_USER\\Software\\Valve\\Tracker\\Rate", speedBuf,
          sizeof(speedBuf) - 1)) {
...
//-----------------------------------------------------------------------------
// Purpose: returns true if the server list is still in the process of talking
// to servers
//-----------------------------------------------------------------------------
...
void CServerList::ServerResponded() {
  for (int i = 0; i < m_Servers.Count(); i++) {
    if (m_Servers[i]->Refreshed()) {
      ...
      // make sure the ping changes each time so the user can see the server has
      // updated
...
int CServerList::CalculateAveragePing(serveritem_t &server) {
  if (server.pings[0]) {
    ...
    if ((server.pings[middlePing] - server.pings[lowPing]) <
        (server.pings[highPing] - server.pings[middlePing])) {
