...
CBaseGamesPage::CBaseGamesPage(vgui::Panel *parent, const char *name)
    : Frame(parent, name), m_Servers(this) {
  ...
  m_pPasswordIcon->SetImage(
      scheme()->GetImage(scheme()->GetDefaultScheme(), "server/icon_password"));
  ...
  // m_pRefreshMenu->AddMenuItem("Refresh", "Get new info for servers in current
  // list  ", "refresh", this); m_pRefreshMenu->AddMenuItem("GetNewList", "Get
  // new server list  ", "getnewlist", this);
  // m_pRefreshMenu->AddMenuItem("StopRefresh", "Stop refreshing server list  ",
  // "stoprefresh", this);
  m_pGameList = new OurListPanel(this, "gamelist");

  // Add the column headers
  m_pGameList->AddColumnHeader(0, "Password", util->GetString(""), 20, false,
                               NOT_RESIZABLE, NOT_RESIZABLE);
  m_pGameList->AddColumnHeader(1, "Name", util->GetString(" Servers"), 50, true,
                               RESIZABLE, RESIZABLE);
  m_pGameList->AddColumnHeader(2, "GameDesc", util->GetString(" Game"), 80,
                               true, RESIZABLE, NOT_RESIZABLE);
  m_pGameList->AddColumnHeader(3, "Players", util->GetString(" Players"), 55,
                               true, RESIZABLE, NOT_RESIZABLE);
  m_pGameList->AddColumnHeader(4, "Map", util->GetString(" Map"), 90, true,
                               RESIZABLE, NOT_RESIZABLE);
  m_pGameList->AddColumnHeader(5, "Ping", util->GetString(" Latency"), 55, true,
                               RESIZABLE, NOT_RESIZABLE);
...
// Purpose:
//-----------------------------------------------------------------------------
void CBaseGamesPage::OnTick() { m_Servers.RunFrame(); }
...
void CBaseGamesPage::ApplySchemeSettings(vgui::IScheme *pScheme) {
  ...
  m_pGameList->SetFont(
      scheme()->GetFont(scheme()->GetDefaultScheme(), "DefaultSmall"));
}
...
void CBaseGamesPage::OnCommand(const char *command) {
  if (!stricmp(command, "Connect")) {
    ...
    PostMessage(
        this,
        new KeyValues("AddServerByName")); // CFavorites handles this message
...
void CBaseGamesPage::OnManage() {
  if (m_pGameList->GetNumSelectedRows()) {
    ...
    PostMessage(m_pParent->GetVPanel(),
                new KeyValues("Manage", "serverID", serverID));
  }
}
...
MessageMapItem_t CBaseGamesPage::m_MessageMap[] = {
    MAP_MESSAGE_PTR_INT(CBaseGamesPage, "ButtonToggled", OnButtonToggled,
                        "panel", "state"),
    MAP_MESSAGE_PTR_CONSTCHARPTR(CBaseGamesPage, "TextChanged", OnTextChanged,
                                 "panel", "text"),
