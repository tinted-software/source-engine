...
public:
  ...
  virtual CDialogGameInfo *JoinGame(IGameList *gameList,
                                    unsigned int serverIndex);

  // joins a game by a specified IP, not attached to any game list
  virtual CDialogGameInfo *JoinGame(int serverIP, int serverPort,
                                    const char *titleName);

  // opens a game info dialog from a game list
  virtual CDialogGameInfo *OpenGameInfoDialog(IGameList *gameList,
                                              unsigned int serverIndex);

  // opens a game info dialog by a specified IP, not attached to any game list
  virtual CDialogGameInfo *OpenGameInfoDialog(int serverIP, int serverPort,
                                              const char *titleName);

  virtual vgui::PropertySheet *GetTabPanel();

  // called by the config panel to setup some global config values
  virtual void SetConfig(bool autorefresh, bool savercon, int refreshtime,
                         bool graphs, int graphrefreshtime, bool getlogs);
  ...
  void SearchForFriend(unsigned int uid, const char *email,
