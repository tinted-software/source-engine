...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CMOTDPanel::CMOTDPanel(vgui::Panel *parent, const char *name)
...
//-----------------------------------------------------------------------------
void CMOTDPanel::OnPageShow() { m_pMOTDPanel->RequestFocus(); }
...
//-----------------------------------------------------------------------------
void CMOTDPanel::SetRcon(CRcon *rcon) { m_pRcon = rcon; }
...
void CMOTDPanel::OnSendMOTD() {
  if (m_pRcon) {
    ...
    if (strlen("motd_write ") !=
        strlen(chat_text)) // check there is something in the text panel
    {
      unsigned int i = 0;
...
MessageMapItem_t CMOTDPanel::m_MessageMap[] = {
    ...
    //	MAP_MESSAGE_PTR_CONSTCHARPTR( CMOTDPanel, "TextChanged", OnTextChanged,
    //"panel", "text" ),
};
