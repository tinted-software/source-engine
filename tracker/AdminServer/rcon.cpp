...
CRcon::CRcon(IResponse *target, serveritem_t &server, const char *password) {
  ...
  m_pQuery->AddMessageHandler(
      new CRconMsgHandlerDetails(this, CMsgHandler::MSGHANDLER_ALL, &bytecode));
}

CRcon::~CRcon() { delete m_pQuery; }

//-----------------------------------------------------------------------------
// Purpose: resets the state of the rcon object back to startup conditions (i.e
// get challenge again)
//-----------------------------------------------------------------------------
void CRcon::Reset() {
  m_bIsRefreshing = false;
...
void CRcon::SendRcon(const char *command) {
  ...
  if (m_bIsRefreshing) { // we are already processing a request, lets queue this
  ...
  if (m_bGotChallenge == false) // haven't got the challenge id yet
  {
    ...
    m_bChallenge = true; // note that we are requesting a challenge and need to
                         // still run this command
  } else {
...
void CRcon::RunFrame() {
  ...
  /*	float curtime = CSocket::GetClock();
          if(m_fQuerySendTime!=0 && (curtime-m_fQuerySendTime)> 10.0f) // 10
     seconds
          {
                  m_fQuerySendTime=	0;
                  m_pResponseTarget->ServerFailedToRespond();
...
void CRcon::RconRequest(const char *command, int challenge) {
  ...
  // Write query string
  char rcon_cmd[600];
  _snprintf(rcon_cmd, 600, "rcon %u \"%s\" %s", challenge, m_sPassword,
            command);
...
void CRcon::UpdateServer(netadr_t *adr, int challenge, const char *resp) {
  ...
  {
    m_bChallenge = false; // m_bChallenge is set to say we just requested the
                          // challenge value
  ...
  } else // this is the result of the RCON request
  {
    ...
    if (requests.Count() > 0) { // we have queued requests
...
//-----------------------------------------------------------------------------
bool CRcon::IsRefreshing() { return m_bIsRefreshing; }

//-----------------------------------------------------------------------------
// Purpose: the server to which this rcon class is bound
//-----------------------------------------------------------------------------
serveritem_t &CRcon::GetServer() { return m_Server; }

//-----------------------------------------------------------------------------
// Purpose: the challenge id used in rcons
//-----------------------------------------------------------------------------
bool CRcon::Challenge() { return m_bChallenge; }
...
//-----------------------------------------------------------------------------
const char *CRcon::RconResponse() { return (const char *)m_sRconResponse; }

//-----------------------------------------------------------------------------
// Purpose: called when the wrong password is used, when a ServerFailed is
// called
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
bool CRcon::Disabled() { return m_bDisable; }
