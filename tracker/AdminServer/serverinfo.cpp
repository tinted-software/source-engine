...
CServerInfo::CServerInfo(IResponse *target, serveritem_t &server) {
  ...
  m_pQuery->AddMessageHandler(new CServerInfoMsgHandlerDetails(
      this, CMsgHandler::MSGHANDLER_ALL, &bytecode));
...
void CServerInfo::UpdateServer(netadr_t *adr, bool proxy,
                               const char *serverName, const char *map,
                               const char *gamedir, const char *gameDescription,
                               int players, int maxPlayers, float recvTime,
                               bool password) {
  ...
  v_strncpy(m_Server.gameDescription, gameDescription,
            sizeof(m_Server.gameDescription) - 1);
...
bool CServerInfo::IsRefreshing() { return m_bIsRefreshing; }
