...
CPlayerList::CPlayerList(IResponse *target, serveritem_t &server,
...
void CPlayerList::SendQuery() {
  ...
  m_pPlayerInfoMsg = new CPlayerMsgHandlerDetails(
      this, &m_PlayerList, CMsgHandler::MSGHANDLER_ALL, &bytecode);
...
void CPlayerList::ServerResponded() {
  ...
  const char *cur =
      strstr(rconResp, "name userid uniqueid frag time ping loss adr\n") +
...
void CPlayerList::UpdateServer() {
  ...
  // you CANNOT delete this handler because we are inside of it at the moment...
  // (yes, this was an ugly bug)
  //	delete m_pPlayerInfoMsg;
...
void CPlayerList::Refresh() { SendQuery(); }

bool CPlayerList::IsRefreshing() { return m_bIsRefreshing; }

serveritem_t &CPlayerList::GetServer() { return m_Server; }

bool CPlayerList::NewPlayerList() { return m_bNewPlayerList; }
