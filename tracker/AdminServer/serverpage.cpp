...
void VInternetDlg::Open(void) {
  ...
  // if serverbrowser file is not there we will try to transfer the favorites
  // list.
...
void VInternetDlg::LoadFilters() {
  ...
  if (!m_pSavedData->LoadFromFile(g_pFullFileSystem, "Admin\\AdminServer.vdf",
                                  true, "PLATFORM")) {
  ...
  if (!m_bGraphs) {
    ...
    m_iGraphsRefreshTime =
        m_pSavedData->FindKey("GraphsRefreshTime", true)->GetInt();
  }
...
void VInternetDlg::SaveFilters() {
  ...
  m_pSavedData->SaveToFile(g_pFullFileSystem, "Admin\\AdminServer.vdf",
                           "PLATFORM");
}

void VInternetDlg::SetConfig(bool autorefresh, bool savercon, int refreshtime,
                             bool graphs, int graphsrefreshtime, bool getlogs) {
...
// Output : VInternetDlg
//----------------------------------------------------------------------------
VInternetDlg *VInternetDlg::GetInstance() { return s_InternetDlg; }

//-----------------------------------------------------------------------------
// Purpose:
// Output : CServerContextMenu
//-----------------------------------------------------------------------------
CServerContextMenu *VInternetDlg::GetContextMenu() { return m_pContextMenu; }
...
//list
//-----------------------------------------------------------------------------
CDialogGameInfo *VInternetDlg::JoinGame(IGameList *gameList,
...
//-----------------------------------------------------------------------------
CDialogGameInfo *VInternetDlg::JoinGame(int serverIP, int serverPort,
                                        const char *titleName) {
  // open the game info dialog, then mark it to attempt to connect right away
  CDialogGameInfo *gameDialog =
      OpenGameInfoDialog(serverIP, serverPort, titleName);
...
//-----------------------------------------------------------------------------
CDialogGameInfo *VInternetDlg::OpenGameInfoDialog(IGameList *gameList,
                                                  unsigned int serverIndex) {
...
// list
//-----------------------------------------------------------------------------
CDialogGameInfo *VInternetDlg::OpenGameInfoDialog(int serverIP, int serverPort,
                                                  const char *titleName) {
  CDialogGameInfo *gameDialog =
      new CDialogGameInfo(NULL, 0, serverIP, serverPort);
...
void VInternetDlg::ConfigPanel() {
  CConfigPanel *config =
      new CConfigPanel(m_bAutoRefresh, m_bSaveRcon, m_iRefreshTime, m_bGraphs,
                       m_iGraphsRefreshTime, m_bDoLogging);
  config->Run();
}
...
void VInternetDlg::OnManageServer(int serverID) {
  ...
  if (i == m_pTabPanel->GetNumPages()) {
    ...
    if (m_bSaveRcon) { // rcons are being saved
      if (strlen(server.rconPassword) >
          0) { // this rcon password is already saved :)
    ...
    box->MakePassword();
    box->Activate(id, "", "rconpassword",
                  "Enter Rcon Password for this Server");
  } else {
...
void VInternetDlg::OnPlayerDialog(vgui::KeyValues *data) {
  ...
  if (!stricmp(type, "rconpassword")) {
    ...
    serveritem_t &server = m_pFavoriteGames->GetServer(
        atoi(playerName)); // we encode the serverid in the name field :)
    strncpy(server.rconPassword, value,
            sizeof(server.rconPassword)); // save this password
...
void VInternetDlg::ManageServer(int serverID, const char *pass) {
  ...
  m_pGamePanelInfo =
      new CGamePanelInfo(this, "Current Server", server.gameDir, m_iRefreshTime,
                         m_iGraphsRefreshTime, m_bDoLogging);
...
vgui::PropertySheet *VInternetDlg::GetTabPanel() { return m_pTabPanel; }

void VInternetDlg::OnOpenContextMenu() {
  //	CServerContextMenu *menu =
  //VInternetDlg::GetInstance()->GetContextMenu();
...
void VInternetDlg::OnTick() {
  ...
  /*
                  ...
                  case TSVC_CHALLENGE:
                          {
                                  ...
                                  ISendMessage *reply =
     CreateServerMessage(TCLS_RESPONSE); reply->SetSessionID( m_iSessionID );
                                  ...
                                  reply->WriteInt("hrate", heartbeatRate);
     // heartbeat rate to expect

                                  //m_iPreviousHeartBeatRateSentToServer =
     heartbeatRate;

                                  // reset the login timeout
                                  //m_iLoginTimeout = system()->getTimeMillis()
     + COnlineStatus::SERVERCONNECT_TIMEOUT;
     ...
     CreateServerMessage(TCLS_ROUTETOFRIEND);
                                  ...
                                  msg->WriteInt("status",
     COnlineStatus::OFFLINE); m_pNet->SendMessage(msg, NET_RELIABLE);
          ...
          //			SendStatusToServer(COnlineStatus::OFFLINE);
                  ...
                  //	{ TSVC_CHALLENGE,
     CServerSession::ReceivedMsg_Challenge },
                  //	{ TSVC_LOGINOK,
     CServerSession::ReceivedMsg_LoginOK },
                  //	{ TSVC_LOGINFAIL,
     CServerSession::ReceivedMsg_LoginFail },
                  //	{ TSVC_DISCONNECT,
     CServerSession::ReceivedMsg_Disconnect },
                  //	{ TSVC_FRIENDS,
     CServerSession::ReceivedMsg_Friends },
                  //	{ TSVC_FRIENDUPDATE,
     CServerSession::ReceivedMsg_FriendUpdate },
                  //	{ TSVC_GAMEINFO,
     CServerSession::ReceivedMsg_GameInfo },
                  //	{ TSVC_HEARTBEAT,
     CServerSession::ReceivedMsg_Heartbeat },
                  //	{ TSVC_PINGACK,
     CServerSession::ReceivedMsg_PingAck },
...
void VInternetDlg::SearchForFriend(unsigned int uid, const char *email,
                                   const char *username, const char *firstname,
                                   const char *lastname) {
...
void VInternetDlg::SendInitialLogin() {
  //	assert(m_iLoginState == LOGINSTATE_WAITINGTORECONNECT || m_iLoginState
...
bool VInternetDlg::CheckMessageValidity(IReceiveMessage *dataBlock) {
  ...
  if (msgID == TSVC_FRIENDS || msgID == TSVC_GAMEINFO ||
      msgID == TSVC_HEARTBEAT || msgID == TSVC_FRIENDUPDATE) {
    // see if the server really knows us
    if (/*m_iStatus < COnlineStatus::ONLINE ||*/ m_iSessionID !=
        dataBlock->SessionID()) {
