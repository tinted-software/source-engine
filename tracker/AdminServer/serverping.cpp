...
CServerPing::CServerPing(IResponse *target, serveritem_t &server) {
  ...
  m_pQuery->AddMessageHandler(new CServerPingMsgHandlerDetails(
...
void CServerPing::RunFrame() {
  /*	float curtime = CSocket::GetClock();
          if(m_fSendTime!=0 && (curtime-m_fSendTime)> 5.0f) // 10 seconds
     timeout
          {
                  m_fSendTime = 0;
...
bool CServerPing::IsRefreshing() { return m_bIsRefreshing; }
