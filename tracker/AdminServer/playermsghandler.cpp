...
CPlayerMsgHandlerDetails::CPlayerMsgHandlerDetails(
    CPlayerList *baseobject, CUtlVector<Players_t> *players, HANDLERTYPE type,
    void *typeinfo /*= NULL*/)
...
bool CPlayerMsgHandlerDetails::Process(netadr_t *from, CMsgBuffer *msg) {
  ...
  if (pNumber <= 0 ||
      pNumber > 32) // you still need to update the player vector if this
                    // happens, the server could be empty
  {
  ...
  /*			serveritem_t server;
          ...
          CDialogGameInfo *gameDialog = new CDialogGameInfo(NULL, 0,*((int
     *)server.ip),pNumber); gameDialog->Run("Stuff");
  */
