...
CBanList::CBanList(IResponse *target, serveritem_t &server,
...
CBanList::~CBanList() { delete m_pRcon; }
...
void CBanList::ServerResponded() {
  ...
  if (m_bGotIPs == false) {
    ...
    const char *cur =
        strstr(rconResp, "UserID filter list:") + strlen("UserID filter list:");
    ...
    const char *rconResp = m_pRcon->RconResponse();
    const char *cur =
        strstr(rconResp, "IP filter list:") + strlen("IP filter list:");
...
bool CBanList::IsRefreshing() { return m_bIsRefreshing; }

serveritem_t &CBanList::GetServer() { return m_Server; }

bool CBanList::NewBanList() { return m_bNewBanList; }
