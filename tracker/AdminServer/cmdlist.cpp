...
//
// Purpose: queries server for the command list, and then use QueryCommand() to
// see
...
CCMDList::CCMDList(IResponse *target, serveritem_t &server,
...
CCMDList::~CCMDList() { delete m_pRcon; }
...
void CCMDList::ServerResponded() {
  ...
  while (cur != NULL) {
    ...
    if (strncmp(cur, "Command List", 12) && strncmp(cur, "-------------", 13) &&
        strncmp(cur, "Total Commands", 14) &&
        strncmp(cur, "CmdList ? for syntax", 20)) {
...
serveritem_t &CCMDList::GetServer() { return m_Server; }
...
bool CCMDList::GotCommands() { return m_bGotCommands; }
