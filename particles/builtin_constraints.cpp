...
bool C_OP_ConstrainDistance::EnforceConstraint(
    ...
    if (IsAnyNegative(NeedAdjust)) // any out of bounds?
    {
      ...
      guess =
          MulSIMD(guess, SubSIMD(Four_Threes,
                                 MulSIMD(dist_squared, MulSIMD(guess, guess))));
...
bool C_OP_PlanarConstraint::EnforceConstraint(
    int nStartBlock, int nNumBlocks, CParticleCollection *pParticles,
    void *pContext, int nNumValidParticlesInLastChunk) const {
  C4VAttributeWriteIterator pXYZ(PARTICLE_ATTRIBUTE_XYZ, pParticles);
  pXYZ += nStartBlock;
...
void CWorldCollideContextData::SetBaseTrace(int nIndex, Vector const &rayStart,
                                            Vector const &traceDir,
                                            int nCollisionGroup,
                                            bool bKeepMisses) {
  CBaseTrace tr;
  Vector rayEnd = rayStart + traceDir;
  g_pParticleSystemMgr->Query()->TraceLine(rayStart, rayEnd, MASK_SOLID, NULL,
                                           nCollisionGroup, &tr);
  if (tr.fraction < 1.0) {
...
void CWorldCollideContextData::CalculatePlanes(CParticleCollection *pParticles,
  ...
  case COLLISION_MODE_INITIAL_TRACE_DOWN: {
    SetBaseTrace(0, rayStart, 1000.0 * Vector(-1, 0, 0), nCollisionGroup,
                 false);
    m_nActivePlanes = 1;
    m_nNumFixedPlanes = 1;
    break;
  ...
  case COLLISION_MODE_PER_FRAME_PLANESET: {
    ...
    for (int i = -1; i <= 1; i++)
      for (int j = -1; j <= 1; j++)
        for (int k = -1; k <= 1; k++) {
          if (i || j || k) {
            SetBaseTrace(nIndexOut++, rayStart, 1000.0 * Vector(i, j, k),
                         nCollisionGroup, false);
          }
        }
...
class C_OP_WorldTraceConstraint : public CParticleOperatorInstance {
  ...
  bool EnforceConstraintInternal(int nStartBlock, int nEndBlock,
...
static void
WorldIntersectTNew(FourVectors const *pStartPnt, FourVectors const *pEndPnt,
                   int nCollisionGroup, int nMask, ISectData_t *pISectData,
                   int nCollisionMode, CWorldCollideContextData *pCtx,
    ...
    if (nCollisionMode == COLLISION_MODE_USE_NEAREST_TRACE) {
      ...
      for (int i = 0; i < pCtx->m_nActivePlanes; i++) {
        if (pCtx->m_bPlaneActive[i]) {
          fltx4 fl4TrialDistance =
              MaxSIMD(pStartPnt->DistSqrToLineSegment(pCtx->m_TraceStartPnt[i],
                                                      pCtx->m_TraceEndPnt[i]),
                      pEndPnt->DistSqrToLineSegment(pCtx->m_TraceStartPnt[i],
                                                    pCtx->m_TraceEndPnt[i]));
          ...
          v4PlaneNormal.x = MaskedAssign(
              fl4Nearestmask, pCtx->m_PlaneNormal[i].x, v4PlaneNormal.x);
          v4PlaneNormal.y = MaskedAssign(
              fl4Nearestmask, pCtx->m_PlaneNormal[i].y, v4PlaneNormal.y);
          v4PlaneNormal.z = MaskedAssign(
              fl4Nearestmask, pCtx->m_PlaneNormal[i].z, v4PlaneNormal.z);
        }
      }
      fltx4 fl4OutOfRange =
          AndSIMD(fl4ParticleValidMask,
      ...
      if (IsAnyNegative(fl4OutOfRange)) {
        ...
        for (int i = 0; i < 4; i++) {
          if (nMask & (1 << i)) {
            ...
            if (tr.fraction < 1.0) {
              SubFloat(v4PointOnPlane.x, i) =
                  start.x + (tr.fraction * (end.x - start.x));
              SubFloat(v4PointOnPlane.y, i) =
                  start.y + (tr.fraction * (end.y - start.y));
              SubFloat(v4PointOnPlane.z, i) =
              ...
              // no hit.  a normal of 0 will prevent the crossing check from
              // ever finding a crossing, since it will check for (p - origin )
              // dot normal < 0
      ...
      if (IsAnyNegative(fl4CrossMask)) {
        ...
        if (IsAnyNegative(fl4CrossMask)) {
          ...
          pISectData->m_ISectNormal.x = MaskedAssign(
              fl4CrossMask, v4PlaneNormal.x, pISectData->m_ISectNormal.x);
          pISectData->m_ISectNormal.y = MaskedAssign(
              fl4CrossMask, v4PlaneNormal.y, pISectData->m_ISectNormal.y);
          pISectData->m_ISectNormal.z = MaskedAssign(
              fl4CrossMask, v4PlaneNormal.z, pISectData->m_ISectNormal.z);
        }
      }
    }
  ...
  if (pCtx) {
    ...
    for (int i = 0; i < pCtx->m_nActivePlanes; i++) {
      if (pCtx->m_bPlaneActive[i]) {
        ...
        if (IsAnyNegative(fl4CrossMask)) {
#ifdef FP_EXCEPTIONS_ENABLED
          // Wherever fl4CrossMask is zero we need to ensure that fl4StartDist
          // does not equal fl4EndDist to avoid divide-by-zero.
...
bool C_OP_WorldTraceConstraint::EnforceConstraint(
  ...
  if (m_nCollisionMode == COLLISION_MODE_USE_NEAREST_TRACE) {
    if (m_bKillonContact)
      return EnforceConstraintInternal<true, true>(
          nStartBlock, nNumBlocks, pParticles, pContext,
          nNumValidParticlesInLastChunk);
    else
      return EnforceConstraintInternal<false, true>(
          nStartBlock, nNumBlocks, pParticles, pContext,
          nNumValidParticlesInLastChunk);
  } else {
    if (m_bKillonContact)
      return EnforceConstraintInternal<true, false>(
          nStartBlock, nNumBlocks, pParticles, pContext,
          nNumValidParticlesInLastChunk);
    else
      return EnforceConstraintInternal<false, false>(
          nStartBlock, nNumBlocks, pParticles, pContext,
          nNumValidParticlesInLastChunk);
  }
}
...
template <bool bKillonContact, bool bCached>
bool C_OP_WorldTraceConstraint::EnforceConstraintInternal(
    int nStartBlock, int nNumBlocks, CParticleCollection *pParticles,
    void *pContext, int nNumValidParticlesInLastChunk) const {
  ...
  do {
    ...
    if (bCached) {
      ...
      if (nNumBlocks > 1)
        ...
        fl4TailMask = LoadAlignedIntSIMD(
            g_SIMD_SkipTailMask[nNumValidParticlesInLastChunk]);

      WorldIntersectTNew(pPrevXYZ, &endPnt, m_nCollisionGroupNumber, nMask,
                         &iData, m_nCollisionMode, pCtx, fl4TailMask, flTol);
    } else
...
DMXELEMENT_UNPACK_FIELD("control point movement distance tolerance", "5", float,
                        m_flCpMovementTolerance)
DMXELEMENT_UNPACK_FIELD("kill particle on collision", "0", bool,
                        m_bKillonContact)
DMXELEMENT_UNPACK_FIELD("trace accuracy tolerance", "24", float,
                        m_flTraceTolerance)
