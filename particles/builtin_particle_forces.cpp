...
void C_OP_AttractToControlPoint::AddForces(FourVectors *pAccumulatedForces,
  ...
  for (int i = 0; i < nBlocks; i++) {
    ...
    ofs *= MulSIMD(fForceScale,
                   ReciprocalSaturateSIMD(len)); // normalize and scale
...
private:
  // virtual void addParticleForce(PhysParticle* a, PhysParticleCacheNode* bcn,
  // float flStrength, float ts) const;
  virtual void addParticleForce(PhysParticle *a, PhysParticle *b, float distSq,
                                float flStrength, float ts) const;
};

// TODO: I should make sure I don't have divide by zero errors.
// TODO: ts is not used
void C_OP_LennardJonesForce::addParticleForce(PhysParticle *a, PhysParticle *b,
  ...
  if (a->group == b->group) // In the same group
  {
    ...
    //  I tried this, and discovered that rather than averaging, I can take
    //  maybe take the larger of the two neighbor counts, so the attraction
    //  between two particles on the surface will be strong, but the attraction
    //  between a particle inside and a particle on the surface will be weak. I
    //  can also try taking the min so that the attraction between a particle on
    //  the surface and a particle inside the fluid will be strong, but the
    //  attraction between two particles completely on the inside will be weak.
    //
    //  int symmetric_neighbor_count = min(a->neighbor_count,
    //  b->neighbor_count);
    //
...
void C_OP_LennardJonesForce::AddForces(FourVectors *pAccumulatedForces,
  ...
  static SmartArray<PhysParticle>
