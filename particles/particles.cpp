...
// This is the soft limit - if we exceed this, we spit out a report of all the
// particles in the frame
#define MAX_PARTICLE_VERTS 50000
// These are some limits that control
// g_pParticleSystemMgr->ParticleThrottleScaling() and
// g_pParticleSystemMgr->ParticleThrottleRandomEnable()
// ConVar cl_particle_scale_lower ( "cl_particle_scale_lower", "20000",
// FCVAR_CLIENTDLL | FCVAR_CHEAT ); ConVar cl_particle_scale_upper (
// "cl_particle_scale_upper", "40000", FCVAR_CLIENTDLL | FCVAR_CHEAT );
...
CParticleSystemDictionary::AddParticleSystem(CDmxElement *pParticleSystem) {
...
CParticleSystemDictionary::FindParticleSystemHandle(const char *pName) {
  return m_ParticleNameMap.Find(pName);
}

CParticleSystemDefinition *
CParticleSystemDictionary::FindParticleSystem(ParticleSystemHandle_t h) {
...
CParticleSystemDefinition *
CParticleSystemDictionary::FindParticleSystem(const char *pName) {
...
CParticleSystemDefinition *
CParticleSystemDictionary::FindParticleSystem(const DmObjectId_t &id) {
  int nCount = m_ParticleIdMap.Count();
  for (int i = 0; i < nCount; ++i) {
    if (IsUniqueIdEqual(m_ParticleIdMap[i]->GetId(), id))
      return m_ParticleIdMap[i];
...
public:
  ...
  CreateInstance(const DmObjectId_t &id) const {
    Assert(0);
    return NULL;
  }
...
//-----------------------------------------------------------------------------
bool CParticleSystemDefinition::IsPrecached() const { return m_bIsPrecached; }
...
void CParticleSystemDefinition::ParseChildren(CDmxElement *pElement) {
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    children[i]->UnpackIntoStructure(&m_Children[j], sizeof(m_Children[j]),
...
void CParticleSystemDefinition::Read(CDmxElement *pElement) {
  ...
  pElement->UnpackIntoStructure(this, sizeof(*this),
                                s_pParticleSystemDefinitionUnpack);
...
CParticleSystemDefinition::FindOperatorById(ParticleFunctionType_t type,
...
void CParticleCollection::Init(CParticleSystemDefinition *pDef, float flDelay,
  ...
  for (int i = 0; i < NELEMS(olists); i++) {
    ...
    for (int j = 0; j < nOperatorCount; j++) {
      (*olists[i])[j]->InitializeContextData(this, m_pOperatorContextData +
                                                       (*offsetlists)[i][j]);
    }
  }
...
bool CParticleCollection::HasMoved() const {
  // It's weird that this is possible, but it apparently is (see the many other
  ...
  for (CParticleCollection *child = m_Children.m_pHead; child;
       child = child->m_pNext) {
    if (child->HasMoved()) {
      return true;
    }
...
void CParticleCollection::InitStorage(CParticleSystemDefinition *pDef) {
  ...
  // We have to zero-init the memory so that any attributes that are not
  // initialized get predictable and sensible values.
  ...
  float *pInitialMem =
      (float *)((size_t)(m_pParticleInitialMemory + 15) & ~0xF);
...
bool CParticleCollection::UsesPowerOfTwoFrameBufferTexture(
  ...
  if (!m_bAnyUsesPowerOfTwoFrameBufferTexture) // quick out if neither us or our
                                               // children ever use
  {
    return false;
  }
...
//-----------------------------------------------------------------------------
bool CParticleCollection::IsBatchable() const { return m_bIsBatchable; }
...
void CParticleCollection::Restart() {
  ...
  for (i = 0; i < nEmitterCount; i++) {
    m_pDef->m_Emitters[i]->Restart(this, m_pOperatorContextData +
                                             m_pDef->m_nEmittersCtxOffsets[i]);
  }
    ...
    if (!bTranslucentOnly || m_pDef->GetMaterial()->IsTranslucent()) {
      ...
      for (int i = 0; i < nCount; i++) {
        if (CheckIfOperatorShouldRun(m_pDef->m_Renderers[i])) {
          // 					pRenderContext->MatrixMode(
          // MATERIAL_VIEW ); 					pRenderContext->PushMatrix();
          // 					pRenderContext->LoadIdentity();
          // 					pRenderContext->MatrixMode(
          // MATERIAL_PROJECTION ); 					pRenderContext->PushMatrix();
          // 					pRenderContext->LoadIdentity();
          // 					pRenderContext->Ortho( -100,
          // -100, 100, 100, -100, 100 );
          m_pDef->m_Renderers[i]->Render(pRenderContext, this,
                                         m_pOperatorContextData +
                                             m_pDef->m_nRenderersCtxOffsets[i]);
          // 					pRenderContext->MatrixMode(
          // MATERIAL_VIEW ); 					pRenderContext->PopMatrix();
          // 					pRenderContext->MatrixMode(
          // MATERIAL_PROJECTION ); 					pRenderContext->PopMatrix();
        }
      }
    }
  }
...
void CParticleCollection::InitializeNewParticles(int nFirstParticle,
                                                 int nParticleCount,
                                                 uint32 nInittedMask) {
  VPROF_BUDGET("CParticleCollection::InitializeNewParticles",
  ...
  for (int i = 0; i < nInitializerCount; i++) {
    ...
    if (((nInitializerAttrMask & nAttrsLeftToInit) == 0) ||
    ...
    if (m_bIsScrubbable && !pOp->IsScrubSafe()) {
      for (int j = 0; j < nParticleCount; ++j) {
        pOp->InitNewParticles(this, nFirstParticle + j, 1, nAttrsLeftToInit,
                              pContext);
      }
    } else {
      pOp->InitNewParticles(this, nFirstParticle, nParticleCount,
                            nAttrsLeftToInit, pContext);
    }
    END_OP;
...
static void CHECKSYSTEM(CParticleCollection *pParticles) {
  ...
  for (int i = 0; i < pParticles->m_nActiveParticles; ++i) {
    ...
    /*
                    const float *rad = pParticles->GetFloatAttributePtr(
       PARTICLE_ATTRIBUTE_RADIUS, i ); Assert( IsFinite( rad[0] ) );

                    RJ: Disabling this assert.  While the proper way is to fix
       the math which leads to the bad number, the fix would result in more
       particles being drawn and in a post shipping world, users were not happy.
                    See Changelists #1368648, #1368635, and #1368434 for proper
       math calculation fixes.

                    In a post shipping world, as these particles would not
       render with infinites, code was added
       C_OP_RenderSprites::RenderSpriteCard() to check for the infinite and not
       add the vert to meshbuilder.
    */
...
void CParticleCollection::Simulate(float dt, bool updateBboxOnly) {
  ...
  if (!updateBboxOnly) {
    ...
    // Limit timestep if needed (prevents short lived particles from being
    // created and destroyed before being rendered.
    ...
    while (flRemainingDt > 0.0) {
      ...
      for (int i = 0; i < nCount; i++) {
        ...
        if (pOp->ShouldRunBeforeEmitters() &&
          ...
          pOp->Operate(this, flStrength,
                       m_pOperatorContextData +
                           m_pDef->m_nOperatorsCtxOffsets[i]);
          END_OP;
          CHECKSYSTEM(this);
      ...
      if (m_nActiveParticles) {
        ...
        for (int unusedParticle = m_nActiveParticles;
             unusedParticle < processedParticles; ++unusedParticle) {
          ...
          float *dtime = GetFloatAttributePtrForWrite(
              PARTICLE_ATTRIBUTE_LIFE_DURATION, unusedParticle);
          *dtime = 1.0f;
        }
#endif
#ifdef _DEBUG
    ...
    m_pDef->m_flMaxMeasuredSimTime =
        max(m_pDef->m_flMaxMeasuredSimTime, flETime);
#endif
  }
...
void CParticleCollection::GetControlPointAtTime(int nControlPoint, float flTime,
  ...
  // in regards to issues that can cause t to go negative. Actually this
  // function is just sometimes called (from InitNewParticlesScalar) with values
  // that cause 't' to go massively negative. So I clamp it.
  if (t < 0.0f)
    t = 0.0f;
  Assert(t <= 1.0f);
...
//
//-----------------------------------------------------------------------------
void CParticleCollection::GetControlPointAtPrevTime(
    int nControlPoint, Vector *pControlPoint) const {
  Assert(m_pDef->ReadsControlPoint(nControlPoint));
  *pControlPoint = m_ControlPoints[nControlPoint].m_PrevPosition;
}

void CParticleCollection::GetControlPointTransformAtCurrentTime(
    int nControlPoint, matrix3x4_t *pMat) {
  Assert(m_pDef->ReadsControlPoint(nControlPoint));
...
void CParticleCollection::GetControlPointTransformAtCurrentTime(
    int nControlPoint, VMatrix *pMat) {
  GetControlPointTransformAtCurrentTime(
      nControlPoint, const_cast<matrix3x4_t *>(&pMat->As3x4()));
...
void CParticleCollection::GetControlPointTransformAtTime(int nControlPoint,
                                                         float flTime,
                                                         VMatrix *pMat) {
  GetControlPointTransformAtTime(nControlPoint, flTime,
                                 const_cast<matrix3x4_t *>(&pMat->As3x4()));
  pMat->m[3][0] = pMat->m[3][1] = pMat->m[3][2] = 0.0f;
  pMat->m[3][3] = 1.0f;
}
...
void CParticleCollection::BloatBoundsUsingControlPoint() {
  ...
  // Don't do the bounding box fixup until after the second simulation (first
  // real simulation) so that we know they're in their correct position.
...
//StopEmission() called 			dormant particle systems don't remove themselves when
//they're 			out of particles, assuming they'll return at some point.
//-----------------------------------------------------------------------------
...
void CParticleCollection::ApplyKillList(void) {
  ...
  // Previously, this code would swap the last item that wasn't dead into this
  // slot. However, some lists require order invariance, so we need to collapse
  // over holes instead of doing the (cheaper) swap from the end to the hole.
  if (!m_bRequiresOrderInvariance) {
    while (nLeftInKillList) {
      ...
      // first, we have to find the last particle (which is not in the kill
      // list)
      while (nLeftInKillList && (pCurKillListSlot[nLeftInKillList - 1] ==
                                 nParticlesActiveNow - 1)) {
        nLeftInKillList--;
        nParticlesActiveNow--;
      }
    ...
    // The calling code may tell us to kill particles that are already out of
    // bounds. That causes this code to kill more particles than we're supposed
    // to (possibly even causing a crash). So remember how many particles we had
    // left to kill so we can properly decrement the count below.
...
void CParticleSystemMgr::SetLastSimulationTime(float flTime) {
  ...
  for (int i = 0; i < c_nNumFramesTracked - 1; i++) {
    ...
    nParticleVertexCountHistoryMax =
        Max(nParticleVertexCountHistoryMax, m_nParticleVertexCountHistory[i]);
  }
  m_nParticleVertexCountHistory[c_nNumFramesTracked - 1] =
      m_nParticleVertexCount;
  nParticleVertexCountHistoryMax =
      Max(nParticleVertexCountHistoryMax, m_nParticleVertexCount);

  // We need to take an average over a decent number of frames because this
  // throttling has a direct feedback effect. Worried about oscillation
  // problems!
...
CParticleSystemDefinition *
CParticleSystemMgr::FindParticleSystem(const char *pName) {
  return m_pParticleSystemDictionary->FindParticleSystem(pName);
}

CParticleSystemDefinition *
CParticleSystemMgr::FindParticleSystem(const DmObjectId_t &id) {
...
bool CParticleSystemMgr::ReadParticleConfigFile(const char *pFileName,
  ...
  if (IsX360()) {
    ...
    bool bHaveParticles =
        g_pFullFileSystem->ReadFile(szTargetName, "GAME", fileBuffer);
    if (bHaveParticles) {
      fileBuffer.SetBigEndian(false);
      return ReadParticleConfigFile(fileBuffer, bPrecache, bDecommitTempMemory,
                                    szTargetName);
    } else if (g_pFullFileSystem->GetDVDMode() != DVDMODE_OFF) {
      // 360 version should have been there, 360 zips can only have binary
      // particles
...
bool CParticleSystemMgr::WriteParticleConfigFile(CDmxElement *pParticleSystem,
                                                 CUtlBuffer &buf,
                                                 bool bPreventNameBasedLookup) {
...
ParticleSystemHandle_t
CParticleSystemMgr::GetParticleSystemIndex(const char *pParticleSystemName) {
  if (!pParticleSystemName)
...
//-----------------------------------------------------------------------------
CParticleCollection *
CParticleSystemMgr::CreateParticleCollection(const char *pParticleSystemName,
                                             float flDelay, int nRandomSeed) {
  if (!pParticleSystemName)
    return NULL;
...
CParticleCollection *
CParticleSystemMgr::CreateParticleCollection(const DmObjectId_t &id,
                                             float flDelay, int nRandomSeed) {
...
//--------------------------------------------------------------------------------
CDmxElement *
CParticleSystemMgr::CreateParticleDmxElement(const DmObjectId_t &id) {
  CParticleSystemDefinition *pDef =
      m_pParticleSystemDictionary->FindParticleSystem(id);
...
CDmxElement *
CParticleSystemMgr::CreateParticleDmxElement(const char *pParticleSystemName) {
  CParticleSystemDefinition *pDef =
      m_pParticleSystemDictionary->FindParticleSystem(pParticleSystemName);
    ...
    for (int j = 0; j < nRenderCount; ++j) {
      ...
      while (nFirstParticle < pParticles->m_nActiveParticles) {
        ...
        step.m_nParticleCount = step.m_pRenderer->GetParticlesToRender(
            ...
            nRemainingIndices, &step.m_nVertCount, &iPart);
...
void CParticleSystemMgr::DrawRenderCache(bool bShadowDepth) {
  ...
  for (int iRenderCache = 0; iRenderCache < nRenderCacheCount; ++iRenderCache) {
    ...
    for (int i = 0; i < nBatchCount; ++i) {
      ...
      g_pParticleSystemMgr->TallyParticlesRendered(batch.m_nVertCount * 3,
                                                   batch.m_nIndexCount * 3);
...
void CParticleSystemMgr::TallyParticlesRendered(int nVertexCount,
                                                int nIndexCount) {
