...
void C_OP_BasicMovement::Operate(CParticleCollection *pParticles,
  ...
  fltx4 adj_dt =
      ReplicateX4((pParticles->m_flDt / pParticles->m_flPreviousDt) *
                  ExponentialDecay((1.0f - max(0.f, m_fDrag)), (1.0f / 30.0f),
                                   pParticles->m_flDt));
  ...
  do {
    ...
    prev_xyz->x =
        AddSIMD(xyz->x, AddSIMD(accFactorX,
                                MulSIMD(adj_dt, SubSIMD(xyz->x, prev_xyz->x))));
    prev_xyz->y =
        AddSIMD(xyz->y, AddSIMD(accFactorY,
                                MulSIMD(adj_dt, SubSIMD(xyz->y, prev_xyz->y))));
    prev_xyz->z =
        AddSIMD(xyz->z, AddSIMD(accFactorZ,
                                MulSIMD(adj_dt, SubSIMD(xyz->z, prev_xyz->z))));
    CHECKSYSTEM(pParticles);
    ++prev_xyz;
    ++xyz;
    pAccIn += nForceStride;
  ...
  if (nConstraints && pParticles->m_nPaddedActiveParticles) {
    ...
    for (int i = 0; i < nConstraints; i++) {
      bFinalConstraint[i] =
          pParticles->m_pDef->m_Constraints[i]->IsFinalConstraint();
  ...
  do {
    ...
    if (IsAnyNegative(ApplyMask)) {
      ...
      if (m_bEaseInAndOut) {
        ...
        fltx4 fl4Frac = MulSIMD(SubSIMD(fl4LifeTime, fl4FadeOutTime),
                                ReciprocalEstSIMD(fl4Lifespan));
...
//-----------------------------------------------------------------------------
// Lifespan Minimum Velocity Decay Operator (kills particles if they cease
// moving)
//-----------------------------------------------------------------------------
...
DEFINE_PARTICLE_OPERATOR(C_OP_VelocityDecay, "Lifespan Minimum Velocity Decay",
                         OPERATOR_GENERIC);
...
void C_OP_VelocityDecay::Operate(CParticleCollection *pParticles,
                                 float flStrength, void *pContext) const {
  ...
  for (int i = 0; i < nLimit; i += 4) {
    fltx4 fl4KillMask =
        CmpLeSIMD(DivSIMD(SubSIMD(*pXYZ, *pPrevXYZ), fl4Dt), fl4MinVelocity);
...
void C_OP_Cull::Operate(CParticleCollection *pParticles, float flStrength,
  ...
  for (int i = 0; i < pParticles->m_nActiveParticles; ++i) {
    ...
    // Find our life percentage
    flLifeTime =
        clamp((pParticles->m_flCurTime - *pCreationTime) / (*pLifeDuration),
              0.0f, 1.0f);
    ...
    if (m_flBias == 0.5f) // no bias case
    {
      ...
      do {
        ...
        if (IsAnyNegative(fl4GoodMask)) {
          ...
          // Wherever fl4GoodMask is zero we need to ensure that fl4FadeWindow
          // is not zero to avoid 0/0 divides in BiasSIMD. Setting those
          // elements to fl4EndTime should do the trick...
          fl4FadeWindow = OrSIMD(AndSIMD(fl4GoodMask, fl4EndTime),
                                 AndNotSIMD(fl4GoodMask, fl4EndTime));
#endif
          fl4FadeWindow = AddSIMD(
              fl4StartScale,
              MulSIMD(BiasSIMD(fl4FadeWindow, m_fl4BiasParam), fl4ScaleWidth));
          *pRadius = MaskedAssign(
              fl4GoodMask, MulSIMD(*pInitialRadius, fl4FadeWindow), *pRadius);
        }
        ++pCreationTime;
...
void C_OP_PositionLock::Operate(CParticleCollection *pParticles,
  ...
  for (int i = 0; i < pParticles->m_nActiveParticles; ++i) {
    ...
    float flLifeTime = *pLifeDuration != 0.0f
                           ? clamp((pParticles->m_flCurTime - *pCreationTime) /
                                       (*pLifeDuration),
                                   0.0f, 1.0f)
                           : 0.0f;
  ...
  if (m_flStartTime_min >= 1.0) // always locked on
  {
    ...
    do {
      ...
      fl4CreationFrameBias =
          MulSIMD(DivSIMD(Four_Ones, fl4_Dt), fl4CreationFrameBias);
...
class C_OP_SetChildControlPoints : public CParticleOperatorInstance {
  ...
  uint32 GetReadAttributes(void) const { return PARTICLE_ATTRIBUTE_XYZ; }
...
void C_OP_SetChildControlPoints::Operate(CParticleCollection *pParticles,
  ...
  int nToSet = min(pParticles->m_nActiveParticles - m_nFirstSourcePoint,
                   m_nNumControlPoints);
...
void C_OP_SetControlPointPositions::Operate(CParticleCollection *pParticles,
  ...
  if (!m_bUseWorldLocation) {
    ...
    matrix3x4_t mat;
    pParticles->GetControlPointTransformAtTime(m_nHeadLocation,
...
void C_OP_SetControlPointToPlayer::Operate(CParticleCollection *pParticles,
  ...
  Vector vecRight;
  Vector vecUp;
  g_pParticleSystemMgr->Query()->GetLocalPlayerEyeVectors(&vecForward,
                                                          &vecRight, &vecUp);
...
//-----------------------------------------------------------------------------
// Model Cull Operator - cull particles inside or outside of a brush/animated
// model
//-----------------------------------------------------------------------------
...
DEFINE_PARTICLE_OPERATOR(C_OP_ModelCull, "Cull relative to model",
                         OPERATOR_GENERIC);

BEGIN_PARTICLE_OPERATOR_UNPACK(C_OP_ModelCull)
DMXELEMENT_UNPACK_FIELD("control_point_number", "0", int, m_nControlPointNumber)
DMXELEMENT_UNPACK_FIELD("use only bounding box", "0", bool, m_bBoundBox)
DMXELEMENT_UNPACK_FIELD("cull outside instead of inside", "0", bool,
                        m_bCullOutside)
END_PARTICLE_OPERATOR_UNPACK(C_OP_ModelCull)

void C_OP_ModelCull::Operate(CParticleCollection *pParticles, float flStrength,
                             void *pContext) const {
  pParticles->UpdateHitBoxInfo(m_nControlPointNumber);
  if (pParticles->m_ControlPointHitBoxes[m_nControlPointNumber]
          .CurAndPrevValid()) {
    for (int i = 0; i < pParticles->m_nActiveParticles; ++i) {
      ...
      bool bInside =
          g_pParticleSystemMgr->Query()->IsPointInControllingObjectHitBox(
              pParticles, m_nControlPointNumber, vecParticlePosition,
              m_bBoundBox);
...
DEFINE_PARTICLE_OPERATOR(C_OP_Orient2DRelToCP, "Rotation Orient Relative to CP",
                         OPERATOR_GENERIC);
...
void C_OP_Orient2DRelToCP::Operate(CParticleCollection *pParticles,
                                   float flStrength, void *pContext) const {
...
void C_OP_RemapDotProductToScalar::Operate(CParticleCollection *pParticles,
  ...
  if ((m_bActiveRange && !m_bUseParticleVelocity &&
       (flInput < m_flInputMin || flInput > m_flInputMax)))
    return;
  ...
  for (int i = 0; i < pParticles->m_nActiveParticles; ++i) {
    if (m_bUseParticleVelocity) {
      ...
      // only use within start/end time frame and, if set, active input range
      if ((m_bActiveRange &&
           (flInputDot < m_flInputMin || flInputDot > m_flInputMax)))
        continue;
    }
...
DEFINE_PARTICLE_OPERATOR(C_OP_RemapCPtoScalar, "Remap Control Point to Scalar",
                         OPERATOR_GENERIC);
...
void C_OP_RemapCPtoScalar::Operate(CParticleCollection *pParticles,
                                   float flStrength, void *pContext) const {
  const float *pCreationTime;
  // clamp the result to 0 and 1 if it's alpha
  float flMin = m_flOutputMin;
...
DEFINE_PARTICLE_OPERATOR(C_OP_MovementRotateParticleAroundAxis,
...
void C_OP_MovementRotateParticleAroundAxis::Operate(
...
class C_OP_RemapSpeedtoCP : public CParticleOperatorInstance {
  ...
  virtual uint64 GetReadControlPointMask() const {
    return (1ULL << m_nInControlPointNumber) |
           (1ULL << m_nOutControlPointNumber);
  }
...
DEFINE_PARTICLE_OPERATOR(C_OP_RemapSpeedtoCP, "Remap CP Speed to CP",
                         OPERATOR_GENERIC);
BEGIN_PARTICLE_OPERATOR_UNPACK(C_OP_RemapSpeedtoCP)
DMXELEMENT_UNPACK_FIELD("input control point", "0", int,
                        m_nInControlPointNumber)
DMXELEMENT_UNPACK_FIELD("input minimum", "0", float, m_flInputMin)
DMXELEMENT_UNPACK_FIELD("input maximum", "1", float, m_flInputMax)
DMXELEMENT_UNPACK_FIELD("output control point", "-1", int,
...
void C_OP_RemapSpeedtoCP::Operate(CParticleCollection *pParticles,
                                  float flStrength, void *pContext) const {
  if (m_nOutControlPointNumber >= 0) {
    ...
    Vector vecDelta;
    vecDelta =
        pParticles->GetControlPointAtCurrentTime(m_nInControlPointNumber) -
        vecPrevPos;
    float flSpeed = vecDelta.Length() / pParticles->m_flPreviousDt;
    float flOutput = RemapValClamped(flSpeed, m_flInputMin, m_flInputMax,
                                     m_flOutputMin, m_flOutputMax);

    Vector vecControlPoint =
        pParticles->GetControlPointAtCurrentTime(m_nOutControlPointNumber);
