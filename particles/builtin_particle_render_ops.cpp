...
void SetupParticleVisibility(CParticleCollection *pParticles,
                             CParticleVisibilityData *pVisibilityData,
                             const CParticleVisibilityInputs *pVisibilityInputs,
                             int *nQueryHandle) {
  ...
  if ( pVisibilityInputs->m_bUseBBox )
  {
          ...
          flScale = ( max(vecBounds.x, max (vecBounds.y, vecBounds.z) ) *
  pVisibilityInputs->m_flBBoxScale );
  }
  if ( pVisibilityInputs->m_nCPin >= 0 )
  {
          vecOrigin = pParticles->GetControlPointAtCurrentTime(
  pVisibilityInputs->m_nCPin );
  }
  */
  vecOrigin =
      pParticles->GetControlPointAtCurrentTime(pVisibilityInputs->m_nCPin);
  float flVisibility = g_pParticleSystemMgr->Query()->GetPixelVisibility(
      nQueryHandle, vecOrigin, flScale);

  pVisibilityData->m_flAlphaVisibility = RemapValClamped(
      flVisibility, pVisibilityInputs->m_flInputMin,
      pVisibilityInputs->m_flInputMax, pVisibilityInputs->m_flAlphaScaleMin,
      pVisibilityInputs->m_flAlphaScaleMax);
  pVisibilityData->m_flRadiusVisibility = RemapValClamped(
      flVisibility, pVisibilityInputs->m_flInputMin,
      pVisibilityInputs->m_flInputMax, pVisibilityInputs->m_flRadiusScaleMin,
      pVisibilityInputs->m_flRadiusScaleMax);
...
static SheetSequenceSample_t s_DefaultSheetSequence = {
    {
        {0.0f, 0.0f, 1.0f, 1.0f, 0.0f, 0.0f, 1.0f,
         1.0f}, // SequenceSampleTextureCoords_t image 0
        {0.0f, 0.0f, 1.0f, 1.0f, 0.0f, 0.0f, 1.0f,
         1.0f} // SequenceSampleTextureCoords_t image 1
    },
    1.0f // m_fBlendFactor
};
...
class C_OP_RenderPoints : public CParticleRenderOperatorInstance {
  ...
  virtual void InitializeContextData(CParticleCollection *pParticles,
                                     void *pContext) const {
    C_OP_RenderPointsContext_t *pCtx =
        reinterpret_cast<C_OP_RenderPointsContext_t *>(pContext);
...
void C_OP_RenderPoints::Render(IMatRenderContext *pRenderContext,
  ...
  int nParticles;
  const ParticleRenderData_t *pRenderList = pParticles->GetRenderList(
      pRenderContext, true, &nParticles, &pCtx->m_VisibilityData);
...
class C_OP_RenderSprites : public C_OP_RenderPoints {
  ...
  void RenderSpriteCard(CMeshBuilder &meshBuilder,
                        C_OP_RenderSpritesContext_t *pCtx,
                        SpriteRenderInfo_t &info, int hParticle,
                        ParticleRenderData_t const *pSortList,
  ...
  void RenderNonSpriteCardZRotating(
      CMeshBuilder &meshBuilder, C_OP_RenderSpritesContext_t *pCtx,
...
DMXELEMENT_UNPACK_FIELD("orientation control point", "-1", int,
...
const SheetSequenceSample_t *
GetSampleForSequence(CSheet *pSheet, float flCreationTime, float flCurTime,
...
void C_OP_RenderSprites::RenderNonSpriteCardCameraFacing(
  ...
  int nParticles;
  const ParticleRenderData_t *pSortList = pParticles->GetRenderList(
      pRenderContext, true, &nParticles, &pCtx->m_VisibilityData);
...
void C_OP_RenderSprites::RenderNonSpriteCardZRotating(
  ...
  while (nParticles) {
    ...
    g_pParticleSystemMgr->TallyParticlesRendered(nParticlesInBatch * 4 * 3,
                                                 nParticlesInBatch * 6 * 3);
    ...
    for (int i = 0; i < nParticlesInBatch; i++) {
      int hParticle = (--pSortList)->m_nIndex;
      RenderNonSpriteCardZRotating(meshBuilder, pCtx, info, hParticle,
                                   vecCameraPos, pSortList);
    }
    meshBuilder.End();
    pMesh->Draw();
  }
...
void C_OP_RenderSprites::RenderUnsortedNonSpriteCardZRotating(
  ...
  int nParticles;
  const ParticleRenderData_t *pSortList = pParticles->GetRenderList(
      pRenderContext, false, &nParticles, &pCtx->m_VisibilityData);
...
void C_OP_RenderSprites::RenderNonSpriteCardOriented(
  ...
  while (nParticles) {
    ...
    for (int i = 0; i < nParticlesInBatch; i++) {
      int hParticle = (--pSortList)->m_nIndex;
      RenderNonSpriteCardOriented(meshBuilder, pCtx, info, hParticle,
                                  vecCameraPos, pSortList, bUseYaw);
    }
...
void C_OP_RenderSprites::RenderSpriteCard(CMeshBuilder &meshBuilder,
  ...
  if (info.m_pSheet) {
    ...
    // ld_stride ], nOffset ); 			flAgeScale = ( flLifetime > 0.0f ) ? ( 1.0f /
    // flLifetime ) * SEQUENCE_SAMPLE_COUNT : 0.0f;
    // 		}
    if (m_bAnimateInFPS) {
...
void C_OP_RenderSprites::RenderTwoSequenceSpriteCard(
    CMeshBuilder &meshBuilder, C_OP_RenderSpritesContext_t *pCtx,
    SpriteRenderInfo_t &info, int hParticle,
...
void C_OP_RenderSprites::Render(IMatRenderContext *pRenderContext,
  ...
  if (pCtx->m_VisibilityData.m_bUseVisibility) {
    SetupParticleVisibility(pParticles, &pCtx->m_VisibilityData,
                            &VisibilityInputs, &pCtx->m_nQueryHandle);
  }
  ...
  if (m_nOrientationType == 2) {
    pVar = pMaterial->FindVarFast("$orientationMatrix",
                                  &pCtx->m_nOrientationMatrixVarToken);
    if (pVar) {
      VMatrix mat;
      if (m_nOrientationControlPoint < 0) {
        MatrixSetIdentity(mat);
      } else {
        pParticles->GetControlPointTransformAtCurrentTime(
            m_nOrientationControlPoint, &mat);
      }
  ...
  while (nParticles) {
    ...
    if (bUseInstancing) {
      g_pParticleSystemMgr->TallyParticlesRendered(
          vertexCount * (primType == MATERIAL_TRIANGLES ? 3 : 4));
    ...
    if (meshBuilder.TextureCoordinateSize(5)) // second sequence?
    {
      for (int i = 0; i < nParticlesInBatch; i++) {
        int hParticle = (--pSortList)->m_nIndex;
        RenderTwoSequenceSpriteCard(meshBuilder, pCtx, info, hParticle,
                                    pSortList, &vecCamera);
      }
    } else {
      for (int i = 0; i < nParticlesInBatch; i++) {
        int hParticle = (--pSortList)->m_nIndex;
        RenderSpriteCard(meshBuilder, pCtx, info, hParticle, pSortList,
                         &vecCamera);
      }
    }
    meshBuilder.End();
...
void C_OP_RenderSprites::RenderUnsorted(CParticleCollection *pParticles,
  ...
  if (!pParticles->m_pDef->GetMaterial()->IsSpriteCard()) {
    ...
    case 0:
      ...
      // sorted version
      Warning("C_OP_RenderSprites::RenderUnsorted: Attempting to use an "
    ...
    case 1:
      RenderUnsortedNonSpriteCardZRotating(pParticles, pContext, pRenderContext,
                                           meshBuilder, nVertexOffset,
                                           nFirstParticle, nParticleCount);
      break;

    case 2:
      RenderUnsortedNonSpriteCardOriented(pParticles, pContext, pRenderContext,
                                          meshBuilder, nVertexOffset,
                                          nFirstParticle, nParticleCount);
      break;
    }
...
class C_OP_RenderSpritesTrail : public CParticleRenderOperatorInstance {
  ...
  virtual void InitializeContextData(CParticleCollection *pParticles,
                                     void *pContext) const {
    C_OP_RenderSpriteTrailContext_t *pCtx =
        reinterpret_cast<C_OP_RenderSpriteTrailContext_t *>(pContext);
    if (VisibilityInputs.m_nCPin >= 0)
      pCtx->m_VisibilityData.m_bUseVisibility = true;
    else
      pCtx->m_VisibilityData.m_bUseVisibility = false;
...
void C_OP_RenderSpritesTrail::RenderSpriteTrail(
    CMeshBuilder &meshBuilder, SpriteTrailRenderInfo_t &info, int hParticle,
    const Vector &vecCameraPos, float flOODt,
    ParticleRenderData_t const *pSortList) const {
  Assert(hParticle != -1);
  int nGroup = hParticle / 4;
...
class C_OP_RenderRope : public CParticleOperatorInstance {
  ...
  // We connect neighboring particle instances to each other, so if the order
  // isn't maintained we will have a particle that jumps back to the wrong place
  // and look terrible.
  virtual bool RequiresOrderInvariance(void) const OVERRIDE { return true; }
...
void C_OP_RenderRope::RenderSpriteCard(CParticleCollection *pParticles,
  ...
  g_pParticleSystemMgr->TallyParticlesRendered(
      2 + nNumSegmentsIWillRenderPerBatch * nNumVerticesPerSegment * 3,
      nNumIndicesPerSegment * nNumSegmentsIWillRenderPerBatch * 3);

  meshBuilder.Begin(pMesh, MATERIAL_TRIANGLES,
                    2 + nNumSegmentsIWillRenderPerBatch *
                            nNumVerticesPerSegment,
...
void C_OP_RenderRope::Render(IMatRenderContext *pRenderContext,
  ...
  while (nParticles) {
    ...
    g_pParticleSystemMgr->TallyParticlesRendered(nVertCount * 3,
                                                 nIndexCount * 3);
...
class C_OP_RenderBlobs : public CParticleRenderOperatorInstance {
  ...
  virtual void InitializeContextData(CParticleCollection *pParticles,
                                     void *pContext) const {
    C_OP_RenderBlobsContext_t *pCtx =
        reinterpret_cast<C_OP_RenderBlobsContext_t *>(pContext);
    if (VisibilityInputs.m_nCPin >= 0)
      pCtx->m_VisibilityData.m_bUseVisibility = true;
    else
      pCtx->m_VisibilityData.m_bUseVisibility = false;
...
void C_OP_RenderBlobs::Render(IMatRenderContext *pRenderContext,
...
#endif
#if 1
  RENDERER_CLASS::setCalcCornerFunc(CALC_CORNER_NORMAL_CI_SIZE,
                                    calcCornerNormal);
  ...
  // FIXME: Make this configurable. Not all shaders perform lighting. Although
  // it's pretty likely for isosurface shaders.
  g_pParticleSystemMgr->Query()->SetUpLightingEnvironment(bbCenter);

  // FIXME: Ugly hack to get particle system location to a special blob shader
  // lighting proxy
