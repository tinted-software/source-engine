...
static ALIGN16 ParticleRenderData_t
    s_SortedIndexList[MAX_PARTICLES_IN_A_SYSTEM] ALIGN16_POST;
...
void s_GenerateData(Vector CameraPos, CParticleVisibilityData *pVisibilityData,
  ...
  // indexing. We will generate the index as float and use magicf2i to convert
  // to integer
  ...
  do {
    ...
    if (eSortKeyMode == SORT_KEY_DISTANCE) {
      ...
      Assert(eSortKeyMode == SORT_KEY_CREATION_TIME ||
             eSortKeyMode == SORT_KEY_NONE);
    ...
    if (bUseVis) {
      fl4FinalAlpha = MaxSIMD(
          Four_Zeros, MinSIMD(Four_Ones, MulSIMD(fl4FinalAlpha, fl4AlphaVis)));
      fl4FinalRadius = MulSIMD(fl4FinalRadius, fl4RadVis);
    }

    // convert float 0..1 to int 0..255
    fl4FinalAlpha =
        AddSIMD(MulSIMD(fl4FinalAlpha, fl4AlphaScale), Four_2ToThe23s);

    // now, we will use simd transpose to write the output
    fltx4 i4Indices =
        AndSIMD(fl4OutIdx, LoadAlignedSIMD((float *)g_SIMD_Low16BitsMask));
...
static bool SortLessFunc(const ParticleRenderData_t &left,
                         const ParticleRenderData_t &right) {
  return TREATASINT(left.m_flSortKey) < TREATASINT(right.m_flSortKey);
}

void CParticleCollection::GenerateSortedIndexList(
    Vector vecCamera, CParticleVisibilityData *pVisibilityData, bool bSorted) {
...
// check data
  ...
  if (bSorted) {
    ...
    std::make_heap(s_SortedIndexList, s_SortedIndexList + m_nActiveParticles,
                   SortLessFunc);
    std::sort_heap(s_SortedIndexList, s_SortedIndexList + m_nActiveParticles,
                   SortLessFunc);
  }
#endif
}
...
CParticleCollection::GetRenderList(IMatRenderContext *pRenderContext,
