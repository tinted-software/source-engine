    ...
    if (fExp != 1.0f) {
      ...
      while (nBlockCount--) {
        fltx4 fl4RandVal = AddSIMD(
            val0,
            MulSIMD(Pow_FixedPoint_Exponent_SIMD(RandSIMD(nRandContext), nExp),
                    val_d));
        fltx4 fl4Sign =
...
static inline void RandomPointOnUnitSphere(int nRandContext, FourVectors &out) {
  // generate 4 random points on the unit sphere. uses Marsaglia (1972) method
  // from http://mathworld.wolfram.com/SpherePointPicking.html

  fltx4 f4x1 =
      SubSIMD(MulSIMD(Four_Twos, RandSIMD(nRandContext)), Four_Ones); // -1..1
  fltx4 f4x2 =
      SubSIMD(MulSIMD(Four_Twos, RandSIMD(nRandContext)), Four_Ones); // -1..1
  ...
  while (IsAnyNegative(badMask)) {
    f4x1 = MaskedAssign(
        badMask, SubSIMD(MulSIMD(Four_Twos, RandSIMD(nRandContext)), Four_Ones),
        f4x1);
    f4x2 = MaskedAssign(
        badMask, SubSIMD(MulSIMD(Four_Twos, RandSIMD(nRandContext)), Four_Ones),
        f4x2);
  ...
  for (; nParticleCount--; start_p++) {
    ...
    if (m_bUseHighestEndCP) {
      ...
      if (m_flEndCPGrowthTime != 0.0f) {
        flStrength = min(pParticles->m_flCurTime, m_flEndCPGrowthTime) /
                     m_flEndCPGrowthTime;
      }
...
void C_INIT_CreateWithinSphere::InitNewParticlesBlock(
  ...
  if ((!bMustUseScalar) &&
    ...
    v4PrevControlPointPosition.DuplicateVector(
        pParticles->m_ControlPoints[m_nControlPointNumber].m_PrevPosition);
    FourVectors v4ControlPointDelta;
    v4ControlPointDelta.DuplicateVector(
        pParticles->m_ControlPoints[m_nControlPointNumber].m_Position);
    v4ControlPointDelta -= v4PrevControlPointPosition;

    float flOODT =
        (pParticles->m_flDt > 0.0) ? (1.0 / pParticles->m_flDt) : 0.0;
    fltx4 fl4OODt = ReplicateX4(flOODT);
    fltx4 fl4PrevTime =
        ReplicateX4(pParticles->m_flCurTime - pParticles->m_flDt);
    ...
    for (int nComp = 0; nComp < 3; nComp++) {
      v4ConditionalAbsMask[nComp] =
          (m_vecDistanceBiasAbs[nComp] > 0)
    ...
    bool bDoRandSpeed = (m_fSpeedMax > 0.) ||
    ...
    v4CPForward.DuplicateVector(
        pParticles->m_ControlPoints[m_nControlPointNumber].m_ForwardVector);
    FourVectors v4CPUp;
    v4CPUp.DuplicateVector(
        pParticles->m_ControlPoints[m_nControlPointNumber].m_UpVector);
    FourVectors v4CPRight;
    v4CPRight.DuplicateVector(
        pParticles->m_ControlPoints[m_nControlPointNumber].m_RightVector);
    ...
    CParticleOperatorInstance::InitNewParticlesBlock(
        pParticles, start_block, n_blocks, nAttributeWriteMask, pContext);
}
...
class C_INIT_RandomColor : public CParticleOperatorInstance {
  ...
  virtual void InitializeContextData(CParticleCollection *pParticles,
                                     void *pContext) const {
    C_OP_RandomColorContext_t *pCtx =
        reinterpret_cast<C_OP_RandomColorContext_t *>(pContext);
  ...
  void InitNewParticlesScalar(CParticleCollection *pParticles, int start_p,
    ...
    if (m_flTintPerc) {
      if (pParticles->m_pParent &&
          pParticles->m_pParent->m_LocalLightingCP == m_nTintCP) {
        ...
        Vector vecOrigin;
        pParticles->GetControlPointAtTime(m_nTintCP, pParticles->m_flCurTime,
                                          &vecOrigin);

        if (((pCtx->m_vPrevPosition - vecOrigin).Length() >=
             m_flUpdateThreshold) ||
  ...
  virtual void InitNewParticlesBlock(CParticleCollection *pParticles,
    ...
    FourVectors *pColor = pParticles->Get4VAttributePtrForWrite(
        PARTICLE_ATTRIBUTE_TINT_RGB, &attr_stride);

    pColor += attr_stride * start_block;

    FourVectors fvColorMin;
    fvColorMin.DuplicateVector(
        Vector(m_flNormColorMin[0], m_flNormColorMin[1], m_flNormColorMin[2]));
    FourVectors fvColorWidth;
    fvColorWidth.DuplicateVector(
        Vector(m_flNormColorMax[0] - m_flNormColorMin[0],
...
DMXELEMENT_UNPACK_FIELD("tint update movement threshold", "32", float,
                        m_flUpdateThreshold)
...
DMXELEMENT_UNPACK_FIELD("bulge control 0=random 1=orientation of start pnt "
...
class C_INIT_CreateInHierarchy : public CParticleOperatorInstance {
  ...
  virtual uint64 GetReadControlPointMask() const {
    ...
    uint64 nEndMask = m_bUseHighestEndCP
...
void C_INIT_CreateInHierarchy::InitNewParticlesScalar(
  ...
  for (; nParticleCount--; start_p++) {
    ...
    if ((pParticles->m_flCurTime <= m_flGrowthTime) && (nRealEndPoint > 0)) {
      float flCurrentEndCP = RemapValClamped(
          *ct, 0.0f, m_flGrowthTime,
          min(m_nDesiredStartPoint + 1, nRealEndPoint), nRealEndPoint);
      nEndCP = pParticles->RandomInt(
          min(m_nDesiredStartPoint + 1, (int)flCurrentEndCP), flCurrentEndCP);
    ...
    //irregular surfaces 	CBaseTrace tr; 	Vector RayStart=Pnt; 	float flRadius =
    //m_flTraceDist; 	g_pParticleSystemMgr->Query()->TraceLine( RayStart, (
    //RayStart + ( TraceDir * flRadius ) ), MASK_SOLID, NULL,
    ...
    if (pCtx->m_nParticleCount >= m_flNumToAssign ||
      ...
      if (m_bLoop) {
        ...
        pCtx->m_nParticleCount =
            min(pCtx->m_nParticleCount, (int)(m_flNumToAssign - 1));
...
void C_INIT_CreateFromParentParticles::InitNewParticlesScalar(
  ...
  if (nActiveParticles == 0) {
    while (nParticleCount--) {
      float *lifespan = pParticles->GetFloatAttributePtrForWrite(
          PARTICLE_ATTRIBUTE_LIFE_DURATION, start_p);
      *lifespan = 0.0f;
      start_p++;
    }
  ...
  for (; nParticleCount--; start_p++) {
    ...
    float flSubFrame =
        RemapValClamped(*ct, flPrevTime, pParticles->m_flCurTime, 0, 1);
...
DEFINE_PARTICLE_OPERATOR(C_INIT_LifespanFromVelocity,
                         "Lifetime from Time to Impact", OPERATOR_GENERIC);

BEGIN_PARTICLE_OPERATOR_UNPACK(C_INIT_LifespanFromVelocity)
DMXELEMENT_UNPACK_FIELD_STRING("trace collision group", "NONE",
                               m_CollisionGroupName)
DMXELEMENT_UNPACK_FIELD("maximum trace length", "1024", float,
                        m_flMaxTraceLength)
DMXELEMENT_UNPACK_FIELD("trace offset", "0", float, m_flTraceOffset)
DMXELEMENT_UNPACK_FIELD("trace recycle tolerance", "64", float,
                        m_flTraceTolerance)
...
void C_INIT_LifespanFromVelocity::InitNewParticlesScalar(
  ...
  if (pParticles->m_pParent)
    ppCtx = &(pParticles->m_pParent
                  ->m_pCollisionCacheData[COLLISION_MODE_INITIAL_TRACE_DOWN]);
  else
    ppCtx =
        &(pParticles->m_pCollisionCacheData[COLLISION_MODE_INITIAL_TRACE_DOWN]);
  ...
  // Trace length takes the max trace and subtracts the offset to get the actual
  // total.
  float flTotalTraceDist = m_flMaxTraceLength - m_flTraceOffset;

  // Offset percentage to account for if we've hit something within the offset
  // (but not spawn) area
  ...
  while (nParticleCount--) {
    ...
    // Normalize the delta and get the offset to use from the normalized delta
    // times the offset
    ...
    // Use SIMD section to interface with plane cache, even though we're not
    // SIMD here Test versus existing Data
    ...
    for (int i = 0; i < pCtx->m_nActivePlanes; i++) {
      if (pCtx->m_bPlaneActive[i]) {
        fltx4 fl4TrialDistance =
            MaxSIMD(fvStartPnt.DistSqrToLineSegment(pCtx->m_TraceStartPnt[i],
                                                    pCtx->m_TraceEndPnt[i]),
                    fvEndPnt.DistSqrToLineSegment(pCtx->m_TraceStartPnt[i],
                                                  pCtx->m_TraceEndPnt[i]));
    ...
    if (IsAllGreaterThan(fl4ClosestDist, ReplicateX4(flTol))) {
      ...
      g_pParticleSystemMgr->Query()->TraceLine(vecStartPnt, vecEndPnt,
                                               CONTENTS_SOLID, NULL,
                                               m_nCollisionGroupNumber, &tr);

      // Set the lifespan to 0 if we start solid, our trace distance is 0, or we
      // hit within the offset area
      if ((tr.fraction < (1 - flOffsetPct)) || tr.startsolid ||
          flTotalTraceDist == 0.0f) {
        *dtime = 0.0f;
        ...
        Vector vPointOnPlane =
            vecStartPnt + (tr.fraction * (vecEndPnt - vecStartPnt));
        ...
        pCtx->m_nActivePlanes =
            min(m_nAllowedPlanes, pCtx->m_nActivePlanes + 1);
      }
    }
    ...
    // Need to use the adjusted value of the trace length and collision point to
...
void C_INIT_LifespanFromVelocity::InitNewParticlesBlock(
    CParticleCollection *pParticles, int start_block, int n_blocks,
    int nAttributeWriteMask, void *pContext) const {
  CWorldCollideContextData **ppCtx;
  if (pParticles->m_pParent)
  ...
  FourVectors *pXYZ = pParticles->Get4VAttributePtrForWrite(
      PARTICLE_ATTRIBUTE_XYZ, &attr_stride);
  pXYZ += attr_stride * start_block;
  FourVectors *pPrev_XYZ = pParticles->Get4VAttributePtrForWrite(
      PARTICLE_ATTRIBUTE_PREV_XYZ, &attr_stride);
  pPrev_XYZ += attr_stride * start_block;
  fltx4 *pLifespan = pParticles->GetM128AttributePtrForWrite(
  ...
  while (n_blocks--) {
    ...
    for (int i = 0; i < pCtx->m_nActivePlanes; i++) {
      if (pCtx->m_bPlaneActive[i]) {
        ...
        fl4ClosestDist =
            MaskedAssign(fl4ClosestDist, fl4TrialDistance, fl4Nearestmask);
        v4PointOnPlane.x = MaskedAssign(
            fl4Nearestmask, pCtx->m_PointOnPlane[i].x, v4PointOnPlane.x);
        v4PointOnPlane.y = MaskedAssign(
            fl4Nearestmask, pCtx->m_PointOnPlane[i].y, v4PointOnPlane.y);
        v4PointOnPlane.z = MaskedAssign(
            fl4Nearestmask, pCtx->m_PointOnPlane[i].z, v4PointOnPlane.z);
      }
    }
      ...
      for (int i = 0; i < 4; i++) {
        if (nMask & (1 << i)) {
          ...
          g_pParticleSystemMgr->Query()->TraceLine(
              start, end, CONTENTS_SOLID, NULL, m_nCollisionGroupNumber, &tr);

          // Set the lifespan to 0 if we start solid, our trace distance is 0,
          // or we hit within the offset area
          if ((tr.fraction < (1 - flOffsetPct)) || tr.startsolid ||
              flTotalTraceDist == 0.0f) {
...
DEFINE_PARTICLE_OPERATOR(C_INIT_CreateFromPlaneCache,
                         "Position from Parent Cache", OPERATOR_PI_POSITION);
