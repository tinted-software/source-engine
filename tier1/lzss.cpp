...
unsigned char *CLZSS::CompressNoAlloc(const unsigned char *pInput,
                                      int inputLength,
...
//-----------------------------------------------------------------------------
unsigned char *CLZSS::Compress(const unsigned char *pInput, int inputLength,
                               unsigned int *pOutputSize) {
...
unsigned int CLZSS::SafeUncompress(const unsigned char *pInput,
  ...
  if (!actualSize || actualSize > unBufSize ||
  ...
  for (;;) {
    ...
    if (cmdByte & 0x01) {
      ...
      if (totalBytes + count > unBufSize || pSource < pOrigOutput)
        return 0;
      ...
      if (totalBytes + 1 > unBufSize || pInput > pInputEnd)
        return 0;
...
//-----------------------------------------------------------------------------
unsigned int CLZSS::Uncompress(const unsigned char *pInput,
                               unsigned char *pOutput) {
  unsigned int totalBytes = 0;
  int cmdByte = 0;
  int getCmdByte = 0;
