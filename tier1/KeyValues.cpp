...
public:
  ...
  CKeyErrorContext(intp symName) { Init(symName); }
...
bool KeyValues::LoadFromFile(IBaseFileSystem *filesystem,
  ...
  */
  ...
  if (!f) {
    COM_TimestampedLog("KeyValues::LoadFromFile(%s%s%s): End / FileNotFound",
                       pathID ? pathID : "", pathID && resourceName ? "/" : "",
                       resourceName ? resourceName : "");
    return false;
  }
...
//already exist 			(which ensures a valid pointer will be returned)
//-----------------------------------------------------------------------------
...
void KeyValues::AddSubkeyUsingKnownLastChild(KeyValues *pSubkey,
  ...
  if (pLastChild == NULL) {
    ...
    //the last one 		#ifdef _DEBUG 			KeyValues *pTempDat = m_pSub; 			while (
...
const wchar_t *KeyValues::GetWString(const char *keyName,
  ...
  if (dat) {
    ...
    case TYPE_STRING: {
      ...
      int result =
          Q_UTF8ToUnicode(dat->m_sValue, pWBuf, bufSize * sizeof(wchar_t));
...
void KeyValues::CopyKeyValuesFromRecursive(const KeyValues &rootSrc) {
  ...
  // version which is uglier but doesn't blow the stack. This uses breadth-first
  // traversal.
  ...
  while (nodeQ.Count() > 0) {
    ...
    // Process all the siblings of the current node. If anyone has a child, add
    // it to the queue.
    while (cs.src) {
...
void KeyValues::ParseIncludedKeys(char const *resourceName,
  ...
  if (newKV->LoadFromFile(pFileSystem, fullpath, pPathID)) {
    ...
    DevMsg("KeyValues::ParseIncludedKeys: Couldn't load included keyvalue file "
           "%s\n",
           fullpath);
    newKV->deleteThis();
  }
...
bool KeyValues::LoadFromBuffer(char const *resourceName, const char *pBuffer,
  ...
  COM_TimestampedLog("KeyValues::LoadFromBuffer(%s%s%s): Begin",
                     pPathID ? pPathID : "", pPathID && resourceName ? "/" : "",
                     resourceName ? resourceName : "");
  ...
  COM_TimestampedLog("KeyValues::LoadFromBuffer(%s%s%s): End",
                     pPathID ? pPathID : "", pPathID && resourceName ? "/" : "",
                     resourceName ? resourceName : "");
...
void KeyValues::RecursiveLoadFromBuffer(char const *resourceName,
  ...
  // re-locate it each time we append the next subkey, which causes O(N^2) time
  KeyValues *pLastChild = FindLastSubKey();
  ;

  // Keep parsing until we hit the closing brace which terminates this block, or
  // a parse error
  while (1) {
    bool bAccepted = true;
