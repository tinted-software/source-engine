...
// behave as if it were running on a case insensitive file system.
// Unfortunately, this is needed by both steam2 and steam3.  It was decided to
// check the source into both locations, otherwise someone would find the .o and
// have no idea where to go for the source if it was in the 'other' tree.  Also,
// because this needs to be linked into every elf binary, the .o is checked in
// for Steam3 so that it is always available.  In Steam2 it sits with the
// PosixWin32.cpp implementation and gets compiled along side of it through the
// make system.  If you are reading this in Steam3, you will probably want to
// actually make your changes in steam2 and do a baseless merge to the steam3
// copy.
//
// HOWTO: Add a new function.  Add the function with _WRAP to the makefiles as
// noted below. Add the implementation to pathmatch.cpp - probably mimicking the
// existing functions. Build steam2 and copy to matching steam3/client.  Take
// the pathmatch.o from steam 2 and check it in to steam3 (in the location noted
// below).  Full rebuild (re-link really) of steam3.  Test steam and check in.
//
// If you are looking at updating this file, please update the following as
// needed:
//
...
//
// STEAM2.../Projects/GazelleProto/Makefile.shlib.base - contains _WRAP
// references STEAM2.../Projects/Common/Misc/pathmatch.cpp - Where the source is
// checked in, keep in sync with: STEAM3.../src/common/pathmatch.cpp - should be
// identical to previous file, but discoverable in steam3.
// STEAM3.../src/lib/linux32/release/pathmatch.o - steam3 checked in version
// STEAM3.../src/devtools/makefile_base_posix.mak - look for the _WRAP
// references
...
//  Even if it wasn't under the zlib license, Ryan wrote all this code
//  originally.

#define UNICODE_BOGUS_CHAR_VALUE 0xFFFFFFFF
#define UNICODE_BOGUS_CHAR_CODEPOINT '?'

inline __attribute__((always_inline)) static uint32_t
utf8codepoint(const char **_str) {
  const char *str = *_str;
  uint32_t retval = 0;
...
locate_case_fold_mapping(const uint32_t from, uint32_t *to) {
...
utf8casecmp_loop(const uint32_t *folded1, const uint32_t *folded2) {
  while (true) {
...
static bool Descend(char *pPath, size_t nStartIdx, bool bAllowBasenameMismatch,
                    size_t nLevel = 0) {
  ...
  while (pEntry) {
    ...
    // the candidate must match the target, but not be a case-identical match
    // (we would have looked there in the short-circuit code above, so don't
    // look again)
    bool bMatches =
        (strcasecmp(CDirTrimmer(pszComponent, cbComponent), pEntry->d_name) ==
             0 &&
  ...
  if (bIsDir) {
    DEBUG_MSG("(%zu) readdir failed to find '%s' in '%s'\n", nLevel,
              (const char *)CDirTrimmer(pszComponent, cbComponent),
              (const char *)CDirTrimmer(pPath, nStartIdx));
  }
...
typedef std::map<std::string, std::pair<std::string, time_t>>::iterator
    resultCacheItr_t;
static resultCache_t resultCache;
static const int k_cMaxCacheLifetimeSeconds = 2;
#endif // DO_PATHMATCH_CACHE

PathMod_t pathmatch(const char *pszIn, char **ppszOut,
                    bool bAllowBasenameMismatch, char *pszOutBuf,
                    size_t OutBufLen) {
  // Path matching can be very expensive, and the cost is unpredictable because
  // it depends on how many files are in directories on a user's machine.
  // Therefore it should be disabled whenever possible, and only enabled in
  // environments (such as running with loose files such as out of Perforce)
  // where it is needed.
  ...
  *ppszOut = NULL;
  ...
  if (pPath) {
    // I believe this code is broken. I'm guessing someone wanted to avoid
    // lowercasing
    //	the path before the steam directory - but it's actually skipping
    //lowercasing 	whenever steam is found anywhere - including the filename. For
    //example,
...
public:
  CWrap(const char *pSuppliedPath, bool bAllowMismatchedBasename)
    ...
    m_eResult =
        pathmatch(m_pSuppliedPath, &m_pBestMatch, bAllowMismatchedBasename,
                  m_BestMatchBuf, sizeof(m_BestMatchBuf));
...
void test(const char *pszFile, bool bAllowBasenameMismatch) {
  char *pNewPath;
  char NewPathBuf[512];
  PathMod_t nStat = pathmatch(pszFile, &pNewPath, bAllowBasenameMismatch,
...
WRAP(__xstat, int, int __ver, __const char *__filename,
     struct stat *__stat_buf) {
  return CALL(__xstat)(__ver, CWrap(__filename, false), __stat_buf);
}

WRAP(__lxstat, int, int __ver, __const char *__filename,
     struct stat *__stat_buf) {
  return CALL(__lxstat)(__ver, CWrap(__filename, false), __stat_buf);
}

WRAP(__xstat64, int, int __ver, __const char *__filename,
     struct stat *__stat_buf) {
  return CALL(__xstat64)(__ver, CWrap(__filename, false), __stat_buf);
}

WRAP(__lxstat64, int, int __ver, __const char *__filename,
     struct stat *__stat_buf) {
...
WRAP(mount, int, const char *source, const char *target,
  ...
  return CALL(mount)(CWrap(source, false), CWrap(target, false), filesystemtype,
                     mountflags, data);
}
