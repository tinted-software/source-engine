...
void CUtlString::SetDirect(const char *pValue, int nChars) {
  if (pValue && nChars > 0) {
    if (pValue == m_pString) {
      ...
      return; // Do nothing. Realloc in AllocMemory might move pValue's location
              // resulting in a bad memcpy.
    }
...
int CUtlString::FormatV(const char *pFormat, va_list marker) {
  char tmpBuf[4096]; //< Nice big 4k buffer, as much memory as my first computer
                     //had, a Radio Shack Color Computer
