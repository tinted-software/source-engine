...
static void CopyPending(int len, uint8 const *rawbytes, uint8 *&outbuf,
                        uint8 const *limit) {
    ...
    if (walk < NewBlock + NewSize - MIN_MATCH_LEN) {
      ...
      for (BlockPtr *b = HashedMatches[hash1]; b; b = b->Next) {
        ...
        if ((match_of > -32768) && (match_of < 32767)) {
          int max_mlength = min(65535, (int)((ptrdiff_t)OldBlock + OldSize -
                                             (ptrdiff_t)b->dataptr));
          max_mlength = min(max_mlength, (int)((ptrdiff_t)NewBlock + NewSize -
                                               (ptrdiff_t)walk));
          int i;
...
int FindDiffsLowMemory(uint8 const *NewBlock, uint8 const *OldBlock,
  ...
  while (walk < NewBlock + NewSize) {
    while ((oldptr - OldBlock < walk - NewBlock + 40) &&
      ...
      uint16 hash1 = (oldptr[0] + oldptr[1] + oldptr[2] + oldptr[3]) &
