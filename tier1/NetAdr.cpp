...
void netadr_t::ToString(char *pchBuffer, uint32 unBufferSize,
                        bool onlyBase) const {
  ...
  if (type == NA_LOOPBACK) {
    ...
    if (onlyBase) {
      V_snprintf(pchBuffer, unBufferSize, "%i.%i.%i.%i", ip[0], ip[1], ip[2],
                 ip[3]);
    } else {
      V_snprintf(pchBuffer, unBufferSize, "%i.%i.%i.%i:%i", ip[0], ip[1], ip[2],
                 ip[3], ntohs(port));
    }
  } else {
...
bool netadr_t::SetFromString(const char *pch, bool bUseDNS) {
  ...
  if (!V_strnicmp(address, "localhost", 9)) {
    V_memcpy(address, "127.0.0.1",
             9); // Note use of memcpy allows us to keep the colon and rest of
                 // string since localhost and 127.0.0.1 are both 9 characters.
  }

  // Starts with a number and has a dot
  if (address[0] >= '0' && address[0] <= '9' && strchr(address, '.')) {
    int n1 = -1, n2 = -1, n3 = -1, n4 = -1,
        n5 = 0; // set port to 0 if we don't parse one
    int nRes = sscanf(address, "%d.%d.%d.%d:%d", &n1, &n2, &n3, &n4, &n5);
    if (nRes < 4 || n1 < 0 || n1 > 255 || n2 < 0 || n2 > 255 || n3 < 0 ||
        n3 > 255 || n4 < 0 || n4 > 255 || n5 < 0 || n5 > 65535)
      return false;
    SetIP(n1, n2, n3, n4);
    SetPort((uint16)n5);
    return true;
  ...
  if (bUseDNS) {
// X360TBD:
// dgoodenough - since this is skipped on X360, seems reasonable to skip as well
// on PS3 PS3_BUILDFIX
// FIXME - Leap of faith, this works without asserting on X360, so I assume it
// will on PS3
...
void netadr_t::SetFromSocket(int hSocket) {
  // dgoodenough - since this is skipped on X360, seems reasonable to skip as
  // well on PS3 PS3_BUILDFIX
  // FIXME - Leap of faith, this works without asserting on X360, so I assume it
  // will on PS3
#if !defined(_X360) && !defined(_PS3)
  Clear();
