...
char const *V_stristr(char const *pStr, char const *pSearch) {
  ...
  while (*pLetter != 0) {
    ...
    if (FastToLower((unsigned char)*pLetter) ==
        FastToLower((unsigned char)*pSearch)) {
      // Check for match
      ...
      while (*pTest != 0) {
        ...
        if (FastToLower((unsigned char)*pMatch) !=
            FastToLower((unsigned char)*pTest))
          break;
...
// warning C6059: Incorrect length parameter in call to 'strncat'. Pass the
// number of remaining characters, not the buffer size of 'argument 1' warning
// C6386: Buffer overrun: accessing 'argument 1', the writable size is
// 'destBufferSize' bytes, but '1000' bytes might be written These warnings were
// investigated through code inspection and writing of tests and they are
...
int V_vsnwprintf(wchar_t *pDest, int maxLen, const wchar_t *pFormat,
                 va_list params) {
  Assert(maxLen > 0);
...
char *V_strncat(char *pDest, const char *pSrc, size_t destBufferSize,
  ...
  ANALYZE_SUPPRESS(6059); // warning C6059: : Incorrect length parameter in call
                          // to 'strncat'. Pass the number of remaining
                          // characters, not the buffer size of 'argument 1'
  char *pOut = strncat(pDest, pSrc, charstocopy);
  return pOut;
}

wchar_t *V_wcsncat(INOUT_Z_CAP(cchDest) wchar_t *pDest, const wchar_t *pSrc,
                   size_t cchDest, int max_chars_to_copy) {
  size_t charstocopy = (size_t)0;
...
//disruptive 			behavior when used in names, web pages, chat windows, etc.
//
//			characters in this set are removed from the beginning
//and/or end of strings 			by Q_AggressiveStripPrecedingAndTrailingWhitespaceW()
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
static wchar_t *StripWhitespaceWorker(int cchLength, wchar_t *pwch,
                                      bool *pbStrippedWhitespace,
                                      bool bAggressive) {
...
bool Q_StripPrecedingAndTrailingWhitespaceW(wchar_t *pwch) {
  ...
  // Early out and don't convert if we don't have any chars or leading/trailing
  // ws.
  ...
  bool bStrippedWhitespace = false;
  pwchT = StripWhitespaceWorker(cch, pwch, &bStrippedWhitespace,
...
//-----------------------------------------------------------------------------
// Purpose: strips leading and trailing whitespace,
//		and also strips punctuation and formatting characters with
...
bool Q_StripPrecedingAndTrailingWhitespace(char *pch) {
  ...
  // Early out and don't convert if we don't have any chars or leading/trailing
  // ws.
  if ((cch < 1) || (!isspace((unsigned char)pch[0]) &&
                    !isspace((unsigned char)pch[cch - 1])))
    return false;
  ...
  bool bStrippedWhitespace = false;
  pwch = StripWhitespaceWorker(cwch - 1, pwch, &bStrippedWhitespace,
                               false /* not aggressive */);
...
//-----------------------------------------------------------------------------
int _V_UCS2ToUnicode(const ucs2 *pUCS2, wchar_t *pUnicode,
...
//-----------------------------------------------------------------------------
int _V_UnicodeToUCS2(const wchar_t *pUnicode, int cubSrcInBytes, char *pUCS2,
                     int cubDestSizeInBytes) {
#ifdef _WIN32
  // Figure out which buffer is smaller and convert from bytes to character
  // counts.
  int cchResult = min((size_t)cubSrcInBytes / sizeof(wchar_t),
...
//-----------------------------------------------------------------------------
int _V_UTF8ToUCS2(const char *pUTF8, int cubSrcInBytes, ucs2 *pUCS2,
                  int cubDestSizeInBytes) {
...
// now counting it as one even Posix since so many times our filepaths aren't
// actual paths but rather text strings passed in from data files, treating \ as
// a pathseparator covers the full range of cases
bool PATHSEPARATOR(char c) { return c == '\\' || c == '/'; }
...
bool V_RemoveDotSlashes(char *pFilename, char separator,
  ...
  bool bBoundary = true;
  while (*pIn) {
    if (bBoundary && pIn[0] == '.' && pIn[1] == '.' &&
        (PATHSEPARATOR(pIn[2]) || !pIn[2])) {
      // Get rid of /../ or trailing /.. by backing pOut up to previous
      // separator
      ...
      while (true) {
        if (pOut == pFilename) {
          bRetVal = false; // backwards compat. return value, even though we
                           // continue handling
          break;
        }
        --pOut;
      ...
      // Skip the '..' but not the slash, next loop iteration will handle
      // separator
      pIn += 2;
      bBoundary = (pOut == pFilename);
    } else if (bBoundary && pIn[0] == '.' &&
               (PATHSEPARATOR(pIn[1]) || !pIn[1])) {
...
bool V_MakeRelativePath(const char *pFullPath, const char *pDirectory,
  ...
  while (*pPath &&
         (FastToLower(*pPath) == FastToLower(*pDir) ||
...
void V_strtowcs(const char *pString, int nInSize, wchar_t *pWString,
  ...
  int result = MultiByteToWideChar(CP_UTF8, 0, pString, nInSize, pWString,
                                   nOutSizeInChars);
  // If the string completely fails to fit then MultiByteToWideChar will return
  // 0. If the string exactly fits but with no room for a null-terminator then
  // MultiByteToWideChar will happily fill the buffer and omit the
  // null-terminator, returning nOutSizeInChars. Either way we need to return an
  // empty string rather than a bogus and possibly not null-terminated result.
  if (result <= 0 || result >= nOutSizeInChars) {
    // If nInSize includes the null-terminator then a result of nOutSizeInChars
    // is legal. We check this by seeing if the last character in the output
    // buffer is a zero.
    ...
    // We have successfully converted our string. Now we need to null-terminate
    // it, because MultiByteToWideChar will only do that if nInSize includes the
...
void V_wcstostr(const wchar_t *pWString, int nInSize, char *pString,
                int nOutSizeInChars) {
#ifdef _WIN32
  int result = WideCharToMultiByte(CP_UTF8, 0, pWString, nInSize, pString,
                                   nOutSizeInChars, NULL, NULL);
  // If the string completely fails to fit then MultiByteToWideChar will return
  // 0. If the string exactly fits but with no room for a null-terminator then
  // MultiByteToWideChar will happily fill the buffer and omit the
  // null-terminator, returning nOutSizeInChars. Either way we need to return an
...
//-----------------------------------------------------------------------------
void Q_URLEncodeInternal(char *pchDest, int nDestLen, const char *pchSource,
                         int nSourceLen, bool bUsePlusForSpace) {
  if (nDestLen < 3 * nSourceLen) {
    pchDest[0] = '\0';
    AssertMsg(false, "Target buffer for Q_URLEncode needs to be 3 times larger "
                     "than source to guarantee enough space\n");
    return;
  }
...
//-----------------------------------------------------------------------------
size_t Q_URLDecodeInternal(char *pchDecodeDest, int nDecodeDestLen,
...
//-----------------------------------------------------------------------------
void Q_URLEncode(char *pchDest, int nDestLen, const char *pchSource,
                 int nSourceLen) {
...
//-----------------------------------------------------------------------------
size_t Q_URLDecode(char *pchDecodeDest, int nDecodeDestLen,
                   const char *pchEncodedSource, int nEncodedSourceLen) {
  return Q_URLDecodeInternal(pchDecodeDest, nDecodeDestLen, pchEncodedSource,
...
// Purpose: Encodes a string (or binary data) from URL encoding format, see
// rfc1738 section 2.2.
//          This version will not encode space as + (which HTML form encoding
//          uses despite not being part of the RFC)
//
//          Dest buffer should be at least as large as source buffer to
//          guarantee room for decode.
//-----------------------------------------------------------------------------
void Q_URLEncodeRaw(char *pchDest, int nDestLen, const char *pchSource,
                    int nSourceLen) {
...
// Purpose: Decodes a string (or binary data) from URL encoding format, see
// rfc1738 section 2.2.
//          This version will not recognize + as a space (which HTML form
//          encoding uses despite not being part of the RFC)
//
//          Dest buffer should be at least as large as source buffer to
//          guarantee room for decode.
//			Dest buffer being the same as the source buffer (decode
//in-place) is explicitly allowed.
//-----------------------------------------------------------------------------
size_t Q_URLDecodeRaw(char *pchDecodeDest, int nDecodeDestLen,
                      const char *pchEncodedSource, int nEncodedSourceLen) {
  return Q_URLDecodeInternal(pchDecodeDest, nDecodeDestLen, pchEncodedSource,
                             nEncodedSourceLen, false);
}

#if defined(LINUX) || defined(_PS3)
extern "C" void qsort_s(void *base, size_t num, size_t width,
...
bool BGetLocalFormattedDateAndTime(time_t timeVal, char *pchDate, int cubDate,
  ...
  if (timeVal) {
    ...
    struct tm tmStruct;
    struct tm tmToDisplay =
        *(Plat_localtime((const time_t *)&timeVal, &tmStruct));
#ifdef POSIX
    if (pchDate != NULL) {
      pchDate[0] = 0;
    ...
    if (pchTime != NULL) {
      ...
      // append am/pm in lower case (since strftime doesn't have a lowercase
      // formatting option)
    ...
    if (pchDate != NULL) {
      pchDate[0] = 0;
      if (!GetDateFormatW(LOCALE_USER_DEFAULT, DATE_LONGDATE, &st, NULL, rgwch,
                          MAX_PATH))
        return false;
    ...
    if (pchTime != NULL) {
      pchTime[0] = 0;
      if (!GetTimeFormatW(LOCALE_USER_DEFAULT, TIME_NOSECONDS, &st, NULL, rgwch,
                          MAX_PATH))
        return false;
...
// And a couple of helpers so people don't have to remember the order of the
...
void V_LogMultiline(bool input, char const *label, const char *data, size_t len,
                    CUtlString &output) {
...
//allows parsing 			strings where tokens between separators may or may not be
//present:
//
...
//-----------------------------------------------------------------------------
const char *nexttoken(char *token, size_t nMaxTokenLen, const char *str,
                      char sep) {
  if (nMaxTokenLen < 1) {
...
const static HtmlEntity_t g_BasicHTMLEntities[] = {
    {'"', "&quot;", 6}, {'\'', "&#039;", 6}, {'<', "&lt;", 4}, {'>', "&gt;", 4},
    {'&', "&amp;", 5},  {0, NULL, 0} // sentinel for end of array
};
...
bool V_BasicHtmlEntityEncode(char *pDest, const int nDestSize, char const *pIn,
                             const int nInSize,
...
bool V_HtmlEntityDecodeToUTF8(char *pDest, const int nDestSize, char const *pIn,
                              const int nInSize) {
  Assert(nDestSize == 0 || pDest != NULL);
  int iOutput = 0;
    ...
    if (pIn[iInput] == '&') {
      ...
      if (pchEnd) {
        if (iInput + 1 < nInSize && pIn[iInput + 1] == '#') {
          ...
          wrgchReplacement[0] =
              (uchar32)V_strtoi64(pIn + iInput + iOffset, NULL, iBase);
          if (!Q_UTF32ToUTF8(wrgchReplacement, rgchReplacement,
                             sizeof(rgchReplacement))) {
            rgchReplacement[0] = 0;
          }
        } else {
          // Lookup in map
          ...
          for (int i = 0; pFullEntities[i].uCharCode != 0; ++i) {
            if (nInSize - iInput - 1 >= pFullEntities[i].nEntityLength) {
              if (Q_memcmp(pIn + iInput, pFullEntities[i].pchEntity,
                           pFullEntities[i].nEntityLength) == 0) {
...
static const char *g_pszSimpleBBCodeReplacements[] = {
    ...
    "[/i]",    "</i>",   "[u]",     "<u>",         "[/u]",   "</u>",
    "[s]",     "<s>",    "[/s]",    "</s>",        "[code]", "<pre>",
    "[/code]", "</pre>", "[h1]",    "<h1>",        "[/h1]",  "</h1>",
    "[list]",  "<ul>",   "[/list]", "</ul>",       "[*]",    "<li>",
    "[/url]",  "</a>",   "[img]",   "<img src=\"", "[/img]", "\"></img>",
};

// Converts BBCode tags to HTML tags
bool V_BBCodeToHTML(OUT_Z_CAP(nDestSize) char *pDest, const int nDestSize,
                    char const *pIn, const int nInSize) {
  Assert(nDestSize == 0 || pDest != NULL);
  int iOutput = 0;

  for (int iInput = 0; iInput < nInSize && iOutput < nDestSize && pIn[iInput];
       ++iInput) {
    if (pIn[iInput] == '[') {
      ...
      for (int r = 0; r < ARRAYSIZE(g_pszSimpleBBCodeReplacements); r += 2) {
        ...
        if (!V_strnicmp(&pIn[iInput], g_pszSimpleBBCodeReplacements[r],
                        nBBCodeLength)) {
          int nHTMLReplacementLength =
              V_strlen(g_pszSimpleBBCodeReplacements[r + 1]);
          for (int c = 0; c < nHTMLReplacementLength && iOutput < nDestSize;
               c++) {
      ...
      if (!bFoundReplacement && !V_strnicmp(&pIn[iInput], "[url=", 5) &&
          nDestSize - iOutput > 9) {
        iInput += 5;
        pDest[iOutput++] = '<';
        pDest[iOutput++] = 'a';
...
//
// Some characters are difficult or unreliably rendered. These characters
// eventually fell out of the Unicode standard, but are abusable by users. For
// example, setting "RIGHT-TO-LEFT OVERRIDE" without popping or undoing the
// action causes the layout instruction to bleed into following characters in
// HTML renderings, or upset layout calculations in vgui panels.
//
// Many games don't cope with these characters well, and end up providing
// opportunities for griefing others. For example, a user might join a game with
// a malformed player name and it turns out that player name can't be selected
// or typed into the admin console or UI to mute, kick, or ban the disruptive
// player.
//
// Ideally, we'd perfectly support these end-to-end but we never realistically
// will. The benefit of doing so far outweighs the cost, anyway.
//-----------------------------------------------------------------------------
...
bool V_IsValidDomainNameCharacter(const char *pch, int *pAdvanceBytes) {
  ...
  // We allow unicode in Domain Names without the an encoding unless it
  // corresponds to a whitespace or control sequence or something we think is an
  // underscore looking thing. If this character is the start of a UTF-8
  // sequence, try decoding it.
  ...
  if ((ch & 0xC0) == 0xC0) {
    ...
    if (iAdvance) {
      // Ick. Want uchar32 versions of unicode character classification
      // functions. Really would like Q_IsWhitespace32 and Q_IsNonPrintable32,
      // but this is OK.
      if (rgch32Buf < 0x10000 && (V_IsMeanSpaceW((wchar_t)rgch32Buf) ||
                                  V_IsDeprecatedW((wchar_t)rgch32Buf) ||
...
bool V_IsValidURLCharacter(const char *pch, int *pAdvanceBytes) {
  ...
  // We allow unicode in URLs unless it corresponds to a whitespace or control
  // sequence. If this character is the start of a UTF-8 sequence, try decoding
  // it.
  unsigned char ch = (unsigned char)*pch;
  if ((ch & 0xC0) == 0xC0) {
    uchar32 rgch32Buf;
    ...
    // Spaces, control characters, quotes, and angle brackets are not legal URL
    // characters.
...
//-----------------------------------------------------------------------------
bool V_ExtractDomainFromURL(const char *pchURL, char *pchDomain,
                            int cchDomain) {
  pchDomain[0] = 0;
  ...
  while (pchDomain[iStrLen]) {
    if (!V_IsValidDomainNameCharacter(pchDomain + iStrLen, &iAdvance) ||
        (pchDomain[iStrLen] == '.' && cLast == '.')) {
      pchDomain[iStrLen] = 0;
      break;
    }
...
bool V_URLContainsDomain(const char *pchURL, const char *pchDomain) {
  ...
  if (V_ExtractDomainFromURL(pchURL, rgchExtractedDomain,
                             sizeof(rgchExtractedDomain))) {
    // see if the last part of the domain matches what we extracted
    int cchExtractedDomain = V_strlen(rgchExtractedDomain);
    if (pchDomain[0] == '.') {
      ++pchDomain; // If the domain has a leading '.', skip it. The test below
    ...
    if (cchDomain > cchExtractedDomain) {
      ...
      // If the actual domain is longer than what we're searching for, the
      // character previous to the domain we're searching for must be a period
      if (cchExtractedDomain > cchDomain &&
          rgchExtractedDomain[cchExtractedDomain - cchDomain - 1] != '.')
        return false;

      if (0 == V_stricmp(rgchExtractedDomain + cchExtractedDomain - cchDomain,
                         pchDomain))
        return true;
    }
  }
...
//			Does some additional formatting, like turning <li> into
//* when not preserving that tag,
//          and auto-closing unclosed tags if they aren't specified in
//          rgszNoCloseTags
//-----------------------------------------------------------------------------
void V_StripAndPreserveHTMLCore(CUtlBuffer *pbuffer, const char *pchHTML,
                                const char **rgszPreserveTags,
                                uint cPreserveTags,
                                const char **rgszNoCloseTags, uint cNoCloseTags,
                                uint cMaxResultSize) {
  uint cHTMLCur = 0;
  ...
  for (int iContents = 0;
       pchHTML[iContents] != '\0' && cHTMLCur < cMaxResultSize; iContents++) {
    char c = pchHTML[iContents];

    // If we are entering a comment, flag as such and skip past the begin
    // comment tag
    ...
    if (bInStrippedTag || bInPreservedTag) {
      ...
      // While inside a tag, ignore ending > properties if they are inside a
      // property value in "" or ''
      if (c == '"') {
        if (bInDoubleQuote)
      ...
      if (c == '<') {
        if (!rgszPreserveTags || cPreserveTags == 0) {
          ...
          if (Q_strnicmp("script", szTagStart, 6) == 0) {
            ...
            for (uint iTag = 0; iTag < cPreserveTags; iTag++) {
              ...
              // make sure characters match, and are followed by some non-alnum
              // char
              //   so "i" can match <i> or <i class=...>, but not <img>
              if (Q_strnicmp(szTag, szTagStart, cchTag) == 0 &&
                  !V_isalnum(szTagStart[cchTag])) {
                bPreserve = true;
                if (bEndTag) {
                  // ending a paragraph tag is optional. If we were expecting to
                  // find one, and didn't, skip
                  if (Q_stricmp(szTag, "p") != 0) {
                    while (vecTagStack.Count() > 0 &&
                           Q_stricmp(vecTagStack[vecTagStack.Count() - 1],
                                     "p") == 0) {
                      vecTagStack.Remove(vecTagStack.Count() - 1);
                    }
                  }

                  if (vecTagStack.Count() > 0 &&
                      vecTagStack[vecTagStack.Count() - 1] == szTag) {
                    vecTagStack.Remove(vecTagStack.Count() - 1);
                    ...
                    // don't preserve this unbalanced tag.  All open tags will
                    // be closed at the end of the blurb
                    bPreserve = false;
                  }
                } else {
          ...
          if (Q_strnicmp(szTagStart, "<li>", Q_strlen("<li>")) == 0) {
            ...
            if (bInListItemTag) {
              ...
              if (pbuffer->TellPut() > 0) {
                cLastChar =
                    (((char *)pbuffer->Base()) + pbuffer->TellPut() - 1)[0];
              }
            ...
            // we're decided to remove a tag, simulate a space in the original
            // text
            pbuffer->PutChar(' ');
            cHTMLCur++;
          }
        ...
        if (nPreTagDepth == 0 && V_isspace(c) &&
          ...
          // don't put anything for whitespace if the previous character was
          // whitespace
...
//* when not preserving that tag
//-----------------------------------------------------------------------------
void V_StripAndPreserveHTML(CUtlBuffer *pbuffer, const char *pchHTML,
                            const char **rgszPreserveTags, uint cPreserveTags,
                            uint cMaxResultSize) {
  const char *rgszNoCloseTags[] = {"br", "img"};
  V_StripAndPreserveHTMLCore(pbuffer, pchHTML, rgszPreserveTags, cPreserveTags,
                             rgszNoCloseTags, V_ARRAYSIZE(rgszNoCloseTags),
                             cMaxResultSize);
}
