...
HMODULE Sys_LoadLibrary(const char *pLibraryName, Sys_Flags flags) {
  ...
  ThreadHandle_t h =
      CreateSimpleThread((ThreadFunc_t)ThreadedLoadLibraryFunc, &context);
...
bool foundLibraryWithPrefix(char *pModuleAbsolutePath, size_t AbsolutePathSize,
                            const char *pPath, const char *pModuleName) {
  char str[1024];
  ...
  struct stat statBuf;
  Q_snprintf(pModuleAbsolutePath, AbsolutePathSize,
             "%s/" DEFAULT_LIB_PATH "lib%s", pPath, str);
  bFound |= stat(pModuleAbsolutePath, &statBuf) == 0;

  if (!bFound) {
    Q_snprintf(pModuleAbsolutePath, AbsolutePathSize,
               "%s/" DEFAULT_LIB_PATH "%s", pPath, str);
    bFound |= stat(pModuleAbsolutePath, &statBuf) == 0;
  }
...
CSysModule *Sys_LoadModule(const char *pModuleName,
  ...
  if (!Q_IsAbsolutePath(pModuleName)) {
    ...
    {
      bFound = foundLibraryWithPrefix(szAbsoluteModuleName,
                                      sizeof(szAbsoluteModuleName), modLibPath,
                                      pModuleName);
    ...
    if (!foundLibraryWithPrefix(szAbsoluteModuleName,
                                sizeof(szAbsoluteModuleName), libPath,
                                pModuleName)) {
...
#elif defined(POSIX)
    if (!foundLibraryWithPrefix(szAbsoluteModuleName,
                                sizeof(szAbsoluteModuleName), szCwd,
                                pModuleName)) {
      Warning("Can't find module - %s\n", pModuleName);
      return reinterpret_cast<CSysModule *>(hDLL);
    }
#else
    Q_snprintf(szAbsoluteModuleName, sizeof(szAbsoluteModuleName), "%s/bin/%s",
               szCwd, pModuleName);
#endif
