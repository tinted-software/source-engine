...
#ifdef OSX
// OS X is having fragmentation issues, and I suspect this 16meg buffer being
// recreated many times during load is hitting a bad case in the default
// allocator. So this is an experiment to see if it reduces crash rates there.
...
ConVar lzma_persistent_buffer(
    "lzma_persistent_buffer", LZMA_DEFAULT_PERSISTENT_BUFFER, FCVAR_NONE,
    "If set, attempt to keep a persistent buffer for the LZMA decoder "
    "dictionary. "
...
static void *SzAlloc(void *p, size_t size) {
  ...
  if (ThreadInMainThread()) {
    // If nobody is using the persistent buffer and size is above a threshold,
    // use it.
    bool bPersistentBuf =
        (g_pStaticLZMABuf || lzma_persistent_buffer.GetBool()) &&
        size >= (1024 * 1024 * 8) && g_unStaticLZMABufRef == 0;
    if (bPersistentBuf) {
      if (g_unStaticLZMABufSize < size) {
        g_pStaticLZMABuf =
            g_pStaticLZMABuf ? realloc(g_pStaticLZMABuf, size) : malloc(size);
...
static void SzFree(void *p, void *address) {
  // Don't touch static buffer on other threads.
  if (ThreadInMainThread()) {
    if (address != NULL && g_unStaticLZMABufRef &&
        address == g_pStaticLZMABuf) {
      g_unStaticLZMABufRef--;
      // If the convar was turned off, free the buffer
      if (g_pStaticLZMABuf && g_unStaticLZMABufRef == 0 &&
          !lzma_persistent_buffer.GetBool()) {
...
unsigned int CLZMA::Uncompress(unsigned char *pInput, unsigned char *pOutput) {
  ...
  ELzmaStatus status;
  SRes result = LzmaDecode((Byte *)pOutput, &outProcessed,
                           (Byte *)(pInput + sizeof(lzma_header_t)),
                           &inProcessed, (Byte *)pHeader->properties,
                           LZMA_PROPS_SIZE, LZMA_FINISH_END, &status, &g_Alloc);
...
CLZMAStream::~CLZMAStream() { FreeDecoderState(); }
...
bool CLZMAStream::CreateDecoderState(const unsigned char *pProperties) {
  ...
  if (LzmaDec_Allocate(pDecoderState, pProperties, LZMA_PROPS_SIZE, &g_Alloc) !=
      SZ_OK) {
...
bool CLZMAStream::Read(unsigned char *pInput, unsigned int nMaxInputBytes,
  ...
  // These are input ( available size ) *and* output ( size processed ) vars for
  // lzma
  SizeT expectedInputRemaining =
      m_nCompressedSize -
      Min(m_nCompressedBytesRead + nCompressedBytesRead, m_nCompressedSize);
  ...
  SRes result = LzmaDec_DecodeToBuf(m_pDecoderState, pOutput, &outSize, pInput,
                                    &inSize, finishMode, &status);
...
void CLZMAStream::InitZIPHeader(unsigned int nCompressedSize,
  ...
  // Signal to TryParseHeader to expect a zip-style header (which wont have the
  // size values)
  m_bZIPStyleHeader = true;
}

CLZMAStream::eHeaderParse
CLZMAStream::TryParseHeader(unsigned char *pInput, unsigned int nBytesAvailable,
  ...
  if (m_bParsedHeader) {
    AssertMsg(
        !m_bParsedHeader,
        "CLZMAStream::ReadSourceHeader called on already initialized stream");
  ...
  if (m_bZIPStyleHeader) {
    ...
    if (nLZMAPropertiesSize != LZMA_PROPS_SIZE) {
      Warning("LZMA stream: Unexpected LZMA properties size: %hu, expecting "
              "%u. Version mismatch?\n",
    ...
    m_nCompressedSize =
        LittleLong(((lzma_header_t *)pInput)->lzmaSize) + sizeof(lzma_header_t);
