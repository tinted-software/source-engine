...
//-----------------------------------------------------------------------------
inline const char *
CUtlSymbolTable::DecoratedStringFromIndex(const CStringPoolIndex &index) const {
...
bool CUtlSymbolTable::CLess::operator()(const CStringPoolIndex &i1,
  ...
  if (i2 == INVALID_STRING_INDEX) {
    ...
    AssertMsg2(storedHash == (!pTable->m_bInsensitive
                                  ? HashString(str2)
                                  : HashStringCaseless(str2)),
               "The stored hash (%d) for symbol '%s' is not correct.",
               storedHash, str2);
    hash2 = storedHash;
  }
...
//-----------------------------------------------------------------------------
FileNameHandle_t
CUtlFilenameSymbolTable::FindOrAddFileName(const char *pFileName) {
  if (!pFileName) {
    return NULL;
  }
...
bool CUtlFilenameSymbolTable::String(const FileNameHandle_t &handle, char *buf,
  ...
  FileNameHandleInternal_t *internalFileHandle =
      (FileNameHandleInternal_t *)&handle;
  ...
  m_lock.LockForRead();
  const char *path =
      m_PathStringPool.HandleToString(internalFileHandle->GetPath());
  const char *fn =
      m_FileStringPool.HandleToString(internalFileHandle->GetFile());
  m_lock.UnlockRead();
