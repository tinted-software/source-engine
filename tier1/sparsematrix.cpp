...
void CSparseMatrix::AdjustAllRowIndicesAfter(int nStartRow, int nDelta) {
  // now, we need to offset the starting position of all subsequent rows by -1
  // to compensate for the removal of this element
...
void CSparseMatrix::SetElement(int nRow, int nCol, float flValue) {
  ...
  if (nCount) {
    ...
    for (i = 0; i < nCount; i++) {
      ...
      if (nIdx == nCol) // we found it!
      {
        if (!bValueIsZero) {
          ...
          // there is a non-zero element currently at this position. We need to
          // remove it and we need to remove its storage.
          m_rowDescriptors[nRow].m_nNonZeroCount--;
          m_entries.Remove(nFirstEntryIndex + i);
          // now, we need to offset the starting position of all subsequent rows
          // by -1 to compensate for the removal of this element
    ...
    // we did not find an entry for this cell. If we were writing zero, fine -
    // we are done, otherwise insert
    if (!bValueIsZero) {
      m_rowDescriptors[nRow].m_nNonZeroCount++;
      ...
      // now, we need to offset the starting position of all subsequent rows by
      // -1 to compensate for the addition of this element
      AdjustAllRowIndicesAfter(nRow, +1);
    }
  } else {
