...
#ifdef _X360
// mandatory ... wary of above comment and isolating, tier0 is built as MT
// though
#include "tier0/memdbgon.h"
#endif
...
inline unsigned int BitForBitnum(int bitnum) { return GetBitForBitnum(bitnum); }
...
const char *bf_write::GetDebugName() { return m_pDebugName; }

void bf_write::SetDebugName(const char *pDebugName) {
  m_pDebugName = pDebugName;
}

void bf_write::SeekToBit(int bitPos) { m_iCurBit = bitPos; }
...
void bf_write::WriteSBitLong(int data, int numbits) {
  ...
  AssertMsg2(nValue == data, "WriteSBitLong: 0x%08x does not fit in %d bits",
             data, numbits);
...
void bf_write::WriteVarInt32(uint32 data) {
  ...
  if ((m_iCurBit & 7) == 0 &&
      (m_iCurBit + bitbuf::kMaxVarint32Bytes * 8) <= m_nDataBits) {
...
void bf_write::WriteVarInt64(uint64 data) {
  // Check if align and we have room, slow path if not
  if ((m_iCurBit & 7) == 0 &&
      (m_iCurBit + bitbuf::kMaxVarintBytes * 8) <= m_nDataBits) {
    uint8 *target = ((uint8 *)m_pData) + (m_iCurBit >> 3);
...
void bf_write::WriteBitCoordMP(const float f, bool bIntegral,
                               bool bLowPrecision) {
#if defined(BB_PROFILING)
  ...
  if (bIntegral) {
    // Integer encoding: in-bounds bit, nonzero bit, optional sign bit + integer
    // value bits
    if (intval) {
    ...
    // Float encoding: in-bounds bit, integer bit, sign bit, fraction value
    // bits, optional integer value bits
    if (intval) {
      // Adjust the integers from [1..MAX_COORD_VALUE] to [0..MAX_COORD_VALUE-1]
      --intval;
      bits = intval * 8 + signbit * 4 + 2 + bInBounds;
      ...
      numbits = 3 + (bLowPrecision ? COORD_FRACTIONAL_BITS_MP_LOWPRECISION
                                   : COORD_FRACTIONAL_BITS);
    }
  }
...
void bf_write::WriteLong(int32 val) { WriteSBitLong(val, sizeof(int32) << 3); }
...
void bf_read::SetDebugName(const char *pName) { m_pDebugName = pName; }
...
int bf_read::ReadBitsClamped_ptr(void *pOutData, size_t outSizeBytes,
                                 size_t nBits) {
...
float bf_read::ReadBitCoordMP(bool bIntegral, bool bLowPrecision) {
...
#endif
  // BitCoordMP float encoding: inbounds bit, integer bit, sign bit, optional
  // int bits, float bits BitCoordMP integer encoding: inbounds bit, integer
  // bit, optional sign bit, optional int bits. int bits are always encoded as
  // (value - 1) since zero is handled by the integer bit

  // With integer-only encoding, the presence of the third bit depends on the
  // second
  ...
  if (bIntegral) {
    if (flags & INTVAL) {
      ...
      unsigned int bits =
          ReadUBitLong((flags & INBOUNDS) ? COORD_INTEGER_BITS_MP + 1
  ...
  static const float mul_table[4] = {
      1.f / (1 << COORD_FRACTIONAL_BITS), -1.f / (1 << COORD_FRACTIONAL_BITS),
      1.f / (1 << COORD_FRACTIONAL_BITS_MP_LOWPRECISION),
      -1.f / (1 << COORD_FRACTIONAL_BITS_MP_LOWPRECISION)};
  // equivalent to: float multiply = mul_table[ ((flags & SIGN) ? 1 : 0) +
  // bLowPrecision*2 ];
  float multiply =
      *(float *)((uintptr_t)&mul_table[0] + (flags & 4) + bLowPrecision * 8);
  ...
  unsigned int bits = ReadUBitLong(
      numbits_table[(flags & (INBOUNDS | INTVAL)) + bLowPrecision * 4]);
...
unsigned int bf_read::ReadBitCoordBits(void) {
  ...
  static const int numbits_table[3] = {
      COORD_INTEGER_BITS + 1, COORD_FRACTIONAL_BITS + 1,
...
unsigned int bf_read::ReadBitCoordMPBits(bool bIntegral, bool bLowPrecision) {
  ...
  if (bIntegral) {
    if (flags & INTVAL) {
      numbits = (flags & INBOUNDS) ? (1 + COORD_INTEGER_BITS_MP)
                                   : (1 + COORD_INTEGER_BITS);
    } else {
...
bool bf_read::ReadString(char *pStr, int maxLen, bool bLine,
                         int *pOutNumChars) {
  Assert(maxLen != 0);
...
int bf_read::CompareBitsAt(int offset, bf_read *RESTRICT other, int otherOffset,
