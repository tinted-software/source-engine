...
bool CheckMMXTechnology(void) {
  ...
  if (retval) {
    if (RegEDX & 0x800000) // bit 23 is set for MMX technology
    {
      ...
      retval =
          false; // processor supports CPUID but does not support MMX technology
...
// BUGBUG, FIXME - Visual C Version 6.0 does not support SSE inline code YET (No
// macros from Intel either) Fix this if VC7 supports inline SSE instructinons
// like "xorps" as shown below.
#if 1
      __try {
        _asm
        {
          // Attempt execution of a SSE instruction to make sure OS supports SSE
          // FPU context switches
					xorps xmm0, xmm0
          // This will work on Win2k+ (Including masking SSE FPU exception to
          // "normalized" values) This will work on Win98+ (But no "masking" of
          // FPU exceptions provided)
        }
      } __except (EXCEPTION_EXECUTE_HANDLER)
#endif
  ...
  if (retval) {
    ...
    if (RegEDX & 0x04000000) // bit 26 is set for SSE2 technology
    {
      ...
      __try {
        _asm
        {
          // Attempt execution of a SSE2 instruction to make sure OS supports
          // SSE FPU context switches
					xorpd xmm0, xmm0
        }
      } __except (EXCEPTION_EXECUTE_HANDLER)
...
bool Check3DNowTechnology(void) {
  ...
  __try {
    ...
    {
#ifdef CPUID
//			xor edx, edx			// Clue the compiler
//that EDX is about to be used.
#endif
            mov eax, 0x80000000 // setup CPUID to return whether AMD >0x80000000 function are
                 // supported. 0x80000000 = Highest 0x80000000+ function,
                 // 0x80000001 = 3DNow support
            CPUID // code bytes = 0fh,  0a2h
