...
CReliableTimer::CReliableTimer() {
  ...
  if (0 == sm_nPerformanceFrequency) {
    ...
    sm_bUseQPC = ((0 == Q_stricmp(cpu.m_szProcessorID, "AuthenticAMD")) &&
                  (cpu.m_nPhysicalProcessors > 1) && !cpu.m_bSSE41);
...
#elif defined(_PS3)
  // On PowerPC, the time base register increment frequency is implementation
  // dependent, and doesn't have to be constant. On PS3, measured it to be just
  // shy of 80Mhz on the PPU and doesn't seem to change
...
int64 CReliableTimer::GetPerformanceCountNow() {
  // VPROF_BUDGET( "CReliableTimer::GetPerformanceCountNow",
