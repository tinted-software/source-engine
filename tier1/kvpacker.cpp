...
bool KVPacker::ReadAsBinary(KeyValues *pNode, CUtlBuffer &buffer) {
  ...
  while (true) {
    ...
    case PACKTYPE_WSTRING: {
      ...
      if (nLength >= 0 &&
          nLength * sizeof(uint16) <= (uint)buffer.GetBytesRemaining()) {
        if (nLength > 0) {
          wchar_t *pTemp = (wchar_t *)malloc(sizeof(wchar_t) * (1 + nLength));

          for (int k = 0; k < nLength; ++k) {
            pTemp[k] =
                buffer.GetShort(); // ugly, but preserving existing behavior
          }
    ...
    case PACKTYPE_COLOR: {
      Color color(buffer.GetUnsignedChar(), buffer.GetUnsignedChar(),
                  buffer.GetUnsignedChar(), buffer.GetUnsignedChar());
