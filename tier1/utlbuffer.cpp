...
CUtlBuffer::CUtlBuffer(const CUtlBuffer &copyFrom)
    : m_Get(copyFrom.m_Get), m_Put(copyFrom.m_Put), m_Error(copyFrom.m_Error),
      m_Flags(copyFrom.m_Flags), m_Reserved(copyFrom.m_Reserved)
#if defined(_GAMECONSOLE)
      ,
      pad(copyFrom.pad)
#endif
      ,
      m_nTab(copyFrom.m_nTab), m_nMaxPut(copyFrom.m_nMaxPut),
  ...
  if (copyFrom.m_Memory.Count() > 0) {
    ...
    memcpy(m_Memory.Base(), copyFrom.m_Memory.Base(),
           copyFrom.m_Memory.Count());
  }
}
...
CUtlBuffer::CUtlBuffer(CUtlBuffer &&moveFrom) // = default
    : m_Memory(Move(moveFrom.m_Memory)), m_Get(Move(moveFrom.m_Get)),
      m_Put(Move(moveFrom.m_Put)), m_Error(Move(moveFrom.m_Error)),
      m_Flags(Move(moveFrom.m_Flags)), m_Reserved(Move(moveFrom.m_Reserved))
#if defined(_GAMECONSOLE)
      ,
      pad(Move(moveFrom.pad))
#endif
      ,
      m_nTab(Move(moveFrom.m_nTab)), m_nMaxPut(Move(moveFrom.m_nMaxPut)),
  ...
  while (c = *pFmt++) {
    ...
    case '%': {
      ...
      case 's': {
        ...
        GetString(s, 64); // [SECURITY EXPLOIT: Scanf %s should be deprecated as
                          // malicious data can overrun stack buffers! Here we'd
                          // assume that at least 64 bytes are available on the
                          // stack, and even if not this shouldn't give
                          // attracker much room for code execution]
      } break;
...
void CUtlBuffer::VaPrintf(const char *pFmt, va_list list) {
  ...
  ErrorIfNot(
      nLen < sizeof(temp),
      ("CUtlBuffer::VaPrintf: String overflowed buffer [%zd]\n", sizeof(temp)));
  PutString(temp);
}
