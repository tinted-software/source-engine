...
void CCommandBuffer::DelayAllQueuedCommands(int nDelay) {
  ...
  for (intp i = m_Commands.Head(); i != m_Commands.InvalidIndex();
       i = m_Commands.Next(i)) {
...
void CCommandBuffer::EndProcessingCommands() {
  ...
  while (i != m_Commands.InvalidIndex()) {
    ...
    AssertMsgOnce(false, "CCommandBuffer::EndProcessingCommands() called "
                         "before all appropriate commands were dequeued.\n");
