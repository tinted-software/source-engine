...
#define ASYNC_FILEIO
#if defined(LINUX)
// Linux hasn't got a good AIO library that we have found yet, so lets punt for
// now
#undef ASYNC_FILEIO
#endif
...
#error                                                                         \
    "trying to use async io without win32 filesystem API usage, that isn't doable"
#endif
#endif
...
CPathString::CPathString(const char *pchUTF8Path) {
  // Need to first turn into an absolute path, so \\?\ pre-pended paths will be
  // ok
...
// on Win32, should only be used with unicode extended path aware filesystem
// calls)
//-----------------------------------------------------------------------------
...
void CPathString::PopulateWCharPath() {
  ...
  // Check if the UTF8 path starts with \\, which on Win32 means it's a UNC
  // path, and then needs a different prefix
  if (m_pchUTF8Path[0] == '\\' && m_pchUTF8Path[1] == '\\') {
    ...
    int cchResult =
#endif
        Q_UTF8ToUnicode(m_pchUTF8Path + 2, m_pwchWideCharPathPrepended + 8,
                        MAX_UNICODE_PATH * sizeof(wchar_t));
    Assert(cchResult);

    // Be sure we NULL terminate within our allocated region incase
    // Q_UTF8ToUnicode failed, though we're already in bad shape then.
    ...
    int cchResult =
#endif
        Q_UTF8ToUnicode(m_pchUTF8Path, m_pwchWideCharPathPrepended + 4,
                        MAX_UNICODE_PATH * sizeof(wchar_t));
    Assert(cchResult);
...
CDirWatcher::~CDirWatcher() {
  ...
  if (m_pOverlapped) {
    ...
    DirWatcherOverlapped *pDirWatcherOverlapped =
        (DirWatcherOverlapped *)m_pOverlapped;
...
//			gets called on the same thread whenever a SleepEx()
//occurs
//-----------------------------------------------------------------------------
class CDirWatcherFriend {
public:
  static void WINAPI DirWatchCallback(DWORD dwErrorCode,
                                      DWORD dwNumberOfBytesTransfered,
                                      OVERLAPPED *pOverlapped) {
    DirWatcherOverlapped *pDirWatcherOverlapped =
        (DirWatcherOverlapped *)pOverlapped;
    ...
    if (dwNumberOfBytesTransfered > sizeof(FILE_NOTIFY_INFORMATION)) {
      FILE_NOTIFY_INFORMATION *pFileNotifyInformation =
          (FILE_NOTIFY_INFORMATION *)
              pDirWatcherOverlapped->m_pDirWatcher->m_pFileInfo;
      do {
        // null terminate the string and turn it to UTF-8
        int cNumWChars =
            pFileNotifyInformation->FileNameLength / sizeof(wchar_t);
        wchar_t *pwchT = new wchar_t[cNumWChars + 1];
        memcpy(pwchT, pFileNotifyInformation->FileName,
               pFileNotifyInformation->FileNameLength);
        pwchT[cNumWChars] = 0;
        CStrAutoEncode strAutoEncode(pwchT);

        // add it to our list
        pDirWatcherOverlapped->m_pDirWatcher->AddFileToChangeList(
            strAutoEncode.ToString());
        delete[] pwchT;
        if (pFileNotifyInformation->NextEntryOffset == 0)
          break;

        // move to the next file
        pFileNotifyInformation =
            (FILE_NOTIFY_INFORMATION *)(((byte *)pFileNotifyInformation) +
                                        pFileNotifyInformation
                                            ->NextEntryOffset);
      } while (1);
    }
...
#elif defined(OSX)
void CheckDirectoryForChanges(const char *path_buff, CDirWatcher *pDirWatch,
                              bool bRecurse) {
  ...
  while ((dirent = readdir(dir)) != NULL) {
    ...
    if (S_ISDIR(st.st_mode) && bRecurse) {
      ...
      ts = st.st_mtimespec;
      bTimeSet = true;
      // the win32 size only sends up the dir relative to the watching dir, so
      // replicate that here
      pDirWatch->AddFileToChangeList(fullpath + pDirWatch->m_BaseDir.Length() +
                                     1);
    }
  }
...
static void fsevents_callback(ConstFSEventStreamRef streamRef,
  ...
  for (int i = 0; i < numEvents; i++) {
    ...
    if (!V_strnicmp(path_buff, pDirWatch->m_BaseDir.String(),
                    pDirWatch->m_BaseDir.Length()))
...
void CDirWatcher::SetDirToWatch(const char *pchDir) {
  ...
  // open the directory
  m_hFile = ::CreateFileW(
      strPath.GetWCharPathPrePended(), FILE_LIST_DIRECTORY,
      FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, NULL,
      OPEN_EXISTING, FILE_FLAG_OVERLAPPED | FILE_FLAG_BACKUP_SEMANTICS, NULL);
  ...
  PostDirWatch();
#elif defined(OSX)
  CFStringRef mypath = CFStringCreateWithCString(NULL, strPath.GetUTF8Path(),
  ...
  m_WatcherStream = (void *)FSEventStreamCreate(
      NULL, &fsevents_callback, &callbackInfo, pathsToWatch,
      kFSEventStreamEventIdSinceNow, latency, kFSEventStreamCreateFlagNoDefer);

  FSEventStreamScheduleWithRunLoop((FSEventStreamRef)m_WatcherStream,
                                   CFRunLoopGetCurrent(),
...
void CDirWatcher::PostDirWatch() {
  ...
  ::ReadDirectoryChangesW(
      m_hFile, m_pFileInfo, k_cubDirWatchBufferSize, TRUE,
...
bool CDirWatcher::GetChangedFile(CUtlString *psFile) {
  ...
  // this does get hit other places in the code; so the callback can happen at
  // any time
  ::SleepEx(0, TRUE);
#endif
...
void CDirWatcher::Validate(CValidator &validator, const char *pchName) {
  ...
  FOR_EACH_LL(m_listChangedFiles, i) { ValidateObj(m_listChangedFiles[i]); }
}
#endif
