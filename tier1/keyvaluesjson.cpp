...
KeyValues *KeyValuesJSONParser::ParseFile() {
  // A valid JSON object should contain a single object, surrounded by curly
  // braces.
  ...
  V_sprintf_safe(m_szErrMsg,
                 "%s not expected here.  A valid JSON document should be a "
                 "single object, which begins with '{' and ends with '}'",
                 GetTokenDebugText());
  return NULL;
}
...
bool KeyValuesJSONParser::ParseObject(KeyValues *pObject) {
  ...
  while (m_eToken != '}') {
    ...
    if (m_eToken == kToken_EOF) {
      ...
      // There's no need to report the line number of the end of file, that is
      // always useless.
      m_nLine = nOpenDelimLine;
      V_strcpy_safe(m_szErrMsg,
    ...
    if (m_eToken != kToken_String) {
      V_sprintf_safe(
          m_szErrMsg,
          "%s not expected here; expected string for key name or '}'",
          GetTokenDebugText());
      return false;
    }
    ...
    if (m_eToken != ':') {
      V_sprintf_safe(m_szErrMsg, "%s not expected here.  Missing ':'?",
                     GetTokenDebugText());
    ...
    if (m_eToken == ',') {
      ...
      V_sprintf_safe(m_szErrMsg, "%s not expected here.  Missing ',' or '}'?",
                     GetTokenDebugText());
      return false;
    }
  }
...
bool KeyValuesJSONParser::ParseArray(KeyValues *pArray) {
  ...
  while (m_eToken != ']') {
    ...
    if (m_eToken == ':') {
      V_sprintf_safe(m_szErrMsg, "':' not expected inside an array.  ('[]' "
                                 "used when '{}' was intended?)");
      return false;
    }
...
bool KeyValuesJSONParser::ParseValue(KeyValues *pValue) {
  ...
  case kToken_NumberInt: {
    ...
    if (*pszNum == '-') {
      ...
      if (val64 < INT32_MIN) {
        ...
        V_sprintf_safe(m_szErrMsg,
                       "%s is out of range for KeyValues, which doesn't "
                       "support signed 64-bit numbers",
                       pszNum);
        return false;
      }
  ...
  V_sprintf_safe(m_szErrMsg, "%s not expected here; missing value?",
                 GetTokenDebugText());
  return false;
}
...
void KeyValuesJSONParser::NextToken() {
  ...
  while (m_cur < m_end) {
    ...
    // String?
    case '\"':
    case '\'': // NOTE: We allow strings to be delimited by single quotes, which
    ...
    // Literal "true"
    case 't':
      if (m_cur + 4 <= m_end && m_cur[1] == 'r' && m_cur[2] == 'u' &&
          m_cur[3] == 'e') {
        m_cur += 4;
    ...
    // Literal "false"
    case 'f':
      if (m_cur + 5 <= m_end && m_cur[1] == 'a' && m_cur[2] == 'l' &&
          m_cur[3] == 's' && m_cur[4] == 'e') {
        m_cur += 5;
    ...
    // Literal "null"
    case 'n':
      if (m_cur + 4 <= m_end && m_cur[1] == 'u' && m_cur[2] == 'l' &&
          m_cur[3] == 'l') {
        m_cur += 4;
        m_eToken = kToken_Null;
        return;
    ...
    unexpected_char:
      if (V_isprint(c))
        V_sprintf_safe(m_szErrMsg, "Unexpected character 0x%02x ('%c')",
                       (uint8)c, c);
      else
...
void KeyValuesJSONParser::ParseNumberToken() {
  ...
  switch (c) {
  case '0':
    // Leading 0 cannot be followed by any more digits, as per JSON spec (and to
    // make sure nobody tries to parse octal).
    ++m_cur;
    bHasWholePart = true;
    break;
  ...
  case '.':
    // strict JSON doesn't allow a number that starts with a decimal point, but
    // we do
    break;
  }
  ...
  if (m_cur < m_end && *m_cur == '.') {
    ...
    if (!bHasWholePart && !bHasFractionPart) {
      ...
      V_sprintf_safe(m_szErrMsg, "Invalid number starting with '%s'",
                     m_vecTokenChars.Base());
      m_eToken = kToken_Err;
      return;
    }
  }
  ...
  // EOF?  That's OK for now, at this lexical parsing level.  We'll handle the
  // error at the higher parse level, when expecting a comma or closing
  // delimiter
  if (m_cur >= m_end)
    return;
  ...
  // Handle these guys as "tokens", to provide a slightly more meaningful error
  // message
  if (c == '[' || c == '{')
    return;
  ...
  if (V_isprint(c))
    V_sprintf_safe(m_szErrMsg,
                   "Number contains invalid character 0x%02x ('%c')", (uint8)c,
                   c);
  else
    V_sprintf_safe(m_szErrMsg, "Number contains invalid character 0x%02x",
                   (uint8)c);
  m_eToken = kToken_Err;
}
...
void KeyValuesJSONParser::ParseStringToken() {
  ...
  while (m_cur < m_end) {
    char c = *(m_cur++);
    if (c == '\r' || c == '\n') {
      V_sprintf_safe(m_szErrMsg, "Hit end of line before closing quote (%c)",
                     c);
      m_eToken = kToken_Err;
      return;
    }
    ...
    case 'u': {
      ...
      if (m_cur + 4 > m_end || !V_isxdigit(m_cur[0]) || !V_isxdigit(m_cur[1]) ||
          !V_isxdigit(m_cur[2]) || !V_isxdigit(m_cur[3])) {
        V_sprintf_safe(m_szErrMsg,
    ...
    default:
      if (V_isprint(c))
        V_sprintf_safe(m_szErrMsg, "Invalid escape character 0x%02x ('\\%c')",
                       (uint8)c, c);
      else
  ...
  V_sprintf_safe(m_szErrMsg, "Hit end of input before closing quote (%c)",
                 cDelim);
  m_eToken = kToken_Err;
}
...
static void JSONTest_ParseInvalid(const char *pszData,
                                  const char *pszExpectedErrMsgSnippet,
                                  int nExpectedFailureLine) {
  KeyValuesJSONParser parser(pszData);
  KeyValues *pFile = parser.ParseFile();
  Assert(pFile == NULL);
