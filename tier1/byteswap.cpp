...
void CByteswap::SwapFieldToTargetEndian(void *pOutputBuffer, void *pData,
  ...
  case FIELD_FLOAT:
    SwapBufferToTargetEndian<uint>((uint *)pOutputBuffer, (uint *)pData,
                                   pField->fieldSize);
    break;
  ...
  case FIELD_INTEGER64:
    SwapBufferToTargetEndian<uint64>((uint64 *)pOutputBuffer, (uint64 *)pData,
                                     pField->fieldSize);
    break;
  ...
  case FIELD_EMBEDDED: {
    ...
    for (int i = 0; i < pField->fieldSize; ++i) {
      SwapFieldsToTargetEndian(
          (byte *)pOutputBuffer + pEmbed->fieldOffset[TD_OFFSET_NORMAL],
...
void CByteswap::SwapFieldsToTargetEndian(void *pOutputBuffer, void *pBaseData,
  ...
  for (int i = 0; i < fieldCount; ++i) {
    ...
    SwapFieldToTargetEndian(
        (BYTE *)pOutputBuffer + pField->fieldOffset[TD_OFFSET_NORMAL],
