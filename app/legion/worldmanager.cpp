...
// ConVars
//-----------------------------------------------------------------------------
static ConVar cam_forwardspeed("cam_forwardspeed", "100", FCVAR_CHEAT,
                               "Sets the camera forward speed");
static ConVar cam_backwardspeed("cam_backwardspeed", "100", FCVAR_CHEAT,
                                "Sets the camera backward speed");

//-----------------------------------------------------------------------------
// Constructor, destructor
//-----------------------------------------------------------------------------
CWorldManager::CWorldManager() { m_pHeightField = NULL; }

CWorldManager::~CWorldManager() { Assert(m_pHeightField == NULL); }
...
//-----------------------------------------------------------------------------
CPlayerEntity *CWorldManager::GetLocalPlayer() { return &m_PlayerEntity; }
...
void CWorldManager::SetInitialLocalPlayerPosition() {
  ...
  VectorMA(Vector(512, 512, 0), -flDistance, vecCameraDirection,
           m_PlayerEntity.m_pCameraProperty->m_Origin);
...
// Draws the UI
//-----------------------------------------------------------------------------
void CWorldManager::DrawWorld() { m_pHeightField->Draw(); }
...
void CWorldManager::ForwardStart(const CCommand &args) {
  ...
  VectorMA(pCamera->m_Velocity, cam_forwardspeed.GetFloat(), vecForward,
           pCamera->m_Velocity);
}
...
void CWorldManager::BackwardStart(const CCommand &args) {
  ...
  VectorMA(pCamera->m_Velocity, -cam_backwardspeed.GetFloat(), vecForward,
           pCamera->m_Velocity);
}
