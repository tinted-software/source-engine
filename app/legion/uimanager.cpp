...
CLegionConsoleDialog::CLegionConsoleDialog(vgui::Panel *pParent,
                                           const char *pName)
    : BaseClass(pParent, pName) {
  AddActionSignalTarget(this);
}

CLegionConsoleDialog::~CLegionConsoleDialog() { g_pUIManager->HideConsole(); }
...
// Deals with close
//-----------------------------------------------------------------------------
void CLegionConsoleDialog::OnClose() { g_pUIManager->HideConsole(); }
...
bool CUIManager::Init() {
  COMPILE_TIME_ASSERT(sizeof(s_pRootPanelNames) / sizeof(const char *) ==
                      UI_ROOT_PANEL_COUNT);
  ...
  for (int i = 0; i < UI_ROOT_PANEL_COUNT; ++i) {
    ...
    m_pRootPanels[i]->SetAutoResize(vgui::Panel::PIN_TOPLEFT,
                                    vgui::Panel::AUTORESIZE_DOWNANDRIGHT, 0, 0,
                                    0, 0);
  }
...
void CUIManager::Shutdown() { g_pVGui->Stop(); }
...
void CUIManager::ToggleConsole(const CCommand &args) {
  if (!m_hConsole.Get()) {
    m_hConsole =
        new CLegionConsoleDialog(m_pRootPanels[UI_ROOT_TOOLS], "Console");
    ...
    m_hConsole->SetBounds(swide / 2 - (offset * 4), offset,
                          (swide / 2) + (offset * 3), stall - (offset * 8));
...
//-----------------------------------------------------------------------------
// Attempt to process an input event, return true if it sholdn't be chained to
