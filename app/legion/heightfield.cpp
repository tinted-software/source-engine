...
// NOTE: x,y are not normalized and are expected to go in the range of (0->w-1,
// 0->h-1)
//-----------------------------------------------------------------------------
float BilerpBitmap(Bitmap_t &bitmap, float x, float y) {
  ...
  // do the bilerp
  return (1 - fx) * (1 - fy) * v00 + fx * (1 - fy) * v10 + (1 - fx) * fy * v01 +
         fx * fy * v11;
}
...
float CHeightField::GetHeight(float x, float y) {
  ...
  float h11 = (gx < (m_nWidth - 1) && gy < (m_nHeight - 1))
                  ? HEIGHT(gx + 1, gy + 1)
                  : 0.0f;
