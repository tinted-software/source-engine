...
void CMenuManager::PushMenu(const char *pMenuName) {
  AssertMsg(!m_pPushRequested,
            "Can't request to push two menus in a single frame!");
...
void CMenuManager::PopMenu() {
  AssertMsg(!m_bPopRequested,
            "Can't request to pop two menus in a single frame!");
  AssertMsg(!m_pPushRequested, "Can't request to pop after requesting to push "
                               "a menu in a single frame!");
...
void CMenuManager::Update() {
  ...
  if (m_bPopRequested) {
    AssertMsg(m_nActiveMenu.Count(),
              "Tried to pop a menu when no menus are active");
  ...
  if (m_pPushRequested) {
    ...
    vgui::Panel *pMenu =
        m_pPushRequested->CreateMenu(g_pUIManager->GetRootPanel(UI_ROOT_MENU));
