...
//-----------------------------------------------------------------------------
LevelRetVal_t CRenderManager::LevelInit(bool bFirstCall) { return FINISHED; }
...
//-----------------------------------------------------------------------------
// Done completely client-side, want total smoothness, so simulate at render
// interval
//-----------------------------------------------------------------------------
void CRenderManager::UpdateLocalPlayerCamera() {
  float dt = IGameManager::DeltaTime();
  CCameraProperty *pCamera =
      g_pWorldManager->GetLocalPlayer()->m_pCameraProperty;
...
void CRenderManager::SetupProjectionMatrix(int nWidth, int nHeight,
                                           float flFOV) {
  VMatrix proj;
  float flZNear = ZNEAR;
  float flZFar = ZFAR;
...
void CRenderManager::RenderWorld() {
  ...
  pRenderContext->Viewport(m_nRenderX, m_nRenderY, m_nRenderWidth,
                           m_nRenderHeight);
