...
LevelRetVal_t IGameManager::InvokeLevelMethod(GameManagerLevelFunc_t f,
                                              bool bFirstCall) {
...
LevelRetVal_t
IGameManager::InvokeLevelMethodReverseOrder(GameManagerLevelFunc_t f,
                                            bool bFirstCall) {
  LevelRetVal_t nRetVal = FINISHED;
  int i;
  int nCount = m_GameManagers.Count();
...
void IGameManager::UpdateLevelStateMachine() {
  ...
  if (m_LevelState == SHUTTING_DOWN_LEVEL) {
    LevelRetVal_t val = InvokeLevelMethodReverseOrder(
        &IGameManager::LevelShutdown, bFirstLevelShutdownFrame);
    if (val != MORE_WORK) {
  ...
  if (m_LevelState == LOADING_LEVEL) {
    LevelRetVal_t val =
        InvokeLevelMethod(&IGameManager::LevelInit, bFirstLevelStartFrame);
    if (val == FAILED) {
      m_LevelState = NOT_IN_LEVEL;
...
void IGameManager::Start() {
  ...
  while (!m_bStopRequested) {
    ...
    int nSimulationFramesNeeded =
        1 + (int)((m_flCurrentTime - flStartTime) / TICK_INTERVAL);
