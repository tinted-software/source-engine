...
//-----------------------------------------------------------------------------
INetworkMessageFactory *CNetworkManager::RegisterNetworkMessage(
    INetworkMessageFactory *pNetworkMessageFactory) {
...
bool CNetworkManager::Init() {
  ...
  for (pFactory = m_pFirstFactory; pFactory;
       pFactory = pFactory->GetNextFactory()) {
...
bool CNetworkManager::StartServer(unsigned short nServerListenPort) {
  ...
  if (nServerListenPort == LEGION_DEFAULT_SERVER_PORT) {
    nServerListenPort = CommandLine()->ParmValue(
        "-serverport", NETWORKSYSTEM_DEFAULT_SERVER_PORT);
  }
...
bool CNetworkManager::StartClient(unsigned short nClientListenPort) {
  ...
  if (nClientListenPort == LEGION_DEFAULT_CLIENT_PORT) {
    nClientListenPort = CommandLine()->ParmValue(
        "-clientport", NETWORKSYSTEM_DEFAULT_CLIENT_PORT);
  }
...
//-----------------------------------------------------------------------------
bool CNetworkManager::ConnectClientToServer(const char *pServerName,
  ...
  m_pClientToServerConnection =
      g_pNetworkSystem->ConnectClientToServer(pServerName, nServerListenPort);
...
bool CNetworkManager::IsClientConnected() {
  return m_bIsClient && (m_pClientToServerConnection != NULL) &&
         (m_pClientToServerConnection->GetConnectionState() ==
...
//-----------------------------------------------------------------------------
void CNetworkManager::BroadcastServerToClientMessage(
    INetworkMessage *pMessage) {
  if (IsServer()) {
...
//-----------------------------------------------------------------------------
void CNetworkManager::AddListener(NetworkMessageRoute_t route, int nGroup,
                                  int nType,
                                  INetworkMessageListener *pListener) {
  CUtlVector<CUtlVector<CUtlVector<INetworkMessageListener *>>> &listeners =
      m_Listeners[route];
...
void CNetworkManager::RemoveListener(NetworkMessageRoute_t route, int nGroup,
                                     int nType,
                                     INetworkMessageListener *pListener) {
  CUtlVector<CUtlVector<CUtlVector<INetworkMessageListener *>>> &listeners =
      m_Listeners[route];
...
//-----------------------------------------------------------------------------
void CNetworkManager::NotifyListeners(NetworkMessageRoute_t route,
                                      INetworkMessage *pMessage) {
  CUtlVector<CUtlVector<CUtlVector<INetworkMessageListener *>>> &listeners =
      m_Listeners[route];
...
case NETWORK_EVENT_CONNECTED:
      {
              NetworkConnectionEvent_t* pConnectEvent =
static_cast<NetworkConnectionEvent_t*>( pEvent ); m_pClientToServerConnection =
pConnectEvent->m_pChannel;
      }
      break;

case NETWORK_EVENT_DISCONNECTED:
      {
              NetworkDisconnectionEvent_t* pDisconnectEvent =
static_cast<NetworkDisconnectionEvent_t*>( pEvent ); Assert(
m_pClientToServerConnection == pDisconnectEvent->m_pChannel ); if (
m_pClientToServerConnection == pDisconnectEvent->m_pChannel )
              {
                      m_pClientToServerConnection = NULL;
              }
      }
...
*/
    ...
    case NETWORK_EVENT_MESSAGE_RECEIVED: {
      NetworkMessageReceivedEvent_t *pPacketEvent =
          static_cast<NetworkMessageReceivedEvent_t *>(pEvent);
      NotifyListeners(NETWORK_MESSAGE_SERVER_TO_CLIENT,
                      pPacketEvent->m_pNetworkMessage);
    } break;
    }
  }
}
...
void CNetworkManager::ProcessServerMessages() {
  ...
  for (; pEvent; pEvent = g_pNetworkSystem->NextNetworkEvent()) {
    ...
    case NETWORK_EVENT_CONNECTED: {
      NetworkConnectionEvent_t *pConnectEvent =
          static_cast<NetworkConnectionEvent_t *>(pEvent);
      m_ServerToClientConnection.AddToTail(pConnectEvent->m_pChannel);
    } break;

    case NETWORK_EVENT_DISCONNECTED: {
      NetworkDisconnectionEvent_t *pDisconnectEvent =
          static_cast<NetworkDisconnectionEvent_t *>(pEvent);
    ...
    case NETWORK_EVENT_MESSAGE_RECEIVED: {
      NetworkMessageReceivedEvent_t *pPacketEvent =
          static_cast<NetworkMessageReceivedEvent_t *>(pEvent);
      NotifyListeners(NETWORK_MESSAGE_CLIENT_TO_SERVER,
                      pPacketEvent->m_pNetworkMessage);
    } break;
    }
  }
