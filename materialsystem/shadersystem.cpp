...
public:
  ...
  virtual ShaderAPITextureHandle_t
  GetShaderAPITextureBindHandle(ITexture *pTexture, int nFrameVar,
...
const char *CShaderSystem::s_pDebugShaderName[MATERIAL_DEBUG_COUNT] = {
    "FillRate",   "DebugNormalMap", "DebugDrawEnvmapMask",
    "DebugDepth", "DebugDepth",     "Wireframe_DX9"};
...
void CShaderSystem::LoadModShaderDLLs(int dxSupportLevel) {
  ...
  // Don't do this for Valve mods. They don't need them, and attempting to load
  // them is an opportunity for cheaters to get their code into the process
  const char *pGameDir = COM_GetModDirectory();
  if (!Q_stricmp(pGameDir, "hl2") || !Q_stricmp(pGameDir, "cstrike") ||
      !Q_stricmp(pGameDir, "cstrike_beta") || !Q_stricmp(pGameDir, "hl2mp") ||
      !Q_stricmp(pGameDir, "lostcoast") || !Q_stricmp(pGameDir, "episodic") ||
      !Q_stricmp(pGameDir, "portal") || !Q_stricmp(pGameDir, "ep2") ||
      !Q_stricmp(pGameDir, "dod") || !Q_stricmp(pGameDir, "tf") ||
      !Q_stricmp(pGameDir, "tf_beta") || !Q_stricmp(pGameDir, "hl1")) {
    return;
  }
...
void CShaderSystem::PrepForShaderDraw(IShader *pShader, IMaterialVar **ppParams,
  ...
  // 360 sidesteps the other negative affect that *all* buffered spew redirects
  // as warning text
  if (IsPC() || !IsX360()) {
...
void CShaderSystem::DrawMeasureFillRate(
  ...
  Assert((VertexFlags(pRenderState->m_VertexFormat) &
...
CShaderSystem::GetShaderAPITextureBindHandle(ITexture *pTexture, int nFrame,
                                             int nTextureChannel) {
  Assert(
  ...
  // Bind away baby
  if (pTexture) {
    // This is ugly. Basically, this is yet another way that textures can be
    // bound. They don't get bound here, but the return is only used to bind
    // them for semistatic command buffer building, which doesn't go through
    // CTexture::Bind for whatever reason. So let's request the mipmaps here. If
    // you run into this, in a situation where we shouldn't be doing the
    // request, we could relocate this code to the appropriate callsites
    // instead.
...
void CShaderSystem::LoadTexture(IMaterialVar *pTextureVar,
  ...
  if (MaterialSystem()->CanUseEditorMaterials() &&
    ...
    pTexture = static_cast<ITextureInternal *>(MaterialSystem()->FindTexture(
        pName, pTextureGroupName, false, nAdditionalCreationFlags));
  }
...
void CShaderSystem::LoadBumpMap(IMaterialVar *pTextureVar,
  ...
  ITexture *pTexture;
  pTexture = MaterialSystem()->FindTexture(pTextureVar->GetStringValue(),
                                           pTextureGroupName, false, 0);
...
void CShaderSystem::LoadCubeMap(IMaterialVar **ppParams,
  ...
  if (stricmp(pTextureVar->GetStringValue(), "env_cubemap") == 0) {
    ...
    // DummyTexture special that says to use the cubemap entity.
    ...
    pTexture = MaterialSystem()->FindTexture(
        textureName, TEXTURE_GROUP_CUBE_MAP, false, nAdditionalCreationFlags);
