...
void CheckMaterial(KeyValues *pKeyValues, const char *pRoot,
  ...
  /*
          if ( Q_stristr( pShaderName, "Water" ) ||
                  ...
                  Q_stristr( pShaderName,
     "DecalBaseTimesLightmapAlphaBlendSelfIllum" ) || Q_stristr( pShaderName,
     "Sprite" ) )
          {
                  return;
          }
          ...
          const char *pBaseTextureName = pKeyValues->GetString( "$basetexture",
     NULL ); if ( pBaseTextureName != NULL )
          {
                  if ( DoesTextureUseAlpha( pBaseTextureName, pFileName ) )
                  {
                          float flAlpha = MaterialFloatKeyValue( pKeyValues,
     "$alpha", 1.0f ); bool bHasVertexAlpha = DoesMaterialHaveKey( pKeyValues,
     "$vertexalpha" );	// Modulation always happens here whether we want it to
     or not bool bHasAlphaTest = DoesMaterialHaveKey( pKeyValues, "$alphatest"
                          ...
                          bool bHasBaseAlphaEnvMapMask = DoesMaterialHaveKey(
     pKeyValues, "$basealphaenvmapmask" ); if ( (flAlpha == 1.0f) &&
     !bHasVertexAlpha && !bHasAlphaTest && !bHasTranslucent && !bHasSelfIllum &&
     !bHasBaseAlphaEnvMapMask )
                          {
                                  Warning("Material \"%s\": BASETEXTURE
  ...
  */
  ...
  // Check for bump, spec, and no normalmapalphaenvmapmask
          ...
          if ( pBumpmapName != NULL )
          {
                  ...
                  {
                          bool bHasEnvmap = DoesMaterialHaveKey( pKeyValues,
  "$envmap" ); bool bHasNormalMapAlphaEnvMapMask = DoesMaterialHaveKey(
  pKeyValues, "$normalmapalphaenvmapmask" ); if ( !bHasEnvmap ||
  ...
  DoesMaterialHaveKey( pKeyValues, "$bumpmap" ) )
          {
                  ...
                  if ( MaterialVectorKeyValue( pKeyValues, "$envmaptint", 3,
  defaultVal, &nDim, retVal ) )
                  {
