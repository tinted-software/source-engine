...
void ColorCorrectionLookup_t::RestoreTexture() {
  ...
  m_pColorCorrectionTexture
      ->OnRestore(); // Give render targets a chance to reinitialize themselves
                     // if necessary (due to AA changes).
  m_pColorCorrectionTexture->Download();
}
...
//-----------------------------------------------------------------------------
ColorCorrectionLookup_t *
CColorCorrectionSystem::FindLookup(ColorCorrectionHandle_t handle) {
  for (int i = 0; i < m_ColorCorrectionList.Count(); i++) {
...
CColorCorrectionSystem::GetLookupHandle(const char *pName) {
...
void CColorCorrectionSystem::GetNormalizedWeights(float *pDefaultWeight,
  ...
  int nLoopCount =
      min(m_ColorCorrectionList.Count(), (int)COLOR_CORRECTION_MAX_TEXTURES);
