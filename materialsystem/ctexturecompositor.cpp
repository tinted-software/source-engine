...
ConVar r_texcomp_dump(
    "r_texcomp_dump", "0", FCVAR_NONE,
    "Whether we should dump the textures to disk or not. 1: Save all; 2: Save "
    "Final; 3: Save Final with name suitable for scripting; 4: Save Final and "
    "skip saving workshop icons.");
#endif

const int cMaxSelectors = 16;

// Ugh, this is annoying and matches TF's enums. That's lame. We should
// workaround this.
...
void ComputeTextureMatrixFromRectangle(VMatrix *pOutMat, const Vector2D &bl,
                                       const Vector2D &tl, const Vector2D &tr);
bool HasCycle(CTextureCompositorTemplate *pStartTempl);
CTextureCompositorTemplate *Advance(CTextureCompositorTemplate *pTmpl,
                                    int nSteps);
...
struct CTCStageResult_t {
  ...
  inline CTCStageResult_t()
      : m_pTexture(NULL), m_pRenderTarget(NULL), m_fAdjustBlackPoint(0.0f),
        m_fAdjustWhitePoint(1.0f), m_fAdjustGamma(1.0f) {
...
public:
  ...
  virtual void OnAsyncCreateComplete(ITexture *pTex,
  ...
  virtual void OnAsyncMapComplete(ITexture *pTex, void *pExtraArgs,
                                  void *pMemory, int pPitch) OVERRIDE {}
  ...
  void ComputeRandomValues(int *pCurIndex, CUniformRandomStream *pRNGs,
                           int nRNGCount);
  ...
  void Render(ITexture *_destRT, IMaterial *_mat,
              const CUtlVector<CTCStageResult_t> &_inputs,
...
protected:
  ...
  // for the compositor to request its textures. Textures should not be
  // requested before this or they can be held waaaay too long.
  virtual void RequestTextures() = 0;

  // This function will be called during Resolve traversal. At the point when
  // this is called, all of this node's children will have had their resolve
  // completed. Our siblings will not have resolved yet.
...
// ------------------------------------------------------------------------------------------------
template <int N> void ParseIntVectorFromKV(KeyValues *_kv, void *_pDest) {
...
// ------------------------------------------------------------------------------------------------
template <class T> CUtlString AsStringT(const T &_val) {
#ifdef _WIN32
...
// ------------------------------------------------------------------------------------------------
template <> CUtlString AsStringT<int>(const int &_val) {
  char buffer[12];
...
// ------------------------------------------------------------------------------------------------
template <class T> void ParseTFromKV(KeyValues *_kv, void *_pDest) {
#ifdef _WIN32
  // Not sure why linux is unhappy here. Error messages unhelpful. Thanks, GCC.
  static_assert(false, "Must add specialization for typename T");
#endif
...
// ------------------------------------------------------------------------------------------------
template <> void ParseTFromKV<int>(KeyValues *_kv, void *_pDest) {
  int *realDest = (int *)_pDest;
  (*realDest) = _kv->GetInt();
}

// ------------------------------------------------------------------------------------------------
template <> void ParseTFromKV<Vector2D>(KeyValues *_kv, void *_pDest) {
...
void ParseRangeFromKV(KeyValues *_kv, void *_pDest) {
  ...
  case 0:
  default:
    Error("Incorrect number of numbers while parsing, using defaults. This "
...
void ParseInverseRangeFromKV(KeyValues *_kv, void *_pDest) {
  ...
  if (realDest->low != 0.0f) {
    ...
    Error("Specified 0.0 for low value, that is illegal in this field. "
          "Substituting %.5f\n",
          kSubstValue);
  ...
  if (realDest->high != 0.0f) {
    ...
    Error("Specified 0.0 for high value, that is illegal in this field. "
          "Substituting %.5f\n",
          kSubstValue);
...
// ------------------------------------------------------------------------------------------------
template <int Div> void ParseRangeThenDivideBy(KeyValues *_kv, void *_pDest) {
...
struct TextureStageParameters {
  ...
  TextureStageParameters()
      : m_AdjustBlack(0, 0), m_AdjustOffset(1, 1), m_AdjustGamma(1, 1),
        m_Rotation(0, 0), m_TranslateU(0, 0), m_TranslateV(0, 0),
        m_ScaleUV(1, 1), m_AllowFlipU(false), m_AllowFlipV(false),
        m_Evaluate(true) {}
};

// ------------------------------------------------------------------------------------------------
const ParseTableEntry cTextureStageParametersParseTable[] = {
    {"texture", ParseStringFromKV,
     offsetof(TextureStageParameters, m_pTexFilename)},
    {"texture_red", ParseStringFromKV,
     offsetof(TextureStageParameters, m_pTexRedFilename)},
    {"texture_blue", ParseStringFromKV,
     offsetof(TextureStageParameters, m_pTexBlueFilename)},
    {"adjust_black", ParseRangeThenDivideBy<255>,
     offsetof(TextureStageParameters, m_AdjustBlack)},
    {"adjust_offset", ParseRangeThenDivideBy<255>,
     offsetof(TextureStageParameters, m_AdjustOffset)},
    {"adjust_gamma", ParseInverseRangeFromKV,
     offsetof(TextureStageParameters, m_AdjustGamma)},
    {"rotation", ParseRangeFromKV,
     offsetof(TextureStageParameters, m_Rotation)},
    {"translate_u", ParseRangeFromKV,
     offsetof(TextureStageParameters, m_TranslateU)},
    {"translate_v", ParseRangeFromKV,
     offsetof(TextureStageParameters, m_TranslateV)},
    ...
    {"evaluate?", ParseBoolFromKV,
     offsetof(TextureStageParameters, m_Evaluate)},
...
public:
  CTCTextureStage(const TextureStageParameters &_tsp,
...
protected:
  ...
  virtual void RequestTextures() {
    if (!m_Parameters.m_pTexFilename.IsEmpty())
      materials->AsyncFindTexture(
          m_Parameters.m_pTexFilename.Get(), TEXTURE_GROUP_RUNTIME_COMPOSITE,
          this, (void *)Neutral, false, TEXTUREFLAGS_IMMEDIATE_CLEANUP);
    if (!m_Parameters.m_pTexRedFilename.IsEmpty())
      materials->AsyncFindTexture(
          m_Parameters.m_pTexRedFilename.Get(), TEXTURE_GROUP_RUNTIME_COMPOSITE,
          this, (void *)Red, false, TEXTUREFLAGS_IMMEDIATE_CLEANUP);
    if (!m_Parameters.m_pTexBlueFilename.IsEmpty())
      materials->AsyncFindTexture(m_Parameters.m_pTexBlueFilename.Get(),
                                  TEXTURE_GROUP_RUNTIME_COMPOSITE, this,
                                  (void *)Blue, false,
  ...
  virtual void ResolveThis(CTextureCompositor *_comp) {
    ...
    // When the texture has finished loading, this will be set to the texture we
    // should use.
    ...
    if (!m_pTex && !m_pTexRed && !m_pTexBlue) {
      _comp->Error(false,
                   "Invalid texture_lookup node, must specify at least texture "
                   "(or texture_red and texture_blue) or all of them.\n");
      return;
    }

    if (m_pTex && m_pTex->IsError()) {
      _comp->Error(false,
                   "Failed to load texture '%s', this is non-recoverable.\n",
    ...
    if (m_pTexRed && m_pTexRed->IsError()) {
      _comp->Error(
          false, "Failed to load texture_red '%s', this is non-recoverable.\n",
    ...
    if (m_pTexBlue && m_pTexBlue->IsError()) {
      _comp->Error(
          false, "Failed to load texture_blue '%s', this is non-recoverable.\n",
  ...
  virtual bool ComputeRandomValuesThis(CUniformRandomStream *pRNG) OVERRIDE {
    // If you change the order of these random numbers being generated, or add
    // new ones, you will change the look of existing players' weapons! Don't do
    // that.
    const bool shouldFlipU =
        m_Parameters.m_AllowFlipU ? pRNG->RandomInt(0, 1) != 0 : false;
    const bool shouldFlipV =
        m_Parameters.m_AllowFlipV ? pRNG->RandomInt(0, 1) != 0 : false;
    const float translateU = pRNG->RandomFloat(m_Parameters.m_TranslateU.low,
                                               m_Parameters.m_TranslateU.high);
    const float translateV = pRNG->RandomFloat(m_Parameters.m_TranslateV.low,
                                               m_Parameters.m_TranslateV.high);
    const float rotation = pRNG->RandomFloat(m_Parameters.m_Rotation.low,
                                             m_Parameters.m_Rotation.high);
    const float scaleUV = pRNG->RandomFloat(m_Parameters.m_ScaleUV.low,
                                            m_Parameters.m_ScaleUV.high);

    const float adjustBlack = pRNG->RandomFloat(
        m_Parameters.m_AdjustBlack.low, m_Parameters.m_AdjustBlack.high);
    const float adjustOffset = pRNG->RandomFloat(
        m_Parameters.m_AdjustOffset.low, m_Parameters.m_AdjustOffset.high);
    const float adjustGamma = pRNG->RandomFloat(
        m_Parameters.m_AdjustGamma.low, m_Parameters.m_AdjustGamma.high);
    ...
    m_mTextureAdjust =
        m_mTextureAdjust.Scale(Vector(finalScaleU, finalScaleV, 1.0f));
...
const char *cCombineMaterialName[] = {
    ...
    "\0 ECO_Legacy_Lerp_FirstPass",  // Procedural; starting with \0 will skip
                                     // precaching
    "\0 ECO_Legacy_Lerp_SecondPass", // Procedural; starting with \0 will skip
                                     // precaching
...
struct CombineStageParameters {
  ...
  CombineStageParameters()
      : m_CombineOp(ECO_Error), m_AdjustBlack(0, 0), m_AdjustOffset(1, 1),
        m_AdjustGamma(1, 1), m_Rotation(0, 0), m_TranslateU(0, 0),
        m_TranslateV(0, 0), m_ScaleUV(1, 1), m_AllowFlipU(false),
        m_AllowFlipV(false), m_Evaluate(true) {}
};
...
const ParseTableEntry cCombineStageParametersParseTable[] = {
    {"adjust_black", ParseRangeThenDivideBy<255>,
     offsetof(CombineStageParameters, m_AdjustBlack)},
    {"adjust_offset", ParseRangeThenDivideBy<255>,
     offsetof(CombineStageParameters, m_AdjustOffset)},
    {"adjust_gamma", ParseInverseRangeFromKV,
     offsetof(CombineStageParameters, m_AdjustGamma)},
    {"rotation", ParseRangeFromKV,
     offsetof(CombineStageParameters, m_Rotation)},
    {"translate_u", ParseRangeFromKV,
     offsetof(CombineStageParameters, m_TranslateU)},
    {"translate_v", ParseRangeFromKV,
     offsetof(CombineStageParameters, m_TranslateV)},
    ...
    {"evaluate?", ParseBoolFromKV,
     offsetof(CombineStageParameters, m_Evaluate)},
...
public:
  CTCCombineStage(const CombineStageParameters &_csp,
                  uint32 nTexCompositeCreateFlags)
      : m_Parameters(_csp), m_pMaterial(NULL) {
    Assert(m_Parameters.m_CombineOp >= 0 &&
           m_Parameters.m_CombineOp < ECO_COUNT);

    SafeAssign(&m_pMaterial, materials->FindMaterial(
                                 cCombineMaterialName[m_Parameters.m_CombineOp],
...
protected:
  ...
  virtual void ResolveThis(CTextureCompositor *_comp) {
    ...
    for (CTCStage *child = GetFirstChild(); child;
    ...
    // TODO: If there are more than 8 children, need to split them into multiple
    // groups here. Skip it for now.
    ...
    // As soon as we have scheduled the read of a child render target, we can
    // release that texture back to the pool for use by another stage.
    // Everything is pipelined, so this just works.
    CleanupChildResults(_comp);
    tmMessage(TELEMETRY_LEVEL0, TMMF_ICON_NOTE, "Completed: %s", __FUNCTION__);
  }
...
const ParseTableEntry cSelectStageParametersParseTable[] = {
    {"groups", ParseStringFromKV,
     offsetof(SelectStageParameters, m_pTexFilename)},
    {"select", ParseVectorFromKV<int, cMaxSelectors>,
     offsetof(SelectStageParameters, m_Select)},
...
public:
  CTCSelectStage(const SelectStageParameters &_ssp,
                 uint32 nTexCompositeCreateFlags)
      : m_Parameters(_ssp), m_pMaterial(NULL), m_pTex(NULL) {
    SafeAssign(&m_pMaterial,
               materials->FindMaterial(cCombineMaterialName[ECO_Select],
                                       TEXTURE_GROUP_RUNTIME_COMPOSITE));
  }
...
protected:
  virtual void RequestTextures() {
    materials->AsyncFindTexture(m_Parameters.m_pTexFilename.Get(),
                                TEXTURE_GROUP_RUNTIME_COMPOSITE, this, NULL,
...
struct Sticker_t {
  ...
  CUtlString
      m_baseFilename; // Name of the base file for the sticker (the albedo).
  CUtlString m_specFilename; // Name of the specular file for the sticker, or if
                             // blank we will assume it is baseFilename + _spec
                             // + baseExtension
...
// ------------------------------------------------------------------------------------------------
template <> void ParseTFromKV<Sticker_t>(KeyValues *_kv, void *_pDest) {
  ...
  if (pSpec)
    ...
    CUtlString specPath = tmpDest.m_baseFilename.StripExtension() + "_s" +
...
// ------------------------------------------------------------------------------------------------
template <> CUtlString AsStringT<Sticker_t>(const Sticker_t &_val) {
  char buffer[80];
  V_sprintf_safe(buffer, "[ weight %.2f; base \"%s\"; spec \"%s\" ]",
                 _val.m_fWeight, _val.m_baseFilename.Get(),
                 _val.m_specFilename.Get());
  return CUtlString(buffer);
}

// ------------------------------------------------------------------------------------------------
template <class T> struct Settable_t {
  T m_val;
  bool m_bSet;
...
// ------------------------------------------------------------------------------------------------
template <class T> void ParseSettable(KeyValues *_kv, void *_pDest) {
...
const ParseTableEntry cApplyStickerStageParametersParseTable[] = {
    {"sticker", ParseVectorFromKV<Sticker_t>,
     offsetof(ApplyStickerStageParameters, m_possibleStickers)},
    {"dest_bl", ParseSettable<Vector2D>,
     offsetof(ApplyStickerStageParameters, m_vDestBL)},
    {"dest_tl", ParseSettable<Vector2D>,
     offsetof(ApplyStickerStageParameters, m_vDestTL)},
    {"dest_tr", ParseSettable<Vector2D>,
     offsetof(ApplyStickerStageParameters, m_vDestTR)},
    {"adjust_black", ParseRangeThenDivideBy<255>,
     offsetof(ApplyStickerStageParameters, m_AdjustBlack)},
    {"adjust_offset", ParseRangeThenDivideBy<255>,
     offsetof(ApplyStickerStageParameters, m_AdjustOffset)},
    {"adjust_gamma", ParseInverseRangeFromKV,
     offsetof(ApplyStickerStageParameters, m_AdjustGamma)},
    {"evaluate?", ParseBoolFromKV,
     offsetof(ApplyStickerStageParameters, m_Evaluate)},
...
public:
  CTCApplyStickerStage(const ApplyStickerStageParameters &_assp,
                       uint32 nTexCompositeCreateFlags)
      : m_Parameters(_assp), m_pMaterial(NULL), m_pTex(NULL),
        m_pTexSpecular(NULL), m_nChoice(0) {
    SafeAssign(&m_pMaterial,
               materials->FindMaterial(cCombineMaterialName[ECO_Blend],
                                       TEXTURE_GROUP_RUNTIME_COMPOSITE));
  }
...
protected:
  ...
  virtual void RequestTextures() {
    if (!m_Parameters.m_possibleStickers[m_nChoice].m_baseFilename.IsEmpty())
      materials->AsyncFindTexture(
          m_Parameters.m_possibleStickers[m_nChoice].m_baseFilename.Get(),
          TEXTURE_GROUP_RUNTIME_COMPOSITE, this, (void *)Albedo, false,
          TEXTUREFLAGS_IMMEDIATE_CLEANUP);

    if (!m_Parameters.m_possibleStickers[m_nChoice].m_specFilename.IsEmpty())
      materials->AsyncFindTexture(
          m_Parameters.m_possibleStickers[m_nChoice].m_specFilename.Get(),
  ...
  virtual void ResolveThis(CTextureCompositor *_comp) {
    ...
    // We expect exactly one or zero children. If we have a child, use its
    // render target to render to, otherwise Get one and use that.
    ITexture *pRenderTarget = _comp->AllocateCompositorRenderTarget();
    ...
    if (pChild)
      ...
      fakeRes.m_pTexture =
          materials->FindTexture("black", TEXTURE_GROUP_RUNTIME_COMPOSITE);
    }
  ...
  virtual bool ComputeRandomValuesThis(CUniformRandomStream *pRNG) OVERRIDE {
    ...
    ComputeTextureMatrixFromRectangle(
        &m_mTextureAdjust, m_Parameters.m_vDestBL.m_val,
        m_Parameters.m_vDestTL.m_val, m_Parameters.m_vDestTR.m_val);
    return true;
  }
  ...
  virtual void OnAsyncFindComplete(ITexture *pTex, void *pExtraArgs) {
    ...
    case Albedo:
      ...
      break;
    case Specular:
      // It's okay if this is the case, we just need to substitute with the
      // black texture.
      if (pTex->IsError()) {
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
// This is a procedural stage we use to copy the results of a composite into a
// texture so we can release the render targets back to a pool to be used later.
  ...
  virtual void ResolveThis(CTextureCompositor *_comp) {
    ...
    if (m_pTex) {
      if (m_pTex->IsError()) {
        _comp->Error(
            false,
            "Error occurred copying render target to texture. This is fatal.");
        return;
      }
      ...
      if (r_texcomp_dump.GetInt() == 2) {
        char buffer[128];
        V_snprintf(buffer, ARRAYSIZE(buffer), "composite_%s_result_%02d.tga",
                   _comp->GetName().Get(), s_nDumpCount++);
    ...
    bool bGenMipmaps =
        !(_comp->GetCreateFlags() & TEX_COMPOSITE_CREATE_FLAGS_NO_MIPMAPS);

    // We want to do this once only.
    char buffer[_MAX_PATH];
    _comp->GetTextureName(buffer, ARRAYSIZE(buffer));

    int nCreateFlags = TEXTUREFLAGS_IMMEDIATE_CLEANUP | TEXTUREFLAGS_TRILINEAR |
                       TEXTUREFLAGS_ANISOTROPIC;

#if defined(STAGING_ONLY)
#if WITH_TEX_COMPOSITE_CACHE
    if (r_texcomp_dump.GetInt() == 0 &&
        (_comp->GetCreateFlags() & TEX_COMPOSITE_CREATE_FLAGS_FORCE) == 0)
      nCreateFlags = 0;
#endif
#endif

    CMatRenderContextPtr pRenderContext(materials);
    pRenderContext->AsyncCreateTextureFromRenderTarget(
        GetFirstChild()->GetResult().m_pRenderTarget, buffer, fmt, bGenMipmaps,
        nCreateFlags, this, NULL);

    SetResolveStatus(ECRS_PendingComposites);
    // Don't clean up here just yet, we'll get cleaned up when the composite is
    // totally complete.
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
CTextureCompositor::CTextureCompositor(int _width, int _height, int nTeam,
                                       const char *pCompositeName,
                                       uint64 nRandomSeed,
                                       uint32 nTexCompositeCreateFlags)
    : m_nReferenceCount(0), m_nWidth(_width), m_nHeight(_height),
      m_nTeam(nTeam), m_nRandomSeed(nRandomSeed), m_pRootStage(NULL),
      m_ResolveStatus(ECRS_Idle), m_bError(false), m_bFatal(false),
      m_nRenderTargetsAllocated(0), m_CompositeName(pCompositeName),
      m_nTexCompositeCreateFlags(nTexCompositeCreateFlags),
      m_bHasTeamSpecifics(false), m_nCompositePaintKitId(0) {}
...
CTextureCompositor::~CTextureCompositor() {
  ...
  // Have to clean up the stages before cleaning up the render target pool,
  // because cleanup up stages will throw things back to the render target pool.
...
void CTextureCompositor::Restart() {
  Assert(!"TODO! Need to clone the root node, then cleanup the old root and "
          "start the new work.");
...
void CTextureCompositor::Shutdown() {
  // If this thing is a template, then it's a faker and doesn't have an
  // m_pRootStage. This is only true during startup when we're just verifying
  // that the templates look sane--later they should have real data.
  if (m_pRootStage)
...
void CTextureCompositor::Update() {
  ...
  if (m_pRootStage->GetResolveStatus() == ECRS_Complete) {
#ifdef STAGING_ONLY
    // One time, go ahead and dump out the texture if we're supposed to right
    // here, at completion time.
...
void CTextureCompositor::ScheduleResolve() {
  ...
  if ((GetCreateFlags() & TEX_COMPOSITE_CREATE_FLAGS_FORCE) == 0) {
    ...
    // I think there's a race condition here, add a flag to FindTexture that
    // says only if loaded, and bumps ref?
    if (materials->IsTextureLoaded(buffer)) {
      ITexture *resTexture = materials->FindTexture(
          buffer, TEXTURE_GROUP_RUNTIME_COMPOSITE, false, 0);
...
void CTextureCompositor::Resolve() {
  ...
  // We can actually get in multiply times for the same one because of the way
  // EconItemView works. So if that's the case, bail.
...
void CTextureCompositor::SetRootStage(CTCStage *rootStage) {
  ...
  // After we set a root, compute everyone's RNG values. Do this once, early, to
  // ensure the values are stable.
  uint32 seedhi = 0;
  uint32 seedlo = 0;
...
ITexture *CTextureCompositor::AllocateCompositorRenderTarget() {
  ...
  // Lie to the material system that we are asking for this allocation way back
  // at the beginning of time. This used to matter to GPUs for perf, but hasn't
  // in a long time.
  materials->OverrideRenderTargetAllocation(true);
  ITexture *retVal = materials->CreateNamedRenderTargetTextureEx(
      "", m_nWidth, m_nHeight, RT_SIZE_LITERAL_PICMIP, IMAGE_FORMAT_RGBA8888,
      MATERIAL_RT_DEPTH_NONE, TEXTUREFLAGS_IMMEDIATE_CLEANUP);
  Assert(retVal);
  materials->OverrideRenderTargetAllocation(false);

  // Used to count how many we actually allocated so we can verify we cleaned
  // them all up at shutdown
...
// ------------------------------------------------------------------------------------------------
void CTextureCompositor::GetTextureName(char *pOutBuffer,
                                        int nBufferLen) const {
  uint32 seedhi = 0;
  uint32 seedlo = 0;
  GetSeed(&seedhi, &seedlo);
  ...
  if (m_pRootStage->HasTeamSpecifics())
    V_snprintf(pOutBuffer, nBufferLen,
               "proc/texcomp/%s_flags%08x_seedhi%08x_seedlo%08x_team%d_w%d_h%d",
               GetName().Get(), GetCreateFlags(), seedhi, seedlo, m_nTeam,
               m_nWidth, m_nHeight);
  else
    V_snprintf(pOutBuffer, nBufferLen,
               "proc/texcomp/%s_flags%08x_seedhi%08x_seedlo%08x_w%d_h%d",
               GetName().Get(), GetCreateFlags(), seedhi, seedlo, m_nWidth,
               m_nHeight);
}
...
void CTextureCompositor::GetSeed(uint32 *pOutHi, uint32 *pOutLo) const {
  ...
  for (int i = 0; i < 32; ++i) {
    ...
    (*pOutLo) |=
        (uint32)((m_nRandomSeed & (uint64(1) << ((2 * i) + 1))) >> (i + 1));
  }
}
...
CTCStage::CTCStage()
    : m_nReferenceCount(
          1) // This is 1 because the common case is to assign these as
             // children, and we don't want to play with refs there.
      ,
...
void CTCStage::Resolve(bool bFirstTime, CTextureCompositor *_comp) {
  ...
  // Update our status, which may be updated below. Only do this the first time
  // through.
  if (bFirstTime) {
    m_ResolveStatus = ECRS_Scheduled;
    // Request textures here. We used to request in the constructor, but it
    // caused us to potentially hold all paintkitted textures for all time.
    // That's bad for Mac, where we are super memory constrained.
    RequestTextures();
  }
...
// ------------------------------------------------------------------------------------------------
void CTCStage::ComputeRandomValues(int *pCurIndex, CUniformRandomStream *pRNGs,
                                   int nRNGCount) {
...
void CTCStage::CleanupChildResults(CTextureCompositor *_comp) {
  // This does not recurse. We call it as we move through the tree to clean up
  // our first-generation children.
  for (CTCStage *child = GetFirstChild(); child;
       child = child->GetNextSibling()) {
...
// ------------------------------------------------------------------------------------------------
void CTCStage::Render(ITexture *_destRT, IMaterial *_mat,
                      const CUtlVector<CTCStageResult_t> &_inputs,
                      CTextureCompositor *_comp, bool bClear) {
  tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s", __FUNCTION__);
  ...
  FOR_EACH_VEC(_inputs, i) {
    ...
    ITexture *inTex = stageParams.m_pTexture ? stageParams.m_pTexture
    ...
    Assert(bFound);
    var->SetVecValue(stageParams.m_fAdjustBlackPoint,
                     stageParams.m_fAdjustWhitePoint,
                     stageParams.m_fAdjustGamma);
  ...
  if (r_texcomp_dump.GetInt() == 1) {
    FOR_EACH_VEC(_inputs, i) {
      if (_inputs[i].m_pTexture) {
        V_snprintf(buffer, ARRAYSIZE(buffer),
                   "composite_%s_input_%02d_in%01d_%08x.tga",
                   _comp->GetName().Get(), s_nDumpCount, i, (int)this);
        _inputs[i].m_pTexture->SaveToFile(buffer);
      }
    }

    V_snprintf(buffer, ARRAYSIZE(buffer), "composite_%s_result_%02d_%08x.tga",
               _comp->GetName().Get(), s_nDumpCount++, (int)this);
  ...
  // After rendering, clean up the leftover texture references or they will be
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
typedef bool (*TBuildNodeFromKVFunc)(CTCStage **ppOutStage, const char *_key,
                                     KeyValues *_kv,
                                     uint32 nTexCompositeCreateFlags);
bool TexStageFromKV(CTCStage **ppOutStage, const char *_key, KeyValues *_kv,
                    uint32 nTexCompositeCreateFlags);
template <int Type>
bool CombineStageFromKV(CTCStage **ppOutStage, const char *_key, KeyValues *_kv,
                        uint32 nTexCompositeCreateFlags);
bool SelectStageFromKV(CTCStage **ppOutStage, const char *_key, KeyValues *_kv,
                       uint32 nTexCompositeCreateFlags);
bool ApplyStickerStageFromKV(CTCStage **ppOutStage, const char *_key,
                             KeyValues *_kv, uint32 nTexCompositeCreateFlags);
...
// ------------------------------------------------------------------------------------------------
template <typename S>
void ParseIntoStruct(S *_outStruct, CUtlVector<KeyValues *> *_leftovers,
                     KeyValues *_kv, uint32 nTexCompositeCreateFlags,
                     const ParseTableEntry *_entries) {
  Assert(_leftovers);
  ...
  FOR_EACH_SUBKEY(_kv, thisKey) {
    ...
    for (int e = 0; _entries[e].keyName; ++e) {
      if (V_stricmp(_entries[e].keyName, thisKey->GetName()) == 0) {
        // If we're instancing, go ahead and run the parse function. If we're
        // just doing template verification then the right hand side may still
        // have variables that need to be expanded, so just verify that the left
        // hand side is sane.
        if ((nTexCompositeCreateFlags &
             TEX_COMPOSITE_CREATE_FLAGS_VERIFY_TEMPLATE_ONLY) == 0) {
          void *pDest =
              ((unsigned char *)_outStruct) + _entries[e].structOffset;
...
// ------------------------------------------------------------------------------------------------
bool ParseNodes(CUtlVector<CTCStage *> *_outStages,
                const CUtlVector<KeyValues *> &_kvs,
                uint32 nTexCompositeCreateFlags) {
  tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s", __FUNCTION__);
  ...
  FOR_EACH_VEC(_kvs, thisKey) {
    ...
    for (int e = 0; cNodeParseTable[e].keyName; ++e) {
      if (V_stricmp(cNodeParseTable[e].keyName, thisKV->GetName()) == 0) {
        ...
        if (!cNodeParseTable[e].buildFunc(&pNewStage, thisKV->GetName(), thisKV,
                                          nTexCompositeCreateFlags))
          anyFails = true;
    ...
    if (!parsed) {
      DevWarning(
          "Compositor Error: Unexpected key '%s' while parsing definition.\n",
          thisKV->GetName());
      anyFails = true;
    }
  }
...
bool TexStageFromKV(CTCStage **ppOutStage, const char *_key, KeyValues *_kv,
  ...
  ParseIntoStruct(&tsp, &leftovers, _kv, nTexCompositeCreateFlags,
                  cTextureStageParametersParseTable);
...
bool CombineStageFromKV(CTCStage **ppOutStage, const char *_key, KeyValues *_kv,
  ...
  static_assert(Type >= 0 && Type < ECO_Error,
                "Invalid type, you need to update the enum.");
  CombineStageParameters csp;
  csp.m_CombineOp = (ECombineOperation)Type;

  CUtlVector<KeyValues *> leftovers;
  CUtlVector<CTCStage *> childNodes;
  ParseIntoStruct(&csp, &leftovers, _kv, nTexCompositeCreateFlags,
                  cCombineStageParametersParseTable);
  if (!ParseNodes(&childNodes, leftovers, nTexCompositeCreateFlags))
    return false;
...
bool SelectStageFromKV(CTCStage **ppOutStage, const char *_key, KeyValues *_kv,
  ...
  CUtlVector<KeyValues *> leftovers;
  CUtlVector<CTCStage *> childNodes;
  ParseIntoStruct(&ssp, &leftovers, _kv, nTexCompositeCreateFlags,
                  cSelectStageParametersParseTable);
  if (!ParseNodes(&childNodes, leftovers, nTexCompositeCreateFlags))
    return false;
...
bool ApplyStickerStageFromKV(CTCStage **ppOutStage, const char *_key,
  ...
  CUtlVector<KeyValues *> leftovers;
  CUtlVector<CTCStage *> childNodes;
  ParseIntoStruct(&assp, &leftovers, _kv, nTexCompositeCreateFlags,
                  cApplyStickerStageParametersParseTable);
  if (!ParseNodes(&childNodes, leftovers, nTexCompositeCreateFlags))
    return false;
...
// ------------------------------------------------------------------------------------------------
KeyValues *ResolveTemplate(const char *pRootName, KeyValues *pValues,
                           uint32 nTexCompositeCreateFlags,
  ...
  FOR_EACH_SUBKEY(pValues, pChild) {
    ...
    if (V_stricmp(pChildName, "implements") == 0) {
      if (bImplementsTemplate) {
        Warning("ERROR[%s]: implements field can only appear once, seen a "
                "second time as 'implements \"%s\"\n",
  ...
  if (bImplementsTemplate && bHasOtherNodes) {
    Warning("ERROR[%s]: if using 'implements', can only have variable "
            "definitions--other fields not allowed.\n",
            pRootName);
    return NULL;
  }
  ...
  if ((nTexCompositeCreateFlags &
       TEX_COMPOSITE_CREATE_FLAGS_VERIFY_TEMPLATE_ONLY) == 0) {
    CTextureCompositorTemplate *pTmpl =
        TextureManager()->FindTextureCompositorTemplate(pTemplateName);
    if (!pTmpl) {
    ...
    // If the verify flag isn't set, we're instancing the template so do all the
    // logic.
    if (pTmpl->ImplementsTemplate()) {
      pNewKV = ResolveTemplate(pRootName, pTmpl->GetKV(),
                               nTexCompositeCreateFlags, pInOutAllocdNew);
    } else {
    ...
    // Just return the original KV back to the caller, who just wants a success
    // code here.
    return pValues;
  }

  // Now, copy any child var definitions from pValues into pNewKV. Because of
  // the recursive call stack, this has the net effect that more concrete
  // templates will write their values later than more remote templates.
  FOR_EACH_SUBKEY(pValues, pChild) {
    const char *pChildName = pChild->GetName();
...
KeyValues *ExtractVariableDefinitions(VariableDefs_t *pOutVarDefs,
  ...
  FOR_EACH_SUBKEY(pKeyValues, pChild) {
    ...
    if (pChildName[0] == '$') {
      if (pChild->GetFirstTrueSubKey()) {
        Warning("ERROR[%s]: All variable definitions must be simple strings, "
                "'%s' was a full subtree.\n",
...
// ------------------------------------------------------------------------------------------------
// Returns the number of characters written into pOutBuffer or -1 if there was
// an error.
int SubstituteVarsRecursive(char *pOutBuffer, int *pOutSubsts,
                            CUtlVector<const char *> &errorStack,
                            const char *pStr, uint32 nTexCompositeCreateFlags,
                            const VariableDefs_t &varDefs) {
  ParseMode mode = Copy;
  ...
  while (pStr[srcPos] != 0) {
    ...
    case DetermineStringForReplace:
      if (srcC[0] == ']') {
        ...
        } else if ((nTexCompositeCreateFlags &
                    TEX_COMPOSITE_CREATE_FLAGS_VERIFY_TEMPLATE_ONLY) != 0) {
          pSubstText = ""; // It's fine to run into these when verifying the
                           // template only.
        } else {
          Warning("ERROR[%s]: Couldn't find variable named $%s that was "
        ...
        int charsWritten = SubstituteVarsRecursive(
            pOutBuffer ? &pDst[dstPos] : NULL, pOutSubsts, errorStack,
            pSubstText, nTexCompositeCreateFlags, varDefs);
        if (charsWritten < 0)
          return -1;
...
bool SubstituteVars(CUtlString *pOutStr, int *pOutSubsts,
                    CUtlVector<const char *> &errorStack, const char *pStr,
                    uint32 nTexCompositeCreateFlags,
                    const VariableDefs_t &varDefs) {
  ...
  // Even though this involves a traversal, we're saving a malloc by walking
  // this thing once looking for the start token.
  ...
  // We could do this as we go, but we're trying to avoid re-mallocing memory
  // repeatedly in here so process once to find out what the size is.
  int expectedLen = SubstituteVarsRecursive(NULL, pOutSubsts, errorStack, pStr,
                                            nTexCompositeCreateFlags, varDefs);
  if (expectedLen < 0)
    return false;

  // We don't need to actually write the string, and we shouldn't. If we're just
  // verifying, exit now with success.
  if ((nTexCompositeCreateFlags &
       TEX_COMPOSITE_CREATE_FLAGS_VERIFY_TEMPLATE_ONLY) != 0)
    return true;
  ...
  int finalLen =
      SubstituteVarsRecursive(outStr.GetForModify(), pOutSubsts, errorStack,
                              pStr, nTexCompositeCreateFlags, varDefs);
...
// ------------------------------------------------------------------------------------------------
bool ResolveAllVariablesRecursive(CUtlVector<const char *> &errorStack,
                                  const VariableDefs_t &varDefs,
                                  KeyValues *pKeyValues,
                                  uint32 nTexCompositeCreateFlags,
                                  CUtlString &tmpStr) {
  ...
  FOR_EACH_SUBKEY(pKeyValues, pChild) {
    ...
    if (pChild->GetFirstSubKey()) {
      if (!ResolveAllVariablesRecursive(errorStack, varDefs, pChild,
                                        nTexCompositeCreateFlags, tmpStr))
        success = false;
    } else {
      int nSubsts = 0;
      if (!SubstituteVars(&tmpStr, &nSubsts, errorStack, pChild->GetString(),
                          nTexCompositeCreateFlags, varDefs))
        success = false;
...
// ------------------------------------------------------------------------------------------------
KeyValues *ResolveAllVariables(const char *pRootName,
                               const VariableDefs_t &varDefs,
                               KeyValues *pKeyValues,
                               uint32 nTexCompositeCreateFlags,
                               bool *pInOutAllocdNew) {
  ...
  // Let's just assume first that if we have any vars, we will need to
  // substitute them. But if we're just verifying the template, no need.
  ...
  if (!ResolveAllVariablesRecursive(errorStack, varDefs, pKeyValues,
                                    nTexCompositeCreateFlags, str))
    return NULL;
...
// ------------------------------------------------------------------------------------------------
// Perform all template expansion and variable substitution here. What should be
// output should look like v1.0 paintkits without templates or variables. Return
// NULL if var substitution fails or if we can't resolve a template or something
// (after outputting a meaningful error message, of course).
KeyValues *ParseTopLevelIntoKV(const char *pRootName, KeyValues *pValues,
                               uint32 nTexCompositeCreateFlags,
                               bool *pOutAllocdNew) {
  ...
  pExpandedKV = ResolveTemplate(pRootName, pValues, nTexCompositeCreateFlags,
                                &bRequiresCleanup);
  if (pExpandedKV == NULL)
    return NULL;

  if (bRequiresCleanup) {
    Assert(autoCleanup_pExpandedKV == nullptr ||
           autoCleanup_pExpandedKV == pExpandedKV);
  ...
  // Only resolve the variables if we're instantiating. During verification
  // time, we'll just check that the keys are sensible and we can skip this.
  pExpandedKV =
      ResolveAllVariables(pRootName, varDefs, pExpandedKV,
                          nTexCompositeCreateFlags, &bRequiresCleanup);
  if (pExpandedKV == NULL)
    return NULL;
  ...
  Assert(bRequiresCleanup || varDefs.Count() == 0 ||
         ((nTexCompositeCreateFlags &
           TEX_COMPOSITE_CREATE_FLAGS_VERIFY_TEMPLATE_ONLY) != 0));
  varDefs.RemoveAll(); // These won't be valid after we cleanup the tree to
...
// ------------------------------------------------------------------------------------------------
CTextureCompositor *CreateTextureCompositor(int _w, int _h,
                                            const char *pCompositeName,
                                            int nTeamNum, uint64 nRandomSeed,
                                            KeyValues *_stageDesc,
                                            uint32 nTexCompositeCreateFlags) {
  ...
  if (r_texcomp_dump.GetInt() == 3 || r_texcomp_dump.GetInt() == 4) {
    ...
    nTexCompositeCreateFlags |= (TEX_COMPOSITE_CREATE_FLAGS_NO_COMPRESSION |
  ...
  _stageDesc = ParseTopLevelIntoKV(pCompositeName, _stageDesc,
                                   nTexCompositeCreateFlags, &bRequiresCleanup);
  if (!_stageDesc) {
    if ((nTexCompositeCreateFlags &
         TEX_COMPOSITE_CREATE_FLAGS_LOG_NODES_ONLY) != 0)
  ...
  const char *pTemplateName = NULL;
  // If we're just doing a template verification, and we still have keys or
  // values that look like template stuff, bail out now.
  if (HasTemplateOrVariables(&pTemplateName, _stageDesc) &&
      ((nTexCompositeCreateFlags &
        TEX_COMPOSITE_CREATE_FLAGS_VERIFY_TEMPLATE_ONLY) != 0)) {
    CTextureCompositor *pComp =
        new CTextureCompositor(_w, _h, nTeamNum, pCompositeName, nRandomSeed,
                               nTexCompositeCreateFlags);
    if (pTemplateName)
  ...
  if (!ParseNodes(&vecStage, kvs, nTexCompositeCreateFlags)) {
    FOR_EACH_VEC(vecStage, i) { SafeRelease(&vecStage[i]); }
  ...
  CTextureCompositor *texCompositor = new CTextureCompositor(
      _w, _h, nTeamNum, pCompositeName, nRandomSeed, nTexCompositeCreateFlags);
  if (pTemplateName)
...
CTextureCompositorTemplate::Create(const char *pName, KeyValues *pTmplDesc) {
  if (!pName || !pTmplDesc)
    return NULL;

  CTextureCompositor *texCompositor = CreateTextureCompositor(
      1, 1, pName, 2, 0, pTmplDesc,
      TEX_COMPOSITE_CREATE_FLAGS_VERIFY_SCHEMA_ONLY |
          TEX_COMPOSITE_CREATE_FLAGS_VERIFY_TEMPLATE_ONLY);

  if (texCompositor) {
    CTextureCompositorTemplate *pTemplate =
        new CTextureCompositorTemplate(pName, pTmplDesc);
...
bool CTextureCompositorTemplate::ResolveDependencies() const {
  // If we don't reference another template, then our verification was validated
  // at construction time.
  if (m_ImplementsName.IsEmpty())
    return true;

  CTextureCompositorTemplate *pImplementsTmpl =
      TextureManager()->FindTextureCompositorTemplate(m_ImplementsName);

  // If we couldn't find our child, then we are not okay.
  if (pImplementsTmpl == NULL) {
    Warning("ERROR[paintkit_template %s]: Couldn't find template '%s' which we "
            "claim to implement.\n",
...
bool CTextureCompositorTemplate::HasDependencyCycles() {
  ...
  if (HasCycle(this)) {
    // Print the cycle. This also marks the nodes as having been tested for
    // cycles.
...
void ComputeTextureMatrixFromRectangle(VMatrix *pOutMat, const Vector2D &bl,
  ...
  float rotationD =
      RAD2DEG(acos(leftEdge.y / magLeftEdge)) * (leftEdge.x < 0 ? 1 : -1);
...
bool HasCycle(CTextureCompositorTemplate *pStartTempl) {
  ...
  while (pHare != NULL) {
    Assert(pTortoise !=
           NULL); // pTortoise should never be NULL unless pHare already is.

    if (pTortoise == pHare)
      return true;

    // There may still actually be a cycle here, but we've already reported it
    // if so, so go ahead and bail out and say "no cycle found."
...
void PrintMinimumCycle(CTextureCompositorTemplate *pTmpl) {
  ...
  // Also mark the nodes as having been cycle-tested to save execution of
  // retesting the same templates.

  // Finding a minimum cycle is O( n log n ) using a map, but we only do this
  // when there's an error.
  CUtlMap<CTextureCompositorTemplate *, int> cycles(
      DefLessFunc(CTextureCompositorTemplate *));
  CUtlLinkedList<const char *> cycleBuilder;

  while (pTmpl != NULL) {
    // Add before we bail so that the first looping element is in the list
    // twice.
  ...
  Warning("ERROR[paintkit_template %s]: Detected cycle in paintkit template "
          "dependency chain: ",
          pFirstNodeName);
  FOR_EACH_LL(cycleBuilder, i) { Warning("%s -> ", cycleBuilder[i]); }
