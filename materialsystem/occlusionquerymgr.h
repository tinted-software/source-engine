...
// because the GPU/driver can buffer frames we need to allow several queries to
// be in flight. The game wants to reiusse the queries every frame so we buffer
// them here to avoid having to block waiting for a query to be available for
// reissue.
...
public:
  ...
  int OcclusionQuery_GetNumPixelsRendered(OcclusionQueryObjectHandle_t h,
                                          bool bDoQuery);
  void
  OcclusionQuery_IssueNumPixelsRenderedQuery(OcclusionQueryObjectHandle_t h);
