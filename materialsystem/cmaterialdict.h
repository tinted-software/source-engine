...
CMaterialDict::GetMaterialInternal(MaterialHandle_t idx) const {
  Assert(ThreadInMainThread());
  Assert((m_MaterialDict[idx].m_pMaterial == NULL) ||
         m_MaterialDict[idx].m_pMaterial->IsRealTimeVersion());
  return m_MaterialDict[idx].m_pMaterial;
}

inline IMaterialInternal *
CMaterialDict::FindMaterial(const char *pszName, bool bManuallyCreated) const {
  Assert(ThreadInMainThread());
  MaterialLookup_t lookup;
  lookup.m_Name = pszName;
  lookup.m_bManuallyCreated =
      bManuallyCreated; // This causes the search to find only file-created
                        // materials
