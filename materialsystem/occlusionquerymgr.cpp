...
void COcclusionQueryMgr::OnCreateOcclusionQueryObject(
  ...
  for (int i = 0; i < COUNT_OCCLUSION_QUERY_STACK; i++) {
    m_OcclusionQueryObjects[(intp)h].m_QueryHandle[i] =
        g_pShaderAPI->CreateOcclusionQueryObject();
  }
}

// Flushes an outstanding query
// HEY - Be very careful using this method - it causes a full pipeline
// flush/stall!
void COcclusionQueryMgr::FlushQuery(
    OcclusionQueryObjectHandle_t hOcclusionQuery, int nIndex) {
  ...
  if (m_OcclusionQueryObjects[h].m_bHasBeenIssued[nIndex]) {
    ShaderAPIOcclusionQuery_t hQuery =
        m_OcclusionQueryObjects[h].m_QueryHandle[nIndex];
...
void COcclusionQueryMgr::DestroyOcclusionQueryObject(
  ...
  if (m_OcclusionQueryObjects.IsValidIndex(h)) {
    for (int i = 0; i < COUNT_OCCLUSION_QUERY_STACK; i++) {
      if (m_OcclusionQueryObjects[h].m_QueryHandle[i] !=
          INVALID_SHADERAPI_OCCLUSION_QUERY_HANDLE) {
        g_pShaderAPI->DestroyOcclusionQueryObject(
            m_OcclusionQueryObjects[h].m_QueryHandle[i]);
      }
    }
    m_Mutex.Lock();
...
void COcclusionQueryMgr::AllocOcclusionQueryObjects(void) {
  FOR_EACH_LL(m_OcclusionQueryObjects, iterator) {
    for (int i = 0; i < COUNT_OCCLUSION_QUERY_STACK; i++) {
      m_OcclusionQueryObjects[iterator].m_QueryHandle[i] =
          g_pShaderAPI->CreateOcclusionQueryObject();
      m_OcclusionQueryObjects[iterator].m_bHasBeenIssued[i] =
...
void COcclusionQueryMgr::FreeOcclusionQueryObjects(void) {
  FOR_EACH_LL(m_OcclusionQueryObjects, iterator) {
    for (int i = 0; i < COUNT_OCCLUSION_QUERY_STACK; i++) {
      if (m_OcclusionQueryObjects[iterator].m_QueryHandle[i] !=
          INVALID_SHADERAPI_OCCLUSION_QUERY_HANDLE) {
        g_pShaderAPI->DestroyOcclusionQueryObject(
            m_OcclusionQueryObjects[iterator].m_QueryHandle[i]);
        m_OcclusionQueryObjects[iterator].m_QueryHandle[i] =
...
void COcclusionQueryMgr::BeginOcclusionQueryDrawing(
  ...
  if (m_OcclusionQueryObjects.IsValidIndex(h)) {
    ...
    ShaderAPIOcclusionQuery_t hQuery =
        m_OcclusionQueryObjects[h].m_QueryHandle[nCurrent];
    if (hQuery != INVALID_SHADERAPI_OCCLUSION_QUERY_HANDLE) {
      ...
      if (m_OcclusionQueryObjects[h].m_bHasBeenIssued[nCurrent]) {
        ...
        while (!OCCLUSION_QUERY_FINISHED(nPixels)) {
          // We're going to reuse this query, so issue a flush to force the
          // query results to come back.
          nPixels =
              g_pShaderAPI->OcclusionQuery_GetNumPixelsRendered(hQuery, true);
        }
        if (nPixels >= 0) {
...
void COcclusionQueryMgr::OcclusionQuery_IssueNumPixelsRenderedQuery(
  ...
  if (m_OcclusionQueryObjects.IsValidIndex(h)) {
    for (int i = 0; i < COUNT_OCCLUSION_QUERY_STACK; i++) {
      int nIndex = (m_OcclusionQueryObjects[h].m_nCurrentIssue + i) %
                   COUNT_OCCLUSION_QUERY_STACK;
      ShaderAPIOcclusionQuery_t hQuery =
          m_OcclusionQueryObjects[h].m_QueryHandle[nIndex];
      if (hQuery != INVALID_SHADERAPI_OCCLUSION_QUERY_HANDLE &&
        ...
        if (nPixels == OCCLUSION_QUERY_RESULT_ERROR) {
          // In GL mode, it's possible for queries to fail (say when
          // mat_queue_mode is toggled). In this case, just clear
          // m_bHasBeenIssued and forget we ever issued this query.
