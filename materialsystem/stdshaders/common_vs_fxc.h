...
void _DecompressShort2NormalTangent(float2 inputNormal, float2 inputTangent,
  ...
  //        (that would make unpacking normal+tangent here together much cheaper
                             ...
                             out float3 outputNormal) // {nX, nY, nZ}
{
  ...
  float2 ztSigns =
      (inputNormal.xy - 128.0f) < 0; // sign bits for zs and binormal (1 or 0)
                                     // set-less-than (slt) asm instruction
...
float4 DecompressBoneWeights(const float4 weights) {
  ...
  if (COMPRESSED_VERTS) {
    ...
    // NOTE: we add 1 here so we can divide by 32768 - which is exact (divide by
    // 32767 is not).
    //       This avoids cracking between meshes with different numbers of bone
    //       weights. We use SHORT2 instead of SHORT2N for a similar reason -
    //       the GPU's conversion from [-32768,+32767] to [-1,+1] is imprecise
    //       in the same way.
    result += 1;
    result /= 32768;
  }
...
#if defined(_X360)
  {
    // X360 dynamic compile hits an internal compiler error using dst(), this is
    // the breakdown of how dst() works from the 360 docs.
    vDist.x = 1;
...
float2 ComputeSphereMapTexCoords(in float3 reflectionVector) {
  ...
  // generate <rx ry rz+1>
  float3 tmp =
      float3(reflectionVector.x, reflectionVector.y, reflectionVector.z + 1.0f);
