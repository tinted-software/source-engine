...
inline int GetReflectionRefractionPixelShaderIndex(bool bReflection,
                                                   bool bRefraction) {
  // "REFLECT" "0..1"
  // "REFRACT" "0..1"
...
inline void DrawReflectionRefraction(IMaterialVar **params,
  ...
  SHADOW_STATE {
    ...
    int pshIndex =
        GetReflectionRefractionPixelShaderIndex(bReflection, bRefraction);
  ...
  DYNAMIC_STATE {
    ...
    SetVertexShaderConstant(VERTEX_SHADER_SHADER_SPECIFIC_CONST_0,
                            REFLECTAMOUNT);
    ...
    float reflectionRefractionScale[4] = {
...
SHADER_DRAW {
  ...
  // * In the default case, it will fade based on these parameters in a single
  // pass
  // * In the expensive case, it will have to perform the fade in a separate
  // pass.
  ...
  DrawCheapType_t type = params[NOFRESNEL]->GetIntValue()
                             ? DRAW_CHEAP_OPAQUE
                             : DRAW_CHEAP_FRESNEL_OPAQUE;
  if (!bForceCheap && (bRefraction || bReflection)) {
    DrawReflectionRefraction(params, pShaderShadow, pShaderAPI, bReflection,
                             bRefraction);
    if (!bReflection) {
      type = params[NOFRESNEL]->GetIntValue() ? DRAW_CHEAP_LOD_ONLY
                                              : DRAW_CHEAP_FRESNEL_AND_LOD;
    } else {
