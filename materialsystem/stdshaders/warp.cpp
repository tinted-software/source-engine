...
SHADER_DRAW {
  CWarp_DX9_Context *pContextData =
      reinterpret_cast<CWarp_DX9_Context *>(*pContextDataPtr);
  ...
  DYNAMIC_STATE {
    ...
    // GetVectorRenderingParameter only return a Vec3, not a Vec4, so I need to
    // repack these :-(
    vTemp = pShaderAPI->GetVectorRenderingParameter(
    ...
    int nDistortType =
        pShaderAPI->GetIntRenderingParameter(INT_RENDERPARM_DISTORTION_TYPE);
