...
void DrawBaseTimesVertexColor(IMaterialVar **params,
  ...
  SHADOW_STATE {
    ...
    // Get alpha from the texture so that alpha blend and alpha test work
    // properly.
    ...
    if (bTextureIsTranslucent) {
      ...
      if (IsAlphaModulating()) {
        ...
        pShaderShadow->CustomTextureOperation(
            SHADER_TEXTURE_STAGE0, SHADER_TEXCHANNEL_ALPHA,
            SHADER_TEXOP_SELECTARG1, SHADER_TEXARG_ONE, SHADER_TEXARG_NONE);
      }
    }

    pShaderShadow->EnableTexture(SHADER_SAMPLER0, true);
    int flags =
        SHADER_DRAW_POSITION | SHADER_DRAW_NORMAL | SHADER_DRAW_TEXCOORD0;
    ...
    if (IsAlphaModulating() || IsColorModulating()) {
      ...
      if (IsAlphaModulating() && bTextureIsTranslucent) {
        pShaderShadow->CustomTextureOperation(
            SHADER_TEXTURE_STAGE1, SHADER_TEXCHANNEL_ALPHA,
            SHADER_TEXOP_MODULATE, SHADER_TEXARG_PREVIOUSSTAGE,
            SHADER_TEXARG_CONSTANTCOLOR);
      } else {
        pShaderShadow->CustomTextureOperation(
...
void DrawVertexColorNoBase(IMaterialVar **params, IShaderDynamicAPI *pShaderAPI,
                           IShaderShadow *pShaderShadow) {
  SHADOW_STATE {
    pShaderShadow->EnableCustomPixelPipe(true);
...
void DrawBaseTimesBakedVertexLighting(IMaterialVar **params,
                                      IShaderDynamicAPI *pShaderAPI,
                                      IShaderShadow *pShaderShadow) {
  SHADOW_STATE {
    // alpha test
...
void DrawBakedVertexLightingNoBase(IMaterialVar **params,
                                   IShaderDynamicAPI *pShaderAPI,
                                   IShaderShadow *pShaderShadow) {
  SHADOW_STATE {
    int flags = SHADER_DRAW_POSITION;
...
SHADER_DRAW {
  bool bBakedLighting =
      IS_FLAG2_SET(MATERIAL_VAR2_USE_FIXED_FUNCTION_BAKED_LIGHTING);
  ...
  if (params[BASETEXTURE]->IsTexture()) {
    ...
    // Lighting is either sent down per vertex from the app, or it's in the
    // second stream as color values.
    if (bBakedLighting) {
