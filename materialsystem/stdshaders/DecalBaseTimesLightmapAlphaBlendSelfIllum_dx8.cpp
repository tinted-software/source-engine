...
void DrawDecal(IMaterialVar **params, IShaderDynamicAPI *pShaderAPI,
  ...
  if (IsSnapshotting()) {
    ...
    pShaderShadow->SetVertexShader("LightmappedGeneric_Decal",
                                   vshIndex.GetIndex());
  ...
  if (IsSnapshotting()) {
    ...
    pShaderShadow->SetPixelShader(
        "DecalBaseTimesLightmapAlphaBlendSelfIllum2_ps11");
...
SHADER_DRAW {
  if (UsingFlashlight(params)) {
    DrawFlashlight_dx80(params, pShaderAPI, pShaderShadow, false, -1, -1, -1,
                        FLASHLIGHTTEXTURE, FLASHLIGHTTEXTUREFRAME, true, false,
                        0, -1, -1);
  } else {
