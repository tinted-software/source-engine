...
SHADER_DRAW {
  ...
  if ((params[STAGE]->GetIntValue() == 0) &&
      ((params[PORTALOPENAMOUNT]->GetFloatValue() <= 0.0f) ||
       (params[PORTALOPENAMOUNT]->GetFloatValue() >=
        1.0f))) // If in stage 0 and not partially opened
  {
    bDraw = false;
  }
