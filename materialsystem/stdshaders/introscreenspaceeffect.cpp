...
BEGIN_VS_SHADER_FLAGS(IntroScreenSpaceEffect, "Help for IntroScreenSpaceEffect",
                      SHADER_NOT_EDITABLE)
BEGIN_SHADER_PARAMS
...
SHADER_DRAW {
  SHADOW_STATE {
    ...
    // textures AND sRGB writes on the way out to the framebuffer.
    ...
    // Only need the adapter if the shader expects sRGB values and we're forced
    // to do an sRGB read by the API/Hardware
    bool bNeedsSRGBAdapter = (params[ENABLESRGB]->GetIntValue() == 0) &&
  ...
  DYNAMIC_STATE {
    ...
    if (g_pHardwareConfig->SupportsPixelShaders_2_b() ||
        g_pHardwareConfig
            ->ShouldAlwaysUseShaderModel2bShaders()) // Posix always goes the
                                                     // ps2b way for this
                                                     // shader, even on "ps20"
                                                     // parts
    {
