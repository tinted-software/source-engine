...
void DrawDecal(IMaterialVar **params, IShaderDynamicAPI *pShaderAPI,
  ...
  if (IsSnapshotting()) {
    ...
    pShaderShadow->EnableSRGBRead(
        SHADER_SAMPLER2, g_pHardwareConfig->GetHDRType() == HDR_TYPE_NONE);
    pShaderShadow->EnableTexture(SHADER_SAMPLER3, true);
    pShaderShadow->EnableSRGBRead(
        SHADER_SAMPLER3, g_pHardwareConfig->GetHDRType() == HDR_TYPE_NONE);

    int pTexCoords[3] = {2, 2, 1};
    pShaderShadow->VertexShaderVertexFormat(VERTEX_POSITION | VERTEX_COLOR, 3,
                                            pTexCoords, 0);
    ...
    Vector vecZValues(g_localBumpBasis[0].z, g_localBumpBasis[1].z,
                      g_localBumpBasis[2].z);
  ...
  if (IsSnapshotting()) {
    ...
    if (g_pHardwareConfig->SupportsPixelShaders_2_b()) {
      ...
      DECLARE_STATIC_PIXEL_SHADER(
          decalbasetimeslightmapalphablendselfillum2_ps20);
...
void DrawPass(IMaterialVar **params, IShaderDynamicAPI *pShaderAPI,
              IShaderShadow *pShaderShadow, bool bUsingFlashlight) {
  if (bUsingFlashlight) {
