...
BEGIN_VS_SHADER_FLAGS(ColorCorrection, "Help for ColorCorrection",
                      SHADER_NOT_EDITABLE)
BEGIN_SHADER_PARAMS
SHADER_PARAM(WEIGHT_DEFAULT, SHADER_PARAM_TYPE_FLOAT, "1",
             "Volume Texture Default Weight")
...
SHADER_PARAM(NUM_LOOKUPS, SHADER_PARAM_TYPE_INTEGER, "0",
             "Number of lookup maps")
SHADER_PARAM(USE_FB_TEXTURE, SHADER_PARAM_TYPE_BOOL, "0",
             "Use frame buffer texture as input")
...
SHADER_DRAW {
  ...
  DYNAMIC_STATE {
    ...
    if (g_pHardwareConfig->SupportsPixelShaders_2_b()) {
      ...
      SET_DYNAMIC_PIXEL_SHADER_COMBO(NUM_LOOKUPS,
                                     params[NUM_LOOKUPS]->GetIntValue());
