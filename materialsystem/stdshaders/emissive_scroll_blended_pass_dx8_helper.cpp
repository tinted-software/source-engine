...
SHADER_PARAM_TYPE_COLOR, "[1 1 1]", "Self-illumination tint" )
...
void InitParamsEmissiveScrollBlendedPass(
  ...
  if ((info.m_nEmissiveScrollVector >= 0) &&
...
void DrawEmissiveScrollBlendedPass(CBaseVSShader *pShader,
  ...
  SHADOW_STATE {
    ...
    // Set stream format
    pShaderShadow->VertexShaderVertexFormat(VERTEX_POSITION | VERTEX_NORMAL, 1,
                                            0, 0);

    // Vertex Shader
    emissive_scroll_blended_pass_dx8_vs11_Static_Index vshIndex;
    pShaderShadow->SetVertexShader("emissive_scroll_blended_pass_dx8_vs11",
                                   vshIndex.GetIndex());
  ...
  DYNAMIC_STATE {
    ...
    pShader->SetVertexShaderTextureTransform(
        VERTEX_SHADER_SHADER_SPECIFIC_CONST_0, BASETEXTURETRANSFORM);

    float vEmissiveScrollVector[4] = {kDefaultEmissiveScrollVector[0],
                                      kDefaultEmissiveScrollVector[1], 0.0f,
                                      0.0f};
    ...
    float curTime = IS_PARAM_DEFINED(info.m_nTime) &&
                            params[info.m_nTime]->GetFloatValue() > 0.0f
                        ? params[info.m_nTime]->GetFloatValue()
                        : pShaderAPI->CurrentTime();
    float selfIllumScroll[4] = {vEmissiveScrollVector[0] * curTime,
                                vEmissiveScrollVector[1] * curTime, 0.0f, 0.0f};
    ...
    c4[0] = IS_PARAM_DEFINED(info.m_nBlendStrength)
