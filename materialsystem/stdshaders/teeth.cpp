...
void DrawUsingVertexShader(IMaterialVar **params, IShaderDynamicAPI *pShaderAPI,
  ...
  SHADOW_STATE {
    ...
    if (hasBump) {
      ...
      pShaderShadow->EnableTexture(
          SHADER_SAMPLER2,
          true); // Normalization sampler for per-pixel lighting
    ...
    if (hasBump) {
...
#endif
      {
        ...
        SET_STATIC_VERTEX_SHADER_COMBO(INTRO,
                                       params[INTRO]->GetIntValue() ? 1 : 0);
...
void DrawFlashlight(IMaterialVar **params, IShaderDynamicAPI *pShaderAPI,
  ...
  DYNAMIC_STATE {
    ...
    BindTexture(SHADER_SAMPLER1, flashlightState.m_pSpotlightTexture,
