...
SHADER_PARAM(DETAILTRANSFORM, SHADER_PARAM_TYPE_MATRIX,
             "center .5 .5 scale 1 1 rotate 0 translate 0 0", "detail scale")
SHADER_PARAM(ENVMAP, SHADER_PARAM_TYPE_TEXTURE, "shadertest/shadertest_env",
             "envmap")
SHADER_PARAM(ENVMAPFRAME, SHADER_PARAM_TYPE_INTEGER, "", "")
SHADER_PARAM(ENVMAPTINT, SHADER_PARAM_TYPE_COLOR, "[1 1 1]", "envmap tint")
SHADER_PARAM(UNLITFACTOR, SHADER_PARAM_TYPE_FLOAT, "0.7",
             "0.0 == multiply by lightmap, 1.0 == multiply by 1")
END_SHADER_PARAMS
...
SHADER_DRAW {
  ...
  SHADOW_STATE {
    ...
    // Alpha blending, enable alpha blending if the detail texture is
    // translucent
    ...
    if (!bHasEnvmap) {
      ...
      if (!IS_FLAG_SET(MATERIAL_VAR_ENVMAPSPHERE)) {
        ...
        pShaderShadow->SetVertexShader("shatteredglass_envmap",
                                       vshIndex.GetIndex());
      } else {
        shatteredglass_envmapsphere_Static_Index vshIndex;
        pShaderShadow->SetVertexShader("shatteredglass_envmapsphere",
                                       vshIndex.GetIndex());
      }
  ...
  DYNAMIC_STATE {
    ...
    // Blend between lightmap + unlit based on the unlit factor to make it look
    // better
