...
void Init_Eyes_Refract(CBaseVSShader *pShader, IMaterialVar **params,
                       Eye_Refract_Vars_t &info) {
  pShader->LoadTexture(info.m_nCorneaTexture); // SHADER_SAMPLER0  (this is a
                                               // normal, hence not sRGB)
  ...
  pShader->LoadTexture(info.m_nAmbientOcclTexture,
                       TEXTUREFLAGS_SRGB); // SHADER_SAMPLER3
...
void Draw_Eyes_Refract_Internal(CBaseVSShader *pShader, IMaterialVar **params,
  ...
  bool bIntro = IS_PARAM_DEFINED(info.m_nIntro)
                    ? (params[info.m_nIntro]->GetIntValue() ? true : false)
                    : false;
  ...
  SHADOW_STATE {
...
#endif
    {
      ...
      SET_STATIC_VERTEX_SHADER_COMBO(FLASHLIGHT,
      ...
      if (g_pHardwareConfig->SupportsPixelShaders_2_b()) {
        bool bSphereTexKillCombo =
            IS_PARAM_DEFINED(info.m_nSphereTexKillCombo)
                ? (params[info.m_nSphereTexKillCombo]->GetIntValue() ? true
                                                                     : false)
                : (kDefaultSphereTexKillCombo ? true : false);
        bool bRayTraceSphere =
            IS_PARAM_DEFINED(info.m_nRaytraceSphere)
        ...
        SET_STATIC_PIXEL_SHADER_COMBO(SPHERETEXKILLCOMBO,
                                      bSphereTexKillCombo ? 1 : 0);
        SET_STATIC_PIXEL_SHADER_COMBO(RAYTRACESPHERE, bRayTraceSphere ? 1 : 0);
        SET_STATIC_PIXEL_SHADER_COMBO(FLASHLIGHT,
                                      bDrawFlashlightAdditivePass ? 1 : 0);
        ...
        if (bDrawFlashlightAdditivePass == true) {
          pShaderShadow->EnableTexture(SHADER_SAMPLER6,
  ...
  DYNAMIC_STATE {
    ...
    if (bDrawFlashlightAdditivePass == true) {
      flashlightState = pShaderAPI->GetFlashlightStateEx(
          worldToTexture, &pFlashlightDepthTexture);
      bFlashlightShadows =
          flashlightState.m_bEnableShadows && (pFlashlightDepthTexture != NULL);
    }

    pShader->BindTexture(SHADER_SAMPLER0,
                         info.m_nCorneaTexture); // Cornea normal
    ...
    if (bDiffuseWarp) {
      if (r_lightwarpidentity.GetBool()) {
        pShaderAPI->BindStandardTexture(SHADER_SAMPLER4,
    ...
    vPSConst[0] = IS_PARAM_DEFINED(info.m_nDilation)
                      ? params[info.m_nDilation]->GetFloatValue()
                      : kDefaultDilation;
    vPSConst[1] = IS_PARAM_DEFINED(info.m_nGlossiness)
                      ? params[info.m_nGlossiness]->GetFloatValue()
                      : kDefaultGlossiness;
    vPSConst[2] = fAverageAmbient;
    vPSConst[3] = IS_PARAM_DEFINED(info.m_nCorneaBumpStrength)
                      ? params[info.m_nCorneaBumpStrength]->GetFloatValue()
                      : kDefaultCorneaBumpStrength;
    pShaderAPI->SetPixelShaderConstant(0, vPSConst, 1);

    pShaderAPI->SetPixelShaderConstant(
        1,
        IS_PARAM_DEFINED(info.m_nEyeOrigin)
            ? params[info.m_nEyeOrigin]->GetVecValue()
            : kDefaultEyeOrigin,
        1);
    pShaderAPI->SetPixelShaderConstant(
        2,
        IS_PARAM_DEFINED(info.m_nIrisU) ? params[info.m_nIrisU]->GetVecValue()
                                        : kDefaultIrisU,
        1);
    pShaderAPI->SetPixelShaderConstant(
        3,
        IS_PARAM_DEFINED(info.m_nIrisV) ? params[info.m_nIrisV]->GetVecValue()
                                        : kDefaultIrisV,
        1);
    ...
    vPackedConst6[1] = IS_PARAM_DEFINED(info.m_nEyeballRadius)
                           ? params[info.m_nEyeballRadius]->GetFloatValue()
    ...
    if (bDrawFlashlightAdditivePass == true) {
      ...
      if (pFlashlightDepthTexture && g_pConfig->ShadowDepthTexture() &&
        ...
        pShaderAPI->BindStandardTexture(SHADER_SAMPLER7,
...
void Draw_Eyes_Refract(CBaseVSShader *pShader, IMaterialVar **params,
  ...
  if (bHasFlashlight && (IsX360() || r_flashlight_version2.GetInt())) {
    Draw_Eyes_Refract_Internal(pShader, params, pShaderAPI, pShaderShadow,
