...
"models/shadertest/shader1_normal", "bump map" )
                        //SHADER_PARAM( BUMPFRAME, SHADER_PARAM_TYPE_INTEGER,
"0", "frame number for $bumpmap" )
...
void DrawCloakBlendedPass(CBaseVSShader *pShader, IMaterialVar **params,
  ...
  bool bBumpMapping = (!g_pConfig->UseBumpmapping()) ||
                              (info.m_nBumpmap == -1) ||
                              !params[info.m_nBumpmap]->IsTexture()
                          ? 0
                          : 1;
  ...
  DYNAMIC_STATE {
    ...
    if ((bBumpMapping) && (info.m_nBumpTransform != -1)) {
      pShader->SetVertexShaderTextureTransform(
          VERTEX_SHADER_SHADER_SPECIFIC_CONST_0, info.m_nBumpTransform);
    }
    ...
    vPackedConst1[0] = IS_PARAM_DEFINED(info.m_nCloakFactor)
                           ? params[info.m_nCloakFactor]->GetFloatValue()
                           : kDefaultCloakFactor;
    vPackedConst1[1] = IS_PARAM_DEFINED(info.m_nRefractAmount)
