...
SHADER_PARAM(
    ALTERNATEVIEWMATRIX, SHADER_PARAM_TYPE_MATRIX, "0",
...
SHADER_DRAW {
  ...
  bool bAlphaMaskTexture =
      ((iMaxTextureStages > 1) && (params[ALPHAMASKTEXTURE]->IsTexture()))
          ? (1)
          : (0); // must support at least 2 texture stages to use any kind of
                 // mask
  ...
  SHADOW_STATE {
    ...
    if (((iMaxTextureStages < 3) && bStaticBlendTexture) ||
        bAlphaMaskTexture) // in multipass modes, we need alpha to mix portal
                           // static, other
    {
    ...
    pShaderShadow->VertexShaderVertexFormat(fmt, iTexCoords, NULL,
                                            userDataSize);
    ...
    SET_STATIC_VERTEX_SHADER_COMBO(
        USEALTERNATEVIEW, (params[USEALTERNATEVIEWMATRIX]->GetIntValue() != 0));
  ...
  DYNAMIC_STATE {
    ...
    if (params[USEALTERNATEVIEWMATRIX]->GetIntValue() != 0) {
      const VMatrix &matCustomView =
          params[ALTERNATEVIEWMATRIX]->GetMatrixValue();
  ...
  if (iMaxTextureStages == 3) {
    SinglePass(pShaderShadow, pShaderAPI, params, fStaticAmount,
               bStaticBlendTexture, bAlphaMaskTexture);
  } else {
    MultiPass_CutoutPass(pShaderShadow, pShaderAPI, params, bAlphaMaskTexture);
    if (bStaticBlendTexture) // if we don't have a static texture, the cutout
                             // pass will mix in gray for any static amount
      MultiPass_StaticPass(pShaderShadow, pShaderAPI, params,
                           bAlphaMaskTexture);
  }
}

void SinglePass(IShaderShadow *pShaderShadow, IShaderDynamicAPI *pShaderAPI,
                IMaterialVar **params, float fStaticAmount,
                bool bStaticBlendTexture, bool bAlphaMaskTexture) {
  SHADOW_STATE {
  ...
  DYNAMIC_STATE {
    ...
    if (bAlphaMaskTexture) {
      ...
      if (bUsingStaticTexture)
        BindTexture(SHADER_SAMPLER2, STATICBLENDTEXTURE,
                    STATICBLENDTEXTUREFRAME);
    } else {
      if (bUsingStaticTexture)
        BindTexture(SHADER_SAMPLER1, STATICBLENDTEXTURE,
                    STATICBLENDTEXTUREFRAME);
    }
...
void MultiPass_CutoutPass(IShaderShadow *pShaderShadow,
...
void MultiPass_StaticPass(IShaderShadow *pShaderShadow,
                          IShaderDynamicAPI *pShaderAPI, IMaterialVar **params,
                          bool bAlphaMaskTexture) {
  SHADOW_STATE {
    pShaderShadow->EnableTexture(SHADER_SAMPLER0, true);
