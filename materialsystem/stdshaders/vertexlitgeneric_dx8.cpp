...
void DrawUnbumpedUsingVertexShader(IMaterialVar **params,
  ...
  SHADOW_STATE {
    ...
    // FIXME: We could enable this, but we'd never get it working on dx7 or
    // lower
    ...
    pShaderShadow->SetVertexShader("vertexlitgeneric_vs11",
                                   vshIndex.GetIndex());
  ...
  DYNAMIC_STATE {
    ...
    if (params[BLENDTINTBYBASEALPHA]->GetIntValue()) {
      float c1[4] = {1.0f, 1.0f, 1.0f, 1.0f};
      c1[0] = c1[1] = c1[2] = c1[3] =
          params[BLENDTINTCOLOROVERBASE]->GetFloatValue();
    ...
    if (pShaderAPI->GetIntRenderingParameter(
            INT_RENDERPARM_WRITE_DEPTH_TO_DESTALPHA)) {
      pShaderAPI->SetPixelShaderIndex(0);
    } else {
      // write 255 to alpha if we aren't told to write depth to dest alpha (We
      // don't ever actually write depth to dest alpha in dx8)
...
SHADER_DRAW {
  ...
  if (false /*disabled! no effect*/ &&
      params[CLOAKPASSENABLED]->GetIntValue() &&
    ...
    if (hasFlashlight) {
      ...
      if (params[ENVMAP]->IsTexture()) {
        DrawModelBumpedSpecularLighting(
            BUMPMAP, BUMPFRAME, ENVMAP, ENVMAPFRAME, ENVMAPTINT, ALPHA,
