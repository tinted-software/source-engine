...
#if defined(_X360)
#define REVERSE_DEPTH_ON_X360 // uncomment to use D3DFMT_D24FS8 with an inverted
                              // depth viewport for better performance. Keep
                              // this in sync with the same named #define in
...
float3 Vec3WorldToTangentNormalized(float3 iWorldVector, float3 iWorldNormal,
  ...
  return normalize(Vec3WorldToTangent(iWorldVector, iWorldNormal, iWorldTangent,
                                      iWorldBinormal));
}
