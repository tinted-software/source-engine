...
//
// Purpose: Visualize shadow z buffers.  Designed to be used when drawing a
// screen-aligned
//          quad with a floating-point z-buffer so that the large z-range is
//          divided down into visual range of grayscale colors.
//
// $NoKeywords: $
//=============================================================================//
...
BEGIN_SHADER_PARAMS
SHADER_PARAM(ALPHADEPTH, SHADER_PARAM_TYPE_INTEGER, "0",
             "Depth is stored in alpha channel")
END_SHADER_PARAMS
...
SHADER_DRAW {
  SHADOW_STATE {
    ...
    if (g_pHardwareConfig->SupportsPixelShaders_2_b()) {
      ...
      SET_STATIC_PIXEL_SHADER_COMBO(DEPTH_IN_ALPHA,
                                    params[ALPHADEPTH]->GetIntValue());
    ...
    pShaderShadow->EnableSRGBWrite(
        true); // The back buffer is sRGB, we should always set this true!
  }
  DYNAMIC_STATE {
