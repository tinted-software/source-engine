...
SHADER_DRAW {
  SHADOW_STATE {
    ...
    if (g_pHardwareConfig->PlatformRequiresNonNullPixelShaders()) {
      ...
      // Workaround for weird AMD bug - if sRGB write isn't enabled here then
      // sRGB write enable in subsequent world rendering passes will randomly
      // not take effect (even though we're enabling it) in the driver.
      if ((IsLinux() || IsWindows() || IsBSD()) &&
          gl_amd_occlusion_workaround.GetBool()) {
