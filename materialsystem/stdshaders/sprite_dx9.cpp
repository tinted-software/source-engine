...
void SetSpriteCommonShadowState(unsigned int shaderFlags) {
  ...
  // Only enabling this on OSX() - it causes GL mode's light glow sprites to be
  // much darker vs. D3D9 under Linux/Win GL.
  bool bSRGBOutputAdapter =
      (IsOSX() && !g_pHardwareConfig->FakeSRGBWrite()) && !bSRGB;
  ...
  if (g_pHardwareConfig->SupportsPixelShaders_2_b() ||
      g_pHardwareConfig
          ->ShouldAlwaysUseShaderModel2bShaders()) // Always send GL down this
                                                   // path
  {
  ...
  // OSX always has to sRGB write (don't do this on Linux/Win GL - it causes
  // glow sprites to be way too dark)
  s_pShaderShadow->EnableSRGBWrite(
      bSRGB || (IsOSX() && !g_pHardwareConfig->FakeSRGBWrite()));
}
...
SHADER_DRAW {
  ...
  case kRenderTransAddFrameBlend: {
    ...
    DYNAMIC_STATE {
      ...
      if (g_pHardwareConfig->SupportsPixelShaders_2_b() ||
          g_pHardwareConfig
              ->ShouldAlwaysUseShaderModel2bShaders()) // Always send GL down
                                                       // this path
      {
        DECLARE_DYNAMIC_PIXEL_SHADER(sprite_ps20b);
        SET_DYNAMIC_PIXEL_SHADER_COMBO(HDRENABLED, IsHDREnabled());
