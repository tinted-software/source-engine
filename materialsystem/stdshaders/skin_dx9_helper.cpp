...
void InitParamsSkin_DX9(CBaseVSShader *pShader, IMaterialVar **params,
  ...
  // Lots of reasons to want tangent space, since we bind a flat normal map in
  // many cases where we don't have a bump map
  bool bBump = (info.m_nBumpmap != -1) && g_pConfig->UseBumpmapping() &&
               params[info.m_nBumpmap]->IsDefined();
  bool bEnvMap = (info.m_nEnvmap != -1) && params[info.m_nEnvmap]->IsDefined();
  bool bDiffuseWarp = (info.m_nDiffuseWarpTexture != -1) &&
...
void InitSkin_DX9(CBaseVSShader *pShader, IMaterialVar **params,
  ...
  if (params[info.m_nBaseTexture]->IsDefined()) {
    ...
    if ((info.m_nWrinkle != -1) && (info.m_nStretch != -1) &&
        params[info.m_nWrinkle]->IsDefined() &&
  ...
  if ((info.m_nPhongExponentTexture != -1) &&
      params[info.m_nPhongExponentTexture]->IsDefined() &&
  ...
  if ((info.m_nPhongWarpTexture != -1) &&
      params[info.m_nPhongWarpTexture]->IsDefined() && (info.m_nPhong != -1) &&
  ...
  if (g_pConfig->UseBumpmapping()) {
    if ((info.m_nBumpmap != -1) && params[info.m_nBumpmap]->IsDefined()) {
      ...
      if ((info.m_nNormalWrinkle != -1) && (info.m_nNormalStretch != -1) &&
          params[info.m_nNormalWrinkle]->IsDefined() &&
          params[info.m_nNormalStretch]->IsDefined()) {
...
// Draws the shader
//-----------------------------------------------------------------------------
void DrawSkin_DX9_Internal(CBaseVSShader *pShader, IMaterialVar **params,
  ...
  bool bHasBaseTexture =
      (info.m_nBaseTexture != -1) && params[info.m_nBaseTexture]->IsTexture();
  bool bHasBump =
      (info.m_nBumpmap != -1) && params[info.m_nBumpmap]->IsTexture();

  bool bHasBaseTextureWrinkle = bHasBaseTexture && (info.m_nWrinkle != -1) &&
                                params[info.m_nWrinkle]->IsTexture() &&
                                (info.m_nStretch != -1) &&
                                params[info.m_nStretch]->IsTexture();

  bool bHasBumpWrinkle = bHasBump && (info.m_nNormalWrinkle != -1) &&
                         params[info.m_nNormalWrinkle]->IsTexture() &&
                         (info.m_nNormalStretch != -1) &&
                         params[info.m_nNormalStretch]->IsTexture();
  ...
  bool bHasSelfIllumFresnel =
      (bHasSelfIllum) && (info.m_nSelfIllumFresnel != -1) &&
      (params[info.m_nSelfIllumFresnel]->GetIntValue() != 0);
  bool bHasSelfIllumMask = (bHasSelfIllum) && (info.m_nSelfIllumMask != -1) &&
                           params[info.m_nSelfIllumMask]->IsTexture();

  // Tie these to specular
  bool bHasPhong =
      (info.m_nPhong != -1) && (params[info.m_nPhong]->GetIntValue() != 0);
  ...
  bool bHasDiffuseWarp = (info.m_nDiffuseWarpTexture != -1) &&
                         params[info.m_nDiffuseWarpTexture]->IsTexture();
  bool bHasPhongWarp = (info.m_nPhongWarpTexture != -1) &&
                       params[info.m_nPhongWarpTexture]->IsTexture();
  ...
  // Rimlight must be set to non-zero to trigger rim light combo (also requires
  // Phong)
  bool bHasRimLight = r_rimlight.GetBool() && bHasPhong &&
                      (info.m_nRimLight != -1) &&
  ...
  float fBlendFactor =
      (info.m_nDetailTextureBlendFactor == -1)
          ? 1
          : params[info.m_nDetailTextureBlendFactor]->GetFloatValue();
  bool hasDetailTexture =
      (info.m_nDetail != -1) && params[info.m_nDetail]->IsTexture();
  int nDetailBlendMode =
      (hasDetailTexture && info.m_nDetailTextureCombineMode != -1)
          ? params[info.m_nDetailTextureCombineMode]->GetIntValue()
          : 0;

  bool bBlendTintByBaseAlpha =
      IsBoolSet(info.m_nBlendTintByBaseAlpha, params) &&
      !bHasSelfIllum; // Pixel shader can't do both BLENDTINTBYBASEALPHA and
                      // SELFILLUM, so let selfillum win

  float flTintReplacementAmount =
      GetFloatParam(info.m_nTintReplacesBaseColor, params);

  BlendType_t nBlendType = pShader->EvaluateBlendRequirements(
      bBlendTintByBaseAlpha ? -1 : info.m_nBaseTexture, true);

  bool bFullyOpaque = (nBlendType != BT_BLENDADD) && (nBlendType != BT_BLEND) &&
                      !bIsAlphaTested &&
                      !bHasFlashlight; // dest alpha is free for special use

  CSkin_DX9_Context *pContextData =
      reinterpret_cast<CSkin_DX9_Context *>(*pContextDataPtr);
  if (!pContextData) {
  ...
  if (pShader->IsSnapshotting()) {
    ...
    bool bCanUseBaseAlphaPhongMaskFastPath =
        ...
        (params[info.m_nBaseMapAlphaPhongMask]->GetIntValue() != 0);

    if (!(params[info.m_nBaseTexture]->GetTextureValue()->IsTranslucent()))
      bCanUseBaseAlphaPhongMaskFastPath = true;

    pContextData->m_bFastPath = (!bHasBump) && (!bHasSpecularExponentTexture) &&
                                (!bHasPhongTintMap) && (!bHasPhongWarp) &&
                                (!bHasRimLight) && (!hasDetailTexture) &&
                                bCanUseBaseAlphaPhongMaskFastPath &&
                                (!bHasSelfIllum) && (!bBlendTintByBaseAlpha);
    ...
    } else                                          // not flashlight pass
    {
      ...
      if (bHasEnvmap) {
        pShaderShadow->EnableTexture(SHADER_SAMPLER8,
                                     true); // Cubic environment map
    ...
    if (bHasBaseTextureWrinkle || bHasBumpWrinkle) {
      pShaderShadow->EnableTexture(SHADER_SAMPLER9,
                                   true); // Base (albedo) compression map
      pShaderShadow->EnableSRGBRead(SHADER_SAMPLER9, true);
      pShaderShadow->EnableTexture(SHADER_SAMPLER10,
                                   true); // Base (albedo) expansion map
      pShaderShadow->EnableSRGBRead(SHADER_SAMPLER10, true);
    }

    if (bHasDiffuseWarp) {
      pShaderShadow->EnableTexture(SHADER_SAMPLER2,
                                   true); // Diffuse warp texture
    }

    if (bHasPhongWarp) {
      pShaderShadow->EnableTexture(SHADER_SAMPLER1,
                                   true); // Specular warp texture
    }

    // Specular exponent map or dummy
    pShaderShadow->EnableTexture(SHADER_SAMPLER7,
                                 true); // Specular exponent map
    ...
    if (bHasBaseTextureWrinkle || bHasBumpWrinkle) {
      pShaderShadow->EnableTexture(SHADER_SAMPLER11,
                                   true); // Normal compression map
      pShaderShadow->EnableTexture(SHADER_SAMPLER12,
                                   true); // Normal expansion map
    }
    ...
    pShaderShadow->VertexShaderVertexFormat(flags, nTexCoordCount, pTexCoordDim,
                                            userDataSize);
...
#endif
    {
      bool bUseStaticControlFlow =
          g_pHardwareConfig->SupportsStaticControlFlow();

      DECLARE_STATIC_VERTEX_SHADER(skin_vs20);
      SET_STATIC_VERTEX_SHADER_COMBO(USE_STATIC_CONTROL_FLOW,
      ...
      SET_STATIC_PIXEL_SHADER_COMBO(SELFILLUM,
                                    bHasSelfIllum && !bHasFlashlight);
      SET_STATIC_PIXEL_SHADER_COMBO(SELFILLUMFRESNEL,
                                    bHasSelfIllumFresnel && !bHasFlashlight);
      SET_STATIC_PIXEL_SHADER_COMBO(LIGHTWARPTEXTURE,
                                    bHasDiffuseWarp && bHasPhong);
      SET_STATIC_PIXEL_SHADER_COMBO(PHONGWARPTEXTURE,
                                    bHasPhongWarp && bHasPhong);
      SET_STATIC_PIXEL_SHADER_COMBO(WRINKLEMAP,
                                    bHasBaseTextureWrinkle || bHasBumpWrinkle);
      ...
      SET_STATIC_PIXEL_SHADER_COMBO(BLENDTINTBYBASEALPHA,
  ...
  } else // not snapshotting -- begin dynamic state
  {
    ...
    if (bHasBaseTextureWrinkle) {
      pShader->BindTexture(SHADER_SAMPLER9, info.m_nWrinkle,
                           info.m_nBaseTextureFrame);
      pShader->BindTexture(SHADER_SAMPLER10, info.m_nStretch,
                           info.m_nBaseTextureFrame);
    } else if (bHasBumpWrinkle) {
      pShader->BindTexture(SHADER_SAMPLER9, info.m_nBaseTexture,
                           info.m_nBaseTextureFrame);
      pShader->BindTexture(SHADER_SAMPLER10, info.m_nBaseTexture,
                           info.m_nBaseTextureFrame);
    }
    ...
    if (!g_pConfig->m_bFastNoBump) {
      ...
      if (bHasBumpWrinkle) {
        pShader->BindTexture(SHADER_SAMPLER11, info.m_nNormalWrinkle,
                             info.m_nBumpFrame);
        pShader->BindTexture(SHADER_SAMPLER12, info.m_nNormalStretch,
                             info.m_nBumpFrame);
      } else if (bHasBaseTextureWrinkle) {
        pShader->BindTexture(SHADER_SAMPLER11, info.m_nBumpmap,
                             info.m_nBumpFrame);
        pShader->BindTexture(SHADER_SAMPLER12, info.m_nBumpmap,
                             info.m_nBumpFrame);
      }
    } else {
      if (bHasBump) {
      ...
      if (bHasBaseTextureWrinkle || bHasBumpWrinkle) {
        pShaderAPI->BindStandardTexture(SHADER_SAMPLER11,
                                        TEXTURE_NORMALMAP_FLAT);
        pShaderAPI->BindStandardTexture(SHADER_SAMPLER12,
                                        TEXTURE_NORMALMAP_FLAT);
      }
    }

    if (hasDetailTexture) {
      pShader->BindTexture(SHADER_SAMPLER13, info.m_nDetail,
                           info.m_nDetailFrame);
    }
    ...
    if (bHasSelfIllum) {
      ...
      {
        pShader->BindTexture(SHADER_SAMPLER14,
                             info.m_nSelfIllumMask); // Bind it
      } else                                         // else
      {
        pShaderAPI->BindStandardTexture(SHADER_SAMPLER14,
    ...
    if (bHasFlashlight) {
      ...
      pShader->BindTexture(SHADER_SAMPLER6, info.m_nFlashlightTexture,
    ...
    {
      if (bHasEnvmap) {
        pShader->BindTexture(SHADER_SAMPLER8, info.m_nEnvmap,
                             info.m_nEnvmapFrame);
      }
    ...
    // don't have an easy way to get this through to GLM, so just print it old
    // school
...
#endif
    {
      ...
      SET_DYNAMIC_VERTEX_SHADER_COMBO(
          NUM_LIGHTS, bUseStaticControlFlow ? 0 : lightState.m_nNumLights);
    ...
    pShader->SetPixelShaderConstant_W(PSREG_SELFILLUMTINT,
                                      info.m_nSelfIllumTint, fBlendFactor);
    bool bInvertPhongMask =
        (info.m_nInvertPhongMask != -1) &&
        (params[info.m_nInvertPhongMask]->GetIntValue() != 0);
    ...
    float vShaderControls[4] = {fHasBaseAlphaPhongMask, 0.0f /*unused*/,
                                flTintReplacementAmount, fInvertPhongMask};
    ...
    if (bHasSelfIllumFresnel && !bHasFlashlight) {
      ...
      pShaderAPI->SetPixelShaderConstant(PSREG_SELFILLUM_SCALE_BIAS_EXP,
    ...
    if (!bHasFlashlight) {
      ...
      // Setting .x to 1 means to apply Fresnel to env map.  Setting w to 1
      // means use separate selfillummask
      ...
      if (bHasEnvmap) {
        ...
        if ((info.m_nEnvmapTint != -1) &&
            params[info.m_nEnvmapTint]->IsDefined())
        ...
        if ((info.m_nEnvmapFresnel != -1) &&
            params[info.m_nEnvmapFresnel]->IsDefined())
          vEnvMapFresnel_SelfIllumMask[0] =
              params[info.m_nEnvmapFresnel]->GetFloatValue();

        // Handle mat_fullbright 2 (diffuse lighting only with 50% gamma space
        // basetexture)
        if (bLightingOnly) {
          vEnvMapTint_MaskControl[0] = vEnvMapTint_MaskControl[1] =
              vEnvMapTint_MaskControl[2] = 0.0f;
        }

        pShaderAPI->SetPixelShaderConstant(PSREG_ENVMAP_TINT__SHADOW_TWEAKS,
                                           vEnvMapTint_MaskControl, 1);
      }

      pShaderAPI->SetPixelShaderConstant(PSREG_ENVMAP_FRESNEL__SELFILLUMMASK,
                                         vEnvMapFresnel_SelfIllumMask, 1);
    }

    pShaderAPI->SetPixelShaderStateAmbientLightCube(
        PSREG_AMBIENT_CUBE,
        !lightState.m_bAmbientLight); // Force to black if not bAmbientLight
    ...
    if (!bHasFlashlight) {
      ...
      vRimMaskControl[0] =
          bHasRimMaskMap ? params[info.m_nRimMask]->GetFloatValue() : 0.0f;

      // Rim mask...if this is true, use alpha channel of spec exponent texture
      // to mask the rim term
    ...
    if ((info.m_nPhongFresnelRanges != -1) &&
      ...
      // Change fresnel range encoding from (min, mid, max) to ((mid-min)*2,
      // mid, (max-mid)*2)
      vFresnelRanges_SpecBoost[0] =
          (vFresnelRanges_SpecBoost[1] - vFresnelRanges_SpecBoost[0]) * 2;
      vFresnelRanges_SpecBoost[2] =
          (vFresnelRanges_SpecBoost[2] - vFresnelRanges_SpecBoost[1]) * 2;
    }
    ...
    if (bHasFlashlight) {
      ...
      pShaderAPI->SetPixelShaderConstant(PSREG_FLASHLIGHT_ATTENUATION, atten,
                                         1);

      pos[0] = flashlightState.m_vecLightOrigin[0]; // Set the flashlight origin
      pos[1] = flashlightState.m_vecLightOrigin[1];
      pos[2] = flashlightState.m_vecLightOrigin[2];
      pShaderAPI->SetPixelShaderConstant(PSREG_FLASHLIGHT_POSITION_RIM_BOOST,
                                         pos, 1); // steps on rim boost
      ...
      if (IsX360()) {
        pShaderAPI->SetBooleanPixelShaderConstant(
            0, &flashlightState.m_nShadowQuality, 1);
      }
    }
  }
  pShader->Draw();
...

{
  ...
  if (bHasFlashlight && (IsX360() || r_flashlight_version2.GetInt())) {
    DrawSkin_DX9_Internal(pShader, params, pShaderAPI, pShaderShadow, false,
                          info, vertexCompression, pContextDataPtr++);
    if (pShaderShadow) {
      pShader->SetInitialShadowState();
    }
  }
  DrawSkin_DX9_Internal(pShader, params, pShaderAPI, pShaderShadow,
