...
BEGIN_VS_SHADER_FLAGS(screenspace_general_dx9, "Help for screenspace_general",
                      SHADER_NOT_EDITABLE)
BEGIN_SHADER_PARAMS
...
SHADER_PARAM(PIXSHADER, SHADER_PARAM_TYPE_STRING, "",
...
SHADER_PARAM(X360APPCHOOSER, SHADER_PARAM_TYPE_INTEGER, "0",
             "Needed for movies in 360 launcher")
...
SHADER_INIT {
  if (params[BASETEXTURE]->IsDefined()) {
    ...
    if ((fmt == IMAGE_FORMAT_RGBA16161616F) ||
      ...
      bSRGB = !params[LINEARREAD_BASETEXTURE]->IsDefined() ||
  ...
  if (params[TEXTURE1]->IsDefined()) {
    ...
    if ((fmt == IMAGE_FORMAT_RGBA16161616F) ||
      ...
      bSRGB = !params[LINEARREAD_TEXTURE1]->IsDefined() ||
  ...
  if (params[TEXTURE2]->IsDefined()) {
    ...
    if ((fmt == IMAGE_FORMAT_RGBA16161616F) ||
      ...
      bSRGB = !params[LINEARREAD_TEXTURE2]->IsDefined() ||
  ...
  if (params[TEXTURE3]->IsDefined()) {
    ...
    if ((fmt == IMAGE_FORMAT_RGBA16161616F) ||
      ...
      bSRGB = !params[LINEARREAD_TEXTURE3]->IsDefined() ||
...
SHADER_DRAW {
  SHADOW_STATE {
    ...
    if (params[BASETEXTURE]->IsDefined()) {
      ...
      if ((fmt == IMAGE_FORMAT_RGBA16161616F) ||
        ...
        pShaderShadow->EnableSRGBRead(
            SHADER_SAMPLER0,
            !params[LINEARREAD_BASETEXTURE]->IsDefined() ||
                !params[LINEARREAD_BASETEXTURE]->GetIntValue());
    }
    if (params[TEXTURE1]->IsDefined()) {
      ...
      if ((fmt == IMAGE_FORMAT_RGBA16161616F) ||
        ...
        pShaderShadow->EnableSRGBRead(
            SHADER_SAMPLER1, !params[LINEARREAD_TEXTURE1]->IsDefined() ||
                                 !params[LINEARREAD_TEXTURE1]->GetIntValue());
    }
    if (params[TEXTURE2]->IsDefined()) {
      ...
      if ((fmt == IMAGE_FORMAT_RGBA16161616F) ||
        ...
        pShaderShadow->EnableSRGBRead(
            SHADER_SAMPLER2, !params[LINEARREAD_TEXTURE2]->IsDefined() ||
                                 !params[LINEARREAD_TEXTURE2]->GetIntValue());
    }
    if (params[TEXTURE3]->IsDefined()) {
      ...
      if ((fmt == IMAGE_FORMAT_RGBA16161616F) ||
        ...
        pShaderShadow->EnableSRGBRead(
            SHADER_SAMPLER3, !params[LINEARREAD_TEXTURE3]->IsDefined() ||
                                 !params[LINEARREAD_TEXTURE3]->GetIntValue());
    }
    int fmt = VERTEX_POSITION;

    if (IS_PARAM_DEFINED(X360APPCHOOSER) &&
        (params[X360APPCHOOSER]->GetIntValue())) {
      fmt |= VERTEX_COLOR;
    ...
    SET_STATIC_VERTEX_SHADER_COMBO(X360APPCHOOSER,
                                   IS_PARAM_DEFINED(X360APPCHOOSER)
                                       ? params[X360APPCHOOSER]->GetIntValue()
                                       : 0);
    vsh_forgot_to_set_static_X360APPCHOOSER =
        0; // This is a dirty workaround to the shortcut [= 0] in the fxc
  ...
  DYNAMIC_STATE {
    ...
    if (params[BASETEXTURE]->IsDefined()) {
      ...
      float vPixelSize[4] = {1.0f / pTarget->GetActualWidth(),
                             1.0f / pTarget->GetActualHeight(), 0.0f, 0.0f};
    ...
    float c0[] = {params[C0_X]->GetFloatValue(), params[C0_Y]->GetFloatValue(),
                  params[C0_Z]->GetFloatValue(), params[C0_W]->GetFloatValue(),
                  params[C1_X]->GetFloatValue(), params[C1_Y]->GetFloatValue(),
                  params[C1_Z]->GetFloatValue(), params[C1_W]->GetFloatValue(),
                  params[C2_X]->GetFloatValue(), params[C2_Y]->GetFloatValue(),
                  params[C2_Z]->GetFloatValue(), params[C2_W]->GetFloatValue(),
                  params[C3_X]->GetFloatValue(), params[C3_Y]->GetFloatValue(),
                  params[C3_Z]->GetFloatValue(), params[C3_W]->GetFloatValue()};
