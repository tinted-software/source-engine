...
float DoShadowNvidiaPCF3x3Box(sampler DepthSampler, const float4 shadowMapPos) {
  ...
  vOneTaps.w =
      tex2Dproj(DepthSampler,
          ...
          .x;
  float flOneTaps =
      dot(vOneTaps, float4(1.0f / 9.0f, 1.0f / 9.0f, 1.0f / 9.0f, 1.0f / 9.0f));

  float4 vTwoTaps;
  vTwoTaps.x =
      tex2Dproj(DepthSampler,
                float4(shadowMapCenter + float2(fTexelEpsilon, 0), objDepth, 1))
          .x;
  ...
  vTwoTaps.w = tex2Dproj(DepthSampler,
                   ...
                   .x;
  float flTwoTaps =
      dot(vTwoTaps, float4(1.0f / 9.0f, 1.0f / 9.0f, 1.0f / 9.0f, 1.0f / 9.0f));
...
float DoShadowNvidiaPCF5x5Gaussian(sampler DepthSampler,
  ...
  vSevenTaps.z =
      tex2Dproj(DepthSampler,
                float4(shadowMapCenter + float2(0, -fTwoEpsilon), objDepth, 1))
          .x;
  vSevenTaps.w =
      tex2Dproj(DepthSampler,
                float4(shadowMapCenter + float2(0, -fTwoEpsilon), objDepth, 1))
  ...
  vFourTapsA.x =
      tex2Dproj(
          DepthSampler,
          float4(shadowMapCenter + float2(fTwoEpsilon, fEpsilon), objDepth, 1))
          .x;
  vFourTapsA.y =
      tex2Dproj(
          DepthSampler,
          float4(shadowMapCenter + float2(fEpsilon, fTwoEpsilon), objDepth, 1))
          .x;
  vFourTapsA.z =
      tex2Dproj(
          DepthSampler,
...
float DoShadowATICheap(sampler DepthSampler, const float4 shadowMapPos) {
  ...
  objDepth =
      min(objDepth, 0.99999); // HACKHACK: On 360, surfaces at or past the far
...
float DoShadowPoisson16Sample(sampler DepthSampler,
  ...
  RMatTop *= flScaleOverMapSize; // Scale up kernel while accounting for texture
                                 // resolution
  ...
  if (bNvidiaHardwarePCF) {
    ...
    TODO: Fix this contact hardening stuff
                    ...
                    for( int j=0; j<8; j++ )
                    {
                            rotOffset.x = dot (RMatTop.xy, vPoissonOffset[j].xy)
    + RMatTop.z; rotOffset.y = dot (RMatBottom.xy, vPoissonOffset[j].xy) +
    RMatBottom.z; vBlockerDepths = tex2D( DepthSampler, rotOffset.xy );
    ...
    receiver? flAccumulatedCloserSamples += dot (vCloserSamples, vBlockerDepths
    );	// Total depths from samples closer than receiver
                    }
    ...
    */
    ...
    fResult = dot(
        accum, float4(1.0f / 32.0f, 1.0f / 32.0f, 1.0f / 32.0f, 1.0f / 32.0f));
...
float DoShadow360Simple(sampler DepthSampler, const float3 vProjCoords) {
  ...
  vWeights =
      float4((1 - vWeights.x) * (1 - vWeights.y), vWeights.x * (1 - vWeights.y),
...
float AmountShadowed_8Tap_360(sampler DepthSampler, float2 tc, float objDepth) {
  ...
  // Optimal 8 rooks pattern to get an idea about whether we're at a penumbra or
  // not From [Kallio07] "Scanline Edge-Flag Algorithm for Antialiasing"
  //
  //        +---+---+---+---+---+---+---+---+
  //        |   |   |   |   |   | o |   |   |
  //        +---+---+---+---+---+---+---+---+
...
float DoFlashlightShadow(sampler DepthSampler, sampler RandomRotationSampler,
  ...
  if (bAllowHighQuality) {
    ...
    if (g_bHighQualityShadows) {
      flShadow = DoShadowPoisson360(DepthSampler, RandomRotationSampler,
                                    vProjCoords, vScreenPos, vShadowTweaks);
    } else {
...
void DoSpecularFlashlight(
  ...
  if (bDoShadows) {
    float flShadow = DoFlashlightShadow(
        FlashlightDepthSampler, RandomRotationSampler, vProjCoords, vScreenPos,
        nShadowLevel, vShadowTweaks, bAllowHighQuality);
    float flAttenuated = lerp(
        flShadow, 1.0f,
        vShadowTweaks.y); // Blend between fully attenuated and not attenuated
...
float3
DoFlashlight(float3 flashlightPos, float3 worldPos,
             float4 flashlightSpacePosition, float3 worldNormal,
             float3 attenuationFactors, float farZ, sampler FlashlightSampler,
             sampler FlashlightDepthSampler, sampler RandomRotationSampler,
