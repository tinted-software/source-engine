...
void InitParamsVertexLitGeneric_DX9(CBaseVSShader *pShader,
  ...
  if ((info.m_nBlendTintByBaseAlpha != -1) &&
      (!params[info.m_nBlendTintByBaseAlpha]->IsDefined())) {
  ...
  if (IS_FLAG_SET(MATERIAL_VAR_BASEALPHAENVMAPMASK) && info.m_nBumpmap != -1 &&
    ...
    Warning("material %s has a normal map and $basealphaenvmapmask.  Must use "
  ...
  if (info.m_nEnvmapMask != -1 && params[info.m_nEnvmapMask]->IsDefined() &&
    ...
    params[info.m_nEnvmapMask]->SetUndefined();
    if (!hasNormalMapAlphaEnvmapMask) {
      Warning("material %s has a normal map and an envmapmask.  Must use "
...
//-----------------------------------------------------------------------------
static void
DrawVertexLitGeneric_DX9_Internal(CBaseVSShader *pShader, IMaterialVar **params,
...

{
  ...
  int nDetailBlendMode =
      bHasDetailTexture ? GetIntParam(info.m_nDetailTextureCombineMode, params)
                        : 0;
  ...
  float fTintReplaceFactor =
      GetFloatParam(info.m_nTintReplacesBaseColor, params, 0.0);

  BlendType_t nBlendType;
  bool bHasBaseTexture = IsTextureSet(info.m_nBaseTexture, params);
  if (bHasBaseTexture) {
    // if base alpha is used for tinting, ignore the base texture for computing
    // translucency
    nBlendType = pShader->EvaluateBlendRequirements(
  ...
  if (pShader->IsSnapshotting() || (!pContextData) ||
    ...
    /*^*/ // 	printf("\t\t[1] snapshotting=%d  pContextData=%08x
          // pContextData->m_bMaterialVarsChanged=%d
    ...
    bool bHasSelfIllumFresnel =
        (!IsTextureSet(info.m_nDetail, params)) && (bHasSelfIllum) &&
        (info.m_nSelfIllumFresnel != -1) &&
        (params[info.m_nSelfIllumFresnel]->GetIntValue() != 0);
    ...
    if (pShader->IsSnapshotting()) {
      ...
      bool bHasEnvmap = (!bHasFlashlight || IsX360()) &&
                        (info.m_nEnvmap != -1) &&
                        params[info.m_nEnvmap]->IsTexture();
      bool bHasLegacyEnvSphereMap =
          bHasEnvmap && IS_FLAG_SET(MATERIAL_VAR_ENVMAPSPHERE);
      ...
      if (bHasFlashlight) {
        if (g_pHardwareConfig->SupportsPixelShaders_2_b()) {
          nShadowFilterMode =
              g_pHardwareConfig
                  ->GetShadowFilterMode(); // Based upon vendor and device
                                           // dependent formats
        }
      ...
      if (bHasBaseTexture) {
        ...
        // If we're on OSX GL on a crappy OS which can't do sRGB from render
        // targets, check to see if we're reading from one...
        if (IsOSX() && !g_pHardwareConfig->CanDoSRGBReadFromRTs()) {
          ITexture *pBaseTexture =
      ...
      /*^*/ // 		printf("\t\t      ->
            // CShaderShadowDX8::VertexShaderVertexFormat( flags=%08x,
      ...
      if (bHasBump || bHasDiffuseWarp) {
...
#endif
        {
          ...
          if (g_pHardwareConfig->SupportsPixelShaders_2_b() ||
              g_pHardwareConfig
                  ->ShouldAlwaysUseShaderModel2bShaders()) // Always send GL
                                                           // this way
          {
      ...
      } else // !(bHasBump || bHasDiffuseWarp)
      {
...
#endif
        {
          ...
          SET_STATIC_VERTEX_SHADER_COMBO(USE_STATIC_CONTROL_FLOW,
                                         bUseStaticControlFlow);
          SET_STATIC_VERTEX_SHADER_COMBO(DONT_GAMMA_CONVERT_VERTEX_COLOR,
                                         (!bSRGBWrite) && bHasVertexColor);
                                                           ...
                                                           // this way
          {
            ...
            SET_STATIC_PIXEL_SHADER_COMBO(CUBEMAP, bHasEnvmap);
            SET_STATIC_PIXEL_SHADER_COMBO(CUBEMAP_SPHERE_LEGACY,
                                          bHasLegacyEnvSphereMap);
            SET_STATIC_PIXEL_SHADER_COMBO(DIFFUSELIGHTING, hasDiffuseLighting);
      ...
      // for underwater stuff and the loadout and character select screens.
      pShaderShadow->EnableAlphaWrites(bFullyOpaque);
    }

    if (pShaderAPI &&
        ((!pContextData) || (pContextData->m_bMaterialVarsChanged))) {
      /*^*/ // 		printf("\t\t[3] pShaderAPI && ( (! pContextData ) || (
            // pContextData->m_bMaterialVarsChanged ) )  TRUE \n");
  ...
  if (pShaderAPI) {
    ...
    if (bHasBump || bHasDiffuseWarp) {
...
#endif
      {
        ...
        if (g_pHardwareConfig->SupportsPixelShaders_2_b() ||
            g_pHardwareConfig
                ->ShouldAlwaysUseShaderModel2bShaders()) // Always send GL this
                                                         // way
        {
          ...
          //PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo() );
    ...
    } else // !( bHasBump || bHasDiffuseWarp )
    {
...
#endif
      {
        ...
        SET_DYNAMIC_VERTEX_SHADER_COMBO(
            STATIC_LIGHT, lightState.m_bStaticLightVertex ? 1 : 0);
        SET_DYNAMIC_VERTEX_SHADER_COMBO(DOWATERFOG, fogIndex);
        SET_DYNAMIC_VERTEX_SHADER_COMBO(SKINNING, numBones > 0);
        SET_DYNAMIC_VERTEX_SHADER_COMBO(
    ...
    float fWriteWaterFogToDestAlpha =
        (pShaderAPI->GetPixelFogCombo() == 1 && bWriteWaterFogToAlpha) ? 1 : 0;
...
void DrawVertexLitGeneric_DX9(CBaseVSShader *pShader, IMaterialVar **params,
  ...
  if (WantsSkinShader(params, info) &&
      g_pHardwareConfig->SupportsPixelShaders_2_b() &&
      g_pConfig->UseBumpmapping() && g_pConfig->UsePhong()) {
