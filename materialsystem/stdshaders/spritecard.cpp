...
int GetDefaultDepthFeatheringValue(void) // Allow the command-line to go against
                                         // the default soft-particle value
{
  ...
  // On low end parts on the Mac, we reduce particles and shut off depth
  // blending here
...
SHADER_PARAM(
    SEQUENCE_BLEND_MODE, SHADER_PARAM_TYPE_INTEGER, "0",
    "defines the blend mode between the images un dual sequence particles. 0 = "
    "avg, 1=alpha from first, rgb from 2nd, 2= first over second")
...
SHADER_INIT {
  ...
  if (params[BASETEXTURE]->IsDefined()) {
    ...
    LoadTexture(BASETEXTURE,
                !bExtractGreenAlpha && !bDX8 ? TEXTUREFLAGS_SRGB : 0);
  }
...
SHADER_DRAW {
  ...
  SHADOW_STATE {
    ...
    int numTexCoords = 4;
    if (true /* bAdditive2ndTexture */) // there is no branch for 2nd texture in
                                        // the VS! -henryg
    {
      numTexCoords = 5;
    }
  ...
  DYNAMIC_STATE {
    ...
    float flStartFade =
        max(1.f, flMaxDistance - params[FARFADEINTERVAL]->GetFloatValue());
    ...
    if (bDX8 &&
        (!bAdditive2ndTexture)) // deal with 0..1 limit for pix shader constants
    {
      C0[2] *= 0.25;
      C0[1] *= 0.25;
    }
