...
SHADER_PARAM(DUDVMAP, SHADER_PARAM_TYPE_TEXTURE,
             "models/shadertest/shader1_dudv", "dudv bump map")
SHADER_PARAM(NORMALMAP, SHADER_PARAM_TYPE_TEXTURE,
             "models/shadertest/shader1_normal", "normal map")
SHADER_PARAM(BUMPFRAME, SHADER_PARAM_TYPE_INTEGER, "0",
             "frame number for $bumpmap")
SHADER_PARAM(DUDVFRAME, SHADER_PARAM_TYPE_INTEGER, "0",
             "frame number for $dudvmap")
...
SHADER_PARAM(BLURAMOUNT, SHADER_PARAM_TYPE_INTEGER, "1",
...
SHADER_PARAM(FALLBACK, SHADER_PARAM_TYPE_STRING, "",
...
SHADER_PARAM(NOWRITEZ, SHADER_PARAM_TYPE_INTEGER, "0",
...
SHADER_FALLBACK {
  if (IsPC()) {
    const char *pFallback = (params && params[FALLBACK]->IsDefined())
                                ? params[FALLBACK]->GetStringValue()
                                : "";
    if (!pFallback[0]) {
    ...
    if (g_pHardwareConfig->PreferReducedFillrate() &&
        (params && (params[FORCEREFRACT]->GetIntValue() == 0)))
      return pFallback;
  }
...
SHADER_DRAW {
  ...
  SHADOW_STATE {
    ...
    if (bMasked) {
      EnableAlphaBlending(SHADER_BLEND_ONE_MINUS_SRC_ALPHA,
  ...
  if (bHasEnvmap) {
    ...
    if (bIsModel) {
      DrawModelBumpedSpecularLighting(NORMALMAP, BUMPFRAME, ENVMAP, ENVMAPFRAME,
                                      ENVMAPTINT, ALPHA, ENVMAPCONTRAST,
                                      ENVMAPSATURATION, BUMPTRANSFORM,
                                      bBlendSpecular, bNoWriteZ);
    } else {
      DrawWorldBumpedSpecularLighting(
          NORMALMAP, ENVMAP, BUMPFRAME, ENVMAPFRAME, ENVMAPTINT, ALPHA,
