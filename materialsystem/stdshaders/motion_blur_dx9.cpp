...
ConVar mat_motion_blur_percent_of_screen_max(
...
BEGIN_SHADER_PARAMS
SHADER_PARAM(MOTIONBLURINTERNAL, SHADER_PARAM_TYPE_VEC4, "[0 0 0 0]",
             "Internal motion blur value set by proxy")
END_SHADER_PARAMS
...
SHADER_DRAW {
  SHADOW_STATE {
    ...
    // On OpenGL OSX, we must do sRGB reads and writes since these render
    // targets are tagged as such
  ...
  DYNAMIC_STATE {
    ...
    pShaderAPI->SetPixelShaderConstant(
        1, params[MOTIONBLURINTERNAL]->GetVecValue(), 1);
    ...
    if (fabs(params[MOTIONBLURINTERNAL]->GetVecValue()[0]) +
            fabs(params[MOTIONBLURINTERNAL]->GetVecValue()[1]) +
