...
float4 DecompressNormal(sampler NormalSampler, float2 tc,
  ...
  } else // ATI2N plus ATI1N for alpha
  {
    ...
    result.z = sqrt(1.0f - dot(result.xy, result.xy));
    result.a =
        tex2D(AlphaSampler, tc).x; // Note that this comes in on the X channel
  }
...
float CalcWaterFogAlpha(const float flWaterZ, const float flEyePosZ,
  ...
  // above the water surface We'll do this with the saturate at the end instead.
...
float CalcRangeFog(const float flProjPosZ, const float flFogStartOverRange,
  ...
  return saturate(
      min(flFogMaxDensity, (flProjPosZ * flFogOORange) - flFogStartOverRange));
#else
...
float CalcPixelFogFactor(int iPIXELFOGTYPE, const float4 fogParams,
  ...
  if (iPIXELFOGTYPE ==
      PIXEL_FOG_TYPE_RANGE) // range fog, or no fog depending on fog parameters
  {
    retVal = CalcRangeFog(flProjPosZ, fogParams.x, fogParams.z, fogParams.w);
  } else if (iPIXELFOGTYPE == PIXEL_FOG_TYPE_HEIGHT) // height fog
  {
    retVal = CalcWaterFogAlpha(fogParams.y, flEyePosZ, flWorldPosZ, flProjPosZ,
                               fogParams.w);
  }
...
float3 SRGBOutput(const float3 vShaderColor) {
  // On ps2b capable hardware we always have the linear->gamma conversion table
  // texture in sampler s15.
  float3 result;
...
float3 SRGBOutput(const float3 vShaderColor) {
  return vShaderColor; // ps 1.1, 1.4, and 2.0 never do srgb conversion in the
                       // pixel shader
}
...
LPREVIEW_PS_OUT FinalOutput(const LPREVIEW_PS_OUT vShaderColor,
  ...
  result.color = FinalOutput(vShaderColor.color, pixelFogFactor, iPIXELFOGTYPE,
                             iTONEMAP_SCALE_TYPE);
...
//vViewTS.z * vViewTS.z ) / vViewTS.z; 			Out.vParallax = vParallaxDirection *
...
//
//		vViewW:
//			float3 vViewW = /*normalize*/(mul( matViewInverse,
//float4( 0, 0, 0, 1)) - inPosition );
//
// OUTPUT:
...
#define TCOMBINE_SSBUMP_NOBUMP                                                 \
  11 // detail is an ssbump but use it as an albedo. shader does the magic here
     // - no user needs to specify mode 11
...
float3 TextureCombinePostLighting(float3 lit_baseColor, float4 detailColor,
  ...
  if (combine_mode == TCOMBINE_RGB_ADDITIVE_SELFILLUM_THRESHOLD_FADE) {
    // fade in an unusual way - instead of fading out color, remap an increasing
    // band of it from 0..1
    // if (fBlendFactor > 0.5)
...
#define flSpriteDepth flDepths.y
...
#if (defined(_X360))
    {
      // Get depth from the depth texture. Need to sample with the offset of
      // (0.5, 0.5) to fix rounding errors
      asm {
...
#else
    {
      ...
      flFeatheredAlpha =
          max(smoothstep(0.75f, 1.0f, flSceneDepth),
              flFeatheredAlpha); // as the sprite approaches the edge of our
                                 // compressed depth space, the math stops
                                 // working. So as the sprite approaches the far
                                 // depth, smoothly remove feathering.
      flFeatheredAlpha = saturate(flFeatheredAlpha);
    }
#endif
