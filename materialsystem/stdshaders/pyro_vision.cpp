...
SHADER_PARAM(SELFILLUMTINT, SHADER_PARAM_TYPE_COLOR, "[ 1.0 1.0 1.0 ]",
...
SHADER_INIT_PARAMS() {
  ...
  SET_PARAM_VEC_IF_NOT_DEFINED(STRIPE_FADE_NORMAL1, kDefaultStripeFadeNormal1,
                               3);
  SET_PARAM_VEC_IF_NOT_DEFINED(STRIPE_FADE_NORMAL2, kDefaultStripeFadeNormal2,
                               3);
...
SHADER_DRAW {
  CPyroVision_DX9_Context *pContextData =
      reinterpret_cast<CPyroVision_DX9_Context *>(*pContextDataPtr);
  ...
  bool bHasBaseTexture2 = (params[BASETEXTURE2]->IsDefined()) &&
                          (params[BASETEXTURE2]->IsTexture());
  bool bHasBlendModulateTexture = bHasBaseTexture2 &&
                                  (params[BLENDMODULATETEXTURE]->IsDefined()) &&
                                  (params[BLENDMODULATETEXTURE]->IsTexture());
  bool bHasHeatHaze =
      (params[WARPTEXTURE]->IsDefined()) && (params[WARPTEXTURE]->IsTexture());
  ...
  bool bHasColorbar =
      (params[COLORBAR]->IsDefined()) && (params[COLORBAR]->IsTexture());
  bool bHasStripes = (params[STRIPETEXTURE]->IsDefined()) &&
                     (params[STRIPETEXTURE]->IsTexture());
  ...
  if (pShaderShadow || bNeedRegenStaticCmds) {
    ...
    staticCmdsBuf.SetVertexShaderConstant(VERTEX_SHADER_SHADER_SPECIFIC_CONST_0,
                                          vParms.Base());
    ...
    if (bHasBlendModulateTexture) {
      staticCmdsBuf.BindTexture(this, SHADER_SAMPLER3, BLENDMODULATETEXTURE,
                                -1);
    ...
    case 0: {
      ...
      staticCmdsBuf.SetVertexShaderConstant(
          VERTEX_SHADER_SHADER_SPECIFIC_CONST_1, vParms.Base());
  ...
  if (pShaderAPI && pContextData->m_bMaterialVarsChanged) {
    ...
    //			vParms.x =
    //pShaderAPI->LinearToGamma_HardwareSpecific( 1.0f );
    ...
    case 0: {
      ...
      vParms[1] *= flLScale;
      vParms[2] *= flLScale;
      pContextData->m_SemiStaticCmdsOut.SetPixelShaderConstant(1,
                                                               vParms.Base());
    } break;
