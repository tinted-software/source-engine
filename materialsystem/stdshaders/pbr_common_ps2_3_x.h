...
// Monte Carlo integration, approximate analytic version based on Dimitar
// Lazarov's work
...
// This expects DirectX normal maps in Mikk Tangent Space
// http://www.mikktspace.com
float3x3 compute_tangent_frame(float3 N, float3 P, float2 uv, out float3 T,
                               out float3 B, out float sign_det) {
  float3 dp1 = ddx(P);
  float3 dp2 = ddy(P);
  float2 duv1 = ddx(uv);
  float2 duv2 = ddy(uv);
...
float3 calculateLight(float3 lightIn, float3 lightIntensity, float3 lightOut,
                      float3 normal, float3 fresnelReflectance, float roughness,
                      float metalness, float lightDirectionAngle,
                      float3 albedo) {
  // Lh
  ...
  float3 F =
      fresnelSchlick(fresnelReflectance, max(0.0, dot(HalfAngle, lightOut)));
  ...
  // dielectric medium Metals on the other hand either reflect or absorb energso
  // diffuse contribution is always, zero To be energy conserving we must scale
  // diffuse BRDF contribution based on Fresnel factor & metalness
#if SPECULAR
  ...
  float3 specularBRDF =
      (F * D * G) / max(EPSILON, 4.0 * cosLightIn * lightDirectionAngle);
#if LIGHTMAPPED && !FLASHLIGHT
  // Ambient light from static lights is already precomputed in the lightmap.
  // Don't add it again
...
float3 ambientLookupLightmap(float3 normal, float3 EnvAmbientCube[6],
                             float3 textureNormal, float4 lightmapTexCoord1And2,
                             float4 lightmapTexCoord3, sampler LightmapSampler,
...
float3 ambientLookup(float3 normal, float3 EnvAmbientCube[6],
                     ...
                     float4 g_DiffuseModulation) {
#if LIGHTMAPPED
  return ambientLookupLightmap(normal, EnvAmbientCube, textureNormal,
                               lightmapTexCoord1And2, lightmapTexCoord3,
                               LightmapSampler, g_DiffuseModulation);
#else
...
void setupEnvMapAmbientCube(out float3 EnvAmbientCube[6],
  ...
  float4 directionPosX = {1, 0, 0, 12};
  float4 directionNegX = {-1, 0, 0, 12};
  float4 directionPosY = {0, 1, 0, 12};
  float4 directionNegY = {0, -1, 0, 12};
  float4 directionPosZ = {0, 0, 1, 12};
  float4 directionNegZ = {0, 0, -1, 12};
  EnvAmbientCube[0] =
      ENV_MAP_SCALE * texCUBElod(EnvmapSampler, directionPosX).rgb;
  EnvAmbientCube[1] =
      ENV_MAP_SCALE * texCUBElod(EnvmapSampler, directionNegX).rgb;
  EnvAmbientCube[2] =
      ENV_MAP_SCALE * texCUBElod(EnvmapSampler, directionPosY).rgb;
  EnvAmbientCube[3] =
      ENV_MAP_SCALE * texCUBElod(EnvmapSampler, directionNegY).rgb;
  EnvAmbientCube[4] =
      ENV_MAP_SCALE * texCUBElod(EnvmapSampler, directionPosZ).rgb;
  EnvAmbientCube[5] =
      ENV_MAP_SCALE * texCUBElod(EnvmapSampler, directionNegZ).rgb;
}

#if PARALLAXOCCLUSION
float2 parallaxCorrect(float2 texCoord, float3 viewRelativeDir,
                       sampler depthMap, float parallaxDepth,
                       float parallaxCenter) {
  float fLength = length(viewRelativeDir);
  float fParallaxLength =
      sqrt(fLength * fLength - viewRelativeDir.z * viewRelativeDir.z) /
      viewRelativeDir.z;
  ...
  while (nStepIndex < nNumSteps) {
    ...
    // Sample height map which in this case is stored in the alpha channel of
    // the normal map:
    fCurrHeight =
        parallaxCenter + tex2Dgrad(depthMap, vTexCurrentOffset, dx, dy).a;
  ...
  // The computed texture offset for the displaced point on the pseudo-extruded
  // surface:
...
float3 worldToRelative(float3 worldVector, float3 surfTangent, float3 surfBasis,
                       float3 surfNormal) {
  return float3(dot(worldVector, surfTangent), dot(worldVector, surfBasis),
