...
static void DrawCompositorStage_ps20(CBaseVSShader *pShader,
                                     IMaterialVar **params,
                                     IShaderShadow *pShaderShadow,
                                     IShaderDynamicAPI *pShaderAPI,
                                     VertexCompressionType_t vertexCompression,
                                     const CompositorInfo_t &info);
static void DrawCompositorStage_ps20b(CBaseVSShader *pShader,
                                      IMaterialVar **params,
                                      IShaderShadow *pShaderShadow,
                                      IShaderDynamicAPI *pShaderAPI,
                                      VertexCompressionType_t vertexCompression,
...
SHADER_PARAM(COMBINE_MODE, SHADER_PARAM_TYPE_INTEGER, "",
             "Which mode should the combiner operate in? 0: Mul, 1: Add, 2: "
             "Lerp, 3: Select")
...
SHADER_DRAW {
  ...
  if (g_pHardwareConfig->SupportsPixelShaders_2_b())
    DrawCompositorStage_ps20b(this, params, pShaderShadow, pShaderAPI,
                              vertexCompression, info);
  else
    DrawCompositorStage_ps20(this, params, pShaderShadow, pShaderAPI,
                             vertexCompression, info);
}
END_SHADER

static void DrawCompositorStage_common(
    CBaseVSShader *pShader, IMaterialVar **params, IShaderShadow *pShaderShadow,
  ...
  SHADOW_STATE {
    ...
    // In 2.0b shaders, we use the skin shader and alpha carries specular mask
    // information-- so we need to write it But with 2.0 shaders used by at
    // least one customer in 2015, we fall back to vertexlitgeneric and it
    // doesn't do the same transforms as skin to the alpha shader Since it won't
    // be used anyways and it breaks the item model panel icons to write alpha
  ...
  DYNAMIC_STATE {
    ...
    float f4TextureCountThisPass[] = {(float)textureCountThisPass, 0.0f, 0.0f,
                                      0.0f};
    ...
    if (nCombineMode == ECO_Select) {
      ...
      float selectors[] = {cFac * GetIntParam(info.m_nSelector[0], params),
...
static void DrawCompositorStage_ps20_muladdblend(
    CBaseVSShader *pShader, IMaterialVar **params, IShaderShadow *pShaderShadow,
    IShaderDynamicAPI *pShaderAPI, VertexCompressionType_t vertexCompression,
    const CompositorInfo_t &info) {
  int nCombineMode = params[info.m_nCombineMode]->GetIntValue();
  ...
  DYNAMIC_STATE {
    ...
    if (textureCountThisPass > 0)
      pShader->SetPixelShaderConstantGammaToLinear(2,
                                                   info.m_nTexAdjustLevels[0]);
    ...
    if (textureCountThisPass > 1)
      pShader->SetPixelShaderConstantGammaToLinear(3,
                                                   info.m_nTexAdjustLevels[1]);
...
static void DrawCompositorStage_ps20_lerp(
    CBaseVSShader *pShader, IMaterialVar **params, IShaderShadow *pShaderShadow,
    IShaderDynamicAPI *pShaderAPI, VertexCompressionType_t vertexCompression,
    const CompositorInfo_t &info) {
  SHADOW_STATE {
...
// Helper function when using ps20 and performing lerp operations.
static void DrawCompositorStage_ps20_select(
    CBaseVSShader *pShader, IMaterialVar **params, IShaderShadow *pShaderShadow,
    IShaderDynamicAPI *pShaderAPI, VertexCompressionType_t vertexCompression,
    const CompositorInfo_t &info) {
  int nCombineMode = params[info.m_nCombineMode]->GetIntValue();
...
static void DrawCompositorStage_ps20(CBaseVSShader *pShader,
                                     ...
                                     VertexCompressionType_t vertexCompression,
                                     const CompositorInfo_t &info) {
  DrawCompositorStage_common(pShader, params, pShaderShadow, pShaderAPI,
                             vertexCompression, info);
  ...
  case ECO_Add:
  case ECO_Blend:
    DrawCompositorStage_ps20_muladdblend(pShader, params, pShaderShadow,
                                         pShaderAPI, vertexCompression, info);
    return;
  case ECO_Lerp:
    DrawCompositorStage_ps20_lerp(pShader, params, pShaderShadow, pShaderAPI,
                                  vertexCompression, info);
    return;
  case ECO_Select:
    DrawCompositorStage_ps20_select(pShader, params, pShaderShadow, pShaderAPI,
                                    vertexCompression, info);
    return;
  default:
    Assert(!"Need to update DrawCompositoreStage_ps20 with how to draw this "
            "mode.");
    break;
  }
}
...
#ifdef STAGING_ONLY
    SET_DYNAMIC_PIXEL_SHADER_COMBO(DEBUG_MODE,
                                   r_texcomp_debug_stickers.GetBool() ? 1 : 0);
#else
    SET_DYNAMIC_PIXEL_SHADER_COMBO(DEBUG_MODE, 0);
#endif
