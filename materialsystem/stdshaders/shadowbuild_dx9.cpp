...
BEGIN_VS_SHADER_FLAGS(ShadowBuild_DX9, "Help for ShadowBuild",
                      SHADER_NOT_EDITABLE)
...
SHADER_DRAW {
  SHADOW_STATE {
    ...
    // base texture.  We just use this for alpha, but enable SRGB read to make
  ...
  DYNAMIC_STATE {
    ...
    if (pTexture) {
      BindTexture(SHADER_SAMPLER0, pTexture,
                  ppTranslucentParams[FRAME]->GetIntValue());

      Vector4D transformation[2];
      const VMatrix &mat =
          ppTranslucentParams[BASETEXTURETRANSFORM]->GetMatrixValue();
