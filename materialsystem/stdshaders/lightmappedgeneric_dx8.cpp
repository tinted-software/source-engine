...
SHADER_INIT_PARAMS() {
  ...
  if (IsUsingGraphics() && params[ENVMAP]->IsDefined() &&
    ...
    if (stricmp(params[ENVMAP]->GetStringValue(), "env_cubemap") == 0) {
      Warning("env_cubemap used on world geometry without rebuilding map. . "
              "ignoring: %s\n",
              pMaterialName);
  ...
  if (params[SEAMLESS_SCALE]->IsDefined() &&
    ...
    if (params[BUMPMAP]->IsDefined()) {
      Warning("Can't use $bumpmap with $seamless_scale for "
              "lightmappedgeneric_dx8.  Implicitly disabling $bumpmap: %s\n",
              pMaterialName);
      params[BUMPMAP]->SetUndefined();
    }
    if (params[ENVMAP]->IsDefined()) {
      Warning("Can't use $envmap with $seamless_scale for "
              "lightmappedgeneric_dx8. Implicitly disabling $envmap: %s\n",
              pMaterialName);
      params[ENVMAP]->SetUndefined();
    }
  }
...
inline const char *
GetAdditiveEnvmapPixelShaderName(bool usingMask, bool usingBaseTexture,
