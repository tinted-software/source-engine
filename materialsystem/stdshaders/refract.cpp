...
SHADER_PARAM(NORMALMAP2, SHADER_PARAM_TYPE_TEXTURE,
             "models/shadertest/shader1_normal", "normal map")
...
SHADER_PARAM(VERTEXCOLORMODULATE, SHADER_PARAM_TYPE_BOOL, "0",
             "Use the vertex color to effect refract color. alpha will adjust "
             "refract amount")
...
SHADER_DRAW {
  ...
  if ((pShaderShadow != NULL) || (bHasFlashlight == false)) {
    DrawRefract_DX9(this, params, pShaderAPI, pShaderShadow, info,
                    vertexCompression);
  } else {
    Draw(false);
  }
