...
float3 PixelShaderGetLightVector(const float3 worldPos,
  ...
  if (nLightIndex == 3) {
    ...
    float3 vLight3Pos =
        float3(cLightInfo[1].pos.w, cLightInfo[2].color.w, cLightInfo[2].pos.w);
...
void SpecularAndRimTerms(const float3 vWorldNormal, const float3 vLightDir,
  ...
  if (bDoSpecularWarp)
    specularLighting *= tex2D(
        specularWarpSampler,
        float2(specularLighting.x, fFresnel)); // Sample at { (L.R)^k, fresnel }
...
float Fresnel(const float3 vNormal, const float3 vEyeDir, float3 vRanges) {
  // float result, f = Fresnel( vNormal, vEyeDir );			//
  // Traditional Fresnel if ( f > 0.5f ) 	result = lerp( vRanges.y, vRanges.z,
  //(2*f)-1 );		// Blend between mid and high values else 	result = lerp(
  //vRanges.x, vRanges.y, 2*f );			// Blend between low and
  //mid values

  // note: vRanges is now encoded as ((mid-min)*2, mid, (max-mid)*2) to optimize
  // math
...
float3 PixelShaderDoLightingLinear(
  ...
  if (bStaticLight) {
    ...
    // premultiplied by $cOOOverbright need to get it into linear space so that
    // we can do adds.
      ...
      if (nNumLights > 2) {
        ...
        if (nNumLights > 3) {
          ...
          float3 vLight3Pos = float3(cLightInfo[1].pos.w, cLightInfo[2].color.w,
                                     cLightInfo[2].pos.w);
          linearColor += PixelShaderDoGeneralDiffuseLight(
              lightAtten.w, worldPos, worldNormal, NormalizeSampler, vLight3Pos,
              vLight3Color, bHalfLambert, bDoAmbientOcclusion,
              fAmbientOcclusion, bDoLightingWarp, lightWarpSampler);
        }
      }
    }
...
void PixelShaderDoSpecularLighting(
  ...
  if (nNumLights > 0) {
    PixelShaderDoSpecularLight(
        ...
        PixelShaderGetLightVector(worldPos, cLightInfo, 0), bDoAmbientOcclusion,
        fAmbientOcclusion, bDoSpecularWarp, specularWarpSampler, fFresnel,
  ...
  if (nNumLights > 1) {
    PixelShaderDoSpecularLight(
        ...
        PixelShaderGetLightVector(worldPos, cLightInfo, 1), bDoAmbientOcclusion,
        fAmbientOcclusion, bDoSpecularWarp, specularWarpSampler, fFresnel,
        bDoRimLighting, fRimExponent, localSpecularTerm, localRimTerm);
  ...
  if (nNumLights > 2) {
    PixelShaderDoSpecularLight(
        ...
        PixelShaderGetLightVector(worldPos, cLightInfo, 2), bDoAmbientOcclusion,
        fAmbientOcclusion, bDoSpecularWarp, specularWarpSampler, fFresnel,
        bDoRimLighting, fRimExponent, localSpecularTerm, localRimTerm);
  ...
  if (nNumLights > 3) {
    PixelShaderDoSpecularLight(
        ...
        PixelShaderGetLightVector(worldPos, cLightInfo, 3), bDoAmbientOcclusion,
        fAmbientOcclusion, bDoSpecularWarp, specularWarpSampler, fFresnel,
        bDoRimLighting, fRimExponent, localSpecularTerm, localRimTerm);
...
// shaders
float3
PixelShaderDoLighting(const float3 worldPos, const float3 worldNormal,
                      const float3 staticLightingColor, const bool bStaticLight,
                      const bool bAmbientLight, const float4 lightAtten,
                      const float3 cAmbientCube[6], in sampler NormalizeSampler,
                      const int nNumLights, PixelShaderLightInfo cLightInfo[3],
  ...
  //		linearColor = HuePreservingColorClamp( linearColor,
  //pow( 2.0f, 2.2 ) );
