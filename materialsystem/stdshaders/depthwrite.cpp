...
SHADER_DRAW {
  ...
  SHADOW_STATE {
    ...
    if (nColorDepth == 0) {
      // Bias primitives when rendering into shadow map so we get slope-scaled
      // depth bias rather than having to apply a constant bias in the filtering
      // shader later
      pShaderShadow->EnablePolyOffset(SHADER_POLYOFFSET_SHADOW_BIAS);
    }

    // Turn off writes to color buffer since we always sample shadows from the
    // DEPTH texture later This gives us double-speed fill when rendering INTO
    // the shadow map
    pShaderShadow->EnableColorWrites((nColorDepth == 1));
    pShaderShadow->EnableAlphaWrites(false);

    // Don't backface cull unless alpha clipping, since this can cause artifacts
    // when the geometry is clipped by the flashlight near plane If a material
    // was already marked nocull, don't cull it
    pShaderShadow->EnableCulling(IS_FLAG_SET(MATERIAL_VAR_ALPHATEST) &&
...
#endif
    {
      ...
      SET_STATIC_VERTEX_SHADER_COMBO(
          ONLY_PROJECT_POSITION,
          !bAlphaClip && IsX360() &&
              !nColorDepth); // 360 needs to know if it *shouldn't* output
                             // texture coordinates to avoid shader patches
      ...
      SET_STATIC_VERTEX_SHADER_COMBO(
          ONLY_PROJECT_POSITION, 0); // 360 only combo, and this is a PC path
      SET_STATIC_VERTEX_SHADER_COMBO(COLOR_DEPTH, nColorDepth);
  ...
  DYNAMIC_STATE {
...
#endif
    {
      ...
      if (bAlphaClip) {
        ...
        if (ALPHATESTREFERENCE != -1 &&
            (params[ALPHATESTREFERENCE]->GetFloatValue() > 0.0f)) {
          vAlphaThreshold[0] = vAlphaThreshold[1] = vAlphaThreshold[2] =
              vAlphaThreshold[3] = params[ALPHATESTREFERENCE]->GetFloatValue();
        }
