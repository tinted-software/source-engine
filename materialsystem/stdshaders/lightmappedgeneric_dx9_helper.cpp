...
void InitLightmappedGeneric_DX9(CBaseVSShader *pShader, IMaterialVar **params,
  ...
  if (params[info.m_nDetail]->IsDefined()) {
    ...
    pShader->LoadTexture(info.m_nDetail,
                         nDetailBlendMode != 0 ? TEXTUREFLAGS_SRGB : 0);
  }
...
void DrawLightmappedGeneric_DX9_Internal(
  ...
  if (pShaderShadow || (!pContextData) ||
    ...
    if (pShaderShadow || bNeedRegenStaticCmds) {
      ...
      if (pShaderShadow) {
        ...
        if (g_pHardwareConfig->SupportsPixelShaders_2_b()) {
          ...
          SET_STATIC_PIXEL_SHADER_COMBO(NORMALMASK_DECODE_MODE,
                                        0); // No normal compression with ps_2_0
    ...
    if (pShaderAPI && pContextData->m_bMaterialVarsChanged) {
      ...
      color[3] *= (IS_PARAM_DEFINED(info.m_nAlpha2) &&
                   params[info.m_nAlpha2]->GetFloatValue() > 0.0f)
  ...
  if (IsPC() && (IS_FLAG_SET(MATERIAL_VAR_ALPHATEST) != 0) &&
