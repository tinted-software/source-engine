...
void InitParamsPortalRefract(CBaseVSShader *pShader, IMaterialVar **params,
...
void InitPortalRefract(CBaseVSShader *pShader, IMaterialVar **params,
                       PortalRefractVars_t &info) {
  int nStage = IS_PARAM_DEFINED(info.m_nStage)
                   ? params[info.m_nStage]->GetIntValue()
                   : 0;
  ...
  if (nStage == 2) // Only load textures for third stage
  {
    if ((info.m_nPortalMaskTexture != -1) &&
        params[info.m_nPortalMaskTexture]->IsDefined()) {
      pShader->LoadTexture(info.m_nPortalMaskTexture);
    }

    if ((info.m_nPortalColorTexture != -1) &&
        params[info.m_nPortalColorTexture]->IsDefined()) {
...
void DrawPortalRefract(CBaseVSShader *pShader, IMaterialVar **params,
  ...
  SHADOW_STATE {
    ...
    // On Leopard / 10.5.8, we can't do an sRGB read from a render target, so we
    // must fake it in shader code
    bool bShaderSRGBRead =
        IsOSX() && !g_pHardwareConfig->CanDoSRGBReadFromRTs();

    // Pixel Shader
    if (g_pHardwareConfig->SupportsPixelShaders_2_b() ||
        g_pHardwareConfig
            ->ShouldAlwaysUseShaderModel2bShaders()) // Always send OpenGL /
                                                     // Posix down this path
    {
    ...
    pShaderShadow->EnableSRGBWrite(
        true); // Stage 1 doesn't actually need srgb conversion, but it skips
               // the software srgb solution anyway.
  ...
  DYNAMIC_STATE {
    ...
    if (IS_PARAM_DEFINED(info.m_nTextureTransform)) {
      pShader->SetVertexShaderTextureTransform(
          VERTEX_SHADER_SHADER_SPECIFIC_CONST_1, info.m_nTextureTransform);
    }
    ...
    vPackedVsConst1[0] -=
        (float)(floor(vPackedVsConst1[0] / 1000.0f)) * 1000.0f;
    ...
    vPackedConst4[0] = (IS_PARAM_DEFINED(info.m_nPortalOpenAmount)
                            ? params[info.m_nPortalOpenAmount]->GetFloatValue()
                            : kDefaultPortalOpenAmount);
    vPackedConst4[1] =
        1.0f - (IS_PARAM_DEFINED(info.m_nPortalStatic)
                    ? params[info.m_nPortalStatic]->GetFloatValue()
                    : kDefaultPortalStatic);
    vPackedConst4[2] = (IS_PARAM_DEFINED(info.m_nPortalColorScale)
                            ? params[info.m_nPortalColorScale]->GetFloatValue()
