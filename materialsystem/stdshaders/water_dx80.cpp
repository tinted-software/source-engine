...
inline void SetCheapWaterFactors(IMaterialVar **params,
...
inline void DrawReflection(IMaterialVar **params, IShaderShadow *pShaderShadow,
  ...
  DYNAMIC_STATE {
    ...
    // The dx9.0c runtime says that we shouldn't have a non-zero dimension when
    ...
    pShaderAPI->SetBumpEnvMatrix(SHADER_TEXTURE_STAGE1, fReflectionAmount, 0.0f,
                                 0.0f, fReflectionAmount);
...
inline void DrawRefraction(IMaterialVar **params, IShaderShadow *pShaderShadow,
  ...
  DYNAMIC_STATE {
    ...
    pShaderAPI->SetBumpEnvMatrix(SHADER_TEXTURE_STAGE1, fRefractionAmount, 0.0f,
                                 0.0f, fRefractionAmount);
    BindTexture(SHADER_SAMPLER0, BUMPMAP, BUMPFRAME);
...
inline void DrawRefractionForFresnel(IMaterialVar **params,
  ...
  DYNAMIC_STATE {
    ...
    BindTexture(SHADER_SAMPLER1, REFRACTTEXTURE);
    BindTexture(SHADER_SAMPLER2, NORMALMAP, BUMPFRAME);
    s_pShaderAPI->BindStandardTexture(SHADER_SAMPLER3,
                                      TEXTURE_NORMALIZATION_CUBEMAP);

    SetVertexShaderTextureTransform(VERTEX_SHADER_SHADER_SPECIFIC_CONST_1,
                                    BUMPTRANSFORM);
    SetCheapWaterFactors(params, pShaderAPI,
...
inline void DrawCheapWater(IMaterialVar **params, IShaderShadow *pShaderShadow,
                           ...
                           bool bBlendFresnel, bool bNoPerVertexFresnel) {
  SHADOW_STATE {
    SetInitialShadowState();
    ...
    if (bBlend) {
      if (bBlendFresnel) {
        EnableAlphaBlending(SHADER_BLEND_DST_ALPHA,
    ...
    if (bNoPerVertexFresnel) {
      ...
      pShaderShadow->SetVertexShader("WaterCheapPerVertexFresnel_vs11",
                                     vshIndex.GetIndex());
    }
...
SHADER_DRAW {
  ...
  bool bForceExpensive =
      !bForceCheap && (params[FORCEEXPENSIVE]->GetIntValue() != 0);
  ...
  if (!bForceCheap) {
    if (bRefraction) {
      ...
      if (g_pHardwareConfig->HasDestAlphaBuffer() && !bReflection &&
          !params[NOFRESNEL]->GetIntValue()) {
  ...
  if (!bReflection && params[ENVMAP]->IsTexture() &&
    ...
    bool bNoPerVertexFresnel =
        (params[NOFRESNEL]->GetIntValue() || bReflectionUseFresnel ||
         bForceCheap || !bRefraction);
    DrawCheapWater(params, pShaderShadow, pShaderAPI, bBlendReflection,
                   bReflectionUseFresnel, bNoPerVertexFresnel);
  }
}
END_SHADER
