...
SHADER_PARAM(ALBEDO, SHADER_PARAM_TYPE_TEXTURE,
             "shadertest/WorldTwoTextureBlend",
...
SHADER_INIT_PARAMS() {
  ...
  if (g_pConfig->UseBumpmapping() && params[BUMPMAP]->IsDefined() &&
      params[ALBEDO]->IsDefined() && params[BASETEXTURE]->IsDefined() &&
...
void DrawPass(IMaterialVar **params, IShaderDynamicAPI *pShaderAPI,
              IShaderShadow *pShaderShadow, bool hasFlashlight,
              VertexCompressionType_t vertexCompression) {
  bool hasBump = params[BUMPMAP]->IsTexture();
  bool hasDiffuseBumpmap =
      hasBump && (params[NODIFFUSEBUMPLIGHTING]->GetIntValue() == 0);
  ...
  bool bHasDetailAlpha =
      params[DETAIL_ALPHA_MASK_BASE_TEXTURE]->GetIntValue() != 0;
  ...
  SHADOW_STATE {
    ...
    //			if( hasLightmap )
    {
      ...
      pShaderShadow->EnableSRGBRead(
          SHADER_SAMPLER1, g_pHardwareConfig->GetHDRType() == HDR_TYPE_NONE);
    }
    if (hasFlashlight) {
    ...
    SET_STATIC_VERTEX_SHADER_COMBO(RELIEF_MAPPING,
                                   0); //( bumpmap_variant == 2 )?1:0);
    SET_STATIC_VERTEX_SHADER_COMBO(
        SEAMLESS, bSeamlessMapping); //( bumpmap_variant == 2 )?1:0);
#ifdef _X360
    ...
    if (g_pHardwareConfig->SupportsPixelShaders_2_b()) {
      ...
      SET_STATIC_PIXEL_SHADER_COMBO(DETAIL_ALPHA_MASK_BASE_TEXTURE,
                                    bHasDetailAlpha);
    ...
    // for underwater stuff. But only do it if we're not using the alpha already
    // for translucency
  ...
  DYNAMIC_STATE {
    ...
    if (hasFlashlight) {
      ...
      BindTexture(SHADER_SAMPLER2, state.m_pSpotlightTexture,
    ...
    if (!(bVertexShaderFastPath && color[0] == 1.0f && color[1] == 1.0f &&
...
SHADER_DRAW {
  ...
  DrawPass(params, pShaderAPI, pShaderShadow, bHasFlashlight,
           vertexCompression);
}
