...
SHADER_DRAW {
  SHADOW_STATE {
    ...
    // SRGB conversions hose the blend on some hardware, so keep we everything
    // in gamma space
    ...
    pShaderShadow->DisableFogGammaCorrection(
        true); // fog should stay exactly middle grey
    FogToGrey();
  ...
  DYNAMIC_STATE {
    if (pShaderAPI->InFlashlightMode() &&
        (!IsX360() && (r_flashlight_version2.GetInt() == 0))) {
...
#endif
    {
      ...
      bool bUnusedTexCoords[3] = {false, false,
                                  !pShaderAPI->IsHWMorphingEnabled()};
