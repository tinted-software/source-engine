...
BEGIN_VS_SHADER_FLAGS(Downsample_nohdr, "Help for Downsample_nohdr",
                      SHADER_NOT_EDITABLE)
...
SHADER_DRAW {
  SHADOW_STATE {
    ...
    // Render targets are pegged as sRGB on OSX, so just force these reads and
    // writes
    bool bForceSRGBReadAndWrite =
    ...
    if (g_pHardwareConfig->SupportsPixelShaders_2_b() ||
      ...
      SET_STATIC_PIXEL_SHADER_COMBO(CSTRIKE,
                                    params[CSTRIKE]->GetIntValue() ? 1 : 0);
#ifndef _X360
  ...
  DYNAMIC_STATE {
    ...
    float flPixelShaderParams[4] = {
        r_bloomtintr.GetFloat(), r_bloomtintg.GetFloat(),
        r_bloomtintb.GetFloat(), r_bloomtintexponent.GetFloat()};
