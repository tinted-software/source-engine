...
BEGIN_SHADER_FLAGS(DebugModifyVertex, "Help for DebugModifyVertex",
                   SHADER_NOT_EDITABLE)
BEGIN_SHADER_PARAMS
...
SHADER_DRAW {
  if (g_pHardwareConfig->GetSamplerCount() >= 2) {
    ...
    DYNAMIC_STATE {
      ...
      for (int i = 0; i < MeshBuilder()->NumVertices(); ++i) {
        ...
        MeshBuilder()->Position3f(
            pPos[0] + amp * sin(currTime + pPos[2] / 4),
    ...
    DYNAMIC_STATE {
      ...
      // Notice here that since we didn't modify the position, and this is a
      // second pass, the position has been reset to it's initial, unmodified
      // position
      float currTime = pShaderAPI->CurrentTime();
      for (int i = 0; i < MeshBuilder()->NumVertices(); ++i) {
        float const *pPos = MeshBuilder()->Position();
