...
// Note: Not upgraded to vs/ps 2.0 fxc's because this shader is unused and there
// are no test cases to verify against.
...
#if !defined(_X360) // not used for anything at time of 360 ship, and we want to
                    // avoid storing/loading assembly shaders

// PC version
BEGIN_VS_SHADER_FLAGS(ShadowModel_DX9, "Help for ShadowModel",
                      SHADER_NOT_EDITABLE)

BEGIN_SHADER_PARAMS
SHADER_PARAM(BASETEXTUREOFFSET, SHADER_PARAM_TYPE_VEC2, "[0 0]",
             "$baseTexture texcoord offset")
SHADER_PARAM(BASETEXTURESCALE, SHADER_PARAM_TYPE_VEC2, "[1 1]",
             "$baseTexture texcoord scale")
SHADER_PARAM(FALLOFFOFFSET, SHADER_PARAM_TYPE_FLOAT, "0",
             "Distance at which shadow starts to fade")
SHADER_PARAM(FALLOFFDISTANCE, SHADER_PARAM_TYPE_FLOAT, "100",
             "Max shadow distance")
SHADER_PARAM(FALLOFFAMOUNT, SHADER_PARAM_TYPE_FLOAT, "0.9",
             "Amount to brighten the shadow at max dist")
END_SHADER_PARAMS
...
SHADER_DRAW {
  ...
  DYNAMIC_STATE {
    ...
    SetVertexShaderMatrix3x4(VERTEX_SHADER_SHADER_SPECIFIC_CONST_0,
                             BASETEXTURETRANSFORM);

    SetVertexShaderConstant(VERTEX_SHADER_SHADER_SPECIFIC_CONST_3,
                            BASETEXTUREOFFSET);
    SetVertexShaderConstant(VERTEX_SHADER_SHADER_SPECIFIC_CONST_4,
                            BASETEXTURESCALE);
