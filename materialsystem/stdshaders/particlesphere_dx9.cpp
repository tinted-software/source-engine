...
BEGIN_VS_SHADER_FLAGS(ParticleSphere_DX9, "Help for BumpmappedEnvMap",
                      SHADER_NOT_EDITABLE)
...
SHADER_PARAM(USINGPIXELSHADER, SHADER_PARAM_TYPE_BOOL, "0",
             "Tells to client code whether the shader is using DX8 "
             "vertex/pixel shaders or not")
SHADER_PARAM(BUMPMAP, SHADER_PARAM_TYPE_TEXTURE,
             "models/shadertest/shader1_normal", "bumpmap")
SHADER_PARAM(LIGHTS, SHADER_PARAM_TYPE_FOURCC, "", "array of lights")
SHADER_PARAM(LIGHT_POSITION, SHADER_PARAM_TYPE_VEC3, "0 0 0",
             "This is the directional light position.")
SHADER_PARAM(LIGHT_COLOR, SHADER_PARAM_TYPE_VEC3, "1 1 1",
...
bool UsePixelShaders(IMaterialVar **params) const {
  return (!params || params[BUMPMAP]->IsDefined()) &&
...
SHADER_DRAW {
  SHADOW_STATE {
    ...
    pShaderShadow->VertexShaderVertexFormat(VERTEX_POSITION | VERTEX_COLOR, 1,
                                            tCoordDimensions, 0);
  ...
  DYNAMIC_STATE {
    ...
    // Separate the light color into something that has a max value of 1 and a
    // scale so the vertex shader can determine if it's going to overflow the
    // color and scale back if it needs to.
    //
    // (It does this by seeing if the intensity*1/distSqr is > 1. If so, then it
    // scales it so it is equal to 1).
