...
SHADER_DRAW {
  ...
  bool bEnableAlphaWrites = (params[CLEARALPHA]->GetIntValue() >= 0)
                                ? (params[CLEARALPHA]->GetIntValue() != 0)
                                : bEnableColorWrites;
  ...
  SHADOW_STATE {
    ...
    // avoid setting a pixel shader when only doing depth/stencil operations, as
    // recommended by PIX
    if (bUsesColor ||
