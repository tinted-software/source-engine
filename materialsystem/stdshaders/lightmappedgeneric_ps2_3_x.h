...
struct PS_INPUT {
  ...
  // detail textures and bumpmaps are mutually exclusive so that we have enough
  // texcoords.
#if (RELIEF_MAPPING == 0)
...
#endif
{
  ...
  GetBaseTextureAndNormal(BaseTextureSampler, BaseTextureSampler2,
                          BumpmapSampler, bBaseTexture2,
                          bBumpmap || bNormalMapAlphaEnvmapMask, baseTexCoords,
  ...
  if ((distAlphaMask >= OUTLINE_MIN_VALUE0) &&
    ...
    if (distAlphaMask <= OUTLINE_MIN_VALUE1) {
      oFactor =
          smoothstep(OUTLINE_MIN_VALUE0, OUTLINE_MIN_VALUE1, distAlphaMask);
    } else {
      oFactor =
          smoothstep(OUTLINE_MAX_VALUE1, OUTLINE_MAX_VALUE0, distAlphaMask);
    }
  ...
  if (bBumpmap) {
...
#if (BUMPMAP2 == 1)
    {
      ...
      if (BUMPMAP == 2)
        ...
        vNormal2 =
            DecompressNormal(BumpmapSampler2, b2TexCoord, NORMAL_DECODE_MODE,
      ...
      vNormalMask = DecompressNormal(BumpMaskSampler, i.baseTexCoord.xy,
  ...
  // Save this off for single-pass flashlight, since we'll still need the SSBump
  // vector, not a real normal
...
// ssbump
    ...
    // now, calculate vNormal for reflection purposes. if vNormal isn't needed,
    // hopefully the compiler will eliminate these calculations
    vNormal.xyz =
  ...
  // FIXME: 8-bit lookup textures like this need a "nice filtering" VTF option,
  // which converts
  ...
  return FinalOutput(HALF4(dotprod * albedo.xyz, alpha), 0, PIXEL_FOG_TYPE_NONE,
                     TONEMAP_SCALE_NONE);
#else
  LPREVIEW_PS_OUT ret;
