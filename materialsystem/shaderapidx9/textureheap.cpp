...
public:
  static void *Alloc(int bytes) {
    DWORD attributes = MAKE_XALLOC_ATTRIBUTES(
        0, false, TRUE, FALSE, eXALLOCAllocatorId_D3D,
        XALLOC_PHYSICAL_ALIGNMENT_4K, XALLOC_MEMPROTECT_WRITECOMBINE, FALSE,
...
CXboxTexture *GetTexture(THInfo_t *pInfo) {
  if (!pInfo->bFree && !pInfo->bNonTexture) {
    return (CXboxTexture *)((byte *)pInfo -
                            offsetof(CXboxTexture, m_fAllocator));
  }
  return NULL;
}
...
public:
  CMixedTextureHeap()
      : m_nLogicalBytes(0), m_nActualBytes(0), m_nAllocs(0), m_nOldBytes(0),
        m_nNonTextureAllocs(0), m_nBytesTotal(0), m_pBase(NULL),
        m_pFirstFree(NULL) {}
  ...
  void Init() {
    ...
    m_pBase =
        XPhysicalAlloc(m_nBytesTotal, MAXULONG_PTR, 4096,
                       PAGE_READWRITE | PAGE_WRITECOMBINE | MEM_16MB_PAGES);
#endif
  ...
  void Free(void *p, THInfo_t *pInfo) {
    ...
    AddToBlocksList(&pFree->heapInfo, pFree->heapInfo.pPrev,
                    pFree->heapInfo.pNext);
  ...
  void *ClaimBlock(THFreeBlock_t *pFreeBlock, THInfo_t *pInfo) {
    ...
    *pInfo = pFreeBlock->heapInfo;
    ...
    if (nBytesRemainder >= MIN_BLOCK_SIZE) {
      ...
      THFreeBlock_t *pRemainder =
          (THFreeBlock_t *)(((byte *)(pFreeBlock)) + nBytesDesired);
  ...
  bool GetExpansionList(THFreeBlock_t *pFreeBlock, THInfo_t **ppStart,
                        THInfo_t **ppEnd, int depth = 1) {
    THInfo_t *pStart;
    THInfo_t *pEnd;
    int i;
    ...
    while (i > 0 && pStart->pPrev) {
      ...
      while (i > 0 && pScan && !pScan->bFree &&
             GetTexture(pScan)->CanRelocate()) {
        pScan = pScan->pPrev;
        i--;
      }
  ...
  void Validate() {
    ...
    while (pInfo) {
      byte *pCurrentAddress =
          (byte *)((pInfo->bFree) ? GetFreeBlock(pInfo)
                                  : GetD3DTextureBasePtr(GetTexture(pInfo)));
    ...
    while (pFree) {
      ...
      StrongAssert((void *)pFree >= m_pBase &&
                   (void *)pFree < (byte *)m_pBase + nBytesHeap);
      StrongAssert(!pFree->pPrevFree ||
                   ((void *)pFree->pPrevFree >= m_pBase &&
                    (void *)pFree->pPrevFree < (byte *)m_pBase + nBytesHeap));
      StrongAssert(!pFree->pNextFree ||
                   ((void *)pFree->pNextFree >= m_pBase &&
                    (void *)pFree->pNextFree < (byte *)m_pBase + nBytesHeap));
      StrongAssert(!pFree->pPrevFree ||
                   pFree->pPrevFree->heapInfo.nBytes <= pFree->heapInfo.nBytes);
      pLastFree = pFree;
...
inline TextureAllocator_t GetTextureAllocator(IDirect3DBaseTexture9 *pTexture) {
  return (pTexture->GetType() == D3DRTYPE_CUBETEXTURE)
             ? ((CXboxCubeTexture *)pTexture)->m_fAllocator
...
CON_COMMAND(mat_texture_heap_stats, "") {
  if (UseStandardAllocator()) {
    ...
    Msg("Allocations:%d Size:%d\n", CD3DTextureAllocator::GetAllocations(),
        CD3DTextureAllocator::GetSize());
  } else {
    ...
    Msg("\n  Old: %.3fmb, New: %.3fmb\n", oldFootprint / (1024.0 * 1024.0),
        newFootprint / (1024.0 * 1024.0));
  }
}
...
CON_COMMAND(mat_texture_heap_compact, "") {
  ...
  unsigned oldLargest = (g_MixedTextureHeap.GetLastFree())
                            ? g_MixedTextureHeap.GetLastFree()->heapInfo.nBytes
                            : 0;
  g_MixedTextureHeap.Compact();
  unsigned newLargest = (g_MixedTextureHeap.GetLastFree())
                            ? g_MixedTextureHeap.GetLastFree()->heapInfo.nBytes
                            : 0;
...
void CompactTextureHeap() {
  ...
  DevMsg("Compacted texture heap. Old largest block: %.3fk, New largest block: "
         "%.3fk\n",
         oldLargest / 1024.0, newLargest / 1024.0);
}
...
//-----------------------------------------------------------------------------
IDirect3DTexture *CTextureHeap::AllocTexture(int width, int height, int levels,
                                             DWORD usage, D3DFORMAT d3dFormat,
                                             bool bFallback,
                                             bool bNoD3DMemory) {
  ...
  DWORD dwTextureSize = XGSetTextureHeaderEx(
      width, height, levels, usage, d3dFormat, 0, 0, 0,
...
//-----------------------------------------------------------------------------
IDirect3DCubeTexture *CTextureHeap::AllocCubeTexture(int width, int levels,
                                                     DWORD usage,
                                                     D3DFORMAT d3dFormat,
                                                     bool bFallback,
                                                     bool bNoD3DMemory) {
  ...
  DWORD dwTextureSize = XGSetCubeTextureHeaderEx(
      width, levels, usage, d3dFormat, 0, 0, 0, XGHEADER_CONTIGUOUS_MIP_OFFSET,
      pD3DCubeTexture, NULL, NULL);
...
//-----------------------------------------------------------------------------
IDirect3DVolumeTexture *CTextureHeap::AllocVolumeTexture(int width, int height,
  ...
  DWORD dwTextureSize = XGSetVolumeTextureHeaderEx(
      width, height, depth, levels, usage, d3dFormat, 0, 0, 0,
...
D3DMULTISAMPLE_TYPE CTextureHeap::GetBackBufferMultiSampleType() {
  ...
  // 2xMSAA at 640x480 and 848x480 are the only supported multisample mode on
  // 360 (2xMSAA for 720p would use predicated tiling, which would require a
  // rewrite of *all* our render target code)
  // FIXME: shuffle the EDRAM surfaces to allow 4xMSAA for standard def
  //        (they would overlap & trash each other with the current allocation
  //        scheme)
  D3DMULTISAMPLE_TYPE backBufferMultiSampleType = g_pShaderDevice->IsAAEnabled()
...
//-----------------------------------------------------------------------------
IDirect3DSurface *CTextureHeap::AllocRenderTargetSurface(int width, int height,
                                                         D3DFORMAT d3dFormat,
                                                         bool bMultiSample,
                                                         int base) {
  ...
  // this should leave 3MB (of 10) free assuming 1280x720 (and 5MB with
  // 640x480@2xMSAA)
  D3DMULTISAMPLE_TYPE backBufferMultiSampleType =
      GetBackBufferMultiSampleType();
  D3DMULTISAMPLE_TYPE multiSampleType =
      bMultiSample ? backBufferMultiSampleType : D3DMULTISAMPLE_NONE;
  if (base < 0) {
    int backWidth, backHeight;
    ShaderAPI()->GetBackBufferDimensions(backWidth, backHeight);
    D3DFORMAT backBufferFormat = ImageLoader::ImageFormatToD3DFormat(
        g_pShaderDevice->GetBackBufferFormat());
    base = 2 * XGSurfaceSize(backWidth, backHeight, backBufferFormat,
  ...
  if ((d3dFormat == D3DFMT_D24FS8) || (d3dFormat == D3DFMT_D24S8) ||
      (d3dFormat == D3DFMT_D16)) {
    surfParameters.HierarchicalZBase = 0;
    if ((surfParameters.HierarchicalZBase +
         XGHierarchicalZSize(width, height, multiSampleType)) >
  ...
  hr =
      Dx9Device()->CreateRenderTarget(width, height, d3dFormat, multiSampleType,
                                      0, FALSE, &pSurface, &surfParameters);
  Assert(!FAILED(hr));
...
bool CTextureHeap::AllocD3DMemory(IDirect3DBaseTexture *pD3DTexture) {
  ...
  if (pD3DTexture->GetType() == D3DRTYPE_TEXTURE) {
    MEM_ALLOC_CREDIT_(__FILE__ ": Standard D3D");
    pBits = CD3DTextureAllocator::Alloc(
        ((CXboxTexture *)pD3DTexture)->m_TextureSize);
    ...
    MEM_ALLOC_CREDIT_(__FILE__ ": Cubemap standard D3D");
    pBits = CD3DTextureAllocator::Alloc(
        ((CXboxCubeTexture *)pD3DTexture)->m_TextureSize);
...
int CTextureHeap::GetSize(IDirect3DBaseTexture *pD3DTexture) {
  ...
  if (pD3DTexture->GetType() == D3DRTYPE_SURFACE) {
    ...
    int size = ImageLoader::GetMemRequired(
        surfaceDesc.Width, surfaceDesc.Height, 0,
