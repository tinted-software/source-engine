...
public:
  ...
  const ShadowShaderState_t &
  GetSnapshotShader(StateSnapshot_t snapshotId) const;
...
private:
  enum { INVALID_TRANSITION_OP = 0xFFFFFF };
  ...
  StateSnapshot_t
  CreateStateSnapshot(ShadowStateId_t shadowStateId,
  ...
  StateSnapshot_t
  FindStateSnapshot(ShadowStateId_t id,
                    const ShadowShaderState_t &currentState) const;
...
// Inline methods
//-----------------------------------------------------------------------------
inline const ShadowState_t &
CTransitionTable::GetSnapshot(StateSnapshot_t snapshotId) const {
  Assert((snapshotId >= 0) && (snapshotId < m_SnapshotList.Count()));
  return m_ShadowStateList[m_SnapshotList[snapshotId].m_ShadowStateId];
}

inline const ShadowShaderState_t &
CTransitionTable::GetSnapshotShader(StateSnapshot_t snapshotId) const {
  Assert((snapshotId >= 0) && (snapshotId < m_SnapshotList.Count()));
...
inline const ShadowShaderState_t *
CTransitionTable::CurrentShadowShaderState() const {
  if (m_CurrentShadowId == -1)
    return NULL;
