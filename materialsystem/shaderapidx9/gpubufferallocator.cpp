...
// Track non-pooled VB/IB physical allocations (used by
// CGPUBufferAllocator::SpewStats)
...
CGPUBufferAllocator::CGPUBufferAllocator(void)
    : m_nBufferPools(0), m_bEnabled(true) {
  memset(&(m_BufferPools[0]), 0, sizeof(m_BufferPools));

  m_bEnabled = USE_GPU_BUFFER_ALLOCATOR &&
               !CommandLine()->FindParm("-no_gpu_buffer_allocator");
  if (m_bEnabled) {
    // Start with one pool (the size should be the lowest-common-denominator for
    // all maps)
...
bool CGPUBufferAllocator::AllocatePool(int nPoolSize) {
  ...
  if (m_BufferPools[m_nBufferPools]->m_pMemory == NULL) {
    // Physical alloc failed! Continue without crashing, we *might* get away
    // with it...
    ExecuteOnce(DebuggerBreakIfDebugging());
    ExecuteNTimes(
        15,
        Warning(
            "CGPUBufferAllocator::AllocatePool - physical allocation failed! "
            "Physical fragmentation is in bad shape... falling back to "
            "non-pooled VB/IB allocations. Brace for a crash :o/\n"));
...
//-----------------------------------------------------------------------------
inline GPUBufferHandle_t
CGPUBufferAllocator::MakeGPUBufferHandle(int nPoolNum, int nPoolEntry) {
  GPUBufferHandle_t newHandle;
  newHandle.nPoolNum = nPoolNum;
  newHandle.nPoolEntry = nPoolEntry;
  newHandle.pMemory =
      m_BufferPools[nPoolNum]->m_pMemory +
      m_BufferPools[nPoolNum]->m_PoolEntries[nPoolEntry].nOffset;
  return newHandle;
}
...
//-----------------------------------------------------------------------------
bool CGPUBufferAllocator::AllocateBuffer(GPUBufferHandle_t *pHandle,
                                         int nBufferSize, void *pObject,
  ...
  if (m_bEnabled && (nBufferSize <= MAX_BUFFER_SIZE)) {
    ...
    for (int nPool = 0; nPool < m_nBufferPools; nPool++) {
      int nPoolEntry =
          m_BufferPools[nPool]->Allocate(nBufferSize, bIsVertexBuffer, pObject);
      if (nPoolEntry >= 0) {
        // Tada.
...
//-----------------------------------------------------------------------------
// Clear the given allocation from our pools (NOTE: the memory cannot be reused
// until Defrag() is called)
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
bool CGPUBufferAllocator::AllocateVertexBuffer(CVertexBuffer *pVertexBuffer,
                                               int nBufferSize) {
  AUTO_LOCK(m_mutex);

  bool bIsVertexBuffer = true;
  GPUBufferHandle_t handle;
  if (AllocateBuffer(&handle, nBufferSize, (void *)pVertexBuffer,
                     bIsVertexBuffer)) {
...
//-----------------------------------------------------------------------------
bool CGPUBufferAllocator::AllocateIndexBuffer(CIndexBuffer *pIndexBuffer,
                                              int nBufferSize) {
  AUTO_LOCK(m_mutex);

  bool bIsNOTVertexBuffer = false;
  GPUBufferHandle_t handle;
  if (AllocateBuffer(&handle, nBufferSize, (void *)pIndexBuffer,
                     bIsNOTVertexBuffer)) {
...
//-----------------------------------------------------------------------------
// Move a buffer from one location to another (could be movement within the same
// pool)
//-----------------------------------------------------------------------------
void CGPUBufferAllocator::MoveBufferMemory(int nDstPool, int *pnDstEntry,
                                           int *pnDstOffset,
                                           CGPUBufferPool &srcPool,
  ...
  // Update the destination pool's allocation entry (NOTE: this could be
  // srcEntry, so srcEntry.nOffset would change)
  ...
  // Move the write address past this entry and increment the pool high water
  // mark
...
//-----------------------------------------------------------------------------
// Reclaim space freed by destroyed buffers and compact our pools ready for new
// allocations
//-----------------------------------------------------------------------------
void CGPUBufferAllocator::Compact(void) {
  // NOTE: this must only be called during map transitions, no rendering must be
  // in flight and everything must be single-threaded!
  AUTO_LOCK(m_mutex);
  ...
  // We just walk the pools and entries in order, moving each buffer down within
  // the same pool, or to the end of a previous pool (if, after compaction, it
  // now has free space). Each pool should end up with contiguous, usable free
  // space (may be zero bytes) at the end.
  ...
  for (int nSrcPool = 0; nSrcPool < m_nBufferPools; nSrcPool++) {
    ...
    for (int nSrcEntry = 0; nSrcEntry < srcPool.m_PoolEntries.Count();
         nSrcEntry++) {
      GPUBufferPoolEntry_t &srcEntry = srcPool.m_PoolEntries[nSrcEntry];
      if (srcEntry.pVertexBuffer) {
        // First, try to move the buffer into one of the previous
        ...
        while (nDstPool < nSrcPool) {
          ...
          if ((nDstOffset + srcEntry.nSize) <= dstPool.m_nSize) {
            ...
            MoveBufferMemory(nDstPool, &nDstEntry, &nDstOffset, srcPool,
                             srcEntry);
            bDone = true;
            break;
          } else {
    ...
    // Discard unused entries from the end of the pool (freed buffers, or
    // buffers moved to other pools)
    srcPool.m_PoolEntries.SetCountNonDestructively(nEntriesRemainingInPool);
  }

  // Now free empty pools (keep the first (very large) one around, since
  // fragmentation makes freeing+reallocing it a big risk)
  int nBytesFreed = 0;
  ...
  if (m_nBufferPools > 1) {
    // The above compaction algorithm could waste space due to large allocs
    // causing nDstPool to increment before that pool is actually full. With our
    // current usage pattern (total in-use memory is less than
    // INITIAL_POOL_SIZE, whenever Compact is called), that doesn't matter. If
    // that changes (i.e. the below warning fires), then the fix would be:
    //  - for each pool, sort its entries by size (largest first) and try to
    //  allocate them on the end of prior (already-compacted) pools
    //  - pack whatever remains in the pool down, and proceed to the next pool
    ExecuteOnce(
        Warning("CGPUBufferAllocator::Compact may be wasting memory due to "
                "changed usage patterns (see code for suggested fix)."));
  }

#ifdef _X360
  // Invalidate the GPU caches for all pooled memory, since stuff has moved
  // around
  for (int nPool = 0; nPool < m_nBufferPools; nPool++) {
    Dx9Device()->InvalidateGpuCache(m_BufferPools[nPool]->m_pMemory,
                                    m_BufferPools[nPool]->m_nSize, 0);
  }
#endif
...
void CGPUBufferAllocator::SpewStats(bool bBrief) {
  ...
  for (int i = 0; i < m_nBufferPools; i++) {
    ...
    for (int j = 0; j < pool->m_PoolEntries.Count(); j++) {
      ...
      if (poolEntry.pVertexBuffer) {
        // Figure out how much memory we WOULD have allocated for this buffer,
        // if we'd allocated it individually:
  ...
  // NOTE: 'unused' memory doesn't count memory used by freed buffers, which
  // should be zero during gameplay. The purpose is
  //       to measure wastage at the END of a pool, to help determine ideal
  //       values for ADDITIONAL_POOL_SIZE and MAX_BUFFER_SIZE.
  int nMemUnused = nMemAllocated - nMemUsed;

  const float KB = 1024.0f, MB = KB * KB;
  if (bBrief) {
    ConMsg("[GPUBUFLOG] Pools:%2d | Size:%5.1fMB | Unused:%5.1fMB | "
           "Freed:%5.1fMB | Unpooled:%5.1fMB\n",
           m_nBufferPools, nMemAllocated / MB, nMemUnused / MB,
           nFreedBufferMem / MB,
    ...
    Msg(" -- %5d     -- Number of freed buffers in pools (should be zero "
        "during gameplay)\n",
        nFreedBuffers);
...
CGPUBufferPool::CGPUBufferPool(int nSize)
    : m_PoolEntries(POOL_ENTRIES_GROW_SIZE, POOL_ENTRIES_INIT_SIZE), m_nSize(0),
      m_nBytesUsed(0) {
  // NOTE: write-combining (PAGE_WRITECOMBINE) is deliberately not used, since
  // it slows down 'Compact' hugely (and doesn't noticeably benefit load times)
  m_pMemory = (byte *)XPhysicalAlloc(nSize, MAXULONG_PTR, 0, PAGE_READWRITE);
  if (m_pMemory) {
    MemAlloc_RegisterExternalAllocation(XMem_CGPUBufferPool, m_pMemory,
                                        XPhysicalSize(m_pMemory));
    m_nSize = nSize;
  }
}
...
CGPUBufferPool::~CGPUBufferPool(void) {
  for (int i = 0; i < m_PoolEntries.Count(); i++) {
    if (m_PoolEntries[i].pVertexBuffer) {
      // Buffers should be cleaned up before the CGPUBufferAllocator is shut
      // down!
      Assert(0);
      Warning("ERROR: Un-freed %s in CGPUBufferPool on shut down! (%6.1fKB\n",
              (m_PoolEntries[i].bIsVertexBuffer ? "VB" : "IB"),
              (m_PoolEntries[i].nSize / 1024.0f));
      break;
    }
  }

  if (m_pMemory) {
    MemAlloc_RegisterExternalDeallocation(XMem_CGPUBufferPool, m_pMemory,
                                          XPhysicalSize(m_pMemory));
...
//-----------------------------------------------------------------------------
int CGPUBufferPool::Allocate(int nBufferSize, bool bIsVertexBuffer,
                             void *pObject) {
