...
// Set this to 1 to allow d3d calls to be buffered and played back on another
// thread Slamming this off - it's causing very hot D3D9 function calls to not
// be inlined and contain a bunch of unused code. (Does this code even work/add
// real value any more?)
#define SHADERAPI_USE_SMP 0

// Set this to 1 to allow buffering of the whole frame to memory and then
// playback (singlethreaded). This is for debugging only and is used to test the
// performance of just calling D3D and rendering without other CPU overhead.
...
#define SHADERAPI_BUFFER_MAXRENDERTARGETS 4
  ...
  PushBuffer *FindFreePushBuffer(
      PushBufferState newstate); // find a free push buffer and change its state
  ...
  FORCEINLINE void Push(PushBufferCommand cmd, void *arg1, uint32 arg2,
                        uint32 arg3, uint32 arg4, void *arg5) {
  ...
  FORCEINLINE void Push(PushBufferCommand cmd, uint32 arg1, void *ptr, int arg2,
                        int arg3) {
...
public:
  ...
  HRESULT CreateCubeTexture(
      ...
      char *debugLabel = NULL // <-- OK to not pass this arg, only passed
                              // through on DX_TO_GL_ABSTRACTION
  ) {
    Synchronize();
  ...
  HRESULT TestCooperativeLevel(void) {
    // hack!  We are going to assume that calling this immediately when in
    // buffered mode isn't going to cause problems.
#if !SHADERAPI_BUFFER_D3DCALLS
    Synchronize();
#endif
  ...
  HRESULT GetRenderTarget(DWORD RenderTargetIndex,
    ...
    if (ASyncMode()) {
      Assert(RenderTargetIndex >= 0 &&
             RenderTargetIndex < SHADERAPI_BUFFER_MAXRENDERTARGETS);
  ...
  FORCEINLINE HRESULT Lock(IDirect3DVertexBuffer9 *vb, size_t offset,
    ...
    case D3DERR_INVALIDCALL:
      Warning("D3DERR_INVALIDCALL - Vertex Buffer Lock Failed in %s on line "
              "%d(offset %d, size %d, flags 0x%x)\n",
              V_UnqualifiedFileName(__FILE__), __LINE__, offset, size, flags);
      break;
    case D3DERR_DRIVERINTERNALERROR:
      Warning("D3DERR_DRIVERINTERNALERROR - Vertex Buffer Lock Failed in %s on "
              "line %d (offset %d, size %d, flags 0x%x)\n",
              V_UnqualifiedFileName(__FILE__), __LINE__, offset, size, flags);
      break;
    case D3DERR_OUTOFVIDEOMEMORY:
      Warning("D3DERR_OUTOFVIDEOMEMORY - Vertex Buffer Lock Failed in %s on "
              "line %d (offset %d, size %d, flags 0x%x)\n",
              V_UnqualifiedFileName(__FILE__), __LINE__, offset, size, flags);
      break;
    }
  ...
  FORCEINLINE HRESULT Lock(IDirect3DVertexBuffer9 *vb, size_t offset,
    ...
    if (ASyncMode()) {
      ...
      case D3DERR_INVALIDCALL:
        ...
        break;
      case D3DERR_DRIVERINTERNALERROR:
        Warning("D3DERR_DRIVERINTERNALERROR - Vertex Buffer Lock Failed in %s "
                "on line %d (offset %d, size %d, flags 0x%x)\n",
                V_UnqualifiedFileName(__FILE__), __LINE__, offset, size, flags);
        break;
      case D3DERR_OUTOFVIDEOMEMORY:
        Warning("D3DERR_OUTOFVIDEOMEMORY - Vertex Buffer Lock Failed in %s on "
  ...
  FORCEINLINE HRESULT Lock(IDirect3DIndexBuffer9 *ib, size_t offset,
    ...
    switch (hr) {
    case D3DERR_INVALIDCALL:
      Warning("D3DERR_INVALIDCALL - Index Buffer Lock Failed in %s on line "
              "%d(offset %d, size %d, flags 0x%x)\n",
              V_UnqualifiedFileName(__FILE__), __LINE__, offset, size, flags);
      break;
    case D3DERR_DRIVERINTERNALERROR:
      Warning("D3DERR_DRIVERINTERNALERROR - Index Buffer Lock Failed in %s on "
              "line %d (offset %d, size %d, flags 0x%x)\n",
              V_UnqualifiedFileName(__FILE__), __LINE__, offset, size, flags);
      break;
    case D3DERR_OUTOFVIDEOMEMORY:
      Warning("D3DERR_OUTOFVIDEOMEMORY - Index Buffer Lock Failed in %s on "
              "line %d (offset %d, size %d, flags 0x%x)\n",
              V_UnqualifiedFileName(__FILE__), __LINE__, offset, size, flags);
      break;
    }
  ...
  FORCEINLINE HRESULT Lock(IDirect3DIndexBuffer9 *ib, size_t offset,
    ...
    if (ASyncMode())
      ...
      case D3DERR_INVALIDCALL:
        ...
        break;
      case D3DERR_DRIVERINTERNALERROR:
        Warning("D3DERR_DRIVERINTERNALERROR - Index Buffer Lock Failed in %s "
                "on line %d (offset %d, size %d, flags 0x%x)\n",
                V_UnqualifiedFileName(__FILE__), __LINE__, offset, size, flags);
        break;
      case D3DERR_OUTOFVIDEOMEMORY:
  ...
  FORCEINLINE HRESULT UpdateSurface(IDirect3DSurface9 *pSourceSurface,
                                    CONST RECT *pSourceRect,
                                    IDirect3DSurface9 *pDestSurface,
                                    CONST POINT *pDestPoint) {
    return m_pD3DDevice->UpdateSurface(pSourceSurface, pSourceRect,
                                       pDestSurface, pDestPoint);
  }
#endif
  ...
  HRESULT CreateVertexShader(CONST DWORD *pFunction,
    ...
    Synchronize();
#ifdef DX_TO_GL_ABSTRACTION
    return m_pD3DDevice->CreateVertexShader(pFunction, ppShader, pShaderName,
                                            debugLabel);
#else
  ...
  HRESULT CreatePixelShader(CONST DWORD *pFunction,
    ...
    Synchronize();
#ifdef DX_TO_GL_ABSTRACTION
    return m_pD3DDevice->CreatePixelShader(pFunction, ppShader, pShaderName,
                                           debugLabel);
#else
...
#ifndef DX_TO_GL_ABSTRACTION
  FORCEINLINE void DrawTessellatedIndexedPrimitive(INT BaseVertexIndex,
                                                   UINT MinIndex,
                                                   UINT NumVertices,
                                                   UINT StartIndex,
                                                   UINT PrimitiveCount) {
    ...
    DO_D3D(
        SetStreamSourceFreq(0, D3DSTREAMSOURCE_INDEXEDDATA | PrimitiveCount));
    DO_D3D(SetStreamSourceFreq(VertexStreamSpec_t::STREAM_MORPH,
                               D3DSTREAMSOURCE_INSTANCEDATA | 1ul));
    DO_D3D(SetStreamSourceFreq(VertexStreamSpec_t::STREAM_SUBDQUADS,
                               D3DSTREAMSOURCE_INSTANCEDATA | 1ul));

    int nIndicesPerPatch =
        (((m_nCurrentTessLevel + 1) * 2 + 2) * m_nCurrentTessLevel) - 2;
    int nVerticesPerPatch = m_nCurrentTessLevel + 1;
    nVerticesPerPatch *= nVerticesPerPatch;
    int nPrimitiveCount = nIndicesPerPatch - 2;
    DO_D3D(DrawIndexedPrimitive(D3DPT_TRIANGLESTRIP, 0, 0, nVerticesPerPatch, 0,
                                nPrimitiveCount));
  ...
  FORCEINLINE void DrawTessellatedPrimitive(UINT StartVertex,
                                            UINT PrimitiveCount) {
...
#ifdef DX_TO_GL_ABSTRACTION
  FORCEINLINE HRESULT LinkShaderPair(IDirect3DVertexShader9 *vs,
  ...
  HRESULT Present(CONST RECT *pSourceRect, CONST RECT *pDestRect,
                  VD3DHWND hDestWindowOverride, CONST RGNDATA *pDirtyRegion) {
