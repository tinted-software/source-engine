...
// PC:    By default, PIX profiling is explicitly disallowed using the
// D3DPERF_SetOptions(1) API on PC X360:  PIX_INSTRUMENTATION will only generate
// PIX events in RELEASE builds on 360 Uncomment to use PIX instrumentation:
#if PIX_ENABLE
...
inline IDirect3D9 *D3D() { return m_pD3D; }
...
public:
  ...
  virtual void
  EnableNonInteractiveMode(MaterialNonInteractiveMode_t mode,
...
public:
  ...
  // This handles any events queued because they were called outside of the
  // owning thread
...
protected:
  ...
  int m_numReleaseResourcesRefCount; // This is holding the number of
                                     ...
                                     // every ReleaseResources call there should
                                     // be a matching call to
...
FORCEINLINE IDirect3DDevice9 *Dx9Device() { return g_pD3DDevice; }
...
FORCEINLINE bool CShaderDeviceDx8::IsDeactivated() const {
  return (IsPC() && ((m_DeviceState != DEVICE_STATE_OK) ||
                     m_bQueuedDeviceLost || m_numReleaseResourcesRefCount));
}
