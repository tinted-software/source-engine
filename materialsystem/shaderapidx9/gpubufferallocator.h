...
//
// Purpose: CGPUBufferAllocator manages allocation of VBs/IBs from shared memory
// pools.
...
struct GPUBufferPoolEntry_t {
  ...
  union {
    // These are set to NULL by CGPUBufferPool::Free() (called when the VB/IB is
    // destroyed)
...
public:
  ...
  // Returns the index (-1 on failure) of a new allocation in the pool, for a
  // buffer of the given size.
  int Allocate(int nSize, bool bIsVertexBuffer, void *pObject);
  // Frees a given entry (just marks it as freed, the memory will not be reused
  // by Allocate() until CGPUBufferAllocator::Defrag() is called )
  void Deallocate(const GPUBufferHandle_t *pHandle);

private:
  // NOTE: these values are specialized for X360 and should be #ifdef'd for
  // other target platforms
  static const int POOL_ENTRIES_INIT_SIZE = 256;
  static const int POOL_ENTRIES_GROW_SIZE = 256;
  static const int POOL_ENTRY_ALIGNMENT =
      4; // 4-byte alignment required for VB/IB data on XBox360
  ...
  CUtlVector<GPUBufferPoolEntry_t>
      m_PoolEntries; // Memory-order array of items allocated in the pool

  // CGPUBufferAllocator is a friend so that CGPUBufferAllocator::Defrag() can
  // shuffle allocations around
...
private:
  ...
  bool AllocateBuffer(GPUBufferHandle_t *pHandle, int nBufferSize,
                      void *pObject, bool bIsVertexBuffer);
  ...
  // Helper for Compact
  void MoveBufferMemory(int nDstPool, int *pnDstEntry, int *pnDstOffset,
