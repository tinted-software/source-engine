...
void CShaderShadowDX8::VertexShaderVertexFormat(unsigned int nFlags,
  ...
  if (CVertexBufferBase::VertexFormatSize(m_ShadowShaderState.m_VertexUsage) <=
    ...
    if (!bComplained) {
      Warning("ERROR: shader asking for a too-narrow vertex format - you will "
              "see errors if running with debug D3D DLLs!\n\tPadding the "
              "vertex format with extra texcoords\n\tWill not warn again.\n");
      bComplained = true;
    }
...
void CShaderShadowDX8::SetVertexShader(const char *pFileName,
  ...
  char debugLabel[500] = "";
#ifdef DX_TO_GL_ABSTRACTION
  Q_snprintf(debugLabel, sizeof(debugLabel), "vs-file %s vs-index %d",
             pFileName, nStaticVshIndex);
#endif

  m_ShadowShaderState.m_VertexShader = ShaderManager()->CreateVertexShader(
      pFileName, nStaticVshIndex, debugLabel);
...
void CShaderShadowDX8::SetPixelShader(const char *pFileName,
  ...
  char debugLabel[500] = "";
#ifdef DX_TO_GL_ABSTRACTION
  Q_snprintf(debugLabel, sizeof(debugLabel), "ps-file %s ps-index %d",
             pFileName, nStaticPshIndex);
#endif

  m_ShadowShaderState.m_PixelShader = ShaderManager()->CreatePixelShader(
      pFileName, nStaticPshIndex, debugLabel);
...
//-----------------------------------------------------------------------------
inline bool
CShaderShadowDX8::IsUsingTextureCoordinates(Sampler_t sampler) const {
  return m_SamplerState[sampler].m_TextureEnable;
}

inline D3DTEXTUREOP
CShaderShadowDX8::OverbrightBlendValue(TextureStage_t stage) {
...
void CShaderShadowDX8::SetDiffuseMaterialSource(
  ...
  COMPILE_TIME_ASSERT((int)D3DMCS_MATERIAL ==
                      (int)SHADER_MATERIALSOURCE_MATERIAL);
