...
static ConVar mat_remoteshadercompile("mat_remoteshadercompile", "127.0.0.1",
                                      FCVAR_CHEAT);
...
// static D3DToGL_ASM sg_D3DToOpenGLTranslator;	// Remove the _ASM to switch to
// the new translator static D3DToGL sg_NewD3DToOpenGLTranslator;
// // Remove the _ASM to switch to the new translator

static const char *GetLightTypeName(VertexShaderLightTypes_t type) {
  static const char *s_VertexShaderLightTypeNames[] = {
      "LIGHT_NONE",        "LIGHT_SPOT",   "LIGHT_POINT",
      "LIGHT_DIRECTIONAL", "LIGHT_STATIC", "LIGHT_AMBIENTCUBE",
  };
...
// mat_autoload_glshaders instructs the engine to load a cached shader table at
// startup it will try for glshaders.cfg first, then fall back to
// glbaseshaders.cfg if not found mat_autosave_glshaders instructs the engine to
// save out the shader table at key points to the filename glshaders.cfg
//
...
static CUtlMap<IDirect3DVertexShader9 *, CRC32_t, int>
    s_VSLookup(0, 0, DefLessFunc(IDirect3DVertexShader9 *));
static CUtlMap<IDirect3DPixelShader9 *, CRC32_t, int>
    s_PSLookup(0, 0, DefLessFunc(IDirect3DPixelShader9 *));
#endif
...
static HardwareShader_t CreateD3DVertexShader(DWORD *pByteCode, int numBytes,
  ...
  HRESULT hr = Dx9Device()->CreateVertexShader(
      pByteCode, (IDirect3DVertexShader9 **)&hShader, pShaderName, debugLabel);
#else
  if (IsEmulatingGL()) {
...
#else
  HRESULT hr = Dx9Device()->CreateVertexShader(
      pByteCode, (IDirect3DVertexShader9 **)&hShader, pShaderName);
#endif
  ...
  if (IsPC()) {
    ...
    while (pShader) {
      ...
      default:
        ...
        pShader =
            pShader +
            ((*pShader & D3DSI_INSTLENGTH_MASK) >> D3DSI_INSTLENGTH_SHIFT) + 1;
      }
    }
  }
}
...
//-----------------------------------------------------------------------------
static HardwareShader_t
CreateD3DPixelShader(DWORD *pByteCode, unsigned int nCentroidMask, int numBytes,
  ...
  HRESULT hr = Dx9Device()->CreatePixelShader(
      pByteCode, (IDirect3DPixelShader **)&shader, pShaderName, debugLabel);
#else
  HRESULT hr = Dx9Device()->CreatePixelShader(
      pByteCode, (IDirect3DPixelShader **)&shader, pShaderName, debugLabel,
      &nCentroidMask);
...
#else
  HRESULT hr = Dx9Device()->CreatePixelShader(
      pByteCode, (IDirect3DPixelShader **)&shader, pShaderName);
#endif
#endif
...
inline int FindShaderStaticCombo(uint32 nStaticComboID,
...
private:
  ...
  void
  SetVertexShaderState(HardwareShader_t shader,
  ...
  void DisassembleShader(ShaderLookup_t *pLookup, int dynamicCombo,
                         uint8 *pByteCode);
  void WriteTranslatedFile(ShaderLookup_t *pLookup, int dynamicCombo,
                           char *pFileContents, char *pFileExtension);

  // DX_TO_GL_ABSTRACTION only, no-op otherwise

  void SaveShaderCache(
      char *cacheName); // query GLM pair cache for all active shader pairs and
                        // write them to disk in named file
  bool LoadShaderCache(
      char *cacheName); // read named file, establish compiled shader sets for
                        // each vertex+static and pixel+static, then link pairs
                        // as listed in table return true on success, false if
                        // file not found
...
CShaderManager::CShaderManager()
    : m_ShaderSymbolTable(0, 32, true /* caseInsensitive */),
      m_VertexShaderDict(32), m_PixelShaderDict(32), m_ShaderFileCache(32) {
...
void CShaderManager::InitRemoteShaderCompile() {
  ...
  if (nResult != 0) {
    Warning("CShaderManager::Init - Could not init socket for remote dynamic "
            "shader compilation\n");
  }
#endif
  ...
  nResult = getaddrinfo(mat_remoteshadercompile.GetString(),
                        REMOTE_SHADER_COMPILE_PORT, &hints, &result);
  if (nResult != 0) {
  ...
  for (struct addrinfo *ptr = result; ptr != NULL; ptr = ptr->ai_next) {
    ...
    // Connect to server.
    nResult = connect(m_RemoteShaderCompileSocket, ptr->ai_addr,
                      (int)ptr->ai_addrlen);
...
#endif // defined ( DYNAMIC_SHADER_COMPILE ) && defined(
...
CShaderManager::CreateVertexShader(IShaderBuffer *pShaderBuffer) {
...
#else
  HRESULT hr = Dx9Device()->CreateVertexShader(
      (const DWORD *)pShaderBuffer->GetBits(), &pVertexShader, NULL);
#endif
...
PixelShaderHandle_t
CShaderManager::CreatePixelShader(IShaderBuffer *pShaderBuffer) {
  // Create the vertex shader
...
#else
  HRESULT hr = Dx9Device()->CreatePixelShader(
      (const DWORD *)pShaderBuffer->GetBits(), &pPixelShader, NULL);
#endif
...
void CShaderManager::CreateStaticShaders() {
  ...
  if (IsPC()) {
    ...
    const DWORD psIllegalMaterial[] = {
        ...
        0xffff0200, 0x05000051, 0xa00f0000, 0x3f800000, 0x00000000,
        0x3f800000, 0x3f800000, 0x02000001, 0x800f0000, 0xa0e40000,
...
#ifdef DYNAMIC_SHADER_COMPILE
const CShaderManager::ShaderCombos_t *
CShaderManager::FindOrCreateShaderCombos(const char *pShaderName) {
  ...
  while (char *line = bffr.InplaceGetLinePtr()) {
    ...
    *pScan = '\0';
    ...
    //		char buf[1024];
    //		sprintf( buf, "\"%s\" \"%s\" %d %d\n", bDynamic ? "DYNAMIC" :
    //"STATIC", pBeginningOfName, begin, end ); 		Plat_DebugString( buf );
...
HardwareShader_t CShaderManager::CompileShader(const char *pShaderName,
  ...
  // plus 1 for null termination, plus 1 for #define SHADER_MODEL_*, and plus 1
  // for #define _X360 on 360
...
#define RECV_BUF_SIZE 40000
  ...
  if (m_RemoteShaderCompileSocket != INVALID_SOCKET) {
    ...
    char pSendbuf[SEND_BUF_SIZE], pRecvbuf[RECV_BUF_SIZE],
        pFixedFilename[MAX_PATH], buf[MAX_PATH];
    ...
    int nResult =
        send(m_RemoteShaderCompileSocket, pSendbuf, (int)strlen(pSendbuf), 0);
    if (nResult == SOCKET_ERROR) {
    ...
    if (m_RemoteShaderCompileSocket != INVALID_SOCKET) {
      ...
      if (m_RemoteShaderCompileSocket != INVALID_SOCKET) {
        ...
        // If is zero, we have an error, so the rest of the data is a text
        // string from the compiler
  ...
  if (m_ShaderCompileFileFunc30 && b30Shader) {
    ...
    {
      if (b30Shader) {
        Warning("Compiling with a stale version of d3dx. Should have "
                "d3d9x_33.dll installed (Apr 2007)\n");
      }
...
#ifdef REMOTE_DYNAMIC_SHADER_COMPILE
      // If we're using the remote compiling service, let's double-check against
      // a local compile
      if ((m_RemoteShaderCompileSocket != INVALID_SOCKET) &&
          pRemotelyCompiledShader) {
        if ((memcmp(pRemotelyCompiledShader, pShader->GetBufferPointer(),
                    pShader->GetBufferSize()) != 0) ||
...
#ifndef _DEBUG
    if (retriesLeft-- > 0) {
      DevMsg(0, "Failed dynamic shader compiled - fix the shader while the "
...
#endif // #if defined( DYNAMIC_SHADER_COMPILE ) && !defined(
       // REMOTE_DYNAMIC_SHADER_COMPILE )
...
#ifdef REMOTE_DYNAMIC_SHADER_COMPILE
    if (bVertexShader) {
      return CreateD3DVertexShader((DWORD *)pRemotelyCompiledShader,
                                   nRemotelyCompiledShaderLength, pShaderName);
    } else {
      return CreateD3DPixelShader((DWORD *)pRemotelyCompiledShader, 0,
                                  nRemotelyCompiledShaderLength,
                                  pShaderName); // hack hack hack!  need to get
                                                // centroid info from the source
    }
#else // local compile, not remote
    if (bVertexShader) {
      return CreateD3DVertexShader((DWORD *)pShader->GetBufferPointer(),
                                   pShader->GetBufferSize(), pShaderName);
    } else {
...
#if defined(DYNAMIC_SHADER_COMPILE)
    // We keep up with whether we hit a compile error above.  If we did, then we
    // likely need to recompile everything again since we could have changed
    // global code.
...
//---------------------------------------------------------------------------------------------------------
// Writes text files named for looked-up shaders.  Used by GL shader translator
// to dump code for debugging
//---------------------------------------------------------------------------------------------------------
void CShaderManager::WriteTranslatedFile(ShaderLookup_t *pLookup,
                                         int dynamicCombo, char *pFileContents,
                                         char *pFileExtension) {
  ...
  char filename[MAX_PATH];
  sprintf(filename, "%s_%d_%d.%s", pName, pLookup->m_nStaticIndex, dynamicCombo,
          pFileExtension);
  g_pFullFileSystem->WriteFile(filename, "DEFAULT_WRITE_PATH", tempBuffer);
}
...
void CShaderManager::DisassembleShader(ShaderLookup_t *pLookup,
                                       int dynamicCombo, uint8 *pByteCode) {
...
bool CShaderManager::CreateDynamicCombos_Ver4(void *pContext,
  ...
  for (int i = 0; i < pHeader->m_nDynamicCombos; i++) {
    ...
    uint8 *pByteCode = (uint8 *)pComboBuffer + pLookup->m_nDataOffset +
    ...
    if (IsPC() && m_bCreateShadersOnDemand) {
      ...
      if (pFileCache->m_bVertexShader) {
        hardwareShader = CreateD3DVertexShader(
            reinterpret_cast<DWORD *>(pByteCode), nByteCodeSize, pShaderName);
      } else {
        hardwareShader = CreateD3DPixelShader(
            reinterpret_cast<DWORD *>(pByteCode), pHeader->m_nCentroidMask,
...
bool CShaderManager::CreateDynamicCombos_Ver5(void *pContext,
  ...
  char *debugLabelPtr =
      debugLabel; // can be moved to point at something else if need be
...
#ifdef DX_TO_GL_ABSTRACTION
          // munge the debug label a bit to aid in decoding... catenate the
          // iIndex on the end
          char temp[1024];
          sprintf(temp, "%s vs-combo %d", (debugLabel) ? debugLabel : "none",
                  iIndex);
          debugLabelPtr = temp;
#endif
          // pass binary code to d3d interface, on GL it will invoke the
          // translator back to asm
          hardwareShader =
              CreateD3DVertexShader(reinterpret_cast<DWORD *>(pReadPtr),
...
//					}
          ...
          // iIndex on the end
          char temp[1024];
          sprintf(temp, "%s ps-combo %d", (debugLabel) ? debugLabel : "",
                  iIndex);
          debugLabelPtr = temp;
#endif
          ...
          // translator back to asm
          hardwareShader =
              CreateD3DPixelShader(reinterpret_cast<DWORD *>(pReadPtr),
                                   pFileCache->m_Header.m_nCentroidMask,
                                   nShaderSize, pShaderName, debugLabelPtr);
        }
        if (hardwareShader == INVALID_HARDWARE_SHADER) {
...
bool CShaderManager::LoadAndCreateShaders(ShaderLookup_t &lookup,
    ...
    g_pFullFileSystem->Seek(hFile,
                            nDictionaryOffset +
                                lookup.m_nStaticIndex *
                                    sizeof(ShaderDictionaryEntry_t),
  ...
  if (IsX360() && g_pQueuedLoader->IsMapLoading()) {
    ...
    // printf("\n CShaderManager::LoadAndCreateShaders - reading %d bytes from
    // file offset %d", nAlignedBytesToRead, nAlignedOffset);
    //  single optimal read of all dynamic combos into monolithic buffer
    uint8 *pOptimalBuffer = (uint8 *)g_pFullFileSystem->AllocOptimalReadBuffer(
...
#ifdef DX_TO_GL_ABSTRACTION
// if shaders are changed in a way that requires the client-side cache to be
// invalidated, increment this string - such changes include combo changes
// (skips, adding combos)
const char *k_pszShaderCacheRootKey = "glshadercachev002";
#endif

void CShaderManager::SaveShaderCache(char *cacheName) {
#ifdef DX_TO_GL_ABSTRACTION // must ifdef, it uses calls which don't exist in
                            // the real DX9 interface
  ...
  do {
    ...
    if (info.m_status == 1) {
      ...
      if (info.m_vsName[0] && info.m_psName[0] &&
          (info.m_vsDynamicIndex > -1) && (info.m_psDynamicIndex > -1)) {
        // make up a key - this thing is really a list of tuples, so need not be
        // keyed by anything particular
...
bool CShaderManager::LoadShaderCache(char *cacheName) {
  ...
  FOR_EACH_SUBKEY(pProgramCache, pProgramKey) {
    ...
    ShaderLookup_t vshLookup;
    vshLookup.m_Name = m_ShaderSymbolTable.AddString(
        pVertexShaderName); // TODO: use String() here and catch this odd case
    vshLookup.m_nStaticIndex = nVertexShaderStaticIndex;
    VertexShader_t vertexShader = m_VertexShaderDict.Find(vshLookup);
    ...
    if (vertexShader == m_VertexShaderDict.InvalidIndex()) {
      ...
      V_snprintf(temp, sizeof(temp), "vs-file %s vs-index %d",
    ...
    if (pixelShader == m_PixelShaderDict.InvalidIndex()) {
      ...
      V_snprintf(temp, sizeof(temp), "ps-file %s ps-index %d", pPixelShaderName,
    ...
    // If we found both shaders, do the link!
    if ((vertexShader != m_VertexShaderDict.InvalidIndex()) &&
        (pixelShader != m_PixelShaderDict.InvalidIndex())) {
      // double check that the hardware shader arrays are actually
      // instantiated.. bail on the attempt if not (odd...)
      if (m_VertexShaderDict[vertexShader]
              .m_ShaderStaticCombos.m_pHardwareShaders &&
        ...
        if ((nVertexShaderDynamicIndex >= 0) &&
          ...
          HardwareShader_t hardwareVertexShader =
              m_VertexShaderDict[vertexShader]
                  .m_ShaderStaticCombos
                  .m_pHardwareShaders[nVertexShaderDynamicIndex];
          HardwareShader_t hardwarePixelShader =
              m_PixelShaderDict[pixelShader]
                  .m_ShaderStaticCombos
                  .m_pHardwareShaders[nPixelShaderDynamicIndex];

          if ((hardwareVertexShader != INVALID_HARDWARE_SHADER) &&
              (hardwarePixelShader != INVALID_HARDWARE_SHADER)) {
            if (S_OK != Dx9Device()->LinkShaderPair(
                            (IDirect3DVertexShader9 *)hardwareVertexShader,
                            (IDirect3DPixelShader9 *)hardwarePixelShader)) {
              Warning(
                  "Could not link OpenGL shaders: %s (%d, %d) : %s (%d, %d)\n",
                  pVertexShaderName, nVertexShaderStaticIndex,
                  nVertexShaderDynamicIndex, pPixelShaderName,
          ...
          Warning("nVertexShaderDynamicIndex or nPixelShaderDynamicIndex was "
                  "negative\n");
        }
      } else {
  ...
  Msg("Loaded program cache file \"%s\", total keyvalues: %i, total "
      "successfully linked: %i\n",
...
VertexShader_t CShaderManager::CreateVertexShader(const char *pFileName,
  ...
  if (shader == m_VertexShaderDict.InvalidIndex()) {
    // printf("\nCShaderManager::CreateVertexShader( filename = %s,
    // staticVshIndex = %d - not in cache", pFileName, nStaticVshIndex );
...
void CShaderManager::BindVertexShader(VertexShaderHandle_t hVertexShader) {
  HardwareShader_t hHardwareShader =
      m_RawVertexShaderDict[(VertexShaderIndex_t)(uintp)hVertexShader];
...
void CShaderManager::SetVertexShader(VertexShader_t shader) {
  ...
  //vshLookup.m_Name ), 		vshLookup.m_nStaticIndex, m_nVertexShaderIndex );
  ...
  if (dxshader == INVALID_HARDWARE_SHADER) {
    ...
    if (IsX360()) {
      // 360 does not respond well at all to bad shaders or Error() calls. So
      // we're staying here until we get something that compiles
      while (dxshader == INVALID_HARDWARE_SHADER) {
        Warning("A dynamically compiled vertex shader has failed to build. "
                "Pausing for 5 seconds and attempting rebuild.\n");
#ifdef _WIN32
        Sleep(5000);
#elif POSIX
        usleep(5000);
  ...
  Q_strncpy(vshDebugName[vshDebugIndex],
            ...
            sizeof(vshDebugName[0]));
#endif
  Assert(vshIndex < vshLookup.m_ShaderStaticCombos.m_nCount);
  HardwareShader_t dxshader =
      vshLookup.m_ShaderStaticCombos.m_pHardwareShaders[vshIndex];
#endif
  ...
  if (IsPC() && (dxshader == INVALID_HARDWARE_SHADER) &&
    ...
    dxshader =
        CreateD3DVertexShader((DWORD *)pCreationData->ByteCode.Base(),
  ...
  if (!dxshader) {
    Error("!!!!!Using invalid shader combo!!!!!  Consult a programmer and tell "
          "them to build debug materialsystem.dll and stdshader*.dll.  Run "
          "with \"mat_bufferprimitives 0\" and look for CMaterial in the call "
          "stack and see what m_pDebugName is.  You are likely using a shader "
          "combo that has been skipped.\n");
  }
#endif
...
void CShaderManager::BindPixelShader(PixelShaderHandle_t hPixelShader) {
  HardwareShader_t hHardwareShader =
      m_RawPixelShaderDict[(PixelShaderIndex_t)(uintp)hPixelShader];
...
void CShaderManager::SetPixelShader(PixelShader_t shader) {
  ...
  //pshLookup.m_Name ), 		pshLookup.m_nStaticIndex, m_nPixelShaderIndex );
  ...
  if (dxshader == INVALID_HARDWARE_SHADER) {
    // compile it since we haven't already!
    dxshader = CompileShader(m_ShaderSymbolTable.String(pshLookup.m_Name),
                             pshLookup.m_nStaticIndex, pshIndex, false);
    //		Assert( dxshader != INVALID_HARDWARE_SHADER );
    ...
    if (IsX360()) {
      ...
      // we're staying here until we get something that compiles
      while (dxshader == INVALID_HARDWARE_SHADER) {
        Warning("A dynamically compiled pixel shader has failed to build. "
                "Pausing for 5 seconds and attempting rebuild.\n");
#ifdef _WIN32
        Sleep(5000);
#elif POSIX
  ...
  Q_strncpy(pshDebugName[pshDebugIndex],
            ...
            sizeof(pshDebugName[0]));
#endif
  HardwareShader_t dxshader =
      pshLookup.m_ShaderStaticCombos.m_pHardwareShaders[pshIndex];
#endif
  ...
  if (IsPC() && (dxshader == INVALID_HARDWARE_SHADER) &&
    ...
    dxshader = CreateD3DPixelShader(
        (DWORD *)pCreationData->ByteCode.Base(), pCreationData->iCentroidMask,
...
void CShaderManager::DestroyAllShaders(void) {
  // Remarking this out because it's conflicting with dxabstract's shutdown
  // resource leak detection code (we leak thousands of shaders at shutdown with
  // this in place). I see no reason why we would want to do this in D3D9 but
  // not GL?
  // #ifdef DX_TO_GL_ABSTRACTION
  //		return;
  // #endif
