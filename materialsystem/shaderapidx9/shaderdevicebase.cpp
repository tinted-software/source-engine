...
//-----------------------------------------------------------------------------
const HardwareCaps_t &
CShaderDeviceMgrBase::GetHardwareCaps(int nAdapter) const {
...
//-----------------------------------------------------------------------------
KeyValues *
CShaderDeviceMgrBase::FindDXLevelSpecificConfig(KeyValues *pKeyValues,
                                                int nDxLevel) {
...
void CShaderDeviceMgrBase::ReadDXSupportLevels(HardwareCaps_t &caps) {
  ...
  if (pDeviceKeyValues) {
    ...
    if (nDXSupportLevel != 0) {
      ...
      // Don't slam up the dxlevel level to 92 on DX10 cards in OpenGL Linux/Win
      // mode (otherwise Intel will get dxlevel 92 when we want 90)
...
void CShaderDeviceMgrBase::LoadHardwareCaps(KeyValues *pGroup,
  ...
  // don't just blanket kill clip planes on POSIX, only shoot them down if we're
  // running ARB, or asked for nouserclipplanes.
  // FIXME need to take into account the caps bit that GLM can now provide, so
  // NV can use normal clipping and ATI can fall back to fastclip.

  if (CommandLine()->FindParm("-arbmode") ||
      CommandLine()->CheckParm("-nouserclip")) {
...
bool CShaderDeviceMgrBase::GetRecommendedConfigurationInfo(
  ...
  V_snprintf(buf, sizeof(buf),
             "CShaderDeviceMgrBase::GetRecommendedConfigurationInfo: CPU "
             "speed: %d MHz, Processor: %s\n",
...
static BOOL CALLBACK EnumWindowsProcNotThis(VD3DHWND hWnd, LPARAM lParam) {
  if (g_pShaderDevice &&
      (GetTopmostParentWindow((VD3DHWND)g_pShaderDevice->GetIPCHWnd()) == hWnd))
    return TRUE;
...
#ifdef USE_ACTUAL_DX
static LRESULT CALLBACK ShaderDX8WndProc(VD3DHWND hWnd, UINT msg, WPARAM wParam,
                                         LPARAM lParam) {
#if !defined(_X360)
...
void CShaderDeviceBase::InstallWindowHook(void *hWnd) {
  ...
  SetWindowLongPtr((VD3DHWND)m_hWndCookie, GWLP_USERDATA,
