...
static unsigned short
    g_nScratchIndexBuffer[6]; // large enough for a fast quad; used when device
                              // is not active
#ifdef _DEBUG
...
protected:
  ...
  int m_nVertexCount;
  int m_nBufferSize;
  int m_nFirstUnwrittenOffset; // Used only for dynamic buffers, indicates where
                               // it's safe to write (nooverwrite)
...
public:
  ...
  virtual void
  CopyToMeshBuilder(int iStartVert, // Which vertices to copy.
                    int nVerts,
                    int iStartIndex, // Which indices to copy.
                    int nIndices,
                    int indexOffset, // This is added to each index.
...
private:
  ...
  void
  CopyStaticMeshIndexBufferToTempMeshIndexBuffer(CTempMeshDX8 *pDstIndexMesh,
...
void Unbind(IDirect3DIndexBuffer9 *pIndexBuffer) {
  ...
  if (pBoundBuffer == pIndexBuffer) {
    // xboxissue - cannot lock indexes set in a d3d device, clear possibly set
    // indices
...
#if !defined(_X360)
  hr = m_pIndexBuffer->Lock(m_nFirstUnwrittenOffset, nMemoryRequired,
                            &pLockedData, nLockFlags);
#else
...
#if !defined(_X360)
  hr = m_pVertexBuffer->Lock(m_nFirstUnwrittenOffset, nMemoryRequired,
                             &pLockedData, nLockFlags);
#else
...
CMeshDX8::CMeshDX8(const char *pTextureGroupName)
    : m_NumVertices(0), m_NumIndices(0), m_pVertexBuffer(0), m_pColorMesh(0),
      m_nColorMeshVertOffsetInBytes(0), m_pIndexBuffer(0),
      m_Type(MATERIAL_TRIANGLES), m_IsVBLocked(false), m_IsIBLocked(false) {
...
bool CMeshDX8::Lock(int nVertexCount, bool bAppend, VertexDesc_t &desc) {
  ...
  if (!pVertexMemory) {
    if (nVertexCount > nMaxVerts) {
      Assert(0);
      Error("Too many verts for a dynamic vertex buffer (%d>%d) Tell a "
      ...
      if ((SYSCALL_SUCCESS == Plat_GetPagedPoolInfo(&ppi)) &&
        ...
        Error("Out of OS Paged Pool Memory! For more information, please "
              ...
              "php/enduser/std_adp.php?p_faqid=150\n");
      } else {
        Assert(0);
        Error("failed to lock vertex buffer in CMeshDX8::LockVertexBuffer: "
              "nVertexCount=%d, nFirstVertex=%d\n",
...
int CMeshDX8::Lock(bool bReadOnly, int nFirstIndex, int nIndexCount,
  ...
  if (!m_pIndexBuffer) {
    SafeAssign(&m_pIndexBuffer,
               new CIndexBuffer(Dx9Device(), nIndexCount,
                                ShaderAPI()->UsingSoftwareVertexProcessing()));
  }
      ...
      if ((vertexFormat & VERTEX_FORMAT_COMPRESSED) == 0) {
        ...
        if (numWarnings++ == 0) {
          ...
          if (g_pHardwareConfig->SupportsCompressedVertices() ==
            ...
            Warning("ERROR: Compressed vertices in use but vertex compression "
                    "is disabled (or not supported on this hardware)!\n");
          else
            Warning("ERROR: Compressed vertices in use but material does not "
                    "support them!\n");
        }
        Assert(0);
        bIsValid = false;
...
void CMeshDX8::SetColorStreamState() {
  if ((m_pColorMesh != g_pLastColorMesh) ||
    ...
    if (m_pColorMesh) {
      ...
      m_pColorMesh->GetVertexBuffer()->HandlePerFrameTextureStats(
          ShaderAPI()->GetCurrentFrameCounter());
    } else {
      RECORD_COMMAND(DX8_SET_STREAM_SOURCE, 4);
      RECORD_INT(-1); // vertex buffer id
  ...
  if (HardwareConfig()->SupportsVertexAndPixelShaders()) {
    if (HasFlexMesh()) {
      ...
      if (g_pHardwareConfig->Caps().m_SupportsVertexShaders_2_0) {
        float c[4] = {
            1.0f,
            g_pMaterialSystemHardwareConfig->SupportsPixelShaders_2_b() ? 1.0f
                                                                        : 0.0f,
            0.0f, 0.0f};
      ...
      if (m_pVertexBuffer->VertexSize() < minimumStreamZeroStride) {
        ...
        if (!bWarned) {
          Warning("Shader specifying too-thin vertex format, should be at "
                  "least %d bytes! (Suppressing furthur warnings)\n",
                  minimumStreamZeroStride);
          bWarned = true;
        }
      }

      // Set a 4kb all-zero static VB into the flex/wrinkle stream with a stride
      // of 1 bytes, so the vertex shader always reads valid floating point
      // values (otherwise it can get NaN's/Inf's, and under OpenGL this is bad
      // on NVidia) togl requires non-zero strides, but on D3D9 we can set a
      // stride of 0 for a little more efficiency.
      D3DSetStreamSource(2, g_MeshMgr.GetZeroVertexBuffer(), 0,
                         IsOpenGL() ? 4 : 0);
...
void CMeshDX8::SetIndexStreamState(int firstVertexIdx) {
  ...
  if ((g_pLastIndexBuffer != NULL) || (g_pLastIndex != m_pIndexBuffer) ||
      (m_pIndexBuffer->IsDynamic()) || (firstVertexIdx != g_LastVertexIdx))
#else
  if ((g_pLastIndexBuffer != NULL) || (g_pLastIndex != m_pIndexBuffer) ||
  ...
  if (m_Mode == D3DPT_TRIANGLELIST || m_Mode == D3DPT_TRIANGLESTRIP) {
    ...
    for (i = 0; i < 2; i++) {
      ...
      for (j = 0; j < nIndexCount; j++) {
        ...
        if ((index < (int)s_FirstVertex) ||
            (index >= (int)(s_FirstVertex + s_NumVertices)))
...
void CMeshDX8::RenderPass() {
  ...
  if (!IsValidVertexFormat(g_LastVertexFormat)) {
    Warning("Material %s does not support vertex format used by the mesh "
            "(maybe missing fields or mismatched vertex compression?), mesh "
            "will not be rendered. Grab a programmer!\n",
  ...
  for (int iPrim = 0; iPrim < s_nPrims; iPrim++) {
    ...
    if ((m_Type == MATERIAL_POINTS) || (m_Type == MATERIAL_INSTANCED_QUADS)) {
...
#endif // CHECK_INDICES
      {
        ...
        VPROF_INCREMENT_GROUP_COUNTER("render/numPrimitives",
                                      COUNTER_GROUP_TELEMETRY, 1);
        ...
        Dx9Device()->DrawIndexedPrimitive(
            ...
            s_FirstVertex, // Minimum vertex index for vertices used during this
                           // call. This is a zero based index relative to
            ...
            pPrim->m_FirstIndex, // Index of the first index to use when
                                 ...
                                 // StartIndex to index vertices from the vertex
                                 // buffer.
...
void CDynamicMeshDX8::LockMesh(int nVertexCount, int nIndexCount,
                               MeshDesc_t &desc) {
  tmZoneFiltered(TELEMETRY_LEVEL0, 50, TMZF_NONE, "%s %d %d", __FUNCTION__,
...
//-----------------------------------------------------------------------------
CMeshMgr::CMeshMgr()
    : m_pDynamicIndexBuffer(0), m_DynamicTempMesh(true), m_pVertexIDBuffer(0),
      m_pCurrentVertexBuffer(NULL), m_CurrentVertexFormat(0),
...
void CMeshMgr::CreateZeroVertexBuffer() {
  if (!m_pZeroVertexBuffer) {
    // In GL glVertexAttribPointer() doesn't support strides of 0, so we need to
    // allocate a dummy vertex buffer large enough to handle 16-bit indices with
    // a stride of 4 byte per vertex, plus a bit more for safety (in case
    // basevertexindex is > 0). We could also try just disabling any vertex
    // attribs that fetch from stream 2 and need 0's, but AMD reports this could
    // hit a slow path in the driver. Argh.
    uint nBufSize = IsOpenGL() ? (65536 * 2 * 4) : 4096;
    HRESULT hr = Dx9Device()->CreateVertexBuffer(nBufSize, D3DUSAGE_WRITEONLY,
                                                 0, D3DPOOL_DEFAULT,
                                                 &m_pZeroVertexBuffer, NULL);
    if (!FAILED(hr)) {
      void *pData = NULL;
...
VertexFormat_t CMeshMgr::ComputeVertexFormat(unsigned int flags,
  ...
  // texcoord slots
  nTexCoordArraySize =
      min(nTexCoordArraySize, (int)VERTEX_MAX_TEXTURE_COORDINATES);
...
int CMeshMgr::GetMaxVerticesToRender(IMaterial *pMaterial) {
  ...
  if (nVertexSize == 0) {
    // unable to determine vertex format information, possibly due to device
    // loss.
...
void CMeshMgr::SetVertexIDStreamState() {
  ...
  // interfaces. MESHFIXME : This path is only used for the new index/vertex
  // buffer interfaces.
...
void CMeshMgr::SetVertexStreamState(int nVertOffsetInBytes, int nVertexStride) {
  ...
  if (HardwareConfig()->SupportsVertexAndPixelShaders()) {
    // Set a 4kb all-zero static VB into the flex/wrinkle stream with a stride
    // of 0 bytes, so the vertex shader always reads valid floating point values
    // (otherwise it can get NaN's/Inf's, and under OpenGL this is bad on
    // NVidia) togl requires non-zero strides, but on D3D9 we can set a stride
    // of 0 for a little more efficiency.
    D3DSetStreamSource(2, g_MeshMgr.GetZeroVertexBuffer(), 0,
                       IsOpenGL() ? 4 : 0);
  ...
  //	for ( int iPrim=0; iPrim < s_nPrims; iPrim++ )
  {
    ...
    if (m_PrimitiveType == MATERIAL_POINTS) {
      ...
      //)( m_nNumIndices / 3 ) );
      {
        ...
        //"DrawIndexedPrimitive", 1 ); 				VPROF_INCREMENT_COUNTER( "numPrimitives",
        //numPrimitives );
        ...
        Dx9Device()->DrawIndexedPrimitive(
            ComputeMode(
                m_PrimitiveType), // Member of the D3DPRIMITIVETYPE enumerated
                                  // type, describing the type of primitive to
                                  // render. D3DPT_POINTLIST is not supported
                                  // with this method.

            /*m_FirstIndex*/ 0, // Offset from the start of the vertex buffer to
                                // the first vertex index. An index of 0 in the
                                // index buffer refers to this location in the
                                // vertex buffer.
            ...
            m_nFirstIndex /*pPrim->m_FirstIndex*/, // Index of the first index
                                                   // to use when accesssing the
                                                   // vertex buffer. Beginning
                                                   // at StartIndex to index
                                                   // vertices from the vertex
                                                   // buffer.
