...
#define X360_INDEX_BUFFER_SIZE_MULTIPLIER                                      \
  4.0 // minimum of 1, only affects dynamic buffers
// #define X360_BLOCK_ON_IB_FLUSH //uncomment to block until all data is
...
class CIndexBuffer {
public:
  CIndexBuffer(IDirect3DDevice9 *pD3D, int count,
...
#if _X360
  // For some IBs, memory allocation is managed by CGPUBufferAllocator, via
  // ShaderAPI
...
private:
  ...
  int m_iAllocationCount; // The total number of indices the buffer we allocated
                          // can hold. Usually greater than the number of
                          // indices asked for
  IDirect3DIndexBuffer9 m_D3DIndexBuffer; // Only need one shared D3D header for
                                          // our usage patterns.
...
inline CIndexBuffer::CIndexBuffer(IDirect3DDevice9 *pD3D, int count,
    ...
    : m_pIB(0), m_Position(0), m_bFlush(true), m_bLocked(false),
      m_bExternalMemory(false), m_bDynamic(dynamic),
      m_bSoftwareVertexProcessing(bSoftwareVertexProcessing),
      m_bLateCreateShouldDiscard(false)
#ifdef _X360
      ,
      m_pAllocatedMemory(NULL), m_iNextBlockingPosition(0),
  ...
  if (g_pShaderUtil->GetThreadMode() != MATERIAL_SINGLE_THREADED ||
      !ThreadInMainThread()) {
...
if (m_bDynamic) {
...
} else if (MeshMgr()->AllocatePooledIB(this, nBufferSize,
  ...
  m_iAllocationCount = count;
  m_pAllocatedMemory = (unsigned char *)XPhysicalAlloc(
      nBufferSize, MAXULONG_PTR, 0, PAGE_READWRITE);
}
...
void CIndexBuffer::Create(IDirect3DDevice9 *pD3D) {
...
#if !defined(_X360)
  HRESULT hr = pD3D->CreateIndexBuffer(m_IndexCount * IndexSize(), desc.Usage,
  ...
  if ((hr == D3DERR_OUTOFVIDEOMEMORY) || (hr == E_OUTOFMEMORY)) {
    ...
    hr = pD3D->CreateIndexBuffer(m_IndexCount * IndexSize(), desc.Usage,
                                 desc.Format, desc.Pool, &m_pIB, NULL);
  }
...
inline CIndexBuffer::CIndexBuffer()
    : m_pIB(0), m_Position(0), m_bFlush(false), m_bLocked(false),
      m_bExternalMemory(true), m_bDynamic(false)
#ifdef VPROF_ENABLED
      ,
      m_Frame(-1)
#endif
...
inline void CIndexBuffer::Init(IDirect3DDevice9 *pD3D, uint16 *pIndexMemory,
                               int count) {
  m_IndexCount = count;
  m_Position = count;
...
inline CIndexBuffer::~CIndexBuffer() {
  ...
  if (m_pAllocatedMemory && !m_bExternalMemory) {
    if (IsPooled()) {
      ...
      MemAlloc_RegisterExternalDeallocation(XMem_CIndexBuffer,
                                            m_pAllocatedMemory,
                                            XPhysicalSize(m_pAllocatedMemory));
      if (!m_bDynamic) {
        // Track non-pooled physallocs, to help tune CGPUBufferAllocator usage
...
//-----------------------------------------------------------------------------
inline void CIndexBuffer::SetBufferAllocationHandle(
    const GPUBufferHandle_t &bufferAllocationHandle) {
  // This IB's memory has been reallocated or freed, update our cached pointer
...
inline void CIndexBuffer::BlockUntilUnused(int nAllocationSize) {
...
#ifdef _X360
  Assert(
      (m_AllocationRing.Count() != 0) ||
      ((m_Position == 0) && (m_iNextBlockingPosition == m_iAllocationCount)));
  ...
  if (Dx9Device()->IsFencePending(FinalFence)) {
    ...
    if ((Dx9Device()->GetDeviceState() & D3DDEVICESTATE_ZPASS_BRACKET) &&
      ...
      // We're about to overrun our IB ringbuffer in a single Z prepass. To
...
// lock, unlock
//-----------------------------------------------------------------------------
inline unsigned short *CIndexBuffer::Lock(bool bReadOnly, int numIndices,
  ...
  if (m_bDynamic) {
    // startPosition now can be != -1, when calling in here with a static
    // (staging) buffer.
#if !defined(_X360)
  ...
  if (m_pSysmemBuffer != NULL) {
    ...
    pLockedData =
        (unsigned short *)(m_pSysmemBuffer + (position * IndexSize()));
  } else {
  ...
  pLockedData =
      (unsigned short *)(m_pAllocatedMemory + (position * IndexSize()));

#endif

  switch (hr) {
  case D3DERR_INVALIDCALL:
    Msg("D3DERR_INVALIDCALL - Index Buffer Lock Failed in %s on line %d(offset "
        "%d, size %d, flags 0x%x)\n",
...
inline void CIndexBuffer::Unlock(int numIndices) {
...
#if DX_TO_GL_ABSTRACTION
    // Knowing how much data was actually written is critical for performance
    // under OpenGL. Important notes: numIndices indicates how much we could
    // move the current position. For dynamic buffer, it should indicate the #
    // of actually written indices, for static buffers it's typically 0. If it's
    // a dynamic buffer (where we actually care about perf), assume the caller
    // isn't lying about numIndices, otherwise just assume they wrote the entire
    // thing. If you modify this code, be sure to test on both AMD and NVidia
    // drivers!
    Assert(numIndices <= (int)m_LockedNumIndices);
    int unlockBytes =
        (m_bDynamic ? numIndices : m_LockedNumIndices) * IndexSize();
#else
    int unlockBytes = 0;
#endif
  ...
  if (m_bDynamic) {
    Assert(
        (m_Position == 0) ||
        (m_AllocationRing[m_AllocationRing.Tail()].m_iEndOffset == m_Position));
    ...
    LockData.m_Fence =
        Dx9Device()->GetCurrentFence(); // This isn't the correct fence, but
                                        ...
                                        // and it'll provide marginal safety if
    ...
    Assert((LockData.m_iStartOffset == 0) ||
           (LockData.m_iStartOffset ==
            m_AllocationRing[m_AllocationRing.Tail()].m_iEndOffset));
    m_AllocationRing.AddToTail(LockData);

    void *pLockedData =
        m_pAllocatedMemory + (LockData.m_iStartOffset * IndexSize());

    // Always re-use the same index buffer header based on the assumption that
    // D3D copies it off in the draw calls.
    m_pIB = &m_D3DIndexBuffer;
    XGSetIndexBufferHeader(numIndices * IndexSize(), 0, D3DFMT_INDEX16, 0, 0,
                           m_pIB);
    ...
    if (!m_pIB) {
      int nBufferSize = m_IndexCount * IndexSize();
      XGSetIndexBufferHeader(nBufferSize, 0, D3DFMT_INDEX16, 0, 0,
                             &m_D3DIndexBuffer);
    ...
    // Invalidate the GPU caches for this memory.
    Dx9Device()->InvalidateGpuCache(m_pAllocatedMemory,
                                    m_IndexCount * IndexSize(), 0);
  }
#endif
...
inline void CIndexBuffer::HandleLateCreation() {
  ...
  void *pWritePtr = NULL;
  const int dataToWriteBytes =
      (m_Position * IndexSize()) - m_nSysmemBufferStartBytes;
  ...
  HRESULT hr = m_pIB->Lock(m_nSysmemBufferStartBytes, dataToWriteBytes,
                           &pWritePtr, dwFlags);
