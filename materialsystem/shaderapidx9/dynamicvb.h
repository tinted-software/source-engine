...
// #define X360_BLOCK_ON_VB_FLUSH //uncomment to block until all data is
// consumed when a flush is requested. Otherwise we only block when absolutely
// necessary
...
class CVertexBuffer {
public:
  CVertexBuffer(IDirect3DDevice9 *pD3D, VertexFormat_t fmt, DWORD theFVF,
...
#ifdef _X360
  CVertexBuffer();
  void Init(IDirect3DDevice9 *pD3D, VertexFormat_t fmt, DWORD theFVF,
            uint8 *pVertexData, int vertexSize, int theVertexCount);
#endif

  ~CVertexBuffer();

  LPDIRECT3DVERTEXBUFFER GetInterface() const {
    // If this buffer still exists, then Late Creation didn't happen. Best case:
    // we'll render the wrong image. Worst case: Crash.
...
#ifdef _X360
  // For some VBs, memory allocation is managed by CGPUBufferAllocator, via
  // ShaderAPI
  const GPUBufferHandle_t *GetBufferAllocationHandle(void);
  void
  SetBufferAllocationHandle(const GPUBufferHandle_t &bufferAllocationHandle);
    ...
    if (m_AllocationRing.Count()) {
      ...
      if (m_AllocationRing.IsValidIndex(iNode)) {
        ...
        if (iEndFreeOffset < m_Position) {
          // Wrapped. Making the arbitrary decision that the return value for
          // this function *should* handle the singe giant allocation case which
  ...
  void MarkUsedInRendering() {
    ...
    if (m_bDynamic && m_pVB) {
      ...
      m_AllocationRing[m_AllocationRing.Tail()].m_Fence =
          Dx9Device()->GetCurrentFence();
    }
#endif
  }
...
private:
  ...
  inline void ReallyUnlock(int unlockBytes) {
    ...
    // Knowing how much data was actually written is critical for performance
    // under OpenGL.
...
#else
    // X360BUG: forcing all locks to gpu flush, otherwise bizarre mesh
    // corruption on decals Currently iterating with microsoft 360 support to
    // track source of gpu corruption
  ...
  int m_iNextBlockingPosition; // m_iNextBlockingPosition >= m_Position where
                               // another allocation is still in use.
  unsigned char *m_pAllocatedMemory;
  int m_iAllocationSize; // Total size of the ring buffer, usually more than
                         // what was asked for
  IDirect3DVertexBuffer9 m_D3DVertexBuffer; // Only need one shared D3D header
                                            // for our usage patterns.
  CUtlLinkedList<DynamicBufferAllocation_t>
      m_AllocationRing; // tracks what chunks of our memory are potentially
                        // still in use by D3D

  GPUBufferHandle_t
      m_GPUBufferHandle; // Handle to a memory allocation within a shared
                         // physical memory pool (see CGPUBufferAllocator)
#endif

  VertexFormat_t
      m_VertexBufferFormat; // yes, Vertex, only used for allocation tracking
  int m_nBufferSize;
  int m_Position;
  int m_VertexCount;
  int m_VertexSize;
...
//-----------------------------------------------------------------------------
inline CVertexBuffer::CVertexBuffer(IDirect3DDevice9 *pD3D, VertexFormat_t fmt,
                                    DWORD theFVF, int vertexSize,
                                    int vertexCount,
                                    const char *pTextureBudgetName,
                                    bool bSoftwareVertexProcessing,
                                    bool dynamic)
    : m_pVB(0), m_Position(0), m_VertexSize(vertexSize),
      m_VertexCount(vertexCount), m_bFlush(true), m_bLocked(false),
      m_bExternalMemory(false), m_nBufferSize(vertexSize * vertexCount),
      m_TheFVF(theFVF), m_bSoftwareVertexProcessing(bSoftwareVertexProcessing),
      m_bDynamic(dynamic), m_VertexBufferFormat(fmt),
      m_bLateCreateShouldDiscard(false)
#ifdef _X360
      ,
      m_pAllocatedMemory(NULL), m_iNextBlockingPosition(0), m_iAllocationSize(0)
#endif
#ifdef VPROF_ENABLED
      ,
      m_Frame(-1)
...
void CVertexBuffer::Create(IDirect3DDevice9 *pD3D) {
  ...
  if (m_bDynamic) {
    ...
    m_pAllocatedMemory = (unsigned char *)XPhysicalAlloc(
        m_iAllocationSize, MAXULONG_PTR, 0,
        PAGE_READWRITE | MEM_LARGE_PAGES | PAGE_WRITECOMBINE);
  } else if (MeshMgr()->AllocatePooledVB(this, m_nBufferSize,
                                         pTextureBudgetName)) {
    // Successfully allocated in a shared ShaderAPI memory pool
    // (SetBufferAllocationHandle will have been called to set the pointer and
    // stream offset)
    m_iAllocationSize = m_nBufferSize;
    Assert(m_pAllocatedMemory);
  } else {
    // Fall back to allocating a standalone VB
    // NOTE: write-combining (PAGE_WRITECOMBINE) is deliberately not used, since
    // it slows down CPU access to the data (decals+defragmentation)
    m_iAllocationSize = m_nBufferSize;
    m_pAllocatedMemory = (unsigned char *)XPhysicalAlloc(
        m_iAllocationSize, MAXULONG_PTR, 0, PAGE_READWRITE);
  }

  if (m_pAllocatedMemory && !IsPooled()) {
    MemAlloc_RegisterExternalAllocation(XMem_CVertexBuffer, m_pAllocatedMemory,
...
#else
  g_VBAllocTracker->CountVB(this, m_bDynamic, m_iAllocationSize, m_VertexSize,
                            m_VertexBufferFormat);
#endif
}
...
inline CVertexBuffer::CVertexBuffer()
    : m_pVB(0), m_Position(0), m_VertexSize(0), m_VertexCount(0),
      m_bFlush(false), m_bLocked(false), m_bExternalMemory(true),
      m_nBufferSize(0), m_bDynamic(false)
#ifdef VPROF_ENABLED
      ,
      m_Frame(-1)
#endif
...
inline void CVertexBuffer::Init(IDirect3DDevice9 *pD3D, VertexFormat_t fmt,
                                DWORD theFVF, uint8 *pVertexData,
  ...
  m_pVB = new (AllocateTempBuffer(sizeof(IDirect3DVertexBuffer9)))
...
inline CVertexBuffer::~CVertexBuffer() {
  ...
  if (m_pAllocatedMemory && !m_bExternalMemory) {
    if (IsPooled()) {
      ...
      MemAlloc_RegisterExternalDeallocation(XMem_CVertexBuffer,
                                            m_pAllocatedMemory,
                                            XPhysicalSize(m_pAllocatedMemory));
      if (!m_bDynamic) {
        // Track non-pooled physallocs, to help tune CGPUBufferAllocator usage
...
//-----------------------------------------------------------------------------
inline void CVertexBuffer::SetBufferAllocationHandle(
    const GPUBufferHandle_t &bufferAllocationHandle) {
  // This VB's memory has been reallocated or freed, update our cached pointer
  // and the D3D header NOTE: this should never be called while any rendering is
  // in flight!
...
#if defined(_X360)
  return numVertices <=
         m_VertexCount; // the ring buffer will free room as needed
#else
...
inline void CVertexBuffer::BlockUntilUnused(int nBufferSize) {
  ...
  Assert((m_AllocationRing.Count() != 0) ||
         ((m_Position == 0) && (m_iNextBlockingPosition == m_iAllocationSize)));

  if ((m_iNextBlockingPosition - nLockOffset) >= nBufferSize)
    return;

  Assert((m_AllocationRing[m_AllocationRing.Head()].m_iStartOffset == 0) ||
         ((m_iNextBlockingPosition ==
           m_AllocationRing[m_AllocationRing.Head()].m_iStartOffset) &&
  ...
  if (iMinBlockPosition > m_iAllocationSize) {
    ...
    // modify the last allocation so that it uses up the whole tail end of the
    // buffer. Makes other code simpler
    Assert(m_AllocationRing.Count() != 0);
    m_AllocationRing[m_AllocationRing.Tail()].m_iEndOffset = m_iAllocationSize;

    // treat all allocations between the current position and the tail end of
    // the ring as freed since they will be before we unblock
  ...
  // now we go through the allocations until we find the last fence we care
  // about. Treat everything up until that fence as freed.
  DWORD FinalFence = 0;
  ...
  if (Dx9Device()->IsFencePending(FinalFence)) {
    ...
    if ((Dx9Device()->GetDeviceState() & D3DDEVICESTATE_ZPASS_BRACKET) &&
      ...
      // We're about to overrun our VB ringbuffer in a single Z prepass. To
      // avoid rendering corruption, close out the Z prepass and continue. This
      // will reduce early-Z rejection efficiency and could cause a momentary
...
inline unsigned char *CVertexBuffer::Lock(int numVerts, int &baseVertexIndex) {
  ...
  if (IsX360() && m_bDynamic) {
    // Block until we have enough room in the buffer, this affects the result of
    // NextLockOffset() in wrap conditions.
...
#if !defined(_X360)
  // If the caller isn't in the thread that owns the render lock, need to return
  // a system memory pointer--cannot talk to GL from the non-current thread.
  if (!m_pSysmemBuffer && !g_pShaderUtil->IsRenderThreadSafe()) {
    m_pSysmemBuffer = (byte *)MemAlloc_AllocAligned(m_nBufferSize, 16);
  ...
  if (m_pSysmemBuffer != NULL) {
    // Ensure that we're never moving backwards in a buffer--this code would
    // need to be rewritten if so. We theorize this can happen if you hit the
    // end of a buffer and then wrap before drawing--but this would probably
    // break in other places as well.
    Assert(nLockOffset >= m_nSysmemBufferStartBytes);
    pLockedData = m_pSysmemBuffer + nLockOffset;
  } else {
    m_pVB->Lock(nLockOffset, nBufferSize,
...
inline unsigned char *CVertexBuffer::Modify(bool bReadOnly, int firstVertex,
                                            int numVerts) {
  unsigned char *pLockedData = 0;
  ...
  Assert(
      !m_pSysmemBuffer); // if this hits, then we need to add code to handle it
...
#if !defined(_X360)
  m_pVB->Lock(firstVertex * m_VertexSize, numVerts * m_VertexSize,
              reinterpret_cast<void **>(&pLockedData), dwFlags);
#else
...
inline void CVertexBuffer::Unlock(int numVerts) {
  ...
  if (m_pSysmemBuffer != NULL) {
    ...
    int unlockBytes =
        (m_bDynamic ? nBufferSize : (m_nLockCount * m_VertexSize));
#else
    int unlockBytes = 0;
#endif
  ...
  if (m_bDynamic) {
    if (numVerts > 0) {
      ...
      LockData.m_Fence =
          Dx9Device()->GetCurrentFence(); // This isn't the correct fence, but
                                          // it's all we have access to for now
                                          // and it'll provide marginal safety
                                          // if something goes really wrong.
      LockData.m_iStartOffset = nLockOffset;
      LockData.m_iEndOffset = LockData.m_iStartOffset + nBufferSize;
      LockData.m_iZPassIdx =
          (Dx9Device()->GetDeviceState() & D3DDEVICESTATE_ZPASS_BRACKET)
              ? ShaderAPI()->Get360ZPassCounter()
              : 0;
      ...
      // Always re-use the same vertex buffer header based on the assumption
      // that D3D copies it off in the draw calls.
...
inline void CVertexBuffer::HandleLateCreation() {
  ...
  void *pWritePtr = NULL;
  const int dataToWriteBytes = m_bDynamic
                                   ? (m_Position - m_nSysmemBufferStartBytes)
                                   : (m_nLockCount * m_VertexSize);
  DWORD dwFlags = D3DLOCK_NOSYSLOCK;
  if (m_bDynamic) {
    dwFlags |=
        (m_bLateCreateShouldDiscard ? D3DLOCK_DISCARD : D3DLOCK_NOOVERWRITE);
  }
  ...
  HRESULT hr = m_pVB->Lock(m_nSysmemBufferStartBytes, dataToWriteBytes,
                           &pWritePtr, dwFlags);

  // If this fails we're about to crash. Consider skipping the update and
  // leaving m_pSysmemBuffer around to try again later. (For example in case of
  // device loss)
  Assert(SUCCEEDED(hr));
  hr;
  memcpy(pWritePtr, m_pSysmemBuffer + m_nSysmemBufferStartBytes,
         dataToWriteBytes);
