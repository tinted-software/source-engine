...
// Set this to 1 to get vprof nodes for playing back the command stream.  This
// is good for counting calls in a frame, etc.
...
#else
      m_pASyncThreadHandle =
          (uintptr_t)CreateSimpleThread(OurThreadInit, this, 128 * 1024);
#endif
    }
  } else {
    Synchronize();
...
void D3DDeviceWrapper::ExecutePushBuffer(PushBuffer const *pb) {
  ...
  for (;;) {
    ...
    case PBCMD_SET_STREAM_SOURCE: {
      ...
      Dx9Device()->SetStreamSource(dptr[1],
                                   (IDirect3DVertexBuffer9 *)FetchPtr(dptr + 2),
                                   dptr[3], dptr[4]);
