...
struct HardwareCaps_t : public MaterialAdapterInfo_t {
  ...
  unsigned long
      m_AlphaToCoverageEnableValue; // Value to set above state to turn on Alpha
                                    // To Coverage  (vendor-dependent)
  unsigned long
  ...
  bool m_bDX10Card : 1; // Indicates DX10 part with performant vertex textures
                        // running DX9 path
  bool m_bDX10Blending : 1; // Indicates DX10 part that does DX10 blending (but
                            // may not have performant vertex textures, such as
                            // Intel parts)
  bool m_bSupportsStaticControlFlow : 1; // Useful on OpenGL, where we have a
                                         // mix of support...
  bool m_FakeSRGBWrite : 1; // Gotta do this on OpenGL.  Mostly hidden, but some
                            // high level code needs to know
  bool m_CanDoSRGBReadFromRTs : 1; // Gotta do this on OpenGL.  Mostly hidden,
                                   // but some high level code needs to know
  bool m_bSupportsGLMixedSizeTargets : 1; // on OpenGL, are mixed size depth
                                          // buffers supported - aka
                                          // ARB_framebuffer_object
  bool m_bCanStretchRectFromTextures : 1; // Does the device expose
