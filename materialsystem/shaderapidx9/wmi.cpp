...
#pragma warning(                                                               \
    disable : 4805) // VS 2013 warning: warning C4805: '==' : unsafe mix of type
...
uint64 GetVidMemBytes(void) {
  ...
  while (pEnumerator) {
    ...
    if (SUCCEEDED(hr)) {
      nBytes =
          vtProp
              .ulVal; // Video RAM in bytes, AdatperRam is returned as the I4
                      // type so we read it out as unsigned int, see
