...
static void ccs_create_convar_from_hwconfig(const char *pVal,
                                            const char *pName) {
  ConVar *pConVar = g_pCVar->FindVar(pName);
  if (pConVar) {
...
CON_COMMAND_F(ccs_create_convars_from_hwconfig,
              "Create convars from the current hardware config, useful for "
              "diffing purposes",
              FCVAR_CHEAT) {
...
#ifdef POSIX
  GLMPRINTF((
      " CHardwareConfig::CHardwareConfig setting m_bHDREnabled to false on %8x",
      this));
#endif
...
bool CHardwareConfig::GetHDREnabled(void) const {
  //	printf("\n CHardwareConfig::GetHDREnabled returning m_bHDREnabled value
  //of %s on %8x", m_bHDREnabled?"true":"false", this );
  return m_bHDREnabled;
}

void CHardwareConfig::SetHDREnabled(bool bEnable) {
  //	printf("\n CHardwareConfig::SetHDREnabled setting m_bHDREnabled to value
  //of %s on %8x", bEnable?"true":"false",  this );
...
void CHardwareConfig::ForceCapsToDXLevel(HardwareCaps_t *pCaps, int nDxLevel,
  ...
  case 90:
    ...
    if (IsOpenGL()) {
      // FIXME this is way too complicated, we should just check the caps bit
      // from GLM
      ...
      {
        // rbarris 03Feb10: this is now hardwired because we are defaulting GLSL
        // mode "on". so this will mean that the engine will always ask for user
        // clip planes. this will misbehave under ARB mode, since ARB shaders
        // won't respect that state. it's difficult to make this fluid without
        // teaching the engine about a cap that could change during run.
      ...
      pCaps->m_MaxNumLights = pCaps->m_SupportsPixelShaders_2_b
                                  ? 4
                                  : 2; // 2b gets four lights, 2.0 gets two...
    }
  ...
  case 92:
    ...
    if (IsOpenGL()) {
      ...
      pCaps->m_MaxUserClipPlanes = 2;
      pCaps->m_UseFastClipping = false;
      pCaps->m_MaxNumLights = pCaps->m_bSupportsStaticControlFlow
                                  ? MAX_NUM_LIGHTS
                                  : (MAX_NUM_LIGHTS - 2);
    } else {
...
void CHardwareConfig::SetupHardwareCaps(int nDXLevel,
  ...
  if (IsOpenGL()) {
    m_Caps.m_MaxNumLights =
        MIN(m_Caps.m_MaxNumLights, (m_Caps.m_bSupportsStaticControlFlow &&
                                    m_Caps.m_SupportsPixelShaders_2_b)
                                       ? MAX_NUM_LIGHTS
                                       : (MAX_NUM_LIGHTS - 2));
...
static ConVar
    r_shader_srgb("r_shader_srgb", "0", FCVAR_ALLOWED_IN_COMPETITIVE,
                  "-1 = use hardware caps. 0 = use hardware srgb. 1 = use "
                  "shader srgb(software lookup)"); // -1=use caps 0=off 1=on
...
int CHardwareConfig::NeedsShaderSRGBConversion() const {
  ...
  default:
    return m_Caps.m_bDX10Blending; // !!! change to return false after portal
                                   // deport built!!!!!
  }
}
...
static ConVar mat_disablehwmorph("mat_disablehwmorph", "0",
...
int CHardwareConfig::GetDXSupportLevel() const {
  if (ShaderUtil()->GetConfig().dxSupportLevel != 0) {
    return min(ShaderUtil()->GetConfig().dxSupportLevel,
               m_Caps.m_nDXSupportLevel);
  }
...
const char *CHardwareConfig::GetShaderDLLName() const {
  return (m_Caps.m_pShaderDLL && m_Caps.m_pShaderDLL[0]) ? m_Caps.m_pShaderDLL
                                                         : "DEFAULT";
}
