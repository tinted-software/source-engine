...
public:
  AllocData(void *buffer, int bufferSize, VertexFormat_t fmt, int numVerts,
            int allocatorHash)
      : m_buffer(buffer), m_bufferSize(bufferSize), m_fmt(fmt),
        m_numVerts(numVerts), m_allocatorHash(allocatorHash) {}
  AllocData()
      : m_buffer(NULL), m_bufferSize(0), m_fmt(0), m_numVerts(0),
        m_allocatorHash(0) {}
...
static const ElementData positionElement = {
    ...
    "POSITION    "}; // (UNDONE: need vertex shader to scale, may cause cracking
                     // w/ static props)
static const ElementData normalElement = {
    VERTEX_ELEMENT_NORMAL, 12, 4, 4,
    "NORMAL      "}; // (UNDONE: PC (2x16-byte Ravi method) or 360
                     // (D3DDECLTYPE_HEND3N))
...
static const ElementData tangentSElement = {VERTEX_ELEMENT_TANGENT_S, 12, 12, 4,
                                            "TANGENT_S   "}; // (all-but-unused)
static const ElementData tangentTElement = {VERTEX_ELEMENT_TANGENT_T, 12, 12, 4,
                                            "TANGENT_T   "}; // (all-but-unused)
static const ElementData wrinkleElement = {
    VERTEX_ELEMENT_WRINKLE, 4, 4, 0,
    "WRINKLE     "}; // (UNDONE: compress it as a SHORTN in Position.w - is it
                     // [0,1]?)
...
static const ElementData texCoord2D0Element = {
    ...
    "TEXCOORD2D_0"}; // (UNDONE: need vertex shader to take scale, account for
                     // clamping)
static const ElementData texCoord2D1Element = {
    VERTEX_ELEMENT_TEXCOORD2D_1, 8, 8, 4,
    "TEXCOORD2D_1"}; // (UNDONE: need vertex shader to take scale, account for
                     // clamping)
...
static ConVar
    mem_vballocspew("mem_vballocspew", "0", FCVAR_CHEAT,
                    "How often to spew vertex buffer allocation stats - 1: "
                    "every alloc, 2+: every 2+ allocs, 0: off");
...
UtlHashFixedHandle_t
CVBAllocTracker::GetCounterHandle(const char *allocatorName,
                                  short allocatorHash) {
...
void CheckForElementTableUpdates(const ElementData &element) {
  ...
  if (tableIndex != element.element) {
    ...
    if (timesToSpew > 0) {
      Warning("VertexElement_t structure has changed, ElementData table in "
...
void CVBAllocTracker::SpewExpectedSavings(void) {
  ...
  // positions (+wrinkle) for studiomdls?	(issue: possible flex artifacts)
  // UNDONE: disable tangents for non-bumped models			(issue:
  ...
  if (IsX360()) {
    // We expect to avoid 4-KB-alignment wastage for color meshes, by allocating
    // them out of a single, shared VB and adding per-mesh offsets in vertex
    // shaders
    AddSaving(alreadySaved, yetToSave, "CColorMeshData::CreateResource",
              VERTEX_ELEMENT_USERDATA4, SAVING_ALIGNMENT);
  }
