...
CTransitionTable::CTransitionTable()
    : m_DefaultStateSnapshot(-1), m_CurrentShadowId(-1),
      m_CurrentSnapshotId(-1), m_TransitionOps(0, 8192),
      m_ShadowStateList(0, 256), m_TransitionTable(0, 256),
      m_SnapshotList(0, 256), m_ShadowStateDict(0, 256), m_SnapshotDict(0, 256),
...
// Creates a shadow, adding an entry into the shadow list and transition table
//-----------------------------------------------------------------------------
CTransitionTable::ShadowStateId_t
CTransitionTable::CreateShadowState(const ShadowState_t &currentState) {
...
//-----------------------------------------------------------------------------
CTransitionTable::ShadowStateId_t
CTransitionTable::FindShadowState(const ShadowState_t &currentState) const {
...
void ApplyShadowFilterEnable(const ShadowState_t &shaderState, int stage) {
  SetSamplerState(stage, D3DSAMP_SHADOWFILTER,
                  shaderState.m_SamplerState[stage].m_ShadowFilterEnable);
...
#if defined(_X360)
  // SetRenderStateConstMacro( D3DRS_HIZWRITEENABLE, shaderState.m_ZWriteEnable
...
void ApplyColorWriteEnable(const ShadowState_t &shaderState, int arg) {
  ...
  g_pTransitionTable->CurrentState().m_ColorWriteEnable =
      shaderState.m_ColorWriteEnable;
...
void CTransitionTable::ApplySRGBWriteEnable(const ShadowState_t &shaderState) {
  ...
  if (CurrentState().m_bLinearColorSpaceFrameBufferEnable) {
    ...
    if (HardwareConfig()->NeedsShaderSRGBConversion()) {
      if (HardwareConfig()
              ->SupportsPixelShaders_2_b()) // in 2b supported devices, we never
                                            // actually enable SRGB writes, but
                                            // instead handle the conversion in
                                            // the pixel shader. But we want all
                                            // other code to be unaware.
      {
        SetRenderStateConstMacro(D3DRS_SRGBWRITEENABLE, 0);
      } else {
        SetRenderStateConstMacro(D3DRS_SRGBWRITEENABLE,
...
void CTransitionTable::ApplySeparateAlphaBlend(const ShadowState_t &state) {
  if (m_CurrentState.m_SeparateAlphaBlendEnable !=
    ...
    SetRenderStateConstMacro(D3DRS_SEPARATEALPHABLENDENABLE,
                             state.m_SeparateAlphaBlendEnable);
...
int CTransitionTable::CreateNormalTransitions(const ShadowState_t &fromState,
  ...
  bool blendOpDifferent =
      toState.m_AlphaBlendEnable && (toState.m_BlendOp != fromState.m_BlendOp);
  if (bForce || blendOpDifferent || blendEnableDifferent || srcBlendDifferent ||
      destBlendDifferent) {
    AddTransition(RENDER_STATE_AlphaBlend);
    ++numOps;
  }

  // Shouldn't have m_SeparateAlphaBlendEnable set unless m_AlphaBlendEnable is
  // also set.
  ...
  bool blendOpAlphaDifferent =
      toState.m_SeparateAlphaBlendEnable &&
      (toState.m_BlendOpAlpha != fromState.m_BlendOpAlpha);
  if (bForce || blendOpAlphaDifferent || blendSeparateAlphaEnableDifferent ||
      srcBlendAlphaDifferent || destBlendAlphaDifferent) {
...
void CTransitionTable::CreateTransitionTableEntry(int to, int from) {
  ...
  unsigned int identicalListFirstElem =
...
//-----------------------------------------------------------------------------
unsigned int
CTransitionTable::FindIdenticalTransitionList(unsigned int firstElem,
...
void CTransitionTable::UseDefaultState() {
  ...
  SetRenderStateConstMacro(D3DRS_ALPHABLENDENABLE,
                           m_CurrentState.m_AlphaBlendEnable);
  ...
  SetRenderStateConstMacro(D3DRS_DESTBLENDALPHA,
...
#if defined(_X360)
  // SetRenderStateConstMacro( D3DRS_HIZENABLE, m_CurrentState.m_ZEnable ?
  // D3DHIZ_AUTOMATIC : D3DHIZ_DISABLE );
#endif
  ...
  SetRenderStateConstMacro(D3DRS_ALPHATESTENABLE,
                           m_CurrentState.m_AlphaTestEnable);
...
#ifdef DX_TO_GL_ABSTRACTION
    SetSamplerState(i, D3DSAMP_SHADOWFILTER,
                    SamplerState(i).m_ShadowFilterEnable);
#endif
  }
...
void CTransitionTable::OverrideDepthEnable(bool bEnable, bool bDepthEnable) {
  if (bEnable != m_CurrentState.m_bOverrideDepthEnable) {
    ...
    if (m_CurrentState.m_bOverrideDepthEnable) {
      ...
      SetRenderStateConstMacro(D3DRS_ZWRITEENABLE,
                               m_CurrentState.m_OverrideZWriteEnable);
#if defined(_X360)
      // SetRenderStateConstMacro( D3DRS_HIZWRITEENABLE,
...
void CTransitionTable::OverrideAlphaWriteEnable(bool bOverrideEnable,
  ...
  if (bOverrideEnable != m_CurrentState.m_bOverrideAlphaWriteEnable) {
    ...
    if (m_CurrentState.m_bOverrideAlphaWriteEnable) {
      ...
      if (CurrentShadowState()) {
        // probably being paranoid, but only copy the alpha flag from the shadow
        // state
        dwSetValue &= ~D3DCOLORWRITEENABLE_ALPHA;
...
void CTransitionTable::OverrideColorWriteEnable(bool bOverrideEnable,
  ...
  if (bOverrideEnable != m_CurrentState.m_bOverrideColorWriteEnable) {
    ...
    if (m_CurrentState.m_bOverrideColorWriteEnable) {
      if (m_CurrentState.m_bOverriddenColorWriteValue) {
        dwSetValue |= (D3DCOLORWRITEENABLE_RED | D3DCOLORWRITEENABLE_GREEN |
...
void CTransitionTable::PerformShadowStateOverrides() {
  ...
  if (m_CurrentState.m_bOverrideAlphaWriteEnable) {
    DWORD dwSetValue =
        m_CurrentState.m_ColorWriteEnable & ~D3DCOLORWRITEENABLE_ALPHA;
    dwSetValue |= m_CurrentState.m_bOverriddenAlphaWriteValue
                      ? D3DCOLORWRITEENABLE_ALPHA
                      : 0;
    if (dwSetValue != m_CurrentState.m_ColorWriteEnable) {
      m_CurrentState.m_ColorWriteEnable = dwSetValue;
