...
//-----------------------------------------------------------------------------
CreateInterfaceFn
CShaderDeviceMgrDx10::SetMode(void *hWnd, int nAdapter,
...
void CShaderDeviceDx10::SetHardwareGammaRamp(float fGamma,
  ...
  if (FAILED(hr)) {
    Warning("CShaderDeviceDx10::SetHardwareGammaRamp: Unable to set gamma "
            "controls!\n");
  }
}
...
IShaderBuffer *CShaderDeviceDx10::CompileShader(const char *pProgram,
  ...
  if (FAILED(hr)) {
    if (pErrorMessages) {
      ...
      Warning("Vertex shader compilation failed! Reported the following "
              "errors:\n%s\n",
              pErrorMessage);
...
CShaderDeviceDx10::CreateVertexShader(IShaderBuffer *pShaderBuffer) {
...
CShaderDeviceDx10::CreateGeometryShader(IShaderBuffer *pShaderBuffer) {
...
//-----------------------------------------------------------------------------
PixelShaderHandle_t
CShaderDeviceDx10::CreatePixelShader(IShaderBuffer *pShaderBuffer) {
...
//-----------------------------------------------------------------------------
ID3D10InputLayout *
CShaderDeviceDx10::GetInputLayout(VertexShaderHandle_t hShader,
...
IIndexBuffer *
CShaderDeviceDx10::GetDynamicIndexBuffer(MaterialIndexFormat_t fmt,
                                         bool bBuffered) {
  LOCK_SHADERAPI();
  return NULL;
}
