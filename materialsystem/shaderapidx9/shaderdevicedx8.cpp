...
#endif
// Define this if you want all d3d9 interfaces hooked and run through the
// dx9hook.h shim interfaces. For profiling, etc.
#define DO_DX9_HOOK
#endif
...
// HACK: need to pass knowledge of D3D9Ex usage into callers of D3D Create*
// methods so they do not try to specify D3DPOOL_MANAGED, which is unsupported
// in D3D9Ex
...
#if defined(PIX_INSTRUMENTATION) && defined(DX_TO_GL_ABSTRACTION) &&           \
    defined(_WIN32)
  m_hD3D9 = NULL;
  m_pBeginEvent = NULL;
  m_pEndEvent = NULL;
...
bool CShaderDeviceMgrDx8::Connect(CreateInterfaceFn factory) {
...
#if defined(IS_WINDOWS_PC) && defined(SHADERAPIDX9) && !defined(RECORDING) &&  \
    !defined(DX_TO_GL_ABSTRACTION)
  m_pD3D = NULL;
  ...
  if (HMODULE hMod = ::LoadLibraryA("d3d9.dll")) {
    ...
    if (CreateD3D9ExFunc_t pfnCreateD3D9Ex =
            (CreateD3D9ExFunc_t)::GetProcAddress(hMod, "Direct3DCreate9Ex")) {
      ...
      if ((*pfnCreateD3D9Ex)(D3D_SDK_VERSION, &pD3D9Ex) == S_OK && pD3D9Ex) {
        ...
        if (bD3D9ExForceDisable) {
          ...
          // The following is more "correct" but incompatible with the Steam
          // overlay:
...
#if defined(PIX_INSTRUMENTATION) && defined(DX_TO_GL_ABSTRACTION) &&           \
    defined(_WIN32)
  // This is a little odd, but AMD PerfStudio hooks D3D9.DLL and intercepts all
  // of the D3DPERF API's (even for OpenGL apps). So dynamically load d3d9.dll
  // and get the address of these exported functions.
  if (!m_hD3D9) {
  ...
  if (m_hD3D9) {
    ...
    m_pBeginEvent = (D3DPERF_BeginEvent_FuncPtr)GetProcAddress(
        m_hD3D9, "D3DPERF_BeginEvent");
    m_pEndEvent =
        (D3DPERF_EndEvent_FuncPtr)GetProcAddress(m_hD3D9, "D3DPERF_EndEvent");
    m_pSetMarker = (D3DPERF_SetMarker_FuncPtr)GetProcAddress(
        m_hD3D9, "D3DPERF_SetOptions");
    m_pSetOptions = (D3DPERF_SetOptions_FuncPtr)GetProcAddress(
        m_hD3D9, "D3DPERF_SetMarker");
  }
#endif
...
void CShaderDeviceMgrDx8::CheckVendorDependentShadowMappingSupport(
  ...
  // Set a default null texture format...may be overridden below by IHV-specific
  // surface type
...
#if defined(_X360)
  pCaps->m_ShadowDepthTextureFormat =
      ReverseDepthOnX360() ? IMAGE_FORMAT_X360_DST24F : IMAGE_FORMAT_X360_DST24;
  pCaps->m_bSupportsShadowDepthTextures = true;
  pCaps->m_bSupportsFetch4 = false;
  return;
#elif defined(DX_TO_GL_ABSTRACTION)
  // We may want to only do this on the higher-end Mac SKUs, since it's not
  // free...
  pCaps->m_ShadowDepthTextureFormat =
      IMAGE_FORMAT_NV_DST16; // This format shunts us down the right shader
                             // combo path
  ...
  if (IsPC() || !IsX360()) {
    bool bToolsMode =
        IsWindows() && (CommandLine()->CheckParm("-tools") != NULL);
        ...
        (pCaps->m_SupportsShaderModel_3_0)) // ps_3_0 parts from nVidia
    {
      ...
      //
      // NVIDIA has two no-PCF formats (these are not filtering modes, but
      // surface formats
      //   NVFMT_RAWZ is supported by NV4x (not supported here yet...requires a
      //   dp3 to reconstruct in shader code, which doesn't seem to work)
      //   NVFMT_INTZ is supported on newer chips as of G8x (just read like ATI
      //   non-fetch4 mode)
      //
      /*
                              if ( m_pD3D->CheckDeviceFormat( nAdapter,
         DX8_DEVTYPE, D3DFMT_X8R8G8B8, D3DUSAGE_RENDERTARGET, D3DRTYPE_TEXTURE,
         NVFMT_INTZ ) == S_OK )
                              {
                                      pCaps->m_ShadowDepthTextureFormat =
         IMAGE_FORMAT_NV_INTZ; pCaps->m_bSupportsFetch4 = false;
                                      pCaps->m_bSupportsShadowDepthTextures =
         true; return;
                              }
      */
      if (m_pD3D->CheckDeviceFormat(nAdapter, DX8_DEVTYPE, D3DFMT_X8R8G8B8,
...
void CShaderDeviceMgrDx8::CheckVendorDependentAlphaToCoverage(
  ...
  pCaps->m_AlphaToCoverageState =
      D3DRS_ADAPTIVETESS_Y; // Just match the NVIDIA state hackery
  return;
#endif
...
bool CShaderDeviceMgrDx8::ComputeCapsFromD3D(HardwareCaps_t *pCaps,
  ...
  // pipe has problems with sparse vertex buffers (it transforms the whole
  // thing.)
...
#ifdef OSX
  // Static control flow is disabled by default on OSX (the Mac version of togl
  // has known bugs preventing this path from working properly that we've fixed
  // in togl linux/win)
  pCaps->m_bSupportsStaticControlFlow =
      CommandLine()->CheckParm("-glslcontrolflow") != NULL;
#else
  pCaps->m_bSupportsStaticControlFlow =
      !CommandLine()->CheckParm("-noglslcontrolflow");
#endif
  ...
  pCaps->m_NumTextureStages =
      min(pCaps->m_NumTextureStages, (int)MAX_TEXTURE_STAGES);
  ...
  // OpenGL does not support this--at least not on OSX which is the primary GL
  // target, so just don't use that path on GL at all.
#if !defined(DX_TO_GL_ABSTRACTION)
  pCaps->m_bCanStretchRectFromTextures =
      ((caps.DevCaps2 & D3DDEVCAPS2_CAN_STRETCHRECT_FROM_TEXTURES) != 0) &&
      (pCaps->m_VendorID != VENDORID_INTEL);
#else
  ...
  if (IsOpenGL()) {
    ...
    pCaps->m_MaxNumLights =
        MIN(pCaps->m_MaxNumLights, (pCaps->m_bSupportsStaticControlFlow &&
                                    pCaps->m_SupportsPixelShaders_2_b)
                                       ? MAX_NUM_LIGHTS
                                       : (MAX_NUM_LIGHTS - 2));
  }
  ...
  if ( CommandLine()->CheckParm( "-nouserclip" ) /* || (IsOpenGL() && (!CommandLine()->FindParm("-glslmode"))) || r_emulategl.GetBool() */ )
	{
    // rbarris 03Feb10: this now ignores POSIX / -glslmode / r_emulategl because
    // we're defaulting GLSL mode "on". so this will mean that the engine will
    // always ask for user clip planes. this will misbehave under ARB mode,
    // since ARB shaders won't respect that state. it's difficult to make this
    // fluid without teaching the engine about a cap that could change during
    // run.
...
#ifdef DX_TO_GL_ABSTRACTION
  // using #if because we're referencing fields in the RHS which don't exist in
  // Windows headers for the caps9 struct
  ...
  if (IsPC() && pCaps->m_SupportsSRGB) {
    if (pCaps->m_VendorID == VENDORID_NVIDIA) {
      ...
      if (IsOpenGL()) {
        // If we're not the Quadro 4500 or GeForce 7x000, we're an NVIDIA DX10
        // part on MacOS
        if (!((pCaps->m_DeviceID == 0x009d) ||
              ((pCaps->m_DeviceID >= 0x0391) &&
               (pCaps->m_DeviceID <= 0x0395)))) {
          pCaps->m_bFogColorAlwaysLinearSpace = true;
          pCaps->m_bDX10Card = true;
          pCaps->m_bDX10Blending = true;
        }
      ...
      if (IsOpenGL()) {
        // If we're not a Radeon X1x00 (device IDs in this range), we're a DX10
        // chip
      ...
      // The two checks are for devices from GMA generation (0x29A2-0x2A43) and
      // HD graphics (0x0042-0x2500)
      pCaps->m_bDX10Blending =
          ((pCaps->m_DeviceID >= 0x29A2) && (pCaps->m_DeviceID <= 0x2A43)) ||
  ...
  bool bSupportsIntegerHDR =
      ...
      bSupportsInteger16Textures && pCaps->m_SupportsSRGB;

  // Do we have everything necessary to run with float HDR?
  bool bSupportsFloatHDR =
      pCaps->m_SupportsShaderModel_3_0 &&
      //		(caps.Caps3 & D3DCAPS3_ALPHA_FULLSCREEN_FLIP_OR_DISCARD)
      //&& 		(caps.PrimitiveMiscCaps & D3DPMISCCAPS_SEPARATEALPHABLEND) &&
      bSupportsFloat16Textures && bSupportsFloat16RenderTargets &&
  ...
  pCaps->m_bSupportsStreamOffset =
      ((caps.DevCaps2 &
        D3DDEVCAPS2_STREAMOFFSET) && // Tie these caps together since we want to
                                     // filter out
  ...
  // This may get more complex if we start using multiple flavors of compressed
  // vertex - for now it's "on or off"
  pCaps->m_SupportsCompressedVertices =
      (pCaps->m_nDXSupportLevel >= 90) && (pCaps->m_CanDoSRGBReadFromRTs)
          ? VERTEX_COMPRESSION_ON
          : VERTEX_COMPRESSION_NONE;
  if (CommandLine()->CheckParm(
          "-no_compressed_verts")) // m_CanDoSRGBReadFromRTs limits us to Snow
                                   // Leopard or later on OSX
  {
...
void CShaderDeviceMgrDx8::ComputeDXSupportLevel(HardwareCaps_t &caps) {
  ...
  if (caps.m_SupportsShaderModel_3_0 &&
      !bIsOpenGL) // Note that we don't tie vertex textures to 30 shaders
                  // anymore
  {
...
bool CShaderDeviceMgrDx8::ValidateMode(int nAdapter,
  ...
  if (info.m_bWindowed) {
    // windowed mode always appears on the primary display, so we should use
    // that adapter's settings
...
char *CShaderDeviceDx8::GetDisplayDeviceName() {
  if (m_sDisplayDeviceName.IsEmpty()) {
    D3DADAPTER_IDENTIFIER9 ident;
    // On Win10, this function is getting called with m_nAdapter still
    // initialized to -1. It's failing, and m_sDisplayDeviceName has garbage,
    // and tf2 fails to launch. To repro this, run "hl2.exe -dev -fullscreen
    // -game tf" on Win10.
...
void CShaderDeviceDx8::SpewDriverInfo() const {
  ...
  Warning("SEPARATETEXTUREMEMORIES %s TEXTURENONLOCALVIDMEM %s "
          "TEXTURESYSTEMMEMORY %s\n",
  ...
  Warning("COLORWRITEENABLE %s MASKZ %s TSSARGTEMP %s\n",
          (caps.PrimitiveMiscCaps & D3DPMISCCAPS_COLORWRITEENABLE) ? " Y "
                                                                   : " N ",
  ...
  Warning("m_bSupportsAnisotropicFiltering: %s\n",
          g_pHardwareConfig->Caps().m_bSupportsAnisotropicFiltering ? "yes"
                                                                    : "no");
  ...
  Warning("m_bDisableShaderOptimizations: %s\n",
          g_pHardwareConfig->Caps().m_bDisableShaderOptimizations ? "yes"
                                                                  : "no");
...
const char *GetD3DErrorText(HRESULT hr) {
  ...
  case D3DERR_WRONGTEXTUREFORMAT:
    pszMoreInfo = "D3DERR_WRONGTEXTUREFORMAT: The pixel format of the texture "
                  "surface is not valid.";
    break;
  case D3DERR_UNSUPPORTEDCOLOROPERATION:
    pszMoreInfo =
        "D3DERR_UNSUPPORTEDCOLOROPERATION: The device does not support a "
        "specified texture-blending operation for color values.";
    break;
  case D3DERR_UNSUPPORTEDCOLORARG:
    pszMoreInfo = "D3DERR_UNSUPPORTEDCOLORARG: The device does not support a "
                  "specified texture-blending argument for color values.";
    break;
  case D3DERR_UNSUPPORTEDALPHAOPERATION:
    pszMoreInfo =
        "D3DERR_UNSUPPORTEDALPHAOPERATION: The device does not support a "
        "specified texture-blending operation for the alpha channel.";
    break;
  case D3DERR_UNSUPPORTEDALPHAARG:
    pszMoreInfo = "D3DERR_UNSUPPORTEDALPHAARG: The device does not support a "
                  "specified texture-blending argument for the alpha channel.";
    break;
  case D3DERR_TOOMANYOPERATIONS:
    pszMoreInfo = "D3DERR_TOOMANYOPERATIONS: The application is requesting "
                  "more texture-filtering operations than the device supports.";
    break;
  case D3DERR_CONFLICTINGTEXTUREFILTER:
    pszMoreInfo = "D3DERR_CONFLICTINGTEXTUREFILTER: The current texture "
                  "filters cannot be used together.";
    break;
  case D3DERR_UNSUPPORTEDFACTORVALUE:
    pszMoreInfo = "D3DERR_UNSUPPORTEDFACTORVALUE: The device does not support "
                  "the specified texture factor value.";
    break;
  case D3DERR_CONFLICTINGRENDERSTATE:
    pszMoreInfo = "D3DERR_CONFLICTINGRENDERSTATE: The currently set render "
                  "states cannot be used together.";
    break;
  case D3DERR_UNSUPPORTEDTEXTUREFILTER:
    pszMoreInfo = "D3DERR_UNSUPPORTEDTEXTUREFILTER: The device does not "
                  "support the specified texture filter.";
    break;
  case D3DERR_CONFLICTINGTEXTUREPALETTE:
    pszMoreInfo = "D3DERR_CONFLICTINGTEXTUREPALETTE: The current textures "
  ...
  case D3DERR_DEVICELOST:
    pszMoreInfo = "D3DERR_DEVICELOST: The device has been lost but cannot be "
                  "reset at this time. Therefore, rendering is not possible.";
    break;
  case D3DERR_DEVICENOTRESET:
    pszMoreInfo = "D3DERR_DEVICENOTRESET: The device has been lost.";
    break;
  case D3DERR_NOTAVAILABLE:
    pszMoreInfo = "D3DERR_NOTAVAILABLE: This device does not support the "
                  "queried technique.";
    break;
  case D3DERR_OUTOFVIDEOMEMORY:
    pszMoreInfo =
        "D3DERR_OUTOFVIDEOMEMORY: Direct3D does not have enough display memory "
        "to perform the operation. The device is using more resources in a "
        "single scene than can fit simultaneously into video memory.";
    break;
  ...
  case D3DERR_WASSTILLDRAWING:
    pszMoreInfo =
        "D3DERR_WASSTILLDRAWING: The previous blit operation that is "
...
//-----------------------------------------------------------------------------
IDirect3DDevice9 *
CShaderDeviceDx8::InvokeCreateDevice(void *hWnd, int nAdapter,
...
#if 1 // with the changes for opengl to enable threading, we no longer need the
      // d3d device to have threading guards
#ifndef _X360
  // Create the device with multi-threaded safeguards if we're using
  // mat_queue_mode 2. The logic to enable multithreaded rendering happens well
  // after the device has been created, so we replicate some of that logic here.
  ConVarRef mat_queue_mode("mat_queue_mode");
  if (mat_queue_mode.GetInt() == 2 ||
      (mat_queue_mode.GetInt() == -2 &&
       GetCPUInformation()->m_nPhysicalProcessors >= 2) ||
      (mat_queue_mode.GetInt() == -1 &&
       GetCPUInformation()->m_nPhysicalProcessors >= 2)) {
  ...
  devType =
      CommandLine()->FindParm("-nulldevice") ? D3DDEVTYPE_NULLREF : devType;
#endif

  HRESULT hr = D3D()->CreateDevice(nAdapter, devType, (VD3DHWND)hWnd,
                                   deviceCreationFlags, &m_PresentParameters,
                                   &pD3DDevice);
  ...
  Sleep(1000);
  hr = D3D()->CreateDevice(nAdapter, devType, (VD3DHWND)hWnd,
                           deviceCreationFlags, &m_PresentParameters,
                           &pD3DDevice);
  if (!FAILED(hr) && pD3DDevice)
    return pD3DDevice;
...
#ifdef _WIN32
  case D3DERR_INVALIDCALL:
    // Override the error text for this error since it has a known meaning for
    // CreateDevice failures.
    pszMoreInfo =
        "D3DERR_INVALIDCALL: The device or the device driver may not support "
        "Direct3D or may not support the resolution or color depth specified.";
    break;
#endif // _WIN32
  default:
  ...
  if (pszMoreInfo) {
    DWarning("init", 0,
             ...
             "enduser/std_adp.php?p_faqid=772\n",
...
bool CShaderDeviceDx8::CreateD3DDevice(void *pHWnd, int nAdapter,
  ...
  if (D3DSupportsCompressedTextures()) {
    g_pHardwareConfig->ActualCapsForEdit().m_SupportsCompressedTextures =
...
void CShaderDeviceDx8::AllocFrameSyncObjects(void) {
  ...
  for (int i = 0; i < ARRAYSIZE(m_pFrameSyncQueryObject); i++) {
    HRESULT hr = Dx9Device()->CreateQuery(D3DQUERYTYPE_EVENT,
                                          &m_pFrameSyncQueryObject[i]);
...
void CShaderDeviceDx8::FreeFrameSyncObjects(void) {
  ...
  for (int i = 0; i < ARRAYSIZE(m_pFrameSyncQueryObject); i++) {
    if (m_pFrameSyncQueryObject[i]) {
      if (m_bQueryIssued[i]) {
        tmZone(TELEMETRY_LEVEL1, TMZF_NONE, "D3DQueryGetData %t",
        ...
        // Make every attempt (within 2 seconds) to get the result from the
        // query.  Doing so may prevent crashes in the driver if we try to
        // release outstanding queries.
        do {
          hr = m_pFrameSyncQueryObject[i]->GetData(
...
bool CShaderDeviceDx8::TryDeviceReset() {
  ...
  // Don't try to reset the device until we're sure our resources have been
  // released
  ...
  if (bResetSuccess && g_ShaderDeviceUsingD3D9Ex) {
    ...
    if (bResetSuccess) {
      Warning("video driver has crashed and been reset, re-uploading resources "
              "now");
    }
  }
#endif
...
void CShaderDeviceDx8::ReleaseResources() {
  if (!ThreadOwnsDevice() || !ThreadInMainThread()) {
    ...
    // We reset this in two places since release resources can be called without
    // a call to TryDeviceReset.
    m_bResourcesReleased = false;
  ...
  if (m_numReleaseResourcesRefCount++ != 0) {
    ...
    DevWarning("ReleaseResources called twice is a bug: use IsDeactivated to "
  ...
  // Signal that our resources have been released so that we can try to reset
  // the device
  ...
  if (bForceReacquire) {
    ...
    if (--m_numReleaseResourcesRefCount != 0) {
      Warning("ReacquireResources has no effect, now at level %d.\n",
              m_numReleaseResourcesRefCount);
      DevWarning("ReacquireResources being discarded is a bug: use "
                 "IsDeactivated to check for a valid device.\n");
      Assert(0);
...
bool CShaderDeviceDx8::AllocNonInteractiveRefreshObjects() {
  ...
  const char *strPixelShaderProgram3 =
      ...
      "		{"
      "			fl360GammaValue = flLinearValue * ( 1023.0f * ( 1.0f / "
      "255.0f ) );"
      "		}"
      "		else"
      "		{"
      "			fl360GammaValue = flLinearValue * ( ( 1023.0f / 2.0f ) "
      "* ( 1.0f / 255.0f ) ) + ( 32.0f / 255.0f );"
      "		}"
      "	}"
      "	else"
      "	{"
      ...
      "		{"
      "			fl360GammaValue = flLinearValue * ( ( 1023.0f / 4.0f ) "
      "* ( 1.0f / 255.0f ) ) + ( 64.0f / 255.0f );"
      "		}"
      "		else"
      "		{"
      "			fl360GammaValue = flLinearValue * ( ( 1023.0f /8.0f ) "
...
void CShaderDeviceDx8::EnableNonInteractiveMode(
...
#ifdef _X360
  if (mode != MATERIAL_NON_INTERACTIVE_MODE_NONE) {
    // HACK: VSync off (prevents us wasting time blocking on VSync due to our
    // irregular present intervals)
    Dx9Device()->SetRenderState(D3DRS_PRESENTINTERVAL,
                                D3DPRESENT_INTERVAL_IMMEDIATE);
  } else {
    // HACK: VSync on (defaulting to on on 360 is fine, but really should
    // save+restore this state)
  ...
  //		m_NonInteractiveRefresh.m_flPeakDt,
  //m_NonInteractiveRefresh.m_flTotalDt / m_NonInteractiveRefresh.m_nSamples,
  //m_NonInteractiveRefresh.m_nSamples, m_NonInteractiveRefresh.m_nCountAbove66
  //);
...
void CShaderDeviceDx8::RefreshFrontBufferNonInteractive() {
  ...
  IDirect3DBaseTexture *pTexture = g_pShaderAPI->GetD3DTexture(
      m_NonInteractiveRefresh.m_Info.m_hTempFullscreenTexture);
  ...
  bool bInStartupMode =
      (m_NonInteractiveRefresh.m_Mode == MATERIAL_NON_INTERACTIVE_MODE_STARTUP);

  float flDepth =
      (ShaderUtil()->GetConfig().bReverseDepth ^ ReverseDepthOnX360()) ? 0.0f
                                                                       : 1.0f;
  ...
  Dx9Device()->SetPixelShader(
      bInStartupMode ? m_NonInteractiveRefresh.m_pPixelShaderStartup
                     : m_NonInteractiveRefresh.m_pPixelShader);
  ...
  if (bInStartupMode) {
    ...
    if (flDt > 0.030f) {
      if (++m_NonInteractiveRefresh.m_nPacifierFrame >=
          m_NonInteractiveRefresh.m_Info.m_nPacifierCount) {
        m_NonInteractiveRefresh.m_nPacifierFrame = 0;
      }
    ...
    pTexture = g_pShaderAPI->GetD3DTexture(
        m_NonInteractiveRefresh.m_Info
            .m_pPacifierTextures[m_NonInteractiveRefresh.m_nPacifierFrame]);
...
void CShaderDeviceDx8::SetHardwareGammaRamp(float fGamma,
  ...
  for (int i = 0; i < 256; i++) {
    ...
    if (IsX360()) // Should we also do this for the PS3?
    {
      ...
      flSrgbGammaValue = SrgbLinearToGamma(
          flLinearValue); // ( flLinearValue <= 0.0031308f ) ? ( flLinearValue
                          // * 12.92f ) : ( 1.055f * powf( flLinearValue, ( 1.0f
                          // / 2.4f ) ) ) - 0.055f;
...
VertexShaderHandle_t
CShaderDeviceDx8::CreateVertexShader(IShaderBuffer *pBuffer) {
...
GeometryShaderHandle_t
CShaderDeviceDx8::CreateGeometryShader(IShaderBuffer *pShaderBuffer) {
  Assert(0);
...
PixelShaderHandle_t
CShaderDeviceDx8::CreatePixelShader(IShaderBuffer *pBuffer) {
