...
static D3DDECLTYPE
VertexElementToDeclType(VertexElement_t element,
...
void ComputeVertexSpec(VertexFormat_t fmt, D3DVERTEXELEMENT9 *pDecl,
  ...
  if (IsX360()) {
    // On 360, there's a performance penalty for reading more than 2 streams in
    // the vertex shader (we don't do this yet, but we should be aware if we
    // start doing it)
#ifdef _DEBUG
    int numStreams =
        1 + (bStaticLit ? 1 : 0) + (bUsingFlex ? 1 : 0) + (bUsingMorph ? 1 : 0);
    Assert(numStreams <= 2);
#endif
  }
...
IDirect3DVertexDeclaration9 *FindOrCreateVertexDecl(VertexFormat_t fmt,
  ...
  Assert(hr == D3D_OK);
  if (hr != D3D_OK) {
    Warning(" ERROR: failed to create vertex decl for vertex format 0x%08llX! "
            "You'll probably see messed-up mesh rendering - to diagnose, build "
