...
struct DynamicState_t {
  ...
  float m_DestAlphaDepthRange; // Dest alpha writes compress the depth to get
...
//-----------------------------------------------------------------------------
typedef void (*StateCommitFunc_t)(IDirect3DDevice9 *pDevice,
                                  const DynamicState_t &desiredState,
                                  DynamicState_t &currentState, bool bForce);
static void CommitSetViewports(IDirect3DDevice9 *pDevice,
                               const DynamicState_t &desiredState,
                               DynamicState_t &currentState, bool bForce);
...
struct Texture_t {
  ...
  int GetLodClamp() const { return m_FinestMipmapLevel; }
...
public:
  ...
  ImageFormat GetNearestSupportedFormat(ImageFormat fmt,
                                        bool bFilteringRequired = true) const;
  ...
  FORCEINLINE void
  AssertValidTextureHandle(ShaderAPITextureHandle_t textureHandle) {
#ifdef _DEBUG
  ...
  CreateTexture(int width, int height, int depth, ImageFormat dstImageFormat,
  ...
  void CopyTextureToRenderTargetEx(int nRenderTargetID,
                                   ...
                                   Rect_t *pDstRect = NULL);
  void CopyRenderTargetToScratchTexture(ShaderAPITextureHandle_t srcHandle,
                                        ShaderAPITextureHandle_t dstHandle,
                                        Rect_t *pSrcRect = NULL,
                                        Rect_t *pDstRect = NULL);
  ...
  virtual void CopyTextureToTexture(ShaderAPITextureHandle_t srcTex,
  ...
  const FlashlightState_t &
  GetFlashlightStateEx(VMatrix &worldToTexture,
  ...
  ShaderAPITextureHandle_t
  CreateRenderTargetSurface(int width, int height, ImageFormat format,
                            const char *pDebugName,
                            const char *pTextureGroupName);
  void PersistDisplay();
  bool PostQueuedTexture(const void *pData, int nSize,
                         ShaderAPITextureHandle_t *pHandles, int nHandles,
                         int nWidth, int nHeight, int nDepth, int nMips,
                         int *pRefCount);
  void *GetD3DDevice();
...
private:
  ...
  IDirect3DSurface *
  GetDepthTextureSurface(ShaderAPITextureHandle_t textureHandle);
  ...
  void
  CheckVendorDependentShadowMappingSupport(bool &bSupportsShadowDepthTextures,
                                           bool &bSupportsFetch4);
  ...
  struct DelayedConstants_t {
    ...
    void Invalidate(void) { iPixelShaderFogParams = -1; }
...
static int AdjustUpdateRange(float const *pVec, void const *pOut, int numVecs,
                             int *pSkip) {
  int skip = 0;
  uint32 *pSrc = (uint32 *)pVec;
  uint32 *pDst = (uint32 *)pOut;
  while (numVecs && !((pSrc[0] ^ pDst[0]) | (pSrc[1] ^ pDst[1]) |
  ...
  *pSkip = skip;
  ...
  while (numVecs > 1 &&
         !((pSrcLast[0] ^ pDstLast[0]) | (pSrcLast[1] ^ pDstLast[1]) |
           (pSrcLast[2] ^ pDstLast[2]) | (pSrcLast[3] ^ pDstLast[3]))) {
    pSrcLast -= 4;
    pDstLast -= 4;
    numVecs--;
  }
...
CShaderAPIDx8::CShaderAPIDx8()
    : m_Textures(32), m_CurrStack(-1),
      m_ModifyTextureHandle(INVALID_SHADERAPI_TEXTURE_HANDLE), m_pRenderMesh(0),
      m_nDynamicVBSize(DYNAMIC_VERTEX_BUFFER_MEMORY), m_pMaterial(NULL),
      m_CurrentFrame(0), m_CachedAmbientLightCube(STATE_CHANGED),
      m_InSelectionMode(false), m_SelectionMinZ(FLT_MAX),
      m_SelectionMaxZ(FLT_MIN), m_pSelectionBuffer(0), m_pSelectionBufferEnd(0),
      m_bResetRenderStateNeeded(false), m_ModifyTextureLockedLevel(-1),
      m_nPixFrame(0), m_bPixCapturing(false), m_nPIXErrorCount(0),
      m_pBackBufferSurface(0), m_pBackBufferSurfaceSRGB(0),
      m_pZBufferSurface(0), m_bResettingRenderState(false),
      m_bReadPixelsEnabled(false), m_ToneMappingScale(1.0f, 1.0f, 1.0f, 1.0f),
      ...
      m_fShadowSlopeScaleDepthBias(16.0f), m_fShadowDepthBias(0.00008f),
...
void CShaderAPIDx8::GetDXLevelDefaults(uint &max_dxlevel,
                                       uint &recommended_dxlevel) {
...
void CShaderAPIDx8::ReleaseInternalRenderTargets() {
  ...
  if (m_pBackBufferSurface) {
    ...
    int nRetVal = m_pBackBufferSurface->Release(
        0, "-B  CShaderAPIDx8::ReleaseInternalRenderTargets public release "
           "color buffer");
#else
    int nRetVal = m_pBackBufferSurface->Release();
#endif
  ...
  if (m_pZBufferSurface) {
    ...
    // Assert( nRetVal == 0 );		//FIXME not sure why we're seeing a
    // refcount of 3 here
...
bool CShaderAPIDx8::OnDeviceInit() {
  ...
  // Apply mandatory initialization HW fixups, GPU state will be left as
  // expected
  if (IsX360()) {
    // place the possible persisted display into the back buffer, ready for
    // present()
    ...
    // place the front buffer image in the back buffer, later systems will
    // detect and grab other systems will detect and grab
...
// Sets the mode...
//-----------------------------------------------------------------------------
bool CShaderAPIDx8::SetMode(void *VD3DHWND, int nAdapter,
  ...
  if (!bSetModeOnce) {
    nDXLevel = MAX(ABSOLUTE_MINIMUM_DXLEVEL,
                   CommandLine()->ParmValue("-dxlevel", nDXLevel));
    bSetModeOnce = true;
  }
...
//-----------------------------------------------------------------------------
static FORCEINLINE void SetSamplerState(IDirect3DDevice9 *pDevice, int stage,
...
#ifdef DX_TO_GL_ABSTRACTION
// Purposely always writing the new state (even if it's not changed), in case
// SetRenderStateConstInline() compiles away to nothing (it sometimes does)
...
// Commits viewports
//-----------------------------------------------------------------------------
static void CommitSetScissorRect(IDirect3DDevice9 *pDevice,
                                 const DynamicState_t &desiredState,
                                 DynamicState_t &currentState, bool bForce) {
  ...
  */
  ...
  if (g_pHardwareConfig->Caps().m_SupportsVertexShaders_2_0) {
    ...
    standardVertexShaderConstant.Init(
        ...
        // vConst.w should be 3, but due to about hack, mul by 255 and add
        // epsilon 360 supports UBYTE4, so no fixup required
...
//-----------------------------------------------------------------------------
static void CommitVertexTextures(IDirect3DDevice9 *pDevice,
                                 const DynamicState_t &desiredState,
                                 DynamicState_t &currentState, bool bForce) {
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    if (pTexture && (pTexture->m_Flags & Texture_t::IS_VERTEX_TEXTURE) == 0) {
      Warning("Attempting to bind a vertex texture (%s) which was not created "
              "as a vertex texture!\n",
...
void CShaderAPIDx8::ResetDXRenderState(void) {
  ...
  // -disable_d3d9_hacks is for debugging. For example, the "CENT" driver hack
  // thing causes the flashlight pass to appear much brighter on NVidia drivers.
  if (IsPC() && !IsOpenGL() &&
      !CommandLine()->CheckParm("-disable_d3d9_hacks")) {
    if (g_pHardwareConfig->Caps().m_bNeedsATICentroidHack &&
        (g_pHardwareConfig->Caps().m_VendorID == VENDORID_ATI)) {
...
bool CShaderAPIDx8::OwnGPUResources(bool bEnable) {
  ...
  if (!bEnable) {
    ...
    // there are 256, but the game only uses 217 (snapped to 220), leaving just
    // enough room for shader literals
    COMPILE_TIME_ASSERT(VERTEX_SHADER_MODEL + 3 * NUM_MODEL_TRANSFORMS == 217);
    Dx9Device()->GpuOwnVertexShaderConstantF(
        0, AlignValue(VERTEX_SHADER_MODEL + 3 * NUM_MODEL_TRANSFORMS, 4));
    // there are 256, but the game only utilizes 32, leaving lots of room for
    // shader literals
...
void CShaderAPIDx8::ResetRenderState(bool bFullReset) {
  ...
  m_WorldSpaceCameraPositon.Init(
      0, 0, 0.01f,
      0); // Don't let z be zero, as some pixel shaders will divide by this
  ...
  for (i = 0; i < g_pHardwareConfig->ActualCaps().m_NumSamplers; ++i) {
    ...
    SetSamplerState(i, D3DSAMP_MAXMIPLEVEL,
...
void CShaderAPIDx8::DrawMesh(CMeshBase *pMesh) {
  ...
  PIXifyName(s_pPIXMaterialName, sizeof(s_pPIXMaterialName),
             m_pMaterial->GetName());
...
void CShaderAPIDx8::UpdateFrameSyncQuery(int queryIndex, bool bIssue) {
  ...
  if (m_bQueryIssued[queryIndex]) {
    ...
    // gets lost
    for (;;) {
      hr = m_pFrameSyncQueryObject[queryIndex]->GetData(
          &dummyData, sizeof(dummyData), D3DGETDATA_FLUSH);
      if (hr != S_FALSE)
        break;
      ...
      if (flCurrTime - flStartTime > 0.200f)
        break;
      // Avoid burning a full core while waiting for the query. Spinning can
      // actually harm performance because there might be driver threads that
      // are trying to do work that end up starved, and the power drawn by the
      // CPU may take away from the power available to the integrated graphics
      // chip. A sleep of one millisecond should never be long enough to affect
      // performance, especially since this should only trigger when the CPU is
      // already ahead of the GPU. On L4D2/TF2 in GL mode this spinning was
      // causing slowdowns.
      ThreadSleep(1);
    }
...
void CShaderAPIDx8::ForceHardwareSync(void) {
  ...
  if (mat_frame_sync_force_texture.GetBool()) {
    ...
    int waitIndex = ((m_currentSyncQuery + NUM_FRAME_SYNC_QUERIES) -
                     (NUM_FRAME_SYNC_FRAMES_LATENCY + 1)) %
...
#if 0 // hack versions for OSX to be able to PIX log even when not built
      // debug...
...
#if defined(_WIN32)
  // AMD PerfStudio integration: Call into D3D9.DLL's D3DPERF_BeginEvent() (this
  // gets intercepted by PerfStudio even in GL mode).
...
#if defined(_WIN32)
  // AMD PerfStudio integration: Call into D3D9.DLL's D3DPERF_EndEvent() (this
  // gets intercepted by PerfStudio even in GL mode).
...
void CShaderAPIDx8::ApplyZBias(const ShadowState_t &shaderState) {
  ...
  if (g_pHardwareConfig->Caps().m_ZBiasAndSlopeScaledDepthBiasSupported) {
    ...
    SetRenderStateConstMacro(this, D3DRS_SLOPESCALEDEPTHBIAS,
...
void CShaderAPIDx8::ApplyTextureEnable(const ShadowState_t &state,
  ...
  if (state.m_SamplerState[nSampler].m_TextureEnable) {
    ...
    // Should not be necessary/possible (SetTextureState() calls
    // D3D9/DXAbstract, so the calling thread must already own the device.
    // LOCK_SHADERAPI();
...
VertexFormat_t CShaderAPIDx8::ComputeVertexUsage(int num,
  ...
  for (int i = num; --i >= 0;) {
    ...
    if ((compression != newCompression) &&
      ...
      Warning("Encountered a material with two passes that specify different "
    ...
    if ((numBones != newNumBones) && (newNumBones != 0)) {
      if (numBones != 0) {
        Warning("Encountered a material with two passes that use different "
                "numbers of bones!\n");
      }
...
//-----------------------------------------------------------------------------
static void CommitVertexShaderConstantRange(IDirect3DDevice9 *pDevice,
                                            const DynamicState_t &desiredState,
...
void CShaderAPIDx8::SetDepthFeatheringPixelShaderConstant(
  ...
  if (IsX360()) {
    ...
    */
    ...
    fConstantValues[0] =
        m_DynamicState.m_DestAlphaDepthRange / fDepthBlendScale;
...
void CShaderAPIDx8::OverrideAlphaWriteEnable(bool bOverrideEnable,
                                             bool bAlphaWriteEnable) {
  LOCK_SHADERAPI();
  if (!g_pShaderDeviceDx8->IsDeactivated()) {
    m_TransitionTable.OverrideAlphaWriteEnable(bOverrideEnable,
                                               bAlphaWriteEnable);
  }
}

void CShaderAPIDx8::OverrideColorWriteEnable(bool bOverrideEnable,
                                             bool bColorWriteEnable) {
  LOCK_SHADERAPI();
  if (!g_pShaderDeviceDx8->IsDeactivated()) {
    m_TransitionTable.OverrideColorWriteEnable(bOverrideEnable,
                                               bColorWriteEnable);
  }
}
...
void CShaderAPIDx8::EnableClipPlane(int index, bool bEnable) {
  ...
  if ((m_DynamicState.m_UserClipPlaneEnabled & (1 << index) ? true : false) !=
    ...
    SetRenderStateConstMacro(this, D3DRS_CLIPPLANEENABLE,
...
void CShaderAPIDx8::CommitFastClipPlane() {
  ...
  D3DXMATRIX viewToProjInvTrans, viewToProjInv,
      viewToProj = GetTransform(MATERIAL_PROJECTION);
  viewToProj._43 *= 0.5f; // pull in zNear because the shear in effect
                          // moves it out: clipping artifacts when looking down
                          // at water could occur if this multiply is not done

  D3DXMATRIX worldToViewInvTrans, worldToViewInv,
      worldToView = GetUserClipTransform();
...
#define ALLOW_FOR_FASTCLIPDUMPS 0
  ...
  if ((clipPlane.z * clipPlane.w) <=
      -0.4f) // a plane with (z*w) > -0.4 at this point is behind the camera and
             // will cause graphical glitches. Toss it. (0.4 found through
             // experimentation)
  {
#if (ALLOW_FOR_FASTCLIPDUMPS == 1)
    if (shader_dumpfastclipprojectioncoords.GetBool())
  ...
  m_CachedFastClipProjectionMatrix =
      viewToProj * m_CachedFastClipProjectionMatrix;
...
void CShaderAPIDx8::SetNumBoneWeights(int numBones) {
  ...
  if (m_DynamicState.m_NumBones != numBones) {
    ...
    if (m_TransitionTable.CurrentShadowState()) {
      SetVertexBlendState(
          m_TransitionTable.CurrentShadowState()->m_VertexBlendEnable ? -1 : 0);
    }
  }
}
...
void CShaderAPIDx8::ApplySRGBReadState(int iTextureStage,
  ...
  DWORD linearFormatBackup =
      pBindTexture->Format
          .dword[0]; // if we convert to srgb format, we need the original
                     // format for reverting. We only need the first DWORD of
                     // GPUTEXTURE_FETCH_CONSTANT.
  if (bSRGBReadEnabled) {
    pBindTexture->Format.SignX = pBindTexture->Format.SignY =
        pBindTexture->Format.SignZ =
...
void CShaderAPIDx8::UpdatePixelFogColorConstant(void) {
  ...
  case MATERIAL_FOG_LINEAR: {
    ...
    if (m_DynamicState.m_bSRGBWritesEnabled) {
      // since the fog color will assuredly get converted from linear to gamma,
      // we should probably convert it from gamma to linear
      for (int i = 0; i != 3; ++i)
    ...
    if (g_pHardwareConfig->NeedsShaderSRGBConversion()) {
      ...
      for (int i = 0; i != 3; ++i)
        fogColor[i] = GammaToLinear_HardwareSpecific(
            m_DynamicState
                .m_PixelFogColor[i]); // this is how water fog color has always
                                      // been setup in the past
    }
...
int CShaderAPIDx8::GetPixelFogCombo(void) {
  if ((m_SceneFogMode != MATERIAL_FOG_NONE) &&
    ...
    return m_SceneFogMode -
           1; // PIXELFOGTYPE dynamic combos are shifted down one.
              // MATERIAL_FOG_NONE is simulated by range fog with rigged
              // parameters. Gets rid of a dynamic combo across the ps2x set
  else
    return MATERIAL_FOG_NONE;
}
...
bool CShaderAPIDx8::ShouldUsePixelFogForMode(MaterialFogMode_t fogMode) {
  ...
  case MATERIAL_FOG_LINEAR:
    return (
        g_pHardwareConfig
            ->SupportsPixelShaders_2_b() && // lightmappedgeneric_ps20.fxc can't
                                            // handle the instruction count
...
int CShaderAPIDx8::GetCurrentDynamicVBSize(void) { return m_nDynamicVBSize; }

FORCEINLINE void
CShaderAPIDx8::SetVertexShaderConstantInternal(int var, float const *pVec,
...
FORCEINLINE void CShaderAPIDx8::SetPixelShaderConstantInternal(
  ...
  if (IsX360() && nStartConst + nNumConsts > m_MaxVectorPixelShaderConstant) {
    ...
    if (m_MaxVectorPixelShaderConstant > 32) {
      // NOTE!  There really are 224 pixel shader constants on the 360, but we
      // do an optimization that only blasts the first 32 always.
      Error("Don't use more then the first 32 pixel shader constants on the "
            "360!");
    }
  }
...
void CShaderAPIDx8::ExecuteCommandBuffer(uint8 *pCmdBuf) {
  ...
  for (;;) {
    ...
    case CBCMD_JSR: {
      ...
      // sizeof( uint8 *); 				pCmdBuf = GetData<uint8 *>(  pCmdBuf + sizeof( int )
      // );
    ...
    case CBCMD_STORE_EYE_POS_IN_PSCONST: {
      int nReg = GetData<int>(pCmdBuf + sizeof(int));
      pCmdBuf += 2 * sizeof(int);
      SetPixelShaderConstantInternal(nReg, m_WorldSpaceCameraPositon.Base(), 1,
                                     false);
      break;
    }
...
inline IDirect3DBaseTexture *
CShaderAPIDx8::GetD3DTexture(ShaderAPITextureHandle_t hTexture) {
...
CShaderAPIDx8::GetBoundTextureBindId(Sampler_t sampler) const {
  return SamplerState(sampler).m_BoundTexture;
}

inline bool
CShaderAPIDx8::WouldBeOverTextureLimit(ShaderAPITextureHandle_t hTexture) {
  if (IsPC()) {
...
#define SETSAMPLESTATEANDMIRROR(sampler, samplerState, state_type,             \
                                mirror_field, value)                           \
...
#define SETSAMPLESTATEANDMIRROR_FLOAT(sampler, samplerState, state_type,       \
                                      mirror_field, value)                     \
  if (samplerState.mirror_field != value) {                                    \
    samplerState.mirror_field = value;                                         \
...
void CShaderAPIDx8::SetTextureState(Sampler_t sampler,
  ...
  if (samplerState.m_SRGBReadEnable) {
    // convert to srgb format for the bind. This effectively emulates the old
  ...
  static MaterialSystem_Config_t &materialSystemConfig =
      ShaderUtil()->GetConfig();
  ...
  // For now do this the old way on OSX since the dxabstract layer doesn't
  // support SetSamplerStates
  ...
  if ((samplerState.m_MinFilter != minFilter) ||
    ...
    samplerState.m_FinestMipmapLevel = finestMipmapLevel;
    samplerState.m_LodBias = lodBias;
    Dx9Device()->SetSamplerStates(sampler, uTexWrap, vTexWrap, wTexWrap,
                                  minFilter, magFilter, mipFilter,
                                  finestMipmapLevel, lodBias);
  }
#else
  SETSAMPLESTATEANDMIRROR(sampler, samplerState, D3DSAMP_ADDRESSU, m_UTexWrap,
                          uTexWrap);
  SETSAMPLESTATEANDMIRROR(sampler, samplerState, D3DSAMP_ADDRESSV, m_VTexWrap,
                          vTexWrap);
  SETSAMPLESTATEANDMIRROR(sampler, samplerState, D3DSAMP_ADDRESSW, m_WTexWrap,
                          wTexWrap);
  SETSAMPLESTATEANDMIRROR(sampler, samplerState, D3DSAMP_MINFILTER, m_MinFilter,
                          minFilter);
  SETSAMPLESTATEANDMIRROR(sampler, samplerState, D3DSAMP_MAGFILTER, m_MagFilter,
                          magFilter);
  SETSAMPLESTATEANDMIRROR(sampler, samplerState, D3DSAMP_MIPFILTER, m_MipFilter,
                          mipFilter);
  SETSAMPLESTATEANDMIRROR(sampler, samplerState, D3DSAMP_MAXMIPLEVEL,
                          m_FinestMipmapLevel, finestMipmapLevel);
  SETSAMPLESTATEANDMIRROR_FLOAT(sampler, samplerState, D3DSAMP_MIPMAPLODBIAS,
                                m_LodBias, lodBias);
...
#if defined(_X360)
  // 360 uses gpu storage size (which accounts for page alignment bloat), not
  // format size
...
ShaderAPITextureHandle_t
CShaderAPIDx8::CreateDepthTexture(ImageFormat renderTargetFormat, int width,
                                  int height, const char *pDebugName,
                                  bool bTexture) {
  LOCK_SHADERAPI();
...
void CShaderAPIDx8::CreateTextures(ShaderAPITextureHandle_t *pHandles,
  ...
  bool isSRGB = (creationFlags & TEXTURE_CREATE_SRGB) !=
                0; // for Posix/GL only... not used here ?
  ...
  usSetFlags |=
      (IsPosix() ||
       (creationFlags & (TEXTURE_CREATE_DYNAMIC | TEXTURE_CREATE_MANAGED)))
          ? Texture_t::IS_LOCKABLE
          : 0;
  ...
  for (int idxFrame = 0; idxFrame < count; ++idxFrame) {
    ...
    if (numCopies <= 1) {
      tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s - CreateD3DTexture",
             __FUNCTION__);

      pTexture->m_NumCopies = 1;
      pD3DTex =
          CreateD3DTexture(width, height, depth, dstImageFormat, numMipLevels,
                           creationFlags, (char *)pDebugName);
...
//-----------------------------------------------------------------------------
IDirect3DSurface *
CShaderAPIDx8::GetTextureSurface(ShaderAPITextureHandle_t textureHandle) {
  MEM_ALLOC_D3D_CREDIT();
...
//-----------------------------------------------------------------------------
IDirect3DSurface *
CShaderAPIDx8::GetDepthTextureSurface(ShaderAPITextureHandle_t textureHandle) {
  AssertValidTextureHandle(textureHandle);
  if (!TextureIsAllocated(textureHandle)) {
    return NULL;
  }
...
void CShaderAPIDx8::SetRenderTargetEx(
  ...
  if (colorTextureHandle == SHADER_RENDERTARGET_BACKBUFFER) {
...
#endif
    {
      ...
      pColorSurface->AddRef(
          0, "+C  CShaderAPIDx8::SetRenderTargetEx public addref 1");
#else
      pColorSurface->AddRef();
#endif
    }
  ...
  if (depthTextureHandle == SHADER_RENDERTARGET_DEPTHBUFFER) {
...
#endif
    {
      ...
      // dxabstract's AddRef/Release have optional args to help track usage
      pZSurface->AddRef(0,
                        "+D  CShaderAPIDx8::SetRenderTargetEx public addref 1");
#else
      pZSurface->AddRef();
#endif
    }
    ...
    if (tex.m_Flags & Texture_t::IS_DEPTH_STENCIL) {
      ...
      if (pZSurface) {
        ...
        // dxabstract's AddRef/Release have optional args to help track usage
        pZSurface->AddRef(
            0, "+D CShaderAPIDx8::SetRenderTargetEx public addref 2");
#else
        pZSurface->AddRef();
#endif
      }
    } else {
      HRESULT hr = ((IDirect3DTexture9 *)tex.GetTexture())
    ...
    if (!pZSurface) {
      ...
      pColorSurface->Release(
          0, "-C  CShaderAPIDx8::SetRenderTargetEx public release 1");
#else
      pColorSurface->Release();
#endif
      return;
  ...
  if (pZSurface) {
    ...
    ref = pZSurface->Release(
        0, "-D  CShaderAPIDx8::SetRenderTargetEx public release (z surface)");
#else
    ref = pZSurface->Release();
#endif
...
#endif
  {
    ...
    ref = pColorSurface->Release(
        0,
        "-C  CShaderAPIDx8::SetRenderTargetEx public release (color surface)");
#else
    ref = pColorSurface->Release();
#endif
    if (assert_on_refzero) {
...
// Returns the nearest supported format
//-----------------------------------------------------------------------------
ImageFormat CShaderAPIDx8::GetNearestSupportedFormat(
...
void CShaderAPIDx8::TexMagFilter(ShaderTexFilterMode_t texFilterMode) {
  ...
  case SHADER_TEXFILTERMODE_LINEAR:
    ...
    break;
  case SHADER_TEXFILTERMODE_NEAREST_MIPMAP_NEAREST:
    Warning("CShaderAPIDx8::TexMagFilter: "
...
void CShaderAPIDx8::CopyRenderTargetToTextureEx(
  ...
  if (tryblit) {
    ...
    hr = Dx9Device()->StretchRect(
        pRenderTargetSurface, RectToRECT(pSrcRect, srcRect), pDstSurf,
  ...
  if (pDstRect) {
    ...
    Assert((destWidth <= pTexture->GetWidth()) &&
  ...
  if (pSrcRect) {
    ...
    // resolve has no stretching ability, and we can only compensate when doing
    // a resolve to a whole texture larger than the source
    Assert(!pDstRect || (pSrcRect->width <= pDstRect->width &&
  ...
  if ((srcWidth != destWidth) || (srcHeight != destHeight)) {
    // Not a 1:1 resolve, we should only have gotten this far if we can downsize
    // the target texture to compensate
    Assert((destWidth > srcWidth) && (destHeight > srcHeight) &&
           (dstPoint.x == 0) && (dstPoint.y == 0));

    // What we're doing is telling D3D that this texture is smaller than it is
    // so the resolve is 1:1. We leave the texture in this state until it
    // resolves from something bigger. All outside code still thinks this
    // texture is it's original size. And it still owns enough memory to go back
    // to it's original size.
    pD3DTexture->Format.Size.TwoD.Width = srcWidth - 1;
    pD3DTexture->Format.Size.TwoD.Height =
        srcHeight - 1; // no idea why they store it as size-1, but they do
    pResolveRect = NULL;
  } else {
    ...
    pD3DTexture->Format.Size.TwoD.Height =
        pTexture->GetHeight() -
        1; // no idea why they store it as size-1, but they do
  }

  // if we convert to srgb format, we need the original format for reverting. We
  // only need the first DWORD of GPUTEXTURE_FETCH_CONSTANT.
  DWORD linearFormatBackup = pD3DTexture->Format.dword[0];
  if (!(flags & D3DRESOLVE_DEPTHSTENCIL) &&
      (m_DynamicState.m_bSRGBWritesEnabled)) {
    ...
    pD3DTexture->Format.SignX = pD3DTexture->Format.SignY =
        pD3DTexture->Format.SignZ = 3;
  }

  HRESULT hr = Dx9Device()->Resolve(flags, (D3DRECT *)pResolveRect, pD3DTexture,
                                    &dstPoint, 0, 0, NULL, 0, 0, NULL);
  Assert(!FAILED(hr));
...
void CShaderAPIDx8::CopyRenderTargetToScratchTexture(
...
//-------------------------------------------------------------------------
// Allows locking and unlocking of very specific surface types. pOutBits and
// pOutPitch will not be touched if the lock fails.
//-------------------------------------------------------------------------
void CShaderAPIDx8::LockRect(void **pOutBits, int *pOutPitch,
                             ShaderAPITextureHandle_t texHandle, int mipmap,
                             int x, int y, int w, int h, bool bWrite,
                             bool bRead) {
  LOCK_SHADERAPI();
...
struct TextureExtents_t {
  ...
  TextureExtents_t()
      : width(0), height(0), depth(0), mipmaps(0), fine(0), coarse(0) {}
};

// Returns positive integer on success, 0 or <0 on failure.
static int FindCommonMipmapRange(int *pOutSrcFine, int *pOutDstFine,
                                 Texture_t *pSrcTex, Texture_t *pDstTex) {
  ...
  // LOD Clamp indicates that there's no actual data in the finer mipmap levels
  // yet, so respect it when determining the source and destination levels that
  // could have data.
...
void CShaderAPIDx8::CopyTextureToTexture(ShaderAPITextureHandle_t srcTex,
                                         ShaderAPITextureHandle_t dstTex) {
  LOCK_SHADERAPI();
  ...
  if (mipmapCount <= 0) {
    // This is legit for things that are streamed in that are very small (near
    // the 32x32 cutoff we do at the tip of the mipmap pyramid). But leaving it
    // here because it's useful if you're tracking a specific bug. Warning(
    // "Attempted to copy textures that had non-overlapping mipmap pyramids.
    // This has failed and no copy has taken place.\n" );
    return;
  }
  ...
  for (int i = 0; i < mipmapCount; ++i) {
    ...
    hr =
        g_pD3DDevice->StretchRect(pSrcSurf, NULL, pDstSurf, NULL, D3DTEXF_NONE);
    Assert(SUCCEEDED(hr));
...
void CShaderAPIDx8::CopyTextureToRenderTargetEx(
  ...
  if (tryblit) {
    ...
    RECT srcRect, dstRect;
    hr = Dx9Device()->StretchRect(
        pDstSurf, RectToRECT(pSrcRect, srcRect), pRenderTargetSurface,
        RectToRECT(pDstRect, dstRect), D3DTEXF_LINEAR);
    Assert(!FAILED(hr));
  }
...
void CShaderAPIDx8::SpewBoardState() {
  ...
  DEFAULT_QUALITY, DEFAULT_PITCH | FF_MODERN, 0 ); Assert( hFont != 0 );

                          HRESULT hr = D3DXCreateFont( Dx9Device(), hFont,
  &pFont );
                  }
  ...
  %6.3f]\n",
                          "[%6.3f %6.3f %6.3f %6.3f]\n[%6.3f %6.3f %6.3f
  %6.3f]\n", ShaderManager()->GetCurrentVertexShader(), m->m[0][0],
  m->m[0][1],	m->m[0][2],	m->m[0][3], m->m[1][0],	m->m[1][1],
  m->m[1][2],	m->m[1][3], m->m[2][0],	m->m[2][1],	m->m[2][2],
  m->m[2][3], m->m[3][0], m->m[3][1], m->m[3][2], m->m[3][3], m2->m[0][0],
  m2->m[0][1], m2->m[0][2], m2->m[0][3], m2->m[1][0], m2->m[1][1], m2->m[1][2],
  m2->m[1][3], m2->m[2][0], m2->m[2][1], m2->m[2][2], m2->m[2][3], m2->m[3][0],
  m2->m[3][1], m2->m[3][2], m2->m[3][3]
                           );
  #else
                  Vector4D *pVec2 =
  &m_DynamicState.m_pVectorVertexShaderConstant[VERTEX_SHADER_MODELVIEWPROJ];
                  Vector4D *pVec3 =
  &m_DynamicState.m_pVectorVertexShaderConstant[VERTEX_SHADER_VIEWPROJ];
                  Vector4D *pVec4 =
  &m_DynamicState.m_pVectorVertexShaderConstant[VERTEX_SHADER_MODEL];
  ...
  */
  ...
  /*
  D3DXMATRIX* m = &GetTransform(MATERIAL_MODEL);
  sprintf(buf,"WorldMat [%4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f
  %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f]\n", m->m[0][0],	m->m[0][1],
  m->m[0][2],	m->m[0][3], m->m[1][0],	m->m[1][1],	m->m[1][2],
  m->m[1][3], m->m[2][0],	m->m[2][1],	m->m[2][2],	m->m[2][3],
          m->m[3][0], m->m[3][1], m->m[3][2], m->m[3][3] );
  Plat_DebugString(buf);

  m = &GetTransform(MATERIAL_MODEL + 1);
  sprintf(buf,"WorldMat2 [%4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f
  %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f]\n", m->m[0][0],	m->m[0][1],
  m->m[0][2],	m->m[0][3], m->m[1][0],	m->m[1][1],	m->m[1][2],
  m->m[1][3], m->m[2][0],	m->m[2][1],	m->m[2][2],	m->m[2][3],
          m->m[3][0], m->m[3][1], m->m[3][2], m->m[3][3] );
  ...
  sprintf(buf,"ViewMat [%4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f
  %4.3f %4.3f %4.3f]\n", m->m[0][0],	m->m[0][1],	m->m[0][2], m->m[1][0],
  m->m[1][1],	m->m[1][2], m->m[2][0],	m->m[2][1],	m->m[2][2], m->m[3][0],
  m->m[3][1], m->m[3][2] ); Plat_DebugString(buf);

  m = &GetTransform(MATERIAL_PROJECTION);
  sprintf(buf,"ProjMat [%4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f
  %4.3f %4.3f %4.3f]\n", m->m[0][0],	m->m[0][1],	m->m[0][2], m->m[1][0],
  m->m[1][1],	m->m[1][2], m->m[2][0],	m->m[2][1],	m->m[2][2], m->m[3][0],
  m->m[3][1], m->m[3][2] ); Plat_DebugString(buf);
  ...
  for (i = 0; i < GetTextureStageCount(); ++i)
  {
          ...
          sprintf(buf,"TexMat%d [%4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f
...
void CShaderAPIDx8::CacheWorldSpaceCameraPosition() {
  ...
  // Protect against zero, as some pixel shaders will divide by this in
  // CalcWaterFogAlpha() in common_ps_fxc.h
...
void CShaderAPIDx8::PushDeformation(DeformationBase_t const *pDef) {
  ...
  case DEFORMATION_CLAMP_TO_BOX_IN_WORLDSPACE: {
    ...
    memcpy(m_pDeformationStackPtr->m_flDeformationParameters,
...
//-----------------------------------------------------------------------------
static void CommitFlexWeights(IDirect3DDevice9 *pDevice,
                              const DynamicState_t &desiredState,
                              DynamicState_t &currentState, bool bForce) {
  if (IsX360()) {
...
void CShaderAPIDx8::SetupSelectionModeVisualizationState() {
  ...
  if (g_pHardwareConfig->Caps().m_SupportsPixelShaders) {
    ...
    SetVertexShaderConstant(VERTEX_SHADER_MODELVIEWPROJ_THIRD_ROW, pRowTwo,
                            1); // Row two of an identity matrix
...
static void printmat4x4(char *label, float *m00) {
  ...
  for (int n = 0; n < 4; n++) {
    ...
    GLMPRINTF(
        ("-M-    [ %7.4f %7.4f %7.4f %7.4f ] T=> [ %7.4f %7.4f %7.4f %7.4f ]",
...
void CShaderAPIDx8::SetVertexShaderViewProj() {
  ...
  if (g_pHardwareConfig->Caps().m_SupportsPixelShaders) {
    ...
    transpose =
        ...
        GetTransform(
            MATERIAL_PROJECTION); // Get the non-FastClip projection matrix
    D3DXMatrixTranspose(&transpose, &transpose);
...
void CShaderAPIDx8::SetVertexShaderModelViewProjAndModelView(void) {
  ...
  if (g_pHardwareConfig->Caps().m_SupportsPixelShaders) {
    ...
    if (0) {
      ...
      // printmat4x4( "matView", (float*)&matView );
      // printmat4x4( "matProj", (float*)&matProj );
      // printmat4x4( "result (model * view * proj) pre-transpose",
      // (float*)&transpose );
    }
    ...
    // If we're doing FastClip, the above modelviewproj matrix won't work well
    // for vertex shaders which compute projPos.z, hence we'll compute a more
    // useful modelviewproj and put the third row of it in another constant
    D3DXMatrixMultiply(
        &transpose, &modelView,
        &GetTransform(
            MATERIAL_PROJECTION)); // Get the non-FastClip projection matrix
...
void CShaderAPIDx8::SetVertexShaderStateSkinningMatrices() {
  ...
  Dx9Device()->SetMaxUsedVertexShaderConstantsHint(VERTEX_SHADER_MODEL +
                                                   (matricesLoaded * 3));
#endif
}
...
void CShaderAPIDx8::CommitPerPassVertexShaderTransforms() {
  ...
  if (projChanged) {
    ...
    // GLMPRINTF(( "-M- projChanged=false in
...
void CShaderAPIDx8::CommitVertexShaderTransforms() {
  ...
  // GLMPRINTF(( "-M-     viewChanged=%s  projChanged=%s  modelChanged = %s in
  ...
  if (viewChanged || projChanged) {
    ...
    // GLMPRINTF(( "-M-       viewChanged||projChanged -->
    // SetVertexShaderViewProj" ));
    SetVertexShaderViewProj();
  }

  if (viewChanged || modelChanged || projChanged) {
    // GLMPRINTF(( "-M-       viewChanged||projChanged||modelChanged -->
    // SetVertexShaderModelViewProjAndModelView" ));
    SetVertexShaderModelViewProjAndModelView();
  }
...
void CShaderAPIDx8::SetSkinningMatrices() {
  ...
  if (m_DynamicState.m_NumBones == 0) {
    // ###OSX### punting on OSX for now
#if defined(DX_TO_GL_ABSTRACTION) && !defined(OSX)
    Dx9Device()->SetMaxUsedVertexShaderConstantsHint(
        VERTEX_SHADER_BONE_TRANSFORM(0) + 3);
#endif
    return;
  }
  ...
  if (nNumLights > 0) {
    ...
    if (nNumLights > 1) // At least two lights
    {
      ...
      if (nNumLights > 2) // At least three lights
      {
        ...
        {
          light = &m_DynamicState
                       .m_Lights[lightIndex[3]]; // Spread 4th light's constants
                                                 // across w components
    ...
    if (bUsingFixedFunction) {
      if (m_DynamicState.m_bUserClipTransformOverride) {
        ...
        worldToProjectionInvTrans =
            viewInverse *
            GetUserClipTransform(); // taking a cue from the multiplication
                                    // below, multiplication goes left into
                                    // right
...
void CShaderAPIDx8::ClearBuffersObeyStencilEx(bool bClearColor,
                                              bool bClearAlpha,
                                              bool bClearDepth) {
  LOCK_SHADERAPI();
  ...
  ShaderUtil()->DrawClearBufferQuad(r, g, b, a, bClearColor, bClearAlpha,
                                    bClearDepth);
...
void CShaderAPIDx8::ClearBuffers(bool bClearColor, bool bClearDepth,
...
#if !defined(_X360)
  if (bSRGBWriteEnable) {
    // This path used to be !IsPosix(), but this makes no sense and causes the
    // clear color to differ in D3D9 vs. GL.
  ...
  if (mask != 0) {
    bool bRenderTargetMatchesViewport =
        ...
        (m_DesiredState.m_Viewport.Width == -1 &&
         m_DesiredState.m_Viewport.Height == -1) ||
...
#if POSIX
    pTmpSurface->AddRef(0,
                        "CShaderAPIDx8::GetBackBufferImageHDR public addref");
#else
    pTmpSurface->AddRef();
#endif
...
void CShaderAPIDx8::ReadPixels(Rect_t *pSrcRect, Rect_t *pDstRect,
  ...
  if (IsPC() || !IsX360()) {
    ...
    ShaderUtil()->ReadBackBuffer(pSrcRect, pDstRect, pData, dstFormat,
                                 nDstStride);
#endif
  }
}
...
void CShaderAPIDx8::WriteHitRecord() {
  ...
  if (m_SelectionNames.Count() && (m_SelectionMinZ != FLT_MAX)) {
    ...
    *m_pCurrSelectionRecord++ = m_SelectionNames.Count();
    // NOTE: because of rounding, "(uint32)(float)UINT32_MAX" yields zero(!),
    // hence the use of doubles. [ ALSO: As of Nov 2011, VS2010 exhibits a debug
    // build code-gen bug if we cast the result to int32 instead of uint32 ]
    *m_pCurrSelectionRecord++ =
        (uint32)(0.5 + m_SelectionMinZ * (double)((uint32)~0));
    *m_pCurrSelectionRecord++ =
        (uint32)(0.5 + m_SelectionMaxZ * (double)((uint32)~0));
...
int CShaderAPIDx8::GetCurrentLightCombo(void) const {
  ...
  COMPILE_TIME_ASSERT(DX8_LIGHTING_COMBINATION_COUNT ==
                      sizeof(g_DX8LightCombinations) /
                          sizeof(g_DX8LightCombinations[0]));
...
void CShaderAPIDx8::SetPixelShaderFogParams(int reg, ShaderFogMode_t fogMode) {
  m_DelayedShaderConstants.iPixelShaderFogParams =
      reg; // save it off in case the ShaderFogMode_t disables fog. We only find
           // out later.
  float fogParams[4];
...
const FlashlightState_t &
CShaderAPIDx8::GetFlashlightState(VMatrix &worldToTexture) const {
  worldToTexture = m_FlashlightWorldToTexture;
  return m_FlashlightState;
}

const FlashlightState_t &
CShaderAPIDx8::GetFlashlightStateEx(VMatrix &worldToTexture,
...
//-----------------------------------------------------------------------------
#if defined(_X360)
HXUIFONT CShaderAPIDx8::OpenTrueTypeFont(const char *pFontname, int tall,
                                         int style) {
  LOCK_SHADERAPI();
...
//-----------------------------------------------------------------------------
#if defined(_X360)
bool CShaderAPIDx8::GetTrueTypeFontMetrics(HXUIFONT hFont,
  ...
  if (!FAILED(hr)) {
    ...
    // take MaxHeight to be the larger of its reported value or (ascent +
    // descent)
    float maxHeight = 0;
...
//-----------------------------------------------------------------------------
#if defined(_X360)
bool CShaderAPIDx8::GetTrueTypeGlyphs(HXUIFONT hFont, int numChars,
                                      wchar_t *pWch, int *pOffsetX,
                                      int *pOffsetY, int *pWidth, int *pHeight,
                                      unsigned char *pRGBA, int *pRGBAOffset) {
  if (!hFont)
    return false;
  ...
  pRTSurface = g_TextureHeap.AllocRenderTargetSurface(rtWidth, rtHeight,
                                                      D3DFMT_A8R8G8B8);
  if (!pRTSurface)
    goto cleanUp;

  Dx9Device()->SetRenderTarget(0, pRTSurface);
  // Disable depth here otherwise you get a colour/depth multisample mismatch
  // error (in 480p)
  Dx9Device()->SetDepthStencilSurface(NULL);
  Dx9Device()->Clear(0, NULL, D3DCLEAR_TARGET, 0x00000000,
                     (ReverseDepthOnX360() ? 0.0 : 1.0f), 0);

  // create texture to get glyph render from EDRAM
  HRESULT hr = Dx9Device()->CreateTexture(rtWidth, rtHeight, 1, 0,
                                          D3DFMT_A8R8G8B8, 0, &pTexture, NULL);
  if (FAILED(hr))
    goto cleanUp;
  ...
  int xCursor = 0;
  for (int i = 0; i < numChars; i++) {
    // FIXME: the drawRect params don't make much sense (should use
    // "(xCursor+pWidth[i]), pHeight[i]", but then some characters disappear!)
    XUIRect drawRect =
        XUIRect(xCursor + pOffsetX[i], pOffsetY[i], rtWidth, rtHeight);
    wchar_t text[2] = {pWch[i], 0};
    XuiDrawText(m_hDC, text,
                XUI_FONT_STYLE_NORMAL | XUI_FONT_STYLE_SINGLE_LINE |
                    XUI_FONT_STYLE_NO_WORDWRAP,
                0, &drawRect);
    xCursor += pWidth[i];
  }
  ...
  for (int i = 0; i < numChars; i++) {
    ...
    xCursor += pWidth[i];
    XGUntileSurface(pLinear, destPitch, NULL, lockedRect.pBits, rtWidth,
                    rtHeight, &copyRect, 4);
    ...
    for (int y = 0; y < pHeight[i]; y++) {
      ...
      for (int x = 0; x < pWidth[i]; x++) {
        // undo pre-multiplied alpha since glyph bits will be sourced as a rgba
        // texture
        if (!pSrc[0])
          a = 1;
        else
...
cleanUp:
  ...
  // XUI changed renderstates behind our back, so we need to reset to defaults
  // again to get back in synch:
  ResetRenderState(false);
...
//-----------------------------------------------------------------------------
#if defined(_X360)
ShaderAPITextureHandle_t CShaderAPIDx8::CreateRenderTargetSurface(
  ...
  pTexture->m_Flags =
      (Texture_t::IS_ALLOCATED | Texture_t::IS_RENDER_TARGET_SURFACE);
  ...
  ImageFormat dstImageFormat =
      FindNearestSupportedFormat(format, false, true, false);
  D3DFORMAT actualFormat = ImageLoader::ImageFormatToD3DFormat(dstImageFormat);

  pTexture->GetRenderTargetSurface(false) =
      g_TextureHeap.AllocRenderTargetSurface(width, height, actualFormat);
  pTexture->GetRenderTargetSurface(true) =
      g_TextureHeap.AllocRenderTargetSurface(
          width, height, (D3DFORMAT)MAKESRGBFMT(actualFormat));
...
void CShaderAPIDx8::WriteShaderConstantsToGPU() {
  ...
  if (m_MaxVectorVertexShaderConstant) {
    if (m_bGPUOwned) {
      // faster path, write directly into GPU command buffer, bypassing shadow
      // state can only set what is actually owned
      Assert(m_MaxVectorVertexShaderConstant <=
             VERTEX_SHADER_MODEL + 3 * NUM_MODEL_TRANSFORMS);
      int numVectors = AlignValue(m_MaxVectorVertexShaderConstant, 4);
      BYTE *pCommandBufferData;
      Dx9Device()->GpuBeginVertexShaderConstantF4(
          0, (D3DVECTOR4 **)&pCommandBufferData, numVectors);
      memcpy(pCommandBufferData,
             m_DesiredState.m_pVectorVertexShaderConstant[0].Base(),
             numVectors * (sizeof(float) * 4));
      Dx9Device()->GpuEndVertexShaderConstantF4();
    } else {
      Dx9Device()->SetVertexShaderConstantF(
          0, m_DesiredState.m_pVectorVertexShaderConstant[0].Base(),
  ...
  if (m_MaxVectorPixelShaderConstant) {
    if (m_bGPUOwned) {
      ...
      Dx9Device()->SetPixelShaderConstantF(
          0, m_DesiredState.m_pVectorPixelShaderConstant[0].Base(),
  ...
  // these are currently extremely small in number, if this changes they may
  // benefit from a fast path pattern
  if (m_MaxBooleanVertexShaderConstant) {
    Dx9Device()->SetVertexShaderConstantB(
        0, m_DesiredState.m_pBooleanVertexShaderConstant,
        m_MaxBooleanVertexShaderConstant);
    memcpy(m_DynamicState.m_pBooleanVertexShaderConstant,
           m_DesiredState.m_pBooleanVertexShaderConstant,
           m_MaxBooleanVertexShaderConstant * sizeof(BOOL));
    m_MaxBooleanVertexShaderConstant = 0;
  }
  if (m_MaxIntegerVertexShaderConstant) {
    Dx9Device()->SetVertexShaderConstantI(
        0, (int *)m_DesiredState.m_pIntegerVertexShaderConstant,
  ...
  if (m_MaxBooleanPixelShaderConstant) {
    Dx9Device()->SetPixelShaderConstantB(
        0, m_DesiredState.m_pBooleanPixelShaderConstant,
        m_MaxBooleanPixelShaderConstant);
    memcpy(m_DynamicState.m_pBooleanPixelShaderConstant,
           m_DesiredState.m_pBooleanPixelShaderConstant,
...
//-----------------------------------------------------------------------------
// The application is about to perform a hard reboot, but wants to hide the
// screen flash by persisting the front buffer across a reboot boundary. The
// persisted frame buffer can be detected and restored.
//-----------------------------------------------------------------------------
#if defined(_X360)
void CShaderAPIDx8::PersistDisplay() {
  if (m_PresentParameters.FrontBufferFormat != D3DFMT_LE_X8R8G8B8) {
    ...
    // the fix may be to slam the presentation parameters to the expected
    // format, do a ResetDevice(), and then PersistDisplay().
    Assert(0);
    return;
  }
...
#if defined(_X360)
bool CShaderAPIDx8::PostQueuedTexture(const void *pData, int nDataSize,
                                      ShaderAPITextureHandle_t *pHandles,
                                      int numHandles, int nWidth, int nHeight,
                                      int nDepth, int numMips, int *pRefCount) {
  ...
  do {
    pVTFTexture->ComputeMipLevelDimensions(iTopMip, &mipWidth, &mipHeight,
                                           &mipDepth);
  ...
  for (int iFrame = 0; iFrame < numHandles; iFrame++) {
    ...
    int nFaceCount = (pTexture->m_CreationFlags & TEXTURE_CREATE_CUBEMAP)
    ...
    for (int iFace = 0; iFace < nFaceCount; ++iFace) {
      for (int iMip = 0; iMip < numMips; ++iMip) {
        pVTFTexture->ComputeMipLevelDimensions(iTopMip + iMip, &mipWidth,
                                               &mipHeight, &mipDepth);
        unsigned char *pSourceBits =
            pVTFTexture->ImageData(iFrame, iFace, iTopMip + iMip, 0, 0, 0);
        ...
        info.m_bCanConvertFormat =
            (pTexture->m_Flags & Texture_t::CAN_CONVERT_FORMAT) != 0;
        LoadTexture(info);
      }
    }
...
#if defined(_X360)
void *CShaderAPIDx8::GetD3DDevice() { return Dx9Device(); }
#endif

#if defined(_X360)
static void r_enable_gpr_allocations_callback(IConVar *var,
...
ConVar r_enable_gpr_allocations(
    "r_enable_gpr_allocations", "1", 0,
    "Enable usage of IDirect3DDevice9::SetShaderGPRAllocation()",
    r_enable_gpr_allocations_callback);

static void CommitShaderGPRs(IDirect3DDevice9 *pDevice,
                             const DynamicState_t &desiredState,
                             DynamicState_t &currentState, bool bForce) {
  if (desiredState.m_iVertexShaderGPRAllocation !=
      currentState.m_iVertexShaderGPRAllocation) {
    pDevice->SetShaderGPRAllocation(
        0, desiredState.m_iVertexShaderGPRAllocation,
        128 - desiredState.m_iVertexShaderGPRAllocation);
    currentState.m_iVertexShaderGPRAllocation =
        desiredState.m_iVertexShaderGPRAllocation;
  }
}
...
void CShaderAPIDx8::EnableVSync_360(bool bEnable) {
  if (bEnable) {
    Dx9Device()->SetRenderState(D3DRS_PRESENTIMMEDIATETHRESHOLD,
                                0); // only swap on vertical blanks
  } else {
    Dx9Device()->SetRenderState(
        D3DRS_PRESENTIMMEDIATETHRESHOLD,
        100); // allow a swap at any point in the DAC scan
  }
}
#endif
...
bool CShaderAPIDx8::SetRenderTargetInternalXbox(
  ...
  if (!bForce &&
      (hRenderTargetTexture == m_hCachedRenderTarget &&
  ...
  // the viewport is a clobbered state, it may not be changed by later callers,
  // so it MUST be put back as expected the other clobbered states are waiting
  // to be discovered ... sigh
#if defined(_X360)
  D3DVIEWPORT9 viewport;
...
void D3DBlockingSpewCallback(DWORD Flags, D3DBLOCKTYPE BlockType,
                             float ClockTime, DWORD ThreadTime) {
...
static void r_blocking_spew_threshold_callback(IConVar *var,
                                               const char *pOldValue,
                                               float flOldValue) {
  if (Dx9Device() != NULL) {
...
ConVar r_blocking_spew_threshold(
    ...
    "Enable spew of Direct3D Blocks. Specify the minimum blocking time in "
    "milliseconds before spewing a warning.",
