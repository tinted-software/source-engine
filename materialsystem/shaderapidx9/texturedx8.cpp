...
static HRESULT GetSurfaceFromTexture(IDirect3DBaseTexture *pBaseTexture,
  ...
  case D3DRTYPE_TEXTURE:
    hr =
        ((IDirect3DTexture *)pBaseTexture)->GetSurfaceLevel(level, ppSurfLevel);
    break;
  case D3DRTYPE_CUBETEXTURE:
    if (cubeFaceID != 0) {
      // Debugger();
...
CreateD3DTexture(int width, int height, int nDepth, ImageFormat dstFormat,
                 int numLevels, int nCreationFlags,
                 char *debugLabel) // OK to skip the last param
{
  if (nDepth <= 0) {
    nDepth = 1;
  }
  ...
  if (IsX360()) {
    ...
    // 360 render target creation path is for the target as a texture source
    // (NOT the EDRAM version) use normal texture format rules
    Assert(!bVertexTexture);
    if (!bVertexTexture) {
      d3dFormat = ImageLoader::ImageFormatToD3DFormat(
          FindNearestSupportedFormat(dstFormat, false, false, false));
    }
  } else {
    d3dFormat = ImageLoader::ImageFormatToD3DFormat(FindNearestSupportedFormat(
  ...
  {
    if (bSRGB) {
      usage |= D3DUSAGE_TEXTURE_SRGB; // does not exist in real DX9... just for
                                      // GL to know that this is an SRGB tex
    }
  }
#endif
  ...
  if (isCubeMap) {
    ...
    hr = Dx9Device()->CreateCubeTexture(
        width, numLevels, usage, d3dFormat, pool, &pD3DCubeTexture, NULL
    ...
    hr = Dx9Device()->CreateVolumeTexture(
        width, height, nDepth, numLevels, usage, d3dFormat, pool,
    ...
    hr = Dx9Device()->CreateTexture(
        width, height, numLevels, usage, d3dFormat, pool, &pD3DTexture, NULL
#if defined(DX_TO_GL_ABSTRACTION)
        ,
        debugLabel // tex create funcs take extra arg for debug name on GL
#endif
  ...
  if (FAILED(hr)) {
    ...
    if (CommandLine()->FindParm("-nulldevice")) {
      Warning("ShaderAPIDX8::CreateD3DTexture: Null device used. Texture not "
              "created.\n");
      return 0;
    }
#endif
...
static void BlitSurfaceBits(TextureLoadInfo_t &info, int xOffset, int yOffset,
  ...
  if (!info.m_bTextureIsLockable) {
    ...
    if (g_ShaderDeviceUsingD3D9Ex &&
        (info.m_SrcFormat == dstFormat ||
         (info.m_SrcFormat == IMAGE_FORMAT_DXT1_ONEBITALPHA &&
          dstFormat == IMAGE_FORMAT_DXT1)) &&
        (!ImageLoader::IsCompressed(dstFormat) ||
         (info.m_nWidth >= 4 || info.m_nHeight >= 4))) {
      if (srcStride == 0 ||
          srcStride ==
              info.m_nWidth * ImageLoader::SizeInBytes(info.m_SrcFormat)) {
        IDirect3DTexture9 *pTempTex = NULL;
        if (Dx9Device()->CreateTexture(info.m_nWidth, info.m_nHeight, 1, 0,
                                       dstFormatD3D, D3DPOOL_SYSTEMMEM,
                                       &pTempTex,
                                       (HANDLE *)&info.m_pSrcData) == S_OK) {
    ...
    // If possible to create a texture of this size, create a temporary texture
    // in system memory and then use the UpdateSurface method to copy between
    // textures.
    if (!pSrcSurface &&
        (g_pHardwareConfig->Caps().m_SupportsNonPow2Textures ||
         (IsPowerOfTwo(info.m_nWidth) && IsPowerOfTwo(info.m_nHeight)))) {
      int tempW = info.m_nWidth, tempH = info.m_nHeight, mip = 0;
      if (info.m_nLevel > 0 && ((tempW | tempH) & 3) &&
          ImageLoader::IsCompressed(dstFormat)) {
        // Loading lower mip levels of DXT compressed textures is sort of tricky
        // because we can't create textures that aren't multiples of 4, and we
        // can't pass subrectangles of DXT textures into UpdateSurface. Create a
        // temporary texture which is 1 or 2 mip levels larger and then lock the
        // appropriate mip level to grab its correctly-dimensioned surface.
        // -henryg 11/18/2011
      ...
      IDirect3DTexture9 *pTempTex = NULL;
      IDirect3DSurface *pTempSurf = NULL;
      if (Dx9Device()->CreateTexture(tempW, tempH, mip + 1, 0, dstFormatD3D,
                                     D3DPOOL_SYSTEMMEM, &pTempTex,
                                     NULL) == S_OK) {
    ...
    if (!pSrcSurface) {
      IDirect3DSurface *pTempSurf = NULL;
      if (Dx9Device()->CreateOffscreenPlainSurface(
              info.m_nWidth, info.m_nHeight, dstFormatD3D, D3DPOOL_SYSTEMMEM,
              &pTempSurf, NULL) == S_OK) {
        pSrcSurface = pTempSurf;
        bCopyBitsToSrcSurface = true;
      }
    ...
    if (bCopyBitsToSrcSurface && pSrcSurface) {
      if (pSrcSurface->LockRect(&lockedRect, NULL, D3DLOCK_NOSYSLOCK) == S_OK) {
        ...
        ShaderUtil()->ConvertImageFormat(
            info.m_pSrcData, info.m_SrcFormat, pImage, dstFormat, info.m_nWidth,
    ...
    if (pSrcSurface) {
      ...
      bSuccess = (Dx9Device()->UpdateSurface(pSrcSurface, NULL, pTextureLevel,
                                             &pt) == S_OK);
      pSrcSurface->Release();
    }

    if (!bSuccess) {
      Warning("CShaderAPIDX8::BlitTextureBits: couldn't lock texture rect or "
              "use UpdateSurface\n");
    }
...

  {
    ...
    if (FAILED(pTextureLevel->LockRect(&lockedRect, &srcRect,
                                       D3DLOCK_NOSYSLOCK))) {
  ...
  {
    tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s - ConvertImageFormat",
           __FUNCTION__);
  ...
  tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s - pTextureLevel->Release",
         __FUNCTION__);
  pTextureLevel->Release();
}
#endif
...
static void BlitSurfaceBits(TextureLoadInfo_t &info, int xOffset, int yOffset,
  ...
  // a d3dformat identifier on the xbox is tiled, the same d3dformat on the pc
  // is expected linear to the app we purposely hide the tiling here, otherwise
  // much confusion for the pc the *entire* target must be un-tiled *only*
  // before any *subrect* blitting linear work the *entire* target must then be
  // re-tiled after the *subrect* blit procedural textures require this to
  // subrect blit their new portions correctly the tiling dance can be avoided
  // if the source and target match in tiled state during a full rect blit
  ...
  if (dstFormat != info.m_SrcFormat) {
    ...
    int srcSize = ImageLoader::GetMemRequired(info.m_nWidth, info.m_nHeight, 1,
                                              info.m_SrcFormat, false);
    int dstSize = ImageLoader::GetMemRequired(info.m_nWidth, info.m_nHeight, 1,
                                              dstFormat, false);
    formatConvertMemory.EnsureCapacity(dstSize);

    // due to format conversion, source is in non-native order
    ImageLoader::PreConvertSwapImageData((unsigned char *)info.m_pSrcData,
                                         srcSize, info.m_SrcFormat,
                                         info.m_nWidth, srcStride);

    // slow conversion operation
    if (!ShaderUtil()->ConvertImageFormat(
            info.m_pSrcData, info.m_SrcFormat,
            (unsigned char *)formatConvertMemory.Base(), dstFormat,
            info.m_nWidth, info.m_nHeight, srcStride, 0)) {
      // conversion failed
      Assert(0);
      return;
    }
    ...
    ImageLoader::PostConvertSwapImageData(
        (unsigned char *)formatConvertMemory.Base(), dstSize, dstFormat);
  ...
  bool bFullSurfBlit = (mipDesc.Width == (unsigned)info.m_nWidth &&
                        mipDesc.Height == (unsigned)info.m_nHeight);
  ...
  if (FAILED(hr)) {
    Warning(
        "CShaderAPIDX8::BlitTextureBits: GetSurfaceFromTexture() failure\n");
    return;
  }
  ...
  if (!bDstIsTiled) {
    ...
    hr = XGCopySurface(pTargetImage, mipDesc.RowPitch, mipDesc.Width,
                       mipDesc.Height, mipDesc.Format, &p, pSrcData, srcStride,
                       mipDesc.Format, &r, 0, 0);
    if (FAILED(hr)) {
    ...
    XGTileTextureLevel(baseDesc.Width, baseDesc.Height, info.m_nLevel,
                       XGGetGpuFormat(baseDesc.Format), tileFlags, pTargetImage,
                       &p, pSrcData, srcStride, &r);
  }
...
static void BlitVolumeBits(TextureLoadInfo_t &info, int xOffset, int yOffset,
  ...
  XGTEXTURE_DESC mipDesc;
  XGGetTextureDesc(info.m_pTexture, info.m_nLevel, &mipDesc);
  bool bFullSurfBlit = (mipDesc.Width == (unsigned int)info.m_nWidth &&
                        mipDesc.Height == (unsigned int)info.m_nHeight);
  ...
  IDirect3DVolumeTexture *pVolumeTexture =
      static_cast<IDirect3DVolumeTexture *>(info.m_pTexture);
  HRESULT hr = pVolumeTexture->LockBox(info.m_nLevel, &lockedBox, NULL,
                                       D3DLOCK_NOSYSLOCK);
  if (FAILED(hr)) {
  ...
  if (!info.m_bSrcIsTiled && bDstIsTiled) {
    ...
    XGTileVolumeTextureLevel(baseDesc.Width, baseDesc.Height, baseDesc.Depth,
                             info.m_nLevel, XGGetGpuFormat(baseDesc.Format),
                             tileFlags, pTargetImage, NULL, pSrcData,
...
void LoadVolumeTextureFromVTF(TextureLoadInfo_t &info, IVTFTexture *pVTF,
                              int iVTFFrame) {
  if (!info.m_pTexture ||
      info.m_pTexture->GetType() != D3DRTYPE_VOLUMETEXTURE) {
    Assert(0);
    return;
  }
  ...
  if (pVTF->Depth() != (int)desc.Depth || pVTF->Width() != (int)desc.Width ||
      pVTF->Height() != (int)desc.Height || pVTF->MipCount() < iMipCount) {
  ...
  if (!info.m_bTextureIsLockable) {
    ...
    if (Dx9Device()->CreateVolumeTexture(
            desc.Width, desc.Height, desc.Depth, iMipCount, 0, desc.Format,
            D3DPOOL_SYSTEMMEM, &pTemp, NULL) != S_OK) {
      Warning("LoadVolumeTextureFromVTF: failed to create temporary staging "
              "texture\n");
      return;
    }
...
void LoadCubeTextureFromVTF(TextureLoadInfo_t &info, IVTFTexture *pVTF,
                            int iVTFFrame) {
  ...
  IDirect3DCubeTexture9 *pCubeTex =
      static_cast<IDirect3DCubeTexture9 *>(info.m_pTexture);
  ...
  if (pVTF->Depth() != 1 || pVTF->Width() != (int)desc.Width ||
      pVTF->Height() != (int)desc.Height || pVTF->FaceCount() < 6 ||
  ...
  if (!info.m_bTextureIsLockable) {
    ...
    if (Dx9Device()->CreateCubeTexture(desc.Width, iMipCount, 0, desc.Format,
                                       D3DPOOL_SYSTEMMEM, &pTemp,
                                       NULL) != S_OK) {
      Warning("LoadCubeTextureFromVTF: failed to create temporary staging "
              "texture\n");
      return;
    }
...
void LoadTextureFromVTF(TextureLoadInfo_t &info, IVTFTexture *pVTF,
                        int iVTFFrame) {
  ...
  if (pVTF->Depth() != 1 || pVTF->Width() != (int)desc.Width ||
      pVTF->Height() != (int)desc.Height || pVTF->MipCount() < iMipCount ||
      pVTF->FaceCount() <= (int)info.m_CubeFaceID) {
    Warning("LoadTextureFromVTF: VTF dimensions do not match texture\n");
    return;
  }

  // Info may have a cube face ID if we are falling back to 2D sphere map
  // support
...
#if !defined(_X360) && !defined(DX_TO_GL_ABSTRACTION)
  // If blitting more than one mip level of an unlockable texture, create a
  // temporary texture for all mip levels only call UpdateTexture once. For
  // textures with only a single mip level, fall back on the support in
  // BlitSurfaceBits. -henryg
  IDirect3DTexture9 *pStagingTexture = NULL;
  if (!info.m_bTextureIsLockable && iMipCount > 1) {
    tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s - CreateSysmemTexture",
           __FUNCTION__);

    IDirect3DTexture9 *pTemp;
    if (Dx9Device()->CreateTexture(desc.Width, desc.Height, iMipCount, 0,
                                   desc.Format, D3DPOOL_SYSTEMMEM, &pTemp,
                                   NULL) != S_OK) {
      Warning(
          "LoadTextureFromVTF: failed to create temporary staging texture\n");
      return;
    }
  ...
  // Get the clamped resolutions from the VTF, then apply any clamping we've
  // done from the higher level code. (For example, we chop off the bottom of
  // the mipmap pyramid at 32x32--that is reflected in iMipCount, so honor that
  // here).
...

  {
    ...
    for (int iMip = finest; iMip <= coarsest; ++iMip) {
      ...
      tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s - BlitTextureBits - %d",
             __FUNCTION__, iMip);
  ...
  if (pStagingTexture) {
    if ((coarsest - finest + 1) == iMipCount) {
      ...
      for (int mip = finest; mip <= coarsest; ++mip) {
        tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s - UpdateSurface - %d",
               __FUNCTION__, mip);

        IDirect3DSurface9 *pSrcSurf = NULL, *pDstSurf = NULL;

        if (pStagingTexture->GetSurfaceLevel(mip, &pSrcSurf) != S_OK)
          Warning("LoadTextureFromVTF: couldn't get surface level %d for "
                  "system surface\n",
                  mip);

        if (pTex->GetSurfaceLevel(mip, &pDstSurf) != S_OK)
          Warning("LoadTextureFromVTF: couldn't get surface level %d for dest "
                  "surface\n",
                  mip);

        {
          tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s - UpdateSurface - Call ",
                 __FUNCTION__, mip);
          if (!pSrcSurf || !pDstSurf ||
              Dx9Device()->UpdateSurface(pSrcSurf, NULL, pDstSurf, NULL) !=
                  S_OK)
