...
enum { COLOR_CORRECTION_MAX_TEXTURES = 4, COLOR_CORRECTION_TEXTURE_SIZE = 32 };
...
public:
  ...
  // Mark a texture as now-unreferenced, so it can be checked for removal at a
  // later (and thread-safe) time.
  virtual void MarkUnreferencedTextureForCleanup(ITextureInternal *
                                                 pTexture) = 0;
  ...
  virtual void AsyncFindOrLoadTexture(
      ...
      bool bComplain, int nAdditionalCreationFlags) = 0;

  // Stream a render target back to system memory, perform format conversion to
  // the specified destination format,
  ...
  virtual bool AddTextureCompositorTemplate(const char *pName,
                                            KeyValues *pTmplDesc) = 0;
  virtual bool VerifyTextureCompositorTemplates() = 0;

  virtual CTextureCompositorTemplate *FindTextureCompositorTemplate(
      const char *pName) = 0;
};
