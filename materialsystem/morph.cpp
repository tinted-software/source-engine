...
private:
  ...
  void
  ComputeTextureDimensions(const CUtlVector<MorphSegmentList_t> &morphSegments);
...
public:
  ...
  virtual bool
  GetMorphAccumulatorTexCoord(IMorphMgrRenderContext *pRenderContext,
...
void CMorph::ComputeTextureDimensions(
  ...
  while (m_nTextureWidth * nTotal4Tuples > nMaxTextureWidth) {
    ...
    if (m_nTextureHeight > nMaxTextureHeight) {
      Warning("Morph texture is too big!!! Make brian add support for morphs "
