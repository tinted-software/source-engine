...
#define USE_32BIT_LIGHTMAPS_ON_360 // uncomment to use 32bit lightmaps, be sure
                                   ...
                                   // in stdshaders/lightmappedgeneric_ps2_3_x.h
...
inline void
CMatLightmaps::SetCurrentMaterialInternal(IMaterialInternal *pCurrentMaterial) {
  return GetMaterialSystem()
      ->GetRenderContextInternal()
      ->SetCurrentMaterialInternal(pCurrentMaterial);
}

inline IMaterialInternal *
CMatLightmaps::GetMaterialInternal(MaterialHandle_t idx) const {
...
void CMatLightmaps::GetLightmapPageSize(int lightmapPageID, int *pWidth,
  ...
  case MATERIAL_SYSTEM_LIGHTMAP_PAGE_USER_DEFINED:
    *pWidth = *pHeight = 1;
    AssertOnce(!"Can't use CMatLightmaps to get properties of "
                "MATERIAL_SYSTEM_LIGHTMAP_PAGE_USER_DEFINED");
    break;
...
int CMatLightmaps::AllocateLightmap(int width, int height,
  ...
  if (!pMaterial) {
    Warning("Programming error: CMatRenderContext::AllocateLightmap: NULL "
            "material\n");
    return m_numSortIDs;
  }
  pMaterial =
  ...
  if (!bAdded) {
    ...
    if (!m_ImagePackers[i].AddBlock(width, height, &offsetIntoLightmapPage[0],
      ...
      Error("MaterialSystem_Interface_t::AllocateLightmap: lightmap (%dx%d) "
            "too big to fit in page (%dx%d)\n",
            width, height, GetMaxLightmapPageWidth(),
            GetMaxLightmapPageHeight());
    }
...
void CMatLightmaps::AllocateLightmapTexture(int lightmap) {
  ...
  int flags =
      bUseDynamicTextures ? TEXTURE_CREATE_DYNAMIC : TEXTURE_CREATE_MANAGED;
  ...
  case STATE_RELEASED:
    ...
    DevMsg("AllocateLightmapTexture(%d) in released lightmap state "
  ...
  default:
    // Not assigned m_LightmapPageTextureHandles[lightmap];
    Warning("AllocateLightmapTexture(%d) in unknown lightmap state (%d), "
            "skipped.\n",
...
int CMatLightmaps::AllocateWhiteLightmap(IMaterial *iMaterial) {
  IMaterialInternal *pMaterial = static_cast<IMaterialInternal *>(iMaterial);
  if (!pMaterial) {
    Warning("Programming error: CMatRenderContext::AllocateWhiteLightmap: NULL "
            "material\n");
    return m_numSortIDs;
  }
  pMaterial =
...
void CMatLightmaps::ReleaseLightmapPages() {
  ...
  default:
    Warning("ReleaseLightmapPages is expected in STATE_DEFAULT, current state "
            "= %d, discarded.\n",
            m_eLightmapsState);
...
void CMatLightmaps::RestoreLightmapPages() {
  ...
  default:
    Warning("RestoreLightmapPages is expected in STATE_RELEASED, current state "
            "= %d, discarded.\n",
            m_eLightmapsState);
...
// this outputs RGBa where RGB are [0..255] and a is the shader's scaling factor
// (also 0..255)
//
// WARNING - this code appears to be vulnerable to a compiler bug. Be very
// careful modifying and be sure to test
fltx4 ConvertLightmapColorToRGBScale(FLTX4 lightmapColor) {
  ...
  static const fltx4 vTwoFiftyFiveOverSixteen = {
      255.0f / 16.0f, 255.0f / 16.0f, 255.0f / 16.0f, 255.0f / 16.0f};
  // static const fltx4 vSixteenOverTwoFiftyFive = { 16.0f / 255.0f, 16.0f /
  // 255.0f, 16.0f / 255.0f, 16.0f / 255.0f };
  ...
  fltx4 fl4OutofRange =
      OrSIMD(CmpGeSIMD(scale, Four_Ones), CmpLeSIMD(scale, FourPoint1s));
  fl4OutofRange = OrSIMD(fl4OutofRange,
                         CmpGtSIMD(minscale, MulSIMD(Four_PointFives, scale)));

  // scale needs to be divided by 16 (because the shader multiplies it by 16)
  // then mapped to 0..255 and quantized.
  scale = __vrfip(
      MulSIMD(scale, vTwoFiftyFiveOverSixteen)); // scale = ceil(scale * 255/16)

  fltx4 result =
      MulSIMD(vTwoFiftyFive,
              lightmapColor); // start the scale cooking on the final result

  fltx4 invScale =
      ReciprocalEstSIMD(scale); // invScale = (16/255)(1/scale). may be +inf
  invScale =
      MulSIMD(invScale,
              vTwoFiftyFiveOverSixteen); // take the quantizing factor back out
  ...
  result = MaskedAssign(fl4OutofRange, SetWSIMD(result, scale),
                        SetWSIMD(MulSIMD(lightmapColor, vTwoFiftyFive),
                                 vTwoFiftyFiveOverSixteen));
  return result;
}
#endif
...
namespace {
// pack a pixel into BGRA8888 and return it with the data packed into the w
// component
...
struct ALIGN128 CPixelWriterStoreGather {
  ...
  ALIGN128 uint32
      m_data[kRows][kWordsPerRow]; // four rows of bgra data, aligned to 4 cache
                                   // lines. dwords so memcpy works better.
  int m_wordsGathered;
  int m_bytesBetweenWriterRows; // the number of bytes spacing the maps inside
                                // the writer from each other if we weren't
                                // gathering, we'd SkipBytes this many between
                                // the base map, bump1, etc.

  // write four rows, as SIMD registers, into the buffers
  inline void write(CPixelWriter *RESTRICT pLightmapPixelWriter, FLTX4 row0,
                    FLTX4 row1, FLTX4 row2, FLTX4 row3) RESTRICT {
    // if full, commit
    Assert(m_wordsGathered <= kWordsPerRow);
    AssertMsg(
        (m_wordsGathered & 3) == 0,
        "Don't call CPixelWriterStoreGather::write after ::writeJustX"); // single-word
                                                                         // writes
                                                                         // have
                                                                         // misaligned
                                                                         // me
  ...
  // pluck the w component out of each of the rows, and store it into the gather
  // buffer. Don't call the other write function after calling this.
  inline void writeJustW(CPixelWriter *RESTRICT pLightmapPixelWriter,
                         FLTX4 row0, FLTX4 row1, FLTX4 row2,
                         FLTX4 row3) RESTRICT {
    // if full, commit
    Assert(m_wordsGathered <= kWordsPerRow);
    if (m_wordsGathered >= kWordsPerRow) {
  ...
  void commit(CPixelWriter *RESTRICT pLightmapPixelWriter) RESTRICT {
    if (m_wordsGathered > 0) {
      unsigned char *RESTRICT pWriteInto =
          pLightmapPixelWriter->GetCurrentPixel();
...
#pragma error(                                                                 \
    "You have overridden memcpy(), which is an XBOX360 intrinsic. This function will not behave optimally.")
#endif
...
#pragma error(                                                                 \
    ...
    COMPILE_TIME_ASSERT(
        (kWordsPerRow & 3) ==
        0); // the number of words per row has to be a multiple of four

    memcpy(pWriteInto, reinterpret_cast<uint64 * RESTRICT>(m_data[0]),
           kWordsPerRow * sizeof(uint32));
    pWriteInto += m_bytesBetweenWriterRows;
    memcpy(pWriteInto, reinterpret_cast<uint64 * RESTRICT>(m_data[1]),
           kWordsPerRow * sizeof(uint32));
    pWriteInto += m_bytesBetweenWriterRows;
    memcpy(pWriteInto, reinterpret_cast<uint64 * RESTRICT>(m_data[2]),
           kWordsPerRow * sizeof(uint32));
    pWriteInto += m_bytesBetweenWriterRows;
    memcpy(pWriteInto, reinterpret_cast<uint64 * RESTRICT>(m_data[3]),
           kWordsPerRow * sizeof(uint32));
  ...
  CPixelWriterStoreGather(int writerSizeBytes)
      : m_wordsGathered(0), m_bytesBetweenWriterRows(writerSizeBytes) {};
};
} // namespace

// this is a function for specifically writing bumped BGRA lightmaps -- in order
// for it to be properly scheduled, I needed to break out the inline functions.
// Also, to make the write-combined memory more efficient (and work around a bug
// in the April 2007 XDK), we need to store-gather our writes on the cache
// before blasting them out to write-combined memory. We can't simply write from
// the SIMD registers into the pixelwriter's data, because the difference
// between the output rows, eg nLightmap0WriterSizeBytes[0], might not be a
// multiple of 16. Unaligned stores to non-cacheable memory cause an alignment
// exception.
static void BumpedLightmapBitsToPixelWriter_HDRI_BGRA_X360(
    ...
    int pLightmapSize[2], int pOffsetIntoLightmapPage[2], FloatBitMap_t *pfmOut,
    CPixelWriter *RESTRICT m_LightmapPixelWriter) {
  AssertMsg(m_LightmapPixelWriter->GetPixelSize() == 4,
            "BGRA format is no longer four bytes long? This is unsupported on "
            "360, and probably immoral as well.");
  const int nLightmap0WriterSizeBytes =
      pLightmapSize[0] * 4 /*m_LightmapPixelWriter->GetPixelSize()*/;
  // const int nRewindToNextPixel = -( ( nLightmap0WriterSizeBytes * 3 ) - 4 );

  // assert that 1 * 4 = 4
  COMPILE_TIME_ASSERT(sizeof(Vector4D) == sizeof(float) * 4);

  AssertMsg(!pfmOut, "Runtime conversion of lightmaps to files is no longer "
                     "supported on 360.\n");
...
#define FOUR                                                                   \
  (sizeof(Vector4D) /                                                          \
   sizeof(                                                                     \
       float)) //  make explicit when we're incrementing by length of a 4dvec
    ...
    // we want to hang on to this index when we're done with groups so we can do
    // the remainder.
    ...
    for (s = 0; s < groupsOfFourLimit; s += 4, srcTexelOffset += 4 * (FOUR)) {
      ...
      fltx4 outBaseMap = Four_Zeros, outBump1 = Four_Zeros,
            outBump2 = Four_Zeros, outBump3 = Four_Zeros;
      // we'll read four at a time
      fltx4 vFloatImage[4], vFloatImageBump1[4], vFloatImageBump2[4],
          vFloatImageBump3[4];
      ...
      vFloatImageBump1[1] =
          LoadUnalignedSIMD(pFloatImageBump1 + srcTexelOffset + 4);
      vFloatImageBump1[2] =
          LoadUnalignedSIMD(pFloatImageBump1 + srcTexelOffset + 8);
      vFloatImageBump1[3] =
          LoadUnalignedSIMD(pFloatImageBump1 + srcTexelOffset + 12);

      vFloatImageBump2[0] =
          LoadUnalignedSIMD(pFloatImageBump2 + srcTexelOffset);
      vFloatImageBump2[1] =
          LoadUnalignedSIMD(pFloatImageBump2 + srcTexelOffset + 4);
      vFloatImageBump2[2] =
          LoadUnalignedSIMD(pFloatImageBump2 + srcTexelOffset + 8);
      vFloatImageBump2[3] =
          LoadUnalignedSIMD(pFloatImageBump2 + srcTexelOffset + 12);

      vFloatImageBump3[0] =
          LoadUnalignedSIMD(pFloatImageBump3 + srcTexelOffset);
      vFloatImageBump3[1] =
          LoadUnalignedSIMD(pFloatImageBump3 + srcTexelOffset + 4);
      vFloatImageBump3[2] =
          LoadUnalignedSIMD(pFloatImageBump3 + srcTexelOffset + 8);
      vFloatImageBump3[3] =
          LoadUnalignedSIMD(pFloatImageBump3 + srcTexelOffset + 12);

      // perform an arcane averaging operation upon the bump map values
      // (todo: make this not an inline so it will schedule better -- inlining
      // is
      //  done by the linker, which is too late for operation scheduling)
      ColorSpace::LinearToBumpedLightmap(
          vFloatImage[0], vFloatImageBump1[0], vFloatImageBump2[0],
          vFloatImageBump3[0],
          // transform "in place":
          vFloatImage[0], vFloatImageBump1[0], vFloatImageBump2[0],
          vFloatImageBump3[0]);
      ColorSpace::LinearToBumpedLightmap(
          vFloatImage[1], vFloatImageBump1[1], vFloatImageBump2[1],
          vFloatImageBump3[1],
          // transform "in place":
          vFloatImage[1], vFloatImageBump1[1], vFloatImageBump2[1],
          vFloatImageBump3[1]);
      ColorSpace::LinearToBumpedLightmap(
          vFloatImage[2], vFloatImageBump1[2], vFloatImageBump2[2],
          vFloatImageBump3[2],
          // transform "in place":
          vFloatImage[2], vFloatImageBump1[2], vFloatImageBump2[2],
          vFloatImageBump3[2]);
      ColorSpace::LinearToBumpedLightmap(
          vFloatImage[3], vFloatImageBump1[3], vFloatImageBump2[3],
          vFloatImageBump3[3],
          // transform "in place":
          vFloatImage[3], vFloatImageBump1[3], vFloatImageBump2[3],
          vFloatImageBump3[3]);
      ...
      vFloatImage[0] =
          PackPixel_BGRA8888(ConvertLightmapColorToRGBScale(vFloatImage[0]));
      vFloatImageBump1[0] = PackPixel_BGRA8888(
          ConvertLightmapColorToRGBScale(vFloatImageBump1[0]));
      vFloatImageBump2[0] = PackPixel_BGRA8888(
          ConvertLightmapColorToRGBScale(vFloatImageBump2[0]));
      vFloatImageBump3[0] = PackPixel_BGRA8888(
          ConvertLightmapColorToRGBScale(vFloatImageBump3[0]));

      vFloatImage[1] =
          PackPixel_BGRA8888(ConvertLightmapColorToRGBScale(vFloatImage[1]));
      vFloatImageBump1[1] = PackPixel_BGRA8888(
          ConvertLightmapColorToRGBScale(vFloatImageBump1[1]));
      vFloatImageBump2[1] = PackPixel_BGRA8888(
          ConvertLightmapColorToRGBScale(vFloatImageBump2[1]));
      vFloatImageBump3[1] = PackPixel_BGRA8888(
          ConvertLightmapColorToRGBScale(vFloatImageBump3[1]));

      vFloatImage[2] =
          PackPixel_BGRA8888(ConvertLightmapColorToRGBScale(vFloatImage[2]));
      vFloatImageBump1[2] = PackPixel_BGRA8888(
          ConvertLightmapColorToRGBScale(vFloatImageBump1[2]));
      vFloatImageBump2[2] = PackPixel_BGRA8888(
          ConvertLightmapColorToRGBScale(vFloatImageBump2[2]));
      vFloatImageBump3[2] = PackPixel_BGRA8888(
          ConvertLightmapColorToRGBScale(vFloatImageBump3[2]));

      vFloatImage[3] =
          PackPixel_BGRA8888(ConvertLightmapColorToRGBScale(vFloatImage[3]));
      vFloatImageBump1[3] = PackPixel_BGRA8888(
          ConvertLightmapColorToRGBScale(vFloatImageBump1[3]));
      vFloatImageBump2[3] = PackPixel_BGRA8888(
          ConvertLightmapColorToRGBScale(vFloatImageBump2[3]));
      vFloatImageBump3[3] = PackPixel_BGRA8888(
          ConvertLightmapColorToRGBScale(vFloatImageBump3[3]));
      ...
      storeGather.write(m_LightmapPixelWriter, outBaseMap, outBump1, outBump2,
                        outBump3);
    }
    ...
    for (; // s is where it should be from the loop above
         s < (unsigned int)pLightmapSize[0]; s++,
                                             // m_LightmapPixelWriter->SkipBytes(nRewindToNextPixel),
                                             // // now handled by store-gather
                                             srcTexelOffset += (FOUR)) {
      ...
      && color[ii][2] == (pack & 0xFF)				&& color[ii][3]
      == (pack & 0xFF000000) >> 24 );
      }
      */
      ...
      storeGather.writeJustW(m_LightmapPixelWriter, vColor[0], vColor[1],
                             vColor[2], vColor[3]);
      ...
      *>(m_LightmapPixelWriter->GetCurrentPixel()) == PackPixel_BGRA8888(
      vColor[0] ).u[3] ); void * RESTRICT pBits =
      m_LightmapPixelWriter->SkipBytes( nLightmap0WriterSizeBytes );
      m_LightmapPixelWriter->WritePixelNoAdvance_BGRA8888( vColor[1], pBits );
      Assert(*reinterpret_cast<unsigned int
      *>(m_LightmapPixelWriter->GetCurrentPixel()) == PackPixel_BGRA8888(
      vColor[1] ).u[3] ); pBits = m_LightmapPixelWriter->SkipBytes(
      nLightmap0WriterSizeBytes );
      m_LightmapPixelWriter->WritePixelNoAdvance_BGRA8888( vColor[2], pBits );
      Assert(*reinterpret_cast<unsigned int
      *>(m_LightmapPixelWriter->GetCurrentPixel()) == PackPixel_BGRA8888(
      vColor[2] ).u[3] ); pBits = m_LightmapPixelWriter->SkipBytes(
      nLightmap0WriterSizeBytes );
...
void CMatLightmaps::BumpedLightmapBitsToPixelWriter_HDRI(
  ...
  if (m_LightmapPixelWriter.IsUsingFloatFormat()) {
    AssertMsg(!IsX360(), "Tried to use a floating-point pixel format for "
                         "lightmaps on 360, which is not supported.");
    if (!IsX360()) {
      for (int t = 0; t < pLightmapSize[1]; t++) {
        int srcTexelOffset =
            (sizeof(Vector4D) / sizeof(float)) * (0 + t * nLightmapSize0);
...
#define FOUR (sizeof(Vector4D) / sizeof(float)) // in case this ever changes
        ...
        for (int s = 0; s < nLightmapSize0; s++,
          ...
          AssertMsg(!pfmOut, "Runtime conversion of lightmaps to files is no "
                             "longer supported on 360.\n");
...
void CMatLightmaps::LightmapBitsToPixelWriter_HDRI(
  ...
  if (m_LightmapPixelWriter.IsUsingFloatFormat()) {
    ...
    } else { // This code is here as an example only, in case floating point
...
#pragma error("This function only supports 32 bit lightmaps.")
    ...
    default:
      ...
      Warning("Unsupported pixel format used in lightmap. Lightmaps could not "
              "be downloaded.\n");
      break;
    }
  }
...
void CMatLightmaps::UpdateLightmap(int lightmapPageID, int lightmapSize[2],
  ...
  if (lightmapPageID >= GetNumLightmapPages() || lightmapPageID < 0) {
    Error("MaterialSystem_Interface_t::UpdateLightmap lightmapPageID=%d out of "
          "range\n",
          lightmapPageID);
    return;
  }
    ...
    if (hasBump) {
      ...
      case HDR_TYPE_NONE:
        BumpedLightmapBitsToPixelWriter_LDR(
            pFloatImage, pFloatImageBump1, pFloatImageBump2, pFloatImageBump3,
            lightmapSize, bLockSubRect ? subRectOffset : offsetIntoLightmapPage,
            pfmOut);
        break;
      case HDR_TYPE_INTEGER:
...
void CMatLightmaps::ComputeSortInfo(MaterialSystem_SortInfo_t *pInfo,
  ...
  for (MaterialHandle_t i = GetMaterialDict()->FirstMaterial();
    ...
    //MATERIAL_PROPERTY_OPACITY ); 	if( ( !alpha && ( pTransVar->GetIntValue() ==
    //MATERIAL_TRANSLUCENT ) ) || 		( alpha && !( pTransVar->GetIntValue() ==
