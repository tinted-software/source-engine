...
void ReportDirtyDisk() {
  ...
  if ((ThreadGetCurrentId() == MaterialSystem()->GetRenderThreadId()) ||
      (ThreadInMainThread() &&
       g_pMaterialSystem->GetThreadMode() != MATERIAL_QUEUED_THREADED)) {
    SpinPresent();
  }
#endif
}
...
CMatRenderContextBase::CMatRenderContextBase()
    : m_pMaterialSystem(NULL), m_RenderTargetStack(16, 32),
  ...
  RenderTargetStackElement_t initialElement = {
      {NULL, NULL, NULL, NULL}, NULL, 0, 0, -1, -1};
#else
...
void CMatRenderContextBase::CompactMemory() {
  if (sm_nRenderLockCount) {
    DevWarning("CMatRenderContext: Trying to compact with render data still "
               "locked!\n");
    sm_nRenderLockCount = 0;
  }
...
void CMatRenderContextBase::MarkRenderDataUnused(bool bFrameBegin) {
  if (sm_nRenderLockCount) {
    DevWarning("CMatRenderContext: Trying to clear render data with render "
               "data still locked (%d)!\n",
               sm_nRenderLockCount);
    sm_nRenderLockCount = 0;
  }

  // JAY: DO NOT MERGE FROM TF2 - L4D HAS CHANGED THE UNDERLYING INTERFACE IN A
  // WAY THAT DOESN'T REQUIRE THIS
#if 0
	// Switch stacks
	if ( bFrameBegin )
	{
...
#else
  // Just for TF2, don't free the stack until the end of frame.  TF2 Allocates
  // render data and holds it over the lock period because we haven't revised
  // the studiorender interface yet to change patterns. Switch stacks
  if (bFrameBegin) {
    sm_nRenderStack = 1 - sm_nRenderStack;
    // Clear the new stack
#ifdef _DEBUG
    memset(sm_RenderData[sm_nRenderStack].GetBase(), 0xFF,
           RenderDataSizeUsed());
#endif
    sm_RenderData[sm_nRenderStack].FreeAll(false);
  }
...
void CMatRenderContextBase::Bind(IMaterial *iMaterial, void *proxyData) {
  ...
  material = material->GetRealTimeVersion(); // always work with the real time
                                             // versions of materials internally

  if (GetCurrentMaterialInternal() != material) {
    if (!material->IsPrecached()) {
      DevWarning("Binding uncached material \"%s\", artificially incrementing "
                 "refcount\n",
                 material->GetName());
...
void *CMatRenderContextBase::LockRenderData(int nSizeInBytes) {
  ...
  if (!pDest) {
    ExecuteNTimes(10,
                  Warning("MaterialSystem: Out of memory in render data!\n"));
  }
  AddRefRenderData();
  return pDest;
}
...
void CMatRenderContextBase::SetHeightClipZ(float z) {
  ...
  // FIXME!  : Need to move user clip plane support back to pre-dx9 cards (all
  // of the pixel shaders have texkill in them. . blich.)
...
void CMatRenderContextBase::PushRenderTargetAndViewport(ITexture *pTexture,
  ...
  RenderTargetStackElement_t element = {{pTexture, NULL, NULL, NULL},
                                        ...
                                        nViewH};
#else
  RenderTargetStackElement_t element = {{pTexture}, pDepthTexture, nViewX,
                                        nViewY,     nViewW,        nViewH};
#endif
  m_RenderTargetStack.Push(element);
  CommitRenderTargetAndViewport();
...
void CMatRenderContextBase::TurnOnToneMapping(void) {
  if ((HardwareConfig()->GetHDRType() != HDR_TYPE_NONE) &&
    ...
    if (mat_tonemap_algorithm.GetInt() == 1) {
      rate *= 2.0f; // Default 2x for the new tone mapping algorithm so it feels
    ...
    if (rate == 0.0f) // Zero indicates instantaneous tonemap scaling
    {
      ...
      if (mat_tonemap_algorithm.GetInt() == 1) {
        ...
        flRateTimesTime =
            min(flRateTimesTime,
                (1.0f / 16.0f) * 0.25f); // 16 is number of HDR sample bins
                                         // defined in viewpostprocess.cpp
        // Warning( " --> %.4f\n", flRateTimesTime );
      }
...
void CMatRenderContextBase::OnAsyncCreateTextureFromRenderTarget(
    ITexture *pSrcRt, const char **ppDstName,
  ...
  // Also, need to allocate a copy of the string and use that one s.t. the
...
// Map and unmap a texture. The pRecipient->OnAsyncMapComplete is called when
// complete.
void CMatRenderContextBase::OnAsyncMap(
    ITextureInternal *pTexToMap, IAsyncTextureOperationReceiver *pRecipient,
    void *pExtraArgs) {
...
void CMatRenderContextBase::OnAsyncCopyRenderTargetToStagingTexture(
    ITexture *pDst, ITexture *pSrc,
    IAsyncTextureOperationReceiver *pRecipient) {
  Assert(pDst != NULL);
  Assert(pSrc != NULL);
  Assert(pRecipient != NULL);
...
CMatRenderContext::GetMaterialInternal(MaterialHandle_t h) const {
...
inline ShaderAPITextureHandle_t
CMatRenderContext::GetBlackTextureHandle() const {
  return GetMaterialSystem()->GetBlackTextureHandle();
}

inline ShaderAPITextureHandle_t
CMatRenderContext::GetFlatNormalTextureHandle() const {
  return GetMaterialSystem()->GetFlatNormalTextureHandle();
}

inline ShaderAPITextureHandle_t
CMatRenderContext::GetGreyTextureHandle() const {
...
inline ShaderAPITextureHandle_t
CMatRenderContext::GetWhiteTextureHandle() const {
...
inline ShaderAPITextureHandle_t
CMatRenderContext::GetMaxDepthTextureHandle() const {
...
void CMatRenderContext::BeginRender() {
  ...
  if (GetMaterialSystem()->GetThreadMode() != MATERIAL_SINGLE_THREADED) {
    VPROF_INCREMENT_GROUP_COUNTER("render/CMatBeginRender",
                                  COUNTER_GROUP_TELEMETRY, 1);

    TelemetrySetLockName(TELEMETRY_LEVEL1, (char const *)&g_MatSysMutex,
                         "MatSysMutex");
    ...
    tmSetLockState(TELEMETRY_LEVEL1, (char const *)&g_MatSysMutex, TMLS_LOCKED,
                   "BeginRender");
  }
#endif
}
...
void CMatRenderContext::PushCustomClipPlane(const float *pPlane) {
  ...
  m_CustomClipPlanes.AddToTail(
      psePlane); // we're doing this as a UtlVector so height clip planes never
                 // change their cached index
  ApplyCustomClipPlanes();
}
...
void CMatRenderContext::PopCustomClipPlane(void) {
  ...
  Assert(i != -1); // only the height clip plane was found, which means this pop
                   // had no associated push
  ApplyCustomClipPlanes();
}
...
void CMatRenderContext::SetRenderTargetEx(int nRenderTargetID,
  ...
  if (pNewTarget != pOldTarget) {
    ...
    if (pNewTarget == NULL) {
      ...
      if (pNewTarget &&
          pNewTarget->GetImageFormat() == IMAGE_FORMAT_RGBA16161616F) {
        g_pShaderAPI->EnableLinearColorSpaceFrameBuffer(true);
      } else {
        g_pShaderAPI->EnableLinearColorSpaceFrameBuffer(false);
      }
...
void CMatRenderContext::DrawScreenSpaceQuad(IMaterial *pMaterial) {
  // Despite saying we render a full screen quad, this actually renders a single
  ...
  // Because we are working in clip space which is 2 units across, the
  ...
  static_assert(ARRAYSIZE(coords) == COORDS_COUNT,
                "Unexpected number of coords in triangle, should match enum.");
...
int CMatRenderContext::CompareMaterialCombos(IMaterial *pMaterial1,
  ...
  pMaterial1 = ((IMaterialInternal *)pMaterial1)
                   ->GetRealTimeVersion(); // always work with the real time
                                           // version of materials internally.
  ...
  int nBaseTexParamType1 = pParams1 && nParams1 > BASETEXTURE
                               ? pParams1[BASETEXTURE]->GetType()
                               : -1;
  int nBaseTexParamType2 = pParams2 && nParams2 > BASETEXTURE
                               ? pParams2[BASETEXTURE]->GetType()
                               : -1;
  if (nBaseTexParamType1 == MATERIAL_VAR_TYPE_TEXTURE ||
      nBaseTexParamType2 == MATERIAL_VAR_TYPE_TEXTURE) {
...
void CMatRenderContext::Bind(IMaterial *iMaterial, void *proxyData) {
  ...
  if (material->GetReferenceCount() <= 0) {
    ...
    if ((!matTextureListConVar.IsValid() || !matTextureListConVar.GetBool()) &&
        (!matShowWaterTextureConVar.IsValid() ||
         !matShowWaterTextureConVar.GetBool())) {
      Warning("Material %s has bad reference count %d when being bound\n",
              material->GetName(), material->GetReferenceCount());
      // The usual solution for this for global materials that really don't need
      // refcounting is to do material->AddRef();
      Assert(0);
      iMaterial = g_pErrorMaterial;
    }
  }
...
void CMatRenderContext::CopyRenderTargetToTextureEx(ITexture *pTexture,
  ...
  if (IsPC() || !IsX360()) {
    ...
    // X360 only does 1:1 resolves. So we can do full resolves to textures of
    // size equal or greater than the viewport trivially. Downsizing is nasty.
    Rect_t srcRect;
    if (!pSrcRect) {
    ...
    if (pSrcRect->width == pDstRect->width &&
      ...
      pTextureInternal->CopyFrameBufferToMe(nRenderTargetID, pSrcRect,
                                            pDstRect);
      return;
    }
    ...
    if ((pDstRect->x == 0) && (pDstRect->y == 0) &&
      ...
      // Resolve takes up the whole texture, and the texture is large enough to
      // hold the resolve. This is turned into a 1:1 resolve within shaderapi by
      // making D3D think the texture is smaller from now on. (Until it resolves
    ...
    ITextureInternal *pFullFrameFB =
        (ITextureInternal *)GetMaterialSystem()->FindTexture(
    ...
    DrawScreenSpaceRectangle(
        GetMaterialSystem()->GetRenderTargetBlitMaterial(), pDstRect->x,
        pDstRect->y, pDstRect->width, pDstRect->height, pSrcRect->x,
        pSrcRect->y, pSrcRect->x + pSrcRect->width - 1,
        pSrcRect->y + pSrcRect->height - 1, pFullFrameFB->GetActualWidth(),
        pFullFrameFB->GetActualHeight());
...
void CMatRenderContext::CopyTextureToRenderTargetEx(int nRenderTargetID,
                                                    ITexture *pTexture,
                                                    Rect_t *pSrcRect,
                                                    Rect_t *pDstRect) {
  if (!pTexture) {
...
void CMatRenderContext::DrawClearBufferQuad(unsigned char r, unsigned char g,
                                            ...
                                            bool bClearColor, bool bClearAlpha,
                                            bool bClearDepth) {
  IMaterialInternal *pClearMaterial = GetBufferClearObeyStencil(
...
void CMatRenderContext::BindLightmap(Sampler_t sampler) {
  ...
  default:
    Assert(
        (m_lightmapPageID == 0 && GetLightmaps()->GetNumLightmapPages() == 0) ||
        (m_lightmapPageID >= 0 &&
...
void CMatRenderContext::GetStandardTextureDimensions(int *pWidth, int *pHeight,
  ...
  if (pTexture) {
    ...
    *pHeight = pTexture->GetActualHeight();
    ...
    Warning("GetStandardTextureDimensions: Couldn't find the texture to get "
            "the dimensions!\n");
    *pWidth = *pHeight = -1;
  }
}
...
void CMatRenderContext::AsyncCreateTextureFromRenderTarget(
  ...
  TextureManager()->AsyncCreateTextureFromRenderTarget(
      pSrcRt, pDstName, dstFmt, bGenMips, nAdditionalCreationFlags, pRecipient,
      pExtraArgs);
}

void CMatRenderContext::AsyncMap(ITextureInternal *pTexToMap,
                                 IAsyncTextureOperationReceiver *pRecipient,
                                 void *pExtraArgs) {
  if (g_pMaterialSystem->GetThreadMode() == MATERIAL_SINGLE_THREADED) {
...
void CMatRenderContext::AsyncCopyRenderTargetToStagingTexture(
    ITexture *pDst, ITexture *pSrc, IAsyncTextureOperationReceiver *pRecipient,
    void *pExtraArgs) {
  if (g_pMaterialSystem->GetThreadMode() == MATERIAL_SINGLE_THREADED) {
