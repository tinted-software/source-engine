...
// this allows the command line to force the "all mips" flag to on for all
// textures
bool g_bForceTextureAllMips = false;

#if defined(IS_WINDOWS_PC)
static void ConVarChanged_mat_managedtextures(IConVar *var,
                                              const char *pOldValue,
                                              float flOldValue);
static ConVar
    mat_managedtextures("mat_managedtextures", "1", FCVAR_ARCHIVE,
                        "If set, allows Direct3D to manage texture uploading "
                        "at the cost of extra system memory",
...
static ConVar mat_spew_on_texture_size(
    "mat_spew_on_texture_size", "0", 0,
...
static ConVar mat_lodin_hidden_pop("mat_lodin_hidden_pop", "1",
...
#else
ConVar mat_spewalloc("mat_spewalloc", "0",
                     FCVAR_ARCHIVE | FCVAR_DEVELOPMENTONLY);
#endif
...
struct TexDimensions_t {
  ...
  TexDimensions_t(uint16 nWidth = 0, uint nHeight = 0, uint nMipCount = 0,
                  uint16 nDepth = 1)
      : m_nWidth(nWidth), m_nHeight(nHeight), m_nMipCount(nMipCount),
        m_nDepth(nDepth) {}
};
...
struct TexInfo_t {
  ...
  uint64 ComputeTexSize() const {
    ...
    for (int mip = 0; mip < m_nMipCount; ++mip) {
      ...
      total +=
          ImageLoader::GetMemRequired(width, height, depth, m_Format, false);
    ...
    return total * Min((unsigned short)1, m_nFrameCount) *
           Min((unsigned short)1, m_nCopies);
  }

  TexInfo_t(const char *name = "", unsigned short w = 0, unsigned short h = 0,
            unsigned short d = 0, unsigned short mips = 0,
            unsigned short frames = 0, unsigned short copies = 0,
            ImageFormat fmt = IMAGE_FORMAT_UNKNOWN)
      : m_nWidth(w), m_nHeight(h), m_nDepth(d), m_nMipCount(mips),
        m_nFrameCount(frames), m_nCopies(copies), m_Format(fmt) {
    if (name && name[0])
      m_Name = name;
    else
      m_Name = "<unnamed>";
...
static bool SLoadTextureBitsFromFile(
    IVTFTexture **ppOutVtfTexture, FileHandle_t hFile, unsigned int nFlags,
    ...
    TexDimensions_t *pOptOutMappingDims = NULL,
    TexDimensions_t *pOptOutActualDims = NULL,
    TexDimensions_t *pOptOutAllocatedDims = NULL,
    unsigned int *pOptOutStripFlags = NULL);
static int ComputeActualMipCount(const TexDimensions_t &actualDims,
                                 unsigned int nFlags);
static int ComputeMipSkipCount(
    const char *pName, const TexDimensions_t &mappingDims, bool bIgnorePicmip,
    IVTFTexture *pOptVTFTexture, unsigned int nFlags,
    int nDesiredDimensionLimit, unsigned short *pOutStreamedMips,
    TextureLODControlSettings_t *pInOutCachedFileLodSettings,
    TexDimensions_t *pOptOutActualDims, TexDimensions_t *pOptOutAllocatedDims,
    unsigned int *pOptOutStripFlags);
static int GetOptimalReadBuffer(CUtlBuffer *pOutOptimalBuffer,
                                FileHandle_t hFile, int nFileSize);
...
public:
  ...
  virtual void CopyMeToFrameBuffer(int nRenderTargetID = 0,
                                   Rect_t *pSrcRect = NULL,
                                   Rect_t *pDstRect = NULL);
  ...
  bool AsyncReadTextureFromFile(IVTFTexture *pVTFTexture,
                                unsigned int nAdditionalCreationFlags);
...
protected:
  ...
  bool GetFileHandle(FileHandle_t *pOutFileHandle, char *pCacheFilename,
                     char **ppResolvedFilename) const;
...
protected:
  ...
  // mapping dimensions and actual dimensions can/will vary due to user
  // settings, hardware support, etc. Allocated is what is physically allocated
  // on the hardware at this instant, and considers texture streaming.
  TexDimensions_t m_dimsMapping;
  TexDimensions_t m_dimsActual;
  TexDimensions_t m_dimsAllocated;

  // This is the iWidth/iHeight for whatever is downloaded to the card, ignoring
  // current streaming settings Some callers want to know how big the texture is
  // if all data was present, and that's this.
  // TODO: Rename this before check in.
  unsigned short m_nFrameCount;

  // These are the values for what is truly allocated on the card, including
  // streaming settings.
  ...
  unsigned short m_nOriginalRTHeight; // and height based on screen size and the
                                      // flags they specify.
  ...
  unsigned short m_nActualDimensionLimit; // value not necessarily accurate, but
                                          // mismatch denotes dirty state

  // m_pStreamingJob is refcounted, but it is not safe to call SafeRelease
  // directly on it--you must call CancelStreamingJob to ensure that releasing
  ...
  // If the read failed, this will be true. We can't just return from the
  // function because the call may happen in the async thread.
...
protected:
  ...
  struct ScratchVTF {
    ScratchVTF(CTexture *_tex)
        : m_pParent(_tex), m_pScratchVTF(_tex->GetScratchVTFTexture()) {}
    ~ScratchVTF() {
      if (m_pScratchVTF)
...
public:
  ...
  virtual bool AsyncReadTextureFromFile(IVTFTexture *pVTFTexture,
                                        unsigned int nAdditionalCreationFlags) {
  ...
  // Map and unmap. These can fail. And can cause a very significant perf
  // penalty. Be very careful with them.
  virtual void Map(void **pOutBits, int *pOutPitch) {}
  virtual void Unmap() {}
...
#ifdef _DEBUG
  if (m_nRefCount != 0) {
    Warning("Reference Count(%d) != 0 in ~CReferenceToHandleTexture for "
            "texture \"%s\"\n",
...
//-----------------------------------------------------------------------------
ITextureInternal *
ITextureInternal::CreateFileTexture(const char *pFileName,
...
ITextureInternal *ITextureInternal::CreateProceduralTexture(
    ...
    int d, ImageFormat fmt, int nFlags, ITextureRegenerator *generator) {
  CTexture *pTex = new CTexture;
...
void ITextureInternal::Destroy(ITextureInternal *pTex, bool bSkipTexMgrCheck) {
  ...
  if (!bSkipTexMgrCheck && TextureManager()->HasPendingTextureDestroys()) {
    // Multithreading badness. This will cause a crash later! Grab JohnS or
    // McJohn know!
...
CTexture::~CTexture() {
  ...
  // Deliberately stomp our VTable so that we can detect cases where code tries
  // to access freed materials.
  int *p = (int *)this;
  *p = 0xdeadbeef;
}
...
void CTexture::Init(int w, int h, int d, ImageFormat fmt, int iFlags,
  ...
  // Clear the m_nFlags bit. If we don't, then m_nFrameCount may end up being 1,
  // and
...
IVTFTexture *CTexture::GetScratchVTFTexture() {
  const bool cbThreadInMatQueue =
      (MaterialSystem()->GetRenderThreadId() == ThreadGetCurrentId());
  cbThreadInMatQueue;
...
void CTexture::ReleaseScratchVTFTexture(IVTFTexture *tex) {
  ...
  Assert(cbThreadInMatQueue || ThreadInMainThread());
  Assert(m_pStreamingVTF == NULL ||
         ThreadInMainThread()); // Can only manipulate m_pStreamingVTF to
                                // release safely in main thread.
...
void CTexture::ApplyRenderTargetSizeMode(int &width, int &height,
  ...
  case RT_SIZE_OFFSCREEN: {
    ...
    // Shrink the buffer if it's bigger than back buffer.  Otherwise, don't mess
    // with it.
  ...
  case RT_SIZE_LITERAL: {
    // Literal means literally don't mess with the dimensions. Unlike what
    // OFFSCREEN does, which is totally to mess with the dimensions.
  } break;

  case RT_SIZE_LITERAL_PICMIP: {
    // Don't do anything here, like literal. Later, we will pay attention to
    // picmip settings s.t. these render targets look like other textures wrt
    // Mapping Dimensions vs Actual Dimensions.
  } break;

  case RT_SIZE_REPLAY_SCREENSHOT: {
    // Compute all possible resolutions if first time we're running this
    // function
    ...
    MaterialSystem()->GetRenderTargetFrameBufferDimensions(nScreenWidth,
                                                           nScreenHeight);
    ...
    nUnpaddedHeight =
        m_aScreenshotWidths[iAspect][iRes] * s_flInvAspectRatios[iAspect];

    // Get dimensions for padded image based on unpadded size - must be power of
    // 2 for a material/texture
  ...
  default: {
    if (!HushAsserts()) {
      ...
      Assert(m_nOriginalRenderTargetType ==
             RENDER_TARGET_NO_DEPTH); // Only can use NO_CHANGE if we don't have
                                      // a depth buffer.
    }
  } break;
  }
...
void CTexture::CopyToStagingTexture(ITexture *pDstTex) {
  ...
  if ((m_pTextureHandles == NULL || m_nFrameCount == 0) ||
      (pDstTexActual->m_pTextureHandles == NULL ||
       pDstTexActual->m_nFrameCount == 0)) {
    Assert(!"Can't copy to a non-existent texture, may need to generate or "
            "something.");
    return;
  }
  ...
  g_pShaderAPI->CopyRenderTargetToScratchTexture(
      m_pTextureHandles[0], pDstTexActual->m_pTextureHandles[0]);
}
...
void CTexture::Map(void **pOutBits, int *pOutPitch) {
  ...
  if (m_pTextureHandles == NULL || m_nFrameCount == 0) {
    Assert(!"Can't map a non-existent texture, may need to generate or "
            "something.");
    return;
  }

  g_pShaderAPI->LockRect(pOutBits, pOutPitch, m_pTextureHandles[0], 0, 0, 0,
                         GetActualWidth(), GetActualHeight(), false, true);
}

void CTexture::Unmap() {
  if (m_pTextureHandles == NULL || m_nFrameCount == 0) {
    Assert(!"Can't unmap a non-existent texture, may need to generate or "
            "something.");
    return;
  }
...
bool CTexture::MakeResident(ResidencyType_t newResidence) {
  ...
  // If we already think we're supposed to go here, nothing to do and we should
  // report success.
...
void CTexture::UpdateLodBias() {
  ...
  if (!bPopIn)
    m_lodBiasCurrent =
        m_lodBiasInitial - (Plat_FloatTime() - m_lodBiasStartTime) /
                               mat_lodin_time.GetFloat() * m_lodBiasInitial;
  else
    m_lodBiasCurrent = m_lodBiasInitial = 0.0f;

  // If we're supposed to pop in when the object isn't visible and we have the
  // opportunity...
  if (mat_lodin_hidden_pop.GetBool() &&
      m_lastLodBiasAdjustFrame != g_FrameNum - 1)
    m_lodBiasCurrent = m_lodBiasInitial = 0.0f;
...
void CTexture::MakePartiallyResident() {
  ...
  if (oldCurrentResidence == RESIDENT_PARTIAL) {
    ...
    oldTargetResidence;
    // If we are already partially resident, then just cancel our job to stream
    // in, cause we don't need that data anymore.
    CancelStreamingJob();
  ...
  if (HardwareConfig()->CanStretchRectFromTextures()) {
    ...
    // Immediately display it as lower res (for consistency) but if we can't
    // (efficiently) copy we just have to re-read everything from disk. Lame!
    m_lodClamp = 3;
    m_lodBiasInitial = m_lodBiasCurrent = 0;
    m_lastLodBiasAdjustFrame = g_FrameNum;
    SetFilteringAndClampingMode(true);
    ...
    MaterialSystem()->AsyncFindTexture(
        GetName(), GetTextureGroupName(), m_pStreamingJob,
        (void *)RESIDENT_PARTIAL, false, TEXTUREFLAGS_STREAMABLE_COARSE);
  }
}
...
bool CTexture::MakeFullyResident() {
  ...
  Assert(m_residenceTarget == RESIDENT_PARTIAL &&
         m_residenceCurrent == RESIDENT_PARTIAL);
  Assert(m_pStreamingJob == NULL);

  SafeAssign(&m_pStreamingJob, new CTextureStreamingJob(this));
  MaterialSystem()->AsyncFindTexture(GetName(), GetTextureGroupName(),
                                     m_pStreamingJob, (void *)RESIDENT_FULL,
                                     false, TEXTUREFLAGS_STREAMABLE_FINE);
...
void CTexture::CancelStreamingJob(bool bJobMustExist) {
  bJobMustExist; // Only used by asserts ensuring correctness, so reference it
                 // for release builds.

  // Most callers should be aware of whether the job exists, but for cleanup we
  // don't know and we should be safe in that case.
  Assert(!bJobMustExist || m_pStreamingJob);
  if (!m_pStreamingJob)
    return;

  // The streaming job and this (this texture) have a circular reference
  // count--each one holds one for the other. As a result, this means that
  // having the streaming job forget about the texture may cause the texture to
  // go away completely! So we need to ensure that after we call "ForgetOwner"
  // that we don't touch any instance variables.
...
void CTexture::OnStreamingJobComplete(ResidencyType_t newResidenceCurrent) {
  ...
  // It's probable that if this assert fires, we should just do nothing in here
  // and return--but I'd like to see that happen to be sure.
...
void CTexture::OnRestore() {
  ...
  if (m_nFlags & TEXTUREFLAGS_RENDERTARGET) {
    ...
    int newWidth, newHeight;
    ApplyRenderTargetSizeMode(newWidth, newHeight, m_ImageFormat);
    if (newWidth != m_dimsMapping.m_nWidth ||
        newHeight != m_dimsMapping.m_nHeight) {
      Shutdown();
...
bool CTexture::AllocateShaderAPITextures() {
  ...
  bool bIsFloat = (m_ImageFormat == IMAGE_FORMAT_RGBA16161616F) ||
  ...
  if (m_nFlags & TEXTUREFLAGS_RENDERTARGET) {
    ...
    if ((m_nFlags & TEXTUREFLAGS_DEPTHRENDERTARGET) &&
        (nCount == 2)) // nCount must be 2 on pc
    {
      --nCount;
    }
  } else {
  ...
  g_pShaderAPI->CreateTextures(
      m_pTextureHandles, nCount, m_dimsAllocated.m_nWidth,
      m_dimsAllocated.m_nHeight, nShaderApiCreateTextureDepth, m_ImageFormat,
      m_dimsAllocated.m_nMipCount, nCopies, nCreateFlags, GetName(),
      GetTextureGroupName());
  ...
  if (m_nFlags & TEXTUREFLAGS_DEPTHRENDERTARGET) {
    ...
    m_pTextureHandles[1] = g_pShaderAPI->CreateDepthTexture(
        m_ImageFormat, m_dimsAllocated.m_nWidth, m_dimsAllocated.m_nHeight,
  ...
  STAGING_ONLY_EXEC(g_currentTextures.InsertOrReplace(
      this,
      TexInfo_t(GetName(), m_dimsAllocated.m_nWidth, m_dimsAllocated.m_nHeight,
                m_dimsAllocated.m_nDepth, m_dimsAllocated.m_nMipCount,
                accountingCount, nCopies, m_ImageFormat)));
...
#ifdef STAGING_ONLY
    // If this hits, there's a leak because we're not deallocating enough
    // textures. Yikes!
    Assert(g_currentTextures[g_currentTextures.Find(this)].m_nFrameCount ==
           m_nFrameCount);
...
void CTexture::MigrateShaderAPITextures() {
  ...
  ShaderAPITextureHandle_t *pTextureHandles =
      (ShaderAPITextureHandle_t *)stackalloc(cBytes);
  ...
  for (int i = 0; i < m_nFrameCount; ++i) {
    Assert(g_pShaderAPI->IsTexture(pTextureHandles[i]) ==
           g_pShaderAPI->IsTexture(m_pTextureHandles[i]));
    if (!g_pShaderAPI->IsTexture(pTextureHandles[i]))
      continue;

    g_pShaderAPI->CopyTextureToTexture(pTextureHandles[i],
                                       m_pTextureHandles[i]);
  }
...
ImageFormat CTexture::ComputeActualFormat(ImageFormat srcFormat) {
  ...
  if (g_config.bCompressedTextures &&
    ...
    // for the runtime compressed formats the srcFormat won't equal the
    // dstFormat, and we need to return srcFormat here
...
int CTexture::ComputeActualSize(bool bIgnorePicmip, IVTFTexture *pVTFTexture,
                                bool bTextureMigration) {
  unsigned int stripFlags = 0;
  return ComputeMipSkipCount(GetName(), m_dimsMapping, bIgnorePicmip,
                             pVTFTexture, m_nFlags, m_nDesiredDimensionLimit,
                             &m_nStreamingMips, &m_cachedFileLodSettings,
                             &m_dimsActual, &m_dimsAllocated, &stripFlags);
...
void CTexture::SetLodState() {
  // Set the lod clamping value to ensure we don't see anything we're not
  // supposed to.
...
void CTexture::Download(Rect_t *pRect, int nAdditionalCreationFlags /* = 0 */) {
  ...
  if (g_pShaderAPI->CanDownloadTextures()) {
    ...
    m_nFlags |=
        nAdditionalCreationFlags; // Path to let stdshaders drive settings like
                                  // sRGB-ness at creation time
...
bool CTexture::SaveToFile(const char *fileName) {
  ...
  if (pTexture->GetImageFormat() == IMAGE_FORMAT_RGBA8888 ||
    ...
    if (width > pTexture->GetActualWidth() ||
      ...
      pTexture = materials->CreateNamedRenderTargetTextureEx(
          "_rt_savetofile", width, height, RT_SIZE_LITERAL,
    ...
    if ((width > 0) && (height > 0)) {
      ...
      if (pixelValue) {
        ...
        pRenderContext->PushRenderTargetAndViewport(pTexture, 0, 0, width,
                                                    height);
        pRenderContext->CopyTextureToRenderTargetEx(0, this, &SrcRect,
                                                    &DstRect);

        pRenderContext->ReadPixels(0, 0, width, height,
                                   (unsigned char *)pixelValue,
                                   pTexture->GetImageFormat());

        // Slap the alpha channel at the bottom of the tga file so we don't have
        // to deal with crappy tools that can't
        //  handle rgb + alpha well. This means we can just do a
        //  "mat_texture_save_fonts" concommand, and then use
        //	something like Beyond Compare to look at the fonts differences
        //between various platforms, etc.
        CPixelWriter pixelWriterSrc;
        CPixelWriter pixelWriterDst;
        pixelWriterSrc.SetPixelMemory(pTexture->GetImageFormat(), pixelValue,
                                      width * sizeof(BGRA8888_t));
        pixelWriterDst.SetPixelMemory(pTexture->GetImageFormat(), pixelValue,
                                      width * sizeof(BGRA8888_t));
        ...
        if (TGAWriter::WriteTGAFile(
                fileName, width, height * 2, pTexture->GetImageFormat(),
                (uint8 *)pixelValue, width * sizeof(BGRA8888_t))) {
          bRet = true;
        }
...
bool CTexture::AsyncReadTextureFromFile(IVTFTexture *pVTFTexture,
                                        unsigned int nAdditionalCreationFlags) {
  tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s", __FUNCTION__);
  ...
  tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s - %s", __FUNCTION__,
         tmDynamicString(TELEMETRY_LEVEL0, pCacheFileName));
  ...
  if (!SLoadTextureBitsFromFile(&pVTFTexture, fileHandle,
                                m_nFlags | nPreserveFlags, &settings,
                                m_nDesiredDimensionLimit, &dontCareStreamedMips,
                                GetName(), pCacheFileName, &m_dimsMapping)) {
...
void CTexture::Bind(Sampler_t sampler1, int nFrame,
  ...
  if (g_pShaderDevice->IsUsingGraphics()) {
    ...
    if (HasBeenAllocated()) {
      ...
      ExecuteNTimes(20,
                    Warning("Trying to bind texture %s, but texture handles "
                            "are not valid. Binding a white texture!\n",
                            GetName()));
...
bool CTexture::SetRenderTarget(int nRenderTargetID, ITexture *pDepthTexture) {
  ...
  ShaderAPITextureHandle_t depthTextureHandle =
      (uintp)SHADER_RENDERTARGET_DEPTHBUFFER;
...
void CTexture::DecrementReferenceCount(void) {
  if ((--m_nRefCount <= 0) &&
      (m_nFlags & TEXTUREFLAGS_IMMEDIATE_CLEANUP) != 0) {
    Assert(m_nRefCount == 0);
    // Just inform the texture manager, it will decide to free us at a later
    // date.
...
void CTexture::Precache() {
  ...
  if ((m_nFlags & TEXTUREFLAGS_STREAMABLE) != 0) {
    // If we were previously streamed in, make sure we still do this time
    // around.
    nAdditionalFlags = TEXTUREFLAGS_STREAMABLE_COARSE;
    Assert((m_nFlags & TEXTUREFLAGS_STREAMABLE_FINE) == 0);
    Assert(m_residenceCurrent == RESIDENT_NONE &&
           m_residenceTarget == RESIDENT_NONE);
    Assert(m_lodClamp == 0);
...
// multiplied with, say, vertex colors which are tinted, resulting in black
// pixels.
    ...
    for (int iFrame = 0; iFrame < pTexture->FrameCount(); ++iFrame) {
      for (int iFace = 0; iFace < pTexture->FaceCount(); ++iFace) {
        for (int iMip = 0; iMip < pTexture->MipCount(); ++iMip) {
          int red =
              sg_nMipLevelColors[iMip][0]; //( ( iMip + 1 ) & 2 ) ? 255 : 0;
          int green =
              sg_nMipLevelColors[iMip][1]; //( ( iMip + 1 ) & 1 ) ? 255 : 0;
          int blue =
              sg_nMipLevelColors[iMip][2]; //( ( iMip + 1 ) & 4 ) ? 255 : 0;
          ...
          if (pTexture->Format() == IMAGE_FORMAT_DXT1 ||
            ...
            int alpha = 255;
            FillCompressedTextureWithSingleColor(red, green, blue, alpha,
                                                 pImageData, nWidth, nHeight,
...
void CTexture::CopyLowResImageToTexture(IVTFTexture *pTexture) {
  ...
  Init(pTexture->Width(), pTexture->Height(), 1, IMAGE_FORMAT_BGR888, nFlags,
       1);
...
bool CTexture::SetupDebuggingTextures(IVTFTexture *pVTFTexture) {
  ...
  if (g_config.nShowMipLevels &&
      !(pVTFTexture->Flags() & TEXTUREFLAGS_ALL_MIPS)) {
...
bool CTexture::ConvertToActualFormat(IVTFTexture *pVTFTexture) {
  ...
  if (fmt != dstFormat) {
    tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s - conversion from (%d to %d)",
           __FUNCTION__, fmt, dstFormat);
    ...
    // This is to force at most the precision of int16 for fp16 texture when
...
IVTFTexture *CTexture::LoadTextureBitsFromFile(char *pCacheFileName,
  ...
  if (!pVTFTexture) {
    ...
    TextureLODControlSettings_t settings = m_cachedFileLodSettings;
    if (!SLoadTextureBitsFromFile(
            &pVTFTexture, fileHandle, m_nFlags | nPreserveFlags, &settings,
            m_nDesiredDimensionLimit, &m_nStreamingMips, GetName(),
            pCacheFileName, &m_dimsMapping, &m_dimsActual, &m_dimsAllocated,
            &stripFlags)) {
      g_pFullFileSystem->Close(fileHandle);
      return HandleFileLoadFailedTexture(pVTFTexture);
    }
  ...
  // Don't reinitialize here if we're streaming in the fine levels, we already
  // have been initialized with coarse.
  if ((m_nFlags & TEXTUREFLAGS_STREAMABLE_FINE) == 0) {
    // Initing resets these, but we're happy with the values now--so store and
    // restore them around the Init call.
    ...
    Init(m_dimsMapping.m_nWidth, m_dimsMapping.m_nHeight,
         m_dimsMapping.m_nDepth, pVTFTexture->Format(),
  ...
  if (m_pStreamingVTF)
    ComputeActualSize(false, pVTFTexture,
                      (m_nFlags & TEXTUREFLAGS_STREAMABLE) != 0);

  VectorCopy(pVTFTexture->Reflectivity(), m_vecReflectivity);

  // If we've only streamed in coarse but haven't started on fine yet, go ahead
  // and mark us as partially resident and set up our clamping values.
  ...
  if (unsigned int uiRsrcCount = pVTFTexture->GetResourceTypes(NULL, 0)) {
    uint32 *arrRsrcTypes =
        (uint32 *)_alloca(uiRsrcCount * sizeof(unsigned int));
  ...
  if (ConvertToActualFormat(pVTFTexture))
    pVTFTexture; // STAGING_ONLY_EXEC ( Warning( "\"%s\" not in final format,
                 // this is causing stutters or load time bloat!\n",
                 // pCacheFileName ) );
...
static void QueuedLoaderCallback(void *pContext, void *pContext2,
                                 ...
                                 LoaderError_t loaderError) {
  reinterpret_cast<CTexture *>(pContext)->FixupTexture(pData, nSize,
                                                       loaderError);
}
...
IVTFTexture *CTexture::ReconstructPartialProceduralBits(const Rect_t *pRect,
  ...
  bool bIgnorePicmip =
      (m_nFlags & (TEXTUREFLAGS_STAGING_MEMORY | TEXTUREFLAGS_IGNORE_PICMIP)) !=
      0;
  ...
  pVTFTexture->Init(m_dimsActual.m_nWidth, m_dimsActual.m_nHeight,
                    m_dimsActual.m_nDepth, ComputeActualFormat(m_ImageFormat),
                    m_nFlags, m_nFrameCount);
...
IVTFTexture *CTexture::ReconstructProceduralBits() {
  ...
  {
    tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s - GetScratchVTFTexture",
           __FUNCTION__);
    pVTFTexture = GetScratchVTFTexture();
  }
  ...
  // Generate the bits from the installed procedural regenerator
  if (m_pTextureRegenerator) {
    tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s - RegenerateTextureBits",
           __FUNCTION__);
    ...
    tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s - GenerateErrorTexture",
           __FUNCTION__);
...
void CTexture::ReconstructTexture(bool bCopyFromCurrent) {
  ...
  if (!HasBeenAllocated() || m_dimsAllocated.m_nWidth != oldWidth ||
    ...
    const bool cbCanStretchRectTextures =
        HardwareConfig()->CanStretchRectFromTextures();
    const bool cbShouldMigrateTextures =
        ((m_nFlags & TEXTUREFLAGS_STREAMABLE_FINE) != 0) &&
        m_nFrameCount == oldFrameCount;

    // If we're just streaming in more data--or demoting ourselves, do a
    // migration instead.
    if (bCopyFromCurrent ||
        (cbCanStretchRectTextures && cbShouldMigrateTextures)) {
      tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s - Migration", __FUNCTION__);

      MigrateShaderAPITextures();

      // Ahh--I feel terrible about this, but we genuinely don't need anything
      // else if we're streaming.
      if (bCopyFromCurrent)
        return;
    } else {
      tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s - Deallocate / Allocate",
             __FUNCTION__);

      // If we're doing a wholesale copy, we need to restore these values that
      // will be cleared by FreeShaderAPITextures. Record them here, restore
      // them below.
      ...
      // Restored once we successfully allocate the shader api textures, but
      // only if we're
      //
  ...
  if (IsRenderTarget()) {
    ...
    if (!IsDepthTextureFormat(m_ImageFormat)) {
      ...
      pRenderContext
          ->PopRenderTargetAndViewport(); // Pop back to previous target
    }
    // no upload
    return;
  }
  ...
  WriteDataToShaderAPITexture(m_nFrameCount, nFaceCount, nFirstFace,
                              m_dimsActual.m_nMipCount, pVTFTexture,
                              m_ImageFormat);
...
void CTexture::GetCacheFilename(char *pOutBuffer, int nBufferSize) const {
  ...
  if (IsProcedural() || IsRenderTarget()) {
    ...
    bool bIsUNCName = (pName[0] == '/' && pName[1] == '/' && pName[2] != '/');
    if (!bIsUNCName) {
      Q_snprintf(pOutBuffer, nBufferSize,
                 "materials/%s" TEXTURE_FNAME_EXTENSION, pName);
    } else {
...
bool CTexture::GetFileHandle(FileHandle_t *pOutFileHandle, char *pCacheFileName,
                             char **ppResolvedFilename) const {
    ...
    if (fileHandle == FILESYSTEM_INVALID_HANDLE) {
      ...
      if (pHdrExt) {
        DevWarning("A custom HDR cubemap \"%s\": cannot be found on disk.\n"
                   "This really should have a HDR version, trying a fall back "
...
ShaderAPITextureHandle_t CTexture::GetTextureHandle(int nFrame,
  ...
  if (nFrame >= m_nFrameCount) {
    // NOTE: This can happen during alt-tab.  If you alt-tab while loading a
    // level then the first local cubemap bind will do this, for example.
  ...
  // Don't get paired handle here...callers of this function don't know about
  // paired textures
...
void CTexture::CopyMeToFrameBuffer(int nRenderTargetID, Rect_t *pSrcRect,
                                   Rect_t *pDstRect) {
  Assert(m_pTextureHandles && m_nFrameCount >= 1);

  if (m_pTextureHandles && m_nFrameCount >= 1) {
    g_pShaderAPI->CopyTextureToRenderTargetEx(
        nRenderTargetID, m_pTextureHandles[0], pSrcRect, pDstRect);
  }
}
...
void CTexture::DeleteIfUnreferenced() {
  ...
  // Can't actually clean up from render thread--just safely mark this texture
  // as one we should check for cleanup next EndFrame when it's safe.
  TextureManager()->MarkUnreferencedTextureForCleanup(this);
}
...
void CTextureStreamingJob::OnAsyncFindComplete(ITexture *pTex,
                                               void *pExtraArgs) {
...
int GetThreadId() {
  ...
  // Turns the current thread into a 0-based index for use in accessing statics
  // in this file.
  int retVal = INT_MAX;
  if (ThreadInMainThread())
    retVal = 0;
    ...
    STAGING_ONLY_EXEC(AssertAlways(!"Unexpected thread in GetThreadId, need to "
                                    "debug this--crash is next. Tell McJohn."));
    DebuggerBreakIfDebugging_StagingOnly();
  }
...
bool SLoadTextureBitsFromFile(
  ...
  // NOTE! NOTE! NOTE! If you are making changes to this function, be aware that
  // it has threading NOTE! NOTE! NOTE! implications. It can be called
  // synchronously by the Main thread, NOTE! NOTE! NOTE! or by the streaming
  // texture code!
  ...
  {
    tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s - ReadHeaderFromFile",
           __FUNCTION__);
    int nHeaderSize = VTFFileHeaderSize(VTF_MAJOR_VERSION);
    ...
    int nBytesRead = g_pFullFileSystem->ReadEx(
        buf.Base(), nBytesOptimalRead,
  ...
  // Need to record this now, before we ask for the trimmed down data to
  // potentially be loaded.
  TexDimensions_t dimsMappingCurrent(
      (*ppOutVtfTexture)->Width(), (*ppOutVtfTexture)->Height(),
  ...
  int nMipSkipCount = ComputeMipSkipCount(
      pName, dimsMappingCurrent, false, *ppOutVtfTexture, nFullFlags,
  ...
  if (nActualFileSize < nFileSize) {
    if (mat_spew_on_texture_size.GetInt())
      DevMsg(
          "Bad VTF data for %s, expected file size:%d actual file size:%d \n",
  ...
  // Some hardware doesn't support copying textures to other textures. For them,
  // we need to reread the whole file, so if they are doing the final read (the
  // fine levels) then reread everything by stripping off the flags we are
  // trying to pass in.
  unsigned int nForceFlags = nFullFlags & TEXTUREFLAGS_STREAMABLE;
  if (!HardwareConfig()->CanStretchRectFromTextures() &&
      (nForceFlags & TEXTUREFLAGS_STREAMABLE_FINE))
    nForceFlags = 0;

  // NOTE: Skipping mip levels here will cause the size to be changed
  bool bRetVal =
      (*ppOutVtfTexture)->UnserializeEx(buf, false, nForceFlags, nMipSkipCount);
...
//-----------------------------------------------------------------------------
int ComputeActualMipCount(const TexDimensions_t &actualDims,
                          unsigned int nFlags) {
  ...
  return ImageLoader::GetNumMipMapLevels(
      actualDims.m_nWidth, actualDims.m_nHeight, actualDims.m_nDepth);
}

// ------------------------------------------------------------------------------------------------
int ComputeMipSkipCount(
    const char *pName, const TexDimensions_t &mappingDims, bool bIgnorePicmip,
    IVTFTexture *pOptVTFTexture, unsigned int nFlags,
    int nDesiredDimensionLimit, unsigned short *pOutStreamedMips,
    TextureLODControlSettings_t *pInOutCachedFileLodSettings,
  ...
  int nClampX =
      actualDims.m_nWidth; // no clamping (clamp to texture dimensions)
  ...
  if (pOptVTFTexture) {
    ...
    // Texture streaming means there are times we call this where we don't have
    // a VTFTexture, even though we're a file. So we need to store off the LOD
    // settings whenever we get in here with a file that has them so that we can
    // use the correct values for when we don't. Otherwise, the texture will be
    // confused about what size to use and everything will die a horrible,
    // horrible death.
    if (pLODInfo)
  ...
  while ((actualDims.m_nWidth > nClampX) || (actualDims.m_nHeight > nClampY) ||
  ...
  Assert(actualDims.m_nWidth > 0 && actualDims.m_nHeight > 0 &&
         actualDims.m_nDepth > 0);
  ...
  // We can only stream things that have a mipmap pyramid (not just a single
  // mipmap).
  ...
  if ((nFlags & TEXTUREFLAGS_STREAMABLE) == TEXTUREFLAGS_STREAMABLE_COARSE) {
    if (actualDims.m_nMipCount > 1) {
      ...
      for (int i = 0; i < STREAMING_START_MIPMAP; ++i) {
        // Stop when width or height is at 4 pixels (or less). We could do
        // better, but some textures really can't function if they're less than
        // 4 pixels (compressed textures, for example).
...
//-----------------------------------------------------------------------------
int GetOptimalReadBuffer(CUtlBuffer *pOutOptimalBuffer, FileHandle_t hFile,
                         int nSize) {
  // NOTE! NOTE! NOTE! If you are making changes to this function, be aware that
  // it has threading NOTE! NOTE! NOTE! implications. It can be called
  // synchronously by the Main thread, NOTE! NOTE! NOTE! or by the streaming
  ...
  (*pOutOptimalBuffer)
      .SetExternalBuffer(s_pOptimalReadBuffer[ti], s_nOptimalReadBufferSize[ti],
                         0);
...
CON_COMMAND(dumptexallocs,
            "List currently allocated textures and properties about them") {
...
static ConVar mat_texture_list_content_path(
    "mat_texture_list_content_path", "", FCVAR_ARCHIVE,
  ...
  if (!stricmp(szCmd, "reset")) {
    for (int k = 0; k < s_OverrideMap.GetNumStrings(); ++k) {
      ...
      if (ITexture *pTx = materials->FindTexture(szTx, ""))
        ...
        Warning(" mat_texture_list_txlod_sync reset - texture '%s' no longer "
                "found.\n",
                szTx);
    }
    ...
    for (int k = 0; k < s_OverrideMap.GetNumStrings(); ++k) {
      ...
      if (!pTx) {
        Warning(" mat_texture_list_txlod_sync save - texture '%s' no longer "
                "found.\n",
                szTx);
        continue;
      }
      ...
      if (!mat_texture_list_content_path.GetString()[0]) {
        ...
        if (!szTextureContentPath) {
          Warning(" mat_texture_list_txlod_sync save - texture '%s' is not "
        ...
        if (!BufferReplace(szTextureContentPath, "\\game\\", "\\content\\") ||
            !BufferReplace(szTextureContentPath, "\\materials\\",
                           "\\materialsrc\\")) {
          Warning(" mat_texture_list_txlod_sync save - texture '%s' cannot be "
                  "mapped to content directory.\n",
                  szTx);
          continue;
        }
      } else {
        V_strncpy(chResolveName, mat_texture_list_content_path.GetString(),
                  MAX_PATH);
      ...
      char *pExtPut = szTextureContentPath + strlen(szTextureContentPath) -
                      strlen(TEXTURE_FNAME_EXTENSION); // compensating the
      ...
      if (g_pFullFileSystem->FileExists(szTextureContentPath)) {
        ...
        if (g_pFullFileSystem->WriteFile(szTextureContentPath, 0,
          ...
          Warning(" '%s' : failed to save - set \"maxwidth %d maxheight %d\" "
                  "manually.\n",
...
usage:
  Warning("Usage:\n"
          "  mat_texture_list_txlod_sync reset - resets all run-time changes "
          "to LOD overrides;\n"
