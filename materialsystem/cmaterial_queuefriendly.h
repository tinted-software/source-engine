...
class CMaterial_QueueFriendly
    : public IMaterialInternal // wraps a CMaterial with queue friendly
                               // functions for game/engine code.
                               // materialsystem/shaderapi code should use
                               // CMaterial directly.
{
public:
  ...
  virtual bool
  IsErrorMaterial() const; // should probably return the realtime error material
                           // instead of this wrapper for it
  virtual bool
  IsSpriteCard(); // lets just assume nobody changes the shader to spritecard
                  // and immediately asks if it's a spritecard
  ...
  virtual bool
  NeedsPowerOfTwoFrameBufferTexture(bool bCheckSpecificToThisFrame = true);
  virtual bool
  NeedsFullFrameBufferTexture(bool bCheckSpecificToThisFrame = true);
