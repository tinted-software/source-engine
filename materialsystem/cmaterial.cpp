...
public:
  ...
  bool IsTranslucent();
  bool IsTranslucentInternal(float fAlphaModulation)
      const; // need to centralize the logic without relying on the *current*
             // alpha modulation being that which is stored in
...
private:
  ...
  // Array to track names of above material proxies. Useful for tracking down
  // issues with proxies.
...
static void ApplyPatchKeyValues(KeyValues &keyValues,
                                KeyValues &patchKeyValues);
static bool
AccumulateRecursiveVmtPatches(KeyValues &patchKeyValuesOut,
...
IMaterialInternal *
IMaterialInternal::CreateMaterial(char const *pMaterialName,
...
void IMaterialInternal::DestroyMaterial(IMaterialInternal *pMaterial) {
  ...
  if (pMaterial) {
    ...
    // Deletion of the error material is deferred until after all other
    // materials have been deleted. See CMaterialSystem::CleanUpErrorMaterial()
    // in cmaterialsystem.cpp.
...
#if defined(_X360)
  // material names are expected to be forward slashed for correct sort and find
  // behavior! assert now to track alternate or regressed path that is source of
  // inconsistency
...
void CMaterial::SetShaderAndParams(KeyValues *pKeyValues) {
  ...
  if (pLoadedKeyValues->LoadFromFile(g_pFullFileSystem, pFileName, pPathID)) {
    ...
    if (V_stricmp(pLoadedKeyValues->GetName(), "patch") == 0) {
      ...
      bool bSuccess = AccumulateRecursiveVmtPatches(
          *pPatchKeyValues, NULL, *pLoadedKeyValues, pPathID, NULL);
      if (bSuccess) {
...
void CMaterial::DetermineProxyReplacements(KeyValues *pFallbackKeyValues) {
  m_pReplacementProxy =
      MaterialSystem()->DetermineProxyReplacements(this, pFallbackKeyValues);
}
...
int ParseVectorFromKeyValueString(KeyValues *pKeyValue,
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (IsEndline(*pScan) || *pScan == ']' || *pScan == '}') {
      if (*pScan != ']' && *pScan != '}') {
        Warning("Warning in .VMT file (%s): no ']' or '}' found in vector key "
                "\"%s\".\n"
  ...
  if ((!pQuestion) || (pQuestion == pVarName)) {
    ...
    *pWasConditional = true;
    ...
    if (!stricmp(pCond, "lowfill")) {
      ...
      bShouldSkip =
          false; //( HardwareConfig()->GetHDRType() == HDR_TYPE_NONE );
...
int CMaterial::ParseMaterialVars(IShader *pShader, KeyValues &keyValues,
  ...
  while (pVar) {
    ...
    if ((nFindContext == MATERIAL_FINDCONTEXT_ISONAMODEL) && pszVarName &&
        pszVarName[0]) {
      ...
      if (varIdx >= 0) {
        if (ppVars[varIdx] && (!bIsConditionalVar)) {
          if (!pOverride[varIdx] || parsingOverrides) {
            Warning("Error! Variable \"%s\" is multiply defined in material "
                    "\"%s\"!\n",
...
KeyValues *CMaterial::InitializeShader(KeyValues &keyValues,
  ...
  while (true) {
    ...
    if (!pShader) {
      ...
      if (!pShader) {
#ifdef LINUX
        // Exit out here. We're running into issues where this material is
        // returned in a horribly broken
        //  state and you wind up crashing in LockMesh() because the vertex and
        //  index buffer pointers are NULL. You can repro this by not dying here
        //  and showing the intro movie. This happens on Linux when you run from
        //  a symlink'd SteamApps directory.
        Error("Shader '%s' for material '%s' not found.\n",
              pCurrentFallback->GetName() ? pCurrentFallback->GetName()
                                          : pShaderName,
              GetName());
#endif
        MaterialSystem()->Unlock(hMaterialLock);
    ...
    varCount = ParseMaterialVars(pShader, keyValues, pFallbackSection,
                                 modelDefault, ppVars, nFindContext);
    ...
    */
    ...
    if (pFallbackMaterial[0]) {
      ...
      if (Q_stricmp(GetName(), pFallbackMaterial)) {
        ...
        Warning("CMaterial::PrecacheVars: fallback material for vmt file %s is "
                "itself!\n",
                GetName());
        keyValues = *(((CMaterial *)g_pErrorMaterial)->m_pVMTKeyValues);
      }
...
inline void CMaterial::SetMaterialVarFlags(int flags, bool on) {
  if (!m_pShaderParams) {
    Assert(0); // are we hanging onto a material that has been cleaned up or
               // isn't ready?
    return;
  }
...
inline void CMaterial::SetMaterialVarFlags2(int flags, bool on) {
  ...
  if (m_pShaderParams && m_VarCount > FLAGS_DEFINED2 &&
      m_pShaderParams[FLAGS_DEFINED2]) {
    // Mark it as being defined...
...
#if DEBUG
    if (IsOSX()) {
      printf("\n ##### CMaterial::ReloadFromWhitelistIfMarked: GetShader "
             "failed on %s, calling SetupErrorShader",
             m_pDebugName);
    }
#endif
...
bool CMaterial::PrecacheVars(KeyValues *pVMTKeyValues,
  ...
  if (!bError) {
    ...
    KeyValues *pFallbackKeyValues =
        InitializeShader(*vmtKeyValues, *patchKeyValues, nFindContext);
...
void CMaterial::Uncache(bool bPreserveVars) {
  ...
  if ((m_Flags & (MATERIAL_VARS_IS_PRECACHED | MATERIAL_IS_MANUALLY_CREATED |
                  MATERIAL_USES_UNC_FILENAME)) == 0) {
...
CMaterial::GetPreviewImageProperties(int *width, int *height,
                                     ImageFormat *imageFormat,
                                     bool *isTranslucent) const {
...
void MergeKeyValues(KeyValues &srcKeys, KeyValues &destKeys) {
  for (KeyValues *pKV = srcKeys.GetFirstValue(); pKV;
       pKV = pKV->GetNextValue()) {
  ...
  for (KeyValues *pKV = srcKeys.GetFirstTrueSubKey(); pKV;
       pKV = pKV->GetNextTrueSubKey()) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void AccumulatePatchKeyValues(KeyValues &srcKeyValues,
                              KeyValues &patchKeyValues) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool AccumulateRecursiveVmtPatches(KeyValues &patchKeyValuesOut,
                                   KeyValues **ppBaseKeyValuesOut,
  ...
  if (V_stricmp(keyValues.GetName(), "patch") != 0) {
    ...
    if (ppBaseKeyValuesOut) {
      // flag to the caller that the passed in keyValues are in fact final
      // non-patch values
  ...
  int nCount = 0;
  while ((nCount < 10) &&
         (V_stricmp(pCurrentKeyValues->GetName(), "patch") == 0)) {
    ...
    CUtlString includeFileName(
        pIncludeFileName); // copy off the string before we clear the keyvalues
                           // it lives in
    pCurrentKeyValues->Clear();
    bool bSuccess = pCurrentKeyValues->LoadFromFile(g_pFullFileSystem,
                                                    includeFileName, pPathID);
    if (bSuccess) {
      if (pIncludes) {
        // Remember that we included this file for the pure server stuff.
        pIncludes->AddToTail(
            g_pFullFileSystem->FindOrAddFileName(includeFileName));
      }
    } else {
...
#endif
      if (!HushAsserts()) {
        AssertMsg(false, "Failed to load $include VMT file (%s)",
                  includeFileName.String());
      }
      return false;
    }
...
void ExpandPatchFile(KeyValues &keyValues, KeyValues &patchKeyValues,
  ...
  if (!patchKeyValues.IsEmpty()) {
    ...
    bool bSuccess = AccumulateRecursiveVmtPatches(
        patchKeyValues, &pNonPatchKeyValues, keyValues, pPathID, pIncludes);
    if (!bSuccess) {
      return;
    }
  }
