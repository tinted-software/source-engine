...
ConVar mat_supportflashlight(
    ...
    "0 - do not support flashlight (don't load flashlight shader combos), 1 - "
...
ConVar mat_texture_reload_frame_swap_workaround(
    ...
    "Workaround certain GL drivers holding unnecessary amounts of data when "
    "loading many materials by forcing synthetic frame swaps");

// This ConVar allows us to skip ~40% of our map load time, but it doesn't work
// on GPUs older than ~2005. We set it automatically and don't expose it to
// players.
ConVar mat_requires_rt_alloc_first("mat_requires_rt_alloc_first", "0",
                                   FCVAR_HIDDEN);

// Make sure this convar gets created before videocfg.lib is initialized, so it
// can be driven by dxsupport.cfg
...
static int ReadListFromFile(CUtlVector<char *> *outReplacementMaterials,
  ...
  virtual bool CreateResource(const char *pName) {
    ...
    if (pMatInternal) {
      ...
      if (!pMatInternal->IsErrorMaterial()) {
        ...
        if (IsPosix()) {
          printf("\n ##### CResourcePreloadMaterial::CreateResource can't find "
                 "material %s\n",
                 pName);
        }
      }
...
void CMaterialSystem::CreateDebugMaterials() {
  if (!m_pDrawFlatMaterial) {
    ...
    m_pBufferClearObeyStencil[BUFFER_CLEAR_DEPTH] =
        static_cast<IMaterialInternal *>(
            CreateMaterial("___buffer_clear_obey_stencil4.vmt", pVMTKeyValues))
            ->GetRealTimeVersion();
    ...
    m_pBufferClearObeyStencil[BUFFER_CLEAR_COLOR_AND_DEPTH] =
        static_cast<IMaterialInternal *>(
            CreateMaterial("___buffer_clear_obey_stencil5.vmt", pVMTKeyValues))
            ->GetRealTimeVersion();
    ...
    m_pBufferClearObeyStencil[BUFFER_CLEAR_ALPHA_AND_DEPTH] =
        static_cast<IMaterialInternal *>(
            CreateMaterial("___buffer_clear_obey_stencil6.vmt", pVMTKeyValues))
            ->GetRealTimeVersion();
    ...
    m_pBufferClearObeyStencil[BUFFER_CLEAR_COLOR_AND_ALPHA_AND_DEPTH] =
        static_cast<IMaterialInternal *>(
            CreateMaterial("___buffer_clear_obey_stencil7.vmt", pVMTKeyValues))
            ->GetRealTimeVersion();
    ...
    if (IsX360()) {
      ...
      m_pRenderTargetBlitMaterial =
          static_cast<IMaterialInternal *>(
              CreateMaterial("___renderTargetBlit.vmt", pVMTKeyValues))
              ->GetRealTimeVersion();
    }
...
void CMaterialSystem::CreateCompositorMaterials() {
  ...
  for (int i = ECO_FirstPrecacheMaterial; i < ECO_LastPrecacheMaterial; i++) {
    ...
    IMaterialInternal *pMatqf = assert_cast<IMaterialInternal *>(
        FindMaterial(pszMaterial, TEXTURE_GROUP_RUNTIME_COMPOSITE));
    Assert(pMatqf);
...
void CMaterialSystem::CleanUpErrorMaterial() {
  // Destruction of g_pErrorMaterial is deferred until after
  // CMaterialDict::Shutdown. The global g_pErrorMaterial is set to NULL so that
  // IsErrorMaterial() will return false and
...
CMaterialSystem::CMaterialSystem() {
  ...
  m_nServiceThread = 0;
  m_nRenderTargetFrameBufferHeightOverride =
      m_nRenderTargetFrameBufferWidthOverride = 0;
...
#if !defined(DEDICATED)
#if defined(USE_SDL)
  g_pLauncherMgr = (ILauncherMgr *)factory(
      "SDLMgrInterface001" /*SDL_MGR_INTERFACE_VERSION*/, NULL);
  if (!g_pLauncherMgr) {
    return false;
  }
#endif // USE_SDL
...
InitReturnVal_t CMaterialSystem::Init() {
  ...
  if (IsX360()) {
    g_pQueuedLoader->InstallLoader(RESOURCEPRELOAD_MATERIAL,
                                   &s_ResourcePreloadMaterial);
    g_pQueuedLoader->InstallLoader(RESOURCEPRELOAD_CUBEMAP,
                                   &s_ResourcePreloadCubemap);
  }
...
void CMaterialSystem::ModShutdown() {
  ...
  // HACK - this is here to unhook ourselves from the client interface, since
  // we're not actually notified when it happens
  m_pMaterialProxyFactory = NULL;
}
...
//-----------------------------------------------------------------------------
IMaterialSystemHardwareConfig *
CMaterialSystem::GetHardwareConfig(const char *pVersion, int *returnCode) {
...
bool CMaterialSystem::IsRenderThreadSafe() {
  return (m_ThreadMode != MATERIAL_QUEUED_THREADED && ThreadInMainThread()) ||
         (m_ThreadMode == MATERIAL_QUEUED_THREADED &&
...
bool CMaterialSystem::AllowThreading(bool bAllow, int nServiceThread) {
  ...
  if (CommandLine()->ParmValue("-threads", 2) <
      2) // if -threads specified on command line to restrict all the pools then
         // obey and not turn on QMS
    bAllow = false;
...
//-----------------------------------------------------------------------------
IMatRenderContext *
CMaterialSystem::CreateRenderContext(MaterialContextType_t type) {
  switch (type) {
...
//-----------------------------------------------------------------------------
IMatRenderContext *
CMaterialSystem::SetRenderContext(IMatRenderContext *pNewContext) {
...
static void ConvertModeStruct(MaterialVideoMode_t *pMode,
  ...
  pMode->m_RefreshRate =
      info.m_nRefreshRateDenominator
...
void CMaterialSystem::ForceSingleThreaded() {
  ...
  if (GetThreadMode() != MATERIAL_SINGLE_THREADED) {
    ...
    m_HardwareRenderContext.InitializeFrom(
    ...
    // NOTE: Must happen after EndQueue or proxies get bound again, which is
    // bad.
...
bool CMaterialSystem::SetMode(void *hwnd,
  ...
  if (config.m_nVRModeAdapter != -1 &&
      config.m_nVRModeAdapter < GetDisplayAdapterCount() &&
      !bPreviouslyUsingGraphics) {
...
//-----------------------------------------------------------------------------
ImageFormatInfo_t const &
CMaterialSystem::ImageFormatInfo(ImageFormat fmt) const {
...
static void ReadFlt(KeyValues *pGroup, const char *pName, float flDefaultVal,
                    float flUndefinedVal, float *pDest) {
  *pDest = pGroup->GetFloat(pName, flDefaultVal);
  //	Warning( "\t%s = %f\n", pName, *pDest );
  Assert(*pDest != flUndefinedVal);
}

static void LoadFlags(KeyValues *pGroup, const char *pName, bool bDefaultValue,
                      unsigned int nFlag, unsigned int *pFlags) {
...
bool BetterResolution(int nRecommendedNumPixels, int nBestNumPixels,
                      int nNewNumPixels) {
...
void CMaterialSystem::GenerateConfigFromConfigKeyValues(
  ...
  pConfig->dxSupportLevel =
      MAX(ABSOLUTE_MINIMUM_DXLEVEL, adapterInfo.m_nDXSupportLevel);
  ...
  // Get the recommended resolution from dxsupport.cfg, this assumes a 4:3
  // aspect ratio
  ...
  int nCurrentScreenAspect =
      GetScreenAspectMode(displayMode.m_nWidth, displayMode.m_nHeight);
  ...
  // Pick the resolution with the right aspect ratio which matches the
  // recommended resolution most closely
  for (int i = 0; i < nNumVideoModes; i++) {
    g_pShaderDeviceMgr->GetModeInfo(&displayMode, 0, i);

    if (nCurrentScreenAspect ==
        GetScreenAspectMode(displayMode.m_nWidth, displayMode.m_nHeight)) {
      int nNumPixels = displayMode.m_nWidth * displayMode.m_nHeight;

      // Initially select the first mode we find of the correct aspect ratio for
      // the display
      if ((nBestMode == -1) ||
          BetterResolution(nRecommendedPixels, nBestPixels, nNumPixels)) {
        nBestMode = i;
...
//-----------------------------------------------------------------------------
static ConVar mat_vsync("mat_vsync", "0", FCVAR_ALLOWED_IN_COMPETITIVE,
                        "Force sync to vertical retrace", true, 0.0, true, 1.0);
...
#ifdef _X360 // The code that reads this out of moddefaults.txt is #if'd out for
             // the 360, so force aniso to 2 here.
static ConVar
    mat_forceaniso("mat_forceaniso", "2",
                   FCVAR_ARCHIVE); // 0 = Bilinear, 1 = Trilinear, 2+ = Aniso
#elif defined(OSX)
static ConVar mat_forceaniso("mat_forceaniso", "1", FCVAR_ARCHIVE,
                             "Filtering level", true, 0, true,
                             8); // 0 = Bilinear, 1 = Trilinear, 2+ = Aniso
#else
static ConVar
    mat_forceaniso("mat_forceaniso", "1",
                   FCVAR_ARCHIVE); // 0 = Bilinear, 1 = Trilinear, 2+ = Aniso
#endif
...
// Debugging textures
static ConVar
    mat_showmiplevels("mat_showmiplevels", "0", FCVAR_CHEAT,
...
static ConVar mat_picmip("mat_picmip", "0", FCVAR_ARCHIVE, "", true, -32, true,
                         8);
...
static ConVar mat_depthbias_normal("mat_depthbias_normal", "0.0f",
                                   FCVAR_CHEAT | FCVAR_ALLOWED_IN_COMPETITIVE);
...
static ConVar
    mat_slopescaledepthbias_shadowmap("mat_slopescaledepthbias_shadowmap", "16",
                                      FCVAR_CHEAT);
static ConVar mat_depthbias_shadowmap("mat_depthbias_shadowmap", "0.0005",
                                      FCVAR_CHEAT);

static ConVar
    mat_monitorgamma("mat_monitorgamma", "2.2", FCVAR_ARCHIVE,
...
static ConVar mat_monitorgamma_tv_exp("mat_monitorgamma_tv_exp", "2.5", 0, "",
                                      true, 1.0f, true, 4.0f);
#ifdef _X360
static ConVar mat_monitorgamma_tv_enabled("mat_monitorgamma_tv_enabled", "1",
                                          FCVAR_ARCHIVE, "");
#else
...
static ConVar
    mat_fastspecular("mat_fastspecular", "1", 0,
                     "Enable/Disable specularity for visual testing.  Will not "
                     "reload materials and will not affect perf.");
...
static ConVar r_waterforcereflectentities("r_waterforcereflectentities", "0",
                                          FCVAR_ALLOWED_IN_COMPETITIVE);
static ConVar mat_motion_blur_enabled("mat_motion_blur_enabled", "0",
                                      FCVAR_ARCHIVE);
...
#ifdef DX_TO_GL_ABSTRACTION
  pConfig->bBufferPrimitives =
      false; // nillerusr: causes rendering bugs and sefaults with nvidia driver
#else
...
bool CMaterialSystem::OverrideConfig(const MaterialSystem_Config_t &_config,
  ...
  if (config.dxSupportLevel != g_config.dxSupportLevel) {
    if (mat_debugalttab.GetBool()) {
      Warning("mat_debugalttab: Setting dxSupportLevelChanged, "
              "bResetAnisotropy, and bReloadMaterials because new dxlevel = %d "
              "and old dxlevel = %d\n",
  ...
  if (config.ShadowDepthTexture() != g_config.ShadowDepthTexture()) {
    if (mat_debugalttab.GetBool()) {
      Warning("mat_debugalttab: Setting forceUpdate, bReloadMaterials and "
  ...
  if (config.VRMode() != g_config.VRMode() ||
      config.m_nVRModeAdapter != g_config.m_nVRModeAdapter) {
  ...
  if (forceUpdate) {
    if (mat_debugalttab.GetBool()) {
      Warning("mat_debugalttab: forceUpdate is true, therefore setting "
  ...
  if (config.UseBumpmapping() != g_config.UseBumpmapping() ||
      config.UsePhong() != g_config.UsePhong()) {
    if (mat_debugalttab.GetBool()) {
      Warning("mat_debugalttab: forceUpdate is true, therefore setting "
              "recomputeSnapshots, bRedownloadLightmaps, bRedownloadTextures, "
              "bResetAnisotropy, and bSetStandardVertexShaderConstants\n");
  ...
  if (config.UseParallaxMapping() != g_config.UseParallaxMapping()) {
    if (mat_debugalttab.GetBool()) {
      Warning("mat_debugalttab: new UseParallaxMapping=%d, old "
              "UseParallaxMapping=%d, setting bReloadMaterials\n",
              (int)config.UseParallaxMapping(),
              (int)g_config.UseParallaxMapping());
    }
    bReloadMaterials = true;
  }
  ...
  if (config.ReduceFillrate() != g_config.ReduceFillrate()) {
    if (mat_debugalttab.GetBool()) {
      Warning("mat_debugalttab: new ReduceFillrate=%d, old ReduceFillrate=%d, "
  ...
  if (config.bFilterLightmaps != g_config.bFilterLightmaps) {
    if (mat_debugalttab.GetBool()) {
      Warning("mat_debugalttab: new bFilterLightmaps=%d, old "
              "bFilterLightmaps=%d, setting EnableLightmapFiltering\n",
  ...
  if (config.bSoftwareLighting != g_config.bSoftwareLighting) {
    if (mat_debugalttab.GetBool()) {
      Warning("mat_debugalttab: new bSoftwareLighting=%d, old "
  ...
  if (config_internal.r_waterforceexpensive !=
    ...
    if (mat_debugalttab.GetBool()) {
      Warning("mat_debugalttab: new r_waterforceexpensive=%d, old "
  ...
  if (config.bAllowCheats != g_config.bAllowCheats) {
    if (mat_debugalttab.GetBool()) {
      Warning("mat_debugalttab: new bAllowCheats=%d, old bAllowCheats=%d, "
              "setting bRedownloadLightmaps\n",
  ...
  if (config.bAllowCheats != g_config.bAllowCheats ||
    ...
    if (mat_debugalttab.GetBool()) {
      Warning("mat_debugalttab: setting bRedownloadTextures, "
              "recomputeSnapshots, and bResetAnisotropy\n");
    }
    bRedownloadTextures = true;
    recomputeSnapshots = true;
    bResetAnisotropy = true;
  ...
  if (config.ForceTrilinear() != g_config.ForceTrilinear()) {
    if (mat_debugalttab.GetBool()) {
      Warning("mat_debugalttab: new forcetrilinear: %d, old forcetrilinear: "
              "%d, setting bResetTextureFilter\n",
  ...
  if (config.m_nForceAnisotropicLevel != g_config.m_nForceAnisotropicLevel) {
    if (mat_debugalttab.GetBool()) {
      Warning("mat_debugalttab: new m_nForceAnisotropicLevel: %d, old "
  ...
  if (config.m_fMonitorGamma != g_config.m_fMonitorGamma ||
    ...
    if (mat_debugalttab.GetBool()) {
      Warning("mat_debugalttab: new monitorgamma: %f, old monitorgamma: %f, "
  ...
  // In GL, we just check this and it's just a function call--no need for device
  // shenanigans.
  ...
  if ((IsX360() || !config.Windowed()) &&
    ...
    {
      if (mat_debugalttab.GetBool()) {
        Warning("mat_debugalttab: video mode changed due to toggle of wait for "
                "vsync\n");
      }
      bVideoModeChange = true;
    }
...
CMaterialSystem::GetShaderAPITextureBindHandle(ITexture *pTexture, int nFrame,
                                               int nTextureChannel) {
  return ShaderSystem()->GetShaderAPITextureBindHandle(pTexture, nFrame,
                                                       nTextureChannel);
}
...
//-----------------------------------------------------------------------------
ITexture *
CMaterialSystem::CreateProceduralTexture(const char *pTextureName,
...
//-----------------------------------------------------------------------------
IMaterial *
CMaterialSystem::FindProceduralMaterial(const char *pMaterialName,
...
#ifdef POSIX
  // strip extensions needs correct slashing for the OS, so fix it up early for
  // Posix
...
IMaterial *CMaterialSystem::FindMaterial(char const *pMaterialName,
  ...
  return FindMaterialEx(pMaterialName, pTextureGroupName,
                        MATERIAL_FINDCONTEXT_NONE, bComplain, pComplainPrefix);
}

//-----------------------------------------------------------------------------
// Search by name
//-----------------------------------------------------------------------------
IMaterial *CMaterialSystem::FindMaterialEx(char const *pMaterialName,
                                           const char *pTextureGroupName,
                                           int nContext, bool bComplain,
                                           const char *pComplainPrefix) {
  // We need lower-case symbols for this to work
  int nLen = Q_strlen(pMaterialName) + 1;
  char *pFixedNameTemp = (char *)stackalloc(nLen);
...
ITexture *CMaterialSystem::FindTexture(char const *pTextureName,
  ...
  if (m_hAsyncLoadFileCache &&
      !TextureManager()->IsTextureLoaded(pTextureName)) {
    bool bIsUNCName = (pTextureName[0] == '/' && pTextureName[1] == '/' &&
                       pTextureName[2] != '/');
    if (!bIsUNCName) {
      ...
      const char *pbuf = buf;
      g_pFullFileSystem->AddFilesToFileCache(m_hAsyncLoadFileCache, &pbuf, 1,
                                             pPathID);
      return TextureManager()->ErrorTexture();
    }
  }

  ITextureInternal *pTexture = TextureManager()->FindOrLoadTexture(
      pTextureName, pTextureGroupName, nAdditionalCreationFlags);
  Assert(pTexture);
  if (pTexture->IsError()) {
    if (IsPC()) {
      for (int i = 0; i < NELEMS(TextureAliases); i += 2) {
        if (!Q_stricmp(pTextureName, TextureAliases[i])) {
          return FindTexture(TextureAliases[i + 1], pTextureGroupName,
                             bComplain, nAdditionalCreationFlags);
        }
      }
    }
    if (bComplain) {
...
void CMaterialSystem::UncacheUnusedMaterials(bool bRecomputeStateSnapshots) {
  ...
  if (IsX360() && bRecomputeStateSnapshots) {
    // Always recompute snapshots because the queued loading process skips it
    // during pre-purge, allowing it to happen just once, here.
    bDidUncacheMaterial = true;
  }
...
void CMaterialSystem::AllocateStandardTextures() {
...
#if !defined(_X360)
  m_BlackTextureHandle = g_pShaderAPI->CreateTexture(
      1, 1, 1, IMAGE_FORMAT_BGRX8888, 1, 1, tcFlagsSRGB, "[BLACK_TEXID]",
      TEXTURE_GROUP_OTHER);
...
#if !defined(_X360)
  m_WhiteTextureHandle = g_pShaderAPI->CreateTexture(
      1, 1, 1, IMAGE_FORMAT_BGRX8888, 1, 1, tcFlagsSRGB, "[WHITE_TEXID]",
      TEXTURE_GROUP_OTHER);
  ...
  // mat_fullbright 2)
#if !defined(_X360)
  m_GreyTextureHandle = g_pShaderAPI->CreateTexture(
      1, 1, 1, IMAGE_FORMAT_BGRX8888, 1, 1, tcFlagsSRGB, "[GREY_TEXID]",
      TEXTURE_GROUP_OTHER);
  ...
  // mat_fullbright 2)
#if !defined(_X360)
  m_GreyAlphaZeroTextureHandle = g_pShaderAPI->CreateTexture(
      1, 1, 1, IMAGE_FORMAT_RGBA8888, 1, 1, tcFlagsSRGB,
...

  {
    ...
    // generate the linear->gamma conversion table texture.
    {
      ...
      for (int i = 0; i != LINEAR_TO_GAMMA_TABLE_WIDTH; ++i) {
        ...
        fLookupResult = g_pShaderAPI->LinearToGamma_HardwareSpecific(
            fLookupResult); // that's right, linear->gamma->gamma2x so that that
                            // gamma->linear srgb read still ends up in gamma
    ...
    // generate the identity conversion table texture.
    {
      ...
      float
          pixelData[LINEAR_TO_GAMMA_IDENTITY_TABLE_WIDTH]; // sometimes used as
                                                           // float, sometimes
                                                           // as uint8,
                                                           // sizeof(float) >
                                                           // sizeof(uint8)
...
void CMaterialSystem::BeginFrame(float frameTime) {
  ...
  if (g_config.ForceHWSync() &&
      (IsPC() || m_ThreadMode != MATERIAL_QUEUED_THREADED)) {
...
ConVar mat_queue_mode("mat_queue_mode", "-1", FCVAR_ARCHIVE,
                      ...
                      "-1=default, 0=synchronous single thread"
#ifdef MAT_QUEUE_MODE_PROFILE
                      ", 1=queued single thread"
#endif
                      ", 2=queued multithreaded");

ConVar mat_queue_report("mat_queue_report", "0", FCVAR_ARCHIVE,
                        "Report thread stalls.  Positive number will filter by "
                        "stalls >= time in ms.  -1 reports all locks.");
...
IThreadPool *CMaterialSystem::CreateMatQueueThreadPool() {
  if (IsX360()) {
    ...
    // The rendering thread has the GL context and the main thread is coming in
    // and
    //  "helping" finish jobs - that breaks OpenGL, which requires TLS. This
    //  flag states
...
void CMaterialSystem::EndFrame(void) {
  ...
  while (!m_scheduledComposites.IsEmpty()) {
    // We hold a ref, so if there's only one count left, it's us. Let it go and
    // move on.
  ...
  FOR_EACH_VEC(m_pendingComposites, i) {
    ...
    if (comp->GetResolveStatus() == ECRS_Complete ||
        comp->GetResolveStatus() == ECRS_Error) {
      comp->Release();
      m_pendingComposites.Remove(i);

      // Stop after the first one reports that it was completed, these can take
      // awhile and we don't want to hammer anyone's framerate.
      break;
    }
  }
  ...
  // For this testing release, -2 is equivalent to 0 (off). When we release,
  // we'll make -2 equivalent to -1 (on)
  ...
  MaterialThreadMode_t nextThreadMode =
      (iConVarThreadMode >= 0) ? (MaterialThreadMode_t)iConVarThreadMode
                               : m_IdealThreadMode;
  // note: This is a hack because there is no explicit query for the device
  // being deactivated due to device lost. however, that is all the current
  // implementation of CanDownloadTextures actually does.
  bool bDeviceReady = g_pShaderAPI->CanDownloadTextures();
  ...
  case MATERIAL_QUEUED_THREADED: {
    ...
    m_pActiveAsyncJob = new CFunctorJob(
        CreateFunctor(this, &CMaterialSystem::ThreadExecuteQueuedContext,
                      pPrevContext),
  ...
  if (m_ThreadMode != nextThreadMode) {
    ...
    case MATERIAL_QUEUED_THREADED: {
      ...
      // probably have a queued context set here, need hardware to flush the
      // queue if the job isn't active
      m_HardwareRenderContext.InitializeFrom(
          &m_QueuedRenderContexts[m_iCurQueuedContext]);
      m_pRenderContext.Set(&m_HardwareRenderContext);
    ...
    case MATERIAL_QUEUED_SINGLE_THREADED: {
      g_pShaderAPI->SetDisallowAccess(false);
      // We have a queued context set here, need hardware to flush the queue if
    ...
    g_pShaderAPI->EnableShaderShaderMutex(
        m_ThreadMode !=
        MATERIAL_SINGLE_THREADED); // use mutex even for queued to allow
                                   // "disalow access" to function properly
...
// the default if not)
const MaterialSystem_Config_t &
CMaterialSystem::GetCurrentConfigForVideoCard() const {
  Assert(m_bGeneratedConfig);
  return g_config;
}
...
ITexture *CMaterialSystem::CreateNamedRenderTargetTextureEx(
  ...
  bool gl_canMixTargetSizes =
      (HardwareConfig() && HardwareConfig()->SupportsGLMixedSizeTargets());

  // On GL, the depth buffer for a render target must be the same size (until we
  // pick up mixed-sized attachments in 10.6.3)
  if ((!gl_canMixTargetSizes && IsPosix()) || IsEmulatingGL()) {
    if (depth != MATERIAL_RT_DEPTH_SEPARATE &&
...
public:
  CTextureBitsRegenerator(int w, int h, int mips, ImageFormat fmt,
    ...
    Assert(ImageLoader::GetMemRequired(
               w, h, 1, fmt, m_nMipmaps > 1 ? true : false) <= srcBufferSize);
    Assert(m_nMipmaps == 1 || m_nMipmaps == ImageLoader::GetNumMipMapLevels(
  ...
  virtual void RegenerateTextureBits(ITexture *pTexture,
    ...
    pVTFTexture->ComputeMipLevelDimensions(0, &destWidth, &destHeight,
                                           &destDepth);
    Assert(destDepth == 1);
    ...
    if (destFmt == m_ImageFormat && destWidth == m_nWidth &&
      ...
      while (mip < m_nMipmaps &&
             (srcResX > destWidth || srcResY > destHeight)) {
        srcOffset += ImageLoader::GetMemRequired(srcResX, srcResY, 1,
                                                 m_ImageFormat, false);
      ...
      for (; mip < m_nMipmaps; ++mip) {
        ...
        ImageLoader::ConvertImageFormat(m_ImageData.Base() + srcOffset,
                                        m_ImageFormat, pDest + dstOffset,
        ...
        srcOffset += ImageLoader::GetMemRequired(srcResX, srcResY, 1,
                                                 m_ImageFormat, false);
        dstOffset +=
            ImageLoader::GetMemRequired(srcResX, srcResY, 1, destFmt, false);
...
ITexture *CMaterialSystem::CreateTextureFromBits(int w, int h, int mips,
                                                 ImageFormat fmt,
                                                 int srcBufferSize,
                                                 byte *srcBits) {
  ...
  return CreateNamedTextureFromBitsEx("frombits", TEXTURE_GROUP_OTHER, w, h,
                                      mips, fmt, srcBufferSize, srcBits, flags);
}
...
ITextureCompositor *CMaterialSystem::NewTextureCompositor(
    int w, int h, const char *pCompositeName, int nTeamNum, uint64 randomSeed,
    KeyValues *stageDesc, uint32 texCompositeCreateFlags) {
  return CreateTextureCompositor(w, h, pCompositeName, nTeamNum, randomSeed,
                                 stageDesc, texCompositeCreateFlags);
}

void CMaterialSystem::ScheduleTextureComposite(
    CTextureCompositor *_texCompositor) {
  Assert(_texCompositor != NULL);
  _texCompositor->AddRef();
  m_scheduledComposites.AddToTail(_texCompositor);
}

void CMaterialSystem::AsyncFindTexture(
  ...
  // Bump the ref count on the recipient before handing it off. This ensures the
  // receiver won't go away before we have completed our work.
  pRecipient->AddRef();

  TextureManager()->AsyncFindOrLoadTexture(pFilename, pTextureGroupName,
                                           pRecipient, pExtraArgs, bComplain,
                                           nAdditionalCreationFlags);
}

// creates a texture suitable for use with materials from a raw stream of bits.
// The bits will be retained by the material system and can be freed upon
// return.
ITexture *CMaterialSystem::CreateNamedTextureFromBitsEx(
    const char *pName, const char *pTextureGroupName, int w, int h, int mips,
    ImageFormat fmt, int srcBufferSize, byte *srcBits, int nFlags) {
  Assert(srcBits);

  CTextureBitsRegenerator *regen =
      new CTextureBitsRegenerator(w, h, mips, fmt, srcBufferSize, srcBits);
  ITextureInternal *tex = TextureManager()->CreateProceduralTexture(
      pName, pTextureGroupName, w, h, 1, fmt, nFlags, regen);
  return tex;
}

bool CMaterialSystem::AddTextureCompositorTemplate(
    const char *pName, KeyValues *pTmplDesc,
...
void CMaterialSystem::SetRenderTargetFrameBufferSizeOverrides(int nWidth,
                                                              int nHeight) {
  m_nRenderTargetFrameBufferWidthOverride = nWidth;
  m_nRenderTargetFrameBufferHeightOverride = nHeight;
}

void CMaterialSystem::GetRenderTargetFrameBufferDimensions(int &nWidth,
                                                           int &nHeight) {
  if (m_nRenderTargetFrameBufferHeightOverride &&
...
//-----------------------------------------------------------------------------------------------------
#if defined(_X360)
HXUIFONT CMaterialSystem::OpenTrueTypeFont(const char *pFontname, int tall,
                                           int style) {
  MaterialLock_t hLock = Lock();
...
bool CMaterialSystem::GetTrueTypeFontMetrics(HXUIFONT hFont,
                                             XUIFontMetrics *pFontMetrics,
                                             XUICharMetrics charMetrics[256]) {
  MaterialLock_t hLock = Lock();
  bool result =
      g_pShaderAPI->GetTrueTypeFontMetrics(hFont, pFontMetrics, charMetrics);
  Unlock(hLock);
  return result;
}
bool CMaterialSystem::GetTrueTypeGlyphs(HXUIFONT hFont, int numChars,
                                        wchar_t *pWch, int *pOffsetX,
                                        int *pOffsetY, int *pWidth,
                                        int *pHeight, unsigned char *pRGBA,
                                        int *pRGBAOffset) {
  MaterialLock_t hLock = Lock();
  bool result =
      g_pShaderAPI->GetTrueTypeGlyphs(hFont, numChars, pWch, pOffsetX, pOffsetY,
                                      pWidth, pHeight, pRGBA, pRGBAOffset);
  Unlock(hLock);
  return result;
}
#endif
...
//-----------------------------------------------------------------------------------------------------
#if defined(_X360)
void CMaterialSystem::ReadBackBuffer(Rect_t *pSrcRect, Rect_t *pDstRect,
                                     unsigned char *pDstData,
                                     ImageFormat dstFormat, int dstStride) {
  ...
  ITexture *pTempRT;
  bool bStretch = (pSrcRect->width != pDstRect->width ||
                   pSrcRect->height != pDstRect->height);
  if (!bStretch) {
    // hijack an unused RT (no surface required) for 1:1 resolve work, fastest
    // path
    pTempRT = FindTexture("_rt_FullFrameFB", TEXTURE_GROUP_RENDER_TARGET);
  } else {
    // hijack an unused RT (with surface abilities) for stretch work, slower
    // path
  ...
  GetRenderContextInternal()->CopyRenderTargetToTextureEx(pTempRT, 0, pSrcRect,
                                                          &rect);

  // access the RT bits
  CPixelWriter writer;
  g_pShaderAPI->ModifyTexture(
      ((ITextureInternal *)pTempRT)->GetTextureHandle(0));
  if (!g_pShaderAPI->TexLock(0, 0, 0, 0, pTempRT->GetActualWidth(),
                             pTempRT->GetActualHeight(), writer))
    return;

  // this will be adequate for non-block formats
  int srcStride = pTempRT->GetActualWidth() *
                  ImageLoader::SizeInBytes(pTempRT->GetImageFormat());
  ...
  XGUntileTextureLevel(pTempRT->GetActualWidth(), pTempRT->GetActualHeight(), 0,
                       XGGetGpuFormat(ImageLoader::ImageFormatToD3DFormat(
                           pTempRT->GetImageFormat())),
                       0, (char *)writer.GetPixelMemory(), srcStride, NULL,
                       writer.GetPixelMemory(), NULL);

  // swap back to x86 order as expected by image conversion
  ImageLoader::ByteSwapImageData((unsigned char *)writer.GetPixelMemory(),
                                 srcStride * pTempRT->GetActualHeight(),
                                 pTempRT->GetImageFormat());

  // convert to callers format
  Assert(dstFormat == IMAGE_FORMAT_RGB888);
  ImageLoader::ConvertImageFormat((unsigned char *)writer.GetPixelMemory(),
                                  pTempRT->GetImageFormat(), pDstData,
                                  dstFormat, pDstRect->width, pDstRect->height,
                                  srcStride, dstStride);
...
#if defined(_X360)
void CMaterialSystem::PersistDisplay() { g_pShaderAPI->PersistDisplay(); }
#endif

#if defined(_X360)
void *CMaterialSystem::GetD3DDevice() { return g_pShaderAPI->GetD3DDevice(); }
#endif
...
void CMaterialSystem::ThreadRelease() {
  ...
  if (do_report) {
    ...
    if (do_report == -1 || flResult > mat_queue_report.GetFloat()) {
      ...
      ConColorMsg(red,
                  "CMaterialSystem::ThreadRelease: %0.2fms = Release:%0.2fms + "
                  "Acquire:%0.2fms\n",
                  flResult, (flEndThreadRelease - flStartTime) * 1000.0,
...
MaterialLock_t CMaterialSystem::Lock() {
  ...
  if (m_ThreadMode != MATERIAL_SINGLE_THREADED) {
    TelemetrySetLockName(TELEMETRY_LEVEL0, (char const *)&g_MatSysMutex,
                         "MatSysMutex");

    tmTryLock(TELEMETRY_LEVEL0, (char const *)&g_MatSysMutex,
              "CMaterialSystem");
    g_MatSysMutex.Lock();
    tmEndTryLock(TELEMETRY_LEVEL0, (char const *)&g_MatSysMutex, TMLR_SUCCESS);
    tmSetLockState(TELEMETRY_LEVEL0, (char const *)&g_MatSysMutex, TMLS_LOCKED,
                   "CMaterialSystem");
  }
#endif
...
void CReplacementProxy::Release() {
  ...
  // Since we have a material-holding-a-material situation here, we need to nuke
  // these if unreferenced to prevent the engine needing to double-call
  // UncacheUnusedMaterials to actually get rid of all materials.
...
IMaterial *CReplacementProxy::GetMaterial() {
  ...
  bool bVisionOverride =
      (localplayer_visionflags.IsValid() &&
       (localplayer_visionflags.GetInt() & (0x01))); // Pyro-vision Goggles
...
EXPOSE_INTERFACE(CReplacementProxy, IMaterialProxy,
                 "replace_proxy" IMATERIAL_PROXY_INTERFACE_VERSION);
...
void CMaterialSystem::ScanDirForReplacements(const char *pszPathName) {
  ...
  const char *pFindFileName =
      g_pFullFileSystem->FindFirst(szBaseName, &FindHandle);
...
void CMaterialSystem::InitReplacementsFromFile(const char *pszPathName) {
  ...
  for (int i = 0; i < replacementCount; ++i) {
    V_snprintf(szBaseName, sizeof(szBaseName), "%s/%s/replacements.vmt",
               pszPathName, replacementFiles[i]);
    if (g_pFullFileSystem->FileExists(szBaseName)) {
      KeyValues *pKV =
          g_pFullFileSystem->LoadKeyValues(IFileSystem::TYPE_VMT, szBaseName);
      if (pKV) {
...
IMaterialProxy *
CMaterialSystem::DetermineProxyReplacements(IMaterial *pMaterial,
  ...
  while (1) {
    ...
    if (m_Replacements.IsValidIndex(nIndex)) {
      ...
      if (!pTemplatesKV || !pPatternsKV) {
        ...
        for (KeyValues *pSubKey = pPatternsKV->GetFirstSubKey(); pSubKey;
             pSubKey = pSubKey->GetNextKey()) {
          const char *pszReplacementName = pSubKey->GetName();

          //					Msg( "  Sub: %s\n",
          //pSubKey->GetName() );
          if (strnicmp(pszFileName, pszReplacementName,
                       strlen(pszReplacementName)) ==
              0) { // We found a replacement!
              ...
              if (pTemplateKV) {
                ...
                if (pTemplateKV && pTemplateKV->GetFirstSubKey()) {
                  pReplacementMaterial =
                      pTemplateKV->GetFirstSubKey()->MakeCopy();
                }
              }
            } else {
            ...
            if (pReplacementMaterial->GetInt("$copyall") == 1) {
              for (KeyValues *pCopyKV = pFallbackKeyValues->GetFirstSubKey();
                   pCopyKV; pCopyKV = pCopyKV->GetNextKey()) {
                const char *pszCopyValue =
                    pReplacementMaterial->GetString(pCopyKV->GetName(), NULL);
                if (!pszCopyValue) {
                  pReplacementMaterial->SetString(pCopyKV->GetName(),
                                                  pCopyKV->GetString());
                }
              }
            } else {
              ...
              while (pszReplacementForceCopy[nReplaceIndex]) {
                const char *pszCopyValue = pFallbackKeyValues->GetString(
                    pszReplacementForceCopy[nReplaceIndex], NULL);
                if (pszCopyValue) {
                  pReplacementMaterial->SetString(
                      pszReplacementForceCopy[nReplaceIndex], pszCopyValue);
                }
                nReplaceIndex++;
              }
            }

            for (KeyValues *pSearchKV = pReplacementMaterial->GetFirstSubKey();
                 pSearchKV; pSearchKV = pSearchKV->GetNextKey()) {
              const char *pszValue = pSearchKV->GetString();
              if (pszValue[0] == '$') {
                const char *pszCopyValue =
                    pFallbackKeyValues->GetString(pszValue, NULL);
                if (pszCopyValue) {
...
void CMaterialSystem::OnRenderingAsyncComplete() {
  ...
  // Update the texture manager, which may cause some textures to become
  // available for compositing. Because updating textures may cause textures to
  // swap out their active texture handles, this can only be done while the
  // async job is not running.
