...
public:
  ...
  float
  ComputePixelWidthOfSphere(const Vector &origin,
                            float flRadius); // FIXME: REMOVE THIS FUNCTION!
...
protected:
  void OnAsyncCreateTextureFromRenderTarget(
      ITexture *pSrcRt, const char **pDstName,
      IAsyncTextureOperationReceiver *pRecipient);
  void OnAsyncMap(ITextureInternal *pTexToMap,
                  IAsyncTextureOperationReceiver *pRecipient, void *pExtraArgs);
  void OnAsyncUnmap(ITextureInternal *pTexToUnmap);
  void OnAsyncCopyRenderTargetToStagingTexture(
...
public:
  ...
  DELEGATE_TO_OBJECT_3V(ClearBuffersObeyStencilEx, bool, bool, bool,
                        g_pShaderAPI);
  ...
  void DrawClearBufferQuad(unsigned char r, unsigned char g, unsigned char b,
                           unsigned char a, bool bClearColor, bool bClearAlpha,
                           bool bClearDepth);
  ...
  void CopyTextureToRenderTargetEx(int nRenderTargetID, ITexture *pTexture,
                                   Rect_t *pSrcRect, Rect_t *pDstRect = NULL);
...
protected:
  ...
  CUtlVector<PlaneStackElement>
      m_CustomClipPlanes; // implemented as a vector so we can remove in special
                          // ways
...
inline void
CMatRenderContext::SetFlashlightState(const FlashlightState_t &state,
                                      const VMatrix &worldToTexture) {
  SetFlashlightStateEx(state, worldToTexture, NULL);
}

inline float
CMatRenderContextBase::ComputePixelWidthOfSphere(const Vector &vecOrigin,
                                                 float flRadius) {
...
//-----------------------------------------------------------------------------
inline IMesh *
CMatRenderContext::CreateStaticMesh(VertexFormat_t vertexFormat,
                                    const char *pTextureBudgetGroup,
                                    IMaterial *pMaterial) {
...
inline enum MaterialHeightClipMode_t
CMatRenderContextBase::GetHeightClipMode(void) {
