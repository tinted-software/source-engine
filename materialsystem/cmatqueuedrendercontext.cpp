...
void FastCopy(byte *pDest, const byte *pSrc, size_t nBytes) {
  ...
  if ((size_t)pDest % 16 == 0 && (size_t)pSrc % 16 == 0) {
    ...
    for (byte *pLimit = pDest + nBytesFull; pDest < pLimit;
...
public:
  CMatQueuedMesh(CMatQueuedRenderContext *pOwner,
      ...
      : m_pLateBoundMesh(&m_pActualMesh), m_pOwner(pOwner),
        m_pCallQueue(pOwner->GetCallQueueInternal()),
        m_pHardwareContext(pHardwareContext), m_pVertexData(NULL),
        m_pIndexData(NULL), m_nVerts(0), m_nIndices(0), m_VertexSize(0),
        m_Type(MATERIAL_TRIANGLES), m_pVertexOverride(NULL),
        m_pIndexOverride(NULL), m_pActualMesh(NULL),
        m_nActualVertexOffsetInBytes(0), m_VertexFormat(0), m_MorphFormat(0) {}
  ...
  void ExecuteDefferredBuild(byte *pVertexData, int nVerts, int nBytesVerts,
                             uint16 *pIndexData, int nIndices) {
...
#define FindMin(desc, pCurrent, tag)                                           \
  ((desc.m_VertexSize_##tag != 0) ? min(pCurrent, (void *)desc.m_p##tag)       \
                                  : pCurrent)
...
#undef FindMin
    ...
    if (pIndexData && pIndexData != &gm_ScratchIndexBuffer[0] &&
      ...
      if (!desc.m_nFirstVertex) {
        ...
        FastCopy((byte *)desc.m_pIndices, (byte *)pIndexData,
        ...
        while (i < nIndices) {
          ...
          FastCopy((byte *)(desc.m_pIndices + i), (byte *)tempIndices,
...
//
//-----------------------------------------------------------------------------
bool CMatQueuedRenderContext::Init(CMaterialSystem *pMaterialSystem,
  ...
  Assert(!"CMatQueuedRenderContext shouldn't be initialized on dedicated "
          "servers...");
...
void CMatQueuedRenderContext::BeginRender() {
  if (++m_iRenderDepth == 1) {
    VPROF_INCREMENT_GROUP_COUNTER("render/CMatQBeginRender",
...
void CMatQueuedRenderContext::CallQueued(bool bTermAfterCall) {
  if (mat_report_queue_status.GetBool()) {
    Msg("%d calls queued for %llu bytes in parameters and overhead, %d bytes "
        "verts, %d bytes indices, %d bytes other\n",
        m_queue.Count(), (uint64)(m_queue.GetMemoryUsed()),
        m_Vertices.GetUsed(), m_Indices.GetUsed(), RenderDataSizeUsed());
  }
...
//
//-----------------------------------------------------------------------------
void CMatQueuedRenderContext::CopyTextureToRenderTargetEx(int i,
                                                          ITexture *pTexture,
                                                          Rect_t *pSrc,
                                                          Rect_t *pDst) {
  m_queue.QueueCall(
      m_pHardwareContext, &IMatRenderContext::CopyTextureToRenderTargetEx, i,
      pTexture, CUtlEnvelope<Rect_t>(pSrc), CUtlEnvelope<Rect_t>(pDst));
}
...
void CMatQueuedRenderContext::DeferredBeginBatch(uint16 *pIndexData,
                                                 int nIndices) {
...
byte *CMatQueuedRenderContext::AllocVertices(int nVerts, int nVertexSize) {
  ...
  const byte *pNextAlloc = (const byte *)(m_Vertices.GetBase()) +
                           m_Vertices.GetUsed() +
                           AlignValue(nVerts * nVertexSize, 16);
  const byte *pCommitLimit =
      (const byte *)(m_Vertices.GetBase()) + m_Vertices.GetSize();
#endif
  ...
  if (!pResult) {
    ...
    // Print some information to the console so that it's picked up in the
    // minidump comment.
    Msg("AllocVertices( %d, %d ) on %p failed. m_Vertices is based at %p with "
        "a size of 0x%x.\n",
    ...
    if (pNextAlloc > pCommitLimit) {
      ...
      const void *pRet = VirtualAlloc((void *)pCommitLimit, commitSize,
                                      MEM_COMMIT, PAGE_READWRITE);
      if (!pRet)
        status = GetLastError();

      Msg("VirtualAlloc( %p, %d ) returned %p on repeat. VirtualAlloc %s with "
          "code %x.\n",
          pCommitLimit, commitSize, pRet,
          (pRet != NULL) ? "succeeded" : "failed", (uint32)status);
    } else {
...
uint16 *CMatQueuedRenderContext::AllocIndices(int nIndices) {
...
#if defined(_WIN32) && !defined(_X360)
  const byte *pNextAlloc = (const byte *)(m_Indices.GetBase()) +
                           m_Indices.GetUsed() +
                           AlignValue(nIndices * sizeof(uint16), 16);
  const byte *pCommitLimit =
      (const byte *)(m_Indices.GetBase()) + m_Indices.GetSize();
#endif
  ...
  if (!pResult) {
    ...
    // Print some information to the console so that it's picked up in the
    // minidump comment.
    Msg("AllocIndices( %d ) on %p failed. m_Indices is based at %p with a size "
        "of 0x%x.\n",
...
byte *CMatQueuedRenderContext::ReallocVertices(byte *pVerts, int nVertsOld,
                                               int nVertsNew, int nVertexSize) {
...
uint16 *CMatQueuedRenderContext::ReallocIndices(uint16 *pIndices,
                                                int nIndicesOld,
                                                int nIndicesNew) {
...
void CMatQueuedRenderContext::FreeVertices(byte *pVerts, int nVerts,
                                           int nVertexSize) {
...
// NOTE: These are synchronous calls!  The rendering thread is stopped, the
// current queue is drained and the pixels are read NOTE: We should also have a
// queued read pixels in the API for doing mid frame reads (as opposed to
// screenshots)
void CMatQueuedRenderContext::ReadPixels(int x, int y, int width, int height,
                                         unsigned char *data,
...
void CMatQueuedRenderContext::ReadPixelsAndStretch(Rect_t *pSrcRect,
                                                   Rect_t *pDstRect,
