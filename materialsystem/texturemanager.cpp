...
// removing leading and trailing whitespace, including newlines. Whitespace
...
void V_StripWhitespace(char *pBuffer) {
  ...
  while (*pSrc) {
    ...
    else if (!leading && !V_isspace(*pDst) &&
...
public:
  ...
  CreateProceduralTexture(const char *pTextureName,
  ...
  virtual ITextureInternal *
  CreateRenderTargetTexture(const char *pRTName, // NULL for auto-generated name
  ...
  virtual void AsyncFindOrLoadTexture(
  ...
  virtual void AsyncCreateTextureFromRenderTarget(
      ITexture *pSrcRt, const char *pDstName, ImageFormat dstFmt, bool bGenMips,
  ...
  virtual bool AddTextureCompositorTemplate(const char *pName,
                                            KeyValues *pTmplDesc) OVERRIDE;
  virtual bool VerifyTextureCompositorTemplates() OVERRIDE;

  virtual CTextureCompositorTemplate *
  FindTextureCompositorTemplate(const char *pName) OVERRIDE;
...
struct AsyncLoadJob_t {
  ...
  AsyncLoadJob_t()
      : m_pRecipient(NULL), m_pExtraArgs(NULL), m_bComplain(false),
        m_nAdditionalCreationFlags(0), m_pResultData(NULL) {}

  AsyncLoadJob_t(const char *pTextureName, const char *pTextureGroupName,
                 IAsyncTextureOperationReceiver *pRecipient, void *pExtraArgs,
                 bool bComplain, int nAdditionalCreationFlags)
      : m_TextureName(pTextureName), m_TextureGroupName(pTextureGroupName),
        m_pRecipient(pRecipient), m_pExtraArgs(pExtraArgs),
...
public:
  ...
  virtual void OnAsyncMapComplete(ITexture *pTex, void *pExtraArgs,
                                  void *pMemory, int nPitch) OVERRIDE {}
...
public:
  CAsyncMapResult(ITextureInternal *pTex)
      : m_pTexToMap(pTex), m_nReferenceCount(0), m_pMemory(NULL), m_nPitch(0),
        m_bSignalled(false) {}
...
struct AsyncReadJob_t {
  ...
  AsyncReadJob_t()
      : m_pSrcRt(NULL), m_pSysmemTex(NULL), m_pAsyncRead(NULL),
        m_pAsyncMap(NULL), m_pDstName(NULL), m_dstFmt(IMAGE_FORMAT_UNKNOWN),
        m_bGenMips(false), m_nAdditionalCreationFlags(0), m_pRecipient(NULL),
        m_pExtraArgs(NULL) {}

  AsyncReadJob_t(ITexture *pSrcRt, const char *pDstName, ImageFormat dstFmt,
                 bool bGenMips, int nAdditionalCreationFlags,
                 IAsyncTextureOperationReceiver *pRecipient, void *pExtraArgs)
      : m_pSrcRt(pSrcRt), m_pSysmemTex(NULL), m_pAsyncRead(NULL),
        m_pAsyncMap(NULL),
        m_pDstName(pDstName) // We take ownership of this string.
        ,
        m_dstFmt(dstFmt), m_bGenMips(bGenMips),
        m_nAdditionalCreationFlags(nAdditionalCreationFlags),
        m_pRecipient(pRecipient), m_pExtraArgs(pExtraArgs) {}
  ...
  ~AsyncReadJob_t() {
    ...
    if (m_pSysmemTex) {
      ...
      assert_cast<CTextureManager *>(g_pTextureManager)
          ->ReleaseReadbackTexture(m_pSysmemTex);
      m_pSysmemTex = NULL;
    }
...
bool IsJobCancelled(AsyncReadJob_t *pJob) {
  ...
  // The texture manager holds a reference to the object, so if we're the only
  // one who is holding a ref then the job has been abandoned. This gives us the
  // opportunity to cleanup and skip some work.
...
//-----------------------------------------------------------------------------
// Functions can be called from any thread, unless they are prefixed with a
// thread name.
class AsyncLoader {
public:
  ...
  void ThreadMain_Update() {
    ...
    if (m_completedJobs.PopItem(&pJob)) {
      ...
      tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s - CompleteAsyncLoad",
             __FUNCTION__);
      // Complete the load, then make the callback.
      assert_cast<CTextureManager *>(g_pTextureManager)
          ->CompleteAsyncLoad(pJob);
      delete pJob;
      pJob = NULL;
    }
  }
...
private:
  ...
  void ThreadLoader_ProcessLoad(AsyncLoadJob_t *pJob,
    ...
    if (!pJob->m_pResultData->AsyncReadTextureFromFile(
            pScratchVTF, pJob->m_nAdditionalCreationFlags))
      m_asyncScratchVTFs.PushItem(pScratchVTF);
...
public:
  ...
  void ThreadMain_Update() {
    ...
    while (!m_queuedMaps.IsEmpty()) {
      ...
      Assert(pMapped != NULL);
      {
        ...
        if (pMapped->m_pAsyncMap->IsSignalled()) {
          if (pMapped->m_pAsyncMap->m_pMemory != 0 &&
              pMapped->m_pAsyncMap->m_nPitch != 0) {
            // Stick it in the queue for the other thread to work on it.
            m_pendingJobs.PushItem(pMapped);
          } else {
            Assert(!"Failed to perform a map that shouldn't fail, need to deal "
                    "with this if it ever happens.");
            DevWarning("Failed to perform a map that shouldn't fail, need to "
                       "deal with this if it ever happens.");
          }
    ...
    // This is ugly, but basically we need to do map and unmap on the main
    // thread. Other stuff can (mostly) happen on the async thread
    while (!m_queuedReads.IsEmpty()) {
      ...
      if (m_queuedReads.RemoveAtHead(pRead)) {
        ...
        SafeAssign(&pRead->m_pAsyncMap,
                   new CAsyncMapResult(pRead->m_pSysmemTex));
        // Trigger the map.
        extern CMaterialSystem g_MaterialSystem;
        g_MaterialSystem.GetRenderContextInternal()->AsyncMap(
            pRead->m_pSysmemTex, pRead->m_pAsyncMap, NULL);
    ...
    if (m_requestedCopies.PopItem(&pRequestCopy)) {
      ...
      extern CMaterialSystem g_MaterialSystem;
      g_MaterialSystem.GetRenderContextInternal()
          ->AsyncCopyRenderTargetToStagingTexture(
              pRequestCopy->m_pSysmemTex, pRequestCopy->m_pSrcRt,
              pRequestCopy->m_pAsyncRead, NULL);
    ...
    while (m_completedJobs.Count() > 0) {
      ...
      if (m_completedJobs.PopItem(&pCreate)) {
        ...
        extern CMaterialSystem g_MaterialSystem;
        g_MaterialSystem.GetRenderContextInternal()->AsyncUnmap(
            pCreate->m_pSysmemTex);
        SafeRelease(&pCreate->m_pAsyncMap);

        assert_cast<CTextureManager *>(g_pTextureManager)
            ->CompleteAsyncRead(pCreate);
        delete pCreate;
        pCreate = NULL;
        // Stop as soon as we complete one successfully.
        break;
...
private:
  ...
  void ThreadReader_ProcessRead(AsyncReadJob_t *pJob) {
    ...
    // 1. Reads from a previously mapped scratch buffer texture and performs
    ...
    const int srcFinestMemRequired =
        ImageLoader::GetMemRequired(w, h, 1, srcFmt, false);
    const int srcAllMemRequired =
        ImageLoader::GetMemRequired(w, h, 1, srcFmt, pJob->m_bGenMips);
    ...
    const int dstMemRequried =
        ImageLoader::GetMemRequired(w, h, 1, dstFmt, pJob->m_bGenMips);
    ...
    // If this fires, you will get data corruption below. We can fix this case,
    // it just doesn't seem to be needed right now.
...

    {
      ...
      ImageLoader::ConvertImageFormat(
          (unsigned char *)pJob->m_pAsyncMap->m_pMemory,
          GetImageFormatRawReadback(srcFmt), srcBufferFinestMip.Base(), srcFmt,
          w, h);
    }

    if (pJob->m_bGenMips) {
      GenerateMipmaps(&srcBufferAllMips, srcBufferFinestMip.Base(), w, h,
                      srcFmt);
    } else {
      // If we're not generating mips, then allmips == finest mip, but the code
      // below expects everything to be in all mips.
    ...
    if (srcFmt != dstFmt) {
      ConvertTexelData(&dstBufferAllMips, dstFmt, srcBufferAllMips, w, h,
                       srcFmt, pJob->m_bGenMips);
  ...
  void GenerateMipmaps(CUtlMemory<unsigned char> *outBuffer,
                       unsigned char *pSrc, int w, int h,
  ...
  void ConvertTexelData(CUtlMemory<unsigned char> *outBuffer,
                        ImageFormat dstFmt,
                        /* const */ CUtlMemory<unsigned char> &inBuffer, int w,
                        int h, ImageFormat srcFmt, bool bGenMips) {
    tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s", __FUNCTION__);

    const int mipmapCount =
        bGenMips ? ImageLoader::GetNumMipMapLevels(w, h) : 1;
...
CTextureManager::CTextureManager(void)
    : m_TextureList(true), m_TextureAliases(true), m_TextureExcludes(true),
      ...
      m_nAsyncLoadThread(0xFFFFFFFF), m_nAsyncReadThread(0xFFFFFFFF) {
...
void CTextureManager::Init(int nFlags) {
  ...
  int nTextureFlags = TEXTUREFLAGS_ENVMAP | TEXTUREFLAGS_NOMIP |
                      TEXTUREFLAGS_NOLOD | TEXTUREFLAGS_SINGLECOPY |
                      TEXTUREFLAGS_CLAMPS | TEXTUREFLAGS_CLAMPT |
                      TEXTUREFLAGS_CLAMPU;

  if (HardwareConfig()->GetMaxDXSupportLevel() >= 80) {
    ImageFormat fmt =
        IsOpenGL() ? IMAGE_FORMAT_RGBA16161616F : IMAGE_FORMAT_BGRX8888;
  ...
  if (HardwareConfig()->GetMaxDXSupportLevel() >= 90) {
    ...
    ImageFormat fmt =
        IsOpenGL() ? IMAGE_FORMAT_RGBA16161616F : IMAGE_FORMAT_UVWQ8888;

#ifdef OSX
    // JasonM - ridiculous hack around R500 lameness...we never use this texture
    // on OSX anyways (right?) Now assuming this was an OSX specific workaround.
  ...
  m_pDebugLuxels2D =
      FindOrLoadTexture("debug/debugluxelsnoalpha", TEXTURE_GROUP_OTHER);
...
void CTextureManager::Shutdown() {
  ...
  // Cool the texture cache first to drop all the refs back to 0 for the
  // streamable things.
  CoolTextureCache();
...
void CTextureManager::CacheExternalStandardRenderTargets() {
  m_pFullScreenDepthTexture = FindTexture(
      "_rt_FullFrameDepth"); // created/destroyed in engine/matsys_interface.cpp
                             // to properly track hdr changes
}
...
void CTextureManager::CleanupPossiblyUnreferencedTextures() {
  if (!ThreadInMainThread() ||
      MaterialSystem()->GetRenderThreadId() != (uintp)-1) {
    Assert(!"CTextureManager::CleanupPossiblyUnreferencedTextures should never "
            "be called here");
  ...
  // It is perfectly valid for a texture to become referenced again (it lives on
  // in our texture list, and can be re-loaded) and then free'd again, so ensure
  // we don't have any duplicates in queue.
  CUtlVector<ITextureInternal *> texturesToDelete(
      /* growSize */ 0,
  ...
  while (m_PossiblyUnreferencedTextures.PopItem(&pMaybeUnreferenced)) {
    ...
    if (pMaybeUnreferenced->GetReferenceCount() == 0 &&
        texturesToDelete.Find(pMaybeUnreferenced) ==
            texturesToDelete.InvalidIndex()) {
...
ITextureInternal *CTextureManager::CreateProceduralTexture(
  ...
  ITextureInternal *pNewTexture = ITextureInternal::CreateProceduralTexture(
      pTextureName, pTextureGroupName, w, h, d, fmt, nFlags, generator);
  if (!pNewTexture)
    return NULL;
...
// Texture dictionary...
//-----------------------------------------------------------------------------
ITextureInternal *CTextureManager::LoadTexture(
    const char *pTextureName, const char *pTextureGroupName,
...
ITextureInternal *
CTextureManager::FindOrLoadTexture(const char *pTextureName,
                                   const char *pTextureGroupName,
                                   int nAdditionalCreationFlags /* = 0 */) {
  ITextureInternal *pTexture = FindTexture(pTextureName);
  if (!pTexture) {
    pTexture =
        LoadTexture(pTextureName, pTextureGroupName, nAdditionalCreationFlags);
    if (pTexture) {
...
void CTextureManager::ResumeTextureStreaming(void) {
  AssertMsg(m_iSuspendTextureStreaming,
            "Mismatched Suspend/Resume texture streaming calls");
...
void CTextureManager::MarkUnreferencedTextureForCleanup(
    ITextureInternal *pTexture) {
...
void CTextureManager::RemoveTexture(ITextureInternal *pTexture) {
  ...
  // If the queue'd rendering thread is running, RemoveTexture() is going to
  // explode. If it isn't, calling RemoveTexture while still dealing with
  // immediate removal textures seems fishy, but could be legit, in which case
  // this assert could be softened.
  int nUnreferencedQueue = m_PossiblyUnreferencedTextures.Count();
  if (nUnreferencedQueue) {
    Assert(!"RemoveTexture() being called while textures sitting in possibly "
            "unreferenced queue");
    // Assuming that this is all a wholesome main-thread misunderstanding, we
    // can try to continue after filtering this texture from the queue.
    ITextureInternal *pPossiblyUnreferenced = NULL;
    for (int i = 0;
         i < nUnreferencedQueue &&
         m_PossiblyUnreferencedTextures.PopItem(&pPossiblyUnreferenced);
         i++) {
  ...
  if (bTextureFound) {
    Assert(!"CTextureManager::RemoveTexture has been called for a texture that "
            "has already requested cleanup. That's a paddlin'.");
    // This is catastrophically bad, don't do this. Someone needs to fix this.
    DebuggerBreakIfDebugging_StagingOnly();
    return;
  }
  ...
  for (int i = m_TextureList.First(); i != m_TextureList.InvalidIndex();
    ...
    if (m_TextureList[i] == pTexture) {
      // This code is always sure that the texture we're tryign to clean up is
      // no longer in the the possibly unreferenced list, So let Destroy work
      // without checking.
      ITextureInternal::Destroy(m_TextureList[i], true);
      m_TextureList.RemoveAt(i);
      break;
...
// Load a texture asynchronously and then call the provided callback.
void CTextureManager::AsyncFindOrLoadTexture(
    const char *pTextureName, const char *pTextureGroupName,
    IAsyncTextureOperationReceiver *pRecipient, void *pExtraArgs,
    bool bComplain, int nAdditionalCreationFlags) {
  tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s", __FUNCTION__);
  ...
  bool bStreamingRequest =
      (nAdditionalCreationFlags & TEXTUREFLAGS_STREAMABLE) != 0;

  ITextureInternal *pLoadedTex = FindTexture(pTextureName);
  // It'd be weird to indicate that we're streaming and not actually have a
  ...
  AsyncLoadJob_t asyncLoad(pTextureName, pTextureGroupName, pRecipient,
                           pExtraArgs, bComplain, nAdditionalCreationFlags);

  // If this is the first person asking to load this, then remember so we don't
  // load the same thing over and over again.
  int pendingIndex = m_PendingAsyncLoads.Find(pTextureName);
  if (pendingIndex == m_PendingAsyncLoads.InvalidIndex()) {
    // Create the texture here, we'll load the data in the async thread. Load is
    // a misnomer, because it doesn't actually load the data--Download does.
    if (bStreamingRequest)
      asyncLoad.m_pResultData = pLoadedTex;
    else
      asyncLoad.m_pResultData = LoadTexture(pTextureName, pTextureGroupName,
                                            nAdditionalCreationFlags, false);
    AsyncLoad(asyncLoad);
...
void CTextureManager::CompleteAsyncLoad(AsyncLoadJob_t *pJob) {
  ...
  pJob->m_pRecipient->OnAsyncFindComplete(pJob->m_pResultData,
                                          pJob->m_pExtraArgs);

  // Finally, deal with any other stragglers that asked for the same surface we
  // did.
  ...
  FOR_EACH_VEC(m_PendingAsyncLoads[pendingIndex], i) {
    ...
    if (!bDownloaded && !IsJobCancelled(&straggler)) {
      bDownloaded = true;
      straggler.m_pResultData->Download(NULL,
                                        straggler.m_nAdditionalCreationFlags);
    }

    straggler.m_pRecipient->OnAsyncFindComplete(straggler.m_pResultData,
                                                straggler.m_pExtraArgs);
  ...
  if (bDownloaded) {
    ...
    // It's possible that the texture wasn't actually unloaded, so we may have
    // reloaded something unnecessarily. If so, just don't re-add it.
    if (m_TextureList.Find(pJob->m_pResultData->GetName()) ==
        m_TextureList.InvalidIndex())
      m_TextureList.Insert(pJob->m_pResultData->GetName(), pJob->m_pResultData);

    materials->Unlock(hMaterialLock);
  } else {
    // If we didn't download, need to clean up the leftover file data that we
    // loaded on the other thread
    pJob->m_pResultData->AsyncCancelReadTexture();
  }

  // Can't release the Recipient until after we tell the stragglers, because the
  // recipient may be the only ref to the texture, and cleaning it up may clean
  // up the texture but leave us with a seemingly valid pointer.
...
void CTextureManager::AsyncCreateTextureFromRenderTarget(
    ITexture *pSrcRt, const char *pDstName, ImageFormat dstFmt, bool bGenMips,
    int nAdditionalCreationFlags, IAsyncTextureOperationReceiver *pRecipient,
    void *pExtraArgs) {
  tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s", __FUNCTION__);
  ...
  AsyncReadJob_t *pAsyncRead =
      new AsyncReadJob_t(pSrcRt, pDstName, dstFmt, bGenMips,
                         nAdditionalCreationFlags, pRecipient, pExtraArgs);
...
void CTextureManager::CompleteAsyncRead(AsyncReadJob_t *pJob) {
  ...
  int nFlags = pJob->m_nAdditionalCreationFlags | TEXTUREFLAGS_SINGLECOPY |
  ...
  ITexture *pFinalTex = materials->CreateNamedTextureFromBitsEx(
      pJob->m_pDstName, TEXTURE_GROUP_RUNTIME_COMPOSITE, w, h, mips,
      pJob->m_dstFmt, pJob->m_finalTexelData.Count(),
...
void CTextureManager::AsyncReadTexture(AsyncReadJob_t *pJob) {
  ...
  pJob->m_pSysmemTex = AcquireReadbackTexture(pJob->m_pSrcRt->GetActualWidth(),
                                              pJob->m_pSrcRt->GetActualHeight(),
...
ITextureInternal *CTextureManager::AcquireReadbackTexture(int w, int h,
                                                          ImageFormat fmt) {
  tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s", __FUNCTION__);

  {
    ...
    FOR_EACH_VEC(m_ReadbackTextures, i) {
      ...
      if (pTex->GetActualWidth() == w && pTex->GetActualHeight() == h &&
  ...
  ITextureInternal *stagingTex = CreateProceduralTexture(
      ...
      TEXTUREFLAGS_STAGING_MEMORY | TEXTUREFLAGS_NOMIP |
          TEXTUREFLAGS_SINGLECOPY | TEXTUREFLAGS_IMMEDIATE_CLEANUP);
...
#ifdef STAGING_ONLY
static int SortTexturesForDump(const CUtlPair<CUtlString, void *> *sz1,
                               const CUtlPair<CUtlString, void *> *sz2) {
...
void CTextureManager::DumpTextureList() {
  ...
  FOR_EACH_DICT(m_TextureList, i) {
    textures.AddToTail(MakeUtlPair(CUtlString(m_TextureList[i]->GetName()),
                                   (void *)m_TextureList[i]));
  }
  materials->Unlock(hMaterialLock);
...
void CTextureManager::WarmTextureCache() {
  ...
  // If this fires, we need to relocate this elsewhere--there's no point in
  // doing the loading if we're not going to be able to download them right now.
  Assert(g_pShaderAPI->CanDownloadTextures());

  g_pFullFileSystem->AddSearchPath("tf2_texture_cache.vpk",
                                   cTextureCachePathDir, PATH_ADD_TO_TAIL);

  CUtlDict<int> filesToLoad(k_eDictCompareTypeCaseSensitive);

  // TODO: Maybe work directly with VPK (still need to add to the filesystem for
  // LoadTexture)? CPackFile

  // Add the pak and then walk through the contents.
  FindFilesToLoad(&filesToLoad, "*.*");

  // Then add the list of files from the cache, which will deal with running
  // without a VPK and also allow us to add late stragglers.
  ...
  FOR_EACH_DICT(filesToLoad, i) {
    ...
    // Load the texture. This will only load the lower mipmap levels because
    // that's the file we'll find now.
    ITextureInternal *pTex = LoadTexture(pFilename, TEXTURE_GROUP_PRECACHED,
                                         TEXTUREFLAGS_STREAMABLE_COARSE);
    COM_TimestampedLog("WarmTextureCache(): LoadTexture( %s ): Complete",
                       pFilename);

    if ((pTex->GetFlags() & TEXTUREFLAGS_STREAMABLE) == 0) {
      STAGING_ONLY_EXEC(
          Warning("%s is listed in texture_preload_list.txt or is otherwise "
                  "marked for streaming. It cannot be streamed and should be "
                  "removed from the streaming system.\n",
                  pFilename));
  ...
  g_pFullFileSystem->RemoveSearchPath("tf2_texture_cache.vpk",
                                      cTextureCachePathDir);

  COM_TimestampedLog("WarmTextureCache() - End");
}

//-----------------------------------------------------------------------------
// Reads the list of files contained in the vpk loaded above, and adds them to
// the list of files we need to load (passing in as pOutFilesToLoad). The map
// contains the
//-----------------------------------------------------------------------------
void CTextureManager::FindFilesToLoad(CUtlDict<int> *pOutFilesToLoad,
                                      const char *pFilename) {
  Assert(pOutFilesToLoad != NULL);

  FileFindHandle_t fh;
  pFilename =
      g_pFullFileSystem->FindFirstEx(pFilename, cTextureCachePathDir, &fh);
  ...
  while (pFilename != NULL) {
    if (g_pFullFileSystem->FindIsDirectory(fh)) {
      ...
      // Add the file to the list, which we will later traverse in order to
      // ensure we're hitting these in the expected order for the VPK.
...
// that we should load.
//-----------------------------------------------------------------------------
void CTextureManager::ReadFilesToLoad(CUtlDict<int> *pOutFilesToLoad,
                                      const char *pFilename) {
  Assert(pOutFilesToLoad != NULL);
...
void CTextureManager::UpdatePostAsync() {
  ...
  // Update the async loader, which affects streaming in (streaming out is
  // handled below). Both stream in and stream out have to happen while the
  // async job is not running because they muck with shaderapi texture handles
  // which could be in use if the async job is currently being run
  if (m_pAsyncLoader)
  ...
  while (m_asyncStreamingRequests.PopItem(&pRequest)) {
    ...
    // Update the LOD bias to smoothly stream the texture in. We only need to do
    // this on frames that we actually have been requested to draw--other frames
    // it doesn't matter (see, because we're not drawing?)
  ...
  FOR_EACH_MAP_FAST(m_textureStreamingRequests, i) {
    if (m_textureStreamingRequests[i] + cThirtySecondsOrSoInFrames <
        g_FrameNum) {
      ITextureInternal *pTex = m_textureStreamingRequests.Key(i);

      // It's been awhile since we were asked to full res this texture, so let's
      // evict if it's still full res.
  ...
  // Finally, flush any immediate release textures marked for cleanup that are
  // still unreferenced.
  CleanupPossiblyUnreferencedTextures();
}
...
bool CTextureManager::AddTextureCompositorTemplate(const char *pName,
                                                   KeyValues *pTmplDesc) {
  ...
  CTextureCompositorTemplate *pNewTmpl =
      CTextureCompositorTemplate::Create(pName, pTmplDesc);
...
CTextureManager::FindTextureCompositorTemplate(const char *pName) {
...
void CTextureManager::CoolTextureCache() {
  FOR_EACH_VEC(m_preloadedTextures, i) { m_preloadedTextures[i]->Release(); }
...
void CTextureManager::RequestAllMipmaps(ITextureInternal *pTex) {
  ...
  // If this isn't a streamable texture or if there are no mipmaps, there's
  // nothing to do.
  if (!(nTexFlags & TEXTUREFLAGS_STREAMABLE) ||
      (nTexFlags & TEXTUREFLAGS_NOMIP))
    return;
...
void CTextureManager::EvictAllTextures() {
  FOR_EACH_DICT_FAST(m_TextureList, i) {
    ...
    if (((pTex->GetFlags() & TEXTUREFLAGS_STREAMABLE) != 0) &&
        pTex->GetTargetResidence() == RESIDENT_FULL)
      pTex->MakeResident(RESIDENT_PARTIAL);
  }
}
...
static ImageFormat GetImageFormatRawReadback(ImageFormat fmt) {
  ...
  case IMAGE_FORMAT_BGRA8888:
    return IMAGE_FORMAT_BGRA8888;
  default:
    Assert(!"Unsupported format in GetImageFormatRawReadback, this will likely "
            "result in color-swapped textures");
  };
