...
private:
  ...
  // force the thread mode to single threaded, synchronizes with render thread
  // if running
...
public:
  ...
  ITexture *FindTexture(char const *pTextureName, const char *pTextureGroupName,
                        bool complain = true, int nAdditionalCreationFlags = 0);
  ...
  void EndRenderTargetAllocation(); // Simulate an Alt-Tab in here, which causes
  ...
  virtual ITexture *CreateNamedTextureFromBitsEx(
      ...
      ImageFormat fmt, int srcBufferSize, byte *srcBits, int nFlags) OVERRIDE;
  ...
  GetShaderAPITextureBindHandle(ITexture *pTexture, int nFrame,
  ...
  DELEGATE_TO_OBJECT_1(ITexture *, GetRenderTargetEx, int,
                       &m_HardwareRenderContext);
  DELEGATE_TO_OBJECT_7V(DrawClearBufferQuad, unsigned char, unsigned char,
  ...
  IMaterialProxy *DetermineProxyReplacements(IMaterial *pMaterial,
...
private:
  ...
  bool m_bThreadingNotAvailable; // this is true if the VirtualAlloc()'s in the
                                 // threading fail to allocate
  ...
  ShaderAPITextureHandle_t
      m_LinearToGammaTableTextureHandle; // linear to gamma srgb conversion
                                         // lookup for the current hardware
  ShaderAPITextureHandle_t
  ...
  enum BufferClearType_t // bClearColor + ( bClearAlpha << 1 ) + ( bClearDepth
                         // << 2 )
  {
    BUFFER_CLEAR_NONE,
    BUFFER_CLEAR_COLOR,
