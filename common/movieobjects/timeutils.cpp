...
// DmeFramerate_t
//
// exact (rational) representation of common framerates - any integral or ntsc
// framerate
//
////////////////////////////////////////////////////////////////////////////////////////

DmeFramerate_t::DmeFramerate_t(float fps) { SetFramerate(fps); }
...
void DmeFramerate_t::SetFramerate(float flFrameRate) {
  if (IsIntegralValue(flFrameRate)) {
    SetFramerate(RoundFloatToInt(flFrameRate));
                             ...
                             1000.0f)) // 1001 is the ntsc divisor (30*1000/1001
                                       // = 29.97, etc)
  {
...
// other (uncommon) options besides 30(29.97 - ntsc video) are 24 (23.976 - ntsc
// film) and 60 (59.94 - ntsc progressive)
...
DmeTime_t::DmeTime_t(int frame, DmeFramerate_t framerate) {
  ...
  // add signed offset to force integer truncation (towards 0) to give us
  // truncation towards -inf
  if (frame < 0) {
    prod -= num - 1;
  }
...
// float operators - comment these out to find potentially incorrect uses of
// DmeTime_t
...
float GetFractionOfTimeBetween(DmeTime_t t, DmeTime_t start, DmeTime_t end,
                               bool bClamp /*= false*/) {
  return GetFractionOfTime(t - start, end - start, bClamp);
}

float GetFractionOfTime(DmeTime_t t, DmeTime_t duration,
                        bool bClamp /*= false*/) {
...
int DmeTime_t::CurrentFrame(DmeFramerate_t framerate, bool bRoundDown) const {
  ...
  // times within this range are considered on a frame: (frame*den/num - 1,
  // frame*den/num] this follows from the truncation towards -inf behavior of
  // the frame,framerate constructor above the following logic is there to
  // ensure the above rule, while working around the truncation towards 0
  // behavior of integer divide
  if (m_tms < 0) {
    if (bRoundDown)
      prod -= den - num;
  } else {
...
DmeTime_t DmeTime_t::TimeAtCurrentFrame(DmeFramerate_t framerate,
                                        bool bRoundDown) const {
...
DmeTime_t DmeTime_t::TimeAtNextFrame(DmeFramerate_t framerate) const {
  // since we always round towards -inf, go to next frame whether we're on a
  // frame or not
