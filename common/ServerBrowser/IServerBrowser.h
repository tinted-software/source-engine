...
public:
  ...
  /// Given a map name, strips off some stuff and returns the "friendly" name of
  /// the map. Returns the cleaned out map name into the caller's buffer, and
  /// returns the friendly game type name.
  virtual const char *GetMapFriendlyNameAndGameType(
      const char *pszMapName, char *szFriendlyMapName, int cchFriendlyName) = 0;

  // Enable filtering of workshop maps, requires the game/tool loading us to
  // feed subscription data. This is a slightly ugly workaround to TF2 not yet
  // having native workshop UI in quickplay, once that is in place this should
  // either be stripped back out or expanded to be directly aware of the steam
  // workshop without being managed.
