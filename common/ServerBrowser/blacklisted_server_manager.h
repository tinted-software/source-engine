...
public:
  ...
  blacklisted_server_t *AddServer(const char *serverName, uint32 serverIP,
                                  int serverPort);
  blacklisted_server_t *AddServer(const char *serverName,
                                  const char *netAddressString,
                                  uint64 timestamp);

  void RemoveServer(
      int iServerID); // remove server with matching 'server id' from list
  ...
  GetServer(int iServerID); // return server with matching 'server id'
  ...
  bool IsServerBlacklisted(uint32 serverIP, int serverPort,
                           const char *serverName) const;

  bool CanServerBeBlacklisted(gameserveritem_t &server) const;
  bool CanServerBeBlacklisted(uint32 serverIP, int serverPort,
                              const char *serverName) const;
