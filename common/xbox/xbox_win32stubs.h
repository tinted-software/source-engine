...
FORCEINLINE DWORD XBX_GetEnvironmentVariable(LPCTSTR lpName, LPTSTR lpBuffer,
                                             DWORD nSize) {
  return 0;
}
...
PLATFORM_INTERFACE DWORD XBX_GetCurrentDirectory(DWORD nBufferLength,
                                                 LPTSTR lpBuffer);
...
FORCEINLINE BOOL SetPriorityClass(HANDLE hProcess, DWORD dwPriorityClass) {
  return FALSE;
}

PLATFORM_INTERFACE int MessageBox(HWND hWnd, LPCTSTR lpText, LPCTSTR lpCaption,
                                  UINT uType);
...
PLATFORM_INTERFACE DWORD XBX_GetModuleFileName(HMODULE hModule,
                                               LPTSTR lpFilename, DWORD nSize);

// FORCEINLINE int WSAStartup( WORD wVersionRequested, LPWSADATA lpWSAData ) {
// return WSASYSNOTREADY; } FORCEINLINE int WSACleanup(void) { return
// WSANOTINITIALISED; }

FORCEINLINE HRESULT CoInitialize(LPVOID pvReserved) { return S_OK; }
FORCEINLINE void CoUninitialize(void) {}

FORCEINLINE LRESULT DefWindowProc(HWND hWnd, UINT message, WPARAM wParam,
                                  LPARAM lParam) {
  return 0L;
}
...
FORCEINLINE BOOL GetConsoleScreenBufferInfo(HANDLE,
                                            PCONSOLE_SCREEN_BUFFER_INFO) {
  return false;
}

FORCEINLINE COORD GetLargestConsoleWindowSize(HANDLE) {
  COORD c = {0, 0};
  return c;
}

FORCEINLINE BOOL SetConsoleWindowInfo(HANDLE, BOOL, SMALL_RECT *) {
  return false;
}

FORCEINLINE BOOL SetConsoleScreenBufferSize(HANDLE, COORD) { return false; }

FORCEINLINE BOOL ReadConsoleOutputCharacter(HANDLE, LPTSTR, DWORD, COORD,
                                            LPDWORD) {
  return false;
}

FORCEINLINE BOOL WriteConsoleInput(HANDLE, CONST INPUT_RECORD *, DWORD,
                                   LPDWORD) {
  return false;
}
...
FORCEINLINE HWND FindWindow(LPCTSTR lpClassName, LPCTSTR lpWindowName) {
  return NULL;
}

FORCEINLINE BOOL EnumWindows(WNDENUMPROC lpEnumFunc, LPARAM lParam) {
  return FALSE;
}

FORCEINLINE BOOL PostMessageA(HWND hWnd, UINT Msg, WPARAM wParam,
                              LPARAM lParam) {
  return FALSE;
}

FORCEINLINE BOOL SystemParametersInfo(UINT uiAction, UINT uiParam,
...
FORCEINLINE BOOL EndPaint(HWND hWnd, CONST PAINTSTRUCT *lpPaint) {
  return TRUE;
}

FORCEINLINE BOOL AdjustWindowRectEx(LPRECT lpRect, DWORD dwStyle, BOOL bMenu,
                                    DWORD dwExStyle) {
  return TRUE;
}

FORCEINLINE LONG ChangeDisplaySettings(LPDEVMODE lpDevMode, DWORD dwflags) {
  return DISP_CHANGE_SUCCESSFUL;
}

FORCEINLINE DWORD GetFileType(HANDLE hFile) { return FILE_TYPE_DISK; }

FORCEINLINE BOOL FileTimeToDosDateTime(const FILETIME *lpFileTime,
...
FORCEINLINE BOOL DosDateTimeToFileTime(const WORD wFatDate, const WORD wFatTime,
...
FORCEINLINE HFONT CreateFontA(int, int, int, int, int, DWORD, DWORD, DWORD,
...
FORCEINLINE HANDLE LoadImage(HINSTANCE, LPCSTR, UINT, int, int, UINT) {
  return 0;
}
FORCEINLINE HANDLE LoadImageA(HINSTANCE, LPCSTR, UINT, int, int, UINT) {
  return 0;
}

FORCEINLINE HICON LoadIcon(HINSTANCE hInstance, ULONG_PTR lpIconName) {
  return 0;
}

FORCEINLINE COLORREF SetPixel(HDC, int, int, COLORREF col) { return col; }

FORCEINLINE BOOL BitBlt(HDC, int, int, int, int, HDC, int, int, DWORD) {
  return false;
}
...
FORCEINLINE int GetDIBits(HDC, HBITMAP, UINT, UINT, LPVOID, LPBITMAPINFO,
                          UINT) {
  return 0;
}
...
FORCEINLINE HBITMAP CreateDIBSection(HDC, CONST BITMAPINFO *, UINT,
                                     VOID **ppBits, HANDLE, DWORD) {
  ppBits = NULL;
  return 0;
}
...
FORCEINLINE UINT joyGetDevCaps(UINT uJoyID, JOYCAPS *pjc, UINT cbjc) {
  return 0;
}
...
FORCEINLINE BOOL PeekMessage(LPMSG lpMsg, HWND hWnd, UINT wMsgFilterMin,
...
FORCEINLINE LONG RegOpenKeyEx(HKEY hKey, LPCTSTR lpSubKey, DWORD ulOptions,
                              REGSAM samDesired, PHKEY phkResult) {
  return -1;
}

FORCEINLINE LONG RegQueryValueEx(HKEY hKey, LPCTSTR lpValueName,
                                 LPDWORD lpReserved, LPDWORD lpType,
                                 LPBYTE lpData, LPDWORD lpcbData) {
  return -1;
}

FORCEINLINE LONG RegCreateKeyEx(HKEY hKey, LPCTSTR lpSubKey, DWORD Reserved,
                                LPTSTR lpClass, DWORD dwOptions,
                                REGSAM samDesired,
...
FORCEINLINE LONG RegSetValueEx(HKEY hKey, LPCTSTR lpValueName, DWORD Reserved,
...
FORCEINLINE BOOL UnregisterClass(LPCTSTR lpClassNAme, HINSTANCE hInstance) {
  return TRUE;
}
FORCEINLINE BOOL UnregisterClassW(LPCWSTR lpClassNAme, HINSTANCE hInstance) {
  return TRUE;
}
...
FORCEINLINE BOOL EnumChildWindows(HWND hWndParent, WNDENUMPROC lpEnumFunc,
                                  LPARAM lParam) {
  return FALSE;
}
...
FORCEINLINE BOOL GetComputerName(LPTSTR lpBuffer, LPDWORD nSize) {
  return FALSE;
}
...
FORCEINLINE MCIERROR mciSendString(LPCTSTR lpszCommand, LPTSTR lpszReturnString,
                                   UINT cchReturn, HANDLE hwndCallback) {
  return 0;
}

FORCEINLINE MCIERROR mciSendCommand(MCIDEVICEID, UINT, DWORD, DWORD) {
  return (UINT)MCIERR_DRIVER;
}
...
FORCEINLINE HANDLE CreateFileMapping(HANDLE, LPSECURITY_ATTRIBUTES, DWORD,
                                     DWORD, DWORD, LPCTSTR) {
  return NULL;
}

FORCEINLINE LPVOID MapViewOfFile(HANDLE, DWORD, DWORD, DWORD, SIZE_T) {
  return NULL;
}

FORCEINLINE BOOL UnmapViewOfFile(LPCVOID) { return false; }

FORCEINLINE BOOL GetVersionEx(LPOSVERSIONINFO lpVersionInfo) {
  lpVersionInfo->dwPlatformId = VER_PLATFORM_WIN32_NT;
  return true;
}

FORCEINLINE BOOL GlobalMemoryStatusEx(LPMEMORYSTATUSEX lpBuffer) {
  return false;
}

FORCEINLINE BOOL TerminateThread(HANDLE hThread, DWORD dwExitCode) {
  return false;
}

FORCEINLINE BOOL HttpQueryInfo(HINTERNET hRequest, DWORD dwInfoLevel,
                               LPVOID lpBuffer, LPDWORD lpdwBufferLength,
                               LPDWORD lpdwIndex) {
  return false;
}

FORCEINLINE struct hostent FAR *_stdcall FAR
gethostbyname(const char FAR *name) {
  return NULL;
}

FORCEINLINE BOOL InternetReadFile(HINTERNET hFile, LPVOID lpBuffer,
...
FORCEINLINE BOOL InternetCrackUrl(LPCSTR lpszUrl, DWORD dwUrlLength,
                                  DWORD dwFlags,
                                  LPURL_COMPONENTS lpUrlComponents) {
  return false;
}

FORCEINLINE HINTERNET InternetOpen(LPCSTR lpszAgent, DWORD dwAccessType,
...
FORCEINLINE INTERNET_STATUS_CALLBACK InternetSetStatusCallback(
    HINTERNET hInternet, INTERNET_STATUS_CALLBACK lpfnInternetCallback) {
  return NULL;
}

FORCEINLINE HINTERNET InternetOpenUrl(HINTERNET hInternet, LPCSTR lpszUrl,
...
FORCEINLINE DWORD MsgWaitForMultipleObjects(DWORD, CONST HANDLE *, BOOL, DWORD,
                                            DWORD) {
  return 0;
}
...
FORCEINLINE BOOL GetProcessTimes(HANDLE, LPFILETIME ft1, LPFILETIME ft2,
                                 LPFILETIME ft3, LPFILETIME ft4) {
  return false;
}

// FORCEINLINE time_t Time(time_t *today) {  ULARGE_INTEGER ul;
// GetSystemTimeAsFileTime( (FILETIME*)&ul ); if(today) *today=ul.QuadPart;
// return ul.QuadPart; }
FORCEINLINE long Time(long *today) {
  ULARGE_INTEGER ul;
...
FORCEINLINE BOOL CreateProcess(LPCSTR lpApplicationName, LPSTR lpCommandLine,
...
FORCEINLINE bool GetProcessAffinityMask(HANDLE hProcess,
...
FORCEINLINE bool SetProcessAffinityMask(HANDLE hProcess,
...
FORCEINLINE DWORD_PTR SetThreadAffinityMask(HANDLE hThread,
...
FORCEINLINE CAudioSource *Voice_SetupAudioSource(int soundsource,
                                                 int entchannel) {
  return NULL;
}
...
FORCEINLINE ISteamMasterServerUpdater *SteamMasterServerUpdater() {
  return NULL;
}
FORCEINLINE bool SteamGameServer_Init(uint32 unIP, uint16 usPort,
                                      uint16 usGamePort, uint16 usSpectatorPort,
                                      uint16 usQueryPort,
                                      EServerMode eServerMode, int nGameAppId,
...
PLATFORM_INTERFACE LONG_PTR SetWindowLongPtr(HWND hWnd, int nIndex,
                                             LONG dwNewLong);
PLATFORM_INTERFACE LONG_PTR GetWindowLongPtrW(
    HWND hWnd, int nIndex); // false stub -- no work has been done to make
                            // unicode version of this function on 360
PLATFORM_INTERFACE LONG_PTR SetWindowLongPtrW(
    HWND hWnd, int nIndex,
    LONG dwNewLong); // false stub -- no work has been done to make unicode
                     // version of this function on 360
PLATFORM_INTERFACE HWND CreateWindow(LPCTSTR lpClassName, LPCTSTR lpWindowName,
                                     DWORD dwStyle, int x, int y, int nWidth,
                                     int nHeight, HWND hWndParent, HMENU hMenu,
                                     HINSTANCE hInstance, LPVOID lpParam);
PLATFORM_INTERFACE HWND CreateWindowEx(DWORD dwExStyle, LPCTSTR lpClassName,
...
PLATFORM_INTERFACE LRESULT SendMessage(HWND hWnd, UINT Msg, WPARAM wParam,
                                       LPARAM lParam);
PLATFORM_INTERFACE LRESULT CallWindowProc(WNDPROC lpPrevWndFunc, HWND hWnd,
                                          UINT Msg, WPARAM wParam,
                                          LPARAM lParam);
PLATFORM_INTERFACE BOOL GetClientRect(HWND hwnd, LPRECT lpRect);
PLATFORM_INTERFACE int GetDeviceCaps(HDC hdc, int nIndex);
PLATFORM_INTERFACE LRESULT SendMessageTimeout(HWND hWnd, UINT Msg,
                                              WPARAM wParam, LPARAM lParam,
                                              UINT fuFlags, UINT uTimeout,
                                              PDWORD_PTR lpdwResult);
PLATFORM_INTERFACE BOOL SetWindowPos(HWND hWnd, HWND hWndInsertAfter, int x,
