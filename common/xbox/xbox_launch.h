...
// id and version are used to tag the data blob, currently only need a singe
// hardcoded id when the version and id don't match, the data blob is not ours
...
#define LF_INTERNALLAUNCH                                                      \
  0x00000001 // set if launch was internal (as opposed to dashboard)
...
#define LF_GAMERESTART                                                         \
  0x00000010 // set if game wants to restart self (skips appchooser)
#define LF_INVITERESTART                                                       \
  0x00000020 // set if game was invited from another app (launches TF and fires
             // off session connect)
...
// Primarily designed to be anchored in tier0, so multiple systems can easily
// query and set the persistent payload.
//--------------------------------------------------------------------------------------
class CXboxLaunch {
public:
  CXboxLaunch() { ResetLaunchData(); }
  ...
  void ResetLaunchData() {
    ...
    memset(&m_Launch.header.nInviteSessionID, 0,
           sizeof(m_Launch.header.nInviteSessionID));
  ...
  bool GetLaunchData(unsigned int *pID, void **pData, int *pDataSize) {
    if (!m_LaunchDataSize) {
      // purposely not doing this in the constructor (unstable as used by
      // tier0), but on first fetch
      bool bValid = false;
      DWORD dwLaunchDataSize;
      DWORD dwStatus = XGetLaunchDataSize(&dwLaunchDataSize);
      if (dwStatus == ERROR_SUCCESS &&
          dwLaunchDataSize <= MAX_LAUNCH_DATA_SIZE) {
    ...
    // a valid launch payload could be ours (re-launch) or from an alternate
    // booter (demo launcher)
    if (m_LaunchDataSize == MAX_LAUNCH_DATA_SIZE &&
        m_Launch.header.id == VALVE_LAUNCH_ID &&
        m_Launch.header.version == VALVE_LAUNCH_VERSION) {
  ...
  void RestoreOrResetLaunchData() {
    RestoreLaunchData();
    if (m_Launch.header.id != VALVE_LAUNCH_ID ||
        m_Launch.header.version != VALVE_LAUNCH_VERSION) {
