...
#define XBX_PERSISTENT_BYTES_NEEDED                                            \
  (XBX_SAVEGAME_BYTES * 10) // 8 save games, 1 autosave, 1 autosavedangerous

#define XMAKECOLOR(r, g, b)                                                    \
  ((unsigned int)(((unsigned char)(r) |                                        \
                   ((unsigned int)((unsigned char)(g)) << 8)) |                \
                  (((unsigned int)(unsigned char)(b)) << 16)))

#define MAKE_NON_SRGB_FMT(x)                                                   \
  ((D3DFORMAT)(((unsigned int)(x)) &                                           \
               ~(D3DFORMAT_SIGNX_MASK | D3DFORMAT_SIGNY_MASK |                 \
                 D3DFORMAT_SIGNZ_MASK)))
...
PLATFORM_INTERFACE void XBX_QueueEvent(xevent_e event, int arg1, int arg2,
                                       int arg3);
