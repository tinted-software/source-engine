...
public:
  ...
  bool GetCachedXUIMetrics(const char *pWindowsFontName, int tall, int style,
                           XUIFontMetrics *pFontMetrics,
                           XUICharMetrics charMetrics[256]);
  void SetCachedXUIMetrics(const char *pWindowsFontName, int tall, int style,
                           XUIFontMetrics *pFontMetrics,
                           XUICharMetrics charMetrics[256]);
#endif
  ...
  void GetKernedCharWidth(vgui::HFont font, wchar_t ch, wchar_t chBefore,
                          wchar_t chAfter, float &wide, float &abcA,
                          float &abcC);

private:
  bool IsFontForeignLanguageCapable(const char *windowsFontName);
  font_t *CreateOrFindWin32Font(const char *windowsFontName, int tall,
                                int weight, int blur, int scanlines, int flags);
...
#if defined(_X360)
  // These are really bounded by the number of fonts that the game would ever
  // realistically create, so ~100 is expected. Many of these fonts are
  // redundant and the same underlying metrics can be used. This avoid the very
