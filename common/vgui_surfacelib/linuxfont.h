...
public:
  ...
  // OS.
  virtual bool CreateFromMemory(const char *windowsFontName, void *data,
                                int size, int tall, int weight, int blur,
  ...
  //  A spacing is the distance to add to the current position before drawing
  //  the character glyph. B spacing is the width of the drawn portion of the
  //  glyph. C spacing is the distance to add to the current position to provide
  //  white space to the right of the glyph.
  ...
  virtual void GetKernedCharWidth(wchar_t ch, wchar_t chBefore, wchar_t chAfter,
                                  float &wide, float &abcA, float &abcC);
...
private:
  ...
  // cache for storing asian abc widths (since it's too big too just store them
  // all)
  struct abc_cache_t {
    wchar_t wch;
    abc_t abc;
  ...
  static bool
  ExtendedKernedABCWidthsCacheLessFunc(const kerned_abc_cache_t &lhs,
                                       const kerned_abc_cache_t &rhs);
