...
struct RequestContext_t {
  inline RequestContext_t() { memset(this, 0, sizeof(RequestContext_t)); }
  ...
  char baseURL[BufferSize]; ///< Base URL (including http://).  Set by main
                            ///< thread.
  char urlPath[BufferSize]; ///< Path to be appended to base URL.  Set by main
                            ///< thread.
  char absLocalPath[BufferSize]; ///< Full local path where the file should go.
                                 ///< Set by main thread.
  char gamePath[BufferSize]; ///< Game path to be appended to base path.  Set by
                             ///< main thread.
  char
      serverURL[BufferSize]; ///< Server URL (IP:port, loopback, etc).  Set by
                             ///< main thread, and used for HTTP Referer header.

  bool bSuppressFileWrite; ///< Set to true by main thread if we do not wish to
