...
public:
  ...
  virtual void PlayReplay(ReplayHandle_t hReplay, int iPerformance,
                          bool bPlaySound) = 0; // Play the given replay, from
                                                // spawn tick to death tick
  ...
  virtual void GetPlaybackTimes(
      ...
      const CReplayPerformance
          *pPerformance) = 0; // Calculate the current time and length of a
                              // replay or performance - takes in tick and out
                              // tick into account for performances - flCurTime
                              // should be gpGlobals->curtime. pPerformance can
                              // be NULL.
