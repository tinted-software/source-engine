...
public:
  ...
  mutable IReplayDownloadEventHandler
      *m_pDownloadEventHandler; // Implemented by replay browser - the reason
                                // we've got one per replay rather than one per
                                // download group is because the browser needs
                                // to receive events per-thumbnail
  ...
  bool m_bComplete; // Indicates whether the replay is done recording - this
                    // should be false until the player dies, the round ends, or
                    // the level changes
  void *m_pUserData;
  ...
  int m_nSpawnTick;
  int m_nDeathTick;
  float m_flLength; // The length of the entire replay, including the post-death
                    // time, in seconds
  ...
  int m_iMaxSessionBlockRequired; // The maximum session block required to
                                  // reconstruct a viewable .dem file from spawn
                                  // tick until length
