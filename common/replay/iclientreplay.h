...
public:
  ...
  virtual bool CacheReplayRagdolls(const char *pFilename,
                                   int nStartTick) = 0; // Cache replay ragdolls
  virtual IReplayScreenshotSystem *
  GetReplayScreenshotSystem() = 0; // Get the client's replay screenshot system
  ...
  virtual void OnDeleteReplay(
      ReplayHandle_t hReplay) = 0; // Called before replay is actually removed
                                   // from the replay manager
