...
//
// NOTE: To make use of this file, g_pFullFileSystem must be defined, or you can
// modify this source to take an IFileSystem * as input.
//
//=======================================================================================//

// @note Tom Bui: we need to use fopen below in the jpeg code, so we can't have
// this on...
...
// clang3 on OSX folks the attribute into the prototype, causing a compile
// failure filed radar bug 10397783
...
ConversionErrorType ImgUtl_ConvertJPEGToTGA(const char *jpegpath,
                                            const char *tgaPath,
                                            bool bRequirePowerOfTwo) {
#if !defined(_X360) && HAVE_JPEG

  //
  // !FIXME! This really probably should use ImgUtl_ReadJPEGAsRGBA, to avoid
  // duplicated code.
  //
  ...
  // exit if not).
  if ((bRequirePowerOfTwo && (!IsPowerOfTwo(jpegInfo.image_height) ||
...
ConversionErrorType ImgUtl_ConvertBMPToTGA(const char *bmpPath,
                                           const char *tgaPath) {
  if (!IsPC())
    return CE_SOURCE_FILE_FORMAT_NOT_SUPPORTED;
  ...
  unsigned char *pBufRGBA =
      ImgUtl_ReadBMPAsRGBA(bmpPath, nWidth, nHeight, result);
  ...
  CUtlBuffer outBuf;
  bool retval =
      TGAWriter::WriteToBuffer(pBufRGBA, outBuf, nWidth, nHeight,
                               IMAGE_FORMAT_RGBA8888, IMAGE_FORMAT_RGB888);
  free(pBufRGBA);
...
unsigned char *ImgUtl_ReadVTFAsRGBA(const char *vtfPath, int &width,
                                    int &height, ConversionErrorType &errcode) {
  ...
  int nMemSize = ImageLoader::GetMemRequired(width, height, 1,
...
// buffer.
unsigned char *ImgUtl_ReadTGAAsRGBA(const char *tgaPath, int &width,
                                    int &height, ConversionErrorType &errcode,
                                    TGAHeader &tgaHeader) {
  ...
  if ((tgaHeader.imagetype !=
       2) // image type 2 is uncompressed RGB, other types not supported.
      || (tgaHeader.descriptor &
          0x10) // Origin on righthand side (flipped horizontally from common
                // sense) --- nobody ever uses this
...
unsigned char *ImgUtl_ReadJPEGAsRGBA(const char *jpegPath, int &width,
                                     int &height,
...
#if HAVE_PNG
static void ReadPNGData(png_structp png_ptr, png_bytep outBytes,
...
unsigned char *ImgUtl_ReadPNGAsRGBA(const char *pngPath, int &width,
                                    int &height, ConversionErrorType &errcode) {
  ...
  // Load it
  return ImgUtl_ReadPNGAsRGBAFromBuffer(bufFileContents, width, height,
                                        errcode);
#else
...
unsigned char *ImgUtl_ReadPNGAsRGBAFromBuffer(CUtlBuffer &buffer, int &width,
                                              int &height,
                                              ConversionErrorType &errcode) {
#if !defined(_X360) && HAVE_PNG
...
unsigned char *ImgUtl_ReadBMPAsRGBA(const char *bmpPath, int &width,
                                    int &height, ConversionErrorType &errcode) {
#ifdef WIN32
  // Load up bitmap
  HBITMAP hBitmap = (HBITMAP)LoadImage(NULL, bmpPath, IMAGE_BITMAP, 0, 0,
  ...
  } else if (bitmap.bmBitsPixel == 8 || bitmap.bmBitsPixel == 4 ||
  ...
  int mem_required =
      nDestStride *
      bitmap
          .bmHeight; // mem required for copying the data out into RGBA format.
  ...
  if (bitmapInfo->bmiHeader.biBitCount == 32) {
    for (int y = 0; y < bitmap.bmHeight; ++y) {
      unsigned char *pDest =
          buf + nDestStride *
                    ((bitmap.bmHeight - 1) - y); // BMPs are stored upside down
      const unsigned char *pSrc =
          (unsigned char *)(bitmap.bmBits) + (y * bitmap.bmWidthBytes);
...
unsigned char *ImgUtl_ReadImageAsRGBA(const char *path, int &width, int &height,
                                      ConversionErrorType &errcode) {
...
ConversionErrorType ImgUtl_ConvertTGA(const char *tgaPath,
                                      int nMaxWidth /*=-1*/,
                                      int nMaxHeight /*=-1*/) {
  ...
  unsigned char *srcBuffer =
      ImgUtl_ReadTGAAsRGBA(tgaPath, tgaWidth, tgaHeight, errcode, tgaHeader);
  ...
  ImgUtl_StretchRGBAImage(srcBuffer, tgaWidth, tgaHeight, resizeBuffer,
  ...
  ImgUtl_PadRGBAImage(resizeBuffer, finalWidth, finalHeight, finalBuffer,
...
ConversionErrorType
ImgUtl_StretchRGBAImage(const unsigned char *srcBuf, const int srcWidth,
                        const int srcHeight, unsigned char *destBuf,
...
ConversionErrorType ImgUtl_PadRGBAImage(const unsigned char *srcBuf,
...
// the same location.
ConversionErrorType ImgUtl_ConvertTGAToVTF(const char *tgaPath,
                                           int nMaxWidth /*=-1*/,
                                           int nMaxHeight /*=-1*/) {
  ...
  if ((nMaxWidth != -1 && header.width > nMaxWidth) ||
      (nMaxHeight != -1 && header.height > nMaxHeight)) {
    fclose(infile);
    Msg("Failed to open TGA - dimensions too large (%d, %d) (max: %d, %d): "
        "%s\n",
...
ConversionErrorType ImgUtl_ConvertToVTFAndDumpVMT(const char *pInPath,
                                                  const char *pMaterialsSubDir,
                                                  int nMaxWidth /*=-1*/,
                                                  int nMaxHeight /*=-1*/) {
#ifndef _XBOX
  ...
  if (stricmp(extension, "tga")) {
    //  It is not a TGA file, so create a temporary file name for the TGA you
    //  have to create
  ...
  if (convertTGAToVTF && !failed) {
    nErrorCode = ImgUtl_ConvertTGA(
        tgaPath, nMaxWidth,
        nMaxHeight); // resize TGA so that it has power-of-two dimensions with a
                     // max size of (nMaxWidth)x(nMaxHeight).
    if (nErrorCode != CE_SUCCESS) {
      failed = true;
    }
  ...
  if (!failed) {
    ...
    //  If I had to convert from another filetype (i.e. the original was NOT a
    //  .vtf)
    if (convertTGAToVTF) {
    ...
    while ((vtfFilename > pInPath) && (*(vtfFilename - 1) != '\\') &&
           (*(vtfFilename - 1) != '/')) {
      --vtfFilename;
    }
...
ConversionErrorType ImgUtl_WriteGenericVMT(const char *vtfPath,
                                           const char *pMaterialsSubDir) {
...
#if HAVE_PNG
static void WritePNGData(png_structp png_ptr, png_bytep inBytes,
...
ConversionErrorType
ImgUtl_WriteRGBAAsPNGToBuffer(const unsigned char *pRGBAData, int nWidth,
                              int nHeight, CUtlBuffer &bufOutData,
                              int nStride) {
#if !defined(_X360)
...
METHODDEF(void) init_destination(j_compress_ptr cinfo) {
  ...
  dest->buffer = (byte *)(*cinfo->mem->alloc_small)(
      (j_common_ptr)cinfo, JPOOL_IMAGE, JPEG_OUTPUT_BUF_SIZE * sizeof(byte));
...
//-----------------------------------------------------------------------------
bool ImgUtl_WriteRGBToJPEG(unsigned char *pSrcBuf, unsigned int nSrcWidth,
                           unsigned int nSrcHeight, const char *lpszFilename) {
#if HAVE_JPEG
  CUtlBuffer dstBuf;
...
ConversionErrorType
ImgUtl_WriteRGBAAsJPEGToBuffer(const unsigned char *pRGBAData, int nWidth,
                               int nHeight, CUtlBuffer &bufOutData,
                               int nStride) {
#if !defined(_X360) && HAVE_JPEG
  ...
  unsigned char *pDstRow =
      (unsigned char *)malloc(sizeof(unsigned char) * nWidth * 4);
  while (cinfo.next_scanline < cinfo.image_height) {
    const unsigned char *pSrcRow =
        &(pRGBAData[cinfo.next_scanline * row_stride]);
...
ConversionErrorType ImgUtl_LoadBitmap(const char *pszFilename,
                                      Bitmap_t &bitmap) {
  bitmap.Clear();
  ConversionErrorType nErrorCode;
  int width, height;
  unsigned char *buffer =
      ImgUtl_ReadImageAsRGBA(pszFilename, width, height, nErrorCode);
  if (nErrorCode != CE_SUCCESS) {
    return nErrorCode;
  }

  // Install the buffer into the bitmap, and transfer ownership
  bitmap.SetBuffer(width, height, IMAGE_FORMAT_RGBA8888, buffer, true,
                   width * 4);
  return CE_SUCCESS;
}

static ConversionErrorType ImgUtl_LoadJPEGBitmapFromBuffer(CUtlBuffer &fileData,
                                                           Bitmap_t &bitmap) {
  // @todo implement
  return CE_SOURCE_FILE_FORMAT_NOT_SUPPORTED;
}

static ConversionErrorType
ImgUtl_SaveJPEGBitmapToBuffer(CUtlBuffer &fileData, const Bitmap_t &bitmap) {
  if (!bitmap.IsValid()) {
  ...
  ConversionErrorType result = ImgUtl_WriteRGBAAsJPEGToBuffer(
      bitmap.GetBits(), bitmap.Width(), bitmap.Height(), fileData,
      bitmap.Stride());
  return result;
}

ConversionErrorType
ImgUtl_LoadBitmapFromBuffer(CUtlBuffer &fileData, Bitmap_t &bitmap,
...
ConversionErrorType
ImgUtl_SaveBitmapToBuffer(CUtlBuffer &fileData, const Bitmap_t &bitmap,
                          ImageFileFormat eImageFileFormat) {
  switch (eImageFileFormat) {
  case kImageFileFormat_PNG:
...
ConversionErrorType ImgUtl_LoadPNGBitmapFromBuffer(CUtlBuffer &fileData,
                                                   Bitmap_t &bitmap) {
#if HAVE_PNG
  bitmap.Clear();
  ConversionErrorType nErrorCode;
  int width, height;
  unsigned char *buffer =
      ImgUtl_ReadPNGAsRGBAFromBuffer(fileData, width, height, nErrorCode);
  if (nErrorCode != CE_SUCCESS) {
    return nErrorCode;
  }
...
ConversionErrorType ImgUtl_SavePNGBitmapToBuffer(CUtlBuffer &fileData,
  ...
  // Do it
  ConversionErrorType result =
      ImgUtl_WriteRGBAAsPNGToBuffer(bitmap.GetBits(), bitmap.Width(),
                                    bitmap.Height(), fileData, bitmap.Stride());
  return result;
#else
  return CE_SOURCE_FILE_FORMAT_NOT_SUPPORTED;
#endif
...
ConversionErrorType ImgUtl_ResizeBitmap(Bitmap_t &destBitmap, int nWidth,
                                        int nHeight,
                                        const Bitmap_t *pImgSource) {
  ...
  if (pImgSource->Format() != IMAGE_FORMAT_RGBA8888 ||
  ...
  // Do it
  return ImgUtl_StretchRGBAImage(pImgSource->GetBits(), pImgSource->Width(),
                                 pImgSource->Height(), destBitmap.GetBits(),
