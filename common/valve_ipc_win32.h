...
private:
  ...
  public:
    explicit Iterator(char *m_pMemory = NULL) {
      ...
      if (*m_szServerName) {
        memcpy(&m_uuid, m_szServerName + strlen(m_szServerName) + 1,
               sizeof(UUID));
      } else {
        m_uuid = GUID_NULL;
      }
  ...
  public:
    ...
    Iterator Next() const {
      return IsValid() ? Iterator(m_szServerName + strlen(m_szServerName) + 1 +
                                  sizeof(UUID))
                       : Iterator(NULL);
    }
...
public:
  virtual BOOL ExecuteCommand(char *bufCommand, DWORD numCommandBytes,
                              char *bufResult, DWORD &numResultBytes) = 0;
...
public:
  BOOL ExecuteCommand(LPVOID bufIn, DWORD numInBytes, LPVOID bufOut,
                      DWORD &numOutBytes);
...
public:
  explicit CValveIpcServerUtl(char const *szServerName)
      : CValveIpcServer(szServerName) {}
  virtual BOOL ExecuteCommand(char *bufCommand, DWORD numCommandBytes,
                              char *bufResult, DWORD &numResultBytes) {
...
public:
  explicit CValveIpcClientUtl(char const *szServerName)
      : CValveIpcClient(szServerName) {}
...
VALVE_IPC_IMPL CValveIpcMgr::CValveIpcMgr()
    : m_hMutex(NULL), m_hMemorySegment(NULL), m_pMemory(NULL) {}
...
VALVE_IPC_IMPL BOOL CValveIpcMgr::Init(DWORD dwTimeout) {
  ...
  m_hMutex = ::CreateMutex(NULL, FALSE,
                           VALVE_IPC_MGR_NAME "_MTX_" VALVE_IPC_PROTOCOL_VER);
  DWORD dwWaitResult =
      m_hMutex ? ::WaitForSingleObject(m_hMutex, dwTimeout) : WAIT_FAILED;

  if (dwWaitResult == WAIT_OBJECT_0 || dwWaitResult == WAIT_ABANDONED_0) {
    // We own the mgr segment

    m_hMemorySegment = ::CreateFileMapping(
        INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, 0, VALVE_IPC_MGR_MEMORY,
        VALVE_IPC_MGR_NAME "_MEM_" VALVE_IPC_PROTOCOL_VER);

    if (m_hMemorySegment) {
      LPVOID lpvMemSegment =
          ::MapViewOfFile(m_hMemorySegment, FILE_MAP_ALL_ACCESS, 0, 0, 0);
...
VALVE_IPC_IMPL BOOL CValveIpcMgr::DiscoverServer(char const *szServerName,
                                                 RPC_CSTR *pszServerUID) {
...
VALVE_IPC_IMPL BOOL CValveIpcMgr::RegisterServer(char const *szServerName,
                                                 RPC_CSTR *pszServerUID) {
  if (!szServerName || !*szServerName)
    return FALSE;
...
VALVE_IPC_IMPL HANDLE CValveIpcMgr::DuplicateMemorySegmentHandle() {
  ...
  HANDLE hDup = NULL;
  ::DuplicateHandle(GetCurrentProcess(), m_hMemorySegment, GetCurrentProcess(),
...
VALVE_IPC_IMPL BOOL CValveIpcServer::WaitForCommand() {
  ...
  m_cbBufferRead = 0;
  BOOL bRead = ::ReadFile(m_hServerPipe, m_pBufferRead, VALVE_IPC_CS_BUFFER,
                          &m_cbBufferRead, &ov);
...
VALVE_IPC_IMPL BOOL CValveIpcServer::WaitForResult() {
  ...
  DWORD cbWrite;
  BOOL bWrite = ::WriteFile(m_hServerPipe, m_pBufferWrite, m_cbBufferWrite,
                            &cbWrite, &ov);
...
VALVE_IPC_IMPL DWORD CValveIpcServer::RunImpl() {
  for (; WaitForClient();) {
    for (; WaitForCommand();) {
      m_cbBufferWrite = 0;
      BOOL bResult = ExecuteCommand(m_pBufferRead, m_cbBufferRead,
                                    m_pBufferWrite, m_cbBufferWrite);
...
VALVE_IPC_IMPL BOOL CValveIpcServer::Start() {
  ...
  m_hThread =
      ::CreateThread(NULL, 0, RunDelegate, this, CREATE_SUSPENDED, NULL);
  if (!m_hThread)
    return FALSE;
...
VALVE_IPC_IMPL BOOL CValveIpcClient::Connect() {
  ...
  sprintf(chPipeName,
          ...
          m_szServerUID);
  m_hClientPipe =
      ::CreateFile(chPipeName, GENERIC_READ | GENERIC_WRITE, 0, NULL,
...
VALVE_IPC_IMPL BOOL CValveIpcClient::ExecuteCommand(LPVOID bufIn,
                                                    DWORD numInBytes,
                                                    LPVOID bufOut,
                                                    DWORD &numOutBytes) {
  ...
  BOOL bTransact = TransactNamedPipe(m_hClientPipe, bufIn, numInBytes, bufOut,
                                     numOutBytes, &numOutBytes, NULL);
