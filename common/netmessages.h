...
public:
  ...
  bool m_bCreateOnClient; // if true, client creates own SendTables & classinfos
                          // from game.dll
...
// SVC_VoiceInit
//   v2 - 2017/02/07
//     - Can detect v2 packets by nLegacyQuality == 255 and presence of
//     additional nSampleRate field.
//     - Added nSampleRate field. Previously, nSampleRate was hard-coded per
//     codec type. ::ReadFromBuffer does a
//       one-time conversion of these old types (which can no longer change)
//     - Marked quality field as deprecated. This was already being ignored. v2
//     clients send 255
//     - Prior to this the sv_use_steam_voice convar was used to switch to steam
//     voice. With this, we properly set
//       szVoiceCodec to "steam".  See ::ReadFromBuffer for shim to fallback to
//       the convar for old streams.
//     - We no longer pass "svc_voiceinit NULL" as szVoiceCodec if it is not
//     selected, just the empty string.  Nothing
...
public:
  ...
  // DEPRECATED:
  //
  // This field used to be a custom quality setting, but it was not honored for
  // a long time: codecs use their own pre-configured quality settings. We never
  // sent anything besides 5, which was then ignored for some codecs.
  //
  // New clients always set 255 here, old clients probably send 5. This could be
  // re-purposed in the future, but beware that very old demos may have non-5
  // values. It would take more archaeology to determine how to properly
  // interpret those packets -- they're probably using settings we simply don't
  // support any longer.
  //
  // int m_nQuality;
...
public:
  ...
  QueryCvarCookie_t m_iCookie;
  const char *m_szCvarName; // The sender sets this, and it automatically points
                            // it at m_szCvarNameBuffer when receiving.
