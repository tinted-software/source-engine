...
extern unsigned int STEAM_fread(void *buffer, unsigned int rdsize,
                                unsigned int count, FILE *file);
extern int STEAM_fgetc(FILE *file);
extern unsigned int STEAM_fwrite(void *buffer, unsigned int rdsize,
                                 unsigned int count, FILE *file);
...
extern HANDLE STEAM_FindFirstFile(char *pszMatchName, STEAMFindFilter filter,
                                  WIN32_FIND_DATA *findInfo);
extern int STEAM_FindNextFile(HANDLE dir, WIN32_FIND_DATA *findInfo);
extern int STEAM_FindClose(HANDLE dir);

extern long STEAM_findfirst(char *pszMatchName, STEAMFindFilter filter,
...
extern STEAM_RegisterAppProgressCallback(void (*fpProgCallBack)(void),
                                         int freq);
extern STEAM_RegisterAppKeepAliveTicCallback(
    void (*fpKeepAliveTicCallBack)(char *scr_msg));
