...
//			(.vvd, .vtx, .mdl, .phy, .ani) so the files can be
//loaded 			on a big-endian machine, specifically the Xbox360. A new file is
//generated 			with an extra extension in the form of <filename>.360.<ext>
//
//=============================================================================//
...
#pragma warning(                                                               \
    disable : 4366) // The result of the unary '&' operator may be unaligned
...
//----------------------------------------------------------------------
template <class T>
inline void WriteObjects(byte **pOutputBuffer, byte **pBaseData,
                         int objectCount = 1) {
  T tempObject;
...
//----------------------------------------------------------------------
template <class T>
inline void WriteBuffer(byte **pOutputBuffer, byte **pBaseData,
                        int objectCount = 1) {
  T tempObject;
  for (int i = 0; i < objectCount; ++i) {
    Q_memcpy(&tempObject, *pBaseData, sizeof(T));
...
//----------------------------------------------------------------------
template <class T> T SrcNative(T *idx) {
  T ret = *idx;
  if (!g_bNativeSrc) {
...
template <class T> T DestNative(T *idx) {
  T ret = *idx;
  if (g_bNativeSrc) {
    g_Swap.SwapBuffer(&ret, idx);
  }
...
//----------------------------------------------------------------------
//	Declares src/dest byte pointers and sets them to some index offset in
//the buffers.
//----------------------------------------------------------------------
...
//----------------------------------------------------------------------
//	Declares src/dest byte pointers and sets them to some index offset in
//the buffers. 	If src pointer is misaligned, the fixup method is called.
//----------------------------------------------------------------------
...
#define FIXUP_OFFSETS(ptr, base, index)                                        \
  {                                                                            \
    ...
    if (ptr##Fixup != ptr##Src) {                                              \
      ...
      Q_memmove((byte *)ptr##Src + nShiftBytes, ptr##Src,                      \
      ...
      if (fixedFileSize > fileSize + BYTESWAP_ALIGNMENT_PADDING) {             \
        Error("Byteswap buffer overrun - increase "                            \
...
void ProcessFields(void *pBaseAddress, void *pData, datamap_t *pDataMap,
  ...
  for (int i = 0; i < fieldCount; ++i) {
    ...
    ProcessField(pBaseAddress,
                 (BYTE *)pData + pField->fieldOffset[TD_OFFSET_NORMAL], pField,
                 pfnProcessFunc);
  }
}
...
void ProcessFieldByName(void *pBaseAddress, void *pData, datamap_t *pDataMap,
  ...
  for (int i = 0; i < fieldCount; ++i) {
    ...
    if (!Q_stricmp(pField->fieldName, pName)) {
      ProcessField(pBaseAddress,
                   (BYTE *)pData + pField->fieldOffset[TD_OFFSET_NORMAL],
...
DEFINE_FIELD(size, FIELD_INTEGER), DEFINE_ARRAY(mass_center, FIELD_FLOAT, 3),
  ...
  for (int i = 0; i < pHdr->solidCount; ++i) {
    ...
    if (baseHdr->vphysicsID != MAKEID('V', 'P', 'H', 'Y')) {
      ...
      if (legacyHdr->dummy[2] == MAKEID('I', 'V', 'P', 'S') ||
          legacyHdr->dummy[2] == 0) {
...
void ByteswapAnimData(mstudioanimdesc_t *pAnimDesc, int section,
  ...
  while (pAnimation) {
    ...
    if (pAnimation->nextoffset) {
      ...
      pAnimationSrc = (mstudioanim_t *)((byte *)pAnimationSrc +
                                        SrcNative(&pAnimation->nextoffset));
      pAnimationDest = (mstudioanim_t *)((byte *)pAnimationDest +
                                         SrcNative(&pAnimation->nextoffset));
...
int ByteswapIKRules(studiohdr_t *&pHdrSrc, int numikrules, int numFrames,
                    byte *&pDataSrc, byte *&pDataDest, int &fixedFileSize,
                    const int fileSize) {
...
int ByteswapMDLFile(void *pDestBase, void *pSrcBase, const int fileSize) {
  ...
  ITERATE_BLOCK(pSequence, pHdr->numlocalseq) {
    ...
    WriteObjects<mstudioactivitymodifier_t>(
        pDataDest, pDataSrc, SrcNative(&pSequence->numactivitymodifiers));
  }
  ...
  WriteBuffer<short>(&pDataDest, &pDataSrc,
                     SrcNative(&pHdr->numskinfamilies) *
                         SrcNative(&pHdr->numskinref));
  ...
  if (pHdr->studiohdr2index) {
    ...
    ITERATE_BLOCK(pStudioHdr2, studiohdr2ct) {
      ...
      if (pStudioHdr2->numsrcbonetransform) {
        ...
        pDataSrc =
            (byte *)pHdrSrc + SrcNative(&pStudioHdr2->srcbonetransformindex);
        pDataDest =
            (byte *)pHdrDest + SrcNative(&pStudioHdr2->srcbonetransformindex);
      ...
      if (pStudioHdr2->m_nBoneFlexDriverIndex) {
        ...
        ITERATE_BLOCK(pBoneFlexDriver, pStudioHdr2->m_nBoneFlexDriverCount) {
          ...
          WriteObjects<mstudioboneflexdrivercontrol_t>(
              &pDataDest, &pDataSrc,
...
void ProcessMDLFields(void *pDataBase, datadescProcessFunc_t pfnProcessFunc) {
  ...
  for (int i = 0; i < SrcNative(&pHdr->numhitboxsets); ++i, ++pHitboxSet) {
    ...
    for (int iHit = 0; iHit < SrcNative(&pHitboxSet->numhitboxes);
         ++iHit, ++pBBox) {
  ...
  for (int i = 0; i < SrcNative(&pHdr->numlocalanim); ++i, ++pAnimDesc) {
    ...
    if (pAnimDesc->ikruleindex) {
      ...
      mstudioikrule_t *pIKRule = (mstudioikrule_t *)pData;
      for (int iIK = 0; iIK < SrcNative(&pAnimDesc->numikrules);
           ++iIK, ++pIKRule) {
        ProcessFields(pIKRule, &mstudioikrule_t::m_DataMap, pfnProcessFunc);
      }
    }
  ...
  for (int i = 0; i < SrcNative(&pHdr->numlocalseq); ++i, ++pSequenceHdr) {
    ...
    for (int iEvent = 0; iEvent < SrcNative(&pSequenceHdr->numevents);
         ++iEvent, ++pEvent) {
  ...
  for (int i = 0; i < SrcNative(&pHdr->numbodyparts); ++i, ++pBodypart) {
    ...
    for (int iModel = 0; iModel < SrcNative(&pBodypart->nummodels);
         ++iModel, ++pModel) {
      ...
      for (int iMesh = 0; iMesh < SrcNative(&pModel->nummeshes);
           ++iMesh, ++pMesh) {
        ...
        for (int iFlesh = 0; iFlesh < SrcNative(&pMesh->numflexes);
             ++iFlesh, ++pFlex) {
      ...
      for (int iEye = 0; iEye < SrcNative(&pModel->numeyeballs);
           ++iEye, ++pEyeball) {
  ...
  if (pHdr->studiohdr2index) {
    ...
    for (int i = 0; i < 1; ++i, ++pStudioHdr2) {
      ...
      for (int iBone = 0; iBone < SrcNative(&pStudioHdr2->numsrcbonetransform);
           ++iBone, ++pSrcBoneTransform) {
...
DEFINE_FIELD(id, FIELD_INTEGER), DEFINE_FIELD(version, FIELD_INTEGER),
    ...
    DEFINE_FIELD(view_bbmax, FIELD_VECTOR), DEFINE_FIELD(flags, FIELD_INTEGER),
    ...
    DEFINE_FIELD(activitylistversion,
                 FIELD_INTEGER), // initialization flag - have the sequences
                                 // been indexed?
    ...
    DEFINE_FIELD(axis, FIELD_INTEGER), DEFINE_ARRAY(pos, FIELD_VECTOR, 6),
    ...
    DEFINE_FIELD(length, FIELD_FLOAT), DEFINE_FIELD(tipMass, FIELD_FLOAT),
    ...
    DEFINE_FIELD(pitchBounce, FIELD_FLOAT), DEFINE_FIELD(baseMass, FIELD_FLOAT),
    ...
    END_BYTESWAP_DATADESC()
        ...
        BEGIN_BYTESWAP_DATADESC(mstudioaimatbone_t)
            DEFINE_FIELD(parent, FIELD_INTEGER),
    DEFINE_FIELD(aim, FIELD_INTEGER), DEFINE_ARRAY(aimvector, FIELD_FLOAT, 3),
    ...
    DEFINE_FIELD(type, FIELD_INTEGER), DEFINE_FIELD(start, FIELD_FLOAT),
    DEFINE_FIELD(end, FIELD_FLOAT), DEFINE_FIELD(rest, FIELD_INTEGER),
    ...
    END_BYTESWAP_DATADESC()
        ...
        BEGIN_BYTESWAP_DATADESC(mstudioattachment_t)
            DEFINE_INDEX(sznameindex, FIELD_INTEGER),
    DEFINE_FIELD(flags, FIELD_INTEGER), DEFINE_FIELD(localbone, FIELD_INTEGER),
    ...
    END_BYTESWAP_DATADESC()
        ...
        BEGIN_BYTESWAP_DATADESC(mstudiobbox_t)
            DEFINE_FIELD(bone, FIELD_INTEGER),
    DEFINE_FIELD(group, FIELD_INTEGER), DEFINE_FIELD(bbmin, FIELD_VECTOR),
    ...
    DEFINE_FIELD(iNewParent, FIELD_INTEGER), DEFINE_FIELD(start, FIELD_FLOAT),
    DEFINE_FIELD(peak, FIELD_FLOAT), DEFINE_FIELD(tail, FIELD_FLOAT),
    DEFINE_FIELD(end, FIELD_FLOAT), DEFINE_FIELD(iStart, FIELD_INTEGER),
    ...
    DEFINE_FIELD(flags, FIELD_CHARACTER), DEFINE_INDEX(nextoffset, FIELD_SHORT),
    END_BYTESWAP_DATADESC()
    ...
    DEFINE_FIELD(type, FIELD_INTEGER), DEFINE_FIELD(chain, FIELD_INTEGER),
    DEFINE_FIELD(bone, FIELD_INTEGER), DEFINE_FIELD(slot, FIELD_INTEGER),
    DEFINE_FIELD(height, FIELD_FLOAT), DEFINE_FIELD(radius, FIELD_FLOAT),
    DEFINE_FIELD(floor, FIELD_FLOAT), DEFINE_FIELD(pos, FIELD_VECTOR),
    DEFINE_FIELD(q, FIELD_QUATERNION),
    DEFINE_INDEX(compressedikerrorindex, FIELD_INTEGER),
    DEFINE_FIELD(unused2, FIELD_INTEGER), DEFINE_FIELD(iStart, FIELD_INTEGER),
    DEFINE_INDEX(ikerrorindex, FIELD_INTEGER), DEFINE_FIELD(start, FIELD_FLOAT),
    DEFINE_FIELD(peak, FIELD_FLOAT), DEFINE_FIELD(tail, FIELD_FLOAT),
    DEFINE_FIELD(end, FIELD_FLOAT), DEFINE_FIELD(unused3, FIELD_FLOAT),
    DEFINE_FIELD(contact, FIELD_FLOAT), DEFINE_FIELD(drop, FIELD_FLOAT),
    DEFINE_FIELD(top, FIELD_FLOAT), DEFINE_FIELD(unused6, FIELD_INTEGER),
    DEFINE_FIELD(unused7, FIELD_INTEGER), DEFINE_FIELD(unused8, FIELD_INTEGER),
    ...
    DEFINE_INDEX(eventindex, FIELD_INTEGER), DEFINE_FIELD(bbmin, FIELD_VECTOR),
    DEFINE_FIELD(bbmax, FIELD_VECTOR), DEFINE_FIELD(numblends, FIELD_INTEGER),
    ...
    DEFINE_ARRAY(unused, FIELD_INTEGER,
                 5), // remove/add as appropriate (grow back to 8 ints on
                     // version change!)
    END_BYTESWAP_DATADESC()

        BEGIN_BYTESWAP_DATADESC(mstudioevent_t)
            DEFINE_FIELD(cycle, FIELD_FLOAT),
    DEFINE_FIELD(event, FIELD_INTEGER), DEFINE_FIELD(type, FIELD_INTEGER),
    ...
    DEFINE_FIELD(iPose, FIELD_SHORT), DEFINE_FIELD(flags, FIELD_INTEGER),
    ...
    DEFINE_FIELD(flags, FIELD_INTEGER), DEFINE_ARRAY(unused, FIELD_INTEGER, 4),
    END_BYTESWAP_DATADESC()
    ...
    DEFINE_INDEX(sznameindex, FIELD_INTEGER),
    DEFINE_FIELD(localToGlobal, FIELD_INTEGER), DEFINE_FIELD(min, FIELD_FLOAT),
    ...
    DEFINE_FIELD(numops, FIELD_INTEGER), DEFINE_INDEX(opindex, FIELD_INTEGER),
    END_BYTESWAP_DATADESC()
    ...
    END_BYTESWAP_DATADESC()
        ...
        BEGIN_BYTESWAP_DATADESC(mstudioiklink_t)
            DEFINE_FIELD(bone, FIELD_INTEGER),
    DEFINE_FIELD(kneeDir, FIELD_VECTOR), DEFINE_FIELD(unused0, FIELD_VECTOR),
    END_BYTESWAP_DATADESC()

        BEGIN_BYTESWAP_DATADESC(mstudiomouth_t)
            DEFINE_FIELD(bone, FIELD_INTEGER),
    DEFINE_FIELD(forward, FIELD_VECTOR), DEFINE_FIELD(flexdesc, FIELD_INTEGER),
    END_BYTESWAP_DATADESC()
    ...
    END_BYTESWAP_DATADESC()
        ...
        BEGIN_BYTESWAP_DATADESC(mstudio_meshvertexdata_t)
            ...
            DEFINE_FIELD(index_ptr_modelvertexdata,
    ...
    END_BYTESWAP_DATADESC()
        ...
        BEGIN_BYTESWAP_DATADESC(mstudioeyeball_t)
            DEFINE_INDEX(sznameindex, FIELD_INTEGER),
    DEFINE_FIELD(bone, FIELD_INTEGER), DEFINE_FIELD(org, FIELD_VECTOR),
    DEFINE_FIELD(zoffset, FIELD_FLOAT), DEFINE_FIELD(radius, FIELD_FLOAT),
    DEFINE_FIELD(up, FIELD_VECTOR), DEFINE_FIELD(forward, FIELD_VECTOR),
    DEFINE_FIELD(texture, FIELD_INTEGER), DEFINE_FIELD(unused1, FIELD_INTEGER),
    ...
    DEFINE_FIELD(target0, FIELD_FLOAT), DEFINE_FIELD(target1, FIELD_FLOAT),
    DEFINE_FIELD(target2, FIELD_FLOAT), DEFINE_FIELD(target3, FIELD_FLOAT),
    ...
    DEFINE_FIELD(speed, FIELD_CHARACTER), DEFINE_FIELD(side, FIELD_CHARACTER),
    ...
    END_BYTESWAP_DATADESC()
        ...
        BEGIN_BYTESWAP_DATADESC(mstudiotexture_t)
            DEFINE_INDEX(sznameindex, FIELD_INTEGER),
    DEFINE_FIELD(flags, FIELD_INTEGER), DEFINE_FIELD(used, FIELD_INTEGER),
    ...
    DEFINE_FIELD(version, FIELD_INTEGER), DEFINE_FIELD(checksum, FIELD_INTEGER),
    ...
    END_BYTESWAP_DATADESC()
        ...
        BEGIN_BYTESWAP_DATADESC(mstudiovertex_t) DEFINE_EMBEDDED(m_BoneWeights),
    ...
    namespace OptimizedModel{
        ...
        END_BYTESWAP_DATADESC()
            ...
            BEGIN_BYTESWAP_DATADESC(Vertex_t)
                DEFINE_ARRAY(boneWeightIndex, FIELD_CHARACTER,
...
BEGIN_BYTESWAP_DATADESC(phyheader_t) DEFINE_FIELD(size, FIELD_INTEGER),
    DEFINE_FIELD(id, FIELD_INTEGER), DEFINE_FIELD(solidCount, FIELD_INTEGER),
    DEFINE_FIELD(checkSum, FIELD_INTEGER), END_BYTESWAP_DATADESC()
