...
// place for including order-dependent library headers (things that need to go
// first).

class CSimpleBitString {
public:
  explicit CSimpleBitString(uint32 ReserveNumBits = 0)
      : m_uNumBits(0), m_vecU8() {
  ...
  void ReversiblyObfusticateBitsFromStart(uint32 NumBits,
                                          const uint8 *pObfusticationData,
...
public:
  ...
  public:
    explicit iterator(const CSimpleBitString &bs)
        : m_rSimpleBitString(bs), m_uNextBitIdx(0) {}
