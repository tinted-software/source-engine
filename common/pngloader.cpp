...
void ReadPNGData(png_structp png_ptr, png_bytep outBytes,
...
// called when we failed to load a png file, we just bail to the previously set
// exit
...
bool GetPNGDimensions(const byte *pubPNGData, int cubPNGData, uint32 &width,
                      uint32 &height) {
  png_const_bytep pngData = (png_const_bytep)pubPNGData;
  if (png_sig_cmp(pngData, 0, 8))
    return false; /* bad signature */

  PngDataStream_t pngDataStream(
      pubPNGData, cubPNGData); // keeps a local copy of the data we a reading
...
bool ConvertPNGToRGBA(const byte *pubPNGData, int cubPNGData,
