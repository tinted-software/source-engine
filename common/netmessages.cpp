...
// XXX(JohnS): This is no longer used, but we're keeping it so we can check if
// old network streams set it to work around
//             missing information in the older SVC_VoiceInit packet. See
//             SVC_VoiceInit::ReadFromBuffer
ConVar sv_use_steam_voice(
    "sv_use_steam_voice", "0", FCVAR_HIDDEN | FCVAR_REPLICATED,
...
bool CLC_FileCRCCheck::ReadFromBuffer(bf_read &buffer) {
  ...
  if (iCode == 0) {
    ...
    V_sprintf_safe(m_szFilename, "%s%c%s", g_MostCommonPrefixes[iCode - 1],
...
const char *CLC_SaveReplay::ToString() const {
  V_snprintf(s_text, sizeof(s_text),
             "%s: filename: %s, start byte: %i, post death record time: %f",
             GetName(), m_szFilename, m_nStartSendByte,
...
const char *Base_CmdKeyValues::ToString(void) const {
  Q_snprintf(s_text, sizeof(s_text), "%s: %s", GetName(),
             m_pKeyValues ? m_pKeyValues->GetName() : "<<null>>");
  return s_text;
}
...
bool SVC_ServerInfo::WriteToBuffer(bf_write &buffer) {
  ...
  buffer.WriteLong(
      0xffffffff); // Used to be client.dll CRC.  This was far before signed
                   // binaries, VAC, and cross-platform play
  buffer.WriteWord(m_nMaxClasses);
  buffer.WriteBytes(m_nMapMD5.bits,
                    MD5_DIGEST_LENGTH); // To prevent cheating with hacked maps
...
bool SVC_VoiceInit::ReadFromBuffer(bf_read &buffer) {
  ...
  if (nLegacyQuality == 255) {
    ...
    // v1 packet
    //
    // Hacky workaround for v1 packets not actually indicating if we were using
    // steam voice -- we've kept the steam voice separate convar that was in use
    // at the time as replicated&hidden, and if whatever network stream we're
    // interpreting sets it, lie about the subsequent voice init's codec &
    // sample rate.
    if (sv_use_steam_voice.GetBool()) {
      Msg("Legacy SVC_VoiceInit - got a set for sv_use_steam_voice convar, "
          "assuming Steam voice\n");
      V_strncpy(m_szVoiceCodec, "steam", sizeof(m_szVoiceCodec));
      // Legacy steam voice can always be parsed as auto sample rate.
      m_nSampleRate = 0;
    } else if (V_strncasecmp(m_szVoiceCodec, "vaudio_celt",
                             sizeof(m_szVoiceCodec)) == 0) {
...
const char *SVC_VoiceInit::ToString(void) const {
  Q_snprintf(s_text, sizeof(s_text), "%s: codec \"%s\", sample rate %i",
             GetName(), m_szVoiceCodec, m_nSampleRate);
  return s_text;
}
...
bool SVC_UserMessage::WriteToBuffer(bf_write &buffer) {
  ...
  buffer.WriteUBitLong(
      m_nLength, NETMSG_LENGTH_BITS); // max 256 * 8 bits, see MAX_USER_MSG_DATA
...
const char *MM_ClientInfo::ToString(void) const {
  Q_snprintf(s_text, sizeof(s_text), "Client Info: ID: %llu, Players: %d", m_id,
             m_cPlayers);
  return s_text;
}
...
const char *MM_JoinResponse::ToString(void) const {
  Q_snprintf(s_text, sizeof(s_text), "ID: %llu, Nonce: %llu, Flags: %u", m_id,
