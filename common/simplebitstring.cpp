...
//
//-----------------------------------------------------------------------------
void CSimpleBitString::AppendBits(uint64 u64Data,
                                  uint32 NumSignificantLowBitsOfData) {
  ...
  while (NumSignificantLowBitsOfData > 0) {
    ...
    if (NumSignificantLowBitsOfData < 64)
      u64Data &=
          ((1ULL << NumSignificantLowBitsOfData) - 1); // will fail for 64 bits
...
//
//-----------------------------------------------------------------------------
void CSimpleBitString::ReversiblyObfusticateBitsFromStart(
    uint NumBits, const uint8 *pObfusticationData,
  ...
  if (NumTailBits > 0) {
    *pBits ^= (*(pObfusticationData++) &
               (((1U << NumTailBits) - 1) << (8U - NumTailBits)));
  }
}
...
uint8 CSimpleBitString::GetByteChecksumFromStart(uint NumBits) const {
  ...
  if (NumTailBits > 0) {
    u8Checksum +=
        (*(pBits) & (((1U << NumTailBits) - 1) << (8U - NumTailBits)));
  }
...
uint64 CSimpleBitString::iterator::GetNextBits64(uint32 NumBitsToGet) {
  ...
  while (NumBitsToGet > 0) {
    ...
    uint8 BitsFromThisByte =
        (m_rSimpleBitString.m_vecU8[Idx] >>
         (NumAvailableBitsInThisByte - NumBitsToGetFromThisByte)) &
