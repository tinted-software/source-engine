...
//-----------------------------------------------------------------------------
bool ShouldLaunchAppViaSteam(const char *lpCmdLine,
                             const char *steamFilesystemDllName,
  ...
  // we have the steam filesystem, and no stdio filesystem, so we must need to
...
void LaunchSelfViaSteam(const char *params) {
  ...
  ::GetModuleFileName((HINSTANCE)GetModuleHandle(NULL), appPath,
                      sizeof(appPath));
  ...
  if (ERROR_SUCCESS ==
    ...
    dwType = REG_DWORD;
    int appID = -1;
    RegSetValueEx(hKey, "TempAppID", NULL, dwType, (LPBYTE)&appID,
                  sizeof(appID));
    RegCloseKey(hKey);
  }
  ...
  if (hwnd) {
    ...
    if (!steamExe[0]) {
      ...
      if (ERROR_SUCCESS ==
        ...
        RegQueryValueEx(hKey, "SteamExe", NULL, &dwType, (LPBYTE)steamExe,
                        &dwSize);
        RegCloseKey(hKey);
      }
    }
    ...
    if (!steamExe[0]) {
      // still no path, error
      ::MessageBox(NULL,
                   "Error running game: could not find steam.exe to launch",
