...
public:
  CJpegDestMgr(CFileWriter &refOutputFileWriter)
      : m_pOutputFileWriter(&refOutputFileWriter), m_pOutputBuffer(NULL) {
    Init();
  }

  CJpegDestMgr(CUtlBuffer &refOutputBuffer)
      : m_pOutputFileWriter(NULL), m_pOutputBuffer(&refOutputBuffer) {
    Init();
  }
  ...
  static void imp_init_dest(j_compress_ptr cinfo) {
    ...
    if (pInstance->m_pOutputBuffer)
      pInstance->m_pOutputBuffer->EnsureCapacity(cinfo->image_width *
                                                 cinfo->image_height * 3);

    const int k_cubMaxTempJpegBuffer = 1024 * 10;
    int cubBufferSize = MIN(cinfo->image_width * cinfo->image_height * 3,
                            k_cubMaxTempJpegBuffer);
  ...
  static boolean imp_empty_output_buffer(j_compress_ptr cinfo) {
    ...
    // Write the entire buffer, ignoring next_output_byte and free_in_buffer as
    they lie (see docs) if (  pInstance->m_pOutputFileWriter )
            pInstance->m_pOutputFileWriter->Write( pInstance->m_Buffer.Base(),
    pInstance->m_Buffer.Size() ); else pInstance->m_pOutputBuffer->Put(
    pInstance->m_Buffer.Base(), pInstance->m_Buffer.Size() );
  ...
  static void imp_term_destination(j_compress_ptr cinfo) {
    ...
    // next_output_byte and free_in_buffer don't lie here like in
    empty_output_buffer int cubToWrite = (byte*)pInstance->next_output_byte -
    (byte*)pInstance->m_Buffer.Base(); if ( cubToWrite > 0 )
    {
            if ( pInstance->m_pOutputFileWriter )
                    pInstance->m_pOutputFileWriter->Write(
    pInstance->m_Buffer.Base(), cubToWrite ); else
                    pInstance->m_pOutputBuffer->Put( pInstance->m_Buffer.Base(),
    cubToWrite );
    }
...
public:
  ...
  static boolean imp_fill_input_buffer(j_decompress_ptr cinfo) {
    // If this is hit it means we are reading a corrupt file.  We can't provide
    // more data as we provided all of it upfront already.
    return 0;
  }

  static void imp_skip_input_data(j_decompress_ptr cinfo, long num_bytes) {
    // The library is asking us to skip a chunk of data, usually exif header
    // data or such.  Need to actually do that.  The library tries to be robust
    // and skip obviously bad data on its own one byte at a time as well, but
    // faster here, and safer as the library can't always do it correctly if we
    // fail these calls.
  ...
  static boolean imp_resync_to_restart(j_decompress_ptr cinfo, int desired) {
    // This happens if the library fails to find a resync marker where expected,
    // we'll then use it's default handler.  This handler will skip ahead trying
    // to find the next point, we could maybe do better going backwards as we
    // have the full buffer, but that's lots more work and this will only get
    // hit on a partially corrupt image anyway.
...
//-----------------------------------------------------------------------------
bool GetJpegDimensions(const byte *pubData, int cubData, uint32 &width,
                       uint32 &height) {
...
//-----------------------------------------------------------------------------
// Purpose: takes the contents of a .jpg file and turns it into RGB data,
// returning the width and height and optionally the number of bytes used
//-----------------------------------------------------------------------------
bool ConvertJpegToRGB(const byte *pubData, int cubData, CUtlVector<byte> &buf,
                      int &width, int &height, int *pcubUsed) {
  CJpegSourceMgr sourceMgr;
  bool bRet = sourceMgr.Init(pubData, cubData);
  if (!bRet)
    return false;
  ...
  // attributes of the cdnfile ready to be read out of the decompress struct.
  ...
  if (pcubUsed)
    *pcubUsed = (int)((byte *)sourceMgr.next_input_byte -
                      (byte *)sourceMgr.m_Data.Base());
...
//-----------------------------------------------------------------------------
void ConvertRGBToRGBAImage(CUtlVector<unsigned char> &srcData, int srcWidth,
                           int srcHeight, byte *destData, int destWidth,
                           int destHeight) {
  int destPixelSize = 4;
...
//-----------------------------------------------------------------------------
bool ConvertJpegToRawInternal(const byte *pubJpegData, int cubJpegData,
                              CUtlBuffer &bufOutput, int &width, int &height,
                              int *pcubUsed, bool bMakeRGBA) {
  // Temporary buffer to decompress Jpeg into as RGB, since libjpeg doesn't do
  // RGBA
  ...
  bool bConverted = false;
  {
    ...
    bConverted = ConvertJpegToRGB(pubJpegData, cubJpegData, vecRGB, width,
                                  height, pcubUsed);
  }
  ...
  if (bConverted) {
    ...
    if (bMakeRGBA) {
      ...
      ConvertRGBToRGBAImage(vecRGB, width, height, (byte *)bufOutput.Base(),
                            width, height);
    } else {
...
// Purpose: Converts raw Jpeg file data and converts to RGBA image buffer
//-----------------------------------------------------------------------------
bool ConvertJpegToRGBA(const byte *pubJpegData, int cubJpegData,
                       CUtlBuffer &bufOutput, int &width, int &height,
                       int *pcubUsed) {
  return ConvertJpegToRawInternal(pubJpegData, cubJpegData, bufOutput, width,
                                  height, pcubUsed, true);
}

//-----------------------------------------------------------------------------
// Purpose: Converts raw Jpeg file data and converts to RGB image buffer
//-----------------------------------------------------------------------------
bool ConvertJpegToRGB(const byte *pubJpegData, int cubJpegData,
                      CUtlBuffer &bufOutput, int &width, int &height,
                      int *pcubUsed) {
  return ConvertJpegToRawInternal(pubJpegData, cubJpegData, bufOutput, width,
                                  height, pcubUsed, false);
}
...
//-----------------------------------------------------------------------------
bool ConvertRGBToJpegInternal(CJpegDestMgr &destMgr, int quality, int width,
                              int height, CUtlBuffer &bufRGB) {
...
// a good value.
//-----------------------------------------------------------------------------
bool ConvertRGBToJpeg(const char *pchFileOut, int quality, int width,
                      int height, CUtlBuffer &bufRGB) {
  /*
...
// a good value.
//-----------------------------------------------------------------------------
bool ConvertRGBToJpeg(CUtlBuffer &bufOutput, int quality, int width, int height,
                      CUtlBuffer &bufRGB) {
