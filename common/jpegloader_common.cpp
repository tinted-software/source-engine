...
//
// Params: bufRGBA should contain the current image, nWidth and nHeight should
// describe the current images dimensions.  nNewWidth and nNewHeight should be
// the new target size, one, but not both, of these may be -1 which will
// indicate to preserve aspect ratio and size that dimension to match the aspect
// ratio adjustment applied to the other dimension which must then be specified
// explicitly.  nNewWidth and nNewHeight may be modified by the function and
// will specify the final width/height after the function returns succesfully.
// If both nNewWidth and nNewHeight are specified the content will be scaled
// without changing the aspect ratio and black letterboxing will be added if
// appropriate
//-----------------------------------------------------------------------------
bool BResizeImageInternal(CUtlBuffer &bufImage, int nWidth, int nHeight,
  ...
  // Must specify at least one, then we'll compute the other to preserve aspect
  // ratio if it's set to -1
  ...
  bufImageOut.SeekPut(CUtlBuffer::SEEK_HEAD,
                      nNewWidth * nNewHeight * bytesPerPixel);
  ...
  if (nPaddingLeft > 0 || nPaddingRight > 0 || nPaddingTop > 0 ||
      nPaddingBottom > 0) {
  ...
  for (int yNew = 0; yNew < nNewContentHeight; ++yNew) {
    ...
    for (int xNew = 0; xNew < nNewContentWidth; ++xNew) {
      ...
      byte alpha = 0;
      if (bytesPerPixel == 4) {
        alpha = (byte)(pSrcBits[aOffset + 3] * (1 - xDiff) * (1 - yDiff) +
      ...
      int targetOffset =
          (nPaddingLeft + xNew + ((nPaddingTop + yNew) * nTargetStride)) *
          bytesPerPixel;
...
bool BResizeImageRGB(CUtlBuffer &bufRGB, int nWidth, int nHeight,
                     int &nNewWidth, int &nNewHeight) {
  return BResizeImageInternal(bufRGB, nWidth, nHeight, nNewWidth, nNewHeight,
                              false);
}

// Resize an RGBA image using linear interpolation
bool BResizeImageRGBA(CUtlBuffer &bufRGB, int nWidth, int nHeight,
                      int &nNewWidth, int &nNewHeight) {
  return BResizeImageInternal(bufRGB, nWidth, nHeight, nNewWidth, nNewHeight,
                              true);
}
...
bool BConvertRGBAToRGB(CUtlBuffer &bufRGBA, int nWidth, int nHeight,
                       Color colorBG) {
  CUtlBuffer bufRGB;
  ...
  for (int y = 0; y < nHeight; ++y) {
    for (int x = 0; x < nWidth; ++x) {
      ...
      *(pBitsOut + 1) =
          *(pBits + 1) * fOpacity + (1.0 - fOpacity) * colorBG.g();
      *(pBitsOut + 2) =
          *(pBits + 2) * fOpacity + (1.0 - fOpacity) * colorBG.b();
