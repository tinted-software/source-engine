/*
     ...
     Copyright:  (c) 2007 (c) 1985-2001 by Apple Computer, Inc., all rights
   reserved.
...
/*
   ...
   scroll the menu contents. This API is available in CarbonLib 1.5 and later,
   and in Mac OS X 10.1 and later. ScrollMenuImage properly supports scrolling
   the alpha channel in the menu's image data. Use of QuickDraw's ScrollRect API
   to scroll the menu contents will result in the alpha channel being set to
...
/*
   A MenuBarHandle is a handle to a MenuBarHeader. An instance of this structure
   is returned by the GetMenuBar and GetNewMBar APIs. It is typedef'd to a plain
   Handle to retain source compatibility with previous versions of this header
   file.
*/
...
inline DEFINE_API_C(void)
    InvokeMenuDefUPP(short message, MenuRef theMenu, Rect *menuRect,
                     Point hitPt, short *whichItem, MenuDefUPP userUPP) {
...
#ifdef __cplusplus
inline DEFINE_API_C(MenuBarDefUPP)
    NewMenuBarDefUPP(MenuBarDefProcPtr userRoutine) {
...
CopyMenuItemData(MenuRef inMenu, /* can be NULL */
                 MenuItemID inItem, Boolean inIsCommandID,
 ...
 */
EXTERN_API(OSStatus)
SetMenuItemData(MenuRef inMenu, MenuItemID inItem, Boolean inIsCommandID,
                const MenuItemDataRec *inData);

/*--------------------------------------------------------------------------------------*/
/*  o Dynamic menu item support (CarbonLib 1.1 and Carbon for Mac OS X, and
 * later)      */
/*                                                                                      */
 ...
 */
EXTERN_API(OSStatus)
InvalidateMenuItems(MenuRef inMenu, MenuItemIndex inFirstItem,
                    ItemCount inNumItems);
...
enum {
  ...
  gestaltContextualMenuHasAttributeAndModifierKeys =
      2, /* Contextual Menu Manager supports keyContextualMenuAttributes and
...
/*                                                                                      */
/*  For Mac OS X 10.1, we support a new type of Contextual Menu Plugin: the
 * CFPlugIn    */
