...
enum {
  ...
  cmParametricType1 =
      1, /* Y = (aX+b)^gamma     [X>=-b/a],  Y = 0    [X<-b/a] */
  cmParametricType2 =
      2, /* Y = (aX+b)^gamma + c [X>=-b/a],  Y = c    [X<-b/a] */
...
struct CMMultiFunctLutType {
  OSType typeDescriptor; /* 'mAB ' = cmSigMultiFunctA2BType or 'mBA ' =
  ...
  UInt32 offsetCLUT;     /* offset to multi-dimensional LUT of type
...
struct CMMultiFunctCLUTType {
  UInt8 gridPoints[16]; /* grigpoints for each input channel dimension
                           (remaining are 0) */
...
struct CMNativeDisplayInfo {
  ...
  CMFixedXYColor
      redPhosphor; /* Phosphors - native cromaticity values of the display  */
  ...
  char gammaData[1];      /* variable size, determined by
                             channels*entryCount*entrySize */
};
