/*
     ...
     Copyright:  (c) 2007 (c) 1999-2002 by Apple Computer, Inc., all rights
   reserved.
 ...
 * version 10.1 and later Mac OS X:         in version 10.1 and later
 */
...
/*
    ...
    // NOTE: As of Mac OS X 10.1, mouse events carry more information which
   allow you
    //       to do less work and gain accuracy of hit testing. First, there is
   the
    //       kEventParamWindowRef parameter. This parameter tells you what
   window the
    //       mouse click/move/etc occurred over. In mouse dragged events, this
   is the
    //       window the mouse went down in, NOT the window the mouse is
   currently over.
    //       Next, there is the kEventParamWindowMouseLocation parameter. This
   is the
    //       window-relative position of the mouse in the window given in the
    //       kEventParamWindowRef parameter. 0, 0 is at the top left of the
   structure
    //       of the window.

    kEventMouseDown
        -->     kEventParamMouseLocation        typeHIPoint
        -->     kEventParamWindowRef            typeWindowRef (Mac OS X 10.1 or
   later)
        -->     kEventParamWindowMouseLocation  typeHIPoint (Mac OS X 10.1 or
   later)
...
/*
    ...
    kEventWindowConstrain
        ...
        --> kEventParamAttributes           typeUInt32 (optional; available in
   Jaguar)
        --> kEventParamWindowRegionCode     typeWindowRegionCode (optional;
   available in Jaguar)
        --> kEventParamRgnHandle            typeQDRgnHandle (optional; available
   in Jaguar)
...
/*
   ...
   CarbonLib 1.3.1 and later)
    ...
    kEventMenuPopulate (on Mac OS X 10.1 and later, and CarbonLib 1.5 and later)
        -->     kEventParamDirectObject             typeMenuRef
        -->     kEventParamMenuContext              typeUInt32
        -->     kEventParamMenuCommand              typeMenuCommand (Jaguar and
   CarbonLib 1.6, for kMenuContextCommandIDSearch)
...
/*
    ...
    kEventCommandProcess
        ...
        -->     kEventParamMenuContext      typeUInt32 (optional, in Jaguar and
   CarbonLib 1.6)
...
/*
   ...
   present)
        -->     kEventParamRgnHandle        typeQDRgnHandle
                    Sent only on Jaguar and in certain situations in compositing
   mode (when view is visible, etc.) Allows you to constrain your drawing to the
   region the Toolbox has clipped you to.
        -->     kEventParamCGContextRef     typeCGContextRef
                    Sent to your control when in compositing mode. It is already
                    transformed and clipped appropriately. You should use this
   instead of creating your own CGContext. This will be extremely important for
   printing, etc.
   ...
   present)
    ...
    kEventControlGetNextFocusCandidate
        ...
        -->     kEventParamStartControl         typeControlRef
                    (find the first/last subcontrol of yourself if
   kEventParamStartControl is not present)
...
/*
    ...
    kEventAccessibleGetChildAtPoint
        -->     kEventParamAccessibleObject             typeCFTypeRef with an
   AXUIElementRef
        -->     kEventParamMouseLocation                typeHIPoint
        <--     kEventParamAccessibleChild              typeCFTypeRef with an
   AXUIElementRef
...
AXUIElementCreateWithHIObjectAndIdentifier(HIObjectRef inHIObject,
                                           UInt64 inIdentifier);
 ...
 */
EXTERN_API_C(void)
AXNotificationHIObjectNotify(CFStringRef inNotification, HIObjectRef inHIObject,
                             UInt64 inIdentifier);
...
enum {
  ...
  kEventParamResult = FOUR_CHAR_CODE(
      'ansr'), /* any type - depends on event like direct object*/
...
enum {
  ...
  kEventParamWindowMouseLocation =
      FOUR_CHAR_CODE('wmou'), /* typeHIPoint (Mac OS X 10.1 or later)*/
  ...
  kEventParamMouseTrackingRef =
      FOUR_CHAR_CODE('mtrf'),                   /* typeMouseTrackingRef*/
...
enum {
  ...
  kEventParamTextInputGlyphInfoArray =
      FOUR_CHAR_CODE('glph') /*    typeGlyphInfoArray*/
};
...
enum {
  ...
  kEventParamControlPreviousPart =
      FOUR_CHAR_CODE('copc'), /* typeControlPartCode*/
  kEventParamControlCurrentPart =
      FOUR_CHAR_CODE('cnpc'),                          /* typeControlPartCode*/
...
enum {
  ...
  kEventParamToolbarItemIdentifier =
      FOUR_CHAR_CODE('tbii'), /* typeCFStringRef*/
...
enum {
  kEventParamAccessibleObject =
      FOUR_CHAR_CODE('aobj'), /* typeCFTypeRef with an AXUIElementRef*/
  kEventParamAccessibleChild =
      FOUR_CHAR_CODE('achl'), /* typeCFTypeRef with an AXUIElementRef*/
  kEventParamAccessibleAttributeName =
      FOUR_CHAR_CODE('atnm'), /* typeCFStringRef*/
  kEventParamAccessibleAttributeNames =
      FOUR_CHAR_CODE('atns'), /* typeCFMutableArrayRef of CFStringRefs*/
  kEventParamAccessibleAttributeValue = FOUR_CHAR_CODE('atvl'), /* variable*/
  kEventParamAccessibleAttributeSettable =
      FOUR_CHAR_CODE('atst'),                               /* typeBoolean*/
  kEventParamAccessibleActionName = FOUR_CHAR_CODE('acnm'), /* typeCFStringRef*/
  kEventParamAccessibleActionNames =
      FOUR_CHAR_CODE('acns'), /* typeCFMutableArrayRef of CFStringRefs*/
  kEventParamAccessibleActionDescription =
      FOUR_CHAR_CODE('acds') /* typeCFMutableStringRef*/
};
 ...
 */
EXTERN_API_C(OSStatus)
CreateMouseTrackingRegion(WindowRef inWindow, RgnHandle inRegion,
                          RgnHandle inClip, /* can be NULL */
                          MouseTrackingOptions inOptions,
                          MouseTrackingRegionID inID, void *inRefCon,
 ...
 */
EXTERN_API_C(OSStatus)
ChangeMouseTrackingRegion(MouseTrackingRef inMouseRef, RgnHandle inRegion,
                          RgnHandle inClip); /* can be NULL */
 ...
 */
EXTERN_API_C(OSStatus)
GetMouseTrackingRegionID(MouseTrackingRef inMouseRef,
 ...
 */
EXTERN_API_C(OSStatus)
MoveMouseTrackingRegion(MouseTrackingRef inMouseRef, SInt16 deltaH,
                        SInt16 deltaV, RgnHandle inClip); /* can be NULL */
 ...
 */
EXTERN_API_C(OSStatus)
ClipWindowMouseTrackingRegions(WindowRef inWindow, OSType inSignature,
                               RgnHandle inClip); /* can be NULL */
 ...
 */
EXTERN_API_C(OSStatus)
MoveWindowMouseTrackingRegions(WindowRef inWindow, OSType inSignature,
                               SInt16 deltaH, SInt16 deltaV,
                               RgnHandle inClip); /* can be NULL */
 ...
 */
EXTERN_API_C(OSStatus)
SetWindowMouseTrackingRegionsEnabled(WindowRef inWindow, OSType inSignature,
                                     Boolean inEnabled);
