...
struct IDHDeviceStatus {
  ...
  UInt8
      inputFormat; /* Expected STYPE of data from device, if version >= 0x200*/
  UInt32 outputFormats; /* Bitmask for supported STYPE values, if version >=
                           0x200*/
};
 ...
 */
EXTERN_API(ComponentResult)
IDHSetFormat(ComponentInstance idh, UInt32 format)
 ...
 */
EXTERN_API(ComponentResult)
IDHGetFormat(ComponentInstance idh, UInt32 *format)
...
#ifdef __cplusplus
inline DEFINE_API_C(IDHNotificationUPP)
    NewIDHNotificationUPP(IDHNotificationProcPtr userRoutine) {
  return userRoutine;
}
#else
