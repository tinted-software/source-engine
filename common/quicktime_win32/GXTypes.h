/*
     ...
     Copyright:  (c) 2007 (c) 1994-1999 by Apple Computer, Inc., all rights
   reserved.
...
enum {
  ...
  gxSingleComponentTransfer = 0x0004 /* duplicate gxTransferComponent[0] for all
                                        components in transfer */
};
...
enum {
  gxFontListFlatten = 0x01, /* if set, generate a gxTag containing list of each
                               gxFont referenced */
  gxFontGlyphsFlatten = 0x02, /* if set, generate a gxTag containing the list of
                                 glyphs referenced inside the gxFont */
  gxFontVariationsFlatten =
      0x04, /* if set, append the gxTag with separate [variation] coordinates */
  gxBitmapAliasFlatten = 0x08 /* if set, open bitmap alias files and flatten out
                                 their image data */
};
...
/* If tracking is not desired, specify the following value in the track field in
   the gxRunControls record (note that a track of 0 does *not* mean to turn
...
/* gxJustificationPriority defines the point during the justification process at
   which a glyph will begin to receive deltas before and after itself. */
enum {
  gxKashidaPriority = 0,
...
/* gxJustificationFlags are used to control which fields of a gxWidthDeltaRecord
   are to be overridden and which are not if a gxPriorityJustificationOverride
   or gxGlyphJustificationOverride (qq.v.) is specified. */
enum {
...
/* A gxPriorityJustificationOverride contains an array of WidthDeltaRecords, one
   for each gxJustificationPriority. */
...
struct gxFlatFontListItem {
  ...
  unsigned short variationCount; /* number of bitsVariationPairs that follow the
                                    (optional) glyphBits */
};
...
struct gxPatternSynonym {
  ...
  gxPoint anchorPoint; /* gxPoint with with respect to which pattern position is
                          calculated */
};
