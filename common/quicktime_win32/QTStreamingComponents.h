...
enum {
  ...
  kQTSInfo_SGChannelSettings =
      FOUR_CHAR_CODE('sesg'), /* QTSSGChannelSettingsParams */
...
enum { kSHNumExtendedDataLongs = 10 };
...
enum {
  ...
  kRTPPayloadLossRecoveryTag =
      FOUR_CHAR_CODE('loss'), /* 0-255, 0 can't handle any loss, 128 can handle
                                 50% packet loss*/
  kRTPPayloadConformanceTag =
      FOUR_CHAR_CODE('conf') /* more than one of these can be present*/
};
...
enum { kRTPRssmMoreInitParamsVersion1 = 1 };

/* get/set info selectors*/
enum { kRTPRssmInfo_MoreInitParams = FOUR_CHAR_CODE('rrmi') };
...
enum { kRTPMediaPacketizerInfoRezType = FOUR_CHAR_CODE('pcki') };
...
enum {
  ...
  kRTPInfo_FormatString = FOUR_CHAR_CODE(
      'fmtp') /* char **, caller allocates ptr, callee disposes */
};
...
#ifdef __cplusplus
inline DEFINE_API_C(RTPMPDataReleaseUPP)
    NewRTPMPDataReleaseUPP(RTPMPDataReleaseProcPtr userRoutine) {
...
#ifdef __cplusplus
inline DEFINE_API_C(RTPPBCallbackUPP)
    NewRTPPBCallbackUPP(RTPPBCallbackProcPtr userRoutine) {
