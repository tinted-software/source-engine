/*
     ...
     Copyright:  (c) 2007 (c) 1997-2002 by Apple Computer, Inc., all rights
   reserved
...
/* StandardAlert uses zero to specify the default position. Other calls use zero
 * to     */
...
enum {
  /* Region values to pass into GetWindowRegion & GetWindowBounds. All regions
     are reported in global coordinates. */
  ...
  kWindowOpaqueRgn = 35, /* Mac OS X: Area of window considered to be opaque.
...
#ifdef __cplusplus
inline DEFINE_API_C(WindowDefUPP)
    NewWindowDefUPP(WindowDefProcPtr userRoutine) {
...
enum {
  uppWindowPaintProcInfo = 0x0003FFF0
    ...
    */
#ifdef __cplusplus
inline DEFINE_API_C(WindowPaintUPP)
    NewWindowPaintUPP(WindowPaintProcPtr userRoutine) {
...
inline DEFINE_API_C(long)
    InvokeWindowDefUPP(short varCode, WindowRef window, short message,
...
/*--------------------------------------------------------------------------------------*/
/* o Window Definition Spec.  Used in Carbon to specify the code that defines a
 * window. */
/*--------------------------------------------------------------------------------------*/
enum {
 ...
 *  Availability:
 *    Non-Carbon CFM:   not available
 *    CarbonLib:        not available in CarbonLib 1.x, is available on Mac OS X
 * version 10.0 and later Mac OS X:         in version 10.0 and later
 */
EXTERN_API_C(OSStatus)
 ...
 *    Non-Carbon CFM:   not available
 *    CarbonLib:        not available in CarbonLib 1.x, is available on Mac OS X
 * version 10.2 and later Mac OS X:         in version 10.2 and later
 */
EXTERN_API_C(OSStatus)
 ...
 */
EXTERN_API_C(OSStatus)
SetWindowResizeLimits(WindowRef inWindow,
 ...
 */
EXTERN_API_C(OSStatus)
GetWindowResizeLimits(WindowRef inWindow,
 ...
 */
EXTERN_API(Boolean)
ResizeWindow(WindowRef inWindow, Point inStartPoint,
 ...
 */
EXTERN_API(Boolean)
IsWindowInStandardState(WindowRef inWindow,
 ...
 */
EXTERN_API(OSStatus)
ZoomWindowIdeal(WindowRef inWindow, WindowPartCode inPartCode,
                Point *ioIdealSize);
...
/*
    ...
    Drawers are supported in Carbon by Jaguar and later. To create and use a
   drawer:

        -   Create a window using kDrawerWindowClass. You may optionally set the
   background brush of the window to kThemeBrushDrawerBackground using the
   SetThemeWindowBackground function.

        -   Either install the standard window event handler on the drawer
   window, or use your own Carbon event or WaitNextEvent code to handle user
   interaction with the drawer. If you do not use the standard window event
   handler, you should call ResizeWindow in response to clicks on the grow
   region of the drawer if you want the drawer to be resizable.

        -   Install a handler on the drawer window for the
   kEventWindowDrawContent event. You must handle this event and draw your
   drawer's content so that the content will be visible when the drawer is slid
   out from underneath the parent window.

        -   Set the drawer's parent window with the SetDrawerParent function.
   The parent is the window on which the drawer will open.

        -   Optionally, install a Carbon event handler on the drawer or the
   drawer's parent window for the
   kEventWindowDrawerOpening/Opened/Closing/Closed events, to be notified when
   the drawer has started or finished opening or closing.

        -   Optionally, set the drawer's preferred edge on the parent window
   with the SetDrawerPreferredEdge function. If you do not set a preferred edge,
   the drawer opens on the parent's left side on a left-to-right system, or on
   the parent's right side on a right-to-left system.

        -   Optionally, Set the drawer's offsets with the SetDrawerOffsets
   function. The offsets control the amount of inset between the edge of the
   parent's content area and the edge of the drawer's structure. If you do not
   set any offsets, the drawer's edges are flush with the edges of the parent's
   content.

        -   Optionally, set the drawer's minimum and maximum sizes with the
   SetWindowResizeLimits function. Or, install a Carbon event handler on the
   drawer to handle the kEventWindow- GetMinimumSize and
   kEventWindowGetMaximumSize events. The drawer's minimum and maximum sizes
   control how small or large it will resize together with the parent. If you do
   not set resize limits, the drawer will be capable of resizing to default
   small and large limits.

        -   Call ToggleDrawer to open or close the drawer, or use OpenDrawer or
   CloseDrawer if you require more control over how the drawer opens or closes.
*/
 ...
 */
EXTERN_API_C(OSStatus)
SetDrawerOffsets(WindowRef inDrawerWindow, float inLeadingOffset,
 ...
 */
EXTERN_API_C(OSStatus)
GetDrawerOffsets(WindowRef inDrawerWindow,
