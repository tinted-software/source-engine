...
enum {
  ...
  codecImageBufferMemoryFlagsValid =
      1L << 6, /* set by ImageCodecNewImageBufferMemory/NewImageGWorld to
                  indicate that it set the AGP/PCI flags (supported in
                  QuickTime 6.0 and later) */
  codecDrawsHigherQualityScaled =
      1L << 7, /* codec will draw higher-quality image if it performs scaling
                  (eg, wipe effect with border) */
  codecSupportsOutOfOrderDisplayTimes =
      1L << 8, /* codec supports frames queued in one order for display in a
                  different order, eg, IPB content */
  codecSupportsScheduledBackwardsPlaybackWithDifferenceFrames =
      1L << 9 /* codec can use additional buffers to minimise redecoding during
                 backwards playback */
};
...
struct CodecDecompressParams {
  ...
  Boolean pad3;
  UInt8 destinationBufferMemoryPreference; /* a codec's PreDecompress/Preflight
                                              call can set this to express a
                                              preference about what kind of
                                              memory its destination buffer
                                              should go into.  no guarantees.*/
  UInt8
      codecBufferMemoryPreference; /* may indicate preferred kind of memory that
                                      NewImageGWorld/NewImageBufferMemory should
                                      create its buffer in, if applicable.*/
  Boolean
      onlyUseCodecIfItIsInUserPreferredCodecList; /* set to prevent this codec
                                                     from being used unless it
                                                     is in the
                                                     userPreferredCodec list*/
  ...
  UInt8 deinterlaceAnswer;  /* codec should set this in PreDecompress/Preflight
                               if it will satisfy the deinterlaceRequest */
...
enum { kICMNoDeinterlacing = 0, kICMDeinterlaceFields = 1 };
...
#ifdef __cplusplus
inline DEFINE_API_C(ImageCodecTimeTriggerUPP)
    NewImageCodecTimeTriggerUPP(ImageCodecTimeTriggerProcPtr userRoutine) {
...
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
inline DEFINE_API_C(void) DisposeImageCodecDrawBandCompleteUPP(
...
struct ImageSubCodecDecompressCapabilities {
  ...
  /* The following fields only exist for QuickTime 7.0 and greater */
  Boolean
      subCodecIsMultiBufferAware; /* set if subcodec always draws using
                                     ImageSubCodecDecompressRecord.baseAddr/rowBytes
                                     passed to ImageCodecDrawBand, and always
                                     writes every pixel in the buffer*/
  ...
  UInt8 pad5[2];
  Boolean
      subCodecSupportsDrawInDecodeOrder;   /* indicates that it's okay for the
                                              subcodec to get a single DrawBand
                                              call for each frame in decode order
                                              even when frames need reordering.
                                              (This will only happen when other
                                              circumstances allow it.)*/
  Boolean subCodecSupportsDecodeSmoothing; /* Frame-reordering subcodecs should
                                              set this to indicate that they can
                                              safely decode a non-droppable
                                              frame before drawing the previous
                                              non-droppable frame.  This enables
                                              smoother playback in GWorlds.*/
  UInt8 pad6[4];
};
...
enum {
  codecSuggestedBufferSentinel =
      FOUR_CHAR_CODE('sent') /* codec public resource containing suggested data
                                pattern to put past end of data buffer */
};
...
enum {
  ...
  kTransitionMajorClass =
      FOUR_CHAR_CODE('tran'), /* multisource morph effects */
...
/*
   Effects can define a number of "preset" values which will be presented to the
   user in a simplified UI.  Each preset is an atom within the parameter
   description list and must have an atom ID from 1 going up sequentially.
   Inside of this atom are three other atoms containing: 1) the name of the
   preset as a Pascal string 2) a preview picture for the preset, 86 x 64 pixels
   in size 3) the ENTIRE set of parameter values needed to create a sample of
   this preset.
*/
enum {
...
enum { kParameterProperty = FOUR_CHAR_CODE('prop') };

struct ParameterProperty {
  OSType propertyClass; /* class to set for this property (0 for default which
...
enum {
  ...
  kDisableWhenGreaterThan =
      (0x00000030 + kDisableControl), /* flags valid for controls*/
...
enum {
   ...
   */
  kPixelAspectRatioImageDescriptionExtension = FOUR_CHAR_CODE(
      'pasp'), /* big-endian PixelAspectRatioImageDescriptionExtension */

  /*
   * Image description extension describing the clean aperture.
   */
  kCleanApertureImageDescriptionExtension = FOUR_CHAR_CODE(
      'clap'), /* big-endian CleanApertureImageDescriptionExtension */
   ...
   */
  kQTRowBytesImageDescriptionExtension =
      FOUR_CHAR_CODE('rowb') /* big-endian SInt32 */
};
...
struct CleanApertureImageDescriptionExtension {
  ...
  SInt32 horizOffN; /* horizontal offset of clean aperture center minus
                       (width-1)/2, numerator, denominator */
  UInt32 horizOffD;
  SInt32 vertOffN; /* vertical offset of clean aperture center minus
...
#ifdef __cplusplus
inline DEFINE_API_C(ImageCodecMPDrawBandUPP)
    NewImageCodecMPDrawBandUPP(ImageCodecMPDrawBandProcPtr userRoutine) {
 ...
 */
EXTERN_API(ComponentResult)
ImageCodecLockBits(ComponentInstance ci, CGrafPtr port)
 ...
 */
EXTERN_API(ComponentResult)
ImageCodecUnlockBits(ComponentInstance ci, CGrafPtr port)
 ...
 */
EXTERN_API(ComponentResult)
ImageCodecGetDITLForSize(ComponentInstance ci, Handle *ditl,
                         Point *requestedSize)
 ...
 */
EXTERN_API(ComponentResult)
ImageCodecDITLEvent(ComponentInstance ci, DialogRef d, short itemOffset,
                    const EventRecord *theEvent, short *itemHit,
                    Boolean *handled)
    FIVEWORDINLINE(0x2F3C, 0x0012, 0x0030, 0x7000, 0xA82A);
 ...
 */
EXTERN_API(ComponentResult)
ImageCodecDITLItem(ComponentInstance ci, DialogRef d, short itemOffset,
                   short itemNum)
 ...
 */
EXTERN_API(ComponentResult)
ImageCodecGetPreferredChunkSizeAndAlignment(ComponentInstance ci,
                                            ImageDescriptionHandle desc,
                                            Fixed frameRate, UInt32 dataRate,
                                            long *bytesPerChunk,
                                            long *alignment)
...
EXTERN_API(ComponentResult)
ImageCodecPrepareToCompressFrames(
    ComponentInstance ci, ICMCompressorSessionRef session,
 ...
 */
EXTERN_API(ComponentResult)
ImageCodecEncodeFrame(ComponentInstance ci,
                      ICMCompressorSourceFrameRef sourceFrame, UInt32 flags)
 ...
 */
EXTERN_API(ComponentResult)
ImageCodecCompleteFrame(ComponentInstance ci,
                        ICMCompressorSourceFrameRef sourceFrame, UInt32 flags)
 ...
 */
EXTERN_API(ComponentResult)
ImageCodecBeginPass(ComponentInstance ci,
                    ICMCompressionPassModeFlags passModeFlags, UInt32 flags,
 ...
 */
EXTERN_API(ComponentResult)
ImageCodecEndPass(ComponentInstance ci)
    FIVEWORDINLINE(0x2F3C, 0x0000, 0x003B, 0x7000, 0xA82A);
...
EXTERN_API(ComponentResult)
ImageCodecProcessBetweenPasses(
    ComponentInstance ci, ICMMultiPassStorageRef multiPassStorage,
 ...
 */
EXTERN_API(ComponentResult)
ImageCodecDecodeBand(ComponentInstance ci, ImageSubCodecDecompressRecord *drp,
                     unsigned long flags)
...
enum {
  ...
  kICMImageCompressorPropertyID_EnforcedEncodedWidth =
      FOUR_CHAR_CODE('enwi'), /* SInt32, Read/Sometimes Listen */
  ...
  kICMImageCompressorPropertyID_EnforcedCleanAperture =
      FOUR_CHAR_CODE('encl'), /* CleanApertureImageDescriptionExtension,
                                 Read/Sometimes Listen */

  /*
   * The pixel aspect ratio enforced for compressed frames.
   */
  kICMImageCompressorPropertyID_EnforcedPixelAspectRatio =
      FOUR_CHAR_CODE('enpa'), /* PixelAspectRatioImageDescriptionExtension,
                                 Read/Sometimes Listen */

  /*
   * The number and order of fields enforced for compressed frames.
   */
  kICMImageCompressorPropertyID_EnforcedFieldInfo = FOUR_CHAR_CODE(
      'enfi') /* FieldInfoImageDescriptionExtension2, Read/Sometimes Listen */
};
...
enum {
  ...
  kDVCompressorPropertyID_ProgressiveScan =
      FOUR_CHAR_CODE('prog'), /* Boolean, Read/Write */
  ...
  kDVCompressorPropertyID_AspectRatio16x9 =
      FOUR_CHAR_CODE('16x9') /* Boolean, Read/Write */
};
