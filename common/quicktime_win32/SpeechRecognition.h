...
enum {
  ...
  kSRCantSetDuringRecognition =
      -5113, /* the property can't be set while recognition is in progress -- do
                before or between utterances. */
  ...
  kSRSubItemNotFound = -5123, /* returned when accessing a non-existent sub item
                                 of a container */
  ...
  kSRCantReadLanguageObject =
      -5133, /* An error while trying to create new Language object from file or
                pointer -- possibly bad format */
...
#ifdef __cplusplus
inline DEFINE_API_C(SRCallBackUPP)
    NewSRCallBackUPP(SRCallBackProcPtr userRoutine) {
...
enum {
  ...
  kSRCleanupOnClientExit = FOUR_CHAR_CODE(
      'clup') /* Boolean: Default is true. The rec system and everything it owns
                 is disposed when the client application quits */
};
...
enum {
  ...
  kSRAutoFinishingParam =
      FOUR_CHAR_CODE('afin'), /* automatic finishing applied on LM for search */
  kSRForegroundOnly =
  ...
  kSRBlockModally = FOUR_CHAR_CODE(
      'blmd'), /* Boolean. Default is false. When true, this client's LM is only
                  active LM; all other LMs are inactive. Be nice, don't be modal
                  for long periods! */
  ...
  kSRSoundInVolume =
      FOUR_CHAR_CODE('volu'), /* short in [0..100] log scaled sound input power.
...
enum {
  ...
  kSRRejectable = FOUR_CHAR_CODE(
      'rjbl'), /* Boolean -- true if SRLanguageObject is rejectable (Recognition
                  System's kSRRejectedWord */
