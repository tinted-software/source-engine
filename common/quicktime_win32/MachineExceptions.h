...
enum {
  ...
  kStackOverflowException =
      14, /* Optional, may be implemented as kAccessException on some systems.*/
  kTaskTerminationException = 15, /* Obsolete*/
  kTaskCreationException = 16,    /* Obsolete*/
  kDataAlignmentException = 17 /* May occur when a task is in little endian mode
                                  or created with kMPTaskTakesAllExceptions.*/
};
...
#ifdef __cplusplus
inline DEFINE_API_C(ExceptionHandlerUPP)
    NewExceptionHandlerUPP(ExceptionHandlerProcPtr userRoutine) {
  return userRoutine;
}
#else
