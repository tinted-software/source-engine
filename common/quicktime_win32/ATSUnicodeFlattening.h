...
enum { kATSUDataStreamUnicodeStyledText = FOUR_CHAR_CODE('ustl') };
...
enum { kATSUFlattenOptionNoOptionsMask = 0x00000000 };
...
enum { kATSUUnFlattenOptionNoOptionsMask = 0x00000000 };
...
/*
   ...
   object covers. This structure is utilized by
   ATSUUnflattenStyleRunsFromStream() to return the style run info to the
   caller.
*/
...
struct ATSFlatDataMainHeaderBlock {
  ...
  /* offset from the beginning of the stream to the flattened text layout
   * data.*/
  ...
  /* This can be set to 0 if there is no flattened style run data in the
   * stream*/
...
/* ------------------ */
/*
   The Block 2 Structures are not currently used by any of ATSUI's internal
   parsing or packing routines. They are, however, part of the 'ustl' standard
   and are put here for developer conveniance, as well as to properly define the
   standard.
*/
...
/*
   ...
   after the array of font data attributes (if any) if the numberOfSetFeatures
   is non-zero. There must be one of these structures for each
   numberOfSetFeatures.
*/
...
/* ------------------------ */
/*
   This is somewhat of an addendum to the 'ustl' structure above. These
   flattened data structures are stored in block 4 as a font attribute with the
   attribute tag of kATSUFontTag. They can store font data in a few different
   ways, such as by a FontSync reference or by simple raw font name data. Just
   as with the 'ustl' above, this structure must maintain four byte alignment.
*/
...
/*
   ...
   will be used, more than one name specifer should be stored. The standard
   ATSUI style run flattening and parsing functions,
   ATSUFlattenStyleRunsToStream and ATSUUnflattenStyleRunsFromStream. These will
   store both the font's full name (kFontFullName) as well as the font's
   manufacturer name (kFontManufacturerName) and match fonts based on both of
   these.
*/
...
ATSUFlattenStyleRunsToStream(ATSUFlattenedDataStreamFormat iStreamFormat,
                             ...
                             ByteCount iStreamBufferSize, void *oStreamBuffer,
