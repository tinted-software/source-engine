...
// Look for various windows environments, boil them all down to defining
// __WIN32__. _WIN32 comes from MS tools.  When using MinGW __WIN32__ is already
// defined.
...
#endif
#if !defined(__MACTYPES__) || (defined(UNIVERSAL_INTERFACES_VERSION) &&        \
...
#define kCFAllocatorDefault                                                    \
  (*((const CFAllocatorRef *)QTGetCFConstant("kCFAllocatorDefault")))
#define kCFAllocatorSystemDefault                                              \
  (*((const CFAllocatorRef *)QTGetCFConstant("kCFAllocatorSystemDefault")))
#define kCFAllocatorMalloc                                                     \
  (*((const CFAllocatorRef *)QTGetCFConstant("kCFAllocatorMalloc")))
#define kCFAllocatorNull                                                       \
  (*((const CFAllocatorRef *)QTGetCFConstant("kCFAllocatorNull")))
#define kCFAllocatorUseContext                                                 \
  (*((const CFAllocatorRef *)QTGetCFConstant("kCFAllocatorUseContext")))
#else
...
/*
        ...
        your app, or do it in a nested fashion (by restoring the previous
   allocator when you exit your context). The latter might be appropriate for
   plug-ins or libraries that wish to set the default allocator.
*/
CF_EXPORT
