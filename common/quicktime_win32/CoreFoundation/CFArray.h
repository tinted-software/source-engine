...
#define kCFTypeArrayCallBacks                                                  \
  (*((const CFArrayCallBacks *)QTGetCFConstant("kCFTypeArrayCallBacks")))
#else
CF_EXPORT
...
CFIndex CFArrayBSearchValues(CFArrayRef theArray, CFRange range,
                             const void *value, CFComparatorFunction comparator,
                             void *context);
...
*/
CF_EXPORT
void CFArrayExchangeValuesAtIndices(CFMutableArrayRef theArray, CFIndex idx1,
                                    CFIndex idx2);
