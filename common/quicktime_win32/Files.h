...
#ifdef __cplusplus
inline DEFINE_API_C(IOCompletionUPP)
    NewIOCompletionUPP(IOCompletionProcPtr userRoutine) {
...
struct FSVolumeInfo {
  ...
  UInt32 nextCatalogID;  /* next unused catalog node ID ooo OYG ooo need to make
...
#ifdef __cplusplus
inline DEFINE_API_C(FNSubscriptionUPP)
    NewFNSubscriptionUPP(FNSubscriptionProcPtr userRoutine) {
  return userRoutine;
}
#else
 ...
 */
EXTERN_API_C(OSStatus)
FSEjectVolumeSync(FSVolumeRefNum vRefNum, OptionBits flags,
 ...
 */
EXTERN_API_C(OSStatus)
FSGetAsyncEjectStatus(FSVolumeOperation volumeOp, FSEjectStatus *status,
