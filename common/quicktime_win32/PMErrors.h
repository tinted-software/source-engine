...
enum {
  ...
  kPMInvalidValue =
      -30889, /* invalid value in ticket or template or dictionary */
  kPMInvalidAllocator = -30890, /* invalid memory allocator */
  kPMInvalidTicket = -30891,    /* invalid job ticket */
  kPMInvalidItem =
      -30892, /* invalid item in ticket or template or dictionary */
  kPMInvalidType =
      -30893, /* invalid type in ticket or template or dictionary */
  ...
  /* Print Job Creator and Printing Dialog Extension error codes (-9500 to
     -9540) */
  ...
  kPMNoSelectedPrinters =
      -9541, /* no selected printers or error getting selection.*/
  ...
  kPMTicketTypeNotFound =
      -9580, /* we can't find the ticket type in our ticket. */
  kPMUpdateTicketFailed =
      -9581, /* attempt to update ticket to current API failed. */
  kPMValidateTicketFailed =
      -9582, /* ticket has at least one key that's invalid. */
  kPMSubTicketNotFound =
      -9583, /* sub ticket requested is not stored in this ticket. */
  ...
  kPMItemIsLocked =
      -9586, /* item's locked flag was true when attempt made to update. */
  kPMTicketIsLocked = -9587,   /* caller may not change a locked ticket. */
  kPMTemplateIsLocked = -9588, /* caller can't change the template. */
  kPMKeyNotFound =
      -9589, /* the requested update is for a key that doesn't exist. */
  kPMKeyNotUnique = -9590, /* the key passed in already exists in the ticket,
                              can't make a new one. */
  kPMUnknownDataType =
      -9591, /* couldn't determine proper CF type for the value passed in. */
             /* ClientPrintingLib (-9620 to -9629) */
  kPMCreateMessageFailed = -9620, /* could not create message */
  kPMServerCommunicationFailed =
      -9621,                     /* communication with print server failed */
  kPMKeyOrValueNotFound = -9623, /* missing required key or value */
  kPMMessagingError = -9624,     /* could not connect to message port or send a
  ...
  kPMCVMSymbolNotFound =
      -9662, /* a required converter module symbol is missing */
  ...
  kPMIOAttrNotAvailable =
      -9787 /* IO attribute not available on current connection type */
};
