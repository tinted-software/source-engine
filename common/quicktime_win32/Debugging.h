...
/*
    ...
    In addition, these macros should also be #defined (they are described in
   detail below): kComponentSignatureString COMPONENT_SIGNATURE
*/
/*
...
/*
 *  The options parameter to DebugAssert and DEBUGASSERTMSG is currently
...
#define DEBUGASSERTMSG(componentSignature, options, assertionString,           \
                       exceptionLabelString, errorString, fileName,            \
                       lineNumber, value)
...
#define DEBUGASSERTMSG(componentSignature, options, assertionString,           \
                       exceptionLabelString, errorString, fileName,            \
                       lineNumber, value)                                      \
  DebugAssert(componentSignature, options, assertionString,                    \
              exceptionLabelString, errorString, 0, 0, (void *)value)
#else
/* include all info */
#define DEBUGASSERTMSG(componentSignature, options, assertionString,           \
                       exceptionLabelString, errorString, fileName,            \
                       lineNumber, value)                                      \
  DebugAssert(componentSignature, options, assertionString,                    \
              exceptionLabelString, errorString, fileName, lineNumber,         \
              (void *)value)
#endif
#endif
...
/*
 *  Include AssertMacros.h where all of the check, verify, and require macros
 * are defined
 */
...
enum {
  ...
  kInSecondaryIntHandlerMask =
      0x00000040, /* Secondary interrupt handlers are executing */
...
enum {
  kComponentDebugOption =
      0 /* optionSelectorNum to turn breaks for component On or Off*/
};
...
#ifdef __cplusplus
inline DEFINE_API_C(DebugAssertOutputHandlerUPP) NewDebugAssertOutputHandlerUPP(
...
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
inline DEFINE_API_C(void) InvokeDebugAssertOutputHandlerUPP(
    OSType componentSignature, UInt32 options, const char *assertionString,
    const char *exceptionLabelString, const char *errorString,
    const char *fileName, long lineNumber, void *value,
...
#define InvokeDebugAssertOutputHandlerUPP(                                     \
    componentSignature, options, assertionString, exceptionLabelString,        \
