...
/*! @header CVDirect3DBuffer.h
   ...
   @discussion A CoreVideo buffer derives from a generic buffer and can be an
   ImageBuffer or PixelBuffer.
...
/*!
    ...
    @result     A CVDirect3DBuffer object that is the same as the passed in
   buffer.
*/
CV_EXPORT CVDirect3DBufferRef
CVDirect3DBufferRetain(CVDirect3DBufferRef texture);
...
/*!
    ...
    @abstract   Create a new CVDirect3DBuffer that may be used for D3D rendering
   purposes
    @param      width The width of the buffer in pixels
    @param      height The height of the buffer in pixels
    @param      attributes A CFDictionaryRef containing other desired attributes
   of the buffer (texture format, max mipmap level, etc.). May be NULL.
    @param      bufferOut    The newly created buffer will be placed here.
    @result     kCVReturnSuccess if the attachment succeeded
*/
CV_EXPORT CVReturn CVDirect3DBufferCreate(CFAllocatorRef allocator,
                                          size_t width, size_t height,
                                          void *d3dDevice,
                                          CFDictionaryRef attributes,
                                          CVDirect3DBufferRef *bufferOut);
...
CVDirect3DBufferGetAttributes(CVDirect3DBufferRef d3DBuffer);
