/*
     ...
     Copyright:  (c) 2007 (c) 1989-2001 by Apple Computer, Inc., all rights
   reserved
...
enum {
  ...
  kFSAliasInfoVolumeFlags =
      0x00000040 /* volumeIsBootVolume, volumeIsAutomounted, volumeIsEjectable
                    and volumeHasPersistentFileIDs are valid*/
};
...
#ifdef __cplusplus
inline DEFINE_API_C(AliasFilterUPP)
    NewAliasFilterUPP(AliasFilterProcPtr userRoutine) {
...
FSNewAliasUnicode(const FSRef *fromFile, /* can be NULL */
                  const FSRef *targetParentRef, UniCharCount targetNameLength,
                  const UniChar *targetName, AliasHandle *inAlias,
 ...
 */
EXTERN_API_C(OSErr)
FSNewAliasMinimalUnicode(const FSRef *targetParentRef,
...
FSMatchAlias(const FSRef *fromFile, /* can be NULL */
             unsigned long rulesMask, AliasHandle inAlias, short *aliasCount,
             FSRef *aliasList, Boolean *needsUpdate, AliasFilterUPP aliasFilter,
             void *yourDataPtr);
...
FSMatchAliasNoUI(const FSRef *fromFile, /* can be NULL */
                 unsigned long rulesMask, AliasHandle inAlias,
