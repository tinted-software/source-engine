...
enum {
  kPMDataPlanar =
      0x0001, /* One color for all pixels, followed by next color. */
...
enum {
  ...
  kPMGray_8 =
      0x0002, /* 8 bit grayscale. 256 possible values, no color table assumed */
  kPMRGB_16 = 0x0004, /* 16 bit, 5-5-5 RGB data, with MSB unused. */
  kPMRGB_24 = 0x0008, /* 24 bit, 8-8-8 RGB data, no unused bits or bytes. */
  kPMXRGB_32 =
      0x0010, /* 32 bit x-8-8-8 RGB data, first byte unused in each pixel. */
  kPMRGBX_32 =
      0x0020, /* 32 bit 8-8-8-x RGB data, last byte unused in each pixel. */
  kPMRGBX_32_Sep_Gray_8 = 0x0040, /* Same as kPMRGBX_32 but with an extra plane
                                     of 8-bit K-separation. */
  kPMCMYK_32 = 0x0080, /* 32 bit 8-8-8-8 CMYK data. All bytes used. */
  kPMUnused =
      0xFF00 /* Remaining bits are unused for now and should be clear. */
};
