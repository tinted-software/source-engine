...
enum {
  ...
  /* kNSLDuplicateSearchInProgress is not strictly an error.  The client is free
     to ignore*/
...
#ifdef __cplusplus
inline DEFINE_API_C(NSLMgrNotifyUPP)
    NewNSLMgrNotifyUPP(NSLMgrNotifyProcPtr userRoutine) {
...
#ifdef __cplusplus
inline DEFINE_API_C(NSLClientNotifyUPP)
    NewNSLClientNotifyUPP(NSLClientNotifyProcPtr userRoutine) {
...
/* ---> serviceListPtr is the address of a pointer which will be set to point at
 * the portion of the newDataPtr that holds the serviceList to be searched */
...
/* ---> svcToFind is an NSLServiceType of a particular service to check if it is
 * in the serviceList */
/*
