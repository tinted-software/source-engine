/*
     ...
     Contains:   This file contains provider-specific definitions for various
   built-in providers.

     Version:    QuickTime 7.3

     Copyright:  (c) 2007 (c) 1993-2001 by Apple Computer, Inc. and Mentat Inc.,
   all rights reserved.
...
enum {
  ...
  T_ATALKROUTERUPEVENT =
      kAppleTalkEvent + 52, /* We didn't have a router, but now one has come up.
                               Cookie is NULL for a normal router coming up,
                               non-NULL for an ARA router coming on-line*/
  T_ATALKZONENAMECHANGEDEVENT =
      kAppleTalkEvent + 53, /* A Zone name change was issued from the router, so
                               our AppleTalk Zone has changed.*/
...
enum {
  ...
  kEnetAddressLength =
      k48BitAddrLength +
      k8022DLSAPLength, /* length of an address field used by the ENET enpoint*/
...
enum {
  ...
  kOTSerialFramingPPP =
      0x10 /* Port does its own PPP framing - wants unframed packets from PPP */
};
...
enum {
  ...
  /* less than the RcvLoWat number of characters.  If RcvLoWat is 0, then the
     RcvTimeout*/
  /* is how long a gap to wait for before delivering characters.  This parameter
     is advisory,*/
  /* and serial drivers are free to deliver data whenever they deem it
     convenient.  For instance,*/
  /* many serial drivers will deliver data whenever 64 bytes have been received,
     since 64 bytes*/
  ...
  /* look at the return value of the option to determine what it was negotiated
     to.*/
  SERIAL_OPT_ERRORCHARACTER = 0x0107, /* This option defines how characters with
                                         parity errors are handled.*/
  ...
  SERIAL_OPT_EXTCLOCK = 0x0108, /* The "ExtClock" requests an external clock.  A
                                   0-value turns off external clocking.*/
