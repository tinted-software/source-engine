/*
     ...
     Copyright:  (c) 2007 (c) 1986-2001 by Apple Computer, Inc., all rights
   reserved
...
enum {
  ...
  kSoundCodecInfoResourceType =
      FOUR_CHAR_CODE('snfo') /*Resource type holding codec information (optional
...
#if CALL_NOT_IN_CARBON
/*command numbers for SndDoCommand and SndDoImmediate that are not available for
 * use in Carbon */
enum {
  initCmd = 1,
  freeCmd = 2,
...
enum {
  ...
  siCompressionSampleRate =
      FOUR_CHAR_CODE('cprt'), /* SetInfo only: compressor's sample rate*/
  siCompressionChannels =
      FOUR_CHAR_CODE('cpct'), /* SetInfo only: compressor's number of channels*/
  siCompressionOutputSampleRate =
      FOUR_CHAR_CODE('cort'), /* GetInfo only: only implemented by compressors
                                 that have differing in and out rates */
  siCompressionInputRateList =
      FOUR_CHAR_CODE('crtl'), /* GetInfo only: only implemented by compressors
                                 that only take certain input rates */
  siCompressionType = FOUR_CHAR_CODE('comp'), /*current compression type*/
  siCompressionConfiguration =
      FOUR_CHAR_CODE('ccfg'),                     /*compression extensions*/
  siContinuous = FOUR_CHAR_CODE('cont'),          /*continous recording*/
  siDecompressionParams = FOUR_CHAR_CODE('wave'), /*decompression parameters*/
  siDecompressionConfiguration =
      FOUR_CHAR_CODE('dcfg'),                  /*decompression extensions*/
  ...
  siRateConverterRollOffSlope = FOUR_CHAR_CODE(
      'rcdb'), /*the roll-off slope for the rate converter's filter, in whole dB
                  as a long this value is a long whose range is from 20 (worst
                  quality/fastest performance) to 90 (best quality/slowest
                  performance)*/
  siOutputLatency =
  ...
  siSourceIsExhausted =
      FOUR_CHAR_CODE('srcx'), /*the ultimate source of data has run out (keep
                                 asking, but when you get nothing, that's it)*/
  siMediaContextID = FOUR_CHAR_CODE('uuid'), /*media context id -- UUID */
  siCompressionMaxPacketSize =
      FOUR_CHAR_CODE('cmxp'), /*maximum compressed packet size for current
                                 configuration -- unsigned long */
  siAudioCodecPropertyValue =
      FOUR_CHAR_CODE('spva'), /*audio codec property value --
                                 SoundAudioCodecPropertyRequestParams* */
  siAudioCodecPropertyInfo =
      FOUR_CHAR_CODE('spin') /*audio codec property info --
                                SoundAudioCodecPropertyRequestParams* */
};
...
enum {
  ...
  kHALCustomComponentSubType =
      FOUR_CHAR_CODE('halx'), /*Registered by the HAL output component ('hal!')
                                 for each HAL output device*/
...
enum {
  ...
  kVariableDurationDVAudioFormat =
      FOUR_CHAR_CODE('vdva') /*Variable Duration DV Audio*/
};
...
enum {
  ...
  kSoundCodecInfoVariableCompression =
      (1L << 1), /* has variable compression format*/
  kSoundCodecInfoHasRestrictedInputRates =
      (1L << 2), /* compressor has restricted set of input sample rates*/
  kSoundCodecInfoCanChangeOutputRate =
      (1L << 3), /* compressor may output a different sample rate than it
                    receives*/
  kSoundCodecInfoRequiresExternalFraming =
      (1L << 4), /* format requires external framing information during
                    decode/encode*/
  kSoundCodecInfoVariableDuration =
      (1L << 5) /* audio packets can vary in duration*/
};
...
enum {
  ...
  audioDoesIndependentChannels =
      (1L << 2) /*Device supports independent software control of each channel*/
};
...
enum {
  ...
  siNoneQuality =
      FOUR_CHAR_CODE('none') /*settings don't match any quality for a get call*/
};
...
enum {
  kExtendedSoundSampleCountNotValid =
      1L << 0, /* set if sampleCount of SoundComponentData isn't meaningful; use
                  buffer size instead*/
  kExtendedSoundBufferSizeValid = 1L << 1, /* set if bufferSize field is valid*/
  kExtendedSoundFrameSizesValid =
      1L << 2, /* set if frameSizesArray is valid (will be nil if all sizes are
                  common and kExtendedSoundCommonFrameSizeValid is set*/
  kExtendedSoundCommonFrameSizeValid =
      1L << 3, /* set if all audio frames have the same size and the
                  commonFrameSize field is valid*/
  kExtendedSoundExtensionsValid =
      1L << 4, /* set if pointer to extensions array is valid*/
  kExtendedSoundBufferFlagsValid = 1L
                                   << 5 /* set if buffer flags field is valid*/
};

/* flags passed in bufferFlags/bufferFlagsMask extended fields if
 * kExtendedSoundBufferFlagsValid extended flag is set*/
enum {
  kExtendedSoundBufferIsDiscontinuous =
      1L << 0, /* buffer is discontinuous with previous buffer*/
...
#ifdef __cplusplus
inline DEFINE_API_C(SndCallBackUPP)
    NewSndCallBackUPP(SndCallBackProcPtr userRoutine) {
...
struct ExtendedScheduledSoundHeader {
  ...
  long
      *frameSizesArray; /* pointer to array of longs with frame sizes in bytes*/
...
#ifdef __cplusplus
inline DEFINE_API_C(SndDoubleBackUPP)
    NewSndDoubleBackUPP(SndDoubleBackProcPtr userRoutine) {
...
enum { kSoundAudioCodecPropertyWritableFlag = 1L << 0 };

struct SoundAudioCodecPropertyRequestParams {
  UInt32 propertyClass;
  UInt32 propertyID;
  UInt32 propertyDataSize; /* out -- GetPropertyInfo, in/out -- GetProperty, in
                              -- SetProperty*/
  ...
  ComponentResult propertyRequestResult; /* out -- GetPropertyInfo, GetProperty,
                                            SetProperty*/
};
typedef struct SoundAudioCodecPropertyRequestParams
    SoundAudioCodecPropertyRequestParams;
...
#ifdef __cplusplus
inline DEFINE_API_C(SoundParamUPP)
    NewSoundParamUPP(SoundParamProcPtr userRoutine) {
...
enum {
  uppSIInterruptProcInfo = 0x1C579802
      ...
      4_bytes:D1) */
#ifdef __cplusplus
inline DEFINE_API_C(SIInterruptUPP)
    NewSIInterruptUPP(SIInterruptProcPtr userRoutine) {
...
#ifdef __cplusplus
inline DEFINE_API_C(SICompletionUPP)
    NewSICompletionUPP(SICompletionProcPtr userRoutine) {
...
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
inline DEFINE_API_C(void) DisposeSoundConverterFillBufferDataUPP(
...
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
inline DEFINE_API_C(Boolean) InvokeSoundConverterFillBufferDataUPP(
    SoundComponentDataPtr *data, void *refCon,
    SoundConverterFillBufferDataUPP userUPP) {
...
#ifdef __cplusplus
inline DEFINE_API_C(FilePlayCompletionUPP)
    NewFilePlayCompletionUPP(FilePlayCompletionProcPtr userRoutine) {
