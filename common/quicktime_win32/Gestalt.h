/*
     ...
     Copyright:  (c) 2007 (c) 1988-2001 by Apple Computer, Inc.  All rights
   reserved
...
#ifdef __cplusplus
inline DEFINE_API_C(SelectorFunctionUPP)
    NewSelectorFunctionUPP(SelectorFunctionProcPtr userRoutine) {
...
enum {
  ...
  gestaltAliasMgrPrefersPath = 7 /* True if the Alias Mgr prioritizes the path
                                    over file id during resolution by default */
};
...
enum {
  ...
  gestaltControlMgrPresent = (1L << 0), /* NOTE: this is a bit mask, whereas all
...
enum {
  ...
  gestaltColorPicker = FOUR_CHAR_CODE(
...
enum {
  ...
  gestaltDisplayMgrSleepNotifies =
      7 /* True if Display Mgr generates "displayWillSleep", "displayDidWake"
           notifications */
};
...
enum {
  ...
  gestaltFSSupportsExclusiveLocks =
      15, /* File system uses POSIX O_EXLOCK for opens */
  gestaltFSSupportsHardLinkDetection =
      16 /* File system returns if an item is a hard link */
};
...
enum {
  ...
  gestaltPwrBk99JISKbd = 201, /* (0xC9) '99 PowerBook JIS Keyboard with Embedded
                                 Keypad, function keys & inverted T */
...
enum {
  ...
  gestaltMPDeviceManager =
      1, /* True if synchronous Device Manager calls can be made from MPTasks */
  gestaltMPTrapCalls =
...
enum {
  ...
  gestaltSplitOSMachineNameStartupDiskIsNonPersistent =
      5 /* The startup disk ( vRefNum == -1 ) is non-persistent, meaning changes
           won't persist across a restart. */
};
...
enum {
  ...
  gestaltATSUByCharacterClusterFeature =
      0x00000010, /* ATSUCursorMovementTypes introduced in ATSUI version 2.4 */
  gestaltATSUAscentDescentControlsFeature =
      0x00000010, /* attributes introduced in ATSUI version 2.4 */
  gestaltATSUHighlightInactiveTextFeature =
      0x00000010, /* feature introduced in ATSUI version 2.4 */
  gestaltATSUPositionToCursorFeature =
      0x00000010, /* features introduced in ATSUI version 2.4 */
...
enum {
  ...
  gestaltVMInfoSizeType =
      1, /* Display whether VM is on or off and the size of the backing store */
...
enum {
  ...
  gestaltFrontWindowMayBeHiddenBit =
      8, /* FrontWindow and related APIs will return the front window even when
            the app is hidden*/
         /* masks for the above bits*/
...
enum {
  ...
  gestaltX86HasPAE =
      6, /* supports physical addresses > 32 bits (Physical Address Extension)*/
