...
/*
   ...
   to an FSSpec that represents a SoundFont 2 or DLS file (otherwise set it to
   NULL ).
...
enum {
  ...
  kGenericMusicResInstrumentList =
      3, /* NmLs of names and shorts, categories prefixed by 'oo' */
...
struct NoteRequestInfo {
  UInt8
      flags; /* 1: dont accept GM match, 2: dont accept same-synth-type match */
  NoteRequestMIDIChannel
      midiChannelAssignment; /* (kNoteRequestSpecifyMIDIChannel | 1->16) as MIDI
                                Channel assignement or zero - see notes above */
...
/*    QuickTime Music Track Event Formats:
   ...
   (7-bit MSB) (7-bit LSB) ( or 15-bit Signed Value) o Short GENERAL Event (4
   bytes/event):
...
#define qtma_StuffNoteEvent(x, part, pitch, volume, duration)                  \
  (x = (kNoteEventType << kEventTypeFieldPos) |                                \
       ...
       (((long)(volume) - kNoteEventVolumeOffset)                              \
...
#ifdef __cplusplus
inline DEFINE_API_C(MusicMIDISendUPP)
    NewMusicMIDISendUPP(MusicMIDISendProcPtr userRoutine) {
...
#ifdef __cplusplus
inline DEFINE_API_C(MusicOfflineDataUPP)
    NewMusicOfflineDataUPP(MusicOfflineDataProcPtr userRoutine) {
...
#ifdef __cplusplus
inline DEFINE_API_C(TuneCallBackUPP)
    NewTuneCallBackUPP(TuneCallBackProcPtr userRoutine) {
...
#ifdef __cplusplus
inline DEFINE_API_C(TunePlayCallBackUPP)
    NewTunePlayCallBackUPP(TunePlayCallBackProcPtr userRoutine) {
