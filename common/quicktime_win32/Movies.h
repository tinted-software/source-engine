...
/*
    ...
    QuickTime APIs that were introduced in QuickTime 6.0 and later are tagged
   with availability macros indicating the first Mac OS X version in which they
   were *always* available.  Such APIs may also be present on systems running
   earlier Mac OS X releases when QuickTime updates have been installed.
    ...
    However, QuickTime 6.4 can also be installed as an update to Mac OS
   X 10.2.x, so QTNewDataReferenceFromCFURL is also available on some systems
   running Mac OS X 10.2.x.
...
/* This sets the user defined exportset name i.e. fw_QuickTime_XManchego,
 * available on 10.5 or later, and comment [4486184] */
...
enum {
  ...
  kUserDataAnimatedGIFLoopCount =
      FOUR_CHAR_CODE('gifc'), /* data is big-endian UInt16 */
  kQTAnimatedGIFLoopCountInfinite = 0,
  kUserDataAnimatedGIFBufferingSize =
      FOUR_CHAR_CODE('gifb') /* data is big-endian UInt32 */
};
...
/*
   Definitions for SoundDescriptionV2:
        LPCMFrame = one uncompressed sample in each of the channels (ie. 44100Hz
   audio has 44100 LPCMFrames per second, whether it is mono, stereo, 5.1, or
   whatever). In other words, LPCMFrames/audioSampleRate is duration in seconds.
        AudioPacket = For compressed audio, an AudioPacket is the natural
   compressed access unit of that format.  For uncompressed audio, an
   AudioPacket is simply one LPCMFrame.
*/
...
struct SoundDescriptionV2 {
  ...
  OSType dataFormat; /* 'lpcm' for uncompressed, compression type otherwise (eg.
                        'ima4') */
  ...
  UInt32 sizeOfStructOnly; /* must be set to sizeof(SoundDescriptionV2), ie.
                              offset to extensions */
  Float64 audioSampleRate; /* audio frames per second, eg. 44100.0 */
  UInt32 numAudioChannels; /* any channel assignment info will be in an
                              extension */

  SInt32 always7F000000;      /* Reserved, must be set to 0x7F000000 */
  UInt32 constBitsPerChannel; /* only set if constant (and only for uncompressed
                                 audio) */

  UInt32 formatSpecificFlags;           /* eg. see LPCM flag definitions in
                                           CoreAudioTypes.h */
  UInt32 constBytesPerAudioPacket;      /* only set if constant */
  UInt32 constLPCMFramesPerAudioPacket; /* only set if constant */

  /* additional atom based extensions ([long size, long type, some data],
   * repeat)*/
};
 ...
 */
EXTERN_API(OSStatus)
QTSoundDescriptionCreate(AudioStreamBasicDescription *inASBD,
                         AudioChannelLayout *inLayout, ByteCount inLayoutSize,
                         void *inMagicCookie, ByteCount inMagicCookieSize,
 ...
 */
EXTERN_API(OSStatus)
QTSoundDescriptionConvert(QTSoundDescriptionKind fromKind,
...
enum {
  ...
  kQTPropertyClass_SoundDescription =
      FOUR_CHAR_CODE('sdes') /* class for SoundDescription properties*/
};
 ...
 */
EXTERN_API(OSStatus)
QTSoundDescriptionGetPropertyInfo(SoundDescriptionHandle inDesc,
 ...
 */
EXTERN_API(OSStatus)
QTSoundDescriptionGetProperty(SoundDescriptionHandle inDesc,
                              QTPropertyClass inPropClass,
 ...
 */
EXTERN_API(OSStatus)
QTSoundDescriptionSetProperty(SoundDescriptionHandle inDesc,
                              QTPropertyClass inPropClass,
                              QTPropertyID inPropID, ByteCount inPropValueSize,
...
enum {
  ...
  kActionSpriteTrackNewImage =
      7182, /* (C string imageURL, QTAtomID desiredID) */
  ...
  kActionListAddAttribute = 13320, /* (C string elementPath, long atIndex, C
                                      string newAttributeName) */
  kActionListRemoveAttributes =
      13321, /* (C string elementPath, long startIndex, long endIndex) */
  kActionListSetAttributeValue =
      13322 /* (C string elementPath, C string attributeName, C string
               valueString) */
};
...
enum {
  ...
  kOperandListGetAttributeNameByIndex =
      7173, /* (C string elementPath, long index) */
  kOperandListGetAttributeValue =
      7174,             /* (C string elementPath, C string attributeName) */
...
enum {
  ...
  kQTEventKeyUp =
      FOUR_CHAR_CODE('keyU') /* qtevent.param1 = key, qtevent.param2 =
                                modifiers, qtEvent.param3 = scanCode */
};
...
typedef CALLBACK_API(void,
                     QTNextTaskNeededSoonerCallbackProcPtr)(TimeValue duration,
                                                            unsigned long flags,
                                                            void *refcon);
...
enum {
  ...
  mediaSampleDroppable = 1 << 27, /* sample is not required to be decoded for
                                     later samples to be decoded properly */
  mediaSamplePartialSync =
      1 << 16, /* sample is a partial sync (e.g., I frame after open GOP) */
  mediaSampleHasRedundantCoding =
      1 << 24, /* sample is known to contain redundant coding */
  mediaSampleHasNoRedundantCoding =
      1 << 25, /* sample is known not to contain redundant coding */
  mediaSampleIsDependedOnByOthers = 1
                                    << 26, /* one or more other samples depend
                                              upon the decode of this sample */
  mediaSampleIsNotDependedOnByOthers =
      1 << 27, /* synonym for mediaSampleDroppable */
  mediaSampleDependsOnOthers =
      1 << 28, /* sample's decode depends upon decode of other samples */
  mediaSampleDoesNotDependOnOthers =
      1
      << 29, /* sample's decode does not depend upon decode of other samples */
  mediaSampleEarlierDisplayTimesAllowed =
      1 << 30 /* samples later in decode order may have earlier display times */
};
...
enum {
  ...
  hintsAllowIdleSleep =
      1L << 25, /* asks media handlers not to call UpdateSystemActivity etc */
...
/*
   Opaque replacement for SampleReferenceRecord/SampleReference64Record arrays
   able to carry information not described in those arrays of those records
*/
...
enum { kQTEnterMoviesFlagDontSetComponentsThreadMode = 1L << 0 };
 ...
 */
EXTERN_API(OSErr)
LoadMovieIntoRam(Movie theMovie, TimeValue time, TimeValue duration,
                 long flags);
 ...
 */
EXTERN_API(OSErr)
LoadTrackIntoRam(Track theTrack, TimeValue time, TimeValue duration,
                 long flags);
 ...
 */
EXTERN_API(OSErr)
LoadMediaIntoRam(Media theMedia, TimeValue time, TimeValue duration,
                 long flags);
 ...
 */
EXTERN_API(OSErr)
QTIdleManagerSetNextIdleTimeDelta(IdleManager im, TimeValue duration,
                                  TimeScale scale);
...
/* Item tags for use in GetControlData() (some with SetControlData()) calls on
 * Movie Controls*/
enum {
...
/*
** CreateMovieControl() -   This is the public API routine that creates a Movie
*Control. Given a window and location
**                          plus a movie, it constructs a Movie Control with a
*Movie Controller in the window.
*/
/*
 *  CreateMovieControl()
 *
 ...
 */
EXTERN_API(OSErr)
CreateMovieControl(WindowRef theWindow, Rect *localRect, Movie theMovie,
                   UInt32 options, ControlRef *returnedControl);
 ...
 */
EXTERN_API(void)
SetMovieMasterTimeBase(Movie theMovie, TimeBase tb,
 ...
 *************************/
/*
   The QTAudioContextRef type encapsulates a connection to an audio output
   device. It represents a destination audio rendering environment that can be
   used for playback of a movie.
*/
...
QTAudioContextCreateForAudioDevice(CFAllocatorRef allocator,
...
/*
   Theory of operations:
    ...
    To unregister a Movie Audio Context Insert:
        Call QTAudioContextRegisterInsert() with a NULL
   QTAudioContextInsertRegistryInfoRef If the registry ptr is non-NULL but the
   processDataCallback is NULL, this has the same effect. To register for a
   Track Audio Context Insert during playback: Set the
   kQTAudioPropertyID_RegisterAudioContextInsert property on the track,
            providing the same registry info structure that is used for the
   QTAudioContextRegisterInsert() call. To unregister a Track Audio Context
   Insert: Set the kQTAudioPropertyID_RegisterAudioContextInsert property on the
   track, with a NULL processDataCallback
*/
...
typedef CALLBACK_API(OSStatus, AudioContextInsertProcessDataCallback)(
    void *inUserData, AudioUnitRenderActionFlags *ioRenderFlags,
    const AudioTimeStamp *inTimeStamp, UInt32 inNumberFrames,
    AudioBufferList *inInputData, AudioBufferList *outOutputData);
 ...
 *      time, in seconds.
 */
typedef CALLBACK_API(OSStatus, AudioContextInsertResetCallback)(
    void *inUserData, Float64 inSampleRate, UInt32 inMaxFrames,
    Float64 *outLatency, Float64 *outTailTime);
...
typedef CALLBACK_API(OSStatus,
                     AudioContextInsertFinalizeCallback)(void *inUserData);
...
typedef struct QTAudioContextInsertRegistryInfo
    QTAudioContextInsertRegistryInfo;
...
EXTERN_API_C(OSStatus)
QTAudioContextRegisterInsert(
    QTAudioContextRef inAudioContext, UInt32 inRegistryInfoSize,
...
enum {
   ...
   */
  kQTAudioPropertyID_RegisterAudioContextInsert = FOUR_CHAR_CODE(
      'regt') /* value is QTAudioContextInsertRegistryInfoRef. Get/Set.*/
};
...
enum {
  ...
  kQTDataLocationPropertyID_DataReference = FOUR_CHAR_CODE(
      'dref'), /* DataReferenceRecord (for semantics of NewMovieFromDataRef)*/
  ...
  kQTDataLocationPropertyID_LegacyMovieResourceHandle =
      FOUR_CHAR_CODE('rezh'), /* QTNewMovieUserProcInfo * (for semantics of
                                 NewMovieFromHandle)*/
  kQTDataLocationPropertyID_MovieUserProc =
      FOUR_CHAR_CODE('uspr'), /* for semantics of NewMovieFromUserProc*/
  kQTDataLocationPropertyID_ResourceFork =
      FOUR_CHAR_CODE('rfrk'), /* for semantics of NewMovieFromFile*/
  kQTDataLocationPropertyID_DataFork =
      FOUR_CHAR_CODE('dfrk'), /* for semantics of NewMovieFromDataFork64*/
  ...
  kQTMovieResourceLocatorPropertyID_LegacyResID =
      FOUR_CHAR_CODE('rezi'), /* (input/result property)*/
  kQTMovieResourceLocatorPropertyID_LegacyResName =
      FOUR_CHAR_CODE('rezn'), /* (result property)*/
  kQTMovieResourceLocatorPropertyID_FileOffset =
      FOUR_CHAR_CODE('foff'), /* NewMovieFromDataFork[64]*/
  kQTMovieResourceLocatorPropertyID_Callback =
      FOUR_CHAR_CODE('calb'), /* NewMovieFromUserProc(getProc,refcon)*/
  ...
  kQTMovieInstantiationPropertyID_ResultDataLocationChanged =
      FOUR_CHAR_CODE('dlch'), /* (result property)*/
  kQTMovieInstantiationPropertyID_AllowMediaOptimization =
      FOUR_CHAR_CODE('amop'),
  kQTPropertyClass_NewMovieProperty = FOUR_CHAR_CODE('mprp'),
  kQTNewMoviePropertyID_DefaultDataRef =
      FOUR_CHAR_CODE('ddrf'), /* DataReferenceRecord*/
 ...
 */
EXTERN_API(OSStatus)
NewMovieFromProperties(ItemCount inputPropertyCount,
 ...
 */
EXTERN_API(OSErr)
PutMovieIntoStorage(Movie theMovie, DataHandler dh, const wide *offset,
                    unsigned long maxSize);
 ...
 */
EXTERN_API(OSErr)
PutMovieForDataRefIntoHandle(Movie theMovie, Handle dataRef, OSType dataRefType,
                             Handle publicMovie);
 ...
 */
EXTERN_API(void)
GetMovieActiveSegment(Movie theMovie, TimeValue *startTime,
                      TimeValue *duration);
...
EXTERN_API(OSErr)
QTInstallNextTaskNeededSoonerCallback(
    QTNextTaskNeededSoonerCallbackUPP callbackProc, TimeScale scale,
...
EXTERN_API(OSErr)
QTUninstallNextTaskNeededSoonerCallback(
    QTNextTaskNeededSoonerCallbackUPP callbackProc, void *refcon);
 ...
 */
EXTERN_API(OSErr)
GetMovieRateChangeConstraints(Movie theMovie, TimeRecord *minimumDelay,
 ...
 *    CarbonLib:        not available
 *    Mac OS X:         in version 10.4 (or QuickTime 7.0) and later
 *    Windows:          in qtmlClient.lib version 10.4 (or QuickTime 7.0) and
 * later
 */
 ...
 */
EXTERN_API(void)
SampleNumToMediaDecodeTime(Media theMedia, SInt64 logicalSampleNum,
 ...
 */
EXTERN_API(void)
MediaDecodeTimeToSampleNum(Media theMedia, TimeValue64 decodeTime,
                           SInt64 *sampleNum, TimeValue64 *sampleDecodeTime,
                           TimeValue64 *sampleDecodeDuration);
 ...
 */
EXTERN_API(void)
SampleNumToMediaDisplayTime(Media theMedia, SInt64 logicalSampleNum,
 ...
 */
EXTERN_API(void)
MediaDisplayTimeToSampleNum(Media theMedia, TimeValue64 displayTime,
                            SInt64 *sampleNum, TimeValue64 *sampleDisplayTime,
                            TimeValue64 *sampleDisplayDuration);
 ...
 */
EXTERN_API(OSErr)
AddMediaSample2(Media theMedia, const UInt8 *dataIn, ByteCount size,
                TimeValue64 decodeDurationPerSample, TimeValue64 displayOffset,
...
EXTERN_API(OSErr)
AddMediaSampleFromEncodedFrame(
    Media theMedia, ICMEncodedFrameRef encodedFrame,
    TimeValue64 *sampleDecodeTimeOut); /* can be NULL */
 ...
 */
EXTERN_API(OSErr)
AddSampleTableToMedia(Media theMedia, QTSampleTableRef sampleTable,
                      SInt64 startSampleNum, SInt64 numberOfSamples,
                      TimeValue64 *sampleDecodeTimeOut); /* can be NULL */
 ...
 */
EXTERN_API(OSErr)
ExtendMediaDecodeDurationToDisplayEndTime(Media theMedia,
 ...
 */
EXTERN_API(OSErr)
GetMediaSample2(Media theMedia, UInt8 *dataOut, ByteCount maxDataSize,
                ByteCount *size, TimeValue64 decodeTime,
                TimeValue64 *sampleDecodeTime,
                TimeValue64 *decodeDurationPerSample,
                TimeValue64 *displayOffset,
                SampleDescriptionHandle sampleDescriptionH,
                ItemCount *sampleDescriptionIndex, ItemCount maxNumberOfSamples,
                ItemCount *numberOfSamples, MediaSampleFlags *sampleFlags);
...
GetMediaSample(Media theMedia, Handle dataOut, long maxSizeToGrow, long *size,
               ...
               TimeValue *durationPerSample,
               SampleDescriptionHandle sampleDescriptionH,
               long *sampleDescriptionIndex, long maxNumberOfSample,
               long *numberOfSamples, short *sampleFlags);
 ...
 */
EXTERN_API(OSErr)
CopyMediaMutableSampleTable(Media theMedia, TimeValue64 startDecodeTime,
 ...
 */
EXTERN_API(OSErr)
GetMediaShadowSync(Media theMedia, long frameDiffSampleNum,
                   long *syncSampleNum);
 ...
 **************************/
/*
    When inserting media that might have nonzero display offsets into a track,
   use display time: InsertMediaIntoTrack( track, 0, // track start time
                            GetMediaDisplayStartTime( media ), // media start
   time GetMediaDisplayDuration( media ), fixed1 );                          //
   normal speed It is safe to use these display time calls for media without
   display offsets.
*/
/*
 ...
 */
EXTERN_API(OSErr)
ConvertMovieToDataRef(Movie m, Track onlyTrack, Handle dataRef,
                      OSType dataRefType, OSType fileType, OSType creator,
                      long flags, ComponentInstance userComp);
 ...
 */
EXTERN_API(OSErr)
ConvertDataRefToMovieDataRef(Handle inputDataRef, OSType inputDataRefType,
                             Handle outputDataRef, OSType outputDataRefType,
                             OSType creator, long flags,
                             ComponentInstance userComp, MovieProgressUPP proc,
                             long refCon);
 ...
 */
EXTERN_API(OSErr)
GetMediaDataSizeTime64(Media theMedia, TimeValue64 startDisplayTime,
                       TimeValue64 displayDuration, SInt64 *dataSize);
...
enum {
  kQTCopyUserDataReplace =
      FOUR_CHAR_CODE('rplc'), /* Delete all destination user data items and then
                                 add source user data items */
  kQTCopyUserDataMerge = FOUR_CHAR_CODE(
      'merg') /* Add source user data items to destination user data */
};
 ...
 */
EXTERN_API(void)
GetMediaNextInterestingDecodeTime(Media theMedia, short interestingTimeFlags,
                                  TimeValue64 decodeTime, Fixed rate,
 ...
 */
EXTERN_API(void)
GetMediaNextInterestingDisplayTime(Media theMedia, short interestingTimeFlags,
                                   TimeValue64 displayTime, Fixed rate,
 ...
 */
EXTERN_API(OSErr)
NewMovieFromFile(Movie *theMovie, short resRefNum,
 ...
 */
EXTERN_API(OSErr)
NewMovieFromStorageOffset(Movie *theMovie, DataHandler dh,
                          const wide *fileOffset, short newMovieFlags,
                          Boolean *dataRefWasChanged);
 ...
 */
EXTERN_API(OSErr)
NewMovieForDataRefFromHandle(Movie *theMovie, Handle h, short newMovieFlags,
                             Boolean *dataRefWasChanged, Handle dataRef,
                             OSType dataRefType);
 ...
 */
EXTERN_API(OSErr)
OpenMovieStorage(Handle dataRef, OSType dataRefType, long flags,
 ...
 */
EXTERN_API(Movie)
FlattenMovieDataToDataRef(Movie theMovie, long movieFlattenFlags,
                          Handle dataRef, OSType dataRefType, OSType creator,
                          ScriptCode scriptTag, long createMovieFileFlags);
 ...
 */
EXTERN_API(OSErr)
GetMovieThreadAttachState(Movie m, Boolean *outAttachedToCurrentThread,
 ...
 */
EXTERN_API(OSErr)
QTNewDataReferenceFromFSRef(const FSRef *fileRef, UInt32 flags,
 ...
 */
EXTERN_API(OSErr)
QTNewDataReferenceFromFSRefCFString(const FSRef *directoryRef,
                                    CFStringRef fileName, UInt32 flags,
                                    Handle *outDataRef, OSType *outDataRefType);
 ...
 */
EXTERN_API(OSErr)
QTNewDataReferenceFromFSSpec(const FSSpec *fsspec, UInt32 flags,
                             Handle *outDataRef, OSType *outDataRefType);
 ...
 */
EXTERN_API(OSErr)
QTNewDataReferenceWithDirectoryCFString(Handle inDataRef, OSType inDataRefType,
                                        CFStringRef targetName, UInt32 flags,
                                        Handle *outDataRef,
 ...
 */
EXTERN_API(OSErr)
QTNewDataReferenceFromFullPathCFString(CFStringRef filePath,
                                       QTPathStyle pathStyle, UInt32 flags,
                                       Handle *outDataRef,
                                       OSType *outDataRefType);
 ...
 */
EXTERN_API(OSErr)
QTNewDataReferenceFromCFURL(CFURLRef url, UInt32 flags, Handle *outDataRef,
                            OSType *outDataRefType);
 ...
 */
EXTERN_API(OSErr)
QTNewDataReferenceFromURLCFString(CFStringRef urlString, UInt32 flags,
                                  Handle *outDataRef, OSType *outDataRefType);
 ...
 */
EXTERN_API(OSErr)
QTGetDataReferenceDirectoryDataReference(Handle dataRef, OSType dataRefType,
                                         UInt32 flags, Handle *outDataRef,
                                         OSType *outDataRefType);
 ...
 */
EXTERN_API(OSErr)
QTGetDataReferenceTargetNameCFString(Handle dataRef, OSType dataRefType,
                                     CFStringRef *name);
 ...
 */
EXTERN_API(OSErr)
QTGetDataReferenceFullPathCFString(Handle dataRef, OSType dataRefType,
                                   QTPathStyle style, CFStringRef *outPath);
 ...
 */
EXTERN_API(OSErr)
QTGetDataHandlerDirectoryDataReference(DataHandler dh, UInt32 flags,
                                       Handle *outDataRef,
                                       OSType *outDataRefType);
 ...
 */
EXTERN_API(OSErr)
QTGetDataHandlerFullPathCFString(DataHandler dh, QTPathStyle style,
                                 CFStringRef *outPath);
 ...
 */
EXTERN_API(OSErr)
SetMediaDataRef(Media theMedia, short index, Handle dataRef,
                OSType dataRefType);
...
enum {
  ...
  fullScreenCaptureDisplay =
      1 << 5L, /* capturedisplay is a mac os x specific parameter */
  fullScreenCaptureAllDisplays =
      1 << 6L /* capturealldisplays is a mac os x specific parameter */
};
...
*/
enum { kRefConNavigationNext = 0, kRefConNavigationPrevious = 1 };
...
*****/
enum {
  kFlashTrackPropertyAcceptAllClicks = FOUR_CHAR_CODE(
      'clik') /* type of media property atom; data is a Boolean */
};
...
enum { kTweenRecordNoFlags = 0, kTweenRecordIsAtInterruptTime = 0x00000001 };
...
/*
    QTDoTweenPtr is an interrupt-safe version of QTDoTween.  It has the
   following limitations:
     - not all tween types support this call (those which must allocated
   memory), in which case they return codecUnimpErr.
     - the QTAtomContainer used for the tween must be locked
     - the dataSize must be large enough to contain the result
     - this call is not supported for sequence tweens, use interpolation tweens
   instead
*/
/*
 *  QTDoTweenPtr()
...
enum { kEffectParentWindowCarbon = 'carb' };
struct QTEventLoopDescriptionRecord {
  ...
  void *parentWindow; /* parent window (for sheets) or NIL to use Carbon
                         FrontWindow() */
  void *eventTarget;  /* EventTargetRef to receive kHICommandOK and
                         kHICommandCancel */
};

typedef struct QTEventLoopDescriptionRecord QTEventLoopDescriptionRecord,
    *QTEventLoopDescriptionPtr;
#endif
...
enum {
  ...
  pdActionGetNextSample = 16, /* param is QTAtomContainer with effect sample to
                                 change - createdDialog may be NIL */
  pdActionGetPreviousSample = 17, /* param is QTAtomContainer with effect sample
                                     to change - createdDialog may be NIL */
  pdActionCompactSample = 18, /* param is QTAtomContainer with effect sample to
                                 compact, - createdDialog may be NIL */
  pdActionSetEditCallout =
      19, /* param is QTParamPreviewCalloutPtr, can be NIL */
  pdActionSetSampleTime = 20, /* param is QTParamSampleTimePtr, can be NIL */
  pdActionDoEditCommand =
      21, /* param is long with menu command (ie, mcMenuCut etc) */
  pdActionGetSubPanelMenuValue =
      22, /* param is long and returns current sub-panel value selected by the
             effect */
  ...
  pdActionCustomPositionControl =
      25, /* param is QTCustomControlPositionControlPtr */
  pdActionCustomShowHideControl =
      26, /* param is QTCustomControlShowHideControlPtr */
  ...
  pdActionCustomSetPreviewPicture =
      30, /* param is QTCustomControlSetPreviewPicturePtr */
  pdActionCustomSetEditCallout =
      31, /* param is QTCustomControlSetEditCalloutPtr */
  pdActionCustomGetEnableValue =
      32, /* param is QTCustomControlGetEnableValuePtr */
  pdActionCustomSetSampleTime =
      33,                      /* param is QTCustomControlSetSampleTimePtr */
  ...
  pdActionRunInEventLoop =
      36, /* param is QTEventLoopDescriptionPtr - OS X only*/
  pdActionConvertSettingsToXML =
      37, /* param is QTAtomContainer* inbound, Handle* outbound contains the
             XML - createdDialog may be NIL */
  pdActionConvertSettingsToXMLWithComments =
      38, /* param is QTAtomContainer* inbound, Handle* outbound contains the
             XML with comments - createdDialog may be NIL */
  pdActionConvertSettingsToText =
      39, /* param is QTAtomContainer* inbound, Handle* outbound contains human
             readable text - createdDialog may be NIL */
  pdActionConvertXMLToSettings =
      40, /* param is Handle* inbound, QTAtomContainer* outbound contains
             parameters - createdDialog may be NIL */
  pdActionSetPropertyComponent = 41 /* param is QTParamComponentPropertyPtr */
};

/* Sample Time information */
enum { pdSampleTimeDisplayOptionsNone = 0x00000000 };

struct QTParamComponentPropertyRecord {
  ComponentInstance component; /* component to call for get/set properties*/
  OSType defaultClass; /* default property class if not overriden by a given
                          parameter*/
};
...
struct QTParamPreviewCalloutRecord {
  MoviePreviewCallOutUPP
      calloutProc; /* called when user makes editing changes to dialog.  May be
                      NIL.  You should return true from your function. */
...
enum {
  ...
  pdOptionsEditCurrentEffectOnly =
      0x00000008,                      /* List of effects will not be shown */
...
/*
    Effects may choose to implement custom controls to allow the user to more
   easily edit complex parameters that are ill-served by simple sliders or type
   in boxes.   Effects may allow a custom control for either a single parameter,
   or for a group of parameters.

    Parameter(s) for a custom control must still be data types defined by the
   standard set, or for complex records of data, must be defined within a group
   as individual parameters made up from base data types (for example, a point
   is a group containing two Fixed point numbers). This is to allow applications
   that do not wish to use the custom control for the effect to set values
   themselves.

    Effects should be aware that these custom controls may be deployed by the
   application in either a dialog or a window, with application defined
   background colors or patterns, along with application defined font
    characteristics for the window.

    It is recommended that effects implement custom controls only when needed,
   and that custom controls be used for specific types of parameters (ie, point,
   rectangle, polygon, path) rather than the entire user interface for the
   effect.  Effects may choose to implement multiple custom controls which
   combine with standard controls to present the total user interface.  For
   effects which have very complex user interfaces not well suited for inclusion
   within a single window, it is recommended to use kParameterImageIsPreset --
   which allows the effect to have an external editing application for
   parameters which may then be set within the standard UI via the open file
    dialog or drag and drop.  The Lens Flare effect's "Flare Type" is an example
   of such a preset.

    For parameters that use a custom control to control a single parameter
   value, a new behavior flag has been added (kCustomControl), and the behavior
   for the parameter should be kParameterItemControl.

    For parameters that are groups, the same flag (kCustomControl) should be
   used, and the behavior should be kParameterItemGroupDivider.  Groups with the
   kCustomControl bit set will be implemented by calling the custom control for
   that group -- the parameters within that group will not be processed in the
   normal manner.

    In both cases, the new customType and customID fields of the behavior must
   be filled in.  These are used in order to allow your custom control to
   determine which parameter is being edited in the case where the custom
   control is used for the editing of multiple parameters.  These values are
   passed into the pdActionCustomNewControl call.  Since the custom control
   mechanism is also used by QuickTime's default effect dialogs, you should be
   prepared to pass onto the base effect any pdActionCustomNewControl calls for
   type/id pairs that you do not handle yourself.  When pdActionCustomNewControl
   is called for controls of types handled by QuickTime, customType is
   kParameterAtomTypeAndID and customID is the ID of the parameter atom.
*/

/*
    pdActionCustomNewControlControl is called by application to create a new
   custom control or set of controls for an effect parameter.  When
   pdActionCustomNewControl is called, the effect should perform any basic
   allocation it needs for storage and return the result in storage. The options
   parameter tells the control if the application wishes to support
   interpolated, optionally interpolated, or a single value parameter.

    Since pdActionCustomNewControlControl may be called upon your effect for
   other items within the dialog, it is recommended that your effect have an
   easy way to determine which controls it implements: a) by having storage be a
   pointer with an OSType at the begining to mark controls implemented by your
   code.
     - or -
     b) keeping track in your component globals those custom controls which you
   have created.

    When pdActionCustomDisposeControl is called any allocation done by the
   control should be disposed. In addition, pdActionCustomDisposeControl is the
   last chance the control has to commit any user changes into the sample.
    Controls which implement type in fields typically need to commit any final
   user edits at this time.
*/
struct QTCustomControlNewRecord {
  ...
  long customType; /* custom type and ID specified by effect for creation of
                      this control*/
  long customID;
};
...
/*
    pdActionCustomPositionControl is called by the application to position the
   control within a window or dialog.

    The control should determine if it will fit in the alloted area and position
   itself there.  It should also return the space taken up by the control. Note
   you are free to implement controls which are variable in size depending upon
    which parameter you are editing.  You need not scale your control to the
   requested size.  If the area presented to your control is too small, set
   didFit to false.  You should still return in used the size you would have
   liked to use for the control.   The application will then try again with a
   new size.  Note that all controls must be able to fit within a minimum of 300
   by 250 pixels.

    Custom controls that draw text should make note of the text font, size, and
   style at this time in order to properly display within application windows.

    Note that the default state for the control is hidden.  You will receive a
   pdActionCustomShowHideControl in order to enable your control.  You should
   not draw your control in response to pdActionCustomPositionControl.
*/
...
typedef struct QTCustomControlPositionControlRecord
    QTCustomControlPositionControlRecord;
typedef QTCustomControlPositionControlRecord *QTCustomControlPositionControlPtr;
/*
    pdActionCustomShowHideControl is called when the application wishes to
   enable/disable your control, or completely disable drawing of the control

    Your control should make note of the new state (if different from the last)
   and perform an InvalRect() on your drawing area, or you may draw your
   control's initial state in the case of show.  You should not attempt to erase
   your control as the result of a hide -- instead call InvalRect() and allow
...
typedef struct QTCustomControlShowHideControlRecord
    QTCustomControlShowHideControlRecord;
typedef QTCustomControlShowHideControlRecord *QTCustomControlShowHideControlPtr;
/*
    pdActionCustomHandleEvent is called to allow your custom control to process
   events.
    ...
    you handled the event, but other controls still need the event, set
   didProcess to false.

    If your control supports the concept of focus for the purposes of typing
   (such as by having a type-in box for the parameter) then you set the
   tookFocus Boolean as part of your processing of the event.  It is assumed
   that your control will draw the appropriate focus UI as a result, and the
   calling application will disable any focus drawing within the remainder of
   the UI.

    By default, custom controls are not given idle time.  If you need idle time,
   set needIdle to true in response to the even that causes you to need idle
   (typically the taking of focus, or the first draw). Your control will
   continue to be given idle events until you set needIdle to false in response
   to a nullEvent.
*/
struct QTCustomControlHandleEventRecord {
  ...
  Boolean tookFocus;   /* did we take focus as a result of this event (typically
                          mouseDowns)*/
...
/*
    pdActionCustomSetFocus is called in order to set or advance the current
   focus of the user interface, typically because the user has pressed the tab
   or shift-tab keys, or because the user clicked within the area defined by
    your control.

    Your control will be called with pdActionFocusFirst,  pdActionFocusLast, or
   pdActionFocusOff to set or clear focus on your control.  Your control will be
   called with pdActionFocusForward or pdActionFocusBackward to cycle focus
   within your control (if your control has multiple focus).  If your control
   does not support focus, or the focus request results in focus moving beyond
   your supported range, return pdActionFocusOff in the focus parameter.
   Otherwise, return the focus that you set.

    Controls which have no focus would always set focus to be pdActionFocusOff.

    Controls with a single focus would set pdActionFocusFirst when requsted to
   set either pdActionFocusFirst or pdActionFocusLast, and would set
   pdActionFocusOff for either pdActionFocusForward or pdActionFocusBackward.
*/
enum {
...
/*
    pdActionCustomSetEditMenu will be called to inform your custom control of
   the location of the edit menu.

    If your control has editing boxes, this is useful in order to allow the user
   to perform cut/copy/paste operations when focus is on one of these boxes.
*/
struct QTCustomControlSetEditMenuRecord {
  void *storage;       /* storage for the control*/
  MenuHandle editMenu; /* edit menu, or NIL*/
};
typedef struct QTCustomControlSetEditMenuRecord
    QTCustomControlSetEditMenuRecord;
typedef QTCustomControlSetEditMenuRecord *QTCustomControlSetEditMenuPtr;
/*
    pdActionCustomSetPreviewPicture will be called to inform your custom control
   of preview information that you may wish to use in the drawing of your user
   interface.
*/
...
typedef struct QTCustomControlSetPreviewPictureRecord
    QTCustomControlSetPreviewPictureRecord;
typedef QTCustomControlSetPreviewPictureRecord
    *QTCustomControlSetPreviewPicturePtr;
/*
    pdActionCustomSetEditCallout tells your control of the need by the
   application to be informed of changes to the parameter values (typically for
   the purposes of updating previews).

    If a callout is available, your custom control should call it whenever a
   change has been made to the parameter(s) that your control is editing (as a
   result of user actions, most typically). If you choose not to implement this,
   live dragging or updating of values will not work.
*/
struct QTCustomControlSetEditCalloutRecord {
  void *storage;                    /* storage for the control*/
  QTParamPreviewCalloutPtr callout; /* requested callout, or NIL to disable*/
};
typedef struct QTCustomControlSetEditCalloutRecord
    QTCustomControlSetEditCalloutRecord;
typedef QTCustomControlSetEditCalloutRecord *QTCustomControlSetEditCalloutPtr;
/*
    pdActionCustomGetEnableValue allows you to return a value for the purposes
   of enabling/disabling other controls. Most custom controls do not need to
   implement this call.

    If your control is able to control the enabling and disabling of other
   parameter controls (such as is done by standard pop up or enumerated type
   controls), you need to supply a value that can be use for greater than/less
   than types of comparisons.
*/
struct QTCustomControlGetEnableValueRecord {
  void *storage;     /* storage for the control*/
  long currentValue; /* value to compare against for enable/disable purposes*/
};
typedef struct QTCustomControlGetEnableValueRecord
    QTCustomControlGetEnableValueRecord;
typedef QTCustomControlGetEnableValueRecord *QTCustomControlGetEnableValuePtr;
/*
    pdActionCustomSetSampleTime tells your control information from the
   application about the duration and start time for the sample being edited.

    Most controls do not need this information, but some may choose to use it in
   the interface they present the user.  However, this call need not be made by
   applications, so the custom control should be prepared to run when the sample
...
typedef struct QTCustomControlSetSampleTimeRecord
    QTCustomControlSetSampleTimeRecord;
typedef QTCustomControlSetSampleTimeRecord *QTCustomControlSetSampleTimePtr;
/*
    pdActionCustomGetValue tells your control to store any value(s) into the
   specified atom container.
...
/*
    pdActionCustomDoEditCommand tells your control to handle edit commands if it
   allow focus and type in boxes.
...
typedef struct QTCustomControlDoEditCommandRecord
    QTCustomControlDoEditCommandRecord;
...
/*
    ...
    that you prefer.
*/
typedef CALLBACK_API(Boolean, QTEffectListFilterProcPtr)(
    Component effect, long effectMinSource, long effectMaxSource,
    OSType majorClass, OSType minorClass, void *refcon);
 ...
 */
EXTERN_API(OSErr)
QTGetEffectsListExtended(QTAtomContainer *returnedList, long minSources,
                         long maxSources, QTEffectListOptions getOptions,
                         OSType majorClass, OSType minorClass,
...
/*
    SetMovieAudioGain:
    This API sets the audio gain level for the mixed audio output of a movie.
   This alters the perceived volume of the movie's playback.  The gain level is
   multiplicative; eg. 0.0 is silent, 0.5 is -6dB, 1.0 is 0dB (ie. the audio
   from the movie is not modified), 2.0 is +6dB, etc.  The gain level can be set
   higher than 1.0 in order to allow quiet movies to be boosted in volume.
   Settings higher than 1.0 may result in audio clipping, of course.  The
   setting is not stored in the movie.  It is only used until the movie is
   closed, at which time it is not saved.
 */
/*
 *  SetMovieAudioGain()
 *
...
/*
    SetTrackAudioGain:
    This API sets the audio gain level for the audio output of a track.  This
   alters the perceived volume of the track's playback.  The gain level is
   multiplicative; eg. 0.0 is silent, 0.5 is -6dB, 1.0 is 0dB (ie. the audio
   from the track is not modified), 2.0 is +6dB, etc.  The gain level can be set
   higher than 1.0 in order to allow quiet tracks to be boosted in volume.
   Settings higher than 1.0 may result in audio clipping, of course.  The
   setting is not stored in the movie.  It is only used until the movie is
   closed, at which time it is not saved.
 */
...
/*
    ...
    This API sets the audio balance level for the mixed audio output of a movie.
   -1.0 means full left, 0.0 means centered, and 1.0 means full right.  The
   setting is not stored in the movie.  It is only used until the movie is
   closed, at which time it is not saved.
 */
/*
...
enum {
  ...
  kQTAudioPropertyID_Gain =
      FOUR_CHAR_CODE('gain'), /* value is Float32. Gettable/Settable.*/
  ...
  kQTAudioPropertyID_Mute =
      FOUR_CHAR_CODE('mute'), /* value is Boolean. Gettable/Settable.*/
  ...
  kQTAudioPropertyID_Balance =
      FOUR_CHAR_CODE('bala'), /* value is Float32. Gettable/Settable.*/
  ...
  kQTAudioPropertyID_RateChangesPreservePitch =
      FOUR_CHAR_CODE('aucp'), /* value is Boolean.  Gettable/Settable.*/
  ...
  kQTAudioPropertyID_Pitch =
      FOUR_CHAR_CODE('pitc'), /* value is Float32. Get/Set/Listenable.*/
  ...
  kQTAudioPropertyID_RenderQuality =
      FOUR_CHAR_CODE('qual'), /* value is UInt32.  Gettable/Settable.*/
   ...
   */
  kQTAudioPropertyID_ChannelLayout = FOUR_CHAR_CODE(
      'tlay'), /* value is AudioChannelLayout. Gettable/Settable.*/
  ...
  kQTAudioPropertyID_SummaryChannelLayout =
      FOUR_CHAR_CODE('clay'), /* value is AudioChannelLayout. Gettable.*/
  ...
  kQTAudioPropertyID_DeviceChannelLayout =
      FOUR_CHAR_CODE('dcly'), /* value is AudioChannelLayout. Gettable.*/
   ...
   */
  kQTAudioPropertyID_DeviceASBD = FOUR_CHAR_CODE(
      'dasd'), /* value is AudioStreamBasicDescription. Gettable.*/
   ...
   * kQTAudioPropertyID_SummaryChannelLayout.
   */
  kQTAudioPropertyID_SummaryASBD = FOUR_CHAR_CODE(
      'sasd'), /* value is AudioStreamBasicDescription. Gettable.*/
  ...
  kQTAudioPropertyID_FormatString =
      FOUR_CHAR_CODE('fstr'), /* value is CFStringRef.  Gettable.*/
  ...
  kQTAudioPropertyID_ChannelLayoutString =
      FOUR_CHAR_CODE('lstr'), /* value is CFStringRef.  Gettable.*/
  ...
  kQTAudioPropertyID_SampleRateString =
      FOUR_CHAR_CODE('rstr'), /* value is CFStringRef.  Gettable.*/
  ...
  kQTAudioPropertyID_SampleSizeString =
      FOUR_CHAR_CODE('sstr'), /* value is CFStringRef.  Gettable.*/
  ...
  kQTAudioPropertyID_BitRateString =
      FOUR_CHAR_CODE('bstr'), /* value is CFStringRef.  Gettable.*/
  ...
  kQTAudioPropertyID_SummaryString =
      FOUR_CHAR_CODE('asum') /* value is CFStringRef.  Gettable.*/
};
...
/*
    ...
    This API enables or disables volume metering of a particular mix of this
   movie.  The only possible mix to meter is currently kQTAudioMeter_DeviceMix.
   See kQTAudioMeter_DeviceMix above to see how to determine the channel layout
   of the movie's device mix.
 */
/*
 *  SetMovieAudioVolumeMeteringEnabled()
 *
 ...
 */
EXTERN_API(OSStatus)
SetMovieAudioVolumeMeteringEnabled(Movie m, FourCharCode whatMixToMeter,
                                   Boolean enabled);
 ...
 */
EXTERN_API(OSStatus)
GetMovieAudioVolumeMeteringEnabled(Movie m, FourCharCode whatMixToMeter,
                                   Boolean *enabled);

/*
    GetMovieAudioVolumeLevels:
    This API returns the current volume meter levels of the movie.  It can
   return both average power levels and peak hold levels.  whatMixToMeter must
   be set to kQTAudioMeter_DeviceMix.  Either QTAudioVolumeLevels parameter may
   be nil.  If non-nil, each must have its numChannels field set to the number
   of channels in the movie's device mix, and must be allocated large enough to
   hold levels for all those channels.  See kQTAudioMeter_DeviceMix above to see
   how to determine the channel layout of the device mix. The levels returned
   are measured in decibels, where 0.0 means full volume, -6.0 means half
   volume, -12.0 means quarter volume, and -inf means silence.
 */
 ...
 */
EXTERN_API(OSStatus)
GetMovieAudioVolumeLevels(Movie m, FourCharCode whatMixToMeter,
                          QTAudioVolumeLevels *pAveragePowerLevels,
                          QTAudioVolumeLevels *pPeakHoldLevels);

/*
    SetTrackAudioVolumeMeteringEnabled:
    This API enables or disables volume metering of a particular track of this
   movie. This API should be used in preference to the legacy SoundMedia
   interface, but may interfere with its operation if both are in use at the
   same time.
*/
/*
 *  SetTrackAudioVolumeMeteringEnabled()
...
/*
    ...
    This API returns the current volume meter levels of the track.  It can
   return both average power levels and peak hold levels.  Either
   QTAudioVolumeLevels parameter may be nil.  If non-nil, each must have its
   numChannels field set to the number of channels of interest, and must be
 ...
 */
EXTERN_API(OSStatus)
GetTrackAudioVolumeLevels(Track t, QTAudioVolumeLevels *pAveragePowerLevels,
                          QTAudioVolumeLevels *pPeakHoldLevels);

/*
    SetMovieAudioFrequencyMeteringNumBands:
    This API configures and enables (or disables) frequency metering for a
   movie. Note that ioNumBands is an in/out parameter.  You specify the number
   of frequency bands you want to meter, and if that number is higher than is
   possible (determined by, among other things, the sample rate of the audio
   being metered), this API will return the number of bands it is actually going
   to meter.  ioNumBands can be nil or a pointer to 0 to disable metering.
    whatMixToMeter must be set to kQTAudioMeter_StereoMix,
   kQTAudioMeter_MonoMix, or kQTAudioMeter_DeviceMix.  When metering movies
   playing to audio devices that offer a large number of channels, it may be
   prohibitively expensive to perform spectral analysis on every channel; in
   these cases, stereo or mono mix metering may be preferable.
 */
/*
 *  SetMovieAudioFrequencyMeteringNumBands()
 *
 ...
 */
EXTERN_API(OSStatus)
SetMovieAudioFrequencyMeteringNumBands(Movie m, FourCharCode whatMixToMeter,
                                       UInt32 *ioNumBands);
 ...
 */
EXTERN_API(OSStatus)
GetMovieAudioFrequencyMeteringNumBands(Movie m, FourCharCode whatMixToMeter,
                                       UInt32 *outNumBands);

/*
    GetMovieAudioFrequencyMeteringBandFrequencies:
    This API returns the actual chosen middle frequency for each band in the
   configured frequency metering of a movie.  This is useful for labeling visual
   meters in a user interface.  Frequencies are returned in Hz.  whatMixToMeter
   must be set to the same value that was passed most recently to
   SetMovieAudioFrequencyMeteringNumBands().
 */
/*
 *  GetMovieAudioFrequencyMeteringBandFrequencies()
 ...
 */
EXTERN_API(OSStatus)
GetMovieAudioFrequencyMeteringBandFrequencies(Movie m,
...
/*
    ...
    This API returns the current frequency meter levels of the movie.
   pAveragePowerLevels should have its numChannels field set to the number of
   channels being metered, and its numBands field set to the number of bands
   being metered (as previously configured).  pAveragePowerLevels must be
    allocated large enough to hold levels for all bands in all channels.  The
   levels are returned with all the band levels for the first channel first,
   then all the band levels for the second channel, etc. whatMixToMeter must be
   set  to the same value that was passed most recently to
    SetMovieAudioFrequencyMeteringNumBands().
 */
struct QTAudioFrequencyLevels {
  UInt32 numChannels;
  UInt32 numFrequencyBands;
  /* numChannels * numFrequencyBands entries, with the frequency bands for a
   * single channel stored contiguously.*/
  Float32 level[1];
};
 ...
 */
EXTERN_API(OSStatus)
GetMovieAudioFrequencyLevels(Movie m, FourCharCode whatMixToMeter,
...
/*
    ...
    This API must be called before doing any movie audio extraction.  The
   returned session object is to be passed to the other movie audio extraction
   APIs.  Note that the extracted format defaults to the aggregate channel
   layout of the movie (eg. all Rights mixed together, all Left Surrounds mixed
   together, etc), 32-bit float, de-interleaved, with the sample rate set to the
   highest sample rate found in the movie.  You can get this info, and you can
   also set the format to be something else (as long as it is uncompressed, and
   you do it before the first call to MovieAudioExtractionFillBuffer).
 */
 ...
 */
EXTERN_API(OSStatus)
MovieAudioExtractionBegin(Movie m, UInt32 flags,
...
enum {
  ...
  kQTMovieAudioExtractionMoviePropertyID_CurrentTime =
      FOUR_CHAR_CODE('time'), /* value is TimeRecord. Get/Set.*/
  ...
  kQTMovieAudioExtractionAudioPropertyID_RenderQuality =
      FOUR_CHAR_CODE('qual') /* value is UInt32. Get/Set.*/
};
...
enum {
  ...
  kQTMovieAudioExtractionAudioPropertyID_AudioStreamBasicDescription =
      FOUR_CHAR_CODE(
          'asbd'), /* value is AudioStreamBasicDescription. Get/Set.*/
 ...
 */
EXTERN_API(OSStatus)
MovieAudioExtractionGetPropertyInfo(MovieAudioExtractionRef session,
                                    QTPropertyClass inPropClass,
                                    QTPropertyID inPropID,
                                    QTPropertyValueType *outPropType,
                                    ByteCount *outPropValueSize,
 ...
 */
EXTERN_API(OSStatus)
MovieAudioExtractionGetProperty(MovieAudioExtractionRef session,
                                QTPropertyClass inPropClass,
                                QTPropertyID inPropID,
 ...
 */
EXTERN_API(OSStatus)
MovieAudioExtractionSetProperty(MovieAudioExtractionRef session,
                                QTPropertyClass inPropClass,
                                QTPropertyID inPropID,
                                ByteCount inPropValueSize,
                                ConstQTPropertyValuePtr inPropValueAddress);
...
/*
    ...
    Each call to MovieAudioExtractionFillBuffer will continue where the last
   call left off. It will extract as many of the requested PCM frames as it can,
   given the limits of the buffer(s) supplied, and the limits of the input
   movie.  ioNumFrames will be updated with the exact number of valid frames
   being returned. When there is no more audio to extract from the movie,
   MovieAudioExtractionFillBuffer will continue to return noErr, but no audio
   data will be returned.  outFlags will have the
   kQTMovieAudioExtractionComplete bit set in this case.  It is possible that
   the kQTMovieAudioExtractionComplete bit will accompany the last buffer of
   valid data.
 */
enum { kQTMovieAudioExtractionComplete = (1L << 0) };
 ...
 */
EXTERN_API(OSStatus)
MovieAudioExtractionFillBuffer(MovieAudioExtractionRef session,
                               UInt32 *ioNumFrames, AudioBufferList *ioData,
                               UInt32 *outFlags);
   ...
   Theory of operations:
    To register for a Movie Audio Context Insert during Movie Audio Extraction:
        Set the kQTMovieAudioExtractionAudioPropertyID_RegisterMovieInsert
            property on the extraction session, providing the same registry info
   structure that is used for the QTAudioContextRegisterInsert call. To register
   for a Track Audio Context Insert during Movie Audio Extraction: Set the
   kQTMovieAudioExtractionAudioPropertyID_RegisterTrackInsert property on the
   extraction session, providing a QTAudioTrackInsertRegistryInfoRef, described
   below. Note: Once extraction has begun (ie, MovieAudioExtractionFillBuffer()
   has been called), attempts to set these properties will return qtReadOnlyErr.
...
/* Movie and Track level audio context inserts for extraction
 * (kQTPropertyClass_MovieAudioExtraction_Audio)*/
...
enum {
   ...
   */
  kQTMovieAudioExtractionAudioPropertyID_RegisterMovieInsert = FOUR_CHAR_CODE(
      'regm'), /* value is QTAudioContextInsertRegistryInfoRef. Get/Set.*/
   ...
   * will be ignored).
   */
  kQTMovieAudioExtractionAudioPropertyID_RegisterTrackInsert = FOUR_CHAR_CODE(
      'regt') /* value is QTAudioTrackInsertRegistryInfoRef. Get/Set.*/
};
...
enum {
   ...
   */
  kQTPerformancePropertyID_MediaStallCount =
      FOUR_CHAR_CODE('stal'), /* UInt32, Get/Set/Listenable */
   ...
   */
  kQTPerformancePropertyID_AudioIOOverloadCount =
      FOUR_CHAR_CODE('ovct') /* UInt32, Get/Set/Listenable*/
};
...
enum {
   ...
   */
  kQTVisualPropertyID_Saturation =
      FOUR_CHAR_CODE('vsat'), /* Float32, Read/Write */
   ...
   * means full white.
   */
  kQTVisualPropertyID_Brightness =
      FOUR_CHAR_CODE('vbrt'), /* Float32, Read/Write */

  /*
   * The contrast adjustment for the movie.  The value is a Float32
   * percentage (1.0f = 100%), such that 0.0 gives solid grey.
   */
  kQTVisualPropertyID_Contrast =
      FOUR_CHAR_CODE('vcon') /* Float32, Read/Write */
};
...
enum {
  ...
  kQTVisualPropertyID_ApertureMode =
      FOUR_CHAR_CODE('apmd') /* OSType, Read/Write/Listen */
};
...
enum {
  ...
  kQTVisualPropertyID_HasApertureModeDimensions =
      FOUR_CHAR_CODE('hamd') /* Boolean, Read */
};
...
EXTERN_API_C(OSErr)
SetTrackApertureModeDimensionsUsingSampleDescription(
    Track track, SampleDescriptionHandle sampleDesc); /* can be NULL */
...
struct QTErrorReplacementRecord {
  long numEntries;
  StringPtr replacementString[1]; /* array of numEntries StringPtrs (each String
...
/*
    QTAddMovieError is used to add orthogonal errors to a list of errors that
   will later be reported (at the end of an import or playback, for example).
   Errors are stored in 'qter' resources within the component.

    QTAddMovieError(Movie       addTo,                          // in: movie to
   add error to Component   adder,                          // in: component
   which is adding the error long        errorCode,                      // in:
   error code being added QTErrorReplacementPtr   stringReplacements);// in:
   list of strings to subsitute (in order) for "^1", "^2", etc
*/
/*
 *  QTAddMovieError()
 *
 ...
 */
EXTERN_API(OSErr)
QTAddMovieError(Movie movieH, Component c, long errorCode,
 ...
 */
EXTERN_API(OSErr)
QTGetMovieRestrictions(Movie theMovie, QTRestrictionSet *outRestrictionSet,
                       UInt32 *outSeed);
...
QTRestrictionsGetInfo(QTRestrictionSet inRestrictionSet,
                      long *outRestrictionClassCount, long *outSeed);
 ...
 */
EXTERN_API(OSErr)
QTRestrictionsGetIndClass(QTRestrictionSet inRestrictionSet, long inIndex,
                          OSType *outClass);
...
QTRestrictionsGetItem(QTRestrictionSet inRestrictionSet,
                      OSType inRestrictionClass, UInt32 *outRestrictions);
...
#ifdef __cplusplus
inline DEFINE_API_C(QTCallBackUPP)
    NewQTCallBackUPP(QTCallBackProcPtr userRoutine) {
...
#ifdef __cplusplus
inline DEFINE_API_C(QTSyncTaskUPP)
    NewQTSyncTaskUPP(QTSyncTaskProcPtr userRoutine) {
...
#ifdef __cplusplus
inline DEFINE_API_C(MovieRgnCoverUPP)
    NewMovieRgnCoverUPP(MovieRgnCoverProcPtr userRoutine) {
...
#ifdef __cplusplus
inline DEFINE_API_C(MovieProgressUPP)
    NewMovieProgressUPP(MovieProgressProcPtr userRoutine) {
...
#ifdef __cplusplus
inline DEFINE_API_C(MovieDrawingCompleteUPP)
    NewMovieDrawingCompleteUPP(MovieDrawingCompleteProcPtr userRoutine) {
...
#ifdef __cplusplus
inline DEFINE_API_C(TrackTransferUPP)
    NewTrackTransferUPP(TrackTransferProcPtr userRoutine) {
...
#ifdef __cplusplus
inline DEFINE_API_C(MoviePreviewCallOutUPP)
    NewMoviePreviewCallOutUPP(MoviePreviewCallOutProcPtr userRoutine) {
...
#ifdef __cplusplus
inline DEFINE_API_C(TextMediaUPP)
    NewTextMediaUPP(TextMediaProcPtr userRoutine) {
...
#ifdef __cplusplus
inline DEFINE_API_C(DoMCActionUPP)
    NewDoMCActionUPP(DoMCActionProcPtr userRoutine) {
...
#ifdef __cplusplus
inline DEFINE_API_C(MovieExecuteWiredActionsUPP) NewMovieExecuteWiredActionsUPP(
...
inline DEFINE_API_C(QTNextTaskNeededSoonerCallbackUPP)
  ...
  return (QTNextTaskNeededSoonerCallbackUPP)NewRoutineDescriptor(
      (ProcPtr)(userRoutine), uppQTNextTaskNeededSoonerCallbackProcInfo,
      GetCurrentArchitecture());
}
#else
...
#ifdef __cplusplus
inline DEFINE_API_C(MoviesErrorUPP)
    NewMoviesErrorUPP(MoviesErrorProcPtr userRoutine) {
...
enum {
  uppTweenerDataProcInfo = 0x003FFFF0
      ...
      4_bytes, 4_bytes) */
#ifdef __cplusplus
inline DEFINE_API_C(TweenerDataUPP)
    NewTweenerDataUPP(TweenerDataProcPtr userRoutine) {
...
enum {
  uppQTEffectListFilterProcInfo = 0x0003FFD0
    ...
    */
#ifdef __cplusplus
inline DEFINE_API_C(QTEffectListFilterUPP)
    NewQTEffectListFilterUPP(QTEffectListFilterProcPtr userRoutine) {
  return (QTEffectListFilterUPP)NewRoutineDescriptor(
      (ProcPtr)(userRoutine), uppQTEffectListFilterProcInfo,
      GetCurrentArchitecture());
}
#else
#define NewQTEffectListFilterUPP(userRoutine)                                  \
  (QTEffectListFilterUPP) NewRoutineDescriptor((ProcPtr)(userRoutine),         \
                                               uppQTEffectListFilterProcInfo,  \
                                               GetCurrentArchitecture())
#endif
#endif
...
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
inline DEFINE_API_C(void) DisposeQTNextTaskNeededSoonerCallbackUPP(
    QTNextTaskNeededSoonerCallbackUPP userUPP) {
  DisposeRoutineDescriptor((UniversalProcPtr)userUPP);
}
#else
...
#ifdef __cplusplus
inline DEFINE_API_C(OSErr)
    InvokeDoMCActionUPP(void *refcon, short action, void *params,
...
#ifdef __cplusplus
inline DEFINE_API_C(void)
    InvokeMoviePrePrerollCompleteUPP(Movie theMovie, OSErr prerollErr,
                                     void *refcon,
                                     MoviePrePrerollCompleteUPP userUPP) {
...
EXTERN_API_C(void)
InvokeQTNextTaskNeededSoonerCallbackUPP(
    TimeValue duration, unsigned long flags, void *refcon,
    QTNextTaskNeededSoonerCallbackUPP userUPP);
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
inline DEFINE_API_C(void) InvokeQTNextTaskNeededSoonerCallbackUPP(
    TimeValue duration, unsigned long flags, void *refcon,
    QTNextTaskNeededSoonerCallbackUPP userUPP) {
  CALL_THREE_PARAMETER_UPP(userUPP, uppQTNextTaskNeededSoonerCallbackProcInfo,
                           duration, flags, refcon);
}
#else
#define InvokeQTNextTaskNeededSoonerCallbackUPP(duration, flags, refcon,       \
                                                userUPP)                       \
  CALL_THREE_PARAMETER_UPP((userUPP),                                          \
                           uppQTNextTaskNeededSoonerCallbackProcInfo,          \
                           (duration), (flags), (refcon))
#endif
#endif
 ...
 */
EXTERN_API_C(Boolean)
InvokeQTEffectListFilterUPP(Component effect, long effectMinSource,
                            long effectMaxSource, OSType majorClass,
...
inline DEFINE_API_C(Boolean)
  ...
  return (Boolean)CALL_SIX_PARAMETER_UPP(
      userUPP, uppQTEffectListFilterProcInfo, effect, effectMinSource,
      effectMaxSource, majorClass, minorClass, refcon);
}
#else
#define InvokeQTEffectListFilterUPP(effect, effectMinSource, effectMaxSource,  \
                                    majorClass, minorClass, refcon, userUPP)   \
...
#define NewQTNextTaskNeededSoonerCallbackProc(userRoutine)                     \
  NewQTNextTaskNeededSoonerCallbackUPP(userRoutine)
...
#define NewQTEffectListFilterProc(userRoutine)                                 \
  NewQTEffectListFilterUPP(userRoutine)
...
#define CallQTNextTaskNeededSoonerCallbackProc(userRoutine, duration, flags,   \
                                               refcon)                         \
...
#define CallTweenerDataProc(userRoutine, tr, tweenData, tweenDataSize,         \
  ...
  InvokeTweenerDataUPP(tr, tweenData, tweenDataSize, dataDescriptionSeed,      \
                       ...
                       refCon, userRoutine)
#define CallQTEffectListFilterProc(userRoutine, effect, effectMinSource,       \
                                   effectMaxSource, majorClass, minorClass,    \
                                   refcon)                                     \
  InvokeQTEffectListFilterUPP(effect, effectMinSource, effectMaxSource,        \
                              majorClass, minorClass, refcon, userRoutine)
...
enum {
  ...
  kQTRestrictionSaveDontExtract =
      (1L << 3), /* don't allow any form of extraction of content*/
  ...
  kQTRestrictionEditDontModify =
      (1L << 4), /* don't allow modification of content*/
  kQTRestrictionEditDontExtract =
      (1L << 5), /* don't allow any form of extraction of content*/
...
enum {
  ...
  spriteHitTestTreatAllSpritesAsHitTestable =
      1L << 5 /* set if you want to override each sprites hittestable property
                 as true*/
};
 ...
 */
EXTERN_API(ComponentResult)
SpriteMediaNewImage(MediaHandler mh, Handle dataRef, OSType dataRefType,
                    QTAtomID desiredID)
 ...
 */
EXTERN_API(ComponentResult)
SpriteMediaDisposeImage(MediaHandler mh, short imageIndex)
    FIVEWORDINLINE(0x2F3C, 0x0002, 0x011C, 0x7000, 0xA82A);
...
typedef CALLBACK_API(Boolean, MCActionNotificationProcPtr)(
    MovieController mc, short action, void *params, UInt32 inFlags,
    UInt32 *outFlags, void *refCon);
...
enum { kKeyboardAllowFocus = 1, kKeyboardHaveFocus = 2, kKeyboardValidate = 4 };

enum { kRefConNavigateClick = FOUR_CHAR_CODE('clik') };
...
struct QTMCActionNotificationRecord {
  OSType returnSignature; /* Set to zero when passed to movieController, set to
                             'noti' if mcActionAddActionNotification is
                             implemented*/
 ...
 */
EXTERN_API(ComponentResult)
MCSetIdleManager(MovieController mc, IdleManager im)
    FIVEWORDINLINE(0x2F3C, 0x0004, 0x0035, 0x7000, 0xA82A);

/* Called (but not implemented) by controllers that derive from the standard
   movie controller. All controllers except standard movie controller must
   delegate this call. */
enum { kControllerUnderstandsIdleManagers = 1 << 0 };
 ...
 */
EXTERN_API(OSErr)
GetTimeBaseRateChangeStatus(TimeBase tb, TimeScale scale, Fixed *ratedChangedTo,
 ...
 */
EXTERN_API(OSErr)
SetTimeBaseOffsetTimeBase(TimeBase tb, TimeBase offsettb,
 ...
 */
EXTERN_API(OSErr)
GetTimeBaseThreadAttachState(TimeBase inTimeBase,
...
typedef CALLBACK_API(void, QTMoviePropertyListenerProcPtr)(
    Movie inMovie, QTPropertyClass inPropClass, QTPropertyID inPropID,
    void *inUserData);
typedef STACK_UPP_TYPE(QTMoviePropertyListenerProcPtr)
 ...
 */
EXTERN_API(OSErr)
QTGetMoviePropertyInfo(Movie inMovie, QTPropertyClass inPropClass,
 ...
 */
EXTERN_API(OSErr)
QTGetMovieProperty(Movie inMovie, QTPropertyClass inPropClass,
                   QTPropertyID inPropID, ByteCount inPropValueSize,
                   QTPropertyValuePtr outPropValueAddress,
                   ByteCount *outPropValueSizeUsed);
 ...
 */
EXTERN_API(OSErr)
QTSetMovieProperty(Movie inMovie, QTPropertyClass inPropClass,
                   QTPropertyID inPropID, ByteCount inPropValueSize,
                   ConstQTPropertyValuePtr inPropValueAddress);
 ...
 */
EXTERN_API(OSErr)
QTAddMoviePropertyListener(Movie inMovie, QTPropertyClass inPropClass,
                           QTPropertyID inPropID,
 ...
 */
EXTERN_API(OSErr)
QTRemoveMoviePropertyListener(Movie inMovie, QTPropertyClass inPropClass,
                              QTPropertyID inPropID,
                              QTMoviePropertyListenerUPP inListenerProc,
                              void *inUserData);
...
typedef CALLBACK_API(void, QTTrackPropertyListenerProcPtr)(
    Track inTrack, QTPropertyClass inPropClass, QTPropertyID inPropID,
    void *inUserData);
typedef STACK_UPP_TYPE(QTTrackPropertyListenerProcPtr)
 ...
 */
EXTERN_API(OSErr)
QTGetTrackPropertyInfo(Track inTrack, QTPropertyClass inPropClass,
                       QTPropertyID inPropID, QTPropertyValueType *outPropType,
                       ByteCount *outPropValueSize, UInt32 *outPropertyFlags);
 ...
 */
EXTERN_API(OSErr)
QTGetTrackProperty(Track inTrack, QTPropertyClass inPropClass,
                   QTPropertyID inPropID, ByteCount inPropValueSize,
                   QTPropertyValuePtr outPropValueAddress,
                   ByteCount *outPropValueSizeUsed);
 ...
 */
EXTERN_API(OSErr)
QTSetTrackProperty(Track inTrack, QTPropertyClass inPropClass,
                   QTPropertyID inPropID, ByteCount inPropValueSize,
                   ConstQTPropertyValuePtr inPropValueAddress);
 ...
 */
EXTERN_API(OSErr)
QTAddTrackPropertyListener(Track inTrack, QTPropertyClass inPropClass,
                           QTPropertyID inPropID,
 ...
 */
EXTERN_API(OSErr)
QTRemoveTrackPropertyListener(Track inTrack, QTPropertyClass inPropClass,
                              QTPropertyID inPropID,
                              QTTrackPropertyListenerUPP inListenerProc,
                              void *inUserData);
 ...
 */
EXTERN_API(OSStatus)
QTSampleTableCreateMutable(CFAllocatorRef allocator, TimeScale timescale,
                           void *hints,
 ...
 */
EXTERN_API(OSStatus)
QTSampleTableCreateMutableCopy(CFAllocatorRef allocator,
                               QTSampleTableRef sampleTable, void *hints,
                               QTMutableSampleTableRef *newSampleTable);
 ...
 */
EXTERN_API(OSStatus)
QTSampleTableSetTimeScale(QTMutableSampleTableRef sampleTable,
 ...
 */
EXTERN_API(OSStatus)
QTSampleTableGetNextAttributeChange(QTSampleTableRef sampleTable,
...
EXTERN_API(OSStatus)
QTSampleTableCopySampleDescription(
    QTSampleTableRef sampleTable, QTSampleDescriptionID sampleDescriptionID,
 ...
 */
EXTERN_API(OSStatus)
QTSampleTableAddSampleReferences(QTMutableSampleTableRef sampleTable,
                                 SInt64 dataOffset, ByteCount dataSizePerSample,
                                 TimeValue64 decodeDurationPerSample,
                                 TimeValue64 displayOffset,
 ...
 */
EXTERN_API(OSStatus)
QTSampleTableReplaceRange(QTMutableSampleTableRef destSampleTable,
                          SInt64 destStartingSampleNum, SInt64 destSampleCount,
...
QTSampleTableGetDataSizePerSample(QTSampleTableRef sampleTable,
                                  SInt64 sampleNum);
...
QTSampleTableGetSampleDescriptionID(QTSampleTableRef sampleTable,
                                    SInt64 sampleNum);
...
enum {
  ...
  kQTSampleTablePropertyID_TotalDecodeDuration =
      FOUR_CHAR_CODE('tded'), /* TimeValue64, Read */
  ...
  kQTSampleTablePropertyID_MinDisplayOffset =
      FOUR_CHAR_CODE('<ddd'), /* TimeValue64, Read */
  ...
  kQTSampleTablePropertyID_MinRelativeDisplayTime =
      FOUR_CHAR_CODE('<dis'), /* TimeValue64, Read */
 ...
 */
EXTERN_API(OSStatus)
QTSampleTableGetPropertyInfo(QTSampleTableRef sampleTable,
                             QTPropertyClass inPropClass, QTPropertyID inPropID,
 ...
 */
EXTERN_API(OSStatus)
QTSampleTableGetProperty(QTSampleTableRef sampleTable,
                         QTPropertyClass inPropClass, QTPropertyID inPropID,
                         ByteCount inPropValueSize,
                         QTPropertyValuePtr outPropValueAddress,
 ...
 */
EXTERN_API(OSStatus)
QTSampleTableSetProperty(QTSampleTableRef sampleTable,
                         QTPropertyClass inPropClass, QTPropertyID inPropID,
                         ByteCount inPropValueSize,
                         ConstQTPropertyValuePtr inPropValueAddress);
 ...
 *  Common Key Format                   *
 ***************************************/
enum { kQTMetaDataKeyFormatCommon = FOUR_CHAR_CODE('comn') };
...
enum {
  kQTMetaDataStorageFormatQuickTime =
      FOUR_CHAR_CODE('mdta') /* QuickTime metadata storage format*/
};
...
enum {
  kQTMetaDataStorageFormatiTunes =
      FOUR_CHAR_CODE('itms') /* iTunes metadata storage format*/
};

/* QTMetaDataKeyFormat type*/
enum {
  kQTMetaDataKeyFormatiTunesShortForm =
      FOUR_CHAR_CODE('itsk'), /* FourCharCode*/
  kQTMetaDataKeyFormatiTunesLongForm =
      FOUR_CHAR_CODE('itlk') /* Reverse DNS format*/
};
...
enum {
  kQTMetaDataStorageFormatUserData =
      FOUR_CHAR_CODE('udta') /* UserData storage format*/
};
...
/* The list of keys are the User Data Identifiers (e.g. kUserDataTextAuthor,
 * kUserDataTextCopyright, etc.)*/
 ...
 ***************************************/
enum { kPropertyClass_MetaData = FOUR_CHAR_CODE('meta') };
...
*/
enum { kPropertyClass_MetaDataItem = FOUR_CHAR_CODE('mdit') };
...
enum {
  ...
  kQTMetaDataTypeSignedIntegerBE =
      21, /* The size of the integer is defined by the value size*/
  kQTMetaDataTypeUnsignedIntegerBE =
      22, /* The size of the integer is defined by the value size*/
 ...
 */
EXTERN_API(OSStatus)
QTMetaDataCreateFromBuffer(const UInt8 *inBufferPtr, ByteCount inBufferSize,
 ...
 */
EXTERN_API(OSStatus)
QTMetaDataGetBytes(QTMetaDataRef inMetaData, ByteCount inBufferSize,
                   UInt8 *inBufferPtr, ByteCount *outBufferSizeNeeded);
 ...
 */
EXTERN_API(OSStatus)
QTMetaDataGetPropertyInfo(QTMetaDataRef inMetaData, QTPropertyClass inPropClass,
                          QTPropertyID inPropID,
                          QTPropertyValueType *outPropType,
                          ByteCount *outPropValueSize, UInt32 *outPropFlags);
 ...
 */
EXTERN_API(OSStatus)
QTMetaDataGetProperty(QTMetaDataRef inMetaData, QTPropertyClass inPropClass,
                      QTPropertyID inPropID, ByteCount inPropValueSize,
                      QTPropertyValuePtr outPropValueAddress,
                      ByteCount *outPropValueSizeUsed);
 ...
 */
EXTERN_API(OSStatus)
QTMetaDataSetProperty(QTMetaDataRef inMetaData, QTPropertyClass inPropClass,
                      QTPropertyID inPropID, ByteCount inPropValueSize,
                      ConstQTPropertyValuePtr inPropValueAddress);
 ...
 */
EXTERN_API(OSStatus)
QTMetaDataGetItemValue(QTMetaDataRef inMetaData, QTMetaDataItem inItem,
                       UInt8 *outValuePtr, ByteCount inValueSize,
 ...
 */
EXTERN_API(OSStatus)
QTMetaDataGetNextItem(QTMetaDataRef inMetaData,
                      QTMetaDataStorageFormat inMetaDataFormat,
                      QTMetaDataItem inCurrentItem,
                      QTMetaDataKeyFormat inKeyFormat, const UInt8 *inKeyPtr,
 ...
 */
EXTERN_API(OSStatus)
QTMetaDataGetItemPropertyInfo(QTMetaDataRef inMetaData, QTMetaDataItem inItem,
                              QTPropertyClass inPropClass,
                              QTPropertyID inPropID,
                              QTPropertyValueType *outPropType,
                              ByteCount *outPropValueSize,
 ...
 */
EXTERN_API(OSStatus)
QTMetaDataGetItemProperty(QTMetaDataRef inMetaData, QTMetaDataItem inItem,
                          QTPropertyClass inPropClass, QTPropertyID inPropID,
                          ByteCount inPropValueSize,
                          QTPropertyValuePtr outPropValueAddress,
                          ByteCount *outPropValueSizeUsed);
 ...
 */
EXTERN_API(OSStatus)
QTMetaDataSetItemProperty(QTMetaDataRef inMetaData, QTMetaDataItem inItem,
                          QTPropertyClass inPropClass, QTPropertyID inPropID,
                          ByteCount inPropValueSize,
                          ConstQTPropertyValuePtr inPropValueAddress);
 ...
 */
EXTERN_API(OSStatus)
QTMetaDataGetItemCountWithKey(QTMetaDataRef inMetaData,
                              QTMetaDataStorageFormat inMetaDataFormat,
                              QTMetaDataKeyFormat inKeyFormat,
                              const UInt8 *inKeyPtr, ByteCount inKeySize,
                              ItemCount *outCount);
 ...
 */
EXTERN_API(OSStatus)
QTMetaDataAddItem(QTMetaDataRef inMetaData,
                  QTMetaDataStorageFormat inMetaDataFormat,
                  QTMetaDataKeyFormat inKeyFormat, const UInt8 *inKeyPtr,
                  ByteCount inKeySize, const UInt8 *inValuePtr,
                  ByteCount inValueSize, UInt32 inDataType,
 ...
 */
EXTERN_API(OSStatus)
QTMetaDataSetItem(QTMetaDataRef inMetaData, QTMetaDataItem inItem,
                  UInt8 *inValuePtr, ByteCount inValueSize, UInt32 inDataType);
 ...
 */
EXTERN_API(OSStatus)
QTMetaDataRemoveItemsWithKey(QTMetaDataRef inMetaData,
                             QTMetaDataStorageFormat inMetaDataFormat,
                             QTMetaDataKeyFormat inKeyFormat,
                             const UInt8 *inKeyPtr, ByteCount inKeySize);
...
enum { BandwidthManagementPrefsType = FOUR_CHAR_CODE('bwmg') };
...
#ifdef __cplusplus
inline DEFINE_API_C(MCActionFilterUPP)
    NewMCActionFilterUPP(MCActionFilterProcPtr userRoutine) {
...
#ifdef __cplusplus
inline DEFINE_API_C(MCActionFilterWithRefConUPP) NewMCActionFilterWithRefConUPP(
...
enum {
  uppMCActionNotificationProcInfo = 0x0003FED0
    ...
    */
#ifdef __cplusplus
inline DEFINE_API_C(MCActionNotificationUPP)
    NewMCActionNotificationUPP(MCActionNotificationProcPtr userRoutine) {
  return (MCActionNotificationUPP)NewRoutineDescriptor(
      (ProcPtr)(userRoutine), uppMCActionNotificationProcInfo,
      GetCurrentArchitecture());
}
#else
#define NewMCActionNotificationUPP(userRoutine)                                \
  (MCActionNotificationUPP) NewRoutineDescriptor(                              \
      (ProcPtr)(userRoutine), uppMCActionNotificationProcInfo,                 \
      GetCurrentArchitecture())
#endif
#endif
...
inline DEFINE_API_C(QTMoviePropertyListenerUPP)
  ...
  return (QTMoviePropertyListenerUPP)NewRoutineDescriptor(
      (ProcPtr)(userRoutine), uppQTMoviePropertyListenerProcInfo,
      GetCurrentArchitecture());
}
#else
#define NewQTMoviePropertyListenerUPP(userRoutine)                             \
  (QTMoviePropertyListenerUPP) NewRoutineDescriptor(                           \
      (ProcPtr)(userRoutine), uppQTMoviePropertyListenerProcInfo,              \
      GetCurrentArchitecture())
#endif
#endif
...
inline DEFINE_API_C(QTTrackPropertyListenerUPP)
  ...
  return (QTTrackPropertyListenerUPP)NewRoutineDescriptor(
      (ProcPtr)(userRoutine), uppQTTrackPropertyListenerProcInfo,
      GetCurrentArchitecture());
}
#else
#define NewQTTrackPropertyListenerUPP(userRoutine)                             \
  (QTTrackPropertyListenerUPP) NewRoutineDescriptor(                           \
      (ProcPtr)(userRoutine), uppQTTrackPropertyListenerProcInfo,              \
      GetCurrentArchitecture())
#endif
#endif
 ...
 */
EXTERN_API_C(Boolean)
InvokeMCActionNotificationUPP(MovieController mc, short action, void *params,
                              UInt32 inFlags, UInt32 *outFlags, void *refCon,
                              MCActionNotificationUPP userUPP);
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
inline DEFINE_API_C(Boolean)
    InvokeMCActionNotificationUPP(MovieController mc, short action,
                                  void *params, UInt32 inFlags,
                                  UInt32 *outFlags, void *refCon,
                                  MCActionNotificationUPP userUPP) {
  return (Boolean)CALL_SIX_PARAMETER_UPP(
      userUPP, uppMCActionNotificationProcInfo, mc, action, params, inFlags,
      outFlags, refCon);
}
#else
#define InvokeMCActionNotificationUPP(mc, action, params, inFlags, outFlags,   \
                                      refCon, userUPP)                         \
  (Boolean) CALL_SIX_PARAMETER_UPP((userUPP), uppMCActionNotificationProcInfo, \
                                   (mc), (action), (params), (inFlags),        \
                                   (outFlags), (refCon))
#endif
#endif
 ...
 */
EXTERN_API_C(void)
InvokeQTMoviePropertyListenerUPP(Movie inMovie, QTPropertyClass inPropClass,
...
inline DEFINE_API_C(void)
    InvokeQTMoviePropertyListenerUPP(Movie inMovie, QTPropertyClass inPropClass,
                                     QTPropertyID inPropID, void *inUserData,
                                     QTMoviePropertyListenerUPP userUPP) {
  CALL_FOUR_PARAMETER_UPP(userUPP, uppQTMoviePropertyListenerProcInfo, inMovie,
                          inPropClass, inPropID, inUserData);
}
#else
#define InvokeQTMoviePropertyListenerUPP(inMovie, inPropClass, inPropID,       \
                                         inUserData, userUPP)                  \
  CALL_FOUR_PARAMETER_UPP((userUPP), uppQTMoviePropertyListenerProcInfo,       \
                          (inMovie), (inPropClass), (inPropID), (inUserData))
#endif
#endif
 ...
 */
EXTERN_API_C(void)
InvokeQTTrackPropertyListenerUPP(Track inTrack, QTPropertyClass inPropClass,
                                 QTPropertyID inPropID, void *inUserData,
...
inline DEFINE_API_C(void)
    InvokeQTTrackPropertyListenerUPP(Track inTrack, QTPropertyClass inPropClass,
                                     QTPropertyID inPropID, void *inUserData,
                                     QTTrackPropertyListenerUPP userUPP) {
  CALL_FOUR_PARAMETER_UPP(userUPP, uppQTTrackPropertyListenerProcInfo, inTrack,
                          inPropClass, inPropID, inUserData);
}
#else
#define InvokeQTTrackPropertyListenerUPP(inTrack, inPropClass, inPropID,       \
                                         inUserData, userUPP)                  \
  CALL_FOUR_PARAMETER_UPP((userUPP), uppQTTrackPropertyListenerProcInfo,       \
                          (inTrack), (inPropClass), (inPropID), (inUserData))
#endif
#endif
...
#define NewMCActionNotificationProc(userRoutine)                               \
  NewMCActionNotificationUPP(userRoutine)
#define NewQTMoviePropertyListenerProc(userRoutine)                            \
  NewQTMoviePropertyListenerUPP(userRoutine)
#define NewQTTrackPropertyListenerProc(userRoutine)                            \
  NewQTTrackPropertyListenerUPP(userRoutine)
...
#define CallMCActionNotificationProc(userRoutine, mc, action, params, inFlags, \
                                     outFlags, refCon)                         \
  InvokeMCActionNotificationUPP(mc, action, params, inFlags, outFlags, refCon, \
                                userRoutine)
#define CallQTMoviePropertyListenerProc(userRoutine, inMovie, inPropClass,     \
                                        inPropID, inUserData)                  \
  InvokeQTMoviePropertyListenerUPP(inMovie, inPropClass, inPropID, inUserData, \
                                   userRoutine)
#define CallQTTrackPropertyListenerProc(userRoutine, inTrack, inPropClass,     \
                                        inPropID, inUserData)                  \
  InvokeQTTrackPropertyListenerUPP(inTrack, inPropClass, inPropID, inUserData, \
                                   userRoutine)
