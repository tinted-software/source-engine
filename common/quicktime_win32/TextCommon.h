/*
     ...
     Contains:   TextEncoding-related types and constants, and prototypes for
   related functions

     Version:    QuickTime 7.3

     Copyright:  (c) 2007 (c) 1995-2002 by Apple Computer, Inc., all rights
   reserved.
...
enum {
  ...
  kTextEncodingNextStepJapanese =
      0x0B02 /* NextStep Japanese encoding (variant of EUC-JP)*/
};
...
enum {
  ...
  kUnicodeCanonicalDecompVariant =
      2, /* canonical decomposition (NFD); excludes composed characters*/
  kUnicodeCanonicalCompVariant = 3, /* canonical composition (NFC); uses the
                                       composed chars as of Unicode 3.1*/
  kUnicodeHFSPlusDecompVariant = 8, /* decomposition for HFS+; doesn't decompose
                                       in 2000-2FFF, F900-FAFF, 2F800-2FAFF*/
...
enum {
  ...
  kUnicodeNoComposedVariant = 3, /* this really meant NoComposing; replaced by
...
enum {
  ...
  kUnicodeUTF16Format = 0, /* UTF16 form (16-bit units), native or external byte
                              order (see below)*/
  kUnicodeUTF32Format = 3, /* UTF32 form (32-bit units), native or external byte
                              order (see below)*/
  kUnicodeUTF16BEFormat =
      4, /* UTF16 form, explicit big-endian byte order, no BOM*/
  kUnicodeUTF16LEFormat =
      5, /* UTF16 form, explicit little-endian byte order, no BOM*/
  kUnicodeUTF32BEFormat =
      6, /* UTF32 form, explicit big-endian byte order, no BOM*/
  kUnicodeUTF32LEFormat =
      7, /* UTF32 form, explicit little-endian byte order, no BOM*/
  kUnicodeSCSUFormat =
      8 /* Std. Compression Scheme for Unicode, Unicode Tech Std. #6*/
};

/*
 Note for kUnicodeUTF16Format and kUnicodeUTF32Format:
 - An array of UTF16Char (UniChar) or UTF32Char is normally understood to use
 "internal" or platform-native byte ordering for kUnicodeUTF16Format and
 kUnicodeUTF32Format; the array MAY begin with byte-order mark (BOM), but the
 BOM should match the internal ordering.
 - If an array of bytes (such as char *) that can be in various encodings is
 specified to be in Unicode with kUnicodeUTF16Format or kUnicodeUTF32Format (not
 explicitly BE or LE), then it is assumed to use "external" byte ordering, which
 means: If there is a BOM at the beginning of text, the BOM specifies the byte
 ordering, otherwise big-endian is assumed. Synonyms for some Unicode formats
*/
/* TextEncoding type */
 ...
 */
EXTERN_API(OSStatus)
GetTextEncodingFromScriptInfo(ScriptCode iTextScriptID,
...
UC_INLINE Boolean UCIsSurrogateHighCharacter(UniChar character) {
  /* return ( ( character >= kUCHighSurrogateRangeStart ) && (character <=
   * kUCHighSurrogateRangeEnd ) ? true : false ); */
...
UC_INLINE Boolean UCIsSurrogateLowCharacter(UniChar character) {
  /* return ( ( character >= kUCLowSurrogateRangeStart ) && ( character <=
   * kUCLowSurrogateRangeEnd ) ? true : false ); */
...
*/
UC_INLINE UnicodeScalarValue UCGetUnicodeScalarValueForSurrogatePair(
    UniChar surrogateHigh, UniChar surrogateLow) {
  return ((surrogateHigh - kUCHighSurrogateRangeStart) << 10) +
         (surrogateLow - kUCLowSurrogateRangeStart) + 0x0010000UL;
}
