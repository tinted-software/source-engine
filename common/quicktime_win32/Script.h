...
enum {
  ...
  verCroatia = 68,        /* CR hr_HR,sh_HR*/
                          /*              69 is unassigned*/
  verGermanReformed = 70, /*       de_DE     Reformed orthography (used formerly
                             unassigned 70) */
...
enum {
  ...
  verYugoslavia = 25, /* Use verYugoCroatian (same number, now unused), or newer
                         verCroatia*/
...
enum {
  ...
  smScriptSysBase =
      -281, /* param is associated font to use w/ system font (old verb = -26)*/
  smScriptAppBase =
...
enum {
  ...
  smKeyRoman = -17 /* Set keyscript to Roman. Does nothing if Roman-only system,
                      unlike KeyScript(smRoman) which forces an update to
                      current default Roman keyboard */
};
...
enum {
  delimPad =
      -2, /* obsolete, misspelled token names kept for backward compatibility */
  tokenTilda = 44,
  tokenCarat = 55
};
