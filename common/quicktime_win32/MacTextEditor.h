/*
     ...
     Copyright:  (c) 2007 (c) 1996-2001 by Apple Computer, Inc., all rights
   reserved.
...
enum {
  ...
  kTXNDoFontSubstitution =
      FOUR_CHAR_CODE('fSub'), /*note : this could degrade performance greatly in
                                 the case of large documents.*/
  ...
  kTXNVisibilityTag =
      FOUR_CHAR_CODE('visb') /*set the visibility state of the object*/
};
...
/*
  In version 1.2 of MLTE and later you can change the top, left and right
  margins. The bottom margin is a placeholder for possible future enhancements.
*/
struct TXNMargins {
  SInt16 topMargin;
  SInt16 leftMargin;
...
enum { kTXNLinkNotPressed = 0, kTXNLinkWasPressed = 1, kTXNLinkTracking = 3 };
...
enum { kTXNHorizontal = 0, kTXNVertical = 1 };
...
typedef CALLBACK_API(void, TXNScrollInfoProcPtr)(
...
#ifdef __cplusplus
inline DEFINE_API_C(TXNActionKeyMapperUPP)
    NewTXNActionKeyMapperUPP(TXNActionKeyMapperProcPtr userRoutine) {
...
#ifdef __cplusplus
inline DEFINE_API_C(TXNScrollInfoUPP)
    NewTXNScrollInfoUPP(TXNScrollInfoProcPtr userRoutine) {
  return (TXNScrollInfoUPP)NewRoutineDescriptor((ProcPtr)(userRoutine),
                                                uppTXNScrollInfoProcInfo,
                                                GetCurrentArchitecture());
}
#else
#define NewTXNScrollInfoUPP(userRoutine)                                       \
  (TXNScrollInfoUPP)                                                           \
      NewRoutineDescriptor((ProcPtr)(userRoutine), uppTXNScrollInfoProcInfo,   \
                           GetCurrentArchitecture())
#endif
#endif
 ...
 */
EXTERN_API_C(void)
InvokeTXNScrollInfoUPP(SInt32 iValue, SInt32 iMaximumValue,
...
inline DEFINE_API_C(void)
    InvokeTXNScrollInfoUPP(SInt32 iValue, SInt32 iMaximumValue,
                           TXNScrollBarOrientation iScrollBarOrientation,
                           SInt32 iRefCon, TXNScrollInfoUPP userUPP) {
  CALL_FOUR_PARAMETER_UPP(userUPP, uppTXNScrollInfoProcInfo, iValue,
                          iMaximumValue, iScrollBarOrientation, iRefCon);
}
#else
#define InvokeTXNScrollInfoUPP(iValue, iMaximumValue, iScrollBarOrientation,   \
                               iRefCon, userUPP)                               \
  CALL_FOUR_PARAMETER_UPP((userUPP), uppTXNScrollInfoProcInfo, (iValue),       \
                          (iMaximumValue), (iScrollBarOrientation), (iRefCon))
#endif
...
#define CallTXNScrollInfoProc(userRoutine, iValue, iMaximumValue,              \
                              iScrollBarOrientation, iRefCon)                  \
  InvokeTXNScrollInfoUPP(iValue, iMaximumValue, iScrollBarOrientation,         \
                         iRefCon, userRoutine)
...
/*
  !! TXNSetViewRect is now deprecated for 10.2 and later.  Please use
  TXNSetFrameBounds or TXNSetRectBounds API !!
  *************************************************************************************************
...
/*
  ****************************************************************************************************************
  ...
  to attach a window to that object.  In version 1.0 of Textension attaching a
  TXNObject to more than one window is not supported.  Note that if a
  CGContextRef was passed to the TXNObject previously thru the API
  TXNSetTXNObjectControls, that CGContextRef will be ignored.  The CGContextRef
  associated with the iWindow will be used instead.  You may revert back to the
  previous CGContextRef by calling the API TXNSetTXNObjectControls with the
  desired CGContextRef again after calling TXNAttachObjectToWindow.
  ...
  support. Output: OSStatus:           function result.  paramErrs.
  ****************************************************************************************************************
*/
/*
 *  TXNAttachObjectToWindow()
 ...
 */
EXTERN_API_C(OSStatus)
TXNScroll(TXNObject iTXNObject, TXNScrollUnit iVerticalScrollUnit,
          TXNScrollUnit iHorizontalScrollUnit, SInt32 *ioVerticalDelta,
 ...
 */
EXTERN_API_C(void)
TXNRegisterScrollInfoProc(TXNObject iTXNObject,
                          TXNScrollInfoUPP iTXNScrollInfoUPP, SInt32 iRefCon);
