/*
     ...
     Copyright:  (c) 2007 (c) 1995-1998 by Apple Computer, Inc., all rights
   reserved.
 ...
 * IO Methods
 */
#define kQ3MethodTypeObjectFileVersion                                         \
  Q3_METHOD_TYPE('v', 'e', 'r', 's') /* version */
#define kQ3MethodTypeObjectTraverse                                            \
  Q3_METHOD_TYPE('t', 'r', 'v', 's') /* byte count */
#define kQ3MethodTypeObjectTraverseData                                        \
  Q3_METHOD_TYPE('t', 'r', 'v', 'd') /* byte count */
#define kQ3MethodTypeObjectWrite                                               \
  Q3_METHOD_TYPE('w', 'r', 'i', 't') /* Dump info to file */
#define kQ3MethodTypeObjectReadData                                            \
  Q3_METHOD_TYPE('r', 'd', 'd', 't') /* Read info from file into buffer or,    \
                                        attach read data to parent */
typedef CALLBACK_API_C(void, TQ3FunctionPointer)(void);
typedef CALLBACK_API_C(TQ3FunctionPointer,
                       TQ3MetaHandler)(TQ3MethodType methodType);
/*
 * MetaHandler:
 *      When you give a metahandler to QuickDraw 3D, it is called multiple times
 * to build method tables, and then is thrown away. You are guaranteed that your
 * metahandler will never be called again after a call that was passed a
 * metahandler returns.
 *
...
typedef CALLBACK_API_C(TQ3Status,
                       TQ3ObjectUnregisterMethod)(TQ3ObjectClass objectClass);
/******************************************************************************
 **                                                                          **
...
typedef CALLBACK_API_C(TQ3Status,
                       TQ3ElementCopyAddMethod)(const void *fromAPIElement,
                                                void *toInternalElement);
typedef CALLBACK_API_C(TQ3Status,
                       TQ3ElementCopyReplaceMethod)(const void *fromAPIElement,
                                                    void *ontoInternalElement);
typedef CALLBACK_API_C(TQ3Status,
                       TQ3ElementCopyGetMethod)(const void *fromInternalElement,
                                                void *toAPIElement);
typedef CALLBACK_API_C(TQ3Status, TQ3ElementCopyDuplicateMethod)(
    const void *fromInternalElement, void *toInternalElement);
typedef CALLBACK_API_C(TQ3Status,
                       TQ3ElementDeleteMethod)(void *internalElement);
#if CALL_NOT_IN_CARBON
/*
...
Q3ElementClass_Register(TQ3ElementType elementType, const char *name,
...
typedef CALLBACK_API_C(TQ3Status, TQ3ObjectTraverseMethod)(TQ3Object object,
                                                           void *data,
                                                           TQ3ViewObject view);
typedef CALLBACK_API_C(TQ3Status, TQ3ObjectWriteMethod)(const void *object,
 ...
 */
EXTERN_API_C(TQ3Status)
Q3View_SubmitWriteData(TQ3ViewObject view, TQ3Size size, void *data,
...
typedef CALLBACK_API_C(TQ3Status,
                       TQ3ObjectReadDataMethod)(TQ3Object parentObject,
                                                TQ3FileObject theFile);
