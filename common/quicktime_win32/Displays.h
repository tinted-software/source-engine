/*
     ...
     Copyright:  (c) 2007 (c) 1993-2001 by Apple Computer, Inc., all rights
   reserved.
...
enum {
  /* Switch Flags */
  kNoSwitchConfirmBit = 0, /* Flag indicating that there is no need to confirm a
                              switch to this mode */
...
enum {
  ...
  kFullDependencyNotify =
      1 /* Only sends to those who want to know about interrelated functionality
           (used for updating UI) */
};

/* DisplayID/DeviceID constants */
enum {
  kDummyDeviceID = 0x00FF, /* This is the ID of the dummy display, used when the
                              last "real" display is disabled.*/
...
enum {
  ...
  kDisplayGestaltBrightnessAffectsGammaMask =
      (1
       << 0), /* Used by default calibrator (should we show brightness panel) */
...
enum {
  ...
  kDMModeListIncludeAllModesMask =
      (1 << 0), /* Include all timing modes not _explicitly_ excluded (see other
                   bits)*/
...
#ifdef __cplusplus
inline DEFINE_API_C(DMNotificationUPP)
    NewDMNotificationUPP(DMNotificationProcPtr userRoutine) {
...
#ifdef __cplusplus
inline DEFINE_API_C(DMProfileListIteratorUPP)
    NewDMProfileListIteratorUPP(DMProfileListIteratorProcPtr userRoutine) {
...
#ifdef __cplusplus
inline DEFINE_API_C(DMDisplayListIteratorUPP)
    NewDMDisplayListIteratorUPP(DMDisplayListIteratorProcPtr userRoutine) {
...
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
inline DEFINE_API_C(void) InvokeDMDisplayModeListIteratorUPP(
    void *userData, DMListIndexType itemIndex,
    DMDisplayModeListEntryPtr displaymodeInfo,
    DMDisplayModeListIteratorUPP userUPP) {
