...
They matched neither the specification nor the implementation -- the "Yes" and
"No" bits were reversed.  The flags have been corrected but renamed to ensure
that code using the incorrect flags is reviewed by developers. enum {
...
enum {
  ...
  kQTSampleDependency_EarlierDisplayTimesAllowed =
      1 << 6, /* corresponds to flag mediaSampleEarlierDisplayTimesAllowed
                 except at different bit offset*/
  kQTSampleDependency_DoesNotDependOnOthers_Corrected =
      1 << 5, /* ie: an I picture*/
  kQTSampleDependency_DependsOnOthers_Corrected =
      1 << 4, /* ie: not an I picture*/
  kQTSampleDependency_IsNotDependedOnByOthers_Corrected =
      1 << 3, /* mediaSampleDroppable*/
...
typedef struct CompositionShiftLeastGreatestAtom
...
struct MediaHeader {
  ...
  long
      modificationTime; /* seconds since Jan 1904 when directory was appended */
...
typedef struct TrackCleanApertureDimensionsAtom
    TrackCleanApertureDimensionsAtom;
struct TrackProductionApertureDimensions {
  long flags; /* 1 byte of version / 3 bytes of flags */
  FixedPoint productionApertureDimensions;
};
typedef struct TrackProductionApertureDimensions
...
typedef struct TrackProductionApertureDimensionsAtom
...
typedef struct TrackEncodedPixelsDimensionsAtom
...
enum { MEDIA_TYPE = FOUR_CHAR_CODE('mdia') };
...
enum {
  ...
  TrackApertureModeDimensionsAID = FOUR_CHAR_CODE(
      'tapt'), /* container atom including TrackCleanApertureDimensionsAID,
                  TrackProductionApertureDimensionsAID and
...
enum {
  kQTFileTypeBrandQuickTimeMovie =
      FOUR_CHAR_CODE('qt  '),                       /* QuickTime movie files*/
  kQTFileTypeBrandISOFile = FOUR_CHAR_CODE('isom'), /* ISO Base Media files*/
  kQTFileTypeBrandMPEG4v1 =
      FOUR_CHAR_CODE('mp41'), /* MPEG-4 (ISO/IEC 14496-1) version 1 files*/
  kQTFileTypeBrandMPEG4v2 =
      FOUR_CHAR_CODE('mp42') /* MPEG-4 (ISO/IEC 14496-1) version 2 files*/
};
