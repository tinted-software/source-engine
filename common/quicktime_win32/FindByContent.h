...
/* An object containing summary information, from which summary text can be
 * obtained*/
...
#ifdef __cplusplus
inline DEFINE_API_C(FBCCallbackUPP)
    NewFBCCallbackUPP(FBCCallbackProcPtr userRoutine) {
  return userRoutine;
}
#else
 ...
 */
EXTERN_API_C(void)
FBCSetSessionCallback(FBCSearchSession searchSession, FBCCallbackUPP fn,
                      void *data);
...
typedef CALLBACK_API_C(Boolean, FBCHitTestProcPtr)(const FSRef *theFile,
                                                   void *data);
...
#ifdef __cplusplus
inline DEFINE_API_C(FBCHitTestUPP)
    NewFBCHitTestUPP(FBCHitTestProcPtr userRoutine) {
  return userRoutine;
}
#else
...
#ifdef __cplusplus
inline DEFINE_API_C(Boolean)
    InvokeFBCHitTestUPP(const FSRef *theFile, void *data,
...
/*      OS X DEPRECATED, NO-OP (will be removed from OS X exports in a future
 * release)*/
/*
 ...
 */
EXTERN_API_C(OSErr)
FBCBlindExampleSearch(const FSSpec examples[], UInt32 numExamples,
                      const FSSpec targetDirs[], UInt32 numTargets,
 ...
 */
EXTERN_API_C(OSErr)
FBCBlindExampleSearchWithCallback(const FSSpec examples[], UInt32 numExamples,
                                  const FSSpec targetDirs[], UInt32 numTargets,
                                  UInt32 maxHits, UInt32 maxHitWords,
                                  Boolean allIndexes, Boolean includeRemote,
                                  FBCSearchSession *theSession,
                                  FBCCallbackUPP callback, void *callbackData,
 ...
 */
EXTERN_API_C(OSErr)
FBCSummarize(const void *inBuf, UInt32 inLength, void *outBuf,
             UInt32 *outLength, UInt32 *numSentences);
...
FBCSummarizeCFString(CFStringRef inString, CFStringRef *outString,
                     UInt32 *numSentences);
 ...
 */
EXTERN_API_C(OSStatus)
FBCGetSummarySentences(FBCSummaryRef summary, CFStringRef *outString,
                       UInt32 *numSentences, Boolean paragraphs);
...
/*      OS X DEPRECATED (will be removed from OS X exports in a future
 * release)*/
/*
 *  FBCIndexItems()
 *
...
enum {
  ...
  centeuroHighWord =
      kFBCcenteuroHighWord, /* Central European languages not using Cyrillic*/
