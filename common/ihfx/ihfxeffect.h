...
public:
  ...
  // copy constructor
  HFX_INLINE HFXStateFlags(const HFXStateFlags &copy)
      : Storage(copy.Storage) {};

  HFX_INLINE HFXStateFlags(HFXStateStorage flags) : Storage(flags) {};

  HFX_INLINE HFXStateFlags(bool bRunning, bool bPaused, bool bMuted,
                           bool bHaulted, bool bExiting)
      : Storage((bRunning ? HFXSTATE_RUNNING : 0) |
                (bPaused ? HFXSTATE_PAUSE : 0) | (bMuted ? HFXSTATE_MUTE : 0) |
...
enum HFXNeeds {
  ...
  // effect needs to know how long its been since the last time its processed a
  // function.
  ...
  HFXNEED_DECL_TOTAL_INFORM =
      (HFXNEED_RUNNING_INFORM | HFXNEED_PAUSE_INFORM | HFXNEED_MUTE_INFORM |
       HFXNEED_EXITING_INFORM | HFXNEED_STATE_INFORM),
  // INFORMATION FLAGS
  HFXNEED_INFO_DEVICE =
      (HFXNEED_DEVICE1 | HFXNEED_DEVICE2 | HFXNEED_DEVICE3 | HFXNEED_DEVICE4 |
       HFXNEED_DEVICE5 | HFXNEED_DEVICE6 | HFXNEED_DEVICE7 | HFXNEED_DEVICE8),
};
...
enum HFXResult {
  ...
  // x, y and z axis  set
  HFXRESULT_XYZCHANGED =
      (HFXRESULT_XCHANGED | HFXRESULT_YCHANGED | HFXRESULT_ZCHANGED),
...
public:
  ...
  // IF YOU WANT THIS TO BE CALLED YOU MUST REGISTER YOUR EFFECT WITH
  // HFXNEED_DECL_INSTANCE
  virtual bool Initialize(IHFXProcessor &processor) = 0;
  // called by the game thread and blocks the haptics thread so the game and
  // haptics loop are safe inside. IF YOU WANT THIS TO BE CALLED YOU MUST
  // REGISTER YOUR EFFECT WITH HFXNEED_DECL_SYNC
  virtual HFXRESULT SyncOp(IHFXProcessor &processor) = 0;
  // called at haptic rate. should set the output parameter to the target output
  // force.
  //  calculation should optimized and precise in here.
  //  IF YOU WANT THIS TO BE CALLED YOU MUST REGISTER YOUR EFFECT WITH
  //  HFXNEED_DECL_PROCESS
  ...
  //  WILL ONLY BE CALLED IF YOU DECLARE YOUR EFFECT REGISTER WITH A _INFORM
  //  decl and will ONLY be called with those.
  //    note: state will be only one bit. for every bit changed on sync this
  //    function will be called.
  virtual void OnStateChange(IHFXProcessor &processor, HFXSTATE state,
                             bool flagged) = 0;
};
...
template <typename T> void HFXDefaultAllocateEffect(IHFXEffect *&ptr) {
  ptr = new T;
};

template <typename T> void HFXDefaultDeallocateEffect(IHFXEffect *&ptr) {
  if (!ptr)
    return;
  delete ((T *)ptr);
  ptr = 0;
...
// T == effect class
template <typename T>
inline HFXEffectID HFXDefaultRegisterEffect(
    IHFXSystem &hfxSystem, const char *tag, HFXNEED needs,
  ...
  hfxSystem.LogMessage((const int)1, "EFFECT REGISTERING! %s Size = %i \n", tag,
                       sizeof(T));
  return hfxSystem.RegisterEffectClass(tag, specialCreate, specialDestroy,
                                       needs, defaults);
};
...
inline HFXEffectID HFXDefaultRegisterEffect(IHFXSystem &hfxSystem,
                                            const char *tag, HFXNEED needs,
                                            HFXCreate_t specialCreate = 0,
                                            HFXDestroy_t specialDestroy = 0) {
  P *pGroup = new P;
  HFXEffectID retval = 0;
  if (pGroup->CopyDefaults(pGroup)) {
    retval = HFXDefaultRegisterEffect<T>(hfxSystem, tag, needs, pGroup,
                                         specialCreate, specialDestroy);
  }
  delete pGroup;
  return retval;
};
