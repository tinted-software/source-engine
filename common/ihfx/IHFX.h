...
typedef int _declspec(dllimport) (*LinkHFX_Fn)(const char *password,
                                               const char *cmdline,
                                               void **effects, void **system);
...
typedef bool (*MouseEmulationFn)(
    const double &devicex_pos, const double &devicey_pos,
    const double &devicez_pos, const double &devicex_delta,
    const double &devicey_delta, const double &devicez_delta,
    const int &buttons_down, const int &buttons_pressed,
    const int &buttons_released, bool moved,
    /* adjust if you would liek to send a mouse button */
    bool buttonsDown[eHFXMC_COUNT]);

struct ApplicationData {};
typedef void (*RollCallIndexSetCallBack)(IDevice *pDevice,
                                         const int devices_left);
...
public:
  ...
  // THIS FUNCTION MUST BE CALLED BY YOUR APPLICATION AND RETURN TRUE PRIOR TO
  // CREATING A DEVICE! window = the HWND of the program. returns weather or not
  // the applicationdata was sufficent.
  virtual bool AttachApplication(ApplicationData &app) = 0;
  virtual bool AttachApplicationByProcessName(ApplicationData &app,
                                              const char *szProcessName) = 0;

  // If you want to use more than 1 device set this before. NOTE: for multiple
  // devices you must use device lock
  // HFX_VIRTUAL bool SetOptionalDeviceCount( unsigned int nDevices );

  // Use this function to create a device. (note: when making multiple devices
  // they must be connected in numerical order.)
  virtual bool RunDevice(IDevice **ppDevice, const char *deviceName = 0,
                         const char *configDir = 0) = 0;

  // When using multiple devices you _MUST_ lock devices after device connection
  // attempts are made.
  virtual void LockDevices() = 0;

  // When using mutliple devices you _MUST_ unlock devices once they are locked
  // to connect more devices and then lock again afterwards.
  virtual void UnlockDevices() = 0;

  // Note: this will return the max number of devices specified in the
  // constructor of HapticsSystem.
  ...
  // this will ask the user to press a button on each device. the order in which
  // they
  //  press a button on each connected device will order their device id from
  //  zero to the number of falcons the user has minus one. note: without roll
  //  call devices are ordered by their serial number. returns false if only one
  //  falcon is connected.
  virtual bool RollCallDevices(RollCallIndexSetCallBack CallBackFn = 0,
                               bool bCreateThread = false) = 0;

  // will return true untill roll call is over.
  virtual bool IsInRollCall() const = 0;

  // updates roll call data. only call this if your in a roll call and the roll
  // call was not started with bCreateThread set to true. returns true if still
  // in roll call.
  virtual bool RollCallUpdate() = 0;

  // cancles roll call in progress. if ApplyMadeChanges is true the devices who
  // responded to the roll call will be set in and the unset will be moved to
  ...
  // void SetMouseMode(HWND window,eHFXMouseMove
  // MoveMode=eHFXMM_Delta,eHFXMouseButton ButtonMode=eHFXMM_1232);
  virtual void SetMouseMode(eMouseMove MoveMode = eHFXMM_Delta,
                            eMouseButton ButtonMode = eHFXMM_1232) = 0;
  ...
  // returns if we are in cursor emulation. If pDevice is null it will return
  // weather any device is in mouse emulation.
  ...
  //  MouseMode RollCall window focus and possibly other things will not allow
  //  forces.
  ...
  virtual IDevice *
  GetRunningDeviceByHardwareIndex(const int nSerialOrder) const = 0;
  ...
  virtual bool AllocateEffectParameter(IHFXParamGroup *&pIEffectParam,
                                       HFXEffectID associatedClass,
                                       const char *copyCachedSettings = 0,
                                       const char *storageName = 0) = 0;
  HFX_INLINE bool
  AllocateEffectParameter(IHFXParamGroup *&pIEffectParam,
                          const char *copyCachedSettings = (const char *)0,
                          const char *storageName = 0) {
    return AllocateEffectParameter(pIEffectParam, 0, copyCachedSettings,
                                   storageName);
  }
  ...
  // Note: any format ( besides Bitfile ) where it cannot find the file
  // specified will try to load a encoded bitfile if it does not exist. A bit
  // file will be generated when you load any format. When release time comes
  // around just delete the iniFile and include the bitfile output
  ...
  // HFX_VIRTUAL bool LoadHFXModule( const char *moduleName, const char
  // *modulePassword=0 ) HFX_PURE;
  ...
  virtual bool CreateNewEffectStack(const char *uniqueName, IStack **ppStack,
                                    unsigned int inputDevices = 0) = 0;
  ...
  // HFX_VIRTUAL bool InitFromXML(ApplicationData &appdata,unsigned int
  // &devices, const char *xmlLoc) HFX_PURE;
  ...
  inline bool SetMouseEmulation(MouseEmulationFn method_function) {
    if (!method_function)
      return false;
    return (SetMouseEmulation(eHFXMouseMeth_useCallback) &&
            SetMouseEmulationFunction(method_function));
  }
#endif
  virtual bool ScaleDeviceCoordsForCursor(double &x, double &y,
                                          double width = -1, double height = -1,
                                          double offsetx = -1,
                                          double offsety = -1) = 0;
  // try and put device to sleep. if null all devices will try to sleep. devices
  // will sleep untill touched.
  virtual void AttemptSlumber(IDevice *device = 0) = 0;

  virtual void
  ActivateDynamicWindowHandler(const char *windowClassName,
  ...
  virtual HFXEffectID RegisterEffectClass(const char *tag, HFXCreate_t alloc,
                                          HFXDestroy_t dealloc, HFXNEED needs,
                                          IHFXParamGroup *defaults = 0) = 0;
#define HFX_NON_ELEMENT 0xFFFFF0
  virtual bool SetParameterGroupVar(IHFXParamGroup &params, HFXParamID var,
                                    const char *string,
                                    unsigned int element = HFX_NON_ELEMENT) = 0;
  virtual bool SetParameterGroupVar(IHFXParamGroup &params,
                                    const char *varString, const char *string,
                                    unsigned int element = HFX_NON_ELEMENT) = 0;
  ...
  virtual HFXParamID EnumerateEffectParameters(HFXEffectID id,
                                               HFXParamID LastParamID) = 0;
  virtual bool CacheEffectParameterGroupCopy(const char *cachename,
                                             IHFXParamGroup *params) = 0;
  virtual bool SaveEffectParameterCache(const char *filename,
                                        const char *type = "ini") = 0;
...
namespace Effects {
struct command_info;
int ProcessCommand(IBaseEffect *pEffect, const char *argv[],
...
typedef int (*OldServoLoopFn)(void *pParam, class IDevice *pDevice,
                              double outforces[3]);
...
public:
  ...
  virtual void GetButtonData(int *down = 0, int *pressed = 0,
                             int *released = 0) const = 0;
  ...
  // This function is here for quick integration into games which already have a
  // few custom effects of their own. Taking the old ServoOp function and adding
  // in pDevice
  virtual void SetServoLoopCallbackFunction(OldServoLoopFn old_fn,
                                            void *pParam) = 0;
  ...
  virtual bool TriggerFade(unsigned int msMuteTime, unsigned int msDuration,
                           bool forceOverride = false) = 0;
...
public:
  ...
  // --
  virtual bool CreateCachedEffect(IHFXEffect *&pHandle, const char *entry,
                                  const char *instancename = 0);

  template <typename T>
  HFX_INLINE bool CreateCachedEffect(T *&pTypedHandle, const char *entry,
                                     const char *instanceName = 0) {
  ...
  virtual bool CreateNewEffect(IHFXEffect *&ppHandle, HFXEffectID effectName,
                               IHFXParamGroup *params,
                               const char *instanceName = 0);

  template <typename T>
  HFX_INLINE bool CreateNewEffect(T *&pTypedHandle, HFXEffectID effectName,
                                  IHFXParamGroup *params,
                                  const char *instanceName = 0) {
    return CreateNewEffect((IHFXEffect *&)pTypedHandle, effectName, params,
                           instanceName);
  }

  HFX_INLINE bool CreateNewEffect(HFXEffectID effectName,
                                  IHFXParamGroup *params) {
  ...
  template <typename T> HFX_INLINE bool DeleteRunningEffect(T *&pTypedEffect) {
  ...
  // returns true if device was added or is already a member. if slot is -1 it
  // will just enter the device as the next available slot.
...
public:
  // call this to mute and unmute this effect. (note: effect will still be
  // updated, just will not output force.) note : this setting will not be
  // applied untill the next sync op ( InputUpdate ) called by the application.
  ...
  // if your effect is not self deleting and requires runtime data you should
  // override this to true.
  HFX_INLINE bool CanBeHandled() const {
    return (GetEffectNeeds() & HFXNEED_ENCAPSULATED) == 0;
  }

  // this lets the haptic effect stack know you are done with this effect and it
  // is waiting to be deleted. CANNOT BE OVERRIDDEN ( see OnFlaggedForRemoval()
  // )
  virtual void FlagForRemoval() = 0;

  // Stack functions!
  friend class NovintHFX::Stack;
  friend int NovintHFX::Effects::ProcessCommand(IBaseEffect *pEffect,
                                                const char *argv[],
                                                const unsigned int argc);
  ...
  HFX_INLINE bool NeedsRemoval() const {
    return ((GetEffectNeeds() & HFXNEED_REMOVE)
                ? (((GetEffectState() & HFXSTATE_RUNNING)) ? false : true)
                : false);
  }

  // DO NOT OVERRIDE!
  HFX_INLINE bool WantsSyncOp() const {
    return (((GetEffectNeeds() & HFXNEED_SYNC) != 0) &&
            ((GetEffectState() & HFXSTATE_WANT_SYNC) != 0));
  }
  // DO NOT OVERRIDE!
  virtual bool WantsUpdate() const {
    return (((GetEffectNeeds() & HFXNEED_PROCESS) != 0) &&
            ((GetEffectState() & HFXSTATE_WANT_UPDATE) != 0) &&
...
#define ConnectNovintHFX                                                       \
  YOU_MUST_INCLUDE_WINDOWS_PRIOR_TO_INCLUDING_INovintHFX_h
#endif

// to connect windows.h must be included
#if (!defined(I_NOVINT_HFX_WINDOWS) &&                                         \
     (defined(_INC_WINDOWS) || defined(LoadLibrary) && defined(HMODULE))) &&   \
    !defined(STATIC_IHFX)
...
#ifdef _DEBUG
      // look at the call stack and please remove whatever is calling this
      // constructor. there should be only one HFX_INIT_VARS.
      DebugBreak();
#endif
      ExitProcess(100);
    }
...
inline bool ConnectNovintHFX(IHapticsSystem **ppSystem, void *window,
                             const char *cmd, void *pCursorEnableFn = 0,
  ...
  if (GetEnvironmentVariableA("NOVINT_DEVICE_SUPPORT", szNovintDir,
                              sizeof(szNovintDir)) == 0 ||
  ...
  if (dllNovintHFX_) {
    connectFn = (NovintHFX_ExposeInterfaceFn)GetProcAddress(
        dllNovintHFX_, HFX_CONNECT_FUNCTION_NAME());
  }
  if (!connectFn) {
    ...
    dllNovintHFX_ = LoadLibraryA(szNovintDll);
    if (dllNovintHFX_) {
      connectFn = (NovintHFX_ExposeInterfaceFn)GetProcAddress(dllNovintHFX_,
                                                              "CreateHFX");
    }
  }
  if (connectFn &&
      connectFn((void **)ppSystem, window, cmd, HFX_VERSION_MAJOR,
                HFX_VERSION_MINOR, pCursorEnableFn, TargetDevices) == 0) {
    _hfx = (*ppSystem);
    return true;
  }
...
#ifndef HFX_STRIPPED
inline bool ConnectNovintHFX_XML(IHapticsSystem **ppSystem, void *window,
                                 const char *xml, void *pCursorEnableFn = 0,
                                 unsigned int TargetDevices = 1) {
  if (ppSystem == 0)
    return false;
  ...
  char szNovintDir[MAX_PATH];
  char szNovintDll[MAX_PATH];
  if (GetEnvironmentVariable("NOVINT_DEVICE_SUPPORT", szNovintDir,
                             sizeof(szNovintDir)) == 0 ||
      !V_IsAbsolutePath(szNovintDir)) {
    return false;
  }
  ...
  if (dllNovintHFX_) {
    connectFn = (NovintHFX_ExposeInterfaceFn)GetProcAddress(
        dllNovintHFX_, HFX_CONNECT_FUNCTION_NAME_XML());
  }
  if (!connectFn) {
    if (dllNovintHFX_) {
      FreeLibrary(dllNovintHFX_);
  ...
  if (connectFn &&
      connectFn((void **)ppSystem, window, xml, HFX_VERSION_MAJOR,
                HFX_VERSION_MINOR, pCursorEnableFn, TargetDevices) == 0) {
    _hfx = (*ppSystem);
    return true;
  }
...
#endif
inline bool ConnectNovintHFX(IHapticsSystem **ppSystem, HWND hwnd,
                             const char *cmd, void *pCursorEnableFn = 0,
                             unsigned int TargetDevices = 1) {
  return ConnectNovintHFX(ppSystem, (void *)hwnd, cmd, pCursorEnableFn,
                          TargetDevices);
}
