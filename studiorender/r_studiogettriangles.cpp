  ...
  for (i = 0; i < info.m_pStudioHdr->numbodyparts; i++) {
    ...
    for (k = 0; k < pModel->nummeshes; ++k) {
      ...
      for (stripGroupID = 0; stripGroupID < pMeshData->m_NumGroup;
        ...
        //				bool bIsFlexed = ( pMeshGroup->m_Flags &
        //MESHGROUP_IS_FLEXED ) != 0; 				bool bIsHWSkinned = ( pMeshGroup->m_Flags
        //& MESHGROUP_IS_HWSKINNED ) != 0;
        ...
        for (stripID = 0; stripID < pMeshGroup->m_NumStrips; stripID++) {
          ...
          //					int boneID;
          //					for( boneID = 0; boneID <
          //pStripData->numBoneStateChanges; boneID++ )
          //					{
          //						OptimizedModel::BoneStateChangeHeader_t
          //*pBoneStateChange = pStripData->pBoneStateChange( boneID );
          //						hardwareBoneToGlobalBone[pBoneStateChange->hardwareID]
