...
IMaterial *CStudioRender::R_StudioSetupSkinAndLighting(
  ...
  // TODO: It's possible we don't want to use the color texels--for example
  // because of a convar. We should check that here in addition to whether or
  // not we have the data available.
  static unsigned int lightmapVarCache = 0;
  IMaterialVar *pLightmapVar =
      pMaterial->FindVarFast("$lightmap", &lightmapVarCache);
  if (pLightmapVar) {
...
static void ScreenAlignBone(matrix3x4_t *pPoseToWorld, mstudiobone_t *pCurBone,
  ...
  matrix3x4_t matBoneBillboard(vX.x, vY.x, vZ.x, vT.x, vX.y, vY.y, vZ.y, vT.y,
                               vX.z, vY.z, vZ.z, vT.z);
