    ...
    for (j = 0; j < phdr->numcdtextures && IsErrorMaterial(pMaterial); j++) {
      ...
      if (phdr->flags & STUDIOHDR_FLAGS_OBSOLETE) {
        ...
        if (IsErrorMaterial(pMaterial)) {
          Warning("StudioRender: OBSOLETE material missing: "
...
bool CStudioRenderContext::R_AddVertexToMesh(
  ...
  // FIXME: if this ever becomes perf-critical... these writes are not in
  // memory-ascending order,
  //        which hurts since VBs are in write-combined memory (See
  //        WriteCombineOrdering_t)
  ...
  */
  ...
  if (vertData->HasTangentData()) {
    ...
    Warning( "\ttangent: %0.1f %0.1f %0.1f with length %0.1f at position %0.1f
    %0.1f %0.1f\n", tangentS.x, tangentS.y, tangentS.z, len,
    vert.m_vecPosition.x, vert.m_vecPosition.y, vert.m_vecPosition.z );
    }
    }
    ...
    Warning( "MODELBUG %s: nearly colinear tangentS (%f %f %f) and normal (%f %f
    %f) at position %f %f %f Probably have 2 or more texcoords that are the same
  ...
  if (hwSkin) {
    ...
    // *desired* bones and more than 1 *actual* bone (we can have 0	vertex
    // bones in the case of static props
...
inline const mstudio_meshvertexdata_t *
GetFatVertexData(mstudiomesh_t *pMesh, studiohdr_t *pStudioHdr) {
  ...
  if (!pVertData) {
    ...
    if (warnCount++ < 20)
      Warning("ERROR: model verts have been compressed, cannot render! (use "
              "\"-no_compressed_vvds\")");
  }
  return pVertData;
}
...
void CStudioRenderContext::R_StudioBuildMorph(
  ...
  char pTemp[256];
  Q_snprintf(pTemp, sizeof(pTemp), "%s [%p]", pStudioHdr->pszName(),
             pMeshGroup);
  ...
  for (int i = 0; i < pMesh->numflexes; ++i) {
    ...
    for (int j = 0; j < pFlex->numverts; ++j) {
      ...
      morphBuilder.PositionDelta3(
          pAnim->GetDeltaFixed(flVertAnimFixedPointScale));
      morphBuilder.NormalDelta3(
          pAnim->GetNDeltaFixed(flVertAnimFixedPointScale));
      ...
      if (pFlex->vertanimtype == STUDIO_VERT_ANIM_WRINKLE) {
        ...
        morphBuilder.WrinkleDelta1f(
            pWrinkleAnim->GetWrinkleDeltaFixed(flVertAnimFixedPointScale));
      } else {
...
void CStudioRenderContext::R_StudioBuildMeshStrips(
  ...
  size_t stripHdrSize =
      (pStripGroup->flags & OptimizedModel::STRIPGROUP_IS_MDL49)
          ? sizeof(OptimizedModel::StripHeader_v49_t)
  ...
  for (i = 0; i < pStripGroup->numStrips; ++i) {
    ...
    pMeshGroup->m_pStripData[i].boneStateChangeOffset =
        boneStateChangeOffset - i * stripHdrSize;
...
VertexFormat_t CStudioRenderContext::CalculateVertexFormat(
  ...
  if (bIsDX7) {
    ...
    for (int i = 0; i < pStudioHdr->numskinfamilies; i++) {
      ...
      //pMesh->material ] ]; 	Assert( pMaterial ); 	VertexFormat_t vertexFormat =
      //pMaterial->GetVertexFormat(); 	newVertexFormat &=
    ...
    // Don't compress the mesh unless it is HW-skinned (we only want to compress
    // static VBs, not dynamic ones - that would slow down the MeshBuilder in
    // dynamic use cases). Also inspect the vertex data to see if it's
    // appropriate for the vertex element compression techniques that we do
    // (e.g. look at UV ranges).
    if ( // IsX360() && // Disabled until the craziness is banished
        bIsHwSkinned &&
        (g_pMaterialSystemHardwareConfig->SupportsCompressedVertices() ==
...
void CStudioRenderContext::ComputeHWMorphDecalBoneRemap(
  ...
  if (nUsedBones > 1) {
    if (nUsedBones >
      ...
      Warning("Hardware morphing of decals will be busted! Too many unique "
              "bones on flexed vertices!\n");
    }
...
bool CStudioRenderContext::LoadModel(studiohdr_t *pStudioHdr, void *pVtxBuffer,
  ...
  pStudioHWData->m_RootLOD =
      min((int)pStudioHdr->rootLOD, pVertexHdr->numLODs - 1);
...
int CStudioRenderContext::GetMaterialList(studiohdr_t *pStudioHdr, int count,
  ...
  AssertMsg(pStudioHdr,
            "Don't ignore this assert! CStudioRenderContext::GetMaterialList() "
            "has null pStudioHdr.");
  ...
  for (i = 0; i < pStudioHdr->numtextures; i++) {
    ...
    for (j = 0; j < pStudioHdr->numcdtextures && IsErrorMaterial(pMaterial);
      ...
      if (pStudioHdr->flags & STUDIOHDR_FLAGS_OBSOLETE) {
        pMaterial = g_pMaterialSystem->FindMaterialEx(
            "models/obsolete/obsolete", TEXTURE_GROUP_MODEL,
            MATERIAL_FINDCONTEXT_ISONAMODEL, false);
      } else {
        pMaterial = g_pMaterialSystem->FindMaterialEx(
            szPath, TEXTURE_GROUP_MODEL, MATERIAL_FINDCONTEXT_ISONAMODEL,
            false);
      }
    }
...
//-----------------------------------------------------------------------------
void CStudioRenderContext::GetMaterialOverride(
    IMaterial **ppOutForcedMaterial, OverrideType_t *pOutOverrideType) {
...
void CStudioRenderContext::DrawModel(DrawModelResults_t *pResults,
  ...
  if (!pCallQueue || studio_queue_mode.GetInt() == 0) {
    ...
    CMatRenderData<matrix3x4_t> rdMatrix(
        pRenderContext, info.m_pStudioHdr->numbones, pBoneToWorld);
    CMatRenderData<float> rdFlex(pRenderContext);
    CMatRenderData<float> rdFlexDelayed(pRenderContext);
    ...
    if (info.m_pStudioHdr->numflexdesc != 0) {
      ...
      if (!pFlexDelayedWeights) {
        ...
        rdFlexDelayed.Lock(info.m_pStudioHdr->numflexdesc,
                           flex.m_pFlexDelayedWeights);
...
void CStudioRenderContext::DrawModelArray(const DrawModelInfo_t &drawInfo,
                                          int arrayCount,
                                          model_array_instance_t *pInstanceData,
                                          int instanceStride, int flags) {
  // UNDONE: Support queue mode?
  g_pStudioRenderImp->DrawModelArray(drawInfo, m_RC, arrayCount, pInstanceData,
                                     instanceStride, flags);
}
...
void CStudioRenderContext::AddShadow(IMaterial *pMaterial, void *pProxyData,
  ...
  if (!pCallQueue || studio_queue_mode.GetInt() == 0) {
    ...
    if (pProxyData != NULL) {
      Warning("Cannot call CStudioRenderContext::AddShadows w/ proxies in "
              "queued mode!\n");
      return;
    }

    CMatRenderData<FlashlightState_t> rdFlashlight(pRenderContext, 1,
                                                   pFlashlightState);
    ...
    pCallQueue->QueueCall(g_pStudioRenderImp, &CStudioRender::AddShadow,
                          pMaterial, (void *)NULL, rdFlashlight.Base(),
                          rdMatrix.Base(), pFlashlightDepthTexture);
  }
}
...
void CStudioRenderContext::AddDecal(StudioDecalHandle_t handle,
  ...
  QUEUE_STUDIORENDER_CALL_RC(AddDecal, CStudioRender, g_pStudioRenderImp,
                             pRenderContext, handle, m_RC, pBoneToWorld,
                             pStudioHdr, ray, decalUp, pDecalMaterial, radius,
                             body, noPokethru, maxLODToDecal);
}

// Function to do replacement because we always need to do this from the main
// thread.
