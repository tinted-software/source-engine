...
CLinkedMiniProfiler g_mp_morph_lower_bound("morph_lower_bound",
...
CLinkedMiniProfiler *g_mp_ComputeFlexedVertex_StreamOffset[8] = {
    NULL,           &g_mp_morph_V1, &g_mp_morph_V2, &g_mp_morph_V3,
    &g_mp_morph_V4, &g_mp_morph_V5, &g_mp_morph_V6, &g_mp_morph_V7};
#else
...
const ALIGN16 int32 g_perm_delta_wrinkle[4] = {
    0x14150000, 0x16170000, 0x18190000,
...
// const ALIGN16 int32 g_perm_w0[4]     =
// {0x00010203,0x08090A0B,0x00010203,0x08090A0B};
const ALIGN16 int32 g_perm_w1[4] = {0x0C0D0E0F, 0x0C0D0E0F, 0x04050607,
                                    0x04050607};
const fltx4 g_sc256_255_special = {256.0f / 255.0f, 256.0f / 255.0f,
                                   -256.0f / 255.0f, -256.0f / 255.0f};
...
//
// V4 rolled - latency of x4, manually scheduled for nearly optimal dual-issue
// and no automatic stalls the ~15 nops mean 1 instruction is issued at that
// cycle, instead of theoretically possible 2 per cycle
//
__declspec(naked) int
ComputeFlexedVertex_StreamOffset_V7(int nThinFlexVertexCount,        // r3
  ...
  __asm
  {
   ...
   // let the compiler schedule the instructions, just use several registers to
   // avoid dependencies
		...
		li r10, -1
		rldicl r7,r7,0,32 // currentTag &= 0xFFFFFFFF ; just to make sure we don't mess up
      // isCacheInvalid computation
...
label_start_V7:  // 52 instructions run in 45 cycles, although compiler predicts 38 cycles
      ////////////////
      // IMPORTANT: DO NOT REMOVE NOPS UNLESS YOU KNOW WHAT YOU ARE DOING AND
     // WHY! nops are essential here, removing them will make the code about 2%
     // slower because dual-issue will be broken
     ////////////////
...
__declspec(naked) int
ComputeFlexedVertex_StreamOffset_V6(int nThinFlexVertexCount,        // r3
                                    CachedPosNorm_t *pThinFlexVerts, // r4
                                    int32 *pFirstThinFlexIndex,      // r5
                                    mstudiovertanim_t *pVert,        // r6
                                    uint32 nCurrentTag,              // r7
...
// 2-stages
__declspec(naked) int
ComputeFlexedVertex_StreamOffset_V5(int nThinFlexVertexCount,        // r3
                                    CachedPosNorm_t *pThinFlexVerts, // r4
                                    int32 *pFirstThinFlexIndex,      // r5
                                    mstudiovertanim_t *pVert,        // r6
                                    uint32 nCurrentTag,              // r7
...
// V3 in asm
__declspec(naked) int
ComputeFlexedVertex_StreamOffset_V4(int nThinFlexVertexCount,        // r3
                                    CachedPosNorm_t *pThinFlexVerts, // r4
                                    int32 *pFirstThinFlexIndex,      // r5
                                    mstudiovertanim_t *pVert,        // r6
                                    uint32 nCurrentTag,              // r7
...
// base for asm
int ComputeFlexedVertex_StreamOffset_V3(int nThinFlexVertexCount,
                                        CachedPosNorm_t *pThinFlexVerts,
                                        int32 *pFirstThinFlexIndex,
                                        mstudiovertanim_t *pVert,
                                        uint32 nCurrentTag,
  ...
  do {
    ...
    fltx4 f4sb =
        MaddSIMD(__vcfsx(__vperm(f4Zero, packedVert, permuteSpeedSide), 8),
                 sc256_255_special, f40011);
    // f4sb = {s,b,1-s,1-b}

    fltx4 f3Delta = __vcfsx(__vperm(f4Zero, packedVert, permuteDelta), 12 + 16);
    fltx4 f3NDelta =
        __vcfsx(__vperm(f4Zero, packedVert, permuteNDelta), 12 + 16);
    uint64 oldCache = uint32(pFirstThinFlexIndex[n]);
    uint64 cacheVertexIndex =
        oldCache ^ nCurrentTag; // if there is trash in high (2^16) bits, we
                                // need to update the cache
    int64 isCacheInvalid =
        int64(0xFFFF - cacheVertexIndex) >>
        32; // the second shift must be arithmetic to form a valid mask
    ...
    CachedPosNorm_t *pFlexedVertex =
        pThinFlexVerts + nVertexIndex; // will be overridden
    ...
    // fltx4 f4sbProd = MulSIMD(__vperm(f4sb,f4sb,permuteW0),
    // __vperm(f4sb,f4sb,permuteW1));
    ...
    StoreAlignedSIMD((float *)&pFlexedVertex->m_Normal,
                     MaddSIMD(scWeight, f3NDelta, vfNormal));
    StoreAlignedSIMD((float *)&pFlexedVertex->m_Position,
                     MaddSIMD(scWeight, f3Delta, vfPosition));
...
// base for asm
int ComputeFlexedVertexWrinkle_StreamOffset_V3(
    int nThinFlexVertexCount, CachedPosNorm_t *pThinFlexVerts,
  ...
  do {
    ...
    pThinFlexVerts[nThinFlexVertexCount + 2].m_Normal.InitZero();
    fltx4 packedVert = LoadUnalignedSIMD((const float *)pVert);
    fltx4 f3PreDelta = __lvlx(pVert, 16); // f3Delta now contains only packed W
                                          // component in high X halfword...
    fltx4 f4sb =
        MaddSIMD(__vcfsx(__vperm(f4Zero, packedVert, permuteSpeedSide), 8),
                 sc256_255_special, f40011);
    // f4sb = {s,b,1-s,1-b}
    ...
    f3PreDelta = __vrlimi(
        f3PreDelta, packedVert, 7,
        0); // don't rotate and move bytes 4..15 from packed vert to f3PreDelta
    fltx4 f3NDelta =
        __vcfsx(__vperm(f4Zero, packedVert, permuteNDelta), 12 + 16);
...
int ComputeFlexedVertex_StreamOffset_V2(int nThinFlexVertexCount,
                                        CachedPosNorm_t *pThinFlexVerts,
                                        int32 *pFirstThinFlexIndex,
                                        mstudiovertanim_t *pVert,
                                        uint32 nCurrentTag,
  ...
  mstudiovertanim_t *pVertEnd = pVert + numVertsToProcess;
  {
    ...
    fltx4 packedVert = LoadUnalignedSIMD((const float *)pVert);
    fltx4 f4sb = MaddSIMD(
        __vcfsx(__vperm(f4Zero, packedVert, permuteSpeedSide), 8),
        sc256_255_special, f40011); // to be completely correct, we'll ned to
                                    // multiply this with 256/255
    // f4sb = {s,b,1-s,1-b}
  ...
  while (pVert < pVertEnd) {
    // stage 1
    {
      ...
      // here we need to form the following vector to compute final w:
      // {s(1-b), (1-s)(1-b), sb, (1-s)b}
      // fltx4 f4sbProd = MulSIMD(__vperm(f4sb_st1,f4sb_st1,permuteW0),
      // __vperm(f4sb_st1,f4sb_st1,permuteW1));
      fltx4 f4sbProd =
          MulSIMD(__vpermwi(f4sb_st1, 0x22), __vpermwi(f4sb_st1, 0xF5));
      fltx4 scWeight = __vmsum4fp(f4sbProd, w1234);

      StoreAlignedSIMD((float *)&pFlexedVertex->m_Normal,
                       MaddSIMD(scWeight, f3NDelta_st1, vfNormal));
      StoreAlignedSIMD((float *)&pFlexedVertex->m_Position,
                       MaddSIMD(scWeight, f3Delta_st1, vfPosition));
    }
    ...
    // stage 0
    {
      ...
      fltx4 f3Delta =
          __vcfsx(__vperm(f4Zero, packedVert, permuteDelta), 12 + 16);
      fltx4 f3NDelta =
          __vcfsx(__vperm(f4Zero, packedVert, permuteNDelta), 12 + 16);
      uint64 oldCache = uint32(pFirstThinFlexIndex[n]);
      uint64 cacheVertexIndex =
      ...
      int64 newCache = nCurrentTag | nThinFlexVertexCount;
      int64 updateCache =
          (newCache & isCacheInvalid) | (oldCache & isCacheValid);
      nThinFlexVertexCount = nThinFlexVertexCount - isCacheInvalid;

      pFirstThinFlexIndex[n] = updateCache; // this may be put wherever it
                                            // doesn't mess up the other stores
...
// branchless
int ComputeFlexedVertex_StreamOffset_V1(int nThinFlexVertexCount,
                                        CachedPosNorm_t *pThinFlexVerts,
                                        int32 *pFirstThinFlexIndex,
                                        mstudiovertanim_t *pVert,
                                        uint32 nCurrentTag,
...
typedef int (*Fn_ComputeFlexedVertex_StreamOffset)(
    int nThinFlexVertexCount, CachedPosNorm_t *pThinFlexVerts,
...
typedef int (*Fn_ComputeFlexedVertexWrinkle_StreamOffset)(
    int nThinFlexVertexCount, CachedPosNorm_t *pThinFlexVerts,
    int32 *pFirstThinFlexIndex, mstudiovertanim_wrinkle_t *pVert,
    uint32 nCurrentTag, uint32 numVertsToProcess, fltx4 w1234);
Fn_ComputeFlexedVertexWrinkle_StreamOffset
    g_fn_ComputeFlexedVertexWrinkle_StreamOffset[8] = {
        NULL,
...
CCachedRenderData::ComputeFlexedVertex_StreamOffset<mstudiovertanim_t>(
    studiohdr_t *pStudioHdr, mstudioflex_t *pflex, mstudiovertanim_t *pvanim,
...
CCachedRenderData::ComputeFlexedVertex_StreamOffset<mstudiovertanim_wrinkle_t>(
    studiohdr_t *pStudioHdr, mstudioflex_t *pflex,
    mstudiovertanim_wrinkle_t *pvanim, int vertCount, float w1, float w2,
    float w3, float w4);

// vectorized
void CCachedRenderData::ComputeFlexedVertex_StreamOffset_Optimized(
    studiohdr_t *pStudioHdr, mstudioflex_t *pflex, mstudiovertanim_t *pvanim,
    int vertCount, float w1, float w2, float w3, float w4) {
#if PROFILE_THIS_FILE
  ...
  if (nMorphPath) {
...
#endif
      pVertEnd =
          std::lower_bound(pvanim, pvanim + pflex->numverts, vertCountStruct,
                           mstudiovertanim_t::CSortByIndex());
    }
    ...
    if (pvanim < pVertEnd) {
      ...
      if(numVertsToProcess > maxVertsSaved)
      {
              ...
              {
                      g_pFullFileSystem->Write(pvanim, sizeof(*pvanim) *
      numVertsToProcess, fh); g_pFullFileSystem->Close(fh);
              }
      }*/
...
#endif
      {
        for (uint32 i = 0; i < 2; ++i) // reset the first 2 positions here as
                                       // it's required by the algorithm..
        {
          pThinFlexVerts[nThinFlexVertexCount + i].m_Position.InitZero();
          pThinFlexVerts[nThinFlexVertexCount + i].m_Normal.InitZero();
        }
        nThinFlexVertexCount =
            g_fn_ComputeFlexedVertex_StreamOffset[nMorphPath](
                nThinFlexVertexCount, pThinFlexVerts, pFirstThinFlexIndex,
                pvanim, nCurrentTag, numVertsToProcess, weights.f4);
      }
#ifdef _DEBUG
      else // Validation path inactive in release, since these static arrays
           // consume 1MB
      {
        bool repeat = false;
        ...
        memcpy(backupFirstThinFlexIndex, m_pThinFlexIndex,
        ...
        do {
          ...
          newThinFlexVertexCount =
              g_fn_ComputeFlexedVertex_StreamOffset[nMorphPath](
                  nThinFlexVertexCount, pThinFlexVerts, pFirstThinFlexIndex,
                  pvanim, nCurrentTag, numVertsToProcess, weights.f4);
          memcpy(checkThinFlexVerts, m_pThinFlexVerts,
                 sizeof(m_pThinFlexVerts));
          memcpy(checkFirstThinFlexIndex, m_pThinFlexIndex,
                 sizeof(m_pThinFlexIndex));
          memcpy(m_pThinFlexVerts, backupThinFlexVerts,
                 sizeof(m_pThinFlexVerts));
          memcpy(m_pThinFlexIndex, backupFirstThinFlexIndex,
                 sizeof(m_pThinFlexIndex));

          ComputeFlexedVertex_StreamOffset(pStudioHdr, pflex, pvanim, vertCount,
                                           w1, w2, w3, w4);
          AlwaysAssert(m_ThinFlexVertexCount == newThinFlexVertexCount);
          for (int i = 0; i < newThinFlexVertexCount; ++i)
            AlwaysAssert(Diff(checkThinFlexVerts[i], m_pThinFlexVerts[i]) <
                         1e-5f);
          int indexOffset = m_pFirstThinFlexIndex - m_pThinFlexIndex;
          for (int i = 0; i < numVertsToProcess; ++i)
            AlwaysAssert(
                *(int *)&checkFirstThinFlexIndex[indexOffset +
                                                 pvanim[i].index] ==
...
void CCachedRenderData::ComputeFlexedVertexWrinkle_StreamOffset_Optimized(
    studiohdr_t *pStudioHdr, mstudioflex_t *pflex,
    mstudiovertanim_wrinkle_t *pvanim, int vertCount, float w1, float w2,
    float w3, float w4) {
#if PROFILE_THIS_FILE
  ...
  if (nMorphPath) {
    ...
    if (pvanim < pVertEnd) {
...
#endif
      {
        ...
        nThinFlexVertexCount =
            g_fn_ComputeFlexedVertexWrinkle_StreamOffset[nMorphPath](
                nThinFlexVertexCount, pThinFlexVerts, pFirstThinFlexIndex,
                pvanim, nCurrentTag, numVertsToProcess, weights.f4);
      }
#ifdef _DEBUG
