  ...
  for (int i = 0; i < m_ShadowState.Count(); ++i) {
    if (m_ShadowState[i].m_pMaterial) {
      ...
      R_StudioRenderModel(pRenderContext, 0, info.m_Body, 0,
                          m_ShadowState[i].m_pProxyData, NULL, NULL, flags,
...
inline void CStudioRender::R_ComputeLightAtPoints3(const FourVectors &pos,
  ...
  for (int i = 0; i < m_pRC->m_NumLocalLights; i++) {
    ...
    case MATERIAL_LIGHT_SPOT: {
      fltx4 dot2 = SubSIMD(
          Four_Zeros, delta * wl->m_Direction); // dot position with spot light
                                                // dir for cone falloff
...
// 			}
      ...
      for (int i = 0; i < 4; i++) {
        ...
        g_StudioRender.R_ComputeLightAtPoint3(dst[i].m_vecPosition,
                                              dst[i].m_vecNormal, color);
...
void CStudioRender::R_StudioProcessFlexedMesh_StreamOffset(mstudiomesh_t *pmesh,
  ...
  for (int i = 0; i < pmesh->numflexes; i++) {
    ...
    if (pflex[i].vertanimtype == STUDIO_VERT_ANIM_NORMAL) {
      ...
      m_VertexCache.ComputeFlexedVertex_StreamOffset_Optimized(
          m_pStudioHdr, &pflex[i], pvanim, vertCount, w1, w2, w3, w4);
    } else {
      mstudiovertanim_wrinkle_t *pvanim = pflex[i].pVertanimWrinkle(0);
      m_VertexCache.ComputeFlexedVertexWrinkle_StreamOffset_Optimized(
          m_pStudioHdr, &pflex[i], pvanim, vertCount, w1, w2, w3, w4);
    }
#else  // PLATFORM_WINDOWS
    if (pflex[i].vertanimtype == STUDIO_VERT_ANIM_NORMAL) {
      mstudiovertanim_t *pvanim = pflex[i].pVertanim(0);
      m_VertexCache.ComputeFlexedVertex_StreamOffset(
          m_pStudioHdr, &pflex[i], pvanim, vertCount, w1, w2, w3, w4);
    } else {
      mstudiovertanim_wrinkle_t *pvanim = pflex[i].pVertanimWrinkle(0);
      m_VertexCache.ComputeFlexedVertex_StreamOffset(
...
//-----------------------------------------------------------------------------
inline VertexFormat_t
CStudioRender::ComputeSWSkinVertexFormat(IMaterial *pMaterial) const {
...
int CStudioRender::R_StudioDrawDynamicMesh(IMatRenderContext *pRenderContext,
  ...
  if (m_pRC->m_Config.bDrawNormals || m_pRC->m_Config.bDrawTangentFrame) {
    ...
    R_StudioSoftwareProcessMesh_Normals(
        pmesh, meshBuilder, pGroup->m_NumVertices,
