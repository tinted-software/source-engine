...
void CStudioRender::R_StudioFlexVerts(mstudiomesh_t *pmesh, int lod) {
  ...
  const float flVertAnimFixedPointScale =
      m_pStudioHdr->VertAnimFixedPointScale();
  ...
  if (!vertData) {
    ...
    if (warnCount++ < 20)
      Warning("ERROR: R_StudioFlexVerts, model verts have been compressed, "
              "cannot render! (use \"-no_compressed_vvds\")");
    return;
  }
  ...
  if ((m_pStudioHdr->flags & STUDIOHDR_FLAGS_FLEXES_CONVERTED) == 0) {
    ...
    if (flexConversionTimesWarned++ < 6)
      Warning("ERROR: flex verts have not been converted (queued loader "
              "refcount bug?) - expect to see 'exploded' faces");
  }
  ...
  for (i = 0; i < pmesh->numflexes; i++) {
    ...
    for (j = 0; j < pflex[i].numverts; j++) {
      ...
      if (n < pmesh->vertexdata.numLODVertexes[lod]) {
        ...
        pFlexedVertex->m_Position +=
            pAnim->GetDeltaFixed(flVertAnimFixedPointScale) * w;
        pFlexedVertex->m_Normal +=
            pAnim->GetNDeltaFixed(flVertAnimFixedPointScale) * w;

        if (pStudioTangentS) {
          pFlexedVertex->m_TangentS.AsVector3D() +=
              pAnim->GetNDeltaFixed(flVertAnimFixedPointScale) * w;
          Assert(pFlexedVertex->m_TangentS.w == -1.0f ||
                 pFlexedVertex->m_TangentS.w == 1.0f);
        }
...
/*
void R_MouthLighting( int count, const Vector *psrcverts, const Vector
*psrcnorms, Vector4D *pdestlightvalues )
{
        Vector forward;

        if (m_pStudioHdr->nummouths < 1) return;

        mstudiomouth_t *pMouth = r_pstudiohdr->pMouth( 0 ); // FIXME: this needs
to get the mouth index from the shader
        ...
        VectorRotate( pMouth->forward, g_StudioInternalState.boneToWorld[
pMouth->bone ], forward );
        ...
        for (int i = 0; i < count; i++)
        {
                ...
                if (dot > 0)
                {
                        dot = LinearToTexture( dot ) / 255.0; // FIXME: this
isn't robust VectorScale( pdestlightvalues[i], dot, pdestlightvalues[i] );
                }
                else
