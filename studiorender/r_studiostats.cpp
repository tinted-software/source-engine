...
void CStudioRender::R_GatherStats(studiomeshgroup_t *pGroup,
  ...
  for (int i = 0; i < MeshBuilder.VertexCount(); i++) {
    ...
    TextureCoords[i] =
        (*(const Vector2D *)MeshBuilder.TexCoord(0)) * vTextureSize;
  }
  ...
  for (int j = 0; j < pGroup->m_NumStrips; ++j) {
    ...
    for (int i = 0; i < pStrip->numIndices; i += 3) {
      Indexes[pStrip->indexOffset + i] =
          pGroup->m_pIndices[pStrip->indexOffset + i];
      Indexes[pStrip->indexOffset + i + 1] =
          pGroup->m_pIndices[pStrip->indexOffset + i + 1];
      Indexes[pStrip->indexOffset + i + 2] =
          pGroup->m_pIndices[pStrip->indexOffset + i + 2];
  ...
  for (int j = 0; j < pGroup->m_NumStrips; ++j) {
    ...
    for (int i = 0; i < pStrip->numIndices; i += 3) {
      ...
      float flTriArea =
          TriangleArea(*(const Vector *)(pPos1), *(const Vector *)(pPos2),
                       *(const Vector *)(pPos3));
  ...
  //nCount, flSurfaceArea, flTriSurfaceArea, nFrontFacing, flTriSurfaceArea /
  //(float)nFrontFacing );
    ...
    for (int i = 0; i < pStrip->numIndices; i += 3) {
      ...
      if (flArea > 0.0f) {
        ...
        if (r_studio_stats.GetInt() == 1) {
          ...
          float flArea = TextureAreas[(pStrip->indexOffset + i) / 3] /
                         TriAreas[(pStrip->indexOffset + i) / 3];
...
//-----------------------------------------------------------------------------
void CStudioRender::ModelStats(const DrawModelInfo_t &info,
