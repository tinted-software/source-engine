...
//-----------------------------------------------------------------------------
StudioDecalHandle_t
CStudioRender::CreateDecalList(studiohwdata_t *pHardwareData) {
...
inline bool
CStudioRender::IsFrontFacing(const Vector *pnorm,
  ...
  if (pboneweight->numbones == 1) {
    z = DotProduct(pnorm->Base(),
                   m_PoseToDecal[(unsigned)pboneweight->bone[0]][2]);
  } else {
    float zbone;

    z = 0;
    for (int i = 0; i < pboneweight->numbones; i++) {
      zbone = DotProduct(pnorm->Base(),
                         m_PoseToDecal[(unsigned)pboneweight->bone[i]][2]);
...
inline bool
CStudioRender::TransformToDecalSpace(DecalBuildInfo_t &build, const Vector &pos,
  ...
  if (pboneweight->numbones == 1) {
    uv.x = DotProduct(pos.Base(),
                      m_PoseToDecal[(unsigned)pboneweight->bone[0]][0]) +
           m_PoseToDecal[(unsigned)pboneweight->bone[0]][0][3];
    uv.y = DotProduct(pos.Base(),
                      m_PoseToDecal[(unsigned)pboneweight->bone[0]][1]) +
           m_PoseToDecal[(unsigned)pboneweight->bone[0]][1][3];
  } else {
    uv.x = uv.y = 0;
    float ubone, vbone;
    for (int i = 0; i < pboneweight->numbones; i++) {
      ubone = DotProduct(pos.Base(),
                         m_PoseToDecal[(unsigned)pboneweight->bone[i]][0]) +
              m_PoseToDecal[(unsigned)pboneweight->bone[i]][0][3];
      vbone = DotProduct(pos.Base(),
                         m_PoseToDecal[(unsigned)pboneweight->bone[i]][1]) +
        ...
        if (pMaterial->m_Decals.Count() == 0) {
          ...
          if (list.m_pLod[iLOD].m_FirstMaterial == pDecalHistory->m_Material) {
            list.m_pLod[iLOD].m_FirstMaterial =
                m_DecalMaterial.Next(pDecalHistory->m_Material);
          }
...
void CStudioRender::AddDecal(StudioDecalHandle_t hDecal,
  ...
  if (nMeshCount >= 255) {
    Warning("Unable to apply decals to model (%s), it has more than 255 unique "
            "meshes!\n",
  ...
  if (!IsX360()) {
    ...
    buildInfo.m_pMeshVertices =
        (MeshVertexInfo_t *)malloc(nMeshCount * sizeof(MeshVertexInfo_t));
    int nVertexCount = ComputeVertexAllocation(
        iMaxLOD, body, list.m_pHardwareData, buildInfo.m_pMeshVertices);
    buildInfo.m_pVertexBuffer = (DecalBuildVertexInfo_t *)malloc(
        nVertexCount * sizeof(DecalBuildVertexInfo_t));
  }
  ...
  if (IsX360()) {
    while (g_nTotalDecalVerts * sizeof(DecalVertex_t) > 256 * 1024 &&
           m_DecalLRU.Head() != m_DecalLRU.InvalidIndex()) {
      DecalId_t nRetireID = m_DecalLRU[m_DecalLRU.Head()].m_nDecalId;
      StudioDecalHandle_t hRetire =
          m_DecalLRU[m_DecalLRU.Head()].m_hDecalHandle;
...
//			meshBuilder.TexCoordSubRect2f( 0, vertex.m_TexCoord.x, vertex.m_TexCoord.y, offset[0], offset[1], scale[0], scale[1] );
    ...
    if (meshBuilder.NumBoneWeights() >
        0) // bone weight of 0 will not write anything, so these calls would be
           // wasted
    {
...
bool CStudioRender::DrawMultiBoneFlexedDecals(IMatRenderContext *pRenderContext,
  ...
  for (DecalVertexList_t::IndexLocalType_t i = verts.Head();
    ...
    if (!pMorph) {
      ...
      if (meshBuilder.NumBoneWeights() >
          0) // bone weight of 0 will not write anything, so these calls would
             // be wasted
      {
        meshBuilder.BoneWeight(0, 1.0f);
        meshBuilder.BoneWeight(1, 0.0f);
      ...
      meshBuilder.BoneWeight(2, 1.0f - pBoneWeights->weight[1] -
                                    pBoneWeights->weight[0]);
      meshBuilder.BoneWeight(3, 0.0f);
