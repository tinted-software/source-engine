...
public:
  ...
  void DrawModelArray(const DrawModelInfo_t &drawInfo,
                      const StudioRenderContext_t &rc, int arrayCount,
  ...
  void ModelStats(const DrawModelInfo_t &info, const StudioRenderContext_t &rc,
                  matrix3x4_t *pBoneToWorld, const FlexWeights_t &flex,
                  int flags);
...
private:
  ...
  inline StudioModelLighting_t
  R_StudioComputeLighting(IMaterial *pMaterial, int materialFlags,
                          ColorMeshInfo_t *pColorMeshes);
  ...
  void R_GatherStats(studiomeshgroup_t *pGroup, CMeshBuilder &MeshBuilder,
                     IMesh *pMesh, IMaterial *pMaterial);
...
template <int nLightType> class CWorldLightAngleWrapper {
public:
...
template <int nLightType> class CWorldLightAngleWrapperConstDirectional {
public:
  FORCEINLINE static float WorldLightAngle(const LightDesc_t *wl,
                                           const Vector &lnormal,
                                           const Vector &snormal,
...
inline float CStudioRender::R_WorldLightAngle(const LightDesc_t *wl,
  ...
  case MATERIAL_LIGHT_POINT:
    return CWorldLightAngleWrapper<MATERIAL_LIGHT_POINT>::WorldLightAngle(
        wl, lnormal, snormal, delta);
  case MATERIAL_LIGHT_DIRECTIONAL:
    return CWorldLightAngleWrapper<MATERIAL_LIGHT_DIRECTIONAL>::WorldLightAngle(
        wl, lnormal, snormal, delta);
  case MATERIAL_LIGHT_SPOT:
...
//-----------------------------------------------------------------------------
inline void
CStudioRender::R_StudioEyeballNormal(mstudioeyeball_t const *peyeball,
                                     Vector &org, Vector &pos, Vector &normal) {
