...
void CStudioRender::InitDebugMaterials(void) {
  ...
  if (g_pMaterialSystemHardwareConfig->GetDXSupportLevel() >= 90) {
    ...
    m_pSSAODepthWrite[0][0] = g_pMaterialSystem->FindProceduralMaterial(
    ...
    pVMTKeyValues->SetInt("$nocull", 1);
    pVMTKeyValues->SetInt("$color_depth", 1);
    m_pSSAODepthWrite[0][1] = g_pMaterialSystem->FindProceduralMaterial(
    ...
    pVMTKeyValues->SetInt("$nocull", 0);
    pVMTKeyValues->SetInt("$color_depth", 1);
    m_pSSAODepthWrite[1][0] = g_pMaterialSystem->FindProceduralMaterial(
    ...
    pVMTKeyValues->SetInt("$nocull", 1);
    pVMTKeyValues->SetInt("$color_depth", 1);
    m_pSSAODepthWrite[1][1] = g_pMaterialSystem->FindProceduralMaterial(
...
void CStudioRender::DrawModelStaticProp(const DrawModelInfo_t &info,
  ...
  if ((flags & STUDIORENDER_GENERATE_STATS) != 0) {
    ...
    ModelStats(info, rc, m_pBoneToWorld, flex,
               flags | STUDIORENDER_DRAW_NO_FLEXES);
  ...
  if ((flags & (STUDIORENDER_SHADOWDEPTHTEXTURE |
                STUDIORENDER_SSAODEPTHTEXTURE)) == 0) {
...
// UNDONE: Currently no flex supported, no per instance cubemap or other
// lighting state supported, no eyeballs supported NOTE: This is a fast path for
// simple models with skeletons but not many other features
void CStudioRender::DrawModelArray(const DrawModelInfo_t &drawInfo,
                                   const StudioRenderContext_t &rc,
                                   int arrayCount,
  ...
  studiomeshdata_t *pMeshDataBase =
      drawInfo.m_pHardwareData->m_pLODs[drawInfo.m_Lod].m_pMeshData;
  IMaterial **ppMaterials =
      drawInfo.m_pHardwareData->m_pLODs[drawInfo.m_Lod].ppMaterials;
  int *pMaterialFlags =
      drawInfo.m_pHardwareData->m_pLODs[drawInfo.m_Lod].pMaterialFlags;
  ...
  for (int body = 0; body < pStudioHdr->numbodyparts; ++body) {
    ...
    for (int meshIndex = 0; meshIndex < pSubmodel->nummeshes; ++meshIndex) {
      ...
      // pMaterial, NULL, drawInfo.m_Lod );
      //  Draw all the various mesh groups...
      for (int meshGroupIndex = 0; meshGroupIndex < pMeshData->m_NumGroup;
           ++meshGroupIndex) {
        ...
        // Needed when we switch back and forth between hardware + software
        // lighting
        ...
        if (pStudioHdr->numbones > 1) {
          ...
          for (int i = 0; i < arrayCount; i++, pData += instanceStride) {
            matrix3x4_t *pBones =
                &(((model_array_instance_t *)pData)->modelToWorld);
            ...
            for (int j = 0; j < pGroup->m_NumStrips; ++j) {
              ...
              MaterialPrimitiveType_t localStripType =
                  pStrip->flags & OptimizedModel::STRIP_IS_TRISTRIP
