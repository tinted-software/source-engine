...
static int d_fill_extended_operator(struct demangle_component *, int,
                                    struct demangle_component *);
...
static struct demangle_component *d_make_demangle_mangled_name(struct d_info *,
                                                               const char *);
...
static struct demangle_component *d_ref_qualifier(struct d_info *,
                                                  struct demangle_component *);
...
static inline void d_growable_string_append_buffer(struct d_growable_string *,
                                                   const char *, size_t);
...
static void d_print_comp(struct d_print_info *, int,
                         const struct demangle_component *);

static void d_print_java_identifier(struct d_print_info *, const char *, int);

static void d_print_mod_list(struct d_print_info *, int, struct d_print_mod *,
                             int);

static void d_print_mod(struct d_print_info *, int,
                        const struct demangle_component *);

static void d_print_function_type(struct d_print_info *, int,
                                  const struct demangle_component *,
                                  struct d_print_mod *);

static void d_print_array_type(struct d_print_info *, int,
                               const struct demangle_component *,
                               struct d_print_mod *);

static void d_print_expr_op(struct d_print_info *, int,
                            const struct demangle_component *);
...
int cplus_demangle_fill_name(struct demangle_component *p, const char *s,
                             int len) {
...
CP_STATIC_IF_GLIBCPP_V3
int cplus_demangle_fill_extended_operator(struct demangle_component *p,
                                          int args,
...
int cplus_demangle_fill_ctor(struct demangle_component *p,
                             ...
                             struct demangle_component *name) {
  if (p == NULL || name == NULL || (int)kind < gnu_v3_complete_object_ctor ||
...
int cplus_demangle_fill_dtor(struct demangle_component *p,
                             ...
                             struct demangle_component *name) {
  if (p == NULL || name == NULL || (int)kind < gnu_v3_deleting_dtor ||
...
static struct demangle_component *
d_make_default_arg(struct d_info *di, int num, struct demangle_component *sub) {
...
struct demangle_component *cplus_demangle_mangled_name(struct d_info *di,
  ...
  if (top_level && (di->options & DMGL_PARAMS) != 0)
    while (d_peek_char(di) == '.' &&
           (IS_LOWER(d_peek_next_char(di)) || d_peek_next_char(di) == '_' ||
...
static struct demangle_component *d_abi_tags(struct d_info *di,
                                             struct demangle_component *dc) {
  char peek;
  while (peek = d_peek_char(di), peek == 'B') {
...
static struct demangle_component *d_name(struct d_info *di) {
  ...
  case 'S': {
    ...
    if (d_peek_char(di) != 'I') {
      ...
      dc =
          d_make_comp(di, DEMANGLE_COMPONENT_TEMPLATE, dc, d_template_args(di));
    }
  ...
  while (1) {
    ...
    if (peek == 'D') {
      ...
      if (peek2 == 'T' || peek2 == 't')
        ...
        /* Destructor name.  */
        dc = d_unqualified_name(di);
    } else if (IS_DIGIT(peek) || IS_LOWER(peek) || peek == 'C' || peek == 'U' ||
               peek == 'L')
      dc = d_unqualified_name(di);
    else if (peek == 'S')
...
static struct demangle_component *d_special_name(struct d_info *di) {
  ...
  if (d_check_char(di, 'T')) {
    ...
    case 'V':
      di->expansion -= 5;
      return d_make_comp(di, DEMANGLE_COMPONENT_VTABLE, cplus_demangle_type(di),
                         NULL);
    case 'T':
      di->expansion -= 10;
    ...
    case 'C': {
      ...
      /* We don't display the offset.  FIXME: We should display
         it in verbose mode.  */
      di->expansion += 5;
      return d_make_comp(di, DEMANGLE_COMPONENT_CONSTRUCTION_VTABLE, base_type,
                         derived_type);
    }
    ...
    case 'H':
      return d_make_comp(di, DEMANGLE_COMPONENT_TLS_INIT, d_name(di), NULL);

    case 'W':
      return d_make_comp(di, DEMANGLE_COMPONENT_TLS_WRAPPER, d_name(di), NULL);
    ...
    case 'A':
      return d_make_comp(di, DEMANGLE_COMPONENT_HIDDEN_ALIAS, d_encoding(di, 0),
                         NULL);
...
const struct demangle_builtin_type_info
    cplus_demangle_builtin_types[D_BUILTIN_TYPE_COUNT] = {
        ...
        /* o */
        {NL("unsigned __int128"), NL("unsigned __int128"), D_PRINT_DEFAULT},
        ...
        /* 32 */
        {NL("decltype(nullptr)"), NL("decltype(nullptr)"), D_PRINT_DEFAULT},
};
...
struct demangle_component *cplus_demangle_type(struct d_info *di) {
  ...
  case 'R':
    d_advance(di, 1);
    ret = d_make_comp(di, DEMANGLE_COMPONENT_REFERENCE, cplus_demangle_type(di),
                      NULL);
    break;
  ...
  case 'G':
    d_advance(di, 1);
    ret = d_make_comp(di, DEMANGLE_COMPONENT_IMAGINARY, cplus_demangle_type(di),
                      NULL);
    break;
  ...
  case 'D':
    ...
    case 't':
      /* decltype (expression) */
      ret =
          d_make_comp(di, DEMANGLE_COMPONENT_DECLTYPE, d_expression(di), NULL);
...
d_cv_qualifiers(struct d_info *di, struct demangle_component **pret,
  ...
  while (peek == 'r' || peek == 'V' || peek == 'K') {
    ...
    d_advance(di, 1);
    if (peek == 'r') {
      t = (member_fn ? DEMANGLE_COMPONENT_RESTRICT_THIS
                     : DEMANGLE_COMPONENT_RESTRICT);
      di->expansion += sizeof "restrict";
    } else if (peek == 'V') {
      t = (member_fn ? DEMANGLE_COMPONENT_VOLATILE_THIS
                     : DEMANGLE_COMPONENT_VOLATILE);
      di->expansion += sizeof "volatile";
    } else {
      t = (member_fn ? DEMANGLE_COMPONENT_CONST_THIS
...
static int op_is_new_cast(struct demangle_component *op) {
  ...
  return (code[1] == 'c' && (code[0] == 's' || code[0] == 'd' ||
                             code[0] == 'c' || code[0] == 'r'));
}
...
static inline struct demangle_component *d_expression_1(struct d_info *di) {
  ...
  if (peek == 'L')
    ...
    if (peek == 't')
      type = cplus_demangle_type(di);
    d_advance(di, 2);
    return d_make_comp(di, DEMANGLE_COMPONENT_INITIALIZER_LIST, type,
                       d_exprlist(di, 'E'));
  } else {
    ...
    case 1: {
      ...
      if (suffix)
        ...
        return d_make_comp(
            di, DEMANGLE_COMPONENT_UNARY, op,
            d_make_comp(di, DEMANGLE_COMPONENT_BINARY_ARGS, operand, operand));
      else
    ...
    case 2: {
      ...
      if (!strcmp(code, "cl"))
        ...
        if (d_peek_char(di) == 'I')
          right = d_make_comp(di, DEMANGLE_COMPONENT_TEMPLATE, right,
                              d_template_args(di));
      } else
        right = d_expression_1(di);

      return d_make_comp(
          di, DEMANGLE_COMPONENT_BINARY, op,
    ...
    case 3: {
      ...
      if (!strcmp(code, "qu")) {
        ...
        return NULL;
      return d_make_comp(
          di, DEMANGLE_COMPONENT_TRINARY, op,
          d_make_comp(
              di, DEMANGLE_COMPONENT_TRINARY_ARG1, first,
...
/* <local-name> ::= Z <(function) encoding> E <(entity) name> [<discriminator>]
                ...
                ::= Z <(function) encoding> E d [<parameter> number>] _ <entity
   name>
*/
...
static struct demangle_component *d_local_name(struct d_info *di) {
  ...
  if (d_peek_char(di) == 's') {
    ...
    if (!d_discriminator(di))
      return NULL;
    return d_make_comp(
        di, DEMANGLE_COMPONENT_LOCAL_NAME, function,
    ...
    for (p = &standard_subs[0]; p < pend; ++p) {
      if (c == p->code) {
        ...
        if (p->set_last_name != NULL)
          di->last_name =
              d_make_sub(di, p->set_last_name, p->set_last_name_len);
        if (verbose) {
          s = p->full_expansion;
          len = p->full_len;
        } else {
...
static void d_checkpoint(struct d_info *di,
                         struct d_info_checkpoint *checkpoint) {
  checkpoint->n = di->n;
...
static void d_backtrack(struct d_info *di,
                        struct d_info_checkpoint *checkpoint) {
  di->n = checkpoint->n;
...
static void d_growable_string_callback_adapter(const char *s, size_t l,
                                               void *opaque) {
...
static void d_count_templates_scopes(int *num_templates, int *num_scopes,
  ...
  case DEMANGLE_COMPONENT_CLONE:
  recurse_left_right:
    d_count_templates_scopes(num_templates, num_scopes, d_left(dc));
    d_count_templates_scopes(num_templates, num_scopes, d_right(dc));
    break;

  case DEMANGLE_COMPONENT_CTOR:
    d_count_templates_scopes(num_templates, num_scopes, dc->u.s_ctor.name);
    break;

  case DEMANGLE_COMPONENT_DTOR:
    d_count_templates_scopes(num_templates, num_scopes, dc->u.s_dtor.name);
    break;
  ...
  case DEMANGLE_COMPONENT_LAMBDA:
  case DEMANGLE_COMPONENT_DEFAULT_ARG:
    d_count_templates_scopes(num_templates, num_scopes, dc->u.s_unary_num.sub);
    break;
  }
}
...
static void d_print_init(struct d_print_info *dpi,
  ...
  d_count_templates_scopes(&dpi->num_copy_templates, &dpi->num_saved_scopes,
                           dc);
...
char *cplus_demangle_print(int options, const struct demangle_component *dc,
  ...
  if (!cplus_demangle_print_callback(
          options, dc, d_growable_string_callback_adapter, &dgs)) {
    free(dgs.buf);
    *palc = 0;
    return NULL;
  }
...
static struct demangle_component *
d_find_pack(struct d_print_info *dpi, const struct demangle_component *dc) {
  struct demangle_component *a;
  if (dc == NULL)
    return NULL;
...
static void d_print_subexpr(struct d_print_info *dpi, int options,
                            const struct demangle_component *dc) {
  int simple = 0;
...
static void d_save_scope(struct d_print_info *dpi,
                         const struct demangle_component *container) {
...
static void d_print_comp(struct d_print_info *dpi, int options,
                         const struct demangle_component *dc) {
  ...
  case DEMANGLE_COMPONENT_TYPED_NAME: {
    ...
    if (typed_name->type == DEMANGLE_COMPONENT_LOCAL_NAME) {
      ...
      while (local_name->type == DEMANGLE_COMPONENT_RESTRICT_THIS ||
             ...
             (local_name->type == DEMANGLE_COMPONENT_RVALUE_REFERENCE_THIS)) {
        if (i >= sizeof adpm / sizeof adpm[0]) {
          d_print_error(dpi);
          return;
        }
  ...
  case DEMANGLE_COMPONENT_TEMPLATE: {
    ...
    if ((options & DMGL_JAVA) != 0 && dcl->type == DEMANGLE_COMPONENT_NAME &&
  ...
  case DEMANGLE_COMPONENT_FUNCTION_TYPE: {
    if ((options & DMGL_RET_POSTFIX) != 0)
      d_print_function_type(dpi, options & ~(DMGL_RET_POSTFIX | DMGL_RET_DROP),
                            dc, dpi->modifiers);
  ...
  case DEMANGLE_COMPONENT_ARRAY_TYPE: {
    ...
    i = 1;
    pdpm = hold_modifiers;
    while (pdpm != NULL && (pdpm->mod->type == DEMANGLE_COMPONENT_RESTRICT ||
                            pdpm->mod->type == DEMANGLE_COMPONENT_VOLATILE ||
                            pdpm->mod->type == DEMANGLE_COMPONENT_CONST)) {
      if (!pdpm->printed) {
        if (i >= sizeof adpm / sizeof adpm[0]) {
...
static void d_print_java_identifier(struct d_print_info *dpi, const char *name,
                                    int len) {
  const char *p;
  const char *end;
...
static void d_print_mod_list(struct d_print_info *dpi, int options,
  ...
  if (mods->printed ||
      (!suffix &&
       (mods->mod->type == DEMANGLE_COMPONENT_RESTRICT_THIS ||
        ...
        (mods->mod->type == DEMANGLE_COMPONENT_RVALUE_REFERENCE_THIS)))) {
...
static void d_print_mod(struct d_print_info *dpi, int options,
                        const struct demangle_component *mod) {
  switch (mod->type) {
  case DEMANGLE_COMPONENT_RESTRICT:
  case DEMANGLE_COMPONENT_RESTRICT_THIS:
...
static void d_print_expr_op(struct d_print_info *dpi, int options,
                            const struct demangle_component *dc) {
...
static void d_print_cast(struct d_print_info *dpi, int options,
                         const struct demangle_component *dc) {
  d_print_comp(dpi, options, d_left(dc));
}
...
static void d_print_conversion(struct d_print_info *dpi, int options,
                               const struct demangle_component *dc) {
  struct d_print_template dpt;
...
static int d_demangle_callback(const char *mangled, int options,
                               demangle_callbackref callback, void *opaque) {
  enum { DCT_TYPE, DCT_MANGLED, DCT_GLOBAL_CTORS, DCT_GLOBAL_DTORS } type;
  struct d_info di;
  struct demangle_component *dc;
  int status;
...
int __gcclibcxx_demangle_callback(const char *mangled_name,
  ...
  status = d_demangle_callback(mangled_name, DMGL_PARAMS | DMGL_TYPES, callback,
                               opaque);
  if (status == 0)
    return -2;
...
int java_demangle_v3_callback(const char *mangled,
  ...
  return d_demangle_callback(
      mangled, DMGL_JAVA | DMGL_PARAMS | DMGL_RET_POSTFIX, callback, opaque);
}
...
static void print_usage(FILE *fp, int exit_value) {
  ...
  fprintf(fp, "If names are provided, they are demangled.  Otherwise filters "
              "standard input.\n");
