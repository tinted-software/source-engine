...
void printPC(void *pc) {
  ...
  snprintf(message, sizeof(message),
           "0x%" PRIXPTR ":\t%s (base=0x%" PRIXPTR ", %s)\n", relative_addr,
           symbol, (uintptr_t)info.dli_fbase, fname);
  Log(message);
}

_Unwind_Reason_Code
UnwindBacktraceCallback(struct _Unwind_Context *unwind_context,
                        void *state_voidp) {
...
static void CrashHandler(int sig, siginfo_t *si, void *uc) {
  ...
  snprintf(message, sizeof(message),
           "Signal=%d, errno=%d, code=%d, addr=0x%" PRIXPTR "\n", sig,
           si->si_errno, si->si_code, (uintptr_t)si->si_addr);
  Log(message);
