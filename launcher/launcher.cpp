...
void CLogAllFiles::Init() {
  ...
  char path[MAX_PATH];
  Q_snprintf(path, sizeof(path), "%s/%s", GetBaseDirectory(),
...
bool CSourceAppSystemGroup::PreInit() {
  ...
  if (!steamInfo.m_pDirectoryName) {
    ...
    if (!steamInfo.m_pDirectoryName) {
      Error("FileSystem_LoadFileSystemModule: no -defaultgamedir or -game "
            "specified.");
    }
  }
  ...
  if (IsPC()) {
    ...
    reslistgenerator->Init(GetBaseDirectory(),
                           CommandLine()->ParmValue("-game", "hl2"));
...
int CSourceAppSystemGroup::Main() { return g_pEngineAPI->Run(); }
...
bool GrabSourceMutex() {
  ...
  // Under OSX use flock in /tmp/source_engine_<game>.lock, create the file if
  // it doesn't exist
  const char *pchGameParam =
  ...
  // If it's NULL, or it doesn't exist, or it isn't a directory, fallback to
  // /tmp.
  struct stat buf;
  if (!tmpdir || stat(tmpdir, &buf) || !S_ISDIR(buf.st_mode))
    tmpdir = "/tmp";

  V_snprintf(g_lockFilename, sizeof(g_lockFilename), "%s/source_engine_%u.lock",
             tmpdir, gameCRC);
   ...
   * OSX
   */
  V_snprintf(g_lockFilename, sizeof(g_lockFilename),
             "/tmp/source_engine_%u.lock", gameCRC);

  g_lockfd = open(g_lockFilename,
                  O_CREAT | O_WRONLY | O_EXLOCK | O_NONBLOCK | O_TRUNC, 0777);
  if (g_lockfd >= 0) {
    // make sure we give full perms to the file, we only one instance per
    // machine
    fchmod(g_lockfd, 0777);

    // we leave the file open, under unix rules when we die we'll automatically
    // close and remove the locks
    return true;
  }

  // We were unable to open the file, it should be because we are unable to
  // retain a lock
...
//-----------------------------------------------------------------------------
#ifdef WIN32
DLL_EXPORT int LauncherMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
...
#endif
{
  ...
  // Temporary fix to stop us from crashing in printf/sscanf functions that
  // don't expect
  //  localization to mess with your "." and "," float seperators. Mac OSX also
  ...
  // Also check if C.UTF-8 exists and use it? This file:
  // /usr/lib/locale/C.UTF-8. It looks like it's only installed on Debian
  // distros right now though.
  ...
  if (Q_stricmp(CurrentLocale, en_US)) {
    Msg("WARNING: setlocale('%s') failed, using locale:'%s'. International "
  ...
  Msg("SDL version: %d.%d.%d rev: %s\n", (int)ver.major, (int)ver.minor,
      (int)ver.patch, SDL_GetRevision());
#endif

#if (defined LINUX || defined PLATFORM_BSD) && defined USE_SDL &&              \
    defined TOGLES && !defined ANDROID
...
#if defined(_X360)
  if (bWaitForConsole)
    COM_TimestampedLog("LauncherMain: Application Start - %s",
  ...
  if (storageID != XBX_INVALID_STORAGE_ID &&
      storageID != XBX_STORAGE_DECLINED) {
    ...
    if (ret != ERROR_SUCCESS) {
      ...
      XBX_QueueEvent(XEV_LISTENER_NOTIFICATION, WM_SYS_STORAGEDEVICESCHANGED, 0,
                     0);
    }
  }
  ...
  if (CommandLine()->CheckParm("-textmode")) {
    ...
    if (!GrabSourceMutex()) {
      // We're going to hijack the existing session and load a new savegame into
      // it. This will mainly occur when users click on links in Bugzilla that
      // will automatically copy saves and load them directly from the web
      // browser. The -hijack command prevents the launcher from objecting that
      // there is already an instance of the game.
      if (CommandLine()->CheckParm("-hijack")) {
        ...
        if (hwndEngine == NULL) {
          ::MessageBox(
              NULL,
              "The modified entity keyvalues could not be sent to the Source "
              "Engine because the engine does not appear to be running.",
          ...
          if (!::SendMessage(hwndEngine, WM_COPYDATA, 0, (LPARAM)&copyData)) {
            ::MessageBox(
                NULL,
                "The Source Engine was found running, but did not accept the "
                "request to load a savegame. It may be an old version of the "
                "engine that does not support this functionality.",
        ...
        if (!g_MultiRun) {
          ::MessageBox(
              NULL, "Only one instance of the game can be running at one time.",
  ...
  g_LeakDump.m_bCheckLeaks =
      CommandLine()->CheckParm("-leakcheck") ? true : false;
  ...
  // here, exec it. This supports the capability of immediately re-launching the
  // the game via Steam in a different audio language
  HKEY hKey;
