...
//-----------------------------------------------------------------------------
CDmeBoneFlexDriverControl *
CDmeBoneFlexDriver::FindOrCreateControl(const char *pszControlName) {
  ...
  for (int i = 0; i < m_eControlList.Count(); ++i) {
    ...
    if (!Q_stricmp(pszControlName,
                   pDmeBoneFlexDriverControl->m_sFlexControllerName.Get()))
      return pDmeBoneFlexDriverControl;
  }

  pDmeBoneFlexDriverControl =
      CreateElement<CDmeBoneFlexDriverControl>("", GetFileId()); // Nameless
  if (!pDmeBoneFlexDriverControl)
    return NULL;
...
//-----------------------------------------------------------------------------
CDmeBoneFlexDriver *
CDmeBoneFlexDriverList::FindOrCreateBoneFlexDriver(const char *pszBoneName) {
  ...
  pDmeBoneFlexDriver =
      CreateElement<CDmeBoneFlexDriver>("", GetFileId()); // Nameless
  if (!pDmeBoneFlexDriver)
    return NULL;
