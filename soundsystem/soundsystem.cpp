...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CSoundSystem, ISoundSystem,
                                  SOUNDSYSTEM_INTERFACE_VERSION, s_SoundSystem);
...
void *CSoundSystem::QueryInterface(const char *pInterfaceName) {
  if (!Q_strncmp(pInterfaceName, SOUNDSYSTEM_INTERFACE_VERSION,
                 Q_strlen(SOUNDSYSTEM_INTERFACE_VERSION) + 1))
...
void CSoundSystem::PlaySound(CAudioSource *source, float volume,
                             CAudioMixer **ppMixer) {
  if (ppMixer) {
    *ppMixer = NULL;
  }
