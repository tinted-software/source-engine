...
static VideoSystemInfo_t s_VideoAppSystems[] = {
    {VideoSystem::QUICKTIME, PLATFORM_WIN32 | PLATFORM_OSX, "video_quicktime",
     VIDEO_SUBSYSTEM_INTERFACE_VERSION},
    {VideoSystem::BINK,
     PLATFORM_WIN32 | PLATFORM_OSX | PLATFORM_XBOX_360 | PLATFORM_LINUX,
     "video_bink", VIDEO_SUBSYSTEM_INTERFACE_VERSION},
    //{ VideoSystem::AVI,			PLATFORM_WIN32,
    //"avi",				VIDEO_SUBSYSTEM_INTERFACE_VERSION }, {
    //VideoSystem::WMV,			PLATFORM_WIN32,
    //"wmv",				VIDEO_SUBSYSTEM_INTERFACE_VERSION },
    {VideoSystem::WEBM, PLATFORM_LINUX, "video_webm",
     VIDEO_SUBSYSTEM_INTERFACE_VERSION},

    {VideoSystem::NONE, PLATFORM_NONE, nullptr,
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CValveVideoServices, IVideoServices,
...
CValveVideoServices::CValveVideoServices()
    : m_nInstalledSystems(0), m_bInitialized(false), m_nMaterialCount(0) {
...
bool CValveVideoServices::Connect(CreateInterfaceFn factory) {
  ...
  if (g_pFullFileSystem == nullptr || materials == nullptr) {
    Msg("Valve Video Services unable to connect due to missing dependent "
        "system\n");
    return false;
  }
...
void *CValveVideoServices::QueryInterface(const char *pInterfaceName) {
  if (Q_strncmp(pInterfaceName, VIDEO_SERVICES_INTERFACE_VERSION,
                Q_strlen(VIDEO_SERVICES_INTERFACE_VERSION) + 1) == 0) {
...
bool CValveVideoServices::ConnectVideoLibraries(CreateInterfaceFn factory) {
  ...
  while (IS_NOT_EMPTY(s_VideoAppSystems[n].m_pModuleName) &&
         s_VideoAppSystems[n].m_SystemID != VideoSystem::NONE) {
    if (BITFLAGS_SET(s_VideoAppSystems[n].m_Platforms, thisPlatform)) {
      ...
      if (pModule != nullptr) {
        ...
        if (fn != nullptr) {
          ...
          IVideoSubSystem *pVideoSystem = (IVideoSubSystem *)fn(
              s_VideoAppSystems[n].m_pInterfaceName, NULL);
        ...
        if (success == false) {
          ...
          Msg("Error occurred while attempting to load and initialize Video "
              "Subsystem\n Video Subsystem module '%s'\n Video Subsystem "
              "Interface  '%s'",
              s_VideoAppSystems[n].m_pModuleName,
              s_VideoAppSystems[n].m_pInterfaceName);
  ...
  // now we query each video system for its capabilities, and supported file
  // extensions
  for (int i = VideoSystem::VIDEO_SYSTEM_FIRST;
       i < VideoSystem::VIDEO_SYSTEM_COUNT; i++) {
    ...
    if (pSubSystem != nullptr) {
      ...
      for (int n = 0; n < eCount; n++) {
        ...
        extInfoRec.m_VideoFeatures =
            pSubSystem->GetSupportedFileExtensionFeatures(n);
...
InitReturnVal_t CValveVideoServices::Init() {
  ...
  for (int n = VideoSystem::VIDEO_SYSTEM_FIRST;
       n < VideoSystem::VIDEO_SYSTEM_COUNT; n++) {
...
VideoSystem_t CValveVideoServices::GetAvailableVideoSystem(int n) {
  ...
  for (int i = VideoSystem::VIDEO_SYSTEM_FIRST, c = 0;
       i < VideoSystem::VIDEO_SYSTEM_COUNT; i++) {
    if (m_VideoSystems[i] != nullptr) {
      if (c == n) {
...
int CValveVideoServices::GetIndexForSystem(VideoSystem_t n) {
  if (n >= VideoSystem::VIDEO_SYSTEM_FIRST &&
      n < VideoSystem::VIDEO_SYSTEM_COUNT && m_nInstalledSystems > 0) {
    int i = (int)n;
    if (m_VideoSystems[i] != nullptr &&
        m_VideoSystemFeatures[i] != VideoSystemFeature::NO_FEATURES) {
      return i;
...
VideoSystemStatus_t
CValveVideoServices::GetVideoSystemStatus(VideoSystem_t videoSystem) {
  int n = GetIndexForSystem(videoSystem);
  return (n != SYSTEM_NOT_FOUND) ? m_VideoSystems[n]->GetSystemStatus()
                                 : VideoSystemStatus::NOT_INSTALLED;
}

VideoSystemFeature_t
CValveVideoServices::GetVideoSystemFeatures(VideoSystem_t videoSystem) {
  int n = GetIndexForSystem(videoSystem);
  return (n != SYSTEM_NOT_FOUND) ? m_VideoSystemFeatures[n]
                                 : VideoSystemFeature::NO_FEATURES;
}

const char *CValveVideoServices::GetVideoSystemName(VideoSystem_t videoSystem) {
  int n = GetIndexForSystem(videoSystem);
  return (n != SYSTEM_NOT_FOUND) ? m_VideoSystems[n]->GetVideoSystemName()
                                 : nullptr;
}

VideoSystem_t
CValveVideoServices::FindNextSystemWithFeature(VideoSystemFeature_t features,
                                               VideoSystem_t startAfter) {
  ...
  if (startAfter != VideoSystem::NONE &&
      IS_IN_RANGECOUNT(startAfter, VideoSystem::VIDEO_SYSTEM_FIRST,
                       VideoSystem::VIDEO_SYSTEM_COUNT)) {
  ...
  for (int i = start; i < VideoSystem::VIDEO_SYSTEM_COUNT; i++) {
    if (m_VideoSystems[i] != nullptr &&
        BITFLAGS_SET(m_VideoSystemFeatures[i], features)) {
...
// ===========================================================================
int CValveVideoServices::GetSupportedFileExtensionCount(
    VideoSystem_t videoSystem) {
  int n = GetIndexForSystem(videoSystem);

  return (n == SYSTEM_NOT_FOUND)
             ? 0
             : m_VideoSystems[n]->GetSupportedFileExtensionCount();
}

const char *
CValveVideoServices::GetSupportedFileExtension(VideoSystem_t videoSystem,
                                               int extNum) {
  int n = GetIndexForSystem(videoSystem);

  int c = (n == SYSTEM_NOT_FOUND)
              ? 0
              : m_VideoSystems[n]->GetSupportedFileExtensionCount();
  ;
  ...
  return (extNum < 0 || extNum >= c)
             ? nullptr
             : m_VideoSystems[n]->GetSupportedFileExtension(extNum);
}

VideoSystemFeature_t CValveVideoServices::GetSupportedFileExtensionFeatures(
    VideoSystem_t videoSystem, int extNum) {
  int n = GetIndexForSystem(videoSystem);
...
VideoSystem_t CValveVideoServices::LocateVideoSystemForPlayingFile(
    const char *pFileName, VideoSystemFeature_t playMode) {
  SetResult(VideoResult::BAD_INPUT_PARAMETERS);
  AssertExitV(IS_NOT_EMPTY(pFileName), VideoSystem::NONE);

  VideoSystem_t theSystem =
      LocateSystemAndFeaturesForFileName(pFileName, nullptr, playMode);
...
// ===========================================================================
VideoResult_t CValveVideoServices::LocatePlayableVideoFile(
    const char *pSearchFileName, const char *pPathID,
    VideoSystem_t *pPlaybackSystem, char *pPlaybackFileName, int fileNameMaxLen,
    VideoSystemFeature_t playMode) {
  AssertExitV(IS_NOT_EMPTY(pSearchFileName) || pPlaybackSystem == nullptr ||
                  pPlaybackSystem == nullptr || fileNameMaxLen <= 0,
              SetResult(VideoResult::BAD_INPUT_PARAMETERS));

  VideoResult_t Status = ResolveToPlayableVideoFile(
      pSearchFileName, pPathID, VideoSystem::DETERMINE_FROM_FILE_EXTENSION,
      playMode, true, pPlaybackFileName, fileNameMaxLen, pPlaybackSystem);
...
// ===========================================================================
IVideoMaterial *CValveVideoServices::CreateVideoMaterial(
    const char *pMaterialName, const char *pVideoFileName, const char *pPathID,
    VideoPlaybackFlags_t playbackFlags, VideoSystem_t videoSystem,
  ...
  AssertExitV(videoSystem == VideoSystem::DETERMINE_FROM_FILE_EXTENSION ||
                  IS_IN_RANGECOUNT(videoSystem, VideoSystem::VIDEO_SYSTEM_FIRST,
                                   VideoSystem::VIDEO_SYSTEM_COUNT),
              nullptr);
  ...
  VideoResult_t Status = ResolveToPlayableVideoFile(
      pVideoFileName, pPathID, videoSystem,
      VideoSystemFeature::PLAY_VIDEO_FILE_IN_MATERIAL,
      PlayAlternateIfNotAvailable, ResolvedFilePath, sizeof(ResolvedFilePath),
      &actualVideoSystem);
  ...
  IVideoMaterial *pMaterial = m_VideoSystems[sysIndex]->CreateVideoMaterial(
      pMaterialName, ResolvedFilePath, playbackFlags);
...
VideoResult_t
CValveVideoServices::DestroyVideoMaterial(IVideoMaterial *pVideoMaterial) {
  ...
  for (int i = 0; i < m_MaterialList.Count(); i++) {
    if (m_MaterialList[i].m_pObject == pVideoMaterial) {
      VideoResult_t Status =
          m_VideoSystems[m_MaterialList[i].m_VideoSystem]->DestroyVideoMaterial(
              pVideoMaterial);
...
// ===========================================================================
VideoResult_t
CValveVideoServices::IsRecordCodecAvailable(VideoSystem_t videoSystem,
                                            VideoEncodeCodec_t codec) {
  AssertExitV(codec >= VideoEncodeCodec::DEFAULT_CODEC &&
                  codec < VideoEncodeCodec::CODEC_COUNT,
              SetResult(VideoResult::BAD_INPUT_PARAMETERS));
...
// ===========================================================================
IVideoRecorder *
CValveVideoServices::CreateVideoRecorder(VideoSystem_t videoSystem) {
  int n = GetIndexForSystem(videoSystem);
  ...
  if (!BITFLAGS_SET(m_VideoSystemFeatures[n],
                    VideoSystemFeature::ENCODE_VIDEO_TO_FILE)) {
...
VideoResult_t
CValveVideoServices::DestroyVideoRecorder(IVideoRecorder *pVideoRecorder) {
  ...
  for (int i = 0; i < m_RecorderList.Count(); i++) {
    if (m_RecorderList[i].m_pObject == pVideoRecorder) {
      VideoResult_t Status =
          m_VideoSystems[m_RecorderList[i].m_VideoSystem]->DestroyVideoRecorder(
              pVideoRecorder);
...
// ===========================================================================
VideoResult_t CValveVideoServices::PlayVideoFileFullScreen(
    ...
    bool windowed, float forcedMinTime, VideoPlaybackFlags_t playbackFlags,
  ...
  if (sysIndex != SYSTEM_NOT_FOUND) {
    return SetResult(m_VideoSystems[sysIndex]->PlayVideoFileFullScreen(
        ResolvedFilePath, mainWindow, windowWidth, windowHeight, desktopWidth,
        desktopHeight, windowed, forcedMinTime, playbackFlags));
  } else {
...
// ===========================================================================
VideoResult_t
CValveVideoServices::SoundDeviceCommand(VideoSoundDeviceOperation_t operation,
                                        void *pDevice, void *pData,
                                        VideoSystem_t videoSystem) {
  AssertExitV(IS_IN_RANGECOUNT(operation, 0,
                               VideoSoundDeviceOperation::OPERATION_COUNT),
              SetResult(VideoResult::BAD_INPUT_PARAMETERS));
  ...
  for (int i = startIdx; i <= lastIdx; i++) {
    ...
    if (n != SYSTEM_NOT_FOUND) {
      result =
          m_VideoSystems[n]->VideoSoundDeviceCMD(operation, pDevice, pData);
    }
  }
...
const wchar_t *CValveVideoServices::GetCodecName(VideoEncodeCodec_t nCodec) {
  static const char *s_pCodecLookup[VideoEncodeCodec::CODEC_COUNT] = {
      "#Codec_MPEG2",    "#Codec_MPEG4",   "#Codec_H261",    "#Codec_H263",
      "#Codec_H264",     "#Codec_MJPEG_A", "#Codec_MJPEG_B", "#Codec_SORENSON3",
      "#Codec_CINEPACK", "#Codec_WEBM",
  };
...
// ===========================================================================
VideoResult_t CValveVideoServices::ResolveToPlayableVideoFile(
  ...
  AssertExitV(videoSystem == VideoSystem::DETERMINE_FROM_FILE_EXTENSION ||
                  IS_IN_RANGECOUNT(videoSystem, VideoSystem::VIDEO_SYSTEM_FIRST,
                                   VideoSystem::VIDEO_SYSTEM_COUNT),
              VideoResult::BAD_INPUT_PARAMETERS);
  AssertExitV(requiredFeature != VideoSystemFeature::NO_FEATURES,
              VideoResult::BAD_INPUT_PARAMETERS);
  AssertExitV(pResolvedFileName != nullptr && resolvedFileNameMaxLen > 0 &&
                  pResolvedVideoSystem != nullptr,
              VideoResult::BAD_INPUT_PARAMETERS);
  ...
  *pResolvedVideoSystem = VideoSystem::NONE;
  ...
  // check the file extension to see if it specifies searching for any
  // compatible video files if so, override a couple input values
  ...
  // We start with either the specified video system.. OR.. we choose the system
  // based on the file extension Get the system and if it's valid, it's
  // available features
  if (videoSystem != VideoSystem::DETERMINE_FROM_FILE_EXTENSION) {
    sysIdx =
        GetIndexForSystem(videoSystem); // Caller specified the video system
    sysFeatures = (sysIdx != SYSTEM_NOT_FOUND)
                      ? m_VideoSystemFeatures[sysIdx]
                      : VideoSystemFeature::NO_FEATURES;
  } else {
    // We need to determine the system to use based on filename
    sysIdx = GetIndexForSystem(LocateSystemAndFeaturesForFileName(
        pFileName, &sysFeatures, requiredFeature));
  }

  // if we don't have a system to play this video.. and aren't allowed to look
  // for an alternative...
  ...
  // Examine the requested of inferred video system to see if it can do what we
  // want, and if so, see if the corresponding file is actually found (we
  // support search paths)

  // Decision Path for when we have a preferred/specified video system specified
  // to use
  if (sysIdx != SYSTEM_NOT_FOUND) {
    bool fileFound = false;

    // if the request system can do the task, see if we can find the file as
    ...
    if (BITFLAGS_SET(sysFeatures, requiredFeature)) {
      if (V_IsAbsolutePath(pFileName)) {
        ...
        fileFound = (g_pFullFileSystem->RelativePathToFullPath(
                         pFileName, pPathID, ActualFilePath,
                         sizeof(ActualFilePath)) != nullptr);
      }
    } else // The specified video system does not support this (required)
           // feature
    {
      // if we can't search for an alternative file, tell them we don't support
      // this
...
search_for_video:
  ...
  const char *pMatchingFile =
      g_pFullFileSystem->FindFirstEx(SearchFileSpec, pPathID, &searchHandle);
          ...
          if (BITFLAGS_SET(m_ExtInfo[i].m_VideoFeatures, requiredFeature)) {
            ...
            if (sysIdx != SYSTEM_NOT_FOUND) {
              ...
              if (V_IsAbsolutePath(ActualFilePath)) {
                V_strncpy(pResolvedFileName, ActualFilePath,
                          resolvedFileNameMaxLen);
              } else {
                g_pFullFileSystem->RelativePathToFullPath(
                    ActualFilePath, pPathID, pResolvedFileName,
                    resolvedFileNameMaxLen);
              }
...
VideoSystem_t CValveVideoServices::LocateSystemAndFeaturesForFileName(
    const char *pFileName, VideoSystemFeature_t *pFeatures,
...
//  centering the video
// ===========================================================================
bool CVideoCommonServices::CalculateVideoDimensions(
  ...
  AssertExitF(pOutputWidth != nullptr && pOutputHeight != nullptr &&
              pXOffset != nullptr && pYOffset != nullptr);
  AssertExitF(videoWidth >= 16 && videoHeight >= 16 && displayWidth > 64 &&
              displayHeight > 64);

  // extract relevant options
  bool bFillWindow =
      BITFLAGS_SET(playbackFlags, VideoPlaybackFlags::FILL_WINDOW);
  bool bLockAspect =
      BITFLAGS_SET(playbackFlags, VideoPlaybackFlags::LOCK_ASPECT_RATIO);
  bool bIntegralScale =
      BITFLAGS_SET(playbackFlags, VideoPlaybackFlags::INTEGRAL_SCALE);
  bool bCenterVideo =
      BITFLAGS_SET(playbackFlags, VideoPlaybackFlags::CENTER_VIDEO_IN_WINDOW);
  ...
  if (!bFillWindow) {
    ...
    {
      // if we aren't locking the aspect ratio, just shrink each axis until it
      // fits
      if (!bLockAspect) {
...
// ===========================================================================
VideoResult_t CVideoCommonServices::InitFullScreenPlaybackInputHandler(
  ...
  s_pfnGetAsyncKeyState =
      (GetAsyncKeyStateFn_t)GetProcAddress(m_UserDLLhInst, "GetAsyncKeyState");
  ...
  m_bScanAll =
      ANY_BITFLAGS_SET(playbackFlags, VideoPlaybackFlags::PAUSE_ON_ANY_KEY |
                                          VideoPlaybackFlags::ABORT_ON_ANY_KEY);

  m_bScanEsc =
      m_bScanAll ||
      ANY_BITFLAGS_SET(playbackFlags, VideoPlaybackFlags::PAUSE_ON_ESC |
                                          VideoPlaybackFlags::ABORT_ON_ESC);
  m_bScanReturn =
      m_bScanAll ||
      ANY_BITFLAGS_SET(playbackFlags, VideoPlaybackFlags::PAUSE_ON_RETURN |
                                          VideoPlaybackFlags::ABORT_ON_RETURN);
  m_bScanSpace =
      m_bScanAll ||
      ANY_BITFLAGS_SET(playbackFlags, VideoPlaybackFlags::PAUSE_ON_SPACE |
                                          VideoPlaybackFlags::ABORT_ON_SPACE);

  m_bPauseEnabled =
      ANY_BITFLAGS_SET(playbackFlags, VideoPlaybackFlags::PAUSE_ON_ESC |
                                          VideoPlaybackFlags::PAUSE_ON_RETURN |
                                          VideoPlaybackFlags::PAUSE_ON_SPACE |
                                          VideoPlaybackFlags::PAUSE_ON_ANY_KEY);
  m_bAbortEnabled =
      ANY_BITFLAGS_SET(playbackFlags, VideoPlaybackFlags::ABORT_ON_ESC |
                                          VideoPlaybackFlags::ABORT_ON_RETURN |
                                          VideoPlaybackFlags::ABORT_ON_SPACE |
                                          VideoPlaybackFlags::ABORT_ON_ANY_KEY);
  ...
  m_bForceMinPlayTime =
      BITFLAGS_SET(playbackFlags, VideoPlaybackFlags::FORCE_MIN_PLAY_TIME) &&
      (forcedMinTime > 0.0f);
...
// ===========================================================================
bool CVideoCommonServices::ProcessFullScreenInput(bool &bAbortEvent,
                                                  bool &bPauseEvent,
                                                  bool &bQuitEvent) {
  ...
  bool bEscPressed =
      (m_bScanEsc) ? (s_pfnGetAsyncKeyState(VK_ESCAPE) & 0x8000) != 0 : false;
  bool bReturnPressed = (m_bScanReturn)
                            ? (s_pfnGetAsyncKeyState(VK_RETURN) & 0x8000) != 0
                            : false;
  bool bSpacePressed =
      (m_bScanSpace) ? (s_pfnGetAsyncKeyState(VK_SPACE) & 0x8000) != 0 : false;
#elif defined(OSX)
  g_pLauncherMgr->PumpWindowsMessageLoop();
  // Escape, return, or space stops or pauses the playback
  bool bEscPressed =
      (m_bScanEsc) ? CGEventSourceKeyState(
                         kCGEventSourceStateCombinedSessionState, kVK_Escape)
                   : false;
  bool bReturnPressed =
      (m_bScanReturn) ? CGEventSourceKeyState(
                            kCGEventSourceStateCombinedSessionState, kVK_Return)
                      : false;
  bool bSpacePressed =
      (m_bScanSpace) ? CGEventSourceKeyState(
                           kCGEventSourceStateCombinedSessionState, kVK_Space)
                     : false;
  ...
  g_pLauncherMgr->PeekAndRemoveKeyboardEvents(&bEscPressed, &bReturnPressed,
                                              &bSpacePressed);
#endif

  // Manual debounce of the keys, only interested in unpressed->pressed
  // transitions
  ...
  if (m_bForceMinPlayTime == false && bAnyKeyEvent) {
    ...
    if (m_bAbortEnabled) {
      bAbortEvent =
          (bAnyKeyEvent &&
           BITFLAGS_SET(m_playbackFlags,
                        VideoPlaybackFlags::ABORT_ON_ANY_KEY)) ||
          (bEscEvent &&
           BITFLAGS_SET(m_playbackFlags, VideoPlaybackFlags::ABORT_ON_ESC)) ||
          (bReturnEvent && BITFLAGS_SET(m_playbackFlags,
                                        VideoPlaybackFlags::ABORT_ON_RETURN)) ||
          (bSpaceEvent &&
           BITFLAGS_SET(m_playbackFlags, VideoPlaybackFlags::ABORT_ON_SPACE));
    }
      ...
      bPauseEvent =
          (bAnyKeyEvent &&
           BITFLAGS_SET(m_playbackFlags,
                        VideoPlaybackFlags::PAUSE_ON_ANY_KEY)) ||
          (bEscEvent &&
           BITFLAGS_SET(m_playbackFlags, VideoPlaybackFlags::PAUSE_ON_ESC)) ||
          (bReturnEvent && BITFLAGS_SET(m_playbackFlags,
                                        VideoPlaybackFlags::PAUSE_ON_RETURN)) ||
          (bSpaceEvent &&
           BITFLAGS_SET(m_playbackFlags, VideoPlaybackFlags::PAUSE_ON_SPACE));
    }
  }
