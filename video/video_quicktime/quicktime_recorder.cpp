...
//-----------------------------------------------------------------------------
enum PixelComponent_t { RED = 0, GREEN, BLUE, ALPHA };

int GetBytesPerPixel(OSType pixelFormat) {
  int bpp =
      (pixelFormat == k24BGRPixelFormat || pixelFormat == k24RGBPixelFormat) ? 3
      : (pixelFormat == k32BGRAPixelFormat || pixelFormat == k32RGBAPixelFormat)
          ? 4
          : 0;
...
int GetPixelCompnentByteOffset(OSType format, PixelComponent_t component) {
  ...
  if (component == ALPHA) {
    return (format == k32BGRAPixelFormat || format == k32RGBAPixelFormat) ? 3
                                                                          : -1;
  }
...
bool CopyBitMapPixels(int width, int height, OSType srcFmt, byte *srcBase,
                      int srcStride, OSType dstFmt, byte *dstBase,
                      int dstStride) {
  AssertExitF(width > 0 && height > 0 && srcBase != nullptr && srcStride > 0 &&
              dstBase != nullptr && dstStride > 0);
...
void SaveToTargaFile(int frameNum, const char *pBaseFileName, int width,
                     int height, void *pPixels, OSType PixelFormat,
                     int strideAdjust) {
  ...
  theHeader.descriptor = (BytesPerPixel == 4)
                             ? (8 | 32)
                             : 32; // Targa32, Upper Left Origin, attribute
                                   // (alpha) bits in bits 0-3
  ...
  if (PixelFormat == k24BGRPixelFormat || PixelFormat == k32BGRAPixelFormat) {
    if (strideAdjust == 0) {
      g_pFullFileSystem->Write(pPixels, width * height * BytesPerPixel,
                               TGAFile);
    } else {
  ...
  } else // we need to convert the bits from RGB to BGR
  {
    ...
    CopyBitMapPixels(width, height, PixelFormat, (byte *)pPixels,
                     width * BytesPerPixel + strideAdjust, tgaFormat, pData,
                     width * BytesPerPixel);
...
static EstVideoEncodeQuality_t s_QualityPresets[] = {
    cVEQuality_Min, cVEQuality_Low, cVEQuality_Normal, cVEQuality_High,
    cVEQuality_Max};

static VideoRes_t s_ResolutionPresets[] = {
    {16, 16},    {720, 480},   {640, 960},   {960, 640},
...
CQuickTimeVideoRecorder::CQuickTimeVideoRecorder()
    : m_pEncoder(nullptr), m_LastResult(VideoResult::SUCCESS),
      m_bHasAudio(false), m_bMovieFinished(false) {}
...
bool CQuickTimeVideoRecorder::CreateNewMovieFile(const char *pFilename,
                                                 bool hasAudio) {
  ...
  if (!m_pEncoder->CreateNewMovie(pFilename, hasAudio)) {
    SetResult(m_pEncoder->GetResult()); // save the error result for after the
                                        // encoder goes poof
    SAFE_DELETE(m_pEncoder);
    return false;
  }
...
bool CQuickTimeVideoRecorder::SetMovieVideoParameters(
    VideoEncodeCodec_t theCodec, int videoQuality, int movieFrameWidth,
    int movieFrameHeight, VideoFrameRate_t movieFPS, VideoEncodeGamma_t gamma) {
  SetResult(VideoResult::BAD_INPUT_PARAMETERS);
  AssertExitF(IS_IN_RANGECOUNT(theCodec, VideoEncodeCodec::DEFAULT_CODEC,
                               VideoEncodeCodec::CODEC_COUNT));
  AssertExitF(IS_IN_RANGE(videoQuality, VideoEncodeQuality::MIN_QUALITY,
                          VideoEncodeQuality::MAX_QUALITY));
  AssertExitF(
      IS_IN_RANGE(movieFrameWidth, cMinVideoFrameWidth, cMaxVideoFrameWidth) &&
      IS_IN_RANGE(movieFrameHeight, cMinVideoFrameHeight,
                  cMaxVideoFrameHeight));
  AssertExitF(IS_IN_RANGE(movieFPS.GetFPS(), cMinFPS, cMaxFPS));
  AssertExitF(IS_IN_RANGECOUNT(gamma, VideoEncodeGamma::NO_GAMMA_ADJUST,
                               VideoEncodeGamma::GAMMA_COUNT));

  SetResult(VideoResult::OPERATION_OUT_OF_SEQUENCE);
  AssertExitF(m_pEncoder != nullptr && !m_bMovieFinished);

  return m_pEncoder->SetMovieVideoParameters(movieFrameWidth, movieFrameHeight,
                                             movieFPS, theCodec, videoQuality,
                                             gamma);
}

bool CQuickTimeVideoRecorder::SetMovieSourceImageParameters(
    VideoEncodeSourceFormat_t srcImageFormat, int imgWidth, int imgHeight) {
  SetResult(VideoResult::BAD_INPUT_PARAMETERS);
  AssertExitF(IS_IN_RANGECOUNT(srcImageFormat,
                               VideoEncodeSourceFormat::VIDEO_FORMAT_FIRST,
                               VideoEncodeSourceFormat::VIDEO_FORMAT_COUNT));
  AssertExitF(
      IS_IN_RANGE(imgWidth, cMinVideoFrameWidth, cMaxVideoFrameWidth) &&
      IS_IN_RANGE(imgHeight, cMinVideoFrameHeight, cMaxVideoFrameHeight));

  SetResult(VideoResult::OPERATION_OUT_OF_SEQUENCE);
  AssertExitF(m_pEncoder != nullptr && !m_bMovieFinished);

  return m_pEncoder->SetMovieSourceImageParameters(imgWidth, imgHeight,
                                                   srcImageFormat);
}

bool CQuickTimeVideoRecorder::SetMovieSourceAudioParameters(
  ...
  SetResult(VideoResult::BAD_INPUT_PARAMETERS);
  AssertExitF(IS_IN_RANGECOUNT(srcAudioFormat,
                               AudioEncodeSourceFormat::AUDIO_NONE,
                               AudioEncodeSourceFormat::AUDIO_FORMAT_COUNT));
  AssertExitF(audioSampleRate == 0 ||
              IS_IN_RANGE(audioSampleRate, cMinSampleRate, cMaxSampleRate));

  SetResult(VideoResult::OPERATION_OUT_OF_SEQUENCE);
  AssertExitF(m_pEncoder != nullptr && !m_bMovieFinished);

  bool result = m_pEncoder->SetMovieSourceAudioParameters(
      srcAudioFormat, audioSampleRate, audioOptions, audioSampleGroupSize);

  m_bHasAudio = m_pEncoder->HasAudio(); // Audio can be turned off after
                                        // specifying, so reload status
  return result;
}

bool CQuickTimeVideoRecorder::IsReadyToRecord() {
  return (m_pEncoder == nullptr || m_bMovieFinished)
             ? false
             : m_pEncoder->IsReadyToRecord();
}
...
bool CQuickTimeVideoRecorder::AppendVideoFrame(void *pFrameBuffer,
                                               int nStrideAdjustBytes) {
  SetResult(VideoResult::BAD_INPUT_PARAMETERS);
...
bool CQuickTimeVideoRecorder::AppendAudioSamples(void *pSampleBuffer,
                                                 size_t sampleSize) {
  SetResult(VideoResult::ILLEGAL_OPERATION);
  AssertExitF(m_bHasAudio);
...
bool CQuickTimeVideoRecorder::EstimateMovieFileSize(
    size_t *pEstSize, int movieWidth, int movieHeight,
    VideoFrameRate_t movieFps, float movieDuration, VideoEncodeCodec_t theCodec,
    int videoQuality, AudioEncodeSourceFormat_t srcAudioFormat,
    int audioSampleRate) {
  SetResult(VideoResult::BAD_INPUT_PARAMETERS);
  ...
  *pEstSize = 0;
  ...
  AssertExitF(
      IS_IN_RANGE(movieWidth, cMinVideoFrameWidth, cMaxVideoFrameWidth) &&
  ...
  int Q1 = VideoEncodeQuality::MIN_QUALITY,
      Q2 = VideoEncodeQuality::MAX_QUALITY;
  float Qlerp = 0.0f;
  bool bQLerp = true;
  ...
  VideoRes_t RES1 = {cMinVideoFrameWidth, cMinVideoFrameHeight},
             RES2 = {cMaxVideoFrameWidth, cMaxVideoFrameHeight};
  float RLerp = 0.0f;
  bool bRLerp = true;
  ...
  for (int i = 0; i < ARRAYSIZE(s_ResolutionPresets); i++) {
    if (s_ResolutionPresets[i].X == movieWidth &&
        s_ResolutionPresets[i].Y == movieHeight) {
...
float CQuickTimeVideoRecorder::GetDataRate(int quality, int width, int height) {
  for (int i = 0; i < ARRAYSIZE(s_H264EncodeRates); i++) {
    if (s_H264EncodeRates[i].m_QualitySetting == quality &&
        s_H264EncodeRates[i].m_XResolution == width &&
...
CQTVideoFileComposer::CQTVideoFileComposer()
    : m_LastResult(VideoResult::SUCCESS),
      ...
      m_bMovieCreated(false), m_bHasAudioTrack(false),

      m_bMovieConfigured(false), m_bSourceImagesConfigured(false),
      m_bSourceAudioConfigured(false),

      m_bComposingMovie(false), m_bMovieCompleted(false),

      m_nFramesAdded(0), m_nAudioFramesAdded(0), m_nSamplesAdded(0),
      m_nSamplesAddedToMedia(0),

      m_MovieFrameWidth(0), m_MovieFrameHeight(0),

      m_MovieTimeScale(0), m_DurationPerFrame(0),

      m_AudioOptions(AudioEncodeOptions::NO_AUDIO_OPTIONS),
      m_SampleGrouping(AG_NONE), m_nAudioSampleGroupSize(0),

      m_AudioSourceFrequency(0), m_AudioBytesPerSample(0),

      m_bBufferSourceAudio(false), m_bLimitAudioDurationToVideo(false),

      m_srcAudioBuffer(nullptr), m_srcAudioBufferSize(0),
      ...
      m_SrcImageWidth(0), m_SrcImageHeight(0), m_ScrImageMaxCompressedSize(0),
      m_SrcImageBuffer(nullptr), m_SrcImageCompressedBuffer(nullptr),
      m_SrcPixelFormat(0), m_SrcBytesPerPixel(0),

      m_GWorldPixelFormat(0), m_GWorldBytesPerPixel(0), m_GWorldImageWidth(0),
      ...
      m_MovieFileDataRef(nullptr), m_MovieFileDataRefType(0),
      m_MovieFileDataHandler(nullptr),

      m_theMovie(nullptr), m_theVideoTrack(nullptr), m_theAudioTrack(nullptr),
      m_theVideoMedia(nullptr), m_theAudioMedia(nullptr)
...
#endif
{
  ...
  m_GWorldRect.top = m_GWorldRect.left = m_GWorldRect.bottom =
      m_GWorldRect.right = 0;
...
bool CQTVideoFileComposer::CreateNewMovie(const char *fileName, bool hasAudio) {
  ...
  status = QTNewDataReferenceFromFullPathCFString(
      imageStrRef, (QTPathStyle)kQTNativeDefaultPathStyle, 0,
      &m_MovieFileDataRef, &m_MovieFileDataRefType);
  AssertExitF(status == noErr);

  status = CreateMovieStorage(
      m_MovieFileDataRef, m_MovieFileDataRefType, movieType, smCurrentScript,
...
bool CQTVideoFileComposer::SetMovieVideoParameters(
    int width, int height, VideoFrameRate_t movieFPS,
  ...
  AssertExitF(IS_IN_RANGE(movieFPS.GetFPS(), cMinFPS, cMaxFPS));
  AssertExitF(IS_IN_RANGECOUNT(desiredCodec, VideoEncodeCodec::DEFAULT_CODEC,
                               VideoEncodeCodec::CODEC_COUNT));
  AssertExitF(IS_IN_RANGE(encodeQuality, VideoEncodeQuality::MIN_QUALITY,
                          VideoEncodeQuality::MAX_QUALITY));
  AssertExitF(IS_IN_RANGECOUNT(gamma, VideoEncodeGamma::NO_GAMMA_ADJUST,
                               VideoEncodeGamma::GAMMA_COUNT));
  ...
  int MaxQ = (int)VideoEncodeQuality::MAX_QUALITY -
             (int)VideoEncodeQuality::MIN_QUALITY;
  ...
  /*	if ( movieFPS.IsNTSCRate() )
          ...
          else if ( movieFPS.GetUnitsPerSecond() % movieFPS.GetUnitsPerFrame()
  ...
  */
  ...
  LogMsg("Video Frame Rate = %f FPS\n   %d time units per second\n   %d time "
         "units per frame\n",
         m_MovieRecordFPS.GetFPS(), m_MovieRecordFPS.GetUnitsPerSecond(),
         m_MovieRecordFPS.GetUnitsPerFrame());
  ...
  m_theVideoTrack = NewMovieTrack(m_theMovie, FixRatio(width, 1),
                                  FixRatio(height, 1), kNoVolume);
  AssertExitF(GetMoviesError() == noErr);

  m_theVideoMedia =
      NewTrackMedia(m_theVideoTrack, VideoMediaType, m_MovieTimeScale, NULL, 0);
  AssertExitF(GetMoviesError() == noErr);
...
bool CQTVideoFileComposer::SetMovieSourceImageParameters(
    int srcWidth, int srcHeight, VideoEncodeSourceFormat_t srcImageFormat) {
  // Validate input and state
  SetResult(VideoResult::BAD_INPUT_PARAMETERS);
  AssertExitF(
      IS_IN_RANGE(srcWidth, cMinVideoFrameWidth, cMaxVideoFrameWidth) &&
  ...
  SetResult(VideoResult::OPERATION_OUT_OF_SEQUENCE);
  AssertExitF(m_bMovieCreated && !m_bMovieCompleted && m_bMovieConfigured &&
              !m_bSourceImagesConfigured);

  // Setup source image format related stuff
  m_SrcPixelFormat = (srcImageFormat == VideoEncodeSourceFormat::BGRA_32BIT)
                         ? k32BGRAPixelFormat
                     : (srcImageFormat == VideoEncodeSourceFormat::BGR_24BIT)
                         ? k24BGRPixelFormat
                     : (srcImageFormat == VideoEncodeSourceFormat::RGB_24BIT)
                         ? k24RGBPixelFormat
                     : (srcImageFormat == VideoEncodeSourceFormat::RGBA_32BIT)
                         ? k32RGBAPixelFormat
                         : 0;
  ...
  m_GWorldPixelFormat =
      k32BGRAPixelFormat; // can use k24BGRPixelFormat on Win32.. but it
                          // compresses wrong on OSX?
  m_GWorldBytesPerPixel = 4;
  m_GWorldImageWidth = CONTAINING_MULTIPLE_OF(
      srcWidth,
      4); // make sure the encoded surface is a multiple of 4 in each dimensions
  ...
  OSErr status = QTNewGWorld(&m_theSrcGWorld, m_GWorldPixelFormat,
                             &m_GWorldRect, nil, nil, 0);
  AssertExitF(status == noErr);
  ...
  status = GetMaxCompressionSize(
      thePixMap, &m_GWorldRect, 0, m_EncodeQuality, m_VideoCodecToUse,
...
bool CQTVideoFileComposer::SetMovieSourceAudioParameters(
    AudioEncodeSourceFormat_t srcAudioFormat, int audioSampleRate,
    AudioEncodeOptions_t audioOptions, int audioSampleGroupSize) {
  SetResult(VideoResult::ILLEGAL_OPERATION);
  ...
  SetResult(VideoResult::OPERATION_OUT_OF_SEQUENCE);
  AssertExitF(m_bMovieCreated && !m_bMovieCompleted && m_bMovieConfigured &&
              !m_bSourceAudioConfigured);

  // it is possible to disable audio here by passing in
  // AudioEncodeSourceFormat::AUDIO_NONE even if the movie was created with the
  // hasHadio flag set to true, or by setting the sample rate to 0

  if (srcAudioFormat == AudioEncodeSourceFormat::AUDIO_NONE ||
      audioSampleRate == 0) {
    ...
    m_theAudioMedia = NewTrackMedia(m_theAudioTrack, SoundMediaType,
                                    (TimeScale)audioSampleRate, NULL, 0);
    AssertExitF(GetMoviesError() == noErr);
    ...
    OSStatus result = QTSoundDescriptionCreate(
        &inASBD, NULL, 0, NULL, 0,
    ...
    m_bLimitAudioDurationToVideo = BITFLAGS_SET(
        audioOptions, AudioEncodeOptions::LIMIT_AUDIO_TRACK_TO_VIDEO_DURATION);

    m_SampleGrouping =
        (BITFLAGS_SET(audioOptions,
                      AudioEncodeOptions::GROUP_SIZE_IS_VIDEO_FRAME))
            ? CQTVideoFileComposer::AG_PER_FRAME
        : (BITFLAGS_SET(audioOptions,
                        AudioEncodeOptions::USE_AUDIO_ENCODE_GROUP_SIZE))
    ...
    if (m_SampleGrouping == AG_FIXED_SIZE &&
        (audioSampleGroupSize < MIN_AUDIO_SAMPLE_GROUP_SIZE ||
         audioSampleGroupSize >
             MAX_AUDIO_GROUP_SIZE_IN_SEC * m_AudioSourceFrequency)) {
      SetResult(VideoResult::BAD_INPUT_PARAMETERS);
      Assert(false);
      return false;
    }
    ...
    m_bBufferSourceAudio =
        (m_SampleGrouping != AG_NONE) || m_bLimitAudioDurationToVideo;

    // Set up an audio buffer than can hold the maxium specified duration
    if (m_bBufferSourceAudio) {
      m_srcAudioBufferSize = m_AudioSourceFrequency * m_AudioBytesPerSample *
                             MAX_AUDIO_GROUP_SIZE_IN_SEC;
...
// This ONLY returns false if it tried to begin the movie creation process and
// failed
bool CQTVideoFileComposer::CheckForReadyness() {
  return (m_bMovieCreated && !m_bMovieCompleted && !m_bComposingMovie &&
          m_bMovieConfigured && m_bSourceImagesConfigured &&
          m_bSourceAudioConfigured == m_bHasAudioTrack)
             ? BeginMovieCreation()
             : true;
}
...
bool CQTVideoFileComposer::AppendVideoFrameToMedia(void *ImageBuffer,
                                                   int strideAdjustBytes) {
#ifdef LOG_ENCODER_OPERATIONS
  LogMsg("AppendVideoFrameToMedia( %8.8x ) called for %d --- ", ImageBuffer,
         m_nFramesAdded + 1);
#endif
  ...
  AssertExitF(bmp.bmWidth == m_SrcImageWidth &&
              bmp.bmHeight == m_SrcImageHeight &&
  ...
  if ((m_nFramesAdded % LOG_FRAMES_TO_TGA_INTERVAL) == 0) {
    SaveToTargaFile(m_nFramesAdded, m_TGAFileBase, m_SrcImageWidth,
                    m_SrcImageHeight, srcBase, m_SrcPixelFormat,
                    strideAdjustBytes);
  }
  ...
  if (!CopyBitMapPixels(m_SrcImageWidth, m_SrcImageHeight, m_SrcPixelFormat,
                        srcBase, srcStride, m_GWorldPixelFormat, dstBase,
                        dstStride)) {
    Assert(false);
    return false;
  }
  ...
  ImageDescriptionHandle theImageDescHandle =
  ...
  OSErr status = CompressImage(thePixMap, &m_GWorldRect, m_EncodeQuality,
                               m_VideoCodecToUse, theImageDescHandle,
  ...
  if (m_EncodeGamma != kQTUseSourceGammaLevel) {
    ...
    status = ICMImageDescriptionSetProperty(
        theImageDescHandle, kQTPropertyClass_ImageDescription,
        kICMImageDescriptionPropertyID_GammaLevel, sizeof(newGamma), &newGamma);
    AssertExitF(status == noErr);
  }

  // add the compressed image to the movie stream
  status = AddMediaSample(m_theVideoMedia, m_SrcImageCompressedBuffer, 0,
                          (**theImageDescHandle).dataSize, m_DurationPerFrame,
                          (SampleDescriptionHandle)theImageDescHandle, 1, 0,
                          &addedTime);
...
#ifdef LOG_ENCODER_OPERATIONS
  LogMsg("Video Frame %d added to Video Media: Duration = %d, Inserted at Time "
         "%d\n",
...
int CQTVideoFileComposer::GetAudioSampleCountThruFrame(int frameNo) {
  ...
  double secondsSoFar =
      (double)(frameNo * m_DurationPerFrame) / (double)m_MovieTimeScale;
  int nAudioSamples = (int)floor(secondsSoFar * (double)m_AudioSourceFrequency);

  return nAudioSamples;
}

bool CQTVideoFileComposer::AppendAudioSamplesToMedia(void *soundBuffer,
                                                     size_t bufferSize) {
  SetResult(VideoResult::ILLEGAL_OPERATION);
  AssertExitF(m_bHasAudioTrack);
  ...
  // Validate input and state
  SetResult(VideoResult::BAD_INPUT_PARAMETERS);
  AssertExitF(soundBuffer != nullptr &&
              bufferSize % m_AudioBytesPerSample == 0);
...
#ifdef LOG_ENCODER_AUDIO_OPERATIONS
  LogMsg("%d Audio Samples Submitted (%d total) -- ", nSamples,
         m_nSamplesAdded);
#endif
  ...
  if (!m_bBufferSourceAudio && nSamples > 0) {
    SetResult(VideoResult::AUDIO_ERROR_OCCURED);
    status =
        AddMediaSample2(m_theAudioMedia, (const UInt8 *)soundBuffer,
                        (ByteCount)bufferSize, (TimeValue64)1, (TimeValue64)0,
                        (SampleDescriptionHandle)m_srcSoundDescription,
    ...
    LogMsg("%d samples (%d total) inserted into Video at time %ld\n", nSamples,
           m_nSamplesAddedToMedia, insertTime64);
#endif
...
retest_here:
  ...
  samplesToEmit = 0;
  retest = false;
  MaxCanAdd = (m_bLimitAudioDurationToVideo)
                  ? (GetAudioSampleCountThruFrame(m_nFramesAdded) -
  ...
  // Now.. we determine if we are ready to insert the audio samples into the
  // media..and if so, how much...
  ...
  if (m_SampleGrouping == AG_NONE) {
    ...
    if ((nSamplesAvailable < m_nAudioSampleGroupSize) ||
        (m_bLimitAudioDurationToVideo &&
         (MaxCanAdd < m_nAudioSampleGroupSize))) {
#ifdef LOG_ENCODER_AUDIO_OPERATIONS
      if (nSamplesAvailable < m_nAudioSampleGroupSize)
    ...
    if (m_bLimitAudioDurationToVideo &&
        m_AudioSampleFrameCounter >= m_nFramesAdded) {
#ifdef LOG_ENCODER_AUDIO_OPERATIONS
    ...
    int nextSampleCount =
        GetAudioSampleCountThruFrame(m_AudioSampleFrameCounter + 1);
  ...
  if (samplesToEmit > 0) {
    ...
    status = AddMediaSample2(m_theAudioMedia, (const UInt8 *)m_srcAudioBuffer,
                             (ByteCount)samplesToEmit * m_AudioBytesPerSample,
                             (TimeValue64)1, (TimeValue64)0,
                             (SampleDescriptionHandle)m_srcSoundDescription,
...
#ifdef LOG_ENCODER_AUDIO_OPERATIONS
    LogMsg("%d samples inserted into Video (%d total) at time %ld  -- ",
           samplesToEmit, m_nSamplesAddedToMedia, insertTime64);
#endif
    ...
    if (nSamplesAvailable > 0) {
      V_memcpy(m_srcAudioBuffer,
               m_srcAudioBuffer + (samplesToEmit * m_AudioBytesPerSample),
...
restart_sync:
  bool bPadWithSilence =
      BITFLAGS_SET(m_AudioOptions, AudioEncodeOptions::PAD_AUDIO_WITH_SILENCE);
  ...
  int nSamplesInBuffer = (m_bBufferSourceAudio) ? m_srcAudioBufferCurrentSize /
  ...
  LogMsg("%d Samples emitted to Audio track so far.  %d samples remain in "
         "audio buffer\n",
         m_nSamplesAddedToMedia, nSamplesInBuffer);
  LogMsg("Delta to sync end of audio to end of video is %d Samples\n",
         CurShortfall);
  LogMsg(
      "Pad With Silence Mode = %d, Align End of Audio With Video Mode = %d\n",
  ...
  if (m_SampleGrouping == AG_NONE) {
    ...
    Assert(m_bLimitAudioDurationToVideo ==
           m_bBufferSourceAudio); // if we're not limiting, we're not buffering
    ...
    if (nSamplesInBuffer > 0 ||
        SilenceToEmit > 0) // force if we have something to add
    {
      forceFlush = true;
    }
  }
  ...
  if (m_SampleGrouping == AG_FIXED_SIZE) {
    ...
    // No matter what, if we have a partially filled buffer, we add silence to
    // it to make a complete group do we have a partially full buffer? if so pad
    // with silence to make a full group
    if (nSamplesInBuffer > 0 &&
        nSamplesInBuffer % m_nAudioSampleGroupSize != 0) {
      SilenceToEmit = m_nAudioSampleGroupSize -
                      (nSamplesInBuffer % m_nAudioSampleGroupSize);
#ifdef LOG_ENCODER_AUDIO_OPERATIONS
    ...
    int newShortFall =
        VideoDurationInSamples - m_nSamplesAddedToMedia - bufferedSamples;

    if (bPadWithSilence && newShortFall > 0) {
      SilenceToEmit +=
          newShortFall; // pad with silence until audio matches video duration
      forceFlush = true;
      forcePartialGroupFlush = true;

#ifdef LOG_ENCODER_AUDIO_OPERATIONS
      LogMsg(
          "Adding %d silence samples to pad to match audio to video duration\n",
          newShortFall);
#endif
    }
  }
  ...
  if (m_SampleGrouping == AG_PER_FRAME) {
    ...
    if (nSamplesInBuffer > 0) {
      ...
      int thisGroupSize =
          GetAudioSampleCountThruFrame(m_AudioSampleFrameCounter + 1) -
          GetAudioSampleCountThruFrame(m_AudioSampleFrameCounter);
      Assert(m_nSamplesAddedToMedia ==
             GetAudioSampleCountThruFrame(m_AudioSampleFrameCounter));

      // if we already have 1 (or more) groups in the buffer.. emit them, and
      // restart
...
#ifdef LOG_ENCODER_AUDIO_OPERATIONS
      LogMsg("Adding %d silence samples to pad current group to match video "
             "frame duration\n",
             SilenceToEmit);
#endif
    }

    // with the output being aligned to a video frame, do we need to add more to
    // pad to end of the video
...
#ifdef LOG_ENCODER_AUDIO_OPERATIONS
      LogMsg("Adding %d silence samples to pad audio to match video duration",
             newShortFall);
#endif
    }
  }
  ...
  if (forcePartialGroupFlush && m_srcAudioBufferCurrentSize > 0) {
    ...
    OSErr status = AddMediaSample2(
        m_theAudioMedia, (const UInt8 *)m_srcAudioBuffer,
        (ByteCount)m_srcAudioBufferCurrentSize, (TimeValue64)1, (TimeValue64)0,
        (SampleDescriptionHandle)m_srcSoundDescription,
...
#ifdef LOG_ENCODER_OPERATIONS
    LogMsg("FORCED FLUSH - Audio Samples added to media.  %d added, %d total "
           "samples, inserted at time %ld\n",
...
bool CQTVideoFileComposer::EndMovieCreation(bool saveMovieData) {
  ...
  if (m_nFramesAdded > 0) {
    ...
    if (saveMovieData) {
      status =
          InsertMediaIntoTrack(m_theVideoTrack, 0, 0, VideoDuration, fixed1);
      Assert(status == noErr);
  ...
  if (m_bHasAudioTrack && m_nSamplesAdded > 0) {
    ...
    if (saveMovieData) {
      status =
          InsertMediaIntoTrack(m_theAudioTrack, 0, 0, AudioDuration, fixed1);
      AssertExitF(status == noErr);
...
bool CQTVideoFileComposer::FinishMovie(bool SaveMovieToDisk) {
  ...
  if (SaveMovieToDisk == false ||
      (m_nFramesAdded <= 0 && m_nSamplesAdded <= 0)) {
    g_pFullFileSystem->RemoveFile(m_FileName);
  }
