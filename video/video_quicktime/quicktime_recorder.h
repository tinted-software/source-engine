...
#if defined(LOG_ENCODER_OPERATIONS) ||                                         \
    defined(LOG_ENCODER_AUDIO_OPERATIONS) || defined(LOG_FRAMES_TO_TGA) ||     \
...
public:
  ...
  bool SetMovieVideoParameters(int width, int height, VideoFrameRate_t movieFPS,
                               VideoEncodeCodec_t desiredCodec,
                               int encodeQuality, VideoEncodeGamma_t gamma);
  bool SetMovieSourceImageParameters(int srcWidth, int srcHeight,
                                     VideoEncodeSourceFormat_t srcImageFormat);
  bool SetMovieSourceAudioParameters(
...
private:
  enum AudioGrouping_t { AG_NONE = 0, AG_FIXED_SIZE, AG_PER_FRAME };
  ...
  //		short				m_ResRefNum;
  //// QuickTime Movie Resource Ref number
