...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CQuickTimeVideoSubSystem, IVideoSubSystem,
...
ConVar QuickTime_EncodeGamma(
    "video_quicktime_encode_gamma", "3", FCVAR_ARCHIVE,
    "QuickTime Video Encode Gamma Target- 0=no gamma adjust  1=platform "
    "default gamma  2 = gamma 1.8  3 = gamma 2.2  4 = gamma 2.5",
    true, 0.0f, true, 4.0f);
ConVar QuickTime_PlaybackGamma(
...
CQuickTimeVideoSubSystem::CQuickTimeVideoSubSystem()
    : m_bQuickTimeInitialized(false), m_LastResult(VideoResult::SUCCESS),
...
bool CQuickTimeVideoSubSystem::Connect(CreateInterfaceFn factory) {
  ...
  if (g_pFullFileSystem == nullptr || materials == nullptr) {
    Msg("QuickTime video subsystem failed to connect to missing a required "
        "system\n");
    return false;
  }
  return true;
...
void CQuickTimeVideoSubSystem::Disconnect() { BaseClass::Disconnect(); }
...
void *CQuickTimeVideoSubSystem::QueryInterface(const char *pInterfaceName) {
  ...
  if (IS_NOT_EMPTY(pInterfaceName)) {
    if (V_strncmp(pInterfaceName, VIDEO_SUBSYSTEM_INTERFACE_VERSION,
                  Q_strlen(VIDEO_SUBSYSTEM_INTERFACE_VERSION) + 1) ==
        STRINGS_MATCH) {
...
// ===========================================================================
bool CQuickTimeVideoSubSystem::InitializeVideoSystem(
    IVideoCommonServices *pCommonServices) {
  m_AvailableFeatures =
      DEFAULT_FEATURE_SET; // Put here because of issue with static const int,
                           // binary OR and DEBUG builds
  ...
  if (!GetGWorldPixMap)
    GetGWorldPixMap =
        (PFNGetGWorldPixMap)dlsym(RTLD_DEFAULT, "GetGWorldPixMap");
  if (!GetPixBaseAddr)
  ...
  if (!GetGWorldPixMap || !GetPixBaseAddr || !LockPixels || !UnlockPixels ||
      !DisposeGWorld || !SetGWorld || !GetPixRowBytes)
    return false;
#endif
...
VideoResult_t CQuickTimeVideoSubSystem::VideoSoundDeviceCMD(
    VideoSoundDeviceOperation_t operation, void *pDevice, void *pData) {
  switch (operation) {
...
const char *CQuickTimeVideoSubSystem::GetSupportedFileExtension(int num) {
  return (num < 0 || num >= s_QuickTimeExtensionCount)
             ? nullptr
             : s_QuickTimeExtensions[num].m_FileExtension;
}

VideoSystemFeature_t
CQuickTimeVideoSubSystem::GetSupportedFileExtensionFeatures(int num) {
  return (num < 0 || num >= s_QuickTimeExtensionCount)
...
// ===========================================================================
VideoResult_t CQuickTimeVideoSubSystem::PlayVideoFileFullScreen(
  ...
  AssertExitV(m_CurrentStatus == VideoSystemStatus::OK,
  ...
  AssertExitV(imageStrRef != nullptr,
              SetResult(VideoResult::SYSTEM_ERROR_OCCURED));

  status = QTNewDataReferenceFromFullPathCFString(
      imageStrRef, (QTPathStyle)kQTNativeDefaultPathStyle, 0, &MovieFileDataRef,
      &MovieFileDataRefType);
  AssertExitV(status == noErr, SetResult(VideoResult::FILE_ERROR_OCCURED));

  CFRelease(imageStrRef);

  status = NewMovieFromDataRef(&theQTMovie, newMovieActive, nil,
  ...
  // on mac OSX, if we are fullscreen, quicktime is bypassing our targets and
  // going to the display directly, so use its dimensions
  ...
  if (!m_pCommonServices->CalculateVideoDimensions(
          nNewWidth, nNewHeight, displayWidth, displayHeight, playbackFlags,
  ...
  bool enableMovieAudio =
      !BITFLAGS_SET(playbackFlags, VideoPlaybackFlags::NO_AUDIO);

  if (!CreateMovieAudioContext(enableMovieAudio, theQTMovie, &theAudioContext,
                               true)) {
#if defined(OSX)
    SetSystemUIMode(oldMode, oldOptions);
#endif
  ...
  SetGWorldDecodeGamma((CGrafPtr)theGrafPtr,
                       VideoPlaybackGamma::USE_GAMMA_CONVAR);
  ...
  if (SetResult(m_pCommonServices->InitFullScreenPlaybackInputHandler(
          playbackFlags, forcedMinTime, windowed)) != VideoResult::SUCCESS) {
#if defined(OSX)
    SetSystemUIMode(oldMode, oldOptions);
#endif
  ...
  while (true) {
    ...
    if (m_pCommonServices->ProcessFullScreenInput(bAbortEvent, bPauseEvent,
                                                  bQuitEvent)) {
    ...
    if (now >= theQTMovieDuration) {
      ...
      if (playbackFlags &
          BITFLAGS_SET(playbackFlags, VideoPlaybackFlags::LOOP_VIDEO)) {
        Assert(ANY_BITFLAGS_SET(
            playbackFlags,
            VideoPlaybackFlags::ABORT_ON_ESC |
                VideoPlaybackFlags::ABORT_ON_RETURN |
                VideoPlaybackFlags::ABORT_ON_SPACE |
                VideoPlaybackFlags::ABORT_ON_ANY_KEY)); // Movie will loop
                                                        // forever otherwise
        StopMovie(theQTMovie);
    ...
    // if the movie is paused, sleep for 5ms to keeps the CPU from spinning so
    // hard
    if (bPaused) {
      ThreadSleep(1);
    } else {
...
// ===========================================================================
IVideoMaterial *
CQuickTimeVideoSubSystem::CreateVideoMaterial(const char *pMaterialName,
...
VideoResult_t
CQuickTimeVideoSubSystem::DestroyVideoMaterial(IVideoMaterial *pVideoMaterial) {
  AssertExitV(m_CurrentStatus == VideoSystemStatus::OK,
              SetResult(VideoResult::SYSTEM_NOT_AVAILABLE));
...
VideoResult_t
CQuickTimeVideoSubSystem::DestroyVideoRecorder(IVideoRecorder *pRecorder) {
  AssertExitV(m_CurrentStatus == VideoSystemStatus::OK,
              SetResult(VideoResult::SYSTEM_NOT_AVAILABLE));
  AssertPtrExitV(pRecorder, SetResult(VideoResult::BAD_INPUT_PARAMETERS));

  if (m_RecorderList.Find(pRecorder) != -1) {
    CQuickTimeVideoRecorder *pVideoRecorder =
        (CQuickTimeVideoRecorder *)pRecorder;
...
VideoResult_t
CQuickTimeVideoSubSystem::CheckCodecAvailability(VideoEncodeCodec_t codec) {
  AssertExitV(m_CurrentStatus == VideoSystemStatus::OK,
              SetResult(VideoResult::SYSTEM_NOT_AVAILABLE));
...
// Status support
// ===========================================================================
VideoResult_t CQuickTimeVideoSubSystem::GetLastResult() { return m_LastResult; }
...
bool CQuickTimeVideoSubSystem::SetupQuickTime() {
  ...
  // This is set early to indicate we have already been through here, even if we
  // error out for some reason
  ...
  if ((status != noErr) || (version < 0x07608000)) {
    ...
    Msg("QuickTime Version reports to be ver= %8.8x, less than 7.6 (07608000) "
        "required\n",
        version);
    Assert(false);
  ...
  // if the Library load didn't hook up the compression functions, remove them
  // from our feature list
#pragma warning(disable : 4551)

  if (!CompressImage) {
    m_AvailableFeatures =
        m_AvailableFeatures & ~(VideoSystemFeature::ENCODE_VIDEO_TO_FILE |
                                VideoSystemFeature::ENCODE_AUDIO_TO_FILE);
  }
...
bool MediaGetStaticFrameRate(Media inMovieMedia, VideoFrameRate_t &theFrameRate,
                             bool AssumeConstantIntervals);
...
bool MovieGetStaticFrameRate(Movie inMovie, VideoFrameRate_t &theFrameRate) {
  ...
  Track videoTrack =
      GetMovieIndTrackType(inMovie, 1, kCharacteristicHasVideoFrameRate,
                           movieTrackCharacteristic |
                               movieTrackEnabledOnly); // get first video track
  ...
  if (MediaHasCharacteristic(movieMediaHandler, kCharacteristicIsAnMpegTrack,
                             &isMPEG) != noErr)
    goto error_out;
  ...
  if (isMPEG) // working with MPEG-1/MPEG-2 media
  {
    ...
    if (MediaGetPublicInfo(movieMediaHandler, kMHInfoEncodedFrameRate,
                           &encodedFrameRate, &encodedFrameRateSize) != noErr)
      goto error_out;
    ...
    theFrameRate.SetRaw(
        MovieTimeScale,
        int((double)MovieTimeScale / Fix2X(encodedFrameRate.encodedFrameRate) +
            0.5));
...
bool MediaGetStaticFrameRate(Media inMovieMedia, VideoFrameRate_t &theFrameRate,
  ...
  if (!AssumeConstantIntervals) {
    ...
    // calculate the frame rate: = (sample count * media time scale) / media
    // duration
  ...
  GetMediaNextInterestingDisplayTime(
      inMovieMedia, nextTimeMediaSample | nextTimeEdgeOK, (TimeValue64)0,
      fixed1, &sample_time, &sample_duration);
  ...
  GetMediaNextInterestingDisplayTime(
      inMovieMedia, nextTimeMediaSample | nextTimeEdgeOK,
...
//  correction
//
//   kQTUsePlatformDefaultGammaLevel = 0,  /* When decompressing into this
//   PixMap, gamma-correct to the platform's standard gamma. */
//   kQTUseSourceGammaLevel        = -1L,  /* When decompressing into this
//   PixMap, don't perform gamma-correction. */ kQTCCIR601VideoGammaLevel     =
//   0x00023333 /* 2.2, standard television video gamma.*/ Fixed cGamma1_8 =
//   0x0001CCCC;		// Gamma 1.8 Fixed cGamma2_5 = 0x00028000; //
//   Gamma 2.5
// ============================================================================

bool SetGWorldDecodeGamma(CGrafPtr theGWorld, VideoPlaybackGamma_t gamma) {
  AssertExitF(theGWorld != nullptr);
  AssertIncRange(gamma, VideoPlaybackGamma::USE_GAMMA_CONVAR,
                 VideoPlaybackGamma::GAMMA_2_5);
  ...
  if (gamma == VideoPlaybackGamma::USE_GAMMA_CONVAR) {
    ...
    if (useGamma < (int)VideoPlaybackGamma::NO_GAMMA_ADJUST ||
        useGamma >= VideoPlaybackGamma::GAMMA_COUNT)
      return false;
...
// ============================================================================
bool CreateMovieAudioContext(bool enableAudio, Movie inMovie,
                             QTAudioContextRef *pAudioContext, bool setVolume,
  ...
  if (enableAudio) {
    ...
    WCHAR strGUID[39];
    int numBytes =
        StringFromGUID2(DSDEVID_DefaultPlayback, (LPOLESTR)strGUID,
                        39); // CLSID_DirectSound is not what you want here

    // create the audio context
    CFStringRef deviceNameStrRef = CFStringCreateWithCharacters(
        kCFAllocatorDefault, (const UniChar *)strGUID, (CFIndex)(numBytes - 1));

    OSStatus result = QTAudioContextCreateForAudioDevice(NULL, deviceNameStrRef,
                                                         NULL, pAudioContext);
    ...
    OSStatus result =
        QTAudioContextCreateForAudioDevice(NULL, NULL, NULL, pAudioContext);
    AssertExitF(result == noErr);
#endif
    // valid?
