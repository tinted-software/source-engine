...
//
//  Purpose - to save typing, make things cleaer, prep for c++0x, prep for
//  64-bit,
...
#define AssertIncRange(_exp, low, high)                                        \
  Assert((_exp) >= (low) && (_exp) <= (high))

// AssertExit macros .. in release builds, or when Assert is disabled, they exit
// (w/ opt return value)
...
#define CONTAINING_MULTIPLE_OF(var, multVal)                                   \
  ((var) + (multVal - 1)) - (((var) - 1) % multVal)
