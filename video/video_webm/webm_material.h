...
public:
  ...
  virtual const char *GetVideoFileName(); // Gets the file name of the video
                                          // this material is playing
  virtual VideoResult_t
  GetLastResult(); // Gets detailed info on the last operation

  virtual VideoFrameRate_t &
  GetVideoFrameRate(); // Returns the frame rate of the associated video in FPS
  ...
  virtual bool
  IsVideoReadyToPlay(); // Queries if the video material was initialized
                        // successfully and is ready for playback, but not
                        // playing or finished
  virtual bool IsVideoPlaying();    // Is the video currently playing (and needs
                                    // update calls, etc)
  ...
  virtual float
  GetVideoDuration(); // Returns the duration of the associated video in seconds
  virtual int
  GetFrameCount(); // Returns the total number of (unique) frames in the video

  virtual bool
  SetFrame(int FrameNum); // Sets the current frame # in the video to play next
  virtual int
  GetCurrentFrame(); // Gets the current frame # for the video playback, 0 Based

  virtual bool SetTime(
      float flTime); // Sets the video playback to specified time (in seconds)
  virtual float
  GetCurrentVideoTime(); // Gets the current time in the video playback

  // Update function
  virtual bool Update(); // Updates the video frame to reflect the time passed,
                         // true = new frame available

  // Material / Texture Info functions
  virtual IMaterial *
  GetMaterial(); // Gets the IMaterial associated with an video material

  virtual void GetVideoTexCoordRange(
      float *pMaxU,
      float *pMaxV); // Returns the max texture coordinate of the video portion
                     // of the material surface ( 0.0, 0.0 to U, V )
  virtual void GetVideoImageSize(
      int *pWidth,
      int *pHeight); // Returns the frame size of the Video Image Frame in
                     // pixels ( the stored in a subrect of the material itself)
...
private:
  ...
  CMaterialReference
      m_Material; // Ref to Material used for rendering the video frame
  CTextureReference m_Texture; // Ref to the renderable texture which contains
                               // the most recent video frame (in a sub-rect)

  float m_TexCordU; // Max U texture coordinate of the texture sub-rect which
                    // holds the video frame
  float m_TexCordV; // Max V texture coordinate of the texture sub-rect which
                    // holds the video frame
