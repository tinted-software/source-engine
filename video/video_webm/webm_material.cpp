...
void CQuicktimeMaterialRGBTextureRegenerator::SetSourceGWorld(
    GWorldPtr theGWorld, int nWidth, int nHeight) {
  m_SrcGWorld = theGWorld;
  m_nSourceWidth = nWidth;
  m_nSourceHeight = nHeight;
}

void CQuicktimeMaterialRGBTextureRegenerator::RegenerateTextureBits(
...
void CQuicktimeMaterialRGBTextureRegenerator::Release() {
  // we don't invoke the destructor here, we're not using the no-release
  // extensions
}
...
CQuickTimeMaterial::CQuickTimeMaterial()
    : m_pFileName(nullptr), m_MovieGWorld(nullptr), m_QTMovie(nullptr),
      m_AudioContext(nullptr), m_bInitCalled(false) {
  Reset();
}
...
void CQuickTimeMaterial::SetQTFileName(const char *theQTMovieFileName) {
  ...
  if (theQTMovieFileName != nullptr) {
    AssertMsg(V_strlen(theQTMovieFileName) <= MAX_QT_FILENAME_LEN,
              "Bad Quicktime Movie Filename");
...
//-----------------------------------------------------------------------------
const char *CQuickTimeMaterial::GetVideoFileName() { return m_pFileName; }

VideoFrameRate_t &CQuickTimeMaterial::GetVideoFrameRate() {
  return m_QTMovieFrameRate;
}

VideoResult_t CQuickTimeMaterial::GetLastResult() { return m_LastResult; }

//-----------------------------------------------------------------------------
// Audio Functions
//-----------------------------------------------------------------------------
bool CQuickTimeMaterial::HasAudio() { return m_bHasAudio; }
...
float CQuickTimeMaterial::GetVolume() { return m_CurrentVolume; }

void CQuickTimeMaterial::SetMuted(bool bMuteState) {
  AssertExitFunc(m_bMoviePlaying,
...
bool CQuickTimeMaterial::IsMuted() { return m_bMuted; }

VideoResult_t
CQuickTimeMaterial::SoundDeviceCommand(VideoSoundDeviceOperation_t operation,
                                       void *pDevice, void *pData) {
  AssertExitV(m_bMovieInitialized || m_bMoviePlaying,
              VideoResult::OPERATION_OUT_OF_SEQUENCE);
...
//-----------------------------------------------------------------------------
bool CQuickTimeMaterial::Init(const char *pMaterialName, const char *pFileName,
  ...
  m_TextureRegen.SetSourceGWorld(m_MovieGWorld, m_VideoFrameWidth,
                                 m_VideoFrameHeight);
  ...
  if (!BITFLAGS_SET(m_PlaybackFlags,
...
//-----------------------------------------------------------------------------
bool CQuickTimeMaterial::IsVideoReadyToPlay() { return m_bMovieInitialized; }

bool CQuickTimeMaterial::IsVideoPlaying() { return m_bMoviePlaying; }
...
bool CQuickTimeMaterial::IsNewFrameReady(void) {
  // Are we waiting to start playing the first frame? if so, tell them we are
  // ready!
...
bool CQuickTimeMaterial::IsFinishedPlaying() { return m_bMovieFinishedPlaying; }

void CQuickTimeMaterial::SetLooping(bool bLoopVideo) {
  m_bLoopMovie = bLoopVideo;
}

bool CQuickTimeMaterial::IsLooping() { return m_bLoopMovie; }
...
// stops movie for good, frees resources, but retains texture & material of last
// frame rendered
...
// supposed to be: Returns true on a new frame of video being downloaded into
// the texture
//-----------------------------------------------------------------------------
bool CQuickTimeMaterial::Update(void) {
  ...
  // Get the next frame after the current time (the movie may have advanced a
  // bit during UpdateMovie() and MovieTasks()
  GetMovieNextInterestingTime(m_QTMovie, nextTimeStep | nextTimeEdgeOK, 1,
                              qTypes, GetMovieTime(m_QTMovie, nullptr), fixed1,
                              &m_NextInterestingTimeToPlay, nullptr);

  // hit the end of the movie?
  if (GetMoviesError() == invalidTime ||
      m_NextInterestingTimeToPlay == END_OF_QUICKTIME_MOVIE) {
    m_NextInterestingTimeToPlay = NO_MORE_INTERESTING_TIMES;
  }
...
float CQuickTimeMaterial::GetVideoDuration() { return m_QTMovieDurationinSec; }

int CQuickTimeMaterial::GetFrameCount() { return m_QTMovieFrameCount; }
...
int CQuickTimeMaterial::GetCurrentFrame() {
  ...
  TimeValue curTime =
      m_bMoviePaused ? m_MoviePauseTime : GetMovieTime(m_QTMovie, nullptr);
...
bool CQuickTimeMaterial::SetTime(float flTime) {
  ...
  TimeValue newTime =
      (TimeValue)(flTime * m_QTMovieFrameRate.GetUnitsPerSecond() + 0.5f);
...
void CQuickTimeMaterial::CreateProceduralTexture(const char *pTextureName) {
  ...
  AssertIncRange(m_VideoFrameHeight, cMinVideoFrameHeight,
  ...
  bool actualSizeTexture =
      BITFLAGS_SET(m_PlaybackFlags, VideoPlaybackFlags::TEXTURES_ACTUAL_SIZE);

  int nWidth = (actualSizeTexture)
                   ? ALIGN_VALUE(m_VideoFrameWidth, TEXTURE_SIZE_ALIGNMENT)
                   : ComputeGreaterPowerOfTwo(m_VideoFrameWidth);
  int nHeight = (actualSizeTexture)
                    ? ALIGN_VALUE(m_VideoFrameHeight, TEXTURE_SIZE_ALIGNMENT)
  ...
  m_Texture.InitProceduralTexture(
      pTextureName, "VideoCacheTextures", nWidth, nHeight,
  ...
  m_TexCordU = (nTextureWidth > 0)
                   ? (float)m_VideoFrameWidth / (float)nTextureWidth
                   : 0.0f;
  m_TexCordV = (nTextureHeight > 0)
                   ? (float)m_VideoFrameHeight / (float)nTextureHeight
                   : 0.0f;
}

void CQuickTimeMaterial::DestroyProceduralTexture() {
  if (m_Texture != nullptr) {
    // DO NOT Call release on the Texture Regenerator, as it will destroy this
    // object!  bad bad bad instead we tell it to assign a NULL regenerator and
    // flag it to not call release
...
void CQuickTimeMaterial::OpenQTMovie(const char *theQTMovieFileName) {
  ...
  CFStringRef imageStrRef =
      CFStringCreateWithCString(NULL, theQTMovieFileName, 0);
  AssertExitFunc(imageStrRef != nullptr,
                 SetResult(VideoResult::SYSTEM_ERROR_OCCURED));

  OSErr status = QTNewDataReferenceFromFullPathCFString(
  ...
  status = NewMovieFromDataRef(&m_QTMovie, newMovieActive, nil,
  ...
  if (BITFLAGS_SET(m_PlaybackFlags, VideoPlaybackFlags::NO_AUDIO)) {
    ...
    Track audioTrack =
        GetMovieIndTrackType(m_QTMovie, 1, SoundMediaType, movieTrackMediaType);
  ...
  m_QTMovieDurationinSec =
      float(double(m_QTMovieDuration) / double(m_QTMovieTimeScale));
  ...
  if (m_QTMovieFrameRate.GetUnitsPerSecond() == m_QTMovieTimeScale) {
    m_QTMovieFrameCount =
        m_QTMovieDuration / m_QTMovieFrameRate.GetUnitsPerFrame();
  } else {
    m_QTMovieFrameCount =
        (int)((float)m_QTMovieDurationinSec * m_QTMovieFrameRate.GetFPS() +
              0.5f);
  }
  ...
  AssertExitFunc(m_QTMovieRect.top == 0 && m_QTMovieRect.left == 0 &&
                     m_QTMovieRect.right >= cMinVideoFrameWidth &&
                     m_QTMovieRect.right <= cMaxVideoFrameWidth &&
                     m_QTMovieRect.bottom >= cMinVideoFrameHeight &&
                     m_QTMovieRect.bottom <= cMaxVideoFrameHeight &&
                     m_QTMovieRect.right % 4 == 0,
                 SetResult(VideoResult::VIDEO_ERROR_OCCURED));

  // Setup the QuiuckTime Graphics World for the Movie
  status = QTNewGWorld(&m_MovieGWorld, k32BGRAPixelFormat, &m_QTMovieRect, nil,
                       nil, 0);
  ...
  if (!CreateMovieAudioContext(m_bHasAudio, m_QTMovie, &m_AudioContext, true,
                               &m_CurrentVolume)) {
    SetResult(VideoResult::AUDIO_ERROR_OCCURED);
  ...
  short qFlags =
      nextTimeStep |
      nextTimeEdgeOK; // use nextTimeStep instead of nextTimeMediaSample for
                      // MPEG 1-2 compatibility

  GetMovieNextInterestingTime(m_QTMovie, qFlags, 1, qTypes, (TimeValue)0,
                              fixed1, &m_MovieFirstFrameTime, NULL);
  AssertExitFunc(GetMoviesError() == noErr,
                 SetResult(VideoResult::VIDEO_ERROR_OCCURED));
