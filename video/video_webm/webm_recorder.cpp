...
// Bitrate table for various resolutions, used to compute estimated size of
// files as well as to set the WebM codec.
...
// Quality is passed into us as a number between 0 and 100, we use that to scale
// between the min and max bitrates.
static WebMEncodingDataRateInfo_t s_WebMEncodeRates[] = {
    {320, 240, 1000, 1000, 128, 128},  {640, 960, 2000, 2000, 128, 128},
    {960, 640, 2000, 2000, 128, 128},  {720, 480, 2500, 2500, 128, 128},
    {1280, 720, 5000, 5000, 384, 384}, {1920, 1080, 8000, 8000, 384, 384},
};
...
CWebMVideoRecorder::CWebMVideoRecorder()
    : m_LastResult(VideoResult::SUCCESS), m_bHasAudio(false),
      m_bMovieFinished(false), m_SrcImageYV12Buffer(NULL), m_nFramesAdded(0),
      m_nAudioFramesAdded(0), m_nSamplesAdded(0), m_audioChannels(2),
      m_audioSampleRate(44100), m_audioBitDepth(16), m_audioSampleGroupSize(0) {
...
bool CWebMVideoRecorder::CreateNewMovieFile(const char *pFilename,
                                            bool hasAudio) {
#ifdef LOG_ENCODER_OPERATIONS
...
bool CWebMVideoRecorder::SetMovieVideoParameters(
  ...
  if (vpx_codec_enc_config_default(vpx_codec_vp8_cx(), &m_vpxConfig, 0) !=
      VPX_CODEC_OK) {
  ...
  m_FrameDuration =
      (unsigned long)(((double)m_DurationPerFrame * (double)1000000000) /
                      (double)m_MovieTimeScale);

  // Set the bitrate for this size and level of quality
  m_vpxConfig.rc_target_bitrate =
      GetVideoDataRate(videoQuality, movieFrameWidth, movieFrameHeight);
  ...
  // Init the codec
  if (vpx_codec_enc_init(&m_vpxContext, vpx_codec_vp8_cx(), &m_vpxConfig, 0) !=
      VPX_CODEC_OK) {
    SetResult(VideoResult::INITIALIZATION_ERROR_OCCURED);
    return false;
  }
  ...
  // add the video track
  m_vid_track = m_mkvMuxerSegment.AddVideoTrack(
      static_cast<int>(m_vpxConfig.g_w), static_cast<int>(m_vpxConfig.g_h), 1);
...
bool CWebMVideoRecorder::SetMovieSourceImageParameters(
  ...
  // Setup the buffers for encoding the frame.  WebM requires a frame in YV12
  // format
  m_SrcImageYV12Buffer = vpx_img_alloc(NULL, VPX_IMG_FMT_YV12, m_SrcImageWidth,
                                       m_SrcImageHeight, 1);
...
bool CWebMVideoRecorder::SetMovieSourceAudioParameters(
  ...
  int ret = vorbis_encode_init(&m_vi, m_audioChannels, m_audioSampleRate, -1,
                               WEBM_AUDIO_BITRATE, -1);
  if (ret) {
    SetResult(VideoResult::BAD_INPUT_PARAMETERS);
    return false;
  }
  ...
  m_aud_track =
      m_mkvMuxerSegment.AddAudioTrack(static_cast<int>(m_audioSampleRate),
                                      static_cast<int>(m_audioChannels), 0);
  if (!m_aud_track) {
...

  {
    ...
    vorbis_analysis_headerout(&m_vd, &m_vc, &ident_packet, &comments_packet,
                              &setup_packet);
    iHeaderLength =
        3 + ident_packet.bytes + comments_packet.bytes + setup_packet.bytes;
...
void CWebMVideoRecorder::ConvertBGRAToYV12(void *pFrameBuffer,
  ...
  // This isn't fast or good, but it works and that's good enough for a first
  // pass
...
bool CWebMVideoRecorder::AppendVideoFrame(void *pFrameBuffer,
                                          int nStrideAdjustBytes) {
  uint64 time_ns;
#ifdef LOG_ENCODER_OPERATIONS
...
// If you are trying to debug how the webm libraries work it's very difficult to
// use the full TF2 replay system as a testbed so this section of code here and
// in the AppendAudioSamples writes out the data TF2 would send to the video
// recorder to plain files on the disc.  You can then use those files with an
// extracted framework to work with the webm libraries
#ifdef NEVER
  {
    FILE *fp = NULL;
    char rgch[256];
  ...
  case VideoEncodeSourceFormat::BGR_24BIT:
    ConvertBGRAToYV12(pFrameBuffer, nStrideAdjustBytes, m_SrcImageYV12Buffer,
                      false);
    break;
  ...
  vpx_codec_err_t vpxError = vpx_codec_encode(
      &m_vpxContext, m_SrcImageYV12Buffer, time_ns, m_FrameDuration, 0, 0);
  ...
  while ((vpxPacket = vpx_codec_get_cx_data(&m_vpxContext, &vpxIter)) != NULL) {
    if (vpxPacket->kind == VPX_CODEC_CX_FRAME_PKT) {
      // Extract if this is a keyframe from the first packet of data for each
      // frame
      bKeyframe = vpxPacket->data.frame.flags & VPX_FRAME_IS_KEY;

      m_mkvMuxerSegment.AddFrame((const uint8 *)vpxPacket->data.frame.buf,
                                 vpxPacket->data.frame.sz, m_vid_track, time_ns,
                                 bKeyframe);
    }
  }
  m_nFramesAdded++;
...
bool CWebMVideoRecorder::FlushAudioSamples() {
  ...
  while (vorbis_analysis_blockout(&m_vd, &m_vb) == 1) {
    ...
    while ((status_analysis = vorbis_bitrate_flushpacket(&m_vd, &op)) > 0) {
      ...
      if (!m_mkvMuxerSegment.AddFrame(op.packet, op.bytes, m_aud_track, time_ns,
                                      true)) {
        return false;
      }
    }
...
bool CWebMVideoRecorder::AppendAudioSamples(void *pSampleBuffer,
                                            size_t sampleSize) {
#ifdef LOG_ENCODER_OPERATIONS
...
bool CWebMVideoRecorder::FinishMovie(bool SaveMovieToDisk) {
  ...
  vpx_codec_err_t vpxError =
      vpx_codec_encode(&m_vpxContext, NULL, time_ns, m_FrameDuration, 0, 0);
...
bool CWebMVideoRecorder::EstimateMovieFileSize(
  ...
  *pEstSize =
      (size_t)((fVidRate * 1000 * movieDurationInSeconds / 8) +
...
float CWebMVideoRecorder::GetVideoDataRate(int quality, int width, int height) {
  ...
  for (int i = 0; i < ARRAYSIZE(s_WebMEncodeRates); i++) {
    if (s_WebMEncodeRates[i].m_XResolution == width &&
        s_WebMEncodeRates[i].m_YResolution == height) {
      return s_WebMEncodeRates[i].m_MinDataRate +
             (((s_WebMEncodeRates[i].m_MaxDataRate -
                s_WebMEncodeRates[i].m_MinDataRate) *
               (float)quality) /
              100.0);
    }
  }
  ...
  for (int i = 0; i < ARRAYSIZE(s_WebMEncodeRates); i++) {
    if (s_WebMEncodeRates[i].m_XResolution == width &&
      ...
      return s_WebMEncodeRates[i].m_MinAudioDataRate +
             (((s_WebMEncodeRates[i].m_MaxAudioDataRate -
                s_WebMEncodeRates[i].m_MinAudioDataRate) *
               (float)quality) /
              100.0);
    }
  }
