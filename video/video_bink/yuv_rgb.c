...
void rgb24_yuv420_std(uint32_t width, uint32_t height, const uint8_t *RGB,
                      uint32_t RGB_stride, uint8_t *Y, uint8_t *U, uint8_t *V,
  ...
  for (y = 0; y < (height - 1); y += 2) {
    ...
    uint8_t *y_ptr1 = Y + y * Y_stride, *y_ptr2 = Y + (y + 1) * Y_stride,
            *u_ptr = U + (y / 2) * UV_stride, *v_ptr = V + (y / 2) * UV_stride;
    ...
    for (x = 0; x < (width - 1); x += 2) {
      ...
      y_tmp = (param->r_factor * rgb_ptr1[0] + param->g_factor * rgb_ptr1[1] +
      ...
      y_tmp = (param->r_factor * rgb_ptr1[3] + param->g_factor * rgb_ptr1[4] +
      ...
      y_tmp = (param->r_factor * rgb_ptr2[0] + param->g_factor * rgb_ptr2[1] +
      ...
      y_tmp = (param->r_factor * rgb_ptr2[3] + param->g_factor * rgb_ptr2[4] +
...
void rgb32_yuv420_std(uint32_t width, uint32_t height, const uint8_t *RGBA,
                      uint32_t RGBA_stride, uint8_t *Y, uint8_t *U, uint8_t *V,
                      uint32_t Y_stride, uint32_t UV_stride,
                      YCbCrType yuv_type) {
  const RGB2YUVParam *const param = &(RGB2YUV[yuv_type]);
  ...
  for (y = 0; y < (height - 1); y += 2) {
    ...
    for (x = 0; x < (width - 1); x += 2) {
      ...
      y_tmp = (param->r_factor * rgb_ptr1[4] + param->g_factor * rgb_ptr1[5] +
      ...
      y_tmp = (param->r_factor * rgb_ptr2[4] + param->g_factor * rgb_ptr2[5] +
...
void nv12_rgb24_std(uint32_t width, uint32_t height, const uint8_t *Y,
                    const uint8_t *UV, uint32_t Y_stride, uint32_t UV_stride,
...
// see rgb.txt
#define UNPACK_RGB24_32_STEP(RS1, RS2, RS3, RS4, RS5, RS6, RD1, RD2, RD3, RD4, \
                             RD5, RD6)                                         \
...
#define RGB2YUV_16(R, G, B, Y, U, V)                                           \
  ...
  Y = _mm_add_epi16(                                                           \
      _mm_srli_epi16(_mm_mullo_epi16(Y, _mm_set1_epi16(param->y_factor)), 7),  \
...
#define RGB2YUV_32                                                             \
  ...
  /* see rgb.txt to get an idea of the algorithm, note that we only go to the  \
   * next to last step*/                                                       \
  /* here, because averaging in horizontal direction is easier like this*/     \
  /* The last step is applied further on the Y channel only*/                  \
  UNPACK_RGB24_32_STEP(rgb1, rgb2, rgb3, rgb4, rgb5, rgb6, tmp1, tmp2, tmp3,   \
                       tmp4, tmp5, tmp6)                                       \
  UNPACK_RGB24_32_STEP(tmp1, tmp2, tmp3, tmp4, tmp5, tmp6, rgb1, rgb2, rgb3,   \
                       rgb4, rgb5, rgb6)                                       \
  UNPACK_RGB24_32_STEP(rgb1, rgb2, rgb3, rgb4, rgb5, rgb6, tmp1, tmp2, tmp3,   \
                       tmp4, tmp5, tmp6)                                       \
  UNPACK_RGB24_32_STEP(tmp1, tmp2, tmp3, tmp4, tmp5, tmp6, rgb1, rgb2, rgb3,   \
  ...
  y1_16 =                                                                      \
      _mm_add_epi16(_mm_mullo_epi16(r_16, _mm_set1_epi16(param->r_factor)),    \
  ...
  y1_16 = _mm_add_epi16(                                                       \
      _mm_srli_epi16(_mm_mullo_epi16(y1_16, _mm_set1_epi16(param->y_factor)),  \
                     7),                                                       \
      _mm_set1_epi16(param->y_offset));                                        \
  y2_16 = _mm_add_epi16(                                                       \
      _mm_srli_epi16(_mm_mullo_epi16(y2_16, _mm_set1_epi16(param->y_factor)),  \
                     7),                                                       \
      _mm_set1_epi16(param->y_offset));                                        \
   ...
   */                                                                          \
  /* Y is saved for each pixel, while only sums of (B-Y') and (R-Y') for pairs \
   * of adjacents pixels are added to the previous values*/                    \
...
// see rgba.txt
#define UNPACK_RGB32_32_STEP(RS1, RS2, RS3, RS4, RS5, RS6, RS7, RS8, RD1, RD2, \
                             RD3, RD4, RD5, RD6, RD7, RD8)                     \
...
#define RGBA2YUV_32                                                            \
  ...
  /* here, because averaging in horizontal direction is easier like this*/     \
  /* The last step is applied further on the Y channel only*/                  \
  UNPACK_RGB32_32_STEP(rgb1, rgb2, rgb3, rgb4, rgb5, rgb6, rgb7, rgb8, tmp1,   \
                       tmp2, tmp3, tmp4, tmp5, tmp6, tmp7, tmp8)               \
  UNPACK_RGB32_32_STEP(tmp1, tmp2, tmp3, tmp4, tmp5, tmp6, tmp7, tmp8, rgb1,   \
                       rgb2, rgb3, rgb4, rgb5, rgb6, rgb7, rgb8)               \
  UNPACK_RGB32_32_STEP(rgb1, rgb2, rgb3, rgb4, rgb5, rgb6, rgb7, rgb8, tmp1,   \
                       tmp2, tmp3, tmp4, tmp5, tmp6, tmp7, tmp8)               \
  UNPACK_RGB32_32_STEP(tmp1, tmp2, tmp3, tmp4, tmp5, tmp6, tmp7, tmp8, rgb1,   \
                       rgb2, rgb3, rgb4, rgb5, rgb6, rgb7, rgb8)               \
...
#define UV2RGB_16(U, V, R1, G1, B1, R2, G2, B2)                                \
  r_tmp =                                                                      \
      _mm_srai_epi16(_mm_mullo_epi16(V, _mm_set1_epi16(param->cr_factor)), 6); \
  g_tmp = _mm_srai_epi16(                                                      \
      _mm_add_epi16(_mm_mullo_epi16(U, _mm_set1_epi16(param->g_cb_factor)),    \
                    _mm_mullo_epi16(V, _mm_set1_epi16(param->g_cr_factor))),   \
      7);                                                                      \
  b_tmp =                                                                      \
      _mm_srai_epi16(_mm_mullo_epi16(U, _mm_set1_epi16(param->cb_factor)), 6); \
...
#define ADD_Y2RGB_16(Y1, Y2, R1, G1, B1, R2, G2, B2)                           \
  Y1 =                                                                         \
      _mm_srli_epi16(_mm_mullo_epi16(Y1, _mm_set1_epi16(param->y_factor)), 7); \
  Y2 =                                                                         \
      _mm_srli_epi16(_mm_mullo_epi16(Y2, _mm_set1_epi16(param->y_factor)), 7); \
                                                                               \
...
#define PACK_RGB24_32_STEP(RS1, RS2, RS3, RS4, RS5, RS6, RD1, RD2, RD3, RD4,   \
                           RD5, RD6)                                           \
...
#define PACK_RGB24_32(R1, R2, G1, G2, B1, B2, RGB1, RGB2, RGB3, RGB4, RGB5,    \
                      RGB6)                                                    \
  PACK_RGB24_32_STEP(R1, R2, G1, G2, B1, B2, RGB1, RGB2, RGB3, RGB4, RGB5,     \
                     RGB6)                                                     \
  PACK_RGB24_32_STEP(RGB1, RGB2, RGB3, RGB4, RGB5, RGB6, R1, R2, G1, G2, B1,   \
                     B2)                                                       \
  PACK_RGB24_32_STEP(R1, R2, G1, G2, B1, B2, RGB1, RGB2, RGB3, RGB4, RGB5,     \
                     RGB6)                                                     \
  PACK_RGB24_32_STEP(RGB1, RGB2, RGB3, RGB4, RGB5, RGB6, R1, R2, G1, G2, B1,   \
...
#define YUV2RGB_32                                                             \
  ...
  __m128i v_16 = _mm_srai_epi16(_mm_unpacklo_epi8(v, v), 8);                   \
                                                                               \
  UV2RGB_16(u_16, v_16, r_uv_16_1, g_uv_16_1, b_uv_16_1, r_uv_16_2, g_uv_16_2, \
            b_uv_16_2)                                                         \
  r_16_1 = r_uv_16_1;                                                          \
  g_16_1 = g_uv_16_1;                                                          \
  b_16_1 = b_uv_16_1;                                                          \
  ...
  __m128i rgb_1, rgb_2, rgb_3, rgb_4, rgb_5, rgb_6;                            \
                                                                               \
  PACK_RGB24_32(r_8_11, r_8_12, g_8_11, g_8_12, b_8_11, b_8_12, rgb_1, rgb_2,  \
                rgb_3, rgb_4, rgb_5, rgb_6)                                    \
  ...
  SAVE_SI128((__m128i *)(rgb_ptr1 + 80), rgb_6);                               \
                                                                               \
  PACK_RGB24_32(r_8_21, r_8_22, g_8_21, g_8_22, b_8_21, b_8_22, rgb_1, rgb_2,  \
                rgb_3, rgb_4, rgb_5, rgb_6)                                    \
