...
CBinkMaterialRGBTextureRegenerator::CBinkMaterialRGBTextureRegenerator()
    : m_nSourceWidth(0), m_nSourceHeight(0) {}

CBinkMaterialRGBTextureRegenerator::~CBinkMaterialRGBTextureRegenerator() {
  // nothing to do
}

void CBinkMaterialRGBTextureRegenerator::SetSourceImage(uint8_t *SrcImage,
                                                        int nWidth,
                                                        int nHeight) {
  m_SrcImage = SrcImage;
...
void CBinkMaterialRGBTextureRegenerator::RegenerateTextureBits(
  ...
  /*	if ( m_SrcGWorld == nullptr )
          {
                  ...
                  memset( pVTFTexture->ImageData(), 0xCC,
     pVTFTexture->ComputeTotalSize() ); return;
          }*/
...
CBinkMaterial::CBinkMaterial()
    : m_pFileName(nullptr), m_bInitCalled(false), m_AVFrame(nullptr),
      m_AVPkt(nullptr) {
...
void CBinkMaterial::SetFileName(const char *theMovieFileName) {
  ...
  if (theMovieFileName != nullptr) {
    AssertMsg(V_strlen(theQTMovieFileName) <= MAX_FILENAME_LEN,
...
//-----------------------------------------------------------------------------
const char *CBinkMaterial::GetVideoFileName() { return m_pFileName; }

VideoFrameRate_t &CBinkMaterial::GetVideoFrameRate() {
  return m_QTMovieFrameRate;
}

VideoResult_t CBinkMaterial::GetLastResult() { return m_LastResult; }

//-----------------------------------------------------------------------------
// Audio Functions
//-----------------------------------------------------------------------------
bool CBinkMaterial::HasAudio() { return m_bHasAudio; }
...
float CBinkMaterial::GetVolume() { return m_CurrentVolume; }
...
bool CBinkMaterial::IsMuted() { return m_bMuted; }

VideoResult_t
CBinkMaterial::SoundDeviceCommand(VideoSoundDeviceOperation_t operation,
...
//-----------------------------------------------------------------------------
bool CBinkMaterial::Init(const char *pMaterialName, const char *pFileName,
...
//-----------------------------------------------------------------------------
bool CBinkMaterial::IsVideoReadyToPlay() { return m_bMovieInitialized; }

bool CBinkMaterial::IsVideoPlaying() { return m_bMoviePlaying; }
...
bool CBinkMaterial::IsFinishedPlaying() { return m_bMovieFinishedPlaying; }

void CBinkMaterial::SetLooping(bool bLoopVideo) { m_bLoopMovie = bLoopVideo; }

bool CBinkMaterial::IsLooping() { return m_bLoopMovie; }
...
bool CBinkMaterial::Update(void) {
  ...
  while ((ret = av_read_frame(m_AVFmtCtx, m_AVPkt)) >= 0) {
    if (m_AVPkt->stream_index == m_AVVideoStreamID) {
      ...
      if (m_AVVideoDecCtx->codec->type == AVMEDIA_TYPE_VIDEO) {
        av_image_copy(m_AVVideoData, m_AVVideoLinesize,
                      (const uint8_t **)(m_AVFrame->data), m_AVFrame->linesize,
                      m_AVPixFormat, m_VideoFrameWidth, m_VideoFrameHeight);
      }
  ...
  yuv420_rgb24_std(m_VideoFrameWidth, m_VideoFrameHeight, m_AVVideoData[0],
                   ...
                   m_AVVideoData[0] + m_VideoFrameWidth * m_VideoFrameHeight +
                       ((m_VideoFrameWidth + 1) / 2) *
                           ((m_VideoFrameHeight + 1) / 2),
                   m_VideoFrameWidth, (m_VideoFrameWidth + 1) / 2, m_RGBData,
...
//-----------------------------------------------------------------------------
IMaterial *CBinkMaterial::GetMaterial() { return m_Material; }
...
float CBinkMaterial::GetVideoDuration() { return m_QTMovieDurationinSec; }

int CBinkMaterial::GetFrameCount() { return m_QTMovieFrameCount; }
...
int CBinkMaterial::GetCurrentFrame() {
  ...
  float curTime; // = m_bMoviePaused ? m_MoviePauseTime : GetMovieTime(
                 // m_QTMovie, nullptr );
...
void CBinkMaterial::CreateProceduralTexture(const char *pTextureName) {
  ...
  m_Texture.InitProceduralTexture(
      pTextureName, "VideoCacheTextures", nWidth, nHeight, IMAGE_FORMAT_RGB888,
...
void CBinkMaterial::OpenMovie(const char *theMovieFileName) {
  ...
  */
  ...
  if (open_codec_context(&m_AVVideoStreamID, &m_AVVideoDecCtx, m_AVFmtCtx,
  ...
  m_MovieFrameDuration = 1.0 / ((double)m_AVVideoStream->r_frame_rate.num /
                                (double)m_AVVideoStream->r_frame_rate.den);
  m_TextureRegen.SetSourceImage(m_RGBData, m_VideoFrameWidth,
                                m_VideoFrameHeight);
  printf("Video FPS: %lf\n", (double)m_AVVideoStream->r_frame_rate.num /
                                 (double)m_AVVideoStream->r_frame_rate.den);
