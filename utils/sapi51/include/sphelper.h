...
public:
  ...
  WCHAR *Append(const WCHAR *pszSrc) {
    if (pszSrc) {
      ...
      if (lenSrc) {
        ULONG lenMe = Length();
        WCHAR *pszNew =
            (WCHAR *)::CoTaskMemAlloc((lenMe + lenSrc + 1) * sizeof(WCHAR));
        if (pszNew) {
  ...
  WCHAR *Append2(const WCHAR *pszSrc1, const WCHAR *pszSrc2) {
    ...
    if (lenSrc1 || lenSrc2) {
      ULONG lenMe = Length();
      WCHAR *pszNew = (WCHAR *)::CoTaskMemAlloc(
          (lenMe + lenSrc1 + lenSrc2 + 1) * sizeof(WCHAR));
      if (pszNew) {
        if (m_psz) // Could append to an empty string so check...
        {
          if (lenMe) {
...
inline HRESULT SpFindBestToken(const WCHAR *pszCategoryId,
  ...
  size_t ulLen = pszOptAttribs
                     ? wcslen(pszOptAttribs) + ulLenVendorPreferred + 1
                     : ulLenVendorPreferred;
  WCHAR *pszOptAttribsVendorPref =
      (WCHAR *)_alloca((ulLen + 1) * sizeof(WCHAR));
 ...
 *   On Win98 and WinMe ::GetLastError() returns 0 for some functions (see
 *MSDN). We therefore check for that and return E_FAIL. This function should
 *only be called in an error case since it will always return an error code!
 *
 *   Returns:
...
inline LANGID SpGetUserDefaultUILanguage(void) {
  ...
  // Get the UI language by one of three methods, depending on the system
    ...
    long lRet = RegOpenKeyEx(
        HKEY_USERS, _T(".Default\\Control Panel\\desktop\\ResourceLocale"), 0,
        KEY_READ, &hKey);
    ...
    // Case 2: Running on Windows 2000 or later. Use GetUserDefaultUILanguage to
    // find the user's prefered UI language
    ...
    if (hMKernel32 == NULL) {
      ...
      LANGID(WINAPI * pfnGetUserDefaultUILanguage)() = (LANGID(WINAPI *)(void))
#ifdef _WIN32_WCE
    ...
    LONG lRet =
        RegOpenKeyEx(HKEY_USERS, _T(".DEFAULT\\Control Panel\\International"),
                     0, KEY_READ, &hKey);
#ifdef _WIN32_WCE_BUG_10655
    if (lRet == ERROR_INVALID_PARAMETER) {
      lRet = ERROR_FILE_NOT_FOUND;
    ...
    if (SUCCEEDED(hr)) {
      lRet = RegQueryValueEx(hKey, _T("Locale"), NULL, NULL, (BYTE *)szData,
                             &dwSize);
  ...
  return (wUILang ? wUILang
                  : ::GetUserDefaultLangID()); // In failure case, try our best!
}

inline HRESULT
SpGetDescription(ISpObjectToken *pObjToken, WCHAR **ppszDescription,
...
public:
  ...
  HRESULT ParamValidateAssignFormat(REFGUID rguidFormatId,
    ...
    if ((pWaveFormatEx &&
         (::IsBadReadPtr(pWaveFormatEx, sizeof(*pWaveFormatEx)) ||
          rguidFormatId != SPDFID_WaveFormatEx)) ||
...
inline UINT SpCodePageFromLcid(LCID lcid) {
  ...
  return (0 != GetLocaleInfoA(lcid, LOCALE_IDEFAULTANSICODEPAGE, achCodePage,
                              sizeof(achCodePage)))
             ? atoi(achCodePage)
             : 65001;
}
...
#ifndef _UNICODE
inline HRESULT SPBindToFile(const TCHAR *pFileName, SPFILEMODE eMode,
                            ISpStream **ppStream, const GUID *pFormatId = NULL,
                            const WAVEFORMATEX *pWaveFormatEx = NULL,
                            ULONGLONG ullEventInterest = SPFEI_ALL_EVENTS) {
...
inline ULONG SpSerializedEventSize(const SPSERIALIZEDEVENT *pSerEvent,
                                   ULONG ulSize) {
  if ((pSerEvent->elParamType == SPET_LPARAM_IS_POINTER) &&
      pSerEvent->SerializedlParam) {
    ulSize += ULONG(pSerEvent->SerializedwParam);
...
inline ULONG SpSerializedEventSize(const SPSERIALIZEDEVENT64 *pSerEvent,
                                   ULONG ulSize) {
  if ((pSerEvent->elParamType == SPET_LPARAM_IS_POINTER) &&
      pSerEvent->SerializedlParam) {
    ulSize += ULONG(pSerEvent->SerializedwParam);
...
public:
  ...
  HRESULT CopyTo(SPEVENT *pDestEvent) const {
    ...
    if ((elParamType == SPET_LPARAM_IS_POINTER) && lParam) {
      ...
      pDestEvent->lParam = (LPARAM)::CoTaskMemAlloc(
          (wcslen((WCHAR *)lParam) + 1) * sizeof(WCHAR));
      if (pDestEvent->lParam) {
  ...
  template <class T> void Serialize(T *pSerEvent) const {
...
// WCE compiler does not work propertly with template
  ...
  ISpRecoResult *RecoResult() const {
    SPDBG_ASSERT(eEventId == SPEI_RECOGNITION ||
                 eEventId == SPEI_FALSE_RECOGNITION ||
                 eEventId == SPEI_HYPOTHESIS);
...
CreatePhraseFromWordArray(const WCHAR **ppWords, ULONG cWords,
  ...
  if (cWords == 0 || ppWords == NULL ||
      ::IsBadReadPtr(ppWords, sizeof(*ppWords) * cWords)) {
    return E_INVALIDARG;
  }

  if (prgDispAttribs != NULL &&
      ::IsBadReadPtr(prgDispAttribs, sizeof(*prgDispAttribs) * cWords)) {
    return E_INVALIDARG;
  }
