...
static void PrintArgSummaryAndExit(int iReturnCode = 1) {
  ...
  printf(
      ...
      //recalculate file CRC's)\n" 		"            Can be used with -k to sign an
      //existing unsigned VPK.\n"
      "\n"
      "MISC:\n"
...
void ReadFile(char const *pName) {
  ...
  if (f) {
    ...
    unsigned bufSize =
        ((IFileSystem *)g_pFullFileSystem)->GetOptimalReadSize(f, fileSize);
    void *buffer =
        ((IFileSystem *)g_pFullFileSystem)->AllocOptimalReadBuffer(f, bufSize);
    // read into local buffer
    (((IFileSystem *)g_pFullFileSystem)->ReadEx(buffer, bufSize, fileSize, f) !=
     0);
...
static void AddFileToPack(CPackedStore &mypack, char const *pSrcName,
                          int nPreloadSize = 0, char const *pDestName = NULL) {
  ...
  ePackedStoreAddResultCode rslt =
      mypack.AddFile(pDestName, Min(fileSize, nPreloadSize), pData, fileSize,
                     s_bMakeMultiChunk);
...
static void CheckLoadKeyFilesForSigning(CPackedStore &mypack) {
  ...
  if (!s_bMakeMultiChunk) {
    Error("Multichunk not specified. Only multi-chunk VPK's support "
          "signatures.\n");
  }
...
private:
  ...
  void MapRangeToChunk(int idxRange, int iChunkFilenameIndex,
...
void VPKBuilder::BuildSteamPipeFriendlyFromInputKeys() {
  ...
  FOR_EACH_VEC(m_vecOldFiles, i) {
    ...
    if (m_dictFiles[idxInDict].m_pOld)
      Error("File '%s' is listed in VPK directory multiple times?!  Cannot "
            "build incrementally.\n",
            szNameInVPK);
  ...
  if (bIncremental) {
    ...
    if (m_pOldInputKeys->LoadFromFile(g_pFullFileSystem, sControlFilenameBak)) {
      ...
      for (KeyValues *i = m_pOldInputKeys; i; i = i->GetNextKey()) {
        ...
        int idxInDict = m_dictFiles.Find(szNameInVPK);
        if (idxInDict == m_dictFiles.InvalidIndex() ||
            m_dictFiles[idxInDict].m_pOld == NULL)
          Error("File '%s' in old KeyValues control file not found in pack "
        ...
        if (bf.m_pOldKey)
          Error("File '%s' appears multiple times in old KeyValues control "
                "file.\nThat control file was probably not used to build the "
                "pack file\n",
                szNameInVPK);
        bf.m_pOldKey = i;
        ...
        if (iPreloadSizeFromControlFile != (int)bf.m_pOld->m_iPreloadSize)
          Error("File '%s' preload size mismatch in old KeyValues control file "
        ...
        if (*pszMD5) {
          ...
          printf("WARNING: Old control file entry '%s' does not have an MD5; "
      ...
      printf("WARNING: %s not present; incremental building will be slow.\n",
             sControlFilenameBak.String());
      printf("         For best results, provide the control file previously "
             "used for building.\n");
      m_pOldInputKeys->deleteThis();
      m_pOldInputKeys = NULL;
    }
  ...
  for (int idxChunk = 0; idxChunk < nNewChunkCount; ++idxChunk) {
    ...
    if (r.m_bKeepExistingFile) {
      ...
      // chunk.
      for (int idxFile = r.m_iFirstInputFile; idxFile <= r.m_iLastInputFile;
           ++idxFile) {
    ...
    for (int idxFile = r.m_iFirstInputFile; idxFile <= r.m_iLastInputFile;
      ...
      int nBytesWritten = g_pFullFileSystem->Write(
          (byte *)buf.Base() + f->m_iPreloadSize, nBytesToWrite, fChunkWrite);
  ...
  for (int iChunkToDelete = nNewChunkCount; iChunkToDelete < nOldChunkCount;
       ++iChunkToDelete) {
    char szDataFilename[MAX_PATH];
    m_packfile.GetDataFileName(szDataFilename, sizeof(szDataFilename),
                               iChunkToDelete);
...
void VPKBuilder::SetInputKeys(KeyValues *pInputKeys,
  ...
  for (KeyValues *i = m_pInputKeys; i; i = i->GetNextKey()) {
    ...
    if (szNameInVPK[0] == '\\')
      Error("destpath '%s' is an absolute path; only relative paths should be "
            "used",
            szNameInVPK);
    ...
    if (IsRestrictedFileType(szNameInVPK)) {
      printf("WARNING: Control file lists '%s'.  We cannot put that type of "
             "file in the pack.\n",
             szNameInVPK);
      continue;
    }
    ...
    if (!bf.m_sNameOnDisk.IsEmpty() || bf.m_pNew || bf.m_iNewSortIndex >= 0)
      Error("destpath '%s' in VPK appears multiple times in the KV control "
...
void VPKBuilder::MapAllRangesToChunks() {
  ...
  for (;;) {
    ...
    FOR_EACH_LL(m_llFileRanges, idxRange) {
      ...
      if (r->m_iChunkFilenameIndex >= 0) {
        Assert(r->m_bKeepExistingFile);
        printf("Chunk %d is currently bigger than desired chunk size of %d "
               "bytes, but we're not splitting it because the contents have "
               "not changed.\n",
...
void VPKBuilder::MapRangeToChunk(int idxRange, int iChunkFilenameIndex,
                                 bool bKeepExistingFile) {
  VPKInputFileRange_t *p = &m_llFileRanges[idxRange];
...
static void CheckSignature(const char *pszFilename) {
  ...
  if (s_sPublicKeyFile.IsEmpty()) {
    if (!s_sPrivateKeyFile.IsEmpty())
      Error("Private keys are not used to verify signatures.  Did you mean to "
            "use -k instead?");
...
void BuildRecursiveFileList(const char *pcDirName, CUtlStringList &fileList) {
  ...
  V_snprintf(szDirWildcard, sizeof(szDirWildcard), "%s%c%s", pcDirName,
  ...
  if (pcResult) {
    do {
      ...
      V_snprintf(szFullResultPath, sizeof(szFullResultPath), "%s%c%s",
                 pcDirName, CORRECT_PATH_SEPARATOR, pcResult);
...
static void DroppedVpk(const char *pszVpkFilename) {
  ...
  for (int i = 0; i < fileNames.Count(); i++) {
    ...
    V_snprintf(szDestFilePath, sizeof(szDestFilePath), "%s%c%s", szVPKParentDir,
               CORRECT_PATH_SEPARATOR, fileNames[i]);
