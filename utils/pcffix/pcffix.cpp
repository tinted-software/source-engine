...
//-----------------------------------------------------------------------------
static bool IsScalarAttributesEqual(CDmElement *pElement1,
                                    CDmElement *pElement2) {
  int nCount = pElement1->AttributeCount();
  if (nCount != pElement2->AttributeCount())
    return false;
  for (CDmAttribute *pAtt1 = pElement1->FirstAttribute(); pAtt1;
       pAtt1 = pAtt1->NextAttribute()) {
...
//-----------------------------------------------------------------------------
static bool IsArrayEqual(CDmElement *pElement1, CDmElement *pElement2,
                         const char *pArrayAttribute) {
...
//-----------------------------------------------------------------------------
static CDmElement *FindOrAddDef(CDmElement *pDef,
  ...
  if (!uniqueNames.Defined(pDef->GetName())) {
    ...
    char pNewName[512];
    Q_snprintf(pNewName, sizeof(pNewName), "%s Version #%d", pDef->GetName(),
               nCopyCount);
...
// Replace references
//-----------------------------------------------------------------------------
static void ReplaceChildReferences(CDmElement *pElement,
                                   CDmElement *pOldVersion,
                                   CDmElement *pNewVersion) {
  for (CDmAttribute *pAtt = pElement->FirstAttribute(); pAtt;
       pAtt = pAtt->NextAttribute()) {
...
//-----------------------------------------------------------------------------
static void AddUniqueElementsToList(CDmElement *pElement,
  ...
  for (int i = 0; i < nCount; ++i) {
    if (IsUniqueIdEqual(pElement->GetId(), list[i]->GetId())) {
      if (pElement != list[i]) {
        Warning(
            "Encountered two different elements with the same unique id!!\n");
...
// Replace references
//-----------------------------------------------------------------------------
static void BuildStartingDefList(CDmElement *pElement, CDmElement *pOldVersion,
                                 CDmElement *pNewVersion) {
  for (CDmAttribute *pAtt = pElement->FirstAttribute(); pAtt;
       pAtt = pAtt->NextAttribute()) {
    if (pAtt->GetType() == AT_ELEMENT) {
...
void CPCFFixApp::FixupPCFFile(CDmElement *pRoot) {
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    // Now replace all child references to startingDefs[i] with references to
    // pActualDef instead.
  ...
  if (nCount != nUniqueCount) {
    ...
    Warning(
        "Removed no duplicated particle systems. This file started out ok.\n");
  }
}
...
void CPCFFixApp::PrintHelp() {
  ...
  Msg("\t-i\t: Source .PCF file to fix up (eliminate copied particle system "
      "definitions.)\n");
...
int CPCFFixApp::Main() {
  ...
  CDmElement *pRoot;
  if (g_pDataModel->RestoreFromFile(pPCFFile, NULL, "pcf", &pRoot,
                                    CR_DELETE_NEW) == DMFILEID_INVALID) {
