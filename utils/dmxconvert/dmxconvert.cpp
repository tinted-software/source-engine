...
//-----------------------------------------------------------------------------
static SpewRetval_t DMXConvertOutputFunc(SpewType_t spewType,
                                         char const *pMsg) {
  printf(pMsg);
  fflush(stdout);
...
int CDmxConvertApp::Main() {
  ...
  DmxHeader_t header;
  CDmElement *pRoot;
  if (g_pDataModel->RestoreFromFile(pInFileName, NULL, pInFormat, &pRoot,
                                    CR_DELETE_NEW,
  ...
  if (!g_pDataModel->SaveToFile(pOutFileName, NULL, pOutEncoding, pOutFormat,
                                pRoot)) {
