...
void ExportCoreDispAllowedVertList(const CCoreDispInfo *pIn,
                                   ddispinfo_t *pOut) {
  ErrorIfNot(pIn->GetAllowedVerts().GetNumDWords() ==
                 sizeof(pOut->m_AllowedVerts) / 4,
...
void DispGetFaceInfo(mapbrush_t *pBrush) {
  ...
  if (pBrush->entitynum != 0) {
    char *pszEntityName =
        ValueForKey(&g_LoadingMap->entities[pBrush->entitynum], "classname");
    Error("Error: displacement found on a(n) %s entity - not supported (entity "
          "%d, brush %d)\n",
