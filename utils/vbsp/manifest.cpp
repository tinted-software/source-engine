...
//-----------------------------------------------------------------------------
ChunkFileResult_t
CManifest::LoadManifestMapKeyCallback(const char *szKey, const char *szValue,
...
ChunkFileResult_t CManifest::LoadManifestVMFCallback(CChunkFile *pFile,
  ...
  ChunkFileResult_t eResult =
      pFile->ReadChunk((KeyHandler_t)LoadManifestMapKeyCallback, pManifestMap);
...
ChunkFileResult_t CManifest::LoadManifestMapsCallback(CChunkFile *pFile,
  ...
  Handlers.AddHandler("VMF", (ChunkHandler_t)LoadManifestVMFCallback,
                      pManifest);
...
// Output :
//-----------------------------------------------------------------------------
ChunkFileResult_t CManifest::LoadCordonBoxCallback(CChunkFile *pFile,
                                                   Cordon_t *pCordon) {
...
// Output :
//-----------------------------------------------------------------------------
ChunkFileResult_t CManifest::LoadCordonBoxKeyCallback(const char *szKey,
                                                      const char *szValue,
                                                      BoundBox *pBox) {
...
// Output :
//-----------------------------------------------------------------------------
ChunkFileResult_t CManifest::LoadCordonKeyCallback(const char *szKey,
                                                   const char *szValue,
                                                   Cordon_t *pCordon) {
  if (!stricmp(szKey, "name")) {
...
// Output :
//-----------------------------------------------------------------------------
ChunkFileResult_t CManifest::LoadCordonCallback(CChunkFile *pFile,
                                                CManifest *pManifest) {
  ...
  CChunkHandlerMap Handlers;
  Handlers.AddHandler("box", (ChunkHandler_t)CManifest::LoadCordonBoxCallback,
                      (void *)&cordon);

  pFile->PushHandlers(&Handlers);
  ChunkFileResult_t eResult =
      pFile->ReadChunk((KeyHandler_t)LoadCordonKeyCallback, (void *)&cordon);
  pFile->PopHandlers();
...
//-----------------------------------------------------------------------------
ChunkFileResult_t CManifest::LoadCordonsKeyCallback(const char *szKey,
                                                    const char *szValue,
                                                    CManifest *pManifest) {
...
//
//-----------------------------------------------------------------------------
ChunkFileResult_t CManifest::LoadCordonsCallback(CChunkFile *pFile,
                                                 CManifest *pManifest) {
  CChunkHandlerMap Handlers;
  Handlers.AddHandler("cordon", (ChunkHandler_t)CManifest::LoadCordonCallback,
                      pManifest);

  pFile->PushHandlers(&Handlers);
  ChunkFileResult_t eResult =
      pFile->ReadChunk((KeyHandler_t)LoadCordonsKeyCallback, pManifest);
  pFile->PopHandlers();

  return (eResult);
}

extern ChunkFileResult_t LoadSolidCallback(CChunkFile *pFile,
...
CManifest::LoadManifestCordoningPrefsCallback(CChunkFile *pFile,
  ...
  //
  CChunkHandlerMap Handlers;
  Handlers.AddHandler("cordons", (ChunkHandler_t)CManifest::LoadCordonsCallback,
                      pDoc);
...
//-----------------------------------------------------------------------------
// Purpose: this function will load in all of the submaps belonging to this
// manifest,
//			except for the top level map, which is loaded
//separately.
// Input  : pMapFile - the top level map that was previously loaded
//			pszFileName - the absolute file name of the top level
//map file
...
bool CManifest::LoadSubMaps(CMapFile *pMapFile, const char *pszFileName) {
  ...
  for (int i = 0; i < m_Maps.Count(); i++) {
    //		if ( m_Maps[ i ]->m_bTopLevelMap == false )
    {
      ...
      sprintf(FileName, "%s%s", m_InstancePath,
              m_Maps[i]->m_RelativeMapFileName);
...
bool CManifest::LoadVMFManifestUserPrefs(const char *pszFileName) {
  ...
  if (eResult == ChunkFile_Ok) {
    ...
    //		Handlers.SetErrorHandler( ( ChunkErrorHandler_t
    //)CMapDoc::HandleLoadError, this);
...
void CManifest::CordonWorld() {
  ...
  for (int i = 0; i < g_MainMap->num_entities; i++) {
    if (i == 0) { // for world spawn, we look at brushes
      for (int nBrushNum = 0; nBrushNum < g_MainMap->entities[i].numbrushes;
           nBrushNum++) {
        ...
        for (int nCordon = 0; nCordon < m_Cordons.Count(); nCordon++) {
          ...
          for (int nBox = 0; nBox < m_Cordons[nCordon].m_Boxes.Count();
               nBox++) {
            if (m_Cordons[nCordon].m_Boxes[nBox].IsIntersectingBox(
                    g_MainMap->mapbrushes[nIndex].mins,
                    g_MainMap->mapbrushes[nIndex].maxs) == true) {
              bRemove = false;
              break;
            }
          }
        ...
        if (bRemove) {
          int nSize = (g_MainMap->entities[i].numbrushes - nBrushNum - 1) *
                      sizeof(g_MainMap->mapbrushes[0]);
          memmove(&g_MainMap->mapbrushes[nIndex],
                  &g_MainMap->mapbrushes[nIndex + 1], nSize);
          g_MainMap->entities[i].numbrushes--;
          nBrushNum--;
        }
      }
    } else if (&g_MainMap->entities[i] !=
               m_CordoningMapEnt) { // for all other entities, even if they
                                    // include brushes, we look at origin
      if (g_MainMap->entities[i].numbrushes == 0 &&
      ...
      for (int nCordon = 0; nCordon < m_Cordons.Count(); nCordon++) {
        ...
        for (int nBox = 0; nBox < m_Cordons[nCordon].m_Boxes.Count(); nBox++) {
          if (m_Cordons[nCordon].m_Boxes[nBox].ContainsPoint(
                  g_MainMap->entities[i].origin) == true) {
            bRemove = false;
            break;
          }
        }
