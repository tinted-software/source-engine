...
winding_t *BaseWindingForNode(node_t *node) {
  ...
  w = BaseWindingForPlane(g_MainMap->mapplanes[node->planenum].normal,
                          g_MainMap->mapplanes[node->planenum].dist);
...
void EmitClipPortalGeometry(node_t *pHeadNode, portal_t *pPortal, int iSrcArea,
  ...
  if (dp->m_FirstClipPortalVert + dp->m_nClipPortalVerts >=
    ...
    Error("MAX_MAP_PORTALVERTS (probably a broken areaportal near %.1f %.1f "
          "%.1f ",
          p->x, p->y, p->z);
  }
...
static void DisplayPortalError(portal_t *p, int viscontents) {
  ...
  Warning("\nFindPortalSide: Couldn't find a good match for which brush to "
          "assign to a portal near (%.1f %.1f %.1f)\n",
          center.x, center.y, center.z);
  Warning("Leaf 0 contents: %s\n", contents[0]);
  Warning("Leaf 1 contents: %s\n", contents[1]);
  Warning("viscontents (node 0 contents ^ node 1 contents): %s\n", contents[2]);
  Warning("This means that none of the brushes in leaf 0 or 1 that touches the "
          "portal has %s\n",
          contents[2]);
  Warning("Check for a huge brush enclosing the coordinates above that has "
          "contents %s\n",
          contents[2]);
