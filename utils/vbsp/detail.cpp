...
//			original face in any visible leaves that a clipped
//fragment falls 			into.
...
bool MergeFace_r(node_t *node, face_t *face, face_t *original) {
  ...
  if (node->planenum == PLANENUM_LEAF) {
    ...
    if (onwinding) {
      ...
      if (DotProduct(g_MainMap->mapplanes[face->planenum].normal,
                     g_MainMap->mapplanes[node->planenum].normal) > 0) {
...
face_t *MergeDetailTree(tree_t *worldtree, int brush_start, int brush_end) {
  ...
  detailbrushes = MakeBspBrushList(brush_start, brush_end, g_MainMap->map_mins,
                                   g_MainMap->map_maxs, ONLY_DETAIL);
  if (detailbrushes) {
