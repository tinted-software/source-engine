...
================
*/
qboolean PlaneEqual(plane_t *p, Vector &normal, vec_t dist, float normalEpsilon,
                    float distEpsilon) {
#if 1
...
//-----------------------------------------------------------------------------
int CMapFile::PlaneFromPoints(const Vector &p0, const Vector &p1,
                              const Vector &p2) {
  Vector t1, t2, normal;
  vec_t dist;
...
//func_areaportal. 			THIS ROUTINE MAY ONLY BE USED DURING ENTITY LOADING.
...
void CMapFile::MoveBrushesToWorldGeneral(entity_t *mapent) {
  ...
  for (i = 1; i < num_entities; i++) {
    if (entities[i].firstbrush <
        mapent->firstbrush) // if we use <=, then we'll remap the passed in ent,
...
ChunkFileResult_t
LoadDispOffsetNormalsKeyCallback(const char *szKey, const char *szValue,
                                 mapdispinfo_t *pMapDispInfo) {
  if (!strnicmp(szKey, "row", 3)) {
    char szBuf[MAX_KEYVALUE_LEN];
    strcpy(szBuf, szValue);
...
LoadOverlayDataTransitionKeyCallback(const char *szKey, const char *szValue,
...
ChunkFileResult_t CMapFile::LoadEntityCallback(CChunkFile *pFile, int nParam) {
  ...
  //
  CChunkHandlerMap Handlers;
  Handlers.AddHandler("solid", (ChunkHandler_t)::LoadSolidCallback,
                      &LoadEntity);
...
// Output : returns the KeyValues of the file, NULL if the file could not be
// read.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: This function will attempt to find a full path given the base and
// relative names. Input  : pszBaseFileName - the base file that referenced this
// instance
//			pszInstanceFileName - the relative file name of this
//instance
// Output : Returns true if it was able to locate the file
//			pszOutFileName - the full path to the file name if
//located
//-----------------------------------------------------------------------------
bool CMapFile::DeterminePath(const char *pszBaseFileName,
  ...
  V_SetExtension(szInstanceFileNameFixed, ".vmf",
                 sizeof(szInstanceFileNameFixed));
  ...
  // second, try to find the master 'maps' directory and make it relative from
  // that
  strcpy(pszOutFileName, pszBaseFileName);
  V_StripFilename(pszOutFileName);
  ...
  if (m_InstancePath[0] != 0) {
    sprintf(szInstanceFileNameFixed, "%s%s", m_InstancePath,
            pszInstanceFileName);

    if (g_pFullFileSystem->FileExists(szInstanceFileNameFixed, "GAME")) {
      char FullPath[MAX_PATH];
      g_pFullFileSystem->RelativePathToFullPath(szInstanceFileNameFixed, "GAME",
...
//-----------------------------------------------------------------------------
// Purpose: this function will check the main map for any func_instances.  It
// will
//			also attempt to load in the gamedata file for instancing
//remapping help.
// Input  : none
// Output : none
//-----------------------------------------------------------------------------
void CMapFile::CheckForInstances(const char *pszFileName) {
  if (this != g_MainMap) { // all sub-instances will be appended to the main map
                           // master list as they are read in
  ...
  g_pFullFileSystem->RelativePathToFullPath("gameinfo.txt", "MOD", GameInfoPath,
                                            sizeof(GameInfoPath));
  ...
  if (!g_pFullFileSystem->RelativePathToFullPath(
          GameDataFile, "EXECUTABLE_PATH", FDGPath, sizeof(FDGPath))) {
    if (!g_pFullFileSystem->RelativePathToFullPath(GameDataFile, NULL, FDGPath,
                                                   sizeof(FDGPath))) {
  ...
  // this list will grow as instances are merged onto it.  sub-instances are
  // merged and automatically done in this processing.
  for (int i = 0; i < num_entities; i++) {
    ...
    if (!strcmp(pEntity, "func_instance")) {
      ...
      if (pInstanceFile[0]) {
        ...
        if (bLoaded == false) {
          ...
          ColorSpewMessage(SPEW_ERROR, &red,
                           "Could not open instance file %s\n", pInstanceFile);
        }
      }
...
//-----------------------------------------------------------------------------
// Purpose: this function will do all of the necessary work to merge the
// instance
//			into the main map.
...
void CMapFile::MergeInstance(entity_t *pInstanceEntity, CMapFile *Instance) {
  ...
  Msg("Instance Remapping: O:( %g, %g, %g ) A:( %g, %g, %g )\n", OriginOffset.x,
      OriginOffset.y, OriginOffset.z, angles.x, angles.y, angles.z);
...
//			InstanceMatrix - the translation / rotation matrix of
//the instance
// Output : none
//-----------------------------------------------------------------------------
void CMapFile::MergePlanes(entity_t *pInstanceEntity, CMapFile *Instance,
...
// Output : none
//-----------------------------------------------------------------------------
void CMapFile::MergeBrushes(entity_t *pInstanceEntity, CMapFile *Instance,
                            Vector &InstanceOrigin, QAngle &InstanceAngle,
                            matrix3x4_t &InstanceMatrix) {
  int max_brush_id = 0;
  ...
  for (int i = 0; i < Instance->nummapbrushes; i++) {
    ...
    if (i < Instance->entities[0].numbrushes ||
        (brush->contents & CONTENTS_LADDER) !=
            0) { // world spawn brushes as well as ladders we physically move
...
// Output : none
//-----------------------------------------------------------------------------
void CMapFile::MergeBrushSides(entity_t *pInstanceEntity, CMapFile *Instance,
                               Vector &InstanceOrigin, QAngle &InstanceAngle,
                               matrix3x4_t &InstanceMatrix) {
  int max_side_id = 0;
  ...
  for (int i = 0; i < Instance->nummapbrushsides; i++) {
    ...
    // The planes got merged & remapped.  So you need to search for the output
    // plane index on each side NOTE: You could optimize this by saving off an
    // index map in MergePlanes
    side->planenum = FindFloatPlane(Instance->mapplanes[side->planenum].normal,
                                    Instance->mapplanes[side->planenum].dist);
    side->id += max_side_id;
    ...
    if (!bNeedsTranslation) { // check for sides that are part of the world
                              // spawn - those need translating
    ...
    if (!bNeedsTranslation) { // sides for ladders are outside of the world
                              // spawn, but also need translating
      for (int j = Instance->entities[0].numbrushes;
           j < Instance->nummapbrushes; j++) {
        int loc = Instance->mapbrushes[j].original_sides - Instance->brushsides;
    ...
    if (bNeedsTranslation) { // we only want to do the adjustment on world spawn
                             // brushes, not entity brushes
      if (side->winding) {
      ...
      VectorRotate(Instance->side_brushtextures[i].UAxis, InstanceMatrix,
                   bt.UAxis);
      VectorRotate(Instance->side_brushtextures[i].VAxis, InstanceMatrix,
                   bt.VAxis);
      bt.shift[0] -=
          InstanceOrigin.Dot(bt.UAxis) / bt.textureWorldUnitsPerTexel[0];
      bt.shift[1] -=
          InstanceOrigin.Dot(bt.VAxis) / bt.textureWorldUnitsPerTexel[1];

      if (!onlyents) {
        side->texinfo = TexinfoForBrushTexture(&mapplanes[side->planenum], &bt,
                                               vec3_origin);
      }
    }
...
//-----------------------------------------------------------------------------
// Purpose: this function will look for replace parameters in the function
// instance
//			to see if there is anything in the epair that should be
//replaced.
// Input  : pPair - the epair with the value
//			pInstanceEntity - the func_instance that may ahve
//replace keywords
...
void CMapFile::ReplaceInstancePair(epair_t *pPair, entity_t *pInstanceEntity) {
  ...
  for (epair_t *epInstance = pInstanceEntity->epairs; epInstance != NULL;
       epInstance = epInstance->next) {
    if (strnicmp(epInstance->key, INSTANCE_VARIABLE_KEY,
                 strlen(INSTANCE_VARIABLE_KEY)) == 0) {
      ...
      *ValuePos = 0;
      ...
      if (!V_StrSubst(Value, InstanceVariable, ValuePos, NewValue,
                      sizeof(NewValue), false)) {
        Overwritten = true;
        break;
      }
    }
...
// Output : none
//-----------------------------------------------------------------------------
void CMapFile::MergeEntities(entity_t *pInstanceEntity, CMapFile *Instance,
                             Vector &InstanceOrigin, QAngle &InstanceAngle,
                             matrix3x4_t &InstanceMatrix) {
  int max_entity_id = 0;
  char temp[2048];
  ...
  for (int i = 0; i < Instance->num_entities; i++) {
    ...
    if (strcmpi(pEntity, "worldspawn") == 0) {
      ...
      // search for variables coming from the func_instance to replace inside of
      // the instance this is done before entity fixup, so fixup may occur on
      // the replaced value.  Not sure if this is a desired order of operation
      // yet.
...
#endif // #ifdef MERGE_INSTANCE_DEBUG_INFO
      GDclass *EntClass = GD.BeginInstanceRemap(pEntity, NameFixup,
                                                InstanceOrigin, InstanceAngle);
      if (EntClass) {
      ...
      if (strcmpi(pEntity, "func_simpleladder") ==
          0) { // hate having to do this, but the key values are so screwed up
        AddLadderKeys(entity);
        /*				Vector	vInNormal, vOutNormal;

                                        vInNormal.x = FloatForKey( entity,
           "normal.x" ); vInNormal.y = FloatForKey( entity, "normal.y" );
                                        vInNormal.z = FloatForKey( entity,
           "normal.z" ); VectorRotate( vInNormal, InstanceMatrix, vOutNormal );

                                        Q_snprintf( temp, sizeof( temp ), "%f",
           vOutNormal.x ); SetKeyValue( entity, "normal.x", temp );

                                        Q_snprintf( temp, sizeof( temp ), "%f",
           vOutNormal.y ); SetKeyValue( entity, "normal.y", temp );

                                        Q_snprintf( temp, sizeof( temp ), "%f",
           vOutNormal.z ); SetKeyValue( entity, "normal.z", temp );*/
      }
    }

#ifdef MERGE_INSTANCE_DEBUG_INFO
    Msg("Instance Entity %d remapped to %d\n", i, num_entities + i);
    Msg("   FirstBrush: from %d to %d\n", Instance->entities[i].firstbrush,
        entity->firstbrush);
    Msg("   KV Pairs:\n");
  ...
  // search for variables coming from the func_instance to replace inside of the
  // instance this is done before connection fix up, so fix up may occur on the
  // replaced value.  Not sure if this is a desired order of operation yet.
  for (CConnectionPairs *Connection = Instance->m_ConnectionPairs; Connection;
...
// Output : none
//-----------------------------------------------------------------------------
void CMapFile::MergeOverlays(entity_t *pInstanceEntity, CMapFile *Instance,
                             Vector &InstanceOrigin, QAngle &InstanceAngle,
                             matrix3x4_t &InstanceMatrix) {
  for (int i = Instance->m_StartMapOverlays; i < g_aMapOverlays.Count(); i++) {
    Overlay_Translate(&g_aMapOverlays[i], InstanceOrigin, InstanceAngle,
                      InstanceMatrix);
  }
  for (int i = Instance->m_StartMapWaterOverlays;
       i < g_aMapWaterOverlays.Count(); i++) {
    Overlay_Translate(&g_aMapWaterOverlays[i], InstanceOrigin, InstanceAngle,
                      InstanceMatrix);
  }
}
...
bool LoadMapFile(const char *pszFileName) {
  ...
  if ((eResult == ChunkFile_Ok) || (eResult == ChunkFile_EOF)) {
    ...
    for (int i = 0; i < g_MainMap->entities[0].numbrushes; i++) {
      ...
      AddPointToBounds(g_LoadingMap->mapbrushes[i].mins, g_LoadingMap->map_mins,
                       g_LoadingMap->map_maxs);
      AddPointToBounds(g_LoadingMap->mapbrushes[i].maxs, g_LoadingMap->map_mins,
                       g_LoadingMap->map_maxs);
    }
    ...
    qprintf("size: %5.0f,%5.0f,%5.0f to %5.0f,%5.0f,%5.0f\n",
            ...
            g_LoadingMap->map_mins[2], g_LoadingMap->map_maxs[0],
            g_LoadingMap->map_maxs[1], g_LoadingMap->map_maxs[2]);
...
// Output : ChunkFileResult_t
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapFile::LoadSideCallback(CChunkFile *pFile,
                                             LoadSide_t *pSideInfo) {
  if (nummapbrushsides == MAX_MAP_BRUSHSIDES) {
    g_MapError.ReportError("MAX_MAP_BRUSHSIDES");
  }
...
//format: 				<target>,<input>,<parameter>,<delay>,<times to fire> 			pLoadEntity -
//Structure to receive loaded entity information.
// Output : ChunkFileResult_t
//-----------------------------------------------------------------------------
ChunkFileResult_t LoadConnectionsKeyCallback(const char *szKey,
                                             const char *szValue,
...
ChunkFileResult_t
CMapFile::LoadConnectionsKeyCallback(const char *szKey, const char *szValue,
                                     LoadEntity_t *pLoadEntity) {
  //
...
// Output : ChunkFileResult_t
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapFile::LoadSolidCallback(CChunkFile *pFile,
                                              LoadEntity_t *pLoadEntity) {
