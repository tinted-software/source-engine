...
static const char *FindDependentMaterial(const char *pMaterialName,
  ...
  for (int i = 0; s_pDependentMaterialVar[i][0]; ++i) {
    ...
    if (!Q_stricmp(pDependentMaterialName, pMaterialName)) {
      Warning("Material %s is depending on itself through materialvar %s! "
              "Ignoring...\n",
...
void CreateDefaultCubemaps(bool bHDR) {
  ...
  if (!skyboxMaterialName) {
    if (s_DefaultCubemapNames.Count()) {
      Warning("This map uses env_cubemap, and you don't have a skybox, so no "
...
void Cubemap_InitCubemapSideData(void) {
  ...
  CUtlRBTree<CubemapInfo_t> lookup(0, g_MainMap->nummapbrushsides,
                                   CubemapLessFunc);
...
void Cubemap_AttachDefaultCubemapToSpecularSides(void) {
  ...
  for (int iSide = 0; iSide < g_MainMap->nummapbrushsides; ++iSide) {
    ...
    int iCubemap = Cubemap_FindClosestCubemap(
        g_MainMap->entities[currentEntity].origin, pSide);
    if (iCubemap == -1)
      continue;
