...
void SynthesizeTexture(FloatBitMap_t &output, FloatBitMap_t const &input) {
  ...
  for (int level = min(input_pyramid.m_nLevels, output_pyramid.m_nLevels) - 2;
       level >= 0; level--) {
    ...
    if ((input.Width < NEIGHBORHOOD_SIZE * 3) ||
    ...
    for (int yloop = 0; yloop < output.Height; yloop++) {
      ...
      for (int xo = 0; xo < output.Width; xo++) {
        ...
        for (int nblk_y = NEIGHBORHOOD_SIZE; nblk_y < input.Height; nblk_y++)
          for (int nblk_x = NEIGHBORHOOD_SIZE;
               nblk_x < input.Width - NEIGHBORHOOD_SIZE; nblk_x++) {
            float our_error = 0;
            // now, compare this block to the neighborhood around our output
            // pixel
