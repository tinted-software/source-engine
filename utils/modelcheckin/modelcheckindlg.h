...
#if !defined(                                                                  \
    AFX_MODELCHECKINDLG_H__75541530_E85C_4FAB_8C76_6EE68353E2E2__INCLUDED_)
...
private:
  enum ProjectType_t { PROJECT_ERROR = -1, PROJECT_HL2 = 0, PROJECT_TF2 };

  bool CheckInfo();
  bool SetCurrentSSProject(ProjectType_t project, char const *pRelativeDir);
  ProjectType_t ComputeRelativeFileName(char const *pInFile,
                                        char *pRelativeFile);
  ...
  void PerformCheckoutCommand(ProjectType_t project, char const *pRelativeDir,
                              char const *pDestPath, char const *pFileName);
  void PerformCheckinCommand(ProjectType_t project, char const *pRelativeDir,
                             char const *pDestPath, char const *pFileName);
};
