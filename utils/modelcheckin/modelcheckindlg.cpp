...
void CModelCheckInDlg::StoreStateIntoRegistry() {
  AfxGetApp()->WriteProfileString(MDL_CHECKOUT_REG_CLASS, MDL_CHECKOUT_REG_USER,
                                  m_UserName);
  AfxGetApp()->WriteProfileString(
      MDL_CHECKOUT_REG_CLASS, MDL_CHECKOUT_REG_HL2_PATH, m_HL2GameDirectory);
  AfxGetApp()->WriteProfileString(
      MDL_CHECKOUT_REG_CLASS, MDL_CHECKOUT_REG_TF2_PATH, m_TF2GameDirectory);
}
...
void CModelCheckInDlg::RestoreStateFromRegistry() {
  ...
  m_HL2GameDirectory = AfxGetApp()->GetProfileString(
      MDL_CHECKOUT_REG_CLASS, MDL_CHECKOUT_REG_HL2_PATH, "u:/hl2/hl2");
  m_TF2GameDirectory = AfxGetApp()->GetProfileString(
      MDL_CHECKOUT_REG_CLASS, MDL_CHECKOUT_REG_TF2_PATH, "u:/hl2/tf2");
}
...
static char *s_ppProjectDir[] = {"$/HL2/release/dev/hl2/",
                                 "$/TF2/release/dev/tf2/"};
...
void CModelCheckInDlg::PerformCheckoutCommand(ProjectType_t project,
                                              char const *pRelativeDir,
                                              char const *pDestPath,
  ...
  for (currExtension = 0; s_ppExtensions[currExtension][0]; ++currExtension) {
    ...
    sprintf(buf, "ss filetype %s%s/%s%s -O- -y%s\n", s_ppProjectDir[project],
            pRelativeDir, pFileName, s_ppExtensions[currExtension],
    ...
    retVal = system(buf);
    if (retVal > 0) {
      len += sprintf(&error[len],
                     "*** SourceSafe error attempting to check out \"%s%s\"\n",
...
void CModelCheckInDlg::PerformCheckinCommand(ProjectType_t project,
  ...
  int currExtension;
  for (currExtension = 0; s_ppExtensions[currExtension][0]; ++currExtension) {
    // Check for the existence of the file on disk. If it's not there, don't
    // bother
    sprintf(buf, "%s/%s%s", pDestPath, pFileName,
    ...
    if (retVal > 0) {
      ...
      sprintf(buf, "ss add %s%s/%s%s -GL%s -O- -y%s\n", s_ppProjectDir[project],
              pRelativeDir, pFileName, s_ppExtensions[currExtension], pDestPath,
              (char const *)m_UserName);
      int retVal = system(buf);
      if (retVal > 0) {
        len += sprintf(&error[len],
                       "SourceSafe error attempting to add \"%s%s\"\n",
                       pFileName, s_ppExtensions[currExtension]);
      }
    } else {
      ...
      sprintf(buf, "ss checkin %s%s/%s%s -GL%s -O- -y%s\n",
              s_ppProjectDir[project], pRelativeDir, pFileName,
              s_ppExtensions[currExtension], pDestPath,
              (char const *)m_UserName);
      retVal = system(buf);
      if (retVal > 0) {
        len += sprintf(&error[len],
                       "SourceSafe error attempting to check in \"%s%s\"\n",
                       pFileName, s_ppExtensions[currExtension]);
      }
    }
  }
...
CModelCheckInDlg::ProjectType_t
CModelCheckInDlg::ComputeRelativeFileName(char const *pInFile,
                                          char *pRelativeFile) {
...
CModelCheckInDlg::ProjectType_t
CModelCheckInDlg::GetFileNames(char const *pTitle, char *&pRelativePath,
  ...
  CString temp = AfxGetApp()->GetProfileString(
      MDL_CHECKOUT_REG_CLASS, MDL_CHECKOUT_REG_LAST_PATH, pCwd);
  ...
  ProjectType_t projectType =
      ComputeRelativeFileName(dlg.GetPathName(), relativeFile);
