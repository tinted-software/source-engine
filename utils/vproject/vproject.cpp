...
void SetVProject(const char *pProjectName) {
  ...
  Sys_SetRegistryString(
      ...
      //Manager\\Environment\\VProject",
      "HKEY_CURRENT_USER\\Environment\\VProject" pGamedir);

  DWORD result;
  SendMessageTimeout(HWND_BROADCAST, WM_SETTINGCHANGE, 0,
                     (LPARAM) "Environment", SMTO_ABORTIFHUNG, 0, &result);
}
...
void ModifyVProject(int index, const char *pName, const char *pGamedir) {
  ...
  if (!pName || !pName[0]) {
    ...
    if (g_numProjects - index - 1 > 0) {
      ...
      memcpy(&g_projects[index], &g_projects[index + 1],
             (g_numProjects - index - 1) * sizeof(project_t));
    }
...
void SaveRegistryValues() {
  ...
  for (i = 0; i < MAX_PROJECTS; i++) {
    ...
    len = _snprintf(valueBuff, sizeof(valueBuff), "\"%s\" \"%s\"", pProjectName,
                    pGamedir);
    if (len == -1) {
      // kill it
      valueBuff[0] = '\0';
...
//
//-----------------------------------------------------------------------------
BOOL CALLBACK ModifyDlg_Proc(HWND hWnd, UINT message, WPARAM wParam,
                             LPARAM lParam) {
  size_t len;
  int width;
  int height;
  ...
  case WM_INITDIALOG:
    ...
    SetWindowPos(hWnd, NULL, (width - (rect.right - rect.left)) / 2,
                 (height - (rect.bottom - rect.top)) / 2, 0, 0,
  ...
  case WM_COMMAND:
    ...
    case IDC_OK:
      GetDlgItemText(hWnd, IDC_MODIFY_PROJECT, g_project_name,
                     sizeof(g_project_name));
      GetDlgItemText(hWnd, IDC_MODIFY_GAMEDIR, g_project_gamedir,
                     sizeof(g_project_gamedir));
...
BOOL ModifyDlg_Open() {
  ...
  result = DialogBox(g_hInstance, MAKEINTRESOURCE(IDD_VPROJECT), g_hWnd,
                     (DLGPROC)ModifyDlg_Proc);
...
void ShowPopupMenu(HWND hWnd, bool bUseCachedMenuPos) {
  ...
  for (int nIndex = 0; nIndex < g_numProjects; nIndex++) {
    ...
    InsertMenu(g_hMenu, nIndex, MF_BYPOSITION | MF_STRING, nIndex + 1,
               szMenuItem);
  }
...
void TrayMessageHandler(HWND hWnd, UINT uMessageID) {
  ...
  case ID_TRAY_MODIFYVPROJECT:
    ...
    if (g_nActiveVProject) {
      ...
      if (ModifyDlg_Open()) {
        ModifyVProject(g_nActiveVProject - 1, g_project_name,
                       g_project_gamedir);
        SaveRegistryValues();
      }
      ShowPopupMenu(hWnd, true);
...
//
//-----------------------------------------------------------------------------
LRESULT CALLBACK Main_WndProc(HWND hWnd, UINT message, WPARAM wParam,
                              LPARAM lParam) {
  switch (message) {
  case WM_DESTROY:
    PostQuitMessage(0);
...
bool Startup() {
  ...
  g_hWnd =
      CreateWindow(VPROJECT_CLASSNAME, 0, WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, 0,
