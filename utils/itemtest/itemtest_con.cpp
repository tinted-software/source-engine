...
int CItemTestConApp::Main() {
  ...
  // If launched with -batch stay in console app and run from parameters
  ...
  if (bListMats || FindParam(kBatch)) {
    ...
    // If launched without -batch then launch the .exe with the same name in the
    // same directory as this .com executable
    RunExe();
  }
