...
char *CItemTestApp::s_pszFlags[][4] = {
    ...
    {"-s", "-steamid", NULL,
     "Print the itemtest steam id for the current user or \"<unknown>\" and "
     "exit"},
    {"-b", "-batch", NULL,
     "Run in batch mode, i.e. no GUI, all parameters must be specified on the "
     "command line."},
    {"-c", "-class", "<class>",
     "Specify the class of the item.  One of: demo, engineer, heavy, medic, "
     "pyro, scout, sniper, soldier, spy"},
    {"-n", "-name", "<name>", "Specify the name of the item"},
    {"-d", "-dev", NULL, "Turn on dev mode"},
    {"-nop4", "-nop4", NULL,
     "If -dev mode is specified optionally turn off perforce.  NOTE: Perforce "
     "is only enabled in dev mode."},
    {"-as", "-autoskin", NULL,
     "Turn on auto skinning of the geometry to bip_head."},
    {"-l", "-lod", "<file>",
     "Specify the LOD, can be specified multiple times, first is LOD0, 2nd is "
     "LOD1, etc.."},
    {"-lm", "-listmats", NULL,
     "If some lod files are specified and this option is specified, the names "
     "of the materials found in the LOD files are printed to stdout and the "
     "program exits, all texture flags are ignored.  The material names are "
     "the arguments needed to -mat"},
    {"-m", "-mat", "<name>",
     "Specify a material.  <name> is the name of the material from the "
     "geometry file"},
    {"-mt", "-mattype", "<type>",
     "Category of previously specified material, one of: primary, secondary, "
     "duplicate_primary, duplicate_secondary"},
    {"-t", "-tex", "<file>",
     "Specify a texture to use with the previously specified material"},
    {"-tt", "-textype", "<type>",
     "Category of previously specified texture, one of: common, red, blue, "
     "normal"},
    {"-at", "-alphatype", "<type>",
     "Category of alpha data of previously specified texture, one of: none, "
     "transparency, paintable, spec"},
    {"-v", "-view", NULL, "Run hlmv on the compiled model if successful"},
    {"-ex", "-explore", NULL,
     "Open explorer window on the compiled zip if successful"},
...
bool CItemTestApp::PreInit() {
...

  {
    ...
    // See if we have to do special initialization because TF & the SourceSDK
    // are configured oddly GetSourceSDKFromExe() will fail if it's not that
    // specific configuration and so normal initialization will proceed in that
    // case

    // TODO: These are specific values for TF but hopefully this code will only
    // be needed for TF & SourceSDK
    const char *pszMod = "tf";
    const int nAppId = 440;

    if (CItemUpload::GetSourceSDKFromExe(sSourceSDKPath, sSourceSDKBin) &&
        CItemUpload::GetSteamAppInstallLocation(sSteamAppInstallLocation,
                                                nAppId)) {
      char szGameDir[MAX_PATH] = "";
      V_ComposeFileName(sSteamAppInstallLocation.String(), pszMod, szGameDir,
                        ARRAYSIZE(szGameDir));
  ...
  V_ExtractFilePath(pszGameInfoPath, szGameInfoParent,
                    ARRAYSIZE(szGameInfoParent));
  char szPlatformDir[MAX_PATH] = "";
  V_ComposeFileName(szGameInfoParent, "platform", szPlatformDir,
                    ARRAYSIZE(szPlatformDir));

  g_pFullFileSystem->AddSearchPath(szPlatformDir, "PLATFORM");

  if (!sSourceSDKPath.IsEmpty()) {
    V_ComposeFileName(sSourceSDKPath.String(), "platform", szPlatformDir,
                      ARRAYSIZE(szPlatformDir));
    g_pFullFileSystem->AddSearchPath(szPlatformDir, "PLATFORM");
  }
...
bool CItemTestApp::ProcessCommandLine(CAsset *pAsset, bool bDoListMats) {
  ...
  if (pszOutput) {
    ...
    if (g_pFullFileSystem->RelativePathToFullPath(
            "workshop", "GAME", pszWorkshopPath, sizeof(pszWorkshopPath)) &&
        !sFileName.IsEmpty()) {
      sFileName += ".zip";
      V_ComposeFileName(pszWorkshopPath, sFileName, pszWorkshopPath,
                        sizeof(pszWorkshopPath));
  ...
  for (int i = 0; i < nParamCount; ++i) {
    ...
    if (!V_stricmp(pszFlag, s_pszFlags[kLod][kShortFlag]) ||
        !V_stricmp(pszFlag, s_pszFlags[kLod][kLongFlag])) {
      if (i + 1 < nParamCount) {
        const char *pszLod = pCmdLine->GetParm(i + 1);
        pAsset->AddTargetDMX(pszLod);
      } else {
        Warning("Error! Command line switch \"%s\" specified without parameter "
                "value\n",
                pszFlag);
      }
    }
  ...
  if (bDoListMats) {
    ...
    if (pAsset->TargetDMXCount() <= 0) {
      ...
      if (nTargetVMTCount <= 0) {
        Warning(
            "Error! %s specified but no materials found in specified LODs\n",
  ...
  for (int i = 0; i < nParamCount; ++i) {
    const char *pszFlag = pCmdLine->GetParm(i);
    if (!V_stricmp(pszFlag, s_pszFlags[kMat][kShortFlag]) ||
        !V_stricmp(pszFlag, s_pszFlags[kMat][kLongFlag])) {
      if (i + 1 < nParamCount) {
        const char *pszMatName = pCmdLine->GetParm(i + 1);
        pTargetVmt = pAsset->FindOrAddMaterial(
            pszMatName, CItemUpload::Manifest()->GetDefaultMaterialType());
        nTexParmIndex = -1;
        bTexNormal = false;
      } else {
        Warning("Error! Command line switch \"%s\" specified without parameter "
      ...
      if (nMatType != kInvalidMaterialType) {
        ...
        Warning(
            "Error! Invalid Parameter Value: \"%s\" \"%s\", expected one of "
            "primary, secondary, duplicate_primary, duplicate_secondary\n",
            pszFlag, pszMatType);
      }
    } else if (!V_stricmp(pszFlag, s_pszFlags[kTex][kShortFlag]) ||
               !V_stricmp(pszFlag, s_pszFlags[kTex][kLongFlag])) {
      if (!pTargetVmt) {
        Warning("Error! No -mat specified before \"%s\"\n", pszFlag);
        continue;
      }
      ...
      if (pTargetVmt->GetDuplicate()) {
        Warning("Error! Previous material specified as duplicate before "
                "\"%s\", ignoring\n",
                pszFlag);
        continue;
      }
      ...
      if (StringHasPrefix(pszTexType, "C")) {
        pTargetVmt->SetTargetVTF(
            "_color", pszTex); // Comes from texture_types in manifest
      } else if (StringHasPrefix(pszTexType, "N")) {
        pTargetVmt->SetTargetVTF(
            "_normal", pszTex); // Comes from texture_types in manifest
        bTexNormal = true;
      } else if (StringHasPrefix(pszTexType, "R")) {
        pTargetVmt->SetTargetVTF(
            "_color", pszTex,
            CItemUpload::Manifest()->GetMaterialSkin(
                "red")); // Comes from texture_types in manifest
      } else if (StringHasPrefix(pszTexType, "B")) {
        pTargetVmt->SetTargetVTF(
            "_color", pszTex,
            CItemUpload::Manifest()->GetMaterialSkin(
                "blue")); // Comes from texture_types in manifest
      }
      ...
      if (StringHasPrefix(pszTexType, "N")) {
        ...
        if (bTexNormal) {
          Warning("Error! Command line switch \"%s\" \"%s\" specified after a "
                  "\"normal\" texture, only applies to color textures\n",
                  pszFlag, pszTexType);
          continue;
        } else {
...
void CItemTestApp::PrintHelp() {
  ...
  sIndent = "   ";
  sLine = "  "; // One less space than indent to start because space gets added
                // below
  ...
  for (int i = 0; i < kFlagsCount; ++i) {
    ...
    nLineLen[i] += s_pszFlags[i][kArgDesc]
                       ? V_strlen(s_pszFlags[i][kArgDesc]) + 1
  ...
  for (int i = 0; i < kFlagsCount; ++i) {
    ...
    sFlags.sprintf(sFmt, s_pszFlags[i][kShortFlag], s_pszFlags[i][kLongFlag],
                   s_pszFlags[i][kArgDesc] ? " " : "",
    ...
    V_SplitString2(s_pszFlags[i][kFlagDesc], pszSeparators,
                   ARRAYSIZE(pszSeparators), outStrings);
    for (int j = 0; j < outStrings.Count(); ++j) {
      const int nSpaceLeft = nLineLength - sLine.Length();
      const int nWordWidth = V_strlen(outStrings[j]) + 1;
...
const char *CItemTestApp::ParmValue(Flags_t nFlag) {
  const char *pszParmValue =
      CommandLine()->ParmValue(s_pszFlags[nFlag][kShortFlag]);
  if (pszParmValue)
    return pszParmValue;
