...
int CItemTestVGUIApp::Main() {
  ...
  if (!g_hMainFrame) {
    ...
    CItemUploadWizard *pItemUploadWizard =
        new CItemUploadWizard(g_pTopPanel, "Item Upload Wizard");
  ...
  if (!g_hConsoleDialog) {
    if (g_pTopPanel) {
      CConsoleDialogNew *pConsoleDialog =
          g_hMainFrame ? new CConsoleDialogNew(g_hMainFrame, "console")
      ...
      if (pConsoleDialog) {
        ...
        g_hConsoleDialog->Print(
            ": The only commands available are 'quit' and 'help'\n");
      }
    }
  }
