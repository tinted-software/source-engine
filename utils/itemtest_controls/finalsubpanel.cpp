...
//
//-----------------------------------------------------------------------------
CFinalSubPanel::CFinalSubPanel(vgui::Panel *pParent, const char *pszName,
  ...
  {
    m_pHLMVButton =
        new vgui::Button(this, "hlmvButton", "#hlmvButton", this, "Hlmv");
...

  {
    m_pExploreMaterialContentButton = new vgui::Button(
        this, "exploreMaterialContentButton", "#exploreMaterialContentButton",
        this, "ExploreMaterialContent");
    m_pExploreMaterialContentLabel = new vgui::Label(
        this, "exploreMaterialContentLabel", "#exploreMaterialContentLabel");

    m_pPanelListPanel->AddItem(m_pExploreMaterialContentButton,
                               m_pExploreMaterialContentLabel);
  }

  {
    m_pExploreModelContentButton = new vgui::Button(
        this, "exploreModelContentButton", "#exploreModelContentButton", this,
        "ExploreModelContent");
    m_pExploreModelContentLabel = new vgui::Label(
        this, "exploreModelContentLabel", "#exploreModelContentLabel");

    m_pPanelListPanel->AddItem(m_pExploreModelContentButton,
                               m_pExploreModelContentLabel);
  }

  {
    m_pExploreMaterialGameButton = new vgui::Button(
        this, "exploreMaterialGameButton", "#exploreMaterialGameButton", this,
        "ExploreMaterialGame");
    m_pExploreMaterialGameLabel = new vgui::Label(
        this, "exploreMaterialGameLabel", "#exploreMaterialGameLabel");

    m_pPanelListPanel->AddItem(m_pExploreMaterialGameButton,
                               m_pExploreMaterialGameLabel);
  }

  {
    m_pExploreModelGameButton =
        new vgui::Button(this, "exploreModelGameButton",
                         "#exploreModelGameButton", this, "ExploreModelGame");
    m_pExploreModelGameLabel = new vgui::Label(this, "exploreModelGameLabel",
                                               "#exploreModelGameLabel");

    m_pPanelListPanel->AddItem(m_pExploreModelGameButton,
                               m_pExploreModelGameLabel);
  }
...
//
//-----------------------------------------------------------------------------
void CFinalSubPanel::UpdateGUI() { UpdateStatus(); }
...
// 1. Gather data from user specified sources into properly named & organized
// content, make VMT's & QC
...
void CFinalSubPanel::OnHlmv() {
  ...
  V_ExtractFilePath(sBinDir.String(), szBinLaunchDir,
                    ARRAYSIZE(szBinLaunchDir));
...
void CFinalSubPanel::OnExplore(bool bMaterial, bool bContent) {
  ...
  const char *pszPrefix =
      bMaterial ? (bContent ? "materialsrc" : "materials") : NULL;
...
bool CFinalSubPanel::GetHlmvCmd(CFmtStrMax &sHlmvCmd) {
  ...
  if (CItemUpload::GetDevMode()) {
    ...
    if (CItemUpload::GetVProjectDir(sVProjectDir)) {
      sHlmvCmd.sprintf("\"%s\\hlmv.exe\" -allowdebug -game \"%s\" -nop4 \"%s\"",
                       sBinDir.Get(), sVProjectDir.Get(), sMdlPath.Get());
    } else {
