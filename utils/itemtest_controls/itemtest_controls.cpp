...
//
//=============================================================================
CStatusLabel::CStatusLabel(vgui::Panel *pPanel, const char *pszName,
                           bool bValid /* = false */)
    : BaseClass(pPanel, pszName, ""), m_bValid(bValid),
      m_cValid(0, 192, 0, 255), m_cInvalid(192, 0, 0, 255) {
...
//
//-----------------------------------------------------------------------------
bool CStatusLabel::GetValid() const { return m_bValid; }
...
//
//=============================================================================
CItemUploadSubPanel::CItemUploadSubPanel(vgui::Panel *pParent,
                                         const char *pszName,
                                         const char *pszNextName)
  ...
  m_pLabel->SizeToContents(); // Supposedly doesn't work until layout but kind
                              // of does...
...
void CItemUploadSubPanel::PerformLayout() {
  ...
  m_pPanelListPanel->SetBounds(nX, nY + m_pLabel->GetTall() + 10,
                               m_pLabel->GetWide(),
...
bool CItemUploadSubPanel::UpdateStatus() {
  CItemUploadWizard *pItemUploadWizard =
      dynamic_cast<CItemUploadWizard *>(GetWizardPanel());
  ...
  for (int i = 1; bValid && i < m_pPanelListPanel->GetItemCount(); i += 2) {
    CStatusLabel *pStatusLabel =
        dynamic_cast<CStatusLabel *>(m_pPanelListPanel->GetItemLabel(i));
    if (!pStatusLabel)
      continue;
  ...
  sTmp.sprintf("#itemtest_wizard_%s_%s", GetName(),
  ...
  if (pItemUploadWizard->GetCurrentSubPanel() == this) {
    pItemUploadWizard->SetFinishButtonEnabled(m_sNextName.IsEmpty() ? true
                                                                    : false);
  }
...
//
//-----------------------------------------------------------------------------
void CItemUploadSubPanel::SetStatus(bool bValid, const char *pszPrefix,
  ...
  CStatusLabel *pStatusLabel = dynamic_cast<CStatusLabel *>(
      m_pPanelListPanel->FindChildByName(sTmp, true));
  if (pStatusLabel) {
  ...
  sTmp.sprintf("%sStatusText", pszPrefix);
  vgui::Label *pStatusText = dynamic_cast<vgui::Label *>(
      m_pPanelListPanel->FindChildByName(sTmp, true));
  if (pStatusText) {
    if (pszMessage) {
...
public:
  CFileLocationPanel(vgui::Panel *pParent, int nLodIndex) : BaseClass(pParent) {
    m_pButtonBrowse =
        new vgui::Button(this, "BrowseButton", "#BrowseButton", pParent);
  ...
  virtual void PerformLayout() {
    ...
    m_pLabel->SetSize(w - m_pButtonBrowse->GetWide(),
                      m_pButtonBrowse->GetTall());
    m_pLabel->SetPos(0, 0);
  }
...
public:
  CLODFileLocationPanel(vgui::Panel *pParent, int nLodIndex)
    ...
    m_pButtonDelete->SetCommand(
        new KeyValues("Delete", "nLODIndex", nLodIndex));
  ...
  virtual void PerformLayout() {
    ...
    m_pButtonBrowse->SetPos(
        w - (m_pButtonDelete->GetWide() + m_pButtonBrowse->GetWide()), 0);
    m_pLabel->SetSize(
        w - (m_pButtonDelete->GetWide() + m_pButtonBrowse->GetWide()),
        m_pButtonBrowse->GetTall());
    m_pLabel->SetPos(0, 0);
  }
...
//
//-----------------------------------------------------------------------------
CGeometrySubPanel::CGeometrySubPanel(vgui::Panel *pParent, const char *pszName,
                                     const char *pszNextName)
...
void CGeometrySubPanel::UpdateGUI() {
  ...
  for (int i = 0; i < asset.TargetDMXCount(); ++i) {
    ...
    CLODFileLocationPanel *pFileLocationPanel =
        dynamic_cast<CLODFileLocationPanel *>(
            m_pPanelListPanel->GetItemPanel(i * 2));
  ...
  if (nGeometryCount == asset.TargetDMXCount()) {
    ...
    // Remove superfluous
    while (m_pPanelListPanel->GetItemCount() / 2 >
           (asset.TargetDMXCount() + 1)) {
      m_pPanelListPanel->RemoveItem(m_pPanelListPanel->GetItemCount() - 1);
      m_pPanelListPanel->RemoveItem(m_pPanelListPanel->GetItemCount() - 1);
    }

    // Set last one to empty
    if ((m_pPanelListPanel->GetItemCount() / 2) ==
        (asset.TargetDMXCount() + 1)) {
      CLODFileLocationPanel *pFileLocationPanel =
          dynamic_cast<CLODFileLocationPanel *>(m_pPanelListPanel->GetItemPanel(
              m_pPanelListPanel->GetItemCount() - 2));
      if (pFileLocationPanel) {
        vgui::Label *pLabel = pFileLocationPanel->m_pLabel;
        if (pLabel) {
          pLabel->SetText("");
...
bool CGeometrySubPanel::UpdateStatus() {
  ...
  for (int i = 0; i < m_pPanelListPanel->GetItemCount(); i += 2, ++nLODIndex) {
    CLODFileLocationPanel *pFileLocationPanel =
        dynamic_cast<CLODFileLocationPanel *>(
            m_pPanelListPanel->GetItemPanel(i));
    if (!pFileLocationPanel)
      continue;
    ...
    if (i < (m_pPanelListPanel->GetItemCount() - 2)) {
      ...
      if (nThisPolyCount <= 0) {
        sErrString.sprintf("LOD %d has bad polygon count: %d", i,
                           nThisPolyCount);
        SetStatus(false, sTmp, sErrString);
        continue;
      }
      ...
      if ((i > 0) && (nThisPolyCount == nLastPolyCount)) {
        sErrString.sprintf("LOD %d (%d polys) has the same number of polygons "
                           "as previous LOD %d (%d polys)",
                           nLODIndex, nThisPolyCount, nLODIndex - 1,
                           nLastPolyCount);
        SetStatus(false, sTmp, sErrString);
        continue;
      }
      ...
      if ((i > 0) && (nThisPolyCount >= nLastPolyCount)) {
        sErrString.sprintf("LOD %d (%d polys) has more polygons than previous "
                           "LOD %d (%d polys)",
                           nLODIndex, nThisPolyCount, nLODIndex - 1,
                           nLastPolyCount);
        SetStatus(false, sTmp, sErrString);
...
//
//-----------------------------------------------------------------------------
void CGeometrySubPanel::SetupFileOpenDialog(vgui::FileOpenDialog *pDialog,
                                            bool bOpenFile,
  ...
  if (!vgui::system()->GetRegistryString(
    ...
    if (CItemUpload::GetVMod(sVMod) && !sVMod.IsEmpty() &&
        CItemUpload::GetContentDir(sContentDir) && !sContentDir.IsEmpty() &&
        CItemUpload::FileExists(sContentDir.Get())) {
      sContentDir += "/";
      sContentDir += sVMod;
      sContentDir += "/models";
      ...
      if (CItemUpload::FileExists(sTmp.Get())) {
        ...
        if (CItemUpload::FileExists(sTmp.Get())) {
          ...
          V_FixupPathName(pszStartingDir, ARRAYSIZE(pszStartingDir),
                          sTmp.Get());
        }
      }
    }
  ...
  pDialog->SetStartDirectoryContext("itemtest_geometry_browser",
                                    pszStartingDir);
...
//
//-----------------------------------------------------------------------------
bool CGeometrySubPanel::OnReadFileFromDisk(const char *pszFileName,
  ...
  for (int i = 0; i < asset.TargetDMXCount(); ++i) {
    ...
    if (!V_strcmp(pTargetDMX->GetInputFile().Get(), szBuf1)) {
      vgui::MessageBox *pMessageBox = new vgui::MessageBox(
          "#duplicate_file_title", "#duplicate_file_text", this);
      if (pMessageBox) {
...
//
//-----------------------------------------------------------------------------
bool CGeometrySubPanel::OnWriteFileToDisk(const char *pszFileName,
                                          const char *pszFileFormat,
                                          KeyValues *pContextKeyValues) {
  return false;
}
...
void CGeometrySubPanel::OnOpen(int nLodIndex) {
  KeyValues *pContextKeyValues =
      new KeyValues("FileOpen", "nLODIndex", nLodIndex);
...
void CGeometrySubPanel::AddGeometry() {
  ...
  CLODFileLocationPanel *pFileLocationPanel =
      new CLODFileLocationPanel(this, nLodIndex);
...
public:
  ...
  CVmtEntry(CMaterialSubPanel *pMaterialSubPanel, const char *pszName,
            int nVmtIndex)
      : CDualPanelList(pMaterialSubPanel, pszName),
        m_pMaterialSubPanel(pMaterialSubPanel), m_nVmtIndex(nVmtIndex) {
    {
      vgui::Label *pMaterialLabel =
          new vgui::Label(this, "MaterialLabel", "#MaterialLabel");
    ...
    {
      vgui::Label *pMaterialTypeLabel =
          new vgui::Label(this, "MaterialTypeLabel", "#MaterialTypeLabel");
      m_pMaterialType =
          new vgui::ComboBox(this, "MaterialTypeComboBox", 0, false);
      ...
      m_pMaterialType->AddItem("#DuplicateOfPrimary",
                               new KeyValues("DuplicateOfPrimary"));
      m_pMaterialType->AddItem("#DuplicateOfSecondary",
                               new KeyValues("DuplicateOfSecondary"));
    ...
    {
      vgui::Label *pRedBlueLabel =
          new vgui::Label(this, "CommonRedBlueLabel", "#CommonRedBlueLabel");
    ...
    {
      vgui::Label *pCommonTextureLabel =
          new vgui::Label(this, "CommonTextureLabel", "#CommonTextureLabel");
    ...
    {
      vgui::Label *pRedTextureLabel =
          new vgui::Label(this, "RedTextureLabel", "#RedTextureLabel");
    ...
    {
      vgui::Label *pBlueTextureLabel =
          new vgui::Label(this, "BlueTextureLabel", "#BlueTextureLabel");
    ...
    {
      vgui::Label *pColorAlphaLabel =
          new vgui::Label(this, "ColorAlphaLabel", "#ColorAlphaLabel");
    ...
    {
      vgui::Label *pNormalMapLabel =
          new vgui::Label(this, "NormalMapLabel", "#NormalMapLabel");
    ...
    {
      vgui::Label *pNormalAlphaLabel =
          new vgui::Label(this, "NormalAlphaLabel", "#NormalAlphaLabel");

      m_pNormalAlpha =
          new vgui::ComboBox(this, "NormalAlphaComboBox", 0, false);
...
//
//=============================================================================
CMaterialSubPanel::CMaterialSubPanel(vgui::Panel *pParent, const char *pszName,
                                     const char *pszNextName)
    : BaseClass(pParent, pszName, pszNextName) {
  m_pFileOpenStateMachine = new vgui::FileOpenStateMachine(this, this);
  m_pFileOpenStateMachine->AddActionSignalTarget(this);
...
void CMaterialSubPanel::UpdateGUI() {
  ...
  for (int i = 0; i < asset.GetTargetVMTCount(); ++i) {
    ...
    CVmtEntry *pVmtEntry =
        dynamic_cast<CVmtEntry *>(m_pPanelListPanel->GetItemPanel(i * 2));
    if (!pVmtEntry)
      continue;
    ...
    case CTargetVMT::kNoColorAlpha:
      pVmtEntry->m_pColorAlpha->SilentActivateItemByRow(
          CTargetVMT::kNoColorAlpha);
      break;
    case CTargetVMT::kTransparency:
      pVmtEntry->m_pColorAlpha->SilentActivateItemByRow(
          CTargetVMT::kTransparency);
      break;
    ...
    if (pBlueTargetVTF.IsValid()) {
      pVmtEntry->m_pBlueTextureFileLocation->m_pLabel->SetText(
          pBlueTargetVTF->GetInputFile());
    } else {
      pVmtEntry->m_pBlueTextureFileLocation->m_pLabel->SetText("");
    }

    if (!pTargetVmt->GetColorMapCommon() && pRedTargetVTF.IsValid() &&
        pBlueTargetVTF.IsValid()) {
      if (pRedTargetVTF->HasAlpha() && pBlueTargetVTF->HasAlpha()) {
        bColorVisible = true;
      }
    }

    pVmtEntry->SetItemVisible(pVmtEntry->m_nColorAlphaId,
                              bVisible && bColorVisible);

    bool bNormalVisible = false;

    CSmartPtr<CTargetVTF> pNormalTargetVTF = pTargetVmt->GetNormalTargetVTF();
    if (pNormalTargetVTF.IsValid()) {
      pVmtEntry->m_pNormalTextureFileLocation->m_pLabel->SetText(
          pNormalTargetVTF->GetInputFile());
    ...
    case CTargetVMT::kNoNormalAlpha:
      pVmtEntry->m_pNormalAlpha->SilentActivateItemByRow(
          CTargetVMT::kNoNormalAlpha);
      break;
    case CTargetVMT::kNormalSpecPhong:
      pVmtEntry->m_pNormalAlpha->SilentActivateItemByRow(
          CTargetVMT::kNormalSpecPhong);
      break;
    default:
      pVmtEntry->m_pNormalAlpha->SilentActivateItemByRow(
          CTargetVMT::kNoNormalAlpha);
    ...
    pVmtEntry->SetItemVisible(pVmtEntry->m_nNormalAlphaId,
                              bVisible && bNormalVisible);
...
bool CMaterialSubPanel::UpdateStatus() {
  ...
  for (int i = 0; i < m_pPanelListPanel->GetItemCount(); i += 2, ++nIndex) {
    CVmtEntry *pVmtEntry =
        dynamic_cast<CVmtEntry *>(m_pPanelListPanel->GetItemPanel(i));
    if (!pVmtEntry)
      continue;
...
//
//-----------------------------------------------------------------------------
void CMaterialSubPanel::SetupFileOpenDialog(vgui::FileOpenDialog *pDialog,
                                            bool bOpenFile,
                                            const char *pszFileName,
                                            KeyValues *pContextKeyValues) {
  char pszStartingDir[MAX_PATH];
...
//
//-----------------------------------------------------------------------------
bool CMaterialSubPanel::OnReadFileFromDisk(const char *pszFileName,
                                           const char *pszFileFormat,
                                           KeyValues *pContextKeyValues) {
  CItemUploadWizard *pItemUploadWizard =
      dynamic_cast<CItemUploadWizard *>(GetWizardPanel());
  ...
  CVmtEntry *pVmtEntry =
      reinterpret_cast<CVmtEntry *>(pContextKeyValues->GetPtr("pVmtEntry"));
  const Browse_t nBrowseType =
      static_cast<Browse_t>(pContextKeyValues->GetInt("nBrowseType"));
...
//
//-----------------------------------------------------------------------------
bool CMaterialSubPanel::OnWriteFileToDisk(const char *pszFileName,
                                          const char *pszFileFormat,
                                          KeyValues *pContextKeyValues) {
  return false;
}
...
void CMaterialSubPanel::Browse(CVmtEntry *pVmtEntry, Browse_t nBrowseType) {
  ...
  KeyValues *pContextKeyValues =
      new KeyValues("FileOpen", "nBrowseType", nBrowseType);
...
//
//-----------------------------------------------------------------------------
CItemUploadWizard::CItemUploadWizard(vgui::Panel *pParent, const char *pszName)
...
void CItemUploadWizard::Run() {
  vgui::WizardSubPanel *pStartPanel =
      dynamic_cast<vgui::WizardSubPanel *>(FindChildByName("global"));
  ...
  CGlobalSubPanel *pGlobalSubPanel =
      dynamic_cast<CGlobalSubPanel *>(pStartPanel);
  if (pGlobalSubPanel) {
...
void CItemUploadWizard::UpdateGUI() {
  for (int i = 0; i < m_hSubPanelList.Count(); ++i) {
    CItemUploadSubPanel *pSubPanel =
        dynamic_cast<CItemUploadSubPanel *>(m_hSubPanelList.Element(i).Get());
    if (!pSubPanel)
      continue;
  ...
  CItemUploadSubPanel *pItemUploadSubPanel =
...
//
//-----------------------------------------------------------------------------
void CItemUploadWizard::OnFinishButton() { m_pFinalSubPanel->OnZip(); }