...
CDualPanelList::CDualPanelList(vgui::Panel *pParent, char const *pszPanelName)
  ...
  if (false && IsProportional()) {
    m_iDefaultHeight = vgui::scheme()->GetProportionalScaledValueEx(
        GetScheme(), DEFAULT_HEIGHT);
    m_iPanelBuffer =
        vgui::scheme()->GetProportionalScaledValueEx(GetScheme(), PANELBUFFER);
  } else {
...
//
//-----------------------------------------------------------------------------
int CDualPanelList::GetItemCount() const { return m_DataItems.Count(); }
...
//
//-----------------------------------------------------------------------------
int CDualPanelList::FirstItem() const { return m_DataItems.Head(); }
...
//
//-----------------------------------------------------------------------------
int CDualPanelList::InvalidItemID() const { return m_DataItems.InvalidIndex(); }
...
void CDualPanelList::PerformLayout() {
  ...
  if (m_pScrollBar) {
    ...
    m_pScrollBar->SetButtonPressedScrollValue(
        nTall / 4); // standard height of labels/buttons etc.
  ...
  // If we're supposed to automatically hide the scrollbar when unnecessary,
  check it now if ( m_bAutoHideScrollbar )
  {
  ...
  */
  ...
  if (m_pScrollBar) {
    iColumnWidth =
        (nWide - xpos - m_pScrollBar->GetWide() - 12) / m_nNumColumns;
  }
  ...
  for (int i = 0; i < m_SortedItems.Count(); i++) {
    ...
    if (item.labelPanel) {
      item.labelPanel->SetBounds(0, y, m_iFirstColumnWidth,
                                 item.panel->GetTall());
    }

    item.panel->SetBounds(xpos + iCurrentColumn * iColumnWidth, y, iColumnWidth,
                          item.panel->GetTall());
...
//
//-----------------------------------------------------------------------------
int CDualPanelList::GetFirstColumnWidth() { return m_iFirstColumnWidth; }
...
//
//-----------------------------------------------------------------------------
int CDualPanelList::GetNumColumns() { return m_nNumColumns; }
...
void CDualPanelList::SetSelectedPanel(vgui::Panel *pPanel) {
  if (pPanel != m_hSelectedItem) {
    ...
    if (m_hSelectedItem) {
      PostMessage(m_hSelectedItem.Get(),
                  new KeyValues("PanelSelected", "state", 0));
    }
...
//
//-----------------------------------------------------------------------------
vgui::Panel *CDualPanelList::GetSelectedPanel() { return m_hSelectedItem; }
