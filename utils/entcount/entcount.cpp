...
//
// Purpose: searches through all bsp files in the current directory parsing out
// entity details
//
//=============================================================================//
...
const char *g_UsageString =
    "usage:  entcount [-fgd <fgdfile>] [-nofgd] [-permap] [-onlyent <entname>] "
    "[-files <searchmask>]\n";
...
char *FindSearchWord(char *buffer, char *bufend) {
  ...
  /*
          for ( int i = SearchWordLen-1; i >= 0; i-- )
          {
                  ...
                  {
                          buffer += ( JumpTable[ (unsigned char)(*(buffer + i -
     SearchWordLen + 1)) ] - 1 );
