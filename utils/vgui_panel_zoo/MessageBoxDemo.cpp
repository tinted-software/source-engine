...
// Purpose: Constructor
//-----------------------------------------------------------------------------
MessageBoxDemo::MessageBoxDemo(Panel *parent, const char *name)
...
void MessageBoxDemo::ShowMessageBox() {
  ...
  // The third arg is if the box starts minimized (yes since we want the button
  // to open it) The fourth arg is a parent window arg.
  MessageBox *pMessage = new MessageBox(
      "Message Window",
  ...
  // the OK button. When the OK button is clicked the message box object is
  // destroyed.
  pMessage->DoModal();
}
