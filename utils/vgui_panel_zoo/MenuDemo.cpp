...
MenuDemo::MenuDemo(Panel *parent, const char *name) : DemoPage(parent, name) {
  ...
  // 3. MenuItems that are listed in the menu and execute commands when
  // selected.
  ...
  // underneath them.
  // The second arg is a KeyValues structure, that in this case holds a string
  // name of the command to be sent when the item is selected. The third arg is
  // the target of the command, we are sending all commands back to this class.
  ...
  // You can also set a fixed width, which we will do so our menu doesn't look
  // so scrawny. Get the size of the menu button
  m_pMenuButton->GetSize(wide, tall);

  // Set the width of the menu to be the same as the menu button.
  // Comment this line out if you want to see the menu in its original scrawny
  // size
