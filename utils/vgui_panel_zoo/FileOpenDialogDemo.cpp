...
// Purpose: Constructor
//-----------------------------------------------------------------------------
FileOpenDemo::FileOpenDemo(Panel *parent, const char *name)
  ...
  if (status) {
    if (!m_hFileDialog.Get()) {
      ...
      FileOpenDialog *pFileDialog =
          new FileOpenDialog(NULL, "Find the TestFile", true);
...
MessageMapItem_t FileOpenDemo::m_MessageMap[] = {
    MAP_MESSAGE_CONSTCHARPTR(FileOpenDemo, "FileSelected", OnFileSelected,
                             "fullpath"),
};
