...
// A wizard is an interactive utility within an application that guides the user
// through each step of a task.
//
...
// in the sequence. After all panels have been completed, the user clicks
// "finish" and the wizard exits.
//
// In VGUI, the Wizard class is the panel that holds the wizard navigation
// buttons to move to the previous or next panel, and the finish and cancel
// buttons to exit. It also creates the panels that display when the buttons are
// pressed, called WizardSubPanels. These panels have thier own layout and
// functions that determine when to enable/disable the Wizard's navigation
// buttons.
//
...
//			Provide some user options that we load from a resource
//file.
//-----------------------------------------------------------------------------
...
WizardSubPanel *CSomeSelections::GetNextSubPanel() {
  ...
  return dynamic_cast<WizardSubPanel *>(
      GetWizardPanel()->FindChildByName("CMoreSelections"));
}
...
void CSomeSelections::PerformLayout() {
  ...
  // Make sure the 'finish' button is disabled, since we are not on the last
  // panel.
...
//-----------------------------------------------------------------------------
WizardSubPanel *CMoreSelections::GetNextSubPanel() { return NULL; }
...
MessageMapItem_t CMoreSelections::m_MessageMap[] = {
    MAP_MESSAGE_PTR(CMoreSelections, "RadioButtonChecked", OnRadioButtonChecked,
...
void CWonderfulWizard::Run(void) {
  ...
  WizardPanel::Run(
      dynamic_cast<WizardSubPanel *>(FindChildByName("CSomeSelections")));
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
WizardPanelDemo::WizardPanelDemo(Panel *parent, const char *name)
