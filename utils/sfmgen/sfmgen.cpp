...
bool SceneManager_LoadSentenceFromWavFileUsingIO(char const *wavfile,
                                                 CSentence &sentence,
                                                 IFileReadBinary &io) {
  sentence.Reset();
...
bool SceneManager_LoadSentenceFromWavFile(char const *wavfile,
                                          CSentence &sentence) {
...
private:
  ...
  void ParseCSVFile(CUtlBuffer &buf, CUtlVector<SFMInfo_t> &infoList,
                    int nSkipLines);
  ...
  void GenerateSFMFile(const SFMGenInfo_t &sfmGenInfo, const SFMInfo_t &info,
                       studiohdr_t *pStudioHdr, const char *pOutputDirectory,
                       const char *pExportFacPath);

  // Saves an SFM file
  void SaveSFMFile(CDmElement *pRoot, const char *pRelativeScenePath,
                   const char *pFileName);

  // Generates a sound clip for the game sound
  CDmeSoundClip *CreateSoundClip(CDmeFilmClip *pShot,
                                 const char *pAnimationSetName,
                                 const char *pGameSound,
                                 studiohdr_t *pStudioHdr,
                                 CDmeGameSound **ppGameSound);

  // Builds the list of all controls in the animation set contributing to facial
  // animation
  void BuildFacialControlList(CDmeFilmClip *pShot,
...
void CSFMGenApp::PrintHelp() {
  Msg("Usage: sfmgen -i <in .csv file> -m <.mdl relative path> -o <output "
      "dir>\n");
  Msg("				[-f <fac output dir>] [-p] [-w] [-r <sample "
      "rate in hz>] [-s <sample filter size>]\n");
  Msg("				[-nop4] [-vproject <path to gameinfo.txt>]\n");
  Msg("\t-i\t: Source .CSV file indicating game sound names, text for phoneme "
      "extraction, output sfm file names.\n");
  Msg("\t-m\t: Indicates the path of the .mdl file under game/mod/models/ to "
      "use in the sfm files.\n");
  Msg("\t-o\t: Indicates output directory to place generated files in.  "
      "Required if Generating SFM files.\n");
  Msg("\t-f\t: [Optional] Indicates that facial files should be created in the "
      "specified fac output dir.\n");
  Msg("\t-p\t: [Optional] Indicates the extracted phonemes should be written "
      "into the wav file.\n");
  Msg("\t-w\t: [Optional] Indicates the phonemes should be read from the wav "
      "file. Cannot also have -p specified.\n");
  Msg("\t-r\t: [Optional] Specifies the phoneme extraction sample rate "
      "(default = 20)\n");
  Msg("\t-s\t: [Optional] Specifies the phoneme extraction sample filter size "
      "(default = 0.08)\n");
  Msg("\t-nop4\t: Disables auto perforce checkout/add.\n");
  Msg("\t-nosfm\t: Disables generating of SFM files (dmx).\n");
  Msg("\t-vproject\t: Specifies path to a gameinfo.txt file (which mod to "
      "build for).\n");
}
...
//-----------------------------------------------------------------------------
CSFMGenApp::TokenRetVal_t CSFMGenApp::ParseToken(CUtlBuffer &buf, char *pToken,
                                                 int nMaxTokenLen) {
  *pToken = 0;
...
// Parses the excel file
//-----------------------------------------------------------------------------
void CSFMGenApp::ParseCSVFile(CUtlBuffer &buf, CUtlVector<SFMInfo_t> &infoList,
                              int nSkipLines) {
  char pToken[512];
...
// Saves an SFM file
//-----------------------------------------------------------------------------
void CSFMGenApp::SaveSFMFile(CDmElement *pRoot, const char *pOutputDir,
                             const char *pFileName) {
  // Construct actual file name from the model name + dmx file name
  char pFullPathBuf[MAX_PATH];
  Q_snprintf(pFullPathBuf, sizeof(pFullPathBuf), "%s/%s.dmx", pOutputDir,
             pFileName);
  const char *pFullPath = pFullPathBuf;

  CP4AutoEditAddFile checkout(pFullPath);

  if (!g_pDataModel->SaveToFile(pFullPath, NULL,
                                g_pDataModel->GetDefaultEncoding("sfm_session"),
                                "sfm_session", pRoot)) {
...
// Generates a sound clip for the game sound
//-----------------------------------------------------------------------------
CDmeSoundClip *CSFMGenApp::CreateSoundClip(CDmeFilmClip *pShot,
                                           const char *pAnimationSetName,
                                           const char *pGameSound,
                                           studiohdr_t *pStudioHdr,
                                           CDmeGameSound **ppGameSound) {
  *ppGameSound = NULL;
  ...
  CDmeTrack *pTrack =
      pTrackGroup->FindOrAddTrack(pAnimationSetName, DMECLIP_SOUND);

  // Get the gender for the model
  gender_t actorGender =
      g_pSoundEmitterSystem->GetActorGender(pStudioHdr->pszName());

  // Get the wav file for the gamesound.
  CSoundParameters params;
  if (!g_pSoundEmitterSystem->GetParametersForSound(pGameSound, params,
                                                    actorGender)) {
  ...
  g_pFullFileSystem->RelativePathToFullPath(pRelativePath, "GAME", pFullPath,
                                            sizeof(pFullPath));
  DmeTime_t duration(GetWavSoundDuration(pFullPath));

  CDmeGameSound *pDmeSound =
      CreateElement<CDmeGameSound>(pGameSound, pTrack->GetFileId());
  Assert(pDmeSound);
  ...
  CDmeSoundClip *pSubClip =
      CreateElement<CDmeSoundClip>(pGameSound, pTrack->GetFileId());
...
// animation
//-----------------------------------------------------------------------------
void CSFMGenApp::BuildFacialControlList(CDmeFilmClip *pShot,
                                        CDmeAnimationSet *pAnimationSet,
                                        CUtlVector<LogPreview_t> &list) {
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    if (pControl->GetValue<bool>("combo")) {
      preview.m_hChannels[LOG_PREVIEW_VALUE] =
          pControl->GetValueElement<CDmeChannel>("valuechannel");
      preview.m_hChannels[LOG_PREVIEW_BALANCE] =
          pControl->GetValueElement<CDmeChannel>("balancechannel");
    } else {
      preview.m_hChannels[LOG_PREVIEW_VALUE] =
          pControl->GetValueElement<CDmeChannel>("channel");
      preview.m_hChannels[LOG_PREVIEW_BALANCE] = NULL;
    }

    if (pControl->GetValue<bool>("multi")) {
      preview.m_hChannels[LOG_PREVIEW_MULTILEVEL] =
          pControl->GetValueElement<CDmeChannel>("multilevelchannel");
    } else {
      preview.m_hChannels[LOG_PREVIEW_MULTILEVEL] = NULL;
    }

    preview.m_hOwner =
        preview.m_hChannels[LOG_PREVIEW_VALUE]->FindOwnerClipForChannel(pShot);
...
// Creates a single sfm file
//-----------------------------------------------------------------------------
void CSFMGenApp::GenerateSFMFile(const SFMGenInfo_t &sfmGenInfo,
                                 const SFMInfo_t &info, studiohdr_t *pStudioHdr,
  ...
  Q_FileBase(pStudioHdr->pszName(), pAnimationSetName,
             sizeof(pAnimationSetName));
  ...
  // Get the shot.
  CDmeFilmClip *pMovie =
      session.Root()->GetValueElement<CDmeFilmClip>("activeClip");
  CDmeFilmClip *pShot =
      CastElement<CDmeFilmClip>(pMovie->GetFilmTrack()->GetClip(0));

  // Create a camera for the shot
  DmeCameraParams_t cameraParams(pAnimationSetName, vec3_origin,
                                 QAngle(0, 180, 0));
  cameraParams.origin.x = pStudioHdr->hull_max.x + 20;
  cameraParams.origin.z =
      Lerp(0.95f, pStudioHdr->hull_min.z, pStudioHdr->hull_max.z);
  ...
  CDmeGameModel *pGameModel = session.CreateEditorGameModel(
      pStudioHdr, vec3_origin, Quaternion(0, 0, 0, 1));
  ...
  CDmeSoundClip *pSoundClip = CreateSoundClip(
      pMovie, pAnimationSetName, info.m_GameSound, pStudioHdr, &pGameSound);
  ...
  if (pSoundClip) {
    ...
    CDmeAnimationSet *pAnimationSet = CreateAnimationSet(
        pMovie, pShot, pGameModel, pAnimationSetName, 0, false);
    ...
    sfm_phonemeextractor->Extract(SPEECH_API_LIPSINC, extractDesc,
                                  sfmGenInfo.m_bWritePhonemesInWavs);

    CExtractInfo &results = extractDesc.m_WorkList[0];
    CDmElement *pExtractionSettings =
    ...
    for (int i = 0; i < results.m_ApplyTags.Count(); ++i) {
      ...
      CDmElement *tagElement = CreateElement<CDmElement>(
          ConvertPhoneme(tag->GetPhonemeCode()), pGameSound->GetFileId());
    ...
    if (sfmGenInfo.m_bGenerateSFMFiles && pExportFacPath) {
      ...
      Q_ComposeFileName(pExportFacPath, info.m_DMXFileName, pFACFileName,
                        sizeof(pFACFileName));
...
//-----------------------------------------------------------------------------
static void ComputeFullPath(const char *pRelativeDir, char *pFullPath,
                            int nBufLen) {
...
void CSFMGenApp::GenerateSFMFiles(SFMGenInfo_t &info) {
  ...
  Q_ComposeFileName("models", info.m_pModelName, pRelativeModelPath,
  ...
  if (info.m_pExportFacDirectory) {
    ComputeFullPath(info.m_pExportFacDirectory, pFullFacPathBuf,
                    sizeof(pFullFacPathBuf));
  ...
  if (!info.m_bExtractPhonemeFromWavsForMp3) {
    ...
    CUtlBuffer bufFilenames(
        0, 0, CUtlBuffer::TEXT_BUFFER); // Affected Files for reference (files
                                        // with Phoneme data)
    for (int i = 0; i < nCount; ++i) {
      CSentence sSentence;
      if (SceneManager_LoadSentenceFromWavFile(infoList[i].m_GameSound,
                                               sSentence)) {
        // Save Phoneme Data
...

    {
      ...
      if (fh) {
        g_pFullFileSystem->Write(bufFilenames.Base(), bufFilenames.TellPut(),
                                 fh);
        g_pFullFileSystem->Close(fh);
      }
    }
...
int CSFMGenApp::Main() {
  ...
  if (info.m_bUsePhonemesInWavs && info.m_bWritePhonemesInWavs) {
    Warning("Cannot simultaneously read the phones from wavs and also write "
            "them into the wavs!\n");
    return 0;
  }

  if (info.m_pExportFacDirectory &&
      !Q_stricmp(info.m_pExportFacDirectory, info.m_pOutputDirectory)) {
