...
void CCodeProcessor::PrintMissingTDFields(void) const {
  ...
  // Now check for things that are in the prediction TD but not marked correctly
  // as being part of the sendtable
  {
    // Now check prediction stuff
    classcount = 0;
    fieldcount = 0;
    ...
    while (cl) {
      if (cl->m_bDerivedFromCBaseEntity || cl->m_bHasPredictionData) {
        if (cl->CheckForPredictionFieldsInRecvTableNotMarkedAsSuchCorrectly(
                c)) {
          classcount++;
          fieldcount += c;
        }
  ...
  while (cl) {
    if (cl->m_bDerivedFromCBaseEntity) {
      ...
      if (cl->m_szTypedefBaseClass[0]) {
        ...
        missing = true;
        sprintf(missingwarning, ", missing typedef %s BaseClass",
                cl->m_szBaseClass);
      }
      ...
      if (GetPrintHierarchy() && GetPrintMembers()) {
        ...
        for (i = 0; i < cl->m_nMemberCount; i++) {
          ...
          if (member->m_szType[0]) {
            ...
            if (stricmp(p, cl->m_szName)) {
              vprint(0,
                     "class %s has member function %s with no return type!!!\n",
        ...
        for (i = 0; i < cl->m_nTDCount; i++) {
          ...
          vprint(0, "%s( %s, %s, %s, ... )\n", td->m_szDefineType, cl->m_szName,
                 td->m_szVariableName, td->m_szType);
        }

        if (!cl->m_bHasSaveRestoreData) {
          //	vprint( 1, "\nSave/Restore TYPEDESCRIPTION not specified for
          //class\n\n" );
        }
        ...
        if (!cl->m_bHasPredictionData) {
          //	vprint( 1, "\nPrediction TYPEDESCRIPTION not specified for
          //class\n\n" );
        }
      }
...
char *CCodeProcessor::ParseTypeDescription(char *current, bool fIsMacroized) {
  ...
  while (1) {
    ...
    if (!stricmp(com_token, "/")) {
      current = CC_ParseToken(current);
      if (!stricmp(com_token, "/")) {
        // There are two styles supported. One is to have the member definition
        // present but commented out:
        //		DEFINE_FIELD( m_member, FIELD_INTEGER ),
        // the other is to have a comment where the first token of the comment
        // is a member name:
        //		m_member
        current = CC_ParseToken(current);
    ...
    if (cl) {
      if (strcmp(vartype, "funcptr") && cl->FindTD(varname)) {
        vprint(0,
               "class %s::%s already has typedescription entry for field %s\n",
...
char *CCodeProcessor::ParseReceiveTable(char *current) {
  ...
  while (1) {
    ...
    if (cl) {
      ...
      vprint(0,
             "class %s::%s found in RecvTable, but no such class is known!!!\n",
             classname, varname);
    }
  }
    ...
    */
    ...
    if (cl) {
      if (cl->FindPredTD(varname)) {
        vprint(0,
               "class %s::%s already has prediction typedescription entry for "
               "field %s\n",
               classname, variablename, varname);
      } else {
...
void CCodeProcessor::AddHeader(int depth, const char *filename,
...
bool CCodeProcessor::CheckShouldSkip(bool forcequiet, int depth,
                                     char const *filename, int &numheaders,
                                     int &skippedfiles) {
...
bool CCodeProcessor::LoadFile(char **buffer, char *filename, char const *module,
                              bool forcequiet, int depth, int &filelength,
                              int &numheaders, int &skippedfiles,
                              char const *srcroot, char const *root,
                              char const *baseroot) {
  for (int i = 0; i < m_IncludePath.Count(); ++i) {
    ...
    if (CheckShouldSkip(forcequiet, depth, filename, numheaders,
                        skippedfiles)) {
      return false;
    }
...
void CCodeProcessor::ProcessModule(bool forcequiet, int depth, int &maxdepth,
                                   int &numheaders, int &skippedfiles,
                                   const char *srcroot, const char *baseroot,
                                   const char *root, const char *module) {
  char filename[256];
  ...
  if (!LoadFile(&buffer, filename, module, forcequiet, depth, filelength,
                numheaders, skippedfiles, srcroot, root, baseroot)) {
    CODE_MODULE module;
    module.skipped = true;
    m_Modules.Insert(filename, module);
    skippedfiles++;
  ...
  while (1) {
    ...
    if (!stricmp(com_token, "#include")) {
      ...
      if (strlen(com_token) > 0 && com_token[0] != '<') {
        ...
        m_nHeadersProcessed++;
        numheaders++;
        ProcessModule(true, depth + 1, maxdepth, numheaders, skippedfiles,
                      srcroot, baseroot, root, com_token);
      }
    ...
    } else if (!strnicmp(com_token, "PREDICTABLE_CLASS",
                         strlen("PREDICTABLE_CLASS"))) {
      char prefix[32];
      prefix[0] = 0;
      int type = 0;
      int bases = 1;
      ...
      if (!strcmp(com_token, "(")) {
        ...
        if (cl) {
          if (cl->m_bHasPredictionData) {
            if (!forcequiet) {
              vprint(0,
                     "Class %s declared predictable and implemented with "
                     "IMPLEMENT_PREDICTABLE_NODATA in typedescription\n",
                     cl->m_szName);
            }
  ...
  if (!forcequiet && !GetQuiet()) {
    vprint(0, " %s: headers (%i game / %i total)", (char *)&filename[m_nOffset],
           numheaders - skippedfiles, numheaders);
    if (maxdepth > 1) {
...
void CCodeProcessor::ProcessModules(const char *srcroot, const char *root,
                                    const char *rootmodule) {
  m_nFilesProcessed++;
  ...
  int maxdepth = 0;
  int skippedfiles = 0;
  ProcessModule(false, 0, maxdepth, numheaders, skippedfiles, srcroot, root,
                root, rootmodule);
}
...
void CCodeProcessor::PrintResults(const char *baseentityclass) {
  ...
  if (elapsed > 0.0) {
    vprint(0, "%.2f K processed in %.3f seconds, throughput %.2f KB/sec\n\n",
           (double)m_nBytesProcessed / 1024.0, elapsed,
           (double)m_nBytesProcessed / (1024.0 * elapsed));
  }
...
void CCodeProcessor::ConstructModuleList_R(int level,
  ...
  do {
    if (wfd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
      ...
      // Once we descend down a branch, don't keep looking for hl2/tf2 in name,
      // just recurse through all children
      if (level == 0 && !strstr(wfd.cFileName, gamespecific))
        continue;

      // Recurse down directory
      sprintf(filename, "%s\\%s", root, wfd.cFileName);
      ConstructModuleList_R(level + 1, baseentityclass, gamespecific, filename,
                            srcroot);
    } else {
...
void CCodeProcessor::SetupIncludePath(const char *sourcetreebase,
                                      const char *subdir,
                                      const char *gamespecific) {
  CleanupIncludePath();
...
void CCodeProcessor::Process(const char *baseentityclass,
                             const char *gamespecific,
                             const char *sourcetreebase, const char *subdir) {
  ...
  ConstructModuleList_R(0, baseentityclass, gamespecific, rootdirectory,
                        sourcetreebase);
