...
static bool FileSystem_LoadDLL(void) {
  ...
  g_pFileSystem =
      (IFileSystem *)g_pFileSystemFactory(FILESYSTEM_INTERFACE_VERSION, NULL);
...
void LoadMaterialSystem(void) {
  ...
  if (g_MaterialsFactory) {
    g_pMaterialSystem = (IMaterialSystem *)g_MaterialsFactory(
        MATERIAL_SYSTEM_INTERFACE_VERSION, NULL);
    if (!g_pMaterialSystem) {
      DisplayError("Could not get the material system interface from "
                   "materialsystem.dll (a)");
    }
  } else {
    DisplayError(
...
void Shader_Init(HWND mainWindow) {
  ...
  g_MaterialSystemClientFactory =
      g_pMaterialSystem->Init(pDLLName, &g_DummyMaterialProxyFactory,
  ...
  //	bool modeSet = g_pMaterialSystem->SetMode( (void*)mainWindow, mode,
  //modeFlags ); 	if (!modeSet)
  //	{
  //		DisplayError( "Unable to set mode\n" );
  //	}

  g_pMaterialSystemHardwareConfig =
      (IMaterialSystemHardwareConfig *)g_MaterialSystemClientFactory(
          MATERIALSYSTEM_HARDWARECONFIG_INTERFACE_VERSION, 0);
  if (!g_pMaterialSystemHardwareConfig) {
    DisplayError(
        "Could not get the material system hardware config interface!");
  }
...
static void CalcWindowSize(int desiredRenderingWidth,
                           int desiredRenderingHeight, int *windowWidth,
                           int *windowHeight) {
  int borderX, borderY;
  borderX = (GetSystemMetrics(SM_CXFIXEDFRAME) + 1) * 2;
  borderY = (GetSystemMetrics(SM_CYFIXEDFRAME) + 1) * 2 +
...
bool CreateAppWindow(const char *pAppName, int width, int height) {
  ...
  g_HWnd =
      CreateWindow(pAppName, pAppName, WS_OVERLAPPEDWINDOW, 0, 0, width, height,
...
void AllocateLightmap(LightmapInfo_t &lightmapInfo, int width, int height) {
  ...
  // HACK!  We don't ever use this material for anything. . just need it for
  // allocating lightmaps.
  CMatRenderContextPtr pRenderContext(materials);
  IMaterial *pHackMaterial = g_pMaterialSystem->FindMaterial(
      "shadertest/lightmappedgeneric", TEXTURE_GROUP_OTHER);
  pRenderContext->Bind(pHackMaterial);
  lightmapInfo.m_SortID = g_pMaterialSystem->AllocateLightmap(
      16, 16, lightmapInfo.m_OffsetIntoLightmapPage, pHackMaterial);
}

void Lightmap_PostAllocation(LightmapInfo_t &lightmapInfo) {
  g_pMaterialSystem->GetLightmapPageSize(lightmapInfo.m_SortID,
                                         &lightmapInfo.m_LightmapPageDims[0],
...
void UpdateLightmap(LightmapInfo_t &lightmapInfo, float *data) {
  g_pMaterialSystem->UpdateLightmap(
      g_pMaterialSortInfo[lightmapInfo.m_SortID].lightmapPageID,
      lightmapInfo.m_LightmapDims, lightmapInfo.m_OffsetIntoLightmapPage, data,
      NULL, NULL, NULL);
}
...
void CreateLightmapPages(void) {
  ...
  AllocateLightmap(g_CheckerboardLightmapInfo, CHECKERBOARD_DIMS,
                   CHECKERBOARD_DIMS);
...
void LightmapTexCoord(CMeshBuilder &meshBuilder, LightmapInfo_t lightmapInfo,
                      float s, float t) {
...
void ScreenShot(const char *pFilename) {
  ...
  unsigned char *pImage =
      (unsigned char *)malloc(g_RenderWidth * 3 * g_RenderHeight);

  // Get Bits from the material system
  CMatRenderContextPtr pRenderContext(materials);
  pRenderContext->ReadPixels(0, 0, g_RenderWidth, g_RenderHeight, pImage,
                             IMAGE_FORMAT_RGB888);

  CUtlBuffer outBuf;
  if (!TGAWriter::WriteToBuffer(pImage, outBuf, g_RenderWidth, g_RenderHeight,
...
void DrawBackground(void) {
  ...
  IMaterial *pMaterial = g_pMaterialSystem->FindMaterial(
      "matsys_regressiontest/background", TEXTURE_GROUP_OTHER);
...
void BeginFrame(void) {
  ...
  pRenderContext->PerspectiveX(90.0f, (g_RenderWidth / g_RenderHeight), 1.0f,
                               500000.0f);
...
void TestLightmappedGeneric_dx80(void) {
  ...
  Versions with DETAIL aren't implemented yet in HLSL, but we'll go ahead and
  test those.
  */
  // The following are shader combos from lightmappedgeneric_ps20
  for (int BUMPMAP = 0; BUMPMAP <= 1; BUMPMAP++) {
    for (int NORMALMAPALPHAENVMAPMASK = 0; NORMALMAPALPHAENVMAPMASK <= 1;
         NORMALMAPALPHAENVMAPMASK++) {
      // The following are shader combos from lightmappedgeneric_ps11 (that
      // aren't in lightmappedgenerc_vs11)
      for (int BASETEXTURE = 0; BASETEXTURE <= 1; BASETEXTURE++) {
        for (int ENVMAPMASK = 0; ENVMAPMASK <= 1; ENVMAPMASK++) {
          for (int SELFILLUM = 0; SELFILLUM <= 1; SELFILLUM++) {
            for (int BASEALPHAENVMAPMASK = 0; BASEALPHAENVMAPMASK <= 1;
                 BASEALPHAENVMAPMASK++) {
              ...
              for (int DETAIL = 0; DETAIL <= 1; DETAIL++) {
                for (int ENVMAP = 0; ENVMAP <= 1; ENVMAP++) {
                  for (int ENVMAPCAMERASPACE = 0; ENVMAPCAMERASPACE <= 1;
                       ENVMAPCAMERASPACE++) {
                    for (int ENVMAPSPHERE = 0; ENVMAPSPHERE <= 1;
                         ENVMAPSPHERE++) {
                      for (int VERTEXCOLOR = 0; VERTEXCOLOR <= 1;
                           VERTEXCOLOR++) {
                        //											for(
                        //int FOGTYPE = 0; FOGTYPE <= 1; FOGTYPE++ )
                        {
                          // conditional beneath here are flags, not shader
                          // combos
                              ...
                              for (int COLORMODULATE = 0; COLORMODULATE <= 1;
                                ...
                                for (int ALPHAMODULATE = 0; ALPHAMODULATE <= 1;
                                     ALPHAMODULATE++) {
																	...
																	ALPHAMODULATE = 0;
#endif
                                  // skip commands from shader
                                  // lightmappedgeneric_vs11
                                  if (ENVMAPCAMERASPACE && ENVMAPSPHERE)
                                    continue;
                                  if (!ENVMAP &&
                                      (ENVMAPCAMERASPACE || ENVMAPSPHERE))
                                    continue;

                                  // skip commands from shader
                                  // lihgtmappedgeneric_ps11
                                  if (!ENVMAP &&
                                  ...
                                  // skip commands from flags that don't make
                                  // sense (or we don't want to test)
                                  if (!BASETEXTURE &&
                                  ...
                                  if (BASEALPHAENVMAPMASK) {
                                    pKeyValues->SetInt("$basealphaenvmapmask",
                                                       1);
                                  }
                                  if (BUMPMAP) {
                                    pKeyValues->SetString(
                                        "$bumpmap", "matsys_regressiontest/"
                                                    "normalmap_normal");
                                  }
                                  if (DETAIL) {
                                    pKeyValues->SetString(
                                        "$detail",
                                        "matsys_regressiontest/detail");
                                    pKeyValues->SetFloat("$detailscale", 0.5f);
                                  }
                                  if (ENVMAP) {
                                    pKeyValues->SetString(
                                        "$envmap",
                                        "matsys_regressiontest/cubemap");
                                  }
                                  if (BASEALPHAENVMAPMASK) {
                                    pKeyValues->SetInt("$basealphaenvmapmask",
                                                       1);
                                  ...
                                  // hack hack - LEAK - need to figure out how
                                  // to clear a material out.
                                  g_pProceduralMaterial =
                                      g_pMaterialSystem->CreateMaterial(
                                          "test", pKeyValues);
                                  ...
                                  sprintf(
                                      filename, "%s\\lightmappedgeneric_dx80",
                                  ...
                                  if (DETAIL)
                                    Q_strcat(filename, "_DETAIL",
                                             sizeof(filename));
                                  if (ENVMAP)
                                    Q_strcat(filename, "_ENVMAP",
                                             sizeof(filename));
                                  if (ENVMAPCAMERASPACE)
                                  ...
                                  if (VERTEXCOLOR)
                                    Q_strcat(filename, "_VERTEXCOLOR",
                                             sizeof(filename));
                                  //							if(
                                  //FOGTYPE ) Q_strcat( filename, "_FOGTYPE",
                                  //sizeof(filename) );
                                  if (ALPHATEST)
...
void RenderFrame(void) { TestLightmappedGeneric_dx80(); }
