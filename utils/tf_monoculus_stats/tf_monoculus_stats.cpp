...
enum EventType {
  ...
  EYEBALL_DEATH,   // eyeball_death (max_dps 467.63) (max_health 12000)
                   // (player_count 23)
  EYEBALL_STUN,    // "Aque0us<174><STEAM_0:0:212532><Red>" eyeball_stunned with
                   // "NoWeapon" (attacker_position "-1033 872 143")
  PURGATORY_ENTER, // "I_DONT_KNOW<181><STEAM_0:1:34210475><Red>"
                   // purgatory_teleport "spawn_purgatory"
  PURGATORY_ESCAPE,  // "I_DONT_KNOW<181><STEAM_0:1:34210475><Red>"
                     // purgatory_escaped
  LOOT_ISLAND_ENTER, // "I_DONT_KNOW<181><STEAM_0:1:34210475><Red>"
                     // purgatory_teleport "spawn_loot"
  EYEBALL_KILLER,    // "Spaghetti Western<192><STEAM_0:1:41812531><Red>"
                  // eyeball_killer with "liberty_launcher" (attacker_position
                  // "-1629 -293 213")
...
//----------------------------------------------------------------------------------
bool ParseEvent(FILE *fp, EventInfo *event) { return false; }
      ...
      if (data) {
        ...
        if (!strncmp("purgatory_escaped", token, 17)) {
          ...
          if (!strcmp("spawn_purgatory", token)) {
            ...
            fprintf(errorFP,
                    "ERROR @ Line %d: Unknown purgatory teleport '%s'\n", line,
                    token);
            continue;
          }
        } else if (!strcmp("eyeball_stunned", token)) {
          // eyeball_stunned with "short_stop" (attacker_position "-1951 -166
          // 256")
          ...
          // eyeball_killer with "short_stop" (attacker_position "-1213 271
          // 236")
  ...
  for (unsigned int i = 0; i < TheData.size(); ++i) {
    ...
    case EYEBALL_SPAWN:
      if (isBossAlive) {
        ...
        // timestamp can go backwards on map change, so just don't count these
        // lifetimes
    ...
    case EYEBALL_ESCAPE:
      if (!isBossAlive) {
        ...
        isBossAlive = false;
        bossLifetime[bossSpawnCount - 1] =
            TheData[i].m_timestamp - bossSpawnTimestamp;
        bossEscapeLifetime[bossSpawnCount - 1] =
            TheData[i].m_timestamp - bossSpawnTimestamp;
        ++bossEscapedCount;
      }
      break;
    ...
    case EYEBALL_DEATH:
      if (!isBossAlive) {
        ...
        // using large time delta to account for stuns while escaping followed
        // by a death
        ...
        bossLifetime[bossSpawnCount - 1] =
            TheData[i].m_timestamp - bossSpawnTimestamp;
        bossDeathLifetime[bossSpawnCount - 1] =
            TheData[i].m_timestamp - bossSpawnTimestamp;
        ++bossDeathCount;
      }
...
#define USE_COLUMNS
    ...
    for (unsigned int i = 0; i < bossSpawnCount; ++i) {
      fprintf(fp, "%d, %d, %d, %d\n", bossLifetime[i], bossEscapeLifetime[i],
              bossDeathLifetime[i], bossStunCount[i]);
