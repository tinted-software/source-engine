...
// Copied and bastardized a few material proxy classes from the TF client dll.
// The purpose here is to make TF materials for paintable items show the default
// paint color (using the SelectFirstIfNotZero proxy) and to completely hide the
// burn detail texture (fake BurnLevel proxy). Implemented a lame material proxy
// factory that only knows about these two proxies.
...
bool CFloatInput::Init(IMaterial *pMaterial, KeyValues *pKeyValues,
                       const char *pKeyName, float flDefault) {
  m_pFloatVar = NULL;
...
void CFunctionProxy::ComputeResultType(MaterialVarType_t &resultType,
                                       int &vecSize) {
...
public:
  virtual void OnBind(void *pC_BaseEntity) {
    // Slam burn level to 0 to avoid the burn detail texture showing through in
    // modelbrowser.
    Assert(m_pResult);
    if (m_pResult) {
