...
CKeyValue *CChunk::FindKey(char const *pKeyName) {
  for (unsigned short i = m_Keys.Head(); i != m_Keys.InvalidIndex();
       i = m_Keys.Next(i)) {
...
CChunk *CChunk::FindChunk(char const *pChunkName) {
  for (unsigned short i = m_Chunks.Head(); i != m_Chunks.InvalidIndex();
       i = m_Chunks.Next(i)) {
...
CKeyValue *CChunk::FindKey(char const *pKeyName, const char *pValue) {
  for (unsigned short i = m_Keys.Head(); i != m_Keys.InvalidIndex();
    ...
    if (stricmp(pKey->m_pKey, pKeyName) == 0 &&
        stricmp(pKey->m_pValue, pValue) == 0)
      return pKey;
  }
...
ChunkFileResult_t MyDefaultHandler(CChunkFile *pFile, void *pData,
...
ChunkFileResult_t MyKeyHandler(const char *szKey, const char *szValue,
                               void *pData) {
...
void WriteChunks_R(CChunkFile *pFile, CChunk *pChunk, bool bRoot) {
  ...
  // Write keys.
  for (unsigned short i = pChunk->m_Keys.Head();
       i != pChunk->m_Keys.InvalidIndex(); i = pChunk->m_Keys.Next(i)) {
    CChunkHolder holder;
  ...
  // Write subchunks.
  for (int i = pChunk->m_Chunks.Head(); i != pChunk->m_Chunks.InvalidIndex();
       i = pChunk->m_Chunks.Next(i)) {
    CChunkHolder holder;
...
void ScanChunks(CChunk *pChunk, void (*fn)(CChunk *)) {
  ...
  for (unsigned short i = pChunk->m_Chunks.Head();
       i != pChunk->m_Chunks.InvalidIndex(); i = pChunk->m_Chunks.Next(i)) {
