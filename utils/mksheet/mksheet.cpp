    ...
    if (in_str[0]) {
      ...
      if ((Words.Count() == 1) && (!stricmp(Words[0], "loop"))) {
        ...
        if (eRequestedMode == PCKM_INVALID)
          printf("*** line %d: invalid packmode specified, allowed values are "
                 "'rgba' or 'rgb+a'!\n",
                 numActualLinesRead),
              exit(-1);
          ...
          // everything else
            ...
            printf("*** line %d: incompatible packmode change when %d "
                   "sequences already defined!\n",
        ...
        if (!stricmp(szSeqType, "") || !stricmp(szSeqType, "-rgba"))
          ...
          printf("*** line %d: invalid sequence type '%s', allowed "
        ...
        case PCKM_FLAT:
          ...
          case Sequence::SQM_RGBA:
            break;
          default:
            printf("*** line %d: invalid sequence type '%s', packing 'flat' "
        ...
        case PCKM_RGB_A:
          ...
          case Sequence::SQM_ALPHA:
            break;
          default:
            printf("*** line %d: invalid sequence type '%s', packing 'rgb+a' "
            ...
            if (s_ePackingMode == PCKM_RGB_A) {
              for (uint16 idx = 0; idx < pBM->m_mapSequences.Count(); ++idx) {
                ...
                if (pSeq->m_eMode != Sequence::SQM_RGBA &&
                  ...
                  printf("*** line %d: 'rgb+a' packing cannot pack frame '%s' "
...
bool PackImages_Rgb_A(char const *pFname, int nWidth) {
  ...
  for (int i = 0; i < ImageList.GetNumStrings(); i++) {
    ...
    case Sequence::SQM_RGBA:
      if (!bPackingRGBA)
        printf("*** error when packing 'rgb+a', bad sequence %d encountered "
               "for frame '%s' after all rgba frames packed!\n",
      ...
      break;
    default:
      printf("*** error when packing 'rgb+a', bad sequence %d encountered for "
             "frame '%s'!\n",
    ...
    for (int i = 0; i < Sequences.Count(); i++) {
      ...
      for (int j = 0; j < Sequences[i]->m_Frames.Count(); j++) {
        ...
        for (int t = 0; t < MAX_IMAGES_PER_FRAME; t++) {
          ...
          // %.2f, %.2f )\n", t, 					UCoord(
          // Sequences[i]->m_Frames[j].m_pSeqFrame[t]->m_XCoord ), 					VCoord(
          // Sequences[i]->m_Frames[j].m_pSeqFrame[t]->m_YCoord ), 					UCoord(
