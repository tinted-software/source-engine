...
//			lpReserved -	bad idea that some Windows developer had
//some day that 							we're stuck with
//-----------------------------------------------------------------------------
#ifdef _WIN32
...
public:
  ...
  // ISQLWrapper
  virtual void Init(const char *pchDB, const char *pchHost,
  ...
  // ISQLHelper
  virtual bool BInternalQuery(const char *pchQueryString,
                              MYSQL_RES **ppMySQLRes,
                              bool bRecurse /* = true */);
...
private:
  CUtlFixedLinkedList<CSQLWrapper>
      m_ListSQLWrapper; // use a fixed in memory data struct so we can return
                        // pointers to the interfaces
};

CSQLWrapperFactory g_SQLWrapperFactory;
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CSQLWrapperFactory, ISQLWrapperFactory,
                                  INTERFACEVERSION_ISQLWRAPPER,
                                  g_SQLWrapperFactory);
...
//-----------------------------------------------------------------------------
ISQLWrapper *CSQLWrapperFactory::Create(const char *pchDB, const char *pchHost,
...
void CSQLWrapperFactory::Validate(CValidator &validator, char *pchName) {
  ...
  FOR_EACH_LL(m_ListSQLWrapper, iListSQLWrapper) {
    m_ListSQLWrapper[iListSQLWrapper].Validate(
        validator, "m_ListSQLWrapper[ iListSQLWrapper ]");
  }
...
//-----------------------------------------------------------------------------
// Purpose: Initializer.  Sets up connection params but DOESN'T actually do the
// connection Input:	pchDB -		database name to connect to
//			pchHost -		host to connect to
//			pchUsername -		username to connect as
//			pchPassword -		password to use
//-----------------------------------------------------------------------------
void CSQLWrapper::Init(const char *pchDB, const char *pchHost,
                       const char *pchUsername, const char *pchPassword) {
...
bool CSQLWrapper::BConnect() {
  ...
  MYSQL *pMYSQL = mysql_real_connect(&m_MySQL, m_pchHost, m_pchUsername,
                                     m_pchPassword, m_pchDB, 0, NULL, 0);
...
//			ppMySQLRes	-			mysql result set
//to set 			bRecurse -				if true allow this function to
//call itself again (to rety the query)
// Output:	true if query succeeds, false otherwise
//-----------------------------------------------------------------------------
bool CSQLWrapper::BInternalQuery(const char *pchQueryString,
                                 MYSQL_RES **ppMySQLRes, bool bRecurse) {
  *ppMySQLRes = NULL;
  if (!m_pchDB || !m_pchHost || !m_pchUsername || !m_pchPassword ||
      !ppMySQLRes) {
    return false;
  }
  ...
  if (!bRet && !m_bConnected &&
      bRecurse) // hmmm, got hung up when running the query
  {
    bRet = BInternalQuery(pchQueryString, ppMySQLRes,
                          false); // run the query again now we reconnected
  }
...
//to set
// Output:	true if query succeeds, false otherwise, result it set on
// success
//-----------------------------------------------------------------------------
bool CSQLWrapper::_Query(const char *pchQueryString, MYSQL_RES **ppMySQLRes) {
  *ppMySQLRes = NULL;
  int iResult =
      mysql_real_query(&m_MySQL, pchQueryString, Q_strlen(pchQueryString));
  if (iResult != 0) {
    int iErrNo = mysql_errno(&m_MySQL);
    if (iErrNo == CR_COMMANDS_OUT_OF_SYNC) // I hate this return code, you just
  ...
  if (mysql_field_count(&m_MySQL) >
      0) // if there are results clear them from the connection
  {
...
//-----------------------------------------------------------------------------
// Purpose: Runs a insert style query on the db (i.e no return set), opens the
// connection if it isn't currently Input:	pchQueryString -
// query string to run Output:	true if query succeeds, false otherwise
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Runs a select style query on the db (i.e a return set), opens the
// connection if it isn't currently Input:	pchQueryString -
// query string to run Output:	returns a pointer to the result set (NULL on
// failure)
//-----------------------------------------------------------------------------
