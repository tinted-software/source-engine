...
void HistogramCPU(CUtlBuffer &inbuf, CUtlBuffer &outbuf, char *pszSearchString,
                  int size, int binwidth) {
...
void ParseFile(CUtlBuffer &inbuf, CUtlBuffer &outbuf, char *pszSearchString,
               int size, int binwidth, int nofilter) {
...
void ParseFile2(CUtlBuffer &inbuf, CUtlBuffer &outbuf, char *pszSearchString,
                int size, int binwidth) {
  binwidth = 100;
...
void ParseFile3(CUtlBuffer &inbuf, CUtlBuffer &outbuf, char *pszSearchString,
                int size, int binwidth) {
  binwidth = 100;
...
void TimeSeriesCPU(CUtlBuffer &inbuf, CUtlBuffer &outbuf, char *pszSearchString,
                   int size, int binwidth) {
  binwidth = 600;
  ...
  for (int i = 0; i < CPUBINS; ++i) {
    outbuf.Printf("-");
    for (int j = 1; j < nTimeList.Size();
         ++j) // use NTIMEBINS for equal sized bins
      outbuf.Printf("\t%d", nQuantity[j][i]);
    outbuf.Printf("\n");
  }
}
...
void TimeSeriesVCard(CUtlBuffer &inbuf, CUtlBuffer &outbuf,
                     char *pszSearchString, int size, int binwidth) {
  // unequally sized bins
  CUtlVector<int> nTimeList;
  ...
  while ((inbuf.TellGet() < size)) {
    ...
    if (Q_stristr(pszCard, "RIVA TNT2") != NULL) {
      ...
      cardIndex = 1;
    } else if (Q_stristr(pszCard, "Microsoft Corporation GDI Generic") !=
               NULL) {
      cardIndex = 2;
...
void InsertResult(int nCpu, CUtlVector<int> &nCpuList,
                  CUtlVector<int> &nQuantity) {
  if (nCpu < 0) // handle corrupt data
    return;
...
void HistogramVidCards(CUtlBuffer &inbuf, CUtlBuffer &outbuf,
                       char *pszSearchString, int size, int binwidth) {
...
void HistogramNetSpeed(CUtlBuffer &inbuf, CUtlBuffer &outbuf,
                       char *pszSearchString, int size, int binwidth) {
...
void HistogramRam(CUtlBuffer &inbuf, CUtlBuffer &outbuf, char *pszSearchString,
                  int size, int binwidth) {
