...
public:
  ...
  virtual void GenerateChildrenOfNode(int itemIndex) {
    ...
    for (int i = 0; i < files.Count(); i++) {
      ...
      KeyValues *kv =
          new KeyValues("node", "text", p4->String(files[i].m_sName));
      ...
      if (files[i].m_bDir) {
        Q_snprintf(szPath, sizeof(szPath), "%s/%s/%%%%1",
                   p4->String(files[i].m_sPath), p4->String(files[i].m_sName));
      } else {
        Q_snprintf(szPath, sizeof(szPath), "%s/%s",
                   p4->String(files[i].m_sPath), p4->String(files[i].m_sName));
      }
  ...
  virtual void GenerateContextMenu(int itemIndex, int x, int y) {
    ...
    if (pkv->GetInt("dir")) {
      pContext->AddMenuItem("Cloak folder",
                            new KeyValues("CloakFolder", "item", itemIndex),
                            GetParent(), NULL);
    } else {
      pContext->AddMenuItem("Open for edit",
                            new KeyValues("EditFile", "item", itemIndex),
                            GetParent(), NULL);
      pContext->AddMenuItem("Open for delete",
                            new KeyValues("DeleteFile", "item", itemIndex),
                            GetParent(), NULL);
    }
...
public:
  CSmallTextListPanel(Panel *parent, const char *name)
...
CVP4Dialog::CVP4Dialog() : BaseClass(NULL, "vp4dialog"), m_Images(false) {
  ...
  m_pChangesList->SetAutoResize(Panel::PIN_TOPLEFT,
                                Panel::AUTORESIZE_DOWNANDRIGHT, 6, 6, -12, -12);
  ...
  m_pRevisionList->AddColumnHeader(0, "change", "change", 52,
                                   ListPanel::COLUMN_HIDDEN);
  m_pRevisionList->AddColumnHeader(1, "date", "date", 52, 0);
  m_pRevisionList->AddColumnHeader(2, "time", "time", 52,
                                   ListPanel::COLUMN_HIDDEN);
  m_pRevisionList->AddColumnHeader(3, "user", "user", 64, 0);
  m_pRevisionList->AddColumnHeader(4, "description", "description", 32,
  ...
  m_pRevisionList->SetAutoResize(
      Panel::PIN_TOPLEFT, Panel::AUTORESIZE_DOWNANDRIGHT, 6, 6, -12, -12);
...
void CVP4Dialog::Activate() {
  ...
  char szTitle[256];
  Q_snprintf(szTitle, sizeof(szTitle), "VP4 - %s",
             p4->String(p4->GetActiveClient().m_sUser));
...
void CVP4Dialog::RefreshChangesList() {
  ...
  // add the changeslists
  {
    for (int i = 0; i < sections.Count(); i++) {
      ...
      if (sections[i] > 0) {
        Q_snprintf(szChangelistName, sizeof(szChangelistName), "CHANGE: %d",
                   sections[i]);
      } else {
        Q_snprintf(szChangelistName, sizeof(szChangelistName),
                   "CHANGE: DEFAULT");
      }

      m_pChangesList->AddColumnToSection(sections[i], "file", szChangelistName,
  ...
  // add the files to the changelists
  {
    for (int i = 0; i < files.Count(); i++) {
      ...
      Q_snprintf(szFile, sizeof(szFile), "%s/%s",
                 p4->String(files[i].m_sPath) + p4->GetDepotRootLength(),
...
void CVP4Dialog::RefreshClientList() {
  ...
  for (int i = 0; i < clients.Count(); i++) {
    ...
    // add in the new item
    char szText[256];
    Q_snprintf(szText, sizeof(szText), "%s    %s",
               p4->String(clients[i].m_sName),
               p4->String(clients[i].m_sLocalRoot));
    m_pClientCombo->AddItem(
        szText,
        new KeyValues("client", "client", p4->String(clients[i].m_sName)));
  }
...
void CVP4Dialog::OnFileSelected() {
  ...
  if (iItem < 0) {
    m_pRevisionList->SetEmptyListText(
        "No file or directory currently selected.");
    return;
  }
  ...
  m_pRevisionList->SetEmptyListText(
      "There is no revision history for the selected file.");

  KeyValues *pkv = m_pFileTree->GetItemData(iItem);

  CUtlVector<P4Revision_t> &revisions =
      p4->GetRevisionList(pkv->GetString("path"), pkv->GetInt("dir"));
  ...
  for (int i = 0; i < revisions.Count(); i++) {
    ...
    char szDate[256];
    Q_snprintf(szDate, sizeof(szDate), "%d/%d/%d", revisions[i].m_nYear,
               revisions[i].m_nMonth, revisions[i].m_nDay);
    pkv->SetString("date", szDate);

    char szTime[256];
    Q_snprintf(szTime, sizeof(szTime), "%d:%d:%d", revisions[i].m_nHour,
               revisions[i].m_nMinute, revisions[i].m_nSecond);
