...
void AddFileSystemSearchPaths(const char *pszExeName) {
  ...
  if (::GetModuleFileName((HINSTANCE)GetModuleHandle(NULL), pExeName,
                          sizeof(pExeName))) {
...
bool InitializeVGui() {
  ...
  // load the scheme
  g_pMainPanel->SetScheme(vgui::scheme()->LoadSchemeFromFile(
      "//PLATFORM/Resource/sourcescheme.res", "PLATFORM"));
...
int CVP4App::Main() {
  if (!InitializeVGui()) {
    ::MessageBoxA(NULL, "Fatal Error: Could not initialize vgui.",
                  "Steam - Fatal Error", MB_OK | MB_ICONERROR);
    return 0;
  }
