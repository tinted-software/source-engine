...
IVEngineServer *engine =
    NULL; // helper functions (messaging clients, loading content, making
          // entities, running commands, etc)
...
IPlayerInfoManager *playerinfomanager =
    NULL; // game dll interface to interact with players
IEntityInfoManager *entityinfomanager =
    NULL; // game dll interface to interact with all entities (like IPlayerInfo)
IGameInfoManager *gameinfomanager =
    NULL; // game dll interface to get data from game rules directly
IBotManager *botmanager = NULL; // game dll interface to interact with bots
IServerPluginHelpers *helpers =
    NULL; // special 3rd party plugin helpers from the engine
...
//---------------------------------------------------------------------------------
CEmptyServerPlugin::CEmptyServerPlugin() { m_iClientCommandIndex = 0; }
...
bool CEmptyServerPlugin::Load(CreateInterfaceFn interfaceFactory,
  ...
  entityinfomanager = (IEntityInfoManager *)gameServerFactory(
      INTERFACEVERSION_ENTITYINFOMANAGER, NULL);
  if (!entityinfomanager) {
    Warning(
        "Unable to load entityinfomanager, ignoring\n"); // this isn't fatal, we
                                                         // just won't be able
                                                         // to access entity
                                                         // data
  }

  playerinfomanager = (IPlayerInfoManager *)gameServerFactory(
      INTERFACEVERSION_PLAYERINFOMANAGER, NULL);
  ...
  botmanager =
      (IBotManager *)gameServerFactory(INTERFACEVERSION_PLAYERBOTMANAGER, NULL);
  if (!botmanager) {
  ...
  gameinfomanager = (IGameInfoManager *)gameServerFactory(
      INTERFACEVERSION_GAMEINFOMANAGER, NULL);
  if (!gameinfomanager) {
  ...
  gameeventmanager = (IGameEventManager *)interfaceFactory(
      INTERFACEVERSION_GAMEEVENTSMANAGER, NULL);
  helpers = (IServerPluginHelpers *)interfaceFactory(
      INTERFACEVERSION_ISERVERPLUGINHELPERS, NULL);
  enginetrace = (IEngineTrace *)interfaceFactory(
      INTERFACEVERSION_ENGINETRACE_SERVER, NULL);
  randomStr = (IUniformRandomStream *)interfaceFactory(
      VENGINE_SERVER_RANDOM_INTERFACE_VERSION, NULL);

  // get the interfaces we want to use
  if (!(engine && gameeventmanager && g_pFullFileSystem && helpers &&
        enginetrace && randomStr)) {
...
void CEmptyServerPlugin::Unload(void) {
  gameeventmanager->RemoveListener(
      this); // make sure we are unloaded from the event system
...
//---------------------------------------------------------------------------------
// Purpose: called when the plugin is paused (i.e should stop running but isn't
// unloaded)
//---------------------------------------------------------------------------------
void CEmptyServerPlugin::Pause(void) {}

//---------------------------------------------------------------------------------
// Purpose: called when the plugin is unpaused (i.e should start executing
// again)
//---------------------------------------------------------------------------------
...
//---------------------------------------------------------------------------------
// Purpose: called on level start, when the server is ready to accept client
// connections
//		edictCount is the number of entities in the level, clientMax is
//the max client count
//---------------------------------------------------------------------------------
void CEmptyServerPlugin::ServerActivate(edict_t *pEdictList, int edictCount,
                                        int clientMax) {}

//---------------------------------------------------------------------------------
// Purpose: called once per server frame, do recurring work here (like checking
// for timeouts)
//---------------------------------------------------------------------------------
...
//---------------------------------------------------------------------------------
// Purpose: called on level end (as the server is shutting down or going to a
// new map)
//---------------------------------------------------------------------------------
void CEmptyServerPlugin::LevelShutdown(
    void) // !!!!this can get called multiple times per map change
{
...
// Purpose: called on
//---------------------------------------------------------------------------------
void CEmptyServerPlugin::ClientPutInServer(edict_t *pEntity,
...
void CEmptyServerPlugin::ClientSettingsChanged(edict_t *pEdict) {
  if (playerinfomanager) {
    ...
    const char *name =
        engine->GetClientConVarValue(engine->IndexOfEdict(pEdict), "name");

    // CAN'T use Q_stricmp here, this dll is made by 3rd parties and may not
    // link to tier0/vstdlib
    if (playerinfo && name && playerinfo->GetName() &&
        stricmp(name,
                playerinfo->GetName())) // playerinfo may be NULL if the MOD
                                        // doesn't support access to player data
                                        // OR if you are accessing the player
                                        // before they are fully connected
    {
      ClientPrint(pEdict, "Your name changed to \"%s\" (from \"%s\"\n", name,
                  playerinfo->GetName());
      // this is the bad way to check this, the better option it to listen for
      // the "player_changename" event in FireGameEvent() this is here to give a
      // real example of how to use the playerinfo interface
    }
  }
}
...
//---------------------------------------------------------------------------------
PLUGIN_RESULT
CEmptyServerPlugin::ClientConnect(bool *bAllowConnect, edict_t *pEntity,
...
CON_COMMAND(DoAskConnect,
            "Server plugin example of using the ask connect dialog") {
  if (args.ArgC() < 2) {
    ...
    kv->SetString("title", pServerIP); // The IP address of the server to
                                       // connect to goes in the "title" field.
    kv->SetInt("time", 3);

    for (int i = 1; i < gpGlobals->maxClients; i++) {
      edict_t *pEdict = engine->PEntityOfEntIndex(i);
      if (pEdict) {
        helpers->CreateMessage(pEdict, DIALOG_ASKCONNECT, kv,
                               &g_EmtpyServerPlugin);
      }
    }
...
const char *classNames[] = {
    "unknown",           "scout", "sniper", "soldier",  "demoman", "medic",
    "heavy weapons guy", "pyro",  "spy",    "engineer",
};
...
void SentryStatus(edict_t *pEntity) {
  ...
  if (playerinfo->GetCustomInfo(TFPLAYERINFO_ENTINDEX_SENTRY, value,
                                emptyVariant)) {
  ...
  if (playerinfo->GetCustomInfo(TFPLAYERINFO_BUILDING_SENTRY, value,
                                emptyVariant)) {
    if (value.Bool())
      Msg("Sentry Under Construction...\n");
  }
  if (playerinfo->GetCustomInfo(TFPLAYERINFO_UPGRADING_SENTRY, value,
                                emptyVariant)) {
    if (value.Bool())
  ...
  int sentryLevel = 0;
  if (playerinfo->GetCustomInfo(TFPLAYERINFO_SENTRY_LEVEL, value,
                                emptyVariant)) {
  ...
  if (playerinfo->GetCustomInfo(TFPLAYERINFO_SENTRY_PROGRESS, value,
                                emptyVariant)) {
    if (sentryLevel < 3) {
  ...
  if (playerinfo->GetCustomInfo(TFPLAYERINFO_SENTRY_AMMO_SHELLS, value,
                                emptyVariant)) {
  ...
  if (sentryLevel > 2) {
    if (playerinfo->GetCustomInfo(TFPLAYERINFO_SENTRY_AMMO_ROCKETS, value,
                                  emptyVariant)) {
...
void DispenserStatus(edict_t *pEntity) {
  ...
  if (playerinfo->GetCustomInfo(TFPLAYERINFO_ENTINDEX_DISPENSER, value,
                                emptyVariant)) {
  ...
  if (playerinfo->GetCustomInfo(TFPLAYERINFO_BUILDING_DISPENSER, value,
                                emptyVariant)) {
    if (value.Bool())
      Msg("Dispenser Under Construction...\n");
  }
  Msg("Health: %i\n", entinfo->GetHealth());
  if (playerinfo->GetCustomInfo(TFPLAYERINFO_DISPENSER_METAL, value,
                                emptyVariant))
    Msg("Metal: %i\n", value.Int());
}
void TeleporterStatus(edict_t *pEntity) {
  ...
  edict_t *pExit = NULL;
  if (playerinfo->GetCustomInfo(TFPLAYERINFO_ENTINDEX_TELEPORTER_ENTRANCE,
                                value, emptyVariant)) {
  ...
  if (playerinfo->GetCustomInfo(TFPLAYERINFO_ENTINDEX_TELEPORTER_EXIT, value,
                                emptyVariant)) {
  ...
  if (pEntrance && entranceInfo) {
    if (playerinfo->GetCustomInfo(TFPLAYERINFO_BUILDING_TELEPORTER_ENTRANCE,
                                  value, emptyVariant)) {
      if (value.Bool())
        Msg("Entrance Under Construction...\n");
    }
    Msg("Entrance Health: %i\n", entranceInfo->GetHealth());
    if (playerinfo->GetCustomInfo(TFPLAYERINFO_TELEPORTER_USES, value,
                                  emptyVariant))
      Msg("Entrance Used %i Times.\n", value.Int());
  }
  if (pExit && exitInfo) {
    if (playerinfo->GetCustomInfo(TFPLAYERINFO_BUILDING_TELEPORTER_EXIT, value,
                                  emptyVariant)) {
      if (value.Bool())
...
void ClassStatus(edict_t *pEntity) {
  ...
  pluginvariant emptyVariant;
  if (!playerinfo->GetCustomInfo(TFPLAYERINFO_CONDITIONS, conditionValue,
                                 emptyVariant)) {
  ...
  case TF_CLASS_SPY: {
    ...
    if (playerinfo->GetCustomInfo(TFPLAYERINFO_SPY_DISGUISEDAS, value,
                                  emptyVariant))
    ...
    if (TFPlayerHasCondition(conditionValue.Int(), TF_COND_STEALTHED))
      Msg("Cloaked!\n");
    if (playerinfo->GetCustomInfo(TFPLAYERINFO_SPY_CLOAKCHARGELEVEL, value,
                                  emptyVariant))
...
void FlagStatus(edict_t *pPlayer) {
  ...
  edict_t *pFlag = NULL;
  while ((pFlag = entityinfomanager->FindEntityByClassname(
              pFlag, "item_teamflag")) != NULL) {
...
//---------------------------------------------------------------------------------
// Purpose: called when a client types in a command (only a subset of commands
...
PLUGIN_RESULT CEmptyServerPlugin::ClientCommand(edict_t *pEntity,
  ...
  if (FStrEq(pcmd, "menu")) {
    ...
    kv->SetString(
        "msg",
        "This is a long long long text string.\n\nIt also has line breaks.");
    ...
    kv->SetString(
        "command",
        "say"); // anything they enter into the dialog turns into a say command
    kv->SetInt("level", 1);
    kv->SetInt("time", 20);
    ...
    Msg("Game Type: %i / %s\n", gameInfo->GetInfo_GameType(),
        gameInfo->GetInfo_GameTypeName());
    ...
    for (int i = 0; i < teamCount; i++) {
      ...
      if (!gameInfo->GetInfo_GetTeamName(i))
        continue;
      Msg("Team: %s, Players: %i\n", gameInfo->GetInfo_GetTeamName(i),
          gameInfo->GetInfo_NumPlayersOnTeam(i));
    }
    return PLUGIN_STOP;
  ...
  // Sample to use the new CustomInfo added to TF2 for plugins
    ...
    Msg("Disguising?: %s\n",
        TFPlayerHasCondition(conditionValue.Int(), TF_COND_DISGUISING) ? "yes"
                                                                       : "no");
    Msg("Disguised?: %s\n",
        TFPlayerHasCondition(conditionValue.Int(), TF_COND_DISGUISED) ? "yes"
                                                                      : "no");
    Msg("Stealthed?: %s\n",
        TFPlayerHasCondition(conditionValue.Int(), TF_COND_STEALTHED) ? "yes"
                                                                      : "no");
    Msg("Invulnerable?: %s\n",
        TFPlayerHasCondition(conditionValue.Int(), TF_COND_INVULNERABLE)
            ? "yes"
            : "no");
    Msg("Teleported Recently?: %s\n",
        TFPlayerHasCondition(conditionValue.Int(), TF_COND_TELEPORTED) ? "yes"
                                                                       : "no");
    Msg("Selected for Teleportation?: %s\n",
        TFPlayerHasCondition(conditionValue.Int(), TF_COND_SELECTED_TO_TELEPORT)
            ? "yes"
            : "no");
    Msg("On Fire?: %s\n",
        TFPlayerHasCondition(conditionValue.Int(), TF_COND_BURNING) ? "yes"
                                                                    : "no");
    ...
    CBaseEntity *pEnt =
        static_cast<CBaseEntity *>(entityinfomanager->GetEntity(pEntity));
    if (pEnt)
...
//---------------------------------------------------------------------------------
PLUGIN_RESULT CEmptyServerPlugin::NetworkIDValidated(const char *pszUserName,
                                                     const char *pszNetworkID) {
  return PLUGIN_CONTINUE;
}
...
//---------------------------------------------------------------------------------
static ConVar empty_cvar("plugin_empty", "0", FCVAR_NOTIFY,
                         "Example plugin cvar");
