...
extern IPlayerInfoManager
    *playerinfomanager; // game dll interface to interact with players
extern IServerPluginHelpers
    *helpers; // special 3rd party plugin helpers from the engine

extern CGlobalVars *gpGlobals;

ConVar bot_forcefireweapon("plugin_bot_forcefireweapon", "", 0,
                           "Force bots with the specified weapon to fire.");
ConVar bot_forceattack2("plugin_bot_forceattack2", "0", 0,
                        "When firing, use attack2.");
ConVar bot_forceattackon("plugin_bot_forceattackon", "0", 0,
                         "When firing, don't tap fire, hold it down.");
ConVar bot_flipout("plugin_bot_flipout", "0", 0,
                   "When on, all bots fire their guns.");
ConVar bot_changeclass("plugin_bot_changeclass", "0", 0,
                       "Force all bots to change to the specified class.");
static ConVar bot_mimic("plugin_bot_mimic", "0", 0,
                        "Bot uses usercmd of player by index.");
static ConVar bot_mimic_yaw_offset("plugin_bot_mimic_yaw_offset", "0", 0,
                                   "Offsets the bot yaw.");

ConVar bot_sendcmd("plugin_bot_sendcmd", "", 0,
...
public:
  CPluginBot()
      : m_bBackwards(0), m_flNextTurnTime(0), m_bLastTurnToRight(0),
        m_flNextStrafeTime(0), m_flSideMove(0), m_ForwardAngle(),
        m_LastAngles() {}
...
void Bot_RunAll(void) {
  ...
  for (int i = 0; i < s_Bots.Count(); i++) {
    ...
    if (bot.m_BotEdict->IsFree() || !bot.m_BotEdict->GetUnknown() ||
        !bot.m_PlayerInfo->IsConnected()) {
      s_Bots.Remove(i);
      --i;
    } else {
      Bot_Think(&bot);
...
bool Bot_RunMimicCommand(CBotCmd &cmd) {
  ...
  IPlayerInfo *playerInfo = playerinfomanager->GetPlayerInfo(
      engine->PEntityOfEntIndex(bot_mimic.GetInt()));
  if (!playerInfo)
    return false;
...
void Bot_Think(CPluginBot *pBot) {
  ...
  // Finally, override all this stuff if the bot is being forced to mimic a
  // player.
  if (!Bot_RunMimicCommand(cmd)) {
    ...
    if (!pBot->m_PlayerInfo->IsDead()) {
      ...
      if (!pBot->m_BotInterface->IsEFlagSet(EFL_BOT_FROZEN) &&
          pBot->m_PlayerInfo->GetHealth() == 100) {
