...
public:
  virtual void ErrorMessage(const char *pMsg) { Msg("ERROR: %s\n", pMsg); }
...
void ErrFunction(char const *pMsg) { Msg("%s", pMsg); }
...
static char const *GetWMessageName(WMessage *pMessages, int nMessages,
                                   int msg) {
  static char str[128];
  int i;
...
template <class T> static void VCRTrace_ReadVal(T &val) {
  ...
  while (!g_bEndOfFile) {
    ...
    case VCREvent_recvfrom: {
      ...
      if (ret == SOCKET_ERROR) {
        ...
        Msg("recvfrom: %d (error %s)\n", ret,
            GetWMessageName(
                g_WinsockMessages,
                sizeof(g_WinsockMessages) / sizeof(g_WinsockMessages[0]), err));
      } else {
        Msg("recvfrom: %d\n", ret);
    ...
    case VCREvent_PeekMessage: {
      ...
      Msg("PeekMessage - msg: %s, wParam: %x, lParam: %x\n",
          GetWMessageName(g_WMessages,
                          sizeof(g_WMessages) / sizeof(g_WMessages[0]),
                          msg.message),
    ...
    case VCREvent_GameMsg: {
      ...
      if (valid) {
        ...
        Msg("GameMsg - msg: %s, wParam: %x, lParam: %x\n",
            GetWMessageName(g_WMessages,
                            sizeof(g_WMessages) / sizeof(g_WMessages[0]), uMsg),
            wParam, lParam);
      } else {
        Msg("GameMsg - <end>\n");
      }
