...
bool com_ignorecolons =
    false; // YWB:  Ignore colons as token separators in COM_Parse
...
// Purpose:
//-----------------------------------------------------------------------------
void CC_UngetToken(void) { s_com_token_unget = true; }
...
skipwhite:
  ...
  if (c == '\"') {
    ...
    while (1) {
      Assert(len < 1024);
      c = *data++;
      if ((c == '\"' && !bEscapeSequence) ||
          !c && len < sizeof(com_token) - 1) {
        com_token[len] = 0;
        return data;
      }
      bEscapeSequence = (c == '\\');
...
// Input  : *buffer -
//-----------------------------------------------------------------------------
void COM_FreeFile(unsigned char *buffer) { delete[] buffer; }
...
// Output : char
//-----------------------------------------------------------------------------
char *CC_DiscardUntilMatchingCharIncludingNesting(char *input,
                                                  const char *pairing) {
  int nestcount = 1;
