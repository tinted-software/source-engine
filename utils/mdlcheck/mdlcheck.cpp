...
bool GetModelNameFromSourceFile(char const *filename, char *modelname,
                                int maxlen) {
  modelname[0] = 0;
...
bool AddModelNameFromSource(CUtlDict<ModelFile, int> &models,
                            char const *filename, char const *modelname,
                            int offset) {
...
void ProcessSourceDirectory(char const *basedir) {
  ...
  for (int i = 0; i < c; i++) {
    ...
    if (GetModelNameFromSourceFile(filename, modelname, sizeof(modelname))) {
      if (AddModelNameFromSource(g_Analysis.models, filename, modelname,
                                 offset)) {
        valid++;
      }
    }
...
bool ValidateModelFile(char const *modelname, int offset) {
  ...
  if (pHdr->id != IDSTUDIOHEADER) {
    vprint_queued(
        0, "Bogus studiomdl header for %s, expecting 'IDST' four cc code\n",
        shortname);
    return false;
  }
  ...
  if (checkani) {
    // HACK: since the sequence data is written after the animation data, this
    // is rough way to determine how much anim data there really is
    int totalanimsize = pHdr->localseqindex - pHdr->localanimindex -
                        pHdr->numlocalanim * sizeof(mstudioanimdesc_t);
...
void ProcessModelsDirectory(char const *basedir) {
  ...
  // Now iterate model files and check version tag and whether a .qc exists
  // which builds the .mdl
...
void CheckForUnbuiltModels() {
  ...
  for (int i = 0; i < c; i++) {
    if (g_Analysis.models[i].version == 0) {
      ...
      vprint_queued(
          0,
          "%s needs $animblocksize command (%d of animdata),\n\tbuilt by %s\n",
