...
SRes LzmaEncode(const Byte *inBuffer, size_t inSize, Byte *outBuffer,
                size_t outSize, size_t *outSizeProcessed) {
...
unsigned char *LZMA_Compress(unsigned char *pInput, unsigned int inputSize,
  ...
  *pOutputSize = 0;
  ...
  int result =
      LzmaEncode(pInput, inputSize, pOutputBuffer + sizeof(lzma_header_t),
  ...
  memcpy(pHeader->properties, pOutputBuffer + sizeof(lzma_header_t),
         LZMA_PROPS_SIZE);
...
//-----------------------------------------------------------------------------
bool LZMA_Uncompress(unsigned char *pInBuffer, unsigned char **ppOutBuffer,
  ...
  *pOutSize = 0;
  ...
  ELzmaStatus status;
  SRes result = LzmaDecode((Byte *)pOutBuffer, &outProcessed,
                           (Byte *)(pInBuffer + sizeof(lzma_header_t)),
