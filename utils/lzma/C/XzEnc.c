...
static SRes WriteBytesAndCrc(ISeqOutStream *s, const void *buf, UInt32 size,
                             UInt32 *crc) {
...
SRes Xz_AddIndexRecord(CXzStream *p, UInt64 unpackSize, UInt64 totalSize,
                       ISzAlloc *alloc) {
    ...
    }
    {
      ...
      *size = sizeOriginal;
      res = p->StateCoder.Code(p->StateCoder.p, data, size, p->buf + p->curPos,
                               &srcLen, p->srcWasFinished, CODER_FINISH_ANY,
                               &wasFinished);
      p->curPos += srcLen;
...
static void Lzma2WithFilters_Construct(CLzma2WithFilters *p, ISzAlloc *alloc,
                                       ISzAlloc *bigAlloc) {
  p->alloc = alloc;
  p->bigAlloc = bigAlloc;
  p->lzma2 = NULL;
