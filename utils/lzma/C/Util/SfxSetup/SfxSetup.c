...
static const char *kNames[] = {"setup", "install", "run", "start"};
...
static unsigned FindItem(const char **items, unsigned num, const wchar_t *s,
                         unsigned len) {
  unsigned i;
  for (i = 0; i < num; i++) {
...
#endif
{
...

  {
    ...
    if (winRes == 0 || winRes > MAX_PATH)
      return 1;
    pathLen = wcslen(path);
    d = (GetTickCount() << 12) ^ (GetCurrentThreadId() << 14) ^
        GetCurrentProcessId();
  ...
  if (res == SZ_OK) {
    ...
    UInt32 i;
    UInt32 blockIndex = 0xFFFFFFFF; /* it can have any value before first call
                                       (if outBuffer = 0) */
    Byte *outBuffer =
    ...
    for (i = 0; i < db.NumFiles; i++) {
      ...
      }
      {
        ...
        if (SzArEx_IsDir(&db, i)) {
          ...
          unsigned extPrice = FindItem(kExts, sizeof(kExts) / sizeof(kExts[0]),
                                       name + len - extLen, extLen);
          unsigned namePrice = FindItem(
              kNames, sizeof(kNames) / sizeof(kNames[0]), name, nameLen);
  ...
  if (res == SZ_OK) {
    ...
    if (useShellExecute) {
      ...
      if (!success || (executeRes <= 32 &&
                       executeRes != 0)) /* executeRes = 0 in Windows CE */
        res = SZ_ERROR_FAIL;
      else
