...
#ifdef _WIN32
  {
      ...
      return SZ_ERROR_MEM;
    {
      ...
      if (len != 0) {
        ...
        BOOL defUsed;
        unsigned numChars = 0;
        numChars = WideCharToMultiByte(codePage, 0, s, len, (char *)buf->data,
...
void PrintError(const char *sz) { printf("\nERROR: %s\n", sz); }
...
#else
static void GetAttribString(UInt32, Bool, char *s) { s[0] = '\0'; }
#endif
...
int MY_CDECL main(int numargs, char *args[]) {
  ...
  if (numargs == 1) {
    printf("Usage: 7zDec <command> <archive_name>\n\n"
           "<Commands>\n"
      ...
      for (i = 0; i < db.NumFiles; i++) {
        ...
        */
        ...
        if (listCommand) {
          ...
          GetAttribString(
              SzBitWithVals_Check(&db.Attribs, i) ? db.Attribs.Vals[i] : 0,
              isDir, attr);
        ...
        fputs(testCommand ? "Testing    " : "Extracting ", stdout);
        res = PrintString(temp);
        if (res != SZ_OK)
          break;
        if (isDir)
