...
/*
static int SzFolder_FindBindPairForInStream(const CSzFolder *p, UInt32
inStreamIndex)
{
  UInt32 i;
...
/*
static MY_NO_INLINE const Byte *SzReadNumbers(const Byte *data, const Byte
*dataLim, UInt64 *values, UInt32 num)
{
  for (; num != 0; num--)
  {
    Byte firstByte;
...
static SRes SzReadHeader2(CSzArEx *p, /* allocMain */
                          ...
                          ILookInStream *inStream, CBuf *tempBufs,
...
static UInt64 SzAr_GetFolderUnpackSize(const CSzAr *p, UInt32 folderIndex)
{
  ...
  // return p->CoderUnpackSizes[f->StartCoderUnpackSizesIndex +
f->IndexOfMainOutStream];
      ...
      if (res == SZ_OK) {
        ...
        if (res == SZ_OK) {
          if (SzBitWithVals_Check(&p->db.FolderCRCs, folderIndex)) {
            if (CrcCalc(*tempBuf, unpackSize) !=
                p->db.FolderCRCs.Vals[folderIndex])
              res = SZ_ERROR_CRC;
          }
        }
      }
  ...
  if (res == SZ_OK) {
    ...
    *outSizeProcessed = (size_t)(p->UnpackPositions[fileIndex + 1] - unpackPos);
    ...
    if (SzBitWithVals_Check(&p->CRCs, fileIndex) &&
        CrcCalc(*tempBuf + *offset, *outSizeProcessed) !=
