...
void BraState_Free(void *pp, ISzAlloc *alloc) { alloc->Free(alloc, pp); }

SRes BraState_SetProps(void *pp, const Byte *props, size_t propSize,
                       ISzAlloc *alloc) {
...
static SRes BraState_Code(void *pp, Byte *dest, SizeT *destLen, const Byte *src,
                          SizeT *srcLen, int srcWasFinished,
                          ECoderFinishMode finishMode, int *wasFinished) {
  CBraState *p = ((CBraState *)pp);
...
SRes BraState_SetFromMethod(IStateCoder *p, UInt64 id, int encodeMode,
  ...
  if (id != XZ_ID_Delta && id != XZ_ID_X86 && id != XZ_ID_PPC &&
      id != XZ_ID_IA64 && id != XZ_ID_ARM && id != XZ_ID_ARMT &&
      id != XZ_ID_SPARC)
    return SZ_ERROR_UNSUPPORTED;
  p->p = 0;
...
static SRes SbState_Code(void *pp, Byte *dest, SizeT *destLen, const Byte *src,
                         SizeT *srcLen, int srcWasFinished,
                         ECoderFinishMode finishMode, int *wasFinished) {
  CSbDec *p = (CSbDec *)pp;
  SRes res;
...
static void Lzma2State_Init(void *pp) { Lzma2Dec_Init((CLzma2Dec *)pp); }

static SRes Lzma2State_Code(void *pp, Byte *dest, SizeT *destLen,
...
static Bool Xz_CheckFooter(CXzStreamFlags flags, UInt64 indexSize,
                           const Byte *buf) {
  ...
  *status = CODER_STATUS_NOT_SPECIFIED;
  for (;;) {
    ...
    if (p->state == XZ_STATE_BLOCK) {
      ...
      res = MixCoder_Code(&p->decoder, dest, &destLen2, src, &srcLen2, False,
                          finishMode, status);
      ...
      if (*status == CODER_STATUS_FINISHED_WITH_MARK) {
        Byte temp[32];
        unsigned num =
            Xz_WriteVarInt(temp, p->packSize + p->blockHeaderSize +
