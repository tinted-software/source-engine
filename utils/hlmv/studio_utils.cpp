...
void *StudioModel::operator new(size_t stAllocateBlock, int nBlockUse,
...
void StudioModel::operator delete(void *pMem, int nBlockUse,
                                  const char *pFileName, int nLine) {
#ifdef _DEBUG
  // set the memory to a known value
  int size = _msize(pMem);
...
bool StudioModel::LoadModel(const char *pModelName) {
  ...
  m_pStudioHdr =
      new CStudioHdr(g_pMDLCache->GetStudioHdr(m_MDLHandle), g_pMDLCache);
  ...
  for (int lodID = pHardwareData->m_RootLOD; lodID < pHardwareData->m_NumLODs;
       lodID++) {
...
//------------------------------------------------------------------------------
// Returns true if the model has at least one body part with model data, false
// if not.
//------------------------------------------------------------------------------
...
int StudioModel::GetSequence() { return m_sequence; }
...
void StudioModel::ClearAnimationLayers(void) { m_iActiveLayers = 0; }
...
float StudioModel::SetOverlayRate(int iLayer, float flCycle,
...
void StudioModel::StartBlending(void) {
  // Switch back to old sequence ( this will oscillate between this one and the
  // last one )
...
LocalFlexController_t StudioModel::LookupFlexController(char *szName) {
  ...
  for (LocalFlexController_t iFlex = LocalFlexController_t(0);
       iFlex < pStudioHdr->numflexcontrollers(); iFlex++) {
...
void StudioModel::SetFlexController(LocalFlexController_t iFlex,
                                    float flValue) {
  CStudioHdr *pStudioHdr = GetStudioHdr();
  if (!pStudioHdr)
    return;
...
void StudioModel::SetFlexControllerRaw(LocalFlexController_t iFlex,
                                       float flValue) {
  CStudioHdr *pStudioHdr = GetStudioHdr();
  if (!pStudioHdr)
    return;
  ...
  if (iFlex >= 0 && iFlex < pStudioHdr->numflexcontrollers()) {
    ...
    //pStudioHdr->pFlexcontroller(iFlex);
    m_flexweight[iFlex] = clamp(flValue, 0.0f, 1.0f);
  }
}
...
void StudioModel::GetSequenceInfo(int iSequence, float *pflFrameRate,
...
float StudioModel::GetFPS(void) { return GetFPS(m_sequence); }
...
float StudioModel::GetDuration() { return GetDuration(m_sequence); }

void StudioModel::GetMovement(float prevcycle[5], Vector &vecPos,
                              QAngle &vecAngles) {
  vecPos.Init();
  vecAngles.Init();
  ...
  Studio_SeqMovement(pStudioHdr, m_sequence, prevcycle[0], m_cycle,
                     m_poseparameter, vecPos, vecAngles);
  prevcycle[0] = m_cycle;
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (m_Layer[i].m_weight > 0.0) {
      vecTmp.Init();
      angTmp.Init();
      if (Studio_SeqMovement(pStudioHdr, m_Layer[i].m_sequence,
                             prevcycle[i + 1], m_Layer[i].m_cycle,
                             m_poseparameter, vecTmp, angTmp)) {
        vecPos =
            vecPos * (1.0 - m_Layer[i].m_weight) + vecTmp * m_Layer[i].m_weight;
      }
    }
    prevcycle[i + 1] = m_Layer[i].m_cycle;
  }
...
void StudioModel::GetMovement(int iSequence, float prevCycle, float nextCycle,
                              Vector &vecPos, QAngle &vecAngles) {
  CStudioHdr *pStudioHdr = GetStudioHdr();
  if (!pStudioHdr) {
    vecPos.Init();
    vecAngles.Init();
  ...
  Studio_SeqMovement(pStudioHdr, iSequence, prevCycle, nextCycle,
                     m_poseparameter, vecPos, vecAngles);
...
//-----------------------------------------------------------------------------
float StudioModel::GetGroundSpeed(void) { return GetGroundSpeed(m_sequence); }
...
float StudioModel::GetCurrentVelocity(void) {
  ...
  CStudioHdr *pStudioHdr = GetStudioHdr();
  if (pStudioHdr && Studio_SeqVelocity(pStudioHdr, m_sequence, m_cycle,
                                       m_poseparameter, vecVelocity)) {
...
void StudioModel::GetSeqAnims(int iSequence, mstudioanimdesc_t *panim[4],
                              float *weight) {
  CStudioHdr *pStudioHdr = GetStudioHdr();
  if (!pStudioHdr)
    return;
  ...
  Studio_SeqAnims(pStudioHdr, seqdesc, iSequence, m_poseparameter, panim,
                  weight);
}
...
float StudioModel::SetController(int iController, float flValue) {
  ...
  return Studio_SetController(pStudioHdr, iController, flValue,
...
int StudioModel::LookupPoseParameter(char const *szName) {
  ...
  for (int iParameter = 0; iParameter < pStudioHdr->GetNumPoseParameters();
       iParameter++) {
    if (stricmp(szName, pStudioHdr->pPoseParameter(iParameter).pszName()) ==
        0) {
      return iParameter;
    }
  }
...
float StudioModel::SetPoseParameter(int iParameter, float flValue) {
  ...
  return Studio_SetPoseParameter(pStudioHdr, iParameter, flValue,
                                 m_poseparameter[iParameter]);
}
...
float *StudioModel::GetPoseParameters() { return m_poseparameter; }
...
bool StudioModel::GetPoseParameterRange(int iParameter, float *pflMin,
                                        float *pflMax) {
  *pflMin = 0;
  *pflMax = 0;
...
int StudioModel::SetBodygroup(int iGroup, int iValue /*= -1*/) {
  ...
  // if the submodel index is not specified or out of range, just use the
  // current value
  if (iValue < 0 || iValue >= pbodypart->nummodels)
    return iCurrent;

  m_bodynum =
      (m_bodynum - (iCurrent * pbodypart->base) + (iValue * pbodypart->base));
...
int StudioModel::Physics_GetBoneCount(void) { return m_pPhysics->Count(); }
...
void StudioModel::Physics_GetData(
    int boneIndex, hlmvsolid_t *psolid,
    constraint_ragdollparams_t *pConstraint) const {
...
void StudioModel::Physics_SetData(
    int boneIndex, const hlmvsolid_t *psolid,
...
float StudioModel::Physics_GetMass(void) { return m_pPhysics->GetMass(); }

void StudioModel::Physics_SetMass(float mass) { m_physMass = mass; }

char *StudioModel::Physics_DumpQC(void) { return m_pPhysics->DumpQC(); }

const vertexFileHeader_t *mstudiomodel_t::CacheVertexData(void *pModelData) {
  Assert(pModelData == NULL);
  Assert(g_pActiveModel);

  return g_pStudioDataCache->CacheVertexData(
      g_pActiveModel->GetStudioRenderHdr());
}
