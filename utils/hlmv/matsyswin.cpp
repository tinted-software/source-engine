...
Vector g_vright(
    50, 50, 0); // needs to be set to viewer's right in order for chrome to work
...
MatSysWindow::MatSysWindow(mxWindow *parent, int x, int y, int w, int h,
  ...
  m_pCubemapTexture =
      g_pMaterialSystem->FindTexture("hlmv/cubemap", NULL, true);
  ...
  // can't do it from the render context
  {
    ...
    g_materialWireframeVertexColorNoCull = g_pMaterialSystem->CreateMaterial(
        "debug/wireframenocull", pVMTKeyValues);
  ...
  g_materialFlatshaded = g_pMaterialSystem->FindMaterial(
      "debug/debugdrawflatpolygons", TEXTURE_GROUP_OTHER, true);
  if (g_materialFlatshaded) {
    g_materialFlatshaded->AddRef();
  }
  g_materialSmoothshaded = g_pMaterialSystem->FindMaterial(
      "debug/debugmrmfullbright2", TEXTURE_GROUP_OTHER, true);
  if (g_materialSmoothshaded) {
    g_materialSmoothshaded->AddRef();
  }
  g_materialBones = g_pMaterialSystem->FindMaterial("debug/debugskeleton",
  ...
  g_materialFloor =
      g_pMaterialSystem->FindMaterial("hlmv/floor", TEXTURE_GROUP_OTHER, true);
  if (g_materialFloor) {
    g_materialFloor->AddRef();
  }
  g_materialVertexColor = g_pMaterialSystem->FindMaterial(
      "debug/debugvertexcolor", TEXTURE_GROUP_OTHER, true);
  if (g_materialVertexColor) {
    g_materialVertexColor->AddRef();
  }
  g_materialShadow =
      g_pMaterialSystem->FindMaterial("hlmv/shadow", TEXTURE_GROUP_OTHER, true);
  if (g_materialShadow) {
...
int MatSysWindow::handleEvent(mxEvent *event) {
  ...
  case mxEvent::Idle: {
    ...
    if (prev != 0.0) {
      ...
      if (g_viewerSettings.animateWeapons) {
        const char *pszMainSequenceName =
            g_pStudioModel->GetSequenceName(g_pStudioModel->GetSequence());
        if (pszMainSequenceName) {
          for (int i = 0; i < HLMV_MAX_MERGED_MODELS; i++) {
            ...
            int iSequence =
                g_pStudioExtraModel[i]->LookupSequence(pszMainSequenceName);
            if (iSequence == -1) {
              g_pStudioExtraModel[i]->SetFrame(0);
            } else {
              g_pStudioExtraModel[i]->SetSequence(iSequence);
              g_pStudioExtraModel[i]->SetFrame(
                  g_pStudioModel->GetCycle() *
  ...
  case mxEvent::MouseDown: {
    ...
    float d = sqrt((float)((event->x - w() / 2) * (event->x - w() / 2) +
                           (event->y - h() / 2) * (event->y - h() / 2)));
    ...
    if (event->buttons & mxEvent::MouseLeftButton) {
      if (event->modifiers & mxEvent::KeyShift) {
        ...
        if (!g_viewerSettings.rotating) {
          ...
          float ang1 =
              (180 / 3.1415) * atan2(oldx - w() / 2.0, oldy - h() / 2.0);
          float ang2 = (180 / 3.1415) *
                       atan2(event->x - w() / 2.0, event->y - h() / 2.0);
          oldx = event->x;
          oldy = event->y;
...
void DrawGroundPlane() {
  ...
  g_pStudioModel->GetMovement(g_pStudioModel->m_prevGroundCycles, deltaPos,
                              deltaAngles);
  ...
  meshBuilder.TexCoord2f(0, tMap.x + (-dxMap.x - dyMap.x) * scale,
                         tMap.y + (dxMap.y + dyMap.y) * scale);
...
void DrawMovementBoxes() {
  ...
  // starting position
  {
    ...
    g_pStudioModel->drawTransparentBox(bboxMin, bboxMax, mStart, color,
                                       wirecolor);
  }
  ...
  // current position
  {
    ...
    g_pStudioModel->drawTransparentBox(bboxMin, bboxMax, mTemp, color,
                                       wirecolor);
  }
...
char const *HLMV_TranslateSoundName(char const *soundname, StudioModel *model) {
  ...
  if (model) {
    return PSkipSoundChars(g_pSoundEmitterBase->GetWavFileForSound(
        soundname, model->GetFileName()));
  }

  return PSkipSoundChars(
      g_pSoundEmitterBase->GetWavFileForSound(soundname, NULL));
}
...
void PlaySound(const char *pSoundName, StudioModel *pStudioModel) {
  ...
  const char *pSoundFileName =
      HLMV_TranslateSoundName(pSoundName, pStudioModel);
    ...
    case CL_EVENT_FOOTSTEP_RIGHT: {
      ...
      /*
                      case AE_CL_PLAYSOUND:
                              ...
                              {
                                      Warning( "-- Error --:  empty soundname,
         .qc error on AE_CL_PLAYSOUND in model %s, sequence %s, animevent #
...
void MatSysWindow::draw() {
  ...
  UpdateSounds(); // need to call this multiple times per frame to avoid audio
                  // stuttering
  ...
  pRenderContext->ClearColor3ub(g_viewerSettings.bgColor[0] * 255,
                                g_viewerSettings.bgColor[1] * 255,
  ...
  pRenderContext->LoadIdentity();
  pRenderContext->PerspectiveX(g_viewerSettings.fov, (float)w() / (float)h(),
                               1.0f, 20000.0f);
  ...
  g_pStudioModel->AddLookTarget(
      Vector(0, 0, 0), g_pStudioModel->GetSolveHeadTurn() ? 1.0f : 0.0f);
  ...
  if (pStudioHdr != NULL) {
    ...
    if (pHardwareData != NULL) {
      studioloddata_t *pLODData =
          &pHardwareData->m_pLODs[pHardwareData->m_RootLOD];
      for (int meshID = 0; meshID < pHardwareData->m_NumStudioMeshes;
           meshID++) {
...
MatSysWindow::loadTexture (const char *filename, int name)
{
        if (!filename || !strlen (filename))
        {
                ...
                {
                        glDeleteTextures (1, (const GLuint *)
&d_textureNames[name]); d_textureNames[name] = 0;
        ...
        if (image)
        {
                ...
                if (image->bpp == 8)
                {
                        ...
                        g_pStudioModel->UploadTexture (&texture, (byte *)
image->data, (byte *) image->palette, name + 1);
                }
                else
                {
                        glBindTexture (GL_TEXTURE_2D, d_textureNames[name]);
                        glTexImage2D (GL_TEXTURE_2D, 0, 3, image->width,
image->height, 0, GL_RGB, GL_UNSIGNED_BYTE, image->data); glTexEnvf
(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE); glTexParameterf
(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR); glTexParameterf
...
void MatSysWindow::dumpViewport(const char *filename) {
  ...
  if (image->create(w, h, 24)) {
    ...
    if (!mxTgaWrite(filename, image))
      mxMessageBox(this, "Error writing screenshot.", g_appTitle,
