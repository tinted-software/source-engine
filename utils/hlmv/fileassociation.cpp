...
FileAssociation::FileAssociation()
  ...
  rbAction[0] =
      new mxRadioButton(this, 10, 50, 120, 22, "program", IDC_ACTION1, true);
  rbAction[1] = new mxRadioButton(this, 10, 72, 120, 22, "associated program",
                                  IDC_ACTION2);
  rbAction[2] =
      new mxRadioButton(this, 10, 94, 120, 22, "HLMV default", IDC_ACTION3);
...
void FileAssociation::setAssociation(int index) {
  ...
  /*
          WORD dw = 0;
          HICON hIcon = ExtractAssociatedIcon ((HINSTANCE) GetWindowLong ((HWND)
     getHandle (), GWL_HINSTANCE), path, &dw); SendMessage ((HWND) getHandle (),
     WM_SETICON, (WPARAM) ICON_SMALL, (LPARAM) hIcon); char str[32]; sprintf
     (str, "%d", (int) hIcon); setLabel (str);
  */
  DeleteFile(path);
...
void FileAssociation::saveAssociations() {
  ...
  for (int i = 0; i < 16; i++) {
    ...
    fprintf(file, "\"%s\" \"%s\" %d\n", d_associations[i].extension,
            d_associations[i].program, d_associations[i].association);
  }
