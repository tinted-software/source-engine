...
// hack!  Should probably use the gamma stuff in mathlib since we already
// linking it.
...
void StudioModel::RunFlexRules() {
  ...
  for (LocalFlexController_t i = LocalFlexController_t(0);
       i < pSrcStudioHdr->numflexcontrollers(); i++) {
    ...
    if (j >= 0 && j < MAXSTUDIOFLEXCTRL * 4) {
      src[j] =
          pSrcModel->m_flexweight[i] * (pflex->max - pflex->min) + pflex->min;
    }
  }
