...
CAttachmentsWindow::CAttachmentsWindow(ControlPanel *pParent)
...
void CAttachmentsWindow::Init() {
  ...
  m_cAttachmentList =
      new mxListBox(this, left, top + 20, 260, 100, IDC_ATTACHMENT_LIST);
  ...
  m_cBoneList =
      new mxListBox(this, left, top + 20, 260, 100, IDC_ATTACHMENT_LIST_BONES);
  ...
  m_cTranslation = new mxLineEdit2(this, left + 70, top, 90, 25, "10 20 30",
                                   IDC_ATTACHMENT_TRANSLATION);

  left = 170;
  top = 120;
  new mxLabel(this, left + 3, top + 4, 60, 18, "Rotation");
  m_cRotation = new mxLineEdit2(this, left + 70, top, 90, 25, "0 90 180",
                                IDC_ATTACHMENT_ROTATION);

  top = 145;
  left = 5;
  new mxLabel(this, left, top, 60, 18, "QC String");
  m_cQCString = new mxLineEdit2(
      this, left + 70, top, 400, 25,
      "$attachment \"controlpanel0_ur\" \"Vgui\" -22 -15 4 rotate 0 0 0",
      IDC_ATTACHMENT_QC_STRING);
}
  ...
  case IDC_ATTACHMENT_TRANSLATION: {
    ...
    if (iAttachment >= 0 && iAttachment < pHdr->GetNumAttachments()) {
      mstudioattachment_t &pAttachment =
          (mstudioattachment_t &)pHdr->pAttachment(iAttachment);
...
void CAttachmentsWindow::OnSelChangeAttachmentList() {
  CStudioHdr *pStudioHdr =
      g_pStudioModel ? g_pStudioModel->GetStudioHdr() : NULL;
...
Vector CAttachmentsWindow::GetCurrentTranslation() {
  ...
  int iAttachment = m_cAttachmentList->getSelectedIndex() - 1;
  if (pStudioHdr && iAttachment >= 0 &&
      iAttachment < pStudioHdr->GetNumAttachments()) {
    mstudioattachment_t &pAttachment =
        (mstudioattachment_t &)pStudioHdr->pAttachment(iAttachment);

    return Vector(pAttachment.local[0][3], pAttachment.local[1][3],
...
void CAttachmentsWindow::UpdateStrings(bool bUpdateQC, bool bUpdateTranslation,
  ...
  if (iAttachment == -1) {
    ...
    if (bUpdateQC) {
      sprintf(str,
              "$attachment \"%s\" \"%s\" %.2f %.2f %.2f rotate %.0f %.0f %.0f",
              pAttachment.pszName(), pHdr->pBone(iBone)->pszName(),
              VectorExpand(vTranslation), VectorExpand(vRotation));
