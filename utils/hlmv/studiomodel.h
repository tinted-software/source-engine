...
public:
  ...
  static void UpdateViewState(const Vector &viewOrigin, const Vector &viewRight,
                              const Vector &viewUp,
  ...
  static void UpdateStudioRenderConfig(bool bWireframe, bool bZBufferWireframe,
  ...
  void GetSequenceInfo(int iSequence, float *pflFrameRate,
  ...
  void GetMovement(int iSequence, float prevCycle, float currCycle,
  ...
  // void
  // CalcBoneTransform( int iBone, Vector pos[], Quaternion q[], matrix3x4_t&
  // bonematrix );

  void UpdateBoneChain(Vector pos[], Quaternion q[], int iBone,
                       matrix3x4_t *pBoneToWorld);
  void SetViewTarget(void); // ???
  void GetBodyPoseParametersFromFlex(void);
  void CalcHeadRotation(Vector pos[], Quaternion q[]);
  float SetHeadPosition(matrix3x4_t &attToWorld, Vector const &vTargetPos,
                        float dt);
  ...
  void Physics_GetData(int boneIndex, hlmvsolid_t *psolid,
                       constraint_ragdollparams_t *pConstraint) const;
  void Physics_SetData(int boneIndex, const hlmvsolid_t *psolid,
                       constraint_ragdollparams_t const *pConstraint);
  ...
  // Get and set the model transform (i.e. what m_origin and m_angles are used
  // to generate).
...
private:
  ...
  void DrawPhysmesh(CPhysmesh *pMesh, int boneIndex, IMaterial *pMaterial,
                    float *color);
...
public:
  ...
  void drawLine(Vector const &p1, Vector const &p2, int r = 0, int g = 0,
                int b = 255);
  void drawTransparentBox(Vector const &bbmin, Vector const &bbmax,
...
//-----------------------------------------------------------------------------
extern Vector
    g_vright; // needs to be set to viewer's right in order for chrome to work
