      ...
      if (event->flags & mxEvent::RightClicked) {
          ...
          if (!pak_ExtractFile(d_pakFile, d_currLumpName, str))
            ...
            if (program) {
              ...
              if ((int)WinExec(path, SW_SHOW) <= 32)
                mxMessageBox(this, "Error executing specified program.",
        ...
        // associated program
          ...
          if (!pak_ExtractFile(d_pakFile, d_currLumpName, str))
            mxMessageBox(this, "Error extracting from PAK file.", g_appTitle,
                         MX_MB_OK | MX_MB_ERROR);
          else if ((int)ShellExecute((HWND)getHandle(), "open", str, 0, 0,
                                     SW_SHOW) <= 32)
            mxMessageBox(this,
                         "Error executing document with associated program.",
                         g_appTitle, MX_MB_OK | MX_MB_ERROR);
        }
...
int PAKViewer::OnLoadModel() {
  ...
  if (!g_pStudioModel->LoadModel(str2)) {
    mxMessageBox(this, "Error reading model header.", g_appTitle,
                 MX_MB_OK | MX_MB_ERROR);
    return 1;
  }
