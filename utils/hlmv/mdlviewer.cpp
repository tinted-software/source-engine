...
bool g_bHlmvMaster =
    false; // This hlmv is controlling a controlled hlmv instance
bool g_bHlmvControlled =
    false; // This hlmv is being controlled by a master hlmv instance
...
AccelTableEntry_t accelTable[] = {
    {VK_F1, IDC_FLUSH_SHADERS, mx::ACCEL_VIRTKEY},
    {VK_F5, IDC_FILE_REFRESH, mx::ACCEL_VIRTKEY},
    {'u', IDC_FILE_UNLOADALLMERGEDMODELS,
     mx::ACCEL_CONTROL | mx::ACCEL_VIRTKEY},
    {'U', IDC_FILE_UNLOADALLMERGEDMODELS,
     mx::ACCEL_CONTROL | mx::ACCEL_VIRTKEY},
...
MDLViewer::MDLViewer() : mxWindow(0, 0, 0, 0, 0, g_appTitle, mxWindow::Normal) {
  ...
  menuUnloadWeapon->add("Unload All Merged Models (Ctrl-U)",
                        IDC_FILE_UNLOADALLMERGEDMODELS);
  ...
  for (int i = IDC_FILE_UNLOADMERGEDMODEL1; i <= IDC_FILE_UNLOADMERGEDMODEL12;
       i++) {
...
void MDLViewer::LoadModelFile(const char *pszFile, int slot) {
  ...
  if (slot == -1) {
    ...
    mb->modify(IDC_FILE_UNLOADMERGEDMODEL1 + slot,
               IDC_FILE_UNLOADMERGEDMODEL1 + slot, pszFile);
  ...
  case mxEvent::Action: {
    ...
    case IDC_FILE_UNLOADMERGEDMODEL12: {
      ...
      if (g_pStudioExtraModel[i]) {
        ...
        mb->modify(IDC_FILE_UNLOADMERGEDMODEL1 + i,
                   IDC_FILE_UNLOADMERGEDMODEL1 + i, "(empty)");
    ...
    case IDC_OPTIONS_LINKHLMV:
      if (!g_bHlmvMaster && !g_HlmvIpcServer.IsRunning() &&
          g_HlmvIpcClient.Connect()) {
        CUtlBuffer cmd;
        CUtlBuffer res;
    ...
    case IDC_HELP_GOTOHOMEPAGE:
      ShellExecute(0, "open",
                   "http://www.swissquake.ch/chumbalum-soft/index.html", 0, 0,
                   SW_SHOW);
      break;
#endif
...
#define HEIGHT 140
  ...
  case mxEvent::Timer: {
    if (g_HlmvIpcServer.HasCommands()) {
      // Execute next command at next msg pump, ~ 1/60th s (60Hz) if controlled,
      // 0.1s if not
      if (g_bHlmvControlled) {
        // Clear up to 10 pending commands if controlled
        for (int nCmdCount = 0; nCmdCount < 10 && g_HlmvIpcServer.HasCommands();
             ++nCmdCount) {
      ...
      // Idling, poll command queue @ ~60Hz if controlled, 0.5s ( 2Hz )
      // otherwise
      if (g_bHlmvControlled) {
        setTimer(17);
      } else {
...
void TranslateMayaToHLMVCoordinates(const Vector &vMayaPos,
                                    const QAngle &vMayaRot, Vector &vHLMVPos,
                                    QAngle &vHLMVAngles) {
...
void MDLViewer::handleIpcCommand(char *szCommand) {
  ...
  if (!strcmp("reload", szCommand)) {
    ...
    char szFirstPart[32];
    sscanf(szCommand, "%s %f %f %f %f %f %f", szFirstPart, &vMayaPos.x,
           &vMayaPos.y, &vMayaPos.z, &vMayaRot.x, &vMayaRot.y, &vMayaRot.z);

    //
    // Obviously, this could all be simplified, but it's nice to make it easy to
    ...
    TranslateMayaToHLMVCoordinates(vMayaPos, vMayaRot, vCameraPos,
                                   vCameraAngles);

    // Now, build a matrix from model space to camera space. The way we're
    // defining camera space, the axes point the same way as in model space
    // (+X=forward, +Y=left, +Z=up). This is a standard
    // put-stuff-in-camera-space matrix (backtranslate and then backrotate).
    ...
    // Now we need to convert the camera space from above to HLMV's specific
    // camera space. This just means negating the X and Y axes because HLMV's
    // camera space is (+X=back, +Y=left, +Z=up).
    ...
    // Tell HLMV our new fancy transform to use, and then we'll see the model
    // from the same place Maya did.
    ...
    sscanf(szCommand, "%*s %f %f %f %f %f %f %f %f %f %f %f %f",
           &m.m_flMatVal[0][0], &m.m_flMatVal[0][1], &m.m_flMatVal[0][2],
           &m.m_flMatVal[0][3], &m.m_flMatVal[1][0], &m.m_flMatVal[1][1],
           &m.m_flMatVal[1][2], &m.m_flMatVal[1][3], &m.m_flMatVal[2][0],
           &m.m_flMatVal[2][1], &m.m_flMatVal[2][2], &m.m_flMatVal[2][3]);
    ...
    sscanf(szCommand, "%*s %f %f %f", &g_viewerSettings.lightrot[0],
           &g_viewerSettings.lightrot[1], &g_viewerSettings.lightrot[2]);
...
void MDLViewer::SendModelTransformToLinkedHlmv() {
  if (g_bHlmvMaster && g_HlmvIpcClient.Connect()) {
    ...
    cmd.Printf("%s %f %f %f %f %f %f %f %f %f %f %f %f", "hlmvModelTransform",
               m.m_flMatVal[0][0], m.m_flMatVal[0][1], m.m_flMatVal[0][2],
               m.m_flMatVal[0][3], m.m_flMatVal[1][0], m.m_flMatVal[1][1],
               m.m_flMatVal[1][2], m.m_flMatVal[1][3], m.m_flMatVal[2][0],
               m.m_flMatVal[2][1], m.m_flMatVal[2][2], m.m_flMatVal[2][3]);
...
void MDLViewer::SendLightRotToLinkedHlmv() {
  if (g_bHlmvMaster && g_HlmvIpcClient.Connect()) {
    ...
    cmdLightRot.Printf(
        "%s %f %f %f", "hlmvLightRot", g_viewerSettings.lightrot[0],
        g_viewerSettings.lightrot[1], g_viewerSettings.lightrot[2]);
