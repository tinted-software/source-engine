...
int DebugDrawModel(IStudioRender *pStudioRender, DrawModelInfo_t &info,
                   matrix3x4_t *pBoneToWorld, const Vector &modelOrigin,
                   int flags) {
  // Make static so that we aren't reallocating everything all the time.
  // TODO: make sure that this actually keeps us from reallocating inside of
  // GetTriangles.
  ...
  for (batchID = 0; batchID < tris.m_MaterialBatches.Count(); batchID++) {
    ...
    meshBuilder.Begin(pBuildMesh, MATERIAL_TRIANGLES,
                      materialBatch.m_Verts.Count(),
    ...
    for (vertID = 0; vertID < materialBatch.m_Verts.Count(); vertID++) {
      ...
      for (k = 0; k < vert.m_NumBones; k++) {
        const matrix3x4_t &poseToWorld =
            tris.m_PoseToWorld[vert.m_BoneIndex[k]];
        Vector tmp;
...
int DebugDrawModelNormals(IStudioRender *pStudioRender, DrawModelInfo_t &info,
  ...
  for (batchID = 0; batchID < tris.m_MaterialBatches.Count(); batchID++) {
    ...
    meshBuilder.Begin(pBuildMesh, MATERIAL_LINES,
                      materialBatch.m_Verts.Count());
    ...
    for (vertID = 0; vertID < materialBatch.m_Verts.Count(); vertID++) {
      ...
      //			skinnedPos += skinnedNormal *
      //NORMAL_OFFSET_FROM_MESH;
      meshBuilder.Position3fv(&skinnedPos.x);
...
int DebugDrawModelTangentT(IStudioRender *pStudioRender, DrawModelInfo_t &info,
  ...
  for (batchID = 0; batchID < tris.m_MaterialBatches.Count(); batchID++) {
    ...
    for (vertID = 0; vertID < materialBatch.m_Verts.Count(); vertID++) {
      ...
      Vector skinnedTangentT =
          CrossProduct(skinnedNormal, skinnedTangentS.AsVector3D()) *
          skinnedTangentS.w;
...
int DebugDrawModelWireframe(IStudioRender *pStudioRender, DrawModelInfo_t &info,
                            ...
                            const Vector &modelOrigin, const Vector &color,
                            int flags) {
  // Make static so that we aren't reallocating everything all the time.
  // TODO: make sure that this actually keeps us from reallocating inside of
  // GetTriangles.
...
int DebugDrawModelBoneWeights(IStudioRender *pStudioRender,
                              DrawModelInfo_t &info, matrix3x4_t *pBoneToWorld,
...
int DebugDrawModelTexCoord(IStudioRender *pStudioRender,
  ...
  for (int batchID = 0; batchID < tris.m_MaterialBatches.Count(); batchID++) {
    GetTriangles_MaterialBatch_t &materialBatch =
        tris.m_MaterialBatches[batchID];
    if (!materialBatch.m_Verts.Count() ||
        V_stricmp(materialBatch.m_pMaterial->GetName(), pMaterialName))
      continue;
  ...
  bool bFound = false;
  IMaterialVar *pBaseVar =
      g_materialDebugCopyBaseTexture->FindVar("$basetexture", &bFound, true);
  if (!bFound)
    return 0;
  ...
  IMaterialVar *pVar =
      materialBatch.m_pMaterial->FindVar("$basetexture", &bFound, true);
  if (!bFound)
    return 0;
  ...
  // now draw all batches with the matching material in wireframe over the
  // render of the base texture
  for (int i = 0; i < batchList.Count(); i++) {
    GetTriangles_MaterialBatch_t &materialBatch =
        tris.m_MaterialBatches[batchList[i]];
