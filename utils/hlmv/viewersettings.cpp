...
ViewerSettings::ViewerSettings() { Q_memset(this, 0, sizeof(*this)); }
...
void InitViewerSettings(const char *subkey) {
  ...
  // Some values should survive.  This is a crappy way to do settings in
  // general.  Sigh.
  {
...
bool RegReadColor(HKEY hKey, const char *szSubKey, float value[4]) {
  ...
  if (sscanf(szBuff, "(%f %f %f %f)", &value[0], &value[1], &value[2],
             &value[3]) != 4)
    return false;
...
LONG RegViewerSettingsKey(const char *filename, PHKEY phKey,
  ...
  sprintf(szModelKey, "Software\\Valve\\%s\\%s",
          g_viewerSettings.registrysubkey, szFileName);
...
bool LoadViewerSettings(const char *filename, StudioModel *pModel) {
  ...
  RegReadInt(hModelKey, "thumbnailsizeanim",
             &g_viewerSettings.thumbnailsizeanim);
  ...
  RegReadBool(hModelKey, "enablenormalmapping",
  ...
  RegReadFloat(hModelKey, "originaxislength",
...
bool LoadViewerRootSettings(void) {
  ...
  RegReadBool(hRootKey, "faceposerToolsDriveMouth",
              &g_viewerSettings.faceposerToolsDriveMouth);
...
bool SaveViewerSettings(const char *filename, StudioModel *pModel) {
  ...
  RegWriteString(hModelKey, "sequence",
                 hdr->pSeqdesc(pModel->GetSequence()).pszLabel());
  RegWriteString(hModelKey, "overlaySequence0",
                 hdr->pSeqdesc(pModel->GetOverlaySequence(0)).pszLabel());
  RegWriteFloat(hModelKey, "overlayWeight0",
                pModel->GetOverlaySequenceWeight(0));
  RegWriteString(hModelKey, "overlaySequence1",
                 hdr->pSeqdesc(pModel->GetOverlaySequence(1)).pszLabel());
  RegWriteFloat(hModelKey, "overlayWeight1",
                pModel->GetOverlaySequenceWeight(1));
  RegWriteString(hModelKey, "overlaySequence2",
                 hdr->pSeqdesc(pModel->GetOverlaySequence(2)).pszLabel());
  RegWriteFloat(hModelKey, "overlayWeight2",
                pModel->GetOverlaySequenceWeight(2));
  RegWriteString(hModelKey, "overlaySequence3",
                 hdr->pSeqdesc(pModel->GetOverlaySequence(3)).pszLabel());
  RegWriteFloat(hModelKey, "overlayWeight3",
                pModel->GetOverlaySequenceWeight(3));
  ...
  RegWriteInt(hModelKey, "thumbnailsizeanim",
              g_viewerSettings.thumbnailsizeanim);
  ...
  RegWriteInt(hModelKey, "enablenormalmapping",
              g_viewerSettings.enableNormalMapping);
  ...
  RegWriteFloat(hModelKey, "originaxislength",
                g_viewerSettings.originAxisLength);
...
bool SaveViewerRootSettings(void) {
  ...
  RegWriteInt(hRootKey, "faceposerToolsDriveMouth",
              g_viewerSettings.faceposerToolsDriveMouth ? 1 : 0);
