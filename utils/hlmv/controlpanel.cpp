...
static void PopulateSoundNameList(mxListBox *pListBox) {
  ...
  if (g_pSoundEmitterBase) {
    for (int i = g_pSoundEmitterBase->First();
         i != g_pSoundEmitterBase->InvalidIndex();
...
CBoneControlWindow::CBoneControlWindow(ControlPanel *pParent)
    : mxWindow(pParent, 0, 0, 0, 0) {
...
void CBoneControlWindow::Init() {
  ...
  // Show bone checkbox
  m_cBoneHighlight = new mxCheckBox(this, left, top + 45, 140, 20,
                                    "Highlight Bone", IDC_BONE_HIGHLIGHT_BONE);
  ...
  m_cSurfaceProp =
      new mxChoice(this, left, top + 85, 140, 22, IDC_BONE_SURFACEPROP);
  mxToolTip::add(m_cSurfaceProp,
                 "Select a surface property to apply to the bone");

  // FIXME: We can't read the surface props yet because the vphysics path isn't
  // known!!! This will only add 'default' to the list
  ReadPhysicsMaterials(m_cSurfaceProp);

  // This button will apply the surface prop to all children
  mxButton *btnApplyChild =
      new mxButton(this, left, top + 115, 140, 20, "Apply to Children",
                   IDC_BONE_APPLY_TO_CHILDREN);
  mxToolTip::add(btnApplyChild,
                 "Apply the surface property to all child bones");

  // Use autogenerated hitboxes
  m_cAutoHitbox =
      new mxCheckBox(this, left, top + 140, 140, 20, "Autogenerate Hitboxes",
                     IDC_BONE_USE_AUTOGENERATED_HITBOXES);
  mxToolTip::add(
      m_cAutoHitbox,
      "When this is checked, studiomdl will automatically generate hitboxes");

  m_cShowDefaultPose =
      new mxCheckBox(this, left, top + 160, 140, 20, "Show Default Pose",
                     IDC_BONE_SHOW_DEFAULT_POSE);
  mxToolTip::add(m_cShowDefaultPose,
                 "Toggles display of the default physics pose");

  left = 160;

  new mxLabel(this, left + 3, top + 50, 80, 18, "Hitbox Set");
  m_cHitboxSet =
      new mxChoice(this, left, top + 66, 100, 22, IDC_BONE_HITBOXSET);
  m_cHitboxSet->add("unnamed");
  m_cHitboxSet->select(0);
  mxToolTip::add(m_cHitboxSet, "Change hitbox set");

  m_eHitboxSetName = new mxLineEdit(this, left, top + 96, 100, 18, "",
                                    IDC_BONE_HITBOXSETNAME_EDIT);
  mxToolTip::add(m_eHitboxSetName, "Type in a name and hit the set button");

  m_bHitboxSetUpdateName = new mxButton(this, left, top + 116, 100, 16,
                                        "Set Name", IDC_BONE_HITBOXSETNAME);
  mxToolTip::add(m_bHitboxSetUpdateName,
                 "Press to set hitbox set name from above text field");

  m_bAddHitboxSet = new mxButton(this, left, top + 140, 100, 20, "Add set",
                                 IDC_BONE_HITBOXADDSET);
  mxToolTip::add(m_bAddHitboxSet, "Add a hitbox set");
  m_bDeleteHitboxSet = new mxButton(this, left, top + 162, 100, 20,
                                    "Delete set", IDC_BONE_HITBOXDELETESET);
  ...
  m_cHitboxHighlight =
      new mxCheckBox(this, left + 3, top + 45, 100, 20, "Highlight Hitbox",
                     IDC_BONE_HIGHLIGHT_HITBOX);
  mxToolTip::add(m_cHitboxHighlight,
                 "Toggle display of the hitbox being modified");

  // Hitbox group
  new mxLabel(this, left + 3, top + 80, 80, 18, "Hitbox Group");
  m_eHitboxGroup = new mxLineEdit(this, left + 80, top + 75, 50, 22, "",
                                  IDC_BONE_HITBOX_GROUP);
  mxToolTip::add(m_eHitboxGroup, "The group of the current hitbox");

  // Hitbox name
  new mxLabel(this, left + 133, top + 80, 80, 18, "Hitbox Name");
  m_eHitboxName = new mxLineEdit(this, left + 133 + 80, top + 75, 50, 22, "",
                                 IDC_BONE_HITBOX_NAME);
  ...
  m_eOriginX = new mxLineEdit(this, left + 80, top + 105, 50, 22, "",
                              IDC_BONE_HITBOX_ORIGINX);
  m_eOriginY = new mxLineEdit(this, left + 145, top + 105, 50, 22, "",
                              IDC_BONE_HITBOX_ORIGINY);
  m_eOriginZ = new mxLineEdit(this, left + 210, top + 105, 50, 22, "",
                              IDC_BONE_HITBOX_ORIGINZ);
  ...
  m_eSizeX = new mxLineEdit(this, left + 80, top + 135, 50, 22, "",
                            IDC_BONE_HITBOX_SIZEX);
  m_eSizeY = new mxLineEdit(this, left + 145, top + 135, 50, 22, "",
                            IDC_BONE_HITBOX_SIZEY);
  m_eSizeZ = new mxLineEdit(this, left + 210, top + 135, 50, 22, "",
                            IDC_BONE_HITBOX_SIZEZ);

  // Update hitboxes here
  m_bUpdateHitbox = new mxButton(this, left, top + 163, 100, 20,
                                 "Update Hitbox", IDC_BONE_UPDATE_HITBOX);
  mxToolTip::add(m_bUpdateHitbox,
                 "Apply hitbox group, origin, and size to the hitbox");

  left += 160;

  // Generate a QC file
  mxButton *btnGenerateQC = new mxButton(this, left, top + 10, 100, 20,
                                         "Generate QC", IDC_BONE_GENERATEQC);
  mxToolTip::add(btnGenerateQC, "Copy a .qc file snippet to the clipboard");

  // Add, remove hitboxes here
  m_bAddHitbox = new mxButton(this, left, top + 30, 100, 20, "Add Hitbox",
                              IDC_BONE_ADD_HITBOX);
  m_bDeleteHitbox = new mxButton(this, left, top + 50, 100, 20, "Delete Hitbox",
                                 IDC_BONE_DELETE_HITBOX);
  mxToolTip::add(m_bAddHitbox,
                 "Create a new hitbox attached to the current bone");
...
void CBoneControlWindow::OnLoadModel() {
  ...
  CStudioHdr *pHdr = g_pStudioModel->GetStudioHdr();
  m_cAutoHitbox->setChecked(
      (pHdr->flags() & STUDIOHDR_FLAGS_AUTOGENERATED_HITBOX) != 0);
...
// Purpose:
//-----------------------------------------------------------------------------
int CBoneControlWindow::GetHitboxSet(void) { return m_nHitboxSet; }
...
void CBoneControlWindow::OnBoneSelected(int boneIndex) {
  ...
  if (physprop && g_pStudioModel->m_SurfaceProps.Count()) {
    ...
    const char *pSurfaceProp =
        g_pStudioModel->m_SurfaceProps[boneIndex].String();
...
void CBoneControlWindow::RefreshHitbox() {
  if (m_nHitboxSet < 0 ||
      m_nHitboxSet >= g_pStudioModel->m_HitboxSets.Count()) {
    m_nHitboxSet = 0;
  }
  ...
  if (m_Hitbox >= 0 && g_pStudioModel->m_HitboxSets.Count() > 0) {
    ...
    mstudiobbox_t *pHitbox =
        &g_pStudioModel->m_HitboxSets[m_nHitboxSet].m_Hitboxes[m_Hitbox].m_BBox;
    ...
    const char *hitboxname =
        g_pStudioModel->m_HitboxSets[m_nHitboxSet].m_Hitboxes[m_Hitbox].m_Name;
...
void CBoneControlWindow::OnHitboxChanged() {
  ...
  // Gotta do it this way since getLabel whacks the previous return result to
  // getLable
...
void CBoneControlWindow::OnHitboxSetChanged(void) {
  ...
  ComputeHitboxList();
  RefreshHitbox();
  // OnHitboxGroupChanged( );  // Refresh hitbox will update the group label
  // from the hitbox's group.  No need to turn that back into a group #
}
...
void CBoneControlWindow::OnAddHitbox() {
  ...
  HitboxInfo_t &hitbox =
      g_pStudioModel->m_HitboxSets[m_nHitboxSet].m_Hitboxes[i];
...
void CBoneControlWindow::OnSurfacePropApplyToChildren_R(int bone,
                                                        CUtlSymbol prop) {
...
bool CBoneControlWindow::SerializeQC(CUtlBuffer &buf) {
  ...
  if (!m_cAutoHitbox->isChecked()) {
    ...
    for (i = 0; i < g_pStudioModel->m_HitboxSets.Count(); i++) {
      ...
      for (unsigned short j = list.Head(); j != list.InvalidIndex();
           j = list.Next(j)) {
        ...
        buf.Printf("$hbox %d \"%s\"\t  %7.2f %7.2f %7.2f  %7.2f %7.2f %7.2f",
                   hitbox.group, pBone->pszName(), hitbox.bbmin.x,
                   hitbox.bbmin.y, hitbox.bbmin.z, hitbox.bbmax.x,
                   hitbox.bbmax.y, hitbox.bbmax.z);
...
void ControlPanel::SetupRenderWindow(mxTab *pTab) {
  ...
  cbGround =
      new mxCheckBox(wRender, 125, 5, 150, 20, "Ground (Ctrl-G)", IDC_GROUND);
  cbGround->setEnabled(true);
  cbMovement = new mxCheckBox(wRender, 125, 25, 150, 20, "Movement (Ctrl-M)",
                              IDC_MOVEMENT);
  cbMovement->setEnabled(true);
  cbBackground = new mxCheckBox(wRender, 125, 45, 150, 20,
                                "Background (Ctrl-B)", IDC_BACKGROUND);
  cbBackground->setEnabled(true);
  cbHitBoxes = new mxCheckBox(wRender, 125, 65, 150, 20, "Hit Boxes (Ctrl-H)",
                              IDC_HITBOXES);
  cbSequenceBoxes = new mxCheckBox(wRender, 125, 85, 150, 20, "Seq. Boxes",
                                   IDC_SEQUENCEBOXES);
  cbShadow =
      new mxCheckBox(wRender, 125, 105, 150, 20, "Shadow (Ctrl-S)", IDC_SHADOW);
  cbSoftwareSkin = new mxCheckBox(wRender, 125, 125, 150, 20, "Software Skin",
                                  IDC_SOFTWARESKIN);
  cbSoftwareSkin->setEnabled(true);
  cbOverbright2 = new mxCheckBox(wRender, 125, 145, 150, 20,
                                 "Enable Overbrightening", IDC_OVERBRIGHT2);
  cbOverbright2->setEnabled(true);
  cbOverbright2->setChecked(true);
  setOverbright(true);

  cbAttachments = new mxCheckBox(wRender, 5, 45, 120, 20,
                                 "Attachments (Ctrl-A)", IDC_ATTACHMENTS);
  cbAttachments->setEnabled(true);

  cbBones =
      new mxCheckBox(wRender, 5, 65, 120, 20, "Bones (Ctrl-O)", IDC_BONES);

  cbNormals =
      new mxCheckBox(wRender, 5, 85, 120, 20, "Normals (Ctrl-N)", IDC_NORMALS);
  cbNormals->setEnabled(true);
  cbNormals->setChecked(false);

  cbTangentFrame = new mxCheckBox(wRender, 5, 105, 120, 20, "Tangents (Ctrl-T)",
                                  IDC_TANGENTFRAME);
  cbTangentFrame->setEnabled(true);
  cbTangentFrame->setChecked(false);

  cbOverlayWireframe = new mxCheckBox(
      wRender, 5, 125, 120, 20, "Wireframe (Ctrl-W)", IDC_OVERLAY_WIREFRAME);
  cbOverlayWireframe->setEnabled(true);
  cbOverlayWireframe->setChecked(false);

  //	cbParallaxMap = new mxCheckBox (wRender, 5, 125, 100, 20, "Parallax
  //Mapping", IDC_PARALLAXMAP); 	cbParallaxMap->setEnabled( true );
  //	cbParallaxMap->setChecked( true );

  cbSpecular =
      new mxCheckBox(wRender, 5, 145, 120, 20, "Specular", IDC_SPECULAR);
  cbSpecular->setEnabled(true);
  cbSpecular->setChecked(true);

  cbNormalMap =
      new mxCheckBox(wRender, 5, 25, 100, 20, "Normal Mapping", IDC_NORMALMAP);
  cbNormalMap->setEnabled(true);
  cbNormalMap->setChecked(true);

  cbRunIK = new mxCheckBox(wRender, 275, 65, 150, 20, "Enable IK", IDC_RUNIK);
  cbEnableHead =
      new mxCheckBox(wRender, 275, 85, 150, 20, "Head Turn", IDC_HEADTURN);

  cbIllumPosition = new mxCheckBox(wRender, 275, 105, 150, 20,
                                   "Illum. Position", IDC_ILLUMPOSITION);

  cbPlaySounds =
      new mxCheckBox(wRender, 275, 125, 150, 20, "Play Sounds", IDC_PLAYSOUNDS);

  cbShowOriginAxis = new mxCheckBox(wRender, 275, 145, 150, 20,
                                    "Show Origin Axis", IDC_SHOWORIGINAXIS);

  new mxLabel(wRender, 275, 170, 45, 18, "Axis Len:");
  leOriginAxisLength =
      new mxSlider(wRender, 320, 165, 105, 22, IDC_ORIGINAXISLENGTH);
  ...
  cHighlightBone =
      new mxChoice(wRender, 275, 25, 150, 22, IDC_PHYSICSHIGHLIGHT);
...
void ControlPanel::SetupSequenceWindow(mxTab *pTab) {
  ...
  for (int i = 0; i < MAX_SEQUENCES; i++) {
    cSequence[i] =
        new mxChoice(wSequence, 5, 5 + i * 22, 200, 22, IDC_SEQUENCE0 + i);
    mxToolTip::add(cSequence[i], "Select Sequence");
    slSequence[i] = new mxSlider(wSequence, 208, 5 + i * 22, 80, 18,
                                 IDC_SEQUENCESCALE0 + i);
    slSequence[i]->setRange(0, 1.0, 100);
    slSequence[i]->setValue(0.0);

    rbFrameSelection[i] =
        new mxRadioButton(wSequence, 300, 5 + i * 22, 35, 22, "",
                          IDC_FRAMESELECTION0 + i, i == 0);
  }
  ...
  for (int i = 0; i < NUM_POSEPARAMETERS; i++) {
    ...
    cPoseParameter[i] =
        new mxChoice(wSequence, 520, y, 96, 22, IDC_POSEPARAMETER + i);
    cPoseParameter[i]->setVisible(false);

    slPoseParameter[i] =
        new mxSlider(wSequence, x, y, 140, 16, IDC_POSEPARAMETER_SCALE + i);
    slPoseParameter[i]->setRange(0.0, 1.0, 1000);
    mxToolTip::add(slPoseParameter[i], "Parameter");
    slPoseParameter[i]->setVisible(false);

    lePoseParameter[i] = new mxLineEdit(wSequence, x + 146, y, 40, 16, "X",
                                        IDC_POSEPARAMETER_VALUE + i);
  ...
  new mxCheckBox(wSequence, 5, 142, 150, 20, "Blend Sequence Changes",
  ...
  cActivityModifiers =
      new mxChoice(wSequence, 105, 166, 350, 22, IDC_ACTIVITY_MODIFIERS);

  new mxCheckBox(wSequence, 460, 166, 350, 22, "Animate weapons",
                 IDC_ANIMATEWEAPONS);
}
...
void ControlPanel::SetupFlexWindow(mxTab *pTab) {
  ...
  for (int i = 0; i < NUM_FLEX_SLIDERS; i++) {
    int w = (i / FLEX_ROWS_OF_SLIDERS) *
                (FLEX_SLIDER_WIDTH + FLEX_DROPDOWN_WIDTH + 4) +
            5;
    ...
    slFlexScale[i] = new mxSlider(wFlex, w + FLEX_DROPDOWN_WIDTH, h,
                                  FLEX_SLIDER_WIDTH, 18, IDC_FLEXSCALE + i);
...

  {
    ...
    new mxButton(wFlex, 5, h + 1, FLEX_DROPDOWN_WIDTH - 2, 18, "Reset",
                 IDC_FLEXDEFAULTS);

    int w = (FLEX_DROPDOWN_WIDTH + 4) + 5;
    new mxButton(wFlex, w, h + 1, FLEX_DROPDOWN_WIDTH - 2, 18, "Random",
                 IDC_FLEXRANDOM);

    w += (FLEX_DROPDOWN_WIDTH + 4) + 5;
    new mxButton(wFlex, w, h + 1, FLEX_DROPDOWN_WIDTH - 2, 18, "Zero",
                 IDC_FLEXZERO);

    w += (FLEX_DROPDOWN_WIDTH + 4) + 5;
    new mxButton(wFlex, w, h + 1, FLEX_DROPDOWN_WIDTH - 2, 18, "One",
                 IDC_FLEXONE);
  }
}
...
void ControlPanel::SetupPhysicsWindow(mxTab *pTab) {
  ...
  rbConstraintAxis[0] =
      new mxRadioButton(wPhysics, x, y, 35, 22, "X", IDC_PHYS_CON_AXIS_X, true);
  rbConstraintAxis[1] =
      new mxRadioButton(wPhysics, x + 35, y, 35, 22, "Y", IDC_PHYS_CON_AXIS_Y);
  rbConstraintAxis[2] =
      new mxRadioButton(wPhysics, x + 70, y, 35, 22, "Z", IDC_PHYS_CON_AXIS_Z);
  setPhysicsAxis(0);
  y += 25;

  new mxLabel(wPhysics, x, y, 45, 18, "Friction");
  slPhysicsFriction =
      new mxSlider(wPhysics, x + 50, y, 180, 18, IDC_PHYS_CON_FRICTION);
  ...
  cbLinked = new mxCheckBox(wPhysics, 200, y, 50, 20, "Link",
                            IDC_PHYS_CON_LINK_LIMITS);
  ...
  slPhysicsParamMassBias =
      new mxSlider(wPhysics, x, y, 76, 18, IDC_PHYS_P_MASSBIAS);
  ...
  slPhysicsParamInertia =
      new mxSlider(wPhysics, x, y, 76, 18, IDC_PHYS_P_INERTIA);
  ...
  slPhysicsParamDamping =
      new mxSlider(wPhysics, x, y, 76, 18, IDC_PHYS_P_DAMPING);
  ...
  slPhysicsParamRotDamping =
      new mxSlider(wPhysics, x, y, 76, 18, IDC_PHYS_P_ROT_DAMPING);
...
void ControlPanel::SetupIKRuleWindow(mxTab *pTab) {
  ...
  leIKAttachment =
      new mxLineEdit(wIKRule, 400, 5, 80, 20, "", IDC_IKRULE_ATTACHMENT);

  cbIKRangeToggle =
      new mxCheckBox(wIKRule, 5, 30, 80, 20, "Range", IDC_IKRULE_RANGE_TOGGLE);
  mxToolTip::add(cbIKRangeToggle, "Toggle range option");
  new mxButton(wIKRule, 90, 30, 30, 20, "start", IDC_IKRULE_RANGE_START_NOW);
  leIKRangeStart =
      new mxLineEdit2(wIKRule, 120, 30, 35, 20, "..", IDC_IKRULE_RANGE_START);
  new mxButton(wIKRule, 160, 30, 30, 20, "peak", IDC_IKRULE_RANGE_PEAK_NOW);
  leIKRangePeak =
      new mxLineEdit2(wIKRule, 190, 30, 35, 20, "..", IDC_IKRULE_RANGE_PEAK);
  new mxButton(wIKRule, 230, 30, 30, 20, "tail", IDC_IKRULE_RANGE_TAIL_NOW);
  leIKRangeTail =
      new mxLineEdit2(wIKRule, 260, 30, 35, 20, "..", IDC_IKRULE_RANGE_TAIL);
  new mxButton(wIKRule, 300, 30, 30, 20, "end", IDC_IKRULE_RANGE_END_NOW);
  leIKRangeEnd =
      new mxLineEdit2(wIKRule, 330, 30, 35, 20, "..", IDC_IKRULE_RANGE_END);

  cbIKContactToggle = new mxCheckBox(wIKRule, 5, 55, 80, 20, "Contact",
                                     IDC_IKRULE_CONTACT_TOGGLE);
  mxToolTip::add(cbIKContactToggle, "Toggle contact option");
  new mxButton(wIKRule, 90, 55, 30, 20, "frame", IDC_IKRULE_CONTACT_FRAME_NOW);
  leIKContactFrame =
      new mxLineEdit2(wIKRule, 120, 55, 35, 20, "", IDC_IKRULE_CONTACT_FRAME);
  ...
  leIKQCString =
      new mxLineEdit2(wIKRule, 90, 105, 500, 20, "", IDC_IKRULE_QC_STRING);
  UpdateIKRuleWindow();
}
...
void ControlPanel::SetupEventWindow(mxTab *pTab) {
  ...
  mxButton *bSoundFrameNow =
      new mxButton(wEvents, 90, 5, 30, 20, "frame", IDC_EVENT_SOUND_FRAME_NOW);
  mxToolTip::add(bSoundFrameNow, "Set sound start at the current frame");
  leEventSoundFrame =
      new mxLineEdit2(wEvents, 120, 5, 35, 20, "0", IDC_EVENT_SOUND_FRAME);
  lbEventSoundName =
      new mxListBox(wEvents, 160, 5, 300, 170, IDC_EVENT_SOUND_NAME);
  ...
  leEventQCString =
      new mxLineEdit2(wEvents, 90, 170, 450, 20, "", IDC_EVENT_QC_STRING);
  BuildEventQCString();
}
...
ControlPanel::~ControlPanel() { g_ControlPanel = NULL; }

void ControlPanel::OnDelete() {
  // for some reason, the destructor only gets called from mx when breakpoints
  // are set, so to be safe, clear the pointer (possibly twice)
  g_ControlPanel = NULL;
}
...
int ControlPanel::handleEvent(mxEvent *event) {
  ...
  if (event->event == mxEvent::KeyDown) {
    ...
    if (event->action >= IDC_POSEPARAMETER_VALUE &&
        event->action < IDC_POSEPARAMETER_VALUE + NUM_POSEPARAMETERS) {
      ...
      slPoseParameter[index]->setValue(
          g_pStudioModel->GetPoseParameter(poseparam));
      return 1;
    }
  ...
  case IDC_ORIGINAXISLENGTH: {
    g_viewerSettings.originAxisLength =
        reinterpret_cast<mxSlider *>(event->widget)->getValue();
    break;
  }
  ...
  case IDC_SEQUENCEBOXES:
    g_viewerSettings.showSequenceBoxes =
        ((mxCheckBox *)event->widget)->isChecked();
    break;
  ...
  case IDC_ILLUMPOSITION:
    g_viewerSettings.showIllumPosition =
        ((mxCheckBox *)event->widget)->isChecked();
    break;

  case IDC_RUNIK:
    g_viewerSettings.enableIK = ((mxCheckBox *)event->widget)->isChecked();
    g_viewerSettings.enableTargetIK =
        ((mxCheckBox *)event->widget)->isChecked();
    break;

  case IDC_HEADTURN:
    g_pStudioModel->SetSolveHeadTurn(
        ((mxCheckBox *)event->widget)->isChecked() ? 1 : 0);
    break;

  case IDC_PHYSICSMODEL:
    g_viewerSettings.showPhysicsModel =
        ((mxCheckBox *)event->widget)->isChecked();
    break;
  ...
  case IDC_NORMALMAP:
    g_viewerSettings.enableNormalMapping =
        ((mxCheckBox *)event->widget)->isChecked();
    break;

    //		case IDC_PARALLAXMAP:
    //			g_viewerSettings.enableParallaxMapping = ((mxCheckBox *)
    //event->widget)->isChecked(); 			break;

  case IDC_SPECULAR:
    g_viewerSettings.enableSpecular =
        ((mxCheckBox *)event->widget)->isChecked();
    break;
  ...
  case IDC_OVERLAY_WIREFRAME:
    g_viewerSettings.overlayWireframe =
        ((mxCheckBox *)event->widget)->isChecked();
    break;

  case IDC_ATTACHMENTS:
    g_viewerSettings.showAttachments =
        ((mxCheckBox *)event->widget)->isChecked();
    break;
  ...
  case IDC_MESSAGES: {
    ...
    if (index >= 0) {
      ...
      if (pStudioR) {
        ...
        g_pStudioRender->GetMaterialList(pStudioR, ARRAYSIZE(pMaterials),
                                         &pMaterials[0]);
  ...
  case IDC_SEQUENCESCALE4: {
    ...
    if (index >= 0) {
      ...
      if (i == 0) {
        ...
        setOverlaySequence(i, index,
                           (float)((mxSlider *)event->widget)->getValue());
      }
    }
  } break;
  ...
  case IDC_BLENDSEQUENCECHANGES:
    g_viewerSettings.blendSequenceChanges =
        ((mxCheckBox *)event->widget)->isChecked();
    break;
  case IDC_ANIMATEWEAPONS:
    g_viewerSettings.animateWeapons =
        ((mxCheckBox *)event->widget)->isChecked();
    break;
  case IDC_BLENDNOW:
    startBlending();
  ...
  default: {
    if (event->action == IDC_FLEXDEFAULTS) {
      ...
      for (int i = 0; i < NUM_FLEX_SLIDERS; i++) {
        LocalFlexController_t index =
            (LocalFlexController_t)cFlex[i]->getSelectedIndex();
    ...
    } else if (event->action >= IDC_FLEX &&
               event->action < IDC_FLEX + NUM_FLEX_SLIDERS) {
      int flex = (event->action - IDC_FLEX);
      LocalFlexController_t index =
          (LocalFlexController_t)cFlex[flex]->getSelectedIndex();

      if (index >= 0) {
        slFlexScale[flex]->setValue(
            g_pStudioModel->GetFlexControllerRaw(index));
      }
    } else if (event->action >= IDC_FLEXSCALE &&
               event->action < IDC_FLEXSCALE + NUM_FLEX_SLIDERS) {
      int flex = (event->action - IDC_FLEXSCALE);
      LocalFlexController_t index =
          (LocalFlexController_t)cFlex[flex]->getSelectedIndex();

      g_pStudioModel->SetFlexControllerRaw(
          index, ((mxSlider *)event->widget)->getValue());
    } else if (event->action >= IDC_PHYS_FIRST &&
               event->action <= IDC_PHYS_LAST) {
      return handlePhysicsEvent(event);
    } else if (event->action >= IDC_POSEPARAMETER &&
               event->action < IDC_POSEPARAMETER + NUM_POSEPARAMETERS) {
      ...
      if (g_pStudioModel->GetPoseParameterRange(poseparam, &flMin, &flMax)) {
        ...
        slPoseParameter[index]->setValue(
            g_pStudioModel->GetPoseParameter(poseparam));
        lePoseParameter[index]->setLabel(
            "%.1f", g_pStudioModel->GetPoseParameter(poseparam));
      }
      ...
      lePoseParameter[index]->setLabel("%.1f",
...
void ControlPanel::OnLoadModel(void) {
  ...
  const bool bNoModelSettings =
      LoadViewerSettings(g_pStudioModel->GetFileName(), g_pStudioModel);
  if (!bNoModelSettings) {
    InitViewerSettings("hlmv");
    setSequence(0);
    ...
    if (g_pStudioExtraModel[slot]->PostLoadModel(filename)) {
      ...
      if (g_MDLViewer && g_MDLViewer->getMenuBar()) {
        g_MDLViewer->getMenuBar()->modify(IDC_FILE_UNLOADMERGEDMODEL1 + slot,
                                          IDC_FILE_UNLOADMERGEDMODEL1 + slot,
                                          filename);
        g_MDLViewer->getMenuBar()->setEnabled(
            IDC_FILE_UNLOADMERGEDMODEL1 + slot, true);
      }
      return LoadModel_Success;
    } else {
      return LoadModel_PostLoadFail;
...
void ControlPanel::resetControlPanel(void) {
  ...
  setOverlaySequence(1, g_pStudioModel->GetOverlaySequence(0),
                     g_pStudioModel->GetOverlaySequenceWeight(0));
  setOverlaySequence(2, g_pStudioModel->GetOverlaySequence(1),
                     g_pStudioModel->GetOverlaySequenceWeight(1));
  setOverlaySequence(3, g_pStudioModel->GetOverlaySequence(2),
                     g_pStudioModel->GetOverlaySequenceWeight(2));
  setOverlaySequence(4, g_pStudioModel->GetOverlaySequence(3),
                     g_pStudioModel->GetOverlaySequenceWeight(3));
...
void ControlPanel::setModelInfo(int nVertCount, int nIndexCount,
                                int nTriCount) {
  static int nSaveVertCount = -10;
  static int nSaveIndexCount = -10;
  static int nSaveTriCount = -10;

  if (nVertCount == nSaveVertCount && nIndexCount == nSaveIndexCount &&
      nTriCount == nSaveTriCount) {
    return;
  }
  ...
  char tmp[128];
  sprintf(tmp, "Verts: %d  Indexes: %d  Triangles: %d", nVertCount, nIndexCount,
          nTriCount);
...
void ControlPanel::CreateSortedSequenceList(CStudioHdr *hdr, int *pSequence) {
  ...
  SortInfo_t *pSort =
      (SortInfo_t *)_alloca(nSequenceCount * sizeof(SortInfo_t));
    ...
    for (int i = 0; i < MAX_SEQUENCES; i++) {
      ...
      for (int j = 0; j < nSequenceCount; j++) {
        ...
        if (g_viewerSettings.showHidden ||
            !(hdr->pSeqdesc(nSequence).flags & STUDIO_HIDDEN)) {
          if (g_viewerSettings.showActivities) {
...
void ControlPanel::startBlending(void) { g_pStudioModel->StartBlending(); }
...
void ControlPanel::setModelInfo() {
  ...
  if (checkSum == hdr->GetRenderHdr()->checksum &&
  ...
  sprintf(str,
          ...
          "Sequences: %d\n",
          hdr->numbones(), boneLODCount, numBatches, hdr->numbonecontrollers(),
          hbcount, hdr->numhitboxsets(), hdr->GetNumSeq());
  ...
  sprintf(str,
          ...
          "Bodyparts: %d\n"
          "Attachments: %d\n",
          g_DrawModelResults.m_NumMaterials, hdr->numskinfamilies(),
          hdr->numbodyparts(), hdr->GetNumAttachments());
...
void ControlPanel::UpdateMaterialList() {
  ...
  if (pStudioR) {
    ...
    int nMaterials = g_pStudioRender->GetMaterialList(
        pStudioR, ARRAYSIZE(pMaterials), &pMaterials[0]);
    ...
    for (int i = 0; i < nMaterials; i++) {
      ...
      if (pMaterials[i]->IsErrorMaterial()) {
        Q_strcat(c_MaterialLine,
                 "*** ERROR *** Model attempted to load one or more VMTs it "
                 "can't find.",
                 sizeof(c_MaterialLine));
      } else {
        Q_strcat(c_MaterialLine, pMaterials[i]->GetName(),
                 sizeof(c_MaterialLine));
      }
...
void ControlPanel::centerView() {
  ...
  for (unsigned short j = list.Head(); j != list.InvalidIndex();
    ...
    ConcatTransforms(rootxform, *g_pStudioModel->BoneToWorld(pBBox->bone),
                     bonesetup);
  ...
  if (hdr && root) {
    for (i = LocalFlexController_t(0); i < hdr->numflexcontrollers(); i++) {
      for (j = LocalFlexController_t(0); j < root->numflexcontrollers(); j++) {
        if (stricmp(hdr->pFlexcontroller(i)->pszName(),
                    root->pFlexcontroller(j)->pszName()) == 0) {
          hdr->pFlexcontroller(i)->localToGlobal =
              root->pFlexcontroller(j)->localToGlobal;
          break;
        }
      }
    }
...
int ControlPanel::handlePhysicsEvent(mxEvent *event) {
  ...
  case IDC_PHYS_CON_AXIS_Z:
    setupPhysicsAxis(cPhysicsBone->getSelectedIndex(),
                     event->action - IDC_PHYS_CON_AXIS_X);
    break;
  ...
  case IDC_PHYS_CON_TEST:
    g_pStudioModel->Physics_SetPreview(cPhysicsBone->getSelectedIndex(),
                                       getPhysicsAxis(),
...
void ControlPanel::setupPhysicsBone(int boneIndex) {
  ...
  SetSlider(slPhysicsParamRotDamping, lPhysicsParamRotDamping,
...
void ControlPanel::UnloadAllMergedModels() {
  for (int i = 0; i < HLMV_MAX_MERGED_MODELS; ++i) {
    g_MDLViewer->getMenuBar()->modify(IDC_FILE_UNLOADMERGEDMODEL1 + i,
                                      IDC_FILE_UNLOADMERGEDMODEL1 + i,
                                      "(empty)");
    g_MDLViewer->getMenuBar()->setEnabled(IDC_FILE_UNLOADMERGEDMODEL1 + i,
                                          false);
