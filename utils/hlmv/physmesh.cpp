  ...
  ~CStudioPhysics(void) {
    if (physcollision) {
      for (int i = 0; i < m_listCount; i++) {
        physcollision->DestroyDebugMesh(m_pList[i].m_vertCount,
                                        m_pList[i].m_pVerts);
...
void CStudioPhysics::Load(MDLHandle_t mdlHandle) {
  ...
  for (i = 0; i < pVCollide->solidCount; i++) {
    m_pList[i].Clear();
    m_pList[i].m_vertCount = physcollision->CreateDebugMesh(
        pVCollide->solids[i], &m_pList[i].m_pVerts);
    m_pList[i].m_pCollisionModel =
        physcollision->CreateQueryModel(pVCollide->solids[i]);
  }
  ...
  for (i = 0; i < pVCollide->solidCount; i++) {
    ...
    if (pmesh->m_constraint.parentIndex >= 0) {
      ...
      Studio_CalcBoneToBoneTransform(&studioHdr, boneIndex, parentIndex,
                                     pmesh->m_matrix);
    } else {
...
public:
  virtual void ParseKeyValue(void *pCustom, const char *pKey,
                             const char *pValue) {
...
void CStudioPhysics::ParseKeydata(void) {
  IVPhysicsKeyParser *pParser =
      physcollision->VPhysicsKeyParserCreate(GetVCollide()->pKeyValues);
...
//-----------------------------------------------------------------------------
template <class T>
static T FindCommonValue(void *pStructArray, int arrayCount, int structSize,
                         int fieldOffset) {
  int maxCount = 0;
  T maxVal = 0;
...
static void CalcDefaultProperties(CPhysmesh *pList, int listCount,
                                  physdefaults_t &defs) {
  defs.surfacePropIndex =
      FindCommonValue<int>(pList, listCount, sizeof(CPhysmesh),
                           offsetof(CPhysmesh, m_solid.surfacePropIndex));
  defs.inertia =
      FindCommonValue<float>(pList, listCount, sizeof(CPhysmesh),
                             offsetof(CPhysmesh, m_solid.params.inertia));
  defs.damping =
      FindCommonValue<float>(pList, listCount, sizeof(CPhysmesh),
                             offsetof(CPhysmesh, m_solid.params.damping));
  defs.rotdamping =
      FindCommonValue<float>(pList, listCount, sizeof(CPhysmesh),
                             offsetof(CPhysmesh, m_solid.params.rotdamping));
}

static void DumpModelProperties(CTextBuffer &out, float mass,
                                physdefaults_t &defs) {
  char tmpbuf[1024];
...
char *CStudioPhysics::DumpQC(void) {
  ...
  if (m_listCount == 1) {
    ...
    for (i = 0; i < m_edit.mergeCount; i++) {
      ...
      if (m_edit.mergeList[i].parent >= 0 && m_edit.mergeList[i].child >= 0) {
        char const *pParentName = g_pStudioModel->GetStudioHdr()
                                      ->pBone(m_edit.mergeList[i].parent)
                                      ->pszName();
        char const *pChildName = g_pStudioModel->GetStudioHdr()
                                     ->pBone(m_edit.mergeList[i].child)
                                     ->pszName();
      ...
      if (pmesh->m_constraint.parentIndex >= 0) {
        for (int j = 0; j < 3; j++) {
          ...
          sprintf(tmpbuf, "\t$jointconstrain %s %s limit %.2f %.2f %.2f\r\n",
                  jointname, pAxis[j], pmesh->m_constraint.axes[j].minRotation,
                  pmesh->m_constraint.axes[j].maxRotation,
  ...
  if (m_listCount != 1) {
    out.WriteText("$sequence ragdoll \t\t\"ragdoll_pose\" \t\tFPS 30 "
