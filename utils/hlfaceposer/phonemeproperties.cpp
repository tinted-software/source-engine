...
typedef long(__stdcall *WINPROCTYPE)(HWND hwnd, UINT uMsg, WPARAM wParam,
                                     LPARAM lParam);
static WINPROCTYPE lpfnOldButtonProc;

static BOOL CALLBACK PhonemeBtnProc(HWND hwnd, UINT uMsg, WPARAM wParam,
                                    LPARAM lParam) {
  switch (uMsg) {
  case WM_MOUSEMOVE: {
    ...
    if (dialog) {
      ...
      if (helpText) {
        CExpression *exp =
            (CExpression *)GetWindowLong((HWND)hwnd, GWL_USERDATA);
        if (exp) {
...
static void ClickedPhoneme(HWND hwndDlg, int phoneme) {
  ...
  if (!g_Params.m_bMultiplePhoneme) {
    ...
    SendMessage(ctrl, WM_GETTEXT, (WPARAM)sizeof(g_Params.m_szName),
                (LPARAM)g_Params.m_szName);
  }
...
static void CreateAndLayoutControls(HWND hwndDlg, CPhonemeParams *params) {
  ...
  if (params->m_bPositionDialog) {
    ...
    MoveWindow(hwndDlg, (GetSystemMetrics(SM_CXFULLSCREEN) - dialogW) / 2,
               (GetSystemMetrics(SM_CYFULLSCREEN) - dialogH) / 2, dialogW,
               dialogH, TRUE);
  }
  ...
  for (int i = 0; i < g_nPhonemeCount; i++) {
    ...
    HWND button = CreateWindowEx(
        0, "BUTTON", va("%s", exp->name), WS_CHILD | WS_VISIBLE | BS_LEFT,
        startx + c * (PHONEME_WIDTH + PHONEME_GAP),
        starty + r * (PHONEME_HEIGHT + PHONEME_VGAP), PHONEME_WIDTH,
        PHONEME_HEIGHT, hwndDlg, (HMENU)(IDC_PHONEME + i),
        (HINSTANCE)GetModuleHandle(0), NULL);
    Assert(button);
    SetWindowLong((HWND)button, GWL_USERDATA, (LONG)exp);

    // Subclass it
    lpfnOldButtonProc = (WINPROCTYPE)SetWindowLong((HWND)button, GWL_WNDPROC,
                                                   (LONG)PhonemeBtnProc);

    SendMessage((HWND)button, WM_SETFONT,
...
// Output : static bool
//-----------------------------------------------------------------------------
static bool ValidatePhonemeString(bool allowmultiple, char const *input,
                                  char *output) {
  // Make sure phonemes are loaded
  FacePoser_EnsurePhonemesLoaded();
...
//-----------------------------------------------------------------------------
static BOOL CALLBACK PhonemePropertiesDialogProc(HWND hwndDlg, UINT uMsg,
  ...
  case WM_COMMAND: {
    ...
    if ((cmd >= IDC_PHONEME) && (cmd < (IDC_PHONEME + g_nPhonemeCount))) {
      ...
      case IDOK: {
        ...
        HWND ctrl = GetDlgItem(hwndDlg, IDC_EDIT_PHONEME);
        if (ctrl) {
          SendMessage(ctrl, WM_GETTEXT, (WPARAM)sizeof(szPhoneme),
                      (LPARAM)szPhoneme);

          ValidatePhonemeString(g_Params.m_bMultiplePhoneme, szPhoneme,
                                g_Params.m_szName);
        }
        PhonemePropertiesDialogExit(hwndDlg, 1);
      } break;
...
int PhonemeProperties(CPhonemeParams *params) {
  ...
  int retval = DialogBox(
      (HINSTANCE)GetModuleHandle(0), MAKEINTRESOURCE(IDD_PHONEMEPROPERTIES),
