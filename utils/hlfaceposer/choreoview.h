...
//-----------------------------------------------------------------------------
class CChoreoView : public mxWindow,
...
public:
  ...
  struct SelectionParams_t {
    enum { SP_CHANNEL = 0, SP_ACTIVE, SP_ALL };
  ...
  void AddEvent(int type, int subtype = 0,
  ...
  bool GetTarget(CChoreoScene *scene, CChoreoEvent *event, Vector &vecTarget,
                 QAngle &vecAngle);
  ...
  int GetMovetoSequence(CChoreoScene *scene, CChoreoEvent *event,
                        StudioModel *model);
  ...
  void DrawTimeLine(CChoreoWidgetDrawHelper &drawHelper, RECT &rc, float left,
                    float right);
  ...
  CEventAbsoluteTag *GetAbsoluteTagUnderCursorPos(CChoreoEventWidget *event,
                                                  int mx, int my);

  void GetObjectsUnderMouse(int mx, int my, CChoreoActorWidget **actor,
                            CChoreoChannelWidget **channel,
                            CChoreoEventWidget **event,
                            CChoreoGlobalEventWidget **globalevent,
                            int *clickedTag, CEventAbsoluteTag **absolutetag,
                            int *clickedCCButton);
  ...
  // void				FinishDraggingGlobalEvent( int mx, int
  // my );
  ...
  typedef void (CChoreoView::*CVMEMBERFUNC)(CChoreoWidget *widget,
  ...
  template <class T> void SetPreservedTimeZoom(T *other, int tz);
  template <class T> int HandleZoomKey(T *other, int keyCode);
...
private:
  void CheckInsertTime(CChoreoEvent *e, float dt, float starttime,
                       float endtime);
  void CheckDeleteTime(CChoreoEvent *d, float dt, float starttime,
  ...
  float FindNextEventTime(CChoreoEvent::EVENTTYPE type, CChoreoChannel *channel,
                          CChoreoEvent *e, bool forward);
  ...
  bool GenerateCombinedFile(char const *outfilename, const char *cctoken,
                            gender_t gender,
                            CUtlRBTree<CChoreoEvent *> &sorted);
  bool ValidateCombinedFileCheckSum(char const *outfilename,
  ...
  bool IsMouseOverEventEdge(CChoreoEventWidget *ew, bool bLeftEdge, int mx,
                            int my);
  ...
  // For mouse dragging
  // How close to right or left edge the mouse has to be before the
  // wider/shorten
  //  cursor shows up
  enum {
...
template <class T> void CChoreoView::SetPreservedTimeZoom(T *other, int tz) {
  POINT pt;
  ::GetCursorPos(&pt);
...
template <class T> int CChoreoView::HandleZoomKey(T *other, int keyCode) {
  int iret = 1;
  switch (keyCode) {
  default: {
    iret = 0;
  ...
  case VK_END: {
    ...
    int pixels =
        max(0, (int)(maxtime * other->GetPixelsPerSecond()) - other->w2());
  ...
  case VK_NEXT: // PgDown
  {
    ...
    int pixels =
        max(0, (int)(maxtime * other->GetPixelsPerSecond()) - other->w2());
    other->m_flLeftOffset =
        min(other->m_flLeftOffset + (float)window, (float)pixels);
    other->MoveTimeSliderToPos((int)other->m_flLeftOffset);
  } break;
  }
