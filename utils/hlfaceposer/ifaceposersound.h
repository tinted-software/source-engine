...
public:
  ...
  virtual void PlaySound(StudioModel *model, float volume, const char *wavfile,
                         CAudioMixer **ppMixer) = 0;
  virtual void PlaySound(CAudioSource *source, float volume,
                         CAudioMixer **ppMixer) = 0;
  virtual void PlayPartialSound(StudioModel *model, float volume,
  ...
  virtual void RenderWavToDC(HDC dc, RECT &outrect, COLORREF clr,
                             ...
                             CAudioSource *pWave, bool selected = false,
                             int selectionstart = 0, int selectionend = 0) = 0;

  virtual float GetAmountofTimeAhead(void) = 0;
  virtual int GetNumberofSamplesAhead(void) = 0;

  // virtual void		InstallPhonemecallback( IPhonemeTag
  // *pTagInterface ) = 0;
