...
const char *CChoreoEventWidget::GetLabelText(void) {
  ...
  if (GetEvent()->GetType() == CChoreoEvent::EXPRESSION) {
    sprintf(label, "%s : %s", GetEvent()->GetParameters(),
            GetEvent()->GetParameters2());
  } else {
...
//			&rcWAV -
//-----------------------------------------------------------------------------
void CChoreoEventWidget::DrawRelativeTags(CChoreoWidgetDrawHelper &drawHelper,
                                          RECT &rcWAV, float length,
                                          CChoreoEvent *event) {
  for (int i = 0; i < event->GetNumRelativeTags(); i++) {
    ...
    int left =
        rcWAV.left +
        (int)(tag->GetPercentage() * (float)(rcWAV.right - rcWAV.left) + 0.5f);
    ...
    drawHelper.DrawColoredText("Arial", 9, FW_NORMAL, RGB(0, 100, 200), rcText,
                               tag->GetName());
  }
}
...
//			&rcWAV -
//-----------------------------------------------------------------------------
void CChoreoEventWidget::DrawAbsoluteTags(CChoreoWidgetDrawHelper &drawHelper,
                                          RECT &rcWAV, float length,
                                          CChoreoEvent *event) {
...
//			rcBounds -
//-----------------------------------------------------------------------------
void CChoreoEventWidget::redrawStatus(CChoreoWidgetDrawHelper &drawHelper,
                                      RECT &rcClient) {
  if (!getVisible())
    return;
  ...
  if (event->IsResumeCondition()) {
    ...
    RECT rcFixed = rc;
    drawHelper.OffsetSubRect(rcFixed);
    DrawBitmapToDC(dc, rcFixed.left, rcFixed.top, rcFixed.right - rcFixed.left,
                   rcFixed.bottom - rcFixed.top,
  ...
  drawHelper.CalcTextWidth("Arial", fontsize, FW_NORMAL, "%s event \"%s\"",
                           event->NameForType(event->GetType()),
                           event->GetName());
  drawHelper.DrawColoredText("Arial", fontsize, FW_NORMAL, COLOR_INFO_TEXT,
                             rcEventLabel, "%s event \"%s\"",
                             event->NameForType(event->GetType()),
                             event->GetName());
...
void CChoreoEventWidget::DrawSpeakEvent(CChoreoWidgetDrawHelper &drawHelper,
                                        RECT &rcEventLine) {
  if (!m_pWaveFile)
    return;
  ...
  HBRUSH brBackground =
      CreateSolidBrush(GrayOutColor(COLOR_CHOREO_DARKBACKGROUND));
  ...
  if (IsSelected()) {
    sound->RenderWavToDC(dc, rcEventLine,
                         GrayOutColor(IsSelected() ? COLOR_CHOREO_EVENT_SELECTED
                                                   : COLOR_CHOREO_EVENT),
                         0.0, m_pWaveFile->GetRunningLength(), m_pWaveFile);
  }
  ...
  // rcEventLine.top -= 3;
  DrawRelativeTags(drawHelper, rcEventLine, m_pWaveFile->GetRunningLength(),
                   event);
}

void CChoreoEventWidget::DrawGestureEvent(CChoreoWidgetDrawHelper &drawHelper,
                                          RECT &rcEventLine) {
  CChoreoEvent *event = GetEvent();
  Assert(event);
  ...
  */
  ...
  COLORREF clrEvent = GrayOutColor(IsSelected() ? COLOR_CHOREO_EVENT_SELECTED
                                                : COLOR_CHOREO_EVENT);
  ...
  if (ramponly && IsSelected()) {
    ...
    drawHelper.DrawColoredLine(GrayOutColor(RGB(127, 127, 127)), PS_SOLID, 1,
                               rcEventLine2.left, rcEventLine2.bottom,
                               rcEventLine2.left, rcEventLine2.top);
    drawHelper.DrawColoredLine(GrayOutColor(RGB(127, 127, 127)), PS_SOLID, 1,
                               rcEventLine2.left, rcEventLine2.top,
                               rcEventLine2.right, rcEventLine2.top);
    drawHelper.DrawColoredLine(GrayOutColor(RGB(31, 31, 31)), PS_SOLID, 1,
                               rcEventLine2.right, rcEventLine2.top,
                               rcEventLine2.right, rcEventLine2.bottom);
    drawHelper.DrawColoredLine(GrayOutColor(RGB(0, 0, 0)), PS_SOLID, 1,
                               rcEventLine2.right, rcEventLine2.bottom,
                               rcEventLine2.left, rcEventLine2.bottom);
  }
  ...
  //	drawHelper.DrawColoredLine( clrBottom, PS_SOLID, 1, rampstart,
  //rcEventLine2.bottom, 		rcEventLine2.left, rcEventLine2.bottom );
  //	drawHelper.DrawColoredLine( clrBottom, PS_SOLID, 1, rcEventLine2.right,
  //rcEventLine2.bottom, 		rampend, rcEventLine2.bottom );

  if (!nullevent) {
    drawHelper.DrawColoredRamp(clrEvent, PS_SOLID, 1, rampstart,
                               rcEventLine2.bottom, rcEventLine2.left,
                               rcEventLine2.top, 0.0f, 1.0f);
    drawHelper.DrawColoredRamp(clrEvent, PS_SOLID, 1, rcEventLine2.right,
                               rcEventLine2.top, rampend, rcEventLine2.bottom,
                               0.0f, 1.0f);
  }

  g_pRampTool->DrawSamplesSimple(drawHelper, event, false,
                                 GrayOutColor(RGB(63, 63, 63)), rcEventLine);
...
void CChoreoEventWidget::DrawGenericEvent(CChoreoWidgetDrawHelper &drawHelper,
                                          RECT &rcEventLine) {
  bool ramponly = m_pView->IsRampOnly();
...
void CChoreoEventWidget::redraw(CChoreoWidgetDrawHelper &drawHelper) {
  ...
  if (event->GetType() == CChoreoEvent::SPEAK && m_pWaveFile &&
      !event->HasEndTime()) {
    event->SetEndTime(event->GetStartTime() + m_pWaveFile->GetRunningLength());
    rcEvent.right =
        (int)(m_pWaveFile->GetRunningLength() * m_pView->GetPixelsPerSecond());
  }
  ...
  if (event->HasEndTime()) {
    ...
    drawHelper.DrawColoredLine(GrayOutColor(COLOR_CHOREO_EVENT), PS_SOLID, 3,
                               rcEventLine.left - 1, rcEventLine.top,
  ...
  if (event->IsUsingRelativeTag()) {
    ...
    drawHelper.DrawColoredText("Arial", 9, FW_NORMAL,
                               GrayOutColor(RGB(0, 100, 200)), rcTagName,
                               event->GetRelativeTagName());

    drawHelper.DrawFilledRect(GrayOutColor(RGB(0, 100, 250)),
                              rcTagName.right - 1, rcTagName.top - 2,
  ...
  int len =
      drawHelper.CalcTextWidth("Arial", fontsize, FW_NORMAL, event->GetName());

  rcEventLabel.right = rcEventLabel.left + len + 2;
  drawHelper.DrawColoredText("Arial", fontsize, FW_NORMAL,
                             GrayOutColor(RGB(0, 0, 120)), rcEventLabel,
                             event->GetName());
}
...
// Output : CChoreoEvent
//-----------------------------------------------------------------------------
CChoreoEvent *CChoreoEventWidget::GetEvent(void) { return m_pEvent; }
...
//-----------------------------------------------------------------------------
// Purpose: If the user changes the association of .mdls to actors, then the
// gender could change and we could need to access a different .wav file Input
// :  -
//-----------------------------------------------------------------------------
void CChoreoEventWidget::RecomputeWave() {
  if (m_pEvent->GetType() == CChoreoEvent::SPEAK) {
    delete m_pWaveFile;
    m_pWaveFile = sound->LoadSound(
        va("sound/%s", FacePoser_TranslateSoundName(m_pEvent)));
  }
}
...
void CChoreoEventWidget::LoadImages(void) {
  ...
  LoadBitmapFromFile("gfx/hlfaceposer/ev_expression.bmp",
                     m_Bitmaps[CChoreoEvent::EXPRESSION]);
  LoadBitmapFromFile("gfx/hlfaceposer/ev_lookat.bmp",
                     m_Bitmaps[CChoreoEvent::LOOKAT]);
  LoadBitmapFromFile("gfx/hlfaceposer/ev_moveto.bmp",
                     m_Bitmaps[CChoreoEvent::MOVETO]);
  LoadBitmapFromFile("gfx/hlfaceposer/ev_speak.bmp",
                     m_Bitmaps[CChoreoEvent::SPEAK]);
  LoadBitmapFromFile("gfx/hlfaceposer/ev_gesture.bmp",
                     m_Bitmaps[CChoreoEvent::GESTURE]);
  LoadBitmapFromFile("gfx/hlfaceposer/ev_face.bmp",
                     m_Bitmaps[CChoreoEvent::FACE]);

  LoadBitmapFromFile("gfx/hlfaceposer/ev_firetrigger.bmp",
                     m_Bitmaps[CChoreoEvent::FIRETRIGGER]);
  LoadBitmapFromFile("gfx/hlfaceposer/ev_sequence.bmp",
                     m_Bitmaps[CChoreoEvent::SEQUENCE]);
  LoadBitmapFromFile("gfx/hlfaceposer/ev_flexanimation.bmp",
                     m_Bitmaps[CChoreoEvent::FLEXANIMATION]);
  LoadBitmapFromFile("gfx/hlfaceposer/ev_subscene.bmp",
                     m_Bitmaps[CChoreoEvent::SUBSCENE]);
  LoadBitmapFromFile("gfx/hlfaceposer/ev_loop.bmp",
                     m_Bitmaps[CChoreoEvent::LOOP]);

  LoadBitmapFromFile("gfx/hlfaceposer/pause.bmp", m_ResumeConditionBitmap);

  LoadBitmapFromFile("gfx/hlfaceposer/ev_interrupt.bmp",
                     m_Bitmaps[CChoreoEvent::INTERRUPT]);
  LoadBitmapFromFile("gfx/hlfaceposer/ev_stoppoint.bmp",
                     m_Bitmaps[CChoreoEvent::STOPPOINT]);
  LoadBitmapFromFile("gfx/hlfaceposer/ev_permit_response.bmp",
                     m_Bitmaps[CChoreoEvent::PERMIT_RESPONSES]);
  LoadBitmapFromFile("gfx/hlfaceposer/ev_generic.bmp",
                     m_Bitmaps[CChoreoEvent::GENERIC]);
