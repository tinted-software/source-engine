...
//			0 -
//-----------------------------------------------------------------------------
CActorBitmapButton::CActorBitmapButton(CChoreoActorWidget *actor,
                                       mxWindow *parent, int x, int y, int w,
                                       int h, int id /*= 0*/,
                                       const char *bitmap /*= 0*/)
...
// Purpose:
//-----------------------------------------------------------------------------
CChoreoActorWidget *CActorBitmapButton::GetActor(void) { return m_pActor; }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
CActorActiveCheckBox::CActorActiveCheckBox(CChoreoActorWidget *actor,
                                           mxWindow *parent, int x, int y,
                                           int w, int h,
                                           const char *label /*= 0*/,
                                           int id /*= 0*/)
...
CChoreoActorWidget *CActorActiveCheckBox::GetActor(void) { return m_pActor; }
...
CChoreoActorWidget::CChoreoActorWidget(CChoreoWidget *parent)
  ...
  m_btnOpen = new CActorBitmapButton(this, m_pView, 0, 0, 0, 0, IDC_CHANNELOPEN,
                                     "gfx/hlfaceposer/channelopen.bmp");
  m_btnClose =
      new CActorBitmapButton(this, m_pView, 0, 0, 0, 0, IDC_CHANNELCLOSE,
                             "gfx/hlfaceposer/channelclose.bmp");
...
//-----------------------------------------------------------------------------
bool CChoreoActorWidget::GetShowChannels(void) { return m_bShowChannels; }
...
void CChoreoActorWidget::Layout(RECT &rc) {
  ...
  bool buttonsVisible = (ypos > m_pView->GetStartRow() &&
                         (ypos + buttonSize) < m_pView->GetEndRow())
                            ? true
                            : false;
...
void CChoreoActorWidget::redraw(CChoreoWidgetDrawHelper &drawHelper) {
  ...
  drawHelper.DrawColoredLine(COLOR_CHOREO_ACTORLINE, PS_SOLID, 1, 0,
                             rcClient.bottom - 2, rcClient.right,
                             rcClient.bottom - 2);
  drawHelper.DrawColoredLine(RGB(200, 206, 255), PS_SOLID, 1, 0,
                             rcClient.bottom - 1, rcClient.right,
                             rcClient.bottom - 1);

  drawHelper.DrawColoredLine(COLOR_CHOREO_DIVIDER, PS_SOLID, 1, rcText.right,
                             rcClient.top, rcText.right, rcClient.bottom - 1);
  ...
  if (!actor->GetActive()) {
    ...
    int len =
        drawHelper.CalcTextWidth("Arial", m_pView->GetFontSize() - 2, 500, n);
  ...
  if (actor->GetFacePoserModelName()[0]) {
    int textWidth = drawHelper.CalcTextWidth(
        "Arial", m_pView->GetFontSize() + 5, 1000, actor->GetName());
    ...
    drawHelper.DrawColoredText("Arial", fontsize, FW_NORMAL,
                               COLOR_CHOREO_LIGHTTEXT, rcModelName, shortname);
  }
  if (m_bShowChannels) {
    for (int j = 0; j < GetNumChannels(); j++) {
      ...
      drawHelper.DrawColoredLine(COLOR_CHOREO_ACTORLINE, PS_SOLID, 1,
                                 rcText.right + 1, rcChannel.top,
                                 rcChannel.right, rcChannel.top);

      drawHelper.DrawColoredLine(COLOR_CHOREO_ACTORLINE, PS_SOLID, 1,
                                 rcText.right + 1, rcChannel.bottom,
  ...
  drawHelper.DrawColoredText("Arial", m_pView->GetFontSize(), FW_NORMAL,
                             COLOR_CHOREO_ACTORNAME, rcName, sz);
}
