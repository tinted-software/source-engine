...
MDLViewer::MDLViewer()
    : mxWindow(0, 0, 0, 0, 0, g_appTitle, mxWindow::Normal),
      menuCloseCaptionLanguages(0), m_bOldSoundScriptsDirty(-1),
      m_bVCDSaved(false) {
  int i;
...
void MDLViewer::UpdateLanguageMenu(int currentLanguageId) {
  ...
  for (int i = 0; i < CC_NUM_LANGUAGES; i++) {
    int id = IDC_OPTIONS_LANGUAGESTART + i;
    menuCloseCaptionLanguages->setChecked(id, i == currentLanguageId ? true
                                                                     : false);
  }
}
...
int MDLViewer::handleEvent(mxEvent *event) {
  ...
  case mxEvent::Size: {
    ...
    gridsettings->setBounds(gridstart, height - rowheight + 1, gridsettingswide,
                            WINDOW_TAB_OFFSET - 2);
  ...
  case mxEvent::Action: {
    ...
    case IDC_HELP_GOTOHOMEPAGE:
      ShellExecute(
          0, "open",
          "http://developer.valvesoftware.com/wiki/Category:Choreography", 0, 0,
          SW_SHOW);
      break;
#endif
    ...
    case IDC_HELP_ABOUT:
      mxMessageBox(this,
                   "v1.0  Copyright © 1996-2007, Valve Corporation. All rights "
                   "reserved.\r\nBuild Date: " __DATE__ "",
...
static bool
CHLFacePoserApp_SuggestGameInfoDirFn(CFSSteamSetupInfo const *pFsSteamSetupInfo,
