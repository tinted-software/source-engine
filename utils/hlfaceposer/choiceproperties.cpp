...
static void PopulateChoiceList(HWND wnd, CChoiceParams *params) {
  ...
  if (params->m_nSelected >= 0 && params->m_nSelected < c) {
    SendMessage(control, WM_SETTEXT, 0,
                (LPARAM)params->m_Choices[params->m_nSelected].choice);
  }
...
//-----------------------------------------------------------------------------
static BOOL CALLBACK ChoicePropertiesDialogProc(HWND hwndDlg, UINT uMsg,
  ...
  case WM_COMMAND:
    ...
    case IDOK: {
      ...
      if (control) {
        SendMessage(control, WM_GETTEXT, (WPARAM)sizeof(selected),
                    (LPARAM)selected);
      }
...
int ChoiceProperties(CChoiceParams *params) {
  ...
  int retval = DialogBox(
      (HINSTANCE)GetModuleHandle(0), MAKEINTRESOURCE(IDD_CHOICEDIALOG),
