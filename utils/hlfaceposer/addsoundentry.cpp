...
static void PopulateScriptList(HWND wnd) {
  ...
  for (int i = 0; i < c; i++) {
    ...
    SendMessage(control, CB_ADDSTRING, 0,
                (LPARAM)soundemitter->GetSoundScriptName(i));
...
//-----------------------------------------------------------------------------
static BOOL CALLBACK AddSoundPropertiesDialogProc(HWND hwndDlg, UINT uMsg,
                                                  WPARAM wParam,
                                                  LPARAM lParam) {
  switch (uMsg) {
  case WM_INITDIALOG:
  ...
  case WM_COMMAND:
    ...
    case IDOK: {
      ...
      GetDlgItemText(hwndDlg, IDC_SOUNDNAME, g_Params.m_szSoundName,
                     sizeof(g_Params.m_szSoundName));
      GetDlgItemText(hwndDlg, IDC_SOUNDSCRIPT, g_Params.m_szScriptName,
                     sizeof(g_Params.m_szScriptName));
      ...
      if (soundemitter->IsValidIndex(idx)) {
        if (!g_Params.m_bAllowExistingSound) {
          mxMessageBox(NULL,
                       va("Sound '%s' already exists", g_Params.m_szSoundName),
                       g_appTitle, MX_MB_OK);
        } else {
          EndDialog(hwndDlg, 1);
        }
      ...
      if (!filesystem->FileExists(g_Params.m_szScriptName)) {
        mxMessageBox(NULL,
                     va("Script '%s' does not exist", g_Params.m_szScriptName),
                     g_appTitle, MX_MB_OK);
        return TRUE;
      }
      ...
      if (!filesystem->IsFileWritable(g_Params.m_szScriptName)) {
        mxMessageBox(
            NULL,
            va("Script '%s' is read-only, you need to check it out of VSS",
               g_Params.m_szScriptName),
            g_appTitle, MX_MB_OK);
        return TRUE;
      }
      ...
      soundemitter->AddSound(g_Params.m_szSoundName, g_Params.m_szScriptName,
                             params);
...
int AddSound(CAddSoundParams *params, HWND parent) {
  ...
  int retval = DialogBox((HINSTANCE)GetModuleHandle(0),
                         MAKEINTRESOURCE(IDD_ADDSOUNDENTRY), parent,
