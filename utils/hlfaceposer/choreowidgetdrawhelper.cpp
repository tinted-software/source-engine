...
CChoreoWidgetDrawHelper::CChoreoWidgetDrawHelper(mxWindow *widget,
                                                 RECT &bounds) {
  Init(widget, bounds.left, bounds.top, bounds.right - bounds.left,
       bounds.bottom - bounds.top, COLOR_CHOREO_BACKGROUND, false);
}
...
CChoreoWidgetDrawHelper::CChoreoWidgetDrawHelper(mxWindow *widget, RECT &bounds,
                                                 bool noPageFlip) {
  Init(widget, bounds.left, bounds.top, bounds.right - bounds.left,
       bounds.bottom - bounds.top, COLOR_CHOREO_BACKGROUND, noPageFlip);
}
...
//			h -
//-----------------------------------------------------------------------------
CChoreoWidgetDrawHelper::CChoreoWidgetDrawHelper(mxWindow *widget, int x, int y,
                                                 int w, int h,
                                                 COLORREF bgColor) {
...
CChoreoWidgetDrawHelper::CChoreoWidgetDrawHelper(mxWindow *widget, RECT &bounds,
                                                 COLORREF bgColor) {
  Init(widget, bounds.left, bounds.top, bounds.right - bounds.left,
       bounds.bottom - bounds.top, bgColor, false);
}
...
//			h -
//-----------------------------------------------------------------------------
void CChoreoWidgetDrawHelper::Init(mxWindow *widget, int x, int y, int w, int h,
                                   COLORREF bgColor, bool noPageFlip) {
...
// Output : int
//-----------------------------------------------------------------------------
int CChoreoWidgetDrawHelper::GetWidth(void) { return m_w; }

//-----------------------------------------------------------------------------
// Purpose:
// Output : int
//-----------------------------------------------------------------------------
int CChoreoWidgetDrawHelper::GetHeight(void) { return m_h; }
...
// Output : HDC
//-----------------------------------------------------------------------------
HDC CChoreoWidgetDrawHelper::GrabDC(void) { return m_dcMemory; }
...
//			... -
//-----------------------------------------------------------------------------
void CChoreoWidgetDrawHelper::CalcTextRect(const char *font, int pointsize,
                                           int weight, int maxwidth,
  ...
  HFONT fnt = CreateFont(-pointsize, 0, 0, 0, weight, FALSE, FALSE, FALSE,
                         ANSI_CHARSET, OUT_TT_PRECIS, CLIP_DEFAULT_PRECIS,
                         ANTIALIASED_QUALITY, DEFAULT_PITCH, font);

  HFONT oldFont = (HFONT)SelectObject(m_dcMemory, fnt);

  DrawText(m_dcMemory, output, -1, &rcText,
           DT_LEFT | DT_NOPREFIX | DT_VCENTER | DT_WORDBREAK | DT_CALCRECT);
...
// Output : int
//-----------------------------------------------------------------------------
int CChoreoWidgetDrawHelper::CalcTextWidth(const char *font, int pointsize,
  ...
  DrawText(screen, output, -1, &rcText,
           DT_LEFT | DT_NOPREFIX | DT_VCENTER | DT_SINGLELINE | DT_CALCRECT);
...
// Output : int
//-----------------------------------------------------------------------------
int CChoreoWidgetDrawHelper::CalcTextWidthW(const char *font, int pointsize,
                                            int weight, const wchar_t *fmt,
                                            ...) {
  va_list args;
  ...
  DrawTextW(screen, output, -1, &rcText,
            DT_LEFT | DT_NOPREFIX | DT_VCENTER | DT_SINGLELINE | DT_CALCRECT);
...
//			... -
//-----------------------------------------------------------------------------
void CChoreoWidgetDrawHelper::DrawColoredText(const char *font, int pointsize,
                                              int weight, COLORREF clr,
                                              RECT &rcText, const char *fmt,
                                              ...) {
  va_list args;
  static char output[1024];
  ...
  DrawColoredTextCharset(font, pointsize, weight, ANSI_CHARSET, clr, rcText,
                         output);
}
...
//			... -
//-----------------------------------------------------------------------------
void CChoreoWidgetDrawHelper::DrawColoredTextW(const char *font, int pointsize,
                                               int weight, COLORREF clr,
  ...
  DrawColoredTextCharsetW(font, pointsize, weight, ANSI_CHARSET, clr, rcText,
                          output);
}
...
//			... -
//-----------------------------------------------------------------------------
void CChoreoWidgetDrawHelper::DrawColoredText(HFONT font, COLORREF clr,
                                              RECT &rcText, const char *fmt,
                                              ...) {
  va_list args;
  static char output[1024];
...
//			... -
//-----------------------------------------------------------------------------
void CChoreoWidgetDrawHelper::DrawColoredTextW(HFONT font, COLORREF clr,
                                               RECT &rcText, const wchar_t *fmt,
                                               ...) {
  va_list args;
  static wchar_t output[1024];
...
//			... -
//-----------------------------------------------------------------------------
void CChoreoWidgetDrawHelper::DrawColoredTextCharset(const char *font,
                                                     int pointsize, int weight,
                                                     DWORD charset,
                                                     COLORREF clr, RECT &rcText,
                                                     const char *fmt, ...) {
  va_list args;
  static char output[1024];
...
void CChoreoWidgetDrawHelper::DrawColoredTextCharsetW(
    const char *font, int pointsize, int weight, DWORD charset, COLORREF clr,
    RECT &rcText, const wchar_t *fmt, ...) {
  va_list args;
  static wchar_t output[1024];
...
//			... -
//-----------------------------------------------------------------------------
void CChoreoWidgetDrawHelper::DrawColoredTextMultiline(const char *font,
                                                       int pointsize,
                                                       int weight, COLORREF clr,
                                                       RECT &rcText,
                                                       const char *fmt, ...) {
  ...
  DrawText(m_dcMemory, output, -1, &rcTextOffset,
           DT_LEFT | DT_NOPREFIX | DT_VCENTER | DT_WORDBREAK |
               DT_WORD_ELLIPSIS);
...
//			y2 -
//-----------------------------------------------------------------------------
void CChoreoWidgetDrawHelper::DrawColoredLine(COLORREF clr, int style,
                                              int width, int x1, int y1, int x2,
                                              int y2) {
...
//			*pts -
//-----------------------------------------------------------------------------
void CChoreoWidgetDrawHelper::DrawColoredPolyLine(COLORREF clr, int style,
                                                  int width,
...
//			y2 -
//-----------------------------------------------------------------------------
POINTL CChoreoWidgetDrawHelper::DrawColoredRamp(COLORREF clr, int style,
                                                int width, int x1, int y1,
                                                int x2, int y2, float rate,
                                                float sustain) {
  HPEN pen = CreatePen(style, width, clr);
  HPEN oldPen = (HPEN)SelectObject(m_dcMemory, pen);
  MoveToEx(m_dcMemory, x1 - m_x, y1 - m_y, NULL);
...
//			y2 -
//-----------------------------------------------------------------------------
void CChoreoWidgetDrawHelper::DrawFilledRect(COLORREF clr, int x1, int y1,
                                             int x2, int y2) {
  HBRUSH br = CreateSolidBrush(clr);
  RECT rc;
  rc.left = x1;
...
//			rc -
//-----------------------------------------------------------------------------
void CChoreoWidgetDrawHelper::DrawOutlinedRect(COLORREF clr, int style,
                                               int width, RECT &rc) {
...
//			thickness -
//-----------------------------------------------------------------------------
void CChoreoWidgetDrawHelper::DrawLine(int x1, int y1, int x2, int y2,
                                       COLORREF clr, int thickness) {
  HPEN oldpen, pen;
  HBRUSH oldbrush, brush;
...
//			fillb -
//-----------------------------------------------------------------------------
void CChoreoWidgetDrawHelper::DrawTriangleMarker(RECT &rc, COLORREF fill,
                                                 bool inverted /*= false*/) {
  POINT region[3];
  int cPoints = 3;
...
void CChoreoWidgetDrawHelper::DrawCircle(COLORREF clr, int x, int y, int radius,
                                         bool filled /*= true*/) {
  RECT rc;
  ...
  if (filled) {
    ...
    Arc(m_dcMemory, rc.left, rc.top, rc.right, rc.bottom, rc.left, rc.top,
        rc.left, rc.top);
  }
...
//			vertical -
//-----------------------------------------------------------------------------
void CChoreoWidgetDrawHelper::DrawGradientFilledRect(RECT &rc, COLORREF clr1,
                                                     COLORREF clr2,
                                                     bool vertical) {
  RECT rcDraw = rc;
  ...
  GradientFill(m_dcMemory, vert, 2, &gradient_rect, 1,
