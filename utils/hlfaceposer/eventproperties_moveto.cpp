...
void CEventPropertiesMoveToDialog::InitControlData(CEventParams *params) {
  ...
  SendMessage(control, TBM_SETPOS, 1,
              (LPARAM)(LONG)params->m_flDistanceToTarget);

  SendMessage(GetControl(IDC_CHECK_FORCESHORTMOVEMENT), BM_SETCHECK,
              (WPARAM)g_Params.m_bForceShortMovement ? BST_CHECKED
                                                     : BST_UNCHECKED,
              (LPARAM)0);

  PopulateNamedActorList(choices1, params);

  SendMessage(GetControl(IDC_CHOICES2PROMPT), WM_SETTEXT, 0,
              (LPARAM) "Movement Style:");
...
void CEventPropertiesMoveToDialog::PopulateMovementStyle(HWND control,
                                                         CEventParams *params) {
...
void CEventPropertiesMoveToDialog::SetDistanceToTargetText(
    CEventParams *params) {
  HWND control;

  control = GetControl(IDC_STATIC_DISTANCEVAL);
  SendMessage(control, WM_SETTEXT, 0,
              (LPARAM)va("%i", (int)params->m_flDistanceToTarget));
}
...
void CEventPropertiesMoveToDialog::ShowControlsForEventType(
    CEventParams *params) {
...
//-----------------------------------------------------------------------------
static BOOL CALLBACK EventPropertiesMoveToDialogProc(HWND hwndDlg, UINT uMsg,
                                                     WPARAM wParam,
                                                     LPARAM lParam) {
  return g_EventPropertiesMoveToDialog.HandleMessage(hwndDlg, uMsg, wParam,
                                                     lParam);
};

BOOL CEventPropertiesMoveToDialog::HandleMessage(HWND hwndDlg, UINT uMsg,
  ...
  case WM_HSCROLL: {
    ...
    if (control == GetControl(IDC_SLIDER_DISTANCE)) {
      g_Params.m_flDistanceToTarget =
          (float)SendMessage(GetControl(IDC_SLIDER_DISTANCE), TBM_GETPOS, 0, 0);
  ...
  case WM_COMMAND:
    ...
    case IDOK: {
      ...
      if (control) {
        SendMessage(control, WM_GETTEXT,
                    (WPARAM)sizeof(g_Params.m_szParameters),
      ...
      if (!g_Params.m_szName[0]) {
        Q_snprintf(g_Params.m_szName, sizeof(g_Params.m_szName), "Moveto %s",
    ...
    case IDC_EVENTCHOICES2: {
      ...
      if (control) {
        if (g_Params.m_nType != CChoreoEvent::MOVETO) {
          ...
          char buf1[256];
          SendMessage(GetControl(IDC_EVENTCHOICES2), WM_GETTEXT,
                      (WPARAM)sizeof(buf1), (LPARAM)buf1);

          Q_snprintf(g_Params.m_szParameters2, sizeof(g_Params.m_szParameters2),
                     "%s", buf1);
        }
      }
    } break;
    case IDC_EVENTCHOICES3: {
      ...
      if (control) {
        if (g_Params.m_nType != CChoreoEvent::MOVETO) {
          SendMessage(control, WM_GETTEXT,
                      (WPARAM)sizeof(g_Params.m_szParameters3),
                      (LPARAM)g_Params.m_szParameters3);
        } else {
          char buf1[256];
          SendMessage(GetControl(IDC_EVENTCHOICES3), WM_GETTEXT,
                      (WPARAM)sizeof(buf1), (LPARAM)buf1);

          Q_snprintf(g_Params.m_szParameters3, sizeof(g_Params.m_szParameters3),
                     "%s", buf1);
        }
      }
    } break;
    ...
    case IDC_CHECK_FORCESHORTMOVEMENT: {
      g_Params.m_bForceShortMovement =
          SendMessage(GetControl(IDC_CHECK_FORCESHORTMOVEMENT), BM_GETCHECK, 0,
                      0) == BST_CHECKED
              ? true
              : false;
    } break;
