...
enum { COL_WAV = 0, COL_DUCKED, COL_PHONEMES, COL_SENTENCE };
...
public:
  CWaveFileTree(mxWindow *parent, int id = 0)
      : mxTreeView(parent, 0, 0, 0, 0, id), m_Paths(0, 0, FileTreeLessFunc) {}
  ...
  mxTreeViewItem *FindOrAddChildItem(mxTreeViewItem *parent,
                                     char const *child) {
  ...
  void PopulateTree() {
    int i;
    for (i = m_Paths.FirstInorder(); i != m_Paths.InvalidIndex();
         i = m_Paths.NextInorder(i)) {
  ...
  static bool FileTreeLessFunc(const FileTreePath &lhs,
                               const FileTreePath &rhs) {
...
public:
  ...
  CWaveOptionsWindow(CWaveBrowser *browser)
      : BaseClass(browser, 0, 0, 0, 0), m_pBrowser(browser) {
    ...
    m_pStopSounds =
        new mxButton(this, 0, 0, 0, 0, "Stop Sounds", IDC_STOP_SOUNDS);

    m_pSearch = new mxLineEdit(this, 0, 0, 0, 0, "", IDC_SEARCH);

    m_pCancelSearch =
        new mxButton(this, 0, 0, 0, 0, "Cancel", IDC_CANCELSEARCH);
  }
...
CWaveBrowser::CWaveBrowser(mxWindow *parent)
    : IFacePoserToolWindow("WaveBrowser", "Waves"),
...
MAKEINTRESOURCE(IDI_WORKSPACE_CHECKEDOUT)); slot = ImageList_AddIcon(list,
...
slot = ImageList_AddIcon(list, hicon);
  ...
  hicon = LoadIcon(GetModuleHandle( 0 ),
MAKEINTRESOURCE(IDI_PROJECT_CHECKEDOUT)); slot = ImageList_AddIcon(list, hicon);
  Assert( slot == c++ );
  DeleteObject( hicon );
...
// Purpose:
//-----------------------------------------------------------------------------
void CWaveBrowser::OnDelete() { RemoveAllSounds(); }
...
int CWaveBrowser::handleEvent(mxEvent *event) {
  ...
  case mxEvent::Action: {
    ...
    case IDC_SB_LISTVIEW: {
      ...
      bool doubleclicked =
          (event->flags == mxEvent::DoubleClicked) ? true : false;
  ...
  case mxEvent::Size: {
    ...
    m_pListView->setBounds(filetreewidth, optionsh, w2() - filetreewidth,
                           h2() - optionsh);
...
//-----------------------------------------------------------------------------
bool CWaveBrowser::LoadWaveFilesInDirectory(
    CUtlDict<CWaveFile *, int> &soundlist, char const *pDirectoryName,
  ...
  while (pFileName) {
    if (!filesystem->FindIsDirectory(findHandle)) {
      ...
      Q_snprintf(pFileNameWithPath, nAllocSize, "%s/%s",
                 &pDirectoryName[SOUND_PREFIX_LEN], pFileName);
...
bool CWaveBrowser::InitDirectoryRecursive(CUtlDict<CWaveFile *, int> &soundlist,
...
void CWaveBrowser::PopulateTree(char const *subdirectory) {
  ...
  if (subdirectory) {
    ...
    check_load_sentence_data =
        (Q_strstr(subdirectory, "/") || subdirectory[0]) ? true : false;
  }
  ...
  for (i = m_Sorted.FirstInorder(); i != m_Sorted.InvalidIndex();
       i = m_Sorted.NextInorder(i)) {
    ...
    if (wav->HasLoadedSentenceInfo()) {
      m_pListView->setLabel(slot, COL_DUCKED,
                            wav->GetVoiceDuck() ? "yes" : "no");
      m_pListView->setLabel(
          slot, COL_PHONEMES,
...
static void SplitFileName(char const *in, char *path, int maxpath,
                          char *filename, int maxfilename) {
...
void CWaveBrowser::JumpToItem(CWaveFile *wav) {
  ...
  SplitFileName(wav->GetFileName(), path, sizeof(path), filename,
                sizeof(filename));
...
int CWaveBrowser::GetSoundCount() const { return m_AllSounds.Count(); }
...
void CWaveBrowser::SetCurrent(char const *filename) {
  ...
  for (i = 0; i < c; ++i) {
    CWaveFile *wav =
        reinterpret_cast<CWaveFile *>(m_pListView->getUserData(i, COL_WAV));
    if (!wav)
      continue;
