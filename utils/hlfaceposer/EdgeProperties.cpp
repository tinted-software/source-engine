...
static void PopulateCurveType(HWND control, CEdgePropertiesParams *params,
                              bool isLeftEdge) {
  SendMessage(control, CB_RESETCONTENT, 0, 0);

  for (int i = 0; i < NUM_INTERPOLATE_TYPES; ++i) {
    SendMessage(control, CB_ADDSTRING, 0,
                (LPARAM)Interpolator_NameForInterpolator(i, true));
  }

  SendMessage(control, CB_SETCURSEL,
              params->m_InterpolatorType[isLeftEdge ? 0 : 1], 0);
}

static void Reset(HWND hwndDlg, bool left) {
  SendMessage(GetDlgItem(hwndDlg, left ? IDC_LEFT_ACTIVE : IDC_RIGHT_ACTIVE),
              BM_SETCHECK, (WPARAM)BST_UNCHECKED, (LPARAM)0);
  SendMessage(
      GetDlgItem(hwndDlg, left ? IDC_LEFT_CURVETYPE : IDC_RIGHT_CURVETYPE),
      CB_SETCURSEL, 0, 0);
  SetDlgItemText(hwndDlg, left ? IDC_LEFT_ZEROVALUE : IDC_RIGHT_ZEROVALUE,
                 "0.0");
...
//-----------------------------------------------------------------------------
static BOOL CALLBACK EdgePropertiesDialogProc(HWND hwndDlg, UINT uMsg,
  ...
  case WM_INITDIALOG: {
    ...
    PopulateCurveType(GetDlgItem(hwndDlg, IDC_RIGHT_CURVETYPE), &g_Params,
                      false);

    SetDlgItemText(hwndDlg, IDC_LEFT_ZEROVALUE,
                   va("%f", g_Params.m_flValue[0]));
    SetDlgItemText(hwndDlg, IDC_RIGHT_ZEROVALUE,
                   va("%f", g_Params.m_flValue[1]));
  ...
  case WM_COMMAND:
    ...
    case IDOK: {
      ...
      g_Params.m_bActive[0] = SendMessage(GetDlgItem(hwndDlg, IDC_LEFT_ACTIVE),
                                          BM_GETCHECK, 0, 0) == BST_CHECKED
                                  ? true
                                  : false;
      g_Params.m_bActive[1] = SendMessage(GetDlgItem(hwndDlg, IDC_RIGHT_ACTIVE),
                                          BM_GETCHECK, 0, 0) == BST_CHECKED
                                  ? true
                                  : false;

      int interpolatorType;
      interpolatorType = SendMessage(GetDlgItem(hwndDlg, IDC_LEFT_CURVETYPE),
                                     CB_GETCURSEL, 0, 0);
      if (interpolatorType != CB_ERR) {
        g_Params.m_InterpolatorType[0] = interpolatorType;
      }
      interpolatorType = SendMessage(GetDlgItem(hwndDlg, IDC_RIGHT_CURVETYPE),
                                     CB_GETCURSEL, 0, 0);
      if (interpolatorType != CB_ERR) {
...
int EdgeProperties(CEdgePropertiesParams *params) {
  ...
  int retval = DialogBox(
      (HINSTANCE)GetModuleHandle(0), MAKEINTRESOURCE(IDD_EDGEPROPERTIES),
