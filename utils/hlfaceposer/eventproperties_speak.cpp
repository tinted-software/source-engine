...
public:
  ...
  virtual BOOL HandleMessage(HWND hwndDlg, UINT uMsg, WPARAM wParam,
                             LPARAM lParam);
...
void CEventPropertiesSpeakDialog::OnCheckFilterUpdate() {
  char curfilter[256];
  HWND control = GetControl(IDC_FILTER);
  SendMessage(control, WM_GETTEXT, (WPARAM)sizeof(curfilter),
              (LPARAM)curfilter);
...
void CEventPropertiesSpeakDialog::InitControlData(CEventParams *params) {
  ...
  SendMessage(attenuate, BM_SETCHECK,
              (WPARAM)params->m_bCloseCaptionNoAttenuate ? BST_CHECKED
                                                         : BST_UNCHECKED,
              0);
...
void CEventPropertiesSpeakDialog::PopulateVolumeLevels(HWND control,
                                                       CEventParams *params) {
  SendMessage(control, CB_RESETCONTENT, 0, 0);
  ...
  if (!Q_stristr(params->m_szParameters, ".wav")) {
    ...
    if (soundindex >= 0) {
      ...
      CSoundParametersInternal *params =
          soundemitter->InternalGetParametersForSound(soundindex);
      if (params) {
        // Found it
        ...
        char const *scriptfile =
            soundemitter->GetSourceFileForSound(soundindex);
        if (scriptfile) {
...
// Output : static void
//-----------------------------------------------------------------------------
void CEventPropertiesSpeakDialog::PopulateSoundList(char const *current,
                                                    HWND wnd) {
...
void CEventPropertiesSpeakDialog::ShowControlsForEventType(
    CEventParams *params) {
  BaseClass::ShowControlsForEventType(params);
}

void CEventPropertiesSpeakDialog::FindWaveInSoundEntries(
    CUtlVector<int> &entryList, char const *search) {
  int c = soundemitter->GetSoundCount();
  for (int i = 0; i < c; i++) {
    CSoundParametersInternal *params =
        soundemitter->InternalGetParametersForSound(i);
    if (!params)
      continue;
    ...
    for (int wave = 0; wave < waveCount; wave++) {
      char const *waveName =
          soundemitter->GetWaveName(params->GetSoundNames()[wave].symbol);
...
void CEventPropertiesSpeakDialog::OnCheckChangedVolumeLevel(
    CEventParams *params) {
  ...
  // Look up the sound level from the soundemitter system
  CSoundParametersInternal *soundparams =
      soundemitter->InternalGetParametersForSound(soundindex);
  if (!params) {
    return;
  }
  ...
  if (!scriptfile || !filesystem->FileExists(scriptfile) ||
      !filesystem->IsFileWritable(scriptfile)) {
    return;
  }
  ...
  SendMessage(control, WM_GETTEXT, (WPARAM)sizeof(newvolumelevel),
              (LPARAM)newvolumelevel);
...
//-----------------------------------------------------------------------------
static BOOL CALLBACK EventPropertiesSpeakDialog(HWND hwndDlg, UINT uMsg,
                                                WPARAM wParam, LPARAM lParam) {
  return g_EventPropertiesSpeakDialog.HandleMessage(hwndDlg, uMsg, wParam,
                                                    lParam);
};
...
void CEventPropertiesSpeakDialog::OnSoundSelected(CEventParams *params) {
  ...
  SendMessage(GetControl(IDC_SOUNDNAME), WM_SETTEXT, 0,
              (LPARAM)params->m_szParameters);
  ...
  if (soundindex >= 0) {
    ...
    if (script && script[0]) {
      SendMessage(scriptname, WM_SETTEXT, (WPARAM)Q_strlen(script) + 1,
                  (LPARAM)script);
      ...
      if (params) {
        // Get wave name
        char const *w =
            soundemitter->GetWaveName(params->GetSoundNames()[0].symbol);
        if (w && w[0]) {
...
BOOL CEventPropertiesSpeakDialog::HandleMessage(HWND hwndDlg, UINT uMsg,
                                                WPARAM wParam, LPARAM lParam) {
  m_hDialog = hwndDlg;

  bool handled = false;
  BOOL bret =
      InternalHandleMessage(&g_Params, hwndDlg, uMsg, wParam, lParam, handled);
  if (handled)
    return bret;
  ...
  case WM_COMMAND:
    ...
    case IDOK: {
      ...
      if (p) {
        Q_strncpy(g_Params.m_szParameters, p + strlen("\\sound\\"),
                  sizeof(g_Params.m_szParameters));
      }

      OnCheckChangedVolumeLevel(&g_Params);

      GetDlgItemText(m_hDialog, IDC_EVENTNAME, g_Params.m_szName,
                     sizeof(g_Params.m_szName));
    ...
    case IDC_SHOW_ALL_SOUNDS: {
      m_bShowAll = SendMessage(GetControl(IDC_SHOW_ALL_SOUNDS), BM_GETCHECK, 0,
                               0) == BST_CHECKED
                       ? true
                       : false;
    ...
    case IDC_CHECK_ENDTIME: {
      g_Params.m_bHasEndTime = SendMessage(GetControl(IDC_CHECK_ENDTIME),
    ...
    case IDC_CHECK_RESUMECONDITION: {
      g_Params.m_bResumeCondition =
          SendMessage(GetControl(IDC_CHECK_RESUMECONDITION), BM_GETCHECK, 0,
                      0) == BST_CHECKED
              ? true
              : false;
    } break;
    case IDC_SOUNDLIST: {
      ...
      if (control) {
        ...
        if (cursel != LB_ERR) {
          SendMessage(control, LB_GETTEXT, cursel,
          ...
          if (HIWORD(wParam) == LBN_DBLCLK) {
            ...
            sound->PlaySound(NULL, 1.0f,
                             va("sound/%s", FacePoser_TranslateSoundName(
    ...
    case IDC_EVENTCHOICES2: {
      HWND control = (HWND)lParam;
      if (control) {
        SendMessage(control, WM_GETTEXT,
                    (WPARAM)sizeof(g_Params.m_szParameters2),
...
int EventProperties_Speak(CEventParams *params) {
  ...
  int retval = DialogBox(
      (HINSTANCE)GetModuleHandle(0), MAKEINTRESOURCE(IDD_EVENTPROPERTIES_SPEAK),
