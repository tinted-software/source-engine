...
public:
  CCustomAnim(const FileNameHandle_t &h)
      : m_bDirty(false), m_ShortName(UTL_INVAL_SYMBOL) {
    m_Handle = h;
  }
...
private: // Methods
  void OnFilter();
  bool SequencePassesFilter(StudioModel *model, int sequence,
  ...
  void DrawThumbNail(int sequence, CChoreoWidgetDrawHelper &helper, int rcx,
                     int rcy, int rcw, int rch);

  void DrawSequenceFocusRect(CChoreoWidgetDrawHelper &helper, int x, int y,
                             int w, int h, COLORREF clr);
  void DrawSequenceDescription(CChoreoWidgetDrawHelper &helper, int x, int y,
                               int w, int h, int sequence,
                               mstudioseqdesc_t &seqdesc);
