...
public:
  ...
  CPhonemeModeTab(mxWindow *parent, int x, int y, int w, int h, int id = 0,
                  int style = 0)
...
PhonemeEditor::PhonemeEditor(mxWindow *parent)
  ...
  m_pHorzScrollBar = new mxScrollbar(this, 0, 0, 18, 100, IDC_PHONEME_SCROLL,
  ...
  m_btnSave =
      new mxButton(this, 0, 0, 16, 16, "Save (Ctrl+S)", IDC_SAVE_LINGUISTIC);
  m_btnRedoPhonemeExtraction = new mxButton(
      this, 38, 14, 80, 16, "Re-extract (Ctrl+R)", IDC_REDO_PHONEMEEXTRACTION);

  m_btnLoad = new mxButton(this, 0, 0, 0, 0, "Load (Ctrl+O)", IDC_LOADWAVEFILE);
  m_btnPlay =
      new mxButton(this, 0, 0, 16, 16, "Play (Spacebar)", IDC_PLAYBUTTON);
...
void PhonemeEditor::OnDelete() {
  if (m_pWaveFile) {
    char fn[512];
    Q_snprintf(fn, sizeof(fn), "%s%s", m_WorkFile.m_szBasePath,
               m_WorkFile.m_szWorkingFile);
...
bool PhonemeEditor::CanClose() {
  ...
  int retval = mxMessageBox(
      this, va("Save current changes to %s", m_WorkFile.m_szWaveFile),
...
//			positionDialog -
//-----------------------------------------------------------------------------
void PhonemeEditor::EditPhoneme(CPhonemeTag *pPhoneme,
                                bool positionDialog /*= false*/) {
  if (!pPhoneme) {
...
//			cursor -
//-----------------------------------------------------------------------------
void PhonemeEditor::StartDragging(int dragtype, int startx, int starty,
                                  HCURSOR cursor) {
    ...
    if (m_nDragType == DRAGTYPE_NONE) {
      ...
      if (IsMouseOverScrubArea(event)) {
        if (IsMouseOverScrubHandle(event)) {
          ...
          m_flScrubberTimeOffset =
              clamp(m_flScrubberTimeOffset, -maxoffset, maxoffset);
        ...
        if (sample) {
          if (shiftdown) {
            ...
            StartDragging(DRAGTYPE_EMPHASIS_MOVE, (short)event->x,
                          (short)event->y, LoadCursor(NULL, IDC_SIZEALL));
          } else {
            if (!shiftdown) {
              Emphasis_DeselectAll();
              redraw();
            ...
            StartDragging(DRAGTYPE_EMPHASIS_SELECT, (short)event->x,
                          (short)event->y, NULL);
          }
          return true;
        } else if (ctrldown) {
        ...
        if (type == BOUNDARY_PHONEME && m_nSelectedPhonemeCount <= 1) {
          StartDragging(DRAGTYPE_PHONEME, (short)event->x, (short)event->y,
                        LoadCursor(NULL, IDC_SIZEWE));
          return true;
        } else if (type == BOUNDARY_WORD && m_nSelectedWordCount <= 1) {
          StartDragging(DRAGTYPE_WORD, (short)event->x, (short)event->y,
                        LoadCursor(NULL, IDC_SIZEWE));
          return true;
        } else if (IsMouseOverSamples((short)event->x, (short)event->y)) {
          if (!m_bSelectionActive) {
            StartDragging(DRAGTYPE_SELECTSAMPLES, (short)event->x,
                          (short)event->y, LoadCursor(NULL, IDC_SIZEWE));
          } else {
            ...
            if (IsMouseOverSelection((short)event->x, (short)event->y)) {
              if (IsMouseOverSelectionStartEdge(event)) {
                StartDragging(DRAGTYPE_MOVESELECTIONSTART, (short)event->x,
                              (short)event->y, LoadCursor(NULL, IDC_SIZEWE));
              } else if (IsMouseOverSelectionEndEdge(event)) {
                StartDragging(DRAGTYPE_MOVESELECTIONEND, (short)event->x,
                              (short)event->y, LoadCursor(NULL, IDC_SIZEWE));
              } else {
                if (shiftdown) {
                  StartDragging(DRAGTYPE_MOVESELECTION, (short)event->x,
                                (short)event->y, LoadCursor(NULL, IDC_SIZEALL));
                }
              }
            } else {
      ...
      if (IsMouseOverTag((short)event->x, (short)event->y)) {
        ...
        if (pt) {
          ...
          if (shiftdown) {
            pt->m_bSelected = true;
            StartDragging(DRAGTYPE_MOVEPHONEME, (short)event->x,
                          (short)event->y, LoadCursor(NULL, IDC_SIZEALL));
          } else {
...
void PhonemeEditor::DrawWords(CChoreoWidgetDrawHelper &drawHelper,
                              RECT &rcWorkSpace, CSentence &sentence, int type,
  ...
  for (int pass = 0; pass < 2; pass++) {
    ...
    for (int k = 0; k < sentence.m_Words.Size(); k++) {
      ...
      for (int p = 0; p < word->m_Phonemes.Size() && !hasselectedphonemes;
           p++) {
      ...
      drawHelper.DrawFilledRect(PEColor(word->m_bSelected
                                            ? COLOR_PHONEME_TAG_SELECTED
                                            : COLOR_PHONEME_TAG_FILLER_NORMAL),
                                rcWord);

      COLORREF border =
          PEColor(word->m_bSelected ? COLOR_PHONEME_TAG_BORDER_SELECTED
                                    : COLOR_PHONEME_TAG_BORDER);

      if (showactive && m_bWordsActive) {
        drawHelper.DrawFilledRect(PEColor(COLOR_PHONEME_ACTIVE_BORDER), xpos,
                                  ypos - m_nTickHeight, xpos2,
                                  ypos - m_nTickHeight + 4);
      }

      drawHelper.DrawColoredLine(border, PS_SOLID, 1, xpos, ypos, xpos2, ypos);
      drawHelper.DrawColoredLine(border, PS_SOLID, 1, xpos, ypos, xpos,
                                 ypos - m_nTickHeight);
      drawHelper.DrawColoredLine(border, PS_SOLID, 1, xpos2, ypos, xpos2,
                                 ypos - m_nTickHeight);
      drawHelper.DrawColoredLine(border, PS_SOLID, 1, xpos,
                                 ypos - m_nTickHeight, xpos2,
                                 ypos - m_nTickHeight);

      if (hasselectedphonemes) {
        drawHelper.DrawFilledRect(PEColor(COLOR_PHONEME_SELECTED_BORDER), xpos,
                                  ypos - 3, xpos2, ypos);
      }
      ...
      // if ( frac >= 0.0 && frac <= 1.0 )
      {
        ...
        int length = drawHelper.CalcTextWidth(fontName, fontsize, FW_NORMAL,
                                              "%s", word->GetWord());
        ...
        drawHelper.DrawColoredText(fontName, fontsize, FW_NORMAL,
                                   PEColor(word->m_bSelected
                                               ? COLOR_PHONEME_TAG_TEXT_SELECTED
                                               : COLOR_PHONEME_TAG_TEXT),
                                   rcText, "%s", word->GetWord());
      }
    }
  }
}

void PhonemeEditor::DrawPhonemes(CChoreoWidgetDrawHelper &drawHelper,
                                 RECT &rcWorkSpace, CSentence &sentence,
                                 int type, bool showactive /* = true */) {
  float starttime = m_nLeftOffset / GetPixelsPerSecond();
  float endtime = w2() / GetPixelsPerSecond() + starttime;
    ...
    for (int i = 0; i < sentence.m_Words.Size(); i++) {
      ...
      for (int k = 0; k < w->m_Phonemes.Size(); k++) {
        ...
        drawHelper.DrawFilledRect(
            PEColor(pPhoneme->m_bSelected ? COLOR_PHONEME_TAG_SELECTED
                                          : COLOR_PHONEME_TAG_FILLER_NORMAL),
            rcFrame);

        COLORREF border =
            PEColor(pPhoneme->m_bSelected ? COLOR_PHONEME_TAG_BORDER_SELECTED
                                          : COLOR_PHONEME_TAG_BORDER);
        ...
        drawHelper.DrawColoredLine(border, PS_SOLID, 1, xpos2, ypos, xpos2,
                                   ypos - m_nTickHeight);
        drawHelper.DrawColoredLine(border, PS_SOLID, 1, xpos, ypos, xpos2,
                                   ypos);

        if (w->m_bSelected) {
          drawHelper.DrawFilledRect(PEColor(COLOR_PHONEME_SELECTED_BORDER),
                                    xpos, ypos - m_nTickHeight + 1, xpos2,
                                    ypos - m_nTickHeight + 4);
        }
        ...
        // if ( frac >= 0.0 && frac <= 1.0 )
        {
          ...
          int length = drawHelper.CalcTextWidth(
              fontName, fontsize, FW_NORMAL, "%s",
          ...
          drawHelper.DrawColoredText(
              ...
              PEColor(pPhoneme->m_bSelected ? COLOR_PHONEME_TAG_TEXT_SELECTED
                                            : COLOR_PHONEME_TAG_TEXT),
...
//			rc -
//-----------------------------------------------------------------------------
void PhonemeEditor::DrawRelativeTags(CChoreoWidgetDrawHelper &drawHelper,
                                     RECT &rc) {
  if (!m_pEvent || !m_pWaveFile)
    return;

  drawHelper.DrawColoredText("Arial", 9, FW_NORMAL,
                             PEColor(COLOR_PHONEME_TIMING_TAG), rc,
                             "Timing Tags:");
  ...
  for (int i = 0; i < m_pEvent->GetNumRelativeTags(); i++) {
    ...
    drawHelper.DrawColoredText("Arial", 9, FW_NORMAL,
                               PEColor(COLOR_PHONEME_TIMING_TAG), rcText,
                               tag->GetName());
  }
}
...
void PhonemeEditor::redraw(void) {
  ...
  drawHelper.DrawColoredLine(PEColor(COLOR_PHONEME_TIMELINE), PS_SOLID, 1,
                             rc.left, rc.bottom - m_nTickHeight, rc.right,
  ...
  sound->RenderWavToDC(dc, rc, PEColor(COLOR_PHONEME_WAVDATA), starttime,
                       endtime, m_pWaveFile, m_bSelectionActive,
  ...
  while (f <= endtime) {
    ...
    if (frac >= 0.0f && frac <= 1.0f) {
      drawHelper.DrawColoredLine((COLOR_PHONEME_TIMELINE_MAJORTICK), PS_SOLID,
                                 1, (int)(frac * rc.right), rc.top,
                                 (int)(frac * rc.right),
                                 rc.bottom - m_nTickHeight);
      ...
      drawHelper.DrawColoredText("Arial", 9, FW_NORMAL,
                                 PEColor(COLOR_PHONEME_TEXT), rcLabel, sz);
    }
    f += granularity;
  }
  ...
  if (m_nLastExtractionResult != SR_RESULT_NORESULT) {
    ...
    drawHelper.DrawColoredText(
        font, fontsize, fontweight, clr, rcText, "Last Extraction Result:  %s",
  ...
  if (m_pEvent && !Q_stristr(m_pEvent->GetParameters(), ".wav")) {
    drawHelper.DrawColoredText(
        font, fontsize, fontweight, PEColor(COLOR_PHONEME_TEXT), rcText,
  ...
  if (m_pEvent) {
    ...
    drawHelper.DrawColoredText(font, fontsize, fontweight,
                               PEColor(COLOR_PHONEME_TEXT), rcText, "Event %s",
                               m_pEvent->GetName());
  }
  ...
  drawHelper.DrawColoredText(font, fontsize, fontweight,
                             PEColor(COLOR_PHONEME_TEXT), rcText, "Using:  %s",
                             GetSpeechAPIName());
  ...
  drawHelper.DrawColoredTextMultiline(
      font, fontsize, fontweight, PEColor(COLOR_PHONEME_TEXT), rcText, text);

  CWordTag *cw = GetSelectedWord();
  if (cw) {
    char wordInfo[512];
    sprintf(wordInfo,
            "Word:  %s, start %.2f end %.2f, duration %.2f ms phonemes %i",
            cw->GetWord(), cw->m_flStartTime, cw->m_flEndTime,
            1000.0f * (cw->m_flEndTime - cw->m_flStartTime),
            cw->m_Phonemes.Size());
    ...
    drawHelper.DrawColoredText(font, fontsize, fontweight,
                               PEColor(COLOR_PHONEME_TEXT), rcText, wordInfo);
  }
  ...
  if (cp) {
    ...
    sprintf(phonemeInfo, "Phoneme:  %s, start %.2f end %.2f, duration %.2f ms",
            ConvertPhoneme(cp->GetPhonemeCode()), cp->GetStartTime(),
            cp->GetEndTime(),
            1000.0f * (cp->GetEndTime() - cp->GetStartTime()));

    int length =
        drawHelper.CalcTextWidth(font, fontsize, fontweight, phonemeInfo);
    ...
    drawHelper.DrawColoredText(font, fontsize, fontweight,
                               PEColor(COLOR_PHONEME_TEXT), rcText,
                               phonemeInfo);
  }
  ...
  if (m_UndoStack.Size() > 0) {
    ...
    drawHelper.DrawColoredText(font, fontsize, fontweight,
                               PEColor(COLOR_PHONEME_EXTRACTION_RESULT_SUCCESS),
                               rcText, "Undo levels:  %i/%i", m_nUndoLevel,
                               m_UndoStack.Size());
  }

  float endfrac =
      (m_pWaveFile->GetRunningLength() - starttime) / (endtime - starttime);
  if (endfrac >= 0.0f && endfrac <= 1.0f) {
    int endpos = (int)(rc.right * endfrac);

    drawHelper.DrawColoredLine(PEColor(COLOR_PHONEME_WAV_ENDPOINT), PS_DOT, 2,
                               endpos, rc.top, endpos,
                               rc.bottom - m_nTickHeight);
  }
...
//-----------------------------------------------------------------------------
// Purpose: Brown noise simulates brownian motion centered around 127.5 but we
// cap the walking
...
//-----------------------------------------------------------------------------
// Purpose: Replace with brownian noice parts of the wav file that we dont' want
// processed by the
//  speech recognizer
// Input  : store -
//			*format -
...
//			buffersize -
//-----------------------------------------------------------------------------
void PhonemeEditor::ResampleChunk(IterateOutputRIFF &store, void *format,
                                  int chunkname, char *buffer, int buffersize,
                                  int start_silence /*=0*/,
                                  int end_silence /*=0*/) {
  ...
  if (pFormat->wFormatTag == WAVE_FORMAT_PCM) {
    ...
    if (startpos + buffersize < resamplesize) {
      WriteBrownNoise(&resamplebuffer[startpos + buffersize],
                      resamplesize - (startpos + buffersize));
    }
...
//			force -
//-----------------------------------------------------------------------------
void PhonemeEditor::SetCurrentWaveFile(const char *wavefile,
                                       bool force /*=false*/,
                                       CChoreoEvent *event /*=NULL*/) {
  // No change?
  ...
  char fullpath[512];
  filesystem->RelativePathToFullPath(wavefile, "GAME", fullpath,
                                     sizeof(fullpath));
  ...
  Q_StripExtension(wavefile, m_WorkFile.m_szWorkingFile,
                   sizeof(m_WorkFile.m_szWorkingFile));
  Q_strncat(m_WorkFile.m_szWorkingFile, "_work.wav",
            sizeof(m_WorkFile.m_szWorkingFile), COPY_ALL_CHARACTERS);
...
void PhonemeEditor::ShowWordMenu(CWordTag *word, int mx, int my) {
  ...
  if (m_nSelectedWordCount > 0 && word) {
    ...
    pop->add(va("Delete %s", m_nSelectedWordCount > 1
                                 ? "words"
                                 : va("'%s'", word->GetWord())),
             IDC_EDIT_DELETEWORD);
      ...
      if (valid) {
        ...
        if (nextGap > MINIMUM_WORD_GAP || prevGap > MINIMUM_WORD_GAP) {
          ...
          if (prevGap > MINIMUM_WORD_GAP) {
            pop->add(va("Insert word before '%s'...", word->GetWord()),
                     IDC_EDIT_INSERTWORDBEFORE);
          }
          if (nextGap > MINIMUM_WORD_GAP) {
            pop->add(va("Insert word after '%s'...", word->GetWord()),
                     IDC_EDIT_INSERTWORDAFTER);
          }
        }

        if (word->m_Phonemes.Size() == 0) {
          pop->addSeparator();
          pop->add(va("Add phoneme to '%s'...", word->GetWord()),
                   IDC_EDIT_INSERTFIRSTPHONEMEOFWORD);
        }

        pop->addSeparator();
        pop->add(va("Select all words after '%s'", word->GetWord()),
                 IDC_SELECT_WORDSRIGHT);
        pop->add(va("Select all words before '%s'", word->GetWord()),
                 IDC_SELECT_WORDSLEFT);
      }
    }
  }
...
void PhonemeEditor::ShowPhonemeMenu(CPhonemeTag *pho, int mx, int my) {
  ...
  if (valid) {
    if (m_nSelectedPhonemeCount == 1) {
      pop->add(va("Edit '%s'...", ConvertPhoneme(pho->GetPhonemeCode())),
               IDC_EDIT_PHONEME);
      ...
      if (nextGap > MINIMUM_PHONEME_GAP || prevGap > MINIMUM_PHONEME_GAP) {
        ...
        if (prevGap > MINIMUM_PHONEME_GAP) {
          pop->add(va("Insert phoneme before '%s'...",
                      ConvertPhoneme(pho->GetPhonemeCode())),
                   IDC_EDIT_INSERTPHONEMEBEFORE);
        }
        if (nextGap > MINIMUM_PHONEME_GAP) {
          pop->add(va("Insert phoneme after '%s'...",
                      ConvertPhoneme(pho->GetPhonemeCode())),
      ...
      pop->addSeparator();
      pop->add(va("Select all phonemes after '%s'",
                  ConvertPhoneme(pho->GetPhonemeCode())),
               IDC_SELECT_PHONEMESRIGHT);
      pop->add(va("Select all phonemes before '%s'",
                  ConvertPhoneme(pho->GetPhonemeCode())),
    ...
    if (m_nSelectedPhonemeCount >= 1) {
      pop->add(va("Delete %s",
                  m_nSelectedPhonemeCount == 1
                      ? va("'%s'", ConvertPhoneme(pho->GetPhonemeCode()))
                      : "phonemes"),
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool PhonemeEditor::FindSpanningPhonemes(float time, CPhonemeTag **pp1,
                                         CPhonemeTag **pp2) {
  Assert(pp1 && pp2);
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool PhonemeEditor::FindSpanningWords(float time, CWordTag **pp1,
                                      CWordTag **pp2) {
  Assert(pp1 && pp2);
...
void PhonemeEditor::FinishWordMove(int startx, int endx) {
  ...
  // Find the phonemes who have the closest start/endtime to the starting click
  // time
  ...
  if (current && !next) {
    ...
    endtime =
        max(endtime, current->m_flStartTime + 1.0f / GetPixelsPerSecond());
...
void PhonemeEditor::FinishPhonemeMove(int startx, int endx) {
  ...
  if (current && !next) {
    ...
    endtime =
        max(endtime, current->GetStartTime() + 1.0f / GetPixelsPerSecond());
...
void PhonemeEditor::EditInsertPhonemeBefore(void) {
  ...
  if (!word) {
    Con_Printf("EditInsertPhonemeBefore:  phoneme not a member of any known "
               "word!!!\n");
    return;
  }
  ...
  if (clicked < 0) {
    Con_Printf("EditInsertPhonemeBefore:  phoneme not a member of any "
               "specified word!!!\n");
    Assert(0);
    return;
  }
...
void PhonemeEditor::EditInsertPhonemeAfter(void) {
  ...
  if (!word) {
    Con_Printf(
        "EditInsertPhonemeAfter:  phoneme not a member of any known word!!!\n");
    return;
  }

  int clicked = word->IndexOfPhoneme(cp);
  if (clicked < 0) {
    Con_Printf("EditInsertPhonemeAfter:  phoneme not a member of any specified "
               "word!!!\n");
    Assert(0);
    return;
  }
...
bool PhonemeEditor::CreateCroppedWave(char const *filename, int startsample,
                                      int endsample) {
  Assert(sound);
  ...
  while (1) {
    ...
    if (!pMixer->MixDataToDevice(&pWaveOutput->m_audioDevice, &channel,
...
void PhonemeEditor::RedoPhonemeExtractionSelected(void) {
  ...
  if (!m_bSelectionActive) {
    Con_Printf("Please select a portion of the .wav from which to re-extract "
               "phonemes\n");
    return;
  }
  ...
  float numsamples =
      m_pWaveFile->GetRunningLength() * m_pWaveFile->SampleRate();
  float selectionstarttime = 0.0f;
  if (numsamples > 0.0f) {
    // Convert sample #'s to time
    selectionstarttime =
        (m_nSelection[0] / numsamples) * m_pWaveFile->GetRunningLength();
  ...
  if (m_nSelectedWordCount == 0) {
    ...
    if (m_InputWords.m_Words.Size() == 0) {
      Con_Printf("You must either select words, or type in a set of words in "
                 "order to extract phonemes!\n");
      return;
    }
  ...
  Q_snprintf(szCroppedFile, sizeof(szCroppedFile), "%s%s_work1.wav",
             m_WorkFile.m_szBasePath, szBaseFile);
  ...
  if (!CreateCroppedWave(szCroppedFile, m_nSelection[0], m_nSelection[1])) {
    Con_Printf("Unable to create cropped wave file %s from samples %i to %i\n",
               szCroppedFile, m_nSelection[0], m_nSelection[1]);
    return;
  }
  ...
  char filename[512];
  Q_snprintf(filename, sizeof(filename), "%s%s", m_WorkFile.m_szBasePath,
             szCroppedFile);

  m_nLastExtractionResult = m_pPhonemeExtractor->Extract(
      filename,
      (int)(m_pCroppedWave->GetRunningLength() * m_pCroppedWave->SampleRate() *
            m_pCroppedWave->TrueSampleSize()),
      Con_Printf, m_InputWords, m_Results);

  if (m_InputWords.m_Words.Size() != m_Results.m_Words.Size()) {
    Con_Printf("Extraction returned %i words, source had %i, try adjusting "
               "selection\n",
  ...
  float bytespersecond =
      m_pCroppedWave->SampleRate() * m_pCroppedWave->TrueSampleSize();

  // Tracker 57389:
  // Total hack to fix a bug where the Lipsinc extractor is messing up the #
  // channels on 16 bit stereo waves
  if (m_pPhonemeExtractor->GetAPIType() == SPEECH_API_LIPSINC &&
      m_pCroppedWave->IsStereoWav() && m_pCroppedWave->SampleSize() == 16) {
    bytespersecond *= 2.0f;
  }
  ...
  if (usingselection) {
    ...
    for (; startWord < m_TagsExt.m_Words.Size(); startWord++) {
      ...
      if (w->m_flStartTime >
          firstWordOfPhrase->m_flStartTime + selectionstarttime)
        break;
    }
...
void PhonemeEditor::ITER_SelectSpanningWords(CWordTag *word, float amount) {
  ...
  if (numsamples > 0.0f) {
    ...
    float starttime =
        (m_nSelection[0] / numsamples) * m_pWaveFile->GetRunningLength();
    float endtime =
        (m_nSelection[1] / numsamples) * m_pWaveFile->GetRunningLength();
...
int PhonemeEditor::GetSampleForMouse(int mx) {
  ...
  int sampleNumber = (int)((float)totalsamples * clickTime /
                           (float)m_pWaveFile->GetRunningLength());
...
void PhonemeEditor::OnImport() {
  char filename[512];
  if (!FacePoser_ShowOpenFileNameDialog(filename, sizeof(filename), "sound",
                                        "*" WORD_DATA_EXTENSION)) {
    return;
  }
...
void PhonemeEditor::OnExport() {
  ...
  char filename[512];
  if (!FacePoser_ShowSaveFileNameDialog(filename, sizeof(filename), "sound",
                                        "*" WORD_DATA_EXTENSION)) {
    return;
  }
...
void PhonemeEditor::ExportValveDataChunk(char const *tempfile) {
  if (m_Tags.m_Words.Count() <= 0) {
    Con_ErrorPrintf(
        "PhonemeEditor::ExportValveDataChunk:  Sentence has no word data\n");
    return;
  }

  FileHandle_t fh = filesystem->Open(tempfile, "wb");
  if (!fh) {
    Con_ErrorPrintf("PhonemeEditor::ExportValveDataChunk:  Unable to write to "
                    "%s (read-only?)\n",
                    tempfile);
    return;
  } else {
...
void PhonemeEditor::ImportValveDataChunk(char const *tempfile) {
  ...
  if (!fh) {
    Con_ErrorPrintf(
        "PhonemeEditor::ImportValveDataChunk:  Unable to read from %s\n",
        tempfile);
    return;
  }
  ...
  if (len <= 4) {
    Con_ErrorPrintf(
        "PhonemeEditor::ImportValveDataChunk:  File %s has length 0\n",
        tempfile);
    return;
  }
...
void PhonemeEditor::SaveLinguisticData(void) {
  ...
  char fullout[512];
  Q_snprintf(fullout, sizeof(fullout), "%s%s", m_WorkFile.m_szBasePath,
             m_WorkFile.m_szWorkingFile);
  ...
  while (walk.ChunkAvailable()) {
    ...
    case WAVE_DATA: {
      ...
      walk.ChunkRead(buffer);
      // Resample it
      ResampleChunk(store, (void *)&format, walk.ChunkName(), buffer,
                    walk.ChunkSize());
...
void PhonemeEditor::CommitChanges(void) {
  ...
  // Open a message box to warn the user if the file was unable to be made
  // non-read only
  if (!IsFileWriteable(m_WorkFile.m_szWaveFile)) {
    mxMessageBox(NULL,
                 va("Unable to save file '%s'. File is read-only or in use.",
                    m_WorkFile.m_szWaveFile),
                 g_appTitle, MX_MB_OK);
  } else {
    // Copy over and overwrite file
  ...
  for (int w = 0; w < m_Tags.m_Words.Size(); w++) {
    ...
    for (int i = 0; i < word->m_Phonemes.Size(); i++) {
      ...
      if (current->m_bSelected) {
        if (prev) {
          // Close enough?
          if (fabs(prev->GetEndTime() - current->GetStartTime()) >
              time_epsilon) {
...
CEventRelativeTag *PhonemeEditor::GetTagUnderMouse(int mx) {
  ...
  for (int i = 0; i < m_pEvent->GetNumRelativeTags(); i++) {
    ...
    int left =
        rcTags.left + (int)(frac * (float)(rcTags.right - rcTags.left) + 0.5f);
...
void PhonemeEditor::ITER_MoveSelectedPhonemes(CPhonemeTag *phoneme,
                                              CWordTag *word, float amount) {
  if (!phoneme->m_bSelected)
    return;
...
void PhonemeEditor::ITER_ExtendSelectedPhonemeEndTimes(CPhonemeTag *phoneme,
                                                       CWordTag *word,
                                                       float amount) {
  if (!phoneme->m_bSelected)
    return;
...
void PhonemeEditor::ITER_ExtendSelectedWordEndTimes(CWordTag *word,
                                                    float amount) {
  if (!word->m_bSelected)
    return;
...
void PhonemeEditor::ITER_AddFocusRectSelectedWords(CWordTag *word,
                                                   float amount) {
  if (!word->m_bSelected)
    return;
...
void PhonemeEditor::ITER_AddFocusRectSelectedPhonemes(CPhonemeTag *phoneme,
                                                      CWordTag *word,
                                                      float amount) {
  if (!phoneme->m_bSelected)
    return;
...
void PhonemeEditor::ITER_CountSelectedPhonemes(CPhonemeTag *phoneme,
                                               CWordTag *word, float amount) {
  if (!phoneme->m_bSelected)
    return;
...
void PhonemeEditor::ShowContextMenu_Phonemes(int mx, int my) {
  ...
  if (m_pWaveFile) {
    ...
    if (m_nSelectedWordCount < 1 || AreSelectedWordsContiguous()) {
      pop->add(va("Redo Extraction of selected words"),
               IDC_REDO_PHONEMEEXTRACTION_SELECTION);
    }
  }
  ...
  if (m_pWaveFile) {
    pop->addSeparator();
    if (m_Tags.m_Words.Count() > 0) {
      pop->add("Export word data to " WORD_DATA_EXTENSION "...",
               IDC_EXPORT_SENTENCE);
    }
    pop->add("Import word data from " WORD_DATA_EXTENSION "...",
             IDC_IMPORT_SENTENCE);
    pop->add(va("%s Voice Duck", m_Tags.GetVoiceDuck() ? "Disable" : "Enable"),
             IDC_TOGGLE_VOICEDUCK);
  }
...
void PhonemeEditor::ExtendSelectedPhonemeEndTime(int direction) {
  ...
  TraversePhonemes(&PhonemeEditor::ITER_ExtendSelectedPhonemeEndTimes,
                   movetime);
  ...
  if (m_nSelectedPhonemeCount != 1) {
    if (m_nSelectedWordCount == 1) {
      ...
      if (word && word->m_Phonemes.Size() > 0) {
        m_nSelectedPhonemeCount = 1;
        CPhonemeTag *p =
            word->m_Phonemes[direction ? word->m_Phonemes.Size() - 1 : 0];
        p->m_bSelected = true;
      } else {
        return;
      }
...
void PhonemeEditor::SelectNextWord(int direction) {
  ...
  if (m_nSelectedWordCount != 1 && m_nSelectedPhonemeCount != 1) {
    ...
    if (m_nSelectedWordCount == 0 && m_Tags.m_Words.Size() > 0) {
      CWordTag *word =
          m_Tags.m_Words[direction ? m_Tags.m_Words.Size() - 1 : 0];
      word->m_bSelected = true;
...
// Output : float
//-----------------------------------------------------------------------------
float PhonemeEditor::GetTimeGapToNextWord(bool forward, CWordTag *currentWord,
                                          CWordTag **ppNextWord /* = NULL */) {
  if (ppNextWord) {
    *ppNextWord = NULL;
  }
...
// Output : float
//-----------------------------------------------------------------------------
float PhonemeEditor::GetTimeGapToNextPhoneme(
    bool forward, CPhonemeTag *currentPhoneme, CWordTag **ppword /* = NULL */,
    ...
    while (wordnum >= 0) {
      ...
      if (m_Tags.m_Words.Size() > 0) {
        if (m_Tags.m_Words[wordnum]->m_Phonemes.Size() > 0) {
          nextPhoneme =
              m_Tags.m_Words[wordnum]
                  ->m_Phonemes[m_Tags.m_Words[wordnum]->m_Phonemes.Size() - 1];
          break;
        }
      }
      wordnum--;
  ...
  if (m_nDragType != DRAGTYPE_NONE) {
    ...
    for (int i = 0; i < m_FocusRects.Size(); i++) {
      ...
      case DRAGTYPE_EMPHASIS_SELECT: {
        ...
        rcFocus.left =
            m_nStartX < (short)event->x ? m_nStartX : (short)event->x;
        rcFocus.right =
            m_nStartX < (short)event->x ? (short)event->x : m_nStartX;

        rcFocus.top = m_nStartY < (short)event->y ? m_nStartY : (short)event->y;
        rcFocus.bottom =
            m_nStartY < (short)event->y ? (short)event->y : m_nStartY;

        rcFocus.top = clamp(rcFocus.top, rcEmphasis.top, rcEmphasis.bottom);
        rcFocus.bottom =
            clamp(rcFocus.bottom, rcEmphasis.top, rcEmphasis.bottom);
    ...
    if (overhandle == BOUNDARY_PHONEME && m_nSelectedPhonemeCount <= 1) {
      ...
      if (IsMouseOverTag((short)event->x, (short)event->y)) {
        ...
        CPhonemeTag *pt =
            GetPhonemeTagUnderMouse((short)event->x, (short)event->y);
...
void PhonemeEditor::RealignPhonemesToWords(bool prepareundo) {
  ...
  for (int i = 0; i < m_Tags.m_Words.Size(); i++) {
    ...
    for (int j = 0; j < word->m_Phonemes.Size(); j++) {
      ...
      if (j == 0) {
        float t =
            (phoneme->GetStartTime() - phoneme_shift) * (word_dt / phoneme_dt) +
            word->m_flStartTime;
        phoneme->SetStartTime(t);
      }

      float t =
          (phoneme->GetEndTime() - phoneme_shift) * (word_dt / phoneme_dt) +
          word->m_flStartTime;
      phoneme->SetEndTime(t);
      if (next) {
        next->SetStartTime(t);
...
float PhonemeEditor::ComputeMaxWordShift(bool forward, bool allowcrop) {
  // skipping selected words, figure out max time shift of words before they
  // selection touches any unselected words if allowcrop is true, then the
  // maximum extends up to end of next word
...
float PhonemeEditor::ComputeMaxPhonemeShift(bool forward, bool allowcrop) {
  // skipping selected phonemes, figure out max time shift of phonemes before
  // they selection touches any unselected words if allowcrop is true, then the
  // maximum extends up to end of next word
  float maxshift = PLENTY_OF_TIME;
...
//			rcEmphasis -
//-----------------------------------------------------------------------------
void PhonemeEditor::Emphasis_GetRect(RECT const &rcWorkSpace,
                                     RECT &rcEmphasis) {
...
void PhonemeEditor::Emphasis_Redraw(CChoreoWidgetDrawHelper &drawHelper,
                                    RECT &rcWorkSpace) {
  ...
  if (fullmode) {
    drawHelper.DrawColoredText("Arial", 15, FW_BOLD,
                               PEColor(COLOR_PHONEME_EMPHASIS_TEXT), rcText,
                               "Emphasis...");
  }

  {
    ...
    drawHelper.DrawGradientFilledRect(rcSpot,
    ...
    drawHelper.DrawGradientFilledRect(
        rcSpot, PEColor(COLOR_PHONEME_EMPHASIS_BG),
        PEColor(COLOR_PHONEME_EMPHASIS_BG_WEAK), true);
  }

  COLORREF gray = PEColor(COLOR_PHONEME_EMPHASIS_MIDLINE);

  drawHelper.DrawOutlinedRect(PEColor(COLOR_PHONEME_EMPHASIS_BORDER), PS_SOLID,
                              1, rcClient);
  ...
  for (int x = 0; x < (w2() + dx); x += dx) {
    ...
    // Draw segment
    drawHelper.DrawColoredLine(lineColor, PS_SOLID, 1, prevx,
                               bottom - prev_value * height, x,
                               bottom - value * height);
...
void PhonemeEditor::GetScrubHandleRect(RECT &rcHandle, bool clipped) {
  ...
  if (m_pWaveFile) {
    ...
    if (clipped) {
      pixel = clamp(pixel, SCRUBBER_HANDLE_WIDTH / 2,
...
void PhonemeEditor::DrawScrubHandle(CChoreoWidgetDrawHelper &drawHelper) {
  ...
  drawHelper.DrawColoredLine(areaBorder, PS_SOLID, 1, 0, rcHandle.top, w2(),
                             rcHandle.top);
  drawHelper.DrawColoredLine(areaBorder, PS_SOLID, 1, 0, rcHandle.bottom, w2(),
                             rcHandle.bottom);
