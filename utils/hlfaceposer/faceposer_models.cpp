...
IFaceposerModels::CFacePoserModel::CFacePoserModel(char const *modelfile,
                                                   StudioModel *model) {
  m_pModel = model;
  m_szActorName[0] = 0;
  m_szShortName[0] = 0;
...
IFaceposerModels::CFacePoserModel::~CFacePoserModel() { FreeBitmaps(); }
...
CRC32_t IFaceposerModels::CFacePoserModel::GetBitmapCRC(int sequence) {
  ...
  CRC32_ProcessBuffer(&crc, (void *)seqdesc.pszLabel(),
                      Q_strlen(seqdesc.pszLabel()));
  CRC32_ProcessBuffer(&crc, (void *)seqdesc.pszActivityName(),
                      Q_strlen(seqdesc.pszActivityName()));
  CRC32_ProcessBuffer(&crc, (void *)&seqdesc.flags, sizeof(seqdesc.flags));
  // CRC32_ProcessBuffer( &crc, (void *)&seqdesc.numevents, sizeof(
  // seqdesc.numevents ) );
  CRC32_ProcessBuffer(&crc, (void *)&seqdesc.numblends,
                      sizeof(seqdesc.numblends));
  CRC32_ProcessBuffer(&crc, (void *)seqdesc.groupsize,
                      sizeof(seqdesc.groupsize));

  KeyValues *seqKeyValues = new KeyValues("");
  if (seqKeyValues->LoadFromBuffer(m_pModel->GetFileName(),
                                   m_pModel->GetKeyValueText(sequence))) {
...
const char *IFaceposerModels::CFacePoserModel::GetBitmapFilename(int sequence) {
  ...
  *out = 0;
  ...
  Q_snprintf(filename, sizeof(filename), "expressions/%s/animation/%s.bmp",
             modelName, GetBitmapChecksum(sequence));
...
void IFaceposerModels::CFacePoserModel::LoadBitmapForSequence(
    mxbitmapdata_t *bitmap, int sequence) {
  // See if it exists
  char filename[512];
...
void IFaceposerModels::CreateNewBitmap(int modelindex,
                                       char const *pchBitmapFilename,
                                       int sequence, int nSnapShotSize,
                                       bool bZoomInOnFace,
                                       CExpression *pExpression,
                                       mxbitmapdata_t *bitmap) {
  CFacePoserModel *m = m_Models[modelindex];
  if (m) {
    m->CreateNewBitmap(pchBitmapFilename, sequence, nSnapShotSize,
                       bZoomInOnFace, pExpression, bitmap);
  }
}

void IFaceposerModels::CFacePoserModel::CreateNewBitmap(
    char const *pchBitmapFilename, int sequence, int nSnapShotSize,
  ...
  if (bZoomInOnFace) {
    ...
    if (hdr->GetNumAttachments() > 0) {
      for (int i = 0; i < hdr->GetNumAttachments(); i++) {
        ...
        VectorITransform(vec3_origin, attachmentPoseToLocal,
                         localSpaceEyePosition);
  ...
  char fullpath[512];
  Q_snprintf(fullpath, sizeof(fullpath), "%s%s", GetGameDirectory(),
             pchBitmapFilename);
...
mxbitmapdata_t *
IFaceposerModels::CFacePoserModel::GetBitmapForSequence(int sequence) {
...
void IFaceposerModels::CFacePoserModel::BuildValidChecksums(
    CUtlRBTree<CRC32_t> &tree) {
  StudioModel *model = m_pModel;
  if (!model)
    return;
...
void IFaceposerModels::CFacePoserModel::ReconcileAnimationBitmaps() {
  // iterate files in directory and see if each checksum is valid and if not
  // delete the .bmp
  char path[512];
  Q_snprintf(path, sizeof(path), "expressions/%s/animation/*.bmp",
             GetShortModelName());
  ...
  g_pProgressDialog->Start(
      CFmtStr("%s - Reconcile Animation Thumbnails", GetShortModelName()), "",
      true);
  ...
  for (int i = 0; i < workList.Count(); ++i) {
    ...
    if (tree.Find(check) == tree.InvalidIndex()) {
      Q_snprintf(testname, sizeof(testname), "expressions/%s/animation/%s",
                 GetShortModelName(), fn);

      char fullpath[512];
      filesystem->RelativePathToFullPath(testname, "MOD", fullpath,
                                         sizeof(fullpath));
      // Delete it
...
void IFaceposerModels::CFacePoserModel::RecreateAllAnimationBitmaps() {
  ...
  g_pProgressDialog->Start(
      CFmtStr("%s - Animation Thumbnails", GetShortModelName()), "", true);
...
void IFaceposerModels::CFacePoserModel::RecreateAnimationBitmap(
    int sequence, bool reconcile) {
  if (sequence < 0 || sequence >= m_AnimationBitmaps.Count()) {
    Assert(0);
    return;
  }
...
int IFaceposerModels::Count(void) const { return m_Models.Count(); }
...
void IFaceposerModels::UnForceActiveModelIndex() { m_nForceModelIndex = -1; }
...
void IFaceposerModels::SetActorNameForModel(int modelindex,
...
void IFaceposerModels::SaveModelList(void) {
  ...
  for (int i = 0; i < c; i++) {
    ...
    workspacefiles->StoreFile(IWorkspaceFiles::MODELDATA,
                              m->GetModelFileName());
  }
...
void IFaceposerModels::LoadModelList(void) {
  ...
  for (int i = 0; i < files; i++) {
    char const *filename =
        workspacefiles->GetStoredFile(IWorkspaceFiles::MODELDATA, i);
    LoadModel(filename);
  }
}
...
mxbitmapdata_t *IFaceposerModels::GetBitmapForSequence(int modelindex,
                                                       int sequence) {
  static mxbitmapdata_t nullbitmap;
  CFacePoserModel *m = GetEntry(modelindex);
  if (!m)
