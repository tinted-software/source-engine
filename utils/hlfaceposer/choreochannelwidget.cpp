...
float CChoreoChannelWidget::GetTimeForMousePosition(int mx) {
  ...
  float time = m_pView->GetStartTime() +
               windowfrac * (m_pView->GetEndTime() - m_pView->GetStartTime());
  return time;
}

static bool EventStartTimeLessFunc(CChoreoEventWidget *const &p1,
                                   CChoreoEventWidget *const &p2) {
...
void CChoreoChannelWidget::LayoutEventInRow(CChoreoEventWidget *event, int row,
                                            RECT &rc) {
  int itemHeight = BaseClass::GetItemHeight();

  RECT rcEvent;
  rcEvent.left =
      m_pView->GetPixelForTimeValue(event->GetEvent()->GetStartTime());
  if (event->GetEvent()->HasEndTime()) {
    rcEvent.right =
        m_pView->GetPixelForTimeValue(event->GetEvent()->GetEndTime());
  } else {
...
static bool
EventCollidesWithRows(CUtlLinkedList<CChoreoEventWidget *, int> &list,
                      CChoreoEventWidget *event) {
  float st = event->GetEvent()->GetStartTime();
  float ed = event->GetEvent()->HasEndTime()
                 ? event->GetEvent()->GetEndTime()
  ...
  for (int i = list.Head(); i != list.InvalidIndex(); i = list.Next(i)) {
    ...
    float testend =
        test->HasEndTime() ? test->GetEndTime() : test->GetStartTime();
...
void CChoreoChannelWidget::Layout(RECT &rc) {
  ...
  for ( int i = 0; i < m_Events.Size(); i++ )
  {
          ...
          RECT rcEvent;
          rcEvent.left = m_pView->GetPixelForTimeValue(
  event->GetEvent()->GetStartTime() ); if ( event->GetEvent()->HasEndTime() )
          {
                  rcEvent.right = m_pView->GetPixelForTimeValue(
  event->GetEvent()->GetEndTime() );
          }
          else
          {
...
void CChoreoChannelWidget::redraw(CChoreoWidgetDrawHelper &drawHelper) {
  ...
  drawHelper.DrawColoredText("Arial", m_pView->GetFontSize() + 2, FW_HEAVY,
                             channel->GetActive()
                                 ? COLOR_CHOREO_CHANNELNAME
                                 : COLOR_CHOREO_ACTORNAME_INACTIVE,
                             rcName, n);
...
//			rcEventArea -
//-----------------------------------------------------------------------------
void CChoreoChannelWidget::RenderCloseCaptionInfo(
    CChoreoWidgetDrawHelper &drawHelper, RECT &rcEventArea) {
  wchar_t wstr[1024];
  ...
  // Walk the events looking for SPEAK events (esp if marked as MASTER with >= 1
  // slave)
  for (int j = GetNumEvents() - 1; j >= 0; j--) {
    CChoreoEventWidget *event = GetEvent(j);
    ...
    if (e->GetCloseCaptionType() == CChoreoEvent::CC_MASTER) {
      ...
      if (e->GetPlaybackCloseCaptionToken(cctoken, sizeof(cctoken))) {
        stateValid = closecaptionmanager->LookupUnicodeText(
            GetCloseCaptionLanguageId(), cctoken, wstr,
    ...
    int pixeloffset = (int)((bestEndTime - e->GetStartTime()) *
                                m_pView->GetPixelsPerSecond() +
                            0.5f);
    ...
    if (showState) {
      ...
      drawHelper.DrawColoredTextCharset("Marlett", m_pView->GetFontSize() - 2,
                                        500, SYMBOL_CHARSET, symColor, rcState,
                                        stateValid ? "a" : "r");
    }

    if (e->IsSuppressingCaptionAttenuation()) {
      drawHelper.DrawColoredText("Arial", m_pView->GetFontSize() - 2, 500,
                                 RGB(80, 80, 100), rcText, "%s [no attenuate]",
                                 label);
...
//			rcEventArea -
//-----------------------------------------------------------------------------
void CChoreoChannelWidget::RenderCloseCaptions(
    CChoreoWidgetDrawHelper &drawHelper, RECT &rcEventArea) {
  {
    RECT rcText = rcEventArea;
    ...
    drawHelper.DrawColoredText(
        "Arial", m_pView->GetFontSize() - 2, 500, COLOR_CHOREO_TEXT, rcText,
        "%s", CSentence::NameForLanguage(GetCloseCaptionLanguageId()));

    // Previous
    GetCloseCaptionLanguageRect(rcText, true);
    drawHelper.DrawColoredTextCharset("Marlett", m_pView->GetFontSize(), 500,
                                      SYMBOL_CHARSET, COLOR_CHOREO_TEXT, rcText,
                                      "3");
  ...
  for (int j = GetNumEvents() - 1; j >= 0; j--) {
    ...
    valid = closecaptionmanager->LookupStrippedUnicodeText(
        GetCloseCaptionLanguageId(), cctoken, wstr,
        sizeof(wstr) / sizeof(wchar_t));
...
void CChoreoChannelWidget::HandleSelectorClicked() {
  ...
  SetUsingCombinedFieldByTokenName(event->GetCloseCaptionToken(),
                                   !event->IsUsingCombinedFile());
}

void CChoreoChannelWidget::SetUsingCombinedFieldByTokenName(
    char const *token, bool usingcombinedfile) {
  int c = GetNumEvents();
  for (int i = 0; i < c; ++i) {
...
void CChoreoChannelWidget::GetCloseCaptionLanguageRect(RECT &rc,
                                                       bool previous) {
  Assert(m_bHasAudio);
  ...
  if (!previous) {
    int textlen = CChoreoWidgetDrawHelper::CalcTextWidth(
        "Arial", m_pView->GetFontSize() - 2, 500,
...
void CChoreoChannelWidget::RenderCloseCaptionSelectors(
    CChoreoWidgetDrawHelper &drawHelper, RECT &rcEventArea) {
  CUtlVector<CloseCaptionInfo> vecSelectors;
  GetCloseCaptions(vecSelectors);
  int c = vecSelectors.Count();
  if (vecSelectors.Count() > 0) {
    for (int i = 0; i < c; ++i) {
      ...
      COLORREF clr =
          RGB(63, 63, 63); // upArrow ? RGB( 255, 0, 0 ) : RGB( 0, 0, 255 );
      ...
      drawHelper.DrawColoredLine(clr, PS_SOLID, 1, startpt.x, startpt.y,
                                 endpt.x, endpt.y);
...
void CChoreoChannelWidget::GetCloseCaptions(
    CUtlVector<CloseCaptionInfo> &selectors) {
  selectors.RemoveAll();
  ...
  for (int j = GetNumEvents() - 1; j >= 0; j--) {
    ...
    bool isSelector =
        (e->GetNumSlaves() >= 1) && capType == CChoreoEvent::CC_MASTER;
...
void CChoreoChannelWidget::RenderCloseCaptionExpandCollapseRect(
    CChoreoWidgetDrawHelper &drawHelper, RECT &rcEventArea) {
  if (!m_bHasAudio)
    return;
  ...
  drawHelper.DrawColoredTextCharset(
      "Marlett", m_pView->GetFontSize(), 900, SYMBOL_CHARSET, symColor,
      rcCCArea, m_pView->GetShowCloseCaptionData() ? "6" : "4");
}

void CChoreoChannelWidget::GetMasterAndSlaves(
    CChoreoEvent *master, CUtlVector<CChoreoEvent *> &fulllist) {
  // Old
  int c = GetNumEvents();
  int i;
  for (i = 0; i < c; ++i) {
...
//			rcBounds -
//-----------------------------------------------------------------------------
void CChoreoChannelWidget::redrawStatus(CChoreoWidgetDrawHelper &drawHelper,
                                        RECT &rcClient, int areaUnderMouse) {
  if (!getVisible())
    return;
  ...
  if ( CChoreoEventWidget::GetImage( event->GetType() ) )
  {
  ...
  ); if ( image )
          {
                  ...
                  DrawBitmapToDC( dc, rcFixed.left, rcFixed.top, leftAdd,
  leftAdd, *image );
          }
  }
  ...
  int len = drawHelper.CalcTextWidth( "Arial", fontsize, FW_NORMAL, "%s event
  \"%s\"", event->NameForType( event->GetType() ), event->GetName() );
  drawHelper.DrawColoredText( "Arial", fontsize, FW_NORMAL, COLOR_INFO_TEXT,
  rcEventLabel, "%s event \"%s\"", event->NameForType( event->GetType() ),
  event->GetName() );
  ...
  */
  ...
  if (showState) {
    ...
    drawHelper.DrawColoredTextCharset("Marlett", fontsize + 2, 500,
                                      SYMBOL_CHARSET, symColor, rcState,
                                      stateValid ? "a" : "r");
  }

  drawHelper.DrawColoredText("Arial", fontsize, 500, labelColor, rcText,
                             "closecaption token:  %s", label);
  ...
  if (e->GetCloseCaptionType() != CChoreoEvent::CC_DISABLED) {
    if (e->GetNumSlaves() >= 1 ||
      ...
      e->ComputeCombinedBaseFileName(cf, sizeof(cf),
                                     e->IsCombinedUsingGenderToken());
      ...
      if (gendermacro) {
        bool valid[2];
        char actualfile[256];
        soundemitter->GenderExpandString(GENDER_MALE, cf, actualfile,
                                         sizeof(actualfile));
        valid[0] = filesystem->FileExists(actualfile);
        soundemitter->GenderExpandString(GENDER_FEMALE, cf, actualfile,
                                         sizeof(actualfile));
      ...
      drawHelper.DrawColoredText("Arial", fontsize, 500, labelColor, rcPartial,
                                 sz);
      ...
      drawHelper.DrawColoredText(
          "Arial", fontsize, 500,
          combinedValid ? RGB(20, 150, 20) : RGB(255, 0, 0), rcPartial, sz);

      rcPartial.left += len;

      Q_snprintf(sz, sizeof(sz), " ]:  %s, %s ", cf,
      ...
      drawHelper.DrawColoredText("Arial", fontsize, 500, statusClr, rcPartial,
                                 exist);
    }
