...
CExpressionToolWorkspace::CExpressionToolWorkspace(mxWindow *parent)
  ...
  m_pVertScrollBar = new mxScrollbar(
      this, 0, 0, 12, 100, IDC_EXPRESSIONTOOLVSCROLL, mxScrollbar::Vertical);
...
int SortFuncByName(const void *elem1, const void *elem2) {
  ...
  return stricmp(track1->GetFlexControllerName(),
                 track2->GetFlexControllerName());
}

void CExpressionToolWorkspace::OnSortByUsed(void) {
  qsort(m_pItems, GLOBAL_STUDIO_FLEX_CONTROL_COUNT, sizeof(TimelineItem *),
        SortFuncByUse);
  LayoutItems(false);
}
...
void CExpressionToolWorkspace::DrawEventEnd(
  ...
  drawHelper.DrawColoredLine(COLOR_CHOREO_ENDTIME, PS_SOLID, 1, leftx,
                             rcClient.top, leftx, rcClient.bottom);
}
        ...
        if (scene) {
          ...
          if (event->height > 0) {
            g_pChoreoView->SetTimeZoom(g_pExpressionTool->GetToolName(),
                                       min(tz + TIME_ZOOM_STEP, MAX_TIME_ZOOM),
                                       false);
          } else {
            g_pChoreoView->SetTimeZoom(g_pExpressionTool->GetToolName(),
                                       min(tz - TIME_ZOOM_STEP, MAX_TIME_ZOOM),
...
void CExpressionToolWorkspace::RepositionVSlider(void) {
  ...
  m_pVertScrollBar->setBounds(w2() - m_nScrollbarHeight, 0, m_nScrollbarHeight,
                              h2());
...
void CExpressionToolWorkspace::OnDeleteColumn() {
  ...
  Q_snprintf(params.m_szInputText, sizeof(params.m_szInputText), "%i",
             clickedframe);
...
void CExpressionToolWorkspace::MoveSelectedSamples(float dfdx, float dfdy,
                                                   bool snap) {
  ...
  for (int controller = 0; controller < GLOBAL_STUDIO_FLEX_CONTROL_COUNT;
       controller++) {
    ...
    // If the track is a combo type track, then move any underlying selected
    // samples, too
    for (int edittype = 0; edittype <= (track->IsComboType() ? 1 : 0);
         edittype++) {
...
ExpressionTool::ExpressionTool(mxWindow *parent)
    : IFacePoserToolWindow("ExpressionTool", "Flex Animation"),
      mxWindow(parent, 0, 0, 0, 0) {
  ...
  m_pHorzScrollBar = new mxScrollbar(this, 0, 0, 18, 100, IDC_FLEXHSCROLL,
...
//			rcHandle -
//-----------------------------------------------------------------------------
void ExpressionTool::DrawScrubHandle(CChoreoWidgetDrawHelper &drawHelper,
                                     RECT &rcHandle) {
...
void ExpressionTool::redraw() {
  ...
  drawHelper.DrawColoredLine(areaBorder, PS_SOLID, 1, 0, rcSelection.top, w2(),
                             rcSelection.top);
  drawHelper.DrawColoredLine(areaBorder, PS_SOLID, 1, 0, rcSelection.bottom,
                             w2(), rcSelection.bottom);
  ...
  if (m_bSelectionActive) {
    ...
    drawHelper.DrawColoredLine(RGB(100, 100, 255), PS_SOLID, 3,
                               rcSelection.left, rcSelection.top,
                               rcSelection.left, rcSelection.bottom);
    drawHelper.DrawColoredLine(RGB(100, 100, 255), PS_SOLID, 3,
                               rcSelection.right, rcSelection.top,
  ...
  if (ev) {
    ...
    if (total > 0) {
      ...
      drawHelper.DrawColoredText("Small Fonts", 8, FW_NORMAL, RGB(0, 100, 0),
                                 rcUndo, "Undo:  %i/%i", current, total);
    }
    ...
    drawHelper.DrawColoredText("Arial", 9, FW_NORMAL, RGB(0, 0, 0), timeRect,
                               sz);
...
// Input  : *tag -
//-----------------------------------------------------------------------------
bool ExpressionTool::GetTimingTagRect(RECT &rcClient, CChoreoEvent *event,
                                      CFlexTimingTag *tag, RECT &rcTag) {
  rcTag = rcClient;
...
void ExpressionTool::DrawRelativeTags(CChoreoWidgetDrawHelper &drawHelper) {
    ...
    for (int j = 0; j < a->GetNumChannels(); j++) {
      ...
      for (int k = 0; k < c->GetNumEvents(); k++) {
        ...
        for (int t = 0; t < e->GetNumRelativeTags(); t++) {
          ...
          // SendMessage( control, CB_ADDSTRING, 0, (LPARAM)va( "\"%s\" \"%s\"",
          // tag->GetName(), e->GetParameters() ) );
          bool clipped;
          int tagx = GetPixelForTimeValue(
              tag->GetStartTime() - event->GetStartTime(), &clipped);
          if (clipped)
            continue;

          // drawHelper.DrawColoredLine( RGB( 180, 180, 220 ), PS_SOLID, 1,
          // tagx, rcClient.top, tagx, rcClient.bottom );
          ...
          int len =
              drawHelper.CalcTextWidth("Arial", 9, FW_NORMAL, tag->GetName());
          rcText.left = tagx - len / 2;
          rcText.right = rcText.left + len + 2;

          rcText.bottom = rcText.top + 10;

          drawHelper.DrawColoredText("Arial", 9, FW_NORMAL, RGB(0, 100, 200),
                                     rcText, tag->GetName());
        }
      }
    }
  ...
  for (int t = 0; t < event->GetNumTimingTags(); t++) {
    ...
    drawHelper.DrawColoredText("Arial", 9, FW_NORMAL, RGB(200, 100, 0), rcText,
                               text);
  }
}
...
void ExpressionTool::ShowContextMenu(mxEvent *event, bool include_track_menus) {
  ...
  if (item && track && item->IsCollapsed()) {
    expand->add(va("Track '%s'", track->GetFlexControllerName()),
                IDC_TL_EXPAND);
  }
  ...
  if (item && track && !item->IsCollapsed()) {
    collapse->add(va("Track '%s'", track->GetFlexControllerName()),
                  IDC_TL_COLLAPSE);
    collapse->add(va("All tracks except '%s'", track->GetFlexControllerName()),
                  IDC_COLLAPSE_ALL_EXCEPT);
  }
  ...
  if (item && track) {
    if (item->IsActive()) {
      pop->add(va("Disable '%s'", track->GetFlexControllerName()),
               IDC_TL_DISABLE);
    } else {
      pop->add(va("Enable '%s'", track->GetFlexControllerName()),
               IDC_TL_ENABLE);
    }
    pop->add(va("Disable all except '%s'", track->GetFlexControllerName()),
             IDC_DISABLE_ALL_EXCEPT);
    ...
    if (FacePoser_IsSnapping()) {
      ...
      snap->add(va("All points in '%s'", track->GetFlexControllerName()),
                IDC_TL_SNAPPOINTS);
      snap->add(va("Selected points in '%s'", track->GetFlexControllerName()),
                IDC_TL_SNAPSELECTED);
    ...
    heightMenu->add(va("Reset '%s'", track->GetFlexControllerName()),
                    IDC_ET_RESET_ITEM_SIZE);
  ...
  if (tag) {
    if (tag->GetLocked()) {
      tagmenu->add(va("Unlock tag '%s'...", tag->GetName()),
                   IDC_UNLOCK_TIMING_TAG);
    } else {
      tagmenu->add(va("Lock tag '%s'...", tag->GetName()), IDC_LOCK_TIMING_TAG);
    }
    tagmenu->addSeparator();
    tagmenu->add(va("Delete tag '%s'...", tag->GetName()),
                 IDC_DELETE_TIMING_TAG);
  } else {
    tagmenu->add("Insert...", IDC_INSERT_TIMING_TAG);
  }

  bool bMouseOverSelection =
      IsMouseOverSelection((short)event->x, (short)event->y);
  ...
  if (bMouseOverSelection || HasCopiedColumn()) {
    ...
    if (bMouseOverSelection) {
      ...
      selectionMenu->add("Delete samples and shift remainder",
                         IDC_ET_SELECTION_EXCISE);
    }
...
//			cursor -
//-----------------------------------------------------------------------------
void ExpressionTool::StartDragging(int dragtype, int startx, int starty,
                                   HCURSOR cursor) {
...
int ExpressionTool::handleEvent(mxEvent *event) {
  ...
  case mxEvent::Size: {
    ...
    m_pWorkspace->setBounds(5, TRAY_HEIGHT + GetCaptionHeight(), w - 10,
                            h - (TRAY_HEIGHT + 5 + GetCaptionHeight()) -
                                m_nScrollbarHeight);
  ...
  case mxEvent::MouseDown: {
    //			bool ctrldown = ( event->modifiers & mxEvent::KeyCtrl )
    //? true : false;
    bool shiftdown = (event->modifiers & mxEvent::KeyShift) ? true : false;
    ...
    if (m_nDragType == DRAGTYPE_NONE) {
      if (IsMouseOverScrubHandle(event)) {
        ...
        StartDragging(DRAGTYPE_SCRUBBER, m_nClickedX, m_nClickedY,
                      LoadCursor(NULL, IDC_SIZEWE));
      } else if (IsMouseOverTag(m_nClickedX, m_nClickedY)) {
        StartDragging(DRAGTYPE_FLEXTIMINGTAG, m_nClickedX, m_nClickedY,
                      LoadCursor(NULL, IDC_SIZEWE));
      } else if (IsMouseOverPoints(m_nClickedX, m_nClickedY)) {
        if (!m_bSelectionActive) {
          StartDragging(DRAGTYPE_SELECTSAMPLES, m_nClickedX, m_nClickedY,
                        LoadCursor(NULL, IDC_SIZEWE));
        } else {
          // Either move, move edge if ctrl key is held, or deselect
          if (IsMouseOverSelection(m_nClickedX, m_nClickedY)) {
            if (IsMouseOverSelectionStartEdge(event)) {
              StartDragging(DRAGTYPE_MOVESELECTIONSTART, m_nClickedX,
      ...
      if (ed > st) {
        ...
        if (prev) {
          ...
          if (frac >= 0.0f && frac <= 1.0f) {
            int tagx = rcClient.left +
                       (int)(frac * (float)(rcClient.right - rcClient.left));
...
//			newposition -
//-----------------------------------------------------------------------------
void ExpressionTool::ResampleControlPoints(CFlexTimingTag *tag,
                                           float newposition) {
  CChoreoEvent *e = tag->GetOwner();
  if (!e)
    return;
  ...
  if (oldleftrange <= 0.0f || oldrightrange <= 0.0f || newleftrange <= 0.0f ||
      newrightrange <= 0.0f) {
...
void ExpressionTool::OnNewExpression(void) {
  ...
  CStudioHdr *hdr = models->GetActiveStudioModel()->GetStudioHdr();
  if (!hdr) {
    Con_ErrorPrintf("ExpressionTool::OnNewExpression:  Can't create new face "
                    "pose, must load a model first!\n");
    return;
  }
  ...
  if (!active) {
    Con_ErrorPrintf("ExpressionTool::OnNewExpression:  Can't create new face "
                    "pose, must load an expression file first!\n");
    return;
  }
  ...
  active->AddExpression(params.m_szName, params.m_szDescription, settings,
                        weights, true, true);
}

LocalFlexController_t FindFlexControllerIndexByName(StudioModel *model,
                                                    char const *searchname) {
  if (!model)
...
bool ExpressionTool::SetFlexAnimationTrackFromExpression(int mx, int my,
                                                         CExpClass *cl,
                                                         CExpression *exp) {
  CChoreoEvent *e = GetSafeEvent();
  if (!e | !e->GetDuration()) {
...
void ExpressionTool::OnExportFlexAnimation(void) {
  ...
  char fafilename[512];
  if (!FacePoser_ShowSaveFileNameDialog(fafilename, sizeof(fafilename),
                                        "flexanimations", "*.vfa")) {
    return;
  }
...
void ExpressionTool::OnImportFlexAnimation(void) {
  ...
  char fafilename[512];
  if (!FacePoser_ShowOpenFileNameDialog(fafilename, sizeof(fafilename),
                                        "flexanimations", "*.vfa")) {
    return;
  }
  ...
  char fullpath[512];
  filesystem->RelativePathToFullPath(fafilename, "MOD", fullpath,
                                     sizeof(fullpath));
  ...
  if (stricmp(tokenprocessor->CurrentToken(), "flexanimations")) {
    Con_Printf("ExpressionTool::OnImportFlexAnimation:  %s, expecting "
               "\"flexanimations\"\n",
               fullpath);
  } else {
    g_pChoreoView->SetDirty(true);
...
//			rcPos -
//-----------------------------------------------------------------------------
void ExpressionTool::DrawMouseOverPos(CChoreoWidgetDrawHelper &drawHelper,
                                      RECT &rcPos) {
...
void ExpressionTool::RepositionHSlider(void) {
  ...
  m_pHorzScrollBar->setBounds(0, h2() - m_nScrollbarHeight, w2(),
                              m_nScrollbarHeight);
...
void ExpressionTool::OnChangeScale(void) {
  ...
  g_pChoreoView->SetTimeZoom(
      GetToolName(),
      clamp((int)(100.0f * atof(params.m_szInputText)), 1, MAX_TIME_ZOOM),
      false);
...
void ExpressionTool::DrawEventEnd(CChoreoWidgetDrawHelper &drawHelper) {
  ...
  drawHelper.DrawColoredLine(COLOR_CHOREO_ENDTIME, PS_SOLID, 1, leftx,
                             rcClient.top + TRAY_HEIGHT, leftx,
                             rcClient.bottom);
}
...
void ExpressionTool::OnPasteColumn() {
  ...
  float flPasteEndTime = flPasteTime + m_ColumnCopy.m_flCopyTimes[1] -
                         m_ColumnCopy.m_flCopyTimes[0];
    ...
    for (t = 0; t < 2; t++) {
      ...
      if (tIndex != m_ColumnCopy.m_Data.InvalidIndex()) {
        ...
        for (int j = 0; j < data.m_Samples[t].Count(); ++j) {
          ...
          CExpressionSample *newSample = track->AddSample(
              s->time - m_ColumnCopy.m_flCopyTimes[0] + flPasteTime, s->value,
              t);
