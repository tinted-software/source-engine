...
bool LoadBitmapFromFile(const char *relative, mxbitmapdata_t &bitmap) {
  ...
  if (bm) {
    ...
    if (memdc) {
      char filename[512];
      filesystem->RelativePathToFullPath(relative, "MOD", filename,
                                         sizeof(filename));

      bmNewImage = (HBITMAP)LoadImage((HINSTANCE)GetModuleHandle(0), filename,
                                      IMAGE_BITMAP, 0, 0,
                                      LR_LOADFROMFILE | LR_DEFAULTSIZE);

      if (!bmNewImage) {
        filesystem->RelativePathToFullPath(relative, "GAME", filename,
                                           sizeof(filename));
...
void DrawBitmapToWindow(mxWindow *wnd, int x, int y, int w, int h,
...
void DrawBitmapToDC(void *hdc, int x, int y, int w, int h,
                    mxbitmapdata_t &bitmap) {
  if (!bitmap.valid)
    return;
