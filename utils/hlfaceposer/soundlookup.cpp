...
static void PopulateSoundEntryList(HWND wnd, CSoundLookupParams *params) {
  ...
  SendMessage(control, WM_SETFONT,
              (WPARAM)(HFONT)GetStockObject(ANSI_FIXED_FONT),
              MAKELPARAM(TRUE, 0));
  ...
  for (int i = 0; i < c; i++) {
    ...
    if (text[0]) {
      ...
      int idx = SendMessage(control, LB_ADDSTRING, 0,
                            (LPARAM)va("%20s:  '%s'", script, text));
...
//-----------------------------------------------------------------------------
static BOOL CALLBACK SoundLookupDialogProc(HWND hwndDlg, UINT uMsg,
  ...
  case WM_COMMAND:
    ...
    case IDOK: {
      int selindex = SendMessage(GetDlgItem(hwndDlg, IDC_SOUNDENTRYLIST),
                                 LB_GETCURSEL, 0, 0);
      if (selindex == LB_ERR) {
        mxMessageBox(NULL, "You must select an entry from the list", g_appTitle,
                     MB_OK);
        return TRUE;
      }

      int soundindex = SendMessage(GetDlgItem(hwndDlg, IDC_SOUNDENTRYLIST),
                                   LB_GETITEMDATA, selindex, 0);
...
int SoundLookup(CSoundLookupParams *params, HWND parent) {
  ...
  int retval =
      DialogBox((HINSTANCE)GetModuleHandle(0), MAKEINTRESOURCE(IDD_WAVELOOKUP),
                parent, (DLGPROC)SoundLookupDialogProc);
