...
void CEventPropertiesGestureDialog::InitControlData(CEventParams *params) {
  ...
  SendMessage(GetControl(IDC_CHECK_SYNCTOFOLLOWINGGESTURE), BM_SETCHECK,
              (WPARAM)g_Params.m_bSyncToFollowingGesture ? BST_CHECKED
                                                         : BST_UNCHECKED,
              (LPARAM)0);
...
bool CEventPropertiesGestureDialog::CheckSequenceType(StudioModel *model,
                                                      int iSequence,
                                                      char *szType) {
...
void CEventPropertiesGestureDialog::ShowControlsForEventType(
    CEventParams *params) {
...
//-----------------------------------------------------------------------------
static BOOL CALLBACK EventPropertiesGestureDialogProc(HWND hwndDlg, UINT uMsg,
                                                      WPARAM wParam,
                                                      LPARAM lParam) {
  return g_EventPropertiesGestureDialog.HandleMessage(hwndDlg, uMsg, wParam,
                                                      lParam);
};

BOOL CEventPropertiesGestureDialog::HandleMessage(HWND hwndDlg, UINT uMsg,
                                                  WPARAM wParam,
                                                  LPARAM lParam) {
  m_hDialog = hwndDlg;
  ...
  case WM_COMMAND:
    ...
    case IDOK: {
      ...
      if (!g_Params.m_szName[0]) {
        Q_strncpy(g_Params.m_szName, g_Params.m_szParameters,
                  sizeof(g_Params.m_szName));
      }
    ...
    case IDC_CHECK_SYNCTOFOLLOWINGGESTURE: {
      g_Params.m_bSyncToFollowingGesture =
          SendMessage(GetControl(IDC_CHECK_SYNCTOFOLLOWINGGESTURE), BM_GETCHECK,
                      0, 0) == BST_CHECKED
              ? true
              : false;
    } break;
