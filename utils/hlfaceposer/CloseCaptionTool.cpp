...
// FIXME: Yahn, what is this, it's coded up as a DELAY before when the closed
// caption is displayed. That seems odd.
...
CCloseCaptionWorkUnit::CCloseCaptionWorkUnit()
    : m_nWidth(0), m_nHeight(0), m_bBold(false), m_bItalic(false),
      m_pszStream(0), m_Color(RGB(255, 255, 255)) {}
...
void CCloseCaptionWorkUnit::SetColor(COLORREF clr) { m_Color = clr; }

COLORREF CCloseCaptionWorkUnit::GetColor() const { return m_Color; }
...
public:
  CCloseCaptionItem(wchar_t *stream, float timetolive, float predisplay,
                    bool valid)
      : m_flTimeToLive(0.0f), m_bValid(false), m_nTotalWidth(0),
        m_nTotalHeight(0), m_bSizeComputed(false) {
    SetStream(stream);
...
CloseCaptionTool::CloseCaptionTool(mxWindow *parent)
    : IFacePoserToolWindow("CloseCaptionTool", "Close Caption"),
      mxWindow(parent, 0, 0, 0, 0) {
  m_nLastItemCount = -1;
  closecaptionmanager = this;

  m_hFonts[CCFONT_NORMAL] =
      CreateFont(-STREAM_POINTSIZE, 0, 0, 0, STREAM_WEIGHT, FALSE, FALSE, FALSE,
                 DEFAULT_CHARSET, OUT_TT_PRECIS, CLIP_DEFAULT_PRECIS,
                 ANTIALIASED_QUALITY, DEFAULT_PITCH, STREAM_FONT);

  m_hFonts[CCFONT_ITALIC] =
      CreateFont(-STREAM_POINTSIZE, 0, 0, 0, STREAM_WEIGHT, TRUE, FALSE, FALSE,
                 DEFAULT_CHARSET, OUT_TT_PRECIS, CLIP_DEFAULT_PRECIS,
                 ANTIALIASED_QUALITY, DEFAULT_PITCH, STREAM_FONT);

  m_hFonts[CCFONT_BOLD] =
      CreateFont(-STREAM_POINTSIZE, 0, 0, 0, 700, FALSE, FALSE, FALSE,
                 DEFAULT_CHARSET, OUT_TT_PRECIS, CLIP_DEFAULT_PRECIS,
                 ANTIALIASED_QUALITY, DEFAULT_PITCH, STREAM_FONT);

  m_hFonts[CCFONT_ITALICBOLD] =
      CreateFont(-STREAM_POINTSIZE, 0, 0, 0, 700, TRUE, FALSE, FALSE,
                 DEFAULT_CHARSET, OUT_TT_PRECIS, CLIP_DEFAULT_PRECIS,
                 ANTIALIASED_QUALITY, DEFAULT_PITCH, STREAM_FONT);
}
...
void CloseCaptionTool::Process(char const *tokenname, float duration,
                               int languageid) {
  bool valid = true;
  wchar_t stream[256];
  if (!LookupUnicodeText(languageid, tokenname, stream,
                         sizeof(stream) / sizeof(wchar_t))) {
    valid = false;
    g_pLocalize->ConvertANSIToUnicode(va("--> Missing Caption[%s]", tokenname),
        ...
        *out = L'\0';
        ...
        if (wcslen(phrase) > 0) {
          CCloseCaptionItem *item = new CCloseCaptionItem(
              phrase, duration + CAPTION_LINGER_TIME, delay, valid);
...
bool CloseCaptionTool::LookupUnicodeText(int languageId, char const *token,
                                         wchar_t *outbuf, size_t count) {
...
bool CloseCaptionTool::LookupStrippedUnicodeText(int languageId,
                                                 char const *token,
                                                 wchar_t *outbuf,
                                                 size_t count) {
  wchar_t *outstr = g_pLocalize->Find(token);
...
bool CloseCaptionTool::SplitCommand(wchar_t const **ppIn, wchar_t *cmd,
                                    wchar_t *args) const {
...
struct WorkUnitParams {
  ...
  int GetFontNumber() { return CloseCaptionTool::GetFontNumber(bold, italic); }
...
void CloseCaptionTool::ComputeStreamWork(CChoreoWidgetDrawHelper &helper,
                                         int available_width,
                                         CCloseCaptionItem *item) {
      ...
      if (!wcscmp(cmd, L"cr")) {
        ...
        if (args[0] == 0 && colorStack.Count() >= 2) {
          ...
          if (6 == swscanf(args, L"%i,%i,%i:%i,%i,%i", &pr, &pg, &pb, &nr, &ng,
            ...
            newcolor = /*item->IsFromPlayer()*/ false ? RGB(pr, pg, pb)
                                                      : RGB(nr, ng, nb);
            colorStack.AddToTail(newcolor);
            params.clr = colorStack[colorStack.Count() - 1];
          }
...
void CloseCaptionTool::DrawStream(CChoreoWidgetDrawHelper &helper, RECT &rcText,
                                  CCloseCaptionItem *item) {
  int c = item->GetNumWorkUnits();
