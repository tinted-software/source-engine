...
GestureTool::GestureTool(mxWindow *parent)
    : IFacePoserToolWindow("GestureTool", "Gesture"),
  ...
  m_pHorzScrollBar = new mxScrollbar(this, 0, 0, 18, 100, IDC_GESTUREHSCROLL,
...
// Input  : rcHandle -
//-----------------------------------------------------------------------------
void GestureTool::GetScrubHandleRect(RECT &rcHandle, float scrub,
                                     bool clipped) {
  float pixel = 0.0f;
  if (w2() > 0) {
...
void GestureTool::GetScrubHandleReferenceRect(RECT &rcHandle, float scrub,
                                              bool clipped /*= false*/) {
  float pixel = 0.0f;
  if (w2() > 0) {
    pixel = GetPixelForTimeValue(scrub);
...
//			rcHandle -
//-----------------------------------------------------------------------------
void GestureTool::DrawScrubHandle(CChoreoWidgetDrawHelper &drawHelper,
                                  RECT &rcHandle, float scrub, bool reference) {
...
void GestureTool::DrawScrubHandles() {
  ...
  if (ev && ev->GetDuration() > 0.0f) {
    float scrub = ev->GetOriginalPercentageFromPlaybackPercentage(
                      m_flScrub / ev->GetDuration()) *
                  ev->GetDuration();
...
void GestureTool::redraw() {
  ...
  if (ev) {
    ...
    drawHelper.DrawColoredText("Arial", 11, 900, RGB(200, 0, 0), rcTextLine,
                               "Sequence:  '%s' %.3f s.", ev->GetParameters(),
                               seqduration);
...
//			cursor -
//-----------------------------------------------------------------------------
void GestureTool::StartDragging(int dragtype, int startx, int starty,
                                HCURSOR cursor) {
  ...
  case mxEvent::MouseDown: {
    ...
    if (m_nDragType == DRAGTYPE_NONE) {
      if (IsMouseOverScrubHandle(event)) {
        ...
        StartDragging(DRAGTYPE_ABSOLUTE_TIMING_TAG, m_nClickedX, m_nClickedY,
                      LoadCursor(NULL, IDC_SIZEWE));
      } else {
        if (w2() > 0) {
          float t = GetTimeValueForMouse((short)event->x);
  ...
  case DRAGTYPE_ABSOLUTE_TIMING_TAG: {
    ...
    if (tag && e && e->GetDuration()) {
      ...
      if (t != -1) {
        ...
        if (i < c) {
          if (prevTag) {
            m_nMinX = GetPixelForTimeValue(prevTag->GetPercentage() *
                                           e->GetDuration()) +
                      1;
          }
          if (nextTag) {
            m_nMaxX = GetPixelForTimeValue(nextTag->GetPercentage() *
                                           e->GetDuration()) -
                      1;
          }
        } else {
...
void GestureTool::OnUndo(void) { g_pChoreoView->Undo(); }

void GestureTool::OnRedo(void) { g_pChoreoView->Redo(); }
...
//			rcPos -
//-----------------------------------------------------------------------------
void GestureTool::DrawMouseOverPos(CChoreoWidgetDrawHelper &drawHelper,
                                   RECT &rcPos) {
...
bool GestureTool::GetAbsTagRect(RECT &rcClient, CChoreoEvent *event,
                                int tagtype, CEventAbsoluteTag *tag,
                                RECT &rcTag) {
  rcTag = rcClient;
...
void GestureTool::DrawAbsoluteTags(CChoreoWidgetDrawHelper &drawHelper) {
  ...
  for (t = 0; t < CChoreoEvent::NUM_ABS_TAG_TYPES; t++) {
    ...
    drawHelper.DrawColoredLine(RGB(220, 220, 220), PS_SOLID, 1, rcTray.left,
                               rcTray.top, rcTray.right, rcTray.top);
    drawHelper.DrawColoredLine(RGB(220, 220, 220), PS_SOLID, 1, rcTray.left,
                               rcTray.bottom, rcTray.right, rcTray.bottom);
    ...
    drawHelper.DrawColoredText("Arial", 9, 500, RGB(150, 150, 150), rcText,
                               "%s",
    ...
    for (int i = 0; i < event->GetNumAbsoluteTags(tagtype); i++) {
      ...
      if (showDots && t == 1) {
        ...
        if (otherTag) {
          GetAbsTagRect(rcClient, event, tagtypeOther, otherTag, rcMark2);
          {
            ...
            drawHelper.DrawColoredLine(RGB(200, 200, 200), PS_SOLID, 1, midx1,
                                       y1, midx2, y2);
          }
        }
      }
      ...
      drawHelper.DrawTriangleMarker(rcMark, RGB(200, 0, 30),
                                    tagtype != CChoreoEvent::PLAYBACK);
      ...
      drawHelper.DrawColoredText("Arial", 9, FW_NORMAL, RGB(200, 100, 100),
                                 rcText, text);
      ...
      // sprintf( text, "%.3f", tag->GetPercentage() * event->GetDuration() +
      // event->GetStartTime() );
...
//			right -
//-----------------------------------------------------------------------------
void GestureTool::DrawTimeLine(CChoreoWidgetDrawHelper &drawHelper, RECT &rc,
...
void GestureTool::OnRevert() {
  ...
  for (int i = 0; i < c; i++) {
    CEventAbsoluteTag *original =
        event->GetAbsoluteTag(CChoreoEvent::ORIGINAL, i);
    CEventAbsoluteTag *playback =
        event->GetAbsoluteTag(CChoreoEvent::PLAYBACK, i);
...
void GestureTool::DrawRelativeTags(CChoreoWidgetDrawHelper &drawHelper,
                                   RECT &rc) {
  ...
  drawHelper.DrawColoredText("Arial", 9, FW_NORMAL, RGB(0, 100, 200), rc,
                             "Timing Tags:");
...
//			rc -
//-----------------------------------------------------------------------------
void GestureTool::DrawRelativeTagsForEvent(CChoreoWidgetDrawHelper &drawHelper,
                                           RECT &rc, CChoreoEvent *gesture,
                                           CChoreoEvent *event, float starttime,
                                           float endtime) {
  if (!event)
    return;
...
void GestureTool::DrawEventEnd(CChoreoWidgetDrawHelper &drawHelper) {
  ...
  drawHelper.DrawColoredLine(COLOR_CHOREO_ENDTIME, PS_SOLID, 1, leftx,
                             GetCaptionHeight() + 73, leftx, rcClient.bottom);
}

void GestureTool::OnModelChanged() { redraw(); }