...
mxExpressionTray::mxExpressionTray(mxWindow *parent, int id /*=0*/)
    : IFacePoserToolWindow("ExpressionTrayTool", "Expressions"),
      mxWindow(parent, 0, 0, 0, 0, "ExpressionTrayTool", id) {
  setId(id);

  m_nTopOffset = 0;
  slScrollbar = new mxScrollbar(this, 0, 0, 18, 100, IDC_TRAYSCROLL,
  ...
  g_pExpressionClass =
      new mxExpressionTab(this, 5, 5, 500, 20, IDC_EXPRESSIONCLASS);

  m_pABButton = new mxButton(this, 520, 8, 50, 18, "A/B", IDC_AB);
  m_pThumbnailIncreaseButton =
      new mxButton(this, 0, 0, 18, 18, "+", IDC_THUMBNAIL_INCREASE);
  m_pThumbnailDecreaseButton =
      new mxButton(this, 0, 0, 18, 18, "-", IDC_THUMBNAIL_DECREASE);
}
...
// Input  : *btn -
//-----------------------------------------------------------------------------
void mxExpressionTray::AddButton(const char *name, const char *tooltip,
                                 const char *bitmap, ETMEMBERFUNC pfnCallback,
...
void mxExpressionTray::CreateButtons(void) {
  ...
  AddButton("undo", "Undo", "gfx/hlfaceposer/undo.bmp",
            &mxExpressionTray::ET_Undo, true, x, y, m_nButtonSquare,
            m_nButtonSquare);

  x += (m_nButtonSquare + 4);

  AddButton("redo", "Redo", "gfx/hlfaceposer/redo.bmp",
            &mxExpressionTray::ET_Redo, true, x, y, m_nButtonSquare,
            m_nButtonSquare);
}
...
//-----------------------------------------------------------------------------
mxExpressionTray::mxETButton *mxExpressionTray::GetItemUnderCursor(int x,
                                                                   int y) {
  ...
  while (p) {
    if (p->m_bActive && x >= cx && x <= cx + cw && y >= cy && y <= cy + ch) {
      ...
      if (cellx >= p->m_rc.left && cellx <= p->m_rc.right &&
          celly >= p->m_rc.top && celly <= p->m_rc.bottom) {
        return p;
      }
    }
    p = p->next;
...
void mxExpressionTray::DrawButton(CChoreoWidgetDrawHelper &helper, int cell,
                                  mxETButton *btn) {
...
int mxExpressionTray::ComputePixelsNeeded(void) {
  ...
  int rowsneeded =
      ((active->GetNumExpressions() + colsperrow - 1) / colsperrow);
  return rowsneeded * (m_nSnapshotHeight + m_nGap) + m_nGap + TOP_GAP +
         GetCaptionHeight();
}

bool mxExpressionTray::ComputeRect(int cell, int &rcx, int &rcy, int &rcw,
                                   int &rch) {
  // Remove scroll bar
  int w = this->w2() - 16;
  ...
  rcy = GetCaptionHeight() + TOP_GAP + (-m_nTopOffset * m_nGranularity) +
        m_nGap + row * (m_nSnapshotHeight + m_nGap);
...
void mxExpressionTray::DrawExpressionFocusRect(CChoreoWidgetDrawHelper &helper,
                                               int x, int y, int w, int h,
                                               COLORREF clr) {
  helper.DrawOutlinedRect(clr, PS_SOLID, 4, x, y, x + w, y + h);
}

void mxExpressionTray::DrawExpressionDescription(
    CChoreoWidgetDrawHelper &helper, int x, int y, int w, int h,
  ...
  helper.DrawColoredText("Arial", 9, FW_NORMAL, RGB(0, 0, 0), textRect, "%s",
                         expressionname);

  //	DrawText( hdc, expressionname, strlen( expressionname ), &textRect,
  //DT_NOPREFIX | DT_SINGLELINE | DT_LEFT | DT_VCENTER | DT_WORD_ELLIPSIS );

  OffsetRect(&textRect, 0, textheight);

  helper.DrawColoredText("Arial", 9, FW_NORMAL, RGB(63, 63, 63), textRect, "%s",
                         description);

  //	DrawText( hdc, description, strlen( description ), &textRect,
  //DT_NOPREFIX | DT_SINGLELINE | DT_LEFT | DT_VCENTER | DT_WORD_ELLIPSIS );
}
...
//			rch -
//-----------------------------------------------------------------------------
void mxExpressionTray::DrawDirtyFlag(CChoreoWidgetDrawHelper &helper,
                                     CExpression *current, int rcx, int rcy,
                                     int rcw, int rch) {
  // Not dirty
  ...
  helper.DrawColoredText("Arial", fontsize, FW_NORMAL, RGB(100, 240, 255),
                         textRect, "*");
}
...
void mxExpressionTray::DrawThumbNail(CExpClass *active, CExpression *current,
                                     CChoreoWidgetDrawHelper &helper, int rcx,
                                     int rcy, int rcw, int rch, int c,
                                     int selected, bool updateselection) {
  if (!current)
    return;

  HDC dc = helper.GrabDC();

  helper.DrawFilledRect(GetSysColor(COLOR_BTNFACE), rcx, rcy, rcw + rcx,
                        rch + rcy);

  if (current->m_Bitmap[models->GetActiveModelIndex()].valid) {
    DrawBitmapToDC(dc, rcx, rcy, rcw, rch - m_nDescriptionHeight,
                   current->m_Bitmap[models->GetActiveModelIndex()]);
    helper.DrawOutlinedRect(RGB(127, 127, 127), PS_SOLID, 1, rcx, rcy,
                            rcx + rcw, rcy + rch - m_nDescriptionHeight);
  }

  DrawDirtyFlag(helper, current, rcx, rcy, rcw, rch);

  DrawExpressionDescription(helper, rcx, rcy, rcw, rch, current->name,
                            current->description);

  if (c == selected) {
    DrawExpressionFocusRect(helper, rcx, rcy, rcw, rch - m_nDescriptionHeight,
                            RGB(255, 100, 63));
...
void mxExpressionTray::redraw() {
  ...
  if (active) {
    ...
    while (c < active->GetNumExpressions()) {
      ...
      DrawThumbNail(active, current, helper, rcx, rcy, rcw, rch, c, selected,
                    updateSelection);
    ...
    helper.DrawColoredText("Arial", pointsize, FW_NORMAL, RGB(80, 80, 80),
                           rcText, sz);
  }
...
void mxExpressionTray::RepositionSlider(void) {
  ...
  slScrollbar->setBounds(w2() - 16, GetCaptionHeight() + TOP_GAP, 16,
                         trueh - TOP_GAP);
...
void mxExpressionTray::SetClickedCell(int cell) { m_nClickedCell = cell; }
...
void mxExpressionTray::ShowRightClickMenu(int mx, int my) {
  ...
  if (exp) {
    ...
    pop->addSeparator();
    pop->add(va("Re-create thumbnail for '%s'", exp->name),
             IDC_CONTEXT_CREATEBITMAP);
  }
    ...
    if (!(event->buttons & mxEvent::MouseRightButton)) {
      ...
      if (active) {
        ...
        if (cell >= 0 && cell < active->GetNumExpressions()) {
          active->SelectExpression(
              cell, event->modifiers & mxEvent::KeyShift ? false : true);
      ...
      if (active) {
        ...
        if (exp && maybeTool) {
          ...
          if (IsWindowOrChild(g_pExpressionTool, maybeTool)) {
            if (g_pExpressionTool->SetFlexAnimationTrackFromExpression(
                    pt.x, pt.y, active, exp)) {
              return iret;
            }
          }
        }
...
bool IsUsingPerPlayerExpressions() {
  ...
  if (CommandLine()->CheckParm("-perplayerexpressions")) {
    ...
    if (g_pFullFileSystem->GetSearchPath("GAME", false, pSearchPath,
                                         sizeof(pSearchPath))) {
...
void mxExpressionTray::OnModelChanged() {
  if (IsUsingPerPlayerExpressions()) {
    ...
    char fn[MAX_PATH];
    Q_snprintf(fn, sizeof(fn), "expressions/%s/phonemes/phonemes.txt",
               models->GetActiveModelName());

    // Load appropriate classes
    char rootDir[MAX_PATH];
    Q_snprintf(rootDir, sizeof(rootDir), "%s/phonemes/",
               models->GetActiveModelName());
