...
// Purpose:
//-----------------------------------------------------------------------------
CExpressionManager::~CExpressionManager(void) { Reset(); }
...
// Purpose:
//-----------------------------------------------------------------------------
CExpClass *CExpressionManager::GetActiveClass(void) { return m_pActiveClass; }
...
// Output : CExpClass
//-----------------------------------------------------------------------------
CExpClass *CExpressionManager::GetClass(int num) { return m_Classes[num]; }
...
// Output : CExpClass *
//-----------------------------------------------------------------------------
CExpClass *CExpressionManager::AddCExpClass(const char *classname,
                                            const char *filename) {
...
// Output : int
//-----------------------------------------------------------------------------
int CExpressionManager::GetNumClasses(void) { return m_Classes.Size(); }
...
CExpClass *CExpressionManager::FindClass(const char *classname,
  ...
  for (int i = 0; i < m_Classes.Size(); i++) {
    ...
    if (!Q_stricmp(search,
                   bMatchBaseNameOnly ? cl->GetBaseName() : cl->GetName())) {
      return cl;
    }
  }
...
// Output : CExpression
//-----------------------------------------------------------------------------
CExpression *CExpressionManager::GetCopyBuffer(void) { return &m_CopyBuffer; }
...
void CExpressionManager::LoadClass(const char *inpath) {
  ...
  if (!hdr) {
    Con_ErrorPrintf(
        "Can't load expressions from %s, must load a .mdl file first!\n",
        filename);
    return;
  }
  ...
  LocalFlexController_t
      localflexmap[128]; // maps file local controls into local controls
  ...
  while (1) {
    ...
    if (stricmp(token, "$keys") == 0) {
      ...
      while (TokenAvailable()) {
        ...
        for (LocalFlexController_t i = LocalFlexController_t(0);
          ...
          if (stricmp(hdr->pFlexcontroller(i)->pszName(), token) == 0) {
            ...
            flexmap[numflexmaps] =
                AddGlobalFlexController(models->GetActiveStudioModel(),
        ...
        if (!bFound) {
          flexmap[numflexmaps] =
              AddGlobalFlexController(models->GetActiveStudioModel(), token);
        }
        numflexmaps++;
      }
        ...
        if (flexmap[i] > -1) {
          ...
          if (bNormalized && localflexmap[i] > -1) {
            mstudioflexcontroller_t *pFlex =
                hdr->pFlexcontroller(localflexmap[i]);
            if (pFlex->min != pFlex->max) {
              setting[flexmap[i]] =
                  Lerp(setting[flexmap[i]], pFlex->min, pFlex->max);
            }
          }
        } else {
          GetToken(false);
      ...
      CExpression *exp =
          active->AddExpression(name, desc, setting, weight, false, false);
      if (active->IsPhonemeClass() && exp) {
        if (exp->index != index) {
          Con_Printf(
              "CExpressionManager::LoadClass (%s):  phoneme index for %s in "
              ".txt file is wrong (expecting %i got %i), ignoring...\n",
...
void CExpressionManager::CreateNewClass(const char *filename) {
  ...
  if (!hdr) {
    Con_ErrorPrintf(
        "Can't create new expression file %s, must load a .mdl file first!\n",
        filename);
    return;
  }
...
bool CExpressionManager::CloseClass(CExpClass *cl) {
  ...
  if (cl->GetDirty()) {
    int retval =
        mxMessageBox(NULL, va("Save changes to class '%s'?", cl->GetName()),
