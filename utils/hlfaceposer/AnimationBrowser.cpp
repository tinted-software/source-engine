...
public:
  CAnimBrowserTab(AnimationBrowser *parent, int x, int y, int w, int h,
                  int id = 0, int style = 0)
  ...
  virtual void ShowRightClickMenu(int mx, int my) {
    ...
    for (int i = 0; i < m_CustomGroups.Count(); ++i) {
      ...
      sub->add(va("%s", m_CustomGroups[i].String()),
               IDC_AB_DELETEGROUPSTART + i);
    }
    if (sub) {
...
AnimationBrowser::AnimationBrowser(mxWindow *parent, int id /*=0*/)
  ...
  m_nTopOffset = 0;
  slScrollbar = new mxScrollbar(this, 0, 0, 18, 100, IDC_AB_TRAYSCROLL,
  ...
  m_pThumbnailIncreaseButton =
      new mxButton(this, 0, 0, 18, 18, "+", IDC_AB_THUMBNAIL_INCREASE);
...
bool AnimationBrowser::ComputeRect(int cell, int &rcx, int &rcy, int &rcw,
                                   int &rch) {
  // Remove scroll bar
  int w = this->w2() - 16;
...
void AnimationBrowser::DrawSequenceFocusRect(CChoreoWidgetDrawHelper &helper,
                                             int x, int y, int w, int h,
                                             COLORREF clr) {
  helper.DrawOutlinedRect(clr, PS_SOLID, 4, x, y, x + w, y + h);
}

void AnimationBrowser::DrawSequenceDescription(CChoreoWidgetDrawHelper &helper,
                                               int x, int y, int w, int h,
                                               int sequence,
  ...
  helper.DrawColoredText("Arial", 9, FW_NORMAL, RGB(63, 63, 63), textRect,
                         "%.2f seconds", mdl->GetDuration(sequence));

  textRect.top = y + h - 4 * textheight - 1;
  textRect.bottom = textRect.top + textheight;

  helper.DrawColoredText("Arial", 9, FW_NORMAL, RGB(50, 200, 255), textRect,
                         "frames %i", mdl->GetNumFrames(sequence));

  OffsetRect(&textRect, 0, textheight - 4);

  helper.DrawColoredText("Arial", 9, FW_NORMAL, RGB(50, 200, 255), textRect,
                         "fps %.2f", (float)mdl->GetFPS(sequence));
}
...
void AnimationBrowser::DrawThumbNail(int sequence,
                                     CChoreoWidgetDrawHelper &helper, int rcx,
                                     int rcy, int rcw, int rch) {
  ...
  mxbitmapdata_t *bm = models->GetBitmapForSequence(
      models->GetActiveModelIndex(), TranslateSequenceNumber(sequence));
  if (bm && bm->valid) {
  ...
  DrawSequenceDescription(helper, rcx, rcy, rcw, rch,
                          TranslateSequenceNumber(sequence), *pseqdesc);

  if (sequence == m_nCurCell) {
    DrawSequenceFocusRect(helper, rcx, rcy, rcw, rch - m_nDescriptionHeight,
                          RGB(255, 100, 63));
  }
}
...
void AnimationBrowser::redraw() {
  ...
  for (int i = 0; i < c; i++) {
    if (!ComputeRect(i, rcx, rcy, rcw, rch)) {
      ...
      // models->GetBitmapForSequence( models->GetActiveModelIndex(),
      // TranslateSequenceNumber( i ) );
      continue;
    }
  ...
  helper.DrawColoredText("Arial", 9, FW_NORMAL, RGB(63, 63, 63), rcText,
                         "%i sequences", curcount);
}
...
void AnimationBrowser::ShowRightClickMenu(int mx, int my) {
  ...
  if (m_CustomAnimationTabs.Count() > 0) {
    ...
    for (int i = 0; i < m_CustomAnimationTabs.Count(); ++i) {
      ...
      if (anim->HasAnimation(pseqdesc->pszLabel())) {
        ca->add(va("%s", anim->m_ShortName.String()),
                IDC_AB_REMOVEFROMGROUPSTART + i);
        useMenu = true;
      }
    }
  ...
  pop->add(va("Re-create thumbnail for '%s'", pseqdesc->pszLabel()),
...
int AnimationBrowser::handleEvent(mxEvent *event) {
  ...
  case mxEvent::Action: {
    ...
    switch (event->action) {
    default:
      if (event->action >= IDC_AB_ADDTOGROUPSTART &&
          event->action <= IDC_AB_ADDTOGROUPEND) {
        int index = event->action - IDC_AB_ADDTOGROUPSTART;
        mstudioseqdesc_t *pseqdesc = GetSeqDesc(m_nCurCell);
        if (pseqdesc) {
          AddAnimationToCustomFile(index, pseqdesc->pszLabel());
        }
      } else if (event->action >= IDC_AB_REMOVEFROMGROUPSTART &&
                 event->action <= IDC_AB_REMOVEFROMGROUPEND) {
        ...
        if (pseqdesc) {
          RemoveAnimationFromCustomFile(index, pseqdesc->pszLabel());
                 ...
                 event->action <= IDC_AB_DELETEGROUPEND) {
        int index = event->action - IDC_AB_DELETEGROUPSTART;
        DeleteCustomFile(index);
      } else if (event->action >= IDC_AB_RENAMEGROUPSTART &&
                 event->action <= IDC_AB_RENAMEGROUPEND) {
    ...
    case IDC_AB_CONTEXT_CREATEBITMAP: {
      ...
      if (m_nClickedCell >= 0) {
        models->RecreateAnimationBitmap(
            current_model, TranslateSequenceNumber(m_nClickedCell));
      }
      redraw();
    } break;
    ...
    if (m_bDragging && m_nClickedCell >= 0) {
      ...
      if (maybeTool && pseqdesc) {
        if (IsWindowOrChild(g_pChoreoView, maybeTool)) {
          if (g_pChoreoView->CreateAnimationEvent(pt.x, pt.y,
                                                  pseqdesc->pszLabel())) {
            return iret;
          }
        }
...
// HACK HACK:  VS2005 is generating bogus code for this little operation in the
// function below...
...
// Output : static bool
//-----------------------------------------------------------------------------
static bool IsTypeOfSequence(StudioModel *model, int sequence,
  ...
  if (seqKeyValues->LoadFromBuffer(model->GetFileName(),
                                   model->GetKeyValueText(sequence))) {
...
bool AnimationBrowser::SequencePassesFilter(StudioModel *model, int sequence,
                                            mstudioseqdesc_t &seqdesc) {
...
void AnimationBrowser::FindCustomFiles(char const *subdir,
...
void AnimationBrowser::BuildCustomFromFiles(
    CUtlVector<FileNameHandle_t> &files) {
  PurgeCustom();
...
void AnimationBrowser::RenameCustomFile(int index) {
  ...
  Q_snprintf(params.m_szDialogTitle, sizeof(params.m_szDialogTitle),
  ...
  Q_snprintf(fn, sizeof(fn), "expressions/%s/animation/%s.txt", basename,
...
void AnimationBrowser::AddAnimationToCustomFile(int index,
...
void AnimationBrowser::RemoveAnimationFromCustomFile(
    int index, char const *animationName) {
  if (index < 0 || index >= m_CustomAnimationTabs.Count())
    return;
