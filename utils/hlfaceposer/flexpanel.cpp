...
FlexPanel::FlexPanel(mxWindow *parent)
    : IFacePoserToolWindow("FlexPanel", "Flex Sliders"),
  ...
  btnResetSliders =
      new mxButton(this, 0, 0, 100, 20, "Zero Sliders", IDC_EXPRESSIONRESET);

  btnCopyToSliders =
      new mxButton(this, 0, 0, 100, 20, "Get Tracks", IDC_COPY_TO_FLEX);
  btnCopyFromSliders =
      new mxButton(this, 0, 0, 100, 20, "Make Keyframe", IDC_COPY_FROM_FLEX);
  ...
  for (int i = 0; i < GLOBAL_STUDIO_FLEX_CONTROL_COUNT; i++) {
    ...
    slFlexScale[i] = new mxExpressionSlider(wFlex, w, h, 220, LINE_HEIGHT,
                                            IDC_FLEXSCALE + i);
  }

  slScrollbar = new mxScrollbar(wFlex, 0, 0, 18, 100, IDC_FLEXSCROLL,
...
void FlexPanel::PositionSliders(int sboffset) {
  int reservedheight = GetCaptionHeight() + 5 /*gap at top*/ +
                       1 * 20 /* space for buttons/edit controls*/;

  int widthofslidercolumn = slFlexScale[0]->w() + 10;

  int colsavailable =
      (this->w2() - 20 /*scrollbar*/ - 10 /*left edge gap + right gap*/) /
      widthofslidercolumn;
  ...
  for (int i = 0; i < GLOBAL_STUDIO_FLEX_CONTROL_COUNT; i++) {
    ...
    int y = row * LINE_HEIGHT + 5 + GetCaptionHeight() -
            sboffset; // (i % 4) * 20 + 5;
  ...
  case mxEvent::Action: {
    ...
    case IDC_FLEXSCROLL: {
      if (event->event == mxEvent::Action &&
        ...
        int offset =
            event->height; // ((mxScrollbar *) event->widget)->getValue( );

        slScrollbar->setValue(
            offset); // if (offset > slScrollbar->getPagesize()
    ...
    case IDC_COPY_TO_FLEX: {
      g_pExpressionTool->OnCopyToFlex(g_pChoreoView->GetScene()->GetTime(),
                                      true);
    } break;
    case IDC_COPY_FROM_FLEX: {
      g_pExpressionTool->OnCopyFromFlex(g_pChoreoView->GetScene()->GetTime(),
                                        false);
    } break;
    ...
    if (event->action >= IDC_FLEXSCALE &&
        event->action < IDC_FLEXSCALE + GLOBAL_STUDIO_FLEX_CONTROL_COUNT) {
      iret = 1;
      ...
      if (active) {
        ...
        if (pushundo && index != -1) {
          ...
          if (exp) {
            ...
            if (settings[flex] != value || settings[flex2] != value2 ||
                weights[flex] != influ) {
      ...
      LocalFlexController_t localflex = FindFlexControllerIndexByName(
          models->GetActiveStudioModel(), GetGlobalFlexControllerName(flex));
      if (localflex >= 0) {
        // Update the face
        // FIXME: I'm not sure this is needed anymore....
        models->GetActiveStudioModel()->SetFlexController(localflex,
                                                          value * influ);
        if (flex2 != flex) {
          LocalFlexController_t localflex2 =
              FindFlexControllerIndexByName(models->GetActiveStudioModel(),
                                            GetGlobalFlexControllerName(flex2));
          if (localflex2 >= 0) {
            models->GetActiveStudioModel()->SetFlexController(localflex2,
                                                              value2 * influ);
          } else {
            Assert(0);
          }
  ...
  if (hdr) {
    ...
    for (LocalFlexController_t k = LocalFlexController_t(0);
      ...
      if (hdr->pFlexcontroller(k)->min != hdr->pFlexcontroller(k)->max)
        slFlexScale[j]->setRange(0, hdr->pFlexcontroller(k)->min,
                                 hdr->pFlexcontroller(k)->max);
      ...
      if (strncmp("right_", hdr->pFlexcontroller(k)->pszName(), 6) == 0) {
        ...
        if (nFlexSliderIndex[controller] == FLEXSLIDER_INVALID_INDEX) {
          ...
          // Con_Printf( "Assigning stereo side of bar %i to barnum %i of slider
          // %s for controller\n", 	j, 1, hdr->pFlexcontroller(k)->pszName(),
          //controller );
...
float FlexPanel::GetSlider(int iFlexController) {
  ...
  return slFlexScale[nFlexSliderIndex[iFlexController]]->getValue(
...
float FlexPanel::GetSliderRawValue(int iFlexController) {
  ...
  return slFlexScale[nFlexSliderIndex[iFlexController]]->getRawValue(
      nFlexSliderBarnum[iFlexController]);
}

void FlexPanel::GetSliderRange(int iFlexController, float &minvalue,
                               float &maxvalue) {
...
void FlexPanel::SetSlider(int iFlexController, float value) {
  ...
  slFlexScale[nFlexSliderIndex[iFlexController]]->setValue(
      nFlexSliderBarnum[iFlexController], value);
}
...
void FlexPanel::SetEdited(int iFlexController, bool isEdited) {
  ...
  slFlexScale[nFlexSliderIndex[iFlexController]]->setEdited(
      nFlexSliderBarnum[iFlexController], isEdited);
}
...
bool FlexPanel::IsEdited(int iFlexController) {
  ...
  return slFlexScale[nFlexSliderIndex[iFlexController]]->isEdited(
      nFlexSliderBarnum[iFlexController]);
}
...
void FlexPanel::SetInfluence(int iFlexController, float value) {
  ...
  // Con_Printf( "SetInfluence( %d, %.0f ) : %d %d\n", iFlexController, value,
  // nFlexSliderIndex[ iFlexController ], nFlexSliderBarnum[ iFlexController ]
  // );
...
int FlexPanel::LookupPairedFlex(int iFlexController) {
  ...
  } else if (nFlexSliderIndex[iFlexController + 1] ==
             nFlexSliderIndex[iFlexController]) {
    return iFlexController + 1;
  }
  return iFlexController;
}
...
void FlexPanel::setExpression(int index) {
  ...
  for (LocalFlexController_t i = LocalFlexController_t(0);
    ...
    models->GetActiveStudioModel()->SetFlexController(i,
                                                      settings[j] * weights[j]);
  }
}
...
void FlexPanel::SaveExpression(int index) {
  ...
  int retval = mxMessageBox(this, "Overwrite existing expression?", g_appTitle,
                            MX_MB_YESNO | MX_MB_QUESTION);
  if (retval != 0)
    return;
  ...
  if (active) {
    ...
    if (index != -1) {
      ...
      if (exp) {
        ...
        if (memcmp(settings, zeroes.GetSettings(),
                   GLOBAL_STUDIO_FLEX_CONTROL_COUNT * sizeof(float))) {
          if (preserveundo) {
            exp->PushUndoInformation();
            needredo = true;
          }
...
void FlexPanel::PasteControllerSettings(void) {
  ...
  if (index != -1) {
    ...
    if (exp) {
      ...
      // UPDATEME
      if (memcmp(settings, paste->GetSettings(),
                 GLOBAL_STUDIO_FLEX_CONTROL_COUNT * sizeof(float))) {
        exp->PushUndoInformation();
        needredo = true;
...
void FlexPanel::EditExpression(void) {
  ...
  if (index == -1) {
    Con_ErrorPrintf(
        "Can't edit face pose, must select a face from list first!\n");
    return;
  }
