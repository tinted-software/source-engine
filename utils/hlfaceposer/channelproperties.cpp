...
//-----------------------------------------------------------------------------
static BOOL CALLBACK ChannelPropertiesDialogProc(HWND hwndDlg, UINT uMsg,
      ...
      if (g_Params.m_bShowActors) {
        ...
        if (control) {
          SendMessage(control, WM_GETTEXT,
                      (WPARAM)sizeof(g_Params.m_szSelectedActor),
...
int ChannelProperties(CChannelParams *params) {
  ...
  int retval = DialogBox(
      (HINSTANCE)GetModuleHandle(0), MAKEINTRESOURCE(IDD_CHANNELPROPERTIES),
