...
void CChoreoView::ShowContextMenu(int mx, int my) {
    ...
    mxPopupMenu *editMenu = new mxPopupMenu();
    {
      ...
      if (ge) {
        ...
        case CChoreoEvent::SECTION: {
          editMenu->add(va("Section Pause '%s'...", ge->GetEvent()->GetName()),
                        IDC_EDITGLOBALEVENT);
        } break;
  ...
  if (a || c || e || ge || (ct != -1)) {
    ...
    if (ge) {
      ...
      default:
        break;
      case CChoreoEvent::SECTION: {
        deleteMenu->add(va("Section Pause '%s'...", ge->GetEvent()->GetName()),
  ...
  if (e) {
    ...
    case CChoreoEvent::GESTURE: {
      pop->add(va("Edit event '%s' in gesture tool", e->GetEvent()->GetName()),
               IDC_GESTURETOOL);
    } break;
    }
...
void SetupFlexControllerTracks(CStudioHdr *hdr, CChoreoEvent *event) {
  ...
  /*
  // FIXME:  Brian hooked this stuff up for some took work, but at this point
  the .mdl files don't look like they've been updated to include the remapping
  data yet... int c = hdr->numflexcontrollerremaps(); for ( i = 0; i < c; ++i )
  {
  ...
  for (LocalFlexController_t i = LocalFlexController_t(0);
    ...
    if (track->GetNumSamples(0) > 0) {
      ...
      for (int i = 0; i < track->GetNumSamples(0); i++) {
        ...
        sample->value =
            clamp((rangedValue - track->GetMin()) / range, 0.0, 1.0);
      }
    }
...
void CChoreoView::LoadNext(void) {
  ...
  filesystem->FullPathToRelativePath(fixedupFile, relativeFile,
                                     sizeof(relativeFile));
...
//-----------------------------------------------------------------------------
CEventAbsoluteTag *
CChoreoView::GetAbsoluteTagUnderCursorPos(CChoreoEventWidget *event, int mx,
                                          int my) {
  if (!event) {
    return NULL;
  }
...
void CChoreoView::OnCombineSpeakEvents() {
  ...
  for (int i = c - 1; i >= 0; --i) {
    ...
    if (!firstChannel) {
      ...
      Con_ErrorPrintf("Can't combine events, all events must reside in the "
                      "same channel.\n");
      return;
    }
  }
  ...
  if (!scriptfile || !scriptfile[0]) {
    Con_ErrorPrintf("Unable to find existing script to use for new combined "
                    "sound entry.\n");
    return;
  }
...
// Input  :  -
//-----------------------------------------------------------------------------
template <class T> void DeleteAllAndPurge(T &tree) {
  T::IndexType_t i;
