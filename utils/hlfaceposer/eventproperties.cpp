...
void CBaseEventPropertiesDialog::PopulateTagList(CEventParams *params) {
  ...
  if (control) {
    ...
    SendMessage(
        control, WM_SETTEXT, 0,
        (LPARAM)va("\"%s\" \"%s\"", params->m_szTagName, params->m_szTagWav));
...
CMapEntities::CMapEntities() { m_szCurrentMap[0] = 0; }

CMapEntities::~CMapEntities() { m_Entities.RemoveAll(); }
...
int CMapEntities::Count(void) { return m_Entities.Count(); }
...
void CBaseEventPropertiesDialog::SetDialogTitle(CEventParams *params,
                                                char const *eventname,
                                                char const *desc) {
  char sz[256];
  Q_snprintf(sz, sizeof(sz), " : %s", eventname);
  Q_strncat(params->m_szDialogTitle, sz, sizeof(params->m_szDialogTitle),
            COPY_ALL_CHARACTERS);
...
void CBaseEventPropertiesDialog::ShowControlsForEventType(
    CEventParams *params) {
...
void CBaseEventPropertiesDialog::InitControlData(CEventParams *params) {
  ...
  if (GetControl(IDC_CHECK_RESUMECONDITION) != (HWND)0) {
    SendMessage(GetControl(IDC_CHECK_RESUMECONDITION), BM_SETCHECK,
                (WPARAM)params->m_bResumeCondition ? BST_CHECKED
                                                   : BST_UNCHECKED,
                (LPARAM)0);
  }
...
BOOL CBaseEventPropertiesDialog::InternalHandleMessage(CEventParams *params,
  ...
  case WM_COMMAND:
    ...
    case IDC_CHECK_DISABLED: {
      params->m_bDisabled = SendMessage(GetControl(IDC_CHECK_DISABLED),
...
void CBaseEventPropertiesDialog::PopulateNamedActorList(HWND wnd,
                                                        CEventParams *params) {
  int i;
  ...
  for (i = 0; i < params->m_pScene->GetNumActors(); i++) {
    ...
    if (actor && actor->GetName() && actor->GetName()[0]) {
      if (m_SortedNames.Find(actor->GetName()) ==
          m_SortedNames.InvalidIndex()) {
  ...
  /*
  // Note have to do this here, after posting data to the control, since we are
  storing a raw string pointer in m_SortedNames!!! if ( allActors )
  {
...
void CBaseEventPropertiesDialog::ParseTags(CEventParams *params) {
  ...
  if (params->m_bUsesTag) {
    ...
    HWND control = GetControl(IDC_TAGS);
    if (control) {
      SendMessage(control, WM_GETTEXT, (WPARAM)sizeof(selectedText),
                  (LPARAM)selectedText);
    }
...
void CBaseEventPropertiesDialog::UpdateTagRadioButtons(CEventParams *params) {
  if (params->m_bUsesTag) {
    SendMessage(GetControl(IDC_RELATIVESTART), BM_SETCHECK, (WPARAM)BST_CHECKED,
                (LPARAM)0);
    SendMessage(GetControl(IDC_ABSOLUTESTART), BM_SETCHECK,
                (WPARAM)BST_UNCHECKED, (LPARAM)0);
  } else {
    SendMessage(GetControl(IDC_ABSOLUTESTART), BM_SETCHECK, (WPARAM)BST_CHECKED,
                (LPARAM)0);
    SendMessage(GetControl(IDC_RELATIVESTART), BM_SETCHECK,
                (WPARAM)BST_UNCHECKED, (LPARAM)0);
  }
}
...
void CBaseEventPropertiesDialog::DrawSpline(HDC hdc, HWND placeholder,
                                            CChoreoEvent *e) {
  RECT rcOut;
