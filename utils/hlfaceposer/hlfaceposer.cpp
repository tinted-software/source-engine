...
// Output : int
//-----------------------------------------------------------------------------
int GetCloseCaptionLanguageId() { return g_viewerSettings.cclanguageid; }
  ...
  if (changed || force) {
    ...
    if (Q_stricmp(suffix, "unknown_language")) {
      ...
      if (filesystem->FileExists(fn)) {
        ...
        Con_ErrorPrintf("PhonemeEditor::SetCloseCaptionLanguageId  Warning, "
                        "can't find localization file %s\n",
                        fn);
      }
...
void MakeFileWriteable(const char *filename) {
  ...
  if (!Q_IsAbsolutePath(filename)) {
    pFullPath = (char *)filesystem->RelativePathToFullPath(
        filename, NULL, pFullPathBuf, sizeof(pFullPathBuf));
  } else {
...
bool MakeFileWriteablePrompt(const char *filename, char const *promptTitle) {
  if (!IsFileWriteable(filename)) {
    int retval = mxMessageBox(
        NULL, va("File '%s' is Read-Only, make writable?", filename),
...
void FPCopyFile(const char *source, const char *dest, bool bCheckOut) {
  ...
  if (!Q_IsAbsolutePath(source)) {
    filesystem->RelativePathToFullPath(source, NULL, fullpaths,
                                       sizeof(fullpaths));
  } else {
...
void FacePoser_MakeToolWindow(mxWindow *w, bool smallcaption) {
  FacePoser_AddWindowStyle(w, WS_VISIBLE | WS_OVERLAPPEDWINDOW |
...
void FacePoser_LoadWindowPositions(char const *name, bool &visible, int &x,
                                   int &y, int &w, int &h, bool &locked,
                                   bool &zoomed) {
  char subkey[512];
  int v;
...
void FacePoser_SaveWindowPositions(char const *name, bool visible, int x, int y,
                                   int w, int h, bool locked, bool zoomed) {
  char subkey[512];
  Q_snprintf(subkey, sizeof(subkey), "%s - visible", name);
...
void FacePoser_EnsurePhonemesLoaded(void) {
  ...
  for (int i = 0; i < ARRAYSIZE(ext); ++i) {
    ...
    if (!expressions->FindClass(clname, false)) {
      ...
      Q_snprintf(clfile, sizeof(clfile), "expressions/%sphonemes%s.txt",
                 g_PhonemeRoot, ext[i]);
      Q_FixSlashes(clfile);
      Q_strlower(clfile);
...
bool FacePoser_ShowFileNameDialog(bool openFile, char *relative, size_t bufsize,
                                  char const *subdir, char const *wildcard) {
  Assert(relative);
  relative[0] = 0;
  Assert(subdir);
  Assert(wildcard);
  ...
  if (openFile) {
    fullpath =
        mxGetOpenFileName(0,
                          inWorkingDirectoryAlready
                              ? "."
                              : FacePoser_MakeWindowsSlashes(
                                    va("%s%s/", GetGameDirectory(), subdir)),
                          wildcard);
  } else {
    fullpath =
        mxGetSaveFileName(0,
...
bool FacePoser_ShowOpenFileNameDialog(char *relative, size_t bufsize,
                                      char const *subdir,
                                      char const *wildcard) {
  return FacePoser_ShowFileNameDialog(true, relative, bufsize, subdir,
                                      wildcard);
}

bool FacePoser_ShowSaveFileNameDialog(char *relative, size_t bufsize,
                                      char const *subdir,
                                      char const *wildcard) {
  return FacePoser_ShowFileNameDialog(false, relative, bufsize, subdir,
                                      wildcard);
}
...
int ConvertUnicodeToANSI(const wchar_t *unicode, char *ansi,
                         int ansiBufferSize) {
  return ::WideCharToMultiByte(CP_ACP, 0, unicode, -1, ansi, ansiBufferSize,
                               NULL, NULL);
}
...
char const *FacePoser_DescribeSnappedTime(float t) {
  ...
  if (fracpart == 0) {
    ...
    Q_snprintf(desc, sizeof(desc), "frame %i (time %i + %i/%i s.)", frame,
               ipart, fracpart, fps);
  }
...
char const *FacePoser_TranslateSoundName(char const *soundname,
  ...
  if (model) {
    Q_strncpy(temp,
              PSkipSoundChars(soundemitter->GetWavFileForSound(
                  soundname, model->GetFileName())),
              sizeof(temp));
  } else {
    Q_strncpy(
        temp,
        PSkipSoundChars(soundemitter->GetWavFileForSound(soundname, NULL)),
        sizeof(temp));
  }
  return temp;
}
...
char const *FacePoser_TranslateSoundName(CChoreoEvent *event) {
  ...
  Q_strncpy(temp,
            PSkipSoundChars(soundemitter->GetWavFileForSound(
                soundname, model ? model->GetFileName() : NULL)),
            sizeof(temp));
  return temp;
}
