...
MatSysWindow::MatSysWindow(mxWindow *parent, int x, int y, int w, int h,
    ...
    : IFacePoserToolWindow("3D View", "3D View"),
  ...
  g_materialBackground = g_pMaterialSystem->FindMaterial(
      "particle/particleapp_background", TEXTURE_GROUP_OTHER, true);
  ...
  g_materialBones = g_pMaterialSystem->FindMaterial("debug/debugmrmwireframe",
...
void MatSysWindow::draw() {
  ...
  pRenderContext->LoadIdentity();
  pRenderContext->PerspectiveX(20.0f, (float)w2() / (float)viewh, 1.0f,
                               20000.0f);
...
void MatSysWindow::EnableStickySnapshotMode() { m_stickyDepth++; }
...
void MatSysWindow::TakeSnapshotRect(const char *pFilename, int x, int y, int w,
                                    int h) {
  int i;
  HANDLE hf;
  ...
  imageSize = w * h * 3;
  // Create the file
  hf = CreateFile(pFilename, GENERIC_READ | GENERIC_WRITE, 0, NULL,
  ...
  if (!WriteFile(hf, (LPVOID)&bi, sizeof(BITMAPINFOHEADER), (LPDWORD)&dwTmp,
                 NULL))
...
//-----------------------------------------------------------------------------
bool MatSysWindow::IsSuppressingResize(void) { return m_bSuppressResize; }
