...
//-----------------------------------------------------------------------------
static BOOL CALLBACK ExpressionPropertiesDialogProc(HWND hwndDlg, UINT uMsg,
                                                    WPARAM wParam,
                                                    LPARAM lParam) {
  switch (uMsg) {
  case WM_INITDIALOG: {
  ...
  case WM_COMMAND:
    ...
    case IDOK:
      ...
      GetDlgItemText(hwndDlg, IDC_EXPRESSIONDESC, g_Params.m_szDescription,
                     256);
      EndDialog(hwndDlg, 1);
      break;
    case IDCANCEL:
...
int ExpressionProperties(CExpressionParams *params) {
  ...
  int retval = DialogBox(
      (HINSTANCE)GetModuleHandle(0), MAKEINTRESOURCE(IDD_EXPRESSIONPROPERTIES),
