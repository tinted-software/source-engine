...
public:
  CBaseMainPanel(Panel *parent, const char *panelName)
...
int CVGuiWnd::GetVGuiContext() { return m_hVGuiContext; }
...
void CVGuiWnd::SetMainPanel(vgui::EditablePanel *pPanel) {
  ...
  vgui::ivgui()->AssociatePanelWithContext(m_hVGuiContext,
                                           m_pMainPanel->GetVPanel());
}
...
vgui::EditablePanel *CVGuiWnd::GetMainPanel() { return m_pMainPanel; }

mxWindow *CVGuiWnd::GetParentWnd() { return m_pParentWnd; }

void CVGuiWnd::SetRepaintInterval(int msecs) { m_pParentWnd->setTimer(msecs); }
...
void CVGuiWnd::DrawVGuiPanel() {
  ...
  pRenderContext->Viewport(rect.left, rect.top, rect.right - rect.left,
  ...
  if (w != rect.right || h != rect.bottom) {
    m_pMainPanel->SetBounds(2 + rect.left, 2 + rect.top,
...
void CVGuiPanelWnd::redraw() { DrawVGuiPanel(); }
...
int CVGuiWnd::HandeEventVGui(mxEvent *event) {
  ...
  /*
          ...
          case WM_KILLFOCUS:
                  {
                          // restore normal arrow cursor when mouse leaves VGUI
     panel SetCursor( vgui::dc_arrow ); break;
                  }
