...
float TimelineItem::GetTimeForMouse(int mx, bool clip /*= false*/) {
  ...
  float frac = (float)(mx - m_rcBounds.left) /
...
CExpressionSample *TimelineItem::GetSampleUnderMouse(
    int mx, int my, float tolerance /*= FP_TL_SELECTION_TOLERANCE*/) {
  CFlexAnimationTrack *track = GetSafeTrack();
  if (!track)
    return NULL;
...
int TimelineItem::handleEvent(mxEvent *event) {
  ...
  case mxEvent::KeyDown: {
    ...
    case 'V':
      ...
      break;
    case 'J': {
      g_pExpressionTool->OnCopyToFlex(g_pExpressionTool->GetScrubberSceneTime(),
                                      true);
    } break;
    case 'K': {
      g_pExpressionTool->OnCopyFromFlex(
          g_pExpressionTool->GetScrubberSceneTime(), false);
    } break;
  ...
  case mxEvent::MouseDown: {
    ...
    if (m_nDragging == DRAGTYPE_NONE) {
      ...
      CExpressionSample *sample = GetSampleUnderMouse(
          event->x, event->y,
          ctrlDown ? FP_TL_ADDSAMPLE_TOLERANCE : FP_TL_SELECTION_TOLERANCE);
      ...
      if (IsMouseOverGrowHandle((short)event->x, (short)event->y)) {
        ...
        if (event->modifiers & mxEvent::KeyShift) {
          ...
          m_nDragging = rightbutton ? DRAGTYPE_MOVEPOINTS_TIME
...
void TimelineItem::MouseDrag(int x, int y, int modifiers,
                             bool snap /*=false*/) {
  if (m_nDragging == DRAGTYPE_NONE)
    return;
  ...
  if (!IsCollapsed() && track) {
    if (m_nEditType == 1) {
      ...
      drawHelper.DrawColoredLine(
          RGB(180, 200, 220), PS_SOLID, 1, rcClient.left,
    ...
    for (int type = (track->IsComboType() ? 1 : 0); type >= 0; type--) {
      ...
      COLORREF dotColorSelected =
          (type == m_nEditType) ? RGB(240, 80, 20) : gray;

      int height = rcClient.bottom - rcClient.top;
      int bottom = rcClient.bottom;

      // Fixme, could look at 1st derivative and do more sampling at high rate
      // of change? or near actual sample points!
      float linelength = g_pExpressionTool->IsFocusItem(this) ? 2.0f : 8.0f;

      float timestepperpixel =
          linelength / g_pExpressionTool->GetPixelsPerSecond();
      ...
      if (type == m_nEditType)
      {
              ...
              for ( float t = starttime; t <= stoptime; t += timestepperpixel )
              {
                      ...
                      float value = Hermite_Spline( i0, i1, i2, (t - time10hz +
      0.1) / 0.1 );
                      ...
                      {
                              // Draw segment
                              //drawHelper.DrawColoredLine( lineColor, PS_SOLID,
      1,
              ...
              if ( segments.Count() >= 2  )
              {
                      drawHelper.DrawColoredPolyLine( shadowColor, PS_SOLID, 1,
      segments );
              }
      ...
      */
      ...
      for (int sample = 0; sample < track->GetNumSamples(type); sample++) {
        ...
        */
        float value =
            start->value; // track->GetFracIntensity( start->time, type );
        bool clipped = false;
...
void TimelineItem::DrawRelativeTags(CChoreoWidgetDrawHelper &drawHelper) {
    ...
    for (int j = 0; j < a->GetNumChannels(); j++) {
      ...
      for (int k = 0; k < c->GetNumEvents(); k++) {
        ...
        for (int t = 0; t < e->GetNumRelativeTags(); t++) {
          ...
          bool clipped = false;
          int tagx = GetMouseForTime(
              tag->GetStartTime() - event->GetStartTime(), &clipped);
          if (clipped)
            continue;

          drawHelper.DrawColoredLine(RGB(180, 180, 220), PS_SOLID, 1, tagx,
                                     rcClient.top, tagx, rcClient.bottom);
        }
      }
    }
  }
  ...
  for (int t = 0; t < event->GetNumTimingTags(); t++) {
    ...
    bool clipped = false;
    int tagx =
        GetMouseForTime(tag->GetStartTime() - event->GetStartTime(), &clipped);
    if (clipped)
      continue;

    // Draw relative tag marker
    drawHelper.DrawColoredLine(RGB(220, 180, 180), PS_SOLID, 1, tagx,
                               rcClient.top, tagx, rcClient.bottom);
  }
}
...
void TimelineItem::DrawEventEnd(CChoreoWidgetDrawHelper &drawHelper) {
  ...
  drawHelper.DrawColoredLine(COLOR_CHOREO_ENDTIME, PS_SOLID, 1, leftx,
                             m_rcBounds.top, leftx, m_rcBounds.bottom);
}
...
//			handleRect -
//-----------------------------------------------------------------------------
void TimelineItem::DrawGrowHandle(CChoreoWidgetDrawHelper &helper,
                                  RECT &handleRect) {
