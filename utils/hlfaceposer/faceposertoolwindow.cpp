...
IFacePoserToolWindow::IFacePoserToolWindow(char const *toolname,
                                           char const *displaynameroot) {
...
void IFacePoserToolWindow::ComputeNewTitle(void) {
  Q_snprintf(m_szWindowTitle, sizeof(m_szWindowTitle), "%s%s%s", m_szPrefix,
             m_szDisplayRoot, m_szSuffix);
  if (GetMxWindow()) {
...
static bool TranslateToolPos(char const *toolname, int workspacew,
                             int workspaceh, int &x, int &y, int &w, int &h,
                             bool &locked) {
...
void IFacePoserToolWindow::LoadPosition(void) {
  ...
  FacePoser_LoadWindowPositions(GetToolName(), visible, x, y, w, h, locked,
                                zoomed);
  ...
  if (w == 0 || h == 0) {
    ...
    bool translated = TranslateToolPos(GetToolName(), g_MDLViewer->w2(),
                                       g_MDLViewer->h2(), x, y, w, h, locked);
    if (!translated) {
      ++s_nToolCount;
      visible = false;
    }
...
void IFacePoserToolWindow::SavePosition(void) {
  ...
  FacePoser_SaveWindowPositions(GetToolName(), visible, xpos, ypos, width,
                                height, IsLocked(), false);
}

int IFacePoserToolWindow::GetToolCount(void) { return g_Tools.Count(); }
...
bool IFacePoserToolWindow::HandleToolEvent(mxEvent *event) {
  ...
  case mxEvent::MouseUp: {
    ...
    if (!isup && w) {
      SetFocus((HWND)w->getHandle());
      SetWindowPos((HWND)w->getHandle(), HWND_TOP, 0, 0, 0, 0,
                   SWP_NOMOVE | SWP_NOSIZE | SWP_SHOWWINDOW);
...
void IFacePoserToolWindow::HandleToolRedraw(CChoreoWidgetDrawHelper &helper) {
  ...
  COLORREF textColor =
      GetSysColor(COLOR_MENUTEXT); // GetSysColor( COLOR_INACTIVECAPTIONTEXT );
  ...
  //  helper.DrawColoredText( "Small Fonts", 9, FW_NORMAL, textColor, rcFrame,
  //  va( "%i", m_nToolFrameCount ) );

  lockRect.bottom++;
  OffsetRect(&lockRect, 1, 1);
  helper.DrawColoredTextCharset("Marlett", 8, FW_NORMAL, SYMBOL_CHARSET,
                                textColor, lockRect, "v");

  closeRect.bottom++;
  helper.DrawOutlinedRect(textColor, PS_SOLID, 1, closeRect);
  OffsetRect(&closeRect, 1, 1);
  helper.DrawColoredTextCharset("Marlett", 8, FW_NORMAL, SYMBOL_CHARSET,
                                textColor, closeRect, "r");
...
//-----------------------------------------------------------------------------
bool IFacePoserToolWindow::GetAutoProcess(void) const { return m_bAutoProcess; }
