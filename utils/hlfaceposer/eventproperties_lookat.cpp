...
void CEventPropertiesLookAtDialog::SetupLookAtControls(CEventParams *params) {
  ...
  SendMessage(control, BM_SETCHECK,
              (WPARAM)params->usepitchyaw ? BST_CHECKED : BST_UNCHECKED, 0);
...
void CEventPropertiesLookAtDialog::ShowControlsForEventType(
    CEventParams *params) {
...
//-----------------------------------------------------------------------------
static BOOL CALLBACK EventPropertiesLookAtDialogProc(HWND hwndDlg, UINT uMsg,
                                                     WPARAM wParam,
                                                     LPARAM lParam) {
  return g_EventPropertiesLookAtDialog.HandleMessage(hwndDlg, uMsg, wParam,
                                                     lParam);
};

BOOL CEventPropertiesLookAtDialog::HandleMessage(HWND hwndDlg, UINT uMsg,
  ...
  case WM_COMMAND:
    ...
    case IDOK: {
      ...
      if (!g_Params.m_szName[0]) {
        Q_snprintf(g_Params.m_szName, sizeof(g_Params.m_szName), "Look at %s",
  ...
  case WM_HSCROLL: {
    ...
    if (control == GetControl(IDC_SLIDER_YAW) ||
        control == GetControl(IDC_SLIDER_PITCH)) {
      g_Params.yaw =
          (float)SendMessage(GetControl(IDC_SLIDER_YAW), TBM_GETPOS, 0, 0);
      g_Params.pitch =
          (float)SendMessage(GetControl(IDC_SLIDER_PITCH), TBM_GETPOS, 0, 0);
      ...
      SendMessage(control, BM_SETCHECK,
                  (WPARAM)g_Params.usepitchyaw ? BST_CHECKED : BST_UNCHECKED,
                  0);
