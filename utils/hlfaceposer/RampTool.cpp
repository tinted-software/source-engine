...
RampTool::RampTool(mxWindow *parent)
  ...
  m_pHorzScrollBar = new mxScrollbar(this, 0, 0, 18, 100, IDC_RAMPHSCROLL,
...
RampTool::~RampTool(void) { delete m_pHelper; }
...
//			rcHandle -
//-----------------------------------------------------------------------------
void RampTool::DrawScrubHandle(CChoreoWidgetDrawHelper &drawHelper,
...
//			cursor -
//-----------------------------------------------------------------------------
void RampTool::StartDragging(int dragtype, int startx, int starty,
                             HCURSOR cursor) {
  ...
  case mxEvent::MouseDown: {
    ...
    if (m_nDragType == DRAGTYPE_NONE) {
      ...
      CExpressionSample *sample = GetSampleUnderMouse(
          event->x, event->y,
          ctrlDown ? FP_RT_ADDSAMPLE_TOLERANCE : FP_RT_SELECTION_TOLERANCE);
...
void RampTool::OnUndo(void) { g_pChoreoView->Undo(); }

void RampTool::OnRedo(void) { g_pChoreoView->Redo(); }
...
//			rcPos -
//-----------------------------------------------------------------------------
void RampTool::DrawMouseOverPos(CChoreoWidgetDrawHelper &drawHelper,
                                RECT &rcPos) {
...
//			right -
//-----------------------------------------------------------------------------
void RampTool::DrawTimeLine(CChoreoWidgetDrawHelper &drawHelper, RECT &rc,
...
void RampTool::DrawTimingTags(CChoreoWidgetDrawHelper &drawHelper, RECT &rc) {
  ...
  for (i = 0; i < c; i++) {
    ...
    if (e->GetNumRelativeTags() > 0) {
      DrawRelativeTagsForEvent(drawHelper, rc, rampevent, e, starttime,
                               endtime);
    }
    if (e->GetNumAbsoluteTags(CChoreoEvent::PLAYBACK) > 0) {
      DrawAbsoluteTagsForEvent(drawHelper, rc, rampevent, e, starttime,
                               endtime);
    }
  }
}
...
//			&rc -
//-----------------------------------------------------------------------------
void RampTool::DrawAbsoluteTagsForEvent(CChoreoWidgetDrawHelper &drawHelper,
                                        RECT &rc, CChoreoEvent *rampevent,
...
//			rc -
//-----------------------------------------------------------------------------
void RampTool::DrawRelativeTagsForEvent(CChoreoWidgetDrawHelper &drawHelper,
                                        RECT &rc, CChoreoEvent *rampevent,
                                        CChoreoEvent *event, float starttime,
                                        float endtime) {
  if (!event)
...
void RampTool::DrawSamplesSimple(CChoreoWidgetDrawHelper &drawHelper,
                                 CChoreoEvent *e, bool clearbackground,
...
void RampTool::DrawSamples(CChoreoWidgetDrawHelper &drawHelper,
                           RECT &rcSamples) {
  drawHelper.DrawFilledRect(RGB(230, 230, 215), rcSamples);
...
CExpressionSample *
RampTool::GetSampleUnderMouse(int mx, int my,
                              float tolerance /*= FP_RT_SELECTION_TOLERANCE*/) {
  CChoreoEvent *e = GetSafeEvent();
  if (!e)
    return NULL;
...
// Output : int
//-----------------------------------------------------------------------------
int RampTool::CountSelected(void) { return m_pHelper->CountSelected(false); }
...
void RampTool::OnModelChanged() { redraw(); }
