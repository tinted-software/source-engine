...
void CChoreoGlobalEventWidget::redraw(CChoreoWidgetDrawHelper &drawHelper) {
  ...
  if (IsSelected()) {
    drawHelper.DrawColoredLine(clr, PS_DOT, 2, rcLine.left, rcLine.top,
                               rcLine.right, rcLine.bottom);
  } else {
    drawHelper.DrawColoredLine(clr, PS_DOT, 1, rcLine.left, rcLine.top,
                               rcLine.right, rcLine.bottom);
  }
  ...
  if (event->GetType() == CChoreoEvent::STOPPOINT) {
    ...
    if (image) {
      ...
      DrawBitmapToDC(drawHelper.GrabDC(), rcTab.left, rcTab.top, 16, 16,
                     *image);
    }
  }
  ...
  drawHelper.DrawColoredLine(clr, PS_SOLID, 1, rcLine.left, rcLine.top,
                             rcLine.right, rcLine.top + 28);

  DrawLabel(drawHelper, labelText, rcLine.left, rcLine.top + 2, true);
}

void CChoreoGlobalEventWidget::DrawLabel(CChoreoWidgetDrawHelper &drawHelper,
  ...
  int len = drawHelper.CalcTextWidth("Arial", 9, FW_NORMAL,
                                     va("%s", event->GetName()));
  ...
  if (!right) {
    drawHelper.DrawColoredTextCharset("Marlett", 9, FW_NORMAL, SYMBOL_CHARSET,
                                      clr, rcText, "3");
...
// Output : CChoreoEvent
//-----------------------------------------------------------------------------
CChoreoEvent *CChoreoGlobalEventWidget::GetEvent(void) { return m_pEvent; }
