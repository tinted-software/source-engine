...
public:
  CUtlSymbolTree(mxWindow *parent, int id = 0)
...
public:
  ...
  CVCDOptionsWindow(CVCDBrowser *browser)
...
// Purpose:
//-----------------------------------------------------------------------------
void CVCDBrowser::OnDelete() { RemoveAllSounds(); }
        ...
        if (m_pListView->getNumSelected() == 1) {
          ...
          if (index >= 0) {
            FileNameHandle_t vcd =
                (FileNameHandle_t)m_pListView->getUserData(index, 0);
            OpenVCD(vcd);
          }
        }
      }
...
bool CVCDBrowser::CNameLessFunc::Less(const FileNameHandle_t &name1,
                                      const FileNameHandle_t &name2,
                                      void *pContext) {
  if (name1 < name2)
    return true;
  return false;
...
void CVCDBrowser::OpenVCD(const FileNameHandle_t &handle) {
  ...
  if (filesystem->String(handle, fn, sizeof(fn))) {
    ...
    const char *pFileName = filesystem->RelativePathToFullPath(
        fn, "GAME", pFullPath, sizeof(pFullPath));
    if (!pFileName) {
      pFileName = fn;
    }
...
//-----------------------------------------------------------------------------
bool CVCDBrowser::LoadVCDsFilesInDirectory(
    CUtlSortVector<FileNameHandle_t, CNameLessFunc> &soundlist,
  ...
  while (pFileName) {
    if (!filesystem->FindIsDirectory(findHandle)) {
      ...
      Q_snprintf(pFileNameWithPath, nAllocSize, "%s/%s",
                 &pDirectoryName[SCENES_PREFIX_LEN], pFileName);
...
bool CVCDBrowser::InitDirectoryRecursive(
    CUtlSortVector<FileNameHandle_t, CNameLessFunc> &soundlist,
...
int CVCDBrowser::GetVCDCount() const { return m_AllVCDs.Count(); }
...
void CVCDBrowser::SetCurrent(char const *filename) {
  ...
  for (i = 0; i < c; ++i) {
    FileNameHandle_t vcd =
        (FileNameHandle_t)(m_pListView->getUserData(i, COL_VCD));
