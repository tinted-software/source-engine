...
void CEventPropertiesFaceDialog::ShowControlsForEventType(
    CEventParams *params) {
...
//-----------------------------------------------------------------------------
static BOOL CALLBACK EventPropertiesFaceDialogProc(HWND hwndDlg, UINT uMsg,
                                                   WPARAM wParam,
                                                   LPARAM lParam) {
  return g_EventPropertiesFaceDialog.HandleMessage(hwndDlg, uMsg, wParam,
                                                   lParam);
};

BOOL CEventPropertiesFaceDialog::HandleMessage(HWND hwndDlg, UINT uMsg,
  ...
  case WM_COMMAND:
    ...
    case IDOK: {
      ...
      if (!g_Params.m_szName[0]) {
        Q_snprintf(g_Params.m_szName, sizeof(g_Params.m_szName), "Face %s",
    ...
    case IDC_CHECK_LOCKBODYFACING: {
      g_Params.m_bLockBodyFacing =
          SendMessage(GetControl(IDC_CHECK_LOCKBODYFACING), BM_GETCHECK, 0,
                      0) == BST_CHECKED
              ? true
              : false;
    } break;
...
int EventProperties_Face(CEventParams *params) {
  ...
  int retval = DialogBox(
      (HINSTANCE)GetModuleHandle(0), MAKEINTRESOURCE(IDD_EVENTPROPERTIES_FACE),
