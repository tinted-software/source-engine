...
void CEventPropertiesGenericDialog::InitControlData(CEventParams *params) {
  ...
  SendMessage(GetControl(IDC_FILENAME), WM_SETTEXT,
              sizeof(params->m_szParameters), (LPARAM)params->m_szParameters);
...
void CEventPropertiesGenericDialog::PopulateAIGeneric(HWND control,
                                                      CEventParams *params) {
...
void CEventPropertiesGenericDialog::ShowControlsForEventType(
    CEventParams *params) {
...
//-----------------------------------------------------------------------------
static BOOL CALLBACK EventPropertiesGenericDialogProc(HWND hwndDlg, UINT uMsg,
                                                      WPARAM wParam,
                                                      LPARAM lParam) {
  return g_EventPropertiesGenericDialog.HandleMessage(hwndDlg, uMsg, wParam,
                                                      lParam);
};

BOOL CEventPropertiesGenericDialog::HandleMessage(HWND hwndDlg, UINT uMsg,
                                                  WPARAM wParam,
                                                  LPARAM lParam) {
  m_hDialog = hwndDlg;
  ...
  case WM_COMMAND:
    ...
    case IDOK: {
      ...
      if (!g_Params.m_szName[0]) {
        Q_snprintf(g_Params.m_szName, sizeof(g_Params.m_szName), "%s to %s",
                   g_Params.m_szParameters, g_Params.m_szParameters2);
      }
