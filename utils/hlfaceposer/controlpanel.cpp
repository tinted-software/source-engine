...
ControlPanel::ControlPanel(mxWindow *parent)
    : IFacePoserToolWindow("ControlPanel", "Control Panel"),
      mxWindow(parent, 0, 0, 0, 0), tab(0) {
  // create tabcontrol with subdialog windows
  tab = new mxTab(this, 0, 0, 0, 0, IDC_TAB);

  CControlPanelTabWindow *wRender =
      new CControlPanelTabWindow(this, 0, 0, 0, 0);
  ...
  cbAllWindowsDriveSpeech = new mxCheckBox(
      wRender, 220, 45, 140, 20, "All tools drive mouth", IDC_TOOLSDRIVEMOUTH);
  cbAllWindowsDriveSpeech->setChecked(
  ...
  cbMovement =
      new mxCheckBox(wRender, 110, 25, 100, 20, "Movement", IDC_MOVEMENT);
  cbMovement->setEnabled(false);
  cbBackground =
      new mxCheckBox(wRender, 110, 45, 100, 20, "Background", IDC_BACKGROUND);
  ...
  mxCheckBox *cbAttachments =
      new mxCheckBox(wRender, 5, 45, 100, 20, "Attachments", IDC_ATTACHMENTS);
  cbAttachments->setEnabled(false);

  CControlPanelTabWindow *wSequence =
      new CControlPanelTabWindow(this, 0, 0, 0, 0);
...
bool ControlPanel::CanClose(void) {
  ...
  // Now close them all, or abort exit if user doesn't want to close any that
  // have changed
  return Closeall();
}
...
int ControlPanel::handleEvent(mxEvent *event) {
  ...
  case mxEvent::Action: {
    ...
    case IDC_SPEEDSCALE: {
      ...
      lSpeedScale->setLabel(
          va("Speed scale %.2f", g_viewerSettings.speedScale));
    } break;
...
void ControlPanel::setSubmodel(int index) {
  models->GetActiveStudioModel()->SetBodygroup(cBodypart->getSelectedIndex(),
                                               index);
}
...
void ControlPanel::setBoneControllerValue(int index, float value) {
  ...
  if (hdr) {
    ...
    models->GetActiveStudioModel()->SetController(pbonecontrollers->inputfield,
                                                  value);
  }
}
...
void ControlPanel::setModelInfo() {
  ...
  sprintf(str,
          ...
          "Sequences: %d\n",
          hdr->numbones(), hdr->numbonecontrollers(), hbcount,
          hdr->numhitboxsets(), hdr->GetNumSeq());
  ...
  sprintf(str,
          ...
          "Bodyparts: %d\n"
          "Attachments: %d\n",
          hdr->numtextures(), hdr->numskinfamilies(), hdr->numbodyparts(),
...
void ControlPanel::CenterOnFace(void) {
  ...
  if (seqKeyValues->LoadFromBuffer(mdl->GetFileName(),
                                   mdl->GetKeyValueText(seq))) {
...
// Output : float
//-----------------------------------------------------------------------------
float ControlPanel::GetModelGap(void) { return slModelGap->getValue(); }
...
// Purpose:
//-----------------------------------------------------------------------------
void ControlPanel::Copy(void) { g_pFlexPanel->CopyControllerSettings(); }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void ControlPanel::Paste(void) { g_pFlexPanel->PasteControllerSettings(); }
