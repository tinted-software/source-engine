...
SceneRampTool::SceneRampTool(mxWindow *parent)
    : IFacePoserToolWindow("SceneRampTool", "Scene Ramp"),
  ...
  m_pHorzScrollBar = new mxScrollbar(this, 0, 0, 18, 100, IDC_SRT_RAMPHSCROLL,
...
SceneRampTool::~SceneRampTool(void) { delete m_pHelper; }
...
// Input  : rcHandle -
//-----------------------------------------------------------------------------
void SceneRampTool::GetScrubHandleRect(RECT &rcHandle, float scrub,
                                       bool clipped) {
  float pixel = 0.0f;
  if (w2() > 0) {
...
//			rcHandle -
//-----------------------------------------------------------------------------
void SceneRampTool::DrawScrubHandle(CChoreoWidgetDrawHelper &drawHelper,
                                    RECT &rcHandle, float scrub,
                                    bool reference) {
...
void SceneRampTool::redraw() {
  ...
  if (scene) {
    ...
    drawHelper.DrawColoredText("Arial", 11, 900, RGB(200, 0, 0), rcTextLine,
                               "Scene:  %s", g_pChoreoView->GetChoreoFile());
...
void SceneRampTool::ShowContextMenu(mxEvent *event, bool include_track_menus) {
  ...
  if (total > 0) {
    if (current > 0) {
      pop->add(va("Undo %s", g_pChoreoView->GetUndoDescription()),
               IDC_UNDO_SRT);
    }

    if (current <= total - 1) {
      pop->add(va("Redo %s", g_pChoreoView->GetRedoDescription()),
               IDC_REDO_SRT);
    }
    pop->addSeparator();
  }
...
//			cursor -
//-----------------------------------------------------------------------------
void SceneRampTool::StartDragging(int dragtype, int startx, int starty,
                                  HCURSOR cursor) {
  ...
  case mxEvent::MouseDown: {
    ...
    if (m_nDragType == DRAGTYPE_NONE) {
      ...
      CExpressionSample *sample = GetSampleUnderMouse(
          event->x, event->y,
          ctrlDown ? FP_SRT_ADDSAMPLE_TOLERANCE : FP_SRT_SELECTION_TOLERANCE);
      ...
      if (IsMouseOverScrubHandle(event)) {
        ...
        if (sample) {
          if (shiftdown) {
            ...
            StartDragging(
                rightbutton ? DRAGTYPE_MOVEPOINTS_TIME
            ...
            StartDragging(DRAGTYPE_SELECTION, m_nClickedX, m_nClickedY,
...
void SceneRampTool::OnUndo(void) { g_pChoreoView->Undo(); }

void SceneRampTool::OnRedo(void) { g_pChoreoView->Redo(); }
...
//			rcPos -
//-----------------------------------------------------------------------------
void SceneRampTool::DrawMouseOverPos(CChoreoWidgetDrawHelper &drawHelper,
                                     RECT &rcPos) {
...
//			right -
//-----------------------------------------------------------------------------
void SceneRampTool::DrawTimeLine(CChoreoWidgetDrawHelper &drawHelper, RECT &rc,
                                 float left, float right) {
  RECT rcLabel;
  float granularity = 0.5f;

  drawHelper.DrawColoredLine(RGB(150, 150, 200), PS_SOLID, 1, rc.left,
                             rc.top + 2, rc.right, rc.top + 2);
  ...
  while (f < right) {
    ...
    if (frac >= 0.0f && frac <= 1.0f) {
      ...
      drawHelper.DrawColoredText("Arial", 9, FW_NORMAL, RGB(0, 50, 150), rcOut,
                                 sz);
    }
    f += granularity;
  }
...
void SceneRampTool::DrawTimingTags(CChoreoWidgetDrawHelper &drawHelper,
                                   RECT &rc) {
  CChoreoScene *scene = GetSafeScene();
  if (!scene)
    return;
...
//			&rc -
//-----------------------------------------------------------------------------
void SceneRampTool::DrawAbsoluteTagsForEvent(
    CChoreoWidgetDrawHelper &drawHelper, RECT &rc, CChoreoEvent *event,
  ...
  for (int i = 0; i < event->GetNumAbsoluteTags(CChoreoEvent::PLAYBACK); i++) {
    ...
    float tagtime =
        (event->GetStartTime() + tag->GetPercentage() * event->GetDuration());
    ...
    drawHelper.DrawColoredText("Arial", 9, FW_NORMAL, clr, rcText,
                               tag->GetName());
  }
}
...
//			rc -
//-----------------------------------------------------------------------------
void SceneRampTool::DrawRelativeTagsForEvent(
    CChoreoWidgetDrawHelper &drawHelper, RECT &rc, CChoreoEvent *event,
    float starttime, float endtime) {
  if (!event)
    return;

  // drawHelper.DrawColoredText( "Arial", 9, FW_NORMAL, PEColor(
  // COLOR_PHONEME_TIMING_TAG ), rc, "Timing Tags:" );
...
void SceneRampTool::RepositionHSlider(void) {
  ...
  m_pHorzScrollBar->setBounds(0, h2() - m_nScrollbarHeight,
                              w2() - m_nScrollbarHeight, m_nScrollbarHeight);
...
void SceneRampTool::DrawSceneEnd(CChoreoWidgetDrawHelper &drawHelper) {
  ...
  drawHelper.DrawColoredLine(COLOR_CHOREO_ENDTIME, PS_SOLID, 1, leftx,
                             rcSample.top, leftx, rcSample.bottom);
}
...
void SceneRampTool::DrawSamplesSimple(CChoreoWidgetDrawHelper &drawHelper,
                                      CChoreoScene *scene, bool clearbackground,
  ...
  for (float x = rcSamples.left; x < rcSamples.right; x += 3) {
    ...
    // Draw segment
    drawHelper.DrawColoredLine(lineColor, PS_SOLID, 1, prev_x,
...
void SceneRampTool::DrawSamples(CChoreoWidgetDrawHelper &drawHelper,
                                RECT &rcSamples) {
  drawHelper.DrawFilledRect(RGB(230, 230, 215), rcSamples);
  ...
  for (int sample = 0; sample < rampCount; sample++) {
    ...
    */
    ...
    char sz[128];
    Q_snprintf(sz, sizeof(sz), "%s",
               Interpolator_NameForCurveType(start->GetCurveType(), true));
    RECT rc;
    int fontSize = 9;
...
void SceneRampTool::DrawAutoHighlight(mxEvent *event) {
  ...
  // Fixme, could look at 1st derivative and do more sampling at high rate of
  // change? or near actual sample points!
...
CExpressionSample *SceneRampTool::GetSampleUnderMouse(
    int mx, int my, float tolerance /*= FP_SRT_SELECTION_TOLERANCE*/) {
  CChoreoScene *s = GetSafeScene();
  if (!s)
    return NULL;
...
void SceneRampTool::SelectPoints(void) {
  ...
  // If they are just clicking and releasing in one spot, capture any items w/in
  // a larger tolerance
  if (selW <= 2 && selH <= 2) {
    tolerance = FP_SRT_SELECTION_TOLERANCE;

    CExpressionSample *sample = GetSampleUnderMouse(
        rcSelection.left + selW * 0.5f, rcSelection.top + selH * 0.5f);
    if (sample) {
...
void SceneRampTool::OnModelChanged() { redraw(); }
...
void SceneRampTool::GetWorkList(bool reflect,
                                CUtlVector<SceneRampTool *> &list) {
  NOTE_UNUSED(reflect);
  list.AddToTail(this);
}
