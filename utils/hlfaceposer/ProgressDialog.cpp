...
public:
  ...
  static BOOL CALLBACK ProgressDialogProc(HWND hwndDlg, UINT uMsg,
                                          WPARAM wParam, LPARAM lParam);

private:
  BOOL ProgressDialogProcImpl(HWND hwndDlg, UINT uMsg, WPARAM wParam,
                              LPARAM lParam);
...
CProgressDialog::CProgressDialog()
    : m_flFraction(0.0f), m_hwndDlg(0), m_bShowCancel(false),
      m_bWantsCancel(false), m_flStartTime(0.0f) {}

bool CProgressDialog::IsCancelled() { return m_bShowCancel && m_bWantsCancel; }
...
//-----------------------------------------------------------------------------
BOOL CALLBACK CProgressDialog::ProgressDialogProc(HWND hwndDlg, UINT uMsg,
                                                  WPARAM wParam,
                                                  LPARAM lParam) {
...
// Output : int
//-----------------------------------------------------------------------------
void CProgressDialog::Start(char const *pchTitle, char const *pchText,
                            bool bShowCancel) {
  if (m_hwndDlg) {
    Finish();
  }
  ...
  m_hwndDlg =
      CreateDialog((HINSTANCE)GetModuleHandle(0), MAKEINTRESOURCE(IDD_PROGRESS),
...
void CProgressDialog::UpdateText(char const *pchFmt, ...) {
  ...
  SetDlgItemText(m_hwndDlg, IDC_FP_PROGRESS_TEXT,
                 CFmtStr("%s", m_sStatus.String()));
  SetDlgItemText(m_hwndDlg, IDC_FP_PROGRESS_PERCENT,
                 CFmtStr("%.2f %%", m_flFraction * 100.0f));
...
BOOL CProgressDialog::ProgressDialogProcImpl(HWND hwndDlg, UINT uMsg,
                                             WPARAM wParam, LPARAM lParam) {
  switch (uMsg) {
  case WM_INITDIALOG:
    ...
    // ...
    {
      ...
      ShowWindow(GetDlgItem(hwndDlg, IDCANCEL),
                 m_bShowCancel ? SW_SHOW : SW_HIDE);
