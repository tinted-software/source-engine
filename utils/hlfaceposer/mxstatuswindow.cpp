...
mxStatusWindow::mxStatusWindow(mxWindow *parent, int x, int y, int w, int h,
                               const char *label /*= 0*/)
    : mxWindow(parent, x, y, w, h, label),
      IFacePoserToolWindow("Status Window", "Output"), m_pScrollbar(NULL) {
  ...
  m_pScrollbar = new mxScrollbar(this, 0, 0, STATUS_SCROLLBAR_SIZE, 100,
                                 IDC_STATUS_SCROLL, mxScrollbar::Vertical);
...
void mxStatusWindow::redraw() {
  ...
  for (int i = 0; i < MAX_TEXT_LINES - offset; i++) {
    ...
    helper.DrawColoredText("Arial", STATUS_FONT_SIZE, FW_NORMAL,
                           RGB(255, 255, 150), rcTime, sz);

    rcTime = rcText;
    rcTime.left += 50;

    helper.DrawColoredText("Arial", STATUS_FONT_SIZE, FW_NORMAL,
                           m_rgTextLines[line].rgb, rcTime, ptext);
...
void mxStatusWindow::StatusPrint(COLORREF clr, bool overwrite,
                                 const char *text) {
...
int mxStatusWindow::handleEvent(mxEvent *event) {
  ...
  default:
    break;
  case mxEvent::Size: {
    m_pScrollbar->setBounds(w2() - STATUS_SCROLLBAR_SIZE, GetCaptionHeight(),
                            STATUS_SCROLLBAR_SIZE, h2() - GetCaptionHeight());
    PositionSliders(0);
    m_pScrollbar->setValue(m_pScrollbar->getMaxValue());
    iret = 1;
  } break;
...
void mxStatusWindow::DrawActiveTool() {
  ...
  sprintf(sz, "%s (%i) at %.3f (%.2f fps) (soundcount %i)",
          ...
          g_MDLViewer->GetCurrentFrame(), (float)realtime, fps,
          models->CountActiveSources());

  lastrealtime = realtime;

  int len =
      CChoreoWidgetDrawHelper::CalcTextWidth("Courier New", 10, FW_NORMAL, sz);

  CChoreoWidgetDrawHelper helper(this, rcTool, RGB(32, 0, 0));

  rcTool.left = rcTool.right - len - 15;

  helper.DrawColoredText("Courier New", 10, FW_NORMAL, RGB(255, 255, 200),
                         rcTool, sz);
}
