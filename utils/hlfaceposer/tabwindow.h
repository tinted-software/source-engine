...
public:
  ...
  CTabWindow(mxWindow *parent, int x, int y, int w, int h, int id = 0,
             int style = 0);
...
protected:
  ...
  virtual void DrawTab(CChoreoWidgetDrawHelper &drawHelper, RECT &rcClient,
                       int tabnum, bool selected = false);

  int RecomputeLayout(int windowWidth, bool dolayout = true);

  class CETItem {
  public:
    enum { MAX_ET_STRING_LENGTH = 64 };
