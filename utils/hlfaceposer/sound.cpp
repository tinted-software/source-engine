...
extern "C" void CALLBACK WaveData(HWAVEIN hwi, UINT uMsg,
                                  CAudioWaveInput *pAudio, DWORD dwParam1,
                                  DWORD dwParam2);
...
CAudioWaveInput::~CAudioWaveInput(void) {
  if (ValidDevice()) {
    ...
    for (int i = 0; i < INPUT_BUFFER_COUNT; i++) {
      if (m_buffers[i]) {
        waveInUnprepareHeader(m_deviceHandle, m_buffers[i],
                              sizeof(*m_buffers[i]));
...
void CALLBACK WaveData(HWAVEIN hwi, UINT uMsg, CAudioWaveInput *pAudio,
...
void CAudioWaveInput::WaveMessage(HWAVEIN hdevice, UINT uMsg, DWORD dwParam1,
                                  DWORD dwParam2) {
...
void CAudioWaveInput::OpenDevice(void) {
  ...
  MMRESULT errorCode =
      waveInOpen(&m_deviceHandle, m_deviceId, &format, (DWORD)WaveData,
                 (DWORD)this, CALLBACK_FUNCTION);
  if (errorCode == MMSYSERR_NOERROR) {
    // valid device opened
    int bufferSize = m_sampleSize * m_sampleRate /
                     INPUT_BUFFER_COUNT; // total of one second of data
...
#define WAVE_FORMAT_STEREO                                                     \
  (WAVE_FORMAT_1S08 | WAVE_FORMAT_1S16 | WAVE_FORMAT_2S08 | WAVE_FORMAT_2S16 | \
...
void CAudioDeviceSWMix::Mix8Mono(channel_t *pChannel, char *pData,
...
void CAudioDeviceSWMix::Mix8Stereo(channel_t *pChannel, char *pData,
                                   int outputOffset, int inputOffset,
                                   int rateScaleFix, int outCount,
                                   int timecompress, bool forward) {
  int sampleIndex = 0;
...
void CAudioDeviceSWMix::Mix16Mono(channel_t *pChannel, short *pData,
                                  int outputOffset, int inputOffset,
                                  int rateScaleFix, int outCount,
                                  int timecompress, bool forward) {
  int sampleIndex = 0;
...
void CAudioDeviceSWMix::Mix16Stereo(channel_t *pChannel, short *pData,
                                    int outputOffset, int inputOffset,
                                    int rateScaleFix, int outCount,
                                    int timecompress, bool forward) {
  int sampleIndex = 0;
...
void CAudioDeviceSWMix::TransferBufferStereo16(short *pOutput,
                                               int sampleCount) {
...
void CAudioWaveOutput::AddToReferencedList(CAudioMixer *mixer,
                                           CAudioBuffer *buffer) {
  // Already in list
...
void CAudioWaveOutput::RemoveFromReferencedList(CAudioMixer *mixer,
                                                CAudioBuffer *buffer) {
  for (int i = 0; i < buffer->m_Referenced.Size(); i++) {
    if (buffer->m_Referenced[i].mixer == mixer) {
...
bool CAudioWaveOutput::IsSoundInReferencedList(CAudioMixer *mixer,
                                               CAudioBuffer *buffer) {
  for (int i = 0; i < buffer->m_Referenced.Size(); i++) {
    if (buffer->m_Referenced[i].mixer == mixer) {
      return true;
...
CAudioWaveOutput::~CAudioWaveOutput(void) {
  if (ValidDevice()) {
    ...
    for (int i = 0; i < OUTPUT_BUFFER_COUNT; i++) {
      if (m_buffers[i].hdr) {
        waveOutUnprepareHeader(m_deviceHandle, m_buffers[i].hdr,
                               sizeof(*m_buffers[i].hdr));
    ...
    while (tempCount > 0) {
      ...
      for (int i = 0; i < MAX_CHANNELS; i++) {
        ...
        if (modelindex >= 0) {
          ...
          if (g_pPhonemeEditor->IsActiveTool() ||
              g_pWaveBrowser->IsActiveTool()) {
        ...
        if (model && !model->m_mouth.IsSourceReferenced(pSource->GetSource())) {
          ...
          bool bIgnorePhonemes =
              pScene ? pScene->ShouldIgnorePhonemes() : false;
        ...
        if (pSource->GetActive()) {
          if (!pSource->MixDataToDevice(&m_audioDevice, pSource->GetChannel(),
          ...
          if (!IsSourceReferencedByActiveBuffer(pSource)) {
            ...
            pSource->IncrementSamples(pSource->GetChannel(), currentsample,
                                      sampleCount, SampleRate(), forward);
          }
        }
      }
...
void CAudioWaveOutput::OpenDevice(void) {
  ...
  MMRESULT errorCode = waveOutOpen(&m_deviceHandle, WAVE_MAPPER, &waveFormat, 0,
                                   0, CALLBACK_NULL);
  if (errorCode == MMSYSERR_NOERROR) {
    int bufferSize = 4 * (OUTPUT_SAMPLE_RATE /
                          OUTPUT_BUFFER_COUNT); // total of 1 second of data
    ...
    for (int i = 0; i < OUTPUT_BUFFER_COUNT; i++) {
      ...
      if (waveOutPrepareHeader(m_deviceHandle, m_buffers[i].hdr,
                               sizeof(*m_buffers[i].hdr)) != MMSYSERR_NOERROR) {
        ClearDevice();
        return;
      }
    }
...
public:
  ...
  void PlaySound(StudioModel *source, float volume, const char *wavfile,
                 CAudioMixer **ppMixer);
  void PlaySound(CAudioSource *source, float volume, CAudioMixer **ppMixer);
  void PlayPartialSound(StudioModel *model, float volume, const char *wavfile,
                        CAudioMixer **ppMixer, int startSample, int endSample);
  ...
  void RenderWavToDC(HDC dc, RECT &outrect, COLORREF clr, float starttime,
                     float endtime, CAudioSource *pWave, bool selected = false,
...
private:
  ...
  void ProcessCloseCaptionData(StudioModel *model, float curtime,
                               CSentence *sentence);
...
void CFacePoserSound::Init(void) {
  ...
  if (filesystem->LoadKeyValues(*manifest, IFileSystem::TYPE_SOUNDEMITTER,
                                "scripts/game_sounds_manifest.txt", "GAME")) {
...
void CFacePoserSound::PlaySound(StudioModel *model, float volume,
                                const char *wavfile, CAudioMixer **ppMixer) {
  if (m_pAudio) {
...
void CFacePoserSound::PlayPartialSound(StudioModel *model, float volume,
                                       const char *wavfile,
                                       CAudioMixer **ppMixer, int startSample,
                                       int endSample) {
  if (!m_pAudio)
    return;
...
void CFacePoserSound::PlaySound(CAudioSource *source, float volume,
                                CAudioMixer **ppMixer) {
  if (ppMixer) {
    *ppMixer = NULL;
  }
...
void CFacePoserSound::AddViseme(float intensity, StudioModel *model,
                                int phoneme, float scale) {
  int i;
      ...
      if (t > 0.0f) {
        for (int w = 0; w < sentence->m_Words.Size(); w++) {
          ...
          for (int k = 0; k < word->m_Phonemes.Size(); k++) {
            ...
            // if the filter starts within this phoneme, make sure the filter
            // size is at least least as long as the current phoneme, or until
            // the end of the next phoneme, whichever is smaller
            if (t > phoneme->GetStartTime() && t < phoneme->GetEndTime()) {
              ...
              } else if (w < sentence->m_Words.Size() - 1 &&
                         sentence->m_Words[w + 1]->m_Phonemes.Size()) {
              ...
              if (next) {
                ...
                if (next->GetStartTime() == phoneme->GetEndTime()) {
                  // no gap, so increase the blend length to the end of the next
                  // phoneme, as long as it's not longer than the current
                  // phoneme
                  dt = max(
                      dt, min(next->GetEndTime() - t,
                              phoneme->GetEndTime() - phoneme->GetStartTime()));
                } else {
                  // dead space, so increase the blend length to the start of
                  // the next phoneme, as long as it's not longer than the
                  // current phoneme
                  dt = max(
                      dt, min(next->GetStartTime() - t,
                              phoneme->GetEndTime() - phoneme->GetStartTime()));
                }
              } else {
                // last phoneme in list, increase the blend length to the length
            ...
            if (t1 < 1.0 && t2 > 0) {
              ...
              AddViseme(emphasis_intensity, model, phoneme->GetPhonemeCode(),
                        scale);
            }
          }
        }
...
void CFacePoserSound::ProcessCloseCaptionData(StudioModel *model, float curtime,
                                              CSentence *sentence) {
  //	closecaptionmanager->Process( g_nSoundFrameCount, model, curtime,
  //sentence, GetCloseCaptionLanguageId() );
}
...
void CFacePoserSound::RenderWavToDC(HDC dc, RECT &outrect, COLORREF clr,
  ...
  float timeperpixel =
      (endtime - starttime) / (float)(outrect.right - outrect.left);
  ...
  float selectionstarttime =
      pWave->GetRunningLength() * (float)selectionstart / (float)totalsamples;
  float selectionendtime =
      pWave->GetRunningLength() * (float)selectionend / (float)totalsamples;
  ...
  while (currenttime < endtime) {
    ...
    if (diff > 0) {
      if (!pMixer->SkipSamples(&channel, currentsample, diff,
                               pWave->SampleRate(), true))
        break;
    }
