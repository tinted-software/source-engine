...
mxExpressionSlider::mxExpressionSlider(mxWindow *parent, int x, int y, int w,
                                       int h, int id)
...
void mxExpressionSlider::setRange(int barnum, float min, float max,
                                  int ticks /*= 100*/) {
  m_flMin[barnum] = min;
  m_flMax[barnum] = max;
...
void mxExpressionSlider::GetThumbRect(int barnum, RECT &rcThumb) {
  ...
  int leftedge =
      rcBar.left + (int)((float)(rcBar.right - rcBar.left) * (float)(tickmark) /
                         (float)m_nTicks[barnum]);
...
void mxExpressionSlider::DrawTitle(HDC &dc) {
  ...
  DrawText(dc, sz, -1, &rc,
           DT_NOPREFIX | DT_VCENTER | DT_SINGLELINE | DT_LEFT |
               DT_WORD_ELLIPSIS);
...
void mxExpressionSlider::MoveThumb(int barnum, int xpos, bool finish) {
  ...
  if (xpos < rcBar.left) {
    ...
    m_flCurrent[barnum] =
        m_flMin[barnum] + ((float)curtick / (float)m_nTicks[0]) *
                              (m_flMax[barnum] - m_flMin[barnum]);
  }
...
int mxExpressionSlider::handleEvent(mxEvent *event) {
  ...
  case mxEvent::MouseDown: {
    ...
    if (!m_bDraggingThumb) {
      ...
      m_nCurrentBar = (event->buttons & mxEvent::MouseRightButton)
                          ? BALANCE_BAR
                          : MAGNITUDE_BAR;
