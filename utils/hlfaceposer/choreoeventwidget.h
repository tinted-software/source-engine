...
public:
  ...
  virtual void redrawStatus(CChoreoWidgetDrawHelper &drawHelper,
                            RECT &rcClient);
  ...
  // If the user changes the association of .mdls to actors, then the gender
  // could change and we could need to access a different .wav file Call this to
  // reconcile things
  void RecomputeWave();
...
private:
  ...
  void DrawRelativeTags(CChoreoWidgetDrawHelper &drawHelper, RECT &rcWAV,
                        float length, CChoreoEvent *event);
  void DrawAbsoluteTags(CChoreoWidgetDrawHelper &drawHelper, RECT &rcWAV,
                        float length, CChoreoEvent *event);
