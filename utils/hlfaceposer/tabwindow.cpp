...
//			style -
//-----------------------------------------------------------------------------
CTabWindow::CTabWindow(mxWindow *parent, int x, int y, int w, int h,
...
//-----------------------------------------------------------------------------
CTabWindow::~CTabWindow(void) { removeAll(); }
...
//			selected -
//-----------------------------------------------------------------------------
void CTabWindow::DrawTab(CChoreoWidgetDrawHelper &drawHelper, RECT &rcClient,
                         int tabnum, bool selected) {
  RECT rcTab;
  ...
  COLORREF hilightcolor =
      m_Colors[selected ? COLOR_HILITE_SELECTED : COLOR_HILITE];
  ...
  // Draw label
  drawHelper.DrawColoredText("Arial", 9, FW_NORMAL, text, rcTab, "%s%s",
                             getPrefix(tabnum), getLabel(tabnum));
}
...
void CTabWindow::redraw(void) {
  ...
  drawHelper.DrawColoredLine(m_Colors[COLOR_HILITE], PS_SOLID, 1, 0, liney, w(),
                             liney);
  RECT rc;
...
int CTabWindow::GetItemUnderMouse(int mx, int my) {
  ...
  for (int i = 0; i < m_Items.Size(); i++) {
    ...
    if (mx < rcTab.left || mx > rcTab.right || my < rcTab.top ||
        my > rcTab.bottom) {
      continue;
    }
    return i;
...
// Output : int
//-----------------------------------------------------------------------------
int CTabWindow::getItemCount() const { return m_Items.Size(); }
...
int CTabWindow::RecomputeLayout(int windowWidth, bool dolayout /*=true*/) {
  ...
  for (int i = 0; i < m_Items.Size(); i++) {
    ...
    int textwidth =
        CChoreoWidgetDrawHelper::CalcTextWidth("Arial", 9, FW_NORMAL, "%s%s",
                                               p->m_szPrefix, p->m_szString) +
        15;
