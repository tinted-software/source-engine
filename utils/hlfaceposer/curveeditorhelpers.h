...
int CCurveEditorHelper<T>::GetBestCurveTypeForSelectedSamples(bool reflect) {
  ...
  for (int i = counts.FirstInorder(); i != counts.InvalidIndex();
       i = counts.NextInorder(i)) {
    if (counts[i] > maxType) {
      maxCount = counts[i];
...
template <class T> int CCurveEditorHelper<T>::CountSelected(bool reflect) {
  int numSelected = 0;
...
template <class T>
void CCurveEditorHelper<T>::ChangeCurveType(bool forward, bool shiftdown,
                                            bool altdown) {
...
template <class T>
void CCurveEditorHelper<T>::SetCurveTypeForSelectedSamples(bool reflect,
                                                           int curvetype) {
  int numSelected = CountSelected(reflect);
  if (!numSelected)
    return;
...
template <class T>
void CCurveEditorHelper<T>::SetCurveTypeForSample(int curvetype,
...
void CCurveEditorHelper<T>::ToggleHoldTypeForSelectedSamples(bool reflect) {
  ...
  for (int w = 0; w < workList.Count(); ++w) {
    ...
    for (int i = 0; i < numSamples; ++i) {
      ...
      int newCurveType =
          MAKE_CURVE_TYPE(l, newValue == 1 ? INTERPOLATE_HOLD : l);
      ...
      if (event->key >= '0' && event->key <= '9') {
        ...
        if (curveType >= 0) {
          if (CountSelected(shiftdown) <= 0) {
            ...
            CExpressionSample *hover =
                GetOuter()->GetSampleUnderMouse(event->x, event->y, 0.0f);
