...
static void InsertTextColumn(HWND listcontrol, int column, int width,
                             char const *label) {
  LVCOLUMN col;
...
static void PopulateCloseCaptionTokenList(HWND wnd,
  ...
  while (INVALID_LOCALIZE_STRING_INDEX != i) {
    ...
    SendMessage(control, LVM_INSERTITEMW, 0,
    ...
    SendMessage(control, LVM_SETITEMW, 0,
                (LPARAM)(const LV_ITEMW FAR *)(&lvItem));

    if (!Q_stricmp(name, params->m_szCCToken)) {
      ListView_SetItemState(control, lvItem.iItem, LVIS_SELECTED,
                            LVIS_STATEIMAGEMASK);
...
//-----------------------------------------------------------------------------
static BOOL CALLBACK CloseCaptionLookupDialogProc(HWND hwndDlg, UINT uMsg,
                                                  WPARAM wParam,
                                                  LPARAM lParam) {
  switch (uMsg) {
  case WM_INITDIALOG:
  ...
  case WM_COMMAND:
    ...
    case IDOK: {
      /*
      //int selindex = SendMessage( GetDlgItem( hwndDlg, IDC_CCTOKENLIST ),
      LB_GETCURSEL, 0, 0 ); if ( selindex == LB_ERR )
      {
              mxMessageBox( NULL, "You must select an entry from the list",
      g_appTitle, MB_OK ); return TRUE;
      }
      */

      SendMessage(GetDlgItem(hwndDlg, IDC_CCTOKEN), WM_GETTEXT,
                  (WPARAM)sizeof(g_Params.m_szCCToken),
      ...
      if (hdr->code == LVN_ITEMCHANGED) {
        ...
        if (item >= 0) {
          ...
          if (ListView_GetItem(control, &lvi)) {
            ...
            if (name) {
              Q_strncpy(g_Params.m_szCCToken, name,
                        sizeof(g_Params.m_szCCToken));
...
int CloseCaptionLookup(CCloseCaptionLookupParams *params) {
  ...
  g_UnicodeFont = CreateFont(-10, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE,
                             ANSI_CHARSET, OUT_TT_PRECIS, CLIP_DEFAULT_PRECIS,
                             ANTIALIASED_QUALITY, DEFAULT_PITCH, "Tahoma");

  int retval = DialogBox(
      (HINSTANCE)GetModuleHandle(0), MAKEINTRESOURCE(IDD_CCLOOKUP),
      (HWND)g_MDLViewer->getHandle(), (DLGPROC)CloseCaptionLookupDialogProc);
