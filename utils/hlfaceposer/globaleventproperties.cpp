...
static void CreateAutoStateFromControls(CGlobalEventParams *params) {
  if (params->m_bAutomate) {
    sprintf(params->m_szAction, "automate %s %f", params->m_szType,
            params->m_flWaitTime);
  } else {
...
//-----------------------------------------------------------------------------
static BOOL CALLBACK GlobalEventPropertiesDialogProc(HWND hwndDlg, UINT uMsg,
                                                     WPARAM wParam,
                                                     LPARAM lParam) {
  switch (uMsg) {
  case WM_INITDIALOG: {
    ...
    SendMessage(GetDlgItem(hwndDlg, IDC_CB_AUTOACTION), WM_SETTEXT, 0,
                (LPARAM)g_Params.m_szType);
    // add text to combo box
    SendMessage(GetDlgItem(hwndDlg, IDC_CB_AUTOACTION), CB_ADDSTRING, 0,
                (LPARAM) "Cancel");
    SendMessage(GetDlgItem(hwndDlg, IDC_CB_AUTOACTION), CB_ADDSTRING, 0,
                (LPARAM) "Resume");
  ...
  case WM_COMMAND:
    ...
    case IDOK: {
      ...
      SendMessage(GetDlgItem(hwndDlg, IDC_CB_AUTOACTION), WM_GETTEXT,
                  (WPARAM)sizeof(g_Params.m_szType), (LPARAM)g_Params.m_szType);

      GetDlgItemText(hwndDlg, IDC_DURATION, szTime, sizeof(szTime));
      g_Params.m_flWaitTime = atof(szTime);

      g_Params.m_bAutomate =
          SendMessage(GetDlgItem(hwndDlg, IDC_CHECK_AUTOCHECK), BM_GETCHECK, 0,
                      0) == BST_CHECKED
              ? true
              : false;
      ...
      GetDlgItemText(hwndDlg, IDC_EVENTNAME, g_Params.m_szName,
...
int GlobalEventProperties(CGlobalEventParams *params) {
  ...
  int retval = DialogBox(
      (HINSTANCE)GetModuleHandle(0), MAKEINTRESOURCE(IDD_GLOBALEVENTPROPERTIES),
