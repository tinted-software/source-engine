...
#define BUGSUB_MOUNT_COMMAND                                                   \
  "mount_smbfs //guest:@bugbait.valvesoftware.com/bugsub " BUGSUB_MOUNT
#else
// We can't do sudo here, so we rely on /etc/fstab being setup for
// '/mnt/bugsub'. See comment in CBugReporter::Init().
...
bool CBugReporter::Init(CreateInterfaceFn engineFactory) {
  ...
  if (_stat(BUGSUB_CONFIG, &mount_info)) {
    ...
    if (_stat(BUGSUB_CONFIG, &mount_info)) {
      ...
      ConColorMsg(clr, "ERROR: failed to mount '" BUGSUB_MOUNT
                       "' with '" BUGSUB_MOUNT_COMMAND "'.\n");
      ConColorMsg(clr,
                  "Bugsub not set up yet? Do 'sudo apt-get install smbfs', "
                  "'sudo mkdir /mnt/bugsub', and add this /etc/fstab:\n");
      ConColorMsg(clr, "  bugbait.valvesoftware.com/bugsub /mnt/bugsub smbfs "
                       "rw,user,username=guest,password=,noauto 0 0\n");
      return false;
    }
#endif
...
#ifdef WIN32
  V_strncpy(m_BugRootDirectory, m_OptionsFile->GetString("bug_directory", "."),
            sizeof(m_BugRootDirectory));
#elif defined(OSX)
  V_strncpy(m_BugRootDirectory,
            m_OptionsFile->GetString("bug_directory_osx", BUGSUB_MOUNT),
            sizeof(m_BugRootDirectory));
#elif defined(LINUX)
  V_strncpy(m_BugRootDirectory,
            m_OptionsFile->GetString("bug_directory_linux", BUGSUB_MOUNT),
            sizeof(m_BugRootDirectory));
#else
#error
#endif
...
void CBugReporter::StartNewBugReport() {
  ...
  do {
    ...
    Q_snprintf(m_CurrentBugDirectory, sizeof(m_CurrentBugDirectory),
               "%s%c%04i%02i%02i-%02i%02i%02i-%s", m_BugRootDirectory,
               CORRECT_PATH_SEPARATOR, t.tm_year + 1900, t.tm_mon + 1,
...
bool CBugReporter::CommitBugReport(int &bugSubmissionId) {
  ...
  Q_snprintf(szBugFileName, sizeof(szBugFileName), "%s%cbug.txt",
             m_CurrentBugDirectory, CORRECT_PATH_SEPARATOR);
