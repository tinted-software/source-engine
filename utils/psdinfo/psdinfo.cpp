...
int ReadInfo() {
  ...
  if (descr.m_pvData) {
    unsigned char const *pvData = descr.m_pvData,
                        *pvEnd = pvData + descr.m_numBytes;
    while (unsigned char const *pvCR =
               (unsigned char const *)memchr(pvData, '\r', pvEnd - pvData)) {
      printf("%.*s\n", pvCR - pvData, pvData);
      pvData = pvCR + 1;
    }
...
int WriteInfo() {
  ...
  if (!LoadFileAndClose(fopen(s_opts.szFilename, "rb"), bufFile,
                        inBuf.TellPut() +
                            0x100)) // Having extra room for insertions
    Error("%s cannot be opened for read!\n", s_opts.szFilename);
  ...
  Wr_PSDResFileInfo resFileInfo(
      PSDResFileInfo(imgres.FindElement(PSDImageResources::eResFileInfo)));

  // Maybe the file info is missing?
  if (!resFileInfo.m_res.m_pvData) {
    // Insert a standard file info
    unsigned char chStdFileInfo[] = {0x38, 0x42, 0x49, 0x4D, 0x04, 0x04, 0,
                                     0,    0,    0,    0x00, 0x07, 0x1C, 0x02,
                                     0x00, 0x00, 0x02, 0x00, 0x02, 0};
    ...
    BufferMove(imgres.m_pvBuffer + addSize, imgres.m_pvBuffer,
               pvBufBase + bufFile.TellPut() - imgres.m_pvBuffer);
  ...
  if (descr.m_pvData) {
    ...
    if (numDeltaBytes)
      BufferMove(
          descr.m_pvData + inBuf.TellPut(), descr.m_pvData + descr.m_numBytes,
    ...
    unsigned char const *pvInsertPoint =
        resFileInfo.m_res.m_pvData + resFileInfo.m_res.m_numBytes;
    BufferMove(pvInsertPoint + numDeltaBytes, pvInsertPoint,
               pvBufBase + bufFile.TellPut() - pvInsertPoint);
  ...
  if (numDeltaBytes & 1) // Odd number of bytes delta
  {
    if (resFileInfo.m_res.m_numBytes & 1) // It was even, becomes odd
    {
      ...
      BufferMove(resFileInfo.m_res.m_pvData + resFileInfo.m_res.m_numBytes + 1,
                 resFileInfo.m_res.m_pvData + resFileInfo.m_res.m_numBytes,
                 pvBufBase + bufFile.TellPut() - resFileInfo.m_res.m_pvData -
                     resFileInfo.m_res.m_numBytes);
      const_cast<unsigned char &>(
          resFileInfo.m_res.m_pvData[resFileInfo.m_res.m_numBytes]) = 0x00;
    ...
    } else // It was odd, becomes even
    {
      ...
      BufferMove(resFileInfo.m_res.m_pvData + resFileInfo.m_res.m_numBytes,
                 ...
                 pvBufBase + bufFile.TellPut() - resFileInfo.m_res.m_pvData -
                     resFileInfo.m_res.m_numBytes - 1);
      bufFile.SeekPut(CUtlBuffer::SEEK_CURRENT, -1);
    }
  }

  ((unsigned short *)resFileInfo.m_res.m_pvData)[-1] =
      BigShort(resFileInfo.m_res.m_numBytes);
