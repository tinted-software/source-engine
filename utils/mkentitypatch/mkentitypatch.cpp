...
void CMkEntityPatchApp::PrintHelp() {
  Msg("Usage: mkentitypatch [-nop4] [-vproject <path to gameinfo.txt>] <in "
      ".bsp file>\n");
  Msg("\t-nop4\t: [Optional] Disables auto perforce checkout/add.\n");
  Msg("\t-vproject\t: [Optional] Specifies path to a gameinfo.txt file (which "
      "mod to build for).\n");
...
entity_t *FindEntity(KeyValues *pEntity) {
  ...
  // Unfortunately, hammmerid appears to be a relatively new feature. Now, we
  // must look for target name
  int nMatch = -1;
  ...
  if (pTargetName && pTargetName[0]) {
    ...
    for (int i = 0; i < num_entities; ++i) {
      ...
      if (!V_stricmp(pTargetName, pMatchTargetName)) {
        if (nMatch >= 0) {
          // Warning( "Encountered multiple entities that matched targetname
          // %s!\n", pTargetName ); return false;
  ...
  if (pClassName && pClassName[0]) {
    ...
    for (int i = 0; i < num_entities; ++i) {
      ...
      if (nMatch >= 0) {
        Warning("Encountered multiple entities that matched classname %s, "
                "origin %s!\n",
                pClassName, pOrigin);
        return false;
      }
...
bool InsertEntity(entity_t *pEntity, KeyValues *pEntityKeys) {
  ...
  for (KeyValues *pKey = pEntityKeys->GetFirstValue(); pKey;
  ...
  for (KeyValues *pKey = pEntityKeys->GetFirstTrueSubKey(); pKey;
...
bool ReplaceEntity(KeyValues *pEntity) {
  ...
  if (!pReplace) {
    Warning("Tried to replace an entity %s, origin %s, but couldn't find the "
            "original!\n",
...
int CMkEntityPatchApp::Main() {
  ...
  GenerateLumpFileName(pFullPath, pOutputFileName, sizeof(pOutputFileName),
                       LUMP_ENTITIES);
  ...
  for (KeyValues *pKey = pKeyValues->GetFirstTrueSubKey(); pKey;
       pKey = pKey->GetNextTrueSubKey()) {
  ...
  UnparseEntities();
  WriteLumpToFile(pBSPFileName, LUMP_ENTITIES, 0, dentdata.Base(),
                  dentdata.Count());
