...
private:
  ...
  void UpdateVcd(const char *pFullPath, const VcdUpdateInfo_t &info,
                 studiohdr_t *pStudioHdr, float flWavDuration);
  bool UpdateVcd(CChoreoScene *pScene, const VcdUpdateInfo_t &info,
                 studiohdr_t *pStudioHdr, float flWavDuration);
};
...
void CVcdUpdateApp::PrintHelp() {
  Msg("Usage: vcdupdate [-w <modified .wav file>] [-m <modified .mdl file>] "
      "[-e] [-n]\n");
  Msg("				[-nop4] [-vproject <path to gameinfo.txt>]\n");
  Msg("vcdupdate will fixup all vcd files in the tree to account for other "
      "asset changes.\n");
  ...
  Msg("\t-e\t: [Optional] Displays a warning about all .vcds that don't have "
      "actors w/ associated .mdls\n");
  Msg("\t\tSuch files cannot be auto-updated by vcdupdate and also can "
      "generate bugs.\n");
  Msg("\t-n\t: [Optional] Displays a warning about all .vcds whose actor "
      "names\n");
...
//-----------------------------------------------------------------------------
bool CVcdUpdateApp::UpdateVcd(CChoreoScene *pScene, const VcdUpdateInfo_t &info,
                              studiohdr_t *pStudioHdr, float flWavDuration) {
  ...
  if (pStudioHdr) {
    g_pFullFileSystem->FullPathToRelativePathEx(info.m_pChangedMDLFile, "GAME",
                                                pModelPath, sizeof(pModelPath));
  }
  ...
  for (int i = 0; i < c; i++) {
    ...
    case CChoreoEvent::SPEAK: {
      ...
      Q_ComposeFileName("sound", pWavFile, pRelativeWAVFile,
                        sizeof(pRelativeWAVFile));

      char pFullWAVFile[MAX_PATH];
      g_pFullFileSystem->RelativePathToFullPath(
          pRelativeWAVFile, "GAME", pFullWAVFile, sizeof(pFullWAVFile));
    ...
    case CChoreoEvent::SEQUENCE: {
      ...
      if (UpdateSequenceLength(pEvent, &studioHdr, pPoseParameters, false,
                               false)) {
        bChanged = true;
      }
    } break;
...
// Checks a single VCD to see if it needs to be updated or not.
//-----------------------------------------------------------------------------
void CVcdUpdateApp::UpdateVcd(const char *pFullPath,
                              const VcdUpdateInfo_t &info,
                              studiohdr_t *pStudioHdr, float flWavDuration) {
  CUtlBuffer buf;
  ...
  CChoreoScene *pScene =
      ChoreoLoadScene(pFullPath, NULL, GetTokenProcessor(), NULL);
  if (!pScene) {
    ...
    for (int i = 0; i < nActorCount; ++i) {
      ...
      if (info.m_bWarnNotMatchingMDL) {
        ...
        if (!StringHasPrefix(pActorName, pBaseModelName)) {
          Warning("\t*** File \"%s\": Actor name and .mdl name suspiciously "
                  "different:\n\t\tMDL \"%s\"\tActor \"%s\"\n",
...
//-----------------------------------------------------------------------------
// Loads up the changed files and builds list of vcds to convert, then converts
// them
//-----------------------------------------------------------------------------
...
int CVcdUpdateApp::Main() {
  ...
  if (info.m_pChangedMDLFile) {
    ...
    if (!Q_IsAbsolutePath(info.m_pChangedMDLFile)) {
      Q_ComposeFileName("models", info.m_pChangedMDLFile, pRelativeMDLPath,
                        sizeof(pRelativeMDLPath));
      g_pFullFileSystem->RelativePathToFullPath(
          pRelativeMDLPath, "GAME", pFullMDLPath, sizeof(pFullMDLPath));
    }
  ...
  if (info.m_pChangedWAVFile) {
    ...
    if (!Q_IsAbsolutePath(info.m_pChangedWAVFile)) {
      Q_ComposeFileName("sound", info.m_pChangedWAVFile, pRelativeWAVPath,
                        sizeof(pRelativeWAVPath));
      g_pFullFileSystem->RelativePathToFullPath(
          pRelativeWAVPath, "GAME", pFullWAVPath, sizeof(pFullWAVPath));
    }
