...
void CBSPPack::AddFileToPack(const char *relativename, const char *fullpath) {
  // Compressing at this point would work, but the usual usage is creating a BSP
  // and using RepackBSP() to apply lump and pack compression as a final pass
...
bool CBSPPack::RepackBSP(CUtlBuffer &inputBuffer, CUtlBuffer &outputBuffer,
                         eRepackBSPFlags repackFlags) {
  return ::RepackBSP(
      inputBuffer, outputBuffer,
      (repackFlags & eRepackBSP_CompressLumps) ? RepackBSPCallback_LZMA : NULL,
      (repackFlags & eRepackBSP_CompressPackfile)
