...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR szCmdLine,
                   int iCmdShow) {
  return g_MaterialSystemApp.WinMain(hInstance, hPrevInstance, szCmdLine,
                                     iCmdShow);
}
...
// This function builds a list the screen resolutions supported by the display
// driver
...
bool MSA_IsKeyDown(char key) { return !!(GetAsyncKeyState(key) & 0x8000); }
...
MaterialSystemApp::MaterialSystemApp() { Clear(); }

MaterialSystemApp::~MaterialSystemApp() { Term(); }
...
int MaterialSystemApp::WinMain(void *hInstance, void *hPrevInstance,
...
long MaterialSystemApp::WndProc(void *inhwnd, long iMsg, long wParam,
                                long lParam) {
  if (inhwnd != m_hWnd) {
...
bool MaterialSystemApp::InitMaterialSystem() {
  ...
  if (clientFactory) {
    m_pMaterialSystem = (IMaterialSystem *)clientFactory(
        MATERIAL_SYSTEM_INTERFACE_VERSION, NULL);
    if (!m_pMaterialSystem) {
      return Sys_Error("Could not get the material system interface from "
                       "materialsystem.dll");
    }
  } else {
    return Sys_Error(
  ...
  // BUGBUG: Can't figure out why this is broken.  EXECUTABLE_PATH should
  // already be there but it isn't so the shader system is computing texture
  // memory on each run...
  g_pFullFileSystem->AddSearchPath("U:\\main\\game\\bin", "EXECUTABLE_PATH",
                                   PATH_ADD_TO_TAIL);
  ...
  if (!m_pMaterialSystem->Init(pShaderDLL, &g_MatSysAppMaterialProxyFactory,
...
bool MaterialSystemApp::CreateMainWindow(int width, int height, int bpp,
                                         bool fullscreen) {
  HWND hwnd;
  WNDCLASSEX wndclass;
  ...
  if (fullscreen) {
    ...
    dwExStyle = 0;
    // dwStyle = WS_CAPTION | WS_SYSMENU | WS_THICKFRAME;  // Use this if you
    // want a "normal" window
    dwStyle = WS_CAPTION;
...
void MaterialSystemApp::GetParameters() {
  ...
  // Make a copy of the command line to count the parameters - strtok is
  // destructive
...
void MaterialSystemApp::MakeWindowTopmost() {
  ::SetWindowPos((HWND)m_hWnd, HWND_TOPMOST, 0, 0, 0, 0,
