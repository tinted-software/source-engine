...
public:
  ...
  int WinMain(void *hInstance, void *hPrevInstance, char *szCmdLine,
              int iCmdShow);
...
// ----------------------------------------------------------------------------------------
// // g_szAppName[]			-- char array applicaton name void
// AppInit( void )		-- Called at init time void AppRender( void )
// -- Called each frame (as often as possible) void AppExit( void )
// -- Called to shut down void AppKey( int key, int down ); -- called on each
// key up/down void AppChar( int key ); -- key was pressed & released
extern "C" char g_szAppName[];
extern "C" void AppInit(void);
extern "C" void AppRender(float frametime, float mouseDeltaX,
                          float mouseDeltaY);
extern "C" void AppExit(void);
extern "C" void AppKey(int key, int down);
extern "C" void AppChar(int key);
extern bool
    g_bCaptureOnFocus; // The app needs to define this to control how matsysapp
                       // handles mouse cursor hiding. If it's set to true, the
                       // mouse is captured and hidden when the app gets focus.
                       // If false, the mouse is only captured and hidden while
