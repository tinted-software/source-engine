...
float Importance(Vector const &direction) {
  // this returns a scale factor which can be used to recurd the importance of
  // certain directions.  in particular, this version makes the ground a lot
...
void main(int argc, char **argv) {
  ...
  if (argc != 2) {
    ...
    for (int f = 0; f < 6; f++)
      for (int y = 0; y < cmap.face_maps[f].Height; y++)
        for (int x = 0; x < cmap.face_maps[f].Width; x++) {
          Vector clr(cmap.face_maps[f].Pixel(x, y, 0),
    ...
    printf("Point light dir=%f %f %f\n", avg_light_dir.x, avg_light_dir.y,
           avg_light_dir.z);
    //		printf("Point light color=%f %f
    ...
    // now, output ambient cube maps for image-based lighting. During this pass,
    // we will also correct the ambient color
      ...
      for (int y = 0; y < conv.face_maps[f].Height; y++)
        for (int x = 0; x < conv.face_maps[f].Width; x++) {
          ...
          for (int comp = 0; comp < 3; comp++) {
            ...
            AmbientColor[comp] +=
                weight * (sumlight[comp] - dot * AverageHue[comp]);
          }
        }
    ...
    for (int f = 0; f < 6; f++)
      for (int y = 0; y < cmap.face_maps[f].Height; y++)
        for (int x = 0; x < cmap.face_maps[f].Width; x++) {
          ...
          dot = pow(dot, 7);
          for (int comp = 0; comp < 3; comp++)
            cmap.face_maps[f].Pixel(x, y, comp) =
                AmbientColor[comp] + dot * AverageHue[comp];
        }
