...
void ServerThread(void *) {
  ...
  uint32 pSendbuf[DEFAULT_SEND_BUF_LEN]; // Error string or binary shader blob
                                         // in reply
  ...
  while (true) {
    ...
    if (nResult > 0) {
      ...
      if (!stricmp(pShaderModel, "vs_2_0") ||
          !stricmp(pShaderModel, "ps_2_0") ||
          !stricmp(pShaderModel, "ps_2_b")) {
        ...
        D3DXMACRO macro;
        for (int i = 0; i < nNumMacros - 1;
             i++) // The last one is the (null) one, so don't bother reading it
        {
        ...
        // This utility needs to change if we want to do ps30 shaders (see logic
        // in vertexshaderdx8.cpp)
        HRESULT hr = D3DXCompileShaderFromFile(
            pShaderFilename, macros.Base(), NULL /* LPD3DXINCLUDE */, "main",
        ...
        if (hr != D3D_OK) {
          ...
          if (pErrorMessages) {
            ...
            nSendBufLen = pErrorMessages->GetBufferSize() +
                          4; // account for uint32 at front of the buffer
          } else {
            ((uint8 *)(pSendbuf + 1))[0] = '?';
        ...
        } else // Success!
        {
          ...
          memcpy(pSendbuf + 1, pShader->GetBufferPointer(),
                 pShader->GetBufferSize());
          nSendBufLen = pShader->GetBufferSize() +
                        4; // account for uint32 at front of the buffer
          ...
          if (g_bPrintDisassembly) {
            ...
            D3DXDisassembleShader((const DWORD *)pShader->GetBufferPointer(),
                                  FALSE, "", &pDisassembly);
            if (pDisassembly) {
      ...
      } else // Not a supported shader profile
      {
        ...
        V_snprintf(pCharSendbuff, DEFAULT_SEND_BUF_LEN,
                   "Unsupported shader profile: %s\n", pShaderModel);
        nSendBufLen = strlen(pCharSendbuff) +
                      4; // account for uint32 at front of the buffer
      }

      // Send the compiled shader back to the game
      int nSendResult =
          send(ClientSocket, (const char *)pSendbuf, nSendBufLen, 0);
...
int main(int argc, char *argv[]) {
  ...
  //
  // For example, if you have your shader source on your Windows machine, you
