...
static bool ParseVertex(CUtlBuffer &bufParse, characterset_t &breakSet, int &v,
                        int &t, int &n) {
  char cmd[1024];
...
int Load_OBJ(s_source_t *psource) {
  ...
  while (GetLineInput()) {
    ...
    if (strncmp(g_szLine, "v ", 2) == 0) {
      ...
      sscanf(g_szLine, "v %f %f %f", &g_vertex[i].x, &g_vertex[i].y,
             &g_vertex[i].z);
      g_bone[i].numbones = 1;
      g_bone[i].bone[0] = 0;
    ...
    if (strncmp(g_szLine, "vn ", 3) == 0) {
      i = g_numnormals++;
      sscanf(g_szLine, "vn %f %f %f", &g_normal[i].x, &g_normal[i].y,
             &g_normal[i].z);
      continue;
    }
    ...
    if (!Q_strncmp(g_szLine, "mtllib ", 7)) {
      ...
      Q_ComposeFileName(pFullDir, cmd, pFullMtlLibPath,
                        sizeof(pFullMtlLibPath));
    ...
    if (strncmp(g_szLine, "f ", 2) == 0) {
      ...
      CUtlBuffer bufParse(pData, nLen,
...
int AppendVTAtoOBJ(s_source_t *psource, char *filename, int frame) {
  ...
  while (GetLineInput()) {
    ...
    if (strncmp(g_szLine, "v ", 2) == 0) {
      ...
      j = sscanf(g_szLine, "f %d/%d/%d %d/%d/%d %d/%d/%d %d/%d/%d", &v0, &t0,
                 &n0, &v1, &t1, &n1, &v2, &t2, &n2, &v3, &t3, &n3);
