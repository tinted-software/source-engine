...
template <class T> class CUtlVectorAuto : public CUtlVector<T> {
  // typedef CUtlVectorAuto< T, CUtlVector<T > > BaseClass;
public:
  T &operator[](int i);
};

template <typename T> inline T &CUtlVectorAuto<T>::operator[](int i) {
  EnsureCount(i + 1);
...
extern int
findGlobalBone(const char *name); // finds a named bone in the global bone table
...
const char *RenameBone(
    const char *pName); // returns new name if available, else return pName.
...
struct s_linearmove_t {
  ...
  int flags; // type of motion.  Only linear, linear accel, and linear decel is
             // allowed
  float v0;
  float v1;
...
struct s_animation_t {
  ...
  char			shared_animgroup_name[ MAXSTUDIONAME ]; // group name in
  share file int				shared_group_subindex;
  ...
  */
  ...
  int endframe;
  int flags;
  // animations processed (time shifted, linearized, and bone adjusted ) from
  // source animations
...
public:
  char name[MAXSTUDIONAME];
  char activityname[MAXSTUDIONAME]; // index into the string table, the name of
                                    // this activity.
  ...
  int numblends;
  int groupsize[2];
  CUtlVectorAuto<CUtlVectorAuto<s_animation_t *>>
      panim; // [MAXSTUDIOBLENDS][MAXSTUDIOBLENDS];
...
// Animations stored in raw off-disk source files.  Raw data should be not
// processed.
...
public:
  ...
  // vertex animation
  bool newStyleVertexAnimations; // new style doesn't store a base pose in
                                 // vertex anim[0]
...
struct s_source_t {
  ...
  int m_nKeyStartIndex; // The index at which the flex keys for this model start
                        // in the global list
...
struct s_flexcontrollerremap_t {
  ...
  int m_Index; ///< The model relative index of the slider control for value for
               ///< this if it's not split, -1 otherwise
  int m_LeftIndex;  ///< The model relative index of the left slider control for
                    ///< this if it's split, -1 otherwise
  int m_RightIndex; ///< The model relative index of the right slider control
                    ///< for this if it's split, -1 otherwise
  int m_MultiIndex; ///< The model relative index of the value slider control
                    ///< for this if it's multi, -1 otherwise
  CUtlString
      m_EyesUpDownFlexName; // The name of the eyes up/down flex controller
  int m_EyesUpDownFlexController; // The global index of the Eyes Up/Down Flex
                                  // Controller
  int m_BlinkController;          // The global index of the Blink Up/Down Flex
                                  // Controller
};
...
EXTERN int g_jigglebonemap[MAXSTUDIOBONES]; // map used jigglebone's to source
                                            // jigglebonebone's
...
EXTERN int g_axisinterpbonemap[MAXSTUDIOBONES]; // map used axisinterpbone's to
                                                // source axisinterpbone's
...
EXTERN int g_quatinterpbonemap[MAXSTUDIOBONES]; // map used quatinterpbone's to
                                                // source axisinterpbone's
...
EXTERN int
    g_aimatbonemap[MAXSTUDIOBONES]; // map used aimatpbone's to source
                                    // aimatpbone's (may be optimized out)
...
s_source_t *Load_Source(char const *filename, const char *ext,
                        bool reverse = false, bool isActiveModel = false);
...
void ProcessOriginalContentFile(const char *szDataFile,
                                const char *szOriginalContentFile);
...
public:
  ...
  const char *GetSrcName(void) const { return m_pSrcName; }
  const char *GetDstName(void) const { return m_pDstName; }
...
public:
  ...
  void EnableFacialAnimation(bool val) { m_bFacialAnimation = val; }
  bool GetFacialAnimationEnabled() const { return m_bFacialAnimation; }

  void StripFromModel(bool val) { m_bStrippedFromModel = val; }
  bool IsStrippedFromModel() const { return m_bStrippedFromModel; }
...
// the first time these are called, the name of the model/QC file is printed so
// that when running in batch mode, no echo, when dumping to a file, it can be
// determined which file is broke.
