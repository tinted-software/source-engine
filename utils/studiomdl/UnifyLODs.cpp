...
// debugging only - enabling turns off remapping to create all lod vertexes as
// unique to ensure remapping logic does not introduce collapse anomalies
...
// Swap bones
//-----------------------------------------------------------------------------
static inline void SwapBones(s_boneweight_t &boneWeight, int nBone1,
                             int nBone2) {
  // swap
...
//-----------------------------------------------------------------------------
CVertexDictionary::CVertexDictionary() { m_nPrevLODCount = 0; }

//-----------------------------------------------------------------------------
// Accessor
//-----------------------------------------------------------------------------
inline VertexInfo_t &CVertexDictionary::Vertex(int i) { return m_Verts[i]; }
...
inline int CVertexDictionary::RootLODVertexEnd() const { return m_nRootLODEnd; }

//-----------------------------------------------------------------------------
// Marks the dictionary as starting defining vertices for a new LOD
//-----------------------------------------------------------------------------
void CVertexDictionary::StartNewLOD() { m_nPrevLODCount = VertexCount(); }
...
// Copies in a particular vertex from the s_source_t
//-----------------------------------------------------------------------------
int CVertexDictionary::AddVertexFromSource(const s_source_t *pSrc,
                                           int nVertexIndex, int nLod) {
...
//-----------------------------------------------------------------------------
int CVertexDictionary::VertexCount() const { return m_Verts.Count(); }
...
//-----------------------------------------------------------------------------
// Computes error between two tangentS vectors; returns false if the error is
// too great
//-----------------------------------------------------------------------------
bool CompareTangentSFuzzy(const Vector4D &n1, const Vector4D &n2,
                          float &flError) {
...
//-----------------------------------------------------------------------------
// Computes the error between two bone weights, returns false if they are too
// far
//-----------------------------------------------------------------------------
bool CompareBoneWeightsFuzzy(const s_boneweight_t &b1, const s_boneweight_t &b2,
  ...
  // Use the index to figure out where in the array for b2 that the
  // corresponding bone in b1 is.
  ...
  // At least one bone matches, so we're going to consider this vertex as a
  // potential match This loop will take care of figuring out the error for all
  // bones that exist in b1 alone, and all bones that exist in b1 and b2
  flError = 0;
  for (i = 0; i < b1.numbones; ++i) {
  ...
  // This loop will take care of figuring out the error for all bones that exist
  // in b2 alone
  for (i = 0; i < b2.numbones; ++i) {
    // If we didn't find a match for this bone, compute a more expensive weight
...
static s_mesh_t *FindOrCullMesh(int nLodID, s_source_t *pSrc, int nMaterialID) {
  ...
  Q_FileBase(g_texture[nTextureID].name, baseMeshName,
             sizeof(baseMeshName) - 1);
  for (int i = 0; i < g_ScriptLODs[nLodID].meshRemovals.Count(); i++) {
    const char *pMeshRemovalName =
        g_ScriptLODs[nLodID].meshRemovals[i].GetSrcName();
...
static void CopyVerts(int nLodID, const s_source_t *pSrc,
                      const s_mesh_t *pSrcMesh, CVertexDictionary &vertexDict,
  ...
  for (int srcVertID = 0; srcVertID < pSrcMesh->numvertices; srcVertID++) {
    ...
    pMeshVertIndexMap[nVertexIndex] =
        vertexDict.AddVertexFromSource(pSrc, nVertexIndex, nLodID) -
        pDstMesh->vertexoffset;
  }

  pDstMesh->numvertices = pSrcMesh->numvertices;
}

static void CopyFaces(const s_source_t *pSrc, const s_mesh_t *pSrcMesh,
                      CUtlVector<s_face_t> &faces, s_mesh_t *pDstMesh) {
  int srcFaceID;
...
//-----------------------------------------------------------------------------
// return -1 if there is no match. The index returned is used to index into
// vertexDict.
//-----------------------------------------------------------------------------
static int FindVertexWithinVertexDictionary(const VertexInfo_t &find,
  ...
  for (int nVertexIndex = nStartVert; nVertexIndex < nEndVert; ++nVertexIndex) {
    ...
    if (!(fIgnore & IGNORE_POSITION) &&
        !ComparePositionFuzzy(find.m_Position,
                              vertexDict.Vertex(nVertexIndex).m_Position,
                              flPositionError))
      continue;

    if (!(fIgnore & IGNORE_TEXCOORD) &&
        !CompareTexCoordsFuzzy(find.m_TexCoord,
                               vertexDict.Vertex(nVertexIndex).m_TexCoord,
                               flTexcoordError))
      continue;

    if (!(fIgnore & IGNORE_BONEWEIGHT) &&
        !CompareBoneWeightsFuzzy(find.m_BoneWeight,
                                 vertexDict.Vertex(nVertexIndex).m_BoneWeight,
                                 flBoneWeightError))
      continue;

    if (!(fIgnore & IGNORE_NORMAL) &&
        !CompareNormalFuzzy(find.m_Normal,
                            vertexDict.Vertex(nVertexIndex).m_Normal,
                            flNormalError))
      continue;

    if (!(fIgnore & IGNORE_TANGENTS) &&
        !CompareTangentSFuzzy(find.m_TangentS,
                              vertexDict.Vertex(nVertexIndex).m_TangentS,
                              flTangentSError))
      continue;
...
//-----------------------------------------------------------------------------
// Use position, normal, and texcoord checks across the entire model to find a
// boneweight
//-----------------------------------------------------------------------------
static void FindBoneWeightWithinModel(const VertexInfo_t &searchVertex,
  ...
  for (int i = 0; i < nVertexCount; i++) {
    ...
    ComparePositionFuzzy(searchVertex.m_Position, srcVertex.position,
                         flPositionError);

    if (!(fIgnore & IGNORE_NORMAL)) {
      CompareNormalFuzzy(searchVertex.m_Normal, srcVertex.normal,
                         flNormalError);
    }

    if (!(fIgnore & IGNORE_TEXCOORD)) {
      CompareTexCoordsFuzzy(searchVertex.m_TexCoord, srcVertex.texcoord,
                            flTexcoordError);
    }

    if (!(fIgnore & IGNORE_TANGENTS)) {
      CompareTangentSFuzzy(searchVertex.m_TangentS, srcVertex.tangentS,
                           flTangentSError);
    }
  ...
  memcpy(&boneWeight, &pSrc->m_GlobalVertices[nBestIndex].boneweight,
         sizeof(s_boneweight_t));
}

//-----------------------------------------------------------------------------
// Modify the bone weights in all of the vertices....
//-----------------------------------------------------------------------------
static void RemapBoneWeights(const CUtlVector<int> &boneMap,
...
//-----------------------------------------------------------------------------
static void CalculateBoneWeightFromRootLod(const VertexInfo_t &searchVertex,
                                           CVertexDictionary &vertexDict,
                                           const s_source_t *pRootLODSrc,
                                           VertexInfo_t &idealVertex) {
  idealVertex = searchVertex;

  // Look through the part of the vertex dictionary associated with the root
  // LODs for a match bone weights are not defined properly in SMDs for lower
  // LODs, so don't consider we can only accept the boneweight from the root LOD
  int nVertexDictID = FindVertexWithinVertexDictionary(
      searchVertex, vertexDict, vertexDict.RootLODVertexStart(),
      vertexDict.RootLODVertexEnd(), IGNORE_BONEWEIGHT | IGNORE_TANGENTS);
  if (nVertexDictID != -1) {
    Assert(nVertexDictID >= vertexDict.RootLODVertexStart() &&
           nVertexDictID < vertexDict.RootLODVertexEnd());
    ...
    // this ensures all the attributes, including bone weight are correct for
    // that vertex the worst case is that the vertex is not an *exact* match for
    // entry attributes just a "close" match
  ...
  // do a *positional check only* to give us a bone weight to assign to this
  // vertex.
...
//-----------------------------------------------------------------------------
static void CalculateIdealVert(const VertexInfo_t &searchVertex,
                               CVertexDictionary &vertexDict,
                               const s_mesh_t *pVertexDictMesh,
                               const s_source_t *pRootLODSrc,
                               VertexInfo_t &idealVertex) {
#ifndef UNIQUE_VERTEXES_FOR_LOD
  // Only look through the part of the vertex dictionary associated with all
  // *higher* LODs for a match
  int nVertexDictID = FindVertexWithinVertexDictionary(
      searchVertex, vertexDict, pVertexDictMesh->vertexoffset,
      vertexDict.PrevLODVertexCount(), 0);
  if (nVertexDictID != -1) {
    Assert(nVertexDictID >= pVertexDictMesh->vertexoffset &&
           nVertexDictID < vertexDict.PrevLODVertexCount());
    Assert(nVertexDictID >= 0 && nVertexDictID < vertexDict.VertexCount());
...
//-----------------------------------------------------------------------------
static bool AreBoneWeightsEqual(const s_boneweight_t &b1,
                                const s_boneweight_t &b2) {
...
//-----------------------------------------------------------------------------
static int FindVertexInDictionaryExact(CVertexDictionary &vertexDict,
                                       int nStartVert, int nEndVert,
  ...
  for (int nVertID = nStartVert; nVertID < nEndVert; ++nVertID) {
    ...
    if (!AreBoneWeightsEqual(vertexDict.Vertex(nVertID).m_BoneWeight,
                             vertex.m_BoneWeight))
      continue;
...
static int FindOrCreateExactVertexInDictionary(CVertexDictionary &vertexDict,
                                               const VertexInfo_t &vertex,
                                               s_mesh_t *pDstMesh) {
  int nMeshVertID = FindVertexInDictionaryExact(
      vertexDict, pDstMesh->vertexoffset,
...
static void PrintBonesUsedInLOD(s_source_t *pSrc) {
  ...
  for (int i = 0; i < nVertexCount; i++) {
    ...
    printf("pos: %f %f %f norm: %f %f %f texcoord: %f %f\n", pos[0], pos[1],
           pos[2], norm[0], norm[1], norm[2], texcoord[0], texcoord[1]);
    ...
    for (j = 0; j < pBoneWeight->numbones; j++) {
      ...
      printf("vert: %d bone: %d boneid: %d weight: %f name: \"%s\"\n", i,
             (int)j, (int)pBoneWeight->bone[j], (float)pBoneWeight->weight[j],
             pBoneName);
    }
    printf("\n");
    fflush(stdout);
...
static void PrintSBoneWeight(s_boneweight_t *pBoneWeight,
                             const s_source_t *pSrc) {
  int j;
  for (j = 0; j < pBoneWeight->numbones; j++) {
    int globalBoneID;
    ...
    printf("bone: %d boneid: %d weight: %f name: \"%s\"\n", (int)j,
           (int)pBoneWeight->bone[j], (float)pBoneWeight->weight[j], pBoneName);
  }
}
...
//-----------------------------------------------------------------------------
static void CreateLODVertsInDictionary(
    int nLodID, const s_source_t *pRootLODSrc, s_source_t *pCurrentLODSrc,
  ...
  for (int nSrcVertID = 0; nSrcVertID < pCurrLODMesh->numvertices;
       ++nSrcVertID) {
...
#endif
    // determine the best bone weight for the desired vertex within the root lod
    // only the root lod contains no bone remappings this ensures we get a
    // vertex with its matched proper boneweight assignment
    VertexInfo_t idealVertex;
    CalculateBoneWeightFromRootLod(vertex, vertexDict, pRootLODSrc,
                                   idealVertex);

    // try again to match the candidate vertex
    // determine the ideal vertex with desired remapped boneweight
    vertex = idealVertex;
    CalculateIdealVert(vertex, vertexDict, pVertexDictMesh, pRootLODSrc,
                       idealVertex);
    ...
    int nMeshVertID = FindOrCreateExactVertexInDictionary(
        vertexDict, idealVertex, pVertexDictMesh);
...
static void PrintSourceVerts(s_source_t *pSrc) {
  ...
  for (int i = 0; i < nVertexCount; i++) {
    const s_vertexinfo_t &srcVertex = pSrc->m_GlobalVertices[i];
    printf("v %d ", i);
    printf("pos: %f %f %f ", srcVertex.position[0], srcVertex.position[1],
           srcVertex.position[2]);
    printf("norm: %f %f %f ", srcVertex.normal[0], srcVertex.normal[1],
           srcVertex.normal[2]);
...
//-----------------------------------------------------------------------------
static void SetProcessedWithDictionary(s_model_t *pSrcModel,
                                       CVertexDictionary &vertexDict,
  ...
  pLodData->vertex =
      (s_lodvertexinfo_t *)kalloc(nVertexCount, sizeof(s_lodvertexinfo_t));
...
//-----------------------------------------------------------------------------
// This fills out boneMap, which is a mapping from src bone to src bone
// replacement (or to itself if there is no bone replacement.
//-----------------------------------------------------------------------------
static void BuildBoneLODMapping(CUtlVector<int> &boneMap, int lodID) {
  ...
  for (i = 0; i < scriptLOD.boneReplacements.Size(); i++) {
    ...
    if (j != -1 && k != -1) {
      ...
      // FIXME: is this really an error?  It could just be  replacement command
      // for bone that doesnt' exist anymore.
      if (g_verbose) {
...
static void MarkRootLODBones(CVertexDictionary &vertexDictionary) {
  ...
  for (int nVertDictID = vertexDictionary.RootLODVertexStart();
    ...
    s_boneweight_t &boneWeight =
        vertexDictionary.Vertex(nVertDictID).m_BoneWeight;
...
static void UnifyModelLODs(s_model_t *pSrcModel) {
  ...
  for (nLodID = 0; nLodID < MAX_NUM_LODS; nLodID++) {
    if (nLodID < nNumLODs && pSrcModel->m_LodSources[nLodID]) {
      int nVertexCount =
          pSrcModel->m_LodSources[nLodID]->m_GlobalVertices.Count();
  ...
  // These hold the aggregate data for the model that grows as lods are
  // processed
  ...
  for (nMeshID = 0; nMeshID < pSrcModel->source->nummeshes; nMeshID++) {
    ...
    CopyVerts(0, pLOD0Source, pLOD0Mesh, vertexDictionary, pVertexDictMesh,
              pMeshVertIndexMaps[0]);
    ...
    // Ideally, vertices used in an LOD would be in LOD 0 for the benefit of
    // shared vertices. If we don't find vertices in LOD 0, this code will add
    // vertices into LOD 0's list of vertices for the next LOD to find
    for (nLodID = 1; nLodID < nNumLODs; ++nLodID) {
      ...
      CreateLODVertsInDictionary(nLodID, pLOD0Source, pCurrLOD, pCurrLODMesh,
                                 pVertexDictMesh, vertexDictionary,
  ...
  SetProcessedWithDictionary(pSrcModel, vertexDictionary, faces, meshes,
                             pMeshVertIndexMaps);
  //	PrintSourceVerts( pSrcModel->m_LodModels[0] );
}

//-----------------------------------------------------------------------------
// Force the vertex array for a model to have all of the vertices that are
// needed for all of the LODs of the model.
//-----------------------------------------------------------------------------
void UnifyLODs(void) {
  // todo: need to fixup the firstref/lastref stuff . . do we really need it
  // anymore?
...
//-----------------------------------------------------------------------------
// Returns the sources associated with the various LODs based on the script
// commands
//-----------------------------------------------------------------------------
static void GetLODSources(CUtlVector<s_source_t *> &lods,
                          const s_model_t *pSrcModel) {
  ...
  for (int lodID = 0; lodID < nNumLODs; lodID++) {
    ...
    bool bFound;
    s_source_t *pSource =
        GetModelLODSource(pSrcModel->filename, scriptLOD, &bFound);
    if (!pSource && !bFound) {
...
static void
ReplaceBonesRecursive(int globalBoneID, bool replaceThis,
                      CUtlVector<CLodScriptReplacement_t> &boneReplacements,
                      const char *replacementName) {
  if (replaceThis) {
    CLodScriptReplacement_t &boneReplacement =
        boneReplacements[boneReplacements.AddToTail()];
...
static void ConvertSingleBoneTreeCollapseToReplaceBones(
    CLodScriptReplacement_t &boneTreeCollapse,
    CUtlVector<CLodScriptReplacement_t> &boneReplacements) {
  ...
  do {
    ...
    for (i = 0; i < lod.boneReplacements.Count(); i++) {
      for (j = 0; j < lod.boneReplacements.Count(); j++) {
        ...
        if (Q_stricmp(lod.boneReplacements[i].GetSrcName(),
                      lod.boneReplacements[j].GetDstName()) == 0) {
          lod.boneReplacements[j].SetDstName(
              lod.boneReplacements[i].GetDstName());
          changed = true;
        }
      }
