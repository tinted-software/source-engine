...
public:
  bool OptimizeFromStudioHdr(studiohdr_t *phdr, s_bodypart_t *pSrcBodyParts,
                             int vertCacheSize, bool usesFixedFunction,
...
private:
  ...
  void SetupMeshProcessing(studiohdr_t *phdr, int vertCacheSize,
                           bool usesFixedFunction, int maxBonesPerVert,
                           int maxBonesPerTri, int maxBonesPerStrip,
                           const char *fileName);
  ...
  void CreateLODTriangleList(s_model_t *pSrcModel, int nLodID,
                             s_source_t *pLODSource,
  ...
  void ProcessModel(studiohdr_t *phdr, s_bodypart_t *pSrcBodyParts,
                    TotalMeshStats_t &stats, bool bForceSoftwareSkin,
                    bool bHWFlex);

  // processes a single mesh within the model
  void ProcessMesh(Mesh_t *pMesh, studiohdr_t *pStudioHeader,
  ...
  void ProcessStripGroup(StripGroup_t *pStripGroup, bool isHWSkinned,
  ...
  void BuildSWSkinnedStrips(TriangleList_t &tris, VertexList_t const &verts,
  ...
  Triangle_t *
  GetNextUntouchedWithLeastBoneStateChanges(TriangleList_t &triangles);
  ...
  void SanityCheckVertBones(VertexIndexList_t const &list,
                            VertexList_t const &vertices);

  // Sets the flags associated with a particular strip group + mesh
  void ComputeStripGroupFlags(StripGroup_t *pStripGroup, bool isHWSkinned,
                              bool isFlexed);
  void ComputeMeshFlags(Mesh_t *pMesh, studiohdr_t *pStudioHeader,
                        mstudiomesh_t *pStudioMesh);

  bool MeshIsTeeth(studiohdr_t *pStudioHeader, mstudiomesh_t *pStudioMesh);

  // Tries to add neighboring vertices that'll fit into the matrix transform
  // state
  void BuildStripsRecursive(VertexIndexList_t &indices, TriangleList_t &list,
                            Triangle_t *triangle);
  ...
  void PostProcessStripGroup(mstudiomodel_t *pStudioModel,
                             mstudiomesh_t *pStudioMesh,
                             StripGroup_t *pStripGroup);
  ...
  void PrintVert(Vertex_t *v, mstudiomodel_t *pStudioModel,
                 mstudiomesh_t *pStudioMesh);
  ...
  void WriteMaterialReplacementLists(int materialReplacementsOffset,
                                     int materialReplacementListOffset);
  void WriteHeader(int vertCacheSize, int maxBonesPerVert, int maxBonesPerTri,
                   int maxBonesPerStrip, int numBodyParts, long checkSum);
  void WriteBodyPart(int bodyPartID, mstudiobodyparts_t *pBodyPart,
                     int modelID);
  ...
  void WriteStripGroup(int stripGroupID, StripGroup_t *pStripGroup, int vertID,
                       int indexID, int stripID);
  int WriteVerts(int vertID, StripGroup_t *pStripGroup);
  int WriteIndices(int indexID, StripGroup_t *pStripGroup);
  void WriteStrip(int stripID, Strip_t *pStrip, int indexID, int vertID,
                  int boneID);
  ...
  void WriteGLViewFile(studiohdr_t *phdr, const char *pFileName,
                       unsigned int flags, float shrinkFactor);
  void ShrinkVerts(float shrinkFactor);
  void GLViewDrawBegin(int mode);
  void CheckVertBoneWeights(Vertex_t *pVert, mstudiomodel_t *pStudioModel,
                            mstudiomesh_t *pStudioMesh);
  void GLViewVert(FILE *fp, Vertex_t vert, int index, Vector &color,
                  mstudiomodel_t *pStudioModel, mstudiomesh_t *pStudioMesh,
                  bool showSubStrips, float shrinkFraction);
  void GLViewDrawEnd(void);
  void SetMeshPropsColor(unsigned int meshFlags, Vector &color);
  void SetFlexedAndSkinColor(unsigned int glViewFlags,
                             unsigned int stripGroupFlags, Vector &color);
  void SetColorFromNumVertexBones(int numBones, Vector &color);
  Vector &GetOrigVertPosition(mstudiomodel_t *pStudioModel,
                              mstudiomesh_t *pStudioMesh, Vertex_t *pVert);
  float GetOrigVertBoneWeightValue(mstudiomodel_t *pStudioModel,
                                   mstudiomesh_t *pStudioMesh, Vertex_t *pVert,
                                   int boneID);
  mstudioboneweight_t &GetOrigVertBoneWeight(mstudiomodel_t *pStudioModel,
                                             mstudiomesh_t *pStudioMesh,
                                             Vertex_t *pVert);
  int GetOrigVertBoneIndex(mstudiomodel_t *pStudioModel,
                           mstudiomesh_t *pStudioMesh, Vertex_t *pVert,
                           int boneID);
  void ShowStats(void);
  ...
  void SortBonesWithinVertex(bool flexed, Vertex_t *vert,
                             mstudiomodel_t *pStudioModel,
                             mstudiomesh_t *pStudioMesh,
                             int *globalToHardwareBoneIndex,
                             int *hardwareToGlobalBoneIndex, int maxBonesPerTri,
                             int maxBonesPerVert);
  ...
  void SanityCheckVertexBoneLODFlags(studiohdr_t *pStudioHdr,
                                     FileHeader_t *pVtxHeader);
...
//-----------------------------------------------------------------------------
// Returns the number of bones that are not represented in the current hardware
// state
//-----------------------------------------------------------------------------

inline int
COptimizedModel::ComputeNewBonesNeeded(Triangle_t const &triangle) const {
  int numNewBones = 0;
...
Triangle_t *COptimizedModel::GetNextUntouchedWithoutBoneStateChange(
...
Triangle_t *COptimizedModel::GetNextUntouchedWithLeastBoneStateChanges(
    TriangleList_t &triangles) {
  Triangle_t *bestTriangle = 0;
  int bestNumNewBones = MAX_NUM_BONES_PER_TRI + 1;
  int i;
...
Triangle_t *COptimizedModel::GetNextTriangle(TriangleList_t &tris,
                                             bool allowNewStrip) {
...
void COptimizedModel::SanityCheckVertBones(VertexIndexList_t const &list,
                                           VertexList_t const &vertices) {
#ifdef _DEBUG
...
void COptimizedModel::Stripify(VertexIndexList_t const &sourceIndices,
                               bool isHWSkinned, int *pNumIndices,
  ...
  if (g_bBuildPreview || isHWSkinned == false) {
    ...
    *ppIndices = new unsigned short[*pNumIndices];
    memcpy(*ppIndices, sourceIndices.Base(),
           (*pNumIndices) * sizeof(unsigned short));
    return;
  }
  ...
  */
  ...
  GenerateStrips(&sourceIndices[0], sourceIndices.Size(), &primGroups,
                 &numPrimGroups);
  Assert(numPrimGroups == 1);
  *pNumIndices = primGroups->numIndices;
  *ppIndices = new unsigned short[*pNumIndices];
  memcpy(*ppIndices, primGroups->indices,
         sizeof(unsigned short) * *pNumIndices);
  delete[] primGroups;
#endif
}
...
void COptimizedModel::BuildHWSkinnedStrips(TriangleList_t &triangles,
  ...
  while (pSeedTri) {
    ...
    // hack - should just build directly into newStrip.verts instead of using a
    // global.
    int i;
    ...
    for (i = 0; i < m_HardwareMatrixState.AllocatedMatrixCount(); i++) {
      ...
      newStrip.boneStateChanges[i].newBoneID =
          m_HardwareMatrixState.GetNthBoneGlobalID(i);
    }

    // Empty out the triangles to strip so that we can start again with a new
    // strip.
...
//-----------------------------------------------------------------------------
bool COptimizedModel::IsVertexFlexed(mstudiomesh_t *pStudioMesh,
                                     int vertID) const {
...
void COptimizedModel::ComputeStripGroupFlags(StripGroup_t *pStripGroup,
  ...
  if (isFlexed) {
    ...
    pStripGroup->flags |= STRIPGROUP_IS_DELTA_FLEXED; // Going forward, DX9
                                                      // models are delta flexed
  }
  if (isHWSkinned) {
...
static void TryToReduceBoneInfluence(Vertex_t &stripGroupVert,
                                     mstudioboneweight_t const &boneWeights,
                                     int maxBones) {
  int i;
...
bool COptimizedModel::GenerateStripGroupVerticesFromFace(
    mstudioiface_t *pFace, mstudiomesh_t *pStudioMesh, int maxPreferredBones,
    Vertex_t *pStripGroupVert) {
  int vertIDs[3];
  vertIDs[0] = pFace->a;
  vertIDs[1] = pFace->b;
  ...
  for (int faceIndex = 0; faceIndex < 3; ++faceIndex) {
    ...
    if (!g_staticprop && bonesAffectingVertex <= 0) {
      ...
      MdlError("too many bones/vert (%d) : MAX_NUM_BONES_PER_VERT needs to be "
               "upped\n",
               bonesAffectingVertex);
    }
    ...
    if (!m_UsesFixedFunction) {
      if ((maxPreferredBones > 0) &&
          (bonesAffectingVertex > maxPreferredBones)) {
...
void COptimizedModel::BuildTriangleBoneData(VertexList_t &list,
                                            Triangle_t &tri) {
  int j, k, l;
...
static void FindMatchingEdge(TriangleList_t &list, int nTriangleId,
                             int nEdgeIndex, const int *pVertIds,
  ...
  for (int hEdge = hFirstEdge; hEdge != edges.InvalidIndex();
       hEdge = edges.Next(hEdge)) {
...
void COptimizedModel::BuildNeighborInfo(TriangleList_t &list,
                                        int nMaxVertexId) {
  // NOTE: vertexToEdges[vertId] contains the index of the head of a linked list
  // stored in edges
  CUtlFixedLinkedList<EdgeInfo_t> edges;
  CUtlVector<int> vertexToEdges;
  ...
  for (int i = 0; i < nTriCount; ++i) {
    ...
    // Add the three edges for this tri into a lookup table indexed by the lower
    // vertID
...
// Processes a single strip group
//-----------------------------------------------------------------------------
void COptimizedModel::ProcessStripGroup(
    StripGroup_t *pStripGroup, bool isHWSkinned, bool isFlexed,
    mstudiomodel_t *pStudioModel, mstudiomesh_t *pStudioMesh,
    CUtlVector<mstudioiface_t> &srcFaces,
  ...
  for (int n = 0; n < srcFaces.Size(); ++n) {
    ...
    newTri.vertID[0] =
        FindOrCreateVertex(stripGroupVertices, stripGroupVert[0]);
    newTri.vertID[1] =
        FindOrCreateVertex(stripGroupVertices, stripGroupVert[1]);
    newTri.vertID[2] =
        FindOrCreateVertex(stripGroupVertices, stripGroupVert[2]);
  ...
  if (isHWSkinned) {
    BuildHWSkinnedStrips(stripGroupSourceTriangles, stripGroupVertices,
                         pStripGroup, maxBonesPerStrip);
    ...
    BuildSWSkinnedStrips(stripGroupSourceTriangles, stripGroupVertices,
                         pStripGroup);
  }
}
...
int COptimizedModel::CountUniqueBonesInStrip(StripGroup_t *pStripGroup,
                                             Strip_t *pStrip) {
...
void COptimizedModel::PostProcessStripGroup(mstudiomodel_t *pStudioModel,
                                            mstudiomesh_t *pStudioMesh,
                                            StripGroup_t *pStripGroup) {
  int i;
  ...
  for (i = 0; i < pStripGroup->strips.Size(); i++) {
    ...
    pStripGroup->indices.EnsureCapacity(pStripGroup->indices.Size() +
                                        pStrip->numIndices);
...
#ifdef _DEBUG
      //	float GetOrigVertBoneWeightValue( mstudiomodel_t *pStudioModel,
      //mstudiomesh_t *pStudioMesh, Vertex_t *pVert, int boneID );
      int i;
      float *pWeight = (float *)_alloca(sizeof(float) * pVert->numBones);
      for (i = 0; i < pVert->numBones; i++) {
        pWeight[i] =
            GetOrigVertBoneWeightValue(pStudioModel, pStudioMesh, pVert, i);
      }
#endif
    ...
    pStrip->numStripGroupIndices =
        pStripGroup->indices.Size() - pStrip->stripGroupIndexOffset;
    pStrip->numStripGroupVerts =
        pStripGroup->verts.Size() - pStrip->stripGroupVertexOffset;
...
bool COptimizedModel::MeshIsTeeth(studiohdr_t *pStudioHeader,
                                  mstudiomesh_t *pStudioMesh) {
...
void COptimizedModel::ComputeMeshFlags(Mesh_t *pMesh,
...
void COptimizedModel::ProcessMesh(Mesh_t *pMesh, studiohdr_t *pStudioHeader,
                                  CUtlVector<mstudioiface_t> &srcFaces,
                                  mstudiomodel_t *pStudioModel,
                                  mstudiomesh_t *pStudioMesh, bool forceNoFlex,
                                  bool bForceSoftwareSkin, bool bHWFlex) {
  ...
  for (isHWSkinned = bForceSoftwareSkin ? 0 : 1; isHWSkinned >= 0;
       --isHWSkinned) {
    for (isFlexed = 1; isFlexed >= 0; --isFlexed) {
      ...
      ProcessStripGroup(&newStripGroup, isHWSkinned ? true : false,
                        isFlexed ? true : false, pStudioModel, pStudioMesh,
                        srcFaces, trianglesProcessed, realMaxBonesPerVert,
                        realMaxBonesPerTri, realMaxBonesPerStrip, forceNoFlex,
                        bHWFlex);
...
void COptimizedModel::SetupMeshProcessing(
    studiohdr_t *pHdr, int vertexCacheSize, bool usesFixedFunction,
...
/*
static void WriteDebugFile( const char *fileName, const char *outFileName, float
red, float grn, float blu )
{
        ...
        for( i = 0; i < pSrc->nummeshes; i++ )
        {
                ...
                for( j = 0; j < pMesh->numfaces; j++ )
                {
                        ...
                        Vector &a = pSrc->vertex[pMesh->vertexoffset +
pFace->a]; Vector &b = pSrc->vertex[pMesh->vertexoffset + pFace->b]; Vector &c =
pSrc->vertex[pMesh->vertexoffset + pFace->c]; fprintf( fp, "3\n" ); fprintf( fp,
"%f %f %f %f %f %f\n", ( float )( a[0] ), ( float )( a[1] ), ( float )( a[2] ),
                                ( float )red, ( float )grn, ( float )blu );
                        fprintf( fp, "%f %f %f %f %f %f\n", ( float )( b[0] ), (
float )( b[1] ), ( float )( b[2] ), ( float )red, ( float )grn, ( float )blu );
                        fprintf( fp, "%f %f %f %f %f %f\n", ( float )( c[0] ), (
float )( c[1] ), ( float )( c[2] ), ( float )red, ( float )grn, ( float )blu );
                }
...
void COptimizedModel::SourceMeshToTriangleList(
    s_model_t *pSrcModel, s_mesh_t *pSrcMesh,
    CUtlVector<mstudioiface_t> &meshTriangleList) {
...
/*
static void WriteSourceMesh( s_model_t *pSrcModel, s_mesh_t *pSrcMesh, int red_,
int grn_, int blu_ )
{
        FILE *fp;
        fp = fopen( "blah.glv", "a+" );
...
+ size ); fprintf( fp, "%f %f %f 255 0 0\n", pos[0] - size, pos[1] + size,
pos[2] + size ); fprintf( fp, "%f %f %f 255 0 0\n", pos[0] + size, pos[1] -
size, pos[2] + size ); fprintf( fp, "%f %f %f 255 0 0\n", pos[0] - size, pos[1]
- size, pos[2] + size );
...
void COptimizedModel::CreateLODTriangleList(
    s_model_t *pSrcModel, int nLodID, s_source_t *pSrc,
    mstudiomodel_t *pStudioModel, mstudiomesh_t *pStudioMesh,
...
#ifdef _DEBUG
//	const mstudio_modelvertexdata_t *vertData =
//pStudioModel->GetVertexData(); 	Assert( vertData ); // This can only return
//NULL on X360 for now 	mstudiovertex_t *modelFirstVert = vertData->Vertex( 0 );
//	mstudiovertex_t *modelLastVert = vertData->Vertex(
//pStudioModel->numvertices - 1 ); 	mstudiovertex_t *meshFirstVert =
//vertData->Vertex( 0 ); 	mstudiovertex_t *meshLastVert = vertData->Vertex(
//pStudioMesh->numvertices - 1 );
#endif
  ...
  */
  ...
  // this is icky.. . pSrc->nummeshes really refers to the total number of
  // non-empty meshes in the model.  In pSrc->meshes, there are some empty
  // meshes in the middle if they don't exist for the material, so you have to
  // go through all of the meshes to find the non-empty ones.
  ...
  for (i = 0; i < pSrcMesh->numvertices; i++) {
    ...
    indexMapping[i] =
        pSrcModel->m_pLodData
            ->pMeshVertIndexMaps[nLodID][pSrcMesh->vertexoffset + i];
  }
...
bool COptimizedModel::MeshNeedsRemoval(studiohdr_t *pHdr,
                                       mstudiomesh_t *pStudioMesh,
                                       LodScriptData_t &scriptLOD) {
  mstudiotexture_t *ptexture;
...
// Process the entire model, return stats...
//-----------------------------------------------------------------------------
void COptimizedModel::ProcessModel(studiohdr_t *pHdr,
  ...
  for (bodyPartID = 0; bodyPartID < pHdr->numbodyparts;
       bodyPartID++, stats.m_TotalBodyParts++) {
    mstudiobodyparts_t *pBodyPart = pHdr->pBodypart(bodyPartID);
    s_bodypart_t *pSrcBodyPart = &pSrcBodyParts[bodyPartID];
    for (modelID = 0; modelID < pBodyPart->nummodels;
         modelID++, stats.m_TotalModels++) {
      ...
      for (lodID = 0; lodID < g_ScriptLODs.Count();
           lodID++, stats.m_TotalModelLODs++) {
        ...
        for (meshID = 0; meshID < pStudioModel->nummeshes;
             meshID++, stats.m_TotalMeshes++) {
#ifdef _DEBUG
//					printf( "bodyPart: %d model: %d
//modellod: %d mesh: %d\n", bodyPartID, modelID, lodID, meshID );
#endif
          mstudiomesh_t *pStudioMesh = pStudioModel->pMesh(meshID);
          s_mesh_t *pSrcMesh =
              &pSrcModel->source->mesh[pSrcModel->source->meshindex[meshID]];
...
#ifdef _DEBUG
//					int textureSearchID =
//material_to_texture( pStudioMesh->material ); 					const char *pDebugName =
//pHdr->pTexture( textureSearchID )->pszName( );
#endif
          CUtlVector<mstudioiface_t> meshTriangleList;
          if (pLODSource) {
            // map the lod data to triangles
            // uses the original mesh redirected through a mapping table
            // this expects built per lod-to-root mapping tables to generate
            // faces
            CreateLODTriangleList(pSrcModel, lodID, pLODSource, pStudioModel,
                                  pStudioMesh, meshTriangleList, false);
          } else {
            // build the triangle list from the unmapped source
            SourceMeshToTriangleList(pSrcModel, pSrcMesh, meshTriangleList);
          }

          ProcessMesh(&newMesh, pHdr, meshTriangleList, pStudioModel,
                      pStudioMesh, !scriptLOD.GetFacialAnimationEnabled(),
                      bForceSoftwareSkin, bHWFlex);
          ...
          stats.m_TotalBoneStateChanges +=
              GetTotalBoneStateChangesForMesh(&newMesh);
        }
      }
    }
  }
...
void COptimizedModel::MapGlobalBonesToHardwareBoneIDsAndSortBones(
    studiohdr_t *phdr) {
    ...
    for (int lodID = 0; lodID < header->numLODs; lodID++) {
      ...
      for (int modelID = 0; modelID < bodyPart->numModels; modelID++) {
        ...
        for (int meshID = 0; meshID < pLOD->numMeshes; meshID++) {
          ...
          mstudiomesh_t *pStudioMesh = pStudioModel->pMesh(meshID);
          for (int stripGroupID = 0; stripGroupID < mesh->numStripGroups;
               stripGroupID++) {
            ...
            for (int stripID = 0; stripID < pStripGroup->numStrips; stripID++) {
              ...
              // Generate mapping back and forth between hardware IDs and
              // original bone IDs
              int boneStateChangeID;
              for (boneStateChangeID = 0;
                   boneStateChangeID < pStrip->numBoneStateChanges;
                   boneStateChangeID++) {
              ...
              for (vertID = 0; vertID < pStrip->numVerts; vertID++) {
                Vertex_t *vert =
                    pStripGroup->pVertex(vertID + pStrip->vertOffset);
                int boneID;
                for (boneID = 0; boneID < header->maxBonesPerVert; boneID++) {
                  ...
                  if (globalBoneID == -1) {
                    ...
                    //									Assert(
                    //vert->boneWeights[boneID] == 0 );
                    continue;
                  }
                  Assert(globalBoneID >= 0 && globalBoneID < m_NumBones);
                  vert->boneID[boneID] =
                      globalToHardwareBoneIndex[globalBoneID];
                  Assert(vert->boneID[boneID] >= 0 &&
                         vert->boneID[boneID] < header->maxBonesPerStrip);
                }

                // We only do index palette skinning when we're not doing fixed
                // function
                if (m_UsesFixedFunction) {
                  //									bool
                  //flexed = pStripGroup->flags & STRIPGROUP_IS_FLEXED;
...
void COptimizedModel::WriteBodyPart(int bodyPartID,
                                    mstudiobodyparts_t *pBodyPart,
                                    int modelID) {
  BodyPartHeader_t bodyPart;
  bodyPart.numModels = pBodyPart->nummodels;
  int bodyPartOffset =
      m_BodyPartsOffset + bodyPartID * sizeof(BodyPartHeader_t);
  int modelFileOffset = m_ModelsOffset + modelID * sizeof(ModelHeader_t);
  bodyPart.modelOffset = modelFileOffset - bodyPartOffset;
  m_FileBuffer->WriteAt(bodyPartOffset, &bodyPart, sizeof(BodyPartHeader_t),
                        "bodypart");
#ifdef _DEBUG
//	BodyPartHeader_t *debug = ( BodyPartHeader_t *
//)m_FileBuffer->GetPointer( bodyPartOffset ); 	ModelHeader_t *pModel =
//debug->pModel( 0 );
#endif
}

void COptimizedModel::WriteModel(int modelID, mstudiomodel_t *pModel,
                                 int lodID) {
  ModelHeader_t model;
  ...
  m_FileBuffer->WriteAt(modelFileOffset, &model, sizeof(ModelHeader_t),
                        "model");
#ifdef _DEBUG
//	ModelHeader_t *debug = ( ModelHeader_t * )m_FileBuffer->GetPointer(
//modelFileOffset ); 	ModelLODHeader_t *pLOD = debug->pLOD( 0 );
#endif
}
...
void COptimizedModel::WriteModelLOD(int lodID, ModelLOD_t *pLOD, int meshID) {
  ...
  m_FileBuffer->WriteAt(lodFileOffset, &lod, sizeof(ModelLODHeader_t),
                        "modellod");
#ifdef _DEBUG
//	ModelLODHeader_t *debug = ( ModelLODHeader_t *
//)m_FileBuffer->GetPointer( lodFileOffset ); 	MeshHeader_t *pMesh =
//debug->pMesh( 0 );
#endif
}
...
void COptimizedModel::WriteMesh(int meshID, Mesh_t *pMesh, int stripGroupID) {
  ...
  int meshFileOffset = m_MeshesOffset + meshID * sizeof(MeshHeader_t);
  int stripGroupFileOffset =
      m_StripGroupsOffset + stripGroupID * sizeof(StripGroupHeader_t);
...
#ifdef _DEBUG
//	MeshHeader_t *debug = ( MeshHeader_t * )m_FileBuffer->GetPointer(
//meshFileOffset ); 	StripGroupHeader_t *pStripGroup = debug->pStripGroup( 0 );
#endif
}

void COptimizedModel::WriteStripGroup(int stripGroupID,
                                      StripGroup_t *pStripGroup, int vertID,
                                      int indexID, int stripID) {
  ...
  m_FileBuffer->WriteAt(stripGroupFileOffset, &stripGroup,
                        sizeof(StripGroupHeader_t), "strip group");
#ifdef _DEBUG
//	StripGroupHeader_t *debug = ( StripGroupHeader_t *
//)m_FileBuffer->GetPointer( stripGroupFileOffset ); 	unsigned short *pIndex =
//debug->pIndex( 0 ); 	Vertex_t *pVert = debug->pVertex( 0 ); 	StripHeader_t
//*pStripHeader = debug->pStrip( 0 );
#endif
}
...
int COptimizedModel::WriteVerts(int vertID, StripGroup_t *pStripGroup) {
  ...
  m_FileBuffer->WriteAt(vertFileOffset, &pStripGroup->verts[0],
                        sizeof(Vertex_t) * numVerts, "verts");
#ifdef _DEBUG
...
int COptimizedModel::WriteIndices(int indexID, StripGroup_t *pStripGroup) {
  ...
  m_FileBuffer->WriteAt(indexFileOffset, &pStripGroup->indices[0],
                        sizeof(unsigned short) * numIndices, "indices");
#ifdef _DEBUG
//	unsigned short *debug = ( unsigned short * )m_FileBuffer->GetPointer(
//indexFileOffset );
#endif
  return numIndices;
}

void COptimizedModel::WriteStrip(int stripID, Strip_t *pStrip, int indexID,
                                 int vertID, int boneID) {
  ...
  int boneFileOffset =
      m_BoneStageChangesOffset + boneID * sizeof(BoneStateChangeHeader_t);
  ...
  m_FileBuffer->WriteAt(stripFileOffset, &stripHeader, sizeof(StripHeader_t),
                        "strip");
#ifdef _DEBUG
//	StripHeader_t *debug = ( StripHeader_t* )m_FileBuffer->GetPointer(
//stripFileOffset ); 	BoneStateChangeHeader_t *pBoneStateChange =
//debug->pBoneStateChange( 0 );
#endif
}

void COptimizedModel::WriteBoneStateChange(int boneID,
                                           BoneStateChange_t *boneStateChange) {
...
#endif
  m_FileBuffer->WriteAt(boneFileOffset, &boneHeader,
                        sizeof(BoneStateChangeHeader_t), "bone");
#ifdef _DEBUG
//	BoneStateChangeHeader_t *debug = ( BoneStateChangeHeader_t *
//)m_FileBuffer->GetPointer( boneFileOffset );
#endif
}
    ...
    for (modelID = 0; modelID < pBodyPart->numModels; modelID++) {
      ...
      for (lodID = 0; lodID < pModel->numLODs; lodID++) {
        ...
        for (meshID = 0; meshID < pLOD->numMeshes; meshID++) {
          ...
          for (stripGroupID = 0; stripGroupID < pMesh->numStripGroups;
               stripGroupID++) {
...
void COptimizedModel::WriteStringTable(int stringTableOffset) {
  ...
  m_FileBuffer->WriteAt(stringTableOffset, pTmp, stringTableSize,
                        "string table");
  //	char *pDebug = ( char * )m_FileBuffer->GetPointer( stringTableOffset );
  delete[] pTmp;
}

void COptimizedModel::WriteMaterialReplacements(
  ...
  for (i = 0; i < numLODs; i++) {
    ...
    for (j = 0; j < scriptLOD.materialReplacements.Size(); j++) {
      CLodScriptReplacement_t &materialReplacement =
          scriptLOD.materialReplacements[j];
      MaterialReplacementHeader_t tmpHeader;
      tmpHeader.materialID =
          FindMaterialByName(materialReplacement.GetSrcName());
      tmpHeader.replacementMaterialNameOffset =
          m_StringTableOffset +
          s_StringTable.StringTableOffset(materialReplacement.GetDstName()) -
          offset;
      m_FileBuffer->WriteAt(offset, &tmpHeader, sizeof(tmpHeader),
...
void COptimizedModel::WriteMaterialReplacementLists(
  ...
  for (i = 0; i < numLODs; i++) {
    ...
    if (pDebugList->numReplacements) {
      ...
      //pDebugList->pMaterialReplacement( 0 ); 			const char *string =
      //pDebug->pMaterialReplacementName();
    }
    replacementOffset +=
        tmpHeader.numReplacements * sizeof(MaterialReplacementHeader_t);
    offset += sizeof(MaterialReplacementListHeader_t);
  }
}

void COptimizedModel::SanityCheckVertexBoneLODFlags(studiohdr_t *pStudioHdr,
                                                    FileHeader_t *pVtxHeader) {
  int bodyPartID;
    ...
    for (modelID = 0; modelID < pBodyPart->nummodels; modelID++) {
      ...
      for (lodID = 0; lodID < pVtxModel->numLODs; lodID++) {
        ...
        for (meshID = 0; meshID < pVtxLOD->numMeshes; meshID++) {
          ...
          int stripGroupID;
          for (stripGroupID = 0; stripGroupID < pVtxMesh->numStripGroups;
               stripGroupID++) {
            StripGroupHeader_t *pStripGroup =
            ...
            for (vertID = 0; vertID < pStripGroup->numVerts; vertID++) {
              ...
              const mstudioboneweight_t &boneWeight =
                  GetOrigVertBoneWeight(pModel, pMesh, pVertex);
              int i;

              for (i = 0; i < boneWeight.numbones; i++) {
                mstudiobone_t *pBone = pStudioHdr->pBone(boneWeight.bone[i]);
                //								const
                //char *pBoneName = pBone->pszName();
...
void COptimizedModel::WriteVTXFile(studiohdr_t *pHdr, const char *pFileName,
  ...
  m_ModelsOffset =
      m_BodyPartsOffset + sizeof(BodyPartHeader_t) * stats.m_TotalBodyParts;
  m_ModelLODsOffset =
      m_ModelsOffset + sizeof(ModelHeader_t) * stats.m_TotalModels;
  m_MeshesOffset =
      m_ModelLODsOffset + sizeof(ModelLODHeader_t) * stats.m_TotalModelLODs;
  m_StripGroupsOffset =
      m_MeshesOffset + sizeof(MeshHeader_t) * stats.m_TotalMeshes;
  m_StripsOffset = m_StripGroupsOffset +
                   sizeof(StripGroupHeader_t) * stats.m_TotalStripGroups;
  ...
  m_BoneStageChangesOffset =
      m_IndicesOffset + sizeof(unsigned short) * stats.m_TotalIndices;
  m_StringTableOffset =
      m_BoneStageChangesOffset +
      sizeof(BoneStateChangeHeader_t) * stats.m_TotalBoneStateChanges;
  m_MaterialReplacementsOffset = m_StringTableOffset + s_StringTable.CalcSize();
  m_MaterialReplacementsListOffset =
      m_MaterialReplacementsOffset +
      stats.m_TotalMaterialReplacements * sizeof(MaterialReplacementHeader_t);
  m_EndOfFileOffset =
      m_MaterialReplacementsListOffset +
      g_ScriptLODs.Count() * sizeof(MaterialReplacementListHeader_t);
  ...
  WriteMaterialReplacementLists(m_MaterialReplacementsOffset,
                                m_MaterialReplacementsListOffset);
    ...
    for (int modelID = 0; modelID < pBodyPart->nummodels; modelID++) {
      ...
      for (int lodID = 0; lodID < g_ScriptLODs.Count(); lodID++) {
        ...
        for (int meshID = 0; meshID < pStudioModel->nummeshes; meshID++) {
          ...
          for (int stripGroupID = 0; stripGroupID < pMesh->stripGroups.Count();
               stripGroupID++) {
            ...
            for (int stripID = 0; stripID < nStripCount; stripID++) {
              ...
              for (int boneStateChangeID = 0;
                   boneStateChangeID < pStrip->numBoneStateChanges;
                   boneStateChangeID++) {
                WriteBoneStateChange(
                    curBoneStateChange + deltaBoneStateChange,
                    &pStrip->boneStateChanges[boneStateChangeID]);
                deltaBoneStateChange++;
              }
              WriteStrip(curStrip + deltaStrip, pStrip, curIndex, curVert,
                         curBoneStateChange);
              deltaStrip++;
              curBoneStateChange += deltaBoneStateChange;
              deltaBoneStateChange = 0;
            }
            WriteStripGroup(curStripGroup + deltaStripGroup, pStripGroup,
                            curVert, curIndex, curStrip);
            deltaStripGroup++;
            curStrip += deltaStrip;
            deltaStrip = 0;
            curVert += deltaVert;
...
static void MergeLikeBoneIndicesWithinVert(mstudioboneweight_t *pBoneWeight) {
  ...
  for (i = 0; i < pBoneWeight->numbones; i++) {
    for (j = i + 1; j < pBoneWeight->numbones; j++) {
      if ((pBoneWeight->bone[i] == pBoneWeight->bone[j]) &&
          (pBoneWeight->weight[i] != 0.0f)) {
  ...
  for (j = pBoneWeight->numbones; j > 1; j--) {
    int k;
    for (k = 0; k < j - 1; k++) {
      if ((pBoneWeight->weight[k] == 0.0f) &&
          (pBoneWeight->weight[k + 1] != 0.0f)) {
        // swap
...
void COptimizedModel::PrintBoneStateChanges(studiohdr_t *phdr, int lod) {
  ...
  for (int bodyPartID = 0; bodyPartID < header->numBodyParts; bodyPartID++) {
    BodyPartHeader_t *bodyPart = header->pBodyPart(bodyPartID);
    //		mstudiobodyparts_t *pStudioBodyPart = phdr->pBodypart(
    //bodyPartID ); 		for( int lodID = 0; lodID < header->numLODs; lodID++ )
    int lodID = lod;
    {
      for (int modelID = 0; modelID < bodyPart->numModels; modelID++) {
        ModelHeader_t *model = bodyPart->pModel(modelID);
        //				mstudiomodel_t *pStudioModel =
        //pStudioBodyPart->pModel( modelID );
        ModelLODHeader_t *pLOD = model->pLOD(lodID);
        for (int meshID = 0; meshID < pLOD->numMeshes; meshID++) {
          MeshHeader_t *mesh = pLOD->pMesh(meshID);
          //					mstudiomesh_t *pStudioMesh =
          //pStudioModel->pMesh( meshID );
          for (int stripGroupID = 0; stripGroupID < mesh->numStripGroups;
               stripGroupID++) {
            StripGroupHeader_t *pStripGroup = mesh->pStripGroup(stripGroupID);
            for (int stripID = 0; stripID < pStripGroup->numStrips; stripID++) {
        ...
        for (int meshID = 0; meshID < pLOD->numMeshes; meshID++) {
          ...
          for (int stripGroupID = 0; stripGroupID < mesh->numStripGroups;
            ...
            for (int vertID = 0; vertID < pStripGroup->numVerts; vertID++) {
              PrintVert(pStripGroup->pVertex(vertID), pStudioModel,
                        pStudioMesh);
            }
          }
        }
...
bool COptimizedModel::OptimizeFromStudioHdr(
    studiohdr_t *pHdr, s_bodypart_t *pSrcBodyParts, int vertCacheSize,
  ...
  if (m_NumSkinnedAndFlexedVerts != 0) {
    MdlWarning("!!!!WARNING!!!!: %d flexed verts had more than one bone "
               "influence. . will use SLOW path in engine\n",
...
enum { GLVIEWDRAW_TRILIST, GLVIEWDRAW_TRISTRIP, GLVIEWDRAW_NONE };
...
void COptimizedModel::PrintVert(Vertex_t *v, mstudiomodel_t *pStudioModel,
                                mstudiomesh_t *pStudioMesh) {
  printf("vert:\n");
#if 0
  ...
  for (i = 0; i < v->numBones; i++) {
    float boneWeight =
        GetOrigVertBoneWeightValue(pStudioModel, pStudioMesh, v, i);
    printf("\tboneID[%d]: %d weight: %f (%s)\n", i, (int)v->boneID[i],
           boneWeight, g_bonetable[v->boneID[i]].name);
  }
}
...
Vector &COptimizedModel::GetOrigVertPosition(mstudiomodel_t *pStudioModel,
                                             mstudiomesh_t *pStudioMesh,
                                             Vertex_t *pVert) {
...
float COptimizedModel::GetOrigVertBoneWeightValue(mstudiomodel_t *pStudioModel,
                                                  mstudiomesh_t *pStudioMesh,
  ...
  return vertData->BoneWeights(pVert->origMeshVertID)
      ->weight[pVert->boneWeightIndex[boneID]];
}

mstudioboneweight_t &COptimizedModel::GetOrigVertBoneWeight(
    mstudiomodel_t *pStudioModel, mstudiomesh_t *pStudioMesh, Vertex_t *pVert) {
  Assert(pStudioMesh->pModel() == pStudioModel);
...
int COptimizedModel::GetOrigVertBoneIndex(mstudiomodel_t *pStudioModel,
                                          mstudiomesh_t *pStudioMesh,
                                          Vertex_t *pVert, int boneID) {
  Assert(pStudioMesh->pModel() == pStudioModel);
  ...
  Assert(vertData); // This can only return NULL on X360 for now
  return vertData->BoneWeights(pVert->origMeshVertID)
      ->bone[pVert->boneWeightIndex[boneID]];
}
...
void COptimizedModel::SetFlexedAndSkinColor(unsigned int glViewFlags,
                                            unsigned int stripGroupFlags,
                                            Vector &color) {
  if ((glViewFlags & WRITEGLVIEW_SHOWFLEXED) &&
      (glViewFlags & WRITEGLVIEW_SHOWSW)) {
...
void COptimizedModel::DrawGLViewTriangle(FILE *fp, Vector &pos1, Vector &pos2,
                                         Vector &pos3, Vector &color1,
                                         Vector &color2, Vector &color3) {
  numGLViewTrangles++;
  fprintf(fp, "3\n");
  fprintf(fp, "%f %f %f %f %f %f\n", pos1[0], pos1[1], pos1[2], color1[0],
          color1[1], color1[2]);
  fprintf(fp, "%f %f %f %f %f %f\n", pos2[0], pos2[1], pos2[2], color2[0],
          color2[1], color2[2]);
  fprintf(fp, "%f %f %f %f %f %f\n", pos3[0], pos3[1], pos3[2], color3[0],
          color3[1], color3[2]);
}
...
void COptimizedModel::GLViewVert(FILE *fp, Vertex_t vert, int index,
                                 Vector &color, mstudiomodel_t *pStudioModel,
                                 mstudiomesh_t *pStudioMesh, bool showSubStrips,
                                 float shrinkFactor) {
  ...
  s_Shrunk[id] = false;
  VectorCopy(
      GetOrigVertPosition(pStudioModel, pStudioMesh, &s_LastThreeVerts[id]),
      s_LastThreePositions[id]);
  if (s_DrawMode == GLVIEWDRAW_TRILIST) {
    // trilist
    if (id == 2) {
      ShrinkVerts(shrinkFactor);
      DrawGLViewTriangle(fp, s_LastThreePositions[0], s_LastThreePositions[1],
                         s_LastThreePositions[2], color, color, color);
    }
  } else {
    // tristrip
    if (s_ListID >= 2) {
      ...
      if (s_LastThreeIndices[0] == s_LastThreeIndices[1] ||
        ...
        DrawGLViewTriangle(fp, s_LastThreePositions[(id + 0 - 2 + 3) % 3],
                           ...
                           s_LastThreePositions[(id + 2 - 2 + 3) % 3], color,
                           color, color);
      } else {
        ShrinkVerts(shrinkFactor);
        DrawGLViewTriangle(fp, s_LastThreePositions[(id + 2 - 2 + 3) % 3],
                           s_LastThreePositions[(id + 1 - 2 + 3) % 3],
                           s_LastThreePositions[(id + 0 - 2 + 3) % 3], color,
                           color, color);
      }
    }
  }
...
void COptimizedModel::DebugCrap( studiohdr_t *phdr )
{
...
)
        {
                BodyPartHeader_t *bodyPart = header->pBodyPart( bodyPartID );
                mstudiobodyparts_t *pStudioBodyPart = phdr->pBodypart(
bodyPartID ); for( int modelID = 0; modelID < bodyPart->numModels; modelID++ )
                {
                        ModelHeader_t *model = bodyPart->pModel( modelID );
                        mstudiomodel_t *pStudioModel = pStudioBodyPart->pModel(
modelID ); for( int lodID = 0; lodID < model->numLODs; lodID++ )
                        {
                                char tmp[256];
...
tmp ); return;
                                ...
                                ModelLODHeader_t *pLOD = model->pLOD( lodID );
                                for( int meshID = 0; meshID < pLOD->numMeshes;
meshID++ )
                                {
                                        MeshHeader_t *mesh = pLOD->pMesh( meshID
); mstudiomesh_t *pStudioMesh = pStudioModel->pMesh( meshID ); for( int
stripGroupID = 0; stripGroupID < mesh->numStripGroups; stripGroupID++ )
                                        {
                                                StripGroupHeader_t *pStripGroup
= mesh->pStripGroup( stripGroupID ); for( int stripID = 0; stripID <
pStripGroup->numStrips; stripID++ )
                                                {
                                                        StripHeader_t *pStrip =
pStripGroup->pStrip( stripID ); for( int indexID = 0; indexID <
pStrip->numIndices; indexID++ )
                                                        {
                                                                int id =
*pStripGroup->pIndex( indexID + pStrip->indexOffset ); Vertex_t& vert =
*pStripGroup->pVertex( id ); Vector& vertPos = GetOrigVertPosition(
pStudioModel, pStudioMesh, &vert ); fprintf( fp, "mesh: %04d origvertid: %04d
pos: %0.2f %0.2f %0.2f ", meshID, vert.origMeshVertID, vertPos[0], vertPos[1],
vertPos[2] ); int i; for( i = 0; i < vert.numBones; i++ )
                                                                {
                                                                        float
boneWeight; boneWeight = GetOrigVertBoneWeightValue( pStudioModel, pStudioMesh,
&vert, i ); int boneID; boneID = GetOrigVertBoneIndex( pStudioModel,
pStudioMesh, &vert, i ); fprintf( fp, "bone: %d %0.1f ", boneID, boneWeight );
                                                                }
                                                                fprintf( fp,
"\n" );
                                                        }
...
void COptimizedModel::WriteGLViewFile(studiohdr_t *phdr, const char *pFileName,
                                      unsigned int flags, float shrinkFactor) {
  Vector color;
  RandomColor(color);
  FileHeader_t *header = (FileHeader_t *)m_FileBuffer->GetPointer(0);
    ...
    for (int modelID = 0; modelID < bodyPart->numModels; modelID++) {
      ...
      for (int lodID = 0; lodID < model->numLODs; lodID++) {
        ...
        /*
                                        ...
                                        int vertID;
                                        for( vertID = 0; vertID <
           pStudioModel->numvertices; vertID++ )
                                        {
                                                const Vector &pos =
           *pStudioModel->pVertex( vertID ); const Vector &norm =
           *pStudioModel->pNormal( vertID ); const Vector4D &sVect =
           *pStudioModel->pTangentS( vertID ); Vector tmpVect; tmpVect = pos +
           norm * .15f; fprintf( fp, "2\n" ); fprintf( fp, "%f %f %f 0.0
           0.0 1.0\n", pos[0], pos[1], pos[2] ); fprintf( fp, "%f %f %f 0.0
           0.0 1.0\n", tmpVect[0], tmpVect[1], tmpVect[2] );

                                                Vector tmpSVect( sVect[0],
           sVect[1], sVect[2] ); tmpVect = pos + tmpSVect * .15f; fprintf( fp,
           "2\n" ); fprintf( fp, "%f %f %f 1.0 0.0 0.0\n", pos[0], pos[1],
           pos[2] ); fprintf( fp, "%f %f %f 1.0 0.0 0.0\n", tmpVect[0],
           tmpVect[1], tmpVect[2] );

                                                Vector tmpTVect;
                                                CrossProduct( norm, tmpSVect,
           tmpTVect ); tmpTVect *= sVect[3]; tmpVect = pos + tmpTVect * .15f;

                                                fprintf( fp, "2\n" );
                                                fprintf( fp, "%f %f %f 0.0 1.0
           0.0\n", pos[0], pos[1], pos[2] ); fprintf( fp, "%f %f %f 0.0 1.0
           0.0\n", tmpVect[0], tmpVect[1], tmpVect[2] );
        ...
        for (int meshID = 0; meshID < pLOD->numMeshes; meshID++) {
          ...
          for (int stripGroupID = 0; stripGroupID < mesh->numStripGroups;
            ...
            for (int stripID = 0; stripID < pStripGroup->numStrips; stripID++) {
              ...
              GLViewDrawBegin((pStrip->flags & STRIP_IS_TRILIST)
                                  ? GLVIEWDRAW_TRILIST
                                  : GLVIEWDRAW_TRISTRIP);
              for (int indexID = 0; indexID < pStrip->numIndices; indexID++) {
                ...
                GLViewVert(fp, vert, id, color, pStudioModel, pStudioMesh,
                           (flags & WRITEGLVIEW_SHOWSUBSTRIP) ? true : false,
                           shrinkFactor);
              }
              GLViewDrawEnd();
            }
...
void COptimizedModel::WriteGLViewFiles(studiohdr_t *pHdr,
                                       char const *glViewFileName) {
  ...
  WriteGLViewFile(pHdr, tmpFileName,
                  WRITEGLVIEW_SHOWSW | WRITEGLVIEW_SHOWFLEXED, .8f);
  strcpy(tmpFileName, glViewFileName);
...
void COptimizedModel::CheckVertBoneWeights(Vertex_t *pVert,
                                           mstudiomodel_t *pStudioModel,
                                           mstudiomesh_t *pStudioMesh) {
  int i;
  float sum = 0;
      ...
      for (int lodID = 0; lodID < 1; lodID++) {
        ...
        for (int meshID = 0; meshID < pLOD->numMeshes; meshID++) {
          ...
          for (int stripGroupID = 0; stripGroupID < mesh->numStripGroups;
            ...
            StripGroupHeader_t *pStripGroup = mesh->pStripGroup(stripGroupID);
            for (int stripID = 0; stripID < pStripGroup->numStrips; stripID++) {
              for (int stripID = 0; stripID < pStripGroup->numStrips;
                   stripID++) {
...
void COptimizedModel::CheckVert(Vertex_t *pVert, int maxBonesPerTri,
                                int maxBonesPerVert) {
#ifndef IGNORE_BONES

#ifdef _DEBUG
  int offset = (int)((unsigned char *)pVert -
                     (unsigned char *)m_FileBuffer->GetPointer(0));
...
void COptimizedModel::SortBonesWithinVertex(
    bool flexed, Vertex_t *vert, mstudiomodel_t *pStudioModel,
...
/*
        for( i = 0; i < m_NumBones; i++ )
        {
                if( globalToHardwareBoneIndex[i] != -1 )
                {
                        if( flexed )
                        {
                                printf( "global bone id: %d hardware bone id:
...
*/
  ...
  for (i = 0; i < MAX_NUM_BONES_PER_VERT; i++) {
    float boneWeight =
        GetOrigVertBoneWeightValue(pStudioModel, pStudioMesh, vert, i);
    if (boneWeight == 0.0f) {
      zeroWeightIndex = i;
      break;
    }
  ...
  for (i = 0; i < vert->numBones; i++) {
    float boneWeight =
        GetOrigVertBoneWeightValue(pStudioModel, pStudioMesh, vert, i);
    int globalBoneIndex =
        GetOrigVertBoneIndex(pStudioModel, pStudioMesh, vert, i);
    //		if( vert->numBones > 1 )
    {
      if (flexed) {
        printf("boneWeight: %f\n", boneWeight);
  ...
  vert->numBones =
      maxBonesPerTri; // this may be different for software t&l stuff
#endif
}
      ...
      for (int lodID = 0; lodID < model->numLODs; lodID++) {
        ...
        for (int meshID = 0; meshID < pLOD->numMeshes; meshID++) {
          ...
          for (int stripGroupID = 0; stripGroupID < mesh->numStripGroups;
            ...
            if (!(pStripGroup->flags & STRIPGROUP_IS_HWSKINNED)) {
              //							printf(
              //"skipping! software skinned stripgroup\n" );
              continue;
            }
            for (int stripID = 0; stripID < pStripGroup->numStrips; stripID++) {
              StripHeader_t *pStrip = pStripGroup->pStrip(stripID);
              ...
              */
              ...
              for (int boneStateChangeID = 0;
                ...
                if (allocated[boneStateChange->hardwareID] &&
                    hardwareBoneState[boneStateChange->hardwareID] ==
                        boneStateChange->newBoneID) {
              ...
              /*
                                                                      // now
                 "changed" should tell us which ones we care about. int
                 curOutBoneID = 0; for( i = 0; i < pStrip->numBoneStateChanges;
                 i++ )
                                                                      {
                                                                              //
                 hack . . going to stomp over what is already there with new
                 data. if( changed[i] )
                                                                              {
                                                                      ...
                                                                      pStrip->numBoneStateChanges
                 = curOutBoneID; printf( "start bone changes: %d end bone
                 changes: %d\n", startNumBoneChanges,
                 pStrip->numBoneStateChanges ); printf( "after optimization\n"
...
// Check that all replacematerial/removemesh commands map to valid source
// materials
...
void WriteOptimizedFiles(studiohdr_t *phdr, s_bodypart_t *pSrcBodyParts) {
  ...
  s_OptimizedModel.OptimizeFromStudioHdr(
      phdr, pSrcBodyParts,
  ...
  s_OptimizedModel.OptimizeFromStudioHdr(
      phdr, pSrcBodyParts, 24 /* vert cache size (real size, not effective!)*/,
      false, /* doesn't use fixed function */
