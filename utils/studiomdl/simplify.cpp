...
// this is computed once so render models and their physics hulls get translated
// by the same amount
...
//
//----------------------------------------------------------------------
void QuaternionSMAngles(float s, Quaternion const &p, Quaternion const &q,
                        RadianEuler &angles) {
  Quaternion qt;
...
void QuaternionMAAngles(Quaternion const &p, float s, Quaternion const &q,
                        RadianEuler &angles) {
  Quaternion qt;
...
void extractLinearMotion(s_animation_t *panim, int motiontype, int iStartFrame,
                         int iEndFrame, int iSrcFrame, s_animation_t *pRefAnim,
                         int iRefFrame /* , Vector pos, QAngle angles */);
...
void worldspaceBlend(s_animation_t *psrc, s_animation_t *pdest, int srcframe,
                     int flags);
void processAutoorigin(s_animation_t *psrc, s_animation_t *pdest, int flags,
                       int srcframe, int destframe, int bone);
void subtractBaseAnimations(s_animation_t *psrc, s_animation_t *pdest,
                            int srcframe, int flags);
void fixupLoopingDiscontinuities(s_animation_t *panim, int start, int end);
void matchBlend(s_animation_t *pDestAnim, s_animation_t *pSrcAnimation,
                int iSrcFrame, int iDestFrame, int iPre, int iPost);
...
void localHierarchy(s_animation_t *panim, char *pBonename, char *pParentname,
                    int start, int peak, int tail, int end);

void linearDelta(s_animation_t *psrc, s_animation_t *pdest, int srcframe,
                 int flags);
void splineDelta(s_animation_t *psrc, s_animation_t *pdest, int srcframe,
                 int flags);
...
void solveBone(s_animation_t *panim, int iFrame, int iBone,
  ...
  for (i = 0; i < g_numani; i++) {
    ...
    for (j = 0; j < panim->numcmds; j++) {
      ...
      case CMD_SUBTRACT:
        ...
        subtractBaseAnimations(pcmd->u.subtract.ref, panim,
                               pcmd->u.subtract.frame, pcmd->u.subtract.flags);
        break;
      case CMD_AO: {
        ...
        processAutoorigin(pcmd->u.ao.ref, panim, pcmd->u.ao.motiontype,
                          pcmd->u.ao.srcframe, pcmd->u.ao.destframe, bone);
      } break;
      case CMD_MATCH:
        processMatch(pcmd->u.match.ref, panim, false);
        break;
      case CMD_FIXUP:
        fixupLoopingDiscontinuities(panim, pcmd->u.fixuploop.start,
                                    pcmd->u.fixuploop.end);
        break;
      case CMD_ANGLE:
      ...
      case CMD_IKRULE:
        ...
        break;
      case CMD_MOTION: {
        extractLinearMotion(panim, pcmd->u.motion.motiontype, startframe,
                            pcmd->u.motion.iEndFrame, pcmd->u.motion.iEndFrame,
                            panim, startframe);
        startframe = pcmd->u.motion.iEndFrame;
      } break;
      case CMD_REFMOTION: {
        extractLinearMotion(panim, pcmd->u.motion.motiontype, startframe,
                            pcmd->u.motion.iEndFrame, pcmd->u.motion.iSrcFrame,
                            pcmd->u.motion.pRefAnim, pcmd->u.motion.iRefFrame);
        startframe = pcmd->u.motion.iEndFrame;
      } break;
      case CMD_DERIVATIVE: {
      ...
      case CMD_COUNTERROTATE: {
        ...
        if (bone != -1) {
          ...
          if (!pcmd->u.counterrotate.bHasTarget) {
            ...
            ConcatTransforms(rootxform, g_bonetable[bone].boneToPose,
                             defaultBoneToWorld);

            MatrixAngles(defaultBoneToWorld, target);
          } else {
            target.Init(pcmd->u.counterrotate.targetAngle[0],
                        pcmd->u.counterrotate.targetAngle[1],
      ...
      case CMD_WORLDSPACEBLEND:
        worldspaceBlend(pcmd->u.world.ref, panim, pcmd->u.world.startframe,
                        pcmd->u.world.loops);
        break;
      case CMD_MATCHBLEND:
        matchBlend(panim, pcmd->u.match.ref, pcmd->u.match.srcframe,
                   pcmd->u.match.destframe, pcmd->u.match.destpre,
                   pcmd->u.match.destpost);
        break;
      case CMD_LOCALHIERARCHY:
        localHierarchy(panim, pcmd->u.localhierarchy.pBonename,
                       pcmd->u.localhierarchy.pParentname,
                       pcmd->u.localhierarchy.start,
                       pcmd->u.localhierarchy.peak, pcmd->u.localhierarchy.tail,
                       pcmd->u.localhierarchy.end);
        // localHierarchy( panim, char	*pBonename, char *pParentname, int
        // start, int peak, int tail, int end );
        break;
      }
    }
    ...
    if (panim->motiontype) {
      ...
      if (!(panim->flags & STUDIO_LOOPING)) {
        ...
        lastframe = max(min(startframe + 1, panim->numframes - 1),
                        panim->numframes - frames - 1);
        // printf("%s : %d %d (%d)\n", panim->name, startframe, lastframe,
        // panim->numframes - 1 );
      } else {
        lastframe = panim->numframes - 1;
      }
      extractLinearMotion(panim, panim->motiontype, startframe, lastframe,
                          panim->numframes - 1, panim, startframe);
    ...
    for (n = 0; n < g_numbones; n++) {
      ...
      for (j = 0; j < g_sequence[i].groupsize[0]; j++) {
        for (k = 0; k < g_sequence[i].groupsize[1]; k++) {
          g_sequence[i].weight[n] = max(g_sequence[i].weight[n],
                                        g_sequence[i].panim[j][k]->weight[n]);
        }
      }
    }
  }
...
/*
void lookupLinearMotion( s_animation_t *panim, int motiontype, int startframe,
int endframe, Vector &p1, Vector &p2 )
{
        ...
        p1 = panim->sanim[iFrame][g_rootIndex].pos * (1 - s) +
panim->sanim[iFrame+1][g_rootIndex].pos * s;
}
*/
...
//			out of the animation and add it back on as a "piecewise
//movement" command
...
void extractLinearMotion(s_animation_t *panim, int motiontype, int iStartFrame,
  ...
  if (panim->numframes <= 1) {
    MdlError(
        "Can't extract motion from sequence %s (%s).  Check your QC options!\n",
  ...
  if (motiontype & (STUDIO_LXR | STUDIO_LYR | STUDIO_LZR)) {
    ...
    AngleQuaternion(panim->sanim[iMidFrame][g_rootIndex].rot,
                    q1); // only used for rotation checking
  ...
  Vector p1 = panim->sanim[iMidFrame][g_rootIndex].pos * (1 - s) +
              panim->sanim[iMidFrame + 1][g_rootIndex].pos * s;
  ...
  // printf("%s  %.1f %.1f %.1f\n", g_bonetable[g_rootIndex].name, p2.x, p2.y,
  // p2.z );
  ...
  if (g_verbose) {
    printf("%s : %d - %d : %.1f %.1f %.1f\n", panim->name, iStartFrame,
           iEndFrame, p2.x, p2.y, RAD2DEG(rot[2]));
  }
  ...
  for (j = 0; j < numframes; j++) {
    ...
    for (k = 0; k < g_numbones; k++) {
      if (g_bonetable[k].parent == -1) {
        // printf(" %.1f %.1f %.1f : ", adjpos[0], adjpos[1], RAD2DEG(
        // adjangle[2] ));

        // printf(" %.1f %.1f %.1f\n", adjpos[0], adjpos[1], adjpos[2] );

        AngleMatrix(panim->sanim[j + iStartFrame][k].rot,
                    panim->sanim[j + iStartFrame][k].pos, bonematrix);
        ConcatTransforms(adjmatrix, bonematrix, bonematrix);

        MatrixAngles(bonematrix, panim->sanim[j + iStartFrame][k].rot,
                     panim->sanim[j + iStartFrame][k].pos);
        // printf("%d : %.1f %.1f %.1f\n", j,
        // panim->sanim[j+iStartFrame][k].pos.x,
        // panim->sanim[j+iStartFrame][k].pos.y, RAD2DEG(
        // panim->sanim[j+iStartFrame][k].rot.z ) );
      }
    }
  }
  ...
  // printf("%d : %.1f %.1f %.1f\n", iEndFrame, pmove[0].pos.x, pmove[0].pos.y,
  // RAD2DEG( pmove[0].rot.z ) );
...
//-----------------------------------------------------------------------------
// Purpose: process the "piecewise movement" commands and return where the
// animation
//			would move to on a given frame (assuming frame 0 is at
//the origin)
//-----------------------------------------------------------------------------
...
Vector calcPosition(s_animation_t *panim, int iFrame) {
  ...
  for (int i = 0; i < panim->numpiecewisekeys; i++) {
    ...
    if (pmove->endframe >= iFrame) {
      ...
      if (iLoops != 0) {
        s_linearmove_t *pmove =
            &panim->piecewisemove[panim->numpiecewisekeys - 1];
...
//-----------------------------------------------------------------------------
// Purpose: try to calculate a "missing" frame of animation, i.e the overlapping
// frame
//-----------------------------------------------------------------------------

void fixupMissingFrame(s_animation_t *panim) {
  // the animations DIDN'T have the end frame the same as the start frame, so
  // fudge it
  ...
  for (int k = 0; k < g_numbones; k++) {
    VectorSubtract(panim->sanim[j - 1][k].pos, panim->sanim[0][k].pos,
                   deltapos);
    VectorMA(panim->sanim[j - 1][k].pos, scale, deltapos,
             panim->sanim[j][k].pos);
...
//-----------------------------------------------------------------------------
// Purpose: shift the frames of the animation so that it starts on the desired
// frame
//-----------------------------------------------------------------------------
...
void realignLooping(s_animation_t *panim) {
  ...
  if (panim->numframes > 1 && panim->looprestart) {
    if (panim->looprestart >= panim->numframes) {
      MdlError("loopstart (%d) out of range for animation %s (%d)",
               panim->looprestart, panim->name, panim->numframes);
    }
...
//-----------------------------------------------------------------------------
// Purpose: find the difference between the src and dest animations, then add
// that
...
void processMatch(s_animation_t *psrc, s_animation_t *pdest, int flags) {
  ...
  for (k = 0; k < g_numbones; k++) {
    if (flags)
      VectorSubtract(psrc->sanim[0][k].pos, pdest->sanim[0][k].pos,
                     delta_pos[k]);
  ...
  for (j = 0; j < pdest->numframes; j++) {
    for (k = 0; k < g_numbones; k++) {
      if (pdest->weight[k] > 0) {
        if (flags)
          VectorAdd(pdest->sanim[j][k].pos, delta_pos[k],
                    pdest->sanim[j][k].pos);
        QuaternionMAAngles(pdest->sanim[j][k].rot, 1.0, delta_q[k],
                           pdest->sanim[j][k].rot);
      }
    }
  }
...
//			Also, blend bone lengths, but only for non root
//animations.
//-----------------------------------------------------------------------------
  ...
  for (j = 0; j < pdest->numframes; j++) {
    ...
    for (k = 0; k < g_numbones; k++) {
      ...
      if (n == -1) {
        ...
        pdest->sanim[j][k].pos =
            Lerp(pdest->posweight[k], pdest->sanim[j][k].pos, srcPos[k]);
      }
    }
  }
}
...
//-----------------------------------------------------------------------------
// Purpose: match one animations position/orientation to another animations
// position/orientation
//-----------------------------------------------------------------------------

void processAutoorigin(s_animation_t *psrc, s_animation_t *pdest,
                       int motiontype, int srcframe, int destframe, int bone) {
  int j, k;
  matrix3x4_t adjmatrix;
  ...
  if (motiontype & (STUDIO_LXR | STUDIO_LYR | STUDIO_LZR | STUDIO_XR |
                    STUDIO_YR | STUDIO_ZR)) {
    Quaternion deltaQ2;
    QuaternionMA(q2, -1, q0, deltaQ2);
    ...
    if ((motiontype & STUDIO_XR) && (motiontype & STUDIO_YR) &&
        (motiontype & STUDIO_ZR)) {
  ...
  if (g_verbose && bone != g_rootIndex) {
    printf("%s aligning to %s - %.2f %.2f %.2f\n", pdest->name,
           g_bonetable[bone].name, adj.x, adj.y, adj.z);
  }
  ...
  for (k = 0; k < g_numbones; k++) {
    if (g_bonetable[k].parent == -1) {
      for (j = 0; j < pdest->numframes; j++) {
        ...
        ConcatTransforms(adjmatrix, bonematrix, bonematrix);
        MatrixAngles(bonematrix, pdest->sanim[j][k].rot,
                     pdest->sanim[j][k].pos);
      }
    }
  }
...
//-----------------------------------------------------------------------------
// Purpose: subtract one animaiton from animation to create an animation of the
// "difference"
//-----------------------------------------------------------------------------
    ...
    for (j = 0; j < pdest->numframes; j++) {
      if (pdest->weight[k] > 0) {
        ...
        printf("     %7.2f %7.2f %7.2f  %7.2f %7.2f %7.2f\n",
                RAD2DEG(pdest->sanim[j][k].pos[0]),
        RAD2DEG(pdest->sanim[j][k].pos[1]), RAD2DEG(pdest->sanim[j][k].pos[2]),
                RAD2DEG(pdest->sanim[j][k].rot[0]),
        ...
        */
        ...
        if (flags & STUDIO_POST) {
          ...
          QuaternionMAAngles(pdest->sanim[j][k].rot, -1, src[k].rot,
                             pdest->sanim[j][k].rot);
          VectorSubtract(src[k].pos, pdest->sanim[j][k].pos,
                         pdest->sanim[j][k].pos);
        }

        /*
        printf("     %7.2f %7.2f %7.2f  %7.2f %7.2f %7.2f\n",
                pdest->sanim[j][k].pos[0], pdest->sanim[j][k].pos[1],
        pdest->sanim[j][k].pos[2], RAD2DEG(pdest->sanim[j][k].rot[0]),
        RAD2DEG(pdest->sanim[j][k].rot[1]), RAD2DEG(pdest->sanim[j][k].rot[2])
        );
        */
      }
...
void QuaternionSlerp(const RadianEuler &r0, const RadianEuler &r1, float t,
                     RadianEuler &r2) {
  Quaternion q0, q1, q2;
...
//-----------------------------------------------------------------------------
// Purpose: subtract each frame running interpolation of the first frame to the
// last frame
//-----------------------------------------------------------------------------
      ...
      if (pdest->weight[k] > 0) {
        ...
        */
        ...
        if (flags & STUDIO_AL_POST) {
          // find pdest in src's reference frame
          QuaternionSMAngles(-1, src.rot, pdest->sanim[j][k].rot,
                             pdest->sanim[j][k].rot);
          VectorSubtract(pdest->sanim[j][k].pos, src.pos,
                         pdest->sanim[j][k].pos);
        } else {
          // find src in pdest's reference frame?
          QuaternionMAAngles(pdest->sanim[j][k].rot, -1, src.rot,
                             pdest->sanim[j][k].rot);
          VectorSubtract(src.pos, pdest->sanim[j][k].pos,
                         pdest->sanim[j][k].pos);
        }
...
//-----------------------------------------------------------------------------
// Purpose: clip or pad the animation as nessesary to be a specified number of
// frames
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: subtract each frame from the previous to calculate the animations
// derivative
//-----------------------------------------------------------------------------
      ...
      if (panim->weight[k] > 0) {
        /*
        {
                printf("%2d : %7.2f %7.2f %7.2f  %7.2f %7.2f %7.2f\n",
                        ...
                        RAD2DEG(psrc[k].rot[0]), RAD2DEG(psrc[k].rot[1]),
        RAD2DEG(psrc[k].rot[2]) );

                printf("     %7.2f %7.2f %7.2f  %7.2f %7.2f %7.2f\n",
                        pdest[k].pos[0], pdest[k].pos[1], pdest[k].pos[2],
                        RAD2DEG(pdest[k].rot[0]), RAD2DEG(pdest[k].rot[1]),
        RAD2DEG(pdest[k].rot[2]) );
        }
        */
...
void BuildRawTransforms(const s_source_t *psource, const char *pAnimationName,
                        int frame, float scale, Vector const &shift,
                        RadianEuler const &rotate, int flags,
  ...
  for (k = 0; k < psource->numbones; k++) {
    ...
    if (psource->localBone[k].parent == -1) {
      ...
      ConcatTransforms(boneToWorld[psource->localBone[k].parent], bonematrix,
                       boneToWorld[k]);
      // ConcatTransforms( worldToBone[psource->localBone[k].parent],
      // boneToWorld[k], bonematrix ); B * C => A C <= B-1 * A
    }
  }
}

void BuildRawTransforms(const s_source_t *psource, const char *pAnimationName,
                        int frame, matrix3x4_t *boneToWorld) {
  BuildRawTransforms(psource, pAnimationName, frame, 1.0f, Vector(0, 0, 0),
                     RadianEuler(0, 0, 0), 0, boneToWorld);
}

//-----------------------------------------------------------------------------
// Purpose: convert source bone animation into global bone animation
//-----------------------------------------------------------------------------
void TranslateAnimations(const s_source_t *pSource,
  ...
  for (int k = 0; k < g_numbones; k++) {
    ...
    if (q == -1) {
      ...
      if (g_bonetable[k].parent >= 0) {
        ...
        ConcatTransforms(pDestBoneToWorld[g_bonetable[k].parent], bonematrix,
                         pDestBoneToWorld[k]);
      } else {
        AngleMatrix(g_bonetable[k].rot, g_bonetable[k].pos,
                    pDestBoneToWorld[k]);
      }
    } else {
      ConcatTransforms(pSrcBoneToWorld[q], g_bonetable[k].srcRealign,
                       pDestBoneToWorld[k]);
    }
  }
}
...
// Purpose: convert source bone animation into global bone animation
//-----------------------------------------------------------------------------
void ConvertAnimation(const s_source_t *psource, const char *pAnimationName,
                      int frame, float scale, Vector const &shift,
  ...
  BuildRawTransforms(psource, pAnimationName, frame, scale, shift, rotate, 0,
                     srcBoneToWorld);
  ...
  */
  ...
  // convert source_space_local_to_world transforms to
  // shared_space_local_to_world transforms
  for (k = 0; k < g_numbones; k++) {
    if (g_bonetable[k].parent == -1) {
      MatrixCopy(destBoneToWorld[k], bonematrix);
    } else {
      // convert my transform into parent relative space
      ConcatTransforms(destWorldToBone[g_bonetable[k].parent],
                       destBoneToWorld[k], bonematrix);

      // printf("%s : %s\n", psource->localBone[q2].name,
      // psource->localBone[q].name );
...
//-----------------------------------------------------------------------------
// Purpose: copy the raw animation data from the source files into the
...
void RemapAnimations(void) {
  ...
  for (i = 0; i < g_numani; i++) {
    ...
    // printf("%s %d:%d\n", g_panimation[i]->filename,
    // g_panimation[i]->startframe, pSourceAnim->startframe );
    for (j = 0; j < panim->numframes; j++) {
      panim->sanim[j] = (s_bone_t *)kalloc(1, size);

      ConvertAnimation(psource, panim->animationname, n + j, panim->scale,
  ...
  for (i = 0; i < g_numweightlist; i++) {
    ...
    for (j = 0; j < g_numbones; j++) {
      if (g_weightlist[i].weight[j] < 0.0) {
        if (g_bonetable[j].parent != -1) {
          g_weightlist[i].weight[j] =
              g_weightlist[i].weight[g_bonetable[j].parent];
          g_weightlist[i].posweight[j] =
              g_weightlist[i].posweight[g_bonetable[j].parent];
        }
      }
    }
  }
...
void addDeltas(s_animation_t *panim, int frame, float s, Vector delta_pos[],
               Quaternion delta_q[]) {
  for (int k = 0; k < g_numbones; k++) {
    if (panim->weight[k] > 0) {
      QuaternionSMAngles(s, delta_q[k], panim->sanim[frame][k].rot,
                         panim->sanim[frame][k].rot);
      VectorMA(panim->sanim[frame][k].pos, s, delta_pos[k],
               panim->sanim[frame][k].pos);
    }
  }
}
...
// start:	negative number, specifies how far back from the end to start
// blending end:		positive number, specifies how many frames from
// the beginning to blend
//-----------------------------------------------------------------------------
...
void fixupLoopingDiscontinuities(s_animation_t *panim, int start, int end) {
  ...
  for (k = 0; k < g_numbones; k++) {
    VectorSubtract(panim->sanim[m][k].pos, panim->sanim[0][k].pos,
                   delta_pos[k]);
    QuaternionMA(panim->sanim[m][k].rot, -1, panim->sanim[0][k].rot,
                 delta_q[k]);
    QAngle ang;
  ...
  // FIXME: remove when "global" extraction moved into normal ordered processing
  // loop
  for (k = 0; k < g_numbones; k++) {
    if (g_bonetable[k].parent == -1) {
...
void matchBlend(s_animation_t *pDestAnim, s_animation_t *pSrcAnimation,
  ...
  for (k = 0; k < g_numbones; k++) {
    VectorSubtract(pSrcAnimation->sanim[iSrcFrame][k].pos,
                   pDestAnim->sanim[iDestFrame][k].pos, delta_pos[k]);
    QuaternionMA(pSrcAnimation->sanim[iSrcFrame][k].rot, -1,
                 pDestAnim->sanim[iDestFrame][k].rot, delta_q[k]);
    /*
    QAngle ang;
    QuaternionAngles( delta_q[k], ang );
    printf("%2d  %.1f %.1f %.1f\n", k, ang.x, ang.y, ang.z );
...
//-----------------------------------------------------------------------------
// Purpose: calculate an single bones animation in a different parent's
// reference frame
//-----------------------------------------------------------------------------
...
void localHierarchy(s_animation_t *panim, char *pBonename, char *pParentname,
  ...
  if (pRule->start == 0 && pRule->peak == 0 && pRule->tail == 0 &&
      pRule->end == 0) {
  ...
  pRule->localData.pError = (s_streamdata_t *)kalloc(pRule->localData.numerror,
                                                     sizeof(s_streamdata_t));
  ...
  for (k = 0; k < pRule->localData.numerror; k++) {
    ...
    BuildRawTransforms(panim->source, pAnimationName,
                       k + pRule->start + panim->startframe -
                           pSourceAnim->startframe,
                       panim->scale, panim->adjust, panim->rotation,
                       panim->flags, srcBoneToWorld);
    ...
    MatrixAngles(local, pRule->localData.pError[k].q,
                 pRule->localData.pError[k].pos);
...
void makeAngle(s_animation_t *panim, float angle) {
  ...
  if (panim->numpiecewisekeys != 0) {
    ...
    for (int i = 0; i < panim->numpiecewisekeys; i++) {
      VectorYawRotate(panim->piecewisemove[i].pos, da,
                      panim->piecewisemove[i].pos);
      VectorYawRotate(panim->piecewisemove[i].vector, da,
                      panim->piecewisemove[i].vector);
    }
  } else {
    // look for movement in root bone
    Vector pos = panim->sanim[(panim->numframes - 1)][g_rootIndex].pos -
                 panim->sanim[0][g_rootIndex].pos;
    if (pos[0] != 0 || pos[1] != 0) {
      float a = atan2(pos[1], pos[0]) * (180 / M_PI);
      da = angle - a;
    }
...
void solveBone(s_animation_t *panim, int iFrame, int iBone,
  ...
  if (iParent == -1) {
    MatrixAngles(pBoneToWorld[iBone], panim->sanim[iFrame][iBone].rot,
  ...
  MatrixAngles(local, panim->sanim[iFrame][iBone].rot,
               panim->sanim[iFrame][iBone].pos);
}

//-----------------------------------------------------------------------------
// Purpose: calc the influence of a ik rule for a specific point in the
// animation cycle
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Lock the ik target to a specific location in order to clean up bad
// animations (shouldn't be needed).
//-----------------------------------------------------------------------------
void fixupIKErrors(s_animation_t *panim, s_ikrule_t *pRule) {
  int k;
  ...
  case IK_GROUND: {
    ...
    // printf("%d %d %d %d (%d)\n", pRule->start, pRule->peak, pRule->tail,
    // pRule->end, pRule->errorData.numerror );
    for (k = 0; k < pRule->errorData.numerror; k++) {
      CalcBoneTransforms(panim, k + pRule->start, boneToWorld);

      float cycle = (panim->numframes <= 1)
                        ? 0
                        : (float)(k + pRule->start) / (panim->numframes - 1);
      float s = IKRuleWeight(pRule, cycle);
      ...
      Vector pos =
          (footfall + calcMovement(panim, k + pRule->start, pRule->contact)) *
              s +
          orig * (1.0 - s);

      // printf("%d (%.1f:%.1f) : %.1f %.1f %1.f\n", k + pRule->start, cycle, s,
      // pos.x, pos.y, pos.z );

      Studio_SolveIK(g_ikchain[pRule->chain].link[0].bone,
                     g_ikchain[pRule->chain].link[1].bone,
                     g_ikchain[pRule->chain].link[2].bone, pos, boneToWorld);

      solveBone(panim, k + pRule->start, g_ikchain[pRule->chain].link[0].bone,
                boneToWorld);
      solveBone(panim, k + pRule->start, g_ikchain[pRule->chain].link[1].bone,
                boneToWorld);
      solveBone(panim, k + pRule->start, g_ikchain[pRule->chain].link[2].bone,
                boneToWorld);
    }
  }
  }
...
//-----------------------------------------------------------------------------
// Purpose: map the vertex animations to their equivalent vertex in the base
// animations
//-----------------------------------------------------------------------------
static void ComputeSideAndScale(const s_flexkey_t &flexKey,
                                s_vertanim_t *pVAnim, float *pSide,
                                float *pScale) {
  *pScale = 1.0f;
  *pSide = 0.0f;
...
static void ComputeVertexAnimationSpeed(s_flexkey_t &flexKey) {
  ...
  for (int m = 0; m < flexKey.numvanims; m++) {
    if (flexKey.decay == 0.0f) {
      ...
      flexKey.vanim[m].speed =
          clamp(flexKey.vanim[m].pos.Length() / (flScale * flexKey.decay), 0.0f,
                1.0f);
    }
  }
}
...
// animations
//-----------------------------------------------------------------------------
static void BuildVAnimFlags(s_source_t *pVSource, s_sourceanim_t *pVSourceAnim,
                            int nCurrentFlexKey) {
  pVSourceAnim->vanim_flag = (int *)kalloc(pVSource->numvertices, sizeof(int));
  for (int n = nCurrentFlexKey; n < g_numflexkeys; n++) {
    // make sure it's the current flex file and that it's not frame 0 (happens
    // with eyeball stuff).
    ...
    const s_sourceanim_t *pAnim =
        FindSourceAnim(g_flexkey[n].source, g_flexkey[n].animationname);
    if (!pAnim)
      continue;

    if (pAnim->newStyleVertexAnimations !=
        pVSourceAnim->newStyleVertexAnimations)
      continue;
...
//-----------------------------------------------------------------------------
// Purpose: Build an array indexed by model vertex which indicates which vanim
// vertex corresponds best to it
//-----------------------------------------------------------------------------
static void BuildModelToVAnimMap(s_source_t *pVSource,
  ...
  static float
      imapdot[MAXSTUDIOVERTS]; // dot product of src norm to vanim normal
  Vector tmp;
  ...
  for (int j = 0; j < pVSource->numvertices; j++) {
    ...
    for (int k = 0; k < pmLodSource->numvertices; k++) {
      ...
      // TODO: take this out when the lod clamping stuff gets moved into the LOD
      // code instead of being a post process
      if (nMinLod && !(pmLodSource->vertex[k].lodFlag & (0xFFFFFF << nMinLod)))
        continue;

      const Vector &vecModelPos = bNewVertexAnimations
                                      ? pVSource->vertex[j].position
      ...
      const Vector &vecModelNormal = bNewVertexAnimations
                                         ? pVSource->vertex[j].normal
                                         : pVSourceAnim->vanim[0][j].normal;
      float flDot = DotProduct(pmLodSource->vertex[k].normal, vecModelNormal);
      if (flDist < imapdist[k] ||
          (flDist == imapdist[k] && flDot > imapdot[k])) {
        imapdist[k] = flDist;
        imapdot[k] = flDot;
        pModelToVAnim[k] = j;
      }
    ...
    if (flMinDist > 0.01) {
      ...
      // printf("%.4f %.4f %.4f\n", pvsource->vanim[0][j].pos[0],
      // pvsource->vanim[0][j].pos[1], pvsource->vanim[0][j].pos[2] );
    }
    ...
    if (n == -1) {
      ...
      // pVSourceAnim->vanim[0][j].pos[0], pVSourceAnim->vanim[0][j].pos[1],
      // pVSourceAnim->vanim[0][j].pos[2] );
    }
  }
  ...
  {
  printf("%4d : %7.4f  %7.4f : %5d", j, imapdist[j], imapdot[j],
  model_to_vanim_vert_imap[j] ); printf(" : %8.4f %8.4f %8.4f", modelpos[j][0],
  modelpos[j][1], modelpos[j][2] ); printf("\n");
  }
  */
  ...
  printf("%4d : %8.2f %8.2f %8.2f : ", j, pvsource->vanim[0][j].pos[0],
  pvsource->vanim[0][j].pos[1], pvsource->vanim[0][j].pos[2] ); for (k = 0; k <
  pmsource->numvertices; k++)
  {
...
// vertex corresponds best to it
//-----------------------------------------------------------------------------
static void BuildVAnimMap(s_source_t *pVSource, s_sourceanim_t *pVSourceAnim,
                          s_loddata_t *pmLodSource, const int *pModelToVAnim) {
  ...
  int n = 0;
  pVSourceAnim->vanim_mapcount =
      (int *)kalloc(pVSource->numvertices, sizeof(int));
  for (int j = 0; j < pmLodSource->numvertices; j++) {
...
//-----------------------------------------------------------------------------
static void AllocateDestVAnim(s_flexkey_t &flexKey,
                              s_sourceanim_t *pVSourceAnim) {
...
void RemapVertexAnimations(void) {
  ...
  static int model_to_vanim_vert_imap[MAXSTUDIOVERTS]; // model vert to vanim
                                                       // vert mapping

  // for all the sources of flexes, find a mapping of vertex animations to base
  // model. There can be multiple "vertices" in the base model for each animated
  // vertex since vertices are duplicated along material boundaries.
  for (i = 0; i < g_numflexkeys; i++) {
    s_source_t *pVSource = g_flexkey[i].source;
    s_sourceanim_t *pVSourceAnim =
        FindSourceAnim(pVSource, g_flexkey[i].animationname);
    ...
    // flag all the vertices that animate (builds the vanim_flag field of the
    // source anim)
    BuildVAnimFlags(pVSource, pVSourceAnim, i);

    s_loddata_t *pLodData = g_model[g_flexkey[i].imodel]->m_pLodData;

    // Map vertex indices specified in the model to ones specified in the vanim
    // data
    BuildModelToVAnimMap(pVSource, pVSourceAnim, pLodData, false,
                         model_to_vanim_vert_imap);
  ...
  if (g_defaultflexkey) {
    ...
    for (m = 0; m < numsrcanims; m++) {
      if (pSourceAnim
              ->vanim_mapcount[psrcanim->vertex]) // bah, only do it for ones
                                                  // that found a match!
      {
        for (n = 0; n < pSourceAnim->vanim_mapcount[psrcanim->vertex]; n++) {
          ...
          // FIXME: the "default" pose needs to be in each vertex animation
          // source since it's likely that the vertices won't be numbered the
          // same in each file.
          VectorCopy(psrcanim->pos,
                     pSourceAnim->vanim[0][psrcanim->vertex].pos);
          VectorCopy(psrcanim->normal,
                     pSourceAnim->vanim[0][psrcanim->vertex].normal);
        }
      }
      psrcanim++;
    }
  ...
  for (i = 0; i < g_numflexkeys; i++) {
    ...
    // frame 0 is special.  Always assume zero vertex animations
    int numsrcanims = (g_flexkey[i].frame != 0)
                          ? pSourceAnim->numvanims[g_flexkey[i].frame]
                          : 0;
    ...
    for (m = 0; m < numsrcanims; m++, psrcanim++) {
      ...
      // VectorSubtract( psrcanim->pos,
      // pSourceAnim->vanim[0][psrcanim->vertex].pos, tmp ); VectorTransform(
      // tmp, pmsource->bonefixup[k].im, delta );
      VectorSubtract(psrcanim->pos, pSourceAnim->vanim[0][psrcanim->vertex].pos,
                     delta);

      // VectorSubtract( psrcanim->normal,
      // pSourceAnim->vanim[0][psrcanim->vertex].normal, tmp ); VectorTransform(
      // tmp, pmsource->bonefixup[k].im, ndelta );
      VectorSubtract(psrcanim->normal,
                     pSourceAnim->vanim[0][psrcanim->vertex].normal, ndelta);
      ...
      if (DotProduct(delta, delta) <= (0.001f * 0.001f) /* 0.0001 */ &&
          DotProduct(ndelta, ndelta) <= 0.001) {
        // printf("%4d  %6.4f %6.4f %6.4f\n", pdestanim->vertex, delta.x,
        // delta.y, delta.z );
        continue;
      }
      ...
      for (n = 0; n < pSourceAnim->vanim_mapcount[psrcanim->vertex]; n++) {
        ...
        pdestanim->vertex,
        // pmsource->vertex[pdestanim->vertex][0],
        pmsource->vertex[pdestanim->vertex][1],
        pmsource->vertex[pdestanim->vertex][2], modelpos[pdestanim->vertex][0],
        modelpos[pdestanim->vertex][1], modelpos[pdestanim->vertex][2],
        psrcanim->vertex,
...
static int SortFlexKeys(s_flexkey_t **ppSortedFlexKeys) {
  ...
  if (nSortedFlexKeyCount > 0) {
    qsort(ppSortedFlexKeys, nSortedFlexKeyCount, sizeof(s_flexkey_t *),
          FlexKeysSortFunc);
  }
...
static void RemapVertexAnimationsNewVersion(void) {
  ...
  s_flexkey_t **ppSortedFlexKeys =
      (s_flexkey_t **)_alloca(g_numflexkeys * sizeof(s_flexkey_t *));
  ...
  for (int i = 0; i < nSortedFlexKeyCount; i++) {
    ...
    s_sourceanim_t *pVSourceAnim =
        FindSourceAnim(pVSource, pFlexKey->animationname);
    s_loddata_t *pLodSource = g_model[pFlexKey->imodel]->m_pLodData;

    if (pVSource != pVLastSource) {
      // Map vertex indices specified in the model to ones specified in the
      // vanim data
      BuildModelToVAnimMap(pVSource, NULL, pLodSource, true,
                           model_to_vanim_vert_imap);
    ...
    for (j = i + 1; j < nSortedFlexKeyCount; ++j) {
      if ((ppSortedFlexKeys[j]->source != pVSource) ||
          Q_stricmp(ppSortedFlexKeys[j]->animationname,
                    pFlexKey->animationname))
        break;
    }
  ...
  for (int i = 0; i < g_numflexkeys; i++) {
    ...
    for (int m = 0; m < nNumSrcVAnims; m++, pSrcVAnim++) {
      ...
      // FIXME: the clamp needs to be paired with the other matching positions.
      // currently this is set to the float16 min value.  Sucky.
      if (DotProduct(pSrcVAnim->pos, pSrcVAnim->pos) <=
              (0.001f * 0.001f) /* 0.0001 */
          && DotProduct(pSrcVAnim->normal, pSrcVAnim->normal) <= 0.001f &&
      ...
      for (int n = 0; n < pVSourceAnim->vanim_mapcount[pSrcVAnim->vertex];
           n++) {
...
bool IsGlobalBoneXSI(const char *name, const char *bonename) {
  ...
  if (len2 == len && strchr(bonename, '.') == NULL &&
      stricmp(bonename, name) == 0)
    return true;
...
void findAnimQuaternionAlignment(int k, int i, Quaternion &qBase,
  ...
  for (j = 1; j < g_panimation[i]->numframes; j++) {
    ...
    /*
    if (i != 0)
            printf("%f %f %f : %f\n", d0, d1, d2, QuaternionDotProduct( qMin,
    qMax ) );
    */
    if (d1 >= d0) {
      if (d0 < dMin) {
  ...
  // printf("%s (%s): %.3f :%.3f\n", g_bonetable[k].name, g_panimation[i]->name,
  // QuaternionDotProduct( qMin, qMax ), QuaternionDotProduct( qMin, qBase ) );
  /*
  if (i != 0)
          exit(0);
  */
...
//			that their mid point of rotation can be used at run time
//to "pre-align" 			the quaternions so that rotations > 180 degrees don't get
//blended the 			"short way round".
//-----------------------------------------------------------------------------
  ...
  for (i = 0; i < g_numlimitrotation; i++) {
    ...
    if (g_limitrotation[i].numseq == 0) {
      for (j = 0; j < g_numani; j++) {
        if (!(g_panimation[j]->flags & STUDIO_DELTA) &&
            g_panimation[j]->numframes > 3) {
...
void limitIKChainLength(void) {
  ...
  for (k = 0; k < g_numikchains; k++) {
      ...
      for (i = 0; i < g_numani; i++) {
        ...
        for (j = 0; j < panim->numframes; j++) {
          ...
          if (l3 > (l1 + l2) * 0.999) {
            ...
            Vector tmp;
            VectorIRotate(ikKneeDir, boneToWorld[g_ikchain[k].link[0].bone],
                          tmp);
            float bend =
                (((DotProduct(worldThigh - worldKnee, worldFoot - worldKnee)) /
                  (l1 * l3)) +
                 1) /
                2.0;
            kneeDir += tmp * bend;
    ...
    if (g_verbose) {
      printf("knee %s %f %f %f\n", g_ikchain[k].name, kneeDir.x, kneeDir.y,
             kneeDir.z);
    }
...
void MakeTransitions() {
  ...
  for (i = 0; i < g_sequence.Count(); i++) {
    if (g_sequence[i].entrynode != g_sequence[i].exitnode) {
      g_xnode[g_sequence[i].entrynode - 1][g_sequence[i].exitnode - 1] =
          g_sequence[i].exitnode;
      if (g_sequence[i].nodeflags) {
        g_xnode[g_sequence[i].exitnode - 1][g_sequence[i].entrynode - 1] =
            g_sequence[i].entrynode;
      }
    }
  }
...
int RadianEulerCompareEpsilon(const RadianEuler &v1, const RadianEuler &v2,
                              float epsilon) {
  int i;
...
bool AnimationDifferent(const Vector &startPos, const RadianEuler &startRot,
                        const Vector &pos, const RadianEuler &rot) {
...
bool BoneHasAnimation(const char *pName) {
  ...
  for (int i = 0; i < g_numani; i++) {
    ...
    // this is not right, but enough of the bones are moved unintentionally
    // between animations that I put this in to catch them.
    first = true;
    int n = g_panimation[i]->startframe - pSourceAnim->startframe;
    // printf("%s %d:%d\n", g_panimation[i]->filename,
    // g_panimation[i]->startframe, psource->startframe );
    for (int j = 0; j < g_panimation[i]->numframes; j++) {
      if (first) {
        ...
        if (AnimationDifferent(pos, rot,
                               pSourceAnim->rawanim[j + n][boneIndex].pos,
...
//-----------------------------------------------------------------------------
// Purpose: Collapse vertex assignments up to parent on bones that are not
// needed
//			This can optimize a model substantially if the animator
//is using 			lots of helper bones with no animation.
//-----------------------------------------------------------------------------
void CollapseBones(void) {
  ...
  for (k = 0; k < g_numbones; k++) {
    ...
    if ((g_bonetable[k].flags != 0 || g_bonetable[k].bPreDefined) &&
        !BoneShouldCollapse(g_bonetable[k].name)) {
...
//-----------------------------------------------------------------------------
// Purpose: replace all animation, rotation and translation, etc. with a single
// bone
//-----------------------------------------------------------------------------
void MakeStaticProp() {
  int i, j, k;
  ...
  for (i = 0; i < g_numsources; i++) {
    ...
    if (g_centerstaticprop) {
      ...
      if (!bFound) {
        ...
        AngleMatrix(vec3_angle, g_PropCenterOffset,
  ...
  Q_strncpy(g_panimation[0]->animationname, "BindPose",
            sizeof(g_panimation[0]->animationname));
...
//-----------------------------------------------------------------------------
static void UpdateBonerefRecursive(s_source_t *psource, int nBoneIndex,
                                   int nFlags) {
  if (nFlags == 0)
    return;
...
int GetRemappedBoneAxis(int nBoneIndex, int nAxis) {
  ...
  // Y Up
  if (g_defaultrotation.x == static_cast<float>(M_PI / 2.0f) &&
...
void MapFlexDriveBonesToGlobalBoneTable() {
  CDmeBoneFlexDriverList *pDmeBoneFlexDriverList =
      GetElement<CDmeBoneFlexDriverList>(g_hDmeBoneFlexDriverList);
  if (!pDmeBoneFlexDriverList)
    return;

  // Loop backwards so we can remove elements as we go
  for (int i = pDmeBoneFlexDriverList->m_eBoneFlexDriverList.Count() - 1;
       i >= 0; --i) {
    CDmeBoneFlexDriver *pDmeBoneFlexDriver =
        pDmeBoneFlexDriverList->m_eBoneFlexDriverList[i];
    ...
    for (int j = 0; j < g_numbones; ++j) {
      if (!Q_stricmp(g_bonetable[j].name,
                     pDmeBoneFlexDriver->m_sBoneName.Get())) {
        if (g_bonetable[j].flags & BONE_ALWAYS_PROCEDURAL) {
          MdlWarning("DmeBoneFlexDriver Bone: %s is marked procedural, "
                     "Ignoring flex drivers\n",
                     pDmeBoneFlexDriver->m_sBoneName.Get());
          pDmeBoneFlexDriverList->m_eBoneFlexDriverList.Remove(i);
        ...
        for (int k = 0; k < pDmeBoneFlexDriver->m_eControlList.Count(); ++k) {
          pDmeBoneFlexDriver->m_eControlList[k]->m_nBoneComponent =
              GetRemappedBoneAxis(
                  j, pDmeBoneFlexDriver->m_eControlList[k]->m_nBoneComponent);
        }
        break;
      }
    }
    ...
    CDmAttribute *pBoneIndexAttr =
        pDmeBoneFlexDriver->GetAttribute("__boneIndex");
    if (pBoneIndexAttr) {
      pBoneIndexAttr->AddFlag(FATTRIB_DONTSAVE);
    } else {
      MdlWarning("DmeBoneFlexDriver Bone: %s - No Bone Found With That Name, "
                 "Ignoring\n",
                 pDmeBoneFlexDriver->m_sBoneName.Get());
      pDmeBoneFlexDriverList->m_eBoneFlexDriverList.Remove(i);
    }
...
//-----------------------------------------------------------------------------
// Purpose: Tag bones in the specified source that are used as a bone flex
// driver
...
void TagFlexDriverBones(s_source_t *pSource) {
  ...
  for (int i = pDmeBoneFlexDriverList->m_eBoneFlexDriverList.Count() - 1;
    ...
    for (int j = pDmeBoneFlexDriver->m_eControlList.Count() - 1; j >= 0; --j) {
      CDmeBoneFlexDriverControl *pDmeBoneFlexDriverControl =
          pDmeBoneFlexDriver->m_eControlList[j];
      ...
      if (pDmeBoneFlexDriverControl->m_nBoneComponent < STUDIO_BONE_FLEX_TX ||
        ...
        MdlWarning("DmeBoneFlexDriver Bone: %s - Flex Controller: %s, Bone "
                   "Component Out Of Range: %d [0-2], Ignoring\n",
                   pDmeBoneFlexDriver->m_sBoneName.Get(),
      ...
      if (!pDmeBoneFlexDriverControl->HasAttribute("__flexControlIndex")) {
        MdlWarning("DmeBoneFlexDriver Bone: %s - No Flex Controller Named: %s, "
                   "Ignoring\n",
                   pDmeBoneFlexDriver->m_sBoneName.Get(),
                   pDmeBoneFlexDriverControl->m_sFlexControllerName.Get());
        pDmeBoneFlexDriver->m_eControlList.Remove(j);
    ...
    if (pDmeBoneFlexDriver->m_eControlList.Count() <= 0) {
      MdlWarning("DmeBoneFlexDriver Bone: %s - No Flex Controllers Defined, "
                 "Ignoring\n",
                 pDmeBoneFlexDriver->m_sBoneName.Get());
      pDmeBoneFlexDriverList->m_eBoneFlexDriverList.Remove(i);
      continue;
    ...
    for (int j = 0; j < pSource->numbones; ++j) {
      if (!Q_stricmp(pSource->localBone[j].name,
                     pDmeBoneFlexDriver->m_sBoneName.Get())) {
...
//-----------------------------------------------------------------------------
// Purpose: set "boneref" for all the source bones used by vertices,
// attachments, eyeballs, etc.
//-----------------------------------------------------------------------------
void TagUsedBones() {
  int i, j, k;
  int n;
  ...
  for (i = 0; i < g_numsources; i++) {
    ...
    for (j = 0; j < psource->numvertices; j++) {
      for (k = 0; k < psource->vertex[j].boneweight.numbones; k++) {
        psource->boneflags[psource->vertex[j].boneweight.bone[k]] |=
  ...
  for (i = 0; i < g_numsources; i++) {
    ...
    // FIXME: this is in the wrong place.  The attachment may be rigid and it
    // never defined in a reference file
    for (k = 0; k < g_numattachments; k++) {
...
//-----------------------------------------------------------------------------
// Purpose: change the names in the source files for bones that max auto-renamed
// on us
//-----------------------------------------------------------------------------
void RenameBones() {
  int i, j, k;
  ...
  for (i = 0; i < g_numimportbones; i++) {
    ...
    if (k == -1) {
      ...
      if (strlen(g_importbone[i].parent) == 0) {
        ...
        if (g_bonetable[k].parent == -1) {
          Warning(
              "Imported bone %s tried to access parent bone %s and failed!\n",
  ...
  for (i = 0; i < g_numsources; i++) {
    ...
    for (j = 0; j < psource->numbones; j++) {
      ...
      if (k == -1) {
        ...
        if (g_bonetable[k].parent == -1 ||
            !g_bonetable[g_bonetable[k].parent].bPreAligned) {
          AngleMatrix(pSourceAnim->rawanim[0][j].rot,
                      pSourceAnim->rawanim[0][j].pos, g_bonetable[k].rawLocal);
          g_bonetable[k].rawLocalOriginal = g_bonetable[k].rawLocal;
        } else {
          // convert the local relative position into a realigned relative
          // position
          matrix3x4_t srcParentBoneToWorld;
          ConcatTransforms(srcBoneToWorld[n],
                           g_bonetable[g_bonetable[k].parent].srcRealign,
                           srcParentBoneToWorld);
          matrix3x4_t invSrcParentBoneToWorld;
          MatrixInvert(srcParentBoneToWorld, invSrcParentBoneToWorld);
          ConcatTransforms(invSrcParentBoneToWorld, srcBoneToWorld[j],
                           g_bonetable[k].rawLocal);
        }

        g_bonetable[k].boneToPose.Invalidate();

        // printf("%d : %s (%s)\n", k, g_bonetable[k].name,
        // g_bonetable[g_bonetable[k].parent].name );
        g_numbones++;
        continue;
      }
      ...
      if (g_bOverridePreDefinedBones && g_bonetable[k].bPreDefined) {
        ...
        ConcatTransforms(srcBoneToWorld[j], g_bonetable[k].srcRealign,
        ...
        if (g_bonetable[k].parent == -1) {
          ...
          ConcatTransforms(tmp, g_bonetable[k].boneToPose,
                           g_bonetable[k].rawLocal);
        }
        continue;
      }
...
void BuildGlobalBoneToPose() {
  ...
  for (k = 0; k < g_numbones; k++) {
    if (g_bonetable[k].parent == -1) {
      ...
      ConcatTransforms(g_bonetable[g_bonetable[k].parent].boneToPose,
...
void RebuildLocalPose() {
  ...
  for (k = 0; k < g_numbones; k++) {
    if (g_bonetable[k].parent == -1) {
      ...
      ConcatTransforms(poseToBone[g_bonetable[k].parent], boneToPose[k],
                       g_bonetable[k].rawLocal);
    }
    MatrixAngles(g_bonetable[k].rawLocal, g_bonetable[k].rot,
                 g_bonetable[k].pos);
...
void EnforceHierarchy() {
  ...
  for (i = 0; i < g_numforcedhierarchy; i++) {
    ...
    if (j > k)
    {
            MdlError( "parent \"%s\" declared after child \"%s\" in forced
    hierarchy\n", g_forcedhierarchy[i].parentname,
    g_forcedhierarchy[i].childname );
    }
    */
    ...
    if (strlen(g_forcedhierarchy[i].subparentname) != 0)
    {
            ...
            if (m != -1)
            {
                    MdlError( "inserted bone \"%s\" matches name of existing
    bone in hierarchy\n", g_forcedhierarchy[i].parentname,
...
void TagProceduralBones() {
  ...
  for (j = 0; j < g_numaxisinterpbones; j++) {
    ...
    g_axisinterpbones[j].control =
        findGlobalBone(g_axisinterpbones[j].controlname);
    ...
    g_bonetable[g_axisinterpbones[j].bone].flags |=
        BONE_ALWAYS_PROCEDURAL; // ??? what about physics rules
  ...
  for (j = 0; j < g_numquatinterpbones; j++) {
    g_quatinterpbones[j].bone =
        findGlobalBoneXSI(g_quatinterpbones[j].bonename);
    g_quatinterpbones[j].control =
        findGlobalBoneXSI(g_quatinterpbones[j].controlname);
    ...
    g_bonetable[g_quatinterpbones[j].bone].flags |=
        BONE_ALWAYS_PROCEDURAL; // ??? what about physics rules
  ...
  for (j = 0; j < g_numaimatbones; j++) {
    ...
    if (g_aimatbones[j].parent == -1) {
      MdlError(
          "Missing parent control bone \"%s\" for procedural bone \"%s\"\n",
    ...
    if (g_aimatbones[j].aimAttach == -1) {
      ...
      if (g_aimatbones[j].aimBone == -1) {
        MdlError("Missing aim control attachment or bone \"%s\" for procedural "
                 "bone \"%s\"\n",
                 g_aimatbones[j].aimname, g_aimatbones[j].bonename);
      }
    }

    g_bonetable[g_aimatbones[j].bone].flags |=
        BONE_ALWAYS_PROCEDURAL; // ??? what about physics rules
  ...
  for (j = 0; j < g_numjigglebones; j++) {
    ...
    g_bonetable[g_jigglebones[j].bone].flags |=
        BONE_ALWAYS_PROCEDURAL; // ??? what about physics rules
...
//-----------------------------------------------------------------------------
// Purpose: convert original procedural bone info into correct values for
// existing skeleton
//-----------------------------------------------------------------------------
void RemapProceduralBones() {
  ...
  for (j = 0; j < g_numquatinterpbones; j++) {
    ...
    if (g_bonetable[pInterp->control].parent != origControlParent) {
      MdlError("procedural bone \"%s\", parent remapping error, control parent "
               "was \"%s\", is now \"%s\"\n",
               pInterp->bonename, pInterp->controlparentname,
               g_bonetable[g_bonetable[pInterp->control].parent].name);
    }

    // remap triggers and movements/rotations due to skeleton changes and
    // realignment
    for (int k = 0; k < pInterp->numtriggers; k++) {
      ...
      if (parent != -1) {
        ...
        ConcatTransforms(g_bonetable[parent].boneToPose,
                         invControlParentRealign, srcControlParentBoneToPose);

        matrix3x4_t srcControlRelative;
        QuaternionMatrix(pInterp->trigger[k], srcControlRelative);

        matrix3x4_t srcControlBoneToPose;
        ConcatTransforms(srcControlParentBoneToPose, srcControlRelative,
                         srcControlBoneToPose);

        matrix3x4_t destControlParentBoneToPose;
        ConcatTransforms(srcControlParentBoneToPose,
                         g_bonetable[parent].srcRealign,
                         destControlParentBoneToPose);

        matrix3x4_t destControlBoneToPose;
        ConcatTransforms(srcControlBoneToPose,
                         g_bonetable[pInterp->control].srcRealign,
                         destControlBoneToPose);

        matrix3x4_t invDestControlParentBoneToPose;
        MatrixInvert(destControlParentBoneToPose,
                     invDestControlParentBoneToPose);

        matrix3x4_t destControlRelative;
        ConcatTransforms(invDestControlParentBoneToPose, destControlBoneToPose,
                         destControlRelative);
        ...
        RAD2DEG( angles.y ), RAD2DEG( angles.z ) );
      ...
      if (parent != -1) {
        ...
        // pInterp->pos[k].y, pInterp->pos[k].z );
        //  get local transform
        matrix3x4_t srcParentRelative;
        QuaternionMatrix(pInterp->quat[k], pInterp->pos[k] + pInterp->basepos,
                         srcParentRelative);
        ...
        ConcatTransforms(g_bonetable[parent].boneToPose, invSrcRealign,
                         origParentBoneToPose);

        // move bone adjustment into world position
        matrix3x4_t srcBoneToWorld;
        ConcatTransforms(origParentBoneToPose, srcParentRelative,
                         srcBoneToWorld);
        ...
        pInterp->pos[k] +=
            g_bonetable[pInterp->control].pos * pInterp->percentage;
...
//-----------------------------------------------------------------------------
// Purpose: go through all source files and link local bone indices and global
// bonetable indicies
//-----------------------------------------------------------------------------
static int MapSourcesToGlobalBonetable() {
  ...
  for (i = 0; i < g_numsources; i++) {
    ...
    memset(pSource->boneLocalToGlobal, 0xFF,
           sizeof(pSource->boneLocalToGlobal));
    memset(pSource->boneGlobalToLocal, 0xFF,
           sizeof(pSource->boneGlobalToLocal));
    ...
    for (j = 0; j < pSource->numbones; j++) {
      ...
      while (m != -1 &&
             (k = findGlobalBone(pSource->localBone[m].name)) == -1) {
...
void RealignBones() {
  ...
  for (int i = 0; i < g_numikchains; i++) {
    ...
    if (childbone[k] == -1 || childbone[k] == g_ikchain[i].link[1].bone) {
      ...
      MdlError(
          ...
          g_bonetable[g_ikchain[i].link[1].bone].name);
    }
    ...
    if (childbone[k] == -1 || childbone[k] == g_ikchain[i].link[2].bone) {
      ...
      MdlError(
          "Trying to realign bone \"%s\" with two children \"%s\", \"%s\"\n",
          g_bonetable[k].name, g_bonetable[childbone[k]].name,
          g_bonetable[g_ikchain[i].link[2].bone].name);
    }
  }
  ...
  for (k = 0; k < g_numbones; k++) {
    // printf("%s  %.4f %.4f %.4f  (%d)\n", g_bonetable[k].name,
    // g_bonetable[k].pos.x, g_bonetable[k].pos.y, g_bonetable[k].pos.z,
    // children[k] );
    if (!g_bonetable[k].bPreAligned && childbone[k] != -1) {
      ...
      if (d - g_bonetable[childbone[k]].pos.x > 0.01) {
        ...
        Vector v3;
        // printf("%s:%s  %.4f %.4f %.4f\n", g_bonetable[k].name,
        // g_bonetable[childbone[k]].name, g_bonetable[childbone[k]].pos.x,
        // g_bonetable[childbone[k]].pos.y, g_bonetable[childbone[k]].pos.z );
        ...
        // try to align to existing bone/boundingbox by finding most
        // perpendicular existing axis and aligning the new Z axis to it.
        ...
        d = fabs(DotProduct(forward, left)) + fabs(DotProduct(left, up)) +
        ...
        // printf("%f %f %f\n", DotProduct( boneToPose[k][0], boneToPose[k][1]
        // ), DotProduct( boneToPose[k][1], boneToPose[k][2] ), DotProduct(
        // boneToPose[k][2], boneToPose[k][0] ) );

        // printf("%f %f %f\n", DotProduct( forward, left ), DotProduct( left,
        // up ), DotProduct( up, forward ) );
  ...
  for (k = 0; k < g_numbones; k++) {
    ...
    if (g_bonetable[k].parent == -1) {
      ...
      ConcatTransforms(g_bonetable[g_bonetable[k].parent].boneToPose,
                       bonematrix, g_bonetable[k].boneToPose);
    }
    /*
    Vector v1;
...
//-----------------------------------------------------------------------------
// Purpose: find all the different bones used in all the source files and map
// everything
...
void RemapBones() {
  ...
  if (g_staticprop) {
    ...
    MdlWarning("Ignoring option $autocenter.  Only supported on $staticprop "
               "models!!!\n");
  }
...
//-----------------------------------------------------------------------------
void CalcBoneTransforms(s_animation_t *panimation, int frame,
                        matrix3x4_t *pBoneToWorld) {
  CalcBoneTransforms(panimation, g_panimation[0], frame, pBoneToWorld);
}

void CalcBoneTransforms(s_animation_t *panimation,
                        s_animation_t *pbaseanimation, int frame,
                        matrix3x4_t *pBoneToWorld) {
  ...
  for (int k = 0; k < g_numbones; k++) {
    ...
    if (!(panimation->flags & STUDIO_DELTA)) {
      AngleMatrix(panimation->sanim[frame][k].rot,
                  panimation->sanim[frame][k].pos, bonematrix);
      ...
      p3 =
          pbaseanimation->sanim[0][k].pos + s * panimation->sanim[frame][k].pos;
    ...
    if (g_bonetable[k].parent == -1) {
      ...
      ConcatTransforms(pBoneToWorld[g_bonetable[k].parent], bonematrix,
                       pBoneToWorld[k]);
    }
  }
}
...
void CalcBoneTransformsCycle(s_animation_t *panimation,
                             s_animation_t *pbaseanimation, float flCycle,
                             matrix3x4_t *pBoneToWorld) {
  ...
  for (int k = 0; k < g_numbones; k++) {
    ...
    // if (!(panimation->flags & STUDIO_DELTA))
    {
      ...
      VectorLerp(panimation->sanim[iFrame1][k].pos,
                 panimation->sanim[iFrame2][k].pos, s, p3);
    ...
    else
    {
            ...
            QuaternionMA( q1, s, q2, q3 );
            //p3 = g_bonetable[k].pos + s * panimation->sanim[frame][k].pos;
            p3 = pbaseanimation->sanim[0][k].pos + s *
    panimation->sanim[frame][k].pos;
...
void SlerpBones(Quaternion q1[MAXSTUDIOBONES], Vector pos1[MAXSTUDIOBONES],
                int sequence, const Quaternion q2[MAXSTUDIOBONES],
                const Vector pos2[MAXSTUDIOBONES], float s) {
  int i;
  Quaternion q3, q4;
  float s1, s2;
...
void AccumulateSeqLayers(Vector pos[], Quaternion q[], int sequence,
                         float frame, float flWeight);

void AccumulatePose(Vector pos[], Quaternion q[], int sequence, float frame,
                    float flWeight) {
...
static void CalcTriangleTangentSpace(s_source_t *pSrc, int v1, int v2, int v3,
  ...
  */
  ...
  Vector p0(pSrc->vertex[v1].position[0], pSrc->vertex[v1].position[1],
            pSrc->vertex[v1].position[2]);
  Vector p1(pSrc->vertex[v2].position[0], pSrc->vertex[v2].position[1],
            pSrc->vertex[v2].position[2]);
  Vector p2(pSrc->vertex[v3].position[0], pSrc->vertex[v3].position[1],
            pSrc->vertex[v3].position[2]);
...
void CalcModelTangentSpaces(s_source_t *pSrc) {
  ...
  for (meshID = 0; meshID < pSrc->nummeshes; meshID++) {
    ...
    for (vertID = 0; vertID < pMesh->numvertices; vertID++) {
      ...
      Vector4D &finalSVect =
          pSrc->vertex[vertID + pMesh->vertexoffset].tangentS;
      Vector sVect, tVect;
      ...
      if (g_bZBrush) {
        int vertID2;
        Vector vertPos1(pSrc->vertex[vertID].position[0],
                        pSrc->vertex[vertID].position[1],
                        pSrc->vertex[vertID].position[2]);
        for (vertID2 = 0; vertID2 < pMesh->numvertices; vertID2++) {
          if (vertID2 == vertID) {
            continue;
          }
          Vector vertPos2(pSrc->vertex[vertID2].position[0],
                          pSrc->vertex[vertID2].position[1],
...
//-----------------------------------------------------------------------------
static void InitRemappedVertex(s_source_t *pSource,
                               matrix3x4_t *pDestBoneToWorld,
  ...
  // FIXME: Remapping will whack tangentS. Need to recompute tangents after
  // remapping
}
...
void RemapVerticesToGlobalBones() {
  ...
  for (int i = 0; i < g_numsources; i++) {
    ...
    for (int j = 0; j < pSource->numvertices; j++) {
      InitRemappedVertex(pSource, destBoneToWorld, pSource->vertex[j],
                         pSource->m_GlobalVertices[j]);
    }
  }
}
    ...
    if (!found) {
      ...
      for (j = 0; j < g_numsources && !found; j++) {
        for (k = 0; k < g_source[j]->numbones && !found; k++) {
          if (!stricmp(g_attachment[i].bonename,
                       g_source[j]->localBone[k].name)) {
            MatrixCopy(g_source[j]->boneToPose[k], boneToPose);

            // check to make sure that this bone is actually referenced in the
            // output model if not, try parent bone until we find a referenced
            // bone in this chain
            while (k != -1 &&
                   g_source[j]->boneGlobalToLocal[g_source[j]
                                                      ->boneLocalToGlobal[k]] !=
                       k) {
...
static void LinkMouths() {
  for (int i = 0; i < g_nummouths; i++) {
    int j;
    for (j = 0; j < g_numbones; j++) {
      if (g_mouth[i].bonename[0] &&
          stricmp(g_mouth[i].bonename, g_bonetable[j].name) == 0)
        break;
    }
    if (j >= g_numbones) {
...
//
//-----------------------------------------------------------------------------
static float CalcPoseParameterValue(int control, RadianEuler &angle,
                                    Vector &pos) {
  switch (control) {
  case STUDIO_X:
    return pos.x;
    ...
    for (int iPose = 0; iPose < 2; iPose++) {
      if (pseq->groupsize[iPose] > 1) {
        if (pseq->paramattachment[iPose] != -1) {
          ...
          ConcatTransforms(boneToWorld[k0], g_attachment[n0].local,
                           boneToWorldMid);
          MatrixAngles(boneToWorldMid, angles, pos);
          // printf("%s : %s : %6.2f %6.2f %6.2f : %6.2f %6.2f %6.2f\n",
          // pseq->name, g_pose[j0].name, RAD2DEG( angles.x ), RAD2DEG( angles.y
          ...
          for (m[iPose] = 0; m[iPose] < pseq->groupsize[iPose]; m[iPose]++) {
            CalcBoneTransforms(pseq->panim[m[0]][m[1]], pseq->paramcompanim, 0,
                               boneToWorld);
            ConcatTransforms(boneToWorld[k0], g_attachment[n0].local,
                             boneToWorldRel);
            ...
            // angles.x ), RAD2DEG( angles.y ), RAD2DEG( angles.z ), pos.x,
            // pos.y, pos.z );

            float v =
                CalcPoseParameterValue(pseq->paramcontrol[iPose], angles, pos);
            ...
            // pseq->param1[i0][i1] = CalcPoseParameterValue(
            // pseq->paramcontrol[1], angles, pos );

            if (m[iPose] == 0) {
              pseq->paramstart[iPose] = (iPose == 0) ? pseq->param0[m[iPose]]
                                                     : pseq->param1[m[iPose]];
            }
            if (m[iPose] == pseq->groupsize[iPose] - 1) {
              pseq->paramend[iPose] = (iPose == 0) ? pseq->param0[m[iPose]]
                                                   : pseq->param1[m[iPose]];
            }
          }
          ...
          for (int m = 0; m < pseq->groupsize[iPose]; m++) {
            ...
            if (iPose == 0) {
              pseq->param0[m] = pseq->paramstart[iPose] * (1.0 - f) +
                                pseq->paramend[iPose] * f;
            } else {
              pseq->param1[m] = pseq->paramstart[iPose] * (1.0 - f) +
                                pseq->paramend[iPose] * f;
            }
          }
        }
...
//			go through all the animations and reset them, but move
//anything on an ikchain back to where it was.
//-----------------------------------------------------------------------------
    ...
    for (j = 0; j < panim->numframes; j++) {
      ...
      for (n = 0; n < g_numikchains; n++) {
        if (panim->weight[g_ikchain[n].link[2].bone] > 0) {
          Vector worldPos;
          MatrixPosition(boneToWorldOriginal[g_ikchain[n].link[2].bone],
                         worldPos);

          Studio_SolveIK(g_ikchain[n].link[0].bone, g_ikchain[n].link[1].bone,
                         g_ikchain[n].link[2].bone, worldPos, boneToWorld);
...
static void ProcessIKRules() {
  ...
  for (i = 0; i < g_numani; i++) {
    ...
    for (j = 0; j < panim->numikrules; j++) {
      ...
      if (pRule->start == -1) {
        ...
        if (pPrev->slot == pRule->slot) {
          if (pRule->peak < pPrev->tail) {
            ...
            pRule->start =
                pRule->peak +
                (pPrev->tail - pRule->peak + panim->numframes - 1) / 2;
          }
          pRule->start =
              (pRule->start + panim->numframes / 2) % (panim->numframes - 1);
          pPrev->end =
              (pRule->start + panim->numframes - 1) % (panim->numframes - 1);
        } else {
          pRule->start = pPrev->tail;
          pPrev->end = pRule->peak;
        }
        // printf("%s : %d (%d) : %d %d %d %d\n", panim->name, pRule->chain,
        // panim->numframes - 1, pRule->start, pRule->peak, pRule->tail,
        // pRule->end );
      }
      ...
      if (pRule->end == -1) {
        ...
        if (pNext->slot == pRule->slot) {
          if (pNext->peak < pRule->tail) {
            ...
            pNext->start =
                pNext->peak +
                (pRule->tail - pNext->peak + panim->numframes - 1) / 2;
          }
          pNext->start =
              (pNext->start + panim->numframes / 2) % (panim->numframes - 1);
          pRule->end =
              (pNext->start + panim->numframes - 1) % (panim->numframes - 1);
        } else {
      ...
      panim->numframes - 1, pRule->start, pRule->peak, pRule->tail, pRule->end,
      ...
      */
      ...
      pRule->errorData.pError = (s_streamdata_t *)kalloc(
          pRule->errorData.numerror, sizeof(s_streamdata_t));
        ...
        for (k = 0; k < pRule->errorData.numerror; k++) {
          ...
          if (pRule->bone != -1) {
            ...
            ConcatTransforms(worldToBone,
                             boneToWorld[g_ikchain[pRule->chain].link[2].bone],
                             local);
          } else {
            MatrixCopy(boneToWorld[g_ikchain[pRule->chain].link[2].bone],
                       local);
          }

          MatrixAngles(local, pRule->errorData.pError[k].q,
                       pRule->errorData.pError[k].pos);
          ...
          printf("%d  %.1f %.1f %.1f : %.1f %.1f %.1f\n",
                  k,
                  pRule->errorData.pError[k].pos.x,
          pRule->errorData.pError[k].pos.y, pRule->errorData.pError[k].pos.z,
                  ang.x, ang.y, ang.z );
          */
        }
      } break;
      ...
      case IK_GROUND: {
        ...
        Vector footfall;
        VectorTransform(g_ikchain[pRule->chain].center, boneToWorld[bone],
                        footfall);
    ...
    // auto release ik chains that are moved but not referenced and have no
    // explicit rules
    int count[16];
        ...
        for (int n = 0; n < panim1->numikrules; n++) {
          if ((panim1->ikrule[n].type != panim2->ikrule[n].type) ||
            ...
            MdlError("%s - mismatched IK rule %d: \n\"%s\" : %d %d %d\n\"%s\" "
                     ": %d %d %d\n",
                     g_sequence[i].name, n, panim1->name,
                     panim1->ikrule[n].type, panim1->ikrule[n].chain,
                     panim1->ikrule[n].slot, panim2->name,
                     panim2->ikrule[n].type, panim2->ikrule[n].chain,
      ...
      for (j = 0; j < g_numbones; j++) {
        ...
        for (k = 0; k < 6; k++) {
          ...
          for (n = 0; n <= iEndFrame - iStartFrame; n++) {
            ...
            case 2: /* Z Position */
              if (panim->flags & STUDIO_DELTA) {
                ...
                // pre-scale pos delta since format only has room for "overall"
                // weight
                float r = panim->posweight[j] / panim->weight[j];
                value[n] *= r;
              } else {
                value[n] = (psrcdata->pos[k] - g_bonetable[j].pos[k]) /
                           g_bonetable[j].posscale[k];
              }

              checkmin[k] =
                  min(value[n] * g_bonetable[j].posscale[k], checkmin[k]);
              checkmax[k] =
                  max(value[n] * g_bonetable[j].posscale[k], checkmax[k]);
              break;
            case 3: /* X Rotation */
            case 4: /* Y Rotation */
            case 5: /* Z Rotation */
          ...
          for (m = 1; m < n; m++) {
            ...
            // or if we're not on a run and the run is less than 3 units
              ...
              if (pcount->num.total != pcount->num.valid) {
                // if (j == 0) printf("%d:%d   ", pcount->num.valid,
                // pcount->num.total );
                pcount = pvalue;
                pvalue = pcount + 1;
              }
          ...
          if (panim->anim[w][j].num[k] == 2 && value[0] == 0) {
            ...
            panim->anim[w][j].data[k] = (mstudioanimvalue_t *)kalloc(
                pvalue - data, sizeof(mstudioanimvalue_t));
            memmove(panim->anim[w][j].data[k], data,
                    (pvalue - data) * sizeof(mstudioanimvalue_t));
          }
        ...
        if (g_bCheckLengths) {
          ...
          for (k = 0; k < 3; k++) {
            if (checkmin[k] != 0) {
              ...
              printf("%s(%.1f: %.1f %.1f) ", tmp[k], g_bonetable[j].pos[k],
...
int compareBonePriority(const void *a, const void *b) {
  return reinterpret_cast<const BonePriority_s *>(a)->m_nGlobalBoneWeight <
                 reinterpret_cast<const BonePriority_s *>(b)
                     ->m_nGlobalBoneWeight
             ? -1
...
void DumpDefineBone(int nBoneId, bool *pBoneDumpedList) {
  ...
  printf("%f %f %f %f %f %f", pos.x, pos.y, pos.z, angles.x, angles.y,
         angles.z);
...
void DumpDefineBones() {
  BonePriority_s *pBonePriorityList = reinterpret_cast<BonePriority_s *>(
      stackalloc(g_numbones * sizeof(BonePriority_s)));
  ...
  for (int i = 0; i < g_ScriptLODs.Count(); ++i) {
    ...
    for (int j = 0; j < scriptLOD.boneReplacements.Count(); ++j) {
      ...
      const int nBoneId =
          findGlobalBone(scriptLOD.boneReplacements[j].GetSrcName());
      if (nBoneId < 0) {
  ...
  // bones used by hitboxes and attachments should always go first since they're
  // used by the server
  for (int i = 0; i < g_numbones; ++i) {
    if (g_bonetable[i].flags & (BONE_USED_BY_HITBOX | BONE_USED_BY_ATTACHMENT |
                                BONE_USED_BY_BONE_MERGE)) {
      pBonePriorityList[i].m_nGlobalBoneWeight = -1.0f;
    }
  }

  qsort(pBonePriorityList, g_numbones, sizeof(BonePriority_s),
        compareBonePriority);

  bool *pBoneDumpedList =
      reinterpret_cast<bool *>(stackalloc(g_numbones * sizeof(bool)));
...
void ReLinkAttachments() {
  ...
  for (i = 0; i < g_nummodelsbeforeLOD; i++) {
    ...
    for (j = 0; j < g_model[i]->numeyeballs; j++) {
      g_model[i]->eyeball[j].bone =
          psource->boneLocalToGlobal[g_model[i]->eyeball[j].bone];
    }
  }
}
  ...
  for (int i = 0; i < g_nummodelsbeforeLOD; i++) {
    for (int j = 0; j < g_model[i]->numeyeballs; j++) {
      ...
      if (peyeball->upperlidflexdesc == -1) {
        //				MdlWarning( "eyeball %s missing upperlid
        //data\n", peyeball->name );
      ...
      if (peyeball->lowerlidflexdesc == -1) {
        //				MdlWarning( "eyeball %s missing lower
        //data\n", peyeball->name );
    ...
    for (i = 0; i < g_nummodelsbeforeLOD; i++) {
      ...
      for (j = 0; j < pLodData->numvertices; j++) {
        for (n = 0; n < pLodData->vertex[j].boneweight.numbones; n++) {
          ...
          VectorITransform(pLodData->vertex[j].position,
                           g_bonetable[k].boneToPose, p);
      ...
      if (g_bonetable[k].bmin[0] < g_bonetable[k].bmax[0] - 1 &&
        ...
        if (dump_hboxes) {
          printf("$hbox %d \"%s\" %.2f %.2f %.2f  %.2f %.2f %.2f\n",
                 ...
                 set->hitbox[set->numhitboxes].bmin[0],
                 set->hitbox[set->numhitboxes].bmin[1],
                 set->hitbox[set->numhitboxes].bmin[2],
                 set->hitbox[set->numhitboxes].bmax[0],
                 set->hitbox[set->numhitboxes].bmax[1],
...
void SetupFullBoneRenderBounds(
  ...
  // Note: shared animation files will need to include the hitboxes or else
  // their sequence boxes won't use this stuff. Now add hitboxes.
  ...
  for (i = 0; i < g_numani; i++) {
    ...
    for (j = 0; j < g_panimation[i]->numframes; j++) {
      ...
      for (k = 0; k < g_numbones; k++) {
        ...
        TransformAABB(bonetransform[k], boneRenderBounds[k].m_Mins,
                      boneRenderBounds[k].m_Maxs, tmpMin, tmpMax);
    ...
    if (bmin.x < g_vecMinWorldspace.x || bmin.y < g_vecMinWorldspace.y ||
        bmin.z < g_vecMinWorldspace.z || bmax.x > g_vecMaxWorldspace.x ||
        bmax.y > g_vecMaxWorldspace.y || bmax.z > g_vecMaxWorldspace.z) {
      MdlWarning(
          "%s : bounding box out of range : %.0f %.0f %.0f : %.0f %.0f %.0f\n",
          g_panimation[i]->name, bmin.x, bmin.y, bmin.z, bmax.z, bmax.y,
          bmax.z);
    ...
    printf("%s : %.0f %.0f %.0f %.0f %.0f %.0f\n",
            g_panimation[i]->name, bmin[0], bmax[0], bmin[1], bmax[1], bmin[2],
    bmax[2] );
    */

    // printf("%s  %.2f\n", g_sequence[i].name,
    // g_sequence[i].panim[0]->pos[9][0][0] / g_bonetable[9].pos[0] );
  }
