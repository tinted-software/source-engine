...
struct BoneTransformMap_t {
  ...
  // The order in which transforms appear in this list specifies their bone
  // indices
...
static CUtlVector<VertIndices_t>
    s_UniqueVertices; // A list of the unique vertices in the mesh
...
struct DeltaIndex_t {
  DeltaIndex_t()
      : m_nPositionIndex(-1), m_nNormalIndex(-1), m_nNextDelta(-1),
        m_nWrinkleIndex(-1), m_bInList(false) {}
...
//-----------------------------------------------------------------------------
// Finds or adds delta states. These pointers are invalidated by calling
// FindOrAddDeltaState again
//-----------------------------------------------------------------------------
static DeltaState_t *FindOrAddDeltaState(const char *pDeltaStateName,
                                         int nBaseStateVertexCount) {
...
static bool DefineUniqueVertices(CDmeVertexData *pBindState,
                                 int nStartingUniqueCount) {
  const CUtlVector<int> &positionIndices =
      pBindState->GetVertexIndexData(CDmeVertexData::FIELD_POSITION);
  const CUtlVector<int> &normalIndices =
      pBindState->GetVertexIndexData(CDmeVertexData::FIELD_NORMAL);
  const CUtlVector<int> &texcoordIndices =
      pBindState->GetVertexIndexData(CDmeVertexData::FIELD_TEXCOORD);
  const CUtlVector<int> &balanceIndices =
      pBindState->GetVertexIndexData(CDmeVertexData::FIELD_BALANCE);
  const CUtlVector<int> &speedIndices =
      pBindState->GetVertexIndexData(CDmeVertexData::FIELD_MORPH_SPEED);
  ...
  if (nNormalCount && nPositionCount != nNormalCount) {
    MdlError("Encountered a mesh with invalid geometry (different number of "
             "indices for various data fields)\n");
    return false;
  }
  ...
  for (int i = 0; i < nPositionCount; ++i) {
    ...
    vert.balance =
        s_Balance.Count() + ((nBalanceCount > 0) ? balanceIndices[i] : 0);
...
// Loads the vertices from the model
//-----------------------------------------------------------------------------
static bool LoadVertices(CDmeVertexData *pBindState, const matrix3x4_t &mat,
                         float flScale, int nBoneAssign, int *pBoneRemap,
                         int nStartingUniqueCount) {
  if (nBoneAssign < 0) {
  ...
  int nJointCount =
      pBindState->HasSkinningData() ? pBindState->JointCount() : 0;
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    if (nJointCount == 0) {
      ...
      int nBoneCount = SortAndBalanceBones(nJointCount, MAXSTUDIOBONEWEIGHTS,
                                           pIndexBuf, pWeightBuf);
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    g_texcoord[g_numtexcoords].y =
        bFlipVCoordinate ? 1.0f - texcoords[i].y : texcoords[i].y;
    ++g_numtexcoords;
  }

  // In the event of no speed or balance map, use the same value of 1 for all
  // vertices
  if (balances.Count()) {
...
static bool LoadDeltaState(CDmeVertexDeltaData *pDeltaState,
                           CDmeVertexData *pBindState, const matrix3x4_t &mat,
                           float flScale, int nStartingUniqueVertex,
                           int nStartingUniqueVertexMap) {
  DeltaState_t *pDeltaStateData =
      FindOrAddDeltaState(pDeltaState->GetName(),
  ...
  const CUtlVector<int> &positionIndices =
      pDeltaState->GetVertexIndexData(CDmeVertexDataBase::FIELD_POSITION);
  const CUtlVector<Vector> &normals = pDeltaState->GetNormalData();
  const CUtlVector<int> &normalIndices =
      pDeltaState->GetVertexIndexData(CDmeVertexDataBase::FIELD_NORMAL);
  const CUtlVector<float> &wrinkle = pDeltaState->GetWrinkleData();
  const CUtlVector<int> &wrinkleIndices =
      pDeltaState->GetVertexIndexData(CDmeVertexDataBase::FIELD_WRINKLE);

  if (positions.Count() != positionIndices.Count()) {
    MdlError("DeltaState %s contains a different number of positions + "
             "position indices!\n",
             pDeltaState->GetName());
    return false;
  }
  ...
  if (normals.Count() != normalIndices.Count()) {
    MdlError("DeltaState %s contains a different number of normals + normal "
             "indices!\n",
             pDeltaState->GetName());
    return false;
  }
  ...
  if (wrinkle.Count() != wrinkleIndices.Count()) {
    MdlError("DeltaState %s contains a different number of wrinkles + wrinkle "
             "indices!\n",
             pDeltaState->GetName());
    return false;
  }
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    const CUtlVector<int> &baseVerts =
        pBindState->FindVertexIndicesFromDataIndex(
            CDmeVertexData::FIELD_POSITION, positionIndices[i]);
    int nBaseVertCount = baseVerts.Count();
    for (int k = 0; k < nBaseVertCount; ++k) {
      int nUniqueVertexIndex =
          s_UniqueVerticesMap[nStartingUniqueVertexMap + baseVerts[k]];
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    // Indices
    const CUtlVector<int> &baseVerts =
        pBindState->FindVertexIndicesFromDataIndex(CDmeVertexData::FIELD_NORMAL,
                                                   normalIndices[i]);
    int nBaseVertCount = baseVerts.Count();
    for (int k = 0; k < nBaseVertCount; ++k) {
      int nUniqueVertexIndex =
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    const CUtlVector<int> &baseVerts =
        pBindState->FindVertexIndicesFromDataIndex(
            CDmeVertexData::FIELD_WRINKLE, wrinkleIndices[i]);
    int nBaseVertCount = baseVerts.Count();
    for (int k = 0; k < nBaseVertCount; ++k) {
      int nUniqueVertexIndex =
          s_UniqueVerticesMap[nStartingUniqueVertexMap + baseVerts[k]];
...
//-----------------------------------------------------------------------------
static void ParseFaceData(CDmeVertexData *pVertexData, int material, int v1,
                          int v2, int v3, int vi, int ni, int ti) {
  s_tmpface_t f;
  f.material = material;
  ...
  Assert(f.a <= (unsigned long)g_numverts && f.b <= (unsigned long)g_numverts &&
         f.c <= (unsigned long)g_numverts);
  Assert(f.na <= (unsigned long)g_numnormals &&
         f.nb <= (unsigned long)g_numnormals &&
         f.nc <= (unsigned long)g_numnormals);
  Assert(f.ta <= (unsigned long)g_numtexcoords &&
         f.tb <= (unsigned long)g_numtexcoords &&
...
//-----------------------------------------------------------------------------
static bool LoadMesh(CDmeMesh *pMesh, CDmeVertexData *pBindState,
                     const matrix3x4_t &mat, float flScale, int nBoneAssign,
  ...
  LoadVertices(pBindState, mat, flScale, nBoneAssign, pBoneRemap,
               nStartingUniqueCount);
  ...
  for (int i = 0; i < nDeltaStateCount; ++i) {
    ...
    if (!LoadDeltaState(pDeltaState, pBindState, mat, flScale,
                        nStartingUniqueCount, nStartingUniqueMapCount))
      return false;
  }
  ...
  for (int i = 0; i < nFaceSetCount; ++i) {
    ...
    while (nFirstIndex < nIndexCount) {
      ...
      if (nVertexCount >= 3) {
        ...
        pMesh->ComputeTriangulatedIndices(pBindState, pFaceSet, nFirstIndex,
                                          pIndices, nOutCount);
        for (int ii = 0; ii < nOutCount; ii += 3) {
          ParseFaceData(pBindState, material, pIndices[ii], pIndices[ii + 2],
                        pIndices[ii + 1], nStartingVertex, nStartingNormal,
                        nStartingTexCoord);
        }
      }
...
static bool LoadMeshes(const LoadMeshInfo_t &info, CDmeDag *pDag,
                       const matrix3x4_t &parentToBindPose, int nBoneAssign) {
  // We want to create an aggregate matrix transforming from this dag to its
  // closest parent which actually is an animated joint. This is done so we can
  // autoskin meshes to their closest parents if they have not been skinned.
  ...
  // Update autoskin to autoskin to non-DmeJoint's if they are children of the
  // DmeModel (i.e. they have no parent bone)
  if (nFoundIndex >= 0) {
  ...
  if (nFoundIndex >= 0) {
    ConcatTransforms(parentToBindPose, info.m_pBindPose[nFoundIndex],
                     dagToBindPose);
  } else {
    // NOTE: This isn't particularly kosher; we're using the current pose
    // instead of the bind pose because there's no transform in the bind pose
  ...
  if (pMesh) {
    ...
    if (!LoadMesh(pMesh, pBindState, dagToBindPose, info.m_flScale, nBoneAssign,
                  info.m_pBoneRemap, info.m_pSource))
      return false;
  }
...
// Method used to add mesh data
//-----------------------------------------------------------------------------
static bool LoadMeshes(CDmeModel *pModel, float flScale, int *pBoneRemap,
                       s_source_t *pSource) {
  matrix3x4_t mat;
  ...
  int nCount = pBindPose ? pBindPose->GetTransformCount()
                         : pModel->GetJointTransformCount();
  for (int i = 0; i < nCount; ++i) {
    CDmeTransform *pTransform =
        pBindPose ? pBindPose->GetTransform(i) : pModel->GetJointTransform(i);
...
static void BuildVertexAnimations(s_source_t *pSource) {
  ...
  s_vertanim_t *pVertAnim =
      (s_vertanim_t *)_alloca(numvlist * sizeof(s_vertanim_t));
  for (int i = 0; i < nCount; ++i) {
    ...
    for (int j = state.m_nFirstDelta; j >= 0;
         j = state.m_DeltaIndices[j].m_nNextDelta) {
      ...
      Assert(delta.m_nPositionIndex >= 0 || delta.m_nNormalIndex >= 0 ||
             delta.m_nWrinkleIndex >= 0);
    ...
    pSourceAnim->vanim[0] =
        (s_vertanim_t *)kalloc(nVertAnimCount, sizeof(s_vertanim_t));
    memcpy(pSourceAnim->vanim[0], pVertAnim,
           nVertAnimCount * sizeof(s_vertanim_t));
  }
}
...
//-----------------------------------------------------------------------------
static bool AddDagJoint(CDmeModel *pModel, CDmeDag *pDag, s_node_t *pNodes,
                        int nParentIndex, BoneTransformMap_t &boneMap) {
  ...
  Q_strncpy(pNodes[nJointIndex].name, pTransformName,
            sizeof(pNodes[nJointIndex].name));
  ...
  for (int i = 0; i < nChildCount; ++i) {
    ...
    if ((nCurrentBoneCount != boneMap.m_nBoneCount) && (nFoundIndex < 0)) {
      MdlWarning("DMX Model has a joint \"%s\" which is not in its joint "
                 "transform list.\n"
                 "This joint has children which are in the joint transform "
                 "list, which is illegal.\n",
                 pDag->GetName());
      return false;
    }
  }
...
//-----------------------------------------------------------------------------
static int LoadSkeleton(CDmeDag *pRoot, CDmeModel *pModel, s_node_t *pNodes,
                        BoneTransformMap_t &map) {
  ...
  // Add a default identity bone used for autoskinning if no joints are
  // specified
  s_nDefaultRootNode = map.m_nBoneCount;
  Q_strncpy(pNodes[s_nDefaultRootNode].name, "defaultRoot",
            sizeof(pNodes[s_nDefaultRootNode].name));
  ...
  for (int i = 0; i < nInitialBoneCount; ++i) {
    ...
    if (pNodes[nIndex].name[0] == 0) {
      ...
      Q_strncpy(pNodes[nIndex].name, pTransform->GetName(),
                sizeof(pNodes[nIndex].name));
      MdlWarning("Joint %s specified in the joint transform list but doesn't "
                 "appear in the dag hierarchy!\n",
                 pTransform->GetName());
    }
  }
  return map.m_nBoneCount + 1;
...
// Loads the skeletal hierarchy from the game model, returns bone count
//-----------------------------------------------------------------------------
static void LoadAttachments(CDmeDag *pRoot, CDmeDag *pDag,
                            s_source_t *pSource) {
  ...
  if (pAttachment && (pDag != pRoot)) {
    ...
    Q_strncpy(attachment.bonename, pDag->GetName(),
              sizeof(attachment.bonename));
...
// Loads the bind pose
//-----------------------------------------------------------------------------
static void LoadBindPose(CDmeModel *pModel, float flScale, int *pBoneRemap,
                         s_source_t *pSource) {
  ...
  pSourceAnim->rawanim[0] =
      (s_bone_t *)kalloc(pSource->numbones, sizeof(s_bone_t));
  ...
  // NOTE: This means that bones that are not in the bind pose are set to
  // identity! Is this correct? I think it shouldn't matter, but we may need to
  // fix this.
  CDmeTransformList *pBindPose = pModel->FindBaseState("bind");
  int nCount = pBindPose ? pBindPose->GetTransformCount()
                         : pModel->GetJointTransformCount();
...
//-----------------------------------------------------------------------------
static void ComputeFramePose(s_sourceanim_t *pSourceAnim, int nFrame,
                             float flScale, BoneTransformMap_t &boneMap) {
  pSourceAnim->rawanim[nFrame] =
      (s_bone_t *)kalloc(boneMap.m_nBoneCount, sizeof(s_bone_t));

  for (int i = 0; i < boneMap.m_nBoneCount; ++i) {
    matrix3x4_t jointTransform;
    boneMap.m_ppTransforms[i]->GetTransform(jointTransform);
    MatrixAngles(jointTransform, pSourceAnim->rawanim[nFrame][i].rot,
                 pSourceAnim->rawanim[nFrame][i].pos);
...
//-----------------------------------------------------------------------------
static void LoadAnimations(s_source_t *pSource,
                           CDmeAnimationList *pAnimationList, float flScale,
  ...
  for (int i = 0; i < nAnimationCount; ++i) {
    ...
    s_sourceanim_t *pSourceAnim =
        FindOrAddSourceAnim(pSource, pAnimation->GetName());
    ...
    pSourceAnim->numframes =
        pSourceAnim->endframe - pSourceAnim->startframe + 1;
    ...
    while (nFrame < pSourceAnim->numframes) {
      ...
      DmeTime_t t = nStartTime + DmeTime_t(nSecond * 10000) +
                    DmeTime_t((float)nFraction * flOOFrameRate);
...
// Loads the skeletal hierarchy from the game model, returns bone count
//-----------------------------------------------------------------------------
static void AddFlexKeys(CDmeDag *pRoot, CDmeDag *pDag,
...
int Load_DMX(s_source_t *pSource) {
  ...
  CDmElement *pRoot;
  if (g_pDataModel->RestoreFromFile(pFullPath, NULL, NULL, &pRoot) ==
      DMFILEID_INVALID)
    return 0;
  ...
  BoneTransformMap_t boneMap;
  pSource->numbones =
      LoadSkeleton(pSkeleton, pModel, pSource->localBone, boneMap);
  ...
  CDmeAnimationList *pAnimationList =
      pRoot->GetValueElement<CDmeAnimationList>("animationList");
  if (pAnimationList) {
