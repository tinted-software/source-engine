...
// process immediately.
LONG __stdcall MdlSecondExceptionFilter(
...
void Cmd_BoneFlexDriver() {
  ...
  if (!pDmeBoneFlexDriverList) {
    pDmeBoneFlexDriverList = CreateElement<CDmeBoneFlexDriverList>(
        "boneDriverFlexList", DMFILEID_INVALID);
    if (pDmeBoneFlexDriverList) {
  ...
  CDmeBoneFlexDriver *pDmeBoneFlexDriver =
      pDmeBoneFlexDriverList->FindOrCreateBoneFlexDriver(token);
  ...
  if (nBoneComponent < STUDIO_BONE_FLEX_TX ||
      nBoneComponent > STUDIO_BONE_FLEX_TZ) {
  ...
  CDmeBoneFlexDriverControl *pDmeBoneFlexDriverControl =
      pDmeBoneFlexDriver->FindOrCreateControl(token);
  if (!pDmeBoneFlexDriverControl) {
    MdlError("%s: Couldn't find or create DmeBoneFlexDriverControl for bone "
             "\"%s\"\n",
             "$boneflexdriver", token);
    return;
  }
    ...
    if (!V_stricmp(token, "0to1")) {
      ...
      if (TokenAvailable() && GetToken(false)) {
        if (V_isdigit(*token)) {
          ...
          if (nOptRes <= 0) {
            MdlError("$checkuv: Invalid resolution, \"%s\", for gutter check "
                     "specified, must be > 0\n",
                     token);
            return;
          }
          ...
          if (TokenAvailable() && GetToken(false)) {
            if (V_isdigit(*token)) {
              ...
              if (nOptMin <= 0) {
                MdlError("$checkuv: Invalid minimum, \"%s\", for gutter check "
                         "specified, must be > 0\n",
                         token);
                return;
              }
      ...
      MdlError("$checkuv: Unknown argument \"%s\", expected one of [ 0to1, "
               "overlap, inverse, gutter ]\n",
               token);
      return;
    }
  }
...
void AddBodyFlexFetchRule(
  ...
  case FLEXCONTROLLER_REMAP_EYELID:
    if (remapLocalIndex == 0) {
      ...
      pRule->op[pRule->numops].d.value = remap.m_EyesUpDownFlexController >= 0
  ...
  for (int i = 0; i < nRemappedControlCount; ++i) {
    ...
    if (remapControl.m_bIsStereo) {
      ...
      if ((g_numflexcontrollers + 1) >= MAXSTUDIOFLEXCTRL)
        MdlError("Line %d: Too many flex controllers, max %d, cannot add split "
      ...
      if (g_numflexcontrollers >= MAXSTUDIOFLEXCTRL)
        MdlError("Line %d: Too many flex controllers, max %d, cannot add "
                 "control %s from source %s",
                 g_iLinecount, MAXSTUDIOFLEXCTRL, remapControl.m_Name.Get(),
                 pSource->filename);
    ...
    if (remapControl.m_RemapType == FLEXCONTROLLER_REMAP_NWAY ||
        remapControl.m_RemapType == FLEXCONTROLLER_REMAP_EYELID) {
      if (g_numflexcontrollers >= MAXSTUDIOFLEXCTRL)
        MdlError("Line %d: Too many flex controllers, max %d, cannot add value "
...
void AddBodyFlexData(s_source_t *pSource, int imodel) {
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    if (g_numflexkeys >= MAXSTUDIOFLEXKEYS)
      MdlError("Line %d: Too many flex keys, max %d, cannot add flexKey %s "
               "from source %s",
...
bool s_attachment_t::operator==(const s_attachment_t &rhs) const {
  ...
  if (Q_stricmp(bonename, rhs.bonename) || bone != rhs.bone ||
      type != rhs.type || flags != rhs.flags ||
    ...
    MdlWarning("Attachments with the same name but different parameters found\n"
               "  %s: ParentBone: %s Type: %d Flags: 0x%08x P: %6.2f %6.2f "
               "%6.2f R: %6.2f %6.2f %6.2f\n"
               "  %s: ParentBone: %s Type: %d Flags: 0x%08x P: %6.2f %6.2f "
               "%6.2f R: %6.2f %6.2f %6.2f\n",
               name, bonename, type, flags, iPos.x, iPos.y, iPos.z,
               RAD2DEG(iEuler.x), RAD2DEG(iEuler.y), RAD2DEG(iEuler.z),
               rhs.name, rhs.bonename, rhs.type, rhs.flags, jPos.x, jPos.y,
               jPos.z, RAD2DEG(jEuler.x), RAD2DEG(jEuler.y), RAD2DEG(jEuler.z));
  ...
  for (int i = 0; i < pSource->m_Attachments.Count(); ++i) {
    ...
    if (g_numattachments >= ARRAYSIZE(g_attachment)) {
      MdlWarning("Too Many Attachments (Max %d), Ignoring Attachment %s:%s\n",
                 ARRAYSIZE(g_attachment), pSource->filename,
                 pSource->m_Attachments[i].name);
      continue;
      ;
    }

    memcpy(&g_attachment[g_numattachments], &(pSource->m_Attachments[i]),
           sizeof(s_attachment_t));
    ++g_numattachments;
  }
}
...
int Option_ActivityModifier(s_sequence_t *psequence) {
  GetToken(false);
  V_strcpy_safe(
      psequence->activitymodifier[psequence->numactivitymodifiers++].name,
      token);
...
void ProcessOriginalContentFile(char const *szDataFile,
  ...
  if (szContentDirRootEnd && szSceneName) {
    ...
    char chScenePath[MAX_PATH] = {0};
    Q_snprintf(chScenePath, sizeof(chScenePath) - 1, "%.*s%s",
               max(strrchr(szDataFile, '\\'), strrchr(szDataFile, '/')) + 1 -
                   szDataFile,
               szDataFile, szOriginalContentFile);
    EnsureDependencyFileCheckedIn(chScenePath);
  } else {
    MdlWarning("ProcessOriginalContentFile for '%s' cannot detect scene source "
               "file from '%s'!\n",
               szDataFile, szOriginalContentFile);
  }
}
...
int ParseCmdlistToken(int &numcmds, s_animcmd_t *cmds) {
  ...
  if (stricmp("fixuploop", token) == 0) {
    ...
    pcmd->u.subtract.flags |= STUDIO_POST;
             ...
             0) // FIXME: rename this to something better
  {
    pcmd->cmd = CMD_SUBTRACT;
  ...
  while (1) {
    ...
    if (stricmp("{", token) == 0) {
      ...
      if (pweightlist->boneweight[i] == 0 &&
        ...
        MdlError("Non-zero Position weight with zero Rotation weight not "
                 "allowed \'%s:%s %f %f\'\n",
                 pweightlist->name, pweightlist->bonename[i],
...
void Cmd_Model() {
  ...
  while (1) {
    ...
    if (!Q_stricmp("{", token)) {
      ...
      Option_Flex(
          FAC, vtafile, g_nummodels,
          split); // FIXME: this needs to point to a model used, not loaded!!!
...
void Cmd_LOD(const char *cmdname) {
  if (gflags & STUDIOHDR_FLAGS_HASSHADOWLOD) {
    MdlError("Model can only have one $shadowlod and it must be the last lod "
             "in the .qc (%d) : %s\n",
...
void Cmd_LimitRotation() {
  ...
  while (TokenAvailable()) {
    ...
    Assert(0);
    // V_strcpy_safe(
    // g_limitrotation[g_numlimitrotation].sequencename[g_limitrotation[g_numlimitrotation].numseq++],
    // token );
  }
...
void Cmd_MinLOD() {
  ...
  if (g_numAllowedRootLODs > 0 && g_numAllowedRootLODs < g_minLod) {
    MdlWarning("$minlod %d overrides $allowrootlods %d, proceeding with "
               "$allowrootlods %d.\n",
...
void Cmd_AllowRootLODs() {
  ...
  if (g_numAllowedRootLODs > 0 && g_numAllowedRootLODs < g_minLod) {
    MdlWarning("$allowrootlods %d is conflicting with $minlod %d, proceeding "
               "with $allowrootlods %d.\n",
...
static bool
CStudioMDLApp_SuggestGameInfoDirFn(CFSSteamSetupInfo const *pFsSteamSetupInfo,
...
int CStudioMDLApp::Main() {
  ...
  // FIXME: add these to an envvar if folks are using complicated directory
  // mappings instead of defaults
