...
int lookup_index(s_source_t *psource, int material, Vector &vertex,
                 Vector &normal, Vector2D texcoord, int iCount, int bones[],
                 float weights[]) {
  int i, j;
...
void ParseFaceData(s_source_t *psource, int material, s_face_t *pFace) {
  ...
  for (j = 0; j < 3; j++) {
    ...
    index[j] =
        lookup_index(psource, material, p, normal, t, iCount, bones, weights);
  }
...
void Grab_Triangles(s_source_t *psource) {
  ...
  while (1) {
    ...
    for (i = strlen(texturename) - 1; i >= 0 && !V_isgraph(texturename[i]);
         i--) {
    }
    texturename[i + 1] = '\0';
...
int Load_SMD(s_source_t *psource) {
  ...
  while (GetLineInput()) {
    ...
    } else if ((strncmp(cmd, "//", 2) == 0) || (strncmp(cmd, ";", 1) == 0) ||
               (strncmp(cmd, "#", 1) == 0)) {
