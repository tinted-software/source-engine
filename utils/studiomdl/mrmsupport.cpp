...
void Grab_Vertexlist(s_source_t *psource) {
  while (1) {
    if (fgets(g_szLine, sizeof(g_szLine), g_fpInput) != NULL) {
      ...
      int i = sscanf(g_szLine, "%d %d %f %f %f %d %d %f %d %f %d %f %d %f", &j,
                     &bone, &p[0], &p[1], &p[2], &iCount, &bones[0],
                     &weights[0], &bones[1], &weights[1], &bones[2],
                     &weights[2], &bones[3], &weights[3]);
      ...
      if (i == 5) {
        ...
        MdlError("%s: error on line %d: %s", g_szFilename, g_iLinecount,
                 g_szLine);
      }
    }
  }
...
void Grab_Materiallist(s_source_t *psource) {
  while (1) {
    if (fgets(g_szLine, sizeof(g_szLine), g_fpInput) != NULL) {
      ...
      if (sscanf(g_szLine,
                 "%d  %f %f %f %f   %f %f %f %f  %f %f %f %f  %f \"%[^\"]s", &j,
                 &a.r, &a.g, &a.b, &a.a, &d.r, &d.g, &d.b, &d.a, &s.r, &s.g,
                 &s.b, &s.a, &g, path) == 15) {
        if (path[0] == '\0') {
          psource->texmap[j] = -1;
...
void Grab_Normallist(s_source_t *psource) {
  while (1) {
    if (fgets(g_szLine, sizeof(g_szLine), g_fpInput) != NULL) {
      ...
      if (sscanf(g_szLine, "%d %d %f %f %f", &j, &bone, &n[0], &n[1], &n[2]) ==
          5) {
        if (bone < 0 || bone >= psource->numbones) {
          MdlWarning("bogus bone index\n");
          MdlWarning("%d %s :\n%s", g_iLinecount, g_szFilename, g_szLine);
...
void Grab_Faceattriblist(s_source_t *psource) {
  while (1) {
    if (fgets(g_szLine, sizeof(g_szLine), g_fpInput) != NULL) {
      ...
      if (sscanf(g_szLine, "%d %d %d %d %d %d %d %d %d", &j, &material, &smooth,
                 &f.ta, &f.tb, &f.tc, &f.na, &f.nb, &f.nc) == 9) {
        f.a = g_face[j].a;
        f.b = g_face[j].b;
        f.c = g_face[j].c;
...
void UnifyIndices(s_source_t *psource) {
  ...
  for (i = 0; i < g_numfaces; i++) {
    ...
    uface[i].a = AddToVlist(g_face[i].a, g_face[i].material, g_face[i].na,
                            g_face[i].ta, g_numverts);
    uface[i].b = AddToVlist(g_face[i].b, g_face[i].material, g_face[i].nb,
                            g_face[i].tb, g_numverts);
    uface[i].c = AddToVlist(g_face[i].c, g_face[i].material, g_face[i].nc,
                            g_face[i].tc, g_numverts);

    // keep an original copy
    g_src_uface[i] = uface[i];
  }

  // printf("%d : %d %d %d\n", numvlist, g_numverts, g_numnormals,
  // g_numtexcoords );
}
...
//-----------------------------------------------------------------------------
static void BuildUniqueVertexList(s_source_t *pSource,
                                  const int *pDesiredToVList) {
  // allocate memory
  pSource->vertex =
...
static void SortFacesByMaterial(int *pDesiredToSrcFace) {
  // NOTE: Unlike SortVerticesByMaterial, srcFaceToDesired isn't needed, so
  // we're not computing it
...
//-----------------------------------------------------------------------------
static void PointMeshesToVertexAndFaceData(s_source_t *pSource,
                                           int *pDesiredToSrcFace) {
...
//-----------------------------------------------------------------------------
static void BuildFaceList(s_source_t *pSource, int *pVListToDesired,
                          int *pDesiredToSrcFace) {
  ...
  for (int m = 0; m < MAXSTUDIOSKINS; m++) {
    ...
    for (int i = pSource->mesh[m].faceoffset;
         i < pSource->mesh[m].numfaces + pSource->mesh[m].faceoffset; i++) {
      int j = pDesiredToSrcFace[i];

      // NOTE: per-face vertex indices a,b,c are mesh relative (hence the
      // subtraction), while g_src_uface are model relative
      pSource->face[i].a =
          pVListToDesired[g_src_uface[j].a] - pSource->mesh[m].vertexoffset;
      pSource->face[i].b =
          pVListToDesired[g_src_uface[j].b] - pSource->mesh[m].vertexoffset;
      pSource->face[i].c =
          pVListToDesired[g_src_uface[j].c] - pSource->mesh[m].vertexoffset;
...
static void RemapVertexAnimations(s_source_t *pSource, int *pVListToDesired) {
  ...
  for (int i = 0; i < nAnimationCount; ++i) {
    ...
    for (int j = 0; j < MAXSTUDIOANIMFRAMES; ++j) {
      ...
      // Have to do it in 2 loops because it'll overwrite itself if we do it in
      // 1
...
//-----------------------------------------------------------------------------
// Sorts vertices by material type, re-maps data structures that refer to those
// vertices to use the new indices
//-----------------------------------------------------------------------------
