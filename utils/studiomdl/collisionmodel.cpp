...
// NOTE: The term joint here is used to mean a bone, collision model, and a
// joint. Each "joint" is the collision geometry at a named bone (or set of
// bones that have been merged) and the joint (with constraints) between that
// set and its parent.  The root "joint" has no constraints. I chose to refer to
// them as joints to avoid confusion.  Yes they encompass bones and joints, but
// they use the same names, and the data is actually linked.
...
public:
  CPhysCollisionModel(void) { memset(this, 0, sizeof(*this)); }
...
public:
  CJointConstraint(void) { m_pJointName = NULL; }

  CJointConstraint(const char *pName, int axis, jointlimit_t type, float min,
                   float max, float friction)
      : m_axis(axis), m_jointType(type), m_limitMin(min), m_limitMax(max),
        m_friction(friction) {
    m_pJointName = pName;
  }
...
int FindBoneInTable(const char *pName) { return findGlobalBone(pName); }

//-----------------------------------------------------------------------------
// Purpose: Contains a complete physical joint system with constraint
// relationships
//-----------------------------------------------------------------------------
...
public:
  ...
  void AddConstraint(const char *pJointName, int axis, jointlimit_t jointType,
                     float limitMin, float limitMax, float friction);
  ...
  bool m_bAssumeWorldspace; // assume the model is already declared in
                            // worldspace, regardless of bone names
};
...
CJointedModel::CJointedModel(void) {
  ...
  // UNDONE: Move these defaults elsewhere?  They are all overrideable by the
  // QC/script
  m_defaultDamping = 0;
...
void CJointedModel::AppendCollisionModel(CPhysCollisionModel *pCollide) {
  if (m_isMassCenterForced) {
    physcollision->CollideSetMassCenter(pCollide->m_pCollisionData,
                                        m_massCenterForced);
  }
...
void CJointedModel::SortCollisionList(void) {
  ...
  while (swapped) {
    swapped = false;
    // loop over all solids and swap any parent/child pairs that are out of
    // order
    for (i = 0; i < m_collisionCount; i++) {
      ...
      // Don't try to move ones where the pPhys and its parent have the same
      // name otherwise an infinite loop results
      ...
      // if the child came before the parent, then swap the parent and child
      // positions
...
void CJointedModel::AppendCollisionPair(const char *pName0,
                                        const char *pName1) {
  ...
  pPair->pName0 =
      (jointIndex0 >= 0) ? m_pModel->localBone[jointIndex0].name : NULL;
  int jointIndex1 = FindLocalBoneNamed(m_pModel, pName1);
  pPair->pName1 =
      (jointIndex1 >= 0) ? m_pModel->localBone[jointIndex1].name : NULL;
...
void CJointedModel::Simplify() {
  ...
  MergeBones(FindLocalBoneNamed(m_pModel, pAnimationRootBone),
             FindLocalBoneNamed(m_pModel, m_rootName));
}
...
void CJointedModel::AddConstraint(const char *pJointName, int axis,
                                  jointlimit_t jointType, float limitMin,
  ...
  CJointConstraint *pConstraint =
      new CJointConstraint(pJointName, axis, jointType, limitMin, limitMax,
                           friction * (1.0f / 5.0f));
...
void CJointedModel::SetTotalMass(float mass) { m_totalMass = mass; }

void CJointedModel::SetAutoMass(void) { m_totalMass = -1; }

void CJointedModel::SetNoSelfCollisions() { m_noSelfCollisions = true; }
...
void CJointedModel::ComputeMass(void) {
  ...
  while (pList) {
    ...
    if (thickness > 0) {
      m_totalMass += pList->m_surfaceArea * thickness *
                     CUBIC_METERS_PER_CUBIC_INCH * density;
    } else {
...
//-----------------------------------------------------------------------------
static CPhysCollisionModel *InitCollisionModel(CJointedModel &joints,
  ...
  if (!pModel) {
    ...
    // this name is the same as pJointName, but guaranteed to be non-volatile
    // (we'd have to copy pJointName)
    pModel->m_name = joints.m_pModel->localBone[boneIndex].name;
    if (joints.m_pModel->localBone[boneIndex].parent >= 0) {
      pModel->m_parent =
          joints.m_pModel
              ->localBone[joints.m_pModel->localBone[boneIndex].parent]
              .name;
    } else {
...
void CJointedModel::DefaultDrag(float drag) { m_defaultDrag = drag; }
...
//			*worldVerts -
//-----------------------------------------------------------------------------
void ConvertToWorldSpace(CJointedModel &joints, s_source_t *psource,
                         CUtlVector<Vector> &worldVerts) {
  int i, n;
  ...
  if (!joints.m_bAssumeWorldspace) {
    ...
    for (i = 0; i < psource->numvertices; i++) {
      ...
      for (n = 0; n < nBoneCount; n++) {
        ...
        ConcatTransforms(psource->boneToPose[localBone],
                         g_bonetable[globalBone].srcRealign, boneToPose);
        ...
        VectorMA(worldVerts[i], psource->vertex[i].boneweight.weight[n], tmp,
                 worldVerts[i]);
      }
    }
  } else {
    matrix3x4_t srcBoneToWorld[MAXSTUDIOSRCBONES]; // bone transformation matrix
    BuildRawTransforms(psource, "BindPose", 0, psource->scale, psource->adjust,
    ...
    for (i = 0; i < psource->numvertices; i++) {
      ...
      for (n = 0; n < nBoneCount; n++) {
        ...
        VectorTransform(psource->vertex[i].position, srcBoneToWorld[localBone],
                        tmp);
...
//			*boneVerts -
//-----------------------------------------------------------------------------
void ConvertToBoneSpace(s_source_t *psource, int boneIndex,
                        CUtlVector<Vector> &boneVerts) {
  int i;
  ...
  if (remapIndex < 0) {
    ...
    ConcatTransforms(psource->boneToPose[boneIndex],
                     g_bonetable[remapIndex].srcRealign, boneToPose);
  }
...
//-----------------------------------------------------------------------------
// Purpose: Test this face to see if any of its verts are assigned to a
// particular bone Input  : &joints -
//			*pmodel -
//			*face -
//			boneIndex -
// Output : Returns true if this face has a vert assigned to boneIndex
//-----------------------------------------------------------------------------
bool FaceHasVertOnBone(const CJointedModel &joints, s_source_t *pSource,
                       s_face_t *face, int boneIndex) {
  if (boneIndex < 0)
    return true;
...
//-----------------------------------------------------------------------------
// Purpose: Fixup the pointers in this face to reference the mesh globally
// (source relative)
//			(faces are mesh relative, each source has several
//meshes)
// Input  : *pout -
//			*pmesh -
//			*pin -
...
//-----------------------------------------------------------------------------
int CopyVertsByBone(Vector **verts, Vector *worldVerts,
                    const CJointedModel &joints, int boneIndex) {
  int vertCount = 0;
...
//-----------------------------------------------------------------------------
// Purpose: Copy all verts that are referenced by a face which has a vert
// assigned
//			to this bone.
//			NOTE: convex hulls of each bone will overlap at the
//joints
// Input  : **verts -
//			*worldVerts -
//			&joints -
...
// Output : int
//-----------------------------------------------------------------------------
int CopyFaceVertsByBone(Vector **verts, Vector *worldVerts,
                        const CJointedModel &joints, int boneIndex) {
  int vertCount = 0;
  s_source_t *pmodel = joints.m_pModel;
...
void BuildVertWeldTable(int *weldTable, s_source_t *pmodel) {
  for (int i = 0; i < pmodel->numvertices; i++) {
    ...
    for (int j = 0; j < i; j++) {
      float dist =
          (pmodel->vertex[j].position - pmodel->vertex[i].position).Length();
      float normalDist =
          DotProduct(pmodel->vertex[j].normal, pmodel->vertex[i].normal);
...
//			the same ID.  IDs are the index of the lowest numbered
//face on the 			mesh
...
void MarkConnectedMeshes(int *vertID, s_source_t *pmodel, int *vertMap) {
  ...
  for (i = 0; i < pmodel->numvertices; i++) {
    ...
    // as already processed to avoid making additional convex objects out of
    // them.
    if (vertMap[i] != i) {
      vertID[i] = -1;
    } else {
...
//-----------------------------------------------------------------------------
CPhysCollisionModel *FindObjectInList(CPhysCollisionModel *pHead,
                                      const char *pName) {
  while (pHead) {
...
//			*pList -
//-----------------------------------------------------------------------------
void FixBoneList(int *boneMap, const s_source_t *pSource,
                 CPhysCollisionModel *pList) {
  if (!g_bJointed)
    return;
  ...
  while (pmodel) {
    ...
    if (nodeIndex < 0) {
      ...
      if (nodeIndex >= 0) {
        ...
        // The vertices are converted to the new reference by
        // ConvertToWorldSpace(), as well as RemapVerticesToGlobalBones()
        pmodel->m_name =
            g_bonetable[pSource->boneLocalToGlobal[nodeIndex]].name;
        pmodel->m_parent = NULL;
        ...
        if (parentIndex >= 0 && parentIndex != nodeIndex) {
          ...
          if (pSource->boneLocalToGlobal[parentIndex] < 0) {
            ...
            pmodel->m_parent =
                g_bonetable[pSource->boneLocalToGlobal[parentIndex]].name;
          }
        }
      } else {
        MdlWarning("Physics for unknown bone %s\n", pmodel->m_name);
...
//-----------------------------------------------------------------------------
// Purpose: Fixup all references to parents by walking up on models whose
// parents
//			have no collision geometry.  Bones without geometry
//cannot be physically 			simulated, so they must be removed. 			NOTE: This is broken.
//It won't work for tree structures with an empty parent 			(i.e. 2 children
//attached to a parent bone that has no physics geometry - thus empty) 			It will
//not convert that parent into a constraint between 2 children
// Input  : *pList -
//			*pSource -
//			*pParentName -
// Output : const char
//-----------------------------------------------------------------------------
const char *FixParent(CPhysCollisionModel *pList, s_source_t *pSource,
...
void CreateCollide(CPhysCollisionModel *pBase, CPhysConvex **pElements,
                   int elementCount, const boundingvolume_t &bv) {
  int i;
  ...
  // each OBB side So compute it for 0.01% error (on the smallest side, less on
  // larger sides)
...
// is this list of verts contained in a slab of epsilon width?  If so, it's
// probably an error of some kind - we shouldn't be authoring flat or 2d
// collision models
bool IsApproximatelyPlanar(Vector **verts, int vertCount, float epsilon) {
  if (vertCount < 4)
    return true;

  // If we're using an un-welded model, then this may generate a degenerate
  // normal loop to search for an actual plane
  int v0 = 1, v1 = 2;
  Vector normal;
...
void BuildConvexListByVertID(s_source_t *pmodel,
...
void BuildSingleConvexForFaceList(s_source_t *pmodel,
                                  CUtlVector<convexlist_t> &convexList,
                                  CUtlVector<int> &vertList,
...
void BuildConvexListForFaceList(s_source_t *pmodel,
                                CUtlVector<convexlist_t> &convexList,
                                CUtlVector<int> &vertList,
                                const CUtlVector<s_face_t> &faceList) {
  ...
  do {
    ...
    // basically this flood fills ids out to the verts until each island of
    // connected verts shares a single id (so new verts got marked)
    for (i = 0; i < faceList.Count(); i++) {
...
// take a list of convex elements (lists of vert indices into master vert list)
// and build CPhysConvex out of them return true if there are no errors detected
bool BuildConvexesForLists(CUtlVector<CPhysConvex *> &convexOut,
                           const CUtlVector<convexlist_t> &convexList,
                           const CUtlVector<int> &vertList,
                           const CUtlVector<Vector> &worldspaceVerts,
                           bool bRemove2d) {
  bool bValid = true;
  ...
  for (int i = 0; i < convexList.Count(); i++) {
    ...
    for (int j = 0; j < elem.numVertIndex; j++) {
      // this is ok because physcollision won't modify these, but wants
      // non-const
      Vector *pVert = const_cast<Vector *>(
          &worldspaceVerts[vertList[j + elem.firstVertIndex]]);
    ...
    if (vertsThisConvex.Count() > 2) {
      ...
      if (IsApproximatelyPlanar(vertsThisConvex.Base(), vertsThisConvex.Count(),
                                g_epsilon_2d)) {
        if (bRemove2d)
          continue;
        MdlWarning("Model has 2-dimensional geometry (less than %.3f inches "
                   "thick on any axis)!!!\n",
                   g_epsilon_2d);
        bValid = false;
      }
      // go ahead and build it out
      CPhysConvex *pConvex = physcollision->ConvexFromVerts(
          vertsThisConvex.Base(), vertsThisConvex.Count());
      if (pConvex) {
...
int ProcessJointedModel(CJointedModel &joints) {
  ...
  for (int boneIndex = 0; boneIndex < joints.m_pModel->numbones; boneIndex++) {
    ...
    for (int i = 0; i < pmodel->nummeshes; i++) {
      ...
      bValid = BuildConvexesForLists(convexOut, convexList, vertList,
                                     bonespaceVerts, joints.m_remove2d);
    }
    ...
    if (!bValid && convexOut.Count()) {
      MdlWarning(
          "Error with convex elements of %s, building single convex!!!!\n",
          pmodel->filename);
    ...
    if (convexOut.Count()) {
      ...
      CPhysCollisionModel *pPhys =
          InitCollisionModel(joints, pmodel->localBone[boneIndex].name);
      ...
      for (i = 0; i < convexList.Count(); i++) {
        const convexlist_t &elem = convexList[i];
        for (int j = 0; j < elem.numVertIndex; j++) {
          AddPointToBounds(bonespaceVerts[vertList[elem.firstVertIndex + j]],
                           bv.mins, bv.maxs);
          vertCount++;
        }
      }
...
int ProcessSingleBody(CJointedModel &joints) {
  ...
  if (joints.m_allowConcave) {
    ...
    BuildConvexListForFaceList(pmodel, convexList, vertList, faceList);
    bValid = BuildConvexesForLists(convexOut, convexList, vertList,
                                   worldspaceVerts, joints.m_remove2d);
  }
  ...
  if (!convexOut.Count()) {
    ...
    BuildConvexesForLists(convexOut, convexList, vertList, worldspaceVerts,
                          true);
  }
...
//			This checks for overflow, but it's not very robust -
...
//			*pLimitMax -
//-----------------------------------------------------------------------------
void CCmd_JointConstrain(CJointedModel &joints, const char *pJointName,
                         const char *pJointAxis, const char *pJointType,
                         const char *pLimitMin, const char *pLimitMax,
  ...
  joints.AddConstraint(pJointName, axis, jointType, limitMin, limitMax,
                       friction);
}

//-----------------------------------------------------------------------------
// Purpose: Remove a joint from the system (don't create physical geometry for
// it) Input  : &joints -
//			args[][ARG_SIZE] -
//			argCount -
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: verts from the bone named pChild are added to the collision model of
// pParent Input  : *pmodel - source model
//			*pParent - destination bone name
//			*pChild - source bone name
//-----------------------------------------------------------------------------
void CCmd_JointMerge(CJointedModel &joints, const char *pParent,
                     const char *pChild) {
  joints.AddMergeCommand(pParent, pChild);
  joints.MergeBones(FindLocalBoneNamed(joints.m_pModel, pParent),
                    FindLocalBoneNamed(joints.m_pModel, pChild));
}
...
void CCmd_JoinAnimatedFriction(CJointedModel &joints, const char *pMinFriction,
                               const char *pMaxFriction, const char *pTimeIn,
                               const char *pTimeHold, const char *pTimeOut) {
...
void ParseCollisionCommands(CJointedModel &joints) {
  ...
  while (GetToken(true)) {
    ...
    // joint commands
      ...
      CCmd_JointConstrain(joints, args[0], args[1], args[2], args[3], args[4],
                          pFriction);
    }
    // joint properties
      ...
      if (argCount == 5) {
        CCmd_JoinAnimatedFriction(joints, args[0], args[1], args[2], args[3],
                                  args[4]);
      }
...
static bool LoadSurfaceProps(const char *pMaterialFilename) {
  ...
  FileHandle_t fp =
      g_pFileSystem->Open(pMaterialFilename, "rb", TOOLS_READ_PATH_ID);
...
//-----------------------------------------------------------------------------
// Purpose: Entry point for script processing.  Delegate to necessary
// subroutines.
//			Parse the collisionmodel {} and collisionjoints {}
//chunks
// Input  : separateJoints - whether this has a constraint system or not (true
// if it does) Output : int
//-----------------------------------------------------------------------------
int DoCollisionModel(bool separateJoints) {
  ...
  CreateInterfaceFn physicsFactory =
      Sys_GetFactory(Sys_LoadModule("vphysics.dll"));
  if (!physicsFactory)
    return 0;
  ...
  if (nummaterials && numtextures &&
      (numtextures != g_numtextures || nummaterials != g_nummaterials)) {
...
void KeyWriteIntPair(FILE *fp, const char *pKeyName, int outputData0,
                     int outputData1) {
...
void KeyWriteVector3(FILE *fp, const char *pKeyName, const Vector &outputData) {
  fprintf(fp, "\"%s\" \"%f %f %f\"\n", pKeyName, outputData[0], outputData[1],
          outputData[2]);
}
...
void FixCollisionHierarchy(CJointedModel &joints) {
  if (joints.m_pCollisionList) {
    ...
    for (; pPhys; pPhys = pPhys->m_pNext) {
      pPhys->m_parent =
          FixParent(joints.m_pCollisionList, joints.m_pModel, pPhys->m_parent);
    }
    ...
    while (pList) {
      pList->m_pJointName = FixParent(joints.m_pCollisionList, joints.m_pModel,
                                      pList->m_pJointName);
      pList = pList->m_pNext;
    }
...
void BuildRagdollConstraint(CPhysCollisionModel *pPhys,
  ...
  while (pList) {
    ...
    CPhysCollisionModel *pListModel =
        g_JointedModel.GetCollisionModel(pList->m_pJointName);
    if (index < 0) {
      MdlError("Rotation constraint on bone \"%s\" which does not appear in "
               "collision model!!!\n",
               pList->m_pJointName);
    } else if ((!pListModel->m_parent ||
                g_JointedModel.CollisionIndex(pListModel->m_parent) < 0) &&
      ...
      case JOINT_LIMIT:
        ragdoll.axes[pList->m_axis].SetAxisFriction(
            pList->m_limitMin, pList->m_limitMax, pList->m_friction);
        break;
      case JOINT_FIXED:
        ragdoll.axes[pList->m_axis].SetAxisFriction(0, 0, 0);
        break;
      case JOINT_FREE:
        ragdoll.axes[pList->m_axis].SetAxisFriction(-360, 360,
                                                    pList->m_friction);
        break;
      }
    }
...
float GetCollisionModelMass() { return g_JointedModel.m_totalMass; }
...
void CollisionModel_ExpandBBox(Vector &mins, Vector &maxs) {
  ...
  if (g_JointedModel.m_pCollisionList) {
    ...
    physcollision->CollideGetAABB(
        &collideMins, &collideMaxs,
        g_JointedModel.m_pCollisionList->m_pCollisionData, vec3_origin,
        vec3_angle);
...
void CollisionModel_Write(long checkSum) {
  if (g_JointedModel.m_pCollisionList) {
    ...
    if (fp) {
      ...
      pPhys = g_JointedModel.m_pCollisionList;
      while (pPhys) {
        pPhys->m_mass = ((pPhys->m_volume * pPhys->m_massBias) / volume) *
                        g_JointedModel.m_totalMass;
        if (pPhys->m_mass < 1.0)
          pPhys->m_mass = 1.0;
      ...
      pPhys = g_JointedModel.m_pCollisionList;
      while (pPhys) {
        // check to see if bone collapse/remap has left this with parent
        // pointing at itself
        if (pPhys->m_parent) {
          constraint_ragdollparams_t ragdoll;
      ...
      if (g_JointedModel.m_noSelfCollisions) {
        ...
        while (pPair) {
          ...
          if (pPair->obj0 >= 0 && pPair->obj1 >= 0 &&
              pPair->obj0 != pPair->obj1) {
      ...
      if (g_JointedModel.m_bHasAnimatedFriction == true) {
        ...
        KeyWriteFloat(fp, "animfrictionmin",
                      g_JointedModel.m_iMinAnimatedFriction);
        KeyWriteFloat(fp, "animfrictionmax",
                      g_JointedModel.m_iMaxAnimatedFriction);
        KeyWriteFloat(fp, "animfrictiontimein",
                      g_JointedModel.m_flFrictionTimeIn);
        KeyWriteFloat(fp, "animfrictiontimeout",
                      g_JointedModel.m_flFrictionTimeOut);
        KeyWriteFloat(fp, "animfrictiontimehold",
                      g_JointedModel.m_flFrictionTimeHold);
        fprintf(fp, "}\n");
      }
      ...
      for (int k = 0; k < g_JointedModel.m_mergeList.Count(); k++) {
        ...
        Q_snprintf(buf, sizeof(buf), "%s,%s",
                   g_JointedModel.m_mergeList[k].pParent,
      ...
      if (g_JointedModel.m_textCommands.Size()) {
        fwrite(g_JointedModel.m_textCommands.Base(),
               g_JointedModel.m_textCommands.Size(), 1, fp);
      }
