...
static void WriteBoneInfo(studiohdr_t *phdr) {
  ...
  for (i = 0; i < g_numbones; i++) {
    ...
    AngleQuaternion(RadianEuler(g_bonetable[i].rot[0], g_bonetable[i].rot[1],
                                g_bonetable[i].rot[2]),
                    pbone[i].quat);
  ...
  if (g_numaimatbones) {
    ...
    for (i = 0; i < g_numaimatbones; i++) {
      ...
      pbone[k].procindex = (byte *)&pProc[i] - (byte *)&pbone[k];
      pbone[k].proctype = g_aimatbones[j].aimAttach == -1
                              ? STUDIO_PROC_AIMATBONE
                              : STUDIO_PROC_AIMATATTACH;
      pProc[i].parent = g_aimatbones[j].parent;
      pProc[i].aim = g_aimatbones[j].aimAttach == -1
                         ? g_aimatbones[j].aimBone
                         : g_aimatbones[j].aimAttach;
  ...
  for (i = 0; i < g_numattachments; i++) {
    ...
    AddToStringTable(&pattachment[i], &pattachment[i].sznameindex,
                     g_attachment[i].name);
  ...
  for (int s = 0; s < g_hitboxsets.Size(); s++, hitboxset++) {
    ...
    for (i = 0; i < hitboxset->numhitboxes; i++) {
      ...
      AddToStringTable(&(pbbox[i]), &(pbbox[i].szhitboxnameindex),
                       set->hitbox[i].hitboxname);
    }
...
static void WriteSequenceInfo(studiohdr_t *phdr) {
  ...
  if (g_vecPreexistingSequences.Count()) {
    ...
    if (g_sequence.Count() < g_vecPreexistingSequences.Count() &&
        !bAllowSequenceRemoval) {
      Msg("\n");
      MdlWarning("This model has fewer sequences than its predecessor.\nPlease "
                 "confirm sequence deletion: [y/n] ");
      int nInput = 0;
      do {
        nInput = getchar();
      ...
      for (i = 0; i < g_sequence.Count(); i++) {
        ...
        if (nIdx >= 0) {
          ...
          if (i < g_vecPreexistingSequences.Count()) {
            Msg("  Found new sequence \"%s\" using index of old sequence "
                "\"%s\".\n",
  ...
  // build an inverted remapping table so autolayer sequence indices can find
  // their sources later
  ...
  for (m = 0; m < g_sequence.Count(); m++, pseqdesc++) {
    ...
    if (bUseSeqOrderRemapping) {
      ...
      if (i != m) {
        Msg("   Remapping sequence %i to index %i (%s) to retain existing "
            "order.\n",
    ...
    AddToStringTable(pseqdesc, &pseqdesc->szactivitynameindex,
                     g_sequence[i].activityname);
    ...
    if (g_sequence[i].groupsize[0] > 1 || g_sequence[i].groupsize[1] > 1) {
      ...
      pData +=
          (pseqdesc->groupsize[0] + pseqdesc->groupsize[1]) * sizeof(float);
    ...
    for (j = 0; j < g_sequence[i].numevents; j++) {
      ...
      if (g_sequence[i].event[j].frame <= k)
        ...
        MdlWarning("Event %d (frame %d) out of range in %s\n",
                   g_sequence[i].event[j].event, g_sequence[i].event[j].frame,
                   g_sequence[i].name);
        bErrors = true;
      }
      ...
      if (V_isdigit(g_sequence[i].event[j].eventname[0])) {
        ...
        AddToStringTable(&pevent[j], &pevent[j].szeventindex,
                         g_sequence[i].event[j].eventname);
        pevent[j].type = NEW_EVENT_STYLE;
      }

      // printf("%4d : %d %f\n", pevent[j].event, g_sequence[i].event[j].frame,
      // pevent[j].cycle ); AddToStringTable( &pevent[j],
      // &pevent[j].szoptionindex, g_sequence[i].event[j].options );
    ...
    for (j = 0; j < g_sequence[i].numautolayers; j++) {
      ...
      // autolayer indices are stored by index, so remap them now using the
      // invertex lookup table
      if (bUseSeqOrderRemapping) {
        int nRemapAutoLayer =
            nSeqOrderRemappingTableInv[pautolayer[j].iSequence];
      ...
      if (!(pautolayer[j].flags & STUDIO_AL_POSE)) {
        pautolayer[j].start = g_sequence[i].autolayer[j].start /
                              (g_sequence[i].panim[0][0]->numframes - 1);
    ...
    for (k = 0; k < m; k++) {
      ...
      if (pseqdesc[k - m].pBoneweight(0) > pweight) {
        ...
        for (j = 0; j < g_numbones; j++) {
          // we're not walking the linear sequence list if we're remapping, so
          // we need to remap this check
          int nRemap = k;
          if (bUseSeqOrderRemapping)
    ...
    pData += (g_sequence[i].groupsize[0] * g_sequence[i].groupsize[1]) *
             sizeof(short);
    ALIGN4(pData);
...
virtualmodel_t *studiohdr_t::GetVirtualModel(void) const { return NULL; }

const studiohdr_t *virtualgroup_t::GetStudioHdr(void) const {
  return (studiohdr_t *)cache;
}

byte *studiohdr_t::GetAnimBlock(int i) const { return NULL; }

int studiohdr_t::GetAutoplayList(unsigned short **pOut) const { return 0; }
...
void WriteAnimationData(s_animation_t *srcanim, mstudioanimdesc_t *destanimdesc,
                        byte *&pLocalData, byte *&pExtData) {
  int j, k, n;
  ...
  for (int w = 0; w < srcanim->numsections; w++) {
    ...
    if (pExtData != NULL && !srcanim->disableAnimblocks &&
        !(w == 0 && srcanim->isFirstSectionLocal)) {
      pData = pExtData;
      bUseExtData = true;
    }
    ...
    for (j = 0; j < g_numbones; j++) {
      ...
      numPos[(psrcdata->num[0] != 0) + (psrcdata->num[1] != 0) +
             (psrcdata->num[2] != 0)]++;
      numAxis[(psrcdata->num[3] != 0) + (psrcdata->num[4] != 0) +
              (psrcdata->num[5] != 0)]++;

      if (psrcdata->num[0] + psrcdata->num[1] + psrcdata->num[2] +
      ...
      if ((srcanim->numframes == 1) ||
          (psrcdata->num[0] <= 2 && psrcdata->num[1] <= 2 &&
           psrcdata->num[2] <= 2 && psrcdata->num[3] <= 2 &&
           psrcdata->num[4] <= 2 && psrcdata->num[5] <= 2)) {
        // printf("%d : %d %d %d : %d %d %d\n", j, psrcdata->num[0],
        // psrcdata->num[1], psrcdata->num[2], psrcdata->num[3],
        // psrcdata->num[4], psrcdata->num[5] ); single frame, if animation
        // detected just store as raw
        int iFrame = min(w * srcanim->sectionframes, srcanim->numframes - 1);
        if (psrcdata->num[3] != 0 || psrcdata->num[4] != 0 ||
            psrcdata->num[5] != 0) {
          Quaternion q;
        ...
        if (psrcdata->num[0] != 0 || psrcdata->num[1] != 0 ||
            psrcdata->num[2] != 0) {
        ...
        if (psrcdata->num[3] >= srcanim->numframes &&
            psrcdata->num[4] >= srcanim->numframes &&
        ...
        if (rotvptr) {
          ...
          for (k = 3; k < 6; k++) {
            if (psrcdata->num[k] == 0) {
              ...
              rotvptr->offset[k - 3] =
                  ((byte *)destanimvalue - (byte *)rotvptr);
    ...
    if (destanimdesc->sectionindex) {
      if (bUseExtData) {
        if (g_numanimblocks &&
            pData - g_animblock[g_numanimblocks - 1].start > g_animblocksize) {
        ...
        destanimdesc->pSection(w)->animindex =
            pStartSection - g_animblock[g_numanimblocks - 1].start;
      } else {
        destanimdesc->pSection(w)->animblock = 0;
        destanimdesc->pSection(w)->animindex =
            pStartSection - (byte *)destanimdesc;
      }
      // printf("%s (%d) : %d:%d\n", srcanim->name, w,
      // destanimdesc->pSection(w)->animblock,
      // destanimdesc->pSection(w)->animindex );
    }
...
byte *WriteIkErrors(s_animation_t *srcanim, byte *pData) {
  ...
  for (j = 0; j < srcanim->numikrules; j++) {
    ...
    if (srcanim->numframes > 1.0) {
      ...
      pikrule->contact =
          srcanim->ikrule[j].contact / (srcanim->numframes - 1.0f);
    } else {
      pikrule->start = 0.0f;
      pikrule->peak = 0.0f;
      pikrule->tail = 1.0f;
    ...
    printf("%d %d %d %d : %.2f %.2f %.2f %.2f\n",
            srcanim->ikrule[j].start, srcanim->ikrule[j].peak,
    srcanim->ikrule[j].tail, srcanim->ikrule[j].end, pikrule->start,
    pikrule->peak, pikrule->tail, pikrule->end );
    */
    ...
    mstudiocompressedikerror_t *pCompressed =
        (mstudiocompressedikerror_t *)pData;
    ...
    for (k = 0; k < 6; k++) {
      ...
      int size =
          srcanim->ikrule[j].errorData.numanim[k] * sizeof(mstudioanimvalue_t);
...
#endif
    // AddToStringTable( pikrule, &pikrule->szattachmentindex,
    // srcanim->ikrule[j].attachment );
  }
...
byte *WriteLocalHierarchy(s_animation_t *srcanim, byte *pData) {
  ...
  for (j = 0; j < srcanim->numlocalhierarchy; j++) {
    ...
    if (srcanim->numframes > 1.0) {
      pHierarchy->start =
          srcanim->localhierarchy[j].start / (srcanim->numframes - 1.0f);
      pHierarchy->peak =
          srcanim->localhierarchy[j].peak / (srcanim->numframes - 1.0f);
      pHierarchy->tail =
          srcanim->localhierarchy[j].tail / (srcanim->numframes - 1.0f);
      pHierarchy->end =
          srcanim->localhierarchy[j].end / (srcanim->numframes - 1.0f);
    } else {
    ...
    for (k = 0; k < 6; k++) {
      ...
      pCompressed->offset[k] = (pData - (byte *)pCompressed);
      int size = srcanim->localhierarchy[j].localData.numanim[k] *
...
#endif
    // AddToStringTable( pHierarchy, &pHierarchy->szattachmentindex,
    // srcanim->ikrule[j].attachment );
  }
...
static byte *WriteAnimations(byte *pData, byte *pStart, studiohdr_t *phdr) {
  ...
  for (i = 0; i < g_numani; i++) {
    ...
    if (srcanim->piecewisemove[j].pos[0] != 0 ||
        srcanim->piecewisemove[j].pos[1] != 0) {
      float t = (srcanim->numframes - 1) / srcanim->fps;

      float r = 1 / t;

      float a = atan2(srcanim->piecewisemove[j].pos[1],
                      srcanim->piecewisemove[j].pos[0]) *
                (180 / M_PI);
      float d = sqrt(DotProduct(srcanim->piecewisemove[j].pos,
                                srcanim->piecewisemove[j].pos));
      if (g_verbose) {
    ...
    if (!pBlockStart ||
        (g_bonesaveframe.Count() == 0 && srcanim->numframes == 1)) {
      // hack
    ...
    if (srcanim->disableAnimblocks || srcanim->isFirstSectionLocal) {
      ...
      if (destanim->sectionindex) {
        // if sections were written, don't move the data already written to the
        // last block
    ...
    // printf("%d %x %x %x   %s : %d\n", g_numanimblocks - 1,
    // g_animblock[g_numanimblocks-1].start, pBlockData, pBlockEnd,
    // srcanim->name, srcanim->numsections );

    if (pBlockData != pBlockEnd &&
        pBlockEnd - g_animblock[g_numanimblocks - 1].start > g_animblocksize) {
    ...
    if (srcanim->numikrules) {
      ...
      if (destanim->animblock == 0) {
        destanim->ikruleindex =
            pIkData - g_animblock[destanim->animblock].start;
      } else {
    ...
    if (srcanim->numlocalhierarchy) {
      ...
      destanim->localhierarchyindex =
          pLocalHierarchy - g_animblock[destanim->animblock].start;
    }
    ...
    // printf("%s : %d:%d\n", srcanim->name, destanim->animblock,
    // destanim->animindex );

    // printf("raw bone data %d : %s\n", (byte *)destanimvalue - pData,
    // srcanim->name);
  }
  ...
  if (!g_quiet) {
    ...
    {
            printf("%2d (%3d:%3d): %d\n", i, g_animblock[i].iStartAnim,
    g_animblock[i].iEndAnim, g_animblock[i].end - g_animblock[i].start );
    }
    */
  }
    ...
    if (panimdesc[i].animblock != 0) {
      ...
      if (k > 2) {
        panimdesc[i].zeroframecount =
            min((panimdesc[i].numframes - 1) / panimdesc[i].zeroframespan,
      ...
      for (j = 0; j < g_numbones; j++) {
        if (g_bonetable[j].flags & BONE_HAS_SAVEFRAME_POS) {
          for (int n = 0; n < panimdesc[i].zeroframecount; n++) {
            *(Vector48 *)pData =
                anim->sanim[panimdesc[i].zeroframespan * n][j].pos;
            pData += sizeof(Vector48);
          }
        }
        if (g_bonetable[j].flags & BONE_HAS_SAVEFRAME_ROT) {
          for (int n = 0; n < panimdesc[i].zeroframecount; n++) {
            Quaternion q;
            AngleQuaternion(anim->sanim[panimdesc[i].zeroframespan * n][j].rot,
                            q);
            *((Quaternion64 *)pData) = q;
            pData += sizeof(Quaternion64);
          }
...
static void WriteBoneTransforms(studiohdr2_t *phdr, mstudiobone_t *pBone) {
  ...
  for (int i = 0; i < g_numbones; i++) {
    ...
    if (MatricesAreEqual(identity, g_bonetable[i].srcRealign) &&
        ((nParent < 0) ||
         MatricesAreEqual(identity, g_bonetable[nParent].srcRealign)))
      continue;
  ...
  for (int i = 0; i < g_numbones; i++) {
    ...
    // So, when we realign a bone, we want to do it in a way so that the child
    // bones have the same bone->world transform. If we take T as the src
    // realignment transform for the parent, P is the parent to world, and C is
    // the child to parent, we expect the child->world is constant after
    // realignment:
    //		CtoW = P * C = ( P * T ) * ( T^-1 * C )
    // therefore Cnew = ( T^-1 * C )
    if (nParent >= 0) {
      MatrixInvert(g_bonetable[nParent].srcRealign,
                   pSrcBoneTransform[bt].pretransform);
    } else {
    ...
    AddToStringTable(&pSrcBoneTransform[bt], &pSrcBoneTransform[bt].sznameindex,
                     g_bonetable[i].name);
    ++bt;
  }
  ALIGN4(pData);
...
static void WriteBoneFlexDrivers(studiohdr2_t *pStudioHdr2) {
  ...
  const int nBoneFlexDriverCount =
      pDmeBoneFlexDriverList->m_eBoneFlexDriverList.Count();
  ...
  for (int i = 0; i < nBoneFlexDriverCount; ++i) {
    ...
    pBoneFlexDriver->m_nBoneIndex =
        pDmeBoneFlexDriver->GetValue<int>("__boneIndex", 0);
    pBoneFlexDriver->m_nControlCount =
        pDmeBoneFlexDriver->m_eControlList.Count();
    pBoneFlexDriver->m_nControlIndex = pData - (byte *)pBoneFlexDriver;

    mstudioboneflexdrivercontrol_t *pControl =
        reinterpret_cast<mstudioboneflexdrivercontrol_t *>(pData);

    for (int j = 0; j < pBoneFlexDriver->m_nControlCount; ++j) {
      CDmeBoneFlexDriverControl *pDmeControl =
      ...
      pControl[j].m_nFlexControllerIndex =
          pDmeControl->GetValue<int>("__flexControlIndex", 0);
    ...
    pData += pBoneFlexDriver->m_nControlCount *
             sizeof(mstudioboneflexdrivercontrol_t);
    ALIGN4(pData);
...
static void WriteVertices(studiohdr_t *phdr) {
  ...
  for (i = 0; i < g_nummodelsbeforeLOD; i++) {
    ...
    for (j = 0; j < pLodData->numvertices; j++) {
      //			printf( "saving bone weight %d for model %d at
      //0x%p\n", 				j, i, &pbone[j] );
...
float ComputeVertAnimFixedPointScale(studiohdr_t *pStudioHdr) {
  ...
  for (int j = 0; j < g_numflexkeys; ++j) {
    ...
    const bool bWrinkleVAnim =
        (g_flexkey[j].vanimtype == STUDIO_VERT_ANIM_WRINKLE);
...
static void WriteModel(studiohdr_t *phdr) {
  ...
  for (i = 0, j = 0; i < g_numbodyparts; i++) {
    AddToStringTable(&pbodypart[i], &pbodypart[i].sznameindex,
                     g_bodypart[i].name);
  ...
  for (j = 0; j < g_numflexcontrollers; j++) {
    AddToStringTable(pflexcontroller, &pflexcontroller->sznameindex,
                     g_flexcontroller[j].name);
    AddToStringTable(pflexcontroller, &pflexcontroller->sztypeindex,
                     g_flexcontroller[j].type);
  ...
  mstudioflexcontrollerui_t *pFlexControllerUI =
  ...
  bool *pControllerHandled =
      (bool *)_alloca(g_numflexcontrollers * sizeof(bool));
  ...
  for (j = 0; j < g_numflexcontrollers; ++j) {
    ...
    for (k = 0; k < g_FlexControllerRemap.Count(); ++k) {
      ...
      if (j == remap.m_Index || j == remap.m_LeftIndex ||
          j == remap.m_RightIndex || j == remap.m_MultiIndex) {
        AddToStringTable(pFlexControllerUI, &pFlexControllerUI->sznameindex,
                         remap.m_Name);
    ...
    if (!found) {
      ...
      if (j < g_numflexcontrollers - 1 &&
          ...
          StringAfterPrefixCaseSensitive(g_flexcontroller[j + 1].name,
                                         "left_") &&
          !Q_strcmp(
              StringAfterPrefixCaseSensitive(flexcontroller.name, "right_"),
              StringAfterPrefixCaseSensitive(g_flexcontroller[j + 1].name,
        ...
        pControllerHandled[j] = true;
                 ...
                 StringAfterPrefixCaseSensitive(g_flexcontroller[j - 1].name,
                                                "right_") &&
                 !Q_strcmp(StringAfterPrefixCaseSensitive(flexcontroller.name,
                                                          "left_"),
                           StringAfterPrefixCaseSensitive(
                               g_flexcontroller[j - 1].name, "right_"))) {
        AddToStringTable(pFlexControllerUI, &pFlexControllerUI->sznameindex,
                         flexcontroller.name + 5);
  ...
  for (i = 0; i < g_nummodelsbeforeLOD; i++) {
    ...
    // AddToStringTable( &pmodel[i], &pmodel[i].sznameindex,
    // g_model[i]->filename );
    ...
    for (m = 0; m < pmodel[i].nummeshes; m++) {
      ...
      if (pmesh[m].numflexes) {
        ...
        for (j = 0; j < g_numflexkeys; j++) {
          ...
          // g_flexdesc[g_flexkey[j].flexdesc].FACS, g_flexkey[j].target0,
          // g_flexkey[j].target1, g_flexkey[j].target2, g_flexkey[j].target3 );
          // if (j < 9) printf("%d %d %s : %d (%d) %f\n", j,
          // g_flexkey[j].flexdesc, g_flexdesc[g_flexkey[j].flexdesc].FACS,
          // g_flexkey[j].numvanims, pflex->numverts, g_flexkey[j].target );

          // printf("%d %d : %d %f\n", j, g_flexkey[j].flexnum,
          // g_flexkey[j].numvanims, g_flexkey[j].target );

          pvanim = g_flexkey[j].vanim;

          bool bWrinkleVAnim =
              (pflex->vertanimtype == STUDIO_VERT_ANIM_WRINKLE);
          int nVAnimDeltaSize = bWrinkleVAnim
            ...
            if (n >= 0 && n < pmesh[m].numvertices) {
              ...
              if (bWrinkleVAnim) {
                ((mstudiovertanim_wrinkle_t *)pvertanim)
                    ->SetWrinkleFixed(pvanim->wrinkle,
                                      flVertAnimFixedPointScale);
              }

              pvertanim =
                  (mstudiovertanim_t *)((byte *)pvertanim + nVAnimDeltaSize);
              ...
              */
              ...
              // pvanim->pos[1], pvanim->pos[2] );
            }
            // printf("%d %.2f %.2f %.2f\n", pvanim->vertex, pvanim->pos[0],
            // pvanim->pos[1], pvanim->pos[2] );
            pvanim++;
          }
          pflex++;
  ...
  for (i = 0; i < g_numincludemodels; i++) {
    AddToStringTable(pincludemodel, &pincludemodel->sznameindex,
                     g_includemodel[i].name);
    pincludemodel++;
  }
  ...
  for (i = 1; i < g_numanimblocks; i++) {
    ...
    // printf("block %d : %x %x (%d)\n", i, panimblock[i].datastart,
    // panimblock[i].dataend, panimblock[i].dataend - panimblock[i].datastart );
  }
...
void LoadMaterials(studiohdr_t *phdr) {
  ...
  if (phdr->textureindex != 0) {
    for (i = 0; i < phdr->numtextures; i++) {
      ...
      // search through all specified directories until a valid material is
      // found
      for (j = 0; j < phdr->numcdtextures && IsErrorMaterial(pMaterial); j++) {
        ...
        pMaterial =
            g_pMaterialSystem->FindMaterial(szPath, TEXTURE_GROUP_OTHER, false);
      }
      ...
      IMaterialVar *clientShaderVar =
          pMaterial->FindVar("$clientShader", &found, false);
      if (found) {
...
void WriteSeqKeyValues(mstudioseqdesc_t *pseqdesc,
                       CUtlVector<char> *pKeyValue) {
...
void WriteModelFiles(void) {
  ...
  */
  ...
  phdr->checksum = 0;
  for (i = 0; i < total; i += 4) {
    // TODO: does this need something more than a simple shift left and add
    // checksum?
  ...
  if (pBlockStart) {
    ...
    if (g_bX360) {
      ...
      void *pOutBase =
          kalloc(1, pblockhdr->length + BYTESWAP_ALIGNMENT_PADDING);
      int finalSize = StudioByteSwap::ByteswapANI(phdr, pOutBase, pBlockStart,
                                                  pblockhdr->length);
      if (finalSize == 0) {
  ...
  // NOTE!  If you don't want to go through the effort of loading studiorender
  // for perf reasons, make sure spewFlags ends up being zero.
...
const vertexFileHeader_t *mstudiomodel_t::CacheVertexData(void *pModelData) {
  ...
  if (pVertexHdr->id != MODEL_VERTEX_FILE_ID) {
    MdlError("Error Vertex File: '%s' (id %d should be %d)\n", filename,
             pVertexHdr->id, MODEL_VERTEX_FILE_ID);
  }
...
//-----------------------------------------------------------------------------
void FindVertexOffsets(int vertexOffset, int offsets[MAX_NUM_LODS],
                       int counts[MAX_NUM_LODS], int numLods,
  ...
  // vertexOffset uniquely identifies a single mesh's vertexes in lod vertex
  // sorted list lod vertex list is sorted from lod N..lod 0
...
//-----------------------------------------------------------------------------
bool BuildSortedVertexList(const studiohdr_t *pStudioHdr, const void *pVtxBuff,
                           vertexPool_t **ppVertexPools, int *pNumVertexPools,
                           usedVertex_t **ppVertexList, int *pNumVertexes) {
  ...
  *pNumVertexes = 0;
  ...
  for (i = 0; i < pVtxHdr->numBodyParts; i++) {
    ...
    for (j = 0; j < pBodyPartHdr->numModels; j++) {
      ...
      for (k = 0; k < pStudioModel->nummeshes; k++) {
        ...
        if (numMeshVertexes) {
          usedVertexes =
              (usedVertex_t *)malloc(numMeshVertexes * sizeof(usedVertex_t));
          pVertexMap = (unsigned short *)malloc(numMeshVertexes *
                                                sizeof(unsigned short));

          for (n = 0; n < numMeshVertexes; n++) {
            // setup mapping
            // due to the hierarchial layout, the vertID's map per mesh's pool
            // a linear layout of the vertexes requires a unique signature to
            // achieve a remap the offset and index form a unique signature
        ...
        for (k = 0; k < pModelLODHdr->numMeshes; k++) {
          ...
          for (m = 0; m < pMeshHdr->numStripGroups; m++) {
            ...
            for (n = 0; n < pStripGroupHdr->numVerts; n++) {
              ...
              if (pStripVertex->origMeshVertID < 0 ||
                  pStripVertex->origMeshVertID >= pPool->numVertexes)
                return false;

              // arrange binary flags for numerical sorting
              // the lowest detail lod's verts at the top, the root lod's verts
              // at the bottom
              pPool->pVertexList[pStripVertex->origMeshVertID].lodFlags |=
                  1 << currLod;
            }
          }
        }
  ...
  for (i = 0; i < numVertexPools; i++) {
    ...
    for (j = 0; j < pPool->numVertexes; j++) {
      if (!pPool->pVertexList[j].lodFlags) {
        // found an orphaned vertex that is unreferenced at any lod strip
        // winding don't know how these occur or who references them cannot cull
        // the orphaned vertexes, otherwise vertex counts are wrong every vertex
        // must be remapped force the vertex to belong to the lowest lod lod
        // flags must be nonzero for proper sorted runs
        pPool->pVertexList[j].lodFlags = 1 << (pVtxHdr->numLODs - 1);
      }
    }

    memcpy(&pVertexList[numVertexes], pPool->pVertexList,
           pPool->numVertexes * sizeof(usedVertex_t));
  ...
  // build a mapping table from mesh relative indexes to the flat lod sorted
  // array
  vertexOffset = 0;
  for (i = 0; i < numVertexPools; i++) {
    pPool = &pVertexPools[i];
  ...
  // calc final fixed vertex location if vertexes were gathered to mesh order
  // from lod sorted list
  finalMeshVertID = 0;
  poolStart = 0;
  for (i = 0; i < pStudioHdr->numbodyparts; i++) {
    pStudioBodyPart = pStudioHdr->pBodypart(i);
    for (j = 0; j < pStudioBodyPart->nummodels; j++) {
      ...
      for (m = 0; m < pStudioModel->nummeshes; m++) {
        ...
        // vertexOffset works as unique key to identify vertexes for a specific
        // mesh a mesh's verts are distributed, but guaranteed sequential in the
        // lod sorted vertex list determine base index and offset and run length
        // for target mesh for all lod levels
        FindVertexOffsets(vertexOffset, offsets, counts, pVtxHdr->numLODs,
                          pVertexList, numVertexes);
        ...
        // set its expected final vertex id, which is its "gathered" index
        // relative to mesh
        ...
        for (n = pVtxHdr->numLODs - 1; n >= 0; n--) {
          ...
          for (p = 0; p < counts[n]; p++) {
            pVertexList[offsets[n] + p].finalMeshVertID =
...
//-----------------------------------------------------------------------------
bool FixupVVDFile(const char *fileName, const studiohdr_t *pStudioHdr,
                  const void *pVtxBuff, const vertexPool_t *pVertexPools,
                  int numVertexPools, const usedVertex_t *pVertexList,
                  int numVertexes) {
  OptimizedModel::FileHeader_t *pVtxHdr;
  ...
  *pFileHdr_new = *pFileHdr_old;
  ...
  for (n = pVtxHdr->numLODs; n < MAX_NUM_LODS; n++) {
    ...
    pFileHdr_new->numLODVertexes[n] =
        pFileHdr_new->numLODVertexes[pVtxHdr->numLODs - 1];
  }
  ...
  if (numFixups) {
      ...
      for (j = 0; j < pStudioBodyPart->nummodels; j++) {
        ...
        for (k = 0; k < pStudioModel->nummeshes; k++) {
          ...
          for (n = pVtxHdr->numLODs - 1; n >= 0; n--) {
            ...
            pFixupTable[numOutFixups].numVertexes =
                pLodMeshInfo->numVertexes[n];
    ...
    if (numOutFixups != numFixups) {
      // logic sync error, final calc should match precalc, otherwise memory
      // corruption
      return false;
    }
  }
  ...
  numFlat = 0;
  pFlatVertexes =
      (mstudiovertex_t **)malloc(numVertexes * sizeof(mstudiovertex_t *));
  ...
  for (i = 0; i < pStudioHdr->numbodyparts; i++) {
    ...
    for (j = 0; j < pStudioBodyPart->nummodels; j++) {
      pStudioModel = pStudioBodyPart->pModel(j);
      pVertex_old =
          (mstudiovertex_t *)&pVertexBase_old[pStudioModel->vertexindex];
      ...
      for (k = 0; k < pStudioModel->nummeshes; k++) {
        ...
        for (n = 0; n < pStudioMesh->numvertices; n++) {
          // old vertex pools are per model, seperated per mesh by a start
          // offset vertexes are then isolated subpools per mesh build the flat
          // linear array of lookup pointers
...
//-----------------------------------------------------------------------------
bool FixupVTXFile(const char *fileName, const studiohdr_t *pStudioHdr,
                  const vertexPool_t *pVertexPools, int numVertexPools,
                  const usedVertex_t *pVertexList, int numVertexes) {
  OptimizedModel::FileHeader_t *pVtxHdr;
  OptimizedModel::BodyPartHeader_t *pBodyPartHdr;
  OptimizedModel::ModelHeader_t *pModelHdr;
        ...
        for (k = 0; k < pModelLODHdr->numMeshes; k++) {
          ...
          for (m = 0; m < pMeshHdr->numStripGroups; m++) {
            ...
            for (n = 0; n < pStripGroupHdr->numVerts; n++) {
              ...
              // remap old mesh relative vertex index to absolute flat sorted
              // list
              newMeshVertID = pVertexPools[poolStart + k]
                                  .pVertexMap[pStripVertex->origMeshVertID];

              // map to expected final fixed vertex locations
              // final fixed vertex location is performed by runtime loading
              // code
...
//-----------------------------------------------------------------------------
bool FixupMDLFile(const char *fileName, studiohdr_t *pStudioHdr,
                  const void *pVtxBuff, const vertexPool_t *pVertexPools,
                  int numVertexPools, const usedVertex_t *pVertexList,
                  int numVertexes) {
  OptimizedModel::FileHeader_t *pVtxHdr;
    ...
    for (j = 0; j < pStudioBodyPart->nummodels; j++) {
      ...
      for (m = 0; m < pStudioModel->nummeshes; m++) {
        ...
        for (p = n; p < MAX_NUM_LODS; p++) {
          ...
          pStudioMesh->vertexdata.numLODVertexes[p] =
              pStudioMesh->vertexdata.numLODVertexes[numLODs - 1];
        }
        ...
        for (n = 0; n < pStudioMesh->numflexes; n++) {
          ...
          for (p = 0; p < pStudioFlex->numverts;
               p++, pvanim += nVAnimSizeBytes) {
            pStudioVertAnim = (mstudiovertanim_t *)(pvanim);

            if (pStudioVertAnim->index < 0 ||
                pStudioVertAnim->index >= pStudioMesh->numvertices)
              return false;

            // remap old mesh relative vertex index to absolute flat sorted list
            newMeshVertID =
                pVertexPools[numMeshes + m].pVertexMap[pStudioVertAnim->index];
...
//
// VVD files get vertexes fixed to a flat sorted order, ascending in lower
// detail lod usage VTX files get their windings remapped to the sort.
//-----------------------------------------------------------------------------
bool FixupToSortedLODVertexes(studiohdr_t *pStudioHdr) {
  char filename[MAX_PATH];
  ...
  if (!BuildSortedVertexList(pStudioHdr, pVtxBuff, &pVertexPools,
                             &numVertexPools, &pVertexList, &numVertexes)) {
    // data sync error
    return false;
  }
  ...
  if (!FixupVVDFile(tmpFileName, pStudioHdr, pVtxBuff, pVertexPools,
                    numVertexPools, pVertexList, numVertexes)) {
    // data error
    return false;
  }
  ...
  for (i = 0; i < ARRAYSIZE(vtxPrefixes); i++) {
    ...
    if (!FixupVTXFile(tmpFileName, pStudioHdr, pVertexPools, numVertexPools,
                      pVertexList, numVertexes)) {
      // data error
      return false;
    }
  ...
  if (!FixupMDLFile(tmpFileName, pStudioHdr, pVtxBuff, pVertexPools,
                    numVertexPools, pVertexList, numVertexes)) {
    // data error
    return false;
  }
...
bool Clamp_VTX_LODS(const char *fileName, int rootLOD,
                    studiohdr_t *pStudioHdr) {
  int i, j, k, m, n;
  int nLodID;
  int size;
  ...
  OptimizedModel::FileHeader_t *pNewVtxHdr =
      (OptimizedModel::FileHeader_t *)calloc(FILEBUFFER, 1);
  ...
  pData += pVtxHdr->numLODs *
           sizeof(OptimizedModel::MaterialReplacementListHeader_t);
  // ALIGN4( pData );
  ...
  for (nLodID = rootLOD; nLodID < pVtxHdr->numLODs; nLodID++) {
    OptimizedModel::MaterialReplacementListHeader_t *pReplacementList =
        pVtxHdr->pMaterialReplacementList(nLodID);
    ...
    pNewReplacementList->replacementOffset =
        (pData - (byte *)pNewReplacementList);
    pData += pNewReplacementList->numReplacements *
             sizeof(OptimizedModel::MaterialReplacementHeader_t);
    // ALIGN4( pData );

    for (i = 0; i < pReplacementList->numReplacements; i++) {
      OptimizedModel::MaterialReplacementHeader_t *pReplacement =
          pReplacementList->pMaterialReplacement(i);
      OptimizedModel::MaterialReplacementHeader_t *pNewReplacement =
          pNewReplacementList->pMaterialReplacement(i);

      pNewReplacement->materialID = pReplacement->materialID;
      AddToStringTable(pNewReplacement,
                       &pNewReplacement->replacementMaterialNameOffset,
  ...
  for (nLodID = 0; nLodID < rootLOD; nLodID++) {
    ...
    pNewReplacementList->numReplacements =
        pRootReplacementList->numReplacements;
    pNewReplacementList->replacementOffset =
        pRootReplacementList->replacementOffset + delta;
  }
  ...
  for (i = 0; i < pStudioHdr->numbodyparts; i++) {
    ...
    OptimizedModel::BodyPartHeader_t *pNewVtxBodyPart =
        pNewVtxHdr->pBodyPart(i);
    ...
    for (j = 0; j < pBodyPart->nummodels; ++j) {
      ...
      for (nLodID = rootLOD; nLodID < pVtxModel->numLODs; nLodID++) {
        ...
        OptimizedModel::ModelLODHeader_t *pNewVtxLOD =
            pNewVtxModel->pLOD(nLodID);
        ...
        for (k = 0; k < pModel->nummeshes; ++k) {
          ...
          pData += pNewVtxMesh->numStripGroups *
                   sizeof(OptimizedModel::StripGroupHeader_t);

          // printf("part %d : model %d : lod %d : mesh %d : strips %d : offset
          // %d\n", i, j, nLodID, k, pVtxMesh->numStripGroups,
          // pVtxMesh->stripGroupHeaderOffset );

          for (m = 0; m < pVtxMesh->numStripGroups; m++) {
            OptimizedModel::StripGroupHeader_t *pStripGroup =
                pVtxMesh->pStripGroup(m);
            OptimizedModel::StripGroupHeader_t *pNewStripGroup =
                pNewVtxMesh->pStripGroup(m);

            // int delta = ((byte *)pStripGroup - (byte *)pVtxHdr) - ((byte
            // *)pNewStripGroup - (byte *)pNewVtxHdr);
    ...
    for (j = 0; j < pBodyPart->nummodels; ++j) {
      ...
      for (nLodID = 0; nLodID < rootLOD; nLodID++) {
        OptimizedModel::ModelLODHeader_t *pVtxLOD =
            pVtxHdr->pBodyPart(i)->pModel(j)->pLOD(nLodID);
        OptimizedModel::ModelLODHeader_t *pRootVtxLOD =
            pNewVtxHdr->pBodyPart(i)->pModel(j)->pLOD(rootLOD);
        OptimizedModel::ModelLODHeader_t *pNewVtxLOD =
            pNewVtxHdr->pBodyPart(i)->pModel(j)->pLOD(nLodID);
...
//----------------------------------------------------------------------
// For a particular .qc, converts all studiomdl generated files to big-endian
// format.
//----------------------------------------------------------------------
void WriteSwappedFile(char *srcname, char *outname,
                      int (*pfnSwapFunc)(void *, const void *, int)) {
