  ...
  for (int i = 0; i < pSource->numbones; ++i) {
    if (pSource->localBone[i].parent >= 0) {
      fprintf(pFile, "# Bone %3d: %s Parent %3d: %s\n", i,
              pSource->localBone[i].name, pSource->localBone[i].parent,
              pSource->localBone[pSource->localBone[i].parent].name);
    } else {
...
//
//-----------------------------------------------------------------------------
bool CCheckUVCmd::CheckUVs(const s_source_t *const *pSourceList,
                           int nSourceCount) const {
...
bool CCheckUVCmd::CheckNormalized(const struct s_source_t *pSource) const {
  ...
  for (int i = 0; i < pSource->numvertices; ++i) {
    ...
    if (v.texcoord.x < 0.0f || v.texcoord.x > 1.0f || v.texcoord.y < 0.0f ||
        v.texcoord.y > 1.0f) {
  ...
  for (int i = 0; i < pSource->nummeshes; ++i) {
    ...
    for (auto vIt = badMeshVertexIndices.FirstInorder();
         badMeshVertexIndices.IsValidIndex(vIt);
...
bool CCheckUVCmd::CheckOverlap(const struct s_source_t *pSource) const {
  ...
  for (int i = 0; i < pSource->numfaces; ++i) {
    ...
    for (int j = i + 1; j < pSource->numfaces; ++j) {
      ...
      const Vector2D tB[] = {pSource->vertex[fB.a].texcoord,
...
//-----------------------------------------------------------------------------
// Check that the distance between edges in UV islands is a minimum number of
// pixels for a given texture size
//-----------------------------------------------------------------------------
...
//
//-----------------------------------------------------------------------------
Vector CCheckUVCmd::Barycentric(const Vector2D &vP, const Vector2D &vA,
                                const Vector2D &vB, const Vector2D &vC) {
...
//
//-----------------------------------------------------------------------------
Vector CCheckUVCmd::Barycentric(const Vector &vP, const Vector &vA,
                                const Vector &vB, const Vector &vC) {
...
//
//-----------------------------------------------------------------------------
int CCheckUVCmd::FindMeshIndex(const struct s_source_t *pSource,
                               int nFaceIndex) {
...
//
//-----------------------------------------------------------------------------
void CCheckUVCmd::PrintVertex(const s_vertexinfo_t &v,
                              const char *pszPrefix /* = "       " */) {
  Msg("%sP: %8.4f %8.4f %8.4f T: %8.4f %8.4f\n", pszPrefix, v.position.x,
      v.position.y, v.position.z, v.texcoord.x, v.texcoord.y);
}

//-----------------------------------------------------------------------------
//
//-----------------------------------------------------------------------------
void CCheckUVCmd::PrintVertex(const s_vertexinfo_t &v, const s_texture_t &t,
                              const char *pszPrefix /* = "       " */) {
  Msg("%sP: %8.4f %8.4f %8.4f T: %8.4f %8.4f M: %s\n", pszPrefix, v.position.x,
      v.position.y, v.position.z, v.texcoord.x, v.texcoord.y, t.name);
}

//-----------------------------------------------------------------------------
//
//-----------------------------------------------------------------------------
void CCheckUVCmd::PrintFace(const s_source_t *pSource, const int nMesh,
                            const int nFace,
                            const char *pszPrefix /* = "       " */) {
