...
uint64 g_nStaticCombosPerWorkUnit = 0, g_numCompletedStaticCombos = 0,
       g_numCommandsCompleted = 0;
uint64 g_numSkippedStaticCombos = 0;

CfgProcessor::CfgEntryInfo const *
GetEntryByStaticComboNum(uint64 nStaticCombo, uint64 *pnStaticCombo) {
...
struct CStaticCombo // all the data for one static combo
{
  ...
  void AddDynamicCombo(uint64 nComboID, void const *pComboData,
                       size_t nCodeSize) {
...
template <class MT_MUTEX_TYPE = CThreadFastMutex> class CSwitchableMutex {
public:
public:
...
template <typename T> char *ConsumeCharacters(char *szString, T pred) {
  if (szString) {
...
static void FlushCombos(size_t *pnTotalFlushedSize,
  ...
  uint8 *pCompressedShader = LZMA_OpportunisticCompress(
      reinterpret_cast<uint8 *>(pDynamicComboBuffer->Base()),
...
static void OutputDynamicCombo(size_t *pnTotalFlushedSize,
                               ...
                               MessageBuffer *pBuf, uint64 nComboID,
...
static void WriteShaderFiles(const char *pShaderName) {
  ...
  DebugOut("%s : %I64u combos centroid mask: 0x%x numDynamicCombos: %I64u "
           "flags: 0x%x\n",
...
size_t AssembleWorkerReplyPackage(CfgProcessor::CfgEntryInfo const *pEntry,
                                  uint64 nComboOfEntry, MessageBuffer *pBuf) {
  GLOBAL_DATA_MTX_LOCK();
...
public:
  explicit CWorkerAccumState(TMutexType *pMutex)
      : m_pMutex(pMutex), m_iFirstCommand(0), m_iNextCommand(0),
        m_iEndCommand(0), m_iLastFinished(0), m_hCombo(NULL),
...
template <typename TMutexType>
void CWorkerAccumState<TMutexType>::RangeBegin(uint64 iFirstCommand,
                                               uint64 iEndCommand) {
...
void CWorkerAccumState<TMutexType>::PrepareSubProcess(
  ...
  if (pSp) {
    ...
    sprintf(chBaseNameBuffer, "SHCMPL_SUB_%08X_%08llX_%08X", pSp->dwSvcThreadId,
            (long long)time(NULL), GetCurrentProcessId());
    pCommObjs = pSp->pCommObjs =
...
template <typename TMutexType> bool CWorkerAccumState<TMutexType>::OnProcess() {
  m_pMutex->Lock();
...
void Worker_ProcessCommandRange_Singleton::ProcessCommandRange(
    uint64 shaderStart, uint64 shaderEnd) {
  if (m_MT.pThreadPool) {
...
void Worker_GetLocalCopyOfBinaries(void) {
  Worker_GetLocalCopyOfBinary("mysql_wrapper.dll"); // This is necessary so VMPI
                                                    // doesn't run in SDK mode.
