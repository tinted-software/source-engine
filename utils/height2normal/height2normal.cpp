...
static void LoadConfigFile(const char *pFileName, float *bumpScale,
...
static void Usage() {
  fprintf(stderr, "Usage: height2normal [-nopause] [-quiet] tex1_normal.txt "
                  "tex2_normal.txt . . .\n");
  fprintf(stderr,
...
void ProcessFiles(const char *pNormalFileNameWithoutExtension, int startFrame,
  ...
  for (frameID = 0; frameID < numFrames; frameID++) {
    if (animated) {
      sprintf(normalTGAFileName, "%s%03d.tga", pNormalFileNameWithoutExtension,
              frameID + startFrame);
    } else {
    ...
    *pcUnderscore = NULL;
    ...
    if (animated) {
      sprintf(heightTGAFileName, "%s_height%03d.tga", buf,
              frameID + startFrame);
    } else {
...
int main(int argc, char **argv) {
  ...
  while (i < argc) {
    if (stricmp(argv[i], "-quiet") == 0) {
      ...
      g_Quiet = true;
      g_NoPause =
          true; // no point in pausing if we aren't going to print anything out.
    }
  ...
  for (; i < argc; i++) {
    ...
    if (!Q_IsAbsolutePath(argv[i])) {
      Q_snprintf(normalFileNameWithoutExtension,
                 sizeof(normalFileNameWithoutExtension), "%s\\%s",
    ...
    if ((startFrame == -1 && endFrame != -1) ||
      ...
      fprintf(stderr, "ERROR: If you use startframe, you must use endframe, "
                      "and vice versa.\n");
      Pause();
      continue;
    }
