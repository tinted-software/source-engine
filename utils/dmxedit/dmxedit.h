...
public:
  ...
  public:
    ...
    CDmeMesh::Falloff_t StringToFalloff(const char *pFalloffTypeString) {
      ...
      Msg("// ERROR: Can't Figure Out Which Falloff Type Is Meant By \"%s\", "
          "Assuming STRAIGHT\n",
          pFalloffTypeString);
  ...
  class CSelectOp {
  public:
    enum SelectOp_t { kAdd, kSubtract, kToggle, kIntersect, kReplace };
    ...
    SelectOp_t StringToSelectOp(const char *pSelectOpString) {
      ...
      Msg("// ERROR: Can't Figure Out Which Select Operation Type Is Meant By "
          "\"%s\", Assuming REPLACE\n",
          pSelectOpString);
  ...
  class CSelectType {
  public:
    enum Select_t { kNone, kAll, kDelta };
  ...
  class CObjType {
  public:
    enum Obj_t { kAbsolute, kRelative };
  ...
  public:
    ...
    CDmeMesh::Distance_t
    DistanceTypeStringToType(const char *pDistanceTypeString) {
      if (pDistanceTypeString &&
          (*pDistanceTypeString == 'a' || *pDistanceTypeString == 'A'))
        return CDmeMesh::DIST_ABSOLUTE;

      if (pDistanceTypeString &&
          (*pDistanceTypeString == 'r' || *pDistanceTypeString == 'R'))
  ...
  class CAxisType {
  public:
    enum Axis_t { kXAxis, kYAxis, kZAxis };
  ...
  bool ImportComboRules(const char *pFilename, bool bOverwrite = true,
                        bool bPurgeDeltas = true);

  bool ResetState();

  bool SetState(const char *pDeltaName);

  bool Select(const char *selectString,
              CDmeSingleIndexedComponent *pPassedSelection = NULL,
              CDmeMesh *pPassedMesh = NULL);

  bool Select(const CSelectType &selectType,
              CDmeSingleIndexedComponent *pPassedSelection = NULL,
              CDmeMesh *pPassedMesh = NULL);
  ...
  bool SelectHalf(const CHalfType &halfType,
                  CDmeSingleIndexedComponent *pPassedSelection = NULL,
                  CDmeMesh *pPassedMesh = NULL);
  ...
  bool Add(AddType addType, const CDmxEditProxy &e, float weight = 1.0f,
           float featherDistance = 0.0f,
           const CFalloffType &falloffType = STRAIGHT,
           const CDistanceType &distanceType = DIST_DEFAULT);

  bool Add(const CDmxEditProxy &e, float weight = 1.0f,
           float featherDistance = 0.0f,
           const CFalloffType &falloffType = STRAIGHT,
           const CDistanceType &distanceType = DIST_DEFAULT) {
  ...
  bool Add(AddType addType, const char *pDeltaName, float weight = 1.0f,
           float featherDistance = 0.0f,
           const CFalloffType &falloffType = STRAIGHT,
           const CDistanceType &distanceType = DIST_DEFAULT);

  bool Add(const char *pDeltaName, float weight = 1.0f,
           float featherDistance = 0.0f,
           const CFalloffType &falloffType = STRAIGHT,
           const CDistanceType &distanceType = DIST_DEFAULT) {
    return Add(kRaw, pDeltaName, weight, featherDistance, falloffType,
               distanceType);
  }

  bool Interp(const CDmxEditProxy &e, float weight = 1.0f,
              float featherDistance = 0.0f,
              const CFalloffType &falloffType = STRAIGHT,
              const CDistanceType &distanceType = DIST_DEFAULT);

  bool Interp(const char *pDeltaName, float weight = 1.0f,
              float featherDistance = 0.0f,
              const CFalloffType &falloffType = STRAIGHT,
              const CDistanceType &distanceType = DIST_DEFAULT);
  ...
  bool Save(const char *pFilename, const CObjType &saveType = ABSOLUTE,
  ...
  bool RemapMaterial(const char *pOldMaterialName,
  ...
  bool CreateDeltasFromPresets(
      const char *pPresetFilename, bool bDeleteNonPresetDeltas = true,
      const CUtlVector<CUtlString> *pPurgeAllButThese = NULL,
      const char *pExpressionFilename = NULL);

  bool CachePreset(const char *pPresetFilename,
                   const char *pExpressionFilename = NULL);

  bool ClearPresetCache();

  bool CreateDeltasFromCachedPresets(
      bool bDeleteNonPresetDeltas = true,
      const CUtlVector<CUtlString> *pPurgeAllButThese = NULL) const;

  bool CreateExpressionFileFromPresets(const char *pPresetFilename,
  ...
  bool Translate(Vector t, float featherDistance = 0.0f,
                 const CFalloffType &falloffType = STRAIGHT,
                 const CDistanceType &distanceType = DIST_DEFAULT,
                 CDmeMesh *pPassedMesh = NULL,
  ...
  bool Translate(Vector t, float featherDistance,
                 const CFalloffType &falloffType = STRAIGHT) {
    return Translate(t, featherDistance, falloffType, m_distanceType);
  }

  bool Rotate(Vector r, Vector o, float featherDistance = 0.0f,
              const CFalloffType &falloffType = STRAIGHT,
              const CDistanceType &passedDistanceType = DIST_DEFAULT,
              CDmeMesh *pPassedMesh = NULL, CDmeVertexData *pPassedBase = NULL,
              CDmeSingleIndexedComponent *pPassedSelection = NULL);

  bool FixPresetFile(const char *pPresetFilename);

  bool GroupControls(const char *pGroupName,
                     CUtlVector<const char *> &rawControlNames);

  bool ReorderControls(CUtlVector<CUtlString> &controlNames);

  bool AddDominationRule(CUtlVector<CUtlString> &dominators,
  ...
  bool SetWrinkleScale(const char *pControlName, const char *pRawControlName,
                       float flScale);
  ...
  int LuaError(lua_State *pLuaState) {
    if (GetLineNumber() >= 0) {
      Error("// ERROR: %s:%d: %s - %s\n", GetSourceFile().Get(),
            GetLineNumber(), GetFuncString().Get(), GetErrorString().Get());
    } else {
      Error("// ERROR: %s - %s\n", GetFuncString().Get(),
            GetErrorString().Get());
    }
...
#error "define vsnprintf type."
    ...
    if (GetLineNumber() >= 0) {
      Error("// ERROR: %s:%d: %s - %s\n", GetSourceFile().Get(),
            GetLineNumber(), GetFuncString().Get(), tmpBuf);
    } else {
...
protected:
  ...
  bool Select(CDmeVertexDeltaData *pDelta,
              CDmeSingleIndexedComponent *pPassedSelection = NULL,
              CDmeMesh *pPassedMesh = NULL);
  ...
  void ImportCombinationControls(CDmeCombinationOperator *pSrcComboOp,
                                 CDmeCombinationOperator *pDstComboOp,
                                 bool bOverwrite);

  void ImportDominationRules(CDmeCombinationOperator *pDestComboOp,
  ...
  GetSelection(CDmeSingleIndexedComponent *pPassedSelection) {
    return pPassedSelection ? pPassedSelection : m_pCurrentSelection;
  }

  CDmeVertexDeltaData *
  FindDeltaState(const char *pDeltaName,
                 const CDmeMesh *pPassedMesh = NULL) const {
...
//=============================================================================
struct LuaFunc_s {
  LuaFunc_s(const char *pName, LuaFunc_t pFunc, const char *pProto,
            const char *pDoc)
      : m_pFuncName(pName), m_pFunc(pFunc), m_pFuncPrototype(pProto),
        m_pFuncDesc(pDoc) {
