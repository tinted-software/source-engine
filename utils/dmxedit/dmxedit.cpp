...
//-----------------------------------------------------------------------------
CDmxEdit::CDmxEdit()
    : m_pRoot(NULL), m_pMesh(NULL), m_pCurrentSelection(NULL),
...
//
//-----------------------------------------------------------------------------
bool CDmxEdit::Load(const char *pFilename,
                    const CObjType &loadType /* = DIST_ABSOLUTE */) {
  Unload();
  ...
  if (sLen > 4 && !Q_stricmp(pFilename + sLen - 4, ".dmx")) {
    ...
    if (pDag) {
      ...
      if (!m_pMesh) {
        if (pFirstMesh) {
          m_pMesh = pFirstMesh;
          LuaWarning("Cannot Find A DmeMesh With Any Delta States In File, "
                     "Using First Found Mesh %s",
                     m_pMesh->GetName());
        } else {
      ...
      LuaWarning("Cannot Find A DmeModel As Element Dme Root Object: %s in %s",
    ...
    m_pRoot = CDmObjSerializer().ReadOBJ(pFilename, &m_pMesh, true,
                                         loadType() == CObjType::kAbsolute);
  ...
  if (m_pMesh) {
    ...
    m_pCurrentSelection = CreateElement<CDmeSingleIndexedComponent>(
        "selection", m_pRoot->GetFileId());
...
LUA_COMMAND(Load, "< $file.dmx | $file.obj >, [ $loadType ]",
            "Replaces the current scene with the specified scene.  The current "
            "mesh will be set to the first mesh with a combination operator "
            "found in the new scene.  $loadType is one of \"absolute\" or "
            "\"relative\".  If not specified, \"absolute\" is assumed.") {
...
bool CDmxEdit::Import(const char *pFilename, const char *pParentName) {
  ...
  if (!pSrcModel) {
    ...
    return SetErrorString(
        "Can't Find \"model\" Element On Root Node In DMX File");
  }
  ...
  //	CDmeCombinationOperator *pCombo = m_pRoot->GetValueElement<
  //CDmeCombinationOperator >( "combinationOperator" );

  // Initialize traversal stack to just model's children (don't want to touch
  // model)
  CUtlStack<CDmeDag *> traverseStack;
...
LUA_COMMAND(Import, "< $file.dmx >, [ $parentBone ]",
            "Imports the specified DMX model into the scene.  The imported "
            "model can optionally be parented (which implictly skins it) to a "
            "specified bone in the existing scene via $parentBone.") {
  LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);

  const char *pFilename = luaL_checkstring(pLuaState, 1);
  const char *pParentBone =
      lua_isstring(pLuaState, 2) ? lua_tostring(pLuaState, 2) : NULL;
...
LUA_COMMAND(DeltaName, "< #deltaIndex >",
            "Returns the name of the delta at the specified index.  Use "
            "DeltaCount() to get a count of how many deltas are present") {
  LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
//
//-----------------------------------------------------------------------------
bool CDmxEdit::ImportComboRules(const char *pFilename,
                                bool bOverwrite /* = true */,
                                bool bPurgeDeltas /* = false */) {
  if (!m_pMesh)
    return SetErrorString("No Mesh");

  CDmeCombinationOperator *pDestCombo(
      FindReferringElement<CDmeCombinationOperator>(m_pMesh, "targets"));
  if (!pDestCombo)
    return SetErrorString("No DmeCombinationOperator On Mesh \"%s\"",
                          m_pMesh->GetName());
...
LUA_COMMAND(
    ...
    "Imports the combintion rules from the specified dmx file and replaces the "
    "current rules with those from the file is #bOverwrite specified or is "
    "true.  If #bOverwrite is false then any existing controls that are not in "
    "the imported rules file are preserved.  If #bPurgeDeltas is specified and "
    "is true, then any delta states which are no longer referred to by any "
    "combination rule or control will be purged. ") {
  LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
  ...
  if (lua_isboolean(pLuaState, 2)) {
    if (lua_isboolean(pLuaState, 3)) {
      if (LuaFunc_s::m_dmxEdit.ImportComboRules(
              pFilename, lua_toboolean(pLuaState, 2) != 0,
...
//-----------------------------------------------------------------------------
void CDmxEdit::ImportDominationRules(CDmeCombinationOperator *pDestComboOp,
  ...
  for (int i = 0; i < nRuleCount; ++i) {
    ...
    // Only add dominance rule if *all* raw controls are present and the rule
...
bool IsDefaultControl(CDmeCombinationOperator *pCombo, int nIndex) {
  if (pCombo->GetRawControlCount(nIndex) == 1 &&
      !Q_strcmp(pCombo->GetControlName(nIndex),
                pCombo->GetRawControlName(nIndex, 0)))
    return true;
...
//-----------------------------------------------------------------------------
bool IsASubsetOfB(CDmeCombinationOperator *pComboA, int nIndexA,
                  CDmeCombinationOperator *pComboB, int nIndexB) {
...
//-----------------------------------------------------------------------------
bool AreControlsTheSame(CDmeCombinationOperator *pComboA, int nIndexA,
                        CDmeCombinationOperator *pComboB, int nIndexB) {
  const int nCountA = pComboA->GetRawControlCount(nIndexA);
  const int nCountB = pComboB->GetRawControlCount(nIndexB);
  ...
  // This is a test for equality now since we've verified that A & B has same
  // number of elements
...
// -1 for failure
//-----------------------------------------------------------------------------
int FindControlIndexFromRawControlName(CDmeCombinationOperator *pCombo,
...
//-----------------------------------------------------------------------------
void CDmxEdit::ImportCombinationControls(CDmeCombinationOperator *pSrcComboOp,
  ...
  if (bOverwrite) {
    ...
    // Remove conflicting controls, i.e. controls in src which are controlling
    // something that's already controlled in dst
    ...
    while (bConflict) {
      ...
      for (int i = 0; !bConflict && i < pSrcComboOp->GetControlCount(); ++i) {
        ...
        for (int j = 0; !bConflict && j < nSrcRawCount; ++j) {
          ...
          const int nDstControlIndex = FindControlIndexFromRawControlName(
              pDstComboOp, pSrcRawControlName);
  ...
  for (int i = 0; i < pSrcComboOp->GetControlCount(); ++i) {
    ...
    ControlIndex_t index =
        pDstComboOp->FindOrCreateControl(pControlName, bIsStereo);
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(ResetState, "",
            "Resets the current mesh back to the default base state, i.e. no "
            "deltas active") {
  LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
//-----------------------------------------------------------------------------
bool CDmxEdit::Select(const char *pSelectTypeString,
  ...
  if (selectType() == CSelectType::kDelta) {
    CDmeVertexDeltaData *pDelta =
        FindDeltaState(pSelectTypeString, pPassedMesh);
    if (pDelta)
...
LUA_COMMAND(Select, "< $delta >, [ $type ]",
            "Changes the selection based on the vertices present in the "
            "specified $delta.  $type is one of \"add\", \"subtract\", "
            "\"toggle\", \"intersect\", \"replace\".  If $type is not "
            "specified, \"add\" is assumed.") {
  LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
//-----------------------------------------------------------------------------
bool CDmxEdit::Select(CDmeVertexDeltaData *pDelta,
                      CDmeSingleIndexedComponent *pPassedSelection /* = NULL */,
                      CDmeMesh *pPassedMesh /* = NULL */) {
  if (!pDelta)
  ...
  CDmeSingleIndexedComponent *pSelection =
      pPassedSelection ? pPassedSelection : m_pCurrentSelection;
...
// Selects vertices of the working mesh that change position in the give delta
//-----------------------------------------------------------------------------
bool CDmxEdit::Select(const CSelectType &selectType,
                      CDmeSingleIndexedComponent *pPassedSelection /* = NULL */,
                      CDmeMesh *pPassedMesh /* = NULL */) {
  if (selectType() == CSelectType::kDelta)
...
bool CDmxEdit::Select(const CSelectOp &selectOp, CDmeVertexDeltaData *pDelta,
  ...
  CDmeSingleIndexedComponent *pTempSelection =
      CreateElement<CDmeSingleIndexedComponent>("tempSelection",
...
//-----------------------------------------------------------------------------
bool CDmxEdit::Select(const CSelectOp &selectOp,
  ...
  case CSelectOp::kToggle: {
    CDmeSingleIndexedComponent *pIntersection =
        CreateElement<CDmeSingleIndexedComponent>("intersection",
                                                  pOriginal->GetFileId());
    if (!pIntersection)
      return false;

    CDmeSingleIndexedComponent *pNewCopy =
        CreateElement<CDmeSingleIndexedComponent>("newCopy",
                                                  pOriginal->GetFileId());
    if (!pNewCopy)
      return false;
...
//
//-----------------------------------------------------------------------------
bool CDmxEdit::SelectHalf(
    const CSelectOp &selectOp, const CHalfType &halfType,
    CDmeSingleIndexedComponent *pPassedSelection /* = NULL */,
    CDmeMesh *pPassedMesh /* = NULL */) {
  CDmeSingleIndexedComponent *pSelection =
      pPassedSelection ? pPassedSelection : m_pCurrentSelection;
...
bool CDmxEdit::SelectHalf(
  ...
  pMesh->SelectHalfVertices(halfType() == CHalfType::kLeft ? CDmeMesh::kLeft
                                                           : CDmeMesh::kRight,
                            pSelection);
...
LUA_COMMAND(SelectHalf, "< $halfType >",
            "Selects all the vertices in half the mesh.  Half is one of left "
            "or right.  Left means the X value of the position is >= 0, right "
            "means that the X value of the position is <= 0.  So it's the "
            "character's left (assuming they're staring down -z)") {
  LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(GrowSelection, "< #size >",
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(ShrinkSelection, "< #size >",
...
//
//-----------------------------------------------------------------------------
bool CDmxEdit::Add(AddType addType, const CDmxEditProxy &e,
                   float weight /* = 1.0f */,
  ...
  const CDistanceType &distanceType =
      passedDistanceType() == CDmeMesh::DIST_DEFAULT ? m_distanceType
                                                     : passedDistanceType;

  CDmeSingleIndexedComponent *pNewSelection =
      featherDistance > 0.0f
          ? m_pMesh->FeatherSelection(featherDistance, falloffType(),
                                      distanceType(), m_pCurrentSelection, NULL)
          : NULL;

  const bool retVal =
      addType == kRaw
          ? m_pMesh->AddMaskedDelta(NULL, NULL, weight,
                                    pNewSelection ? pNewSelection
                                                  : m_pCurrentSelection)
          : m_pMesh->AddCorrectedMaskedDelta(
                NULL, NULL, weight,
                pNewSelection ? pNewSelection : m_pCurrentSelection);
...
//
//-----------------------------------------------------------------------------
bool CDmxEdit::Add(AddType addType, const char *pDeltaName,
                   float weight /* = 1.0f */,
                   float featherDistance /* = 0.0f */,
                   const CFalloffType &falloffType,
                   const CDistanceType &passedDistanceType) {
  if (!Q_stricmp("BASE", pDeltaName))
    return Add(addType, CDmxEditProxy(*this), weight, featherDistance,
               falloffType);
  ...
  const bool retVal =
      addType == kRaw
          ...
          : m_pMesh->AddCorrectedMaskedDelta(pDelta, NULL, weight,
                                             pNewSelection);
...
LUA_COMMAND(
    ...
    "Adds specified state to the current state of the mesh weighted and "
    "feathered by the specified #weight, #featherDistance & $falloffType.  "
    "$falloffType is one of \"straight\", \"spike\", \"dome\", \"bell\".  Note "
    "that only the specified delta is added.  "
    "i.e. If Add( \"A_B\" ); is called then just A_B is added, A & B are not.  "
    "See AddCorrected();") {
  LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
  ...
  float featherDistance =
      lua_isnumber(pLuaState, 3) ? lua_tonumber(pLuaState, 3) : 0.0f;
  const char *pFalloffType =
      lua_isstring(pLuaState, 4) ? lua_tostring(pLuaState, 4) : "straight";

  if (LuaFunc_s::m_dmxEdit.Add(CDmxEdit::kRaw, pString1, weight,
                               featherDistance, pFalloffType))
    return LuaFunc_s::m_dmxEdit.LuaOk(pLuaState);
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(AddCorrected,
            "< $delta >, [ #weight ], [ #featherDistance ], < $falloffType >",
            "Same as Add() except that the corrected delta is added. i.e. If "
            "AddCorrected( \"A_B\" ); is called "
            "then A, B & A_B are all added.  This works similarly to "
            "SetState() whereas Add() just adds "
            "the named delta.") {
  LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
  ...
  if (LuaFunc_s::m_dmxEdit.Add(CDmxEdit::kCorrected, pString1, weight,
                               featherDistance, pFalloffType))
    return LuaFunc_s::m_dmxEdit.LuaOk(pLuaState);
...
//
//-----------------------------------------------------------------------------
bool CDmxEdit::Interp(const CDmxEditProxy &e, float weight /* = 1.0f */,
                      float featherDistance /* = 0.0f */,
                      const CFalloffType &falloffType,
                      const CDistanceType &passedDistanceType) {
  if (!m_pMesh)
  ...
  const bool retVal = m_pMesh->InterpMaskedDelta(
      NULL, NULL, weight, pNewSelection ? pNewSelection : m_pCurrentSelection);
...
//
//-----------------------------------------------------------------------------
bool CDmxEdit::Interp(const char *pDeltaName, float weight /* = 1.0f */,
                      float featherDistance /* = 0.0f */,
                      const CFalloffType &falloffType,
                      const CDistanceType &passedDistanceType) {
  if (!Q_stricmp("BASE", pDeltaName))
...
LUA_COMMAND(Interp,
            "< $delta >, [ #weight ], [ #featherDistance ], < $falloffType >",
            "Interpolates the current state of the mesh towards the specified "
            "state, $weight, #featherDistance and $falloffType.  $falloffType "
            "is one of \"straight\", \"spike\", \"dome\", \"bell\"") {
  LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
  ...
  if (LuaFunc_s::m_dmxEdit.Interp(pString1, weight, featherDistance,
                                  pFalloffType))
    return LuaFunc_s::m_dmxEdit.LuaOk(pLuaState);
...
//
//-----------------------------------------------------------------------------
bool CDmxEdit::Save(const char *pFilename,
                    const CObjType &saveType /* = ABSOLUTE */,
  ...
  if (sLen > 4 && !Q_stricmp(pFilename + sLen - 4, ".dmx")) {
    ...
    retVal = g_pDataModel->SaveToFile(pFilename, NULL, "keyvalues2", "model",
                                      m_pRoot);
    if (!retVal) {
    ...
    if (pDeltaName) {
      if (!Q_stricmp("base", pDeltaName)) {
        retVal = CDmObjSerializer().WriteOBJ(pFilename, m_pRoot, false, NULL,
                                             absolute);
      } else {
        retVal = CDmObjSerializer().WriteOBJ(pFilename, m_pRoot, true,
                                             pDeltaName, absolute);
      }
    } else {
      retVal =
          CDmObjSerializer().WriteOBJ(pFilename, m_pRoot, true, NULL, absolute);
    }
  }
...
void CDmxEdit::UpdateMakefile(CDmElement *pRoot) {
  ...
  CDmeDCCMakefile *pMakefile =
      CreateElement<CDmeDCCMakefile>("dmxedit", pRoot->GetFileId());
  if (!pMakefile)
    return;

  pRoot->SetValue("makefile", pMakefile);

  CDmrElementArray<CDmeSourceDCCFile> sources(
      pMakefile->GetAttribute("sources"));
  sources.AddToTail(
      CreateElement<CDmeSourceDCCFile>(m_filename, pRoot->GetFileId()));
}
...
void CDmxEdit::AddExportTags(CDmElement *pRoot, const char *pFilename) {
  ...
  CDmElement *pExportTags =
      CreateElement<CDmElement>("dmxedit_exportTags", pRoot->GetFileId());
...
//
//-----------------------------------------------------------------------------
void CDmxEdit::RemoveExportTags(CDmElement *pRoot,
                                const char *pExportTagsName) {
  if (!pRoot)
    return;
...
LUA_COMMAND(
    ...
    "Saves the current scene to either a single dmx file or a sequence of obj "
    "files.  If $file is not specified, the filename used in the last Load() "
    "command will be used to save the file.  $saveType is one of \"absolute\" "
    "or \"relative\".  If not specified, \"absolute\" is assumed.  If $delta "
    "is passed and the save type is OBJ, then only a single OBJ of that delta "
    "is saved.  \"base\" is the base state.") {
  LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
  ...
  if (pSave1) {
    if (pSaveType) {
      const char *pDeltaName =
          lua_isstring(pLuaState, 3) ? lua_tostring(pLuaState, 3) : NULL;
...
bool CDmxEdit::Merge(const char *pInFilename, const char *pOutFilename) {
  ...
  if (retVal) {
    ...
    if (!g_pDataModel->SaveToFile(pOutFilename, NULL, "keyvalues2", "model",
                                  pRoot)) {
      retVal = false;
...
LUA_COMMAND(Merge, "< $src.dmx >, < $dst.dmx >",
            "Merges the current mesh onto the specified dmx file and saves to "
            "the second specified dmx file") {
  LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
bool CDmxEdit::SaveDelta(const char *pDeltaName) {
  ...
  CDmeVertexDeltaData *pDelta =
      m_pMesh->ModifyOrCreateDeltaStateFromBaseState(pDeltaName);
  if (pDelta)
    return true;
...
//
//-----------------------------------------------------------------------------
bool CDmxEdit::RemapMaterial(const char *pOldMaterialName,
                             const char *pNewMaterialName) {
  return CDmMeshUtils::RemapMaterial(m_pMesh, pOldMaterialName,
                                     pNewMaterialName);
}
...
LUA_COMMAND(RemapMaterial, "< #index | $srcMaterial >, < $dstMaterial >",
            "Remaps the specified material to the new material, material can "
            "be specified by index or name") {
  LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
  ...
  if (lua_isnumber(pLuaState, 1)) {
    ...
    if (!LuaFunc_s::m_dmxEdit.RemapMaterial(nMaterialIndex, pString2)) {
      LuaFunc_s::m_dmxEdit.LuaWarning(
          "Invalid Material Index To Remap, Couldn't Find Material %d\n",
          nMaterialIndex);
    }
  } else {
    const char *pString1 = luaL_checkstring(pLuaState, 1);
    if (!LuaFunc_s::m_dmxEdit.RemapMaterial(pString1, pString2)) {
      LuaFunc_s::m_dmxEdit.LuaWarning(
          "Invalid Material To Remap, Couldn't Find Material \"%s\"\n",
          pString1);
    }
  }
...
LUA_COMMAND(RemoveFacesWithMoreThanNVerts, "< #vertexCount >",
            "Removes faces from the current mesh which have more than the "
...
LUA_COMMAND(Mirror, "[ $axis ]",
            "Mirrors the mesh in the specified axis.  $axis is one of \"x\", "
            "\"y\", \"z\". If $axis is not specified, \"x\" is assumed (i.e. "
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(ComputeNormals, "",
            "Computes new smooth normals for the current mesh and all of its "
            "delta states.") {
  LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
bool CDmxEdit::ComputeWrinkles(bool bOverwrite) {
  ...
  CDmeCombinationOperator *pCombo(
      FindReferringElement<CDmeCombinationOperator>(m_pMesh, "targets"));
  if (!pCombo)
    return SetErrorString("No Combination Operator On Mesh \"%s\"",
                          m_pMesh->GetName());
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(ComputeWrinkles, "[ #bOverwrite = false ]",
            "Uses the current combo rules ( i.e. The wrinkleScales, via "
            "SetWrinkleScale()) to compute wrinkle delta values.  "
            "If #bOverwrite is false, only wrinkle data that doesn't currently "
            "exist on the mesh will be computed.  "
            "If #bOverwrite is true then all wrinkle data will be computed "
            "from the combo rules wiping out any existing data computed using "
            "ComputeWrinkle().  ") {
  LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);

  const bool bOverwrite = lua_isboolean(pLuaState, 1)
...
bool CDmxEdit::ComputeWrinkle(const char *pDeltaName, float flScale,
  ...
  CDmMeshUtils::WrinkleOp wrinkleOp = StringHasPrefix(pOperation, "r")
                                          ? CDmMeshUtils::kReplace
                                          : CDmMeshUtils::kAdd;
  ...
  for (int nControlIndex = 0; nControlIndex < nControlCount; ++nControlIndex) {
    ...
    for (int nRawControlIndex = 0; nRawControlIndex < nRawControlCount;
         ++nRawControlIndex) {
      if (Q_strcmp(pDeltaName, pDmeCombo->GetRawControlName(nControlIndex,
                                                            nRawControlIndex)))
        continue;
  ...
  return CDmMeshUtils::CreateWrinkleDeltaFromBaseState(pDelta, flScale,
                                                       wrinkleOp, m_pMesh);
}
...
LUA_COMMAND(ComputeWrinkle, "< $delta > [ , #scale = 1 [ , $operation ] ] ",
            "Updates the wrinkle stretch/compress data for the specified delta "
            "based on the position deltas "
            "for the current state scaled by the scale value.  If $operation "
            "isn't specified then \"REPLACE\" "
            "is assumed. $operation is one of: \"REPLACE\", \"ADD\".  "
            "Note that Negative (-) values enable the compress map and "
            "Positive (+) values enable the stretch map. "
            "This also sets the wrinkle scale to 1") {
  LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);

  const char *pDeltaName = luaL_checkstring(pLuaState, 1);
  const float flScale =
      lua_isnumber(pLuaState, 2) ? lua_tonumber(pLuaState, 2) : 1.0f;
  const char *pOperation =
      lua_isstring(pLuaState, 3) ? lua_tostring(pLuaState, 3) : "REPLACE";
...
bool CDmxEdit::CreateDeltasFromPresets(
    const char *pPresetFilename, bool bPurge /* = true */,
  ...
  const bool bRetVal = CDmMeshUtils::CreateDeltasFromPresets(
      m_pMesh, NULL, m_presetCache, bPurge, pPurgeAllButThese);
...
LUA_COMMAND(CreateDeltasFromPresets,
            "< $preset.pre >, [ #bPurgeExisting = true ], [ { \"NoPurge1\", "
            "... } ], [ $expression.txt ]",
            "Creates a new delta state for every present defined in the "
            "specified preset file.  #bPurgeExisting specifies whether "
            "existing controls should be purged.  If #bPurgeExisting is true, "
            "then an optional array of deltas to not purge can be specified.") {
  LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);

  const char *pPresetFilename = luaL_checkstring(pLuaState, 1);
  const bool bPurge = lua_isboolean(pLuaState, 2)
                          ? lua_toboolean(pLuaState, 2) ? true : false
                          : true;
  const char *pExpressionFilename =
      lua_isstring(pLuaState, 4) ? lua_tostring(pLuaState, 4) : NULL;
  ...
  if (LuaFunc_s::m_dmxEdit.CreateDeltasFromPresets(
          pPresetFilename, bPurge, pPurgeAllBut, pExpressionFilename))
    return LuaFunc_s::m_dmxEdit.LuaOk(pLuaState);
...
//
//-----------------------------------------------------------------------------
bool CDmxEdit::CachePreset(const char *pPresetFilename,
                           const char *pExpressionFilename /* = NULL  */) {
...
LUA_COMMAND(
    ...
    "CreateDeltasFromCachedPresets().  If an $expression.txt file pathname is "
    "specified, an expression file will be written out for this preset file") {
  LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(ClearPresetCache, "",
            "Removes all preset filenames that have been specified via "
            "CachePreset() calls") {
  LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
bool CDmxEdit::CreateDeltasFromCachedPresets(
  ...
  return CDmMeshUtils::CreateDeltasFromPresets(m_pMesh, NULL, m_presetCache,
                                               bPurge, pPurgeAllButThese);
}

//-----------------------------------------------------------------------------
//
//-----------------------------------------------------------------------------
LUA_COMMAND(CreateDeltasFromCachedPresets,
            "[ #bPurgeExisting = true ], [ { \"NoPurge1\", ... } ]",
            "Creates a new delta state for every preset defined in the preset "
            "files specified by previous CachePreset() calls.  Calling this "
            "calls ClearPresetCache().  #bPurgeExisting specifies whether "
            "existing controls should be purged.  If #bPurgeExisting is true, "
            "then an optional array of deltas to not purge can be specified.") {
  LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);

  const bool bPurge = lua_isboolean(pLuaState, 1)
                          ? lua_toboolean(pLuaState, 1) ? true : false
                          : true;
  ...
  bool bRetVal =
      LuaFunc_s::m_dmxEdit.CreateDeltasFromCachedPresets(bPurge, pPurgeAllBut);
...
//
//-----------------------------------------------------------------------------
bool CDmxEdit::CreateExpressionFileFromPresets(
    const char *pPresetFilename, const char *pExpressionFilename) {
  ClearPresetCache();
  CachePreset(pPresetFilename, pExpressionFilename);
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(CreateExpressionFileFromPresets,
            "< $preset.pre >, < $expression.txt >",
            "Creates a faceposer expression.txt and expression.vfe file for "
            "the specified preset file") {
  LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);

  const char *pPresetFilename = luaL_checkstring(pLuaState, 1);
  const char *pExpressionFilename = luaL_checkstring(pLuaState, 2);

  if (LuaFunc_s::m_dmxEdit.CreateExpressionFileFromPresets(pPresetFilename,
                                                           pExpressionFilename))
    return LuaFunc_s::m_dmxEdit.LuaOk(pLuaState);
...
LUA_COMMAND(CreateExpressionFilesFromCachedPresets, "",
            "Creates a faceposer expression.txt and expression.vfe file for "
            "each of the preset/expression file pairs specified by previous "
            "CachePreset calls") {
  LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
//
//-----------------------------------------------------------------------------
void ScaleDeltaPositions(const CDmrArrayConst<Vector> &bindPosData,
                         CDmeVertexDeltaData *pDelta, float sx, float sy,
                         float sz) {
  ...
  Vector *pPosArray =
      reinterpret_cast<Vector *>(alloca(nPosDataCount * sizeof(Vector)));
...
bool CDmxEdit::Scale(float sx, float sy, float sz) {
...

  {
    CDmeVertexData *pBind = m_pMesh->GetBindBaseState();
    const int nPosIndex =
        pBind ? pBind->FindFieldIndex(CDmeVertexData::FIELD_POSITION) : -1;
...
LUA_COMMAND(
    ...
    "Scales the position values by the specified amount.  If only 1 scale "
    "value is supplied a uniform scale in all three dimensions is applied") {
  LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(SetDistanceType, "< $distanceType >",
            "Sets the way distances will be interpreted after the command.  "
            "$distanceType is one of \"absolute\" or \"relative\".  By default "
            "distances are \"absolute\".  All functions that work with "
            "distances (Add, Interp and Translate) work on the currently "
            "selected vertices.  \"absolute\" means use the distance as that "
            "number of units, \"relative\" means use the distance as a scale "
            "of the radius of the bounding sphere of the selected vertices.") {
  LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
//
//-----------------------------------------------------------------------------
bool CDmxEdit::Translate(Vector t, float featherDistance,
                         const CFalloffType &falloffType,
                         const CDistanceType &passedDistanceType,
                         CDmeMesh *pPassedMesh, CDmeVertexData *pPassedBase,
  ...
  CDmeVertexData *pBase =
      pPassedBase ? pPassedBase : pMesh->GetCurrentBaseState();
...
LUA_COMMAND(
    Translate,
    "< #xTranslate, #yTranslate, #zTranslate >, [ #falloffDistance, "
    "$falloffType ]",
  ...
  if (lua_gettop(pLuaState) < 4) {
    ...
    if (lua_gettop(pLuaState) == 4 && lua_isstring(pLuaState, 4)) {
      ...
      flFeatherDistance = lua_isnumber(pLuaState, 4)
                              ? lua_tonumber(pLuaState, 4)
                              : flFeatherDistance;
      pFalloffType = lua_isstring(pLuaState, 5) ? lua_tostring(pLuaState, 5)
                                                : pFalloffType;
      pDistanceType = lua_isstring(pLuaState, 6) ? lua_tostring(pLuaState, 6)
                                                 : pDistanceType;
    }

    if (LuaFunc_s::m_dmxEdit.Translate(Vector(tx, ty, tz), flFeatherDistance,
                                       pFalloffType, pDistanceType))
      return LuaFunc_s::m_dmxEdit.LuaOk(pLuaState);
  }
...
bool CDmxEdit::Rotate(
  ...
  if (pNewSelection) {
    ...
    for (int j = 0; j < nSelectionCount; ++j) {
      ...
      const Vector &s = posData.Get(nIndex);
      Vector &d = pPosArray[nIndex];
      AngleMatrix(RadianEuler(DEG2RAD(r.x * fWeight), DEG2RAD(r.y * fWeight),
                              DEG2RAD(r.z * fWeight)),
                  rMat);
...
//-----------------------------------------------------------------------------
LUA_COMMAND(Rotate,
            "< #xRotate, #yRotate, #zRotate >, [ #xOffset, #yOffset, #zOffset "
            "], [ #falloffDistance, $falloffType ]",
            "Rotates the current selection around the center of the selection "
            "by the specified Euler angles (in degrees).  The optional offset "
            "is added to the computed pivot point.") {
  LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
  ...
  if (lua_gettop(pLuaState) < 4) {
    if (LuaFunc_s::m_dmxEdit.Rotate(Vector(rx, ry, rz),
    ...
    if (lua_gettop(pLuaState) < 7) {
      ...
      flFeatherDistance = lua_isnumber(pLuaState, 7)
                              ? lua_tonumber(pLuaState, 7)
                              : flFeatherDistance;
      pFalloffType = lua_isstring(pLuaState, 8) ? lua_tostring(pLuaState, 8)
                                                : pFalloffType;
      pDistanceType = lua_isstring(pLuaState, 9) ? lua_tostring(pLuaState, 9)
                                                 : pDistanceType;

      if (LuaFunc_s::m_dmxEdit.Rotate(Vector(rx, ry, rz), Vector(ox, oy, oz),
                                      flFeatherDistance, pFalloffType,
                                      pDistanceType))
        return LuaFunc_s::m_dmxEdit.LuaOk(pLuaState);
    }
  }
...
LUA_COMMAND(
  ...
  return LuaFunc_s::m_dmxEdit.LuaError(pLuaState, "File \"%s\" Doesn't Exist",
                                       pFilename);
}
...
// found one
//-----------------------------------------------------------------------------
bool HasDuplicateControlName(CDmeCombinationOperator *pDmeCombo,
...
//
//-----------------------------------------------------------------------------
bool CDmxEdit::GroupControls(const char *pGroupName,
  ...
  // Loop through controls to see if any are already group controls, warn and
  // remove
  ...
  for (int i = 0; i < rawControlNames.Count(); ++i) {
    ControlIndex_t nControlIndex =
        pDmeCombo->FindControlIndex(rawControlNames[i]);
    if (nControlIndex < 0) {
  ...
  // Create new control
  ControlIndex_t nNewControl =
      pDmeCombo->FindOrCreateControl(pGroupName, bStereo);
...
LUA_COMMAND(
    ...
    "Groups the specified raw controls under a common group control.  If the "
    "group control already exists, the raw controls are added to it") {
  LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
bool CDmxEdit::ReorderControls(CUtlVector<CUtlString> &controlNames) {
  ...
  for (int i = 0; i < controlNames.Count(); ++i) {
    ...
    if (nControlIndex < 0) {
      LuaWarning("Control \"%s\" doesn't exist, ignoring",
                 controlNames[i].Get());
      continue;
    }
  ...
  for (int i = 0; i < validControlNames.Count(); ++i) {
    pDmeCombo->MoveControlBefore(validControlNames[i],
                                 pDmeCombo->GetControlName(i));
  }
...
LUA_COMMAND(ReorderControls, "< $controlName [ , ... ] >",
            ...
            "The specified controls will be moved to the front of the list of "
            "controls.  "
            "Not all of the controls need to be specified.  "
            "Unspecified controls will be left in the order they already are "
            "after the specified controls ") {
  LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
//
//-----------------------------------------------------------------------------
bool CDmxEdit::AddDominationRule(CUtlVector<CUtlString> &dominators,
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(AddDominationRule,
            "{ < $dominator > [ , ... ] }, { < $supressed > [ , ... ] }",
            "Create a domination rule.  A lua array of strings is passed as "
            "the 1st argument which are "
            "the controls which are the dominators and a lua array of strings "
            "is passed as the 2nd "
  ...
  if (dominators.Count() <= 0)
    return LuaFunc_s::m_dmxEdit.LuaError(pLuaState,
                                         "No Dominator Controls Specified");
  ...
  if (supressed.Count() <= 0)
    return LuaFunc_s::m_dmxEdit.LuaError(pLuaState,
                                         "No Supressed Controls Specified");
...
bool CDmxEdit::SetStereoControl(const char *pControlName, bool bStereo) {
  ...
  const ControlIndex_t nControlIndex =
      pDmeCombo->FindControlIndex(pControlName);
  if (nControlIndex < 0)
    return SetErrorString("No Control Named \"%s\" On DmeCombinationOperator "
                          "\"%s\" On Mesh \"%s\"",
                          pControlName, pDmeCombo->GetName(),
                          m_pMesh->GetName());
...
LUA_COMMAND(
  ...
  const bool bStereo =
      lua_isboolean(pLuaState, 2) ? lua_toboolean(pLuaState, 2) != 0 : true;
...
LUA_COMMAND(SetEyelidControl, " < $controlName > [ , < true | false > ]",
            "Sets the specified morph control to be an eyelid control if the "
...
LUA_COMMAND(MaxDeltaDistance, " < $delta >",
            "Returns the maximum distance any vertex moves in the specified "
            "delta.  Returns 0 if the delta specified doesn't exist") {
  LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
float CDmxEdit::DeltaRadius(const char *pDeltaName) {
  ...
  const CUtlVector<int> &deltaPosIndices =
...
LUA_COMMAND(DeltaRadius, " < $delta >",
            "Returns the radius of the specified delta, if it exists.  Radius "
            "of a delta is defined as the radius of the tight bounding sphere "
...
LUA_COMMAND(
    SelectionRadius, "",
    "Returns the radius of the currently selected vertices, if nothing is "
    "selected, returns 0.  Radius of a the vertices defined as the radius of "
    "the tight bounding sphere containing all of the vertices") {
  LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
//
//-----------------------------------------------------------------------------
bool CDmxEdit::SetWrinkleScale(const char *pControlName,
                               const char *pRawControlName, float flScale) {
  if (!m_pMesh)
    return SetErrorString("No Mesh");
  ...
  for (int nRawControlIndex = 0;
       nRawControlIndex < pDmeCombo->GetRawControlCount(nControlIndex);
       ++nRawControlIndex) {
    if (!Q_strcmp(pRawControlName, pDmeCombo->GetRawControlName(
                                       nControlIndex, nRawControlIndex))) {
  ...
  if (!bFoundRawControl) {
    ...
    for (int nRawControlIndex = 0;
      ...
      rawControls +=
          pDmeCombo->GetRawControlName(nControlIndex, nRawControlIndex);
    }

    return SetErrorString(
        "Control \"%s\" Does Not Have Raw Control \"%s\", Raw Controls: %s ",
        pControlName, pRawControlName, rawControls.Get());
  }
...
LUA_COMMAND(
    ...
    "NOTE: SetWrinkleScale merely sets the wrinkle scale value of the "
    "specified delta on the combination operator.  "
    "This value is only used if ComputeWrinkles() is called after all "
    "SetWrinkleScale() calls are made.  "
    "A wrinkle scale of 0 means there will be no wrinkle deltas.  "
    "The function to compute a single delta's wrinkle values, ComputeWrinkle() "
    "does not use the wrinkle scale value at all.  "
  ...
  if (LuaFunc_s::m_dmxEdit.SetWrinkleScale(pControlName, pRawControlName,
                                           fScale))
    return LuaFunc_s::m_dmxEdit.LuaOk(pLuaState);
...
void CDmxEdit::Error(const tchar *pMsgFormat, ...) {
  ...
  assert(
      _tcslen(pMsgFormat) <
      sizeof(
          pTempBuffer)); // check that we won't artificially truncate the string
  ...
  if (CommandLine()->FindParm("-coe") ||
      CommandLine()->FindParm("-continueOnError"))
    return;
...
//
//-----------------------------------------------------------------------------
void CDmxEdit::GetFuncArg(lua_State *pLuaState, int nIndex,
                          CUtlString &funcString) {
...
CDmxEditLua::CDmxEditLua() : m_pLuaState(lua_open()) {
  ...
  for (LuaFunc_s *pFunc = LuaFunc_s::s_pFirstFunc; pFunc;
       pFunc = pFunc->m_pNextFunc) {
  ...
  const char *pLuaInit =
      ...
      "vsLuaDir = string.gsub( vsLuaDir, \"/*$\", \"\" );\n"
      "vsLuaDir = ( string.gsub( vsLuaDir, \"[^/]+$\", \"sdktools/lua\" ) .. "
      "\"/?.lua\" );\n"
      "package.path = ( package.path .. \";\" .. vsLuaDir );\n"
      "require( \"vs\" );\n";

  if (luaL_loadstring(m_pLuaState, pLuaInit) ||
...
void CDmxEditLua::SetVar(const CUtlString &var, const CUtlString &val) {
  ...
  if (luaL_loadstring(m_pLuaState, sLuaCmd.Get()) ||
      lua_pcall(m_pLuaState, 0, LUA_MULTRET, 0)) {
    Error("Error: %s\n", lua_tostring(m_pLuaState, -1));
  }
}
