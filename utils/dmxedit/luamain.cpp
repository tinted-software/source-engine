...
int CDmxEditApp::Main() {
  ...
  for (int i = 0; i < nParamCount - 1; ++i) {
    ...
    if (StringHasPrefix(pCmd, "-s")) {
      ...
      if (!pEquals) {
        Warning("Warning: Invalid command line args, no ='s in -set argument: "
                "%s %s\n",
                pCmd, pArg);
      }
    ...
    } else if (StringHasPrefix(pCmd, "-coe") ||
               StringHasPrefix(pCmd, "-ContinueOnError")) {
      // Don't issue warning on -nop4
...
//
//-----------------------------------------------------------------------------
template <class T_t> int Sort_LuaFunc_s(const T_t *pA, const T_t *pB) {
...
void CDmxEditApp::PrintHelp(bool bWiki /* = false */) {
  ...
  for (int i = 0; i < luaFuncs.Count(); ++i) {
    ...
    const char *pWordBegin =
        pFunc->m_pFuncDesc + strspn(pFunc->m_pFuncDesc, pWhitespace);
