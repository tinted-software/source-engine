...
bool CBugReporter::Init(CreateInterfaceFn engineFactory) {
  ...
  if (!m_OptionsFile->LoadFromFile(g_filesystem, "bugreporter_text.txt",
                                   "EXECUTABLE_PATH")) {
...
void CBugReporter::StartNewBugReport() {
  ...
  do {
    m_CurrentBugID++;
    Q_snprintf(m_CurrentBugDirectory, sizeof(m_CurrentBugDirectory), "%s/%d",
               m_BugRootDirectory, m_CurrentBugID);
...
bool CBugReporter::CommitBugReport(int &bugSubmissionId) {
  ...
  // Ignoring, since the bsp name is in the title
  buf.Printf("\n%s\n\n", m_pBug->desc);
...
bool CBugReporter::PopulateLists() {
  ...
  /*
          ...
          for ( int i = 1 ; i < c; i++ )
          {
                  for ( int j = i + 1; j < c; j++ )
                  {
                          char const *p1 = m_BugStrings.String(
     m_SortedDisplayNames[ i ] ); char const *p2 = m_BugStrings.String(
     m_SortedDisplayNames[ j ] );

                          int cmp = Q_stricmp( p1, p2 );
                          if ( cmp > 0 )
                          {
                                  CUtlSymbol t = m_SortedDisplayNames[ i ];
                                  m_SortedDisplayNames[ i ] =
     m_SortedDisplayNames[ j ]; m_SortedDisplayNames[ j ] = t;
                          }
                  }
          }
  */
