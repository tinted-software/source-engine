...
void main(int argc, char **argv) {
  ...
  // test sse noise
  FourVectors start;
  start.LoadAndSwizzle(Vector(-1, 4, 3), Vector(0, 7, 8), Vector(8, 1, 2),
                       Vector(0, 0, 0));
  FourVectors delta;
  delta.LoadAndSwizzle(Vector(.1, -.1, .05), Vector(.1, .1, .1),
  ...
  for (int i = 0; i < 130; i++) {
    ...
    printf(" %f,%f,%f,%f,%f\n", start.X(0), noise.m128_f32[0],
           noise.m128_f32[1], noise.m128_f32[2], noise.m128_f32[3]);
    start += delta;
  }
