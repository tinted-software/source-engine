...
void GenerateStrips(const unsigned short *in_indices,
  ...
  stripifier.Stripify(tempIndices, cacheSize, minStripSize, maxIndex,
                      tempStrips, tempFaces);
    ...
    for (int i = 0; i < tempStrips.size(); i++) {
      for (int j = 0; j < tempStrips[i]->m_faces.size(); j++) {
        ...
        if (!NvStripifier::IsDegenerate(tempStrips[i]->m_faces[j])) {
          primGroupArray[0].indices[indexCtr++] =
              AsUShort(tempStrips[i]->m_faces[j]->m_v0);
          primGroupArray[0].indices[indexCtr++] =
              AsUShort(tempStrips[i]->m_faces[j]->m_v1);
          primGroupArray[0].indices[indexCtr++] =
              AsUShort(tempStrips[i]->m_faces[j]->m_v2);
        } else {
    ...
    *numGroups = AsUShort(numSeparateStrips); // for the strips
    ...
    for (int stripCtr = 0; stripCtr < numSeparateStrips; stripCtr++) {
      ...
      for (int i = startingLoc; i < stripLength + startingLoc; i++)
        primGroupArray[stripCtr].indices[indexCtr++] =
            AsUShort(stripIndices[i]);
    ...
    if (tempFaces.size() != 0) {
      ...
      primGroupArray[faceGroupLoc].indices =
          new unsigned short[tempFaces.size() * 3];
      primGroupArray[faceGroupLoc].numIndices =
          (unsigned int)(tempFaces.size() * 3);
      int indexCtr = 0;
      for (int i = 0; i < tempFaces.size(); i++) {
        primGroupArray[faceGroupLoc].indices[indexCtr++] =
            AsUShort(tempFaces[i]->m_v0);
        primGroupArray[faceGroupLoc].indices[indexCtr++] =
            AsUShort(tempFaces[i]->m_v1);
        primGroupArray[faceGroupLoc].indices[indexCtr++] =
            AsUShort(tempFaces[i]->m_v2);
      }
    }
  }
