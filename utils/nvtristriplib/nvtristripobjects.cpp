...
void NvStripifier::BuildStripifyInfo(NvFaceInfoVec &faceInfos,
  ...
  for (int i = 0; i < numTriangles; i++) {
    ...
    if (edgeInfo01 == NULL) {
      // since one of it's edges isn't in the edge data structure, it can't
      // already exist in the face structure
      ...
      if (edgeInfo01->m_face1 != NULL) {
        //				printf("BuildStripifyInfo: > 2 triangles
        //on an edge... uncertain consequences\n");
      } else {
...
//
void NvStripifier::GetSharedVertices(NvFaceInfo *faceA, NvFaceInfo *faceB,
...
//
inline int NvStripifier::GetNextIndex(const WordVec &indices,
                                      NvFaceInfo *face) {
...
// RemoveSmallStrips()
//
// allStrips is the whole strip vector...all small strips will be deleted from
...
bool NvStripifier::FaceContainsIndex(const NvFaceInfo &face,
                                     const unsigned int index) {
  return ((face.m_v0 == (int)index) || (face.m_v1 == (int)index) ||
          (face.m_v2 == (int)index));
}

bool NvStripifier::IsMoneyFace(const NvFaceInfo &face) {
  if (FaceContainsIndex(face, 800) && FaceContainsIndex(face, 812) &&
      ...
      if (nStripFaceCount > 1) {
        ...
        if (nStripFaceCount > 2) {
          if (IsDegenerate(strip->m_faces[1])) {
            ...
            GetSharedVertices(strip->m_faces[2], &tFirstFace, &nShared0,
                              &nShared1);
...
bool NvStripifier::IsDegenerate(const unsigned short v0,
...
//
// Splits the input vector of strips (allBigStrips) into smaller, cache friendly
// pieces, then
...
void NvStripifier::SplitUpStripsAndOptimize(NvStripInfoVec &allStrips,
  ...
  for (int i = 0; i < allStrips.size(); i++) {
    ...
    if (actualStripSize /*allStrips[i]->m_faces.size()*/ > threshold) {
      ...
      int numTimes =
          actualStripSize /*allStrips[i]->m_faces.size()*/ / threshold;
      int numLeftover =
          actualStripSize /*allStrips[i]->m_faces.size()*/ % threshold;
      ...
      for (j; j < numTimes; j++) {
        ...
        while (faceCtr < threshold + (j * threshold) + degenerateCount) {
          if (IsDegenerate(allStrips[i]->m_faces[faceCtr])) {
            ...
            if ((((faceCtr + 1) !=
                  threshold + (j * threshold) + degenerateCount) ||
                 ((j == numTimes - 1) && (numLeftover < 4) &&
                  (numLeftover > 0))) &&
                !bFirstTime) {
    ...
    while (1) {
      ...
      for (int i = 0; i < tempStrips2.size(); i++) {
        ...
        if (numHits > bestNumHits) {
          ...
          // check previous strip to see if this one requires it to switch
          // polarity
          ...
          // unique vertex is first
          if (nStripFaceCount > 1) {
            int nUnique = NvStripifier::GetUniqueVertexInB(strip->m_faces[1],
                                                           &tFirstFace);
            if (nUnique == tFirstFace.m_v1) {
              SWAP(tFirstFace.m_v0, tFirstFace.m_v1);
            } else if (nUnique == tFirstFace.m_v2) {
      ...
      bWantsCW = (tempStrips2[bestIndex]->m_faces.size() % 2 == 0) ? bWantsCW
                                                                   : !bWantsCW;
    }
...
void NvStripifier::FindAllStrips(NvStripInfoVec &allStrips,
  ...
  while (!done) {
    ...
    for (int i = 0; i < numExperiments; i++) {
      ...
      float value =
          avgStripSize * avgStripSizeWeight + (numStrips * numStripsWeight);
