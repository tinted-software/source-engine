...
#pragma warning(                                                               \
    disable : 4311) // warning C4311: 'reinterpret_cast' : pointer truncation
                    // from 'CConsoleWnd *const ' to 'LONG'
#pragma warning(disable : 4312) // warning C4312: 'type cast' : conversion from
                                // 'LONG' to 'CConsoleWnd *' of greater size
...
public:
  ...
  bool Init(void *hInstance, int dialogResourceID, int editControlID,
            bool bVisible);
  void Term();
...
CConsoleWnd::~CConsoleWnd() { Term(); }

bool CConsoleWnd::Init(void *hInstance, int dialogResourceID, int editControlID,
                       bool bVisible) {
  // Create the window.
  m_hWnd = CreateDialog((HINSTANCE)hInstance, MAKEINTRESOURCE(dialogResourceID),
                        NULL, &CConsoleWnd::StaticWindowProc);
...
void CConsoleWnd::Release() { delete this; }
...
void CConsoleWnd::SetVisible(bool bVisible) {
  ...
  if (bVisible) {
    ...
    SetWindowPos(m_hWnd, 0, 0, 0, 0, 0,
                 SWP_NOMOVE | SWP_NOSIZE | SWP_HIDEWINDOW | SWP_NOOWNERZORDER);
  }

  m_bVisible = bVisible;
}

bool CConsoleWnd::IsVisible() const { return m_bVisible; }
...
void CConsoleWnd::RepositionEditControl() {
  ...
  SetWindowPos(m_hEditControl, NULL, rcNew.left, rcNew.top,
               rcNew.right - rcNew.left, rcNew.bottom - rcNew.top,
               SWP_NOZORDER);
}

void CConsoleWnd::SetDeleteOnClose(bool bDelete) { m_bDeleteOnClose = bDelete; }
