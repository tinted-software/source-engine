...
SetScriptLoadedCallback(SCRIPT_LOADED_CALLBACK pfnNewScriptLoadedCallback) {
...
//-----------------------------------------------------------------------------
static void FindFileAbsoluteList(CUtlVector<CUtlString> &outAbsolutePathNames,
                                 const char *pszFindName) {
  ...
  for (const char *pszFoundFile =
           g_pFullFileSystem->FindFirst(pszFindName, &hFile);
