...
AUTO_INCREMENT, index index_jobid( JobID ), index index_jobworkerid( JobWorkerID
        ...
        JobID					INTEGER UNSIGNED NOT NULL,
-- links to job_master_start::JobID IsMaster				BOOL NOT
NULL,							-- Set to 1 if this
"worker" is the master process. RunningTimeMS			INTEGER UNSIGNED
NOT NULL default 0, MachineName				TEXT NOT NULL,
...
create table text_messages (
        JobWorkerID				INTEGER UNSIGNED NOT NULL,
index id( JobWorkerID, MessageIndex ), MessageIndex			INTEGER
UNSIGNED NOT NULL, Text					TEXT NOT NULL
        );
...
public:
  ...
  virtual int RunCommand() {
    CMySQLQuery query;
    query.Format("insert into graph_entry (JobWorkerID, MSSinceJobStart, "
...
public:
  ...
  virtual int RunCommand() {
    CMySQLQuery query;
    query.Format("insert into text_messages (JobWorkerID, MessageIndex, Text) "
                 "values ( %lu, %lu, \"%s\" )",
...
bool VMPI_Stats_Init_Worker(const char *pHostName, const char *pDBName,
  ...
  CMySQLQuery query;
  query.Format("insert into job_worker_start ( JobID, CurrentStage, IsMaster, "
