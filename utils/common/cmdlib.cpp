...
SpewRetval_t CmdLib_SpewOutputFunc(SpewType_t type, char const *pMsg) {
  ...
  EnterCriticalSection(&g_SpewCS);
  {
    if ((type == SPEW_MESSAGE) || (type == SPEW_LOG)) {
      ...
      old = SetConsoleTextColor(1, 0, 0, 1);
      retVal = SPEW_ABORT; // doesn't matter.. we exit below so we can return an
                           // errorlevel (which dbg.dll doesn't do).
    } else {
...
int CmdLib_ExpandWithBasePaths(CUtlVector<CUtlString> &expandedPathList,
  ...
  pszPath = ExpandPath(
      const_cast<char *>(pszPath)); // Kind of redundant but it's how
                                    // CmdLib_HasBasePath needs things
