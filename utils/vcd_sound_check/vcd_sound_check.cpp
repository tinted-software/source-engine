...
void printusage(void) {
  vprint(
      0,
      "usage:  vcd_sound_check <.wav root directory> <scenes root directory>\n\
		\t-v = verbose output\n\
		\t-m = spew moveto info\n\
...
void SpewMoveto(bool first, char const *vcdname, CChoreoEvent *e) {
  ...
  logprint("moveto.txt", "\"%s\",%i,\"%s\",%.3f,\"%s\",%s\n", vcdname,
           ++ecounter, e->GetName(), e->GetStartTime(), e->GetParameters(),
...
static bool EventCollidesWithRows(CUtlLinkedList<CChoreoEvent *, int> &list,
                                  CChoreoEvent *event, char *trackName,
...
void CheckForOverlappingFlexTracks(CChoreoScene *scene) {
  for (int a = 0; a < scene->GetNumActors(); ++a) {
    ...
    CUtlRBTree<CChoreoEvent *> actorFlexEvents(0, 0,
                                               ChoreEventStartTimeLessFunc);
    ...
    for (i = actorFlexEvents.FirstInorder();
         i != actorFlexEvents.InvalidIndex() && !done;
...
void ProcessVCD(CUtlDict<VCDList, int> &database, CUtlSymbol &vcdname) {
  ...
  if (scene) {
    ...
    for (int i = 0; i < c; i++) {
      ...
      char const *wavename =
          soundemitter->GetWavFileForSound(e->GetParameters(), NULL);
      if (!wavename || !wavename[0]) {
        continue;
      }

      char fullwavename[512];
      Q_snprintf(fullwavename, sizeof(fullwavename), "%ssound\\%s", gamedir,
                 wavename);
...
void CorrelateWavsAndVCDs(CUtlVector<CUtlSymbol> &vcdfiles,
    ...
    if (idx != database.InvalidIndex()) {
      ...
      if (vcdcount >= 2 && verbose) {
        ...
        for (j = 0; j < vcdcount; j++) {
          vprint(1, "%i -- '%s'\n", j + 1,
                 g_Analysis.symbols.String(listentry->vcds[j]));
        }
      }
      continue;
    }
...
int main(int argc, char *argv[]) {
  ...
  CSysModule *pSoundEmitterModule =
      g_pFullFileSystem->LoadModule("soundemittersystem.dll");
  ...
  soundemitter = (ISoundEmitterSystemBase *)hSoundEmitterFactory(
