...
static CFmtStr PrettyPrintVector(const Vector &v) {
  return CFmtStr("%s %s %s", PrettyPrintFloat(v.x).Get(),
                 PrettyPrintFloat(v.y).Get(), PrettyPrintFloat(v.z).Get());
}
...
//-----------------------------------------------------------------------------
static const mstudiosrcbonetransform_t *
GetSrcBoneTransform(const studiohdr_t *pStudioHdr, int nBoneIndex) {
  ...
  for (int i = 0; i < nSrcBoneTransformCount; ++i) {
    const mstudiosrcbonetransform_t *pSrcBoneTransform =
        pStudioHdr->SrcBoneTransform(i);
    if (pSrcBoneTransform &&
        !Q_stricmp(pSrcBoneTransform->pszName(), pszBoneName))
...
void CMdlInfoApp::HandleOptions() {
  if (CommandLineHasOpt(kOptGeneral) || CommandLineHasOpt(kOptBones) ||
      CommandLineHasOpt(kOptAttachments) || CommandLineHasOpt(kOptAnimation) ||
      CommandLineHasOpt(kOptSequence) || CommandLineHasOpt(kOptMesh) ||
...
void CMdlInfoApp::PrintHelp() {
  ...
  for (int ii = 0; ii < ARRAYSIZE(s_binaryOptions); ++ii) {
    ...
    Msg("    %s | %s : %s\n", opt.m_pszShortName, opt.m_pszLongName,
        opt.m_pszDoc);
  }
  ...
  Msg("    Multiple section specifiers can be specified but print order is "
      "fixed\n");
  Msg("\n");
  Msg("\n");
}
...
static void PrintBoneFlags(const mstudiobone_t *pBone) {
  ...
  for (int ii = 0; ii < ARRAYSIZE(boneFlags); ++ii) {
    if (pBone->flags & boneFlags[ii].m_nMask) {
      Msg("//     - 0x%08x %s\n", boneFlags[ii].m_nMask,
          boneFlags[ii].m_pzsName);
    }
  }
}
...
void CMdlInfoApp::DumpBones(CStudioHdr &cStudioHdr) const {
  ...
  for (int ii = 0; ii < nBoneCount; ++ii) {
    ...
    const mstudiobone_t *pParentBone = pBone->parent < ii && pBone->parent >= 0
                                           ? cStudioHdr.pBone(pBone->parent)
                                           : NULL;
    const mstudiosrcbonetransform_t *pSrcBoneTransform =
        GetSrcBoneTransform(cStudioHdr.GetRenderHdr(), ii);
    ...
    if (m_bOptVerbose) {
      ...
      if (pBone->flags & BONE_ALWAYS_PROCEDURAL) {
        static const char *s_ProcType[] = {"<ERROR",     "AXISINTERP",
                                           "QUATINTERP", "AIMATBONE",
                                           "AIMATTACH",  "JIGGLE"};
    ...
    CFmtStrMax sTmp(
        "%s$definebone \"%s\" \"%s\" %s %s %s %s %s %s",
        (pBone->flags & BONE_ALWAYS_PROCEDURAL) ? "// procedural " : "",
        pBone->pszName(), pParentBone ? pParentBone->pszName() : "",
        PrettyPrintFloat(pBone->pos.x).Get(),
        PrettyPrintFloat(pBone->pos.y).Get(),
        PrettyPrintFloat(pBone->pos.z).Get(), PrettyPrintFloat(aRot.x).Get(),
        PrettyPrintFloat(aRot.y).Get(), PrettyPrintFloat(aRot.z).Get());
  ...
  for (int ii = 0; ii < nSeqCount; ++ii) {
    ...
    if (pHdr != cStudioHdr.GetRenderHdr()) {
      Msg("// %3d: %-50s %-50s [Included From %s]\n", ii, seqDesc.pszLabel(),
          seqDesc.pszActivityName(), pHdr->pszName());
    } else {
      Msg("// %3d: %-50s %-50s\n", ii, seqDesc.pszLabel(),
