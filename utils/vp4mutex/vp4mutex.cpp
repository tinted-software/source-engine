...
//
// NOTE: All of this crap is here since we don't want to have the .exe depend on
// tier0 or vstdlib.dll.  If we change that, this can go away and std::vector
// can go back
...
static char *Q_strncat(char *pDest, const char *pSrc, size_t destBufferSize,
...
// value
//-----------------------------------------------------------------------------
static void SplitP4Output(const_char *data, char *pszCmd, char *pszInfo,
                          int bufLen) {
...
//-----------------------------------------------------------------------------
template <class T> class CDataRetrievalUser : public ClientUser {
public:
...
class CP4Counter {
public:
  CP4Counter() : m_nValue(0) { m_szName[0] = 0; }
...
private:
  virtual void OutputRecord(CP4Counter &counter, const char *szCmd,
                            const char *szInfo) {
...
public:
  ...
  char const *String() { return m_szValue; }

private:
  char m_szValue[256];
};

int FindLockUsers(CCountersUser &counters, char const *branchspec,
                  std::vector<CUtlSymbol> &users, std::vector<int> &locktimes,
                  std::vector<CUtlSymbol> *branchnames = NULL) {
  users.RemoveAll();
...
static void GetHourMinuteSecondsString(int nInputSeconds, char *pOut,
                                       int outLen) {
...
int _tmain(int argc, _TCHAR *argv[]) {
  ...
  if (describeLocksOnly) {
    ...
    int holdCount =
        FindLockUsers(g_CountersUser, NULL, users, locktimes, &branchnames);

    for (int i = 0; i < holdCount; ++i) {
      char timestr[128];
      ComputeHoldTime(locktimes[i], timestr, sizeof(timestr));
      printf("'%s' HELD by:  %s\n\ttime: %s\n", branchnames[i].String(),
             users[i].String(), timestr);
    }
  } else {
    ...
    std::vector<int> locktimes;
    int holdCount =
        FindLockUsers(g_CountersUser, branchspec.String(), users, locktimes);

    if (holdCount >= 2) {
      char userlist[1024];
      userlist[0] = 0;
      for (int i = 0; i < (int)users.Count(); ++i) {
        Q_strncat(userlist, users[i].String(), sizeof(userlist),
                  COPY_ALL_CHARACTERS);
        if (i != users.Count() - 1) {
      ...
      printf("%s:  ERROR, multiple users (%s) holding lock on '%s'\n", basefile,
             userlist, branchspec.String());
      printusage(basefile);
      return -1;
    }

    char setcountername[256];
    Q_snprintf(setcountername, sizeof(setcountername), "%s_lock_%s",
    ...
    case MUTEX_LOCK: {
      printf("%s: Attempting to lock the '%s' codeline for %s on %s\n\n",
             basefile, branchspec.String(), clientname.String(),
             ipport.String());

      // p4mutex: Attempting to lock the 'main_src' codeline for yahn on
      // 207.173.178.12:1666.
      ...
      if (holdCount == 1) {
        ...
        if (isHeldByLocal) {
          ...
          printf(
              "Success: You already have the '%s' codeline lock\nInfo:  %s\n",
    ...
    case MUTEX_RELEASE: {
      ...
      // p4mutex: Attempting to release the 'main_src' codeline for yahn on
      // 207.173.178.12:1666.
