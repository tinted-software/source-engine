...
bool AddConfig(int configID) {
  ...
  g_ConfigManager.GetRootGameDirectory(fullDir, sizeof(fullDir),
                                       g_ConfigManager.GetRootDirectory());
...
bool RemoveConfig(int configID) {
  ...
  for (KeyValues *pGame = gameBlock->GetFirstTrueSubKey(); pGame;
       pGame = pGame->GetNextTrueSubKey(), i++) {
    if (i == configID) {
...
//-----------------------------------------------------------------------------
// Purpose: Updates the internal data of the keyvalue buffer with the edited
// info Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool UpdateConfigs(void) {
  ...
  for (KeyValues *pGame = gameBlock->GetFirstTrueSubKey(); pGame != NULL;
       pGame = pGame->GetNextTrueSubKey(), i++) {
...
static LRESULT CALLBACK messageProc(HWND hwnd, UINT msg, WPARAM wparam,
  ...
  if (msg == WM_SETTINGCHANGE) {
    if (g_pMainFrame != NULL) {
      // Reset the list and pop an error if they've chosen something we don't
      // understand
...
void CreateMessageWindow(void) {
  ...
  staticHwnd = CreateWindowEx(0, "VConfig_Window", "Hidden Window", 0, 0, 0, 1,
                              1, NULL, NULL, GetModuleHandle(NULL), NULL);
}
...
void SetMayaScriptSettings() {
  ...
  Q_snprintf(pMayaScriptPath, sizeof(pMayaScriptPath),
             "%%VPROJECT%%\\..\\sdktools\\maya\\scripts");
...
void SetXSIScriptSettings() {
  ...
  // so we can set up the appropriate search paths. There's no easy way of doing
  // this currently so I'm defining my own environment variable
  char pXSIVersionBuf[MAX_PATH];
  if (GetVConfigRegistrySetting("XSI_VERSION", pXSIVersionBuf,
                                sizeof(pXSIVersionBuf))) {
    pXSIVersion = pXSIVersionBuf;
  }

  char pXSIPluginPath[MAX_PATH];
  Q_snprintf(pXSIPluginPath, sizeof(pXSIPluginPath),
             "%%VPROJECT%%\\..\\sdktools\\xsi\\%s\\valvesource", pXSIVersion);
...
bool CVConfigApp::Create() {
  ...
  // If they pass in -game, just set the registry key to the value they asked
  // for.
