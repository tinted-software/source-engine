...
// Constructor
//-----------------------------------------------------------------------------
CManageGamesDialog::CManageGamesDialog(Panel *parent, const char *name,
                                       int configID)
...
// Destructor
//-----------------------------------------------------------------------------
CManageGamesDialog::~CManageGamesDialog(void) { g_pManageGamesDialog = NULL; }
...
void CManageGamesDialog::OnCommand(const char *command) {
  ...
  if (Q_stricmp(command, "Select") == 0) {
    ...
    if (IsGameNameUnique(textBuffer) == false) {
      ...
      VGUIMessageBox(
          this, "Error",
          "Game name %s already exists!  Please enter a unique name.",
          textBuffer);
    ...
    if (m_nConfigID == NEW_CONFIG_ID) {
      ...
      // Send an overidden action signal to notify that we've added, not edited
      // a field
  ...
  // Modified to allow more than one browse button
    ...
    // Create a new dialog
    CModalDirectorySelectDialog *pDlg = vgui::SETUP_PANEL(
        new CModalDirectorySelectDialog(this, "Select Game Directory"));
