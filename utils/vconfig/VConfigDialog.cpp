...
public:
  ...
  virtual void OnCommand(const char *command) {
    ...
    if (Q_stricmp(command, "ShowFAQ") == 0) {
      system()->ShellExecute(
          "open", "http://www.valve-erc.com/srcsdk/faq.html#convertINI");
    }
  }
};
...
// Constructor
//-----------------------------------------------------------------------------
CVConfigDialog::CVConfigDialog(Panel *parent, const char *name)
  ...
  if (g_ConfigManager.WasConvertedOnLoad()) {
    // VGUIMessageBox( this, "Update Occured", "Your game configurations have
    // been updated.\n\nA backup file GameCfg.INI.OLD has been
    // created.\n\nPlease visit
    // http://www.valve-erc.com/srcsdk/faq.html#GameConfigUpdate for more
    // information." );
...
void CVConfigDialog::PopulateConfigList(bool bSelectActiveConfig /*=true*/) {
  ...
  if (bSelectActiveConfig) {
    ...
    if (GetVConfigRegistrySetting(GAMEDIR_TOKEN, szKeyValue,
                                  sizeof(szKeyValue)) == false) {
      // NOTE: We may want to pop an info dialog here if there was no initial
      // VPROJECT setting
      activeItem = 0;
    }
  } else {
  ...
  if (activeItem < 0) {
    ...
    VGUIMessageBox(
        this, "Invalid Game Directory",
        "The currently selected game directory: %s is invalid.\nChoose a new "
        "directory, or select 'Cancel' to exit.\n",
        szKeyValue);
...
//-----------------------------------------------------------------------------
// Purpose: Select the item from the list (updating the environment variable as
// well) Input  : index - item to select
//-----------------------------------------------------------------------------
...
void CVConfigDialog::OnCommand(const char *command) {
  if (Q_stricmp(command, "Select") == 0) {
    ...
    CManageGamesDialog *pDialog =
        new CManageGamesDialog(this, "ManageGamesDialog", activeID);
    ...
    CManageGamesDialog *pDialog =
        new CManageGamesDialog(this, "ManageGamesDialog", NEW_CONFIG_ID);
    ...
    if (g_Configs.Count() <= 1) {
      VGUIMessageBox(this, "Error",
                     "Cannot remove last configuration from list!");
      return;
    }
...
void CVConfigDialog::OnManageSelect(void) {
  // Publish the config changes to the internal data in the configuration
  // manager
  UpdateConfigs();
