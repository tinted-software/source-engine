...
//
//--------------------------------------------------------------------------------------------------
static void WriteIndent(IBaseFileSystem *pFileSystem, FileHandle_t hFile,
                        bool bOptTabs, int nOptTabWidth, int nIndentLevel) {
  if (nIndentLevel <= 0)
    return;
...
//
//--------------------------------------------------------------------------------------------------
static void WritePadding(IBaseFileSystem *pFileSystem, FileHandle_t hFile,
                         bool bOptTabs, int nOptTabWidth, int nCurrentColumn,
                         int nTargetColumn) {
  if (nCurrentColumn <= 0 || nTargetColumn <= 0 ||
      nTargetColumn <= nCurrentColumn) {
  ...
  if (bOptTabs || nOptTabWidth <= 0) {
    ...
    nTargetColumn = (nTargetColumn + nOptTabWidth - 1) / nOptTabWidth *
                    nOptTabWidth; // Round to next tab stop
...
//
//--------------------------------------------------------------------------------------------------
static void WriteConvertedString(IBaseFileSystem *pFileSystem,
                                 FileHandle_t hFile, const char *pszString) {
  ...
  char *szConvertedString =
      (char *)stackalloc((nSrcLen + 1) * sizeof(char) * 2);
  int j = 0;
...
//
//--------------------------------------------------------------------------------------------------
static void SaveToFile_R(KeyValues *pkv, IBaseFileSystem *pFileSystem,
                         FileHandle_t hFile, bool bOptTabs, int nOptTabWidth,
                         int nIndentLevel, int nValueColumn = -1) {
  ...
  if (pkv->GetFirstSubKey()) {
    ...
    for (KeyValues *pkvSubKey = pkv->GetFirstSubKey(); pkvSubKey;
    ...
    for (KeyValues *pkvSubKey = pkv->GetFirstSubKey(); pkvSubKey;
         pkvSubKey = pkvSubKey->GetNextKey()) {
      SaveToFile_R(pkvSubKey, pFileSystem, hFile, bOptTabs, nOptTabWidth,
                   nIndentLevel + 1, nSubKeyValueColumn);
    }
    WriteIndent(pFileSystem, hFile, bOptTabs, nOptTabWidth, nIndentLevel);
    pFileSystem->Write("}\n", 2, hFile);
  } else {
    if (nValueColumn > 0) {
      ...
      WritePadding(pFileSystem, hFile, bOptTabs, nOptTabWidth, nNameLength,
                   nValueColumn);
    } else {
      WriteIndent(pFileSystem, hFile, bOptTabs, nOptTabWidth,
                  1); // Don't know which column to align things on
    }
...
#ifdef WIN32
      Q_snprintf(szTmpBuf, sizeof(szTmpBuf), "0x%016I64X",
                 *((uint64 *)pkv->GetUint64()));
#else
      Q_snprintf(szTmpBuf, sizeof(szTmpBuf), "0x%016llX",
                 *((uint64 *)pkv->GetUint64()));
#endif
      pFileSystem->Write(szTmpBuf, V_strlen(szTmpBuf), hFile);
      break;
...
//
//--------------------------------------------------------------------------------------------------
static bool SaveCleanKeyValuesToFile(KeyValues *pkv,
                                     IBaseFileSystem *pFileSystem,
                                     const char *pszFileName,
                                     const char *pszPathID = nullptr,
                                     bool bOptTabs = true,
                                     int nOptSpaceIndent = 4) {
  // Write out KeyValues to the specified file but cleaner than
  // KeyValues::SaveToFile create a write file
  FileHandle_t hFile = pFileSystem->Open(pszFileName, "wb", pszPathID);

  if (hFile == FILESYSTEM_INVALID_HANDLE) {
    Msg("CleanSaveKeyValuesToFile: Couldn't open file \"%s\" for writing in "
        "path \"%s\".\n",
...
//
//--------------------------------------------------------------------------------------------------
void ProcessPaintKitKeyValuesFile(const char *pszPaintKitKeyValuesFile,
                                  bool bOptFix, bool bOptVerbose) {
  if (bOptVerbose) {
  ...
  if (!V_IsAbsolutePath(pszPaintKitKeyValuesFile)) {
    g_pFullFileSystem->RelativePathToFullPath_safe(pszPaintKitKeyValuesFile,
                                                   nullptr, szFullPath);
    if (!V_IsAbsolutePath(szFullPath)) {
      ...
      if (!g_pFullFileSystem->GetCurrentDirectoryA(szTmpA, ARRAYSIZE(szTmpB))) {
        Msg("Error: non-resolvable, non-absolute path specified (%s) and "
            "couldn't determine current directory\n",
            pszPaintKitKeyValuesFile);
        return;
      }

      V_ComposeFileName(szTmpA, pszPaintKitKeyValuesFile, szTmpB,
                        ARRAYSIZE(szTmpB));
  ...
  if (bLoaded) {
    ...
    if (bOptFix) {
      ...
      if (!SaveCleanKeyValuesToFile(pkv, g_pFullFileSystem, szFilename, nullptr,
                                    false)) {
      ...
      if (CopyFile(szFullPath, szFilename, false)) {
        ...
        if (!SaveCleanKeyValuesToFile(pkv, g_pFullFileSystem, szFullPath,
                                      nullptr, false)) {
          Msg("Error: Couldn't write: %s\n", szFullPath);
        }
      } else {
