...
//
// Purpose: unusedcontent.cpp : Defines the entry point for the console
// application.
//
//=============================================================================//
#include "cbase.h"
...
char const *CUtlSymbol::String() const { return CurrTable()->String(m_Id); }
...
inline const char *
CUtlSymbolTable::StringFromIndex(const CStringPoolIndex &index) const {
...
bool CUtlSymbolTable::SymLess(CStringPoolIndex const &i1,
                              CStringPoolIndex const &i2) {
  char const *str1 = (i1 == INVALID_STRING_INDEX)
                         ? g_LessCtx.m_pUserString
                         : g_LessCtx.m_pTable->StringFromIndex(i1);
  char const *str2 = (i2 == INVALID_STRING_INDEX)
                         ? g_LessCtx.m_pUserString
...
bool CUtlSymbolTable::SymLessi(CStringPoolIndex const &i1,
                               CStringPoolIndex const &i2) {
  char const *str1 = (i1 == INVALID_STRING_INDEX)
                         ? g_LessCtx.m_pUserString
                         : g_LessCtx.m_pTable->StringFromIndex(i1);
...
CUtlSymbol CUtlSymbolTable::AddString(char const *pString) {
  ...
  Assert(pPool->m_SpaceUsed <
         0xFFFF); // This should never happen, because if we had a string > 64k,
                  // it would have been given its entire own pool.
...
char *directories_to_check[] = {
    "",       "bin",     "maps",  "materials", "models",
    "scenes", "scripts", "sound", "hl2",
};

char *directories_to_ignore[] = // don't include these dirs in the others list
    {"reslists", "reslists_temp", "logs",        "media", "downloads",
     "save",     "screenshots",   "testscripts", "logos"};

enum { REFERENCED_NO = 0, REFERENCED_WHITELIST, REFERENCED_GAME };
...
struct ReferencedFile {
  ReferencedFile() { sym = UC_UTL_INVAL_SYMBOL; }
...
void logprint(char const *logfile, const char *fmt, ...) {
  ...
  static CUtlRBTree<UnusedContent::CUtlSymbol, int> previousfiles(
      0, 0, DefLessFunc(UnusedContent::CUtlSymbol));
...
void BuildFileList_R(int depth, CUtlVector<FileEntry> &files,
                     CUtlVector<FileEntry> *otherfiles, char const *dir,
  ...
  if (!Q_stricmp(wild, "...")) {
    canrecurse = true;
    sprintf(directory, "%s%s%s", dir[0] == '\\' ? dir + 1 : dir,
            dir[0] != 0 ? "\\" : "", "*.*");
  } else {
  ...
  do {
    if (wfd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
      ...
      BuildFileList_R(depth + 1, useOtherFiles ? *otherfiles : files, NULL,
                      filename, wild, skipchars);
    } else {
      ...
      if (dirlen <= skipchars) {
        ...
        Q_snprintf(filename, sizeof(filename), "%s\\%s", &dir[skipchars],
                   wfd.cFileName);
      }
      _strlwr(filename);
...
void BuildFileList(int depth, CUtlVector<FileEntry> &files,
                   CUtlVector<FileEntry> *otherfiles, char const *rootdir,
                   int skipchars) {
  files.RemoveAll();
  Assert(otherfiles);
...
void BuildFileListWildcard(int depth, CUtlVector<FileEntry> &files,
                           char const *rootdir, char const *wildcard,
                           int skipchars) {
  files.RemoveAll();
...
static CUtlRBTree<UnusedContent::CUtlSymbol, int>
    g_WhiteList(0, 0, DefLessFunc(UnusedContent::CUtlSymbol));
...
void BuildWhiteList() {
  ...
  if (verbose || printwhitelist) {
    ...
    for (int i = g_WhiteList.FirstInorder(); i != g_WhiteList.InvalidIndex();
...
void ParseFilesFromResList(UnusedContent::CUtlSymbol &resfilesymbol,
                           CUtlRBTree<ReferencedFile, int> &files,
                           char const *resfile) {
  int addedStrings = 0;
        ...
        while (1) {
          ...
          if (strlen(token) > 0) {
            ...
            if (idx == files.InvalidIndex()) {
              ...
              if (slot.maplist.Find(resfilesymbol) ==
                  slot.maplist.InvalidIndex()) {
...
bool BuildReferencedFileList(CUtlVector<UnusedContent::CUtlSymbol> &resfiles,
                             CUtlRBTree<ReferencedFile, int> &files,
                             const char *resfile) {
    ...
    if (length > 0) {
      ...
      if (pStart &&
        ...
        while (1) {
          ...
          Q_snprintf(szResList, sizeof(szResList), "%s%s.lst", g_szReslistDir,
                     com_token);
          _strlwr(szResList);
          ...
          UnusedContent::CUtlSymbol sym =
              g_Analysis.symbols.AddString(szResList);
  ...
  if (g_pFileSystem->FileExists(CFmtStr("%sall.lst", g_szReslistDir))) {
    UnusedContent::CUtlSymbol sym =
        g_Analysis.symbols.AddString(CFmtStr("%sall.lst", g_szReslistDir));
    resfiles.AddToTail(sym);
  }

  if (g_pFileSystem->FileExists(CFmtStr("%sengine.lst", g_szReslistDir))) {
    UnusedContent::CUtlSymbol sym =
        g_Analysis.symbols.AddString(CFmtStr("%sengine.lst", g_szReslistDir));
    resfiles.AddToTail(sym);
  }
  ...
  if (resfiles.Count() <= 0) {
    vprint(0,
           "%s didn't have any actual .lst files in the reslists folder, have "
           "you run the engine with %s\n",
...
void PrintHeader() {
  ...
  vprint(0, "--- Compares reslists with actual game content tree to show "
            "unreferenced content and stats ---\n");
}

static bool ReferencedFileLessFunc(const ReferencedFile &lhs,
                                   const ReferencedFile &rhs) {
...
static bool RefFileLessFunc(const ReferencedFile &lhs,
                            const ReferencedFile &rhs) {
  char const *s1 = g_Analysis.symbols.String(lhs.sym);
  char const *s2 = g_Analysis.symbols.String(rhs.sym);
...
void Correlate(CUtlRBTree<ReferencedFile, int> &referencedfiles,
               CUtlVector<FileEntry> &contentfiles, const char *modname) {
  int i;
  ...
  // Analysis is to walk tree and see which files on disk are referenced in the
  // .lst files Need a fast lookup from file symbol to referenced list
  ...
  // Now walk the on disk file and see check off resources which are in
  // referenced
  ...
  while (i != invalidindex) {
    ...
    if (showreferencedfiles) {
      vprint(1, "%6i %12s: %s\n", ++index, Q_pretifymem(entry.size, 2),
  ...
  if (showmapfileusage) {
    ...
    // Now walk the list of referenced files and print out how many and which
    // maps reference them
    i = tree.FirstInorder();
    ...
    while (i != invalidindex) {
      ...
      char ext[32];
      Q_ExtractFileExtension(g_Analysis.symbols.String(entry.sym), ext,
                             sizeof(ext));

      logprint("referenced.csv", "\"%s\",\"%s\",%d",
               g_Analysis.symbols.String(entry.sym), ext,
               entry.maplist.Count());

      int mapcount = entry.maplist.Count();
      for (int j = 0; j < mapcount; ++j) {
        char basemap[128];
        Q_FileBase(g_Analysis.symbols.String(entry.maplist[j]), basemap,
                   sizeof(basemap));
  ...
  // Now build summaries by root branch off of gamedir (e.g., for sound,
  ...
  while (i != invalidindex) {
    ...
    Q_strncpy(unrefstring, Q_pretifymem(de.unreferenced, 2),
              sizeof(unrefstring));
    Q_strncpy(refstring, Q_pretifymem(remainder, 2), sizeof(refstring));
    Q_strncpy(whiteliststring, Q_pretifymem(de.whitelist, 2),
              sizeof(whiteliststring));

    vprint(
        0,
        "%15s (%8.3f%%)   %15s (%8.3f%%)   %15s (%8.3f%%)   %15s => dir: %s\n",
        refstring, 100.0f - percent_unref, whiteliststring, percent_white,
        unrefstring, percent_unref, totalstring, directories.GetElementName(i));
  ...
  Q_strncpy(unrefstring, Q_pretifymem(grand_total_unref, 2),
            sizeof(unrefstring));
  Q_strncpy(refstring, Q_pretifymem(grand_total - grand_total_unref, 2),
            sizeof(refstring));
  Q_strncpy(whiteliststring, Q_pretifymem(grand_total_white, 2),
            sizeof(whiteliststring));
  ...
  vprint(0, "%15s (%8.3f%%)   %15s (%8.3f%%)   %15s (%8.3f%%)   %15s\n",
         refstring, 100.0f - percent_unref, whiteliststring, percent_white,
         unrefstring, percent_unref, totalstring);
}
...
int main(int argc, char *argv[]) {
  ...
  vprint(0,
         "    Comparing results of resfile (%s) with files under current "
         "directory...\n",
         resfile);
  ...
  filesystem = (IFileSystem *)(CmdLib_GetFileSystemFactory()(
  ...
  filesystem->RemoveFile(CFmtStr("%sunreferenced_files.lst", g_szReslistDir),
                         "GAME");
  ...
  for (i = 0; i < c; ++i) {
    ...
    logprint(CFmtStr("%sunreferenced_files.lst", g_szReslistDir),
             "\"%s\\%s\"\n", modname, name);
  }

  // also include the files from deletions.bat, as we don't actually run that
  // now
  c = contentfiles.Count();
  for (i = 0; i < c; ++i) {
    FileEntry &entry = contentfiles[i];
