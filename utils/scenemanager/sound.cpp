    ...
    while (tempCount > 0) {
      ...
      for (int i = 0; i < MAX_CHANNELS; i++) {
        ...
        if (pSource->GetActive()) {
          if (!pSource->MixDataToDevice(&m_audioDevice, &channel, currentsample,
          ...
          if (!IsSourceReferencedByActiveBuffer(pSource)) {
            ...
            pSource->IncrementSamples(&channel, currentsample, sampleCount,
                                      SampleRate(), forward);
          }
        }
      }
...
void CSceneManagerSound::PlaySound(CAudioSource *source,
                                   CAudioMixer **ppMixer) {
  if (ppMixer) {
    *ppMixer = NULL;
  }
