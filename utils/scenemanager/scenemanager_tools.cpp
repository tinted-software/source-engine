...
void SceneManager_MakeToolWindow(mxWindow *w, bool smallcaption) {
  SceneManager_AddWindowStyle(w, WS_VISIBLE | WS_OVERLAPPEDWINDOW |
...
static void SceneManager_VSSCheckout(char const *pUserName,
                                     char const *pProjectDir,
                                     char const *pRelativeDir,
                                     char const *pDestPath,
                                     char const *pFileNameWithExtension) {
  char buf[1024];

  // Check for the existence of the file in source safe...
  sprintf(buf, "filetype %s/%s%s -O- -y%s\n", pProjectDir, pRelativeDir,
          pFileNameWithExtension, pUserName);
...
static void SceneManager_VSSCheckin(char const *pUserName,
                                    char const *pProjectDir,
                                    char const *pRelativeDir,
                                    char const *pDestPath,
                                    char const *pFileNameWithExtension) {
  ...
  if (retVal != 0) {
    ...
    sprintf(buf, "add %s%s -GL%s -O- -I- -y%s\n", pRelativeDir,
            pFileNameWithExtension, pDestPath, pUserName);
    Sys_Exec("ss.exe", buf, true);
  } else {
...
void VSS_Checkout(char const *name, bool updatestaticons /*= true*/) {
  ...
  SceneManager_VSSCheckout(ws->GetVSSUserName(), ws->GetVSSProject(), path,
                           va("%s%s", gamedir, path), filename);
...
void VSS_Checkin(char const *name, bool updatestaticons /*= true*/) {
  ...
  SceneManager_VSSCheckin(ws->GetVSSUserName(), ws->GetVSSProject(), path,
                          va("%s%s", gamedir, path), filename);
...
// Input  : store -
//-----------------------------------------------------------------------------
static void SceneManager_StoreValveDataChunk(CSentence &sentence,
...
bool SceneManager_SaveSentenceToWavFile(char const *wavfile,
                                        CSentence &sentence) {
  char tempfile[512];
...
void SceneManager_SaveWindowPositions(CUtlBuffer &buf, int indent,
                                      mxWindow *wnd) {
  int x, y, w, h;
