...
CStatusWindow::CStatusWindow(mxWindow *parent, int x, int y, int w, int h,
                             const char *label /*= 0*/)
...
void CStatusWindow::redraw() {
  ...
  for (int i = 0; i < MAX_TEXT_LINES - offset; i++) {
    ...
    COLORREF clr = RGB(m_rgTextLines[line].r, m_rgTextLines[line].g,
                       m_rgTextLines[line].b);
    ...
    helper.DrawColoredText("Arial", STATUS_FONT_SIZE, FW_NORMAL, clr, rcTime,
                           ptext);
...
void CStatusWindow::StatusPrint(int r, int g, int b, bool overwrite,
                                const char *text) {
...
int CStatusWindow::handleEvent(mxEvent *event) {
  ...
  default:
    break;
  case mxEvent::Size: {
    m_pScrollbar->setBounds(w2() - STATUS_SCROLLBAR_SIZE, 0,
                            STATUS_SCROLLBAR_SIZE, h2());
    PositionSliders(0);
    m_pScrollbar->setValue(m_pScrollbar->getMaxValue());
    iret = 1;
  } break;
