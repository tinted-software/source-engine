...
static void SoundProperties_Multiple_OnOK(HWND hwndDlg) {
  ...
  for (int i = 0; i < c; i++) {
    ...
    CSoundParametersInternal *baseparams =
        g_pSoundEmitterSystem->InternalGetParametersForSound(idx);
    if (!baseparams)
      return;
    ...
    bool owneronly = SendMessage(GetDlgItem(hwndDlg, IDC_OWNERONLY),
...
//-----------------------------------------------------------------------------
static BOOL CALLBACK SoundProperties_MultipleDialogProc(HWND hwndDlg, UINT uMsg,
                                                        WPARAM wParam,
                                                        LPARAM lParam) {
  switch (uMsg) {
  case WM_INITDIALOG: {
