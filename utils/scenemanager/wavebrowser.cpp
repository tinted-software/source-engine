...
enum { COL_WAV = 0, COL_DUCKED, COL_SENTENCE };

class CWaveList : public mxListView {
public:
  CWaveList(mxWindow *parent, int id = 0) : mxListView(parent, 0, 0, 0, 0, id) {
    // SendMessage ( (HWND)getHandle(), WM_SETFONT, (WPARAM) (HFONT)
    // GetStockObject (ANSI_FIXED_FONT), MAKELPARAM (TRUE, 0));
...
CWorkspaceManager *CWaveBrowser::GetManager() { return m_pManager; }
...
void SplitFileName(char const *in, char *path, int maxpath, char *filename,
                   int maxfilename);
...
void CWaveBrowser::OnSearch() { PopulateTree(GetSearchString(), true); }
