...
void CWorkspace::LoadFromFile() {
  ...
  if (kv->LoadFromFile(filesystem, m_szFile)) {
    for (KeyValues *proj = kv->GetFirstSubKey(); proj;
         proj = proj->GetNextKey()) {
      // Add named projects
      if (!Q_stricmp(proj->GetName(), "project")) {
        ...
        for (KeyValues *sub = proj->GetFirstSubKey(); sub;
             sub = sub->GetNextKey()) {
        ...
        if (!Q_stricmp(windowname, "workspace")) {
          SceneManager_LoadWindowPositions(proj,
                                           GetWorkspaceManager()->GetBrowser());
          continue;
        } else if (!Q_stricmp(windowname, "soundbrowser")) {
          SceneManager_LoadWindowPositions(
              proj, GetWorkspaceManager()->GetSoundBrowser());
          continue;
        } else if (!Q_stricmp(windowname, "wavebrowser")) {
          SceneManager_LoadWindowPositions(
              proj, GetWorkspaceManager()->GetWaveBrowser());
          continue;
...
static void SaveWindowPositions(CUtlBuffer &buf, char const *windowname,
                                mxWindow *wnd) {
  buf.Printf("\t\"window\"\n");
  buf.Printf("\t{\n");
...
void CWorkspace::SaveToFile() {
  ...
  SaveWindowPositions(buf, "soundbrowser",
                      GetWorkspaceManager()->GetSoundBrowser());
  SaveWindowPositions(buf, "wavebrowser",
                      GetWorkspaceManager()->GetWaveBrowser());

  buf.Printf("}\n");

  if (filesystem->FileExists(m_szFile) &&
      !filesystem->IsFileWritable(m_szFile)) {
    int retval = mxMessageBox(NULL, va("Check out '%s'?", m_szFile), g_appTitle,
                              MX_MB_YESNOCANCEL);
    if (retval != 0)
      return;
    ...
    if (!filesystem->IsFileWritable(m_szFile)) {
      mxMessageBox(NULL, va("Unable to check out'%s'!!!", m_szFile), g_appTitle,
                   MX_MB_OK);
      return;
    }
  }
...
void CWorkspace::ValidateTree(mxTreeView *tree, mxTreeViewItem *parent) {
  ...
  for (int i = 0; i < c; i++) {
    ...
    if (proj->GetComments() && proj->GetComments()[0]) {
      Q_snprintf(sz, sizeof(sz), "%s : %s", proj->GetName(),
                 proj->GetComments());
    } else {
...
bool CWorkspace::CanClose(void) {
  ...
  int retval =
      mxMessageBox(NULL, va("Save changes to workspace '%s'?", GetName()),
...
char const *CWorkspace::GetVSSUserName() const { return m_szVSSUserName; }

char const *CWorkspace::GetVSSProject() const { return m_szVSSProject; }
