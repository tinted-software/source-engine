...
public:
  int open(const char *pFileName) {
    char filename[512];
    Q_snprintf(filename, sizeof(filename), "%s%s",
               SceneManager_GetGameDirectory(), pFileName);
...
public:
  ...
  // Caller should call lock before accessing any of these methods and unlock
  // afterwards!!!
...
int CFileLoaderThread::DoThreadWork() {
  ...
  for (i = 0; i < workitems; i++) {
    ...
    r->valid = SceneManager_LoadSentenceFromWavFileUsingIO(
        r->filename, r->sentence, m_ThreadIO);
  }
...
void CFileLoaderThread::Start() {
  m_hThread = CreateThread(NULL, 0, FileLoaderThreadFunc, (void *)this, 0,
                           &m_nThreadId);
  Assert(m_hThread);
}
...
// Input  : *wavefile -
//-----------------------------------------------------------------------------
void CFileLoaderThread::AddWaveFilesToThread(
    CUtlVector<CWaveFile *> &wavefiles) {
  Lock();
  ...
  for (int i = 0; i < c; i++) {
    ...
    Q_strncpy(request->filename, request->wavefile->GetFileName(),
