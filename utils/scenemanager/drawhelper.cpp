...
CDrawHelper::CDrawHelper(mxWindow *widget, RECT &bounds) {
  Init(widget, bounds.left, bounds.top, bounds.right - bounds.left,
       bounds.bottom - bounds.top, COLOR_BACKGROUND);
}
...
//			h -
//-----------------------------------------------------------------------------
CDrawHelper::CDrawHelper(mxWindow *widget, int x, int y, int w, int h,
                         COLORREF bgColor) {
...
CDrawHelper::CDrawHelper(mxWindow *widget, RECT &bounds, COLORREF bgColor) {
  Init(widget, bounds.left, bounds.top, bounds.right - bounds.left,
       bounds.bottom - bounds.top, bgColor);
}
...
//			h -
//-----------------------------------------------------------------------------
void CDrawHelper::Init(mxWindow *widget, int x, int y, int w, int h,
                       COLORREF bgColor) {
  m_x = x;
  m_y = y;
...
// Output : int
//-----------------------------------------------------------------------------
int CDrawHelper::GetWidth(void) { return m_w; }

//-----------------------------------------------------------------------------
// Purpose:
// Output : int
//-----------------------------------------------------------------------------
int CDrawHelper::GetHeight(void) { return m_h; }
...
// Output : HDC
//-----------------------------------------------------------------------------
HDC CDrawHelper::GrabDC(void) { return m_dcMemory; }
...
//			... -
//-----------------------------------------------------------------------------
void CDrawHelper::CalcTextRect(const char *font, int pointsize, int weight,
                               int maxwidth, RECT &rcText, const char *fmt,
                               ...) {
  va_list args;
...
// Output : int
//-----------------------------------------------------------------------------
int CDrawHelper::CalcTextWidth(const char *font, int pointsize, int weight,
                               const char *fmt, ...) {
  va_list args;
  static char output[1024];
...
// Output : int
//-----------------------------------------------------------------------------
int CDrawHelper::CalcTextWidthW(const char *font, int pointsize, int weight,
                                const wchar_t *fmt, ...) {
  va_list args;
  ...
  DrawTextW(m_dcMemory, output, -1, &rcText,
            DT_LEFT | DT_NOPREFIX | DT_VCENTER | DT_SINGLELINE | DT_CALCRECT);
...
//			... -
//-----------------------------------------------------------------------------
void CDrawHelper::DrawColoredText(const char *font, int pointsize, int weight,
...
//			... -
//-----------------------------------------------------------------------------
void CDrawHelper::DrawColoredTextW(const char *font, int pointsize, int weight,
...
//			... -
//-----------------------------------------------------------------------------
void CDrawHelper::DrawColoredText(HFONT font, COLORREF clr, RECT &rcText,
                                  const char *fmt, ...) {
  va_list args;
  static char output[1024];
...
//			... -
//-----------------------------------------------------------------------------
void CDrawHelper::DrawColoredTextW(HFONT font, COLORREF clr, RECT &rcText,
                                   const wchar_t *fmt, ...) {
  va_list args;
  static wchar_t output[1024];
...
//			... -
//-----------------------------------------------------------------------------
void CDrawHelper::DrawColoredTextCharset(const char *font, int pointsize,
                                         int weight, DWORD charset,
                                         COLORREF clr, RECT &rcText,
                                         const char *fmt, ...) {
  va_list args;
  static char output[1024];
...
void CDrawHelper::DrawColoredTextCharsetW(const char *font, int pointsize,
                                          int weight, DWORD charset,
                                          COLORREF clr, RECT &rcText,
                                          const wchar_t *fmt, ...) {
  va_list args;
...
//			... -
//-----------------------------------------------------------------------------
void CDrawHelper::DrawColoredTextMultiline(const char *font, int pointsize,
...
//			y2 -
//-----------------------------------------------------------------------------
void CDrawHelper::DrawColoredLine(COLORREF clr, int style, int width, int x1,
...
//			*pts -
//-----------------------------------------------------------------------------
void CDrawHelper::DrawColoredPolyLine(COLORREF clr, int style, int width,
...
//			y2 -
//-----------------------------------------------------------------------------
POINTL CDrawHelper::DrawColoredRamp(COLORREF clr, int style, int width, int x1,
                                    int y1, int x2, int y2, float rate,
                                    float sustain) {
  HPEN pen = CreatePen(style, width, clr);
  HPEN oldPen = (HPEN)SelectObject(m_dcMemory, pen);
  MoveToEx(m_dcMemory, x1 - m_x, y1 - m_y, NULL);
...
//			rc -
//-----------------------------------------------------------------------------
void CDrawHelper::DrawOutlinedRect(COLORREF clr, int style, int width,
                                   RECT &rc) {
...
//			y2 -
//-----------------------------------------------------------------------------
void CDrawHelper::DrawOutlinedRect(COLORREF clr, int style, int width, int x1,
                                   int y1, int x2, int y2) {
  HPEN oldpen, pen;
...
//			thickness -
//-----------------------------------------------------------------------------
void CDrawHelper::DrawLine(int x1, int y1, int x2, int y2, COLORREF clr,
                           int thickness) {
  HPEN oldpen, pen;
  HBRUSH oldbrush, brush;
...
//			fillb -
//-----------------------------------------------------------------------------
void CDrawHelper::DrawTriangleMarker(RECT &rc, COLORREF fill,
                                     bool inverted /*= false*/) {
  POINT region[3];
  int cPoints = 3;
...
// Input  : rc -
//-----------------------------------------------------------------------------
void CDrawHelper::OffsetSubRect(RECT &rc) { OffsetRect(&rc, -m_x, -m_y); }
...
void CDrawHelper::DrawCircle(COLORREF clr, int x, int y, int radius,
                             bool filled /*= true*/) {
  RECT rc;
  rc.left = x - radius / 2;
...
//			vertical -
//-----------------------------------------------------------------------------
void CDrawHelper::DrawGradientFilledRect(RECT &rc, COLORREF clr1, COLORREF clr2,
                                         bool vertical) {
  RECT rcDraw = rc;
