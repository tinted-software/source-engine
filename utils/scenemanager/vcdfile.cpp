...
CScene *CVCDFile::GetOwnerScene() { return m_pOwner; }
...
CChoreoScene *CVCDFile::LoadScene(char const *filename) {
  if (filesystem->FileExists(filename)) {
    char fullpath[512];
    filesystem->RelativePathToFullPath(filename, "GAME", fullpath,
                                       sizeof(fullpath));
    LoadScriptFile(fullpath);
    CChoreoScene *scene =
        ChoreoLoadScene(filename, this, &g_TokenProcessor, Con_Printf);
    return scene;
  }
...
char const *CVCDFile::GetName() const { return m_szName; }
...
int CVCDFile::GetSoundEntryCount() const { return m_Sounds.Count(); }
...
void CVCDFile::ValidateTree(mxTreeView *tree, mxTreeViewItem *parent) {
  ...
  for (int i = 0; i < c; i++) {
    ...
    char sz[256];
    Q_snprintf(sz, sizeof(sz), "\"%s\" : script %s", sound->GetName(),
               sound->GetScriptFile());
...
bool CVCDFile::IsChildFirst(ITreeItem *child) { return false; }

bool CVCDFile::IsChildLast(ITreeItem *child) { return false; }
