...
static void PopulateScriptList(HWND wnd, char const *curscript) {
  ...
  for (int i = 0; i < c; i++) {
    ...
    SendMessage(control, CB_ADDSTRING, 0,
                (LPARAM)g_pSoundEmitterSystem->GetSoundScriptName(i));
...
static void PopulateWaveList(HWND wnd, CSoundParametersInternal *p) {
  ...
  for (int i = 0; i < c; i++) {
    char const *name = strdup(
        g_pSoundEmitterSystem->GetWaveName(p->GetSoundNames()[i].symbol));
...
static void SoundProperties_GetSelectedWaveList(HWND hwndDlg, HWND control,
                                                CUtlVector<int> &list) {
  if (!control)
    return;
...
static const char *SoundProperties_GetSelectedWave(HWND hwndDlg, HWND control,
                                                   int *item = NULL) {
  if (item)
    *item = -1;
...
static void SoundProperties_OnOK(HWND hwndDlg) {
  ...
  GetDlgItemText(hwndDlg, IDC_SOUNDSCRIPT, outscriptfile,
                 sizeof(outscriptfile));
  ...
  bool owneronly = SendMessage(GetDlgItem(hwndDlg, IDC_OWNERONLY), BM_GETCHECK,
                               0, 0) == BST_CHECKED
                       ? true
                       : false;
  ...
  for (int i = 0; i < c; i++) {
    char wavename[256];
    SendMessage(GetDlgItem(hwndDlg, IDC_WAVELIST), LB_GETTEXT, i,
                (LPARAM)wavename);
...
//-----------------------------------------------------------------------------
static BOOL CALLBACK SoundPropertiesDialogProc(HWND hwndDlg, UINT uMsg,
  ...
  case WM_COMMAND:
    ...
    case IDC_WAVELIST: {
      if (HIWORD(wParam) == LBN_SELCHANGE) {
        ...
        char const *wave = SoundProperties_GetSelectedWave(
            hwndDlg, GetDlgItem(hwndDlg, IDC_WAVELIST));
        if (wave) {
          // Look it up
          CWaveBrowser *wb = GetWorkspaceManager()->GetWaveBrowser();
          if (wb) {
      ...
      if (wave) {
        ...
        if (wb) {
          ...
          if (wavefile) {
            ...
            Q_snprintf(wp.m_szDialogTitle, sizeof(wp.m_szDialogTitle),
                       "Wave Properties");
    ...
    case IDC_REMOVEWAVE: {
      ...
      SoundProperties_GetSelectedWaveList(
          hwndDlg, GetDlgItem(hwndDlg, IDC_WAVELIST), selected);
      ...
      if (count >= 1) {
        ...
        for (i = 0; i < count; i++) {
          ...
          SendMessage(GetDlgItem(hwndDlg, IDC_WAVELIST), LB_GETTEXT,
                      selected[i], (LPARAM)wavename);
        ...
        for (i = count - 1; i >= 0; i--) {
          ...
          SendMessage(GetDlgItem(hwndDlg, IDC_WAVELIST), LB_DELETESTRING,
                      selected[i], 0);
        }
      }
    } break;
    case IDC_ADDWAVE: {
      CUtlVector<int> selected;
      SoundProperties_GetSelectedWaveList(
          hwndDlg, GetDlgItem(hwndDlg, IDC_WAVELIST_AVAILABLE), selected);
      int count = selected.Count();
      if (count >= 1) {
        int i;
        for (i = 0; i < count; i++) {
          ...
          SendMessage(GetDlgItem(hwndDlg, IDC_WAVELIST_AVAILABLE), LB_GETTEXT,
                      selected[i], (LPARAM)wavename);
