...
CSoundEntry::CSoundEntry(CVCDFile *vcd, char const *name) : m_pOwner(vcd) {
  ...
  if (g_pSoundEmitterSystem->IsValidIndex(slot)) {
    Q_FileBase(g_pSoundEmitterSystem->GetSourceFileForSound(slot), filebase,
               sizeof(filebase));
    ...
    if (p) {
      ...
      for (int wave = 0; wave < waveCount; wave++) {
        char const *wavname =
            g_pSoundEmitterSystem->GetWaveName(p->GetSoundNames()[wave].symbol);
        if (wavname) {
...
CSoundEntry::~CSoundEntry() { m_Waves.RemoveAll(); }

CVCDFile *CSoundEntry::GetOwnerVCDFile() { return m_pOwner; }

int CSoundEntry::GetWaveCount() const { return m_Waves.Count(); }
...
void CSoundEntry::ClearWaves() { m_Waves.RemoveAll(); }
...
void CSoundEntry::RemoveWave(CWaveFile *wave) { m_Waves.FindAndRemove(wave); }
...
char const *CSoundEntry::GetName() const { return m_szName; }
...
void CSoundEntry::ValidateTree(mxTreeView *tree, mxTreeViewItem *parent) {
  ...
  for (int i = 0; i < c; i++) {
    ...
    if (wave->HasLoadedSentenceInfo()) {
      Q_snprintf(sz, sizeof(sz), "\"%s\" : '%s'", wave->GetName(),
                 wave->GetSentenceText());
    } else {
      Q_snprintf(sz, sizeof(sz), "\"%s\" : '%s'", wave->GetName(),
                 "(loading...)");
    }
...
char const *CSoundEntry::GetScriptFile() const { return m_szScriptFile; }
...
bool CSoundEntry::IsChildFirst(ITreeItem *child) { return false; }

bool CSoundEntry::IsChildLast(ITreeItem *child) { return false; }