...
int CALLBACK CWorkspaceBrowser::CompareFunc(LPARAM lParam1, LPARAM lParam2,
                                            LPARAM lParamSort) {
...
// Input  : *parent -
//-----------------------------------------------------------------------------
CWorkspaceBrowser::CWorkspaceBrowser(mxWindow *parent,
                                     CWorkspaceManager *manager, int id)
...
// Output : CWorkspace
//-----------------------------------------------------------------------------
CWorkspace *CWorkspaceBrowser::GetWorkspace() { return m_pCurrentWorkspace; }
...
void CWorkspaceBrowser::PopulateTree() {
  ...
  if (w->GetProjectCount() == 1) {
    Q_snprintf(sz, sizeof(sz), "Workspace '%s': %i project", w->GetName(),
               w->GetProjectCount());
  } else {
    Q_snprintf(sz, sizeof(sz), "Workspace '%s': %i projects", w->GetName(),
               w->GetProjectCount());
  }
...
ITreeItem *CWorkspaceBrowser::GetSelectedItem() { return m_pLastSelected; }

CWorkspaceManager *CWorkspaceBrowser::GetManager() { return m_pManager; }

void CWorkspaceBrowser::OnTreeItemSelected(int x, int y, bool rightmouse,
                                           bool doubleclick) {
