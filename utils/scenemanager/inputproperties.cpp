...
//-----------------------------------------------------------------------------
static BOOL CALLBACK InputPropertiesDialogProc(HWND hwndDlg, UINT uMsg,
  ...
  case WM_INITDIALOG:
    ...
    // ...
    {
      ...
      SendMessage(GetDlgItem(hwndDlg, IDC_INPUTSTRING), EM_SETSEL, 0,
                  MAKELONG(0, 0xffff));
    }
    return FALSE;
  ...
  case WM_COMMAND:
    ...
    case IDOK:
      ...
      GetDlgItemText(hwndDlg, IDC_INPUTSTRING, g_Params.m_szInputText,
                     sizeof(g_Params.m_szInputText));
      EndDialog(hwndDlg, 1);
      break;
    case IDCANCEL:
      EndDialog(hwndDlg, 0);
...
int InputProperties(CInputParams *params) {
  ...
  int retval =
      DialogBox((HINSTANCE)GetModuleHandle(0), MAKEINTRESOURCE(IDD_INPUTDIALOG),
