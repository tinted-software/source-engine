...
public:
  ...
  CSoundFilterTab(mxWindow *parent, int x, int y, int w, int h, int id = 0,
                  int style = 0)
  ...
  void UpdatePrefixes() {
    ...
    for (int i = 2; i < c; i++) {
      ...
      int scriptindex =
          g_pSoundEmitterSystem->FindSoundScript(va("scripts/%s.txt", script));
      if (scriptindex < 0)
        continue;
...
public:
  ...
  COptionsWindow(CSoundBrowser *browser)
    ...
    m_pChanVoiceOnly =
        new mxCheckBox(this, 0, 0, 0, 0, "CHAN_VOICE only", IDC_VOICE_ONLY);
  ...
  bool IsChanVoiceOnly() const { return m_pChanVoiceOnly->isChecked(); }
...
// Input  : *parent -
//-----------------------------------------------------------------------------
CSoundBrowser::CSoundBrowser(mxWindow *parent, CWorkspaceManager *manager,
                             int id)
...
void CSoundBrowser::LoadAllSounds() {
  ...
  int i;
  for (i = g_pSoundEmitterSystem->First();
       i != g_pSoundEmitterSystem->InvalidIndex();
    ...
    char filebase[512];
    Q_FileBase(g_pSoundEmitterSystem->GetSourceFileForSound(i), filebase,
               sizeof(filebase));
...
void CSoundBrowser::PopulateTree(bool voiceonly, char const *scriptonly) {
  ...
  for (i = m_Sorted.FirstInorder(); i != m_Sorted.InvalidIndex();
    ...
    if (waveCount >= 1) {
      m_pListView->setLabel(slot, COL_COUNT,
                            waveCount > 1 ? va("%i", waveCount) : "");
      m_pListView->setLabel(slot, COL_WAV,
                            g_pSoundEmitterSystem->GetWaveName(
                                params->GetSoundNames()[0].symbol));
    }
    ...
    Q_FileBase(g_pSoundEmitterSystem->GetSourceFileForSound(soundIndex),
               filebase, sizeof(filebase));
...
void CSoundBrowser::OnAddSound() {
  ...
  if (params.items.Count() == 1) {
    ...
    if (g_pSoundEmitterSystem->IsValidIndex(slot)) {
      CSoundParametersInternal *p =
          g_pSoundEmitterSystem->InternalGetParametersForSound(slot);
      if (p) {
        CWaveBrowser *wb = GetWorkspaceManager()->GetWaveBrowser();
        Assert(wb);
