...
CWorkspace *CProject::GetOwnerWorkspace() { return m_pOwner; }

char const *CProject::GetName() const { return m_szName; }

char const *CProject::GetFileName() const { return m_szFile; }

bool CProject::IsDirty(void) const { return m_bDirty; }

void CProject::SetDirty(bool dirty) { m_bDirty = dirty; }
...
int CProject::GetSceneCount() const { return m_Scenes.Count(); }
    ...
    for (KeyValues *s = kv->GetFirstSubKey(); s; s = s->GetNextKey()) {
      ...
      for (KeyValues *sub = s->GetFirstSubKey(); sub; sub = sub->GetNextKey()) {
        ...
        if (!Q_stricmp(sub->GetName(), "vcd")) {
          ...
          for (KeyValues *vcdKeys = sub->GetFirstSubKey(); vcdKeys;
               vcdKeys = vcdKeys->GetNextKey()) {
...
void CProject::ValidateTree(mxTreeView *tree, mxTreeViewItem *parent) {
  ...
  for (int i = 0; i < c; i++) {
    ...
    char sz[256];
    if (scene->GetComments() && scene->GetComments()[0]) {
      Q_snprintf(sz, sizeof(sz), "%s : %s", scene->GetName(),
                 scene->GetComments());
    } else {
