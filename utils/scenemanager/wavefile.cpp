...
//-----------------------------------------------------------------------------
bool CWaveFile::HasLoadedSentenceInfo() const { return m_bSentenceLoaded; }

CVCDFile *CWaveFile::GetOwnerVCDFile() { return m_pOwner; }

CSoundEntry *CWaveFile::GetOwnerSoundEntry() { return m_pOwnerSE; }
...
char const *CWaveFile::GetName() const { return m_szName; }

char const *CWaveFile::GetFileName() const { return m_szFileName; }
...
bool CWaveFile::IsChildFirst(ITreeItem *child) { return false; }

bool CWaveFile::IsChildLast(ITreeItem *child) { return false; }
...
void CWaveFile::ExportValveDataChunk(char const *tempfile) {
  ...
  if (m_Sentence.m_Words.Count() <= 0) {
    Con_ColorPrintf(
        ...
        "CWaveFile::ExportValveDataChunk:  Sentence has no word data\n");
    return;
  }
  ...
  if (!fh) {
    Con_ColorPrintf(ERROR_R, ERROR_G, ERROR_B,
                    "CWaveFile::ExportValveDataChunk:  Unable to write to %s "
                    "(read-only?)\n",
                    tempfile);
    return;
  } else {
...
void CWaveFile::ImportValveDataChunk(char const *tempfile) {
  ...
  if (!fh) {
    Con_ColorPrintf(
        ERROR_R, ERROR_G, ERROR_B,
        "CWaveFile::ImportValveDataChunk:  Unable to read from %s\n", tempfile);
    return;
  }
  ...
  if (len <= 4) {
    Con_ColorPrintf(ERROR_R, ERROR_G, ERROR_B,
                    "CWaveFile::ImportValveDataChunk:  File %s has length 0\n",
                    tempfile);
    return;
  }
