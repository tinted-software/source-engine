...
public:
  CWorkspaceWorkArea(mxWindow *parent) : mxWindow(parent, 0, 0, 0, 0, "") {
    SceneManager_AddWindowStyle(this,
...
CWorkspaceManager::CWorkspaceManager()
...

  {
    ...
    m_pMenuCloseCaptionLanguages->setChecked(
        IDC_WSM_OPTIONS_LANGUAGESTART + CC_ENGLISH, true);
    ...
    if (filesystem->FileExists(scriptname)) {
      if (!filesystem->IsFileWritable(scriptname)) {
        ...
        if (retval == 0) {
          ...
          if (!filesystem->IsFileWritable(scriptname)) {
            mxMessageBox(
                NULL, va("Aborting shutdown, %s, not writable!!", scriptname),
                g_appTitle, MX_MB_OK);
            return false;
          }
        } else if (retval == 1) {
      ...
      if (filesystem->IsFileWritable(scriptname)) {
        int retval =
            mxMessageBox(NULL, va("Save changes to out '%s'?", scriptname),
...
void CWorkspaceManager::UpdateMenus() {
  ...
  if (hasproject) {
    m_pMenuBar->modify(
        IDC_WSM_FILE_SCENE_NEW, IDC_WSM_FILE_SCENE_NEW,
...
int CWorkspaceManager::GetMaxRecentFiles() const {
  return IDC_WSM_FILE_RECENT_WORKSPACE_END -
         IDC_WSM_FILE_RECENT_WORKSPACE_START;
}
...
void CWorkspaceManager::PerformLayout(bool movebrowsers) {
  ...
  if (movebrowsers) {
    ...
    if (GetSoundBrowser()) {
      GetSoundBrowser()->setBounds(0, m_pWorkArea->h2() / 3, m_pWorkArea->w2(),
                                   m_pWorkArea->h2() / 3);
    }
    if (GetWaveBrowser()) {
      GetWaveBrowser()->setBounds(0, 2 * m_pWorkArea->h2() / 3,
                                  m_pWorkArea->w2(), m_pWorkArea->h2() / 3);
    }
  }
...
void CWorkspaceManager::OnUpdateTitle(void) {
  ...
  if (ws) {
    ...
    Q_snprintf(szTitle, sizeof(szTitle), fmt, ws->GetName(),
               CSentence::NameForLanguage(m_nLanguageId));
  } else {
...
void CWorkspaceManager::SaveRecentFilesMenuToDisk() {
  ...
  if (fh) {
    ...
    Con_Printf(
        "CWorkspace::SaveRecentFilesMenuToDisk:  Unable to write file %s!!!\n",
        recentfiles);
  }
}
...
void CWorkspaceManager::UpdateRecentFilesMenu() {
  ...
  while (c > m_nRecentMenuItems) {
    m_pRecentFileMenu->add("(empty)", IDC_WSM_FILE_RECENT_WORKSPACE_START +
                                          m_nRecentMenuItems++);
  }

  for (int i = 0; i < c; i++) {
    m_pMenuBar->modify(IDC_WSM_FILE_RECENT_WORKSPACE_START + i,
                       IDC_WSM_FILE_RECENT_WORKSPACE_START + i,
  ...
  if (!ws) {
    ...
    if (!filesystem->FileExists(workspace_name)) {
      int retval = mxMessageBox(
          NULL, va("Automatically create workspace %s?", workspace_name),
          g_appTitle, MX_MB_YESNOCANCEL);
      if (retval != 0) {
...
void CWorkspaceManager::OnNewScene() {
  ...
  Q_snprintf(params.m_szDialogTitle, sizeof(params.m_szDialogTitle),
             "Create scene in '%s'", project->GetName());
...
void CWorkspaceManager::OnModifyProjectComments() {
  ...
  memset(&params, 0, sizeof(params));
  Q_snprintf(params.m_szDialogTitle, sizeof(params.m_szDialogTitle),
             "Edit comments for '%s'", project->GetName());
  Q_strcpy(params.m_szPrompt, "Comments:");
  Q_strncpy(params.m_szInputText, project->GetComments(),
            sizeof(params.m_szInputText));
...
void CWorkspaceManager::OnChangeLanguage(int lang_index,
                                         bool force /* = false */) {
  ...
  if (changed || force) {
    ...
    if (Q_stricmp(suffix, "unknown_language")) {
      ...
      if (filesystem->FileExists(fn)) {
        ...
        Con_Printf("CWorkspaceManager::OnChangeLanguage  Warning, can't find "
                   "localization file %s\n",
                   fn);
      }
    }
...
void CWorkspaceManager::OnModifySceneComments() {
  ...
  memset(&params, 0, sizeof(params));
  Q_snprintf(params.m_szDialogTitle, sizeof(params.m_szDialogTitle),
             "Edit comments for '%s'", scene->GetName());
  Q_strcpy(params.m_szPrompt, "Comments:");
  Q_strncpy(params.m_szInputText, scene->GetComments(),
            sizeof(params.m_szInputText));
...
void CWorkspaceManager::OnModifyVCDComments() {
  ...
  memset(&params, 0, sizeof(params));
  Q_snprintf(params.m_szDialogTitle, sizeof(params.m_szDialogTitle),
             "Edit comments for '%s'", file->GetName());
  Q_strcpy(params.m_szPrompt, "Comments:");
  Q_strncpy(params.m_szInputText, file->GetComments(),
            sizeof(params.m_szInputText));
...
void CWorkspaceManager::ShowContextMenu_Workspace(int x, int y,
                                                  CWorkspace *ws) {
  if (!ws)
    return;
...
void CWorkspaceManager::ShowContextMenu_Project(int x, int y,
                                                CProject *project) {
  ...
  if (!project->IsFirstChild() || !project->IsLastChild()) {
    ...
    if (!project->IsFirstChild()) {
      pop->add(va("Move '%s' Up", project->GetName()),
               IDC_WSM_SELECTION_MOVEUP);
    }
    if (!project->IsLastChild()) {
      pop->add(va("Move '%s' Down", project->GetName()),
               IDC_WSM_SELECTION_MOVEDOWN);
    }
  }
  pop->addSeparator();
  ...
  if (!filesystem->IsFileWritable(project->GetFileName())) {
    pop->add(va("Checkout '%s'", project->GetName()),
             IDC_WSM_SELECTION_CHECKOUT);
  } else {
...
void CWorkspaceManager::ShowContextMenu_Scene(int x, int y, CScene *scene) {
  ...
  if (!scene->IsFirstChild() || !scene->IsLastChild()) {
    ...
    if (!scene->IsLastChild()) {
      pop->add(va("Move '%s' Down", scene->GetName()),
               IDC_WSM_SELECTION_MOVEDOWN);
    }
  }
...
void CWorkspaceManager::ShowContextMenu_VCD(int x, int y, CVCDFile *vcd) {
  ...
  if (!vcd->IsFirstChild() || !vcd->IsLastChild()) {
    ...
    if (!vcd->IsLastChild()) {
      pop->add(va("Move '%s' Down", vcd->GetName()),
               IDC_WSM_SELECTION_MOVEDOWN);
    }
  }
  pop->addSeparator();
...
void CWorkspaceManager::ShowContextMenu_SoundEntry(int x, int y,
                                                   CSoundEntry *entry) {
  // New scene, edit comments
  mxPopupMenu *pop = new mxPopupMenu();
...
void CWorkspaceManager::ShowContextMenu_WaveFile(int x, int y,
                                                 CWaveFile *entry) {
  // New scene, edit comments
  mxPopupMenu *pop = new mxPopupMenu();
...
HIMAGELIST CWorkspaceManager::CreateImageList() {
  ...
  hicon =
      LoadIcon(GetModuleHandle(0), MAKEINTRESOURCE(IDI_WORKSPACE_CHECKEDOUT));
  slot = ImageList_AddIcon(list, hicon);
  Assert(slot == c++);
  DeleteObject(hicon);
  ...
  //	hicon = LoadIcon(GetModuleHandle( 0 ),
  //MAKEINTRESOURCE(IDI_SCENE_CHECKEDOUT)); 	slot = ImageList_AddIcon(list,
  //hicon); 	Assert( slot == c++ ); 	DeleteObject( hicon );
