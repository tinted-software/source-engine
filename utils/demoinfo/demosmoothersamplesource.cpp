...
//
// Purpose: 00000000000000Sample Source Code for "demo smoothing" tool in the
// engine.  This could be ported into the client .dll
//  pretty easily -- ywb
//
//
// $NoKeywords: $
...
void CDemoSmootherPanel::DrawSmoothingSample(bool original, bool processed,
  ...
  if (original) {
    Draw_Line(sample->info.viewOrigin + m_vecEyeOffset,
              next->info.viewOrigin + m_vecEyeOffset, 180, 180, 180, false);
  ...
  if (processed && sample->info.flags != 0) {
    Draw_Line(sample->info.GetViewOrigin() + m_vecEyeOffset,
              next->info.GetViewOrigin() + m_vecEyeOffset, 255, 255, 180,
              false);
  ...
  if (sample->samplepoint) {
    ...
    Draw_Box(sample->vecmoved + m_vecEyeOffset, Vector(-2, -2, -2),
             Vector(2, 2, 2), sample->angmoved, r, g, b, 127);
  ...
  if (sample->targetpoint) {
    ...
    Draw_Box(sample->vectarget, Vector(-2, -2, -2), Vector(2, 2, 2), vec3_angle,
             r, g, b, 127);
  }
  ...
  if (samplenumber == m_nPreviewLastFrame + 1) {
    ...
    g = 100;
    b = 250;
    Draw_Box(sample->info.GetViewOrigin() + m_vecEyeOffset, Vector(-2, -2, -2),
             Vector(2, 2, 2), sample->info.GetViewAngles(), r, g, b, 92);
  }
  ...
  if (sample->targetpoint) {
    ...
    Draw_Line(sample->info.GetViewOrigin() + m_vecEyeOffset, sample->vectarget,
              r, g, b, false);
  }
}
...
demosmoothing_t *
CDemoSmootherPanel::GetBoundedSample(CUtlVector<demosmoothing_t *> &points,
                                     int sample) {
  int c = points.Count();
  if (sample < 0)
    return points[0];
...
void CDemoSmootherPanel::ParseSmoothingInfo(
    CDemoFile &demoFile, CUtlVector<demosmoothing_t> &smooth) {
  democmdinfo_t info;
  int dummy;
