...
void getMiniDumpHandles(char *pszQuery, const char *errorid,
                        CUtlVector<HANDLE> *pMiniDumpHandles) {
  char rgchQueryBuf[1024];

  Q_snprintf(rgchQueryBuf, sizeof(rgchQueryBuf), pszQuery);
  IResultSet *results =
      g_pSqlWrapper->PResultSetQuery(rgchQueryBuf); // do the query
  Assert(results != NULL);
  ...
  for (int i = 0; i < results->GetCSQLRow(); i++) {
    ...
    HANDLE hFile = CreateFile(newPath, GENERIC_READ, FILE_SHARE_READ, NULL,
...
void errorsToListPanel(vgui::ListPanel *pTokenList, char *pszQuery) {
  ...
  for (int i = 0; i < results->GetCSQLRow(); i++) {
    ...
    KeyValues *kv =
        new KeyValues(keyNameBuf, "errorid", errorbuf, "module", module);
