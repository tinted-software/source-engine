...
CMDModulePanel::~CMDModulePanel(void) { ReleaseDebugEngine(); }
...
void CMDModulePanel::Create(const char *filename) {
  if (g_pFullFileSystem->FileExists(filename)) {
    ...
    CMiniDumpObject *newMDObj =
        new CMiniDumpObject(filename, &m_knownModuleList);
...
void SeparateVersion(const char *version, char *v1buf, char *v2buf, char *v3buf,
                     char *v4buf) {
  const char *endV1 = strchr(version, '.') + 1;
  const char *endV2 = strchr(endV1 + 1, '.') + 1;
  const char *endV3 = strchr(endV2 + 1, '.') + 1;
  _mbsnbcpy((unsigned char *)v1buf, (const unsigned char *)version,
            endV1 - version);
  v1buf[endV1 - version - 1] = 0;
  _mbsnbcpy((unsigned char *)v2buf, (const unsigned char *)endV1,
            endV2 - endV1);
  v2buf[endV2 - endV1 - 1] = 0;
  _mbsnbcpy((unsigned char *)v3buf, (const unsigned char *)endV2,
            endV3 - endV2);
...
void CMDModulePanel::UpdateKnownDB(char *type) {
  ...
  if (kv) {
    ...
    SeparateVersion(((KeyValues *)kv)->GetString("version"), v1buf, v2buf,
                    v3buf, v4buf);
    if (key == 0) {
      ...
      if (!Q_strcmp(type, "UNKNOWN")) {
        ...
        char query[1024];
        sprintf(query,
                "select * from knownmodules where name = \"%s\" and version1 = "
                "%s and version2 = %s and version3 = %s and version4 = %s;",
                name, v1buf, v2buf, v3buf, v4buf);
        IResultSet *results =
            g_pSqlWrapper->PResultSetQuery(query); // do the query
        if (!results) {
          return;
        }
        ...
        if (numResults > 0) {
          ...
          if (!Q_strcmp(realType, type)) {
            // this user was out of sync with the database.  It doesn't actually
            // need updating.
            return;
          } else {
            char update[1024];
            sprintf(update,
                    "update knownmodules set type=\"%s\" where id = %i;", type,
                    realKey);
          ...
          char update[1024];
          sprintf(update,
                  "insert into knownmodules set name = \"%s\", version1 = %s, "
                  "version2 = %s, version3 = %s, version4 = %s, type = \"%s\";",
      ...
      if (numResults > 0) {
        ...
        Assert(row != NULL);
        Assert(numResults == 1);
        Assert(!Q_stricmp(name, row->PchData(1)) &&
               atoi(v1buf) == row->NData(2) && atoi(v2buf) == row->NData(3) &&
        ...
        char update[1024];
        sprintf(update, "update knownmodules set type=\"%s\" where id = %i;",
                type, realKey);
        g_pSqlWrapper->BInsert(update);

      } else {
        // the module entry was mis-keyed.  First, check for an existing entry
        // of this module.
        char query[1024];
        sprintf(query,
                "select * from knownmodules where name = \"%s\" and version1 = "
        ...
        int numResults = results->GetCSQLRow();
        if (numResults > 0) {
          // there is an existing entry.  Update its type and update the key for
          // this keyvalue;
          const ISQLRow *row = results->PSQLRowNextResult();
          int realKey = row->NData(0);
          ((KeyValues *)kv)->SetInt("key", realKey);
...
void CMDModulePanel::OnCompare(KeyValues *data) {
  ...
  CUtlVector<HANDLE> *pMiniDumpHandles =
      (CUtlVector<HANDLE> *)(void *)data->GetInt("handlePointer");

  DWORD error;
  int returnValue = 0;
  for (int i = 0; i < pMiniDumpHandles->Count(); i++) {
    m_MiniDumpList.AddToTail(
        new CMiniDumpObject(pMiniDumpHandles->Element(i), &m_knownModuleList));
...
void CMDModulePanel::LoadKnownModules() {
  ...
  if (!results) {
    ...
    VGUIMessageBox(GetParent(), "Error",
                   "Unable to retrieve known modules from database");
    return;
  }
...
void CMDModulePanel::InitializeDebugEngine(void) {
  ...
  if (S_OK == DebugCreate(__uuidof(IDebugClient), (void **)&m_pDbgClient)) {
    ...
    if (m_pDbgSymbols) {
      ...
      ExpandEnvironmentStrings("%_NT_SYMBOL_PATH%", szSymbolSrv,
                               sizeof(szSymbolSrv));
      if (!Q_stricmp("%_NT_SYMBOL_PATH%", szSymbolSrv)) {
        ivgui()->DPrintf("Setting symbol server");
        Q_strcpy(szSymbolSrv, "SRV*c:\\localsymbols*\\\\perforce\\symbols*http:"
...
void CMDModulePanel::AnalyzeDumpFile(const char *pszDumpFile) {
  if (m_pDbgClient && m_pDbgControl) {
    ...
    DWORD dwThreadId = 0;
    m_hThread = CreateThread(NULL, 0, StaticAnalyzeThread, (LPVOID)this, 0,
                             &dwThreadId);
  }
}
...
DWORD CMDModulePanel::AnalyzeThread(void) {
  if (m_pDbgControl) {
    ...
    m_pDbgControl->Execute(DEBUG_OUTCTL_THIS_CLIENT, "!analyze -v",
