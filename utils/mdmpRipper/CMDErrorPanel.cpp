...
void CMDErrorPanel::CheckModules() {
  char sql[255] = "";
  extern void getMiniDumpHandles(char *pszQuery, const char *errorid,
  ...
  if (kv) {
    ...
    strcat(sql, ";");
    getMiniDumpHandles(sql, ((KeyValues *)kv)->GetString("errorid", ""),
                       &m_MiniDumpHandles);
    KeyValues *kv =
        new KeyValues("Compare", "handlePointer", (int)(&m_MiniDumpHandles));
    this->PostActionSignal(kv);
  }
}
...
void CMDErrorPanel::NewQuery() {
  ...
  char sql[255] = "";
  char temp[10];
  strcat(sql, "select errorid, module, count, minidumpsonhand from error_types "
              "where processed=0 and minidumpsonhand > 0 and count > ");
...
void CMDErrorPanel::DetailScreen() {
  ...
  void *kv = m_pTokenList->GetItem(selectedIndex);
  if (kv) {
    KeyValues *kvPost = new KeyValues(
        "Detail", "errorID", ((KeyValues *)kv)->GetString("errorid", ""));
