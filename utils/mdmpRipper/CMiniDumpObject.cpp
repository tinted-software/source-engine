...
CMiniDumpObject::CMiniDumpObject(const char *pszFilename,
                                 CUtlVector<module> *pKnownModuleList) {
  InitFromFilename(pszFilename, pKnownModuleList);
}

CMiniDumpObject::CMiniDumpObject(HANDLE pMiniDumpHandle,
                                 CUtlVector<module> *pKnownModuleList) {
  InitFromHandle(pMiniDumpHandle, pKnownModuleList);
}

void CMiniDumpObject::Init(HANDLE pszFileMap,
                           CUtlVector<module> *pKnownModuleList) {
  ...
  MiniDumpReadDumpStream(pMiniDump, ModuleListStream, &miniDumpDir, &pOutput,
                         &ulSize);

  int numberOfModules = ((int *)pOutput)[0];
  MINIDUMP_MODULE *pModules =
      (MINIDUMP_MODULE *)((byte *)pOutput + sizeof(int));
  ...
  for (int i = 0; i < numberOfModules; i++) {
    ...
    for (int j = 0; j < pKnownModuleList->Count(); j++) {
      if (!Q_stricmp(strrchr(nameBuf, '\\') + 1,
                     pKnownModuleList->Element(j).name)) {
        newModule->myType = pKnownModuleList->Element(j).myType;

        if (newModule->versionInfo ==
            pKnownModuleList->Element(j).versionInfo) {
...
void CMiniDumpObject::InitFromHandle(HANDLE pMiniDumpHandle,
                                     CUtlVector<module> *pKnownModuleList) {
  if (INVALID_HANDLE_VALUE != pMiniDumpHandle) {
    ...
    if (ReadFile(pMiniDumpHandle, (LPVOID)szMdmp, 4, &dwNumRead, NULL)) {
      if (!Q_strcmp("MDMP", szMdmp)) {
        HANDLE hFileMap =
            CreateFileMapping(pMiniDumpHandle, NULL, PAGE_READONLY, 0, 0, NULL);
...
void CMiniDumpObject::InitFromFilename(const char *pszFilename,
                                       CUtlVector<module> *pKnownModuleList) {
...
int CMiniDumpObject::ModuleListToListPanel(vgui::ListPanel *pTokenList,
                                           CUtlVector<module> *pModuleList,
                                           bool bCumulative,
                                           int startingModule) {
  ...
  for (int i = 0; i < pModuleList->Count(); i++) {
    ...
    if (bCumulative) {
      ...
      for (int j = 0; j < tokenCount; j++) {
        ...
        if (!stricmp(moduleName, secondModuleName) &&
            !strcmp(kv->GetString("version"), version)) {
          int count = kv->GetInt("count");
          int key = kv->GetInt("key");
          KeyValues *newKv = new KeyValues(
              keyNameBuf, "name", kv->GetString("name"), "checksum", checksum);
...
void CMiniDumpObject::PopulateListPanel(vgui::ListPanel *pTokenList,
                                        bool bCumulative) {
  int nextModuleNumber = 0;
  nextModuleNumber = ModuleListToListPanel(pTokenList, &m_goodModuleList,
                                           bCumulative, nextModuleNumber);
  nextModuleNumber = ModuleListToListPanel(pTokenList, &m_unknownModuleList,
                                           bCumulative, nextModuleNumber);
  nextModuleNumber = ModuleListToListPanel(pTokenList, &m_badModuleList,
                                           bCumulative, nextModuleNumber);
  ModuleListToListPanel(pTokenList, &m_badChecksumList, bCumulative,
                        nextModuleNumber);
}
