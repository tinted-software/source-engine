...
void printusage() {
  ...
  printf("The directory containing the <modelname>.obj file must also contain "
         "a\n");
  printf("<modelname>.mtl file.\n");
}

void CreateTemplateQC(const char *pcDirectory, const char *pcBaseName,
  ...
  V_snprintf(sz_Buffer, MAX_PATH, "$cdmaterials \"models\\contrib\\%s\\%s\"\n",
             pcSteamName, pcBaseName);
  fputs(sz_Buffer, fp);
  V_snprintf(sz_Buffer, MAX_PATH,
             "$sequence \"idle\" \"%s.obj\" fps 30 numframes 30 loop\n",
             pcBaseName);
  fputs(sz_Buffer, fp);
...
void CreateTemplateVMT(const char *pcDirectory, const char *pcBaseName,
  ...
  /*	fputs( "\"UnlitGeneric\"\n",  fp );
          fputs( "{\n",  fp );
          V_StripExtension( pcTGAColorFile, szTGAColorFileBase, sizeof(
     szTGAColorFileBase ) ); V_snprintf( sz_Buffer, MAX_PATH,
     "\t\"$baseTexture\" \"models/contrib/%s/%s/%s\"\n", pcSteamName,
     pcBaseName, szTGAColorFileBase ); fputs( sz_Buffer,  fp ); fputs(
     "\t\$translucent\t1\n",  fp ); fputs( "\t\"$vertexcolor\"\t1\n}\n",  fp );
   */

  fputs("\"VertexlitGeneric\"\n", fp);
  fputs("{\n", fp);
  V_StripExtension(pcTGAColorFile, szTGAColorFileBase,
                   sizeof(szTGAColorFileBase));
  V_snprintf(sz_Buffer, MAX_PATH,
             "\t\"$baseTexture\" \"models/contrib/%s/%s/%s\"\n", pcSteamName,
             pcBaseName, szTGAColorFileBase);
  fputs(sz_Buffer, fp);

  if (pcTGANormalFile && (V_strlen(pcTGANormalFile) > 0)) {
    V_StripExtension(pcTGANormalFile, szTGANormalFileBase,
                     sizeof(szTGANormalFileBase));
    V_snprintf(sz_Buffer, MAX_PATH,
               "\t\"$bumpmap\" \"models/contrib/%s/%s/%s\"\n", pcSteamName,
               pcBaseName, szTGANormalFileBase);
    fputs(sz_Buffer, fp);
  }
  ...
  V_snprintf(sz_Buffer, MAX_PATH, "\t\"%s\" \"%s\"\n", "$lightwarptexture",
             "models\\lightwarps\\weapon_lightwarp");
  fputs(sz_Buffer, fp);
...
bool CheckFilesExist(const char *pcDirectory, const char *pcOBJFile,
                     const char *pcMTLFile) {
  WIN32_FIND_DATA wfd;
  HANDLE ff;
...
bool ParseMTL(const char *pcDirectory, const char *pcOBJFile,
              const char *pcMTLFile, char *pcTGAColorFile,
              size_t nTGAColorBufSize, char *pcTGASpecularFile,
              size_t nTGASpecularBufSize) {
  ...
  while (fgets(szLine, sizeof(szLine), fp)) {
    ...
    if (V_stristr(szToken, "map_Kd")) {
      V_strncpy(pcTGAColorFile, V_UnqualifiedFileName(szValue),
                nTGAColorBufSize);
    } else if (V_stristr(szLine, "map_Ks")) {
      V_strncpy(pcTGASpecularFile, V_UnqualifiedFileName(szValue),
                nTGASpecularBufSize);
    }
  }
...
bool ParseArguments(int argc, char *argv[], char *pcDirectory,
                    size_t nDirectoryBufSize, char *pcBaseName,
                    size_t nBaseNameBufSize, char *pcOBJFile,
                    size_t nOBJFileBufSize, char *pcMTLFile,
...
bool GetSteamUserName(char *pcSteamName, size_t nSteamNameBufSize) {
  ...
  if (ERROR_SUCCESS ==
    ...
    RegQueryValueEx(hKey, "ModInstallPath", NULL, NULL,
                    (LPBYTE)szModInstallPath, &dwSize);
    RegCloseKey(hKey);
...
bool GetSDKBinDirectory(char *pcSDKBinDir, size_t nBuffSize) {
  ...
  *pcSDKBinDir = szEngineVersion[0] = szSDKPath[0] = NULL;
  ...
  if (szEngineVersion[0] && szSDKPath[0]) {
    V_snprintf(pcSDKBinDir, nBuffSize, "%s\\bin\\%s\\bin", szSDKPath,
               szEngineVersion);
    return true;
  }
...
bool GetSDKSourcesDirectory(char *pcSDKSourcesDir, size_t nBufSize) {
  ...
  *pcSDKSourcesDir = NULL;
  if (ERROR_SUCCESS == RegOpenKey(HKEY_CURRENT_USER, "Environment", &hKey)) {
    ...
    RegQueryValueEx(hKey, "vproject", NULL, NULL, (LPBYTE)szVProjectPath,
                    &dwSize);
    RegCloseKey(hKey);

    V_snprintf(pcSDKSourcesDir, nBufSize, "%s_content\\%s", szSDKPath,
               V_UnqualifiedFileName(szVProjectPath));
...
void RunCommandLine(const char *pCmdLine, const char *pWorkingDir) {
  ...
  CreateProcess(NULL, (char *)pCmdLine, NULL, NULL, FALSE, CREATE_NO_WINDOW,
                NULL, pWorkingDir, &startupInfo, &pi);
...
void CompileVTEX(const char *pcDirectory, const char *pcTGAColorFile,
  ...
  V_snprintf(szCmdLine, sizeof(szCmdLine),
             "\"%s\\vtex.exe\" -nopause "
             "\"%s\\materialsrc\\models\\contrib\\%s\\%s\\%s\"",
             szSDKBinDir, szSDKSourcesDir, pcSteamName, pcBaseName,
             pcTGAColorFile);
  RunCommandLine(szCmdLine, szSDKBinDir);
}

void CompileMDL(const char *pcObjFile, const char *pcSteamName,
  ...
  V_snprintf(
      ...
      "\"%s\\studiomdl.exe\" -nop4 \"%s\\modelsrc\\contrib\\%s\\%s\\%s.qc\"",
...
bool GetVProjectDirectory(char *pcVProjectPath, size_t nBufSize) {
  ...
  if (ERROR_SUCCESS == RegOpenKey(HKEY_CURRENT_USER, "Environment", &hKey)) {
    ...
    RegQueryValueEx(hKey, "vproject", NULL, NULL, (LPBYTE)pcVProjectPath,
                    &dwSize);
    RegCloseKey(hKey);
...
void LaunchHLMV(const char *pcSteamName, const char *pcBaseName) {
  ...
  V_snprintf(
      szCmdLine, sizeof(szCmdLine),
      "\"%s\\hlmv.exe\" -game \"%s\" \"%s\\models\\contrib\\%s\\%s\\%s.mdl\"",
      szSDKBinDir, szVProjectPath, szVProjectPath, pcSteamName, pcBaseName,
      pcBaseName);
  RunCommandLine(szCmdLine, szSDKBinDir);
}

void CopyVMT(const char *pcDirectory, const char *pcBaseName,
             const char *pcSteamName) {
  char szVProjectPath[MAX_PATH];
  ...
  V_snprintf(szSourceVMT, sizeof(szSourceVMT), "%s%s.vmt", pcDirectory,
             pcBaseName);
  V_snprintf(szDestVMT, sizeof(szDestVMT),
             "%s\\materials\\models\\contrib\\%s\\%s\\%s.vmt", szVProjectPath,
...
bool CopyFiles(const char *pcSourceDir, const char *pcPattern,
               const char *pcDestDir) {
  char szFindPattern[MAX_PATH];
  bool bAllSucceeded = true;

  V_snprintf(szFindPattern, sizeof(szFindPattern), "%s%s", pcSourceDir,
             pcPattern);
  ...
  if (hFind == INVALID_HANDLE_VALUE) {
    ...
    do {
      ...
      V_snprintf(szSrcPath, sizeof(szSrcPath), "%s%s", pcSourceDir,
                 findData.cFileName);
      V_snprintf(szDestPath, sizeof(szDestPath), "%s\\%s", pcDestDir,
                 findData.cFileName);
...
bool CopyMaterialSourcesToSrcTree(const char *pcDirectory,
                                  const char *pcSteamName,
                                  const char *pcBaseName) {
  char szSDKSourcesDir[MAX_PATH];
  ...
  // Materials
  //
  V_snprintf(szBuffer, sizeof(szBuffer), "%s%s", szSDKSourcesDir,
             "\\materialsrc");
  ...
  V_snprintf(szBuffer, sizeof(szBuffer), "%s%s", szSDKSourcesDir,
             "\\materialsrc\\models\\contrib");
  CreateDirectory(szBuffer, NULL);
  bAllSucceeded &= DirectoryExists(szBuffer);

  V_snprintf(szBuffer, sizeof(szBuffer), "%s%s%s", szSDKSourcesDir,
             "\\materialsrc\\models\\contrib\\", pcSteamName);
  CreateDirectory(szBuffer, NULL);
  bAllSucceeded &= DirectoryExists(szBuffer);

  V_snprintf(szBuffer, sizeof(szBuffer), "%s%s%s\\%s", szSDKSourcesDir,
             "\\materialsrc\\models\\contrib\\", pcSteamName, pcBaseName);
  CreateDirectory(szBuffer, NULL);
  bAllSucceeded &= DirectoryExists(szBuffer);
...
bool ReplaceLineInTXTFile(const char *pcFilePath, const char *pcFirstChars,
  ...
  FOR_EACH_VEC(fileContents, i) { fputs(fileContents[i], fp); }
...
bool CopyModelSourcesToSrcTree(const char *pcDirectory, const char *pcSteamName,
                               const char *pcBaseName) {
  char szSDKSourcesDir[MAX_PATH];
  char szBuffer[MAX_PATH];
  bool bAllSucceeded = true;
  ...
  V_snprintf(szBuffer, sizeof(szBuffer), "%s%s%s", szSDKSourcesDir,
             "\\modelsrc\\contrib\\", pcSteamName);
  CreateDirectory(szBuffer, NULL);
  bAllSucceeded &= DirectoryExists(szBuffer);

  V_snprintf(szBuffer, sizeof(szBuffer), "%s%s%s\\%s", szSDKSourcesDir,
             "\\modelsrc\\contrib\\", pcSteamName, pcBaseName);
  CreateDirectory(szBuffer, NULL);
  bAllSucceeded &= DirectoryExists(szBuffer);
  ...
  // Replace some tags in the OBJ and MTL to satisfy some assumptions made by
  // studiomdl
  char szFileToChange[MAX_PATH];
  char szReplacementLine[MAX_PATH];

  V_snprintf(szFileToChange, sizeof(szFileToChange), "%s\\%s.mtl", szBuffer,
             pcBaseName);
  ...
  V_snprintf(szFileToChange, sizeof(szFileToChange), "%s\\%s.obj", szBuffer,
             pcBaseName);
...
int main(int argc, char *argv[]) {
  ...
  szDirectory[0] = szMTLFile[0] = szOBJFile[0] = szSteamName[0] =
      szTGAColorFile[0] = szTGANormalFile[0] = szTGATransparencyFile[0] =
          szTGASpecularFile[0] = NULL;

  if (ParseArguments(argc, argv, szDirectory, sizeof(szDirectory), szBaseName,
                     sizeof(szBaseName), szOBJFile, sizeof(szOBJFile),
    ...
    if (CheckFilesExist(szDirectory, szOBJFile, szMTLFile)) {
      ...
      ParseMTL(szDirectory, szOBJFile, szMTLFile, szTGAColorFile,
               sizeof(szTGAColorFile), szTGASpecularFile,
               sizeof(szTGASpecularFile));
      printf("--- Creating VMT and QC files ---\n");
      CreateTemplateVMT(szDirectory, szBaseName, szTGAColorFile,
                        szTGANormalFile, szSteamName);
