...
static void Usage(void) {
  fprintf(stderr, "Usage: vice [-quiet] [-nopause] [-encrypt key] [-decrypt "
                  "key] [-newext name] file file2 . . .\n");
  fprintf(stderr,
...
int main(int argc, char *argv[]) {
  ...
  for (; i < argc; i++) {
    ...
    if (strstr(pInputBaseName, "*.")) {
      ...
      _splitpath(pInputBaseName, NULL, NULL, fname,
                 ext);              // find extension wanted
