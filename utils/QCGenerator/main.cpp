...
bool CQCGeneratorApp::PreInit() {
  ...
  g_pFullFileSystem->AddSearchPath(GetBaseDirectory(),
                                   QCGENERATOR_MAIN_PATH_ID);

  // the main platform dir
  g_pFullFileSystem->AddSearchPath("platform", "PLATFORM", PATH_ADD_TO_HEAD);
  g_pFullFileSystem->AddSearchPath(".\\QCGenerator\\", QCGENERATOR_WRITE_PATH,
                                   PATH_ADD_TO_HEAD);

  return true;
}

void CQCGeneratorApp::PostShutdown() { BaseClass::PostShutdown(); }
...
int CQCGeneratorApp::Main() {
  if (CommandLine()->ParmValue("-path")) {
    ...
    ::MessageBox(NULL,
                 "Usage: QCGenerator.exe -path [path to smd files] -scene "
                 "[name of scene]\n",
                 "Error", MB_OK);
    return 0;
  }
