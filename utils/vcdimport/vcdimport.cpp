...
int CVcdImportApp::Main() {
  ...
  info.m_nInterpolationType = Interpolator_InterpolatorForName(
      CommandLine()->ParmValue("-c", "linear_interp"));
  info.m_bIgnorePhonemes = CommandLine()->FindParm("-p") != 0;
  if (!pImportFileName || !pInFileName) {
    Msg("Usage: vcdimport -f <imported .fac file> -i <in .vcd file> [-o <out "
        ".vcd file>]\n");
    Msg("\t\t[-nop4] [-p] [-s <simplification_amount>] [-c <curve type>]\n");
    Msg("\t-f : .FAC file containing SFM-authored facial animation exported "
        "from the animation set editor in the SFM.\n");
    Msg("\t-i : Source .VCD file to import the facial animation into.\n");
    Msg("\t-o : If no output file is specified, vcdimport will overwrite the "
        "input .vcd file.\n");
    Msg("\t-p : Causes the generated .VCD file to not use phonemes in "
        "associated wav files to autogenerate animation.\n");
    Msg("\t-nop4 : Disables auto perforce checkout.\n");
    Msg("\t-s : Requires a second argument, the amount of error we're willing "
        "to accept in simplification. Ranges from 0.0 - 1.0 [default = "
        "0.05]\n");
    Msg("\t-c : Specify interpolation type. Options are: [linear_interp is "
        "used if -c isn't specified]\n");
  ...
  bool bOk =
      ImportLogsIntoVCD(pImportFileName, pInFileName, pOutFileName, info);
  if (bOk) {
    Msg("Wrote file %s.\n", pOutFileName);
  }
  return bOk;
