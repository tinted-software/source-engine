...
void printusage(void) {
  vprint(
      0,
      "usage:  paginate startpage \"designation\" <root directory> [<outdir>]\n\
		\t-p = preview only\n\
...
void PaginateFilesInDirectory(char const *rootdir) {
  ...
  for (int i = 0; i < c; ++i) {
    ...
    Q_snprintf(p2, sizeof(p2), "VLV%i",
               START_PAGE + stats.pagecount + pagestats.pagecount);

    vprint(1, "%-160s\t%8i\t%10i\t%12s\t%35s\n", fn, pagestats.pagecount,
           pagestats.linecount, Q_pretifymem(pagestats.totalbytes, 2),
           va("%s -> %s", p1, p2));
...
int main(int argc, char *argv[]) {
  ...
  Q_strncpy(g_szDesignation, "Confidential - Attorney's Eyes Only",
            sizeof(g_szDesignation));
