...
BEGIN_DEFINE_LOGGING_CHANNEL(LOG_ITEMTEST, "ItemTest", LCF_CONSOLE_ONLY,
                             LS_MESSAGE);
...
#define FUNCTION_LINE_STRING                                                   \
  __FUNCTION__ "(" FUNCTION_LINE_TOSTRING(__LINE__) "): "
#endif
...
bool CItemUpload::InitManifest(void) {
  ...
  m_pItemTestManifest =
      new CItemTestManifest("scripts/itemtest_manifest.txt", new CItemLog());
...
//
//-----------------------------------------------------------------------------
CItemTestManifest::CItemTestManifest(const char *pszManifestFile,
                                     CItemLog *pItemLog)
  ...
  if (!ParseStringsFromManifest(m_pManifestKV, "animation_mdl_files",
                                m_vecAnimationMDLExtensions))
    return;

  // Material types
  KeyValues *pKVMaterialTypes = m_pManifestKV->FindKey("material_types");
  if (!pKVMaterialTypes) {
    m_pItemLog->Warning("ERROR: Failed to find a 'material_types' section in "
                        "manifest file: %s\n",
                        pszManifestFile);
    return;
  }
  ...
  const char *pszDefaultMatTypeString =
      m_pManifestKV->GetString("default_material_type");
  if (!pszDefaultMatTypeString || !pszDefaultMatTypeString[0]) {
    m_pItemLog->Warning("ERROR: Failed to find a 'default_material_type' "
                        "string in manifest file: %s\n",
                        pszManifestFile);
    return;
  }
  m_nDefaultMaterialType = GetMaterialType(pszDefaultMatTypeString);
  if (m_nDefaultMaterialType == kInvalidMaterialType) {
    m_pItemLog->Warning("ERROR: Default material type '%s' wasn't found in the "
                        "material type list in manifest file: %s\n",
  ...
  if (pKVMaterialSkins) {
    FOR_EACH_SUBKEY(pKVMaterialSkins, pKVMaterialSkin) {
      ...
      m_vecMaterialSkins[nIdx].pszFilenameAppend =
          pKVMaterialSkin->GetString("file_append");
    }

    const char *pszDefaultMatSkinString =
        m_pManifestKV->GetString("default_material_skin");
    if (!pszDefaultMatSkinString || !pszDefaultMatSkinString[0]) {
      m_pItemLog->Warning("ERROR: Failed to find a 'default_material_skin' "
                          "string in manifest file: %s\n",
                          pszManifestFile);
      return;
    }
    ...
    if (m_nDefaultMaterialSkin == kInvalidMaterialSkin) {
      m_pItemLog->Warning("ERROR: Default material skin '%s' wasn't found in "
                          "the material skin list in manifest file: %s\n",
  ...
  if (!pKVTextureTypes) {
    m_pItemLog->Warning("ERROR: Failed to find a 'texture_types' section in "
                        "manifest file: %s\n",
                        pszManifestFile);
    return;
  }
  FOR_EACH_SUBKEY(pKVTextureTypes, pKVTexture) {
    ...
    m_vecTextureTypes[nIdx].pkvAddToVTEXConfig =
        pKVTexture->FindKey("add_to_vtex_config");
  }

  // VMT templates
  KeyValues *pKVTemplates = m_pManifestKV->FindKey("vmt_templates");
  if (!pKVTemplates) {
    m_pItemLog->Warning("ERROR: Failed to find a 'vmt_templates' section in "
                        "manifest file: %s\n",
                        pszManifestFile);
    return;
  }
  ...
  if (pKVClassTemplates) {
    ...
    FOR_EACH_SUBKEY(pKVClassTemplates, pKVClassTemplate) {
      ...
      if (iClass == m_vecClasses.InvalidIndex()) {
        m_pItemLog->Warning("ERROR: Found an invalid class '%s' in the "
                            "vmt_templates entries in manifest file: %s\n",
  ...
  if (pKVIconTypes) {
    FOR_EACH_SUBKEY(pKVIconTypes, pKVIcon) {
      ...
      m_vecIconTypes[nIdx].pszFilenameAppend =
          pKVIcon->GetString("file_append");
      m_vecIconTypes[nIdx].pkvAddToVTEXConfig =
          pKVIcon->FindKey("add_to_vtex_config");
  ...
  if (V_strlen(m_pQCITemplate) == 0) {
    m_pItemLog->Warning(
        "ERROR: qci_template not defined in manifest file: %s\n",
        pszManifestFile);
    return;
  }
  ...
  m_bItemPathUsesSteamId =
      m_pManifestKV->GetBool("item_path_has_steamid", true);
}

//-----------------------------------------------------------------------------
//
//-----------------------------------------------------------------------------
bool CItemTestManifest::ParseStringsFromManifest(
    KeyValues *pKV, const char *pszKeyName, CUtlVector<CUtlString> &vecList) {
  KeyValues *pKVSub = pKV->FindKey(pszKeyName);
  if (!pKVSub) {
    m_pItemLog->Warning(
        "ERROR: Failed to find a '%s' section in manifest file.\n", pszKeyName);
    return false;
  }
...
//-----------------------------------------------------------------------------
KeyValues *
CItemTestManifest::GetTextureAddToVTEXConfig(const char *pszTextureType) {
...
//
//-----------------------------------------------------------------------------
bool CItemTestManifest::GetIconDimensions(int nIcon, int &nWidth,
                                          int &nHeight) {
...
//
//-----------------------------------------------------------------------------
int GetClassCount() { return CItemUpload::Manifest()->GetNumClasses(); }

//-----------------------------------------------------------------------------
//
//-----------------------------------------------------------------------------
const char *GetClassString(int i) {
  return (i < 0 || i >= GetClassCount()) ? NULL
                                         : CItemUpload::Manifest()->GetClass(i);
}
...
//
//-----------------------------------------------------------------------------
template <class T> const Vector &CItemUploadGame<T>::GetBipHead(int i) {
  static const Vector vOrigin(0, 0, 0);

  return (i < 0 || i >= GetClassCount()) ? vOrigin
                                         : CItemUploadGame<T>::s_vBipHead[i];
}
...
const RadianEuler &CItemUploadGame<T>::GetBipHeadRotation(int i) {
  ...
  return (i < 0 || i >= GetClassCount()) ? eOrigin
                                         : CItemUploadGame<T>::s_eBipHead[i];
}
...
COMPILE_TIME_ASSERT(ARRAYSIZE(CItemUploadGame<CItemUploadTF>::s_vBipHead) ==
                    CItemUploadTF::kClassCount);
...
COMPILE_TIME_ASSERT(ARRAYSIZE(CItemUploadGame<CItemUploadTF>::s_eBipHead) ==
                    CItemUploadTF::kClassCount);
...
bool CItemUpload::GetSteamId(CUtlString &sSteamId) {
  ...
  if (!SteamClient()) {
    ...
    SteamAPI_SetTryCatchCallbacks(
        false); // We don't use exceptions, so tell steam not to use try/catch
...
//       should be needed
//-----------------------------------------------------------------------------
bool CItemUpload::GetSourceSDKFromExe(CUtlString &sSourceSDK,
                                      CUtlString &sSourceSDKBin) {
  if (GetDevMode())
    return false;
  ...
  const char *pszBinOrangeBoxBin =
      V_strstr(sCurrentExecutableFileName.String(), "/bin/orangebox/bin/");
  if (pszBinOrangeBoxBin) {
    sSourceSDK.SetDirect(sCurrentExecutableFileName.String(),
                         pszBinOrangeBoxBin -
                             sCurrentExecutableFileName.String());

    char szBinDir[MAX_PATH];
    V_ExtractFilePath(sCurrentExecutableFileName.String(), szBinDir,
                      ARRAYSIZE(szBinDir));
  ...
  if (ERROR_SUCCESS ==
      RegOpenKey(HKEY_CURRENT_USER, "Software\\Valve\\Source SDK", &hKey)) {
    DWORD dwSize = sizeof(szEngineVersion);
    RegQueryValueEx(hKey, "EngineVer", NULL, NULL, (LPBYTE)szEngineVersion,
                    &dwSize);
    RegCloseKey(hKey);
  } else {
...
static bool CheckToolPath(CUtlString &sBinDir) {
  ...
  V_ComposeFileName(sBinDir, "vtex.exe", szVtexFileName,
                    ARRAYSIZE(szVtexFileName));

  char szStudiomdlFileName[MAX_PATH];
  V_ComposeFileName(sBinDir, "studiomdl.exe", szStudiomdlFileName,
                    ARRAYSIZE(szStudiomdlFileName));

  return g_pFullFileSystem->FileExists(szVtexFileName) &&
         g_pFullFileSystem->FileExists(szStudiomdlFileName);
}
...
//
//-----------------------------------------------------------------------------
bool CItemUpload::CopyFiles(const char *pszSourceDir, const char *pszPattern,
                            const char *pszDestDir) {
  char szFindPattern[k64KB];
  bool bAllSucceeded = true;

  V_snprintf(szFindPattern, sizeof(szFindPattern), "%s%s", pszSourceDir,
             pszPattern);
  ...
  if (hFind == INVALID_HANDLE_VALUE) {
    ...
    do {
      ...
      V_snprintf(szSrcPath, sizeof(szSrcPath), "%s%s", pszSourceDir,
                 findData.cFileName);
      V_snprintf(szDestPath, sizeof(szDestPath), "%s\\%s", pszDestDir,
                 findData.cFileName);
...
//
//=============================================================================
static bool RemoveTextBlock(const char *str, char const *search,
                            char *pszOutBuf, int nSizeofOutBuf) {
  if (str != pszOutBuf) {
...
CTargetBase::CTargetBase(CAsset *pAsset, const CTargetBase *pTargetParent)
    : CItemLog(pAsset), m_pAsset(pAsset), m_nRefCount(0),
      m_pTargetParent(pTargetParent), m_bIgnoreP4(false),
...
//
//-----------------------------------------------------------------------------
bool CTargetBase::GetOutputPaths(CUtlVector<CUtlString> &sOutputPaths,
  ...
  if (bRecurse) {
    ...
    if (GetInputs(inputs)) {
      for (int i = 0; i < inputs.Count(); ++i) {
        ...
        bRet = pTargetBase->GetOutputPaths(sOutputPaths, bRelative, bRecurse,
                                           bExtension) &&
               bRet;
      }
    } else {
...
//
//-----------------------------------------------------------------------------
bool CTargetBase::GetInputPaths(CUtlVector<CUtlString> &sInputPaths,
                                bool bRelative /* = true */,
                                bool bRecurse /* = true */,
                                bool bExtension /* = true */) {
  CUtlString sTmp;
  ...
  for (int i = 0; bRet && i < inputs.Count(); ++i) {
    ...
    bRet = pTargetBase->GetOutputPaths(sInputPaths, bRelative, false,
                                       bExtension) &&
           bRet;

    if (bRecurse) {
      bRet = pTargetBase->GetInputPaths(sInputPaths, bRelative, bRecurse,
                                        bExtension) &&
             bRet;
    }
  }
...
//
//-----------------------------------------------------------------------------
CAsset *CTargetBase::Asset() const { return m_pAsset; }
...
//
//-----------------------------------------------------------------------------
bool CTargetBase::GetDirName(CUtlString &sDirName,
                             uint nPathFlags /* = PATH_FLAG_ALL */) const {
  sDirName.Clear();
...
void CTargetBase::AddOrEditP4File(const char *pszFilePath) {
  ...
  if (CItemUpload::GetP4()) {
    ...
    g_pFullFileSystem->GetCaseCorrectFullPath(pszFilePath,
...
//
//=============================================================================
bool VTFGetInfo(const char *fileName, int *width, int *height,
...
CTargetTGA::CTargetTGA(CAsset *pAsset, const CTargetVMT *pTargetVMT)
    : CTargetBase(pAsset, pTargetVMT), m_pTargetVMT(pTargetVMT),
      m_nSrcImageType(IMAGE_FILE_UNKNOWN), m_nWidth(0), m_nHeight(0),
      m_nChannelCount(0), m_bNoNiceFiltering(false), m_bAlpha(false),
      m_bPowerOfTwo(false) {}
...
//
//-----------------------------------------------------------------------------
bool CTargetTGA::IsModelPath() const { return m_pTargetVMT->IsModelPath(); }

//-----------------------------------------------------------------------------
//
//-----------------------------------------------------------------------------
bool CTargetTGA::GetInputPaths(CUtlVector<CUtlString> &sInputPaths,
                               bool bRelative /* = true */,
                               bool bRecurse /* = true */,
                               bool bExtension /* = true */) {
  sInputPaths.AddToTail(m_sInputFile);

  return CTargetBase::GetInputPaths(sInputPaths, bRelative, bRecurse,
                                    bExtension);
}
...
bool CTargetTGA::Compile() {
  // 'reset' the input file from the current value to force a reload on compile,
  // make a temp copy as it's going to be overwritten
  ...
  case IMAGE_FILE_TGA: {
    ...
    if (!TGALoader::LoadRGBA8888(m_sInputFile.String(), tgaBits, nWidth,
                                 nHeight)) {
    ...
    bitmap.SetBuffer(nWidth, nHeight, IMAGE_FORMAT_RGBA8888, tgaBits.Base(),
                     false, nWidth * 4);
  } break;
  ...
  case IMAGE_FILE_VTF: {
    // Just load the whole file into a memory buffer
    CUtlBuffer bufFileContents;
    if (!g_pFullFileSystem->ReadFile(m_sInputFile.String(), NULL,
                                     bufFileContents)) {
    ...
    int nMemSize = ImageLoader::GetMemRequired(nWidth, nHeight, 1,
  ...
  if (bitmap.Format() != IMAGE_FORMAT_RGBA8888 || bitmap.Width() != m_nWidth ||
      bitmap.Height() != m_nHeight) {
    Warning("CTarget%s::Compile( %s ) - Invalid Bitmap Size, Expected %d x %d "
            "x %d (%d)\n",
            GetTypeString(), sName.String(), m_nWidth, m_nHeight, 4,
  ...
  // If we're supposed to resize these to another resolution, setup the VTEX
  // config file to do so.
  int nTargetWidth = m_pTargetVMT->GetTargetWidth();
  int nTargetHeight = m_pTargetVMT->GetTargetHeight();
  if (nTargetWidth && nTargetHeight) {
    // We want to use "reduce", not "maxwidth" & "maxheight", so we throw away
    // the higher resolution mips. Determine the right amount of reduction based
    // on the texture's width & height (and fail if it's the wrong aspect ratio)
    int nFactor = (m_nWidth / nTargetWidth);
    if (nFactor <= 0) {
      Warning("CTarget%s::Compile( %s ) - Failed to determine reduction factor "
    ...
    if (nFactor != nHeightFactor) {
      Warning("CTarget%s::Compile( %s ) - Failed to determine reduction factor "
              "(target size aspect ratio (%dx%d) doesn't match texture's "
              "aspect ratio (%dx%d))\n",
              GetTypeString(), sName.String(), nTargetWidth, nTargetHeight,
              m_nWidth, m_nHeight);
      return false;
    }
  ...
  CUtlString sTGAOutput = Asset()->CheckRedundantOutputFilePath(
      GetInputFile().String(), bufVTEXConfig.String(), sAbsPath.String());
  ...
  // TODO: Don't write alpha if VMT using this doesn't specify the alpha is used
  // for anything
  CUtlBuffer outBuf;
  const bool bWriteTGA = TGAWriter::WriteToBuffer(
      bitmap.GetBits(), outBuf, bitmap.Width(), bitmap.Height(),
      bitmap.Format(), m_bAlpha ? IMAGE_FORMAT_BGRA8888 : IMAGE_FORMAT_BGR888);
  ...
  V_StripExtension(V_GetFileName(szVTEXConfigFilename), szFileName,
...
bool CTargetTGA::SetInputFile(const char *pszFilename) {
  ...
  if (TGALoader::GetInfo(pszFilename, &m_nWidth, &m_nHeight, &imageFormat,
                         &flSourceGamma)) {
    m_nSrcImageType = IMAGE_FILE_TGA;
  } else if (PSDGetInfo(pszFilename, NULL, &m_nWidth, &m_nHeight, &imageFormat,
                        &flSourceGamma)) {
    m_nSrcImageType = IMAGE_FILE_PSD;
  } else if (VTFGetInfo(pszFilename, &m_nWidth, &m_nHeight, &imageFormat,
                        &flSourceGamma)) {
  ...
  if (nWidthPow == m_nWidth && nHeightPow == m_nHeight) {
    ...
    Warning("ERROR: Specified texture file (%s) Size %dx%d dimensions not "
            "powers of two, perhaps resize to %dx%d\n",
...
//
//-----------------------------------------------------------------------------
const CUtlString &CTargetTGA::GetInputFile() const { return m_sInputFile; }

//-----------------------------------------------------------------------------
//
//-----------------------------------------------------------------------------
template <class T> T CTargetTGA::NearestPowerOfTwo(T v) {
  if (v == 0)
...
void CTargetTGA::UpdateManifest(KeyValues *pKv) {
  ...
  if (GetOutputPath(sOutName, 0,
                    PATH_FLAG_PATH | PATH_FLAG_FILE | PATH_FLAG_PREFIX |
                        PATH_FLAG_MODELS | PATH_FLAG_EXTENSION)) {
  ...
  if (pszTextureType) {
    KeyValues *pVTEXKV =
        CItemUpload::Manifest()->GetTextureAddToVTEXConfig(pszTextureType);
    if (pVTEXKV) {
...
//
//-----------------------------------------------------------------------------
bool CTargetVTF::IsModelPath() const { return m_pTargetVMT->IsModelPath(); }
...
bool CTargetVTF::Compile() {
  ...
  if (sAbsInputPaths.Count() != 2) {
    Warning(
        "CTarget%s::Compile( %s ) - GetPaths returned %d paths, expected 2\n",
  ...
  if (CItemUpload::IgnoreEnvironmentVariables()) {
    // We can't rely on environment variables in VTEX. So tell it to just built
    // it on the spot, and we'll move it afterwards.
    sCmd.sprintf("\"%s\\vtex.exe\" -nop4 -nopause -dontusegamedir \"%s\"",
                 sBinDir.String(), sAbsInputPaths.Element(0).String());
  } else {
    CUtlString sVProject;
    CItemUpload::GetVProjectDir(sVProject);
    sCmd.sprintf("\"%s\\vtex.exe\" -nop4 -nopause -vproject \"%s\" \"%s\"",
                 sBinDir.String(), sVProject.String(),
  ...
  if (CItemUpload::IgnoreEnvironmentVariables()) {
    ...
    // We built the VTF in the directory with the content. Now move it to the
    // out dir.
    if (::MoveFileEx(sVTFName, sAbsPath.String(), MOVEFILE_REPLACE_EXISTING) ==
        0) {
...
//
//-----------------------------------------------------------------------------
const char *CTargetVTF::GetPrefix() const { return m_pTargetVMT->GetPrefix(); }
...
CTargetVMT::CTargetVMT(CAsset *pAsset, const CTargetBase *pTargetParent)
    : CTargetBase(pAsset, pTargetParent), m_pVMTKV(NULL),
      m_nColorAlphaType(kNoColorAlpha), m_nNormalAlphaType(kNoNormalAlpha),
      m_nMaterialType(kInvalidMaterialType), m_bDuplicate(false),
      m_nTargetWidth(0), m_nTargetHeight(0) {
...
bool CTargetVMT::Compile() {
  ...
  if (!GetOutputPaths(sAbsOutputPaths, false, false, true) ||
      !GetOutputPaths(sRelOutputPaths, true, false, true)) {
    Warning("CTarget%s::Compile( %s ) - GetOutputPaths failed\n",
            GetTypeString(), sName.String());
    return false;
  }

  if (sAbsOutputPaths.Count() != nVmtCount ||
      sAbsOutputPaths.Count() != sRelOutputPaths.Count()) {
    Warning("CTarget%s::Compile( %s ) - GetOutputPaths returned %d paths, "
            "expected %d\n",
  ...
  for (int i = 0; i < nVmtCount; ++i) {
    ...
    Msg("Compiling %s: %s\n", GetTypeString(),
        sRelOutputPaths.Element(i).String());
    ...
    FOR_EACH_VEC(m_vecTargetVTFs[i], iVTF) {
      if (m_vecTargetVTFs[i][iVTF].IsValid()) {
        const char *pszVMTVar =
            CItemUpload::Manifest()->GetVMTVarForTextureType(
                CItemUpload::Manifest()->GetTextureType(iVTF));
        if (pszVMTVar && pszVMTVar[0]) {
          sChildRelOutputPaths.RemoveAll();
          m_vecTargetVTFs[i][iVTF]->GetOutputPaths(sChildRelOutputPaths, true,
                                                   false, false, false);

          if (sChildRelOutputPaths.Count() > 0) {
            V_strncpy(szBuf, sChildRelOutputPaths.Element(0).String(),
                      ARRAYSIZE(szBuf));
            V_FixSlashes(szBuf, '/');
    ...
    g_pFullFileSystem->WriteFile(sAbsOutputPaths.Element(i).String(), "MOD",
                                 fileBuf);
...
const char *CTargetVMT::MaterialTypeToString(int nMaterialType) {
  ...
  const char *pszMaterialName =
      CItemUpload::Manifest()->GetMaterialType(nMaterialType);
  if (pszMaterialName)
...
bool CTargetVMT::IsOk(CUtlString &sMsg) const {
  ...
  if (!g_bCompilePreview) {
    ...
    FOR_EACH_VEC(m_vecTargetVTFs[nSkinIndex], i) {
      if (!m_vecTargetVTFs[nSkinIndex][i].IsValid() &&
          CItemUpload::Manifest()->IsTextureTypeRequired(i)) {
        sMsg += "\"";
        sMsg += m_sMaterialId;
        sMsg += "\": ";
  ...
  FOR_EACH_VEC(m_vecTargetVTFs, i) {
    FOR_EACH_VEC(m_vecTargetVTFs[i], j) {
      if (m_vecTargetVTFs[i][j].IsValid()) {
        CUtlString sExtension =
            CItemUpload::Manifest()->GetMaterialSkinFilenameAppend(i);
        sExtension += ".vmt";
...
bool CTargetVMT::SetMaterialType(int nMaterialType) {
  ...
  if (nMaterialType < 0 ||
      nMaterialType >= CItemUpload::Manifest()->GetNumMaterialTypes())
    return false;
  ...
  // Recursively increment other non-duplicate materials which match this
  // material and so on...
  for (int i = 0; i < pAsset->GetTargetVMTCount(); ++i) {
    ...
    if (!pTargetVMT->GetDuplicate() &&
        pTargetVMT->GetMaterialType() == nMaterialType) {
      pTargetVMT->SetMaterialType(
          ((nMaterialType + 1) %
           CItemUpload::Manifest()->GetNumMaterialTypes()));
    }
  }
...
//
//-----------------------------------------------------------------------------
int CTargetVMT::GetMaterialType() const { return m_nMaterialType; }
...
//
//-----------------------------------------------------------------------------
bool CTargetVMT::SetTargetVTF(const char *pszTextureType,
                              const char *pszFilename, int nSkinIndex) {
  ...
  return SetTargetVTF(m_vecTargetVTFs[nSkinIndex][nTextureType], pszFilename,
                      sSuffix.String());
}
...
//
//-----------------------------------------------------------------------------
bool CTargetVMT::SetTargetVTF(CSmartPtr<CTargetVTF> &pTargetVTF,
...
KeyValues *CTargetVMT::GetVMTKV(int nSkinIndex) {
  ...
  if (CItemUpload::Manifest()->HasClassVMTTemplates()) {
    ...
    const char *pszVMTTemplate =
        CItemUpload::Manifest()->GetClassVMTTemplate(nClassIndex);
    if (!pszVMTTemplate) {
...
void CTargetVMT::CreateLegacyTemplate(KeyValues *pVMTKV) {
  ...
  for (int iVTF = 0; iVTF < CItemUpload::Manifest()->GetNumTextureTypes();
       ++iVTF) {
    const char *pszVMTVar = CItemUpload::Manifest()->GetVMTVarForTextureType(
        CItemUpload::Manifest()->GetTextureType(iVTF));
  ...
  // Weapons are the more custom case, so we'll default to a good wearable
  // lightwarp
  ...
  pVMTKV->SetString("$rimlight",
                    "1"); // To enable rim lighting (requires phong)
  pVMTKV->SetString("$rimlightexponent",
                    "4"); // Exponent for phong component of rim
  pVMTKV->SetString("$rimlightboost",
  ...
  case kPaintable:
    ...
    pVMTKV->SetString("$blendtintcoloroverbase",
                      "0"); // between 0-1 determines how much blended by
                            // tinting vs. replacing the color

    pVMTKV->SetString(
        "$colortint_base",
        "{ 255 255 255 }"); // put the RGB value of whats being colored when no
                            // paint is present, if $blendtintcoloroverbase is 0
                            // then put [255 255 255] here.
  ...
  KeyValues *pProxiesAnimatedTexture =
      pProxies->FindKey("AnimatedTexture", true);
  pProxiesAnimatedTexture->SetString("animatedtexturevar", "$detail");
  pProxiesAnimatedTexture->SetString("animatedtextureframenumvar",
                                     "$detailframe");
  ...
  if (m_nColorAlphaType == kPaintable) {
    ...
    KeyValues *pProxiesSelectFirstIfNonZero =
        pProxies->FindKey("SelectFirstIfNonZero", true);
...
void CTargetVMT::UpdateManifest(KeyValues *pKV) {
  ...
  if (!GetDuplicate()) {
    ...
    pColorSubKey->SetString("alphaType",
                            ColorAlphaTypeToString(GetColorAlphaType()));
      ...
      FOR_EACH_VEC(m_vecTargetVTFs[i], j) {
        if (m_vecTargetVTFs[i][j].IsValid()) {
          const char *pszTextureType =
              CItemUpload::Manifest()->GetTextureType(j);
...
CTargetDMX::CTargetDMX(CAsset *pAsset, const CTargetQC *pTargetQC)
    : CTargetBase(pAsset, pTargetQC), m_pDmRoot(NULL), m_nLod(-1),
      m_flAnimationLoopStartTime(-1.f) {}

//-----------------------------------------------------------------------------
//
//-----------------------------------------------------------------------------
CTargetDMX::~CTargetDMX() { Clear(); }
...
#ifdef _DEBUG
  const bool bRet = g_pDataModel->SaveToFile(sAbsPath.String(), NULL,
                                             "keyvalues2", "model", m_pDmRoot);
#else  // ifdef _DEBUG
  const bool bRet = g_pDataModel->SaveToFile(sAbsPath.String(), NULL, "binary",
                                             "model", m_pDmRoot);
#endif // _DEBUG
...
//
//-----------------------------------------------------------------------------
bool CTargetDMX::GetInputPaths(CUtlVector<CUtlString> &sInputPaths,
                               bool bRelative /* = true */,
                               bool bRecurse /* = true */,
                               bool bExtension /* = true */) {
  sInputPaths.AddToTail(m_sInputFile);
...
int CTargetDMX::GetPolyCount() const {
  ...
  for (DmElementHandle_t hElement = g_pDataModel->FirstAllocatedElement();
    ...
    CDmeMesh *pDmeMesh =
        CastElement<CDmeMesh>(g_pDataModel->GetElement(hElement));
...
bool CTargetDMX::GetAnimationFrameInfo(float &flFrameRate,
                                       int &nFrameCount) const {
  if (!m_pDmRoot)
    return false;

  CDmeAnimationList *pDmeAnimationList =
      m_pDmRoot->GetValueElement<CDmeAnimationList>("animationList");
  CDmeChannelsClip *pDmeChannelsClip = NULL;

  if (pDmeAnimationList) {
    if (pDmeAnimationList->GetAnimationCount() < 0) {
      Warning("No DmeChannelsClips found in root.animationList of DMX file: "
              "\"%s\"\n",
              m_sInputFile.String());
      return false;
    }
    ...
    if (!pDmeChannelsClip) {
      Warning("No DmeChannelsClip found in root.animationList[0] of DMX file: "
              "\"%s\"\n",
              m_sInputFile.String());
      return false;
    }
    ...
    nFrameCount =
        (int)((pDmeChannelsClip->GetDuration().GetSeconds() * flFrameRate) +
              0.5f) +
        1;
...
//
//-----------------------------------------------------------------------------
const CUtlString &CTargetDMX::GetInputFile() const { return m_sInputFile; }
...
bool CTargetDMX::SetInputFile(const char *pszFilename) {
  // Because of the way datamodel works, bad things will happen if the same file
  // is loaded twice
  Clear();
  ...
  // we must store the root handle so we don't reload the same file multiple
  // time
  ...
  for (DmElementHandle_t hElement = g_pDataModel->FirstAllocatedElement();
    ...
    CDmeMaterial *pDmeMaterial =
        CastElement<CDmeMaterial>(g_pDataModel->GetElement(hElement));
    if (!pDmeMaterial || pDmeMaterial->GetFileId() != hRootFileId)
      continue;

    CSmartPtr<CTargetVMT> pTargetVMT = Asset()->FindOrAddMaterial(
        pDmeMaterial->GetMaterialName(), nMaterialType);
...
bool CTargetDMX::ReloadFile() {
  // Keep references to all target VMT's to prevent them from being destroyed by
  // a reload of the same file
...
void CTargetDMX::ReplaceMaterials() const {
  ...
  for (int i = 0; i < pAsset->GetTargetVMTCount(); ++i) {
    ...
    if (sMaterialId.IsEmpty() ||
        materialMap.IsValidIndex(materialMap.Find(sMaterialId)))
      continue;
    ...
    if (pTargetVMT->GetDuplicate()) {
      ...
      if (nMaterialType < 0 ||
          nMaterialType >= CItemUpload::Manifest()->GetNumMaterialTypes()) {
        Warning("CTarget%s::ReplaceMaterials - Duplicate VMT (%s) with bad "
                "material type (%d)\n",
      ...
      if (pTargetVMT == pSrcTargetVMT) {
        Warning("CTarget%s::ReplaceMaterials - Duplicate VMT (%s) Cannot Find "
                "Source Material Type (%d)\n",
                GetTypeString(), sMaterialId.String(), nMaterialType);
        continue;
      }
    }
    ...
    // The full extension can be _red.vmt or _blue.vmt so we need to keep the
    // _red but not the .vmt
    CUtlString sMaterialPath;
    pSrcTargetVMT->GetOutputPath(sMaterialPath, 0,
                                 PATH_FLAG_FILE | PATH_FLAG_EXTENSION |
                                     PATH_FLAG_PATH | PATH_FLAG_MODELS);
...
void CTargetDMX::SkinToBipHead() {
  ...
  CDmElement *pDstRoot =
      CreateElement<CDmElement>(m_pDmRoot->GetName(), nFileId);

  CDmeModel *pDstModel =
      CreateElement<CDmeModel>(pSrcModel->GetName(), nFileId);
...
//
//-----------------------------------------------------------------------------
void CTargetDMX::SkinToBipHead_R(CDmeModel *pDstModel, CDmeDag *pSrcDag,
                                 const Vector &vBipHead) {
  ...
  CDmeDag *pDstDag =
      CreateElement<CDmeDag>(pSrcDag->GetName(), pDstModel->GetFileId());
  if (!pDstDag)
    return;
  ...
  matrix3x4_t
      mIdentity; // TODO: Is there a static identity matrix like quat_identity?
  ...
  for (int i = 0; i < pDstMesh->BaseStateCount(); ++i) {
    ...
    // This seems a little odd, creating data to remove it but if it already
    // exists this is the only way of setting the joint count to 0 without
    // cheating (i.e. referring to the field by name)
    FieldIndex_t nJointWeightField;
    FieldIndex_t nJointIndexField;
    pDstVertexData->CreateJointWeightsAndIndices(0, &nJointWeightField,
                                                 &nJointIndexField);

    pDstVertexData->RemoveAttribute(
        pDstVertexData->FieldName(nJointWeightField));
    pDstVertexData->RemoveAttribute(
        pDstVertexData->FieldName(nJointIndexField));
    ...
    // Convert data to world space if required
    {
      ...
      FieldIndex_t nPositionField =
          pDstVertexData->FindFieldIndex(CDmeVertexData::FIELD_POSITION);
      if (nPositionField >= 0) {
        ...
        if (pPosAttr) {
          ...
          pDstVertexData->CreateJointWeightsAndIndices(1, &nJointWeightField,
                                                       &nJointIndexField);
          ...
          if (bConvertToWorldSpace) {
            for (int iPos = 0; iPos < posData.Count(); ++iPos) {
              pDstVertexData->SetVertexData(nJointIndexField, iPos, 1, AT_INT,
                                            &nZero);
              pDstVertexData->SetVertexData(nJointWeightField, iPos, 1,
                                            AT_FLOAT, &flOne);
            }
          } else {
            for (int iPos = 0; iPos < posData.Count(); ++iPos) {
              ...
              pDstVertexData->SetVertexData(nJointIndexField, i, 1, AT_INT,
                                            &nZero);
              pDstVertexData->SetVertexData(nJointWeightField, i, 1, AT_FLOAT,
                                            &flOne);
            }
          }
        }
      ...
      FieldIndex_t nNormalField =
          pDstVertexData->FindFieldIndex(CDmeVertexData::FIELD_NORMAL);
      if (nNormalField >= 0) {
  ...
  // See where the model was modelled... around the bip_head bone or at the
  // origin? There are probably other problems which aren't accounted for this
  // is a really naive algorithm
...
bool CTargetDMX::OutputQCIFile() {
  ...
  if (!m_strQCITemplate.IsEmpty()) {
    ...
    CUtlString strBuf = m_strQCITemplate.Replace(
        "<ITEMTEST_SEQUENCE_LAYER_NAME>", strLayerName.String());
    ...
    strBuf =
        strBuf.Replace("<BLEND_LAYER_RANGE>",
                       CFmtStr("0 %d %d <LAST_FRAME_INDEX>", nBlendFrameOffset,
                               nLastFrameIndex - nBlendFrameOffset));
    strBuf = strBuf.Replace("<FRAME_COUNT>", CFmtStr("%d", nFrameCount));
    strBuf =
        strBuf.Replace("<LAST_FRAME_INDEX>", CFmtStr("%d", nLastFrameIndex));
...
void LeftRightToValueBalance(float flLeft, float flRight, float &flOutValue,
                             float &flOutBalance) {
  if (flLeft > flRight) {
    flOutValue = flLeft;
...
void CTargetDMX::OutputSounds(CUtlBuffer &buf, int nIndentLevel,
                              CDmElement *pExportedSounds) {
  if (!pExportedSounds)
    return;
  ...
  if (!m_strSoundScriptFile.IsEmpty()) {
    if (!g_pFullFileSystem->ReadFile(m_strSoundScriptFile.String(), "MOD",
                                     soundScriptBuffer)) {
  ...
  BufBeginBlock(buf, nIndentLevel);
  {
    for (int i = 0; i < nSounds; ++i) {
      ...
      if (!g_pFullFileSystem->FileExists(
              CFmtStr("sound/%s", strWaveName.String()), "GAME")) {
        CUtlString strSoundName = strWaveName.StripExtension();
        if (!g_pFullFileSystem->FileExists(
                CFmtStr("sound/%s.mp3", strSoundName.String()), "GAME")) {
      ...
      const char *pszSoundScriptName = CFmtStr(
          "<CLASS_NAME>_<ITEMTEST_SEQUENCE_NAME>_%s", pSound->GetName());
      ...
      {
        BufPrintf(soundScriptBuffer, nSoundIndentLevel,
                  "\"channel\"			\"%s\"\n",
  ...
  V_MakeAbsolutePath(szSoundScriptFullPath, sizeof(szSoundScriptFullPath),
                     m_strSoundScriptFile.String(), sWorkingDir.String());
  if (!g_pFullFileSystem->WriteFile(szSoundScriptFullPath, NULL,
                                    soundScriptBuffer)) {
  ...
  if (GetCustomModPath()) {
    ...
    V_MakeAbsolutePath(
        szCustomPath, sizeof(szCustomPath),
        CFmtStr("%s/%s", GetCustomModPath(), m_strSoundScriptFile.String()),
        sWorkingDir.String());
...
// pChannelsClip is assumed to be an exported channelsclip (vs a live
// channelsclip)
  ...
  BufBeginBlock(vcdBuf, nLevel);
  {
    ...
    BufBeginBlock(vcdBuf, nLevel);
    {
      ...
      for (int ci = 0; ci < nChannelCount; ++ci) {
        ...
        // skip all transform
        if (!pChannel || !pChannel->GetToAttribute() ||
            pChannel->GetToAttribute()->GetType() != AT_FLOAT)
          continue;
        ...
        CDmeChannel *pLeftChannel =
            NULL; // pChannel->GetValueElement< CDmeChannel >(
                  // "leftvaluechannel" );
        ...
        if (bIsStereo) {
          ...
          CDmeFloatLogLayer *pLeftLogLayer =
              GetTopmostLogLayer<float>(pLeftChannel);
          CDmeFloatLogLayer *pRightLogLayer =
              GetTopmostLogLayer<float>(pRightChannel);
          ...
          if (pLeftLogLayer->GetKeyCount() == 0 ||
              pRightLogLayer->GetKeyCount() == 0) {
            // something is wrong here.
            Assert(0);
            continue;
          }

          DmeTime_t tStartOnFrame =
              MIN(pLeftLogLayer->GetKeyTime(0), pRightLogLayer->GetKeyTime(0));
          DmeTime_t tEndOnFrame = MAX(
              pLeftLogLayer->GetKeyTime(pLeftLogLayer->GetKeyCount() - 1),
              pRightLogLayer->GetKeyTime(pRightLogLayer->GetKeyCount() - 1));
#if 1 // resample time
          for (DmeTime_t tCurr = tStartOnFrame; tCurr <= tEndOnFrame;
               tCurr = tCurr.TimeAtNextFrame(framerate)) {
            float flLeftValue = pLeftLogLayer->GetValue(
                pDmeChannelsClip->FromChildMediaTime(tCurr));
            float flRightValue = pLeftLogLayer->GetValue(
                pDmeChannelsClip->FromChildMediaTime(tCurr));

            float flValue, flBalance;
            LeftRightToValueBalance(flLeftValue, flRightValue, flValue,
                                    flBalance);
          ...
          int iLeft = pLeftLogLayer->FindKey(
              pDmeChannelsClip->ToChildMediaTime(tStart));
          int iRight = pLeftLogLayer->FindKey(
              pDmeChannelsClip->ToChildMediaTime(tStart));
          while (iLeft < nLeft || iRight < nRight) {
            DmeTime_t tLeft = iLeft < nLeft
                                  ? pDmeChannelsClip->FromChildMediaTime(
                                        pLeftLogLayer->GetKeyTime(iLeft))
                                  : DMETIME_MAXTIME;
            DmeTime_t tRight = iRight < nRight
                                   ? pDmeChannelsClip->FromChildMediaTime(
                                         pRightLogLayer->GetKeyTime(iRight))
                                   : DMETIME_MAXTIME;
          ...
          BufBeginBlock(vcdBuf, nLevel);
          {
            ...
            BufPrintf(vcdBuf, nLevel,
                      "param \"player\\<CLASS_NAME>\\emotion\\emotion\"\n");
            ...
            BufBeginBlock(vcdBuf, nLevel);
            {
              ...
              for (int k = 0; k < nKeys; ++k) {
                DmeTime_t t = pDmeChannelsClip->FromChildMediaTime(
                    pLogLayer->GetKeyTime(k));
      ...
      // channel sounds
      OutputSounds(vcdBuf, nLevel,
                   m_pDmRoot->GetValueElement<CDmElement>("exportedSounds"));
    }
    BufEndBlock(vcdBuf, nLevel);
  }
  BufEndBlock(vcdBuf, nLevel);
...
bool CTargetVCD::Compile() {
  ...
  if (GetCustomModPath()) {
    ...
    V_MakeAbsolutePath(szCustomPath, sizeof(szCustomPath),
                       CFmtStr("%s/%s", GetCustomModPath(), sRelPath.String()),
                       sWorkingDir.String());
    V_FixSlashes(szCustomPath);
...
CTargetQC::CTargetQC(CAsset *pAsset, const CTargetMDL *pTargetMDL)
    : CTargetBase(pAsset, pTargetMDL),
      m_QCTemplate(0, 0, CUtlBuffer::TEXT_BUFFER), m_TargetVCD(NULL) {}
...
bool CTargetQC::Compile() {
  ...
  for (int nLOD = 1; nLOD < m_TargetDMXs.Count(); ++nLOD) {
    strBuf = strBuf.Replace(
        CFmtStr("<ITEMTEST_REPLACE_LOD%d_HEADER>", nLOD),
        CFmtStr("$lod %d", CItemUpload::Manifest()->GetQCLODDistance(nLOD)));
  }

  // Remove REPLACE_LOD block if it's not necessary
  RemoveTextBlock(strBuf.String(), "<ITEMTEST_REPLACE_LOD1_HEADER>",
                  strBuf.GetForModify(), strBuf.Length());
  RemoveTextBlock(strBuf.String(), "<ITEMTEST_REPLACE_LOD2_HEADER>",
                  strBuf.GetForModify(), strBuf.Length());

  CUtlString sSearch = "<ITEMTEST_REPLACE_SKIN_OPTIONALBLOCK>";
  CUtlString sReplace =
      "$cdmaterials \"<ITEMTEST_REPLACE_MATERIALS_PATH>\"\n$texturegroup "
      "skinfamilies\n{\n";
  CUtlString sMaterialPath;
  for (int nSkin = 0; nSkin < CItemUpload::Manifest()->GetNumMaterialSkins();
       ++nSkin) {
    int nVMTCount = 0;
    ...
    for (int nVMT = 0; nVMT < Asset()->GetTargetVMTCount(); ++nVMT) {
      ...
      // We grab the extension which includes the skin variant, and then trim
      // the actual file extension
      CUtlString sVMTName;
      if (!pTargetVMT->GetOutputPath(sVMTName, nSkin,
                                     PATH_FLAG_FILE | PATH_FLAG_EXTENSION)) {
        Warning(
            "CTarget%s::Compile( %s ) - VMT %d skin %d GetOutputPath failed\n",
            GetTypeString(), sName.String(), nVMT, nSkin);
        continue;
      }
      V_StripExtension(sVMTName.String(), sVMTName.GetForModify(),
                       sVMTName.Length());

      if (sMaterialPath.IsEmpty()) {
        pTargetVMT->GetOutputPath(sMaterialPath, 0,
                                  PATH_FLAG_PATH | PATH_FLAG_MODELS);
      }
  ...
  strBuf = strBuf.Replace("<ITEMTEST_REPLACE_MATERIALS_PATH>",
                          sMaterialPath.String());

  for (int nLOD = 0; nLOD < m_TargetDMXs.Count(); ++nLOD) {
    CUtlString sLOD;
    m_TargetDMXs[nLOD]->GetOutputPath(sLOD, 0,
                                      PATH_FLAG_FILE | PATH_FLAG_EXTENSION);

    strBuf = strBuf.Replace(CFmtStr("<ITEMTEST_REPLACE_LOD%d>", nLOD),
                            sLOD.String());
  }
  ...
  if (!m_strQCITemplate.IsEmpty()) {
    ...
    m_TargetDMXs[0]->GetOutputPath(strQCIDir, 1,
                                   PATH_FLAG_PATH & ~PATH_FLAG_ABSOLUTE);
    strQCIDir = strQCIDir.Replace('\\', '/');
    strBuf = strBuf.Replace("<QCI_RELATIVE_DIR>", strQCIDir.String());

    CUtlString strQCIRel;
    m_TargetDMXs[0]->GetOutputPath(
        strQCIRel, 1, PATH_FLAG_ALL & ~PATH_FLAG_ABSOLUTE & ~PATH_FLAG_MODELS);
...
//
//-----------------------------------------------------------------------------
int CTargetQC::TargetDMXCount() const { return m_TargetDMXs.Count(); }
...
bool CTargetMDL::Compile() {
  ...
  if (sAbsInputPaths.Count() != 1) {
    Warning(
        "CTarget%s::Compile( %s ) - GetPaths returned %d paths, expected 1\n",
        GetTypeString(), sName.String(), sAbsInputPaths.Count());
    return false;
  }
  ...
  if (CItemUpload::IgnoreEnvironmentVariables()) {
    ...
    sCmd.sprintf("\"%s\\studiomdl.exe\" -nop4 -game \"%s\" \"%s\"",
                 sBinDir.String(), sWorkingDir.String(),
                 sAbsInputPaths.Element(0).String());
  } else {
    CItemUpload::GetVProjectDir(sWorkingDir);
    sCmd.sprintf("\"%s\\studiomdl.exe\" -nop4 -vproject \"%s\" \"%s\"",
                 sBinDir.String(), sWorkingDir.String(),
                 sAbsInputPaths.Element(0).String());
  }
...
const ExtensionList *CTargetMDL::GetExtensionsAndCount(void) const {
  ...
  /*
          ...
          if ( !s_bGameInfoParsed )
          {
                  ...
                  if ( pKeyValues != NULL )
                  {
                          if ( g_pFullFileSystem && pKeyValues->LoadFromFile(
     g_pFullFileSystem, "gameinfo.txt" ) )
                          {
...
//=============================================================================
CAsset::CAsset()
    : CTargetBase(NULL, NULL), m_bSkinToBipHead(false), m_nCurrentModel(-1),
      m_vmtMap(UtlStringLessThan), m_bShouldBuildScenesImage(false) {
  m_pAsset = this;
...
CAsset::CAsset(const char *pszName, bool *pbOk /* = NULL */)
    : CTargetBase(NULL, NULL), m_nCurrentModel(-1),
...
bool CAsset::IsOk(CUtlString &sMsg) const {
  ...
  if (!CItemUpload::GetContentDir(sTmp) || sTmp.IsEmpty()) {
    sMsg = "Cannot figure out content directory, have you installed the Source "
           "SDK and restarted Steam?";
    return false;
  }
...
bool CAsset::BuildScenesImage() {
  ...
  V_MakeAbsolutePath(szScenesImage, sizeof(szScenesImage),
                     "scenes\\scenes.image", sWorkingDir.String());
  ...
  if (GetCustomModPath()) {
    ...
    V_MakeAbsolutePath(
        szCustomPath, sizeof(szCustomPath),
        CFmtStr("%s/%s", GetCustomModPath(), "scenes/scenes.image"),
        sWorkingDir.String());
    V_FixSlashes(szCustomPath);
...
bool CAsset::CompilePreview() {
  ...
  if (GetInputs(inputs)) {
    for (int i = 0; i < inputs.Count(); ++i) {
      ...
      pTargetBase->GetOutputPaths(
          m_sRelPaths, (PATH_FLAG_ALL & ~PATH_FLAG_ABSOLUTE) | PATH_FLAG_ZIP,
          true);
    }
  }
...
bool CAsset::Compile() {
  ...
  m_sRelPaths.RemoveAll();
  GetOutputPaths(m_sRelPaths,
                 (PATH_FLAG_ALL & ~PATH_FLAG_ABSOLUTE) | PATH_FLAG_ZIP, true);
  ...
  HANDLE m_hOutputZipFile =
      CreateFile(m_sArchivePath.String(), GENERIC_WRITE, 0, NULL, CREATE_ALWAYS,
                 FILE_ATTRIBUTE_NORMAL, NULL);
  if (m_hOutputZipFile == INVALID_HANDLE_VALUE) {
    Warning(
        "CTarget%s::Compile CreateZip Failed - Unable to create ZIP file %s.\n",
  ...
  for (int i = 0; i < m_sAbsPaths.Count(); ++i) {
    const char *pszFileExtension =
        V_GetFileExtension(m_sAbsPaths.Element(i).String());
    ...
    pZip->AddFileToZip(m_sRelPaths.Element(i).String(),
                       m_sAbsPaths.Element(i).String());
  }
...
//-----------------------------------------------------------------------------
bool CAsset::GetRelativeDir(CUtlString &sRelativeDir, const char *pszPrefix,
...
//-----------------------------------------------------------------------------
// Returns GetAbsoluteContentDir()/GetRelativeDir() or
// GetAbsoluteGameDir()/GetRelativeDir()
//-----------------------------------------------------------------------------
bool CAsset::GetAbsoluteDir(CUtlString &sAbsoluteDir,
...
//
//-----------------------------------------------------------------------------
bool CAsset::IsNameValid() const { return m_sName.Length() > 0; }

//-----------------------------------------------------------------------------
//
//-----------------------------------------------------------------------------
const char *CAsset::GetSteamId() const { return m_sSteamId.String(); }
...
//
//-----------------------------------------------------------------------------
int CAsset::GetTargetVMTCount() const { return m_vmtMap.Count(); }
...
//
//-----------------------------------------------------------------------------
CSmartPtr<CTargetVMT> CAsset::FindOrAddMaterial(const char *pszMaterial,
                                                int nMaterialType) {
  const CUtlString sMaterial(pszMaterial);

  CUtlMap<CUtlString, CTargetVMT *>::IndexType_t nIndex =
      m_vmtMap.Find(sMaterial);
...
const char *
CAsset::CheckRedundantOutputFilePath(const char *pszInputFilePath,
                                     const char *pszVTEXConfig,
                                     const char *pszOutputFilePath) {
  const char *pszLocalVTEXConfig = pszVTEXConfig ? pszVTEXConfig : "";

  // we don't want to output multiple of the same texture file with the same
  // vtex config
...
//
//-----------------------------------------------------------------------------
const CUtlString &CAsset::GetAssetName() const { return m_sName; }
