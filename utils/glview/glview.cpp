...
/*	YWB:  3/13/98
        You run the program like normal with any file.  If you want to read
   portals for the file type, you type:  glview -portal filename.gl0 (or
   whatever).  glview will then try to read in the .prt file filename.prt.

        The portals are shown as white lines superimposed over your image.  You
   can toggle the view between showing portals or not by hitting the '2' key.
...
void ReadPolyFileType(const char *name, int nList, BOOL drawLines) {
  ...
  while (1) {
    ...
    for (i = 0; i < numverts; i++) {
      ...
      /*
      if (!(fabs( v[0] ) < 32768.0&& fabs( v[1] ) < 32768.0 && fabs( v[2] ) <
      32768.0 ) ) Error( "Out of range data\n");
      */
      ...
      if (drawLines) // YELLOW OUTLINES
        ...
        if (g_bReadPortals) // Gray scale it, leave portals blue
        {
          if (fabs(fabs(v[5]) - 1.0f) <
              0.01) // Is this a detail brush (color 0,0,1 blue)
          {
          ...
          glColor4f(v[3] / divisor, v[4] / divisor, v[5] / divisor,
                    0.6); // divisor is one, bright colors
        };
      };
...
void GL_DrawLine(const Vector &start, const Vector &dir, float length, int r,
                 int g, int b) {
...
void WCam_Create(HINSTANCE hInstance) {
  ...
  camerawindow =
      CreateWindow("camera", "Camera View",
