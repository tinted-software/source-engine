...
enum FileUpdateStatus_t {
  ...
  FILEUPDATE_CRC_MATCH, // The CRC matched the last one that was shoved in there
                        // so it didn't have to update the symbol store.
  FILEUPDATE_UPDATED,
  FILEUPDATE_ERROR
};

typedef enum { FILETYPE_LOCAL = 0, FILETYPE_VSS, FILETYPE_PERFORCE } FileType_t;
...
public:
  ...
  char m_P4Client[256]; // If nonzero length, then this is a perforce binary we
                        // need to get.
  char m_BinaryName[256]; // Name of the dll or exe file.

  bool m_bDidCRC; // Used so we don't keep shoving the same things into the
                  // symbol store.
  CRC32_t m_LastCRC;

  enum {
    STATUS_OK,
    STATUS_ON_PROBATION // This means we sent an email warning about this file
                        // not being available. We can only get back to
                        // STATUS_OK when we can update the file successfully.
  };
  int m_Status;
};
...
void ParseConfigFile() {
  ...
  g_nUpdateIntervalSeconds =
      pKV->GetInt("UpdateIntervalSeconds", g_nUpdateIntervalSeconds);
  Msg("Update interval set to %d seconds.\n", g_nUpdateIntervalSeconds);

  // Create a tracker for each file in the info.
  for (KeyValues *pKey = pKV->GetFirstSubKey(); pKey;
    ...
    if (bVSSBinary || bLocalBinary || bPerforceBinary) {
      ...
      strncpy(pInfo->m_BinaryName, pKey->GetString(),
              sizeof(pInfo->m_BinaryName));
...
void InitFileSystem(const char *pchExeDir) {
  ...
  if (!Sys_LoadInterface("filesystem_stdio", FILESYSTEM_INTERFACE_VERSION,
                         &g_pFSModule, (void **)&g_pFileSystem)) {
...
int RunCommandGetOutput(const char *cmdLine, CUtlVector<char> &output) {
  ...
  Q_snprintf(fullCmdLine, sizeof(fullCmdLine), "%s > %s", cmdLine,
             pTempOutputFilename);
...
void GetFilenameBase(const char *pFilename, const char *&pPrefix,
...
bool GetMostRecentP4Revision(CFileInfo *pInfo, char syncCommand[512]) {
  ...
  if (sscanf(pNumber, "%d", &iRevisionNum) == 1) {
    if (pInfo->m_FileType == FILETYPE_PERFORCE) {
      ...
      Q_snprintf(syncCommand, 512, "p4 print -o \"%s\" \"%s\"@%d",
                 pFilenameBase, pInfo->m_BinaryName, iRevisionNum);
    } else {
      Q_snprintf(syncCommand, 512, "p4 sync //valvegames/main/src/...@%d",
                 iRevisionNum);
    }
...
bool GetLocalCopyOfFile(CFileInfo *pInfo, const char *&pPrefix,
                        const char *&pFilenameBase) {
  // Get the file.
  pPrefix = "";
  ...
  if (pInfo->m_FileType == FILETYPE_LOCAL) {
    ...
    Q_snprintf(
        ...
        "ss.exe get %s -I- -GLSymbolStoreUpdateTempFiles >> command_output.txt",
        pInfo->m_BinaryName);
    ret = system(cmdLine);
    ...
    Q_snprintf(cmdLine, sizeof(cmdLine),
               "p4 print -o \"SymbolStoreUpdateTempFiles\\%s\" \"%s\" >> "
               "command_output.txt",
               pFilenameBase, pInfo->m_BinaryName);
    ret = system(cmdLine);
...
void StoreP4Revision(CFileInfo *pInfo, const char *pFilenameBase) {
  if (pInfo->m_FileType != FILETYPE_VSS &&
      pInfo->m_FileType != FILETYPE_PERFORCE)
    return;
  ...
  if (GetMostRecentP4Revision(pInfo, syncCommand)) {
    // Now find the directory where it put the file and put the current src_main
    // Perforce revision up there.
    CUtlVector<char> output;
    Q_snprintf(cmdLine, sizeof(cmdLine),
               "symstore query /f SymbolStoreUpdateTempFiles\\%s /s %s",
               pFilenameBase, g_SymbolStoreDir);
    ...
    if (ret == 0) {
      ...
      if (pStr) {
        ...
        *pSpace = 0;
        ...
        Q_snprintf(revisionFilename, sizeof(revisionFilename),
                   "%s\\p4revision.txt", pStr);
...
FileUpdateStatus_t UpdateFileInSymbolStore(CFileInfo *pInfo) {
  ...
  Q_snprintf(fullFilename, sizeof(fullFilename), "%s%s", pPrefix,
             pFilenameBase);
  CRC32_t crc;
  ...
  // If the file's CRC is the same, then don't bother updating the symbol store
  // again.
  ...
  char cmdLine[512];
  Q_snprintf(cmdLine, sizeof(cmdLine),
             "symstore add /f \"%s\" /s \"%s\" /o /t SourceEngine >> "
             "command_output.txt",
...
int main(int argc, char **argv) {
  ...
  while (1) {
    ...
    if (GetTickCount() >= nextUpdateTime) {
      ...
      // For each file, grab its exe and put it in the store, then grab its PDB
      // and do the same.
      int iCount = 0;
      FOR_EACH_LL(g_FileInfos, j) {
        ...
        Msg("\rUpdated %d of %d - %d CRC matches...      ", iCount,
            g_FileInfos.Count(), nCRCMatches);
        if (kbhit())
          break;
      }
