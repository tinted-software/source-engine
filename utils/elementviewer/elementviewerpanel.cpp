...
// main editor panel
//-----------------------------------------------------------------------------
class CElementViewerPanel : public vgui::Panel,
                            public IDmNotify,
                            public CBaseElementPropertiesChoices {
...
public:
  ...
  int NumDocs() { return m_Docs.Count(); }

  CDmElement *GetRoot(int docNum) {
    return GetElement<CDmElement>(
        g_pDataModel->GetFileRoot(m_Docs[docNum].m_fileid));
  }
...
// CElementView *CreateView( vgui::Panel *parent, CDmElement *pRoot, const char
// *title );

CElementViewerPanel::CElementViewerPanel()
    : vgui::Panel(NULL, "ElementViewer") {
  ...
  pMenu->AddMenuItem("&New", new KeyValues("Command", "command", "OnNew"),
                     this);
  pMenu->AddMenuItem("&Open", new KeyValues("Command", "command", "OnOpen"),
                     this);
  pMenu->AddMenuItem("&Save", new KeyValues("Command", "command", "OnSave"),
                     this);
  pMenu->AddMenuItem("Save &As",
                     new KeyValues("Command", "command", "OnSaveAs"), this);
  pMenu->AddMenuItem("E&xit", new KeyValues("Command", "command", "OnExit"),
                     this);
  ...
  if (fileName) {
    // trim off any quotes (paths with spaces need to be quoted on the
    // commandline)
    char buf[MAX_PATH];
...
void CElementViewerPanel::NotifyDataChanged(const char *pReason,
                                            int nNotifySource,
                                            int nNotifyFlags) {
  //	if ( flags & INotifyUI::NOTIFY_REFRESH_PROPERTIES_VALUES ) // FIXME - do
  //we need new flags for file association changes?
  {
    m_pFileManager->Refresh();
...
void CElementViewerPanel::OnSaveAs() {
  ...
  FileOpenDialog *pFileOpenDialog =
      new FileOpenDialog(this, "Save .dmx File As", false, pContextKeyValues);

  const char *pFileFormat = g_pDataModel->GetFileFormat(fileid);
  const char *pDescription =
      (pFileFormat && *pFileFormat)
...
void CElementViewerPanel::OnOpen() {
  // Open file
  FileOpenDialog *pFileOpenDialog =
      new FileOpenDialog(this, "Choose .dmx file", true);
...
void CElementViewerPanel::OnFileSelected(KeyValues *pKeyValues) {
  ...
  if (pKeyValues->FindKey("OnSaveAs")) {
    ...
    // TODO - figure out which panel is on top, and save the file associated
    // with it

    int docCount = m_Docs.Count();
    if (docCount == 1) {
      g_pDataModel->SetFileName(m_Docs[0].m_fileid, pFullPath);
      g_pDataModel->SaveToFile(
          pFullPath, NULL, g_pDataModel->GetDefaultEncoding(pFormat), pFormat,
          GetElement<CDmElement>(
              g_pDataModel->GetFileRoot(m_Docs[0].m_fileid)));
    }
    return;
  }

  //	char relativepath[ 512 ];
  //	g_pFileSystem->FullPathToRelativePath( fullpath, relativepath, sizeof(
  //relativepath ) );
  ...
  DmFileId_t fileid = g_pDataModel->RestoreFromFile(pFullPath, NULL, NULL,
                                                    &pRoot, CR_DELETE_NEW);
  if (pRoot) {
    ViewerDoc_t doc;
    doc.m_fileid = fileid;
