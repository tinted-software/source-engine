...
void CElementViewerApp::Destroy() { BaseClass::Destroy(); }
...
bool CElementViewerApp::PreInit() {
  ...
  if (!g_pFullFileSystem || !g_pMaterialSystem || !g_pVGui || !g_pVGuiSurface ||
      !g_pDataModel || !g_pMatSystemSurface || !p4) {
...
// Purpose:
//-----------------------------------------------------------------------------
void VGui_RecursivePrintTree(int depth, int start, int end,
                             vgui::Panel *current, int &totaldrawn) {
  // No more room
  if (totaldrawn >= 128)
    return;
  ...
  for (int i = 0; i < count; i++) {
    ...
    if (depth >= start && depth <= end) {
      ...
      Msg(
          ...
          depth + 1, i + 1, panel, panel->IsVisible() ? "visible" : "hidden",
          panel->IsPopup() ? "popup" : "", x, y, width, height);
    }
...
void VGui_DrawPopups(void) {
  ...
  for (int i = 0; i < c; i++) {
    ...
    Msg("%i:  %s : %x, %s pos(%i,%i) w(%i) h(%i)\n", i, p, popup,
        visible ? "visible" : "hidden", x, y, width, height);
  }
}
