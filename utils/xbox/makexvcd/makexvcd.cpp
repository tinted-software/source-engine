...
static CUtlCachedFileData<CSceneCache>
    g_SceneCache("scene.cache", SCENECACHE_VERSION, 0,
...
void BuildFileList(char const *basedir, CUtlVector<CUtlSymbol> &files,
                   char const *rootdir, char const *extension) {
  files.RemoveAll();
  char root[512];
...
//-----------------------------------------------------------------------------
// Purpose: For each .wav file in the list, see if any vcd file in the list
// references it
//  First build an index of .wav to .vcd mappings, then search wav list and
//  print results
// Input  : vcdfiles -
//			wavfiles -
//-----------------------------------------------------------------------------
...
CChoreoScene *BlockingLoadScene(char const *vcdname) {
  ...
  Q_snprintf(scenefile, sizeof(scenefile), "%s\\%s", g_szCurrentGameDir,
             vcdname);

  LoadScriptFile(scenefile);

  CChoreoScene *scene =
      ChoreoLoadScene(scenefile, NULL, &g_TokenProcessor, Con_Printf);
  return scene;
}
...
void ProcessVCD(char const *vcdname, CUtlVector<CUtlSymbol> &disposition) {
  ...
  CChoreoScene *scene =
      ChoreoLoadScene(vcdname, NULL, &g_TokenProcessor, Con_Printf);
  if (scene) {
...
static CUtlMap<CStudioHdr *, MDLHandle_t> g_ModelMap(0, 0,
                                                     DefLessFunc(CStudioHdr *));

CStudioHdr *ModelSoundsCache_LoadModel(char const *filename) {
  MDLHandle_t handle = g_pMDLCache->FindMDL(filename);

  CStudioHdr *studioHdr =
      new CStudioHdr(g_pMDLCache->GetStudioHdr(handle), g_pMDLCache);
...
bool CMakeCachesApp::Create() {
  ...
  soundemitterbase = (ISoundEmitterSystemBase *)FindSystem(
...
int CMakeCachesApp::Main() {
  ...
  Q_snprintf(modelsdir, sizeof(modelsdir), "models",
  ...
  if (scenecache)
    filesystem->RemoveFile("scene.cache", "MOD");
  if (modelsoundscache)
    filesystem->RemoveFile("modelsounds.cache", "MOD");
  if (virtualmodel)
    filesystem->RemoveFile("virtualmodel.cache", "MOD");
