...
public:
  CImage(vgui::Panel *pParent, const char *pName, const char *pImageName)
  ...
  CImage(vgui::Panel *pParent, const char *pName, IMaterial *pMaterial)
      : BaseClass(pParent, pName) {
    m_imageID = vgui::surface()->CreateNewTextureID();
    ...
    if (m_Color.a() != 0) {
      ...
      if (m_AspectRatio > 1.0f) {
        if (m_bLetterbox) {
          ...
          vgui::surface()->DrawFilledRect(0, panelHeight - bandHeight,
                                          panelWidth, panelHeight);

          y = bandHeight;
          panelHeight = adjustedHeight;
        } else {
          // hold the panel's height constant, determine the corresponding
          // aspect corrected image width
      ...
      vgui::surface()->DrawTexturedSubRect(0, y, panelWidth, y + panelHeight,
                                           s0, 0.0f, s1, 1.0f);
    }
  }
...
public:
  CMovieImage(vgui::Panel *pParent, const char *pName, const char *pUniqueName,
              int nDecodeWidth, int nDecodeHeight, float fadeDelay = 1.0f)
      : BaseClass(pParent, pName) {
    // decoupled from actual panel bounds, can be decoded at any power of two
    // resolution
    m_nDecodeWidth = nDecodeWidth;
    m_nDecodeHeight = nDecodeHeight;

    char materialName[MAX_PATH];
    V_snprintf(materialName, sizeof(materialName), "MovieMaterial_%s.vmt",
               pUniqueName);
    ...
    m_pTexture = g_pMaterialSystem->CreateProceduralTexture(
        textureName, TEXTURE_GROUP_OTHER, m_nDecodeWidth, m_nDecodeHeight,
    ...
    m_pMaterial =
        g_pMaterialSystem->CreateMaterial(materialName, pVMTKeyValues);
  ...
  void UpdateMovieVolume(bool bForce, float frametime = 1.0f) {
    ...
    if (bForce || g_TargetMovieVolume != m_CurrentVolume) {
      ...
      m_CurrentVolume =
          Approach(g_TargetMovieVolume, m_CurrentVolume, frametime * 0.5f);

      // UNDONE: Under what conditions can this fail?  If it fails it could
      // cause audible pops
  ...
  bool StartMovieFromMemory(const void *pBuffer, int bufferSize, bool bAudio,
                            bool bLoop,
                            MovieEndCallback_t pMovieEndCallback = NULL) {
    ...
    IDirect3DDevice9 *pD3DDevice =
        (IDirect3DDevice9 *)g_pMaterialSystem->GetD3DDevice();
    HRESULT hr =
        XMediaCreateXmvPlayer(pD3DDevice, &xmvParameters, &m_pXMVPlayer);
    if (FAILED(hr)) {
      return false;
    }
  ...
  bool StartMovieFromFile(const char *pMovieName, bool bAudio, bool bLoop,
                          MovieEndCallback_t pMovieEndCallback = NULL) {
    if (m_pXMVPlayer || m_bStopping) {
      // already started or currently stopping
      return false;
  ...
  bool RenderVideoFrame() {
    ...
    // NOTE: This is only used to time audio fades, so if it's wrong by 2X it's
    // not going to be noticeable.  Probably fine to ship this.
...
public:
  CShadowLabel(vgui::Panel *pParent, const char *pName, const wchar_t *pText)
...
public:
  CGamePanel(vgui::Panel *pParent, const char *pName, game_t *pGame,
             bool bIsWidescreen, KeyValues *pKVSettings)
      : BaseClass(pParent, pName) {
    ...
    char szFilename[MAX_PATH];
    V_snprintf(szFilename, sizeof(szFilename), "vgui/appchooser/%s",
               pGame->pGameDir);
    ...
    // thumbnail movie
    m_pMovieImage =
        new CMovieImage(this, "Movie", pGame->pGameDir, 256, 256, 0.2f);
    SETUP_PANEL(m_pMovieImage);
    m_pMovieImage->SetBounds(x, y, w, h);
    m_pMovieImage->SetVisible(true);
    V_snprintf(szFilename, sizeof(szFilename), "%s\\thumb_%s.wmv", MOVIE_PATH,
               pGame->pGameDir);
    ...
    m_pTitleShadow->SetFont(
        pSourceScheme->GetFont("AppChooserGameTitleFontBlur"));
    ...
    m_pButtonText = new vgui::Label(
        this, "GameButton", g_pVGuiLocalize->Find("#GameUI_Icons_A_BUTTON"));
  ...
  void StartMovie() {
    m_pMovieImage->StartMovieFromMemory(m_MovieBuffer.Base(),
                                        m_MovieBuffer.TellMaxPut(), true, true);
  }
...
//-----------------------------------------------------------------------------
// Simple XAudio wrapper to instance a sound. Provides lightweight audio
// feedback for ui. Instance this per sound.
//-----------------------------------------------------------------------------
class CXSound {
public:
  CXSound() {
  ...
  //-----------------------------------------------------------------------------
  // Setup the wave, caller can clamp the number of simultaneous playing
  // instances of this sound. Useful for ui clicks to keep up with rapid input.
  //-----------------------------------------------------------------------------
  bool SetupWave(const char *pFilename, int numSimultaneous = 1) {
    ...
    m_pXMAData = (unsigned char *)XPhysicalAlloc(m_nXMADataSize, MAXULONG_PTR,
                                                 0, PAGE_READWRITE);
    V_memcpy(m_pXMAData, (unsigned char *)buf.Base() + pHeader->dataOffset,
             m_nXMADataSize);
...
bool CAppChooser::InitVGUI(void) {
  ...
  if (m_pKVSettings->LoadFromFile(g_pFullFileSystem,
                                  "resource/UI/AppChooser.res", "GAME")) {
  ...
  m_pBackgroundMovie = new CMovieImage(m_pRootPanel, "Movie", "Background",
                                       m_nScreenWidth, m_nScreenHeight, 0.0f);
  ...
  h = pKV->GetInt("tall");
  m_pProductTitle =
      new CImage(m_pRootPanel, "Logo", "vgui/appchooser/orangeboxlogo");
  ...
  for (int i = 0; i < ARRAYSIZE(g_Games); i++) {
    m_pGames[i] = new CGamePanel(m_pRootPanel, "GamePanel", &g_Games[i],
                                 bIsWidescreen, m_pKVSettings);
  ...
  for (int i = 0; i < ARRAYSIZE(g_Games); i++) {
    ...
    V_snprintf(filename, sizeof(filename), "vgui/appchooser/background_%s%s",
               g_Games[i].pGameDir, bIsWidescreen ? "_widescreen" : "");
  ...
  m_pLoadingIcon =
      new CImage(m_pRootPanel, "LoadingIcon", "vgui/appchooser/loading_icon");
  ...
  if (XboxLaunch()->GetLaunchFlags() & LF_INTERNALLAUNCH) {
    pTexture = g_pMaterialSystem->CreateProceduralTexture(
        "PersistedTexture", TEXTURE_GROUP_OTHER, m_nScreenWidth,
        m_nScreenHeight, g_pMaterialSystem->GetBackBufferFormat(),
        TEXTUREFLAGS_NOMIP | TEXTUREFLAGS_SINGLECOPY);
  ...
  // the fade-in material is topmost, fully opaque, and fades to transluscent on
  // initial rendering
  KeyValues *pVMTKeyValues = new KeyValues("UnlitGeneric");
  pVMTKeyValues->SetInt("$vertexcolor", 1);
  pVMTKeyValues->SetInt("$vertexalpha", 1);
  pVMTKeyValues->SetInt(
      "$linearwrite",
      1); // These 2 lines are needed so that we don't lose bits of precision in
  pVMTKeyValues->SetInt(
      "$gammacolorread",
      1); // the dark colors due to the 360's lossy sRGB read hardware
  pVMTKeyValues->SetInt("$ignorez", 1);
  if (pTexture) {
  ...
  IMaterial *pFadeInMaterial =
      g_pMaterialSystem->CreateMaterial("__FadeInMaterial.vmt", pVMTKeyValues);

  // the persisted image is either the image from the relaunch or black during
  // first boot
  m_pPersistedImage =
      new CImage(m_pRootPanel, "FadeInOverlay", pFadeInMaterial);
  ...
  m_pDemoMovieImage = new CMovieImage(m_pRootPanel, "Movie", "Demo",
                                      m_nScreenWidth, m_nScreenHeight, 2.0f);
  ...
  m_pStartupMovieImage = new CMovieImage(m_pRootPanel, "Movie", "Startup",
                                         m_nScreenWidth, m_nScreenHeight, 0.0f);
...
bool CAppChooser::InitAudio() {
  ...
  EngineInit.ThreadUsage =
      XAUDIOTHREADUSAGE_THREAD4 | XAUDIOTHREADUSAGE_THREAD5;
...
void CAppChooser::ExitChooser() {
  ...
  // relaunches, so remove it first in case we came from tf and are going to
  // another game.
  ...
  if (m_bInviteAccepted) {
    // In the case of an invitation acceptance, we need to pack extra data into
    // the payload
  ...
  // Save out the data
  bool bLaunch =
      XboxLaunch()->SetLaunchData((void *)pPayload, nPayloadSize, fFlags);
  if (bLaunch) {
...
void CAppChooser::OnActivateGame() {
  if (!m_FadeInTime || Plat_FloatTime() <= m_FadeInTime + 1.0f) {
    // lockout user selection input until screen has stable rendering and
    // completed its fade in prevents button mashing doing an immediate game
    // selection just as the startup movies end
    return;
  }
...
void CAppChooser::OnInactivityTimeout() {
  ...
  m_bPlayingDemoMovie = m_pDemoMovieImage->StartMovieFromFile(
      pDemoMovieName, true, false, DemoMovieCallback);
...
void CAppChooser::FrameTick() {
  ...
  if (m_bExiting) {
    ...
    if (m_pOverlay->GetAlpha() == 255) {
      // exiting needs to have fade overlay fully opaque before stopping
      // background movie
      m_pBackgroundMovie->StopMovie();

      // exiting commences after all movies full release and enough frames have
      // swapped to ensure stability strict time (frame rate) cannot be trusted,
      // must allow a non trivial amount of presents
  ...
  if (m_bPlayingStartupMovies) {
    if (m_pStartupMovieImage->IsFullyReleased()) {
      ...
      if (m_StartupMovie >= ARRAYSIZE(g_StartupMovies)) {
        ...
        m_bPlayingStartupMovies = m_pStartupMovieImage->StartMovieFromFile(
            g_StartupMovies[m_StartupMovie].pMovieName, true, false);
      }
    }
    ...
    if (!m_bPlayingStartupMovies && !m_bPlayingDemoMovie && !m_bExiting) {
      for (int i = 0; i < ARRAYSIZE(g_Games); i++) {
        if (V_stristr("tf", g_Games[i].pGameDir)) {
          ...
          // must fixup state, invite could have happened very early, at any
          // time, before the initial fade-in ensure no movies start
          m_BackgroundMovieStartTime = 0;
          m_GameMovieStartTime = 0;
  ...
  if (!m_bExiting && !m_bPlayingDemoMovie && (m_GameMovieStartTime != 0) &&
  ...
  if (!m_bExiting && !m_bPlayingDemoMovie &&
      (m_BackgroundMovieStartTime != 0) &&
      (Plat_FloatTime() >= m_BackgroundMovieStartTime)) {
    char szFilename[MAX_PATH];
    V_snprintf(szFilename, sizeof(szFilename), "background_%s.wmv",
               g_Games[m_Selection].pGameDir);

    // keep trying the current selection, it will eventually start
    bool bStarted =
        m_pBackgroundMovie->StartMovieFromFile(szFilename, false, true);
    if (bStarted) {
      m_LastBackgroundMovie = m_Selection;
    }
  }
...
void CAppChooser::RenderScene() {
  ...
  if (m_ExitingFrameCount > 1 && m_pBackgroundMovie->IsFullyReleased() &&
      m_pGames[m_Selection]->m_pMovieImage->IsFullyReleased()) {
...
//-----------------------------------------------------------------------------
extern "C" __declspec(dllexport) int AppChooserMain(HINSTANCE hInstance,
