...
void LoadPHYFile(phyfile_t *pOut, const char *name) {
  ...
  physcollision->VCollideLoad(&pOut->collide, pOut->header.solidCount,
                              (const char *)buf, pOut->fileSize);
}
...
void OverrideDefaultsForModel(const char *keyname, simplifyparams_t &params) {
  ...
  while (pKeys) {
    if (!Q_stricmp(pKeys->GetName(), keyname)) {
      for (KeyValues *pData = pKeys->GetFirstSubKey(); pData;
        ...
        if (!Q_stricmp(pData->GetName(), "tolerance")) {
          ...
          if (!g_bQuiet) {
            Msg("%s: AABB %s\n", keyname,
                params.addAABBToSimplifiedHull ? "on" : "off");
          }
          ...
          params.mergeConvexElements =
              params.mergeConvexTolerance > 0 ? true : false;
          if (!g_bQuiet) {
            Msg("%s: Merge convex %.2f\n", keyname,
                params.mergeConvexTolerance);
          }
        }
      }
      return;
...
void WritePHXFile(const char *pName, const phyfile_t &file) {
  ...
  params.addAABBToSimplifiedHull =
      (file.collide.solidCount == 1) ? true : false;
  ...
  vcollide_t *pNewCollide = ConvertVCollideToPHX(
      &file.collide, params, &outSize, false, bStoreSolidNames);
  ...
  if (!g_bQuiet) {
    Msg("%s Compressed %d (%d text) to %d (%d text)\n", outName, file.fileSize,
        file.collide.descSize, out.TellPut(), pNewCollide->descSize);
  }
...
void MakeFilename(char *pDest, int destSize, const char *pPathname,
                  const char *pFilenameExt) {
  Q_strncpy(pDest, pPathname, destSize);
  Q_AppendSlash(pDest, destSize);
  Q_strncat(pDest, pFilenameExt, destSize);
}

void MakeDirname(char *pDest, int destSize, const char *pPathname,
                 const char *pSubdir) {
...
int main(int argc, char *argv[]) {
  ...
  do {
    if (g_bRecursive) {
      ...
      const char *pFilename =
          g_pFullFileSystem->FindFirst(currentFile, &handle);
      while (pFilename) {
        if (pFilename[0] != '.' && g_pFullFileSystem->FindIsDirectory(handle)) {
          MakeDirname(currentFile, sizeof(currentFile), directoryList[current],
                      pFilename);
  ...
  if (count) {
    if (!g_bQuiet) {
      Msg("\n------\nTotal %s, %s\nSaved %s\n", Q_pretifymem(g_TotalOut),
          Q_pretifymem(g_TotalCompress),
          Q_pretifymem(g_TotalOut - g_TotalCompress));
      Msg("%.2f%% savings\n",
