...
void WritePHXFile(const char *pName, const phyfile_t &file) {
  ...
  vcollide_t *pNewCollide =
      ConvertVCollideToPHX(&file.collide, tolerance, &outSize, false);
  ...
  CPackedPhysicsDescription *pPacked = physcollision->CreatePackedDesc(
      pNewCollide->pKeyValues, pNewCollide->descSize);
  Msg("NEW:\n-----------------------------------\n");
...
#endif
    g_pFullFileSystem->RelativePathToFullPath(pFilename, NULL, filebase,
                                              sizeof(filebase));
