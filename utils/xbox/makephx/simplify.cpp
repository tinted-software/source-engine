...
// this is the list of candidate planes that will be added one by one to the
// convex hull until none of the surface lies outside the tolerance
struct planetest_t {
  Vector normal;
  float dist;
  int inUse;
...
struct convexoptimize_t {
  ...
  void InitPlanes(CPhysCollide *pCollide, bool addAABBToSimplifiedHull) {
    ...
    for (i = 0; i < pQuery->TriangleCount(0); i++) {
      ...
      elem.bestDist =
          DotProduct(elem.normal, BoxSupport(elem.normal, mins, maxs)) -
          elem.dist;
    }
    physcollision->DestroyQueryModel(pQuery);
  }

  CPhysConvex *ConvertToConvex() { return ::ConvertPlaneListToConvex(list); }
  ...
  bool AddBestPlane() {
    ...
    CPhysCollide *pCurrentCollide =
        physcollision->ConvertConvexToCollideParams(&pConvex, 1, params);
    int bestIndex = -1;
    float bestDist = 0;
    while (true) {
      if (bestIndex >= 0) {
      ...
      Vector point = physcollision->CollideGetExtent(
          pCurrentCollide, vec3_origin, vec3_angle, dir);
      ...
      CPhysCollide *pCollide =
          physcollision->ConvertConvexToCollideParams(&pConvex, 1, params);
      Vector p2 = physcollision->CollideGetExtent(pCollide, vec3_origin,
                                                  vec3_angle, dir);
...
CPhysConvex *SimplifyConvexFromVerts(Vector **verts, int vertCount,
                                     bool addAABBToSimplifiedHull,
                                     float tolerance, int index) {
...
void BuildSingleConvex(CPhysConvex **convexListOut, ICollisionQuery *pQuery,
                       Vector **ppVerts, const simplifyparams_t &params) {
  int vertCount = 0;
  ...
  convexListOut[0] = SimplifyConvexFromVerts(
      ppVerts, vertCount, params.addAABBToSimplifiedHull, params.tolerance, 0);
  physcollision->SetConvexGameData(convexListOut[0], pQuery->GetGameData(0));
}

void SimplifyConvexElements(CPhysConvex **convexListOut,
                            ICollisionQuery *pQuery, Vector **ppVerts,
                            const simplifyparams_t &params) {
  for (int i = 0; i < pQuery->ConvexCount(); i++) {
    int vertCount = 0;
    Vector v[3];
    for (int j = 0; j < pQuery->TriangleCount(i); j++) {
    ...
    convexListOut[i] = SimplifyConvexFromVerts(ppVerts, vertCount,
                                               params.addAABBToSimplifiedHull,
                                               params.tolerance, i);
...
int VertsForElem(ICollisionQuery *pQuery, Vector **ppVerts,
                 const mergeconvex_t &elems0, int vertCount) {
...
void PlanesForElem(ICollisionQuery *pQuery, CUtlVector<float> &planes,
                   const mergeconvex_t &elem0) {
...
float ConvexVolumeFromPlanes(CUtlVector<float> &planes) {
  CPhysConvex *pConvex =
      planes.Count() ? physcollision->ConvexFromPlanes(
                           planes.Base(), planes.Count() / 4, DIST_EPSILON)
                     : NULL;
  float volume = 0;
  if (pConvex) {
...
float MergedDeltaVolume(ICollisionQuery *pQuery, Vector **ppVerts,
                        const mergeconvex_t &elem0,
                        const mergeconvex_t &elem1) {
  // build vert list
...
int MergeAndSimplifyConvexElements(CPhysConvex **convexListOut,
                                   const CPhysCollide *pCollideIn,
                                   ICollisionQuery *pQuery, Vector **ppVerts,
                                   const simplifyparams_t &params) {
...
loop:
  for (i = 0; i < elems.Count(); i++) {
    for (int j = i + 1; j < elems.Count(); j++) {
      float volume =
          fabs(MergedDeltaVolume(pQuery, ppVerts, elems[i], elems[j]));
      volume /= totalVolume;
  ...
  for (i = 0; i < elems.Count(); i++) {
    ...
    convexListOut[i] = SimplifyConvexFromVerts(ppVerts, vertCount,
                                               params.addAABBToSimplifiedHull,
                                               params.tolerance, i);
    physcollision->SetConvexGameData(convexListOut[i],
                                     pQuery->GetGameData(elems[i].list[0]));
  }
  return elems.Count();
}

CPhysCollide *SimplifyCollide(CPhysCollide *pCollideIn, int indexIn,
                              const simplifyparams_t &params) {
  ...
  if (params.forceSingleConvex) {
    ...
    outputConvexCount = MergeAndSimplifyConvexElements(convexList, pCollideIn,
                                                       pQuery, ppVerts, params);
  ...
  CPhysCollide *pCollideOut = physcollision->ConvertConvexToCollideParams(
      convexList, outputConvexCount, params);
