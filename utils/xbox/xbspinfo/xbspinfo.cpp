...
lumpName_t g_lumpNames[] = {
    ...
    {"Disp Lightmap Alphas", "LUMP_DISP_LIGHTMAP_ALPHAS",
     LUMP_DISP_LIGHTMAP_ALPHAS},
    {"Disp Verts", "LUMP_DISP_VERTS", LUMP_DISP_VERTS},
    {"Disp Lightmap Sample Positions", "LUMP_DISP_LIGHTMAP_SAMPLE_POSITIONS",
    ...
    {"Tex Data String Data", "LUMP_TEXDATA_STRING_DATA",
     LUMP_TEXDATA_STRING_DATA},
    {"Tex Data String Table", "LUMP_TEXDATA_STRING_TABLE",
     LUMP_TEXDATA_STRING_TABLE},
    {"Overlays", "LUMP_OVERLAYS", LUMP_OVERLAYS},
    {"Leaf Min Dist To Water", "LUMP_LEAFMINDISTTOWATER",
     LUMP_LEAFMINDISTTOWATER},
    {"Face Macro Texture Info", "LUMP_FACE_MACRO_TEXTURE_INFO",
     LUMP_FACE_MACRO_TEXTURE_INFO},
    {"Disp Tris", "LUMP_DISP_TRIS", LUMP_DISP_TRIS},
    {"Phys Collide Surface", "LUMP_PHYSCOLLIDESURFACE",
     LUMP_PHYSCOLLIDESURFACE},
    {"Water Overlays", "LUMP_WATEROVERLAYS", LUMP_WATEROVERLAYS},
    {"Leaf Ambient index HDR", "LUMP_LEAF_AMBIENT_INDEX_HDR",
    ...
    {"Leaf Ambient Lighting (HDR)", "LUMP_LEAF_AMBIENT_LIGHTING_HDR",
     LUMP_LEAF_AMBIENT_LIGHTING_HDR},
    {"Leaf Ambient Lighting", "LUMP_LEAF_AMBIENT_LIGHTING",
...
void ExtractZip(const char *pFilename, void *pBSPFile) {
  ...
  if (pBSPHeader->lumps[LUMP_PAKFILE].filelen) {
    ...
    fwrite((unsigned char *)pBSPFile + pBSPHeader->lumps[LUMP_PAKFILE].fileofs,
           pBSPHeader->lumps[LUMP_PAKFILE].filelen, 1, fp);
    fclose(fp);
  } else {
    Msg("Nothing to do!\n");
  }
...
void DumpInfo(const char *pFilename, void *pBSPFile, int bspSize) {
  ...
  for (int i = 0; i < HEADER_LUMPS; ++i) {
    ...
    if (g_bAsPercent) {
      Msg("%5.2f%s (%2d) %s\n",
          100.0f * pBSPHeader->lumps[lump].filelen / (float)bspSize, "%%", lump,
          pName);
    } else if (g_bAsBytes) {
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 2 || CommandLine()->FindParm("?") ||
  ...
  g_bAsPercent = CommandLine()->FindParm("-p") != 0 ||
                 CommandLine()->FindParm("-percent") != 0;
  g_bAsBytes = CommandLine()->FindParm("-b") != 0 ||
               CommandLine()->FindParm("-bytes") != 0;
