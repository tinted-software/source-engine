...
void TL_Setup(char *appname, int argc, char **argv) {
  ...
  g_tl_quiet = (TL_CheckParm("q") > 0) || (TL_CheckParm("quiet") > 0) ||
               (TL_CheckParm("noheader") > 0);
...
int TL_SafeOpenRead(const char *filename) {
  ...
  if (handle == -1)
    TL_Error("TL_SafeOpenRead(): Error opening %s: %s", filename,
             strerror(errno));
...
int TL_SafeOpenWrite(const char *filename) {
  ...
  if (handle == -1)
    TL_Error("TL_SafeOpenWrite(): Error opening %s: %s", filename,
             strerror(errno));
...
*****************************************************************************/
void TL_ReplaceExtension(const char *inPath, const char *extension,
                         char *outPath) {
  int len;
  char *src;
...
*****************************************************************************/
int TL_GetByteOrder(void) { return (g_tl_byteorder); }
...
*****************************************************************************/
void TL_SetByteOrder(int flag) { g_tl_byteorder = flag; }
...
*****************************************************************************/
short TL_LittleShort(short l) { return (l); }
...
*****************************************************************************/
long TL_LittleLong(long l) { return (l); }
...
int TL_GetFileList(char *dirpath, char *pattern, tlfile_t ***filelist) {
  ...
  do {
    ...
    if (!filecount)
      ...
      *filelist = (tlfile_t **)TL_Realloc(*filelist,
                                          (filecount + 1) * sizeof(tlfile_t *));
...
void _RecurseFileTree(char *dirpath, int depth) {
  ...
  if (!numfiles) {
    ...
    if (!g_tl_dircount)
      ...
      g_tl_dirlist = (char **)TL_Realloc(g_tl_dirlist,
                                         (g_tl_dircount + 1) * sizeof(char *));
  ...
  g_tl_dirlist =
      (char **)TL_Realloc(g_tl_dirlist, (g_tl_dircount + 1) * sizeof(char *));
...
//-----------------------------------------------------------------------------
bool TL_IsWildcardMatch(const char *wildcardString, const char *stringToCheck,
                        bool caseSensitive) {
  char wcChar;
  char strChar;
