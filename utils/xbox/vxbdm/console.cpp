...
DLL_EXPORT IXboxConsole *GetConsoleInterface() { return &XboxConsole; }

//-----------------------------------------------------------------------------
//	Low level string output.
//	Input string should be stack based, can get clobbered.
//-----------------------------------------------------------------------------
static void OutputStringToDevice(unsigned int color, char *pString,
                                 bool bRemoteValid) {
  if (!bRemoteValid) {
    // local debug only
  ...
  while (pStart < pEnd) {
    ...
    int length = _snprintf(strBuffer, XBX_MAX_RCMDLENGTH, "%s!%s%s",
                           XBX_DBGPRINTPREFIX, colorString, pStart);
    if (length == -1) {
...
//
//-----------------------------------------------------------------------------
HRESULT __stdcall _DebugCommandHandler(const CHAR *strCommand,
  ...
  if (_xdbg_strnicmp(args[0], "__connect__", 11)) {
    if (numArgs > 1 && atoi(args[1]) == VXCONSOLE_PROTOCOL_VERSION) {
      ...
      _xdbg_strcpyn(strResponse,
                    XBX_DBGRESPONSEPREFIX "Connected To Application.",
                    dwResponseLen);
      ...
      _xdbg_strcpyn(strResponse,
                    XBX_DBGRESPONSEPREFIX
                    "Rejecting Connection: Wrong Protocol Version.",
                    dwResponseLen);
    }
    goto cleanUp;
  }
  ...
  if (_xdbg_strnicmp(args[0], "__disconnect__", 14)) {
    ...
    _xdbg_strcpyn(strResponse, XBX_DBGRESPONSEPREFIX "Disconnected.",
                  dwResponseLen);
  ...
  if (_xdbg_strnicmp(args[0], "__memory__", 10)) {
    ...
    if (_xdbg_findtoken(args, numArgs, "quiet") > 0) {
      ...
      _snprintf(strResponse, dwResponseLen,
                XBX_DBGRESPONSEPREFIX
                "Available: %.2f MB, Used: %.2f MB, Free: %.2f MB",
                stat.dwTotalPhys / (1024.0f * 1024.0f) - 32.0f,
                (stat.dwTotalPhys - stat.dwAvailPhys) / (1024.0f * 1024.0f) -
                    32.0f,
  ...
  if (g_xbx_dbgRemoteBuf[0]) {
    ...
    _xdbg_strcpyn(strResponse,
                  XBX_DBGRESPONSEPREFIX
                  "Cannot execute: Previous command still pending",
                  dwResponseLen);
  } else {
...
void CXboxConsole::SendRemoteCommand(const char *pCommand, bool async) {
  ...
  _snprintf(cmdString, sizeof(cmdString), "%s!%s", XBX_DBGCOMMANDPREFIX,
            pCommand);
  ...
  if (!async) {
    ...
    if (WaitForSingleObject(g_xbx_dbgCmdCompleteEvent, timeout) ==
        WAIT_TIMEOUT) {
...
static unsigned _DebugThreadFunc(void *pParam) {
  while (1) {
    ...
    if (!g_xbx_DebugStringQueue.Count() && !g_xbx_numProfileCounters &&
        !g_xbx_freeMemory) {
      continue;
    }
    ...
    if (g_xbx_numProfileCounters) {
      ...
      _snprintf(dbgCommand, sizeof(dbgCommand), "SetProfileData() %s 0x%8.8x",
                g_xbx_profileName, g_xbx_profileCounters);
    ...
    if (g_xbx_freeMemory) {
      // build and send asynchronously
      char dbgCommand[XBX_MAX_RCMDLENGTH];
      _snprintf(dbgCommand, sizeof(dbgCommand), "FreeMemory() 0x%8.8x",
                g_xbx_freeMemory);
      XBX_SendRemoteCommand(dbgCommand, true);
    ...
    while (1) {
      ...
      OutputStringToDevice(debugString.color, debugString.pString,
                           bRemoteValid);
...
void CXboxConsole::InitConsoleMonitor(bool bWaitForConnect) {
  ...
  HRESULT hr =
      DmRegisterCommandProcessor(XBX_DBGCOMMANDPREFIX, _DebugCommandHandler);
  if (FAILED(hr)) {
  ...
  ThreadId_t threadID;
  g_xbx_hDebugThread =
      CreateSimpleThread(_DebugThreadFunc, NULL, &threadID, 16 * 1024);
...
void CXboxConsole::DisconnectConsoleMonitor() {
  ...
  // caller is trying to safely stop vxconsole traffic, disconnect must be
  // synchronous
...
void CXboxConsole::CrashDump(bool b) { DmCrashDump(b); }
  ...
  do {
    ...
    if (XBDM_NOERR == error && !stricmp(modLoad.Name, pName)) {
      Msg("0x%8.8x, %5.2f MB, %s\n", modLoad.BaseAddress,
          modLoad.Size / (1024.0f * 1024.0f), modLoad.Name);
      size = modLoad.Size;
...
//-----------------------------------------------------------------------------
char const *
HACK_stristr(char const *pStr,
             char const *pSearch) // hack because moved code from above vstdlib
{
...
void CXboxConsole::DumpDllInfo(const char *pBasePath) {
  ...
  static char *dllDirs[] = {"bin", "hl2\\bin", "tf\\bin", "portal\\bin",
                            "episodic\\bin"};
    ...
    if (INVALID_HANDLE_VALUE == hFind) {
      ...
      do {
        if (!HACK_stristr(wfd.cFileName, "_360.dll")) {
          ...
          // FindFirstFile does not support a spec mask of *_360.dll on the Xbox
          // HDD
          continue;
        }
...
void CXboxConsole::OutputDebugString(const char *p) { ::OutputDebugStringA(p); }

bool CXboxConsole::IsDebuggerPresent() { return (DmIsDebuggerPresent() != 0); }
