...
//-----------------------------------------------------------------------------
int CXboxConsole::SetProfileAttributes(const char *pProfileName,
                                       int numCounters, const char *names[],
                                       unsigned int colors[]) {
  ...
  for (int i = 0; i < numCounters; i++) {
    strncpy(profileList[i].labelString, names[i],
            sizeof(profileList[i].labelString));
    profileList[i].labelString[sizeof(profileList[i].labelString) - 1] = '\0';

    profileList[i].color = colors[i];
  }

  _snprintf(dbgCommand, sizeof(dbgCommand),
            "SetProfile() %s 0x%8.8x 0x%8.8x 0x%8.8x", pProfileName,
            numCounters, profileList, &retVal);
...
//	XBX_rSetProfileData
//
//	Expose profile counters to the console. Expected to be called once per
//game frame.
//-----------------------------------------------------------------------------
void CXboxConsole::SetProfileData(const char *pProfileName, int numCounters,
  ...
  if (g_xbx_numProfileCounters == 0) {
    ...
    memcpy(g_xbx_profileCounters, pCounters,
           numCounters * sizeof(unsigned int));
...
int CXboxConsole::TimeStampLog(float time, const char *pString) {
  ...
  _snprintf(dbgCommand, sizeof(dbgCommand), "TimeStampLog() 0x%8.8x 0x%8.8x",
            &timeStamp, &retVal);
  XBX_SendRemoteCommand(dbgCommand, false);
...
//-----------------------------------------------------------------------------
int CXboxConsole::MaterialList(int nMaterials,
                               const xMaterialList_t *pXMaterialList) {
  char dbgCommand[XBX_MAX_RCMDLENGTH];
  int retVal;
  ...
  for (int i = 0; i < nMaterials; i++) {
    strncpy(pRemoteList[i].nameString, pXMaterialList[i].pName,
            sizeof(pRemoteList[i].nameString));
    pRemoteList[i].nameString[sizeof(pRemoteList[i].nameString) - 1] = '\0';

    strncpy(pRemoteList[i].shaderString, pXMaterialList[i].pShaderName,
            sizeof(pRemoteList[i].shaderString));
  ...
  _snprintf(dbgCommand, sizeof(dbgCommand),
            "MaterialList() 0x%8.8x 0x%8.8x 0x%8.8x", nMaterials, pRemoteList,
            &retVal);
  XBX_SendRemoteCommand(dbgCommand, false);
...
//-----------------------------------------------------------------------------
int CXboxConsole::TextureList(int nTextures,
                              const xTextureList_t *pXTextureList) {
  char dbgCommand[XBX_MAX_RCMDLENGTH];
  int retVal;
  ...
  for (int i = 0; i < nTextures; i++) {
    strncpy(pRemoteList[i].nameString, pXTextureList[i].pName,
            sizeof(pRemoteList[i].nameString));
    pRemoteList[i].nameString[sizeof(pRemoteList[i].nameString) - 1] = '\0';

    strncpy(pRemoteList[i].groupString, pXTextureList[i].pGroupName,
            sizeof(pRemoteList[i].groupString));
    pRemoteList[i].groupString[sizeof(pRemoteList[i].groupString) - 1] = '\0';

    strncpy(pRemoteList[i].formatString, pXTextureList[i].pFormatName,
            sizeof(pRemoteList[i].formatString));
  ...
  _snprintf(dbgCommand, sizeof(dbgCommand),
            "TextureList() 0x%8.8x 0x%8.8x 0x%8.8x", nTextures, pRemoteList,
            &retVal);
  XBX_SendRemoteCommand(dbgCommand, false);
...
int CXboxConsole::SoundList(int nSounds, const xSoundList_t *pXSoundList) {
  ...
  for (int i = 0; i < nSounds; i++) {
    strncpy(pRemoteList[i].nameString, pXSoundList[i].name,
            sizeof(pRemoteList[i].nameString));
    pRemoteList[i].nameString[sizeof(pRemoteList[i].nameString) - 1] = '\0';

    strncpy(pRemoteList[i].formatString, pXSoundList[i].formatName,
            sizeof(pRemoteList[i].formatString));
    pRemoteList[i].formatString[sizeof(pRemoteList[i].formatString) - 1] = '\0';
  ...
  _snprintf(dbgCommand, sizeof(dbgCommand),
            "SoundList() 0x%8.8x 0x%8.8x 0x%8.8x", nSounds, pRemoteList,
            &retVal);
  XBX_SendRemoteCommand(dbgCommand, false);
...
int CXboxConsole::MapInfo(const xMapInfo_t *pXMapInfo) {
  ...
  _snprintf(dbgCommand, sizeof(dbgCommand), "MapInfo() 0x%8.8x 0x%8.8x",
            &xrMapInfo, &retVal);
  XBX_SendRemoteCommand(dbgCommand, false);
...
//-----------------------------------------------------------------------------
int CXboxConsole::AddCommands(int numCommands, const char *commands[],
                              const char *help[]) {
  char dbgCommand[XBX_MAX_RCMDLENGTH];
  int retVal;
  xrCommand_t *cmdList;
  ...
  _snprintf(dbgCommand, sizeof(dbgCommand),
            "AddCommands() 0x%8.8x 0x%8.8x 0x%8.8x", numCommands, cmdList,
            &retVal);
  XBX_SendRemoteCommand(dbgCommand, false);
