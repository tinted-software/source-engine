...
const short CImaAdpcmCodec::m_asNextStep[16] = {-1, -1, -1, -1, 2, 4, 6, 8,
                                                -1, -1, -1, -1, 2, 4, 6, 8};
...
BOOL CImaAdpcmCodec::Initialize(LPCIMAADPCMWAVEFORMAT pwfxEncode,
                                CODEC_MODE cmCodecMode) {
  static const LPFNIMAADPCMCONVERT apfnConvert[2][2] = {{DecodeM16, DecodeS16},
                                                        {EncodeM16, EncodeS16}};
  ...
  m_pfnConvert = apfnConvert[!(m_cmCodecMode == CODEC_MODE_DECODE)]
                            [m_wfxEncode.wfx.nChannels - 1];
...
BOOL CImaAdpcmCodec::Convert(LPCVOID pvSrc, LPVOID pvDst, UINT cBlocks) {
  // Array of decoders
  static const LPFNIMAADPCMCONVERT apfnDecoders[2] = {DecodeM16, DecodeS16};

  // Both destination and source block sizes
  DWORD dwSrcBlockSize =
      m_wfxEncode.wfx.nChannels * m_wfxEncode.wSamplesPerBlock * sizeof(short);
  ...
  case CODEC_MODE_DECODE:
    ...
    return m_pfnConvert(
        (LPBYTE)pvSrc, (LPBYTE)pvDst, cBlocks, m_wfxEncode.wfx.nBlockAlign,
        m_wfxEncode.wSamplesPerBlock, &m_nStepIndexL, &m_nStepIndexR);
  ...
  case CODEC_MODE_ENCODE_NORMAL:
    ...
    // Otherwise, it would be the same as CODEC_MODE_DECODE
    {
      ...
      // Encode the stream
      if (!m_pfnConvert((LPBYTE)pvSrc, (LPBYTE)pvDst, cBlocks,
                        m_wfxEncode.wfx.nBlockAlign,
                        m_wfxEncode.wSamplesPerBlock, &m_nStepIndexL,
                        &m_nStepIndexR)) {
        delete[] pvDecoded;
        return FALSE;
      }
      ...
      // Decode it back
      if (!pfnOppConvert(
              (LPBYTE)pvDst, pvDecoded, cBlocks, m_wfxEncode.wfx.nBlockAlign,
              XBOX_ADPCM_SAMPLES_PER_BLOCK, &m_nStepIndexL, &m_nStepIndexR)) {
        delete[] pvDecoded;
        return FALSE;
      }
      ...
      printf("Difference between original and decoded streams: 0x%I64x\n",
             CalcDifference((LPBYTE)pvSrc, pvDecoded, cBlocks, cBlocks,
                            dwSrcBlockSize));
  ...
  case CODEC_MODE_ENCODE_OPTIMIZE_WHOLE_FILE:
    ...
    // and pick the best one
    {
      ...
      for (UINT i = 0; i < ARRAYSIZE(m_asStep); ++i) {
        ...
        if (!m_pfnConvert(
                (LPBYTE)pvSrc, pvTempDst, cBlocks, m_wfxEncode.wfx.nBlockAlign,
                m_wfxEncode.wSamplesPerBlock, &m_nStepIndexL, &m_nStepIndexR))
          continue;

        // Decode
        if (!pfnOppConvert(
                pvTempDst, pvDecoded, cBlocks, m_wfxEncode.wfx.nBlockAlign,
                XBOX_ADPCM_SAMPLES_PER_BLOCK, &m_nStepIndexL, &m_nStepIndexR))
          continue;

        // Diff
        ULONGLONG ullDiff = CalcDifference((LPBYTE)pvSrc, pvDecoded, cBlocks,
                                           cBlocks, dwSrcBlockSize);
    ...
    // best one for each block
    {
      ...
      for (UINT c = 0; c < cBlocks; ++c) {
        ...
        for (INT i = iStartIndex; i < iStopIndex; ++i) {
          ...
          // Encode
          m_nStepIndexL = m_nStepIndexR = i;
          if (!m_pfnConvert((LPBYTE)pvSrc + c * dwSrcBlockSize, pvTempDst, 1,
                            m_wfxEncode.wfx.nBlockAlign,
                            m_wfxEncode.wSamplesPerBlock, &m_nStepIndexL,
                            &m_nStepIndexR))
            continue;
          ...
          // Decode
          if (!pfnOppConvert(
                  pvTempDst, pvDecoded, 1, m_wfxEncode.wfx.nBlockAlign,
                  XBOX_ADPCM_SAMPLES_PER_BLOCK, &m_nStepIndexL, &m_nStepIndexR))
            continue;

          // Diff
          ULONGLONG ullDiff =
              CalcDifference((LPBYTE)pvSrc + c * dwSrcBlockSize, pvDecoded, 1,
                             cBlocks, dwSrcBlockSize);
          if (ullDiff < ullLeastDiff) {
            ullLeastDiff = ullDiff;
            iThisBestStepIndex = i;
            CopyMemory((LPBYTE)pvDst + c * dwDstBlockSize, (LPBYTE)pvTempDst,
                       dwDstBlockSize);
          }
        }
      ...
      pfnOppConvert((LPBYTE)pvDst, pvDecoded, cBlocks,
                    m_wfxEncode.wfx.nBlockAlign, XBOX_ADPCM_SAMPLES_PER_BLOCK,
                    &m_nStepIndexL, &m_nStepIndexR);
      printf("Difference between original and decoded streams: 0x%I64x\n",
             CalcDifference((LPBYTE)pvSrc, pvDecoded, cBlocks, cBlocks,
                            dwSrcBlockSize));
...
WORD CImaAdpcmCodec::GetEncodeAlignment(void) {
...
WORD CImaAdpcmCodec::GetDecodeAlignment(void) {
  return m_wfxEncode.wSamplesPerBlock * m_wfxEncode.wfx.nChannels *
         IMAADPCM_PCM_BITS_PER_SAMPLE / 8;
}
...
WORD CImaAdpcmCodec::CalculateEncodeAlignment(WORD nChannels,
...
void CImaAdpcmCodec::CreateImaAdpcmFormat(WORD nChannels, DWORD nSamplesPerSec,
  ...
  pwfx->wfx.nAvgBytesPerSec =
      nSamplesPerSec * pwfx->wfx.nBlockAlign / nSamplesPerBlock;
...
BOOL CImaAdpcmCodec::IsValidPcmFormat(LPCWAVEFORMATEX pwfx) {
...
BOOL CImaAdpcmCodec::IsValidImaAdpcmFormat(LPCIMAADPCMWAVEFORMAT pwfx) {
  ...
  if ((pwfx->wfx.nChannels < 1) ||
      (pwfx->wfx.nChannels > IMAADPCM_MAX_CHANNELS)) {
    return FALSE;
  }
  ...
  if (CalculateEncodeAlignment(pwfx->wfx.nChannels, pwfx->wSamplesPerBlock) !=
      pwfx->wfx.nBlockAlign) {
    return FALSE;
  }
 ...
 *
 ****************************************************************************/
ULONGLONG CImaAdpcmCodec::CalcDifference(LPBYTE pvBuffer1, LPBYTE pvBuffer2,
                                         UINT cBlocks, UINT cTotalBlocks,
                                         DWORD dwBlockSize) {
  ULONGLONG ullDiff = 0;
  ...
  const ULONGLONG ullMaxBlockDiff =
      ((ULONGLONG)1 << 32) * m_wfxEncode.wSamplesPerBlock;
  ...
  for (UINT i = 0; i < cBlocks; ++i) {
    ...
    for (UINT j = 0; j < m_wfxEncode.wSamplesPerBlock; ++j) {
      ULONGLONG ullSampleDiff =
          (ULONGLONG)(pSamples2[j]) - (ULONGLONG)(pSamples1[j]);
    ...
    ullDiff += (ULONGLONG)(((DOUBLE)ullBlockDiff / (DOUBLE)ullMaxBlockDiff) *
                           ullMaxBlockContribution);
  }
...
int CImaAdpcmCodec::EncodeSample(int nInputSample, LPINT pnPredictedSample,
                                 int nStepSize) {
...
int CImaAdpcmCodec::DecodeSample(int nEncodedSample, int nPredictedSample,
                                 int nStepSize) {
  LONG lDifference;
  LONG lNewSample;
...
BOOL CImaAdpcmCodec::EncodeM16(LPBYTE pbSrc, LPBYTE pbDst, UINT cBlocks,
                               UINT nBlockAlignment, UINT cSamplesPerBlock,
                               LPINT pnStepIndexL, LPINT) {
  LPBYTE pbBlock;
  UINT cSamples;
  int nSample;
  int nStepSize;
...
BOOL CImaAdpcmCodec::EncodeS16(LPBYTE pbSrc, LPBYTE pbDst, UINT cBlocks,
                               UINT nBlockAlignment, UINT cSamplesPerBlock,
...
BOOL CImaAdpcmCodec::DecodeM16(LPBYTE pbSrc, LPBYTE pbDst, UINT cBlocks,
                               UINT nBlockAlignment, UINT cSamplesPerBlock,
                               LPINT, LPINT) {
  BOOL fSuccess = TRUE;
  LPBYTE pbBlock;
...
BOOL CImaAdpcmCodec::DecodeS16(LPBYTE pbSrc, LPBYTE pbDst, UINT cBlocks,
                               UINT nBlockAlignment, UINT cSamplesPerBlock,
                               LPINT, LPINT) {
  BOOL fSuccess = TRUE;
  LPBYTE pbBlock;
...
int XboxADPCMSize(int sampleCount, int channelCount, int sampleRate) {
  ...
  codec.CreateImaAdpcmFormat((WORD)channelCount, sampleRate,
                             XBOX_ADPCM_SAMPLES_PER_BLOCK, &wfxEncode);
...
void Convert16ToXboxADPCM(const short *pInputBuffer, byte *pOutputBuffer,
                          byte *pOutFormat, int sampleCount, int channelCount,
                          int sampleRate) {
  CImaAdpcmCodec codec;
  IMAADPCMWAVEFORMAT wfxEncode;
  ...
  DWORD dwDestBlocks = sampleCount / XBOX_ADPCM_SAMPLES_PER_BLOCK;
  if (FALSE ==
      codec.Convert((const byte *)pInputBuffer, pOutputBuffer, dwDestBlocks))
    return;
}
