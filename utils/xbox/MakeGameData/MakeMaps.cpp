...
//
// Purpose: .360 Creation for all studiomdl generated files (mdl, vvd, vtx, ani,
// phy)
//
//=====================================================================================//
...
//-----------------------------------------------------------------------------
bool ConvertVHV(const char *pVhvFilename, const char *pModelName,
                CUtlBuffer &sourceBuffer, CUtlBuffer &targetBuffer) {
  ...
  if (!g_pFullFileSystem->ReadFile(vsiFilename, NULL, vsiBuffer)) {
    ...
    Msg("Error! Missing expected model conversion file '%s'. Cannot perform "
        "VHV fixup.\n",
        vsiFilename);
    return false;
  }
  ...
  HardwareVerts::FileHeader_t *pVHVhdr =
      (HardwareVerts::FileHeader_t *)sourceBuffer.Base();
...
//-----------------------------------------------------------------------------
bool CreateTargetFile_BSP(const char *pSourceName, const char *pTargetName,
                          bool bWriteToZip) {
  ...
  if (!Sys_LoadInterface("bsppack.dll", IBSPPACK_VERSION_STRING, &pBSPModule,
                         &pBSPPack)) {
    Msg("Failed to load bsppack interface\n");
    return false;
  }

  scriptlib->MakeTemporaryFilename(g_szModPath, tempSwapName,
                                   sizeof(tempSwapName));

  // Swaps the bsp directly to disk
  bool bOK =
      ((IBSPPack *)pBSPPack)
          ->SwapBSPFile(g_pFullFileSystem, pSourceName, tempSwapName, false,
                        ConvertVTFTo360Format, ConvertVHV, CompressCallback);
  if (!bOK) {
    goto cleanUp;
  }

  // get the pak file from the swapped bsp
  bOK = ((IBSPPack *)pBSPPack)
            ->GetPakFileLump(g_pFullFileSystem, tempSwapName, &pPakData,
                             &pakSize);
  if (!bOK) {
    goto cleanUp;
  }
  ...
  if (pPakData && pakSize) {
    ...
    scriptlib->MakeTemporaryFilename(g_szModPath, tempZipName,
                                     sizeof(tempZipName));
    ...
    for (;;) {
      ...
      filename[0] = '\0';
      int fileSize = 0;
      zipIndex = pOldZip->GetNextFilename(zipIndex, filename, sizeof(filename),
                                          fileSize);
      if (zipIndex == -1) {
        break;
      }
    ...
    bOK = ((IBSPPack *)pBSPPack)
              ->SetPakFileLump(g_pFullFileSystem, tempSwapName, tempSwapName,
                               zipBuffer.Base(), zipBuffer.TellMaxPut());
    if (!bOK) {
      goto cleanUp;
    }
  }
  ...
  bOK =
      WriteBufferToFile(pTargetName, targetBuffer, false, WRITE_TO_DISK_ALWAYS);
...
//-----------------------------------------------------------------------------
bool CreateTargetFile_AIN(const char *pSourceName, const char *pTargetName,
                          bool bWriteToZip) {
  ...
  // the pc ain is tied to the pc bsp and should have been generated after the
  // bsp
  ...
  // Check the version
  if (sourceBuf.GetChar() == 'V' && sourceBuf.GetChar() == 'e' &&
      sourceBuf.GetChar() == 'r') {
  ...
  if (version != AINET_VERSION_NUMBER) {
    Msg("%s: Need to regenerate PC nodegraph (got version '%d', expected "
        "'%d')\n",
  ...
  if (ReadBSPHeader(szBspPath, &bspHeader)) {
    if (mapVersion != bspHeader.mapRevision) {
      Msg("%s: Need to regenerate PC nodegraph (ai revision '%d' does not "
          "match bsp revision '%d')\n",
  ...
  for (int node = 0; node < nodeCt; ++node) {
    ...
    swap.SwapBufferToTargetEndian<float>(
        (float *)targetBuf.PeekPut(), (float *)sourceBuf.PeekGet(), numFloats);
  ...
  swap.SwapBufferToTargetEndian<int>((int *)targetBuf.PeekPut(),
                                     (int *)sourceBuf.PeekGet(), nodeCt);
  targetBuf.SeekPut(CUtlBuffer::SEEK_CURRENT, nodeCt * sizeof(int));

  // Write the file out
  return WriteBufferToFile(pTargetName, targetBuf, bWriteToZip,
...
bool GetDependants_BSP(const char *pBspName, CUtlVector<CUtlString> *pList) {
  ...
  V_ComposeFileName(g_szGamePath, pBspName, szBspFilename,
                    sizeof(szBspFilename));

  bool bOK = ((IBSPPack *)pBSPPack)
                 ->GetBSPDependants(g_pFullFileSystem, szBspFilename, pList);
