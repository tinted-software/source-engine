...
bool CreateTargetFile_PCF(const char *pSourceName, const char *pTargetName,
  ...
  DmxHeader_t header;
  CDmElement *pRoot;
  if (g_pDataModel->RestoreFromFile(pSourceName, NULL, NULL, &pRoot,
                                    CR_DELETE_NEW,
  ...
  if (!g_pDataModel->Serialize(binaryBuffer, "binary", pOutFormat,
                               pRoot->GetHandle())) {
  ...
  WriteBufferToFile(pTargetName, binaryBuffer, bWriteToZip,
