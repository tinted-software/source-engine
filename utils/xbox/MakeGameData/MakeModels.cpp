...
//-----------------------------------------------------------------------------
bool CreateTargetFile_Model(const char *pSourceName, const char *pTargetName,
                            bool bWriteToZip) {
...
bool GetDependants_MDL(const char *pModelName, CUtlVector<CUtlString> *pList) {
  ...
  if (pStudioHdr->version != STUDIO_VERSION) {
    Msg("Error! Bad Model '%s', Expecting Version (%d), got (%d)\n", pModelName,
        STUDIO_VERSION, pStudioHdr->version);
    return false;
  }
  ...
  if (pStudioHdr->flags & STUDIOHDR_FLAGS_OBSOLETE) {
    ...
    for (i = 0; i < pStudioHdr->numtextures; i++) {
      ...
      for (j = 0; j < pStudioHdr->numcdtextures; j++) {
        ...
        V_ComposeFileName("materials", pStudioHdr->pCdtexture(j), szPath,
                          sizeof(szPath));
        ...
        V_ComposeFileName(szPath, pStudioHdr->pTexture(i)->pszName(), szOutName,
                          sizeof(szOutName));
...
//-----------------------------------------------------------------------------
bool GetPreloadData_VHV(const char *pFilename, CUtlBuffer &fileBufferIn,
                        CUtlBuffer &preloadBufferOut) {
  HardwareVerts::FileHeader_t *pHeader =
      (HardwareVerts::FileHeader_t *)fileBufferIn.Base();
...
//-----------------------------------------------------------------------------
bool GetPreloadData_VTX(const char *pFilename, CUtlBuffer &fileBufferIn,
                        CUtlBuffer &preloadBufferOut) {
  OptimizedModel::FileHeader_t *pHeader =
      (OptimizedModel::FileHeader_t *)fileBufferIn.Base();
  ...
  if (version != OPTIMIZED_MODEL_FILE_VERSION) {
    // bad version
    Msg("Can't preload: '%s', expecting version %d got version %d\n", pFilename,
        OPTIMIZED_MODEL_FILE_VERSION, version);
    return false;
  }
...
//-----------------------------------------------------------------------------
bool GetPreloadData_VVD(const char *pFilename, CUtlBuffer &fileBufferIn,
                        CUtlBuffer &preloadBufferOut) {
  ...
  if (version != MODEL_VERTEX_FILE_VERSION) {
    // bad version
    Msg("Can't preload: '%s', expecting version %d got version %d\n", pFilename,
        MODEL_VERTEX_FILE_VERSION, version);
    return false;
  }
...
bool CompressFunc(const void *pInput, int inputSize, void **pOutput,
                  int *pOutputSize) {
  *pOutput = NULL;
  *pOutputSize = 0;
  ...
  unsigned char *pCompressedOutput = LZMA_OpportunisticCompress(
      (unsigned char *)pInput, inputSize, &compressedSize);
...
static bool GenerateModelFiles(const char *pMdlFilename) {
  ...
  if (bHasVtx && bHasVvd) {
    ...
    bool bResult =
        mdllib->StripModelBuffers(mdlBuffer, vvdBuffer, vtxBuffer, &pStripInfo);
    if (!bResult) {
      return false;
    }
    if (pStripInfo) {
  ...
  if (bHasAni) {
    ...
    if (((studiohdr_t *)mdlBuffer.Base())->numanimblocks != 0) {
      ...
      swappedSize = StudioByteSwap::ByteswapStudioFile(
          szAniFilename, tempBuffer.Base(), aniBuffer.PeekGet(), fileSize,
          (studiohdr_t *)mdlBuffer.Base(), CompressFunc);
      if (swappedSize > 0) {
        // .ani buffer is replaced with swapped data
        aniBuffer.Purge();
        aniBuffer.Put(tempBuffer.Base(), swappedSize);
        WriteBufferToFile(szAniFilename, aniBuffer, false,
  ...
  if (bHasPhy) {
    ...
    swappedSize = StudioByteSwap::ByteswapStudioFile(
        szPhyFilename, tempBuffer.Base(), phyBuffer.PeekGet(), fileSize,
        (studiohdr_t *)mdlBuffer.Base(), CompressFunc);
    if (swappedSize > 0) {
  ...
  if (bHasVtx) {
    ...
    swappedSize = StudioByteSwap::ByteswapStudioFile(
        szVtxFilename, tempBuffer.Base(), vtxBuffer.PeekGet(), fileSize,
        (studiohdr_t *)mdlBuffer.Base(), CompressFunc);
    if (swappedSize > 0) {
  ...
  if (bHasVvd) {
    ...
    swappedSize = StudioByteSwap::ByteswapStudioFile(
        szVvdFilename, tempBuffer.Base(), vvdBuffer.PeekGet(), fileSize,
        (studiohdr_t *)mdlBuffer.Base(), CompressFunc);
    if (swappedSize > 0) {
  ...
  swappedSize = StudioByteSwap::ByteswapStudioFile(
      szMdlFilename, tempBuffer.Base(), mdlBuffer.PeekGet(), fileSize, NULL,
      CompressFunc);
  if (swappedSize > 0) {
...
static bool ModelNeedsUpdate(const char *pMdlSourcePath) {
  ...
  for (int i = 0; i < ARRAYSIZE(pExtensions); i++) {
    ...
    V_SetExtension(szSourcePath, pExtensions[i].pSourceExtension,
                   sizeof(szSourcePath));
    ...
    V_SetExtension(szTargetPath, pExtensions[i].pTargetExtension,
                   sizeof(szTargetPath));
