...
bool CreateTargetFile_CCDAT(const char *pSourceName, const char *pTargetName,
  ...
  if (SwapClosecaptionFile(targetBuffer.Base())) {
    bOk = WriteBufferToFile(pTargetName, targetBuffer, bWriteToZip,
...
//-----------------------------------------------------------------------------
bool FindOrAddFileToResourceList(const char *pFilename,
                                 CUtlRBTree<CUtlString, int> *pTree,
                                 bool bAdd = true) {
...
//-----------------------------------------------------------------------------
bool RemoveFileFromResourceList(const char *pFilename,
                                CUtlRBTree<CUtlString, int> *pTree) {
  char szOutName[MAX_PATH];
  char *pOutName;
  V_strncpy(szOutName, pFilename, sizeof(szOutName));
...
bool LoadReslist(const char *pReslistName, CUtlRBTree<CUtlString, int> *pTree) {
  ...
  for (;;) {
    ...
    if (!pExt) {
      // unknown
      continue;
    } else if (!V_stricmp(pExt, "vmt") || !V_stricmp(pExt, "vhv") ||
               !V_stricmp(pExt, "mdl") || !V_stricmp(pExt, "raw") ||
               !V_stricmp(pExt, "wav")) {
      bKeep = true;
  ...
  for (int i = pTree->FirstInorder(); i != pTree->InvalidIndex();
       i = pTree->NextInorder(i)) {
  ...
  V_ComposeFileName(g_szGamePath, szBspName, szCommentaryName,
                    sizeof(szCommentaryName));
  V_StripExtension(szCommentaryName, szCommentaryName,
                   sizeof(szCommentaryName));
  ...
  if (ReadFileToBuffer(szCommentaryName, commentaryBuffer, true, true)) {
    ...
    for (;;) {
      int nTokenSize = commentaryBuffer.ParseToken(&breakSet, szCommentaryToken,
                                                   sizeof(szCommentaryToken));
      if (nTokenSize < 0) {
        break;
      }
      if (nTokenSize > 0 && !V_stricmp(szCommentaryToken, "commentaryfile")) {
        // get the commentary file
        nTokenSize = commentaryBuffer.ParseToken(&breakSet, szCommentaryToken,
                                                 sizeof(szCommentaryToken));
        if (nTokenSize > 0) {
  ...
  V_ComposeFileName(g_szGamePath, "reslistfixes_xbox.xsc", szBlacklist,
                    sizeof(szBlacklist));
  CUtlBuffer blacklistBuffer;
  if (ReadFileToBuffer(szBlacklist, blacklistBuffer, true, true)) {
    for (;;) {
      int nTokenSize =
          blacklistBuffer.ParseToken(&breakSet, szToken, sizeof(szToken));
      if (nTokenSize <= 0) {
        break;
      }
      ...
      if (!V_stricmp(szToken, "-")) {
        ...
        Msg("Bad Syntax, expecting '+' or '-' as first token in reslist fixup "
            "file '%s'.\n",
            szBlacklist);
        continue;
      }
...
bool CreateTargetFile_RESLST(const char *pSourceName, const char *pTargetName,
  ...
  for (int iIndex = rbTree.FirstInorder(); iIndex != rbTree.InvalidIndex();
       iIndex = rbTree.NextInorder(iIndex)) {
