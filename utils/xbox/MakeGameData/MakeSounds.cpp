...
//
// Description: converts from a sample index to a block index + the number of
// samples
...
//      out_pOffset:        number of samples in the block before the desired
//      sample
//--------------------------------------------------------------------------------------
bool SampleToXMABlockOffset(DWORD dwSampleIndex, const DWORD *pdwSeekTable,
                            DWORD nEntries, DWORD *out_pBlockIndex,
                            DWORD *out_pOffset) {
  ...
  // Calculate the sample offset by figuring out what the sample index of the
  // first sample in the block is, then subtracting that from dwSampleIndex.
  if (bFound) {
    DWORD dwStartOfBlock = (*out_pBlockIndex == 0)
                               ? 0
                               : BigLong(pdwSeekTable[*out_pBlockIndex - 1]);
...
//-----------------------------------------------------------------------------
bool ReadChunks(const char *pFileName, int &numChunks,
  ...
  while (walk.ChunkAvailable()) {
    ...
    if (walk.ChunkName() == WAVE_FMT && size < sizeof(WAVEFORMATEXTENSIBLE)) {
      ...
      // ensure the data footprint is at least the structure we want to
      // manipulate
...
void ConvertADPCMDataChunkTo16(chunk_t *pFormatChunk, chunk_t *pDataChunk) {
  ...
  int sampleCount =
      ADPCMSampleCount((byte *)pFormat, pDataChunk->pData, pDataChunk->size);
  int outputSize = sizeof(short) * sampleCount * pFormat->nChannels;
  short *pOut = (short *)malloc(outputSize);

  // convert to PCM 16bit format
  DecompressADPCMSamples((byte *)pFormat, (byte *)pDataChunk->pData,
                         pDataChunk->size, pOut);
...
void ConvertPCMDataChunk16To22K(chunk_t *pFormatChunk, chunk_t *pDataChunk) {
  ...
  if (pFormat->nSamplesPerSec != 44100 || pFormat->wBitsPerSample != 16 ||
      pFormat->wFormatTag != WAVE_FORMAT_PCM) {
  ...
  int sampleCount = pDataChunk->size / sampleSize;
  short *pOut =
      (short *)malloc(sizeof(short) * (sampleCount * pFormat->nChannels));

  DecimateSampleRateBy2_16((short *)pDataChunk->pData, pOut, sampleCount,
                           pFormat->nChannels);
...
bool EncodeAsXMA(const char *pDebugName, CUtlBuffer &targetBuff, int quality,
                 bool bIsVoiceOver) {
#ifdef NO_X360_XDK
  return false;
#else
  ...
  if (g_chunks[formatChunk].size < sizeof(WAVEFORMATEXTENSIBLE)) {
    memset((unsigned char *)&wfx + g_chunks[formatChunk].size, 0,
           sizeof(WAVEFORMATEXTENSIBLE) - g_chunks[formatChunk].size);
  }
  ...
  if (loopStart != -1) {
    ...
    inputStream.LoopLength =
        inputStream.BufferSize / (pFormat->nChannels * sizeof(short)) -
        loopStart;
  }
  ...
  while (numAttempts < 10) {
    hr = XMA2InMemoryEncoder(1, &inputStream, quality, xmaFlags,
                             XMA_BLOCK_SIZE / 1024, &pXMAData, &XMADataSize,
                             &pXMA2Format, &XMA2FormatSize, &pXMASeekTable,
                             &XMASeekTableSize);
    if (!FAILED(hr))
      break;
  ...
  if (loopStart != -1) {
    ...
    if (!SampleToXMABlockOffset(BigLong(pXMA2Format->LoopBegin), pXMASeekTable,
                                XMASeekTableSize / sizeof(DWORD),
                                &loopBlockStartIndex, &loopBlockStartOffset)) {
    ...
    if (BigLong(pXMA2Format->LoopEnd) < BigLong(pXMA2Format->SamplesEncoded)) {
      ...
      if (!SampleToXMABlockOffset(BigLong(pXMA2Format->LoopEnd), pXMASeekTable,
                                  XMASeekTableSize / sizeof(DWORD),
                                  &loopBlockEndIndex, &loopBlockEndOffset)) {
        // could not determine loop point, out of range of encoded samples
        Msg("XMA Loop Encoding Error on '%s', loop %d\n", pDebugName,
            loopStart);
        return false;
      ...
      if (loopBlockEndIndex != BigLong(pXMA2Format->BlockCount) - 1) {
        ...
        Msg("XMA Loop Encoding Error on '%s', block end is %d/%d\n", pDebugName,
            loopBlockEndOffset, BigLong(pXMA2Format->BlockCount));
        return false;
      }

      numTrailingSamples =
          BigLong(pXMA2Format->SamplesEncoded) - BigLong(pXMA2Format->LoopEnd);
    }
    ...
    if (numLeadingSamples > 32767) {
      Msg("XMA Loop Encoding Error on '%s', leading samples exceeds 16 bits "
          "%d\n",
          pDebugName, numLeadingSamples);
      return false;
    }
    if (numTrailingSamples > 32767) {
      Msg("XMA Loop Encoding Error on '%s', trailing samples exceeds 16 bits "
          "%d\n",
  ...
  header.dataOffset = BigLong(AlignValue(
      sizeof(xwvHeader_t) + seekTableSize + vdatSize, XBOX_DVD_SECTORSIZE));
  ...
  if (loopStart != -1) {
    // the loop start is in source space (now meaningless), need the loop in XMA
    // decoding sample space
...
bool EncodeAsPCM(const char *pTargetName, CUtlBuffer &targetBuff) {
  ...
  header.dataOffset =
      BigLong(AlignValue(sizeof(xwvHeader_t) + vdatSize, XBOX_DVD_SECTORSIZE));
...
//-----------------------------------------------------------------------------
bool CreateTargetFile_WAV(const char *pSourceName, const char *pTargetName,
                          bool bWriteToZip) {
  g_numChunks = 0;
  ...
  // for safety (not trusting their decoding) and simplicity convert all data to
  ...
  if (bSuccess) {
    WriteBufferToFile(pTargetName, targetBuff, bWriteToZip,
...
//-----------------------------------------------------------------------------
bool CreateTargetFile_MP3(const char *pSourceName, const char *pTargetName,
                          bool bWriteToZip) {
  CUtlBuffer targetBuffer;

  // ignore the .mp3 source, the .360.wav target should have been pre-converted,
  // checked in, and exist use the expected target as the source
  ...
  bool bSuccess = WriteBufferToFile(pTargetName, targetBuffer, bWriteToZip,
                                    WRITE_TO_DISK_NEVER);
  return bSuccess;
}
...
//-----------------------------------------------------------------------------
bool GetPreloadData_WAV(const char *pFilename, CUtlBuffer &fileBufferIn,
  ...
  unsigned int preloadSize =
      BigLong(pHeader->headerSize) + BigLong(pHeader->staticDataSize);
