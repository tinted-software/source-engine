...
// conversion routines based on file extension.  The conversion routines should
// be
// !!!elsewhere!!! in libraries that the game also uses at runtime to convert
// data. This tool as spec'd should just be file iteration.
//
//=====================================================================================//
...
const char *g_GameNames[] = {"ep2",      "episodic", "hl2", "portal",
                             "platform", "tf",       NULL};

// all known languages
const char *g_pLanguageSuffixes[] = {
    "_dannish",   "_dutch",   "_english",    "_finnish",  "_french",
    "_german",    "_italian", "_japanese",   "_korean",   "_koreana",
    "_norwegian", "_polish",  "_portuguese", "_russian",  "_russion_buka",
    "_schinese",  "_spanish", "_swedish",    "_tchinese", "_thai",
};
...
void GetGamePath() {
  ...
  if (!pGameDir) {
    Msg("ERROR: Failed to determine game directory from current path. "
        "Expecting 'game' in current path.");
    exit(1);
  }
...
bool GetModPath() {
  ...
  while (1) {
    V_ComposeFileName(szDirectory, "gameinfo.txt", g_szModPath,
                      sizeof(g_szModPath));
...
//-----------------------------------------------------------------------------
bool ReadFileToBuffer(const char *pSourceName, CUtlBuffer &buffer, bool bText,
                      bool bNoOpenFailureWarning) {
  return scriptlib->ReadFileToBuffer(pSourceName, buffer, bText,
                                     bNoOpenFailureWarning);
}

//-----------------------------------------------------------------------------
// Purpose: Helper utility, Write buffer to file
//-----------------------------------------------------------------------------
bool WriteBufferToFile(const char *pTargetName, CUtlBuffer &buffer,
                       bool bWriteToZip, DiskWriteMode_t writeMode) {
  if (g_bTest)
    return true;
...
bool CompressCallback(CUtlBuffer &inputBuffer, CUtlBuffer &outputBuffer) {
  ...
  unsigned char *pCompressedOutput = LZMA_OpportunisticCompress(
      (unsigned char *)inputBuffer.Base() + inputBuffer.TellGet(),
      inputBuffer.TellPut() - inputBuffer.TellGet(), &compressedSize);
...
void DoPostProcessingFunctions(bool bWriteToZip) {
  if (g_bMakeScenes || g_bMakeScenesPC) {
    ...
    CreateSceneImageFile(g_szModPath, bWriteToZip, g_bMakeScenesPC, g_bQuiet,
                         g_WriteModeForConversions);
  }
...
//-----------------------------------------------------------------------------
bool InitConversionModules() { return true; }
...
//-----------------------------------------------------------------------------
bool CreateTargetFile(const char *pSourceName, const char *pTargetName,
                      fileType_e fileType, bool bWriteToZip) {
...
// target name.
//-----------------------------------------------------------------------------
fileType_e ResolveFileType(const char *pSourceName, char *pTargetName,
                           int targetNameSize) {
  ...
  if (!V_stricmp(sourceExtension, "wav")) {
    ...
    // slam the expected name here to simplify the external logic which will do
    // the right thing
    ...
    if (V_stristr(szFullSourcePath, "reslists_xbox\\")) {
      // only want reslists map versions, due to special processing, ignore all
      // others
...
bool IncludeInZip(const char *pSourceName) {
  ...
  for (int i = 0; i < ARRAYSIZE(s_AllowedExtensionsInZip); ++i) {
    ...
    if (!V_stricmp(pAllowedExtension, ".txt") ||
        !V_stricmp(pAllowedExtension, ".360.dat")) {
      ...
      if (!V_stricmp(pAllowedExtension, ".txt") &&
          V_stristr(pSourceName, "closecaption_")) {
...
void GenerateTargetFiles(CUtlVector<fileList_t> &fileList) {
  ...
  for (int i = 0; i < fileList.Count(); i++) {
    ...
    fileType_e fileType =
        ResolveFileType(sourceFile, targetFile, sizeof(targetFile));
    ...
    if (fileType == FILETYPE_UNKNOWN) {
      ...
      // the wildcard match may catch existing converted files, which need to be
      // rejected cull exisiting conversions from the work list the
      // non-converted filename is part of the same wildcard match, gets caught
      // and resolved the non-converted filename will then go through the proper
      // conversion path
    ...
    if (fileType == FILETYPE_BSP || fileType == FILETYPE_AIN ||
        fileType == FILETYPE_RESLST) {
      if (g_ValidMapList.Count() &&
          !IsMapNameInList(sourceFile, g_ValidMapList)) {
        // cull completely
        updateList[i] = 0;
        continue;
      }
    ...
    if (fileType == FILETYPE_MDL || fileType == FILETYPE_ANI ||
        fileType == FILETYPE_VTX || fileType == FILETYPE_VVD ||
        fileType == FILETYPE_PHY) {
  ...
  for (int i = 0; i < fileList.Count(); i++) {
    ...
    if (!g_bQuiet) {
      Msg("%d/%d:%s -> %s\n", progress + 1, numWorkItems, sourceFile,
          targetFile);
    }
    ...
    if (updateList[i] & DO_UPDATE) {
      ...
      bSuccess = CreateTargetFile(sourceFile, targetFile, fileType,
                                  (updateList[i] & DO_ZIP) != 0);
      if (!bSuccess) {
        // add to error list
...
void Usage() {
  ...
  Msg("[-h] [-help] [-?]       Help\n");
  Msg("[-t targetPath]         Alternate output path, will generate output at "
      "target\n");
  ...
  Msg("[-z <zipname>]          Generate zip file AND create or update stale "
      "conversions\n");
  Msg("[-zo <zipname>]         Generate zip file ONLY (existing stale "
      "conversions get updated, no new conversions are written)\n");
  ...
  Msg("[-usemaplist]           For BSP related conversions, restricts to "
      "maplist.txt.\n");
...
bool MakeGameDataApp::Create() {
  ...
  // Load vphysics.dll
  if (!Sys_LoadInterface("vphysics.dll", VPHYSICS_COLLISION_INTERFACE_VERSION,
                         &g_pPhysicsModule, (void **)&g_pPhysicsCollision)) {
...
int MakeGameDataApp::Main() {
  ...
  if (CommandLine()->ParmCount() < 2 || CommandLine()->FindParm("?") ||
      CommandLine()->FindParm("-h") || CommandLine()->FindParm("-help")) {
    Usage();
  }
...
#ifndef MAKESCENESIMAGE
  bRecurse =
      CommandLine()->FindParm("-recurse") || CommandLine()->FindParm("-r");
  ...
  g_bIsPlatformZip =
      g_bMakeZip && (V_stristr(g_szModPath, "\\platform") != NULL);
  ...
  if (g_bMakeZip || g_bUseMapList) {
    // zips use the map list to narrow the bsp conversion to actual shipping
    // maps
