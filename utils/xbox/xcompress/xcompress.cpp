...
static void *__stdcall jcalgAlloc(DWORD size) { return malloc(size); }
  ...
  for (;;) {
    ...
    if (header->nMagic == xCompressHeader::MAGIC &&
      ...
      printf("Found header:\n"
             ...
             header->nVersion, header->nUncompressedFileSize,
             header->nReadBlockSize, header->nDecompressionBufferSize);

      outputBufferLength = JCALG1_Decompress_Formatted_Buffer(
          bytesRead - sizeof(*header), inputBuffer + sizeof(*header),
          g_BlockReadSize * 8, outputBuffer);

    } else {
      outputBufferLength = JCALG1_Decompress_Formatted_Buffer(
          bytesRead, inputBuffer, g_BlockReadSize * 8, outputBuffer);
    }
...
void compressSimple(char *filenameIn, char *filenameOut) {
  ...
  char *compressedData =
      (char *)malloc(uncompressedSize * 4 + sizeof(xCompressSimpleHeader));
  int compressedSize =
      JCALG1_Compress(uncompressedData, uncompressedSize,
                      compressedData + sizeof(xCompressSimpleHeader),
                      uncompressedSize, jcalgAlloc, jcalgDealloc, NULL, true);
...
void decompressSimple(char *filenameIn, char *filenameOut) {
  ...
  char *decompressedData = (char *)malloc(
      ((xCompressSimpleHeader *)compressedData)->nUncompressedSize);

  unsigned decompressedSize =
      JCALG1_Decompress_Simple_Buffer(compressedData, decompressedData);
  FILE *hOut = fopen(filenameOut, "wb+");
  if (!hIn) {
    printf("Failed to open output file: %s\n", filenameOut);
    fclose(hIn);
...
int main(int argc, char *argv[]) {
  if (argc < 4 || argc > 5) {
    ...
    puts("\nIf cs is specified, a 'simple' archive is created.  (unaligned and "
         "decompresses the entire thing to a buffer)");
    return EXIT_FAILURE;
  }
  ...
  while (1) {
    ...
    unsigned compressedSize = JCALG1_Compress(
        inputBuffer, (unsigned)bytesRead, outputBuffer + sizeof(short), 16384,
        jcalgAlloc, jcalgDealloc, NULL, true);
    ...
    if (counter % 4 == 0) {
      printf("\r                                                               "
             "        \rInput:%uk Output:%uk (%0.1f%%)",
             inputBytes / 1024, totalBytes / 1024,
             ((double)totalBytes / (double)inputBytes) * 100);
      fflush(stdout);
    }
  }

  // Grab the last block (may be the only block)Compute the minimum size of the
