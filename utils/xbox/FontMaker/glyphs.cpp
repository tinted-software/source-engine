...
//       range specified.
//-----------------------------------------------------------------------------
HRESULT CTextureFont::ExtractValidGlyphsFromRange(WORD wStartGlyph,
                                                  WORD wEndGlyph) {
...
HRESULT CTextureFont::ReadFontInfoFile(CHAR *strFileName) {
  ...
  if (bitmapFont.m_id != BITMAPFONT_ID ||
...
HRESULT CTextureFont::WriteFontInfoFile(CHAR *strFileName) {
  ...
  //	FLOAT fBottomPadding = ( m_bOutlineEffect ? ( m_bShadowEffect ? 2.0f
  //: 1.0f ) : ( m_bShadowEffect ? 2.0f : 0.0f ) ); 	FLOAT fFontYAdvance  =
  //fFontHeight - fTopPadding - fBottomPadding;
...
// Desc: Writes 32-bit RGBA data to a .tga file
//-----------------------------------------------------------------------------
HRESULT CTextureFont::WriteFontImageFile(CHAR *strFileName, bool bAdditiveMode,
                                         bool bCustomFont) {
  ...
  for (DWORD i = 0; i < m_dwTextureWidth * m_dwTextureHeight; i++) {
    ...
    if (bCustomFont) {
      ...
      pRGBAData[i] =
          (alpha << 24L) | (red << 16L) | (green << 8L) | (blue << 0L);
    } else {
      if (bAdditiveMode) {
  ...
  // Write the file
  HRESULT hr = WriteTargaFile(strFileName, m_dwTextureWidth, m_dwTextureHeight,
                              pRGBAData);
...
//-----------------------------------------------------------------------------
void SetTextureBits(HBITMAP hBitmap, int bitmapWidth, int bitmapHeight, int x,
                    int y, int w, int h, unsigned char *pRGBA) {
  // get the enitre bitmap
  unsigned char *pBitmapBits =
      (unsigned char *)malloc(bitmapWidth * bitmapHeight * 4);
...
//-----------------------------------------------------------------------------
unsigned char *GetTextureBits(HBITMAP hBitmap, int bitmapWidth,
                              int bitmapHeight, int x, int y, int w, int h) {
  // get the enitre bitmap
  unsigned char *pBitmapBits =
...
//-----------------------------------------------------------------------------
void GetBlurValueForPixel(unsigned char *src, int blur,
                          float *gaussianDistribution, int srcX, int srcY,
                          int srcWide, int srcTall, unsigned char *dest) {
  int r = 0, g = 0, b = 0, a = 0;
...
//-----------------------------------------------------------------------------
void ApplyGaussianBlurToTexture(int blur, int rgbaX, int rgbaY, int rgbaWide,
                                int rgbaTall, unsigned char *rgba) {
  ...
  if (blur > 1 && blur != g_blur) {
    ...
    for (int x = 0; x <= (blur * 2); x++) {
      int val = x - blur;
      g_pGaussianDistribution[x] =
          (float)((1.0 / sqrt(2.0 * 3.14 * sigma * sigma)) *
                  pow(2.7, -1.0 * (val * val) / (2.0 * sigma * sigma)));
  ...
  for (int y = 0; y < rgbaTall; y++) {
    for (int x = 0; x < rgbaWide; x++) {
      ...
      GetBlurValueForPixel(src, blur, g_pGaussianDistribution, x, y, rgbaWide,
                           rgbaTall, dest);
...
//-----------------------------------------------------------------------------
void ApplyScanlineEffectToTexture(int scanLines, int rgbaX, int rgbaY,
                                  int rgbaWide, int rgbaTall,
                                  unsigned char *rgba) {
  if (scanLines < 2)
    return;
  ...
  for (int y = 0; y < rgbaTall; y++) {
    ...
    for (int x = 0; x < rgbaWide; x++, pBits++) {
      ...
      pBits[0] = (((int)(r * 255.0f)) << 16) | (((int)(g * 255.0f)) << 8) |
                 ((int)(b * 255.0f));
      pBits[0] |= 0xFF000000;
    }
  }
...
//-----------------------------------------------------------------------------
GLYPH_ATTR *CTextureFont::RenderTTFGlyphs(
    HFONT hFont, HBITMAP hBitmap, DWORD dwTextureWidth, DWORD dwTextureHeight,
    BOOL bOutlineEffect, BOOL bShadowEffect, int nScanlineEffect,
    int nBlurEffect, BOOL bAntialias, BYTE *ValidGlyphs, DWORD dwNumGlyphs) {
  // Create a DC
  HDC hDC = CreateCompatibleDC(NULL);
    ...
    if (ValidGlyphs[i] == 2) {
      ...
      if (bOutlineEffect || bShadowEffect) {
        if (bOutlineEffect) {
          ...
          MoveToEx(hDC, sx + 0, sy + 0, NULL);
          ExtTextOutW(hDC, 0, 0, ETO_OPAQUE, NULL, str, 1, NULL);
          MoveToEx(hDC, sx + 1, sy + 0, NULL);
          ExtTextOutW(hDC, 0, 0, ETO_OPAQUE, NULL, str, 1, NULL);
          MoveToEx(hDC, sx + 2, sy + 0, NULL);
          ExtTextOutW(hDC, 0, 0, ETO_OPAQUE, NULL, str, 1, NULL);
          MoveToEx(hDC, sx + 0, sy + 1, NULL);
          ExtTextOutW(hDC, 0, 0, ETO_OPAQUE, NULL, str, 1, NULL);
          MoveToEx(hDC, sx + 2, sy + 1, NULL);
          ExtTextOutW(hDC, 0, 0, ETO_OPAQUE, NULL, str, 1, NULL);
          MoveToEx(hDC, sx + 0, sy + 2, NULL);
          ExtTextOutW(hDC, 0, 0, ETO_OPAQUE, NULL, str, 1, NULL);
          MoveToEx(hDC, sx + 1, sy + 2, NULL);
          ExtTextOutW(hDC, 0, 0, ETO_OPAQUE, NULL, str, 1, NULL);
          MoveToEx(hDC, sx + 2, sy + 2, NULL);
          ExtTextOutW(hDC, 0, 0, ETO_OPAQUE, NULL, str, 1, NULL);

          if (bShadowEffect) {
            MoveToEx(hDC, sx + 3, sy + 3, NULL);
            ExtTextOutW(hDC, 0, 0, ETO_OPAQUE, NULL, str, 1, NULL);
          }
        ...
        // apply blur effect
        unsigned char *pBGRA = GetTextureBits(hBitmap, dwTextureWidth,
                                              dwTextureHeight, x, y, w, h);
        if (pBGRA) {
          ApplyGaussianBlurToTexture(nBlurEffect, 0, 0, w, h, pBGRA);
          SetTextureBits(hBitmap, dwTextureWidth, dwTextureHeight, x, y, w, h,
                         pBGRA);
          delete[] pBGRA;
        }
      } else {
...
GLYPH_ATTR *CTextureFont::RenderCustomGlyphs(HBITMAP hBitmap) {
  ...
  unsigned char *pBGRA =
      GetTextureBits(hBitmap, m_dwTextureWidth, m_dwTextureHeight, 0, 0,
                     m_dwTextureWidth, m_dwTextureHeight);
  ...
  SetTextureBits(hBitmap, m_dwTextureWidth, m_dwTextureHeight, 0, 0,
                 m_dwTextureWidth, m_dwTextureHeight, pBGRA);
  ...
  for (DWORD i = 0; i < m_dwNumGlyphs; i++) {
    ...
    SetTextureBits(hBitmap, m_dwTextureWidth, m_dwTextureHeight, x, y, w, h,
                   pTGAPixels);
    TL_Free(pTGAPixels);
...
HRESULT CTextureFont::CalculateAndRenderGlyphs() {
  // Create a bitmap
  HBITMAP hBitmap =
      CreateBitmap(m_dwTextureWidth, m_dwTextureHeight, 1, 32, NULL);
  ...
  if (m_pCustomFilename) {
    ...
    m_pGlyphs = RenderTTFGlyphs(
        m_hFont, hBitmap, m_dwTextureWidth, m_dwTextureHeight, m_bOutlineEffect,
        m_bShadowEffect, m_nScanlines, m_nBlur, m_bAntialiasEffect,
        m_ValidGlyphs, m_dwNumGlyphs);
  }
