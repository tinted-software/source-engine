...
BOOL CFontMakerApp::InitInstance() {
  ...
  m_pDialogBar->GetDlgItem(IDC_FONTNAME_STATIC)
      ->SetWindowText(_T("<Choose font>"));
...
void CFontMakerApp::OnNewFontButton() {
  ...
  g_Font.m_LogFont.lfHeight =
      -MulDiv(iPointSize, GetDeviceCaps(hDC, LOGPIXELSY), 720);
  ...
  if (FAILED(CalculateAndRenderGlyphs())) {
    ...
    MessageBox(m_pMainWnd->m_hWnd, "Could not create the requested font!",
  ...
  if (g_bFirstTime) {
    ...
    if (g_Font.m_bAntialiasEffect) {
      ((CButton *)m_pDialogBar->GetDlgItem(IDC_EFFECTSANTIALIAS_CHECK))
          ->SetCheck(TRUE);
    } else {
      ((CButton *)m_pDialogBar->GetDlgItem(IDC_EFFECTSANTIALIAS_CHECK))
          ->SetCheck(FALSE);
    ...
    m_pDialogBar->GetDlgItem(IDC_GLYPHSFILESELECTOR_BUTTON)
        ->EnableWindow(FALSE);
    m_pDialogBar->GetDlgItem(IDC_GLYPHSCUSTOM_RADIO)->EnableWindow(TRUE);

    // Set a default range of glyphs to use
    ((CButton *)m_pDialogBar->GetDlgItem(IDC_GLYPHSFROMRANGE_RADIO))
        ->SetCheck(TRUE);
...
void CFontMakerApp::OnGlyphsFromRangeRadio() {
  ...
  ((CButton *)m_pDialogBar->GetDlgItem(IDC_GLYPHSFROMFILE_RADIO))
      ->SetCheck(false);
  ((CButton *)m_pDialogBar->GetDlgItem(IDC_GLYPHSFROMRANGE_RADIO))
      ->SetCheck(true);
  ((CButton *)m_pDialogBar->GetDlgItem(IDC_GLYPHSCUSTOM_RADIO))
      ->SetCheck(false);
...
void CFontMakerApp::OnChangeGlpyhsRangeEdit() {
  ...
  CEdit *pGlyphRangeFromEdit =
      (CEdit *)m_pDialogBar->GetDlgItem(IDC_GLYPHSRANGEFROM_EDIT);
  CEdit *pGlyphRangeToEdit =
      (CEdit *)m_pDialogBar->GetDlgItem(IDC_GLYPHSRANGETO_EDIT);
...
void CFontMakerApp::OnChangeGlyphsFileEdit() {
  CEdit *pGlyphFileNameEdit =
      (CEdit *)m_pDialogBar->GetDlgItem(IDC_GLYPHSFILE_EDIT);
...
void CFontMakerApp::OnEffectsCheck() {
  g_Font.m_bOutlineEffect =
      ((CButton *)m_pDialogBar->GetDlgItem(IDC_EFFECTSOUTLINED_CHECK))
          ->GetCheck();
  g_Font.m_bShadowEffect =
      ((CButton *)m_pDialogBar->GetDlgItem(IDC_EFFECTSSHADOWED_CHECK))
          ->GetCheck();
  g_Font.m_bAntialiasEffect =
      ((CButton *)m_pDialogBar->GetDlgItem(IDC_EFFECTSANTIALIAS_CHECK))
          ->GetCheck();
  ...
  if (g_Font.m_bOutlineEffect || g_Font.m_bShadowEffect) {
    ...
    ((CButton *)m_pDialogBar->GetDlgItem(IDC_EFFECTSBLURRED_CHECK))
        ->SetCheck(false);
    ((CButton *)m_pDialogBar->GetDlgItem(IDC_EFFECTSSCANLINES_CHECK))
        ->SetCheck(false);
  } else {
    m_pDialogBar->GetDlgItem(IDC_EFFECTSBLURRED_CHECK)->EnableWindow(true);
    m_pDialogBar->GetDlgItem(IDC_EFFECTSSCANLINES_CHECK)->EnableWindow(true);
  ...
  if (bValveEffects &&
      ((CButton *)m_pDialogBar->GetDlgItem(IDC_EFFECTSSCANLINES_CHECK))
          ->GetCheck()) {
    CEdit *pScanlineEdit =
        (CEdit *)m_pDialogBar->GetDlgItem(IDC_SCANLINES_EDIT);
...
//-----------------------------------------------------------------------------
void CFontMakerApp::UpdateSelectedGlyph(BOOL bGlyphSelected,
                                        int iSelectedGlyph) {
  ...
  m_pDialogBar->GetDlgItem(IDC_SELECTEDGLYPH_GROUPBOX)
      ->EnableWindow(g_bIsGlyphSelected);
  m_pDialogBar->GetDlgItem(IDC_GLYPH_VALUE_LABEL)
      ->EnableWindow(g_bIsGlyphSelected);
  ...
  m_pDialogBar->GetDlgItem(IDC_GLYPH_VALUE_STATIC)
      ->EnableWindow(g_bIsGlyphSelected);
  m_pDialogBar->GetDlgItem(IDC_GLYPH_X_STATIC)
      ->EnableWindow(g_bIsGlyphSelected);
  m_pDialogBar->GetDlgItem(IDC_GLYPH_Y_STATIC)
      ->EnableWindow(g_bIsGlyphSelected);
  m_pDialogBar->GetDlgItem(IDC_GLYPH_W_STATIC)
      ->EnableWindow(g_bIsGlyphSelected);
  m_pDialogBar->GetDlgItem(IDC_GLYPH_H_STATIC)
      ->EnableWindow(g_bIsGlyphSelected);
  m_pDialogBar->GetDlgItem(IDC_GLYPH_A_STATIC)
      ->EnableWindow(g_bIsGlyphSelected);
  m_pDialogBar->GetDlgItem(IDC_GLYPH_B_STATIC)
      ->EnableWindow(g_bIsGlyphSelected);
  m_pDialogBar->GetDlgItem(IDC_GLYPH_C_STATIC)
      ->EnableWindow(g_bIsGlyphSelected);
...
void CFontMakerApp::OnSaveButton() {
  ...
  if (FAILED(g_Font.WriteFontImageFile(strTGAFileName, bAdditiveMode,
                                       bCustomFont))) {
...
void CFontMakerApp::OnHelp() {
  ...
  if (ERROR_SUCCESS == RegOpenKeyEx(HKEY_LOCAL_MACHINE,
                                    _T("SOFTWARE\\Microsoft\\XboxSDK"), 0,
                                    KEY_QUERY_VALUE, &hRegKey)) {
    DWORD dwSize = MAX_PATH;
    CHAR InstallPath[MAX_PATH];

    if (ERROR_SUCCESS == RegQueryValueEx(hRegKey, _T("InstallPath"), NULL, NULL,
