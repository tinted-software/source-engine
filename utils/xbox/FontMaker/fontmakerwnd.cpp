...
//       and redraws
//-----------------------------------------------------------------------------
BOOL CFontMakerView::OnEraseBkgnd(CDC *pDC) { return TRUE; }
...
VOID RenderSelectedGlyph(CDC *pDC) {
  ...
  HRGN rgn =
      CreateRectRgn(0, 0, g_Font.m_dwTextureWidth, g_Font.m_dwTextureHeight);
  ...
  GLYPH_ATTR *pGlyph = g_pSelectedGylph;
  {
    ...
    if (g_Font.m_ValidGlyphs[c] == 2) {
      ...
      pDC->FillSolidRect(pGlyph->x, pGlyph->y, pGlyph->w, pGlyph->h,
                         COLOR_BLACK);
    } else {
      int sx = pGlyph->x;
      int sy = pGlyph->y;
...
VOID CFontMakerView::OnNewFontGlyphs() {
  ...
  Bitmap.DeleteObject();
  Bitmap.CreateBitmap(g_Font.m_dwTextureWidth, g_Font.m_dwTextureHeight, 1, 32,
                      g_Font.m_pBits);
...
VOID CFontMakerView::OnDraw(CDC *pDC) {
  ...
  if (g_Font.m_hFont == NULL && !g_Font.m_pCustomFilename) {
    ...
    pDC->FillSolidRect(g_Font.m_dwTextureWidth, 0,
                       max((int)g_Font.m_dwTextureWidth, rc.right),
                       g_Font.m_dwTextureHeight, COLOR_BLACK);
    pDC->FillSolidRect(0, g_Font.m_dwTextureHeight,
                       max((int)g_Font.m_dwTextureWidth, rc.right),
                       max((int)g_Font.m_dwTextureHeight, rc.bottom),
                       COLOR_BLACK);

    // Display the bitmap of all the rendered glyphs
    pDC->BitBlt(0, 0, g_Font.m_dwTextureWidth, g_Font.m_dwTextureHeight,
                &m_memDC, 0, 0, SRCCOPY);
...
void CFontMakerView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) {
  ...
  if (g_Font.m_dwNumGlyphs > 0) {
    if (FALSE == g_bIsGlyphSelected) {
      if (nChar == 37 || nChar == 38 || nChar == 39 ||
          nChar == 40) // Left, right, up or down
      {
      ...
      if (nChar == 37) // Left
        ...
        theApp.UpdateSelectedGlyph(
            TRUE, min((int)g_Font.m_dwNumGlyphs - 1, g_iSelectedGlyphNum + 1));
