...
//	{VK_F1,		"F1",	"WireFrame",		"incrementvar
//mat_wireframe 0 2 1"}, 	{VK_F2, 	"F2",	"FullBright",
//"incrementvar mat_fullbright 0 2 1"}, 	{VK_F3,		"F3",	"Impulse 101",
//"impulse 101"}, 	{VK_F4,		"F4",	"Screenshot",
//"*screenshot"},

bind_t g_bindings[MAX_BINDINGS] = {
    {VK_F1, "F1", NULL, NULL},   {VK_F2, "F2", NULL, NULL},
    {VK_F3, "F3", NULL, NULL},   {VK_F4, "F4", NULL, NULL},
    {VK_F5, "F5", NULL, NULL},   {VK_F6, "F6", NULL, NULL},
    {VK_F7, "F7", NULL, NULL},   {VK_F8, "F8", NULL, NULL},
    {VK_F9, "F9", NULL, NULL},   {VK_F10, "F10", NULL, NULL},
    {VK_F11, "F11", NULL, NULL}, {VK_F12, "F12", NULL, NULL},
};
...
//
//-----------------------------------------------------------------------------
void Bindings_ModifyEntry(int i, const char *pMenuName,
...
void Bindings_UpdateMenu() {
  ...
  for (i = MAX_BINDINGS - 1; i >= 0; i--) {
    ...
    sprintf(menuBuff, "%s\tF%d", g_bindings[i].pMenuName,
            g_bindings[i].keyCode - VK_F1 + 1);
...
//
//-----------------------------------------------------------------------------
BOOL CALLBACK BindingsModifyDlg_Proc(HWND hWnd, UINT message, WPARAM wParam,
                                     LPARAM lParam) {
  switch (message) {
  case WM_INITDIALOG:
  ...
  case WM_COMMAND:
    ...
    case IDC_OK:
      GetDlgItemText(hWnd, IDC_MODIFYBIND_MENUNAME, g_bindingsModify_menuName,
                     sizeof(g_bindingsModify_menuName));
      GetDlgItemText(hWnd, IDC_MODIFYBIND_COMMAND, g_bindingsModify_command,
                     sizeof(g_bindingsModify_command));
    case IDCANCEL:
    case IDC_CANCEL:
...
void BindingsModifyDlg_Open(int selection) {
  ...
  result = DialogBox(g_hInstance, MAKEINTRESOURCE(IDD_MODIFYBIND), g_hDlgMain,
                     (DLGPROC)BindingsModifyDlg_Proc);
  if (LOWORD(result) != IDC_OK)
    return;

  // accept changes and update
  Bindings_ModifyEntry(selection, g_bindingsModify_menuName,
                       g_bindingsModify_command);
...
void Bindings_LoadConfig() {
  ...
  numArgs = sscanf(buff, "%d %d %d %d", &g_bindings_windowRect.left,
                   &g_bindings_windowRect.top, &g_bindings_windowRect.right,
                   &g_bindings_windowRect.bottom);
  if (numArgs != 4)
...
void Bindings_SaveConfig() {
  ...
  for (i = 0; i < MAX_BINDINGS; i++) {
    ...
    len = _snprintf(valueBuff, sizeof(valueBuff), "%d \"%s\" \"%s\"",
                    g_bindings[i].keyCode, pMenuName, pCommandString);
    if (len == -1) {
      // kill it
      valueBuff[0] = '\0';
    }
...
//
//-----------------------------------------------------------------------------
LRESULT CALLBACK Bindings_WndProc(HWND hwnd, UINT message, WPARAM wParam,
                                  LPARAM lParam) {
...
void Bindings_Open() {
  ...
  hWnd = CreateWindowEx(WS_EX_CLIENTEDGE, "BINDINGSCLASS", "Edit Bindings",
  ...
  hWnd = CreateWindow(WC_LISTVIEW, "",
                      ...
                      clientRect.bottom - clientRect.top, g_bindings_hWnd,
                      (HMENU)ID_BINDINGS_LISTVIEW, g_hInstance, NULL);
  g_bindings_hWndListView = hWnd;

  // init list view columns
  for (i = 0; i < sizeof(g_bindings_labels) / sizeof(g_bindings_labels[0]);
       i++) {
    LVCOLUMN lvc;
  ...
  ListView_SetItemState(g_bindings_hWndListView, 0, LVIS_SELECTED,
                        LVIS_SELECTED);
  SetFocus(g_bindings_hWndListView);

  if (g_bindings_windowRect.right && g_bindings_windowRect.bottom)
    MoveWindow(g_bindings_hWnd, g_bindings_windowRect.left,
               g_bindings_windowRect.top,
               g_bindings_windowRect.right - g_bindings_windowRect.left,
               g_bindings_windowRect.bottom - g_bindings_windowRect.top, FALSE);
