...
char *SystemTimeToString(SYSTEMTIME *systemTime, char *buffer, int bufferSize) {
  ...
  GetDateFormat(LOCALE_SYSTEM_DEFAULT, 0, systemTime, "MM'/'dd'/'yyyy",
                dateString, sizeof(dateString));
  GetTimeFormat(LOCALE_SYSTEM_DEFAULT, 0, systemTime, "hh':'mm':'ss tt",
...
//
//-----------------------------------------------------------------------------
int CompareFileTimes_NTFStoFATX(FILETIME *ntfsFileTime, char *ntfsTimeString,
                                int ntfsStringSize, FILETIME *fatxFileTime,
                                char *fatxTimeString, int fatxStringSize) {
  ...
  // operate on local times, assumes xbox and pc are both set for same time zone
  // and daylight saving
  ...
  // allow for +/- 3s error
  ntfsSeconds = ntfsLocalTime.wHour * 60 * 60 + ntfsLocalTime.wMinute * 60 +
                ntfsLocalTime.wSecond;
  fatxSeconds = fatxLocalTime.wHour * 60 * 60 + fatxLocalTime.wMinute * 60 +
...
//
//-----------------------------------------------------------------------------
bool GetTargetFileList_r(char *targetPath, bool recurse, int attributes,
                         int level, fileNode_t **pFileList) {
  HRESULT hr;
  ...
  while (1) {
    ...
    if ((bGetNormal && !fileAttr.Attributes) ||
        (fileAttr.Attributes & fixedAttributes)) {
    ...
    if (fileAttr.Attributes & FILE_ATTRIBUTE_DIRECTORY) {
      if (recurse) {
        ...
        valid = GetTargetFileList_r(filename, recurse, attributes, level + 1,
                                    pFileList);
        if (!valid)
          return false;
      }
...
//-----------------------------------------------------------------------------
int FileSyncEx(const char *localFilename, const char *targetFilename,
               int fileSyncMode, bool bVerbose, bool bNoWrite) {
  bool copy;
  bool pathExist;
  ...
  if (!GetFileAttributesEx(localFilename, GetFileExInfoStandard,
                           &localAttributes)) {
    // failed to get the local file's attributes
    if (bVerbose) {
      ConsoleWindowPrintf(XBX_CLR_RED,
                          "Sync Failure: Local file %s not available\n",
                          localFilename);
    }
    return -1;
  }
    ...
    if (hr != XBDM_NOERR) {
      ...
      if (bVerbose) {
        ConsoleWindowPrintf(XBX_CLR_DEFAULT,
                            "No Update, Target file %s not available\n",
                            targetFilename);
      }
      return 0;
    }
  ...
  if ((fileSyncMode & FSYNC_TYPEMASK) == FSYNC_IFNEWER) {
    ...
    if (hr == XBDM_NOERR) {
      ...
      // compare times
      deltaTime = CompareFileTimes_NTFStoFATX(
          &localAttributes.ftLastWriteTime, localTimeString,
          sizeof(localTimeString), &targetAttributes.ChangeTime,
          targetTimeString, sizeof(targetTimeString));
      if (deltaTime < 0) {
        // ntfs is older, fatx is newer, no update
        if (bVerbose) {
          ConsoleWindowPrintf(
              XBX_CLR_DEFAULT, "No Update, %s [%s] is newer than %s [%s]\n",
        ...
        if (bVerbose) {
          if (copy) {
            ConsoleWindowPrintf(
                XBX_CLR_DEFAULT,
                "Update, %s [%s] [%d] has different size than %s [%s] [%d]\n",
                targetFilename, targetTimeString, targetAttributes.SizeLow,
                localFilename, localTimeString, localAttributes.nFileSizeLow);
          } else {
            ConsoleWindowPrintf(XBX_CLR_DEFAULT,
                                "No Update, %s [%s] [%d] has same time and "
                                "file size as %s [%s] [%d]\n",
                                targetFilename, targetTimeString,
                                targetAttributes.SizeLow, localFilename,
                                localTimeString, localAttributes.nFileSizeLow);
          }
        }
      } else {
        ...
        if (bVerbose) {
          ConsoleWindowPrintf(
              XBX_CLR_DEFAULT, "Update, %s [%s] is older than %s [%s]\n",
              targetFilename, targetTimeString, localFilename, localTimeString);
        }
      }
    }
...
bool LoadTargetFile(const char *pTargetPath, int *pFileSize, void **pData) {
  ...
  *pData = (void *)NULL;
  ...
  hr = DmReadFilePartial(pTargetPath, 0, (LPBYTE)pBuffer,
                         fileAttributes.SizeLow, &bytesRead);
