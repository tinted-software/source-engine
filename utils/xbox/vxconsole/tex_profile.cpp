...
void TexProfile_LoadConfig() {
  ...
  numArgs = sscanf(buff, "%d %d %d %d", &g_texProfile_samplesWindowRect.left,
                   &g_texProfile_samplesWindowRect.top,
                   &g_texProfile_samplesWindowRect.right,
                   &g_texProfile_samplesWindowRect.bottom);
  if (numArgs != 4 || g_texProfile_samplesWindowRect.left < 0 ||
      g_texProfile_samplesWindowRect.top < 0 ||
      g_texProfile_samplesWindowRect.right < 0 ||
      g_texProfile_samplesWindowRect.bottom < 0)
    memset(&g_texProfile_samplesWindowRect, 0,
           sizeof(g_texProfile_samplesWindowRect));
  Sys_GetRegistryInteger("texProfileSamplesScale", 0,
                         g_texProfile_samples_scale);
  if (g_texProfile_samples_scale < -TEXPROFILE_SAMPLES_SCALESTEPS ||
      g_texProfile_samples_scale > TEXPROFILE_SAMPLES_SCALESTEPS)
    g_texProfile_samples_scale = 0;

  // profile history
  Sys_GetRegistryString("texProfileHistoryWindowRect", buff, "", sizeof(buff));
  numArgs = sscanf(buff, "%d %d %d %d", &g_texProfile_historyWindowRect.left,
                   &g_texProfile_historyWindowRect.top,
                   &g_texProfile_historyWindowRect.right,
                   &g_texProfile_historyWindowRect.bottom);
  if (numArgs != 4 || g_texProfile_historyWindowRect.left < 0 ||
      g_texProfile_historyWindowRect.top < 0 ||
      g_texProfile_historyWindowRect.right < 0 ||
      g_texProfile_historyWindowRect.bottom < 0)
    memset(&g_texProfile_historyWindowRect, 0,
           sizeof(g_texProfile_historyWindowRect));
  Sys_GetRegistryInteger("texProfileHistoryScale", 0,
                         g_texProfile_history_scale);
  if (g_texProfile_history_scale < -TEXPROFILE_HISTORY_SCALESTEPS ||
      g_texProfile_history_scale > TEXPROFILE_HISTORY_SCALESTEPS)
    g_texProfile_history_scale = 0;

  Sys_GetRegistryInteger("texProfileCurrentFrame", 0,
                         g_texProfile_currentFrame);
}
...
void TexProfile_SaveConfig() {
  ...
  if (g_texProfile_hWndSamples) {
    ...
    sprintf(buff, "%d %d %d %d", wp.rcNormalPosition.left,
...
void TexProfile_SetTitle() {
  ...
  if (g_texProfile_hWndSamples) {
    ...
    if (VProf_GetState() == VPROF_TEXTURE ||
        VProf_GetState() == VPROF_TEXTUREFRAME)
      strcat(titleBuff, " [ON]");
...
int rc_SetTexProfile(char *commandPtr) {
  ...
  DmGetMemory((void *)profileList, numProfiles * sizeof(xrProfile_t), localList,
              NULL);
  ...
  for (i = 0; i < g_texProfile_numCounters; i++) {
    ...
    V_strncpy(g_texProfile_counters[i].label, localList[i].labelString,
              sizeof(g_texProfile_counters[i].label));
    g_texProfile_counters[i].color = localList[i].color;
  }

  // build out the reserved last counter as total count
  memset(&g_texProfile_counters[g_texProfile_numCounters], 0,
         sizeof(profileCounter_t));
...
int rc_SetTexProfileData(char *commandPtr) {
  ...
  if (g_texProfile_numCounters)
    DmGetMemory((void *)counters, (g_texProfile_numCounters - 1) * sizeof(int),
                localCounters, NULL);
  ...
  for (i = 0; i < g_texProfile_numCounters; i++) {
    ...
    if (newPeaks || g_texProfile_counters[i].peakSample <
                        g_texProfile_counters[i].samples[currentSample])
      g_texProfile_counters[i].peakSample =
          g_texProfile_counters[i].samples[currentSample];
  }
...
void TexProfileSamples_Draw(HDC hdc, RECT *clientRect) {
  ...
  // zoom
  scale = TexProfile_CalcScale(
      g_texProfile_samples_scale, TEXPROFILE_SAMPLES_SCALESTEPS,
      TEXPROFILE_SAMPLES_MINSCALE, TEXPROFILE_SAMPLES_MAXSCALE);
  ...
  for (i = 0; i < numTicks; i++) {
    ...
    if (g_texProfile_samples_tickMarks &&
        g_texProfile_samples_scale > -TEXPROFILE_SAMPLES_SCALESTEPS) {
      // minor ticks
      x0 = x;
      y0 = y + TEXPROFILE_SAMPLES_ITEMHEIGHT;
  ...
  for (i = 0; i < g_texProfile_numCounters; i++) {
    ...
    rect.left = x;
    rect.right =
        x + TEXPROFILE_SAMPLES_LABELWIDTH - TEXPROFILE_SAMPLES_LABELGAP;
    rect.top = y;
    rect.bottom = y + TEXPROFILE_SAMPLES_ITEMHEIGHT;
    DrawText(hdc, g_texProfile_counters[i].label, -1, &rect,
             DT_VCENTER | DT_RIGHT | DT_SINGLELINE | DT_END_ELLIPSIS |
                 DT_MODIFYSTRING);

    // draw the under line
    MoveToEx(hdc, x, y + TEXPROFILE_SAMPLES_ITEMHEIGHT, NULL);
    LineTo(hdc, x + TEXPROFILE_SAMPLES_LABELWIDTH,
           y + TEXPROFILE_SAMPLES_ITEMHEIGHT);
  ...
  for (i = 0; i < g_texProfile_numCounters; i++) {
    ...
    hColoredBrush = CreateSolidBrush(g_texProfile_samples_colors
                                         ? g_texProfile_counters[i].color
                                         : g_backgroundColor);
    hbrushOld = (HBRUSH)SelectObject(hdc, hColoredBrush);

    // bar - count is in bytes, scale to major tick
    t = (float)g_texProfile_counters[i].samples[currentSample] /
        TEXPROFILE_MAJORTICKBYTES;
    w = (int)(t * (float)tickWidth);
    if (w > windowWidth)
      w = windowWidth;
    x0 = x;
    y0 = y +
         (TEXPROFILE_SAMPLES_ITEMHEIGHT - TEXPROFILE_SAMPLES_BARHEIGHT) / 2 + 1;
...
void TexProfileHistory_Draw(HDC hdc, RECT *clientRect) {
  ...
  scale = TexProfile_CalcScale(
      g_texProfile_history_scale, TEXPROFILE_HISTORY_SCALESTEPS,
      TEXPROFILE_HISTORY_MINSCALE, TEXPROFILE_HISTORY_MAXSCALE);
  ...
  for (i = 0; i < numTicks; i++) {
    ...
    if (g_texProfile_history_tickMarks &&
        g_texProfile_history_scale > -TEXPROFILE_HISTORY_SCALESTEPS) {
      // minor ticks
      y0 = y;
      SelectObject(hdc, hGreyPen);
  ...
  if (g_texProfile_numSamples) {
    ...
    for (x = numbars - 1; x >= 0; x -= 4) {
      ...
      for (j = 0; j < g_texProfile_numCounters - 1; j++) {
        ...
        t = (float)g_texProfile_counters[j]
                .samples[currentSample % TEXPROFILE_MAXSAMPLES] /
            TEXPROFILE_MAJORTICKBYTES;
        ...
        if (h) {
          ...
          hColoredBrush = CreateSolidBrush(g_texProfile_history_colors
                                               ? g_texProfile_counters[j].color
                                               : RGB(80, 80, 80));
          hBrushOld = (HBRUSH)SelectObject(hdc, hColoredBrush);
...
//
//-----------------------------------------------------------------------------
LRESULT CALLBACK TexProfile_WndProc(HWND hwnd, UINT message, WPARAM wParam,
                                    LPARAM lParam) {
  ...
  case WM_CREATE:
    ...
    SetWindowLong(hwnd, GWL_USERDATA + 0,
                  (LONG)createStructPtr->lpCreateParams);
  ...
  case WM_INITMENU:
    if (bIsSamples) {
      CheckMenuItem(
          (HMENU)wParam, IDM_TEXPROFILE_TICKMARKS,
          MF_BYCOMMAND |
              (g_texProfile_samples_tickMarks ? MF_CHECKED : MF_UNCHECKED));
      CheckMenuItem(
          (HMENU)wParam, IDM_TEXPROFILE_COLORS,
          MF_BYCOMMAND |
              (g_texProfile_samples_colors ? MF_CHECKED : MF_UNCHECKED));
    } else if (bIsHistory) {
      CheckMenuItem(
          (HMENU)wParam, IDM_TEXPROFILE_TICKMARKS,
          MF_BYCOMMAND |
              (g_texProfile_history_tickMarks ? MF_CHECKED : MF_UNCHECKED));
      CheckMenuItem(
          (HMENU)wParam, IDM_TEXPROFILE_COLORS,
          MF_BYCOMMAND |
              (g_texProfile_history_colors ? MF_CHECKED : MF_UNCHECKED));
    }
    CheckMenuItem((HMENU)wParam, IDM_TEXPROFILE_ENABLE,
                  MF_BYCOMMAND | ((VProf_GetState() == VPROF_TEXTURE ||
                                   VProf_GetState() == VPROF_TEXTUREFRAME)
                                      ? MF_CHECKED
                                      : MF_UNCHECKED));
    CheckMenuItem((HMENU)wParam, IDM_TEXPROFILE_CURRENTFRAME,
                  MF_BYCOMMAND |
                      (g_texProfile_currentFrame ? MF_CHECKED : MF_UNCHECKED));
    return 0L;
  ...
  case WM_KEYDOWN:
    ...
    case VK_INSERT:
      if (bIsSamples)
        TexProfile_ZoomIn(g_texProfile_samples_scale,
                          TEXPROFILE_SAMPLES_SCALESTEPS);
      else if (bIsHistory)
        TexProfile_ZoomIn(g_texProfile_history_scale,
                          TEXPROFILE_HISTORY_SCALESTEPS);
      return 0L;

    case VK_DELETE:
      if (bIsSamples)
        TexProfile_ZoomOut(g_texProfile_samples_scale,
                           TEXPROFILE_SAMPLES_SCALESTEPS);
      else if (bIsHistory)
  ...
  case WM_COMMAND:
    ...
    case IDM_TEXPROFILE_ENABLE:
      bIsEnabled = (VProf_GetState() == VPROF_TEXTURE ||
                    VProf_GetState() == VPROF_TEXTUREFRAME);
      bIsEnabled ^= 1;
      if (!bIsEnabled)
        VProf_Enable(VPROF_OFF);
...
void TexProfileHistory_Open() {
  ...
  hWnd = CreateWindowEx(WS_EX_CLIENTEDGE, "TEXPROFILEHISTORYCLASS", "",
                        WS_POPUP | WS_CAPTION | WS_SYSMENU | WS_SIZEBOX |
                            WS_MINIMIZEBOX | WS_MAXIMIZEBOX,
                        0, 0, 600, 500, g_hDlgMain, NULL, g_hInstance,
                        (void *)ID_TEXPROFILE_HISTORY);
  g_texProfile_hWndHistory = hWnd;

  TexProfile_SetTitle();

  if (g_texProfile_historyWindowRect.right &&
      g_texProfile_historyWindowRect.bottom)
    MoveWindow(g_texProfile_hWndHistory, g_texProfile_historyWindowRect.left,
               g_texProfile_historyWindowRect.top,
               g_texProfile_historyWindowRect.right -
                   g_texProfile_historyWindowRect.left,
               g_texProfile_historyWindowRect.bottom -
                   g_texProfile_historyWindowRect.top,
               FALSE);
...
void TexProfileSamples_Open() {
  ...
  hWnd = CreateWindowEx(WS_EX_CLIENTEDGE, "TEXPROFILESAMPLESCLASS", "",
                        WS_POPUP | WS_CAPTION | WS_SYSMENU | WS_SIZEBOX |
                            WS_MINIMIZEBOX | WS_MAXIMIZEBOX,
                        0, 0, 600, 500, g_hDlgMain, NULL, g_hInstance,
  ...
  if (g_texProfile_samplesWindowRect.right &&
      g_texProfile_samplesWindowRect.bottom)
    MoveWindow(g_texProfile_hWndSamples, g_texProfile_samplesWindowRect.left,
               g_texProfile_samplesWindowRect.top,
               g_texProfile_samplesWindowRect.right -
                   g_texProfile_samplesWindowRect.left,
               g_texProfile_samplesWindowRect.bottom -
                   g_texProfile_samplesWindowRect.top,
               FALSE);
