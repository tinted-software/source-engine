...
GameSystem_t g_Games[] = {
    {"Half Life 2", "hl2", true},     {"Episode 1", "episodic", true},
    {"Episode 2", "ep2", true},       {"Portal", "portal", false},
...
IBugReporter *BugReporter_LoadDLL(const char *pDLLName, HMODULE *phModule) {
  ...
  *phModule = NULL;
  ...
  pBugReporter = (IBugReporter *)((CreateInterfaceFn)pCreateInterface)(
      INTERFACEVERSION_BUGREPORTER, NULL);
  if (!pBugReporter) {
...
bool BugReporter_GetInterfaces() {
  if (!g_bug_pReporter1) {
    g_bug_pReporter1 =
        BugReporter_LoadDLL(BUG_REPORTER_DLLNAME_1, &g_bug_hBugReporter1);
    if (!g_bug_pReporter1) {
  ...
  if (!g_bug_pReporter2) {
    g_bug_pReporter2 =
        BugReporter_LoadDLL(BUG_REPORTER_DLLNAME_2, &g_bug_hBugReporter2);
    if (!g_bug_pReporter2) {
  ...
  if (g_bug_pReporter1) {
    ...
    ConsoleWindowPrintf(BUG_COLOR,
                        "Bug Reporter: PVCS Username: '%s' Display As: '%s'\n",
                        g_bug_pReporter1->GetUserName(),
                        g_bug_pReporter1->GetUserName_Display());
  }
  if (g_bug_pReporter2) {
    // determine submitter name
    ConsoleWindowPrintf(
        BUG_COLOR, "Bug Reporter: BugBait Username: '%s' Display As: '%s'\n",
        g_bug_pReporter2->GetUserName(),
        g_bug_pReporter2->GetUserName_Display());
  }
  ...
  char fn[MAX_PATH];
  V_snprintf(fn, sizeof(fn), "%s/%s", GetRepositoryURL(),
...
bool BugDlg_CompressScreenshot() {
  ...
  hBitmap = (HBITMAP)LoadImage(NULL, g_bug_szScreenshot, IMAGE_BITMAP, 0, 0,
  ...
  // get the raw bits
  if (!GetDIBits(hDC, hBitmap, 0, nHeight, pBMPBits, &bitmapInfo,
                 DIB_RGB_COLORS))
    goto cleanUp;
  ...
  while (cinfo.next_scanline < cinfo.image_height) {
    ...
    if (bitmapInfo.bmiHeader.biHeight < 0) {
      ...
      // bottom up
      pSrc = &pBMPBits[(nHeight - 1 - cinfo.next_scanline) *
...
//
//-----------------------------------------------------------------------------
void BugDlg_GetDataFileBase(char const *suffix, bool bLocalPath, char *buf,
                            int bufsize) {
  ...
  sprintf(filename, "%i_%02i_%02i_%s", t.tm_year + 1900, t.tm_mon + 1,
          t.tm_mday, who);
...
bool BugDlg_CheckSubmit(HWND hWnd) {
  ...
  if (g_Games[g_bug_GameType].bUsesSystem1) {
    if (g_bug_szTitle[0] && g_bug_szDescription[0] && g_bug_szSeverity[0] &&
        g_bug_szOwner[0] && g_bug_szReportType[0] && g_bug_szPriority[0] &&
        g_bug_szArea[0]) {
      bEnableSubmit = true;
    }
  } else {
    if (g_bug_szTitle[0] && g_bug_szDescription[0] && g_bug_szSeverity[0] &&
        g_bug_szOwner[0] && g_bug_szArea[0] && g_bug_szMapNumber[0]) {
      bEnableSubmit = true;
    }
  }
...
bool BugDlg_GetChanges(HWND hWnd) {
  ...
  // description
  GetDlgItemText(hWnd, IDC_BUG_DESCRIPTION, g_bug_szDescription,
                 sizeof(g_bug_szDescription));
  ...
  if (curSel == CB_ERR) {
    ...
    strncpy(g_bug_szOwner, g_bug_pReporter->GetDisplayName(curSel),
            sizeof(g_bug_szOwner));
  ...
  if (curSel == CB_ERR) {
    ...
    V_strncpy(g_bug_szSeverity, g_bug_pReporter->GetSeverity(curSel),
              sizeof(g_bug_szSeverity));
  }
  ...
  if (curSel == CB_ERR) {
    ...
    V_strncpy(g_bug_szReportType, g_bug_pReporter->GetReportType(curSel),
              sizeof(g_bug_szReportType));
  }
  ...
  if (curSel == CB_ERR) {
    ...
    V_strncpy(g_bug_szPriority, g_bug_pReporter->GetPriority(curSel),
              sizeof(g_bug_szPriority));
  }
  ...
  if (curSel == CB_ERR) {
    ...
    V_strncpy(g_bug_szArea, g_bug_pReporter->GetArea(curSel),
              sizeof(g_bug_szArea));
    areaIndex = curSel;
  }
  ...
  if (!g_Games[g_bug_GameType].bUsesSystem1) {
    ...
    if (curSel == CB_ERR) {
      ...
      V_strncpy(g_bug_szMapNumber, g_bug_pReporter->GetLevel(areaIndex, curSel),
                sizeof(g_bug_szMapNumber));
    }
  }

  g_bug_bCompressScreenshot =
      (IsDlgButtonChecked(hWnd, IDC_BUG_COMPRESS_SCREENSHOT) != 0);
...
void BugDlg_Populate(HWND hWnd) {
  ...
  // title
  SendDlgItemMessage(hWnd, IDC_BUG_TITLE, EM_LIMITTEXT,
                     sizeof(g_bug_szTitle) - 1, 0);
  SetDlgItemText(hWnd, IDC_BUG_TITLE, g_bug_szTitle);

  // description
  SendDlgItemMessage(hWnd, IDC_BUG_DESCRIPTION, EM_LIMITTEXT,
                     sizeof(g_bug_szDescription) - 1, 0);
  SetDlgItemText(hWnd, IDC_BUG_DESCRIPTION, g_bug_szDescription);

  // submitter
  SetDlgItemText(hWnd, IDC_BUG_SUBMITTER_LABEL,
                 g_bug_pReporter->GetUserName_Display());

  // owner
  SendDlgItemMessage(hWnd, IDC_BUG_OWNER, CB_RESETCONTENT, 0, 0);
  count = g_bug_pReporter->GetDisplayNameCount();
  for (i = 0; i < count; i++) {
    SendDlgItemMessage(hWnd, IDC_BUG_OWNER, CB_ADDSTRING, 0,
                       (LPARAM)(LPCSTR)g_bug_pReporter->GetDisplayName(i));
  }
  if (count) {
    curSel = SendDlgItemMessage(hWnd, IDC_BUG_OWNER, CB_FINDSTRINGEXACT,
                                (WPARAM)-1, (LPARAM)(LPCSTR)g_bug_szOwner);
    if (curSel != CB_ERR) {
  ...
  for (i = 0; i < count; i++) {
    SendDlgItemMessage(hWnd, IDC_BUG_SEVERITY, CB_ADDSTRING, 0,
                       (LPARAM)(LPCSTR)g_bug_pReporter->GetSeverity(i));
  }
  if (count) {
    curSel = SendDlgItemMessage(hWnd, IDC_BUG_SEVERITY, CB_FINDSTRINGEXACT,
                                (WPARAM)-1, (LPARAM)(LPCSTR)g_bug_szSeverity);
    if (curSel != CB_ERR) {
  ...
  EnableWindow(GetDlgItem(hWnd, IDC_BUG_REPORTTYPE),
               g_Games[g_bug_GameType].bUsesSystem1);
  if (g_Games[g_bug_GameType].bUsesSystem1) {
    count = g_bug_pReporter->GetReportTypeCount();
    for (i = 0; i < count; i++) {
      SendDlgItemMessage(hWnd, IDC_BUG_REPORTTYPE, CB_ADDSTRING, 0,
                         (LPARAM)(LPCSTR)g_bug_pReporter->GetReportType(i));
    }
    if (count) {
      curSel =
          SendDlgItemMessage(hWnd, IDC_BUG_REPORTTYPE, CB_FINDSTRINGEXACT,
                             (WPARAM)-1, (LPARAM)(LPCSTR)g_bug_szReportType);
      if (curSel != CB_ERR) {
  ...
  EnableWindow(GetDlgItem(hWnd, IDC_BUG_PRIORITY),
               g_Games[g_bug_GameType].bUsesSystem1);
  if (g_Games[g_bug_GameType].bUsesSystem1) {
    count = g_bug_pReporter->GetPriorityCount();
    for (i = 0; i < count; i++) {
      SendDlgItemMessage(hWnd, IDC_BUG_PRIORITY, CB_ADDSTRING, 0,
                         (LPARAM)(LPCSTR)g_bug_pReporter->GetPriority(i));
    }
    if (count) {
      curSel = SendDlgItemMessage(hWnd, IDC_BUG_PRIORITY, CB_FINDSTRINGEXACT,
                                  (WPARAM)-1, (LPARAM)(LPCSTR)g_bug_szPriority);
      if (curSel != CB_ERR) {
  ...
  int areaIndex = 0;
  for (i = 0; i < count; i++) {
    SendDlgItemMessage(hWnd, IDC_BUG_AREA, CB_ADDSTRING, 0,
                       (LPARAM)(LPCSTR)g_bug_pReporter->GetArea(i));
  }
  if (count) {
    curSel = SendDlgItemMessage(hWnd, IDC_BUG_AREA, CB_FINDSTRINGEXACT,
                                (WPARAM)-1, (LPARAM)(LPCSTR)g_bug_szArea);
    if (curSel != CB_ERR) {
  ...
  EnableWindow(GetDlgItem(hWnd, IDC_BUG_MAPNUMBER),
               g_Games[g_bug_GameType].bUsesSystem1 == false);
  if (!g_Games[g_bug_GameType].bUsesSystem1) {
    // new system has map names
    count = g_bug_pReporter->GetLevelCount(areaIndex);
    for (i = 0; i < count; i++) {
      SendDlgItemMessage(
          hWnd, IDC_BUG_MAPNUMBER, CB_ADDSTRING, 0,
          (LPARAM)(LPCSTR)g_bug_pReporter->GetLevel(areaIndex, i));
    }
  }
  if (count) {
    curSel = SendDlgItemMessage(hWnd, IDC_BUG_MAPNUMBER, CB_FINDSTRINGEXACT,
                                (WPARAM)-1, (LPARAM)(LPCSTR)g_bug_szMapNumber);
    if (curSel != CB_ERR) {
  ...
  for (i = 0; i < count; i++) {
    SendDlgItemMessage(hWnd, IDC_BUG_GAME, CB_ADDSTRING, 0,
                       (LPARAM)(LPCSTR)g_Games[i].pFriendlyName);
  }
  if (count) {
    curSel = SendDlgItemMessage(
        hWnd, IDC_BUG_GAME, CB_FINDSTRINGEXACT, (WPARAM)-1,
        (LPARAM)(LPCSTR)g_Games[g_bug_GameType].pFriendlyName);
    if (curSel != CB_ERR) {
      SendDlgItemMessage(hWnd, IDC_BUG_GAME, CB_SETCURSEL, curSel, 0);
    }
  }

  CheckDlgButton(hWnd, IDC_BUG_COMPRESS_SCREENSHOT,
                 g_bug_bCompressScreenshot ? BST_CHECKED : BST_UNCHECKED);
...
void BugDlg_TakeScreenshot(HWND hWnd) {
  ...
  BugDlg_GetDataFileBase(g_bug_pReporter->GetUserName(), true,
                         g_bug_szScreenshot, sizeof(g_bug_szScreenshot));
...
void BugDlg_SaveGame(HWND hWnd) {
  ...
  BugDlg_GetDataFileBase(g_bug_pReporter->GetUserName(), true, g_bug_szSavegame,
                         sizeof(g_bug_szSavegame));
...
void BugDlg_IncludeBSP(HWND hWnd) {
  ...
  BugDlg_GetDataFileBase(g_bug_pReporter->GetUserName(), true, g_bug_szBSPName,
                         sizeof(g_bug_szBSPName));
...
//
//-----------------------------------------------------------------------------
bool BugDlg_UploadFile(char const *pLocalName, char const *pRemoteName,
                       bool bDeleteLocal) {
  FILE *fp;
  int len;
  void *pLocalData;
  ...
  if (!pLocalData || !len) {
    ConsoleWindowPrintf(XBX_CLR_RED,
                        "UploadFile: Unable to open local path '%s'\n",
                        pLocalName);
    return false;
  }
  ...
  if (!fp) {
    ConsoleWindowPrintf(XBX_CLR_RED,
                        "UploadFile: Unable to open remote path '%s'\n",
                        pRemoteName);
    Sys_Free(pLocalData);
    return false;
  }
...
//-----------------------------------------------------------------------------
bool BugDlg_UploadBugSubmission(int bugID, char const *pSavefile,
                                char const *pScreenshot, char const *pBspFile,
                                char const *pVmfFile) {
  ...
  if (pSavefile && pSavefile[0]) {
    ...
    V_snprintf(szRemotefile, sizeof(szRemotefile), "%s/%s",
               GetSubmissionURL(bugID), szFilename);
...
bool BugDlg_Submit(HWND hWnd) {
  ...
  sprintf(positionName, "%f %f %f", g_bug_mapInfo.position[0],
          g_bug_mapInfo.position[1], g_bug_mapInfo.position[2]);
  SetDlgItemText(g_bug_hWnd, IDC_BUG_POSITION_LABEL, positionName);

  sprintf(orientationName, "%f %f %f", g_bug_mapInfo.angle[0],
          g_bug_mapInfo.angle[1], g_bug_mapInfo.angle[2]);
  ...
  g_bug_pReporter->SetOwner(
      g_bug_pReporter->GetUserNameForDisplayName(g_bug_szOwner));
  ...
  if (g_bug_szScreenshot[0]) {
    ...
    V_snprintf(filename, sizeof(filename), "%s/BugId/%s", GetRepositoryURL(),
               basename);
  ...
  if (bSuccess) {
    if (!BugDlg_UploadBugSubmission(bugID, g_bug_szSavegame, g_bug_szScreenshot,
                                    g_bug_szBSPName, NULL)) {
      Sys_MessageBox(BUG_ERRORTITLE,
                     "Unable to upload files to bug repository!\n");
      bSuccess = false;
    }
  } else {
  ...
  if (bSuccess) {
    if (g_Games[g_bug_GameType].bUsesSystem1) {
      ConsoleWindowPrintf(
          BUG_COLOR, "Bug Reporter: PVCS submission succeeded for bug! (%d)\n",
          bugID);
    } else {
      ConsoleWindowPrintf(
          BUG_COLOR, "Bug Reporter: BugBait submission succeeded for bug!\n");
    }
  } else {
...
//
//-----------------------------------------------------------------------------
BOOL CALLBACK BugDlg_Proc(HWND hwnd, UINT message, WPARAM wParam,
                          LPARAM lParam) {
  switch (message) {
  case WM_INITDIALOG:
    BugDlg_Setup(hwnd);
...
void BugDlg_LoadConfig() {
  ...
  Sys_GetRegistryString("bug_severity", g_bug_szSeverity, "",
                        sizeof(g_bug_szSeverity));
  Sys_GetRegistryString("bug_reporttype", g_bug_szReportType, "",
                        sizeof(g_bug_szReportType));
  Sys_GetRegistryString("bug_priority", g_bug_szPriority, "",
                        sizeof(g_bug_szPriority));
...
void BugDlg_Open(void) {
  ...
  result = DialogBox(g_hInstance, MAKEINTRESOURCE(IDD_BUG), g_hDlgMain,
                     (DLGPROC)BugDlg_Proc);
  ...
  if (g_bug_hWnd) {
    if (g_bug_mapInfo.mapPath[0]) {
      ...
      if (!g_Games[g_bug_GameType].bUsesSystem1) {
        ...
        int curSel = SendDlgItemMessage(g_bug_hWnd, IDC_BUG_MAPNUMBER,
                                        CB_FINDSTRINGEXACT, (WPARAM)-1,
                                        (LPARAM)(LPCSTR)g_bug_szMapNumber);
        if (curSel == CB_ERR)
          curSel = 0;
        SendDlgItemMessage(g_bug_hWnd, IDC_BUG_MAPNUMBER, CB_SETCURSEL, curSel,
                           0);
      }

      sprintf(buff, "%.2f %.2f %.2f", g_bug_mapInfo.position[0],
              g_bug_mapInfo.position[1], g_bug_mapInfo.position[2]);
      SetDlgItemText(g_bug_hWnd, IDC_BUG_POSITION_LABEL, buff);

      sprintf(buff, "%.2f %.2f %.2f", g_bug_mapInfo.angle[0],
              g_bug_mapInfo.angle[1], g_bug_mapInfo.angle[2]);
    ...
    EnableWindow(GetDlgItem(g_bug_hWnd, IDC_BUG_SAVEGAME),
                 g_bug_mapInfo.savePath[0] != '\0' &&
                     g_bug_mapInfo.mapPath[0] != '\0');
    EnableWindow(GetDlgItem(g_bug_hWnd, IDC_BUG_INCLUDEBSP),
                 g_bug_mapInfo.mapPath[0] != '\0');
  }
