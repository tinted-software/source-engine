...
char *g_showTextures_loadStrings[LS_MAX] = {
    "Static", "Procedural", "Synchronous", "Fallback", "", "Failed",
};

label_t g_showTextures_Labels[] = {
    {"Name", 150, ID_ST_NAME},       {"D3D Size", 80, ID_ST_SIZE},
    {"Group", 150, ID_ST_GROUP},     {"Format", 120, ID_ST_FORMAT},
    {"Width", 80, ID_ST_WIDTH},      {"Height", 80, ID_ST_HEIGHT},
    {"Depth", 80, ID_ST_DEPTH},      {"NumLevels", 80, ID_ST_NUMLEVELS},
    {"Binds", 80, ID_ST_BINDS},      {"RefCount", 80, ID_ST_REFCOUNT},
...
void ShowTextures_SaveConfig() {
  ...
  Sys_SetRegistryInteger("showTexturesCurrentFrame",
                         g_showTextures_currentFrame);
  Sys_SetRegistryInteger("showTexturesSortColumn", g_showTextures_sortColumn);
  Sys_SetRegistryInteger("showTexturesSortDescending",
                         g_showTextures_sortDescending);
  ...
  sprintf(buff, "%d %d %d %d", g_showTextures_windowRect.left,
          g_showTextures_windowRect.top, g_showTextures_windowRect.right,
          g_showTextures_windowRect.bottom);
...
void ShowTextures_LoadConfig() {
  ...
  Sys_GetRegistryInteger("showTexturesCurrentFrame", false,
                         g_showTextures_currentFrame);
  Sys_GetRegistryInteger("showTexturesSortColumn", ID_ST_NAME,
                         g_showTextures_sortColumn);
  Sys_GetRegistryInteger("showTexturesSortDescending", false,
                         g_showTextures_sortDescending);

  Sys_GetRegistryString("showTexturesWindowRect", buff, "", sizeof(buff));
  numArgs =
      sscanf(buff, "%d %d %d %d", &g_showTextures_windowRect.left,
             &g_showTextures_windowRect.top, &g_showTextures_windowRect.right,
             &g_showTextures_windowRect.bottom);
  if (numArgs != 4 || g_showTextures_windowRect.left < 0 ||
...
void ShowTextures_Export() {
  ...
  for (i = 0; i < g_showTextures_numTextures; i++) {
    ...
    fprintf(fp, ",\"%s\"",
            g_showTextures_loadStrings[g_showTextures_pTextures[i].loadState]);
    fprintf(fp, "\n");
  }
...
void ShowTextures_Summary() {
  ...
  sprintf(buff,
          ...
          g_showTextures_numTextures, (float)total / (1024.0F * 1024.0F));

  MessageBox(g_showTextures_hWnd, buff, "Texture Summary",
             MB_OK | MB_APPLMODAL);
}
...
void ShowTextures_DrawTexture() {
  ...
  if (!V_stricmp(g_showTextures_drawTextureName, pTexture->pLongName)) {
    ...
    V_strncpy(g_showTextures_drawTextureName, pTexture->pLongName,
              sizeof(g_showTextures_drawTextureName));
  }
...
//
//-----------------------------------------------------------------------------
int CALLBACK ShowTextures_CompareFunc(LPARAM lParam1, LPARAM lParam2,
                                      LPARAM lParamSort) {
  ...
  case ID_ST_LOAD:
    sort = stricmp(g_showTextures_loadStrings[pTextureA->loadState],
                   g_showTextures_loadStrings[pTextureB->loadState]);
    break;
  }
...
void ShowTextures_SortItems() {
  ...
  for (i = 0;
       i < sizeof(g_showTextures_Labels) / sizeof(g_showTextures_Labels[0]);
       i++) {
    char symbol;
    LVCOLUMN lvc;
    ...
    sprintf(g_showTextures_Labels[i].nameBuff, "%s %c",
            g_showTextures_Labels[i].name, symbol);
...
//
//-----------------------------------------------------------------------------
LRESULT CALLBACK ShowTextures_WndProc(HWND hwnd, UINT message, WPARAM wParam,
                                      LPARAM lParam) {
  ...
  case WM_INITMENU:
    CheckMenuItem((HMENU)wParam, IDM_OPTIONS_CURRENTFRAME,
                  MF_BYCOMMAND | (g_showTextures_currentFrame ? MF_CHECKED
                                                              : MF_UNCHECKED));
    CheckMenuItem((HMENU)wParam, IDM_OPTIONS_FULLPATH,
                  MF_BYCOMMAND |
                      (g_showTextures_fullPath ? MF_CHECKED : MF_UNCHECKED));
    return 0L;
...
void ShowTextures_Open() {
  ...
  hWnd = CreateWindowEx(WS_EX_CLIENTEDGE, "SHOWTEXTURESCLASS", "",
  ...
  hWnd = CreateWindow(WC_LISTVIEW, "", WS_VISIBLE | WS_CHILD | LVS_REPORT, 0, 0,
                      ...
                      clientRect.bottom - clientRect.top, g_showTextures_hWnd,
                      (HMENU)ID_SHOWTEXTURES_LISTVIEW, g_hInstance, NULL);
  ...
  ListView_SetExtendedListViewStyleEx(g_showTextures_hWndListView, style,
                                      style);
  ...
  if (g_showTextures_windowRect.right && g_showTextures_windowRect.bottom) {
    MoveWindow(g_showTextures_hWnd, g_showTextures_windowRect.left,
               g_showTextures_windowRect.top,
               g_showTextures_windowRect.right - g_showTextures_windowRect.left,
               g_showTextures_windowRect.bottom - g_showTextures_windowRect.top,
               FALSE);
  }
...
int rc_TextureList(char *commandPtr) {
  ...
  DmGetMemory((void *)textureList, numTextures * sizeof(xrTexture_t),
              pLocalList, NULL);
  ...
  for (int i = 0; i < numTextures; i++) {
    ...
    Sys_StripPath(g_showTextures_pTextures[i].pLongName, shortName,
                  sizeof(shortName));
    ...
    if (pLocalList[i].edram ||
        V_stristr(g_showTextures_pTextures[i].pGroupName, "RenderTarget")) {
      loadState = LS_STATIC;
