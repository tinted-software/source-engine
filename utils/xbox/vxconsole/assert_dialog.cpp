...
//
//-----------------------------------------------------------------------------
int CALLBACK AssertDialogProc(HWND hDlg, UINT uMsg, WPARAM wParam,
                              LPARAM lParam) {
  switch (uMsg) {
  case WM_INITDIALOG: {
...
int rc_Assert(char *commandPtr) {
  ...
  // Flash the taskbar icon (otherwise users may not realise the app has stalled
  // on an Assert, esp. during loading)
  FLASHWINFO flashWInfo = {sizeof(FLASHWINFO), g_hDlgMain,
                           FLASHW_ALL | FLASHW_TIMERNOFG, 0, 1000};
  ...
  // (convert '\t' to '\n'; way simpler than tokenizing a general assert
  // expression)
  ...
  DialogBox(g_hInstance, MAKEINTRESOURCE(IDD_ASSERT_DIALOG), g_hDlgMain,
            (DLGPROC)AssertDialogProc);
  g_AssertDialogActive = false;

  // Write the (endian-converted) result directly back into the application's
  // memory:
