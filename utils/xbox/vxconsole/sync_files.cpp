...
void SyncFilesDlg_Setup(HWND hWnd) {
  ...
  CheckDlgButton(hWnd, IDC_SYNCFILES_FORCESYNC,
                 g_syncFiles_force ? BST_CHECKED : BST_UNCHECKED);
  CheckDlgButton(hWnd, IDC_SYNCFILES_NOWRITE,
                 g_syncFiles_noWrite ? BST_CHECKED : BST_UNCHECKED);
  CheckDlgButton(hWnd, IDC_SYNCFILES_VERBOSE,
                 g_syncFiles_verbose ? BST_CHECKED : BST_UNCHECKED);
}
...
//
//-----------------------------------------------------------------------------
BOOL CALLBACK SyncFilesDlg_Proc(HWND hwnd, UINT message, WPARAM wParam,
                                LPARAM lParam) {
  switch (message) {
  case WM_INITDIALOG:
...
bool SyncFiles_DoUpdates() {
  ...
  while (nodePtr) {
    ...
    if (GetFileAttributesEx(sourceFilename, GetFileExInfoStandard,
                            &localAttributes)) {
      sourceFileSize =
          (float)localAttributes.nFileSizeLow / (1024.0f * 1024.0f);
    }

    _snprintf(statusBuff1, sizeof(statusBuff1), "Local File: %s (%.2f MB)",
              sourceFilename, sourceFileSize);
    statusBuff1[sizeof(statusBuff1) - 1] = '\0';
    _snprintf(statusBuff2, sizeof(statusBuff2), "Target File: %s",
              nodePtr->filename);
    statusBuff1[sizeof(statusBuff2) - 1] = '\0';
    g_syncFiles_progress->SetStatus(NULL, statusBuff1, statusBuff2);

    retVal =
        FileSyncEx(sourceFilename, nodePtr->filename, fileSyncMode,
                   g_syncFiles_verbose != FALSE, g_syncFiles_noWrite != FALSE);
    if (retVal == 1) {
...
bool SyncFiles_GetTargetList(char *pTargetPath) {
  ...
  if (!GetTargetFileList_r(pTargetPath, true, FA_NORMAL | FA_READONLY, 0,
                           &g_syncfiles_targetFiles)) {
    ConsoleWindowPrintf(RGB(255, 0, 0), "Bad Target Path '%s'\n", pTargetPath);
    return false;
  }

  fileNode_t *pFileNode;
  for (pFileNode = g_syncfiles_targetFiles; pFileNode;
       pFileNode = pFileNode->nextPtr) {
    g_syncfiles_numTargetFiles++;
  }
...
void SyncFilesDlg_Open(void) {
  ...
  result = DialogBox(g_hInstance, MAKEINTRESOURCE(IDD_SYNCFILES), g_hDlgMain,
                     (DLGPROC)SyncFilesDlg_Proc);
...
void SyncFilesDlg_LoadConfig() {
  // get our config
  Sys_GetRegistryString("syncfiles_localPath", g_syncfiles_localPath,
                        g_localPath, sizeof(g_syncfiles_localPath));
  Sys_GetRegistryString("syncfiles_targetPath", g_syncfiles_targetPath,
                        g_targetPath, sizeof(g_syncfiles_targetPath));
...
//
//-----------------------------------------------------------------------------
void InstallDlg_InstallImage(const char *pInstallPath, bool bForce,
                             bool bCleanTarget) {
  int errCode;
  char *pToken;
  char arg1[MAXTOKEN];
  ...
  V_ComposeFileName(pInstallPath, "../dvd_install.txt", szScriptPath,
                    sizeof(szScriptPath));
  ...
  V_ComposeFileName(pInstallPath, "default.xex", szTempFilename,
                    sizeof(szTempFilename));
  if (!Sys_Exists(szTempFilename)) {
    ConsoleWindowPrintf(XBX_CLR_RED,
                        "Cancelled Installation! DVD Image Auto-Build Process "
                        "Failed - Missing default.xex!\n");
    return;
  }
  ...
  V_ComposeFileName(pInstallPath, "version.txt", szVersionPath,
                    sizeof(szVersionPath));
  if (Sys_LoadFile(szVersionPath, (void **)&pVersionDetailString) <= 0) {
    ConsoleWindowPrintf(XBX_CLR_RED,
                        "Cancelled Installation! DVD Image Auto-Build Process "
                        "Failed! - Missing version.txt\n");
    return;
  }
    ...
    if (!stricmp(pToken, "$version")) {
      ...
      if (bCleanTarget) {
        ...
        if (hr == XBDM_NOERR) {
          ...
          V_ComposeFileName(szRootPath, "*.*", szTempFilename,
                            sizeof(szTempFilename));
          char *pArgs[3];
          pArgs[0] = "*del";
          ...
          if (!lc_del(3, pArgs)) {
            ConsoleWindowPrintf(
                XBX_CLR_RED, "Failed To Delete Files At '%s'.\n", szRootPath);
            break;
          }
        }
      }
      ...
      V_ComposeFileName(szRootPath, "version.txt", szTargetVersionPath,
                        sizeof(szTargetVersionPath));

      if (!bForce) {
        int fileSize;
        char *pTargetVersionDetailString;
        if (!LoadTargetFile(szTargetVersionPath, &fileSize,
                            (void **)&pTargetVersionDetailString)) {
          // expected version does not exist
          // force full install
          bForce = true;
        } else if (strcmp(pVersionDetailString, pTargetVersionDetailString) !=
                   0) {
          // different versions
          bForce = true;
        }
      ...
      if (bForce && !FileSyncEx(szVersionPath, szTargetVersionPath,
                                FSYNC_ALWAYS, true, false)) {
        numFailed++;
        break;
      }
      ...
      if (h != INVALID_HANDLE_VALUE) {
        do {
          ...
          char statusBuff1[MAX_PATH];
          V_snprintf(statusBuff1, sizeof(statusBuff1),
                     "Copying (%.2f MB) ... Please Wait", sourceFileSize);
          pProgress->SetStatus(statusBuff1, sourceFilename, targetFilename);

          int fileSyncMode = bForce ? FSYNC_ALWAYS : FSYNC_IFNEWER;
          errCode = FileSyncEx(sourceFilename, targetFilename, fileSyncMode,
                               true, false);
          if (errCode < 0) {
...
bool InstallDlg_GetChanges(HWND hWnd) {
  ...
  g_install_bCleanTarget =
      IsDlgButtonChecked(hWnd, IDC_INSTALL_CLEANTARGET) != 0;

  g_install_Selection = -1;
  for (int i = 0; i < g_install_dvdImages.Count(); i++) {
    int state = ListView_GetItemState(GetDlgItem(hWnd, IDC_INSTALL_LIST), i,
...
void InstallDlg_Populate(HWND hWnd) {
  ...
  if (h != INVALID_HANDLE_VALUE) {
    do {
      ...
      V_ComposeFileName(szInstallPath, "default.xex", szBooterPath,
                        sizeof(szBooterPath));
      if (!Sys_Exists(szBooterPath)) {
        continue;
      }
      char szVersionPath[MAX_PATH];
      V_ComposeFileName(szInstallPath, "version.txt", szVersionPath,
                        sizeof(szVersionPath));
      char *pVersionDetailString = NULL;
      ...
      int minute = 0;
      char timeOfDay[256];
      sscanf(findData.cFileName, "DVD_%d_%d_%d_%d_%d_%s", &year, &month, &day,
      ...
      V_snprintf(image.szString, sizeof(image.szString),
                 "%2.2d/%2.2d/%4.4d %2.2d:%2.2d%s", month, day, year, hour,
                 minute, szCommentBuff);
...
//
//-----------------------------------------------------------------------------
BOOL CALLBACK InstallDlg_Proc(HWND hWnd, UINT message, WPARAM wParam,
                              LPARAM lParam) {
  switch (message) {
  case WM_INITDIALOG:
  ...
  case WM_NOTIFY:
    ...
    case LVN_GETDISPINFO:
      ...
      case 1:
        plvdi->item.pszText =
            (LPSTR)g_install_dvdImages[item].versionDetailString.String();
        return TRUE;
      }
    }
    break;
...
void InstallDlg_Open(void) {
  int result = DialogBox(g_hInstance, MAKEINTRESOURCE(IDD_INSTALL), g_hDlgMain,
                         (DLGPROC)InstallDlg_Proc);
  if (LOWORD(result) == IDC_OK && g_install_Selection != -1) {
    InstallDlg_InstallImage(
        g_install_dvdImages[g_install_Selection].installPath.String(),
        g_install_bForceSync, g_install_bCleanTarget);
  }
