...
label_t g_showSounds_Labels[] = {
    {"Name", 300, ID_SS_NAME},    {"Prefix", 80, ID_SS_PREFIX},
    {"Format", 80, ID_SS_FORMAT}, {"Rate", 80, ID_SS_RATE},
    {"Bits", 80, ID_SS_BITS},     {"Channels", 80, ID_SS_CHANNELS},
    {"Size", 80, ID_SS_SIZE},     {"Streamed", 80, ID_SS_STREAMED},
    {"Looped", 80, ID_SS_LOOPED}, {"Length", 80, ID_SS_LENGTH},
};
...
void ShowSounds_SaveConfig() {
  ...
  Sys_SetRegistryInteger("showSoundsSortDescending",
                         g_showSounds_sortDescending);
  ...
  sprintf(buff, "%d %d %d %d", g_showSounds_windowRect.left,
          g_showSounds_windowRect.top, g_showSounds_windowRect.right,
          g_showSounds_windowRect.bottom);
...
void ShowSounds_LoadConfig() {
  ...
  Sys_GetRegistryInteger("showSoundsSortColumn", ID_SS_NAME,
                         g_showSounds_sortColumn);
  Sys_GetRegistryInteger("showSoundsSortDescending", false,
                         g_showSounds_sortDescending);

  Sys_GetRegistryString("showSoundsWindowRect", buff, "", sizeof(buff));
  numArgs = sscanf(buff, "%d %d %d %d", &g_showSounds_windowRect.left,
                   &g_showSounds_windowRect.top, &g_showSounds_windowRect.right,
                   &g_showSounds_windowRect.bottom);
  if (numArgs != 4 || g_showSounds_windowRect.left < 0 ||
      g_showSounds_windowRect.top < 0 || g_showSounds_windowRect.right < 0 ||
...
void ShowSounds_Summary() {
  ...
  sprintf(buff,
          ...
          g_showSounds_numSounds, (float)totalStatic / (1024.0F * 1024.0F),
...
//
//-----------------------------------------------------------------------------
int CALLBACK ShowSounds_CompareFunc(LPARAM lParam1, LPARAM lParam2,
                                    LPARAM lParamSort) {
...
void ShowSounds_SortItems() {
  ...
  for (i = 0; i < sizeof(g_showSounds_Labels) / sizeof(g_showSounds_Labels[0]);
       i++) {
    char symbol;
    LVCOLUMN lvc;
    ...
    if (i == g_showSounds_sortColumn)
      ...
      symbol = ' ';
    sprintf(g_showSounds_Labels[i].nameBuff, "%s %c",
            g_showSounds_Labels[i].name, symbol);
...
void ShowSounds_AddViewItem(sound_t *pSound) {
  ...
  sprintf(pSound->lengthBuff, "%2.2d:%2.2d:%3.3d", (int)pSound->length / 60,
          (int)pSound->length % 60,
          (int)(1000 * (pSound->length - (int)pSound->length)) % 1000);
...
//
//-----------------------------------------------------------------------------
LRESULT CALLBACK ShowSounds_WndProc(HWND hwnd, UINT message, WPARAM wParam,
                                    LPARAM lParam) {
...
void ShowSounds_Open() {
  ...
  hWnd = CreateWindowEx(WS_EX_CLIENTEDGE, "SHOWSOUNDSCLASS", "Sounds",
  ...
  hWnd = CreateWindow(
      WC_LISTVIEW, "",
      WS_VISIBLE | WS_CHILD | LVS_REPORT | LVS_SHOWSELALWAYS | LVS_SINGLESEL, 0,
      0, clientRect.right - clientRect.left, clientRect.bottom - clientRect.top,
      g_showSounds_hWnd, (HMENU)ID_SHOWSOUNDS_LISTVIEW, g_hInstance, NULL);
  ...
  if (g_showSounds_windowRect.right && g_showSounds_windowRect.bottom)
    MoveWindow(g_showSounds_hWnd, g_showSounds_windowRect.left,
               g_showSounds_windowRect.top,
               g_showSounds_windowRect.right - g_showSounds_windowRect.left,
               g_showSounds_windowRect.bottom - g_showSounds_windowRect.top,
               FALSE);
...
int rc_SoundList(char *commandPtr) {
  ...
  DmGetMemory((void *)soundList, numSounds * sizeof(xrSound_t), pLocalList,
              NULL);
  ...
  for (int i = 0; i < numSounds; i++) {
    ...
    if (g_showSounds_pSounds[i].rate > 0)
      g_showSounds_pSounds[i].length =
          (float)g_showSounds_pSounds[i].numSamples /
          (float)g_showSounds_pSounds[i].rate;
    else
