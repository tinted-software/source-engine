...
void MemProfile_LoadConfig() {
  ...
  numArgs = sscanf(buff, "%d %d %d %d", &g_memProfile_WindowRect.left,
                   &g_memProfile_WindowRect.top, &g_memProfile_WindowRect.right,
                   &g_memProfile_WindowRect.bottom);
  if (numArgs != 4) {
  ...
  if (g_memProfile_scale < -PROFILE_SCALESTEPS ||
      g_memProfile_scale > PROFILE_SCALESTEPS) {
...
int rc_FreeMemory(char *commandPtr) {
  ...
  g_memProfile_samples[g_memProfile_numSamples % PROFILE_MAXSAMPLES] =
      freeMemory;
...
void MemProfile_Draw(HDC hdc, RECT *clientRect) {
  ...
  // zoom
  scale = MemProfile_CalcScale(g_memProfile_scale, PROFILE_SCALESTEPS,
                               PROFILE_MINSCALE, PROFILE_MAXSCALE);
  ...
  for (i = 0; i < numTicks; i++) {
    ...
    if (g_memProfile_tickMarks) {
      // could be very zoomed out, gap must be enough for label, otherwise don't
      // draw
  ...
  if (g_memProfile_numSamples) {
    ...
    for (x = numbars - 1; x >= 0; x -= 4) {
      float sample = g_memProfile_samples[currentSample % PROFILE_MAXSAMPLES] /
                     (1024.0f * 1024.0f);
      ...
      if (h) {
        ...
        hColoredBrush =
            CreateSolidBrush(g_memProfile_colors ? barColor : RGB(80, 80, 80));
...
//
//-----------------------------------------------------------------------------
LRESULT CALLBACK MemProfile_WndProc(HWND hwnd, UINT message, WPARAM wParam,
                                    LPARAM lParam) {
  ...
  case WM_INITMENU:
    CheckMenuItem((HMENU)wParam, IDM_MEMPROFILE_TICKMARKS,
                  MF_BYCOMMAND |
                      (g_memProfile_tickMarks ? MF_CHECKED : MF_UNCHECKED));
    CheckMenuItem((HMENU)wParam, IDM_MEMPROFILE_COLORS,
                  MF_BYCOMMAND |
                      (g_memProfile_colors ? MF_CHECKED : MF_UNCHECKED));
    CheckMenuItem((HMENU)wParam, IDM_MEMPROFILE_ENABLE,
                  MF_BYCOMMAND |
                      (g_memProfile_Timer != NULL ? MF_CHECKED : MF_UNCHECKED));
    return 0L;
...
void MemProfile_Open() {
  ...
  hWnd = CreateWindowEx(WS_EX_CLIENTEDGE, "MEMPROFILECLASS", "",
  ...
  if (g_memProfile_WindowRect.right && g_memProfile_WindowRect.bottom)
    MoveWindow(g_memProfile_hWnd, g_memProfile_WindowRect.left,
               g_memProfile_WindowRect.top,
               g_memProfile_WindowRect.right - g_memProfile_WindowRect.left,
               g_memProfile_WindowRect.bottom - g_memProfile_WindowRect.top,
               FALSE);
