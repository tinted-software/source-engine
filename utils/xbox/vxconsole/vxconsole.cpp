...
LRESULT CALLBACK CommandWindow_SubclassedProc(HWND hDlg, UINT msg,
                                              WPARAM wParam, LPARAM lParam);

bool ParseCommandLineArg(const char *pCmdLine, const char *pKey,
                         char *pValueBuff, int valueBuffSize) {
...
void MakeConfigString(const char *pString, int configID, char *pOutBuff,
                      int outBuffSize) {
  if (configID <= 0) {
    // as-is, undecorated
...
void LoadConfig() {
  ...
  numArgs =
      sscanf(buff, "%d %d %d %d", &g_mainWindowRect.left, &g_mainWindowRect.top,
             &g_mainWindowRect.right, &g_mainWindowRect.bottom);
  if (numArgs != 4 || g_mainWindowRect.left < 0 || g_mainWindowRect.top < 0 ||
      g_mainWindowRect.right < 0 || g_mainWindowRect.bottom < 0)
...
void SaveConfig() {
  ...
  sprintf(buff, "%d %d %d %d", g_mainWindowRect.left, g_mainWindowRect.top,
          g_mainWindowRect.right, g_mainWindowRect.bottom);
...
HRESULT DmAPI_SendCommand(const char *strCommand, bool wait) {
  ...
  HRESULT hr =
      DmSendCommand(g_pdmConnection, strCommand, strResponse, &dwResponseLen);
  if (!FAILED(hr)) {
    // success
    switch (hr) {
    case XBDM_NOERR:
    ...
    case XBDM_BINRESPONSE:
      ConsoleWindowPrintf(XBX_CLR_DEFAULT,
                          "Binary response - not implemented\n");
      break;

    case XBDM_READYFORBIN:
      ConsoleWindowPrintf(XBX_CLR_DEFAULT,
                          "Ready for binary - not implemented\n");
      break;
...
void PrintToQueue(COLORREF rgb, LPCTSTR strFormat, ...) {
  ...
  // when the queue is full, the main thread is probably blocked and not
  // dequeing
...
void ProcessPrintQueue() {
  ...
  for (int i = 0; i < g_PrintQueue.numMessages; i++) {
    ConsoleWindowPrintf(g_PrintQueue.aColors[i], "%s",
                        g_PrintQueue.pMessages[i]);
...
int ConsoleWindowPrintf(COLORREF rgb, LPCTSTR strFormat, ...) {
  ...
  if (rgb != XBX_CLR_DEFAULT) {
    ...
    cf.dwEffects = 0;
    cf.crTextColor = rgb;
    SendDlgItemMessage(g_hDlgMain, IDC_OUTPUT, EM_SETCHARFORMAT, SCF_SELECTION,
                       (LPARAM)&cf);
  }
  ...
  SendDlgItemMessage(g_hDlgMain, IDC_OUTPUT, EM_REPLACESEL, 0,
                     (LONG)(LPSTR)strTemp);
  ...
  if (g_bPlayTestMode) {
    ...
    V_snprintf(szLogName, sizeof(szLogName), "vxconsole_%s.log",
               g_xboxTargetName);
  ...
  for (int i = 0; i < g_numLocalCommands; i++) {
    ...
    if ((g_localCommands[i].flags & FN_XBOX) && !g_connectedToXBox) {
      ...
      ConsoleWindowPrintf(RGB(255, 0, 0),
                          "'%s' is not available until connected to XBox.\n",
                          argv[0]);
      return true;
      ...
      ConsoleWindowPrintf(
          RGB(255, 0, 0),
          "'%s' is not available until connected to Application.\n", argv[0]);
      return true;
    }
...
BOOL CommandWindow_HandleKey(WPARAM wParam) {
  ...
  case VK_DOWN:
    if (IsWindowVisible(g_hwndCommandHint)) {
      ...
      SendMessage(g_hwndCommandHint, (UINT)LB_GETTEXT, (WPARAM)curSel,
                  (LPARAM)hintCmd);
      ...
      SendMessage(g_hwndCommandCombo, (UINT)CB_SETEDITSEL, (WPARAM)0,
                  MAKELONG(MAX_PATH, MAX_PATH));
      ...
      if (curSel < 0) {
        ...
        numItems =
            SendMessage(g_hwndCommandCombo, (UINT)CB_GETCOUNT, NULL, NULL);
        if (numItems > 0) {
          if (wParam == VK_UP) {
          ...
          SendMessage(g_hwndCommandCombo, (UINT)CB_SETCURSEL, (WPARAM)curSel,
                      NULL);
          bHandled = TRUE;
        }
      }
...
// Main_SizeWindow
//
// Handles a WM_SIZE message by resizing all our child windows to match the main
// window
//-----------------------------------------------------------------------------
void Main_SizeWindow(HWND hDlg, UINT wParam, int cx, int cy) {
  ...
  if (g_hwndCommandCombo && cx > 64 && cy > 64) {
    ...
    int y = cy - 8 - dy;
    SetWindowPos(g_hwndCommandCombo, NULL, x, y, dx, dy, SWP_NOZORDER);
    ...
    SetWindowPos(g_hwndCommandHint, NULL, rcCmd.left,
                 (rcCmd.top - 4) - (rcHint.bottom - rcHint.top + 1), 0, 0,
    ...
    SetWindowPos(hStaticCmd, NULL, 8,
                 y + (dy - (rcStaticCmd.bottom - rcStaticCmd.top)) / 2 - 1, 0,
    ...
    SetWindowPos(g_hwndOutputWindow, NULL, 0, 0, dwWidth, dwHeight,
...
void EnableCommandHint(bool enable) {
  ...
  numRemoteCommands = MatchRemoteCommands(strCmd, cmds + numLocalCommands,
                                          256 - numLocalCommands);
  ...
  qsort(cmds, numLocalCommands + numRemoteCommands, sizeof(const char *),
        _SortCommands);
  ...
  // position the "hint popup" window above the "command" window
  SetWindowPos(g_hwndCommandHint, NULL, rcCmd.left, (rcCmd.top - 4) - h, w, h,
               SWP_NOZORDER);
...
//
//-----------------------------------------------------------------------------
LRESULT CALLBACK Main_DlgProc(HWND hDlg, UINT message, WPARAM wParam,
                              LPARAM lParam) {
  ...
  case WM_TIMER:
    ...
    // (it uses a synchronous command, so calling 'Dm' funcs here can cause a
    // lockup)
  ...
  case WM_INITMENU:
    ...
    CheckMenuItem((HMENU)wParam, IDM_AUTOCONNECT,
                  MF_BYCOMMAND | (g_autoConnect ? MF_CHECKED : MF_UNCHECKED));
    CheckMenuItem((HMENU)wParam, IDM_CAPTUREGAMESPEW,
                  MF_BYCOMMAND |
                      (g_captureGameSpew ? MF_CHECKED : MF_UNCHECKED));
    CheckMenuItem((HMENU)wParam, IDM_CAPTUREDEBUGSPEW,
                  MF_BYCOMMAND |
                      (g_captureDebugSpew ? MF_CHECKED : MF_UNCHECKED));
    CheckMenuItem((HMENU)wParam, IDM_DEBUGCOMMANDS,
                  MF_BYCOMMAND | (g_debugCommands ? MF_CHECKED : MF_UNCHECKED));
    CheckMenuItem((HMENU)wParam, IDM_PLAYTESTMODE,
                  MF_BYCOMMAND | (g_bPlayTestMode ? MF_CHECKED : MF_UNCHECKED));
    EnableMenuItem((HMENU)wParam, IDM_SYNCFILES,
                   MF_BYCOMMAND | (g_connectedToXBox ? MF_ENABLED : MF_GRAYED));
    EnableMenuItem((HMENU)wParam, IDM_SYNCINSTALL,
                   MF_BYCOMMAND | (g_connectedToXBox ? MF_ENABLED : MF_GRAYED));
    return 0L;
...
void CreateCommandHint() {
  ...
  g_hwndCommandHint = CreateWindowEx(
      WS_EX_NOPARENTNOTIFY, "LISTBOX", "",
      WS_BORDER | WS_CHILD | LBS_HASSTRINGS | WS_VSCROLL, 0, 0, 100, 0,
      g_hDlgMain, (HMENU)IDC_COMMANDHINT, g_hInstance, NULL);
  ...
  // set font
  SendDlgItemMessage(g_hDlgMain, IDC_COMMANDHINT, WM_SETFONT,
                     (WPARAM)g_hFixedFont, TRUE);
}
...
bool CreateResources() {
  ...
  // get icons
  g_hIcons[ICON_APPLICATION] =
      LoadIcon(g_hInstance, MAKEINTRESOURCE(IDI_VXCONSOLE));
  g_hIcons[ICON_DISCONNECTED] =
      LoadIcon(g_hInstance, MAKEINTRESOURCE(IDI_DISCONNECTED));
  g_hIcons[ICON_CONNECTED_XBOX] =
      LoadIcon(g_hInstance, MAKEINTRESOURCE(IDI_CONNECT1_ON));
  g_hIcons[ICON_CONNECTED_APP0] =
      LoadIcon(g_hInstance, MAKEINTRESOURCE(IDI_CONNECT2_OFF));
  g_hIcons[ICON_CONNECTED_APP1] =
      LoadIcon(g_hInstance, MAKEINTRESOURCE(IDI_CONNECT2_ON));
...
//-----------------------------------------------------------------------------
bool Startup() {
  // Load the xenon debugging dll (xbdm.dll) manually due to its absence from
  // system path
  ...
  g_hDlgMain =
      CreateDialog(g_hInstance, MAKEINTRESOURCE(IDD_VXCONSOLE), 0, NULL);
  if (!g_hDlgMain)
    return false;
  ...
  g_hwndCommandSubclassed = SubclassWindow(
      GetWindow(g_hwndCommandCombo, GW_CHILD), CommandWindow_SubclassedProc);
  ...
  SendDlgItemMessage(g_hDlgMain, IDC_COMMAND, WM_SETFONT, (WPARAM)g_hFixedFont,
                     TRUE);
  ...
  ConsoleWindowPrintf(XBX_CLR_DEFAULT,
                      "VXConsole %s [%s Build: %s %s] [Protocol: %d]\n",
                      VXCONSOLE_VERSION, VXCONSOLE_BUILDTYPE, __DATE__,
                      __TIME__, VXCONSOLE_PROTOCOL_VERSION);
  ConsoleWindowPrintf(XBX_CLR_DEFAULT,
  ...
  if (g_mainWindowRect.right && g_mainWindowRect.bottom)
    MoveWindow(g_hDlgMain, g_mainWindowRect.left, g_mainWindowRect.top,
               g_mainWindowRect.right - g_mainWindowRect.left,
               g_mainWindowRect.bottom - g_mainWindowRect.top, FALSE);
