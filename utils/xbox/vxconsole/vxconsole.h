...
#define VXCONSOLE_REGISTRY                                                     \
  "HKEY_CURRENT_USER\\Software\\VXConsole\\" VXCONSOLE_MAGIC
#ifdef _DEBUG
...
// FILEIO.CPP
//-----------------------------------------------------------------------------
extern void RemoteToLocalFilename(const char *inFilename, char *outFilename,
                                  int outSize);
extern void RemoteToTargetFilename(const char *inFilename, char *outFilename,
                                   int outSize);
extern void FreeTargetFileList(fileNode_t *pFileList);
extern bool GetTargetFileList_r(char *targetPath, bool recurse, int attributes,
                                int level, fileNode_t **pFileList);
extern char *SystemTimeToString(SYSTEMTIME *systemTime, char *buffer,
                                int bufferSize);
...
extern DWORD __stdcall Remote_NotifyDebugString(ULONG dwNotification,
                                                DWORD dwParam);
...
extern int MatchRemoteCommands(char *cmdStr, const char *cmdList[],
                               int maxCmds);
...
extern void FileLog_UpdateItem(unsigned int log, unsigned int fp,
                               fileLogFunctions_e functionId, int value);
...
// SYNC_FILES.CPP
//-----------------------------------------------------------------------------
extern int FileSyncEx(const char *localFilename, const char *remoteFilename,
...
// FILEIO.CPP
//-----------------------------------------------------------------------------
extern int CompareFileTimes_NTFStoFATX(FILETIME *ntfsFileTime,
                                       char *ntfsTimeString, int ntfsStringSize,
                                       FILETIME *fatxFileTime,
                                       char *fatxTimeString,
                                       int fatxStringSize);
