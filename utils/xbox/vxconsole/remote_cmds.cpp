...
DWORD __stdcall Remote_NotifyPrintFunc(const CHAR *pStrNotification) {
  ...
  if (!strnicmp(pStrNotification, VXCONSOLE_PRINT_PREFIX,
                strlen(VXCONSOLE_PRINT_PREFIX))) {
    // skip past prefix!
  ...
  if (!strnicmp(pStrNotification, VXCONSOLE_COLOR_PREFIX,
                strlen(VXCONSOLE_COLOR_PREFIX))) {
...
bool Remote_AddCommand(char *command, char *helptext) {
  ...
  g_remoteCommands[g_numRemoteCommands]->strCommand =
      new char[strlen(command) + 1];
  strcpy(g_remoteCommands[g_numRemoteCommands]->strCommand, command);

  g_remoteCommands[g_numRemoteCommands]->strHelp =
      new char[strlen(helptext) + 1];
...
int rc_AddCommands(char *commandPtr) {
  ...
  DmGetMemory((void *)cmdList, numCommands * sizeof(xrCommand_t), locallist,
              NULL);
  ...
  // sort the list
  qsort(g_remoteCommands, g_numRemoteCommands, sizeof(remoteCommand_t *),
        Remote_CompareCommands);
...
DWORD __stdcall Remote_NotifyCommandFunc(const CHAR *strNotification) {
  ...
  if (cmdToken && !stricmp(cmdToken, "AddCommands()")) {
    ...
    // send command status first while connection valid, then do actual
    // disconnect disconnect is always assumed to be valid, can't be denied
    CommandCompleted(0);
    DoDisconnect(TRUE);
    return S_OK;
  } else {
