...
void Sys_SetRegistryPrefix(const CHAR *pPrefix) {
  _snprintf_s(g_szRegistryPrefix, sizeof(g_szRegistryPrefix), _TRUNCATE,
              pPrefix);
}
...
//
//-----------------------------------------------------------------------------
static BOOL Sys_SplitRegistryKey(const CHAR *key, CHAR *key0, int key0Len,
...
BOOL Sys_SetRegistryString(const CHAR *keyName, const CHAR *value) {
  ...
  if (RegCreateKeyEx(hSlot, key0, NULL, NULL, REG_OPTION_NON_VOLATILE,
                     value ? KEY_WRITE : KEY_ALL_ACCESS, NULL, &hKey,
                     NULL) != ERROR_SUCCESS)
    return false;

  if (RegSetValueEx(hKey, key1, NULL, REG_SZ, (UCHAR *)value,
                    (int)strlen(value) + 1) != ERROR_SUCCESS) {
    RegCloseKey(hKey);
    return false;
  }
...
//
//-----------------------------------------------------------------------------
BOOL Sys_GetRegistryString(const CHAR *keyName, CHAR *value,
                           const CHAR *defValue, int valueLen) {
  HKEY hKey;
  CHAR key0[256];
  CHAR key1[256];
  CHAR keyBuff[256];
  ...
  if (RegQueryValueEx(hKey, key1, NULL, NULL, (UCHAR *)value, &len) !=
      ERROR_SUCCESS) {
    RegCloseKey(hKey);
    return false;
  }
...
BOOL Sys_SetRegistryInteger(const CHAR *keyName, int value) {
  ...
  if (RegSetValueEx(hKey, key1, NULL, REG_DWORD, (UCHAR *)&value, 4) !=
      ERROR_SUCCESS) {
    RegCloseKey(hKey);
    return false;
  }
...
bool Sys_SaveFile(const CHAR *filename, void *buffer, long count, bool bText) {
  ...
  int openFlags = bText ? _O_TEXT : _O_BINARY;
  _sopen_s(&handle, filename, _O_RDWR | _O_CREAT | _O_TRUNC | openFlags,
...
//-----------------------------------------------------------------------------
bool Sys_IsWildcardMatch(const CHAR *wildcardString, const CHAR *stringToCheck,
                         bool caseSensitive) {
  CHAR wcChar;
  CHAR strChar;
