...
label_t g_timeStampLog_Labels[] = {
    {"Time", 100, ID_TSL_TIME},       {"Delta Time", 100, ID_TSL_DELTATIME},
    {"Memory", 100, ID_TSL_MEMORY},   {"Delta Memory", 100, ID_TSL_DELTAMEMORY},
...
//
//-----------------------------------------------------------------------------
int CALLBACK TimeStampLog_CompareFunc(LPARAM lParam1, LPARAM lParam2,
                                      LPARAM lParamSort) {
...
void TimeStampLog_SortItems() {
  ...
  for (i = 0;
       i < sizeof(g_timeStampLog_Labels) / sizeof(g_timeStampLog_Labels[0]);
       i++) {
    char symbol;
    LVCOLUMN lvc;
    ...
    if (i == g_timeStampLog_sortColumn)
      ...
      symbol = ' ';
    sprintf(g_timeStampLog_Labels[i].nameBuff, "%s %c",
            g_timeStampLog_Labels[i].name, symbol);
...
void TimeStampLog_AddViewItem(timeStampLogNode_t *pNode) {
  ...
  sprintf(pNode->timeBuff, "%2.2d:%2.2d:%2.2d:%3.3d", (int)pNode->time / 3600,
          ((int)pNode->time / 60) % 60, (int)pNode->time % 60,
          (int)(1000 * (pNode->time - (int)pNode->time)) % 1000);
...
//
//-----------------------------------------------------------------------------
void TimeStampLog_AddItem(float time, float deltaTime, int memory,
                          int deltaMemory, const char *pMessage) {
  timeStampLogNode_t *pNode;
...
void TimeStampLog_SaveConfig() {
  ...
  Sys_SetRegistryInteger("timeStampLogSortDescending",
                         g_timeStampLog_sortDescending);
  ...
  sprintf(buff, "%d %d %d %d", g_timeStampLog_windowRect.left,
          g_timeStampLog_windowRect.top, g_timeStampLog_windowRect.right,
          g_timeStampLog_windowRect.bottom);
...
void TimeStampLog_LoadConfig() {
  ...
  Sys_GetRegistryInteger("timeStampLogSortColumn", ID_TSL_TIME,
                         g_timeStampLog_sortColumn);
  Sys_GetRegistryInteger("timeStampLogSortDescending", false,
                         g_timeStampLog_sortDescending);

  Sys_GetRegistryString("timeStampLogWindowRect", buff, "", sizeof(buff));
  numArgs =
      sscanf(buff, "%d %d %d %d", &g_timeStampLog_windowRect.left,
             &g_timeStampLog_windowRect.top, &g_timeStampLog_windowRect.right,
             &g_timeStampLog_windowRect.bottom);
  if (numArgs != 4 || g_timeStampLog_windowRect.left < 0 ||
...
void TimeStampLog_SizeWindow(HWND hwnd, int width, int height) {
  ...
  SetWindowPos(g_timeStampLog_hWndListView, NULL, 0, 0, width, height,
               SWP_NOZORDER);
}
...
//
//-----------------------------------------------------------------------------
LRESULT CALLBACK TimeStampLog_WndProc(HWND hwnd, UINT message, WPARAM wParam,
                                      LPARAM lParam) {
...
void TimeStampLog_Open() {
  ...
  hWnd = CreateWindowEx(WS_EX_CLIENTEDGE, "TIMESTAMPLOGCLASS", "TimeStamp Log",
                        WS_POPUP | WS_CAPTION | WS_SYSMENU | WS_SIZEBOX |
                            WS_MINIMIZEBOX | WS_MAXIMIZEBOX,
                        0, 0, 600, 300, g_hDlgMain, NULL, g_hInstance, NULL);
  ...
  hWnd = CreateWindow(WC_LISTVIEW, "", WS_VISIBLE | WS_CHILD | LVS_REPORT, 0, 0,
                      ...
                      clientRect.bottom - clientRect.top, g_timeStampLog_hWnd,
                      (HMENU)ID_TIMESTAMPLOG_LISTVIEW, g_hInstance, NULL);
  ...
  ListView_SetExtendedListViewStyleEx(g_timeStampLog_hWndListView, style,
                                      style);
  ...
  if (g_timeStampLog_windowRect.right && g_timeStampLog_windowRect.bottom)
    MoveWindow(g_timeStampLog_hWnd, g_timeStampLog_windowRect.left,
               g_timeStampLog_windowRect.top,
               g_timeStampLog_windowRect.right - g_timeStampLog_windowRect.left,
               g_timeStampLog_windowRect.bottom - g_timeStampLog_windowRect.top,
               FALSE);
...
int rc_TimeStampLog(char *commandPtr) {
  ...
  // add to log
  TimeStampLog_AddItem(timeStamp.time, timeStamp.deltaTime, timeStamp.memory,
                       timeStamp.deltaMemory, timeStamp.messageString);
  TimeStampLog_SortItems();
