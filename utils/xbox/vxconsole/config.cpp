...
void ConfigDlg_LoadConfig() {
  // get our config
  Sys_GetRegistryString("xboxName", g_xboxTargetName, "",
                        sizeof(g_xboxTargetName));
  Sys_GetRegistryString("localPath", g_localPath, "u:\\dev\\game",
                        sizeof(g_localPath));
  Sys_GetRegistryString("targetPath", g_targetPath, "e:\\valve",
                        sizeof(g_targetPath));
  Sys_GetRegistryString("installPath", g_installPath,
                        "\\\\fileserver\\user\\xbox\\xbox_orange",
                        sizeof(g_installPath));
  ...
  Sys_GetRegistryInteger("captureDebugSpew", true,
                         g_captureDebugSpew_StartupState);
}
...
void ConfigDlg_Setup(HWND hWnd) {
  ...
  CheckDlgButton(hWnd, IDC_CONFIG_CLEARONCONNECT,
                 g_clsOnConnect ? BST_CHECKED : BST_UNCHECKED);
  CheckDlgButton(hWnd, IDC_CONFIG_ALWAYSAUTOCONNECT,
                 g_alwaysAutoConnect ? BST_CHECKED : BST_UNCHECKED);
  CheckDlgButton(hWnd, IDC_CONFIG_STARTMINIMIZED,
                 g_startMinimized ? BST_CHECKED : BST_UNCHECKED);
  CheckDlgButton(hWnd, IDC_CONFIG_CAPTUREDEBUGSPEW,
                 g_captureDebugSpew_StartupState ? BST_CHECKED : BST_UNCHECKED);
}
...
bool ConfigDlg_GetChanges(HWND hwnd) {
  ...
  g_captureDebugSpew_StartupState =
      IsDlgButtonChecked(hwnd, IDC_CONFIG_CAPTUREDEBUGSPEW);
...
//
//-----------------------------------------------------------------------------
BOOL CALLBACK ConfigDlg_Proc(HWND hwnd, UINT message, WPARAM wParam,
                             LPARAM lParam) {
  switch (message) {
  case WM_INITDIALOG:
...
void ConfigDlg_Open(void) {
  ...
  result = DialogBox(g_hInstance, MAKEINTRESOURCE(IDD_CONFIG), g_hDlgMain,
                     (DLGPROC)ConfigDlg_Proc);
