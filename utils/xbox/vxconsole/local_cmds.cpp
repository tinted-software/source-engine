...
localCommand_t g_localCommands[] = {
    // Command name,        Flags		Handler
    // Help string console commands
    {"*cls", FN_CONSOLE, lc_cls, ": Clear the screen"},
    {"*connect", FN_CONSOLE, lc_autoConnect,
     ": Connect and listen until successful"},
    {"*disconnect", FN_CONSOLE, lc_disconnect,
     ": Terminate Debug Console session"},
    ...
    {"*screenshot", FN_XBOX, lc_screenshot,
     "<file.bmp> : Copy the screen to file.bmp"},
    ...
    {"*sections", FN_XBOX, lc_sections,
     "<module> : Lists the sections in the module"},
    {"*bug", FN_CONSOLE, lc_bug, ": Open bug submission form"},
    {"*clearconfigs", FN_XBOX, lc_ClearConfigs, ": Erase all game configs"},

    // xcommands
    {"*break", FN_XBOX, NULL,
     " addr=<address> | 'Write'/'Read'/'Execute'=<address> size=<DataSize> "
     "['clear']: Sets/Clears a breakpoint"},
    //  { "*bye",				FN_XBOX,	NULL,
    //  ": Closes connection" },
    {"*continue", FN_XBOX, NULL,
     " thread=<threadid>: Resumes execution of a thread which has been "
     "stopped"},
    //  { "*delete",			FN_XBOX,	NULL,
    //  " name=<remotefile>: Deletes a file on the Xbox" },
    //  { "*dirlist",			FN_XBOX,	NULL,
    //  " name=<remotedir>: Lists the items in the directory" },
    {"*getcontext", FN_XBOX, NULL,
     " thread=<threadid> 'Control' | 'Int' | 'FP' | 'Full':  Gets the context "
     "of the thread"},
    {"*getfileattributes", FN_XBOX, NULL,
     " name=<remotefile>: Gets attributes of a file"},
    {"*getmem", FN_XBOX, NULL,
     " addr=<address> length=<len>: Reads memory from the Xbox"},
    {"*go", FN_XBOX, NULL, ": Resumes suspended title threads"},
    {"*halt", FN_XBOX, NULL, " thread=<threadid> Breaks a thread"},
    {"*isstopped", FN_XBOX, NULL,
     " thread=<threadid>: Determines if a thread is stopped and why"},
    {"*mkdir", FN_XBOX, NULL,
     " name=<remotedir>: Creates a new directory on the Xbox"},
    {"*modlong", FN_XBOX, NULL,
     " name=<module>: Lists the long name of the module"},
    //  { "*reboot",			FN_XBOX,	NULL,
    //  " [warm] [wait]: Reboots the xbox" },
    {"*rename", FN_XBOX, NULL,
     " name=<remotefile> newname=<newname>: Renames a file on the Xbox"},
    {"*resume", FN_XBOX, NULL, " thread=<threadid>: Resumes thread execution"},
    {"*setcontext", FN_XBOX, NULL,
     " thread=<threadid>: Sets the context of the thread."},
    {"*setfileattributes", FN_XBOX, NULL,
     " <remotefile> <attrs>: Sets attributes of a file"},
    {"*setmem", FN_XBOX, NULL,
     " addr=<address> data=<rawdata>: Sets memory on the Xbox"},
    ...
    //  { "*title",				FN_XBOX,	NULL,
    //  " dir=<remotedir> name=<remotexex> [cmdline=<cmdline>]: Sets title to
    //  run" },
    {"*xexinfo", FN_XBOX, NULL,
     " name=<remotexex | 'running'>: Gets info on an xex"},
    {"*crash", FN_XBOX, lc_crashdump, " crash the console, emitting a dump"},
};
const int g_numLocalCommands =
    sizeof(g_localCommands) / sizeof(g_localCommands[0]);
...
//
//-----------------------------------------------------------------------------
void DecodeRebootArgs(int argc, char **argv, char *xexPath, char *xexName,
                      char *xexArgs) {
  char drive[MAX_PATH];
  char dir[MAX_PATH];
...
BOOL lc_dir(int argc, char *argv[]) {
  ...
  if (!GetTargetFileList_r(filePath, recurse,
                           FA_NORMAL | FA_DIRECTORY | FA_READONLY, 0,
                           &pFileList)) {
  ...
  for (nPass = 0; nPass < 2; nPass++) {
    for (nodePtr = pFileList; nodePtr; nodePtr = nodePtr->nextPtr) {
      ...
      ConsoleWindowPrintf(XBX_CLR_DEFAULT, "%s  %s %12s %s\n", dateTimeString,
                          dirString, sizeString, targetName);
    }
  }
...
BOOL lc_del(int argc, char *argv[]) {
  ...
  if (!GetTargetFileList_r(filePath, recurse,
                           FA_NORMAL | FA_READONLY | FA_DIRECTORY, 0,
                           &pFileList)) {
  ...
  for (nodePtr = pFileList; nodePtr; nodePtr = nodePtr->nextPtr) {
    ...
    hr = DmDeleteFile(nodePtr->filename,
                      (nodePtr->attributes & FA_DIRECTORY) != 0);
    if (hr != XBDM_NOERR) {
      if (!bQuiet) {
...
BOOL lc_memory(int argc, char *argv[]) {
  ...
  hr = DmAPI_SendCommand(VXCONSOLE_COMMAND_PREFIX "!"
                                                  "__memory__",
                         true);
  if (FAILED(hr))
    return FALSE;
...
BOOL lc_modules(int argc, char *argv[]) {
  ...
  for (int i = 0; i < list.Count() - 1; i++) {
    ...
    ConsoleWindowPrintf(XBX_CLR_DEFAULT,
                        "Base: 0x%8.8x, Size: %5.2f MB, [%s]\n",
                        pModule->BaseAddress,
                        pModule->Size / (1024.0f * 1024.0f), pModule->Name);
  ...
  for (int core = 0; core < 3; core++) {
    ...
    for (int unit = 0; unit < 2; unit++) {
      for (int i = 0; i < (int)numThreads; i++) {
        ...
        nameBuff[0] = '\0';
        DmGetMemory(threadInfoEx.ThreadNameAddress,
                    threadInfoEx.ThreadNameLength, nameBuff, NULL);
        if (!nameBuff[0]) {
          strcpy(nameBuff, "???");
        }
        ...
        ConsoleWindowPrintf(
            XBX_CLR_DEFAULT,
            "   Id: 0x%8.8x Pri: %2d Proc: %1d Stack: %7.2f KB [%s] %s\n",
            threadList[i], threadInfoEx.Priority, threadInfoEx.CurrentProcessor,
            (float)((unsigned int)threadInfoEx.StackBase -
                    (unsigned int)threadInfoEx.StackLimit) /
                1024.0f,
            nameBuff, suspendBuff);
      }
    }
  }
...
BOOL lc_help(int argc, char *argv[]) {
  if (argc <= 1) {
    ...
    for (int i = 0; i < g_numLocalCommands; i++) {
      if (!_strnicmp(g_localCommands[i].strCommand, argv[1], cch) &&
          g_localCommands[i].strHelp) {
    ...
    if (g_connectedToApp) {
      for (int i = 0; i < g_numRemoteCommands; i++) {
        if (!_strnicmp(g_remoteCommands[i]->strCommand, argv[1], cch) &&
...
BOOL lc_connect(int argc, char *argv[]) {
  ...
  if (!g_connectCount)
    ConsoleWindowPrintf(
        XBX_CLR_DEFAULT, "Connected To: '%s'(%d.%d.%d.%d)\n", g_xboxName,
        ((byte *)&g_xboxAddress)[3], ((byte *)&g_xboxAddress)[2],
        ((byte *)&g_xboxAddress)[1], ((byte *)&g_xboxAddress)[0]);
  g_connectCount++;
...
BOOL lc_listen(int argc, char *argv[]) {
  ...
  hr = DmRegisterNotificationProcessor(g_pdmnSession, VXCONSOLE_COMMAND_PREFIX,
                                       Remote_NotifyCommandFunc);
  if (FAILED(hr)) {
    DmAPI_DisplayError(
        "lc_session(): DmRegisterNotificationProcessor() failure", hr);
    goto cleanUp;
  }

  // get print notifications
  hr = DmRegisterNotificationProcessor(g_pdmnSession, VXCONSOLE_PRINT_PREFIX,
  ...
  // Send initial connect command to the External Command Processor so it knows
  // we're here
  sprintf(cmdStr, "%s %d",
          VXCONSOLE_COMMAND_PREFIX "!"
                                   "__connect__",
          VXCONSOLE_PROTOCOL_VERSION);
  ...
  if (!g_connectedToApp) {
    ...
    if (!g_connectFailure) {
      ...
      if (g_connectFailure == 1) {
        // console may be rebooting, allow sufficient dvd boot up delay, then
        // attempt re-connection 5 seconds barely covers the xbox splash
...
BOOL lc_disconnect(int argc, char *argv[]) {
  ...
  if (g_connectedToApp) {
    ...
    DmAPI_SendCommand(VXCONSOLE_COMMAND_PREFIX "!"
                                               "__disconnect__",
                      false);
...
BOOL lc_ClearConfigs(int argc, char *argv[]) {
  ...
  V_ComposeFileName("HDD:\\Content", "*.*", szTempFilename,
