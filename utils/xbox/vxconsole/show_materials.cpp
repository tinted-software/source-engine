...
void ShowMaterials_SaveConfig() {
  ...
  Sys_SetRegistryInteger("showMaterialsCurrentFrame",
                         g_showMaterials_currentFrame);
  Sys_SetRegistryInteger("showMaterialsSortColumn", g_showMaterials_sortColumn);
  Sys_SetRegistryInteger("showMaterialsSortDescending",
                         g_showMaterials_sortDescending);
  ...
  sprintf(buff, "%d %d %d %d", g_showMaterials_windowRect.left,
          g_showMaterials_windowRect.top, g_showMaterials_windowRect.right,
          g_showMaterials_windowRect.bottom);
...
void ShowMaterials_LoadConfig() {
  ...
  Sys_GetRegistryInteger("showMaterialsCurrentFrame", false,
                         g_showMaterials_currentFrame);
  Sys_GetRegistryInteger("showMaterialsSortColumn", ID_SM_NAME,
                         g_showMaterials_sortColumn);
  Sys_GetRegistryInteger("showMaterialsSortDescending", false,
                         g_showMaterials_sortDescending);

  Sys_GetRegistryString("showMaterialsWindowRect", buff, "", sizeof(buff));
  numArgs =
      sscanf(buff, "%d %d %d %d", &g_showMaterials_windowRect.left,
             &g_showMaterials_windowRect.top, &g_showMaterials_windowRect.right,
             &g_showMaterials_windowRect.bottom);
  if (numArgs != 4 || g_showMaterials_windowRect.left < 0 ||
...
//
//-----------------------------------------------------------------------------
int CALLBACK ShowMaterials_CompareFunc(LPARAM lParam1, LPARAM lParam2,
                                       LPARAM lParamSort) {
...
void ShowMaterials_SortItems() {
  ...
  ListView_SortItems(g_showMaterials_hWndListView, ShowMaterials_CompareFunc,
                     0);
  ...
  for (i = 0;
       i < sizeof(g_showMaterials_Labels) / sizeof(g_showMaterials_Labels[0]);
       i++) {
    char symbol;
    LVCOLUMN lvc;
    ...
    if (i == g_showMaterials_sortColumn)
      ...
      symbol = ' ';
    sprintf(g_showMaterials_Labels[i].nameBuff, "%s %c",
            g_showMaterials_Labels[i].name, symbol);
...
void ShowMaterials_AddViewItem(material_t *pMaterial) {
  ...
  pMaterial->listIndex =
      ListView_InsertItem(g_showMaterials_hWndListView, &lvi);
}
...
//
//-----------------------------------------------------------------------------
LRESULT CALLBACK ShowMaterials_WndProc(HWND hwnd, UINT message, WPARAM wParam,
                                       LPARAM lParam) {
  ...
  case WM_INITMENU:
    CheckMenuItem((HMENU)wParam, IDM_OPTIONS_CURRENTFRAME,
                  MF_BYCOMMAND | (g_showMaterials_currentFrame ? MF_CHECKED
                                                               : MF_UNCHECKED));
    return 0L;
...
void ShowMaterials_Open() {
  ...
  hWnd = CreateWindowEx(WS_EX_CLIENTEDGE, "SHOWMATERIALSCLASS", "",
  ...
  hWnd = CreateWindow(WC_LISTVIEW, "", WS_VISIBLE | WS_CHILD | LVS_REPORT, 0, 0,
                      ...
                      clientRect.bottom - clientRect.top, g_showMaterials_hWnd,
                      (HMENU)ID_SHOWMATERIALS_LISTVIEW, g_hInstance, NULL);
  ...
  ListView_SetExtendedListViewStyleEx(g_showMaterials_hWndListView, style,
                                      style);
  ...
  if (g_showMaterials_windowRect.right && g_showMaterials_windowRect.bottom)
    MoveWindow(
        g_showMaterials_hWnd, g_showMaterials_windowRect.left,
        g_showMaterials_windowRect.top,
        g_showMaterials_windowRect.right - g_showMaterials_windowRect.left,
        g_showMaterials_windowRect.bottom - g_showMaterials_windowRect.top,
        FALSE);
...
int rc_MaterialList(char *commandPtr) {
  ...
  DmGetMemory((void *)materialList, numMaterials * sizeof(xrMaterial_t),
              pLocalList, NULL);
  ...
  for (int i = 0; i < numMaterials; i++) {
    ...
    g_showMaterials_pMaterials[i].pShaderName =
        strdup(pLocalList[i].shaderString);
