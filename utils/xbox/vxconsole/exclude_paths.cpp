...
GameName_t g_GameNames[] = {
    {"bin", false}, {"platform", false}, {"tf", true}, {"portal", true},
    {"hl2", true},  {"episodic", true},  {"ep2", true}};
...
//-----------------------------------------------------------------------------
static HTREEITEM AddItemToTree(HWND hWndTree, LPSTR lpszItem, int nLevel,
                               bool bIsModPath) {
  TVITEM tvi = {0};
...
//-----------------------------------------------------------------------------
static int GetFileList(const char *pDirPath, const char *pPattern,
...
//-----------------------------------------------------------------------------
static void RecurseFileTree_r(const char *pBasePath, const char *pDirPath,
                              int depth, CUtlVector<CUtlString> &dirList,
                              bool bIsModPath) {
  if (depth >= 2) {
  ...
  for (int i = 0; i < dirCount; i++) {
    ...
    RecurseFileTree_r(pBasePath, fileList[i].String(), depth + 1, dirList,
                      bIsModPath);
  }
...
//-----------------------------------------------------------------------------
void ExcludePathsDlg_SetCheckState_r(HWND hWndTree, HTREEITEM hTree, int depth,
                                     int checkState) {
  if (!hTree) {
    return;
  }
  ...
  if (TreeView_GetItem(hWndTree, &tvi)) {
    if (tvi.cChildren) {
      ...
      if (hChild) {
        ExcludePathsDlg_SetCheckState_r(hWndTree, hChild, depth + 1,
                                        checkState);
      }
    }
  } else {
...
//
//	Propogate the check state to all "mod path" children that match the
//specified name. 	A NULL name matches all.
//-----------------------------------------------------------------------------
void ExcludePathsDlg_SetCheckStateLinked_r(HWND hWndTree, HTREEITEM hTree,
                                           int depth, int checkState,
                                           const char *pName) {
  if (!hTree) {
    return;
  }
  ...
  if (TreeView_GetItem(hWndTree, &tvi)) {
    ...
    if (tvi.cChildren) {
      HTREEITEM hChild = TreeView_GetChild(hWndTree, hTree);
      if (hChild) {
        ExcludePathsDlg_SetCheckStateLinked_r(hWndTree, hChild, depth + 1,
                                              checkState, pName);
      }
    }
  } else {
...
//-----------------------------------------------------------------------------
bool ExcludePathsDlg_GetCheckState_r(HWND hWndTree, HTREEITEM hTree, int depth,
                                     int *pCheckState) {
  int checkState;
  ...
  if (TreeView_GetItem(hWndTree, &tvi)) {
    if (tvi.cChildren) {
      HTREEITEM hChild = TreeView_GetChild(hWndTree, hTree);
      if (hChild) {
        if (!ExcludePathsDlg_GetCheckState_r(hWndTree, hChild, depth + 1,
                                             pCheckState)) {
          return false;
        }
      }
...
//-----------------------------------------------------------------------------
void ExcludePathsDlg_Expand_r(HWND hWndTree, HTREEITEM hTree, int depth,
                              int maxDepth, bool bExpand) {
  if (!hTree) {
    return;
  }
  ...
  if (TreeView_GetItem(hWndTree, &tvi)) {
    if (tvi.cChildren) {
      ...
      HTREEITEM hChild = TreeView_GetChild(hWndTree, hTree);
      if (hChild) {
        ExcludePathsDlg_Expand_r(hWndTree, hChild, depth + 1, maxDepth,
                                 bExpand);
      }
    }
  } else {
...
//
//-----------------------------------------------------------------------------
void ExcludePathsDlg_ExpandToShowState_r(HWND hWndTree, HTREEITEM hTree,
                                         int depth) {
  if (!hTree) {
    return;
  }
  ...
  if (TreeView_GetItem(hWndTree, &tvi)) {
    int checkState = -1;
    bool bStateIsSame =
        ExcludePathsDlg_GetCheckState_r(hWndTree, hTree, 0, &checkState);
  ...
  while (1) {
    ...
    if (TreeView_GetItem(hWndTree, &tvi)) {
      int checkState = -1;
      bool bStateIsSame =
          ExcludePathsDlg_GetCheckState_r(hWndTree, hSibling, 0, &checkState);
...
//
//-----------------------------------------------------------------------------
HTREEITEM ExcludePathsDlg_Find_r(HWND hWndTree, HTREEITEM hTree, int depth,
                                 const char *pBasePath, const char *pFindPath) {
  TVITEM tvi = {0};
  ...
  if (tvi.cChildren) {
    HTREEITEM hChild = TreeView_GetChild(hWndTree, hTree);
    if (hChild) {
      HTREEITEM hFindTree = ExcludePathsDlg_Find_r(hWndTree, hChild, depth + 1,
                                                   szPath, pFindPath);
      if (hFindTree) {
        return hFindTree;
      }
  ...
  while (1) {
    ...
    if (tvi.cChildren) {
      HTREEITEM hChild = TreeView_GetChild(hWndTree, hSibling);
      if (hChild) {
        HTREEITEM hFindTree = ExcludePathsDlg_Find_r(
            hWndTree, hChild, depth + 1, szPath, pFindPath);
        if (hFindTree) {
          return hFindTree;
        }
      }
...
//
//	An exclude path represents the path head, and thus does not need to
//iterate its children 	if they are deemed to the same exclusion state.
//-----------------------------------------------------------------------------
void ExcludePathsDlg_BuildExcludeList_r(HWND hWndTree, HTREEITEM hTree,
                                        int depth, const char *pPath) {
  int checkState = -1;
  bool bStateIsSame =
      ExcludePathsDlg_GetCheckState_r(hWndTree, hTree, 0, &checkState);
  if (checkState == -1) {
...
void ExcludePathsDlg_Populate(HWND hWnd, bool bRefresh) {
  ...
  if (bRefresh || !g_PathTable.Count()) {
    ...
    for (int i = 0; i < gamePaths.Count(); i++) {
      ...
      RecurseFileTree_r(g_localPath, gamePaths[i].pathName.String(), 0, dirList,
                        gamePaths[i].bIsModPath);
    }
  }
  ...
  // build the tree view
  for (int iIndex = g_PathTable.FirstInorder();
       iIndex != g_PathTable.InvalidIndex();
       iIndex = g_PathTable.NextInorder(iIndex)) {
  ...
  for (int i = 0; i < g_ExcludePaths.Count(); i++) {
    HTREEITEM hTree = ExcludePathsDlg_Find_r(hWndTree, hTreeRoot, 0, "",
                                             g_ExcludePaths[i].String());
    if (hTree) {
...
void ExcludePathsDlg_Setup(HWND hWnd) {
  ...
  CheckDlgButton(hWnd, IDC_PATHS_LINKGAMEDIRS,
                 g_bLinkGameDirs ? BST_CHECKED : BST_UNCHECKED);

  // read the exisiting exclude paths
  g_ExcludePaths.Purge();
  char szFilename[MAX_PATH];
  V_ComposeFileName(g_localPath, EXCLUDEPATHS_FILE, szFilename,
                    sizeof(szFilename));
...
//
//-----------------------------------------------------------------------------
BOOL CALLBACK ExcludePathsDlg_Proc(HWND hWnd, UINT message, WPARAM wParam,
                                   LPARAM lParam) {
  HWND hWndTree;
  LPNMHDR lpnmh;
  HTREEITEM hTree;
  ...
  case UM_CHECKSTATECHANGE:
    ...
    if (checkState != -1) {
      ...
      if (g_bLinkGameDirs && bIsModPath) {
        ...
        // a child of a mod path (depth > 1), will match all other children in
        // mod paths a mod path (depth = 1) will match all other modpaths
        ExcludePathsDlg_SetCheckStateLinked_r(
            hWndTree, hTreeRoot, 0, checkState, nDepth == 1 ? NULL : szName);
      } else {
  ...
  case WM_PAINT:
    if (s_bAllowPopulate) {
      // unfortunate, but tree view needs to paint first before its state can be
      // set related to checkbox style which doesn't manifest until after
      // WM_INITDIALOG stall the initial population until after the first paint
  ...
  case WM_COMMAND:
    ...
    case IDC_PATHS_EXPAND:
      // expand from root
      hWndTree = GetDlgItem(hWnd, IDC_PATHS_TREE);
      ExcludePathsDlg_Expand_r(hWndTree, TreeView_GetRoot(hWndTree), 0, -1,
                               true);
      return TRUE;
...
//
//-----------------------------------------------------------------------------
bool ExcludePathsDlg_Init() { return true; }
...
void ExcludePathsDlg_Open() {
  ...
  int result = DialogBox(g_hInstance, MAKEINTRESOURCE(IDD_EXCLUDEPATHS),
                         g_hDlgMain, (DLGPROC)ExcludePathsDlg_Proc);
