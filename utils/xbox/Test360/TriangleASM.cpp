...
// Note: VPHYSICS is just used via the datacache to load a model's physics
// collision mesh
...
private:
  ...
  matrix3x4_t *SetUpBones(studiohdr_t *pStudioHdr,
                          const matrix3x4_t &modelMatrix);
#endif
...
#if defined(USE_STUDIORENDER)
matrix3x4_t *CTest360App::SetUpBones(studiohdr_t *pStudioHdr,
...
void CTest360App::RenderScene() {
  ...
  g_pStudioRender->DrawModel(NULL, modelInfo, pBoneToWorld, modelOrigin,
                             drawFlags);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT iMsg, WPARAM wParam, LPARAM lParam) {
  ...
  case WM_XCONTROLLER_KEY:
    ...
    Msg("Port %d: Button %d %s\n", HIWORD(lParam), wParam,
        LOWORD(lParam) ? "Pressed" : "Released");
    switch (wParam) {
...
int CTest360App::InitializeVGUI(void) {
  s_pFactoryList[0] =
      Sys_GetFactory(m_pFileSystem->LoadModule("filesystem_stdio"));
  s_pFactoryList[1] =
      Sys_GetFactory(m_pFileSystem->LoadModule("vguimatsurface"));
  ...
  //"BudgetLabel" ); 	pLabel->SetFont( hFont );
