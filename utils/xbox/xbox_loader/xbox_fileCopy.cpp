...
DWORD WINAPI ReadFileThread(LPVOID lParam) {
  ...
  for (unsigned int readCycle = 0; readCycle < pCopyFile->m_numReadCycles;
       ++readCycle) {
  ...
  retry:
    // read file from DVD
    bResult = ReadFile(pCopyFile->m_hSrcFile, pBuffer->pData,
                       pCopyFile->m_readBufferSize, NULL, &overlappedRead);
    ...
    if (!bResult && dwError != ERROR_IO_PENDING) {
      ...
      GetOverlappedResult(pCopyFile->m_hSrcFile, &overlappedRead, &dwBytesRead,
                          TRUE);
    ...
    pCopyFile->m_pCopyStats->m_totalReadSize +=
        pCopyFile->m_pCopyStats->m_bufferReadSize;
    pCopyFile->m_pCopyStats->m_totalReadTime +=
        pCopyFile->m_pCopyStats->m_bufferReadTime;
...
DWORD WINAPI WriteFileThread(LPVOID lParam) {
  ...
  while (overlappedWrite.Offset < pCopyFile->m_dstFileSize) {
    ...
    if (pCopyFile->m_bInflate) {
      ...
      dwWriteSize = JCALG1_Decompress_Formatted_Buffer(
          pBuffer->dwSize - dwSkip, pBuffer->pData + dwSkip,
          pCopyFile->m_inflateBufferSize, pCopyFile->m_pInflateBuffer);
  ...
  retry:
    // write file to HDD
    bResult = WriteFile(pCopyFile->m_hDstFile, pWriteBuffer,
                        (dwWriteSize / 512) * 512, NULL, &overlappedWrite);
    dwError = GetLastError();
    if (!bResult && dwError != ERROR_IO_PENDING) {
      numAttempts++;
      if (numAttempts == 3) {
      ...
      // Wait for the operation to finish
      GetOverlappedResult(pCopyFile->m_hDstFile, &overlappedWrite,
                          &dwBytesWrite, TRUE);
    ...
    pCopyFile->m_pCopyStats->m_totalWriteSize +=
        pCopyFile->m_pCopyStats->m_bufferWriteSize;
    pCopyFile->m_pCopyStats->m_totalWriteTime +=
        pCopyFile->m_pCopyStats->m_bufferWriteTime;
...
//
//-----------------------------------------------------------------------------
bool CopyFileOverlapped(const char *pSrcFilename, const char *pDstFilename,
                        xCompressHeader *pxcHeader, CopyStats_t *pCopyStats) {
  ...
  copyFile.m_hSrcFile =
      CreateFile(pSrcFilename, GENERIC_READ, 0, NULL, OPEN_EXISTING,
  ...
  copyFile.m_hDstFile =
      CreateFile(pDstFilename, GENERIC_WRITE, 0, NULL, OPEN_ALWAYS,
                 FILE_FLAG_OVERLAPPED | FILE_FLAG_NO_BUFFERING, NULL);
  ...
  copyFile.m_numReadCycles =
      (copyFile.m_srcFileSize + copyFile.m_readBufferSize - 1) /
      copyFile.m_readBufferSize;

  // setup write buffer
  if (pxcHeader) {
    copyFile.m_pInflateBuffer =
        new unsigned char[pxcHeader->nDecompressionBufferSize];
  ...
  if (copyFile.m_dstFileSize % 512) {
    ...
    HANDLE hFile = CreateFile(pDstFilename, GENERIC_WRITE, 0, NULL,
