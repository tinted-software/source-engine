...
HRESULT CALLBACK GetNextPacket(DWORD dwContext, void **ppPacket,
...
// Funtion Prototypes for packet loading functions for loading from a block of
// memory.
HRESULT CALLBACK GetNextMemoryPacket(DWORD dwContext, void **ppPacket,
                                     DWORD *pOffsetToNextPacket);
...
// Desc: Helper function for the three Open functions. Enables the audio
// streams, initializes the video descriptor, and allocates textures if needed.
//-----------------------------------------------------------------------------
HRESULT CXMVPlayer::FinishOpeningFile(D3DFORMAT format,
  ...
  for (int i = 0; i < XMVPLAYER_NUMTEXTURES; i++) {
    ...
    if (bAllocateTextures) {
      hr = pDevice->CreateTexture(m_VideoDesc.Width, m_VideoDesc.Height, 1, 0,
                                  format, 0, &m_pTextures[i]);
      if (FAILED(hr)) {
  ...
  // Initialize what texture we are decoding to, if decoding for texture
  // mapping.
...
//-----------------------------------------------------------------------------
HRESULT CXMVPlayer::OpenFile(const CHAR *lpFilename, D3DFORMAT format,
                             LPDIRECT3DDEVICE8 pDevice,
                             BOOL bAllocateTextures) {
  HRESULT hr = S_OK;
  ...
  hr = XMVDecoder_CreateDecoderForFile(XMVFLAG_SYNC_ON_NEXT_VBLANK,
                                       (CHAR *)lpFilename, &m_pXMVDecoder);
  if (FAILED(hr)) {
...
//-----------------------------------------------------------------------------
HRESULT CXMVPlayer::OpenFileForPackets(const CHAR *lpFilename, D3DFORMAT format,
                                       LPDIRECT3DDEVICE8 pDevice,
                                       BOOL bAllocateTextures) {
  HRESULT hr = S_OK;
  ...
  // Open the input file.
  m_loadContext.hFile = CreateFile(
      lpFilename, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING,
  ...
  // Start the read.
  if (0 == ReadFile(m_loadContext.hFile, first4Kbytes, sizeof(first4Kbytes),
                    NULL, &m_loadContext.Overlapped)) {
  ...
  DWORD dwBytesRead;
  if (!GetOverlappedResult(m_loadContext.hFile, &m_loadContext.Overlapped,
                           &dwBytesRead, TRUE)) {
    Destroy();
    return E_FAIL;
  }
  ...
  hr = XMVDecoder_CreateDecoderForPackets(
      XMVFLAG_SYNC_ON_NEXT_VBLANK, first4Kbytes, (DWORD)&m_loadContext,
  ...
  // The size of the first packet and the minimum size of the two packet buffers
  // are stored in the second and third DWORDS of the file. From xmv.h:
  ...
  // XPhysicalAlloc is used so that 5.1 or compressed audio streams can be
  // played.
  m_physicalBuffer = (BYTE *)XPhysicalAlloc(dwRequiredPacketSize * 2,
  ...
  if (0 == ReadFile(m_loadContext.hFile, m_physicalBuffer, dwThisPacketSize,
                    NULL, &m_loadContext.Overlapped)) {
    if (GetLastError() != ERROR_IO_PENDING) {
      Destroy();
      return E_FAIL;
...
//-----------------------------------------------------------------------------
HRESULT CXMVPlayer::OpenMovieFromMemory(const CHAR *lpFilename,
                                        D3DFORMAT format,
                                        LPDIRECT3DDEVICE8 pDevice,
                                        BOOL bAllocateTextures) {
  HRESULT hr = S_OK;
  ...
  hr = XMVDecoder_CreateDecoderForPackets(
      XMVFLAG_SYNC_ON_NEXT_VBLANK, first4Kbytes, (DWORD)&m_loadContext,
      GetNextMemoryPacket, ReleasePreviousMemoryPacket, &m_pXMVDecoder);
  if (FAILED(hr)) {
    Destroy();
    m_bError = TRUE;
    return E_FAIL;
...
//-----------------------------------------------------------------------------
LPDIRECT3DTEXTURE8
CXMVPlayer::AdvanceFrameForTexturing(LPDIRECT3DDEVICE8 pDevice) {
  // You must pass bAllocateTextures==TRUE to Open if you're going to use
  // GetTexture/AdvanceFrame.
...
//-----------------------------------------------------------------------------
LPDIRECT3DTEXTURE8
CXMVPlayer::AdvanceFrameForOverlays(LPDIRECT3DDEVICE8 pDevice) {
  // You must pass bAllocateTextures==TRUE to Open if you're going to use
  // GetTexture/AdvanceFrame.
  assert(m_pTextures[0]);

  // You have to call CXMVPlayer::EnableOverlays() if you are going to use
  // overlays.
  ...
  // the previous texture has *stopped* being displayed. Once
  // GetUpdateOverlayState() returns TRUE then we know that pSubmittedTexture is
  // being displayed, which means that, pShowingTexture is available as a
  // decoding target.
...
//-----------------------------------------------------------------------------
void CXMVPlayer::TerminatePlayback() { m_pXMVDecoder->TerminatePlayback(); }
...
static HRESULT CALLBACK GetNextPacket(DWORD dwContext, VOID **ppPacket,
  ...
  DWORD dwBytesRead;
  if (GetOverlappedResult(pContext->hFile, &pContext->Overlapped, &dwBytesRead,
                          FALSE)) {
...
//-----------------------------------------------------------------------------
static HRESULT CALLBACK ReleasePreviousPacket(DWORD dwContext,
                                              LONGLONG llNextReadByteOffset,
                                              DWORD dwNextPacketSize) {
  LOAD_CONTEXT *pContext = (LOAD_CONTEXT *)dwContext;
  if (NULL == pContext)
...
static HRESULT CALLBACK GetNextMemoryPacket(DWORD dwContext, VOID **ppPacket,
  ...
  memcpy(pContext->pLoadingPacket,
         pContext->pInputBuffer + pContext->readOffset, dwBytesRead);
  pContext->readOffset += dwBytesRead;

  // Swap pointers so that next time we load it goes into the other packet
  // block.
...
// and setup for the next packet.
//-----------------------------------------------------------------------------
static HRESULT CALLBACK ReleasePreviousMemoryPacket(
    DWORD dwContext, LONGLONG llNextReadByteOffset, DWORD dwNextPacketSize) {
  LOAD_CONTEXT *pContext = (LOAD_CONTEXT *)dwContext;
  if (NULL == pContext)
    return E_FAIL;
  ...
  // Record the size of the next packet we are supposed to read, for
  // GetNextMemoryPacket.
