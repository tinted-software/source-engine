...
public:
  ...
  void DrawDebug();
  BOOL PlayVideoFrame();
  HRESULT StartVideo(const CHAR *strFilename, bool bFromMemory,
                     bool bFatalOnError);
  void StopVideo();
  ...
  void DrawTexture(D3DTexture *pD3DTexture, int x, int y, int w, int h,
                   int color);
...
extern bool CopyFileOverlapped(const char *pSrc, const char *pDest,
                               xCompressHeader *pxcHeader,
                               CopyStats_t *pCopyStats);
