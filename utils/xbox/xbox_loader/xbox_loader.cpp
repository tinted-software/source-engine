...
//-----------------------------------------------------------------------------
bool FixupNamespaceFilename(const char *pFilename, char *pOutFilename,
                            int languageID) {
...
bool GetXCompressedHeader(const char *pFilename, xCompressHeader *pHeader) {
  ...
  if (hFile != INVALID_HANDLE_VALUE) {
    ...
    CloseHandle(hFile);
    if (pHeader->nMagic == xCompressHeader::MAGIC &&
        pHeader->nVersion == xCompressHeader::VERSION) {
      // valid
      return true;
    }
  }
...
void CXBoxLoader::FatalMediaError() {
  ...
  if (m_pDefaultTrueTypeFont) {
    ...
    m_pDefaultTrueTypeFont->TextOut(pBackBuffer, pLine1, (unsigned)-1, 320,
                                    240 - 15);
    m_pDefaultTrueTypeFont->TextOut(pBackBuffer, pLine2, (unsigned)-1, 320,
                                    240 + 15);
  }
...
// DrawTexture
//-----------------------------------------------------------------------------
void CXBoxLoader::DrawTexture(D3DTexture *pD3DTexture, int x, int y, int w,
                              int h, int color) {
  struct VERTEX {
    D3DXVECTOR4 p;
    D3DCOLOR c;
  ...
  if (!m_pVB) {
    ...
    D3DDevice::CreateVertexBuffer(4 * sizeof(VERTEX), D3DUSAGE_WRITEONLY, 0L,
...
// May take a few ms.
//-----------------------------------------------------------------------------
HRESULT CXBoxLoader::StartVideo(const CHAR *strFilename, bool bFromMemory,
                                bool bFatalOnError) {
  HRESULT hr;
  if (bFromMemory) {
    // play from memory, so as no to interfere with disc access
    hr = m_player.OpenMovieFromMemory(strFilename, D3DFMT_LIN_A8R8G8B8,
                                      m_pd3dDevice, TRUE);
  } else {
    // play from disc
    hr =
        m_player.OpenFile(strFilename, D3DFMT_LIN_A8R8G8B8, m_pd3dDevice, TRUE);
  }
...
bool CXBoxLoader::LoadInstallScript() {
  ...
  while (1) {
    ...
    bTargetIsLocalized =
        FixupNamespaceFilename(dstFile, localizedFile, languageID);

    if (bTargetIsLocalized) {
      // localized files are allowed to change without requiring a full
      // re-install
      ...
      do {
        ...
        if (strstr(sourceFilename, ".xz_") && strstr(targetFilename, ".xzp")) {
          ...
          if (GetXCompressedHeader(sourceFilename, &xcHeader)) {
            ...
            if (!bForce && IsTargetFileValid(targetFilename,
                                             xcHeader.nUncompressedFileSize)) {
...
DWORD WINAPI InstallThreadFunc(LPVOID lpParam) {
  ...
  *(DWORD *)lpParam = 1;
  ...
  for (int i = 0; i < g_installData.m_numFiles; ++i) {
    ...
    bSuccess = CopyFileOverlapped(g_installData.m_ppSrcFiles[i],
                                  g_installData.m_ppDstFiles[i],
                                  g_installData.m_ppxcHeaders[i], &g_copyStats);
    ...
    if (bSuccess) {
      // add marker
      hFile = CreateFile(mrkFilename, GENERIC_READ | GENERIC_WRITE, 0, NULL,
      ...
      g_copyStats.m_bytesCopied =
          bytesCopied + g_installData.m_pDstFileSizes[i];
    }
  }
...
bool CXBoxLoader::StartInstall(void) {
  ...
  m_installThread =
      CreateThread(NULL, 0, &InstallThreadFunc, &m_dwLoading, 0, 0);
  if (!m_installThread) {
    // failed
    return false;
  }
...
void CXBoxLoader::DrawSlideshow() {
  ...
  if (!m_bFinalSlide) {
    ...
    if (fadeInColor != 0xFFFFFFFF && m_SlideShowCount != -1)
      DrawTexture(
          m_pSlideShowTextures[m_SlideShowCount % MAX_SLIDESHOW_TEXTURES], 0, 0,
          640, 480, 0xFFFFFFFF);
    DrawTexture(
        m_pSlideShowTextures[(m_SlideShowCount + 1) % MAX_SLIDESHOW_TEXTURES],
        0, 0, 640, 480, fadeInColor);
  } else {
    // fade last slide out
...
void CXBoxLoader::DrawDebug() {
  ...
  swprintf(textBuffer, L"From: %s (%.2f MB)", srcFilename,
           (float)g_copyStats.m_readSize / (1024.0f * 1024.0f));
  m_Font.DrawText(xPos, yPos + 20, 0xffffffff, textBuffer, XBFONT_CENTER_X);
  swprintf(textBuffer, L"To: %s (%.2f MB)", dstFilename,
           (float)g_copyStats.m_writeSize / (1024.0f * 1024.0f));
  m_Font.DrawText(xPos, yPos + 40, 0xffffffff, textBuffer, XBFONT_CENTER_X);

  if (g_copyStats.m_bufferReadTime && m_dwLoading)
    rate = (g_copyStats.m_bufferReadSize / (1024.0f * 1024.0f)) /
           (g_copyStats.m_bufferReadTime * 0.001f);
  else
    rate = 0;
  swprintf(textBuffer, L"Buffer Read: %.2f MB (%.2f MB/s) (%d)",
           g_copyStats.m_bufferReadSize / (1024.0f * 1024.0f), rate,
           g_copyStats.m_numReadBuffers);
  m_Font.DrawText(xPos, yPos + 80, 0xffffffff, textBuffer, XBFONT_CENTER_X);

  rate = g_copyStats.m_inflateTime && m_dwLoading
             ? (float)g_copyStats.m_inflateSize /
                   (g_copyStats.m_inflateTime * 0.001f)
             : 0;
  swprintf(textBuffer, L"Inflate: %.2f MB (%.2f MB/s)",
           g_copyStats.m_inflateSize / (1024.0f * 1024.0f),
  ...
  if (g_copyStats.m_bufferWriteTime && m_dwLoading)
    rate = (g_copyStats.m_bufferWriteSize / (1024.0f * 1024.0f)) /
           (g_copyStats.m_bufferWriteTime * 0.001f);
  else
    rate = 0;
  swprintf(textBuffer, L"Buffer Write: %.2f MB (%.2f MB/s) (%d)",
           g_copyStats.m_bufferWriteSize / (1024.0f * 1024.0f), rate,
           g_copyStats.m_numWriteBuffers);
  m_Font.DrawText(xPos, yPos + 120, 0xffffffff, textBuffer, XBFONT_CENTER_X);

  rate = g_copyStats.m_totalReadTime && m_dwLoading
             ? (float)g_copyStats.m_totalReadSize /
                   (g_copyStats.m_totalReadTime * 0.001f)
             : 0;
  swprintf(textBuffer, L"Total Read: %d MB (%.2f MB/s)",
           g_copyStats.m_totalReadSize / (1024 * 1024),
           rate / (1024.0f * 1024.0f));
  m_Font.DrawText(xPos, yPos + 160, 0xffffffff, textBuffer, XBFONT_CENTER_X);

  rate = g_copyStats.m_totalWriteTime && m_dwLoading
             ? (float)g_copyStats.m_totalWriteSize /
                   (g_copyStats.m_totalWriteTime * 0.001f)
             : 0;
  swprintf(textBuffer, L"Total Write: %d MB (%.2f MB/s)",
           g_copyStats.m_totalWriteSize / (1024 * 1024),
           rate / (1024.0f * 1024.0f));
  ...
  swprintf(textBuffer, L"Progress: %d/%d MB Elapsed: %d secs (%.2f MB/s)",
           g_copyStats.m_bytesCopied / (1024 * 1024),
           g_installData.m_totalSize / (1024 * 1024), (int)elapsed,
           rate / (1024.0f * 1024.0f));
...
void CXBoxLoader::DrawLoadingMarquee() {
  ...
  int y = 0.80f * 480;
  DrawTexture(m_pLoadingIconTexture, (640 - 64) / 2, y - 64, 64, 64,
              0xFFFFFFFF);

  // draw loading text
  m_Font.Begin();
  m_Font.SetScaleFactors(0.8f, 0.8f);
  m_Font.DrawText(320, y, PROGRESS_TEXT_COLOR, GetLocalizedLoadingString(),
                  XBFONT_CENTER_X);
  m_Font.End();
}
...
void CXBoxLoader::DrawProgressBar() {
  ...
  float tUp =
      (float)(GetTickCount() - m_LoadingBarStartTime) / LOADINGBAR_UPTIME;
  if (tUp > 1.0f)
    tUp = 1.0f;
  ...
  if (tUp == 1.0f) {
    ...
    if (t0 > 1.0f)
      t0 = 1.0f;
    t1 = (float)(GetTickCount() - m_LoadingBarStartTime - LOADINGBAR_WAITTIME) /
         LOADINGBAR_SLIDETIME;
    if (t1 < 0.0f)
      t1 = 0.0f;
    else if (t1 > 1.0f)
      t1 = 1.0f;
  ...
  DrawTexture(m_pFooterTexture, x, y, FOOTER_W, 480 - PROGRESS_Y,
              PROGRESS_FOOTER_COLOR);
  x += FOOTER_W - 35;
  ...
  m_Font.DrawText(x, y + 20, PROGRESS_TEXT_COLOR, GetLocalizedLoadingString(),
                  XBFONT_LEFT);
  m_Font.End();
...
BOOL CXBoxLoader::PlayVideoFrame() {
  ...
  if (!m_player.IsPlaying()) {
    if (m_bCaptureLastMovieFrame) {
      ...
      if (pTexture) {
        ...
        m_pd3dDevice->CreateTexture(d3dSurfaceDesc.Width, d3dSurfaceDesc.Height,
                                    0, 0, d3dSurfaceDesc.Format, 0,
                                    &m_pLastMovieFrame);
        pTexture->LockRect(0, &srcRect, NULL,
                           D3DLOCK_READONLY | D3DLOCK_NOOVERWRITE);
        m_pLastMovieFrame->LockRect(0, &dstRect, NULL,
                                    D3DLOCK_READONLY | D3DLOCK_NOOVERWRITE);
        memcpy(dstRect.pBits, srcRect.pBits,
               srcRect.Pitch * d3dSurfaceDesc.Height);
      }
    }
  ...
  // On linear textures the texture coordinate range is from 0,0 to
  // width,height, instead of 0,0 to 1,1.
...
HRESULT CXBoxLoader::Initialize() {
  ...
  if (retVal == ERROR_SUCCESS) {
    if (launchType == LDT_FROM_DASHBOARD) {
      ...
      LD_FROM_DASHBOARD *pLaunchFromDashboard =
          (LD_FROM_DASHBOARD *)(&launchData);
...
HRESULT CXBoxLoader::FrameMove() {
  ...
  if (m_bAllowAttractAbort &&
      (m_DefaultGamepad.bPressedAnalogButtons[XINPUT_GAMEPAD_A] ||
       (m_DefaultGamepad.wPressedButtons & XINPUT_GAMEPAD_START))) {
    StopVideo();
