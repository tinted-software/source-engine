...
void PrintHelp() {
  printf(
      "Usage: rpt -i <in .csv file> -m <.mdl relative path> -o <output dir>\n");
  printf("				[-f <fac output dir>] [-p] [-w] [-r "
         "<sample rate in hz>] [-s <sample filter size>]\n");
  printf(
      "				[-nop4] [-vproject <path to gameinfo.txt>]\n");
  printf("\t-h,-help,-?\t:Displays this help message.\n");
  printf("\t-m\t: Indicates the path of the .mdl file under game/mod/models/ "
         "to use in the sfm files.\n");
  printf("\t-o\t: Indicates output directory to place generated files in.\n");
  printf("\t-f\t: [Optional] Indicates that facial files should be created in "
         "the specified fac output dir.\n");
  printf("\t-p\t: [Optional] Indicates the extracted phonemes should be "
         "written into the wav file.\n");
  printf("\t-w\t: [Optional] Indicates the phonemes should be read from the "
         "wav file. Cannot also have -p specified.\n");
  printf("\t-r\t: [Optional] Specifies the phoneme extraction sample rate "
         "(default = 20)\n");
  printf("\t-s\t: [Optional] Specifies the phoneme extraction sample filter "
         "size (default = 0.08)\n");
  printf("\t-nop4\t: Disables auto perforce checkout/add.\n");
  printf("\t-vproject\t: Specifies path to a gameinfo.txt file (which mod to "
         "build for).\n");
}
...
static size_t StripFileName(char *pFileName) {
  // In this case, we queried the key properly. Strip off the steamclient.dll
  // name to make a good full path to steam.exe
...
static DWORD GetActiveSteamPID() {
  HKEY hKey;
  LONG hResult = RegOpenKey(HKEY_CURRENT_USER,
                            "Software\\Valve\\Steam\\ActiveProcess", &hKey);
  ...
  hResult = RegQueryValueEx(hKey, "pid", NULL, &nType,
                            (unsigned char *)pTempBuf, &nLen);
  RegCloseKey(hKey);
...
static bool DetectSteamPath(char *pFileName, int nBufLen) {
  *pFileName = 0;
  ...
  if (hResult == ERROR_SUCCESS) {
    nLen = (DWORD)nBufLen;
    hResult = RegQueryValueEx(hKey, "SteamClientDll", NULL, NULL,
                              (LPBYTE)pFileName, &nLen);
    RegCloseKey(hKey);
    if (hResult == ERROR_SUCCESS) {
      // In this case, we queried the key properly. Strip off the
      // steamclient.dll name to make a good full path to steam.exe
  ...
  nLen = (DWORD)nBufLen;
  hResult =
      RegQueryValueEx(hKey, "SteamExe", NULL, NULL, (LPBYTE)pFileName, &nLen);
  RegCloseKey(hKey);
...
static void DisplayWindowsError(const char *pMessage) {
  ...
  FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, NULL, dw,
                MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), pErrorMessage,
...
// Deletes steam.cfg
//-----------------------------------------------------------------------------
static void DeleteCfgFile(const char *pFullPath) { remove(pFullPath); }
...
static DWORD LaunchSteam(const char *pFullPath, const char *pArgs) {
  ...
  if (!CreateProcess(pFullPath, (char *)pArgs, NULL, NULL, TRUE,
...
bool CopyFilesRecursively(const char *pSrcPath, const char *pDestPath) {
  ...
  do {
    ...
    if ((find.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) &&
        ((find.dwFileAttributes & FILE_ATTRIBUTE_HIDDEN) == 0)) {
      _snprintf(pSrcChildPath, sizeof(pSrcChildPath), "%s%s\\", pSrcPath,
                find.cFileName);
      _snprintf(pDestChildPath, sizeof(pDestChildPath), "%s%s\\", pDestPath,
                find.cFileName);
...
static bool CopyFilesIntoPosition(const char *pSteamPath) {
  ...
  if (!::GetModuleFileName((HINSTANCE)GetModuleHandle(NULL), pSrcRoot,
                           sizeof(pSrcRoot))) {
...
int main(int argc, char **argv) {
  if (FindParm("-h", argc, argv) || FindParm("-help", argc, argv) ||
      FindParm("-?", argc, argv)) {
    PrintHelp();
    return 0;
  }
