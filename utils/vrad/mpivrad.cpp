...
void UnSerializeFace(MessageBuffer *pmb, int facenum, int iSource) {
  ...
  if (pmb->read(fl, sizeof(facelight_t)) < 0)
    Error("UnSerializeFace - invalid facelight_t from %s (mb len: %d, offset: "
          "%d)",
          VMPI_GetMachineName(iSource), pmb->getLen(), pmb->getOffset());

  fl->sample = (sample_t *)calloc(fl->numsamples, sizeof(sample_t));
  if (pmb->read(fl->sample, sizeof(sample_t) * fl->numsamples) < 0)
    Error("UnSerializeFace - invalid sample_t from %s (mb len: %d, offset: %d, "
          "fl->numsamples: %d)",
          VMPI_GetMachineName(iSource), pmb->getLen(), pmb->getOffset(),
          fl->numsamples);
    ...
    for (n = 0; n < NUM_BUMP_VECTS + 1; ++n) {
      if (fl->light[i][n]) {
        ...
        if (ReadValues(pmb, fl->light[i][n], fl->numsamples) < 0)
          Error("UnSerializeFace - invalid fl->light from %s (mb len: %d, "
                "offset: %d)",
                VMPI_GetMachineName(iSource), pmb->getLen(), pmb->getOffset());
      }
    }
  ...
  if (fl->luxel) {
    ...
    if (ReadValues(pmb, fl->luxel, fl->numluxels) < 0)
      Error("UnSerializeFace - invalid fl->luxel from %s (mb len: %d, offset: "
            "%d)",
            VMPI_GetMachineName(iSource), pmb->getLen(), pmb->getOffset());
  }
  ...
  if (fl->luxelNormals) {
    ...
    if (ReadValues(pmb, fl->luxelNormals, fl->numluxels) < 0)
      Error("UnSerializeFace - invalid fl->luxelNormals from %s (mb len: %d, "
            "offset: %d)",
            VMPI_GetMachineName(iSource), pmb->getLen(), pmb->getOffset());
  }
}

void MPI_ReceiveFaceResults(uint64 iWorkUnit, MessageBuffer *pBuf,
                            int iWorker) {
...
void MPI_ReceiveVisLeafsResults(uint64 iWorkUnit, MessageBuffer *pBuf,
                                int iWorker) {
