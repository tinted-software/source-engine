...
bool WriteTGAFile(const char *pFilename, bool bAllowTranslucent, byte *pBits,
                  ...
                  bool *bTranslucent, bool *bResized) {
...
int PrintUsage(const char *pExtra, ...) {
  ...
  printf("ex: %s -vtex -BaseDir c:\\hl2\\dod -WadFile c:\\hl1\\dod\\*.wad\n",
         __argv[0]);
  printf("ex: %s -vtex -BaseDir c:\\hl2\\dod -bmpfile test.bmp -SubDir "
         "models\\props\n",
         __argv[0]);
  printf("ex: %s -vtex -vmtparam $ignorez 1 -BaseDir c:\\hl2\\dod -sprfile "
  ...
  for (i = 0; i < header.numframes; i++) {
    ...
    if (type == SPR_SINGLE) {
      ...
      if (frame.width > 5000 || frame.height > 5000 || frame.width < 1 ||
          frame.height < 1) {
        Warning("WARNING: sprite %s has an invalid frame size (%d x %d) for "
                "frame %d.\n",
      ...
      Error("Sprite %s uses type SPR_GROUP. Get a programmer to add support "
            "for it.\n",
            pFilename);
    } else {
      Warning(
