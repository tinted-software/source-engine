...
public:
  ...
  void
  MaybeFlushAckChunks(unsigned short chunksToAck[NUM_BUFFERED_CHUNK_ACKS][2],
                      int &nChunksToAck, DWORD &lastAckTime) {
  ...
  int CheckFileChunkPackets(char *data, int dataSize) {
    ...
    if (pPacket->m_Len > dataSize) {
      len = -1;
      Warning("CWorkerMulticastListener::ListenFor: Got a section of data too "
              "long (%d bytes).",
              pPacket->m_Len);
    } else {
  ...
  CWorkerFile *ListenFor(const char *pFilename, const char *pPathID) {
    ...
    while (m_nUnfinishedFiles > 0) {
      ...
      if (payloadLen < 0) {
        Warning("CWorkerMulticastListener::ListenFor: invalid packet received "
                "on multicast group\n");
        continue;
      }
      ...
      if (!(pTestFile->m_ChunksReceived[iChunk >> 3] & (1 << (iChunk & 7)))) {
        ...
        if ((int)pInfo->m_UncompressedSize !=
          ...
          Error("ListenFor(): invalid compressed or uncompressed size.\n"
                ...
                pTestFilename, pTestFile->GetFilename(),
                pInfo->m_CompressedSize, pTestFile->m_CompressedData.Count(),
