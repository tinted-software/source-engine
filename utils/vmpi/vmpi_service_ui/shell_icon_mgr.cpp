...
CShellIconMgr::~CShellIconMgr() { Term(); }

bool CShellIconMgr::Init(IShellIconMgrHelper *pHelper, const char *pToolTip,
  ...
  // Don't handle errors here because the taskbar may not be created yet if we
  // are a service.
...
void CShellIconMgr::ChangeIcon(int iIconResourceID) {
  ...
  data.hWnd = m_hWnd;
  data.hIcon =
      LoadIcon(m_pHelper->GetHInstance(), MAKEINTRESOURCE(iIconResourceID));
...
void CShellIconMgr::CreateTrayIcon() {
  ...
  data.hWnd = m_hWnd;
  data.hIcon = LoadIcon(m_pHelper->GetHInstance(),
                        MAKEINTRESOURCE(m_iCurIconResourceID));
  Q_strncpy(data.szTip, m_pToolTip, sizeof(data.szTip));

  Shell_NotifyIcon(NIM_ADD, &data);
}

LRESULT CShellIconMgr::WindowProc(HWND hWnd, UINT uMsg, WPARAM wParam,
                                  LPARAM lParam) {
  if (hWnd != m_hWnd) {
...
LRESULT
CShellIconMgr::StaticWindowProc(HWND hwnd,     // handle to window
