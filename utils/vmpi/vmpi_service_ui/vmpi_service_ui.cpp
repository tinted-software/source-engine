...
void LoadStateFromRegistry() {
  ...
  if (hKey) {
    ...
    if (RegQueryValueEx(hKey, "HighlightIconWhenBusy", 0, &type,
...
void SaveStateToRegistry() {
  ...
  if (hKey) {
    ...
    RegSetValueEx(hKey, "HighlightIconWhenBusy", 0, REG_DWORD,
...
void CUIConnMgr::HandlePacket(const char *pData, int len) {
  ...
  if (packetID == VMPI_SERVICE_TO_UI_CONSOLE_TEXT) {
    ...
    if (CreateProcess(NULL, commandLine, NULL, NULL, false, CREATE_NO_WINDOW,
...
void InitConsoleWindow() {
  ...
  g_pConsoleWnd =
      CreateConsoleWnd(g_hInstance, IDD_VMPI_SERVICE, IDC_DEBUG_OUTPUT, false);
}
...
void UpdatePopupMenuState() {
  bool bEnabled = (g_iCurState == VMPI_SERVICE_STATE_IDLE ||
                   g_iCurState == VMPI_SERVICE_STATE_BUSY);
  EnableMenuItem(g_hPopupMenu, ID_ENABLE_WORKER,
                 bEnabled ? MF_GRAYED : MF_ENABLED);
  EnableMenuItem(g_hPopupMenu, ID_DISABLE_WORKER,
                 !bEnabled ? MF_GRAYED : MF_ENABLED);

  // Enable or disable console items.
  EnableMenuItem(g_hPopupMenu, ID_SHOW_CONSOLE_WINDOW,
                 g_pConsoleWnd->IsVisible() ? MF_GRAYED : MF_ENABLED);
  EnableMenuItem(g_hPopupMenu, ID_HIDE_CONSOLE_WINDOW,
                 !g_pConsoleWnd->IsVisible() ? MF_GRAYED : MF_ENABLED);

  CheckMenuItem(g_hPopupMenu, ID_SCREENSAVER_MODE,
                g_bScreensaverMode ? MF_CHECKED : MF_UNCHECKED);
  CheckMenuItem(g_hPopupMenu, ID_HIGHLIGHT_ICON_WHEN_BUSY,
                g_bHighlightIconWhenBusy ? MF_CHECKED : MF_UNCHECKED);
}
...
public:
  virtual HINSTANCE GetHInstance() { return g_hInstance; }
  ...
  virtual int WindowProc(void *pWnd, int uMsg, long wParam, long lParam) {
    ...
    case MYWM_NOTIFYICON: {
      if (lParam == WM_RBUTTONDOWN) {
        ...
        TrackPopupMenu(g_hPopupMenu, TPM_RIGHTALIGN | TPM_BOTTOMALIGN,
    ...
    case WM_COMMAND: {
      ...
      case ID_SET_PASSWORD: {
        DialogBox(g_hInstance, MAKEINTRESOURCE(IDD_SET_PASSWORD), NULL,
                  SetPasswordDlgProc);
      } break;
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
  ...
  if (!g_ShellIconMgr.Init(&g_ShellIconMgrHelper, g_pIconTooltip,
                           MYWM_NOTIFYICON, IDI_WAITING_ICON)) {
    return false;
  }
  ...
  // Important that we call this instead of letting the destructor do it because
  // it deletes its socket and it needs to cleanup some threads.
  g_ConnMgr.Term();
