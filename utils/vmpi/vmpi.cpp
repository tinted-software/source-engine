...
bool g_bVMPISDKModeSet =
    false; // If g_bVMPISDKMode has not been set, then VMPI_IsSDKMode just looks
           // for VMPI_Transfer (and doesn't check the command line).
...
public:
  ...
  bool m_bIsAService; // If true, then this is just a service getting the files.
                      // Don't count it as an active worker.
  ...
  if (pBuf->getLen() >= 2) {
    if (pBuf->data[1] == VMPI_INTERNAL_SUBPACKET_MACHINE_NAME) {
      ...
      if (pBuf->ReadString(g_MasterExeName, sizeof(g_MasterExeName)) == -1)
        Error("Error in ReadString() while reading "
...
void VMPI_ReceiveCommandLine() {
  ...
  while (!g_bReceivedWorkerCommandLine) {
    if (Plat_FloatTime() - startTime > 30)
      Error("VMPI_ReceiveCommandLine: timeout. Is the master running in SDK "
            "mode?");
...
void VMPI_ReceiveExeName() {
  ...
  if (V_stricmp(fileBase, "vmpi_transfer") != 0) {
    if (V_stricmp(fileBase, g_MasterExeName) != 0) {
      Error("VMPI_ReceiveExeName: mismatched exe names (master: %s, me: "
            "%s).\nThis usually just means the master finished"
            " a job like vvis really fast and started a vrad immediately, and "
            "an old vvis worker connected to the new vrad job.",
  ...
  for (int i = 0; i < argc - 1; i++) {
    if (V_stricmp(argv[i], "-mpi_PatchWorkers") == 0) {
      ...
      for (int iWorker = 0; iWorker < workerCount; iWorker++) {
        ...
        if (iArg >= argc)
          Error("-mpi_PatchWorkers: %d specified for count, but not enough IPs "
                "following.\n",
                workerCount);
...
void CMasterBroadcaster::BuildBroadcastPacket(bf_write &buf) {
  ...
  buf.WriteShort(m_iDownloaderListenPort); // Tell the port that we're listening
                                           // for downloaders on.
}
...
bool SpawnLocalWorker(int argc, char **argv, int iListenPort,
  ...
  if (CreateProcess(
          ...
          NULL,                    // environment
          workingDir, // current directory (use c:\\ because we don't want it to
...
void VerifyValidSDKMode() {
  ...
  if (!IsValidSDKBinPath(outStrings, &err)) {
    ...
    if (err == 5)
      Error("VMPI running in SDK mode but Steam hasn't been run recently. "
            "Please run Steam and retry.");
    else
      Error("VMPI running in SDK mode but incorrect SDK install detected "
            "(error %d).",
            err);
  }
}
...
void VMPI_Init_PatchMaster(int argc, char **argv) {
  ...
  if (!pPatchDirectory)
    Error("-mpi_PatchDirectory <dir> must be specified if using -PatchHost "
          "mode.");
