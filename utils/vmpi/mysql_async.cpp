...
public:
  CMySQLAsync() {
    ...
    m_hExitEvent =
        CreateEvent(NULL, true, false,
                    NULL); // Use manual reset because we want it to cascade out
  ...
  bool Init(IMySQL *pSQL) {
    ...
    DWORD dwThreadID;
    m_hThread = CreateThread(NULL, 0, &CMySQLAsync::StaticThreadFn, this, 0,
                             &dwThreadID);
    if (m_hThread) {
      m_pSQL = pSQL;
      return true;
  ...
  void Term() {
    ...
    FOR_EACH_LL(m_QueryResults, i) { m_QueryResults[i].m_pResults->Release(); }
