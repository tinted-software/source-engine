...
private:
  ...
  HANDLE m_hMessageObj; // This signals to the thread that there's a message to
                        // send.
  HANDLE m_hMessageSentObj;   // This signals back to the main thread that the
                              // message was sent.
...
bool CMessageMgr::Init() {
  ...
  if (!m_hExitObj || !m_hExitResponseObj || !m_hMessageObj ||
      !m_hMessageSentObj)
    return false;
  ...
  m_pListenSocket = NULL;
  for (m_iListenPort = MSGMGR_LISTEN_PORT_FIRST;
       m_iListenPort <= MSGMGR_LISTEN_PORT_LAST; m_iListenPort++) {
  ...
  m_flLastBroadcast = 0;
  m_hThread = CreateThread(NULL, 0, &CMessageMgr::StaticThreadFn, this, 0,
                           &m_dwThreadID);
