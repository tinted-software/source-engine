...
private:
  ...
  bool InternalSend(void const *const *pChunks, const int *pChunkLengths,
    ...
    if (bPrependLength) {
      *((int *)pOut) =
          totalLength - 4; // The length we prepend is the size of the data, not
                           // data size + integer for length.
      pOut += 4;
    }
    for (int i = 0; i < nChunks; i++) {
...
private:
  ...
  int m_NextPacketLen; // Data is received INTO here before it receives each
                       // packet. This holds the length of each incoming packet.

  WSAOVERLAPPED m_RecvOverlapped;
  CEvent m_hRecvEvent;
  CTCPPacket *
      m_pRecvBuffer; // This is allocated for each packet we're receiving and
                     // given to the app when the packet is done being received.
...
public:
  ...
  static ITCPConnectSocket *Create(IHandlerCreator *pHandlerCreator,
    ...
    if (nQueueLength < 0) {
      Error("CTCPConnectSocket_Listener::Create - SOMAXCONN not allowed - "
            "causes some XP SP2 systems to stop receiving any network data "
            "(systemwide).");
    }
...
ITCPConnectSocket *
ThreadedTCP_CreateConnector(const CIPAddr &addr, const CIPAddr &localAddr,
                            IHandlerCreator *pHandlerCreator) {
