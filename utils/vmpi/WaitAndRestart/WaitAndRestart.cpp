...
int main(int argc, char *argv[]) {
  Sleep(5000);
  if (argc < 4) {
    PrintLog("WaitAndRestart <seconds to wait> <working directory> command "
             "line...\n");
    return 1;
  }

  PrintLog(
      "WaitAndRestart <seconds to wait> <working directory> command line...\n");

  const char *pTimeToWait = argv[1];
  const char *pWorkingDir = argv[2];

  // If a * precedes the time-to-wait arg, then it's a process ID and we wait
  // for that process to exit.
  if (pTimeToWait[0] == '*') {
    ++pTimeToWait;
    DWORD dwProcessId;
    ...
    if (hProcess) {
      while (1) {
        ...
        if (val == WAIT_OBJECT_0) {
          ...
          PrintLog("Got WAIT_ABANDONED (error). Waiting 5 seconds, then "
                   "continuing.\n");
          Sleep(5000);
          break;
        }
