...
//
//=============================================================================//
// ThreadedTCPSocketTest.cpp : Defines the entry point for the console
// application.
//
...
public:
  virtual ITCPSocketHandler *CreateNewHandler() { return new CHandler_Server; }
};

class CHandlerCreator_Client : public IHandlerCreator {
public:
  virtual ITCPSocketHandler *CreateNewHandler() { return new CHandler_Client; }
};
...
int main(int argc, char *argv[]) {
  ...
  ITCPConnectSocket *pListener =
      ThreadedTCP_CreateListener(&serverHandler, (unsigned short)iPort);

  // Setup the "client".
  CHandlerCreator_Client clientCreator;
  ITCPConnectSocket *pConnector = ThreadedTCP_CreateConnector(
      CIPAddr(127, 0, 0, 1, iPort), CIPAddr(), &clientCreator);
  ...
  while ((GetAsyncKeyState(VK_SHIFT) & 0x8000) == 0) {
    ...
    if (g_pClientSocket->Send(buf.Base(), size)) {
      // Server receives the data and echoes it back. Verify that the data is
      // good.
      ...
      Msg("Packet %d, %d bytes, %dk/sec\n", iPacket++, size,
          (int)(((totalBytes + 511) / 1024) / flSeconds));
    }
  }
