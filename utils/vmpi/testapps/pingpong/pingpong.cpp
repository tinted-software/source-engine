...
void DoServerConnect() {
    ...
    if (len > 3) {
      ...
      if (buf.ReadByte() == VMPI_PROTOCOL_VERSION) {
        if (buf.ReadByte() == VMPI_LOOKING_FOR_WORKERS) {
          ...
          _snprintf(ipString, sizeof(ipString), "%d.%d.%d.%d:%d", ipFrom.ip[0],
                    ipFrom.ip[1], ipFrom.ip[2], ipFrom.ip[3], iListenPort);
  ...
  CIPAddr addr;
  ITCPSocket *pTCPSocket =
      TCPSocket_ListenForOneConnection(pListen, &addr, 50000);
  if (!pTCPSocket) {
    assert(false);
  }
  pListen->Release();
...
void RecvData(CUtlVector<unsigned char> &recvBuf) {
  ...
  MPI_Recv(recvBuf.Base(), stat.count, MPI_BYTE, MPI_ANY_SOURCE, MPI_ANY_TAG,
           MPI_COMM_WORLD, &stat);
#else
...
int main(int argc, char *argv[]) {
  ...
  if (bClient) {
    ...
    // Ok, now start blasting packets of different sizes and measure how long it
    // takes to get an ack back.
    int nIterations = 30;
