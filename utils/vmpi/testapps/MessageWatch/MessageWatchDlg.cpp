...
void CMessageWatchDlg::OnIdle() {
  ...
  int iNext;
  for (int iSender = m_Senders.Head(); iSender != m_Senders.InvalidIndex();
       iSender = iNext) {
  ...
  while (1) {
    ...
    if (data[0] == MSGMGR_PACKETID_ANNOUNCE_PRESENCE &&
      ...
      // NOTE: we'll accept connections from machines we were connected to
      // earlier but lost the connection to.
      ...
      if (!pSender || !pSender->m_pSocket) {
        ...
        if (pNew->BindToAny(0) && TCPSocket_Connect(pNew, &connectAddr, 1000)) {
          ...
          if (!ConvertIPAddrToString(&ipFrom, nameStr, sizeof(nameStr)))
            Q_snprintf(nameStr, sizeof(nameStr), "%d.%d.%d.%d", ipFrom.ip[0],
                       ipFrom.ip[1], ipFrom.ip[2], ipFrom.ip[3]);
          ...
          if (!pSender) {
            ...
            IConsoleWnd *pWnd = CreateConsoleWnd(
                AfxGetInstanceHandle(), IDD_OUTPUT, IDC_DEBUG_OUTPUT, false);
...
void CMessageWatchDlg::OnShowall() {
  FOR_EACH_LL(m_Senders, i) { m_Senders[i]->m_pConsoleWnd->SetVisible(true); }
}

void CMessageWatchDlg::OnHideall() {
  FOR_EACH_LL(m_Senders, i) { m_Senders[i]->m_pConsoleWnd->SetVisible(false); }
}
