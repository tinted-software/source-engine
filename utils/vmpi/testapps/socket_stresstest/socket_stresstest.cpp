...
class CSocketInfo {
public:
  bool IsValid() { return m_pSocket != 0; }
    ...
    if (m_pSocket) {
      ...
      if (iConnectPort != -1) {
        ...
        for (int iTry = 0; iTry < 32; iTry++) {
          ...
          ITCPListenSocket *pListenSocket =
              CreateTCPListenSocket(m_iListenPort);
          if (pListenSocket) {
            Msg("%02d: listen on %d.\n", iInfo, m_iListenPort);
            ...
            m_pSocket = TCPSocket_ListenForOneConnection(pListenSocket,
                                                         &connectedAddr, 4.0);
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < ARRAYSIZE(g_Infos); i++) {
    DWORD dwThreadID = 0;
    g_hThreads[i] =
        CreateThread(NULL, 0, ThreadFn, &g_Infos[i], 0, &dwThreadID);
  }
