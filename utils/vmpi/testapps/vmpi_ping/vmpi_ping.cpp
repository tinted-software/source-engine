...
int main(int argc, char *argv[]) {
  ...
  printf(
      "    -patch <timeout>   ..  stops VMPI services for <timeout> seconds\n");
  printf("    -mpi_pw <password> ..  only talk to services with the specified "
         "password\n");
  printf("    -dns               ..  enable DNS lookups (slows the listing "
         "down)\n");
  printf("    -ShowConsole       ..  show the console window\n");
  printf("    -HideConsole       ..  hide the console window\n");

  // Scary to show these to users...
  // printf( "    -ShowCache         ..  show the cache directory and its
  // capacity\n" ); printf( "    -FlushCache        ..  flush the cache of ALL
  // VMPI services\n" );
  ...
  */
  ...
  while (!kbhit()) {
    ...
    while (1) {
      ...
      if (len >= 2 && in[0] == VMPI_PROTOCOL_VERSION &&
          in[1] == VMPI_PING_RESPONSE && addrs.Find(ipFrom) == -1) {
        ...
        if (FindArg(argc, argv, "-dns") &&
            ConvertIPAddrToString(&ipFrom, nameStr, sizeof(nameStr))) {
          printf("%02d. %s - %s:%d (%d.%d.%d.%d)", nMachines, pStateString,
                 nameStr, ipFrom.port, ipFrom.ip[0], ipFrom.ip[1], ipFrom.ip[2],
                 ipFrom.ip[3]);
        } else {
          printf("%02d. %s - %d.%d.%d.%d:%d", nMachines, pStateString,
