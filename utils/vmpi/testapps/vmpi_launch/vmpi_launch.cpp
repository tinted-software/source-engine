...
int PrintUsage() {
  printf("vmpi_launch -machine <remote machine> -priority <priority> [-mpi_pw "
         "<password>] -command \"command line...\"\n");
...
const char *FindArg(int argc, char **argv, const char *pName,
                    const char *pDefault) {
...
int ParseArgs(int argc, char **argv, CIPAddr &remoteIP, int &iPriority,
              int &iFirstArg) {
...
void SendJobRequest(ISocket *pSocket, int argc, char **argv, CIPAddr &remoteIP,
                    int &iPriority, int &iFirstArg, int jobID[4]) {
  ...
  packetBuf.WriteShort(
      0); // Tell the port that we're listening on.
          // In this case, there is no VMPI master waiting for the app to
          // connect, so this parameter doesn't matter.
  ...
  for (int iBroadcastPort = VMPI_SERVICE_PORT;
    ...
    if (g_bBroadcast == false)
      pSocket->SendTo(&remoteIP, packetBuf.GetBasePointer(),
                      packetBuf.GetNumBytesWritten());
    else
      pSocket->Broadcast(packetBuf.GetBasePointer(),
...
bool WaitForJobStart(ISocket *pSocket, const CIPAddr &remoteIP,
                     const int jobID[4]) {
  while (1) {
    CIPAddr senderAddr;
    char data[4096];
...
void WaitForJobEnd(ISocket *pSocket, const CIPAddr &remoteIP,
                   const int jobID[4]) {
  while (1) {
    CIPAddr senderAddr;
    char data[4096];
