...
class CServiceInfo {
public:
  bool IsOff() const; // Returns true if the time since we've heard from this
                      // guy is too long.
...
public:
  ...
  // Since the live time is always changing, we only update it every 10 seconds
  // or so.
  DWORD
      m_LiveTimeMS; // How long the service has been running (in milliseconds).

  DWORD m_WorkerAppTimeMS; // How long the worker app has been running (0 if
                           // it's not running).

  DWORD m_LastPingTimeMS; // Last time we heard from this machine. Used to
                          // detect if the service is off or not.
  ...
  // Last time we updated the service in the listbox.. used to make sure we
  // update its on/off status every once in a while.
...
protected:
  void
  BuildVMPIPingPacket(CUtlVector<char> &out, char cPacketID,
                      unsigned char cProtocolVersion = VMPI_PROTOCOL_VERSION,
                      bool bIgnorePassword = false);
