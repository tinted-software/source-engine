...
void CJobSearchDlg::OnDblclkJobsList(NMHDR *pNMHDR, LRESULT *pResult) {
  ...
  if (iItem != -1) {
    ...
    CString cmdLine;
    cmdLine.Format("vmpi_job_watch -JobID %d -dbname \"%s\" -hostname \"%s\" "
                   "-username \"%s\"",
                   pInfo->m_JobID, (const char *)m_DBName,
                   (const char *)m_HostName, (const char *)m_UserName);
...
void CJobSearchDlg::RepopulateJobsList() {
  // It's assumed coming into this routine that the caller just executed a query
  ...
  while (GetMySQL()->NextRow()) {
    ...
    pInfo->m_StartTimeUnformatted =
        GetMySQL()->GetColumnValue("StartTime").String();
    ...
    pInfo->m_RunningTimeMS =
        GetMySQL()->GetColumnValue("RunningTimeMS").Int32();
  ...
  FOR_EACH_LL(jobInfos, j) {
    ...
    {
      Q_snprintf(dateStr, sizeof(dateStr), "%c%c/%c%c %c%c:%c%c:00", pDate[4],
                 pDate[5], pDate[6], pDate[7], pDate[8], pDate[9], pDate[10],
                 pDate[11]);
    }
...
void CJobSearchDlg::OnDblclkUserList() {
  ...
  if (sel != LB_ERR) {
    ...
    Q_snprintf(query, sizeof(query),
               "select RunningTimeMS, JobID, BSPFilename, StartTime, "
               "MachineName from job_master_start where MachineName=\"%s\"",
...
void CJobSearchDlg::OnDblclkWorkerList() {
  ...
  if (sel != LB_ERR) {
    ...
    char query[4096];
    Q_snprintf(query, sizeof(query),
               "select job_master_start.RunningTimeMS, job_master_start.JobID, "
               "job_master_start.BSPFilename, job_master_start.StartTime, "
               "job_master_start.MachineName "
...
BOOL CJobSearchDlg::OnInitDialog() {
  ...
  if (pDBName && pHostName && pUserName) {
    ...
    if (!fp) {
      Q_snprintf(str, sizeof(str), "Can't open '%s' for database info.",
                 pFilename);
  ...
  if (!Sys_LoadInterface("mysql_wrapper", MYSQL_WRAPPER_VERSION_NAME,
                         &m_hMySQLDLL, (void **)&m_pSQL))
    return false;

  if (!m_pSQL->InitMySQL(m_DBName, m_HostName, m_UserName)) {
    Q_snprintf(str, sizeof(str),
               "Can't init MYSQL db (db = '%s', host = '%s', user = '%s')",
               (const char *)m_DBName, (const char *)m_HostName,
  ...
  for (int i = 0; i < ARRAYSIZE(titles); i++) {
    m_JobsList.InsertColumn(i, titles[i].pText, LVCFMT_LEFT, titles[i].width,
                            i);
  }
  ...
  while (1) {
    ...
    if (GetTickCount() - startTime > 5000) {
      Q_snprintf(str, sizeof(str),
                 "Unable to get computer names Can't init MYSQL db (db = '%s', "
                 "host = '%s', user = '%s')",
                 (const char *)m_DBName, (const char *)m_HostName,
                 (const char *)m_UserName);
      MessageBox(str, "Error", MB_OK);
  ...
  // Setup our anchors.
  m_AnchorMgr.AddAnchor(this, GetDlgItem(IDC_SEARCH_BY_USER_PANEL), ANCHOR_LEFT,
                        ANCHOR_TOP, ANCHOR_WIDTH_PERCENT,
                        ANCHOR_HEIGHT_PERCENT);
  m_AnchorMgr.AddAnchor(this, GetDlgItem(IDC_USER_LIST), ANCHOR_LEFT,
                        ANCHOR_TOP, ANCHOR_WIDTH_PERCENT,
                        ANCHOR_HEIGHT_PERCENT);

  m_AnchorMgr.AddAnchor(this, GetDlgItem(IDC_WORKER_PANEL),
                        ANCHOR_WIDTH_PERCENT, ANCHOR_TOP, ANCHOR_RIGHT,
                        ANCHOR_HEIGHT_PERCENT);
  m_AnchorMgr.AddAnchor(this, GetDlgItem(IDC_WORKER_LIST), ANCHOR_WIDTH_PERCENT,
                        ANCHOR_TOP, ANCHOR_RIGHT, ANCHOR_HEIGHT_PERCENT);

  m_AnchorMgr.AddAnchor(this, GetDlgItem(IDC_JOBS_PANEL), ANCHOR_LEFT,
                        ANCHOR_HEIGHT_PERCENT, ANCHOR_RIGHT, ANCHOR_BOTTOM);
  m_AnchorMgr.AddAnchor(this, GetDlgItem(IDC_JOBS_LIST), ANCHOR_LEFT,
                        ANCHOR_HEIGHT_PERCENT, ANCHOR_RIGHT, ANCHOR_BOTTOM);

  m_AnchorMgr.AddAnchor(this, GetDlgItem(IDC_QUIT), ANCHOR_WIDTH_PERCENT,
                        ANCHOR_BOTTOM, ANCHOR_WIDTH_PERCENT, ANCHOR_BOTTOM);
...
void CJobSearchDlg::OnQuit() { EndDialog(0); }
