...
bool CWindowAnchor::Init(CWnd *pParentWnd, CWnd *pChildWnd, int aLeft, int aTop,
...
void AnchorElement(long *pOut, CRect &rcParent, int startElement,
                   CRect &rcParentStart, int flags) {
  if (flags == ANCHOR_LEFT)
    ...
    *pOut = rcParent.left +
            (rcParent.Width() * (startElement - rcParentStart.left)) /
                rcParentStart.Width();
  else if (flags == ANCHOR_HEIGHT_PERCENT)
    *pOut = rcParent.top +
            (rcParent.Height() * (startElement - rcParentStart.top)) /
                rcParentStart.Height();
}
...
void CWindowAnchor::Update(CWnd *pParentWnd) {
  ...
  AnchorElement(&rcNew.bottom, rcParent, m_Rect.bottom, m_ParentRect,
                m_aBottom);

  pParentWnd->ScreenToClient(rcNew);
  m_pWnd->SetWindowPos(NULL, rcNew.left, rcNew.top, rcNew.Width(),
                       rcNew.Height(), SWP_NOZORDER);
...
bool CWindowAnchorMgr::AddAnchor(CWnd *pParentWnd, CWnd *pChildWnd, int aLeft,
                                 int aTop, int aRight, int aBottom) {
  int index = m_Anchors.AddToTail();
  return m_Anchors[index].Init(pParentWnd, pChildWnd, aLeft, aTop, aRight,
                               aBottom);
}

void CWindowAnchorMgr::UpdateAnchors(CWnd *pParentWnd) {
  FOR_EACH_LL(m_Anchors, i) { m_Anchors[i].Update(pParentWnd); }
}
