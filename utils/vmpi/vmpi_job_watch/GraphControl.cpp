...
void CGraphControl::Render(CDC *pDC) {
  ...
  // First, find which order of magnitude to use on the vertical scale by
  ...
  for (int x = 0; x < nIntervals; x++) {
    if (curTime >= 0) {
      ...
      if (curTime >= prevEntry.m_msTime && curTime <= curEntry.m_msTime) {
        ...
        int nBytesSent =
            (int)RemapVal(curTime, prevEntry.m_msTime, curEntry.m_msTime,
        ...
        int nBytesReceived = (int)RemapVal(
            curTime, prevEntry.m_msTime, curEntry.m_msTime,
...
ENDLOOP:;
  ...
  /*
          ...
          // Now find the value at the top of the graph. We choose the smallest
     enclosing tenth of the
          // order of magnitude we're at (so if we were at 1,000,000, and our
     max value was 350,000, we'd choose 400,000). int iTenth; int topValue; for
     ( iTenth=1; iTenth <= 10; iTenth++ )
          {
  ...
  */
  ...
  for (int iSample = 0; iSample < sentPoints.Count(); iSample++) {
    ...
    flHeight =
        ((double)sentPoints[iSample].y / topValue) * (rcClient.Height() - 1);
    sentPoints[iSample].y = (int)(rcClient.Height() - flHeight);

    flHeight = ((double)receivedPoints[iSample].y / topValue) *
               (rcClient.Height() - 1);
