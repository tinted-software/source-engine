...
static int CALLBACK SortByCurrentStage(LPARAM iItem1, LPARAM iItem2,
                                       LPARAM lpParam) {
  CWorkerInfo *pInfo1 = (CWorkerInfo *)iItem1;
  CWorkerInfo *pInfo2 = (CWorkerInfo *)iItem2;
...
BOOL CJobWatchDlg::OnInitDialog() {
  ...
  for (int i = 0; i < ARRAYSIZE(g_ColumnInfos); i++) {
    m_Workers.InsertColumn(i, g_ColumnInfos[i].pText, LVCFMT_LEFT,
  ...
  if (!pJobID) {
    str.Format("Missing a command line parameter (-JobID or -dbname or "
  ...
  if (!pDBName || !pHostName || !pUserName) {
    ...
    if (!fp) {
      Q_snprintf(errString, sizeof(errString),
                 "Can't open '%s' for database info.", pFilename);
    ...
    if (!ReadStringFromFile(fp, hostName, sizeof(hostName)) ||
      ...
      fclose(fp);
      Q_snprintf(errString, sizeof(errString), "'%s' has invalid format.",
                 pFilename);
      MessageBox(errString, "Error", MB_OK);
      EndDialog(0);
      return FALSE;
  ...
  if (!pSQL->InitMySQL(pDBName, pHostName, pUserName)) {
    pSQL->Release();
    str.Format("Can't init MYSQL db (db = '%s', host = '%s', user = '%s')",
               pDBName, pHostName, pUserName);
    MessageBox(str, "Error", MB_OK);
    EndDialog(0);
    return FALSE;
  }
  ...
  Q_snprintf(cmdLine, sizeof(cmdLine),
             "vmpi_job_watch -JobID %s -hostname %s -dbname %s -username %s",
             pJobID, pHostName, pDBName, pUserName);
  SetDlgItemText(IDC_COMMAND_LINE, cmdLine);

  // Setup anchors.
  m_AnchorMgr.AddAnchor(this, GetDlgItem(IDC_WORKERS_PANEL), ANCHOR_LEFT,
                        ANCHOR_TOP, ANCHOR_WIDTH_PERCENT,
                        ANCHOR_HEIGHT_PERCENT);
  m_AnchorMgr.AddAnchor(this, GetDlgItem(IDC_WORKERS), ANCHOR_LEFT, ANCHOR_TOP,
                        ANCHOR_WIDTH_PERCENT, ANCHOR_HEIGHT_PERCENT);

  m_AnchorMgr.AddAnchor(this, GetDlgItem(IDC_TEXT_OUTPUT_PANEL), ANCHOR_LEFT,
                        ANCHOR_HEIGHT_PERCENT, ANCHOR_WIDTH_PERCENT,
                        ANCHOR_BOTTOM);
  m_AnchorMgr.AddAnchor(this, GetDlgItem(IDC_TEXTOUTPUT), ANCHOR_LEFT,
                        ANCHOR_HEIGHT_PERCENT, ANCHOR_WIDTH_PERCENT,
                        ANCHOR_BOTTOM);

  m_AnchorMgr.AddAnchor(this, GetDlgItem(IDC_GRAPHS_PANEL),
                        ANCHOR_WIDTH_PERCENT, ANCHOR_TOP, ANCHOR_RIGHT,
                        ANCHOR_HEIGHT_PERCENT);
  m_AnchorMgr.AddAnchor(this, &m_GraphControl, ANCHOR_WIDTH_PERCENT, ANCHOR_TOP,
                        ANCHOR_RIGHT, ANCHOR_HEIGHT_PERCENT);
...
void CJobWatchDlg::UpdateWorkersList() {
  ...
  while ((nIndex = m_Workers.GetNextItem(nIndex, LVNI_ALL)) != -1) {
    ...
    SetWorkerListItemInt(nIndex, COLUMN_WORK_UNITS_DONE,
                         pInfo->m_nWorkUnitsDone);
  ...
  if (curTime - m_LastQueryTime >= 1000) {
    ...
    if (!m_bQueriesInProgress[QUERY_TEXT] && bJobWorkerIDValid) {
      Q_snprintf(query, sizeof(query),
                 "select * from text_messages where JobWorkerID=%lu and "
                 "MessageIndex >= %lu",
                 jobWorkerID, m_CurMessageIndex);
      m_pSQL->Execute(query,
                      (void *)(QUERY_TEXT | (m_CurWorkerTextToken << 16)));
      m_bQueriesInProgress[QUERY_TEXT] = true;
    }

    if (!m_bQueriesInProgress[QUERY_GRAPH] && bJobWorkerIDValid) {
      Q_snprintf(query, sizeof(query),
                 "select * from graph_entry where JobWorkerID=%lu",
                 jobWorkerID);
    ...
    if (!m_bQueriesInProgress[QUERY_WORKER_STATS]) {
      Q_snprintf(query, sizeof(query),
                 ...
                 "RunningTimeMS, CurrentStage, Thread0WU, Thread1WU, "
                 "Thread2WU, Thread3WU, IsMaster, MachineName "
...
void CJobWatchDlg::ProcessQueryResults_WorkerStats(IMySQLRowSet *pSet) {
  ...
  while (pSet->NextRow()) {
    ...
    if (pInfo) {
      ...
      if (bIsMaster) {
        char tempStr[512];
        Q_snprintf(tempStr, sizeof(tempStr), "%s [master]",
...
BOOL CJobWatchDlg::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT *pResult) {
  ...
  if (pHdr->idFrom == IDC_WORKERS) {
    if (pHdr->code == LVN_COLUMNCLICK) {
      ...
      g_iSortColumn =
          max(0, min(pListView->iSubItem, (int)ARRAYSIZE(g_ColumnInfos) - 1));
      ResortItems();
    }
  }
...
void CJobWatchDlg::OnOdstatechangedWorkers(NMHDR *pNMHDR, LRESULT *pResult) {
  ...
  if (!(pStateChanged->uOldState & LVIS_SELECTED) &&
      (pStateChanged->uNewState & LVIS_SELECTED)) {
    OnSelChangeWorkers();
  }
