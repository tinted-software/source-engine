  ...
  if (VMPI_GetFileSystemMode() == VMPI_FILESYSTEM_TCP) {
    ...
    if (VMPI_GetFileSystemMode() == VMPI_FILESYSTEM_BROADCAST) {
      ...
      if (setsockopt(m_Socket, SOL_SOCKET, SO_BROADCAST, (char *)&bBroadcast,
        ...
        Term();
        Warning("CMasterMulticastThread::Init - setsockopt() failed to set "
                "broadcast mode\n");
        return false;
      }
    }
...
void CMasterMulticastThread::OnChunkReceived(int fileID, int clientID,
  ...
  if (!m_Files.IsValidIndex(fileID)) {
    Warning("CMasterMulticastThread::OnChunkReceived: invalid file (%d) from "
            "client %d\n",
            fileID, clientID);
    return;
  }
  ...
  if (VMPI_GetFileSystemMode() == VMPI_FILESYSTEM_TCP) {
    ...
    if (!pFile->m_Chunks.IsValidIndex(iChunk)) {
      Warning("CMasterMulticastThread::OnChunkReceived: invalid chunk index "
              "(%d) for file %s\n",
...
void CMasterMulticastThread::CreateVirtualFile(const char *pFilename,
  ...
  if (iFile != -1)
    Error("CMasterMulticastThread::CreateVirtualFile( %s ) - file already "
          "exists!",
          pFilename);
...
inline bool
CMasterMulticastThread::Thread_SendFileChunk_Multicast(int *pnBytesSent) {
  ...
  if (bSuccess) {
    *pnBytesSent = (int)nBytesSent;
    ...
    if (nWarnings < 10) {
      ...
      Warning("\nThis machine's ability to multicast may be broken. Please "
...
void CMasterMulticastThread::DecrementChunkRefCount(int iFile, int iChunk) {
  ...
  if (pChunk->m_RefCount == 0) {
    Error("CMasterMulticastThread::DecrementChunkRefCount - refcount already "
          "zero!\n");
  }
