...
public:
  void Init(WUIndexType nWorkUnits, IShuffleRequester *pRequester) {
    ...
    for (WUIndexType iWU = 0; iWU < nWorkUnits; iWU++) {
      ...
      if (index != iWU) {
        Error("CShuffledWorkUnitWalker: assumption on CUtlLinkedList indexing "
              "failed.\n");
      }
    }
  }
...
private:
  ...
  CUtlVector<WUIndexType>
      m_WorkUnitsMap; // Maps the 0-N indices in the CWorkUnitWalker to the list
                      // of remaining work units.
  ...
  unsigned int m_iLastShuffleRequest; // The index of the shuffle we last
                                      // requested a reshuffle on (don't request
                                      // a reshuffle on the same one).
...
public:
  ...
  class CWorkersReady {
  public:
    CUtlVector<int> m_WorkersReady; // The list of workers who have said they're
