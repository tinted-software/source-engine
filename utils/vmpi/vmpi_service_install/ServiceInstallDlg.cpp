...
char *g_pInstallFiles[] = {
    "vmpi_service.exe",         "vmpi_service_ui.exe", "WaitAndRestart.exe",
...
void ScanDirectory(const char *pDirName, CUtlVector<CString> &subDirs,
...
int DeleteDirectory(const char *pRootDir, bool bDeleteSubdirectories,
...
bool CreateDirectory_R(const char *pDirName) {
  ...
  return CreateDirectory(pDirName, NULL) ||
         GetLastError() == ERROR_ALREADY_EXISTS;
}

bool SetupStartMenuSubFolderName(const char *pSubFolderName, char *pOut,
                                 int outLen) {
  LPITEMIDLIST pidl;

  // Get a pointer to an item ID list that represents the path of a special
  // folder
...
bool CreateStartMenuLink(const char *pSubFolderName, const char *pLinkName,
                         const char *pLinkTarget, const char *pArguments) {
  char fullFolderName[MAX_PATH];
  if (!SetupStartMenuSubFolderName(pSubFolderName, fullFolderName,
                                   sizeof(fullFolderName)))
    return false;
  ...
  if (SUCCEEDED(hres)) {
    ...
    if (SUCCEEDED(hres)) {
      ...
      V_ComposeFileName(fullFolderName, pLinkName, linkFilename,
                        sizeof(linkFilename));
...
bool LaunchApp(char *pCommandLine, const char *pBaseDir) {
  ...
  return CreateProcess(NULL, pCommandLine, NULL, NULL, FALSE, 0, NULL, pBaseDir,
                       &si, &pi) != 0;
}
...
bool StartVMPIServiceUI(const char *pInstallLocation) {
  ...
  V_ComposeFileName(pInstallLocation, "vmpi_service_ui.exe", cmdLine,
                    sizeof(cmdLine));
...
bool StartVMPIService(SC_HANDLE hSCManager) {
  ...
  SC_HANDLE hMyService =
      OpenService(hSCManager, VMPI_SERVICE_NAME_INTERNAL, SERVICE_START);
  if (hMyService) {
...
bool StopRunningApp() {
  if (g_bDontTouchUI) {
    Msg("StopRunningApp: -DontTouchUI was specified, so exiting before "
        "stopping the app.\n");
    return true;
  }
  ...
  if (pSocket) {
    if (pSocket->BindToAny(0)) {
      ...
      if (VMPI_PROTOCOL_VERSION == 5)
        protocolVersions.AddToTail(
            4); // We want this installer to kill the previous services too.

      for (int iProtocolVersion = 0;
           iProtocolVersion < protocolVersions.Count(); iProtocolVersion++) {
        char cPacket[4] = {
        ...
        for (int iPort = VMPI_SERVICE_PORT; iPort <= VMPI_LAST_SERVICE_PORT;
             iPort++) {
          addr.port = iPort;
      ...
      if (hInst) {
        typedef BOOL(WINAPI * EnumProcessesFn)(DWORD * lpidProcess, DWORD cb,
                                               DWORD * cbNeeded);
        typedef BOOL(WINAPI * EnumProcessModulesFn)(
            HANDLE hProcess, HMODULE * lphModule, DWORD cb, LPDWORD lpcbNeeded);
        typedef DWORD(WINAPI * GetModuleBaseNameFn)(
            HANDLE hProcess, HMODULE hModule, LPTSTR lpBaseName, DWORD nSize);

        EnumProcessesFn EnumProcesses =
            (EnumProcessesFn)GetProcAddress(hInst, "EnumProcesses");
          ...
          if (EnumProcesses(procIDs, sizeof(procIDs), &nBytes)) {
            ...
            for (DWORD i = 0; i < nProcs; i++) {
              ...
              if (hProc) {
                ...
                if (EnumProcessModules(hProc, hModules, sizeof(hModules),
                                       &nBytes)) {
                  DWORD nModules = nBytes / sizeof(hModules[0]);
                  for (DWORD iModule = 0; iModule < nModules; iModule++) {
                    char filename[512];
                    if (GetModuleBaseName(hProc, hModules[iModule], filename,
                                          sizeof(filename))) {
...
bool StopOrDeleteService(SC_HANDLE hSCManager, bool bDelete) {
  ...
  // First, get rid of an old service with the same name.
  SC_HANDLE hOldService = OpenService(hSCManager, VMPI_SERVICE_NAME_INTERNAL,
                                      SERVICE_STOP | DELETE);
  if (hOldService) {
    // Stop the service.
...
bool GetExistingInstallationLocation(CString &strInstallLocation) {
  ...
  DWORD dwType;
  if (RegQueryValueEx(g_hVMPIKey, SERVICE_INSTALL_LOCATION_KEY, NULL, &dwType,
...
BOOL CServiceInstallDlg::OnInitDialog() {
  ...
  if (RegCreateKey(HKEY_LOCAL_MACHINE, VMPI_SERVICE_KEY, &g_hVMPIKey) !=
      ERROR_SUCCESS) {
...
bool InstallService(SC_HANDLE hSCManager, const char *pBaseDir) {
  ...
  V_ComposeFileName(pBaseDir, "vmpi_service_ui.exe", uiFilename,
                    sizeof(uiFilename));
  ...
  if (!hUIKey || RegSetValueEx(hUIKey, VMPI_SERVICE_UI_VALUE_NAME, 0, REG_SZ,
                               (unsigned char *)uiFilename,
...
void SetupStartMenuLinks(const char *pInstallerFilename) {
  CreateStartMenuLink("Valve\\VMPI", "Start VMPI Service", pInstallerFilename,
                      "-start");
  CreateStartMenuLink("Valve\\VMPI", "Stop VMPI Service", pInstallerFilename,
                      "-stop");
  CreateStartMenuLink("Valve\\VMPI", "Uninstall VMPI", pInstallerFilename,
                      NULL);
}

void RemoveStartMenuLinks() {
  char fullFolderName[MAX_PATH];
  if (!SetupStartMenuSubFolderName("Valve\\VMPI", fullFolderName,
                                   sizeof(fullFolderName)))
    return;
...
void CServiceInstallDlg::OnInstall() {
  ...
  for (int i = 0; i < ARRAYSIZE(g_pInstallFiles); i++) {
    ...
    V_ComposeFileName(chDir, g_pInstallFiles[i], srcFilename,
                      sizeof(srcFilename));
    V_ComposeFileName(strInstallLocation, g_pInstallFiles[i], destFilename,
                      sizeof(destFilename));
    ...
    if (!CopyFile(srcFilename, destFilename, FALSE)) {
      ...
      if (!CopyFile(srcFilename, destFilename, FALSE)) {
        Error("CopyFile() failed.\nSrc: %s\nDest: %s\n%s", srcFilename,
              destFilename, GetLastErrorString());
        return;
      }
    }
  }
  ...
  if (RegSetValueEx(g_hVMPIKey, SERVICE_INSTALL_LOCATION_KEY, 0, REG_SZ,
                    (BYTE *)(const char *)strInstallLocation,
                    V_strlen(strInstallLocation) + 1) != ERROR_SUCCESS) {
    Error("RegSetValueEx( %s, %s ) failed.", SERVICE_INSTALL_LOCATION_KEY,
          (const char *)strInstallLocation);
    return;
  }
  ...
  V_ComposeFileName(strInstallLocation, "vmpi_service_install.exe",
                    installerFilename, sizeof(installerFilename));
...
bool CServiceInstallDlg::DoUninstall(bool bShowMessage) {
  ...
  if (_access(strInstallLocation, 0) == 0 && !g_bNoOutput) {
    // Don't ask if they care if we delete all the files in that directory if
    // the only exes in there are the install exes.
    if (AnyNonInstallFilesInDirectory(strInstallLocation)) {
      char str[512];
      V_snprintf(str, sizeof(str),
                 "Warning: this will delete all files under this directory: "
                 "\n%s\nContinue?",
                 strInstallLocation);
...
bool CServiceInstallDlg::NukeDirectory(const char *pDir,
                                       char errorFile[MAX_PATH]) {
...
void CServiceInstallDlg::VerifyInstallFiles() {
  ...
  for (int i = 0; i < ARRAYSIZE(g_pInstallFiles); i++) {
    ...
    if (_access(filename, 0) != 0) {
      ...
      V_snprintf(szErrorMessage, sizeof(szErrorMessage),
                 "Required installation file missing: %s", filename);
