...
public:
  ...
  virtual void Init(unsigned long dwProcessID) {
    ...
    if (GetInstanceNameFromProcessID(m_dwProcessID, instanceName,
                                     sizeof(instanceName))) {
      // Create a counter to watch this process' time.
      char str[512];
      V_snprintf(str, sizeof(str), "\\Process(%s)\\%% Processor Time",
                 instanceName);
      if (PdhAddCounter(m_hQuery, str, 0, &m_hProcessorTimeCounter) !=
          ERROR_SUCCESS) {
        m_hProcessorTimeCounter = NULL;
      }

      V_snprintf(str, sizeof(str), "\\Process(%s)\\Private Bytes",
                 instanceName);
      if (PdhAddCounter(m_hQuery, str, 0, &m_hPrivateBytesCounter) !=
          ERROR_SUCCESS) {
  ...
  virtual unsigned long GetProcessID() { return m_dwProcessID; }

  virtual void GetPerfData(int &processorPercentage,
    ...
    if (PdhGetFormattedCounterValue(m_hProcessorTimeCounter,
                                    PDH_FMT_LONG | PDH_FMT_NOCAP100, &dwType,
    ...
    // Check memory usage.
    if (PdhGetFormattedCounterValue(m_hPrivateBytesCounter,
                                    PDH_FMT_DOUBLE | PDH_FMT_NOCAP100, &dwType,
                                    &counterValue) == ERROR_SUCCESS)
      memoryUsageMegabytes =
          (int)(counterValue.doubleValue / (1024.0 * 1024.0));
    else
      memoryUsageMegabytes = 0;
  }

private:
  bool GetInstanceNameFromProcessID(DWORD processID, char *instanceName,
                                    int instanceNameLen) {
    instanceName[0] = 0;

    bool bRet = false;

    // This refreshes the object list. If we don't do this, it won't get new
    // process IDs correctly.
    DWORD dummy = 0;
    ...
    PDH_STATUS stat = PdhEnumObjectItems(
        NULL, NULL, "Process", counterList, &counterListLen, instanceList,
    ...
    if (stat == PDH_MORE_DATA) {
      ...
      stat = PdhEnumObjectItems(NULL, NULL, "Process", counterList,
                                &counterListLen, instanceList, &instanceListLen,
                                PERF_DETAIL_NOVICE, 0);
      if (stat == ERROR_SUCCESS) {
        // We need the # of each one..
        CUtlDict<int, int> counts;

        // The instance name list is a bunch of strings terminated with nulls.
        // The final one has two nulls after it. Walk through the list and get
        // the process ID associated with each instance name.
        ...
        // Each instance (like "vrad") might have multiple versions, like if
        // you're running multiple vrad processes at the same time.
        for (int i = counts.First(); i != counts.InvalidIndex();
             i = counts.Next(i)) {
          ...
          for (int iInstance = 0; iInstance < nInstances; iInstance++) {
            ...
            V_snprintf(testInstanceName, sizeof(testInstanceName), "%s#%d",
                       pInstanceName, iInstance);
            V_snprintf(fullObjectName, sizeof(fullObjectName),
                       "\\Process(%s)\\ID Process", testInstanceName);
            ...
            if (stat == ERROR_SUCCESS) {
              ...
              if (stat == ERROR_SUCCESS) {
                DWORD dwType;
                PDH_FMT_COUNTERVALUE counterValue;
                stat = PdhGetFormattedCounterValue(hCounter, PDH_FMT_LONG,
                                                   &dwType, &counterValue);
...
IPerfTracker *CreatePerfTracker() { return new CPerfTracker; }
...
#define PROCESSOR_TIME_COUNTER_INDEX                                           \
  6 // '% processor time' counter (for Win2K/XP)
...
//	Instead, calling the registry functions with the HKEY_PEFORMANCE_DATA
//key 	causes the system to collect the data from the appropriate system 	object
//managers.
//
...
void QueryPerformanceData(PERF_DATA_BLOCK **pPerfData, DWORD dwObjectIndex,
  ...
  while ((lRes = RegQueryValueEx(HKEY_PERFORMANCE_DATA, keyName, NULL, NULL,
                                 (LPBYTE)Buffer.Base(), &BufferSize)) ==
         ERROR_MORE_DATA) {
...
inline PPERF_INSTANCE_DEFINITION
...
template <class T>
T GetCounterValueForProcessID(PPERF_OBJECT_TYPE pPerfObj, DWORD dwCounterIndex,
                              DWORD dwProcessID) {
  ...
  if (pPerfObj->NumInstances == PERF_NO_INSTANCES) {
    pCounterBlock =
        (PPERF_COUNTER_BLOCK)((LPBYTE)pPerfObj + pPerfObj->DefinitionLength);
  } else {
    pPerfInst = FirstInstance(pPerfObj);

    for (int k = 0; k < pPerfObj->NumInstances; k++) {
      pCounterBlock =
          (PPERF_COUNTER_BLOCK)((LPBYTE)pPerfInst + pPerfInst->ByteLength);
      if (pCounterBlock) {
        DWORD processID =
            *(DWORD *)((LPBYTE)pCounterBlock + pProcIDPerfCntr->CounterOffset);
  ...
  if (bProcessIDExist && pCounterBlock) {
    T *lnValue = NULL;
    lnValue =
        (T *)((LPBYTE)pCounterBlock + pTheRequestedPerfCntr->CounterOffset);
    return *lnValue;
  }
  return -1;
}
...
template <class T>
T GetCounterValueForProcessID(PERF_DATA_BLOCK **pPerfData, DWORD dwObjectIndex,
  ...
  for (DWORD i = 0; i < (*pPerfData)->NumObjectTypes; i++) {
    ...
    if (pPerfObj->ObjectNameTitleIndex == dwObjectIndex) {
      lnValue =
          GetCounterValueForProcessID<T>(pPerfObj, dwCounterIndex, dwProcessID);
      break;
    }
...
unsigned long CPerfTracker::GetProcessID() { return m_dwProcessID; }
...
int CPerfTracker::GetCPUPercentage() {
  ...
  LONGLONG lnNewValue = GetCounterValueForProcessID<LONGLONG>(
      &pPerfData, dwObjectIndex, dwCpuUsageIndex, m_dwProcessID);
  ...
  double DeltaPerfTime100nSec = (double)NewPerfTime100nSec.QuadPart -
                                (double)m_OldPerfTime100nSec.QuadPart;
