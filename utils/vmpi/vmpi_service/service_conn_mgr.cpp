...
bool CServiceConnMgr::InitServer() {
  ...
  for (int iPort = VMPI_SERVICE_FIRST_UI_PORT;
       iPort <= VMPI_SERVICE_LAST_UI_PORT; iPort++) {
...
void CServiceConnMgr::Update() {
  ...
  if (m_bServer) {
    ...
    if (!IsConnected() && curTime - m_LastConnectAttemptTime >=
                              SERVICECONNMGR_CONNECT_ATTEMPT_INTERVAL) {
      AttemptConnect();
    }
  }
  ...
  int iNext;
  for (int iCur = m_Connections.Head(); iCur != m_Connections.InvalidIndex();
       iCur = iNext) {
