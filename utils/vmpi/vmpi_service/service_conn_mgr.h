...
// ------------------------------------------------------------------------------------------
// // CServiceConnMgr. This class manages connections to all the UIs (there
// should only be one UI at any given time, but it's conceivable that multiple
// people can be logged into NT servers simultaneously).
// ------------------------------------------------------------------------------------------
// //
...
public:
  ...
  // Returns true if there are any connections. If you used InitClient() and
  // there are no connections, it will continuously try to connect with a
  // server.
  bool IsConnected();

  // This should be called as often as possible. It checks for dead connections
  // and it handles incoming packets from UIs.
  void Update();
