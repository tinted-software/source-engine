...
// If we couldn't get into a job (maybe they weren't accepting more workers at
// the time), then we wait this long and retry the connection.
...
bool g_bScreensaverMode =
    false; // If this is true, then it'll act like the service is disabled while
           // a screensaver isn't running.

// If this is on, it runs the exes out of c:/hl2/bin instead of the network. If
// the exes are built in debug, this makes it possible to catch nasty crashes.
bool g_bSuperDebugMode = false;

bool g_bScreensaverRunning =
    false; // Updated each frame to tell if the screensaver is running.
...
#ifdef _DEBUG
#define MAX_DOWNLOADER_TIME_ALLOWED                                            \
  300000 // If the downloader takes longer than this, kill it.
#else
#define MAX_DOWNLOADER_TIME_ALLOWED                                            \
  30 // If the downloader takes longer than this, kill it.
#endif

// If this is non-NULL, then there is NOT a VMPI worker app running currently..
// the downloader
...
bool g_Waiting_bPatching =
    0; // If this is nonzero, then we're downloading so we can apply a patch.

// Used to track the services browsers that have been talking to us lately.
#define SERVICES_BROWSER_TIMEOUT                                               \
  10 // We remove a services browser from the list if we don't hear from it for
     // this long.
...
void LoadStateFromRegistry() {
  if (g_hVMPIServiceKey) {
    ...
    if (RegQueryValueEx(g_hVMPIServiceKey, "ScreensaverMode", 0, &type,
                        (unsigned char *)&val, &size) == ERROR_SUCCESS &&
        type == REG_DWORD && size == sizeof(val)) {
      g_bScreensaverMode = (val != 0);
    }

    if (RegQueryValueEx(g_hVMPIServiceKey, "Disabled", 0, &type,
                        (unsigned char *)&val, &size) == ERROR_SUCCESS &&
...
void SaveStateToRegistry() {
  if (g_hVMPIServiceKey) {
    ...
    RegSetValueEx(g_hVMPIServiceKey, "ScreensaverMode", 0, REG_DWORD,
                  (unsigned char *)&val, sizeof(val));

    val = (g_iCurState == VMPI_SERVICE_STATE_DISABLED);
    RegSetValueEx(g_hVMPIServiceKey, "Disabled", 0, REG_DWORD,
                  (unsigned char *)&val, sizeof(val));
  }
}
...
char *FindArg(int argc, char **argv, const char *pArgName,
              char *pDefaultValue = "") {
...
// We remember which jobs we did because Winsock likes to queue up the job
// packets on our socket, so if we don't remember which jobs we ran, we'd run
// the job a bunch of times.
class CJobMemory {
public:
...
bool FindJobMemory(int id[4]) {
  int iNext;
  for (int i = g_JobMemories.Head(); i != g_JobMemories.InvalidIndex();
       i = iNext) {
...
bool VMPI_Waiter_Init() {
  ...
  if (RegQueryValueEx(hKey, "LowPriority", NULL, &dummyType, (LPBYTE)&dwVal,
                      &dwValLen) == ERROR_SUCCESS) {
    if (dwVal) {
      SetPriorityClass(GetCurrentProcess(), IDLE_PRIORITY_CLASS);
    }
  } else {
    RegSetValueEx(hKey, "LowPriority", 0, REG_DWORD, (unsigned char *)&dwVal,
                  sizeof(dwVal));
  }
  ...
  // Bind to the first port we find in the range [VMPI_SERVICE_PORT,
  // VMPI_LAST_SERVICE_PORT].
  int iTest;
...
void RunInDLL(const char *pFilename, CUtlVector<char *> &newArgv) {
  ...
  if (pModule) {
    ...
    if (fn) {
      ILaunchableDLL *pDLL =
          (ILaunchableDLL *)fn(LAUNCHABLE_DLL_INTERFACE_VERSION, NULL);
      if (pDLL) {
        // Do this here because the executables we would have launched usually
        // would do it.
...
void GetArgsFromBuffer(bf_read &buf, CUtlVector<char *> &newArgv,
                       bool *bShowAppWindow) {
...
void BuildCommandLineFromArgs(CUtlVector<char *> &newArgv, char *pOut,
                              int outLen) {
  pOut[0] = 0;
...
bool RunProcessFromArgs(CUtlVector<char *> &newArgv, bool bShowAppWindow,
                        bool bCreateSuspended, const char *pWorkingDir,
                        PROCESS_INFORMATION *pOut) {
...
void RunProcessAtCommandLine(CUtlVector<char *> &newArgv, bool bShowAppWindow,
                             bool bCreateSuspended, int iPriority) {
  ...
  if (RunProcessFromArgs(newArgv, bShowAppWindow, bCreateSuspended,
                         g_FileCachePath, &pi)) {
    if (g_pConnMgr)
      g_pConnMgr->SetAppState(VMPI_SERVICE_STATE_BUSY);

    if (newArgv.Count() > 0 && newArgv[0]) {
      V_FileBase(newArgv[0], g_RunningProcess_ExeName,
                 sizeof(g_RunningProcess_ExeName));

      if (V_stricmp(g_RunningProcess_ExeName, "vrad") == 0 ||
          V_stricmp(g_RunningProcess_ExeName, "vvis") == 0)
        V_FileBase(newArgv[newArgv.Count() - 1], g_RunningProcess_MapName,
                   sizeof(g_RunningProcess_MapName));
    }
...
void GetRunningProcessStats(int &processorPercentage,
                            int &memoryUsageMegabytes) {
  static int lastProcessorPercentage = 0;
  static int lastMemory = 0;
  if (g_hRunningProcess) {
    // Only update this every couple seconds. It's not too expensive (about 800
    // microseconds), but we don't need to do it a whole lot.
...
void BuildPingHeader(CUtlVector<char> &data, char packetID, int iState) {
  ...
  // Write the EXE name.
  data.AddMultipleToTail(V_strlen(g_RunningProcess_ExeName) + 1,
                         g_RunningProcess_ExeName);
  ...
  // Write the map name.
  data.AddMultipleToTail(V_strlen(g_RunningProcess_MapName) + 1,
                         g_RunningProcess_MapName);
}
...
void UpdateServicesBrowserIPs() {
  ...
  for (int i = 0; i < g_ServicesBrowsers.Count(); i++) {
    if ((curTime - g_ServicesBrowsers[i].m_flLastPingTime) >=
        SERVICES_BROWSER_TIMEOUT) {
...
// --------------------------------------------------------------------------------
// // Purpose: Launches vmpi_transfer.exe to download the required files from
// the master so we can launch.
//
// If successful, it sets hProcess to the process handle of the downloader.
// When that process terminates, we look for [cache dir]\ReadyToGo.txt and if
// it's there, then we start the job.
// --------------------------------------------------------------------------------
// //
bool StartDownloadingAppFiles(CUtlVector<char *> &newArgv, char *cacheDir,
                              int cacheDirLen, bool bShowAppWindow,
  ...
  *hProcess = NULL;
  ...
  if (ret != -1) {
    do {
      ...
      V_ComposeFileName(cacheDir, findData.name, fullFilename,
                        sizeof(fullFilename));
  ...
  // Change the EXE name to an absolute path to exactly where it is in the cache
  // directory.
  int maxExeNameLen = 1024;
  char *pExeName = new char[maxExeNameLen];
  if (bPatching) {
    V_ComposeFileName(cacheDir, "vmpi_service_install.exe", pExeName,
                      maxExeNameLen);

    // Add args for the installer.
    newArgv.InsertAfter(0, CopyString("-DontTouchUI"));

    // When patching, we can't start the UI and the installer can't because
    // we're running in the local system account and the UI is running on the
    // account of whoever logged in. So what we do is send a message to the UI
    // telling it to run <cacheDir>\WaitAndRestart and restart itself in N
    // seconds.
  ...
  V_ComposeFileName(g_BaseAppPath, "vmpi_transfer.exe", fullExeFilename,
                    sizeof(fullExeFilename));
...
#endif
  downloaderArgs.AddToTail(
      "-CachePath"); // Tell it where to download the files to.
  downloaderArgs.AddToTail(cacheDir);

  // Pass all the -mpi_worker, -mpi_file, -mpi_filebase args into the downloader
  // app.
  for (int i = 1; i < (int)newArgv.Count() - 1; i++) {
    if (V_stricmp(newArgv[i], "-mpi_filebase") == 0 ||
        V_stricmp(newArgv[i], "-mpi_file") == 0) {
      ...
      // We need this arg so it knows what IP to connect to, but we want to
      // leave it in the final launch args too.
      downloaderArgs.AddToTail(newArgv[i]);
      downloaderArgs.AddToTail(newArgv[i + 1]);
      ++i;
    }
  ...
  // Transfer each file.
  PROCESS_INFORMATION pi;
  if (!RunProcessFromArgs(downloaderArgs, bShowAppWindow, false, g_BaseAppPath,
                          &pi))
    return false;
...
void SendPatchCommandToUIs(DWORD dwInstallerProcessId) {
  ...
  // First argument is the working directory, which is the cache path in this
  // case.
  data.AddMultipleToTail(V_strlen(g_FileCachePath) + 1, g_FileCachePath);

  // Second argument is the command line.
  char waitAndRestartExe[MAX_PATH], serviceUIExe[MAX_PATH],
      commandLine[1024 * 8];
  V_ComposeFileName(g_FileCachePath, "WaitAndRestart.exe", waitAndRestartExe,
                    sizeof(waitAndRestartExe));
  V_ComposeFileName(g_BaseAppPath, "vmpi_service_ui.exe", serviceUIExe,
                    sizeof(serviceUIExe)); // We're running the UI from the same
                                           // directory this exe is in.
  char strSeconds[64];
  V_snprintf(strSeconds, sizeof(strSeconds), "*%lu", dwInstallerProcessId);

  // IMPORTANT to use BuildCommandLineFromArgs here because it'll handle slashes
  // and quotes correctly. If we don't do that, the command often won't work.
...
bool CheckDownloaderFinished() {
  ...
  V_ComposeFileName(g_FileCachePath, "ReadyToGo.txt", testFilename,
                    sizeof(testFilename));
  ...
  for (int iArg = 1; iArg < g_Waiting_Argv.Count() - 1; iArg++) {
    if (stricmp(g_Waiting_Argv[iArg], "-mpi_MasterName") == 0) {
      Q_strncpy(g_CurMasterName, g_Waiting_Argv[iArg + 1],
                sizeof(g_CurMasterName));
    }
  }
  ...
  if (FindArg(__argc, __argv, "-TryDLLMode") && g_RunMode == RUNMODE_CONSOLE &&
    ...
    RunProcessAtCommandLine(g_Waiting_Argv, g_Waiting_bShowAppWindow,
                            g_Waiting_bPatching, g_Waiting_Priority);
    ...
    if (g_Waiting_bPatching) {
      ...
      ResumeThread(
          g_hRunningThread); // We started the installer suspended so we could
                             // make sure we'd send out the patch command.

      // We just ran the installer, but let's forget about it, otherwise we'll
      // kill its process when we exit here.
      CloseHandle(g_hRunningProcess);
      CloseHandle(g_hRunningThread);
      g_hRunningProcess = g_hRunningThread = NULL;
      g_RunningProcess_ExeName[0] = 0;
...
void HandlePacket_LOOKING_FOR_WORKERS(bf_read &buf, const CIPAddr &ipFrom) {
  // If we're downloading files for a job request, don't process any more
  // "looking for workers" packets.
  if (g_Waiting_hProcess)
    return;
  ...
  V_snprintf(strDownloaderIP, sizeof(strDownloaderIP), "%d.%d.%d.%d:%d",
             ipFrom.ip[0], ipFrom.ip[1], ipFrom.ip[2], ipFrom.ip[3],
             iDownloaderPort);
  V_snprintf(strMainIP, sizeof(strMainIP), "%d.%d.%d.%d:%d", ipFrom.ip[0],
             ipFrom.ip[1], ipFrom.ip[2], ipFrom.ip[3], iPort);
  ...
  if (versionString[0] != 0) {
    ...
    // Check that we haven't applied this patch version yet. This case usually
    // happens right after we've applied a patch and we're restarting. The
    // vmpi_transfer master is still pinging us telling us to patch, but we
    // don't want to reapply this patch.
  ...
  // If there's already a job running, only interrupt it if this new one has a
  // higher priority.
  if (WaitForProcessToExit()) {
    if (iPriority > g_CurJobPriority) {
      KillRunningProcess("Interrupted by a higher priority process", true);
    } else {
      // This means we're already running a job with equal to or greater
      // priority than the one that has been requested. We're going to ignore
      // this request.
      newArgv.PurgeAndDeleteElements();
      return;
    }
  ...
  if (!g_Waiting_bShowAppWindow &&
      FindArg(__argc, __argv, "-mpi_ShowAppWindow"))
    g_Waiting_bShowAppWindow = true;

  // Copy all the files from the master and put them in our cache dir to run
  // with.
  char cacheDir[MAX_PATH];
  if (StartDownloadingAppFiles(newArgv, cacheDir, sizeof(cacheDir),
                               g_Waiting_bShowAppWindow, &g_Waiting_hProcess,
                               bPatching)) {
...
void HandlePacket_KILL_PROCESS(const CIPAddr *ipFrom) {
  if (Plat_FloatTime() - g_flLastKillProcessTime > 5) {
    KillRunningProcess(
        "Got a KILL_PROCESS packet. Stopping the worker executable.\n", true);
...
void VMPI_Waiter_Update() {
  ...
  while (1) {
    ...
    // Recv off the socket first so it clears the queue while we're waiting for
    // the process to exit.
    char data[4096];
    CIPAddr ipFrom;
    ...
    if (pwString[0] == VMPI_PASSWORD_OVERRIDE) {
      // Always process these packets regardless of the password (these usually
      // come from the installer when it is trying to stop a previously-running
      // instance).
    ...
    if (packetID == VMPI_KILL_PROCESS) {
      ...
      // The key to doing this here is that we ignore whether we're disabled or
      // in screensaver mode.. we always handle the patch command (unless we've
      // already handled this job ID OR if we've already applied this patch
      // version).
    ...
    if (g_iCurState == VMPI_SERVICE_STATE_DISABLED ||
        (g_bScreensaverMode && !g_bScreensaverRunning))
      continue;
...
// This function runs us as a console app. Useful for debugging or if you want
// to run more than one instance of VRAD on the same machine.
void RunAsNonServiceApp() { InternalRunService(); }

// This function runs inside the service thread.
void ServiceThreadFn(void *pParam) { InternalRunService(); }

// This function works with the service manager and runs as a system service.
void RunService() {
  if (!ServiceHelpers_StartService(VMPI_SERVICE_NAME_INTERNAL, ServiceThreadFn,
                                   NULL)) {
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
  ...
  LoadString(hInstance, VMPI_SERVICE_IDS_VERSION_STRING, g_VersionString,
             sizeof(g_VersionString));

  // Setup the base app path.
  if (!GetModuleFileName(GetModuleHandle(NULL), g_BaseAppPath,
                         sizeof(g_BaseAppPath))) {
  ...
  V_ComposeFileName(g_BaseAppPath, "vmpi_service_cache", g_FileCachePath,
                    sizeof(g_FileCachePath));
