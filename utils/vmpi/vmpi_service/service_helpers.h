...
bool ServiceHelpers_StartService(const char *pServiceName,
                                 void (*pFn)(void *pParam), void *pParam);

// Call this to exit the service early. This will make ShouldServiceExit()
// return true, and your main thread function should pick it up and exit.
//
// NOTE: this can be used even if the service isn't running as long as you call
// ServiceHelpers_Init().
void ServiceHelpers_ExitEarly();

// Your thread loop should call this each time around. If this function returns
// true, then your thread function should return, causing the service to exit.
//
// NOTE: this can be used even if the service isn't running as long as you call
// ServiceHelpers_Init().
