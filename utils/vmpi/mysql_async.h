...
// This provides a way to do asynchronous MySQL queries. They are executed in a
// thread, then you can pop the results off as they arrive.
class IMySQLAsync {
public:
  virtual void Release() = 0;

  // After finishing the current query, if there is one, this immediately
  ...
  // NOTE: if this returns true, but pResults is set to NULL, then that query
  // had an error.
