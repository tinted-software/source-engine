...
static void Usage() {
  fprintf(stderr, "Usage: gamestats_reader <hostname> <database> <username> "
                  "<password> <table> <directory to parse> ( -verbose )\n");
  Pause();
  exit(-1);
}
...
static const char *pszClassNames[] = {"rifleman", "assault", "support",
                                      "sniper",   "mg",      "rocket"};
...
void ParseFile(const char *fileName) {
  ...
  if (stats.header.iVersion != DOD_STATS_BLOB_VERSION ||
      Q_stricmp(stats.header.szGameName, "dod")) {
    ...
    if (factory) {
      ...
      if (mysql) {
        if (mysql->InitMySQL(argv[2], argv[1], argv[3], argv[4])) {
          ...
          Q_strncat(searchString, "*.dat", sizeof(searchString),
                    COPY_ALL_CHARACTERS);
      ...
      printf("Unable to get factory from mysql_wrapper.dll, not updating "
             "access mysql table!!!");
    }
