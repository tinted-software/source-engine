...
void CMakefileCreator::CreateBaseDirs(CVCProjConvert::CConfiguration &config) {
  ...
  for (int i = 0; i < config.GetNumFileNames(); i++) {
    if (config.GetFileType(i) == CVCProjConvert::CConfiguration::FILE_SOURCE) {
      ...
      Q_snprintf(fulldir, sizeof(fulldir), "%s/%s", m_BaseDir.String(),
...
void CMakefileCreator::CreateMakefileName(
  ...
  Q_snprintf(makefileName, sizeof(makefileName), "Makefile.%s_%s", projectName,
...
void CMakefileCreator::OutputIncludes(CVCProjConvert::CConfiguration &config,
                                      FileHandle_t f) {
...
void CMakefileCreator::OutputDirs(FileHandle_t f) {
  for (int i = m_BaseDirs.First(); i != m_BaseDirs.InvalidIndex();
       i = m_BaseDirs.Next(i)) {
    ...
    Q_strncat(friendlyDirName, "_SRC_DIR", sizeof(friendlyDirName),
              COPY_ALL_CHARACTERS);
    struct OutputDirMapping_t dirs;
    dirs.m_SrcDir = friendlyDirName;
    dirs.m_iBaseDirIndex = i;
    friendlyDirName[dirLen] = 0;
    Q_strncat(friendlyDirName, "_OBJ_DIR", sizeof(friendlyDirName),
              COPY_ALL_CHARACTERS);
    dirs.m_ObjDir = friendlyDirName;
    friendlyDirName[dirLen] = 0;
    Q_strncat(friendlyDirName, "_OBJS", sizeof(friendlyDirName),
              COPY_ALL_CHARACTERS);
    dirs.m_ObjName = friendlyDirName;

    char objDirName[MAX_PATH];
    Q_snprintf(objDirName, sizeof(objDirName), "obj%c$(NAME)_$(ARCH)%c",
...
void CMakefileCreator::OutputObjLists(CVCProjConvert::CConfiguration &config,
                                      FileHandle_t f) {
  for (int buildDirIndex = 0; buildDirIndex < m_BuildDirectories.Count();
       buildDirIndex++) {
    struct OutputDirMapping_t &dirs = m_BuildDirectories[buildDirIndex];
    FileWrite(f, "%s= \\\n", dirs.m_ObjName.String());

    for (int j = m_FileToBaseDirMapping.FirstInorder();
...
void CMakefileCreator::OutputBuildTarget(FileHandle_t f) {
  for (int i = 0; i < m_BuildDirectories.Count(); i++) {
    ...
    FileWrite(f, "$(%s)/%%.o: $(%s)/%%.cpp\n", dirs.m_ObjDir.String(),
              dirs.m_SrcDir.String());
    FileWrite(f, "\t$(DO_CC)\n\n");
  }
}
