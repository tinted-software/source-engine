...
// the elements of the list are meant to represent a state that endures over
// time and when it is switched to another state, that is represented by an
// element in this list with the value of the new state at index t, where t is
// the time that the switch occured. in TFStats, this is used for player names
// and player classes
//------------------------------------------------------------------------------------------------------
template <class T> class CTimeIndexedList {
public:
...
template <class T> time_t CTimeIndexedList<T>::PENDING = -1;
...
CTimeIndexedList<T>::accumulator CTimeIndexedList<T>::accumulate() {
  CTimeIndexedList<T>::accumulator
      accum; // maps from a value of T, to the time that value was played
...
//				b -
//------------------------------------------------------------------------------------------------------
template <class T> void CTimeIndexedList<T>::add(time_t a, T b) {
  bool lastIsB = false;
...
// Purpose:	returns the element whose time index is the closest to and less
// than or
//	equal to the queried time
// Input:	tm - the time that is being queried
// Output:	T, the element
//------------------------------------------------------------------------------------------------------
template <class T> T CTimeIndexedList<T>::atTime(time_t tm, T errorvalue) {
...
// Output:	T, the element
//------------------------------------------------------------------------------------------------------
template <class T> T CTimeIndexedList<T>::favourite(T errorvalue) {
  if (theList.empty())
    return errorvalue;

  CTimeIndexedList<T>::accumulator accum = accumulate();

  // now scan that intermediate map, and determine the element with the longest
  // duration
...
//------------------------------------------------------------------------------------------------------
template <class T> bool CTimeIndexedList<T>::contains(const T &t) {
  CTimeIndexedList<T>::iterator it = theList.begin();
  for (it = theList.begin(); it != theList.end(); ++it) {
    if (it->data == t)
      return true;
...
template <class T> time_t CTimeIndexedList<T>::howLong(const T &t) {
  if (theList.begin() == theList.end())
    return 0;

  CTimeIndexedList<T>::accumulator accum =
      accumulate(); // maps from a value of T, to the time that value was played

  return accum[t];
}

template <class T> CTimeIndexedList<T>::iterator CTimeIndexedList<T>::begin() {
  return theList.begin();
}

template <class T> CTimeIndexedList<T>::iterator CTimeIndexedList<T>::end() {
  return theList.end();
}

template <class T> int CTimeIndexedList<T>::size() { return theList.size(); }

template <class T> int CTimeIndexedList<T>::numDifferent() {
  if (theList.begin() == theList.end())
    return 0;
...
template <class T> void CTimeIndexedList<T>::cut(time_t t) {
  if (t > endTime)
    endTime = t;
  if (!theList.empty()) {
    // find the last element and fix up its end-time (if it's pending)
...
template <class T> void CTimeIndexedList<T>::remove(T b) {
  CTimeIndexedList<T>::iterator it = theList.begin();
