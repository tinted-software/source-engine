...
// Purpose:	Constructor that builds the object out of the passed in string
// of text Input:	text - text representing the argument
//------------------------------------------------------------------------------------------------------
CLogEventArgument::CLogEventArgument(const char *text) { init(text); }
...
// Purpose:	treats the argument as a player name, and copies/returns the
// player name. Note: PlayerName args have this form: "name<pid><WONID:wonid>"
...
//------------------------------------------------------------------------------------------------------
// Function:	CLogEventArgument::asPlayerGetName
// Purpose:	an alternate form of the above function that returns the
// playername as a C++ string, rather than buffercopying it around Output:
...
// Purpose:	treats the argument as a player name, and returns the player's
// wonid Note: PlayerName args have this form: "name<pid><WON:wonid>" Output:
// int: the WONID of the player
//------------------------------------------------------------------------------------------------------
...
// Output:	double
//------------------------------------------------------------------------------------------------------
double CLogEventArgument::getFloatValue() const { return atof(m_ArgText); }

//------------------------------------------------------------------------------------------------------
// Function:	CLogEventArgument::getStringValue
// Purpose:	 treats the argument as a string and returns a pointer to the
// argument text itself.  note the pointer is const, so the argument can't be
// modified by the caller (unless they perform some nefarious casting on the
// returned pointer) Output:	const char*
//------------------------------------------------------------------------------------------------------
const char *CLogEventArgument::getStringValue() const { return m_ArgText; }

//------------------------------------------------------------------------------------------------------
// Function:	CLogEventArgument::getStringValue
// Purpose:	 an alternate form of the above that copies the string into a
// caller supplied buffer then returns a pointer to that buffer Input:	copybuf
// - the buffer into which the string is to be copied Output:	char* the
// pointer to the buffer that the caller passed in
//------------------------------------------------------------------------------------------------------
