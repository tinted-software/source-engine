...
#endif
  printf("the available switches are detailed in the documentation. "
         "(TFStats.htm): \n");
  printf("---\n");
  printf("After TFStats is done, several new HTML files will be in the current "
         "directory,\n");
  printf("point your browser to the index.html in this directory and it will "
         "do the rest.\n");
#ifdef WIN32
...
// Purpose:	This parses variables and values out of passed in commandline
// args Input:	in - the full command line argument
//				var - the output buffer in which to place the
//name of the variable 				val - the output buffer in which to place the value of
//the variable
//------------------------------------------------------------------------------------------------------
void CTFStatsApplication::parseCmdLineArg(const char *in, char *var,
                                          char *val) {
  char *pEq = strchr(in, '=');
  if (!pEq || pEq == in) {
    var[0] = val[0] = 0;
...
void CTFStatsApplication::ParseCommandLine(int argc, const char *argv[]) {
  ...
  for (int i = 2; i < argc; i++) {
    ...
    if (!var[0])
      fatalError("Malformed switch,  required format is <variable>=<value> "
                 "with no spaces");
...
// Purpose:	 Creates a directory and writes the full directory path back
// into the
//	passed in string.  In effect, this turns a potentially relative path
//into an 	absolute path, and ensures the directory exists.
// Input:	dir - the string that contains the pathname, and upon return
// will contain
...
void CTFStatsApplication::makeAndSaveDirectory(string &dir) {
  ...
  if (!os->makeHier(dir)) {
    fatalError("Failed to make directory \"%s\", aborting. (Reason: %s)",
...
void CTFStatsApplication::makeDirectory(string &dir) {
  ...
  if (!os->makeHier(dir)) {
    fatalError("Failed to make directory \"%s\". (Reason: %s)", dir.c_str(),
               strerror(errno));
  }
  os->chdir(startpath);
}
...
// Purpose:	Turns the number of cutoff days into seconds. Cutoff days are
// how
//	many days players can be absent from the server and still have their
//stats
