...
// triggers. An award trigger is an object that recognizes a certain type of
// event in the log file, and if it matches that event, then it "triggers" and
// the custom award which owns it increments the counter for the player who
...
public:
  CBroadcastTrigger(int value, int teamValue, vector<string> &keys,
...
public:
  CGoalTrigger(int value, int teamValue, vector<string> &keys,
               map<string, string> extras);
...
public:
  ...
  CFullSearchTrigger(int value, int teamValue, vector<string> &ks,
                     map<string, string> extras);
