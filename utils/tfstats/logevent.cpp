...
// Purpose:	helper function for parseArgs, this actually removes the
// argument Input:	raw - the string from which we want to remove the
// argument
//------------------------------------------------------------------------------------------------------
...
// Purpose:	 tests to see if all of the given keywords occur in the text for
// this event Input:	s1 - first keyword (required)
//				s2 - second keyword (optional)
//				s3 - third keyword (optional)
// Output:	Returns true if the event text contains all of the keywords
// passed in
//------------------------------------------------------------------------------------------------------
...
// Purpose:	 this is a big dumb if statement to determine the type of this
// event
//------------------------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------------------
CLogEvent::CLogEvent()
    : m_EventCode('\0'), m_EventTime(0), m_Valid(false), m_Next(NULL),
      m_StrippedText(NULL), m_EventType(INVALID), m_EventMessage(NULL) {}
...
//------------------------------------------------------------------------------------------------------
// Function:	CLogEvent::CLogEvent
// Purpose:	CLogEvent constructor that reads an event from the specified
// file Input:	f - the file to read from
//------------------------------------------------------------------------------------------------------
...
// Purpose:	reads an event by reading each part, then checking if it was
// successful Input:	f - the file to read from
//------------------------------------------------------------------------------------------------------
...
// Purpose:	reads the event code, the first character on the line (should be
// 'L') Input:	f - the file to read from
//------------------------------------------------------------------------------------------------------
...
void CLogEvent::readEventMessage(FILE *f) {
  ...
  if (strncmp(temp, "Named Broadcast:", 16) == 0 ||
      strncmp(temp, "Broadcast:", 16) == 0) {
    while (1) {
      fpos_t temp_pos;
      fgetpos(f, &temp_pos);
      CLogEvent cle(f);
      ...
      if (cle.isValid()) {
        // if the next log event is valid, then this broadcast did not span
        // lines
        break;
      } else {
...
void CLogEvent::readEventTime(FILE *f) {
  ...
  fscanf(f, " %d/%d/%d - %d:%d:%d: ", &month, &day, &year, &hour, &minute,
         &second);
