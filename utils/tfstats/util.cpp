...
//
// Purpose: Contains lots of stuff that really doesn't fit anywhere else.
// Including
...
//		this function returns the non friendly name if the friendly one
//isn't found
// Output:	const string&
//------------------------------------------------------------------------------------------------------
...
char *Util::daysofWeek[] = {{"Sunday"},    {"Monday"},   {"Tuesday"},
...
char *plrClassNames[] = {
    ...
    {"medic"},     {"hwguy"},    {"pyro"},     {"spy"},     {"engineer"},
    {"civilian"},  {"RandomPC"}, {"observer"},
};
...
const char *Util::makeDurationString(time_t start, time_t end, char *out,
                                     char *tostr) {
  // TODO:
  ...
  if (pstart->tm_isdst)
    shour = (shour + 23) %
            24; // this substracts 1 while accounting for backing up past 0
  ...
  sprintf(out, "%02li:%02li%s%02li:%02li %s, %s %s %li%s %li", shour, smin,
          tostr, ehour, emin, matchtz, Util::daysofWeek[sweekday],
          Util::Months[smo], sday,
