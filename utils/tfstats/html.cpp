...
//				fPrintBody - true if the <body> tag is to be
//written. 				bgimage - name of a background image, if desired 				leftmarg - pixels on
//the left margin (if desired) 				topmarg - pixels on the top margin (if desired)
//------------------------------------------------------------------------------------------------------
CHTMLFile::CHTMLFile(const char *filenm, const char *title, bool fPrintBody,
                     const char *bgimage, int leftmarg, int topmarg) {
...
void CHTMLFile::open(const char *filename) {
  ...
  if (!out)
    g_pApp->fatalError(
        "Can't open output file \"%s\"!\nPlease make sure that the file does "
