...
void CTFStatsReport::genJavaScript() {
  ...
  if (!f)
    g_pApp->fatalError("Can't open output file \"%s\"! (reason: %s)\nPlease "
                       "make sure that the file does not exist OR\nif the file "
                       "does exit, make sure it is not read-only",
...
void CTFStatsReport::genIndex() {
  ...
  html.write("{\n");
  html.write("	if (navigator.appName != \"Microsoft Internet Explorer\" && "
  ...
  html.write("else\n");
  html.write("{\n");
  html.write("	document.write(\"<div class=whitetext align = left> Your "
             "browser:</div>\");\n");
  html.write("	document.write(\"<div class=whitetext align = left>\", "
             "navigator.appName, \"</div>\");\n");
  html.write("	document.write(\"<div class=whitetext align = left>\", "
             "navigator.appVersion, \"</div>\");\n");
  html.write("	document.write(\"<div class=whitetext align = left>  is not "
             "compatible with TFStats </font></div>\");\n");
  html.write("	document.write(\"<div class=whitetext align = left> TFStats "
             "supports IE5, IE4 and Netscape 4</font></div>\");\n");
  html.write(
      "	document.write(\"<div class=whitetext align = left> <a  target=_top "
      "href=\\\"http://www.microsoft.com/windows/ie/download/"
      "ie5all.htm\\\">Download IE5</a></div>\");\n");
  html.write(
      "	document.write(\"<div class=whitetext align = left> <a  target=_top "
      "href=\\\"http://www.netscape.com/computing/download/"
      "index.html\\\">Download Netscape 4</a></div>\");\n");
  html.write("}\n");
  html.write("//-->\n");
  html.write("</script>\n");
  ...
  html2.write("<frame src=top.html marginheight=0 marginwidth=0 scrolling=no "
              "name=top>\n");
  html2.write("<frameset framespacing=0 frameborder=0 cols=211,*>\n");
  html2.write("<frame src=nav.html marginheight=0 marginwidth=0 scrolling=no "
              "name=nav>\n");
  html2.write("<frame src=MatchResults.html marginheight=0 marginwidth=0 "
              "scrolling=yes name=data>\n");
...
void CTFStatsReport::genTopFrame() {
  ...
  html.write("<table width=100%% height=100%% cellpadding=0 cellspacing=0 "
             "border=0 background=\"%s/bgtop.jpg\">\n",
...
inline void writeEventHandlers(CHTMLFile &html, char *id) {
  html.write(" OnClick=\" clearAll();  %sImgLink.on(); return true; \" ", id,
             id);
  // html.write(" OnMouseDown=\"imageOn('%s');return true;\" ",id);
  html.write(
      " OnMouseOver=\" window.status='%s'; %sImgLink.over();return true; \" ",
      id, id);
  html.write(
...
void CTFStatsReport::genNavFrame() {
  ...
  imgname += "/bgleft.jpg";
  CHTMLFile html("nav.html", "top", CHTMLFile::printBody, imgname.c_str(), 0,
                 0);
  ...
  html.write("MatchStatsImgLink = new "
             "SuperImage(\"MatchStats\",\"%s/match.statistics.on.gif\",\"%s/"
             "match.statistics.off.gif\");\n",
             g_pApp->supportHTTPPath.c_str(), g_pApp->supportHTTPPath.c_str());
  html.write("DialoguePageImgLink = new "
             "SuperImage(\"DialoguePage\",\"%s/game.dialog.on.gif\",\"%s/"
             "game.dialog.off.gif\");\n",
             g_pApp->supportHTTPPath.c_str(), g_pApp->supportHTTPPath.c_str());
  html.write("ServerSettingsImgLink = new "
             "SuperImage(\"ServerSettings\",\"%s/server.settings.on.gif\",\"%s/"
             "server.settings.off.gif\");\n",
             g_pApp->supportHTTPPath.c_str(), g_pApp->supportHTTPPath.c_str());
  html.write("PlayerStatsMatchImgLink = new "
             "SuperImage(\"PlayerStatsMatch\",\"%s/"
             "player.statistics.match.on.gif\",\"%s/"
             "player.statistics.match.off.gif\");\n",
             g_pApp->supportHTTPPath.c_str(), g_pApp->supportHTTPPath.c_str());
  html.write("PlayerStatsServerImgLink = new "
             "SuperImage(\"PlayerStatsServer\",\"%s/"
             "player.statistics.server.on.gif\",\"%s/"
...
const char* CTFStatsReport::makeDurationString()
{
        ...
        if (pstart->tm_isdst)
                shour=(shour+23)%24; //this substracts 1 while accounting for
backing up past 0 int smin=pstart->tm_min;
        ...
        if (pend->tm_isdst)
                ehour=(ehour+23)%24; //this substracts 1 while accounting for
backing up past 0
        ...
        sprintf(DurationString,"%02li:%02li - %02li:%02li %s, %s %s %li%s
%li",shour,smin,ehour,emin,matchtz,
