...
// Purpose:	generates html from the intermediate data generated by
// generate() Input:	html - the html file to write the html code into
//------------------------------------------------------------------------------------------------------
void CScoreboard::writeHTML(CHTMLFile &html) {
  ...
  for (t = 0; t < MAX_TEAMS; t++) {
    ...
    if (!teamFound) {
      html.write("<p>");
      html.write("<img src=\"%s/scores.gif\">",
    ...
    html.write("<td ><font class=boardtext>%s</font></td>",
               g_pMatchInfo->teamName(t).c_str());
    ...
    html.write("<td><font class=boardtext>%.2lf (avg)</font></td>",
               teamrank / numplrs);
    html.write("</tr>");
    html.write("</table>\n<p>\n");
  }
