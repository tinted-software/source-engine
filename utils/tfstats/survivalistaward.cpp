...
// Purpose:	generates a winner for this award by determining who died the
// least while playing a scout
//------------------------------------------------------------------------------------------------------
void CSurvivalistAward::getWinner() {
  map<PID, bool>
      isScout; // to keep track of whether or not a player is a scout now
  map<PID, bool> wasScout; // true if a player ever was a scout (and therefore
                           // is in contention for the award)
  ...
  for (it = g_pMatchInfo->eventList()->begin();
    ...
    case CLogEvent::CLASS_CHANGE: {
      ...
      player_class newpc =
          playerClassNameToClassID((*it)->getArgument(1)->getStringValue());
  ...
  for (deathiter = numdeaths.begin(); deathiter != numdeaths.end();
       ++deathiter) {
...
void CSurvivalistAward::extendedinfo(CHTMLFile &html) {
  if (numdeaths[winnerID] == 0)
    ...
    html.write("%s only died %li times as a scout!", winnerName.c_str(),
               numdeaths[winnerID]);
}
