...
/* JM_INT32                  The type for 32-bit integers - what C calls
 * intfast32_t */
/* #define JM_INT32 */
...
/* JM_NO_TRICKY_DEFAULT_PARAM If templates can not have derived default
 * parameters. */
...
/* JM_NO_WCSTRING            If there are no wcslen and wcsncmp functions
 * available. */
...
/* JM_NO_MEMBER_TEMPLATES    If member function templates or nested template
 * classes are not allowed. */
/* #define JM_NO_MEMBER_TEMPLATES */

/* JM_NO_TEMPLATE_RETURNS    If template functions based on return type are not
 * supported. */
/* #define JM_NO_TEMPLATE_RETURNS */

/* JM_NO_PARTIAL_FUNC_SPEC   If partial template function specialisation is not
 * supported */
...
/* JM_TEMPLATE_SPECIALISE    Defaults to template<> , ie the template
   specialisation prefix, can be redefined to nothing for older compilers. */
/* #define JM_TEMPLATE_SPECIALISE */

/* JM_NESTED_TEMPLATE_DECL   Defaults to template, the standard prefix when
   accessing nested template classes, can be redefined to nothing if the
   compiler does not support this. */
/* #define JM_NESTED_TEMPLATE_DECL */

/* JM_NO_TEMPLATE_INST       If explicit template instantiation with the
   "template class X<T>" syntax is not supported */
/* #define JM_NO_TEMPLATE_INST */

/* JM_NO_TEMPLATE_MERGE      If template in separate translation units don't
 * merge at link time */
/* #define JM_NO_TEMPLATE_MERGE */

/* JM_NO_TEMPLATE_MERGE_A    If template merging from library archives is not
 * supported */
/* #define JM_NO_TEMPLATE_MERGE_A */

/* JM_NO_TEMPLATE_SWITCH_MERGE If merging of templates containing switch
 * statements is not supported */
/* #define JM_NO_TEMPLATE_SWITCH_MERGE */

/* RE_CALL                   Optionally define a calling convention for C++
 * functions */
/* #define RE_CALL */

/* RE_CCALL                  Optionally define a calling convention for C
 * functions */
/* #define RE_CCALL */
...
/* JM_ITERATOR_T             Defines generic standard iterator type if
   available, use this as a shortcut to define all the other iterator types. 1 =
   __JM_STD::iterator<__JM_STD::tag_type, T, D, T*, T&> 2 =
   __JM_STD::iterator<__JM_STD::tag_type, T, D> */
/* #define JM_ITERATOR_T */

/* JM_OI_T                   For output iterators:
                             0 = NA
                      1 = __JM_STD::iterator<__JM_STD::output_iterator_tag, T,
   D, T*, T&> 2 = __JM_STD::iterator<__JM_STD::output_iterator_tag, T, D> 3 =
   __JM_STD::output_iterator */
/* #define JM_OI_T */

/* JM_II_T                   For input iterators:
                             0 = NA
                      1 = __JM_STD::iterator<__JM_STD::input_iterator_tag, T, D,
   T*, T&> 2 = __JM_STD::iterator<__JM_STD::input_iterator_tag, T, D> 3 =
   __JM_STD::input_iterator<T, D> 4 = __JM_STD::input_iterator<T> */
/* #define JM_II_T */

/* JM_FI_T                   For forward iterators:
                             0 = NA
                      1 = __JM_STD::iterator<__JM_STD::forward_iterator_tag, T,
   D, T*, T&> 2 = __JM_STD::iterator<__JM_STD::forward_iterator_tag, T, D> 3 =
   __JM_STD::forward_iterator<T, D> */
/* #define JM_FI_T */

/* JM_BI_T                   For bidirectional iterators:
                             0 = NA
                      1 =
   __JM_STD::iterator<__JM_STD::bidirectional_iterator_tag, T, D, T*, T&> 2 =
   __JM_STD::iterator<__JM_STD::bidirectional_iterator_tag, T, D> 3 =
   __JM_STD::bidirectional_iterator<T, D> */
/* #define JM_BI_T */

/* JM_RI_T                   For random access iterators:
                             0 = NA
                             1 =
   __JM_STD::iterator<__JM_STD::random_access_iterator_tag, T, D, T*, T&> 2 =
   __JM_STD::iterator<__JM_STD::random_access_iterator_tag, T, D> 3 =
   __JM_STD::random_access_iterator<T, D> */
/* #define JM_RI_T */

/* JM_NO_OI_ASSIGN           If output iterators ostream_iterator<>,
   back_insert_iterator<> and front_insert_iterator<> do not have assignment
   operators */
...
#elif JM_OI_T == 1
#define JM_OUTPUT_ITERATOR(T, D)                                               \
  __JM_STD::iterator<__JM_STD::output_iterator_tag, T, D, T *, T &>
#elif JM_OI_T == 2
#define JM_OUTPUT_ITERATOR(T, D)                                               \
  __JM_STD::iterator<__JM_STD::output_iterator_tag, T, D>
#elif JM_OI_T == 3
...
#elif JM_II_T == 1
#define JM_INPUT_ITERATOR(T, D)                                                \
  __JM_STD::iterator<__JM_STD::input_iterator_tag, T, D, T *, T &>
#elif JM_II_T == 2
#define JM_INPUT_ITERATOR(T, D)                                                \
  __JM_STD::iterator<__JM_STD::input_iterator_tag, T, D>
#elif JM_II_T == 3
...
#define JM_FWD_ITERATOR(T, D)                                                  \
  __JM_STD::iterator<__JM_STD::forward_iterator_tag, T, D, T *, T &>
#elif JM_FI_T == 2
#define JM_FWD_ITERATOR(T, D)                                                  \
  __JM_STD::iterator<__JM_STD::forward_iterator_tag, T, D>
#elif JM_FI_T == 3
...
#define JM_BIDI_ITERATOR(T, D)                                                 \
  __JM_STD::iterator<__JM_STD::bidirectional_iterator_tag, T, D, T *, T &>
#elif JM_BI_T == 2
#define JM_BIDI_ITERATOR(T, D)                                                 \
  __JM_STD::iterator<__JM_STD::bidirectional_iterator_tag, T, D>
#elif JM_BI_T == 3
...
#define JM_RA_ITERATOR(T, D)                                                   \
  __JM_STD::iterator<__JM_STD::random_access_iterator_tag, T, D, T *, T &>
#elif JM_RI_T == 2
#define JM_RA_ITERATOR(T, D)                                                   \
  __JM_STD::iterator<__JM_STD::random_access_iterator_tag, T, D>
#elif JM_RI_T == 3
...
#else
#include <memory>
#define REBIND_INSTANCE(x, y, inst)                                            \
  y::JM_NESTED_TEMPLATE_DECL rebind<x>::other(inst)
