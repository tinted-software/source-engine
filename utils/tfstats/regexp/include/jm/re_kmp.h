...
template <class charT> struct kmp_info {
  unsigned int size;
  unsigned int len;
  const charT *pstr;
  int kmp_next[1];
...
kmp_info<charT> *kmp_compile(iterator first, iterator last, charT,
                             Trans translate, Allocator a
#ifdef RE_LOCALE_CPP
                             ,
  ...
  ++m;
  unsigned int size =
      sizeof(kmp_info<charT>) + sizeof(int) * m + sizeof(charT) * m;
  --m;
  //
  ...
  pinfo->size = size;
  pinfo->len = m;
  charT *p = (charT *)((char *)pinfo + sizeof(kmp_info<charT>) +
                       sizeof(int) * (m + 1));
  pinfo->pstr = p;
  while (first != last) {
