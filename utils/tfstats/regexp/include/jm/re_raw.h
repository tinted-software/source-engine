...
template <class Allocator> class raw_storage {
public:
  typedef Allocator alloc_type;
  typedef typename REBIND_TYPE(unsigned char, alloc_type)::size_type size_type;
  typedef JM_MAYBE_TYPENAME REBIND_TYPE(unsigned char,
                                        alloc_type) alloc_inst_type;
...
public:
  ...
  ~raw_storage() { alloc_inst.deallocate(start, (alloc_inst.last - start)); }
  ...
  size_type RE_CALL size() { return end - start; }

  size_type RE_CALL capacity() { return alloc_inst.last - start; }

  void *RE_CALL data() const { return start; }

  size_type RE_CALL index(void *ptr) { return (unsigned char *)ptr - start; }

  void RE_CALL clear() { end = start; }

  void RE_CALL align() {
    // move end up to a boundary:
    end = (unsigned char *)((long)(end + sizeof(padding) - 1) &
                            ~((long)sizeof(padding) - 1));
  }
...
template <class Allocator>
CONSTRUCTOR_INLINE raw_storage<Allocator>::raw_storage(size_type n,
                                                       const Allocator &a)
    : alloc_inst(a) {
