...
template <class charT> size_t RE_CALL re_strlen(const charT *s) {
  size_t len = 0;
  while (*s) {
    ++s;
    ++len;
...
template <class charT> int RE_CALL re_strcmp(const charT *s1, const charT *s2) {
  while (*s1 && *s2) {
    if (*s1 != *s2)
      return *s1 - *s2;
    ++s1;
...
template <class charT> charT *RE_CALL re_strcpy(charT *s1, const charT *s2) {
  charT *base = s1;
  while (*s2) {
    *s1 = *s2;
    ++s1;
...
inline size_t RE_CALL re_strlen(const char *s) { return strlen(s); }
...
inline size_t RE_CALL re_strlen(const wchar_t *s) { return wcslen(s); }
...
JM_IX_DECL unsigned int RE_CALL _re_strnarrow(char *s1, unsigned int len,
                                              const wchar_t *s2);
JM_IX_DECL unsigned int RE_CALL _re_strwiden(wchar_t *s1, unsigned int len,
                                             const char *s2);

inline unsigned int RE_CALL re_strnarrow(char *s1, unsigned int len,
                                         const wchar_t *s2) {
  return _re_strnarrow(s1, len, s2);
}

inline unsigned int RE_CALL re_strwiden(wchar_t *s1, unsigned int len,
                                        const char *s2) {
...
template <class charT> charT *RE_CALL re_strdup(const charT *p) {
...
template <class charT> inline void RE_CALL re_strfree(charT *p) { delete[] p; }

template <class charT> class re_str {
  charT *buf;
...
template <class charT> CONSTRUCTOR_INLINE re_str<charT>::~re_str() {
  re_strfree(buf);
}
...
template <class charT> CONSTRUCTOR_INLINE re_str<charT>::re_str(charT c) {
  buf = re_strdup(&c, &c + 1);
}

#ifndef JM_NO_WCSTRING
JM_IX_DECL void RE_CALL re_transform(re_str<wchar_t> &out,
                                     const re_str<wchar_t> &in);
#endif
JM_IX_DECL void RE_CALL re_transform(re_str<char> &out, const re_str<char> &in);

template <class charT> void RE_CALL re_trunc_primary(re_str<charT> &s) {
