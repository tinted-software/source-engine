...
//
template <class T, class Allocator JM_DEF_ALLOC_PARAM(T)> class jstack {
private:
...
public:
  ...
  node *RE_CALL get_node() {
    node *new_stack =
        (node *)alloc_inst.allocate(sizeof(node) + sizeof(T) * block_size);
...
template <class T, class Allocator> jstack<T, Allocator>::~jstack() {
  node *condemned;
  while (good())
    pop();
  while (unused) {
    condemned = unused;
    unused = unused->next;
    alloc_inst.deallocate((unsigned char *)condemned,
                          sizeof(node) + sizeof(T) * block_size);
  }
  while (stack != &base) {
    condemned = stack;
    stack = stack->next;
