...
re_is_set_member(iterator next, iterator last, re_set_long *set,
  ...
  for (i = 0; i < set->csingles; ++i) {
    ptr = next;
    while (*p && (ptr != last)) {
      if (traits_type::translate(*ptr, icase MAYBE_PASS_LOCALE(e.locale())) !=
          *p)
        break;
      ++p;
      ++ptr;
  ...
  charT col =
      traits_type::translate(*next, icase MAYBE_PASS_LOCALE(e.locale()));
  ...
  if (traits_type::is_class(col, set->cclasses MAYBE_PASS_LOCALE(e.locale())) ==
      true)
    return set->isnot ? next : ++next;
  return set->isnot ? ++next : next;
}
...
public:
  ...
  ~__priv_match_data() { free(); }
  void free();
  void set_accumulator_size(unsigned int size);
  int *get_accumulators() { return accumulators; }
  iterator *get_loop_starts() { return loop_starts; }
};

template <class iterator, class Allocator>
__priv_match_data<iterator, Allocator>::__priv_match_data(
    const reg_match_base<iterator, Allocator> &m)
    : temp_match(m), matches(64, m.allocator()), prev_pos(64, m.allocator()),
...
void __priv_match_data<iterator, Allocator>::free() {
  if (caccumulators) {
    // REBIND_INSTANCE(int, Allocator,
    // temp_match.allocator()).deallocate(accumulators, caccumulators);
    i_alloc temp1(temp_match.allocator());
    temp1.deallocate(accumulators, caccumulators);
    for (unsigned i = 0; i < caccumulators; ++i)
      jm_destroy(loop_starts + i);
    // REBIND_INSTANCE(iterator, Allocator,
    // temp_match.allocator()).deallocate(loop_starts, caccumulators);
...
template <class iterator, class Allocator, class charT, class traits,
          class Allocator2>
bool query_match(iterator first, iterator last,
...
template <class ST, class SA, class Allocator, class charT, class traits,
          class Allocator2>
inline bool query_match(
    const __JM_STD::basic_string<charT, ST, SA> &s,
    reg_match<typename __JM_STD::basic_string<charT, ST, SA>::const_iterator,
              Allocator> &m,
    const reg_expression<charT, traits, Allocator2> &e,
    unsigned flags = match_default) {
...
inline bool query_match(const char *str, cmatch &m, const regex &e,
                        unsigned flags = match_default) {
  return query_match(str, str + regex::traits_type::length(str), m, e, flags);
}
#ifndef JM_NO_WCSTRING
inline bool query_match(const wchar_t *str, wcmatch &m, const wregex &e,
                        unsigned flags = match_default) {
...
#ifndef JM_NO_STRING_H
inline bool
query_match(const __JM_STD::string &s,
...
inline bool
query_match(const __JM_STD::basic_string<wchar_t> &s,
            reg_match<__JM_STD::basic_string<wchar_t>::const_iterator,
                      wregex::alloc_type> &m,
...
template <class iterator, class Allocator, class charT, class traits,
          class Allocator2>
bool query_match_aux(iterator first, iterator last,
                     reg_match<iterator, Allocator> &m,
                     const reg_expression<charT, traits, Allocator2> &e,
                     unsigned flags, __priv_match_data<iterator, Allocator> &pd,
                     iterator *restart) {
...
retry:
  ...
  while (first != last) {
    ...
    match_jump: {
      ...
      match_found = true;
      if ((flags & match_any) ||
          ((first == last) && (need_push_match == false))) {
    ...
    case syntax_element_literal: {
      ...
      for (unsigned int i = 0; i < len; ++i, ++first) {
        if ((first == last) ||
            (traits::translate(*first, icase MAYBE_PASS_LOCALE(locale_inst)) !=
             what[i]))
          goto failure;
      }
      ptr = ptr->next.p;
    ...
    case syntax_element_word_start:
      ...
      if (traits::is_class(*first,
        ...
        --t;
        if (traits::is_class(
                *t, char_class_word MAYBE_PASS_LOCALE(locale_inst)) == false) {
          ptr = ptr->next.p;
          continue;
        }
      }
    ...
    case syntax_element_word_end:
      ...
      // otherwise inside buffer:
      if (traits::is_class(*first, char_class_word MAYBE_PASS_LOCALE(
                                       locale_inst)) == false) {
        iterator t(first);
        --t;
        if (traits::is_class(*t,
                             char_class_word MAYBE_PASS_LOCALE(locale_inst))) {
          ptr = ptr->next.p;
          continue;
        }
    ...
    case syntax_element_set:
      ...
      if (((re_set *)ptr)
              ->__map[(traits_uchar_type)traits::translate(
                  *first, icase MAYBE_PASS_LOCALE(locale_inst))]) {
        ptr = ptr->next.p;
        ++first;
        continue;
      }
    ...
    case syntax_element_alt: {
      // alt_jump:
      if (reg_expression<charT, traits, Allocator2>::can_start(
              *first, ((re_jump *)ptr)->__map, (unsigned char)mask_take,
              width_type())) {
        // we can take the first alternative,
        // see if we need to push next alternative:
        if (reg_expression<charT, traits, Allocator2>::can_start(
                *first, ((re_jump *)ptr)->__map, mask_skip, width_type())) {
          if (need_push_match)
    ...
    case syntax_element_rep: {
      ...
      if (((re_repeat *)ptr)->alt.p->type == syntax_element_match) {
        ...
        if (((unsigned int)accumulators[cur_acc] < ((re_repeat *)ptr)->max) &&
            reg_expression<charT, traits, Allocator2>::can_start(
                *first, ((re_repeat *)ptr)->__map, mask_take, width_type())) {
          // push terminating match as fallback:
          if ((unsigned int)accumulators[cur_acc] >= ((re_repeat *)ptr)->min) {
            if ((prev_record.empty() == false) &&
                (prev_record.peek() == ((re_repeat *)ptr)->alt.p)) {
        ...
        if (((unsigned int)accumulators[cur_acc] >= ((re_repeat *)ptr)->min) &&
            reg_expression<charT, traits, Allocator2>::can_start(
                *first, ((re_repeat *)ptr)->__map, mask_skip, width_type())) {
      ...
      if (((unsigned int)accumulators[cur_acc] >= ((re_repeat *)ptr)->min) &&
        ...
        if (((unsigned int)accumulators[cur_acc] < ((re_repeat *)ptr)->max) &&
          ...
          if (((unsigned int)accumulators[cur_acc] ==
               ((re_repeat *)ptr)->min) ||
    ...
    case syntax_element_combining:
      if (traits::is_combining(
              traits::translate(*first, icase MAYBE_PASS_LOCALE(locale_inst))))
        goto failure;
      ++first;
      while ((first != last) &&
             traits::is_combining(traits::translate(
                 *first, icase MAYBE_PASS_LOCALE(locale_inst))))
        ++first;
      ptr = ptr->next.p;
      continue;
    case syntax_element_soft_buffer_end: {
      if (flags & match_not_eob)
        goto failure;
      iterator p(first);
      while ((p != last) && traits::is_separator(traits::translate(
                                *first, icase MAYBE_PASS_LOCALE(locale_inst))))
        ++p;
      if (p != last)
        goto failure;
  ...
  while (true) {
    ...
    case syntax_element_rep:
      ...
      // otherwise see if we can take the repeat:
      if (((unsigned int)accumulators[cur_acc] < ((re_repeat *)ptr)->max) &&
          ((ptr->can_be_null & (mask_take | mask_skip)) ==
           (mask_take | mask_skip))) {
        // move to next item in list:
        ++accumulators[cur_acc];
        ptr = ptr->next.p;
...
void __skip_and_inc(unsigned int &clines, iterator &last_line, iterator &first,
                    const iterator last) {
  while (first != last) {
    if (*first == '\n') {
      last_line = ++first;
...
template <class iterator>
void __skip_and_dec(unsigned int &clines, iterator &last_line, iterator &first,
                    iterator base, unsigned int len) {
...
template <class iterator>
inline void __inc_one(unsigned int &clines, iterator &last_line,
                      iterator &first) {
  if (*first == '\n') {
    last_line = ++first;
    ++clines;
...
template <class iterator, class Allocator> struct grep_search_predicate {
...
template <class iterator, class Allocator>
inline const reg_match_base<iterator, Allocator> &
grep_out_type(const grep_search_predicate<iterator, Allocator> &o,
              const Allocator &) {
  return *(o.pm);
}
...
template <class Predicate, class I, class charT, class traits, class A,
          class A2>
unsigned int reg_grep2(Predicate foo, I first, I last,
  ...
  if (reg_expression<charT, traits, A>::can_start(
          *first, __map, (unsigned char)mask_any, width_type())) {
    if (query_match_aux(first, last, m, e, flags, pd, &restart)) {
      ++cmatches;
      if (foo(m) == false)
        return cmatches;
      ...
      if (first == m[0].second) {
        ...
        if (query_match_aux(first, last, m, e, flags | match_not_null, pd,
                            &restart)) {
          ++cmatches;
          if (foo(m) == false)
            return cmatches;
          ...
          for (unsigned int i = 0;
               (restart != first) && (i < e.leading_length()); ++i, --restart)
            ;
          if (restart != last)
            ++restart;
  ...
  // optimise the search:
  type =
      (flags & match_continuous) ? regbase::restart_continue : e.restart_type();
        ...
        if (query_match_aux(first, last, m, e, flags, pd, &restart)) {
          ...
          if (first == m[0].second) {
            ...
            if (query_match_aux(first, last, m, e, flags | match_not_null, pd,
              ...
              need_init = false;
              for (unsigned int i = 0;
                   (restart != first) && (i < e.leading_length());
                   ++i, --restart)
                ;
              if (restart != last)
                ++restart;
  ...
  case regbase::restart_word: {
    ...
    while (first != last) {
      ...
      while ((first != last) &&
             traits::is_class(*first,
                              char_class_word MAYBE_PASS_LOCALE(locale_inst)))
        ++first;
      // now skip the white space:
      while ((first != last) &&
             (traits::is_class(*first, char_class_word MAYBE_PASS_LOCALE(
                                           locale_inst)) == false))
        __inc_one(clines, last_line, first);
      if (first == last)
        break;
...
template <class iterator, class Allocator, class charT, class traits,
          class Allocator2>
bool reg_search(iterator first, iterator last,
                reg_match<iterator, Allocator> &m,
                const reg_expression<charT, traits, Allocator2> &e,
                unsigned flags = match_default) {
  if (e.flags() & regbase::failbit)
  ...
  flags |= match_init;
  return reg_grep2(grep_search_predicate<iterator, Allocator>(&m), first, last,
                   e, flags, m.allocator());
}
...
#endif
#else // partial specialisation
inline bool reg_search(const char *str, cmatch &m, const regex &e,
                       unsigned flags = match_default) {
  return reg_search(str, str + regex::traits_type::length(str), m, e, flags);
}
#ifndef JM_NO_WCSTRING
inline bool reg_search(const wchar_t *str, wcmatch &m, const wregex &e,
                       unsigned flags = match_default) {
...
#ifndef JM_NO_STRING_H
inline bool
reg_search(const __JM_STD::string &s,
           reg_match<__JM_STD::string::const_iterator, regex::alloc_type> &m,
           const regex &e, unsigned flags = match_default) {
  return reg_search(s.begin(), s.end(), m, e, flags);
}
#if !defined(JM_NO_STRING_DEF_ARGS) && !defined(JM_NO_WCSTRING)
inline bool
reg_search(const __JM_STD::basic_string<wchar_t> &s,
           reg_match<__JM_STD::basic_string<wchar_t>::const_iterator,
                     wregex::alloc_type> &m,
           const wregex &e, unsigned flags = match_default) {
  return reg_search(s.begin(), s.end(), m, e, flags);
...
#ifndef JM_NO_STRING_H
template <class Predicate, class ST, class SA, class Allocator, class charT,
          class traits>
inline bool reg_grep(Predicate foo,
                     const __JM_STD::basic_string<charT, ST, SA> &s,
                     const reg_expression<charT, traits, Allocator> &e,
                     unsigned flags = match_default) {
...
inline bool reg_grep(
    bool (*foo)(
        const reg_match<__JM_STD::string::const_iterator, regex::alloc_type> &),
    const __JM_STD::string &s, const regex &e, unsigned flags = match_default) {
  return reg_grep(foo, s.begin(), s.end(), e, flags);
}
#if !defined(JM_NO_STRING_DEF_ARGS) && !defined(JM_NO_WCSTRING)
inline bool reg_grep(
    bool (*foo)(const reg_match<__JM_STD::basic_string<wchar_t>::const_iterator,
                                wregex::alloc_type> &),
    const __JM_STD::basic_string<wchar_t> &s, const wregex &e,
    unsigned flags = match_default) {
  return reg_grep(foo, s.begin(), s.end(), e, flags);
}
#endif
...
//
template <class iterator, class Allocator> struct grep_match {
  unsigned int line;
  iterator line_start;
  ...
  grep_match(unsigned int l, iterator p1,
...
template <class O, class I, class A> struct grep_adaptor {
  O oi;
  reg_match<I, A> m;
...
inline unsigned int
reg_grep_old(Out oi, iterator first, iterator last,
             const reg_expression<charT, traits, Allocator> &e,
             unsigned flags = match_default) {
  return reg_grep2(grep_adaptor<Out, iterator, Allocator>(oi, e.allocator()),
