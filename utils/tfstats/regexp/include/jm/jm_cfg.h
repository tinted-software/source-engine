...
turned off by defining the appropriate macros.  Borland C++, Borland C++
Builder, and Microsoft Visual C++ should be auto-recognised and configured. The
HP aCC and SunPro C++ compiler should also be supported - but run configure for
best results. The SGI, HP, Microsoft and Rogue Wave STL's should be
auto-recognised and configured. Do not change this file unless you really really
have to, add options to <jm_opt.h> instead. See <jm_opt.h> for a full list of
...
#define REBIND_INSTANCE(x, y, inst)                                            \
  JM_MAYBE_ACCESS_SPEC re_alloc_binder<x, y>(inst)
...
template <class T> inline void RE_CALL jm_destroy(T *t) { t->~T(); }
...
template <class T> inline void RE_CALL jm_construct(void *p, const T &t) {
  new (p) T(t);
}

template <class T, class Allocator> class re_alloc_binder : public Allocator {
public:
...
template <class T> struct dummy_iterator_base {
  typedef T value_type;
  typedef __JM_STDC::ptrdiff_t difference_type;
  typedef T *pointer;
  typedef T &reference;
...
template <class I> inline bool RE_CALL boolify(I val) {
...
public: // construct/copy/destroy:
  ...
  RE_CALL operator X *() const { return ptr; }
...
template <class X> inline auto_array<X>::~auto_array() { delete[] ptr; }

template <class X> inline X &RE_CALL auto_array<X>::operator*() const {
  return *ptr;
}
...
template <class X> inline X *RE_CALL auto_array<X>::get() const { return ptr; }

template <class X> inline X &RE_CALL auto_array<X>::operator[](int i) {
  return ptr[i];
}

template <class X> inline X *RE_CALL auto_array<X>::release() {
  X *tmp = ptr;
  ptr = NULL;
  return tmp;
}
...
#if defined(JM_PLATFORM_W32) && !defined(RE_LOCALE_W32) &&                     \
    !defined(RE_LOCALE_C) && !defined(RE_LOCALE_CPP) && !defined(JM_NO_W32)
...
#define jm_assert(x)                                                           \
  if ((x) == 0) {                                                              \
    cerr << "Assertion failed: " << #x << " in file " << __FILE__              \
...
#pragma message                                                                \
    "macro jm_input_iterator: " JM_INPUT_ITERATOR(char, __JM_STDC::ptrdiff_t)
#pragma message "macro jm_output_iterator: " JM_OUTPUT_ITERATOR(               \
    char, __JM_STDC::ptrdiff_t)
#pragma message                                                                \
    "macro jm_fwd_iterator: " JM_FWD_ITERATOR(char, __JM_STDC::ptrdiff_t)
#pragma message                                                                \
    "macro jm_bidi_iterator: " JM_BIDI_ITERATOR(char, __JM_STDC::ptrdiff_t)
#pragma message                                                                \
    "macro jm_ra_iterator: " JM_RA_ITERATOR(char, __JM_STDC::ptrdiff_t)
#ifdef RE_LOCALE_CPP
