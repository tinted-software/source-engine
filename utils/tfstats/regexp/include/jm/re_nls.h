...
enum char_class_type {
  ...
  char_class_print =
      C1_UPPER | C1_LOWER | C1_DIGIT | C1_PUNCT | C1_BLANK | C1_ALPHA,
  ...
  char_class_graph = char_class_alpha | char_class_digit | char_class_punct |
                     char_class_underscore,
  char_class_print = char_class_alpha | char_class_digit | char_class_punct |
                     char_class_underscore | char_class_blank,
...
template <class charT> class re_initialiser {
public:
  void update();
};
...
#ifdef RE_LOCALE_CPP
__JM_STD::string RE_CALL re_get_error_str(unsigned int id,
...
//
template <class charT>
unsigned int RE_CALL re_toi(const charT *&first, const charT *const last,
                            int radix
#ifdef RE_LOCALE_CPP
                            ,
                            const __JM_STD::locale &l
  ...
  while ((first != last) && re_istype(*first, type MAYBE_PASS_LOCALE(l)) &&
         (result <= maxval)) {
    result *= radix;
