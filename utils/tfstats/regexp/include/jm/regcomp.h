...
template <class traits> struct kmp_translator {
...
inline bool RE_CALL reg_expression<charT, traits, Allocator>::can_start(
    charT c, const unsigned char *__map, unsigned char mask,
    const __wide_type &) {
...
template <class charT, class traits, class Allocator>
CONSTRUCTOR_INLINE reg_expression<charT, traits, Allocator>::reg_expression(
    const charT *p, jm_uintfast32_t f, const Allocator &a)
    : data(a), pkmp(0) {
  set_expression(p, f);
}
...
template <class charT, class traits, class Allocator>
CONSTRUCTOR_INLINE reg_expression<charT, traits, Allocator>::reg_expression(
    const charT *p1, const charT *p2, jm_uintfast32_t f, const Allocator &a)
    : data(a), pkmp(0) {
...
template <class charT, class traits, class Allocator>
CONSTRUCTOR_INLINE reg_expression<charT, traits, Allocator>::reg_expression(
    const charT *p, size_type len, jm_uintfast32_t f, const Allocator &a)
    : data(a), pkmp(0) {
...
reg_expression<charT, traits, Allocator>::operator=(
    const reg_expression<charT, traits, Allocator> &e) {
  //
  // we do a deep copy only if e is a valid expression, otherwise fail.
  //
...
template <class charT, class traits, class Allocator>
inline bool RE_CALL reg_expression<charT, traits, Allocator>::operator==(
    const reg_expression<charT, traits, Allocator> &e) {
  return (_flags == e.flags()) &&
         (re_strcmp(expression(), e.expression()) == 0);
}

template <class charT, class traits, class Allocator>
bool RE_CALL reg_expression<charT, traits, Allocator>::operator<(
    const reg_expression<charT, traits, Allocator> &e) {
...
template <class charT, class traits, class Allocator>
unsigned int RE_CALL reg_expression<charT, traits, Allocator>::parse_inner_set(
    const charT *&first, const charT *last) {
  //
  // we have an inner [...] construct
  //
  jm_assert(traits_type::syntax_type(
                (traits_size_type)(traits_uchar_type)*first MAYBE_PASS_LOCALE(
                    locale_inst)) == syntax_open_set);
  const charT *base = first;
  while ((first != last) &&
         (traits_type::syntax_type(
              (traits_size_type)(traits_uchar_type)*first MAYBE_PASS_LOCALE(
  ...
  if (*(base + 1) != *(first - 2))
    return 0;
  unsigned int result =
      traits_type::syntax_type((traits_size_type)(traits_uchar_type) *
                               (base + 1) MAYBE_PASS_LOCALE(locale_inst));
  if ((result == syntax_colon) && ((first - base) == 5)) {
    return traits_type::syntax_type((traits_size_type)(traits_uchar_type) *
                                    (base + 2) MAYBE_PASS_LOCALE(locale_inst));
  }
  return ((result == syntax_colon) || (result == syntax_dot) ||
          (result == syntax_equal))
             ? result
             : 0;
}
...
template <class charT, class traits, class Allocator>
bool RE_CALL reg_expression<charT, traits, Allocator>::skip_space(
    const charT *&first, const charT *last) {
  //
  // returns true if we get to last:
  //
  while ((first != last) &&
         (traits_type::is_class(*first, char_class_space MAYBE_PASS_LOCALE(
                                            locale_inst)) == true)) {
    ++first;
  }
  return first == last;
...
template <class charT, class traits, class Allocator>
void RE_CALL reg_expression<charT, traits, Allocator>::parse_range(
    const charT *&ptr, const charT *end, unsigned &min, unsigned &max) {
  //
  ...
  } else if (traits_type::syntax_type(
                 (traits_size_type)(traits_uchar_type)*ptr MAYBE_PASS_LOCALE(
                     locale_inst)) != syntax_close_brace) {
    fail(REG_BADBR);
    return;
...
template <class charT, class traits, class Allocator>
charT RE_CALL reg_expression<charT, traits, Allocator>::parse_escape(
    const charT *&first, const charT *last) {
  charT c;
  ...
  case syntax_x:
    ...
    // maybe have \x{ddd}
    if (traits_type::syntax_type(*first MAYBE_PASS_LOCALE(locale_inst)) ==
        syntax_open_brace) {
      ++first;
      if (first == last) {
        fail(REG_EESCAPE);
      ...
      if (traits_type::is_class(*first, char_class_xdigit MAYBE_PASS_LOCALE(
                                            locale_inst)) == false) {
        fail(REG_BADBR);
        break;
      }
      c = (charT)traits_type::toi(first, last,
                                  -16 MAYBE_PASS_LOCALE(locale_inst));
      if ((first == last) || (traits_type::syntax_type(*first MAYBE_PASS_LOCALE(
                                  locale_inst)) != syntax_close_brace)) {
        fail(REG_BADBR);
      }
      ++first;
...
void RE_CALL reg_expression<charT, traits, Allocator>::compile_maps() {
  ...
  while (record->type != syntax_element_match) {
    if ((record->type == syntax_element_alt) ||
        (record->type == syntax_element_rep)) {
      memset(&(((re_jump *)record)->__map), 0, 256);
      record->can_be_null = 0;
      compile_map(record->next.p, ((re_jump *)record)->__map,
                  &(record->can_be_null), mask_take,
                  ((re_jump *)record)->alt.p);
      compile_map(((re_jump *)record)->alt.p, ((re_jump *)record)->__map,
                  &(record->can_be_null), mask_skip);
    } else {
      record->can_be_null = 0;
...
bool RE_CALL
re_maybe_set_member(charT c, re_set_long *set,
...
bool RE_CALL reg_expression<charT, traits, Allocator>::probe_start(
  ...
  case syntax_element_literal:
    ...
    if (*(charT *)(((re_literal *)node) + 1) ==
        traits_type::translate(cc, (_flags & regbase::icase)
                                       MAYBE_PASS_LOCALE(locale_inst)))
      return true;
    return false;
  case syntax_element_end_line:
    // next character (if there is one!) must be a newline:
    if (traits_type::is_separator(traits_type::translate(
            cc, (_flags & regbase::icase) MAYBE_PASS_LOCALE(locale_inst))))
      return true;
    return false;
  ...
  case syntax_element_soft_buffer_end:
    ...
    // syntax_element_match, if its not then we can
    // never match anything anyway!!
    return traits_type::is_separator(traits_type::translate(
        cc, (_flags & regbase::icase) MAYBE_PASS_LOCALE(locale_inst)));
  ...
  case syntax_element_long_set:
    ...
    return re_maybe_set_member(cc, (re_set_long *)node, *this) ||
           re_is_set_member((const charT *)&cc, (const charT *)(&cc + 1),
                            (re_set_long *)node, *this) != &cc;
  case syntax_element_set:
    // set all the elements that are set in corresponding set:
    c = (traits_size_type)(traits_uchar_type)traits_type::translate(
        cc, (_flags & regbase::icase) MAYBE_PASS_LOCALE(locale_inst));
  ...
  case syntax_element_combining:
    return !traits_type::is_combining(traits_type::translate(
        cc, (_flags & regbase::icase) MAYBE_PASS_LOCALE(locale_inst)));
  }
  return false;
}
...
template <class charT, class traits, class Allocator>
bool RE_CALL reg_expression<charT, traits, Allocator>::probe_start_null(
    re_syntax_base *node, re_syntax_base *terminal) const {
  switch (node->type) {
  case syntax_element_startmark:
  case syntax_element_endmark:
  case syntax_element_start_line:
  ...
  case syntax_element_alt:
    // we need to take the OR of the two alternatives:
    return probe_start_null(((re_jump *)node)->alt.p, terminal) ||
           probe_start_null(node->next.p, terminal);
  case syntax_element_rep:
...
void RE_CALL reg_expression<charT, traits, Allocator>::compile_map(
    re_syntax_base *node, unsigned char *__map, unsigned int *pnull,
    unsigned char mask, re_syntax_base *terminal) const {
  if (__map) {
...
template <class charT, class traits, class Allocator>
void RE_CALL reg_expression<charT, traits, Allocator>::move_offsets(
    re_syntax_base *j, unsigned size) {
...
reg_expression<charT, traits, Allocator>::compile_set_simple(
    re_syntax_base *dat, unsigned long cls, bool isnot) {
  ...
  return compile_set_aux(singles, ranges, classes, equivalents, isnot,
                         is_byte<charT>::width_type());
}

template <class charT, class traits, class Allocator>
re_syntax_base *RE_CALL reg_expression<charT, traits, Allocator>::compile_set(
    const charT *&first, const charT *last) {
  jstack<re_str<charT>, Allocator> singles(64, data.allocator());
  jstack<re_str<charT>, Allocator> ranges(64, data.allocator());
  jstack<jm_uintfast32_t, Allocator> classes(64, data.allocator());
  ...
  enum last_type { last_single, last_none, last_dash };
  ...
  while ((first != last) && !done) {
    ...
    case syntax_open_set: {
      ...
      case syntax_colon: {
        ...
        jm_uintfast32_t id = traits_type::lookup_classname(
            base + 2, first - 2 MAYBE_PASS_LOCALE(locale_inst));
      ...
      case syntax_dot:
        ...
        //
        if (traits_type::lookup_collatename(
                s, base + 2, first - 2 MAYBE_PASS_LOCALE(locale_inst))) {
          --first;
          if (s.size() > 1)
            has_digraphs = true;
          goto char_set_literal;
      ...
      case syntax_equal:
        ...
        if (traits_type::lookup_collatename(
          ...
          unsigned i = 0;
          while (s[i]) {
            s[i] = traits_type::translate(
                s[i], (_flags & regbase::icase) MAYBE_PASS_LOCALE(locale_inst));
            ++i;
          }
          re_str<charT> s2;
      ...
      default:
        if (started == false) {
          unsigned int t = traits_type::syntax_type(
              (traits_size_type)(traits_uchar_type) *
              (base + 1) MAYBE_PASS_LOCALE(locale_inst));
    ...
    case syntax_slash:
      if (_flags & regbase::escape_in_lists) {
        ...
        if (first == last)
          continue;
        switch (
            traits_type::syntax_type(*first MAYBE_PASS_LOCALE(locale_inst))) {
        case syntax_w:
          if (l == last_dash) {
            fail(REG_ERANGE);
            return NULL;
  ...
  if (has_digraphs)
    result = compile_set_aux(singles, ranges, classes, equivalents, isnot,
                             __wide_type());
  else
    result = compile_set_aux(singles, ranges, classes, equivalents, isnot,
                             is_byte<charT>::width_type());
...
template <class charT, class traits, class Allocator>
re_syntax_base *RE_CALL
reg_expression<charT, traits, Allocator>::compile_set_aux(
    jstack<re_str<charT>, Allocator> &singles,
...
reg_expression<charT, traits, Allocator>::compile_set_aux(
  ...
  while (singles.empty() == false) {
    dat->__map[(traits_size_type)(traits_uchar_type) *
    ...
    for (unsigned int i = 0; i < 256; ++i) {
      if (traits_type::is_class(charT(i), flags MAYBE_PASS_LOCALE(locale_inst)))
        dat->__map[(traits_uchar_type)traits_type::translate(
            (charT)i, (_flags & regbase::icase)
                          MAYBE_PASS_LOCALE(locale_inst))] = mask_all;
    }
  }
...
template <class charT, class traits, class Allocator>
void RE_CALL reg_expression<charT, traits, Allocator>::fixup_apply(
    re_syntax_base *b, unsigned cbraces) {
  ...
  try {
    ...
    while (ptr->next.i) {
      ...
      case syntax_element_rep:
        ((re_jump *)ptr)->alt.p =
            (re_syntax_base *)(base + ((re_jump *)ptr)->alt.i);
      ...
      case syntax_element_alt:
        ...
        goto rebase;
      case syntax_element_backref:
        if ((((re_brace *)ptr)->index >= cbraces) ||
            (pb[((re_brace *)ptr)->index] == false)) {
          fail(REG_ESUBREG);
...
unsigned int RE_CALL reg_expression<charT, traits, Allocator>::set_expression(
    const charT *p, const charT *end, jm_uintfast32_t f) {
  if (p == expression()) {
  ...
  if (_flags & regbase::literal) {
    while (ptr != end) {
      dat = add_literal(
          dat, traits::translate(*ptr, (_flags & regbase::icase)
                                           MAYBE_PASS_LOCALE(locale_inst)));
      ++ptr;
    }
  }
  ...
  while (ptr < end) {
    ...
    close_bracked_jump:
      ...
      if (mark.peek() == data.index(dat)) {
        re_syntax_base *para =
            (re_syntax_base *)((char *)data.data() + mark.peek());
      ...
      case syntax_Q:
        ...
        while (true) {
          ...
          if (traits_type::syntax_type(
            ...
            if ((ptr != end) &&
                (traits_type::syntax_type(
                     (traits_size_type)(traits_uchar_type)*ptr
                         MAYBE_PASS_LOCALE(locale_inst)) == syntax_E))
              break;
            else {
    ...
    repeat_jump: {
      ...
      case syntax_element_literal:
        if (((re_literal *)dat)->length > 1) {
          // update previous:
          charT lit =
              *(charT *)((char *)dat + sizeof(re_literal) +
                         ((((re_literal *)dat)->length - 1) * sizeof(charT)));
          --((re_literal *)dat)->length;
          dat = add_simple(dat, syntax_element_literal,
                           sizeof(re_literal) + sizeof(charT));
      ...
      dat =
          (re_syntax_base *)((char *)data.data() + data.size() - re_jump_size);
    ...
    alt_string_jump:
      ...
      if (mark.empty() == false) {
        ...
        offset = mark.peek();
        re_syntax_base *base =
            (re_syntax_base *)((unsigned char *)data.data() + offset);
        offset = base->next.i;
      }
      ...
      dat = (re_syntax_base *)((unsigned char *)data.data() + data.size() -
                               re_jump_size);
  ...
  startmap =
      (unsigned char *)data.extend(256 + ((end - base + 1) * sizeof(charT)));
  //
  ...
  *(_expression + (end - base)) = charT(0);
  ...
  if ((sbase->type == syntax_element_literal) &&
      (sbase->next.p->type == syntax_element_match)) {
    _restart_type = restart_fixed_lit;
    if (0 == pkmp) {
      charT *p1 = (charT *)((char *)sbase + sizeof(re_literal));
      charT *p2 = p1 + ((re_literal *)sbase)->length;
      pkmp = kmp_compile(p1, p2, charT(),
                         kmp_translator<traits>(_flags & regbase::icase),
...
re_syntax_base *RE_CALL reg_expression<charT, traits, Allocator>::add_simple(
    re_syntax_base *dat, syntax_element_type type, unsigned int size) {
  if (dat) {
    data.align();
    dat->next.i = data.size();
  }
...
template <class charT, class traits, class Allocator>
re_syntax_base *RE_CALL reg_expression<charT, traits, Allocator>::add_literal(
    re_syntax_base *dat, charT c) {
  if (dat && (dat->type == syntax_element_literal)) {
    // add another charT to the list:
    __JM_STDC::ptrdiff_t pos =
        (unsigned char *)dat - (unsigned char *)data.data();
    *(charT *)data.extend(sizeof(charT)) = traits::translate(
        c, (_flags & regbase::icase) MAYBE_PASS_LOCALE(locale_inst));
    ...
    dat = add_simple(dat, syntax_element_literal,
                     sizeof(re_literal) + sizeof(charT));
    ((re_literal *)dat)->length = 1;
    *((charT *)(((re_literal *)dat) + 1)) = traits::translate(
        c, (_flags & regbase::icase) MAYBE_PASS_LOCALE(locale_inst));
  }
  return dat;
}
...
reg_expression<charT, traits, Allocator>::probe_restart(re_syntax_base *dat) {
  switch (dat->type) {
  case syntax_element_startmark:
  case syntax_element_endmark:
...
template <class charT, class traits, class Allocator>
unsigned int RE_CALL
reg_expression<charT, traits, Allocator>::fixup_leading_rep(
    re_syntax_base *dat, re_syntax_base *end) {
  unsigned int len = 0;
