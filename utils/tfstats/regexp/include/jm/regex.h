...
#endif
  bad_expression(const bad_expression &e)
      : __JM_STD::exception(e), code(e.code) {}
...
template <class charT> class char_regex_traits {
public:
  ...
  // equivalence classes.
  static void RE_CALL transform_primary(re_str<charT> &out,
                                        const re_str<charT> &in
#ifdef RE_LOCALE_CPP
                                        ,
                                        const __JM_STD::locale &
#endif
  ...
  //
  static jm_uintfast32_t RE_CALL lookup_classname(const charT *first,
                                                  const charT *last
#ifdef RE_LOCALE_CPP
                                                  ,
                                                  const __JM_STD::locale &
  ...
  //
  static bool RE_CALL lookup_collatename(re_str<charT> &s, const charT *first,
                                         const charT *last
#ifdef RE_LOCALE_CPP
                                         ,
                                         const __JM_STD::locale &
...
public:
  ...
  static size_t RE_CALL length(const char_type *p) { return strlen(p); }
...
#ifdef RE_LOCALE_CPP
    return icase ? JM_USE_FACET(l, __JM_STD::ctype<char>).tolower((char_type)c)
                 : c;
#else
  ...
  static void RE_CALL transform(re_str<char> &out, const re_str<char> &in
    ...
    re_transform(out, in);
#else
    out = JM_USE_FACET(l, __JM_STD::collate<char>)
              .transform(in.c_str(), in.c_str() + in.size())
              .c_str();
#endif
  }

  static void RE_CALL transform_primary(re_str<char> &out,
                                        const re_str<char> &in
#ifdef RE_LOCALE_CPP
                                        ,
                                        const __JM_STD::locale &l
#endif
  ...
  static bool RE_CALL is_combining(char) { return false; }
...
#ifdef RE_LOCALE_CPP
    if (JM_USE_FACET(l, __JM_STD::ctype<char>)
            .is((__JM_STD::ctype<char>::mask)(f & char_class_all_base), c))
      return true;
  ...
  static jm_uintfast32_t RE_CALL lookup_classname(const char *first,
                                                  const char *last
#ifdef RE_LOCALE_CPP
                                                  ,
                                                  const __JM_STD::locale &l
  ...
  static bool RE_CALL lookup_collatename(re_str<char> &s, const char *first,
                                         const char *last
#ifdef RE_LOCALE_CPP
                                         ,
                                         const __JM_STD::locale &l
...
public:
  ...
  typedef unsigned int size_type;
  static size_t RE_CALL length(const char_type *p) { return wcslen(p); }
  static unsigned int RE_CALL syntax_type(size_type c
#ifdef RE_LOCALE_CPP
                                          ,
                                          const __JM_STD::locale &l
...
#ifdef RE_LOCALE_CPP
    return icase
               ? JM_USE_FACET(l, __JM_STD::ctype<wchar_t>).tolower((char_type)c)
               : c;
#else
    return icase ? ((c < 256) ? re_lower_case_map_w[(uchar_type)c]
                              : re_wtolower(c))
                 : c;
#endif
  }
  ...
  static void RE_CALL transform(re_str<wchar_t> &out, const re_str<wchar_t> &in
    ...
    re_transform(out, in);
#else
    out = JM_USE_FACET(l, __JM_STD::collate<wchar_t>)
              .transform(in.c_str(), in.c_str() + in.size())
              .c_str();
#endif
  }

  static void RE_CALL transform_primary(re_str<wchar_t> &out,
                                        const re_str<wchar_t> &in
#ifdef RE_LOCALE_CPP
                                        ,
                                        const __JM_STD::locale &l
#endif
  ...
  static bool RE_CALL is_separator(wchar_t c) {
    return JM_MAKE_BOOL((c == L'\n') || (c == L'\r') ||
                        (c == (wchar_t)0x2028) || (c == (wchar_t)0x2029));
  }

  static bool RE_CALL is_combining(wchar_t c) { return re_is_combining(c); }
  ...
  static bool RE_CALL is_class(wchar_t c, jm_uintfast32_t f
...
#ifdef RE_LOCALE_CPP
    if (JM_USE_FACET(l, __JM_STD::ctype<wchar_t>)
            .is((__JM_STD::ctype<wchar_t>::mask)(f & char_class_all_base), c))
      return true;
    if ((f & char_class_underscore) && (c == '_'))
      return true;
    if ((f & char_class_blank) && ((c == ' ') || (c == '\t')))
    ...
    return false;
#else
    return JM_MAKE_BOOL(((uchar_type)c < 256)
                            ? (re_unicode_classes[(size_type)(uchar_type)c] & f)
                            : re_iswclass(c, f));
#endif
  }
  ...
  static jm_uintfast32_t RE_CALL lookup_classname(const wchar_t *first,
                                                  const wchar_t *last
#ifdef RE_LOCALE_CPP
                                                  ,
                                                  const __JM_STD::locale &l
  ...
  static bool RE_CALL lookup_collatename(re_str<wchar_t> &s,
                                         const wchar_t *first,
                                         const wchar_t *last
#ifdef RE_LOCALE_CPP
                                         ,
...
public:
  ...
  static jm_uintfast32_t RE_CALL lookup_classname(const wchar_t *first,
    ...
    jm_uintfast32_t result = char_regex_traits<wchar_t>::lookup_classname(
        first, last MAYBE_PASS_LOCALE(l));
...
template <class charT> class is_byte;
...
enum re_jump_size_type {
  re_jump_size =
      (sizeof(re_jump) + sizeof(padding) - 1) & ~(sizeof(padding) - 1),
  re_repeater_size =
      (sizeof(re_repeat) + sizeof(padding) - 1) & ~(sizeof(padding) - 1)
};
...
public:
  enum flag_type {
    ...
    basic = char_classes | intervals | limited_ops | bk_braces | bk_parens |
            bk_refs,
  ...
  jm_uintfast32_t RE_CALL flags() const { return _flags; }
#ifdef RE_LOCALE_CPP
  __JM_STD::string RE_CALL errmsg() const {
    return re_get_error_str(code, locale_inst);
  }
#else
  const char *RE_CALL errmsg() const { return re_get_error_str(code); }
#endif
  ...
  const __JM_STD::locale &RE_CALL locale() const { return locale_inst; }
#endif
};
...
template <class charT,
          class traits JM_TRICKY_DEFAULT_PARAM(char_regex_traits<charT>),
...
private:
  ...
  void RE_CALL compile_map(re_syntax_base *node, unsigned char *__map,
                           unsigned int *pnull, unsigned char mask,
                           re_syntax_base *terminal = NULL) const;
  bool RE_CALL probe_start(re_syntax_base *node, charT c,
                           re_syntax_base *terminal) const;
  bool RE_CALL probe_start_null(re_syntax_base *node,
                                re_syntax_base *terminal) const;
  ...
  re_syntax_base *RE_CALL
  compile_set_aux(jstack<re_str<charT>, Allocator> &singles,
  ...
  compile_set_aux(jstack<re_str<charT>, Allocator> &singles,
                  ...
                  jstack<re_str<charT>, Allocator> &equivalents, bool isnot,
                  const __wide_type &);
  re_syntax_base *RE_CALL compile_set_simple(re_syntax_base *dat,
                                             unsigned long cls,
                                             bool isnot = false);
  unsigned int RE_CALL parse_inner_set(const charT *&first, const charT *last);

  re_syntax_base *RE_CALL
  add_simple(re_syntax_base *dat, syntax_element_type type,
  ...
  void RE_CALL parse_range(const charT *&first, const charT *last,
                           unsigned &min, unsigned &max);
  bool RE_CALL skip_space(const charT *&first, const charT *last);
  unsigned int RE_CALL probe_restart(re_syntax_base *dat);
  unsigned int RE_CALL fixup_leading_rep(re_syntax_base *dat,
                                         re_syntax_base *end);

public:
  unsigned int RE_CALL set_expression(const charT *p, const charT *end,
                                      jm_uintfast32_t f = regbase::normal);
  unsigned int RE_CALL set_expression(const charT *p,
                                      jm_uintfast32_t f = regbase::normal) {
    return set_expression(p, p + traits_type::length(p), f);
  }
  reg_expression(const Allocator &a = Allocator());
  reg_expression(const charT *p, jm_uintfast32_t f = regbase::normal,
                 const Allocator &a = Allocator());
  reg_expression(const charT *p1, const charT *p2,
                 jm_uintfast32_t f = regbase::normal,
                 const Allocator &a = Allocator());
  reg_expression(const charT *p, size_type len, jm_uintfast32_t f,
                 const Allocator &a = Allocator());
  ...
  unsigned int RE_CALL
  set_expression(const __JM_STD::basic_string<charT, ST, SA> &p,
                 jm_uintfast32_t f = regbase::normal) {
    return set_expression(p.data(), p.data() + p.size(), f);
  }

  template <class ST, class SA>
  reg_expression(const __JM_STD::basic_string<charT, ST, SA> &p,
                 jm_uintfast32_t f = regbase::normal,
                 const Allocator &a = Allocator())
      : data(a), pkmp(0) {
    set_expression(p, f);
  ...
  unsigned int RE_CALL set_expression(const __JM_STD::basic_string<charT> &p,
                                      jm_uintfast32_t f = regbase::normal) {
    return set_expression(p.data(), p.data() + p.size(), f);
  }

  reg_expression(const __JM_STD::basic_string<charT> &p,
                 jm_uintfast32_t f = regbase::normal,
                 const Allocator &a = Allocator())
      : data(a), pkmp(0) {
    set_expression(p, f);
...
#if !defined(JM_NO_TEMPLATE_FRIEND) &&                                         \
    (!defined(JM_NO_TEMPLATE_SWITCH_MERGE) || defined(JM_NO_NAMESPACES))
#if 0
...
private:
  ...
  static bool RE_CALL can_start(charT c, const unsigned char *__map,
...
template <class charT, class traits, class Allocator>
sub_match<iterator>::operator __JM_STD::basic_string<charT, traits, Allocator>()
    const {
...
#endif
template <class iterator> sub_match<iterator>::operator int() const {
  iterator i = first;
  int neg = 1;
  if ((i != second) && (*i == '-')) {
    neg = -1;
...
template <class iterator> sub_match<iterator>::operator unsigned int() const {
  iterator i = first;
  unsigned int result =
      (int)re_toi(i, second, 10 MAYBE_PASS_LOCALE(__JM_STD::locale()));
#if !defined(JM_NO_EXCEPTIONS) && !defined(JM_NO_TYPEINFO)
  if (i != second) {
    throw __JM_STD::bad_cast();
  }
...
public:
  ...
  size_type RE_CALL size() const { return ref->cmatches; }

  const sub_match<iterator> &RE_CALL operator[](int n) const {
    if ((n >= 0) && ((unsigned int)n < ref->cmatches))
      return *(sub_match<iterator> *)((char *)ref + sizeof(reference) +
                                      sizeof(sub_match<iterator>) * n);
  ...
  size_t RE_CALL length() const {
    ...
    size_t n = 0;
    JM_DISTANCE(((sub_match<iterator> *)(ref + 1))->first,
                ((sub_match<iterator> *)(ref + 1))->second, n);
    return n;
  }

  unsigned int RE_CALL line() const { return ref->lines; }

  iterator RE_CALL line_start() const { return ref->line_pos; }
  ...
  friend class reg_match<iterator, Allocator>;
    ...
    (!defined(JM_NO_TEMPLATE_SWITCH_MERGE) || defined(JM_NO_NAMESPACES))
private:
  template <class Predicate, class I, class charT, class traits, class A,
            class A2>
  friend unsigned int reg_grep2(Predicate foo, I first, I last,
                                const reg_expression<charT, traits, A> &e,
                                unsigned flags, A2 a);

  template <class I, class A, class charT, class traits, class A2>
  friend bool query_match(I first, I last, reg_match<I, A> &m,
                          const reg_expression<charT, traits, A2> &e,
                          unsigned flags);

  template <class I, class A, class charT, class traits, class A2>
  friend bool query_match_aux(I first, I last, reg_match<I, A> &m,
                              const reg_expression<charT, traits, A2> &e,
                              unsigned flags, __priv_match_data<I, A> &pd,
                              I *restart);

  template <class I, class A, class charT, class traits, class A2>
  friend bool reg_search(I first, I last, reg_match<I, A> &m,
                         const reg_expression<charT, traits, A2> &e,
                         unsigned flags);
#endif
  ...
  void RE_CALL set_first(iterator i, size_t pos) {
    cow();
    ((sub_match<iterator> *)((char *)ref + sizeof(reference) +
                             sizeof(sub_match<iterator>) * pos))
        ->first = i;
    if (pos == 0) {
      ref->head.second = i;
      ref->head.matched = (ref->head.first == ref->head.second) ? false : true;
...
reg_match_base<iterator, Allocator>::reg_match_base(const Allocator &a) {
  ref = (reference *)c_alloc(a).allocate(sizeof(sub_match<iterator>) +
                                         sizeof(reference));
  ...
  try {
    ...
    c_alloc(a).deallocate((char *)(void *)ref,
                          sizeof(sub_match<iterator>) + sizeof(reference));
    throw;
  }
#endif
}
...
reg_match_base<iterator, Allocator>::operator=(
    const reg_match_base<iterator, Allocator> &m) {
  if (ref != m.ref) {
    free();
    ref = m.ref;
    ++(ref->count);
...
void RE_CALL reg_match_base<iterator, Allocator>::free() {
  if (--(ref->count) == 0) {
    ...
    jm_destroy(ref);
    a.deallocate((char *)(void *)ref,
                 sizeof(sub_match<iterator>) * ref->cmatches +
                     sizeof(reference));
  }
}
...
void RE_CALL reg_match_base<iterator, Allocator>::set_size(size_type n) {
  if (ref->cmatches != n) {
    reference *newref = (reference *)ref->allocate(
        sizeof(sub_match<iterator>) * n + sizeof(reference));
#ifndef JM_NO_EXCEPTIONS
    try {
#endif
      ...
      ref->deallocate((char *)(void *)newref,
                      sizeof(sub_match<iterator>) * n + sizeof(reference));
      throw;
    }
#endif
...
template <class iterator, class Allocator>
inline void RE_CALL reg_match_base<iterator, Allocator>::init_fail(iterator i,
                                                                   iterator j) {
  set_size(ref->cmatches, i, j);
}

template <class iterator, class Allocator>
void RE_CALL reg_match_base<iterator, Allocator>::maybe_assign(
    const reg_match_base<iterator, Allocator> &m) {
  sub_match<iterator> *p1, *p2;
  p1 = (sub_match<iterator> *)(ref + 1);
...
void RE_CALL reg_match_base<iterator, Allocator>::cow() {
  if (ref->count > 1) {
    reference *newref = (reference *)ref->allocate(
        sizeof(sub_match<iterator>) * ref->cmatches + sizeof(reference));
#ifndef JM_NO_EXCEPTIONS
    try {
#endif
      new (newref) reference(*ref);
...
reg_match<iterator, Allocator>::reg_match(
  ...
  try {
#endif
    new (reg_match_base<iterator, Allocator>::ref)
        typename reg_match_base<iterator, Allocator>::reference(*m.ref);
    ...
    try {
      ...
      p2 =
          (sub_match<iterator> *)(reg_match_base<iterator, Allocator>::ref + 1);
      while (p2 != p1) {
        jm_destroy(p2);
        ++p2;
      }
    ...
    m.ref->deallocate(
        (char *)(void *)reg_match_base<iterator, Allocator>::ref,
        sizeof(sub_match<iterator>) * m.ref->cmatches +
            sizeof(typename reg_match_base<iterator, Allocator>::reference));
    throw;
  }
...
template <class iterator, class Allocator>
reg_match<iterator, Allocator> &reg_match<iterator, Allocator>::operator=(
    const reg_match<iterator, Allocator> &m) {
...
iterator RE_CALL
re_is_set_member(iterator next, iterator last, re_set_long *set,
...
#include <jm/regcomp.h>
    ...
    JM_NAMESPACE(__JM)
        ...
        typedef reg_expression<char, char_regex_traits<char>,
                               JM_DEF_ALLOC(char)> regex;
#ifndef JM_NO_WCSTRING
typedef reg_expression<wchar_t, char_regex_traits<wchar_t>,
                       JM_DEF_ALLOC(wchar_t)>
    wregex;
#endif
