...
template <class O, class I> O RE_CALL re_copy_out(O out, I first, I last) {
  while (first != last) {
    *out = *first;
    ++out;
    ++first;
  ...
  while (*fmt) {
    ...
    case syntax_dollar:
      ...
      case syntax_start_buffer:
        oi_assign(&out, re_copy_out(out, iterator(m[-1].first),
                                    iterator(m[-1].second)));
        ++fmt;
        continue;
      case syntax_end_buffer:
        oi_assign(&out, re_copy_out(out, iterator(m[-2].first),
                                    iterator(m[-2].second)));
        ++fmt;
        continue;
      case syntax_digit: {
        unsigned int index =
            re_traits_type::toi(fmt, fmt_end, 10 MAYBE_PASS_LOCALE(l));
        oi_assign(&out, re_copy_out(out, iterator(m[index].first),
                                    iterator(m[index].second)));
        continue;
      }
      }
      // anything else:
      if (*fmt == '&') {
        oi_assign(&out, re_copy_out(out, iterator(m[0].first),
                                    iterator(m[0].second)));
        ++fmt;
      } else {
    ...
    case syntax_slash: {
      ...
      case syntax_x:
        ...
        // maybe have \x{ddd}
        if (re_traits_type::syntax_type(*fmt MAYBE_PASS_LOCALE(l)) ==
            syntax_open_brace) {
          ++fmt;
          if (fmt == fmt_end) {
            fmt -= 2;
          ...
          if (re_traits_type::is_class(
                  *fmt, char_class_xdigit MAYBE_PASS_LOCALE(l)) == false) {
            fmt -= 2;
            *out = *fmt;
            ++out;
            ++fmt;
          ...
          c = (charT)re_traits_type::toi(fmt, fmt_end,
                                         -16 MAYBE_PASS_LOCALE(l));
          if (re_traits_type::syntax_type(*fmt MAYBE_PASS_LOCALE(l)) !=
              syntax_close_brace) {
            while (re_traits_type::syntax_type(*fmt MAYBE_PASS_LOCALE(l)) !=
                   syntax_slash)
              --fmt;
            ++fmt;
            *out = *fmt;
      ...
      case syntax_c:
        ...
        if (((typename re_traits_type::uchar_type)(*fmt) <
            ...
            ((typename re_traits_type::uchar_type)(*fmt) >
             (typename re_traits_type::uchar_type)127)) {
          --fmt;
          *out = *fmt;
          ++out;
          ++fmt;
    ...
    case syntax_open_bracket:
      ++fmt; // recurse
      oi_assign(&out,
                __reg_format_aux(out, m, fmt, false MAYBE_PASS_LOCALE(l)));
      continue;
    ...
    case syntax_question: {
      ...
      if (m[id].matched) {
        ...
        if (re_traits_type::syntax_type(*(fmt - 1) MAYBE_PASS_LOCALE(l)) ==
            syntax_colon)
...
template <class OutputIterator, class iterator, class Allocator, class charT>
OutputIterator RE_CALL
reg_format(OutputIterator out, const reg_match<iterator, Allocator> &m,
           const charT *fmt
#ifdef RE_LOCALE_CPP
           ,
...
template <class S> class string_out_iterator {
  S *out;
...
__JM_STD::basic_string<charT>
    RE_CALL reg_format(const reg_match<iterator, Allocator> &m, const charT *fmt
#ifdef RE_LOCALE_CPP
                       ,
                       __JM_STD::locale locale_inst = __JM_STD::locale()
#endif
...
__JM_STD::string RE_CALL
reg_format(const reg_match<iterator, Allocator> &m, const char *fmt
#ifdef RE_LOCALE_CPP
           ,
           __JM_STD::locale locale_inst = __JM_STD::locale()
#endif
...
template <class OutputIterator, class iterator, class traits, class Allocator,
          class charT>
OutputIterator RE_CALL
reg_merge(OutputIterator out, iterator first, iterator last,
...
#endif
  iterator l = first;
  merge_out_predicate<OutputIterator, iterator, charT, Allocator> oi(
      out, l, fmt, copy MAYBE_PASS_LOCALE(locale_inst));
...
__JM_STD::basic_string<charT>
    RE_CALL reg_merge(const __JM_STD::basic_string<charT> &s,
                      const reg_expression<charT, traits, Allocator> &e,
                      const charT *fmt, bool copy = true,
...
__JM_STD::string RE_CALL reg_merge(
    const __JM_STD::string &s, const reg_expression<char, traits, Allocator> &e,
