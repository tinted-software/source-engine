...
// Purpose:		a wrapper for strtok, to return the next directory name
// out of a path Input:	path - the path (will be modified by the call to strtok)
// Output:	char*
//------------------------------------------------------------------------------------------------------
...
// Purpose:	 makes a directory hierarchy. because mkdir can't make nested
// dirs Input:	dir - the string of the path to make Output:	Returns true on
// success, false on failure. Note: Notice how easy the linux part of this
// function is...  no drive letters. :)
//------------------------------------------------------------------------------------------------------
bool CTFStatsOSInterface::makeHier(string dir) {
  ...
  // have to parse out directories one at a time. because mkdir just can't
  // handle making nested directories that don't exist (just like md.) in
  // otherwords, it's lame.

#ifdef WIN32
  // get drive out of path change to it.
  // if it's only one character, then interpret it as a path, make it and
  // return; only do this test because the tests below rely on at least 2
  ...
  if (path[0] == '\\' && path[1] == '\\') {
    Util::debug_dir_printf(
        ...
        "drive and specify that drive instead.\n");
    retval = false;
    goto end;
  }
  ...
  do {
    if (this->chdir(nextDir) != 0) {
      if (this->mkdir(nextDir) != 0) {
        char buf[500];
        Util::debug_dir_printf("Could not create directory (current directory "
                               "is %s) (failed on %s)\n",
      ...
      if (this->chdir(nextDir) != 0) {
        char buf[500];
        Util::debug_dir_printf(
            "Could not create directory (current directory is %s) failed on "
            "second attempt to change to %s\n",
            getcwd(buf, 500), nextDir);
        retval = false;
        goto end;
      }
...
bool CTFStatsLinuxInterface::findfirstfile(char *filemask, string &filename) {
  ...
  n = scandir(".", &namelist, CTFStatsLinuxInterface::filenameCompare,
              alphasort);
...
int CTFStatsLinuxInterface::filenameCompare(dirent *file) {
  ...
  bool result = query_match((const char *)file->d_name,
                            (const char *)(file->d_name + strlen(file->d_name)),
                            what, expression);
...
char *CTFStatsLinuxInterface::ultoa(unsigned long theNum, char *buf,
                                    int radix) {
  {
