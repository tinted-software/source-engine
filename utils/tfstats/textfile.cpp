...
//				eliminateComments - if true, C++ style comments
//will not be handed 				back as tokens to the user of this object
// Output:
//------------------------------------------------------------------------------------------------------
...
// Output:	const char*
//------------------------------------------------------------------------------------------------------
const char *CTextFile::getToken() { return getToken(wordBuf); }

//------------------------------------------------------------------------------------------------------
// Function:	CTextFile::getToken
// Purpose:	gets and returns the next token in the file, with an arbitrary
// buffer Input:	outputBuf - the buffer to put the token in Output:
// const char*
//------------------------------------------------------------------------------------------------------
...
readToken:
  ...
  while (!isDelim(c)) {
    ...
    // these are both in the normal delimiter set, so this case won't happen
    // unless we're reading a string which is exactly the behaviour we want
    if (c == '\n' || c == '\r') {
      outputBuf[write] = 0;
      g_pApp->fatalError("new line in string constant (or unterminated string "
                         "constant):\n\"%s\"",
                         outputBuf);
    }
    outputBuf[write++] = c;
...
// Purpose:	discards the next token, also checks to see if it is the same as
// the token
...
bool CTextFile::discard(char *test) {
  ...
  if (result != 0)
    g_pApp->fatalError("While parsing %s, expecting \"%s\", got \"%s\"",
                       filename.c_str(), test, wordBuf2);
...
// Purpose: "pushes" a token back into the stream.  Only can have one pushed
// back
//		token at a time. getting a token or peeking will destroy the
//pushed back token
...
//------------------------------------------------------------------------------------------------------
void CTextFile::reset() { fseek(theFile, 0, SEEK_SET); }
