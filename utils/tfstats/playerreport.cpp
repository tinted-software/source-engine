...
void CPlayerReport::writeHTML(CHTMLFile &html) {
  // if we're writing the stats for a persistent player, just pass execution off
  // to that function!
  ...
  html.write("<font class=whitetext>rank:</font> <font class=awards2> %.2lf "
             "</font><br>\n",
             pPlayer->perteam[tid].rank());
  html.write("<font class=whitetext>kills/deaths:</font> <font "
             "class=awards2>%li/%li </font><br>\n",
             pPlayer->perteam[tid].kills, pPlayer->perteam[tid].deaths);
  html.write("<font class=whitetext>time:</font> <font class=awards2> "
             "%01li:%02li:%02li </font><br>\n",
             Util::time_t2hours(pPlayer->perteam[tid].timeOn()),
             Util::time_t2mins(pPlayer->perteam[tid].timeOn()),
  ...
  if (numClassesPlayed == 1) {
    if (faveClass != PC_UNDEFINED)
      html.write("<font class=whitetext>class:</font> <font class=awards2> %s "
                 "</font><br>\n",
                 plrClassNames[faveClass]);
  } else if (numClassesPlayed > 1) {
    if (faveClass != PC_UNDEFINED)
      html.write("<font class=whitetext>favorite class:</font> <font "
                 "class=awards2> %s </font><br>\n",
                 plrClassNames[faveClass]);

    html.write(
        "<font class=whitetext>classes played:</font> <font class=awards2> ");
  ...
  if (pPlayer->perteam[tid].kills != 0) {
    ...
    html.write("<font class=whitetext>favorite weapon:</font> <font "
               "class=awards2> %s</font><br>\n",
               faveWeap.c_str());
    html.write("<font class=whitetext>kills with %s:</font> <font "
               "class=awards2> %li</font><br>\n",
  ...
  if (numTeamsPlayed > 1) {
    if (iWhichTeam == ALL_TEAMS)
      html.write(
          "<font class=whitetext>Played on</font> <font class=whitetext> ");
    else
      html.write("<font class=whitetext>Also played on</font> <font "
                 "class=whitetext> ");
    ...
    for (tmiter; tmiter != pPlayer->teams.end(); ++tmiter) {
      ...
      if (team != iWhichTeam && !alreadyPrinted[team]) {
        if (printed1)
          html.write(" and ");
        html.write("<font class=player%s>%s</font>", Util::teamcolormap[team],
                   Util::teamcolormap[team]); //
        printed1 = true;
  ...
  if (numTeamsPlayed > 1 && iWhichTeam != ALL_TEAMS) {
    html.write("<a class=whitetext href=\"%lu.html\"> <u> Combined stats for "
               "this match </u> </a> <br> \n",
               pPlayer->pid);
    ...
    if (!alreadyWroteCombStats[pPlayer->pid]) {
      ...
      sprintf(namebuf, "Combined match statistics for %s",
              pPlayer->name.c_str());
  ...
  if (g_pApp->cmdLineSwitches["persistplayerstats"] == "yes" &&
    ...
    html.write("<a class=whitetext href=\"%s/allplayers.html#%lu\"> <u> "
               "Combined stats on this server </u> </a> <br> \n",
               g_pApp->playerHTTPPath.c_str(), pPlayer->WONID,
               pPlayer->name.c_str());
...
// Purpose:		writes a persistent player's stats out. these look
// slightly different
...
void CPlayerReport::writePersistHTML(CHTMLFile &html) {
  ...
  for (nmiter; nmiter != pPersist->nickmap.end(); ++nmiter) {
    if (namePrinted[nmiter->first] != true) {
      if (!printed1)
        html.write("<font class=whitetext>other names used:</font> <font "
                   "class=awards2>");
      if (printed1)
        html.write(", ");
  ...
  html.write("<font class=whitetext>time:</font> <font class=awards2> "
             "%01li:%02li:%02li </font><br>\n",
             Util::time_t2hours(pPersist->timeon),
             Util::time_t2mins(pPersist->timeon),
             Util::time_t2secs(pPersist->timeon));
  html.write("<font class=whitetext>matches played:</font> <font "
             "class=awards2> %li </font><br>\n",
             pPersist->matches);
  ...
  for (classit; classit != pPersist->classmap.end(); ++classit) {
    if (classPrinted[classit->first] == false) {
      if (!printedone)
        html.write("<font class=whitetext>other classes played:</font> <font "
                   "class=awards2> ");
      if (printedone)
        html.write(", ");
