...
void *operator new(size_t size, const char *file, const int line) {
  ...
  numBytesAllocated += size;
  cout << "new: Allocating " << size << " bytes in file " << file << ", line "
       << line << ", address is " << ptr << " (" << numBytesAllocated
...
void operator delete(void *ptr, const char *file, const int line) {
  cout << "delete: Freeing memory allocated at file " << file << ", line "
       << line << ", address is " << ptr << endl;
  delete[] (char *)ptr;
}
...
// Purpose:	this function will be called if TFStats runs out of memory
// (unlikely)
//		this is a win32 specific version, the linux version does not
//pass an argument
...
// Purpose:	this function will be called if TFStats runs out of memory
// (unlikely)
//		this is a linux specific version, the win32 version passes an
//argument
//------------------------------------------------------------------------------------------------------
void TFStats_linux_new_handler(void) { printf("TFStats ran out of memory!\n"); }
