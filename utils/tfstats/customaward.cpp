...
// Purpose:	writes extra info about the award winner, like their score or
// something. The extra info string is defined by the user in the configuration
// file like the
...
// Purpose:	 writes some html saying that no one won this award. The
// noWinnerMsg is defined by the user in the configuration file like all other
// custom award properties Input:	html - the html file to write to
//------------------------------------------------------------------------------------------------------
...
// Purpose:	 Factory method to read an award from a config file and return
// an instance of the CCustomAward class Input:	f - the configuration file to
// read from
//				g_pMatchInfo - a pointer to a matchinfo object
//to give to the new award
// Output:	CCustomAward*
//------------------------------------------------------------------------------------------------------
CCustomAward *CCustomAward::readCustomAward(CTextFile &f) {
  ...
  while (token) {
    if (stricmp(token, "trigger") == 0) {
      ...
      g_pApp->fatalError("Unrecognized Award property name while parsing %s:  "
                         "\"%s\" is not a property of an Award!",
...
void CCustomAward::getWinner() {
  ...
  for (it = g_pMatchInfo->eventList()->begin();
    ...
    for (tli = triggers.begin(); tli != triggers.end(); ++tli) {
      if ((*tli)->matches(*it)) {
        ...
        // if this returns -1, store based on name. (this way we can give awards
        // to things other than player names while remaining in this
        // award/trigger hierarchy structure)
  ...
  if (!namemode) {
    ...
    for (scores_it = plrscores.begin(); scores_it != plrscores.end();
         ++scores_it) {
    ...
    for (scores_it = stringscores.begin(); scores_it != stringscores.end();
         ++scores_it) {
