...
void CSentryRebuildAward::getWinner() {
  ...
  for (builditer = numbuilds.begin(); builditer != numbuilds.end();
       ++builditer) {
...
void CSentryRebuildAward::extendedinfo(CHTMLFile &html) {
  ...
  if (numbuilds[winnerID] == 1)
    html.write("No one had to rebuild a sentry gun in this match.!",
               winnerName.c_str());
  else
    html.write("%s had to build a sentry gun %li times!", winnerName.c_str(),
               numbuilds[winnerID]);
}
