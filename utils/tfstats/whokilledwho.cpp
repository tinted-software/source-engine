...
void CWhoKilledWho::generate() {
  ...
  // now the pids table is all full of pids and you index it using Board IDs
  // (bids) also there is a bids table so you know which pid each board entry
  // has.
  ...
  for (it = g_pMatchInfo->eventList()->begin();
    ...
    if ((*it)->getType() == CLogEvent::SUICIDE ||
      ...
      deaths[plrbid]++;
               ...
               (*it)->getType() == CLogEvent::TEAM_FRAG) {
...
// Purpose:	 Helper function that returns the class of a cell on the board
// depending
...
// Purpose:	takes the intermediate data generated by generate() and writes
// out
...
void CWhoKilledWho::writeHTML(CHTMLFile &html) {
  ...
  html.write("<script language=\"JavaScript\"> "
             "thisBrowser.writeDetailTableTag(%li,%li,%li); </script>\n",
             tableWid, numrows, numcols);

  // print columns
  html.write("<tr class=header align=center> <td width=%li align =left><font "
             "class=boardtext>Player</font></td>\n",
             playerNameWid);
  for (i = 0; i < size; i++) {
    ...
    html.write("<td width=%li><font class=player%s>%2d</font></th>\n", cellWid,
               Util::teamcolormap[tid], i);
  }
  html.write(
      "<td align=left width=%li><font class=boardtext>Kills</font></th>\n",
      lastColWid);
  html.write("</tr>\n");
  ...
  for (killerbid = 0; killerbid < size; ++killerbid) {
    ...
    strncpy(truncatedPlayerName,
            g_pMatchInfo->playerName(bidMap2[killerbid].second).c_str(), 20);
    truncatedPlayerName[20] = 0;
    // int tid=g_pMatchInfo->playerTeamID(pids[killerbid]);
    int tid = bidMap2[killerbid].first;
    html.write("<tr><td class=boardcell_b align=left width=%li><font "
               "class=player%s><nobr>%d. %s</nobr></font></th>\n",
               playerNameWid, Util::teamcolormap[tid], killerbid,
               truncatedPlayerName);

    int killeebid;
    for (killeebid = 0; killeebid < size; ++killeebid) {
      const char *tdclass = getCellClass(killeebid, killerbid);
      html.write("<td align=center width=%li %s><font "
                 "class=boardtext>%i</font></td>\n",
    ...
    totalkills += tot;
    html.write(
        "<td align=center width=%li><font class=boardtext>%i</font></td>\n",
        lastColWid, tot);
    html.write("</tr>\n");
  }
  ...
  // html.write("<td align=left ><font
  // class=boardtext>%3i\\%3i</font></td>\n",totaldeaths,totalkills);
  html.write("<td></td>\n");
  html.write("</tr>\n");
  html.write("</table>\n");
}
