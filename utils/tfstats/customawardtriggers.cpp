...
// Purpose:	reads a trigger definition from the given rule file and returns
// a new trigger Input:	f - a pointer to the TextFile object that represents the
// rule file to read from Output:	CCustomAwardTrigger*
//------------------------------------------------------------------------------------------------------
CCustomAwardTrigger *CCustomAwardTrigger::readTrigger(CTextFile &f) {
  ...
  while (token) {
    if (stricmp(token, "value") == 0) {
      ...
      char lowerbuf[500];
      char lowerbuf2[500];
      // oops, have to do this first.  CTextfile uses a static buffer to return
      // strings
  ...
  if (type == "broadcast")
    ...
    g_pApp->fatalError(
        "Invalid trigger type while parsing %s:\n\"%s\" is not a valid trigger "
        "type, please use \"broadcast\", \"goal\"  or \"fullsearch\"",
...
//				teamValue - the teamValue of the trigger (not
//used) 				keys - strings to search for in the text of any broadcast event
//------------------------------------------------------------------------------------------------------
CBroadcastTrigger::CBroadcastTrigger(int value, int teamValue,
                                     vector<string> &keys,
                                     map<string, string> extras)
    : CCustomAwardTrigger(value, teamValue, extras) {
  // this line works in win32, but not in G++... g++ doesn't seem to have
  // vector::assign broadcastStrings.assign(keys.begin(),keys.end());
...
// Purpose:	 Determines if a given event is a broadcast and  matches any of
// the keys Input:	le - the event we're testing Output:	Returns true if
// the given event triggers this trigger
//------------------------------------------------------------------------------------------------------
bool CBroadcastTrigger::matches(const CLogEvent *le) {
  if (le->getType() == CLogEvent::NAMED_BROADCAST) // || le->getType() ==
                                                   // CLogEvent::ANON_BROADCAST)
  {
    // broadcastID is arg0
...
// Input:	value - the value of the trigger relative to other triggers
//				teamValue - the teamValue of the trigger (not
//used) 				keys - the names of goals that will cause this trigger to trigger
//------------------------------------------------------------------------------------------------------
CGoalTrigger::CGoalTrigger(int value, int teamValue, vector<string> &keys,
                           map<string, string> extras)
    : CCustomAwardTrigger(value, teamValue, extras) {
  // this line works in win32, but not in G++... g++ doesn't seem to have
  // vector::assign goalNames.assign(keys.begin(),keys.end());
...
// Purpose:	 Determines if a given event is a goal activation and  matches
// any of the keys Input:	le - the event we're testing Output:	Returns
// true if the given event triggers this trigger
//------------------------------------------------------------------------------------------------------
...
// Input:	value - the value of the trigger relative to other triggers
//				teamValue - the teamValue of the trigger (not
//used) 				ks - the names of FullSearchs that will cause this trigger to trigger
//------------------------------------------------------------------------------------------------------
CFullSearchTrigger::CFullSearchTrigger(int value, int teamValue,
                                       vector<string> &ks,
                                       map<string, string> extras)
    : CCustomAwardTrigger(value, teamValue, extras) {
  // this line works in win32, but not in G++... g++ doesn't seem to have
  // vector::assign FullSearchNames.assign(keys.begin(),keys.end());
...
int regExprCompare(string sexpr, string scmp) {
  ...
  cmatch what;
  if (query_match(scmp.c_str(), scmp.c_str() + strlen(scmp.c_str()), what,
                  expression)) {
    // matched!
    return 0;
  } else
...
bool CFullSearchTrigger::compare(string str_msg, string str_key,
...
#else
  if (winnerVar.at(0) == '%' && winnerVar.at(1) == 'p' &&
      winnerVar.at(2) == 'l' && winnerVar.at(3) == 'r' &&
      winnerVar.at(4) == '_')
#endif
  {
    map<string, string> varmatches;
