...
// Purpose:	writes out html displaying extra information about the winning
// of this award Input:	html - the html file to output to
//------------------------------------------------------------------------------------------------------
void CAssaultCannonAward::extendedinfo(CHTMLFile &html) {
  if (accum[winnerID] == 1)
    html.write("%s got 1 kill with the Assault Cannon!", winnerName.c_str());
  else
    html.write("%s got %li kills with the Assault Cannon!", winnerName.c_str(),
               accum[winnerID]);
}
...
void CKnifeAward::extendedinfo(CHTMLFile &html) {
  if (accum[winnerID] == 1)
    ...
    html.write("%s backstabbed %li people!", winnerName.c_str(),
               accum[winnerID]);
}
...
void CRocketryAward::extendedinfo(CHTMLFile &html) {
  if (accum[winnerID] == 1)
    ...
    html.write("%s rocketed %li people to oblivion!", winnerName.c_str(),
               accum[winnerID]);
}
...
void CGrenadierAward::extendedinfo(CHTMLFile &html) {
  if (accum[winnerID] == 1)
    ...
    html.write("%s got %li people with grenades!", winnerName.c_str(),
               accum[winnerID]);
}
...
void CDemolitionsAward::extendedinfo(CHTMLFile &html) {
  if (accum[winnerID] == 1)
    ...
    html.write("%s smeared %li people with detpacks!", winnerName.c_str(),
               accum[winnerID]);
}
...
void CBiologicalWarfareAward::extendedinfo(CHTMLFile &html) {
  if (accum[winnerID] == 1)
    ...
    html.write("%li people succumbed to %s's disease.", accum[winnerID],
               winnerName.c_str());
}
...
void CBestSentryAward::extendedinfo(CHTMLFile &html) {
  if (accum[winnerID] == 1)
    ...
    html.write("%li people were killed by %s's well placed sentries.",
               accum[winnerID], winnerName.c_str());
}
