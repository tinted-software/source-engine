...
void CMatchInfo::generate() {
  if (plogfile->empty())
    g_pApp->fatalError("No data in log file!\nPlease ensure that you are "
                       "running TFstats on a valid log file!");
  ...
  for (it; it != plogfile->end(); ++it) {
    ...
    case CLogEvent::CLASS_CHANGE: {
      ...
      player_class newpc =
          playerClassNameToClassID(curr->getArgument(1)->getStringValue());

      // keep the pseudonym list updated
      players[pid].nameFound(curr->getTime(),
    ...
    case CLogEvent::NAME_CHANGE: {
      // keep the pseudonym list updated
      players[curr->getArgument(0)->asPlayerGetPID()].nameFound(
          curr->getTime(), curr->getArgument(1)->asPlayerGetName());
    } break;
    ...
    case CLogEvent::TEAM_FRAG: {
      ...
      // keep the pseudonym list updated
      players[killerid].nameFound(curr->getTime(),
                                  curr->getArgument(0)->asPlayerGetName());
      players[killedid].nameFound(curr->getTime(),
    ...
    case CLogEvent::TEAM_JOIN: {
      ...
      if (p.allclassesplayed.anythingAtTime(curr->getTime())) {
        player_class plrcurrclass =
            players[pid].allclassesplayed.atTime(curr->getTime());
        players[pid].perteam[oldteam].classesplayed.cut(curr->getTime());
        players[pid].perteam[team].classesplayed.add(curr->getTime(),
                                                     plrcurrclass);
      }
    } break;
    ...
    case CLogEvent::NAMED_GOAL_ACTIVATE: {
      // keep the pseudonym list updated
      players[curr->getArgument(0)->asPlayerGetPID()].nameFound(
          curr->getTime(), curr->getArgument(0)->asPlayerGetName());
    } break;
  ...
  for (it2 = players.begin(); it2 != players.end(); ++it2) {
    ...
    for (int i = 0; i < MAX_TEAMS; i++) {
      // if you have no kills you have to play on a team at least 30 seconds to
      // be counted part of it also give a one-suicide grace so they can
      // killthemselves to get onto another team?
      if (p.teams.howLong(i) < 30 &&
          p.perteam[i].kills == 0) // && p.perteam[i].deaths < 1)
        p.teams.remove(i);
