  ...
  for (it = g_pMatchInfo->eventList()->begin();
    ...
    if ((*it)->getType() == CLogEvent::FRAG) {
      if (strcmp((*it)->getArgument(2)->getStringValue(), "sniperrifle") == 0) {
        ...
        numshots[pid]++;
      } else if (strcmp((*it)->getArgument(2)->getStringValue(), "headshot") ==
                 0) {
        PID pid = (*it)->getArgument(0)->asPlayerGetPID();
...
void CSharpshooterAward::extendedinfo(CHTMLFile &html) {
  ...
  if (hs && shots)
    html.write("%s got %li headshots and %li normal shots!", winnerName.c_str(),
               hs, shots);
  else if (hs && !shots)
    html.write("All of %s's %li snipes were headshots!", winnerName.c_str(),
               hs);
  else if (shots && !hs)
