...
CSwitchProps::CSwitchProps(CWnd *pParent /*=NULL*/)
    : CPropertyPage(CSwitchProps::IDD), m_persistDefall("AllSwitchesDefault"),
      ...
      m_persistElimDays("DefaultElimDays"), m_persistPause("DefaultPause"),
...
BOOL CSwitchProps::OnKillActive() {
  ...
  m_persistPersistPlayerStats = theApp.persistPlayerStats =
      m_PersistPlayerStats.GetCheck() == 1;
  m_persistUseSupportDir = theApp.useSupportDir =
      m_UseSupportDir.GetCheck() == 1;
  m_persistDisplayStartupInfo = theApp.displayStartupInfo =
      m_DisplayStartupInfo.GetCheck() == 1;
...
void CSwitchProps::OnPlrpersist() {
  m_ElimOldPlrs.EnableWindow(m_PersistPlayerStats.GetCheck() &&
                             !m_Defall.GetCheck());
  m_elimDays.EnableWindow(m_PersistPlayerStats.GetCheck() &&
                          !m_Defall.GetCheck());
  m_OnlyHereToBeDisabledToo.EnableWindow(m_PersistPlayerStats.GetCheck() &&
                                         !m_Defall.GetCheck());
}
