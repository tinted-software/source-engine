...
// Purpose:	fills in the fields of this with the data in the given CPlayer
// object Input:	cp - the player object to get data from
//------------------------------------------------------------------------------------------------------
void CPlrPersist::generate(CPlayer &cp) {
  ...
  for (nameiter = cp.aliases.begin(); nameiter != cp.aliases.end();
       ++nameiter) {
...
//	This is the key operation of this class.  This is how player stats are
//kept up 	to date over time.  If the two data files have playtimes that overlap,
//they 	are not merged (unless the mergeOverlaps flag is true)
// Input:	other - the CPlrPersist object that we want to merge into this
// one
//				mergeOverlaps - if true, overlapping playtimes
//are ignored.
//------------------------------------------------------------------------------------------------------
void CPlrPersist::merge(CPlrPersist &other, bool mergeOverlaps) {
  ...
  if (WONID != other.WONID) {
    g_pApp->warning("merging stats for two different WONIDs (%lu, %lu)", WONID,
                    other.WONID);
    ...
    for (itOther; itOther != other.playtimes.end(); ++itOther) {
      list<pair<time_t, time_t>>::iterator overlap =
          timesOverlap(itOther->first, itOther->second);
      ...
      if (mergeOverlaps || overlap == playtimes.end())
        ...
        g_pApp->warning("not merging stats for WON ID# %lu, playtime ranges "
                        "overlap\n\t((%lu-%lu) overlaps with (%lu-%lu))",
                        WONID, itOther->first, itOther->second, overlap->first,
                        overlap->second);
        return;
      }
    }
...
//------------------------------------------------------------------------------------------------------
// Function:	CPlrPersist::read
// Purpose:	 converts the WONID to a file name (<wonid>.tfs) and passes
// execution
...
void CPlrPersist::write() {
  ...
  for (it; it != nickmap.end(); ++it) {
    ...
    int time = it->second;
    fprintf(fout,
            "\t\"%s\" %li //has used the name \"%s\" for %02li:%02li:%02li\n",
            name.c_str(), time, name.c_str(), Util::time_t2hours(time),
            Util::time_t2mins(time), Util::time_t2secs(time));
  }
  ...
  for (it; it != classmap.end(); ++it) {
    ...
    int time = it->second;
    fprintf(fout,
            "\t\"%s\" %li //has played as a \"%s\" for %02li:%02li:%02li\n",
            name.c_str(), time, name.c_str(), Util::time_t2hours(time),
            Util::time_t2mins(time), Util::time_t2secs(time));
  }
  ...
  for (it2; it2 != playtimes.end(); ++it2) {
    ...
    list<pair<time_t, time_t>>::iterator overlap =
        timesOverlap(it2->first, it2->second, false);
    ...
    if (doesOverlap)
      fprintf(fout, "Warning! overlaps with time range (%lu-%lu)",
...
list<pair<time_t, time_t>>::iterator
CPlrPersist::timesOverlap(time_t start, time_t end, bool testself) {
...
string CPlrPersist::faveName() { return faveString(nickmap); }
string CPlrPersist::faveWeap() {
  string s = faveString(weapmap);
  faveweapkills = weapmap[s];
  return s;
}
string CPlrPersist::faveClass() { return faveString(classmap); }
