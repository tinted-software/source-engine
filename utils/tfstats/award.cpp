...
// Purpose:	overrides generate to call the more-semantically correct
// getWinner() when dealing with subclasses of awards. Also after getWinner has
// determined which PID is the winner, this assigns the correct name to the
// winnerName field
//------------------------------------------------------------------------------------------------------
...
void CAward::writeHTML(CHTMLFile &html) {
  if (fNoWinner || (winnerID == -1 && winnerName == ""))
    ...
    html.write("The <font class=brightawards>%s</font> award goes to %s!    ",
               awardName.c_str(), winnerName.c_str());
    extendedinfo(html);
  }
  html.br();
  html.write("\n");
