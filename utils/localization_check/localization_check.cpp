          ...
          if (internal && internal->NumSoundNames() > 0) {
            ...
            if (wavname &&
              ...
              for (int w = 0; w < internal->NumSoundNames(); ++w) {
                wavname = g_pSoundEmitterSystem->GetWaveName(
...
void MakeBatchFile(CUtlVector<CUtlSymbol> &wavfiles, char const *pchFromdir,
                   char const *pchTodir) {
  ...
  for (int i = 0; i < c; ++i) {
    ...
    char fullname[512];
    Q_snprintf(
        fullname, sizeof(fullname), "%s/%s", pchTodir,
...
struct OrderedCaption_t {
  OrderedCaption_t()
      : sym(UTL_INVAL_SYMBOL), commands(NULL), english(NULL),
        blankenglish(false) {}
...
int CLocalizationCheckApp::Main() {
    ...
    if (pArg[0] == '-') {
      ...
      case 'l':
        if (!Q_stricmp(&pArg[1], "loop")) {
          checkforloops = true;
          Q_strncpy(sounddir, CommandLine()->GetParm(iArg + 1),
                    sizeof(sounddir));
          iArg++;
        } else {
          uselogfile = true;
      ...
      case 'i': {
        importcaptions = true;
        Q_strncpy(importfile, CommandLine()->GetParm(iArg + 1),
                  sizeof(importfile));
        iArg++;
      } break;
      case 'm': {
  ...
  if (forceducking) {
    ...
    if (extractenglish && !language[0]) {
      ...
      if (language[0] != 0) {
        vprint(0, "\nChecking for missing or out of date localized combined "
                  ".wav files\n\n");
