...
void Worker_ProcessWorkUnitFn(int iThread, uint64 iWorkUnit,
  ...
  for (i = textureStart; i < textureEnd; i++) {
    ...
    if (fp) {
      ...
      //= true; 				CUtlVector<char> fileNameBuf; 				fileNameBuf.AddToTail( ( char
      //)TEXTUREHADERROR_PACKETID ); 				int len = strlen( g_CompileCommands[i] );
      //				fileNameBuf.AddMultipleToTail( len + 1,
      //g_CompileCommands[i] ); 				VMPI_SendData( fileNameBuf.Base(),
      //fileNameBuf.Count(), VMPI_MASTER_ID );
      //			}
...
int TextureCompile_Main(int argc, char *argv[]) {
  ...
  if (g_bMPIMaster) {
    /*
                    ...
                    {
                            if( g_Master_TextureHadError.Defined(
    g_ByteCode.String( i ) ) )
                            {
                                    printf( "FAILED: \"%s\"\n",
    g_ByteCode.String( i ) );
                            ...
                            {
    //				printf( "\"%s\" succeeded!\n",
    g_ByteCode.String( i ) ); WriteTexture( g_ByteCode.String( i ) );
                            }
                    }
                    ...
                    char str[512];
                    GetHourMinuteSecondsString( (int)( end - g_flStartTime ),
    str, sizeof( str ) ); Msg( "%s elapsed\n", str );
    */
  }
  return 0;
}
