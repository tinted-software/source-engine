...
struct DeviceRecord_t {
  char szName[256];
  char szData[MAX_RECORDS]
             [64]; // No more than MAX_RECORDS columns of data in dxsupport.cfg
};
...
static void GrabSurveyData(char const *pInputFilename,
                           char const *pOutputFilename) {
  CRequiredInputTextFile f(pInputFilename);
  COutputTextFile o(pOutputFilename);
  char linebuffer[4096], szDummy[32], szVendorID[32], szDeviceID[32],
      szPartName[256], goodLine[] = "$vend_dev_to_name[ MakeLookup(";

  while (f.ReadLine(linebuffer, sizeof(linebuffer))) {
    if (!strncmp(linebuffer, goodLine,
                 strlen(goodLine))) // specific pattern on the line we want
    {
...
static bool DidMerge(CUtlVector<DeviceRecord_t> &deviceRecords, int nRecord) {
  ...
  if (!V_strnicmp(deviceRecords[nRecord].szName,
                  deviceRecords[nRecord + 1].szName, 256) &&
      !V_strnicmp(deviceRecords[nRecord].szData[g_nVendorIDIndex],
                  deviceRecords[nRecord + 1].szData[g_nVendorIDIndex], 64)) {
    // Convert hex device ID strings into integers for comparisons
    int nMinIDCur, nMaxIDCur, nMinIDNext, nMaxIDNext;
    sscanf(deviceRecords[nRecord].szData[g_nDeviceMinIDIndex], "%x",
           &nMinIDCur);
    sscanf(deviceRecords[nRecord].szData[g_nDeviceMaxIDIndex], "%x",
           &nMaxIDCur);
    sscanf(deviceRecords[nRecord + 1].szData[g_nDeviceMinIDIndex], "%x",
           &nMinIDNext);
    sscanf(deviceRecords[nRecord + 1].szData[g_nDeviceMaxIDIndex], "%x",
           &nMaxIDNext);

    // If the ranges overlap or are adjacent, merge the data
    if ((nMinIDNext >= nMinIDCur) && (nMinIDNext <= (nMaxIDCur + 1))) {
      // Set current max to max of the two ranges, merge other elements and flag
      // next record for deletion
      V_snprintf(deviceRecords[nRecord].szData[g_nDeviceMaxIDIndex], 64,
                 "0x%04x", MAX(nMaxIDNext, nMaxIDCur));

      for (int i = g_nDeviceMaxIDIndex + 1; i < g_nNumDXSupportColumns;
           i++) // Run through elements after maxID
      {
        if (strlen(deviceRecords[nRecord + 1].szData[i]) !=
            0) // If the next record has any data in this element
        {
          if (V_strnicmp(
                  deviceRecords[nRecord].szData[i],
                  deviceRecords[nRecord + 1].szData[i],
                  64)) // If it doesn't match the current record's element
          {
            ...
            } else // This is the interesting case, where we have a bona fide
                   // conflict
            {
              V_strncat(deviceRecords[nRecord].szData[i], " FIXME ",
                        64); // Mark the element with ***
              V_strncat(deviceRecords[nRecord].szData[i],
                        deviceRecords[nRecord + 1].szData[i],
...
// Look for duplicate adjacent entries (same vendorID and min/max, ignoring
// device name string)
static bool FoundDuplicate(CUtlVector<DeviceRecord_t> &deviceRecords,
                           int nRecord) {
  // Spew cases with same vendor ID and device min/maxes...these are likely the
  // same device
  if (!V_strnicmp(deviceRecords[nRecord].szData[g_nVendorIDIndex],
                  deviceRecords[nRecord + 1].szData[g_nVendorIDIndex], 64)) {
    // Convert hex device ID strings into integers for comparisons
    int nMinIDCur, nMaxIDCur, nMinIDNext, nMaxIDNext;
    sscanf(deviceRecords[nRecord].szData[g_nDeviceMinIDIndex], "%x",
    ...
    if ((nMinIDCur == nMinIDNext) && (nMaxIDCur == nMaxIDNext)) {
      printf("Duplicate: %s  %s  0x%04x  0x%04x\n",
             ...
             deviceRecords[nRecord].szData[g_nVendorIDIndex], nMinIDCur,
             nMaxIDCur);
...
static void CleanCSVFile(char const *pInputFilename,
                         char const *pOutputFilename) {
  CRequiredInputTextFile f(pInputFilename);
  COutputTextFile o(pOutputFilename);
  ...
  // At this point, the output file has the header block from the file and we
  // have records for all of the real device lines
  ...
  //
  i = 0;
  while (i < deviceRecords.Count() -
                 1) // March through devices, looking for duplicates
  {
    if (FoundDuplicate(deviceRecords,
                       i)) // Does i+1 look like a duplicate of i?
    {
      deviceRecords.Remove(i + 1); // Remove i+1 and check with the same i again
      continue;                    // don't increment
    }
  ...
  for (int i = 0; i < deviceRecords.Count(); i++) {
    ...
    V_snprintf(deviceRecords[i].szData[g_nDeviceMinIDIndex], 64, "0x%04x",
               nDummy);

    sscanf(deviceRecords[i].szData[g_nDeviceMaxIDIndex], "%x", &nDummy);
    V_snprintf(deviceRecords[i].szData[g_nDeviceMaxIDIndex], 64, "0x%04x",
               nDummy);
  }
...
void main(int argc, char **argv) {
  ...
  if (argc != 5) {
    printf("\n");
    printf("  dxsupportclean output depends on the input file type (.csv or "
           "not).\n");
    printf("  The output is always a CSV file.\n\n");
    printf("   usage: dxsupportclean -i <inputfile> -o <outputfile.csv>\n");
    printf("          If the input file is not csv, dxsupportclean expects "
           "hardware\n");
    printf("          survey php syntax.  If the input file is csv, "
           "dxsupportclean expects\n");
    printf("          the first line to contain specific header columns.\n");
    printf("          If you want to provide some other type of input or if "
           "you modify the\n");
    printf("          header columns in dxsupport, you'll have to modify "
           "dxsupportclean.\n");
    return;
  }
  ...
  if (!V_strnicmp(pInputFile, pOutputFile, MAX_PATH)) {
    printf("Input and out files must not be the same file!  No output "
           "produced.\n");
    return;
  }
  ...
  if (!stricmp("csv", Q_GetFileExtension(pInputFile))) {
    ...
    if (g_bMergeConflict) {
      printf("\n***************************************************************"
             "**********\n");
      printf(" Merge Conflict detected.  Search output for FIXME and resolve "
             "manually!\n");
      printf("*****************************************************************"
             "********\n\n");
    }
