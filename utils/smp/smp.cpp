...
LRESULT CALLBACK WinProc(HWND hWnd, UINT msg, WPARAM wparam, LPARAM lparam) {
  ...
  case WM_CREATE: {
    ...
    SetTimer(hWnd, ID_SKIP_FADE_TIMER, 1000,
             NULL); // if the fade doesn't start in 1 second, then just jump to
                    // the video
    ...
    if (wparam == ID_DRAW_TIMER) {
      ...
      if (g_timeAtFadeStart == 0.0) {
        ...
        SetTimer(hWnd, ID_SKIP_FADE_TIMER, 100 + UINT(FADE_TIME * 1000),
                 NULL); // restart skip fade timer and give it an extra 100ms to
                        // allow the fade to draw fully black once
      }
...
*/
      ...
      if (!BitBlt((HDC)wparam, 0, 0, g_screenWidth, g_screenHeight, g_hdcBlend,
                  0, 0, SRCCOPY)) {
...
//RDW_INVALIDATE );
    ...
    //	case WM_TIMER:
    {
      if (msg == WM_TIMER) {
        /*
                                        char str[ 256 ];
                                        sprintf( str, "Timer 0x%x triggered for
