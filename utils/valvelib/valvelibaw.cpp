...
static void SetupCompilerSettings(ProjectInfo_t &info) {
  ...
  case CONFIG_DEBUG:
    // Remove the following switches (avoid duplicates, in addition to other
    // things)
    ...
    // Edit and Continue. /Zi - Includes debug information in a program database
    // (no Edit and Continue) /Od - Disable optimization /GZ - Catch release
    // build problems (uninitialized variables, fp stack problems) /Op -
    // Improves floating-point consistency /Gm - Minimal rebuild /FR - Generate
    // Browse Info
  ...
  case CONFIG_RELEASE:
    ...
    // /ZI - Includes debug information in a program database compatible with
    // Edit and Continue. /Gm - Minimal rebuild /GZ - Catch release build
    // problems (uninitialized variables, fp stack problems)
...
static void SetupLinkerSettings(ProjectInfo_t &info) {
  ...
  // Remove windows libraries by default... hopefully we don't have windows
  // dependencies
  varSwitch = "kernel32.lib user32.lib gdi32.lib winspool.lib comdlg32.lib "
...
static void SetupCustomBuildSteps(ProjectInfo_t &info) {
  ...
  if (info.m_Project == PROJECT_LIB) {
    ...
    targetPath.Format("%s%s.%s", info.m_RelativeTargetPath, info.m_RootName,
                      info.m_Ext);

    // NOTE: The second attrib is commented out because I'm fairly certain it
    // causes a bug in VSS to make it overwrite the target on a get
    copySteps.Format(
  ...
  CString publishDir;
  publishDir.Format("Publishing to target directory (%s)...",
                    info.m_RelativeTargetPath);
  info.m_pConfig->AddCustomBuildStep(
      (char const *)copySteps, (char const *)targets, (char const *)publishDir);
}
...
void CValvelibAppWiz::CustomizeProject(IBuildProject *pProject) {
  ...
  ProjectInfo_t info;
  if (!Valvelibaw.m_Dictionary.Lookup("VALVE_RELATIVE_PATH",
                                      info.m_RelativeTargetPath))
    return;
  ...
  if (!Valvelibaw.m_Dictionary.Lookup("VALVE_TARGET_TYPE", info.m_Ext))
    return;
  if (!Valvelibaw.m_Dictionary.Lookup("VALVE_ROOT_RELATIVE_PATH",
                                      info.m_RelativeRootPath))
    return;
  if (!Valvelibaw.m_Dictionary.Lookup("VALVE_SRC_RELATIVE_PATH",
                                      info.m_RelativeSrcPath))
    return;
  ...
  info.m_Public =
      (Valvelibaw.m_Dictionary.Lookup("VALVE_PUBLIC_PROJECT", tmp) != 0);
  info.m_PublishImportLib =
      (Valvelibaw.m_Dictionary.Lookup("VALVE_PUBLISH_IMPORT_LIB", tmp) != 0);
  info.m_Project = PROJECT_LIB;
  if (strstr(info.m_Ext, "dll") != 0) {
    info.m_Project = PROJECT_DLL;
    if (!Valvelibaw.m_Dictionary.Lookup("VALVE_IMPLIB_RELATIVE_PATH",
                                        info.m_RelativeImplibPath))
      return;
