...
void main(int argc, char *argv[]) {
  ...
  if (argc < 2) {
    ...
    printf("Options:\n");
    printf("-o output dir -- output directory (by default, replaces "
           "original)\n\n");
    return;
  }
  ...
  for (x = 0; x < numsrc; x++) {
    ...
    if (!opn)
      ...
      sprintf(ofname, "%s/%s", odname, fname[x]);
...
void i_save_tga(t_i_image *image, char *fname) {
  ...
  fputc(0, img); // top
  fputc(img_w & 255, img);
  fputc(img_w >> 8, img); // width
  fputc(img_h & 255, img);
  fputc(img_h >> 8, img); // height
  fputc(24, img);         // bpp
...
void i_putpixel_rgba(t_i_image *img, int32 x, int32 y, int32 r, int32 g,
                     int32 b, int32 a) {
  uint32 co;
