...
bool CCodeProcessor::TryBuild(const char *rootdir, const char *filename,
                              unsigned char *buffer, int filelength) {
  ...
  sprintf(
      ...
      "Studio\\Common\\MSDev98\\Bin\\msdev.exe\" %s /MAKE \"%s\" /OUT log.txt",
      m_szDSP, m_szConfig);
...
void CCodeProcessor::ProcessModule(bool forcequiet, int depth, int &maxdepth,
                                   ...
                                   const char *baseroot, const char *root,
                                   const char *module) {
  char filename[256];
    ...
    if (!stricmp(com_token, "#include")) {
      ...
      if (strlen(com_token) > 0) {
        ...
        if (firstheader) {
          ...
          if (!TryBuild(baseroot, filename, (unsigned char *)buffer,
                        filelength)) {
...
void CCodeProcessor::ProcessModules(const char *root, const char *rootmodule) {
  ...
  ProcessModule(false, 0, maxdepth, numheaders, skippedfiles, root, root,
                rootmodule);
}
...
void CCodeProcessor::ConstructModuleList_R(int level, const char *gamespecific,
                                           const char *root) {
  char directory[256];
  char filename[256];
  WIN32_FIND_DATA wfd;
...
void CCodeProcessor::Process(const char *gamespecific, const char *root,
                             const char *dsp, const char *config) {
  strcpy(m_szDSP, dsp);
