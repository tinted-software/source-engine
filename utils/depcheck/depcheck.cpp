...
void printusage(void) {
  vprint(0,
         "usage:  depcheck -q -l <root source directory> <game:  hl2 | tf2>\n\
		\t-q = quiet\n\
...
//			*baseentityclass -
//-----------------------------------------------------------------------------
void ProcessDirectory(const char *game, const char *sourcetreebase,
                      const char *subdir, const char *dsp, const char *config) {
...
int main(int argc, char *argv[]) {
  ...
  //	ProcessDirectory( argv[ i-1 ], sourcetreebase, "engine", "engdll.dsp",
  //"quiver - Win32 GL Debug" ); 	ProcessDirectory( argv[ i-1 ], sourcetreebase,
  //"dlls", "hl.dsp", va( "hl - Win32 %s Debug", argv[ i - 1 ] ) );
  ProcessDirectory(argv[i - 1], sourcetreebase, "cl_dll", "client.dsp",
                   va("client - Win32 Debug %s", argv[i - 1]));
