...
//
// Purpose: Processes .zip directories so every file is aligned to a
// user-defined sector boundary.
//
// $NoKeywords: $
//=============================================================================//
...
static const uint DEF_SECTOR_SIZE =
    512; // in bytes. (512 is xbox HD sector size)
...
//---------------------------------------------------------------
static bool ReadFileData(ZIP_LocalFileHeader &fileHeader, char *filename,
                         char *&data) {
  // Read in the header
...
static bool AlignZipDirectory(const char *infile, const char *outfile) {
  ...
  char filename[256]; // filename size field is 2 bytes, so to be safe the
                      // filename length is checked before copying into this
                      // buffer.
...
void main(int argc, char **argv) {
  if (argc < 2) {
    printf("\n");
    printf("  Usage: zipalign {input filename} [output filename] [sector "
           "size]\n\n");
    printf(
        "  output filename: default = input filename prepended with \"a_\"\n");
    printf("  sector size: minimum sector size (in bytes) to align to. Default "
           "= %d\n\n",
           DEF_SECTOR_SIZE);
    return;
  }
