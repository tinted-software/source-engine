...
int main(int argc, char **argv) {
  ...
  if (argc != 5) {
    printf(
        "format is dist2alpha src_bm.tga dest_bm.tga dest_width dest_height\n");
    exit(1);
  }
  ...
  float max_rad = 2.0 * flSpread *
                  max(hires_image.Width * (1.0 / out_w),
                      hires_image.Height * (1.0 / out_h));
        ...
        if (bCodeChannel[comp]) {
          ...
          for (int oy = -irad; oy <= irad; oy++)
            for (int ox = -irad; ox <= irad; ox++) {
              int x1 = orig_x + ox;
              int y1 = orig_y + oy;
              if ((x1 >= 0) && (x1 < hires_image.Width) && (y1 >= 0) &&
                  (y1 < hires_image.Height) &&
                  (inside_test !=
                   (hires_image.Pixel(orig_x + ox, orig_y + oy, comp) > .9))) {
