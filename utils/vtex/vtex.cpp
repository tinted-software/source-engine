...
static inline uint32 &SetFlagValue(uint32 &field, uint32 const &flag,
                                   int bSetFlag) {
...
static ImageFormat ComputeDesiredImageFormat(IVTFTexture *pTexture,
  ...
  if ((nFlags & TEXTUREFLAGS_PROCEDURAL) ||
    ...
    targetFormat = IMAGE_FORMAT_BGRA8888;
#else
    targetFormat = IsPosix() ? IMAGE_FORMAT_BGRA8888
                             : IMAGE_FORMAT_DXT5; // No DXT compressor on Posix
#endif
    ...
    targetFormat = IMAGE_FORMAT_BGR888;
#else
    targetFormat = IsPosix() ? IMAGE_FORMAT_BGR888
                             : IMAGE_FORMAT_DXT1; // No DXT compressor on Posix
#endif
  }
  return targetFormat;
}
...
static void InitializeSrcTexture_PNG(IVTFTexture *pTexture,
                                     const char *pInputFileName,
                                     CUtlBuffer &pngBuffer, int nDepth,
                                     int nFrameCount,
                                     const VTexConfigInfo_t &info) {
  int nWidth, nHeight;
  ImageFormat imageFormat;
  ...
  unsigned char *data =
      ImgUtl_ReadPNGAsRGBAFromBuffer(pngBuffer, nWidth, nHeight, error);
  if (error != CE_SUCCESS)
...
static void InitializeSrcTexture(IVTFTexture *pTexture,
  ...
  case eModeTGA:
    ...
    break;
  case eModePNG:
    InitializeSrcTexture_PNG(pTexture, pInputFileName, tgaBuffer, nDepth,
                             nFrameCount, info);
    break;
  case eModePFM:
...
static bool LoadFaceFromPSD(IVTFTexture *pTexture, CUtlBuffer &psdBuffer, int z,
  ...
  if ((info.m_bAlphaToDistance) || (nWidth != pTexture->Width()) ||
    ...
    if (info.m_bAlphaToDistance) {
      ...
      if (fmtInfo.m_NumAlphaBits == 0) {
        ...
        for (int x = 0; x < pTexture->Width(); x++) {
          for (int y = 0; y < pTexture->Height(); y++) {
            ...
            float flOutDist =
                min(0.5f, FLerp(0, .5, 0, flMaxRad, flClosest_Dist));
            if (!bInOrOut) {
              // negative distance
              flOutDist = -flOutDist;
            }
      ...
      if (bWarnEdges) {
        VTexWarning("%s: There are non-zero distance pixels along the image "
                    "edge. You may need"
    ...
    ok = PSDReadFileRGBA8888(psdBuffer, bmPsdData);
    if (ok) {
      memcpy(pDestBits, bmPsdData.GetBits(),
             bmPsdData.Height() * bmPsdData.Stride());
    }
    return ok;
  }
}
...
//-----------------------------------------------------------------------------
static bool LoadFaceFromPNG(IVTFTexture *pTexture, CUtlBuffer &tgaBuffer, int z,
                            int nFrame, int nFace, float flGamma,
                            const VTexConfigInfo_t &info) {
  // NOTE: This only works because all mip levels are stored sequentially
  // in memory, starting with the highest mip level. It also only works
...
static bool LoadFaceFromX(IVTFTexture *pTexture, CUtlBuffer &tgaBuffer, int z,
  ...
  case eModeTGA:
    ...
    break;
  case eModePNG:
    return LoadFaceFromPNG(pTexture, tgaBuffer, z, nFrame, nFace, flGamma,
                           info);
    break;
  case eModePFM:
...
static bool LoadFace(IVTFTexture *pTexture, CUtlBuffer &tgaBuffer, int z,
  ...
  if (info.m_numChannelsMax < 1 || info.m_numChannelsMax > 4) {
    VTexWarning("%s: Invalid setting restricting number of channels to %d, "
                "discarded!\n",
...
void PreprocessSkyBox(char *pFullNameWithoutExtension, int *iSkyboxFace) {
  ...
  Error("PreprocessSkyBox: filename %s doesn't have a proper extension (bk, "
        "dn, rt, etc..)\n",
...
bool ProcessFiles(const char *pFullNameWithoutExtension, const char *pOutputDir,
  ...
  bool bAllowFixCubemapOrientation =
      !info.m_bSkyBox; // Don't let it rotate our pseudo-cubemap faces around if
                       // it's a skybox.
...
//succeeds. 		returns false if: 			a) end-of-buffer is reached (then "val" is empty)
...
static bool LoadConfigFile(const char *pFileBaseName, VTexConfigInfo_t &info,
  ...
  if ((info.m_bNormalToDuDv || (info.m_vtfProcOptions.flags0 &
    ...
    printf("Implicitly setting premultcolorbyoneovermiplevel since you are "
...
void Usage(void) {
  VTexError(
      "Usage: vtex [-quiet] [-mkdir] [-shader ShaderName] [-vmtparam Param "
      "Value] tex1.txt tex2.txt . . .\n"
      ...
      "-quickconvert     : use with \"-dontusegamedir -quickconvert\" to "
...
bool GetOutputDir(const char *inputName, char *outputDir) {
  if (g_ForcedOutputDir[0]) {
    ...
    V_snprintf(szSearch, sizeof(szSearch), "materialsrc%c",
...
bool Process_File(char *pInputBaseName, int maxlen) {
  ...
  // Usage:
  //			vtex -dontusegamedir -quickconvert
...
public:
  ...
  virtual int main(int argc, char **argv) {
    ...
    // Being used as a launchable DLL, we don't want to blow away the host app's
    // command line
  ...
  virtual int VTex(CreateInterfaceFn fsFactory, const char *pGameDir, int argc,
    ...
    // When being used embedded in a host app, we don't want to blow away the
    // host app's command line
    CUtlString strOrigCmdLine(CommandLine()->GetCmdLine());
...
int CVTex::VTex(int argc, char **argv) {
  ...
  if (g_bUsedAsLaunchableDLL) {
    // Make sure any further spew doesn't call the function in this module
    // (which will be unloaded shortly)
    SpewOutputFunc(NULL);
  }
