...
bool InitDPlayVoice(HINSTANCE &hInst, IDirectPlayVoiceTest *&pVoice) {
  typedef HRESULT(WINAPI * DirectPlayVoiceCreateFn)(
      GUID * pcIID, void **ppvInterface, IUnknown *pUnknown);
  ...
  if (hInst) {
    DirectPlayVoiceCreateFn fn =
        (DirectPlayVoiceCreateFn)GetProcAddress(hInst, "DirectPlayVoiceCreate");
    if (fn) {
      HRESULT hr =
          fn((GUID *)&IID_IDirectPlayVoiceTest, (void **)&pVoice, NULL);
      if (SUCCEEDED(hr))
        return true;
    }
  }
