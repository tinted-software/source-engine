...
int SmdExportClass::DoExport(const TCHAR *name, ExpInterface *ei, Interface *i,
  ...
  if (!suppressPrompts) {
    ...
    if (DialogBoxParam(hInstance, MAKEINTRESOURCE(IDD_EXPORTOPTIONS),
                       GetActiveWindow(), ExportOptionsDlgProc,
                       (LPARAM)this) <= 0)
  ...
  if (m_fReferenceFrame)
          sprintf(szFile,  "%s\\%s_model.%s",  (char*)strPath, (char*)strFile,
  DEFAULT_EXT);
  */
  ...
  if (!suppressPrompts) {
    // Tell user that exporting is finished (it can take a while with no
    // feedback)
...
int CollectNodesTEP::callback(INode *node) {
  ...
  m_phec->m_rgmaxnode[iNode].imaxnodeParent =
      SmdExportClass::UNDESIRABLE_NODE_MARKER;
...
int DumpNodesTEP::callback(INode *pnode) {
  ...
  m_phec->m_rgmaxnode[iNode].imaxnodeParent =
      fNodeIsRoot ? SmdExportClass::UNDESIRABLE_NODE_MARKER : iNodeParent;
  ...
  m_phec->m_rgmaxnode[iNode].isMirrored =
      DotProd(CrossProd(
                  m_phec->m_rgmaxnode[iNode].mat3ObjectTM.GetRow(0).Normalize(),
                  m_phec->m_rgmaxnode[iNode].mat3ObjectTM.GetRow(1).Normalize())
                  .Normalize(),
              m_phec->m_rgmaxnode[iNode].mat3ObjectTM.GetRow(2).Normalize()) <
      0;
...
int DumpFrameRotationsTEP::callback(INode *pnode) {
  ...
  // check to see if the parent bone was mirrored.  If so, mirror invert this
  // bones position
  if (m_phec->m_rgmaxnode[iNode].imaxnodeParent >= 0 &&
      m_phec->m_rgmaxnode[m_phec->m_rgmaxnode[iNode].imaxnodeParent]
          .isMirrored) {
...
int DumpModelTEP::callback(INode *pnode) {
  ...
  if (pmtlNode == NULL) {
    return TREE_CONTINUE;
    fHasMat = FALSE;
  } else if (!(pmtlNode->ClassID() == Class_ID(MULTI_CLASS_ID, 0) &&
               pmtlNode->IsMultiMtl())) {
    // sprintf(st_szDBG, "ERROR--Material on node %s isn't a Multi/Sub-Object",
    // (char*)strNodeName); ASSERT_AND_ABORT(FALSE, st_szDBG); fHasMat = FALSE;
  }
  ...
  BOOL fConvertedToTriObject = pobj->CanConvertToType(triObjectClassID) &&
                               (ptriobj = (TriObject *)pobj->ConvertToType(
                                    m_tvToDump, triObjectClassID)) != NULL;
  ...
  for (int iFace = 0; iFace < cFaces; iFace++) {
    ...
    ASSERT_AND_ABORT((int)iVertex0 < pmesh->getNumVerts(),
                     "Bogus Vertex 0 index");
    ASSERT_AND_ABORT((int)iVertex1 < pmesh->getNumVerts(),
                     "Bogus Vertex 1 index");
    ASSERT_AND_ABORT((int)iVertex2 < pmesh->getNumVerts(),
                     "Bogus Vertex 2 index");
    ...
    // NOTE: I'm using getRVertPtr instead of getRVert to work around a 3DSMax
    // bug
    ...
    if (fHasMat) {
      ...
      if (pmtlNode->IsMultiMtl()) {
        if (mtlidFace >= pmtlNode->NumSubMtls()) {
          sprintf(st_szDBG,
                  "ERROR--Bogus sub-material index %d in node %s; highest "
                  "valid index is %d",
        ...
        /*
        if ((pmtlFace->ClassID() == Class_ID(MULTI_CLASS_ID, 0) &&
        pmtlFace->IsMultiMtl()))
        {
                // it's a sub-sub material.  Gads.
                pmtlFace = pmtlFace->GetSubMtl(mtlidFace);
                ASSERT_AND_ABORT(pmtlFace != NULL, "NULL Sub-material
        returned");
        }
        */
        ...
        if (!(pmtlFace->ClassID() == Class_ID(DMTL_CLASS_ID, 0))) {
          ...
          sprintf(st_szDBG,
                  "ERROR--Sub-material with index %d (used in node %s) isn't a "
                  "'default/standard' material [%x].",
      ...
      if (ptexmap != NULL) {
        if (!(ptexmap->ClassID() == Class_ID(BMTEX_CLASS_ID, 0))) {
          sprintf(st_szDBG,
                  "ERROR--Sub-material with index %d (used in node %s) doesn't "
                  "have a bitmap as its diffuse texture.",
    ...
    if (ptvface && (pface->flags & HAS_TVERTS)) {
      ...
      ASSERT_AND_ABORT((int)iTVertex0 < pmesh->getNumTVerts(),
                       "Bogus TVertex 0 index");
      ASSERT_AND_ABORT((int)iTVertex1 < pmesh->getNumTVerts(),
                       "Bogus TVertex 1 index");
      ASSERT_AND_ABORT((int)iTVertex2 < pmesh->getNumTVerts(),
                       "Bogus TVertex 2 index");
      ...
      // sprintf(st_szDBG, "ERROR--Node %s has a textureless face.  All faces
      // must have an applied texture.", (char*)strNodeName);
      // ASSERT_AND_ABORT(FALSE, st_szDBG);
    }

    /*
    const char *szExpectedExtension = ".bmp";
    if (stricmp(szBitmapName+strlen(szBitmapName)-strlen(szExpectedExtension),
    szExpectedExtension) != 0)
            {
            sprintf(st_szDBG, "Node %s uses %s, which is not a %s file",
    (char*)strNodeName, szBitmapName, szExpectedExtension);
            ASSERT_AND_ABORT(FALSE, st_szDBG);
            }
    */
    ...
    // Simple 3dsMax model: the vertices are owned by the object, and hence the
    // node
    iNodeV0 = iNode;
    iNodeV1 = iNode;
    iNodeV2 = iNode;
    ...
    fprintf(m_pfile, "%3d %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f",
            iNodeV0, v0.x, v0.y, v0.z, pt3Vertex0Normal.x, pt3Vertex0Normal.y,
            pt3Vertex0Normal.z, UVvertex0.x, UVvertex0.y);
    DumpWeights(iVertex0);
    fprintf(m_pfile, "%3d %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f",
            iNodeV1, v1.x, v1.y, v1.z, pt3Vertex1Normal.x, pt3Vertex1Normal.y,
            pt3Vertex1Normal.z, UVvertex1.x, UVvertex1.y);
    DumpWeights(iVertex1);
    fprintf(m_pfile, "%3d %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f",
            iNodeV2, v2.x, v2.y, v2.z, pt3Vertex2Normal.x, pt3Vertex2Normal.y,
            pt3Vertex2Normal.z, UVvertex2.x, UVvertex2.y);
  ...
  if (m_mcExport) {
    ...
    if (vtxExport) {
      if (vtxExport->GetVertexType() & BLENDED_TYPE) {
        ...
        for (int i = 0; i < pBlend->GetNumberNodes(); i++) {
          iCount = AddWeight(iCount, GetIndexOfINode(pBlend->GetNode(i)),
                             pBlend->GetWeight(i));
        }

        fprintf(m_pfile, " %2d  ", iCount);
        for (i = 0; i < iCount; i++) {
          fprintf(m_pfile, " %2d %5.3f ", aWeights[i].iNode,
                  aWeights[i].flWeight);
        }
      } else {
    ...
    for (int iBone = 0; iBone < m_wa->nb; iBone++) {
      if (m_wa->w[iVertex * m_wa->nb + iBone] > 0.0) {
        ...
        if (bone.node != NULL) {
          iCount = AddWeight(iCount, GetIndexOfINode(bone.node),
                             m_wa->w[iVertex * m_wa->nb + iBone]);
        }
      }
    }
...
static BOOL CALLBACK ExportOptionsDlgProc(HWND hDlg, UINT message,
  ...
  case WM_INITDIALOG:
    ...
    CheckRadioButton(hDlg, IDC_CHECK_SKELETAL, IDC_CHECK_REFFRAME,
                     IDC_CHECK_SKELETAL);
