...
// texturecompile_launcher.cpp : Defines the entry point for the console
// application.
//
...
int main(int argc, char *argv[]) {
  ...
  if (fp) {
    if (fgets(dllName, sizeof(dllName), fp)) {
      ...
      if (pModule)
        printf("Loaded alternate texturecompile DLL (%s) specified in "
               "texturecompile.redirect.\n",
               dllName);
      else
  ...
  if (!pDLL) {
    printf("texturecompile_launcher error: can't get ITextureCompileDLL "
           "interface from vrad_dll.dll\n");
    Sys_UnloadModule(pModule);
    return 3;
  }
