...
int CUniformRandomStream::GenerateRandomNumber() {
  ...
  if (j >= NTAB || j < 0) {
    ...
    Warning("CUniformRandomStream had an array overrun: tried to write to "
            "element %d of 0..31. Contact Tom or Elan.\n",
            j);
...
int CUniformRandomStream::RandomInt(int iLow, int iHigh) {
  ...
  // If you hit either of these assert, you're not getting back the random
  // number that you thought you were.
  Assert(x == iHigh - (int64)iLow + 1); // Check that we didn't overflow int
  Assert(x - 1 <=
         MAX_RANDOM_RANGE); // Check that the values provide an acceptable range

  if (x <= 1 || MAX_RANDOM_RANGE < x - 1) {
    Assert(iLow == iHigh); // This is the only time it is OK to have a range
