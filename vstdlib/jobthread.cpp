...
public:
  ...
  int Count(JobPriority_t priority) { return m_pQueues[priority]->Count(); }
  ...
  CThreadEvent &GetEventHandle() { return m_JobAvailableEvent; }
...
private:
  ...
  // Some jobs should only be executed on the threadpool thread(s). Ie: the
  // rendering thread has the GL context
  //	and the main thread coming in and "helping" with jobs breaks that pretty
  //nicely. This flag states that 	only the threadpool threads should execute
  //these jobs.
...
JOB_INTERFACE void DestroyThreadPool(IThreadPool *pPool) { delete pPool; }
...
public:
  CJobThread(CThreadPool *pOwner, int iThread)
      : m_SharedQueue(pOwner->m_SharedQueue), m_pOwner(pOwner),
        m_iThread(iThread) {}
...
#ifdef _DEBUG
    while ((waitResult = WaitForMultipleObjects(ARRAYSIZE(waitHandles),
                                                waitHandles, FALSE, 10)) ==
           WAIT_TIMEOUT) {
      waitResult = waitResult; // break here
    }
#else
    waitResult = WaitForMultipleObjects(ARRAYSIZE(waitHandles), waitHandles,
                                        FALSE, INFINITE);
#endif
#else // !win32
    bool bSet = false;
  ...
  int Run() {
    ...
    while (!bExit && ((waitResult = Wait()) != WAIT_FAILED)) {
      if (PeekCall()) {
        ...
        tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s PeekCall():%d", __FUNCTION__,
               GetCallParam());
...
int CThreadPool::YieldWait(CThreadEvent **pEvents, int nEvents, bool bWaitAll,
                           unsigned timeout) {
  tmZone(TELEMETRY_LEVEL0, TMZF_IDLE, "%s(%d) SPINNING %t", __FUNCTION__,
         timeout, tmSendCallStack(TELEMETRY_LEVEL0, 0));

  Assert(timeout == TT_INFINITE); // unimplemented

  int result;
  CJob *pJob;
  // Always wait for zero milliseconds initially, to let us process jobs on this
  // thread.
  timeout = 0;
  while ((result = CThreadEvent::WaitForMultiple(nEvents, pEvents, bWaitAll,
                                                 timeout)) == TW_TIMEOUT) {
    if (!m_bExecOnThreadPoolThreadsOnly && m_SharedQueue.Pop(&pJob)) {
      ServiceJobAndRelease(pJob);
      m_nJobs--;
    } else {
      // Since there are no jobs for the main thread set the timeout to
      // infinite. The only disadvantage to this is that if a job thread creates
      // a new job then the main thread will not be available to pick it up, but
      // if that is a problem you can just create more worker threads. Debugging
      // test runs of TF2 suggests that jobs are only ever added from the main
      // thread which means that there is no disadvantage. Waiting on the events
      // instead of busy spinning has multiple advantages. It avoids wasting CPU
      // time/electricity, it makes it more obvious in profiles when the main
      // thread is idle versus busy, and it allows ready thread analysis in
      // xperf to find out what woke up a waiting thread. It also avoids
      // unnecessary CPU starvation -- seen on customer traces of TF2.
      timeout = TT_INFINITE;
    }
  }
  return result;
...
void CThreadPool::AddJob(CJob *pJob) {
  ...
  if (m_Threads.Count() == 0) {
    // So only threadpool jobs are supposed to execute the jobs, but there are
    // no threadpool threads?
  ...
  if (!m_bExecOnThreadPoolThreadsOnly &&
      ((flags & (JF_IO | JF_QUEUE)) == 0) /* @TBD && !m_queue.Count() */) {
    if (!NumIdleThreads()) {
      pJob->Execute();
      return;
    }
...
void CThreadPool::ChangePriority(CJob *pJob, JobPriority_t priority) {
  ...
  if (pJob->GetPriority() < priority) {
    ...
    ExecuteOnce(if (pJob->GetPriority() != priority)
...
bool CThreadPool::Start(const ThreadPoolStartParams_t &startParams,
  ...
  if (nThreads < 0) {
    ...
    if (startParams.bIOThreads) {
      ...
      if (IsPC()) {
        if (nThreads > 3) {
          DevMsg("Defaulting to limit of 3 worker threads, use -threads on "
                 "command line if want more\n"); // Current >4 processor configs
                                                 // don't really work so well,
                                                 // probably due to cache
                                                 // issues? (toml 7/12/2007)
          nThreads = 3;
        }
      }
    }

    if ((startParams.nThreadsMax >= 0) &&
  ...
  if (bDistribute) {
    ...
    if (ci.m_nLogicalProcessors > 1) {
      if (!pAffinityTable) {
        ...
        if (hInst) {
          typedef DWORD(WINAPI * SetThreadIdealProcessorFn)(
              ThreadHandle_t hThread, DWORD dwIdealProcessor);
          SetThreadIdealProcessorFn Thread_SetIdealProcessor =
              (SetThreadIdealProcessorFn)GetProcAddress(
                  hInst, "SetThreadIdealProcessor");
          if (Thread_SetIdealProcessor) {
            ...
            for (int i = 0; i < m_Threads.Count(); i++) {
              ...
              Thread_SetIdealProcessor(
                  (ThreadHandle_t)m_Threads[i]->GetThreadHandle(), iProc);
            }
          }
          FreeLibrary(hInst);
        }
    ...
    for (g_iSleep = -10; g_iSleep <= 10; g_iSleep += 10) {
      ...
      for (int i = 1; i <= nMaxThreads; i += nIncrement) {
        ...
        Msg("ThreadPoolTest:         %d threads -- %d (%d) jobs processed in "
            ...
            i, g_nTotalAtFinish, (int)CCountJob::m_nCount,
