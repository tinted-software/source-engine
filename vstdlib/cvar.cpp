    ...
    if (variable->IsCommand() || pOther->IsCommand()) {
      ...
      if (s_pCVarQuery->AreConVarsLinkable(pChildVar, pParentVar)) {
        ...
        if ((pChildVar->m_nFlags & FCVAR_CHEAT) !=
          ...
          Warning("Convar %s has conflicting FCVAR_CHEAT flags (child: %s, "
        ...
        if ((pChildVar->m_nFlags & FCVAR_REPLICATED) !=
          ...
          Warning("Convar %s has conflicting FCVAR_REPLICATED flags (child: "
                  "%s, parent: %s, parent wins)\n",
                  variable->GetName(),
                  (pChildVar->m_nFlags & FCVAR_REPLICATED)
                      ? "FCVAR_REPLICATED"
                      : "no FCVAR_REPLICATED",
                  (pParentVar->m_nFlags & FCVAR_REPLICATED)
                      ? "FCVAR_REPLICATED"
        ...
        if ((pChildVar->m_nFlags & FCVAR_DONTRECORD) !=
          ...
          Warning("Convar %s has conflicting FCVAR_DONTRECORD flags (child: "
                  "%s, parent: %s, parent wins)\n",
                  variable->GetName(),
                  (pChildVar->m_nFlags & FCVAR_DONTRECORD)
                      ? "FCVAR_DONTRECORD"
                      : "no FCVAR_DONTRECORD",
                  (pParentVar->m_nFlags & FCVAR_DONTRECORD)
                      ? "FCVAR_DONTRECORD"
...
// Purpose:
//-----------------------------------------------------------------------------
ConCommandBase *CCvar::GetCommands(void) { return m_pConCommandList; }
