...
//				need to be set for coroutine.cpp itself.  These
//are the VPC 				entries for those options: 	$Compiler
//	{
//		$EnableC++Exceptions				"No"
//		$BasicRuntimeChecks "Default" 		$EnableFloatingPointExceptions
//"No"
//	}
//
//			If you have not set these options you will get a strange
//popup in 		Visual Studio at the end of Coroutine_Continue().
//
//=============================================================================
...
// the VS2105 longjmp() seems to freak out jumping back into a coroutine (just
// like linux if _FORTIFY_SOURCE is defined)
//  I can't find an analogy to _FORTIFY_SOURCE for MSVC at the moment, so I
//  wrote a quick assembly to longjmp() without any safety checks
extern "C" NORETURN void Coroutine_LongJmp_Unchecked(jmp_buf buffer,
                                                     int nResult);
#define Coroutine_longjmp Coroutine_LongJmp_Unchecked

#ifdef _WIN64
#define Q_offsetof(s, m)                                                       \
  (size_t)((ptrdiff_t)&reinterpret_cast<const volatile char &>((((s *)0)->m)))
#else
#define Q_offsetof(s, m)                                                       \
  (size_t)&reinterpret_cast<const volatile char &>((((s *)0)->m))
#endif
#define SIZEOF_MEMBER(className, memberName)                                   \
  sizeof(((className *)nullptr)->memberName)

#define Validate_Jump_Buffer(_Member)                                          \
  COMPILE_TIME_ASSERT((Q_offsetof(_JUMP_BUFFER, _Member) ==                    \
                       Q_offsetof(_Duplicate_JUMP_BUFFER, _Member)) &&         \
                      (SIZEOF_MEMBER(_JUMP_BUFFER, _Member) ==                 \
                       SIZEOF_MEMBER(_Duplicate_JUMP_BUFFER, _Member)))

// validate that the structure in assembly matches what the crt setjmp thinks it
// is
...
// it *feels* like we should need barriers around our setjmp/longjmp calls, and
// the memcpy's to make sure the optimizer doesn't reorder us across register
// load/stores, so I've put them in what seem like the appropriate spots, but we
// seem to run ok without them, so...
#ifdef GNUC
...
#ifdef _DEBUG
// In debug builds, we'll end up with much more stack usage in some scenarios
// that isn't representative of release builds.  We should still warn if we're
// going way above what we could expect the optimizer to save us from, but the
...
#elif defined(GNUC)
// Apple's version of gcc/g++ doesn't return the expected value using the
// intrinsic, so do it the old fashioned way - this will also use asm on linux
// (since we don't compile with llvm/clang there) but that seems fine.
// #if defined(__llvm__) || defined(__clang__)
#define GetStackPtr(pStackPtr)                                                 \
  byte *pStackPtr = (byte *)__builtin_frame_address(0)
// #else
...
public:
...
#ifdef _M_X64
    // Did we get moved in memory in such a way that the registers became
    // unaligned? If so, fix them up now
  ...
  FORCEINLINE void RestoreStack() {
    if (m_cubSavedStack) {
...
#if defined(_PS3) && defined(_DEBUG)
      // Our (and Sony's) memory tracking tools may try to walk the stack during
      // a free() call if we do the free here at our normal point though the
      // stack is invalid since it's in the middle of swapping.  Instead move it
      // to a temp buffer now and free  while the stack frames in place are
      // still ok.
      ...
      if (pStack >= m_pStackLow && pStack <= m_pStackHigh) {
        CoroutineDbgMsg(
            g_fmtstr.sprintf("Restoring stack over ESP (%x, %x, %x)\n", pStack,
                             m_pStackLow, m_pStackHigh));
        AssertMsg3(false, "Restoring stack over ESP (%p, %p, %p)\n", pStack,
                   m_pStackLow, m_pStackHigh);
      }

      // Make sure we can access the our instance pointer after restoring the
      // stack. This function is inlined, so the compiler could decide to use an
      // existing coroutine pointer that is already on the stack from the
      // previous function (does so on the PS3), and will be overwritten when we
      // memcpy below. Any allocations here should be ok, as the caller should
      // have advanced the stack past the stack area where the new stack will be
      // copied
      ...
      // WARNING: The stack has been replaced.. do not use previous stack
      // variables or this
      ...
      Assert(0 ==
             Q_memcmp(pThis->m_digest, pThis->m_digest2, MD5_DIGEST_LENGTH));
      ...
      // Make sure we are added after the coroutine we just copied onto the
      // stack
#if defined(VPROF_ENABLED)
      ...
      if (g_VProfCurrentProfile.IsEnabled()) {
        FOR_EACH_VEC_BACK(pThis->m_vecProfNodeStack, i) {
          g_VProfCurrentProfile.EnterScope(
              ...
              g_VProfCurrentProfile.GetBudgetGroupName(
                  pThis->m_vecProfNodeStack[i]->GetBudgetGroupID()),
              false,
              g_VProfCurrentProfile.GetBudgetGroupFlags(
                  pThis->m_vecProfNodeStack[i]->GetBudgetGroupID()));
        }
      }
  ...
  FORCEINLINE void SaveStack() {
    ...
    // if you hit this assert, it's because you're allocating way too much stuff
    // on the stack in your job check you haven't got any overly large string
    // buffers allocated on the stack
    Assert(m_cubSavedStack < k_cubMaxCoroutineStackSize);

#if defined(VPROF_ENABLED)
    // Exit any current vprof scope when we yield, and remember the vprof stack
    // so we can restore it when we run again
  ...
  byte *m_pStackHigh; // position of initial entry to the coroutine (stack ptr
                      // before continue is ran)
  byte *m_pStackLow; // low point on the stack we plan on saving (stack ptr when
                     // we yield)
...
public:
  ...
  HCoroutine CreateCoroutine(CoroutineFunc_t pFunc, void *pvParam) {
    ...
    CoroutineDbgMsg(g_fmtstr.sprintf(
        "Coroutine_Create() hCoroutine = %x pFunc = 0x%x pvParam = 0x%x\n",
    ...
    m_ListCoroutines[hCoroutine].m_pStackHigh =
        m_ListCoroutines[hCoroutine].m_pStackLow = NULL;
  ...
  CCoroutine &GetPreviouslyActiveCoroutine() {
    ...
    return m_ListCoroutines[m_VecCoroutineStack[m_VecCoroutineStack.Count() -
                                                2]];
  }
...
//			pchDebugMsg -		if non-NULL, it will generate an
//assertion in 								that coroutine, then that coroutine will 								immediately yield back
//to this thread
//-----------------------------------------------------------------------------
static const char *k_pchDebugMsg_GenericBreak = (const char *)1;

bool Internal_Coroutine_Continue(HCoroutine hCoroutine, const char *pchDebugMsg,
                                 const char *pchName) {
  ...
  __asm mov eax, dword ptr fs
      : [0] __asm mov topofexceptionchain,
        eax if (GCoroutineMgr().m_topofexceptionchain == 0) GCoroutineMgr()
            .m_topofexceptionchain = topofexceptionchain;
  else {
  ...
  if (iResult == k_iSetJmpStateSaved) {
    ...
    if (coroutine.m_pSavedStack) {
      // save any of the main stack that overlaps where the coroutine stack is
      // going to go
      ...
      // If the coroutine's stack is close enough to where we are on the stack,
      // we need to push ourselves down past it, so that the memcpy() doesn't
      // screw up the RestoreStack->memcpy call chain.
      if (coroutine.m_pStackHigh > (pStackSavePoint - 2048)) {
        ...
        if (coroutine.m_pStackLow < pStackSavePoint) {
          ...
          CoroutineDbgMsg(
              g_fmtstr.sprintf("Adjusting stack point by %d (%x <- %x)\n",
                               cubPush, pvStackGap, &pvStackGap[cubPush]));
        }
      }

      // This needs to go right here - after we've maybe padded the stack (so
      // that iJumpCode does not get stepped on) and before the RestoreStack()
      // call (because that might step on pchDebugMsg!).
      if (pchDebugMsg == NULL) {
      ...
      CoroutineDbgMsg(g_fmtstr.sprintf(
          "RestoreStack() %s#%x [%x - %x] (current %x)\n", coroutine.m_pchName,
          coroutine.m_hCoroutine, coroutine.m_pStackLow, coroutine.m_pStackHigh,
          pStackSavePoint));
      coroutine.RestoreStack();

      // the new stack is in place, so no code here can reference local stack
      // vars move the program counter
      RW_MEMORY_BARRIER;
      ...
      // this needs to be a stackalloc() instead of a static buffer, so it won't
      // get optimized out in release build
      int cubGap = bInCoroutineAlready ? k_cubCoroutineStackGapSmall
                                       : k_cubCoroutineStackGap;
    ...
    // when the job yields, the above setjmp() will be called again with
    // non-zero value code here will never run
...
void NOINLINE Coroutine_Launch(CCoroutine &coroutine) {
  ...
  GetStackPtr(pEsp);
#else
  // The stack pointer for the current stack frame points to the top of the
  // stack which already includes space for the ABI linkage area. We need to
  // include this area as part of our coroutine stack, as the calling function
  // will copy the link register (return address to this function) into this
  // area after calling m_pFunc below. Failing to do so could result in the
  // coroutine to return to garbage when complete
  uint64 *pStackFrameTwoUp = (uint64 *)__builtin_frame_address(2);

  // Need to terminate the stack frame sequence so if someone tries to walk the
  // stack in a co-routine they don't go forever.
  *pStackFrameTwoUp = 0;

  // Need to track where we we save up to on yield, add a few bytes so we save
  // just the beginning linkage area of the stack frame we added  the null
  // termination to.
  ...
  // align the stack
  coroutine.m_pStackHigh =
      (byte *)(((uintptr_t)pEsp + 32 + 15) & ~(uintptr_t)15);
  ...
  // return!
  CatchAndWriteMiniDumpForVoidPtrFn(coroutine.m_pFunc, coroutine.m_pvParam,
                                    /*bExitQuietly*/ true);
#else
...
//-----------------------------------------------------------------------------
bool Coroutine_IsActive() { return GCoroutineMgr().IsAnyCoroutineActive(); }
...
void Coroutine_YieldToMain() {
  // if you've hit this assert, it's because you're calling yield when not in a
  // coroutine
  Assert(Coroutine_IsActive());
  CCoroutine &coroutinePrev = GCoroutineMgr().GetPreviouslyActiveCoroutine();
  CCoroutine &coroutine = GCoroutineMgr().GetActiveCoroutine();
  ...
  if ((iResult == k_iSetJmpStateSaved) || (iResult == k_iSetJmpDbgBreak)) {
    ...
    if (iResult == k_iSetJmpDbgBreak) {
      ...
      if (coroutine.m_pchDebugMsg) {
        ...
        AssertMsg1(!"Coroutine assert requested", "%s",
                   coroutine.m_pchDebugMsg);
      } else {
    ...
    // save our stack - all the way to the top, err bottom err, the end of it (
    // where esp is )
    coroutine.SaveStack();
    CoroutineDbgMsg(g_fmtstr.sprintf(
        "SaveStack() %s#%x [%x - %x]\n", coroutine.m_pchName,
    ...
    // allocate a bunch of stack padding so we don't kill ourselves while in
    // stack restoration If the coroutine's stack is close enough to where we
    // are on the stack, we need to push ourselves down past it, so that the
    // memcpy() doesn't screw up the RestoreStack->memcpy call chain.
    GetStackPtr(pStackPtr);
    if (pStackPtr >=
            (coroutinePrev.m_pStackHigh - coroutinePrev.m_cubSavedStack) &&
      ...
      CoroutineDbgMsg(
          g_fmtstr.sprintf("Adjusting stack point by %d (%x <- %x)\n", cubPush,
                           pvStackGap, &pvStackGap[cubPush]));
    }

    CoroutineDbgMsg(g_fmtstr.sprintf(
        "RestoreStack() %s#%x [%x - %x]\n", coroutinePrev.m_pchName,
        coroutinePrev.m_hCoroutine, coroutinePrev.m_pStackLow,
        coroutinePrev.m_pStackHigh));
    ...
    RW_MEMORY_BARRIER;
    Coroutine_longjmp(
        GCoroutineMgr().GetPreviouslyActiveCoroutine().GetRegisters(),
        k_iSetJmpContinue);
...
void Coroutine_Finish() {
  ...
  CoroutineDbgMsg(
      g_fmtstr.sprintf("Coroutine_Finish() %s#%x -> %s#%x\n",
                       GCoroutineMgr().GetActiveCoroutine().m_pchName,
                       GCoroutineMgr().GetActiveCoroutineHandle(),
                       GCoroutineMgr().GetPreviouslyActiveCoroutine().m_pchName,
                       &GCoroutineMgr().GetPreviouslyActiveCoroutine()));

  // allocate a bunch of stack padding so we don't kill ourselves while in stack
  // restoration
  volatile byte *pvStackGap = (byte *)stackalloc(
      GCoroutineMgr().GetPreviouslyActiveCoroutine().m_cubSavedStack + 512);
  pvStackGap[GCoroutineMgr().GetPreviouslyActiveCoroutine().m_cubSavedStack +
             511] = 0xf;
...
void CoroutineTestFunc(void *pvRelaunch) {
  ...
  if (!pvRelaunch) {
    ...
    HCoroutine hCoroutine =
        Coroutine_Create(&CoroutineTestFunc, (void *)(size_t)0xFFFFFFFF);
...
void CoroutineTestL1(void *pvecCoroutineL2) {
  CUtlVector<HCoroutine> &vecCoroutineL2 =
      *(CUtlVector<HCoroutine> *)pvecCoroutineL2;
