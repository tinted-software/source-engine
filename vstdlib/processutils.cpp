...
private:
  ...
  // creates the process, adds it to the list and writes the windows HANDLE into
  // info.m_hProcess
...
ProcessHandle_t CProcessUtils::CreateProcess(ProcessInfo_t &info,
  ...
  if (::CreateProcess(NULL, info.m_CommandLine.GetForModify(), NULL, NULL, TRUE,
  ...
  if (CreatePipe(&info.m_hChildStdoutRd, &info.m_hChildStdoutWr, &saAttr, 0)) {
    if (CreatePipe(&info.m_hChildStdinRd, &info.m_hChildStdinWr, &saAttr, 0)) {
      if (DuplicateHandle(GetCurrentProcess(), info.m_hChildStdoutWr,
        ...
        //); 				_setmode( info.m_hChildStdoutWr, _O_TEXT ); 				_setmode(
        //info.m_hChildStderrWr, _O_TEXT );
