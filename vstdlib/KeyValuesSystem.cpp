...
public:
  ...
  // maintain a cache of KeyValues we load from disk. This saves us quite a lot
  // of time on app startup.
  virtual void AddFileKeyValuesToCache(const KeyValues *_kv,
                                       const char *resourceName,
                                       const char *pathID);
  virtual bool LoadFileKeyValuesFromCache(KeyValues *outKv,
                                          const char *resourceName,
                                          const char *pathID,
...
//KeyValues objects 			the sizes will usually never differ but this is for
//versioning safety
//-----------------------------------------------------------------------------
...
void *CKeyValuesSystem::AllocKeyValuesMemory(int size) {
  ...
  if (!m_pMemPool) {
    m_pMemPool =
        new CUtlMemoryPool(m_iMaxKeyValuesSize, 1024, UTLMEMORYPOOL_GROW_FAST,
...
//-----------------------------------------------------------------------------
// Purpose: Removes a particular key value file (from a particular source) from
// the cache if present.
//-----------------------------------------------------------------------------
void CKeyValuesSystem::InvalidateCacheForFile(const char *resourceName,
                                              const char *pathID) {
  CUtlString identString(CFmtStr("%s::%s", resourceName ? resourceName : "",
                                 pathID ? pathID : ""));

  CUtlMap<CUtlString, KeyValues *>::IndexType_t index =
      m_KeyValueCache.Find(identString);
...
//-----------------------------------------------------------------------------
// Purpose: Adds a particular key value file (from a particular source) to the
// cache if not already present.
//-----------------------------------------------------------------------------
void CKeyValuesSystem::AddFileKeyValuesToCache(const KeyValues *_kv,
                                               const char *resourceName,
                                               const char *pathID) {
  CUtlString identString(CFmtStr("%s::%s", resourceName ? resourceName : "",
                                 pathID ? pathID : ""));
  // Some files actually have multiple roots, and if you use regular MakeCopy
  // (without passing true), those will be missed. This caused a bug in
  // soundscapes on dedicated servers.
  m_KeyValueCache.Insert(identString, _kv->MakeCopy(true));
}

//-----------------------------------------------------------------------------
// Purpose: Fetches a particular keyvalue from the cache, and copies into _outKv
// (clearing anything there already).
//-----------------------------------------------------------------------------
bool CKeyValuesSystem::LoadFileKeyValuesFromCache(
    KeyValues *outKv, const char *resourceName, const char *pathID,
    IBaseFileSystem *filesystem) const {
  Assert(outKv);
  Assert(resourceName);

  COM_TimestampedLog(
      "CKeyValuesSystem::LoadFileKeyValuesFromCache(%s%s%s): Begin",
  ...
  if (m_KeyValueCache.IsValidIndex(index)) {
    ...
    COM_TimestampedLog(
        "CKeyValuesSystem::LoadFileKeyValuesFromCache(%s%s%s): End / Hit",
        pathID ? pathID : "", pathID && resourceName ? "/" : "",
        resourceName ? resourceName : "");
    return true;
  }
  ...
  COM_TimestampedLog(
      "CKeyValuesSystem::LoadFileKeyValuesFromCache(%s%s%s): End / Miss",
      pathID ? pathID : "", pathID && resourceName ? "/" : "",
      resourceName ? resourceName : "");
...
//-----------------------------------------------------------------------------
void CKeyValuesSystem::InvalidateCache() { DoInvalidateCache(); }
...
void CKeyValuesSystem::DoInvalidateCache() {
  ...
  // Apparently you cannot call RemoveAll on a map without also purging the
  // contents because... ? If you do and you continue to use the map, you will
  // eventually wind up in a case where you have an empty map but it still
  // iterates over elements. Awesome?
