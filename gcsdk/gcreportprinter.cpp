...
CGCReportPrinter::Variant_t::Variant_t() : m_nInt(0), m_fFloat(0) {}

CGCReportPrinter::CGCReportPrinter() {}

CGCReportPrinter::~CGCReportPrinter() { Clear(); }
...
bool CGCReportPrinter::AddIntColumn(
    const char *pszColumn, ESummaryType eSummary,
    EIntDisplayType eIntDisplay /* = eIntDisplay_Normal */) {
  // don't allow adding columns if data is already present
  if (m_Rows.Count() > 0)
    return false;
...
bool CGCReportPrinter::AddFloatColumn(const char *pszColumn,
                                      ESummaryType eSummary,
                                      uint32 unNumDecimal /* = 2 */) {
  // don't allow adding columns if data is already present
  if (m_Rows.Count() > 0)
    return false;
...
void CGCReportPrinter::ClearData() { m_Rows.PurgeAndDeleteElements(); }
...
// called to add the various data to the report, the order of this must match
// the columns that were added originally
bool CGCReportPrinter::StrValue(const char *pszStr, const char *pszLink) {
  ...
  if ((m_RowBuilder.Count() >= m_Columns.Count()) ||
      (m_Columns[m_RowBuilder.Count()].m_eType != eCol_String))
    return false;
...
bool CGCReportPrinter::IntValue(int64 nValue, const char *pszLink) {
  // make sure we have a following column and that the type matches
  if ((m_RowBuilder.Count() >= m_Columns.Count()) ||
      (m_Columns[m_RowBuilder.Count()].m_eType != eCol_Int))
    return false;
...
bool CGCReportPrinter::FloatValue(double fValue, const char *pszLink) {
  // make sure we have a following column and that the type matches
  if ((m_RowBuilder.Count() >= m_Columns.Count()) ||
      (m_Columns[m_RowBuilder.Count()].m_eType != eCol_Float))
    return false;
...
bool CGCReportPrinter::SteamIDValue(CSteamID id, const char *pszLink) {
  // make sure we have a following column and that the type matches
  if ((m_RowBuilder.Count() >= m_Columns.Count()) ||
      (m_Columns[m_RowBuilder.Count()].m_eType != eCol_SteamID))
    return false;
...
// class that implements sorting our rows based upon a provided column with
// ascending or descending ordering
class CReportRowSorter {
public:
  CReportRowSorter(bool bDescending, uint32 nCol,
                   CGCReportPrinter::EColumnType eType)
      : m_bDescending(bDescending), m_nCol(nCol), m_eType(eType) {}

  bool operator()(const CGCReportPrinter::TRow *pR1,
                  const CGCReportPrinter::TRow *pR2) {
...
void CGCReportPrinter::SortReport(uint32 nColIndex, bool bDescending) {
  if (nColIndex < (uint32)m_Columns.Count()) {
    CReportRowSorter sorter(bDescending, nColIndex,
                            m_Columns[nColIndex].m_eType);
    std::sort(m_Rows.begin(), m_Rows.end(), sorter);
  }
}
...
static uint CountDigits(int64 nInt) {
  // the zero special case, since it would otherwise fall out of the loop too
  // early
  if (nInt == 0)
    return 1;
...
static uint CountIntWidth(int64 nValue,
                          CGCReportPrinter::EIntDisplayType eIntDisplay) {
  uint unDigits;
  switch (eIntDisplay) {
...
static const char *
GetIntValueDisplay(int64 nValue,
                   CGCReportPrinter::EIntDisplayType eIntDisplay) {
  ...
  case CGCReportPrinter::eIntDisplay_Memory_MB:
    // "1234.56 MB"
    s_fmtResult.sprintf(
        "%lld.%02u MB", (nValue / k_nMegabyte),
        (uint32)(100.0f * ((abs(nValue) % k_nMegabyte) / (float)k_nMegabyte)));
    break;
...
void CGCReportPrinter::PrintReport(CGCEmitGroup &eg, uint32 nTop) {
  // we need to determine our totals and maximum row widths for our columns
  // first
  ...
  FOR_EACH_VEC(m_Columns, nCol) {
    ...
    FOR_EACH_VEC(m_Rows, nRow) {
      ...
      case eCol_SteamID:
        ...
        break;
      case eCol_Float:
        nColWidth = MAX(nColWidth, CountDigits((int64)v.m_fFloat) + 1 +
                                       col.m_nNumDecimals);
    ...
    switch (col.m_eType) {
    case eCol_Float:
      nColWidth = MAX(nColWidth, CountDigits((int64)summary.m_fFloat) + 1 +
                                     col.m_nNumDecimals);
      break;
    case eCol_Int:
      nColWidth =
          MAX(nColWidth, CountIntWidth(summary.m_nInt, col.m_eIntDisplay));
      break;
    }
  ...
  FOR_EACH_VEC(m_Rows, nRow) {
    ...
    FOR_EACH_VEC(m_Columns, nCol) {
      ...
      // print out spaces before we do the link (so we don't have the whole
      // table underlined)
  ...
  // and our summary
  {
    ...
    FOR_EACH_VEC(m_Columns, nCol) {
      ...
      if ((col.m_eType == eCol_String) || (col.m_eSummary == eSummary_None)) {
        ...
        switch (col.m_eType) {
        case eCol_Float:
          vMsg.AppendFormat("%*.*f ", nColWidth, col.m_nNumDecimals,
                            v.m_fFloat);
          break;
        case eCol_Int:
          vMsg.AppendFormat("%*s ", nColWidth,
                            GetIntValueDisplay(v.m_nInt, col.m_eIntDisplay));
          break;
        }
      }
    }
