...
CSQLRecord::CSQLRecord() : m_pResultSet(NULL), m_unRow(0) {}
...
//-----------------------------------------------------------------------------
bool CSQLRecord::BGetColumnData(uint32 unColumn, uint8 **ppubField,
                                int *pcubField) {
  size_t sz;
...
bool CSQLRecord::BGetColumnData(uint32 unColumn, uint8 **ppubField,
  ...
  *pcubField = 0;
  return m_pResultSet->GetData(m_unRow, unColumn, ppubField,
                               (uint32 *)pcubField);
}
...
// Output:	true if successful, false if data not present or not of correct
// type
//-----------------------------------------------------------------------------
...
bool CSQLRecord::BGetIntValue(uint32 unColumn, int *pnVal) {
  ...
  *pnVal = 0;
  ...
  if (bRet) {
    ...
    case k_EGCSQLType_int64: {
      ...
      if (ul >= LONG_MIN && ul <= LONG_MAX) {
        *pnVal = (int)ul;
        ...
        AssertMsg1(
            false,
            "GetIntValue tried to catch %lld in an int, which is too small",
            ul);
        return false;
      }
    } break;
    ...
    default:
      AssertMsg1(false,
                 "GetIntValue tried to catch a %s, which is the wrong type",
                 PchNameFromEGCSQLType(m_pResultSet->GetColumnType(unColumn)));
      return false;
    }
  }
...
bool CSQLRecord::BGetFloatValue(uint32 unColumn, float *pfVal) {
  ...
  *pfVal = 0.0f;
  ...
  if (bRet) {
    ...
    AssertMsg2(sizeof(float) == cubData,
               "GetValue expected %llu bytes, found %d", (uint64)sizeof(float),
               cubData);
...
bool CSQLRecord::BGetDoubleValue(uint32 unColumn, double *pdVal) {
  ...
  *pdVal = 0.0f;
  ...
  if (bRet) {
    ...
    AssertMsg2(sizeof(double) == cubData,
               "GetValue expected %llu bytes, found %d", (uint64)sizeof(double),
               cubData);
...
bool CSQLRecord::BValidateColumnIndex(uint32 unColumn) {
  if (unColumn >= m_pResultSet->GetColumnCount()) {
    AssertMsg2(false,
               "CSQLRecord::BValidateColumnIndex: invalid column index %d.  # "
               "columns: %d",
               unColumn, m_pResultSet->GetColumnCount());
    return false;
  }
  return true;
}
...
// Output:	returns false if this call would advance the record past the
// last row.
...
void CSQLRecord::RenderField(uint32 unColumn, int cchBuffer, char *pchBuffer) {
  ...
  ConvertFieldToText(m_pResultSet->GetColumnType(unColumn), pubData, cubData,
                     pchBuffer, cchBuffer, false);
}

//-----------------------------------------------------------------------------
// Purpose: Copies a CSQLRecord to CRecordBase
//-----------------------------------------------------------------------------
bool CSQLRecord::BWriteToRecord(CRecordBase *pRecord,
                                const CColumnSet &csWriteFields) {
  bool bSuccess = true;
  FOR_EACH_COLUMN_IN_SET(csWriteFields, unSQLColumn) {
    ...
    if (!BGetColumnData(unSQLColumn, &pubData, &cubData)) {
      ...
      bSuccess =
          pRecord->BSetField(unRecordColumn, pubData, cubData) && bSuccess;
    }
  }
  return bSuccess;
}
