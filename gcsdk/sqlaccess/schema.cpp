...
// Purpose: Destructor
//-----------------------------------------------------------------------------
CSchema::~CSchema() { SAFE_RELEASE(m_pRecordInfo); }
...
void CSchema::PrepareForUse() {
  // Create a record description (new form of schema information, for SQL) that
  // corresponds to this schema object. This contains essentially the
  // information as the CSchema object, we keep both for the moment to bridge
  // the DS and SQL worlds.
...
//-----------------------------------------------------------------------------
// Purpose: For a record that has variable-length fields, gets the info block
// from
//			the tail end
// Input  : pvRecord -			Record data
//-----------------------------------------------------------------------------
VarFieldBlockInfo_t *
CSchema::PVarFieldBlockInfoFromRecord(const void *pvRecord) const {
  if (!m_bHasVarFields)
    return NULL;
...
//			For records that have no variable-length fields, it will
//return zero
// Input  : pvRecord -			Record data
//-----------------------------------------------------------------------------
uint32 CSchema::CubRecordVariable(const void *pvRecord) const {
  VarFieldBlockInfo_t *pVarFieldsBlockInfo =
      PVarFieldBlockInfoFromRecord(pvRecord);
...
void CSchema::RenderField(uint8 *pubRecord, int iField, int cchBuffer,
                          char *pchBuffer) {
  ...
  if (pField->BIsVariableLength()) {
    if (!BGetVarField(pubRecord,
                      (VarField_t *)(pubRecord + pField->m_dubOffset), &pubData,
                      &cubData)) {
...
//			ppubField -			receives pointer to
//field data 			pcubField -			receives size in bytes of that
//data
// Output:	true if successful
//-----------------------------------------------------------------------------
bool CSchema::BGetFieldData(const void *pvRecord, int iField, uint8 **ppubField,
                            uint32 *pcubField) const {
  *ppubField = NULL;
  *pcubField = 0;
  const Field_t &field = m_VecField[iField];
  if (field.BIsVariableLength()) {
    return BGetVarField(pvRecord,
                        (VarField_t *)((uint8 *)pvRecord + field.m_dubOffset),
                        ppubField, pcubField);
  } else {
...
//			pubField -			pointer to field data to
//copy from 			cubField -			size in bytes of that data
// Output:	true if successful
//-----------------------------------------------------------------------------
bool CSchema::BSetFieldData(void *pvRecord, int iField, uint8 *pubField,
                            uint32 cubField, bool *pbVarBlockRealloced) {
  *pbVarBlockRealloced = false;
  ...
  if (field.BIsVariableLength()) {
    return BSetVarField(
        pvRecord, (VarField_t *)((uint8 *)pvRecord + field.m_dubOffset),
        pubField, cubField, pbVarBlockRealloced, /*bFreeOnRealloc*/ true);
  } else // fixed length
  {
    ...
    if (cubField > field.m_cubLength ||
        (k_EGCSQLType_String == field.m_EType &&
         Q_strnlen(reinterpret_cast<char *>(pubField), cubField) == -1)) {
      Assert(false);
      return false;
    }

    if (k_EGCSQLType_Blob != field.m_EType &&
        k_EGCSQLType_Image != field.m_EType) {
...
// Input:	pvRecord -			Fixed-length part of record
//			pVarField -			fixed part of field in
//that record 			ppubField -			receives pointer to field data
//			pcubField -			receives size in bytes
//of that data
//-----------------------------------------------------------------------------
bool CSchema::BGetVarField(const void *pvRecord, const VarField_t *pVarField,
                           uint8 **ppubField, uint32 *pcubField) const {
  Assert(m_bHasVarFields);

  *ppubField = 0;
  *pcubField = 0;

  VarFieldBlockInfo_t *pVarFieldBlockInfo =
      PVarFieldBlockInfoFromRecord(pvRecord);
...
//-----------------------------------------------------------------------------
// Purpose: Update a variable-length field in a record (may realloc the var
// block) Input:	pvRecord -			Fixed-length part of
// record
//			pVarField -			fixed part of field in
//that record 			pvData -			data to place in the field 			cubData -
//size of that data 			pbRealloced -		set to indicate if the var block
//was realloced 			bFreeOnRealloc -	If we have to grow or shrink the var
//block, should we free the old memory? 								Usually true, unless it lives in a
//NetPacket or something like that
//-----------------------------------------------------------------------------
bool CSchema::BSetVarField(void *pvRecord, VarField_t *pVarField,
                           const void *pvData, uint32 cubData,
                           bool *pbRealloced, bool bFreeOnRealloc) {
  VarFieldBlockInfo_t *pVarFieldBlockInfo =
      PVarFieldBlockInfoFromRecord(pvRecord);
  *pbRealloced = false;
  ...
  if (pVarFieldBlockInfo->m_cubBlock == 0) {
    ...
    *pbRealloced = true;
    ...
    if (cubData != 0 && (cubData == pVarField->m_cubField)) {
      ...
      Q_memcpy(pVarFieldBlockInfo->m_pubBlock + pVarField->m_dubOffset, pvData,
               cubData);
    } else {
      // size change - realloc
      *pbRealloced = true;
      ...
      uint32 cubBlockNew =
          pVarFieldBlockInfo->m_cubBlock - pVarField->m_cubField + cubData;
      ...
      if (cubBlockNew) {
        // if this field has never been placed in the block (that is, it's not
        // changing an old value) and we have enough space, fastest to put it at
        // the end in the free space.
        if (pVarField->m_cubField == 0 && pVarField->m_dubOffset == 0 &&
            cubData <= pVarFieldBlockInfo->m_cubBlockFree) {
          uint8 *pubLastUsed = pVarFieldBlockInfo->m_pubBlock;
          for (int iField = 0; iField < m_VecField.Count(); ++iField) {
            Field_t &field = m_VecField[iField];
            if (field.BIsVariableLength()) {
              // Needs updating
              VarField_t *pVarFieldCur =
                  (VarField_t *)((uint8 *)pvRecord + field.m_dubOffset);
          ...
          // set up the field
          pVarField->m_cubField = cubData;
          pVarField->m_dubOffset =
              static_cast<int>((pubLastUsed - pVarFieldBlockInfo->m_pubBlock));
          ...
          while (pubBlockOldCursor < (pVarFieldBlockInfo->m_pubBlock +
                                      pVarFieldBlockInfo->m_cubBlock)) {
            if (pVarField->m_cubField &&
                ((int)pVarField->m_dubOffset ==
                 (pubBlockOldCursor - pVarFieldBlockInfo->m_pubBlock))) {
          ...
          pVarField->m_cubField = cubData;
          if (cubData > 0)
            pVarField->m_dubOffset = static_cast<int>(
                pubBlockNewCursor - pVarFieldBlockInfo->m_pubBlock);
          else
            pVarField->m_dubOffset = 0;
...
//			a stream, then the var block is after the fixed-length
//part of the 			record. So, we need to update the record's pointer to reflect that
// Input:	pvRecord -			Beginning of the serialized
// record
//-----------------------------------------------------------------------------
...
// Input:	pubRecord -			The record's in-memory data that
// we'll fill out
//			unPrimaryIndex -	Primary index of the record
//			pbRealloced -		Set to 'true' if the var-fields
//block for this record was reallocated 								(or allocated for the first time)
//			bFreeOnRealloc -	If true, we should Free() the
//var block after reallocating a new block 								(should be false if that block lives
//inside a NetPacket)
//-----------------------------------------------------------------------------
void CSchema::InitRecordRandom(uint8 *pubRecord, uint32 unPrimaryIndex,
                               bool *pbVarBlockRealloced,
                               bool bFreeVarBlockOnRealloc) {
...
//inside a NetPacket)
//-----------------------------------------------------------------------------
void CSchema::SetFieldRandom(uint8 *pubRecord, int iField,
                             bool *pbVarBlockRealloced,
                             bool bFreeVarBlockOnRealloc) {
  Field_t &field = m_VecField[iField];
  *pbVarBlockRealloced = false;
  ...
  if (k_EGCSQLType_String == field.m_EType) {
    ...
    // For strings, put in (cubData-1) random characters (each randomly in the
    // range [32,126]) then a trailing NULL
    ...
    BSetVarField(pubRecord, (VarField_t *)(pubRecord + field.m_dubOffset),
                 rgubBuff, cubData, pbVarBlockRealloced,
                 bFreeVarBlockOnRealloc);
  }
...
uint32 CSchema::CalcChecksum() {
  ...
  FOR_EACH_VEC(m_VecField, nField) {
    CRC32_ProcessBuffer(&crc32, &m_VecField[nField],
                        sizeof(m_VecField[nField]));
  }
...
void CSchema::AddIntField(char *pchName, char *pchSQLName, EGCSQLType eType,
                          int cubSize) {
  int nExpectedSize = -1;
  switch (eType) {
  ...
  AssertMsg2(nExpectedSize != -1,
             "Unexpected EType in AddIntField: %d for column %s", eType,
             pchSQLName);
  AssertMsg3(
      nExpectedSize == cubSize,
      "Unexpected size for in AddIntField for column %s: %d doesn't match %d ",
      pchSQLName, nExpectedSize, cubSize);
...
//			pchSQLName -		Name of the field in SQL
//database 			eType -				Type of the field 			cubSize -
//Size of the field 			pfnCompare -		Function used to compare fields
//(NULL if none specified)
//-----------------------------------------------------------------------------
void CSchema::AddField(char *pchName, char *pchSQLName, EGCSQLType eType,
                       uint32 cubSize, int cchMaxLength) {
...
//		pchName -			Name of the field to make
//primary index
//-----------------------------------------------------------------------------
int CSchema::PrimaryKey(bool bClustered, int nFillFactor, const char *pchName) {
  int iField = FindIField(pchName);
  AssertFatalMsg2(k_iFieldNil != iField,
                  "Could not find index column \"%s\" on table \"%s\"", pchName,
                  m_rgchName);

  Assert(m_nHasPrimaryKey ==
         k_EPrimaryKeyTypeNone); // may not already have a primary key defined
  Assert(m_iPKIndex == k_iFieldNil);

  // must not have been indexed in any way before
  Assert(0 == (m_VecField[iField].m_nColFlags &
               (k_nColFlagPrimaryKey | k_nColFlagIndexed | k_nColFlagUnique |
                k_nColFlagClustered)));
...
//			nFillFactor -		fill facto to use; 0 is database
//default 			pchName -			Name of the field to make primary index
//-----------------------------------------------------------------------------
int CSchema::PrimaryKeys(bool bClustered, int nFillFactor,
                         const char *pchNames) {
  ...
  if (nNewIndex != k_iFieldNil) {
    m_nHasPrimaryKey =
        k_EPrimaryKeyTypeMulti; // remember that we have multiple keys
...
// Purpose:	Marks a set of fields as the primary index
// Input:
//			pchName -			Names of the fields for
//the primary index; comma-separated if multiple 			pchIndexName -		Name of
//the index object (not in SQL) 			nFlags -			flags for
//CColumnInfo on this object 			nFillFactor -		fill facto to use; 0 is
//database default
//-----------------------------------------------------------------------------
int CSchema::AddIndexToFieldList(const char *pchNames, const char *pchIndexName,
                                 int nFlags, int nFillFactor) {
  ...
  do {
    ...
    AssertFatalMsg2(k_iFieldNil != iField,
                    "Could not find index column \"%s\" on table \"%s\"",
                    pchCurrent, m_rgchName);
...
//			pchIndexName -		Name of the index object (not in
//SQL)
//-----------------------------------------------------------------------------
...
//			pchName -			Name of the fields to
//index; comma separated if multiple
//-----------------------------------------------------------------------------
int CSchema::IndexFields(const char *pchIndexName, const char *pchNames) {
  ...
  // go add all those fields as Indexed
  int nNewIndex =
      AddIndexToFieldList(pchNames, pchIndexName, k_nColFlagIndexed, 0);
  return nNewIndex;
}

//-----------------------------------------------------------------------------
// Purpose:	Includes a column (or columns) in an index for the INCLUDE
// clause Input:	pchIndexName -		Name of the index object (not in
// SQL)
//			pchName -			Name of the fields to
//index; comma separated if multiple
//-----------------------------------------------------------------------------
void CSchema::AddIncludedFields(const char *pchIndexName,
                                const char *pchNames) {
  Assert(pchNames != NULL);
  Assert(pchIndexName != NULL);
...
int CSchema::UniqueFields(const char *pchIndexName, const char *pchNames) {
  ...
  // go add all those fields as Indexed
  int nNewIndex = AddIndexToFieldList(pchNames, pchIndexName,
                                      k_nColFlagIndexed | k_nColFlagUnique, 0);
  return nNewIndex;
}
...
//SQL)
//-----------------------------------------------------------------------------
int CSchema::ClusteredIndexField(int nFillFactor, const char *pchIndexName,
                                 const char *pchName) {
  Assert(pchName != NULL);
  Assert(pchIndexName != NULL);
  ...
  FOR_EACH_VEC(m_VecIndexes, iIndex) {
    if (m_VecIndexes[iIndex].IsClustered()) {
      AssertFatalMsg3(false,
                      "On table %s, can't make index %s clustered because %s "
                      "is already the clustered index\n",
                      m_rgchName, pchIndexName,
                      m_VecIndexes[iIndex].GetIndexName());
      return -1;
    }
  }
  ...
  // add an index to it
  int iIndex =
      AddIndexToFieldNumber(iField, pchIndexName, true /* clustered */);
...
//index; comma separated if multiple
//-----------------------------------------------------------------------------
int CSchema::ClusteredIndexFields(int nFillFactor, const char *pchIndexName,
                                  const char *pchNames) {
  Assert(pchNames != NULL);
  Assert(pchIndexName != NULL);
  ...
  // go add all those fields as Indexed
  int nNewIndex =
      AddIndexToFieldList(pchNames, pchIndexName,
                          k_nColFlagClustered | k_nColFlagIndexed, nFillFactor);
  return nNewIndex;
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CSchema::AddFullTextIndex(CSchemaFull *pSchemaFull,
                               const char *pchCatalogName,
                               const char *pchColumnName) {
  ...
  int nCatalogID =
      pSchemaFull->GetFTSCatalogByName(m_eSchemaCatalog, pchCatalogName);
  AssertFatalMsg2(nCatalogID != -1,
                  "Could not find fulltext catalog \"%s\" on table \"%s\"",
                  pchCatalogName, m_rgchName);
...
//-----------------------------------------------------------------------------
int CSchema::AddIndexToFieldNumber(int iField, const char *pchIndexName,
                                   bool bClustered) {
...
int CSchema::FindIField(const char *pchName) {
  for (int iField = 0; iField < m_VecField.Count(); iField++) {
    if (0 ==
        Q_strncmp(pchName, m_VecField[iField].m_rgchName, k_cSQLObjectNameMax))
      return iField;
  }
...
int CSchema::FindIFieldSQL(const char *pchName) {
  for (int iField = 0; iField < m_VecField.Count(); iField++) {
    if (0 == Q_strncmp(pchName, m_VecField[iField].m_rgchSQLName,
                       k_cSQLObjectNameMax))
      return iField;
  }
...
void CSchema::AddDeleteField(const char *pchFieldName) {
  ...
  Q_strncpy(deleteField.m_rgchFieldName, pchFieldName,
            sizeof(deleteField.m_rgchFieldName));
}
...
//			a different Schema to this one).
//-----------------------------------------------------------------------------
void CSchema::AddRenameField(const char *pchFieldNameOld,
                             const char *pchFieldNameNew) {
  RenameField_t &renameField = m_VecRenameField[m_VecRenameField.AddToTail()];
  Q_strncpy(renameField.m_rgchFieldNameOld, pchFieldNameOld,
            sizeof(renameField.m_rgchFieldNameOld));
...
//			pchFieldNameMew -	Name of the field in the new
//schema 			pfnAlterField -		Function to translate data from the old
//format to 								the new
//-----------------------------------------------------------------------------
void CSchema::AddAlterField(const char *pchFieldNameOld,
                            const char *pchFieldNameNew,
                            PfnAlterField_t pfnAlterField) {
  Assert(pfnAlterField);
  AlterField_t &alterField = m_VecAlterField[m_VecAlterField.AddToTail()];
  Q_strncpy(alterField.m_rgchFieldNameOld, pchFieldNameOld,
            sizeof(alterField.m_rgchFieldNameOld));
...
//			First we check our conversion instructions to see if any
//apply, 			and then we look for a straightforward match.
// Input:	pchFieldName -		Name of the field we're trying to map
//			piFieldDst -		[Return] Index of the field to
//map it to 			ppfnAlterField -	[Return] Optional function to convert data
// Output:	true if we know what to do with this field (if false, the
// conversion
//			is undefined and dangerous).
//-----------------------------------------------------------------------------
bool CSchema::BCanConvertField(const char *pchFieldName, int *piFieldDst,
                               PfnAlterField_t *ppfnAlterField) {
  *ppfnAlterField = NULL;

  // Should this field be deleted?
  for (int iDeleteField = 0; iDeleteField < m_VecDeleteField.Count();
       iDeleteField++) {
    if (0 == Q_strcmp(pchFieldName,
                      m_VecDeleteField[iDeleteField].m_rgchFieldName)) {
  ...
  for (int iRenameField = 0; iRenameField < m_VecRenameField.Count();
       iRenameField++) {
    if (0 == Q_strcmp(pchFieldName,
                      m_VecRenameField[iRenameField].m_rgchFieldNameOld)) {
  ...
  for (int iAlterField = 0; iAlterField < m_VecAlterField.Count();
       iAlterField++) {
    if (0 == Q_strcmp(pchFieldName,
                      m_VecAlterField[iAlterField].m_rgchFieldNameOld)) {
...
//-----------------------------------------------------------------------------
bool Field_t::BIsStringType() const { return (k_EGCSQLType_String == m_EType); }

//-----------------------------------------------------------------------------
// Purpose: Tell whether or not the field is of variable-length type.
//-----------------------------------------------------------------------------
bool Field_t::BIsVariableLength() const {
  return (k_EGCSQLType_String == m_EType) || (k_EGCSQLType_Blob == m_EType) ||
         (k_EGCSQLType_Image == m_EType);
}

//-----------------------------------------------------------------------------
// Purpose: Add a foreign key
//-----------------------------------------------------------------------------
void CSchema::AddFK(const char *pchName, const char *pchColumn,
                    const char *pchParentTable, const char *pchParentColumn,
                    EForeignKeyAction eOnDeleteAction,
                    EForeignKeyAction eOnUpdateAction) {
  int iTail = m_VecFKData.AddToTail();
  FKData_t &fkData = m_VecFKData[iTail];

  Q_snprintf(fkData.m_rgchName, Q_ARRAYSIZE(fkData.m_rgchName), "%s_%s",
             GetPchName(), pchName);
    ...
    if (pchColumn[i] != ',' && pchColumn[i] != 0) {
      ...
      while (iParentString < (uint)Q_strlen(pchParentColumn)) {
        if (pchParentColumn[iParentString] != ',') {
          colRelation.m_rgchParentCol[iParentColumn++] =
              pchParentColumn[iParentString];
          ++iParentString;
        } else {
          ++iParentString;
          break;
      ...
      AssertMsg(Q_strlen(colRelation.m_rgchParentCol),
                "Column counts for FK do not match between child/parent\n");
...
//-----------------------------------------------------------------------------
// Purpose: Caches the insert via MERGE statement for each table so we don't
// need to
//	generate it for each row we insert.
//-----------------------------------------------------------------------------
const char *
CSchema::GetMergeStatementTextOnPKWhenMatchedUpdateWhenNotMatchedInsert() {
  if (m_sMergeStatementTextOnPKWhenMatchedUpdateWhenNotMatchedInsert
          .IsEmpty()) {
    TSQLCmdStr sBuilder;
    BuildMergeStatementTextOnPKWhenMatchedUpdateWhenNotMatchedInsert(
        &sBuilder, GetRecordInfo());
    m_sMergeStatementTextOnPKWhenMatchedUpdateWhenNotMatchedInsert.Set(
        sBuilder);
  }
...
//-----------------------------------------------------------------------------
int CSchema::GetFKCount() { return m_VecFKData.Count(); }

//-----------------------------------------------------------------------------
// Purpose: Get data for a foreign key by index (valid for 0...GetFKCount()-1)
//-----------------------------------------------------------------------------
FKData_t &CSchema::GetFKData(int iIndex) { return m_VecFKData[iIndex]; }
...
void CSchema::Validate(CValidator &validator, const char *pchName) {
  ...
  FOR_EACH_VEC(m_VecFKData, i) { ValidateObj(m_VecFKData[i]); }
...
void CSchema::ValidateRecord(uint8 *pubRecord) {
  ...
  for (int iField = 0; iField < m_VecField.Count(); iField++) {
    ...
    // Ensure that strings are null-terminated, and that everything after the
    // terminator is 0
    if (k_EGCSQLType_String == field.m_EType) {
