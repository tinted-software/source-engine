...
static void RunAndClearListenerList(std::vector<LISTENER_FUNC> &vecListeners) {
  // Let us not underestimate the ability of random listeners to re-enter
  // everything.
...
CSQLAccess::CSQLAccess(ESchemaCatalog eSchemaCatalog)
    : m_eSchemaCatalog(eSchemaCatalog), m_pCurrentQuery(NULL),
...
CSQLAccess::~CSQLAccess() {
  ...
  AssertMsg(!m_bInTransaction,
            "GCSDK::CSQLAccess object being destroyed with a transaction "
            "pending. Use BCommitTransaction or RollbackTransaction to match "
            "your BBeginTransaction call.");
}

//------------------------------------------------------------------------------------
// Purpose: Perform a query
//------------------------------------------------------------------------------------
bool CSQLAccess::BYieldingExecute(const char *pchName,
                                  const char *pchSQLCommand,
                                  uint32 *pcRowsAffected, bool bSpewOnError) {
  if (NULL == pchName) {
...
bool CSQLAccess::BCommitTransaction(bool bAllowEmpty) {
  ...
  if (!m_pCurrentQuery && !m_pQueryGroup->GetStatementCount()) {
    if (bAllowEmpty) {
      ...
      AssertMsg1(false, "BCommitTransaction with empty transaction at %s",
                 m_pQueryGroup->PchName());
      return false;
    }
  }

  AssertMsg1(!m_pCurrentQuery,
             "Unexecuted query present in BCommitTransaction: %s",
             m_pCurrentQuery->PchCommand());
  if (m_pCurrentQuery)
    return false;
  ...
  // The transaction presumably did make the database, so we do not notify
  // rollback listeners beyond here.
  RunListeners_Commit();
...
//------------------------------------------------------------------------------------
// Purpose: Adds a listener to be called synchronously should the transaction
// successfully commit
//------------------------------------------------------------------------------------
void CSQLAccess::AddCommitListener(std::function<void(void)> &&listener) {
  if (!BInTransaction()) {
    AssertMsg(BInTransaction(),
              "Adding a listener to a non-transaction access, will never fire");
    return;
  }
...
//------------------------------------------------------------------------------------
// Purpose: Notifies listeners of a implicitly or explicitly rolled back
// transactions and clears the listener list.
//------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------
CSQLAccess::EReadSingleResultResult
CSQLAccess::BYieldingExecuteSingleResultDataInternal(
    const char *pchName, const char *pchSQLCommand, EGCSQLType eType,
    uint8 **ppubData, uint32 *punSize, uint32 *pcRowsAffected,
    bool bHasDefaultValue) {
  AssertMsg(
      !BInTransaction(),
      "BYieldingExecuteSingleResultData is not supported in a transaction");
  if (BInTransaction())
  ...
  if (m_pQueryGroup->GetResults()->GetResultSetCount() != 1) {
    AssertMsg1(false, "Expected single result set, found %d",
               m_pQueryGroup->GetResults()->GetResultSetCount());
    return eReadSingle_Error;
  }
  ...
  // If we either have more than one row or no default value specified, that's
  // an error.
  if (pResultSet->GetRowCount() != 1) {
    AssertMsg1(false, "Expected single result, found %d",
               pResultSet->GetRowCount());
    return eReadSingle_Error;
  }

  if (pResultSet->GetColumnCount() != 1) {
    AssertMsg1(false, "Expected single column, found %d",
               pResultSet->GetColumnCount());
    return eReadSingle_Error;
  }
  if (pResultSet->GetColumnType(0) != eType) {
    AssertMsg2(false, "Expected column of type %s, found %s",
               PchNameFromEGCSQLType(eType),
               PchNameFromEGCSQLType(pResultSet->GetColumnType(0)));
    return eReadSingle_Error;
  }

  return pResultSet->GetData(0, 0, ppubData, punSize) ? eReadSingle_ResultFound
                                                      : eReadSingle_Error;
}
...
// Purpose: Perform a query that returns a single string
//------------------------------------------------------------------------------------
bool CSQLAccess::BYieldingExecuteString(const char *pchName,
                                        const char *pchSQLCommand,
                                        CFmtStr1024 *psResult,
                                        uint32 *pcRowsAffected) {
  uint8 *pubData;
  uint32 cubData;
  if (CSQLAccess::BYieldingExecuteSingleResultDataInternal(
          pchName, pchSQLCommand, k_EGCSQLType_String, &pubData, &cubData,
          pcRowsAffected, false) != eReadSingle_ResultFound)
    return false;
...
//------------------------------------------------------------------------------------
bool CSQLAccess::BYieldingExecuteScalarInt(const char *pchName,
                                           const char *pchSQLCommand,
                                           int *pnResult,
                                           uint32 *pcRowsAffected) {
  return BYieldingExecuteSingleResult<int32, uint32>(
      pchName, pchSQLCommand, k_EGCSQLType_int32, pnResult, pcRowsAffected);
}

bool CSQLAccess::BYieldingExecuteScalarIntWithDefault(const char *pchName,
                                                      const char *pchSQLCommand,
                                                      int *pnResult,
                                                      int iDefaultValue,
                                                      uint32 *pcRowsAffected) {
  return BYieldingExecuteSingleResultWithDefault<int32, uint32>(
      pchName, pchSQLCommand, k_EGCSQLType_int32, pnResult, iDefaultValue,
      pcRowsAffected);
}
...
//------------------------------------------------------------------------------------
bool CSQLAccess::BYieldingExecuteScalarUint32(const char *pchName,
                                              const char *pchSQLCommand,
                                              uint32 *punResult,
                                              uint32 *pcRowsAffected) {
  return BYieldingExecuteSingleResult<uint32, uint32>(
      pchName, pchSQLCommand, k_EGCSQLType_int32, punResult, pcRowsAffected);
}

bool CSQLAccess::BYieldingExecuteScalarUint32WithDefault(
    const char *pchName, const char *pchSQLCommand, uint32 *punResult,
    uint32 unDefaultValue, uint32 *pcRowsAffected) {
  return BYieldingExecuteSingleResultWithDefault<uint32, uint32>(
      pchName, pchSQLCommand, k_EGCSQLType_int32, punResult, unDefaultValue,
      pcRowsAffected);
}
...
void CSQLAccess::AddBindParamRaw(EGCSQLType eType, const byte *pubData,
                                 uint32 cubData) {
...
uint32 CSQLAccess::GetResultSetRowCount(uint32 unResultSet) {
  if (m_pQueryGroup->GetResults() &&
      unResultSet < m_pQueryGroup->GetResults()->GetResultSetCount())
    return m_pQueryGroup->GetResults()
        ->GetResultSet(unResultSet)
        ->GetRowCount();
  else
    return 0;
}
...
CSQLRecord CSQLAccess::GetResultRecord(uint32 unResultSet, uint32 unRow) {
  if (m_pQueryGroup->GetResults() &&
      unResultSet < m_pQueryGroup->GetResults()->GetResultSetCount()) {
    IGCSQLResultSet *pResultSet =
        m_pQueryGroup->GetResults()->GetResultSet(unResultSet);
...
bool CSQLAccess::BYieldingInsertRecord(const CRecordBase *pRecordBase) {
  ...
  const char *pchStatement =
      pRecordBase->GetPSchema()->GetInsertStatementText();
...
bool CSQLAccess::BYieldingInsertWhenNotMatchedOnPK(CRecordBase *pRecordBase) {
  ...
  uint32 nRows;
  const char *pchStatement =
      pRecordBase->GetPSchema()
          ->GetMergeStatementTextOnPKWhenNotMatchedInsert();
...
bool CSQLAccess::BYieldingInsertOrUpdateOnPK(CRecordBase *pRecordBase) {
  ...
  const char *pchStatement =
      pRecordBase->GetPSchema()
          ->GetMergeStatementTextOnPKWhenMatchedUpdateWhenNotMatchedInsert();
...
//-----------------------------------------------------------------------------
// Purpose: Inserts a new record into the DB and reads non-insertable fields
// back
//			into the record.
// Input:	pRecordBase - record to insert
// Output:	true if successful, false otherwise
...
bool CSQLAccess::BYieldingInsertWithIdentity(CRecordBase *pRecordBase) {
  AssertMsg(!BInTransaction(),
            "BYieldingInsertWithIdentity is not supported in a transaction");
  if (BInTransaction())
    return false;
  ClearParams();
  ...
  AssertMsg(vecOutputFields.Count() > 0,
            "BYieldingInsertAndReadRecord called for a record type with no "
  ...
  for (uint32 nColumn = 0; nColumn < pResultSet->GetColumnCount(); nColumn++) {
    ...
    Assert(pResultSet->GetColumnType(nColumn) ==
           pRecordInfo->GetColumnInfo(nSchColumn).GetType());
...
// Output:	true if successful, false otherwise
//-----------------------------------------------------------------------------
EResult CSQLAccess::YieldingReadRecordWithWhereColumns(
    CRecordBase *pRecord, const CColumnSet &readSet, const CColumnSet &whereSet,
    const char *pchOrderClause) {
  AssertMsg(
      !BInTransaction(),
      "BYieldingReadRecordWithWhereColumns is not supported in a transaction");
  if (BInTransaction())
    return k_EResultInvalidState;

  // if there is an order by clause, only take the top one, if there isn't, then
  // validate that we have a single instance
  ...
  if (GetResultSetCount() != 1) {
    AssertMsg(
        GetResultSetCount() == 1,
        "Unexpected number of result sets returned from select statement");
    return k_EResultFail;
  }
  ...
  // note that since we only take the top one when there is an order by clause,
  // we don't need to handle that case down here, only if top 2 is selected
  if (pResultSet->GetRowCount() != 1) {
    ...
    AssertMsg1(false,
               "BYieldingReadRecordWithWhereColumns from %s failing because "
               "multiple records match WHERE clause",
               readSet.GetRecordInfo()->GetName());
  ...
  FOR_EACH_COLUMN_IN_SET(readSet, nColumnIndex) {
    ...
    AssertMsg2(eResultType == eRecordType, "Column %d type mismatch in %s",
               nColumnIndex, readSet.GetRecordInfo()->GetName());
  ...
  FOR_EACH_COLUMN_IN_SET(readSet, nColumnIndex) {
    ...
    DbgVerify(
        sqlRecord.BGetColumnData(nColumnIndex, &pubData, (int *)&cubData));
    DbgVerify(
        pRecord->BSetField(readSet.GetColumn(nColumnIndex), pubData, cubData));
  }
...
// Input:	record - data source for columns to match against (whereColumns)
// and
...
// Output:	true if successful, false otherwise
//-----------------------------------------------------------------------------
bool CSQLAccess::BYieldingUpdateRecord(
    const CRecordBase &record, const CColumnSet &whereColumns,
    const CColumnSet &updateColumns,
    const CSQLOutputParams *pOptionalOutputParams /* = NULL */) {
  return BYieldingUpdateRecords(record, whereColumns, record, updateColumns,
                                pOptionalOutputParams);
}

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
bool CSQLAccess::BYieldingUpdateRecords(
    const CRecordBase &whereRecord, const CColumnSet &whereColumns,
  ...
  if (whereColumns.GetRecordInfo() != updateColumns.GetRecordInfo())
    return false;
  Assert(whereColumns.GetRecordInfo() ==
         whereRecord.GetPSchema()->GetRecordInfo());
  if (whereColumns.GetRecordInfo() != whereRecord.GetPSchema()->GetRecordInfo())
    return false;
  Assert(whereColumns.GetRecordInfo() ==
         updateRecord.GetPSchema()->GetRecordInfo());
  if (whereColumns.GetRecordInfo() !=
      updateRecord.GetPSchema()->GetRecordInfo())
    return false;

  AssertMsg(
      !updateColumns.IsEmpty(),
      "Someone is calling BYieldingUpdateRecord with no columns to update.");
  ...
  if (pOptionalOutputParams) {
    ...
    AddRecordParameters(pOptionalOutputParams->GetRecord(),
                        pOptionalOutputParams->GetColumnSet());
  }
...
//			whereColumns - columns to use when searching for this
//record
//-----------------------------------------------------------------------------
bool CSQLAccess::BYieldingDeleteRecords(const CRecordBase &record,
                                        const CColumnSet &whereColumns) {
...
CSQLUpdateOrInsert::CSQLUpdateOrInsert(const char *pszName, int nTable,
                                       const CColumnSet &whereColumns,
                                       const CColumnSet &updateColumns,
                                       const char *pszWhereClause,
                                       const char *pszUpdateClause) {
  const CRecordInfo *pRecordInfo =
      GSchemaFull().GetSchema(nTable).GetRecordInfo();
  ...
  sStatement.Append(GSchemaFull().GetDefaultSchemaNameForCatalog(
      pRecordInfo->GetESchemaCatalog()));
  sStatement.Append('.');
  sStatement.Append(pRecordInfo->GetName());
  sStatement.Append(" WITH(HOLDLOCK) AS D USING(VALUES(");
  sStatement.AppendFormat("%.*s", GetInsertArgStringChars(nNumColumns),
                          GetInsertArgString());
  sStatement.Append("))AS S(");

  // add each column that we are adding the values for, along with the parameter
  // from the structure
  ...
  if (pszWhereClause) {
    ...
    FOR_EACH_COLUMN_IN_SET(whereColumns, nCurrColumn) {
      const char *pszColName =
          pRecordInfo->GetColumnInfo(whereColumns.GetColumn(nCurrColumn))
              .GetName();
      if (nCurrColumn > 0)
  ...
  if (pszUpdateClause || !updateColumns.IsEmpty()) {
    ...
    if (pszUpdateClause) {
      ...
      FOR_EACH_COLUMN_IN_SET(updateColumns, nCurrColumn) {
        const char *pszColName =
            pRecordInfo->GetColumnInfo(updateColumns.GetColumn(nCurrColumn))
                .GetName();
        if (nCurrColumn > 0)
          sStatement.Append(',');
...
bool CSQLUpdateOrInsert::BYieldingExecute(
    CSQLAccess &sqlAccess, const CRecordBase &record,
    uint32 *out_punRowsAffected /* = NULL */) const {
  AssertMsg2(record.GetITable() == m_nTable,
             "Error: Merge was compiled for table %s, but was attempted to be "
             "executed against %s",
             GSchemaFull().GetSchema(m_nTable).GetRecordInfo()->GetName(),
             record.GetPRecordInfo()->GetName());
...
//-----------------------------------------------------------------------------
// Purpose: Adds bind parameters to the list based on a set of fields in a
// record Input:	record - record to insert
//			columnSet - The set of columns to add as params
//-----------------------------------------------------------------------------
void CSQLAccess::AddRecordParameters(const CRecordBase &record,
                                     const CColumnSet &columnSet) {
  ...
  FOR_EACH_COLUMN_IN_SET(columnSet, nColumnIndex) {
    ...
    uint8 *pubData;
    uint32 cubData;
    DbgVerify(record.BGetField(columnSet.GetColumn(nColumnIndex), &pubData,
                               &cubData));
...
// Note:	PERFORMANCE WARNING: this is slow on big tables, not intended
// for use
//				in production
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Returns the current query to add stuff to, creating it if there
// isn't
