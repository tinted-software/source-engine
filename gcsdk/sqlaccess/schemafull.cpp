...
CSchemaFull &GSchemaFull() { return g_SchemaFull; }
...
//			by loading it or by in GenerateIntrinsic).  It
//calculates our checksum 			and allocates our scratch buffer.
//-----------------------------------------------------------------------------
...
void CSchemaFull::SetITable(CSchema *pSchema, int iTable) {
  ...
  for (int iSchema = 0; iSchema < m_VecSchema.Count(); iSchema++) {
    if (pSchema != &m_VecSchema[iSchema])
      AssertFatalMsg(m_VecSchema[iSchema].GetITable() != iTable,
...
//-----------------------------------------------------------------------------
// Purpose: Uninits the schema.  Need to call this explicitly before app
// shutdown
//			on static instances of this object, as the CSchema
//objects 			point to memory in static memory pools which may destruct 			before
//static instances of this object.
//-----------------------------------------------------------------------------
...
//			added a new schema to ourselves, and we need to make
//sure that it 			matches the corresponding C class.
// Input:	pSchema -			Schema to check
//			cField -			Number of fields the
//schema should contain. 			cubRecord -			Size of a record in the
//schema
//-----------------------------------------------------------------------------
void CSchemaFull::CheckSchema(CSchema *pSchema, int cField, uint32 cubRecord) {
  ...
  // same source.  We check a couple of things to make sure that they're
  // properly in sync.
  ...
  // This is unlikely to fail.  It indicates some kind of size mismatch (maybe a
  // packing problem?)
  if (pSchema->CubRecordFixed() != cubRecord) {
    // You may hit this if END_FIELDDATA_HAS_VAR_FIELDS is not used properly
    EmitError(SPEW_SQL,
              "Table %s has an inconsistent size (class = %d, schema = %d)\n",
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CSchemaFull::AddFullTextCatalog(enum ESchemaCatalog eCatalog,
                                     const char *pstrCatalogName,
                                     int nFileGroup) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CSchemaFull::GetFTSCatalogByName(enum ESchemaCatalog eCatalog,
                                     const char *pstrCatalogName) {
  int nIndex = -1;
...
void CSchemaFull::AddDeleteTable(const char *pchTableName) {
  ...
  Q_strncpy(deleteTable.m_rgchTableName, pchTableName,
            sizeof(deleteTable.m_rgchTableName));
}
...
//			a different SchemaFull to this one).
//-----------------------------------------------------------------------------
void CSchemaFull::AddRenameTable(const char *pchTableNameOld,
                                 const char *pchTableNameNew) {
  RenameTable_t &renameTable = m_VecRenameTable[m_VecRenameTable.AddToTail()];
  Q_strncpy(renameTable.m_rgchTableNameOld, pchTableNameOld,
            sizeof(renameTable.m_rgchTableNameOld));
...
//			a different SchemaFull to this one).
//-----------------------------------------------------------------------------
void CSchemaFull::AddDeleteField(const char *pchTableName,
...
//			a different SchemaFull to this one).
//-----------------------------------------------------------------------------
void CSchemaFull::AddRenameField(const char *pchTableName,
...
//			a different SchemaFull to this one).
//-----------------------------------------------------------------------------
void CSchemaFull::AddAlterField(const char *pchTableName,
                                const char *pchFieldNameOld,
                                const char *pchFieldnameNew,
  ...
  m_VecSchema[iSchema].AddAlterField(pchFieldNameOld, pchFieldnameNew,
                                     pfnAlterField);
}
...
//-----------------------------------------------------------------------------
void CSchemaFull::AddTrigger(ESchemaCatalog eCatalog, const char *pchTableName,
                             const char *pchTriggerName,
                             ETriggerType eTriggerType,
                             const char *pchTriggerText) {
  CTriggerInfo trigger;
  trigger.m_eTriggerType = eTriggerType;
  trigger.m_eSchemaCatalog = eCatalog;
  Q_strncpy(trigger.m_szTriggerName, pchTriggerName,
            Q_ARRAYSIZE(trigger.m_szTriggerName));
  Q_strncpy(trigger.m_szTriggerTableName, pchTableName,
            Q_ARRAYSIZE(trigger.m_szTriggerTableName));
...
//			piTableDst -		[Return] Index of the table to
//map it to
// Output:	true if we know what to do with this table (if false, the
// conversion
...
bool CSchemaFull::BCanConvertTable(const char *pchTableName, int *piTableDst) {
  ...
  for (int iDeleteTable = 0; iDeleteTable < m_VecDeleteTable.Count();
       iDeleteTable++) {
    if (0 == Q_strcmp(pchTableName,
                      m_VecDeleteTable[iDeleteTable].m_rgchTableName)) {
  ...
  for (int iRenameTable = 0; iRenameTable < m_VecRenameTable.Count();
       iRenameTable++) {
    if (0 == Q_strcmp(pchTableName,
                      m_VecRenameTable[iRenameTable].m_rgchTableNameOld)) {
...
//-----------------------------------------------------------------------------
const char *
CSchemaFull::GetDefaultSchemaNameForCatalog(ESchemaCatalog eCatalog) {
...
void CSchemaFull::Validate(CValidator &validator, const char *pchName) {
  ...
  FOR_EACH_VEC(m_vecFTSCatalogs, i) { ValidateObj(m_vecFTSCatalogs[i]); }

  ValidateObj(m_VecTriggers);
  FOR_EACH_VEC(m_VecTriggers, i) { ValidateObj(m_VecTriggers[i]); }
