...
CColumnSet::CColumnSet(const CRecordInfo *pRecordInfo, int col1, int col2,
                       int col3, int col4, int col5, int col6, int col7)
    : m_pRecordInfo(pRecordInfo) {
...
CColumnSet &CColumnSet::operator+=(const CColumnSet &rhs) {
  ...
  FOR_EACH_COLUMN_IN_SET(rhs, i) { BAddColumn(rhs.GetColumn(i)); }
...
void CColumnSet::BAddColumn(int nColumn) {
  if (nColumn >= 0 && nColumn < m_pRecordInfo->GetNumColumns()) {
    ...
    AssertMsg3(false,
               "Attempting to set an out of range column on schema type %s, %d "
               "(of %d)",
               GetRecordInfo()->GetName(), nColumn,
               m_pRecordInfo->GetNumColumns());
  }
}
...
//-----------------------------------------------------------------------------
// Purpose: Makes the column set be the full set of all columns in the record
// info
//-----------------------------------------------------------------------------
void CColumnSet::MakeFull() {
  ...
  for (int nColumn = 0; nColumn < m_pRecordInfo->GetNumColumns(); nColumn++) {
    // do a direct add to avoid the exponential cost since we know we won't have
    // conflicts
    m_vecColumns.AddToTail(nColumn);
  }
}
...
void CColumnSet::MakeInsertable() {
  ...
  for (int nColumn = 0; nColumn < m_pRecordInfo->GetNumColumns(); nColumn++) {
    ...
    if (columnInfo.BIsInsertable()) {
      // do a direct add to avoid the exponential cost since we know we won't
      // have conflicts
      m_vecColumns.AddToTail(nColumn);
    }
  }
...
//-----------------------------------------------------------------------------
// determines if the current column set has all fields set. Useful for detection
// of new columns being added to the schema
//-----------------------------------------------------------------------------
