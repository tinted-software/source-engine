...
void CRecordBase::Copy(const CRecordBase &that) {
  Cleanup();
  Q_memcpy(PubRecordFixed(), that.PubRecordFixed(),
           GetPSchema()->CubRecordFixed());
}
...
void CRecordVar::Copy(const CRecordBase &baseThat) {
  ...
  if (VarFieldBlockInfo_t *pVarBlockInfo =
          GetPSchema()->PVarFieldBlockInfoFromRecord(PubRecordFixed())) {
    if (pVarBlockInfo->m_cubBlock) {
      void *pvNewBlock = malloc(pVarBlockInfo->m_cubBlock);
      Q_memcpy(pvNewBlock, pVarBlockInfo->m_pubBlock,
               pVarBlockInfo->m_cubBlock);
...
void CRecordExternal::Copy(const CRecordBase &baseThat) {
  ...
  Q_memcpy(m_pubRecordFixedExternal, that.PubRecordFixed(),
           m_pSchema->CubRecordFixed());
  SetFlag(k_EAllocatedFixed, true);

  SetFlag(k_EAllocatedVarBlock, false);
  if (VarFieldBlockInfo_t *pVarBlockInfo =
          m_pSchema->PVarFieldBlockInfoFromRecord(PubRecordFixed())) {
    if (pVarBlockInfo->m_cubBlock) {
      void *pvNewBlock = malloc(pVarBlockInfo->m_cubBlock);
      Q_memcpy(pvNewBlock, pVarBlockInfo->m_pubBlock,
               pVarBlockInfo->m_cubBlock);
...
// Input:	pSchema -			Schema for the record this will
// hold
//-----------------------------------------------------------------------------
...
// Input:	pSchema -			Schema for the record this will
// hold
//			pubRecord -			Pointer to fixed record
//data 			bTakeOwnership -	Should we delete the record when destroyed
...
int CRecordExternal::Init(CSchema *pSchema, uint8 *pubRecord,
                          bool bTakeOwnership) {
  m_pSchema = pSchema;
...
//-----------------------------------------------------------------------------
void CRecordBase::RenderField(uint32 unColumn, int cchBuffer,
                              char *pchBuffer) const {
  ...
  ConvertFieldToText(GetPRecordInfo()->GetColumnInfo(unColumn).GetType(),
                     pubData, cubData, pchBuffer, cchBuffer, false);
}
...
void CRecordVar::Cleanup() {
  ...
  if (BFlagSet(k_EAllocatedVarBlock)) {
    void *pvVarBlock =
        m_pSchema->PVarFieldBlockInfoFromRecord(PubRecordFixed())->m_pubBlock;
    free(pvVarBlock);
    m_pSchema->PVarFieldBlockInfoFromRecord(PubRecordFixed())->m_pubBlock =
        NULL;
    SetFlag(k_EAllocatedVarBlock, false);
...
bool CRecordBase::BAssureRecordVarStorage(uint32 cVariableBytes) {
  ...
  if (pVarFieldBlockInfo) {
    ...
    if (pVarFieldBlockInfo->m_cubBlock != 0) {
      ...
      Q_memcpy(pubData, pVarFieldBlockInfo->m_pubBlock,
    ...
    pVarFieldBlockInfo->m_cubBlockFree =
        cVariableBytes - pVarFieldBlockInfo->m_cubBlock;
...
void CRecordExternal::InitRecordRandom(uint32 unPrimaryIndex) {
  ...
  GetPSchema()->InitRecordRandom(PubRecordFixed(), unPrimaryIndex, &bRealloced,
                                 BFlagSet(k_EAllocatedVarBlock));
...
void CRecordExternal::SetFieldRandom(int iField) {
  bool bRealloced = false;
  GetPSchema()->SetFieldRandom(PubRecordFixed(), iField, &bRealloced,
                               BFlagSet(k_EAllocatedVarBlock));
...
// Input:	iField -			Field to set
//			ppubData -			Receives pointer to
//fields data 			pcubField -			Receives count of bytes of data
//(will count the null for strings)
// Output:	true if succeeds
//-----------------------------------------------------------------------------
bool CRecordBase::BGetField(int iField, uint8 **ppubData,
                            uint32 *pcubField) const {
  return GetPSchema()->BGetFieldData(PubRecordFixed(), iField, ppubData,
                                     pcubField);
}
...
//			pubData -			pointer to field data to
//copy from 			cubData -			size in bytes of that data
...
bool CRecordBase::BSetField(int iField, void *pvData, uint32 cubData,
  ...
  if (!GetPSchema()->BSetFieldData(PubRecordFixed(), iField, pubData, cubData,
                                   pbRealloced))
    return false;
...
//-----------------------------------------------------------------------------
// Purpose: Erases a field, setting it to 0 length (if possible) and filling
// with nulls Input:	iField -			index of field to wipe
// NOTE:	This relies on CSchema::BSetFieldData nulling out the rest of a
// field when it is set to 0 length!
//-----------------------------------------------------------------------------
void CRecordBase::WipeField(int iField) {
  ...
  // Length should be 0, except for non-variable length strings where length
  // should be 1 (for an empty string "")
  int cub = 0;
  ...
  GetPSchema()->BSetFieldData(PubRecordFixed(), iField, (uint8 *)&un, cub,
                              &bRealloced);
...
//-----------------------------------------------------------------------------
// Purpose: Get a string field - will return empty string instead of NULL if
// field has no datas Input:	iField -			Field to get
//			pcubField -			Receives count of bytes
//of data (will count the null for strings)
...
const uint8 *CRecordBase::ReadVarDataField(const CVarField &field,
                                           uint32 *pcubField) const {
  Assert(false);
  return NULL;
}
...
bool CRecordBase::SetVarCharField(CVarCharField &field, const char *pchString,
                                  bool bTruncate, int32 iField) {
  Assert(false);
  return false;
}

void CRecordBase::SetVarDataField(CVarField &field, const void *pvData,
                                  uint32 cubData) {
  Assert(false);
}
...
// Output:	pointer to data - may be NULL if that field is empty.
//-----------------------------------------------------------------------------
const uint8 *CRecordVar::ReadVarDataField(const CVarField &field,
                                          uint32 *pcubField) const {
  Assert(GetPSchema()->BHasVariableFields());
...
//-----------------------------------------------------------------------------
bool CRecordVar::SetVarCharField(CVarCharField &field, const char *pchString,
                                 bool bTruncate, int32 iField) {
  Assert(GetPSchema()->BHasVariableFields());
  if (iField < 0) {
    AssertMsg1(false,
               "Encountered a bad call to SetVarCharField with an invalid "
               "field specified: %d",
               iField);
    return false;
  }
  ...
  if ((cchMaxLength > 0) && (cchLen > cchMaxLength)) {
    if (bTruncate) {
      ...
      AssertMsg4(
          false,
          "Overflow in SetVarCharField (%u > %u) for column %s in table %s",
          cchLen, cchMaxLength, m_pSchema->GetField(iField).m_rgchName,
          m_pSchema->GetPchName());
      return false;
    }
  }
  ...
  bool bRealloced = false;
  bool fSuccess =
      GetPSchema()->BSetVarField(PubRecordFixed(), &field, pchString, cchLen,
                                 &bRealloced, BFlagSet(k_EAllocatedVarBlock));
  if (fSuccess && bTruncated) {
    // make sure the last character is NULL if we truncated
    VarFieldBlockInfo_t *pBlock =
        GetPSchema()->PVarFieldBlockInfoFromRecord(PubRecordFixed());
...
// Input:	field -				opaque field object to update
//			pvData -			pointer to data to put
//there 			cubData -			size in bytes of the data
//-----------------------------------------------------------------------------
void CRecordVar::SetVarDataField(CVarField &field, const void *pvData,
                                 uint32 cubData) {
  Assert(GetPSchema()->BHasVariableFields());

  bool bRealloced = false;
  GetPSchema()->BSetVarField(PubRecordFixed(), &field, pvData, cubData,
                             &bRealloced, BFlagSet(k_EAllocatedVarBlock));
...
void CRecordVar::Validate(CValidator &validator, const char *pchName) {
  ...
  if (BFlagSet(k_EAllocatedVarBlock)) {
    validator.ClaimMemory(GetPSchema()
                              ->PVarFieldBlockInfoFromRecord(PubRecordFixed())
                              ->m_pubBlock);
  }
}
