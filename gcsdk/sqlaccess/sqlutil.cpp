...
uint32 GetInsertArgStringChars(uint32 nNumParams) {
  AssertMsg(nNumParams <= GetInsertArgStringMaxParams(),
            "Error: Requested more characters than are provided by the "
            "GetInsertArgString");
  if (nNumParams == 0)
    return 0;
...
uint32 GetInsertArgStringMaxParams() { return 512; }
...
//			bForPreparedStatement - Should we prepare the text for a
//prepared statement or directly place the values?
//-----------------------------------------------------------------------------
void ConvertFieldArrayToInText(const CColumnInfo &columnInfo, byte *pubData,
                               int cubData, char *rgchResult, int cubResultLen,
                               bool bForPreparedStatement) {
  ...
  for (int i = 0; i < nArrayCount; ++i) {
    if (bForPreparedStatement) {
      ...
      case k_EGCSQLType_int8:
        if (i < nArrayCount - 1)
          len += Q_snprintf(rgchResult + len, cubResultLen - len, "%d,",
                            *((byte *)pubData));
        else
          len += Q_snprintf(rgchResult + len, cubResultLen - len, "%d)",
                            *((byte *)pubData));
      ...
      case k_EGCSQLType_int64:
        if (i < nArrayCount - 1)
          len += Q_snprintf(rgchResult + len, cubResultLen - len, "%lld,",
                            *((int64 *)pubData));
        else
          len += Q_snprintf(rgchResult + len, cubResultLen - len, "%lld)",
                            *((int64 *)pubData));
        break;
      default:
        AssertMsg(false, "Unsupported data type for non prepares statement "
                         "with IN clause\n");
        rgchResult[0] = 0;
        return;
      }
...
//-----------------------------------------------------------------------------
void ConvertFieldToText(EGCSQLType eFieldType, uint8 *pubRecord, int cubRecord,
                        char *rgchField, int cchField, bool bQuoteString) {
...
//			In the worst case, the text may expand by 2x.  (If the
//field is all 			single quotes.)  So, you must pass in a buffer which is at least
//			twice as long as the text length so we can guarantee to
//be able to 			escape the string.
//-----------------------------------------------------------------------------
void EscapeStringValue(char *rgchField, int cchField) {
  ...
  // don't need to be escaped, so try to be as fast as possible in the normal
  // case.

  // first, walk through the string and count the string length and number of
  // single quotes
  while (*pubCur) {
    if ('\'' == *pubCur)
      cSingleQuotes++;
  ...
  // We know exactly how many characters the string will expand by (the # of
  // single quotes).  Walk backward and copy the characters into the right
  // places.  This touches each character only once.
  ...
  *pubCur = 0;
  ...
  while (pubCur > rgchField && cSingleQuotes > 0) {
    ...
    *pubCur = *pubRead;
    if ('\'' == *pubRead) {
      // if the character is a single quote, back up one more and insert another
      // single quote to escape it
      pubCur--;
      *pubCur = '\'';
...
//-----------------------------------------------------------------------------
// Purpose: Adds constraint information to a SQL command to add or remove
// constraint Input:	pchTableName - name of table
...
//-----------------------------------------------------------------------------
void AppendConstraint(const char *pchTableName, const char *pchColumnName,
                      int nColFlagConstraint, bool bForAdd, bool bClustered,
  ...
  case k_nColFlagPrimaryKey:
    sCmd.AppendFormat(" CONSTRAINT %s_%s_PrimaryKey", pchTableName,
                      pchColumnName);
    if (bForAdd) {
      sCmd += " PRIMARY KEY ";
      if (bClustered) {
...
//-----------------------------------------------------------------------------
// Purpose: Adds constraint information to a SQL command to add or remove
// constraint Input:	pRecordInfo - record info describing table
//			pColumnInfo - record info describing column
//			bForAdd - whether constraint is being added or removed
//			pchCmd - buffer to append SQL command to
//			cchCmd - size of buffer
//-----------------------------------------------------------------------------
void AppendConstraints(const CRecordInfo *pRecordInfo,
                       const CColumnInfo *pColumnInfo, bool bForAdd,
                       CFmtStrMax &sCmd) {
  ...
  if (pColumnInfo->BIsPrimaryKey()) {
    ...
    if (pRecordInfo->GetPrimaryKeyType() == k_EPrimaryKeyTypeSingle) {
      ...
      int nFillFactor =
          refFields.Element(pRecordInfo->GetPKIndex()).GetFillFactor();
      AppendConstraint(pRecordInfo->GetName(), pColumnInfo->GetName(),
                       k_nColFlagPrimaryKey, bForAdd,
                       pColumnInfo->BIsClustered(), sCmd, nFillFactor);
    }
  } else if (pColumnInfo->BIsUnique()) {
    AppendConstraint(pRecordInfo->GetName(), pColumnInfo->GetName(),
                     k_nColFlagUnique, bForAdd, pColumnInfo->BIsClustered(),
                     sCmd, 0);
  }

  if (pColumnInfo->BIsAutoIncrement()) {
    AppendConstraint(pRecordInfo->GetName(), pColumnInfo->GetName(),
                     k_nColFlagAutoIncrement, bForAdd,
                     pColumnInfo->BIsClustered(), sCmd, 0);
  }
}
...
//-----------------------------------------------------------------------------
void BuildTablePKConstraintText(TSQLCmdStr *psStatement,
...
//-----------------------------------------------------------------------------
// Purpose: Adds constraint information to a SQL command to add or remove
// table-level constraints Input:	pRecordInfo - record info describing
// table
//			pchCmd - buffer to append SQL command to
//			cchCmd - size of buffer
//-----------------------------------------------------------------------------
...
void AppendTableConstraints(CRecordInfo *pRecordInfo, CFmtStrMax &sCmd) {
  ...
  for (int i = 0; i < cFKs; ++i) {
    ...
    TSQLCmdStr sTmp;
    sTmp.sprintf(", CONSTRAINT %s FOREIGN KEY (%s) REFERENCES %s(%s) ON DELETE "
                 "%s ON UPDATE %s",
                 fkData.m_rgchName, sColumns.Access(),
                 fkData.m_rgchParentTableName, sParentColumns.Access(),
                 PchNameFromEForeignKeyAction(fkData.m_eOnDeleteAction),
                 PchNameFromEForeignKeyAction(fkData.m_eOnUpdateAction));
...
//			pRecordInfo - record info describing table inserting
//into
//-----------------------------------------------------------------------------
void BuildInsertStatementText(TSQLCmdStr *psStatement,
                              const CRecordInfo *pRecordInfo) {
  psStatement->sprintf("INSERT INTO %s.%s (",
                       GSchemaFull().GetDefaultSchemaNameForCatalog(
  ...
  psStatement->AppendFormat(") VALUES (%.*s)",
                            GetInsertArgStringChars(nInsertable),
                            GetInsertArgString());
}

//-----------------------------------------------------------------------------
// Purpose: Builds a SQL INSERT statement
//			IMPORTANT NOTE - This Insert statement will use the
//Microsoft SQL Server 			specific clause 'OUTPUT Inserted.ColumnName' 			The result
//of that will be that the SQL statement will return to us 			the columns that
//could not be specified by the Insert. 			At the time of writing, that is
//primarily AutoIncrement columns, 			however in theory we should be able to
//recover any computed column 			from SQL server, with the caveats specified at :
...
void BuildInsertAndReadStatementText(TSQLCmdStr *psStatement,
                                     CUtlVector<int> *pvecOutputFields,
                                     const CRecordInfo *pRecordInfo) {
  psStatement->sprintf("INSERT INTO %s.%s (",
                       GSchemaFull().GetDefaultSchemaNameForCatalog(
                           pRecordInfo->GetESchemaCatalog()),
  ...
  for (int iColumn = 0; iColumn < cColumns; iColumn++) {
    ...
    //
    //	If we can't Insert it - we want SQL Server to tell us what value was
...
//-----------------------------------------------------------------------------
// Purpose: Builds a SQL MERGE statement update or insert using in-flight values
// table Input:	psStatement - The string to put the statement into
//			pRecordInfo - record info describing table inserting
//into
//-----------------------------------------------------------------------------
void BuildMergeStatementTextOnPKWhenMatchedUpdateWhenNotMatchedInsert(
    TSQLCmdStr *psStatement, const CRecordInfo *pRecordInfo) {
  psStatement->sprintf("MERGE INTO %s.%s WITH( HOLDLOCK, ROWLOCK ) T USING ( "
                       "VALUES (%.*s) ) AS S(",
                       GSchemaFull().GetDefaultSchemaNameForCatalog(
                           pRecordInfo->GetESchemaCatalog()),
                       pRecordInfo->GetName(),
                       GetInsertArgStringChars(pRecordInfo->GetNumColumns()),
                       GetInsertArgString());
  ...
  const FieldSet_t &fsPK =
      pRecordInfo->GetIndexFields()[pRecordInfo->GetPKIndex()];
  {
    int cColumns = fsPK.GetCount();
    for (int iColumn = 0; iColumn < cColumns; iColumn++) {
      const CColumnInfo &columnInfo =
          pRecordInfo->GetColumnInfo(fsPK.GetField(iColumn));
      if (iColumn)
...
//-----------------------------------------------------------------------------
// Purpose: Builds a SQL MERGE statement using CTE_MergeParams as supplied table
// holding rows Input:	psStatement - The string to put the statement into
//			pRecordInfo - record info describing table inserting
//into
//-----------------------------------------------------------------------------
void BuildMergeStatementTextOnPKWhenNotMatchedInsert(
    TSQLCmdStr *psStatement, const CRecordInfo *pRecordInfo) {
  psStatement->sprintf("MERGE INTO %s.%s WITH( HOLDLOCK, ROWLOCK ) T USING ( "
                       "VALUES (%.*s) ) AS S(",
                       GSchemaFull().GetDefaultSchemaNameForCatalog(
...
void BuildSelectStatementText(TSQLCmdStr *psStatement,
                              const CColumnSet &selectSet,
                              const char *pchTopClause) {
  *psStatement = "SELECT ";
  ...
  psStatement->Append(GSchemaFull().GetDefaultSchemaNameForCatalog(
      selectSet.GetRecordInfo()->GetESchemaCatalog()));
  psStatement->Append('.');
...
//			bForPreparedStatement - if true, inserts values as '?'
//for later 				binding.  If false, values are inserted in text. 			pchStatement -
//pointer to buffer to build statement in 			cchStatement - size of buffer
...
//-----------------------------------------------------------------------------
void BuildUpdateStatementText(TSQLCmdStr *psStatement,
                              const CColumnSet &updateColumns) {
  // build the UPDATE statement
  psStatement->sprintf("UPDATE %s.%s SET ",
                       GSchemaFull().GetDefaultSchemaNameForCatalog(
                           updateColumns.GetRecordInfo()->GetESchemaCatalog()),
                       updateColumns.GetRecordInfo()->GetName());
...
// Purpose: Builds a SQL UPDATE statement
//-----------------------------------------------------------------------------
void BuildDeleteStatementText(TSQLCmdStr *psStatement,
                              const CRecordInfo *pRecordInfo) {
  psStatement->sprintf("DELETE FROM %s.%s",
                       GSchemaFull().GetDefaultSchemaNameForCatalog(
                           pRecordInfo->GetESchemaCatalog()),
                       pRecordInfo->GetName());
}
...
////-----------------------------------------------------------------------------
//// Purpose: our own special "upsert" into a column with a uniqueness
///constraint
////-----------------------------------------------------------------------------
// EResult UpdateOrInsertUnique( CSQLAccess &sqlAccess, int iTable, int iField,
// CRecordBase *pRecordBase, int iIndexID )
//{
//	// attempt an update - if it fails due to duplicate primary key, they
//can't use this
//	// url (it's taken) - if it succeeds but affects 0 rows, they didn't
//have a vanity url
//	// and we need to do an insert (which could again fail due to primary
//key constraints) 	int cRecordsUpdated = 0; 	bool bRet =
//sqlAccess.BYieldingUpdateFieldFromRecordWithIndex( iTable, &cRecordsUpdated,
//iField, pRecordBase, iIndexID ); 	if ( !bRet )
//	{
...
//		if ( !bRet )
//		{
//			// ODBC is the suck - give me Spring JDBC templates,
//please. 			if ( sqlAccess.GetLastError()->IsDuplicateInsertAttempt() )
//			{
//				return k_EResultDuplicateName;
//			}
//			return k_EResultFail;
