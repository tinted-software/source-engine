...
CThreadSafeClassMemoryPool<CRecordInfo>
    CRecordInfo::sm_MemPoolRecordInfo(10, UTLMEMORYPOOL_GROW_FAST);

#ifdef _DEBUG
// validation tracking
CUtlRBTree<CRecordInfo *, int>
    CRecordInfo::sm_mapPMemPoolRecordInfo(DefLessFunc(CRecordInfo *));
...
//-----------------------------------------------------------------------------
/* static */
bool FieldSet_t::CompareFieldSets(const FieldSet_t &refThis,
  ...
  for (int m = 0; m < cColumns; m++) {
    ...
    const CColumnInfo &refThisColumn =
        pRecordInfoThis->GetColumnInfo(nThisField);

    int nOtherField = refOther.GetField(m);
    const CColumnInfo &refOtherColumn =
        pRecordInfoOther->GetColumnInfo(nOtherField);
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CRecordInfo::CRecordInfo() : m_MapIColumnInfo(0, 0, CaselessStringLessThan) {
  m_rgchName[0] = 0;
...
void CRecordInfo::InitFromDSSchema(CSchema *pSchema) {
  ...
  for (int iField = 0; iField < cFields; iField++) {
    ...
    AddColumn(field.m_rgchSQLName, iField + 1, field.m_EType, field.m_cubLength,
              field.m_nColFlags, field.m_cchMaxLength);
  }
...
//-----------------------------------------------------------------------------
void CRecordInfo::AddColumn(const char *pchName, int nSQLColumn,
                            EGCSQLType eType, int cubFixedSize, int nColFlags,
                            int cchMaxSize) {
  ...
  columnInfo.Set(pchName, nSQLColumn, eType, cubFixedSize, nColFlags,
                 cchMaxSize);
}
...
//-----------------------------------------------------------------------------
int __cdecl CompareColumnInfo(const CColumnInfo *pColumnInfoLeft,
                              const CColumnInfo *pColumnInfoRight) {
...
void CRecordInfo::GetIndexFieldList(CFmtStr1024 *pstr, int nIndents) const {
  ...
  for (int n = 0; n < m_VecIndexes.Count(); n++) {
    ...
    pstr->AppendFormat("Index %d (%s): %sclustered, %sunique {", n,
                       fs.GetIndexName(), fs.IsClustered() ? "" : "non-",
                       fs.IsUnique() ? "" : "non-");
...
//-----------------------------------------------------------------------------
int CRecordInfo::GetFKCount() { return m_VecFKData.Count(); }

//-----------------------------------------------------------------------------
// Purpose: Get data for a foreign key by index (valid for 0...GetFKCount()-1)
//-----------------------------------------------------------------------------
FKData_t &CRecordInfo::GetFKData(int iIndex) { return m_VecFKData[iIndex]; }
...
void CRecordInfo::GetFKListString(CFmtStr1024 *pstr, int nIndents) {
  ...
  if (m_VecFKData.Count() == 0) {
    ...
    for (int n = 0; n < m_VecFKData.Count(); n++) {
      ...
      pstr->AppendFormat(
          "CONSTRAINT %s FOREIGN KEY (%s) REFERENCES %s(%s) ON DELETE %s ON "
          "UPDATE %s\n",
          fkData.m_rgchName, sColumns.Access(), fkData.m_rgchParentTableName,
          sParentColumns.Access(),
...
//			column order in an FTS is irrelevant, so this is a
//simple match
//-----------------------------------------------------------------------------
bool CRecordInfo::CompareFTSIndexLists(CRecordInfo *pOther) const {
  ...
  for (int nColumnIndex = 0; nColumnIndex < m_vecFTSFields.Count();
       nColumnIndex++) {
    bool bFound = false;
    for (int nInnerIndex = 0; nInnerIndex < pOther->m_vecFTSFields.Count();
         nInnerIndex++) {
...
void CRecordInfo::PrepareForUse() {
  ...
  /*
  Assert( ( m_nHasPrimaryKey == k_EPrimaryKeyTypeNone && m_VecPKFields.Count()
  == 0 ) || ( m_nHasPrimaryKey == k_EPrimaryKeyTypeMulti &&
  m_VecPKFields.Count() > 1) || ( m_nHasPrimaryKey == k_EPrimaryKeyTypeSingle &&
  m_VecPKFields.Count() == 1) );
  */
...
// Input:	pchName - name
// Notes:	record info that describes a table will have a name (the table
// name);
...
//			found, we'll set the flags on the new item from the
//existing one.
//-----------------------------------------------------------------------------
...
//			nColFlags - attributes
//-----------------------------------------------------------------------------
void CColumnInfo::Set(const char *pchName, int nSQLColumn, EGCSQLType eType,
                      int cubFixedSize, int nColFlags, int cchMaxSize) {
  Assert(!m_rgchName[0]);
  Q_strncpy(m_rgchName, pchName, Q_ARRAYSIZE(m_rgchName));
...
bool CColumnInfo::BIsVariableLength() const {
  return m_eType == k_EGCSQLType_Blob || m_eType == k_EGCSQLType_String ||
         m_eType == k_EGCSQLType_Image;
}
...
void CRecordInfo::ValidateStatics(CValidator &validator, const char *pchName) {
  ...
  FOR_EACH_MAP_FAST(sm_mapPMemPoolRecordInfo, i) {
    sm_mapPMemPoolRecordInfo[i]->Validate(validator,
                                          "sm_mapPMemPoolRecordInfo[i]");
  }
#endif
}
