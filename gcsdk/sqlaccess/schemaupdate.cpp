...
//-----------------------------------------------------------------------------
void EmitAndAppend(CFmtStr1024 &sTarget, const CGCEmitGroup &Group, int iLevel,
                   int iLevelLog, PRINTF_FORMAT_STRING const char *pchMessage,
                   ...) {
  va_list args;
...
//		[WITH (FILLFACTOR = n)]
//-----------------------------------------------------------------------------
CUtlString GetAddIndexSQL(CRecordInfo *pRecordInfo,
                          const FieldSet_t &refFields) {
  CFmtStrMax sCmd;
  sCmd.sprintf("CREATE %s%sINDEX %s ON App%u.%s (",
               refFields.IsClustered() ? "CLUSTERED " : "",
               refFields.IsUnique() ? "UNIQUE " : "", refFields.GetIndexName(),
  ...
  for (int n = 0; n < refFields.GetCount(); n++) {
    ...
    sCmd.AppendFormat("%s%s", (n > 0) ? "," : "", refInfo.GetName());
  }
  sCmd += ")";
...
CUtlString GetAlterColumnText(CRecordInfo *pRecordInfo,
  ...
  CUtlString sCmd;
  sCmd.Format(
      "ALTER TABLE App%u.%s ALTER COLUMN %s %s %s", GGCBase()->GetAppID(),
      pRecordInfo->GetName(), pColumnInfoDesired->GetName(),
...
void CSchemaUpdate::AddRecordInfoDesired(CRecordInfo *pRecordInfo) {
  ...
  Assert(m_mapPRecordInfoDesired.InvalidIndex() ==
         m_mapPRecordInfoDesired.Find(pchName));
...
//			should exist in the database
//-----------------------------------------------------------------------------
void CSchemaUpdate::AddTriggerInfos(
    const CUtlVector<CTriggerInfo> &refTriggerInfo) {
...
bool CJobUpdateSchema::BYieldingUpdateSchema(ESchemaCatalog eSchemaCatalog) {
  ...
  // TODO - do one round of inspection only first so we can send watchdog alert
  // about what's about to happen, then do conversions.  Also force conversions
  // in dev system.
  ...
  for (int iTable = 0; iTable < m_iTableCount; iTable++) {
    // MERGE COMMENT: "schema" cannot be used as a variable name because it is
    // an empty #define resulting in error C2059
  ...
  SQLRETURN nRet =
      YieldingEnsureDatabaseSchemaCorrect(eSchemaCatalog, pSchemaUpdate);
  if (!SQL_OK(nRet)) {
...
//-----------------------------------------------------------------------------
// Purpose: Examines actual running schema of database, compares to specified
// desired
//			schema, and changes the actual schema to correspond to
//desired schema
// Input:	pSQLThread - SQL thread to execute on
//			pSchemaUpdate - pointer to object with desired schema
// Output:  SQL return code.
//-----------------------------------------------------------------------------
SQLRETURN CJobUpdateSchema::YieldingEnsureDatabaseSchemaCorrect(
    ESchemaCatalog eSchemaCatalog, CSchemaUpdate *pSchemaUpdate) {
  Assert(pSchemaUpdate);

  CMapPRecordInfo &mapPRecordInfoDesired =
      pSchemaUpdate->m_mapPRecordInfoDesired;
  const CUtlVector<CTriggerInfo> &vecTriggerInfoDesired =
      pSchemaUpdate->m_vecTriggerInfo;
  ...
  //
  // Do some up-front bookkeeping to see how many tables need to be created
  // and/or altered
  //
  ...
  nRet = YieldingGetRecordInfoForAllTables(eSchemaCatalog, nSchemaID,
  ...
  // Look through the list of desired tables, find any that are missing or
  // different from the actual schema
  FOR_EACH_MAP_FAST(mapPRecordInfoDesired, iRecordInfoDesired) {
    ...
    if (pRecordInfoDesired->GetESchemaCatalog() == eSchemaCatalog) {
      ...
      int iRecordInfoActual =
          mapPRecordInfoActual.Find(pRecordInfoDesired->GetName());
      if (mapPRecordInfoDesired.InvalidIndex() == iRecordInfoActual) {
        ...
        // Table with same name exists in desired & actual schemas; is it
        // exactly the same?
        CRecordInfo *pRecordInfoActual =
            mapPRecordInfoActual[iRecordInfoActual];
        if (!pRecordInfoDesired->EqualTo(pRecordInfoActual)) {
          // This desired table exists but the actual table is different than
          // desired
  ...
  // Now, look through the list of actual tables and find any that do not exist
  // in the list of desired tables
  FOR_EACH_MAP_FAST(mapPRecordInfoActual, iRecordInfoActual) {
    ...
    int iRecordInfoDesired =
        mapPRecordInfoDesired.Find(pRecordInfoActual->GetName());
  ...
  FOR_EACH_VEC(vecTriggerInfoDesired, iDesired) {
    ...
    FOR_EACH_VEC(vecTriggerInfoActual, iActual) {
      ...
      if (vecTriggerInfoActual[iActual] == vecTriggerInfoDesired[iDesired]) {
        ...
        if (vecTriggerInfoActual[iActual].IsDifferent(
                vecTriggerInfoDesired[iDesired])) {
  ...
  EmitAndAppend(
      sDetail, SPEW_GC, 2, 2,
      "# of specified tables that do not currently exist in database: %d\n",
  ...
  for (int iTable = 0; iTable < vecPRecordInfoDesiredMissing.Count();
       iTable++) {
  ...
  EmitAndAppend(
      sDetail, SPEW_GC, 2, 2,
      "[GC]: # of specified tables that differ from schema in database: %d\n",
  ...
  for (int iTable = 0; iTable < vecPRecordInfoActualDifferent.Count();
       iTable++) {
    ...
    // We know something is different between the actual & desired schema for
    // this table, but don't yet know what

    // Compare each column
    for (int iColumnDesired = 0;
         iColumnDesired < pRecordInfoDesired->GetNumColumns();
         iColumnDesired++) {
      const CColumnInfo &columnInfoDesired =
          pRecordInfoDesired->GetColumnInfo(iColumnDesired);
      int iColumnActual = -1;
      bool bRet = pRecordInfoActual->BFindColumnByName(
          columnInfoDesired.GetName(), &iColumnActual);
      if (bRet) {
        const CColumnInfo &columnInfoActual =
            pRecordInfoActual->GetColumnInfo(iColumnActual);
    ...
    for (int iColumnActual = 0;
         iColumnActual < pRecordInfoActual->GetNumColumns(); iColumnActual++) {
      const CColumnInfo &columnInfoActual =
          pRecordInfoActual->GetColumnInfo(iColumnActual);
      int iColumnDesired = -1;
      bool bRet = pRecordInfoDesired->BFindColumnByName(
          columnInfoActual.GetName(), &iColumnDesired);
      if (!bRet) {
        // this column exists in the running schema, but not in the desired
        // schema (e.g. old column)
        vecPColumnInfoActualUnknown.AddToTail(&columnInfoActual);
      }
    }

    if ((vecPColumnInfoDesiredMissing.Count() > 0) ||
        (vecPColumnInfoActualDifferent.Count() > 0)) {
      pSchemaUpdate->m_bConversionNeeded = true;
    ...
    if (vecPColumnInfoDesiredMissing.Count() > 0) {
      EmitAndAppend(sDetail, SPEW_GC, 2, 2,
                    "\t\tDesired columns missing in table %s:\n",
                    pRecordInfoActual->GetName());

      for (int iColumn = 0; iColumn < vecPColumnInfoDesiredMissing.Count();
           iColumn++) {
        const CColumnInfo *pColumnInfoDesired =
            vecPColumnInfoDesiredMissing[iColumn];
        EmitAndAppend(sDetail, SPEW_GC, 2, 2, "\t\t\t%s\n",
        ...
        if (bDoConversion) {
          ...
          tickCounter.Start();
          // Add the column
          nRet = YieldingAlterTableAddColumn(eSchemaCatalog, pRecordInfoActual,
                                             pColumnInfoDesired);
          EXIT_ON_SQL_FAILURE(nRet);
          tickCounter.End();
          int nElapsedMilliseconds =
              tickCounter.GetDuration().GetMilliseconds();
          EmitAndAppend(sDetail, SPEW_GC, 2, 2,
                        "\t\t\t\tCreated column %s: %d millisec\n",
    ...
    for (int idx = 0; idx < pRecordInfoActual->GetIndexFieldCount(); ++idx) {
      ...
      if (idx == pRecordInfoActual->GetPKIndex()) {
        EmitAndAppend(
            sDetail, SPEW_GC, 2, 2,
            "\tTable %s primary key in database differs from specification.\n",
            pRecordInfoDesired->GetName());
        nRet = YieldingProcessUnsafeConversion(
            eSchemaCatalog,
            CFmtStr("ALTER TABLE App%u.%s DROP CONSTRAINT %s",
                    GGCBase()->GetAppID(), pRecordInfoActual->GetName(),
                    fs.GetIndexName())
                .String(),
            CFmtStr("%s: remove old primary key.",
                    pRecordInfoDesired->GetName())
                .String());
        EXIT_ON_SQL_FAILURE(nRet);
      } else {
        EmitAndAppend(sDetail, SPEW_GC, 2, 2,
                      "\tTable %s index %s exists in database but is not in "
                      "specification.  (Possible performance problem?).\n",
                      pRecordInfoDesired->GetName(), fs.GetIndexName());
        nRet = YieldingProcessUnsafeConversion(
            eSchemaCatalog,
            CFmtStr("DROP INDEX %s ON App%u.%s", fs.GetIndexName(),
                    GGCBase()->GetAppID(), pRecordInfoActual->GetName())
                .String(),
            CFmtStr("%s: cleanup stray index %s.",
                    pRecordInfoDesired->GetName(), fs.GetIndexName())
                .String());
        EXIT_ON_SQL_FAILURE(nRet);
      }
    ...
    if (vecPColumnInfoActualDifferent.Count() > 0) {
      EmitAndAppend(sDetail, SPEW_GC, 2, 2,
                    "\tColumns that differ between specification and database "
                    "in table %s:\n",
                    pRecordInfoActual->GetName());
      for (int iColumn = 0; iColumn < vecPColumnInfoActualDifferent.Count();
           iColumn++) {
        const CColumnInfo *pColumnInfoActual =
            vecPColumnInfoActualDifferent[iColumn];
        EmitAndAppend(sDetail, SPEW_GC, 2, 2, "\t\t%s",
                      pColumnInfoActual->GetName());
        int iColumnDesired = -1;
        DbgVerify(pRecordInfoDesired->BFindColumnByName(
            pColumnInfoActual->GetName(), &iColumnDesired));
        const CColumnInfo *pColumnInfoDesired =
            &pRecordInfoDesired->GetColumnInfo(iColumnDesired);
        ...
        if ((pColumnInfoDesired->GetType() != pColumnInfoActual->GetType()) ||
            ...
            (!pColumnInfoDesired->BIsVariableLength() &&
             (pColumnInfoDesired->GetFixedSize() !=
              pColumnInfoActual->GetFixedSize())) ||
            // variable length field, and the sizes differ
            // fixed length field, and the sizes differ
            (pColumnInfoDesired->BIsVariableLength() &&
             (pColumnInfoDesired->GetMaxSize() !=
              pColumnInfoActual->GetMaxSize()))) {
          if (k_EConversionModeConvertIrreversible != m_eConversionMode) {
            pSchemaUpdate->m_bSkippedAChange = true;
            if (pColumnInfoDesired->GetType() != pColumnInfoActual->GetType())
              EmitAndAppend(
                  sDetail, SPEW_GC, 2, 2,
                  "\t(data types differ: desired=%s, actual=%s)",
                  PchNameFromEGCSQLType(pColumnInfoDesired->GetType()),
                  PchNameFromEGCSQLType(pColumnInfoActual->GetType()));
            if (pColumnInfoDesired->GetFixedSize() !=
                pColumnInfoActual->GetFixedSize())
              EmitAndAppend(sDetail, SPEW_GC, 2, 2,
                            "\t(column sizes differ: desired=%d, actual=%d)",
                            pColumnInfoDesired->GetFixedSize(),
                            pColumnInfoActual->GetFixedSize());
            if (pColumnInfoDesired->GetMaxSize() !=
                pColumnInfoActual->GetMaxSize())
              EmitAndAppend(
                  sDetail, SPEW_GC, 2, 2,
                  "\t(maximum column sizes differ: desired=%d, actual=%d)",
                  pColumnInfoDesired->GetMaxSize(),
                  pColumnInfoActual->GetMaxSize());
          }
          nRet = YieldingChangeColumnTypeOrLength(
              eSchemaCatalog, pRecordInfoActual, pColumnInfoDesired);
          EXIT_ON_SQL_FAILURE(nRet);
        }

        // If column constraints/indexes are different, make appropriate
        // adjustments do this second so it has a chance to succeed - otherwise,
        // we may create an index here that prevents us from performing an alter
        // table
        if (pColumnInfoDesired->GetColFlags() !=
            pColumnInfoActual->GetColFlags()) {
          if (k_EConversionModeConvertIrreversible != m_eConversionMode) {
            char szDesiredFlags[k_nKiloByte];
            char szActualFlags[k_nKiloByte];
            pColumnInfoDesired->GetColFlagDescription(
                szDesiredFlags, Q_ARRAYSIZE(szDesiredFlags));
            pColumnInfoActual->GetColFlagDescription(
                szActualFlags, Q_ARRAYSIZE(szActualFlags));

            pSchemaUpdate->m_bSkippedAChange = true;
            EmitAndAppend(
                sDetail, SPEW_GC, 2, 2,
                "\t(column flags differ: desired=\"%s\", actual=\"%s\")",
                szDesiredFlags, szActualFlags);
          }
          nRet = YieldingChangeColumnProperties(
              eSchemaCatalog, pRecordInfoActual, pColumnInfoActual,
              pColumnInfoDesired);
          EXIT_ON_SQL_FAILURE(nRet);
        }
    ...
    for (int idx = 0; idx < pRecordInfoDesired->GetIndexFieldCount(); ++idx) {
      ...
      if (idx == pRecordInfoDesired->GetPKIndex()) {
        EmitAndAppend(
            sDetail, SPEW_GC, 2, 2,
            "\tTable %s primary key in specification differs from database.\n",
            pRecordInfoDesired->GetName());
        sComment.Format("%s: create new primary key constraint",
                        pRecordInfoDesired->GetName());
        TSQLCmdStr cmd;
        ...
        for (int i = 0; i < fs.GetCount(); ++i) // make sure they are non-NULL
        {
          ...
          const CColumnInfo *pColInfo =
              &pRecordInfoDesired->GetColumnInfo(idxField);
        ...
        sCreatePK.Format("GO\nALTER TABLE App%u.%s ADD %s\n",
                         GGCBase()->GetAppID(), pRecordInfoDesired->GetName(),
                         cmd.String());
        sCommand += sCreatePK;
      } else {
        ...
        // detect this common case and provide a more specific message.
        // (Otherwise, we will report it as a missing index, and an extra
        // unwanted index --- which is correct but a bit more confusing.)

        iActualIdx = pRecordInfoActual->FindIndexByName(fs.GetIndexName());
        if (iActualIdx < 0) {
          sComment.Format("%s: add index %s", pRecordInfoDesired->GetName(),
                          fs.GetIndexName());
          EmitAndAppend(
              sDetail, SPEW_GC, 2, 2,
              "\tTable %s index %s is specified but not present in database.\n",
              pRecordInfoDesired->GetName(), fs.GetIndexName());
        } else {
          EmitAndAppend(sDetail, SPEW_GC, 2, 2,
                        "\tTable %s index %s differs between specification and "
                        "database.\n",
                        pRecordInfoDesired->GetName(), fs.GetIndexName());
          sComment.Format("%s: fix index %s", pRecordInfoDesired->GetName(),
                          fs.GetIndexName());
      ...
      nRet =
          YieldingProcessUnsafeConversion(eSchemaCatalog, sCommand, sComment);
      EXIT_ON_SQL_FAILURE(nRet);
    }

    // Just to be safe, let's run the old code, too.
    if (!bIndexMismatch &&
        !pRecordInfoActual->CompareIndexLists(pRecordInfoDesired)) {
      EmitAndAppend(sDetail, SPEW_GC, 2, 2,
                    "\tIndex sets in table %s differ between specification and "
                    "database [GC]:\n",
                    pRecordInfoDesired->GetName());
      CFmtStr1024 sTemp;
    ...
    if (!pRecordInfoActual->CompareFKs(pRecordInfoDesired)) {
      EmitAndAppend(sDetail, SPEW_GC, 2, 2,
                    "\tForeign Key constraints in table %s differs between "
                    "specification and database [GC]:\n",
                    pRecordInfoDesired->GetName());
    ...
    if (vecPColumnInfoActualUnknown.Count() > 0) {
      EmitAndAppend(sDetail, SPEW_GC, 2, 2,
                    "\tColumns in database [GC] table %s that are not in "
                    "specification (ignored):\n",
                    pRecordInfoActual->GetName());

      // Since this can actually destroy data, let's not ever, ever run it
      // automatically.
      CUtlString sCommand;
      sCommand.Format("ALTER TABLE App%u.%s DROP COLUMN ",
                      GGCBase()->GetAppID(), pRecordInfoActual->GetName());

      for (int iColumn = 0; iColumn < vecPColumnInfoActualUnknown.Count();
           iColumn++) {
    ...
    pSchemaUpdate->m_cColumnsDesiredMissing +=
        vecPColumnInfoDesiredMissing.Count();
    pSchemaUpdate->m_cColumnsActualDifferent +=
        vecPColumnInfoActualDifferent.Count();
    pSchemaUpdate->m_cColumnsActualUnknown +=
        vecPColumnInfoActualUnknown.Count();
  }

  EmitAndAppend(sDetail, SPEW_GC, 2, 2,
                "[GC]: # of tables that currently exist in database but were "
                "unspecified: %d\n",
  ...
  if (vecTriggerInfoMissing.Count() > 0) {
    EmitAndAppend(
        sDetail, SPEW_GC, 2, 2,
        "[GC]: # of specified triggers that do not currently exist: %d\n",
    ...
    FOR_EACH_VEC(vecTriggerInfoMissing, iMissing) {
      ...
      // Create the trigger
      nRet = YieldingCreateTrigger(eSchemaCatalog,
                                   vecTriggerInfoMissing[iMissing]);
      EXIT_ON_SQL_FAILURE(nRet);
      tickCounter.End();
      int nElapsedMilliseconds = tickCounter.GetDuration().GetMilliseconds();
      EmitAndAppend(sDetail, SPEW_GC, 2, 2,
                    "[GC]: Created trigger %s on table %s: %d millisec\n",
                    vecTriggerInfoMissing[iMissing].m_szTriggerName,
                    vecTriggerInfoMissing[iMissing].m_szTriggerTableName,
  ...
  FOR_EACH_VEC(vecTriggerInfoDifferent, iDifferent) {
    EmitAndAppend(
        sDetail, SPEW_GC, 2, 2,
        "[GC]: Trigger %s on table %s differs from the desired trigger\n",
  ...
  FOR_EACH_VEC(vecTriggerInfoActual, iActual) {
    ...
    if (!vecTriggerInfoActual[iActual].m_bMatched) {
      SQLRETURN nSQLReturn =
          YieldingDropTrigger(eSchemaCatalog, vecTriggerInfoActual[iActual]);

      EmitAndAppend(sDetail, SPEW_GC, 2, 2,
                    "[GC]: Trigger %s on table %s is not in the declared "
                    "schema ... Drop %s",
...
Exit:
  ...
  if (!m_sRecommendedSQL.IsEmpty() || !m_sDataDestroyingCleanupSQL.IsEmpty()) {
    EmitAndAppend(
        sDetail, SPEW_GC, 2, 2,
        "\tThe following SQL might work to fixup schema differences.\n");
    EmitAndAppend(sDetail, SPEW_GC, 2, 2, "\t** \n");
    EmitAndAppend(sDetail, SPEW_GC, 2, 2,
                  "\t** DISCLAIMER ** This conversion code is not well tested. "
                  " Review the SQL and use at your own risk.\n");
    EmitAndAppend(sDetail, SPEW_GC, 2, 2, "\t** \n");
    {
    ...
    if (!m_sDataDestroyingCleanupSQL.IsEmpty()) {
      EmitAndAppend(
          sDetail, SPEW_GC, 2, 2,
          "\t\t-- WARNING: The following operations will *destroy* data that "
          "is in the database but not in the specification.\n");
      EmitAndAppend(sDetail, SPEW_GC, 2, 2,
                    "\t\t--          If you have manually created extra tables "
                    "or columns in your database, it will appear here!\n");
      CUtlVectorAutoPurge<char *> vecLines;
  ...
  pSchemaUpdate->m_cTablesActualDifferent =
      vecPRecordInfoActualDifferent.Count();
...
// Output:  SQL return code.
//-----------------------------------------------------------------------------
SQLRETURN CJobUpdateSchema::YieldingGetSchemaID(ESchemaCatalog eSchemaCatalog,
                                                int *pSchemaID) {
  CSQLAccess sqlAccess(eSchemaCatalog);
  CFmtStr1024 sDefaultSchema;
  if (!sqlAccess.BYieldingExecuteString(
          FILE_AND_LINE,
          "SELECT default_schema_name FROM sys.database_principals WHERE "
          "name=CURRENT_USER",
          &sDefaultSchema))
    return SQL_ERROR;
  ...
  if (0 != Q_stricmp(sDefaultSchema, sExpectedDefaultSchema)) {
    AssertMsg2(
        false,
        "SQL connection has the wrong default schema. Expected: %s. Actual %s",
        sExpectedDefaultSchema.Get(), sDefaultSchema.Get());
    return SQL_ERROR;
  }

  sqlAccess.AddBindParam(sDefaultSchema);
  if (!sqlAccess.BYieldingExecuteScalarInt(FILE_AND_LINE, "SELECT SCHEMA_ID(?)",
                                           pSchemaID))
    return SQL_ERROR;
...
// Output:  SQL return code.
//-----------------------------------------------------------------------------
SQLRETURN CJobUpdateSchema::YieldingGetRecordInfoForAllTables(
    ESchemaCatalog eSchemaCatalog, int nSchemaID,
    CMapPRecordInfo &mapPRecordInfo) {
  CSQLAccess sqlAccess(eSchemaCatalog);

  // create a query that returns all tables in the database
  sqlAccess.AddBindParam(nSchemaID);
  RETURN_SQL_ERROR_ON_FALSE(sqlAccess.BYieldingExecute(
      FILE_AND_LINE, "SELECT object_id, name FROM sys.objects WHERE type_desc "
                     "= 'USER_TABLE' AND is_ms_shipped = 0 AND schema_id = ?"));
  ...
  FOR_EACH_SQL_RESULT(sqlAccess, 0, tableIDRecord) {
    ...
    YieldingGetColumnInfoForTable(eSchemaCatalog, mapPRecordInfo, nTableID,
                                  sTableName);
  }
...
SQLRETURN CJobUpdateSchema::YieldingGetColumnInfoForTable(
    ESchemaCatalog eSchemaCatalog, CMapPRecordInfo &mapPRecordInfo,
    int nTableID, const char *pchTableName) {
  CSQLAccess sqlAccess(eSchemaCatalog);

  sqlAccess.AddBindParam(nTableID);
  RETURN_SQL_ERROR_ON_FALSE(sqlAccess.BYieldingExecute(
      FILE_AND_LINE, "SELECT name, column_id, user_type_id, max_length, "
                     "is_identity FROM sys.columns WHERE object_id = ?"));
  ...
  FOR_EACH_SQL_RESULT(sqlAccess, 0, columnInfo) {
    ...
    pRecordInfo->AddColumn(sColumnName, nColumnID,
                           GetEGCSQLTypeForMSSQLType(nType), nMaxLength,
                           nColFlags, nMaxLength);
  }
...
//-----------------------------------------------------------------------------
SQLRETURN CJobUpdateSchema::YieldingGetTriggers(
    ESchemaCatalog eSchemaCatalog, int nSchemaID,
    CUtlVector<CTriggerInfo> &vecTriggerInfo) {
  CSQLAccess sqlAccess(eSchemaCatalog);
  ...
  // trigger, and its text. Doesn't bring back any disabled or MS-shipped
  // triggers, and gets only DML triggers and not DDL triggers.
  const char *pchStatement =
      "SELECT ST.name AS TriggerName, SOT.name AS TableName, "
      "ST.is_instead_of_trigger, SC.Text, SC.ColID"
  ...
  RETURN_SQL_ERROR_ON_FALSE(
      sqlAccess.BYieldingExecute(FILE_AND_LINE, pchStatement));
  ...
  FOR_EACH_SQL_RESULT(sqlAccess, 0, sqlRecord) {
    ...
    if (nColID == 1) {
      ...
      Q_strncpy(info.m_szTriggerName, pchTriggerName,
                Q_ARRAYSIZE(info.m_szTriggerName));
      Q_strncpy(info.m_szTriggerTableName, pchTableName,
                Q_ARRAYSIZE(info.m_szTriggerTableName));
      ...
      Assert(
          0 ==
          Q_strcmp(vecTriggerInfo[vecTriggerInfo.Count() - 1].m_szTriggerName,
                   pchTriggerName));
    }
  }
...
//-----------------------------------------------------------------------------
// Purpose: retrieves the index information for the specified table, then adds
// this
//			information to the record info. This is a SQL
//Server-specific implementation 			which gets data describing index features not
//available through plain ODBC 			queries.
// Input:	pSQLConnection - SQL connection to execute on
//			pRecordInfo - CRecordInfo to add the index info into
// Output:  SQL return code.
//-----------------------------------------------------------------------------
SQLRETURN
CJobUpdateSchema::YieldingGetColumnIndexes(ESchemaCatalog eSchemaCatalog,
                                           CRecordInfo *pRecordInfo) {
  // query the system management views for all of the indexes on this table
  static const char *pstrStatement =
      "SELECT SI.Name AS INDEX_NAME, SC.Name AS COLUMN_NAME, SI.Type AS "
      "[TYPE], IS_INCLUDED_COLUMN, SIC.KEY_Ordinal AS [ORDINAL_POSITION], "
      "IS_UNIQUE, IS_PRIMARY_KEY, SI.INDEX_ID"
      "  FROM sys.indexes SI "
  ...
  RETURN_SQL_ERROR_ON_FALSE(
      sqlAccess.BYieldingExecute(FILE_AND_LINE, pstrStatement));
  ...
  FOR_EACH_SQL_RESULT(sqlAccess, 0, typeRecord) {
    ...
    RETURN_SQL_ERROR_ON_FALSE(
        pRecordInfo->BFindColumnByName(sColumnName, &nColumnIndexed));
    ...
    if (bIsIncluded) {
      ...
      if (bIsPrimaryKey) {
        ...
        // if we're working a "regular" index, we need to know the uniqueness of
        // the index ...
...
// Input:	pRecordInfo - CRecordInfo to add the index info into (and lookup
// table name out of) Output:  SQL return code.
//-----------------------------------------------------------------------------
SQLRETURN
CJobUpdateSchema::YieldingGetTableFKConstraints(ESchemaCatalog eSchemaCatalog,
                                                CRecordInfo *pRecordInfo) {
  ...
  CSQLAccess sqlAccess(eSchemaCatalog);
  const char *pchStatement =
      "SELECT fk.name AS FKName, current_table.name AS TableName, "
      "parent_table.name AS ParentTableName, "
      "table_column.name AS ColName, parent_table_column.name AS "
      "ParentColName, "
      "fk.delete_referential_action_desc AS OnDelete, "
      "fk.update_referential_action_desc AS OnUpdate "
      "FROM sys.objects AS current_table "
      "JOIN sys.foreign_keys AS fk ON "
      "fk.parent_object_id=current_table.object_id AND fk.is_ms_shipped=0 "
      "JOIN sys.foreign_key_columns AS fk_col ON "
      "fk_col.constraint_object_id=fk.object_id "
      "JOIN sys.objects AS parent_table ON "
      "parent_table.object_id=fk_col.referenced_object_id "
      "JOIN sys.columns AS table_column ON "
      "table_column.object_id=fk_col.parent_object_id AND "
      "table_column.column_id=fk_col.parent_column_id "
      "JOIN sys.columns AS parent_table_column ON "
      "parent_table_column.object_id=fk_col.referenced_object_id AND "
      "parent_table_column.column_id=fk_col.referenced_column_id "
  ...
  FOR_EACH_SQL_RESULT(sqlAccess, 0, sqlRecord) {
    ...
    AssertMsg(Q_stricmp(pchTableName, pRecordInfo->GetName()) == 0,
              "FOREIGN KEY schema conversion found FK for table not matching "
              "search!\n");
    ...
    // Is this more data for the FK we are already tracking?  If so just append
    // the column data, otherwise, assuming some data exists, add the key to the
    // record info
    if (Q_strcmp(fkData.m_rgchName, pchFKName) == 0) {
      ...
      FKColumnRelation_t &colRelation =
          fkData.m_VecColumnRelations[iColRelation];
      Q_strncpy(colRelation.m_rgchCol, pchColName,
                Q_ARRAYSIZE(colRelation.m_rgchCol));
      Q_strncpy(colRelation.m_rgchParentCol, pchParentColName,
                Q_ARRAYSIZE(colRelation.m_rgchParentCol));
    } else {
      ...
      Q_strncpy(fkData.m_rgchParentTableName, pchParentTable,
                Q_ARRAYSIZE(fkData.m_rgchParentTableName));
...
// Output:  SQL return code.
//-----------------------------------------------------------------------------
SQLRETURN CJobUpdateSchema::YieldingCreateTable(ESchemaCatalog eSchemaCatalog,
                                                CRecordInfo *pRecordInfo) {
  CFmtStrMax sCmd;
  SQLRETURN nRet = 0;
  const char *pchName = pRecordInfo->GetName();
  ...
  for (int iColumn = 0; iColumn < pRecordInfo->GetNumColumns(); iColumn++) {
    ...
    char *pchType =
        SQLTypeFromField(columnInfo, rgchType, Q_ARRAYSIZE(rgchType));
    Assert(pchType[0]);
  ...
  // create the table
  // metadata operations aren't transactional, so we'll set bAutoTransaction =
  // true
...
// Output:  SQL return code.
//-----------------------------------------------------------------------------
SQLRETURN CJobUpdateSchema::YieldingAddIndex(ESchemaCatalog eSchemaCatalog,
                                             CRecordInfo *pRecordInfo,
                                             const FieldSet_t &refFields) {
  CSQLAccess sqlAccess(eSchemaCatalog);
...
SQLRETURN CJobUpdateSchema::YieldingProcessUnsafeConversion(
    ESchemaCatalog eSchemaCatalog, const char *pszSQL, const char *pszComment) {
  if (k_EConversionModeConvertIrreversible != m_eConversionMode) {
    if (pszComment) {
    ...
    m_sRecommendedSQL.Append(
        "\nGO\n \n"); // that space is a kludge, because we are using
                      // V_splitlines, which will ignore consecutive seperators
    return SQL_SUCCESS;
  }
  CSQLAccess sqlAccess(eSchemaCatalog);
...
//-----------------------------------------------------------------------------
// Purpose: Add a SQL command to cleanup the schema that will actually destroy
// (supposedly unused) data.
//-----------------------------------------------------------------------------

void CJobUpdateSchema::AddDataDestroyingConversion(const char *pszSQL,
...
//-----------------------------------------------------------------------------
SQLRETURN
CJobUpdateSchema::YieldingAlterTableAddColumn(ESchemaCatalog eSchemaCatalog,
                                              CRecordInfo *pRecordInfo,
                                              const CColumnInfo *pColumnInfo) {
  CSQLAccess sqlAccess(eSchemaCatalog);
  CFmtStrMax sCmd;
  char rgchType[k_cSmallBuff];
  ...
  sCmd.sprintf("ALTER TABLE %s ADD %s %s", pchTableName, pchColumnName,
               rgchType);
  ...
  if (V_strstr(sCmd, "NOT NULL")) {
    EmitError(SPEW_SQL,
              "Cannot add column %s to table %s with NOT NULL constraint\n",
              pchColumnName, pchTableName);
    EmitInfo(SPEW_SQL, SPEW_ALWAYS, LOG_ALWAYS,
             "Here is the SQL that should be run to add the column:\n");
...
//			pRecordInfo - record info describing table
//			pColumnInfo - column info describing column to add
//contraint for 			nColFlagConstraint - constraint to add
// Output:  SQL return code.
//-----------------------------------------------------------------------------
SQLRETURN CJobUpdateSchema::YieldingAddConstraint(
    ESchemaCatalog eSchemaCatalog, CRecordInfo *pRecordInfo,
    const CColumnInfo *pColumnInfo, int nColFlagConstraint) {
  Assert(pRecordInfo);
  Assert(pColumnInfo);

  CFmtStrMax sCmd;
  sCmd.sprintf("ALTER TABLE App%u.%s ADD", GGCBase()->GetAppID(),
               pRecordInfo->GetName());

  Assert(!pColumnInfo->BIsClustered());
  AppendConstraint(pRecordInfo->GetName(), pColumnInfo->GetName(),
                   nColFlagConstraint, true, pColumnInfo->BIsClustered(), sCmd,
                   0);
...
// Input:	hDBC - SQL connection to execute on
//			pRecordInfo - record info describing table
//			pColumnInfoDesired - column info describing new column
//type or length
// Output:  SQL return code.
//-----------------------------------------------------------------------------
SQLRETURN CJobUpdateSchema::YieldingChangeColumnTypeOrLength(
    ESchemaCatalog eSchemaCatalog, CRecordInfo *pRecordInfo,
    const CColumnInfo *pColumnInfoDesired) {
...
// Input:	hDBC - SQL connection to execute on
//			pRecordInfo - record info describing table
//			pColumnInfoActual - column info describing existing
//column properties 			pColumnInfoActual - column info describing desired new
//column properties
// Output:  SQL return code.
//-----------------------------------------------------------------------------
SQLRETURN CJobUpdateSchema::YieldingChangeColumnProperties(
    ESchemaCatalog eSchemaCatalog, CRecordInfo *pRecordInfo,
    const CColumnInfo *pColumnInfoActual,
    const CColumnInfo *pColumnInfoDesired) {
  CSQLAccess sqlAccess(eSchemaCatalog);
  ...
  if (!pColumnInfoActual->BIsPrimaryKey() &&
      pColumnInfoDesired->BIsPrimaryKey()) {
    bAddPrimaryKeyConstraint = true;
  } else if (pColumnInfoActual->BIsPrimaryKey() &&
             !pColumnInfoDesired->BIsPrimaryKey()) {
    bRemovePrimaryKeyConstraint = true;
  }

  if (!pColumnInfoActual->BIsExplicitlyUnique() &&
      pColumnInfoDesired->BIsExplicitlyUnique()) {
    bAddUniqueConstraint = true;
  } else if (pColumnInfoActual->BIsExplicitlyUnique() &&
             !pColumnInfoDesired->BIsExplicitlyUnique()) {
    bRemoveUniqueConstraint = true;
  }

  if (!pColumnInfoActual->BIsExplicitlyIndexed() &&
      pColumnInfoDesired->BIsExplicitlyIndexed()) {
    bAddExplicitIndex = true;
  } else if (pColumnInfoActual->BIsExplicitlyIndexed() &&
             !pColumnInfoDesired->BIsExplicitlyIndexed()) {
    bRemoveExplicitIndex = true;
  }

  // sanity check
  Assert(!(bAddUniqueConstraint &&
           bAddPrimaryKeyConstraint)); // primary key constraint adds implicit
                                       // uniqueness constraint; it's a bug if
                                       // we decide to do both
  Assert(!(bAddUniqueConstraint &&
           bAddExplicitIndex)); // uniqueness constraint adds implicit index;
                                // it's a bug if we decide to do both
  Assert(!(bAddPrimaryKeyConstraint &&
           bAddExplicitIndex)); // primary key constraint adds implicit index;
                                // it's a bug if we decide to do both

  // The index conversion stuff already handles dropping of unexpected indices
  // and primary key constraints.  I'm not even sure that this works or is used
  // anymore.
  if (bAddUniqueConstraint) {
    nRet = YieldingAddConstraint(eSchemaCatalog, pRecordInfo, pColumnInfoActual,
                                 k_nColFlagUnique);
    EXIT_ON_SQL_FAILURE(nRet);
  }
...
//-----------------------------------------------------------------------------
SQLRETURN
CJobUpdateSchema::YieldingCreateTrigger(ESchemaCatalog eSchemaCatalog,
                                        CTriggerInfo &refTriggerInfo) {
  char rgchCmd[k_cchSQLStatementTextMax];
  CSQLAccess sqlAccess(eSchemaCatalog);

  // build the create command
  Q_snprintf(
      rgchCmd, Q_ARRAYSIZE(rgchCmd),
      "CREATE TRIGGER [%s] ON [%s] "
      "%s "
      "AS BEGIN"
      "%s\n"
      "END",
      refTriggerInfo.m_szTriggerName, refTriggerInfo.m_szTriggerTableName,
      refTriggerInfo.GetTriggerTypeString(), refTriggerInfo.m_strText.Get());
...
// Output:  SQL return code.
//-----------------------------------------------------------------------------
SQLRETURN CJobUpdateSchema::YieldingDropTrigger(ESchemaCatalog eSchemaCatalog,
                                                CTriggerInfo &refTriggerInfo) {
  char rgchCmd[k_cchSQLStatementTextMax];
  CSQLAccess sqlAccess(eSchemaCatalog);

  // build the create command
  Q_snprintf(rgchCmd, Q_ARRAYSIZE(rgchCmd), "DROP TRIGGER [%s];",
...
static SQLTypeMapping_t g_rSQLTypeMapping[] = {
    {"tinyint", k_EGCSQLType_int8},   {"smallint", k_EGCSQLType_int16},
    {"int", k_EGCSQLType_int32},      {"bigint", k_EGCSQLType_int64},
    {"real", k_EGCSQLType_float},     {"float", k_EGCSQLType_double},
    {"text", k_EGCSQLType_String},    {"ntext", k_EGCSQLType_String},
    {"char", k_EGCSQLType_String},    {"varchar", k_EGCSQLType_String},
    {"nchar", k_EGCSQLType_String},   {"nvarchar", k_EGCSQLType_String},
    {"sysname", k_EGCSQLType_String}, {"varbinary", k_EGCSQLType_Blob},
...
bool CJobUpdateSchema::YieldingBuildTypeMap(ESchemaCatalog eSchemaCatalog) {
  CSQLAccess sqlAccess(eSchemaCatalog);
  if (!sqlAccess.BYieldingExecute(FILE_AND_LINE,
