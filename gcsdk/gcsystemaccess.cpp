...
static GCConVar gcaccess_enable(
    "gcaccess_enable", "1",
    "Kill switch that disables all gcaccess tracking and systems");
...
void CGCAccessContext::Init(const char *pszName, uint32 nActions,
                            uint32 nSuppressActions) {
  m_sName = pszName;
  m_nActions = nActions;
...
CGCAccess &GGCAccess() { return g_GCAccess; }

CGCAccessSystem::AccessStats_t::AccessStats_t()
    : m_nNumFail(0), m_nNumFailID(0), m_nNumValid(0) {}
...
CGCAccess::CGCAccess()
    : m_nActions(GCAccessAction_TrackSuccess | GCAccessAction_TrackFail |
                 GCAccessAction_ReturnFail),
      m_nSuppressActions(0) {
  m_GlobalContext.Init("Global");
}

void CGCAccess::RegisterSystem(const char *pszName, GCAccessSystem_t nID,
                               uint32 nActions, uint32 nSupressActions) {
  ...
  if (nIndex != m_Systems.InvalidIndex()) {
    ...
    AssertMsg3(false,
               "Multiple systems conflciting in Register System: ID %d, "
               "original: %s, new %s",
               nID, m_Systems[nIndex]->m_sName.String(), pszName);
    return;
  }
...
bool CGCAccess::ValidateSteamIDAccess(GCAccessSystem_t nSystem,
                                      CSteamID steamID) {
  // global kill switch
  if (!gcaccess_enable.GetBool())
    return true;
  ...
  CSteamID expectedID =
      (g_pJobCur) ? g_pJobCur->SOVALIDATE_GetSteamID() : steamID;
  return InternalValidateAccess(nSystem, steamID, expectedID);
}

bool CGCAccess::InternalValidateAccess(GCAccessSystem_t nSystem,
                                       CSteamID steamID, CSteamID expectedID) {
  // see if tracking for this system is disabled. This list is almost always
  // empty, so just use a linear search for now.
  ...
  if (nSystemIndex == m_Systems.InvalidIndex()) {
    ...
    // AssertMsg varargs
    AssertMsg1(false,
               "Error: Tracking a system that has not been registered. Make "
               "sure to register system %u",
               nSystem);
  } else {
  ...
  FOR_EACH_VEC(m_SingleAsserts, nCurrAssert) {
    ...
    if (nSystem == pAssert->m_System) {
      if (V_stricmp((pAssert->m_bContext) ? pContext->GetName() : pszJobName,
                    pAssert->m_sContextOrJob) == 0) {
        // log this assert
        {
          // !FIXME! DOTAMERGE
          ...
          // AssertMsg varargs
          AssertMsg3(false,
                     "GCSystemAccess Caught %s (context %s) calling into %s",
                     pszJobName, pContext->GetName(),
                     pSystem ? pSystem->m_sName.String() : "unknown");
        }
  ...
  if (nActions & (GCAccessAction_Msg | GCAccessAction_Assert)) {
    int nSystemIndex = m_Systems.Find(nSystem);
    const char *pszSystem = (nSystemIndex != m_Systems.InvalidIndex())
                                ? m_Systems[nSystemIndex]->m_sName.String()
                                : "<unregistered>";
    ...
    if (!bValidSystem) {
      szMsg.sprintf(
          "Job %s Accessed invalid system %s (%u) while in context %s\n",
          pszJobName, pszSystem, nSystem, pContext->GetName());
    } else {
      szMsg.sprintf("Job %s Accessed invalid steam ID %s but expected %s in "
                    "system %s (%u) while in context %s\n",
                    pszJobName, steamID.RenderLink(), expectedID.RenderLink(),
                    pszSystem, nSystem, pContext->GetName());
    }
...
void CGCAccess::SuppressAccess(GCAccessSystem_t nSystem, bool bEnable) {
  // see if it is an existing item already suppressed that we need to modify the
  // ref count of
  FOR_EACH_VEC(m_SuppressAccess, nAccess) {
    if (m_SuppressAccess[nAccess].m_nSystem == nSystem) {
      if (bEnable) {
        // to enable, we want to decrease the disable count, or remove if we are
        // fully re-enabled
...
void CGCAccess::ClearSystemStats() {
  FOR_EACH_MAP_FAST(m_Systems, nSystem) { m_Systems[nSystem]->m_Jobs.Purge(); }
}

bool CGCAccess::CatchSingleAssert(const char *pszSystem, bool bContext,
                                  const char *pszContextOrJob) {
  // find the system we are referencing so we don't add invalid breakpoints if
  // we can avoid it
...
static bool ShouldDisplayStats(CGCAccess::EDisplay eDisplay,
                               const CGCAccessSystem::AccessStats_t &stats) {
...
void CGCAccess::ReportJobs(const char *pszContext, EDisplay eDisplay) const {
  ...
  FOR_EACH_MAP_FAST(jobs, nJob) {
    rp.StrValue((const char *)jobs.Key(nJob),
                CFmtStr("gcaccess_dump_job \"%s\" %d",
                        (const char *)jobs.Key(nJob), eDisplay)
                    .String());
    rp.StrValue(jobs[nJob].m_sContext,
                CFmtStr("gcaccess_dump_context \"%s\" %d",
                        jobs[nJob].m_sContext.String(), eDisplay)
                    .String());
...
void CGCAccess::ReportSystems(const char *pszContext, EDisplay eDisplay) const {
  ...
  FOR_EACH_MAP_FAST(m_Systems, nSystem) {
    ...
    rp.StrValue(pSystem->m_sName, CFmtStr("gcaccess_dump_system \"%s\" %d",
                                          pSystem->m_sName.String(), eDisplay)
                                      .String());
...
void CGCAccess::FullReport(const char *pszSystemFilter,
                           const char *pszContextFilter,
                           const char *pszJobFilter, EDisplay eDisplay) const {
  CGCReportPrinter rp;
  rp.AddStringColumn("Job");
  rp.AddStringColumn("Context");
  rp.AddStringColumn("System");
  ...
  FOR_EACH_MAP_FAST(m_Systems, nSystem) {
    ...
    FOR_EACH_MAP_FAST(pSystem->m_Jobs, nJob) {
      ...
      rp.StrValue(
          pszJob,
          CFmtStr("gcaccess_dump_job \"%s\" %d", pszJob, eDisplay).String());
      rp.StrValue(pszContext, CFmtStr("gcaccess_dump_context \"%s\" %d",
                                      pszContext, eDisplay)
                                  .String());
      rp.StrValue(pSystem->m_sName, CFmtStr("gcaccess_dump_system \"%s\" %d",
                                            pSystem->m_sName.String(), eDisplay)
...
void CGCAccess::DependencyReport(const char *pszSystem,
                                 EDisplay eDisplay) const {
...
GC_CON_COMMAND_PARAMS(
    gcaccess_dump_job, 1,
    "<Job Name> Dumps all of the accesses associated with the specified job") {
  CGCAccess::EDisplay eDisplay = (args.ArgC() >= 3)
                                     ? (CGCAccess::EDisplay)V_atoi(args[2])
                                     : CGCAccess::eDisplay_Referenced;
  GGCAccess().FullReport(NULL, NULL, args[1], eDisplay);
}

GC_CON_COMMAND_PARAMS(gcaccess_dump_context, 1,
                      "<Job Name> Dumps all of the accesses associated with "
...
GC_CON_COMMAND_PARAMS(gcaccess_dump_system, 1,
                      "<Job Name> Dumps all of the accesses associated with "
...
GC_CON_COMMAND_PARAMS(gcaccess_dump_context_jobs, 1,
                      "<Context Name> Dumps all of the accesses associated "
...
GC_CON_COMMAND_PARAMS(gcaccess_dump_context_systems, 1,
                      "<Context Name> Dumps all of the accesses associated "
                      "with the specified context") {
  CGCAccess::EDisplay eDisplay = (args.ArgC() >= 3)
                                     ? (CGCAccess::EDisplay)V_atoi(args[2])
...
GC_CON_COMMAND_PARAMS(
    gcaccess_dump_system_dependencies, 1,
    "<System Name> This will dump out for all jobs that reference the "
    "specified system, what other systems they reference") {
  CGCAccess::EDisplay eDisplay = (args.ArgC() >= 3)
                                     ? (CGCAccess::EDisplay)V_atoi(args[2])
                                     : CGCAccess::eDisplay_Referenced;
...
GC_CON_COMMAND(gcaccess_dump_jobs,
               "Dumps all the jobs that have been tracked with the gcaccess "
               "system as well as count of violations") {
  CGCAccess::EDisplay eDisplay = (args.ArgC() >= 2)
                                     ? (CGCAccess::EDisplay)V_atoi(args[1])
                                     : CGCAccess::eDisplay_Referenced;
  GGCAccess().ReportJobs(NULL, eDisplay);
}

GC_CON_COMMAND(gcaccess_dump_systems,
               "Dumps all the systems that have been tracked with the gcaccess "
               "system as well as count of violations") {
  CGCAccess::EDisplay eDisplay = (args.ArgC() >= 2)
                                     ? (CGCAccess::EDisplay)V_atoi(args[1])
                                     : CGCAccess::eDisplay_Referenced;
...
GC_CON_COMMAND(gcaccess_dump_full,
               "Dumps all the information tracked by the gcaccess") {
  CGCAccess::EDisplay eDisplay = (args.ArgC() >= 2)
                                     ? (CGCAccess::EDisplay)V_atoi(args[1])
                                     : CGCAccess::eDisplay_Referenced;
...
GC_CON_COMMAND_PARAMS(gcaccess_catch_context, 2,
                      "<system> <context> - Catches and generates an assert "
                      "when the specified context calls into the provided "
                      "system. This will only generate a single assert") {
  if (!GGCAccess().CatchSingleAssert(args[1], true, args[2]))
    EG_MSG(SPEW_CONSOLE,
           "Unable to register catch, likely \'%s\' is not a valid system.",
           args[1]);
}

GC_CON_COMMAND_PARAMS(
    gcaccess_catch_job, 2,
    "<system> <job> - Catches and generates an assert when the specified job "
    "calls into the provided system. This will only generate a single assert") {
...
GC_CON_COMMAND(gcaccess_catch_clear,
               "Clears all previously specified catches") {
