...
// Purpose: Constructor
//------------------------------------------------------------------------------
CGCClient::CGCClient(ISteamGameCoordinator *pSteamGameCoordinator,
                     bool bGameserver)
    : m_pSteamGameCoordinator(NULL), m_memMsg(0, 1024),
#ifndef STEAM
...
CGCClient::~CGCClient() {
  ...
  FOR_EACH_MAP_FAST(m_mapSOCache, i) { delete m_mapSOCache[i]; }
...
//			time ran out.
//------------------------------------------------------------------------------
bool CGCClient::BMainLoop(uint64 ulLimitMicroseconds,
                          uint64 ulFrameTimeMicroseconds) {
  ...
  CJobTime::UpdateJobTime(ulFrameTimeMicroseconds ? ulFrameTimeMicroseconds
...
//			cubData - The number of bytes of data in this message
//including any 				variable-lengthed data.
// Outputs: Returns false if the send failed. A return value of true doesn't
//			mean that the message was necessarily received by the GC
//just that 			it didn't fail in obvious ways on the client.
//------------------------------------------------------------------------------
bool CGCClient::BSendMessage(uint32 unMsgType, const uint8 *pubData,
                             uint32 cubData) {
  if (m_pSteamGameCoordinator)
    return m_pSteamGameCoordinator->SendMessage(unMsgType, pubData, cubData) ==
           k_EGCResultOK;
  else
    return false;
}
...
bool CGCClient::BSendMessage(const CGCMsgBase &msg) {
  return BSendMessage(msg.Hdr().m_eMsg, msg.PubPkt() + sizeof(GCMsgHdr_t),
...
//-----------------------------------------------------------------------------
class CProtoBufGCClientSendHandler
    : public CProtoBufMsgBase::IProtoBufSendHandler {
public:
  CProtoBufGCClientSendHandler(CGCClient *pGCClient) : m_pClient(pGCClient) {}
  virtual bool BAsyncSend(MsgType_t eMsg, const uint8 *pubMsgBytes,
    ...
    {
      VPROF_BUDGET("CGCClient - BSendGCMsgToClient (ProtoBuf)",
                   VPROF_BUDGETGROUP_STEAM);
      return m_pClient->BSendMessage(eMsg | k_EMsgProtoBufFlag, pubMsgBytes,
                                     cubSize);
    }
  }
...
void CGCClient::OnGCMessageAvailable(GCMessageAvailable_t *pCallback) {
  uint32 cubData;
  uint32 unMsgType;
  while (m_pSteamGameCoordinator &&
         m_pSteamGameCoordinator->IsMessageAvailable(&cubData)) {
    // Get the size of the full message. sizeof( GCMsgHdr_t ) was not sent in
    // the binary data
    ...
    EGCResults eResult = m_pSteamGameCoordinator->RetrieveMessage(
        &unMsgType, pPacketFromGC, m_memMsg.Count() - sizeof(GCMsgHdr_t),
        &cubData);
      ...
      if (unMsgType & k_EMsgProtoBufFlag) {
        ...
        if (pMsgNetPacket.Get() != NULL) {
          ...
          GetJobMgr().BRouteMsgToJob(
              this, pMsgNetPacket.Get(),
              JobMsgInfo_t(
                  pMsgNetPacket->GetEMsg(), pMsgNetPacket->GetSourceJobID(),
                  pMsgNetPacket->GetTargetJobID(), k_EServerTypeGCClient));
...
//------------------------------------------------------------------------------
CGCClientSharedObjectCache *CGCClient::FindSOCache(const CSteamID &steamID,
                                                   bool bCreateIfMissing) {
  CUtlMap<CSteamID, CGCClientSharedObjectCache *>::IndexType_t nCache =
  ...
  if (m_mapSOCache.IsValidIndex(nCache))
    ...
    if (bCreateIfMissing) {
      ...
      CGCClientSharedObjectCache *pCache =
          new CGCClientSharedObjectCache(steamID);
...
//------------------------------------------------------------------------------
void CGCClient::AddSOCacheListener(const CSteamID &ownerID,
                                   ISharedObjectListener *pListener) {
...
//------------------------------------------------------------------------------
bool CGCClient::RemoveSOCacheListener(const CSteamID &ownerID,
                                      ISharedObjectListener *pListener) {
...
//------------------------------------------------------------------------------
void CGCClient::Dump() {
  FOR_EACH_MAP(m_mapSOCache, idx) { m_mapSOCache[idx]->Dump(); }
}

//------------------------------------------------------------------------------
// Purpose: Finds the shared object for this steam ID and key object
//------------------------------------------------------------------------------
CSharedObject *CGCClient::FindSharedObject(const CSteamID &ownerID,
...
public:
  ...
  virtual bool BYieldingRunGCJob(GCSDK::IMsgNetPacket *pNetPacket) {
    ...
    CGCClientSharedObjectCache *pSOCache =
        m_pGCClient->FindSOCache(msg.Body().owner());
    if (pSOCache) {
      pSOCache->BCreateFromMsg(msg.Body().type_id(),
                               msg.Body().object_data().data(),
                               msg.Body().object_data().size());
...
public:
  ...
  virtual bool BYieldingRunGCJob(GCSDK::IMsgNetPacket *pNetPacket) {
    ...
    SOCDebug("CGCSODestroyJob(owner=%s, type=%d)\n",
             CSteamID(msg.Body().owner()).Render(), msg.Body().type_id());
    CGCClientSharedObjectCache *pCache =
        m_pGCClient->FindSOCache(msg.Body().owner(), false);
    if (pCache) {
      pCache->BDestroyFromMsg(msg.Body().type_id(),
                              msg.Body().object_data().data(),
                              msg.Body().object_data().size());
      Assert(msg.Body().has_version());
...
public:
  ...
  virtual bool BYieldingRunGCJob(GCSDK::IMsgNetPacket *pNetPacket) {
    ...
    CGCClientSharedObjectCache *pSOCache =
        m_pGCClient->FindSOCache(msg.Body().owner());
    if (pSOCache) {
      pSOCache->BUpdateFromMsg(msg.Body().type_id(),
                               msg.Body().object_data().data(),
                               msg.Body().object_data().size());
      Assert(msg.Body().has_version());
      pSOCache->SetVersion(msg.Body().version());
  ...
  virtual bool BYieldingRunGCJob(GCSDK::IMsgNetPacket *pNetPacket) {
    ...
    if (pSOCache) {
      ...
      for (int i = 0; i < msg.Body().objects_size(); ++i) {
        const CMsgSOMultipleObjects_SingleObject &objMessage =
            msg.Body().objects(i);
        SOCDebug("     type %d\n", objMessage.type_id());
        pSOCache->BUpdateFromMsg(objMessage.type_id(),
                                 objMessage.object_data().data(),
                                 objMessage.object_data().size());
      }
...
GC_REG_JOB(CGCClient, CGCSOUpdateMultipleJob, "CGCSOUpdateMultipleJob",
           k_ESOMsg_UpdateMultiple, GCSDK::k_EServerTypeGCClient);
...
public:
  ...
  virtual bool BYieldingRunGCJob(GCSDK::IMsgNetPacket *pNetPacket) {
    ...
    CGCClientSharedObjectCache *pSOCache =
        m_pGCClient->FindSOCache(msg.Body().owner(), true);
...
GC_REG_JOB(CGCClient, CGCSOCacheSubscribedJob, "CGCSOCacheSubscribedJob",
           k_ESOMsg_CacheSubscribed, GCSDK::k_EServerTypeGCClient);
...
GC_REG_JOB(CGCClient, CGCSOCacheUnsubscribedJob, "CGCSOCacheUnsubscribedJob",
           k_ESOMsg_CacheUnsubscribed, GCSDK::k_EServerTypeGCClient);
...
GC_REG_JOB(CGCClient, CGCSOCacheSubscriptionCheck,
           "CGCSOCacheSubscriptionCheck", k_ESOMsg_CacheSubscriptionCheck,
           GCSDK::k_EServerTypeGCClient);
