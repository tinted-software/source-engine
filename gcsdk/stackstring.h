...
template <size_t cubSize> class CStackString {
public:
  class CStackString() {
    Reset();
  } class CStackString(const char *pchRHS) {
    Set(pchRHS);
  }
  ...
  CStackString &operator=(const char *pchValue) {
    Set(pchValue);
    return *this;
  }
  CStackString &operator+=(const char *pchValue) {
...
template <size_t cubSize>
void CStackString<cubSize>::AppendFormatV(const char *pchFormat,
                                          va_list marker) {
  size_t cubPrinted =
      Q_vsnprintf(m_pchEnd, GetCubSize() - Length(), pchFormat, marker);
  m_pchEnd += cubPrinted;
}
...
template <size_t cubSize>
void CStackString<cubSize>::AppendFormat(
    PRINTF_FORMAT_STRING const char *pchFormat, ...) {
  va_list marker;
  va_start(marker, pchFormat);
