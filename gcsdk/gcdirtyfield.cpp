...
CSharedObjectDirtyFieldList::CSharedObjectDirtyFieldList(CSharedObject *obj)
    : m_obj(obj), m_firstFieldBits(0), m_pExtendedFields(NULL) {}
...
CSharedObject *CSharedObjectDirtyFieldList::Obj() const { return m_obj; }

void CSharedObjectDirtyFieldList::DirtyField(int index) {
  // if the field index fits within our dirty fields struct, set a bit to track
  // it
  if (index < 32) {
    m_firstFieldBits |= (1 << index);
  }
  // if the field index is too big, store it in our backup structure; this is
  // less efficient but more flexible, especially when dealing with fields that
  // themselves contain flags so would be interpreted as huge numbers
  else {
    if (!m_pExtendedFields)
...
void CSharedObjectDirtyFieldList::GetDirtyFieldSet(
    CUtlVector<int> &fieldSet) const {
  fieldSet.Purge();
...
void CSharedObjectDirtyList::DirtyObjectField(CSharedObject *obj, int field) {
  ...
  if (index == InvalidIndex()) {
    index = m_sharedObjectDirtyFieldList.AddToTail(
        CSharedObjectDirtyFieldList(obj));
  }
...
bool CSharedObjectDirtyList::GetDirtyFieldSetByIndex(
    int index, CSharedObject **ppObj, CUtlVector<int> &fieldSet) const {
  VPROF_BUDGET("CSharedObjectDirtyList::GetDirtyFieldSetByIndex",
  ...
  const CSharedObjectDirtyFieldList &fieldList =
      m_sharedObjectDirtyFieldList[index];
  if (ppObj) {
    *ppObj = fieldList.Obj();
  }
...
bool CSharedObjectDirtyList::GetDirtyFieldSetByObj(CSharedObject *pObj,
...
void CSharedObjectDirtyList::Validate(CValidator &validator,
                                      const char *pchName) {
  VALIDATE_SCOPE();
