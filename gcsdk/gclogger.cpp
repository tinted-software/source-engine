...
//-----------------------------------------------------------------------------
void EmitBaseMessageV(const char *pchGroupName, SpewType_t spewType,
  ...
  //		if ( pHandler )
  //		{
  //			bool bOutputSpew = pHandler->OnEmitSpew( pchGroupName,
  //spewType, iSpewLevel, iLevelLog, pchBuf ); 			if ( !bOutputSpew )
  //			{
  //				return;
  //			}
  //		}
...

  {
    ...
    // !FIXME! DOTAMERGE
    // GGCInterface()->EmitSpew( pchGroupName, spewType, iSpewLevel, iLevelLog,
    // pchBuf );
    GGCHost()->EmitMessage(pchGroupName, spewType, iSpewLevel, iLevelLog,
                           pchBuf);
  }
#else
...
// similar to the above, but takes in a group, and handles filtering messages
// out that are disabled, and extracting other info from the group
void EmitBaseMessageV(const CGCEmitGroup &Group, SpewType_t spewType,
                      int iConsoleLevel, int iLogLevel, const char *pchMsg,
                      va_list vaArgs) {
  int iClampConsoleLevel =
      (iConsoleLevel <= MIN(Group.GetConsoleLevel(), g_nMaxSpewLevel))
          ? SPEW_ALWAYS
          : SPEW_NEVER;
  int iClampLogLevel = (iLogLevel <= MIN(Group.GetLogLevel(), g_nMaxLogLevel))
                           ? LOG_ALWAYS
                           : LOG_NEVER;
  if ((iClampConsoleLevel != SPEW_NEVER) || (iClampLogLevel != LOG_NEVER))
    EmitBaseMessageV(Group.GetName(), spewType, iClampConsoleLevel,
                     iClampLogLevel, pchMsg, vaArgs);
}
...
void CGCEmitGroup::Internal_Emit(EMsgLevel eLvl,
...
void CGCEmitGroup::EmitV(EMsgLevel eLvl,
                         PRINTF_FORMAT_STRING const char *pchMsg,
                         va_list vaArgs) const {
  switch (eLvl) {
  case kMsg_Error:
    ErrorV(pchMsg, vaArgs);
    break;
  case kMsg_Warning:
    WarningV(pchMsg, vaArgs);
    break;
  case kMsg_Msg:
    MsgV(pchMsg, vaArgs);
    break;
  case kMsg_Verbose:
    VerboseV(pchMsg, vaArgs);
    break;
  default:
    AssertMsg1(false,
               "Unexpected error level of %d provided to GCEmitGroup::Emit",
               eLvl);
    break;
  }
}
...
void EmitInfoV(const CGCEmitGroup &Group, int iLevel, int iLevelLog,
               const char *pchMsg, va_list vaArgs) {
