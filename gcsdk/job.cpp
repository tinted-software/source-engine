...
static const char *const k_prgchJobPauseReason[] = {
    "active",           "not started", "netmsg", "sleep for time",
    "waiting for lock", "yielding",    "SQL",    "work item",
};
...
CJob::CJob(CJobMgr &jobMgr, const char *pchJobName)
    : m_JobMgr(jobMgr), m_pchJobName(pchJobName) {
...
CJob::~CJob() {
  ...
  FOR_EACH_VEC(m_vecNetPackets, i) { m_vecNetPackets[i]->Release(); }
  m_vecNetPackets.RemoveAll();

  AssertMsg2(0 == GetDoNotYieldDepth(),
             "Job ending with %d open Do Not Yields. Are we missing a "
             "END_DO_NOT_YIELD()? Innermost delared at %s",
             GetDoNotYieldDepth(),
             m_stackDoNotYieldGuards[m_stackDoNotYieldGuards.Head()]);
}
...
void CJob::WaitForThreadFuncWorkItemBlocking() {
  if (m_pWaitingOnWorkItem) {
    ...
    default:
      AssertMsg1(false,
                 "CJob::WaitForThreadFuncWorkItemBlocking job %s has "
...
const char *CJob::GetPauseReasonDescription() const {
  ...
  if (GetPauseReason() < Q_ARRAYSIZE(k_prgchJobPauseReason)) {
    ...
    case k_EJobPauseReasonWaitingForLock: {
      Q_snprintf(srgchPauseReason, k_cSmallBuff, "WOL: 0x%x (%s)",
                 (unsigned int)m_pWaitingOnLock,
    ...
    case k_EJobPauseReasonNetworkMsg: {
      ...
      if (g_theMessageList.GetMessage(m_unWaitMsgType, &pchMsgType, 0xFF)) {
        ...
        Q_snprintf(srgchPauseReason, k_cSmallBuff, "NetMsg: Unknown %d",
                   m_unWaitMsgType);
        return srgchPauseReason;
      }
    }
...
//			cubPkt -			The size of the message
//packet
//-----------------------------------------------------------------------------
void CJob::StartJobFromNetworkMsg(IMsgNetPacket *pNetPacket,
                                  const JobID_t &gidJobIDSrc) {
...
void CJob::StartJob(void *pvStartParam) {
  ...
  AssertMsg1(m_ePauseReason == k_EJobPauseReasonNotStarted,
...
//-----------------------------------------------------------------------------
// Purpose: setup the debug memory and job name before running a job the first
// time
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: marks a net packet as being finished with, releases the packet and
// frees the memory
//-----------------------------------------------------------------------------
void CJob::ReleaseNetPacket(IMsgNetPacket *pNetPacket) {
  ...
  if (iVec != m_vecNetPackets.InvalidIndex()) {
    ...
    AssertMsg(false,
              "Job failed trying to release a IMsgNetPacket it doesn't own");
  }
}
...
void CJob::Continue() {
  ...
  // Record frame we're starting in and start a timer to track how long we work
  // for within the frame Also, add in how much time has passed since we were
  // last paused to track heartbeat requirements
  ...
  // WARNING: MEMBER VARIABLES ARE NOW UNSAFE TO ACCESS - this CJob may be
  // deleted
...
void CJob::Pause(EJobPauseReason eReason) {
  ...
  AssertMsg1(0 == m_stackDoNotYieldGuards.Count(),
             "Yielding while in a BEGIN_DO_NOT_YIELD() block declared at %s",
             m_stackDoNotYieldGuards[m_stackDoNotYieldGuards.Head()]);
  ...
  if (m_FastTimerDelta.GetDuration().GetMicroseconds() >
      k_cMicroSecTaskGranularity * 10) {
...
bool CJob::BYieldingWaitForMsg(CGCMsgBase *pMsg, MsgType_t eMsg) {
  ...
  if (BHasFailedToReceivedMsgType(eMsg)) {
    AssertMsg2(false,
               "Job %s cannot wait for msg %u, it has already failed to wait "
               "for that msg type.",
               GetName(), eMsg);
    return false;
  }
  ...
  if (!BYieldingWaitForMsg(&pNetPacket)) {
    // Remember this event, so we can at least detect if a reply comes late, we
    // don't get confused.
...
bool CJob::BYieldingWaitForMsg(CProtoBufMsgBase *pMsg, MsgType_t eMsg) {
  ...
  if (pMsg->GetEMsg() != eMsg) {
    ...
    EmitError(SPEW_GC,
              "CJob::BYieldingWaitForMsg expected msg %u but received %u", eMsg,
              pMsg->GetEMsg());
    return false;
  }
...
bool CJob::BYieldingWaitForMsg(CGCMsgBase *pMsg, MsgType_t eMsg,
                               const CSteamID &expectedID) {
  if (!BYieldingWaitForMsg(pMsg, eMsg))
    return false;

  if (pMsg->Hdr().m_ulSteamID != expectedID.ConvertToUint64()) {
    EmitError(SPEW_GC,
              "CJob::BYieldingWaitForMsg expected reply from steam ID %s, but "
              "instead got a response from %s for message %d\n",
              expectedID.Render(), CSteamID(pMsg->Hdr().m_ulSteamID).Render(),
              eMsg);
    return false;
  }
...
// Purpose: pauses the job until a network msg for the job arrives
//-----------------------------------------------------------------------------
bool CJob::BYieldingRunQuery(CGCSQLQueryGroup *pQueryGroup,
...
//-----------------------------------------------------------------------------
// Purpose: CWorkItem for processing functions in CJob-derived classes on
// another thread
//-----------------------------------------------------------------------------
class CJobThreadFuncWorkItem : public CWorkItem {
public:
  DECLARE_WORK_ITEM(CJobThreadFuncWorkItem);
  CJobThreadFuncWorkItem(CJob *pJob, JobThreadFunc_t jobThreadFunc,
                         CFunctor *pFunctor)
      : CWorkItem(pJob->GetJobID()), m_pJob(pJob),
        m_pJobThreadFunc(jobThreadFunc), m_pFunctor(pFunctor) {}
...
bool CJob::BYieldingWaitForThreadFunc(CFunctor *jobFunctor) {
  ...
  CJobThreadFuncWorkItem *pJobThreadFuncWorkItem =
      new CJobThreadFuncWorkItem(this, NULL, jobFunctor);
...
//			need to repeatedly sleep while waiting for something to
//happen. 			This function uses STime (server "pseudo" time) to determine 			timeout
//conditions.
//
// Input:	cMicrosecondsToSleep - duration to sleep this call
//			stimeStarted - the time to calculate timeout from.
//(Typically, 				the time you start calling this in a loop, passing the same 				start
//time each time you call this method.) 			nMicroSecLimit - duration from
//stimeStarted to consider timed out
// Output : Returns true if not timed out yet, false if timed out
//-----------------------------------------------------------------------------
bool CJob::BYieldingWaitTimeWithLimit(uint32 cMicrosecondsToSleep,
                                      CJobTime &stimeStarted,
                                      int64 nMicroSecLimit) {
...
// Purpose: waits for specified time and checks for timeout.  Useful when you
//			need to repeatedly sleep while waiting for something to
//happen. 			This function uses RTime (wall-clock "real" time) to determine 			timeout
//conditions.
//
// Input:	cMicrosecondsToSleep - duration to sleep this call
//			nSecLimit - duration from stimeStarted to consider timed
//out
// Output : Returns true if not timed out yet, false if timed out
//-----------------------------------------------------------------------------
bool CJob::BYieldingWaitTimeWithLimitRealTime(uint32 cMicrosecondsToSleep,
                                              int nSecondsLimit) {
  return BYieldingWaitTime(cMicrosecondsToSleep);
}
...
//-----------------------------------------------------------------------------
// Purpose: Pauses the job ONLY IF JobMgr decides it needs to based on time run
// and priority
//			If pausing, pauses until the next time the JobMgr Run()
//is called
...
bool CJob::_BYieldingAcquireLock(CLock *pLock, const char *filename, int line) {
  ...
  // Skip the path info from the filename.  It just maks the debug messages
  // excessively long.
  ...
  FOR_EACH_VEC(m_vecLocks, i) {
    if (m_vecLocks[i]->GetLockType() == pLock->GetLockType()) {
      if (m_vecLocks[i]->GetLockSubType() <= pLock->GetLockSubType()) {
        AssertMsg7(false,
                   "Job %s Locking %s at %s:(%d) with yielding; holds lock %s "
                   "from %s(%d)\n",
                   GetName(), pLock->GetName(), filename, line,
                   m_vecLocks[i]->GetName(), m_vecLocks[i]->m_pFilename,
                   m_vecLocks[i]->m_line);
        return false;
      }
  ...
  if (m_pWaitingOnLock != NULL) {
    AssertMsg7(
        false,
        "Job (%s) locking %s at %s(%d); already waiting on %s at %s(%d).\n",
        GetName(), pLock->GetName(), filename, line,
        m_pWaitingOnLock->GetName(), m_pWaitingOnLock->m_pFilename,
        m_pWaitingOnLock->m_line);
    return false;
  }
  ...
  if (pLock->BIsLocked()) {
    ...
    if (index != m_vecLocks.InvalidIndex() && this == pLock->GetJobLocking()) {
      ...
      EmitWarning(SPEW_JOB, LOG_ALWAYS,
                  "Failed to get lock %s at %s(%d) after waiting in %s\n",
                  pLock->GetName(), filename, line, GetName());
      if (m_vecLocks.Count() == 0) {
        EmitWarning(SPEW_JOB, LOG_ALWAYS,
                    "m_vecLocks.Count(): %d, this: 0x%p, "
                    "pLock->GetJobLocking(): %s (0x%p)\n",
                    m_vecLocks.Count(), this,
                    pLock->GetJobLocking() ? pLock->GetJobLocking()->GetName()
                                           : "(null)",
                    pLock->GetJobLocking());
      } else {
        EmitWarning(SPEW_JOB, LOG_ALWAYS,
                    "m_vecLocks.Count(): %d this: 0x%p, pLock: 0x%p "
                    "pLock->GetJobLocking(): %s (0x%p)\n",
                    m_vecLocks.Count(), this, pLock,
                    pLock->GetJobLocking() ? pLock->GetJobLocking()->GetName()
                                           : "(null)",
                    pLock->GetJobLocking());
        FOR_EACH_VEC(m_vecLocks, i) {
          EmitWarning(
              SPEW_JOB, LOG_ALWAYS, "m_vecLocks[%d]: %s (0x%p) %s(%d)\n", i,
              m_vecLocks[i] ? m_vecLocks[i]->GetName() : "(null)",
              m_vecLocks[i], m_vecLocks[i]->m_pFilename, m_vecLocks[i]->m_line);
        }
      }
      return false;
    }
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CJob::_BAcquireLockImmediate(CLock *pLock, const char *filename,
                                  int line) {
  AssertRunningThisJob();

  AssertMsg5(m_pWaitingOnLock == NULL,
             "Job (%s) at %s(%d) trying to take a lock while it was already "
             "waiting for the first one at %s(%d)",
             GetName(), filename, line, m_pWaitingOnLockFilename,
             m_waitingOnLockLine);
...
//-----------------------------------------------------------------------------
// Purpose: Releases the specified lock, passing it on to the next job if
// necessary
//-----------------------------------------------------------------------------
void CJob::_ReleaseLock(CLock *pLock, bool bForce, const char *filename,
                        int line) {
  Assert(pLock);
  if (!pLock)
    return;
  ...
  if (pLock->GetJobLocking() != this) {
    EmitError(SPEW_JOB,
              "Job %s trying to release lock %s at %s(%d) though the lock is "
              "held by %s\n",
              GetName(), pLock->GetName(), filename, line,
              pLock->GetJobLocking()->GetName());
    return;
  }
  ...
  if (pLock->m_pJobToNotifyOnLockRelease) {
    ...
    // just release
    UnsetLock(pLock);
    Assert(NULL == pLock->m_pJobWaitingQueueTail ||
...
//-----------------------------------------------------------------------------
// Purpose: Release all locks this job holds. This is only to be used by long
// lived jobs that don't destruct.
//-----------------------------------------------------------------------------
void CJob::ReleaseLocks() {
  // release any locks - do this in reverse order because they're being removed
  // from the vector in the loop
...
//-----------------------------------------------------------------------------
// Purpose: Assert that we don't hold any locks, and if we hold them, release
// them
//-----------------------------------------------------------------------------
void CJob::ShouldNotHoldAnyLocks() {
  ...
  FOR_EACH_VEC_BACK(m_vecLocks, nLock) {
    ...
    sErrMsg.Append(CFmtStr("    Lock %s, acquired %s(%d)\n", pLock->GetName(),
                           pLock->m_pFilename, pLock->m_line)
                       .Access());
  }
...
void CJob::AddJobToNotifyOnLockRelease(CJob *pJob) {
  // if we already are going to be notifying someone, then have them notify the
  // new requester
  if (m_pJobToNotifyOnLockRelease) {
    AssertMsg(false, "AddJobToNotifyOnLockRelease attempting to walk the "
                     "linked list. We've optimized this out.");
...
void CJob::UnsetLock(CLock *pLock) {
  ...
  if (/*!BIsTest() &&*/ pLock->m_sTimeAcquired.CServerMicroSecsPassed() >=
    ...
    if (pLock->m_pJobToNotifyOnLockRelease) {
      pLock->m_pJobToNotifyOnLockRelease->m_flags.m_bits.m_bLocksLongWait =
          true;
      EmitWarning(SPEW_JOB, 4,
                  "Job of type %s held lock for %.2f seconds while job of type "
                  "%s was waiting\n",
                  GetName(),
                  (double)pLock->m_sTimeAcquired.CServerMicroSecsPassed() /
                      k_nMillion,
                  pLock->m_pJobToNotifyOnLockRelease->GetName());
    } else
      EmitWarning(
          SPEW_JOB, 4, "Job of type %s held lock for %.2f seconds\n", GetName(),
          (double)pLock->m_sTimeAcquired.CServerMicroSecsPassed() / k_nMillion);
  }
  m_vecLocks.FindAndRemove(pLock);
}

//-----------------------------------------------------------------------------
// Purpose: Releases the lock from the old job, and immediately passes it on to
// the waiting job
//-----------------------------------------------------------------------------
void CJob::PassLockToJob(CJob *pNewJob, CLock *pLock) {
  ...
  if (pNewJob->GetPauseReason() == k_EJobPauseReasonWaitingForLock &&
      pNewJob->m_pWaitingOnLock == pLock) {
    // give the job the lock
    pNewJob->_SetLock(pLock, pNewJob->m_pWaitingOnLockFilename,
                      m_waitingOnLockLine);
    ...
    EmitError(SPEW_JOB,
              "Job passed lock it wasn't waiting for. Job: %s, Lock: %s "
              "%s(%d), Paused for %s, Waiting on %s\n",
              pNewJob->GetName(), pLock->GetName(), pLock->m_pFilename,
              pLock->m_line, pNewJob->GetPauseReasonDescription(),
              m_pWaitingOnLock ? m_pWaitingOnLock->GetName() : "none");
  }
}
...
void CJob::OnLockDeleted(CLock *pLock) {
  ...
  while (pJob) {
    // insert the job into the sleep list with 0 time, so it wakes up
    // immediately it will see it doesn't have the desired lock and suicide
...
void CJob::PopDoNotYield() {
  AssertMsg(m_stackDoNotYieldGuards.Count() > 0,
            "Could not pop a Do Not Yield guard when the job's stack is empty");
...
//-----------------------------------------------------------------------------
// Purpose: Run a global validation pass on all of our static data structures
// and memory
//			allocations.
// Input:	validator -		Our global validator object
//			pchName -		Our name (typically a member var
...
CLock::CLock()
    : m_pJob(NULL), m_pJobToNotifyOnLockRelease(NULL),
      m_pJobWaitingQueueTail(NULL), m_nWaitingCount(0), m_nsLockType(0),
      m_nsNameType(k_ENameTypeNone), m_ulID(0), m_pchConstStr(NULL),
      m_unLockSubType(0), m_nRefCount(0), m_pFilename("unknown"), m_line(0) {}
...
void CLock::Dump(const char *pszPrefix, int nPrintMax,
                 bool bPrintWaiting) const {
  if (m_pJob != NULL) {
    EmitInfo(SPEW_JOB, SPEW_ALWAYS, LOG_ALWAYS,
             "%s%s: Lock owner: %s, Type: %d, %d Waiting\n", pszPrefix,
             GetName(),
             CFmtStr("%s (%llu), Reason: %s", m_pJob->GetName(),
                     m_pJob->GetJobID(), m_pJob->GetPauseReasonDescription())
                 .Access(),
             (int32)m_nsLockType, m_nWaitingCount);
    EmitInfo(SPEW_JOB, SPEW_ALWAYS, LOG_ALWAYS, "%sLock acquired: %s:%d\n",
             pszPrefix, m_pFilename, m_line);
  } else {
    EmitInfo(SPEW_JOB, SPEW_ALWAYS, LOG_ALWAYS,
             "%s%s: Lock owner: None, Type: %d, %d Waiting\n", pszPrefix,
             GetName(), (int32)m_nsLockType, m_nWaitingCount);
  }
  ...
  while (pCurrWaiting != NULL && nPrinted < nPrintMax) {
    ...
    if (pCurrWaiting->GetPauseReason() == k_EJobPauseReasonWaitingForLock &&
        pCurrWaiting->m_pWaitingOnLock == this) {
      bPrint = true;
    }

    if (bPrint && bPrintWaiting) {
      EmitInfo(SPEW_JOB, SPEW_ALWAYS, LOG_ALWAYS,
               "%s\tOther jobs waiting for this lock: %s (%llu)\n", pszPrefix,
               pCurrWaiting->GetName(), pCurrWaiting->GetJobID());
      if (pCurrWaiting->GetPauseReason() == k_EJobPauseReasonWaitingForLock &&
          pCurrWaiting->m_pWaitingOnLock == this) {
        EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, 3, "%s\tAt: %s:%d\n", pszPrefix,
                 pCurrWaiting->m_pWaitingOnLockFilename,
                 pCurrWaiting->m_waitingOnLockLine);
      }
      nPrinted++;
    }
  ...
  if (bPrintWaiting || nTotal != 0) {
    EmitInfo(SPEW_JOB, SPEW_ALWAYS, LOG_ALWAYS,
             "%s%d out of %d waiting jobs printed.\n", pszPrefix, nPrinted,
             nTotal);
  }
}
