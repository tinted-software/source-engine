...
//-------------------------------------------------------------------------
CGlobalScheduledFunction::CGlobalScheduledFunction() : m_pfn(NULL) {}
...
//-------------------------------------------------------------------------
void CGlobalScheduledFunction::OnEvent() { m_pfn(); }
...
void CScheduledFunctionMgr::InitStartingTime() {
  ...
  for (uint32 nCurrBucket = 0; nCurrBucket < ARRAYSIZE(m_Resolutions);
       nCurrBucket++) {
    m_Resolutions[nCurrBucket].m_nAbsLastScheduleBucket =
        m_Resolutions[nCurrBucket].GetAbsScheduleBucketIndex(nCurrTime);
  }
}

//-------------------------------------------------------------------------

void CScheduledFunctionMgr::InternalSchedule(uint32 nResolution,
                                             IGCScheduledFunction *pEvent,
                                             uint32 nMSDelay) {
  // if the event is already registered, deregister it, double registration
  ...
  uint32 nAbsBucket = m_Resolutions[nResolution].GetAbsScheduleBucketIndex(
      CJobTime::LJobTimeCur() + (uint64)nMSDelay * 1000);
  // so we can remove it, and deal with wrapping
  pEvent->m_nAbsScheduleBucket = nAbsBucket;

  // add it to our list
  uint32 nInsertAfter =
      m_Resolutions[nResolution]
          .m_pBuckets[nAbsBucket % m_Resolutions[nResolution].m_nNumBuckets];
  pEvent->m_nLLIndex = m_ScheduleList.InsertAfter(nInsertAfter, pEvent);
}

void CScheduledFunctionMgr::ScheduleMS(IGCScheduledFunction *pEvent,
                                       uint32 nMSDelay) {
  InternalSchedule(0, pEvent, nMSDelay);
}

void CScheduledFunctionMgr::ScheduleSecond(IGCScheduledFunction *pEvent,
                                           uint32 nSDelay) {
  InternalSchedule(1, pEvent, nSDelay * 1000);
}

void CScheduledFunctionMgr::ScheduleMinute(IGCScheduledFunction *pEvent,
                                           uint32 nMinuteDelay) {
...
void CScheduledFunctionMgr::Cancel(IGCScheduledFunction *pEvent) {
  ...
  if (m_ScheduleList.IsValidIndex(pEvent->m_nLLIndex)) {
    ...
    AssertMsg(
        false,
        "Warning: Ecountered a remove request for a scheduled event but was "
        "unable to find it in the bucket that it was supposed to be in");
  }
...
CScheduledFunctionMgr::CScheduleBucket::CScheduleBucket()
    : m_pBuckets(NULL), m_nNumBuckets(0), m_nMicroSPerBucket(0),
...
void CScheduledFunctionMgr::CScheduleBucket::Init(TScheduleList &MasterList,
                                                  uint32 nNumBuckets,
                                                  uint32 nMicroSPerBucket) {
  // init should never happen more than once
  AssertMsg(
      !m_pBuckets,
      "Error: Schedule buckets should never be initialized multiple times");
...
void CScheduledFunctionMgr::CScheduleBucket::RunFunctions(
    TScheduleList &MasterList, uint64 nMicroSTime) {
  // determine the absolute bucket index of our current frame that we want to
  // run to
  uint32 nCurrFrameBucket = GetAbsScheduleBucketIndex(nMicroSTime);

  // note that we include the starting bucket, but not the ending bucket. This
  // addresses two issues: it avoids checking buckets multiple times, and since
  // we may not have completely spent all the time for the frame, events will
  // not fire early
  for (uint32 nAbsBucket = m_nAbsLastScheduleBucket;
       nAbsBucket < nCurrFrameBucket; nAbsBucket++) {
    // map this to a relative bucket
    uint32 nStartIndex = m_pBuckets[nAbsBucket % m_nNumBuckets];
    AssertMsg2(nStartIndex != MasterList.InvalidIndex(),
               "Error: Detected list corruption when accessing list bucket %d "
               "(%d micro s per bucket)",
               nAbsBucket % m_nNumBuckets, m_nMicroSPerBucket);
    for (uint32 nCurrEvent = MasterList.Next(nStartIndex);
         nCurrEvent != MasterList.InvalidIndex();) {
      // make sure to advance to the next item immediately since we'll be
      // removing this element from our list
      ...
      // skip any event that happens in the future (can occur since we wrap the
      // frame list over itself to map to an actual bucket)
      if (pEvent->m_nAbsScheduleBucket > nAbsBucket) {
        // just skip over this element
        continue;
      }

      // sanity check that this bucket matches exactly. We'll process stale
      // ones, but this warns us that we missed an event and cycled through the
      // bucket, causing it be very delayed
      AssertMsg2(pEvent->m_nAbsScheduleBucket == nAbsBucket,
                 "Warning: Encountered a scheduled event that was intended to "
                 "be fired on bucket %d but wasn't until bucket %d",
                 pEvent->m_nAbsScheduleBucket, nAbsBucket);
      ...
      pEvent->OnEvent();
      // NOTE: You cannot use the object any more, it may have destroyed
      // itself!!!!
      pEvent = NULL;
    }
  }
