...
GCConVar
    cv_account_details_failure_cache_time("account_details_failure_cache_time",
                                          "10");
...
GCConVar cv_persona_name_failure_cache_time("persona_name_failure_cache_time",
                                            "10");
...
CAccountDetails::CAccountDetails()
    : m_rtimeCached(CRTime::RTime32TimeCur()), m_bValid(false),
      ...
      m_bSchoolAccount(false), m_bFreeTrialAccount(false), m_bSubscribed(false),
      m_bLowViolence(false), m_bLimited(false), m_bAccountLocked(false),
      m_bCommunityBanned(false), m_bTradeBanned(false),
      m_bIsSteamGuardEnabled(false), m_bIsPhoneVerified(false),
      m_bIsTwoFactorAuthEnabled(false), m_bIsPhoneIdentifying(false),
      m_unPackage(0), m_rtimeVACBanEnd(0), m_unSteamLevel(0),
      m_unFriendCount(0), m_rtimeAccountCreated(0), m_rtimeTwoFactorEnabled(0),
      m_rtimePhoneVerified(0), m_unPhoneID(0) {}

//-----------------------------------------------------------------------------
// Purpose: Initialize a fresh CAccountDetails with data from Steam
//-----------------------------------------------------------------------------
void CAccountDetails::Init(
    CGCSystemMsg_GetAccountDetails_Response &msgResponse) {
  m_bValid = true;
...
bool CAccountDetails::BIsExpired() const {
  int nCacheSeconds = BIsValid()
                          ? cv_account_details_cache_time.GetInt()
                          : cv_account_details_failure_cache_time.GetInt();
...
public:
  CGCJobSendGetAccountDetailsRequest(CGCBase *pGC,
                                     CAccountDetailsManager *pManager,
  ...
  virtual bool BYieldingRunGCJob() {
    ...
    // These requests should come back very quickly, so if they don't we
    // shouldn't wait very long jamming up the system
    SetJobTimeout(account_details_timeout.GetInt());

    // Get an empty account details object
    CAccountDetails *pAccount =
        m_pManager->m_hashAccountDetailsCache.PvRecordFind(
            m_SteamID.GetAccountID());
    if (NULL == pAccount) {
      pAccount = m_pManager->m_hashAccountDetailsCache.PvRecordInsert(
          m_SteamID.GetAccountID());
    } else {
    ...
    CProtoBufMsg<CGCSystemMsg_GetAccountDetails> msgReqeust(
    ...
    for (int iTries = 0; iTries < kMaxTries; iTries++) {
      if (!m_pGC->BSendSystemMessage(msgReqeust)) {
        EmitWarning(SPEW_GC, 2,
                    "Unable to send GetAccountDetails system message\n");
        continue;
      }
      ...
      if (!BYieldingWaitForMsg(&msgReply, k_EGCMsgGetAccountDetailsResponse)) {
        EmitWarning(
            SPEW_GC, 2,
            "Timeout waiting for GetAccountDetails reply for SteamID %s\n",
            m_SteamID.Render());
        continue;
      }
      ...
      if (k_EResultOK != msgReply.GetEResult()) {
        EmitInfo(
            SPEW_GC, 4, 4,
            "GetAccountDetails request failed with result %d for SteamID %s\n",
      ...
      if (msgReply.Body().has_accountid() &&
          msgReply.Body().accountid() != m_SteamID.GetAccountID()) {
        static bool bHasAlerted = false;
        if (!bHasAlerted) {
          GGCBase()->PostAlert(
              k_EAlertTypeInfo, true,
              CFmtStr("GetAccountDetails got a response for account %d, but we "
                      "were expecting a response for account %s\n",
                      msgReply.Body().accountid(), m_SteamID.Render()));
          bHasAlerted = true;
        }

        EmitError(SPEW_GC,
                  "GetAccountDetails got a response for account %d, but we "
                  "were expecting a response for account %s\n",
                  msgReply.Body().accountid(), m_SteamID.Render());
        break;
      }
      ...
      if (!msgReply.Body().has_account_name()) {
        EmitError(SPEW_GC,
                  "GetAccountDetails got a response with missing fields for "
                  "SteamID %s\n",
                  m_SteamID.Render());
        break;
      }
      ...
      m_pManager->CachePersonaName(CSteamID(m_SteamID),
                                   msgReply.Body().persona_name().c_str());
...
CCachedPersonaName::CCachedPersonaName()
    : m_rtimeCached(
          0) // This initialization value is important because it makes it
             // expired on creation, and the rest of the code will only ask
             // Steam for a name if the cached entry is expired
      ,
      m_nLoading(0), m_bPreloading(false) {}

//-----------------------------------------------------------------------------
// Purpose: Destructor
//-----------------------------------------------------------------------------
CCachedPersonaName::~CCachedPersonaName() { Assert(!BIsLoading()); }
...
bool CCachedPersonaName::BIsExpired() const {
  int nCacheSeconds = BIsValid() ? cv_persona_name_cache_time.GetInt()
                                 : cv_persona_name_failure_cache_time.GetInt();
...
//-----------------------------------------------------------------------------
bool CCachedPersonaName::BIsValid() const { return !m_sPersonaName.IsEmpty(); }
...
//-----------------------------------------------------------------------------
void CCachedPersonaName::SetPreloading() { m_bPreloading = true; }

//-----------------------------------------------------------------------------
// Purpose: Sets that a job is waiting for this name to be loaded
//-----------------------------------------------------------------------------
void CCachedPersonaName::AddLoadingRef() { m_nLoading++; }

//-----------------------------------------------------------------------------
// Purpose: Releases the loading ref
//-----------------------------------------------------------------------------
void CCachedPersonaName::ReleaseLoadingRef() { DbgVerify(--m_nLoading >= 0); }
...
public:
  CGCJobSendGetPersonaNamesRequest(CGCBase *pGC,
                                   CAccountDetailsManager *pManager,
  ...
  virtual bool BYieldingRunGCJob() {
    ...
    if (!m_pGC->BSendSystemMessage(msgReqeust) ||
      ...
      // if we are shutting down, don't bother reporting this issue, we know we
      // won't be able to get persona names
      if (!GGCBase()->GetIsShuttingDown()) {
        EmitWarning(SPEW_GC, 2, "GetPersonaNames request failed for %d IDs:",
                    m_vecSteamIDs.Count());
      ...
      for (int i = 0; i < msgReply.Body().succeeded_lookups_size(); i++) {
        const CMsgGCGetPersonaNames_Response_PersonaName &result =
            msgReply.Body().succeeded_lookups(i);
        m_pManager->CachePersonaName(CSteamID(result.steamid()),
                                     result.persona_name().c_str());
      }
      for (int i = 0; i < msgReply.Body().failed_lookup_steamids_size(); i++) {
        m_pManager->CachePersonaNameFailure(
            CSteamID(msgReply.Body().failed_lookup_steamids(i)));
      }
    }
...
CAccountDetailsManager::CAccountDetailsManager()
    : m_hashAccountDetailsCache(k_nAccountDetailsRunInterval /
                                k_cMicroSecPerShellFrame),
      m_hashPersonaNameCache(k_nAccountDetailsRunInterval /
                             k_cMicroSecPerShellFrame) {
  m_hashAccountDetailsCache.Init(k_cAccountDetailsInit,
                                 k_cBucketAccountDetails);
...
//-----------------------------------------------------------------------------
CAccountDetails *
CAccountDetailsManager::YieldingGetAccountDetails(const CSteamID &steamID,
                                                  bool bForceReload) {
  AssertRunningJob();
  ...
  if (!m_mapQueuedAccountDetailsRequests.IsValidIndex(iMapIndex)) {
    ...
    CGCJobSendGetAccountDetailsRequest *pJob =
        new CGCJobSendGetAccountDetailsRequest(GGCBase(), this, steamID);
    pJob->StartJob(NULL);
  }
...
//-----------------------------------------------------------------------------
bool CAccountDetailsManager::BFindAccountDetailsInLocalCache(
    const CSteamID &steamID, CAccountDetails **ppAccount) {
  CAccountDetails *pAccountLocal =
      m_hashAccountDetailsCache.PvRecordFind(steamID.GetAccountID());
...
//-----------------------------------------------------------------------------
void CAccountDetailsManager::WakeWaitingAccountDetailsJobs(
    const CSteamID &steamID) {
  int iMapIndex = m_mapQueuedAccountDetailsRequests.Find(steamID);
  if (!m_mapQueuedAccountDetailsRequests.IsValidIndex(iMapIndex))
    return;

  CCopyableUtlVector<JobID_t> &vecJobsWaiting =
      m_mapQueuedAccountDetailsRequests[iMapIndex];
  FOR_EACH_VEC(vecJobsWaiting, i) {
    GGCBase()->GetJobMgr().BRouteWorkItemCompletedDelayed(vecJobsWaiting[i],
                                                          false);
  }
...
//-----------------------------------------------------------------------------
const char *
CAccountDetailsManager::YieldingGetPersonaName(const CSteamID &steamID) {
  VPROF_BUDGET("CAccountDetailsManager::YieldingGetPersonaName",
  ...
  if (!m_mapQueuedPersonaNameRequests.IsValidIndex(iMapIndex)) {
    ...
    if (m_vecPendingPersonaNameLookups.Count() >=
        cv_persona_name_batch_size.GetInt()) {
      SendBatchedPersonaNamesRequest();
    }
  }
...
void CAccountDetailsManager::SendBatchedPersonaNamesRequest() {
  ...
  CGCJobSendGetPersonaNamesRequest *pJob = new CGCJobSendGetPersonaNamesRequest(
      GGCBase(), this, m_vecPendingPersonaNameLookups);
  pJob->StartJob(NULL);
}
...
//-----------------------------------------------------------------------------
void CAccountDetailsManager::CachePersonaName(const CSteamID &steamID,
                                              const char *pchPersonaName) {
  CCachedPersonaName *pCachedPersonaName =
      FindOrCreateCachedPersonaName(steamID);
...
void CAccountDetailsManager::ClearCachedPersonaName(const CSteamID &steamID) {
  CCachedPersonaName *pPersonaNameLocal =
      m_hashPersonaNameCache.PvRecordFind(steamID.GetAccountID());
...
//-----------------------------------------------------------------------------
CCachedPersonaName *
CAccountDetailsManager::FindOrCreateCachedPersonaName(const CSteamID &steamID) {
  CCachedPersonaName *pPersonaName =
      m_hashPersonaNameCache.PvRecordFind(steamID.GetAccountID());
...
// Purpose: Wakes any jobs waiting on this result
//-----------------------------------------------------------------------------
void CAccountDetailsManager::WakeWaitingPersonaNameJobs(
    const CSteamID &steamID) {
  int iMapIndex = m_mapQueuedPersonaNameRequests.Find(steamID);
  if (!m_mapQueuedPersonaNameRequests.IsValidIndex(iMapIndex))
    return;
  ...
  CCopyableUtlVector<JobID_t> &vecJobsWaiting =
      m_mapQueuedPersonaNameRequests[iMapIndex];
  FOR_EACH_VEC(vecJobsWaiting, i) {
    GGCBase()->GetJobMgr().BRouteWorkItemCompletedDelayed(vecJobsWaiting[i],
                                                          false);
  }
...
bool CAccountDetailsManager::BExpireRecords(CLimitTimer &limitTimer) {
  ...
  for (CAccountDetails *pDetails = m_hashAccountDetailsCache.PvRecordRun();
       NULL != pDetails; pDetails = m_hashAccountDetailsCache.PvRecordRun()) {
  ...
  for (CCachedPersonaName *pPersonaName = m_hashPersonaNameCache.PvRecordRun();
       NULL != pPersonaName;
       pPersonaName = m_hashPersonaNameCache.PvRecordRun()) {
...
void CAccountDetailsManager::Dump() const {
  ...
  EmitInfo(
      SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
      "\tAccount Details: %d cached, %d lookups in flight, %d jobs waiting\n",
      m_hashAccountDetailsCache.Count(),
      m_mapQueuedAccountDetailsRequests.Count(), nJobsWaiting);
  ...
  EmitInfo(
      SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
      "\tPersona Names: %d cached, %d lookups in flight, %d jobs waiting\n",
      m_hashPersonaNameCache.Count(), m_mapQueuedPersonaNameRequests.Count(),
      nJobsWaiting);
}
