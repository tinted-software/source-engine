...
//
// Purpose: Extra functionality on top of CGCClientSharedObjectCache for
// GCClients
//
//=============================================================================
...
// Purpose: constructor
//----------------------------------------------------------------------------
CGCClientSharedObjectContext::CGCClientSharedObjectContext(
...
//----------------------------------------------------------------------------
bool CGCClientSharedObjectContext::BAddListener(
...
//Listener.
//----------------------------------------------------------------------------
bool CGCClientSharedObjectContext::BRemoveListener(
    ISharedObjectListener *pListener) {
...
//			context
//----------------------------------------------------------------------------
void CGCClientSharedObjectContext::SOCreated(const CSharedObject *pObject,
                                             ESOCacheEvent eEvent) const {
...
void CGCClientSharedObjectContext::SOUpdated(const CSharedObject *pObject,
                                             ESOCacheEvent eEvent) const {
  tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s", __FUNCTION__);
  FOR_EACH_VEC(m_vecListeners, nListener) {
...
void CGCClientSharedObjectContext::SODestroyed(const CSharedObject *pObject,
                                               ESOCacheEvent eEvent) const {
  tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s", __FUNCTION__);
  FOR_EACH_VEC(m_vecListeners, nListener) {
...
void CGCClientSharedObjectContext::SOCacheSubscribed(
    const CSteamID &steamIDOwner, ESOCacheEvent eEvent) const {
  tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s", __FUNCTION__);
  FOR_EACH_VEC(m_vecListeners, nListener) {
...
void CGCClientSharedObjectContext::SOCacheUnsubscribed(
    const CSteamID &steamIDOwner, ESOCacheEvent eEvent) const {
  tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s", __FUNCTION__);
  FOR_EACH_VEC(m_vecListeners, nListener) {
...
CGCClientSharedObjectTypeCache::CGCClientSharedObjectTypeCache(
    int nTypeID, const CGCClientSharedObjectContext &context)
...
//----------------------------------------------------------------------------
bool CGCClientSharedObjectTypeCache::BParseCacheSubscribedMsg(
  ...
  for (uint16 usObject = 0; usObject < msg.object_data_size(); usObject++) {
    ...
    CSharedObject *pObject =
        BCreateFromMsg(msg.object_data(usObject).data(),
                       msg.object_data(usObject).size(), &bUpdatedExisting);
    if (pObject == NULL) {
      Assert(pObject);
      return false;
    }
...
void CGCClientSharedObjectTypeCache::RemoveAllObjects(
    CUtlVector<CSharedObject *> &vecObjects) {
...
//----------------------------------------------------------------------------
CSharedObject *CGCClientSharedObjectTypeCache::BCreateFromMsg(
    const void *pvData, uint32 unSize, bool *bUpdatedExisting) {
...
//			the client/gameserver
//----------------------------------------------------------------------------
bool CGCClientSharedObjectTypeCache::BDestroyFromMsg(const void *pvData,
                                                     uint32 unSize) {
...
//			the client/gameserver
//----------------------------------------------------------------------------
bool CGCClientSharedObjectTypeCache::BUpdateFromMsg(const void *pvData,
                                                    uint32 unSize) {
  CUtlBuffer bufUpdate(pvData, unSize, CUtlBuffer::READ_ONLY);
  CSharedObject *pIndexObj = CSharedObject::Create(GetTypeID());
  AssertMsg1(pIndexObj, "Unable to create index object of type %d",
             GetTypeID());
  if (!pIndexObj)
    return false;
...
CGCClientSharedObjectCache::CGCClientSharedObjectCache(
    const CSteamID &steamIDOwner)
    : m_context(steamIDOwner), m_bInitialized(false), m_bSubscribed(false) {}
...
//----------------------------------------------------------------------------
bool CGCClientSharedObjectCache::BParseCacheSubscribedMsg(
    const CMsgSOCacheSubscribed &msg) {

  // Assume all type caches will be untouched
  CUtlVector<int> vecUntouchedTypes;
  for (int i = FirstTypeCacheIndex(); i != InvalidTypeCacheIndex();
       i = NextTypeCacheIndex(i)) {
  ...
  for (uint16 usObject = 0; usObject < msg.objects_size(); usObject++) {
    ...
    CGCClientSharedObjectTypeCache *pTypeCache =
        CreateTypeCache(msgType.type_id());
    if (pTypeCache) {
    ...
    Assert(pTypeCache);
    if (!pTypeCache ||
        !pTypeCache->BParseCacheSubscribedMsg(
            msgType, vecCreatedObjects, vecUpdatedObjects, vecObjectsToDestroy))
      bResult = false;
  }
  ...
  for (int i = FirstTypeCacheIndex(); i != InvalidTypeCacheIndex();
    ...
    if (vecUntouchedTypes.Find(pTypeCache->GetTypeID()) !=
        vecUntouchedTypes.InvalidIndex()) {
...
void CGCClientSharedObjectCache::NotifyUnsubscribe() {
  if (m_bSubscribed) {
    ...
    AssertMsg(m_bSubscribed, "GC Sending us Unsubscribed message when we "
                             "weren't subscribed"); // Might not be a bug, but
                                                    // something worth checking
  }
}

//----------------------------------------------------------------------------
// Purpose: GC is telling us that the version we have is up-to-date,a nd we are
// subscribed
//----------------------------------------------------------------------------
void CGCClientSharedObjectCache::NotifyResubscribedUpToDate() {
  if (!m_bSubscribed) {
    ...
    AssertMsg(m_bSubscribed, "Got NotifyResubscribedUpToDate when we were "
                             "already subscribed?"); // Might not be a bug, but
                                                     // something worth checking
  }
}
...
// Purpose: Process an incoming create message on a client/gameserver.
//----------------------------------------------------------------------------
bool CGCClientSharedObjectCache::BCreateFromMsg(int nTypeID, const void *pvData,
                                                uint32 unSize) {
  ...
  if (!m_bInitialized || !m_bSubscribed) {
    ...
    // EmitWarning( SPEW_SHAREDOBJ, 1, "Received SOCache incremental update for
    // cache we were not subscribed to (object type %d)\n", nTypeID );
  }
  ...
  bool bUpdatedExisting = false;
  CSharedObject *pObject =
      pTypeCache->BCreateFromMsg(pvData, unSize, &bUpdatedExisting);
  if (pObject == NULL)
    return false;
  ...
  if (bUpdatedExisting) {
    ...
    // Assert( !bUpdatedExisting ); // shouldn't the GC know what it's already
    // sent us?  This is weird
...
//----------------------------------------------------------------------------
bool CGCClientSharedObjectCache::BDestroyFromMsg(int nTypeID,
                                                 const void *pvData,
                                                 uint32 unSize) {
...
//----------------------------------------------------------------------------
bool CGCClientSharedObjectCache::BUpdateFromMsg(int nTypeID, const void *pvData,
                                                uint32 unSize) {
  CGCClientSharedObjectTypeCache *pTypeCache = FindTypeCache(nTypeID);
  if (pTypeCache) {
...
//			cache. The shared object cache does not own this object
//and will 			not free it.
//----------------------------------------------------------------------------
...
//----------------------------------------------------------------------------
bool CGCClientSharedObjectCache::RemoveListener(
    ISharedObjectListener *pListener) {
  Assert(pListener);
