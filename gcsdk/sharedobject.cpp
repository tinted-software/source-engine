...
//
// Purpose: Base class for objects that are kept in synch between client and
// server
//
//=============================================================================
...
//----------------------------------------------------------------------------
CUtlMap<int, CSharedObject::SharedObjectInfo_t>
    CSharedObject::sm_mapFactories(DefLessFunc(int));

//----------------------------------------------------------------------------
// Purpose: Registers a new CSharedObject class
//----------------------------------------------------------------------------
void CSharedObject::RegisterFactory(int nTypeID, SOCreationFunc_t fnFactory,
                                    uint32 unFlags, const char *pchClassName) {
  SharedObjectInfo_t info;
  info.m_pFactoryFunction = fnFactory;
  info.m_unFlags = unFlags;
  info.m_pchClassName = pchClassName;
  info.m_sBuildCacheSubNodeName.Format("BuildCacheSubscribed(%s)",
                                       pchClassName);
...
CSharedObject *CSharedObject::Create(int nTypeID) {
  ...
  AssertMsg1(sm_mapFactories.IsValidIndex(nIndex),
...
//			steam ID.
//----------------------------------------------------------------------------
bool CSharedObject::BSendCreateToSteamID(const CSteamID &steamID,
                                         const CSteamID &steamIDOwner,
                                         uint64 ulVersion) const {
...
//----------------------------------------------------------------------------
bool CSharedObject::BSendDestroyToSteamID(const CSteamID &steamID,
                                          const CSteamID &steamIDOwner,
                                          uint64 ulVersion) const {
  // Don't send these while shutting down.  We'll use higher-level
  // connection-oriented messages instead
  ...
  if (!BAddDestroyToMessage(msg.Body().mutable_object_data())) {
    EmitWarning(SPEW_SHAREDOBJ, SPEW_ALWAYS,
                "Failed to add key to message in create");
    return false;
  }
  return GGCBase()->BSendGCMsgToClient(steamID, msg);
}
...
bool CSharedObject::BYieldingAddToDatabase() {
  ...
  sqlAccess.BBeginTransaction(
      CFmtStr("CSharedObject::BYieldingAddToDatabase Type %d", GetTypeID()));
...
bool CSharedObject::BYieldingWriteToDatabase(const CUtlVector<int> &fields) {
  CSQLAccess sqlAccess;
  sqlAccess.BBeginTransaction(
      CFmtStr("CSharedObject::BYieldingWriteToDatabase Type %d", GetTypeID()));
...
bool CSharedObject::BYieldingRemoveFromDatabase() {
  CSQLAccess sqlAccess;
  sqlAccess.BBeginTransaction(CFmtStr(
      "CSharedObject::BYieldingRemoveFromDatabase Type %d", GetTypeID()));
...
CSharedObjectStats::CSharedObjectStats()
    : m_bCollectingStats(false), m_nMicroSElapsed(0) {}
...
void CSharedObjectStats::RegisterSharedObjectType(int nTypeID,
                                                  const char *pszTypeName) {
  if (nTypeID < 0) {
    AssertMsg2(false,
               "Error registering shared object type %d (%s), negative type "
  ...
  if (nTypeID >= m_vTypeToIndex.Count()) {
    ...
    AssertMsg(
        nTypeID < 8 * 1024,
        "Warning: Using a very large type ID which can be inefficient for the "
        "shared object stats. Try to keep values to a smaller range");
    ...
    AssertMsg2(false,
               "Error registering shared object type %d (%s), may have "
               "multiple registrations of this type, check for conflicts",
               nTypeID, pszTypeName);
    return;
  }

  // we need to register this type by adding a new record, and updating our
  // index
...
void CSharedObjectStats::TrackSharedObjectLifetime(int nTypeID, int32 nCount) {
  uint16 nIndex = (m_vTypeToIndex.IsValidIndex(nTypeID))
                      ? m_vTypeToIndex[nTypeID]
                      : knInvalidIndex;
...
void CSharedObjectStats::TrackSharedObjectSendCreate(int nTypeID,
                                                     uint32 nCount) {
  uint16 nIndex = (m_vTypeToIndex.IsValidIndex(nTypeID))
                      ? m_vTypeToIndex[nTypeID]
                      : knInvalidIndex;
...
void CSharedObjectStats::TrackSharedObjectSendDestroy(int nTypeID,
                                                      uint32 nCount) {
  uint16 nIndex = (m_vTypeToIndex.IsValidIndex(nTypeID))
                      ? m_vTypeToIndex[nTypeID]
                      : knInvalidIndex;
...
void CSharedObjectStats::TrackSubscription(int nTypeID, uint32 nFlags,
                                           uint32 nNumSubscriptions) {
  if (!m_bCollectingStats)
    return;
...
void CSharedObjectStats::TrackSharedObjectSend(int nTypeID, uint32 nNumClients,
                                               uint32 nMsgSize) {
  if (!m_bCollectingStats)
    return;
...
void CSharedObjectStats::ResetStats() {
  FOR_EACH_VEC(m_Stats, nCurrSO) { m_Stats[nCurrSO].ResetStats(); }
}

bool CSharedObjectStats::SortSOStatsSent(const SOStats_t *pLhs,
                                         const SOStats_t *pRhs) {
...
bool CSharedObjectStats::SortSOStatsSubscribe(const SOStats_t *pLhs,
                                              const SOStats_t *pRhs) {
  // sort based on total number of subscriptions
  uint32 nLhsSub = pLhs->m_nNumSubOwner + pLhs->m_nNumSubOtherUsers +
                   pLhs->m_nNumSubGameServer;
  uint32 nRhsSub = pRhs->m_nNumSubOwner + pRhs->m_nNumSubOtherUsers +
                   pRhs->m_nNumSubGameServer;
  if (nLhsSub != nRhsSub)
...
void CSharedObjectStats::ReportCollectedStats() const {
  // build up a list of our stats, so we can sort them appropriately (also total
  // how many bytes we send)
  ...
  uint64 nMicroSElapsed = (m_bCollectingStats)
                              ? m_CollectTime.CServerMicroSecsPassed()
                              : m_nMicroSElapsed;
  ...
  EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
           "SO Cache Transmits - %.2f second capture\n", fSeconds);
  EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS, "\n");

  // now run through and display our report
  EmitInfo(
      SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS, "%s",
      "Type Name                           SendKB  %      SendKB/S MPlex  "
      "Create  C/S     Destroy D/S     Update  U/S     Active    U/S (%)\n");
  EmitInfo(
      SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS, "%s",
      "---- ------------------------------ ------- ------ -------- ------ "
      "------- ------- ------- ------- ------- ------- --------- -------\n");
  ...
  FOR_EACH_VEC(sortedStats, nCurrSO) {
    ...
    EmitInfo(
        ...
        "%4d %-30s %7u %5.1f%% %8.1f %6.2f %7u %7.0f %7u %7.0f %7u %7.0f %9u "
        "%6.3f%%\n",
        ...
        stats.m_nNumCreated, stats.m_nNumCreated * fToS, stats.m_nNumDestroyed,
        stats.m_nNumDestroyed * fToS, stats.m_nNumSends,
        stats.m_nNumSends * fToS, stats.m_nNumActive,
  ...
  EmitInfo(
      ...
      "------- ------- ------- ------- ------- ------- --------- -------\n");
  EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
           "     Totals                         %7u %5.1f%% %8.1f %6.2f %7u "
           "%7.0f %7u %7.0f %7u %7.0f %9u %6.3f%%\n",
           (uint32)(nTotalRawBytes / 1024), 100.0,
           (double)(nTotalRawBytes / 1024) * fToS,
           (double)nTotalMultiplexBytes / (double)MAX(1, nTotalRawBytes),
           nTotalCreates, nTotalCreates * fToS, nTotalFrees, nTotalFrees * fToS,
           nTotalSends, nTotalSends * fToS, nTotalActive,
  ...
  EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS, "\n");
  EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
           "SO Cache Subscriptions Sends - %.2f second capture\n", fSeconds);
  EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS, "\n");

  EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS, "%s",
           "Type Name                           Owner    Owner/S  Other    "
           "Other/S  Server   Server/S\n");
  EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS, "%s",
           "---- ------------------------------ -------- -------- -------- "
           "-------- -------- --------\n");
  ...
  FOR_EACH_VEC(sortedStats, nCurrSO) {
    ...
    EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
             "%4d %-30s %8u %8.0f %8u %8.0f %8u %8.0f\n", stats.m_nTypeID,
             stats.m_sName.Get(), stats.m_nNumSubOwner,
             stats.m_nNumSubOwner * fToS, stats.m_nNumSubOtherUsers,
             stats.m_nNumSubOtherUsers * fToS, stats.m_nNumSubGameServer,
  ...
  EmitInfo(
      SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
      "     Totals                         %8u %8.0f %8u %8.0f %8u %8.0f\n",
      nTotalSubOwner, nTotalSubOwner * fToS, nTotalSubOtherUsers,
      nTotalSubOtherUsers * fToS, nTotalSubGameServer,
...
void CSharedObject::ValidateStatics(CValidator &validator) {
  CValidateAutoPushPop validatorAutoPushPop(validator, NULL,
                                            "CSharedObject::ValidateStatics",
                                            "CSharedObject::ValidateStatics");
