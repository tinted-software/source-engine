...
public:
  CCachedSubscriptionMessage() : message(k_ESOMsg_CacheSubscribed) {}
...
IMPLEMENT_CLASS_MEMPOOL(CCachedSubscriptionMessage, 10 * 1000,
                        UTLMEMORYPOOL_GROW_SLOW);
} // namespace GCSDK
...
public:
  ...
  virtual bool
  BShouldSendAnyObjectsInCache(CGCSharedObjectTypeCache *pTypeCache,
                               uint32 unFlags) const {
    return (CSharedObject::GetTypeFlags(pTypeCache->GetTypeID()) & unFlags) !=
           0;
  }

  virtual bool BShouldSendObject(CSharedObject *pSharedObject,
                                 uint32 unFlags) const {
    return (CSharedObject::GetTypeFlags(pSharedObject->GetTypeID()) &
            unFlags) != 0;
  }
};
...
//			Returns false if it simply incremented the refcount of
//the subscribers of this cache
//----------------------------------------------------------------------------
...
//			will have destroy messages sent for them to the new
//subscriber.
//----------------------------------------------------------------------------
...
CGCSharedObjectTypeCache::CGCSharedObjectTypeCache(
    int nTypeID, const CSharedObjectContext &context)
...
//----------------------------------------------------------------------------
CGCSharedObjectTypeCache::~CGCSharedObjectTypeCache() {
  g_SharedObjectStats.TrackSharedObjectLifetime(GetTypeID(),
                                                -((int32)GetCount()));
}
...
//----------------------------------------------------------------------------
CSharedObject *
CGCSharedObjectTypeCache::RemoveObject(const CSharedObject &soIndex) {
...
//			type in the cache.
//----------------------------------------------------------------------------
void CGCSharedObjectTypeCache::BuildCacheSubscribedMsg(
    CMsgSOCacheSubscribed_SubscribedType *pMsgType, uint32 unFlags,
    const CISubscriberMessageFilter &filter) {
  // MEM_ALLOC_CREDIT_( CFmtStr( "StartPlaying - Type BuildMsg - %s",
  // CSharedObject::PchClassName( GetTypeID() ) ) );
  VPROF_BUDGET(CSharedObject::PchClassBuildCacheNodeName(GetTypeID()),
  ...
  for (uint32 i = 0; i < GetCount(); i++) {
    ...
    if (!pObj->BAddToMessage(pMsgType->add_object_data())) {
      EmitWarning(SPEW_SHAREDOBJ, SPEW_ALWAYS,
                  "Failed to add create fields to message in create");
    } else {
      nNumSubscribed++;
    }
  }
...
CGCSharedObjectCache::CGCSharedObjectCache(const CSteamID &steamIDOwner)
    : m_context(steamIDOwner), m_bInWriteback(false), m_unWritebackTime(0),
      m_unLRUHandle(CUtlLinkedList<CSteamID, uint32>::InvalidIndex()),
      m_unCachedSubscriptionMsgFlags(0), m_pCachedSubscriptionMsg(NULL),
...
const CISubscriberMessageFilter &
CGCSharedObjectCache::GetSubscriberMessageFilter() {
...
bool CGCSharedObjectCache::AddObject(CSharedObject *pSharedObject) {
  ...
  if (!GGCBase()->BIsSOCacheBeingLoaded(GetOwner())) {
    AssertMsg(GGCBase()->IsSteamIDLockedByCurJob(GetOwner()),
              "Attempt to add an object to an unlocked SO cache!");
  }
...
//----------------------------------------------------------------------------
// Purpose: Adds a shared object to the cache without dirtying the object or the
// cache
//----------------------------------------------------------------------------
bool CGCSharedObjectCache::AddObjectClean(CSharedObject *pSharedObject) {
  CSharedObjectTypeCache *pTypeCache =
      CreateBaseTypeCache(pSharedObject->GetTypeID());
...
//----------------------------------------------------------------------------
CSharedObject *
CGCSharedObjectCache::RemoveObject(const CSharedObject &soIndex) {
  AssertMsg(GGCBase()->IsSteamIDLockedByCurJob(GetOwner()),
            "Attempt to remove an object from an unlocked SO cache!");
  ...
  if (pObject) {
    ...
    if (BShouldSendToAnyClients(unTypeFlags)) {
      ...
      const CUtlVector<CSharedObjectContext::Subscriber_t> &vecSubscribers =
          m_context.GetSubscribers();
      ...
      FOR_EACH_VEC(vecSubscribers, nSub) {
        ...
        if (filter.BShouldSendObject(pObject, unFlags)) {
          VPROF_BUDGET(
              CSharedObject::PchClassCreateNodeName(pObject->GetTypeID()),
              VPROF_BUDGETGROUP_STEAM);
      ...
      if (vecRecipients.Count() > 0) {
        ...
        FOR_EACH_VEC(vecRecipients, i) {
          pObject->BSendDestroyToSteamID(vecRecipients[i], m_context.GetOwner(),
                                         GetVersion());
        }
#endif
        g_SharedObjectStats.TrackSharedObjectSendDestroy(pObject->GetTypeID(),
                                                         vecRecipients.Count());
      }
    }
...
//			same one or simply one with matching index fields.
//----------------------------------------------------------------------------
bool CGCSharedObjectCache::BDestroyObject(const CSharedObject &soIndex,
                                          bool bRemoveFromDatabase) {
...
//			be sent as create messages to the new subscriber
//----------------------------------------------------------------------------
void CGCSharedObjectCache::AddSubscriber(const CSteamID &steamID,
                                         bool bForceSendSubscriptionMsg) {
  MEM_ALLOC_CREDIT_("StartPlaying - AddSubscriber");
  VPROF_BUDGET("CGCSharedObjectCache::AddSubscriber()",
               VPROF_BUDGETGROUP_STEAM);

  Assert(steamID.IsValid());
  bool bAdded = m_context.BAddSubscriber(steamID);
  if (bAdded || bForceSendSubscriptionMsg) {
    // ask the client if it really needs the contents of this cache, because we
    // could potentially send a lot of stuff
    CProtoBufMsg<CMsgSOCacheSubscriptionCheck> msg(
...
//----------------------------------------------------------------------------
CCachedSubscriptionMessage *
CGCSharedObjectCache::BuildSubscriberMessage(uint32 unFlags) {
  ...
  for (int nTypeIndex = 0; nTypeIndex < GetTypeCacheCount(); nTypeIndex++) {
    CGCSharedObjectTypeCache *pTypeCache =
        (CGCSharedObjectTypeCache *)GetTypeCacheByIndex(nTypeIndex);
...
//----------------------------------------------------------------------------
bool CGCSharedObjectCache::IsObjectCached(const CSharedObject *pObj,
                                          uint32 nTypeID) const {
...
// Purpose: Remembers that this field and this object are dirty
//----------------------------------------------------------------------------
void CGCSharedObjectCache::DirtyDatabaseObjectField(CSharedObject *pObj,
                                                    int nFieldIndex) {
#ifdef DEBUG
  // Make sure the object is in the shared object cache.
  Assert(IsObjectCached(pObj, pObj->GetTypeID()));
...
// Purpose: Remembers that this field and this object are dirty
//----------------------------------------------------------------------------
void CGCSharedObjectCache::DirtyObjectField(CSharedObject *pObj,
                                            int nFieldIndex) {
...
void CGCSharedObjectCache::SendAllNetworkUpdates() {
  VPROF_BUDGET("CGCSharedObjectCache::SendAllNetworkUpdates",
               VPROF_BUDGETGROUP_STEAM);
  ...
  CUtlVector<CMsgSOMultipleObjects_SingleObject> vecObjectMsgs(
      0, m_networkDirtyObjs.Count());
  CUtlVector<CSharedObject *> vecObjects(0, m_networkDirtyObjs.Count());

  {
    VPROF_BUDGET("CGCSharedObjectCache::SendAllNetworkUpdates - build messages",
                 VPROF_BUDGETGROUP_STEAM);
  ...
  // Check if anything is still dirty. It's possible that everything dirty are
  // not sent to clients
  ...
  {
    VPROF_BUDGET(
        "CGCSharedObjectCache::SendAllNetworkUpdates - send all messages",
        VPROF_BUDGETGROUP_STEAM);
    ...
    FOR_EACH_VEC(vecSubscribers, nSub) {
      ...
      if (vecObjsToSendToThisUser.Count() > 0) {
        VPROF_BUDGET(
            "CGCSharedObjectCache::SendAllNetworkUpdates - Sending Msg",
            VPROF_BUDGETGROUP_STEAM);
        ...
        FOR_EACH_VEC(vecObjsToSendToThisUser, idx) {
          const CMsgSOMultipleObjects_SingleObject &objectMsg =
              vecObjectMsgs[vecObjsToSendToThisUser[idx]];
          g_SharedObjectStats.TrackSharedObjectSend(
              objectMsg.type_id(), 1, objectMsg.object_data().size());
          // KFIXME: Tracking for when multiplex changes integrated
          // g_SharedObjectStats.TrackSharedObjectSend(
          // vecObjsToSendToThisUser[idx]->type_id(),
          // mapFlagsToObjectsToSend[iMap]->m_vecRecipients.Count(),
          // vecObjsToSendToThisUser[idx]->object_data().size() );
          CMsgSOMultipleObjects_SingleObject *pObjMsg =
              msg.Body().add_objects();
          *pObjMsg = objectMsg;
        }
...
//----------------------------------------------------------------------------
bool CGCSharedObjectCache::BYieldingAddWriteToTransaction(
    CSharedObject *pObj, CSQLAccess &sqlAccess) {
  ...
  // Add a listener to the transaction to re-add these to the cache should the
  // transaction fail.  This stores off a copy of the dirty object list, such
  // that the caller can clear theirs now and take other actions - we'll merge
  ...
  sqlAccess.AddRollbackListener([pSelf, cacheOwner, pObj, dirtyFields]() {
    ...
    // we don't need to worry about re-adding to writeback -- Callers must
    // maintain a lock of the cache for the duration, so we can't be processed
    // for writeback unless the caller is writeback itself.
  });
...
void CGCSharedObjectCache::SendNetworkCreateInternal(CSharedObject *pObj) {
  ...
  VPROF_BUDGET(CSharedObject::PchClassUpdateNodeName(pObj->GetTypeID()),
               VPROF_BUDGETGROUP_STEAM);
  ...
  FOR_EACH_VEC(vecSubscribers, nSub) {
    ...
    uint32 unFlags = CalcSendFlags(steamID);
    if (filter.BShouldSendObject(pObj, unFlags)) {
      VPROF_BUDGET(CSharedObject::PchClassCreateNodeName(pObj->GetTypeID()),
                   VPROF_BUDGETGROUP_STEAM);
  ...
  if (vecRecipients.Count() > 0) {
    g_SharedObjectStats.TrackSharedObjectSendCreate(pObj->GetTypeID(),
                                                    vecRecipients.Count());
  }
}
...
void CGCSharedObjectCache::MarkDirty() {
  // see if we are detecting a version change when we weren't expecting one.
  // This is often tied to loading or other events that should leave the version
  // in a constant state or runs the risk of creating unecessary sends
  AssertMsg(!m_bDetectVersionChanges,
            "Warning: Detected a change to the version of the SO cache when it "
            "was not expected. This is often done when an SO cache is being "
            "loaded, and can result in costly unnecessary updates of caches");
...
uint32 CGCSharedObjectCache::CalcSendFlags(const CSteamID &steamID) const {
  ...
  if (steamID.BIndividualAccount()) {
    ...
    return k_ESOFlag_SendToOtherGameservers |
           k_ESOFlag_SendToQuestObjectiveTrackers;
  }
...
bool CGCSharedObjectCache::BShouldSendToAnyClients(uint32 unFlags) const {
  return 0 !=
         (unFlags & (k_ESOFlag_SendToOwner | k_ESOFlag_SendToOtherGameservers |
                     k_ESOFlag_SendToOtherUsers));
}
...
void CGCSharedObjectCache::Validate(CValidator &validator,
                                    const char *pchName) {
  VALIDATE_SCOPE();
...
GC_CON_COMMAND(
    so_profile_dump,
    "Displays the stats collected from profiling shared object stats") {
