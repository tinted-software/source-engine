...
static GCConVar cv_assert_max_minidumps_in_window(
    "assert_max_minidumps_in_window", "5",
    "The amount of times each unique assert will write a dump in "
    "assert_minidump_window seconds");
static GCConVar enable_assert_minidumps(
    "enable_assert_minidumps", "1",
    "An emergency shutoff to prevent the recording or tracking of asserts");
static GCConVar
    filter_blank_lines("filter_blank_lines", "1",
                       "Prevents blank lines from being written or logged");

//-----------------------------------------------------------------------------
// Purpose: Creates a global pointer to the interface and exposes it to the host
//-----------------------------------------------------------------------------
CGCInterface g_GCInterface;
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CGCInterface, IGameCoordinator,
  ...
  virtual void Log(const LoggingContext_t *pContext, const tchar *pMessage) {
    ...
    case LS_ASSERT:
      ...
      if (ThreadInMainThread() && (g_pJobCur != NULL)) {
        pszFmt =
            (sizeof(tchar) == sizeof(char)) ? "[Job %s] %hs" : "[Job %s] %ls";
...
public:
  CGCAssertionFailureListener(void) : IAssertionFailureListener(false) {}

  virtual void *AssertionFailure(const char *pFormattedMsg,
                                 const tchar *pchFile, int nLine,
                                 const tchar *pchFunction,
                                 const tchar *pchRawExpression,
                                 int nInstanceReportCount,
                                 AssertionType_t nType, bool bFatal) OVERRIDE {
    if (Plat_IsInDebugSession())
      return NULL;

    bool bShouldWriteMinidump = false;
    GGCInterface()->RecordAssert(pchFile, nLine, pFormattedMsg,
                                 &bShouldWriteMinidump);

    return bShouldWriteMinidump ? this : NULL;
  }

  virtual void MiniDumpHandler(const MiniDumpHandlerData_t &HandlerData,
                               const char *pFormattedMsg, const tchar *pchFile,
                               int nLine, const tchar *pchFunction,
                               const tchar *pchRawExpression,
                               int nInstanceReportCount, AssertionType_t nType,
                               bool bFatal) OVERRIDE {
    ...
    // write to disk
    Tier0GenericMiniDumpHandlerEx(modifiableHandlerData, NULL,
                                  MINIDUMP_ADDITIONAL_FLAG_PRINT_MESSAGE);
  }
};
...
static void ProtobufLogHandler(::google::protobuf::LogLevel level,
  ...
  EG_MSG(g_EGMessages, "Protobuf %s(%d): %s\n", filename, line,
         message.c_str());
  AssertFatalMsg(level != google::protobuf::LOGLEVEL_FATAL,
                 "Fatal protobuf assert %s(%d): %s", filename, line,
                 message.c_str());
}
...
public:
  ...
  virtual bool Create() OVERRIDE {
    ...
    while (pSystem->m_pModuleName[0] != '\0') {
      ...
      strNewPath.Format("%s%s%s", m_sBinaryPath.Get(), CORRECT_PATH_SEPARATOR_S,
                        pSystem->m_pModuleName);
...
CGCInterface::CGCInterface()
    : m_pGCHost(NULL), m_pGC(NULL), m_pGCDirProcess(NULL),
      m_nAppID(k_uAppIdInvalid), m_eUniverse(k_EUniverseInvalid),
      m_bDevMode(false), m_ullGID(0), m_bLogCaptureEnabled(false),
      m_nVersion(0), m_hParentProcess(NULL) {}
...
//-----------------------------------------------------------------------------
IGameCoordinator *CGCInterface::GetGC() { return m_pGC; }

//-----------------------------------------------------------------------------
// Purpose: Returns true if the GC is running in a dev environment
//-----------------------------------------------------------------------------
bool CGCInterface::BIsDevMode() const { return m_bDevMode; }

//-----------------------------------------------------------------------------
// Purpose: Gets the GC's appID
//-----------------------------------------------------------------------------
AppId_t CGCInterface::GetAppID() const { return m_nAppID; }

//-----------------------------------------------------------------------------
// Purpose: Gets the directory gc.dll is running in
//-----------------------------------------------------------------------------
const char *CGCInterface::GetGCDLLPath() const { return m_sGCDLLPath; }
...
bool CGCInterface::BReadConfigDirectory(KeyValuesAD &configValues) {
  ...
  case k_EUniversePublic:
    pchBaseConfigName = "gcconfig_public.vdf";
    break;
  case k_EUniverseBeta:
    pchBaseConfigName = "gcconfig_beta.vdf";
    break;
  case k_EUniverseInternal:
    pchBaseConfigName = "gcconfig_internal.vdf";
    break;
  case k_EUniverseDev:
    pchBaseConfigName = "gcconfig_dev.vdf";
    break;
  }

  if (!pchBaseConfigName ||
      !configValues->LoadFromFile(g_pFullFileSystem, pchBaseConfigName,
                                  "CONFIG")) {
    GCSDK::EmitError(SPEW_GC, "Unable to read config file: %s. Aborting.\n",
                     pchBaseConfigName ? pchBaseConfigName
                                       : "unknown universe specified");
    return false;
  }

  // load up our directory
  if (!GDirectory()->BInit(configValues->FindKey("directory"))) {
    GCSDK::EmitError(SPEW_GC,
                     "Unable to find 'directory' key within config file %s.\n",
                     pchBaseConfigName);
    return false;
  }
...
bool CGCInterface::BReadConvars(KeyValuesAD &configValues) {
  ...
  // we can't load more if we don't have a directory as we don't know our GC
  // type
  if (!m_pGCDirProcess) {
    AssertMsg(
        false,
        "Attempted to read console variables without any GC type specified");
    return false;
  }

  // get convars for this specific configuration name
  InitConVars(configValues->FindKey(
      CFmtStr("%s-process-convars", m_pGCDirProcess->GetName())));

  // now load the GC type specific convars. Note that these can stomp, so we
  // must do all of them in sequence
  for (uint32 nInstance = 0;
       nInstance < m_pGCDirProcess->GetTypeInstanceCount(); nInstance++) {
    const char *pszTypeName = GDirectory()->GetNameForGCType(
        m_pGCDirProcess->GetTypeInstance(nInstance)->GetType());
  ...
  if (m_pGCDirProcess->GetConfig()) {
    const char *pszAdditionalConvars =
        m_pGCDirProcess->GetConfig()->GetString("convars", NULL);
    if (pszAdditionalConvars) {
      // now load the convars that are specific to this instance
      InitConVars(
          configValues->FindKey(CFmtStr("%s-convars", pszAdditionalConvars)));
    }
  }
  ...
  if (k_EUniverseDev != GetUniverse()) {
    ...
    if (pkvSavedConvars->LoadFromFile(
            g_pFullFileSystem,
            CFmtStr("%u_%s_%s_savedconvars.vdf", GetAppID(),
                    m_pGCDirProcess->GetName(),
                    PchNameFromEUniverse(GetUniverse())),
            "CONFIG")) {
      InitConVars(pkvSavedConvars);
    } else {
      EmitInfo(
          SPEW_GC, SPEW_ALWAYS, LOG_ALWAYS,
          "Unable to read saved convars file. Continuing with defaults.\n");
    }
  }
...
void CGCInterface::InitConVars(KeyValues *pkvConvars) {
  ...
  FOR_EACH_VALUE(pkvConvars, pkvVar) {
    if (!pkvVar->GetString()) {
      EmitWarning(SPEW_CONSOLE, SPEW_ALWAYS,
                  "variable %s missing value, skipping\n", pkvVar->GetName());
    }

    ConVar *pVar = NULL;
    const char *pchSuffix = V_strrchr(pkvVar->GetName(), '_');
    if (NULL != pchSuffix &&
        0 == V_strcmp(pchSuffix, CFmtStr("_%u", GetAppID()))) {
    ...
    if (!pVar) {
      EmitWarning(SPEW_CONSOLE, SPEW_ALWAYS,
                  "config file references unknown convar %s\n",
                  pkvVar->GetName());
    } else {
...
bool CGCInterface::BSaveConvars() {
  ...
  return pkvConvars->SaveToFile(g_pFullFileSystem,
                                CFmtStr("%u_%s_%s_savedconvars.vdf", GetAppID(),
                                        m_pGCDirProcess->GetName(),
                                        PchNameFromEUniverse(GetUniverse())),
                                "CONFIG");
...
//-----------------------------------------------------------------------------
void CGCInterface::RecordAssert(const char *pchFile, int nLine,
                                const char *pchMessage,
                                bool *pbShouldWriteMinidump) {
  ...
  if (!pAssert) {
    ...
    // also, remove any newlines from the asserts. The default assert inserts
    // them and this creates problems for a lot of the exporting of the data
    // from SQL into Excel
  ...
  // remove any recorded asserts that are older than our window, so that we can
  // record new asserts
  int nStale = 0;
  CUtlVector<RTime32> &vecTimes = pAssert->m_vRecordTimes;
  const RTime32 nStaleTime =
      CRTime::RTime32TimeCur() - (uint32)cv_assert_minidump_window.GetInt();
  while ((nStale < vecTimes.Count()) && (vecTimes[nStale] < nStaleTime)) {
    nStale++;
  }
  vecTimes.RemoveMultipleFromHead(nStale);

  // see if we have room in how many asserts we want to track, if so, we want to
  // record this assert
  if ((vecTimes.Count() < cv_assert_max_minidumps_in_window.GetInt()) ||
      (CDisableAssertRateLimit::s_nDisabledCount > 0)) {
...
// flag indicating whether or not we should force a crash if we encounter an
// exit
static bool g_bCrashIfExitDetected = false;

// callback handler registered to force a crash on exit conditions so we can
// track when/why the GC ever exits
...
//	creates it
//-----------------------------------------------------------------------------
bool CGCInterface::BAsyncInit(uint32 unAppID, const char *pchDebugName,
                              int iGCIndex, IGameCoordinatorHost *pHost) {
  // called to handle registration of exception handlers so that we will always
  // crash rather than an unexpected termination
  ...
  if (pHost->GetUniverse() != k_EUniverseDev) {
    pHost->EmitSpew(
        SPEW_GC.GetName(), SPEW_ERROR, SPEW_ALWAYS, LOG_ALWAYS,
  ...
  pHost->EmitSpew(SPEW_GC.GetName(), SPEW_ERROR, SPEW_ALWAYS, LOG_ALWAYS,
                  CFmtStr("Initializing %d bit GC, Dir Index %d, PID:%u\n",
                          (uint32)(sizeof(void *) * 8), iGCIndex,
                          GetCurrentProcessId())
                      .Access());
  pHost->EmitSpew(
      SPEW_GC.GetName(), SPEW_ERROR, SPEW_ALWAYS, LOG_ALWAYS,
      CFmtStr1024("Command Line: %s\n", Plat_GetCommandLine()).Access());
  ...
  // open our a handle to our parent
  {
    ...
    if (nParentPID == 0) {
      pHost->EmitSpew(
          SPEW_GC.GetName(), SPEW_ERROR, SPEW_ALWAYS, LOG_ALWAYS,
          "Parent process ID was not specified via -parentpid, unable to get "
          "information about the launching process\n");
    } else {
      m_hParentProcess = OpenProcess(
          PROCESS_QUERY_INFORMATION | PROCESS_VM_READ, FALSE, nParentPID);
      if (!m_hParentProcess) {
        pHost->EmitSpew(
            SPEW_GC.GetName(), SPEW_ERROR, SPEW_ALWAYS, LOG_ALWAYS,
            "Unable to open the parent process with read access. Unable to get "
            "information about the launching process\n");
      }
    }
  }
  ...
  if (s_bInitCalled) {
    pHost->EmitSpew(SPEW_GC.GetName(), SPEW_ERROR, SPEW_ALWAYS, LOG_ALWAYS,
                    "BInit called twice on the game IGameCoordinator");
    return false;
  }
  s_bInitCalled = true;
  ...
  if (::GetModuleHandleExA(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS |
                           ...
                           (LPCSTR)&g_GCInterface, &hModuleGC)) {
  ...
  if (Q_stristr(rgchGCModulePath, "bin\\gc\\x64") != NULL) {
    Q_MakeAbsolutePath(sContentPath.Access(), FMTSTR_STD_LEN, "..\\..\\..",
                       rgchGCModulePath);
    Q_MakeAbsolutePath(sBinaryPath.Access(), FMTSTR_STD_LEN,
    ...
    Q_MakeAbsolutePath(sContentPath.Access(), FMTSTR_STD_LEN, "..\\..",
                       rgchGCModulePath);
    Q_MakeAbsolutePath(sBinaryPath.Access(), FMTSTR_STD_LEN, "..\\..\\..\\bin",
                       rgchGCModulePath);
    m_bDevMode = true;
    m_sDevBinaryName = rgchGCModuleFile;
  } else {
    // launch through standard GC, so try and extract the version from our path
    // (not a great solution, should extend interface so that the GCH provides
    // us with the version it expects). The format is ....\vNNN\ so try and
    // extract that
  ...
  // Select folder and prefix for dumps (using just the dir index for now, but
  // update this later once we have the name
  Tier0GenericMiniDumpHandler_SetFilenamePrefix(
      CFmtStr("dumps\\gc%d_idx%d", unAppID, iGCIndex));
  ...
  m_ullGID |= (uint64)CRTime::RTime32TimeCur()
              << 24; // 32 bits of UTC time in seconds
  // 24 bits/second of incremental counter space

  // This system assumes there are less than 256 GCs. Make sure of that
  AssertMsg(
      iGCIndex >= 0 && iGCIndex < 256,
      "iGCIndex out of range. There can only be 256 GC processes for an app");
  ...
  g_pFullFileSystem->AddSearchPath(rgchGCModulePath,
                                   "CONFIG"); // config files go with gc.dll

  // load the config file first thing so that we can use it for all the other
  // startup code
  ...
  CDirectory::GCFactory_t pfnFactory =
      GDirectory()->GetFactoryForProcessType(m_pGCDirProcess->GetProcessType());
  if (NULL == pfnFactory) {
    GCSDK::EmitError(
        SPEW_CONSOLE,
        "Failed to start GC for index %d (type %s). Got a NULL factory "
        "function, likely missing registration for this type\n",
        iGCIndex, m_pGCDirProcess->GetProcessType());
    return false;
  }

  // now that we have more information about which GC we are, update our
  // minidump name to reflect this
  Tier0GenericMiniDumpHandler_SetFilenamePrefix(
      CFmtStr("dumps\\gc%d_%s", unAppID, m_pGCDirProcess->GetName()));

  // now that we know our GC type, we can actually load up our convars (which
  // are dependent on this info)
...
//	processes.
//-----------------------------------------------------------------------------
GID_t CGCInterface::GenerateGID() { return ++m_ullGID; }

// we have the GC index encoded in the high bits when we init the gid, so just
// extract that
...
//-----------------------------------------------------------------------------
// Purpose: Wrappers for GCHost functions that allow GCInterface to hook the
// calls
//-----------------------------------------------------------------------------
bool CGCInterface::BProcessSystemMessage(uint32 unGCSysMsgType,
                                         const void *pubData, uint32 cubData) {
  AssertMsg(unGCSysMsgType != 0,
            "Message sent without an ID. Did you use the wrong constructor?");

  // track this message that we are sending (always just strip off the protobuff
  // flag so it works with all message types)
  g_theMessageList.TallySendMessage(unGCSysMsgType & ~k_EMsgProtoBufFlag,
                                    cubData, eMsgSource_System);
...
//-----------------------------------------------------------------------------
bool CGCInterface::BSendMessageToClient(uint64 ullSteamID, uint32 unMsgType,
                                        const void *pubData, uint32 cubData) {
  AssertMsg(unMsgType != 0,
            "Message sent without an ID. Did you use the wrong constructor?");

  // sanity check on our side that we are sending with a valid steam ID. Useful
  // to catch message failures on the GC side since otherwise it must be caught
  // in the GCH side
  if (ullSteamID == k_steamIDNil.ConvertToUint64()) {
    AssertMsg(false,
              "Message %d sent to invalid steam ID. This message will not be "
              "processed.",
  ...
  // track this message that we are sending (always just strip off the protobuff
  // flag so it works with all message types)
  g_theMessageList.TallySendMessage(unMsgType & ~k_EMsgProtoBufFlag, cubData,
                                    eMsgSource_Client);

  VPROF_BUDGET("GCHost", VPROF_BUDGETGROUP_STEAM);
  {
    VPROF_BUDGET("GCHost - SendMessageToClient", VPROF_BUDGETGROUP_STEAM);
    return m_pGCHost->BSendMessageToClient(ullSteamID, unMsgType, pubData,
                                           cubData);
  }
}

//-----------------------------------------------------------------------------
bool CGCInterface::BSendMessageToGC(int iGCServerIDTarget, uint32 unMsgType,
                                    const void *pubData, uint32 cubData) {
  AssertMsg(unMsgType != 0,
            "Message sent without an ID. Did you use the wrong constructor?");
  ...
  VPROF_BUDGET("GCHost", VPROF_BUDGETGROUP_STEAM);
  {
    ...
    return m_pGCHost->BSendMessageToGC(iGCServerIDTarget, unMsgType, pubData,
                                       cubData);
  }
}

//-----------------------------------------------------------------------------
void CGCInterface::AddBlockEmitString(const char *pszStr, bool bBlockConsole,
                                      bool bBlockLog) {
...
//-----------------------------------------------------------------------------
void CGCInterface::EmitSpew(const char *pchGroupName, SpewType_t spewType,
                            int iSpewLevel, int iLevelLog, const char *pchMsg) {
  // see if this output is being squelched by going through our blocked string
  FOR_EACH_VEC(m_BlockEmitStrings, nStr) {
    // if our output contains the blocked string, turn off the severity for that
    // level
...
//-----------------------------------------------------------------------------
void CGCInterface::SetShutdownComplete() { m_pGCHost->ShutdownComplete(); }
...
//-----------------------------------------------------------------------------
void CGCInterface::HandleMessageFromClient(uint64 ullSenderID, uint32 unMsgType,
                                           void *pubData, uint32 cubData) {
  if (NULL == pubData || 0 == cubData) {
    EG_ERROR(g_EGMessages,
             "Received invalid message from user %s. MessageID: %u pubData: %p "
             "cubData: %u\n",
             CSteamID(ullSenderID).Render(), unMsgType, pubData, cubData);
  } else if (m_pGC) {
    g_theMessageList.TallyReceiveMessage(unMsgType & ~k_EMsgProtoBufFlag,
                                         cubData, eMsgSource_Client);
    m_pGC->HandleMessageFromClient(ullSenderID, unMsgType, pubData, cubData);
  }
}

//-----------------------------------------------------------------------------
void CGCInterface::HandleMessageFromSystem(uint32 unGCSysMsgType, void *pubData,
                                           uint32 cubData) {
  if (NULL == pubData || 0 == cubData) {
    EG_ERROR(g_EGMessages,
             "Received invalid message from system. MessageID: %u pubData: %p "
             "cubData: %u\n",
             unGCSysMsgType, pubData, cubData);
  } else if (m_pGC) {
    g_theMessageList.TallyReceiveMessage(unGCSysMsgType & ~k_EMsgProtoBufFlag,
                                         cubData, eMsgSource_System);
...
//-----------------------------------------------------------------------------
void CGCInterface::HandleMessageFromGC(int iGCServerIDSender, uint32 unMsgType,
                                       void *pubData, uint32 cubData) {
  if (NULL == pubData || 0 == cubData) {
    EG_ERROR(g_EGMessages,
             "Received invalid message from GC. MessageID: %u pubData: %p "
             "cubData: %u\n",
             iGCServerIDSender, pubData, cubData);
  } else if (m_pGC) {
    g_theMessageList.TallyReceiveMessage(unMsgType & ~k_EMsgProtoBufFlag,
                                         cubData, eMsgSource_GC);
    m_pGC->HandleMessageFromGC(iGCServerIDSender, unMsgType, pubData, cubData);
  }
}

//-----------------------------------------------------------------------------
void CGCInterface::StartLogCapture() { m_bLogCaptureEnabled = true; }

//-----------------------------------------------------------------------------
void CGCInterface::EndLogCapture() { m_bLogCaptureEnabled = false; }
...
//-----------------------------------------------------------------------------
void CGCInterface::ClearLogCapture() { m_vecLogCapture.RemoveAll(); }
...
// TEMP: Disabled on VS2013+ because I didn't know how to fix the linking
// problems
