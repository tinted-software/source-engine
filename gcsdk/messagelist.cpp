...
//			list will assemble them all into a single list when it
//initializes.
//-----------------------------------------------------------------------------
class CMessageListRegistration {
public:
  CMessageListRegistration(MsgInfo_t *pMsgInfo, int cMsgInfo,
                           void *pExtra = NULL);
...
// function called when a message is tallied but not registered properly. Used
// to help consolidate behavior for untracked messages
static void HandleUntrackedMsg(const char *pszMsgOperation, uint32 nMsgID) {
  // for now output this as verbose so that we can clean up the initial spam.
  // Then promote this to warnings in the future
  EG_VERBOSE(g_EGMessages,
             "Found %s message with ID %d that was not properly registered. "
             "Unable to tally information\n",
             pszMsgOperation, nMsgID);
}

CMessageListRegistration::CMessageListRegistration(MsgInfo_t *pMsgInfo,
                                                   int cMsgInfo, void *pExtra)
    : m_pMsgInfo(pMsgInfo), m_cMsgInfo(cMsgInfo) {
  m_pNext = sm_pFirst;
  sm_pFirst = this;
}
...
//-----------------------------------------------------------------------------
void MsgRegistrationFromEnumDescriptor(
    const ::google::protobuf::EnumDescriptor *pEnumDescriptor, int nTypeMask) {
  ...
  for (int i = 0; i < pEnumDescriptor->value_count(); ++i) {
    const ::google::protobuf::EnumValueDescriptor *pEnumValueDescriptor =
        pEnumDescriptor->value(i);
...
bool CMessageList::BInit() {
  ...
  int cMessageInfos = 0;
  for (CMessageListRegistration *pReg = CMessageListRegistration::sm_pFirst;
       pReg != NULL; pReg = pReg->m_pNext) {
...
//-----------------------------------------------------------------------------
void CMessageList::TallySendMessage(MsgType_t eMsgType, uint32 unMsgSize,
                                    uint32 nSourceMask) {
  ...
  TallyMessageInternal(m_vecMsgInfo[nIndex], MsgInfo_t::k_EStatsTypeSent,
                       unMsgSize, nSourceMask);
}

//-----------------------------------------------------------------------------
// Tally the receiving of a message
//-----------------------------------------------------------------------------
void CMessageList::TallyReceiveMessage(MsgType_t eMsgType, uint32 unMsgSize,
                                       uint32 nSourceMask) {
  short nIndex = GetMessageIndex(eMsgType);
  if (nIndex == -1) {
  ...
  TallyMessageInternal(m_vecMsgInfo[nIndex], MsgInfo_t::k_EStatsTypeReceived,
                       unMsgSize, nSourceMask);
}
...
// Tally the receiving of a message
//-----------------------------------------------------------------------------
void CMessageList::TallyMultiplexedMessage(MsgType_t eMsgType, uint32 uSent,
                                           uint32 cRecipients, uint32 uMsgSize,
                                           uint32 nSourceMask) {
  short nIndex = GetMessageIndex(eMsgType);
  if (nIndex == -1) {
  ...
  TallyMessageInternal(msgInfo, MsgInfo_t::k_EStatsTypeMultiplexedSends, uSent,
                       nSourceMask, 1);
  TallyMessageInternal(msgInfo, MsgInfo_t::k_EStatsTypeMultiplexedSendsRaw,
                       uMsgSize * cRecipients, nSourceMask, cRecipients);
}

//-----------------------------------------------------------------------------
// Tally the receiving of a message
//-----------------------------------------------------------------------------
void CMessageList::TallyMessageInternal(MsgInfo_t &msgInfo,
                                        MsgInfo_t::EStatsType eBucket,
                                        uint32 unMsgSize, uint32 nSourceMask,
                                        uint32 cMessages) {
  ...
  msgInfo.stats[MsgInfo_t::k_EStatsGroupGlobal][eBucket].nSourceMask |=
      nSourceMask;
  ...
  msgInfo.stats[MsgInfo_t::k_EStatsGroupWindow][eBucket].nSourceMask |=
      nSourceMask;
  ...
  if (m_bProfiling) {
    ...
    msgInfo.stats[MsgInfo_t::k_EStatsGroupProfile][eBucket].nSourceMask |=
        nSourceMask;
  }
}

// called to obtain the totals for the timing window
const MsgInfo_t::Stats_t &
CMessageList::GetWindowTotal(MsgInfo_t::EStatsType eType) const {
...
void CMessageList::ResetWindow() {
  ...
  FOR_EACH_VEC(m_vecMsgInfo, nIndex) {
    for (uint32 nType = 0; nType < MsgInfo_t::k_EStatsType_Count; nType++) {
      m_vecMsgInfo[nIndex].stats[MsgInfo_t::k_EStatsGroupWindow][nType].nCount =
          0;
      m_vecMsgInfo[nIndex].stats[MsgInfo_t::k_EStatsGroupWindow][nType].uBytes =
          0;
      m_vecMsgInfo[nIndex]
          .stats[MsgInfo_t::k_EStatsGroupWindow][nType]
          .nSourceMask = 0;
    }
  }
...
void CMessageList::EnableProfiling(bool bEnableProfiling) {
  ...
  if (m_bProfiling) {
    ...
    FOR_EACH_VEC(m_vecMsgInfo, nIndex) {
      for (uint32 nType = 0; nType < MsgInfo_t::k_EStatsType_Count; nType++) {
        m_vecMsgInfo[nIndex]
            .stats[MsgInfo_t::k_EStatsGroupProfile][nType]
            .nCount = 0;
        m_vecMsgInfo[nIndex]
            .stats[MsgInfo_t::k_EStatsGroupProfile][nType]
            .uBytes = 0;
    ...
    m_ulProfileMicrosecs = m_sCollectTime[MsgInfo_t::k_EStatsGroupProfile]
                               .CServerMicroSecsPassed();
  }
}

uint64 CMessageList::GetGroupDuration(MsgInfo_t::EStatsGroup eGroup) const {
  // handle the special case of it being a profile, where if we are no longer
  // profiling, we want to use our cached value
...
//-----------------------------------------------------------------------------
void CMessageList::PrintStats(bool bShowAll, bool bSortByFrequency,
                              MsgInfo_t::EStatsGroup eGroup,
                              MsgInfo_t::EStatsType eType,
                              uint32 nSourceMask) const {
  ...
  FOR_EACH_VEC(m_vecMsgInfo, n) {
    // see if we are looking for a particular source, and if so, if this has
    // that source set
    if (nSourceMask &&
        ((m_vecMsgInfo[n].stats[eGroup][eType].nSourceMask & nSourceMask) == 0))
      continue;
  ...
  EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS, "%s",
           "EMsg  MessageName                                         Count    "
           "  %          Bytes      %     MsgPS    Avg/Msg       KBPS\n");
  EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS, "%s",
           "----- ---------------------------------------------- ---------- "
           "------ -------------- ------ --------- ---------- ----------\n");

  for (uint16 iValue = mapValueToMsg.LastInorder();
       iValue != mapValueToMsg.InvalidIndex();
       iValue = mapValueToMsg.PrevInorder(iValue)) {
    ...
    EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
             "%5u %-46s %10u %5.1f%% %14llu %5.1f%% %9.1f %10u %10.2f\n",
             m_vecMsgInfo[n].eMsg, m_vecMsgInfo[n].pchMsgName, nCount,
             flCountPerc, uBytes, flBytesPerc, fMsgPS, uAvgMsg, fKBPS);
  }
...
// print statistics bout each message we handle
//-----------------------------------------------------------------------------
void CMessageList::PrintMultiplexStats(MsgInfo_t::EStatsGroup eGroup,
                                       bool bSortByFrequency,
                                       uint32 nSourceMask) const {
  // Figure out which time value we should use for rate calcs
  uint64 ulMicroseconds = GetGroupDuration(eGroup);
  ...
  const MsgInfo_t::EStatsType eMultiType =
      MsgInfo_t::k_EStatsTypeMultiplexedSends;
  const MsgInfo_t::EStatsType eRawType =
      MsgInfo_t::k_EStatsTypeMultiplexedSendsRaw;
  ...
  FOR_EACH_VEC(m_vecMsgInfo, n) {
    ...
    // that source set
    if (nSourceMask &&
        (msgInfo.stats[eGroup][eMultiType].nSourceMask & nSourceMask) == 0)
      continue;
  ...
  for (uint16 iValue = mapValueToMsg.LastInorder();
    ...
    int32 nMessagesSaved = msgInfo.stats[eGroup][eRawType].nCount -
                           msgInfo.stats[eGroup][eMultiType].nCount;
    int64 uBytesSaved = msgInfo.stats[eGroup][eRawType].uBytes -
                        msgInfo.stats[eGroup][eMultiType].uBytes;
    ...
    EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
             "%5u %-46s %10u %5.1f%% %14llu %5.1f%% %10u %10.2f %10d %5.1f%% "
             "%14lld %5.1f%%\n",
             msgInfo.eMsg, msgInfo.pchMsgName, nCount, flCountPerc,
             uBytes / 1024, flBytesPerc, uAvgMsg, fKBPS, nMessagesSaved,
             flMessagesSavedPerc, uBytesSaved / 1024, flBytesSavedPerc);
  }
  ...
  int32 nTotalMessagesSaved =
      rgTotals[eRawType].nCount - rgTotals[eMultiType].nCount;
  int64 nTotalBytesSaved =
      rgTotals[eRawType].uBytes - rgTotals[eMultiType].uBytes;

  int32 nTotalMessagesIfNotMultiplex =
      unTotalMessages - rgTotals[eMultiType].nCount + rgTotals[eRawType].nCount;
  int64 uTotalBytesIfNotMultiplex =
      unTotalBytes - rgTotals[eMultiType].uBytes + rgTotals[eRawType].uBytes;

  float flTotalMessagesSavedPct = 0.0f;
  if (nTotalMessagesIfNotMultiplex > 0)
    flTotalMessagesSavedPct =
        nTotalMessagesSaved * 100.0f / nTotalMessagesIfNotMultiplex;

  float flTotalBytesSavedPct = 0.0f;
  if (uTotalBytesIfNotMultiplex > 0)
    flTotalBytesSavedPct =
        nTotalBytesSaved * 100.0f / uTotalBytesIfNotMultiplex;
  ...
  EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
           "Totals                                               %10u %5.1f%% "
           "%14llu %5.1f%% %10u %10.2f %10d %5.1f%% %14lld %5.1f%%\n",
           unTotalMessages, flTotalMessagesPct, unTotalBytes / 1024,
           flTotalBytesPct, uAvgMsgTotal, fKBPSTotal, nTotalMessagesSaved,
           flTotalMessagesSavedPct, nTotalBytesSaved / 1024,
           flTotalBytesSavedPct);
}
...
void CMessageList::AssureBucket(int nBucket) {
  ...
  if (m_vecMessageInfoBuckets[nBucket] == NULL) {
    ...
    m_vecMessageInfoBuckets[nBucket] =
        (short *)PvAlloc(sizeof(short) * m_kcBucketSize);
...
//-----------------------------------------------------------------------------
// Purpose: Hash an MsgType_t and return the index into m_vecMsgInfo where it
// can be found
...
bool CMessageList::GetMessage(MsgType_t eMsg, const char **ppMsgName,
                              int nTypeMask) {
