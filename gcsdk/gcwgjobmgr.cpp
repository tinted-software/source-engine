...
ENUMSTRINGS_START(EGCWebApiPrivilege){k_EGCWebApiPriv_None, "None"},
    {k_EGCWebApiPriv_Account, "Account"},
    {k_EGCWebApiPriv_Approved, "Approved"},
    {k_EGCWebApiPriv_Session, "Session"}, {k_EGCWebApiPriv_Support, "Support"},
    {k_EGCWebApiPriv_Admin, "Admin"}, {k_EGCWebApiPriv_EditApp, "EditApp"},
...
bool CGCWGJobMgr::BHandleMsg(IMsgNetPacket *pNetPacket) {
  ...
  AssertMsg(
      msg.GetEMsg() == k_EGCMsgWGRequest,
      "GCWGJobMgr asked to route a message, but it is not a MsgWGRequest");
  ...
  CGCWGJob *job =
      (CGCWGJob *)(*((GMapGCWGJobCreationFuncs())[iJobType]))(GGCBase(), NULL);
  Assert(job);
...
bool CGCWGJobMgr::BVerifyPrivileges(const CGCMsg<MsgGCWGRequest_t> &msg,
                                    const WebApiFunc_t *pFunc) {
  if (msg.Body().m_unPrivilege != (uint32)pFunc->m_eRequiredPrivilege) {
    SendErrorMessage(
        msg,
        CFmtStr("Privilege mismatch in %s gc call. Expected: %s, actual: %s",
                pFunc->m_pchRequestName,
                PchNameFromEGCWebApiPrivilege(pFunc->m_eRequiredPrivilege),
                PchNameFromEGCWebApiPrivilege(
                    (EGCWebApiPrivilege)msg.Body().m_unPrivilege)),
...
//				be routed to the right place and eventually back
//to the right 				browser. 			pchErrorMsg - The message to display 			nResult - The error
//code to return
//-----------------------------------------------------------------------------
void CGCWGJobMgr::SendErrorMessage(const CGCMsg<MsgGCWGRequest_t> &msg,
...
//-----------------------------------------------------------------------------
void CGCWGJobMgr::SetErrorMessage(KeyValues *pkvErr, const char *pchErrorMsg,
                                  int32 nResult) {
  pkvErr->SetString("error", pchErrorMsg);
  pkvErr->SetInt("success", nResult);
}
...
// Inputs:	msg - The message we're responding to. This causes the error to
//				be routed to the right place and eventually back
//to the right 				browser. 			pkvResponse - The KeyValues containing the response data
//-----------------------------------------------------------------------------
void CGCWGJobMgr::SendResponse(const CGCMsg<MsgGCWGRequest_t> &msg,
...
//-----------------------------------------------------------------------------
void CGCWGJobMgr::RegisterWGJob(const WebApiFunc_t *pWGJobType,
                                const JobType_t *pJobCreationFunc) {
  const char *pchRequestName = pWGJobType->m_pchRequestName;
  GMapGCWGJobCreationFuncs().Insert(pchRequestName,
                                    &(pJobCreationFunc->m_pJobFactory));
