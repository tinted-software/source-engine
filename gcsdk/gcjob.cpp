...
GCConVar cv_webapi_serialize_threads(
    "webapi_serialize_threads", "1", 0,
    "Switch for serializing webAPI responses on threads");
static GCConVar
    webapi_account_tracking("webapi_account_tracking", "1",
                            "Controls whether or not account tracking stats "
                            "are collected for web api usage");
static GCConVar webapi_kill_switch(
    "webapi_kill_switch", "0",
    "When set to zero this will block no web api calls, when set to 1 this "
    "will block all web api except those sent by steam. To block steam, use "
    "the webapi_enable_steam_<priority> controls");
static GCConVar webapi_kill_switch_error_response(
    "webapi_kill_switch_error_response", "1",
    "Determines if a response should be sent when the kill switch kills a web "
    "api request");
static GCConVar webapi_rate_limit_calls_per_min(
    "webapi_rate_limit_calls_per_min", "600",
    "Determines how many messages can be sent from an account via the web api "
    "per minute. <0 disables this limiting");
static GCConVar webapi_rate_limit_mb_per_min(
    "webapi_rate_limit_mb_per_min", "20",
    "Determines how many megabytes of data can be sent to an account via the "
    "web api per minute. <0 disables this limiting");
static GCConVar webapi_elevated_rate_limit_calls_per_min(
    "webapi_elevated_rate_limit_calls_per_min", "-1",
    "Determines how many messages can be sent from an account via the web api "
    "per minute for elevated accounts. <0 disables this limiting");
static GCConVar webapi_elevated_rate_limit_mb_per_min(
    "webapi_elevated_rate_limit_mb_per_min", "-1",
    "Determines how many megabytes of data can be sent to an account via the "
    "web api per minute for elevated accounts. <0 disables this limiting");
static GCConVar webapi_ip_rate_limit(
    "webapi_ip_rate_limit", "1",
    "Controls whether or not we rate limit based upon IPs or just accounts");
...
public:
  // called when a web api request is made to track the call. A return value of
  // true indicates that it should be allowed, false indicates it should be
  // blocked
  bool TrackUser(AccountID_t nID, uint32 nIP);
  // called once the size of the response is known and will track bandwidth and
  // caller attributed to the provided function
  void TrackFunction(AccountID_t nID, uint32 nIP, const char *pszFunction,
  ...
  void DumpTotalCallers(EDumpCaller eFilter,
                        const char *pszFunctionFilter = NULL) const;
  void DumpTotalIPs(EDumpCaller eFilter,
                    const char *pszFunctionFilter = NULL) const;
  ...
  // for steam level requests, we have a priority provided, and to track stats
  // separately we break them up to different account IDs instead of just zero
...
private:
  ...
  struct SCallerStats {
    SCallerStats()
        : m_nBlockedCalls(0), m_eLevel(eWebAPIAccountLevel_RateLimited) {
      ResetRateInterval(0);
    }
    ...
    // the most recent rate interval that we received a message from the user
    // (used to expire old counts)
    RTime32 m_nRateIntervalStartTime;
    // how many messages have been sent within this rate interval (used for rate
    // limiting)
  ...
  struct SFunctionStats {
    // track the number of calls and bandwidth. The profile versions are
    // separate and used for displaying profiles over a window
    ...
    // a map of who has called us, which consists of the caller account and IP
    // as the key
    struct SCaller {
      bool operator==(const SCaller &rhs) const {
        return m_nAccountID == rhs.m_nAccountID && m_nIP == rhs.m_nIP;
      }
  ...
  // a structure used to simplify reporting so a vector can just be built of
  // these, and then provided to the report function which will handle sorting
  // it and displaying
  struct SReportRow {
    SReportRow(const char *pszFunction, uint32 nCalls, uint64 nSize)
        : m_pszFunction(pszFunction), m_nCalls(nCalls), m_nSize(nSize) {}
  ...
  // called to print a report of the provided report rows as either an ID list
  // or a function list. This will re-sort the provided vector
  ...
  CUtlHashMapLarge<const char *, SSteamServer *, CaseSensitiveStrEquals,
                   MurmurHash3ConstCharPtr>
      m_SteamServers;
...
RTime32 CWebAPIAccountTracker::GetRateIntervalStart(AccountID_t nCaller) const {
  // we shift the time by the account ID so that all users don't wrap at the
  // same time which can cause a temporary surge in web API requests
  RTime32 curTime = CRTime::RTime32TimeCur() + (nCaller % knRateIntervalTimeS);
  return curTime - (curTime % knRateIntervalTimeS);
}

CWebAPIAccountTracker::SCallerStats *
CWebAPIAccountTracker::CreateAccountUser(AccountID_t nID,
                                         RTime32 nRateIntervalStart) {
...
CWebAPIAccountTracker::SCallerStats *
CWebAPIAccountTracker::CreateIPUser(uint32 nIP, RTime32 nRateIntervalStart) {
...
// determines what the resulting account level access should be based upon the
// access rights of the IP address and the account
static EWebAPIAccountLevel DetermineAccessLevel(EWebAPIAccountLevel eAccount,
                                                EWebAPIAccountLevel eIP) {
  // unrestricted users should always be allowed, regardless of the IP range
  // that they are making requests from, same with unlimited IP addresses
  if ((eAccount == eWebAPIAccountLevel_Unlimited) ||
      (eIP == eWebAPIAccountLevel_Unlimited))
    return eWebAPIAccountLevel_Unlimited;

  // otherwise, if either is blocked, then block
  if ((eAccount == eWebAPIAccountLevel_Blocked) ||
      (eIP == eWebAPIAccountLevel_Blocked))
    return eWebAPIAccountLevel_Blocked;

  // now we are dealing with default case versus elevated. Elevated wins over
  // default
  if ((eAccount == eWebAPIAccountLevel_Elevated) ||
      (eIP == eWebAPIAccountLevel_Elevated))
...
bool CWebAPIAccountTracker::TrackUser(AccountID_t nID, uint32 nIP) {
  ...
  // first off update their aggregate caller stats
  {
    ...
    // determine what our policy should be based upon the access level of the IP
    // and the user
    EWebAPIAccountLevel eAccessLevel =
        DetermineAccessLevel(pAccountCaller->m_eLevel, pIPCaller->m_eLevel);
    ...
    if ((eAccessLevel == eWebAPIAccountLevel_RateLimited) ||
        (eAccessLevel == eWebAPIAccountLevel_Elevated)) {
      // determine the rate we want to limit
      int32 nCallsPerMin =
          (eAccessLevel == eWebAPIAccountLevel_RateLimited)
              ? webapi_rate_limit_calls_per_min.GetInt()
              : webapi_elevated_rate_limit_calls_per_min.GetInt();
      int32 nBytesPerMin =
          ((eAccessLevel == eWebAPIAccountLevel_RateLimited)
               ? webapi_rate_limit_mb_per_min.GetInt()
               : webapi_elevated_rate_limit_mb_per_min.GetInt()) *
          1024 * 1024;
      ...
      if ((eAccessLevel == eWebAPIAccountLevel_RateLimited)) {
        ...
        // see if we are being limited based upon call rate limiting (tracking
        // based upon ip and account) Note that we don't return until we've
        // dones stat tracking for both so the reports are accurate and capture
        // it at both levels
        if ((nCallsPerMin >= 0 &&
             pAccountCaller->m_nRateIntervalCalls >= (uint32)nCallsPerMin) ||
            (nBytesPerMin >= 0 &&
             pAccountCaller->m_nRateIntervalBytes >= (uint32)nBytesPerMin)) {
          pAccountCaller->m_nBlockedCalls++;
          bAllow = false;
        }
        ...
        if (webapi_ip_rate_limit.GetBool()) {
          if ((nCallsPerMin >= 0 &&
               pIPCaller->m_nRateIntervalCalls >= (uint32)nCallsPerMin) ||
              (nBytesPerMin >= 0 &&
               pIPCaller->m_nRateIntervalBytes >= (uint32)nBytesPerMin)) {
            pIPCaller->m_nBlockedCalls++;
            bAllow = false;
          }
        }
...
void CWebAPIAccountTracker::TrackFunction(AccountID_t nID, uint32 nIP,
  ...
  // now update the function specific stats
  {
    ...
    function.m_nProfileMaxBytes =
        MAX(function.m_nProfileMaxBytes, nResponseSize);
...
void CWebAPIAccountTracker::SetAccountPermission(AccountID_t nID,
                                                 EWebAPIAccountLevel eLevel) {
...
void CWebAPIAccountTracker::ResetStats() {
  FOR_EACH_MAP_FAST(m_AccountCallers, nCaller) {
    m_AccountCallers[nCaller].ResetRateInterval(
        GetRateIntervalStart(m_AccountCallers.Key(nCaller)));
    m_AccountCallers[nCaller].m_nBlockedCalls = 0;
  }
  FOR_EACH_MAP_FAST(m_IPCallers, nCaller) {
    m_IPCallers[nCaller].ResetRateInterval(
        GetRateIntervalStart(m_IPCallers.Key(nCaller)));
...
//-----------------------------------------------------------------------------
// Purpose: Renders the steam ID to a buffer with an admin console link.  NOTE:
// for convenience of
//			calling code, this code returns a pointer to a static
...
static const char *CSteamID_RenderLink(const CSteamID &steamID) {
  ...
  const int k_cBufs = 4; // # of static bufs to use (so people can compose
                         // output with multiple calls to RenderLink() )
  ...
  Q_snprintf(pchBuf, k_cBufLen, "<link cmd=\"steamid64 %llu\">%s</link>",
             steamID.ConvertToUint64(), steamID.Render());
  return pchBuf;
}

//-----------------------------------------------------------------------------
// Purpose: Renders the passed-in steam ID to a buffer with admin console link.
// NOTE: for convenience
//			of calling code, this code returns a pointer to a static
//buffer and is NOT thread-safe.
...
void CWebAPIAccountTracker::DumpCaller(AccountID_t nID) const {
  ...
  FOR_EACH_MAP_FAST(m_Functions, nFunc) {
    ...
    FOR_EACH_MAP_FAST(m_Functions[nFunc]->m_Callers, nCaller) {
      const CWebAPIAccountTracker::SFunctionStats::SCaller &caller =
          m_Functions[nFunc]->m_Callers.Key(nCaller);
      if (caller.m_nAccountID == nID) {
        const CWebAPIAccountTracker::SFunctionStats::SCalls &calls =
            m_Functions[nFunc]->m_Callers[nCaller];
    ...
    if (nFnCalls > 0) {
      vFuncs.AddToTail(
          SReportRow((const char *)m_Functions.Key(nFunc), nFnCalls, nFnBytes));
    }
  ...
  EG_MSG(SPEW_CONSOLE,
         "\tAccess: %u, Total Calls: %u, Blocked calls: %u, Total: %.2fMB, "
         "MB/h: %.2f, Calls/h: %.0f\n",
         caller.m_eLevel, nTotalCalls, caller.m_nBlockedCalls, fTotalMB,
         fMBPerHour, fCallsPerHour);
  // don't let someone accidentally change Steam's access!
  if (nID != 0) {
    if (caller.m_eLevel == eWebAPIAccountLevel_RateLimited) {
      EG_MSG(SPEW_CONSOLE,
             "\t<link cmd=\"webapi_account_set_access %u %d\">[Block "
             "Account]</link>",
             nID, eWebAPIAccountLevel_Blocked);
      EG_MSG(SPEW_CONSOLE,
             "\t<link cmd=\"webapi_account_set_access %u %d\">[Elevate "
             "Account]</link>\n",
             nID, eWebAPIAccountLevel_Elevated);
    } else if (caller.m_eLevel == eWebAPIAccountLevel_Blocked) {
      EG_MSG(SPEW_CONSOLE,
             "\t<link cmd=\"webapi_account_set_access %u %d\">[Unblock "
             "Account]</link>\n",
             nID, eWebAPIAccountLevel_RateLimited);
    } else if (caller.m_eLevel == eWebAPIAccountLevel_Elevated) {
      EG_MSG(SPEW_CONSOLE,
             "\t<link cmd=\"webapi_account_set_access %u %d\">[Demote "
             "Account]</link>\n",
             nID, eWebAPIAccountLevel_RateLimited);
    }
  }
  ...
  // print a report of the IP addresses that they are calling from
  {
    ...
    rp.AddIntColumn("MB", CGCReportPrinter::eSummary_Total,
                    CGCReportPrinter::eIntDisplay_Memory_MB);

    FOR_EACH_MAP_FAST(ipCalls, nIP) {
      rp.StrValue(CFmtStr("%u.%u.%u.%u", iptod(ipCalls.Key(nIP))),
                  CFmtStr("webapi_account_dump_ip %u", ipCalls.Key(nIP)));
...
void CWebAPIAccountTracker::DumpIP(uint32 nIP) const {
    ...
    FOR_EACH_MAP_FAST(m_Functions[nFunc]->m_Callers, nCaller) {
      ...
      if (caller.m_nIP == nIP) {
        ...
        if (nAccountIndex == accountCalls.InvalidIndex()) {
          ...
          GGCBase()->PreloadPersonaName(
              GGCInterface()->ConstructSteamIDForClient(caller.m_nAccountID));
        } else {
  ...
  // print a report of the accounts that they are calling from
  {
    ...
    FOR_EACH_MAP_FAST(accountCalls, nAccount) {
      CSteamID steamID =
          GGCInterface()->ConstructSteamIDForClient(accountCalls.Key(nAccount));
      rp.SteamIDValue(steamID, CFmtStr("webapi_account_dump_caller %u",
                                       steamID.GetAccountID()));
      rp.StrValue(
          GGCBase()->YieldingGetPersonaName(steamID, "[unknown]"),
          CFmtStr("webapi_account_dump_caller %u", steamID.GetAccountID()));
...
void CWebAPIAccountTracker::DumpTotalCallers(
    EDumpCaller eFilter, const char *pszFunctionFilter) const {
  ...
  FOR_EACH_MAP_FAST(m_Functions, nCurrFunction) {
    ...
    if (pszFunctionFilter &&
        (V_stristr(pszFunctionName, pszFunctionFilter) == NULL))
      continue;
    ...
    FOR_EACH_MAP_FAST(function.m_Callers, nCurrCaller) {
      ...
      if (nStatIndex == mapCallers.InvalidIndex()) {
        ...
        GGCBase()->PreloadPersonaName(
            GGCInterface()->ConstructSteamIDForClient(key));
      }

      mapCallers[nStatIndex].m_nFunctions += 1;
      mapCallers[nStatIndex].m_nCalls +=
          function.m_Callers[nCurrCaller].m_nCalls;
      mapCallers[nStatIndex].m_nBytes +=
          function.m_Callers[nCurrCaller].m_nBytes;
    }
  }
  ...
  rp.AddIntColumn("MB/h", CGCReportPrinter::eSummary_Total,
                  CGCReportPrinter::eIntDisplay_Memory_MB);
  ...
  FOR_EACH_MAP_FAST(m_AccountCallers, nCurrCaller) {
    ...
    if ((eFilter == eDumpCaller_Blocked) &&
        (m_AccountCallers[nCurrCaller].m_nBlockedCalls == 0))
      continue;
    if ((eFilter == eDumpCaller_Status) &&
        (m_AccountCallers[nCurrCaller].m_eLevel ==
         eWebAPIAccountLevel_RateLimited))
      continue;
    ...
    const CSteamID steamID(
        GGCInterface()->ConstructSteamIDForClient(accountID));
    rp.StrValue(steamID.Render(),
                CFmtStr("webapi_account_dump_caller %u", accountID));
    ...
    rp.IntValue((pStats && pStats->m_nCalls > 0)
                    ? (pStats->m_nBytes / pStats->m_nCalls) / 1024
                    : 0);
...
void CWebAPIAccountTracker::DumpTotalIPs(EDumpCaller eFilter,
                                         const char *pszFunctionFilter) const {
  // accumulate stats for each unique caller
  ...
  FOR_EACH_MAP_FAST(m_IPCallers, nCurrCaller) {
    // apply filters to our results
    if ((eFilter == eDumpCaller_Blocked) &&
        (m_IPCallers[nCurrCaller].m_nBlockedCalls == 0))
      continue;
    if ((eFilter == eDumpCaller_Status) &&
        (m_IPCallers[nCurrCaller].m_eLevel == eWebAPIAccountLevel_RateLimited))
      continue;
    ...
    rp.StrValue(CFmtStr("%u.%u.%u.%u", iptod(nIP)),
                CFmtStr("webapi_account_dump_ip %u", nIP));
    rp.IntValue((pStats) ? pStats->m_nCalls : 0);
    rp.IntValue((pStats) ? pStats->m_nBytes : 0);
...
void CWebAPIAccountTracker::DumpFunctions() const {
  ...
  FOR_EACH_MAP_FAST(m_Functions, i) {
    vFuncs.AddToTail(SReportRow((const char *)m_Functions.Key(i),
                                m_Functions[i]->m_nTotalCalls,
                                m_Functions[i]->m_nTotalBytes));
  }
  PrintReport(vFuncs);
}
...
void CWebAPIAccountTracker::DumpProfile(bool bAllTime) const {
  ...
  // determine how much time we are covering, and come up with a scale to
  // normalize the values
  uint64 nSampleMicroS = (bAllTime) ? (uint64)GGCBase()->GetGCUpTime() * 1000000
                                    : m_ProfileTime.CServerMicroSecsPassed();
  double fToPS = (nSampleMicroS > 0) ? 1000000.0 / (double)nSampleMicroS : 1.0;

  EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
           "Web API Profile: Sampled %.2f seconds\n",
...
void CWebAPIAccountTracker::PrintReport(const CUtlVector<SReportRow> &vec) {
  ...
  for (int i = 0; i < vec.Count(); i++) {
    rp.StrValue(vec[i].m_pszFunction,
                CFmtStr("webapi_account_dump_function_callers %s",
                        vec[i].m_pszFunction));
...
GC_CON_COMMAND(webapi_account_dump_steam_servers,
               "Dumps the ID listings of the various steam servers encoded in "
               "the IP address of Steam requests") {
  g_WebAPIAccountTracker.DumpSteamServers();
}

GC_CON_COMMAND(webapi_account_dump_callers,
               "Dumps the most frequent callers of web api's for the current "
               "run of the GC") {
  g_WebAPIAccountTracker.DumpTotalCallers(
      CWebAPIAccountTracker::eDumpCaller_Calls);
}

GC_CON_COMMAND(webapi_account_dump_ips,
               "Dumps the most frequent ip callers of web api's for the "
               "current run of the GC") {
  g_WebAPIAccountTracker.DumpTotalIPs(CWebAPIAccountTracker::eDumpCaller_Calls);
}

GC_CON_COMMAND(webapi_account_dump_blocked_callers,
               "Dumps the callers that have been blocked and how many calls "
               "have been blocked") {
  g_WebAPIAccountTracker.DumpTotalCallers(
      CWebAPIAccountTracker::eDumpCaller_Blocked);
}

GC_CON_COMMAND(webapi_account_dump_caller_access,
               "Dumps the access rights of any caller that is not the default "
               "rate limiting") {
  g_WebAPIAccountTracker.DumpTotalCallers(
      CWebAPIAccountTracker::eDumpCaller_Status);
}

GC_CON_COMMAND(webapi_account_dump_functions,
               "Dumps the most frequently called web api functions") {
  g_WebAPIAccountTracker.DumpFunctions();
}

GC_CON_COMMAND_PARAMS(webapi_account_dump_function_callers, 1,
                      "<function name> - Dumps the most frequent callers of "
                      "functions that match the provided substring") {
  g_WebAPIAccountTracker.DumpTotalCallers(
      CWebAPIAccountTracker::eDumpCaller_Calls, args[1]);
}

GC_CON_COMMAND_PARAMS(webapi_account_dump_caller, 1,
                      "<caller account> - Dumps the functions that the "
                      "provided account ID has been calling the most") {
  g_WebAPIAccountTracker.DumpCaller((AccountID_t)V_atoui64(args[1]));
}

GC_CON_COMMAND_PARAMS(webapi_account_dump_ip, 1,
                      "<ip> - Dumps the functions that the provided ip has "
                      "been calling the most") {
  g_WebAPIAccountTracker.DumpIP((AccountID_t)V_atoui64(args[1]));
}

GC_CON_COMMAND(
    webapi_account_reset_stats,
    "Forces a reset of all stats collected for web api account stats") {
  g_WebAPIAccountTracker.ResetStats();
}

// utility class for dumping out the profile results after time has expired
static void DumpWebAPIProfile() { g_WebAPIAccountTracker.DumpProfile(false); }

GC_CON_COMMAND_PARAMS(webapi_profile, 1,
                      "<seconds to profile> Turns on web api profiling for N "
                      "seconds and dumps the results") {
...
GC_CON_COMMAND(webapi_profile_dump,
               "Displays stats collected while web api profiling was enabled") {
  g_WebAPIAccountTracker.DumpProfile(false);
}

GC_CON_COMMAND(webapi_profile_dump_total,
               "Displays stats collected while web api profiling was enabled") {
...
//			pMsgIn - Pointer to the message that will contain the
//response 			eMsg - The type of message the response should be
...
//-----------------------------------------------------------------------------
bool CGCJob::BYldSendMessageAndGetReply(CSteamID &steamIDTarget,
                                        CGCMsgBase &msgOut, uint nTimeoutSec,
                                        CGCMsgBase *pMsgIn, MsgType_t eMsg) {
  IMsgNetPacket *pNetPacket = NULL;

  if (!BYldSendMessageAndGetReply(steamIDTarget, msgOut, nTimeoutSec,
                                  &pNetPacket))
    return false;
...
//			msgOut - The message to send
//			nTimeoutSec - Number of seconds to wait for a response
//			ppNetPackets - Pointer to a IMsgNetPacket pointer which
...
CGCWGJob::CGCWGJob(CGCBase *pGCBase)
    : CGCJob(pGCBase), m_steamID(k_steamIDNil), m_pWebApiFunc(NULL) {}
...
//-----------------------------------------------------------------------------
// Purpose: Receive a k_EMsgWGRequest and manage keyvalues
...
bool CGCWGJob::BYieldingRunGCJob(IMsgNetPacket *pNetPacket) {
  ...
  if (!packer.ReadAsBinary(pkvRequest, m_bufRequest)) {
    ...
    CGCWGJobMgr::SendErrorMessage(
        msg, "Failed to deserialize key values from WG request",
...
bool CGCWGJob::BVerifyParams(const CGCMsg<MsgGCWGRequest_t> &msg,
                             KeyValues *pkvRequest,
                             const WebApiFunc_t *pWebApiFunc) {
  ...
  for (int i = 0; i < Q_ARRAYSIZE(pWebApiFunc->m_rgParams); i++) {
    ...
    if (!pWebApiFunc->m_rgParams[i].m_bOptional &&
        !pkvRequest->FindKey(pWebApiFunc->m_rgParams[i].m_pchParam)) {
      CGCWGJobMgr::SendErrorMessage(
          msg,
          CFmtStr("Error: Missing parameter '%s' from web request '%s'\n",
                  pWebApiFunc->m_rgParams[i].m_pchParam,
                  pWebApiFunc->m_pchRequestName),
          k_EResultInvalidParam);
      EmitWarning(
          SPEW_GC, 2, "Error: Missing parameter '%s' from web request '%s'\n",
...
void CGCWGJob::SetErrorMessage(KeyValues *pkvErr, const char *pchErrorMsg,
                               int32 nResult) {
  CGCWGJobMgr::SetErrorMessage(pkvErr, pchErrorMsg, nResult);
}

//-----------------------------------------------------------------------------
// CGCJobVerifySession - A job that asks steam if a given user is still
// connected
...
//-----------------------------------------------------------------------------
// Called to handle converting a web api response to a serialized result and
// free the object as well on a background thread

class CEmitWebAPIData {
public:
  CEmitWebAPIData(CMsgHttpRequest *pRequest)
      : m_bResult(false), m_Request(pRequest) {}
...
// the worker thread function that is responsible for serializing the response
// from web api values to a text buffer, freeing the web api value tree, and
// serializing the message to a protobuf for direct sending. If this function
// fails (a false response value) the message will not be serialized
static void ThreadedEmitFormattedOutputWrapperAndFreeResponse(
    CWebAPIResponse *pResponse, CEmitWebAPIData *pEmitData,
    EWebAPIOutputFormat eDefaultFormat, size_t unMaxResultSize) {
  // parse the output type that we want the result to be in
  EWebAPIOutputFormat eOutputFormat = eDefaultFormat;
  const char *pszParamOutput =
      pEmitData->m_Request.GetGETParamString("format", NULL);
  if (!pszParamOutput)
  ...
  pEmitData->m_bResult = pResponse->BEmitFormattedOutput(
      eOutputFormat, *(pEmitData->m_Response.GetBodyBuffer()), unMaxResultSize);
  delete pResponse;

  // update the response code on the output (can probably be done elsewhere),
  // but must be done before we pack the message below
  switch (eOutputFormat) {
  case k_EWebAPIOutputFormat_JSON:
    pEmitData->m_Response.SetResponseHeaderValue(
        "content-type", "application/json; charset=UTF-8");
    break;
  case k_EWebAPIOutputFormat_XML:
    pEmitData->m_Response.SetResponseHeaderValue("content-type",
  ...
  // if successful, we can go ahead and convert this all the way into a
  // completely serialized form for sending over the wire
...
static void WebAPIRespondToRequest(const char *pszName, uint32 nSenderIP,
                                   const CHTTPResponse &response,
                                   const CProtoBufMsg<CMsgWebAPIRequest> &msg) {
  VPROF_BUDGET("WebAPI - sending result", VPROF_BUDGETGROUP_STEAM);
  CProtoBufMsg<CMsgHttpResponse> msgResponse(
      k_EGCMsgWebAPIJobRequestHttpResponse, msg);
  ...
  g_WebAPIAccountTracker.TrackFunction(msg.Body().api_key().account_id(),
                                       nSenderIP, pszName,
                                       msgResponse.Body().body().size());
}

// responses to the web api message with an error code
static void WebAPIRespondWithError(const char *pszName, uint32 nSenderIP,
                                   const CProtoBufMsg<CMsgWebAPIRequest> &msg,
...
// parses an IP address out of the provided string. This will be the last IP
// address in the list
static uint32 ParseIPAddrFromForwardHeader(const char *pszHeader) {
  ...
  // if no match, then we just have the ip address, otherwise advance past the
  // comma
  if (!pszStart)
    pszStart = pszHeader;
  else
...
//-----------------------------------------------------------------------------
// Purpose: Receive a k_EMsgWebAPIJobRequest and manage
// serialization/deserialization to web request/response objects
//-----------------------------------------------------------------------------
bool CWebAPIJob::BYieldingRunJobFromMsg(IMsgNetPacket *pNetPacket) {
  ...
  bool bMsgParsedOK = msg.Body().has_api_key() &&
                      ...
                      msg.Body().has_request() && msg.Body().has_version();
  ...
  // if this isn't a system request, try and identify the IP address of the
  // sender so we can rate limit accordingly
  if (nSenderAccountID != 0) {
    const int nNumHeaders = msg.Body().request().headers_size();
    for (int nHeader = 0; nHeader < nNumHeaders; nHeader++) {
      if (strcmp(msg.Body().request().headers(nHeader).name().c_str(),
                 "X-Forwarded-For") != 0)
        continue;

      // this is our IP address
      nSenderIP = ParseIPAddrFromForwardHeader(
          msg.Body().request().headers(nHeader).value().c_str());
    }

    // see if we have the kill switch turned on
    if (webapi_kill_switch.GetBool()) {
      if (webapi_kill_switch_error_response.GetBool())
        WebAPIRespondWithError(GetName(), nSenderIP, msg,
    ...
    //		uint32 nPriority;
    //		nSenderIP = GetSteamRequestIPAddress( msg.Body().request(),
    //nPriority );
    //		//and allow for a kill switch based upon this priority
    //		if( ( ( nPriority == CWebAPIAccountTracker::k_nSteamIP_Low ) &&
    //!webapi_enable_steam_low.GetBool() ) || 			( ( nPriority ==
    //CWebAPIAccountTracker::k_nSteamIP_Normal ) &&
    //!webapi_enable_steam_normal.GetBool() ) || 			( ( nPriority ==
    //CWebAPIAccountTracker::k_nSteamIP_High ) &&
    //!webapi_enable_steam_high.GetBool() ) )
    //		{
    //			if( webapi_kill_switch_error_response.GetBool() )
  ...
  // allocate the data that we'll use to fill out the request and send it to the
  // background thread for work
  CPlainAutoPtr<CEmitWebAPIData> pEmitData(new CEmitWebAPIData(
      const_cast<CMsgHttpRequest *>(&msg.Body().request())));
...

  {
    ...
    if (!BYieldingRunJobFromAPIRequest(msg.Body().interface_name().c_str(),
                                       msg.Body().method_name().c_str(),
                                       msg.Body().version(), &request,
                                       &response, pwebAPIResponse.Get())) {
  ...
  //		{
  //			AssertMsg( false, "Error: WebAPI %s attempting to
  //re-route a web api message to itself (%d)", GetName(), m_nRerouteRequest );
  //		}
  //		else
  //		{
  //			//route to the other GC and discard this message
  //			CProtoBufMsg< CMsgGCMsgWebAPIJobRequestForwardResponse >
  //msgRoute( k_EGCMsgWebAPIJobRequestForwardResponse );
  ...
  response.SetExpirationHeaderDeltaFromNow(
      pwebAPIResponse->GetExpirationSeconds());
  if (pwebAPIResponse->GetLastModified())
    response.SetHeaderTimeValue("last-modified",
                                pwebAPIResponse->GetLastModified());

  // if we aren't allowed to have a message body on this, simply send the result
  // back now
  if (!CHTTPUtil::BStatusCodeAllowsBody(pwebAPIResponse->GetStatusCode())) {
    AssertMsg(pwebAPIResponse->GetRootValue() == NULL,
              "Response HTTP status code %d doesn't allow a body, but one was "
              "present",
              pwebAPIResponse->GetStatusCode());
    // since we didn't have a body to serialize, we need to handle sending just
    // the response
    WebAPIRespondToRequest(GetName(), nSenderIP, response, msg);
    return true;
  }

  // let the job convert the formatting and free our response for us (quite
  // costly). Note that we detach the web API response since this job will free
  // it
  bool bThreadFuncSucceeded = BYieldingWaitForThreadFunc(CreateFunctor(
      ThreadedEmitFormattedOutputWrapperAndFreeResponse,
      pwebAPIResponse.Detach(), pEmitData.Get(), m_eDefaultOutputFormat,
      (size_t)cv_webapi_result_size_limit.GetInt()));

  // if we called the function successfully and had a valid result, just send
  // back the preserialized body
  if (bThreadFuncSucceeded && pEmitData->m_bResult) {
    m_pGC->BReplyToMessageWithPreSerializedBody(
        k_EGCMsgWebAPIJobRequestHttpResponse, msg,
        (const byte *)pEmitData->m_sSerializedResponse.c_str(),
        pEmitData->m_sSerializedResponse.size());
    g_WebAPIAccountTracker.TrackFunction(
        nSenderAccountID, nSenderIP, GetName(),
        pEmitData->m_sSerializedResponse.size());
  } else {
    // we failed to generate a response, see if we ran out of space
    if (response.GetBodyBuffer()->TellMaxPut() >
        cv_webapi_result_size_limit.GetInt()) {
      // !FIXME! DOTAMERGE
      // CGCAlertInfo alert( "WebAPIResponseSize", "WebAPI request %s failed to
      // emit because it exceeded %d characters", request.GetURL(),
      // cv_webapi_result_size_limit.GetInt() );
      //
      ...
      //		{
      //			alert.AddExtendedInfoLine( "%s=%s",
      //request.GetGETParamName( nParam ), request.GetGETParamValue( nParam ) );
      //		}
      //	}
      //	break;
      ...
      //		{
      //			alert.AddExtendedInfoLine( "%s=%s",
      //request.GetPOSTParamName( nParam ), request.GetPOSTParamValue( nParam )
      //);
      //		}
      //	}
      //	break;
      ...
      GGCBase()->PostAlert(k_EAlertTypeInfo, false,
                           CFmtStr("WebAPI request %s failed to emit because "
                                   "it exceeded %d characters",
                                   request.GetURL(),
                                   cv_webapi_result_size_limit.GetInt()));
    } else {
      EG_WARNING(SPEW_GC,
                 "WebAPI %s - request %s failed to emit for unknown reason\n",
...
void CWebAPIJob::AddLocalizedString(CWebAPIValues *pOutDefn,
                                    const char *pchFieldName,
                                    const char *pchKeyName, ELanguage eLang,
                                    bool bReturnTokenIfNotFound) {
  ...
  if (eLang == k_Lang_None) {
    ...
    pchValue =
        GGCBase()->LocalizeToken(pchKeyName, eLang, bReturnTokenIfNotFound);
  }
...
//-----------------------------------------------------------------------------
/*static*/ void CWebAPIJob::ThreadedEmitFormattedOutputWrapper(
    CWebAPIResponse *pResponse, EWebAPIOutputFormat eFormat,
    CUtlBuffer *poutputBuffer, size_t unMaxResultSize, bool *pbResult) {
  *pbResult =
      pResponse->BEmitFormattedOutput(eFormat, *poutputBuffer, unMaxResultSize);
}
