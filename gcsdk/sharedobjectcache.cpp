...
#ifdef GC
static GCConVar add_object_clean_do_has_element(
    "add_object_clean_do_has_element", "0", 0,
    "Enables AddObjectClean() checking that the cache doesn't already have "
    "this pointer");
#endif
...
CSharedObjectTypeCache::~CSharedObjectTypeCache() {
  for (int i = 0; i < m_vecObjects.Count(); i++) {
    ...
    if (pObj->BShouldDeleteByCache()) {
      ...
      --pObj->m_nRefCount;
      AssertMsg1(pObj->m_nRefCount == 0,
                 "Destroying shared object %s that's still in use!",
                 pObj->GetDebugString().String());
...
#ifdef GC
#if ENABLE_SO_CONSTRUCT_DESTRUCT_PARANOIA
  AssertMsg1(pObject->m_nRefCount >= 0,
             "AddObjectInternal(): Invalid ref count for shared object %s",
...
//----------------------------------------------------------------------------
CSharedObject *
CSharedObjectTypeCache::RemoveObject(const CSharedObject &soIndex) {
  Assert(m_nTypeID == soIndex.GetTypeID()); // This is probably harmless, but
...
#ifdef GC
#if ENABLE_SO_CONSTRUCT_DESTRUCT_PARANOIA
  AssertMsg1(pObj->m_nRefCount > 0, "Invalid ref count for shared object %s",
             pObj->GetDebugString().String());
  --pObj->m_nRefCount;
#endif // ENABLE_SO_CONSTRUCT_DESTRUCT_PARANOIA
#endif // GC
...
void CSharedObjectTypeCache::DestroyAllObjects() {
  for (int i = 0; i < m_vecObjects.Count(); i++) {
    ...
    if (m_vecObjects[i]->BShouldDeleteByCache()) {
      ...
      AssertMsg1(m_vecObjects[i]->m_nRefCount == 0,
                 "Destroying shared object %s that's still in use!",
                 m_vecObjects[i]->GetDebugString().String());
#endif // ENABLE_SO_CONSTRUCT_DESTRUCT_PARANOIA
...
//----------------------------------------------------------------------------
CSharedObject *
CSharedObjectTypeCache::FindSharedObject(const CSharedObject &soIndex) {
  int nIndex = FindSharedObjectIndex(soIndex);
  if (m_vecObjects.IsValidIndex(nIndex))
...
//			object on its index fields.
//----------------------------------------------------------------------------
int CSharedObjectTypeCache::FindSharedObjectIndex(
...
void CSharedObjectTypeCache::Dump() const {
  EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
           ...
           m_vecObjects.Count());
  FOR_EACH_VEC(m_vecObjects, nObj) { m_vecObjects[nObj]->Dump(); }
}
...
void CSharedObjectTypeCache::Validate(CValidator &validator,
                                      const char *pchName) {
  VALIDATE_SCOPE();
...
CSharedObjectCache::CSharedObjectCache()
    : m_mapObjects(DefLessFunc(int)), m_ulVersion(0) {}

//----------------------------------------------------------------------------
// Purpose: Destructor
//----------------------------------------------------------------------------
CSharedObjectCache::~CSharedObjectCache() {
  FOR_EACH_MAP(m_mapObjects, nTypeIndex) { delete m_mapObjects[nTypeIndex]; }
  m_mapObjects.Purge();
}
...
CSharedObjectCache::FindBaseTypeCache(int nClassID) const {
...
//			cache and returning it if one didn't previously exist.
//Never intended 			to return NULL.
//----------------------------------------------------------------------------
...
//----------------------------------------------------------------------------
CSharedObject *
CSharedObjectCache::FindSharedObject(const CSharedObject &soIndex) {
  CSharedObjectTypeCache *pTypeCache = FindBaseTypeCache(soIndex.GetTypeID());
  if (pTypeCache)
...
void CSharedObjectCache::Dump() const {
  EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
           ...
           m_mapObjects.Count());
  FOR_EACH_MAP(m_mapObjects, nTypeIndex) { m_mapObjects[nTypeIndex]->Dump(); }
}
