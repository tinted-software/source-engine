...
// Purpose: Constructor
//----------------------------------------------------------------------------
CBufferPool::CBufferPool(const char *pchName, const GCConVar &cvMaxSizeMB,
                         const GCConVar &cvInitBufferSize, int nFlags)
    : m_nBuffersInUse(0), m_nBuffersTotal(0), m_nHighWatermark(0), m_cubFree(0),
      m_sName(pchName), m_cvMaxSizeMB(cvMaxSizeMB),
      m_cvInitBufferSize(cvInitBufferSize), m_nFlags(nFlags) {
...
void CBufferPool::ReturnBuffer(CUtlBuffer *pBuffer) {
  ...
  if ((int64)(m_cubFree + pBuffer->Size()) <=
      (m_cvMaxSizeMB.GetInt() * k_nMegabyte)) {
    pBuffer->Clear();
    m_cubFree += pBuffer->Size();
...
void CBufferPool::DumpPools() {
  ...
  EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
           "  Pool                                Buffers         Free  Max "
           "Buffers  Total Mem (est)         Free Mem\n");
  EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
           "  ------------------------------ ------------ ------------ "
           "------------ ---------------- ----------------\n");
  ...
  FOR_EACH_VEC(sm_vecBufferPools, i) {
    ...
    if (pPool->m_vecFreeBuffers.Count() > 0) {
      nTotalEst = pPool->m_cubFree / pPool->m_vecFreeBuffers.Count() *
                  pPool->m_nBuffersTotal;
    }

    EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
             "  %-30s %12d %12d %12d %16s %16s\n", pPool->m_sName.Get(),
             pPool->m_nBuffersTotal, pPool->m_vecFreeBuffers.Count(),
             pPool->m_nHighWatermark, Q_pretifymem(nTotalEst, 0, true),
