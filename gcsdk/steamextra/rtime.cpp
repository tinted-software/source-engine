...
// This strptime implementation is taken from the Goolge Site Map Generator
// project:
...
static const char *kWeekAbbr[] = {"Sun", "Mon", "Tue", "Wed",
                                  "Thu", "Fri", "Sat"};
...
RTime32 CRTime::sm_nTimeLastSystemTimeUpdate =
    0; // initialize to large negative value to trigger immediate FileTimeCur
       // update
...
//-----------------------------------------------------------------------------
int CRTime::CSecsPassed() const { return (sm_nTimeCur - m_nStartTime); }

//-----------------------------------------------------------------------------
// Purpose: Updates our current time value.  We only
//			update the time once per frame-- the rest of the time,
//we just 			access a cached copy of the time. 			NOTE: This should only be called
//once per frame.
//-----------------------------------------------------------------------------
void CRTime::UpdateRealTime() {
  ...
  if (sm_nTimeCur < nTimePrev) {
    // time can go backwards sometimes if clock sync adjusts system time; warn
    // when this happens
    EmitInfo(SPEW_SYSTEM_MISC, SPEW_ALWAYS, LOG_ALWAYS,
...
#ifdef _WIN32
    // get the local time, generate time & date strings and cache the strings,
    // as we will need these frequently for logs.
    SYSTEMTIME systemTimeLocal;
    GetLocalTime(&systemTimeLocal);
    GetTimeFormat(LOCALE_USER_DEFAULT, 0, &systemTimeLocal, "HH:mm:ss",
                  sm_rgchLocalTimeCur, Q_ARRAYSIZE(sm_rgchLocalTimeCur));
    GetDateFormat(LOCALE_USER_DEFAULT, 0, &systemTimeLocal, "MM/dd/yy",
                  sm_rgchLocalDateCur, Q_ARRAYSIZE(sm_rgchLocalDateCur));
#elif defined(POSIX)
    time_t now;
    time(&now);
    struct tm tmStruct;
    struct tm *localTime = Plat_gmtime(&now, &tmStruct);
    strftime(sm_rgchLocalTimeCur, Q_ARRAYSIZE(sm_rgchLocalTimeCur), "%H:%M:%S",
             localTime);
    strftime(sm_rgchLocalDateCur, Q_ARRAYSIZE(sm_rgchLocalDateCur), "%m/%d/%y",
             localTime);
#else
#error "Implement me"
#endif
...
void CRTime::SetSystemClock(RTime32 rTime32Current) {
  ...
  // convert from seconds since 1/1/1970 to filetime (100 nanoseconds since
  // 1/1/1601) with this magic formula courtesy of MSDN
  uint64 ulTmp =
      (((uint64)rTime32Current) * 10 * k_nMillion) + 116444736000000000;
  fileTime.dwLowDateTime = (DWORD)ulTmp;
  fileTime.dwHighDateTime = ulTmp >> 32;

  // convert from filetime to system time (note this also does time zone
  // conversion to UTC)
...
// Output :	ptr to time string
//-----------------------------------------------------------------------------
const char *CRTime::Render(const RTime32 rTime32,
                           char (&buf)[k_RTimeRenderBufferSize]) {
  if (!buf) {
    Assert(buf);
    return nullptr;
  }
...
int CRTime::GetYear() const {
  ...
  struct tm tmStruct;
  struct tm *ptmCur = m_bGMT ? Plat_gmtime(&timeCur, &tmStruct)
                             : Plat_localtime(&timeCur, &tmStruct);
...
// Input:	pchFmt -	Format string that describes how to parse the
// value
//						YY or YYYY is year, MM month, DD
//day of the month, 						hh mm ss is hour minute second. 						Z0000 is a time-zone offset,
//eg -0700. 						Everything except YY is optional (will be considered 0 if not given)
...
RTime32 CRTime::RTime32FromFmtString(const char *pchFmt, const char *pchValue) {
  ...
  if (pchMnt) {
    static const char *rgszMonthNames[] = {"Jan", "Feb", "Mar", "Apr",
                                           "May", "Jun", "Jul", "Aug",
                                           "Sep", "Oct", "Nov", "Dec"};
  ...
  if (pchTzone) {
    ...
    // OK, so this is somewhat lame: mktime assumes our tm units are in LOCAL
    // time. However, we have just created a UTC time by using the supplied
    // timezone offset. The rational thing to do here would be to call
    // mkgmtime() instead of mktime(), but that function isn't available in
    // unix-land. SO, instead we will MANUALLY convert this tm back to local
    // time
...
#define daylight _daylight
    ...
    // subtract timezone, which is in SECONDS. timezone is (UTC - local), so
    // local = UTC - timezone
    tm.tm_sec -= timezone;
    // timezone does NOT account for DST, so if we are in DST, we need to ADD an
    // hour. This is because the value of timezone we subtracted was one hour
    // TOO LARGE
...
//-----------------------------------------------------------------------------
// Purpose: Calculate and return a time value corresponding to given sting which
// is expected to be in one of the common HTTP date formats.
//-----------------------------------------------------------------------------
// STATIC
...
// STATIC
const char *
CRTime::RTime32ToRFC3339UTCString(const RTime32 rTime32,
                                  char (&buf)[k_RTimeRenderBufferSize]) {
  if (!buf) {
    Assert(buf);
    return nullptr;
  ...
  // Store the result in a temporary buffer, so that you can use several in a
  // single printf.
  time_t tTime = rTime32;
  struct tm tmStruct;
  ...
  Q_snprintf(buf, k_RTimeRenderBufferSize, "%04u-%02u-%02uT%02u:%02u:%02uZ",
...
// Input:	rTime32 -
//			bNoPunct -	No dashes, colons or spaces will be in
//the output string 			bOnlyDate -	Only output the date
// Output:	const char * -- only usable till the next yield
//-----------------------------------------------------------------------------
// STATIC
const char *CRTime::RTime32ToString(const RTime32 rTime32,
                                    char (&buf)[k_RTimeRenderBufferSize],
                                    bool bNoPunct /*=false*/,
                                    bool bOnlyDate /*= false*/) {
  if (!buf) {
    return nullptr;
  }
  ...
  const char *pchDateTimeFmt =
      bNoPunct ? "%04u%02u%02u%02u%02u%02u" : "%04u-%02u-%02u %02u:%02u:%02u";
  if (rTime32 == k_RTime32Nil || !ptm)
    return "NIL";
...
//-----------------------------------------------------------------------------
// STATIC
const char *CRTime::RTime32ToDayString(const RTime32 rTime32,
                                       char (&buf)[k_RTimeRenderBufferSize],
                                       bool bGMT) {
  if (!buf) {
    return nullptr;
  ...
  struct tm tmStruct;
  struct tm *ptm =
      bGMT ? Plat_gmtime(&tTime, &tmStruct) : Plat_localtime(&tTime, &tmStruct);
...
//			the month. If that month only has K days, K < N, it will
//return 			the Kth day. The input should be reasonable (don't ask for the -5th 			day
//of the month).
//
// Input:	rtime32 -		Time representing some time in the month
// interested in
//			nDay -			The day of that month you want
//the return to be set to
//
...
//			month.
//
// Input:	nNthDayOfMonth -		Day of the target month to
// return a date for
//			rtime32StartDate -		Time value to add X
//months to 			nMonthsToAdd -			X
//
// Return:	Time value equal to midnight on that day.
//-----------------------------------------------------------------------------
// STATIC
RTime32 CRTime::RTime32MonthAddChooseDay(int nNthDayOfMonth,
                                         RTime32 rtime32StartDate,
                                         int nMonthsToAdd) {
  // Get the first day of start month
  RTime32 rtime32FirstDayOfStartMonth =
      CRTime(rtime32StartDate).GetFirstDayOfMonth();

  // Add X months to that - guaranteed to be correct month
  RTime32 rtime32FirstDayOfTargetMonth = CRTime::RTime32DateAdd(
      rtime32FirstDayOfStartMonth, nMonthsToAdd, k_ETimeUnitMonth);

  // Then get the Nth day of that month
  RTime32 rtime32Target = CRTime::RTime32NthDayOfMonth(
      rtime32FirstDayOfTargetMonth, nNthDayOfMonth);
...
//			nAmount -				Number of units
//to add (neg for subtract) 			eTimeFlagAmountType -	Indicates what units are
//on nAmount
//
// Return:	The newly calculated offset time (the input is unmodified)
//-----------------------------------------------------------------------------
// STATIC
RTime32 CRTime::RTime32DateAdd(const RTime32 rtime32, int nAmount,
                               ETimeUnit eTimeAmountType) {
  time_t timeCur = rtime32;
  struct tm tmStruct;
  struct tm *ptmCur = Plat_localtime(&timeCur, &tmStruct);
  if (!ptmCur)
  ...
  // mktime() is smart enough to take day-of-month values that are out of range
  // and adjust everything to make sense. So you can go back 3 weeks by just
  // subtracting 21 from tm_mday.
...
//			been crossed (eg day, month, hour) between the two
//times.
//
//			Note: in general, the crossing of a large boundary will
//be accompanied 			by the crossing of all smaller boundaries. The exception is
//Week: 			the Week boundary is from Saturday to Sunday, and it is possible to 			go
//over a Month or Year boundary without beginning a new week.
//
//			So, the return value is the largest time boundary that
//was crossed. 			However, the pbWeekChanged value will be set to indicate if the
//week 			changed in cases where the return value is Month or Year.
//
...
//-----------------------------------------------------------------------------
// STATIC
ETimeUnit CRTime::FindTimeBoundaryCrossings(RTime32 unTime1, RTime32 unTime2,
                                            bool *pbWeekChanged) {
