...
public:
  ...
  // Amount of seconds that have passed between this CRTime being set and the
  ...
  static const char *Render(const RTime32 rTime32,
  ...
  // comparisons against RTime32 numbers (to avoid implicit construct/destruct
  // of a temporary CRTime)
  ...
  static const char *
  RTime32ToRFC3339UTCString(const RTime32 rTime32,
                            char (&buf)[k_RTimeRenderBufferSize]);

  // parse time from string "YYYY-MM-DD hh:mm:ss" or just "YYYY-MM-DD", the '
  // ',':','-' are optional
  static RTime32 RTime32FromString(const char *pszValue);

  // turns RTime32 in a string like "YYYY-MM-DD hh:mm:ss"
  static const char *RTime32ToString(const RTime32 rTime32,
                                     char (&buf)[k_RTimeRenderBufferSize],
                                     bool bNoPunct = false,
                                     bool bOnlyDate = false);

  // turns RTime32 into a string like "Aug 21"
  static const char *RTime32ToDayString(const RTime32 rTime32,
                                        char (&buf)[k_RTimeRenderBufferSize],
                                        bool bGMT = false);
  ...
  // Add X months but return the Nth day of that month. If the month only has K
  // days, K < N, returns Kth day.
  static RTime32 RTime32MonthAddChooseDay(int nNthDayOfMonth,
  ...
  static RTime32 RTime32DateAdd(const RTime32, int nAmount,
  ...
  // Compare two times, and return what the largest time boundary crossed
  // between the two was. Week boundaries do not line up with Month or Year
  // boundaries, so you must rely on pbWeekChanged for them!
  static ETimeUnit FindTimeBoundaryCrossings(RTime32 unTime1, RTime32 unTime2,
                                             bool *pbWeekChanged);
...
private:
  ...
  RTime32
      m_nStartTime; // the time store by this instance (wall clock, in seconds)
  ...
  static RTime32 sm_nTimeLastSystemTimeUpdate; // last time we updated above two
                                               // logging strings
