...
#define DECLARE_STEAM_CLASS_NOBASE(className) typedef className ThisClass;

// Type for our memory output debugging.
class IDisplayMemPoolStats {
public:
  virtual void Display(const char *pszClassName, uint64 ulClassSize,
                       uint32 unPoolInstanceCount, uint64 ulPoolMemoryUsage,
                       uint32 unPoolPeakInstanceCount,
                       uint64 ulPoolPeakMemoryUsage) = 0;
};
...
public:
  ...
  CDumpMemFnReg(DumpMemFn_t fn) : m_fn(fn), m_pNext(sm_Head) { sm_Head = this; }

  DumpMemFn_t m_fn;
  CDumpMemFnReg *m_pNext;
};

// Helper macros for creating and using CClassMemoryPool on our frequently
// allocated objects
...
public:                                                                        \
  ...
  static void *operator new(size_t nSize, int nBlockUse,                       \
...
#define IMPLEMENT_CLASS_MEMPOOL(className, initSize, growMode)                 \
  CUtlMemoryPool className::sm_classMemPool(sizeof(className), (initSize),     \
                                            (growMode),                        \
                                            MEM_ALLOC_CLASSNAME(className));   \
                                                                               \
  void *className::operator new(size_t nSize) {                                \
    if (nSize != sizeof(className)) {                                          \
      EmitError(                                                               \
          SPEW_CONSOLE,                                                        \
          #className                                                           \
          "::operator new() called on wrong size! Expected %llu, Got %llu\n",  \
    ...
    return sm_classMemPool.Alloc();                                            \
                                                                               ...
                                                                               \
  void *className::operator new(size_t nSize, int nBlockUse,                   \
                                const char *pFileName, int nLine) {            \
    if (nSize != sizeof(className)) {                                          \
      EmitError(                                                               \
          SPEW_CONSOLE,                                                        \
