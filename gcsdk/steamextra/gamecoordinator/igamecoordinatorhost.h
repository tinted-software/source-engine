...
public:
  virtual bool BSendMessageToClient(AppId_t unAppID,
                                    const CSteamID &steamIDTarget,
                                    uint32 unMsgType, const void *pubData,
                                    uint32 cubData) = 0;
  virtual GID_t GenerateGID() = 0;
  virtual void EmitMessage(const char *pchGroupName, SpewType_t spewType,
                           int iSpewLevel, int iLevelLog,
                           const char *pchMsg) = 0;
  virtual void SQLQuery(GID_t gidContextID, IGCSQLQuery *pQuery,
                        int eSchemaCatalog) = 0;
