...
void CSteamID::SetFromString(const char *pchSteamID,
  ...
  if (*pchSteamID == 'A') {
    ...
    if (pchColon && *pchColon != 0 && strchr(pchColon + 1, ':')) {
      sscanf(pchSteamID, "%u:%u:%u", (uint *)&eUniverse, &nAccountID,
             &nInstance);
    } else if (pchColon) {
    ...
    AssertMsg1(this->operator==(StrictID),
               "Steam ID does not pass strict parsing: '%s'", pchSteamIDString);
    return;
  ...
  if (strchr(pchSteamID, ':')) {
    ...
    if (unVal64 > UINT_MAX) {
      ...
      // might not want to allow.
      AssertMsg1(this->operator==(StrictID),
                 "Steam ID does not pass strict parsing: '%s'",
                 pchSteamIDString);
      return;
    } else {
...
bool CSteamID::SetFromStringStrict(const char *pchSteamID,
                                   EUniverse eDefaultUniverse) {
  uint nAccountID = 0;
  uint nInstance = 1;
  uint unMaxVal = 2;
  ...
  for (;;) {
    ...
    if (*pchEnd == '(' && chPrefix == 'A') {
      ...
      pchEnd =
          DecimalToUint64(pchEnd + 1, k_unSteamAccountInstanceMask, &unVal[2]);
      if (!pchEnd || *pchEnd != ')') {
        bValid = false;
        break;
      } else {
...
bool CSteamID::SetFromSteam2String(const char *pchSteam2ID,
  ...
  if (V_strnicmp(pchSteam2ID, pchOptionalLeadString,
                 V_strlen(pchOptionalLeadString)) == 0)
...
const char *CSteamID::Render() const {
  ...
  if (k_EAccountTypeAnonGameServer == m_steamid.m_comp.m_EAccountType) {
    V_snprintf(pchBuf, k_cBufLen, "[A:%u:%u:%u]", m_steamid.m_comp.m_EUniverse,
    ...
    V_snprintf(pchBuf, k_cBufLen, "[G:%u:%u]", m_steamid.m_comp.m_EUniverse,
               m_steamid.m_comp.m_unAccountID);
  } else if (k_EAccountTypeMultiseat == m_steamid.m_comp.m_EAccountType) {
    V_snprintf(pchBuf, k_cBufLen, "[M:%u:%u:%u]", m_steamid.m_comp.m_EUniverse,
               m_steamid.m_comp.m_unAccountID,
               m_steamid.m_comp.m_unAccountInstance);
  } else if (k_EAccountTypePending == m_steamid.m_comp.m_EAccountType) {
    V_snprintf(pchBuf, k_cBufLen, "[P:%u:%u]", m_steamid.m_comp.m_EUniverse,
               m_steamid.m_comp.m_unAccountID);
  } else if (k_EAccountTypeContentServer == m_steamid.m_comp.m_EAccountType) {
    V_snprintf(pchBuf, k_cBufLen, "[C:%u:%u]", m_steamid.m_comp.m_EUniverse,
               m_steamid.m_comp.m_unAccountID);
    ...
    if (m_steamid.m_comp.m_unAccountInstance & k_EChatInstanceFlagClan) {
      V_snprintf(pchBuf, k_cBufLen, "[c:%u:%u]", m_steamid.m_comp.m_EUniverse,
                 m_steamid.m_comp.m_unAccountID);
    } else if (m_steamid.m_comp.m_unAccountInstance &
               k_EChatInstanceFlagLobby) {
      V_snprintf(pchBuf, k_cBufLen, "[L:%u:%u]", m_steamid.m_comp.m_EUniverse,
                 m_steamid.m_comp.m_unAccountID);
    } else // Anon chat
    {
      V_snprintf(pchBuf, k_cBufLen, "[T:%u:%u]", m_steamid.m_comp.m_EUniverse,
                 m_steamid.m_comp.m_unAccountID);
    }
  } else if (k_EAccountTypeInvalid == m_steamid.m_comp.m_EAccountType) {
    V_snprintf(pchBuf, k_cBufLen, "[I:%u:%u]", m_steamid.m_comp.m_EUniverse,
               m_steamid.m_comp.m_unAccountID);
    ...
    if (m_steamid.m_comp.m_unAccountInstance != k_unSteamUserDesktopInstance)
      V_snprintf(pchBuf, k_cBufLen, "[U:%u:%u:%u]",
                 m_steamid.m_comp.m_EUniverse, m_steamid.m_comp.m_unAccountID,
                 m_steamid.m_comp.m_unAccountInstance);
    else
      V_snprintf(pchBuf, k_cBufLen, "[U:%u:%u]", m_steamid.m_comp.m_EUniverse,
                 m_steamid.m_comp.m_unAccountID);
  } else if (k_EAccountTypeAnonUser == m_steamid.m_comp.m_EAccountType) {
    V_snprintf(pchBuf, k_cBufLen, "[a:%u:%u]", m_steamid.m_comp.m_EUniverse,
               m_steamid.m_comp.m_unAccountID);
  } else {
    V_snprintf(pchBuf, k_cBufLen, "[i:%u:%u]", m_steamid.m_comp.m_EUniverse,
               m_steamid.m_comp.m_unAccountID);
...
#ifdef STEAM
//-----------------------------------------------------------------------------
// Purpose:	Returns the matching chat steamID, with the default instance of
// 0 Input:	SteamID, either a Clan or a Chat type Output:	SteamID with
// account type changed to chat, and the Clan flag set.
//			If account type was not chat to start with, instance
//will be set to 0
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose:	Returns the matching chat steamID, with the default instance of
// 0 Input:	SteamID, either a Clan type or a Chat type w/ the Clan flag set
// Output:	SteamID with account type changed to clan.
//			If account type was not clan to start with, instance
//will be set to 0
//-----------------------------------------------------------------------------
...
CSteamID ChatIDFromClanID(const CSteamID &steamIDClan) {
  ...
  return CSteamID(steamIDClan.GetAccountID(), k_EChatInstanceFlagClan,
                  steamIDClan.GetEUniverse(), k_EAccountTypeChat);
}
...
CSteamID ClanIDFromChatID(const CSteamID &steamIDChat) {
  ...
  return CSteamID(steamIDChat.GetAccountID(), 0, steamIDChat.GetEUniverse(),
                  k_EAccountTypeClan);
}
