...
CThreadSafeMultiMemoryPool::CThreadSafeMultiMemoryPool(
    ...
    int nGrowMode /*= GROW_FAST*/) {
  m_cubReallocedTotal = 0;
  m_MapRawAllocation.SetLessFunc(DefLessFunc(void *));

  for (int iMemPoolConfig = 0; iMemPoolConfig < cnMemPoolConfig;
       iMemPoolConfig++) {
    MemPoolRecord_t memPoolRecord;
    // verify that the mem pool sizes are in ascending order
    Assert(iMemPoolConfig == 0 ||
           (iMemPoolConfig > 0 &&
            pMemPoolConfig[iMemPoolConfig - 1].m_cubBlockSize <
                pMemPoolConfig[iMemPoolConfig].m_cubBlockSize));
    AssertMsg(pMemPoolConfig[iMemPoolConfig].m_cubBlockSize % 32 == 0,
              "Mempools sizes must be multiples of 32");
    // add an int to the block size so we can note the alloc size
    memPoolRecord.m_pMemPool = new CThreadSafeMemoryPool(
        pMemPoolConfig[iMemPoolConfig].m_cubBlockSize + k_cubMemBlockPrefixSize,
  ...
  for (int i = 1; i < (int)m_nBlockSizeMax; i++) {
    for (int iMemPool = 0; iMemPool < m_VecMemPool.Count(); iMemPool++) {
      if ((int)m_VecMemPool[iMemPool].m_nBlockSize >= i) {
        AssertMsg(m_VecMemPoolLookup[ALLOCSIZE_TO_LOOKUP(i)] ==
                      &m_VecMemPool[iMemPool],
                  "Invalid mempool block size, can't generate lookup table");
        break;
      }
    }
  }
...
void *CThreadSafeMultiMemoryPool::Alloc(uint32 cubAllocSize) {
  ...
  if (cubAllocSize <= m_nBlockSizeMax) {
    MemPoolRecord_t *pMemPoolRecord =
        m_VecMemPoolLookup[ALLOCSIZE_TO_LOOKUP(cubAllocSize)];
    void *pvMem = pMemPoolRecord->m_pMemPool->Alloc(cubAllocSize +
                                                    k_cubMemBlockPrefixSize);
...
void CThreadSafeMultiMemoryPool::Free(void *pvMem) {
  ...
  if (cubAllocSize <= m_nBlockSizeMax) {
    MemPoolRecord_t *pMemPoolRecord =
        m_VecMemPoolLookup[ALLOCSIZE_TO_LOOKUP(cubAllocSize)];
    pMemPoolRecord->m_pMemPool->Free((char *)pvMem - k_cubMemBlockPrefixSize,
  ...
  if (m_MapRawAllocation.InvalidIndex() == iRawAllocation) {
    AssertMsg3(false,
               "CThreadSafeMultiMemoryPool::Free: raw allocation %p (original "
               "alloc: %p, %d bytes) not found in allocation map",
...
//-----------------------------------------------------------------------------
// Purpose: reallocate an existing block of memory to a new size (and copy the
// data
...
void *CThreadSafeMultiMemoryPool::ReAlloc(void *pvMem, uint32 cubAlloc) {
  ...
  if (cubOldAlloc > m_nBlockSizeMax) {
    ...
    if (m_MapRawAllocation.InvalidIndex() == iRawAllocation) {
      AssertMsg3(false,
                 "CThreadSafeMultiMemoryPool::ReAlloc: raw allocation %p "
                 "(original alloc: %p, %d bytes) not found in allocation map",
    ...
    // realloc the memory
    void *pvNewMem =
        PvRealloc(pvAllocedMem, cubAlloc + k_cubMemBlockPrefixSize);
    if (!pvNewMem) {
      m_MapRawAllocation.RemoveAt(iRawAllocation);
      return NULL;
    }
    ...
    *(uint32 *)pvNewMem = cubAlloc;
    if (pvAllocedMem == pvNewMem) {
      // if pointer is the same, use the same map entry with the same key (the
      // pointer given to caller)
      m_MapRawAllocation[iRawAllocation].m_pvMem = pvNewMem;
      m_MapRawAllocation[iRawAllocation].m_nBlockSize = cubAlloc;
    } else {
      // if pointer changed, need to remove the old entry and re-insert with new
      // key
      m_MapRawAllocation.RemoveAt(iRawAllocation);
      RawAllocation_t rawAllocation;
    ...
    MemPoolRecord_t *pMemPoolRecord =
        m_VecMemPoolLookup[ALLOCSIZE_TO_LOOKUP(cubOldAlloc)];
...
//in our container)
//-----------------------------------------------------------------------------
void CThreadSafeMultiMemoryPool::Validate(CValidator &validator,
                                          const char *pchName) {
  ...
  for (int iMemPool = 0; iMemPool < m_VecMemPool.Count(); iMemPool++) {
    ...
    m_VecMemPool[iMemPool].m_pMemPool->Validate(
        validator, "m_VecMemPool[iMemPool].m_pMemPool");
  }
