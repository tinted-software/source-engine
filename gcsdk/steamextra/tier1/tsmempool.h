...
public:
  enum {
    ...
    GROW_FAST =
        1, // New blob size is numElements * (i+1)  (ie: the blocks it allocates
           // get larger and larger each time it allocates one).
    GROW_SLOW = 2, // New blob size is numElements.
    GROW_TIL_YOU_CANT =
        3 // GROW_SLOW til alloc fails - then STOP and dont assert!
  };

  CThreadSafeMemoryPool(int blockSize, int numElements,
                        int growMode = GROW_FAST);
  ...
  static void *operator new(size_t size) {
    CThreadSafeMemoryPool *pNode =
        (CThreadSafeMemoryPool *)MemAlloc_AllocAligned(size, 8, __FILE__,
                                                       __LINE__
#ifdef STEAM
                                                       ,
                                                       true // new operator
  ...
  static void *operator new(size_t size, int nBlockUse, const char *pFileName,
                            int nLine) {
    CThreadSafeMemoryPool *pNode =
        (CThreadSafeMemoryPool *)MemAlloc_AllocAligned(size, 8, pFileName, nLine
#ifdef STEAM
                                                       ,
                                                       true // new operator
#endif
...
private:
  ...
  // CThreadSpinRWLock needs 8 byte alignment to work but we new()
  // CThreadSafeMemoryPool so simply place it at the start of the class to make
  // sure it fits on the 8-byte boundary
...
template <class T> T *CThreadSafeClassMemoryPool<T>::Alloc() {
