...
template <class T1, class T2> inline T2 ModPowerOf2(T1 a, T2 b) {
  return T2(a) & (b - 1);
}
} // namespace basetypes

// default comparison operator
template <typename T> class CDefEquals {
public:
  CDefEquals() {}
  CDefEquals(int i) {}
...
template <typename T> class CDefEquals<T *> {
public:
  CDefEquals() {}
  CDefEquals(int i) {}
  inline bool operator()(const T *lhs, const T *rhs) const {
...
template <> struct MurmurHash3Functor<CUtlString> {
...
//-----------------------------------------------------------------------------
//
// Purpose:	An associative container. Pretty much identical to CUtlMap
// without the ability to walk in-order
//	This container is well suited for large and growing datasets. It uses
//less 	memory than other hash maps and incrementally rehashes to reduce
//reallocation spikes. 	However, it is slower (by about 20%) than CUtlHashTable
//
//-----------------------------------------------------------------------------
template <typename K, typename T, typename L = CDefEquals<K>,
          typename H = MurmurHash3Functor<K>>
class CUtlHashMapLarge {
public:
  // This enum exists so that FOR_EACH_MAP and FOR_EACH_MAP_FAST cannot
  // accidentally be used on a type that is not a CUtlMap. If the code compiles
  // then all is well. The check for IsUtlMap being true should be free. Using
  // an enum rather than a static const bool ensures that this trick works even
  ...
  bool IsValidIndex(IndexType_t i) const {
    return i >= 0 && i < m_nMaxElement &&
           !IsFreeNodeID(m_memNodes[i].m_iNextNode);
  }
...
private:
  ...
  IndexType_t InsertInternal(const KeyType_t &key, const ElemType_t &insert,
                             EInsertPolicy ePolicy);
...
inline int CUtlHashMapLarge<K, T, L, H>::InsertInternal(
    const KeyType_t &key, const ElemType_t &insert, EInsertPolicy ePolicy) {
...
inline void CUtlHashMapLarge<K, T, L, H>::EnsureCapacity(int amount) {
  ...
  memset(&m_vecHashBuckets[iFirst], 0xFFFFFFFF,
         grow * sizeof(m_vecHashBuckets[iFirst]));
...
//-----------------------------------------------------------------------------
// Purpose: takes a bucket of nodes and re-hashes them into a more optimal
// bucket
//-----------------------------------------------------------------------------
template <typename K, typename T, typename L, typename H>
inline void CUtlHashMapLarge<K, T, L, H>::RehashNodesInBucket(int iBucketSrc) {
  ...
  while (iNode != InvalidIndex()) {
    ...
    if (iBucketDest != iBucketSrc) {
      //	::OutputDebugStr( CFmtStr( "moved key %d from bucket %d to
...
template <typename K, typename T, typename L, typename H>
inline int
CUtlHashMapLarge<K, T, L, H>::FindInBucket(int iBucket,
                                           const KeyType_t &key) const {
...
//-----------------------------------------------------------------------------
template <typename K, typename T, typename L, typename H>
void CUtlHashMapLarge<K, T, L, H>::LinkNodeIntoBucket(int iBucket,
                                                      int iNewNode) {
...
//-----------------------------------------------------------------------------
template <typename K, typename T, typename L, typename H>
void CUtlHashMapLarge<K, T, L, H>::UnlinkNodeFromBucket(int iBucket,
                                                        int iNodeToUnlink) {
...
template <typename K, typename T, typename L, typename H>
inline bool
CUtlHashMapLarge<K, T, L, H>::RemoveNodeFromBucket(IndexType_t iBucket,
                                                   int iNodeToRemove) {
  IndexType_t iNode = m_vecHashBuckets[iBucket].m_iNode;
  while (iNode != InvalidIndex()) {
...
inline void CUtlHashMapLarge<K, T, L, H>::RemoveAll() {
  ...
  m_nMaxRehashedBucket = InvalidIndex();
  m_bitsMigratedBuckets.Resize(0);
  memset(m_vecHashBuckets.Base(), 0xFF,
         m_vecHashBuckets.Count() * sizeof(HashBucket_t));
}
...
inline void CUtlHashMapLarge<K, T, L, H>::PurgeAndDeleteElements() {
  FOR_EACH_MAP_FAST(*this, i) { delete this->Element(i); }
  ...
  if (m_nMinRehashedBucket < m_nMaxRehashedBucket) {
    while (m_nMinRehashedBucket < m_nMaxRehashedBucket) {
      ...
      if (m_vecHashBuckets[m_nMinRehashedBucket].m_iNode != InvalidIndex() &&
        ...
        // only actively do one - don't want to do it too fast since we may be
