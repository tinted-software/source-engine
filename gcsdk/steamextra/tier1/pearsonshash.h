...
//	The generic implementation is a template, plus a couple of
//specializations 	for commonly used types.
//
//	Take a look at http://en.wikipedia.org/wiki/Pearson_hashing for more
//info.
//
//=============================================================================
...
template <typename T> struct PearsonsHashFunctor {
...
//
template <typename T> struct PearsonsHashFunctor<T *> {
  typedef uint32 TargetType;
...
//
//	This functor specializes for unsigned 32 bit integers, a commonly used
//type in Steam. 	It should return the exact same result as the unspecialized
//version on Intel Architecture machines.
//
template <> struct PearsonsHashFunctor<uint32> {
  typedef uint32 TargetType;
...
//
//	This functor specializes for unsigned 64 bit integers, another commonly
//used type in Steam. 	It should return the exact same result as the
//unspecialized version on Intel Architecture machines.
//
template <> struct PearsonsHashFunctor<uint64> {
  typedef uint32 TargetType;
  TargetType operator()(const uint64 unKey) const {
    ...
    uint32 n;
    //
    //	On Intel Machines, to make this return the exact same result as the
    //generic version 	we have to go from least significant byte to most
    //significant byte !
    //
...
//	It is setup so that if you had a char array containing a NULL terminated
//string 	and correctly sized, ie char rgch[16] = { "123456789012345" } and a
//	null terminated string i.e. char *sz = "123456789012345" these will
//return identical 	results, and both include the NULL terminator in the hash
//calculation.
//
template <> struct PearsonsHashFunctor<char *> {
  typedef uint32 TargetType;
...
//
//	This functor compares two objects of a particular type and returns a
//result 	that follows the strcmp/memcmp. 	If the type doesn't provide an
//operator== or operator< then you can provide 	a type specific specialization to
//override this defualt functor !
//
...
//
template <typename T> struct ComparisonFunctor<T *> {
  int operator()(const T *lhs, const T *rhs) const {
...
template <> struct ComparisonFunctor<int> {
  int operator()(const int lhs, const int rhs) const { return lhs - rhs; }
};

template <> struct ComparisonFunctor<unsigned int> {
  int operator()(const unsigned int lhs, const unsigned int rhs) const {
    return static_cast<int>(lhs) - static_cast<int>(rhs);
  }
};

template <> struct ComparisonFunctor<uint64> {
...
template <> struct ComparisonFunctor<char *> {
