...
//-----------------------------------------------------------------------------
// Purpose: A container of a range of mem pool sizes (for network buffers for
// example)
//			and a raw alloc capability (for sizes greater than any
//contained mem pool).
//-----------------------------------------------------------------------------
...
public:
  ...
  CThreadSafeMultiMemoryPool(const MemPoolConfig_t *pnBlock,
                             int cnMemPoolConfig,
                             int nGrowMode = UTLMEMORYPOOL_GROW_FAST);
...
private:
  ...
  CUtlVector<MemPoolRecord_t *>
      m_VecMemPoolLookup; // quick lookup table of mempools

  struct RawAllocation_t {
    void *m_pvMem;
    uint32 m_nBlockSize;
  };
  CUtlMap<void *, RawAllocation_t, int>
      m_MapRawAllocation; // stores our list of raw alloc'd mem
