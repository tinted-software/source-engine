...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CThreadSafeMemoryPool::CThreadSafeMemoryPool(int blockSize, int numElements,
                                             int growMode) {
...
void *CThreadSafeMemoryPool::Alloc(unsigned int amount) {
  ...
  // there appears to be a case where memory corruption can get this into an
  // infinite loop normally 1 or 2 attempts are necessary to get a block, so if
  // we hit 1000 we know something is wrong
  int cAttempts = 1000;
  while (--cAttempts) {
    ...
    byte *pBlobBase = (byte *)MemAlloc_AllocAligned(
        cubBlob,
        TSLIST_NODE_ALIGNMENT /*, (m_nGrowMode == GROW_TIL_YOU_CANT)*/);
    if (!pBlobBase)
      return NULL;
...
void CThreadSafeMemoryPool::PrintStats() {
  ...
  Msg("Block size: %-11s Alloc'd: %8d Num blobs: %5d (%s)\n",
      ...
      m_vecBlockSets.Count(), Q_pretifymem(m_cubAllocated, 2, true));
}
...
//in our container)
//-----------------------------------------------------------------------------
void CThreadSafeMemoryPool::Validate(CValidator &validator,
                                     const char *pchName) {
  AUTO_LOCK_SPIN_WRITE(m_threadRWLock);
  VALIDATE_SCOPE();
  FOR_EACH_VEC(m_vecBlockSets, i) {
