...
enum EAccountFlags {
  m_EAccountFlagNormalUser =
      0, // Standard user level (yes, this is meant to be zero)
  k_EAccountFlagPersonaNameSet =
      (1 << 0), // true if the user has set the persona name they really want,
                // instead of the auto-generated one
  k_EAccountFlagUnbannable =
      (1 << 1), // whatever happens, this account can't be banned
  k_EAccountFlagPasswordSet =
      (1 << 2), // we've set the password at least once on this account
  ...
  k_EAccountFlagPersonalQASet =
      (1 << 8),                     // user has personal Question & anser set
  k_EAccountFlagVacBeta = (1 << 9), // user participates in VAC beta tests
  k_EAccountFlagDebug =
      (1 << 10), // user is in debug mode, eg VAC doesn't kick etc
  k_EAccountFlagDisabled = (1 << 11), // account is disabled.
  k_EAccountFlagLimitedUser =
      (1
       << 12), // account is limited user account because it doesnt own anything
  k_EAccountFlagLimitedUserForce =
      (1 << 13), // account is limited user account because we forced it to be
  k_EAccountFlagEmailValidated =
      (1 << 14), // user has verified email address via WG
  k_EAccountFlagMarketingTreatment =
      (1 << 15), // account is flagged as being in a treatment for
...
enum ECommunityProfileState {
  ...
  k_ECommunityProfileLocked =
      3, // user got locked, content can't be changed but is still accessible
  k_ECommunityProfileDisabled =
      4, // user got disabled, site not accessible anymore
};
...
enum ECommunityVisibilityState {
  k_ECommunityVisibilityPrivate =
      1, // private, requester see only public fields
  k_ECommunityVisibilityFriendsOnly =
      2, // friends only, requester sees only public fields
  k_ECommunityVisibilityOpen =
      3, // it is visible to requester; they are owner or friend or public
  k_ECommunityVisibilitySupportPrivate =
      4, // was private, but it's a support account asking
  k_ECommunityVisibilitySupportFriendsOnly =
      5, // was friends only, but it's a support account asking
};
...
enum ECommentPermission {
  ...
  k_ECommentPermissionSelfOnly =
      2, // only the account owner can leave a comment
};
...
enum EPaymentMethod {
  ...
  k_EPaymentMethodActivationCode =
      0x001, // user paid by entering unused CD-Key or other activation code
  ...
  k_EPaymentMethodAutoGrant = 0x040, // server side purchased package, things
                                     // like German specific TF2 free weekend
  k_EPaymentMethodWallet = 0x080,    // user paid with wallet
  k_EPaymentMethodOEMTicket =
      0x100,                     // user paid by redeeming a OEM license ticket
...
enum EWalletSource {
  ...
  k_EWalletSourcePurchase =
      1, // Created from a purchase, refund, chargeback, or reverse chargeback
         // (PurchaseRefGID -> TransID)
  k_EWalletSourceGuestPass =
      2, // Created from a guest pass (PurchaseRefGID -> GuestPassID)
  k_EWalletSourceConversion =
      3, // Created from a wallet conversion (PurchaseRefGID -> GID shared
         // between debit & credit records)
  k_EWalletSourceRebate =
      4, // Created from a rebate (PurchaseRefGID -> TransID)
};
...
enum ELicenseType {
  ...
  k_ERecurringChargeLimitedUse =
      4, // recurring subscription w/ limited minutes per period
  k_ERecurringChargeLimitedUseWithOverages =
      5, // like above but w/ soft limit and overage charges
};
...
enum ELicenseFlags {
  ...
  k_ELicenseFlagLowViolenceContent =
      0x40, // license is for low violence content
};
...
enum EPackageStatus {
  ...
  k_EPackageUnavailable =
      2, // Not available for new purchases, may still be owned
  k_EPackageInvalid =
      3, // Either an unknown package or a deleted one that nobody should own
};
...
enum EPurchaseStatus {
  ...
  k_EPurchaseChargedback =
      5, // the user issued a chargeback, we removed subscription
  k_EPurchaseRevoked = 6,   // we revoked the purchase and removed subscription.
                            // Usually stolen CD-Keys
  k_EPurchaseInDispute = 7, // the purchase is being disputed by the user,
                            // preliminary to a chargeback
};
...
enum ELineItemType {
  ...
  k_ELineItemTypeMicroTxn =
      (1 << 0), // Transaction has data in MicroTxnLineItem table
  k_ELineItemTypeWallet =
      (1 << 1), // Transaction has data in WalletLineItem table
  k_ELineItemTypePkg =
      (1 << 2), // Transaction has data in PurchaseLineItem table
};
...
enum EPurchaseResultDetail {
  ...
  k_EPurchaseResultUseOtherPaymentMethod =
      16, // User should try a different payment method
  k_EPurchaseResultUseOtherFundingSource =
      17, // Select a different funding source (paypal)
  k_EPurchaseResultInvalidShippingAddress =
      18, // Shipping address is invalid (paypal)
  k_EPurchaseResultRegionNotSupported =
      19, // This region is not supported with this payment type

  k_EPurchaseResultAcctIsBlocked =
      20, // Acct has been blocked by provider - user should contact provider to
          // resolve
  k_EPurchaseResultAcctNotVerified =
      21, // Provider indicated account needs to be verified for transaction to
          // complete

  k_EPurchaseResultInvalidAccount =
      22, // Provider indicated the account is invalid or no longer usable
  k_EPurchaseResultStoreBillingCountryMismatch =
      23, // store country code & billing country code do not match
  k_EPurchaseResultDoesNotOwnRequiredApp =
      24, // user does not own one of the apps required for purchase
  k_EPurchaseResultCanceledByNewTransaction =
      25, // user made a new transaction which canceled an old, pending
          // transaction
  k_EPurchaseResultForceCanceledPending =
      26, // A pending transaction was force canceled (no response from
          // provider)
  k_EPurchaseResultFailCurrencyTransProvider =
      27, // selected transaction provider does not support calculated currency
  k_EPurchaseResultFailedCyberCafe =
      28, // cybercafe account tried to purchase or use an activation code

  k_EPurchaseResultNeedsPreApproval =
      29, // Transaction needs approval from support
  k_EPurchaseResultPreApprovalDenied = 30, // Transaction was denied by support
  k_EPurchaseResultWalletCurrencyMismatch =
      31, // Currency of purchase does not match currency of wallet
};

// Type of system IM.  The client can use this to do special UI handling in
// specific circumstances WARNING: DO NOT RENUMBER EXISTING VALUES - STORED IN
// DATABASE
enum ESystemIMType {
  ...
  k_ESystemIMGuestPassReceived =
      5, // User has received a guest pass from a friend
  k_ESystemIMGuestPassGranted =
      6, // System has granted a user a guest pass to give out
...
enum ESupportEvent {
  ...
  eSupportChangeAccountPassword =
      7, // support or user changed password, data: DONT include old password
  eSupportChangeAccountEmail =
      8, // support or user changed email, data: old email
  eSupportChangeAccountName = 9, // support or user changed name, data: old name
  eSupportChangeAccountPlayer =
      10, // support or user changed player name, data: old name

  eSupportPurchaseChargedback = 11, // a charge back was issued
  eSupportPurchaseRefunded = 12,    // a refund was issued
  eSupportPurchaseForcedCompletion =
      13, // support forced a pending purchase to complete
  ...
  eSupportSetNoFraudCheckFlag =
      21, // support disabled fraud check, data: reason
  ...
  eSupportLockProfile =
      34, // temp block a user from modifying community content
  ...
  eSupportChargebackStatusUpdate =
      38, // support updated the pending chargeback status

  eSupportRefundForcedCompletion =
      39,                      // support forced a pending refund to complete
  eSupportRefundCanceled = 40, // support forced a pending refund to cancel

  eSupportRevokeActivationKey =
      41,                         // support revoked and unlocked activation key
  eSupportReverseChargeback = 42, // a charge back was reversed
  eSupportRejectedActivationKey =
      43, // activation key was rejected (invalid or already used)
  ...
  // comments
  eSupportDeleteComments =
      49, // cleared comments on this account (not written by this account)
...
enum ESupportTicketState {
  ...
  k_ETicketStateUnresolved =
      4, // support issue couldn't be fixed. closed anyway
...
enum EClanAction {
  ...
  k_ESupportLockedGroup =
      19, // support locked this clan, it can't be modified anymore
  ...
  k_ESupportDisabledGroup =
      24,             // support disabled this group, nobody can see it anymore
...
// WARNING: Do not change these if an instance of this record may exist in a
// database!!! BUGBUG derrick - Since these also define schema, they should be
// moved into steamschema.h
...
enum EMicroTxnAuthResponse {
  ...
  k_EMicroTxnAuthResponseAutoDeny =
      3, // client automatically denied microtransaction (user wasn't in game,
         // etc.)
};
...
enum EMicroTxnAuthResult {
  ...
  k_EMicroTxnAuthResultInsufficientFunds =
      3, // User has insufficient funds to complete microtransaction
};
