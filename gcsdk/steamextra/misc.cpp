...
// integrated with our automated tests so that we can perform reproducibly
// "random" test cases.
//-----------------------------------------------------------------------------
...
const uint32 k_rgunMask[17] = {
    0x1739a3b0, 0xb8907fe1, 0x8290d3b7, 0x72839cd0, 0x242df096, 0x3829750b,
    0x38de7a77, 0x72f0924c, 0x44783927, 0x01925372, 0x20902714, 0x27585920,
    0x27890632, 0x82910476, 0x72906721, 0x28798904, 0x78592700,
};
...
uint32 UNRandFast() {
  g_iunRandMask++;
  g_unRandCur +=
      637429601; // Just add a large prime number (we'll wrap frequently)
...
//-----------------------------------------------------------------------------
char CHRandFast() { return (UNRandFast() % 95) + 32; }
...
//-----------------------------------------------------------------------------
uint64 GetRandSeed() { return ((((uint64)g_unRandCur) << 32) + g_iunRandMask); }
...
//			Does a gradual fade of 0% rejections at the specified
//threshold 			up to 100% at the limit.  The gradual fade reduces system
//oscillations 			that could occur if you abruptly stop allowing all operations.
// Input:	nBacklogCur - the current backlog (in arbitrary units)
//			nBacklogThreshold - the threshold backlog at which to
//begin rejecting 			nBacklogLimit - hard limit at which to reject 100% of
//operations 			iItem - a monotonically increasing counter of items submitted. Used
//				to determine which operations are allowed if
//there is a partial 				rejection rate.
//-----------------------------------------------------------------------------
bool BRejectDueToBacklog(int nBacklogCur, int nBacklogThreshold,
                         int nBacklogLimit, int iItem) {
  bool bRefuse = false;
  ...
  if (nBacklogCur >= nBacklogLimit) {
    ...
    // if we're near the hard backlog limit, start refusing an increasing % of
    // operations, so we don't snap abruptly in and out of accepting operations
    // and potentially cause oscillations

    // ramp from refusing 0% of operations at the backlog threshold up to 100%
    // at the backlog hard limit

    // calculate refuse % to nearest 10%, to make it easy to mod the item # and
    ...
    // compare the operations submitted count mod 10 to the refusal percent
    // decile to decide if we should accept or refuse this particular operation
