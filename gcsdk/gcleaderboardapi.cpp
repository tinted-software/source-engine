...
ENUMSTRINGS_START(ELeaderboardSortMethod){k_ELeaderboardSortMethodNone, ""},
    ...
    ENUMSTRINGS_REVERSE(ELeaderboardUploadScoreMethod,
                        k_ELeaderboardUploadScoreMethodNone)

        namespace GCSDK{uint32 Leaderboard_YieldingFind(
            const char *pName, ELeaderboardSortMethod eLeaderboardSortMethod,
            ELeaderboardDisplayType eLeaderboardDisplayType,
            bool bCreateIfNotFound){
            CSteamAPIRequest apiRequest(k_EHTTPMethodPOST, "ISteamLeaderboards",
                                        "FindOrCreateLeaderboard", 1);
apiRequest.SetPOSTParamUInt32("appid", GGCBase()->GetAppID());
apiRequest.SetPOSTParamString("name", pName);

apiRequest.SetPOSTParamString(
    "sortmethod", PchNameFromELeaderboardSortMethod(eLeaderboardSortMethod));
apiRequest.SetPOSTParamString(
    "displaytype", PchNameFromELeaderboardDisplayType(eLeaderboardDisplayType));
apiRequest.SetPOSTParamBool("createifnotfound", bCreateIfNotFound);

KeyValuesAD kvAPIResponse("response");
const EResult eCallResult =
    GGCBase()->YieldingSendHTTPRequestKV(&apiRequest, kvAPIResponse);
const EResult eAPIResult =
    eCallResult == k_EResultOK
        ? static_cast<EResult>(kvAPIResponse->GetInt("result", k_EResultFail))
        : k_EResultFail;
...
bool Leaderboard_YieldingSetScore(
    uint32 unLeaderboardID, const CSteamID &steamID,
    ELeaderboardUploadScoreMethod eLeaderboardUploadScoreMethod, int score) {
  Assert(unLeaderboardID != kInvalidLeaderboardID);
  if (unLeaderboardID == kInvalidLeaderboardID)
    return false;

  CSteamAPIRequest apiRequest(k_EHTTPMethodPOST, "ISteamLeaderboards",
                              "SetLeaderboardScore", 1);
  apiRequest.SetPOSTParamUInt32("appid", GGCBase()->GetAppID());
  ...
  apiRequest.SetPOSTParamString(
      "scoremethod",
      PchNameFromELeaderboardUploadScoreMethod(eLeaderboardUploadScoreMethod));
  ...
  EmitError(
      SPEW_GC,
      __FUNCTION__ ": error code %u/%u setting leaderboard %u to %i (%s) for "
                   "user '%s'.\n",
      eCallResult, eAPIResult, unLeaderboardID, score,
      PchNameFromELeaderboardUploadScoreMethod(eLeaderboardUploadScoreMethod),
      steamID.Render());
  return false;
}
