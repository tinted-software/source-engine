...
// Probably this makes more sense true by default, but we're spewing a ton and
// Fletcher says it isn't a big deal for TF so here we go.
GCConVar gs_session_assert_valid_addr_and_port(
    "gs_session_assert_valid_addr_and_port", "0");
...
GCConVar
    max_user_messages_per_second("max_user_messages_per_second", "20", 0,
                                 "Maximum number of messages a user can send "
                                 "per second. 0 disables the rate limiting");
static GCConVar user_message_rate_limit_warning_period(
    "user_message_rate_limit_warning_period", "30", 0,
    "Number of seconds between warning about rate limiting for users");

static GCConVar msg_rate_limit_report_user_bucket_1(
    "msg_rate_limit_report_user_bucket_1", "10", 0,
    "These values control where various users are bucketed in rate limiting "
    "reports to help identify how frequently users are running into rate "
    "limiting");
static GCConVar msg_rate_limit_report_user_bucket_2(
    "msg_rate_limit_report_user_bucket_2", "100", 0,
    "These values control where various users are bucketed in rate limiting "
    "reports to help identify how frequently users are running into rate "
    "limiting");

static GCConVar msg_rate_limit_list_user(
    "msg_rate_limit_list_user", "0", 0,
    "When set to a user account ID, this will report all the messages that are "
    "rate limited for that user to the console");
...
void CMsgRateLimitTracker::TrackRateLimitedMsg(const CSteamID steamID,
                                               MsgType_t eMsgType) {
  // update message stat
  {
  ...
  // determine the severity to output the warning at. Assume verbose unless we
  // are tracking a specific account ID (note that no account has 0 so 0 still
...
void CMsgRateLimitTracker::ReportTopUsers(uint32 nMinMsgs,
                                          uint32 nListTop) const {
...
void CMsgRateLimitTracker::ReportUserStats() const {
  ...
  const uint32 nBucketLimit1 =
      (uint32)max(0, min(msg_rate_limit_report_user_bucket_1.GetInt(),
                         msg_rate_limit_report_user_bucket_2.GetInt()));
  const uint32 nBucketLimit2 =
      (uint32)max(0, max(msg_rate_limit_report_user_bucket_1.GetInt(),
                         msg_rate_limit_report_user_bucket_2.GetInt()));
...
GC_CON_COMMAND(msg_rate_limit_dump,
               "Dumps stats about rate limiting of messages") {
  g_RateLimitTracker.ReportUserStats();
  g_RateLimitTracker.ReportMsgStats();
  g_RateLimitTracker.ReportTopUsers(0, 20);
}

GC_CON_COMMAND(msg_rate_limit_dump_users,
               "Dumps a list of users that have been rate limited. Optional "
               "parameters can specify the number to dump or the minimum "
               "number of messages required.") {
  if (args.ArgC() < 3) {
    EG_MSG(SPEW_CONSOLE,
           "Proper usage is: %s <min messages> <top users> - Specify 0 for one "
           "or both to have it be ignored\n",
           args[0]);
    return;
  }
  g_RateLimitTracker.ReportTopUsers((uint32)max(0, atoi(args[1])),
                                    (uint32)max(0, atoi(args[2])));
}

GC_CON_COMMAND(msg_rate_limit_dump_msgs,
               "Dumps a list of messages that have been rate limited.") {
  g_RateLimitTracker.ReportMsgStats();
}

GC_CON_COMMAND(msg_rate_limit_clear,
               "Clears all the accumulated msg rate limit stats") {
...
CSteamIDRateLimit::CSteamIDRateLimit(const GCConVar &cvNumPerPeriod,
                                     const GCConVar *pcvPeriodS)
    : m_cvNumPerPeriod(cvNumPerPeriod), m_pcvPeriodS(pcvPeriodS),
      m_LastClear(CRTime::RTime32TimeCur()),
      m_FrameFunction("SteamIDRateLimit",
                      CBaseFrameFunction::k_EFrameType_RunOnce) {
...
CGCSession::CGCSession(const CSteamID &steamID, CGCSharedObjectCache *pSOCache)
    : m_steamID(steamID), m_pSOCache(pSOCache), m_bIsShuttingDown(false),
      m_osType(k_eOSUnknown), m_bIsTestSession(false), m_bIsSecure(false),
      m_unIPPublic(0), m_flLatitude(0.0f), m_flLongitude(0.0f),
      m_haveGeoLocation(false), m_bInitialized(false),
...
bool CGCSession::BRateLimitMessage(MsgType_t unMsgType) {
  ...
  if (m_unMessagesRecievedThisSecond >
      (uint32)max_user_messages_per_second.GetInt()) {
    // log this message
...
//			approximately every k_nUserSessionRunInterval
//microseconds (or 			k_nGSSessionRunInterval for GS Sessions)
//-----------------------------------------------------------------------------
void CGCSession::Run() {
  // These cached subscription messages are very expensive and only needed for a
  // short period of time If we're hitting the run loop, it's been around long
  // enough
...
CGCUserSession::~CGCUserSession() {
  if (m_steamIDGS.BGameServerAccount()) {
    EmitError(SPEW_GC,
              "Destroying user %s while still connected to server %s\n",
...
//			False if the session could not be found or if the user
...
bool CGCUserSession::BSetServer(const CSteamID &steamIDGS) {
  ...
  if (steamIDGS.IsValid()) {
    ...
    if (!pGSSession) {
      EmitError(SPEW_GC,
                "User %s attempting to join server %s which has no session\n",
    ...
    if (!pGSSession->BAddUser(GetSteamID())) {
      EmitWarning(SPEW_GC, SPEW_ALWAYS,
                  "Server %s already had user %s in its user list\n",
...
// this is ifdef'd out in Steam because GCSDK can't depend on steamid.cpp
  ...
  EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS, "User Session %s (%s)\n",
           GetSteamID().Render(),
           BIsShuttingDown() ? "SHUTTING DOWN" : "Active");
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CGCGSSession::CGCGSSession(const CSteamID &steamID,
                           CGCSharedObjectCache *pCache, uint32 unServerAddr,
                           uint16 usServerPort)
    : CGCSession(steamID, pCache), m_unServerAddr(unServerAddr),
...
CGCGSSession::~CGCGSSession() {
  if (m_vecUsers.Count() > 0) {
    EmitError(SPEW_GC,
              "Destroying game server %s while %d users are still connected\n",
...
bool CGCCacheSubscriptionRefresh::BYieldingRunJobFromMsg(
...
GC_REG_JOB(CGCBase, CGCCacheSubscriptionRefresh, "CGCCacheSubscriptionRefresh",
           k_ESOMsg_CacheSubscriptionRefresh, k_EServerTypeGC);
