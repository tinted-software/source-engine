...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CProtoBufNetPacket::CProtoBufNetPacket(CNetPacket *pNetPacket,
                                       GCProtoBufMsgSrc eMsgSrc,
                                       const CSteamID steamID,
                                       uint32 nGCDirIndex, MsgType_t msgType)
  ...
  // Pull the length of the header out of the packet, but then validate it's not
  // longer than the packet itself (ie, corrupt packet)
  const uint32 unLenProtoBufHeader = GetFixedHeader().m_cubProtoBufExtHdr;
  if (unLenProtoBufHeader + sizeof(ProtoBufMsgHeader_t) <=
      pNetPacket->CubData()) {
    if (!m_pHeader->ParseFromArray(pNetPacket->PubData() +
                                       sizeof(ProtoBufMsgHeader_t),
                                   unLenProtoBufHeader)) {
      AssertMsg4(false,
                 "Failed parsing ProtoBuf extended header out of message: "
                 "%s(%d) from %s size=%u",
                 PchMsgNameFromEMsg(GetEMsg()), GetEMsg(), steamID.Render(),
                 pNetPacket->CubData());
    } else {
      // if this packet doesn't have a source provided, we need to set the
      // source of this message. If one is already provided, we should never
      // stomp that data as that is the ultimate source of the message
      if (m_pHeader->gc_msg_src() == GCProtoBufMsgSrc_Unspecified) {
        // make sure to set the steam ID to what steam provided so clients can't
        // spoof it
        m_pHeader->set_client_steam_id(steamID.ConvertToUint64());
        // and track our original source type and which GC it was first received
        // by
    ...
    AssertMsg4(
        false,
        "Unparseable protobuf message arrived: message %s(%u) from %s size=%u",
        PchMsgNameFromEMsg(GetEMsg()), GetEMsg(), steamID.Render(),
        pNetPacket->CubData());
  }
}
...
//-----------------------------------------------------------------------------
bool CProtoBufNetPacket::GetMsgBody(const uint8 *&pubData,
                                    uint32 &cubData) const {
  if (!IsValid()) {
    pubData = NULL;
    cubData = 0;
    return false;
  ...
  // when it initializes, it verifies the size, so we don't need to do
  // validation here
  const uint32 nHdrOffset =
      sizeof(ProtoBufMsgHeader_t) + GetFixedHeader().m_cubProtoBufExtHdr;
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CProtoBufMsgMemoryPoolBase::CProtoBufMsgMemoryPoolBase(uint32 unTargetLow,
                                                       uint32 unTargetHigh) {
...
CProtoBufMsgMemoryPoolBase::~CProtoBufMsgMemoryPoolBase() {
  AssertMsg(
      ...
      "CProtoBufMsgMemoryPoolBase destructor called with items still in the "
      "queue. They must be freed by the derived class or they will be leaked");
...
::google::protobuf::Message *CProtoBufMsgMemoryPoolBase::Alloc() {
  ...
  if (!m_pTSQueueFreeObjects->PopItem(&pObject) || !pObject) {
    ...
    // We'll free an extra cached msg every alloc if we are over the higher
    // limit, and every 6th if we are over the lower limit.  This allows some
    // elasticity to peaks in demand.
    bool bFreeAnother =
        (unCount > m_unTargetCountHigh) ||
...
void CProtoBufMsgMemoryPoolBase::Free(::google::protobuf::Message *pObject) {
  // We always cache for re-use on free, though we may throw out later on alloc
  // to shrink the pool
  pObject->Clear();
...
uint32 CProtoBufMsgMemoryPoolBase::GetEstimatedSize() {
  ...
  // Should be "while (pObject != NULL)" because if we were actually using
  // threads depending on count would be bad. Unfortunately that gives me an
  // infinite loop. See if this can be changed when this is updated
...
//-----------------------------------------------------------------------------
// Purpose: Takes control of a message pool registered for a specific protobuf
// type
//-----------------------------------------------------------------------------
void CProtoBufMsgMemoryPoolMgr::RegisterPool(
    CProtoBufMsgMemoryPoolBase *pPool) {
...
void CProtoBufMsgMemoryPoolMgr::DumpPoolInfo() {
  ...
  FOR_EACH_VEC(m_vecMsgPools, i) {
    ...
    Msg("%43s%12d%12d%12s%12s\n", m_vecMsgPools[i]->GetName().String(),
        m_vecMsgPools[i]->GetAllocated(), m_vecMsgPools[i]->GetFree(),
        Q_pretifymem((float)unEstimate, 2, true),
        CFmtStr("%f%%", flHitRate).Access());
...
CProtoBufMsgBase::CProtoBufMsgBase()
    : m_pNetPacket(NULL), m_eMsg(0), m_pProtoBufHdr(NULL) {}

//-----------------------------------------------------------------------------
// Purpose: Constructor - Has an eMsg, so it's a send constructor. Alloc a
// header
//-----------------------------------------------------------------------------
CProtoBufMsgBase::CProtoBufMsgBase(MsgType_t eMsg)
    : m_pNetPacket(NULL), m_eMsg(eMsg), m_pProtoBufHdr(NULL) {
...
//-----------------------------------------------------------------------------
CProtoBufMsgBase::~CProtoBufMsgBase() {
  // If we have a net packet then it allocated the header, so we don't free it
  // ourselves If we don't have a net packet then we still may have not
  // allocated a header ( default constructor )
  if (m_pNetPacket) {
    m_pNetPacket->Release();
    m_pNetPacket = NULL;
...
bool CProtoBufMsgBase::InitFromPacket(IMsgNetPacket *pNetPacket) {
  VPROF_BUDGET("CProtoBufMsg::InitFromPacket( IMsgNetPacket )",
               VPROF_BUDGETGROUP_OTHER_NETWORKING);

  AssertMsg(NULL == m_pProtoBufHdr,
            "Encountered a packet being initialized that has outstanding "
            "memory. This will leak memory, and is often caused by receiving "
            "messages using the wrong constructor which preallocates or not "
            "clearing the message between uses");
...
//-----------------------------------------------------------------------------
bool CProtoBufMsgBase::BAsyncSendProto(
    IProtoBufSendHandler &sender, MsgType_t eMsgType,
    const CMsgProtoBufHeader &hdr, const ::google::protobuf::Message &proto) {
  VPROF_BUDGET("CProtoBufMsg::BAsyncSendProto",
               VPROF_BUDGETGROUP_OTHER_NETWORKING);
  // Compute message sizes and serialize out to buffer.  Need to work on a more
  // efficient way to write these to the net without so much copying.
  uint32 cubBodySize = proto.ByteSize();

  uint32 cubTotalSize = 0;
  uint8 *pMsgMem =
      AllocateMessageMemory(eMsgType, hdr, cubBodySize, &cubTotalSize);
...
// Purpose: Sends a message using the given handler
//-----------------------------------------------------------------------------
bool CProtoBufMsgBase::BAsyncSendWithPreSerializedBody(
    IProtoBufSendHandler &pSender, const byte *pubBody, uint32 cubBody) const {
  return BAsyncSendWithPreSerializedBody(pSender, GetEMsg(), *m_pProtoBufHdr,
                                         pubBody, cubBody);
}

//-----------------------------------------------------------------------------
// free standing version to send a protobuff given a header and pre-serialized
// body. Primarily used for efficient message routing
//-----------------------------------------------------------------------------
bool CProtoBufMsgBase::BAsyncSendWithPreSerializedBody(
    IProtoBufSendHandler &sender, MsgType_t eMsgType,
    const CMsgProtoBufHeader &hdr, const byte *pubBody, uint32 cubBody) {
  VPROF_BUDGET("CProtoBufMsg::BAsyncSendWithPreSerializedBody",
               VPROF_BUDGETGROUP_OTHER_NETWORKING);
  // Compute message sizes and serialize out to buffer.  Need to work on a more
  // efficient way to write these to the net without so much copying.
  uint32 cubTotalSize = 0;
...
//-----------------------------------------------------------------------------
// utility function that handles allocating a memory pool big enough for the
// provided header and specified body size and writing the header into the pool.
// This will return a pointer to the memory, as well as the header size.
//-----------------------------------------------------------------------------
uint8 *CProtoBufMsgBase::AllocateMessageMemory(MsgType_t eMsgType,
                                               const CMsgProtoBufHeader &hdr,
                                               uint32 cubBodySize,
                                               uint32 *pCubTotalSizeOut) {
  uint32 cubExtHdrSize = hdr.ByteSize();
  uint32 cubTotalSize =
      sizeof(ProtoBufMsgHeader_t) + cubExtHdrSize + cubBodySize;
...
// We place bytes on the network in little endian to optimize for those
// platforms, even though network byte order is really big endian.
  ...
  hdrByteSwapped.m_cubProtoBufExtHdr =
      DWordSwap(hdrByteSwapped.m_cubProtoBufExtHdr);
  Q_memcpy(pubMsgBytes, &hdrByteSwapped, sizeof(ProtoBufMsgHeader_t));
#else
  Q_memcpy(pubMsgBytes, &fixedHdr, sizeof(ProtoBufMsgHeader_t));
#endif

  // Serialize extended pb header, we guarantee we didn't modify the message
  // since the last call to ByteSize above, so use the cached sizes serialize
  uint8 *pEnd = hdr.SerializeWithCachedSizesToArray(
      pubMsgBytes + sizeof(ProtoBufMsgHeader_t));
  NOTE_UNUSED(pEnd);
