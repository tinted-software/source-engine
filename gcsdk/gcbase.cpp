...
static void OnConVarChangeJobMgrThreadPoolSize(IConVar *pConVar,
                                               const char *pOldString,
                                               float flOldValue);
GCConVar
    jobmgr_threadpool_size("jobmgr_threadpool_size", "-1", 0,
                           "Maximum threads in the job manager thread pool. "
                           "Values <= 0 mean number_logical_cpus - this.",
                           OnConVarChangeJobMgrThreadPoolSize);

static uint32 GetThreadPoolSizeFromConVar() {
  int nVal = jobmgr_threadpool_size.GetInt();
  int nRet =
      (nVal > 0) ? nVal : GetCPUInformation()->m_nLogicalProcessors + nVal;
...
GCConVar cv_concurrent_start_playing_limit("concurrent_start_playing_limit",
                                           "1000");
GCConVar cv_logon_surge_start_playing_limit("logon_surge_start_playing_limit",
                                            "2000");
GCConVar cv_logon_surge_request_session_jobs("logon_surge_request_session_jobs",
                                             "1000");
GCConVar cv_webapi_throttle_job_threshold(
    "webapi_throttle_job_threshold", "2000", 0,
    "If the job count exceeds this threshold, reject low-priority webapi jobs");
GCConVar enable_startplaying_gameserver_creation_spew(
    "enable_startplaying_gameserver_creation_spew", "0");
// Enable the restore-version-from-memcache machinery. Disabled because it
// assumes reloading an SOCache is deterministic, which is no longer true for
// us, resulting in clients with stale versions believing themselves to be in
// sync.
//
// This probably needs a look -- ideally we'd delineate deterministic objects
// that can be assumed to remain in sync in GC reboots, and dynamic objects that
// cannot.
//
// Note that we already removed hacks for this in player groups and started
// using lazy-loaded objects in SOCaches that violate the assumptions this was
// making, so re-enabling it requires work. We probably really want to split
// type caches into deterministic-between-GC-reboots and not, and resend based
// on said flag.
GCConVar socache_persist_version_via_memcached(
    "socache_persist_version_via_memcached", "0");

static GCConVar cv_assert_minidump_window(
    "assert_minidump_window", "28800", 0,
    "Size of the minidump window in seconds. Each unique assert will dump at "
    "most assert_max_minidumps_in_window times in this many seconds");
static GCConVar cv_assert_max_minidumps_in_window(
    "assert_max_minidumps_in_window", "5", 0,
    "The amount of times each unique assert will write a dump in "
    "assert_minidump_window seconds");

static GCConVar
    cv_debug_steam_startplaying("cv_debug_steam_startplaying", "0", 0,
                                "Turn this ON to debug the stream of "
                                "startplaying messages we get from Steam");

static GCConVar temp_list_mismatched_replies(
    "temp_list_mismatched_replies", "0",
    "When set to 1, this report all replies that fail because the incoming "
    "message didn't expect a response. Temporary to help track down some "
    "failed state");

static GCConVar writeback_queue_max_accumulate_time(
    "writeback_queue_max_accumulate_time", "10", 0,
    "The maximum amount of time in seconds that the writeback queue will "
    "accumulate database writes before performing queries. This is the time "
    "*before* the queries are executed, which is unbounded.");
static GCConVar writeback_queue_max_caches(
    "writeback_queue_max_caches", "0", 0,
    "The maximum amount of caches to write back in a single transaction. Set "
    "to zero to remove this restriction.");
static GCConVar
    geolocation_spewlevel("geolocation_spewlevel", "4", 0,
                          "Spewlevel to use for geolocation debug spew");
static GCConVar
    geolocation_loglevel("geolocation_loglevel", "4", 0,
                         "Spewlevel to use for geolocation debug spew");

extern GCConVar max_user_messages_per_second;

// There is also a GCConVar writeback_delay to control how frequently we do
// writebacks.

// !KLUDGE! Temp shim.  Will get rid of this when we bring over the real
// gcinterface stuff from DOTA.
CGCInterface g_GCInterface;
CGCInterface *GGCInterface() { return &g_GCInterface; }
CSteamID
CGCInterface::ConstructSteamIDForClient(AccountID_t unAccountID) const {
...
#ifndef WIN32
    // Non-win32
    bool bRaiseOnAssert = getenv("RAISE_ON_ASSERT") ||
                          !!CommandLine()->FindParm("-raiseonassert");
#elif defined(_DEBUG)
    // Win32 debug
...
public:
  ...
  virtual bool BYieldingRunGCJob() {
    ...
    for (CGCGSSession **ppSession = m_pGC->GetFirstGSSession();
         ppSession != NULL; ppSession = m_pGC->GetNextGSSession(ppSession)) {
    ...
    for (CGCUserSession **ppSession = m_pGC->GetFirstUserSession();
         ppSession != NULL; ppSession = m_pGC->GetNextUserSession(ppSession)) {
      vecIDsToStop.AddToTail((*ppSession)->GetSteamID());
    }
...
GC_REG_JOB(CGCBase, CPreTestSetupJob, "CPreTestSetupJob", k_EGCMsgPreTestSetup,
           k_EServerTypeGC);
...
public:
  ...
  virtual bool BYieldingRunGCJob(GCSDK::IMsgNetPacket *pNetPacket) {
    ...
    if (cv_debug_steam_startplaying.GetBool()) {
      ...
      EmitInfo(SPEW_GC, 1, 1,
               "Received StartPlaying( user = %s, GS = %s @ %s )\n",
               msg.Body().m_steamID.Render(), msg.Body().m_steamIDGS.Render(),
               serverAdr.ToString());
      SpewSerializedKeyValues(msg.PubVarData(), msg.CubVarData());
    }
    m_pGC->QueueStartPlaying(
        msg.Body().m_steamID, msg.Body().m_steamIDGS, msg.Body().m_unServerAddr,
        msg.Body().m_usServerPort, msg.PubVarData(), msg.CubVarData());

    return true;
  }
};

GC_REG_JOB(CGCBase, CStartPlayingJob, "CStartPlayingJob", k_EGCMsgStartPlaying,
           k_EServerTypeGC);
...
GC_REG_JOB(CGCBase, CStopPlayingJob, "CStopPlayingJob", k_EGCMsgStopPlaying,
           k_EServerTypeGC);
...
public:
  ...
  virtual bool BYieldingRunGCJob(GCSDK::IMsgNetPacket *pNetPacket) {
    ...
    m_pGC->QueueStartPlaying(
        msg.Body().m_steamID, CSteamID(), msg.Body().m_unServerAddr,
        msg.Body().m_usServerPort, msg.PubVarData(), msg.CubVarData());
...
GC_REG_JOB(CGCBase, CStartGameserverJob, "CStartGameserverJob",
           k_EGCMsgStartGameserver, k_EServerTypeGC);
...
GC_REG_JOB(CGCBase, CStopGameserverJob, "CStopGameserverJob",
           k_EGCMsgStopGameserver, k_EServerTypeGC);
...
public:
  ...
  virtual bool BYieldingRunGCJob(GCSDK::IMsgNetPacket *pNetPacket) {
    ...
    CProtoBufMsg<CGCMsgGetSystemStatsResponse> msgResponse(
        k_EGCMsgGetSystemStatsResponse);
    msgResponse.Body().set_gc_app_id(m_pGC->GetAppID());

    // @note Tom Bui: we don't support dynamic stats yet, but once we do, we can
    // use the KV stuff
    ...
    //		{
    //			msgResponse.Body().set_stats_kv( buffer.Base(),
    //buffer.TellPut() );
    //		}
...
GC_REG_JOB(CGCBase, CGetSystemStatsJob, "CGetSystemStatsJob",
           k_EGCMsgGetSystemStats, k_EServerTypeGC);
...
public:
  ...
  bool BYieldingRunJobFromMsg(IMsgNetPacket *pNetPacket) {
    ...
    const CAccountDetails *pAccountDetails =
        GGCBase()->YieldingGetAccountDetails(steamID, bForceReload);

    // Account details is up to date so just return
    if (pAccountDetails && bIsVacBanned != pAccountDetails->BIsVacBanned()) {
      EmitWarning(
          SPEW_GC, 2,
          "VAC status didn't update for %s afetr receiving VacStatusChange and "
          "the force reloading the account details\n",
          steamID.Render());
    }
    return true;
  }
};
GC_REG_JOB(CGCBase, CGCJobAccountVacStatusChange,
           "CGCJobAccountVacStatusChange", k_EGCMsgGCAccountVacStatusChange,
           k_EServerTypeGC);
...
public:
  ...
  bool BYieldingRunJobFromMsg(IMsgNetPacket *pNetPacket) {
    ...
    // Account details is up to date so just return
    if (pAccountDetails &&
        (bHasPhoneVerified != pAccountDetails->BIsPhoneVerified() ||
         bIsPhoneIdentifying != pAccountDetails->BIsPhoneIdentifying())) {
      EmitWarning(SPEW_GC, 2,
                  "Phone status didn't update for %s afetr receiving "
                  "PhoneNumberChange and force reloading the account details\n",
                  steamID.Render());
    }
...
GC_REG_JOB(CGCBase, CGCJobAccountPhoneNumberChange,
           "CGCJobAccountPhoneNumberChange", k_EGCMsgAccountPhoneNumberChange,
           k_EServerTypeGC);
...
public:
  ...
  bool BYieldingRunJobFromMsg(IMsgNetPacket *pNetPacket) {
    ...
    // Account details is up to date so just return
    if (pAccountDetails &&
        bHasTwoFactor != pAccountDetails->BIsTwoFactorAuthEnabled()) {
      EmitWarning(
          SPEW_GC, 2,
          "VAC status didn't update for %s afetr receiving VacStatusChange and "
          "the force reloading the account details\n",
...
GC_REG_JOB(CGCBase, CGCJobAccountTwoFactorChange,
           "CGCJobAccountTwoFactorChange", k_EGCMsgAccountTwoFactorChange,
           k_EServerTypeGC);
...
//-----------------------------------------------------------------------------
CGCBase::CGCBase()
    : m_mapSOCache(), m_rbtreeSOCachesBeingLoaded(DefLessFunc(CSteamID)),
      ...
      m_bStartupComplete(false), m_bIsShuttingDown(false),
      m_bStartProfiling(false), m_bStopProfiling(false),
      m_bDumpVprofImbalances(false), m_nStartPlayingJobCount(0),
      m_nRequestSessionJobsActive(0),
      m_nLogonSurgeFramesRemaining(
          k_nMillion * 10 /
          k_cMicroSecPerShellFrame), // stay in "logon surge" mode for at least
...
//-----------------------------------------------------------------------------
bool CGCBase::BInit(AppId_t unAppID, const char *pchAppPath,
                    IGameCoordinatorHost *pHost) {
...
// Make sure we can't deploy debug GCs outside the dev environment
  ...
  if (pHost->GetUniverse() != k_EUniverseDev) {
    ...
    //unAppID ) ); return false;
    pHost->EmitMessage(
        SPEW_GC.GetName(), SPEW_WARNING, SPEW_ALWAYS, LOG_ALWAYS,
  ...
  MsgRegistrationFromEnumDescriptor(EGCSystemMsg_descriptor(),
                                    GCSDK::MT_GC_SYSTEM);
  MsgRegistrationFromEnumDescriptor(EGCBaseClientMsg_descriptor(),
                                    GCSDK::MT_GC);
  MsgRegistrationFromEnumDescriptor(EGCToGCMsg_descriptor(),
                                    GCSDK::MT_GC_SYSTEM);
  ...
  // Make sure the assert dialog doesn't come up and hang the process in
  // production
  // SetAssertDialogDisabled( pHost->GetUniverse() != k_EUniverseDev );
  SetAssertFailedNotifyFunc(CGCBase::AssertCallbackFunc);

  // init the time very early so CRTime::RTime32TimeCur will return the right
  // thing
  ...
  // when we do, we'll need to specify the how the values are aggregated over
  all the same GCs
  ...
  if ( packer.WriteAsBinary( pKVStats, buffer ) )
  {
          CProtoBufMsg< CGCMsgSystemStatsSchema > msg(
  GCSDK::k_EGCMsgSystemStatsSchema ); msg.Body().set_gc_app_id( GetAppID() );
...
void CGCBase::SetStartupComplete(bool bSuccess) {
  ...
  if (!bSuccess) {
    ...
    EmitError(
        SPEW_GC,
        "GC failed to startup.  Error mesage is probably directly above\n");
    EmitError(
        SPEW_GC,
        "**************************************************************\n");
  }
...
void CGCBase::Uninit() {
  ...
  // clean up all of the sessions and caches here so we can be sure it happens
  // before the memory pools go away at static destruction time
  for (CGCUserSession **ppSession = m_hashUserSessions.PvRecordFirst();
       ppSession != NULL;
       ppSession = m_hashUserSessions.PvRecordNext(ppSession)) {
    delete (*ppSession);
  }
  m_hashUserSessions.RemoveAll();
  for (CGCGSSession **ppSession = m_hashGSSessions.PvRecordFirst();
       ppSession != NULL;
       ppSession = m_hashGSSessions.PvRecordNext(ppSession)) {
    delete (*ppSession);
  }
...
GCConVar cv_flush_inventory_cache_jobs(
    "cv_flush_inventory_cache_jobs", "20", 0,
    "The maximum number of jobs flushing inventory caches that can be in "
    "flight at once, zero to disable flushing");
GCConVar cv_flush_inventory_cache_contextid(
    "cv_flush_inventory_cache_contextid", "2" /* k_EEconContextBackpack */, 0,
    "Which context id we flush for Steam web user-facing inventory");
GCConVar cv_flush_inventory_cache_spew(
    "cv_flush_inventory_cache_spew", "0", 0,
    "Controls spew level for jobs flushing inventory cache (0=off; 1=summary; "
    "2=verbose)");
class CFlushInventoryCacheAccountsJob : public CGCJob,
                                        public IYieldingParallelFarmJobHandler {
public:
  CFlushInventoryCacheAccountsJob(
      CGCBase *pGC,
      CUtlRBTree<AccountID_t, int32, CDefLess<AccountID_t>> &rbAccounts)
      : CGCJob(pGC) {
  ...
  virtual bool BYieldingRunGCJob() OVERRIDE {
    ...
    { // Run parallel processing of the workload
      ...
      (void)BYieldingExecuteParallel(numJobs,
                                     "YieldingFlushInventoryCacheAccountsJob");
    }

    if (bShouldSpew) {
      EmitInfo(SPEW_GC, SPEW_ALWAYS, LOG_ALWAYS,
               "IEconService/FlushInventoryCache: Batch for %d accounts "
               "completed in %u ms\n",
  ...
  virtual bool BYieldingRunWorkload(int iJobSequenceCounter,
                                    bool *pbWorkloadCompleted) OVERRIDE {
    if (m_rbAccounts.Count()) {
  ...
  bool BYieldingFlushRequest(AccountID_t unAccountID) {
    ...
    CSteamAPIRequest apiRequest(k_EHTTPMethodPOST, "IEconService",
                                "FlushInventoryCache", 1);
    ...
    bool bSucceededQuery =
        m_pGC->BYieldingSendHTTPRequest(&apiRequest, &apiResponse);
    if (!bSucceededQuery) {
      EmitErrorRatelimited(SPEW_GC,
                           "IEconService/FlushInventoryCache: Web call did not "
                           "get a response for %s.\n",
                           steamID.Render());
    } else if (k_EHTTPStatusCode200OK != apiResponse.GetStatusCode()) {
      EmitErrorRatelimited(SPEW_GC,
                           "IEconService/FlushInventoryCache: Web call got "
                           "failure code %d for %s\n",
    ...
    if (bSucceededQuery) {
      ...
      if (!pKVResponse->LoadFromBuffer("webResponse",
                                       *apiResponse.GetBodyBuffer())) {
        EmitErrorRatelimited(SPEW_GC,
                             "IEconService/FlushInventoryCache: Web call got "
                             "code %d for %s, but failed to parse response\n",
                             apiResponse.GetStatusCode(), steamID.Render());
        bSucceededQuery = false;
    ...
    if (bShouldSpew) {
      EmitInfo(
          SPEW_GC, SPEW_ALWAYS, LOG_ALWAYS,
          "IEconService/FlushInventoryCache: Web call for %s %s in %u ms\n",
          steamID.Render(), bSucceededQuery ? "succeeded" : "failed",
          Plat_MSTime() - msTimeStart);
    }
...
bool CGCBase::BMainLoopOncePerFrame(uint64 ulLimitMicroseconds) {
  ...
  if (!g_VProfCurrentProfile.AtRoot() && m_bDumpVprofImbalances) {
    EmitWarning(SPEW_GC, SPEW_ALWAYS,
                "VProf not at root at end of frame. Stack:\n");
  }
  ...
  if (m_llStartPlaying.Count() > 0) {
    ...
    int nJobsNeeded = min(m_llStartPlaying.Count(),
                          cv_concurrent_start_playing_limit.GetInt() -
  ...
  // This might be a good idea, but let's see what the real numbers are during
  // logon surge.
  //|| m_nRequestSessionJobsActive >=
  //cv_logon_surge_request_session_jobs.GetInt();

  // Check if we're already in logon surge, is it time to check if we should
  // leave, and should we dump our status periodically?
  const int k_nLogonSurgeFrameInterval =
      k_nMillion * 10 / k_cMicroSecPerShellFrame;
  ...
  if (m_rbFlushInventoryCacheAccounts.Count() &&
      ((m_jobidFlushInventoryCacheAccounts == k_GIDNil) ||
       !GetJobMgr().BJobExists(m_jobidFlushInventoryCacheAccounts))) {
    m_numFlushInventoryCacheAccountsLastScheduled =
        m_rbFlushInventoryCacheAccounts.Count();
    m_jobidFlushInventoryCacheAccounts =
        StartNewJobDelayed(new CFlushInventoryCacheAccountsJob(
                               this, m_rbFlushInventoryCacheAccounts))
            ->GetJobID();
  }
...
bool CGCBase::BMainLoopUntilFrameCompletion(uint64 ulLimitMicroseconds) {
...

  {
    ...
    for (CLock *pLock = m_hashSteamIDLocks.PvRecordRun(); NULL != pLock;
         pLock = m_hashSteamIDLocks.PvRecordRun()) {
      if (!pLock->BIsLocked() &&
          pLock->GetMicroSecondsSinceLock() > k_cMicroSecLockLifetime) {
...
//			are about to start over with the first session in the
//list.
//-----------------------------------------------------------------------------
...
//			get a chance.
//-----------------------------------------------------------------------------
void CGCBase::QueueStartPlaying(const CSteamID &steamID,
                                const CSteamID &gsSteamID, uint32 unServerAddr,
                                uint16 usServerPort, const uint8 *pubVarData,
                                uint32 cubVarData) {
  ...
  Assert((size_t)m_mapStartPlayingQueueIndexBySteamID.Count() ==
         (size_t)m_llStartPlaying.Count());
  ...
  if (nMapIndex != m_mapStartPlayingQueueIndexBySteamID.InvalidIndex()) {
    // We already have an entry for this guy, let's update this one, rather than
    // creating a new one
    ...
    if (pWork->m_steamID == steamID) {
      ...
      //			// This could definitely happen occasionally,
      //but if it happens with massive frequency,
      //			// something is wrong
      //			if ( gsSteamID == pWork->m_gsSteamID )
      //			{
      //				EmitInfo( SPEW_GC, 4, LOG_ALWAYS, "Got
      //StartPlaying message for %s, who was already in the startplaying queue
      //for the same gameserver %s.\n", steamID.Render(), gsSteamID.Render() );
      //			}
      //			else
      //			{
      //				EmitInfo( SPEW_GC, 4, LOG_ALWAYS, "Got
      //StartPlaying message for %s, who was already in the startplaying queue;
      //changing gameserver %s -> %s.\n", steamID.Render(),
      //pWork->m_gsSteamID.Render(), gsSteamID.Render() );
      //			}
    } else {
      EmitWarning(SPEW_GC, SPEW_ALWAYS,
                  "Start playing queue corruption!  Map entry points to wrong "
                  "queue entry!\n");
      pWork = NULL;
      m_mapStartPlayingQueueIndexBySteamID.RemoveAt(nMapIndex);
    }
  } else {
    //		EmitInfo( SPEW_GC, 4, LOG_ALWAYS, "Got StartPlaying message for
    //%s, new queue for gameserver %s.\n", steamID.Render(), gsSteamID.Render()
    //);
  }
  ...
  if (pWork == NULL) {
    ...
    // Add it to the steam ID map, so we can locate this guy quickly in the
    // future
...
bool CGCBase::BRemoveStartPlayingQueueEntry(const CSteamID &steamID) {
  ...
  // EmitInfo( SPEW_GC, 4, LOG_ALWAYS, "Removed startplaying queue entry for
  // %s.\n", steamID.Render() );
  ...
  if (m_llStartPlaying[nQueueIndex].m_steamID == steamID) {
    ...
    EmitWarning(SPEW_GC, SPEW_ALWAYS,
                "Start playing queue corruption!  Map entry doesn't point to "
                "matching queue index (found while removing entry in "
                "BRemoveStartPlayingQueueEntry)!\n");
  }
...
//-----------------------------------------------------------------------------
// Purpose: Executes a single entry from the start playing queue, given the
// linked list handle
//-----------------------------------------------------------------------------
void CGCBase::YieldingExecuteStartPlayingQueueEntryByIndex(
    int idxStartPlayingQueue) {
  VPROF_BUDGET(
      "CGCBase::YieldingExecuteStartPlayingQueueEntryByIndex - LinkedList",
      VPROF_BUDGETGROUP_STEAM);
  // Remove the entry from the queue
  StartPlayingWork_t work = m_llStartPlaying[idxStartPlayingQueue];
  m_llStartPlaying.Remove(idxStartPlayingQueue);

  VPROF_BUDGET("CGCBase::YieldingExecuteStartPlayingQueueEntryByIndex",
               VPROF_BUDGETGROUP_STEAM);
  ...
  if (nMapIndex == m_mapStartPlayingQueueIndexBySteamID.InvalidIndex()) {
    EmitWarning(
        SPEW_GC, SPEW_ALWAYS,
        "Start playing queue corruption!  Queue entry is not in map!\n");
  } else if (m_mapStartPlayingQueueIndexBySteamID[nMapIndex] !=
             idxStartPlayingQueue) {
    EmitWarning(SPEW_GC, SPEW_ALWAYS,
                "Start playing queue corruption!  Map entry doesn't have "
                "proper queue index!\n");
  } else {
    m_mapStartPlayingQueueIndexBySteamID.RemoveAt(nMapIndex);
  }
  ...
  if (work.m_steamID.BIndividualAccount()) {
    YieldingStartPlaying(work.m_steamID, work.m_gsSteamID, work.m_unServerAddr,
                         work.m_usServerPort, work.m_pVarData);
    ...
    YieldingStartGameserver(work.m_steamID, work.m_unServerAddr,
                            work.m_usServerPort, pVarData, cubVarData);
  } else {
    AssertMsg1(false, "Bogus steam ID %s in start playing queue",
               work.m_steamID.Render());
  }
...
void CGCBase::SetUserSessionDetails(CGCUserSession *pUserSession,
                                    KeyValues *pkvDetails) {
  if (pkvDetails) {
    pUserSession->m_unIPPublic = pkvDetails->GetInt("ip", 0);
    pUserSession->m_osType =
        static_cast<EOSType>(pkvDetails->GetInt("osType", k_eOSUnknown));
    pUserSession->m_bIsTestSession =
        pkvDetails->GetInt("isTestSession", 0) != 0;
...
//-----------------------------------------------------------------------------
void CGCBase::YieldingStartPlaying(const CSteamID &steamID,
                                   const CSteamID &gsSteamID,
                                   uint32 unServerAddr, uint16 usServerPort,
                                   CUtlBuffer *pVarData) {
  ...
  // if var data came with this StartPlaying message, parse it into a KV and
  ...
  if (!pSession) {
    ...
    VPROF_BUDGET("CGCBase::YieldingStartPlaying - Load SOCache",
                 VPROF_BUDGETGROUP_STEAM);
    ...
    VPROF_BUDGET("CGCBase::YieldingStartPlaying - CreateUserSession",
                 VPROF_BUDGETGROUP_STEAM);
    ...
    VPROF_BUDGET("CGCBase::YieldingStartPlaying - LRU Update",
                 VPROF_BUDGETGROUP_STEAM);
    RemoveCacheFromLRU(pSOCache);

    CGCUserSession **ppSession =
        m_hashUserSessions.PvRecordInsert(steamID.ConvertToUint64());
    *ppSession = pSession;

    SetUserSessionDetails(pSession, pkvDetails);

    // Do game-specific logic here.  Note that we're still holding the game
    // server lock...
    VPROF_BUDGET("CGCBase::YieldingStartPlaying - Game-specific start playing",
                 VPROF_BUDGETGROUP_STEAM);
  ...
  VPROF_BUDGET("CGCBase::YieldingStartPlaying - Game Server binding",
               VPROF_BUDGETGROUP_STEAM);
  // Make sure the server exists and then try to join it
  if (gsSteamID.IsValid() && gsSteamID.BGameServerAccount() &&
      BYieldingLockSteamID(gsSteamID, __FILE__, __LINE__)) {
    ...
    if (YieldingGetLockedGSSession(gsSteamID, __FILE__, __LINE__) != NULL) {
      ...
      if (enable_startplaying_gameserver_creation_spew.GetBool()) {
        ...
        EmitInfo(SPEW_GC, 2, LOG_ALWAYS,
                 "Creating gameserver session %s @ %s as a result of user %s "
                 "StartPlaying.\n",
                 gsSteamID.Render(), serverAdr.ToString(), steamID.Render());
      }
      YieldingFindOrCreateGSSession(gsSteamID, unServerAddr, usServerPort, NULL,
                                    0);
    }
    ...
    // Steam was sometimes sending us messages with zero Steam ID, even when
    // we're on a server.
    if (cv_debug_steam_startplaying.GetBool())
      EmitInfo(SPEW_GC, 1, 1,
               "YieldingStartPlaying ( user = %s ) with invalid GS steam ID "
               "%s, calling LeaveServer\n",
               steamID.Render(), gsSteamID.Render());
...
//-----------------------------------------------------------------------------
void CGCBase::YieldingStartGameserver(const CSteamID &steamID,
                                      uint32 unServerAddr, uint16 usServerPort,
                                      const uint8 *pubVarData,
                                      uint32 cubVarData) {
  ...
  YieldingFindOrCreateGSSession(steamID, unServerAddr, usServerPort, pubVarData,
                                cubVarData);
...
IMsgNetPacket *CreateIMsgNetPacket(GCProtoBufMsgSrc eReplyType,
                                   const CSteamID senderID, uint32 nGCDirIndex,
                                   uint32 unMsgType, void *pubData,
                                   uint32 cubData) {
  ...
  if (0 != (unMsgType & k_EMsgProtoBufFlag)) {
    if (cubData < sizeof(ProtoBufMsgHeader_t)) {
      ...
      AssertMsg3(
          false,
          "Received packet %s(%u) from %s less than the minimum protobuf size",
          PchMsgNameFromEMsg(unMsgTypeNoFlag), unMsgTypeNoFlag,
          senderID.Render());
      return NULL;
    }
    ...
    CProtoBufNetPacket *pMsgNetPacket =
        new CProtoBufNetPacket(pGCPacket, eReplyType, senderID, nGCDirIndex,
    ...
    // note that we do not currently support reply to GC messages through this
    // pipeline
    AssertMsg(eReplyType != GCProtoBufMsgSrc_FromGC,
              "Warning: Encountered a message from GC to GC that was not of "
              "protobuff type, will be unable to reply to this message. "
              "Message type: %d",
              unMsgType);
    ...
    if (cubData < sizeof(GCMsgHdrEx_t) - sizeof(GCMsgHdr_t)) {
      AssertMsg(
          false,
          "Received packet %s(%u) from %s less than the minimum struct size",
          PchMsgNameFromEMsg(unMsgType), unMsgType, senderID.Render());
      return NULL;
    }

    // Determine the size of the packet. sizeof(GCMsgHdr_t) was not sent as part
    // of the data
...
//-----------------------------------------------------------------------------
void CGCBase::MessageFromClient(const CSteamID &senderID, uint32 unMsgType,
                                void *pubData, uint32 cubData) {
  VPROF_BUDGET("CGCBase::MessageFromClient", VPROF_BUDGETGROUP_STEAM);

  // if we don't have a GCHost yet, we won't be able to do much with this
  // message
  if (!GGCHost())
    return;
  ...
  IMsgNetPacket *pMsgNetPacket =
      CreateIMsgNetPacket(GCProtoBufMsgSrc_FromSteamID, senderID, nGCDirIndex,
                          unMsgType, pubData, cubData);
  ...
  default:
    GetJobMgr().BRouteMsgToJob(
        this, pMsgNetPacket,
        JobMsgInfo_t(pMsgNetPacket->GetEMsg(), pMsgNetPacket->GetSourceJobID(),
                     pMsgNetPacket->GetTargetJobID(), k_EServerTypeGC));
    g_theMessageList.TallySendMessage(pMsgNetPacket->GetEMsg(), cubData);
    break;
  }
...
//-----------------------------------------------------------------------------
bool CGCBase::BSendGCMsgToClient(const CSteamID &steamIDTarget,
                                 const CGCMsgBase &msg) {
  g_theMessageList.TallySendMessage(msg.Hdr().m_eMsg,
                                    msg.CubPkt() - sizeof(GCMsgHdr_t));
  VPROF_BUDGET("GCHost", VPROF_BUDGETGROUP_STEAM);
  {
    VPROF_BUDGET("GCHost - SendMessageToClient", VPROF_BUDGETGROUP_STEAM);
    return m_pHost->BSendMessageToClient(
        m_unAppID, steamIDTarget, msg.Hdr().m_eMsg,
        msg.PubPkt() + sizeof(GCMsgHdr_t), msg.CubPkt() - sizeof(GCMsgHdr_t));
  }
}
...
//-----------------------------------------------------------------------------
class CProtoBufClientSendHandler
    : public CProtoBufMsgBase::IProtoBufSendHandler {
public:
  CProtoBufClientSendHandler(const CSteamID &steamIDTarget)
      : m_steamIDTarget(steamIDTarget), m_cubSent(0) {}
  virtual bool BAsyncSend(MsgType_t eMsg, const uint8 *pubMsgBytes,
                          uint32 cubSize) OVERRIDE {
    m_cubSent = cubSize;
    // !FIXME! DOTAMERGE
    // return GGCInterface()->BProcessSystemMessage( eMsg | k_EMsgProtoBufFlag,
    // pubMsgBytes, cubSize );
    g_theMessageList.TallySendMessage(eMsg & ~k_EMsgProtoBufFlag, cubSize);
    VPROF_BUDGET("GCHost", VPROF_BUDGETGROUP_STEAM);
    {
      VPROF_BUDGET("GCHost - SendMessageToClient (ProtoBuf)",
                   VPROF_BUDGETGROUP_STEAM);
      return GGCHost()->BSendMessageToClient(
          GGCBase()->GetAppID(), m_steamIDTarget, eMsg | k_EMsgProtoBufFlag,
          pubMsgBytes, cubSize);
    }
  }
...
//-----------------------------------------------------------------------------
class CProtoBufSystemSendHandler
    : public CProtoBufMsgBase::IProtoBufSendHandler {
public:
  CProtoBufSystemSendHandler() : m_cubSent(0) {}
  virtual bool BAsyncSend(MsgType_t eMsg, const uint8 *pubMsgBytes,
                          uint32 cubSize) OVERRIDE {
    m_cubSent = cubSize;
    // !FIXME! DOTAMERGE
    ...
    VPROF_BUDGET("GCHost", VPROF_BUDGETGROUP_STEAM);
    {
      VPROF_BUDGET("GCHost - SendMessageToSystem (ProtoBuf)",
                   VPROF_BUDGETGROUP_STEAM);
      return GGCHost()->BSendMessageToClient(GGCBase()->GetAppID(), CSteamID(),
                                             eMsg | k_EMsgProtoBufFlag,
                                             pubMsgBytes, cubSize);
    }
  }
  uint32 GetCubSent() const { return m_cubSent; }
...
bool CGCBase::BSendSystemMessage(const CGCMsgBase &msg, uint32 *pcubSent) {
  ...
  // !FIXME! DOTAMERGE
  // return GGCInterface()->BProcessSystemMessage( msg.Hdr().m_eMsg,
  // msg.PubPkt() + sizeof(GCMsgHdr_t), cubSent );
...
// Purpose: Sends a system message to the GC Host
//-----------------------------------------------------------------------------
bool CGCBase::BSendSystemMessage(const CProtoBufMsgBase &msg,
                                 uint32 *pcubSent) {
...
bool CGCBase::BSendSystemMessage(const ::google::protobuf::Message &msgOut,
                                 MsgType_t eSendMsg) {
  CProtoBufSystemSendHandler sender;
...
// Purpose: send msgOut to the place that msgIn came from
//-----------------------------------------------------------------------------
bool CGCBase::BReplyToMessage(CProtoBufMsgBase &msgOut,
                              const CProtoBufMsgBase &msgIn) {
  // Don't reply if the source is not expecting it
...
// Purpose: Sends a message to the given SteamID
//-----------------------------------------------------------------------------
bool CGCBase::BSendGCMsgToClientWithPreSerializedBody(
    const CSteamID &steamIDTarget, MsgType_t eMsgType,
    const CMsgProtoBufHeader &hdr, const byte *pubBody, uint32 cubBody) const {
  CProtoBufClientSendHandler sender(steamIDTarget);
  return CProtoBufMsgBase::BAsyncSendWithPreSerializedBody(
      sender, eMsgType, hdr, pubBody, cubBody);
}

//-----------------------------------------------------------------------------
// Purpose: Sends a message that has already been packed to the system handler
//-----------------------------------------------------------------------------
bool CGCBase::BSendGCMsgToSystemWithPreSerializedBody(
    MsgType_t eMsgType, const CMsgProtoBufHeader &hdr, const byte *pubBody,
...
// Purpose: send msgOut to the place that msgIn came from
//-----------------------------------------------------------------------------
bool CGCBase::BReplyToMessageWithPreSerializedBody(
    MsgType_t eMsgType, const CProtoBufMsgBase &msgIn, const byte *pubBody,
    uint32 cubBody) const {
  // Don't reply if the source is not expecting it
  if (!msgIn.GetJobIDSource())
    return true;
  ...
  if (temp_list_mismatched_replies.GetBool() && !msgIn.BIsExpectingReply()) {
    EG_MSG(g_EGMessages,
           "Message %s was sent to client %s which did not expect a reply\n",
  ...
  if (bSystemReply) {
    return BSendGCMsgToSystemWithPreSerializedBody(eMsgType, hdr, pubBody,
                                                   cubBody);
  } else {
    return BSendGCMsgToClientWithPreSerializedBody(
        msgIn.GetClientSteamID(), eMsgType, hdr, pubBody, cubBody);
  }
}
...
bool CGCBase::BYldSendMessageAndGetReply(const CSteamID &steamIDTarget,
                                         CProtoBufMsgBase &msgOut,
                                         CProtoBufMsgBase *pMsgIn,
                                         MsgType_t eMsg) {
  CJob &curJob = GJobCur();
...
// bool CGCBase::BYldSendGCMessageAndGetReply( int32 nGCDirIndex,
// CProtoBufMsgBase &msgOut, CProtoBufMsgBase *pMsgIn, MsgType_t eMsg )
//{
//	CJob& curJob = GJobCur();
//	msgOut.ExpectingReply( curJob.GetJobID() );
//
...
bool CGCBase::BYldSendSystemMessageAndGetReply(CGCMsgBase &msgOut,
                                               CGCMsgBase *pMsgIn,
                                               MsgType_t eMsg) {
  CJob &curJob = GJobCur();
  msgOut.ExpectingReply(curJob.GetJobID());
...
bool CGCBase::BYldSendSystemMessageAndGetReply(CProtoBufMsgBase &msgOut,
                                               CProtoBufMsgBase *pMsgIn,
                                               MsgType_t eMsg) {
  CJob &curJob = GJobCur();
  msgOut.ExpectingReply(curJob.GetJobID());
...
bool CGCBase::BYldSendSystemMessageAndGetReply(
    const ::google::protobuf::Message &msgSend, MsgType_t eSendMsg,
    ::google::protobuf::Message *pMsgResponse, MsgType_t eRespondMsg) {
  CJob &curJob = GJobCur();
  ...
  return curJob.BYieldingWaitForMsg(
      &protoMsg,
      eRespondMsg); // !FIXME! For some reason system replies are coming back
                    // with a universe and instance set (but account ID zero).
}
...
//-----------------------------------------------------------------------------
CGCUserSession *
CGCBase::CreateUserSession(const CSteamID &steamID,
                           CGCSharedObjectCache *pSOCache) const {
...
// Purpose: Creates a new session for the steam ID
//-----------------------------------------------------------------------------
CGCGSSession *CGCBase::CreateGSSession(const CSteamID &steamID,
                                       CGCSharedObjectCache *pSOCache,
                                       uint32 unServerAddr,
                                       uint16 usServerPort) const {
...
//			if the lock could not be granted or if the session could
//not be 			found.
//-----------------------------------------------------------------------------
CGCUserSession *CGCBase::YieldingGetLockedUserSession(const CSteamID &steamID,
  ...
  CGCUserSession *pSession = FindUserSession(steamID);
  if (!pSession) {
    // EmitInfo( SPEW_GC, SPEW_ALWAYS, LOG_ALWAYS, "Unable to find session %s to
    // lock it. Attempting to fetch it from the AM\n", steamID.Render() );
...
bool CGCBase::BUserSessionPending(const CSteamID &steamID) const {
  int nStartPlayingMapIndex =
      m_mapStartPlayingQueueIndexBySteamID.Find(steamID);
  return (nStartPlayingMapIndex !=
          m_mapStartPlayingQueueIndexBySteamID.InvalidIndex());
}
...
CGCUserSession *CGCBase::FindUserSession(const CSteamID &steamID) const {
  ...
  if (!steamID.IsValid()) {
    AssertMsg1(steamID.IsValid(),
               "CGCBase::FindUserSession was passed invalid Steam ID %s",
               steamID.Render());
    return NULL;
  }
  if (!steamID.BIndividualAccount()) {
    AssertMsg1(steamID.BIndividualAccount(),
               "CGCBase::FindUserSession was passed non-individual Steam ID %s",
               steamID.Render());
    return NULL;
  }

  CGCUserSession **ppSession =
      m_hashUserSessions.PvRecordFind(steamID.ConvertToUint64());
  if (ppSession) {
...
//-----------------------------------------------------------------------------
// Purpose: Returns true if the session associated with the steam id is online,
// false otherwise
//-----------------------------------------------------------------------------
bool CGCBase::BYieldingIsOnline(const CSteamID &steamID) {
  ...
  if (!GJobCur().BYieldingWaitForMsg(&msgReply,
                                     k_EGCMsgValidateSessionResponse)) {
    EmitWarning(SPEW_GC, LOG_ALWAYS,
                "Didn't get reply from AM for %s in YieldingRequestSession\n",
                steamID.Render());
    return false;
  }
...
//-----------------------------------------------------------------------------
template <typename T> class CScopedIncrement {
public:
...
CGCSession *CGCBase::YieldingRequestSession(const CSteamID &steamID) {
  ...
  int nStartPlayingMapIndex =
      m_mapStartPlayingQueueIndexBySteamID.Find(steamID);
  if (nStartPlayingMapIndex !=
      m_mapStartPlayingQueueIndexBySteamID.InvalidIndex()) {

    // Sanity
    int idxStartPlayingQueue =
        m_mapStartPlayingQueueIndexBySteamID[nStartPlayingMapIndex];
  ...
  if (steamID.BIndividualAccount()) {
    if (msgReply.Body().m_bOnline) {
      ...
      // Check if they have an entry in the startplaying queue, then get rid of
      // it! They data we just received is the most up-to-date we have.  We
      // should prefer this data over anything in the queue for sure.
      BRemoveStartPlayingQueueEntry(steamID);

      YieldingStartPlaying(steamID, msgReply.Body().m_ulSteamIDGS,
                           msgReply.Body().m_unServerAddr,
                           msgReply.Body().m_usServerPort,
                           msgReply.CubVarData() ? &bufVarData : NULL);
      return FindUserSession(steamID);
    } else {
      // EmitWarning( SPEW_GC, LOG_ALWAYS, "Reply from AM is logging off %s in
      // YieldingRequestSession\n", steamID.Render() );
      YieldingStopPlaying(steamID);
      return NULL;
    }
  } else {
    if (msgReply.Body().m_bOnline) {
      YieldingStartGameserver(steamID, msgReply.Body().m_unServerAddr,
                              msgReply.Body().m_usServerPort,
                              msgReply.PubVarData(), msgReply.CubVarData());
      return FindGSSession(steamID);
    } else {
      // EmitWarning( SPEW_GC, LOG_ALWAYS, "Reply from AM is stopping %s in
      // YieldingRequestSession\n", steamID.Render() );
...
//			request to steam itself, but it could be a request on a
//more 			remote server.
//-----------------------------------------------------------------------------
bool CGCBase::BYieldingSendHTTPRequest(const CHTTPRequest *pRequest,
  ...
  if (!BYldSendSystemMessageAndGetReply(pRequest->GetProtoObj(),
                                        k_EGCMsgSendHTTPRequest, &msgResponse,
                                        k_EGCMsgSendHTTPRequestResponse)) {
    ReportHTTPError(CFmtStr("No response to HTTP system message for %s",
                            pRequest->GetURL()),
                    CGCEmitGroup::kMsg_Error);
    return false;
  }

  if (!msgResponse.has_status_code()) {
    ReportHTTPError(
        CFmtStr("No status code on HTTP response for %s", pRequest->GetURL()),
        CGCEmitGroup::kMsg_Error);
    return false;
  }
  ...
  if (msgResponse.status_code() != k_EHTTPStatusCode200OK) {
    ReportHTTPError(CFmtStr("Invalid status code %u for %s",
                            msgResponse.status_code(), pRequest->GetURL()),
                    CGCEmitGroup::kMsg_Warning);
  } else {
    ReportHTTPError(CFmtStr("Success status code for %s", pRequest->GetURL()),
                    CGCEmitGroup::kMsg_Verbose);
  }
...
//-----------------------------------------------------------------------------
EResult CGCBase::YieldingSendHTTPRequestKV(const CHTTPRequest *pRequest,
                                           KeyValues *pKVResponse) {
  CHTTPResponse apiResponse;
...
//not be 			found.
//-----------------------------------------------------------------------------
CGCGSSession *CGCBase::YieldingGetLockedGSSession(const CSteamID &steamID,
                                                  const char *pszFilename,
                                                  int nLineNum) {
...
void CGCBase::ReportHTTPError(const char *pszError,
                              CGCEmitGroup::EMsgLevel eLevel) {
  ...
  if (nIndex != m_HTTPErrors.InvalidIndex()) {
    ...
    m_HTTPErrors[nIndex]->m_eSeverity =
        MIN(eLevel, m_HTTPErrors[nIndex]->m_eSeverity);
  } else {
    // add one
...
void CGCBase::DumpHTTPErrors() {
  FOR_EACH_MAP_FAST(m_HTTPErrors, nCurrError) {
    ...
    EG_EMIT(g_EGHTTPRequest, m_HTTPErrors[nCurrError]->m_eSeverity,
...
//			not be found.
//-----------------------------------------------------------------------------
CGCGSSession *CGCBase::YieldingFindOrCreateGSSession(const CSteamID &steamID,
                                                     uint32 unServerAddr,
                                                     uint16 usServerPort,
                                                     const uint8 *pubVarData,
                                                     uint32 cubVarData) {
  ...
  //	// Since we might have to lock the session in some cases, let's just
  //always grab the lock here,
  ...
  if (!pSession) {
    ...
    Assert(pSession);
    if (!pSession) {
      AssertMsg1(false, "Failed creating GC GS session for %llu",
    ...
    CGCGSSession **ppSession =
        m_hashGSSessions.PvRecordInsert(steamID.ConvertToUint64());
    *ppSession = pSession;

    // Do game-specific work
    YieldingSessionStartServer(pSession);
  } else {
    if (unServerAddr != 0 && usServerPort != 0 &&
        (unServerAddr != pSession->GetAddr() ||
         usServerPort != pSession->GetPort())) {
...
//-----------------------------------------------------------------------------
void CGCBase::UpdateGSSessionAddress(CGCGSSession *pSession,
                                     uint32 unServerAddr, uint16 usServerPort) {
...
CGCGSSession *CGCBase::FindGSSession(const CSteamID &steamID) const {
  ...
  if (!steamID.IsValid() || steamID.GetAccountID() == 0) {
    AssertMsg1(false, "CGCBase::FindGSSession was passed invalid Steam ID %s",
               steamID.Render());
    return NULL;
  }
  if (!steamID.BGameServerAccount()) {
    AssertMsg1(steamID.BGameServerAccount(),
               "CGCBase::FindGSSession was passed non-gameserver Steam ID %s",
               steamID.Render());
    return NULL;
  }

  CGCGSSession **ppSession =
      m_hashGSSessions.PvRecordFind(steamID.ConvertToUint64());
  if (ppSession) {
    (*ppSession)->MarkAccess();
    return *ppSession;
  } else {
...
CGCSession *CGCBase::FindUserOrGSSession(const CSteamID &steamID) const {
  ...
  if (steamID.BGameServerAccount())
    return FindGSSession(steamID);
  AssertMsg1(false,
             "CGCBase::FindUserOrGSSession, steam ID %s isn't an individual or "
             "a gameserver ID",
             steamID.Render());
  return NULL;
}
...
CGCSharedObjectCache *CGCBase::FindSOCache(const CSteamID &steamID) {
  CUtlMap<CSteamID, CGCSharedObjectCache *, int>::IndexType_t nCache =
      m_mapSOCache.Find(steamID);
...
void CGCBase::YieldingSOCacheLoaded(CGCSharedObjectCache *pSOCache) {
  ...
  // stomp the version with the one we set in memcached previously if possible,
  // otherwise, re-add it to the set
...
void CGCBase::RemoveSOCache(const CSteamID &steamID) {
  ...
  if (m_mapSOCache.IsValidIndex(nCache)) {
    ...
    if (pSOCache->BIsDatabaseDirty()) {
      EmitError(SPEW_GC,
                "Attempting to remove SO Cache %s while it was dirty. Adding "
                "to Writeback instead\n",
                steamID.Render());
      pSOCache->DumpDirtyObjects();
      AddCacheToWritebackQueue(pSOCache);

      // adding the cache to the LRU list too, just so it will go away once
      // writeback does its thing
...
//-----------------------------------------------------------------------------
// Purpose: Enqueues a flush instruction to Econ service for Web Inventory to
// update
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
bool CGCBase::UnloadUnusedCaches(uint32 unMaxCacheCount,
                                 CLimitTimer *pLimitTimer) {
  VPROF_BUDGET("UnloadUnusedCaches", VPROF_BUDGETGROUP_STEAM);

  uint32 unCachesUnloaded = 0;
  for (uint32 unCache = m_listCachesToUnload.Head(),
              unNextCache = m_listCachesToUnload.InvalidIndex();
       unCache != m_listCachesToUnload.InvalidIndex(); unCache = unNextCache) {
    ...
    // only loop until we need to stop consuming heartbeat time. We'll finish in
    // later frames
    ...
    if (!pSOCache) {
      EmitError(
          SPEW_GC,
          "Cache for %s could not be found even though it is in the LRU list\n",
          ownerID.Render());
    ...
    if ((ownerID.BIndividualAccount() && FindUserSession(ownerID)) ||
      ...
      EmitError(
          SPEW_GC,
          "Cache for %s has a session even though it is in the LRU list\n",
          ownerID.Render());
      Assert(pSOCache->GetLRUHandle() == unCache);
      if (pSOCache->GetLRUHandle() != unCache) {
        EmitError(SPEW_GC,
                  "Cache for %s has a different LRU handle than the one "
                  "retrieved from the iterator! 0x%08x vs 0x%08x\n",
    ...
    // Being in the writeback queue means that you haven't actually been unused
    // for very long. Just move on to the next one in those cases.
    ...
    if (iPreRemoveCount != m_listCachesToUnload.Count() + 1 &&
      ...
      EmitError(SPEW_GC,
                "CGCBase::UnloadUnusedCaches() sanity check failed! List size "
                "changed dramatically removing 0x%08x; delta %i\n",
...
void CGCBase::VerifySOCacheLRU() {
  CUtlRBTree<CSteamID, int> rbTreeUsersEncountered(
      0, m_listCachesToUnload.Count(), DefLessFunc(CSteamID));
  ...
  for (uint32 unCache = m_listCachesToUnload.Head(),
    ...
    if (!pSOCache) {
      EmitError(SPEW_GC,
                "CGCBase::UnloadUnusedCaches() sanity[0] check failed! Empty "
                "cache in list in slot 0x%08x\n",
                unCache);
      continue;
    }
    ...
    if (pSOCache->GetLRUHandle() != unCache) {
      EmitError(SPEW_GC,
                "CGCBase::UnloadUnusedCaches() sanity[1] check failed! Cache "
                "entry mismatch [ 0x%08x vs 0x%08x ] (owner: %s)\n",
                pSOCache->GetLRUHandle(), unCache,
                pSOCache->GetOwner().Render());
    }

    if (!rbTreeUsersEncountered.IsValidIndex(
            rbTreeUsersEncountered.InsertIfNotFound(ownerID))) {
      EmitError(SPEW_GC,
                "CGCBase::UnloadUnusedCaches() sanity[2] check failed! "
                "Duplicate entry in list for 0x%08x (owner: %s)\n",
...
void CGCBase::AddCacheToLRU(CGCSharedObjectCache *pSOCache) {
  ...
  if (pSOCache->GetLRUHandle() != m_listCachesToUnload.InvalidIndex()) {
    EmitError(SPEW_GC,
              "CGCBase::AddCacheToLRU() sanity[4] check failed! Adding SO "
              "Cache with existing LRU handle: 0x%08x\n",
  ...
  if (pSOCache->GetLRUHandle() != m_listCachesToUnload.InvalidIndex()) {
    EmitError(SPEW_GC,
              "CGCBase::AddCacheToLRU() sanity[5] check failed! Adding SO "
              "Cache with existing LRU handle: 0x%08x\n",
              pSOCache->GetLRUHandle());
  }
#endif
...
#if WITH_SOCACHE_LRU_DEBUGGING
  if (m_listCachesToUnload.IsValidIndex(pSOCache->GetLRUHandle()) ==
      (pSOCache->GetLRUHandle() == m_listCachesToUnload.InvalidIndex())) {
    EmitError(
        SPEW_GC,
        "CGCBase::RemoveCacheFromLRU() sanity[6] check failed! SO Cache has an "
        "invalid index, but IsValidIndex() is returning true: 0x%08x\n",
        pSOCache->GetLRUHandle());
  }
#endif
  if (m_listCachesToUnload.IsValidIndex(pSOCache->GetLRUHandle())) {
    if (m_listCachesToUnload[pSOCache->GetLRUHandle()] !=
        pSOCache->GetOwner()) {
      EmitError(SPEW_GC,
                "CGCBase::RemoveCacheFromLRU() Attempting to remove SOCache "
                "LRU index %d for %s, which really holds %s\n",
                pSOCache->GetLRUHandle(), pSOCache->GetOwner().Render(),
                m_listCachesToUnload[pSOCache->GetLRUHandle()].Render());
    } else {
...
// Purpose: Finds the cache in the map for a new session and locks it
//-----------------------------------------------------------------------------
CGCSharedObjectCache *CGCBase::YieldingGetLockedSOCache(const CSteamID &steamID,
                                                        const char *pszFilename,
                                                        int nLineNum) {
  if (!BYieldingLockSteamID(steamID, pszFilename, nLineNum))
    return NULL;
...
// Purpose: Finds the cache in the map for a new session
//-----------------------------------------------------------------------------
CGCSharedObjectCache *
CGCBase::YieldingFindOrLoadSOCache(const CSteamID &steamID) {
  AssertRunningJob();

  if (!steamID.IsValid()) {
    AssertMsg1(false, "Unable to load SO cache for invalid steam ID %s",
               steamID.Render());
    EmitError(
        SPEW_GC,
        "Unable to load SO cache for invalid steam ID %s (instance: %d)\n",
        steamID.Render(), steamID.GetUnAccountInstance());
    return NULL;
  }

  // check to see if the SO cache is being loaded--if so, then we yield until it
  // is done the reason we are not just locking the steam id is because the
  // current job may have a lock on something else, and jobs can only have one
  // lock active at a time.
  CJobTime timeStartedWaiting;
  timeStartedWaiting.SetToJobTime();
  while (m_rbtreeSOCachesBeingLoaded.Find(steamID) !=
         m_rbtreeSOCachesBeingLoaded.InvalidIndex()) {

    // !TEST! Looks like we might have a bug where we're spinning here waiting
    // forever. Add a timeout just in case.
    if (timeStartedWaiting.CServerMicroSecsPassed() > 180 * k_nMillion) {
      AssertMsg1(false, "Timed out waiting for SO cache %s to finish loading",
                 steamID.Render());
      return false;
    }
  ...
  if (!pSOCache) {
    ...
    if (BYieldingLoadSOCache(pSOCache)) {
      if (FindSOCache(steamID) != NULL) {
        EmitError(SPEW_GC,
                  "HOLY FUCKING SHIT WE ARE DUPLICATING SO CACHES [%s]\n",
                  steamID.Render());
      }
      m_mapSOCache.Insert(steamID, pSOCache);

      float flSecondsToLoad =
          (float)timeStartedLoading.CServerMicroSecsPassed() /
          (float)k_nMillion;
      ...
      float flSecondsToNotify =
          (float)timeStartedNotify.CServerMicroSecsPassed() / (float)k_nMillion;
      if (flSecondsToNotify > 10.0f) {
        EmitInfo(SPEW_GC, 1, 1, "YieldingSOCacheLoaded for %s took %.1fs\n",
                 steamID.Render(), flSecondsToNotify);
      }

      AddCacheToLRU(pSOCache); // in case the cache isn't about to be attached
                               // to a session
      m_rbtreeSOCachesBeingLoaded.Remove(steamID);
    } else {
      AssertMsg1(false, "Unable to load SO cache for %llu",
                 steamID.ConvertToUint64());
...
void CGCBase::YieldingReloadCache(CGCSharedObjectCache *pSOCache) {
  ...
  if (!sqlAccess.BCommitTransaction(true)) {
    EmitError(SPEW_SHAREDOBJ,
              "%s: Unable to flush pending writes for %s, reload failed",
  ...
  if (!BYieldingLoadSOCache(pNewCache)) {
    EmitError(SPEW_SHAREDOBJ,
              "Unable to reload cache for %s because of a SQL error",
              pSOCache->GetOwner().Render());
    return;
  }

  // process every object in the new cache and move it to the old one if
  // necessary
  FOR_EACH_MAP_FAST(CSharedObject::GetFactories(), nType) {
    ...
    if (pOldTypeCache) {
      for (uint32 nCurrObj = 0; nCurrObj < pOldTypeCache->GetCount();) {
        // not all objects should be deleted (for example lobbies/parties), so
        // for those objects don't delete and instead just skip over them
    ...
    if (pNewTypeCache) {
      for (uint unObject = 0; unObject < pNewTypeCache->GetCount();
           unObject++) {
...
//-----------------------------------------------------------------------------
bool CGCBase::BYieldingLockSteamID(const CSteamID &steamID,
                                   const char *pszFilename, int nLineNum) {
  AssertRunningJob();
  ...
  if (!pLock) {
    ...
    if (steamID.BIndividualAccount()) {
      ...
      AssertMsg1(false, "Lock taken for unexpected steamID: %s",
                 steamID.Render());
    }
  }
...
//-----------------------------------------------------------------------------
bool CGCBase::BYieldingLockSteamIDPair(const CSteamID &steamIDA,
                                       const CSteamID &steamIDB,
                                       const char *pszFilename, int nLineNum) {
  ...
  //         concept of locking two SteamID's is probably broken when we split
  //         up things on the GC, though, so this might not be worth fixing.
  //
...
void CGCBase::UnlockSteamID(const CSteamID &steamID) {
  ...
  if (!pLock) {
    AssertMsg2(
        false,
        "UnlockSteamID( '%s' ) called by %s but unable to find lock in map",
        steamID.Render(), GJobCur().GetName());
    return;
  }

  if (pLock->GetJobLocking() != &GJobCur()) {
    AssertMsg2(false,
               "UnlockSteamID( '%s' ) called when job %s doesn't own the lock",
               steamID.Render(), GJobCur().GetName());
    return;
  }
...
//-----------------------------------------------------------------------------
bool CGCBase::IsSteamIDLockedByJob(const CSteamID &steamID,
                                   const CJob *pJob) const {
  CLock *pLock = m_hashSteamIDLocks.PvRecordFind(steamID);
  if (pLock)
...
//-----------------------------------------------------------------------------
// Purpose: returns true if the specified steamID is unlocked, or locked by the
// current job
//-----------------------------------------------------------------------------
bool CGCBase::IsSteamIDUnlockedOrLockedByCurJob(const CSteamID &steamID) {
  ...
  // It is in the hash of locks and is locked return true only if it is locked
  // by the current job
  if (pLock->BIsLocked()) {
...
//-----------------------------------------------------------------------------
// Purpose: returns a pointer to the job holding the lock for this steamID, NULL
// if none
//-----------------------------------------------------------------------------
...
bool CGCBase::YieldingWritebackDirtyCaches(uint32 unSecondToDelayWrite) {
  ...
  FOR_EACH_VEC(m_vecCacheWritebacks, nCache) {
    ...
    if (unSecondToDelayWrite > 0 &&
        pSOCache->GetWritebackTime() > unFirstTimeToWrite) {
      continue;
    }
    ...
    if (GJobCur().GetMicrosecondsRun() >
            (uint64)(writeback_queue_max_accumulate_time.GetInt() *
                     k_nThousand) ||
        (writeback_queue_max_caches.GetInt() > 0 &&
         vecCachesWritten.Count() > writeback_queue_max_caches.GetInt())) {
  ...
  if (!sqlAccess.BCommitTransaction(true)) {
    ...
    EmitError(SPEW_GC,
              "CGCBase::YieldingWritebackDirtyCaches() - Writeback failed\n");

    m_vecCacheWritebacks.AddMultipleToTail(vecCachesWritten.Count(),
                                           vecCachesWritten.Base());
    ...
    // the transaction was successful. Tell those caches to forget their
    // dirtiness
    FOR_EACH_VEC(vecCachesWritten, nCache) {
      CGCSharedObjectCache *pSOCache = vecCachesWritten[nCache];
...
void CGCBase::AddCacheToWritebackQueue(CGCSharedObjectCache *pSOCache) {
  Assert(pSOCache);
  if ((g_pJobCur != NULL) &&
      PJobHoldingLock(pSOCache->GetOwner()) != g_pJobCur &&
      !GGCBase()->BIsSOCacheBeingLoaded(pSOCache->GetOwner())) {
    AssertMsg2(false,
               "CGCBase::AddCacheToWritebackQueue called by job %s for %s, but "
               "job does not own lock",
               g_pJobCur->GetName(), pSOCache->GetOwner().Render());
  }
...
bool CGCBase::BYieldingRetrieveCacheVersion(CGCSharedObjectCache *pSOCache) {
  ...
  CFmtStr1024 key("SOCacheVersionV2_%llu",
                  pSOCache->GetOwner().ConvertToUint64());

  GCMemcachedGetResult_t data;
  if (!BYieldingMemcachedGet(key.Access(), data) || !data.m_bKeyFound ||
      sizeof(uint64) != data.m_bufValue.Count()) {
#ifdef _DEBUG
    EmitInfo(SPEW_CONSOLE, 3, 3,
             "SOCacheVersion - Failed to retrieve SO Cache version for: %s\n",
             pSOCache->GetOwner().Render());
#endif
    return false;
  }

  // we have a memcached version, so make sure that our version matches what was
  // stored in memcache
  ...
  EmitInfo(
      SPEW_CONSOLE, 3, 3,
      "SOCacheVersion::Load - Loaded version from memcached for %s (%llu)\n",
...
void CGCBase::UpdateSOCacheVersions() {
  CUtlVector<CUtlString> vecSetKeys(0,
                                    m_rbtreeSOCachesWithDirtyVersions.Count());
  CUtlVector<GCMemcachedBuffer_t> vecSetValues(
      0, m_rbtreeSOCachesWithDirtyVersions.Count());
  CUtlBuffer bufData(
      0, (sizeof(uint64) * m_rbtreeSOCachesWithDirtyVersions.Count()) + 1);

  CUtlVector<CUtlString> vecDeleteKeys(
      0, m_rbtreeSOCachesWithDirtyVersions.Count());

  for (int idx = 0; idx < m_rbtreeSOCachesWithDirtyVersions.MaxElement();
       ++idx) {
    ...
    if (m_rbtreeSOCachesBeingLoaded.Find(steamID) !=
        m_rbtreeSOCachesBeingLoaded.InvalidIndex())
      continue;
...
#ifdef _DEBUG
      EmitInfo(SPEW_CONSOLE, 3, 3,
               "SOCacheVersion - storing version in memcached for %s (%llu).\n",
               steamID.Render(), pSOCache->GetVersion());
#endif
    } else {
      // SO Cache is gone, so to be safe, remove the cached version number from
      // memcached
...
#ifdef _DEBUG
      EmitInfo(SPEW_CONSOLE, 3, 3,
               "SOCacheVersion - no SO Cache, removing version in memcached "
               "for %s.\n",
               steamID.Render());
#endif
    }
...
//-----------------------------------------------------------------------------
// Purpose: Returns true if the protobuf object was stored successfully, false
// otherwise
//-----------------------------------------------------------------------------
bool CGCBase::BMemcachedSet(const char *pKey,
                            const ::google::protobuf::Message &protoBufObj) {
  // build key
...
//-----------------------------------------------------------------------------
// Purpose: Returns true if the memcached value stored via pKey was removed
...
//-----------------------------------------------------------------------------
// Purpose: Returns true if the protobuf object was retrieved from memcached
// successfully, false otherwise
//-----------------------------------------------------------------------------
bool CGCBase::BYieldingMemcachedGet(const char *pKey,
  ...
  if (!BYieldingMemcachedGet(vecKeys, vecResults) || vecResults.Count() != 1 ||
      vecResults[0].m_bKeyFound == false) {
    return false;
  }
  if (!protoBufMsg.ParseFromArray(vecResults[0].m_bufValue.Base(),
                                  vecResults[0].m_bufValue.Count())) {
    return false;
  }
...
//-----------------------------------------------------------------------------
bool CGCBase::BMemcachedSet(const CUtlVector<CUtlString> &vecKeys,
                            const CUtlVector<GCMemcachedBuffer_t> &vecValues) {
...
bool CGCBase::BMemcachedSet(const CUtlString &strKey, const CUtlBuffer &buf) {
  ...
  CGCBase::GCMemcachedBuffer_t &memcachedBuffer =
      memcachedMemberValues[memcachedMemberValues.AddToTail()];
...
//-----------------------------------------------------------------------------
bool CGCBase::BYieldingMemcachedGet(
    const CUtlVector<CUtlString> &vecKeys,
    CUtlVector<GCMemcachedGetResult_t> &vecResults) {
  ...
  if (!GJobCur().BYieldingWaitForMsg(&msgResponse,
                                     k_EGCMsgMemCachedGetResponse)) {
    EmitWarning(SPEW_GC, LOG_ALWAYS,
                "Didn't get reply from IS for BYieldingMemcachedGet\n");
    return false;
  }
  ...
  if (msgRequest.Body().keys_size() != msgResponse.Body().values_size()) {
    EmitWarning(SPEW_GC, LOG_ALWAYS,
                "Mismatched reply from IS for BYieldingMemcachedGet, asked for "
                "%d keys, got %d back\n",
                (int)msgRequest.Body().keys_size(),
                (int)msgResponse.Body().values_size());
  ...
  for (int i = 0; i < msgResponse.Body().values_size(); ++i) {
    ...
    if (result.m_bKeyFound) {
      result.m_bufValue.Copy(&(*msgResponse.Body().values(i).value().begin()),
                             msgResponse.Body().values(i).value().size());
    }
  }
...
// Purpose: Overload for a single key/value
//-----------------------------------------------------------------------------
bool CGCBase::BYieldingMemcachedGet(const CUtlString &strKeys,
                                    GCMemcachedGetResult_t &result) {
  CUtlVector<CUtlString> memcachedMemberKeys(0, 1);
...
//-----------------------------------------------------------------------------
bool CGCBase::BYieldingGetIPLocations(CUtlVector<uint32> &vecIPs,
                                      CUtlVector<CIPLocationInfo> &infos) {
  CProtoBufMsg<CGCMsgGetIPLocation> msgRequest(k_EGCMsgGetIPLocation);
  FOR_EACH_VEC(vecIPs, i) { msgRequest.Body().add_ips(vecIPs[i]); }
  ...
  if (!GJobCur().BYieldingWaitForMsg(&msgResponse,
    ...
    EmitWarning(SPEW_GC, LOG_ALWAYS,
                "Didn't get reply from IS for BYieldingGetIPLocation\n");
    return false;
  }
...
//-----------------------------------------------------------------------------
bool CGCBase::BYieldingUpdateGeoLocation(
    CUtlVector<CSteamID> const &requestedVecSteamIds) {
    ...
    if (pSession) {
      if (!pSession->GetIPPublic()) {
        EmitInfo(SPEW_GC, 4, LOG_ALWAYS,
                 "BYieldingUpdateGeoLocation Session %s IP == 0, unable to "
                 "retrieve\n",
...
#define iptod(x)                                                               \
  ...
  FOR_EACH_VEC(vecIPs, i) {
    EmitInfo(
        SPEW_GC, geolocation_spewlevel.GetInt(), geolocation_loglevel.GetInt(),
        "BYieldingUpdateGeoLocation GetIPLocation[%d] = (%s,%u.%u.%u.%u)\n", i,
        vecSteamIds[i].Render(), iptod(vecIPs[i]));
  }

  CUtlVector<CIPLocationInfo> infos;
  if (BYieldingGetIPLocations(vecIPs, infos)) {
    // The current IS has a bug where the IP will be blank/zero in the replies.
    // If infos.Count() == vecIPs.Count() assume the order is correct
    if (vecSteamIds.Count() == vecIPs.Count() &&
        vecIPs.Count() == infos.Count()) {
      FOR_EACH_VEC(vecSteamIds, i) {
        CGCSession *pSession = FindUserOrGSSession(vecSteamIds[i]);
        if (pSession) {
          EmitInfo(SPEW_GC, geolocation_spewlevel.GetInt(),
                   geolocation_loglevel.GetInt(),
                   "BYieldingUpdateGeoLocation[MATCHED] "
                   "SetIPLocation[%s(%u.%u.%u.%u)] = (%6.3f,%6.3f)\n",
                   pSession->GetSteamID().Render(), iptod(vecIPs[i]),
                   infos[i].latitude(), infos[i].longitude());
      ...
      FOR_EACH_VEC(vecSteamIds, i) {
        FOR_EACH_VEC(infos, j) {
          if (infos[j].ip() == vecIPs[i]) {
            ...
            if (pSession) {
              EmitInfo(SPEW_GC, 4, LOG_ALWAYS,
                       "BYieldingUpdateGeoLocation[SEARCHED] "
                       "SetIPLocation[%s(%u.%u.%u.%u)] = (%6.3f,%6.3f)\n",
                       pSession->GetSteamID().Render(), iptod(vecIPs[i]),
                       infos[j].latitude(), infos[j].longitude());
              pSession->SetGeoLocation(infos[j].latitude(),
                                       infos[j].longitude());
            }
          }
        }
...
//-----------------------------------------------------------------------------
CGCBase *GGCBase() { return g_pGCBase; }
...
void CGCBase::DumpSteamIDLocks(bool bFull, int nMax) {
  ...
  for (CLock *pLock = m_hashSteamIDLocks.PvRecordFirst(); NULL != pLock;
       pLock = m_hashSteamIDLocks.PvRecordNext(pLock)) {
    if (pLock->BIsLocked()) {
  ...
  EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
           "%d locks total. %d locked, %d displayed\n",
           m_hashSteamIDLocks.Count(), vecLocks.Count(), nMax);
  EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
           "Lock                     Holding Job            First Waiting Job  "
           "    Wait Count  Lock Time\n");
  ...
  for (int nLock = 0; nLock < nMax; nLock++) {
    ...
    EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
             ...
             pLock->GetJobWaitingQueueHead()
                 ? pLock->GetJobWaitingQueueHead()->GetName()
                 : "--",
...
//-----------------------------------------------------------------------------
void CGCBase::DumpJobs(const char *pszJobName, int nMax,
                       int nPrintLocksMax) const {
...
//-----------------------------------------------------------------------------
int CGCBase::GetSOCacheCount() const { return m_mapSOCache.Count(); }

bool CGCBase::IsSOCached(const CSharedObject *pObj, uint32 nTypeID) const {
  // OPT: If there are many caches, this is very slow - it would be faster have
  // a ref count on the shared object to track this. However this is debug only
  // code.
#if defined(DEBUG)
  ...
  AssertMsg(
      false,
      "Calling IsSOCached() in release mode. This is a debug only function");
#endif
  return false;
}

int CGCBase::GetUserSessionCount() const { return m_hashUserSessions.Count(); }

int CGCBase::GetGSSessionCount() const { return m_hashGSSessions.Count(); }
...
//-----------------------------------------------------------------------------
bool CGCBase::GetVprofImbalances() { return m_bDumpVprofImbalances; }
...
CSteamID CGCBase::YieldingGuessSteamIDFromInput(const char *pchInput) {
  ...
  if (!pchInput) {
    EmitError(SPEW_CONSOLE,
              "Invalid NULL string passed to YieldingGuessSteamIDFromInput\n");
    return CSteamID();
  }
  ...
  if (!BSendSystemMessage(msg)) {
    EmitError(SPEW_CONSOLE,
              "Unable to query GCHost in YieldingGuessSteamIDFromInput\n");
    return CSteamID();
  }
  ...
  if (!GJobCur().BYieldingWaitForMsg(&msgReply, k_EGCMsgGenericReply)) {
    EmitError(SPEW_CONSOLE,
              "No response from GCHost in YieldingGuessSteamIDFromInput\n");
    return CSteamID();
  }
...
//			empty in that case.
//-----------------------------------------------------------------------------
bool CGCBase::BYieldingLookupAccount(EAccountFindType eFindType,
                                     const char *pchInput,
                                     CUtlVector<CSteamID> *prSteamIDs) {
  if (eFindType == k_EFindAccountTypeURL) {
    CSteamAPIRequest apiRequest(k_EHTTPMethodGET, "ISteamUser",
                                "ResolveVanityURL", 1);
    apiRequest.SetGETParamString("vanityurl", pchInput);

    KeyValuesAD kvAPIResponse("response");
    CUtlString sWebApiErrMsg;
    EResult eResult = YieldingSendWebAPIRequest(apiRequest, kvAPIResponse,
                                                sWebApiErrMsg, false);
...
GC_CON_COMMAND(gc_search_vanityurl,
               "Tests searching for an account by vanity URL") {
  CUtlVector<CSteamID> vecIDs;
  if (GGCBase()->BYieldingLookupAccount(k_EFindAccountTypeURL, args[1],
                                        &vecIDs)) {
    Msg("Search success.\n");
...
//-----------------------------------------------------------------------------
bool CGCBase::BYieldingRecordSupportAction(const CSteamID &actorID,
                                           const CSteamID &targetID,
                                           const char *pchData,
                                           const char *pchNote) {
  CGCMsg<MsgGCRecordSupportAction_t> msgRecordSupportAction(
...
//-----------------------------------------------------------------------------
void CGCBase::PostAlert(EAlertType eAlertType, bool bIsCritical,
                        const char *pchAlertText,
                        const CUtlVector<CUtlString> *pvecExtendedInfo,
                        bool bAlsoSpew) {
...
//-----------------------------------------------------------------------------
bool CGCBase::BYieldingGetAccountLicenses(
    const CSteamID &steamID, CUtlVector<PackageLicense_t> &vecPackages) {
  CProtoBufMsg<CMsgAMGetLicenses> msg(k_EGCMsgGetLicenses);
  msg.Body().set_steamid(steamID.ConvertToUint64());
  msg.ExpectingReply(GJobCur().GetJobID());
  if (!BSendSystemMessage(msg)) {
    EmitWarning(SPEW_GC, SPEW_ALWAYS,
                "Unable to send GetAccountLicenses system message\n");
    return false;
  }
  ...
  if (!GJobCur().BYieldingWaitForMsg(&msgReply, k_EGCMsgGenericReply)) {
    EmitWarning(SPEW_GC, SPEW_ALWAYS,
                "Timeout waiting for GetAccountLicenses reply\n");
    return false;
  }

  if (msgReply.Body().result() != k_EResultOK) {
    EmitWarning(SPEW_GC, SPEW_ALWAYS,
                "GetAccountLicenses for %s failed with %d\n", steamID.Render(),
  ...
  for (int i = 0; i < msgReply.Body().license_size(); i++) {
    ...
    // skip packages that they directly don't own (they may be lent to them via
    // library sharing, and we don't want to grant based on that). we count
    // account ID of zero as matching so we can deal with old Steam versions
    // that didn't provide this field
    if ((msgPackage.owner_id() != steamID.GetAccountID()) &&
        (msgPackage.owner_id() != 0))
      continue;
...
// Purpose: Fills the vector with all package IDs this account has a license to
//-----------------------------------------------------------------------------
bool CGCBase::BYieldingAddFreeLicense(const CSteamID &steamID,
                                      uint32 unPackageID, uint32 unIPPublic,
                                      const char *pchStoreCountryCode) {
  ...
  if (msgReply.Body().eresult() != k_EResultOK) {
    EmitWarning(SPEW_GC, SPEW_ALWAYS,
                "BYieldingAddFreeLicense for %s failed with %d\n",
...
// Purpose: Fills the vector with all package IDs this account has a license to
//-----------------------------------------------------------------------------
int CGCBase::YieldingGrantGuestPass(const CSteamID &steamID, uint32 unPackageID,
                                    uint32 unPassesToGrant,
                                    int32 nDaysToExpiration) {
  ...
  if (!BSendSystemMessage(msg)) {
    EmitWarning(SPEW_GC, SPEW_ALWAYS,
                "Unable to send GrantGuestPass system message\n");
    return 0;
  }
  ...
  if (!GJobCur().BYieldingWaitForMsg(&msgReply,
    ...
    EmitWarning(SPEW_GC, SPEW_ALWAYS,
                "Timeout waiting for GrantGuestPass reply\n");
    return 0;
  }

  if (msgReply.Body().eresult() != k_EResultOK) {
    EmitWarning(SPEW_GC, SPEW_ALWAYS,
                "YieldingGrantGuestPass for %s failed with %d\n",
                steamID.Render(), msgReply.Body().eresult());
    return 0;
  }
...
//-----------------------------------------------------------------------------
const CAccountDetails *
CGCBase::YieldingGetAccountDetails(const CSteamID &steamID, bool bForceReload) {
  return m_AccountDetailsManager.YieldingGetAccountDetails(steamID,
                                                           bForceReload);
}
...
//-----------------------------------------------------------------------------
const char *CGCBase::YieldingGetPersonaName(const CSteamID &steamID,
                                            const char *szUnknownName) {
  const char *szPersonaName =
      m_AccountDetailsManager.YieldingGetPersonaName(steamID);
...
bool CGCBase::BSendWebApiRegistration() {
  ...
  if (CGCWebAPIInterfaceMapRegistrar::VecInstance().Count() > 0) {
    CGCMsg<MsgGCWebAPIRegisterInterfaces_t> msgWebRegistration(
        k_EGCMsgWebAPIRegisterInterfaces);
    msgWebRegistration.Body().m_cInterfaces =
        CGCWebAPIInterfaceMapRegistrar::VecInstance().Count();
    CUtlBuffer bufRegistrations;
    FOR_EACH_VEC(CGCWebAPIInterfaceMapRegistrar::VecInstance(), nInterface) {
      KeyValues *pkvInterface =
          CGCWebAPIInterfaceMapRegistrar::VecInstance()[nInterface]();
      Assert(pkvInterface);
      if (!pkvInterface)
        return false;
    ...
    msgWebRegistration.AddVariableLenData(bufRegistrations.Base(),
                                          bufRegistrations.TellPut());
...
void CGCBase::Dump() const {
  ...
  EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
           "\tStartPlaying: waiting=%d, jobs running=%d of %d\n",
           m_llStartPlaying.Count(), m_nStartPlayingJobCount,
           cv_concurrent_start_playing_limit.GetInt());
  EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
           "\tJobs: active=%d, yielding=%d\n", m_JobMgr.CountJobs(),
           m_JobMgr.CountYieldingJobs());
  EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
           "\tSessions: user=%d, gameserver=%d\n", m_hashUserSessions.Count(),
           m_hashGSSessions.Count());
  EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
           "\tCaches: %d (%d waiting to unload, %d currently loading, %s %d /+ "
           "%d)\n",
           m_mapSOCache.Count(), m_listCachesToUnload.Count(),
           m_rbtreeSOCachesBeingLoaded.Count(),
           (((m_jobidFlushInventoryCacheAccounts == k_GIDNil) ||
             !m_JobMgr.BJobExists(m_jobidFlushInventoryCacheAccounts))
                ? "last flushed"
                : "currently flushing"),
           m_numFlushInventoryCacheAccountsLastScheduled,
           m_rbFlushInventoryCacheAccounts.Count());
  EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
           "\tWriteback Queue: %d (oldest: %s)\n", m_vecCacheWritebacks.Count(),
           m_vecCacheWritebacks.Count() > 0
               ? CRTime::Render(m_vecCacheWritebacks[0]->GetWritebackTime(),
                                rtimeBuf)
               : "none");
  EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
           "\tYieldingRequestSession: %d active\n",
           m_nRequestSessionJobsActive);
...
void CGCBase::AssertCallbackFunc(const char *pchFile, int nLine,
  ...
  // Our spew handler should have already spewed this once, no need to spew it
  // again
  // EmitError( SPEW_CONSOLE, "%s (%d): %s\n", V_GetFileName( pchFile ), nLine,
  // pchMessage );
  if (!Plat_IsInDebugSession()) {
    ...
    CFmtStr sFileAndLine("assert_%s(%d)%s%s", V_GetFileName(pchFile), nLine,
                         rchCleanedJobName[0] ? "_" : "", rchCleanedJobName);
    ...
    int nStale = 0;
    while (nStale < vecTimes.Count() &&
           (CRTime::RTime32TimeCur() - vecTimes[nStale]) >
               (uint32)cv_assert_minidump_window.GetInt()) {
      nStale++;
    }
    vecTimes.RemoveMultipleFromHead(nStale);

    bool bWriteDump =
        (vecTimes.Count() < cv_assert_max_minidumps_in_window.GetInt());
    if (bWriteDump) {
      ...
      sDumpComment.Format(
          "%s%s%s(%d): %s",
          GGCBase()->GetIsShuttingDown()
              ? "[During shutdown]\n"
              : "", // Asserts during shutdown are much more often spurious.
                    // Let's make it clear if a shutdown happens during shutdown
          sCurJob.String(), // The name of the current job name is often an
                            // incredibly useful piece of info.  If the dumps
                            // are not valid, this can narrow the search space
                            // immensely
...
EResult YieldingSendWebAPIRequest(CSteamAPIRequest &request,
                                  KeyValues *pKVResponse, CUtlString &errMsg,
                                  bool b200MeansSuccess) {
  CHTTPResponse apiResponse;
  ...
  if (k_EHTTPStatusCode200OK != apiResponse.GetStatusCode()) {
    ...
    //) )
    //		{
    //			EmitError( SPEW_GC, "Web call to %s failed with error
    //%d: %s\n", 					   request.GetURL(), 					   pKVResponse->GetInt( "error/errorcode",
    //k_EResultFail ), 					   pKVResponse->GetString( "error/errordesc" ) ); 			return
...
GC_CON_COMMAND(ip_geolocation, "<a.b.c.d> Perform geolocation lookup") {
  ...
  for (int i = 0; i < vecInfos.Count(); ++i) {
    ...
    EmitInfo(SPEW_GC, 1, 1, "%s:  %.1f, %.1f\n", adr.ToString(true),
             vecInfos[i].latitude(), vecInfos[i].longitude());
  }
}
