...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CJobTime::CJobTime() { m_lTime = sm_lTimeCur; }

//----------------------------------------------------------------------------
// Purpose: Sets our value to the current time
//-----------------------------------------------------------------------------
void CJobTime::SetToJobTime() { m_lTime = sm_lTimeCur; }
...
//we just 			access a cached copy of the time.  On the server, we use an arbitrary
//			time value beginning at 0, and incremented by 50
//milliseconds every 			frame. 			NOTE: This should only be called once per frame.
//-----------------------------------------------------------------------------
