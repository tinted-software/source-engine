...
CJobMgr::CJobMgr()
    : m_MapJob(0, 0, DefLessFunc(GID_t)),
      m_QueueJobSleeping(0, 0, &JobSleepingLessFunc), m_unNextJobID(0),
...
//-----------------------------------------------------------------------------
// Purpose: Run jobs
//			Runs once per frame and resumes any sleeping jobs that
//are scheduled 			to run again, also checks for jobs which have timed out.
//
...
bool CJobMgr::BFrameFuncRunSleepingJobs(CLimitTimer &limitTimer) {
  ...
  {
    VPROF_BUDGET("CJobMgr::BResumeSleepingJobs",
                 VPROF_BUDGETGROUP_JOBS_COROUTINES);
    bWorkRemaining |= BResumeSleepingJobs(limitTimer);
  }

  {
    VPROF_BUDGET("CJobMgr::CheckForJobTimeouts",
                 VPROF_BUDGETGROUP_JOBS_COROUTINES);
...
//-----------------------------------------------------------------------------
// Purpose: Run jobs
//			This function is called repeatedly in a single frame if
//time is left 			and will first run any yielding jobs
// Input:   limitTimer - limit timer not to exceed
// Output:  true if there is still work remaining to do, false otherwise
//-----------------------------------------------------------------------------
bool CJobMgr::BFrameFuncRunYieldingJobs(CLimitTimer &limitTimer) {
  ...
  {
    VPROF_BUDGET("CJobMgr::BResumeYieldingJobs",
                 VPROF_BUDGETGROUP_JOBS_COROUTINES);
    bWorkRemaining |= BResumeYieldingJobs(limitTimer);
  }

  {
    VPROF_BUDGET("CJobMgr -- Dispatch completed work items",
                 VPROF_BUDGETGROUP_JOBS_COROUTINES);
    bWorkRemaining |=
        m_WorkThreadPool.BDispatchCompletedWorkItems(limitTimer, this);
  }
...
void CJobMgr::AccumulateStatsofJob(CJob &job) {
  ...
  pJobStatsBucket->m_cLocksLongHeld +=
      job.m_flags.m_bits.m_bLocksLongHeld ? 1 : 0;
  pJobStatsBucket->m_cLocksLongWait +=
      job.m_flags.m_bits.m_bLocksLongWait ? 1 : 0;
  ...
  pJobStatsBucket->m_cLongInterYieldTime +=
      job.m_flags.m_bits.m_bLongInterYield ? 1 : 0;
  pJobStatsBucket->m_cTimeoutNetMsg +=
      job.m_flags.m_bits.m_bTimeoutNetMsg ? 1 : 0;

  pJobStatsBucket->m_u64RunTime += job.m_cyclecountTotal.GetLongCycles();
  if ((uint64)job.m_cyclecountTotal.GetLongCycles() >
      pJobStatsBucket->m_u64RunTimeMax)
  ...
  if (job.m_STimeSwitched != job.m_STimeStarted) {
    ...
    pJobStatsBucket->m_u64JobDuration +=
        job.m_STimeStarted.CServerMicroSecsPassed();
  } else {
    pJobStatsBucket->m_u64JobDuration +=
...
void CJobMgr::RecordOrphanedMessage(MsgType_t eMsg, JobID_t jobIDTarget) {
  EG_MSG(SPEW_JOB,
         "Message %s arrived responding to job %lld which no longer exists, "
         "dropping message\n",
...
void CJobMgr::RemoveJob(CJob &job) {
  ...
  m_JobStats.m_flSumSqJobTimeMicrosec +=
      ((double)u64JobDuration * (double)u64JobDuration);
...
bool CJobMgr::BResumeSQLJob(JobID_t jobID) {
  ...
  if (m_mapSQLQueriesInFlight.IsValidIndex(iMap)) {
    if (m_bSQLProfiling && m_dictSQLBuckets.IsValidIndex(
                               m_mapSQLQueriesInFlight[iMap].m_iBucket)) {
      SQLProfileBucket_t &bucket =
          m_dictSQLBuckets[m_mapSQLQueriesInFlight[iMap].m_iBucket];
      bucket.m_unCount++;
      bucket.m_nTotalMicrosec +=
          (int64)m_sqlTimer.GetDurationInProgress().GetUlMicroseconds() -
          m_mapSQLQueriesInFlight[iMap].m_nStartMicrosec;
    }
  ...
  if (!BGetIJob(jobID, k_EJobPauseReasonSQL, true, &iJob)) {
    EG_MSG(SPEW_JOB,
           "BResumeSQLJob called for a job that could not be found!\n");
    return false;
  }
...
//			and if so, sends it to that job.  Creates a new job if
//necessary.
// Output:	true if the message was routed to a job
//-----------------------------------------------------------------------------
bool CJobMgr::BRouteMsgToJob(void *pParent, IMsgNetPacket *pNetPacket,
                             const JobMsgInfo_t &jobMsgInfo) {
  if (pNetPacket == NULL) {
  ...
  if (jobMsgInfo.m_JobIDTarget != k_GIDNil) {
    ...
    VPROF_BUDGET("CJobMgr::BRouteMsgToJob() - continue job",
                 VPROF_BUDGETGROUP_JOBS_COROUTINES);
    ...
    // The job is no longer running, it most likely timed out before the
    // response arrived. Continue and see if a job is registered to launch from
    // this message
  }

  // no job, so try creating a job that can handle the msg
  // We pass in a pointer to m_JobIDTarget so that it gets set to the new Job's
  // ID. This ensures that anyone replying to this message from within the new
  // job has the right JobIDSource.
  VPROF_BUDGET("CJobMgr::BRouteMsgToJob() - job",
               VPROF_BUDGETGROUP_JOBS_COROUTINES);
  ...
  if (!bRet && jobMsgInfo.m_JobIDTarget != k_GIDNil) {
    ...
    // return that we've handled this message (as much as it possibly can be) --
    // was intended for a job that has timed out, no one else can do anything
    // with it
    return true;
  }
...
//-----------------------------------------------------------------------------
void CJobMgr::PassMsgToJob(CJob &job, IMsgNetPacket *pNetPacket,
                           const JobMsgInfo_t &jobMsgInfo) {
  ...
  if (job.BHasFailedToReceivedMsgType(jobMsgInfo.m_eMsg)) {
    EmitInfo(SPEW_JOB, 2, LOG_ALWAYS,
             "Reply msg type %s to job %s is too late; discarding\n",
  ...
  if (job.GetPauseReason() != k_EJobPauseReasonNetworkMsg) {
    AssertMsg3(false,
               "CJobMgr::PassMsgToJob() job %s received unexpected message %s "
               "when paused for %s\n",
               job.GetName(), PchMsgNameFromEMsg(jobMsgInfo.m_eMsg),
               job.GetPauseReasonDescription());
  }
...
// Input:	jobID -						The job that
// should be paused for the given reason
//			eJobPauseReason -			Pause reason
//			bShouldExist -				If true, job
//should exist, so asserts on not finding it ok 			pIJob -
//IJob to fill in
// Output:	true if job paused for matching reason found
//-----------------------------------------------------------------------------
bool CJobMgr::BGetIJob(JobID_t jobID, EJobPauseReason eJobPauseReason,
                       bool bShouldExist, int *pIJob) {
...
//			m_cMicrosecondsToSleep - number of microseconds to wait
//for before resuming job
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
...
bool CJobMgr::BYieldingRunQuery(CJob &job, CGCSQLQueryGroup *pQueryGroup,
                                ESchemaCatalog eSchemaCatalog) {
  ...
  return pQueryGroup->GetResults() &&
         pQueryGroup->GetResults()->GetError() == k_EGCSQLErrorNone;
}
...
int CJobMgr::SQLProfileSortFunc(void *pCtx, const int *lhs, const int *rhs) {
  ...
  default:
  case k_ESQLProfileSortTotalTime:
    return rhsBucket.m_nTotalMicrosec - lhsBucket.m_nTotalMicrosec;
  case k_ESQLProfileSortTotalCount:
    return rhsBucket.m_unCount - lhsBucket.m_unCount;
  case k_ESQLProfileSortAvgTime:
    return (rhsBucket.m_nTotalMicrosec / rhsBucket.m_unCount) -
           (lhsBucket.m_nTotalMicrosec / lhsBucket.m_unCount);
  case k_ESQLProfileSortName:
    return Q_stricmp(pDictBuckets->GetElementName(*lhs),
                     pDictBuckets->GetElementName(*rhs));
  }
}
...
void CJobMgr::DumpSQLProfile(ESQLProfileSort eSort) {
  ...
  if (0 == vecSort.Count()) {
    EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
             "\tNo SQL stats collected; use sql_profile_on / sql_profile_off "
             "to collect stats first\n");
    return;
  }
  ...
  V_qsort_s(vecSort.Base(), vecSort.Count(), sizeof(int),
            (QSortCompareFuncCtx_t)SQLProfileSortFunc, &ctx);
  ...
  FOR_EACH_VEC(vecSort, i) {
    ...
    float fTime =
        bucket.m_nTotalMicrosec / 1000.0f / (bSeconds ? 1000.0f : 1.0f);

    // render
    EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS, "%8d %8.2f%s %8.2f  %s\n",
             bucket.m_unCount, fTime, bSeconds ? "s " : "ms",
...
void CJobMgr::SetIsShuttingDown() {
  m_WorkThreadPool.AllowTimeouts(
      true); // during shutdown, we might abort jobs before waiting for the work
             // item to complete
...
// Output:	true if the message was routed to a job
//-----------------------------------------------------------------------------
bool CJobMgr::BRouteWorkItemCompletedInternal(JobID_t jobID,
                                              bool bWorkItemCanceled,
                                              bool bShouldExist,
...
//-----------------------------------------------------------------------------
// called by a job that has just been started to place itself on the yield queue
// instead of running
//-----------------------------------------------------------------------------
void CJobMgr::AddDelayedJobToYieldList(CJob &job) {
  // make sure that this job is setup to be yielded at this point, otherwise it
  // will not resume properly
  AssertMsg1(job.GetPauseReason() == k_EJobPauseReasonYield,
             "Delayed job %s was added to yield list but was not in expected "
             "yield state\n",
             job.GetName());
  AddToYieldList(job);
}
...
//-----------------------------------------------------------------------------
bool CJobMgr::BResumeYieldingJobsFromList(
    CUtlLinkedList<JobYielding_t, int> &listJobsYielding,
    uint nCurrentIteration, CLimitTimer &limitTimer) {
...
bool CJobMgr::BResumeYieldingJobs(CLimitTimer &limitTimer) {
  return BResumeYieldingJobsFromList(
      m_ListJobsYieldingRegPri, m_nCurrentYieldIterationRegPri++, limitTimer);
}
...
bool CJobMgr::BResumeSleepingJobs(CLimitTimer &limitTimer) {
  while (m_QueueJobSleeping.Count()) {
    ...
    if (jobSleeping.m_SWakeupTime.LTime() > CJobTime::LJobTimeCur()) {
      ...
      if (jobSleeping.m_STimeTouched.CServerMicroSecsPassed() >=
          k_cMicroSecJobHeartbeat) {
    ...
    // should never be false, but if it is we don't want to do anything to this
    // job
...
//-----------------------------------------------------------------------------
bool CJobMgr::JobSleepingLessFunc(JobSleeping_t const &lhs,
                                  JobSleeping_t const &rhs) {
...
void CJobMgr::CheckForJobTimeouts(CLimitTimer &limitTimer) {
  ...
  // since the timeout time is constant, we only have to check until we find a
  // job
  ...
  while (m_ListJobTimeouts.Head() != m_ListJobTimeouts.InvalidIndex()) {
    ...
    // Break if limit timer is reached and we've already processed at least one
    // item.
    ...
    if (!m_bIsShuttingDown &&
        jobtimeout.m_STimeTouched.CServerMicroSecsPassed() <
            k_cMicroSecJobHeartbeat) {
      // we haven't reached our recycle or timeout limit, which means none of
      // the jobs passed us in the queue would have either
      break;
    }
    ...
    if (pJob->GetTimeSwitched().LTime() == jobtimeout.m_STimePaused.LTime()) {
      ...
      // Always heartbeat so anyone waiting on the job (say on another server)
      // will know it is still alive Note that we even do this right before we
      // timeout, since the job will actually be continued and may just loop
      // itself right back into this waiting state Note also that we do NOT
      // check pJob->GetNextHeartbeatTime() since we've already been watching
      // our own timer
      pJob->Heartbeat();

      if (m_bIsShuttingDown || jobtimeout.m_cHeartbeatsBeforeTimeout <= 0) {
        // Job finished all its available heartbeats before its timeout limit,
        // timeout if appropriate and remove from the list
        ...
        // BUGBUG taylor we should fix things so that we can timeout Jobs
        // waiting on Locks and have them properly unlink themselves from the
        // Lock chain
        if (bShouldTimeout) {
          TimeoutJob(*(pJob));
        }
      } else {
        // Job has not yet used up all its available heartbeats before its
        // timeout limit We've already decremented its
        // m_cHeartbeatsBeforeTimeout, now Reset its touched time too
        jobtimeout.m_STimeTouched.SetToJobTime();
        // Move it back to the end of the queue so it can come back up to the
        // top for either another heartbeat or a timeout
      ...
      // This is really the common heartbeating case, where the job waited a
      // short while without ever reaching the k_cMicroSecJobHeartbeat limit
      // Thus, we need to heartbeat before removing it from the list IF the job
      // has gone too long without heartbeating
...
void CJobMgr::TimeoutJob(CJob &job) {
  ...
  if (job.GetPauseReason() == k_EJobPauseReasonNetworkMsg)
    ...
    EG_WARNING(
        SPEW_JOB,
        "Resuming job '%s (id: %lld)' due to timeout while paused for %s\n",
        job.GetName(), job.GetJobID(), job.GetPauseReasonDescription());
...
void CJobMgr::PauseJob(CJob &job, EJobPauseReason eJobPauseReason) {
  ...
  // add to list to check for timeouts later (or update the existing entry if it
  // is already there)
  ...
  if (iMapIndex == m_MapJobTimeoutsIndexByJobID.InvalidIndex()) {
    ...
    m_MapJobTimeoutsIndexByJobID.Insert(job.GetJobID(),
                                        m_ListJobTimeouts.Tail());
  } else {
    // There was an existing entry, in addition to updating it, move it to the
    // tail
  ...
  if (eJobPauseReason == k_EJobPauseReasonWorkItem) {
    ...
    pJobTimeout->m_cHeartbeatsBeforeTimeout =
        (6 * 60 * 60 * k_nMillion) / k_cMicroSecJobHeartbeat;
  }

  if (pJobTimeout->m_cHeartbeatsBeforeTimeout <= 0) {
    pJobTimeout->m_cHeartbeatsBeforeTimeout =
        k_cJobHeartbeatsBeforeTimeoutDefault;
  }
...
int CJobMgr::DumpJobSummary() {
  CUtlMap<uint32, JobStatsBucket_t, int> mapStatsBucket(0, 0,
                                                        DefLessFunc(uint32));
  ...
  FOR_EACH_MAP_FAST(m_MapJob, i) {
    ...
    pJobStatsBucket->m_cCompletes++; // overloading this to really mean "jobs
                                     // running" for this spew
    pJobStatsBucket->m_cLocksAttempted +=
        job.m_vecLocks.Count(); // overloading this to really be used for "locks
                                // held" for this spew
    pJobStatsBucket->m_u64JobDuration +=
        job.m_STimeStarted.CServerMicroSecsPassed();
    ...
    case k_EJobPauseReasonNetworkMsg:
      pJobStatsBucket->m_cPauseReasonNetworkMsg++;
      break;
    case k_EJobPauseReasonSleepForTime:
      pJobStatsBucket->m_cPauseReasonSleepForTime++;
      break;
    case k_EJobPauseReasonWaitingForLock:
      pJobStatsBucket->m_cPauseReasonWaitingForLock++;
      break;
    case k_EJobPauseReasonYield:
      pJobStatsBucket->m_cPauseReasonYield++;
      break;
    case k_EJobPauseReasonSQL:
      pJobStatsBucket->m_cPauseReasonSQL++;
      break;
    case k_EJobPauseReasonWorkItem:
      pJobStatsBucket->m_cPauseReasonWorkItem++;
      break;
    default:
      break;
    }
  ...
  EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
           "%50s --- running jobs (usec)-- -- locks held -- ----- pause "
           "reasons ---------------------------------\n",
           " ");
  EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
           "%50s    count      aveduration                    netmsg      sql  "
           "  sleep waitlock    yield workitem\n",
           "name");
  ...
  FOR_EACH_MAP_FAST(mapStatsBucket, iBucket) { vecSort.AddToTail(iBucket); }
  V_qsort_s(vecSort.Base(), vecSort.Count(), sizeof(int),
            (QSortCompareFuncCtx_t)ProfileSortFunc, &jobprofilestats);
  ...
  FOR_EACH_VEC(vecSort, iVec) {
    ...
    EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
             "%50s %8lld %16lld %13lld %11lld %8lld %8lld %8lld %8lld %8lld \n",
             bucket.m_rgchName, bucket.m_cCompletes, msecDurationAve,
             bucket.m_cLocksAttempted,

             bucket.m_cPauseReasonNetworkMsg, bucket.m_cPauseReasonSQL,
             bucket.m_cPauseReasonSleepForTime,
             bucket.m_cPauseReasonWaitingForLock, bucket.m_cPauseReasonYield,
...
void CJobMgr::DumpJob(JobID_t jobID, int nPrintLocksMax) const {
  ...
  if (!pJob) {
    ...
    EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS, "%llu\t%12s %12s\n",
             pJob->GetJobID(), pJob->GetName(),
             pJob->GetPauseReasonDescription());

    if (pJob->GetPauseReason() == k_EJobPauseReasonWaitingForLock &&
        pJob->m_pWaitingOnLock != NULL) {
      EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
               "\tWaiting for lock %s from: %s line %d\n",
               pJob->m_pWaitingOnLock->GetName(),
               pJob->m_pWaitingOnLockFilename, pJob->m_waitingOnLockLine);
...
//-----------------------------------------------------------------------------
bool JobTypeSortFuncByMsg(JobType_t const *const &lhs,
                          JobType_t const *const &rhs) {
...
//-----------------------------------------------------------------------------
bool JobTypeSortFuncByName(JobType_t const *const &lhs,
                           JobType_t const *const &rhs) {
...
CUtlRBTree<const JobType_t *> &GMapJobTypesByMsg() {
  static CUtlRBTree<const JobType_t *> s_MapJobTypes(0, 0,
                                                     JobTypeSortFuncByMsg);
...
//-----------------------------------------------------------------------------
bool CJobMgr::BLaunchJobFromNetworkMsg(void *pParent,
                                       const JobMsgInfo_t &jobMsgInfo,
                                       IMsgNetPacket *pNetPacket) {
  if (pNetPacket == NULL) {
    AssertMsg(pNetPacket,
              "CJobMgr::BLaunchJobFromNetworkMsg received NULL packet.");
    return false;
  }

  if (pNetPacket->BHasTargetJobName() &&
      BIsValidSystemMsg(pNetPacket->GetEMsg(), NULL)) {
    JobType_t jobSearch = {pNetPacket->GetTargetJobName(), k_EGCMsgInvalid,
                           jobMsgInfo.m_eServerType};
    ...
    if (GMapJobTypesByMsg().IsValidIndex(iJobType)) {
      ...
      if (job == NULL) {
        AssertMsg3(job,
                   "Job factory returned NULL for job msg %d, server type %d "
                   "(named '%s')!\n",
                   (int)jobMsgInfo.m_eMsg, (int)jobMsgInfo.m_eServerType,
                   pJobType->m_pchName);
        return false;
      }
...
int CJobMgr::ProfileSortFunc(void *pCtx, const int *lhs, const int *rhs) {
  ...
  case k_EJobProfileSortOrder_Alpha:
    return Q_stricmp(
        pJobprofilestats->pmapStatsBucket->Element(*lhs).m_rgchName,
...
void CJobMgr::ProfileJobs(EJobProfileAction ejobProfileAction,
                          EJobProfileSortOrder iSortOrder) {
  ...
  EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
           "%44s  --- completed jobs (usec)----------------------------------  "
           "------ lock counts----------------------------------  ------ "
           "failures -----------\n",
           " ");
  EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
           "%44s        count   averuntime   maxruntime  aveduration #yielded  "
           "attempted waited   failed longheld longwait wait-t/o  t/o-msg "
           "jobfailed longslice\n",
           "name");
  ...
  FOR_EACH_MAP_FAST(m_mapStatsBucket, iBucket) { vecSort.AddToTail(iBucket); }
  V_qsort_s(vecSort.Base(), vecSort.Count(), sizeof(int),
            (QSortCompareFuncCtx_t)ProfileSortFunc, &jobprofilestats);
  ...
  FOR_EACH_VEC(vecSort, iVec) {
    ...
    if (bucket.m_cCompletes) {
      ...
      EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
               "%44s %12lld %12lld %12lld %12lld %8lld %8lld %8lld %8lld %8lld "
               "%8lld %8lld %8lld %8lld %8lld\n",
               bucket.m_rgchName, bucket.m_cCompletes, usecAve, usecMax,
               msecDurationAve, bucket.m_cJobsPaused, bucket.m_cLocksAttempted,
               bucket.m_cLocksWaitedFor, bucket.m_cLocksFailed,
               bucket.m_cLocksLongHeld, bucket.m_cLocksLongWait,
               bucket.m_cWaitTimeout, bucket.m_cTimeoutNetMsg,
               bucket.m_cJobsFailed, bucket.m_cLongInterYieldTime);
    }
  }
  if (m_mapOrphanMessages.Count()) {
    EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
             "Messages that arrived responding to jobs that no longer exists "
             "and were dropped\n");
    FOR_EACH_MAP_FAST(m_mapOrphanMessages, iBucket) {
      EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS, "%44s %12d\n",
               PchMsgNameFromEMsg(m_mapOrphanMessages.Key(iBucket)),
               m_mapOrphanMessages[iBucket]);
    }
...
//-----------------------------------------------------------------------------
void CJobMgr::DumpJobs(const char *pszJobName, int nMax,
  ...
  FOR_EACH_MAP_FAST(m_MapJob, iJob) {
    ...
    if (pszJobName == NULL ||
        V_strcmp(pszJobName, m_MapJob[iJob]->GetName()) == 0) {
...
void CJobMgr::Validate(CValidator &validator, const char *pchName) {
  ...
  FOR_EACH_MAP_FAST(m_MapJob, iJob) { ValidatePtr(m_MapJob[iJob]); }
