...
CWorkThread::CWorkThread(CWorkThreadPool *pThreadPool)
    : m_pThreadPool(pThreadPool), m_bExitThread(false), m_bFinished(false) {}
...
//-----------------------------------------------------------------------------
// Purpose:	Tell work thread pool not to set event on every item added
// (SetEvent is very expensive)
//-----------------------------------------------------------------------------
void CWorkThreadPool::SetNeverSetEventOnAdd(bool bNeverSet) {
  bool bWasSet = m_bNeverSetOnAdd;
  m_bNeverSetOnAdd = bNeverSet;

  // In case of disabling set right away to make sure if we have pending work we
  // execute it now with no latency
...
//			notifies the owner (the pool) as it completes work and
//before it exits
//-----------------------------------------------------------------------------
int CWorkThread::Run() {
  ...
  while (!m_bExitThread) {
#if 0 // game vprof doesn't yet support TLS'd vprof instances, until new vprof
      // code is ported
#if defined(VPROF_ENABLED)
		if ( pProfile )
    ...
    nIterations = 0;
    while ((pPool->BNeverSetEventOnAdd() && nIterations < nMaxFastIterations) ||
           nIterations == 0) {
      ...
      while (pWorkItem) {
        ...
        // If we are flagged as exiting don't try to get more work, we need to
        // exit right away and orphan the work to avoid blocking shutdown.
        if (!m_bExitThread) {
    ...
    // wait for a new work item to arrive in the queue, check the counts first
    // just to be sure
    {
      VPROF_BUDGET("CWorkThread -- Sleep", VPROF_BUDGETGROUP_SLEEPING);
#ifdef _SERVER
  ...
  // Since we are exiting, we must have been signaled to shutdown, and we should
  // signal any remaining threads since each signal wakes only one thread.
...
#endif
      m_bThreadsInitialized(false), m_cThreadsRunning(0), m_cActiveThreads(0),
      m_bMayHaveJobTimeouts(false), m_bExiting(false),
      m_bAutoCreateThreads(false), m_cMaxThreads(0), m_cFailures(0),
      m_cSuccesses(0), m_pWorkThreadConstructor(NULL),
      m_ulLastCompletedSequenceNumber(0), m_ulLastUsedSequenceNumber(0),
      m_ulLastDispatchedSequenceNumber(0), m_bEnsureOutputOrdering(false),
...
//-----------------------------------------------------------------------------
// Purpose:  destructor; does assertion checks to make sure we weere shut down
// cleanly
...
CWorkThreadPool::~CWorkThreadPool() {
  ...
  AssertMsg1((!m_bThreadsInitialized || m_bExiting) && 0 == m_cThreadsRunning,
             "CWorkThreadPool::~CWorkThreadPool(): Thread pool %s shutdown "
             "incorrectly.\n",
             m_szThreadNamePfx);
  ...
  if (m_pTSQueueCompleted->Count() > 0) {
    EmitWarning(SPEW_THREADS, 2,
                "CWorkThreadPool::~CWorkThreadPool: work complete queue not "
                "empty, %d items discarded.\n",
  ...
  if (m_pTSQueueToProcess->Count() > 0) {
    EmitWarning(SPEW_THREADS, 2,
                "CWorkThreadPool::~CWorkThreadPool: work processing queue not "
                "empty: %d items discarded.\n",
...
void CWorkThreadPool::StartWorkThread(CWorkThread *pWorkThread, int iName) {
  ...
  Q_snprintf(rgchThreadName, sizeof(rgchThreadName), "%s:%d", m_szThreadNamePfx,
             iName);
  pWorkThread->SetName(rgchThreadName);
  if (!pWorkThread->Start())
    EmitError(SPEW_THREADS,
...
void CWorkThreadPool::StartWorkThreads() {
  ...
  if (0 == m_WorkThreads.Count()) {
    EmitWarning(SPEW_THREADS, 2,
                "CWorkThreadPool::StartWorkThreads: called with no threads in "
                "the pool, this is probably a bug.\n");
    return;
  }
  m_bExiting = false;
  m_cThreadsRunning = 0;
  AUTO_LOCK(m_WorkThreadMutex);
  FOR_EACH_VEC(m_WorkThreads, i) { StartWorkThread(m_WorkThreads[i], i); }
...
//			this must be called before the thread pool object is
//destroyed
//-----------------------------------------------------------------------------
void CWorkThreadPool::StopWorkThreads() {
  ...
  while (true) {
    // This thread already holds the mutex; recursive try-lock should always
    // succeed
    ...
    FOR_EACH_VEC(m_WorkThreads, iPS3) {
      ...
      sys_net_abort_resolver(m_WorkThreads[iPS3]->GetThreadID(),
                             SYS_NET_ABORT_STRICT_CHECK);
    }
#endif

    const uint k_uJoinTimeoutMillisec =
        10000; // 10 seconds seems pretty arbitrary.
    ...
    if (!bJoined) {
      ...
      EmitWarning(SPEW_THREADS, 2, "Thread \"%s\" (ID %p) failed to shut down",
                  pWorkThread->GetName(), (void *)pWorkThread->GetThreadID());
    } else {
      ...
      if (!pWorkThread->m_bFinished) {
        ...
        // Recover by flagging the thread as potentially eligable for deletion,
        // since it's dead.
...
bool CWorkThreadPool::HasWorkItemsToProcess() const {
  return (m_pTSQueueToProcess->Count() > 0) || (m_cActiveThreads > 0);
}

//-----------------------------------------------------------------------------
// Purpose: sets dynamic thread construction
//-----------------------------------------------------------------------------
void CWorkThreadPool::SetWorkThreadAutoConstruct(
    int cMaxThreads, IWorkThreadFactory *pWorkThreadConstructor) {
  AUTO_LOCK(m_WorkThreadMutex);
...
//			false if a low priority work item is not added due to a
//busy system 			false if this work pool is shutting down and work isn't being
//accepted
...
bool CWorkThreadPool::AddWorkItem(CWorkItem *pWorkItem) {
  ...
  if (m_bEnsureOutputOrdering) {
    AssertMsg(pWorkItem->m_bResubmit == false,
              "CWorkThreadPool can't support item auto resubmission when "
              "ensuring output ordering");
  }
  ...
  if (m_bAutoCreateThreads && m_WorkThreads.Count() < m_cMaxThreads) {
    ...
    // we shouldn't get more than 12 items queued per already existing thread,
    // otherwise we want to create a new thread to help us keep up.
    if (m_WorkThreads.Count() < 1 ||
        m_WorkThreads.Count() * 12 < (cPendingItems + 1)) {
      if (m_WorkThreads.Count() >= 2 &&
          !m_LimitTimerCreateNewThreads.BLimitReached()) {
  ...
  //
  //	Do we actually have any threads ?   If creating threads can fail, then
  //maybe we don't ! 	In that case, this WorkItem is not going to run !
  //
  if (m_WorkThreads.Count() == 0) {
    Assert(false);
    return false;
  ...
  // WARNING: We need to call pWorkItem AddRef() and Release() at all entry/exit
  // points for the thread pool system.
  pWorkItem->AddRef();
...
CWorkItem *CWorkThreadPool::GetNextCompletedWorkItem() {
  ...
  // Use a while loop just in case ref counts get screwed up and an item gets
  // deleted when we release our reference to it
  while (m_pTSQueueCompleted->PopItem(&pWorkItem)) {
    // WARNING: We need to call workitem AddRef() and Release() at all
    // entry/exit points for the thread pool system. Release() returns the
    // current refcount of the object (after decrementing it by one) and should
    // be non-zero unless the the caller has released it already.
...
//			returns NULL immediately if there's nothing left in the
//queue. 			otherwise, a pointer to the next CWorkItem.
//-----------------------------------------------------------------------------
CWorkItem *CWorkThreadPool::GetNextWorkItemToProcess() {
  CWorkItem *pWorkItem = NULL;

  if (m_pTSQueueToProcess->Count() &&
      m_pTSQueueToProcess->PopItem(&pWorkItem)) {
    return pWorkItem;
  }
...
bool CWorkThreadPool::BDispatchCompletedWorkItems(const CLimitTimer &limitTimer,
                                                  CJobMgr *pJobMgr) {
  ...
  while (pWorkItem != NULL) {
    ...
    if (!pWorkItem->DispatchCompletedWorkItem(pJobMgr)) {
      EmitWarning(SPEW_THREADS, 2,
                  "Work Item for Work Pool %s completed but job no longer "
                  "existed to notify\n",
                  m_szThreadNamePfx == NULL ? "UNKNOWN" : m_szThreadNamePfx);
      AssertMsg1(m_bMayHaveJobTimeouts,
                 "Work Item for Work Pool %s completed but job no longer "
                 "existed to notify",
                 m_szThreadNamePfx == NULL ? "UNKNOWN" : m_szThreadNamePfx);
    }
...
//			if they haven't, they'll remain in the threads to delete
//list
//-----------------------------------------------------------------------------
...
bool CWorkItem::DispatchCompletedWorkItem(CJobMgr *pJobMgr) {
  ...
  if (pJobMgr && k_GIDNil != m_JobID) {
    ...
    // This should never happen since we have already released our reference to
    // the work item and the calling job should have released its ref when it
    // exited
    AssertMsg(
        false,
...
//-----------------------------------------------------------------------------
// Purpose:	Called by the worker thread when it finishes an individual work
// item
//			This function will see if our work is meant to be
//well-ordred; if so, 			it will do the necessary work to ensure ordering.
//
//			It adds the item to the completed work item list so
//			the pool owner can retrieve it and checks to see if any
//threads 			deserve to be shut down.
//-----------------------------------------------------------------------------
void CWorkThreadPool::OnWorkItemCompleted(CWorkItem *pWorkItem) {
  ...
  if (!m_bEnsureOutputOrdering) {
    // Since we aren't locking this sequence number could get screwed up a bit,
    // but it's pretty meaningless if ensure output ordering if off anyway...
    m_ulLastCompletedSequenceNumber = pWorkItem->m_ulSequenceNumber;
    m_pTSQueueCompleted->PushItem(pWorkItem);
  } else {
    // In the ordered case we need to lock completely here since we'll be moving
    // around between various data structures and also need to ensure the
    // ordering of items in the TS queue
    m_MutexOnItemCompletedOrdered.Lock();
    if (m_ulLastCompletedSequenceNumber + 1 == pWorkItem->m_ulSequenceNumber) {
      m_ulLastCompletedSequenceNumber = pWorkItem->m_ulSequenceNumber;
      m_pTSQueueCompleted->PushItem(pWorkItem);

      // We walk the vector multiple times, but it should be very short as items
      // are likely to come in close to in order, just mixed up a little if we
      // have lots of threads or one item is much more costly than others.
      bool bFoundNext = false;
      do {
        bFoundNext = false;
        FOR_EACH_VEC(m_vecCompletedAndWaiting, i) {
          CWorkItem *pWaiting = m_vecCompletedAndWaiting[i];
          if (m_ulLastCompletedSequenceNumber + 1 ==
              pWaiting->m_ulSequenceNumber) {
...
//-----------------------------------------------------------------------------
// Purpose: return the count of items we've completed but not notified the
// consumer about
//-----------------------------------------------------------------------------
