...
//-------------------------------------------------------------------------
CBaseFrameFunction::CBaseFrameFunction(const char *pchName,
                                       EFrameType eFrameType)
    : m_sName(pchName), m_EFrameType(eFrameType), m_nNumCalls(0),
      m_nTrackedTime(0), m_bRegistered(false) {}

//-------------------------------------------------------------------------
CBaseFrameFunction::~CBaseFrameFunction() { Deregister(); }

// called to handle registering for updates and unregistering. Not registered by
// default
...
CFrameFunctionMgr::CFrameFunctionMgr()
    : m_nNumProfileFrames(0), m_bCompletedHighPri(false) {}
...
void CFrameFunctionMgr::Register(CBaseFrameFunction *pFrameFunc) {
  ...
  if (m_MainLoopFrameFuncs[eType].Find(pFrameFunc) ==
      m_MainLoopFrameFuncs[eType].InvalidIndex()) {
...
bool CFrameFunctionMgr::RunFrameTick(const CLimitTimer &limitTimer) {
  ...
  if (!m_bCompletedHighPri) {
    if (RunFrameList(CBaseFrameFunction::k_EFrameType_RunUntilCompleted,
                     limitTimer)) {
  ...
  // if we are still here, we have completed high priority, so run until we are
  // done with low priority
  return RunFrameList(CBaseFrameFunction::k_EFrameType_RunUntilCompletedLowPri,
                      limitTimer);
}

//-------------------------------------------------------------------------
bool CFrameFunctionMgr::RunFrameList(CBaseFrameFunction::EFrameType eType,
...
bool CFrameFunctionMgr::SortFrameFuncByTime(const CBaseFrameFunction *pLhs,
...
void CFrameFunctionMgr::DumpProfile() {
  ...
  for (uint32 nCurrType = 0; nCurrType < CBaseFrameFunction::k_EFrameType_Count;
       nCurrType++) {
  ...
  double fInvFrame =
      (m_nNumProfileFrames > 0) ? 1.0 / m_nNumProfileFrames : 1.0;
  ...
  FOR_EACH_VEC(FrameFuncs, nFunc) {
    ...
    EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
             "%-40s %12.2f %10d %5.1f%% %10.3f %10.1f\n", pFunc->m_sName.Get(),
             pFunc->m_nTrackedTime / 1000.0, pFunc->m_nNumCalls,
             pFunc->m_nTrackedTime / (double)nTotalTime,
             pFunc->m_nTrackedTime * fInvFrame / 1000.0,
             pFunc->m_nNumCalls * fInvFrame);
  }
  ...
  EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
           "%-40s %12.2f %10d %5.1f%% %10.3f %10.1f\n", "Totals",
           nTotalTime / 1000.0, nTotalCalls, 100.0,
           nTotalTime * fInvFrame / 1000.0, nTotalCalls * fInvFrame);
}
...
static void DumpFrameFunctionProfile() { GFrameFunctionMgr().DumpProfile(); }

GC_CON_COMMAND(
    framefunc_profile,
    "Turns on frame function profiling for N seconds and dumps the results") {
  if (args.ArgC() < 2) {
    EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
             "Proper usage is framefunc_profile <n> where n is the number of "
             "seconds to profile for\n");
    return;
  }
