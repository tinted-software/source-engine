...
//-----------------------------------------------------------------------------
// Purpose: Helper for emitting properly escaped XML string values, we always
// use UTF8, so we only really need to encode & ' " < >
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Helper for emitting properly escaped VDF string values, we always
// use UTF8,
...
enum {
  k_LineBreakEveryNGroups = 18
}; // line break every 18 groups of 4 characters (every 72 characters)

uint32 Base64EncodeMaxOutput(const uint32 cubData, const char *pszLineBreak) {
  // terminating null + 4 chars per 3-byte group + line break after every 18
  // groups (72 output chars) + final line break
  uint32 nGroups = (cubData + 2) / 3;
  uint32 cchRequired =
      1 + nGroups * 4 +
      (pszLineBreak ? Q_strlen(pszLineBreak) *
                          (1 + (nGroups - 1) / k_LineBreakEveryNGroups)
                    : 0);
  return cchRequired;
}

bool Base64Encode(const uint8 *pubData, uint32 cubData, char *pchEncodedData,
                  uint32 *pcchEncodedData, const char *pszLineBreak) {
  if (pchEncodedData == NULL) {
    AssertMsg(*pcchEncodedData == 0,
  ...
  const char *const pszBase64Chars =
  ...
  --cchEncodedData; // pre-decrement for the terminating null so we don't forget
                    // about it
  ...
  if (pubData != pubDataEnd) {
    ...
    pchEncodedData[0] = pszBase64Chars[(un24BitsData >> 18) & 63];
    pchEncodedData[1] = pszBase64Chars[(un24BitsData >> 12) & 63];
    pchEncodedData[2] = pubData + 1 != pubDataEnd
                            ? pszBase64Chars[(un24BitsData >> 6) & 63]
                            : '=';
    pchEncodedData[3] = '=';
    pchEncodedData += 4;
    cchEncodedData -= 4;
...
out_of_space:
  *pchEncodedData = 0;
  *pcchEncodedData = Base64EncodeMaxOutput(cubData, pszLineBreak);
  AssertMsg(false, "CCrypto::Base64Encode: insufficient output buffer (up to "
                   "n*4/3+5 bytes required, plus linebreaks)");
  return false;
}

bool Base64Decode(const char *pchData, uint32 cchDataMax, uint8 *pubDecodedData,
                  uint32 *pcubDecodedData, bool bIgnoreInvalidCharacters) {
  uint32 cubDecodedData = *pcubDecodedData;
  uint32 cubDecodedDataOrig = cubDecodedData;

  if (pubDecodedData == NULL) {
    AssertMsg(*pcubDecodedData == 0,
  ...
  if (un24BitsWithSentinel >= (1 << 6)) {
    ...
    while (nWriteBytes-- > 0) {
      if (cubDecodedData == 0) {
        AssertMsg(false, "CCrypto::Base64Decode: insufficient output buffer "
                         "(up to n*3/4+2 bytes required)");
        goto decode_failed;
      }
      if (pubDecodedData) {
...
//-----------------------------------------------------------------------------
bool CWebAPIResponse::BEmitFormattedOutput(EWebAPIOutputFormat eFormat,
                                           CUtlBuffer &outputBuffer,
                                           size_t unMaxResultSize) {
  VPROF_BUDGET("CWebAPIResponse::BEmitFormattedOutput",
...
//-----------------------------------------------------------------------------
bool CWebAPIResponse::BEmitJSON(CUtlBuffer &outputBuffer,
                                size_t unMaxResultSize) {
  outputBuffer.PutChar('{');
  outputBuffer.PutChar('\n');
  ...
  // if we have an anonymous root, get the first child instead of the root
  // itself
  ...
  if (pValues) {
    if (!CWebAPIValues::BEmitJSONRecursive(pValues, outputBuffer, 1,
                                           unMaxResultSize, m_bExtendedArrays))
      return false;
  }
...
//-----------------------------------------------------------------------------
bool CWebAPIResponse::BEmitVDF(CUtlBuffer &outputBuffer,
                               size_t unMaxResultSize) {
  if (m_pValues)
    if (!CWebAPIValues::BEmitVDFRecursive(m_pValues, outputBuffer, 0, 0,
                                          unMaxResultSize, m_bExtendedArrays))
      return false;
...
//-----------------------------------------------------------------------------
bool CWebAPIResponse::BEmitXML(CUtlBuffer &outputBuffer,
                               size_t unMaxResultSize) {
  ...
  if (m_pValues)
    if (!CWebAPIValues::BEmitXMLRecursive(m_pValues, outputBuffer, 0,
                                          unMaxResultSize))
      return false;
...
CWebAPIValues *CWebAPIResponse::CreateRootValue(const char *pchName) {
  if (m_pValues) {
    AssertMsg(
        false,
        "CWwebAPIResponse::CreateRootValue called while root already existed.");
    Clear();
  }
...
//----------------------------------------------------------------------------
#ifdef GC
static GCConVar webapi_values_max_pool_size_mb(
    "webapi_values_max_pool_size_mb", "10",
    "Maximum size in bytes of the WebAPIValues buffer pool");
static GCConVar webapi_values_init_buffer_size(
    "webapi_values_init_buffer_size", "65536",
    "Initial buffer size for buffers in the WebAPIValues buffer pool");
/*static*/ CBufferPoolMT &CWebAPIValues::GetBufferPool() {
  static CBufferPoolMT s_bufferPool("WebAPIValues",
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CWebAPIValues::CWebAPIValues(CWebAPIValues *pParent, const char *pchName,
                             EWebAPIValueType eValueType,
                             const char *pchArrayElementNames) {
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CWebAPIValues::CWebAPIValues(CWebAPIValues *pParent, int nNamePos,
                             EWebAPIValueType eValueType,
                             const char *pchArrayElementNames) {
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
void CWebAPIValues::InitInternal(CWebAPIValues *pParent, int nNamePos,
                                 EWebAPIValueType eValueType,
                                 const char *pchArrayElementNames) {
  if (NULL == pParent) {
#ifdef GC
...
CWebAPIValues::~CWebAPIValues() {
  ...
  // our first child and next peer will lead to the full tree being deleted
  // correctly.
  m_pLastChild = NULL;
  m_pParent = NULL;
}
...
void CWebAPIValues::SetName(const char *pchName) {
  if (pchName == NULL) {
    AssertMsg(false, "CWebAPIValues constructed with NULL name, breaks some "
                     "output serialization.  Shouldn't do this.");
    m_nNamePos = -1;
  } else {
    // Shouldn't use ', ", &, <, or > in names since they can't output well in
    // XML.  : is no good since it implies namespacing in XML. Assert about it
    // so we don't end up with responses that are badly formed in XML output.
    int unLen = 0;
    while (pchName[unLen] != 0) {
      if (pchName[unLen] == '\'' || pchName[unLen] == '"' ||
          pchName[unLen] == '&' || pchName[unLen] == '>' ||
          pchName[unLen] == '>' || pchName[unLen] == ':') {
        AssertMsg(false,
                  "Shouldn't use any of '\"&<>: in CWebAPIValues node names, "
                  "you used %s",
                  pchName);
        break;
      }
...
//-----------------------------------------------------------------------------
// Purpose: Assert that we don't have any child nodes, this is used when setting
// a native type value.  We don't support having both our own value and
// children.  You are either an array of more values, or you are a value
// yourself.
//-----------------------------------------------------------------------------
void CWebAPIValues::AssertNoChildren() {
  AssertMsg(m_pFirstChild == NULL,
            "CWebAPIValues has child nodes, but you are trying to set a direct "
            "value for it.  Can't have both children and your own value.");
}
...
int32 CWebAPIValues::GetInt32Value() const {
  ...
  case k_EWebAPIValueType_Int32:
    return m_nValue;
  case k_EWebAPIValueType_UInt32: // because we can't tell the type of an int
                                  // when we parse this node might have
                                  // different type
  case k_EWebAPIValueType_UInt64:
  case k_EWebAPIValueType_String: {
    uint32 uiVal = GetUInt32Value();
    AssertMsg(uiVal < INT_MAX,
              "GetInt32Value called on node with %u, which is too big to fit "
              "in an Int32",
              uiVal);
    return (int32)uiVal;
  }
  ...
  AssertMsg(false,
            "Shouldn't call CWebAPIValues::GetInt32Value unless value type is "
            "int32, uint32, uint64, or string. %d does not.",
            m_eValueType);
  return 0;
}
...
uint32 CWebAPIValues::GetUInt32Value() const {
  ...
  case k_EWebAPIValueType_String: {
    ...
    AssertMsg(uiVal <= UINT_MAX,
              "GetUInt32Value called on node with %llu, which is too big to "
              "fit in an UInt32",
              uiVal);
    return (uint32)uiVal;
  }
  ...
  AssertMsg(false,
            "Shouldn't call CWebAPIValues::GetUInt32Value unless value type is "
            "uint32, uint64, or string. %d does not",
            m_eValueType);
  return 0;
}
...
int64 CWebAPIValues::GetInt64Value() const {
  ...
  case k_EWebAPIValueType_String:
    if (m_nStrValuePos < 0) {
      ...
      return strtoll((const char *)m_pStringBuffer->Base() + m_nStrValuePos,
                     NULL, 10);
#else
      return _strtoi64((const char *)m_pStringBuffer->Base() + m_nStrValuePos,
                       NULL, 10);
#endif
    }
  default:
    AssertMsg1(false,
               "Shouldn't call CWebAPIValues::GetInt64Value unless value type "
               "matches. %d does not",
               m_eValueType);
    return 0;
  }
...
#if defined(_PS3) || defined(POSIX)
      return strtoull((const char *)m_pStringBuffer->Base() + m_nStrValuePos,
                      NULL, 10);
#else
      return _strtoui64((const char *)m_pStringBuffer->Base() + m_nStrValuePos,
                        NULL, 10);
#endif
    }
  default:
    AssertMsg1(false,
               "Shouldn't call CWebAPIValues::GetUInt64Value unless value type "
               "matches. %d does not",
               m_eValueType);
    return 0;
  }
...
double CWebAPIValues::GetDoubleValue() const {
  ...
  default:
    AssertMsg1(false,
               "Shouldn't call CWebAPIValues::GetDoubleValue unless value type "
               "matches. %d does not",
               m_eValueType);
    return 0.0f;
  }
...
bool CWebAPIValues::GetBoolValue() const {
  if (m_eValueType != k_EWebAPIValueType_Bool) {
    AssertMsg(
        false,
        "Shouldn't call CWebAPIValues::GetBoolValue unless value type matches");
    return false;
  }
...
void CWebAPIValues::GetStringValue(CUtlString &stringOut) const {
  ...
  default:
    AssertMsg1(false,
               "CWebAPIValues::GetStringValue(), unable to convert data type "
               "%d to string",
               m_eValueType);
    stringOut = "";
    return;
...
void CWebAPIValues::GetBinaryValue(CUtlBuffer &bufferOut) const {
  if (m_eValueType != k_EWebAPIValueType_BinaryBlob) {
    AssertMsg(false, "Shouldn't call CWebAPIValues::GetBinaryValue unless "
                     "value type matches");
    bufferOut.Clear();
    return;
  }
  ...
  if (m_BinaryValue.m_unBytes)
    bufferOut.Put((byte *)m_pStringBuffer->Base() + m_BinaryValue.m_nDataPos,
                  m_BinaryValue.m_unBytes);
...
// as the element name for each of the children of the array, in JSON it will
// simply be a numerically indexed [] array.
//-----------------------------------------------------------------------------
CWebAPIValues *
CWebAPIValues::CreateChildArray(const char *pchName,
                                const char *pchArrayElementNames) {
  return CreateChildInternal(pchName, k_EWebAPIValueType_NumericArray,
                             pchArrayElementNames);
}

//-----------------------------------------------------------------------------
// Purpose:  Create a child of this node.  Note, it's possible to create
// multiple, children with the same name, but you really don't want to.  We'll
// assert about it in debug builds to detect, but not in release.  If you do
// create duplicates you'll have broken JSON output.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Return an existing child object - otherwise create one and return
// that.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Add a child object to the array, this should only be called on
// objects that are of the array type
//-----------------------------------------------------------------------------
CWebAPIValues *CWebAPIValues::AddChildObjectToArray() {
  if (m_eValueType != k_EWebAPIValueType_NumericArray) {
    AssertMsg(m_eValueType == k_EWebAPIValueType_NumericArray,
              "Can't add array elements to CWebAPIVAlues unless type is of "
              "numeric array.");
    return NULL;
  }
...
//-----------------------------------------------------------------------------
// Purpose: Add a child array to the array, this should only be called on
// objects that are of the array type
//-----------------------------------------------------------------------------
CWebAPIValues *
CWebAPIValues::AddChildArrayToArray(const char *pchArrayElementNames) {
  ...
  // Use child element array name as name of all children of arrays
  return CreateChildInternal(NULL, k_EWebAPIValueType_NumericArray,
                             pchArrayElementNames);
}
...
//-----------------------------------------------------------------------------
CWebAPIValues *
CWebAPIValues::CreateChildInternal(const char *pchName,
                                   EWebAPIValueType eValueType,
                                   const char *pchArrayElementNames) {
  // Shouldn't create children if you have a direct value.  You are either an
  // object or array of children, or a native value type.  Not both.

  AssertMsg(
      m_eValueType == k_EWebAPIValueType_Object ||
          m_eValueType == k_EWebAPIValueType_NumericArray,
      "You are trying to create a child node of a CWebAPIValues object, but it "
      "has a direct value already.  Can't have children and a value.");
  if (m_eValueType != k_EWebAPIValueType_Object &&
      m_eValueType != k_EWebAPIValueType_NumericArray)
    ClearValue();
  ...
  if (m_eValueType == k_EWebAPIValueType_NumericArray) {
    if (pchName) {
      AssertMsg(false, "Can't create named child of CWebAPIValues object of "
                       "type NumericArray.  Should call AddArrayElement "
                       "instead of CreateChild*.");
    }
    // Force name to match what all items in the array should use
    pNewNode = new CWebAPIValues(this, m_nArrayChildElementNamePos, eValueType,
                                 pchArrayElementNames);
  } else {
    pNewNode =
        new CWebAPIValues(this, pchName, eValueType, pchArrayElementNames);
  }
...
#ifdef _DEBUG
    // In debug, traverse all children so we can check for duplicate names,
    // which will break JSON output!
    pCurLastChild = m_pFirstChild;
    if (m_eValueType != k_EWebAPIValueType_NumericArray) {
      if (Q_stricmp(pCurLastChild->GetName(), pchName) == 0) {
        AssertMsg(false,
                  "Trying to create CWebAPIValues child with name %s that "
                  "conflicts with existing child.  Breaks JSON output!",
                  pchName);
      }
    }
...
//-----------------------------------------------------------------------------
void CWebAPIValues::SetChildStringValue(const char *pchChildName,
                                        const char *pchValue) {
...
//-----------------------------------------------------------------------------
void CWebAPIValues::SetChildUInt32Value(const char *pchChildName,
                                        uint32 unValue) {
...
//-----------------------------------------------------------------------------
void CWebAPIValues::SetChildUInt64Value(const char *pchChildName,
                                        uint64 ulValue) {
...
//-----------------------------------------------------------------------------
void CWebAPIValues::SetChildDoubleValue(const char *pchChildName,
                                        double flValue) {
...
//-----------------------------------------------------------------------------
void CWebAPIValues::SetChildBinaryValue(const char *pchChildName,
                                        const uint8 *pValue, uint32 unBytes) {
...
//-----------------------------------------------------------------------------
// Purpose: Get a child node's int32 value or return the default if the node
// doesn't exist
//-----------------------------------------------------------------------------
int32 CWebAPIValues::GetChildInt32Value(const char *pchChildName,
                                        int32 nDefault) const {
...
//-----------------------------------------------------------------------------
// Purpose: Get a child node's uint32 value or return the default if the node
// doesn't exist
//-----------------------------------------------------------------------------
uint32 CWebAPIValues::GetChildUInt32Value(const char *pchChildName,
                                          uint32 unDefault) const {
  const CWebAPIValues *pChild = FindChild(pchChildName);
  if (pChild)
...
//-----------------------------------------------------------------------------
// Purpose: Get a child node's int64 value or return the default if the node
// doesn't exist
//-----------------------------------------------------------------------------
int64 CWebAPIValues::GetChildInt64Value(const char *pchChildName,
                                        int64 lDefault) const {
  const CWebAPIValues *pChild = FindChild(pchChildName);
  if (pChild)
...
//-----------------------------------------------------------------------------
// Purpose: Get a child node's uint64 value or return the default if the node
// doesn't exist
//-----------------------------------------------------------------------------
uint64 CWebAPIValues::GetChildUInt64Value(const char *pchChildName,
                                          uint64 ulDefault) const {
  const CWebAPIValues *pChild = FindChild(pchChildName);
  if (pChild)
...
//-----------------------------------------------------------------------------
// Purpose: Get a child node's double value or return the default if the node
// doesn't exist
//-----------------------------------------------------------------------------
double CWebAPIValues::GetChildDoubleValue(const char *pchChildName,
                                          double flDefault) const {
  const CWebAPIValues *pChild = FindChild(pchChildName);
  if (pChild)
...
//-----------------------------------------------------------------------------
// Purpose: Get a child node's string value or return the default if the node
// doesn't exist
//-----------------------------------------------------------------------------
void CWebAPIValues::GetChildStringValue(CUtlString &stringOut,
                                        const char *pchChildName,
                                        const char *pchDefault) const {
  const CWebAPIValues *pChild = FindChild(pchChildName);
  if (pChild) {
...
//-----------------------------------------------------------------------------
// Purpose: Get a child node's binary blob value (returns false if the child
// wasn't found)
//-----------------------------------------------------------------------------
bool CWebAPIValues::BGetChildBinaryValue(CUtlBuffer &bufferOut,
                                         const char *pchChildName) const {
  const CWebAPIValues *pChild = FindChild(pchChildName);
  if (pChild) {
...
//-----------------------------------------------------------------------------
// Purpose: Get a child node's bool value or return the default if the node
// doesn't exist
//-----------------------------------------------------------------------------
bool CWebAPIValues::GetChildBoolValue(const char *pchChildName,
                                      bool bDefault) const {
  const CWebAPIValues *pChild = FindChild(pchChildName);
  if (pChild)
...
//-----------------------------------------------------------------------------
// Purpose:  Find first matching child by name, O(N) on number of children, this
// class isn't designed for searching
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose:  Call this on the returned value from GetFirstChild() or a previous
// GetNextChild() call to proceed to the next child of the parent
// GetFirstChild() was originally called on.
//-----------------------------------------------------------------------------
...
void CWebAPIValues::DeleteChild(const char *pchName) {
  CWebAPIValues *pChild =
      NULL; // child we're examining, could be NULL at exit if we don't find it
  CWebAPIValues *pPrev = NULL; // previous sibling, or NULL

  for (pChild = m_pFirstChild; pChild != NULL;
       pPrev = pChild, pChild = pChild->m_pNextPeer) {
  ...
  // debug only, check that there is no child by the specified name any more and
  // that it was excised OK
  AssertMsg(FindChild(pchName) == NULL,
            "cwebapivalues deleted child is still lurking");
...
//
// when bEmitOldStyleArrays is true, arrays are emitted as a child of a singlet
// object, and any empty arrays will be emitted as having a single null member.
//
// when bEmitOldStyleArrays is false, arrays are emitted bare (no subobject) and
// empty arrays are emitted empty.
//-----------------------------------------------------------------------------
bool CWebAPIValues::BEmitJSONRecursive(const CWebAPIValues *pCurrent,
                                       CUtlBuffer &outputBuffer, int nTabLevel,
                                       size_t unMaxResultSize,
                                       bool bEmitOldStyleArrays) {
  bool bSuccess = true;

  while (pCurrent) {
    // don't let the buffer grow until it consumes all available memory
    if (unMaxResultSize &&
        (size_t)outputBuffer.TellMaxPut() > unMaxResultSize) {
      return false;
    }
    ...
    if (pCurrent->GetName()) {
      ...
      if (!pCurrent->m_pParent ||
          pCurrent->m_pParent->GetType() != k_EWebAPIValueType_NumericArray) {
        EmitJSONString(outputBuffer, pCurrent->GetName());
        outputBuffer.PutChar(':');
        outputBuffer.PutChar(' ');
      }

      if (pCurrent->m_eValueType == k_EWebAPIValueType_Object ||
          pCurrent->m_eValueType == k_EWebAPIValueType_NumericArray) {

        if (bEmitOldStyleArrays ||
            pCurrent->m_eValueType == k_EWebAPIValueType_Object) {
          outputBuffer.PutChar('{');
          outputBuffer.PutChar('\n');
        }
        ...
        if (pCurrent->m_eValueType == k_EWebAPIValueType_NumericArray) {
          if (bEmitOldStyleArrays) {
            ...
            EmitJSONString(outputBuffer,
                           (const char *)pCurrent->m_pStringBuffer->Base() +
                               pCurrent->m_nArrayChildElementNamePos);
            outputBuffer.PutChar(':');
            outputBuffer.PutChar(' ');
          }
        ...
        if (pCurrent->m_pFirstChild) {
          ...
          bSuccess = BEmitJSONRecursive(pCurrent->m_pFirstChild, outputBuffer,
                                        nChildTabLevel, unMaxResultSize,
                                        bEmitOldStyleArrays);
          if (!bSuccess)
            return false;
        ...
        case k_EWebAPIValueType_BinaryBlob: {
          if (pCurrent->m_BinaryValue.m_unBytes == 0)
            ...
            DbgVerify(Base64EncodeIntoUTLMemory(
                (const uint8 *)pCurrent->m_pStringBuffer->Base() +
                    pCurrent->m_BinaryValue.m_nDataPos,
                pCurrent->m_BinaryValue.m_unBytes, buffEncoded));
...
//-----------------------------------------------------------------------------
bool CWebAPIValues::BEmitVDFRecursive(const CWebAPIValues *pCurrent,
                                      CUtlBuffer &outputBuffer, int nTabLevel,
                                      uint32 nArrayElement,
                                      size_t unMaxResultSize,
                                      bool bIncludeArrayElementName) {
  bool bSuccess = true;

  // We can have lots of peers, so this is an optimization to avoid tail
  // recursion and resulting stack-overflows!
  while (pCurrent) {
    // don't let the buffer grow until it consumes all available memory
    if (unMaxResultSize &&
        (size_t)outputBuffer.TellMaxPut() > unMaxResultSize) {
    ...
    if (pCurrent->GetName()) {
      ...
      if (pCurrent->m_eValueType == k_EWebAPIValueType_Object ||
        ...
        if (pCurrent->m_eValueType == k_EWebAPIValueType_NumericArray &&
            bIncludeArrayElementName) {
          for (int i = 0; i < nTabLevel + 1; ++i)
            outputBuffer.PutChar('\t');

          EmitVDFString(outputBuffer,
                        (const char *)pCurrent->m_pStringBuffer->Base() +
                            pCurrent->m_nArrayChildElementNamePos);
          outputBuffer.PutChar('\n');
        ...
        if (pCurrent->m_pFirstChild) {
          ...
          bSuccess = BEmitVDFRecursive(pCurrent->m_pFirstChild, outputBuffer,
                                       nChildTabLevel, 0, unMaxResultSize,
                                       bIncludeArrayElementName);
          if (!bSuccess)
            return false;
        }
...
//-----------------------------------------------------------------------------
bool CWebAPIValues::BEmitXMLRecursive(const CWebAPIValues *pCurrent,
                                      CUtlBuffer &outputBuffer, int nTabLevel,
                                      size_t unMaxResultSize) {
  bool bSuccess = true;
    ...
    if (pCurrent->GetName()) {
      ...
      if (pCurrent->m_eValueType == k_EWebAPIValueType_Object ||
        ...
        if (pCurrent->m_pFirstChild) {
          ...
          bSuccess = BEmitXMLRecursive(pCurrent->m_pFirstChild, outputBuffer,
                                       nTabLevel + 1, unMaxResultSize);
          if (!bSuccess)
            return false;
...
static int JSONParserCallback(void *void_ctx, int type,
  ...
  case JSON_T_ARRAY_BEGIN:
    ...
    if (!ctx->m_pRootNode) {
      ...
      ctx->m_pRootNode = ctx->m_pCurrentNode =
          new CWebAPIValues(ctx->m_sChildName, "e");
      break;
    }
    ...
    if (ctx->m_pCurrentNode->IsArray()) {
      ...
      Assert(!ctx->m_sChildName.IsEmpty());
      ctx->m_pCurrentNode =
          ctx->m_pCurrentNode->CreateChildArray(ctx->m_sChildName, "e");
  ...
  case JSON_T_OBJECT_BEGIN:
    ...
    if (!ctx->m_pRootNode) {
      ctx->m_pRootNode = ctx->m_pCurrentNode =
          new CWebAPIValues(ctx->m_sChildName);
      break;
    }
...
CWebAPIValues *CWebAPIValues::ParseJSON(const char *pchJSONString) {
  CUtlBuffer bufJSON(pchJSONString, Q_strlen(pchJSONString),
  ...
  case k_EWebAPIValueType_Object: {
    ...
    while (pSourceChild) {
      ...
      if (pSourceChild->IsArray())
        pChild = CreateChildArray(pSourceChild->GetName(),
                                  pSourceChild->GetElementName());
      else
...
//-----------------------------------------------------------------------------
bool ProtoBufHelper::RecursiveAddProtoBufToWebAPIValues(
    CWebAPIValues *pWebAPIRoot, const ::google::protobuf::Message &msg) {
    ...
    if (pField->is_repeated()) {
      if (pReflection->FieldSize(msg, pField) == 0) {
        continue; // No need to create the array if it is empty
                  // If the field has been disabled externally, it has been
                  // cleared already (and thus will be skipped)
      }
      ...
      case FieldDescriptor::CPPTYPE_STRING: {
        if (pField->type() == FieldDescriptor::TYPE_STRING) {
          ...
          AssertMsg1(pField->type() == FieldDescriptor::TYPE_BYTES,
                     "Unrecognized field type: %d", pField->type());
          pchTypeName = "bytes";
        }
        break;
      }
      ...
      CWebAPIValues *pContainer =
          pWebAPIRoot->CreateChildArray(pFieldName, pchTypeName);
      for (int iRepeated = 0; iRepeated < pReflection->FieldSize(msg, pField);
           iRepeated++) {
        switch (pField->cpp_type()) {
        case FieldDescriptor::CPPTYPE_INT32:
          pContainer->SetChildInt32Value(
              NULL, pReflection->GetRepeatedInt32(msg, pField, iRepeated));
          break;
        case FieldDescriptor::CPPTYPE_INT64:
          pContainer->SetChildInt64Value(
              NULL, pReflection->GetRepeatedInt64(msg, pField, iRepeated));
          break;
        case FieldDescriptor::CPPTYPE_UINT32:
          pContainer->SetChildUInt32Value(
              NULL, pReflection->GetRepeatedUInt32(msg, pField, iRepeated));
          break;
        case FieldDescriptor::CPPTYPE_UINT64:
          pContainer->SetChildUInt64Value(
              NULL, pReflection->GetRepeatedUInt64(msg, pField, iRepeated));
          break;
        case FieldDescriptor::CPPTYPE_DOUBLE:
          pContainer->SetChildDoubleValue(
              NULL, pReflection->GetRepeatedDouble(msg, pField, iRepeated));
          break;
        case FieldDescriptor::CPPTYPE_FLOAT:
          pContainer->SetChildDoubleValue(
              NULL, pReflection->GetRepeatedFloat(msg, pField, iRepeated));
          break;
        case FieldDescriptor::CPPTYPE_BOOL:
          pContainer->SetChildBoolValue(
              NULL, pReflection->GetRepeatedBool(msg, pField, iRepeated));
          break;
        case FieldDescriptor::CPPTYPE_ENUM:
          pContainer->SetChildInt32Value(
              NULL,
              pReflection->GetRepeatedEnum(msg, pField, iRepeated)->number());
          break;
        case FieldDescriptor::CPPTYPE_STRING: {
          const std::string &strValue =
              pReflection->GetRepeatedString(msg, pField, iRepeated);
          if (pField->type() == FieldDescriptor::TYPE_STRING) {
            pContainer->SetChildStringValue(NULL, strValue.c_str());
          } else {
            // Binary blobs are automatically encoded in Base64 when converted
            // to string by Web request
            pContainer->SetChildBinaryValue(
                NULL, (const uint8 *)strValue.c_str(), strValue.size());
          }
          break;
        }
        case FieldDescriptor::CPPTYPE_MESSAGE: {
          const ::google::protobuf::Message &subMsg =
              pReflection->GetRepeatedMessage(msg, pField, iRepeated);
      ...
      if (((pReflection->HasField(msg, pField) == false) &&
           (pField->has_default_value() == false))
          //	|| pField->options().GetExtension( is_field_disabled_externally
          //) // Steam vesion supports this
      ) {
        continue; // If no field set and no default value set, there is no need
                  // to send the field Or it is externally disabled (it has been
                  // cleared already but there still may be a default value set)
      }

      switch (pField->cpp_type()) {
      case FieldDescriptor::CPPTYPE_INT32:
        pWebAPIRoot->SetChildInt32Value(pFieldName,
                                        pReflection->GetInt32(msg, pField));
        break;
      case FieldDescriptor::CPPTYPE_INT64:
        pWebAPIRoot->SetChildInt64Value(pFieldName,
                                        pReflection->GetInt64(msg, pField));
        break;
      case FieldDescriptor::CPPTYPE_UINT32:
        pWebAPIRoot->SetChildUInt32Value(pFieldName,
                                         pReflection->GetUInt32(msg, pField));
        break;
      case FieldDescriptor::CPPTYPE_UINT64:
        pWebAPIRoot->SetChildUInt64Value(pFieldName,
                                         pReflection->GetUInt64(msg, pField));
        break;
      case FieldDescriptor::CPPTYPE_DOUBLE:
        pWebAPIRoot->SetChildDoubleValue(pFieldName,
                                         pReflection->GetDouble(msg, pField));
        break;
      case FieldDescriptor::CPPTYPE_FLOAT:
        pWebAPIRoot->SetChildDoubleValue(pFieldName,
                                         pReflection->GetFloat(msg, pField));
        break;
      case FieldDescriptor::CPPTYPE_BOOL:
        pWebAPIRoot->SetChildBoolValue(pFieldName,
                                       pReflection->GetBool(msg, pField));
        break;
      case FieldDescriptor::CPPTYPE_ENUM:
        pWebAPIRoot->SetChildInt32Value(
            pFieldName, pReflection->GetEnum(msg, pField)->number());
        break;
      case FieldDescriptor::CPPTYPE_STRING: {
        ...
        if (pField->type() == FieldDescriptor::TYPE_STRING) {
          ...
          AssertMsg1(pField->type() == FieldDescriptor::TYPE_BYTES,
                     "Unrecognized field type: %d", pField->type());
          // Binary blobs are automatically encoded in Base64 when converted to
          // string by Web request
          pWebAPIRoot->SetChildBinaryValue(
              pFieldName, (const uint8 *)strValue.c_str(), strValue.size());
        }
        break;
      }
      case FieldDescriptor::CPPTYPE_MESSAGE: {
...
#undef GetMessage // Work around unfortunate Microsoft macro
        const ::google::protobuf::Message &subMsg =
            pReflection->GetMessage(msg, pField);
#ifdef UNICODE
