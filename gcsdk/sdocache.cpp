...
GCConVar s_ConVarSDOCacheLRULimitMB("@SDOCacheLRULimitMB", "400",
                                    FCVAR_REPLICATED);
GCConVar s_ConVarSDOCacheMaxMemcachedReadJobs("@SDOCacheMaxMemcachedReadJobs",
                                              "4", FCVAR_REPLICATED);
GCConVar s_ConVarSDOCacheMaxMemcachedReadBatchSize(
    "@SDOCacheMaxMemcachedReadBatchSize", "100", FCVAR_REPLICATED);
GCConVar s_ConVarSDOCacheMaxSQLReadJobsPerSDOType(
    "@SDOCacheMaxSQLReadJobsPerSDOType", "4", FCVAR_REPLICATED);
GCConVar s_ConVarSDOCacheMaxSQLReadBatchSize("@SDOCacheMaxSQLReadBatchSize",
                                             "100", FCVAR_REPLICATED);
GCConVar s_ConVarSDOCacheMaxPendingSQLReads("@SDOCacheMaxPendingSQLReads",
                                            "2000", FCVAR_REPLICATED);
GCConVar s_ConVarSDOCacheMaxPendingMemcachedReads(
    "@SDOCacheMaxPendingMemcachedReads", "25000", FCVAR_REPLICATED);
...
//-----------------------------------------------------------------------------
void CSDOCache::CreateSimpleMemcachedName(CFmtStr &strDest,
...
// Purpose: constructor
//-----------------------------------------------------------------------------
CSDOCache::CSDOCache() : m_cubLRUItems(0), m_mapTypeStats(DefLessFunc(int)) {
...
CSDOCache::~CSDOCache() {
  ...
  FOR_EACH_MAP_FAST(m_mapISDOLoaded, iMap) { delete m_mapISDOLoaded.Key(iMap); }
...
void CSDOCache::OnSDOReleased(ISDO *pSDO) {
  ...
  if (m_mapISDOLoaded.IsValidIndex(iMap)) {
    ...
    // it's actually valid it's not in the SDO cache anymore - could be an
    // orphaned pointer
...
public:
  CGCJobLoadSDOSetFromMemcached(CGCBase *pGC) : CGCJob(pGC) {}
  ...
  bool BYieldingRunJob(void *) {
    ...
    FOR_EACH_VEC(m_vecRequests, i) {
      if (bGetSuccess && vecGetResults.IsValidIndex(i) &&
          vecGetResults[i].m_bKeyFound) {
        const CGCBase::GCMemcachedGetResult_t &result = vecGetResults[i];
        bool bNilObj =
            (result.m_bufValue.Count() == sizeof(k_rgchNilObjSerializedValue) &&
             0 == V_memcmp(result.m_bufValue.Base(),
                           k_rgchNilObjSerializedValue,
                           sizeof(k_rgchNilObjSerializedValue)));

        // we've loaded OK
        if (bNilObj ||
            (result.m_bufValue.Count() &&
             m_vecRequests[i].m_pSDO->BReadFromBuffer(
                 result.m_bufValue.Base(), result.m_bufValue.Count()))) {
          GSDOCache().OnSDOLoadSuccess(m_vecRequests[i].m_pSDO->GetType(),
                                       m_vecRequests[i].m_iRequestID, bNilObj,
                                       &m_vecRequests[i].m_pSDO);
          GSDOCache().GetStats().m_cItemsLoadedFromMemcached += 1;
          GSDOCache().GetStats().m_cNilItemsLoadedFromMemcached +=
              bNilObj ? 1 : 0;
        } else {
          // couldn't load; delete the entry
          m_pGC->BMemcachedDelete(vecKeys[i]);
          GSDOCache().OnMemcachedSDOLoadFailure(
              m_vecRequests[i].m_pSDO->GetType(),
              m_vecRequests[i].m_iRequestID);
        }
      } else {
        // post back failure
        GSDOCache().OnMemcachedSDOLoadFailure(
            m_vecRequests[i].m_pSDO->GetType(), m_vecRequests[i].m_iRequestID);
      }
    }
...
public:
  ...
  bool BYieldingRunJob(void *) {
    Assert(m_vecPSDO.Count() == m_vecRequestID.Count() &&
           m_vecPSDO.Count() == m_vecResults.Count());
    Assert(m_vecPSDO.Count() > 0);
    if (0 == m_vecPSDO.Count() || m_vecPSDO.Count() != m_vecRequestID.Count() ||
        m_vecPSDO.Count() != m_vecResults.Count()) {
    ...
    bool bSQLLayerSucceeded =
        m_vecPSDO[0]->BYldLoadFromSQL(m_vecPSDO, m_vecResults);
    ...
    FOR_EACH_VEC(m_vecRequestID, i) {
      if (bSQLLayerSucceeded) {
        // loaded, great
        GSDOCache().OnSDOLoadSuccess(m_eSDOType, m_vecRequestID[i],
                                     !m_vecResults[i], &m_vecPSDO[i]);
        GSDOCache().WriteSDOToMemcached(m_vecPSDO[i]);

        GSDOCache().GetStats().m_cItemsLoadedFromSQL += 1;
        GSDOCache().GetStats().m_cNilItemsLoadedFromSQL +=
            m_vecResults[i] ? 0 : 1;
      } else {
        // no good, item couldn't load
        GSDOCache().OnSQLSDOLoadFailure(m_eSDOType, m_vecRequestID[i],
                                        bSQLLayerSucceeded);
...
private:
  ...
  // they need to be separate vectors so that they can be easily passed into
  // other API's
...
bool CSDOCache::BFrameFuncRunJobsUntilCompleted(CLimitTimer &limitTimer) {
  ...
  while (m_queueJobsToContinue.Count() && !limitTimer.BLimitReached()) {
    ...
    GGCBase()->GetJobMgr().BRouteWorkItemCompletedIfExists(
        jobToWake.m_jobID, !jobToWake.m_bLoadLayerSuccess);
  }

  // if we're over the limit, LRU an item
  while (!limitTimer.BLimitReached() &&
         m_cubLRUItems >
             (uint32)(s_ConVarSDOCacheLRULimitMB.GetInt() * k_nMegabyte) &&
         m_listLRU.Count()) {
...
// Purpose: runs the queued batch loader
//-----------------------------------------------------------------------------
bool CSDOCache::BFrameFuncRunMemcachedQueriesUntilCompleted(
  ...
  if (m_queueMemcachedRequests.Count()) {
    ...
    if (m_vecMemcachedJobs.Count() <
        s_ConVarSDOCacheMaxMemcachedReadJobs.GetInt()) {
      CGCJobLoadSDOSetFromMemcached *pJob =
          new CGCJobLoadSDOSetFromMemcached(GGCBase());

      // add a full batch to the job
      int cItemsInBatch = 0;
      while (m_queueMemcachedRequests.Count() &&
             cItemsInBatch <
                 s_ConVarSDOCacheMaxMemcachedReadBatchSize.GetInt()) {
        m_StatsSDOCache.m_cQueuedMemcachedRequests--;
        int iMapQueuedRequest =
            m_queueMemcachedRequests[m_queueMemcachedRequests.Head()];
        ...
        if (m_mapQueuedRequests.IsValidIndex(iMapQueuedRequest)) {
          cItemsInBatch++;
          pJob->AddSDOToLoad(m_mapQueuedRequests.Key(iMapQueuedRequest),
                             iMapQueuedRequest);
        }
      }
      ...
      if (cItemsInBatch > 0) {
        ...
        m_StatsSDOCache.m_nMemcachedBatchSizeAvgx100 =
            (int)m_StatMemcachedBatchSize.GetAveragedSample();
...
bool CSDOCache::BFrameFuncRunSQLQueriesUntilCompleted(CLimitTimer &limitTimer) {
  ...
  FOR_EACH_MAP_FAST(m_mapQueueSQLRequests, iMapType) {
    ...
    if (sqlRequestManager->m_queueRequestIDsToLoadFromSQL.Count()) {
      // pass these off to a job
      if (sqlRequestManager->m_vecSQLJobs.Count() <
          s_ConVarSDOCacheMaxSQLReadJobsPerSDOType.GetInt()) {
        int nSDOType = m_mapQueueSQLRequests.Key(iMapType);
        CGCJobLoadSDOSetFromSQL *pJob =
            new CGCJobLoadSDOSetFromSQL(GGCBase(), nSDOType);

        // add a full batch to the job
        int cItemsInBatch = 0;
        while (sqlRequestManager->m_queueRequestIDsToLoadFromSQL.Count() &&
               cItemsInBatch < s_ConVarSDOCacheMaxSQLReadBatchSize.GetInt()) {
          m_StatsSDOCache.m_cQueuedSQLRequests--;
          int iMapQueuedRequest =
              sqlRequestManager->m_queueRequestIDsToLoadFromSQL
                  [sqlRequestManager->m_queueRequestIDsToLoadFromSQL.Head()];
          sqlRequestManager->m_queueRequestIDsToLoadFromSQL.Remove(
              sqlRequestManager->m_queueRequestIDsToLoadFromSQL.Head());
          Assert(m_mapQueuedRequests.IsValidIndex(iMapQueuedRequest));
          if (m_mapQueuedRequests.IsValidIndex(iMapQueuedRequest)) {
            pJob->AddSDOToLoad(m_mapQueuedRequests.Key(iMapQueuedRequest),
        ...
        if (cItemsInBatch > 0) {
          ...
          m_StatsSDOCache.m_nSQLBatchSizeAvgx100 =
              (int)m_StatSQLBatchSize.GetAveragedSample();
  ...
  if (m_StatsSDOCache.m_cItemsUnreferenced) {
    // estimate the total bytes from the size of the average size of an
    // unreferenced item
    m_StatsSDOCache.m_cBytesInCacheEst =
        (m_StatsSDOCache.m_cItemsInCache *
         m_StatsSDOCache.m_cBytesUnreferenced) /
...
int CSDOCache::QueueMemcachedLoad(ISDO *pSDO) {
  int iMap = -1;
  if (m_queueMemcachedRequests.Count() <
      s_ConVarSDOCacheMaxPendingMemcachedReads.GetInt()) {
...
// Purpose: job results
//-----------------------------------------------------------------------------
void CSDOCache::OnSDOLoadSuccess(int eSDOType, int iRequestID, bool bNilObj,
                                 ISDO **ppSDO) {
  ...
  // If the query succeeded but the object doesn't exist then we need to cache
  // that
  if (bNilObj) {
  ...
  // put us in the LRU - if it's just a hint, we may not be referenced
  // immediately
  Assert(m_mapISDOLoaded.IsValidIndex(iMap));
  if (m_mapISDOLoaded.IsValidIndex(iMap)) {
    LRUItem_t item = {pSDO, pSDO->CubBytesUsed()};
...
void CSDOCache::OnMemcachedSDOLoadFailure(int eSDOType, int iRequestID) {
  ...
  AssertMsg1(m_mapQueueSQLRequests.IsValidIndex(iSQLIndex),
             "Error, could not load SDO from SQL for unregistered type %d",
             eSDOType);
  ...
  if (sqlRequestManager->m_queueRequestIDsToLoadFromSQL.Count() <
      s_ConVarSDOCacheMaxPendingSQLReads.GetInt()) {
...
// Purpose: job results
//-----------------------------------------------------------------------------
void CSDOCache::OnSQLSDOLoadFailure(int eSDOType, int iRequestID,
                                    bool bSQLLayerSucceeded) {
  Assert(m_mapQueuedRequests.IsValidIndex(iRequestID));
  if (!m_mapQueuedRequests.IsValidIndex(iRequestID))
    return;
  ...
  // kill the object - no one should have references to it, since it's only in
  // the request list
  ISDO *pSDO = m_mapQueuedRequests.Key(iRequestID);
  m_mapQueuedRequests.RemoveAt(iRequestID);
...
int CSDOCache::CubReferencedEst() {
  return MAX(0, (int)m_StatsSDOCache.m_cBytesInCacheEst -
                    (int)m_StatsSDOCache.m_cBytesUnreferenced);
}
...
void CSDOCache::Dump() {
  ...
  EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
           ...
           m_StatsSDOCache.m_cItemsInCache);
  EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
           "    Estimated Bytes Cached:      %s\n",
           V_pretifymem(m_StatsSDOCache.m_cBytesInCacheEst, 2, true));
  EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
           "    Loads Queued:                %llu\n",
           m_StatsSDOCache.m_cItemsQueuedToLoad);
  EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
           "    Memcached Loads Queued:      %llu\n",
           m_StatsSDOCache.m_cQueuedMemcachedRequests);
  EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
           "    SQL Loads Queued:            %llu\n",
           m_StatsSDOCache.m_cQueuedSQLRequests);
  EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
           "    Items Unreferenced:          %llu\n",
           m_StatsSDOCache.m_cItemsUnreferenced);
  EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
           "    Bytes Unreferenced:          %s\n",
  ...
  FOR_EACH_MAP_FAST(m_mapTypeStats, i) {
    ...
    if (m_mapTypeStats[i].m_nRefed < nLoaded &&
        m_mapTypeStats[i].m_cubUnrefed > 0) {
      pchRefed = V_pretifymem(
          (((int64)nLoaded * (int64)m_mapTypeStats[i].m_cubUnrefed) /
           (nLoaded - m_mapTypeStats[i].m_nRefed)),
          2, true);
    }

    EmitInfo(
        SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS, "%-35s %11d %14d %15d %14s\n",
        m_mapTypeStats[i].m_strName.String(), m_mapTypeStats[i].m_nLoaded,
        m_mapTypeStats[i].m_nRefed, m_mapTypeStats[i].m_nNilObjects, pchRefed);
  }
}

//**tempcomment** - This is good code. We'll hook it back up later
//
// static ConVar s_ConVarVerifyMemcacheDeletesBadEntries(
// "@VerifyMemcacheDeletesBadEntries", "1", FCVAR_REPLICATED );
//
////-----------------------------------------------------------------------------
//// Purpose: verifies a set of memcached data against what's in SQL
////-----------------------------------------------------------------------------
// void CSDOCache::YldVerifyMemcachedData( CreateSDOFunc_t pCreateSDOFunc,
// CUtlVector<uint32> &vecIDs, int *pcMatches, int *pcMismatches )
//{
//	// create the objects
...
//	bool bSQLLayerSucceeded = vecPSDOSQL[0]->BYldLoadFromSQL( vecIDs,
//vecPSDOSQL, vecSQLResults ); 	if ( bSQLLayerSucceeded )
//	{
...
//); 		CUtlVector<MemcachedGetResult_t> vecMemcacheResults; 		if (
//memcachedAccess.BYldGetMulti( vecSKeys, vecMemcacheResults ) )
//		{
//			Assert( vecMemcacheResults.Count() ==
//vecSQLResults.Count() );
//
...
//				if ( vecSQLResults[i] &&
//vecMemcacheResults[i].m_bKeyFound )
//				{
//					// compare the results
//					ISDO *pSDOCached = pCreateSDOFunc(
//*this, vecIDs[i] ); 					const CUtlAllocation &allocMemcache =
//vecMemcacheResults[i].m_bufValue; 					if ( pSDOCached->BReadFromBuffer(
//allocMemcache.Base(), allocMemcache.Count() ) )
//					{
//						if ( pSDOCached->IsEqual(
//vecPSDOSQL[i] ) )
//						{
//							// cool
//							*pcMatches += 1;
...
//							/*
//							Msg( "key %s
//differs:\n", vecSKeys[i] ); 							for ( int n = 0; n < allocMemcache.Count(); n++ )
//							{
//								const byte *p1 =
//(byte*)bufSQL.Base() + n; 								const byte *p2 = (byte*)bufCached.Base() + n; 								if (
//*p1 != *p2 )
//								{
//									Msg( "
//%3d: %2x %2x\n", n, *p1, *p2 );
...
//
//							if (
//s_ConVarVerifyMemcacheDeletesBadEntries.GetBool() ) 								bClearKey = true;
//						}
//					}
//					else
//					{
...
bool CompareSDOObjects(ISDO *pSDO1, ISDO *pSDO2) {
  ...
  return (b1.TellPut() == b2.TellPut() &&
          0 == Q_memcmp(b1.Base(), b2.Base(), b1.TellPut()));
}
