...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
  ...
  int len =
#endif
      _snprintf(szBuffer, sizeof(szBuffer), "PATH=%s\\bin\\;%s", pRootDir,
                pPath);
...
#else
  HINSTANCE launcher =
      LoadLibraryEx(szBuffer, NULL, LOAD_WITH_ALTERED_SEARCH_PATH);
#endif
  if (!launcher) {
    char *pszError;
  ...
  LauncherMain_t main =
      (LauncherMain_t)GetProcAddress(launcher, "LauncherMain");
...
LauncherMain_t GetLaunchEntryPoint(char *pNewCommandLine) {
  ...
  if (!ParseCommandLineArg(pCmdLine, "-dvd")) {
    ...
    if (hFindFile == INVALID_HANDLE_VALUE) {
      Spew("*******************************************************************"
           "\n");
      Spew("FATAL: Access to remote share '%s' on host PC lost. Forcing cold "
           "reboot.\n",
           szFullPath);
      Spew(
