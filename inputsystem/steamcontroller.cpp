...
ConVar sc_joystick_map("sc_joystick_map", "1", FCVAR_ARCHIVE,
                       "How to map the analog joystick deadzone and extents 0 "
                       "= Scaled Cross, 1 = Concentric Mapping to Square.");
...
#define STEAMPAD_ANALOG_SCALE_LEFT(x)                                          \
  ((float)STEAMPAD_MAX_ANALOGSAMPLE_LEFT /                                     \
   (float)(STEAMPAD_MAX_ANALOGSAMPLE_LEFT - (x)))
#define STEAMPAD_ANALOG_SCALE_RIGHT(x)                                         \
  ((float)STEAMPAD_MAX_ANALOGSAMPLE_RIGHT /                                    \
   (float)(STEAMPAD_MAX_ANALOGSAMPLE_RIGHT - (x)))
#define STEAMPAD_ANALOG_SCALE_DOWN(x)                                          \
  ((float)STEAMPAD_MAX_ANALOGSAMPLE_DOWN /                                     \
   (float)(STEAMPAD_MAX_ANALOGSAMPLE_DOWN - (x)))
#define STEAMPAD_ANALOG_SCALE_UP(x)                                            \
  ((float)STEAMPAD_MAX_ANALOGSAMPLE_UP /                                       \
   (float)(STEAMPAD_MAX_ANALOGSAMPLE_UP - (x)))

#define STEAMPAD_ANALOG_TRIGGER_THRESHOLD                                      \
  ((int)(STEAMPAD_MAX_ANALOGSAMPLE_TRIGGER * 0.5f))
#define STEAMPAD_ANALOG_PAD_THRESHOLD                                          \
  ((int)(STEAMPAD_MAX_ANALOGSAMPLE_MAX * 0.25f))
#define STEAMPAD_ANALOG_GYRO_THRESHOLD                                         \
  ((int)(STEAMPAD_MAX_ANALOGSAMPLE_GYRO * 0.3f))
#define STEAMPAD_DIGITAL_PAD_THRESHOLD                                         \
  ((int)(STEAMPAD_MAX_ANALOGSAMPLE_MAX * 0.52f))
...
#define PAD_ANALOG_BUTTON_THRESHOLD                                            \
  ((int)(STEAMPAD_MAX_ANALOGSAMPLE_MAX * 0.285f))
#define PAD_ANALOG_BUTTON_THRESHOLD_STRONG                                     \
  ((int)(STEAMPAD_MAX_ANALOGSAMPLE_MAX * 0.68f))
...
static SDigitalMenuAction g_DigitalMenuActions[] = {
    ...
    {"cl_trigger_first_notification",
     ...
     0,
     {false},
     {false}}, // Command is in the in-game action set (use for hitting
               // accept/ok on dialogs)
    {"cl_decline_first_notification",
     STEAMCONTROLLER_F2,
     0,
     {false},
     {false}}, // Command is in the in-game action set (use for hitting
               // cancel/decline on dialogs)
    {
    ...
    {"menu_alt_function",
     ...
     0,
     {false},
     {false}}, // Command is in the in-game HUD action set
    {"cancelselect",
     STEAMCONTROLLER_START,
     0,
     {false},
     {false}}, // Command is in the in-game action set
    {"toggleready",
     STEAMCONTROLLER_F4,
     0,
     {false},
...
// Table that maps a physical steam controller origin to the corresponding
// character in our icon font. If the EControllerActionOrigin enum or the font
// changes, this table must be changed to match.
...
// Table that maps a physical steam controller origin to a short description
// string for user display (only use this if it's impractical to use the icon
// font). If the EControllerActionOrigin enum changes, this table must be
// changed to match.
...
bool CInputSystem::InitializeSteamControllerActionSets() {
  ...
  for (int i = 0; i != ARRAYSIZE(g_DigitalMenuActions); ++i) {
    g_DigitalMenuActions[i].handle = psteamcontroller->GetDigitalActionHandle(
        g_DigitalMenuActions[i].strName);
  ...
  for (int i = 0; i != ARRAYSIZE(g_GameActionSets); ++i) {
    g_GameActionSets[i].handle =
        psteamcontroller->GetActionSetHandle(g_GameActionSets[i].strName);
...
void CInputSystem::PollSteamControllers(void) {
  ...
  m_unNumConnected =
      steamcontroller->GetConnectedControllers(nControllerHandles);

  if (m_unNumConnected > 0) {
    if (!m_bSteamControllerActionsInitialized) {
      // Retry initialization of controller actions if we didn't acquire them
      // all before for some reason.
      m_bSteamControllerActionsInitialized =
          m_bSteamController && InitializeSteamControllerActionSets();
      g_pInputSystem->ActivateSteamControllerActionSet(
          GAME_ACTION_SET_MENUCONTROLS);
    }

    if (m_bSteamControllerActionsInitialized) {
      // If we successfully initialized all the actions, and we have a connected
      // controller, then we're considered "active".
      ...
      for (int i = 0; i != ARRAYSIZE(g_DigitalMenuActions); ++i) {
        ...
        for (uint64 j = 0; j < m_unNumConnected; ++j) {
          ...
          ControllerDigitalActionData_t data =
              steamcontroller->GetDigitalActionData(nControllerHandles[j],
                                                    action.handle);

          // We only care if the action is active
          if (data.bActive) {
            action.bAwaitingDebounce[j] =
                action.bAwaitingDebounce[j] && data.bState;
            ...
            if (data.bState != action.bState[j]) {
              ...
              ButtonCode_t buttonCode =
                  ButtonCodeToJoystickButtonCode(action.buttonCode, j);
  ...
  if (steamcontroller) {
    ...
    if (m_unNumConnected > 0) {
      for (uint32 i = 0; i < m_unNumConnected; i++) {
        ...
        m_nControllerType[m_Device[i].m_nJoystickIndex] =
...
CInputSystem::GetActionSetHandle(GameActionSet_t eActionSet) {
...
void CInputSystem::ActivateSteamControllerActionSetForSlot(
    uint64 nSlot, GameActionSet_t eActionSet) {
  auto steamcontroller = SteamControllerInterface();
  ...
  if (steamcontroller) {
    if (nSlot == STEAM_CONTROLLER_HANDLE_ALL_CONTROLLERS) {
      ...
      steamcontroller->ActivateActionSet(
          STEAM_CONTROLLER_HANDLE_ALL_CONTROLLERS,
      ...
      if (nSlot < unNumConnected) {
        ...
        steamcontroller->ActivateActionSet(nControllerHandles[nSlot],
                                           g_GameActionSets[eActionSet].handle);
      }
    }
  }
  ...
  if (bChangedActionSet) {
    // If we changed action set, then flag everything for a debounce (meaning we
    // demand to see an unpressed state before we'll register a pressed one)
    for (int i = 0; i < STEAM_CONTROLLER_MAX_COUNT; i++) {
...
//-----------------------------------------------------------------------------
/* void CInputSystem::PulseHapticOnSteamController( uint32 nControllerIndex,
ESteamControllerPad ePad, unsigned short durationMicroSec )
{
        auto steamcontroller = SteamControllerInterface();
        if ( steamcontroller )
        {
                        steamcontroller->TriggerHapticPulse( nControllerIndex,
ePad, durationMicroSec );
        }
}*/
...
void CInputSystem::PostKeyEvent(int iIndex, sKey_t sKey, int nSample) {
  ...
  if (ANALOG_CODE_LAST != code) {
    ...
    if (state.m_pAnalogDelta[code] != 0) {
      PostEvent(IE_AnalogValueChanged, m_nLastSampleTick, code, (int)value,
                state.m_pAnalogDelta[code]);
    }
  }
...
// Gets the action origin (i.e. which physical input) maps to the given virtual
// button for the given action set
EControllerActionOrigin
CInputSystem::GetSteamControllerActionOrigin(const char *action,
                                             GameActionSet_t action_set) {
  ...
  if (pSC) {
    ...
    if (pActionSet) {
      ...
      int nOrigins = pSC->GetDigitalActionOrigins(
          hConnected[0], pActionSet->handle, actionHandle, origins);
      if (nOrigins > 0) {
        return origins[0];
      }
    }
...
// Gets the action origin (i.e. which physical input) maps to the given virtual
// button for the given action set
EControllerActionOrigin CInputSystem::GetSteamControllerActionOrigin(
    const char *action, ControllerActionSetHandle_t action_set_handle) {
  auto pSC = SteamControllerInterface();
  if (pSC && action_set_handle) {
    ...
    EControllerActionOrigin origins[STEAM_CONTROLLER_MAX_ORIGINS];
    int nOrigins = pSC->GetDigitalActionOrigins(
        hConnected[0], action_set_handle, actionHandle, origins);
    if (nOrigins > 0) {
      return origins[0];
    }
  }
...
// Maps a Steam Controller action origin to a string (consisting of a single
// character) in our SC icon font
const wchar_t *CInputSystem::GetSteamControllerFontCharacterForActionOrigin(
...
// Maps a Steam Controller action origin to a short text string (e.g. "X", "LB",
// "LDOWN") describing the control. Prefer to actually use the icon font
// wherever possible.
const wchar_t *CInputSystem::GetSteamControllerDescriptionForActionOrigin(
    EControllerActionOrigin origin) {
  if (origin >= 0 &&
      origin < ARRAYSIZE(g_MapSteamControllerOriginToDescription)) {
