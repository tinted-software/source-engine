...
static ButtonCode_t
ControllerButtonToButtonCode(SDL_GameControllerButton button);
...
ConVar
    joy_axis_deadzone("joy_axis_deadzone", "0.2", FCVAR_ARCHIVE,
                      "Dead zone near the zero point to not report movement.");

static void joy_active_changed_f(IConVar *var, const char *pOldValue,
                                 float flOldValue);
ConVar joy_active(
    "joy_active", "-1", FCVAR_NONE,
    "Which of the connected joysticks / gamepads to use (-1 means first found)",
    &joy_active_changed_f);

static void joy_gamecontroller_config_changed_f(IConVar *var,
                                                const char *pOldValue,
                                                float flOldValue);
ConVar joy_gamecontroller_config("joy_gamecontroller_config", "", FCVAR_ARCHIVE,
                                 "Game controller mapping (passed to SDL with "
                                 "SDL_HINT_GAMECONTROLLERCONFIG), can also be "
                                 "configured in Steam Big Picture mode.",
...
void SearchForDevice() {
  ...
  for (int device_index = 0; device_index < SDL_NumJoysticks();
       ++device_index) {
...
//---------------------------------------------------------------------------------------
void joy_active_changed_f(IConVar *var, const char *pOldValue,
                          float flOldValue) {
  SearchForDevice();
}

//---------------------------------------------------------------------------------------
// Reinitialize the game controller layer when the joy_gamecontroller_config is
// updated.
//---------------------------------------------------------------------------------------
void joy_gamecontroller_config_changed_f(IConVar *var, const char *pOldValue,
                                         float flOldValue) {
  CInputSystem *pInputSystem = (CInputSystem *)g_pInputSystem;
  if (pInputSystem && SDL_WasInit(SDL_INIT_GAMECONTROLLER)) {
    ...
    // We need to reinitialize the whole thing (i.e. undo
    // CInputSystem::InitializeJoysticks and then call it again) due to
    // SDL_GameController only reading the SDL_HINT_GAMECONTROLLERCONFIG on
    // init.
...
int JoystickSDLWatcher(void *userInfo, SDL_Event *event) {
  ...
  switch (event->type) {
  case SDL_CONTROLLERAXISMOTION: {
    pInputSystem->JoystickAxisMotion(event->caxis.which, event->caxis.axis,
                                     event->caxis.value);
    break;
  }

  case SDL_CONTROLLERBUTTONDOWN:
    pInputSystem->JoystickButtonPress(event->cbutton.which,
                                      event->cbutton.button);
    break;
  case SDL_CONTROLLERBUTTONUP:
    pInputSystem->JoystickButtonRelease(event->cbutton.which,
                                        event->cbutton.button);
    break;
...
void CInputSystem::InitializeJoysticks(void) {
  ...
  if (strlen(controllerConfig) > 0) {
    DevMsg("Passing joy_gamecontroller_config to SDL ('%s').\n",
           controllerConfig);
    // We need to pass this hint to SDL *before* we init the gamecontroller
    // subsystem, otherwise it gets ignored.
    SDL_SetHint(SDL_HINT_GAMECONTROLLERCONFIG, controllerConfig);
  }

  if (SDL_InitSubSystem(SDL_INIT_GAMECONTROLLER | SDL_INIT_HAPTIC) == -1) {
    Warning("Joystick init failed -- "
            "SDL_Init(SDL_INIT_GAMECONTROLLER|SDL_INIT_HAPTIC) failed: %s.\n",
            SDL_GetError());
    return;
  }
  ...
  for (int i = 0; i < totalSticks; i++) {
    if (SDL_IsGameController(i)) {
      ...
      Msg("Found joystick '%s' (%s), but no recognized controller "
...
// Update the joy_xcontroller_found convar to force CInput::JoyStickMove to
// re-exec 360controller-linux.cfg
...
void CInputSystem::JoystickHotplugAdded(int joystickIndex) {
  ...
  if (!SDL_IsGameController(joystickIndex)) {
    Warning("Joystick is not recognized by the game controller system. You can "
            "configure the controller in Steam Big Picture mode.\n");
    return;
  }
  ...
  if (activeJoystick < 0) {
    ...
    if (info.m_nDeviceId != -1) {
      Msg("Detected supported joystick #%i '%s'. Currently active joystick is "
          "#%i.\n",
          joystickId, SDL_JoystickNameForIndex(joystickIndex),
          info.m_nDeviceId);
      return;
    }
  ...
  // XXX: This will fail if this is a *real* hotplug event (and not coming from
  // the initial InitializeJoysticks call). That's because the SDL haptic
  // subsystem currently doesn't do hotplugging. Everything but haptics will
  // work fine.
  SDL_Haptic *haptic =
  ...
  // We reset joy_active to -1 because joystick ids are never reused - until you
  // restart. Setting it to -1 means that you get expected hotplugging behavior
  // if you disconnect the current joystick.
...
void CInputSystem::UpdateJoystickButtonState(int nJoystick) {
  // We don't sample - we get events posted by SDL_GameController in
  // JoystickSDLWatcher
}
...
void CInputSystem::PollJoystick(void) {
  // We only pump the SDL event loop if we're not an SDL app, since otherwise
  // PollInputState_Platform calls into CSDLMgr to pump it. Our state updates
  // happen in events posted by SDL_GameController in JoystickSDLWatcher, so the
  // loop is empty.
#if !defined(USE_SDL)
  SDL_Event event;
...
void CInputSystem::SetXDeviceRumble(float fLeftMotor, float fRightMotor,
  ...
  if (SDL_HapticRumblePlay((SDL_Haptic *)info.m_pHaptic, strength,
                           SDL_HAPTIC_INFINITY) != 0) {
...
ButtonCode_t ControllerButtonToButtonCode(SDL_GameControllerButton button) {
  ...
  case SDL_CONTROLLER_BUTTON_GUIDE:
    return KEY_XBUTTON_BACK; // XXX: How are we supposed to handle this? Steam
                             // overlay etc.
