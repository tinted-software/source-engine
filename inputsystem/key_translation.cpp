...
#define SCONTROLLERBUTTONS_BUTTONS(x)                                          \
  "SC_A", "SC_B", "SC_X", "SC_Y", "SC_DPAD_UP", "SC_DPAD_RIGHT",               \
      "SC_DPAD_DOWN", "SC_DPAD_LEFT", "SC_LEFT_BUMPER", "SC_RIGHT_BUMPER",     \
      "SC_LEFT_TRIGGER", "SC_RIGHT_TRIGGER", "SC_LEFT_GRIP", "SC_RIGHT_GRIP",  \
      "SC_LEFT_PAD_TOUCH", "SC_RIGHT_PAD_TOUCH", "SC_LEFT_PAD_CLICK",          \
      "SC_RIGHT_PAD_CLICK", "SC_LPAD_UP", "SC_LPAD_RIGHT", "SC_LPAD_DOWN",     \
      "SC_LPAD_LEFT", "SC_RPAD_UP", "SC_RPAD_RIGHT", "SC_RPAD_DOWN",           \
      "SC_RPAD_LEFT", "SC_SELECT", "SC_START", "SC_STEAM", "SC_NULL"

#define SCONTROLLERBUTTONS_AXIS(x)                                             \
  "SC_LPAD_AXIS_RIGHT", "SC_LPAD_AXIS_LEFT", "SC_LPAD_AXIS_DOWN",              \
      "SC_LPAD_AXIS_UP", "SC_AXIS_L_TRIGGER", "SC_AXIS_R_TRIGGER",             \
      "SC_RPAD_AXIS_RIGHT", "SC_RPAD_AXIS_LEFT", "SC_RPAD_AXIS_DOWN",          \
      "SC_RPAD_AXIS_UP", "SC_GYRO_AXIS_PITCH_POSITIVE",                        \
      "SC_GYRO_AXIS_PITCH_NEGATIVE", "SC_GYRO_AXIS_ROLL_POSITIVE",             \
      "SC_GYRO_AXIS_ROLL_NEGATIVE", "SC_GYRO_AXIS_YAW_POSITIVE",               \
...
void ButtonCode_InitKeyTranslationTable() {
  ...
  s_pSKeytoButtonCode[SK_BUTTON_LPAD_TOUCH] =
      STEAMCONTROLLER_LEFT_PAD_FINGERDOWN;
  s_pSKeytoButtonCode[SK_BUTTON_RPAD_TOUCH] =
  ...
  s_pSKeytoButtonCode[SK_BUTTON_INACTIVE_START] =
      STEAMCONTROLLER_INACTIVE_START;

  // These are fake ("virtual") steam controller buttons that don't physically
  // exist, but we can manufacture to make internal routing to old school UI
  // (which is expecting button code rather than actions) without clashing with
  // other butt
...
ButtonCode_t ButtonCode_SKeyToButtonCode(int nPort, int keyCode) {
  ...
  if (keyCode < 0 ||
      keyCode >= sizeof(s_pSKeytoButtonCode) / sizeof(s_pSKeytoButtonCode[0])) {
    Assert(false);
    return KEY_NONE;
  }
