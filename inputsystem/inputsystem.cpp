...
ConVar joy_xcontroller_found(
    "joy_xcontroller_found", "1", FCVAR_HIDDEN,
...
typedef UINT(WINAPI *GetRawInputData_t)(HRAWINPUT hRawInput, UINT uiCommand,
...
InitReturnVal_t CInputSystem::Init() {
  ...
  // Initialize the input system copy of the steam API context, for use by
  // controller stuff (don't do this if we're a dedicated server).
  ...
  if (IsPC() && !m_bConsoleTextMode) {
    ...
    // NVNT try and load and initialize through the haptic dll, but only if the
  ...
  // Check if this version of windows supports raw mouse input (later than
  // win2k)
  m_bRawInputSupported = false;

  CSysModule *m_pRawInputDLL = Sys_LoadModule("USER32.dll");
  if (m_pRawInputDLL) {
    pfnRegisterRawInputDevices = (RegisterRawInputDevices_t)GetProcAddress(
        (HMODULE)m_pRawInputDLL, "RegisterRawInputDevices");
    pfnGetRawInputData = (GetRawInputData_t)GetProcAddress(
        (HMODULE)m_pRawInputDLL, "GetRawInputData");
...
void CInputSystem::AttachToWindow(void *hWnd) {
  ...
  if (m_hAttachedHWnd) {
    Warning("CInputSystem::AttachToWindow: Cannot attach to two windows at "
            "once!\n");
    return;
  }
...
#if defined(PLATFORM_WINDOWS)
  if (m_ChainedWndProc) {
    SetWindowLongPtrW(m_hAttachedHWnd, GWLP_WNDPROC,
                      (LONG_PTR)m_ChainedWndProc);
    m_ChainedWndProc = 0;
  }
#endif
...
static void initKeymap(void) {
  ...
  // Map keybad enter to enter for vgui. This means vgui dialog won't ever see
  // KEY_PAD_ENTER
...
bool MapCocoaVirtualKeyToButtonCode(int nCocoaVirtualKeyCode,
                                    ButtonCode_t *pOut) {
...
void CInputSystem::PollInputState_Platform() {
  ...
  // These are Carbon virtual key codes. AFAIK they don't have a header that
  // defines these, but they are supposed to map to the same letters across
  // international keyboards, so our mapping here should work.
  ...
  while (1) {
    ...
    for (int iEvent = 0; iEvent < nEvents; iEvent++) {
      ...
      case CocoaEvent_KeyDown: {
        ButtonCode_t virtualCode;
        if (MapCocoaVirtualKeyToButtonCode(pEvent->m_VirtualKeyCode,
                                           &virtualCode)) {
          ButtonCode_t scanCode = virtualCode;

          if ((scanCode != BUTTON_CODE_NONE)) {
            // For SDL, hitting spacebar causes a SDL_KEYDOWN event, then
            // SDL_TEXTINPUT with
            //	event.text.text[0] = ' ', and then we get here and wind up
            //sending two events 	to PostButtonPressedEvent. The first is
            //virtualCode = ' ', the 2nd has virtualCode = 0.
            // This will confuse Button::OnKeyCodePressed(), which is checking
            // for space keydown
            //	followed by space keyup. So we ignore all BUTTON_CODE_NONE
            //events here.
            PostButtonPressedEvent(IE_ButtonPressed, m_nLastSampleTick,
                                   scanCode, virtualCode);
          }
        ...
        if (!(pEvent->m_ModifierKeyMask & (1 << eCommandKey)) &&
            pEvent->m_VirtualKeyCode >= 0 && pEvent->m_UnicodeKey > 0) {
          InputEvent_t event;
          memset(&event, 0, sizeof(event));
          event.m_nTick = GetPollTick();
          // IE_KeyTyped
      ...
      case CocoaEvent_MouseMove: {
        UpdateMousePositionState(state, (short)pEvent->m_MousePos[0],
                                 (short)pEvent->m_MousePos[1]);
      ...
      case CocoaEvent_MouseScroll: {
        ButtonCode_t code = (short)pEvent->m_MousePos[1] > 0 ? MOUSE_WHEEL_UP
                                                             : MOUSE_WHEEL_DOWN;
      ...
      case CocoaEvent_AppActivate: {
        InputEvent_t event;
        memset(&event, 0, sizeof(event));
        event.m_nType = IE_FirstAppEvent +
                        2; // IE_AppActivated (defined in sys_mainwind.cpp).
        event.m_nData = (bool)(pEvent->m_ModifierKeyMask != 0);

        g_pInputSystem->PostUserEvent(event);

        if (pEvent->m_ModifierKeyMask == 0) {
          // App just lost focus. Handle like WM_ACTIVATEAPP in
          // CInputSystem::WindowProc(). Otherwise alt+tab will bring focus away
          // from our app, vgui will still think that
...
void CInputSystem::UpdateMousePositionState(InputState_t &state, short x,
                                            short y) {
...
LRESULT CInputSystem::WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam,
  ...
  case WM_SYSKEYDOWN: {
    ...
    if (!(lParam & (1 << 30))) {
      ...
      // which returns KEY_ENTER from virtual keys, and KEY_PAD_ENTER from scan
      // codes Since things like vgui care about virtual keys; we're going to
      ...
      // Post ETW events describing key presses to help correlate input events
      // to performance problems in the game.
      ...
      if (scanCode == KEY_CAPSLOCK || scanCode == KEY_SCROLLLOCK ||
        ...
        PostButtonPressedEvent(bToggleState ? IE_ButtonPressed
                                            : IE_ButtonReleased,
  ...
  case WM_INPUT: {
    if (m_bRawInputSupported) {
      ...
      pfnGetRawInputData((HRAWINPUT)lParam, RID_INPUT, lpb, &dwSize,
  ...
  case WM_MOUSEMOVE: {
    UpdateMousePositionState(state, (short)LOWORD(lParam),
                             (short)HIWORD(lParam));
