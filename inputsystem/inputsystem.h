...
public:
  ...
  // Called for mouse move events. Sets the current x and y and posts events for
  // the mouse moving.
  void UpdateMousePositionState(InputState_t &state, short x, short y);

  // Initializes SteamControllers - Returns true if steam is running and finds
  // controllers, otherwise false
  ...
  void JoystickButtonPress(int joystickId,
                           int button); // button is a SDL_CONTROLLER_BUTTON;
  ...
  void FingerEvent(int eventType, int fingerId, float x, float y, float dx,
                   float dy);
  ...
  void ActivateSteamControllerActionSetForSlot(uint64 nSlot,
                                               GameActionSet_t eActionSet);
  ...
  // Gets the action origin (i.e. which physical input) maps to the given action
  // name
  virtual EControllerActionOrigin
  GetSteamControllerActionOrigin(const char *action,
  ...
  virtual const wchar_t *GetSteamControllerFontCharacterForActionOrigin(
      EControllerActionOrigin origin);

  // Maps a Steam Controller action origin to a short text string (e.g. "X",
  // "LB", "LDOWN") describing the control. Prefer to actually use the icon font
  // wherever possible.
  virtual const wchar_t *
  GetSteamControllerDescriptionForActionOrigin(EControllerActionOrigin origin);

  // Converts SteamController keys to button codes
  ButtonCode_t SKeyToButtonCode(int nUserId, int nXKey) const;

  // This is called with "true" by dedicated server initialization (before
  // calling Init) in order to force us to skip initialization of Steam (which
  // messes up dedicated servers).
  ...
  bool m_bSteamController; // true if the Steam Controller system has been
                           // initialized successfully (this doesn't mean one is
                           // actually connected necessarily)
  bool m_bSteamControllerActionsInitialized; // true if our action sets and
                                             // handles were successfully
                                             // initialized (this doesn't mean a
                                             // controller is necessarily
                                             // connected, or that in-game
                                             // client actions were initialized)
  bool m_bSteamControllerActive; // true if our action sets and handles were
                                 // successfully initialized *and* that at least
                                 // one controller is actually connected and
                                 // switched on.
  ...
  float m_touchAccumX[TOUCH_FINGER_MAX_COUNT],
      m_touchAccumY[TOUCH_FINGER_MAX_COUNT];
