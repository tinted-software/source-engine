...
//-----------------------------------------------------------------------------
float CNetworkSystem::GetTime(void) { return Plat_FloatTime(); }
...
INetworkMessage *CNetworkSystem::FindNetworkMessage(int group, int type) {
  int nCount = m_NetworkMessages.Count();
  for (int i = 0; i < nCount; i++) {
    if ((m_NetworkMessages[i]->GetGroup() == group) &&
        (m_NetworkMessages[i]->GetType() == type))
...
// Server update
//-----------------------------------------------------------------------------
INetChannel *CNetworkSystem::ConnectClientToServer(const char *pServer,
...
//-----------------------------------------------------------------------------
void CNetworkSystem::EnqueueConnectionlessNetworkPacket(
    CNetPacket *pPacket, IConnectionlessPacketHandler *pHandler) {
...
void CNetworkSystem::EnqueueNetworkPacket(CNetPacket *pPacket,
...
NetworkEvent_t *CNetworkSystem::NextNetworkEvent() {
  ...
  while (true) {
    ...
    // one that encounters a packet we need to process, or we exhaust the event
    // queue
