...
//-----------------------------------------------------------------------------
static CUtlMemoryPool s_PacketBufferAlloc(NET_MAX_MESSAGE, 8,
...
CNetPacket::~CNetPacket() { s_PacketBufferAlloc.Free(m_pData); }

void CNetPacket::AddRef() { ++m_nRefCount; }
...
void UDP_ProcessSocket(CUDPSocket *pSocket,
                       IConnectionlessPacketHandler *pHandler,
                       ILookupChannel *pLookup) {
  CNetPacket *pPacket;
  ...
  while ((pPacket = UDP_GetPacket(pSocket)) != NULL) {
    /*
    if ( Filter_ShouldDiscard ( pPacket->m_From ) )	// filterning is done by
    network layer
    {
            Filter_SendBan( pPacket->m_From );	// tell them we aren't
    listening... continue;
    }
    */
    ...
    if (*(unsigned int *)pPacket->m_pData == CONNECTIONLESS_HEADER) {
      g_pNetworkSystemImp->EnqueueConnectionlessNetworkPacket(pPacket,
                                                              pHandler);
    } else {
      INetChannel *pNetChan = pLookup->FindNetChannel(pPacket->m_From);
      g_pNetworkSystemImp->EnqueueNetworkPacket(
          pPacket, static_cast<CNetChannel *>(pNetChan));
    }
  }
}

void UDP_ProcessConnectionlessPacket(CNetPacket *pPacket,
                                     IConnectionlessPacketHandler *pHandler) {
  ...
  pPacket->m_nSizeInBytes, pPacket->m_pData[4] );
  }
  */
