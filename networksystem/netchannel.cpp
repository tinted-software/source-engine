...
CNetChannel::CNetChannel() {
  ...
  m_StreamUnreliable.StartWriting(m_UnreliableDataBuffer,
                                  sizeof(m_UnreliableDataBuffer));
  m_StreamUnreliable.SetDebugName("netchan_t::unreliabledata");

  m_StreamReliable.StartWriting(m_ReliableDataBuffer,
                                sizeof(m_ReliableDataBuffer));
...
CNetChannel::~CNetChannel() { Shutdown("NetChannel removed."); }

//-----------------------------------------------------------------------------
// called to open a channel to a remote system
//-----------------------------------------------------------------------------
void CNetChannel::Setup(bool serverSide, const netadr_t *adr,
                        CUDPSocket *sendSocket, char const *name,
...
/*
void CNetChannel::GetSequenceData( int &nOutSequenceNr, int &nInSequenceNr, int
&nOutSequenceNrAck )
{
...
void CNetChannel::SetSequenceData( int nOutSequenceNr, int nInSequenceNr, int
nOutSequenceNrAck )
{
        Assert( IsPlayback() );
...
const char *CNetChannel::GetName() const { return m_Name; }
...
void CNetChannel::FlowNewPacket(int flow, int seqnr, int acknr, int nChoked, int
nSize )
{
        ...
        if ( seqnr > pflow->currentindex )
        {
                for ( int i = pflow->currentindex+1; i <= seqnr; i++ )
                {
                        ...
                        Q_memset( &pframe->msggroups, 0,
sizeof(pframe->msggroups) );
                }
        ...
        netframe_t * aframe = &m_DataFlow[aflow].frames[ acknr  &
NET_FRAMES_MASK ];

        if ( aframe->valid && aframe->latency == -1.0f )
        {
                // update ping for acknowledged packet, if not already
acknowledged before
...
void CNetChannel::FlowUpdate(int flow, int addbytes)
{
        ...
        if ( totaltime > 0.0f )
        {
                ...
                pflow->avgbytespersec += ( 1.0f - FLOW_AVG ) *
((float)totalbytes / totaltime);

                pflow->avgpacketspersec *= FLOW_AVG;
                pflow->avgpacketspersec += ( 1.0f - FLOW_AVG ) *
((float)totalvalid / totaltime);
        }
...
((float)(totalinvalid-totalchoked)/totalPackets);
                ...
                pflow->avgchoke += ( 1.0f - FLOW_AVG ) *
((float)totalchoked/totalPackets);
        }
...
int CNetChannel::SendDatagram(bf_write *datagram) {
  ...
  */
  ...
  int nHeaderSize = g_pNetworkSystemImp->GetGroupBitCount() +
                    g_pNetworkSystemImp->GetTypeBitCount();
  ...
  //m_nChokedPackets, nTotalSize ); 	FlowUpdate( FLOW_OUTGOING, nTotalSize );
  ...
  while (true) {
    ...
    if (group == net_group_networksystem) {
      if (!ProcessControlMessage(type, buf))
        return g_pNetworkSystemImp
            ->IsNetworkEventCreated(); // disconnect or error
      continue;
    }

    // see if we have a registered message object for this type
    INetworkMessage *pNetMessage =
        g_pNetworkSystemImp->FindNetworkMessage(group, type);
    if (!pNetMessage) {
    ...
    if (!pNetMessage->ReadFromBuffer(buf)) {
      Msg("Netchannel: failed reading message %s [%s] from %s.\n", pMessageName,
          pGroupName, remote_address.ToString());
      Assert(0);
      return false;
    }
    ...
    NetworkMessageReceivedEvent_t *pReceived =
        g_pNetworkSystemImp
            ->CreateNetworkEvent<NetworkMessageReceivedEvent_t>();
...
int CNetChannel::ProcessPacketHeader(bf_read &message) {
  ...
  // FlowNewPacket( FLOW_INCOMING, m_nInSequenceNr, m_nOutSequenceNrAck,
  // nChoked, packet->size + UDP_HEADER_SIZE );
    ...
    for ( i=0; i<MAX_STREAMS; i++ )
    {
            if ( msg.ReadOneBit() != 0 )
            {
                    if ( !ReadSubChannelData( msg, i ) )
                            return false; // error while reading fragments, drop
    whole packet
            }
    }
...
bool CNetChannel::AddData(bf_write &msg, bool bReliable) {
  ...
  if (msg.GetNumBitsWritten() > buf->GetNumBitsLeft()) {
    if (bReliable) {
      Msg("ERROR! SendData reliabe data too big (%i)",
...
int CNetChannel::GetDataRate() const { return m_Rate; }
...
const netadr_t &CNetChannel::GetRemoteAddress() const { return remote_address; }
...
bool CNetChannel::IsTimingOut() const {
  if (m_Timeout == -1.0f)
    ...
    return (last_received + CONNECTION_PROBLEM_TIME) <
           g_pNetworkSystemImp->GetTime();
}
...
CUDPSocket *CNetChannel::GetSocket() { return m_pSocket; }
