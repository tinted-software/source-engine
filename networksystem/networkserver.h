...
// Server class
//-----------------------------------------------------------------------------
class CNetworkServer : public IConnectionlessPacketHandler,
                       public INetworkMessageHandler,
                       public ILookupChannel {
public:
  CNetworkServer();
  ...
  virtual bool ProcessConnectionlessPacket(
      CNetPacket *packet); // process a connectionless packet
...
public:
  ...
  const netadr_t &GetRemoteAddress() { return m_NetChan.GetRemoteAddress(); }

  char const *GetName() { return "Foozle"; }
