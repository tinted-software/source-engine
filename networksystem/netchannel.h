...
public:
  virtual void OnConnectionClosing(INetChannel * channel,
                                   char const *reason) = 0;
...
public:
  ...
  virtual void ConnectionClosing(
      const char *reason) = 0; // network channel is being closed by remote site

  virtual void
  ConnectionCrashed(const char *reason) = 0; // network error occured
...
public:
  virtual bool ProcessConnectionlessPacket(
      CNetPacket * packet) = 0; // process a connectionless packet
...
public:
  ...
  void Setup(bool serverSide, const netadr_t *remote_address,
  ...
  int SendDatagram(bf_write *data); // Adds data to unreliable payload and then
                                    // calls transmits the data
  bool
  Transmit(bool onlyReliable =
               false); // send data from buffers (calls SendDataGram( NULL ) )
...
private:
  ...
  byte m_ReliableDataBuffer[8 * 1024]; // In SP, we don't need much reliable
                                       // buffer, so save the memory (this is
                                       // mostly for xbox).
