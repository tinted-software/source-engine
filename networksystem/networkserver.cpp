...
bool CNetworkServer::ProcessConnectionlessPacket(CNetPacket *packet) {
  ...
  default: {
    Warning("CNetworkServer::ProcessConnectionlessPacket:  Unknown code '%i' "
            "from '%s'\n",
...
void CNetworkServer::OnConnectionStarted(INetChannel *pChannel) {
  ...
  NetworkConnectionEvent_t *pConnection =
      g_pNetworkSystemImp->CreateNetworkEvent<NetworkConnectionEvent_t>();
  pConnection->m_nType = NETWORK_EVENT_CONNECTED;
  pConnection->m_pChannel = pChannel;
}

void CNetworkServer::OnConnectionClosing(INetChannel *pChannel,
                                         char const *reason) {
