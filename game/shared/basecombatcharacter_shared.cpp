...
//-----------------------------------------------------------------------------
// Purpose: Switches to the best weapon that is also better than the given
// weapon. Input  : pCurrent - The current weapon used by the player. Output :
// Returns true if the weapon was switched, false if there was no better
//			weapon to switch to.
//-----------------------------------------------------------------------------
bool CBaseCombatCharacter::SwitchToNextBestWeapon(CBaseCombatWeapon *pCurrent) {
  CBaseCombatWeapon *pNewWeapon =
      g_pGameRules->GetNextBestWeapon(this, pCurrent);
...
//-----------------------------------------------------------------------------
bool CBaseCombatCharacter::Weapon_Switch(CBaseCombatWeapon *pWeapon,
...
//-----------------------------------------------------------------------------
// Purpose: Returns the amount of ammunition of the specified type the
...
//-----------------------------------------------------------------------------
CBaseCombatWeapon *
CBaseCombatCharacter::Weapon_OwnsThisType(const char *pszWeapon,
                                          int iSubType) const {
...
int CBaseCombatCharacter::BloodColor() { return m_bloodColor; }
...
//-----------------------------------------------------------------------------
/**
        The main visibility check.  Checks all the entity specific reasons that
   could make IsVisible fail.  Then checks points in space to get environmental
   reasons. This is LOS, plus invisibility and fog and smoke and such.
*/
...
enum { VIS_CACHE_INVALID = 0x80000000 };
...
public:
  ...
  int LookupVisibility(const CBaseCombatCharacter *pChar1,
                       CBaseCombatCharacter *pChar2);
  VisCacheResult_t HasVisibility(int iCache) const;
  void RegisterVisibility(int iCache, bool bChar1SeesChar2,
                          bool bChar2SeesChar1);
...
private:
  ...
  public:
    ...
    bool operator()(const VisCacheEntry_t &lhs,
                    const VisCacheEntry_t &rhs) const {
...
void CCombatCharVisCache::FrameUpdatePreEntityThink() {
  ...
  for (int i = 0; i < 8; ++i) {
    ...
    if (!entry.m_hEntity1.IsValid() || !entry.m_hEntity2.IsValid() ||
        (gpGlobals->curtime - entry.m_flTime > 10.0f)) {
      m_VisCache.RemoveAt(n);
    }
  }
}

void CCombatCharVisCache::LevelShutdownPreEntity() { m_VisCache.Purge(); }

int CCombatCharVisCache::LookupVisibility(const CBaseCombatCharacter *pChar1,
                                          CBaseCombatCharacter *pChar2) {
  ...
  if (iCache == m_VisCache.InvalidIndex()) {
    ...
    m_VisCache[iCache].m_flTime =
        gpGlobals->curtime - 2.0f * VIS_CACHE_ENTRY_LIFE;
  }
...
VisCacheResult_t CCombatCharVisCache::HasVisibility(int iCache) const {
  ...
  bool bIsVisible =
      !bReverse ? entry.m_bEntity1CanSeeEntity2 : entry.m_bEntity2CanSeeEntity1;
  return bIsVisible ? VISCACHE_IS_VISIBLE : VISCACHE_IS_NOT_VISIBLE;
}

void CCombatCharVisCache::RegisterVisibility(int iCache,
                                             bool bEntity1CanSeeEntity2,
                                             bool bEntity2CanSeeEntity1) {
  if (iCache == VIS_CACHE_INVALID)
    return;
...
bool CBaseCombatCharacter::IsAbleToSee(const CBaseEntity *pEntity,
                                       FieldOfViewCheckType checkFOV) {
  CBaseCombatCharacter *pBCC =
      const_cast<CBaseEntity *>(pEntity)->MyCombatCharacterPointer();
  if (pBCC)
  ...
  if (flDistToOther > NavObscureRange.GetFloat()) {
    ...
    TerrorNavArea *pTargetArea = static_cast<TerrorNavArea *>(
        TheNavMesh->GetNearestNavArea(vecTargetPosition, false, flMaxDistance));
    if (!pTargetArea ||
        pTargetArea->HasSpawnAttributes(TerrorNavArea::SPAWN_OBSCURED))
      return false;

    if (ComputeTargetIsInDarkness(vecEyePosition, pTargetArea,
                                  vecTargetPosition))
      return false;
  }
#endif
...
static void ComputeSeeTestPosition(Vector *pEyePosition,
                                   CBaseCombatCharacter *pBCC) {
#if defined(GAME_DLL) && defined(TERROR)
  if (pBCC->IsPlayer()) {
    CTerrorPlayer *pPlayer = ToTerrorPlayer(pBCC);
    *pEyePosition = !pPlayer->IsDead() ? pPlayer->EyePosition()
                                       : pPlayer->GetDeathPosition();
  } else
#endif
  {
...
bool CBaseCombatCharacter::IsAbleToSee(CBaseCombatCharacter *pBCC,
                                       FieldOfViewCheckType checkFOV) {
...
#ifdef TERROR
  // Check this every time also, it's cheap; check to see if the enemy is in an
  // obscured area.
  bool bIsInNavObscureRange = (flDistToOther > NavObscureRange.GetFloat());
  if (bIsInNavObscureRange) {
    TerrorNavArea *pOtherNavArea =
        static_cast<TerrorNavArea *>(pBCC->GetLastKnownArea());
    if (!pOtherNavArea ||
        pOtherNavArea->HasSpawnAttributes(TerrorNavArea::SPAWN_OBSCURED))
      return false;
  }
#endif // TERROR
#endif
  ...
  if (nResult == VISCACHE_UNKNOWN) {
    ...
    if (ComputeLOS(vecEyePosition, vecOtherEyePosition)) {
      ...
      if (!bIsInNavObscureRange) {
        ...
        bThisCanSeeOther = !ComputeTargetIsInDarkness(
            vecEyePosition, pBCC->GetLastKnownArea(), vecOtherEyePosition);
        bOtherCanSeeThis = !ComputeTargetIsInDarkness(
            vecOtherEyePosition, GetLastKnownArea(), vecEyePosition);
      }
#else
      bThisCanSeeOther = true, bOtherCanSeeThis = true;
#endif
    ...
    s_CombatCharVisCache.RegisterVisibility(iCache, bThisCanSeeOther,
                                            bOtherCanSeeThis);
...
public:
  CTraceFilterNoCombatCharacters(const IHandleEntity *passentity = NULL,
...
bool CBaseCombatCharacter::ComputeLOS(const Vector &vecEyePosition,
  ...
  UTIL_TraceLine(vecEyePosition, vecTarget,
...
#if defined(GAME_DLL) && defined(TERROR)
bool CBaseCombatCharacter::ComputeTargetIsInDarkness(
    const Vector &vecEyePosition, CNavArea *pTargetNavArea,
  ...
  if (!pTargetNavArea ||
      (pTargetNavArea->GetLightIntensity() >= flMinLightIntensity))
    return false;
  ...
  trace_t result;
  UTIL_TraceLine(vecTargetPos, vecTargetPos + vecSightDirection * 32768.0f,
                 MASK_L4D_VISION, &lightingFilter, &result);
  if ((result.fraction < 1.0f) && ((result.surface.flags & SURF_SKY) == 0)) {
    const float flMaxDistance = 100.0f;
    TerrorNavArea *pFarArea = (TerrorNavArea *)TheNavMesh->GetNearestNavArea(
        result.endpos, false, flMaxDistance);

    // Target is in darkness, the wall behind him is too, and we are too far
    // away
...
/**
        ...
        within the cosine of the angular tolerance. LINE OF SIGHT IS NOT
   CHECKED.
*/
bool CBaseCombatCharacter::IsLookingTowards(const CBaseEntity *target,
...
/**
   ...
   CHECKED.
*/
bool CBaseCombatCharacter::IsLookingTowards(const Vector &target,
                                            float cosTolerance) const {
...
//-----------------------------------------------------------------------------
/**
        Returns true if we are looking towards something within a tolerence
   determined by our field of view
*/
bool CBaseCombatCharacter::IsInFieldOfView(CBaseEntity *entity) const {
  ...
  float flTolerance = pPlayer ? cos((float)pPlayer->GetFOV() * 0.5f)
                              : BCC_DEFAULT_LOOK_TOWARDS_TOLERANCE;
  ...
  // Check 3 spots, or else when standing right next to someone looking at their
  // eyes, the angle will be too great to see their center.
...
bool CBaseCombatCharacter::IsLineOfSightClear(
    CBaseEntity *entity, LineOfSightCheckType checkType) const {
#ifdef CLIENT_DLL
...
/**
        Strictly checks Line of Sight only.
*/
static bool TraceFilterNoCombatCharacters(IHandleEntity *pServerEntity,
                                          int contentsMask) {
  ...
  return (entity->MyCombatCharacterPointer() == NULL &&
          !entity->MyCombatWeaponPointer() && entity->BlocksLOS());
}

bool CBaseCombatCharacter::IsLineOfSightClear(
    const Vector &pos, LineOfSightCheckType checkType,
  ...
  if (checkType == IGNORE_ACTORS) {
...
#if defined(GAME_DLL) && defined(TERROR)
    return IsLineOfSightBetweenTwoEntitiesClear(
        const_cast<CBaseCombatCharacter *>(this), EOFFSET_MODE_EYEPOSITION,
        entityToIgnore, EOFFSET_MODE_WORLDSPACE_CENTER, entityToIgnore,
        COLLISION_GROUP_NONE, MASK_L4D_VISION, TraceFilterNoCombatCharacters,
        1.0);
#else
    trace_t trace;
    CTraceFilterNoCombatCharacters traceFilter(entityToIgnore,
                                               COLLISION_GROUP_NONE);
    UTIL_TraceLine(EyePosition(), pos,
                   MASK_OPAQUE | CONTENTS_IGNORE_NODRAW_OPAQUE |
                       CONTENTS_MONSTER,
                   &traceFilter, &trace);
    ...
    trace_t trace;
    CTraceFilterSkipTwoEntities traceFilter(this, entityToIgnore,
                                            COLLISION_GROUP_NONE);
    UTIL_TraceLine(EyePosition(), pos,
                   MASK_OPAQUE | CONTENTS_IGNORE_NODRAW_OPAQUE, &traceFilter,
                   &trace);
...
surfacedata_t * CBaseCombatCharacter::GetGroundSurface( void ) const
{
        ...
        trace_t	trace;
        UTIL_TraceRay( ray, MASK_SOLID, this, COLLISION_GROUP_PLAYER_MOVEMENT,
&trace );
