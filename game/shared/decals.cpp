...
public:
  ...
  virtual char const *TranslateDecalForGameMaterial(char const *decalName,
                                                    unsigned char gamematerial);
...
void CDecalEmitterSystem::LevelInitPreEntity() {
  ...
  for (int i = 0; i < c; i++) {
    ...
    e.precache_index = effects->Draw_DecalIndexFromName(
        (char *)m_DecalFileNames.String(e.name));
#else
...
void CDecalEmitterSystem::LoadDecalsFromScript(char const *filename) {
  ...
  if (kv) {
      ...
      while (p) {
        if (p->GetFirstSubKey()) {
          ...
          if (!Q_stricmp(keyname, TRANSLATION_DATA_SECTION)) {
            ...
            for (KeyValues *sub = p->GetFirstSubKey(); sub != NULL;
                 sub = sub->GetNextKey()) {
              MEM_ALLOC_CREDIT();
    ...
    if (!translation) {
      Msg("CDecalEmitterSystem::LoadDecalsFromScript:  Script '%s' missing "
          "section '%s'\n",
          filename, TRANSLATION_DATA_SECTION);
    } else {
      // Now parse game material to entry translation table
      for (KeyValues *sub = translation->GetFirstSubKey(); sub != NULL;
           sub = sub->GetNextKey()) {
        ...
        if (idx != m_Decals.InvalidIndex()) {
          ...
          Msg("CDecalEmitterSystem::LoadDecalsFromScript:  Translation for "
              "game material type '%s' references unknown decal '%s'\n",
...
// Purpose:
//-----------------------------------------------------------------------------
void CDecalEmitterSystem::Shutdown() { Clear(); }
...
//-----------------------------------------------------------------------------
char const *
CDecalEmitterSystem::TranslateDecalForGameMaterial(char const *decalName,
                                                   unsigned char gamematerial) {
