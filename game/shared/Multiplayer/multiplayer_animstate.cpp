...
ConVar anim_showmainactivity(
    "anim_showmainactivity", "0", FCVAR_CHEAT,
...
ConVar anim_showstate("anim_showstate", "-1",
                      FCVAR_CHEAT | FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY,
                      "Show the (client) animation state for the specified "
                      "entity (-1 for none).");
ConVar anim_showstatelog("anim_showstatelog", "0",
                         FCVAR_CHEAT | FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY,
                         "1 to output anim_showstate to Msg(). 2 to store in "
                         "AnimState.log. 3 for both.");
ConVar mp_showgestureslots(
    "mp_showgestureslots", "-1",
    FCVAR_CHEAT | FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY,
    "Show multiplayer client/server gesture slot information for the specified "
    "player index (-1 for no one).");
ConVar mp_slammoveyaw("mp_slammoveyaw", "0",
...
#ifdef CLIENT_DLL
  m_iv_flMaxGroundSpeed.Setup(&m_flMaxGroundSpeed,
...
//			&movementData -
//-----------------------------------------------------------------------------
void CMultiPlayerAnimState::Init(CBasePlayer *pPlayer,
...
// Input  : event -
//-----------------------------------------------------------------------------
void CMultiPlayerAnimState::DoAnimationEvent(PlayerAnimEvent_t event,
                                             int nData) {
  switch (event) {
  ...
  case PLAYERANIMEVENT_RELOAD: {
...
#if defined(TF_CLIENT_DLL) || defined(TF_DLL)
    CALL_ATTRIB_HOOK_FLOAT_ON_OTHER(GetBasePlayer(), flPlaybackRate,
                                    mult_reload_time);
    CALL_ATTRIB_HOOK_FLOAT_ON_OTHER(GetBasePlayer(), flPlaybackRate,
    ...
    CALL_ATTRIB_HOOK_FLOAT_ON_OTHER(GetBasePlayer(), flPlaybackRate,
                                    fast_reload);
#endif
    m_aGestureSlots[GESTURE_SLOT_ATTACK_AND_RELOAD]
        .m_pAnimLayer->m_flPlaybackRate = flPlaybackRate;
  ...
  case PLAYERANIMEVENT_CUSTOM: {
    ...
    m_nSpecificMainSequence =
        GetBasePlayer()->SelectWeightedSequence(iIdealActivity);
    RestartMainSequence();
  } break;
...
bool CMultiPlayerAnimState::InitGestureSlots(void) {
  ...
  // Assign all of the the CAnimationLayer pointers to null early in case we
  // bail.
...
// Purpose:
//-----------------------------------------------------------------------------
void CMultiPlayerAnimState::RunGestureSlotAnimEventsToCompletion(
    GestureSlot_t *pGesture) {
  CBasePlayer *pPlayer = GetBasePlayer();
  if (!pPlayer)
    return;
  ...
  mstudioseqdesc_t &seqdesc =
      pStudioHdr->pSeqdesc(pGesture->m_pAnimLayer->m_nSequence);
  if (seqdesc.numevents > 0) {
    ...
    for (int i = 0; i < (int)seqdesc.numevents; i++) {
      ...
      if (pevent[i].cycle > pGesture->m_pAnimLayer->m_flPrevCycle &&
        ...
        pPlayer->FireEvent(pPlayer->GetAbsOrigin(), pPlayer->GetAbsAngles(),
                           pevent[i].event, pevent[i].pszOptions());
      }
    }
  }
}
...
bool CMultiPlayerAnimState::VerifyAnimLayerInSlot(int iGestureSlot) {
  ...
  if (GetBasePlayer()->GetNumAnimOverlays() < iGestureSlot + 1) {
    AssertMsg2(false, "Player %d doesn't have gesture slot %d any more.",
  ...
  if (m_aGestureSlots[iGestureSlot].m_pAnimLayer != pExpected) {
    AssertMsg3(false,
               "Gesture slot %d pointing to wrong address %p. Updating to new "
               "address %p.",
               iGestureSlot, m_aGestureSlots[iGestureSlot].m_pAnimLayer,
               pExpected);
    Msg("Gesture slot %d pointing to wrong address %p. Updating to new address "
        "%p.\n",
...
// Purpose:
//-----------------------------------------------------------------------------
bool CMultiPlayerAnimState::IsGestureSlotPlaying(int iGestureSlot,
                                                 Activity iGestureActivity) {
  // Sanity Check
  Assert(iGestureSlot >= 0 && iGestureSlot < GESTURE_SLOT_COUNT);
...
// Purpose:
//-----------------------------------------------------------------------------
void CMultiPlayerAnimState::RestartGesture(int iGestureSlot,
                                           Activity iGestureActivity,
                                           bool bAutoKill) {
  // Sanity Check
  Assert(iGestureSlot >= 0 && iGestureSlot < GESTURE_SLOT_COUNT);
...
// Purpose:
//-----------------------------------------------------------------------------
void CMultiPlayerAnimState::AddToGestureSlot(int iGestureSlot,
                                             Activity iGestureActivity,
                                             bool bAutoKill) {
  // Sanity Check
  Assert(iGestureSlot >= 0 && iGestureSlot < GESTURE_SLOT_COUNT);
  ...
  m_aGestureSlots[iGestureSlot].m_pAnimLayer->m_flLastEventCheck =
      gpGlobals->curtime;
  m_aGestureSlots[iGestureSlot].m_pAnimLayer->m_bLooping =
      false; //( ( GetSequenceFlags( GetModelPtr(), iGestureSequence ) &
             //STUDIO_LOOPING ) != 0);
  if (bAutoKill) {
    m_aGestureSlots[iGestureSlot].m_pAnimLayer->m_fFlags |= ANIM_LAYER_AUTOKILL;
  } else {
    m_aGestureSlots[iGestureSlot].m_pAnimLayer->m_fFlags &=
        ~ANIM_LAYER_AUTOKILL;
  }
...
// Purpose:
//-----------------------------------------------------------------------------
void CMultiPlayerAnimState::AddVCDSequenceToGestureSlot(int iGestureSlot,
                                                        int iGestureSequence,
                                                        float flCycle,
                                                        bool bAutoKill) {
  // Sanity Check
  Assert(iGestureSlot >= 0 && iGestureSlot < GESTURE_SLOT_COUNT);
...
bool CMultiPlayerAnimState::HandleJumping(Activity &idealActivity) {
  if (m_bJumping) {
    ...
    // Don't check if he's on the ground for a sec.. sometimes the client still
    // has the on-ground flag set right when the message comes in.
...
float CMultiPlayerAnimState::GetCurrentMaxGroundSpeed() {
  ...
  GetBasePlayer()->SetPoseParameter(pStudioHdr, m_PoseParameterData.m_iMoveX,
                                    newX);
  GetBasePlayer()->SetPoseParameter(pStudioHdr, m_PoseParameterData.m_iMoveY,
                                    newY);
...
void CMultiPlayerAnimState::ComputeMainSequence() {
  ...
  if ((oldActivity == ACT_MP_CROUCH_IDLE || oldActivity == ACT_MP_STAND_IDLE ||
       oldActivity == ACT_MP_DEPLOYED_IDLE ||
...
void CMultiPlayerAnimState::UpdateInterpolators() {
  ...
  if (gpGlobals->curtime - m_flLastGroundSpeedUpdateTime >=
      flGroundSpeedInterval) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CMultiPlayerAnimState::UpdateGestureLayer(CStudioHdr *pStudioHdr,
                                               GestureSlot_t *pGesture) {
  // Sanity check.
  ...
  flCycle += pPlayer->GetSequenceCycleRate(
             ...
             gpGlobals->frametime * GetGesturePlaybackRate() *
  ...
  if (pGesture->m_iActivity != ACT_INVALID &&
      pGesture->m_pAnimLayer->m_nActivity == ACT_INVALID) {
    ResetGestureSlot(pGesture->m_iGestureSlot);
  }
...
bool CMultiPlayerAnimState::SetupPoseParameters(CStudioHdr *pStudioHdr) {
  ...
  /*
  if ( ( m_PoseParameterData.m_iMoveX < 0 ) || ( m_PoseParameterData.m_iMoveY <
  0 ) ) return false;
  */

  // Look for the aim pitch blender.
  m_PoseParameterData.m_iAimPitch =
      GetBasePlayer()->LookupPoseParameter(pStudioHdr, "body_pitch");
  /*
  ...
  */
  ...
  m_PoseParameterData.m_iAimYaw =
      GetBasePlayer()->LookupPoseParameter(pStudioHdr, "body_yaw");
  /*
  if ( m_PoseParameterData.m_iAimYaw < 0 )
          return false;
  */

  m_PoseParameterData.m_iMoveYaw =
      GetBasePlayer()->LookupPoseParameter(pStudioHdr, "move_yaw");
  m_PoseParameterData.m_iMoveScale =
      GetBasePlayer()->LookupPoseParameter(pStudioHdr, "move_scale");
  /*
  if ( ( m_PoseParameterData.m_iMoveYaw < 0 ) || (
  m_PoseParameterData.m_iMoveScale < 0 ) ) return false;
  */
...
//-----------------------------------------------------------------------------
void CMultiPlayerAnimState::DoMovementTest(CStudioHdr *pStudioHdr, float flX,
                                           float flY) {
  GetBasePlayer()->SetPoseParameter(pStudioHdr, m_PoseParameterData.m_iMoveX,
                                    flX);
  GetBasePlayer()->SetPoseParameter(pStudioHdr, m_PoseParameterData.m_iMoveY,
...
#ifdef STAGING_ONLY
  float flTestSpeed =
      GetBasePlayer()->GetSequenceGroundSpeed(m_nMovementSequence);
  ...
  /*
  GetBasePlayer()->SetPoseParameter( pStudioHdr, m_PoseParameterData.m_iMoveX,
  flX ); GetBasePlayer()->SetPoseParameter( pStudioHdr,
  m_PoseParameterData.m_iMoveY, flY ); float flDuration =
  GetBasePlayer()->SequenceDuration( m_nMovementSequence );

  GetBasePlayer()->SetPoseParameter( pStudioHdr,
  m_PoseParameterData.m_iMoveX, 1.0f ); GetBasePlayer()->SetPoseParameter(
  pStudioHdr, m_PoseParameterData.m_iMoveY, 0.0f ); float flForward =
  GetBasePlayer()->SequenceDuration( m_nMovementSequence );

  GetBasePlayer()->SetPoseParameter( pStudioHdr, m_PoseParameterData.m_iMoveX,
  0.0f ); GetBasePlayer()->SetPoseParameter( pStudioHdr,
  m_PoseParameterData.m_iMoveY, 0.0f ); float flCenter =
  GetBasePlayer()->SequenceDuration( m_nMovementSequence );

  if ( flDuration > flForward * 1.1f || flDuration < flForward * 0.9f )
  {
          Warning( "%s : %s (X %.0f Y %.0f) mismatched duration with forward
  %.1f vs %.1f\n", pStudioHdr->pszName(), GetBasePlayer()->GetSequenceName(
  m_nMovementSequence ), flX, flY, flDuration, flForward );
  }

  if ( flDuration > flCenter * 1.1f || flDuration < flCenter * 0.9f )
  {
          Warning( "%s : %s (X %.0f Y %.0f) mismatched duration with center %.1f
  vs %.1f\n", pStudioHdr->pszName(), GetBasePlayer()->GetSequenceName(
  m_nMovementSequence ), flX, flY, flDuration, flCenter );
  }
  */
}
...
void CMultiPlayerAnimState::GetMovementFlags(CStudioHdr *pStudioHdr) {
  ...
  KeyValues *seqKeyValues =
      GetBasePlayer()->GetSequenceKeyValues(m_nMovementSequence);
...
void CMultiPlayerAnimState::ComputePoseParam_MoveYaw(CStudioHdr *pStudioHdr) {
  ...
  if (bIsMoving) {
    ...
    if (m_LegAnimType == LEGANIM_9WAY) {
      ...
      float flInvScale =
          MAX(fabs(vecCurrentMoveYaw.x), fabs(vecCurrentMoveYaw.y));
      if (flInvScale != 0.0f) {
      ...
      float flMaxSpeed = GetBasePlayer()->GetSequenceGroundSpeed(
          GetBasePlayer()->GetSequence());
      ...
      // find what speed was actually authored
      GetBasePlayer()->SetPoseParameter(pStudioHdr,
                                        m_PoseParameterData.m_iMoveYaw, flYaw);
      GetBasePlayer()->SetPoseParameter(pStudioHdr,
                                        m_PoseParameterData.m_iMoveScale, 1.0f);
      float flMaxSpeed = GetBasePlayer()->GetSequenceGroundSpeed(
          GetBasePlayer()->GetSequence());
      ...
      if (flMaxSpeed > flSpeed) {
        GetBasePlayer()->SetPoseParameter(
            pStudioHdr, m_PoseParameterData.m_iMoveScale, flSpeed / flMaxSpeed);
      }
    }
  } else {
    // Set the 9-way blend movement pose parameters.
...
void CMultiPlayerAnimState::EstimateYaw(void) {
  ...
  if (vecEstVelocity.x == 0.0f && vecEstVelocity.y == 0.0f) {
    ...
    m_PoseParameterData.m_flEstimateYaw =
        (atan2(vecEstVelocity.y, vecEstVelocity.x) * 180.0f / M_PI);
    m_PoseParameterData.m_flEstimateYaw =
        clamp(m_PoseParameterData.m_flEstimateYaw, -180.0f, 180.0f);
  }
}
...
void CMultiPlayerAnimState::ComputePoseParam_AimPitch(CStudioHdr *pStudioHdr) {
  ...
  GetBasePlayer()->SetPoseParameter(pStudioHdr, m_PoseParameterData.m_iAimPitch,
                                    -flAimPitch);
...
void CMultiPlayerAnimState::ComputePoseParam_AimYaw(CStudioHdr *pStudioHdr) {
  ...
  if (bMoving || m_bForceAimYaw) {
    // The feet match the eye direction when moving - the move yaw takes care of
    // the rest.
  ...
  // Else if we are not moving.
    ...
    // TODO: Do something better here!
      ...
      if (fabs(flYawDelta) > 45.0f /*m_AnimConfig.m_flMaxBodyYawDegrees*/) {
        ...
        m_flGoalFeetYaw +=
            (45.0f /*m_AnimConfig.m_flMaxBodyYawDegrees*/ * flSide);
      }
    }
  }
  ...
  GetBasePlayer()->SetPoseParameter(pStudioHdr, m_PoseParameterData.m_iAimYaw,
                                    -flAimYaw);
  m_DebugAnimData.m_flAimYaw = flAimYaw;

  // Turn off a force aim yaw - either we have already updated or we don't need
  // to.
  m_bForceAimYaw = false;
...
//			&flCurrentYaw -
//-----------------------------------------------------------------------------
void CMultiPlayerAnimState::ConvergeYawAngles(float flGoalYaw, float flYawRate,
                                              float flDeltaTime,
                                              float &flCurrentYaw) {
...
// Output : const QAngle&
//-----------------------------------------------------------------------------
const QAngle &CMultiPlayerAnimState::GetRenderAngles() { return m_angRender; }
...
void Anim_StateLog(const char *pMsg, ...) {
  ...
  if (anim_showstatelog.GetInt() > 1) {
    //		static FileHandle_t hFile = filesystem->Open( "AnimState.log",
    //"wt" ); 		filesystem->FPrintf( hFile, "%s", str ); 		filesystem->Flush( hFile
    //);
  }
}
...
void CMultiPlayerAnimState::DebugShowAnimStateForPlayer(bool bIsServer) {
  ...
  Anim_StatePrintf(iLine++, "Time: %.2f, Speed: %.2f, MaxSpeed: %.2f",
                   gpGlobals->curtime, vecVelocity.Length2D(),
  ...
  Anim_StatePrintf(
      ...
      m_angRender[YAW], m_DebugAnimData.m_flAimYaw,
      m_DebugAnimData.m_flAimPitch, m_DebugAnimData.m_vecMoveYaw.x,
      m_DebugAnimData.m_vecMoveYaw.y);
...
void CMultiPlayerAnimState::DebugShowAnimState(int iStartLine) {
  ...
  //	AnimStatePrintf( iLine++, "ent yaw: %.2f, body_yaw: %.2f, body_pitch:
  //%.2f, move_x: %.2f, move_y: %.2f", 		m_angRender[YAW], g_flLastBodyYaw,
  //g_flLastBodyPitch, m_vLastMovePose.x, m_vLastMovePose.y );
    ...
    if (pGesture) {
      if (pGesture->m_bActive) {
        Anim_StatePrintf(iLine++,
                         ...
                         GetSequenceName(pPlayer->GetModelPtr(),
                                         pGesture->m_pAnimLayer->m_nSequence),
                         (pGesture->m_bAutoKill ? "true" : "false"),
                         (float)pGesture->m_pAnimLayer->m_flCycle,
