...
#ifdef CLIENT_DLL
void RecvProxy_SDKGameRules(const RecvProp *pProp, void **pOut, void *pData,
                            int objectID) {
...
RecvPropDataTable("sdk_gamerules_data", 0, 0,
                  &REFERENCE_RECV_TABLE(DT_SDKGameRules),
                  RecvProxy_SDKGameRules) END_RECV_TABLE()
#else
...
SendPropDataTable("sdk_gamerules_data", 0,
                  &REFERENCE_SEND_TABLE(DT_SDKGameRules),
                  SendProxy_SDKGameRules) END_SEND_TABLE()
#endif
...
// etc.
char *sTeamNames[] = {"Unassigned", "Spectator", "Terrorist",
                      "Counter-Terrorist"};
...
void CSDKGameRules::RadiusDamage(const CTakeDamageInfo &info,
  ...
  for (CEntitySphereQuery sphere(vecSrc, flRadius);
    ...
    if (pEntity->m_takedamage != DAMAGE_NO) {
      ...
      if (bIgnoreWorld) {
        ...
        UTIL_TraceLine(vecSrc, vecSpot, MASK_SOLID_BRUSHONLY,
...
void CSDKGameRules::Think() { BaseClass::Think(); }
    ...
    bool CSDKGameRules::ShouldCollide(int collisionGroup0,
                                      int collisionGroup1) {
...
CAmmoDef *GetAmmoDef() {
  ...
  if (!bInitted) {
    ...
    // def.AddAmmoType( BULLET_PLAYER_50AE,		DMG_BULLET, TRACER_LINE,
    // 0, 0, "ammo_50AE_max",		2400, 0, 10, 14 );
    def.AddAmmoType(AMMO_GRENADE, DMG_BLAST, TRACER_LINE, 0, 0, 1 /*max carry*/,
                    1, 0);
...
//-----------------------------------------------------------------------------
int CSDKGameRules::PlayerRelationship(CBaseEntity *pPlayer,
                                      CBaseEntity *pTarget) {
#ifndef CLIENT_DLL
