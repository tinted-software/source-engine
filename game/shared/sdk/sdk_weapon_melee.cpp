...
static const Vector g_meleeMins(-MELEE_HULL_DIM, -MELEE_HULL_DIM,
                                -MELEE_HULL_DIM);
...
// Constructor
//-----------------------------------------------------------------------------
CWeaponSDKMelee::CWeaponSDKMelee() { m_bFiresUnderwater = true; }
...
void CWeaponSDKMelee::ItemPostFrame(void) {
  ...
  if ((pPlayer->m_nButtons & IN_ATTACK) &&
      (m_flNextPrimaryAttack <= gpGlobals->curtime) && pPlayer->CanAttack()) {
    PrimaryAttack();
...
// Output  :
//------------------------------------------------------------------------------
void CWeaponSDKMelee::SecondaryAttack() { Swing(true); }
...
Activity CWeaponSDKMelee::ChooseIntersectionPointAndActivity(
...
void CWeaponSDKMelee::Swing(int bIsSecondary) {
  ...
  if (traceHit.fraction == 1.0) {
    float meleeHullRadius =
        1.732f *
        MELEE_HULL_DIM; // hull is +/- 16, so use cuberoot of 2 to determine how
                        // big the hull is from center to the corner point

    // Back off by hull "radius"
    swingEnd -= forward * meleeHullRadius;

    UTIL_TraceHull(swingStart, swingEnd, g_meleeMins, g_meleeMaxs,
    ...
    if (traceHit.fraction < 1.0 && traceHit.m_pEnt) {
      ...
      if (dot < 0.70721f) {
        ...
        nHitActivity = ChooseIntersectionPointAndActivity(traceHit, g_meleeMins,
                                                          g_meleeMaxs, pOwner);
      }
    }
  }
