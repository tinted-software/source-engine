...
void FX_WeaponSound(int iPlayerIndex, WeaponSound_t sound_type,
                    const Vector &vOrigin, CSDKWeaponInfo *pWeaponInfo) {
...
void FX_FireBullets(int iPlayerIndex, const Vector &vOrigin,
...
#ifdef CLIENT_DLL
  C_SDKPlayer *pPlayer =
      ToSDKPlayer(ClientEntityList().GetBaseEntity(iPlayerIndex));
#else
  ...
  CSDKWeaponInfo *pWeaponInfo =
      static_cast<CSDKWeaponInfo *>(GetFileWeaponInfoFromHandle(hWpnInfo));

#ifdef CLIENT_DLL
  // Do the firing animation event.
  if (pPlayer && !pPlayer->IsDormant()) {
    if (iMode == Primary_Mode)
      pPlayer->m_PlayerAnimState->DoAnimationEvent(
  ...
  for (int iBullet = 0; iBullet < pWeaponInfo->m_iBullets; iBullet++) {
    ...
    pPlayer->FireBullet(vOrigin, vAngles, flSpread, iDamage, iAmmoType, pPlayer,
                        bDoEffects, x, y);
  }
