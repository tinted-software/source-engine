...
public:
  ...
  void InitSDK(CBaseAnimatingOverlay *pPlayer,
...
void CSDKPlayerAnimState::InitSDK(CBaseAnimatingOverlay *pEntity,
                                  ISDKPlayerAnimStateHelpers *pHelpers,
                                  LegAnimType_t legAnimType,
                                  bool bUseAimSequences) {
...
#ifdef CLIENT_DLL
void CSDKPlayerAnimState::UpdateLayerSequenceGeneric(CStudioHdr *pStudioHdr,
...
bool CSDKPlayerAnimState::IsOuterGrenadePrimed() {
  ...
  if (pChar) {
    CBaseSDKGrenade *pGren =
        dynamic_cast<CBaseSDKGrenade *>(pChar->GetActiveWeapon());
...
int CSDKPlayerAnimState::CalcAimLayerSequence(float *flCycle,
...
int CSDKPlayerAnimState::CalcFireLayerSequence(PlayerAnimEvent_t event) {
  ...
  // Don't rely on their weapon here because the player has usually switched to
  // their pistol or rifle by the time the PLAYERANIMEVENT_THROW_GRENADE message
  // gets to the client.
