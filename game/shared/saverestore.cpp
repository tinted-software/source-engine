...
// #define EXTRACT_INPUTFUNC_FUNCTIONPTR(x)		(*(inputfunc_t
// **)(&(x)))
...
void CSave::Log(const char *pName, fieldtype_t fieldType, void *value,
  ...
  for (int iCount = 0; iCount < count; ++iCount) {
    ...
    case FIELD_COLOR32: {
      ...
      Q_snprintf(szTempBuf, sizeof(szTempBuf), "(%d %d %d %d)", (int)pColor[0],
                 (int)pColor[1], (int)pColor[2], (int)pColor[3]);
      Q_strncat(szBuf, szTempBuf, sizeof(szTempBuf), COPY_ALL_CHARACTERS);
    }
    case FIELD_EMBEDDED:
    case FIELD_CUSTOM:
  ...
  case FIELD_EMBEDDED: {
    ...
    while (--nFieldCount >= 0) {
      ...
      for (; pTestField < pLimit; ++pTestField) {
        if (ShouldSaveField(pTestData +
                                pTestField->fieldOffset[TD_OFFSET_NORMAL],
                            pTestField))
          return true;
      }
...
bool CSave::WriteBasicField(const char *pname, void *pData, datamap_t *pRootMap,
  ...
  case FIELD_EMBEDDED: {
    AssertMsg(((pField->flags & FTYPEDESC_PTR) == 0) ||
                  (pField->fieldSize == 1),
              "Arrays of embedded pointer types presently unsupported by "
              "save/restore");
  ...
  case FIELD_CUSTOM: {
    ...
    SaveRestoreFieldInfo_t fieldInfo = {
        pData, ((char *)pData) - pField->fieldOffset[TD_OFFSET_NORMAL], pField};
...
void CSave::WriteFunction(datamap_t *pRootMap, const char *pname,
                          inputfunc_t **data, int count) {
  AssertMsg(count == 1, "Arrays of functions not presently supported");
  const char *functionName =
      UTIL_FunctionToName(pRootMap, *(inputfunc_t *)data);
  if (!functionName) {
...
bool CSave::WriteGameField(const char *pname, void *pData, datamap_t *pRootMap,
  ...
  // For now, just write the address out, we're not going to change memory while
  // doing this yet!
  case FIELD_FUNCTION:
    WriteFunction(pRootMap, pField->fieldName, (inputfunc_t **)(char *)pData,
                  pField->fieldSize);
    break;
  ...
  case FIELD_POINTER:
    WriteData(pField->fieldName, sizeof(void *) * pField->fieldSize,
              (char *)pData);
    break;
...
void CRestore::ReadBasicField(const SaveRestoreRecordHeader_t &header,
  ...
  case FIELD_EMBEDDED: {
    ...
    if (!(pField->flags & FTYPEDESC_PTR) || *((void **)pDest)) {
      ...
      Warning("Attempted to restore FIELD_EMBEDDEDBYREF %s but there is no "
              "destination memory\n",
              pField->fieldName);
    }
    break;
  ...
  case FIELD_CUSTOM: {
    // No corresponding "block" (see write) as it was used as the header of the
    // field
    int posNextField = GetReadPos() + header.size;

    SaveRestoreFieldInfo_t fieldInfo = {
        pDest, ((char *)pDest) - pField->fieldOffset[TD_OFFSET_NORMAL], pField};
...
void CRestore::EmptyFields(void *pBaseData, typedescription_t *pFields,
  ...
  for (i = 0; i < fieldCount; i++) {
    ...
    case FIELD_EMBEDDED: {
      ...
      int nFieldCount = pField->fieldSize;
      char *pFieldMemory =
          (char *)((!(pField->flags & FTYPEDESC_PTR)) ? pFieldData
                                                      : *((void **)pFieldData));
      while (--nFieldCount >= 0) {
...
void CRestore::ReadGameField(const SaveRestoreRecordHeader_t &header,
  ...
  case FIELD_FUNCTION: {
    ReadFunction(pRootMap, (inputfunc_t **)pDest, pField->fieldSize,
                 header.size);
    break;
  }
...
int CRestore::ReadFunction(datamap_t *pMap, inputfunc_t **pValue, int count,
                           int nBytesAvailable) {
  AssertMsg(nBytesAvailable > 0,
...
// entities of different classes with the same global name
CBaseEntity *
CEntitySaveRestoreBlockHandler::FindGlobalEntity(string_t classname,
                                                 string_t globalname) {
  ...
  if (pReturn) {
    if (!FClassnameIs(pReturn, STRING(classname))) {
      Warning("Global entity found %s, wrong class %s [expects class %s]\n",
              STRING(globalname), STRING(pReturn->m_iClassname),
              STRING(classname));
      pReturn = NULL;
    }
  }
  ...
  if (pEntity->m_iGlobalname != NULL_STRING) {
    ...
    if (globalIndex >= 0) {
      ...
      if (GlobalEntity_GetState(globalIndex) == GLOBAL_DEAD)
        ...
        pEntity->MakeDormant(); // Hasn't been moved to this level yet, wait but
                                // stay alive
      }
