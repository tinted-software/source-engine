...
#ifdef CLIENT_DLL
ConVar cl_showhelp("cl_showhelp", "1", FCVAR_USERINFO | FCVAR_ARCHIVE,
...
// Purpose:
//-----------------------------------------------------------------------------
void CHintSystem::Init(CBasePlayer *pPlayer, int iMaxHintTypes,
...
//			bForce - always play this hint even if they have seen it
//before
//-----------------------------------------------------------------------------
bool CHintSystem::HintMessage(int hint, bool bForce /* = false */,
...
void CHintSystem::ResetHints(void) {
  ...
  for (int i = 0; i < m_RegisteredResetHints.Count(); i++) {
    ReAddHintTimerIfNotDisplayed(m_RegisteredResetHints[i].iHintID,
                                 m_RegisteredResetHints[i].flTimer);
  }
...
//-----------------------------------------------------------------------------
void CHintSystem::RegisterHintTimer(int iHintID, float flTimerDuration,
                                    bool bOnlyIfClear /* = false */,
...
//-----------------------------------------------------------------------------
void CHintSystem::ReAddHintTimerIfNotDisplayed(int iHintID,
                                               float flTimerDuration) {
  Assert(iHintID < m_HintHistory.GetNumBits());
...
#ifdef CLIENT_DLL
ConCommand cl_clearhinthistory(
    "cl_clearhinthistory", HintClear,
    "Clear memory of client side hints displayed to the player.");
#else
ConCommand sv_clearhinthistory(
    "sv_clearhinthistory", HintClear,
