...
public:
  void ApplyNewSpringLength() {
    ResetSpringLength((m_flRopeLength + m_flCurSlack + ROPESLACK_FUDGEFACTOR) /
                      (NumNodes() - 1));
  }
...
void CalcRopeStartingConditions(const Vector &vStartPos, const Vector &vEndPos,
  ...
  // Initialize the rope as a straight line with no slack as our first
  // approximation. We then relax the rope by adding slack until it hangs to the
  // desired height.
  //
  ...
  rope.m_flStartZ =
      MIN(vStartPos.z, vEndPos.z); // Calculate hang as the Z distance from the
