...
void CHalfLife2Survival::ReadSurvivalScriptFile(void) {
  ...
  if (pkvFile->LoadFromFile(filesystem, szFullName, "MOD")) {
    ...
    // in the script, so that we don't remove them if they violate survival
    // rules. i.e. we want the player to start with a shotgun, but prevent all
    // future shotguns from spawning.
    m_bActive = true;
  } else {
    m_bActive = false;
  }
