...
static ConVar sv_autoladderdismount("sv_autoladderdismount", "1",
                                    FCVAR_REPLICATED,
                                    "Automatically dismount from ladders when "
                                    "you reach the end (don't have to +USE).");
static ConVar sv_ladderautomountdot(
    "sv_ladderautomountdot", "0.4", FCVAR_REPLICATED,
    "When auto-mounting a ladder by looking up its axis, this is the tolerance "
    "for looking now directly along the ladder axis.");

static ConVar
    sv_ladder_useonly("sv_ladder_useonly", "0", FCVAR_REPLICATED,
...
// This is a simple helper class to reserver a player sized hull at a spot,
// owned by the current player so that nothing
...
public:
  static CReservePlayerSpot *ReserveSpot(CBasePlayer *owner, const Vector &org,
...
CReservePlayerSpot *CReservePlayerSpot::ReserveSpot(CBasePlayer *owner,
                                                    const Vector &org,
                                                    const Vector &mins,
                                                    const Vector &maxs,
                                                    bool &validspot) {
  CReservePlayerSpot *spot =
      (CReservePlayerSpot *)CreateEntityByName("reserved_spot");
  Assert(spot);
  ...
  trace_t tr;
  UTIL_TraceHull(org, org, mins, maxs, MASK_PLAYERSOLID, owner,
  ...
  if (!validspot) {
    ...
    // See if spot is valid
    trace_t tr;
    UTIL_TraceHull(org2, org2, mins, maxs, MASK_PLAYERSOLID, owner,
                   COLLISION_GROUP_PLAYER_MOVEMENT, &tr);
    validspot = !tr.startsolid;
  }
...
//			*ladder -
//-----------------------------------------------------------------------------
void CHL2GameMovement::StartForcedMove(bool mounting, float transit_speed,
  ...
  // Don't get stuck during this traversal since we'll just be slamming the
  // player origin
...
bool CHL2GameMovement::ContinueForcedMove() {
  ...
  float frac = (gpGlobals->curtime - lm->m_flStartTime) /
               (lm->m_flArrivalTime - lm->m_flStartTime);
  if (frac > 1.0f) {
  ...
  // If finished moving, reset player to correct movetype (or put them on the
  // ladder)
  if (!lm->m_bForceLadderMove) {
...
//			ladderOrigin -
//-----------------------------------------------------------------------------
void CHL2GameMovement::Findladder(float maxdist, CFuncLadder **ppLadder,
                                  Vector &ladderOrigin,
  ...
  for (int i = 0; i < c; i++) {
    ...
    Vector closest;
    CalcClosestPointOnLineSegment(mv->GetAbsOrigin(), bottomPosition,
                                  topPosition, closest, NULL);
    ...
    UTIL_TraceLine(mv->GetAbsOrigin(), closest, MASK_PLAYERSOLID, player,
                   COLLISION_GROUP_NONE, &tr);

    if (tr.fraction != 1.0f && tr.m_pEnt && tr.m_pEnt != ladder) {
      // Try a trace stepped up from the ground a bit, in case there's something
      // at ground level blocking us.
      float sizez = GetPlayerMaxs().z - GetPlayerMins().z;

      UTIL_TraceLine(mv->GetAbsOrigin() + Vector(0, 0, sizez * 0.5f), closest,
                     MASK_PLAYERSOLID, player, COLLISION_GROUP_NONE, &tr);
...
static bool NearbyDismountLessFunc(const NearbyDismount_t &lhs,
                                   const NearbyDismount_t &rhs) {
  return lhs.distSqr < rhs.distSqr;
}

void CHL2GameMovement::GetSortedDismountNodeList(
    const Vector &org, float radius, CFuncLadder *ladder,
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CHL2GameMovement::ExitLadderViaDismountNode(CFuncLadder *ladder,
                                                 bool strict,
                                                 bool useAlternate) {
  ...
  CUtlRBTree<NearbyDismount_t, int> nearbyDismounts(0, 0,
                                                    NearbyDismountLessFunc);

  GetSortedDismountNodeList(mv->GetAbsOrigin(), 100.0f, ladder,
                            nearbyDismounts);

  int i;

  for (i = nearbyDismounts.FirstInorder(); i != nearbyDismounts.InvalidIndex();
       i = nearbyDismounts.NextInorder(i)) {
    ...
    UTIL_TraceHull(
        ...
        MASK_PLAYERSOLID, player, COLLISION_GROUP_PLAYER_MOVEMENT, &tr);
  ...
  if (useAlternate) {
    // Desperate. Don't refuse to let a person off of a ladder if it can be
    // helped. Use the alternate dismount if there is one.
...
void CHL2GameMovement::FullLadderMove() {
  ...
  // Pressed buttons are "changed(xor)" and'ed with the mask of currently held
  // buttons
  int buttonsChanged =
      (mv->m_nOldButtons ^
  ...
  bool pressing_forward_or_side =
      mv->m_flForwardMove != 0.0f || mv->m_flSideMove != 0.0f;
  ...
  bool neardismountnode2 =
      (dist < ladderUnitsPerTick * ladderUnitsPerTick) ? true : false;

  // Really close to node, cvar is set, and pressing a key, then simulate a +USE
  bool auto_dismount_use =
      (neardismountnode2 && sv_autoladderdismount.GetBool() &&
       pressing_forward_or_side && !moving_along_ladder);

  bool fully_underwater = (player->GetWaterLevel() == WL_Eyes) ? true : false;

  // If the user manually pressed use or we're simulating it, then use_dismount
  // will occur
  ...
  // If the movement would leave the ladder and they're not automated or
  // pressing use, disallow the movement
  if (!use_dismount) {
    if (wouldleaveladder) {
  ...
  // If the move would not leave the ladder and we're near close to the end,
  // then just accept the move
...
bool CHL2GameMovement::CheckLadderAutoMountEndPoint(CFuncLadder *ladder,
...
bool CHL2GameMovement::CheckLadderAutoMountCone(CFuncLadder *ladder,
                                                const Vector &bestOrigin,
                                                float maxAngleDelta,
  ...
  if (ladder != NULL && (mv->m_flForwardMove > 0.0f)) {
    ...
    bool closetoladder =
        (dist != 0.0f && dist < maxDistToLadder) ? true : false;
    bool reallyclosetoladder = (dist != 0.0f && dist < 4.0f) ? true : false;

    bool facingladderaxis = (angle < maxAngleDelta) ? true : false;
    bool facingalongaxis = ((float)fabs(ladderAxis.Dot(m_vecForward)) >
                            sv_ladderautomountdot.GetFloat())
                               ? true
                               : false;
#if 0
    ...
    if (((facingDot > 0.0f && !strafing) || facingalongaxis) &&
        (facingladderaxis || reallyclosetoladder) && closetoladder) {
      StartForcedMove(true, player->MaxSpeed(), bestOrigin, ladder);
      return true;
    }
  }
...
bool CHL2GameMovement::LookingAtLadder(CFuncLadder *ladder) {
  ...
  // Because the ladder itself is not a solid, the player's origin may actually
  // be permitted to pass it, and that will screw up our dot product. So back up
  // the player's origin a bit to do the facing calculation.
...
// Input  : &trace -
//-----------------------------------------------------------------------------
bool CHL2GameMovement::CheckLadderAutoMount(CFuncLadder *ladder,
                                            const Vector &bestOrigin) {
#if !defined(CLIENT_DLL)
...
bool CHL2GameMovement::LadderMove(void) {
  ...
  if (!ladder && !pressed_use) {
    // If flying through air, allow mounting ladders if we are facing < 15
    // degress from the ladder and we are close
    if (!ladder && !sv_ladder_useonly.GetBool()) {
      // Tracker 6625:  Don't need to be leaping to auto mount using this
      // method... But if we are on the ground, then we must not be backing into
      // the ladder (Tracker 12961)
      ...
      // Pressing forward while looking at ladder and standing (or floating)
      // near a mounting point
  ...
  if (!ladder && LookingAtLadder(bestLadder) &&
  ...
  if (forwardSpeed != 0 || rightSpeed != 0) {
    ...
    // Check to see if we've mounted the ladder in a bogus spot and, if so, just
    // fall off the ladder...
    float dummyt = 0.0f;
    float distFromLadderSqr = CalcDistanceSqrToLine(
        mv->GetAbsOrigin(), topPosition, bottomPosition, &dummyt);
    ...
    bool ishorizontal =
        fabs(topPosition.z - bottomPosition.z) < 64.0f ? true : false;
    ...
    if (sv_ladders_useonly.GetBool()) {
      // Stick up climbs up, stick down climbs down. No matter which way you're
      // looking.
      if (mv->m_nButtons & IN_FORWARD) {
        factor = 1.0f;
...
void CHL2GameMovement::SetGroundEntity(trace_t *pm) {
  ...
  if (newGround &&
      newGround->GetCollisionGroup() == HL2COLLISION_GROUP_COMBINE_BALL_NPC) {
    return;
  }
...
#ifndef PORTAL // Portal inherits from this but needs to declare it's own global
               // interface
// Expose our interface.
static CHL2GameMovement g_GameMovement;
IGameMovement *g_pGameMovement = (IGameMovement *)&g_GameMovement;

EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CGameMovement, IGameMovement,
                                  INTERFACENAME_GAMEMOVEMENT, g_GameMovement);
#endif