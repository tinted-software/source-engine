...
DEFINE_FIELD(m_vecStartPosition, FIELD_POSITION_VECTOR),
    ...
    END_DATADESC()
        ...
        BEGIN_NETWORK_TABLE_NOBASE(CEnvHeadcrabCanisterShared,
...
void CEnvHeadcrabCanisterShared::InitInWorld(float flLaunchTime,
  ...
  if (!bLaunchedFromWithinWorld) {
    ...
    VectorMA(vecImpactPosition, m_flFlightTime * m_flFlightSpeed, vecDelta,
             vecActualStartPosition);
  }
  ...
  if (m_bLaunchedFromWithinWorld) {
    ...
    VectorSubtract(vecImpactPosition, vecStartPosition,
                   m_vecParabolaDirection.GetForModify());
    m_vecParabolaDirection.GetForModify().z = 0;
    float flTotalDistance =
        VectorNormalize(m_vecParabolaDirection.GetForModify());
    ...
    m_flFlightSpeed =
        sqrt(m_flHorizSpeed * m_flHorizSpeed + flFinalZSpeed * flFinalZSpeed);
    m_flInitialZSpeed = (2.0f * (vecImpactPosition.z - vecStartPosition.z) -
                         flFinalZSpeed * m_flFlightTime) /
                        m_flFlightTime;
...
void CEnvHeadcrabCanisterShared::InitInSkybox(float flLaunchTime,
  ...
  flTotalDistance -=
      vecStartPosition.AsVector2D().DistTo(vecImpactPosition.AsVector2D());
  if (flTotalDistance <= 0.0f) {
    InitInWorld(flLaunchTime, vecStartPosition, vecStartAngles, vecDirection,
                vecImpactPosition);
    return;
  }
  ...
  VectorMA(vecSkyboxOrigin, 1.0f / m_flSkyboxScale, vecStartPosition,
           m_vecStartPosition.GetForModify());
  VectorMA(m_vecStartPosition.Get(), -m_flHorizSpeed * m_flWorldEnterTime,
           m_vecParabolaDirection, m_vecStartPosition.GetForModify());

  float flLaunchHeight = m_flLaunchHeight / flSkyboxScale;
  float flFinalZSpeed =
      m_vecDirection.Get().z * m_flFlightSpeed / flSkyboxScale;
  m_vecStartPosition.GetForModify().z += flLaunchHeight;
  m_flZAcceleration = 2.0f *
                      (flLaunchHeight + flFinalZSpeed * m_flWorldEnterTime) /
                      (m_flWorldEnterTime * m_flWorldEnterTime);
...
//-----------------------------------------------------------------------------
void CEnvHeadcrabCanisterShared::GetPositionAtTime(float flTime,
                                                   Vector &vecPosition,
                                                   QAngle &vecAngles) {
  ...
  if (m_bLaunchedFromWithinWorld || m_bInSkybox) {
    VectorMA(m_vecStartPosition, flDeltaTime * m_flHorizSpeed,
             m_vecParabolaDirection, vecPosition);
    vecPosition.z += m_flInitialZSpeed * flDeltaTime +
                     0.5f * m_flZAcceleration * flDeltaTime * flDeltaTime;
    ...
    vecForward.z = -(m_flInitialZSpeed + m_flZAcceleration * flDeltaTime) /
                   m_flHorizSpeed; // This is -dz/dx.
  ...
  VMatrix rotation;
  MatrixBuildRotationAboutAxis(rotation, Vector(1, 0, 0),
                               flDeltaTime * ROTATION_SPEED);
...
#define HEADCRABCANISTER_TRIGGER_EPSILON	0.001f
  ...
  for( int iAxis = 0; iAxis < 3; iAxis++ )
  {
          ...
          {
                  flFrac = ( flDistStart - HEADCRABCANISTER_TRIGGER_EPSILON ) /
( flDistStart - flDistEnd ); if ( flFrac > flEnterFrac ) { flEnterFrac = flFrac;
}
          }
          ...
          {
                  flFrac = ( flDistStart + HEADCRABCANISTER_TRIGGER_EPSILON ) /
( flDistStart - flDistEnd ); if( flFrac < flExitFrac ) { flExitFrac = flFrac; }
          }
...
); VectorScale( vecDeltaPosition, flEnterFrac, vecEnterPoint ); VectorScale(
vecDeltaPosition, flExitFrac, vecExitPoint );
