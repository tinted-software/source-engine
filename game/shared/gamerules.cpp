...
#ifndef CLIENT_DLL
ConVar
    log_verbose_enable("log_verbose_enable", "0", FCVAR_GAMEDLL,
...
bool CGameRules::CanHavePlayerItem(CBasePlayer *pPlayer,
  ...
  /*
     ...
     this type if ( pPlayer->Weapon_OwnsThisType( pWeapon ) )
          ...
          {
                  // weapon doesn't use ammo, don't take another if you already
     have it. if ( pPlayer->Weapon_OwnsThisType( pWeapon ) )
                  {
                          return FALSE;
                  }
          }
...
void CGameRules::RadiusDamage(const CTakeDamageInfo &info,
  ...
  for (CEntitySphereQuery sphere(vecSrc, flRadius);
    ...
    // This value is used to scale damage when the explosion is blocked by some
    // other object.
    ...
    if (old_radius_damage.GetBool()) {
      ...
      if (tr.fraction != 1.0) {
        if (IsExplosionTraceBlocked(&tr)) {
          if (ShouldUseRobustRadiusDamage(pEntity)) {
            if (vecSpot.DistToSqr(vecSrc) > flHalfRadiusSqr) {
              // Only use robust model on a target within one-half of the
              // explosion's radius.
              continue;
            }
            ...
            UTIL_TraceLine(tr.endpos,
                           ...
                           COLLISION_GROUP_NONE, &tr);
            // NDebugOverlay::Line( tr.startpos, tr.endpos, 255, 255, 0, false,
            // 10 );

            // ...to see if there's a nearby edge that the explosion would
            // 'spill over' if the blast were fully simulated.
        ...
        // UNDONE: Probably shouldn't let children block parents either?  Or
        // maybe those guys should set their owner if they want this behavior?
        ...
        if (tr.m_pEnt && tr.m_pEnt != pEntity &&
          ...
          // Some entity was hit by the trace, meaning the explosion does not
          // have clear line of sight to the entity that it's trying to hurt. If
          // the world is also blocking, we do no damage.
          CBaseEntity *pBlockingEntity = tr.m_pEnt;
          // Msg( "%s may be blocked by %s...", pEntity->GetClassname(),
          // pBlockingEntity->GetClassname() );

          UTIL_TraceLine(vecSrc, vecSpot, CONTENTS_SOLID, info.GetInflictor(),
                         COLLISION_GROUP_NONE, &tr);
          ...
          // Now, if the interposing object is physics, block some explosion
          // force based on its mass.
          if (pBlockingEntity->VPhysicsGetObject()) {
            ...
            // Msg("  Object (%s) weighing %fkg blocked %f percent of explosion
            // damage\n", pBlockingEntity->GetClassname(), flMass, scale *
            // 100.0f);
          } else {
            // Some object that's not the world and not physics. Generically
            // block 25% damage
    ...
    // pEntity->GetClassname(), flBlockedDamagePercent * 100, flAdjustedDamage,
    // flAdjustedDamage - (flAdjustedDamage * flBlockedDamagePercent) );
    ...
    if (info.GetAttacker() && info.GetAttacker()->IsPlayer() &&
        pEntity->IsNPC()) {
      // An explosion set off by the player is harming an NPC. Adjust damage
      // accordingly.
    ...
    if (adjustedInfo.GetDamagePosition() == vec3_origin ||
      ...
      // Assume the force passed in is the maximum force. Decay it based on
      // falloff.
    ...
    if (info.GetAttacker() && info.GetAttacker()->IsPlayer() &&
      ...
      gamestats->Event_WeaponHit(player, bIsPrimary,
                                 (pWeapon != NULL)
                                     ? player->GetActiveWeapon()->GetClassname()
                                     : "NULL",
                                 info);
    }
#endif
...
//achievement to mark (see TODO:Kerry, what file will have the mod's achievement
//enum?)
//-----------------------------------------------------------------------------
...
CBaseCombatWeapon *
CGameRules::GetNextBestWeapon(CBaseCombatCharacter *pPlayer,
                              CBaseCombatWeapon *pCurrentWeapon) {
  return NULL;
}
