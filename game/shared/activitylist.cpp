...
// (activityIndex can be reused by private activities), so a custom table is
// necessary
...
// This stores the actual activity names.  Also, the string ID in the registry
// is simply an index into the g_ActivityList array.
...
void ActivityList_Init(void) { g_HighestActivity = 0; }
...
activitylist_t *ActivityList_AddActivityEntry(const char *pName,
                                              int iActivityIndex,
                                              bool isPrivate) {
  MEM_ALLOC_CREDIT();
...
bool ActivityList_RegisterSharedActivity(const char *pszActivityName,
                                         int iActivityIndex) {
  ...
  // technically order isn't dependent, but it's too damn easy to forget to add
  // new ACT_'s to all three lists.
  static int lastActivityIndex = -1;
  Assert(iActivityIndex < LAST_SHARED_ACTIVITY &&
         (iActivityIndex == lastActivityIndex + 1 || iActivityIndex == 0));
...
Activity ActivityList_RegisterPrivateActivity(const char *pszActivityName) {
  ...
  pList = ActivityList_AddActivityEntry(pszActivityName, g_HighestActivity + 1,
                                        true);
  return (Activity)pList->activityIndex;
}
...
void ActivityList_RegisterSharedActivities(void) {
  ...
  // Viewmodel activities may belong elsewhere, but since where is unclear right
  // now, they'll be placed here.
  ...
  AssertMsg(
      g_HighestActivity == LAST_SHARED_ACTIVITY - 1,
...
public:
  ...
  virtual void Save(const SaveRestoreFieldInfo_t &fieldInfo, ISave *pSave) {
    ...
    if (!pActivityName) {
      AssertOnce(activityIndex == -1); // FIXME: whatever activity this was,
                                       // it's now being saved out as ACT_RESET
...
ISaveRestoreOps *ActivityDataOps() { return &g_ActivityDataOps; }
...
void UTIL_LoadActivityRemapFile(const char *filename, const char *section,
  ...
  if (pkvFile->LoadFromFile(filesystem, filename, NULL)) {
    ...
    while (pTestKey) {
      Activity ActBase =
          (Activity)ActivityList_IndexForName(pTestKey->GetName());
      ...
      if (ActBase != ACT_INVALID) {
        ...
        while (pRemapKey) {
          ...
          if (!stricmp(pKeyName, "remapactivity")) {
            ...
            if (Act == ACT_INVALID) {
              actMap.mappedActivity =
                  ActivityList_RegisterPrivateActivity(pKeyValue);
            } else {
...
int ActivityList_HighestIndex() { return g_HighestActivity; }
