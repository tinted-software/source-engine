  ...
  virtual void Save(const SaveRestoreFieldInfo_t &fieldInfo, ISave *pSave) {
    ...
    if (pArrayTypeDatamap == NULL) {
      if (nElems) {
        ...
        pSave->WriteFields("elems", &((*pUtlVector)[0]), &dataMap, &dataDesc,
                           1);
      }
    } else {
  ...
  virtual void Restore(const SaveRestoreFieldInfo_t &fieldInfo,
    ...
    if (pArrayTypeDatamap == NULL) {
      if (nElems) {
        ...
        dataDesc.fieldSizeInBytes =
            nElems * CDatamapFieldSizeDeducer<FIELD_TYPE>::FieldSize();
        pRestore->ReadFields("elems", &((*pUtlVector)[0]), &dataMap, &dataDesc,
                             1);
      }
    } else {
      // @Note (toml 11-21-02): Save load does not support arrays of user
...
template <int FIELD_TYPE> class CUtlVectorDataopsInstantiator {
public:
  template <class UTLVECTOR> static ISaveRestoreOps *GetDataOps(UTLVECTOR *) {
...
#define DEFINE_GLOBAL_UTLVECTOR(name, fieldtype)                               \
  {FIELD_CUSTOM,                                                               \
   ...
   {offsetof(classNameTypedef, name), 0},                                      \
   1,                                                                          \
   FTYPEDESC_SAVE | FTYPEDESC_GLOBAL,                                          \
   NULL,                                                                       \
   CUtlVectorDataopsInstantiator<fieldtype>::GetDataOps(                       \
       &(((classNameTypedef *)0)->name)),                                      \
   NULL}
