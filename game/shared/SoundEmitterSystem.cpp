...
static ConVar
    sv_soundemitter_trace("sv_soundemitter_trace", "0", FCVAR_REPLICATED,
                          "Show all EmitSound calls including their symbolic "
                          "name and the actual wave file they resolved to\n");
#ifdef STAGING_ONLY
static ConVar sv_snd_filter("sv_snd_filter", "", FCVAR_REPLICATED,
                            "Filters out all sounds not containing the "
                            "specified string before being emitted\n");
#endif // STAGING_ONLY
...
void WaveTrace(char const *wavname, char const *funcname) {
  ...
  if (UTL_INVAL_SYMBOL == s_WaveTrace.Find(wavname)) {
    DevMsg("%s directly referenced wave %s (should use game_sounds.txt system "
           "instead)\n",
           funcname, wavname);
...
public:
  ...
  virtual void LevelInitPreEntity() {
    ...
    if (V_stristr(mapname, "mvm")) {
      ...
      if (filesystem->FileExists("scripts/mvm_level_sound_tweaks.txt",
                                 "GAME")) {
        soundemitterbase->AddSoundOverrides(
      ...
      if (filesystem->FileExists("scripts/game_sounds_vo_mvm_mighty.txt",
                                 "GAME")) {
        soundemitterbase->AddSoundOverrides(
            "scripts/game_sounds_vo_mvm_mighty.txt", true);
      }
  ...
  void InternalPrecacheWaves(int soundIndex) {
    ...
    if (!waveCount) {
      DevMsg("CSoundEmitterSystem:  sounds.txt entry '%s' has no waves listed "
...
public:
  void EmitSoundByHandle(IRecipientFilter &filter, int entindex,
...
#ifdef STAGING_ONLY
    if (sv_snd_filter.GetString()[0] &&
        !V_stristr(params.soundname, sv_snd_filter.GetString())) {
      return;
    }
#endif // STAGING_ONLY
    ...
    if (!Q_strncasecmp(params.soundname, "vo", 2) &&
      ...
      DevMsg("EmitSound:  Voice wave file %s doesn't specify CHAN_VOICE, "
             "CHAN_VOICE2 or CHAN_STREAM for sound %s\n",
    ...
    enginesound->EmitSound(filter, entindex, params.channel, params.soundname,
                           ...
                           ep.m_nFlags, params.pitch, ep.m_nSpecialDSP,
                           ep.m_pOrigin, NULL, &ep.m_UtlVecSoundOrigin, true,
  ...
  void EmitSound(IRecipientFilter &filter, int entindex,
...
#ifdef STAGING_ONLY
    if (sv_snd_filter.GetString()[0] &&
        !V_stristr(ep.m_pSoundName, sv_snd_filter.GetString())) {
      return;
    }
#endif // STAGING_ONLY
    ...
    if (ep.m_pSoundName &&
      ...
      enginesound->EmitSound(filter, entindex, ep.m_nChannel, ep.m_pSoundName,
                             ep.m_flVolume, ep.m_SoundLevel, ep.m_nFlags,
                             ep.m_nPitch, ep.m_nSpecialDSP, ep.m_pOrigin, NULL,
                             &ep.m_UtlVecSoundOrigin, true, ep.m_flSoundTime,
                             ep.m_nSpeakerEntity);
      if (ep.m_pflSoundDuration) {
...
#ifdef STAGING_ONLY
    if (sv_snd_filter.GetString()[0] &&
        !V_stristr(pSample, sv_snd_filter.GetString())) {
      return;
    }
#endif // STAGING_ONLY
...
#else
CON_COMMAND_F(sv_soundemitter_flush,
...
CON_COMMAND_F(sv_soundemitter_filecheck,
...
CON_COMMAND_F(sv_findsoundname,
...
#ifdef STAGING_ONLY
  if (sv_snd_filter.GetString()[0] &&
      !V_stristr(samp, sv_snd_filter.GetString())) {
    return;
  }
#endif // STAGING_ONLY
