...
//
// 1) Use a random number generator that can return the ith random number,
// namely:
//
...
//		have already been generated (namely, compute the i in part 1
//above).
//
...
void CEnvWindShared::Init(int nEntIndex, int iRandomSeed, float flTime,
  ...
  CSoundEnvelopeController::GetController(); m_pWindSound =
  controller.SoundCreate( -1, CHAN_STATIC, "EnvWind.Loop", ATTN_NONE );
          controller.Play( m_pWindSound, 0.0f, 100 );
  }
  */

  // Next time a change happens (which will happen immediately), it'll stop
  // gusting
  m_bGusting = true;
}
...
#define WIND_ACCELERATION                                                      \
  150.0f // wind speed can accelerate this many units per second
#define WIND_DECELERATION                                                      \
  15.0f // wind speed can decelerate this many units per second
...
float CEnvWindShared::WindThink(float flTime) {
  ...
  while (true) {
    ...
    if (!bGotToSwitchTime) {
      ...
      // If we reached a steady state, we don't need to be called until the
      // switch time Otherwise, we should be called immediately
    ...
    if (m_bGusting) {
      ...
      m_bGusting = false;
      m_flSwitchTime +=
          m_flMinGustDelay + m_Stream.RandomFloat(0, m_flMaxGustDelay);
      ...
      m_iWindDir = anglemod(
          m_iWindDir + m_Stream.RandomInt(-m_iGustDirChange, m_iGustDirChange));
      ...
      // !!!HACKHACK - gust duration tied to the length of a particular wave
      // file
...
//-----------------------------------------------------------------------------
void ResetWindspeed() { s_vecWindVelocity.Init(0, 0, 0); }
