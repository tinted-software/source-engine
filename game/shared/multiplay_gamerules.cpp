...
#ifdef GAME_DLL
void MPTimeLimitCallback(IConVar *var, const char *pOldString,
                         float flOldValue) {
...
ConVar fraglimit("mp_fraglimit", "0", FCVAR_NOTIFY | FCVAR_REPLICATED,
                 "The number of kills at which the map ends");

ConVar mp_show_voice_icons(
    "mp_show_voice_icons", "1", FCVAR_REPLICATED,
...
ConVar mp_mapcycle_empty_timeout_seconds(
    ...
    "If nonzero, server will cycle to the next map if it has been empty on the "
    "current map for N seconds");
...
ConCommand
    skip_next_map("skip_next_map", cc_SkipNextMapInCycle,
                  "Skips the next map in the map rotation for the server.");
ConCommand changelevel_next(
    "changelevel_next", cc_GotoNextMapInCycle,
...
void CMultiplayRules::FrameUpdatePostEntityThink() {
  ...
  if (mp_mapcycle_empty_timeout_seconds.GetInt() > 0) {
    ...
    if (iIdleSeconds >= mp_mapcycle_empty_timeout_seconds.GetInt()) {
      ...
      Log("Server has been empty for %d seconds on this map, cycling map as "
          "per mp_mapcycle_empty_timeout_seconds\n",
          iIdleSeconds);
      ChangeLevel();
    }
  }
...
//-----------------------------------------------------------------------------
CBaseCombatWeapon *
CMultiplayRules::GetNextBestWeapon(CBaseCombatCharacter *pPlayer,
...
bool CMultiplayRules::PlayFootstepSounds(CBasePlayer *pl) {
  ...
  if (pl->IsOnLadder() || pl->GetAbsVelocity().Length2D() > 220)
    return true; // only make step sounds in multiplayer if the player is moving
                 // fast enough
...
void CMultiplayRules::DetermineMapCycleFilename(char *pszResult,
                                                int nSizeResult,
                                                bool bForceSpew) {
...
void CMultiplayRules::LoapMapCycleFileIntoVector(const char *pszMapCycleFile,
...
void CMultiplayRules::FreeMapCycleFileVector(CUtlVector<char *> &mapList) {
  // Clear out existing map list. Not using Purge() or PurgeAndDeleteAll()
  // because they won't delete [] each element.
  for (int i = 0; i < mapList.Count(); i++) {
    delete[] mapList[i];
  }
...
void CMultiplayRules::LoadMapCycleFile(void) {
  ...
  if (g_pStringTableServerMapCycle) {
    ...
    g_pStringTableServerMapCycle->AddString(
        CBaseEntity::IsServer(), "ServerMapCycle", sFileList.Length() + 1,
        sFileList.String());
  }
  ...
  if (g_pStringTableServerPopFiles) {
    ...
    V_snprintf(szBaseName, sizeof(szBaseName), "scripts/population/%s*.pop",
    ...
    while (pPopFileName && pPopFileName[0] != '\0') {
      ...
      const char *pchPopPostfix =
          StringAfterPrefix(pPopFileName, STRING(gpGlobals->mapname));
      if (pchPopPostfix) {
        char szShortName[_MAX_PATH];
        V_strncpy(szShortName,
                  ((pchPopPostfix[0] == '_') ? (pchPopPostfix + 1) : "normal"),
    ...
    if (sFileList.Length() > 0) {
      g_pStringTableServerPopFiles->AddString(
          CBaseEntity::IsServer(), "ServerPopFiles", sFileList.Length() + 1,
          sFileList.String());
    }
  }
  ...
  if (g_pStringTableServerMapCycleMvM) {
    ...
    if (pKV->LoadFromFile(g_pFullFileSystem,
                          tf_mvm_missioncyclefile.GetString(), "MOD")) {
      ...
      for (int iCat = 1; iCat <= iMaxCat; iCat++) {
        ...
        if (pCategory) {
          ...
          for (int iMap = 1; iMap <= iMapCount; ++iMap) {
            KeyValues *pMission =
                pCategory->FindKey(UTIL_VarArgs("%d", iMap), false);
            if (pMission) {
      ...
      if (mapList.Count()) {
        ...
        g_pStringTableServerMapCycleMvM->AddString(
            CBaseEntity::IsServer(), "ServerMapCycleMvM",
  ...
  // If the current map selection is in the list, set m_nMapCycleindex to the
  // map that follows it.
  for (int i = 0; i < m_MapList.Count(); i++) {
  ...
  if (pKV->LoadFromFile(filesystem, "scripts/voicecommands.txt", "GAME")) {
    for (KeyValues *menu = pKV->GetFirstSubKey(); menu != NULL;
      ...
      for (KeyValues *menuitem = menu->GetFirstSubKey(); menuitem != NULL;
        ...
        if (iNumItems > 9) {
          Warning("Trying to load more than 9 menu items in voicecommands.txt, "
                  "extras ignored");
          continue;
        }
        ...
        if (iConcept == MP_CONCEPT_NONE) {
          Warning("Voicecommand script attempting to use unknown concept. Need "
                  "to define new concepts in code. ( %s )\n",
...
void CMultiplayRules::SkipNextMapInCycle() {
  ...
  if (nextlevel.GetString() && *nextlevel.GetString() &&
      engine->IsMapValid(nextlevel.GetString())) {
    Msg("Warning! \"nextlevel\" is set to \"%s\" and will override the next "
        "map to be played.\n",
        nextlevel.GetString());
  }
}
...
void CMultiplayRules::ClientCommandKeyValues(edict_t *pEntity,
                                             KeyValues *pKeyValues) {
  CBaseMultiplayerPlayer *pPlayer =
      dynamic_cast<CBaseMultiplayerPlayer *>(CBaseEntity::Instance(pEntity));
...
CMultiplayRules::VoiceCommand(CBaseMultiplayerPlayer *pPlayer, int iMenu,
                              int iItem) {
...
void CMultiplayRules::HaveAllPlayersSpeakConceptIfAllowed(
    int iConcept, int iTeam /* = TEAM_UNASSIGNED */,
...
void CMultiplayRules::RandomPlayersSpeakConceptIfAllowed(
    int iConcept, int iNumRandomPlayer /*= 1*/, int iTeam /*= TEAM_UNASSIGNED*/,
    const char *modifiers /*= NULL*/) {
