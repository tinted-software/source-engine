...
static CUtlMemoryPool g_EdictTouchLinks(sizeof(touchlink_t), MAX_EDICTS,
                                        CUtlMemoryPool::GROW_NONE,
                                        "g_EdictTouchLinks");
static CUtlMemoryPool g_EntityGroundLinks(sizeof(groundlink_t), MAX_EDICTS,
                                          CUtlMemoryPool::GROW_NONE,
                                          "g_EntityGroundLinks");
...
public:
  ...
  CDataObjectAccessSystem() {
    // Cast to int to make it clear that we know we are comparing different enum
    // types.
...
#ifdef STAGING_ONLY
#ifndef CLIENT_DLL
ConVar sv_groundlink_debug(
    "sv_groundlink_debug", "0", FCVAR_NONE,
...
inline groundlink_t *AllocGroundLink(void) {
  ...
  if (link) {
    ...
    DevMsg("AllocGroundLink: failed to allocate groundlink_t.!!!  "
           "groundlinksallocated=%d g_EntityGroundLinks.Count()=%d\n",
