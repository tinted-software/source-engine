...
void CHL2MP_Player::PrecacheFootStepSounds(void) {
  ...
  for (i = 0; i < iFootstepSounds; ++i) {
    ...
    Q_snprintf(szFootStepName, sizeof(szFootStepName), "%s.RunFootstepRight",
               g_ppszPlayerSoundPrefixNames[i]);
    PrecacheScriptSound(szFootStepName);
  }
}
...
#ifndef CLIENT_DLL
  // Adrian: Make the model's angle match the legs so the hitboxes match on both
  // sides.
...
void CPlayerAnimState::ComputePoseParam_BodyLookYaw(void) {
  ...
  if (!isMoving) {
    ...
    turning = ConvergeAngles(m_flGoalFeetYaw, turnrate, gpGlobals->frametime,
                             m_flCurrentFeetYaw);
    ...
    // Snap upper body into position since the delta is already smoothed for the
    // feet
