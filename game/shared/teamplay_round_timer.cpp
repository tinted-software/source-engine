...
ConVar tf_overtime_nag("tf_overtime_nag", "0", FCVAR_NOTIFY,
...
static void RecvProxy_TimerPaused(const CRecvProxyData *pData, void *pStruct,
                                  void *pOut) {
...
RecvPropInt(RECVINFO(m_bTimerPaused), 0, RecvProxy_TimerPaused),
    ...
    RecvPropInt(RECVINFO(m_nSetupTimeLength)), RecvPropInt(RECVINFO(m_nState)),
...
SendPropBool(SENDINFO(m_bTimerPaused)),
    ...
    SendPropInt(SENDINFO(m_nSetupTimeLength)), SendPropInt(SENDINFO(m_nState)),
    ...
    END_NETWORK_TABLE()
        ...
        BEGIN_DATADESC(CTeamRoundTimer)
            DEFINE_KEYFIELD(m_nTimerInitialLength, FIELD_INTEGER,
                            "timer_length"),
...
void CTeamRoundTimer::Spawn(void) {
  ...
  if (IsDisabled()) // we need to get the data initialized before actually
                    // become disabled
  {
    m_bIsDisabled = false;
...
#ifndef GAME_DLL
  // We need to add a couple seconds to the time remaining because we've
  // probably lost ~0.5 seconds from the timer while waiting for the update to
  // arrive from the server and we don't want to miss any critical countdown
  // messages.  If the time remaining is over 10 seconds...adding 2 seconds to
  // the total when calculating our output messages won't affect anything
  if (flTime > 10.0f) {
    flTime += 2.0f;
  }
#endif
...
void CTeamRoundTimer::OnDataChanged(DataUpdateType_t updateType) {
  ...
  // if we were in state_setup and now we're in state_normal, play the bell
  // sound
...
void CTeamRoundTimer::SendTimeWarning(int nWarning) {
  ...
  if (TFGameRules() && TFGameRules()->IsHalloweenScenario(
                           CTFGameRules::HALLOWEEN_SCENARIO_HIGHTOWER))
    return;
#endif

  // don't play sounds if the level designer has turned them off or if it's
  // during the WaitingForPlayers time
  if (!m_bTimerPaused && m_bAutoCountdown &&
      ...
      if (ObjectiveResource()) {
        ...
        if (TeamplayRoundBasedRules()->IsInTournamentMode() == true &&
          ...
          if (IsStopWatchTimer() == true && IsWatchingTimeStamps() == false) {
            CTeamRoundTimer *pTimer = dynamic_cast<CTeamRoundTimer *>(
                ClientEntityList().GetEnt(iActiveTimer));
            ...
            CTeamRoundTimer *pStopWatch = dynamic_cast<CTeamRoundTimer *>(
                ClientEntityList().GetEnt(iStopWatchTimer));
...
void CTeamRoundTimer::SetTimerThink(int nType) {
  if (nType == RT_THINK_SETUP) {
    SetContextThink(&CTeamRoundTimer::RoundTimerSetupThink,
                    gpGlobals->curtime + 0.05, ROUND_TIMER_SETUP_THINK);
    SetContextThink(NULL, 0, ROUND_TIMER_THINK);
  } else {
    SetContextThink(&CTeamRoundTimer::RoundTimerThink,
                    gpGlobals->curtime + 0.05, ROUND_TIMER_THINK);
...
void CTeamRoundTimer::RoundTimerSetupThink(void) {
  if (TeamplayRoundBasedRules()->IsInPreMatch() == true &&
      IsDisabled() == false) {
    inputdata_t data;
    InputDisable(data);
  ...
  if (flTime <= 0.0f && m_bFireFinished) {
    IGameEvent *event =
        gameeventmanager->CreateEvent("teamplay_setup_finished");
    if (event) {
...
void CTeamRoundTimer::RoundTimerThink(void) {
  ...
  // Don't do anything when the game has been won or if we're loading a bugbait
  // report
  if (TeamplayRoundBasedRules()->RoundHasBeenWon() ||
    ...
    // force mapmakers to deal with having to unpause it. This little hack works
    // around that.
    if (!m_bTimerPaused) {
      PauseTimer();
  ...
  if (flTime <= 0.0f && m_bFireFinished) {
    // Allow the gamerules to prevent timer expiration (i.e. while a control
    // point is contested)
    if (!TeamplayGameRules()->TimerMayExpire()) {
      ...
      if (ShowInHud()) {
        if (!TeamplayRoundBasedRules()->InOvertime()) {
          ...
          if (tf_overtime_nag.GetBool() &&
              (gpGlobals->curtime > m_flNextOvertimeNag)) {
  ...
  SetContextThink(&CTeamRoundTimer::RoundTimerThink, gpGlobals->curtime + 0.05,
                  ROUND_TIMER_THINK);
}
...
//-----------------------------------------------------------------------------
void CTeamRoundTimer::AddTimerSeconds(
    int iSecondsToAdd, int iTeamResponsible /* = TEAM_UNASSIGNED*/) {
  if (IsDisabled())
    return;
  ...
  // we only want to add time if we're round_running or team_win so the control
  // points don't add time when they try to set their default owner when the map
  // is first loading
  ...
  if ((ObjectiveResource() &&
       ObjectiveResource()->GetTimerInHUD() == entindex()) ||
      (TeamplayRoundBasedRules()->IsInKothMode())) {
    if (!TeamplayRoundBasedRules()->InStalemate() &&
        !TeamplayRoundBasedRules()->RoundHasBeenWon() &&
        !TeamplayRoundBasedRules()->IsInKothMode()) {
      if (iTeamResponsible >= LAST_SHARED_TEAM + 1) {
        for (int iTeam = LAST_SHARED_TEAM + 1; iTeam < GetNumberOfTeams();
             iTeam++) {
    ...
    if (m_bShowInHUD) {
      IGameEvent *event =
          gameeventmanager->CreateEvent("teamplay_timer_time_added");
      if (event) {
...
void CTeamRoundTimer::InputShowInHUD(inputdata_t &input) {
  ...
  if (m_bShowInHUD && !nShow) {
    ...
    SetActiveTimer(this);
    SetState(m_nState, false); // set our current state again so the gamerules
                               // are updated with our setup state
  }
