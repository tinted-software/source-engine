...
void CChoreoScene::Think(float curtime) {
  ...
  if (m_flCurrentTime != oldt) {
    ...
    // curtime = m_flCurrentTime + ( oldt - flLoopPoint ); // if we overshot,
    // skip by how much we overshot
...
void CChoreoScene::ProcessActiveListEntry(ActiveList *entry) {
  ...
  case PROCESSING_TYPE_START_RESUMECONDITION: {
    ...
    // This event can "pause" the scene, so we need to remember who "paused" the
    // scene so that
    //  when we resume we can resume any suppressed events dependent on this
    //  pauser...
    if (entry->e->GetType() == CChoreoEvent::SECTION) {
      // So this event should be in the pauseevents list, otherwise this'll be
      // -1
