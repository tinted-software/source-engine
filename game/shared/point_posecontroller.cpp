...
DEFINE_AUTO_ARRAY(m_hProps, FIELD_EHANDLE),
    ...
    // Inputs
    DEFINE_INPUTFUNC(FIELD_STRING, "SetPoseParameterName",
                     InputSetPoseParameterName),
    DEFINE_INPUTFUNC(FIELD_FLOAT, "SetPoseValue", InputSetPoseValue),
    DEFINE_INPUTFUNC(FIELD_FLOAT, "SetInterpolationTime",
    ...
    END_DATADESC()
        ...
        IMPLEMENT_SERVERCLASS_ST(CPoseController, DT_PoseController)
            SendPropArray3(SENDINFO_ARRAY3(m_hProps),
                           SendPropEHandle(SENDINFO_ARRAY(m_hProps))),
    SendPropArray3(SENDINFO_ARRAY3(m_chPoseIndex),
                   SendPropInt(SENDINFO_ARRAY(m_chPoseIndex), 5,
                               SPROP_UNSIGNED)), // bits sent must be enough to
                                                 // represent MAXSTUDIOPOSEPARAM
    SendPropBool(SENDINFO(m_bPoseValueParity)),
    SendPropFloat(SENDINFO(m_fPoseValue), 11, 0, 0.0f, 1.0f),
    SendPropFloat(SENDINFO(m_fInterpolationTime), 11, 0, 0.0f,
                  MAX_POSE_INTERPOLATION_TIME),
    SendPropBool(SENDINFO(m_bInterpolationWrap)),
    SendPropFloat(SENDINFO(m_fCycleFrequency), 11, 0, -MAX_POSE_CYCLE_FREQUENCY,
                  MAX_POSE_CYCLE_FREQUENCY),
    SendPropInt(SENDINFO(m_nFModType), 3, SPROP_UNSIGNED),
    SendPropFloat(SENDINFO(m_fFModTimeOffset), 11, 0, -1.0f, 1.0f),
    SendPropFloat(SENDINFO(m_fFModRate), 11, 0, -MAX_POSE_FMOD_RATE,
                  MAX_POSE_FMOD_RATE),
    SendPropFloat(SENDINFO(m_fFModAmplitude), 11, 0, 0.0f,
                  MAX_POSE_FMOD_AMPLITUDE),
    END_SEND_TABLE()
...
void CPoseController::BuildPropList(void) {
  ...
  while (pEnt && iPropNum < MAX_POSE_CONTROLLED_PROPS) {
    ...
    if (pProp) {
      ...
      SetPoseIndex(iPropNum,
                   pProp->LookupPoseParameter(m_iszPoseParameterName.ToCStr()));
...
void CPoseController::BuildPoseIndexList(void) {
  for (int iPropNum = 0; iPropNum < MAX_POSE_CONTROLLED_PROPS; ++iPropNum) {
    CBaseAnimating *pProp =
        dynamic_cast<CBaseAnimating *>(m_hProps[iPropNum].Get());
...
void CPoseController::SetCycleFrequency(float fValue) {
  m_fCycleFrequency =
      clamp(fValue, -MAX_POSE_CYCLE_FREQUENCY, MAX_POSE_CYCLE_FREQUENCY);
}
...
void CPoseController::RandomizeFMod(float fExtremeness) {
  ...
  SetFModRate(RandomFloat(fExtremeness * -MAX_POSE_FMOD_RATE,
                          fExtremeness * MAX_POSE_FMOD_RATE));
...
void CPoseController::InputGetFMod(inputdata_t &inputdata) {
  DevMsg("FMod values for pose controller %s\nTYPE: %i\nTIME OFFSET: %f\nRATE: "
         "%f\nAMPLITUDE: %f\n",
         STRING(GetEntityName()), m_nFModType.Get(), m_fFModTimeOffset.Get(),
         m_fFModRate.Get(), m_fFModAmplitude.Get());
}
...
RecvPropArray3(RECVINFO_ARRAY(m_hProps),
               RecvPropEHandle(RECVINFO(m_hProps[0]))),
    RecvPropArray3(RECVINFO_ARRAY(m_chPoseIndex),
                   RecvPropInt(RECVINFO(m_chPoseIndex[0]))),
...
void C_PoseController::OnDataChanged(DataUpdateType_t updateType) {
  ...
  if (m_bOldPoseValueParity != m_bPoseValueParity) {
    ...
    float fClientPoseValue =
        m_fCurrentPoseValue + m_PoseTransitionValue.Interp(gpGlobals->curtime);
    ...
    if (m_bInterpolationWrap) {
      float fInterpBackward =
          (fClientPoseValue + ((fClientPoseValue < 0.5f) ? (1.0f) : (-1.0f))) -
          m_fPoseValue;

      m_PoseTransitionValue.Init(
          ((fabsf(fInterpForward) < fabsf(fInterpBackward))
               ? (fInterpForward)
               : (fInterpBackward)),
          0.0f, m_fInterpolationTime);
    } else {
...
void C_PoseController::UpdateModulation(void) {
  ...
  case POSECONTROLLER_FMODTYPE_SQUARE: {
    float fCycleTime =
        fabsf(m_fFModRate * 2.0f * (gpGlobals->curtime + m_fFModTimeOffset));
  ...
  case POSECONTROLLER_FMODTYPE_TRIANGLE: {
    float fCycleTime =
        fabsf(m_fFModRate * 4.0f * (gpGlobals->curtime + m_fFModTimeOffset));
...
void CPoseController::SetCurrentPose(float fCurrentPoseValue) {
  for (int iPropNum = 0; iPropNum < MAX_POSE_CONTROLLED_PROPS; ++iPropNum) {
    ...
    if (pProp) {
      ...
      pProp->GetPoseParameterRange(m_chPoseIndex[iPropNum], fPoseValueMin,
                                   fPoseValueMax);
      pProp->SetPoseParameter(
          m_chPoseIndex[iPropNum],
