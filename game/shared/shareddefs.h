...
#define VEC_DEAD_VIEWHEIGHT_SCALED(player)                                     \
  (g_pGameRules->GetViewVectors()->m_vDeadViewHeight * player->GetModelScale())
...
#define DMG_CRUSH                                                              \
  (1                                                                           \
   << 0) // crushed by falling or moving object.
         ...
         // DON'T use DMG_CRUSH when damaging entities unless it's the result of
         // a physics collision. You probably want DMG_CLUB instead.
...
#define DMG_NEVERGIB                                                           \
  (1 << 12) // with this bit OR'd in, no damage type will be able to gib victims
            // upon death
#define DMG_ALWAYSGIB                                                          \
  (1 << 13) // with this bit OR'd in, any damage type can be made to gib victims
            // upon death.
...
#define DMG_REMOVENORAGDOLL                                                    \
  (1 << 22) // with this bit OR'd in, no ragdoll will be created, and the target
            // will be quietly removed. use this to kill an entity that you've
            // already got a server-side ragdoll for
...
enum FireBulletsFlags_t {
  ...
  FIRE_BULLETS_TEMPORARY_DANGER_SOUND =
      0x8, // Danger sounds added from this impact can be stomped immediately if
           // another is queued
};
...
struct EmitSound_t {
  EmitSound_t()
      : m_nChannel(0), m_pSoundName(0), m_flVolume(VOL_NORM),
        m_SoundLevel(SNDLVL_NONE), m_nFlags(0), m_nPitch(PITCH_NORM),
        m_nSpecialDSP(0), m_pOrigin(0), m_flSoundTime(0.0f),
        m_pflSoundDuration(0), m_bEmitCloseCaption(true),
        m_bWarnOnMissingCloseCaption(false),
        m_bWarnOnDirectWaveReference(false), m_nSpeakerEntity(-1),
        m_UtlVecSoundOrigin(), m_hSoundScriptHandle(-1) {}
  ...
  mutable CUtlVector<Vector>
      m_UtlVecSoundOrigin; ///< Actual sound origin(s) (can be multiple if sound
                           ///< routed through speaker entity(ies) )
