...
CVoiceBanMgr::CVoiceBanMgr() { Clear(); }

CVoiceBanMgr::~CVoiceBanMgr() { Term(); }
...
void CVoiceBanMgr::Term() {
  ...
  for (int i = 0; i < 256; i++) {
    ...
    BannedPlayer *pNext;
    for (BannedPlayer *pCur = pListHead->m_pNext; pCur != pListHead;
         pCur = pNext) {
      pNext = pCur->m_pNext;
      delete pCur;
    }
...
void CVoiceBanMgr::SetPlayerBan(char const playerID[SIGNED_GUID_LEN],
                                bool bSquelch) {
  if (bSquelch) {
...
CVoiceBanMgr::BannedPlayer *
CVoiceBanMgr::InternalFindPlayerSquelch(char const playerID[SIGNED_GUID_LEN]) {
...
CVoiceBanMgr::BannedPlayer *
CVoiceBanMgr::AddBannedPlayer(char const playerID[SIGNED_GUID_LEN]) {
