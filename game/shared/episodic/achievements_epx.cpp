...
protected:
  ...
  virtual void ListenForEvents() { ListenForGameEvent("physgun_pickup"); }

  void FireGameEvent_Internal(IGameEvent *event) {
    if (0 == Q_strcmp(event->GetName(), "physgun_pickup")) {
      // was the object picked up a frag grenade?
      CBaseEntity *pEntityPickedUp =
          UTIL_EntityByIndex(event->GetInt("entindex"));
      if (pEntityPickedUp &&
          pEntityPickedUp->ClassMatches("npc_grenade_frag")) {
        ...
        if (pGrenade) {
          ...
          if (pOriginalThrower &&
              pOriginalThrower->ClassMatches("npc_zombine")) {
            IncrementCount();
          }
        }
      }
...
DECLARE_ACHIEVEMENT(CAchievementEpXGetZombineGrenade,
                    ACHIEVEMENT_EPX_GET_ZOMBINEGRENADE,
                    "EPX_GET_ZOMBINEGRENADE", 5);
...
protected:
  ...
  virtual void ListenForEvents() { ListenForGameEvent("flare_ignite_npc"); }

  void FireGameEvent_Internal(IGameEvent *event) {
    if (0 == Q_strcmp(event->GetName(), "flare_ignite_npc")) {
      CBaseEntity *pEntityIgnited =
          UTIL_EntityByIndex(event->GetInt("entindex"));
      // was it a zombie that got set on fire?
      if (pEntityIgnited &&
          ((pEntityIgnited->ClassMatches("npc_zombie")) ||
           (pEntityIgnited->ClassMatches("npc_zombine")) ||
           (pEntityIgnited->ClassMatches("npc_fastzombie")) ||
           (pEntityIgnited->ClassMatches("npc_poisonzombie")))) {
        IncrementCount();
      }
    }
  }
};
DECLARE_ACHIEVEMENT(CAchievementEpXKillZombiesWithFlares,
                    ACHIEVEMENT_EPX_KILL_ZOMBIES_WITHFLARES,
