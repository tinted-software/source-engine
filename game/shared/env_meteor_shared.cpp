...
void CEnvMeteorShared::Init(int nID, float flStartTime, float flPassiveTime,
                            ...
                            float flDamageRadius, const Vector &vecTriggerMins,
...
void CEnvMeteorShared::GetPositionAtTime(float flTime, Vector &vecPosition) {
  ...
  Vector vecVelocity(m_vecDirection.x * m_flSpeed, m_vecDirection.y * m_flSpeed,
                     m_vecDirection.z * m_flSpeed);
...
void CEnvMeteorShared::ConvertFromSkyboxToWorld(void) {
  ...
  m_vecPos +=
      (m_flSpeed * m_vecDirection) * (m_flWorldEnterTime - m_flStartTime);
...
#define METEOR_TRIGGER_EPSILON 0.001f
  ...
  for (int iAxis = 0; iAxis < 3; iAxis++) {
    ...
    if ((flDistStart > 0.0f) && (flDistEnd < 0.0f)) {
      flFrac =
          (flDistStart - METEOR_TRIGGER_EPSILON) / (flDistStart - flDistEnd);
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CEnvMeteorSpawnerShared::Init(IMeteorFactory *pFactory, int nRandomSeed,
                                   float flTime, const Vector &vecMinBounds,
                                   const Vector &vecMaxBounds,
...
float CEnvMeteorSpawnerShared::MeteorThink(float flTime) {
  ...
  while (m_flNextSpawnTime < flTime) {
    ...
    for (int iMeteor = 0; iMeteor < nMeteorCount; iMeteor++) {
      ...
      Vector meteorOrigin(
          GetRandomFloat(m_vecMinBounds.GetX(), m_vecMaxBounds.GetX()) /* x */,
      ...
      if (m_aTargets.Count() > 0) {
        ...
        vecTargetPos.x += GetRandomFloat(-m_aTargets[iTarget].m_flRadius,
                                         m_aTargets[iTarget].m_flRadius);
        vecTargetPos.y += GetRandomFloat(-m_aTargets[iTarget].m_flRadius,
                                         m_aTargets[iTarget].m_flRadius);
        vecTargetPos.z += GetRandomFloat(-m_aTargets[iTarget].m_flRadius,
                                         m_aTargets[iTarget].m_flRadius);
      ...
      m_pFactory->CreateMeteor(
          m_nMeteorCount, m_iMeteorType, meteorOrigin,
          vecDirection /* direction */,
          GetRandomFloat(m_flMinSpeed, m_flMaxSpeed) /* speed */,
          m_flNextSpawnTime, m_flMeteorDamageRadius, m_vecTriggerMins,
          m_vecTriggerMaxs);
    }
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CEnvMeteorSpawnerShared::AddToTargetList(const Vector &vecPosition,
                                              float flRadius) {
