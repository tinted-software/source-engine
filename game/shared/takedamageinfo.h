...
public:
  ...
  // weapons, the projectile is the inflictor, and this contains the weapon that
  // created the projectile.
...
protected:
  ...
  float m_flDamage;
  float m_flMaxDamage;
  float m_flBaseDamage; // The damage amount before skill leve adjustments are
                        // made. Used to get uniform damage forces.
  ...
  float m_flDamageBonus; // Anything that increases damage (crit) - store the
                         // delta
  bool m_bForceFriendlyFire; // Ideally this would be a dmg type, but we can't
                             // add more
...
inline void
CTakeDamageInfo::SetReportedPosition(const Vector &reportedPosition) {
