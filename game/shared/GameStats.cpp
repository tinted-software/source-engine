...
CBaseGameStats_Driver::CBaseGameStats_Driver(void)
    : BaseClass("CGameStats"), m_iLoadedVersion(-1), m_bEnabled(false),
      m_bShuttingDown(false), m_bInLevel(false), m_bFirstLevel(true),
      m_flLevelStartTime(0.0f), m_bStationary(false),
      m_flLastMovementTime(0.0f), m_bGamePaused(false), m_pGamestatsData(NULL),
      m_bBufferFull(false), m_nWriteIndex(0), m_flLastRealTime(-1),
      m_flLastSampleTime(-1), m_flTotalTimeInLevels(0), m_iNumLevels(0),
...
bool CBaseGameStats::SaveToFileNOW(bool bForceSyncWrite /* = false */) {
  ...
  // this code path is only for old format stats.  Products that use new format
...
void CBaseGameStats::Event_DecrementPlayerEnteredNoClip(
  ...
  StatsLog("CBaseGameStats::Event_DecrementPlayerEnteredNoClip [%s] "
           "decrementing NOCLIPe\n",
           pBasePlayer->GetPlayerName());
}
...
bool CBaseGameStats::UploadStatsFileNOW(void) {
  if (!StatsTrackingIsFullyEnabled() || !HaveValidData() ||
      !gamestats->UseOldFormat())
    return false;
...
#ifdef CLIENT_DLL
      // The stat system isn't designed to handle split screen players. Until it
      // get's redesigned, let's take the first player's split screen ping,
      // since all other stats will be based on the first player
      IGameResources *gr = GameResources();
      int ping = 0;
      C_BasePlayer *pPlayer =
          C_BasePlayer::GetLocalPlayer(); // GetLocalPlayer(
...
void CBaseGameStats_Driver::LevelInitPreEntity() {
  ...
  if (Q_stricmp(s_szPseudoUniqueID, "unknown") == 0) {
    // "unknown" means this is a dedicated server and we weren't able to
    // generate a unique ID (e.g. Linux server). Change the unique ID to be a
    // hash of IP & port.  We couldn't do this earlier because IP is not known
    // until level init time.
    ...
    if (hostip && hostport) {
      ...
      if (crcInput[0] && crcInput[1]) {
        ...
        Q_snprintf(s_szPseudoUniqueID, ARRAYSIZE(s_szPseudoUniqueID), "H:%x",
                   crc);
      }
    }
  }
...
void CBaseGameStats_Driver::LevelShutdown() {
  ...
  if (flElapsed < 0.0f) {
    Assert(0);
    Warning("EVENT_LEVELSHUTDOWN:  with negative elapsed time (rt %f starttime "
            "%f)\n",
...
void CBaseGameStats_Driver::CollectData(StatSendType_t sendType) {
...
#ifdef GAME_DLL
  // for server, check with the engine to see if there already a gamestats data
  // container registered.  (There will be if there is a client running in the
  // same process.)
  ...
  if (sendType == STATSEND_LEVELSHUTDOWN && pGamestatsData &&
      pGamestatsData->m_bHaveData) {
    GetSteamWorksSGameStatsUploader().AddClientPerfData(
        pGamestatsData->m_pKVData);
  }
...
// Need to initialiate a reset since cs isn't using the gamestat system to add
// data
...
bool CBaseGameStats_Driver::AddBaseDataForSend(KeyValues *pKV,
  ...
  case STATSEND_LEVELSHUTDOWN:
    ...
    if (m_bBufferFull) {
      ...
      float flAverageFrameRate =
          AverageStat(&StatsBufferRecord_t::m_flFrameRate);
      ...
      float flAverageServerPing =
          AverageStat(&StatsBufferRecord_t::m_flServerPing);
      ...
      if (steamapicontext && steamapicontext->SteamApps()) {
        ...
        pKV->SetString("Language",
                       currentLanguage ? currentLanguage : "unknown");
      }
#endif

      // We need to filter out client side dev work from playtest work for the
      // stat reporting. The simplest way is to check for sv_cheats, since we
      // also do NOT want client stat reports where the player has cheated.
      if (NULL != sv_cheats) {
...
void CBaseGameStats_Driver::ResetData() {
#ifdef GAME_DLL
  // on the server, if there is a gamestats data container registered (by a
  // client in the same process), they're in charge of resetting it, nothing for
  // us to do
  ...
  pKV->SetString("GPUDriverVersion", CFmtStr("%d.%d", gpu.m_nDriverVersionHigh,
