...
public:
  ...
  void Dump(void) {
    ...
    for (UtlHashHandle_t i = m_Strings.FirstHandle();
         i != m_Strings.InvalidHandle(); i = m_Strings.NextHandle(i)) {
...
//-----------------------------------------------------------------------------
IGameSystem *GameStringSystem() { return &g_GameStringPool; }
#endif
...
string_t
AllocPooledString_StaticConstantStringPointer(const char *pszGlobalConstValue) {
  Assert(pszGlobalConstValue && *pszGlobalConstValue);
  return MAKE_STRING(g_GameStringPool.AllocateWithKey(pszGlobalConstValue,
                                                      pszGlobalConstValue));
}
...
static ConCommand dumpgamestringtable(
    "dumpgamestringtable", CC_DumpGameStringTable,
