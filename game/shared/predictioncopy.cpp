...
CPredictionCopy::difftype_t
CPredictionCopy::CompareData(int size, char *outdata, const char *indata) {
  if (!m_bErrorCheck)
    return DIFFERS;
...
void CPredictionCopy::DescribeVector(difftype_t dt, Vector *outValue,
  ...
  if (dt == DIFFERS) {
    ...
    ReportFieldsDiffer("vec[] differs (1st diff) (net %f %f %f - pred %f %f "
                       "%f) delta(%f %f %f)\n",
                       inValue[i].x, inValue[i].y, inValue[i].z, outValue[i].x,
                       outValue[i].y, outValue[i].z, delta.x, delta.y, delta.z);
  }
...
void CPredictionCopy::DescribeQuaternion(difftype_t dt, Quaternion &outValue,
  ...
  if (dt == DIFFERS) {
    ...
    ReportFieldsDiffer("quaternion differs (net %f %f %f %f - pred %f %f %f "
                       "%f) delta(%f %f %f %f)\n",
...
void CPredictionCopy::DescribeQuaternion(difftype_t dt, Quaternion *outValue,
  ...
  if (dt == DIFFERS) {
    ...
    ReportFieldsDiffer("quaternion[] differs (1st diff) (net %f %f %f %f - "
                       "pred %f %f %f %f) delta(%f %f %f %f)\n",
                       (float)inValue[i][0], (float)inValue[i][1],
                       (float)inValue[i][2], (float)inValue[i][3],
                       (float)outValue[i][0], (float)outValue[i][1],
                       (float)outValue[i][2], (float)outValue[i][3], delta[0],
                       delta[1], delta[2], delta[3]);
  }

  DescribeFields(dt, "quaternion (%f %f %f %f)\n", (float)outValue[0][0],
                 (float)outValue[0][1], (float)outValue[0][2],
                 (float)outValue[0][3]);
}
...
void CPredictionCopy::WatchQuaternion(difftype_t dt, Quaternion &outValue,
  ...
  WatchMsg("quaternion (%f %f %f %f)", (float)outValue[0], (float)outValue[1],
           (float)outValue[2], (float)outValue[3]);
}
...
CPredictionCopy::difftype_t
CPredictionCopy::CompareInt(int *outvalue, const int *invalue, int count) {
  if (!m_bErrorCheck)
    return DIFFERS;
...
CPredictionCopy::difftype_t
CPredictionCopy::CompareBool(bool *outvalue, const bool *invalue, int count) {
  if (!m_bErrorCheck)
    return DIFFERS;
...
CPredictionCopy::difftype_t
CPredictionCopy::CompareString(char *outstring, const char *instring) {
  if (!m_bErrorCheck)
    return DIFFERS;
...
CPredictionCopy::difftype_t
CPredictionCopy::CompareVector(Vector &outValue, const Vector &inValue) {
  if (!m_bErrorCheck)
    return DIFFERS;
...
CPredictionCopy::difftype_t
CPredictionCopy::CompareQuaternion(Quaternion &outValue,
                                   const Quaternion &inValue) {
  if (!m_bErrorCheck)
    return DIFFERS;
...
CPredictionCopy::difftype_t
CPredictionCopy::CompareEHandle(EHANDLE *outvalue, EHANDLE const *invalue,
                                int count) {
  if (!m_bErrorCheck)
    return DIFFERS;
  ...
  for (i = 0; i < fieldCount; i++) {
    ...
    case FIELD_FLOAT: {
      ...
      if (m_bErrorCheck && m_bShouldDescribe)
        DescribeFloat(difftype, (float *)pOutputData, (float const *)pInputData,
                      fieldSize);
      if (bShouldWatch)
    ...
    case FIELD_VECTOR: {
      ...
      if (m_bErrorCheck && m_bShouldDescribe)
        DescribeVector(difftype, (Vector *)pOutputData,
                       (Vector const *)pInputData, fieldSize);
      if (bShouldWatch)
        WatchVector(difftype, (Vector *)pOutputData, (Vector const *)pInputData,
                    fieldSize);
    } break;
    ...
    case FIELD_BOOLEAN: {
      difftype =
          CompareBool((bool *)pOutputData, (bool const *)pInputData, fieldSize);
    ...
    case FIELD_SHORT: {
      ...
      CopyShort(difftype, (short *)pOutputData, (short const *)pInputData,
                fieldSize);
      if (m_bErrorCheck && m_bShouldDescribe)
        DescribeShort(difftype, (short *)pOutputData, (short const *)pInputData,
                      fieldSize);
      if (bShouldWatch)
    ...
    case FIELD_EHANDLE: {
      ...
      CopyEHandle(difftype, (EHANDLE *)pOutputData, (EHANDLE const *)pInputData,
                  fieldSize);
      if (m_bErrorCheck && m_bShouldDescribe)
...
static ConVar
    pwatchvar("pwatchvar", "", FCVAR_CHEAT,
...
public:
        ...
        void	DescribeFields( datamap_t *pMap, typedescription_t *pFields, int
fieldCount );
...
void CValueChangeTracker::GetValue(char *buf, size_t bufsize) {
  ...
  case FIELD_COLOR32: {
    ...
    Q_snprintf(buf, bufsize, "%d %d %d %d", color->r(), color->g(), color->b(),
               color->a());
  } break;
