...
#ifdef _DEBUG
ConVar tf_weapon_criticals_force_random("tf_weapon_criticals_force_random", "0",
                                        FCVAR_REPLICATED | FCVAR_CHEAT);
#endif // _DEBUG
ConVar tf_weapon_criticals_bucket_cap("tf_weapon_criticals_bucket_cap",
                                      "1000.0", FCVAR_REPLICATED | FCVAR_CHEAT);
ConVar tf_weapon_criticals_bucket_bottom("tf_weapon_criticals_bucket_bottom",
                                         "-250.0",
                                         FCVAR_REPLICATED | FCVAR_CHEAT);
ConVar tf_weapon_criticals_bucket_default("tf_weapon_criticals_bucket_default",
                                          "300.0",
                                          FCVAR_REPLICATED | FCVAR_CHEAT);
#endif // TF
...
void CBaseCombatWeapon::SetOwner(CBaseCombatCharacter *owner) {
  if (!owner) {
    ...
    // We have to force an active state change, because it's being dropped and
    // won't call UpdateClientData()
...
void CBaseCombatWeapon::GiveTo(CBaseEntity *pOther) { DefaultTouch(pOther); }
...
void CBaseCombatWeapon::SetActivity(Activity act, float duration) {
  ...
  if (sequence != ACTIVITY_NOT_AVAILABLE) {
    ...
    if (duration > 0) {
      ...
      m_flPlaybackRate =
          MIN(m_flPlaybackRate,
              12.0); // FIXME; magic number!, network encoding range
    } else {
...
void CBaseCombatWeapon::AddToCritBucket(float flAmount) {
  ...
  if (m_flCritTokenBucket < flCap) {
    // Treat raw damage as the resource by which we add or subtract from the
    // bucket
...
bool CBaseCombatWeapon::IsAllowedToWithdrawFromCritBucket(float flDamage) {
  ...
  float flMult =
      (IsMeleeWeapon())
          ? 0.5f
          : RemapValClamped(((float)m_nCritSeedRequests / (float)m_nCritChecks),
                            0.1f, 1.f, 1.f, 3.f);
...
void CBaseCombatWeapon::ItemPostFrame(void) {
  ...
  if (!bFired && (pOwner->m_nButtons & IN_ATTACK) &&
    ...
    if (!IsMeleeWeapon() && ((UsesClipsForAmmo1() && m_iClip1 <= 0) ||
      ...
      // guns catch the leading edge trigger 			on the player hitting the attack
      //key.  It relies on the gun catching that case in the same frame.
      //			However, because the player can also be doing a
      //secondary attack, the edge trigger may be missed. 			We really need to hold
      //onto the edge trigger and only clear the condition when the gun has
      //fired its 			first shot.  Right now that's too much of an architecture
      //change -- jdw
...
// to attack yet
//-----------------------------------------------------------------------------
void CBaseCombatWeapon::ItemBusyFrame(void) { UpdateAutoFire(); }
...
void CBaseCombatWeapon::FinishReload(void) {
  ...
  if (pOwner) {
    ...
    if (UsesClipsForAmmo1()) {
      int primary = MIN(GetMaxClip1() - m_iClip1,
    ...
    if (UsesClipsForAmmo2()) {
      int secondary = MIN(GetMaxClip2() - m_iClip2,
...
void CBaseCombatWeapon::PrimaryAttack(void) {
  ...
  if (UsesClipsForAmmo1()) {
    ...
    info.m_iShots =
        MIN(info.m_iShots, pPlayer->GetAmmoCount(m_iPrimaryAmmoType));
...
#ifdef GAME_DLL
//-----------------------------------------------------------------------------
// Collect trace attacks for weapons that fire multiple bullets per attack that
// also penetrate
//-----------------------------------------------------------------------------
void CDmgAccumulator::AccumulateMultiDamage(const CTakeDamageInfo &info,
                                            CBaseEntity *pEntity) {
  if (!pEntity)
    return;
  ...
  if (iIndex == m_TargetsDmgInfo.InvalidIndex()) {
    ...
    if (pInfo) {
      ...
      m_TargetsDmgInfo[iIndex].SetDamageForce(pInfo->GetDamageForce() +
                                              info.GetDamageForce());
      ...
      m_TargetsDmgInfo[iIndex].SetMaxDamage(
          MAX(pInfo->GetMaxDamage(), info.GetDamage()));
...
DEFINE_PRED_FIELD(m_nNextThinkTick, FIELD_INTEGER, FTYPEDESC_INSENDTABLE),
    ...
    // Function pointers
    DEFINE_ENTITYFUNC(DefaultTouch), DEFINE_THINKFUNC(FallThink),
    DEFINE_THINKFUNC(Materialize), DEFINE_THINKFUNC(AttemptToMaterialize),
    DEFINE_THINKFUNC(DestroyItem), DEFINE_THINKFUNC(SetPickupTouch),
...
//-----------------------------------------------------------------------------
void *SendProxy_SendNonLocalWeaponDataTable(const SendProp *pProp,
                                            const void *pStruct,
                                            const void *pVarData,
                                            CSendProxyRecipients *pRecipients,
                                            int objectID) {
