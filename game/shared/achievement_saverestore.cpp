...
public:
  const char *GetBlockName() { return "Achievement"; }
  ...
  void Save(ISave *pSave) {
    ...
    // save global achievement mgr state to separate file if there have been any
    // changes, so in case of a crash the global state is consistent with last
    // save game
    ...
    for (int i = 0; i < iTotalAchievements; i++) {
      IAchievement *pAchievement = pAchievementMgr->GetAchievementByIndex(i);
      if (pAchievement->ShouldSaveWithGame()) {
        CBaseAchievement *pBaseAchievement =
            dynamic_cast<CBaseAchievement *>(pAchievement);
        if (pBaseAchievement) {
  ...
  void ReadRestoreHeaders(IRestore *pRestore) {
    ...
    // only load if version matches and if we are loading a game, not a
    // transition
    m_fDoLoad = ((version == ACHIEVEMENT_SAVE_RESTORE_VERSION) &&
                 ((MapLoad_LoadGame == gpGlobals->eLoadType) ||
                  (MapLoad_NewGame == gpGlobals->eLoadType)));
  }
    ...
    if (m_fDoLoad) {
      ...
      while (nSavedAchievements--) {
        ...
        CBaseAchievement *pAchievement =
            pAchievementMgr->GetAchievementByID(iAchievementID);
        Assert(
            pAchievement); // It's a bug if we don't understand this achievement
        if (pAchievement) {
          // read achievement data
          pRestore->ReadAll(pAchievement, pAchievement->GetDataDescMap());
        } else {
          // if we don't recognize the achievement for some reason, read and
          // discard the data and keep going
