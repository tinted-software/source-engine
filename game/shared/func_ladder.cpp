...
/*static*/ ConVar sv_showladders("sv_showladders", "0", 0,
                                 "Show bbox and dismount points for all "
                                 "ladders (must be set before level load.)\n");
#endif
...
//-----------------------------------------------------------------------------
// Purpose: Called after all entities have spawned or after reload from .sav
// file
//-----------------------------------------------------------------------------
...
void CFuncLadder::SetEndPoints(const Vector &p1, const Vector &p2) {
  ...
  if (m_vecPlayerMountPositionBottom.GetZ() >
      m_vecPlayerMountPositionTop.GetZ()) {
  ...
  trace_t result;
  UTIL_TraceHull(m_vecPlayerMountPositionTop + Vector(0, 0, 4),
                 m_vecPlayerMountPositionTop, playerMins, playerMaxs,
                 MASK_PLAYERSOLID_BRUSHONLY, NULL,
                 COLLISION_GROUP_PLAYER_MOVEMENT, &result);

  if (!result.startsolid) {
    m_vecPlayerMountPositionTop = result.endpos;
  }

  UTIL_TraceHull(m_vecPlayerMountPositionBottom + Vector(0, 0, 4),
                 m_vecPlayerMountPositionBottom, playerMins, playerMaxs,
                 MASK_PLAYERSOLID_BRUSHONLY, NULL,
                 COLLISION_GROUP_PLAYER_MOVEMENT, &result);
...
void CFuncLadder::DrawDebugGeometryOverlays() {
  ...
  for (int i = 0; i < c; i++) {
    ...
    NDebugOverlay::Box(pt->GetAbsOrigin(), Vector(-16, -16, 0),
                       Vector(16, 16, 8), 150, 0, 0, 63, 0);
  }
#endif
}
...
//			list -
//-----------------------------------------------------------------------------
void CFuncLadder::FindNearbyDismountPoints(
  ...
  while ((pEntity = gEntList.FindEntityByClassnameWithin(
              pEntity, "info_ladder_dismount", origin, radius)) != NULL) {
    CInfoLadderDismount *landingspot =
        static_cast<CInfoLadderDismount *>(pEntity);
    Assert(landingspot);

    // If spot has a target, then if the target is not this ladder, don't add to
    // our list.
...
DEFINE_KEYFIELD(m_vecPlayerMountPositionTop, FIELD_VECTOR, "point0"),
  ...
  if (developer.GetBool()) {
    NDebugOverlay::Box(GetAbsOrigin(), Vector(-16, -16, 0), Vector(16, 16, 8),
                       127, 127, 127, 127, 0);
  }
#endif
}
