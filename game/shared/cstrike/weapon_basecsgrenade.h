...
protected:
  ...
  CNetworkVar(bool, m_bPinPulled); // Set to true when the pin has been pulled
                                   // but the grenade hasn't been thrown yet.
  CNetworkVar(
      float,
      m_fThrowTime); // the time at which the grenade will be thrown.  If this
                     // value is 0 then the time hasn't been set yet.

private:
  CBaseCSGrenade(const CBaseCSGrenade &) {}
};

inline bool CBaseCSGrenade::IsPinPulled() const { return m_bPinPulled; }
