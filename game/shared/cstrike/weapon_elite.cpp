...
void CWeaponElite::Precache() {
  ...
  m_droppedModelIndex =
      CBaseEntity::PrecacheModel(GetCSWpnData().m_szDroppedModel);
  m_inPrecache = false;
}
...
void CWeaponElite::PrimaryAttack() {
  ...
  FX_FireBullets(pPlayer->entindex(), pPlayer->Weapon_ShootPosition(),
                 ...
                 GetWeaponID(), FiringLeft() ? Secondary_Mode : Primary_Mode,
...
void CWeaponElite::WeaponIdle() {
  ...
  /*
          // switching to the idle with the slide back on the right pistol
     causes animation pops transitioning
          // from/to the depot/holster animations. The pop transition to the
     reload is less noticeable, so
...
bool CWeaponElite::OnFireEvent(C_BaseViewModel *pViewModel,
