...
ConVar sv_showimpacts("sv_showimpacts", "0", FCVAR_REPLICATED,
                      "Shows client (red) and server (blue) bullet impact "
                      "point (1=both, 2=client-only, 3=server-only)");
ConVar sv_showplayerhitboxes("sv_showplayerhitboxes", "0", FCVAR_REPLICATED,
                             "Show lag compensated hitboxes for the specified "
                             "player index whenever a player fires.");
...
void AddBulletStat(float time, float dist, Vector &impact) {
  ...
  float weight =
      (float)s_bullet_stats[i].count / (float)(s_bullet_stats[i].count + 1);
...
CON_COMMAND(stats_bullets_reset, "Reset bullet stats") { ResetBulletStats(); }

CON_COMMAND(stats_bullets_print, "Print bullet stats") { PrintBulletStats(); }
...
float CCSPlayer::GetPlayerMaxSpeed() {
  ...
  bool bValidMoveState =
      (State_Get() == STATE_ACTIVE || State_Get() == STATE_OBSERVER_MODE);
...
void CCSPlayer::GetBulletTypeParameters(int iBulletType,
...
static void GetMaterialParameters(int iMaterial, float &flPenetrationModifier,
  ...
  switch (iMaterial) {
  case CHAR_TEX_METAL:
    flPenetrationModifier = 0.5; // If we hit metal, reduce the thickness of the
                                 // brush we can't penetrate
...
static bool TraceToExit(Vector &start, Vector &dir, Vector &end,
                        float flStepSize, float flMaxDistance) {
  float flDistance = 0;
  Vector last = start;
...
inline void UTIL_TraceLineIgnoreTwoEntities(const Vector &vecAbsStart,
                                            const Vector &vecAbsEnd,
                                            unsigned int mask,
                                            const IHandleEntity *ignore,
                                            const IHandleEntity *ignore2,
...
void CCSPlayer::FireBullet(
  ...
  float fCurrentDamage =
      iDamage; // damage of the bullet at it's current trajectory
  float flCurrentDistance = 0.0; // distance that the bullet has traveled so far

  Vector vecDirShooting, vecRight, vecUp;
  AngleVectors(shootAngles, &vecDirShooting, &vecRight, &vecUp);

  // MIKETODO: put all the ammo parameters into a script file and allow for
  // CS-specific params.
  float flPenetrationPower =
      0; // thickness of a wall that this bullet can penetrate
  float flPenetrationDistance =
      0; // distance at which the bullet is capable of penetrating a wall
  float flDamageModifier = 0.5; // default modification of bullets power after
                                // they go through a wall.
  float flPenetrationModifier = 1.f;

  GetBulletTypeParameters(iBulletType, flPenetrationPower,
  ...
  // This is used to show where the lag compesator thinks the player should be
  // at.
#if 0
  ...
  while (fCurrentDamage > 0) {
    ...
    UTIL_TraceLineIgnoreTwoEntities(vecSrc, vecEnd,
                                    CS_MASK_SHOOT | CONTENTS_HITBOX, this,
                                    lastPlayerHit, COLLISION_GROUP_NONE, &tr);
    {
      CTraceFilterSkipTwoEntities filter(this, lastPlayerHit,
                                         COLLISION_GROUP_NONE);

      // Check for player hitboxes extending outside their collision bounds
      const float rayExtension = 40.0f;
      UTIL_ClipTraceToPlayers(vecSrc, vecEnd + vecDir * rayExtension,
                              CS_MASK_SHOOT | CONTENTS_HITBOX, &filter, &tr);
    }
...
#ifdef _DEBUG
    if (bFirstHit)
      AddBulletStat(gpGlobals->realtime, VectorLength(vecSrc - tr.endpos),
                    tr.endpos);
#endif
    ...
    // since some railings in de_inferno are CONTENTS_GRATE but
    // CHAR_TEX_CONCRETE, we'll trust the CONTENTS_GRATE and use a high damage
    // modifier.
    if (hitGrate) {
      // If we're a concrete grate (TOOLS/TOOLSINVISIBLE texture) allow more
      // penetrating power.
    ...
    if (sv_showimpacts.GetInt() == 1 || sv_showimpacts.GetInt() == 2) {
      ...
      debugoverlay->AddBoxOverlay(tr.endpos, Vector(-2, -2, -2),
                                  Vector(2, 2, 2), QAngle(0, 0, 0), 255, 0, 0,
                                  127, 4);
    ...
    if (sv_showimpacts.GetInt() == 1 || sv_showimpacts.GetInt() == 3) {
      ...
      NDebugOverlay::Box(tr.endpos, Vector(-2, -2, -2), Vector(2, 2, 2), 0, 0,
                         255, 127, 4);
    ...
    if (bDoEffects) {
      ...
      if (enginetrace->GetPointContents(tr.endpos) &
          (CONTENTS_WATER | CONTENTS_SLIME)) {
        trace_t waterTrace;
        UTIL_TraceLine(vecSrc, tr.endpos,
        ...
        if (!(tr.surface.flags &
          ...
          if (!(!friendlyfire.GetBool() && pEntity &&
    ...
    if (bWasAlive && !pEntity->IsAlive() && pEntity->IsPlayer() &&
        pEntity->GetTeamNumber() != GetTeamNumber()) {
      ++iPenetrationKills;
    }
    ...
    trace_t exitTr;
    UTIL_TraceLine(penetrationEnd, tr.endpos, CS_MASK_SHOOT | CONTENTS_HITBOX,
                   NULL, &exitTr);

    if (exitTr.m_pEnt != tr.m_pEnt && exitTr.m_pEnt != NULL) {
      // something was blocking, trace again
      UTIL_TraceLine(penetrationEnd, tr.endpos, CS_MASK_SHOOT | CONTENTS_HITBOX,
                     exitTr.m_pEnt, COLLISION_GROUP_NONE, &exitTr);
    }
    ...
    // check if bullet has enough power to penetrate this distance for this
    // material
    ...
    // NDebugOverlay::Box( exitTr.endpos, Vector(-2,-2,-2), Vector(2,2,2),
    // 0,255,0,127, 8 );
...
void CCSPlayer::UpdateStepSound(surfacedata_t *psurface,
...
void CCSPlayer::KickBack(float up_base, float lateral_base, float up_modifier,
                         float lateral_modifier, float up_max,
...
//-------------------------------------------------------------------------------------------------------------------------------
/**
 * Track the last time we were on a ladder, along with the ladder's normal and
 * where we were grabbing it, so we don't reach behind us and grab it again as
 * we are trying to dismount.
 */
 ...
 *  - if the 2D pos is very different, let us grab, since it's probably a
 * different ladder
 */
bool CCSPlayer::CanGrabLadder(const Vector &pos, const Vector &normal) {
  ...
  if (pos.AsVector2D().DistToSqr(m_lastLadderPos.AsVector2D()) <
      MaxDist * MaxDist) {
    return false;
  }
...
bool CCSPlayer::CSAnim_CanMove() { return CanMove(); }
...
void CFootstepControl::Spawn(void) { InitTrigger(); }
...
IMPLEMENT_CLIENTCLASS_DT(C_FootstepControl, DT_FootstepControl,
                         CFootstepControl)
...
surfacedata_t *CCSPlayer::GetFootstepSurface(const Vector &origin,
  ...
  for (int i = 0; i < s_footstepControllers.Count(); ++i) {
    ...
    if (FStrEq(control->m_source, surfaceName)) {
      if (control->CollisionProp()->IsPointInBounds(origin)) {
        return physprops->GetSurfaceData(
            physprops->GetSurfaceIndex(control->m_destination));
      }
    }
  }
