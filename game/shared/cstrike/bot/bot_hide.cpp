...
public:
  CollectHidingSpotsFunctor(CBaseEntity *me, const Vector &origin, float range,
                            int flags, Place place = UNDEFINED_PLACE)
      : m_origin(origin) {
    m_me = me;
    m_count = 0;
    m_range = range;
  ...
  bool operator()(CNavArea *area) {
    // if a place is specified, only consider hiding spots from areas in that
    // place
    ...
    FOR_EACH_VEC((*pSpots), it) {
      ...
      if (spot->GetArea() &&
          (spot->GetArea()->GetAttributes() & NAV_MESH_DONT_HIDE)) {
        // the area has been marked as DONT_HIDE since the last analysis, so
        // let's ignore it
        continue;
      }
      ...
      if (m_flags & spot->GetFlags()) {
        ...
        if (spot->GetArea() &&
            (spot->GetArea()->GetAttributes() & NAV_MESH_AVOID)) {
          m_totalWeight += 1;
        } else {
          m_totalWeight += 2;
        }
  ...
  int GetRandomHidingSpot(void) {
    ...
    for (int i = 0; i < m_count - 1; ++i) {
      // if the next spot's starting weight is over the target weight, this spot
      // is the one
 ...
 */
const Vector *FindNearbyHidingSpot(CBaseEntity *me, const Vector &pos,
  ...
  if (isSniper) {
    CollectHidingSpotsFunctor collector(me, pos, maxRange,
  ...
  if (collector.m_count == 0) {
    // no hiding spots at all - if we're not a sniper, try to find a sniper spot
    // to use instead
    if (!isSniper) {
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Select a random hiding spot among the nav areas that are tagged with the
 * given place
 */
const Vector *FindRandomHidingSpot(CBaseEntity *me, Place place,
                                   bool isSniper) {
  // collect set of nearby hiding spots
  if (isSniper) {
    CollectHidingSpotsFunctor collector(me, me->GetAbsOrigin(), -1.0f,
                                        HidingSpot::IDEAL_SNIPER_SPOT, place);
    ...
    if (collector.m_count) {
      ...
      // no ideal sniping spots, look for "good" sniping spots
      CollectHidingSpotsFunctor collector(me, me->GetAbsOrigin(), -1.0f,
                                          HidingSpot::GOOD_SNIPER_SPOT, place);
      TheNavMesh->ForAllAreas(collector);
 ...
 */
const Vector *FindNearbyRetreatSpot(CBaseEntity *me, const Vector &start,
                                    float maxRange, int avoidTeam) {
  CNavArea *startArea = TheNavMesh->GetNearestNavArea(start);
  if (startArea == NULL)
    return NULL;

  // collect hiding spots with decent "cover"
  CollectHidingSpotsFunctor collector(me, start, maxRange,
                                      HidingSpot::IN_COVER);
  ...
  // find the closest unoccupied hiding spot that crosses the least lines of
  // fire and has the best cover
  for (int i = 0; i < collector.m_count; ++i) {
    ...
    if (avoidTeam) {
      float range;
      if (UTIL_GetClosestPlayer(*collector.m_hidingSpot[i], avoidTeam,
                                &range)) {
...
//--------------------------------------------------------------------------------------------------------------------
/**
 * Functor to collect all hiding spots in range that we can reach before the
 * enemy arrives. NOTE: This only works for the initial rush.
 */
class CollectArriveFirstSpotsFunctor {
public:
  CollectArriveFirstSpotsFunctor(CBaseEntity *me, const Vector &searchOrigin,
                                 float enemyArriveTime, float range, int flags)
  ...
  bool operator()(CNavArea *area) {
    ...
    FOR_EACH_VEC((*pSpots), it) {
      ...
      // make sure hiding spot is in range
      if (m_range > 0.0f) {
        if ((spot->GetPosition() - m_searchOrigin)
      ...
      // only collect this hiding spot if we can reach it before the enemy
      // arrives NOTE: This assumes the area is fairly small and the difference
      // of moving to the corner vs the center is small
      const float settleTime = 1.0f;
      if (spot->GetArea()->GetEarliestOccupyTime(m_me->GetTeamNumber()) +
              settleTime <
          m_enemyArriveTime) {
 ...
 */
const HidingSpot *FindInitialEncounterSpot(CBaseEntity *me,
  ...
  // collect set of nearby hiding spots
  if (isSniper) {
    CollectArriveFirstSpotsFunctor collector(me, searchOrigin, enemyArriveTime,
                                             maxRange,
                                             HidingSpot::IDEAL_SNIPER_SPOT);
    ...
    if (collector.m_count) {
      ...
      // no ideal sniping spots, look for "good" sniping spots
      CollectArriveFirstSpotsFunctor collector(me, searchOrigin,
                                               enemyArriveTime, maxRange,
                                               HidingSpot::GOOD_SNIPER_SPOT);
      SearchSurroundingAreas(startArea, searchOrigin, collector, maxRange);
  ...
  // collect hiding spots with decent "cover"
  CollectArriveFirstSpotsFunctor collector(
      me, searchOrigin, enemyArriveTime, maxRange,
