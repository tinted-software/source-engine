...
void CBotManager::StartFrame(void) {
  ...
  for (int i = 1; i <= gpGlobals->maxClients; ++i) {
    ...
    // Hack for now so the temp bot code works. The temp bots are very useful
    ...
    if (player->IsBot() && IsEntityValid(player)) {
      ...
      if (bot) {
        ...
        if (((gpGlobals->tickcount + bot->entindex()) % g_BotUpdateSkipCount) ==
            0) {
          bot->ResetCommand();
          bot->Update();
        }
...
bool CBotManager::IsLineBlockedBySmoke(const Vector &from, const Vector &to,
  ...
  float totalSmokedLength =
      0.0f; // distance along line of sight covered by smoke
  ...
  FOR_EACH_LL(m_activeGrenadeList, it) {
    ActiveGrenade *ag = m_activeGrenadeList[it];
    const float smokeRadiusSq =
        ag->GetRadius() * ag->GetRadius() * grenadeBloat * grenadeBloat;
    ...
    if (ag->IsSmoke()) {
      ...
      // if closest point is within smoke radius, the line overlaps the smoke
      // cloud
      ...
      if (lengthSq < smokeRadiusSq) {
        ...
        if (fromSq < smokeRadiusSq) {
          if (toSq < smokeRadiusSq) {
            ...
            // compute half of total smoked length as if ray crosses entire
            // cloud chord
          ...
          // compute half of total smoked length as if ray crosses entire cloud
          // chord
          float halfSmokedLength = (float)sqrt(smokeRadiusSq - lengthSq);
          ...
          // 'from' and 'to' lie outside of the cloud - the line of sight
          // completely crosses it determine the length of the chord that
          // crosses the cloud
...
void CBotManager::AddDebugMessage(const char *msg) {
  ...
  Q_strncpy(m_debugMessage[m_currentDebugMessage].m_string, msg,
            MAX_DBG_MSG_SIZE);
