...
/// @todo Remove this nasty hack - CreateFakeClient() calls CBot::Spawn, which
/// needs the profile
...
template <class T> T *CreateBot(const BotProfile *profile, int team) {
  ...
  // This is a backdoor we use so when the engine calls ClientPutInServer (from
  // CreateFakeClient), expecting the game to make an entity for the fake
  // client, we can make our special bot class instead of a CCSPlayer.
 ...
 * A template is needed here because the CBot class must be derived from
 * CBasePlayer, but also may need to be derived from a more specific player
 * class, such as CCSPlayer
 */
template <class PlayerType> class CBot : public PlayerType {
public:
  ...
  virtual void
  Upkeep(void) = 0; ///< lightweight maintenance, invoked frequently
  ...
  void PushPostureContext(
      void); ///< push the current posture context onto the top of the stack
  void PopPostureContext(
      void); ///< restore the posture context to the next context on the stack
...
#define MUST_JUMP true
  ...
  float
  GetActiveWeaponAmmoRatio(void) const; ///< returns ratio of ammo left to max
                                        ///< ammo (1 = full clip, 0 = empty)
  bool IsActiveWeaponClipEmpty(
      void) const; ///< return true if active weapon has any empty clip
  bool IsActiveWeaponOutOfAmmo(
      void) const; ///< return true if active weapon has no ammo at all
  bool IsActiveWeaponRecoilHigh(
      void) const; ///< return true if active weapon's bullet spray has become
                   ///< large and inaccurate
  ...
  bool IsEnemy(
      CBaseEntity *ent) const; ///< returns TRUE if given entity is our enemy
  ...
  bool IsPlayerFacingMe(
      CBasePlayer *enemy) const; ///< return true if player is facing towards us
  bool IsPlayerLookingAtMe(CBasePlayer *enemy, float cosTolerance = 0.9f)
      const; ///< returns true if other player is pointing right at us
  bool IsLookingAtPosition(const Vector &pos, float angleTolerance = 20.0f)
      const; ///< returns true if looking (roughly) at given position

  bool IsLocalPlayerWatchingMe(
      void) const; ///< return true if local player is observing this bot

  void PrintIfWatched(PRINTF_FORMAT_STRING const char *format,
                      ...) const; ///< output message to console if we are being
                                  ///< watched by the local player

  virtual void UpdatePlayer(
      void); ///< update player physics, movement, weapon firing commands, etc
  ...
  virtual bool
  ClientCommand(const CCommand &args); ///< Do a "client command" - useful for
                                       ///< invoking menu choices, etc.
  virtual int
  Cmd_Argc(void); ///< Returns the number of tokens in the command string
...
private:
  ...
  Vector m_viewForward; ///< forward view direction (only valid when
                        ///< GetViewVector() is used)

  /// the PostureContext represents the current settings of walking and
  /// crouching
...
//--------------------------------------------------------------------------------------------------------------
template <class T> inline void CBot<T>::SetModel(const char *modelName) {
  BaseClass::SetModel(modelName);
}

//-----------------------------------------------------------------------------------------------------------
template <class T> inline float CBot<T>::GetMoveSpeed(void) {
  return this->MaxSpeed();
}

//-----------------------------------------------------------------------------------------------------------
template <class T> inline void CBot<T>::Run(void) { m_isRunning = true; }

//-----------------------------------------------------------------------------------------------------------
template <class T> inline void CBot<T>::Walk(void) { m_isRunning = false; }

//-----------------------------------------------------------------------------------------------------------
template <class T> inline bool CBot<T>::IsActiveWeaponRecoilHigh(void) const {
...
//-----------------------------------------------------------------------------------------------------------
template <class T> inline void CBot<T>::PushPostureContext(void) {
...
//-----------------------------------------------------------------------------------------------------------
template <class T> inline void CBot<T>::PopPostureContext(void) {
...
//-----------------------------------------------------------------------------------------------------------
template <class T>
inline bool CBot<T>::IsPlayerLookingAtMe(CBasePlayer *other,
                                         float cosTolerance) const {
  Vector toOther = other->GetAbsOrigin() - this->GetAbsOrigin();
...
//-----------------------------------------------------------------------------------------------------------
template <class T> inline const Vector &CBot<T>::GetViewVector(void) {
...
//-----------------------------------------------------------------------------------------------------------
template <class T>
inline bool CBot<T>::IsLookingAtPosition(const Vector &pos,
                                         float angleTolerance) const {
  // forced to do this since many methods in CBaseEntity are not const, but
  // should be
...
//--------------------------------------------------------------------------------------------------------------
template <class PlayerType> inline void CBot<PlayerType>::Spawn(void) {
...
{
float g_flBotFullThinkInterval	= 1.0 / 15.0;	// full AI at lower frequency
(was 10 in GoldSrc)
        ...
        {
                m_flNextFullBotThink = gpGlobals->curtime +
g_flBotFullThinkInterval;
...
//--------------------------------------------------------------------------------------------------------------
template <class PlayerType> inline void CBot<PlayerType>::MoveForward(void) {
...
//--------------------------------------------------------------------------------------------------------------
template <class PlayerType> inline void CBot<PlayerType>::MoveBackward(void) {
  m_forwardSpeed = -GetMoveSpeed();
...
//--------------------------------------------------------------------------------------------------------------
template <class PlayerType> inline void CBot<PlayerType>::StrafeLeft(void) {
...
//--------------------------------------------------------------------------------------------------------------
template <class PlayerType> inline void CBot<PlayerType>::StrafeRight(void) {
  m_strafeSpeed = GetMoveSpeed();
...
//--------------------------------------------------------------------------------------------------------------
template <class PlayerType> inline bool CBot<PlayerType>::Jump(bool mustJump) {
 ...
 */
template <class PlayerType> void CBot<PlayerType>::ClearMovement(void) {
  m_forwardSpeed = 0.0;
  m_strafeSpeed = 0.0;
  m_verticalSpeed =
      100.0; // stay at the top of water, so we don't drown.  TODO: swim logic
 ...
 */
template <class PlayerType> inline bool CBot<PlayerType>::IsJumping(void) {
...
//--------------------------------------------------------------------------------------------------------------
template <class PlayerType> inline void CBot<PlayerType>::Crouch(void) {
  m_isCrouching = true;
}

//--------------------------------------------------------------------------------------------------------------
template <class PlayerType> inline void CBot<PlayerType>::StandUp(void) {
  m_isCrouching = false;
}

//--------------------------------------------------------------------------------------------------------------
template <class PlayerType> inline void CBot<PlayerType>::UseEnvironment(void) {
  SETBITS(m_buttonFlags, IN_USE);
}

//--------------------------------------------------------------------------------------------------------------
template <class PlayerType> inline void CBot<PlayerType>::PrimaryAttack(void) {
...
//--------------------------------------------------------------------------------------------------------------
template <class PlayerType> inline void CBot<PlayerType>::Reload(void) {
 ...
 */
template <class PlayerType> inline bool CBot<PlayerType>::IsUsingScope(void) {
  // if our field of view is less than 90, we're looking thru a scope (maybe
  // only true for CS...)
...
template <class PlayerType>
inline void
CBot<PlayerType>::BuildUserCmd(CUserCmd &cmd, const QAngle &viewangles,
                               float forwardmove, float sidemove, float upmove,
                               int buttons, byte impulse) {
 ...
 */
template <class PlayerType> inline void CBot<PlayerType>::UpdatePlayer(void) {
  if (m_isCrouching) {
  ...
  // Fill in a CUserCmd with our data
  this->BuildUserCmd(m_userCmd, this->EyeAngles(), m_forwardSpeed,
                     m_strafeSpeed, m_verticalSpeed, m_buttonFlags, 0);
...
//--------------------------------------------------------------------------------------------------------------
template <class PlayerType> inline void CBot<PlayerType>::ResetCommand(void) {
  m_forwardSpeed = 0.0;
  m_strafeSpeed = 0.0;
  m_verticalSpeed =
      100.0; // stay at the top of water, so we don't drown.  TODO: swim logic
...
inline byte CBot< PlayerType >::ThrottledMsec( void ) const
{
        ...
        // Estimate Msec to use for this command based on time passed from the
previous command iNewMsec = (int)( (gpGlobals->curtime -
m_flPreviousCommandTime) * 1000 ); if (iNewMsec > 255)		// Doh, bots are
going to be slower than they should if this happens. iNewMsec = 255;
 ...
 */
template <class PlayerType> inline int CBot<PlayerType>::Cmd_Argc() {
  return m_args.Count();
}
 ...
 */
template <class PlayerType> inline char *CBot<PlayerType>::Cmd_Argv(int argc) {
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Return true if the local player is currently in observer mode watching this
 * bot.
 */
template <class PlayerType>
...
template <class PlayerType>
inline void
CBot<PlayerType>::PrintIfWatched(PRINTF_FORMAT_STRING const char *format,
                                 ...) const {
  ...
  if ((IsLocalPlayerWatchingMe() &&
       (cv_bot_debug.GetInt() == 1 || cv_bot_debug.GetInt() == 3)) ||
    ...
    // prefix the console message with the bot's name (this can be NULL if bot
    // was just added)
    ClientPrint(
        UTIL_GetListenServerHost(), HUD_PRINTCONSOLE,
...
extern bool IsSpotOccupied(
    CBaseEntity *me,
    const Vector &pos); // if a player is at the given spot, return true
extern const Vector *FindNearbyHidingSpot(CBaseEntity *me, const Vector &pos,
                                          float maxRange = 1000.0f,
                                          bool isSniper = false,
                                          bool useNearest = false);
extern const Vector *FindRandomHidingSpot(CBaseEntity *me, Place place,
                                          bool isSniper = false);
extern const Vector *FindNearbyRetreatSpot(CBaseEntity *me, const Vector &start,
                                           float maxRange = 1000.0f,
                                           int avoidTeam = 0);
