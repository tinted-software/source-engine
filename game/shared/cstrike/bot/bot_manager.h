...
#pragma warning(disable : 4530) // STL uses exceptions, but we are not compiling
                                // with them - ignore warning
...
public:
  ...
  CBasePlayer *AllocateAndBindBotEntity(
      edict_t *ed); ///< allocate the appropriate entity for the bot and bind it
                    ///< to the given edict
  virtual CBasePlayer *
  AllocateBotEntity(void) = 0; ///< factory method to allocate the appropriate
                               ///< entity for the bot
  ...
  void RemoveGrenade(
      CBaseGrenade *grenade); ///< the grenade entity in the world is going away
  void SetGrenadeRadius(CBaseGrenade *grenade,
                        float radius); ///< the radius of the grenade entity (or
                                       ///< associated smoke cloud)
  void ValidateActiveGrenades(void);   ///< destroy any invalid active grenades
  void DestroyAllGrenades(void);
  bool IsLineBlockedBySmoke(
      const Vector &from, const Vector &to,
      float grenadeBloat =
          1.0f); ///< return true if line intersects smoke volume, with grenade
  ...
  //
  template <typename T> bool ForEachGrenade(T &func) {
...
inline const CBotManager::DebugMessage *
CBotManager::GetDebugMessage(int which) const {
