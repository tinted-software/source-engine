...
extern bool UTIL_IsNameTaken(
    const char *name,
    bool ignoreHumans = false); ///< return true if given name is already in use
                                ///< by another player
...
extern void UTIL_DrawBeamFromEnt(int iIndex, Vector vecEnd, int iLifetime,
                                 byte bRed, byte bGreen, byte bBlue);
extern void UTIL_DrawBeamPoints(Vector vecStart, Vector vecEnd, int iLifetime,
                                byte bRed, byte bGreen, byte bBlue);
extern CBasePlayer *UTIL_GetClosestPlayer(const Vector &pos,
                                          float *distance = NULL);
extern CBasePlayer *UTIL_GetClosestPlayer(const Vector &pos, int team,
                                          float *distance = NULL);
extern bool
UTIL_KickBotFromTeam(int kickTeam); ///< kick a bot from the given team. If no
                                    ///< bot exists on the team, return false.

extern bool
UTIL_IsVisibleToTeam(const Vector &spot,
                     int team); ///< return true if anyone on the given team can
                                ///< see the given spot

/// return true if moving from "start" to "finish" will cross a player's line of
/// fire.
extern bool IsCrossingLineOfFire(const Vector &start, const Vector &finish,
                                 CBaseEntity *ignore = NULL,
                                 int ignoreTeam = 0);

extern void UTIL_ConstructBotNetName(
    char *name, int nameLength,
    const BotProfile *bot); ///< constructs a complete name including prefix

/**
 * Echos text to the console, and prints it on the client's screen.  This is NOT
 * tied to the developer cvar. If you are adding debugging output in cstrike,
 * use UTIL_DPrintf() (debug.h) instead.
 */
...
bool WildcardMatch(
    const char *query,
    const char *test); ///< Performs a simple case-insensitive string
                       ///< comparison, honoring trailing * wildcards
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Given two line segments: startA to endA, and startB to endB, return true if
 * they intesect and put the intersection point in "result". Note that this
 * computes the intersection of the 2D (x,y) projection of the line segments.
 */
inline bool IsIntersecting2D(const Vector &startA, const Vector &endA,
                             const Vector &startB, const Vector &endB,
                             Vector *result = NULL) {
  float denom = (endA.x - startA.x) * (endB.y - startB.y) -
  ...
  float numS = (startA.y - startB.y) * (endB.x - startB.x) -
               (startA.x - startB.x) * (endB.y - startB.y);
  if (numS == 0.0f) {
    // coincident
    return true;
  }

  float numT = (startA.y - startB.y) * (endA.x - startA.x) -
               (startA.x - startB.x) * (endA.y - startA.y);
