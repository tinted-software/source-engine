...
bool UTIL_IsNameTaken(const char *name, bool ignoreHumans) {
  for (int i = 1; i <= gpGlobals->maxClients; ++i) {
    ...
    if (player->IsPlayer() && player->IsBot()) {
      ...
      if (bot && bot->GetProfile()->GetName() &&
          FStrEq(name, bot->GetProfile()->GetName())) {
        return true;
      }
    } else {
      if (!ignoreHumans) {
 ...
 * If 'distance' is non-NULL, the distance to the closest player is returned in
 * it.
 */
...
//--------------------------------------------------------------------------------------------------------------
// Takes the bot pointer and constructs the net name using the current bot name
// prefix.
void UTIL_ConstructBotNetName(char *name, int nameLength,
  ...
  if ((cv_bot_prefix.GetString() == NULL) ||
      (strlen(cv_bot_prefix.GetString()) == 0)) {
...
bool UTIL_IsVisibleToTeam(const Vector &spot, int team) {
  for (int i = 1; i <= gpGlobals->maxClients; ++i) {
    ...
    trace_t result;
    UTIL_TraceLine(player->EyePosition(), spot, CONTENTS_SOLID, player,
...
//------------------------------------------------------------------------------------------------------------
void UTIL_DrawBeamFromEnt(int i, Vector vecEnd, int iLifetime, byte bRed,
                          byte bGreen, byte bBlue) {
  /* BOTPORT: What is the replacement for MESSAGE_BEGIN?
          MESSAGE_BEGIN( MSG_PVS, SVC_TEMPENTITY, vecEnd );   // vecEnd =
     origin??? WRITE_BYTE( TE_BEAMENTPOINT ); WRITE_SHORT( i ); WRITE_COORD(
     vecEnd.x ); WRITE_COORD( vecEnd.y ); WRITE_COORD( vecEnd.z ); WRITE_SHORT(
     s_iBeamSprite ); WRITE_BYTE( 0 );		 // startframe WRITE_BYTE( 0 );
     // framerate WRITE_BYTE( iLifetime ); // life WRITE_BYTE( 10 );
     // width WRITE_BYTE( 0 );		 // noise WRITE_BYTE( bRed );
     // r, g, b WRITE_BYTE( bGreen );		 // r, g, b WRITE_BYTE( bBlue );
     // r, g, b WRITE_BYTE( 255 );	 // brightness WRITE_BYTE( 0 );
     // speed MESSAGE_END();
                                          */
}

//------------------------------------------------------------------------------------------------------------
void UTIL_DrawBeamPoints(Vector vecStart, Vector vecEnd, int iLifetime,
 ...
 * fire. The path from "start" to "finish" is assumed to be a straight line.
 * "start" and "finish" are assumed to be points on the ground.
 */
bool IsCrossingLineOfFire(const Vector &start, const Vector &finish,
                          CBaseEntity *ignore, int ignoreTeam) {
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Performs a simple case-insensitive string comparison, honoring trailing *
 * wildcards
 */
