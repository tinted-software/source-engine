...
public:
  ...
  bool IsDifficulty(
      BotDifficultyType diff) const; ///< return true if this profile can be
                                     ///< used for the given difficulty level
...
private:
  ...
  void Inherit(const BotProfile *parent,
               const BotProfile *baseline); ///< copy values from parent if they
...
public:
  ...
  const BotProfile *
  GetProfileMatchingTemplate(const char *profileName, int team,
                             BotDifficultyType difficulty) const {
    FOR_EACH_LL(m_profileList, it) {
      BotProfile *profile = m_profileList[it];
  ...
  const char *
  GetCustomSkin(int index); ///< Returns custom skin name at a particular index
  ...
  int FindVoiceBankIndex(
      const char *filename); ///< return index of the (custom) bot phrase db,
