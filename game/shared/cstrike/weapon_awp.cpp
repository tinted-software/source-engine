...
#ifdef AWP_UNZOOM
ConVar sv_awpunzoomdelay("sv_awpunzoomdelay", "1.0", 0,
...
void CWeaponAWP::SecondaryAttack() {
...
#ifndef CLIENT_DLL
  // If this isn't guarded, the sound will be emitted twice, once by the server
  // and once by the client. Let the server play it since if only the client
  // plays it, it's liable to get played twice cause of a prediction error. joy.

  //=============================================================================
  // HPE_BEGIN:
  // [tj] Playing this from the player so that we don't try to play the sound
  // outside the level.
  //=============================================================================
  if (GetPlayerOwner()) {
  ...
  m_zoomFullyActiveTime =
      gpGlobals->curtime + 0.15; // The worst zoom time from above.
}
...
float CWeaponAWP::GetInaccuracy() const {
  if (weapon_accuracy_model.GetInt() == 1) {
    ...
    // If we are not zoomed in, or we have very recently zoomed and are still
    // transitioning, the bullet diverts more.
    if (pPlayer->GetFOV() == pPlayer->GetDefaultFOV() ||
        (gpGlobals->curtime < m_zoomFullyActiveTime)) {
      fSpread += 0.08f;
    }
...
#ifdef AWP_UNZOOM
    SetContextThink(&CWeaponAWP::UnzoomThink,
                    gpGlobals->curtime + sv_awpunzoomdelay.GetFloat(),
                    SNIPER_ZOOM_CONTEXT);
#else
...
bool CWeaponAWP::IsAwp() const { return true; }
...
bool CWeaponAWP::Deploy() {
  ...
  m_flNextSecondaryAttack =
      MAX(m_flNextSecondaryAttack, fOldNextSecondaryAttack);
  return true;
}
