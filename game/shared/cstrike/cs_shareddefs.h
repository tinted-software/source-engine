...
template <class T> class CUtlVectorInitialized : public CUtlVector<T> {
public:
  CUtlVectorInitialized(T *pMemory, int numElements)
...
// These go in CCSPlayer::m_iAddonBits and get sent to the client so it can
// create grenade models hanging off players.
...
#define MAX_CLAN_TAG_LENGTH                                                    \
  16 // max for new tags is actually 12, this allows some backward compat.

//=============================================================================
// HPE_BEGIN:
// [menglish] CS specific death animation time now that freeze cam is
// implemented
...
enum CSPlayerState {
  ...
  STATE_DEATH_WAIT_FOR_KEY, // Done playing death anim. Waiting for keypress to
