...
#else
RecvPropBool(RECVINFO(m_bRedraw)), RecvPropBool(RECVINFO(m_bPinPulled)),
...
#ifndef CLIENT_DLL
ConVar sv_ignoregrenaderadio("sv_ignoregrenaderadio", "0", 0,
...
// Purpose:
//-----------------------------------------------------------------------------
void CBaseCSGrenade::Precache() { BaseClass::Precache(); }
...
bool CBaseCSGrenade::Holster(CBaseCombatWeapon *pSwitchingTo) {
  m_bRedraw = false;
  m_bPinPulled =
      false; // when this is holstered make sure the pin isn’t pulled.
  m_fThrowTime = 0;
...
void CBaseCSGrenade::PrimaryAttack() {
  ...
  // The pull pin animation has to finish, then we wait until they aren't
  // holding the primary attack button, then throw the grenade.
...
void CBaseCSGrenade::ItemPostFrame() {
  ...
  if (m_bPinPulled && !(pPlayer->m_nButtons & IN_ATTACK)) {
    ...
    m_flNextPrimaryAttack = gpGlobals->curtime +
                            SequenceDuration(); // we're still throwing, so
                                                // reset our next primary attack
...
void CBaseCSGrenade::ThrowGrenade() {
  ...
  if (angThrow.x < 0) {
    angThrow.x +=
        360; // make sure we have a positive angle from LocalEyeAngles()
  }
  ...
  // We want to throw the grenade from 16 units out.  But that can cause
  // problems if we're facing a thin wall.  Do a hull trace to be safe.
  trace_t trace;
  Vector mins(-2, -2, -2);
  Vector maxs(2, 2, 2);
  UTIL_TraceHull(vecSrc, vecSrc + vForward * 16, mins, maxs, MASK_SOLID,
  ...
  if (pCSPlayer) {
    if (!sv_ignoregrenaderadio.GetBool()) {
      pCSPlayer->Radio("Radio.FireInTheHole",
...
void CBaseCSGrenade::DropGrenade() {
  ...
  Vector vecSrc =
      pPlayer->GetAbsOrigin() + pPlayer->GetViewOffset() + vForward * 16;

  Vector vecVel = pPlayer->GetAbsVelocity();

  EmitGrenade(vecSrc, vec3_angle, vecVel,
              AngularImpulse(600, random->RandomInt(-1200, 1200), 0), pPlayer);
...
void CBaseCSGrenade::EmitGrenade(Vector vecSrc, QAngle vecAngles, Vector vecVel,
                                 AngularImpulse angImpulse,
                                 CBasePlayer *pPlayer) {
  Assert(0 && "CBaseCSGrenade::EmitGrenade should not be called. Make sure to "
              "implement this in your subclass!\n");
}

bool CBaseCSGrenade::AllowsAutoSwitchFrom(void) const { return !m_bPinPulled; }
