...
template <int BitLength> class BitArray {
...
//=============================================================================
// HPE_BEGIN:
// [tj] A couple variations on the RoundStats structure to handle extra
// operations
...
struct RoundStatsDirectAverage_t {
  ...
  RoundStatsDirectAverage_t() { Reset(); }
...
struct RoundStatsRollingAverage_t {
  ...
  RoundStatsRollingAverage_t() { Reset(); }
  ...
  operator+=(const RoundStatsRollingAverage_t &other) {
    for (int i = 0; i < ARRAYSIZE(m_fStat); i++) {
...
#define END_STAT_TABLE()                                                       \
  pKV->SetUint64(::BaseStatData::m_bUseGlobalData ? "TimeSubmitted"            \
                                                  : "SessionTime",             \
                 ::BaseStatData::TimeSubmitted);                               \
...
template <typename T> class UniqueStatID_t {
public:
  static unsigned GetNext(void) { return ++s_nLastID; }
...
template <typename T> unsigned GetUniqueIDForStatTable(const T &table) {
...
public:
  ...
  //-----------------------------------------------------------------------------
  template <typename T> void SubmitStat(T &stat) {
  ...
  //-----------------------------------------------------------------------------
  template <typename T> void SubmitStat(T *pStat) {
...
protected:
  ...
  //-----------------------------------------------------------------------------
  // Used to get a list of all stats containers being tracked by the deriving
  // class
  //-----------------------------------------------------------------------------
...
private:
  ...
  public:
    ...
    virtual void SendData(KeyValues *pKV) {
      ...
      for (int i = 0; i < this->m_Size; ++i) {
        ...
        KeyValues *pKVCopy = this->operator[](i)->m_bUseGlobalData
                                 ? pKV->MakeCopy()
                                 : new KeyValues("");
    ...
    //-----------------------------------------------------------------------------
    virtual void Clear(void) { this->PurgeAndDeleteElements(); }
  ...
  //-----------------------------------------------------------------------------
  template <typename T> CGameStatList<T> *GetStatTable(void) {
