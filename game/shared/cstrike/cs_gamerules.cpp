...
/**
 * Player hull & eye position for standing, ducking, etc.  This version has a
 * taller player height, but goldsrc-compatible collision bounds.
 */
...
RecvPropDataTable("cs_gamerules_data", 0, 0,
                  &REFERENCE_RECV_TABLE(DT_CSGameRules), RecvProxy_CSGameRules)
    END_RECV_TABLE()
#else
void *SendProxy_CSGameRules(const SendProp *pProp, const void *pStructBase,
                            const void *pData,
...
SendPropDataTable("cs_gamerules_data", 0, &REFERENCE_SEND_TABLE(DT_CSGameRules),
                  SendProxy_CSGameRules) END_SEND_TABLE()
#endif
...
// ConVar mp_dynamicpricing( "mp_dynamicpricing", "0", FCVAR_REPLICATED,
// "Enables or Disables the dynamic weapon prices" );

extern ConVar sv_stopspeed;

ConVar
    mp_buytime("mp_buytime", "1.5", FCVAR_REPLICATED,
               "How many minutes after round start players can buy items for.",
               true, 0.25, false, 0);

ConVar mp_playerid("mp_playerid", "0", FCVAR_REPLICATED,
                   "Controls what information player see in the status bar: 0 "
                   "all names; 1 team names; 2 no names",
                   true, 0, true, 2);

ConVar mp_playerid_delay(
    "mp_playerid_delay", "0.5", FCVAR_REPLICATED,
    "Number of seconds to delay showing information in the status bar", true, 0,
    true, 1);

ConVar mp_playerid_hold(
    "mp_playerid_hold", "0.25", FCVAR_REPLICATED,
    "Number of seconds to keep showing old information in the status bar", true,
    0, true, 1);

ConVar mp_round_restart_delay(
    "mp_round_restart_delay", "5.0", FCVAR_REPLICATED,
    "Number of seconds to delay before restarting a round after a win", true,
    0.0f, true, 10.0f);

ConVar sv_allowminmodels(
    "sv_allowminmodels", "1", FCVAR_REPLICATED | FCVAR_NOTIFY,
...
ConVar cl_autohelp("cl_autohelp", "1", FCVAR_ARCHIVE | FCVAR_USERINFO,
                   "Auto-help");
...
#define CS_DAMAGE_FOR_FALL_SPEED                                               \
  ((float)100 /                                                                \
   (CS_PLAYER_FATAL_FALL_SPEED -                                               \
    CS_PLAYER_MAX_SAFE_FALL_SPEED)) // damage per unit per second.

// These entities are preserved each round restart. The rest are removed and
// recreated.
...
ConVar mp_startmoney("mp_startmoney", "800", FCVAR_REPLICATED | FCVAR_NOTIFY,
                     "amount of money each player gets when they reset", true,
                     800, true, 16000);

ConVar mp_roundtime("mp_roundtime", "2.5", FCVAR_REPLICATED | FCVAR_NOTIFY,
                    "How many minutes each round takes.", true, 1, // min value
                    true, 9                                        // max value
);

ConVar mp_freezetime(
    "mp_freezetime", "6", FCVAR_REPLICATED | FCVAR_NOTIFY,
    "how many seconds to keep players frozen when the round starts", true,
    0,       // min value
    true, 60 // max value
);

ConVar mp_c4timer("mp_c4timer", "45", FCVAR_REPLICATED | FCVAR_NOTIFY,
                  "how long from when the C4 is armed until it blows", true,
                  10,      // min value
                  true, 90 // max value
);

ConVar mp_limitteams(
    "mp_limitteams", "2", FCVAR_REPLICATED | FCVAR_NOTIFY,
    "Max # of players 1 team can have over another (0 disables check)", true,
    0,       // min value
    true, 30 // max value
);

ConVar
    mp_tkpunish("mp_tkpunish", "0", FCVAR_REPLICATED,
                "Will a TK'er be punished in the next round?  {0=no,  1=yes}");

ConVar mp_autokick("mp_autokick", "1", FCVAR_REPLICATED,
                   "Kick idle/team-killing players");

ConVar mp_spawnprotectiontime(
    "mp_spawnprotectiontime", "5", FCVAR_REPLICATED,
    "Kick players who team-kill within this many seconds of a round restart.");

ConVar mp_humanteam("mp_humanteam", "any", FCVAR_REPLICATED,
                    "Restricts human players to a single team {any, CT, T}");

ConVar mp_ignore_round_win_conditions(
    "mp_ignore_round_win_conditions", "0", FCVAR_REPLICATED,
    "Ignore conditions which would end the current round");

ConCommand EndRound("endround", &CCSGameRules::EndRound,
                    "End the current round.", FCVAR_CHEAT);
...
// entity.
//			Given an origin to start looking from and a minimum
//radius to place the entity at, 			it will sweep out a circle around vOrigin and
//try to find a valid spot (on the ground) 			where mins and maxs will fit.
...
int UTIL_HumansInGame(bool ignoreSpectators) {
  ...
  for (int i = 1; i <= gpGlobals->maxClients; i++) {
    ...
    if (entity && !FNullEnt(entity->edict())) {
      ...
      if (ignoreSpectators && entity->GetTeamNumber() != TEAM_TERRORIST &&
          entity->GetTeamNumber() != TEAM_CT)
        continue;
...
CCSGameRules::CCSGameRules() {
  ...
  if (filesystem->FileExists(
          UTIL_VarArgs("maps/cfg/%s.cfg", STRING(gpGlobals->mapname)))) {
    // Execute a map specific cfg file - as in Day of Defeat
    // Map names cannot contain quotes or control characters so this is safe but
    // silly that we have to do it.
    engine->ServerCommand(
        UTIL_VarArgs("exec \"%s.cfg\" */maps\n", STRING(gpGlobals->mapname)));
  ...
  if (g_flGameStatsUpdateTime == 0.0f) {
    ...
    g_flGameStatsUpdateTime =
        CS_GAME_STATS_UPDATE; // Next update is between 22 and 24 hours.
  }
#endif
}
...
void CCSGameRules::AddPricesToTable(weeklyprice_t prices) {
  ...
  if (iIndex == INVALID_STRING_INDEX) {
    m_StringTableBlackMarket->AddString(CBaseEntity::IsServer(),
                                        "blackmarket_prices",
                                        sizeof(weeklyprice_t), &prices);
  } else {
    m_StringTableBlackMarket->SetStringUserData(iIndex, sizeof(weeklyprice_t),
                                                &prices);
  }
...
//-----------------------------------------------------------------------------
void CCSGameRules::ClientCommandKeyValues(edict_t *pEntity,
                                          KeyValues *pKeyValues) {
  CCSPlayer *pPlayer =
      dynamic_cast<CCSPlayer *>(CBaseEntity::Instance(pEntity));
  if (pPlayer) {
    ...
    if (pszCommand && pszCommand[0]) {
      if (FStrEq(pszCommand, "ClanTagChanged")) {
        ...
        UTIL_LogPrintf(
            "\"%s<%i><%s><%s>\" triggered \"clantag\" (value \"%s\")\n",
            pPlayer->GetPlayerName(), pPlayer->GetUserID(),
            pPlayer->GetNetworkIDString(), teamName,
...
void CCSGameRules::PlayerSpawn(CBasePlayer *pBasePlayer) {
  ...
  while ((pWeaponEntity = gEntList.FindEntityByClassname(
    ...
    if (addDefault) {
      // remove all our weapons and armor before touching the first
      // game_player_equip
...
float CCSGameRules::GetExplosionDamageAdjustment(Vector &vecSrc, Vector &vecEnd,
  ...
  if (tr.fraction == 1.0) {
    ...
    if (tr.fraction == 1.0) {
      if ((blockingEntity != NULL) &&
        ...
        retval = 0.75; // we're blocked by something that isn't an entity with a
                       // physics module or world geometry, just cut damage in
                       // half for now.
      }
    }
  }
...
float CCSGameRules::GetAmountOfEntityVisible(Vector &vecSrc,
                                             CBaseEntity *entity) {
  float retval = 0.0;
  ...
  // check what parts of the player we can see from this point and modify the
  ...
  chestHeightFromFeet =
      vecChest.z - vecFeet.z; // compute the distance from the chest to the
                              // feet. (this accounts for ducking and the like)
  ...
  float damageAdjustment =
      GetExplosionDamageAdjustment(vecSrc, vecChest, entity);
...
void CCSGameRules::RadiusDamage(const CTakeDamageInfo &info,
...
void CCSGameRules::RadiusDamage(const CTakeDamageInfo &info,
                                const Vector &vecSrcIn, float flRadius,
                                int iClassIgnore, bool bIgnoreWorld) {
  ...
  bool isGrenade = pInflictor && V_strcmp(pInflictor->GetClassname(),
                                          "hegrenade_projectile") == 0;
  bool isBomb =
      pInflictor && V_strcmp(pInflictor->GetClassname(), "planted_c4") == 0;
  ...
  for (CEntitySphereQuery sphere(vecSrc, flRadius);
    ...
    //=============================================================================
    // HPE_BEGIN:
    // [tj] We have to save whether or not the player is killed so we don't give
    // credit
    ...
    if (pEntity->m_takedamage != DAMAGE_NO) {
      ...
      if (bHit) {
        ...
        // use a Gaussian function to describe the damage falloff over distance,
        // with flRadius equal to 3 * sigma this results in the following
        // values:
        //
        ...
        float fSigma =
            flRadius / 3.0f; // flRadius specifies 3rd standard deviation
                             // (0.0111 damage at this range)
        float fGaussianFalloff = exp(-fDist * fDist / (2.0f * fSigma * fSigma));
        float flAdjustedDamage =
            info.GetDamage() * fGaussianFalloff * damagePercentage;
        ...
        if (flAdjustedDamage > 0) {
          ...
          UTIL_TraceLine(vecSrc, vecTarget, MASK_SHOT, NULL,
                         COLLISION_GROUP_NONE, &tr);
          ...
          if (pCSExplosionVictim && pCSExplosionAttacker && isGrenade) {
            CCS_GameStats.IncrementStat(
                pCSExplosionAttacker, CSSTAT_GRENADE_DAMAGE,
    ...
    if (pCSExplosionVictim) {
      // If the bomb is exploding, set the attacker to the planter (we can't put
      // this in the CTakeDamageInfo, since players aren't supposed to get
      // credit for bomb kills)
      if (isBomb) {
      ...
      if (pCSExplosionAttacker && !pCSExplosionVictim->IsAlive() &&
          wasAliveBeforeExplosion &&
          pCSExplosionVictim->GetTeamNumber() !=
              pCSExplosionAttacker->GetTeamNumber()) {
  ...
  //=============================================================================
  // HPE_BEGIN:
  // [tj] //Depending on which type of explosion it was, award the appropriate
  // achievement.
  //=============================================================================

  if (pCSExplosionAttacker && isGrenade &&
      numberOfEnemyPlayersKilledByThisExplosion >=
          AchievementConsts::GrenadeMultiKill_MinKills) {
    pCSExplosionAttacker->AwardAchievement(CSGrenadeMultikill);
    pCSExplosionAttacker->CheckMaxGrenadeKills(
        numberOfEnemyPlayersKilledByThisExplosion);
  }
...
//			*pInflictor -
//-----------------------------------------------------------------------------
void CCSGameRules::DeathNotice(CBasePlayer *pVictim,
  ...
  if (pScorer) // Is the killer a client?
  {
    ...
    if (pInflictor) {
      if (pInflictor == pScorer) {
        // If the inflictor is the killer,  then it must be their current weapon
        // doing the damage
        if (pScorer->GetActiveWeapon()) {
          killer_weapon_name = pScorer->GetActiveWeapon()
                                   ->GetClassname(); // GetDeathNoticeName();
        }
      } else {
        killer_weapon_name =
  ...
  } else if (strncmp(killer_weapon_name, "hegrenade", 9) ==
             0) //"hegrenade_projectile"
  {
    killer_weapon_name = "hegrenade";
  } else if (strncmp(killer_weapon_name, "flashbang", 9) ==
  ...
  if (event) {
    ...
    event->SetInt("priority",
                  bHeadshot ? 8 : 7); // HLTV event priority, not transmitted
...
//=========================================================
//=========================================================
void CCSGameRules::PlayerKilled(CBasePlayer *pVictim,
                                const CTakeDamageInfo &info) {
  CBaseEntity *pInflictor = info.GetInflictor();
  CBaseEntity *pKiller = info.GetAttacker();
  CBasePlayer *pScorer = GetDeathScorer(pKiller, pInflictor);
  ...
  // Specifically, we shouldn't lose any points or show death notices, to match
  // goldsrc
  if (Q_strcmp(pKiller->GetClassname(), "planted_c4") == 0) {
    // dvsents2: uncomment when removing all FireTargets
    // variant_t value;
    // g_EventQueue.AddEvent( "game_playerdie", "Use", value, 0, pVictim,
    // pVictim );
  ...
  if (IPointsForKill(pScorer, pVictim) < 0) {
    ...
    if (mp_autokick.GetBool()) {
      ...
      Q_snprintf(strTeamKills, sizeof(strTeamKills), "%d",
                 pCSScorer->m_iTeamKills);
      ...
      if (pCSScorer->m_iTeamKills >= 3) {
        ClientPrint(pCSScorer, HUD_PRINTCONSOLE,
                    "#Banned_For_Killing_Teammates");
        engine->ServerCommand(
            UTIL_VarArgs("kickid %d\n", pCSScorer->GetUserID()));
      } else if (mp_spawnprotectiontime.GetInt() > 0 &&
                 GetRoundElapsedTime() < mp_spawnprotectiontime.GetInt()) {
        ClientPrint(pCSScorer, HUD_PRINTCONSOLE,
                    "#Banned_For_Killing_Teammates");
        engine->ServerCommand(
            UTIL_VarArgs("kickid %d\n", pCSScorer->GetUserID()));
...
//-----------------------------------------------------------------------------
bool CCSGameRules::FShouldSwitchWeapon(CBasePlayer *pPlayer,
...
void CCSGameRules::LevelInitPostEntity() {
  ...
  m_bLevelInitialized =
      false; // re-count CT and T start spots now that they exist

  // Figure out from the entities in the map what kind of map this is (bomb run,
  // prison escape, etc).
  CheckMapConditions();
}
...
void CCSGameRules::CreateCustomNetworkStringTables(void) {
  ...
  if (0) // mp_dynamicpricing.GetBool() )
  {
    ...
    if (m_bBlackMarket == false) {
      Msg("ERROR: mp_dynamicpricing set to 1 but couldn't download the price "
          "list!\n");
    }
  } else {
    m_bBlackMarket = false;
 ...
 * Return true if the scenario is over, false if the scenario is still in
 * progress
 */
bool CCSGameRules::CheckWinConditions(void) {
  ...
  if (m_iRoundWinStatus != WINNER_NONE) {
    // still check if we lost players to where we need to do a full reset next
    // round...
    int NumDeadCT, NumDeadTerrorist, NumAliveTerrorist, NumAliveCT;
    InitializePlayerCounts(NumAliveTerrorist, NumAliveCT, NumDeadTerrorist,
                           NumDeadCT);
  ...
  if (TeamExterminationCheck(NumAliveTerrorist, NumAliveCT, NumDeadTerrorist,
                             NumDeadCT, bNeededPlayers))
    return true;
...
void CCSGameRules::InitializePlayerCounts(int &NumAliveTerrorist,
                                          int &NumAliveCT,
...
bool CCSGameRules::HostageRescueRoundEndCheck(bool bNeededPlayers) {
  ...
  for (i = 0; i < iNumHostages; i++) {
    ...
    if (hostage->m_iHealth > 0 &&
        !hostage
             ->IsRescued()) // We've found a live hostage. don't end the round
      iNumLeftToRescue++;
  }
...
bool CCSGameRules::PrisonRoundEndCheck() {
  ...
  if (m_bMapHasEscapeZone == true)
  {
          ...
          else if ( NumAliveTerrorist == 0 && flEscapeRatio <
  m_flRequiredEscapeRatio)
          {
                  BroadcastSound( "Event.CTWin" );
                  m_iAccountCT += (1 - flEscapeRatio) * 3500; // CTs are
  rewarded based on how many terrorists have escaped...
          ...
          else if ( NumAliveTerrorist == 0 && NumDeadTerrorist != 0 &&
  m_iNumSpawnableCT > 0 )
          {
                  BroadcastSound( "Event.CTWin" );
                  m_iAccountCT += (1 - flEscapeRatio) * 3250; // CTs are
  rewarded based on how many terrorists have escaped...
                  ...
                  EndRoundMessage( "#Escaping_Terrorists_Neutralized",
  Escaping_Terrorists_Neutralized ); TerminateRound(
  mp_round_restart_delay.GetFloat(), WINNER_CT ); return;
          }
          // else return;
  }
  */
...
bool CCSGameRules::VIPRoundEndCheck(bool bNeededPlayers) {
  ...
  if (m_pVIP->m_bEscaped == true) {
    ...
    MessageBegin( MSG_SPEC, SVC_DIRECTOR );
            ...
            WRITE_SHORT( ENTINDEX(m_pVIP->edict()) );	// index number of
    primary entity WRITE_SHORT( 0 );	// index number of secondary entity
            WRITE_LONG( 15 | DRC_FLAG_FINAL);   // eventflags (priority and
    flags) MessageEnd();
    */
...
bool CCSGameRules::BombRoundEndCheck(bool bNeededPlayers) {
  // Check to see if the bomb target was hit or the bomb defused.. if so, then
  // let's end the round!
  if ((m_bTargetBombed == true) && (m_bMapHasBombTarget == true)) {
    ...
    m_iAccountTerrorist += 800; // give the T's a little bonus for planting the
                                // bomb even though it was defused.
...
bool CCSGameRules::TeamExterminationCheck(int NumAliveTerrorist, int NumAliveCT,
                                          int NumDeadTerrorist, int NumDeadCT,
                                          bool bNeededPlayers) {
...
void CCSGameRules::PickNextVIP() {
  ...
  {
          // Remove the current VIP from his VIP status and make him a regular
  CT. if (m_pVIP != NULL) ResetCurrentVIP();
          ...
          for (int i = 0; i <= 4; i++)
          {
                  if (VIPQueue[i] != NULL)
                  {
                          ...
                          VIPQueue[i] = NULL;		// remove this player
  from the VIP queue StackVIPQueue();		// and re-organize the queue
                          m_iConsecutiveVIP = 0;
                          return;
                  }
          }
  }
  else if (m_iConsecutiveVIP >= 3)	// If it's been the same VIP for 3
  rounds already.. then randomly pick a new one
  {
          m_iLastPick++;
  ...
  (!FNullEnt(pPlayer->edict()))	)
          {
                  if (	!(pPlayer->pev->flags & FL_DORMANT)	)
                  {
                          ...
                          if (	(player->m_iTeam == CT) && (iCount ==
  m_iLastPick)	)
                          {
                                  if (	(player == m_pVIP) && (pLastPlayer !=
  NULL)	) player = pLastPlayer;

                                  // Remove the current VIP from his VIP status
  and make him a regular CT. if (m_pVIP != NULL) ResetCurrentVIP();
  ...
  else if (m_pVIP == NULL)  // There is no VIP and there is no one waiting to be
  the VIP.. therefore just pick the first CT player we can find.
  {
          CBaseEntity* pPlayer = NULL;
          CBasePlayer* player = NULL;
...
void CCSGameRules::RestartRound() {
  ...
  if (g_pReplay) {
    ...
    if (nActivePlayerCount && g_pReplay->SV_ShouldBeginRecording(false)) {
      // Tell the replay manager that it should begin recording the new round as
      // soon as possible
  ...
  //=============================================================================
  for (int clientIndex = 1; clientIndex <= gpGlobals->maxClients;
       clientIndex++) {
  ...
  if (gEntList.FindEntityByClassname(NULL, "func_escapezone")) {
    ...
    m_iHaveEscaped = 0;
    m_iNumEscapers =
        0; // Will increase this later when we count how many Ts are starting
    if (m_iNumEscapeRounds >= 3) {
      SwapAllPlayers();
      m_iNumEscapeRounds = 0;
    }

    m_iNumEscapeRounds++; // Increment the number of rounds played... After 8
                          // rounds, the players will do a whole sale switch..
  } else
    m_bMapHasEscapeZone = false;
  ...
  //*******Catch up code by SupraFiend. Scale up the loser bonus when teams fall
  //into losing streaks
  ...
  } else if (m_iRoundWinStatus == WINNER_CT) // CT Won
  {
    ...
    m_iNumConsecutiveTerroristLoses++; // increment the number of wins the
                                       // Terrorists have had
  }

  // check if the losing team is in a losing streak & that the loser bonus
  // hasen't maxed out.
  ...
  } else if (m_iRoundWinStatus == WINNER_CT) // CT Won
  {
    ...
    if (m_bMapHasEscapeZone ==
        false) // only give them the bonus if this isn't an escape map
  ...
  //=============================================================================
  // HPE_BEGIN:
  // [tj] Keep track of number of players per side and if they have the same
  // uniform
  //=============================================================================
  ...
  for (i = 1; i <= gpGlobals->maxClients; i++) {
    ...
    if (pPlayer->GetTeamNumber() == TEAM_CT &&
        pPlayer->PlayerClass() >= FIRST_CT_CLASS &&
    ...
    if (pPlayer->GetTeamNumber() == TEAM_TERRORIST &&
        pPlayer->PlayerClass() >= FIRST_T_CLASS &&
  ...
  for (i = 1; i <= gpGlobals->maxClients; i++) {
    ...
    if (pPlayer->GetTeamNumber() == TEAM_TERRORIST &&
        allTerroristsWearingSameUniform &&
...
void CCSGameRules::GiveC4() {
  ...
  if (numAliveTs[which] > 0) {
    ...
    CCSPlayer *pPlayer =
        ToCSPlayer(UTIL_PlayerByIndex(iTerrorists[which][index]));

    Assert(pPlayer && pPlayer->GetTeamNumber() == TEAM_TERRORIST &&
           pPlayer->IsAlive());
    ...
    // UTIL_LogPrintf("\"%s<%i><%s><TERRORIST>\" triggered
...
// The bots do their processing after physics simulation etc so their visibility
// checks don't recompute bone positions multiple times a frame.
...
bool CCSGameRules::CheckGameOver() {
  if (g_fGameOver) // someone else quit the game already
  {
    //=============================================================================
    // HPE_BEGIN:
    // [Forrest] Calling ChangeLevel multiple times was causing
    // IncrementMapCycleIndex to skip over maps in the list.  Avoid this using a
    // technique from CTeamplayRoundBasedRules::Think.
    //=============================================================================
      ...
      if (team) {
        ...
        case TEAM_TERRORIST:
          ...
          winEvent->SetInt("t_objectives_done",
                           CCS_GameStats.GetTeamStats(
                               teamIndex)[CSSTAT_OBJECTIVES_COMPLETED]);
          winEvent->SetInt(
              "t_money_earned",
              CCS_GameStats.GetTeamStats(teamIndex)[CSSTAT_MONEY_EARNED]);
          break;
        case TEAM_CT:
...
ConCommand test_round_winpanel("test_round_winpanel", TestRoundWinpanel, "",
...
ConCommand test_match_winpanel("test_match_winpanel", TestMatchWinpanel, "",
                               FCVAR_DEVELOPMENTONLY | FCVAR_CHEAT);
...
ConCommand test_freezepanel("test_freezepanel", TestFreezePanel, "",
                            FCVAR_DEVELOPMENTONLY | FCVAR_CHEAT);
#endif // _DEBUG
...
void CCSGameRules::DumpTimers(void) const {
  ...
  PrintToConsole(player,
                 str.sprintf("m_bCompleteReset: %d\n", m_bCompleteReset));
  PrintToConsole(
      player, str.sprintf("m_iTotalRoundsPlayed: %d\n", m_iTotalRoundsPlayed));
  PrintToConsole(player, str.sprintf("m_iRoundTime: %d\n", m_iRoundTime.Get()));
  PrintToConsole(player,
                 str.sprintf("m_iRoundWinStatus: %d\n", m_iRoundWinStatus));

  PrintToConsole(player,
                 str.sprintf("first connected: %d\n", m_bFirstConnected));
  PrintToConsole(player, str.sprintf("intermission end time: %f\n",
                                     m_flIntermissionEndTime));
  PrintToConsole(player,
                 str.sprintf("freeze period: %d\n", m_bFreezePeriod.Get()));
  PrintToConsole(player,
                 str.sprintf("round restart time: %f\n", m_flRestartRoundTime));
  PrintToConsole(player,
                 str.sprintf("game start time: %f\n", m_flGameStartTime.Get()));
  PrintToConsole(
      player, str.sprintf("m_fRoundStartTime: %f\n", m_fRoundStartTime.Get()));
  PrintToConsole(player, str.sprintf("freeze time: %d\n", m_iFreezeTime));
  PrintToConsole(player,
                 str.sprintf("next think: %f\n", m_tmNextPeriodicThink));

  PrintToConsole(player, str.sprintf("fraglimit: %d\n", fraglimit.GetInt()));
  PrintToConsole(player,
                 str.sprintf("mp_maxrounds: %d\n", mp_maxrounds.GetInt()));
  PrintToConsole(player,
                 str.sprintf("mp_winlimit: %d\n", mp_winlimit.GetInt()));
  ...
  PrintToConsole(player, str.sprintf("num CTs (spawnable): %d (%d)\n", m_iNumCT,
                                     m_iNumSpawnableCT));
  PrintToConsole(player,
                 str.sprintf("num Ts (spawnable): %d (%d)\n", m_iNumTerrorist,
                             m_iNumSpawnableTerrorist));
...
// next round.
void CCSGameRules::MarkLivingPlayersOnTeamAsNotReceivingMoneyNextRound(
    int team) {
  int playerNum;
...
void CCSGameRules::CheckLevelInitialized(void) {
  if (!m_bLevelInitialized) {
    ...
    while ((ent = gEntList.FindEntityByClassname(
                ent, "info_player_terrorist")) != NULL) {
    ...
    while ((ent = gEntList.FindEntityByClassname(
                ent, "info_player_counterterrorist")) != NULL) {
      if (IsSpawnPointValid(ent, NULL)) {
...
void CCSGameRules::ShowSpawnPoints(void) {
  ...
  while ((ent = gEntList.FindEntityByClassname(ent, "info_player_terrorist")) !=
         NULL) {
    if (IsSpawnPointValid(ent, NULL)) {
      NDebugOverlay::Box(ent->GetAbsOrigin(), VEC_HULL_MIN, VEC_HULL_MAX, 0,
                         255, 0, 200, 600);
    } else {
      NDebugOverlay::Box(ent->GetAbsOrigin(), VEC_HULL_MIN, VEC_HULL_MAX, 255,
                         0, 0, 200, 600);
    }
  }
...
void CCSGameRules::CheckRestartRound(void) {
  ...
  if (iRestartDelay > 0) {
    ...
    UTIL_ClientPrintAll(HUD_PRINTCENTER, "#Game_will_restart_in",
                        strRestartDelay,
                        iRestartDelay == 1 ? "SECOND" : "SECONDS");
    UTIL_ClientPrintAll(HUD_PRINTCONSOLE, "#Game_will_restart_in",
                        strRestartDelay,
                        iRestartDelay == 1 ? "SECOND" : "SECONDS");
...
void CCSGameRules::BalanceTeams(void) {
  ...
  if (m_iMapHasVIPSafetyZone ==
      1) // The ratio for teams is different for Assasination maps
  {
  ...
  if (iNumToSwap > 3) // Don't swap more than 3 players at a time.. This is a
                      // naive method of avoiding infinite loops.
    iNumToSwap = 3;
  ...
  for (int i = 0; i < iNumToSwap; i++) {
    ...
    // search for player with highest UserID = most recently joined to switch
    // over
...
int CCSGameRules::SelectDefaultTeam(bool ignoreBots /*= false*/) {
  if (ignoreBots && (FStrEq(cv_bot_join_team.GetString(), "T") ||
...
bool CCSGameRules::FPlayerCanRespawn(CBasePlayer *pBasePlayer) {
  ...
  // If they're dead after the map has ended, and it's about to start the next
  // round, wait for the round restart to respawn them.
...
static bool IsClanTeam(CTeam *pTeam) {
  ...
  for (int iPlayer = 0; iPlayer < pTeam->GetNumPlayers(); iPlayer++) {
    ...
    const char *pClanID =
        engine->GetClientConVarValue(pPlayer->entindex(), "cl_clanid");
...
// [tj] This is where we check non-player-specific that occur at the end of the
// round
void CCSGameRules::ProcessEndOfRoundAchievements(int iWinnerTeam, int iReason) {
  if (iWinnerTeam == WINNER_CT || iWinnerTeam == WINNER_TER) {
    ...
    if (((iReason == CTs_Win && m_bNoCTsKilled) ||
         (iReason == Terrorists_Win && m_bNoTerroristsKilled)) &&
        losingTeam &&
        losingTeam->GetNumPlayers() - ignoreCount >=
    ...
    // [tj] Check flawless victory achievement - currently requiring
    // extermination
    if (((iReason == CTs_Win && m_bNoCTsDamaged) ||
         (iReason == Terrorists_Win && m_bNoTerroristsDamaged)) &&
        losingTeam &&
        losingTeam->GetNumPlayers() - ignoreCount >=
            AchievementConsts::DefaultMinOpponentsForAchievement) {
      CTeam *pTeam = GetGlobalTeam(iWinnerTeam);
    ...
    if (pWinningTeam &&
        pWinningTeam->GetNumPlayers() >=
            AchievementConsts::DefaultMinOpponentsForAchievement &&
        losingTeam &&
        losingTeam->GetNumPlayers() - ignoreCount >=
            AchievementConsts::DefaultMinOpponentsForAchievement &&
        IsClanTeam(pWinningTeam) && IsClanTeam(losingTeam)) {
      for (int iPlayer = 0; iPlayer < pWinningTeam->GetNumPlayers();
           iPlayer++) {
...
//[tj] Counts the number of players in each category in the struct (dead, alive,
//etc...)
  ...
  public:
    ...
    virtual CBaseEntity *CreateNextEntity(const char *pClassname) {
      if (m_iIterator == g_MapEntityRefs.InvalidIndex()) {
        ...
        // used CCSMapLoadEntityFilter, which should have built the
...
CBaseEntity *CCSGameRules::GetPlayerSpawnSpot(CBasePlayer *pPlayer) {
  ...
  pPlayer->Teleport(&pSpawnSpot->GetAbsOrigin(), &pSpawnSpot->GetLocalAngles(),
                    &vec3_origin);
...
void CCSGameRules::HostageTouched() {
  if (gpGlobals->curtime > m_flNextHostageAnnouncement &&
      m_iRoundWinStatus == WINNER_NONE) {
...
void CCSGameRules::CreateStandardEntities() {
  ...
  g_pPlayerResource = (CPlayerResource *)CBaseEntity::Create(
      "cs_player_manager", vec3_origin, vec3_angle);
...
void CCSGameRules::UploadGameStats(void) {
  ...
  if (DataHasChanged() == true) {
    ...
    Q_strncpy(stats.header.szGameName, "cstrike",
              sizeof(stats.header.szGameName));
    ...
    stats.iMinutesPlayed =
        clamp((short)(gpGlobals->curtime / 60), 0, MY_USHRT_MAX);

    memcpy(stats.iTerroristVictories, g_iTerroristVictories,
           sizeof(g_iTerroristVictories));
    memcpy(stats.iCounterTVictories, g_iCounterTVictories,
           sizeof(g_iCounterTVictories));
    memcpy(stats.iBlackMarketPurchases, g_iWeaponPurchases,
           sizeof(g_iWeaponPurchases));
    ...
    if (gamestatsuploader) {
      gamestatsuploader->UploadGameStats(STRING(gpGlobals->mapname),
                                         CS_STATS_BLOB_VERSION, uBlobSize,
                                         pvBlobData);
    }
...
CBaseCombatWeapon *
CCSGameRules::GetNextBestWeapon(CBaseCombatCharacter *pPlayer,
  ...
  for (int i = 0; i < MAX_WEAPONS; i++) {
    ...
    if (csPlayer && csPlayer->IsBot() &&
        !TheCSBots()->IsWeaponUseable(csWeapon))
      continue;
#endif // CLIENT_DLL
    ...
    if (bestWeapon) {
      if (weapon->GetSlot() < bestWeapon->GetSlot()) {
        bestWeapon = weapon;
      } else if (weapon->GetSlot() == bestWeapon->GetSlot() &&
                 weapon->GetPosition() < bestWeapon->GetPosition()) {
        bestWeapon = weapon;
      }
    } else {
      bestWeapon = weapon;
...
float CCSGameRules::GetMapRemainingTime() {
  ...
  float flTimeLeft =
      (m_flGameStartTime + mp_timelimit.GetInt() * 60) - gpGlobals->curtime;
...
bool CCSGameRules::ShouldCollide(int collisionGroup0, int collisionGroup1) {
  ...
  // TODO: make a CS-SPECIFIC COLLISION GROUP FOR PHYSICS PROPS THAT USE THIS
  // COLLISION BEHAVIOR.
...
CAmmoDef *GetAmmoDef() {
  ...
  if (!bInitted) {
    ...
    ammoDef.AddAmmoType(BULLET_PLAYER_50AE, DMG_BULLET, TRACER_LINE, 0, 0,
                        "ammo_50AE_max", 2400 * BULLET_IMPULSE_EXAGGERATION, 0,
                        10, 14);
    ammoDef.AddAmmoType(BULLET_PLAYER_762MM, DMG_BULLET, TRACER_LINE, 0, 0,
                        "ammo_762mm_max", 2400 * BULLET_IMPULSE_EXAGGERATION, 0,
                        10, 14);
    ammoDef.AddAmmoType(BULLET_PLAYER_556MM, DMG_BULLET, TRACER_LINE, 0, 0,
                        "ammo_556mm_max", 2400 * BULLET_IMPULSE_EXAGGERATION, 0,
                        10, 14);
    ammoDef.AddAmmoType(BULLET_PLAYER_556MM_BOX, DMG_BULLET, TRACER_LINE, 0, 0,
                        "ammo_556mm_box_max",
                        2400 * BULLET_IMPULSE_EXAGGERATION, 0, 10, 14);
    ammoDef.AddAmmoType(BULLET_PLAYER_338MAG, DMG_BULLET, TRACER_LINE, 0, 0,
                        "ammo_338mag_max", 2800 * BULLET_IMPULSE_EXAGGERATION,
                        0, 12, 16);
    ammoDef.AddAmmoType(BULLET_PLAYER_9MM, DMG_BULLET, TRACER_LINE, 0, 0,
                        "ammo_9mm_max", 2000 * BULLET_IMPULSE_EXAGGERATION, 0,
                        5, 10);
    ammoDef.AddAmmoType(BULLET_PLAYER_BUCKSHOT, DMG_BULLET, TRACER_LINE, 0, 0,
                        "ammo_buckshot_max", 600 * BULLET_IMPULSE_EXAGGERATION,
                        0, 3, 6);
    ammoDef.AddAmmoType(BULLET_PLAYER_45ACP, DMG_BULLET, TRACER_LINE, 0, 0,
                        "ammo_45acp_max", 2100 * BULLET_IMPULSE_EXAGGERATION, 0,
                        6, 10);
    ammoDef.AddAmmoType(BULLET_PLAYER_357SIG, DMG_BULLET, TRACER_LINE, 0, 0,
                        "ammo_357sig_max", 2000 * BULLET_IMPULSE_EXAGGERATION,
                        0, 4, 8);
    ammoDef.AddAmmoType(BULLET_PLAYER_57MM, DMG_BULLET, TRACER_LINE, 0, 0,
                        "ammo_57mm_max", 2000 * BULLET_IMPULSE_EXAGGERATION, 0,
                        4, 8);
    ammoDef.AddAmmoType(AMMO_TYPE_HEGRENADE, DMG_BLAST, TRACER_LINE, 0, 0,
                        "ammo_hegrenade_max", 1, 0);
    ammoDef.AddAmmoType(AMMO_TYPE_FLASHBANG, 0, TRACER_LINE, 0, 0,
                        "ammo_flashbang_max", 1, 0);
    ammoDef.AddAmmoType(AMMO_TYPE_SMOKEGRENADE, 0, TRACER_LINE, 0, 0,
                        "ammo_smokegrenade_max", 1, 0);
...
const char *CCSGameRules::GetChatLocation(bool bTeamOnly,
                                          CBasePlayer *pPlayer) {
  if (!pPlayer) // dedicated server output
  {
    return NULL;
...
void CCSGameRules::ClientSettingsChanged(CBasePlayer *pPlayer) {
  ...
  if (pCSPlayer->IsNetClient()) {
    const char *pShowHints = engine->GetClientConVarValue(
        engine->IndexOfEdict(pCSPlayer->edict()), "cl_autohelp");
...
void DrawSphere(const Vector &pos, float radius, int r, int g, int b,
                float lifetime) {
...
CON_COMMAND_F(
  ...
  CBaseEntity *ent = NULL;
  while ((ent = gEntList.FindEntityByClassname(ent, "func_bomb_target")) !=
         NULL) {
...
CON_COMMAND_F(map_setbombradius, "Sets the bomb radius for the map.",
              FCVAR_CHEAT) {
  if (args.ArgC() != 2)
    return;
...
void CreateBlackMarketString(void) {
  g_StringTableBlackMarket =
      networkstringtable->CreateStringTable("BlackMarketTable", 1);
}
...
//=============================================================================
// HPE_BEGIN:
// [menglish] Set up anything for all players that changes based on new players
// spawning mid-game
//				Find and return fun fact data
//=============================================================================

//-----------------------------------------------------------------------------
// Purpose: Called when a player joins the game after it's started yet can still
// spawn in
//-----------------------------------------------------------------------------
...
void CCSGameRules::PlayerTookDamage(CCSPlayer *player,
  ...
  if (player && pCSScorer) {
    ...
    // set the first blood if this is the first and the victim is on a different
    // team then the player
    if (m_pFirstBlood == NULL && pCSScorer != player &&
        pCSScorer->GetTeamNumber() != player->GetTeamNumber()) {
...
const weeklyprice_t *CCSGameRules::GetBlackMarketPriceList(void) {
  if (m_StringTableBlackMarket == NULL) {
    m_StringTableBlackMarket =
        networkstringtable->FindTable(CS_GAMERULES_BLACKMARKET_TABLE_NAME);
  }
...
void TestTable(void) {
  CSGameRules()->m_StringTableBlackMarket =
      networkstringtable->FindTable(CS_GAMERULES_BLACKMARKET_TABLE_NAME);

  if (CSGameRules()->m_StringTableBlackMarket == NULL)
    return;

  int iIndex = CSGameRules()->m_StringTableBlackMarket->FindStringIndex(
      "blackmarket_prices");
  int iSize = 0;

  const weeklyprice_t *pPrices = NULL;

  pPrices = (const weeklyprice_t *)(CSGameRules()->m_StringTableBlackMarket)
                ->GetStringUserData(iIndex, &iSize);
}
...
//-----------------------------------------------------------------------------
void EnforceCompetitiveCVar(const char *szCvarName, float fMinValue,
                            float fMaxValue = FLT_MAX, int iArgs = 0, ...) {
  ...
  if (fValue < fMinValue || fValue > fMaxValue) {
    ...
    DevMsg("Convar %s enforced by server (see sv_competitive_minspec.) Set to "
           "%2f.\n",
...
public:
  ...
  static void BackupAllConvars() {
    FOR_EACH_VEC(*GetConvarList(), i) { (*GetConvarList())[i]->BackupConvar(); }
  }
  ...
  virtual void Shutdown() {
    FOR_EACH_VEC(*GetConvarList(), i) { delete (*GetConvarList())[i]; }
...
static CCompetitiveCvarManager *s_pCompetitiveCvarManager =
    new CCompetitiveCvarManager();
CCompetitiveCvarManager::CompetitiveConvarList_t
    *CCompetitiveCvarManager::s_pCompetitiveConvars = null;
...
// Usage: ENABLE_COMPETITIVE_CONVAR( convarName, minValue, maxValue,
// optionalValues, opVal1, opVal2, ...
//-----------------------------------------------------------------------------
#define ENABLE_COMPETITIVE_CONVAR(convarName, ...)                             \
  ...
  public:                                                                      \
    ...
    static void on_changed_##convarName(IConVar *var, const char *pOldValue,   \
                                        float flOldValue) {                    \
      if (sv_competitive_minspec.GetBool()) {                                  \
        EnforceCompetitiveCVar(#convarName, __VA_ARGS__);                      \
      } else {                                                                 \
        CCompetitiveCvarManager::GetConVarBackupKV()->SetFloat(                \
            #convarName, ConVarRef(#convarName).GetFloat());                   \
      }                                                                        \
    }                                                                          \
    ...
    virtual void RestoreOriginalValue() {                                      \
      ConVarRef(#convarName)                                                   \
          .SetValue(CCompetitiveCvarManager::GetConVarBackupKV()->GetFloat(    \
              #convarName));                                                   \
    }                                                                          \
    virtual void InstallChangeCallback() {                                     \
      static_cast<ConVar *>(ConVarRef(#convarName).GetLinkedConVar())          \
          ->InstallChangeCallback(                                             \
              CCompetitiveMinspecConvar##convarName::on_changed_##convarName); \
    }                                                                          \
  };                                                                           \
  static CCompetitiveMinspecConvar##convarName                                 \
      *s_pCompetitiveConvar##convarName =                                      \
          new CCompetitiveMinspecConvar##convarName();

//-----------------------------------------------------------------------------
// Callback function for sv_competitive_minspec convar value change.
//-----------------------------------------------------------------------------
void sv_competitive_minspec_changed_f(IConVar *var, const char *pOldValue,
                                      float flOldValue) {
...
ENABLE_COMPETITIVE_CONVAR(r_drawdetailprops, true,
                          true); // force r_drawdetailprops on
ENABLE_COMPETITIVE_CONVAR(r_staticprop_lod, -1,
                          3); // force r_staticprop_lod from -1 to 3
ENABLE_COMPETITIVE_CONVAR(
    fps_max, 59, FLT_MAX, 1,
    0); // force fps_max above 59. One additional value (0) works
ENABLE_COMPETITIVE_CONVAR(cl_detailfade, 400); // force cl_detailfade above 400.
ENABLE_COMPETITIVE_CONVAR(cl_detaildist,
                          1200); // force cl_detaildist above 1200.
ENABLE_COMPETITIVE_CONVAR(cl_interp_ratio, 1,
                          2); // force cl_interp_ratio from 1 to 2
ENABLE_COMPETITIVE_CONVAR(cl_interp, 0,
                          0.031); // force cl_interp from 0.0152 to 0.031
