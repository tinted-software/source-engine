...
void FX_FireBullets(int iPlayer, const Vector &vOrigin, const QAngle &vAngles,
                    int iWeaponID, int iMode, int iSeed, float fInaccuracy,
                    float fSpread, float flSoundTime = 0.0f);
...
enum PlantBombOption_t {
  ...
  // NOTE: If you add additional items to this enum then m_option in
  // CTEPlantBomb will need to have its SendPropInt setting changed to have more
  // than one bit.
};
