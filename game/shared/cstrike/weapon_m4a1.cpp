...
SendPropBool(SENDINFO(m_bSilencerOn)),
    ...
    END_NETWORK_TABLE()
        ...
        BEGIN_PREDICTION_DATA(CWeaponM4A1) END_PREDICTION_DATA()
...
void CWeaponM4A1::PrimaryAttack() {
  ...
  // CSBaseGunFire can kill us, forcing us to drop our weapon, if we shoot
...
bool CWeaponM4A1::Reload() {
  ...
  if (m_bSilencerOn)
    iResult =
        DefaultReload(GetMaxClip1(), GetMaxClip2(), ACT_VM_RELOAD_SILENCED);
  else
