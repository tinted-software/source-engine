...
public:
  ...
  bool GetAwardTime(int &year, int &month, int &day, int &hour, int &minute,
                    int &second);
...
//----------------------------------------------------------------------------------------------------------------
// Helper class for achievements that check that the player was playing on a
// game team for the full round
...
#if !defined(NO_STEAM)
  STEAM_CALLBACK(CAchievement_Meta, Steam_OnUserAchievementStored,
                 UserAchievementStored_t, m_CallbackUserAchievement);
#endif
