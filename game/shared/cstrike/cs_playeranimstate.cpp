...
class CCSPlayerAnimState : public CBasePlayerAnimState,
                           public ICSPlayerAnimState {
public:
  DECLARE_CLASS(CCSPlayerAnimState, CBasePlayerAnimState);
  friend ICSPlayerAnimState *
  CreatePlayerAnimState(CBaseAnimatingOverlay *pEntity,
  ...
  void InitCS(CBaseAnimatingOverlay *pPlayer,
...
protected:
  ...
  void UpdateLayerSequenceGeneric(CStudioHdr *pStudioHdr, int iLayer,
                                  bool &bEnabled, float &flCurCycle,
...
private:
  ...
  float m_flReloadHoldEndTime; // Intermediate shotgun reloads get held a
                               // fraction of a second

  // This is set to true if ANY animation is being played in the fire layer.
  bool m_bFiring; // If this is on, then it'll continue the fire animation in
                  // the fire layer until it completes.
  int m_iFireSequence; // (For any sequences in the fire layer, including
                       // grenade throw).
  float m_flFireCycle;
  PlayerAnimEvent_t
      m_delayedFire; // if we fire while reloading, delay the fire by one frame
                     // so we can cancel the reload first
  ...
  int m_iLastThrowGrenadeCounter; // used to detect when the guy threw the
                                  // grenade.
  ...
  int m_sequenceCache[ACT_CROUCHIDLE + 1]; // Cache the first N sequences, since
                                           // we don't have weights.
  int m_cachedModelIndex; // Model index for which the sequence cache is valid.

  CUtlDict<int, int> m_namedSequence; // Dictionary of sequences computed with
                                      // CalcSequenceIndex.  This is because
                                      // LookupSequence is a performance hit -
                                      // CS:S player models have 750+ sequences!
};

ICSPlayerAnimState *CreatePlayerAnimState(CBaseAnimatingOverlay *pEntity,
                                          ICSPlayerAnimStateHelpers *pHelpers,
...
public:
  ...
  // No need to cache sequences, and we *do* have multiple sequences per
  // activity
  virtual int SelectWeightedSequence(Activity activity) {
...
//----------------------------------------------------------------------------------------------
ICSPlayerAnimState *CreateHostageAnimState(CBaseAnimatingOverlay *pEntity,
                                           ICSPlayerAnimStateHelpers *pHelpers,
                                           LegAnimType_t legAnimType,
                                           bool bUseAimSequences) {
...
void CCSPlayerAnimState::InitCS(CBaseAnimatingOverlay *pEntity,
                                ICSPlayerAnimStateHelpers *pHelpers,
                                LegAnimType_t legAnimType,
                                bool bUseAimSequences) {
...
void CCSPlayerAnimState::CheckCachedSequenceValidity(void) {
  if (m_cachedModelIndex != GetOuter()->GetModelIndex()) {
    ...
    if (m_cachedModelIndex > 0) {
      m_sequenceCache[ACT_HOP - 1] =
          GetOuter()->SelectWeightedSequence(ACT_HOP);
      m_sequenceCache[ACT_IDLE - 1] =
          GetOuter()->SelectWeightedSequence(ACT_IDLE);
      m_sequenceCache[ACT_RUN_CROUCH - 1] =
          GetOuter()->SelectWeightedSequence(ACT_RUN_CROUCH);
      m_sequenceCache[ACT_CROUCHIDLE - 1] =
          GetOuter()->SelectWeightedSequence(ACT_CROUCHIDLE);
      m_sequenceCache[ACT_RUN - 1] =
          GetOuter()->SelectWeightedSequence(ACT_RUN);
      m_sequenceCache[ACT_WALK - 1] =
          GetOuter()->SelectWeightedSequence(ACT_WALK);
      m_sequenceCache[ACT_IDLE - 1] =
          GetOuter()->SelectWeightedSequence(ACT_IDLE);
    }
  }
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Cache the sequence numbers for the first ACT_HOP activities, since the CS
 * player doesn't have multiple sequences per activity.
 */
int CCSPlayerAnimState::SelectWeightedSequence(Activity activity) {
  ...
  if (sequence < 0) {
    ...
    sequence = m_sequenceCache[activity - 1] =
        GetOuter()->SelectWeightedSequence(activity);
  }
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Try to look up named sequences in a CUtlDict cache before falling back to the
 * normal LookupSequence.  It's best to avoid the normal LookupSequence when
 * your models have 750+ sequences...
 */
...
void CCSPlayerAnimState::DoAnimationEvent(PlayerAnimEvent_t event, int nData) {
  ...
  case PLAYERANIMEVENT_FIRE_GUN_SECONDARY:
    ...
    // If we are interrupting a (shotgun) reload, cancel the reload, and fire
    // next frame.
  ...
  case PLAYERANIMEVENT_RELOAD: {
    // ignore normal reload events for shotguns - they get sent to trigger
    // sounds etc only
  ...
  case PLAYERANIMEVENT_RELOAD_LOOP:
    // Set the hold time for _start and _loop anims, then fall through to the
    // _end case
  ...
  case PLAYERANIMEVENT_RELOAD_END: {
    ...
    CWeaponCSBase *pWeapon = m_pHelpers->CSAnim_GetActiveWeapon();
    if (pWeapon && pWeapon->GetCSWpnData().m_WeaponType != WEAPONTYPE_SHOTGUN) {
      m_flReloadHoldEndTime =
          0.0f; // clear this out in case we set it in _START or _LOOP above
    } else {
      m_iReloadSequence = CalcReloadLayerSequence(event);
      if (m_iReloadSequence != -1) {
        m_bReloading = true;
...
int CCSPlayerAnimState::CalcReloadLayerSequence(PlayerAnimEvent_t event) {
  ...
  char szName[512];
  Q_snprintf(szName, sizeof(szName), "%s_reload_%s%s", prefix, weaponSuffix,
             reloadSuffix);
...
void CCSPlayerAnimState::UpdateLayerSequenceGeneric(CStudioHdr *pStudioHdr,
  ...
  flCurCycle += m_pOuter->GetSequenceCycleRate(pStudioHdr, iSequence) *
                gpGlobals->frametime;
  if (flCurCycle > 1) {
    if (bWaitAtEnd) {
      flCurCycle = 1;
...
bool CCSPlayerAnimState::IsOuterGrenadePrimed() {
  ...
  if (pChar) {
    CBaseCSGrenade *pGren =
        dynamic_cast<CBaseCSGrenade *>(pChar->GetActiveWeapon());
...
void CCSPlayerAnimState::ComputeGrenadeSequence(CStudioHdr *pStudioHdr) {
  ...
  if (m_bThrowingGrenade) {
    UpdateLayerSequenceGeneric(pStudioHdr, GRENADESEQUENCE_LAYER,
                               m_bThrowingGrenade, m_flGrenadeCycle,
    ...
    if (IsOuterGrenadePrimed() || bThrowPending) {
      if (!m_bPrimingGrenade) {
        // If this guy just popped into our PVS, and he's got his grenade
        // primed, then let's assume that it's all the way primed rather than
        // playing the prime animation from the start.
      ...
      UpdateLayerSequenceGeneric(pStudioHdr, GRENADESEQUENCE_LAYER,
                                 m_bPrimingGrenade, m_flGrenadeCycle,
...
void CCSPlayerAnimState::ComputeReloadSequence(CStudioHdr *pStudioHdr) {
  ...
  UpdateLayerSequenceGeneric(pStudioHdr, RELOADSEQUENCE_LAYER, m_bReloading,
                             m_flReloadCycle, m_iReloadSequence, hold);
  if (!m_bReloading) {
    m_flReloadHoldEndTime = 0.0f;
  }
}

int CCSPlayerAnimState::CalcAimLayerSequence(float *flCycle,
...
int CCSPlayerAnimState::CalcFireLayerSequence(PlayerAnimEvent_t event) {
  ...
  if (pWeapon->GetWeaponID() == WEAPON_ELITE) {
    ...
    Q_snprintf(tempsuffix, sizeof(tempsuffix), "%s_%c", pSuffix,
               bPrimary ? 'r' : 'l');
    pSuffix = tempsuffix;
  }
...
float CCSPlayerAnimState::GetCurrentMaxGroundSpeed() {
  Activity currentActivity =
      m_pOuter->GetSequenceActivity(m_pOuter->GetSequence());
...
void CCSPlayerAnimState::ClearAnimationLayers() {
  ...
  for (int i = 0; i < m_pOuter->GetNumAnimOverlays(); i++) {
    // Client obeys Order of CBaseAnimatingOverlay::MAX_OVERLAYS (15), but
    // server trusts only the ANIM_LAYER_ACTIVE flag.
