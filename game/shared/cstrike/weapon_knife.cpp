...
// Output : void*
//-----------------------------------------------------------------------------
void *SendProxy_SendActiveLocalKnifeDataTable(const SendProp *pProp,
                                              const void *pStruct,
                                              const void *pVarData,
...
#if !defined(CLIENT_DLL)
            SendPropDataTable(
                "LocalActiveWeaponKnifeData", 0,
                &REFERENCE_SEND_TABLE(DT_LocalActiveWeaponKnifeData),
                SendProxy_SendActiveLocalKnifeDataTable),
#else
            RecvPropDataTable(
                "LocalActiveWeaponKnifeData", 0, 0,
                &REFERENCE_RECV_TABLE(DT_LocalActiveWeaponKnifeData)),
#endif
    END_NETWORK_TABLE()
...
void FindHullIntersection(const Vector &vecSrc, trace_t &tr, const Vector &mins,
  ...
  UTIL_TraceLine(vecSrc, vecHullEnd, MASK_SOLID, pEntity, COLLISION_GROUP_NONE,
                 &tmpTrace);
  ...
  for (i = 0; i < 2; i++) {
    for (j = 0; j < 2; j++) {
      for (k = 0; k < 2; k++) {
        ...
        UTIL_TraceLine(vecSrc, vecEnd, MASK_SOLID, pEntity,
  ...
  if (m_trHit.m_pEnt) {
    ...
    if (m_trHit.m_pEnt->IsPlayer()) {
      EmitSound(filter, entindex(),
                m_bStab ? "Weapon_Knife.Stab" : "Weapon_Knife.Hit");
    } else {
...
bool CKnife::SwingOrStab(bool bStab) {
  ...
  trace_t tr;
  UTIL_TraceLine(vecSrc, vecEnd, MASK_SOLID, pPlayer, COLLISION_GROUP_NONE,
                 &tr);

  // check for hitting glass - TODO - fix this hackiness, doesn't always line up
  // with what FindHullIntersection returns
#ifndef CLIENT_DLL
  CTakeDamageInfo glassDamage(pPlayer, pPlayer, 42.0f,
  ...
  if (bDidHit) {
    ...
    if (bStab) {
      ...
      if (pEntity && pEntity->IsPlayer()) {
        ...
        Vector2D vecLOS =
            (pEntity->GetAbsOrigin() - pPlayer->GetAbsOrigin()).AsVector2D();
