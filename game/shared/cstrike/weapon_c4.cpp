...
SendPropBool(SENDINFO(m_bBombTicking)),
    ...
    END_SEND_TABLE()
        ...
        BEGIN_PREDICTION_DATA(CPlantedC4) END_PREDICTION_DATA()
...
int CPlantedC4::ShouldTransmit(const CCheckTransmitInfo *pInfo) {
  ...
  // Everybody needs it for hiding the round timer and showing the planted C4
  // scenario icon
...
void CPlantedC4::Precache() {
  ...
  engine->ForceModelBounds(PLANTED_C4_MODEL, Vector(-7, -13, -3),
                           Vector(9, 12, 11));
...
void CPlantedC4::GetControlPanelClassName(int nPanelIndex,
...
CPlantedC4 *CPlantedC4::ShootSatchelCharge(CCSPlayer *pevOwner, Vector vecStart,
                                           QAngle vecAngles) {
  CPlantedC4 *pGrenade =
      dynamic_cast<CPlantedC4 *>(CreateEntityByName("planted_c4"));
  if (pGrenade) {
    vecAngles[0] = 0;
    vecAngles[2] = 0;
...
void CPlantedC4::C4Think() {
  ...
  if (m_flC4Blow <= gpGlobals->curtime) {
    ...
    UTIL_TraceLine(vecSpot, vecSpot + Vector(0, 0, -40), MASK_SOLID, this,
    ...
    if (m_flDefuseCountDown > gpGlobals->curtime) {
      ...
      if (m_flNextDefuse < gpGlobals->curtime || !iOnGround) {
        if (!iOnGround && m_pBombDefuser->IsAlive())
          ClientPrint(m_pBombDefuser, HUD_PRINTCENTER,
    ...
    if (!m_pBombDefuser->IsDead()) {
      ...
      if (event) {
        ...
        if ((timeToDetonation > 0.0f) &&
            (timeToDetonation <=
             AchievementConsts::BombDefuseCloseCall_MaxTimeRemaining)) {
        ...
        if ((timeToDetonation > 0.0f) && (m_pBombDefuser->HasDefuser()) &&
            (timeToDetonation <
             AchievementConsts::BombDefuseNeededKit_MaxTime)) {
          // Give achievement for defusing with a defuse kit when not having the
          // kit would have taken too long
      ...
      // The bomb has just been disarmed.. Check to see if the round should end
      // now
...
void CPlantedC4::Explode(trace_t *pTrace, int bitsDamageType) {
  ...
  //=============================================================================
  // HPE_BEGIN:
  // [tj] Saving off this value so we can see if the detonation is what caused
  // the round to end.
  //=============================================================================
  ...
  while ((pTarget = gEntList.FindEntityByClassname(
              pTarget, "func_bomb_target")) != NULL) {
...

  {
    ...
    te->Explosion(filter, 0.0, &pos, g_sModelIndexFireball, 50.0, 25,
                  TE_EXPLFLAG_NONE, flBombRadius * 3.5, 200);
  }
  ...
  MESSAGE_BEGIN( MSG_SPEC, SVC_DIRECTOR );
          ...
          WRITE_SHORT( ENTINDEX(this->edict()) );	// index number of
...
void CPlantedC4::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
    ...
    SendPropFloat(SENDINFO(m_fArmedTime), 0, SPROP_NOSCALE)
        ...
        END_NETWORK_TABLE()
            ...
            BEGIN_PREDICTION_DATA(CC4)
                DEFINE_PRED_FIELD(m_bStartedArming, FIELD_INTEGER,
...
bool CC4::OnFireEvent(C_BaseViewModel *pViewModel, const Vector &origin,
...
bool CC4::ShouldRemoveOnRoundRestart() {
  // Doesn't matter if we have an owner or not.. always remove the C4 when the
  // round restarts. The gamerules will give another C4 to some lucky player.
  CCSPlayer *pPlayer = GetPlayerOwner();
...
void CC4::PrimaryAttack() {
  ...
  if (groundEntity) {
    // Don't let us stand on players, breakables, or pushaway physics objects to
    // plant
  ...
  if (m_bStartedArming == false && m_bBombPlanted == false) {
    ...
    if (!onGround || !pPlayer->m_bInBombZone) {
      ...
      if (m_bBombPlacedAnimation ==
          true) // this means the placement animation is canceled
      {
      ...
      } else if ((gpGlobals->curtime >= (m_fArmedTime - 0.75)) &&
  ...
  if (bArmingTimeSatisfied && m_bStartedArming) {
    ...
    if (pPlayer->m_bInBombZone) {
#if !defined(CLIENT_DLL)
      CPlantedC4 *pC4 = CPlantedC4::ShootSatchelCharge(
          pPlayer, pPlayer->GetAbsOrigin(), pPlayer->GetAbsAngles());

      if (pC4) {
        pC4->SetBombSiteIndex(pPlayer->m_iBombSiteIndex);

        trace_t tr;
        UTIL_TraceEntity(pC4, GetAbsOrigin(),
                         GetAbsOrigin() + Vector(0, 0, -200), MASK_SOLID, this,
                         COLLISION_GROUP_NONE, &tr);
        pC4->SetAbsOrigin(tr.endpos);

        CBombTarget *pBombTarget =
            (CBombTarget *)UTIL_EntityByIndex(pPlayer->m_iBombSiteIndex);

        if (pBombTarget) {
          CBaseEntity *pAttachPoint = gEntList.FindEntityByName(
              NULL, pBombTarget->GetBombMountTarget());
      ...
      // Determine how elapsed time from start of round until the bomb was
      // planted
      float plantingTime =
  ...
  SetWeaponIdleTime(gpGlobals->curtime +
                    SharedRandomFloat("C4IdleTime", 10, 15));
}
...
void CC4::WeaponIdle() {
  ...
  if (m_bStartedArming) {
    ...
    // TODO: make this use SendWeaponAnim and activities when the C4 has the
    // activities hooked up.
    if (pPlayer) {
      SendWeaponAnim(ACT_VM_IDLE);
      pPlayer->SetNextAttack(gpGlobals->curtime);
    }
...
void CC4::PlayArmingBeeps(void) {
  ...
  float flProgress =
      (gpGlobals->curtime - flStartTime) / (WEAPON_C4_ARM_TIME - 0.75);
    ...
    if (currentFrame <= m_iBeepFrames[i]) {
      ...
      for (i = 1; i <= gpGlobals->maxClients; i++) {
        ...
        if (pPlayer->GetObserverMode() == OBS_MODE_IN_EYE &&
            pPlayer->GetObserverTarget() == GetOwner()) {
