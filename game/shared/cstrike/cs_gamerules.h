...
public:
  ...
  float
  GetMapRemainingTime(); // time till end of map, -1 if timelimit is disabled
  ...
  float
  GetRoundElapsedTime(); // How much time has elapsed since the round started.
...
private:
  float GetExplosionDamageAdjustment(
      ...
      CBaseEntity
          *pEntityToIgnore); // returns multiplier between 0.0 and 1.0 that is
                             // the percentage of any damage done from vecSrc to
                             // vecEnd that actually makes it.
  float GetAmountOfEntityVisible(
      Vector &src,
      CBaseEntity *player); // returns a value from 0 to 1 that is the
                            // percentage of player visible from src.

  CNetworkVar(bool, m_bFreezePeriod); // TRUE at beginning of round, set to
                                      // FALSE when the period expires
  CNetworkVar(int, m_iRoundTime); // (From mp_roundtime) - How many seconds long
                                  // this round is.
  ...
  CNetworkVar(bool, m_bLogoMap); // If there's an info_player_logo entity, then
                                 // it's a logo map.
...
public:
  ...
  GetNextBestWeapon(CBaseCombatCharacter *pPlayer,
  ...
  void
  DumpTimers(void) const; // debugging to help track down a stuck server (rare?)
  ...
  //=============================================================================
  // HPE_BEGIN:
  // [menglish] Set up anything for all players that changes based on new
  // players spawning mid-game
  ...
  // Checks if it still needs players to start a round, or if it has enough
  // players to start rounds. Starts a round and returns true if there are
  // enough players.
  bool NeededPlayersCheck(bool &bNeededPlayers);

  // Setup counts for m_iNumTerrorist, m_iNumCT, m_iNumSpawnableTerrorist,
  // m_iNumSpawnableCT, etc.
  void InitializePlayerCounts(int &NumAliveTerrorist, int &NumAliveCT,
                              int &NumDeadTerrorist, int &NumDeadCT);

  // Check to see if the round is over for the various game types. Terminates
  // the round and returns true if the round should end.
  bool PrisonRoundEndCheck();
  bool BombRoundEndCheck(bool bNeededPlayers);
  bool HostageRescueRoundEndCheck(bool bNeededPlayers);

  // Check to see if the teams exterminated each other. Ends the round and
  // returns true if so.
  bool TeamExterminationCheck(int NumAliveTerrorist, int NumAliveCT,
                              int NumDeadTerrorist, int NumDeadCT,
                              bool bNeededPlayers);
...
public:
  ...
  int m_iRoundWinStatus; // 1 == CT's won last round, 2 == Terrorists did, 3 ==
                         // Draw, no winner
  int m_iTotalRoundsPlayed;
  int m_iUnBalancedRounds; // keeps track of the # of consecutive rounds that
                           // have gone by where one team outnumbers the other
                           // team by more than 2

  // GAME TIMES
  int m_iFreezeTime; // (From mp_freezetime) - How many seconds long the intro
                     // round (when players are frozen) is.
  float m_flRestartRoundTime; // the global time when the round is supposed to
                              // end, if this is not 0

  int m_iNumTerrorist; // The number of terrorists on the team (this is
                       // generated at the end of a round)
  int m_iNumCT; // The number of CTs on the team (this is generated at the end
                // of a round)
  int m_iNumSpawnableTerrorist;
  int m_iNumSpawnableCT;

  bool m_bFirstConnected;
  bool m_bCompleteReset; // Set to TRUE to have the scores reset next time round
                         // restarts
  ...
  int m_iNumConsecutiveCTLoses; // SupraFiend: the number of rounds the CTs have
                                // lost in a row.
  int m_iNumConsecutiveTerroristLoses; // SupraFiend: the number of rounds the
                                       // Terrorists have lost in a row.
  ...
  int m_iLoserBonus; // SupraFiend: the amount of money the losing team gets.
                     // This scales up as they lose more rounds in a row
  ...
  // [tj] To avoid rewriting the same piece of code, we can get all the
  // information
  //		we want from one call that fills in an array of structures.
  struct TeamPlayerCounts {
    int totalPlayers;
    int totalAlivePlayers;
    int totalDeadPlayers; // sum of killedPlayers + suicidedPlayers +
                          // unenteredPlayers
    int killedPlayers;
    int suicidedPlayers;
  ...
  int m_iNumEscapers;
  int m_iNumEscapeRounds; // keeps track of the # of consecutive rounds of
                          // escape played.. Teams will be swapped after 8
                          // rounds

  // VIP VARIABLES
  int m_iMapHasVIPSafetyZone; // 0 = uninitialized;   1 = has VIP safety zone;
                              // 2 = DOES not have VIP safetyzone
