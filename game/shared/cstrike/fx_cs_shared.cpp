...
void FX_WeaponSound(int iPlayerIndex, WeaponSound_t sound_type,
                    const Vector &vOrigin, CCSWeaponInfo *pWeaponInfo,
                    float flSoundTime) {
...
void FX_FireBullets(int iPlayerIndex, const Vector &vOrigin,
...
#ifdef CLIENT_DLL
  C_CSPlayer *pPlayer =
      ToCSPlayer(ClientEntityList().GetBaseEntity(iPlayerIndex));
#else
  ...
  if (weapon_accuracy_logging.GetBool()) {
    ...
    Msg("FireBullets @ %10f [ %s ]: inaccuracy=%f  spread=%f  max "
        "dispersion=%f  mode=%2i  vel=%10f  seed=%3i  %s\n",
        gpGlobals->curtime, weaponAlias, fInaccuracy, fSpread,
        fInaccuracy + fSpread, iMode, fVelocity, iSeed, szFlags);
  }
#endif
  ...
  CCSWeaponInfo *pWeaponInfo =
      static_cast<CCSWeaponInfo *>(GetFileWeaponInfoFromHandle(hWpnInfo));

  // Do the firing animation event.
  if (pPlayer && !pPlayer->IsDormant()) {
    if (iMode == Primary_Mode)
      pPlayer->GetPlayerAnimState()->DoAnimationEvent(
  ...
  // the RNG can be desynchronized by FireBullet(), so pre-generate all spread
  // offsets
  ...
  for (int iBullet = 0; iBullet < pWeaponInfo->m_iBullets; iBullet++) {
    pPlayer->FireBullet(vOrigin, vAngles, flRange, iPenetration, iAmmoType,
