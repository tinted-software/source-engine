...
public:
  float m_flMaxSpeed; // How fast the player can run while this is his primary
                      // weapon.
  ...
  int m_iTeam; // Which team can have this weapon. TEAM_UNASSIGNED if both can
               // have it.
  ...
  char m_WrongTeamMsg[32]; // Reference to a string describing the error if
                           // someone tries to buy this weapon but they're on
                           // the wrong team to have it. Zero-length if no
                           // specific message for this weapon.

  char m_szAnimExtension[16];
  char m_szShieldViewModel[64];

  char m_szAddonModel[MAX_WEAPON_STRING];   // If this is set, it is used as the
                                            // addon model. Otherwise,
                                            // szWorldModel is used.
  char m_szDroppedModel[MAX_WEAPON_STRING]; // Alternate dropped model, if
                                            // different from the szWorldModel
                                            // the player holds
  char m_szSilencerModel[MAX_WEAPON_STRING]; // Alternate model with silencer
                                             // attached
  ...
  // Variables that control how fast the weapon's accuracy changes as it is
  // fired.
