...
void CWeaponCSBaseGun::ItemPostFrame() {
  ...
  // GOOSEMAN : Return zoom level back to previous zoom level before we fired a
  // shot. This is used only for the AWP.
  //  And Scout.
  if ((m_flNextPrimaryAttack <= gpGlobals->curtime) &&
    ...
    if (m_iClip1 != 0 || (GetWeaponFlags() & ITEM_FLAG_NOAUTORELOAD)) {
      ...
      m_zoomFullyActiveTime =
          gpGlobals->curtime +
          0.05f; // Make sure we think that we are zooming on the server so we
                 // don't get instant acc bonus
    }
  }
...
bool CWeaponCSBaseGun::CSBaseGunFire(float flCycleTime,
  ...
  if (m_iClip1 > 0) {
    ...
    if (pCSInfo.m_flAccuracyDivisor != -1) {
      ...
      m_flAccuracy = (iShotsFired / pCSInfo.m_flAccuracyDivisor) +
                     pCSInfo.m_flAccuracyOffset;
    ...
    if (m_bFireOnEmpty) {
      ...
      // NOTE[pmf]: we don't want to actually play the dry fire animations, as
      // most seem to depict the weapon actually firing. SendWeaponAnim(
      // ACT_VM_DRYFIRE );
  ...
  FX_FireBullets(pPlayer->entindex(), pPlayer->Weapon_ShootPosition(),
                 ...
                 GetWeaponID(), weaponMode,
                 CBaseEntity::GetPredictionRandomSeed() & 255, GetInaccuracy(),
                 GetSpread(), flCurAttack);

  DoFireEffects();

  SetWeaponIdleTime(gpGlobals->curtime +
                    GetCSWpnData().m_flTimeToIdleAfterFire);
