...
ConVar weapon_accuracy_model("weapon_accuracy_model", "2",
...
const char *GetWeaponAliasFromTranslated(const char *translatedAlias) {
  int i = 0;
  const WeaponAliasTranslationInfoStruct *info =
      &(s_WeaponAliasTranslationInfo[i]);
...
RecvPropInt(RECVINFO(m_weaponMode)),
    ...
    END_NETWORK_TABLE()
        ...
        BEGIN_PREDICTION_DATA(CWeaponCSBase)
            DEFINE_PRED_FIELD(m_flTimeWeaponIdle, FIELD_FLOAT,
                              FTYPEDESC_OVERRIDE | FTYPEDESC_NOERRORCHECK),
    DEFINE_PRED_FIELD(m_flNextPrimaryAttack, FIELD_FLOAT,
                      FTYPEDESC_OVERRIDE | FTYPEDESC_NOERRORCHECK),
    DEFINE_PRED_FIELD(m_flNextSecondaryAttack, FIELD_FLOAT,
                      FTYPEDESC_OVERRIDE | FTYPEDESC_NOERRORCHECK),
    ...
    DEFINE_PRED_FIELD_TOL(m_fAccuracyPenalty, FIELD_FLOAT,
                          FTYPEDESC_INSENDTABLE, 0.00005f),
    END_PREDICTION_DATA()
#endif
...
ConVar cl_crosshaircolor(
    "cl_crosshaircolor", "0", FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
    "Set crosshair color: 0=green, 1=red, 2=blue, 3=yellow, 4=cyan, 5=custom");
ConVar
    cl_dynamiccrosshair("cl_dynamiccrosshair", "1",
                        FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
                        "Enables dynamic crosshair; 0=off, 1=normal behavior "
                        "(based on actual weapon accuracy), 2=legacy simulated "
                        "dynamic behavior, 3=legacy simulated static behavior");
ConVar cl_crosshairspreadscale("cl_crosshairspreadscale", "0.3",
                               FCVAR_CLIENTDLL | FCVAR_ARCHIVE);
ConVar cl_scalecrosshair("cl_scalecrosshair", "1",
                         FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
                         "Enable crosshair scaling (deprecated)");
ConVar cl_crosshairscale("cl_crosshairscale", "0",
                         FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
                         "Crosshair scaling factor (deprecated)");
ConVar cl_crosshairalpha("cl_crosshairalpha", "200",
                         FCVAR_CLIENTDLL | FCVAR_ARCHIVE);
ConVar cl_crosshairusealpha("cl_crosshairusealpha", "1",
                            FCVAR_CLIENTDLL | FCVAR_ARCHIVE);
ConVar cl_crosshairsize("cl_crosshairsize", "5",
                        FCVAR_CLIENTDLL | FCVAR_ARCHIVE);
ConVar cl_crosshairthickness("cl_crosshairthickness", "0.5",
                             FCVAR_CLIENTDLL | FCVAR_ARCHIVE);
ConVar cl_crosshairdot("cl_crosshairdot", "0", FCVAR_CLIENTDLL | FCVAR_ARCHIVE);
ConVar cl_crosshaircolor_r("cl_crosshaircolor_r", "50",
                           FCVAR_CLIENTDLL | FCVAR_ARCHIVE);
ConVar cl_crosshaircolor_g("cl_crosshaircolor_g", "250",
                           FCVAR_CLIENTDLL | FCVAR_ARCHIVE);
ConVar cl_crosshaircolor_b("cl_crosshaircolor_b", "50",
                           FCVAR_CLIENTDLL | FCVAR_ARCHIVE);

#if ALLOW_WEAPON_SPREAD_DISPLAY
ConVar weapon_debug_spread_show("weapon_debug_spread_show", "0",
                                FCVAR_CLIENTDLL | FCVAR_DEVELOPMENTONLY,
                                "Enables display of weapon accuracy; 1: show "
                                "accuracy box, 2: show box with recoil offset");
ConVar weapon_debug_spread_gap("weapon_debug_spread_gap", "0.67",
                               FCVAR_CLIENTDLL | FCVAR_DEVELOPMENTONLY);
#endif

// [paquin] make sure crosshair scales independent of frame rate
// unless legacy cvar is set
ConVar cl_legacy_crosshair_recoil(
    "cl_legacy_crosshair_recoil", "0", FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
    "Enable legacy framerate dependent crosshair recoil");

// use old scaling behavior
ConVar cl_legacy_crosshair_scale("cl_legacy_crosshair_scale", "0",
                                 FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
...
void CWeaponCSBase::ItemPostFrame() {
  ...
  if ((m_bInReload) && (pPlayer->m_flNextAttack <= gpGlobals->curtime)) {
    // complete the reload.
    int j = MIN(GetMaxClip1() - m_iClip1,
                pPlayer->GetAmmoCount(m_iPrimaryAmmoType));
  ...
  if ((pPlayer->m_nButtons & IN_ATTACK2) &&
    ...
    if (GetCSWpnData().m_WeaponType != WEAPONTYPE_GRENADE) {
      IGameEvent *event = gameeventmanager->CreateEvent(
          (HasAmmo()) ? "weapon_fire" : "weapon_fire_on_empty");
      if (event) {
    ...
    PrimaryAttack();
             ...
             GetMaxClip1() != WEAPON_NOCLIP && !m_bInReload &&
             m_flNextPrimaryAttack < gpGlobals->curtime) {
    ...
    // The following code prevents the player from tapping the firebutton
    // repeatedly to simulate full auto and retaining the single shot accuracy
    // of single fire
    if (m_bDelayFire) {
      m_bDelayFire = false;
    ...
    // if it's a pistol then set the shots fired to 0 after the player releases
    // a button
    if (IsPistol()) {
      pPlayer->m_iShotsFired = 0;
    } else {
...
float CWeaponCSBase::GetInaccuracy() const {
  ...
  return m_fAccuracyPenalty +
         RemapValClamped(pPlayer->GetAbsVelocity().Length2D(),
                         ...
                         fMaxSpeed * 0.95f, // max out at 95% of run speed to
                                            // avoid jitter near max speed
...
// Purpose:
//-----------------------------------------------------------------------------
const char *CWeaponCSBase::GetViewModel(
...
bool CWeaponCSBase::DefaultDeploy(char *szViewModel, char *szWeaponModel,
...
// whats going on here is that if the player drops this weapon, they shouldn't
// take it back themselves for a little while.  But if they throw it at someone
// else, the other player should get it immediately.
void CWeaponCSBase::DefaultTouch(CBaseEntity *pOther) {
  if ((m_prevOwner != NULL) && (pOther == m_prevOwner) &&
      (gpGlobals->curtime < m_nextPrevOwnerTouchTime)) {
    return;
  }
...
void CWeaponCSBase::DrawCrosshair() {
  ...
  bool bAdditive =
      !cl_crosshairusealpha.GetBool() && !pPlayer->m_bNightVisionOn;
  if (bAdditive) {
  ...
  bool bCrosshairVisible = crosshair.GetBool() && GetCSWpnData().m_WeaponType !=
  ...
  int iBarThickness =
      MAX(1, RoundFloatToInt(YRES(cl_crosshairthickness.GetFloat())));

  switch (cl_dynamiccrosshair.GetInt()) {
  case 0:
  default: {
    // static crosshair
    float fSpread = (GetCSWpnData().m_fSpread[m_weaponMode] +
                     GetCSWpnData().m_fInaccuracyStand[m_weaponMode]) *
                    320.0f / tanf(fHalfFov);
    iCrosshairDistance = MAX(
        0, RoundFloatToInt(YRES(fSpread * cl_crosshairspreadscale.GetFloat())));
  } break;
  ...
  case 3: {
    float fCrosshairDistanceGoal =
        GetCSWpnData().m_iCrosshairMinDistance; // The minimum distance the
                                                // crosshair can achieve...
    ...
    // [jpaquin] changed to only bump up the crosshair size if the player is
    // still shooting or is spectating someone else
    int iDeltaDistance =
        GetCSWpnData()
            .m_iCrosshairDeltaDistance; // Amount by which the crosshair expands
                                        // when shooting (per frame)
    if (pPlayer->m_iShotsFired > m_iAmmoLastCheck &&
        (pPlayer->m_nButtons & (IN_ATTACK | IN_ATTACK2)))
    ...
    if (m_flCrosshairDistance > fCrosshairDistanceGoal) {
      // [jpaquin] if we're not in legacy crosshair mode, use an exponential
      // decay function so that the crosshair shrinks at the same rate
      // regardless of the frame rate
      if (!cl_legacy_crosshair_recoil.GetBool()) {
        // .44888 on the next line makes the decay very close to what old method
        // produces at 100fps.
        m_flCrosshairDistance =
            Lerp(expf(-gpGlobals->frametime / 0.44888f), fCrosshairDistanceGoal,
                 m_flCrosshairDistance);
      } else {
    ...
    m_flCrosshairDistance =
        clamp(m_flCrosshairDistance, fCrosshairDistanceGoal, 25.0f);
    ...
    if (cl_legacy_crosshair_scale.GetBool()) {
      ...
      iBarSize =
          XRES(5); //  + (iCrosshairDistance - fCrosshairDistanceGoal) / 2;
      iBarSize = MAX(1, (int)((float)iBarSize * scale));
      iBarThickness = MAX(1, (int)floor(scale + 0.5f));
    } else {
      iCrosshairDistance =
          RoundFloatToInt(m_flCrosshairDistance * ScreenHeight() / 1200.0f);
    }
  } break;
  }
  ...
  if (weapon_debug_spread_show.GetInt() == 1 ||
      weapon_debug_spread_show.GetInt() == 2) {
    if (weapon_debug_spread_show.GetInt() == 2) {
      ...
      float iOffsetX =
          RoundFloatToInt(YRES(vecDirShooting.y * 320.0f / tanf(fHalfFov)));
      float iOffsetY =
          RoundFloatToInt(YRES(vecDirShooting.z * 320.0f / tanf(fHalfFov)));
    ...
    float fSpreadDistance =
        (GetInaccuracy() + GetSpread()) * 320.0f / tanf(fHalfFov);
    ...
    int iGap =
        RoundFloatToInt(weapon_debug_spread_gap.GetFloat() * iSpreadDistance);

    // draw horizontal lines
    DrawCrosshairRect(iOuterLeft, iOuterTop, iCenterX - iGap, iInnerTop,
                      bAdditive);
    DrawCrosshairRect(iCenterX + iGap, iOuterTop, iOuterRight, iInnerTop,
                      bAdditive);
    DrawCrosshairRect(iOuterLeft, iInnerBottom, iCenterX - iGap, iOuterBottom,
                      bAdditive);
    DrawCrosshairRect(iCenterX + iGap, iInnerBottom, iOuterRight, iOuterBottom,
                      bAdditive);

    // draw vertical lines
    DrawCrosshairRect(iOuterLeft, iOuterTop, iInnerLeft, iCenterY - iGap,
                      bAdditive);
    DrawCrosshairRect(iOuterLeft, iCenterY + iGap, iInnerLeft, iOuterBottom,
                      bAdditive);
    DrawCrosshairRect(iInnerRight, iOuterTop, iOuterRight, iCenterY - iGap,
                      bAdditive);
    DrawCrosshairRect(iInnerRight, iCenterY + iGap, iOuterRight, iOuterBottom,
                      bAdditive);
  }
#endif
}
...
void CWeaponCSBase::ProcessMuzzleFlashEvent() {
  // This is handled from the player's animstate, so it can match up to the
  // beginning of the fire animation
}

bool CWeaponCSBase::OnFireEvent(C_BaseViewModel *pViewModel,
...
//-----------------------------------------------------------------------------
void CWeaponCSBase::BulletWasFired(const Vector &vecStart,
                                   const Vector &vecEnd) {}
...
CBaseEntity *CWeaponCSBase::Respawn() {
  ...
  CBaseEntity *pNewWeapon = CBaseEntity::Create(
      GetClassname(), g_pGameRules->VecWeaponRespawnSpot(this), GetAbsAngles(),
      GetOwner());
...
// Purpose:
//-----------------------------------------------------------------------------
void CWeaponCSBase::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
...
void CWeaponCSBase::Spawn() {
  ...
  // Override the bloat that our base class sets as it's a little bit bigger
  // than we want. If it's too big, you drop a weapon and its box is so big that
  // you're still touching it when it falls and you pick it up again right away.
...
bool CWeaponCSBase::DefaultReload(int iClipSize1, int iClipSize2,
                                  int iActivity) {
...
bool CWeaponCSBase::IsUseable() {
  ...
  if (Clip1() <= 0) {
    if (pPlayer->GetAmmoCount(GetPrimaryAmmoType()) <= 0 &&
        GetMaxClip1() != -1) {
      // clip is empty (or nonexistant) and the player has no more ammo of this
      // type.
      return false;
    }
  }
...
float CWeaponCSBase::CalcViewmodelBob(void) {
  ...
  if ((!gpGlobals->frametime) || (player == NULL) ||
      (cl_bobcycle.GetFloat() <= 0.0f) || (cl_bobup.GetFloat() <= 0.0f) ||
  ...
  speed =
      clamp(speed, lastspeed - flmaxSpeedDelta, lastspeed + flmaxSpeedDelta);
  ...
  cycle = bobtime -
          (int)(bobtime / cl_bobcycle.GetFloat()) * cl_bobcycle.GetFloat();
  ...
  if (cycle < cl_bobup.GetFloat()) {
    ...
    cycle = M_PI +
            M_PI * (cycle - cl_bobup.GetFloat()) / (1.0 - cl_bobup.GetFloat());
  }
  ...
  cycle = bobtime - (int)(bobtime / cl_bobcycle.GetFloat() * 2) *
                        cl_bobcycle.GetFloat() * 2;
...
//			viewmodelindex -
//-----------------------------------------------------------------------------
void CWeaponCSBase::AddViewmodelBob(CBaseViewModel *viewmodel, Vector &origin,
                                    QAngle &angles) {
...
#ifndef CLIENT_DLL
bool CWeaponCSBase::PhysicsSplash(const Vector &centerPoint,
                                  const Vector &normal, float rawSpeed,
                                  float scaledSpeed) {
  if (rawSpeed > 20) {
...
void CWeaponCSBase::UpdateAccuracyPenalty() {
  ...
  if (pPlayer->GetMoveType() == MOVETYPE_LADDER) {
    fNewPenalty += weaponInfo.m_fInaccuracyStand[m_weaponMode] +
                   weaponInfo.m_fInaccuracyLadder[m_weaponMode];
  }
  // in the air?
  // 	else if ( !FBitSet( pPlayer->GetFlags(), FL_ONGROUND ) )
  // 	{
  // 		fNewPenalty += weaponInfo.m_fInaccuracyStand[m_weaponMode] +
  // weaponInfo.m_fInaccuracyJump[m_weaponMode];
  // 	}
  ...
  if (fNewPenalty > m_fAccuracyPenalty) {
    ...
    {
      // enforce a large recovery speed penalty (300%) for players in the air;
      // this helps to provide comparable in-air accuracy to the old weapon
      // model
    ...
    m_fAccuracyPenalty = Lerp(expf(TICK_INTERVAL * -fDecayFactor), fNewPenalty,
                              (float)m_fAccuracyPenalty);
  }
}

const float kJumpVelocity =
    sqrtf(2.0f * 800.0f * 57.0f); // see CCSGameMovement::CheckJumpButton()

void CWeaponCSBase::OnJump(float fImpulse) {
  m_fAccuracyPenalty +=
      GetCSWpnData().m_fInaccuracyJump[m_weaponMode] * fImpulse / kJumpVelocity;
}
...
void CWeaponCSBase::OnLand(float fVelocity) {
  ...
  /*
          // this bit of code is only if we want to punch the player view on all
     landings
          ...
          float fHKick = SharedRandomFloat("LandPunchAngleYaw", -1.0f, +1.0f) *
     fVKick * 0.1f;
