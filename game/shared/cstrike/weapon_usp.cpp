...
public:
  ...
  // We overload this so we can translate all weapon activities to silenced
  // versions.
...
private:
  ...
  CNetworkVar(float, m_flDoneSwitchingSilencer); // soonest time switching the
                                                 // silencer will be complete
  float m_flLastFire;
...
SendPropBool(SENDINFO(m_bSilencerOn)),
    ...
    END_NETWORK_TABLE()
        ...
        BEGIN_PREDICTION_DATA(CWeaponUSP)
            DEFINE_PRED_FIELD(m_bSilencerOn, FIELD_BOOLEAN,
...
void CWeaponUSP::Precache() {
  ...
  m_silencedModelIndex =
      CBaseEntity::PrecacheModel(GetCSWpnData().m_szSilencerModel);
  m_inPrecache = false;
}
