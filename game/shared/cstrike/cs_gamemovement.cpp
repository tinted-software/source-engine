...
ConVar
    sv_timebetweenducks("sv_timebetweenducks", "0", FCVAR_REPLICATED,
                        "Minimum time before recognizing consecutive duck key",
                        true, 0.0, true, 2.0);
ConVar sv_enableboost("sv_enableboost", "0", FCVAR_REPLICATED | FCVAR_NOTIFY,
                      "Allow boost exploits");
...
bool CheckForStandable(IHandleEntity *pHandleEntity, int contentsMask) {
  ...
  return (pEntity->IsPlayer() && pEntity->GetGroundEntity() != NULL) ||
         pEntity->IsStandable();
}
...
CCSGameMovement::CCSGameMovement() { m_fTimeLastUnducked = 0.0f; }
...
void CCSGameMovement::CheckParameters(void) {
  ...
  if (m_pCSPlayer->m_duckUntilOnGround) {
    if (!player->GetGroundEntity()) {
      if (mv->m_nButtons & IN_DUCK) {
        m_pCSPlayer->m_duckUntilOnGround =
  ...
  // it would be nice to put this into the player->GetPlayerMaxSpeed() method,
  // but this flag is only stored in the move!
  ...
  if (!player->IsObserver() && (player->GetMoveType() != MOVETYPE_LADDER)) {
    ...
    if (nLevels == 1000)
      Warning("BUG: CCSGameMovement::CheckParameters - too many stacking "
              "levels.\n");
...
void CCSGameMovement::ProcessMovement(CBasePlayer *pBasePlayer,
                                      CMoveData *pMove) {
...
void CCSGameMovement::PlayerMove() {
  ...
  if (m_pCSPlayer->IsAlive()) {
    ...
    // This is needed because we have taller models with the old collision
    // bounds.

    const float eyeClearance =
        12.0f; // eye pos must be this far below the ceiling
    ...
    end.z += player->m_Local.m_bDucked ? VEC_DUCK_VIEW_SCALED(player).z
                                       : VEC_VIEW_SCALED(player).z;
    ...
    if (trace.fraction < 1.0f) {
      float est = start.z + trace.fraction * (end.z - start.z) -
                  player->GetAbsOrigin().z - eyeClearance;
      if ((player->GetFlags() & FL_DUCKING) == 0 &&
          !player->m_Local.m_bDucking && !player->m_Local.m_bDucked) {
        offset.z = est;
      } else {
      ...
      if ((player->GetFlags() & FL_DUCKING) == 0 &&
        ...
        // We've got the duck hull, pulled up to the top of where the player
        // should be
...
void CCSGameMovement::WalkMove(void) {
  if (m_pCSPlayer->m_flStamina > 0) {
    ...
    flRatio = (STAMINA_MAX -
               ((m_pCSPlayer->m_flStamina / 1000.0) * STAMINA_RECOVER_RATE)) /
              STAMINA_MAX;

    // This Goldsrc code was run with variable timesteps and it had framerate
    // dependencies. People looking at Goldsrc for reference are usually (these
    // days) measuring the stoppage at 60fps or greater, so we need to account
    // for the fact that Goldsrc was applying more stopping power since it
    // applied the slowdown across more frames.
...
bool CCSGameMovement::LadderMove(void) {
  ...
  if (isOnLadder && m_pCSPlayer) {
    m_pCSPlayer->SurpressLadderChecks(mv->GetAbsOrigin(),
                                      m_pCSPlayer->m_vecLadderNormal);
  }
...
//-------------------------------------------------------------------------------------------------------------------------------
/**
 * Looks behind and beneath the player in the air, in case he skips out over the
 * top of a ladder.  If the trace hits a ladder, the player is snapped to the
 * ladder.
 */
void CCSGameMovement::CheckForLadders(bool wasOnGround) {
  if (!wasOnGround) {
    // If we're higher than the last place we were on the ground, bail -
    // obviously we're not dropping past a ladder we might want to grab.
    if (mv->GetAbsOrigin().z > m_pCSPlayer->m_lastStandingPos.z)
      return;

    Vector dir = -m_pCSPlayer->m_lastStandingPos + mv->GetAbsOrigin();
    if (!dir.x && !dir.y) {
      // If we're dropping straight down, we don't know which way to look for a
      // ladder.  Oh well.
      return;
    }
    ...
    TracePlayerBBox(mv->GetAbsOrigin(),
                    ...
                    (PlayerSolidMask() & (~CONTENTS_PLAYERCLIP)),
                    COLLISION_GROUP_PLAYER_MOVEMENT, trace);
    ...
    if (trace.fraction != 1.0f && OnLadder(trace) &&
      ...
      if (m_pCSPlayer->CanGrabLadder(trace.endpos, trace.plane.normal)) {
        ...
        // The ladder check ignored playerclips, to fix a bug exposed by
        // de_train, where a clipbrush is flush with a ladder.  This causes the
        // above tracehull to fail unless we ignore playerclips. However, we
        // have to check for playerclips before we snap to that pos, so we don't
        // warp a player into a clipbrush.
        TracePlayerBBox(mv->GetAbsOrigin(),
                        m_pCSPlayer->m_lastStandingPos - dir * (5 + dist),
...
ConVar sv_enablebunnyhopping("sv_enablebunnyhopping", "0",
...
bool CCSGameMovement::CheckJumpButton(void) {
  ...
  m_pCSPlayer->PlayStepSound((Vector &)mv->GetAbsOrigin(),
                             player->m_pSurfaceData, 1.0, true);
  ...
  if (m_pCSPlayer->m_duckUntilOnGround || (m_pCSPlayer->m_Local.m_bDucking) ||
      (m_pCSPlayer->GetFlags() & FL_DUCKING)) {
    ...
    mv->m_vecVelocity[2] =
        flGroundFactor * sqrt(2 * 800 * 57.0); // 2 * gravity * height
  } else {
    mv->m_vecVelocity[2] +=
        flGroundFactor * sqrt(2 * 800 * 57.0); // 2 * gravity * height
  }
  ...
  m_pCSPlayer->m_flStamina =
      (STAMINA_COST_JUMP / STAMINA_RECOVER_RATE) * 1000.0;
...
void CCSGameMovement::HandleDuckingSpeedCrop() {
  ...
  if (!(m_iSpeedCropped & SPEED_CROPPED_DUCK)) {
    if ((mv->m_nButtons & IN_DUCK) || (player->m_Local.m_bDucking) ||
...
bool CCSGameMovement::CanUnduck() {
  ...
  UTIL_TraceHull(mv->GetAbsOrigin(), newOrigin, VEC_HULL_MIN_SCALED(player),
                 VEC_HULL_MAX_SCALED(player), PlayerSolidMask(), player,
                 COLLISION_GROUP_PLAYER_MOVEMENT, &trace);
...
void CCSGameMovement::Duck(void) {
  ...
  // Fix taken from zblock for rapid crouch/stand not showing stand on other
  // clients
  ...
  if (player->GetFlags() & FL_ONGROUND) {
    ...
    if (!(mv->m_nButtons & IN_DUCK) && (mv->m_nOldButtons & IN_DUCK)) {
      ...
      if ((player->GetFlags() & FL_DUCKING) &&
          (m_fTimeLastUnducked >
           (gpGlobals->curtime - sv_timebetweenducks.GetFloat()))) {
        // if the server thinks the player is still crouched
        // AND the time the player started to stand (from being ducked) was less
        // than 300ms ago prevent the player from ducking again
        mv->m_nButtons &= ~IN_DUCK;
      }
    }
  }
  ...
  if (m_pCSPlayer->m_duckUntilOnGround) {
    if (!bInAir) {
      ...
      // Check if we can un-duck.  We want to unduck if we have space for the
      // standing hull, and if it is less than 2 inches off the ground.
      trace_t trace;
      Vector newOrigin;
      Vector groundCheck;
      ...
      UTIL_TraceHull(newOrigin, groundCheck, VEC_HULL_MIN_SCALED(player),
                     VEC_HULL_MAX_SCALED(player), PlayerSolidMask(), player,
                     COLLISION_GROUP_PLAYER_MOVEMENT, &trace);
  ...
  if ((mv->m_nButtons & IN_DUCK) || (player->m_Local.m_bDucking) ||
    ...
    if (mv->m_nButtons & IN_DUCK) {
      ...
      float duckmilliseconds =
          MAX(0.0f, 1000.0f - (float)player->m_Local.m_flDucktime);
      ...
      if (player->m_Local.m_bDucking) {
        ...
        if ((duckseconds > TIME_TO_DUCK) ||
            (player->GetGroundEntity() == NULL) || alreadyDucked) {
          FinishDuck();
        } else {
        ...
        if (CanUnduck()) {
              ...
              player->m_Local.m_bDucked) // or unducking
          {
            ...
            if ((duckseconds > TIME_TO_UNDUCK) ||
              ...
              // Calc parametric time
              float duckFraction =
                  SimpleSpline(1.0f - (duckseconds / TIME_TO_UNDUCK));
              SetDuckedEyeOffset(duckFraction);
            }
          }
        } else {
...
//-----------------------------------------------------------------------------
void CCSGameMovement::TryTouchGround(const Vector &start, const Vector &end,
                                     const Vector &mins, const Vector &maxs,
                                     unsigned int fMask, int collisionGroup,
                                     trace_t &pm) {
