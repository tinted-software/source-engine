...
CFlexAnimationTrack::GetRawFlexControllerIndex(int side /*= 0*/) {
  Assert(side == 0 || side == 1);
...
public:
  CCheckCCNames() {
    if (ARRAYSIZE(g_CCNameMap) != CChoreoEvent::NUM_CC_TYPES) {
      Error(
          "g_CCNameMap contains %zd entries, CChoreoEvent::NUM_CC_TYPES == %i!",
...
CChoreoEvent::TypeForAbsoluteTagName(const char *name) {
