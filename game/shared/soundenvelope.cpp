...
#include "tier0/memdbgoff.h"
    ...
    // It has envelopes for pitch and volume and can manage state changes to
    // those
    class CSoundPatch {
public:
  DECLARE_SIMPLE_DATADESC();
...
DEFINE_EMBEDDED(m_pitch), DEFINE_EMBEDDED(m_volume),
    ...
    //CHAN_STATIC) 			*pSoundName - sound script string name 			attenuation -
    //attenuation of this sound (not animated)
    //-----------------------------------------------------------------------------
...
void CSoundPatch::ResumeSound(void) {
  if (IsPlaying() && m_Filter.IsActive()) {
    if (EntIndex() >= 0) {
      ...
      DevWarning("CSoundPatch::ResumeSound: Lost EHAndle on restore - destroy "
                 "the sound patch in your entity's StopLoopingSounds! (%s)\n",
                 STRING(m_iszSoundName));
    }
  }
}
...
struct SoundCommand_t {
  ...
  SoundCommand_t(CSoundPatch *pSound, float executeTime,
      ...
      : m_pPatch(pSound), m_time(executeTime), m_deltaTime(deltaTime),
...
// NOTE: This doesn't need to be saved, sound commands are saved right after the
// patch they are associated with
//	DEFINE_FIELD( m_pPatch, FIELD_????? )
DEFINE_FIELD(m_time, FIELD_TIME), DEFINE_FIELD(m_deltaTime, FIELD_FLOAT),
    DEFINE_FIELD(m_command, FIELD_INTEGER), DEFINE_FIELD(m_value, FIELD_FLOAT),
...
CSoundPatch *CSoundControllerImp::SoundCreate(IRecipientFilter &filter,
  ...
  if (GameRules()) {
    pSoundName =
        GameRules()->TranslateEffectForVisionFilter("sounds", pSoundName);
  }
#endif
...
//			In singleplayer, the player's not ready to receive
//sounds then, so restart 			and SoundPatches that are active and have no
//receivers.
//-----------------------------------------------------------------------------
