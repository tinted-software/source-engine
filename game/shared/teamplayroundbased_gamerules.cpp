...
RecvPropInt(RECVINFO(m_iRoundState), 0, RecvProxy_TeamplayRoundState),
    ...
    RecvPropArray3(RECVINFO_ARRAY(m_bTeamReady),
                   RecvPropBool(RECVINFO(m_bTeamReady[0]))),
    RecvPropBool(RECVINFO(m_bStopWatch)),
    RecvPropBool(RECVINFO(m_bMultipleTrains)),
    RecvPropArray3(RECVINFO_ARRAY(m_bPlayerReady),
                   RecvPropBool(RECVINFO(m_bPlayerReady[0]))),
...
SendPropInt(SENDINFO(m_iRoundState), 5),
    ...
    SendPropArray3(SENDINFO_ARRAY3(m_bTeamReady),
                   SendPropBool(SENDINFO_ARRAY(m_bTeamReady))),
    SendPropBool(SENDINFO(m_bStopWatch)),
    SendPropBool(SENDINFO(m_bMultipleTrains)),
    SendPropArray3(SENDINFO_ARRAY3(m_bPlayerReady),
...
RecvPropDataTable("teamplayroundbased_gamerules_data", 0, 0,
                  ...
                  RecvProxy_TeamplayRoundBasedRules) END_RECV_TABLE()
...
ConVar
    mp_blockstyle("mp_blockstyle", "1",
                  FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY,
                  "Sets the style of capture point blocking used. 0 = Blocks "
                  "break captures completely. 1 = Blocks only pause captures.");
...
#if defined(TF_CLIENT_DLL) || defined(TF_DLL)
ConVar mp_highlander("mp_highlander", "0", FCVAR_REPLICATED | FCVAR_NOTIFY,
                     "Allow only 1 of each player class type.");
#endif

// Arena Mode
ConVar tf_arena_preround_time("tf_arena_preround_time", "10",
                              FCVAR_NOTIFY | FCVAR_REPLICATED,
                              "Length of the Pre-Round time", true, 5.0, true,
                              15.0);
ConVar tf_arena_round_time("tf_arena_round_time", "0",
                           FCVAR_NOTIFY | FCVAR_REPLICATED);
ConVar tf_arena_max_streak(
    "tf_arena_max_streak", "3", FCVAR_NOTIFY | FCVAR_REPLICATED,
    "Teams will be scrambled if one team reaches this streak");
ConVar tf_arena_use_queue("tf_arena_use_queue", "1",
...
ConVar mp_winlimit("mp_winlimit", "0", FCVAR_REPLICATED | FCVAR_NOTIFY,
                   ...
                   true, 0, false, 0);
ConVar mp_disable_respawn_times("mp_disable_respawn_times", "0",
                                FCVAR_NOTIFY | FCVAR_REPLICATED);
...
ConVar mp_stalemate_meleeonly(
    "mp_stalemate_meleeonly", "0", FCVAR_REPLICATED | FCVAR_NOTIFY,
    "Restrict everyone to melee weapons only while in Sudden Death.");
ConVar mp_forceautoteam("mp_forceautoteam", "0",
                        FCVAR_REPLICATED | FCVAR_NOTIFY,
...
ConVar mp_stalemate_enable("mp_stalemate_enable", "0", FCVAR_NOTIFY,
...
ConVar mp_holiday_nogifts(
    "mp_holiday_nogifts", "0", FCVAR_NOTIFY,
...
ConVar mp_scrambleteams_auto("mp_scrambleteams_auto", "1", FCVAR_NOTIFY,
                             "Server will automatically scramble the teams if "
                             "criteria met.  Only works on dedicated servers.");
...
bool CTeamplayRoundBasedRules::TimerMayExpire(void) {
  ...
  CTeamTrainWatcher *pWatcher = dynamic_cast<CTeamTrainWatcher *>(
      gEntList.FindEntityByClassname(NULL, "team_train_watcher"));
  while (pWatcher) {
    if (!pWatcher->TimerMayExpire()) {
      return false;
    }

    pWatcher = dynamic_cast<CTeamTrainWatcher *>(
        gEntList.FindEntityByClassname(pWatcher, "team_train_watcher"));
  }
#endif
  ...
  if (IsInTournamentMode() == false) {
    if (m_bAwaitingReadyRestart &&
      ...
      if (iTeam > LAST_SHARED_TEAM && iTeam < GetNumberOfTeams()) {
        ...
        IGameEvent *event =
            gameeventmanager->CreateEvent("teamplay_team_ready");
        if (event) {
...
void CTeamplayRoundBasedRules::CheckWaitingForPlayers(void) {
  ...
  if ((mp_waitingforplayers_cancel.GetBool() || IsInItemTestingMode()) &&
      IsInTournamentMode() == false) {
...
// Purpose:
//-----------------------------------------------------------------------------
CGameRulesRoundStateInfo *
CTeamplayRoundBasedRules::State_LookupInfo(gamerules_roundstate_t state) {
  static CGameRulesRoundStateInfo playerStateInfos[] = {
      ...
      {GR_STATE_BETWEEN_RNDS, "GR_STATE_BETWEEN_RNDS",
...
void CTeamplayRoundBasedRules::State_Think_PREGAME(void) {
  ...
  if (CountActivePlayers() > 0 ||
      (IsInArenaMode() == true && m_flWaitingForPlayersTimeEnds == 0.0f)) {
  ...
  if (gpGlobals->curtime > m_flStateTransitionTime) {
    if (!IsInTraining() && !IsInItemTestingMode()) {
      ...
      if (mp_waitingforplayers_time.GetFloat() > 0 &&
          tf_bot_offline_practice.GetInt() == 0) {
...
void CTeamplayRoundBasedRules::State_Enter_PREROUND(void) {
  ...
  if (IsInArenaMode() == true) {
...
#endif
      IGameEvent *event =
          gameeventmanager->CreateEvent("teamplay_update_timer");
      if (event) {
        gameeventmanager->FireEvent(event);
      }
    }
    ...
    m_flStateTransitionTime =
        gpGlobals->curtime + tf_arena_preround_time.GetInt();
  }
#if defined(TF_CLIENT_DLL) || defined(TF_DLL)
...
bool CTeamplayRoundBasedRules::AreDefendingPlayersReady() {
  ...
  for (int i = 0; i < vecMvMDefenders.Count(); i++) {
    ...
    if (!p.m_bConnected || p.m_iTeam == TEAM_UNASSIGNED || p.m_nEntNum <= 0 ||
        p.m_nEntNum >= MAX_PLAYERS) {
...
void CTeamplayRoundBasedRules::State_Think_RND_RUNNING(void) {
  ...
  if (State_Get() == GR_STATE_RND_RUNNING && mp_timelimit.GetInt() > 0 &&
      IsInPreMatch() == false && GetTimeLeft() <= 0) {
    if (m_bAllowStalemateAtTimelimit ||
        (mp_match_end_at_timelimit.GetBool() && !IsValveMap())) {
...
void CTeamplayRoundBasedRules::State_Think_TEAM_WIN(void) {
  if (gpGlobals->curtime > m_flStateTransitionTime) {
    bool bDone = !(!CheckTimeLimit() && !CheckWinLimit() && !CheckMaxRounds() &&
                   !CheckNextLevelCvar());
...
#ifdef TF_DLL
        if (TFGameRules() && TFGameRules()->IsMannVsMachineMode()) {
          // one of the convars mp_timelimit, mp_winlimit, mp_maxrounds, or
          // nextlevel has been triggered
...
// Purpose:
//-----------------------------------------------------------------------------
void CTeamplayRoundBasedRules::State_Enter_BONUS(void) { SetupOnBonusStart(); }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CTeamplayRoundBasedRules::State_Leave_BONUS(void) { SetupOnBonusEnd(); }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CTeamplayRoundBasedRules::State_Think_BONUS(void) { BonusStateThink(); }
  ...
  if (bForceMapReset && mp_scrambleteams_auto.GetBool()) {
    ...
    if (m_iWinningTeam != TEAM_UNASSIGNED) {
      ...
      if (m_GameTeams.Count() == 0) {
        ...
        CTeam *pTeam;
        for (pTeam = GetGlobalTeam(iTeamIndex); pTeam != NULL;
             pTeam = GetGlobalTeam(++iTeamIndex)) {
    ...
    if (mp_winlimit.GetInt() || mp_maxrounds.GetInt()) {
      ...
      for (int iIndex = m_GameTeams.FirstInorder();
           iIndex != m_GameTeams.InvalidIndex();
...
static ConVar mp_tournament_allow_non_admin_restart(
    ...
    "Allow mp_tournament_restart command to be issued by players other than "
    "admin.");
...
static ConCommand
    mp_tournament_restart("mp_tournament_restart", CC_CH_TournamentRestart,
...
void CTeamplayRoundBasedRules::HandleTimeLimitChange(void) {
  // check that we have an active timer in the HUD and use mp_timelimit if we
  // don't
  if (!MapHasActiveTimer() &&
      (mp_timelimit.GetInt() > 0 && GetTimeLeft() > 0)) {
...
bool CTeamplayRoundBasedRules::MapHasActiveTimer(void) {
  ...
  while ((pEntity = gEntList.FindEntityByClassname(
    ...
    CTeamRoundTimer *pTimer = assert_cast<CTeamRoundTimer *>(pEntity);
    if (pTimer && pTimer->ShowInHud() &&
        (Q_stricmp(STRING(pTimer->GetEntityName()),
                   "zz_teamplay_timelimit_timer") != 0)) {
      return true;
    }
  }
#endif
...
#ifndef CSTRIKE_DLL
  if (!m_hTimeLimitTimer) {
    m_hTimeLimitTimer = (CTeamRoundTimer *)CBaseEntity::Create(
        "team_round_timer", vec3_origin, vec3_angle);
...
void CTeamplayRoundBasedRules::RoundRespawn(void) {
  ...
  if (m_bForceMapReset || m_bPrevRoundWasWaitingForPlayers) {
    ...
    if (mp_timelimit.GetInt() > 0 && GetTimeLeft() > 0) {
      // check that we have an active timer in the HUD and use mp_timelimit if
      // we don't
      if (!MapHasActiveTimer()) {
        CreateTimeLimitTimer();
      }
  ...
  if (g_pReplay &&
      g_pReplay->SV_ShouldBeginRecording(bShouldWaitToStartRecording)) {
...
void CTeamplayRoundBasedRules::CheckRespawnWaves(void) {
  for (int team = LAST_SHARED_TEAM + 1; team < GetNumberOfTeams(); team++) {
    if (m_flNextRespawnWave[team] &&
        m_flNextRespawnWave[team] > gpGlobals->curtime)
      continue;

    RespawnTeam(team);

    // Set m_flNextRespawnWave to 0 when we don't have a respawn time to reduce
    // networking
...
void CTeamplayRoundBasedRules::BalanceTeams(bool bRequireSwitcheesToBeDead) {
  if (mp_autoteambalance.GetBool() == false ||
      (IsInArenaMode() == true && tf_arena_use_queue.GetBool() == true)) {
    return;
  }
  ...
  // wrap with this bool, indicates it's a round running switch and not a
  // between rounds insta-switch
...
bool CTeamplayRoundBasedRules::PlayThrottledAlert(int iTeam, const char *sound,
...
float CTeamplayRoundBasedRules::GetRespawnWaveMaxLength(
  ...
  // Let's just turn off respawn times while players are messing around waiting
...
bool CTeamplayRoundBasedRules::AreTeamsUnbalanced(int &iHeaviestTeam,
                                                  int &iLightestTeam) {
  if (IsInArenaMode() == false ||
      (IsInArenaMode() && tf_arena_use_queue.GetBool() == false)) {
    if (ShouldBalanceTeams() == false) {
      return false;
    }
  }
...
#if defined(TF_CLIENT_DLL) || defined(TF_DLL)
//-----------------------------------------------------------------------------
// Purpose: Are you now, or are you ever going to be, a member of the defending
// party?
//-----------------------------------------------------------------------------
void CTeamplayRoundBasedRules::GetMvMPotentialDefendersLobbyPlayerInfo(
    CUtlVector<LobbyPlayerInfo_t> &vecMvMDefenders,
...
// Purpose:
//-----------------------------------------------------------------------------
void CTeamplayRoundBasedRules::GetAllPlayersLobbyInfo(
    CUtlVector<LobbyPlayerInfo_t> &vecPlayers, bool bIncludeBots) {
