...
extern CEngineSprite *Draw_SetSpriteTexture(const model_t *pSpriteModel,
...
void CSpriteTrail::SetSkybox(const Vector &vecSkyboxOrigin,
                             float flSkyboxScale) {
  m_flTextureRes /= m_flSkyboxScale;
  m_flMinFadeLength *= m_flSkyboxScale;
  m_flStartWidth *= m_flSkyboxScale;
...
void CSpriteTrail::OnDataChanged(DataUpdateType_t updateType) {
  ...
  if (updateType == DATA_UPDATE_CREATED) {
    ...
    if ((m_flPrevSkyboxScale != m_flSkyboxScale) ||
        (m_vecPrevSkyboxOrigin != m_vecSkyboxOrigin)) {
      ConvertSkybox();
    }
  }
...
void CSpriteTrail::ConvertSkybox() {
  for (int i = 0; i < m_nStepCount; ++i) {
    ...
    VectorSubtract(pPoint->m_vecScreenPos, m_vecPrevSkyboxOrigin,
                   pPoint->m_vecScreenPos);
    pPoint->m_vecScreenPos *= m_flPrevSkyboxScale / m_flSkyboxScale;
    VectorSubtract(pPoint->m_vecScreenPos, m_vecSkyboxOrigin,
                   pPoint->m_vecScreenPos);
...
TrailPoint_t *CSpriteTrail::GetTrailPoint(int n) {
  ...
  COMPILE_TIME_ASSERT(
      (MAX_SPRITE_TRAIL_POINTS & (MAX_SPRITE_TRAIL_POINTS - 1)) == 0);
...
void CSpriteTrail::UpdateTrail(void) {
  ...
  if ((pLast == NULL) || (pLast->m_vecScreenPos.DistToSqr(screenPos) > 4.0f)) {
    ...
    pNewPoint->m_flWidthVariance =
        random->RandomFloat(-m_flStartWidthVariance, m_flStartWidthVariance);
    if (pLast) {
      pNewPoint->m_flTexCoord =
          pLast->m_flTexCoord +
          pLast->m_vecScreenPos.DistTo(screenPos) * m_flTextureRes;
    } else {
  ...
  m_flUpdateTime =
      gpGlobals->curtime + (m_flLifeTime / (float)MAX_SPRITE_TRAIL_POINTS);
}
...
int CSpriteTrail::DrawModel(int flags) {
  ...
  CEngineSprite *pSprite =
      Draw_SetSpriteTexture(GetModel(), m_flFrame, GetRenderMode());
  if (pSprite == NULL)
    return 0;
  ...
  segDraw.Start(pRenderContext, m_nStepCount + 1,
                pSprite->GetMaterial(GetRenderMode()));
  ...
  for (int i = 0; i <= m_nStepCount; ++i) {
    ...
    float flLifePerc =
        (pPoint->m_flDieTime - gpGlobals->curtime) / m_flLifeTime;
    ...
    if (flTailAlphaDist > 0.0f) {
      if (pPrevPoint) {
        float flDist =
            pPoint->m_vecScreenPos.DistTo(pPrevPoint->m_vecScreenPos);
        flTailAlphaDist -= flDist;
      }

      if (flTailAlphaDist > 0.0f) {
        float flTailFade =
            Lerp((m_flMinFadeLength - flTailAlphaDist) / m_flMinFadeLength,
                 0.0f, 1.0f);
    ...
    if (m_flEndWidth >= 0.0f) {
      curSeg.m_flWidth =
          Lerp(flLifePerc, m_flEndWidth.Get(), m_flStartWidth.Get());
    } else {
...
// Output : CSpriteTrail
//-----------------------------------------------------------------------------
CSpriteTrail *CSpriteTrail::SpriteTrailCreate(const char *pSpriteName,
                                              const Vector &origin,
                                              bool animate) {
...
//-----------------------------------------------------------------------------
// Purpose: TF prevents drawing of any entity attached to players that aren't
// items in the inventory of the player.
//			This is to prevent servers creating fake cosmetic items
//and attaching them to players.
//-----------------------------------------------------------------------------
bool CSpriteTrail::ValidateEntityAttachedToPlayer(bool &bShouldRetry) {
  ...
  {
          // We attach sprites directly to players in some cases, such as phase
trails on an evading scout for ( int i=0; i<ARRAYSIZE( g_spriteWhiteList ); ++i
)
          {
