...
public:
  ...
  virtual void GetRandomPointsOnControllingObjectHitBox(
      ...
      Vector *pPntsOut, Vector vecDirectionalBias,
...
//-----------------------------------------------------------------------------
#ifdef CLIENT_DLL
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CParticleSystemQuery, IParticleSystemQuery,
                                  PARTICLE_SYSTEM_QUERY_INTERFACE_VERSION,
...
// Inherited from IParticleSystemQuery
//-----------------------------------------------------------------------------
void CParticleSystemQuery::GetLightingAtPoint(const Vector &vecOrigin,
                                              Color &cTint) {
#ifdef GAME_DLL
  ...
  if (engine->IsInGame()) {
    ...
    cTint.SetColor(totalColor.x * 255, totalColor.y * 255, totalColor.z * 255,
                   0);
  } else {
    // FIXME: Go through to the engine from the server to get these values
    cTint.SetColor(255, 255, 255, 255);
...
void CParticleSystemQuery::TraceLine(const Vector &vecAbsStart,
  ...
  if (bDoTrace) {
    trace_t tempTrace;
    UTIL_TraceLine(vecAbsStart, vecAbsEnd, mask, ignore, collisionGroup,
                   &tempTrace);
...
void CParticleSystemQuery::GetRandomPointsOnControllingObjectHitBox(
  ...
  EHANDLE *phMoveParent = reinterpret_cast<EHANDLE *>(
      pParticles->m_ControlPoints[nControlPointNumber].m_pObject);
  ...
  if (pMoveParent) {
    ...
    Vector vecBasePos;
    pParticles->GetControlPointAtTime(nControlPointNumber,
                                      pParticles->m_flCurTime, &vecBasePos);
          ...
          if (set) {
            ...
            for (int i = 0; i < nNumPtsOut; i++) {
              ...
              do {
                ...
                vecLocalPosition.x = GetSurfaceCoord(
                    flTryU, pBox->bbmin.x * pAnimating->GetModelScale(),
                    pBox->bbmax.x * pAnimating->GetModelScale());
                vecLocalPosition.y = GetSurfaceCoord(
                    flTryV, pBox->bbmin.y * pAnimating->GetModelScale(),
                    pBox->bbmax.y * pAnimating->GetModelScale());
                vecLocalPosition.z = GetSurfaceCoord(
                    flTryW, pBox->bbmin.z * pAnimating->GetModelScale(),
                    pBox->bbmax.z * pAnimating->GetModelScale());

                Vector vecTryWorldPosition;

                VectorTransform(vecLocalPosition, *hitboxbones[pBox->bone],
                                vecTryWorldPosition);
    ...
    if (pMoveParent->IsBrushModel()) {
      ...
      for (int i = 0; i < nNumPtsOut; i++) {
        ...
        do {
          ...
          Vector vecTryWorldPosition;
          VectorTransform(vecLocalPosition, matOrientation,
                          vecTryWorldPosition);
...
#endif
  if (!bSucesss) {
    // don't have a model or am in editor or something - fill return with
    // control point
    for (int i = 0; i < nNumPtsOut; i++) {
      pPntsOut[i] = pParticles->m_ControlPoints[nControlPointNumber]
                        .m_Position; // fallback if anything goes wrong
...
int CParticleSystemQuery::GetCollisionGroupFromName(
...
void CParticleSystemQuery::GetLocalPlayerEyeVectors(Vector *pForward,
                                                    Vector *pRight,
                                                    Vector *pUp) {
#ifdef CLIENT_DLL
  C_BasePlayer *pPlayer = C_BasePlayer::GetLocalPlayer();
...
float CParticleSystemQuery::GetPixelVisibility(int *pQueryHandle,
