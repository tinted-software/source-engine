...
// NOTE: If CStringRegistry allowed storing arbitrary data, we could just use
// that. in this case we have the "isPrivate" member and the replacement rules
// (eventIndex can be reused by private activities), so a custom table is
// necessary
struct eventlist_t {
  int eventIndex;
  int iType;
...
// This stores the actual event names.  Also, the string ID in the registry is
// simply an index into the g_EventList array.
...
eventlist_t *EventList_AddEventEntry(const char *pName, int iEventIndex,
                                     bool isPrivate, int iType) {
  MEM_ALLOC_CREDIT();
  ...
  // UNDONE: This implies that ALL shared activities are added before ANY custom
  // activities UNDONE: Segment these instead?  It's a 32-bit int, how many
...
bool EventList_RegisterSharedEvent(const char *pszEventName, int iEventIndex,
                                   int iType) {
  // UNDONE: Do we want to do these checks when not in developer mode? or maybe
  // DEBUG only? They really only matter when you change the list of code
  // controlled activities.  IDs for content controlled activities never collide
...
Animevent EventList_RegisterPrivateEvent(const char *pszEventName) {
  eventlist_t *pList = ListFromString(pszEventName);
  if (pList) {
    // this activity is already in the list. If the activity we collided with is
    // also private, then the collision is OK. Otherwise, it's a bug.
    if (pList->isPrivate) {
      return (Animevent)pList->eventIndex;
    } else {
      // this private activity collides with a shared activity. That is not
      // allowed.
  ...
  pList = EventList_AddEventEntry(pszEventName, g_HighestEvent + 1, true,
                                  AE_TYPE_SERVER);
  return (Animevent)pList->eventIndex;
}
...
void EventList_RegisterSharedEvents(void) {
  ...
  REGISTER_SHARED_ANIMEVENT(AE_NPC_WEAPON_FIRE,
  ...
  REGISTER_SHARED_ANIMEVENT(AE_CL_BODYGROUP_SET_VALUE_CMODEL_WPN,
                            AE_TYPE_CLIENT);

  REGISTER_SHARED_ANIMEVENT(AE_WPN_PRIMARYATTACK,
                            AE_TYPE_CLIENT | AE_TYPE_SERVER);
  REGISTER_SHARED_ANIMEVENT(AE_WPN_INCREMENTAMMO,
                            AE_TYPE_CLIENT | AE_TYPE_SERVER);
  ...
  REGISTER_SHARED_ANIMEVENT(AE_WPN_PLAYWPNSOUND,
                            AE_TYPE_CLIENT | AE_TYPE_SERVER);
}