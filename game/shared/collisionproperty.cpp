...
//-----------------------------------------------------------------------------
class CDirtySpatialPartitionEntityList : public CAutoGameSystem,
                                         public IPartitionQueryCallback {
public:
...
// Singleton instance
//-----------------------------------------------------------------------------
static CDirtySpatialPartitionEntityList
    s_DirtyKDTree("CDirtySpatialPartitionEntityList");
...
void CDirtySpatialPartitionEntityList::OnPreQuery(
  ...
  // if you're holding a read lock, then these are entities that were still
  // dirty after your trace started or became dirty due to some other thread or
  // callback. Updating them may cause corruption further up the stack (e.g.
  // partition iterator).  Ignoring the state change should be safe since it
  // happened after the trace was requested or was unable to be resolved in a
  // previous attempt (still dirty).
  if (m_DirtyEntities.Count() && !m_nReadLockCount[nThreadID]) {
    ...
    while ((pCurrent = m_DirtyEntities.Detach()) != NULL) {
      while (pCurrent) {
        ...
        if (pEntity) {
          // If an entity is in the middle of bone setup, don't call
          // UpdatePartition
          //  which can cause it to redo bone setup on the same frame causing a
          //  recursive call to bone setup.
...
// Makes sure all entries in the KD tree are in the correct position
//-----------------------------------------------------------------------------
void CDirtySpatialPartitionEntityList::OnPostQuery(
    SpatialPartitionListMask_t listMask) {
#ifdef CLIENT_DLL
...
static void RecvProxy_OBBMinsPreScaled(const CRecvProxyData *pData,
...
static void RecvProxy_OBBMaxsPreScaled(const CRecvProxyData *pData,
                                       void *pStruct, void *pOut) {
  CCollisionProperty *pProp = ((CCollisionProperty *)pStruct);
...
static void RecvProxy_VectorDirtySurround(const CRecvProxyData *pData,
                                          void *pStruct, void *pOut) {
  Vector &vecold = *((Vector *)pOut);
  Vector vecnew(pData->m_Value.m_Vector[0], pData->m_Value.m_Vector[1],
...
static void RecvProxy_IntDirtySurround(const CRecvProxyData *pData,
                                       void *pStruct, void *pOut) {
  if (*((unsigned char *)pOut) != pData->m_Value.m_Int) {
    *((unsigned char *)pOut) = pData->m_Value.m_Int;
    ((CCollisionProperty *)pStruct)->MarkSurroundingBoundsDirty();
        ...
        static void SendProxy_Solid(const SendProp *pProp, const void *pStruct,
                                    const void *pData, DVariant *pOut,
                                    int iElement, int objectID) {
  pOut->m_Int = ((CCollisionProperty *)pStruct)->GetSolid();
}

static void SendProxy_SolidFlags(const SendProp *pProp, const void *pStruct,
                                 const void *pData, DVariant *pOut,
                                 int iElement, int objectID) {
...
RecvPropVector(RECVINFO(m_vecMinsPreScaled), 0, RecvProxy_OBBMinsPreScaled),
    ...
    RecvPropVector(RECVINFO(m_vecSpecifiedSurroundingMinsPreScaled), 0,
                   RecvProxy_VectorDirtySurround),
    RecvPropVector(RECVINFO(m_vecSpecifiedSurroundingMaxsPreScaled), 0,
                   RecvProxy_VectorDirtySurround),
    RecvPropVector(RECVINFO(m_vecSpecifiedSurroundingMins), 0,
                   RecvProxy_VectorDirtySurround),
    RecvPropVector(RECVINFO(m_vecSpecifiedSurroundingMaxs), 0,
                   RecvProxy_VectorDirtySurround),
#else
SendPropVector(SENDINFO(m_vecMinsPreScaled), 0, SPROP_NOSCALE),
    ...
    SendPropInt(SENDINFO(m_usSolidFlags), FSOLID_MAX_BITS, SPROP_UNSIGNED,
                SendProxy_SolidFlags),
    SendPropInt(SENDINFO(m_nSurroundType), SURROUNDING_TYPE_BIT_COUNT,
                SPROP_UNSIGNED),
    SendPropInt(SENDINFO(m_triggerBloat), 0, SPROP_UNSIGNED),
    SendPropVector(SENDINFO(m_vecSpecifiedSurroundingMinsPreScaled), 0,
                   SPROP_NOSCALE),
    SendPropVector(SENDINFO(m_vecSpecifiedSurroundingMaxsPreScaled), 0,
                   SPROP_NOSCALE),
...
CCollisionProperty::~CCollisionProperty() { DestroyPartitionHandle(); }
...
void CCollisionProperty::Init(CBaseEntity *pEntity) {
  ...
  // NOTE: This replicates previous behavior; we may always want to use
  // BEST_COLLISION_BOUNDS
...
// EntityHandle
//-----------------------------------------------------------------------------
IHandleEntity *CCollisionProperty::GetEntityHandle() { return m_pOuter; }
...
bool CCollisionProperty::ShouldTouchTrigger(int triggerSolidFlags) const {
  ...
  // triggers don't touch other triggers (might be solid to other ents as well
  // as trigger)
...
#ifndef CLIENT_DLL
    // UNDONE: This should be fine in the client DLL too.  Move GetAllChildren()
    // into shared code. If the root of the hierarchy is SOLID_BSP, then assume
    // that the designer wants the collisions to rotate with this hierarchy so
    // that the player can move while riding the hierarchy.
    if (!GetOuter()->GetMoveParent()) {
      // NOTE: This assumes things don't change back from SOLID_BSP
      // NOTE: This is 100% true for HL2 - need to support removing the flag to
      // support changing from SOLID_BSP
...
void CCollisionProperty::SetSolidFlags(int flags) {
  ...
  if ((oldFlags & (FSOLID_FORCE_WORLD_ALIGNED | FSOLID_USE_TRIGGER_BOUNDS)) !=
      (m_usSolidFlags &
       (FSOLID_FORCE_WORLD_ALIGNED | FSOLID_USE_TRIGGER_BOUNDS))) {
    MarkSurroundingBoundsDirty();
  }

  if ((oldFlags & (FSOLID_NOT_SOLID | FSOLID_TRIGGER)) !=
...
//-----------------------------------------------------------------------------
void CCollisionProperty::SetCollisionBounds(const Vector &mins,
                                            const Vector &maxs) {
...
//-----------------------------------------------------------------------------
// Rebuilds the scaled bounds from the prescaled bounds after a model's scale
// has changed
//-----------------------------------------------------------------------------
void CCollisionProperty::RefreshScaledCollisionBounds(void) {
  SetCollisionBounds(m_vecMinsPreScaled, m_vecMaxsPreScaled);

  SurroundingBoundsType_t nSurroundType =
      static_cast<SurroundingBoundsType_t>(m_nSurroundType.Get());
...
// calculate this in SetCollisionBounds above and cache the results in a data
// member!
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
void CCollisionProperty::WorldSpaceTriggerBounds(Vector *pVecWorldMins,
...
//-----------------------------------------------------------------------------
bool CCollisionProperty::TestCollision(const Ray_t &ray,
                                       unsigned int fContentsMask,
                                       trace_t &tr) {
  return m_pOuter->TestCollision(ray, fContentsMask, tr);
}

bool CCollisionProperty::TestHitboxes(const Ray_t &ray,
...
//-----------------------------------------------------------------------------
const Vector &
CCollisionProperty::NormalizedToCollisionSpace(const Vector &in,
...
//-----------------------------------------------------------------------------
const Vector &
CCollisionProperty::CollisionToNormalizedSpace(const Vector &in,
                                               Vector *pResult) const {
  Vector vecSize = OBBSize();
  pResult->x =
      (vecSize.x != 0.0f) ? (in.x - m_vecMins.Get().x) / vecSize.x : 0.5f;
  pResult->y =
      (vecSize.y != 0.0f) ? (in.y - m_vecMins.Get().y) / vecSize.y : 0.5f;
  pResult->z =
      (vecSize.z != 0.0f) ? (in.z - m_vecMins.Get().z) / vecSize.z : 0.5f;
  return *pResult;
}
...
//-----------------------------------------------------------------------------
const Vector &
CCollisionProperty::NormalizedToWorldSpace(const Vector &in,
                                           Vector *pResult) const {
...
//-----------------------------------------------------------------------------
const Vector &
CCollisionProperty::WorldToNormalizedSpace(const Vector &in,
                                           Vector *pResult) const {
  Vector vecCollisionSpace;
...
//-----------------------------------------------------------------------------
void CCollisionProperty::RandomPointInBounds(const Vector &vecNormalizedMins,
                                             const Vector &vecNormalizedMaxs,
                                             Vector *pPoint) const {
  Vector vecNormalizedSpace;
  vecNormalizedSpace.x =
      random->RandomFloat(vecNormalizedMins.x, vecNormalizedMaxs.x);
  vecNormalizedSpace.y =
      random->RandomFloat(vecNormalizedMins.y, vecNormalizedMaxs.y);
  vecNormalizedSpace.z =
      random->RandomFloat(vecNormalizedMins.z, vecNormalizedMaxs.z);
  NormalizedToWorldSpace(vecNormalizedSpace, pPoint);
}

//-----------------------------------------------------------------------------
// Transforms an AABB measured in entity space to a box that surrounds it in
// world space
//-----------------------------------------------------------------------------
...
/*
void CCollisionProperty::WorldAABBToCollisionAABB( const Vector &worldMins,
const Vector &worldMaxs, Vector *pEntityMins, Vector *pEntityMaxs ) const
{
        if ( !IsBoundsDefinedInEntitySpace() || (GetCollisionAngles() ==
vec3_angle) )
        {
        ...
        {
                ITransformAABB( CollisionToWorldTransform(), worldMins,
worldMaxs, *pEntityMins, *pEntityMaxs );
        }
}
*/
...
bool CCollisionProperty::IsPointInBounds(const Vector &vecWorldPt) const {
  ...
  return ((vecLocalSpace.x >= m_vecMins.Get().x &&
           vecLocalSpace.x <= m_vecMaxs.Get().x) &&
          (vecLocalSpace.y >= m_vecMins.Get().y &&
           vecLocalSpace.y <= m_vecMaxs.Get().y) &&
          (vecLocalSpace.z >= m_vecMins.Get().z &&
           vecLocalSpace.z <= m_vecMaxs.Get().z));
}

//-----------------------------------------------------------------------------
// Computes the nearest point in the OBB to a point specified in world space
//-----------------------------------------------------------------------------
void CCollisionProperty::CalcNearestPoint(const Vector &vecWorldPt,
  ...
  CalcClosestPointOnAABB(m_vecMins.Get(), m_vecMaxs.Get(), localPt,
                         localClosestPt);
...
// Computes the nearest point in the OBB to a point specified in world space
//-----------------------------------------------------------------------------
float CCollisionProperty::CalcDistanceFromPoint(
    const Vector &vecWorldPt) const {
  // Calculate physics force
  Vector localPt, localClosestPt;
  WorldToCollisionSpace(vecWorldPt, &localPt);
  CalcClosestPointOnAABB(m_vecMins.Get(), m_vecMaxs.Get(), localPt,
...
float CCollisionProperty::ComputeSupportMap(const Vector &vecDirection) const {
  ...
  flResult +=
      ((vecCollisionDir.x >= 0.0f) ? m_vecMaxs.Get().x : m_vecMins.Get().x) *
      vecCollisionDir.x;
  flResult +=
      ((vecCollisionDir.y >= 0.0f) ? m_vecMaxs.Get().y : m_vecMins.Get().y) *
      vecCollisionDir.y;
  flResult +=
      ((vecCollisionDir.z >= 0.0f) ? m_vecMaxs.Get().z : m_vecMins.Get().z) *
      vecCollisionDir.z;
...
//-----------------------------------------------------------------------------
void CCollisionProperty::ComputeVPhysicsSurroundingBox(Vector *pVecWorldMins,
...
// Expand trigger bounds..
//-----------------------------------------------------------------------------
bool CCollisionProperty::ComputeHitboxSurroundingBox(Vector *pVecWorldMins,
                                                     Vector *pVecWorldMaxs) {
  CBaseAnimating *pAnim = GetOuter()->GetBaseAnimating();
  if (pAnim) {
...
// Expand trigger bounds..
//-----------------------------------------------------------------------------
bool CCollisionProperty::ComputeEntitySpaceHitboxSurroundingBox(
    Vector *pVecWorldMins, Vector *pVecWorldMaxs) {
  CBaseAnimating *pAnim = GetOuter()->GetBaseAnimating();
  if (pAnim) {
    return pAnim->ComputeEntitySpaceHitboxSurroundingBox(pVecWorldMins,
                                                         pVecWorldMaxs);
  }
...
//-----------------------------------------------------------------------------
void CCollisionProperty::ComputeRotationExpandedBounds(Vector *pVecWorldMins,
                                                       Vector *pVecWorldMaxs) {
  if (!IsBoundsDefinedInEntitySpace()) {
    ...
    *pVecWorldMaxs = m_vecMaxs;
    ...
    float flMaxVal;
    flMaxVal = MAX(FloatMakePositive(m_vecMins.Get().x),
                   FloatMakePositive(m_vecMaxs.Get().x));
    pVecWorldMins->x = -flMaxVal;
    pVecWorldMaxs->x = flMaxVal;

    flMaxVal = MAX(FloatMakePositive(m_vecMins.Get().y),
                   FloatMakePositive(m_vecMaxs.Get().y));
    pVecWorldMins->y = -flMaxVal;
    pVecWorldMaxs->y = flMaxVal;

    flMaxVal = MAX(FloatMakePositive(m_vecMins.Get().z),
                   FloatMakePositive(m_vecMaxs.Get().z));
...
// want...
//-----------------------------------------------------------------------------
void CCollisionProperty::ComputeCollisionSurroundingBox(bool bUseVPhysics,
                                                        Vector *pVecWorldMins,
                                                        Vector *pVecWorldMaxs) {
  Assert(GetSolid() != SOLID_CUSTOM);

  // NOTE: For solid none, we are still going to use the bounds; necessary
  // because the surrounding box is used for the PVS...
  // FIXME: Should we make some other call for the PVS stuff?? If so, we should
  // return a point bounds for SOLID_NONE...
...
// want...
//-----------------------------------------------------------------------------
void CCollisionProperty::ComputeSurroundingBox(Vector *pVecWorldMins,
                                               Vector *pVecWorldMaxs) {
  if ((GetSolid() == SOLID_CUSTOM) && (m_nSurroundType != USE_GAME_CODE)) {
    // NOTE: This can only happen in transition periods, say during network
    // reception on the client. We expect USE_GAME_CODE to be used with
    // SOLID_CUSTOM
    *pVecWorldMins = GetCollisionOrigin();
    *pVecWorldMaxs = *pVecWorldMins;
    return;
  ...
  case USE_OBB_COLLISION_BOUNDS: {
    ...
    if ((GetSolid() == SOLID_VPHYSICS) &&
        (GetOuter()->GetMoveType() == MOVETYPE_VPHYSICS)) {
  ...
  case USE_BEST_COLLISION_BOUNDS:
    Assert(GetSolid() != SOLID_CUSTOM);
    ComputeCollisionSurroundingBox((GetSolid() == SOLID_VPHYSICS),
  ...
  case USE_SPECIFIED_BOUNDS:
    VectorAdd(GetCollisionOrigin(), m_vecSpecifiedSurroundingMins,
              *pVecWorldMins);
    VectorAdd(GetCollisionOrigin(), m_vecSpecifiedSurroundingMaxs,
              *pVecWorldMaxs);
    break;
...
#ifdef DEBUG
  /*
  // For debugging purposes, make sure the bounds actually does surround the
  thing.
  // Otherwise the optimization we were using isn't really all that great, is
  it? Vector vecTestMins, vecTestMaxs; ComputeCollisionSurroundingBox(
  (GetSolid() == SOLID_VPHYSICS), &vecTestMins, &vecTestMaxs );
  ...
  Assert( vecTestMins.x >= pVecWorldMins->x && vecTestMins.y >= pVecWorldMins->y
  && vecTestMins.z >= pVecWorldMins->z ); Assert( vecTestMaxs.x <=
  pVecWorldMaxs->x && vecTestMaxs.y <= pVecWorldMaxs->y && vecTestMaxs.z <=
  pVecWorldMaxs->z );
  */
#endif
}
...
//-----------------------------------------------------------------------------
void CCollisionProperty::SetSurroundingBoundsType(SurroundingBoundsType_t type,
                                                  const Vector *pMins,
                                                  const Vector *pMaxs) {
...
#ifdef CLIENT_DLL
  g_pClientShadowMgr->MarkRenderToTextureShadowDirty(
      GetOuter()->GetShadowHandle());
#else
...
bool CCollisionProperty::DoesVPhysicsInvalidateSurroundingBox() const {
  ...
  case USE_OBB_COLLISION_BOUNDS:
    return (GetSolid() == SOLID_VPHYSICS) &&
           (GetOuter()->GetMoveType() == MOVETYPE_VPHYSICS) &&
...
// want...
//-----------------------------------------------------------------------------
void CCollisionProperty::WorldSpaceSurroundingBounds(Vector *pVecMins,
                                                     Vector *pVecMaxs) {
...
void CCollisionProperty::UpdatePartition() {
  if (m_pOuter->IsEFlagSet(EFL_DIRTY_SPATIAL_PARTITION)) {
    ...
    if (IsSolid() || IsSolidFlagSet(FSOLID_TRIGGER) ||
        m_pOuter->IsEFlagSet(EFL_USE_PARTITION_WHEN_NOT_SOLID)) {
      // Bloat a little bit...
      if (BoundingRadius() != 0.0f) {
        ...
        partition->ElementMoved(GetPartitionHandle(), vecSurroundMins,
                                vecSurroundMaxs);
      } else {
        partition->ElementMoved(GetPartitionHandle(), GetCollisionOrigin(),
                                GetCollisionOrigin());
      }
    }
  }
