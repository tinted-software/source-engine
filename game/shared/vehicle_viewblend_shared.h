...
struct ViewLockData_t {
  ...
  float
      flUnlockBlendInterval; // If this DOF is locked, how long to spend
                             // blending out of the locked view when we unlock.
};
...
struct ViewSmoothingData_t {
  ...
  QAngle vecAngleDiffSaved; // The original angular error between the entry/exit
                            // anim and player's view when we started playing
                            // the anim.
  QAngle vecAngleDiffMin; // Tracks the minimum angular error achieved so we can
                          // converge on the anim's angles.
};
