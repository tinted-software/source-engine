...
void ThirdPersonChange(IConVar *pConVar, const char *pOldValue,
                       float flOldValue) {
...
ConVar cl_thirdperson("cl_thirdperson", "0",
                      FCVAR_NOT_CONNECTED | FCVAR_USERINFO | FCVAR_ARCHIVE |
...
void CThirdPersonManager::Update(void) {
  ...
  if (sv_cheats && !sv_cheats->GetBool() && GameRules() &&
      GameRules()->AllowThirdPersonCamera() == false) {
  ...
  if (IsOverridingThirdPerson() == false) {
    if ((bool)input->CAM_IsThirdPerson() !=
            (cl_thirdperson.GetBool() || m_bForced) &&
        GameRules() && GameRules()->AllowThirdPersonCamera() == true) {
...
Vector CThirdPersonManager::GetDesiredCameraOffset(void) {
  if (IsOverridingThirdPerson() == true) {
    return Vector(cam_idealdist.GetFloat(), cam_idealdistright.GetFloat(),
                  cam_idealdistup.GetFloat());
  }
...
Vector CThirdPersonManager::GetDistanceFraction(void) {
  ...
  float flFrac = RemapValClamped(gpGlobals->curtime - m_flLerpTime, 0,
                                 CAMERA_OFFSET_LERP_TIME, 0, 1);
  ...
  flFrac = RemapValClamped(gpGlobals->curtime - m_flUpLerpTime, 0,
                           CAMERA_UP_OFFSET_LERP_TIME, 0, 1);
...
void CThirdPersonManager::PositionCamera(CBasePlayer *pPlayer, QAngle angles) {
  if (pPlayer) {
    ...
    Vector vecCamOffset =
        endPos + (camForward * -GetDesiredCameraOffset()[DIST_FORWARD]) +
    ...
    UTIL_TraceHull(endPos, vecCamOffset, CAM_HULL_MIN, CAM_HULL_MAX,
                   MASK_SOLID & ~CONTENTS_MONSTER, &traceFilter, &trace);
    ...
    if (trace.fraction < 1.0) {
      ...
      UTIL_TraceHull(endPos,
                     endPos +
                         (camForward * -GetDesiredCameraOffset()[DIST_FORWARD]),
                     CAM_HULL_MIN, CAM_HULL_MAX, MASK_SOLID & ~CONTENTS_MONSTER,
                     &traceFilter, &trace);
...
bool CThirdPersonManager::WantToUseGameThirdPerson(void) {
  return cl_thirdperson.GetBool() && GameRules() &&
         GameRules()->AllowThirdPersonCamera() &&
