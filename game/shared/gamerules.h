...
public:
  ...
  virtual void Think(void) = 0; // GR_Think - runs every server frame, should
                                // handle any timer tasks, periodic events, etc.
  ...
  virtual bool ClientConnected(
      ...
      char *reject, int maxrejectlen) = 0; // a client just connected to the
                                           // server (player hasn't spawned yet)
  ...
  virtual int IPointsForKill(
      ...
      CBasePlayer *
          pKilled) = 0; // how many points do I award whoever kills this player?
  ...
  virtual void PlayerGotItem(CBasePlayer * pPlayer,
                             CItem *
                                 pItem) = 0; // call each time a player picks up
                                             // an item (battery, healthkit)
  ...
  virtual void PlayerGotAmmo(CBaseCombatCharacter * pPlayer, char *szName,
                             int iCount) = 0; // called each time a player picks
                                              // up some ammo in the world
