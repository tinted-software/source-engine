...
bool MapEntity_ExtractValue(const char *pEntData, const char *keyName,
  ...
  while (inputData) {
    ...
    if (!strcmp(token, keyName)) {
      inputData =
          MapEntity_ParseToken(inputData, token); // get value and return it
...
//			the token gets fully read no matter what the length, but
//only MAPKEY_MAXLENGTH 			characters are written into newToken
// Input  : char *data - the data to parse
//			char *newToken - the buffer into which the new token is
//written 			char *braceChars - a string of characters that constitute braces. this
//pointer needs to be 			distince for each set of braceChars, since the usage is
//cached.
// Output : const char * - returns a pointer to the position in the data
// following the newToken
//-----------------------------------------------------------------------------
...
bool CEntityMapData::SetValue(const char *keyName, char *NewValue,
                              int nKeyInstance) {
  // If this is -1, the size of the string is unknown and cannot be safely
  // modified!
  ...
  while (inputData) {
    ...
    if (!strcmp(token, keyName)) {
      ...
      if (nCurrKeyInstance > nKeyInstance) {
        ...
        prevData = inputData;
        inputData =
            (char *)MapEntity_ParseToken(inputData, token); // get keyname
        ...
        int iPadding = iNewValueLen - Q_strlen(token) -
                       2; // -2 for the quotes (token doesn't have them)

        // prevData has a space at the start, seperating the value from the key.
        // Add 1 to prevData when pasting in the new Value, to account for the
        // space.
        Q_strncpy(prevData + 1, newvaluebuf,
                  iNewValueLen + 1); // +1 for the null terminator
        Q_strcat(prevData, postData,
                 m_nEntDataSize - ((prevData - m_pEntData) + 1));
    ...
    inputData =
        (char *)MapEntity_ParseToken(inputData, token); // skip over value
  }
