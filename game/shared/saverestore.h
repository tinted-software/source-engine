...
public:
  ...
  void
  WriteString(const string_t *stringId,
  ...
  void
  WriteString(const char *pname, const string_t *stringId,
              int count = 1); // Save a null-terminated string (engine string)
  ...
  void
  WritePositionVector(const char *pname,
  ...
  virtual void WriteTick(const int *value,
                         int count = 1); // Save a int (timevalue)
  virtual void
  WritePositionVector(const Vector &value); // Offset for landmark if necessary
