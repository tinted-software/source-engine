...
extern bool ParseKeyvalue(void *pObject, typedescription_t *pFields,
                          int iNumFields, const char *szKeyName,
                          const char *szValue);
extern bool ExtractKeyvalue(void *pObject, typedescription_t *pFields,
                            int iNumFields, const char *szKeyName,
...
// Set default max values for entities based on the existing constants from
// elsewhere
float k_flMaxEntityPosCoord = MAX_COORD_FLOAT;
float k_flMaxEntityEulerAngle =
    360.0 * 1000.0f; // really should be restricted to +/-180, but some code
                     // doesn't adhere to this.  let's just trap NANs, etc
...
ConVar ai_debug_shoot_positions("ai_debug_shoot_positions", "0",
                                FCVAR_REPLICATED | FCVAR_CHEAT);

// Utility func to throttle rate at which the "reasonable position" spew goes
// out
...
bool CheckEmitReasonablePhysicsSpew() {
  ...
  if (now >= s_LastEntityReasonableEmitTime &&
      now < s_LastEntityReasonableEmitTime + 5.0) {
  ...
  if (nEffects != m_fEffects) {
    ...
    if (!IsPlayer()) {
      if ((nEffects & (EF_BRIGHTLIGHT | EF_DIMLIGHT)) &&
          !(m_fEffects & (EF_BRIGHTLIGHT | EF_DIMLIGHT))) {
        AddEntityToDarknessCheck(this);
      } else if (!(nEffects & (EF_BRIGHTLIGHT | EF_DIMLIGHT)) &&
                 (m_fEffects & (EF_BRIGHTLIGHT | EF_DIMLIGHT))) {
...
void CBaseEntity::ParseMapData(CEntityMapData *mapData) {
  ...
  // loop through all keys in the data block and pass the info back into the
  // object
...
bool CBaseEntity::KeyValue(const char *szKeyName, const char *szValue) {
  ...
  if (FStrEq(szKeyName, "angle")) {
    ...
    if (y >= 0) {
      Q_snprintf(szBuf, sizeof(szBuf), "%f %f %f", GetLocalAngles()[0], y,
                 GetLocalAngles()[2]);
    } else if ((int)y == -1) {
  ...
  if (FStrEq(szKeyName, "angles")) {
    ...
    // calling SetLocalAngles from within a KeyValues method.. use SetAbsAngles
    // instead!
  ...
  if (FStrEq(szKeyName, "origin")) {
    ...
    // If you're hitting this assert, it's probably because you're
    // calling SetLocalOrigin from within a KeyValues method.. use SetAbsOrigin
    // instead!
    Assert((GetMoveParent() == NULL) && !IsEFlagSet(EFL_DIRTY_ABSTRANSFORM));
  ...
  if (!*ent_debugkeys.GetString()) {
    for (datamap_t *dmap = GetDataDescMap(); dmap != NULL;
         dmap = dmap->baseMap) {
      if (::ParseKeyvalue(this, dmap->dataDesc, dmap->dataNumFields, szKeyName,
                          szValue))
        return true;
    }
  } else {
    ...
    if (*ent_debugkeys.GetString() &&
        !Q_stricmp(ent_debugkeys.GetString(), STRING(m_iClassname))) {
...
bool CBaseEntity::KeyValue(const char *szKeyName, const Vector &vecValue) {
  ...
  Q_snprintf(string, sizeof(string), "%f %f %f", vecValue.x, vecValue.y,
             vecValue.z);
...
bool CBaseEntity::GetKeyValue(const char *szKeyName, char *szValue,
                              int iMaxLen) {
  if (FStrEq(szKeyName, "rendercolor") || FStrEq(szKeyName, "rendercolor32")) {
  ...
  if (FStrEq(szKeyName, "origin")) {
    ...
    Q_snprintf(szValue, iMaxLen, "%f %f %f", vecOrigin.x, vecOrigin.y,
               vecOrigin.z);
    return true;
  }
  ...
  for (datamap_t *dmap = GetDataDescMap(); dmap != NULL; dmap = dmap->baseMap) {
    if (::ExtractKeyvalue(this, dmap->dataDesc, dmap->dataNumFields, szKeyName,
                          szValue, iMaxLen))
      return true;
  }
#endif
...
//-----------------------------------------------------------------------------
void CBaseEntity::ImpactTrace(trace_t *pTrace, int iDamageType,
...
int CBaseEntity::GetIndexForThinkContext(const char *pszContext) {
  for (int i = 0; i < m_aThinkFunctions.Size(); i++) {
    if (!Q_strncmp(STRING(m_aThinkFunctions[i].m_iszContext), pszContext,
                   MAX_CONTEXT_LENGTH))
      return i;
  }
...
// Purpose:
//-----------------------------------------------------------------------------
BASEPTR CBaseEntity::ThinkSet(BASEPTR func, float thinkTime,
  ...
  // Find the think function in our list, and if we couldn't find it, register
  // it
  int iIndex = GetIndexForThinkContext(szContext);
  if (iIndex == NO_THINK_CONTEXT) {
...
#ifdef _DEBUG
  FunctionCheck(
      *(reinterpret_cast<void **>(&m_aThinkFunctions[iIndex].m_pfnThink)),
      szContext);
#endif
#endif

  if (thinkTime != 0) {
    int thinkTick = (thinkTime == TICK_NEVER_THINK) ? TICK_NEVER_THINK
                                                    : TIME_TO_TICKS(thinkTime);
...
int CBaseEntity::GetNextThinkTick(const char *szContext /*= NULL*/) {
  ...
  if ((iIndex == -1) ||
      (m_aThinkFunctions[iIndex].m_nNextThinkTick == TICK_NEVER_THINK)) {
    return TICK_NEVER_THINK;
  }
...
// NOTE: pass in the isThinking hint so we have to search the think functions
// less
void CBaseEntity::CheckHasThinkFunction(bool isThinking) {
  ...
  } else if (!isThinking && !IsEFlagSet(EFL_NO_THINK_FUNCTION) &&
             !WillThink()) {
...
void CBaseEntity::VPhysicsUpdate(IPhysicsObject *pPhysics) {
  ...
  case MOVETYPE_VPHYSICS: {
    ...
    if (IsEntityPositionReasonable(origin)) {
      ...
      if (CheckEmitReasonablePhysicsSpew()) {
        Warning("Ignoring unreasonable position (%f,%f,%f) from vphysics! "
                "(entity %s)\n",
...
IPhysicsObject *CBaseEntity::VPhysicsInitStatic(void) {
  ...
  if (GetSolid() == SOLID_BBOX) {
    pPhysicsObject = PhysModelCreateBox(this, WorldAlignMins(),
                                        WorldAlignMaxs(), GetAbsOrigin(), true);
  } else {
    pPhysicsObject = PhysModelCreateUnmoveable(this, GetModelIndex(),
...
bool CBaseEntity::VPhysicsInitSetup() {
  ...
  // If this entity already has a physics object, then it should have been
  // deleted prior to making this call.
...
//			physics alone determines where it goes (gravity,
//friction, etc) 			and the entity receives updates from vphysics.  SetAbsOrigin(),
//etc do not affect the object!
//-----------------------------------------------------------------------------
IPhysicsObject *CBaseEntity::VPhysicsInitNormal(SolidType_t solidType,
                                                int nSolidFlags,
                                                bool createAsleep,
                                                solid_t *pSolid) {
  if (!VPhysicsInitSetup())
  ...
  IPhysicsObject *pPhysicsObject = PhysModelCreate(
      this, GetModelIndex(), GetAbsOrigin(), GetAbsAngles(), pSolid);
  if (pPhysicsObject) {
    VPhysicsSetObject(pPhysicsObject);
...
IPhysicsObject *CBaseEntity::VPhysicsInitShadow(bool allowPhysicsMovement,
  ...
  if (GetSolid() == SOLID_BBOX) {
    // adjust these so the game tracing epsilons match the physics minimum
    // separation distance this will shrink the vphysics version of the model by
    // the difference in epsilons
    ...
    pPhysicsObject =
        PhysModelCreateOBB(this, CollisionProp()->OBBMins(),
                           CollisionProp()->OBBMaxs(), origin, angles, false);
  } else {
    pPhysicsObject =
        PhysModelCreate(this, GetModelIndex(), origin, angles, pSolid);
  }
  if (!pPhysicsObject)
    return NULL;
  ...
  pPhysicsObject->SetShadow(1e4, 1e4, allowPhysicsMovement,
...
void CBaseEntity::InvalidatePhysicsRecursive(int nChangeFlags) {
  ...
  // 1) If the origin changes, then we have to update abstransform, Shadow
  // projection, PVS, KD-tree,
  //    client-leaf system.
  // 2) If the angles change, then we have to update abstransform, Shadow
  // projection,
  ...
  //	  If the surrounding bounds actually update, when we also need to update
  //the KD-tree and the PVS.
  // 3) If it's due to attachment, then all children who are attached to an
  // attachment point
  //    are assumed to have dirty origin + angles.

  // Other stuff:
  // 1) Marking the surrounding bounds dirty will automatically mark KD tree +
  // PVS dirty.
  ...
  if (nChangeFlags & ANIMATION_CHANGED) {
    ...
    if (!(nChangeFlags &
          (POSITION_CHANGED | VELOCITY_CHANGED | ANGLES_CHANGED))) {
  ...
  for (CBaseEntity *pChild = FirstMoveChild(); pChild;
       pChild = pChild->NextMovePeer()) {
    // If this is due to the parent animating, only invalidate children that are
    // parented to an attachment Entities that are following also access
    // attachments points on parents and must be invalidated.
  ...
  //
  // This code should really be in here, or the bone cache should not be in
  // world space. Since the bone transforms are in world space, if we move or
  // rotate the entity, its bones should be marked invalid.
  //
  // As it is, we're near ship, and don't have time to setup a good A/B test of
  // how much overhead this fix would add. We've also only got one known case
  // where the lack of this fix is screwing us, and I just fixed it, so I'm
  // leaving this commented out for now.
  //
  // Hopefully, we'll put the bone cache in entity space and remove the need for
  // this fix.
  //
  // #ifdef CLIENT_DLL
  //	if ( nChangeFlags & (POSITION_CHANGED | ANGLES_CHANGED |
  //ANIMATION_CHANGED) )
  //	{
  ...
  bool ShouldHitEntity(IHandleEntity *pHandleEntity, int contentsMask) {
    if (m_PassEntities.Count()) {
      ...
      if (pEntity && pPassEntity && pEntity->GetOwnerEntity() == pPassEntity &&
          pPassEntity->IsSolidFlagSet(FSOLID_NOT_SOLID) &&
          pPassEntity->IsSolidFlagSet(FSOLID_CUSTOMBOXTEST) &&
...
void CBaseEntity::FireBullets(const FireBulletsInfo_t &info) {
  ...
  traceFilter.SetPassEntity(
      this); // Standard pass entity for THIS so that it can be easily removed
             // from the list after passing through a portal
  ...
  if (MyCombatCharacterPointer() != NULL &&
      MyCombatCharacterPointer()->IsInAVehicle()) {
    traceFilter.AddEntityToIgnore(
        MyCombatCharacterPointer()->GetVehicleEntity());
  }
#endif // SERVER_DLL
  ...
  if (bUnderwaterBullets) {
    bStartedInWater = (enginetrace->GetPointContents(info.m_vecSrc) &
  ...
  for (int iShot = 0; iShot < info.m_iShots; iShot++) {
    ...
    // If we're firing multiple shots, and the first shot has to be bang on
    // target, ignore spread
    if (iShot == 0 && info.m_iShots > 1 &&
    ...
    if (IsPlayer() && info.m_iShots > 1 && iShot % 2) {
      // Half of the shotgun pellets are hulls that make it easier to hit
      // targets with the shotgun.
#ifdef PORTAL
      Ray_t rayBullet;
      rayBullet.Init(info.m_vecSrc, vecEnd);
      pShootThroughPortal =
          UTIL_Portal_FirstAlongRay(rayBullet, fPortalFraction);
      if (!UTIL_Portal_TraceRay_Bullets(pShootThroughPortal, rayBullet,
                                        MASK_SHOT, &traceFilter, &tr)) {
        pShootThroughPortal = NULL;
      }
#else
      AI_TraceHull(info.m_vecSrc, vecEnd, Vector(-3, -3, -3), Vector(3, 3, 3),
                   MASK_SHOT, &traceFilter, &tr);
#endif // #ifdef PORTAL
    } else {
#ifdef PORTAL
...
#elif TF_DLL
      CTraceFilterIgnoreFriendlyCombatItems traceFilterCombatItem(
          this, COLLISION_GROUP_NONE, GetTeamNumber());
      if (TFGameRules() && TFGameRules()->GameModeUsesUpgrades()) {
        CTraceFilterChain traceFilterChain(&traceFilter,
                                           &traceFilterCombatItem);
    ...
    // Fixes bug where trace from turret with attachment point outside of
    // Vcollide
    //  starts solid so doesn't hit anything else in the world and the final
    //  coord is outside of the MAX_COORD_FLOAT range.  This cause trying to
    //  send the end pos of the tracer down to the client with an origin which
    //  is out-of-range for networking
    if (tr.startsolid) {
    ...
    if (bStartedInWater) {
      ...
      if (pShootThroughPortal) {
        ...
        UTIL_Portal_PointTransform(pShootThroughPortal->MatrixThisToLinked(),
                                   vBubbleEnd, vTransformedIntersection);
    ...
    CTakeDamageInfo triggerInfo(pAttacker, pAttacker, info.m_flDamage,
                                nDamageType);
    CalculateBulletDamageForce(&triggerInfo, info.m_iAmmoType, vecDir,
                               tr.endpos);
    ...
    if (tr.fraction != 1.0) {
      ...
      CSoundEnt::InsertSound(SOUND_BULLET_IMPACT, tr.endpos, 200, 0.5, this,
                             soundEntChannel);
#endif

      // See if the bullet ended up underwater + started out of the water
      if (!bHitWater && (enginetrace->GetPointContents(tr.endpos) &
                         (CONTENTS_WATER | CONTENTS_SLIME))) {
        bHitWater = HandleShotImpactingWater(info, vecEnd, &traceFilter,
                                             &vecTracerDest);
      }

      float flActualDamage = info.m_flDamage;

      // If we hit a player, and we have player damage specified, use that
      // instead Adrian: Make sure to use the currect value if we hit a vehicle
      ...
      if (flActualDamage == 0.0) {
        flActualDamage =
            g_pGameRules->GetAmmoDamage(pAttacker, tr.m_pEnt, info.m_iAmmoType);
      } else {
        nActualDamageType =
            nDamageType |
      ...
      if (!bHitWater ||
        ...
        CTakeDamageInfo dmgInfo(this, pAttacker, flActualDamage,
                                nActualDamageType);
        ModifyFireBulletsDamage(&dmgInfo);
        CalculateBulletDamageForce(&dmgInfo, info.m_iAmmoType, vecDir,
                                   tr.endpos);
    ...
    if (tr.m_pEnt != NULL) {
      ...
      if ((psurf != NULL) && (psurf->game.material == CHAR_TEX_GLASS) &&
        ...
        // Query the func_breakable for whether it wants to allow for bullet
        // penetration
    ...
    if ((info.m_iTracerFreq != 0) &&
        (tracerCount++ % info.m_iTracerFreq) == 0 && (bHitGlass == false)) {
      if (bDoServerEffects == true) {
        ...
        if (pShootThroughPortal) {
          Tracer.endpos =
              info.m_vecSrc + (vecEnd - info.m_vecSrc) * fPortalFraction;
        }
#endif // #ifdef PORTAL
        ...
        if (pShootThroughPortal) {
          Vector vTransformedIntersection;
          UTIL_Portal_PointTransform(pShootThroughPortal->MatrixThisToLinked(),
                                     Tracer.endpos, vTransformedIntersection);
          ...
          // Shooting through a portal, the damage direction is translated
          // through the passed-through portal so the damage indicator hud
          // animation is correct
          Vector vDmgOriginThroughPortal;
          UTIL_Portal_PointTransform(pShootThroughPortal->MatrixThisToLinked(),
                                     info.m_vecSrc, vDmgOriginThroughPortal);
    ...
    // NOTENOTE: We could expand this to a more general solution for various
    // material penetration types (wood, thin metal, etc)
  ...
  if (bDoServerEffects == false) {
    TE_HL2MPFireBullets(entindex(), tr.startpos, info.m_vecDirShooting,
                        info.m_iAmmoType, iEffectSeed, info.m_iShots,
                        info.m_vecSpread.x, bDoTracers, bDoImpacts);
  }
#endif
  ...
  if (IsPlayer() && flCumulativeDamage > 0.0f) {
    ...
    gamestats->Event_WeaponHit(pPlayer, info.m_bPrimaryAttack,
                               pPlayer->GetActiveWeapon()->GetClassname(),
                               dmgInfo);
  }
#endif
}
...
bool CBaseEntity::ShouldDrawUnderwaterBulletBubbles() {
  ...
  CBaseEntity *pPlayer =
      (gpGlobals->maxClients == 1) ? UTIL_GetLocalPlayer() : NULL;
...
bool CBaseEntity::HandleShotImpactingWater(const FireBulletsInfo_t &info,
  ...
  AI_TraceLine(info.m_vecSrc, vecEnd,
               (MASK_SHOT | CONTENTS_WATER | CONTENTS_SLIME), pTraceFilter,
               &waterTrace);

  // See if this is the point we entered
  if ((enginetrace->GetPointContents(waterTrace.endpos - Vector(0, 0, 0.1f)) &
       (CONTENTS_WATER | CONTENTS_SLIME)) == 0)
    return false;
  ...
  if (ShouldDrawUnderwaterBulletBubbles()) {
    CWaterBullet *pWaterBullet =
        (CWaterBullet *)CreateEntityByName("waterbullet");
    if (pWaterBullet) {
...
void CBaseEntity::DispatchTraceAttack(const CTakeDamageInfo &info,
...
void CBaseEntity::TraceAttack(const CTakeDamageInfo &info, const Vector &vecDir,
...
//-----------------------------------------------------------------------------
void CBaseEntity::ComputeTracerStartPosition(const Vector &vecShotSrc,
                                             Vector *pVecTracerStart) {
#ifndef HL2MP
  if (g_pGameRules->IsMultiplayer()) {
    // NOTE: we do this because in MakeTracer, we force it to use the attachment
    // position in multiplayer, so the results from this function should never
    // actually get used.
...
//			as they see fit.
//
// Input  : vecTracerSrc - the point at which to start the tracer (not always
// the
...
// Output :
//-----------------------------------------------------------------------------
void CBaseEntity::MakeTracer(const Vector &vecTracerSrc, const trace_t &tr,
                             int iTracerType) {
  ...
  switch (iTracerType) {
  case TRACER_LINE:
    UTIL_Tracer(vNewSrc, tr.endpos, entindex(), iAttachment, 0.0f, false,
                pszTracerName);
    break;

  case TRACER_LINE_AND_WHIZ:
    UTIL_Tracer(vNewSrc, tr.endpos, entindex(), iAttachment, 0.0f, true,
                pszTracerName);
    break;
  }
}
...
void CBaseEntity::TraceBleed(float flDamage, const Vector &vecDir, trace_t *ptr,
                             int bitsDamageType) {
  ...
  if (!(bitsDamageType & (DMG_CRUSH | DMG_BULLET | DMG_SLASH | DMG_BLAST |
                          DMG_CLUB | DMG_AIRBOAT)))
    return;
  ...
  for (i = 0; i < cCount; i++) {
    vecTraceDir = vecDir * -1; // trace in the opposite direction the shot came
                               // from (the direction the shot is going)
    ...
    AI_TraceLine(ptr->endpos, ptr->endpos + vecTraceDir * -flTraceDist,
                 MASK_SOLID_BRUSHONLY & ~CONTENTS_GRATE, this,
                 COLLISION_GROUP_NONE, &Bloodtr);
...
#ifdef CLIENT_DLL
  if (GameRules()) {
    params.m_pSoundName = GameRules()->TranslateEffectForVisionFilter(
        "sounds", params.m_pSoundName);
  }
#endif
}
...
void CBaseEntity::ApplyLocalAngularVelocityImpulse(
...
void CBaseEntity::CollisionRulesChanged() {
  // ivp maintains state based on recent return values from the collision
  // filter, so anything that can change the state that a collision filter will
  // return (like m_Solid) needs to call RecheckCollisionFilter.
  if (VPhysicsGetObject()) {
    extern bool PhysIsInCallback();
    if (PhysIsInCallback()) {
      Warning("Changing collision rules within a callback is likely to cause "
              "crashes!\n");
      Assert(0);
    }
