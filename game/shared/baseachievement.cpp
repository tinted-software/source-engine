...
void CBaseAchievement::FireGameEvent(IGameEvent *event) {
  ...
  // if the achievement only applies to a specific map, and it's not the current
  // map, skip it
  if (m_pMapNameFilter &&
      (0 != Q_strcmp(m_pAchievementMgr->GetMapName(), m_pMapNameFilter)))
    return;

  const char *name = event->GetName();
  if (0 == Q_strcmp(name, "teamplay_round_win")) {
    // if this is a round win and the achievement wants full round events only,
    // filter this out if this is not the end of a full round
    if ((m_iFlags & ACH_FILTER_FULL_ROUND_ONLY) &&
        (false == event->GetBool("full_round")))
      return;
  }
...
//			only be compiled into products they pertain to.  But if
//there are 			any game-specific achievements which need to be in a binary shared
//			across products (e.g. Ep1 & Ep2), use the game dir as a
//runtime 			filter.
//-----------------------------------------------------------------------------
...
//			is the default implementation, achievements can override
//to 			do special handling
//-----------------------------------------------------------------------------
void CBaseAchievement::Event_EntityKilled(CBaseEntity *pVictim,
                                          CBaseEntity *pAttacker,
                                          CBaseEntity *pInflictor,
                                          IGameEvent *event) {
  // extra paranoid check: should only get here if registered as a kill event
  // listener
...
void CBaseAchievement::IncrementCount(int iOptIncrement) {
  if (!IsAchieved() && LocalPlayerCanEarn()) {
    ...
    // if this achievement gets saved w/global state, flag our global state as
    // dirty
    ...
    // if this achievement's progress should be stored in Steam, set the steam
    // stat for it
    if (StoreProgressInSteam() && steamapicontext->SteamUserStats()) {
      // Set the Steam stat with the same name as the achievement.  Only cached
      // locally until we upload it.
      char pszProgressName[1024];
      Q_snprintf(pszProgressName, 1024, "%s_STAT", GetStat());
      bool bRet =
          steamapicontext->SteamUserStats()->SetStat(pszProgressName, m_iCount);
      if (!bRet) {
        DevMsg("ISteamUserStats::GetStat failed to set progress value in Steam "
               "for achievement %s\n",
               pszProgressName);
      }
...
void CBaseAchievement::HandleProgressUpdate() {
  ...
  if (m_iProgressMsgIncrement > 0)
    iProgress = m_iCount / m_iProgressMsgIncrement;

  // if we haven't already shown this progress step, show it
  if (iProgress > m_iProgressShown || m_iCount == 1) {
    ShowProgressNotification();
    // remember progress step shown so we don't show it again if the player
    // loads an earlier save game and gets past this point again
...
//			If it does get set, evaluate if this satisfies an
//achievement
//-----------------------------------------------------------------------------
void CBaseAchievement::EnsureComponentBitSetAndEvaluate(int iBitNumber) {
  ...
  if (0 == (iBitMask & m_iComponentBits)) {
    ...
    if (cc_achievement_debug.GetInt()) {
      Msg("Component %d for achievement %s found, but already had that "
          "component\n",
          iBitNumber, GetName());
    }
  }
  ...
  // (this fixes some older achievements that are stuck in the 9/9 state and
...
//-----------------------------------------------------------------------------
// Purpose: called after the data in this achievement has been restored from
// saved game
//-----------------------------------------------------------------------------
...
bool CBaseAchievement::ShouldSaveWithGame() {
  // save if we should get saved with the game, have a non-zero count, and have
  // not been achieved (at which point the achievement state gets saved
  // globally)
  return ((m_iFlags & ACH_SAVE_WITH_GAME) > 0 && (GetCount() > 0) &&
          !IsAchieved());
}
...
bool CBaseAchievement::ShouldSaveGlobal() {
  // save if we should get saved globally and have a non-zero count, or if we
  // have been achieved, or if the player has pinned this achievement to the HUD
  return (((m_iFlags & ACH_SAVE_GLOBAL) > 0 && (GetCount() > 0)) ||
          IsAchieved() || (m_iProgressShown > 0) || ShouldShowOnHUD());
}
...
bool CBaseAchievement::IsActive() {
  ...
  // if there's a map filter and we're not on the specified map, we're not
  // active
  if ((m_pMapNameFilter) &&
      (0 != Q_strcmp(m_pAchievementMgr->GetMapName(), m_pMapNameFilter)))
    return false;
...
void CFailableAchievement::PostRestoreSavedGame() {
  // if there is no activation event set for this achievement, it is always
...
void CFailableAchievement::OnMapEvent(const char *pEventName) {
  ...
  // if this is the evaluation event, see if we've failed or not
           ...
           0 == Q_stricmp(pEventName, GetEvaluationEventName())) {
    OnEvaluationEvent();
  }
}
...
void CAchievement_AchievedCount::SetAchievementsRequired(int iNumRequired,
                                                         int iLowRange,
                                                         int iHighRange) {
