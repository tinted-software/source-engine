...
// Output : static int
//-----------------------------------------------------------------------------
static int ClassFileLineHash(const char *classname, const char *module,
                             int line) {
  CRC32_t retval;
  ...
  // ACK, have to go lower case due to issues with .dsp having different cases
  // of drive
  //  letters, etc.!!!
...
//			line -
//-----------------------------------------------------------------------------
void CPredictableId::Init(int player, int command, const char *classname,
...
const char *CPredictableId::Describe(void) const {
  ...
  Q_snprintf(desc, sizeof(desc), "pl(%i) cmd(%i) hash(%i) inst(%i) ack(%s)",
             GetPlayer(), GetCommandNumber(), GetHash(), GetInstanceNumber(),
