...
ConVar mp_facefronttime(
    "mp_facefronttime", "3", FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY,
    "After this amount of time of standing in place but aiming to one side, go "
    "ahead and move feet to face upper body.");
...
void CBasePlayerAnimState::UpdateAimSequenceLayers(
  ...
  if (pTransitioner->m_animationQueue.Count() == 1) {
    ...
    *pDest1 = *pSource1;
    ...
    pDest1->m_flWeight =
        1 -
        pDest0->m_flWeight; // This layer just mirrors the other layer's weight
                            // (one fades in while the other fades out).
