...
static CUtlDict<FilePlayerClassInfo_t *, unsigned short>
...
// used to track whether or not two player classes have been mistakenly assigned
// the same slot
...
static ConCommand dod_reloadplayerclasses("dod_reloadplayerclasses",
                                          CC_ReloadPlayerClasses_f,
...
GetFilePlayerClassInfoFromHandle(PLAYERCLASS_FILE_INFO_HANDLE handle) {
...
#ifndef _XBOX
KeyValues *
ReadEncryptedKVPlayerClassFile(IFileSystem *filesystem,
  ...
  if (!pKV->LoadFromFile(filesystem, szFullName,
                         "GAME")) // try to load the normal .txt file first
  {
    if (pICEKey) {
...
void FilePlayerClassInfo_t::Parse(KeyValues *pKeyValuesData,
                                  const char *szPlayerClassName) {
  ...
  // Printable name
  Q_strncpy(m_szPrintName,
            pKeyValuesData->GetString("printname",
                                      "!! Missing printname on Player Class"),
            MAX_PLAYERCLASS_NAME_LENGTH);

  // Player Model
  Q_strncpy(m_szPlayerModel,
            pKeyValuesData->GetString("playermodel",
                                      "!! Missing playermodel on Player Class"),
            MAX_PLAYERCLASS_NAME_LENGTH);

  // Select command
  Q_strncpy(m_szSelectCmd,
            pKeyValuesData->GetString("selectcmd",
