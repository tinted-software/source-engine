...
struct LobbyPlayerInfo_t {
  ...
  bool
      m_bSquadSurplus; //< Did he present a voucher to get surplus for his squad
};
...
public:
  ...
  void (
      CTeamplayRoundBasedRules::*pfnEnterState)(); // Init and deinit the state.
  void (CTeamplayRoundBasedRules::*pfnLeaveState)();
  void (
      CTeamplayRoundBasedRules::*pfnThink)(); // Do a PreThink() in this state.
};
...
public:
  ...
  // Return false if players aren't allowed to cap points at this time (i.e. in
  // WaitingForPlayers)
  ...
  void GetMvMPotentialDefendersLobbyPlayerInfo(
...
protected:
  ...
  void
  State_Enter(gamerules_roundstate_t newState); // Initialize the new state.
  void State_Leave();                           // Cleanup the previous state.
  void State_Think();                           // Update the current state.
  static CGameRulesRoundStateInfo *
  State_LookupInfo(gamerules_roundstate_t
...
protected:
  ...
  bool m_bForceMapReset; // should the map be reset when a team wins and the
                         // round is restarted?
  bool m_bPrevRoundWasWaitingForPlayers; // was the previous map reset after a
  ...
  string_t m_iszFirstRoundPlayed; // store the first round played after a full
                                  // restart so we can pick a different one next
                                  // time if we have other options
