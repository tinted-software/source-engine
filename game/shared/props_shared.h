...
#define SF_PHYSPROP_MOTIONDISABLED                                             \
  0x000008 // motion disabled at startup (flag only valid in spawn - motion can
           // be enabled via input)
...
#define SF_PHYSPROP_PREVENT_PLAYER_TOUCH_ENABLE                                \
  0x000400 // If set, the player will not cause the object to enable its motion
           // when bumped into
...
#define SF_PHYSPROP_RADIUS_PICKUP                                              \
  0x004000 // For Xbox, makes small objects easier to pick up by allowing them
           // to be found
...
enum {
  ...
  PARSE_SUCCEEDED_ALLOWED_STATIC, // Parsed propdata. Prop allowed to be
                                  // prop_physics or prop_dynamic/prop_static.
...
struct breakablepropparams_t {
  breakablepropparams_t(const Vector _origin, const QAngle _angles,
      ...
      : origin(_origin), angles(_angles), velocity(_velocity),
...
void BuildGibList(CUtlVector<breakmodel_t> &list, int modelindex,
                  float defBurstScale, int defCollisionGroup);
CBaseEntity *
CreateGibsFromList(CUtlVector<breakmodel_t> &list, int modelindex,
