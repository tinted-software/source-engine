...
protected:
  ...
  int m_iPointValue; // # of points this achievement is worth (currently only
                     // used for XBox Live)
  bool
      m_bHideUntilAchieved; // should this achievement be hidden until achieved?
  ...
  const char *m_pInflictorEntityNameFilter; // if non-NULL, inflictor entity
                                            // name to filter with
  const char *
      m_pVictimClassNameFilter; // if non-NULL, victim class name to filter with
  const char *m_pAttackerClassNameFilter; // if non-NULL, attacker class name to
                                          // filter with
  ...
  bool m_bShowOnHUD; // if set, the player wants this achievement pinned to the
                     // HUD
...
protected:
  ...
  bool m_bActivated; // are we activated? (If there is a map event that turns us
                     // on, has that happened)
