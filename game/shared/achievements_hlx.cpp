...
DECLARE_ACHIEVEMENT(CAchievementHLXKillWithPhysicsObjects,
                    ACHIEVEMENT_HLX_KILL_ENEMIES_WITHPHYSICS,
...
class CAchievementHLXKillWithHopper : public CBaseAchievement {
  ...
  virtual void Event_EntityKilled(CBaseEntity *pVictim, CBaseEntity *pAttacker,
                                  CBaseEntity *pInflictor, IGameEvent *event) {
    // If we get here, a combine mine has killed a player enemy.  Now check and
    // see if the player planted it
...
DECLARE_ACHIEVEMENT(CAchievementHLXKillWithHopper,
                    ACHIEVEMENT_HLX_KILL_ENEMY_WITHHOPPERMINE,
...
class CAchievementHLXKillWithManhack : public CBaseAchievement {
  ...
  virtual void Event_EntityKilled(CBaseEntity *pVictim, CBaseEntity *pAttacker,
                                  CBaseEntity *pInflictor, IGameEvent *event) {
    // We've already filtered to only get called when a player enemy gets killed
    // with a manhack.  Now just check for the case of player smashing manhack
    // into something, in which case the manhack is both the victim and
...
DECLARE_ACHIEVEMENT(CAchievementHLXKillWithManhack,
                    ACHIEVEMENT_HLX_KILL_ENEMIES_WITHMANHACK,
...
protected:
  ...
  virtual void Event_EntityKilled(CBaseEntity *pVictim, CBaseEntity *pAttacker,
    ...
    if (pGrenade) {
      ...
      // check if player was most recent thrower, but the victim was the
      // original thrower
...
DECLARE_ACHIEVEMENT(CAchievementHLXKillSoldierWithOwnGrenade,
...
protected:
  ...
  virtual void Event_EntityKilled(CBaseEntity *pVictim, CBaseEntity *pAttacker,
                                  CBaseEntity *pInflictor, IGameEvent *event) {
    // to count # of kills with same energy ball, keep track of previous
    // inflictor
...
DECLARE_ACHIEVEMENT(CAchievementHLXKillWithOneEnergyBall,
                    ACHIEVEMENT_HLX_KILL_ENEMIES_WITHONEENERGYBALL,
                    "HLX_KILL_ENEMIES_WITHONEENERGYBALL", 5);

class CAchievementHLXKillEliteSoldierWithOwnEnergyBall
    : public CBaseAchievement {
protected:
  virtual void Init() {
    SetFlags(ACH_LISTEN_PLAYER_KILL_ENEMY_EVENTS | ACH_SAVE_WITH_GAME);
    SetInflictorFilter("prop_combine_ball");
...
DECLARE_ACHIEVEMENT(CAchievementHLXKillEliteSoldierWithOwnEnergyBall,
                    ACHIEVEMENT_HLX_KILL_ELITESOLDIER_WITHHISENERGYBALL,
                    "HLX_KILL_ELITESOLDIER_WITHHISENERGYBALL", 10);

//-----------------------------------------------------------------------------
// Purpose: Counts the accumulated # of primary and secondary attacks from all
//			weapons (except grav gun).  If bBulletOnly is true, only
//counts 			attacks with ammo that does bullet damage.
//-----------------------------------------------------------------------------
int CalcPlayerAttacks(bool bBulletOnly) {
  ...
  for (int i = 0; i < iWeapons; i++) {
    CBaseHLCombatWeapon *pWeapon =
        dynamic_cast<CBaseHLCombatWeapon *>(pPlayer->GetWeapon(i));
    if (pWeapon) {
      // add primary attacks if we were asked for all attacks, or only if it
      // uses bullet ammo if we were asked to count bullet attacks
      if (!bBulletOnly ||
          (pAmmoDef->m_AmmoType[pWeapon->GetPrimaryAmmoType()].nDamageType ==
           DMG_BULLET)) {
        iTotalAttacks += pWeapon->m_iPrimaryAttacks;
      }
      // add secondary attacks if we were asked for all attacks, or only if it
      // uses bullet ammo if we were asked to count bullet attacks
      if (!bBulletOnly ||
          (pAmmoDef->m_AmmoType[pWeapon->GetSecondaryAmmoType()].nDamageType ==
           DMG_BULLET)) {
...
#endif // ( defined( HL2_DLL ) || defined( HL2_EPISODIC ) ) && ( !defined (
       // PORTAL ) )
