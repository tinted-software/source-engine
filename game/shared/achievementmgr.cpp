...
static void WriteAchievementGlobalState(KeyValues *pKV,
                                        bool bPersistToSteamCloud = false)
...

{
  ...
  if (IsX360()) {
    Q_snprintf(szFilename, sizeof(szFilename), "cfg:/%s_GameState.txt",
               COM_GetModDirectory());
  } else {
...
void CAchievementMgr::PostInit() {
  if (!g_AchievementSaveThread.IsAlive()) {
    ...
    if (IsX360()) {
      ThreadSetAffinity(
          (ThreadHandle_t)g_AchievementSaveThread.GetThreadHandle(),
          XBOX_PROCESSOR_3);
    }
#endif // WIN32
  }
...
void CAchievementMgr::Shutdown() {
  ...
  SaveGlobalState(false); // we just told the thread to shutdown so don't try an
                          // async save here
...
#ifdef GAME_DLL
void CAchievementMgr::FrameUpdatePostEntityThink() { Update(0.0f); }
#endif
...
void CAchievementMgr::Update(float frametime) {
  ...
  // Call think functions. Work backwards, because we may remove achievements
  // from the list.
  ...
  for (int i = iCount - 1; i >= 0; i--) {
    if (m_vecThinkListeners[i].m_flThinkTime < gpGlobals->curtime) {
      ...
      // The think function may have pushed out the think time. If not, remove
...
//debugging purposes. 			Use GetAchievementByID for fast lookup.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Returns true if the achievement with the specified name has been
// achieved
//-----------------------------------------------------------------------------
...
void CAchievementMgr::DownloadUserData() {
  if (IsPC()) {
    ...
    uint bytes;
    int ret = xboxsystem->EnumerateAchievements(
        XBX_GetPrimaryUserId(), 0, 0, nTotalAchievements, &bytes, 0, false);
    ...
    if (bDownloadSuccessful) {
      ret = xboxsystem->EnumerateAchievements(XBX_GetPrimaryUserId(), 0, 0,
                                              nTotalAchievements, pBuffer,
                                              bytes, false);
    ...
    if (bDownloadSuccessful) {
      // Give live a chance to mark achievements as unlocked, in case the
      // achievement manager wasn't able to get that data (storage device
      // missing, read failure, etc)
      ...
      for (int i = 0; i < nTotalAchievements; ++i) {
        CBaseAchievement *pAchievement =
            GetAchievementByID(pXboxAchievements[i].dwId);
        if (!pAchievement)
          continue;
    ...
    if (pRemoteStorage) {
      if (pRemoteStorage->FileExists(szFilename)) {
        ...
        if (fileSize > 0) {
          ...
          if (pData) {
            int32 sizeRead =
                pRemoteStorage->FileRead(szFilename, pData, fileSize);

            if (sizeRead == fileSize) {
              // Write out data to a filesystem GameState file that can be read
              // by the original code below
...
void CAchievementMgr::AwardAchievement(int iAchievementID) {
...
#ifdef CLIENT_DLL
  if (gamestats) {
    gamestats->Event_AchievementProgress(pAchievement->GetAchievementID(),
                                         pAchievement->GetName());
  }
#endif
  ...
  if (IsPC()) {
    ...
    if (steamapicontext->SteamUserStats()) {
      ...
      bool bRet = steamapicontext->SteamUserStats()->SetAchievement(
          pAchievement->GetName());
      //		Assert( bRet );
      if (bRet) {
...
bool CAchievementMgr::CheckAchievementsEnabled() {
  ...
  if (TFGameRules() && TFGameRules()->IsInTraining() &&
      TFGameRules()->AllowTrainingAchievements() == false) {
    return false;
  }
...
//specified # of 			teammates.  Involves cross-process calls to Steam so this is
//mildly 			expensive, don't call this every frame.
//-----------------------------------------------------------------------------
    ...
    if ((iPlayerIndex != iLocalPlayerIndex) &&
      ...
      if (IsPC()) {
        ...
        if (!steamapicontext->SteamFriends()->HasFriend(
                steamID, /*k_EFriendFlagImmediate*/ 0x04))
          continue;
#endif
      } else if (IsX360()) {
        uint64 XUid[1];
...
//calls to Steam 			so this is mildly expensive, don't call this every frame.
//-----------------------------------------------------------------------------
bool CalcHasNumClanPlayers(int iClanTeammates) {
  ...
  if (IsPC()) {
    ...
    for (int iClan = 0; iClan < steamapicontext->SteamFriends()->GetClanCount();
         iClan++) {
        ...
        if ((iPlayerIndex != iLocalPlayerIndex) &&
          ...
          if (engine->GetPlayerInfo(iPlayerIndex, &pi) && (pi.friendsID)) {
            ...
            CSteamID steamID(
                ...
                steamapicontext->SteamUtils()->GetConnectedUniverse(),
                k_EAccountTypeIndividual);
            if (steamapicontext->SteamFriends()->IsUserInSource(steamID,
                                                                clanID)) {
              iClanMembersOnTeam++;
...
void CAchievementMgr::Steam_OnUserStatsStored(
  ...
  if (k_EResultOK != pUserStatsStored->m_eResult &&
      k_EResultInvalidParam != pUserStatsStored->m_eResult) {
    // We had some failure (like not connected or timeout) that means the stats
    // were not stored. Redirty to try again
    SetDirty(true);
  } else {
    if (k_EResultInvalidParam == pUserStatsStored->m_eResult) {
      // for whatever reason, stats and achievements were rejected by Steam. In
      // order to remain in synch, we get the current values/state from Steam.
      UpdateStateFromSteam_Internal();

      // In the case of k_EResultInvalidParam, some of the stats may have been
      // stored, so we should still fall through to fire events related to
    ...
    while (m_AchievementsAwarded.Count() > 0) {
      ...
      if (g_pGameRules && g_pGameRules->IsMultiplayer()) {
        ...
        if (pLocalPlayer) {
          ...
          // verify that it is still achieved (it could have been rejected by
          // Steam)
          if (pAchievement->IsAchieved()) {
            ...
            bool bAchieved;
            bool bRet =
                steamapicontext->SteamUserStats()->GetAchievementAndUnlockTime(
...
void CAchievementMgr::CheckMetaAchievements(void) {
  ...
  FOR_EACH_MAP(m_mapMetaAchievement, iAchievement) {
    CAchievement_AchievedCount *pMetaAchievement =
        m_mapMetaAchievement[iAchievement];
    if (!pMetaAchievement || pMetaAchievement->IsAchieved())
      continue;

    int iAchieved = 0;
    for (int i = pMetaAchievement->GetLowRange();
         i <= pMetaAchievement->GetHighRange(); i++) {
...
#ifndef NO_STEAM
  if (steamapicontext->SteamUserStats()) {
    steamapicontext->SteamUserStats()->ClearAchievement(
        pAchievement->GetName());
  }
#endif
...
void CAchievementMgr::SetAchievementThink(CBaseAchievement *pAchievement,
                                          float flThinkTime) {
...
void CAchievementMgr::UpdateStateFromSteam_Internal() {
  ...
  FOR_EACH_MAP(m_mapAchievement, i) {
    ...
    if (pAchievement->StoreProgressInSteam()) {
      ...
      bRet =
          steamapicontext->SteamUserStats()->GetStat(pszProgressName, &iValue);
      if (bRet) {
        pAchievement->SetCount(iValue);
        pAchievement->EvaluateNewAchievement();
      } else {
        DevMsg("ISteamUserStats::GetStat failed to get progress value from "
               "Steam for achievement %s\n",
               pszProgressName);
      }
    }
  }
