...
DEFINE_FIELD(vecAnglesSaved, FIELD_VECTOR),
    ...
    // 0 <= t < start :		f(t) = 0
    // start <= t <= end :	f(t) = end * spline(( t-start) / (end-start) )
    // // s curve between clamped and linear end < t :			f(t) = t
...
//			view depending on this vehicle's preferences. When
//vehicles wreck 			catastrophically, it's often better to lock the view for a
//little 			while until things settle down than to keep trying to clamp/flatten 			the
//view artificially because we can never really catch up with 			the chaotic
//flipping.
//-----------------------------------------------------------------------------
