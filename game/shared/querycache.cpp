...
ConVar sv_disable_querycache("sv_disable_querycache", "0", FCVAR_CHEAT,
                             "debug - disable trace query cache");

static QueryCacheEntry_t *
FindOrAllocateCacheEntry(QueryCacheKey_t const &entry) {
  QueryCacheEntry_t *pFound = NULL;
  // see if we find it
  for (QueryCacheEntry_t *pNode = s_HashChains[entry.m_nHashIdx].m_pHead; pNode;
       pNode = pNode->m_pNext) {
...
static QueryCacheEntry_t *
FindOrAllocateCacheEntry(EQueryType_t nType, CBaseEntity *pEntity1,
                         CBaseEntity *pEntity2, EEntityOffsetMode_t nMode1,
                         EEntityOffsetMode_t nMode2, unsigned int nTraceMask) {
...
bool QueryCacheKey_t::Matches(QueryCacheKey_t const *pNode) const {
  if ((pNode->m_Type != m_Type) || (pNode->m_nTraceMask != m_nTraceMask) ||
    ...
    return false;
  for (int i = 0; i < m_nNumValidPoints; i++) {
    if ((pNode->m_pEntities[i] != m_pEntities[i]) ||
...
static void CalculateOffsettedPosition(CBaseEntity *pEntity,
                                       EEntityOffsetMode_t nMode,
                                       Vector *pVecOut) {
  switch (nMode) {
...
void ProcessQueryCacheUpdate(QueryCacheUpdateRecord_t &workItem) {
  ...
  for (int i = 0; i < workItem.m_nNumHashChainsToUpdate; i++) {
    ...
    for (QueryCacheEntry_t *pEntry =
             s_HashChains[i + workItem.m_nStartHashChain].m_pHead;
         pEntry; pEntry = pNext) {
...
static void PreUpdateQueryCache() {
  // mdlcache->BeginCoarseLock();			// x360 only - will need
  // to port for this in the future
  mdlcache->BeginLock();
}

static void PostUpdateQueryCache() {
  mdlcache->EndLock();
  // mdlcache->EndCoarseLock();			// x360 only - will need to port
...
void UpdateQueryCache(void) {
  ...
  for (int i = 0; i < N_WAYS_TO_SPLIT_CACHE_UPDATE; i++) {
    ...
    if (i != N_WAYS_TO_SPLIT_CACHE_UPDATE - 1)
      workList[i].m_nNumHashChainsToUpdate =
          ARRAYSIZE(s_HashChains) / N_WAYS_TO_SPLIT_CACHE_UPDATE;
    else
      workList[i].m_nNumHashChainsToUpdate =
          ARRAYSIZE(s_HashChains) - nCurEntry;
    nCurEntry += ARRAYSIZE(s_HashChains) / N_WAYS_TO_SPLIT_CACHE_UPDATE;
  }
  ParallelProcess("ProcessQueryCacheUpdate", workList,
                  N_WAYS_TO_SPLIT_CACHE_UPDATE, ProcessQueryCacheUpdate,
                  PreUpdateQueryCache, PostUpdateQueryCache,
                  (sv_disable_querycache.GetBool()) ? 0 : INT_MAX);
  // now, we need to take all of the obsolete cache entries each thread
  // generated and add them to the victim cache
  for (int i = 0; i < N_WAYS_TO_SPLIT_CACHE_UPDATE; i++) {
...
CON_COMMAND_F(cl_querycache_stats,
              "Display status of the query cache (client only)", FCVAR_CHEAT)
#else
CON_COMMAND(sv_querycache_stats,
