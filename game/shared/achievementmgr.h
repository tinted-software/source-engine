...
private:
  ...
  CUtlVector<CBaseAchievement *>
      m_vecKillEventListeners; // vector of achievements that are listening for
                               // kill events
  CUtlVector<CBaseAchievement *>
      m_vecMapEventListeners; // vector of achievements that are listening for
                              // map events
  CUtlVector<CBaseAchievement *>
      m_vecComponentListeners; // vector of achievements that are listening for
                               // components that make up an achievement
  ...
  CUtlVector<achievementthink_t>
      m_vecThinkListeners; // vector of achievements that are actively thinking

  float m_flLevelInitTime;

  float m_flLastClassChangeTime; // Time when player last changed class
  float m_flTeamplayStartTime; // Time when player joined a non-spectating team.
                               // Not updated if she switches game teams;
                               // cleared if she joins spectator
  float m_iMiniroundsCompleted; // # of minirounds played since game start (for
                                // maps that have minirounds)
  char m_szMap[MAX_PATH];    // file base of map name, cached since we access it
                             // frequently in this form
  bool m_bGlobalStateDirty;  // do we have interesting state changes that needs
                             // to be saved?
