  ...
  if (pOther->m_takedamage != DAMAGE_NO) {
    ...
    if (pOther->IsPlayer()) {
      ...
      CTakeDamageInfo dmgInfo(this, GetOwnerEntity(),
                              sk_plr_dmg_xbow_bolt_npc.GetFloat() *
                                  g_pGameRules->GetDamageMultiplier(),
    ...
    UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + vForward * 128, MASK_OPAQUE,
                   pOther, COLLISION_GROUP_NONE, &tr2);
    ...
    SetNextThink(gpGlobals->curtime); // this will get changed below if the bolt
                                      // is allowed to stick in what it hit.
...
void CWeaponCrossbow::FireBolt(void) {
  ...
  if (g_pGameRules->IsMultiplayer() && m_fInZoom) {
    ...
    trace_t trace;
    UTIL_TraceLine(vecSrc, vecEnd, MASK_SHOT, GetOwner(), COLLISION_GROUP_NONE,
                   &trace);
