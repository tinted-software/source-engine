...
private:
  ...
  float m_flAmmoUseTime; // since we use < 1 point of ammo per update, we
                         // subtract ammo on a timer.
  float m_flShakeTime;
...
void CWeaponEgon::UseAmmo(int count) {
  ...
  if (pPlayer->GetAmmoCount(m_iPrimaryAmmoType) >= count)
    ...
    pPlayer->RemoveAmmo(pPlayer->GetAmmoCount(m_iPrimaryAmmoType),
                        m_iPrimaryAmmoType);
}
...
void CWeaponEgon::Fire(const Vector &vecOrigSrc, const Vector &vecDir) {
  ...
  trace_t tr;
  UTIL_TraceLine(vecOrigSrc, vecDest, MASK_SHOT, pPlayer, COLLISION_GROUP_NONE,
                 &tr);
  ...
  if (m_flDmgTime < gpGlobals->curtime) {
    ...
    if (pEntity->m_takedamage != DAMAGE_NO) {
      ClearMultiDamage();
      CTakeDamageInfo info(this, pPlayer,
                           sk_plr_dmg_egon_wide.GetFloat() *
                               g_pGameRules->GetDamageMultiplier(),
    ...
    if (g_pGameRules->IsMultiplayer()) {
      ...
      RadiusDamage(CTakeDamageInfo(this, pPlayer,
                                   sk_plr_dmg_egon_wide.GetFloat() *
                                       g_pGameRules->GetDamageMultiplier() / 4,
...
void CWeaponEgon::UpdateEffect(const Vector &startPoint,
...
void CWeaponEgon::CreateEffect(void) {
  ...
  m_hBeam->AddSpawnFlags(SF_BEAM_TEMPORARY); // Flag these to be destroyed on
                                             // save/restore or level transition
  ...
  m_hSprite->SetTransparency(kRenderGlow, 255, 255, 255, 255,
