...
void CWeaponGlock::Precache(void) { BaseClass::Precache(); }
...
void CWeaponGlock::GlockFire(float flSpread, float flCycleTime,
                             bool fUseAutoAim) {
  ...
  //	pPlayer->FireBullets( 1, vecSrc, vecAiming, Vector( flSpread, flSpread,
  //flSpread ), MAX_TRACE_LENGTH, m_iPrimaryAmmoType, 0 );
  FireBulletsInfo_t info(1, vecSrc, vecAiming,
                         Vector(flSpread, flSpread, flSpread), MAX_TRACE_LENGTH,
                         m_iPrimaryAmmoType);
...
bool CWeaponGlock::Reload(void) {
  ...
  if (m_iClip1 == 0)
    iResult =
        DefaultReload(GetMaxClip1(), GetMaxClip2(), ACT_GLOCK_SHOOT_RELOAD);
  else
