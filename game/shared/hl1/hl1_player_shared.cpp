...
class CPlayerAnimState : public IHL1MPPlayerAnimState,
                         public CBasePlayerAnimState {
public:
...
int CPlayerAnimState::CalcAimLayerSequence(float *flCycle,
  ...
  iSequence =
      CalcSequenceIndex("%s_%s_%s", pPrefix, pAimOrShoot, pWeaponSuffix);
...
void CPlayerAnimState::DoAnimationEvent(PlayerAnimEvent_t event, int nData) {
  if (event == PLAYERANIMEVENT_JUMP) {
    ...
    // The middle part of the aim layer sequence becomes "shoot" until that
...
float CPlayerAnimState::CalcMovementPlaybackRate(bool *bIsMoving) {
  ...
  Activity eActivity =
      GetOuter()->GetSequenceActivity(GetOuter()->GetSequence());

  if (eActivity == ACT_RUN || eActivity == ACT_WALK ||
      eActivity == ACT_CROUCH) {
    VectorNormalize(vel);
