...
DEFINE_FIELD(m_hOwner, FIELD_EHANDLE),
    ...
    // Function Pointers
    DEFINE_ENTITYFUNC(RocketTouch), DEFINE_THINKFUNC(IgniteThink),
...
void CRpgRocket::IgniteThink(void) {
  ...
  TE_BeamFollow(filter, 0.0, entindex(), m_iTrail, 0, 4, 5, 5, 0, 224, 224, 255,
                255);
...
void CRpgRocket::SeekThink(void) {
  ...
  while ((pOther = gEntList.FindEntityByClassname(pOther, "laser_spot")) !=
         NULL) {
    CLaserDot *pDot = dynamic_cast<CLaserDot *>(pOther);

    //		if ( pDot->IsActive() )
    if (pDot->IsOn()) {
      UTIL_TraceLine(GetAbsOrigin(), pDot->GetAbsOrigin(), MASK_SHOT, this,
  ...
  // this acceleration and turning math is totally wrong, but it seems to
  // respond well so don't change it.
  ...
  if (gpGlobals->curtime - m_flIgniteTime < 1.0) {
    ...
    if (GetWaterLevel() == 3) {
      ...
      UTIL_BubbleTrail(GetAbsOrigin() - GetAbsVelocity() * 0.1, GetAbsOrigin(),
                       4);
    } else {
...
// Output : CRpgRocket
//-----------------------------------------------------------------------------
CRpgRocket *CRpgRocket::Create(const Vector &vecOrigin, const QAngle &angAngles,
...
#ifndef CLIENT_DLL
  CLaserDot *pLaserDot =
      (CLaserDot *)CBaseEntity::Create("laser_spot", origin, QAngle(0, 0, 0));
...
int CLaserDot::DrawModel(int flags) {
  ...
  if (pOwner != NULL && pOwner->IsDormant() == false) {
    ...
    trace_t tr;
    UTIL_TraceLine(vecAttachment, vecAttachment + (vecDir * MAX_TRACE_LENGTH),
...
#ifdef CLIENT_DLL
            DEFINE_PRED_FIELD(m_bIntialStateUpdate, FIELD_BOOLEAN,
                              FTYPEDESC_INSENDTABLE),
    DEFINE_PRED_FIELD(m_bGuiding, FIELD_BOOLEAN, FTYPEDESC_INSENDTABLE),
    DEFINE_PRED_FIELD(m_bLaserDotSuspended, FIELD_BOOLEAN,
                      FTYPEDESC_INSENDTABLE),
    DEFINE_PRED_FIELD(m_flLaserDotReviveTime, FIELD_FLOAT,
                      FTYPEDESC_INSENDTABLE),
#endif
    END_PREDICTION_DATA()
...
bool CWeaponRPG::Deploy(void) {
  ...
  if (m_iClip1 <= 0) {
    return DefaultDeploy((char *)GetViewModel(), (char *)GetWorldModel(),
                         ACT_RPG_DRAW_UNLOADED, (char *)GetAnimPrefix());
  }
...
void CWeaponRPG::PrimaryAttack(void) {
  ...
  Vector muzzlePoint = pOwner->Weapon_ShootPosition() + vForward * 16.0f +
                       vRight * 8.0f + vUp * -8.0f;
  ...
  pMissile->SetAbsVelocity(pMissile->GetAbsVelocity() +
                           vForward *
                               DotProduct(pOwner->GetAbsVelocity(), vForward));
...
bool CWeaponRPG::Reload(void) {
  ...
  // LTD is on, the weapons code is constantly calling into this function, but
  // is often denied because a) missiles are in flight, but the LTD is on or b)
  // player is totally out of ammo and has nothing to switch to, and should be
  // allowed to
  ...
  if ((m_hMissile != NULL) && IsGuiding()) {
    ...
    // ward off future autoreload attempts by setting next attack time into the
    // future for a bit.
    return false;
  }
...
void CWeaponRPG::UpdateSpot(void) {
  ...
  if (IsGuiding() && m_bLaserDotSuspended &&
      (m_flLaserDotReviveTime <= gpGlobals->curtime)) {
#ifndef CLIENT_DLL
    m_hLaserDot->TurnOn();
#endif
    m_bLaserDotSuspended = false;
  ...
  Vector forward;
  AngleVectors(pPlayer->EyeAngles() + pPlayer->m_Local.m_vecPunchAngle,
               &forward);
