...
#define SATCHELRADIO_WORLD_MODEL                                               \
  "models/w_satchel.mdl" // this needs fixing if we do multiplayer
...
#ifdef CLIENT_DLL
DEFINE_PRED_FIELD(m_iRadioViewIndex, FIELD_INTEGER,
                  FTYPEDESC_INSENDTABLE | FTYPEDESC_MODELINDEX),
    DEFINE_PRED_FIELD(m_iRadioWorldIndex, FIELD_INTEGER,
                      FTYPEDESC_INSENDTABLE | FTYPEDESC_MODELINDEX),
    DEFINE_PRED_FIELD(m_iSatchelViewIndex, FIELD_INTEGER,
                      FTYPEDESC_INSENDTABLE | FTYPEDESC_MODELINDEX),
    DEFINE_PRED_FIELD(m_iSatchelWorldIndex, FIELD_INTEGER,
                      FTYPEDESC_INSENDTABLE | FTYPEDESC_MODELINDEX),
...
void CWeaponSatchel::Equip(CBaseCombatCharacter *pOwner) {
  ...
  m_iChargeReady = 0; // this satchel charge weapon now forgets that any
                      // satchels are deployed by it.
}
...
void CWeaponSatchel::PrimaryAttack(void) {
  ...
  case 1: {
    ...
    while ((pSatchel = gEntList.FindEntityByClassname(
                pSatchel, "monster_satchel")) != NULL) {
...
void CWeaponSatchel::Throw(void) {
  ...
  if (pPlayer->GetAmmoCount(m_iPrimaryAmmoType) > 0) {
    ...
    CBaseEntity *pSatchel =
        Create("monster_satchel", vecSrc, QAngle(0, 0, 90), pPlayer);
    if (pSatchel) {
...
void CWeaponSatchel::WeaponIdle(void) {
  ...
  SetWeaponIdleTime(
      gpGlobals->curtime +
      random->RandomFloat(10, 15)); // how long till we do this again.
}
...
DEFINE_FIELD(m_flNextBounceSoundTime, FIELD_TIME),
    ...
    // Function Pointers
    DEFINE_ENTITYFUNC(SatchelTouch), DEFINE_THINKFUNC(SatchelThink),
...
// Output :
//-----------------------------------------------------------------------------
void CSatchelCharge::SatchelUse(CBaseEntity *pActivator, CBaseEntity *pCaller,
...
// Output :
//-----------------------------------------------------------------------------
CSatchelCharge::CSatchelCharge(void) { m_vLastPosition.Init(); }
