...
void CWeaponMP5::PrimaryAttack(void) {
  ...
  if (!g_pGameRules->IsMultiplayer()) {
    ...
    //VECTOR_CONE_6DEGREES, MAX_TRACE_LENGTH, m_iPrimaryAmmoType, 2 );
    FireBulletsInfo_t info(1, vecSrc, vecAiming, VECTOR_CONE_6DEGREES,
    ...
    //		pPlayer->FireBullets( 1, vecSrc, vecAiming,
    //VECTOR_CONE_3DEGREES, MAX_TRACE_LENGTH, m_iPrimaryAmmoType, 2 );
    FireBulletsInfo_t info(1, vecSrc, vecAiming, VECTOR_CONE_3DEGREES,
                           MAX_TRACE_LENGTH, m_iPrimaryAmmoType);
    info.m_pAttacker = pPlayer;
    info.m_iTracerFreq = 2;
...
void CWeaponMP5::SecondaryAttack(void) {
  ...
  CGrenadeMP5 *m_pMyGrenade =
      (CGrenadeMP5 *)Create("grenade_mp5", vecSrc, angGrenAngle, GetOwner());
