...
bool CHL1GameMovement::CheckJumpButton(void) {
  ...
  m_pHL1Player->PlayStepSound((Vector &)mv->GetAbsOrigin(),
  ...
  if ((m_pHL1Player->m_Local.m_bDucking) ||
      (m_pHL1Player->GetFlags() & FL_DUCKING)) {
    ...
    // UNDONE -- note this should be based on forward angles, not current
    // velocity.
    if (m_pHL1Player->m_bHasLongJump && (mv->m_nButtons & IN_DUCK) &&
      ...
      mv->m_vecVelocity[2] =
          flGroundFactor * sqrt(2 * 800 * 45.0); // 2 * gravity * height
    }
  } else {
    mv->m_vecVelocity[2] +=
        flGroundFactor * sqrt(2 * 800 * 45.0); // 2 * gravity * height
...
#ifdef CLIENT_DLL
    (dynamic_cast<C_HL1MP_Player *>(m_pHL1Player))
        ->DoAnimationEvent(PLAYERANIMEVENT_JUMP);
#else
    (dynamic_cast<CHL1MP_Player *>(m_pHL1Player))
        ->DoAnimationEvent(PLAYERANIMEVENT_JUMP);
#endif
