...
#define VECTOR_CONE_DM_SHOTGUN                                                 \
  Vector(0.08716, 0.04362, 0.00) // 10 degrees by 5 degrees
...
void CWeaponShotgun::Precache(void) { BaseClass::Precache(); }
...
void CWeaponShotgun::PrimaryAttack(void) {
  ...
  if (g_pGameRules->IsMultiplayer()) {
    FireBulletsInfo_t info(4, vecSrc, vecAiming, VECTOR_CONE_DM_SHOTGUN,
                           MAX_TRACE_LENGTH, m_iPrimaryAmmoType);
    info.m_pAttacker = pPlayer;

    pPlayer->FireBullets(info);
  } else {
    FireBulletsInfo_t info(6, vecSrc, vecAiming, VECTOR_CONE_10DEGREES,
                           MAX_TRACE_LENGTH, m_iPrimaryAmmoType);
    info.m_pAttacker = pPlayer;

    pPlayer->FireBullets(info);

    //		pPlayer->FireBullets( 6, vecSrc, vecAiming,
    //VECTOR_CONE_10DEGREES, MAX_TRACE_LENGTH, m_iPrimaryAmmoType, 0 );
  }
...
void CWeaponShotgun::SecondaryAttack(void) {
  ...
  // Fire the bullets
  if (g_pGameRules->IsMultiplayer()) {
    FireBulletsInfo_t info(8, vecSrc, vecAiming, VECTOR_CONE_DM_DOUBLESHOTGUN,
                           MAX_TRACE_LENGTH, m_iPrimaryAmmoType);
    info.m_pAttacker = pPlayer;

    pPlayer->FireBullets(info);

    //		pPlayer->FireBullets( 8, vecSrc, vecAiming,
    //VECTOR_CONE_DM_DOUBLESHOTGUN, MAX_TRACE_LENGTH, m_iPrimaryAmmoType, 0 );
  } else {
    FireBulletsInfo_t info(12, vecSrc, vecAiming, VECTOR_CONE_10DEGREES,
                           MAX_TRACE_LENGTH, m_iPrimaryAmmoType);

    pPlayer->FireBullets(info);
    //		pPlayer->FireBullets( 12, vecSrc, vecAiming,
    //VECTOR_CONE_10DEGREES, MAX_TRACE_LENGTH, m_iPrimaryAmmoType, 0 );
  }
...
void CWeaponShotgun::WeaponIdle(void) {
  ...
  if (HasWeaponIdleTimeElapsed()) {
    if (m_iClip1 == 0 && m_fInSpecialReload == 0 &&
