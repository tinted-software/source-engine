...
#ifdef CLIENT_DLL
            DEFINE_PRED_FIELD(m_flRechargeTime, FIELD_FLOAT,
...
void CWeaponHgun::PrimaryAttack(void) {
  ...
  CBaseEntity *pHornet = CBaseEntity::Create(
      "hornet",
      pPlayer->Weapon_ShootPosition() + vForward * 16 + vRight * 8 + vUp * -12,
      vecAngles, pPlayer);
...
bool CWeaponHgun::Holster(CBaseCombatWeapon *pSwitchingTo) {
  ...
  if (bRet) {
    ...
    if (pPlayer) {
#if !defined(CLIENT_DLL)
      //!!!HACKHACK - can't select hornetgun if it's empty! no way to get ammo
      //!for it, either.
...
bool CWeaponHgun::Reload(void) {
  ...
  while ((m_flRechargeTime < gpGlobals->curtime) &&
         g_pGameRules->CanHaveAmmo(pPlayer, m_iPrimaryAmmoType)) {
