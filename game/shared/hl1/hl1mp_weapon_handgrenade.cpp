...
DEFINE_ENTITYFUNC(BounceTouch), END_DATADESC()
...
void CHandGrenade::ShootTimed(CBaseCombatCharacter *pOwner, Vector vecVelocity,
                              float flTime) {
...
#ifdef CLIENT_DLL
            DEFINE_PRED_FIELD(m_flStartThrow, FIELD_INTEGER,
...
void CWeaponHandGrenade::WeaponIdle(void) {
  ...
  if (m_flStartThrow) {
    ...
    CHandGrenade *pGrenade =
        (CHandGrenade *)Create("grenade_hand", vecSrc, angles);
    if (pGrenade) {
