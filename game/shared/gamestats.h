...
public:
  BasicGameStatsRecord_t()
      : m_nCount(0), m_nSeconds(0), m_nCommentary(0), m_nHDR(0), m_nCaptions(0),
        m_bSteam(true), m_bCyberCafe(false), m_nDeaths(0) {
...
public:
  BasicGameStats_t()
      : m_nSecondsToCompleteGame(0), m_nHL2ChaptureUnlocked(0), m_bSteam(true),
        m_bCyberCafe(false), m_nDXLevel(0) {}
...
public:
  ...
  // override this to declare what format you want to send.  New products should
  // use new format.
  virtual bool UseOldFormat() {
#ifdef GAME_DLL
    return true; // servers by default send old format for backward compat
#else
    return false; // clients never used old format so no backward compat issues,
                  // they use new format by default
#endif
  }
  ...
  // These methods used for new format gamestats only and control when data gets
  // sent.
  ...
  virtual bool StatTrackingEnabledForMod(void) {
    return false;
  } // Override this to turn on the system. Stat tracking is disabled by default
    // and will always be disabled at the user's request
  ...
  }; // when loading the saved stats file, this will point to where you started
     // saving data to the save buffer
  ...
  virtual bool SaveToFileNOW(
      bool bForceSyncWrite = false); // saves buffers to their respective files
                                     // now, returns success or failure
...
public:
#ifdef GAME_DLL
  BasicGameStats_t m_BasicStats; // exposed in case you do a complete overhaul
