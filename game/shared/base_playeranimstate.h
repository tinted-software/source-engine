...
#define MAIN_IDLE_SEQUENCE_LAYER                                               \
  0 // For 8-way blended models, this layer blends an idle on top of the
    // run/walk animation to simulate a 9-way blend. For 9-way blended models,
    // we don't use this layer.

#define AIMSEQUENCE_LAYER                                                      \
  1 // Aim sequence uses layers 0 and 1 for the weapon idle animation (needs 2
    // layers so it can blend).
...
public:
  ...
  // and the upper body overlay based on the player's velocity and look
  // direction.
  //
  // It also modulates these based on events triggered by DoAnimationEvent.
  virtual void Update(float eyeYaw, float eyePitch);

  // This is called by the client when a new player enters the PVS to clear any
  // events the dormant version of the entity may have been playing.
  ...
  // (since as the guy turns, it will change his body_yaw pose parameter before
  // changing his rendered angle).
...
public:
  ...
  // This is called to calculate the aim layer sequence. It usually figures out
  // the animation prefixes and suffixes and calls CalcSequenceIndex().
  ...
  // with an idle aim layer sequence based on how fast the character is moving,
  // so it doesn't play the upper-body run at full speed if he's moving really
  // slowly.
  //
  ...
  // For the body left/right rotation, some models use a pose parameter and some
  // use a bone controller.
  ...
  // player's movement speed. The return value from here is interpolated so the
  // playback rate or pose params don't move sharply.
  ...
  // Allow inheriting classes to translate their desired activity, while keeping
  // all internal ACT comparisons using the base activity
...
private:
  ...
  // Get the string that's appended to animation names for the player's current
  // weapon.
...
private:
  ...
  void UpdateAimSequenceLayers(float flCycle, int iFirstLayer, bool bForceIdle,
