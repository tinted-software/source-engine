...
// Container to hold all the KeyValue stats to send only if the convar
// "steamworks_immediate_upload" is set to 0. Otherwise, the stats are uploaded
...
class CSteamWorksGameStatsUploader : public CAutoGameSystemPerFrame,
...
#ifndef NO_STEAM
  STEAM_CALLBACK_MANUAL(CSteamWorksGameStatsUploader,
                        Steam_OnSteamSessionInfoIssued,
                        GameStatsSessionIssued_t,
                        m_CallbackSteamSessionInfoIssued);
  STEAM_CALLBACK_MANUAL(CSteamWorksGameStatsUploader,
                        Steam_OnSteamSessionInfoClosed,
...
#ifdef GAME_DLL
#ifndef NO_STEAM
  STEAM_GAMESERVER_CALLBACK(CSteamWorksGameStatsUploader,
                            Steam_OnSteamSessionInfoIssued,
                            GameStatsSessionIssued_t,
                            m_CallbackSteamSessionInfoIssued);
  STEAM_GAMESERVER_CALLBACK(CSteamWorksGameStatsUploader,
...
private:
  ...
  EResult WriteInt64ToTable(const uint64 value, uint64 iTableID,
                            const char *pzRow);
  EResult WriteFloatToTable(const float value, uint64 iTableID,
                            const char *pzRow);
  EResult WriteStringToTable(const char *pzValue, uint64 iTableID,
                             const char *pzRow);
  EResult WriteOptionalFloatToTable(KeyValues *pKV, const char *keyName,
                                    uint64 iTableID, const char *pzRow);
  EResult WriteOptionalIntToTable(KeyValues *pKV, const char *keyName,
                                  uint64 iTableID, const char *pzRow);

  ISteamGameStats *GetInterface(void);
  EResult
  ParseKeyValuesAndSendStats(KeyValues *pKV,
                             bool bIncludeClientsServerSessionID = true);
