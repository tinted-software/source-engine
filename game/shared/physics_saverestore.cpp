...
DEFINE_FIELD(nSaved, FIELD_INTEGER),
    // NOTE: We want to save the actual address here for remapping, so use an
    // integer
...
struct PhysObjectHeader_t {
  PhysObjectHeader_t() { memset(this, 0, sizeof(*this)); }
...
DEFINE_FIELD(type, FIELD_INTEGER), DEFINE_FIELD(hEntity, FIELD_EHANDLE),
...
public:
      ...
      if (pOwner) {
        ...
        if (item.header.nObjects) {
          for (int i = 0; i < item.header.nObjects; i++) {
            // Starting a block here allows the implementation of any individual
            // physics class save/load to change non-trivially while retaining
            // the overall integrity of the savefile.
            pSave->StartBlock();
            SavePhysicsObject(pSave, pOwner, item.ppPhysObj[i],
                              item.header.type);
            pSave->EndBlock();
          }
        }
  ...
  void RestoreBlock(IRestore *pRestore, const PhysObjectHeader_t &header) {
    ...
    if (iQueued != m_QueuedRestores.InvalidIndex()) {
      ...
      if (pOwner->ShouldSavePhysics() && header.nObjects > 0) {
        QueuedItem_t *pItem =
            m_QueuedRestores[iQueued]->FindItem(header.fieldName);
            ...
            for (int i = 0; i < nObjects; i++) {
              ...
              if (header.type == PIID_IPHYSICSMOTIONCONTROLLER) {
                ...
                IPhysicsMotionController *pController =
                    (IPhysicsMotionController *)pObj;
                if (pController) {
                  // If the entity is the motion callback handler, then
                  // automatically set it NOTE: This is usually the case
  ...
  void RestorePhysicsObjectAndModel(
      IRestore *pRestore, const PhysObjectHeader_t &header,
    ...
    if (nObjects == 1) {
        ...
        if (modelIndex != -1) {
          ...
          if (pCollide) {
            if (pCollide->solidCount > 0 && pCollide->solids &&
                header.iCollide < pCollide->solidCount)
              pPhysCollide = pCollide->solids[header.iCollide];
          }
        }
      } else if (header.bbox.mins != vec3_origin ||
                 header.bbox.maxs != vec3_origin) {
      ...
      if (pPhysCollide) {
        if (!(*pItem->ppPhysObj)) {
          RestorePhysicsObject(pRestore, header, pItem->ppPhysObj,
                               pPhysCollide);
          if ((*pItem->ppPhysObj)) {
          ...
          DevMsg("Physics object pointer unexpectedly non-null before restore. "
                 "Should be creating physics object in CreatePhysics()?\n");
      } else
  ...
  void QueueSave(CBaseEntity *pOwner, typedescription_t *pTypeDesc,
                 void **ppPhysObj, PhysInterfaceId_t type) {
    if (!pOwner)
      return;
    ...
    if (!fOnlyNotingExistence && type == PIID_IPHYSICSOBJECT) {
      ...
      if (pPhysObj) {
        ...
        item.header.iCollide =
            physcollision->CollideIndex(pPhysObj->GetCollide());
        if (item.header.modelName == NULL_STRING) {
          ...
          if (pBBox != NULL) {
            ...
            if (pPhysObj && pPhysObj->GetSphereRadius() != 0) {
              ...
              DevMsg("Don't know how to save model for physics object (class "
                     "\"%s\")\n",
                     pOwner->GetClassname());
            }
          }
  ...
  void QueueRestore(CBaseEntity *pOwner, typedescription_t *pTypeDesc,
                    void **ppPhysObj, PhysInterfaceId_t type) {
  ...
  void SavePhysicsObject(ISave *pSave, CBaseEntity *pOwner, void *pObject,
                         PhysInterfaceId_t type) {
    if (physenv) {
      if (!pObject)
        return;
  ...
  void RestorePhysicsObject(IRestore *pRestore,
                            const PhysObjectHeader_t &header, void **ppObject,
                            const CPhysCollide *pCollide = NULL) {
    if (physenv) {
      physrestoreparams_t params = {pRestore,
                                    ppObject,
                                    header.type,
                                    header.hEntity.Get(),
  ...
  virtual void NoteBBox(const Vector &mins, const Vector &maxs,
    ...
    if (pCollide && m_PhysCollideBBoxModels.Find(pCollide) ==
                        m_PhysCollideBBoxModels.InvalidIndex()) {
      BBox_t box;
      box.mins = mins;
      box.maxs = maxs;
  ...
  virtual void AssociateModel(IPhysicsObject *pObject, int modelIndex) {
    Assert(m_PhysObjectModels.Find(pObject) ==
           m_PhysObjectModels.InvalidIndex());
    m_PhysObjectModels.Insert(pObject, modelIndex);
  }

  //---------------------------------

  virtual void AssociateModel(IPhysicsObject *pObject,
                              const CPhysCollide *pModel) {
    Assert(m_PhysObjectCustomModels.Find(pObject) ==
           m_PhysObjectCustomModels.InvalidIndex());
  ...
  string_t GetModelName(IPhysicsObject *pObject) {
    ...
    if (i == m_PhysObjectModels.InvalidIndex())
      return NULL_STRING;
    return AllocPooledString(
        modelinfo->GetModelName(modelinfo->GetModel(m_PhysObjectModels[i])));
  }
...
private:
  ...
  public:
    int Add(CBaseEntity *pOwner, typedescription_t *pTypeDesc, void **ppPhysObj,
            PhysInterfaceId_t type) {
      int i = AddToTail();
      ...
      item.header.fieldName = AllocPooledString(
          pTypeDesc->fieldName); // See comment in
    ...
    QueuedItem_t *FindItem(string_t itemFieldName) {
      // generally, the set is very small, usually one, so linear search is not
      // too gruesome;
      for (int i = 0; i < Count(); i++) {
        string_t testName = Element(i).header.fieldName;
        Assert((testName == itemFieldName &&
                strcmp(STRING(testName), STRING(itemFieldName)) == 0) ||
               (testName != itemFieldName &&
                strcmp(STRING(testName), STRING(itemFieldName)) != 0));
  ...
  static bool SaveQueueFunc(const QueuedItem_t &left,
                            const QueuedItem_t &right) {
    if (left.header.type == right.header.type)
      return (left.header.hEntity->entindex() >
              right.header.hEntity->entindex());
...
IPhysSaveRestoreManager *g_pPhysSaveRestoreManager =
...
public:
  virtual void Save(const SaveRestoreFieldInfo_t &fieldInfo, ISave *pSave) {
    CBaseEntity *pOwnerEntity =
        pSave->GetGameSaveRestoreInfo()->GetCurrentEntityContext();
...
#if defined(CLIENT_DLL)
      pOwnerEntity = ClientEntityList().GetBaseEntityFromHandle(
          pOwnerEntity->GetRefEHandle());
    ...
    AssertMsg(pOwnerEntity && bFoundEntity == true,
    ...
    g_PhysSaveRestoreBlockHandler.QueueSave(pOwnerEntity, fieldInfo.pTypeDesc,
                                            (void **)fieldInfo.pField, m_type);
  }

  virtual void Restore(const SaveRestoreFieldInfo_t &fieldInfo,
                       IRestore *pRestore) {
    CBaseEntity *pOwnerEntity =
        pRestore->GetGameSaveRestoreInfo()->GetCurrentEntityContext();
    ...
    g_PhysSaveRestoreBlockHandler.QueueRestore(
