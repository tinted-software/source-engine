...
CGameRulesRegister::CGameRulesRegister(const char *pClassName,
...
void CGameRulesRegister::CreateGameRules() { m_pFn(); }
...
// ------------------------------------------------------------------------------------------
// // Functions to dispatch the messages to create the game rules object on the
// client.
// ------------------------------------------------------------------------------------------
// //
...
void OnGameRulesCreationStringChanged(void *object,
  ...
  if (!pReg) {
    Error("OnGameRulesCreationStringChanged: missing gamerules class '%s' on "
          "the client",
          pClassName);
  }
...
void InstallStringTableCallback_GameRules() {
  if (!g_StringTableGameRules) {
    g_StringTableGameRules =
        networkstringtable->FindTable(GAMERULES_STRINGTABLE_NAME);
    if (g_StringTableGameRules) {
      g_StringTableGameRules->SetStringChangedCallback(
          NULL, OnGameRulesCreationStringChanged);
    }
  }
}
...
void CreateNetworkStringTables_GameRules() {
  ...
  g_StringTableGameRules =
      networkstringtable->CreateStringTable(GAMERULES_STRINGTABLE_NAME, 1);
...
void CreateGameRulesObject(const char *pClassName) {
  ...
  CGameRulesRegister *pReg = CGameRulesRegister::FindByName(pClassName);
  if (!pReg)
    Error("InitGameRules: missing gamerules class '%s' on the server",
          pClassName);
  ...
  g_StringTableGameRules->AddString(true, "classname", strlen(pClassName) + 1,
                                    pClassName);
