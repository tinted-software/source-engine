...
void GetParticleManifest(CUtlVector<CUtlString> &list) {
  ...
  if (manifest->LoadFromFile(filesystem, PARTICLES_MANIFEST_FILE, "GAME")) {
    for (KeyValues *sub = manifest->GetFirstSubKey(); sub != NULL;
      ...
      Warning("CParticleMgr::Init:  Manifest '%s' with bogus file type '%s', "
              "expecting 'file'\n",
...
void ReloadParticleEffectsInList(IFileList *pFilesToReload) {
  ...
  // CAB 2/17/11 Reload all the particles regardless (Fixes filename change
  // exploits).
  ...
  //	V_strncat( szDX80Filename, V_GetFileExtension( pFile ), sizeof(
  //szDX80Filename ) );

  //	if ( pFilesToReload->IsFileInList( pFile ) ||
  //pFilesToReload->IsFileInList( szDX80Filename ) )
  //	{
...
void ParseParticleEffectsMap(const char *pMapName, bool bLoadSheets,
  ...
  if (pMapName && *pMapName) {
    V_snprintf(szMapManifestFilename, sizeof(szMapManifestFilename),
               "maps/%s_particles.txt", pMapName);
  }
    ...
    for (KeyValues *sub = manifest->GetFirstSubKey(); sub != NULL;
      ...
      if (!Q_stricmp(sub->GetName(), "file")) {
        ...
        if (pszPath && pszPath[0] && !Q_stricmp(pszPath, "particles")) {
          ...
          Warning("CParticleMgr::LevelInit:  Manifest '%s' contains a particle "
                  "file '%s' that's not under the particles directory. Custom "
                  "particles must be placed in the particles directory.\n",
                  szMapManifestFilename, sub->GetString());
        }
      } else {
        Warning("CParticleMgr::LevelInit:  Manifest '%s' with bogus file type "
                "'%s', expecting 'file'\n",
                szMapManifestFilename, sub->GetName());
      }
    }
  } else {
    // Don't print a warning, and don't proceed any further if the file doesn't
    // exist!
    return;
  }
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    if (!pFilesToReload ||
        (pFilesToReload && pFilesToReload->IsFileInList(files[i]))) {
...
#ifdef GAME_DLL
  // Now add each particle system name to the network string pool, so we can
  // send string_t's down to the client instead of full particle system names.
...
void DispatchParticleEffect(const char *pszParticleName,
  ...
  if (pEntity && pEntity->GetBaseAnimating()) {
    ...
    if (iAttachment <= 0) {
      Warning("Model '%s' doesn't have attachment '%s' to attach particle "
              "system '%s' to.\n",
  ...
  DispatchParticleEffect(pszParticleName, iAttachType, pEntity, iAttachment,
...
void DispatchParticleEffect(const char *pszParticleName,
                            ParticleAttachment_t iAttachType,
                            CBaseEntity *pEntity, const char *pszAttachmentName,
                            Vector vecColor1, Vector vecColor2, bool bUseColors,
                            bool bResetAllParticlesOnEntity) {
  int iAttachment = -1;
  if (pEntity && pEntity->GetBaseAnimating()) {
    // Find the attachment point index
...
void DispatchParticleEffect(const char *pszParticleName, Vector vecOrigin,
                            ...
                            Vector vecColor2, bool bUseColors,
                            CBaseEntity *pEntity, int iAttachType) {
...
ConVar particle_test_file("particle_test_file", "", FCVAR_CHEAT,
...
ConVar particle_test_attach_mode(
    ...
    "Possible Values: 'start_at_attachment', 'follow_attachment', "
    "'start_at_origin', 'follow_origin'");
ConVar particle_test_attach_attachment("particle_test_attach_attachment", "0",
                                       FCVAR_CHEAT,
...
void Particle_Test_Start(CBasePlayer *pPlayer, const char *name, bool bStart) {
  ...
  int iAttachType =
      GetAttachTypeFromString(particle_test_attach_mode.GetString());

  if (iAttachType < 0) {
    Warning("Invalid attach type specified for particle_test in cvar "
  ...
  while ((pEntity = GetNextCommandEntity(pPlayer, name, pEntity)) != NULL) {
    ...
    */
    ...
    DispatchParticleEffect(pszParticleFile, (ParticleAttachment_t)iAttachType,
                           pEntity, iAttachmentIndex,
...
static ConCommand particle_test_start(
    ...
    "Dispatches the test particle system with the parameters specified in "
    "particle_test_file,\n particle_test_attach_mode and "
    "particle_test_attach_param on the entity the player is looking "
...
static ConCommand particle_test_stop(
    "particle_test_stop", CC_Particle_Test_Stop,
