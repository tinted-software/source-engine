...
//-----------------------------------------------------------------------------
inline const CBaseEntity *
EntityFromEntityHandle(const IHandleEntity *pConstHandleEntity) {
...
#define IMPLEMENT_AUTO_LIST(interfaceName)                                     \
  ...
  interfaceName::~interfaceName() { Remove(this); }
...
private:
  float m_timestamp;
  float Now(void) const; // work-around since client header doesn't like inlined
                         // gpGlobals->curtime
};
