...
void CWeaponIFMBaseCamera::SetRenderInfo(float flAspectRatio, float flFOV,
                                         float flArmLength,
...
CON_COMMAND(ifm_basecamera_camerastate, "Set camera state") {
  ...
  for (int i = 0; i < nCount; ++i) {
    CWeaponIFMBaseCamera *pCamera =
        dynamic_cast<CWeaponIFMBaseCamera *>(pPlayer->GetWeapon(i));
    if (!pCamera)
      continue;

    pCamera->SetRenderInfo(flAspectRatio, flFOV, flArmLength, vecPosition,
                           angAngles);
  }
}
...
void CWeaponIFMBaseCamera::OnDataChanged(DataUpdateType_t updateType) {
  ...
  if (updateType == DATA_UPDATE_CREATED) {
    ...
    m_FrustumWireframeMaterial.Init("shadertest/wireframevertexcolor",
                                    TEXTURE_GROUP_OTHER);
  }
}
...
void CWeaponIFMBaseCamera::TransmitRenderInfo() {
  float flAspectRatio = (m_nScreenHeight != 0)
  ...
  Q_snprintf(
      pBuf, sizeof(pBuf),
      "ifm_basecamera_camerastate %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f",
...
//-----------------------------------------------------------------------------
void CWeaponIFMBaseCamera::ComputeAbsCameraTransform(Vector &vecAbsOrigin,
...
//-----------------------------------------------------------------------------
// When drawing the model, if drawing the viewmodel, draw an overlay of what's
// being rendered
//-----------------------------------------------------------------------------
void CWeaponIFMBaseCamera::ViewModelDrawn(CBaseViewModel *pBaseViewModel) {
  // NOTE: This is not recursively called because we do not draw viewmodels in
  // the overlay
  ...
  GetOverlayBounds(overlayView.x, overlayView.y, overlayView.width,
                   overlayView.height);
  ...
  // give the toolsystem a chance to override the view
  ToolFramework_SetupEngineView(overlayView.origin, overlayView.angles,
                                overlayView.fov);

  view->QueueOverlayRenderView(overlayView, VIEW_CLEAR_COLOR | VIEW_CLEAR_DEPTH,
...
void CWeaponIFMBaseCamera::DrawCrosshair(void) {
  ...
  vgui::surface()->DrawFilledRect(x - nBorderSize, y - nBorderSize,
                                  x + w + nBorderSize, y);
  vgui::surface()->DrawFilledRect(x - nBorderSize, y + h, x + w + nBorderSize,
                                  y + h + nBorderSize);
