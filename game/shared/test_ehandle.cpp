...
// In this test, the server makes an entity (call it A) that has an EHANDLE to
// another entity (call it B). Intitially, A is sent to the client but B is not.
// Later, the server decides to send B to the client too. At that point, without
// resending A's EHANDLE, the client's EHANDLE should access B.
...
public:
  ...
  CHandleTest() { m_bSendHandle = false; }
...
SendPropEHandle(SENDINFO(m_Handle)),
    SendPropInt(SENDINFO(m_bSendHandle)) END_SEND_TABLE()
...
void CC_Test_EHandle() {
  if (g_HandleTest.Get()) {
    ...
    Msg("Created EHANDLE test entity. Run this command again to transmit the "
        "second ent.\n");
  }
}
...
RecvPropEHandle(RECVINFO(m_Handle)),
    RecvPropInt(RECVINFO(m_bSendHandle)) END_RECV_TABLE()
