...
void PlayImpactSounds(soundlist_t &list) {
  for (int i = list.Count() - 1; i >= 0; --i) {
    ...
    if (psurf->sounds.impactHard) {
      ...
      // JAY: If this entity gets deleted, the sound comes out at the world
      // origin this sounds bad!  Play on ent 0 for now.
      EmitSound_t ep;
...
void AddImpactSound(soundlist_t &list, void *pGameData, int entityIndex,
  ...
  for (int i = list.Count() - 1; i >= 0; --i) {
    ...
    // UNDONE: Doing one slot per entity is too noisy.  So now we use one slot
    // per material
