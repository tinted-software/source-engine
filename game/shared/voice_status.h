...
public:
  ...
  // back from the server). When the server acknowledges that the local client
  // is talking, then entindex will be gEngfuncs.GetLocalPlayer(). entindex is
  // -2 to represent the local client's voice being acked by the server.
...
private:
  ...
  // This is who the server THINKS we have banned (it can become incorrect when
  // a new player arrives on the server). It is checked periodically, and the
  // server is told to squelch or unsquelch the appropriate players.
...
public:
  CVoiceBanMgr
      m_BanMgr; // Tracks which users we have squelched and don't want to hear.
