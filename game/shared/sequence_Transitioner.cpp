...
void CSequenceTransitioner::CheckForSequenceChange(CStudioHdr *hdr,
  ...
  // FIXME?: this should detect that what's been asked to be drawn isn't what
  // was expected due to not only sequence change, by frame index, rate, or
  // whatever.  When that happens, it should insert the previous rules.
  ...
  if (currentblend->m_flLayerAnimtime &&
    ...
    if ((seqdesc.flags & STUDIO_SNAP) || !bInterpolate) {
      ...
      currentblend->m_flLayerFadeOuttime =
          MIN(prevseqdesc.fadeouttime, seqdesc.fadeintime);
      /*
