...
//			*soundname -
//-----------------------------------------------------------------------------
void CModelSoundsCache::FindOrAddScriptSound(CUtlVector<unsigned short> &sounds,
...
//			sounds -
//-----------------------------------------------------------------------------
void CModelSoundsCache::BuildAnimationEventSoundList(
    ...
    for (int iEvent = 0; iEvent < (int)pSeq->numevents; iEvent++) {
      ...
      case AE_CL_PLAYSOUND: {
        ...
        if (pEvent->pszOptions()[0]) {
          ...
          Warning("-- Error --:  empty soundname, .qc error on AE_CL_PLAYSOUND "
                  "in model %s, sequence %s, animevent # %i\n",
