...
CPortalCollideableEnumerator::CPortalCollideableEnumerator(
    const CProp_Portal *pAssociatedPortal) {
...
IterationRetval_t
CPortalCollideableEnumerator::EnumElement(IHandleEntity *pHandleEntity) {
  ...
  {
          //we're dealing with a static prop, which unfortunately doesn't have
  everything I want to use for checking
          ...
          Vector ptTest( (m_vPlaneNormal.x > 0.0f)?(vMaxs.x):(vMins.x),
                                          (m_vPlaneNormal.y >
  0.0f)?(vMaxs.y):(vMins.y), (m_vPlaneNormal.z > 0.0f)?(vMaxs.z):(vMins.z) );
  ...
  else*/
  {
    ...
    if (!(fPtPlaneDist > fBoundRadius) &&
        (fPtPlaneDist >
         -fBoundRadius)) // object is not wholly in front of the portal, but
                         // could be partially in front, do more checks
    {
      Vector ptNearest;
      pEntityCollision->CalcNearestPoint(m_ptForward1000, &ptNearest);
      fPtPlaneDist = m_vPlaneNormal.Dot(ptNearest) - m_fPlaneDist;
      if (fPtPlaneDist < 0.0f)
        return ITERATION_CONTINUE; // closest point was behind the portal plane,
                                   // we don't want it
    }
  }
