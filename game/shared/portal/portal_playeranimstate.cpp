...
CPortalPlayerAnimState *CreatePortalPlayerAnimState(CPortal_Player *pPlayer) {
  ...
  CPortalPlayerAnimState *pRet =
      new CPortalPlayerAnimState(pPlayer, movementData);
...
Activity CPortalPlayerAnimState::TranslateActivity(Activity actDesired) {
  ...
  if (GetPortalPlayer()->GetActiveWeapon()) {
    translateActivity = GetPortalPlayer()->GetActiveWeapon()->ActivityOverride(
        translateActivity, NULL);
  }
...
// Input  : event -
//-----------------------------------------------------------------------------
void CPortalPlayerAnimState::DoAnimationEvent(PlayerAnimEvent_t event,
                                              int nData) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CPortalPlayerAnimState::Teleport(const Vector *pNewOrigin,
                                      const QAngle *pNewAngles,
  ...
  if (pPlayer) {
    ...
    m_flCurrentFeetYaw = m_flGoalFeetYaw = m_flEyeYaw =
        pPlayer->EyeAngles()[YAW];
  }
}
...
bool CPortalPlayerAnimState::HandleMoving(Activity &idealActivity) {
  ...
  if (flSpeed > MOVING_MINIMUM_SPEED) {
    ...
    // Unless we move, hold the deployed pose for a number of seconds after
    // being deployed
...
bool CPortalPlayerAnimState::HandleDucking(Activity &idealActivity) {
  if (GetBasePlayer()->m_Local.m_bDucking ||
      GetBasePlayer()->m_Local.m_bDucked) {
...
bool CPortalPlayerAnimState::HandleJumping(Activity &idealActivity) {
  ...
  if ((vecVelocity.z > 300.0f || m_bInAirWalk)) {
    // Check to see if we were in an airwalk and now we are basically on the
    // ground.
  ...
  // Jumping.
    ...
    if (m_bJumping) {
      ...
      // Don't check if he's on the ground for a sec.. sometimes the client
      // still has the on-ground flag set right when the message comes in.
