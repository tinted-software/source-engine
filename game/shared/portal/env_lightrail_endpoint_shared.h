...
enum // Enumeration of the 4 states the endpoints can be in.
{
  ...
  ENDPOINT_STATE_SMALLFX, // Just the small particle trail is displayed and a
                          // faint glow
  ENDPOINT_STATE_CHARGING, // Ramp up over a certain amount of time to the large
                           // bright glow
...
public:
  ...
  void StartCharge(
      float flWarmUpTime); // Charging difference between the small and large fx
  void StartSmallFX(); // Start discharging the scaled down version of the FX
  void StartLargeFX(); // Start discharging the larger brighter version of the
                       // FX
