...
bool CWeaponPortalBase::IsPredicted() const { return false; }

void CWeaponPortalBase::WeaponSound(WeaponSound_t sound_type,
...
int CWeaponPortalBase::DrawModel(int flags) {
  ...
  // Sometimes the return value of ShouldDrawLocalPlayer() fluctuates too often
  // to draw the correct model all the time, so this is a quick fix if it's
  // changed too fast
...
bool CWeaponPortalBase::ShouldDraw(void) {
  ...
  // if ( GetOwner() && GetOwner() == C_BasePlayer::GetLocalPlayer() &&
  // materials->GetRenderTarget() == 0 ) 	return false;
...
bool CWeaponPortalBase::OnFireEvent(C_BaseViewModel *pViewModel,
