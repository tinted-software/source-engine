...
//			Game Coordinator for Portal.  Note that we never use
//types 			with undefined length (like int).  Always use an explicit type 			(like
//int32).
//
//=============================================================================
...
enum EGCMsg {
  ...
  k_EMsgGCReportWarKill =
      k_EMsgGCPortalBase + 1, // War kill tracking. No longer in use

  // Development only messages
  k_EMsgGCPortalDEVBase = 6000,
  k_EMsgGCDev_GrantWarKill =
      k_EMsgGCPortalDEVBase + 1, // War kill tracking. No longer in use
};
