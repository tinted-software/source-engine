...
// -------------------------------------------------------
// DO NOT EDIT
//   This file was generated from portal\portal_gcschema.sch by
//   SchemaCompiler.EXE
...
CSchGameAccountClient::CSchGameAccountClient(
    const CSchGameAccountClient &that) {
  *this = that;
}
...
void GenerateIntrinsicSQLSchema(GCSDK::CSchemaFull &schemaFull) {
  ...
  pSchema->AddField("unAccountID", "AccountID", k_EGCSQLType_int32,
                    sizeof(uint32), 0, true, 0);
  pSchema->AddField("unRewardPoints", "RewardPoints", k_EGCSQLType_int32,
                    sizeof(uint32), 0, true, 0);
  pSchema->AddField("unPointCap", "PointCap", k_EGCSQLType_int32,
                    sizeof(uint32), 0, true, 0);
  pSchema->AddField("unLastCapRollover", "LastCapRollover", k_EGCSQLType_int32,
                    sizeof(RTime32), 0, true, 0);
  CSchGameAccount::m_nPrimaryKeyID =
      pSchema->PrimaryKey(true, 100, "unAccountID");
  ...
  pSchema->AddField("unAccountID", "AccountID", k_EGCSQLType_int32,
                    sizeof(uint32), 0, true, 0);
  CSchGameAccountClient::m_nPrimaryKeyID =
      pSchema->PrimaryKey(true, 80, "unAccountID");
  pSchema->SetTestWipePolicy(GCSDK::k_EWipePolicyWipeForAllTests);
  pSchema->SetBAllowWipeTableInProd(false);
  pSchema->CalcOffsets();
  schemaFull.CheckSchema(pSchema, CSchGameAccountClient::k_iFieldMax,
                         sizeof(CSchGameAccountClient) -
                             sizeof(GCSDK::CRecordBase));
  pSchema->PrepareForUse();
