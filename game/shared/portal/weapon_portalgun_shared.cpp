...
bool CWeaponPortalgun::ShouldDrawCrosshair(void) {
  return true; //( m_fCanPlacePortal1OnThisSurface > 0.5f ||
               //m_fCanPlacePortal2OnThisSurface > 0.5f );
}
...
// Output :
//-----------------------------------------------------------------------------
bool CWeaponPortalgun::Reload(void) { return true; }
...
void CWeaponPortalgun::SetCanFirePortal1(bool bCanFire /*= true*/) {
  ...
  pOwner->ViewPunch(
      QAngle(random->RandomFloat(-1, -0.5f), random->RandomFloat(-1, 1), 0));
