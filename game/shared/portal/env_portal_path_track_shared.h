...
public:
  ...
  void ActivateTrackFX(void); // Activate all of the track's beams (at least the
                              // ones that are flagged to display)
  void ActivateEndpointFX(void); // Activate all of the endpoint's glowy bits
                                 // that are flagged to display

  void DeactivateTrackFX(void); // Activate all of the track's beams (at least
                                // the ones that are flagged to display)
  void DeactivateEndpointFX(void); // Activate all of the endpoint's glowy bits
                                   // that are flagged to display

protected:
  CNetworkVar(bool, m_bTrackActive);
  CNetworkVar(bool, m_bEndpointActive);
  //	CNetworkVar( float, m_fScaleEndpoint );				// Scale
  //of the endpoint for this beam 	CNetworkVar( float, m_fScaleTrack );
  //// Scale of the track effect 	CNetworkVar( float, m_fFadeOutEndpoint );
  //// Scale of the track effect 	CNetworkVar( float, m_fFadeInEndpoint );
  //// Scale of the track effect
