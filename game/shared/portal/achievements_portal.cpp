...
protected:
  virtual void FireGameEvent(IGameEvent *event) {
    ...
    if (0 == Q_strcmp(name, "portal_player_portaled")) {
      ...
      CProp_Portal *pOutPortal =
          CProp_Portal::FindPortal(0, !bIsPortal2, false);

      if (pInPortal && pOutPortal) {
        if (m_bIsFlinging) {
          // Add up how far we traveled since the last teleport
          m_fAccumulatedDistance +=
              m_fZPortalPosition - pInPortal->GetAbsOrigin().z;

          if (m_fAccumulatedDistance > 30000.0f * 12)
            IncrementCount();
        }

        // Remember the Z position to get the distance when the teleport again
        // or land
      ...
      if (m_bIsFlinging) {
        ...
        if (pLocalPlayer) {
          m_fAccumulatedDistance +=
              m_fZPortalPosition - pLocalPlayer->GetAbsOrigin().z;
...
DECLARE_ACHIEVEMENT(CAchievementPortalInfiniteFall,
                    ACHIEVEMENT_PORTAL_INFINITEFALL, "PORTAL_INFINITEFALL", 5);
  ...
  virtual void FireGameEvent(IGameEvent *event) {
    ...
    if (0 == Q_strcmp(name, "portal_player_portaled")) {
      ...
      if (pInPortal && pOutPortal) {
        if (m_bIsFlinging) {
          // Add up how far we traveled since the last teleport
          float flDist = pInPortal->GetAbsOrigin().AsVector2D().DistTo(
              m_vec2DPortalPosition);

          // Ignore small distances that can be caused by microadjustments in
          // infinite falls
          if (flDist > 63.0f) {
        ...
        // Remember the 2D position to get the distance when the teleport again
        // or land
      ...
      if (m_bIsFlinging) {
        ...
        if (pLocalPlayer) {
          float flDist = pLocalPlayer->GetAbsOrigin().AsVector2D().DistTo(
              m_vec2DPortalPosition);
...
DECLARE_ACHIEVEMENT(CAchievementPortalLongJump, ACHIEVEMENT_PORTAL_LONGJUMP,
                    "PORTAL_LONGJUMP", 5);
...
DECLARE_ACHIEVEMENT(CAchievementPortalBeat2AdvancedMaps,
                    ACHIEVEMENT_PORTAL_BEAT_2ADVANCEDMAPS,
...
DECLARE_ACHIEVEMENT(CAchievementPortalBeat4AdvancedMaps,
                    ACHIEVEMENT_PORTAL_BEAT_4ADVANCEDMAPS,
...
DECLARE_ACHIEVEMENT(CAchievementPortalBeat6AdvancedMaps,
                    ACHIEVEMENT_PORTAL_BEAT_6ADVANCEDMAPS,
...
DECLARE_ACHIEVEMENT(CAchievementPortalGetAllBronze,
                    ACHIEVEMENT_PORTAL_GET_ALLBRONZE, "PORTAL_GET_ALLBRONZE",
                    10);
...
DECLARE_ACHIEVEMENT(CAchievementPortalGetAllSilver,
                    ACHIEVEMENT_PORTAL_GET_ALLSILVER, "PORTAL_GET_ALLSILVER",
                    20);
...
DECLARE_ACHIEVEMENT(CAchievementPortalGetAllGold,
                    ACHIEVEMENT_PORTAL_GET_ALLGOLD, "PORTAL_GET_ALLGOLD", 40);
...
DECLARE_ACHIEVEMENT(CAchievementPortalDetachAllCameras,
                    ACHIEVEMENT_PORTAL_DETACH_ALL_CAMERAS,
                    "PORTAL_DETACH_ALL_CAMERAS", 5);

class CAchievementPortalHitTurretWithTurret : public CBaseAchievement {
protected:
  virtual void ListenForEvents() { ListenForGameEvent("turret_hit_turret"); }
...
DECLARE_ACHIEVEMENT(CAchievementPortalHitTurretWithTurret,
                    ACHIEVEMENT_PORTAL_HIT_TURRET_WITH_TURRET,
    ...
    if (0 == Q_strcmp(event->GetName(), "dinosaur_signal_found")) {
      ...
      if (id >= 0 && id < m_iNumComponents) {
        ...
        steamapicontext->SteamUserStats()->SetStat(
            "PORTAL_TRANSMISSION_RECEIVED_STAT", m_iCount);
#endif
      } else {
        Warning("Failed to set achievement progress. Dinosaur ID(%d) out of "
                "range (0 to %d)\n",
                id, m_iNumComponents);
      }
    }
...
DECLARE_ACHIEVEMENT(CAchievementPortalFindAllDinosaurs,
                    ACHIEVEMENT_PORTAL_TRANSMISSION_RECEIVED,
                    "PORTAL_TRANSMISSION_RECEIVED", 0);
#endif // _XBOX

// achievements which are won by a map event firing once
DECLARE_MAP_EVENT_ACHIEVEMENT(ACHIEVEMENT_PORTAL_GET_PORTALGUNS,
                              "PORTAL_GET_PORTALGUNS", 5);
DECLARE_MAP_EVENT_ACHIEVEMENT(ACHIEVEMENT_PORTAL_KILL_COMPANIONCUBE,
                              "PORTAL_KILL_COMPANIONCUBE", 5);
DECLARE_MAP_EVENT_ACHIEVEMENT(ACHIEVEMENT_PORTAL_ESCAPE_TESTCHAMBERS,
                              "PORTAL_ESCAPE_TESTCHAMBERS", 5);
DECLARE_MAP_EVENT_ACHIEVEMENT(ACHIEVEMENT_PORTAL_BEAT_GAME, "PORTAL_BEAT_GAME",
                              10);
