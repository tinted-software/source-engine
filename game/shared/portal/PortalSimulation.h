...
#define PORTAL_SIMULATORS_EMBED_GUID // define this to embed a unique integer
                                     // with each portal simulator for debugging
                                     // purposes

struct StaticPropPolyhedronGroups_t // each static prop is made up of a group of
                                    // polyhedrons, these help us pull those
...
enum PortalSimulationEntityFlags_t {
  PSEF_OWNS_ENTITY = (1 << 0), // this environment is responsible for the
                               // entity's physics objects
  PSEF_OWNS_PHYSICS = (1 << 1),
  PSEF_IS_IN_PORTAL_HOLE = (1 << 2), // updated per-phyframe
  PSEF_CLONES_ENTITY_FROM_MAIN =
      (1 << 3), // entity is close enough to the portal to affect objects
                // intersecting the portal
  // PSEF_HAS_LINKED_CLONE = (1 << 1), //this environment has a clone of the
  // entity which is transformed from its linked portal
};
...
struct PortalTransformAsAngledPosition_t // a matrix transformation from this
                                         // portal to the linked portal, stored
                                         // as vector and angle transforms
{
...
class CPortalSimulatorEventCallbacks // sends out notifications of events to
                                     // game specific code
{
public:
  ...
  virtual void
  PortalSimulator_TookPhysicsOwnershipOfEntity(CBaseEntity *pEntity) {};
  virtual void
  PortalSimulator_ReleasedPhysicsOwnershipOfEntity(CBaseEntity *pEntity) {};
};
...
//
// The answer. The previous incarnation of server side portal simulation
// suffered terribly from evolving variables with increasingly cryptic names
// with no clear definition of what part of the system the variable was involved
// with.
//
// It's my hope that a nested structure with clear boundaries will eliminate
// that horrible, awful, nasty, frustrating confusion. (It was really really
// bad). This system has the added benefit of pseudo-forcing a naming structure.
//
...
//		pointers/references could be made while the primary instance of
//the 		data was private/protected.
//
...
struct PS_PlacementData_t // stuff useful for geometric operations
{
  ...
  CPhysCollide
      *pHoleShapeCollideable; // used to test if a collideable is in the hole,
                              // should NOT be collided against in general
  PS_PlacementData_t(void) { memset(this, 0, sizeof(PS_PlacementData_t)); }
};

struct PS_SD_Static_World_Brushes_t {
  CUtlVector<CPolyhedron *>
      Polyhedrons; // the building blocks of more complex collision
...
struct PS_SD_Static_World_StaticProps_ClippedProp_t {
  ...
  static const char *szTraceSurfaceName; // same for all static props, here just
                                         // for easy reference
  static const int iTraceSurfaceFlags; // same for all static props, here just
                                       // for easy reference
};
...
struct PS_SD_Static_World_StaticProps_t {
  CUtlVector<CPolyhedron *>
      Polyhedrons; // the building blocks of more complex collision
  CUtlVector<PS_SD_Static_World_StaticProps_ClippedProp_t>
      ClippedRepresentations;
  bool bCollisionExists; // the shortcut to know if collideables exist for each
                         // prop
  bool bPhysicsExists; // the shortcut to know if physics obects exist for each
                       // prop
  PS_SD_Static_World_StaticProps_t(void)
      : bCollisionExists(false), bPhysicsExists(false) {};
};
...
struct PS_SD_Static_Wall_Local_Tube_t // a minimal tube, an object must fit
                                      // inside this to be eligible for
                                      // portaling
{
  CUtlVector<CPolyhedron *>
      Polyhedrons; // the building blocks of more complex collision
...
#ifndef CLIENT_DLL
  IPhysicsObject *pPhysicsObject;
  PS_SD_Static_Wall_Local_Tube_t()
      : pCollideable(NULL), pPhysicsObject(NULL) {};
#else
...
struct PS_SD_Static_Wall_Local_t // things in the wall that are completely
                                 // independant of having a linked portal
{
...
struct PS_SD_Static_Wall_RemoteTransformedToLocal_t // things taken from the
                                                    // linked portal's "World"
                                                    // collision and transformed
                                                    // into local space
{
...
struct PS_SD_Static_SurfaceProperties_t // surface properties to pretend every
                                        // collideable here is using
{
  int contents;
  csurface_t surface;
  CBaseEntity *pEntity;
...
struct PS_SD_Dynamic_PhysicsShadowClones_t {
  CUtlVector<CBaseEntity *>
      ShouldCloneFromMain; // a list of entities that should be cloned from main
                           // if physics simulation is enabled in
                           // single-environment mode, this helps us track who
                           // should collide with who
...
{
  unsigned int EntFlags[MAX_EDICTS]; // flags maintained for every entity in the
                                     // world based on its index
...
struct PS_SimulationData_t // compartmentalized data for coherent management
{
  ...
  CPSCollisionEntity *pCollisionEntity; // the entity we'll be tying physics
                                        // objects to for collision
...
public:
  ...
  void DetachFromLinked(void); // detach portals to sever the connection, saves
                               // work when planning on moving both portals
  ...
  void SetCollisionGenerationEnabled(
      bool bEnabled); // enable/disable collision generation for the hole in the
                      // wall, needed for proper vphysics simulation
  bool IsCollisionGenerationEnabled(void) const;

  void SetVPhysicsSimulationEnabled(
      bool bEnabled); // enable/disable vphysics simulation. Will automatically
                      // update the linked portal to be the same
  bool IsSimulatingVPhysics(void)
      const; // this portal is setup to handle any physically simulated object,
             // false means the portal is handling player movement only

  bool EntityIsInPortalHole(CBaseEntity *pEntity)
      const; // true if the entity is within the portal cutout bounds and
             // crossing the plane. Not just *near* the portal
  bool EntityHitBoxExtentIsInPortalHole(CBaseAnimating *pBaseAnimating)
      const; // true if the entity is within the portal cutout bounds and
             // crossing the plane. Not just *near* the portal
  void RemoveEntityFromPortalHole(
      CBaseEntity *pEntity); // if the entity is in the portal hole, this
                             // forcibly moves it out by any means possible

  bool RayIsInPortalHole(const Ray_t &ray)
      const; // traces a ray against the same detector for
             // EntityIsInPortalHole(), bias is towards false positives

#ifndef CLIENT_DLL
  int GetMoveableOwnedEntities(
      CBaseEntity **pEntsOut,
      int iEntOutLimit); // gets owned entities that aren't either world or
                         // static props. Excludes fake portal ents such as
                         // physics clones

  static CPortalSimulator *GetSimulatorThatOwnsEntity(
      const CBaseEntity *pEntity); // fairly cheap to call
  static CPortalSimulator *GetSimulatorThatCreatedPhysicsObject(
  ...
  // these three really should be made internal and the public interface changed
  // to a "watch this entity" setup
  void TakeOwnershipOfEntity(
      CBaseEntity
          *pEntity); // general ownership, not necessarily physics ownership
  void ReleaseOwnershipOfEntity(
      CBaseEntity *pEntity,
      bool bMovingToLinkedSimulator =
          false); // if bMovingToLinkedSimulator is true, the code skips some
                  // steps that are going to be repeated when the entity is
                  // added to the other simulator
  void ReleaseAllEntityOwnership(void); // go back to not owning any entities

  // void				TeleportEntityToLinkedPortal(
  // CBaseEntity *pEntity );
  ...
  bool
  CreatedPhysicsObject(const IPhysicsObject *pObject,
                       PS_PhysicsObjectSourceType_t *pOut_SourceType =
                           NULL) const; // true if the physics object was
                                        // generated by this portal simulator
...
protected:
  bool m_bLocalDataIsReady; // this side of the portal is properly setup, no
                            // guarantees as to linkage to another portal
  bool m_bSimulateVPhysics;
  bool m_bGenerateCollision;
  bool m_bSharedCollisionConfiguration; // when portals are in certain
                                        // configurations, they need to
                                        // cross-clip and share some collision
                                        // data and things get nasty. For the
                                        // love of all that is holy, pray that
                                        // this is false.
  CPortalSimulator *m_pLinkedPortal;
  bool m_bInCrossLinkedFunction; // A flag to mark that we're already in a
                                 // linked function and that the linked portal
  ...
  void ReleasePhysicsOwnership(CBaseEntity *pEntity,
                               bool bContinuePhysicsCloning = true,
                               bool bMovingToLinkedSimulator = false);

  void CreateAllPhysics(void);
  void
  CreateMinimumPhysics(void); // stuff needed by any part of physics simulations
  ...
  void CreatePolyhedrons(void); // carves up the world around the portal's
                                // position into sets of polyhedrons
...
inline bool CPortalSimulator::OwnsEntity(const CBaseEntity *pEntity) const {
  return ((m_InternalData.Simulation.Dynamic.EntFlags[pEntity->entindex()] &
           PSEF_OWNS_ENTITY) != 0);
}

inline bool
CPortalSimulator::OwnsPhysicsForEntity(const CBaseEntity *pEntity) const {
  return ((m_InternalData.Simulation.Dynamic.EntFlags[pEntity->entindex()] &
           PSEF_OWNS_PHYSICS) != 0);
}
#endif
...
inline bool CPortalSimulator::IsReadyToSimulate(void) const {
  return m_bLocalDataIsReady && m_pLinkedPortal &&
         m_pLinkedPortal->m_bLocalDataIsReady;
}
