...
static struct {
  ...
  int m_iFlagValue;
    ...
    {"ALTWPN_CRITERIA_PRONE_DEPLOYED_RELOAD",
     ALTWPN_CRITERIA_PRONE_DEPLOYED_RELOAD}};
...
void CDODWeaponInfo::Parse(KeyValues *pKeyValuesData,
  ...
  m_flAccuracyMovePenalty =
      pKeyValuesData->GetFloat("AccuracyMovePenalty", 0.1);
  ...
  //"PlayerAnimationExtension", "error" ); 	Q_strncpy( m_szAnimExtension,
  //pAnimEx, sizeof( m_szAnimExtension ) );

  // if this key exists, use this for reload animations instead of anim_prefix
  //	Q_strncpy( m_szReloadAnimPrefix, pKeyValuesData->GetString(
  //"reload_anim_prefix", "" ), MAX_WEAPON_PREFIX );
  ...
  Q_strncpy(m_szReloadModel, pKeyValuesData->GetString("reloadmodel"),
            sizeof(m_szReloadModel));
  Q_strncpy(m_szDeployedModel, pKeyValuesData->GetString("deployedmodel"),
            sizeof(m_szDeployedModel));
  Q_strncpy(m_szDeployedReloadModel,
            pKeyValuesData->GetString("deployedreloadmodel"),
            sizeof(m_szDeployedReloadModel));
  Q_strncpy(m_szProneDeployedReloadModel,
            pKeyValuesData->GetString("pronedeployedreloadmodel"),
  ...
  const char *szNormalOffset =
      pKeyValuesData->GetString("vm_normal_offset", "0 0 0");
  const char *szProneOffset =
      pKeyValuesData->GetString("vm_prone_offset", "0 0 0");
  const char *szIronSightOffset =
      pKeyValuesData->GetString("vm_ironsight_offset", "0 0 0");

  sscanf(szNormalOffset, "%f %f %f", &m_vecViewNormalOffset[0],
         &m_vecViewNormalOffset[1], &m_vecViewNormalOffset[2]);
  sscanf(szProneOffset, "%f %f %f", &m_vecViewProneOffset[0],
         &m_vecViewProneOffset[1], &m_vecViewProneOffset[2]);
  sscanf(szIronSightOffset, "%f %f %f", &m_vecIronSightOffset[0],
         &m_vecIronSightOffset[1], &m_vecIronSightOffset[2]);
