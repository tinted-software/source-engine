...
CDODViewModel::CDODViewModel()
    : m_LagAnglesHistory("CDODViewModel::m_LagAnglesHistory") {
  m_vLagAngles.Init();
...
void CDODViewModel::CalcViewModelLag(Vector &origin, QAngle &angles,
  ...
  m_vLagAngles = angles;
  m_LagAnglesHistory.NoteChanged(gpGlobals->curtime,
                                 cl_wpn_sway_interp.GetFloat(), false);

  // Interpolate back 100ms.
  m_LagAnglesHistory.Interpolate(gpGlobals->curtime,
                                 cl_wpn_sway_interp.GetFloat());

  // Now take the 100ms angle difference and figure out how far the forward
  // vector moved in local space.
  ...
  origin +=
      forward * vForwardDiff.x + right * -vForwardDiff.y + up * vForwardDiff.z;
#endif
}
...
ConVar cl_test_vm_offset("cl_test_vm_offset", "0 0 0",
                         FCVAR_CHEAT | FCVAR_CLIENTDLL);
#endif

void CDODViewModel::CalcViewModelView(CBasePlayer *owner,
  ...
  m_vLoweredWeaponOffset.x =
      Approach(bLowered ? cl_gunlowerangle.GetFloat() : 0,
               m_vLoweredWeaponOffset.x, cl_gunlowerspeed.GetFloat());
