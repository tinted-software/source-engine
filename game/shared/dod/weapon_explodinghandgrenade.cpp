...
public:
  ...
  virtual void EmitGrenade(Vector vecSrc, QAngle vecAngles, Vector vecVel,
    ...
    CDODHandGrenade::Create(vecSrc, vecAngles, vecVel, angImpulse, pPlayer,
                            flLifeTime, GetWeaponID());
  }
...
IMPLEMENT_NETWORKCLASS_ALIASED(WeaponExplodingHandGrenade,
                               DT_WeaponExplodingHandGrenade)

BEGIN_NETWORK_TABLE(CWeaponExplodingHandGrenade, DT_WeaponExplodingHandGrenade)
END_NETWORK_TABLE()

BEGIN_PREDICTION_DATA(
    CWeaponExplodingHandGrenade) // MATTTODO: are these necessary?
END_PREDICTION_DATA()
