...
#define VEC_PRONE_HULL_MIN_SCALED(player)                                      \
  (DODGameRules()->GetDODViewVectors()->m_vProneHullMin *                      \
   player->GetModelScale())
#define VEC_PRONE_HULL_MAX_SCALED(player)                                      \
  (DODGameRules()->GetDODViewVectors()->m_vProneHullMax *                      \
   player->GetModelScale())
...
typedef struct DodVoiceCommand_s {
  const char
      *pszCommandName; // console command that will produce the voice command
...
#define VEC_PRONE_VIEW_SCALED(player)                                          \
  (Vector(0, 0, 10) * player->GetModelScale())
...
extern const char
    *g_pszAchievementAwardMaterials_Allies[NUM_ACHIEVEMENT_AWARDS];
