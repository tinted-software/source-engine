...
void CDODBaseBombWeapon::Precache() { BaseClass::Precache(); }
...
void CDODBaseBombWeapon::PrimaryAttack() {
  ...
  if (IsPlanting()) {
    ...
    if (pTarget->CanPlantHere(GetDODPlayerOwner()) == false) {
      // if the target is not active anymore, cancel ( someone planted there
      // already? )
      CancelPlanting();
    } else if ((pTarget->GetAbsOrigin() - pPlayer->WorldSpaceCenter())
                   .Length() > DOD_BOMB_PLANT_RADIUS) {
      // if we're too far away, cancel
      CancelPlanting();
    } else if (IsLookingAtBombTarget(pPlayer, pTarget) == false ||
               (pPlayer->GetFlags() & FL_ONGROUND) == 0) {
  ...
  while ((pEnt = gEntList.FindEntityByClassname(pEnt, "dod_bomb_target")) !=
    ...
    // if we are looking directly at a bomb target and it is within our radius,
...
void CDODBaseBombWeapon::SecondaryAttack() { PrimaryAttack(); }
...
bool CDODBaseBombWeapon::IsLookingAtBombTarget(CBasePlayer *pPlayer,
                                               CDODBombTarget *pTarget) {
  Vector forward;
...
void CDODBaseBombWeapon::SetPlanting(bool bPlanting) {
  ...
  if (pPlayer) {
    pPlayer->SetPlanting(m_bPlanting ? (CDODBombTarget *)m_hBombTarget.Get()
                                     : NULL);
  }
#endif
}

bool CDODBaseBombWeapon::IsPlanting(void) { return m_bPlanting; }