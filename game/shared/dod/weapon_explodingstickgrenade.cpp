...
public:
  ...
  virtual void EmitGrenade(Vector vecSrc, QAngle vecAngles, Vector vecVel,
                           ...
                           float flLifeTime = GRENADE_FUSE_LENGTH) {
    ...
    CDODStickGrenade::Create(vecSrc, vecNewAngles, vecVel, angNewImpulse,
                             pPlayer, flLifeTime, GetWeaponID());
  }
...
IMPLEMENT_NETWORKCLASS_ALIASED(WeaponExplodingStickGrenade,
                               DT_WeaponExplodingStickGrenade)

BEGIN_NETWORK_TABLE(CWeaponExplodingStickGrenade,
                    DT_WeaponExplodingStickGrenade)
END_NETWORK_TABLE()

BEGIN_PREDICTION_DATA(
    CWeaponExplodingStickGrenade) // MATTTODO: are these necessary?
END_PREDICTION_DATA()
