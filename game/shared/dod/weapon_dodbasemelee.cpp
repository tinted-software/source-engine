...
BEGIN_NETWORK_TABLE_NOBASE(CWeaponDODBaseMelee,
...
void CWeaponDODBaseMelee::Spawn() {
  ...
  CDODWeaponInfo *pWeaponInfo =
      dynamic_cast<CDODWeaponInfo *>(GetFileWeaponInfoFromHandle(hWpnInfo));
...
CBaseEntity *CWeaponDODBaseMelee::MeleeAttack(int iDamageAmount,
                                              int iDamageType, float flDmgDelay,
                                              float flAttackDelay) {
  if (!CanAttack())
    return NULL;
  ...
  if (bDidHit && tr.m_pEnt->IsPlayer() &&
      tr.m_pEnt->m_takedamage != DAMAGE_YES) {
    bDidHit = 0; // still play the animation, we just dont attempt to damage
                 // this player
  }
  ...
  if (bDidHit) // if the swing hit
  {
    ...
    if (tr.m_pEnt->IsPlayer() && tr.m_pEnt->m_takedamage == DAMAGE_YES &&
        (iVictimTeam != iOwnerTeam ||
         (iVictimTeam == iOwnerTeam && friendlyfire.GetBool()))) {
