...
public:
  ...
  void ViewOffsetAnimation(Vector vecDest, float flTime,
...
protected:
  ...
  void TracePlayerBBoxWithStep(const Vector &vStart, const Vector &vEnd,
...
void CDODGameMovement::SetPlayerSpeed(void) {
  ...
  } else // not prone, dead or deployed - standing or crouching and possibly
         // moving
  {
    ...
    // check for slowed from leg hit or firing a machine gun
      ...
      if ((mv->m_nButtons & IN_SPEED) && (stamina > 0) &&
          (mv->m_nButtons & IN_FORWARD) && !bZoomingIn) {
  ...
  if (m_pDODPlayer->GetGroundEntity() != NULL) {
    if (m_pDODPlayer->m_Shared.IsGoingProne()) {
      float pronetime =
          m_pDODPlayer->m_Shared.m_flGoProneTime - gpGlobals->curtime;
      ...
      mv->m_flClientMaxSpeed = ((1 - flProneFraction) * PLAYER_SPEED_PRONE) +
                               (flProneFraction * maxSpeed);
    } else if (m_pDODPlayer->m_Shared.IsGettingUpFromProne()) {
      float pronetime =
          m_pDODPlayer->m_Shared.m_flUnProneTime - gpGlobals->curtime;
      ...
      mv->m_flClientMaxSpeed = (flProneFraction * PLAYER_SPEED_PRONE) +
                               ((1 - flProneFraction) * maxSpeed);
    }
  }
}

ConVar cl_show_speed("cl_show_speed", "0", FCVAR_CHEAT | FCVAR_REPLICATED,
                     "spam console with local player speed");
...
void CDODGameMovement::ProcessMovement(CBasePlayer *pBasePlayer,
                                       CMoveData *pMove) {
...
void CDODGameMovement::WalkMove(void) {
  ...
  if (bSprintButtonPressed && (mv->m_nButtons & IN_FORWARD) &&
...
void CDODGameMovement::CheckForLadders(bool wasOnGround) {
  if (!wasOnGround || !ResolveStanding()) {
    trace_t trace;
    TracePlayerBBox(mv->GetAbsOrigin(), mv->GetAbsOrigin() + Vector(0, 0, -5),
                    MASK_PLAYERSOLID, COLLISION_GROUP_PLAYER_MOVEMENT, trace);
    if (trace.fraction == 1.0f) {
      ...
      vel *= 5.0f;
      Vector vecStartPos(mv->GetAbsOrigin().x + vel.x,
                         mv->GetAbsOrigin().y + vel.y, mv->GetAbsOrigin().z);
      vel *= 5.0f;
      Vector vecStandPos(
          mv->GetAbsOrigin().x - vel.x, mv->GetAbsOrigin().y - vel.y,
          mv->GetAbsOrigin().z - (player->m_Local.m_flStepSize * 1.0f));

      TracePlayerBBoxWithStep(vecStartPos, vecStandPos, MASK_PLAYERSOLID,
...
inline void CDODGameMovement::TracePlayerBBoxWithStep(const Vector &vStart,
                                                      const Vector &vEnd,
                                                      unsigned int fMask,
                                                      int collisionGroup,
                                                      trace_t &trace) {
...
bool CDODGameMovement::ResolveStanding(void) {
  ...
  Vector vecStandPos(mv->GetAbsOrigin().x, mv->GetAbsOrigin().y,
                     mv->GetAbsOrigin().z - (flMaxStepDrop));

  trace_t trace;
  TracePlayerBBoxWithStep(mv->GetAbsOrigin(), vecStandPos, MASK_PLAYERSOLID,
                          COLLISION_GROUP_PLAYER_MOVEMENT, trace);
  ...
  if (trace.fraction > 0.5f) {
    trace.fraction -= 0.5f;
    Vector vecNewOrigin;
    vecNewOrigin = mv->GetAbsOrigin() +
                   trace.fraction * (vecStandPos - mv->GetAbsOrigin());
  ...
  vecStandPos.z =
      (mv->GetAbsOrigin().z +
       ((0.5f - trace.fraction) * (player->m_Local.m_flStepSize * 2.0f)));
  TracePlayerBBoxWithStep(mv->GetAbsOrigin(), vecStandPos, MASK_PLAYERSOLID,
                          COLLISION_GROUP_PLAYER_MOVEMENT, trace);
...
void CDODGameMovement::ReduceTimers(void) {
  ...
  bool bSprinting =
      ((mv->m_nButtons & IN_SPEED) && (mv->m_nButtons & IN_FORWARD));

  // If we're holding the sprint key and also actually moving, remove some
  // stamina
  Vector vel = m_pDODPlayer->GetAbsVelocity();
...
bool CDODGameMovement::CheckJumpButton(void) {
  ...
  m_pDODPlayer->PlayStepSound((Vector &)mv->GetAbsOrigin(),
                              player->GetSurfaceData(), 1.0, true);
  ...
  */
  ...
  if ((m_pDODPlayer->m_Local.m_bDucking) ||
      (m_pDODPlayer->GetFlags() & FL_DUCKING)) {
    ...
    mv->m_vecVelocity[2] =
        flGroundFactor * 268.3281572999747f; // flJumpSpeed of 45
    // mv->m_vecVelocity[2] = flGroundFactor * sqrt(2 * 800 * flJumpSpeed);  //
    // 2 * gravity * height
  } else {
    mv->m_vecVelocity[2] +=
        flGroundFactor * 268.3281572999747f; // flJumpSpeed of 45
...
bool CDODGameMovement::CanUnprone() {
  ...
  if (player->GetGroundEntity() != NULL) {
    ...
    Vector hullSizeProne =
        VEC_PRONE_HULL_MAX_SCALED(player) - VEC_PRONE_HULL_MIN_SCALED(player);
  ...
  TracePlayerBBox(mv->GetAbsOrigin(), newOrigin, MASK_PLAYERSOLID,
                  COLLISION_GROUP_PLAYER_MOVEMENT, trace);
...
void CDODGameMovement::SetDeployedEyeOffset(void) {
  ...
  float flTimeSinceDeployChange =
      gpGlobals->curtime - m_pDODPlayer->m_Shared.m_flDeployChangeTime;

  if (m_pDODPlayer->m_Shared.IsInMGDeploy() ||
      flTimeSinceDeployChange < TIME_TO_DEPLOY) {
    if (m_pDODPlayer->m_Shared.IsInMGDeploy()) {
      // anim to deployed
      if (m_pDODPlayer->m_Shared.GetLastViewAnimTime() <
          m_pDODPlayer->m_Shared.m_flDeployChangeTime.m_Value) {
        // Deployed height
        float flViewOffset =
            clamp(m_pDODPlayer->m_Shared.GetDeployedHeight(),
                  CROUCHING_DEPLOY_HEIGHT, STANDING_DEPLOY_HEIGHT);
      ...
      if (m_pDODPlayer->m_Shared.GetLastViewAnimTime() <
          m_pDODPlayer->m_Shared.m_flDeployChangeTime.m_Value) {
        ViewOffsetAnimation(GetPlayerViewOffset(player->m_Local.m_bDucked),
                            TIME_TO_DEPLOY, VIEW_ANIM_LINEAR_Z_ONLY);
        m_pDODPlayer->m_Shared.SetLastViewAnimTime(gpGlobals->curtime);
      }
    }
...
void CDODGameMovement::SetProneEyeOffset(float proneFraction) {
  ...
  Vector temp = player->GetViewOffset();
  temp.z = SimpleSplineRemapVal(proneFraction, 1.0, 0.0, vecPropViewOffset.z,
                                vecStandViewOffset.z);
...
void CDODGameMovement::Duck(void) {
  ...
  if (gpGlobals->curtime > m_pDODPlayer->m_Shared.m_flNextProneCheck) {
    if (buttonsPressed & IN_ALT1 &&
      ...
      if (m_pDODPlayer->m_Shared.IsProne() == false &&
        ...
        // do unprone anim
        ViewOffsetAnimation(VEC_PRONE_VIEW, TIME_TO_PRONE,
        ...
        ViewOffsetAnimation(GetPlayerViewOffset(m_bUnProneToDuck),
                            TIME_TO_PRONE, VIEW_ANIM_EXPONENTIAL_Z_ONLY);
      ...
      CanUnprone()) // BUGBUG - even calling this will unzoom snipers.
  {
    ...
    // do unprone anim
    ViewOffsetAnimation(GetPlayerViewOffset(m_bUnProneToDuck), TIME_TO_PRONE,
                        VIEW_ANIM_EXPONENTIAL_Z_ONLY);
...
void CDODGameMovement::ViewOffsetAnimation(Vector vecDest, float flTime,
                                           ViewAnimationType type) {
