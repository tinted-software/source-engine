...
ConVar dod_bodyheightoffset("dod_bodyheightoffset", "4",
                            FCVAR_CHEAT | FCVAR_REPLICATED,
                            "Deploy height offset.");
...
class CDODPlayerAnimState : public CBasePlayerAnimState,
                            public IDODPlayerAnimState {
public:
  ...
  virtual int CalcAimLayerSequence(float *flCyle, float *flAimSequenceWeight,
                                   bool bForceIdle) {
    return 0;
  }
...
private:
  ...
  bool m_bLastDeployState; // true = last was deployed, false = last was not
                           // deployed
...
void CDODPlayerAnimState::DoAnimationEvent(PlayerAnimEvent_t event, int nData) {
  if (event == PLAYERANIMEVENT_FIRE_GUN) {
    ...
    if (GetOuterDOD()->m_Shared.IsBazookaDeployed()) {
      m_flFireIdleTime =
          gpGlobals->curtime + 0.1; // don't hold this pose after firing
    } else {
  ...
  if (event == PLAYERANIMEVENT_SECONDARY_ATTACK) {
    ...
    if (pPlayer &&
        !(pPlayer->m_Shared.IsBazookaDeployed() ||
          pPlayer->m_Shared.IsProne() || pPlayer->m_Shared.IsProneDeployed() ||
          pPlayer->m_Shared.IsSniperZoomed() ||
...
//-----------------------------------------------------------------------------
bool CDODPlayerAnimState::HandleProneDown(CDODPlayer *pPlayer,
...
// Purpose: Handle the prone up animation.
//-----------------------------------------------------------------------------
bool CDODPlayerAnimState::HandleProneUp(CDODPlayer *pPlayer,
                                        Activity *idealActivity) {
  if ((m_pOuter->GetCycle() > 0.99f) ||
      (!pPlayer->m_Shared.IsGettingUpFromProne())) {
    m_bGettingUp = false;
    RestartMainSequence();
...
Activity CDODPlayerAnimState::CalcMainActivity() {
  ...
  if (HandleJumping(&idealActivity) || HandleProne(&idealActivity) ||
  ...
  // Shouldn't be here but we need to ship - bazooka deployed reload/running
  // check.
  if (IsPlayingGesture(GESTURE_RELOAD)) {
    if (flSpeed >= DOD_RUN_SPEED &&
        m_pOuterDOD->m_Shared.IsBazookaOnlyDeployed()) {
      CancelGestures();
    }
  }
...
void CDODPlayerAnimState::RestartGesture(int iGestureType, Activity act,
                                         bool bAutoKill /* = true */) {
...
Activity CDODPlayerAnimState::TranslateActivity(Activity actDesired) {
  ...
  } else if (m_pOuterDOD->m_Shared.IsSniperZoomed() ||
             m_pOuterDOD->m_Shared.IsBazookaDeployed()) {
    switch (idealActivity) {
    case ACT_IDLE:
...
float CDODPlayerAnimState::CalcMovementPlaybackRate(bool *bIsMoving) {
  ...
  // it would be a good idea to ramp up from 0.5 to 1.0 as they go from stop to
  // moveing, it looks more natural.
  *bIsMoving = true;
  return 1.0;
}
...
void CDODPlayerAnimState::ComputeSequences(CStudioHdr *pStudioHdr) {
  ...
  if (!m_bGettingDown && !m_bGettingUp) {
    ...
    // get the weapon's swap criteria ( reload? attack? deployed? deployed
    // reload? ) and determine whether we should use alt model or not
    ...
    if (pWeapon) {
      ...
      if (!IsPlayingGesture(GESTURE_HAND_SIGNAL) &&
          !IsPlayingGesture(GESTURE_FIDGET) && !m_bGettingDown &&
          !m_bGettingUp) {
...
void CDODPlayerAnimState::ComputeGestureSequence(CStudioHdr *pStudioHdr) {
  UpdateLayerSequenceGeneric(pStudioHdr, GESTURE_LAYER, m_bPlayingGesture,
                             m_flGestureCycle, m_iGestureSequence,
                             !m_bAutokillGesture);
}

void CDODPlayerAnimState::UpdateLayerSequenceGeneric(
    CStudioHdr *pStudioHdr, int iLayer, bool &bEnabled, float &flCurCycle,
...
void CDODPlayerAnimState::ComputePoseParam_MoveYaw(CStudioHdr *pStudioHdr) {
  ...
  if (GetOuterDOD()->m_Shared.IsInMGDeploy() ||
      GetOuterDOD()->m_Shared.IsProne()) {
    ...
    if (vecEstVelocity.x != 0.0f &&
        fabs(vecEstVelocity.x) > fabs(vecEstVelocity.y)) {
    ...
    {
            GetOuter()->SetPoseParameter( pStudioHdr, m_iMoveX,
    vecCurrentMoveYaw.x ); GetOuter()->SetPoseParameter( pStudioHdr, m_iMoveY,
    vecCurrentMoveYaw.y ); GetOuterDOD()->GetBlendedLinearVelocity(
    &vecAnimVelocity ); DevMsgRT("(%.2f) %.3f : (%.2f) %.3f\n",
    ...
    */
    ...
    do {
      ...
      retry =
          retry ||
          ((vecCurrentMoveYaw.y < 1.0 && vecCurrentMoveYaw.y > -1.0) &&
...
void CDODPlayerAnimState::ComputePoseParam_AimYaw(CStudioHdr *pStudioHdr) {
  ...
  bool bDeployed = GetOuterDOD()->m_Shared.IsSandbagDeployed() ||
                   GetOuterDOD()->m_Shared.IsProneDeployed();
  ...
  if (m_flGoalFeetYaw != m_flCurrentFeetYaw) {
    ConvergeYawAngles(m_flGoalFeetYaw, DOD_BODYYAW_RATE, gpGlobals->frametime,
                      m_flCurrentFeetYaw);
    m_flLastAimTurnTime = gpGlobals->curtime;
  }
...
// Purpose:
//-----------------------------------------------------------------------------
void CDODPlayerAnimState::ConvergeYawAngles(float flGoalYaw, float flYawRate,
                                            float flDeltaTime,
                                            float &flCurrentYaw) {
...
void CDODPlayerAnimState::ComputePoseParam_BodyHeight(CStudioHdr *pStudioHdr) {
  if (m_pOuterDOD->m_Shared.IsSandbagDeployed()) {
    //		float flHeight = m_pOuterDOD->m_Shared.GetDeployedHeight()
    //- 4.0f;
    float flHeight = m_pOuterDOD->m_Shared.GetDeployedHeight() -
                     dod_bodyheightoffset.GetFloat();
...
void CDODPlayerAnimState::DebugShowAnimStateForPlayer(bool bIsServer) {
  ...
  for (int iAnim = 0; iAnim < GetOuter()->GetNumAnimOverlays(); ++iAnim) {
    ...
    if (pLayer && (pLayer->m_nOrder != CBaseAnimatingOverlay::MAX_OVERLAYS)) {
      Anim_StatePrintf(
          ...
          GetSequenceName(GetOuter()->GetModelPtr(), pLayer->m_nSequence),
  ...
  Anim_StatePrintf(iLine++,
                   "EntityYaw: %.2f, AimYaw: %.2f, AimPitch: %.2f, MoveX: "
                   "%.2f, MoveY: %.2f Body: %.2f",
                   m_angRender[YAW], m_flLastAimYaw, m_flLastAimPitch,
                   m_vecLastMoveYaw.x, m_vecLastMoveYaw.y, m_flLastBodyHeight);
