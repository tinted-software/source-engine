...
public:
  ...
  bool TestDeploy(float *flDeployedHeight, CBaseEntity **pDeployedOn,
                  float *flYawLimitLeft = NULL, float *flYawLimitRight = NULL);
  bool TestDeployAngle(CDODPlayer *pPlayer, float *flDeployedHeight,
                       CBaseEntity **pDeployedOn, QAngle angles);

  bool FindYawLimits(float *flLeftLimit, float *flRightLimit);

  virtual void DoFireEffects();

  void DeployBipod(float flHeight, CBaseEntity *pDeployedOn,
