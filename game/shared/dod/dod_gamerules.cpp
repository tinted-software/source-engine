...
RecvPropDataTable("dod_gamerules_data", 0, 0,
                  ...
                  RecvProxy_DODGameRules) END_RECV_TABLE()
#else
...
void CDODGameRules::UploadLevelStats(void) {
  if (Q_strlen(STRING(gpGlobals->mapname)) > 0) {
    ...
    // convert to minutes
    for (j = 0; j < 7; j++) {
      stats.iMinutesPlayedPerClass_Allies[j] = clamp(
          (short)(m_flSecondsPlayedPerClass_Allies[j] / 60), 0, MY_USHRT_MAX);
      stats.iMinutesPlayedPerClass_Axis[j] = clamp(
          (short)(m_flSecondsPlayedPerClass_Axis[j] / 60), 0, MY_USHRT_MAX);
    }

    for (i = 0; i < 6; i++) {
      stats.iKillsPerClass_Allies[i] =
          clamp((short)m_iStatsKillsPerClass_Allies[i], 0, MY_USHRT_MAX);
      stats.iKillsPerClass_Axis[i] =
          clamp((short)m_iStatsKillsPerClass_Axis[i], 0, MY_USHRT_MAX);

      stats.iSpawnsPerClass_Allies[i] =
          clamp((short)m_iStatsSpawnsPerClass_Allies[i], 0, MY_USHRT_MAX);
      stats.iSpawnsPerClass_Axis[i] =
          clamp((short)m_iStatsSpawnsPerClass_Axis[i], 0, MY_USHRT_MAX);

      stats.iCapsPerClass_Allies[i] =
          clamp((short)m_iStatsCapsPerClass_Allies[i], 0, MY_USHRT_MAX);
      stats.iCapsPerClass_Axis[i] =
          clamp((short)m_iStatsCapsPerClass_Axis[i], 0, MY_USHRT_MAX);

      stats.iDefensesPerClass_Allies[i] =
          clamp(m_iStatsDefensesPerClass_Allies[i], 0, MY_UCHAR_MAX);
      stats.iDefensesPerClass_Axis[i] =
          clamp(m_iStatsDefensesPerClass_Axis[i], 0, MY_UCHAR_MAX);
    }

    // Server Settings
    stats.iClassLimits_Allies[0] =
        clamp(mp_limitAlliesRifleman.GetInt(), -1, 254);
    stats.iClassLimits_Allies[1] =
        clamp(mp_limitAlliesAssault.GetInt(), -1, 254);
    stats.iClassLimits_Allies[2] =
        clamp(mp_limitAlliesSupport.GetInt(), -1, 254);
    stats.iClassLimits_Allies[3] =
        clamp(mp_limitAlliesSniper.GetInt(), -1, 254);
    stats.iClassLimits_Allies[4] =
        clamp(mp_limitAlliesMachinegun.GetInt(), -1, 254);
    stats.iClassLimits_Allies[5] =
        clamp(mp_limitAlliesRocket.GetInt(), -1, 254);
    ...
    stats.iClassLimits_Axis[4] =
        clamp(mp_limitAxisMachinegun.GetInt(), -1, 254);
    ...
    for (i = 0; i < DOD_NUM_DISTANCE_STAT_WEAPONS; i++) {
      ...
      stats.weaponStatsDistance[i].iNumHits =
          clamp(m_iWeaponShotsHit[weaponId], 0, MY_USHRT_MAX);
      stats.weaponStatsDistance[i].iNumAttacks =
          clamp(m_iWeaponShotsFired[weaponId], 0, MY_USHRT_MAX);
...
bool CDODGameRules::ShouldCollide(int collisionGroup0, int collisionGroup1) {
  ...
  if (collisionGroup1 == DOD_COLLISIONGROUP_BLOCKERWALL)
    return (collisionGroup0 == COLLISION_GROUP_PLAYER) ||
...
void CDODGameRules::State_Enter_PREROUND(void) {
  ...
  if (!pEnt) {
    Warning("No dod_control_point_master found in level - control points will "
...
void CDODGameRules::SetWinningTeam(int team) {
  ...
  if (winEvent) {
    ...
    CDODTeam *pLosingTeam =
        GetGlobalDODTeam((team == TEAM_ALLIES) ? TEAM_AXIS : TEAM_ALLIES);

    iNumPlayers = pLosingTeam->GetNumPlayers();

    for (i = 0; i < iNumPlayers; i++) {
      CDODPlayer *pPlayer =
          dynamic_cast<CDODPlayer *>(pLosingTeam->GetPlayer(i));
...
void CDODGameRules::AddWaveTime(int team, float flTime) {
  ...
  case TEAM_ALLIES: {
    ...
    DevMsg(1, "AddWaveTime ALLIES head %d tail %d numtotal %d time %.1f\n",
           m_iAlliesRespawnHead.Get(), m_iAlliesRespawnTail.Get(),
  ...
  case TEAM_AXIS: {
    ...
    DevMsg(1, "AddWaveTime AXIS head %d tail %d numtotal %d time %.1f\n",
           m_iAxisRespawnHead.Get(), m_iAxisRespawnTail.Get(),
           m_iNumAxisRespawnWaves, gpGlobals->curtime + flTime);
  } break;
  default:
    Assert(0);
    break;
...
void CDODGameRules::PopWaveTime(int team) {
  ...
  case TEAM_ALLIES: {
    ...
    DevMsg(1, "PopWaveTime ALLIES head %d tail %d numtotal %d time %.1f\n",
           m_iAlliesRespawnHead.Get(), m_iAlliesRespawnTail.Get(),
           m_iNumAlliesRespawnWaves, gpGlobals->curtime);
  } break;
  case TEAM_AXIS: {
    ...
    DevMsg(1, "PopWaveTime AXIS head %d tail %d numtotal %d time %.1f\n",
           m_iAxisRespawnHead.Get(), m_iAxisRespawnTail.Get(),
           m_iNumAxisRespawnWaves, gpGlobals->curtime);
  } break;
  default:
    Assert(0);
    break;
...
//-----------------------------------------------------------------------------
void CDODGameRules::CalcDominationAndRevenge(CDODPlayer *pAttacker,
                                             CDODPlayer *pVictim,
                                             int *piDeathFlags) {
  ...
  int iKillsUnanswered =
      ++(pVictim->iNumKilledByUnanswered[pAttacker->entindex()]);
...
//-----------------------------------------------------------------------------
CBaseCombatWeapon *
CDODGameRules::GetNextBestWeapon(CBaseCombatCharacter *pPlayer,
                                 CBaseCombatWeapon *pCurrentWeapon) {
  CBaseCombatWeapon *pCheck;
  CBaseCombatWeapon *pBest; // this will be used in the event that we don't find
                            // a weapon in the same category.
  ...
  // if pBest is null, we didn't find ANYTHING. Shouldn't be possible- should
...
ConVar dod_winter_never_drop_presents("dod_winter_never_drop_presents", "0",
                                      FCVAR_CHEAT);
ConVar dod_winter_always_drop_presents("dod_winter_always_drop_presents", "0",
                                       FCVAR_CHEAT);
ConVar dod_winter_present_drop_chance("dod_winter_present_drop_chance", "0.2",
...
// Purpose:
//-----------------------------------------------------------------------------
bool CFuncNewTeamWall::ShouldCollide(int collisionGroup,
...
IMPLEMENT_CLIENTCLASS_DT(C_FuncNewTeamWall, DT_FuncNewTeamWall,
                         CFuncNewTeamWall)
END_RECV_TABLE()
...
// Purpose:
//-----------------------------------------------------------------------------
bool C_FuncNewTeamWall::ShouldCollide(int collisionGroup,
                                      int contentsMask) const {
  if (GetTeamNumber() == TEAM_UNASSIGNED)
    return false;
...
int C_FuncNewTeamWall::DrawModel(int flags) { return 1; }
