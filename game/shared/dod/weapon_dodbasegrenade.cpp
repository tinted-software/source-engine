...
RecvPropBool(RECVINFO(m_bPinPulled)), RecvPropBool(RECVINFO(m_bArmed)),
    ...
    END_NETWORK_TABLE()
        ...
        BEGIN_PREDICTION_DATA(CWeaponDODBaseGrenade) END_PREDICTION_DATA()
...
acttable_t CWeaponDODBaseGrenade::m_acttable[] = {
    ...
    {ACT_DOD_PRIMARYATTACK_CROUCH, ACT_DOD_PRIMARYATTACK_CROUCH_GREN_FRAG,
     false},
...
enum { THROW_THROW = 0, THROW_PRIME };
...
void CWeaponDODBaseGrenade::ItemPostFrame() {
  ...
  if (m_bPinPulled && !(pPlayer->m_nButtons & IN_ATTACK) &&
      !(pPlayer->m_nButtons & IN_ATTACK2) // If they let go of the fire button,
                                          // they want to throw the grenade.
  ) {
...
void CWeaponDODBaseGrenade::ThrowGrenade(bool bDrop) {
  ...
  Vector vecThrow =
      DotProduct(vecPlayerVel, vecPlayerForward) * vecPlayerForward;
  ...
  } else // we are throwing the grenade
  {
    ...
    flSpeed = clamp(flSpeed, dod_grenademinspeed.GetFloat(),
                    dod_grenademaxspeed.GetFloat());
...
// Purpose:
//-----------------------------------------------------------------------------
void CWeaponDODBaseGrenade::EmitGrenade(
    Vector vecSrc, QAngle vecAngles, Vector vecVel, AngularImpulse angImpulse,
