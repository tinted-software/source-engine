...
public:
...
#ifndef CLIENT_DLL
  float
  GetPresentDropChance(void); // holiday 2011, presents instead of ammo boxes
#endif
  ...
  virtual void CalcDominationAndRevenge(CDODPlayer *pAttacker,
                                        CDODPlayer *pVictim, int *piDeathFlags);
...
private:
  ...
  float GetAmountOfEntityVisible(
      ...
      CBaseEntity *pEntityToIgnore); // returns a value from 0 to 1 that is the
...
#define MAX_PLAYERCLASSES_PER_TEAM 16
  ...
  bool m_bInitialSpawn; // first time activating? longer wait time for people to
                        // join
...
public:
  ...
  int m_iWeaponDistanceBuckets
      [WEAPON_MAX][DOD_NUM_WEAPON_DISTANCE_BUCKETS]; // distances of buckets are
                                                     // defined per-weapon
