...
public:
  ...
  void StartAnimation(Vector vecStart, Vector vecDest, float flTime,
  ...
  void Think(void) {
    ...
    if (m_pPlayer) {
      ...
      case VIEW_ANIM_LINEAR_Z_ONLY:
        vecCurrentView.z =
            flFraction * m_vecStart.z + (1.0 - flFraction) * m_vecDest.z;
        break;

      case VIEW_ANIM_SPLINE_Z_ONLY:
        vecCurrentView.z = SimpleSplineRemapVal(flFraction, 1.0, 0.0,
                                                m_vecStart.z, m_vecDest.z);
        break;
...
public:
  float
      m_flNextProneCheck; // Prevent it switching their prone state constantly.
