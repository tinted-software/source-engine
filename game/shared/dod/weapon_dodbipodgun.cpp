...
void CDODBipodWeapon::Precache(void) {
  ...
  if (Q_strlen(info.m_szDeployedReloadModel) > 0) {
    ...
    m_iDeployedReloadModelIndex =
        CBaseEntity::PrecacheModel(info.m_szDeployedReloadModel);
  }

  if (Q_strlen(info.m_szProneDeployedReloadModel) > 0) {
    Assert(info.m_iAltWpnCriteria & ALTWPN_CRITERIA_PRONE_DEPLOYED_RELOAD);
    m_iProneDeployedReloadModelIndex =
        CBaseEntity::PrecacheModel(info.m_szProneDeployedReloadModel);
  }
...
bool CDODBipodWeapon::CanDrop(void) { return (IsDeployed() == false); }

bool CDODBipodWeapon::CanHolster(void) { return (IsDeployed() == false); }
...
// in future can be expanded to check when deploying on other ents that may move
// / die / break
...
void CDODBipodWeapon::DoFireEffects() {
  ...
  // in future can be expanded to check when deploying on other ents that may
  // move / die / break
...
void CDODBipodWeapon::DeployBipod(float flHeight, CBaseEntity *pDeployedOn,
                                  float flYawLimitLeft, float flYawLimitRight) {
  ...
  // Save this off so we do duck checks later, even though we won't be flagged
  // as ducking
...
bool CDODBipodWeapon::AttemptToDeploy(void) {
  ...
  if (pPlayer->m_Shared.IsGettingUpFromProne() ||
  ...
  if (TestDeploy(&flDeployedHeight, &pDeployedOn, &flYawLimitLeft,
                 &flYawLimitRight)) {
    if (pPlayer->m_Shared.IsProne() &&
      ...
      if (flDeployedHeight >= flMinDeployHeight) {
        DeployBipod(flDeployedHeight, pDeployedOn, flYawLimitLeft,
                    flYawLimitRight);
        return true;
      }
    }
...
bool CDODBipodWeapon::CheckDeployEnt(void) {
  ...
  if (pDeployedOn) {
    if (m_DeployedEntOrigin != pDeployedOn->GetAbsOrigin()) {
      ...
      TestDeploy(&flDeployedHeight, &pDeployedOn, &flYawLimitLeft,
                 &flYawLimitRight);
...
bool CDODBipodWeapon::TestDeploy(float *flDeployedHeight,
                                 CBaseEntity **pDeployedOn,
                                 float *flYawLimitLeft /* = NULL */,
  ...
  if (flYawLimitLeft == NULL && flYawLimitRight == NULL) {
    ...
    while (flLeft <= flMaxYaw) {
      ...
      if (TestDeployAngle(pPlayer, &flTestDeployHeight, &pTestDeployedOn,
                          angles) == true) {
        if (flLeft ==
            0) // first sweep is authoritative on deploy height and entity
        {
...
public:
  ...
  CTraceFilterIgnorePlayersExceptFor(const IHandleEntity *passentity,
                                     int collisionGroup)
...
bool CDODBipodWeapon::TestDeployAngle(CDODPlayer *pPlayer,
                                      float *flDeployedHeight,
                                      CBaseEntity **pDeployedOn,
                                      QAngle angles) {
  ...
  // check us as ducking if we are ducked, or if were ducked when we were
  // deployed
  bool bDucking =
      pPlayer->m_Shared.IsDucking() || (IsDeployed() && m_bDuckedWhenDeployed);
  ...
  // sandbags are around 50 units high. Shouldn't be able to deploy on anything
  // a lot higher than that

  // optimal standing height ( for animation's sake ) is around 42 units
  // optimal ducking height is around 20 units ( 20 unit high object, plus 8
  // units of gun )

  // Start one half box width away from the edge of the player hull
  Vector vecForwardStart =
      vecStart +
      forward * (VEC_HULL_MAX_SCALED(pPlayer)[0] + vecDeployTraceBoxSize[0]);
  ...
  CTraceFilterIgnorePlayersExceptFor deployedFilter(pDeployedOnPlayer,
  ...
  UTIL_TraceHull(vecForwardStart,
                 vecForwardStart + forward * (flForwardTraceDist -
                                              2 * vecDeployTraceBoxSize[0]),
                 -vecDeployTraceBoxSize, vecDeployTraceBoxSize, traceMask,
                 filter, &tr);
  ...
  if (dod_debugmgdeploy.GetBool()) {
    NDebugOverlay::Line(
        vecForwardStart,
        vecForwardStart +
            forward * (flForwardTraceDist - 2 * vecDeployTraceBoxSize[0]),
        0, 0, 255, true, 0.1);
    NDebugOverlay::Box(vecForwardStart, -vecDeployTraceBoxSize,
                       vecDeployTraceBoxSize, 255, 0, 0, 128, 0.1);
    NDebugOverlay::Box(tr.endpos, -vecDeployTraceBoxSize, vecDeployTraceBoxSize,
                       0, 0, 255, 128, 0.1);
  }
#endif
  ...
  if (pPlayer->m_Shared.IsProne() &&
      !pPlayer->m_Shared.IsGettingUpFromProne()) {
    // MATTTODO: do trace from *front* of player, not from the edge of crouch
    // hull this is sufficient
  ...
  // Else trace down
  Vector vecDownTraceStart =
      vecStart + forward * (VEC_HULL_MAX_SCALED(pPlayer)[0] +
                            DEPLOY_DOWNTRACE_FORWARD_DIST);
  int iTraceHeight = -(pPlayer->WorldAlignMaxs().z);

  // search down from the forward trace
  // use the farthest point first. If that fails, move towards the player a few
  // times to see if they are trying to deploy on a thin railing
  ...
  while (maxAttempts > 0) {
    UTIL_TraceHull(
        vecDownTraceStart,
        vecDownTraceStart +
            Vector(0, 0, iTraceHeight), // trace forward one box width
    ...
    if (dod_debugmgdeploy.GetBool()) {
      ...
      NDebugOverlay::Box(vecDownTraceStart, -vecDeployTraceBoxSize,
                         vecDeployTraceBoxSize, 255, 0, 0, 128, 0.1);
      NDebugOverlay::Box(tr.endpos, -vecDeployTraceBoxSize,
                         vecDeployTraceBoxSize, 0, 0, 255, 128, 0.1);
    }
#endif
  ...
  *flDeployedHeight = flHighestTraceEnd - vecDeployTraceBoxSize[0] +
                      DEPLOY_DOWNTRACE_OFFSET - pPlayer->GetAbsOrigin().z;
  return true;
}

Activity CDODBipodWeapon::GetUndeployActivity(void) { return ACT_VM_UNDEPLOY; }

Activity CDODBipodWeapon::GetDeployActivity(void) { return ACT_VM_DEPLOY; }
...
void CDODBipodWeapon::CheckForAltWeapon(int iCurrentState) {
  ...
  if (bReloading) {
    if (IsDeployed() && iCurrentState & ALTWPN_CRITERIA_PRONE &&
      ...
      m_iCurrentWorldModel =
          m_iProneDeployedReloadModelIndex; // prone deployed reload
...
ConVar
    deployed_mg_sensitivity("deployed_mg_sensitivity", "0.9", FCVAR_CHEAT,
