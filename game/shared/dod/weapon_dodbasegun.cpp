...
// Output : void*
//-----------------------------------------------------------------------------
void *SendProxy_SendActiveLocalBaseGunDataTable(
...
#ifdef CLIENT_DLL
RecvPropDataTable("LocalActiveWeaponBaseGunData", 0, 0,
                  &REFERENCE_RECV_TABLE(DT_LocalActiveWeaponBaseGunData))
#else
SendPropDataTable("LocalActiveWeaponBaseGunData", 0,
                  &REFERENCE_SEND_TABLE(DT_LocalActiveWeaponBaseGunData),
                  SendProxy_SendActiveLocalBaseGunDataTable)
#endif
    END_NETWORK_TABLE()
...
bool CWeaponDODBaseGun::DODBaseGunFire() {
  ...
  FX_FireBullets(pPlayer->entindex(), pPlayer->Weapon_ShootPosition(),
                 ...
                 Primary_Mode, CBaseEntity::GetPredictionRandomSeed() & 255,
...
bool CWeaponDODBaseGun::Reload() {
  ...
  int iResult =
      DefaultReload(GetMaxClip1(), GetMaxClip2(), GetReloadActivity());
  if (!iResult)
    return false;
