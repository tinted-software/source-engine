...
SendPropBool(SENDINFO(m_bSemiAuto))
    ...
    END_NETWORK_TABLE()
        ...
        BEGIN_PREDICTION_DATA(CDODFireSelectWeapon) END_PREDICTION_DATA()
...
bool CDODFireSelectWeapon::IsSemiAuto(void) { return m_bSemiAuto; }
...
Vector CDODFireSelectWeapon::GetDesiredViewModelOffset(C_DODPlayer *pOwner) {
  ...
  float flZoomPercent =
      (m_bAnimToSemiAuto ? (1.0 - flPosChangePercent) : flPosChangePercent);
