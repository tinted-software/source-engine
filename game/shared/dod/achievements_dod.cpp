...
class CAchievementDODThrowBackGren : public CBaseAchievement {
  void Init() {
    // listen for player kill enemy events, base class will increment count each
    // time that happens
  ...
  virtual void Event_EntityKilled(CBaseEntity *pVictim, CBaseEntity *pAttacker,
    ...
    if (pAttacker == pLocalPlayer &&
      ...
      if ((iLocalTeam == TEAM_ALLIES &&
           (FStrEq(killedwith, "frag_ger") ||
            FStrEq(killedwith, "riflegren_ger"))) ||
          (iLocalTeam == TEAM_AXIS && (FStrEq(killedwith, "frag_us") ||
                                       FStrEq(killedwith, "riflegren_us")))) {
...
DECLARE_ACHIEVEMENT(CAchievementDODThrowBackGren,
                    ACHIEVEMENT_DOD_THROW_BACK_GREN, "DOD_THROW_BACK_GREN", 1);
...
DECLARE_ACHIEVEMENT(CAchievementDODConsecutiveHeadshots,
...
DECLARE_ACHIEVEMENT(CAchievementDODMGPositionStreak,
                    ACHIEVEMENT_DOD_MG_POSITION_STREAK,
                    "DOD_MG_POSITION_STREAK", 1);
...
DECLARE_ACHIEVEMENT(CAchievementDODWinKnifeFight,
                    ACHIEVEMENT_DOD_WIN_KNIFE_FIGHT, "DOD_WIN_KNIFE_FIGHT", 1);

const char *pszOfficialMaps[] = {"dod_anzio",  "dod_avalanche", "dod_argentan",
                                 "dod_colmar", "dod_donner",    "dod_flash",
                                 "dod_jagd",   "dod_kalt",      "dod_palermo"};

class CAchievementDODCustomMaps : public CBaseAchievement {
  // Requires a player to kill at least one player on 5 different non-official
  // maps
...
DECLARE_ACHIEVEMENT(CAchievementDODCustomMaps, ACHIEVEMENT_DOD_PLAY_CUSTOM_MAPS,
                    "DOD_PLAY_CUSTOM_MAPS", 1);
...
DECLARE_ACHIEVEMENT(CAchievementDODKillsWithGrenade,
                    ACHIEVEMENT_DOD_KILLS_WITH_GRENADE,
                    "DOD_KILLS_WITH_GRENADE", 1);
...
DECLARE_ACHIEVEMENT(CAchievementDODLongRangeRocket,
                    ACHIEVEMENT_DOD_LONG_RANGE_ROCKET, "DOD_LONG_RANGE_ROCKET",
                    1);
...
class CAchievementDODEndRoundKills : public CBaseAchievement {
  ...
  virtual void ListenForEvents() { ListenForGameEvent("player_spawn"); }

  // Reset the count when we spawn
  void FireGameEvent_Internal(IGameEvent *event) {
    if (m_iKillCount > 0 && 0 == Q_strcmp(event->GetName(), "player_spawn") &&
  ...
  // count kills in endround. No requirement that your team must have won the
  // round - grenades count
  virtual void Event_EntityKilled(CBaseEntity *pVictim, CBaseEntity *pAttacker,
                                  CBaseEntity *pInflictor, IGameEvent *event) {
...
DECLARE_ACHIEVEMENT(CAchievementDODEndRoundKills,
                    ACHIEVEMENT_DOD_END_ROUND_KILLS, "DOD_END_ROUND_KILLS", 1);
...
DECLARE_ACHIEVEMENT(CAchievementDODCapLastFlag, ACHIEVEMENT_DOD_CAP_LAST_FLAG,
                    "DOD_CAP_LAST_FLAG", 1);
...
DECLARE_ACHIEVEMENT(CAchievementDODUseEnemyWeapons,
                    ACHIEVEMENT_DOD_USE_ENEMY_WEAPONS, "DOD_USE_ENEMY_WEAPONS",
                    1);
...
DECLARE_ACHIEVEMENT(CAchievementDODKillDominatingMG,
                    ACHIEVEMENT_DOD_KILL_DOMINATING_MG,
                    "DOD_KILL_DOMINATING_MG", 1);
...
DECLARE_ACHIEVEMENT(CAchievementDODColmarDefense,
                    ACHIEVEMENT_DOD_COLMAR_DEFENSE, "DOD_COLMAR_DEFENSE", 1);
...
DECLARE_ACHIEVEMENT(CAchievementDODJagdOvertimeCap,
                    ACHIEVEMENT_DOD_JAGD_OVERTIME_CAP, "DOD_JAGD_OVERTIME_CAP",
                    1);
...
DECLARE_ACHIEVEMENT(CAchievementDODWeaponMastery,
                    ACHIEVEMENT_DOD_WEAPON_MASTERY, "DOD_WEAPON_MASTERY", 1);
...
class CAchievementDODBlockCaptures : public CBaseAchievement {
  ...
  virtual void ListenForEvents() { ListenForGameEvent("dod_capture_blocked"); }

  // New achievement rule - block a capture that would have lost the game for
  // your team
  ...
  void FireGameEvent_Internal(IGameEvent *event) {
    ...
    if (pPlayer && pPlayer->entindex() == event->GetInt("blocker")) {
      ...
      for (int i = 0; i < g_pObjectiveResource->GetNumControlPoints(); i++) {
        ...
        // if we find any other points owned by us that aren't hidden, this
        // wasn't the last point
...
DECLARE_ACHIEVEMENT(CAchievementDODBlockCaptures,
                    ACHIEVEMENT_DOD_BLOCK_CAPTURES, "DOD_BLOCK_CAPTURES", 1);
...
DECLARE_ACHIEVEMENT(CAchievementKillsAsAllies, ACHIEVEMENT_DOD_KILLS_AS_ALLIES,
                    "DOD_KILLS_AS_ALLIES", 1);

class CAchievementKillsAsAxis : public CBaseAchievementKillsOnTeam {
  virtual int GetTeam(void) { return TEAM_AXIS; }
};
DECLARE_ACHIEVEMENT(CAchievementKillsAsAxis, ACHIEVEMENT_DOD_KILLS_AS_AXIS,
                    "DOD_KILLS_AS_AXIS", 1);
...
#define DECLARE_KILLS_AS_CLASS_ACHIEVEMENT(classIndex, achievementID)          \
  ...
  DECLARE_ACHIEVEMENT(CAchievement_##achievementID,                            \
                      ACHIEVEMENT_##achievementID, #achievementID, 1)
...
#define DECLARE_KILLS_WITH_WEAPON_ACHIEVEMENT(weaponName, goalKills,           \
                                              achievementID)                   \
  class CAchievement_##achievementID                                           \
      : public CBaseAchievementKillsWithWeapon {                               \
    void Init() {                                                              \
      SetFlags(ACH_LISTEN_PLAYER_KILL_ENEMY_EVENTS | ACH_SAVE_GLOBAL);         \
      SetGoal(goalKills);                                                      \
      SetStoreProgressInSteam(true);                                           \
...
DECLARE_KILLS_WITH_WEAPON_ACHIEVEMENT("k98_scoped", 500,
                                      DOD_KILLS_WITH_K98SCOPED);
...
DECLARE_KILLS_WITH_WEAPON_ACHIEVEMENT("m1carbine", 150,
                                      DOD_KILLS_WITH_M1CARBINE);

DECLARE_KILLS_WITH_WEAPON_ACHIEVEMENT("amerknife", 150,
                                      DOD_KILLS_WITH_AMERKNIFE);
...
DECLARE_KILLS_WITH_WEAPON_ACHIEVEMENT("riflegren_us", 250,
                                      DOD_KILLS_WITH_RIFLEGREN_US);
DECLARE_KILLS_WITH_WEAPON_ACHIEVEMENT("riflegren_ger", 250,
                                      DOD_KILLS_WITH_RIFLEGREN_GER);
...
class CAchievementDODFlagCaptureGrind : public CBaseAchievement {
  ...
  virtual void ListenForEvents() { ListenForGameEvent("dod_point_captured"); }
...
DECLARE_ACHIEVEMENT(CAchievementDODFlagCaptureGrind,
                    ACHIEVEMENT_DOD_CAPTURE_GRIND, "DOD_CAPTURE_GRIND", 1);
...
DECLARE_ACHIEVEMENT(CAchievementDODBlockCapturesGrind,
                    ACHIEVEMENT_DOD_BLOCK_CAPTURES_GRIND,
                    "DOD_BLOCK_CAPTURES_GRIND", 1);
...
class CAchievementDODRoundsWonGrind : public CBaseAchievement {
  ...
  virtual void ListenForEvents() { ListenForGameEvent("dod_round_win"); }
...
DECLARE_ACHIEVEMENT(CAchievementDODRoundsWonGrind,
                    ACHIEVEMENT_DOD_ROUNDS_WON_GRIND, "DOD_ROUNDS_WON_GRIND",
                    1);
...
class CAchievementDODBombsPlantedGrind : public CBaseAchievement {
  ...
  virtual void ListenForEvents() { ListenForGameEvent("dod_bomb_planted"); }
...
DECLARE_ACHIEVEMENT(CAchievementDODBombsPlantedGrind,
                    ACHIEVEMENT_DOD_BOMBS_PLANTED_GRIND,
                    "DOD_BOMBS_PLANTED_GRIND", 1);
...
class CAchievementDODBombsDefusedGrind : public CBaseAchievement {
  ...
  virtual void ListenForEvents() { ListenForGameEvent("dod_bomb_defused"); }

  void FireGameEvent_Internal(IGameEvent *event) {
    Assert(FStrEq(event->GetName(), "dod_bomb_defused"));

    if (event->GetInt("userid") ==
        C_BasePlayer::GetLocalPlayer()->GetUserID()) {
      IncrementCount();
    }
  }
};
DECLARE_ACHIEVEMENT(CAchievementDODBombsDefusedGrind,
                    ACHIEVEMENT_DOD_BOMBS_DEFUSED_GRIND,
                    "DOD_BOMBS_DEFUSED_GRIND", 1);
...
DECLARE_ACHIEVEMENT(CAchievementDOD_All_Pack_1, ACHIEVEMENT_DOD_ALL_PACK_1,
                    "DOD_ALL_PACK_1", 5);
...
DECLARE_ACHIEVEMENT(CAchievementDODBeatTheHeat, ACHIEVEMENT_DOD_BEAT_THE_HEAT,
                    "DOD_BEAT_THE_HEAT", 1);
...
DECLARE_ACHIEVEMENT(CAchievementDODCollectHolidayGifts,
