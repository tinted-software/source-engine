...
IMPLEMENT_NETWORKCLASS_ALIASED(WeaponBaseRifleGrenade,
                               DT_WeaponBaseRifleGrenade)
...
void CWeaponBaseRifleGrenade::Spawn() { BaseClass::Spawn(); }
...
void CWeaponBaseRifleGrenade::PrimaryAttack(void) {
  ...
  // decrement before calling PlayPrimaryAttackAnim, so we can play the empty
  // anim if necessary
  m_iClip1--;
  ...
  m_flNextPrimaryAttack = m_flNextSecondaryAttack =
      gpGlobals->curtime + GetFireDelay();
  m_flTimeWeaponIdle =
...
void CWeaponBaseRifleGrenade::ShootGrenade(void) {
  ...
  // raise origin enough so that you can't shoot it straight down through the
  // world
  ...
  // uses a constant ang velocity instead of the derivative of the flight path,
  // oh well.
  ...
  if (vecThrow.z > 0)
    angImpulse.y =
        -angles.x / (sqrt((-2 * vecThrow.z) / dod_grenadegravity.GetFloat()));
  else
    angImpulse.y = -10;

  EmitGrenade(vecSrc, angles, vecThrow, angImpulse, pPlayer,
              RIFLEGRENADE_FUSE_LENGTH);
}

void CWeaponBaseRifleGrenade::EmitGrenade(Vector vecSrc, QAngle vecAngles,
                                          Vector vecVel,
                                          AngularImpulse angImpulse,
                                          CBasePlayer *pPlayer,
                                          float flLifeTime) {
...
bool CWeaponBaseRifleGrenade::CanDeploy(void) {
  ...
  CBaseCombatWeapon *pOwnedWeapon =
      pPlayer->Weapon_OwnsThisType(GetCompanionWeaponName());
