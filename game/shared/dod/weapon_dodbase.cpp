...
DEFINE_PRED_FIELD(m_flSmackTime, FIELD_FLOAT,
                  FTYPEDESC_INSENDTABLE), // for rifle melee attacks
    DEFINE_FIELD(m_bInAttack, FIELD_BOOLEAN) END_PREDICTION_DATA()
#endif
...
void CWeaponDODBase::ItemPostFrame() {
  ...
  if ((pPlayer->m_nButtons & IN_ATTACK2) &&
    ...
    pPlayer->m_nButtons &= ~IN_ATTACK2;
             ...
             (m_flNextPrimaryAttack <= gpGlobals->curtime) && !m_bInAttack) {
...
// Purpose:
//-----------------------------------------------------------------------------
const char *CWeaponDODBase::GetViewModel(
...
#ifdef DEBUG
  // Make sure that if we declare an alt weapon, that we have criteria to show
  // it and vice-versa
  // Assert( ((info.m_iAltWpnCriteria & (ALTWPN_CRITERIA_RELOADING |
  // ALTWPN_CRITERIA_FIRING)) > 0 ) == 		(iWpnNameLen > 0) );
#endif
...
bool CWeaponDODBase::DefaultDeploy(char *szViewModel, char *szWeaponModel,
...
void CWeaponDODBase::PostDataUpdate(DataUpdateType_t updateType) {
  ...
  // clientside animation sequences on this model, which will be using bad
  // sequences for the world model.
  int iDesiredModelIndex = 0;
  C_BasePlayer *localplayer = C_BasePlayer::GetLocalPlayer();
  if (localplayer && localplayer == GetOwner() &&
      !C_BasePlayer::ShouldDrawLocalPlayer()) // FIXME: use
                                              // localplayer->ShouldDrawThisPlayer()
                                              // instead.
  {
...
void CWeaponDODBase::OnDataChanged(DataUpdateType_t type) {
  if (m_iState == WEAPON_NOT_CARRIED && m_iOldState != WEAPON_NOT_CARRIED) {
    ...
    /*Msg( "estimated velocity ( %.1f %.1f %.1f )  initial velocity ( %.1f %.1f
...
void CWeaponDODBase::SetDieThink(bool bDie) {
  if (bDie)
    SetContextThink(&CWeaponDODBase::Die, gpGlobals->curtime + 45.0f,
                    "DieContext");
  else
...
bool CWeaponDODBase::DefaultReload(int iClipSize1, int iClipSize2,
                                   int iActivity) {
  ...
  if (UsesClipsForAmmo1()) {
    ...
    int primary =
        min(iClipSize1 - m_iClip1, pOwner->GetAmmoCount(m_iPrimaryAmmoType));
    if (primary != 0) {
      bReload = true;
    }
  }
  ...
  if (UsesClipsForAmmo2()) {
    ...
    int secondary =
        min(iClipSize2 - m_iClip2, pOwner->GetAmmoCount(m_iSecondaryAmmoType));
    if (secondary != 0) {
      bReload = true;
    }
  }
...
#ifndef CLIENT_DLL
ConVar dod_meleeattackforcescale("dod_meleeattackforcescale", "8.0",
                                 FCVAR_CHEAT | FCVAR_GAMEDLL);
#endif

void CWeaponDODBase::RifleButt(void) {
  // MeleeAttack( 60, MELEE_DMG_BUTTSTOCK | MELEE_DMG_SECONDARYATTACK, 0.2f,
  // 0.9f );
}
...
// flDmgDelay - delay between attack and the giving of damage, usually timed to
// animation flAttackDelay - time until we can next attack
//--------------------------------------------
CBaseEntity *CWeaponDODBase::MeleeAttack(int iDamageAmount, int iDamageType,
                                         float flDmgDelay,
                                         float flAttackDelay) {
  if (!CanAttack())
    return NULL;
  ...
  UTIL_ClipTraceToPlayers(vecSrc, vecEnd + vForward * rayExtension, iTraceMask,
                          &filter, &tr);
  ...
  if (tr.fraction >= 1.0) {
    ...
    UTIL_TraceHull(vecSrc, vecEnd, head_hull_mins, head_hull_maxs, MASK_SOLID,
                   &filter, &tr);
    if (tr.fraction < 1.0) {
      // Calculate the point of intersection of the line (or hull) and the
      // object we hit This is and approximation of the "best" intersection
      CBaseEntity *pHit = tr.m_pEnt;
      if (!pHit || pHit->IsBSPModel())
        FindHullIntersection(vecSrc, tr, VEC_DUCK_HULL_MIN, VEC_DUCK_HULL_MAX,
                             pPlayer);
      vecEnd = tr.endpos; // This is the point on the actual surface (the hull
                          // could have hit space)
  ...
  if (bDidHit) // if the swing hit
  {
    ...
    // Store the ent in an EHANDLE, just in case it goes away by the time we get
    // into our think function.
...
// Think function to delay the impact decal until the animation is finished
// playing
...
//			viewmodelindex -
//-----------------------------------------------------------------------------
void CWeaponDODBase::AddViewmodelBob(CBaseViewModel *viewmodel, Vector &origin,
                                     QAngle &angles) {
...
bool CWeaponDODBase::OnFireEvent(C_BaseViewModel *pViewModel,
  ...
  if (event == 5001) {
    if (ShouldDrawMuzzleFlash()) {
      ...
      if (pszMuzzleFlashEffect) {
        pViewModel->ParticleProp()->Create(pszMuzzleFlashEffect,
    ...
    data.m_hEntity = GetPlayerOwner() ? GetPlayerOwner()->GetRefEHandle()
...
Vector CWeaponDODBase::GetDesiredViewModelOffset(C_DODPlayer *pOwner) {
  ...
  float flPercent =
      (viewOffset.z - VEC_PRONE_VIEW_SCALED(pOwner).z) /
...
bool CWeaponDODBase::ShouldDraw(void) {
  if (GetModel() == NULL) {
    // XXX(johns): Removed, doesn't seem to be the proper spot for this warning
    // given that weapons can call
    //             ShouldDraw before their properties are filled.

    // C_DODPlayer *pPlayer = C_DODPlayer::GetLocalDODPlayer();

    // Warning( "BADNESS! Tell Matt that the weapon '%s' tried to draw with a
    // null model ( %d, %d, %s ) \n", 	GetDODWpnData().szClassName,
    // 	m_iWorldModelIndex.Get(), m_iReloadModelIndex.Get(),
    // m_bUseAltWeaponModel ? "alt" : "not alt" );
...
void CWeaponDODBase::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
