...
ConVar dod_bonusround(
    "dod_bonusround", "1", FCVAR_REPLICATED,
    "If true, the winners of the round can attack in the intermission.");
ConVar
    sv_showimpacts("sv_showimpacts", "0", FCVAR_REPLICATED | FCVAR_CHEAT,
...
static bool TraceToExit(const Vector &start, const Vector &dir, Vector &end,
                        const float flStepSize, const float flMaxDistance) {
  float flDistance = 0;
  Vector last = start;
  ...
  while (flDistance < flMaxDistance) {
    ...
    // point contents fails to return proper contents inside a func_detail
    // brush, eg the dod_flash stairs
...
void CDODPlayer::FireBullets(const FireBulletsInfo_t &info) {
  ...
  static int iTraceMask = ((MASK_SOLID | CONTENTS_DEBRIS | CONTENTS_HITBOX |
                            CONTENTS_PRONE_HELPER) &
                           ~CONTENTS_GRATE);

  CBaseEntity *pLastHitEntity =
      this; // start with us so we don't trace ourselves
  ...
  while (flDamage > 0 && iPenetrations < iMaxPenetrations) {
    ...
    CTraceFilterSkipTwoEntities ignoreShooterAndPrevious(this, pPreviousHit,
                                                         iCollisionGroup);
    UTIL_TraceLine(vecSrc, vecEnd, iTraceMask, &ignoreShooterAndPrevious, &tr);

    const float rayExtension = 40.0f;
    UTIL_ClipTraceToPlayers(vecSrc,
                            vecEnd + info.m_vecDirShooting * rayExtension,
                            iTraceMask, &ignoreShooterAndPrevious, &tr);
    ...
    if (tr.m_pEnt && tr.m_pEnt->IsPlayer()) {
      ...
      case HITGROUP_RIGHTARM: {
        ...
        // re-fire the trace
        UTIL_TraceLine(vecSrc, vecEnd, iTraceMask, &ignoreShooterAndPrevious,
                       &newTr);
        ...
        if (tr.m_pEnt == newTr.m_pEnt) {
          ...
          Assert(tr.hitgroup !=
                 newTr.hitgroup); // If we hit this, hitbox sets are broken
    ...
    if (sv_showimpacts.GetBool()) {
      ...
      debugoverlay->AddBoxOverlay(tr.endpos, Vector(-1, -1, -1),
                                  Vector(1, 1, 1), QAngle(0, 0, 0), 255, 0, 0,
                                  127, 4);
      ...
      NDebugOverlay::Box(tr.endpos, Vector(-1, -1, -1), Vector(1, 1, 1), 0, 0,
                         255, 127, 4);
    ...
    // Get surface where the bullet entered ( if it had different surfaces on
    // enter and exit )
    ...
    CTakeDamageInfo dmgInfo(info.m_pAttacker, info.m_pAttacker, flActualDamage,
                            iDamageType);
    CalculateBulletDamageForce(&dmgInfo, info.m_iAmmoType,
                               info.m_vecDirShooting, tr.endpos);
    tr.m_pEnt->DispatchTraceAttack(dmgInfo, info.m_vecDirShooting, &tr);

    DevMsg(2, "Giving damage ( %.1f ) to entity of type %s\n", flActualDamage,
           tr.m_pEnt->GetClassname());

    TraceAttackToTriggers(dmgInfo, tr.startpos, tr.endpos,
                          info.m_vecDirShooting);
#endif
    ...
    // trace through the solid to find the exit point and how much material we
    // went through
    if (!TraceToExit(tr.endpos, info.m_vecDirShooting, penetrationEnd, stepsize,
                     flMaxDistance)) {
    ...
    UTIL_TraceLine(penetrationEnd, tr.endpos, iTraceMask, &ignoreShooter,
                   &reverseTr);
    ...
    if (reverseTr.m_pEnt != tr.m_pEnt && reverseTr.m_pEnt != NULL) {
      ...
      CTraceFilterSkipTwoEntities ignoreShooterAndBlocker(
          this, reverseTr.m_pEnt, iCollisionGroup);
      UTIL_TraceLine(penetrationEnd, tr.endpos, iTraceMask,
                     &ignoreShooterAndBlocker, &reverseTr);
    }

    if (sv_showimpacts.GetBool()) {
      debugoverlay->AddLineOverlay(penetrationEnd, reverseTr.endpos, 255, 0, 0,
                                   true, 3.0);
    }
    ...
    // Reduce bullet damage by material and distanced travelled through that
    // material if it is < 0 we won't go through the loop again
...
void CDODPlayerShared::SetProne(bool bProne, bool bNoAnimation /* = false */) {
  ...
  if (!bProne /*&& IsSniperZoomed()*/) // forceunzoom for going prone is in
                                       // StartGoingProne
  {
    ForceUnzoom();
  }
...
int CDODPlayerShared::DesiredPlayerClass(void) { return m_iDesiredPlayerClass; }

void CDODPlayerShared::SetPlayerClass(int playerclass) {
  m_iPlayerClass = playerclass;
}

int CDODPlayerShared::PlayerClass(void) { return m_iPlayerClass; }
...
CWeaponDODBase *CDODPlayerShared::GetActiveDODWeapon() const {
  ...
  if (pWeapon) {
    Assert(dynamic_cast<CWeaponDODBase *>(pWeapon) ==
           static_cast<CWeaponDODBase *>(pWeapon));
...
void CDODPlayerShared::ClampDeployedAngles(QAngle *vecTestAngles) {
  ...
  // Clamp Pitch
  vecTestAngles->x =
      clamp(vecTestAngles->x, MAX_DEPLOY_PITCH, MIN_DEPLOY_PITCH);
...
void CDODPlayer::UpdateStepSound(surfacedata_t *psurface,
...
#ifdef CLIENT_DLL
  bool bShouldPlay = (groundspeed > 10) && (onground == true) &&
                     m_Shared.IsProne() && IsAlive();
...
//-----------------------------------------------------------------------------
void CDODPlayer::PlayStepSound(Vector &vecOrigin, surfacedata_t *psurface,
  ...
  unsigned short stepSoundName = m_Local.m_nStepside
                                     ? psurface->sounds.stepleft
...
Activity CDODPlayer::TranslateActivity(Activity baseAct,
                                       bool *pRequired /* = NULL */) {
...
void CDODPlayerShared::ViewOffsetAnimation(Vector vecDest, float flTime,
  ...
  if (m_pViewOffsetAnim) {
    m_pViewOffsetAnim->StartAnimation(m_pOuter->GetViewOffset(), vecDest,
                                      flTime, type);
  }
}
...
void CDODPlayerShared::ComputeWorldSpaceSurroundingBox(Vector *pVecWorldMins,
...
//-----------------------------------------------------------------------------
void CDODPlayerShared::SetPlayerDominated(CDODPlayer *pPlayer,
                                          bool bDominated) {
...
//-----------------------------------------------------------------------------
void CDODPlayerShared::SetPlayerDominatingMe(CDODPlayer *pPlayer,
                                             bool bDominated) {
  int iPlayerIndex = pPlayer->entindex();
