...
SendPropInt(SENDINFO(m_iWeaponHeat), 7, SPROP_UNSIGNED),
    ...
    END_NETWORK_TABLE()
        ...
        BEGIN_PREDICTION_DATA(CWeaponMG42)
            DEFINE_PRED_FIELD(m_iWeaponHeat, FIELD_INTEGER,
                              FTYPEDESC_INSENDTABLE),
    DEFINE_PRED_FIELD_TOL(m_flNextCoolTime, FIELD_FLOAT, FTYPEDESC_INSENDTABLE,
                          TD_MSECTOLERANCE),
...
acttable_t CWeaponMG42::m_acttable[] = {
    ...
    {ACT_DOD_PRIMARYATTACK_PRONE_DEPLOYED,
     ACT_DOD_PRIMARYATTACK_PRONE_DEPLOYED_MG, false},
...
void CWeaponMG42::Spawn(void) {
  ...
  m_hParticleMaterial =
      ParticleMgr()->GetPMaterial("sprites/effects/bazookapuff");
#endif
...
void CWeaponMG42::ClientThink(void) {
  ...
  // Only smoke if we are dropped ( no owner ) or if we have an owner and are
  // active
...
void CWeaponMG42::EmitSmokeParticle(void) {
  ...
  SimpleParticle *pParticle =
      m_pEmitter->AddSimpleParticle(m_hParticleMaterial, vPos);
  if (pParticle) {
    pParticle->m_vecVelocity = Vector(0, 0, 12);
    pParticle->m_flRoll = random->RandomFloat(0, 0.5);
    pParticle->m_flRollDelta =
        (random->RandomInt(0, 1) == 0 ? 1 : -1) * random->RandomFloat(0.5, 1.0);
...
void CWeaponMG42::CoolThink(void) {
  ...
  SetContextThink(&CWeaponMG42::CoolThink, gpGlobals->curtime + 0.1,
                  COOL_CONTEXT);
}
...
void CWeaponMG42::CleanupToolRecordingState(KeyValues *msg) {
  ...
  if (m_pEmitter->GetToolParticleEffectId() == TOOLPARTICLESYSTEMID_INVALID) {
    ...
    KeyValues *pPosition = pInitializers->FindKey(
        "DmeRandomAttachmentPositionEntityInitializer", true);
    ...
    KeyValues *pRollSpeed =
        pInitializers->FindKey("DmeSplitRandomRollSpeedInitializer", true);
