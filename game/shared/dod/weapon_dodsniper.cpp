...
void RecvProxy_AnimStart(const CRecvProxyData *pData, void *pStruct,
                         void *pOut) {
    ...
    SendPropBool(SENDINFO(m_bDoViewAnim))
        ...
        END_NETWORK_TABLE()
            ...
            BEGIN_PREDICTION_DATA(CDODSniperWeapon)
                DEFINE_PRED_FIELD(m_bZoomed, FIELD_BOOLEAN,
...
void CDODSniperWeapon::PrimaryAttack(void) {
  ...
  if (IsZoomed() && ShouldZoomOutBetweenShots()) {
    // If we have more bullets, zoom out, play the bolt animation and zoom back
    // in
    if (m_iClip1 > 0 && m_bShouldRezoomAfterShot &&
        (pPlayer && pPlayer->ShouldAutoRezoom())) {
  ...
  // overwrite the next secondary attack, so we can zoom sooned after we've
  // fired
...
float CDODSniperWeapon::GetWeaponAccuracy(float flPlayerSpeed) {
  // snipers and deployable weapons inherit this and override when we need a
  // different accuracy
...
void CDODSniperWeapon::ZoomIn(void) {
  ...
  m_flNextSecondaryAttack =
      MAX(gpGlobals->curtime + 0.5, m_flNextSecondaryAttack);
  m_flTimeWeaponIdle =
      gpGlobals->curtime + m_pWeaponInfo->m_flTimeToIdleAfterFire;
...
void CDODSniperWeapon::ZoomOut(void) {
  ...
  if (bWasZoomed) {
    m_flNextPrimaryAttack =
        MAX(gpGlobals->curtime + 0.5, m_flNextPrimaryAttack);
    m_flNextSecondaryAttack =
        MAX(gpGlobals->curtime + 0.5, m_flNextSecondaryAttack);
    m_flTimeWeaponIdle =
        gpGlobals->curtime + m_pWeaponInfo->m_flTimeToIdleAfterFire;
...
bool CDODSniperWeapon::IsFullyZoomed(void) {
  return (IsZoomed() == true && ((gpGlobals->curtime - m_flZoomChangeTime) >
                                 DOD_SNIPER_ZOOM_CHANGE_TIME));
}
...
Vector CDODSniperWeapon::GetDesiredViewModelOffset(C_DODPlayer *pOwner) {
  ...
  float flPercent =
      clamp((viewOffset.z - VEC_PRONE_VIEW_SCALED(pOwner).z) /
  ...
  float flZoomAnimPercent = clamp(((m_flViewAnimTimer - gpGlobals->curtime) /
                                   (DOD_SNIPER_ZOOM_CHANGE_TIME)),
                                  0.0, 1.0);

  m_flZoomPercent =
      (IsZoomed() ? (1.0 - flZoomAnimPercent) : flZoomAnimPercent);

  // Msg( "(%.1f) zoom %.2f %.2f\n", gpGlobals->curtime, m_flViewAnimTimer -
  // gpGlobals->curtime, m_flZoomPercent );
