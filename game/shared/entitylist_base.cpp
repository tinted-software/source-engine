...
enum {
  SERIAL_MASK = 0x7fff // the max value of a serial number, rolls back to 0 when
                       // it hits this limit
};
...
void CBaseEntityList::CEntInfoList::LinkBefore(CEntInfo *pBefore,
                                               CEntInfo *pElement) {
  Assert(pElement);
...
void CBaseEntityList::CEntInfoList::LinkAfter(CEntInfo *pAfter,
                                              CEntInfo *pElement) {
  Assert(pElement);
...
CBaseEntityList::CBaseEntityList() {
  ...
  for (i = 0; i < NUM_ENT_ENTRIES; i++) {
    ...
    m_EntPtrArray[i].m_SerialNumber =
        (rand() & SERIAL_MASK); // generate random starting serial number
...
CBaseHandle CBaseEntityList::AddNetworkableEntity(IHandleEntity *pEnt,
                                                  int index,
...
CBaseHandle CBaseEntityList::AddEntityAtSlot(IHandleEntity *pEnt, int iSlot,
                                             int iForcedSerialNum) {
