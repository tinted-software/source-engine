...
int CStudioHdr::CActivityToSequenceMapping::SelectWeightedSequence(
  ...
  if (!ValidateAgainst(pstudiohdr)) {
    AssertMsg1(
        false,
        "CStudioHdr %s has changed its vmodel pointer without reinitializing "
        "its activity mapping! Now performing emergency reinitialization.",
        pstudiohdr->pszName());
    ExecuteOnce(DebuggerBreakIfDebugging());
    Reinitialize(pstudiohdr);
  }

  // a null m_pSequenceTuples just means that this studio header has no
  // activities.
  ...
  const SequenceTuple *const stopHere =
      sequenceInfo +
      actData->count; // this is a backup
                      // in case the weights are somehow miscalculated -- we
                      // don't read or write through it (because it aliases the
                      // restricted pointer above); it's only here for the
                      // comparison.
