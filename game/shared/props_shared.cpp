...
#else
ConVar cl_burninggibs("cl_burninggibs", "0", 0,
                      "A burning player that gibs has burning gibs.");
#endif // GAME_DLL

extern bool
PropBreakableCapEdictsOnCreateAll(int modelindex, IPhysicsObject *pPhysics,
                                  const breakablepropparams_t &params,
                                  CBaseEntity *pEntity,
                                  int iPrecomputedBreakableCount = -1);
extern CBaseEntity *
BreakModelCreateSingle(CBaseEntity *pOwner, breakmodel_t *pModel,
...
//around the unfortunate 			fact that the interaction KV sections
//("physgun_interactions", "fire_interactions", etc) 			are OUTSIDE the "prop_data"
//KV section in the model, but may be contained WITHIN the 			specified Base's
//"prop_data" section (i.e. in propdata.txt)
//-----------------------------------------------------------------------------
...
int CPropData::ParsePropFromBase(CBaseEntity *pProp, const char *pszPropData) {
  ...
  if (!pSection) {
    Warning("%s '%s' has a base specified as '%s', but there is no matching "
            "entry in propdata.txt.\n",
    ...
    for (int i = 0; i < list.Count(); i++) {
      ...
      if (pBreakable) {
        ...
        if (bBurning && cl_burninggibs.GetBool()) {
          pBreakable->ParticleProp()->Create(
              "burninggibs", PATTACH_POINT_FOLLOW, "bloodpoint");
        }
#endif
        if (pOwnerEntity && pOwnerEntity->IsEffectActive(EF_NOSHADOW)) {
          pBreakable->AddEffects(EF_NOSHADOW);
