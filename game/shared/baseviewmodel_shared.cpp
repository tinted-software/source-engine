...
CBaseViewModel::CBaseViewModel() {
#if defined(CLIENT_DLL)
  // NOTE: We do this here because the color is never transmitted for the view
  // model.
  ...
  // Prefix of the animations that should be used by the player carrying this
  // weapon
...
void CBaseViewModel::SpawnControlPanels() {
  ...
  {
          pEntityToSpawnOn =
  dynamic_cast<CBaseAnimating*>((CBaseEntity*)m_hBuiltOnEntity.Get()); if (
  pEntityToSpawnOn )
          {
                  char sBuildPointLL[64];
                  char sBuildPointUR[64];
                  Q_snprintf( sBuildPointLL, sizeof( sBuildPointLL ),
  "bp%d_controlpanel%%d_ll", m_iBuiltOnPoint ); Q_snprintf( sBuildPointUR,
  sizeof( sBuildPointUR ), "bp%d_controlpanel%%d_ur", m_iBuiltOnPoint );
...
// Purpose:
//-----------------------------------------------------------------------------
int CBaseViewModel::ViewModelIndex() const { return m_nViewModelIndex; }
...
// Input  : *modelname -
//-----------------------------------------------------------------------------
void CBaseViewModel::SetWeaponModel(const char *modelname,
...
void CBaseViewModel::SendViewModelMatchingSequence(int sequence) {
  ...
  m_nAnimationParity =
      (m_nAnimationParity + 1) & ((1 << VIEWMODEL_ANIMATION_PARITY_BITS) - 1);
...
void CBaseViewModel::CalcViewModelView(CBasePlayer *owner,
                                       const Vector &eyePosition,
                                       const QAngle &eyeAngles) {
  // UNDONE: Calc this on the server?  Disabled for now as it seems unnecessary
  // to have this info on the server
#if defined(CLIENT_DLL)
  ...
  // Add model-specific bob even if no weapon associated (for head bob for off
  // hand models)
  AddViewModelBob(owner, vmorigin, vmangles);
#if !defined(CSTRIKE_DLL)
  // This was causing weapon jitter when rotating in updated CS:S; original
  // Source had this in above InPrediction block  07/14/10 Add lag
  CalcViewModelLag(vmorigin, vmangles, vmangoriginal);
#endif

#if defined(CLIENT_DLL)
  if (!prediction->InPrediction()) {
    // Let the viewmodel shake at about 10% of the amplitude of the player's
    // view
  ...
  if (UseVR()) {
    g_ClientVirtualReality.OverrideViewModelTransform(
        vmorigin, vmangles,
  ...
  if (g_pSixenseInput->IsEnabled() &&
      (owner->GetObserverMode() == OBS_MODE_NONE) && !UseVR()) {
    const float max_gun_pitch = 20.0f;

    float viewmodel_fov_ratio =
        g_pClientMode->GetViewModelFOV() / owner->GetFOV();
    QAngle gun_angles =
        g_pSixenseInput->GetViewAngleOffset() * -viewmodel_fov_ratio;
...
#ifdef WIN32 // ShouldFlipViewModel comes up unresolved on osx? Mabye because
             // it's defined inline? fixme
...
void CBaseViewModel::CalcViewModelLag(Vector &origin, QAngle &angles,
  ...
  if (gpGlobals->frametime != 0.0f) {
    ...
    // Solves the problem with fast cl_yawspeed, m_yaw or joysticks
    //  rotating quickly.  The old code would slam lastfacing with origin
    //  causing the viewmodel to pop to a new position
    ...
    VectorMA(m_vecLastFacing, flSpeed * gpGlobals->frametime, vDifference,
             m_vecLastFacing);
  ...
  // FIXME: These are the old settings that caused too many exposed polys on
  // some models
...
//-----------------------------------------------------------------------------
#if defined(CLIENT_DLL)
extern void RecvProxy_EffectFlags(const CRecvProxyData *pData, void *pStruct,
                                  void *pOut);
void RecvProxy_SequenceNum(const CRecvProxyData *pData, void *pStruct,
                           void *pOut);
#endif
...
//-----------------------------------------------------------------------------
#if defined(CLIENT_DLL)
static void RecvProxy_Weapon(const CRecvProxyData *pData, void *pStruct,
                             void *pOut) {
...
#if !defined(CLIENT_DLL)
SendPropModelIndex(SENDINFO(m_nModelIndex)), SendPropInt(SENDINFO(m_nBody), 8),
    ...
    SendPropEHandle(SENDINFO(m_hWeapon)), SendPropEHandle(SENDINFO(m_hOwner)),
    ...
    SendPropArray(SendPropFloat(SENDINFO_ARRAY(m_flPoseParameter), 8, 0, 0.0f,
                                1.0f),
                  m_flPoseParameter),
#endif
#else
RecvPropInt(RECVINFO(m_nModelIndex)), RecvPropInt(RECVINFO(m_nSkin)),
    ...
    DEFINE_PRED_FIELD_TOL(m_flPlaybackRate, FIELD_FLOAT, FTYPEDESC_INSENDTABLE,
                          0.125f),
    DEFINE_PRED_FIELD(m_fEffects, FIELD_INTEGER,
                      FTYPEDESC_INSENDTABLE | FTYPEDESC_OVERRIDE),
...
// Purpose:
//-----------------------------------------------------------------------------
bool CBaseViewModel::GetAttachmentVelocity(int number, Vector &originVel,
                                           Quaternion &angleVel) {
  if (m_hWeapon.Get() && m_hWeapon.Get()->WantsToOverrideViewmodelAttachments())
