...
extern short g_sModelIndexFireball; // (in combatweapon.cpp) holds the index for
                                    // the fireball
extern short g_sModelIndexWExplosion; // (in combatweapon.cpp) holds the index
                                      // for the underwater explosion
...
DEFINE_FIELD(m_hThrower, FIELD_EHANDLE),
    ...
    // Function Pointers
    DEFINE_THINKFUNC(Smoke), DEFINE_ENTITYFUNC(BounceTouch),
    DEFINE_ENTITYFUNC(SlideTouch), DEFINE_ENTITYFUNC(ExplodeTouch),
    DEFINE_USEFUNC(DetonateUse), DEFINE_THINKFUNC(DangerSoundThink),
    DEFINE_THINKFUNC(PreDetonate), DEFINE_THINKFUNC(Detonate),
    DEFINE_THINKFUNC(TumbleThink),

    END_DATADESC()

        void SendProxy_CropFlagsToPlayerFlagBitsLength(
...
#else
RecvPropFloat(RECVINFO(m_flDamage)), RecvPropFloat(RECVINFO(m_DmgRadius)),
    RecvPropInt(RECVINFO(m_bIsLive)),
    //	RecvPropTime( RECVINFO( m_flDetonateTime ) ),
    RecvPropEHandle(RECVINFO(m_hThrower)),

    // Need velocity from grenades to make animation system work correctly when
    // running
...
DEFINE_PRED_FIELD(m_hThrower, FIELD_EHANDLE, FTYPEDESC_INSENDTABLE),
    ...
    DEFINE_PRED_FIELD_TOL(m_vecVelocity, FIELD_VECTOR, FTYPEDESC_INSENDTABLE,
                          0.5f),
    DEFINE_PRED_FIELD_TOL(m_flNextAttack, FIELD_FLOAT, FTYPEDESC_INSENDTABLE,
                          TD_MSECTOLERANCE),
...
// Grenades flagged with this will be triggered when the owner calls
...
#define SF_DETONATE 0x0001
  ...
  // creates. This solves a problem with remote detonating the pipebombs (client
  // wasn't seeing the explosion effect)
  ...
  if (pTrace->fraction != 1.0) {
    ...
    surfacedata_t *pdata =
        physprops->GetSurfaceData(pTrace->surface.surfaceProps);
    CPASFilter filter(vecAbsOrigin);

    te->Explosion(filter, -1.0, // don't apply cl_interp delay
                  &vecAbsOrigin,
                  !(contents & MASK_WATER) ? g_sModelIndexFireball
                                           : g_sModelIndexWExplosion,
                  m_DmgRadius * .03, 25, TE_EXPLFLAG_NONE, m_DmgRadius,
                  m_flDamage, &vecNormal, (char)pdata->game.material);
  } else {
    CPASFilter filter(vecAbsOrigin);
    te->Explosion(filter, -1.0, // don't apply cl_interp delay
                  &vecAbsOrigin,
  ...
  CSoundEnt::InsertSound(SOUND_COMBAT, GetAbsOrigin(),
                         BASEGRENADE_EXPLOSION_VOLUME, 3.0);
#endif

  // Use the thrower's position as the reported position
  Vector vecReported = m_hThrower ? m_hThrower->GetAbsOrigin() : vec3_origin;

  CTakeDamageInfo info(this, m_hThrower, GetBlastForce(), GetAbsOrigin(),
                       m_flDamage, bitsDamageType, 0, &vecReported);
...
#if HL2_EPISODIC
  // Because the grenade is zipped out of the world instantly, the EXPLOSION
  // sound that it makes for the AI is also immediately destroyed. For this
  // reason, we now make the grenade entity inert and throw it away in 1/10th of
  // a second instead of right away. Removing the grenade instantly causes
  // intermittent bugs with env_microphones who are listening for explosions.
  // They will 'randomly' not hear explosion sounds when the grenade is removed
  // and the SoundEnt thinks (and removes the sound) before the env_microphone
  // thinks and hears the sound.
...
void CBaseGrenade::Smoke(void) {
  ...
  if (UTIL_PointContents(vecAbsOrigin) & MASK_WATER) {
    UTIL_Bubbles(vecAbsOrigin - Vector(64, 64, 64),
                 vecAbsOrigin + Vector(64, 64, 64), 100);
  } else {
    CPVSFilter filter(vecAbsOrigin);

    te->Smoke(filter, 0.0, &vecAbsOrigin, g_sModelIndexSmoke,
              m_DmgRadius * 0.03, 24);
  }
#if !defined(CLIENT_DLL)
  SetThink(&CBaseGrenade::SUB_Remove);
...
void CBaseGrenade::Event_Killed(const CTakeDamageInfo &info) { Detonate(); }

#if !defined(CLIENT_DLL)
//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CBaseGrenade::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
...
//-----------------------------------------------------------------------------
void CBaseGrenade::DetonateUse(CBaseEntity *pActivator, CBaseEntity *pCaller,
                               USE_TYPE useType, float value) {
...
void CBaseGrenade::Detonate(void) {
  ...
  UTIL_TraceLine(vecSpot, vecSpot + Vector(0, 0, -32), MASK_SHOT_HULL, this,
                 COLLISION_GROUP_NONE, &tr);

  if (tr.startsolid) {
    // Since we blindly moved the explosion origin vertically, we may have
    // inadvertently moved the explosion into a solid, in which case nothing is
    // going to be harmed by the grenade's explosion because all subsequent
    // traces will startsolid. If this is the case, we do the downward trace
    // again from the actual origin of the grenade. (sjb) 3/8/2007  (for
    // ep2_outland_09)
    UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + Vector(0, 0, -32),
...
void CBaseGrenade::ExplodeTouch(CBaseEntity *pOther) {
  ...
  UTIL_TraceLine(vecSpot, vecSpot + velDir * 64, MASK_SOLID_BRUSHONLY, this,
                 COLLISION_GROUP_NONE, &tr);
...
void CBaseGrenade::DangerSoundThink(void) {
  ...
  CSoundEnt::InsertSound(SOUND_DANGER, GetAbsOrigin() + GetAbsVelocity() * 0.5,
                         GetAbsVelocity().Length(), 0.2, this);
#endif
...
void CBaseGrenade::BounceTouch(CBaseEntity *pOther) {
  ...
  // this is my heuristic for modulating the grenade velocity because grenades
  // dropped purely vertical or thrown very far tend to slow down too quickly
  // for me to always catch just by testing velocity. trimming the Z velocity a
  // bit seems to help quite a bit.
  vecTestVelocity = GetAbsVelocity();
  vecTestVelocity.z *= 0.45;

  if (!m_bHasWarnedAI && vecTestVelocity.Length() <= 60) {
    // grenade is moving really slow. It's probably very close to where it will
    // ultimately stop moving. emit the danger sound.

    // register a radius louder than the explosion, so we make sure everyone
    // gets out of the way
#if !defined(CLIENT_DLL)
  ...
  if (GetFlags() & FL_ONGROUND) {
    ...
    // SetSequence( random->RandomInt( 1, 1 ) ); // FIXME: missing tumble
    // animations
  } else {
    // play bounce sound
    BounceSound();
