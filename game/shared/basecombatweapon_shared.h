...
public:
  ...
  virtual void
  ItemBusyFrame(void); // called each frame by the player PostThink, if the
                       // player's not ready to attack yet
  ...
  virtual char *
  GetDeathNoticeName(void); // Get the string to print death notices with
...
// Server Only Methods
  ...
  virtual void
  FallThink(void); // make the weapon fall to the ground after spawning
  ...
  virtual void
  CheckRespawn(void); // see if this weapon should respawn after being picked up
...
// Client only methods
  ...
  virtual void
  GetViewmodelBoneControllers(C_BaseViewModel *pViewModel,
...
protected:
  ...
  COutputEvent m_OnCacheInteraction; // For awarding lambda cache achievements
                                     // in HL2 on 360. See .FGD file for details
