...
ConVar steamworks_sessionid_client(
    "steamworks_sessionid_client", "0", FCVAR_HIDDEN,
    "The client session ID for the new steamworks gamestats.");
#endif

// This is used to replicate our server id to the client so that client data can
// be associated with the server's.
ConVar steamworks_sessionid_server(
    "steamworks_sessionid_server", "0", FCVAR_REPLICATED | FCVAR_HIDDEN,
...
// This is a stop gap to disable the steam works game stats from ever
// initializing in the event that we need it
...
// This is used to control when the stats get uploaded. If we wait until the end
// of the session, we miss out on all the stats if the server crashes. If we
// upload as we go, then we will have the data
#define steamworks_immediate_upload 1

// If set to zero all cvars will be tracked.
#define steamworks_track_cvar 0

// If set to non zero only cvars that are don't have default values will be
// tracked, but only if steamworks_track_cvar is also not zero.
#define steamworks_track_cvar_diff_only 1

// Methods that clients connect to servers.  note that positive numbers are
// reserved for quickplay sessions
enum {
...
static ConCommand
    ShowSteamStatsSessionID("ShowSteamStatsSessionID",
                            Show_Steam_Stats_Session_ID,
                            "Prints out the game stats session ID's (developer "
                            "convar must be set to non-zero).",
                            FCVAR_DEVELOPMENTONLY);
#endif

#ifdef CLIENT_DLL
//-----------------------------------------------------------------------------
// Purpose: Clients store the server's session IDs so we can associate client
// rows with server rows.
//-----------------------------------------------------------------------------
void ServerSessionIDChangeCallback(IConVar *pConVar, const char *pOldString,
                                   float flOldValue) {
  ConVarRef var(pConVar);
  if (var.IsValid()) {
    // Treat the variable as a string, since the sessionID is 64 bit and the
    // convar int interface is only 32 bit.
...
//-----------------------------------------------------------------------------
// Purpose: Constructor. Sets up the steam callbacks accordingly depending on
// client/server dll
//-----------------------------------------------------------------------------
CSteamWorksGameStatsUploader::CSteamWorksGameStatsUploader()
    : CAutoGameSystemPerFrame("CSteamWorksGameStatsUploader")
      ...
      m_CallbackSteamSessionInfoIssued(
          this, &CSteamWorksGameStatsUploader::Steam_OnSteamSessionInfoIssued),
      m_CallbackSteamSessionInfoClosed(
          this, &CSteamWorksGameStatsUploader::Steam_OnSteamSessionInfoClosed)
#endif
{

#if !defined(NO_STEAM) && defined(CLIENT_DLL)
  m_CallbackSteamSessionInfoIssued.Register(
      this, &CSteamWorksGameStatsUploader::Steam_OnSteamSessionInfoIssued);
  m_CallbackSteamSessionInfoClosed.Register(
      this, &CSteamWorksGameStatsUploader::Steam_OnSteamSessionInfoClosed);
#endif
...
//-----------------------------------------------------------------------------
// Purpose: Checks to see if Cvar diff tracking is enabled and if so uploads the
// diffs
//-----------------------------------------------------------------------------
...
bool CSteamWorksGameStatsUploader::Init() {
  ...
  steamworks_sessionid_server.InstallChangeCallback(
      ServerSessionIDChangeCallback);
#endif
...
#if 0 // the next three disabled if()'s are in the latest l4d2 branch, I'm not
      // sure if they should be brought over
		if ( !m_pzServerIP[0] )
#endif
    {
      const char *pzAddress = event->GetString("address");
      if (pzAddress) {
        V_snprintf(m_pzServerIP, ARRAYSIZE(m_pzServerIP), "%s:%d", pzAddress,
                   event->GetInt("port"));
        ServerAddressToInt();
      } else {
  ...
  // Started attempting connection to gameserver
    ...
    if (pszSource[0] != '\0') {
      if (FStrEq("listenserver", pszSource))
        ...
        Warning(
            "Unrecognized client_beginconnect event 'source' argument: '%s'\n",
            pszSource);
    }
...
//-----------------------------------------------------------------------------
// Purpose:	Sets the server session ID but ONLY if it's not 0. We are using
// this to avoid a race 			condition where a server sends their session stats
// before a client does, thereby,
...
void CSteamWorksGameStatsUploader::ClientDisconnect() {
  ...
  uint64 ulRowID = 0;
  m_SteamWorksInterface->AddNewRow(&ulRowID, m_SessionID,
                                   "ClientSessionLookup");
  WriteInt64ToTable(m_SessionID, ulRowID, "SessionID");
  WriteInt64ToTable(m_ActiveSession.m_ServerSessionID, ulRowID,
                    "ServerSessionID");
...
EResult CSteamWorksGameStatsUploader::RequestSessionID() {
  ...
  if (steamworks_stats_disable) {
    DevMsg("Steamworks Stats: No stats collection because "
           "steamworks_stats_disable is set to 1.\n");
    return k_EResultAccessDenied;
  }

  if (developer.GetInt() == 1) {
    //		DevMsg( "Steamworks Stats: No stats collection because developer
    //is set to 1.\n" ); 		return k_EResultAccessDenied;
  }
  ...
  // Do not continue if we are waiting a server response for this session
  // request.
  if (m_SessionIDRequestPending) {
    return k_EResultPending;
  }

  // If a request is unsent, it will be sent as soon as the steam API is
  // available.
  ...
  if (m_SteamWorksInterface) {
    ...
    m_SteamWorksInterface->GetNewSession(accountType, m_UserID, m_iAppID,
                                         GetTimeSinceEpoch());
  }
...
//-----------------------------------------------------------------------------
void CSteamWorksGameStatsUploader::Steam_OnSteamSessionInfoIssued(
...
//-----------------------------------------------------------------------------
void CSteamWorksGameStatsUploader::Steam_OnSteamSessionInfoClosed(
...
//-----------------------------------------------------------------------------
// Purpose: Per frame think. Used to periodically check if we have queued
// operations. For example: we may request a session id before steam is ready.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
void CSteamWorksGameStatsUploader::StartSession() { RequestSessionID(); }
...
void CSteamWorksGameStatsUploader::WriteSessionRow() {
  ...
  // Don't send SessionID. It's provided by steam. Same with the account's id
  // and type.
  //	m_SteamWorksInterface->AddSessionAttributeInt64( m_SessionID,
  //"SessionID", m_SessionID );

#ifdef CLIENT_DLL
//	m_SteamWorksInterface->AddSessionAttributeInt64( m_SessionID,
//"ServerSessionID", m_ServerSessionID );
#endif

  m_SteamWorksInterface->AddSessionAttributeInt(m_SessionID, "AppID", m_iAppID);
  m_SteamWorksInterface->AddSessionAttributeInt(m_SessionID, "StartTime",
                                                m_StartTime);
  m_SteamWorksInterface->AddSessionAttributeInt(m_SessionID, "EndTime",
                                                m_EndTime);

#ifndef CLIENT_DLL
  m_SteamWorksInterface->AddSessionAttributeString(m_SessionID, "ServerIP",
                                                   m_pzServerIP);
  m_SteamWorksInterface->AddSessionAttributeString(m_SessionID, "ServerName",
                                                   m_pzHostName);
  m_SteamWorksInterface->AddSessionAttributeString(m_SessionID, "StartMap",
                                                   m_pzMapStart);
#endif

#ifdef CLIENT_DLL
  // TODO CAB: Need to get these added into the clientsessionlookup table
  m_SteamWorksInterface->AddSessionAttributeInt(m_SessionID, "PlayersInGame",
                                                m_HumanCntInGame);
  m_SteamWorksInterface->AddSessionAttributeInt(m_SessionID, "FriendsInGame",
                                                m_FriendCntInGame);
#endif
}
...
//-----------------------------------------------------------------------------
// Purpose: Verifies that we have a valid interface and will attempt to obtain a
// new one if we don't.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
EResult CSteamWorksGameStatsUploader::WriteIntToTable(const int value,
                                                      uint64 iTableID,
                                                      const char *pzRow) {
  if (!VerifyInterface())
    return k_EResultNoConnection;
...
//-----------------------------------------------------------------------------
EResult CSteamWorksGameStatsUploader::WriteInt64ToTable(const uint64 value,
                                                        uint64 iTableID,
                                                        const char *pzRow) {
  if (!VerifyInterface())
    return k_EResultNoConnection;
...
//-----------------------------------------------------------------------------
EResult CSteamWorksGameStatsUploader::WriteFloatToTable(const float value,
                                                        uint64 iTableID,
                                                        const char *pzRow) {
  if (!VerifyInterface())
    return k_EResultNoConnection;
...
//-----------------------------------------------------------------------------
EResult CSteamWorksGameStatsUploader::WriteStringToTable(const char *value,
                                                         uint64 iTableID,
                                                         const char *pzRow) {
  if (!VerifyInterface())
    return k_EResultNoConnection;
...
//-----------------------------------------------------------------------------
// Purpose: Wrapper function to search a KeyValues for a value with the given
// keyName and add the result to the row. If the key isn't present, return
// ResultNoMatch to indicate such.
//-----------------------------------------------------------------------------
EResult CSteamWorksGameStatsUploader::WriteOptionalFloatToTable(
    KeyValues *pKV, const char *keyName, uint64 iTableID, const char *pzRow) {
  if (!VerifyInterface())
    return k_EResultNoConnection;
...
// ResultNoMatch to indicate such.
//-----------------------------------------------------------------------------
EResult CSteamWorksGameStatsUploader::WriteOptionalIntToTable(
    KeyValues *pKV, const char *keyName, uint64 iTableID, const char *pzRow) {
  if (!VerifyInterface())
    return k_EResultNoConnection;
...
#ifdef GAME_DLL
  return (steamgameserverapicontext &&
...
//-----------------------------------------------------------------------------
// Purpose: There's no guarantee that your interface pointer will persist across
// level transitions,
...
ISteamGameStats *CSteamWorksGameStatsUploader::GetInterface(void) {
  ...
  if (steamgameserverapicontext &&
      steamgameserverapicontext->SteamGameServer() &&
      steamgameserverapicontext->SteamGameServerUtils()) {
    m_UserID = steamgameserverapicontext->SteamGameServer()
                   ->GetSteamID()
                   .ConvertToUint64();
  ...
  if (g_pSteamClientGameServer && engine &&
      (/*engine->IsDedicatedServerForXbox() ||*/ engine->IsDedicatedServer())) {
    return (ISteamGameStats *)
        g_pSteamClientGameServer->GetISteamGenericInterface(
            hSteamUser, hSteamPipe, STEAMGAMESTATS_INTERFACE_VERSION);
  }
#elif CLIENT_DLL
  ...
  if (SteamClient()) {
    return (ISteamGameStats *)SteamClient()->GetISteamGenericInterface(
        hSteamUser, hSteamPipe, STEAMGAMESTATS_INTERFACE_VERSION);
  }
...
//-----------------------------------------------------------------------------
// Purpose: Creates a table from the KeyValue file. Do NOT send nested KeyValue
// objects into this function!
//-----------------------------------------------------------------------------
EResult CSteamWorksGameStatsUploader::AddStatsForUpload(KeyValues *pKV,
...
//-----------------------------------------------------------------------------
// Purpose: Parses all the keyvalue files we've been sent and creates tables
// from them and uploads them
//-----------------------------------------------------------------------------
double g_rowCommitTime = 0.0f;
double g_rowWriteTime = 0.0f;
EResult CSteamWorksGameStatsUploader::ParseKeyValuesAndSendStats(
    KeyValues *pKV, bool bIncludeClientsServerSessionID) {
  if (!pKV) {
    return k_EResultFail;
  }
  ...
  if (!m_SteamWorksInterface) {
    DevMsg("WARNING: Attempted to send a steamworks gamestats row when the "
           "steamworks interface was not available!");
    return k_EResultNoConnection;
  }
  ...
  */
  ...
  for (KeyValues *pData = pKV->GetFirstSubKey(); pData != NULL;
...
void CSteamWorksGameStatsUploader::AddClientPerfData(KeyValues *pKV) {
  ...
  WriteInt64ToTable(m_SessionID, ulRowID, "SessionID");
  //	WriteInt64ToTable(	m_ServerSessionID,
  //ulRowID,	"ServerSessionID" );
  ...
  WriteFloatToTable(pKV->GetFloat("Map/perfdata/StdDevFPS"), ulRowID,
                    "StdDevFPS");
  ...
  //	WriteIntToTable(	pKV->GetBool( "Map/WindowedNoBorder" ), ulRowID,
  //"WindowedNoBorder");
  ...
  WriteFloatToTable(pKV->GetFloat("Map/perfdata/AvgServerPing"), ulRowID,
                    "AvgServerPing");
  ...
  WriteIntToTable(pKV->GetBool("Map/SteamControllerActive"), ulRowID,
                  "UsingController");

  // Loading time information

  // If a player exits a map and then exits the game, LoadTimeMap will not be in
  // the key list the second time (when we are sending for app shutdown), so
  // only write it if it's here.
  WriteOptionalFloatToTable(pKV, "Map/LoadTimeMap", ulRowID, "SessionLoadTime");

  // Main menu load time is only added once, even if we play many games in a
  // row. This prevents a stats bias for people who replay over and over again.
  WriteOptionalFloatToTable(pKV, "Map/LoadTimeMainMenu", ulRowID,
                            "MainmenuLoadTime");
...
int CSteamWorksGameStatsUploader::GetHumanCountInGame() {
  int iHumansInGame = 0;
  // TODO: Need to add server/client code to count the number of connected
  // humans.
...
int CSteamWorksGameStatsUploader::GetFriendCountInGame() {
  ...
  if (AccessToSteamAPI()) {
    ...
    // Let's get our game info so we can use that to test if our friends are
    // connected to the same game as us
    FriendGameInfo_t myGameInfo;
    steamapicontext->SteamFriends()->GetFriendGamePlayed(m_SteamID,
                                                         &myGameInfo);
    CSteamID myLobby = steamapicontext->SteamMatchmaking()->GetLobbyOwner(
        myGameInfo.m_steamIDLobby);

    // This returns the number of friends that are playing a game
    int activeFriendCnt =
        steamapicontext->SteamFriends()->GetFriendCount(k_EFriendFlagImmediate);
    ...
    for (int h = 0; h < activeFriendCnt; ++h) {
      ...
      CSteamID friendID = steamapicontext->SteamFriends()->GetFriendByIndex(
          h, k_EFriendFlagImmediate);

      if (steamapicontext->SteamFriends()->GetFriendGamePlayed(friendID,
                                                               &friendInfo)) {
        // Does our friend have a valid lobby ID?
        if (friendInfo.m_gameID.IsValid()) {
          // Get our friend's lobby info
          CSteamID friendLobby =
              steamapicontext->SteamMatchmaking()->GetLobbyOwner(
                  friendInfo.m_steamIDLobby);

          // Double check the validity of the friend lobby ID then check to see
          // if they are in our game
