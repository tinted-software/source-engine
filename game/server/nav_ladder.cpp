 ...
 */
void CNavLadder::OnDestroyNotify(CNavArea *dead) { Disconnect(dead); }
 ...
 */
bool CNavLadder::IsConnected(const CNavArea *area,
                             LadderDirectionType dir) const {
  if (dir == LADDER_DOWN) {
    return area == m_bottomArea;
  } else if (dir == LADDER_UP) {
    return (area == m_topForwardArea || area == m_topLeftArea ||
            area == m_topRightArea || area == m_topBehindArea);
  } else {
    return (area == m_bottomArea || area == m_topForwardArea ||
...
void CNavLadder::SetDir(NavDirType dir) {
  ...
  m_normal.Init();
  AddDirectionVector(&m_normal, m_dir,
                     1.0f); // worst-case, we have the NavDirType as a normal
  ...
  trace_t result;
#ifdef TERROR
  // TERROR: use the MASK_ZOMBIESOLID_BRUSHONLY contents, since that's what
  // zombies use
  UTIL_TraceLine(from, to, MASK_ZOMBIESOLID_BRUSHONLY, NULL,
...
void CNavLadder::DrawLadder(void) const {
  ...
  if (4 == sscanf(nav_area_bgcolor.GetString(), "%d %d %d %d", &(bgcolor[0]),
    ...
    if (bgcolor[3] > 0) {
      ...
      NDebugOverlay::Triangle(topLeft + offset, topRight + offset,
                              bottomRight + offset, bgcolor[0], bgcolor[1],
                              bgcolor[2], bgcolor[3], true, 0.15f);
      NDebugOverlay::Triangle(bottomRight + offset, bottomLeft + offset,
                              topLeft + offset, bgcolor[0], bgcolor[1],
                              bgcolor[2], bgcolor[3], true, 0.15f);
    }
  }
  ...
  if (!TheNavMesh->IsEditMode(CNavMesh::PLACE_PAINTING)) {
    ...
    if (m_bottomArea) {
      ...
      // don't draw the bottom connection too high if the ladder is high above
      // the area
      if (bottom.z - areaBottom.z > GenerationStepSize * 1.5f)
        offset = 0.0f;

      NavDrawLine(bottom + Vector(0, 0, offset), areaBottom,
    ...
    if (m_topForwardArea)
      NavDrawLine(top, m_topForwardArea->GetCenter(),
                  ((m_topForwardArea->IsConnected(this, LADDER_DOWN))
                       ? NavConnectedTwoWaysColor
                       : NavConnectedOneWayColor));

    if (m_topLeftArea)
      NavDrawLine(top, m_topLeftArea->GetCenter(),
                  ((m_topLeftArea->IsConnected(this, LADDER_DOWN))
                       ? NavConnectedTwoWaysColor
                       : NavConnectedOneWayColor));

    if (m_topRightArea)
      NavDrawLine(top, m_topRightArea->GetCenter(),
                  ((m_topRightArea->IsConnected(this, LADDER_DOWN))
                       ? NavConnectedTwoWaysColor
                       : NavConnectedOneWayColor));

    if (m_topBehindArea)
      NavDrawLine(top, m_topBehindArea->GetCenter(),
                  ((m_topBehindArea->IsConnected(this, LADDER_DOWN))
                       ? NavConnectedTwoWaysColor
                       : NavConnectedOneWayColor));
  }
}
 ...
 */
void CNavLadder::OnRoundRestart(void) { FindLadderEntity(); }

//--------------------------------------------------------------------------------------------------------------
void CNavLadder::FindLadderEntity(void) {
  m_ladderEntity = gEntList.FindEntityByClassnameNearest(
      "func_simpleladder", (m_top + m_bottom) * 0.5f, HalfHumanWidth);
}
...
void CNavLadder::Load(CUtlBuffer &fileBuffer, unsigned int version) {
  ...
  if (!m_bottomArea) {
    ...
    DevWarning("nav_unmark; nav_mark ladder %d; nav_warp_to_mark\n", m_id);
             ...
             !m_topRightArea) // can't include behind area, since it is not used
