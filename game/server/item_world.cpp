...
//
// Purpose: Handling for the base world item. Most of this was moved from
// items.cpp.
//
// $NoKeywords: $
//===========================================================================//
...
void CWorldItem::Spawn(void) {
  ...
  case 44: // ITEM_BATTERY:
    pEntity =
        CBaseEntity::Create("item_battery", GetLocalOrigin(), GetLocalAngles());
    break;
  case 45: // ITEM_SUIT:
    pEntity =
        CBaseEntity::Create("item_suit", GetLocalOrigin(), GetLocalAngles());
    break;
  }
...
DEFINE_FIELD(m_bActivateWhenAtRest, FIELD_BOOLEAN),
    ...
    // Function Pointers
    DEFINE_ENTITYFUNC(ItemTouch), DEFINE_THINKFUNC(Materialize),
...
void CItem::Use(CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType,
                float value) {
...
//grounded, 			we change its solid type to trigger and set it in a large box that
...
void CItem::FallThink(void) {
  ...
  if (!(GetFlags() & FL_ONGROUND)) {
    if (!GetAbsVelocity().Length() && GetMoveType() == MOVETYPE_FLYGRAVITY) {
      // Mr. Game, meet Mr. Hammer.  Mr. Hammer, meet the uncooperative Mr.
      // Physics. Mr. Physics really doesn't want to give our friend the
      // FL_ONGROUND flag. This means our wonderfully helpful radius currency
      // collection code will be sad. So in the name of justice, we ask that
      // this flag be delivered unto him.
...
bool UTIL_ItemCanBeTouchedByPlayer(CBaseEntity *pItem, CBasePlayer *pPlayer) {
  ...
  // For now, always allow a vehicle riding player to pick up things they're
  // driving over
  ...
  // FIXME: This is the simple first try solution towards the problem.  We need
  // to take edges and shape more into account
  //		  for this to be fully robust.

  // Trace between to see if we're occluded
  trace_t tr;
  CTraceFilterSkipTwoEntities filter(pPlayer, pItem,
                                     COLLISION_GROUP_PLAYER_MOVEMENT);
  UTIL_TraceLine(vecStartPos, vecEndPos, MASK_SOLID, &filter, &tr);

  // Occluded
  // FIXME: For now, we exclude starting in solid because there are cases where
  // this doesn't matter
  if (tr.fraction < 1.0f)
    return false;
...
//-----------------------------------------------------------------------------
// Purpose: Whether or not the item can be touched and picked up by the player,
// taking
...
void CItem::ItemTouch(CBaseEntity *pOther) {
  ...
  // Must be a valid pickup scenario (no blocking). Though this is a more
  // expensive check than some that follow, this has to be first Obecause it's
  // the only one that inhibits firing the output OnCacheInteraction.
...
CBaseEntity *CItem::Respawn(void) {
  ...
  UTIL_SetOrigin(this, g_pGameRules->VecItemRespawnSpot(
                           this)); // blip to whereever you should respawn.
