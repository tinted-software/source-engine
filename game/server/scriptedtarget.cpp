...
// Purpose:
//------------------------------------------------------------------------------
void CScriptedTarget::InputDisable(inputdata_t &inputdata) { TurnOff(); }
...
CScriptedTarget *CScriptedTarget::NextScriptedTarget(void) {
  ...
  if (gpGlobals->curtime >= m_flPauseDoneTime) {
    ...
    // ------------------------------------------------
      ...
      // ----------------------------------------------------
      trace_t tr;
      UTIL_TraceLine(GetAbsOrigin(), pNextTarget->GetAbsOrigin(), MASK_SHOT,
...
CBaseEntity *CScriptedTarget::FindEntity(void) {
  ...
  for (CEntitySphereQuery sphere(GetAbsOrigin(), m_flRadius);
       (pTestEnt = sphere.GetCurrentEntity()) != NULL; sphere.NextEntity()) {
...
int CScriptedTarget::DrawDebugTextOverlays(void) {
  ...
  if (m_debugOverlays & OVERLAY_TEXT_BIT) {
    ...
    // --------------
    if (GetTarget() != NULL) {
      Q_snprintf(tempstr, sizeof(tempstr), "User: %s",
...
void CScriptedTarget::DrawDebugGeometryOverlays(void) {
  ...
  if (m_debugOverlays & (OVERLAY_BBOX_BIT | OVERLAY_ABSBOX_BIT)) {
    if (m_iDisabled) {
      NDebugOverlay::Box(GetAbsOrigin(), Vector(-5, -5, -5), Vector(5, 5, 5),
                         200, 100, 100, 0, 0);
    } else {
      NDebugOverlay::Cross3D(m_vLastPosition, Vector(-8, -8, -8),
                             Vector(8, 8, 8), 255, 0, 0, true, 0.1);
      NDebugOverlay::Box(GetAbsOrigin(), Vector(-5, -5, -5), Vector(5, 5, 5),
                         255, 0, 0, 0, 0);
      NDebugOverlay::Line(GetAbsOrigin(), m_vLastPosition, 255, 0, 0, true,
                          0.0);
    }

    CBaseEntity *pTarget = GetNextTarget();
    if (pTarget) {
      NDebugOverlay::Line(GetAbsOrigin(), pTarget->GetAbsOrigin(), 200, 100,
                          100, true, 0.0);
    }
    if (GetTarget() != NULL) {
      NDebugOverlay::Line(GetAbsOrigin(), GetTarget()->EyePosition(), 0, 255, 0,
                          true, 0.0);
    }
  }
