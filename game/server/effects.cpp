...
#define SF_FUNNEL_REVERSE                                                      \
  1 // funnel effect repels particles instead of attracting them.

#define SF_GIBSHOOTER_REPEATABLE (1 << 0) // allows a gibshooter to be refired
#define SF_SHOOTER_FLAMING (1 << 1)       // gib is on fire
#define SF_SHOOTER_STRICT_REMOVE                                               \
  (1 << 2) // remove this gib even if it is in the player's view
...
// Purpose:
//-----------------------------------------------------------------------------
void CBubbling::InputActivate(inputdata_t &inputdata) { TurnOn(); }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CBubbling::InputDeactivate(inputdata_t &inputdata) { TurnOff(); }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CBubbling::InputToggle(inputdata_t &inputdata) { Toggle(); }
...
void CGibShooter::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
...
void CGibShooter::Spawn(void) {
  ...
  m_nMaxGibModelFrame =
      modelinfo->GetModelFrameCount(modelinfo->GetModel(m_iGibModelIndex));
}
...
CGib *CGibShooter::CreateGib(void) {
  ...
  pGib->m_nBody = random->RandomInt(
      1, m_nMaxGibModelFrame -
             1); // avoid throwing random amounts of the 0th gib. (skull).
...
void CGibShooter::InitPointGib(CGib *pGib, const Vector &vecShootDir,
  ...
  if (pGib) {
    ...
    QAngle angVel(random->RandomFloat(100, 200), random->RandomFloat(100, 300),
                  0);
    ...
    pGib->m_lifeTime =
        (m_flGibLife * random->RandomFloat(0.95, 1.05)); // +/- 5%
    ...
    if (m_bIsSprite == true) {
      pGib->SetSprite(CSprite::SpriteCreate(STRING(GetModelName()),
                                            pGib->GetAbsOrigin(), false));
      ...
      if (pSprite) {
        ...
        pSprite->SetScale(1);
        pSprite->SetTransparency(m_nRenderMode, m_clrRender->r, m_clrRender->g,
                                 m_clrRender->b, m_clrRender->a, m_nRenderFX);
        pSprite->AnimateForTime(5, m_flGibLife +
                                       1); // This framerate is totally wrong
      }
    }
  }
}
...
CBaseEntity *CGibShooter::SpawnGib(const Vector &vecShootDir, float flSpeed) {
  ...
  case GIB_SIMULATE_RAGDOLL: {
    ...
    return CreateRagGib(STRING(GetModelName()), GetAbsOrigin(), GetAbsAngles(),
                        force, m_flGibLife);
  }
  ...
  case GIB_SIMULATE_PHYSICS: {
    ...
    if (pGib) {
      ...
      IPhysicsObject *pPhysicsObject = pGib->VPhysicsInitNormal(
          SOLID_VPHYSICS, pGib->GetSolidFlags(), false);
...
void CGibShooter::ShootThink(void) {
  ...
  vecShootDir =
      vecShootDir + vRight * random->RandomFloat(-1, 1) * m_flVariance;
  vecShootDir =
      vecShootDir + vForward * random->RandomFloat(-1, 1) * m_flVariance;
...
#if HL2_EPISODIC
  float
      m_flMassOverride; // allow designer to force a mass for gibs in some cases
#endif
};
...
public:
  ...
  virtual CBaseEntity *DoWashPush(float flTimeSincePushStarted,
...
DEFINE_KEYFIELD(m_flTimeUnderRotor, FIELD_FLOAT, "rotortime"),
    DEFINE_KEYFIELD(m_flTimeUnderRotorVariance, FIELD_FLOAT,
                    "rotortimevariance"),
...
// Inherited from IRotorWashShooter
//-----------------------------------------------------------------------------
CBaseEntity *CRotorWashShooter::DoWashPush(float flWashStartTime,
                                           const Vector &vecForce) {
  ...
  m_flNextGibTime = gpGlobals->curtime + m_flTimeUnderRotor +
                    random->RandomFloat(-1, 1) * m_flTimeUnderRotorVariance;
  if (m_flNextGibTime <= gpGlobals->curtime) {
...
void CTestEffect::Precache(void) { PrecacheModel("sprites/lgtning.vmt"); }
...
void CTestEffect::Think(void) {
  ...
  if (m_iBeam < 24) {
    ...
    UTIL_TraceLine(vecSrc, vecSrc + vecDir * 128, MASK_SOLID_BRUSHONLY, this,
  ...
  if (t < 3.0) {
    for (i = 0; i < m_iBeam; i++) {
      t = (gpGlobals->curtime - m_flBeamTime[i]) /
          (3 + m_flStartTime - m_flBeamTime[i]);
...
void CTestEffect::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
                      USE_TYPE useType, float value) {
  SetNextThink(gpGlobals->curtime + 0.1f);
  m_flStartTime = gpGlobals->curtime;
}
...
#define SF_BLOOD_GORE 0x0040
  ...
  QAngle angSprayDir =
      QAngle(m_vecSprayDir.x, m_vecSprayDir.y, m_vecSprayDir.z);
...
Vector CBlood::BloodPosition(CBaseEntity *pActivator) {
  if (HasSpawnFlags(SF_BLOOD_PLAYER)) {
    ...
    if (player) {
      return (player->EyePosition()) + Vector(random->RandomFloat(-10, 10),
...
void CBlood::InputEmitBlood(inputdata_t &inputdata) {
  if (HasSpawnFlags(SF_BLOOD_STREAM)) {
    UTIL_BloodStream(BloodPosition(inputdata.pActivator), Direction(), Color(),
                     BloodAmount());
  } else {
    UTIL_BloodDrips(BloodPosition(inputdata.pActivator), Direction(), Color(),
                    BloodAmount());
  }
  ...
  if (HasSpawnFlags(SF_BLOOD_DECAL)) {
    ...
    UTIL_TraceLine(start, start + forward * BloodAmount() * 2,
  ...
  if (HasSpawnFlags(SF_BLOOD_CLOUD | SF_BLOOD_DROPS | SF_BLOOD_GORE)) {
    ...
    UTIL_BloodSpray(GetAbsOrigin(), Direction(), Color(), BloodAmount(),
                    nFlags);
  }
}
...
void CC_BloodSpray(const CCommand &args) {
  ...
  while ((pEnt = gEntList.FindEntityGeneric(pEnt, args[1])) != NULL) {
    ...
    UTIL_BloodSpray((forward * 4) +
                        (pEnt->EyePosition() + pEnt->WorldSpaceCenter()) * 0.5f,
                    forward, BLOOD_COLOR_RED, 4, FX_BLOODSPRAY_ALL);
  }
}
...
void CEnvFunnel::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
                     USE_TYPE useType, float value) {
  CBroadcastRecipientFilter filter;
  te->LargeFunnel(filter, 0.0, &GetAbsOrigin(), m_iSprite,
                  HasSpawnFlags(SF_FUNNEL_REVERSE) ? 1 : 0);
...
void CEnvBeverage::Precache(void) { PrecacheModel("models/can.mdl"); }
...
void CEnvBeverage::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
                       USE_TYPE useType, float value) {
  if (m_CanInDispenser || m_iHealth <= 0) {
    // no more cans while one is waiting in the dispenser, or if I'm out of
    // cans.
    return;
  }

  CBaseAnimating *pCan = (CBaseAnimating *)CBaseEntity::Create(
      "item_sodacan", GetLocalOrigin(), GetLocalAngles(), this);
...
DEFINE_KEYFIELD(m_nPrecipType, FIELD_INTEGER, "preciptype"), END_DATADESC()

    // Just send the normal entity crap
    IMPLEMENT_SERVERCLASS_ST(CPrecipitation, DT_Precipitation)
        SendPropInt(SENDINFO(m_nPrecipType),
                    Q_log2(NUM_PRECIPITATION_TYPES) + 1, SPROP_UNSIGNED)
            END_SEND_TABLE()
...
private:
  // #ifdef POSIX
  CEnvWindShared m_EnvWindShared; // FIXME - fails to compile as networked var
                                  // due to operator= problem
  // #else
...
DEFINE_KEYFIELD(m_EnvWindShared.m_iMinWind, FIELD_INTEGER, "minwind"),
    ...
    DEFINE_KEYFIELD(m_EnvWindShared.m_flMinGustDelay, FIELD_FLOAT,
                    "mingustdelay"),
    DEFINE_KEYFIELD(m_EnvWindShared.m_flMaxGustDelay, FIELD_FLOAT,
                    "maxgustdelay"),
    DEFINE_KEYFIELD(m_EnvWindShared.m_iGustDirChange, FIELD_INTEGER,
                    "gustdirchange"),
    DEFINE_KEYFIELD(m_EnvWindShared.m_flGustDuration, FIELD_FLOAT,
                    "gustduration"),
    //	DEFINE_KEYFIELD( m_EnvWindShared.m_iszGustSound, FIELD_STRING,
    //"gustsound" ),
    ...
    SendPropFloat(SENDINFO(m_flMinGustDelay), 0,
                  SPROP_NOSCALE), // NOTE: Have to do this, so it's *exactly*
                                  // the same on client
    ...
    // Sound related
    //	SendPropInt		(SENDINFO(m_iszGustSound),	10,
    //SPROP_UNSIGNED ),
    END_SEND_TABLE()

    // This table encodes the CBaseEntity data.
    IMPLEMENT_SERVERCLASS_ST_NOBASE(CEnvWind, DT_EnvWind)
        SendPropDataTable(SENDINFO_DT(m_EnvWindShared),
                          &REFERENCE_SEND_TABLE(DT_EnvWindShared)),
    END_SEND_TABLE()
...
void CEnvWind::Spawn(void) {
  ...
  m_EnvWindShared.Init(entindex(), 0, gpGlobals->frametime, GetLocalAngles().y,
                       0);
...
public:
  ...
  void EmberUse(CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType,
                float value);
...
//			value -
//-----------------------------------------------------------------------------
void CEmbers::EmberUse(CBaseEntity *pActivator, CBaseEntity *pCaller,
                       USE_TYPE useType, float value) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CPhysicsWire::Precache(void) { BaseClass::Precache(); }
...
bool CPhysicsWire::SetupPhysics(void) {
  /*
          ...
          for ( int i = 0; i < m_nDensity; i++ )
          {
                  ...
                  //socket = (CPhysBallSocket *) CreateEntityByName(
     "phys_ballsocket" );

                  Q_snprintf( anchorName,sizeof(anchorName), "__PWIREANCHOR%d",
     i ); Q_snprintf( freeName,sizeof(freeName), "__PWIREFREE%d", i+1 );

                  iAnchorName = MAKE_STRING( anchorName );
                  iFreeName	= MAKE_STRING( freeName );

                  //Fake the names
                  //socket->m_nameAttach1 = anchorEnt->m_iGlobalname	=
     iAnchorName;
                  //socket->m_nameAttach2 = freeEnt->m_iGlobalname	=
     iFreeName
...
void CEnvMuzzleFlash::Spawn() {
  if ((m_iszParentAttachment != NULL_STRING) && GetParent() &&
      GetParent()->GetBaseAnimating()) {
    CBaseAnimating *pAnim = GetParent()->GetBaseAnimating();
    int nParentAttachment =
        pAnim->LookupAttachment(STRING(m_iszParentAttachment));
...
void CEnvMuzzleFlash::InputFire(inputdata_t &inputdata) {
  g_pEffects->MuzzleFlash(GetAbsOrigin(), GetAbsAngles(), m_flScale,
...
void CEnvSplash::InputSplash(inputdata_t &inputdata) {
  ...
  if (HasSpawnFlags(SF_ENVSPLASH_FINDWATERSURFACE)) {
    if (UTIL_PointContents(GetAbsOrigin()) & MASK_WATER) {
      // No splash if I'm supposed to find the surface of the water, but I'm
      // underwater.
      return;
    }
    ...
    trace_t tr;
    UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() - Vector(0, 0, 4096),
                   (MASK_WATER | MASK_SOLID_BRUSHONLY), this,
                   COLLISION_GROUP_NONE, &tr);
  ...
  if (HasSpawnFlags(SF_ENVSPLASH_DIMINISH)) {
    ...
    trace_t tr;
    UTIL_TraceLine(data.m_vOrigin, data.m_vOrigin - Vector(0, 0, 4096),
...
DEFINE_KEYFIELD(m_iMinBurstSize, FIELD_INTEGER, "minburstsize"),
    ...
    END_DATADESC() LINK_ENTITY_TO_CLASS(env_gunfire, CEnvGunfire);

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CEnvGunfire::Precache() { PrecacheScriptSound(STRING(m_iszShootSound)); }
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CEnvGunfire::StopShooting() { SetThink(NULL); }
...
void CEnvGunfire::ShootThink() {
  ...
  if (m_bCollide) {
    ...
    UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + vecDir * 8192, MASK_SHOT,
  ...
  if (m_iszTracerType != NULL_STRING) {
    UTIL_Tracer(GetAbsOrigin(), vecEnd, 0, TRACER_DONT_USE_ATTACHMENT, 5000,
                true, STRING(m_iszTracerType));
  } else {
    UTIL_Tracer(GetAbsOrigin(), vecEnd, 0, TRACER_DONT_USE_ATTACHMENT, 5000,
                true);
  }
  ...
  if (m_iShotsRemaining == 0) {
    StartShooting();
    SetNextThink(gpGlobals->curtime +
                 random->RandomFloat(m_flMinBurstDelay, m_flMaxBurstDelay));
  }
}
...
CEnvQuadraticBeam *CreateQuadraticBeam(const char *pSpriteName,
  ...
  CEnvQuadraticBeam *pBeam = (CEnvQuadraticBeam *)CBaseEntity::Create(
      "env_quadraticbeam", start, vec3_angle, pOwner);
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CEnvViewPunch::InputViewPunch(inputdata_t &inputdata) { DoViewPunch(); }
