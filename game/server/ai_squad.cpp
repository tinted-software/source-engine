...
DEFINE_FIELD(hEnemy, FIELD_EHANDLE), DEFINE_BITSTRING(slots),
    ...
    // 							m_pNextSquad
    // (rebuilt) 							m_Name				(rebuilt) 							m_SquadMembers
    // (rebuilt) 			 				m_SquadMembers.Count()		(rebuilt)
...
bool CAI_Squad::IsSilentMember(const CAI_BaseNPC *pNPC) {
  if (!pNPC || (pNPC->GetMoveType() == MOVETYPE_NONE &&
                pNPC->GetSolid() == SOLID_NONE)) // a.k.a., enemy finder
    return true;
...
void CAI_Squad::AddToSquad(CAI_BaseNPC *pNPC) {
  ...
  if (m_SquadMembers.Count() > 1) {
    ...
    while (pInfo) {
      pNPC->UpdateEnemyMemory(pInfo->hEnemy, pInfo->vLastKnownLocation,
                              pCopyFrom);
...
CAI_BaseNPC *CAI_Squad::SquadMemberInRange(const Vector &vecLocation,
                                           float flDist) {
...
//-----------------------------------------------------------------------------
CAI_BaseNPC *CAI_Squad::GetFirstMember(AISquadIter_t *pIter,
                                       bool bIgnoreSilentMembers) {
  intp i = 0;
...
CAI_BaseNPC *CAI_Squad::GetNextMember(AISquadIter_t *pIter,
                                      bool bIgnoreSilentMembers) {
  int &i = (int &)*pIter;
  i++;
  if (bIgnoreSilentMembers) {
...
void CAI_Squad::SquadNewEnemy(CBaseEntity *pEnemy) {
  ...
  for (int i = 0; i < m_SquadMembers.Count(); i++) {
    ...
    if (pMember) {
      // reset members who aren't activly engaged in fighting (only do this if
      // the NPC's using the squad memory, or it'll fail)
      if (!pMember->GetEnemy() ||
...
//			sender - who sent the message (NULL by default, if not,
//will not resend to the sender)
//-------------------------------------

int CAI_Squad::BroadcastInteraction(int interactionType, void *data,
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CAI_Squad::OccupyStrategySlotRange(CBaseEntity *pEnemy, int slotIDStart,
                                        int slotIDEnd, int *pSlot) {
#ifndef PER_ENEMY_SQUADSLOTS
  // FIXME: combat slots need to be per enemy, not per squad.
  // As it is, once a squad is occupied it stops making even simple attacks to
  // other things nearby. This code may make soldiers too aggressive
...
bool CAI_Squad::IsStrategySlotRangeOccupied(CBaseEntity *pEnemy,
                                            int slotIDStart, int slotIDEnd) {
  for (int i = slotIDStart; i <= slotIDEnd; i++) {
    if (!IsSlotOccupied(pEnemy, i))
      return false;
  }
...
void CAI_Squad::UpdateEnemyMemory(CAI_BaseNPC *pUpdater, CBaseEntity *pEnemy,
  ...
  if (gpGlobals->curtime > m_flEnemyInfoCleanupTime) {
    if (m_EnemyInfos.Count()) {
      ...
      for (i = 0; i < m_SquadMembers.Count(); i++) {
        ...
        if (pMemberEnemy &&
            activeEnemies.Find(pMemberEnemy) == activeEnemies.InvalidIndex()) {
      ...
      for (i = m_EnemyInfos.Count() - 1; i >= 0; --i) {
        if (m_EnemyInfos[i].hEnemy == NULL ||
            activeEnemies.Find(m_EnemyInfos[i].hEnemy) ==
                activeEnemies.InvalidIndex()) {
