...
public:
  ...
  void InputToggle(
      inputdata_t &input); // Input function for toggling our effect's state
  void InputScale(inputdata_t &input);

private:
  CNetworkVar(bool,
              m_bEmit); // Marks whether the effect should be active or not
...
DEFINE_FIELD(m_bEmit, FIELD_BOOLEAN),
    ...
    DEFINE_INPUTFUNC(FIELD_VOID, "Toggle",
                     InputToggle), // Declare our toggle input function
    ...
    IMPLEMENT_SERVERCLASS_ST(CSparkler, DT_Sparkler)
        SendPropInt(SENDINFO(m_bEmit), 1,
                    SPROP_UNSIGNED), // Declare our boolean state variable
