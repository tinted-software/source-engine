...
void CPropJeep::AimGunAt(Vector *endPos, float flInterval) {
  ...
  float newTargetYaw =
      clamp(targetYaw, -CANNON_MAX_LEFT_YAW, CANNON_MAX_RIGHT_YAW);
  float newTargetPitch =
      clamp(targetPitch, -CANNON_MAX_DOWN_PITCH, CANNON_MAX_UP_PITCH);
...
void CPropJeep::FireChargedCannon(void) {
  /*	bool penetrated = false;
          ...
          trace_t	tr;
          UTIL_TraceLine( m_vecGunOrigin, endPos, MASK_SHOT, this,
     COLLISION_GROUP_NONE, &tr );

          ClearMultiDamage();

          //Find how much damage to do
          float flChargeAmount = ( gpGlobals->curtime -
     m_flCannonChargeStartTime ) / MAX_GAUSS_CHARGE_TIME;
          ...
          if ( tr.DidHitWorld() && !(tr.surface.flags & SURF_SKY) )
          {
                  ...
                  CPVSFilter filter( tr.endpos );
                  te->GaussExplosion( filter, 0.0f, tr.endpos, tr.plane.normal,
     0 );
     ...
     COLLISION_GROUP_NONE, &tr );
          ...
          {
                  CTakeDamageInfo dmgInfo( this, GetDriver(), flDamage,
     DMG_SHOCK ); CalculateBulletDamageForce( &dmgInfo,
     GetAmmoDef()->Index("GaussEnergy"), aimDir, tr.endpos, 1.0f +
     flChargeAmount * 4.0f );
     ...
     0 );
          ...
          if ( penetrated == true )
          {
                  RadiusDamage( CTakeDamageInfo( this, this, flDamage, DMG_SHOCK
     ), tr.endpos, 200.0f, CLASS_NONE, NULL );
          } */
}
...
void CPropJeep::UpdateSteeringAngle(void) {
  ...
  float flSteeringDegrees =
      JEEP_STEERING_FAST_ANGLE +
      ((JEEP_STEERING_SLOW_ANGLE - JEEP_STEERING_FAST_ANGLE) * flRatio);
  flSteeringDegrees = clamp(flSteeringDegrees, JEEP_STEERING_FAST_ANGLE,
                            JEEP_STEERING_SLOW_ANGLE);
