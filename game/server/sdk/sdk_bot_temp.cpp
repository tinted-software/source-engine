...
class CBotManager {
public:
  static CBasePlayer *ClientPutInServerOverride_Bot(edict_t *pEdict,
...
CBasePlayer *BotPutInServer(bool bFrozen) {
  ...
  // This trick lets us create a CSDKBot for this client instead of the
  // CSDKPlayer that we would normally get when ClientPutInServer is called.
...
// Output : 	virtual void
//-----------------------------------------------------------------------------
static void RunPlayerMove(CSDKPlayer *fakeclient, CUserCmd &cmd,
                          float frametime) {
  if (!fakeclient)
    return;
...
void Bot_UpdateDirection(CSDKBot *pBot) {
  ...
  while (--maxtries >= 0) {
    ...
    UTIL_TraceHull(vecSrc, vecEnd, VEC_HULL_MIN_SCALED(pBot),
                   VEC_HULL_MAX_SCALED(pBot), MASK_PLAYERSOLID, pBot,
