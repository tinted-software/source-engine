...
static ConVar sv_maxusrcmdprocessticks(
    "sv_maxusrcmdprocessticks", "24", FCVAR_NOTIFY,
...
ConVar cl_forwardspeed("cl_forwardspeed", "450",
...
// feet will inflict damage #define	PLAYER_FATAL_FALL_DIST		60//
// 100% damage inflicted if player falls this many feet #define
// DAMAGE_PER_UNIT_FALLEN		(float)( 100 ) / ( (
...
// ConVar	player_usercommand_timeout( "player_usercommand_timeout", "10",
// 0, "After this many seconds without a usercommand from a player, the client
// is kicked." );
#ifdef _DEBUG
ConVar sv_player_net_suppress_usercommands(
    "sv_player_net_suppress_usercommands", "0", FCVAR_CHEAT,
...
ConVar sv_player_display_usercommand_errors(
    ...
    "1 = Display warning when command values are out-of-range. 2 = Spew "
    "invalid ranges.");
...
static ConCommand
    givecurrentammo("givecurrentammo", CC_GiveCurrentAmmo,
...
DEFINE_FIELD(v_angle, FIELD_VECTOR), DEFINE_FIELD(deadflag, FIELD_BOOLEAN),
    ...
    // // Don't need to restore DEFINE_FIELD( m_iClientBattery, FIELD_INTEGER ),
    // // Don't restore, client needs reset DEFINE_FIELD( m_iClientHideHUD,
    // FIELD_INTEGER ), // Don't restore, client needs reset DEFINE_FIELD(
    // m_vecAutoAim, FIELD_VECTOR ), // Don't save/restore - this is recomputed
...
void CBasePlayer::DamageEffect(float flDamage, int fDamageType) {
  if (fDamageType & DMG_CRUSH) {
    ...
    // Both -0.1 and 0.1 map to 0 when converted to integer, so all of these
    // RandomInt calls are just expensive ways of returning zero. This code has
    // always been this way and has never had any value. clang complains about
    // the conversion from a literal floating-point number to an integer.
    // ViewPunch(QAngle(random->RandomInt(-0.1,0.1),
    // random->RandomInt(-0.1,0.1), random->RandomInt(-0.1,0.1)));
...
int CBasePlayer::OnTakeDamage(const CTakeDamageInfo &inputInfo) {
  ...
  // NVNT if player's client has a haptic device send them a user message with
  // the damage.
  if (HasHaptics())
...
void CBasePlayer::PackDeadPlayerItems(void) {
  ...
  int i;
  CBaseCombatWeapon
      *rgpPackWeapons[20]; // 20 hardcoded for now. How to determine exactly how
                           // many weapons we have?
...
bool CBasePlayer::SetObserverMode(int mode) {
  ...
  case OBS_MODE_IN_EYE:
    ...
    //=============================================================================
    // HPE_BEGIN:
    // [menglish] Added freeze cam to the setter.  Uses same setup as the
    // roaming mode
    //=============================================================================
...
void CBasePlayer::JumptoPosition(const Vector &origin, const QAngle &angles) {
  ...
  newangles.z =
      clamp(angles.z, MIN_COORD_FLOAT,
            MAX_COORD_FLOAT); // not clamped in original valve's code, idk why
...
static ConVar sv_playerperfhistorycount(
    "sv_playerperfhistorycount", "60", 0,
...
void CBasePlayer::PhysicsSimulate(void) {
  ...
  if (gpGlobals->maxClients != 1 &&
      numUsrCmdProcessTicksMax) // don't apply this filter in SP games
  {
    ...
    if (m_flMovementTimeForUserCmdProcessingRemaining >
        numUsrCmdProcessTicksMax * TICK_INTERVAL)
  ...
  // 	// Kick the player if they haven't sent a user command in awhile in
  // order to prevent clients
  // 	// from using packet-level manipulation to mess with gamestate.  Not
  // sending usercommands seems
  // 	// to have all kinds of bad effects, such as stalling a bunch of
  // Think()'s and gamestate handling.
  // 	// An example from TF: A medic stops sending commands after deploying an
  // uber on another player.
  // 	// As a result, invuln is permanently on the heal target because the
  // maintenance code is stalled. 	if ( GetTimeSinceLastUserCommand() >
  // player_usercommand_timeout.GetFloat() )
  // 	{
  // 		// If they have an active netchan, they're almost certainly
  // messing with usercommands? 		INetChannelInfo *pNetChanInfo =
  // engine->GetPlayerNetInfo( entindex() ); 		if ( pNetChanInfo &&
  // pNetChanInfo->GetTimeSinceLastReceived() < 5.f )
  // 		{
...
bool CBasePlayer::IsUserCmdDataValid(CUserCmd *pCmd) {
  ...
  bool bValid =
      ...
      // Prevent clients from sending invalid view angles to try to get leaf
      // server code to crash
      (pCmd->viewangles.IsValid() &&
       IsEntityQAngleReasonable(pCmd->viewangles)) &&
      // Movement ranges
      (IsFinite(pCmd->forwardmove) &&
       IsEntityCoordinateReasonable(pCmd->forwardmove)) &&
      (IsFinite(pCmd->sidemove) &&
       IsEntityCoordinateReasonable(pCmd->sidemove)) &&
  ...
  if (!bValid && nWarningLevel > 0) {
    ...
    if (nWarningLevel == 2) {
      DevMsg(" tick_count: %i\n viewangles: %5.2f %5.2f %5.2f \n forward: "
             "%5.2f \n side: \t%5.2f \n up: \t%5.2f\n",
             pCmd->tick_count, pCmd->viewangles.x, pCmd->viewangles.y,
             pCmd->viewangles.x, pCmd->forwardmove, pCmd->sidemove,
             pCmd->upmove);
    }
  }
...
/* Time based Damage works as follows:
   ...
   movement rate, 30 second duration. Nervegas		- 5 points per second,
   16 second duration = 80 points max dose. Poison			- 2
   points per second, 25 second duration = 50 points max dose. Radiation
   - 1 point per second, 50 second duration = 50 points max dose. Drown
   - 5 points per second, 2 second duration. Acid/Chemical	- 5 points per
   second, 10 second duration = 50 points max. Burn			- 10
   points per second, 2 second duration. Freeze			- 3 points per
   second, 10 second duration = 30 points max.
   ...
   to armor power, then to body
                                                        - recharged by suit
   recharger Air In Lungs		- drowning damage is done to air in
   ...
   provides protection vs one poisoning (nervegas or poison). Health kit
   - Immediate stop to acid/chemical, fire or freeze damage. Radiation Shower
   - Immediate stop to radiation damage, acid/chemical or fire damage.
...
// to take damage #define PARALYZE_DAMAGE		0.0		//
  ...
  for (i = 0; i < CDMG_TIMEBASED; i++) {
    ...
    if (m_bitsDamageType & iDamage) {
      ...
      case itbd_NerveGas:
        ...
        //this, POISON_DAMAGE, DMG_GENERIC ) ); 				bDuration = POISON_DURATION;
        //				break;
      case itbd_Radiation:
...
void CBasePlayer::SetSuitUpdate(const char *name, int fgroup,
                                int iNoRepeatTime) {
  int i;
  int isentence;
  int iempty = -1;
  ...
  if (iNoRepeatTime) {
    if (iempty < 0)
      iempty = random->RandomInt(0, CSUITNOREPEAT -
                                        1); // pick random slot to take over
...
//flag set, that 			is the entity that will be returned. Otherwise, the first
//entity of 			the given classname is returned.
...
void CBasePlayer::Spawn(void) {
  ...
  m_flgeigerDelay =
      gpGlobals->curtime + 2.0; // wait a few seconds until user-defined message
                                // registrations are recieved by all clients

  m_flFieldOfView = 0.766; // some NPCs use this to determine whether or not the
  ...
  m_Local.m_bDucked =
      false; // This will persist over round restart if you hold duck otherwise.
...
//trace will 			hit the collision hull of entities. Otherwise, the trace will hit
//			hitboxes.
// Input  :
// Output :
//-----------------------------------------------------------------------------
...
void CBasePlayer::ImpulseCommands() {
  ...
  case 201: // paint decal
    ...
    if (tr.fraction != 1.0) { // line hit something, so paint a decal
      ...
      //=============================================================================
      // HPE_BEGIN:
      // [pfreese] Fire off a game event - the Counter-Strike stats manager
      // listens to these achievements for one of the CS achievements.
      //=============================================================================
...
void CBasePlayer::CheatImpulseCommands(int iImpulse) {
  ...
  case 107: {
    ...
    UTIL_TraceLine(start, end, MASK_SOLID_BRUSHONLY, this, COLLISION_GROUP_NONE,
                   &tr);
    if (tr.m_pEnt)
...
void CBasePlayer::EquipWearable(CEconWearable *pItem) {
  ...
  // Networked Vector has a max size of MAX_WEARABLES_SENT_FROM_SERVER, should
  // never have more then 7 wearables in public Search for : RecvPropUtlVector(
  // RECVINFO_UTLVECTOR( m_hMyWearables ), MAX_WEARABLES_SENT_FROM_SERVER,
...
// Purpose:
//-----------------------------------------------------------------------------
void CBasePlayer::PlayWearableAnimsForPlaybackEvent(
...
// If HL2_DLL is defined, then baseflex.cpp already sends these.
    ...
    SendPropFloat(SENDINFO_VECTORELEM(m_vecViewOffset, 1), 8, SPROP_ROUNDDOWN,
                  -32.0, 32.0f),
    SendPropFloat(SENDINFO_VECTORELEM(m_vecViewOffset, 2), 20,
                  SPROP_CHANGES_OFTEN, 0.0f, 256.0f),
#endif

    SendPropFloat(SENDINFO(m_flFriction), 8, SPROP_ROUNDDOWN, 0.0f, 4.0f),

    SendPropArray3(SENDINFO_ARRAY3(m_iAmmo),
                   SendPropInt(SENDINFO_ARRAY(m_iAmmo), -1,
                               SPROP_VARINT | SPROP_UNSIGNED)),
...
#if defined USES_ECON_ITEMS
SendPropDataTable(SENDINFO_DT(m_AttributeList),
                  &REFERENCE_SEND_TABLE(DT_AttributeList)),
#endif
...
#if defined USES_ECON_ITEMS
    SendPropUtlVector(SENDINFO_UTLVECTOR(m_hMyWearables),
                      MAX_WEARABLES_SENT_FROM_SERVER, SendPropEHandle(NULL, 0)),
#endif // USES_ECON_ITEMS
...
void CBasePlayer::InitVCollision(const Vector &vecAbsOrigin,
  ...
  CPhysCollide *pModel =
      PhysCreateBbox(VEC_HULL_MIN_SCALED(this), VEC_HULL_MAX_SCALED(this));
  CPhysCollide *pCrouchModel = PhysCreateBbox(VEC_DUCK_HULL_MIN_SCALED(this),
                                              VEC_DUCK_HULL_MAX_SCALED(this));
...
bool CBasePlayer::HandleVoteCommands(const CCommand &args) {
  ...
  if (FStrEq(args[0], "Vote")) {
    ...
    CVoteController::TryCastVoteResult nTryResult =
        g_voteController->TryCastVote(entindex(), arg2);
    switch (nTryResult) {
    ...
    case CVoteController::CAST_FAIL_SERVER_DISABLE: {
      Q_snprintf(szResultString, MAX_COMMAND_LENGTH,
