...
enum RouteBuildFlags_e {
  ...
  bits_BUILD_GET_CLOSE = 0x00000100, // the route will be built even if it can't
...
public:
  CAI_Pathfinder(CAI_BaseNPC *pOuter)
      : CAI_Component(pOuter), m_flLastStaleLinkCheckTime(0), m_pNetwork(NULL) {
  }
  ...
  virtual AI_Waypoint_t *BuildRadialRoute(const Vector &vStartPos,
                                          const Vector &vCenterPos,
                                          const Vector &vGoalPos,
                                          float flRadius, float flArc,
                                          float flStepDist, bool bClockwise,
                                          float goalTolerance, bool bAirRoute);

  virtual AI_Waypoint_t *
  BuildTriangulationRoute(const Vector &vStart, const Vector &vEnd,
                          CBaseEntity const *pTarget, int endFlags, int nodeID,
                          float flYaw, float flDistToBlocker,
                          Navigation_t navType);

  virtual AI_Waypoint_t *BuildOBBAvoidanceRoute(const Vector &vStart,
                                                const Vector &vEnd,
...
private:
  ...
  AI_Waypoint_t *BuildSimpleRoute(Navigation_t navType, const Vector &vStart,
                                  ...
                                  int nodeID, int nodeTargetType, float flYaw);
