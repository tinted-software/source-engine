...
public:
  CPlayerSimInfo()
      : m_flTime(0.0f), m_nNumCmds(0), m_nTicksCorrected(0),
        m_flFinalSimulationTime(0.0f), m_flGameSimulationTime(0.0f),
        m_flServerFrameTime(0.0f), m_vecAbsOrigin(0, 0, 0) {}
...
enum PlayerPhysFlag_e {
  ...
  PFLAG_GAMEPHYSICS_ROTPUSH = (1 << 5), // game physics did a rotating push that
                                        // we may want to override with vphysics
...
public:
  ...
  virtual void OnMyWeaponFired(
      CBaseCombatWeapon *weapon); // call this when this player fires a weapon
                                  // to allow other systems to react
  virtual float GetTimeSinceWeaponFired(void)
      const; // returns the time, in seconds, since this player fired a weapon
  ...
  virtual CBaseEntity *
  GetObserverTarget(void); // returns players targer or NULL
  ...
  virtual void
  ValidateCurrentObserverTarget(void); // Checks the current observer target,
                                       // and moves on if it's not valid anymore
  ...
  virtual void
  ForceDropOfCarriedPhysObjects(CBaseEntity *pOnlyIfHoldindThis = NULL) {}
...
public:
  ...
  void
  SetupVPhysicsShadow(const Vector &vecAbsOrigin, const Vector &vecAbsVelocity,
  ...
  virtual bool
  IsBot() const; // IMPORTANT: This returns true for ANY type of bot. If your
                 // game uses different, incompatible types of bots check your
                 // specific bot type before casting
  virtual bool
  IsBotOfType(int botType) const; // return true if this player is a bot of the
  ...
  float ConsumeMovementTimeForUserCmdProcessing(float flTimeNeeded) {
    if (m_flMovementTimeForUserCmdProcessingRemaining <= 0.0f) {
      return 0.0f;
               ...
               m_flMovementTimeForUserCmdProcessingRemaining + FLT_EPSILON) {
...
protected:
  ...
  CNetworkHandle(
      CBaseEntity,
      m_hUseEntity); // the player is currently controlling this entity because
                     // of +USE latched, NULL if no entity

  int m_iTrain; // Train control position

  float m_iRespawnFrames; // used in PlayerDeathThink() to make sure players can
                          // always respawn
  ...
  CNetworkVar(float, m_flDeathTime); // the time at which the player died  (used
                                     // in PlayerDeathThink())
...
protected:
  ...
  int m_nVehicleViewSavedFrame; // Used to mark which frame was the last one the
                                // view was calculated for
...
public:
  virtual unsigned int PlayerSolidMask(bool brushOnly = false)
      const; // returns the solid mask for the given player, so bots can have a
 ...
 */
template <typename Functor> bool ForEachPlayer(Functor &func) {
 ...
 */
template <> inline bool ForEachPlayer(IPlayerFunctor &func) {
