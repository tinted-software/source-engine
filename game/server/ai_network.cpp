...
public:
  ...
  virtual bool NodeIsValid(CAI_Node &node) {
    ...
    if ((node.GetType() == NODE_AIR && !(m_capabilities & bits_CAP_MOVE_FLY)) ||
        (node.GetType() == NODE_GROUND &&
         !(m_capabilities & bits_CAP_MOVE_GROUND)))
      return false;
  ...
  virtual float NodeDistanceSqr(CAI_Node &node) {
    // UNDONE: This call to Position() really seems excessive here.  What is the
    // real error % relative to 800 units (MAX_NODE_LINK_DIST) ?
    if (m_pNPC)
      ...
      for (int link = 0; link < pNode->NumLinks(); link++) {
        ...
        if (pLink) {
          ...
          if (destID > node && destID < m_iNumNodes) {
            ...
            for (int destLink = 0; destLink < pDestNode->NumLinks();
                 destLink++) {
...
//			index of the smallest number in the array whose it is
//set
//-----------------------------------------------------------------------------

int CAI_Network::FindBSSmallest(CVarBitVec *bitString, float *float_array,
                                int array_size) {
  int winIndex = -1;
...
int CAI_Network::ListNodesInBox(CNodeList &list, int maxListCount,
  ...
  // UNDONE: Store the nodes in a tree and query the tree instead of the entire
  // list!!!
  for (int node = 0; node < m_iNumNodes; node++) {
    CAI_Node *pNode = m_pAInode[node];
    const Vector &origin = pNode->GetOrigin();
    // in box?
    if (origin.x < mins.x || origin.x > maxs.x || origin.y < mins.y ||
        origin.y > maxs.y || origin.z < mins.z || origin.z > maxs.z)
      continue;
...
//			tolerance distance.  If a route is required to get to
//the node 			node_route is set.
//-----------------------------------------------------------------------------

int CAI_Network::NearestNodeToPoint(CAI_BaseNPC *pNPC, const Vector &vecOrigin,
  ...
  if (cachedNode != NO_NODE) {
    if (bCheckVisibility) {
      ...
      Vector vTestLoc =
          (pNPC) ? m_pAInode[cachedNode]->GetPosition(pNPC->GetHullType()) +
                       pNPC->GetViewOffset()
                 : m_pAInode[cachedNode]->GetOrigin();

      CTraceFilterNav traceFilter(pNPC, true, pNPC, COLLISION_GROUP_NONE);
      AI_TraceLine(vecOrigin, vTestLoc, MASK_NPCSOLID_BRUSHONLY, &traceFilter,
                   &tr);
    ...
    if (cachedNode != NO_NODE &&
        (!pFilter || pFilter->IsValid(m_pAInode[cachedNode]))) {
      m_NearestCache[cachePos].expiration =
          gpGlobals->curtime + NEARNODE_CACHE_LIFE;
      return cachedNode;
    }
  }
  ...
  AI_NearNode_t *pBuffer =
      (AI_NearNode_t *)stackalloc(sizeof(AI_NearNode_t) * MAX_NEAR_NODES);
  ...
  if (pNPC && (pNPC->CapabilitiesGet() & bits_CAP_MOVE_FLY)) {
    ext.Init(MAX_AIR_NODE_LINK_DIST, MAX_AIR_NODE_LINK_DIST,
             MAX_AIR_NODE_LINK_DIST);
  }

  ListNodesInBox(list, MAX_NEAR_NODES, vecOrigin - ext, vecOrigin + ext,
                 &filter);
  ...
  for (; list.Count(); list.RemoveAtHead()) {
    ...
    if (bCheckVisibility) {
      ...
      Vector vTestLoc =
          (pNPC) ? m_pAInode[smallest]->GetPosition(pNPC->GetHullType()) +
                       pNPC->GetNodeViewOffset()
                 : m_pAInode[smallest]->GetOrigin();

      Vector vecVisOrigin = vecOrigin + Vector(0, 0, 1);

      CTraceFilterNav traceFilter(pNPC, true, pNPC, COLLISION_GROUP_NONE);
      AI_TraceLine(vecVisOrigin, vTestLoc, MASK_NPCSOLID_BRUSHONLY,
                   &traceFilter, &tr);
    ...
    SetCachedNearestNode(vecOrigin, smallest,
                         (pNPC) ? pNPC->GetHullType() : HULL_NONE);

    return smallest;
  }

  // Store inability to reach in cache for later use
  SetCachedNearestNode(vecOrigin, NO_NODE,
                       (pNPC) ? pNPC->GetHullType() : HULL_NONE);
...
//			node if it exists in the cache.  Doesn't care about
//reachability, 			only if the node is visible
//-----------------------------------------------------------------------------
int CAI_Network::GetCachedNode(const Vector &checkPos, Hull_t nHull,
                               int *pCachePos) {
  ...
  for (int i = 0; i < NEARNODE_CACHE_SIZE; i++) {
    ...
    if (m_NearestCache[iCurrent].hull == nHull &&
        m_NearestCache[iCurrent].expiration > gpGlobals->curtime) {
      if ((m_NearestCache[iCurrent].vTestPosition - checkPos).LengthSqr() <
          Square(24.0)) {
        if (pCachePos)
          *pCachePos = iCurrent;
...
int CAI_Network::GetCachedNearestNode(const Vector &checkPos, CAI_BaseNPC *pNPC,
                                      int *pCachePos) {
  if (pNPC) {
    ...
    if (nodeID >= 0) {
      if (filter.NodeIsValid(*m_pAInode[nodeID]) &&
          pNPC->GetNavigator()->CanFitAtNode(nodeID))
        return nodeID;
    }
  }
  return NO_NODE;
...
//			if nHull == HULL_NONE, reachability of this node wasn't
//checked
//-----------------------------------------------------------------------------

void CAI_Network::SetCachedNearestNode(const Vector &checkPos, int nodeID,
                                       Hull_t nHull) {
  if (ai_no_node_cache.GetBool())
    return;
  ...
  m_NearestCache[m_iNearestCacheNext].expiration =
      gpGlobals->curtime + NEARNODE_CACHE_LIFE;
...
CAI_Node *CAI_Network::AddNode(const Vector &origin, float yaw) {
  ...
  m_pNodeTree->CreateHandle(
      (IHandleEntity *)m_iNumNodes, PARTITION_NODE, origin,
...
CAI_Link *CAI_Network::CreateLink(int srcID, int destID,
...
bool CAI_Network::IsConnected(int srcID, int destID) {
  ...
  if (srcZone == AI_NODE_ZONE_UNIVERSAL ||
      destZone == AI_NODE_ZONE_UNIVERSAL) // only happens in WC edit case
    return true;
