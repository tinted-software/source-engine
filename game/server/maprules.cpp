...
//
// Purpose: Contains entities for implementing/changing game rules dynamically
// within each BSP.
//
//=============================================================================//
...
// Default behavior is to set up like a trigger, invisible, but keep the model
// for volume testing
//
...
//	Points +/- total
//	Flag: Allow negative scores
//SF_SCORE_NEGATIVE 	Flag: Award points to team in teamplay
//SF_SCORE_TEAM
...
void CGameScore::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
...
void CGameEnd::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
                   USE_TYPE useType, float value) {
  if (!CanFireForActivator(pActivator))
    return;
...
//
// CGameText / game_text	-- NON-Localized HUD Message (use env_message to
// display a titles.txt message)
//	Flag: All players
...
// Save parms as a block.  Will break save/restore if the structure changes, but
// this entity didn't ship with Half-Life, so it can't impact saved Half-Life
// games.
...
//
// CGameTeamSet / game_team_set	-- Changes the team of the entity it targets to
the activator's team
// Flag: Fire once
// Flag: Clear team				-- Sets the team to "NONE"
...
public:
        ...
        inline bool RemoveOnFire( void ) { return (m_spawnflags &
SF_TEAMSET_FIREONCE) ? true : false; } inline bool ShouldClearTeam( void ) {
return (m_spawnflags & SF_TEAMSET_CLEARTEAM) ? true : false; } void
InputTrigger( inputdata_t &inputdata );
...
//
// CGamePlayerZone / game_player_zone -- players in the zone fire my target when
// I'm fired
//
// Needs master?
...
DEFINE_INPUTFUNC(FIELD_VOID, "CountPlayersInZone", InputCountPlayersInZone),
    ...
    //			in the zone, one output per player out of the zone, and
    //outputs 			with the total counts of players in and out of the zone.
    //-----------------------------------------------------------------------------
  ...
  for (int i = 1; i <= gpGlobals->maxClients; i++) {
    ...
    if (pPlayer) {
      ...
      UTIL_TraceModel(pPlayer->GetAbsOrigin(), pPlayer->GetAbsOrigin(),
                      NAI_Hull::Mins(hullType), NAI_Hull::Maxs(hullType), this,
...
USE_TYPE useType, float value ); inline bool RemoveOnFire( void ) { return
(m_spawnflags & SF_PKILL_FIREONCE) ? true : false; }

        DECLARE_DATADESC();

private:

        float m_flDamage;		// Damage to inflict, negative values
give health.
...
void CGamePlayerHurt::Use( CBaseEntity *pActivator, CBaseEntity *pCaller,
USE_TYPE useType, float value )
{
        if ( !CanFireForActivator( pActivator ) )
                return;
        ...
        if ( pActivator->IsPlayer() )
        {
                ...
                {
                        pActivator->TakeDamage( this, this, m_flDamage,
DMG_GENERIC );
                }
        }

        SUB_UseTargets( pActivator, useType, value );
        m_OnUse.FireOutput(pActivator, this); // dvsents2: handle useType and
value here - they are passed through
...
DEFINE_AUTO_ARRAY(m_weaponNames, FIELD_STRING),
    ...
    END_DATADESC()
        ...
        bool CGamePlayerEquip::KeyValue(const char *szKeyName,
                                        const char *szValue) {
...
void CGamePlayerEquip::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
                           USE_TYPE useType, float value) {
  EquipPlayer(pActivator);
}

//
// CGamePlayerTeam / game_player_team	-- Changes the team of the player who
// fired it Flag: Fire once Flag: Kill Player Flag: Gib Player
...
const char *CGamePlayerTeam::TargetTeamName(const char *pszTargetName,
  ...
  while ((pTeamEntity = gEntList.FindEntityByName(pTeamEntity, pszTargetName,
                                                  NULL, pActivator)) != NULL) {
...
void CGamePlayerTeam::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
                          USE_TYPE useType, float value) {
  if (!CanFireForActivator(pActivator))
    return;
  ...
  if (pActivator->IsPlayer()) {
    ...
    if (pszTargetTeam) {
      ...
      g_pGameRules->ChangePlayerTeam(pPlayer, pszTargetTeam, ShouldKillPlayer(),
                                     ShouldGibPlayer());
    }
  }
