...
ConVar g_debug_transitions(
    "g_debug_transitions", "0", FCVAR_NONE,
    "Set to 1 and restart the map to be warned if the map has no "
    "trigger_transition volumes. Set to 2 to see a dump of all entities & "
    ...
    for (int i = iSize - 1; i >= 0; i--) {
      ...
      if (!hOther) {
        ...
        AssertMsg(
            0,
            CFmtStr(
                "Dead player [%s] is still touching this trigger at [%f %f %f]",
...
public:
  ...
  float
      m_flTimeoutDuration; // Number of seconds after start touch to fire anyway
...
int CChangeLevel::InTransitionVolume(CBaseEntity *pEntity,
  ...
  int inVolume =
      TRANSITION_VOLUME_NOT_FOUND; // Unless we find a trigger_transition,
                                   // everything is in the volume
  ...
  while (pVolume) {
    if (pVolume && FClassnameIs(pVolume, "trigger_transition")) {
      ...
      inVolume =
          TRANSITION_VOLUME_SCREENED_OUT; // Found a trigger_transition, but I
                                          // don't intersect it -- if I don't
                                          // find another, don't go!
    }
  ...
  for (i = 0; i < nCount; ++i) {
    ...
    for (int j = 0; j < nDepCount; ++j) {
      ...
      if (flags) {
        ...
        Warning("Warning!! Save dependency is linked to an entity that doesn't "
                "want to be saved!\n");
      }
    }
  }
...
//the target 			by their initial offset from the landmark and their angles are 			left
//alone.
//
...
DEFINE_KEYFIELD(m_iSearchType, FIELD_INTEGER, "SearchType"),
  ...
  while (!pResult) {
    ...
    case 2: {
      pEntity = gEntList.FindEntityInSphere(pEntity, GetLocalOrigin(),
                                            (m_flRadius != 0.0) ? m_flRadius
                                                                : FLT_MAX);
      break;
    }
    }
  ...
  if (m_hTarget) {
    ...
    if (m_iszTargetAttachment != NULL_STRING) {
      if (!m_hTarget->GetBaseAnimating()) {
        Warning("%s tried to target an attachment (%s) on target %s, which has "
                "no model.\n",
...
//the specified 			entity. The nearest entity distance is output as a number from
//[0 - 1].
//-----------------------------------------------------------------------------
...
protected:
  ...
  float
      m_fRadius; // The radius around the measure target that we measure within.
...
void CTriggerProximity::Activate(void) {
  ...
  if ((m_hMeasureTarget == NULL) || (m_hMeasureTarget->edict() == NULL)) {
    Warning("TriggerProximity - Missing measure target or measure target with "
            "no origin!\n");
  }
}
...
//measure 			target and outputs the distance as a normalized value from [0..1].
//-----------------------------------------------------------------------------
...
void CTriggerWind::WindThink(void) {
  ...
  if (m_bSwitch) {
    ...
    if (bDone) {
      ...
      SetContextThink(&CTriggerWind::WindThink,
                      m_nHoldBase +
                          random->RandomFloat(-m_nHoldNoise, m_nHoldNoise),
                      WIND_THINK_CONTEXT);
      m_bSwitch = true;
    }
  }
...
void CTriggerPlayerMovement::Spawn(void) {
  if (HasSpawnFlags(SF_TRIGGER_ONLY_PLAYER_ALLY_NPCS)) {
    ...
    DevMsg("*** trigger_playermovement using obsolete spawnflag. Remove and "
           "reset with new value for \"Disable auto player movement\"\n");
...
CTriggerVPhysicsMotion::Simulate(IPhysicsMotionController *pController,
                                 IPhysicsObject *pObject, float deltaTime,
                                 Vector &linear, AngularImpulse &angular) {
  if (m_bDisabled)
    return SIM_NOTHING;
