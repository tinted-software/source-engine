...
// Purpose:
//-----------------------------------------------------------------------------
CRecipientFilter::CRecipientFilter() { Reset(); }
...
void CRecipientFilter::MakeReliable(void) { m_bReliable = true; }

bool CRecipientFilter::IsReliable(void) const { return m_bReliable; }
...
void CRecipientFilter::AddRecipient(CBasePlayer *player) {
  ...
  // If we're predicting and this is not the first time we've predicted this
  // sound
...
void CRecipientFilter::RemoveAllRecipients(void) { m_Recipients.RemoveAll(); }
...
void CRecipientFilter::AddPlayersFromBitMask(
...
void CRecipientFilter::RemovePlayersFromBitMask(
    CBitVec<ABSOLUTE_PLAYER_LIMIT> &playerbits) {
  int index = playerbits.FindNextSetBit(0);
...
bool CRecipientFilter::IsInitMessage(void) const { return m_bInitMessage; }

void CRecipientFilter::MakeInitMessage(void) { m_bInitMessage = true; }
...
void CRecipientFilter::UsePredictionRules(void) {
  ...
  CBasePlayer *pPlayer = ToBasePlayer(
      (CBaseEntity *)g_RecipientFilterPredictionSystem.GetSuppressHost());
...
//			ATTN_NORM -
//-----------------------------------------------------------------------------
void CPASAttenuationFilter::Filter(const Vector &origin,
