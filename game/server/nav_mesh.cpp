...
#define DrawLine(from, to, duration, red, green, blue)                         \
  NDebugOverlay::Line(from, to, red, green, blue, true,                        \
...
ConVar nav_edit("nav_edit", "0", FCVAR_GAMEDLL | FCVAR_CHEAT,
                "Set to one to interactively edit the Navigation Mesh. Set to "
                "zero to leave edit mode.");
ConVar nav_quicksave(
    "nav_quicksave", "1", FCVAR_GAMEDLL | FCVAR_CHEAT,
    "Set to one to skip the time consuming phases of the analysis.  Useful for "
    "data collection and testing."); // TERROR: defaulting to 1, since we don't
                                     // need the other data
ConVar nav_show_approach_points("nav_show_approach_points", "0",
                                FCVAR_GAMEDLL | FCVAR_CHEAT,
                                "Show Approach Points in the Navigation Mesh.");
ConVar nav_show_danger("nav_show_danger", "0", FCVAR_GAMEDLL | FCVAR_CHEAT,
                       "Show current 'danger' levels.");
ConVar nav_show_player_counts("nav_show_player_counts", "0",
                              FCVAR_GAMEDLL | FCVAR_CHEAT,
                              "Show current player counts in each area.");
ConVar nav_show_func_nav_avoid("nav_show_func_nav_avoid", "0",
                               FCVAR_GAMEDLL | FCVAR_CHEAT,
                               "Show areas of designer-placed bot avoidance "
                               "due to func_nav_avoid entities");
ConVar nav_show_func_nav_prefer("nav_show_func_nav_prefer", "0",
                                FCVAR_GAMEDLL | FCVAR_CHEAT,
                                "Show areas of designer-placed bot preference "
                                "due to func_nav_prefer entities");
ConVar nav_show_func_nav_prerequisite(
    "nav_show_func_nav_prerequisite", "0", FCVAR_GAMEDLL | FCVAR_CHEAT,
    "Show areas of designer-placed bot preference due to func_nav_prerequisite "
    "entities");
ConVar nav_max_vis_delta_list_length("nav_max_vis_delta_list_length", "64",
                                     FCVAR_CHEAT);
...
void CNavMesh::DestroyNavigationMesh(bool incremental) {
  ...
  if (!incremental) {
    ...
    FOR_EACH_VEC(TheNavAreas, it) { DestroyArea(TheNavAreas[it]); }
...
void CNavMesh::Update(void) {
  ...
  if (m_updateBlockedAreasTimer.HasStarted() &&
      m_updateBlockedAreasTimer.IsElapsed()) {
    ...
    if (player && player->GetLastKnownArea()) {
      ...
      if (eyepointArea) {
        FOR_EACH_VEC(TheNavAreas, it) {
          ...
          } else if (eyepointArea->IsPotentiallyVisible(area) &&
                     nav_show_potentially_visible.GetInt() == 1) {
  ...
  for (int it = 0; it < m_walkableSeeds.Count(); ++it) {
    ...
    DrawLine(spot.pos + Vector(width, 0, 0), spot.pos + height * spot.normal, 3,
             255, 0, 255);
    DrawLine(spot.pos + Vector(-width, 0, 0), spot.pos + height * spot.normal,
             3, 255, 0, 255);
    DrawLine(spot.pos + Vector(0, width, 0), spot.pos + height * spot.normal, 3,
             255, 0, 255);
    DrawLine(spot.pos + Vector(0, -width, 0), spot.pos + height * spot.normal,
             3, 255, 0, 255);
  }
}
...
void CNavMesh::FireGameEvent(IGameEvent *gameEvent) {
  ...
  if (FStrEq(gameEvent->GetName(), "break_prop") ||
      FStrEq(gameEvent->GetName(), "break_breakable")) {
    CheckAreasOverlappingBreakable collector(
        UTIL_EntityByIndex(gameEvent->GetInt("entindex")));
    ForAllAreas(collector);
  }

  if (FStrEq(gameEvent->GetName(), "round_start") ||
      FStrEq(gameEvent->GetName(), "teamplay_round_start")) {
    OnRoundRestart();
...
void CNavMesh::AddNavArea(CNavArea *area) {
  if (!m_grid.Count()) {
    // If we somehow have no grid (manually creating a nav area without loading
    // or generating a mesh), don't crash
...
public:
  CRegisterPrerequisite(CFuncNavPrerequisite *prereq) { m_prereq = prereq; }
...
void CNavMesh::OnRoundRestart(void) {
  ...
  for (int i = 0; i < IFuncNavPrerequisiteAutoList::AutoList().Count(); ++i) {
    CFuncNavPrerequisite *prereq = static_cast<CFuncNavPrerequisite *>(
        IFuncNavPrerequisiteAutoList::AutoList()[i]);
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Invoked when a game round restarts, but before entities are deleted and
 * recreated
 */
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Given a position, return the nav area that IsOverlapping and is *immediately*
 * beneath it
 */
...
// beneath it
//----------------------------------------------------------------------------
CNavArea *CNavMesh::GetNavArea(CBaseEntity *pEntity, int nFlags,
                               float flBeneathLimit) const {
  ...
  if (use && (nFlags && GETNAVAREA_CHECK_LOS) &&
      (useZ < testPos.z - flStepHeight)) {
    // trace directly down to see if it's below us and unobstructed
    trace_t result;
    UTIL_TraceLine(testPos, Vector(testPos.x, testPos.y, useZ),
                   MASK_NPCSOLID_BRUSHONLY, NULL, COLLISION_GROUP_NONE,
                   &result);
    if ((result.fraction != 1.0f) &&
        (fabs(result.endpos.z - useZ) > flStepHeight))
      return NULL;
  }
  return use;
}
 ...
 */
CNavArea *CNavMesh::GetNearestNavArea(const Vector &pos, bool anyZ,
                                      float maxDist, bool checkLOS,
                                      bool checkGround, int team) const {
  ...
  for (int shift = 0; shift <= shiftLimit; ++shift) {
    for (int x = originX - shift; x <= originX + shift; ++x) {
      ...
      for (int y = originY - shift; y <= originY + shift; ++y) {
        ...
        if (x > originX - shift && x < originX + shift && y > originY - shift &&
            y < originY + shift)
          continue;
        ...
        FOR_EACH_VEC((*areaVector), it) {
          ...
          // TERROR: Using the original pos for distance calculations.  Since
          // it's a pure 3D distance, with no Z restrictions or LOS checks, this
          // should work for passing in bot foot positions. This needs to be
          // ported back to CS:S.
          ...
          // check LOS to area
          // REMOVED: If we do this for !anyZ, it's likely we wont have LOS and
          // will enumerate every area in the mesh It is still good to do this
          // in some isolated cases, however
          if (checkLOS) {
            trace_t result;
            ...
            UTIL_TraceLine(pos, pos + Vector(0, 0, StepHeight),
                           MASK_NPCSOLID_BRUSHONLY, NULL, COLLISION_GROUP_NONE,
                           &result);
            ...
            // Don't bother tracing from the nav area up to safePos.z if it's
            // within StepHeight of the area, since areas can be embedded in the
            // ground a bit
            float heightDelta = fabs(areaPos.z - safePos.z);
            if (heightDelta > StepHeight) {
              // trace to the height of the original point
              UTIL_TraceLine(areaPos + Vector(0, 0, StepHeight),
                             Vector(areaPos.x, areaPos.y, safePos.z),
                             MASK_NPCSOLID_BRUSHONLY, NULL,
            ...
            UTIL_TraceLine(
                safePos, Vector(areaPos.x, areaPos.y, safePos.z + StepHeight),
...
// @todo Make sure area is not on the other side of the wall from goal.
//----------------------------------------------------------------------------
CNavArea *CNavMesh::GetNearestNavArea(CBaseEntity *pEntity, int nFlags,
                                      float maxDist) const {
  ...
  return GetNearestNavArea(pEntity->GetAbsOrigin(), false, maxDist, bCheckLOS,
                           bCheckGround, pEntity->GetTeamNumber());
}
...
void CNavMesh::LoadPlaceDatabase(void) {
...
#ifdef TERROR
  // TODO: LoadPlaceDatabase happens during the constructor, so we can't
  // override it! Population.txt holds all the info we need for place names in
  // Left4Dead, so let's not make Phil edit yet another text file.
  ...
  if (populationData->LoadFromFile(filesystem, "scripts/population.txt")) {
    ...
    for (KeyValues *key = populationData->GetFirstTrueSubKey(); key != NULL;
         key = key->GetNextTrueSubKey()) {
      if (FStrEq(key->GetName(),
                 "default")) // default population is the undefined place
        continue;
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Given the first part of a place name, return a place ID or zero if no place
 * is defined, or the partial match is ambiguous
 */
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Given a partial place name, fill in possible place names for ConCommand
 * autocomplete
 */
int CNavMesh::PlaceNameAutocomplete(
  ...
  for (unsigned int i = 0; i < m_placeCount; ++i) {
    if (!Q_strnicmp(m_placeName[i], partial, partialLength)) {
      ...
      Q_snprintf(commands[numMatches++], COMMAND_COMPLETION_ITEM_LENGTH,
                 "nav_use_place %s", m_placeName[i]);
...
public:
  CTraceFilterGroundEntities(const IHandleEntity *passentity,
...
bool CNavMesh::GetGroundHeight(const Vector &pos, float *height,
                               Vector *normal) const {
  VPROF("CNavMesh::GetGroundHeight");

  const float flMaxOffset = 100.0f;

  CTraceFilterGroundEntities filter(NULL, COLLISION_GROUP_NONE,
  ...
  while (to.z - pos.z < flMaxOffset) {
    ...
    if (!result.startsolid &&
        ((result.fraction == 1.0f) ||
         ((from.z - result.endpos.z) >= HalfHumanHeight))) {
      *height = result.endpos.z;
      if (normal) {
        *normal = !result.plane.normal.IsZero() ? result.plane.normal
                                                : Vector(0, 0, 1);
      }
      return true;
    }
 ...
 * This function is much faster, but less tolerant. Make sure the give position
 * is "well behaved". Return false if position is invalid (outside of map, in a
 * solid area, etc).
 */
bool CNavMesh::GetSimpleGroundHeight(const Vector &pos, float *height,
                                     Vector *normal) const {
  Vector to;
  to.x = pos.x;
  to.y = pos.y;
  ...
  UTIL_TraceLine(pos, to, MASK_NPCSOLID_BRUSHONLY, NULL, COLLISION_GROUP_NONE,
                 &result);
...
void CNavMesh::DrawPlayerCounts(void) const {
  ...
  FOR_EACH_VEC(TheNavAreas, it) {
    ...
    if (area->GetPlayerCount() > 0) {
      NDebugOverlay::Text(area->GetCenter(),
                          msg.sprintf("%d (%d/%d)", area->GetPlayerCount(),
                                      area->GetPlayerCount(1),
                                      area->GetPlayerCount(2)),
 ...
 */
void CNavMesh::IncreaseDangerNearby(int teamID, float amount,
                                    CNavArea *startArea, const Vector &pos,
                                    float maxRadius, float dangerLimit) {
  if (startArea == NULL)
    return;
  ...
  if (dangerLimit > 0.0f &&
      startArea->GetDanger(teamID) + finalDanger > dangerLimit) {
      ...
      for (int i = 0; i < count; ++i) {
        ...
        if (!adjArea->IsMarked()) {
          ...
          if (cost <= maxRadius) {
            ...
            if (dangerLimit > 0.0f &&
                adjArea->GetDanger(teamID) + finalDanger > dangerLimit) {
              // clamp danger to given limit
...
static ConCommand nav_remove_jump_areas(
    "nav_remove_jump_areas", CommandNavRemoveJumpAreas,
...
static ConCommand nav_delete("nav_delete", CommandNavDelete,
...
static ConCommand
    nav_delete_marked("nav_delete_marked", CommandNavDeleteMarked,
                      "Deletes the currently marked Area (if any).",
                      FCVAR_GAMEDLL | FCVAR_CHEAT);

//--------------------------------------------------------------------------------------------------------------
CON_COMMAND_F(nav_flood_select,
              "Selects the current Area and all Areas connected to it, "
              "recursively. To clear a selection, use this command again.",
              FCVAR_GAMEDLL | FCVAR_CHEAT) {
  if (!UTIL_IsCommandIssuedByServerAdmin())
    return;
...
static ConCommand nav_store_selected_set(
    "nav_store_selected_set", CommandNavStoreSelectedSet,
...
static ConCommand nav_recall_selected_set("nav_recall_selected_set",
...
static ConCommand nav_add_to_selected_set(
    "nav_add_to_selected_set", CommandNavAddToSelectedSet,
    "Add current area to the selected set.", FCVAR_GAMEDLL | FCVAR_CHEAT);

//--------------------------------------------------------------------------------------------------------------
CON_COMMAND_F(nav_add_to_selected_set_by_id,
...
static ConCommand nav_remove_from_selected_set(
    "nav_remove_from_selected_set", CommandNavRemoveFromSelectedSet,
...
static ConCommand nav_toggle_in_selected_set(
    "nav_toggle_in_selected_set", CommandNavToggleInSelectedSet,
    "Remove current area from the selected set.", FCVAR_GAMEDLL | FCVAR_CHEAT);
...
static ConCommand nav_clear_selected_set("nav_clear_selected_set",
...
CON_COMMAND_F(nav_dump_selected_set_positions,
              "Write the (x,y,z) coordinates of the centers of all selected "
              "nav areas to a file.",
              FCVAR_GAMEDLL | FCVAR_CHEAT) {
...
//----------------------------------------------------------------------------------
CON_COMMAND_F(nav_show_dumped_positions,
...
static ConCommand
    nav_begin_selecting("nav_begin_selecting", CommandNavBeginSelecting,
...
static ConCommand
    nav_end_selecting("nav_end_selecting", CommandNavEndSelecting,
...
static ConCommand nav_begin_drag_selecting("nav_begin_drag_selecting",
...
static ConCommand nav_end_drag_selecting("nav_end_drag_selecting",
...
static ConCommand nav_begin_drag_deselecting("nav_begin_drag_deselecting",
...
static ConCommand nav_end_drag_deselecting("nav_end_drag_deselecting",
...
static ConCommand nav_raise_drag_volume_max(
    "nav_raise_drag_volume_max", CommandNavRaiseDragVolumeMax,
...
static ConCommand nav_lower_drag_volume_max(
    "nav_lower_drag_volume_max", CommandNavLowerDragVolumeMax,
...
static ConCommand nav_raise_drag_volume_min(
    "nav_raise_drag_volume_min", CommandNavRaiseDragVolumeMin,
...
static ConCommand nav_lower_drag_volume_min(
    "nav_lower_drag_volume_min", CommandNavLowerDragVolumeMin,
...
static ConCommand nav_toggle_selecting(
    "nav_toggle_selecting", CommandNavToggleSelecting,
...
static ConCommand
    nav_begin_deselecting("nav_begin_deselecting", CommandNavBeginDeselecting,
...
static ConCommand
    nav_end_deselecting("nav_end_deselecting", CommandNavEndDeselecting,
...
static ConCommand nav_toggle_deselecting(
    "nav_toggle_deselecting", CommandNavToggleDeselecting,
    "Start or stop continuously removing from the selected set.",
    FCVAR_GAMEDLL | FCVAR_CHEAT);

//--------------------------------------------------------------------------------------------------------------
CON_COMMAND_F(nav_select_half_space,
...
static ConCommand nav_begin_shift_xy("nav_begin_shift_xy",
...
static ConCommand nav_end_shift_xy("nav_end_shift_xy", CommandNavEndShiftXY,
...
static ConCommand nav_select_invalid_areas(
    "nav_select_invalid_areas", CommandNavSelectInvalidAreas,
    "Adds all invalid areas to the Selected Set.", FCVAR_GAMEDLL | FCVAR_CHEAT);

//--------------------------------------------------------------------------------------------------------------
CON_COMMAND_F(nav_select_blocked_areas,
              "Adds all blocked areas to the selected set", FCVAR_CHEAT) {
  if (!UTIL_IsCommandIssuedByServerAdmin())
    return;
...
//--------------------------------------------------------------------------------------------------------------
CON_COMMAND_F(nav_select_obstructed_areas,
              "Adds all obstructed areas to the selected set", FCVAR_CHEAT) {
  if (!UTIL_IsCommandIssuedByServerAdmin())
    return;
...
//--------------------------------------------------------------------------------------------------------------
CON_COMMAND_F(nav_select_damaging_areas,
              "Adds all damaging areas to the selected set", FCVAR_CHEAT) {
  if (!UTIL_IsCommandIssuedByServerAdmin())
    return;
...
//--------------------------------------------------------------------------------------------------------------
CON_COMMAND_F(nav_select_stairs, "Adds all stairway areas to the selected set",
              FCVAR_CHEAT) {
  if (!UTIL_IsCommandIssuedByServerAdmin())
    return;
...
static ConCommand nav_split("nav_split", CommandNavSplit,
                            "To split an Area into two, align the split line "
                            "using your cursor and invoke the split command.",
                            FCVAR_GAMEDLL | FCVAR_CHEAT);
...
static ConCommand
    nav_make_sniper_spots("nav_make_sniper_spots", CommandNavMakeSniperSpots,
                          "Chops the marked area into disconnected sub-areas "
...
static ConCommand nav_merge(
    "nav_merge", CommandNavMerge,
    "To merge two Areas into one, mark the first Area, highlight the second by "
    "pointing your cursor at it, and invoke the merge command.",
    FCVAR_GAMEDLL | FCVAR_CHEAT);
...
static ConCommand nav_mark("nav_mark", CommandNavMark,
                           "Marks the Area or Ladder under the cursor for "
                           "manipulation by subsequent editing commands.",
                           FCVAR_GAMEDLL | FCVAR_CHEAT);
...
static ConCommand nav_unmark("nav_unmark", CommandNavUnmark,
...
static ConCommand
    nav_begin_area("nav_begin_area", CommandNavBeginArea,
                   "Defines a corner of a new Area or Ladder. To complete the "
                   "Area or Ladder, drag the opposite corner to the desired "
                   "location and issue a 'nav_end_area' command.",
                   FCVAR_GAMEDLL | FCVAR_CHEAT);
...
static ConCommand nav_end_area(
...
static ConCommand nav_connect(
    "nav_connect", CommandNavConnect,
    "To connect two Areas, mark the first Area, highlight the second Area, "
    "then invoke the connect command. Note that this creates a ONE-WAY "
    "connection from the first to the second Area. To make a two-way "
    "connection, also connect the second area to the first.",
    FCVAR_GAMEDLL | FCVAR_CHEAT);
...
static ConCommand
    nav_disconnect("nav_disconnect", CommandNavDisconnect,
                   "To disconnect two Areas, mark an Area, highlight a second "
                   "Area, then invoke the disconnect command. This will remove "
...
static ConCommand nav_disconnect_outgoing_oneways(
    ...
    "For each area in the selected set, disconnect all outgoing one-way "
    "connections.",
    FCVAR_GAMEDLL | FCVAR_CHEAT);
...
static ConCommand nav_splice(
    "nav_splice", CommandNavSplice,
    "To splice, mark an area, highlight a second area, then invoke the splice "
...
static ConCommand nav_crouch(
    "nav_crouch", CommandNavCrouch,
...
static ConCommand nav_precise(
    "nav_precise", CommandNavPrecise,
...
static ConCommand nav_jump(
    "nav_jump", CommandNavJump,
...
static ConCommand nav_no_jump(
    "nav_no_jump", CommandNavNoJump,
...
static ConCommand nav_stop("nav_stop", CommandNavStop,
                           "Toggles the 'must stop when entering this area' "
                           "flag used by the AI system.",
                           FCVAR_GAMEDLL | FCVAR_CHEAT);
...
static ConCommand nav_walk(
    "nav_walk", CommandNavWalk,
...
static ConCommand nav_run(
    "nav_run", CommandNavRun,
...
static ConCommand nav_avoid(
    "nav_avoid", CommandNavAvoid,
...
static ConCommand nav_transient("nav_transient", CommandNavTransient,
                                "Toggles the 'area is transient and may become "
                                "blocked' flag used by the AI system.",
                                FCVAR_GAMEDLL | FCVAR_CHEAT);
...
static ConCommand nav_dont_hide("nav_dont_hide", CommandNavDontHide,
                                "Toggles the 'area is not suitable for hiding "
                                "spots' flag used by the AI system.",
                                FCVAR_GAMEDLL | FCVAR_CHEAT);
...
static ConCommand
    nav_stand("nav_stand", CommandNavStand,
...
static ConCommand nav_no_hostages(
    "nav_no_hostages", CommandNavNoHostages,
...
static ConCommand nav_strip("nav_strip", CommandNavStrip,
                            "Strips all Hiding Spots, Approach Points, and "
                            "Encounter Spots from the current Area.",
                            FCVAR_GAMEDLL | FCVAR_CHEAT);
...
static ConCommand nav_save("nav_save", CommandNavSave,
...
static ConCommand nav_load("nav_load", CommandNavLoad,
                           "Loads the Navigation Mesh for the current map.",
                           FCVAR_GAMEDLL | FCVAR_CHEAT);

//--------------------------------------------------------------------------------------------------------------
static int
PlaceNameAutocompleteCallback(char const *partial,
...
static ConCommand nav_use_place(
    "nav_use_place", CommandNavUsePlace,
    "If used without arguments, all available Places will be listed. If a "
...
static ConCommand nav_place_replace(
    "nav_place_replace", CommandNavPlaceReplace,
...
static ConCommand nav_place_list("nav_place_list", CommandNavPlaceList,
...
static ConCommand
    nav_toggle_place_mode("nav_toggle_place_mode", CommandNavTogglePlaceMode,
                          "Toggle the editor into and out of Place mode. Place "
                          "mode allows labelling of Area with Place names.",
                          FCVAR_GAMEDLL | FCVAR_CHEAT);
...
static ConCommand
    nav_set_place_mode("nav_set_place_mode", CommandNavSetPlaceMode,
                       "Sets the editor into or out of Place mode. Place mode "
...
static ConCommand nav_place_floodfill(
    ...
    "Sets the Place of the Area under the cursor to the curent Place, and "
    "'flood-fills' the Place to all adjacent Areas. Flood-filling stops when "
    "it hits an Area with the same Place, or a different Place than that of "
    "the initial Area.",
    FCVAR_GAMEDLL | FCVAR_CHEAT);
...
static ConCommand
    nav_place_set("nav_place_set", CommandNavPlaceSet,
...
static ConCommand nav_place_pick(
    "nav_place_pick", CommandNavPlacePick,
...
static ConCommand nav_toggle_place_painting(
    ...
    "Toggles Place Painting mode. When Place Painting, pointing at an Area "
...
static ConCommand
    nav_mark_unnamed("nav_mark_unnamed", CommandNavMarkUnnamed,
                     "Mark an Area with no Place name. Useful for finding "
                     "stray areas missed when Place Painting.",
                     FCVAR_GAMEDLL | FCVAR_CHEAT);
...
static ConCommand
    nav_corner_select("nav_corner_select", CommandNavCornerSelect,
                      "Select a corner of the currently marked Area. Use "
                      "multiple times to access all four corners.",
                      FCVAR_GAMEDLL | FCVAR_CHEAT);

//--------------------------------------------------------------------------------------------------------------
CON_COMMAND_F(nav_corner_raise,
...
//--------------------------------------------------------------------------------------------------------------
CON_COMMAND_F(nav_corner_lower,
...
static ConCommand nav_warp_to_mark("nav_warp_to_mark", CommandNavWarpToMark,
...
static ConCommand nav_ladder_flip("nav_ladder_flip", CommandNavLadderFlip,
...
static ConCommand nav_generate(
    "nav_generate", CommandNavGenerate,
...
static ConCommand nav_generate_incremental(
    "nav_generate_incremental", CommandNavGenerateIncremental,
    "Generate a Navigation Mesh for the current map and save it to disk.",
    FCVAR_GAMEDLL | FCVAR_CHEAT);
...
static ConCommand
    nav_analyze("nav_analyze", CommandNavAnalyze,
...
static ConCommand
    nav_analyze_scripted("nav_analyze_scripted", CommandNavAnalyzeScripted,
...
static ConCommand
    nav_mark_walkable("nav_mark_walkable", CommandNavMarkWalkable,
                      "Mark the current location as a walkable position. These "
...
static ConCommand
    nav_clear_walkable_marks("nav_clear_walkable_marks",
...
static ConCommand
    nav_compress_id("nav_compress_id", CommandNavCompressID,
...
static ConCommand nav_show_ladder_bounds(
    "nav_show_ladder_bounds", CommandNavShowLadderBounds,
...
static ConCommand nav_build_ladder(
    "nav_build_ladder", CommandNavBuildLadder,
...
static ConCommand
    ClearAllNavAttributes("wipe_nav_attributes", NavEditClearAllAttributes,
...
bool NavAttributeToggler::operator()(CNavArea *area) {
  ...
  if (TheNavMesh->IsSelectedSetEmpty() &&
      (area->GetAttributes() & m_attribute) != 0) {
 ...
 */
static int
NavAttributeAutocomplete(const char *input,
                         char commands[COMMAND_COMPLETION_MAXITEMS]
                                      [COMMAND_COMPLETION_ITEM_LENGTH]) {
  ...
  *partialArg = '\000';
  ...
  for (unsigned int i = 0;
       TheNavAttributeTable[i].name && count < COMMAND_COMPLETION_MAXITEMS;
       ++i) {
    if (!Q_strnicmp(TheNavAttributeTable[i].name, partialArg,
                    partialArgLength)) {
      // Add to the autocomplete array
      Q_snprintf(commands[count++], COMMAND_COMPLETION_ITEM_LENGTH, "%s %s",
                 command, TheNavAttributeTable[i].name);
    }
  }
...
static ConCommand NavClearAttribute(
    "nav_clear_attribute", NavEditClearAttribute,
    "Remove given nav attribute from all areas in the selected set.",
    FCVAR_CHEAT, NavAttributeAutocomplete);
...
static ConCommand
    NavMarkAttribute("nav_mark_attribute", NavEditMarkAttribute,
...
static ConCommand nav_pick_area( "nav_pick_area", CommandNavPickArea, "Marks an
area (and corner) based on the surface under the cursor.", FCVAR_GAMEDLL |
FCVAR_CHEAT );
...
static ConCommand nav_resize_horizontal( "nav_resize_horizontal",
CommandNavResizeHorizontal, "TODO", FCVAR_GAMEDLL | FCVAR_CHEAT );
...
static ConCommand nav_resize_vertical( "nav_resize_vertical",
CommandNavResizeVertical, "TODO", FCVAR_GAMEDLL | FCVAR_CHEAT );
 ...
 * Hiding Spot factory
 */
HidingSpot *CNavMesh::CreateHidingSpot(void) const { return new HidingSpot; }
...
void CNavMesh::DestroyHidingSpots(void) {
  ...
  FOR_EACH_VEC(TheHidingSpots, hit) { delete TheHidingSpots[hit]; }
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Construct a Hiding Spot.  Assign a unique ID which may be overwritten if
 * loaded.
 */
...
extern CUtlHash<NavVisPair_t, CVisPairHashFuncs, CVisPairHashFuncs>
    *g_pNavVisPairHash;
...
void CNavMesh::BeginVisibilityComputations(void) {
  if (!g_pNavVisPairHash) {
    g_pNavVisPairHash =
        new CUtlHash<NavVisPair_t, CVisPairHashFuncs, CVisPairHashFuncs>(16 *
                                                                         1024);
  } else {
...
void CNavMesh::EndVisibilityComputations(void) {
  ...
  // Optimize visibility storage of nav mesh by doing a kind of run-length
  // encoding. Pick an "anchor" area and compare adjacent areas visibility lists
  // to it. If the delta is small, point back to the anchor and just store the
  // delta.
  FOR_EACH_VEC(TheNavAreas, it) {
    ...
    for (int dir = NORTH; dir < NUM_DIRECTIONS; ++dir) {
      ...
      for (int i = 0; i < count; ++i) {
        CNavArea *adjArea =
            (CNavArea *)area->GetAdjacentArea((NavDirType)dir, i);

        // do not inherit from an area that is inheriting - use its ultimate
        // source
        ...
        const CNavArea::CAreaBindInfoArray &delta =
            area->ComputeVisibilityDelta(adjArea);
    ...
    if (anchor && bestDelta.Count() <= nav_max_vis_delta_list_length.GetInt() &&
        anchor != area) {
