...
SendPropFloat(SENDINFO(m_SpreadSpeed), 0, SPROP_NOSCALE),
    ...
    SendPropInt(SENDINFO(m_bFaceLeft), 1,
                SPROP_UNSIGNED), // For support of legacy env_steamjet, which
                                 // faced left instead of forward.
...
LINK_ENTITY_TO_CLASS(env_steamjet,
                     CSteamJet); // For support of legacy env_steamjet, which
                                 // faced left instead of forward.
...
void CSteamJet::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
...
//-----------------------------------------------------------------------------
void CSteamJet::InputToggle(inputdata_t &data) { m_bEmit = !m_bEmit; }

//-----------------------------------------------------------------------------
// Purpose: Input handler for turning on the steam jet.
//-----------------------------------------------------------------------------
void CSteamJet::InputTurnOn(inputdata_t &data) { m_bEmit = true; }

//-----------------------------------------------------------------------------
// Purpose: Input handler for turning off the steam jet.
//-----------------------------------------------------------------------------
void CSteamJet::InputTurnOff(inputdata_t &data) { m_bEmit = false; }
