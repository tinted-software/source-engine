...
#define SF_TELEPORT_INTO_DUCK                                                  \
  0x00000002 ///< episodic only: player should be ducked after this teleport
...
DEFINE_FIELD(m_vSaveOrigin, FIELD_VECTOR),
  ...
  if (pTarget->GetMoveParent() != NULL) {
    // Passengers in a vehicle are allowed to teleport; their behavior handles
    // it
...
void CPointTeleport::InputTeleport(inputdata_t &inputdata) {
  ...
  CBaseEntity *pTarget = gEntList.FindEntityByName(
      NULL, m_target, this, inputdata.pActivator, inputdata.pCaller);
  if (pTarget == NULL)
    return;
