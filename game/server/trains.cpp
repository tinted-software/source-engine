...
DEFINE_KEYFIELD(m_NoiseMoving, FIELD_SOUNDNAME, "noise1"),
    ...
    END_DATADESC()
        ...
        bool CBasePlatTrain::KeyValue(const char *szKeyName,
                                      const char *szValue) {
...
DEFINE_KEYFIELD(m_length, FIELD_FLOAT, "wheels"),
    ...
    // DEFINE_FIELD( m_lastBlockPos, FIELD_POSITION_VECTOR ), // temp values for
    // blocking, don't save DEFINE_FIELD( m_lastBlockTick, FIELD_INTEGER ),
    ...
    DEFINE_INPUTFUNC(FIELD_STRING, "TeleportToPathTrack",
    ...
    // Function Pointers
    DEFINE_FUNCTION(Next), DEFINE_FUNCTION(Find), DEFINE_FUNCTION(NearestPath),
...
//-----------------------------------------------------------------------------
// Purpose: Input handler that sets the speed of the train and the direction
//			based on the sign of the speed, and accels/decels to
//that speed
// Input  : Float speed scale from -1 to 1. Negatives values indicate a reversed
//			direction.
//-----------------------------------------------------------------------------
...
void CFuncTrackTrain::InputTeleportToPathTrack(inputdata_t &inputdata) {
  ...
  CPathTrack *pTrack =
      dynamic_cast<CPathTrack *>(gEntList.FindEntityByName(NULL, pszName));
...
//m_bSoundPlaying should become 			false, which will cause the looped sound to
//restart.
//-----------------------------------------------------------------------------
  ...
  case TrainVelocity_EaseInEaseOut: {
    if (m_bAccelToSpeed) {
      ...
      if (flPrevSpeed != flNextSpeed) {
        float flSpeedChangeTime = (abs(flNextSpeed) > abs(flPrevSpeed))
                                      ? m_flAccelSpeed
                                      : m_flDecelSpeed;
        m_flSpeed = UTIL_Approach(m_flDesiredSpeed, m_flSpeed,
                                  flSpeedChangeTime * gpGlobals->frametime);
      }
    } else if (pPrev && pNext) {
...
void CFuncTrackTrain::UpdateOrientationAtPathTracks(CPathTrack *pPrev,
  ...
  if (m_bManualSpeedChanges) {
    if (pNextNode &&
        pNextNode->GetOrientationType() == TrackOrientation_FacePathAngles) {
...
void CFuncTrackTrain::Next(void) {
  ...
  // If we're moving towards a dead end, but our desired speed goes in the
  // opposite direction this fixes us from stalling
