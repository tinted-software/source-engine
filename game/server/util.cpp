...
//-----------------------------------------------------------------------------
const char *
CEntityFactoryDictionary::GetCannonicalName(const char *pClassName) {
...
bool CFlaggedEntitiesEnum::AddToList(CBaseEntity *pEntity) {
  if (m_count >= m_listMax) {
    AssertMsgOnce(0, "reached enumerated list limit.  Increase limit, decrease "
...
IterationRetval_t
CFlaggedEntitiesEnum::EnumElement(IHandleEntity *pHandleEntity) {
...
int UTIL_DropToFloor(CBaseEntity *pEntity, unsigned int mask,
  ...
  UTIL_TraceEntity(pEntity, pEntity->GetAbsOrigin() + Vector(0, 0, 1),
...
CBasePlayer *UTIL_GetListenServerHost(void) {
  ...
  if (engine->IsDedicatedServer()) {
    ...
    Warning("UTIL_GetListenServerHost() called from a dedicated server or "
            "single-player game.\n");
    return NULL;
  }
...
//				SHAKE_START - starts the screen shake for all
//players within the radius 				SHAKE_STOP - stops the screen shake for all players
//within the radius 				SHAKE_AMPLITUDE - modifies the amplitude of the screen shake
//									for all
//players within the radius 				SHAKE_FREQUENCY - modifies the frequency of the
//screen shake 									for all players within the radius 			bAirShake - if this is false,
//then it will only shake players standing on the ground.
//-----------------------------------------------------------------------------
...
//			bInAir - if this is false, then it will only punch
...
// seconds
//			Use this function with caution, it has great potential
//for annoying the player, especially 			if used for target firing predition
...
// Input  : *pDest - entity to be pointed at the target
//			strTarget - name of entity to target (will only choose
//the first!)
//-----------------------------------------------------------------------------
...
//line 			clampEnds - clamps returned points to being on the line segment specified
  ...
  if (pkvFile->LoadFromFile(filesystem, pScriptFile, "MOD")) {
    ...
    while (pkvNode) {
      ...
      if (pNode) {
        ...
        Warning("UTIL_LoadAndSpawnEntitiesFromScript: Failed to spawn entity, "
                "type: '%s'\n",
                pNodeName);
      }
...
void CC_KDTreeTest(const CCommand &args) {
  ...
  case 1: {
    ...
    trace_t trace;
    for (int iTest = 0; iTest < NUM_KDTREE_TESTS; ++iTest) {
      UTIL_TraceHull(vecStart, vecTargets[iTest], VEC_HULL_MIN_SCALED(pPlayer),
                     VEC_HULL_MAX_SCALED(pPlayer), MASK_SOLID, pPlayer,
                     COLLISION_GROUP_NONE, &trace);
    }
    break;
  }
...
void CC_VoxelTreePlayerView(void) {
  ...
  partition->RenderObjectsInPlayerLeafs(vecStart - VEC_HULL_MIN_SCALED(pPlayer),
                                        vecStart + VEC_HULL_MAX_SCALED(pPlayer),
                                        3.0f);
}
