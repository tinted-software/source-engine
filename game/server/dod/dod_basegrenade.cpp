...
ConVar dod_grenadegravity("dod_grenadegravity", "-420", FCVAR_CHEAT,
                          "gravity applied to grenades", true, -2000, true,
                          -300);
extern ConVar dod_bonusround;

IMotionEvent::simresult_e
CGrenadeController::Simulate(IPhysicsMotionController *pController,
...
void CDODBaseGrenade::Spawn(void) {
  ...
  if (m_bUseVPhysics) {
    ...
    if (pPhysicsObject) {
      m_pMotionController =
          physenv->CreateMotionController(&m_GrenadeController);
  ...
  // Don't collide with players on the owner's team for the first bit of our
  // life
...
void CDODBaseGrenade::DetonateThink(void) {
  ...
  if (gpGlobals->curtime > m_flCollideWithTeammatesTime &&
      m_bCollideWithTeammates == false) {
...
void CDODBaseGrenade::ResolveFlyCollisionCustom(trace_t &trace,
                                                Vector &vecVelocity) {
...
void CDODBaseGrenade::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
  ...
  if (dod_bonusround.GetBool()) {
    ...
    // if its after the round and bonus round is on, we can only pick it up if
    // we are winners
    ...
    // if its after the round, and bonus round is off, don't allow anyone to
    // pick it up
  ...
  CBaseCombatWeapon *pWeapon =
      dynamic_cast<CBaseCombatWeapon *>(pPlayer->GiveNamedItem(szClsName));

  Assert(pWeapon &&
         "Wpn pointer has to be valid for us to pick up this grenade");
...
void CDODBaseGrenade::Detonate() {
  ...
  if (dod_bonusround.GetBool() == false &&
  ...
  CTakeDamageInfo info(this, GetThrower(), GetBlastForce(), GetAbsOrigin(),
                       flStunDamage, DMG_STUN);
...
void CDODBaseGrenade::Explode(trace_t *pTrace, int bitsDamageType) {
  ...
  Vector vecReported =
      GetThrower() ? GetThrower()->GetAbsOrigin() : vec3_origin;
...
// this will hit only things that are in newCollisionGroup, but NOT in
// collisionGroupAlreadyChecked
...
public:
  ...
  CTraceFilterCollisionGroupDelta(const IHandleEntity *passentity,
                                  int collisionGroupAlreadyChecked,
  ...
  virtual bool ShouldHitEntity(IHandleEntity *pHandleEntity, int contentsMask) {
    ...
    if (pEntity) {
      if (g_pGameRules->ShouldCollide(m_collisionGroupAlreadyChecked,
                                      pEntity->GetCollisionGroup()))
        return false;
      if (g_pGameRules->ShouldCollide(m_newCollisionGroup,
                                      pEntity->GetCollisionGroup()))
        return true;
    }
...
void CDODBaseGrenade::VPhysicsUpdate(IPhysicsObject *pPhysics) {
  ...
  // find all entities that my collision group wouldn't hit, but
  // COLLISION_GROUP_NONE would and bounce off of them as a ray cast
  CTraceFilterCollisionGroupDelta filter(this, GetCollisionGroup(),
                                         COLLISION_GROUP_NONE);
  trace_t tr;

  UTIL_TraceLine(start, start + vel * gpGlobals->frametime,
                 CONTENTS_HITBOX | CONTENTS_MONSTER | CONTENTS_SOLID, &filter,
                 &tr);
  ...
  if (tr.DidHit() && bHitTeammate == false) {
    ...
    // send a tiny amount of damage so the character will react to getting
    // bonked
    CTakeDamageInfo info(this, GetThrower(), pPhysics->GetMass() * vel,
                         GetAbsOrigin(), flDmg, DMG_CRUSH);
    tr.m_pEnt->DispatchTraceAttack(info, dir, &tr);
    ApplyMultiDamage();

    if (vel.Length() > 1000) {
      CTakeDamageInfo stunInfo(this, GetThrower(), vec3_origin, GetAbsOrigin(),
                               flDmg, DMG_STUN);
...
// with a damped speed
void CDODBaseGrenade::VPhysicsCollision(int index,
                                        gamevcollisionevent_t *pEvent) {
  CBreakable *pBreakable =
      dynamic_cast<CBreakable *>(pEvent->pEntities[!index]);
  if (pBreakable && pBreakable->GetMaterialType() == matGlass &&
      VPhysicsGetObject()) {
    // don't stop, go through this entity after breaking it
    Vector dampedVelocity =
        DOD_GRENADE_WINDOW_BREAK_DAMPING_AMOUNT * pEvent->preVelocity[index];
    VPhysicsGetObject()->SetVelocity(&dampedVelocity,
                                     &pEvent->preAngularVelocity[index]);
  } else
