...
unsigned int CDODBaseRocket::PhysicsSolidMaskForEntity(void) const {
  ...
  if (m_bCollideWithTeammates == false) {
    ...
    teamContents =
        (GetTeamNumber() == TEAM_ALLIES) ? CONTENTS_TEAM1 : CONTENTS_TEAM2;
  } else {
...
void CDODBaseRocket::DoExplosion(trace_t *pTrace) {
  /*
     ...
     SF_ENVEXPLOSION_NOSMOKE, 0.0f,				//explosion
     force this);				//inflictor
  */
  ...
  CTakeDamageInfo info(this, GetOwnerEntity(), vec3_origin, GetAbsOrigin(),
                       GetDamage(), DMG_BLAST, 0);
  RadiusDamage(info, vecOrigin,
               mp_rocketradius.GetFloat() /* GetDamageRadius() */, CLASS_NONE,
               NULL);
  ...
  CTakeDamageInfo stunInfo(this, GetOwnerEntity(), vec3_origin, GetAbsOrigin(),
...
void CDODBaseRocket::RocketTouch(CBaseEntity *pOther) {
  ...
  if (!pOther->IsPlayer() && pOther->m_takedamage == DAMAGE_YES) {
    ...
    info.SetDamage(50);
    info.SetDamageForce(
        vec3_origin); // don't worry about this not having a damage force.
...
//-----------------------------------------------------------------------------
CDODBaseRocket *CDODBaseRocket::Create(const char *szClassname,
                                       const Vector &vecOrigin,
                                       const QAngle &vecAngles,
                                       CBaseEntity *pOwner = NULL) {
  CDODBaseRocket *pMissile = (CDODBaseRocket *)CBaseEntity::Create(
      szClassname, vecOrigin, vecAngles, pOwner);
...
void CDODBaseRocket::SetupInitialTransmittedGrenadeVelocity(
