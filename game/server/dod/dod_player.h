...
public:
  ...
  void IncrementWeaponStat(DODWeaponID iWeaponID, DODStatType_t statType,
                           int iValue = 1);
...
private:
  bool m_bRecordingStats; // not recording until we get a valid class. stop
                          // recording when we join spectator
...
public:
  ...
  // from CBasePlayer
  void SetupVisibility(CBaseEntity *pViewEntity, unsigned char *pvs,
                       int pvssize);
  ...
  // Voice Commands
  //============
  void HandleCommand_Voice(
      const char *pcmd); // player submitted a raw voice_ command
  void HandleCommand_HandSignal(
      const char *pcmd);                // player wants to show a hand signal
  ...
  int m_iCapAreaIconIndex; // which area's icon to show - we are not necessarily
                           // capping it.
  ...
  void HandleSignals(
      void); // check if signals need to do anything, like turn icons on or off
...
public:
  ...
  void Stats_WeaponHit(CDODPlayer *pVictim, int weaponID, int iDamage,
                       int iDamageGiven, int hitgroup, float flHitDistance);
  void Stats_HitByWeapon(CDODPlayer *pAttacker, int weaponID, int iDamage,
...
public:
  ...
  int m_flNextTimeCheck; // Next time the player can execute a "timeleft"
                         // command
  ...
  int iNumKilledByUnanswered[MAX_PLAYERS +
                             1]; // how many unanswered kills this player has
                                 // been dealt by every other player

#if !defined(NO_STEAM)
  STEAM_GAMESERVER_CALLBACK(CDODPlayer, OnGSStatsReceived, GSStatsReceived_t,
                            m_CallbackGSStatsReceived);
#endif

private:
  bool SelectSpawnSpot(const char *pEntClassName, CBaseEntity *&pSpot);

  CBaseEntity *SelectSpawnSpot(CUtlVector<EHANDLE> *pSpawnPoints,
                               int &iLastSpawnIndex);
  ...
  // it (ie: observer is movetype_noclip, non-solid, invisible, etc).
...
private:
  friend void
  Bot_Think(CDODPlayer *pBot); // needs to use the HandleCommand_ stuff.
  bool HandleCommand_JoinTeam(int iTeam);
  bool HandleCommand_JoinClass(int iClass);

  CDODPlayerStateInfo *m_pCurStateInfo; // This can be NULL if no state info is
                                        // defined for m_iPlayerState.

  bool m_bTeamChanged; // have we changed teams this spawn? Used to enforce one
                       // team switch per death rule
  ...
  bool m_bAutoReload; // does the player want to autoreload their weapon when
                      // empty

  bool m_bAutoRezoom; // does the player want to re-zoom after each shot for
                      // sniper rifles and bazookas
...
public:
  ...
  int GetPerRoundCaps(void) { return m_iPerRoundCaptures; }

  int GetPerRoundDefenses(void) { return m_iPerRoundDefenses; }

  int GetPerRoundBombsDetonated(void) { return m_iPerRoundBombsDetonated; }

  int GetPerRoundKills(void) { return m_iPerRoundKills; }
...
inline DODPlayerState CDODPlayer::State_Get() const { return m_iPlayerState; }
