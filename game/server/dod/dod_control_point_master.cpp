...
ConVar mp_tickpointinterval("mp_tickpointinterval", "30", FCVAR_GAMEDLL,
                            "Delay between point gives.", true, 1, false, 0);
...
bool CControlPointMaster::FindControlPoints(void) {
  ...
  for (i = 0; i < m_ControlPoints.Count(); i++) {
    ...
    g_pObjectiveResource->SetBombsRequired(iPointIndex,
                                           pPoint->GetBombsRequired());
    g_pObjectiveResource->SetBombsRemaining(iPointIndex,
                                            pPoint->GetBombsRemaining());

    g_pObjectiveResource->SetCPIcons(
        iPointIndex, pPoint->GetHudIconIndexForTeam(TEAM_ALLIES),
        pPoint->GetHudIconIndexForTeam(TEAM_AXIS),
        pPoint->GetHudIconIndexForTeam(TEAM_UNASSIGNED),
        pPoint->GetTimerCapHudIcon(), pPoint->GetBombedHudIcon());
  }
...
// Think
// =====
// Think is called every 0.1 seconds and checks the status of all the control
...
//
// I moved the search for spawn points to the initial think - sometimes the
// points spawned after the master and it wasnt finding them.
...
void CControlPointMaster::CPMThink(void) {
  ...
  // go through all the control points and make sure theyre the same as the last
  // think check masters
  ...
  //---------------------------------------------------------------------------
  // below here we shouldn't execute unless we are an active control point
  // master
  ...
  if (m_bDisabled || iRoundState != STATE_RND_RUNNING) {
    SetContextThink(&CControlPointMaster::CPMThink, gpGlobals->curtime + 0.2,
                    FLAGS_CONTEXT);
    return;
  }
  ...
  if (gpGlobals->curtime > m_fGivePointsTime) {
    ...
    if (AlliesPoints > 0 && bFoundAllies &&
...
void CControlPointMaster::InputRoundInit(inputdata_t &input) {
  ...
  if (FindControlPoints()) {
    SetContextThink(&CControlPointMaster::CPMThink, gpGlobals->curtime + 0.1,
                    FLAGS_CONTEXT);
  }
...
int CControlPointMaster::TeamOwnsAllPoints(
    CControlPoint *pOverridePoint /* = NULL */,
...
bool CControlPointMaster::WouldNewCPOwnerWinGame(CControlPoint *pPoint,
                                                 int iNewOwner) {
