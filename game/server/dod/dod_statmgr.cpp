...
void CDODStatManager::FireGameEvent(IGameEvent *event) {
  ...
  if (FStrEq(eventName, "dod_stats_weapon_attack")) {
    CDODPlayer *pAttacker =
        ToDODPlayer(UTIL_PlayerByUserId(event->GetInt("attacker")));
    ...
    pAttacker->Stats_WeaponHit(pVictim, weaponID, iDamage, iDamageGiven,
                               hitgroup, flDistance);
    pVictim->Stats_HitByWeapon(pAttacker, weaponID, iDamage, iDamageGiven,
                               hitgroup);
