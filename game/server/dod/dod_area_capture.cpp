...
// Triggers are fired on start of capture, on end of capture and on broken
// capture Can either be capped by both teams at once, or just by one Time to
// capture and number of people required to capture are both passed by the
// mapper
...
void CAreaCapture::AreaTouch(CBaseEntity *pOther) {
  // if they are touching, set their SIGNAL flag on, and their m_iCapAreaNum to
  // ours then in think do all the scoring
  ...
  if (pPlayer->GetTeamNumber() != m_nOwningTeam) {
    bool bAbleToCap =
        (pPlayer->GetTeamNumber() == TEAM_ALLIES && m_bAlliesCanCap) ||
 ...
 * 2) have less than the required number on your team, new required num is
 * everyone 3) have less than the required number alive, new required is
 * numAlive, but time is lengthened
 */

ConVar dod_simulatemultiplecappers("dod_simulatemultiplecappers", "1",
                                   FCVAR_CHEAT);
...
void CAreaCapture::Think(void) {
  ...
  if (DODGameRules()->State_Get() != STATE_RND_RUNNING) {
    ...
    if (m_nNumAllies > 0 || m_nNumAxis > 0) {
      ...
      if (m_pPoint) {
        g_pObjectiveResource->SetCappingTeam(m_pPoint->GetPointIndex(),
                                             TEAM_UNASSIGNED);
      }
    }
    return;
  ...
  for (int i = 1; i <= gpGlobals->maxClients; i++) {
    ...
    if (ent) {
      ...
      if ((pPlayer->m_signals.GetState() & SIGNAL_CAPTUREAREA) &&
          ...
          pPlayer->IsAlive()) // alive check is kinda unnecessary, but there is
                              // some case where non-present people are messing
                              // up this count
      {
  ...
  if (m_bCapturing) {
    ...
    if (iNumAllies > 0 && iNumAxis > 0) {
      ...
      if (flPercentToGo <= 0.5 && m_pPoint) {
        ...
        CDODPlayer *pBlockingPlayer = (m_nCapturingTeam == TEAM_ALLIES)
                                          ? pFirstAxisTouching
                                          : pFirstAlliedTouching;

        if (pBlockingPlayer) {
          if (pBlockingPlayer->GetCapAreaIndex() == m_iAreaIndex &&
              pBlockingPlayer->GetLastBlockCapAttempt() ==
                  m_iCapAttemptNumber) {
            // this is a repeat block on the same cap, ignore it
            NULL;
          } else {
            m_pPoint->CaptureBlocked(pBlockingPlayer);
            pBlockingPlayer->StoreCaptureBlock(m_iAreaIndex,
                                               m_iCapAttemptNumber);
          }
        }
      }
  ...
  } else // not capturing yet
  {
    ...
    if (iNumAllies > 0 && iNumAxis <= 0 && m_bAlliesCanCap &&
        m_nOwningTeam != TEAM_ALLIES) {
      if (iNumAllies >= m_nAlliesNumCap) {
    ...
    } else if (iNumAxis > 0 && iNumAllies <= 0 && m_bAxisCanCap &&
               m_nOwningTeam != TEAM_AXIS) {
      if (iNumAxis >= m_nAxisNumCap) {
...
void CAreaCapture::EndCapture(int team) {
  ...
  if (pTeam) {
    ...
    for (int i = 0; i < iCount; i++) {
      ...
      if (pPlayer) {
        if ((pPlayer->m_signals.GetState() & SIGNAL_CAPTUREAREA) &&
            pPlayer->GetCapAreaIndex() == m_iAreaIndex && pPlayer->IsAlive()) {
  ...
  // he hasnt gotten points yet, and his name will go in the cap string if its
  ...
  if (m_pPoint) {
    ...
    g_pObjectiveResource->SetOwningTeam(m_pPoint->GetPointIndex(),
                                        m_nOwningTeam);
...
void CAreaCapture::InputRoundInit(inputdata_t &inputdata) {
  ...
  if (!m_pPoint) {
    m_pPoint = dynamic_cast<CControlPoint *>(
        gEntList.FindEntityByName(NULL, STRING(m_iszCapPointName)));

    if (m_pPoint) {
      m_pPoint->SetNumCappersRequired(m_nAlliesNumCap, m_nAxisNumCap);
      g_pObjectiveResource->SetCPRequiredCappers(
          m_pPoint->GetPointIndex(), m_nAlliesNumCap, m_nAxisNumCap);
      g_pObjectiveResource->SetCPCapTime(m_pPoint->GetPointIndex(), m_flCapTime,
                                         m_flCapTime);
    }
  }
}
...
// break )
//-----------------------------------------------------------------------------
bool CAreaCapture::CheckIfDeathCausesBlock(CDODPlayer *pVictim,
                                           CDODPlayer *pKiller) {
