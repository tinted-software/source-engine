...
//-----------------------------------------------------------------------------
float CDODBombTarget::GetBombTimerLength(void) { return DOD_BOMB_TIMER_LENGTH; }
...
// Purpose:
//-----------------------------------------------------------------------------
void CDODBombTarget::State_Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
...
// Purpose:
//-----------------------------------------------------------------------------
CDODBombTargetStateInfo *
CDODBombTarget::State_LookupInfo(BombTargetState state) {
  static CDODBombTargetStateInfo bombTargetStateInfos[] = {
      {BOMB_TARGET_INACTIVE, "BOMB_TARGET_INACTIVE",
       &CDODBombTarget::State_Enter_INACTIVE, NULL, NULL, NULL},
      {BOMB_TARGET_ACTIVE, "BOMB_TARGET_ACTIVE",
       &CDODBombTarget::State_Enter_ACTIVE, NULL, NULL,
       &CDODBombTarget::State_Use_ACTIVE},
      {BOMB_TARGET_ARMED, "BOMB_TARGET_ARMED",
       &CDODBombTarget::State_Enter_ARMED, &CDODBombTarget::State_Leave_Armed,
...
void CDODBombTarget::State_Leave_Armed(void) { StopSound("Weapon_C4.Fuse"); }
...
void CDODBombTarget::Explode(void) {
  ...
  CTakeDamageInfo dmgInfo(this, m_pPlantingPlayer, vec3_origin, origin,
                          flDamage, DMG_BLAST | DMG_BOMB);
  DODGameRules()->RadiusDamage(dmgInfo, origin, flDmgRadius, CLASS_NONE, NULL,
                               true);

  // stun players in a radius
  const float flStunDamage = 100;

  CTakeDamageInfo stunInfo(this, this, vec3_origin, GetAbsOrigin(),
...
bool CDODBombTarget::CanPlayerStartDefuse(CDODPlayer *pPlayer) {
  ...
  if (flDist > DOD_BOMB_DEFUSE_MAXDIST) // PLAYER_USE_RADIUS is not actually
                                        // used by the playerUse code!!
  {
...
bool CDODBombTarget::CanPlayerContinueDefusing(CDODPlayer *pPlayer,
                                               DefusingPlayer *pDefuseRecord) {
  if (!pDefuseRecord ||
      pDefuseRecord->m_flDefuseTimeoutTime < gpGlobals->curtime) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CDODBombTarget::State_Use_ACTIVE(CBaseEntity *pActivator,
...
void CDODBombTarget::CompletePlanting(CDODPlayer *pPlantingPlayer) {
  if (pPlantingPlayer && (pPlantingPlayer->GetTeamNumber() == m_iBombingTeam ||
                          m_iBombingTeam == TEAM_UNASSIGNED)) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CDODBombTarget::State_Use_ARMED(CBaseEntity *pActivator,
                                     CBaseEntity *pCaller, USE_TYPE useType,
                                     float value) {
  // check for people disarming
  CDODPlayer *pPlayer = ToDODPlayer(pActivator);

  if (!pPlayer || pPlayer->GetTeamNumber() == m_iBombingTeam ||
  ...
  if (pDefusingPlayerRecord) {
    ...
    defusingPlayer.m_flDefuseCompleteTime =
        gpGlobals->curtime + DOD_BOMB_DEFUSE_TIME;
...
CControlPoint *CDODBombTarget::GetControlPoint(void) {
  if (!m_pControlPoint) {
    ...
    // try to find it
    m_pControlPoint = dynamic_cast<CControlPoint *>(
        gEntList.FindEntityByName(NULL, STRING(m_iszCapPointName)));

    if (!m_pControlPoint)
      Warning("Could not find dod_control_point named \"%s\" for "
              "dod_bomb_target \"%s\"\n",
...
bool CDODBombTarget::CanPlantHere(CDODPlayer *pPlayer) {
  ...
  if (pPlayer->GetTeamNumber() != m_iBombingTeam &&
      m_iBombingTeam != TEAM_UNASSIGNED)
    return false;
