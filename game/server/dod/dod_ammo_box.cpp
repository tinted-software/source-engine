...
DEFINE_THINKFUNC(FlyThink), DEFINE_ENTITYFUNC(BoxTouch), END_DATADESC();
...
CAmmoBox *CAmmoBox::Create(const Vector &vecOrigin, const QAngle &vecAngles,
                           CBaseEntity *pOwner, int team) {
  CAmmoBox *p = static_cast<CAmmoBox *>(
      CBaseAnimating::Create("dod_ammo_box", vecOrigin, vecAngles, pOwner));
...
void CAmmoBox::BoxTouch(CBaseEntity *pOther) {
  ...
  // Don't let the person who threw this ammo pick it up until it hits the
  // ground. This way we can throw ammo to people, but not touch it as soon as
  // we throw it ourselves
