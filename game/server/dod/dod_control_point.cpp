...
DEFINE_KEYFIELD(m_iszPrintName, FIELD_STRING, "point_printname"),
    ...
    DEFINE_KEYFIELD(m_iTimedPointsAllies, FIELD_INTEGER,
                    "point_timedpoints_allies"),
    DEFINE_KEYFIELD(m_iTimedPointsAxis, FIELD_INTEGER,
    ...
    DEFINE_OUTPUT(
        m_AlliesCapOutput,
        "OnAlliesCap"), // these are fired whenever the point changes modes
    DEFINE_OUTPUT(m_AxisCapOutput, "OnAxisCap"),
    DEFINE_OUTPUT(m_PointResetOutput, "OnCapReset"),

    DEFINE_OUTPUT(m_OwnerChangedToAllies,
                  "AlliesCapturePoint"), // these are fired when a team does the
                                         // work to change the owner
...
void CControlPoint::SetNumCappersRequired(int alliesRequired,
                                          int axisRequired) {
...
void CControlPoint::SetOwner(int owner, bool bMakeSound, int iNumCappers,
                             int *pCappingPlayers) {
  // Only allow cp caps when round is running ( but not when in warmup )
  if (DODGameRules()->State_Get() == STATE_RND_RUNNING &&
      !DODGameRules()->IsInWarmup()) {
...
int CControlPoint::GetTimerCapHudIcon(void) { return m_iTimerCapIcon; }

int CControlPoint::GetBombedHudIcon(void) { return m_iBombedIcon; }
...
void CControlPoint::InternalSetOwner(int owner, bool bMakeSound,
                                     int iNumCappers, int *pCappingPlayers) {
  m_iTeam = owner;
  ...
  if (bMakeSound) // make sure this is a real cap and not a reset
  {
    for (int i = 0; i < iNumCappers; i++) {
      ...
      if (pPlayer) {
        ...
        DODGameRules()->Stats_PlayerCap(pPlayer->GetTeamNumber(),
  ...
  if (bMakeSound && m_iTeam != TEAM_UNASSIGNED &&
      iNumCappers > 0) // dont print message if its a reset
  ...
  if ((DODGameRules()->State_Get() == STATE_ALLIES_WIN &&
       m_iTeam == TEAM_ALLIES) ||
    ...
    if (m_iBombsRequired == 0) {
      for (int i = 0; i < iNumCappers; i++) {
        CDODPlayer *pDODPlayer =
            ToDODPlayer(UTIL_PlayerByIndex(pCappingPlayers[i]));
...
void CControlPoint::SendCapString(int team, int iNumCappers,
                                  int *pCappingPlayers) {
...
void CControlPoint::CaptureBlocked(CDODPlayer *pPlayer) {
  ...
  DODGameRules()->Stats_PlayerDefended(pPlayer->GetTeamNumber(),
                                       pPlayer->m_Shared.DesiredPlayerClass());
}
...
//
// Values:
// point_name - the name of the point displayed in the capture string ( and
// ControlStatus command ) point_reset_time - time after which the point
// spontaneously resets - currently not used point_default_owner - the owner of
// this point at the start and after resets

// point_allies_capsound	|
// point_axis_capsound		|	the sounds that are made when the points
// are capped by each team point_resetsound			|
//
// point_allies_model		|
// point_axis_model			|	the models that the ent is set
// to after each team caps it point_reset_model		|

// point_team_points - number of team points to give to the capper's team for
// capping
...
bool CControlPoint::KeyValue(const char *szKeyName, const char *szValue) {
  if (FStrEq(szKeyName, "point_default_owner")) {
    ...
    Assert(m_iDefaultOwner == TEAM_ALLIES || m_iDefaultOwner == TEAM_AXIS ||
           m_iDefaultOwner == TEAM_UNASSIGNED);

    if (m_iDefaultOwner != TEAM_ALLIES && m_iDefaultOwner != TEAM_AXIS &&
        m_iDefaultOwner != TEAM_UNASSIGNED) {
      Warning(
...
void CControlPoint::BombPlanted(float flTimerLength,
                                CDODPlayer *pPlantingPlayer) {
  if (m_bBombPlanted == true) {
    Warning("ERROR - dod_control_point only supports one bomb being planted at "
            "once\n");
    return;
  }
  ...
  if (event) {
    int iOppositeTeam = (pPlantingPlayer->GetTeamNumber() == TEAM_ALLIES)
                            ? TEAM_AXIS
                            : TEAM_ALLIES;
...
void CControlPoint::BombExploded(CDODPlayer *pPlantingPlayer /* = NULL */,
                                 int iPlantingTeam /* = TEAM_UNASSIGNED */) {
  m_bBombPlanted = false;
