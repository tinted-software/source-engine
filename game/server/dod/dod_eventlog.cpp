...
ConVar mp_logdetail(
    "mp_logdetail", "0", FCVAR_NONE,
    "Logs attacks.  Values are: 0=off, 1=enemy, 2=teammate, 3=both)", true,
    0.0f, true, 3.0f);
    ...
    if (FStrEq(eventName, "player_death")) {
      ...
      if (pPlayer == pAttacker) {
        ...
        UTIL_LogPrintf(
            ...
            pAttacker->GetNetworkIDString(), pAttacker->GetTeam()->GetName(),
      ...
      // pAttacker //int attackerid = engine->GetPlayerForUserID( event->GetInt(
      // "attacker" ) ); pPlayer //int userid = engine->GetPlayerForUserID(
      // event->GetInt( "userid" ) );

      if (event->GetInt("dominated") > 0 && pAttacker) {
        UTIL_LogPrintf(
            "\"%s<%i><%s><%s>\" triggered \"domination\" against "
            "\"%s<%i><%s><%s>\"\n",
            pAttacker->GetPlayerName(), attackerid,
            pAttacker->GetNetworkIDString(), pAttacker->GetTeam()->GetName(),
            pPlayer->GetPlayerName(), userid, pPlayer->GetNetworkIDString(),
      ...
      if (event->GetInt("revenge") > 0 && pAttacker) {
        UTIL_LogPrintf(
            "\"%s<%i><%s><%s>\" triggered \"revenge\" against "
            "\"%s<%i><%s><%s>\"\n",
            pAttacker->GetPlayerName(), attackerid,
            pAttacker->GetNetworkIDString(), pAttacker->GetTeam()->GetName(),
            pPlayer->GetPlayerName(), userid, pPlayer->GetNetworkIDString(),
      ...
      if ((isTeamAttack && (detail & LOG_DETAIL_TEAMMATE_ATTACKS)) ||
        ...
        UTIL_LogPrintf(
            "\"%s<%i><%s><%s>\" attacked \"%s<%i><%s><%s>\" with \"%s\" "
            "(damage \"%d\") (health \"%d\") (hitgroup \"%s\")\n",
            pAttacker->GetPlayerName(), attackerid,
            pAttacker->GetNetworkIDString(), pAttacker->GetTeam()->GetName(),
            pPlayer->GetPlayerName(), userid, pPlayer->GetNetworkIDString(),
            pPlayer->GetTeam()->GetName(), weapon, event->GetInt("damage"),
            event->GetInt("health"), hitgroupStr);
      }
      return true;
      ...
      if (iClass == PLAYERCLASS_RANDOM) {
        ...
        const CDODPlayerClassInfo &pInfo =
            GetGlobalDODTeam(iTeam)->GetPlayerClassInfo(iClass);
      ...
      UTIL_LogPrintf("Team \"Allies\" triggered \"team_scores\" (roundswon "
                     "\"%d\") (tickpoints \"%d\") (numplayers \"%d\")\n",
      ...
      UTIL_LogPrintf("Team \"%s\" triggered \"round_win\" (rounds_won \"%d\") "
                     "(numplayers \"%d\")\n",
                     pTeam->GetName(), pTeam->GetRoundsWon(),
                     pTeam->GetNumPlayers());
      ...
      UTIL_LogPrintf("Team \"%s\" triggered \"tick_score\" (score \"%i\") "
                     "(totalscore \"%d\") (numplayers \"%d\")\n",
                     pTeam->GetName(), iScore, iTotalScore,
                     pTeam->GetNumPlayers());
      ...
      //    players (1 "Matt<UID><STEAMID><TEAM>") (2
      //    "Player2<2><STEAMID><TEAM>")
      ...
      for (int i = 0; i < iNumCappers; i++) {
        ...
        Assert(iPlayerIndex != '\0' && iPlayerIndex > 0 &&
        ...
        if (i == 0) {
          Q_snprintf(buf, sizeof(buf),
                     "Team \"%s\" triggered \"captured_loc\" (flagindex "
                     "\"%d\") (flagname \"%s\") (numplayers \"%d\") ",
      ...
      // "Matt<2><UNKNOWN><Allies>" triggered "capblock" (flagindex "2")
      // (flagname "#map_flag_2nd_axis")

      UTIL_LogPrintf("\"%s<%i><%s><%s>\" triggered \"capblock\" (flagindex "
                     "\"%d\") (flagname \"%s\")\n",
                     pPlayer->GetPlayerName(), pPlayer->GetUserID(),
                     pPlayer->GetNetworkIDString(),
                     pPlayer->GetTeam()->GetName(), iPointIndex, szPointName);
      ...
      // "Matt<2><UNKNOWN><Allies>" triggered "bomb_plant" (flagindex "2")
      // (flagname "#map_flag_2nd_axis")

      UTIL_LogPrintf("\"%s<%i><%s><%s>\" triggered \"bomb_plant\" (flagindex "
                     "\"%d\") (flagname \"%s\")\n",
                     pPlayer->GetPlayerName(), pPlayer->GetUserID(),
                     pPlayer->GetNetworkIDString(),
                     pPlayer->GetTeam()->GetName(), iPointIndex, szPointName);
      ...
      // "Matt<2><UNKNOWN><Allies>" triggered "bomb_defuse" (flagindex "2")
      // (flagname "#map_flag_2nd_axis")

      UTIL_LogPrintf("\"%s<%i><%s><%s>\" triggered \"bomb_defuse\" (flagindex "
                     "\"%d\") (flagname \"%s\")\n",
                     pPlayer->GetPlayerName(), pPlayer->GetUserID(),
                     pPlayer->GetNetworkIDString(),
                     pPlayer->GetTeam()->GetName(), iPointIndex, szPointName);
      ...
      // "Matt<2><UNKNOWN><Allies>" triggered "kill_planter" against
      // "Fred<3><UNKNOWN><Axis>"

      UTIL_LogPrintf("\"%s<%i><%s><%s>\" triggered \"kill_planter\" against "
                     "\"%s<%i><%s><%s>\"\n",
                     pPlayer->GetPlayerName(), pPlayer->GetUserID(),
                     pPlayer->GetNetworkIDString(),
                     pPlayer->GetTeam()->GetName(), pVictim->GetPlayerName(),
                     pVictim->GetUserID(), pVictim->GetNetworkIDString(),
      ...
      // "Matt<2><UNKNOWN><Allies>" triggered "kill_defuser" against
      // "Fred<3><UNKNOWN><Axis>"

      UTIL_LogPrintf("\"%s<%i><%s><%s>\" triggered \"kill_defuser\" against "
                     "\"%s<%i><%s><%s>\"\n",
                     pPlayer->GetPlayerName(), pPlayer->GetUserID(),
                     pPlayer->GetNetworkIDString(),
                     pPlayer->GetTeam()->GetName(), pVictim->GetPlayerName(),
...
// Singleton access
//-----------------------------------------------------------------------------
IGameSystem *GameLogSystem() { return &g_DODEventLog; }
