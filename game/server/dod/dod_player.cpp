...
#pragma warning(disable : 4355) // disables ' 'this' : used in base member
                                // initializer list'
...
ConVar sv_motd_unload_on_dismissal("sv_motd_unload_on_dismissal", "0", 0,
                                   "If enabled, the MOTD contents will be "
                                   "unloaded when the player closes the MOTD.");
...
ConVar dod_playerstatetransitions("dod_playerstatetransitions", "-2",
                                  FCVAR_CHEAT,
                                  "dod_playerstatetransitions <ent index or -1 "
                                  "for all>. Show player state transitions.");
ConVar dod_debugdamage("dod_debugdamage", "0", FCVAR_CHEAT);

ConVar mp_bandage_heal_amount(
    "mp_bandage_heal_amount", "40", FCVAR_GAMEDLL,
...
SendPropInt(SENDINFO(m_iPlayerClass), 4),
    ...
    BEGIN_SEND_TABLE_NOBASE(CDODPlayerShared, DT_DODPlayerShared)
        SendPropFloat(SENDINFO(m_flStamina), 0,
                      SPROP_NOSCALE | SPROP_CHANGES_OFTEN),
    SendPropTime(SENDINFO(m_flSlowedUntilTime)),
    SendPropBool(SENDINFO(m_bProne)), SendPropBool(SENDINFO(m_bIsSprinting)),
    ...
    SendPropBool(SENDINFO(m_bPlanting)), SendPropBool(SENDINFO(m_bDefusing)),
    SendPropDataTable("dodsharedlocaldata", 0,
                      &REFERENCE_SEND_TABLE(DT_DODSharedLocalPlayerExclusive),
...
SendPropVector(SENDINFO(m_vecOrigin), -1, SPROP_NOSCALE | SPROP_CHANGES_OFTEN,
    ...
    // dod_playeranimstate and clientside animation takes care of these on the
    // client
    ...
    SendPropDataTable(SENDINFO_DT(m_Shared),
                      &REFERENCE_SEND_TABLE(DT_DODPlayerShared)),

    // Data that only gets sent to the local player.
    SendPropDataTable("dodlocaldata", 0,
                      &REFERENCE_SEND_TABLE(DT_DODLocalPlayerExclusive),
                      SendProxy_SendLocalDataTable),
    SendPropDataTable("dodnonlocaldata", 0,
                      &REFERENCE_SEND_TABLE(DT_DODNonLocalPlayerExclusive),
                      SendProxy_SendNonLocalDataTable),

    SendPropAngle(SENDINFO_VECTORELEM(m_angEyeAngles, 0), 13,
                  SPROP_CHANGES_OFTEN),
    SendPropAngle(SENDINFO_VECTORELEM(m_angEyeAngles, 1), 13,
                  SPROP_CHANGES_OFTEN),
    SendPropEHandle(SENDINFO(m_hRagdoll)),
    SendPropBool(SENDINFO(m_bSpawnInterpCounter)),
    SendPropInt(SENDINFO(m_iAchievementAwardsMask), NUM_ACHIEVEMENT_AWARDS,
                SPROP_UNSIGNED),
    END_SEND_TABLE()

        BEGIN_DATADESC(CDODPlayer) DEFINE_THINKFUNC(PushawayThink),
    DEFINE_THINKFUNC(DeafenThink) END_DATADESC()
...
#endif
{
  ...
  memset(&m_flTimePlayedPerClass_Allies, 0,
         sizeof(m_flTimePlayedPerClass_Allies));
...
void CDODPlayer::Precache() {
  ...
  //"materials/models/player/american/american_body.vmt" );
  //	engine->ForceSimpleMaterial(
  //"materials/models/player/american/american_gear.vmt" );
  //	engine->ForceSimpleMaterial(
  //"materials/models/player/german/german_body.vmt" );
  //	engine->ForceSimpleMaterial(
  //"materials/models/player/german/german_gear.vmt" );
...
//-----------------------------------------------------------------------------
void CDODPlayer::RunPlayerMove(const QAngle &viewangles, float forwardmove,
...
void CDODPlayer::Spawn() {
  ...
  SetContextThink(&CDODPlayer::PushawayThink,
  ...
  m_StatProperty.SetClassAndTeamForThisLife(m_Shared.PlayerClass(),
                                            GetTeamNumber());
}
...
void CDODPlayer::Event_Killed(const CTakeDamageInfo &info) {
  ...
  CDODPlayer *pScorer =
      ToDODPlayer(DODGameRules()->GetDeathScorer(pKiller, pInflictor));
  ...
  if (GetDeployedKillStreak() >= ACHIEVEMENT_MG_STREAK_IS_DOMINATING) {
    if (pScorer && pScorer->GetTeamNumber() != GetTeamNumber() &&
        DODGameRules()->State_Get() == STATE_RND_RUNNING) {
  ...
  if (DODGameRules()->GetPresentDropChance() >= RandomFloat()) {
    ...
    CHolidayGift::Create(WorldSpaceCenter(), GetAbsAngles(), EyeAngles(),
                         GetAbsVelocity(), this);
  }
  ...
  if (pWpn && pWpn->GetDODWpnData().m_WeaponType == WPN_TYPE_GRENADE) {
    CWeaponDODBaseGrenade *pGrenade =
        dynamic_cast<CWeaponDODBaseGrenade *>(pWpn);
  ...
  if (pScorer && pScorer != this &&
      pScorer->GetTeamNumber() != GetTeamNumber()) {
    ...
    if (m_bIsDefusing && m_pDefuseTarget &&
        pScorer->GetTeamNumber() != GetTeamNumber()) {
      // Re-enable if we want to give a defend point to someone who kills a
      // defuser
  ...
  if (pAttacker && pAttacker->IsPlayer()) {
    ...
    IGameEvent *event =
        gameeventmanager->CreateEvent("dod_stats_player_killed");
    if (event) {
...
bool CDODPlayer::ShouldInstantRespawn(void) {
  CUtlVector<EHANDLE> *pSpawnPoints =
      DODGameRules()->GetSpawnPointListForTeam(GetTeamNumber());
...
// Purpose:
//-----------------------------------------------------------------------------
void CDODPlayer::InitVCollision(const Vector &vecAbsOrigin,
...
void CDODPlayer::SetupVisibility(CBaseEntity *pViewEntity, unsigned char *pvs,
                                 int pvssize) {
...
void CDODPlayer::VoiceCommand(int iVoiceCommand) {
  ...
  char szSound[128];
  Q_snprintf(szSound, sizeof(szSound), "Voice.%s_%s", pszCountry,
             g_VoiceCommands[iVoiceCommand].pszSoundName);
...
void CDODPlayer::DropGenericAmmo(void) {
  ...
  CAmmoBox *pAmmo =
      CAmmoBox::Create(WorldSpaceCenter(), vec3_angle, this, GetTeamNumber());
...
ConVar dod_friendlyfiresafezone("dod_friendlyfiresafezone", "100",
                                FCVAR_ARCHIVE,
                                "Units around a player where they will not "
                                "damage teammates, even if FF is on",
                                true, 0, false, 0);

void CDODPlayer::TraceAttack(const CTakeDamageInfo &info, const Vector &vecDir,
  ...
  if (pAttacker && (GetTeamNumber() == pAttacker->GetTeamNumber())) {
    ...
    if (bFriendlyFire &&
        (info.GetDamageType() & (DMG_SLASH | DMG_BULLET | DMG_CLUB))) {
  ...
  if (bTakeDamage) {
    if (dod_debugdamage.GetInt()) {
      ...
      Q_snprintf(buf, sizeof(buf),
                 "%s hit %s in the %s hitgroup for %f damage ( %f base damage "
                 ") ( %s now has %d health )\n",
                 pAttacker->GetPlayerName(), GetPlayerName(), szHitbox,
                 flDamage, flOriginalDmg, GetPlayerName(),
    ...
    // Since this code only runs on the server, make sure it shows the tempents
    // it creates.
...
void CDODPlayer::PreThink() {
  ...
  // Reset bullet force accumulator, only lasts one frame, for ragdoll forces
  // from multiple shots
...
bool CDODPlayer::DODWeaponDrop(CBaseCombatWeapon *pWeapon, bool bThrowForward) {
  ...
  if (pWeapon) {
    ...
    if (iBIndex != -1) {
      ...
      // not exactly correct bounds, we haven't rotated them to match the
      // attachment
      pWeapon->CollisionProp()->WorldSpaceSurroundingBounds(&mins, &maxs);

      UTIL_TraceHull(WorldSpaceCenter(), origin, mins, maxs, MASK_SOLID, this,
...
void CDODPlayer::PopHelmet(Vector vecDir, Vector vecForceOrigin) {
  ...
  const CDODPlayerClassInfo &pClassInfo =
      pTeam->GetPlayerClassInfo(iPlayerClass);
...
bool CDODPlayer::BumpWeapon(CBaseCombatWeapon *pBaseWeapon) {
  ...
  if( pOwnedWeapon != NULL && ( pWeapon->GetWpnData().iFlags &
...
bool CDODPlayer::HandleCommand_JoinClass(int iClass) {
  ...
  const char *classname =
      DODGameRules()->GetPlayerClassName(iClass, GetTeamNumber());

  if (DODGameRules()->CanPlayerJoinClass(this, iClass)) {
    m_Shared.SetDesiredPlayerClass(
        iClass); // real class value is set when the player spawns
    ...
    if (ShouldInstantRespawn()) {
      ...
      // if we dropped our primary weapon, and noone else picked it up, destroy
      // it
      CBaseCombatWeapon *pWeapon =
          dynamic_cast<CBaseCombatWeapon *>(m_hLastDroppedWeapon.Get());
      if (pWeapon) {
        if (!pWeapon->GetOwner()) {
...
int CDODPlayer::GetPlayerClassAsString(char *pDest, int iDestSize) {
  ...
  if (team == TEAM_ALLIES || team == TEAM_AXIS) {
    const char *pszClassName = DODGameRules()->GetPlayerClassName(
        m_Shared.PlayerClass(), GetTeamNumber());
...
void CDODPlayer::CheckChatText(char *p, int bufsize) {
  ...
  for (char *pSrc = p; pSrc != NULL && *pSrc != 0 && pos < bufsize - 1;
    ...
    if (*pSrc == '%') {
      ...
      int iResult =
          GetStringForEscapeSequence(*(pSrc + 1), szSubst, sizeof(szSubst));
...
bool CDODPlayer::ClientCommand(const CCommand &args) {
  ...
  if (FStrEq(pcmd, "jointeam")) {
    ...
    trace_t tr;
    UTIL_TraceLine(EyePosition(), EyePosition() + vecForward * 1000, MASK_SOLID,
                   NULL, &tr);
    ...
    if (tr.fraction < 1.0) {
      CTakeDamageInfo info(this, this, vec3_origin, tr.endpos, flStunAmount,
                           DMG_STUN);
...
bool CDODPlayer::HandleCommand_JoinTeam(int team) {
  ...
  if (mp->TeamStacked(
          team, GetTeamNumber())) // players are allowed to change to their own
                                  // team so they can just change their model
  {
...
void CDODPlayer::State_Enter(DODPlayerState newState) {
  ...
  if (dod_playerstatetransitions.GetInt() == -1 ||
      dod_playerstatetransitions.GetInt() == entindex()) {
    if (m_pCurStateInfo)
...
CDODPlayerStateInfo *CDODPlayer::State_LookupInfo(DODPlayerState state) {
  ...
  static CDODPlayerStateInfo playerStateInfos[] = {
      {STATE_ACTIVE, "STATE_ACTIVE", &CDODPlayer::State_Enter_ACTIVE, NULL,
       &CDODPlayer::State_PreThink_ACTIVE},
      {STATE_WELCOME, "STATE_WELCOME", &CDODPlayer::State_Enter_WELCOME, NULL,
       &CDODPlayer::State_PreThink_WELCOME},
      {STATE_PICKINGTEAM, "STATE_PICKINGTEAM",
       &CDODPlayer::State_Enter_PICKINGTEAM, NULL,
       &CDODPlayer::State_PreThink_PICKING},
      {STATE_PICKINGCLASS, "STATE_PICKINGCLASS",
       &CDODPlayer::State_Enter_PICKINGCLASS, NULL,
       &CDODPlayer::State_PreThink_PICKING},
      {STATE_DEATH_ANIM, "STATE_DEATH_ANIM",
       &CDODPlayer::State_Enter_DEATH_ANIM, NULL,
       &CDODPlayer::State_PreThink_DEATH_ANIM},
      {STATE_OBSERVER_MODE, "STATE_OBSERVER_MODE",
       &CDODPlayer::State_Enter_OBSERVER_MODE, NULL,
...
void CDODPlayer::State_Enter_WELCOME() {
  ...
  // Show info panel
  if (IsBot()) {
    // If they want to auto join a team for debugging, pretend they clicked the
    // button.
    CCommand args;
    ...
    const char *title =
        (hostname) ? hostname->GetString() : "MESSAGE OF THE DAY";
    ...
    data->SetInt(
        "cmd", TEXTWINDOW_CMD_CHANGETEAM); // exec this command if panel closed
...
void CDODPlayer::State_PreThink_DEATH_ANIM() {
  ...
  if (dod_freezecam.GetBool()) {
    ...
    float flFreezeEnd =
        (m_flDeathTime + DOD_DEATH_ANIMATION_TIME + DOD_FREEZECAM_LENGTH);

    if (!m_bPlayedFreezeCamSound && GetObserverTarget() &&
        GetObserverTarget() != this) {
      // Start the sound so that it ends at the freezecam lock on time
      float flFreezeSoundLength = 0.3;
      float flFreezeSoundTime = m_flDeathTime + DOD_DEATH_ANIMATION_TIME -
                                flFreezeSoundLength + 0.4f /* travel time */;
    ...
    if (gpGlobals->curtime >=
        (m_flDeathTime + DOD_DEATH_ANIMATION_TIME)) // allow 2 seconds death
                                                    // animation / death cam
    {
      ...
      // Don't allow anyone to respawn until freeze time is over, even if
      // they're not in freezecam. This prevents players skipping freezecam to
      // spawn faster.
    ...
    if (gpGlobals->curtime >=
        (m_flDeathTime +
         DEATH_CAM_TIME)) // allow x seconds death animation / death cam
    {
      m_lifeState = LIFE_DEAD;
...
void CDODPlayer::AttemptToExitFreezeCam(void) {
  float flFreezeTravelTime = (m_flDeathTime + DOD_DEATH_ANIMATION_TIME) +
                             0.4 /*spec_freeze_traveltime.GetFloat()*/ + 0.5;
...
void CDODPlayer::MoveToNextIntroCamera() {
  ...
  if (!m_pIntroCamera) // if there are no cameras find a spawn point and black
                       // out the screen
  {
...
CBaseEntity *CDODPlayer::SelectSpawnSpot(CUtlVector<EHANDLE> *pSpawnPoints,
                                         int &iLastSpawnIndex) {
  Assert(pSpawnPoints);
  ...
  for (int i = 0; i < iNumSpawns; i++) {
    ...
    // if we find a clear spot, create a new spawn point there, copied from
    // pSpot
  ...
  // if we get here, we're really screwed. Spawning the person is going to stick
  // them into someone or something, but we really tried hard, I swear.
...
CBaseEntity *CDODPlayer::EntSelectSpawnPoint() {
  ...
  case TEAM_ALLIES: {
    CUtlVector<EHANDLE> *pSpawnList =
        DODGameRules()->GetSpawnPointListForTeam(TEAM_ALLIES);
    pSpot = SelectSpawnSpot(pSpawnList, g_iLastAlliesSpawnIndex);
  } break;
  case TEAM_AXIS: {
    CUtlVector<EHANDLE> *pSpawnList =
        DODGameRules()->GetSpawnPointListForTeam(TEAM_AXIS);
...
bool CDODPlayer::IsReadyToPlay(void) {
  bool bResult =
      ((GetTeamNumber() == TEAM_ALLIES || GetTeamNumber() == TEAM_AXIS) &&
...
int CDODPlayer::OnTakeDamage_Alive(const CTakeDamageInfo &info) {
  ...
  if (event) {
    ...
    if (attacker->IsPlayer()) {
      ...
      if (pDODPlayer->GetTeamNumber() == GetTeamNumber() &&
          pDODPlayer != this && !FBitSet(info.GetDamageType(), DMG_BOMB)) {
    ...
    if (strncmp(weaponName, "weapon_", 7) == 0) {
      ...
      CDODBaseRocket *pRocket =
          dynamic_cast<CDODBaseRocket *>(info.GetInflictor());
      if (pRocket) {
        weaponID = pRocket->GetEmitterWeaponID();
      }
      ...
      CDODBaseGrenade *pGren =
          dynamic_cast<CDODBaseGrenade *>(info.GetInflictor());
      if (pGren) {
        weaponID = pGren->GetEmitterWeaponID();
      }
...
#ifndef CLIENT_DLL
    IGameEvent *stats_event =
        gameeventmanager->CreateEvent("dod_stats_player_damage");
    if (stats_event && attacker->IsPlayer() && weaponID != WEAPON_NONE) {
      ...
      // damage_given is the amount of damage applied, not to exceed how much
      // life we have
...
int CDODPlayer::OnTakeDamage(const CTakeDamageInfo &inputInfo) {
  ...
  if (bFriendlyFire || info.GetAttacker()->GetTeamNumber() != GetTeamNumber() ||
...
void CDODPlayer::OnDamagedByExplosion(const CTakeDamageInfo &info) {
  if (info.GetDamage() >= 30.0f) {
    SetContextThink(&CDODPlayer::DeafenThink, gpGlobals->curtime + 0.3,
                    DOD_DEAFEN_CONTEXT);
...
void CDODPlayer::OnDamageByStun(const CTakeDamageInfo &info) {
  ...
  m_flStunDuration = 0.0; // mark it as dirty so it transmits, incase we get the
                          // same value twice
  m_flStunDuration = 4.0f * flPercent;
  m_flStunMaxAlpha = 255;

  float flPitch = flPercent *
                  RandomFloat(dod_stun_min_pitch.GetFloat(),
                              dod_stun_max_pitch.GetFloat()) *
                  ((RandomInt(0, 1)) ? 1 : -1);

  float flYaw =
      flPercent *
      RandomFloat(dod_stun_min_yaw.GetFloat(), dod_stun_max_yaw.GetFloat()) *
      ((RandomInt(0, 1)) ? 1 : -1);

  float flRoll =
      flPercent *
      RandomFloat(dod_stun_min_roll.GetFloat(), dod_stun_max_roll.GetFloat()) *
      ((RandomInt(0, 1)) ? 1 : -1);
...
void CDODPlayer::RecordDamageGiven(CDODPlayer *pVictim, int iDamageGiven) {
  ...
  CDamageRecord *record = new CDamageRecord(pVictim->GetPlayerName(),
                                            pVictim->GetLifeID(), iDamageGiven);
...
void CDODPlayer::RecordDamageTaken(CDODPlayer *pAttacker, int iDamageTaken) {
  ...
  CDamageRecord *record = new CDamageRecord(
      pAttacker->GetPlayerName(), pAttacker->GetLifeID(), iDamageTaken);
  ...
  FOR_EACH_LL(m_DamageTakenList, i) {
    ...
    if (pRecord) {
      Q_snprintf(buf, sizeof(buf), "( life ID %i ) - %d in %d %s",
                 pRecord->GetLifeID(), pRecord->GetDamage(),
                 pRecord->GetNumHits(),
...
bool CDODPlayer::CanHearChatFrom(CBasePlayer *pPlayer) {
  ...
  if (DODGameRules()->IsInBonusRound() ||
      DODGameRules()->State_Get() == STATE_GAME_OVER)
    return true;
...
void CDODPlayer::CommitSuicide(bool bExplode /*= false*/,
                               bool bForce /*= false*/) {
  CommitSuicide(vec3_origin, false, bForce);
}

void CDODPlayer::CommitSuicide(const Vector &vecForce,
                               bool bExplode /*= false*/,
                               bool bForce /*= false*/) {
...
void CDODPlayer::PickUpWeapon(CWeaponDODBase *pWeapon) {
  ...
  CWeaponDODBase *pCurrentPrimaryWpn =
      (CWeaponDODBase *)Weapon_GetSlot(WPN_SLOT_PRIMARY);
...
//-----------------------------------------------------------------------------
// Purpose: like regular FindUseEntity, but picks up grenades before other
// things
//-----------------------------------------------------------------------------
CBaseEntity *CDODPlayer::FindUseEntity() {
  ...
  UTIL_TraceLine(searchCenter, searchCenter + forward * 1024, useableContents,
  ...
  while (!IsUseableEntity(pObject, 0) && count < NUM_TANGENTS) {
    ...
    //							45 deg, 30 deg, 20 deg,
    //15 deg, 10 deg, -10, -15
    const float tangents[NUM_TANGENTS] = {1,
                                          0.57735026919f,
                                          0.3639702342f,
                                          0.267949192431f,
                                          0.1763269807f,
    ...
    UTIL_TraceHull(searchCenter, searchCenter + down * 72, -Vector(16, 16, 16),
                   Vector(16, 16, 16), useableContents, this,
                   COLLISION_GROUP_NONE, &tr);
    pObject = tr.m_pEnt;
    count++;
  }
  ...
  for (CEntitySphereQuery sphere(searchCenter, PLAYER_USE_RADIUS);
    ...
    if ((iPriority > iHighestPriority) ||
        (iPriority == iHighestPriority && dot > nearestDot)) {
  ...
  // Special check for bomb targets, whose radius for +use is larger than other
  // entities
  if (pNearest == NULL) {
    CBaseEntity *pEnt = NULL;
    float flBestDist = FLT_MAX;

    // If we didn't find anything, check to see if the bomb target is close
    // enough to use. This is done separately since there might be something
    // blocking our LOS to it but we might want to use it anyway if it's close
    // enough.

    while ((pEnt = gEntList.FindEntityByClassname(pEnt, "dod_bomb_target")) !=
           NULL) {
      ...
      // if we are looking directly at a bomb target and it is within our
      // radius, that automatically wins
...
void CDODPlayer::PrintLifetimeStats(void) {
  ...
  Msg("                  (shots) (hits) (damage) (avg.dist) (kills) (hits "
      "taken) (dmg taken) (times killed) (accuracy)\n");
  // ( "* thompson     4     3       110      89.8      1         1            0
  // 0           75.0
  // Msg( "*%9s        %2i    %2i     %3i     %5.1f     %2i        %2i %3i %2i
  // %3.1f\n",

  for (i = 0; i < WEAPON_MAX; i++) {
    if (m_WeaponStats[i].m_iNumShotsTaken > 0) {
      Msg("* %10s (%2i) %6i %6i %8i  %9.1f %7i    %9i   %9i      %9i   %9.1f\n",
          WeaponIDToAlias(i), i, m_WeaponStats[i].m_iNumShotsTaken,
          m_WeaponStats[i].m_iNumShotsHit, m_WeaponStats[i].m_iTotalDamageGiven,
          m_WeaponStats[i].m_flAverageHitDistance, m_WeaponStats[i].m_iNumKills,
          m_WeaponStats[i].m_iNumHitsTaken,
          m_WeaponStats[i].m_iTotalDamageTaken, m_WeaponStats[i].m_iTimesKilled,
          100.0 * ((float)m_WeaponStats[i].m_iNumShotsHit /
                   (float)m_WeaponStats[i].m_iNumShotsTaken));
    }
  }
  ...
  for (i = 0; i < m_KilledPlayers.Count(); i++) {
    ...
    const char *pName = pPlayer ? pPlayer->GetPlayerName()
                                : m_KilledByPlayers[i].m_szPlayerName;

    Msg("* Killed Player '%s' %i times ( %i damage )\n", pName,
        m_KilledPlayers[i].m_iKills, m_KilledPlayers[i].m_iTotalDamage);
  }
  ...
  for (i = 0; i < m_KilledByPlayers.Count(); i++) {
    ...
    Msg("* Player '%s' killed you %i times ( %i damage )\n", pName,
        m_KilledByPlayers[i].m_iKills, m_KilledByPlayers[i].m_iTotalDamage);
  }
  ...
  /*
  m_flTimePlayedPerClass[playerclass] += ( gpGlobals->curtime -
  m_flLastClassChangeTime ); m_flLastClassChangeTime = gpGlobals->curtime;
...
void CDODPlayer::Stats_WeaponHit(CDODPlayer *pVictim, int weaponID, int iDamage,
                                 int iDamageGiven, int hitgroup,
                                 float flHitDistance) {
  // distance
  float flTotalHitDistance = m_WeaponStats[weaponID].m_flAverageHitDistance *
                             m_WeaponStats[weaponID].m_iNumShotsHit;
  flTotalHitDistance += flHitDistance;
  m_WeaponStats[weaponID].m_flAverageHitDistance =
      flTotalHitDistance / (m_WeaponStats[weaponID].m_iNumShotsHit + 1);
  ...
  if (lookup == m_KilledPlayers.InvalidIndex()) {
    // make a new one
    playerstat_t p;
    Q_strncpy(p.m_szPlayerName, pVictim->GetPlayerName(),
...
void CDODPlayer::Stats_HitByWeapon(CDODPlayer *pAttacker, int weaponID,
                                   int iDamage, int iDamageGiven,
                                   int hitgroup) {
  // damage
  ...
  if (lookup == m_KilledByPlayers.InvalidIndex()) {
    // make a new one
    playerstat_t p;
    Q_strncpy(p.m_szPlayerName, pAttacker->GetPlayerName(),
              MAX_PLAYER_NAME_LENGTH);
    p.m_iUserID = userID;
    p.m_iKills = 0;
    p.m_iTotalDamage = iDamageGiven;
...
bool CDODPlayer::WantsLagCompensationOnEntity(
...
void CDODPlayer::TallyLatestTimePlayedPerClass(int iOldTeam,
                                               int iOldPlayerClass) {
  // Tally the time we spent in the last class, for stats purposes
  if (iOldPlayerClass != PLAYERCLASS_UNDEFINED &&
      (iOldTeam == TEAM_ALLIES || iOldTeam == TEAM_AXIS)) {
    ...
    if (iOldTeam == TEAM_ALLIES) {
      m_flTimePlayedPerClass_Allies[iOldPlayerClass] +=
          (gpGlobals->curtime - m_flLastClassChangeTime);
    } else if (iOldTeam == TEAM_AXIS) {
      m_flTimePlayedPerClass_Axis[iOldPlayerClass] +=
          (gpGlobals->curtime - m_flLastClassChangeTime);
    }
  }
...
// The capture attempt number that was taking place the last time we blocked a
// capture
...
// NOTE! This is unused, unless we reenable our collision bounds to use
// USE_GAME_CODE
// - This extends our trigger bounds out a long ways and breaks many things, so
// only do this if you know what you're doing.
void CDODPlayer::ComputeWorldSpaceSurroundingBox(Vector *pVecWorldMins,
...
void CDODPlayer::HandleDeployedMGKillCount(int iNumDeployedKills) {
  if (iNumDeployedKills <= 0) {
    ...
    int iKillsFromPos =
        GetPerLifeCounterKV("deployed_mg_kills") + iNumDeployedKills;
...
void CDODPlayer::HandleComboWeaponKill(int iWeaponType) {
  ...
  const int iRequiredMask =
      (WPN_TYPE_MG | WPN_TYPE_SNIPER | WPN_TYPE_RIFLE | WPN_TYPE_SUBMG);
...
void CDODPlayer::RecalculateAchievementAwardsMask(void) {
  ...
  if (steamgameserverapicontext->SteamGameServerStats()) {
    ...
    if (GetSteamID(&steamIDForPlayer) &&
        steamIDForPlayer.BIndividualAccount()) {
      steamgameserverapicontext->SteamGameServerStats()->RequestUserStats(
          steamIDForPlayer);
    }
  }
#endif
  ...
  if (pResponse->m_steamIDUser == GetSteamIDAsUInt64()) {
    for (int i = 1; i < NUM_ACHIEVEMENT_AWARDS; i++) {
      ...
      if (steamgameserverapicontext->SteamGameServerStats()->GetUserAchievement(
              pResponse->m_steamIDUser, g_pszAchievementAwards[i],
              &bUnlocked)) {
        if (bUnlocked) {
...
void CDODPlayerStatProperty::SetClassAndTeamForThisLife(int iPlayerClass,
                                                        int iTeam) {
  m_bRecordingStats =
      ((iTeam == TEAM_ALLIES || iTeam == TEAM_AXIS) &&
       (iPlayerClass >= 0 && iPlayerClass <= NUM_DOD_PLAYERCLASSES));

  m_iCurrentLifePlayerClass = iPlayerClass;
  m_iCurrentLifePlayerTeam = iTeam;
}

void CDODPlayerStatProperty::IncrementPlayerClassStat(DODStatType_t statType,
                                                      int iValue /* = 1 */) {
...
void CDODPlayerStatProperty::IncrementWeaponStat(DODWeaponID iWeaponID,
                                                 DODStatType_t statType,
                                                 int iValue /* = 1 */) {
  if (!m_bRecordingStats)
    return;
...
void CDODPlayerStatProperty::SendStatsToPlayer(CDODPlayer *pPlayer) {
  ...
  // make a bit field of all the stats we want to send (all with non-zero
  // values)
  int iStat;
  ...
  WRITE_LONG(iSendPlayerBits); // write the bit mask of which stats follow in
                               // the message
