...
public:
  ...
  void SetOwner(int team, bool bMakeSound = true, int iNumCappers = 0,
  ...
  void BombExploded(CDODPlayer *pPlantingPlayer = NULL,
...
private:
  void InternalSetOwner(int team, bool bMakeSound = true, int iNumCappers = 0,
                        int *iCappingPlayers = NULL);
  ...
  int m_iAxisRequired;   // these hold the number of cappers required. Used to
                         // calc point value
  ...
  bool m_bSetOwnerIsBombPlant; // temp flag to indicate if the set owner we're
                               // doing is the result of a bomb
