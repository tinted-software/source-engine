...
// ====================
// One ControlPointMaster is spawned per level. Shortly after spawning it
// detects all the Control points in the map and puts them into the
...
private:
  ...
  int TeamOwnsAllPoints(CControlPoint *pOverridePoint = NULL,
  ...
  bool m_bFoundPoints; // true when the control points have been found and the
                       // array is initialized

  float m_fGivePointsTime; // the time at which we give points for holding
                           // control points
...
public:
  ...
  void GiveRemainingPoints(void) { GivePoints(m_iRemainingPoints); }
  ...
  int GetPointTeam(void) { return m_iPointTeam; }
