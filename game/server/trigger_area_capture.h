...
// another entity
//			Triggers are fired on start of capture, on end of
//capture and on broken capture 			Can either be capped by both teams at once, or
//just by one 			Time to capture and number of people required to capture are both
//passed by the mapper
//-----------------------------------------------------------------------------
...
public:
  ...
  void ForceOwner(int team); // by the control_point_round to force an owner of
                             // this point (so we can play a specific round)
...
private:
  ...
  struct perteamdata_t {
    ...
    int iNumTouching;
    int iBlockedTouching; // Number of capping players on the cap while it's
                          // being blocked
    bool bCanCap;
    int iSpawnAdjust;
