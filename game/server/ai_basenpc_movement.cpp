...
// Output :
//-----------------------------------------------------------------------------
void CAI_BaseNPC::ForceSelectedGo(CBaseEntity *pPlayer, const Vector &targetPos,
  ...
  while (npc) {
    if (npc->m_debugOverlays & OVERLAY_NPC_SELECTED_BIT) {
      ...
      AI_TraceHull(chasePosition, vUpBit, npc->GetHullMins(),
                   npc->GetHullMaxs(), MASK_NPCSOLID, npc, COLLISION_GROUP_NONE,
                   &tr);
      if (tr.startsolid || tr.fraction != 1.0) {
        NDebugOverlay::BoxAngles(chasePosition, npc->GetHullMins(),
                                 npc->GetHullMaxs(), npc->GetAbsAngles(), 255,
                                 0, 0, 20, 0.5);
      }
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CAI_BaseNPC::ScheduledMoveToGoalEntity(int scheduleType,
                                            CBaseEntity *pGoalEntity,
                                            Activity movementActivity) {
  if (m_NPCState == NPC_STATE_NONE) {
    // More than likely being grabbed before first think. Set ideal state to
    // prevent schedule stomp
  ...
  // UNDONE: Remove this and have NPCs that need this functionality fix up paths
  // in the movement system instead of when they are specified.
  AI_NavGoal_t goal(pGoalEntity->GetAbsOrigin(), movementActivity,
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CAI_BaseNPC::ScheduledFollowPath(int scheduleType, CBaseEntity *pPathStart,
                                      Activity movementActivity) {
  if (m_NPCState == NPC_STATE_NONE) {
    // More than likely being grabbed before first think. Set ideal state to
    // prevent schedule stomp
  ...
  // HACKHACK: Call through TranslateNavGoal to fixup this goal position
  AI_NavGoal_t goal(GOALTYPE_PATHCORNER, pPathStart->GetLocalOrigin(),
                    movementActivity, AIN_DEF_TOLERANCE, AIN_YAW_TO_DEST);
...
// Output :
//-----------------------------------------------------------------------------
bool CAI_BaseNPC::IsMoving(void) { return GetNavigator()->IsGoalSet(); }
...
//-----------------------------------------------------------------------------
// Purpose: Used to specify that the NPC has a reason not to use the a
// navigation node Input  : Output :
//-----------------------------------------------------------------------------
bool CAI_BaseNPC::IsUnusableNode(int iNodeID, CAI_Hint *pHint) {
  if (m_bHintGroupNavLimiting && m_strHintGroup != NULL_STRING &&
      STRING(m_strHintGroup)[0] != 0) {
...
bool CAI_BaseNPC::IsJumpLegal(const Vector &startPos, const Vector &apex,
...
bool CAI_BaseNPC::IsJumpLegal(const Vector &startPos, const Vector &apex,
  ...
  return IsJumpLegal(startPos, apex, endPos, MAX_JUMP_RISE, MAX_JUMP_DROP,
                     MAX_JUMP_DISTANCE);
}
...
// Output :
//-----------------------------------------------------------------------------
Vector CAI_BaseNPC::CalcThrowVelocity(const Vector &startPos,
...
// Output :
//-----------------------------------------------------------------------------
bool CAI_BaseNPC::AutoMovement(CBaseEntity *pTarget,
...
//-----------------------------------------------------------------------------
bool CAI_BaseNPC::AutoMovement(float flInterval, CBaseEntity *pTarget,
                               AIMoveTrace_t *pTraceResult) {
  bool ignored;
  Vector newPos;
  QAngle newAngles;
  ...
  if (GetIntervalMovement(flInterval, ignored, newPos, newAngles)) {
    ...
    // GetLocalOrigin()).Length(), newPos.x, newPos.y, newAngles.y );
    ...
    if (GetMoveType() == MOVETYPE_STEP) {
      if (!(GetFlags() & FL_FLY)) {
        ...
        return (GetMotor()->MoveGroundStep(newPos, pTarget, newAngles.y, false,
                                           true, pTraceResult) == AIM_SUCCESS);
      } else {
        // FIXME: here's no direct interface to a fly motor, plus this needs to
        // support a state where going through the world is okay.
        // FIXME: add callbacks into the script system for validation
        // FIXME: add function on scripts to force only legal movements
        // FIXME: GetIntervalMovement deals in Local space, nor global.
        // Currently now way to communicate that through these interfaces.
...
float CAI_BaseNPC::MaxYawSpeed(void) { return 45; }
