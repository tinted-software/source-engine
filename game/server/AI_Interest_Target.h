...
public:
  enum CAI_InterestTarget_e { LOOKAT_ENTITY = 0, LOOKAT_POSITION, LOOKAT_BOTH };
...
public:
  ...
  void Add(const Vector &vecPosition, float flImportance, float flDuration,
           float flRamp);
  void Add(CBaseEntity *pTarget, const Vector &vecPosition, float flImportance,
...
private:
  void Add(CAI_InterestTarget_t::CAI_InterestTarget_e type,
           CBaseEntity *pTarget, const Vector &vecPosition, float flImportance,
           float flDuration, float flRamp);
};
