...
float CAI_InterestTarget_t::Interest(void) {
  float t =
      (gpGlobals->curtime - m_flStartTime) / (m_flEndTime - m_flStartTime);
...
void CAI_InterestTarget::Add(CBaseEntity *pTarget, float flImportance,
  ...
  Add(CAI_InterestTarget_t::LOOKAT_ENTITY, pTarget, Vector(0, 0, 0),
      flImportance, flDuration, flRamp);
}

void CAI_InterestTarget::Add(const Vector &vecPosition, float flImportance,
                             float flDuration, float flRamp) {
  int i;
  ...
  Add(CAI_InterestTarget_t::LOOKAT_POSITION, NULL, vecPosition, flImportance,
      flDuration, flRamp);
}

void CAI_InterestTarget::Add(CBaseEntity *pTarget, const Vector &vecPosition,
  ...
  Add(CAI_InterestTarget_t::LOOKAT_BOTH, pTarget, vecPosition, flImportance,
      flDuration, flRamp);
}

void CAI_InterestTarget::Add(CAI_InterestTarget_t::CAI_InterestTarget_e type,
