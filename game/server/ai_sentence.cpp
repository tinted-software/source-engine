...
DEFINE_FIELD(m_voicePitch, FIELD_INTEGER),
    ...
    DEFINE_FIELD(m_nQueueSoundPriority, FIELD_INTEGER), END_DATADESC();

//-----------------------------------------------------------------------------
// Speech
//-----------------------------------------------------------------------------
CAI_SentenceBase::CAI_SentenceBase() { ClearQueue(); }
...
void CAI_SentenceBase::SentenceMsg(const char *pStatus, const char *pSentence) {
  ...
  case 1:
    DevMsg("SENTENCE [%d %.2f] %s: %s\n", GetOuter()->entindex(),
           gpGlobals->curtime, pStatus, pSentence);
    break;
...
//-----------------------------------------------------------------------------
void CAI_SentenceBase::ClearQueue() { m_nQueuedSentenceIndex = -1; }
...
void CAI_SentenceBase::UpdateSentenceQueue() {
  ...
  if (GetOuter()->FOkToMakeSound(m_nQueueSoundPriority)) {
    SENTENCEG_PlaySentenceIndex(GetOuter()->edict(), m_nQueuedSentenceIndex,
                                GetVolume(), GetSoundLevel(), 0,
                                GetVoicePitch());
...
bool CAI_SentenceBase::MatchesCriteria(SentenceCriteria_t nCriteria) {
  ...
  case SENTENCE_CRITERIA_NORMAL:
    return (GetOuter()->GetState() == NPC_STATE_COMBAT) ||
           (GetOuter()->HasSpawnFlags(SF_NPC_GAG) == 0);

  case SENTENCE_CRITERIA_IN_SQUAD:
    if ((GetOuter()->GetState() != NPC_STATE_COMBAT) &&
        GetOuter()->HasSpawnFlags(SF_NPC_GAG))
      return false;
...
int CAI_SentenceBase::PlaySentence(const char *pSentence) {
  int nSentenceIndex =
      SENTENCEG_PlayRndSz(GetOuter()->edict(), pSentence, GetVolume(),
                          GetSoundLevel(), 0, GetVoicePitch());
  if (nSentenceIndex < 0) {
...
// Speech
//-----------------------------------------------------------------------------
int CAI_SentenceBase::Speak(const char *pSentence,
  ...
  if (GetOuter()->FOkToMakeSound(nSoundPriority)) {
    ...
    //		float flSentenceTime = enginesound->GetSoundDuration(
    //nSentenceIndex );
...
// Speech w/ queue
//-----------------------------------------------------------------------------
int CAI_SentenceBase::SpeakQueued(const char *pSentence,
                                  SentencePriority_t nSoundPriority,
                                  SentenceCriteria_t nCriteria) {
  if (!MatchesCriteria(nCriteria))
    return -1;
  ...
  int nSquadCount =
      GetOuter()->GetSquad() ? GetOuter()->GetSquad()->NumMembers() : 1;
