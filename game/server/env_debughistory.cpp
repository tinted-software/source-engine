...
void CDebugHistory::Spawn() {
  ...
  if (g_pGameRules && g_pGameRules->IsMultiplayer()) {
    ...
    Warning("DEBUG HISTORY IS ENABLED. Disable before release (in "
            "env_debughistory.h).\n");
  }
#endif
...
void CDebugHistory::AddDebugHistoryLine(int iCategory, const char *szLine) {
  if (iCategory < 0 || iCategory >= MAX_HISTORY_CATEGORIES) {
    Warning("Attempted to add a debughistory line to category %d. Valid "
            "categories are %d to %d.\n",
  ...
  int iCharsToWrite =
      strlen(pszRemaining) + 1; // Add 1 so that we copy the null terminator
...
void CDebugHistory::DumpDebugHistory(int iCategory) {
  if (iCategory < 0 || iCategory >= MAX_HISTORY_CATEGORIES) {
    Warning("Attempted to dump a history for category %d. Valid categories are "
            "%d to %d.\n",
            iCategory, 0, (MAX_HISTORY_CATEGORIES - 1));
    return;
  }
  ...
  // Are we at the start of a line? If there's a null terminator before us, then
  // we're good to go.
