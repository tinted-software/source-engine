...
DEFINE_FIELD(m_vecDesiredPosition, FIELD_POSITION_VECTOR),
    ...
    DEFINE_INPUTFUNC(FIELD_STRING, "FlyToSpecificTrackViaPath",
                     InputFlyToPathTrack),
    DEFINE_INPUTFUNC(FIELD_VOID, "StartPatrol", InputStartPatrol),
    DEFINE_INPUTFUNC(FIELD_VOID, "StopPatrol", InputStopPatrol),
    DEFINE_INPUTFUNC(FIELD_VOID, "StartBreakableMovement",
                     InputStartBreakableMovement),
    DEFINE_INPUTFUNC(FIELD_VOID, "StopBreakableMovement",
                     InputStopBreakableMovement),
    DEFINE_INPUTFUNC(FIELD_VOID, "ChooseFarthestPathPoint",
                     InputChooseFarthestPathPoint),
    DEFINE_INPUTFUNC(FIELD_VOID, "ChooseNearestPathPoint",
    ...
    DEFINE_INPUTFUNC(FIELD_VOID, "StartPatrolBreakable",
    ...
    //-----------------------------------------------------------------------------
    void CAI_TrackPather::InitPathingData(float flTrackArrivalTolerance,
                                          float flTargetDistance,
  ...
  m_vecSegmentStartPoint = m_vecSegmentStartSplinePoint = m_vecDesiredPosition =
      GetAbsOrigin();
...
void CAI_TrackPather::OnRestore(void) {
  ...
  if (m_strCurrentPathName != NULL_STRING) {
    m_pCurrentPathTarget =
        (CPathTrack *)gEntList.FindEntityByName(NULL, m_strCurrentPathName);
  } else {
    m_pCurrentPathTarget = NULL;
  }
  ...
  if (m_strDestPathName != NULL_STRING) {
    m_pDestPathTarget =
        (CPathTrack *)gEntList.FindEntityByName(NULL, m_strDestPathName);
  } else {
    m_pDestPathTarget = NULL;
  }
  ...
  if (m_strLastPathName != NULL_STRING) {
    m_pLastPathTarget =
        (CPathTrack *)gEntList.FindEntityByName(NULL, m_strLastPathName);
  } else {
    m_pLastPathTarget = NULL;
  }
  ...
  if (m_strTargetNearestPathName != NULL_STRING) {
    m_pTargetNearestPath = (CPathTrack *)gEntList.FindEntityByName(
        NULL, m_strTargetNearestPathName);
  } else {
    m_pTargetNearestPath = NULL;
  }
}
...
void CAI_TrackPather::OnSave(IEntitySaveUtils *pUtils) {
  ...
  m_strCurrentPathName = (m_pCurrentPathTarget != NULL)
                             ? m_pCurrentPathTarget->GetEntityName()
                             : NULL_STRING;
  m_strDestPathName = (m_pDestPathTarget != NULL)
                          ? m_pDestPathTarget->GetEntityName()
                          : NULL_STRING;
  m_strLastPathName = (m_pLastPathTarget != NULL)
                          ? m_pLastPathTarget->GetEntityName()
                          : NULL_STRING;
  m_strTargetNearestPathName = (m_pTargetNearestPath != NULL)
                                   ? m_pTargetNearestPath->GetEntityName()
                                   : NULL_STRING;
}
...
void CAI_TrackPather::EnableLeading(bool bEnable) {
  ...
  if (m_bLeading) {
    ...
    // to prevent us from hovering around our old, no longer valid lead
    // position.
...
//-----------------------------------------------------------------------------
inline CPathTrack *
CAI_TrackPather::NextAlongCurrentPath(CPathTrack *pPath) const {
  return CPathTrack::ValidPath(m_bMovingForward ? pPath->GetNext()
                                                : pPath->GetPrevious());
}

inline CPathTrack *
CAI_TrackPather::PreviousAlongCurrentPath(CPathTrack *pPath) const {
  return CPathTrack::ValidPath(m_bMovingForward ? pPath->GetPrevious()
                                                : pPath->GetNext());
}

inline CPathTrack *
CAI_TrackPather::AdjustForMovementDirection(CPathTrack *pPath) const {
...
//-----------------------------------------------------------------------------
CBaseEntity *CAI_TrackPather::FindTrackBlocker(const Vector &vecViewPoint,
...
// Output : CBaseEntity
//-----------------------------------------------------------------------------
CPathTrack *CAI_TrackPather::BestPointOnPath(CPathTrack *pPath,
                                             const Vector &targetPos,
                                             float flAvoidRadius, bool visible,
                                             bool bFarthestPoint) {
  // Find the node nearest to the destination path target if a path is not
  // specified
  if (pPath == NULL) {
  ...
  float flFarthestDistSqr =
      (m_flFarthestPathDist - 2.0f * m_flTargetDistanceThreshold);
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    for (; CPathTrack::ValidPath(pTravPath);
         pTravPath = pNextPath, loopCheck++) {
      ...
      if (bFarthestPoint) {
        if ((flPathDist <= flNearestDist) &&
            (flNearestDist <= flFarthestDistSqr))
          continue;
      } else {
        if (flPathDist >= flNearestDist)
          continue;
      ...
      if (flAvoidRadius &&
          (pTravPath->GetAbsOrigin() - targetPos).Length2DSqr() <=
              flAvoidRadius)
        continue;
      ...
      if (visible) {
        ...
        CBaseEntity *pBlocker =
            FindTrackBlocker(pTravPath->GetAbsOrigin(), targetPos);

        // Check to see if we've hit the target, or the player's vehicle if it's
        // a player in a vehicle
        bool bHitTarget = (pTargetEnt && (pTargetEnt == pBlocker)) ||
                          (pVehicle && (pVehicle == pBlocker));

        // If we hit something, and it wasn't the target or his vehicle, then no
        // dice If we hit the target and forced move was set, *still* no dice
...
//-----------------------------------------------------------------------------
CPathTrack *CAI_TrackPather::ComputeLeadingPointAlongPath(
    const Vector &vecStartPoint, CPathTrack *pFirstTrack, float flDistance,
    Vector *pTarget) {
  ...
  *pTarget = vecStartPoint;
  ...
  for (; CPathTrack::ValidPath(pTravPath); pTravPath = pNextPath) {
    pNextPath =
        bMovingForward ? pTravPath->GetNext() : pTravPath->GetPrevious();
    ...
    ComputeClosestPoint(*pTarget, flDistance, pTravPath->GetAbsOrigin(),
                        pTarget);
    return bMovingForward ? pTravPath : pTravPath->GetNext();
  }
...
//-----------------------------------------------------------------------------
float CAI_TrackPather::ComputeDistanceAlongPathToPoint(
    CPathTrack *pStartTrack, CPathTrack *pDestTrack,
  ...
  for (; CPathTrack::ValidPath(pTravPath); pTravPath = pNextPath) {
    ...
    // NOTE: This would be made more accurate if we did the path direction check
    // here too. The starting vecPoint is sometimes *not* within the bounds of
    // the line segment.

    // Find the distance between this test point and our goal point
    flTotalDist +=
        (bMovingForward ? 1.0f : -1.0f) *
        vecPoint.AsVector2D().DistTo(pTestPath->GetAbsOrigin().AsVector2D());
...
//------------------------------------------------------------------------------
void CAI_TrackPather::VisualizeDebugInfo(const Vector &vecNearestPoint,
                                         const Vector &vecTarget) {
  if (g_debug_trackpather.GetInt() == TRACKPATHER_DEBUG_PATH) {
    NDebugOverlay::Line(m_vecSegmentStartPoint, vecTarget, 0, 0, 255, true,
                        0.1f);
    NDebugOverlay::Cross3D(vecNearestPoint, -Vector(16, 16, 16),
                           Vector(16, 16, 16), 255, 0, 0, true, 0.1f);
    NDebugOverlay::Cross3D(m_pCurrentPathTarget->GetAbsOrigin(),
                           -Vector(16, 16, 16), Vector(16, 16, 16), 0, 255, 0,
                           true, 0.1f);
    NDebugOverlay::Cross3D(m_vecDesiredPosition, -Vector(16, 16, 16),
                           Vector(16, 16, 16), 0, 0, 255, true, 0.1f);
    NDebugOverlay::Cross3D(m_pDestPathTarget->GetAbsOrigin(),
                           -Vector(16, 16, 16), Vector(16, 16, 16), 255, 255,
                           255, true, 0.1f);

    if (m_pTargetNearestPath) {
      NDebugOverlay::Cross3D(m_pTargetNearestPath->GetAbsOrigin(),
                             -Vector(24, 24, 24), Vector(24, 24, 24), 255, 0,
                             255, true, 0.1f);
    }
  }
  ...
  if (g_debug_trackpather.GetInt() == TRACKPATHER_DEBUG_TRACKS) {
    if (m_pCurrentPathTarget) {
      ...
      for (; CPathTrack::ValidPath(pPathTrack);
           pPathTrack = pPathTrack->GetNext()) {
        NDebugOverlay::Box(pPathTrack->GetAbsOrigin(), -Vector(2, 2, 2),
                           Vector(2, 2, 2), 0, 255, 0, 8, 0.1);
        if (CPathTrack::ValidPath(pPathTrack->GetNext())) {
          NDebugOverlay::Line(pPathTrack->GetAbsOrigin(),
                              pPathTrack->GetNext()->GetAbsOrigin(), 0, 255, 0,
                              true, 0.1);
        }
...
bool CAI_TrackPather::HasLOSToTarget(CPathTrack *pTrack) {
  ...
  // Check to see if we've hit the target, or the player's vehicle if it's a
  // player in a vehicle
  bool bHitTarget = (pTargetEnt && (pTargetEnt == pBlocker)) ||
                    (pVehicle && (pVehicle == pBlocker));
...
void CAI_TrackPather::UpdateCurrentTarget() {
  ...
  if ((t < 1.0f) && (vecPoint.DistToSqr(vecTarget) >
                     m_flTargetTolerance * m_flTargetTolerance))
  ...
  if (m_pCurrentPathTarget != m_pDestPathTarget) {
    ...
    // We're at rest (no patrolling behavior), which means we're moving forward
    // now.
    m_bMovingForward = true;
  }
...
float CAI_TrackPather::ComputeDistanceToLeadingPosition() {
  return ComputeDistanceAlongPathToPoint(
      m_pCurrentPathTarget, m_pDestPathTarget, GetDesiredPosition(),
      m_bMovingForward);
}
...
float CAI_TrackPather::ComputeDistanceToTargetPosition() {
  ...
  CPathTrack *pDest = m_bMovingForward ? m_pTargetNearestPath.Get()
                                       : m_pTargetNearestPath->GetPrevious();
  if (!pDest) {
  ...
  return ComputeDistanceAlongPathToPoint(pStart, pDest, m_vecTargetPathPoint,
                                         bMovingForward);
}
...
//-----------------------------------------------------------------------------
void CAI_TrackPather::ComputePathDirection(CPathTrack *pPath,
                                           Vector *pVecPathDir) {
  if (pPath->GetPrevious()) {
    VectorSubtract(pPath->GetAbsOrigin(), pPath->GetPrevious()->GetAbsOrigin(),
                   *pVecPathDir);
  } else {
    if (pPath->GetNext()) {
      VectorSubtract(pPath->GetNext()->GetAbsOrigin(), pPath->GetAbsOrigin(),
                     *pVecPathDir);
    } else {
...
//-----------------------------------------------------------------------------
void CAI_TrackPather::ComputePointAlongCurrentPath(float flDistance,
                                                   float flPerpDist,
                                                   Vector *pTarget) {
  Vector vecPathDir;
  ...
  *pTarget = vecStartPoint;
  ...
  if (flDistance != 0.0f) {
    ...
    for (; CPathTrack::ValidPath(pTravPath);
      ...
      if (pTravPath == pAdjustedDest) {
        ...
        if (flDistance > flPathDist) {
          *pTarget = GetDesiredPosition();
          ...
          ComputeClosestPoint(*pTarget, flDistance, GetDesiredPosition(),
                              pTarget);
        }
        break;
      }
      ...
      if (flPathDist <= flDistance) {
        ...
        *pTarget = pTravPath->GetAbsOrigin();
        ...
        // FIXME: Reduce the distance further based on the angle between this
        // segment + the next
        continue;
      }
    ...
    VectorSubtract(m_pCurrentPathTarget->GetAbsOrigin(), m_vecSegmentStartPoint,
                   vecPathDir);
...
//-----------------------------------------------------------------------------
float CAI_TrackPather::ComputePerpDistanceFromPath(
    const Vector &vecPointOnPath, const Vector &vecPathDir,
...
void CAI_TrackPather::ComputePointFromPerpDistance(const Vector &vecPointOnPath,
...
// where negative means on the left side of the path (when travelled from prev
// to next) and positive means on the right side
//-----------------------------------------------------------------------------
CPathTrack *CAI_TrackPather::FindClosestPointOnPath(CPathTrack *pPath,
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    for (; CPathTrack::ValidPath(pTravPath);
      ...
      if (pTravPath->m_paltpath) {
        Warning("%s: Alternative paths in path_track not allowed when using "
      ...
      Vector vecClosest;
      CalcClosestPointOnLineSegment(targetPos, pTravPath->GetAbsOrigin(),
                                    pNextPath->GetAbsOrigin(), vecClosest);
      ...
      VectorSubtract(pNextPath->GetAbsOrigin(), pTravPath->GetAbsOrigin(),
                     vecNearestPathSegment);
      if (i == 0) {
  ...
  *pDistanceFromPath = ComputePerpDistanceFromPath(
      vecNearestPoint, vecNearestPathSegment, targetPos);
...
void CAI_TrackPather::SelectNewDestTarget() {
...
#endif
  {
    ...
    while (true) {
      ...
      if (!pNextTrack || (pNextTrack == pOldDest) ||
          pNextTrack->HasBeenVisited())
        break;
...
void CAI_TrackPather::UpdateCurrentTargetLeading() {
  ...
  if (m_pCurrentPathTarget == pAdjustedDest) {
    ...
    // NOTE: Must do this again since SelectNewDestTarget may change
    // m_pDestPathTarget
    pAdjustedDest = AdjustForMovementDirection(m_pDestPathTarget);
  }
...
void CAI_TrackPather::UpdateTargetPositionLeading(void) {
  ...
  CPathTrack *pNextPath =
      FindClosestPointOnPath(m_pCurrentPathTarget, targetPos, &vecClosestPoint,
                             &vecPathDir, &flTargetDistanceFromPath);

  // This means that a valid path could not be found to our target!
  if (CPathTrack::ValidPath(pNextPath) == NULL)
    return;

  //	NDebugOverlay::Cross3D( vecClosestPoint, -Vector(24,24,24),
  //Vector(24,24,24), 0, 255, 255, true, 0.1f ); 	NDebugOverlay::Cross3D(
  //pNextPath->GetAbsOrigin(), -Vector(24,24,24), Vector(24,24,24), 255, 255, 0,
  //true, 0.1f );
  ...
  // NOTE: This will *also* return a path track on the "far" end of the line
  // segment containing the leading position, namely the "next" end of the
  // segment as opposed to the "prev" end of the segment.
  CPathTrack *pDest = ComputeLeadingPointAlongPath(
      vecClosestPoint, pNextPath, m_flLeadDistance, &targetPos);
  SetDesiredPosition(targetPos);

  // We only want to switch movement directions when absolutely necessary
  // so convert dest into a more appropriate value based on the current movement
  // direction
  if (pDest != m_pDestPathTarget) {
    // NOTE: This is really tricky + subtle
    // For leading, we don't want to ever change direction when the current
    // target == the adjusted destination target. Namely, if we're going
    // forward, both dest + curr mean the "next"most node so we can compare them
    // directly against eath other. If we're moving backward, dest means
    // "next"most, but curr means "prev"most. We first have to adjust the dest
    // to mean "prev"most, and then do the comparison. If the adjusted dest ==
    // curr, then maintain direction. Otherwise, use the forward along path
    // test.
  ...
  //	NDebugOverlay::Cross3D( m_pCurrentPathTarget->GetAbsOrigin(),
  //-Vector(36,36,36), Vector(36,36,36), 255, 0, 0, true, 0.1f );
  //	NDebugOverlay::Cross3D( m_pDestPathTarget->GetAbsOrigin(),
  //-Vector(48,48,48), Vector(48,48,48), 0, 255, 0, true, 0.1f );
  //	NDebugOverlay::Cross3D( targetPos, -Vector(36,36,36), Vector(36,36,36),
  //0, 0, 255, true, 0.1f );
}
...
void CAI_TrackPather::UpdateTargetPosition(void) {
  ...
  // Find the best position to be on our path
  CPathTrack *pDest =
      BestPointOnPath(m_pCurrentPathTarget, targetPos, m_flAvoidDistance, true,
...
float CAI_TrackPather::MaxDistanceFromCurrentPath() const {
  ...
  // NOTE: Can't use m_vecSegmentStartPoint because we don't have a radius
  // defined for it
  float t;
  Vector vecTemp;
  ...
  t = clamp(t, 0.0f, 1.0f);
  float flRadius = (1.0f - t) * pPrevPath->GetRadius() +
                   t * m_pCurrentPathTarget->GetRadius();
  return flRadius;
}

//------------------------------------------------------------------------------
// Purpose : A different version of the track pather which is more explicit
// about the meaning of dest, current, and prev path points
//------------------------------------------------------------------------------
void CAI_TrackPather::UpdateTrackNavigation(void) {
  // No target? Use the string specified. We have no spawn method (sucky!!) so
...
void CAI_TrackPather::SetupNewCurrentTarget(CPathTrack *pTrack) {
  ...
  VectorMA(m_vecSegmentStartPoint, -2.0f, GetAbsVelocity(),
...
void CAI_TrackPather::MoveToTrackPoint(CPathTrack *pTrack) {
  if (IsOnSameTrack(pTrack, m_pDestPathTarget)) {
    ...
    CPathTrack *pClosestTrack =
        BestPointOnPath(pTrack, WorldSpaceCenter(), 0.0f, false, false);
...
//-----------------------------------------------------------------------------
bool CAI_TrackPather::IsOnSameTrack(CPathTrack *pPath1,
                                    CPathTrack *pPath2) const {
  if (pPath1 == pPath2)
    return true;
...
void CAI_TrackPather::Teleported() {
  ...
  CPathTrack *pClosestTrack = BestPointOnPath(
      GetDestPathTarget(), WorldSpaceCenter(), 0.0f, false, false);
  m_pDestPathTarget = NULL;
...
//-----------------------------------------------------------------------------
float CAI_TrackPather::ComputePathDistance(CPathTrack *pPath, CPathTrack *pDest,
                                           bool bForward) const {
  float flDist = 0.0f;
...
//-----------------------------------------------------------------------------
// Is pPathTest in "front" of pPath on the same path? (Namely, does GetNext()
// get us there?)
//-----------------------------------------------------------------------------
bool CAI_TrackPather::IsForwardAlongPath(CPathTrack *pPath,
                                         CPathTrack *pPathTest) const {
...
void CAI_TrackPather::ComputePathTangent(float t, Vector *pVecTangent) const {
  ...
  Catmull_Rom_Spline_Tangent(m_vecSegmentStartSplinePoint,
                             m_vecSegmentStartPoint,
                             m_pCurrentPathTarget->GetAbsOrigin(),
                             pNextTrack->GetAbsOrigin(), t, *pVecTangent);
  VectorNormalize(*pVecTangent);
}

//-----------------------------------------------------------------------------
// Computes the *normalized* velocity at which the helicopter should approach
// the final point
//-----------------------------------------------------------------------------
void CAI_TrackPather::ComputeNormalizedDestVelocity(
    Vector *pVecVelocity) const {
  ...
  if ((pNextTrack == m_pCurrentPathTarget) ||
      (m_pCurrentPathTarget == m_pDestPathTarget)) {
    pVecVelocity->Init(0, 0, 0);
    return;
  }

  VectorSubtract(pNextTrack->GetAbsOrigin(),
                 m_pCurrentPathTarget->GetAbsOrigin(), *pVecVelocity);
