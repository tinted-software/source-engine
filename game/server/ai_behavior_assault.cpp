...
bool CRallyPoint::IsExclusive() {
  ...
  if (m_sExclusivity == RALLY_EXCLUSIVE_NOT_EVALUATED) {
    ...
    CAssaultPoint *pAssaultEnt =
        (CAssaultPoint *)gEntList.FindEntityByName(NULL, m_AssaultPointName);

    if (!pAssaultEnt) {
      // Well, this is awkward. Leave it up to other assault code to tattle on
      // the missing assault point. We will just assume this assault is not
      // exclusive.
      m_sExclusivity = RALLY_EXCLUSIVE_NO;
      return false;
    }
    ...
    if (pAssaultEnt) {
      CAssaultPoint *pFirstAssaultEnt =
          pAssaultEnt; // some assault chains are circularly linked

      do {
        if (pAssaultEnt->m_bNeverTimeout) {
          // We found a never timeout assault point! That makes this whole chain
          // exclusive.
          m_sExclusivity = RALLY_EXCLUSIVE_YES;
          break;
        }

        pAssaultEnt = (CAssaultPoint *)gEntList.FindEntityByName(
            NULL, pAssaultEnt->m_NextAssaultPointName);
...
DEFINE_KEYFIELD(m_AssaultHintGroup, FIELD_STRING, "assaultgroup"),
    ...
    DEFINE_KEYFIELD(m_flAllowDiversionRadius, FIELD_FLOAT,
    ...
    // Inputs
    DEFINE_INPUTFUNC(FIELD_BOOLEAN, "SetClearOnContact",
                     InputSetClearOnContact),
    DEFINE_INPUTFUNC(FIELD_BOOLEAN, "SetAllowDiversion",
                     InputSetAllowDiversion),
    DEFINE_INPUTFUNC(FIELD_BOOLEAN, "SetForceClear", InputSetForceClear),

    // Outputs
    DEFINE_OUTPUT(m_OnArrival, "OnArrival"),
    DEFINE_OUTPUT(m_OnAssaultClear, "OnAssaultClear"), END_DATADESC();

LINK_ENTITY_TO_CLASS(assault_rallypoint,
                     CRallyPoint); // just a copy of info_target for now
LINK_ENTITY_TO_CLASS(
    assault_assaultpoint,
    CAssaultPoint); // has its own class because it needs the entity I/O
...
DEFINE_FIELD(m_hAssaultPoint, FIELD_EHANDLE),
    ...
    DEFINE_FIELD(m_AssaultPointName, FIELD_STRING) END_DATADESC();
...
int CAI_AssaultBehavior::DrawDebugTextOverlays(int text_offset) {
  ...
  if (GetOuter()->m_debugOverlays & OVERLAY_TEXT_BIT) {
    Q_snprintf(tempstr, sizeof(tempstr), "Assault Point: %s %s",
...
//			If more than one assault point of this type is found,
//randomly 			use any of them EXCEPT the one most recently used.
//-----------------------------------------------------------------------------
CAssaultPoint *
CAI_AssaultBehavior::FindAssaultPoint(string_t iszAssaultPointName) {
  CUtlVector<CAssaultPoint *> pAssaultPoints;
  CUtlVector<CAssaultPoint *> pClearAssaultPoints;

  CAssaultPoint *pAssaultEnt =
      (CAssaultPoint *)gEntList.FindEntityByName(NULL, iszAssaultPointName);

  while (pAssaultEnt != NULL) {
    pAssaultPoints.AddToTail(pAssaultEnt);
    pAssaultEnt = (CAssaultPoint *)gEntList.FindEntityByName(
        pAssaultEnt, iszAssaultPointName);
  }
  ...
  for (int i = 0; i < pAssaultPoints.Count(); i++) {
    ...
    AI_TraceHull(pAssaultPoint->GetAbsOrigin(), pAssaultPoint->GetAbsOrigin(),
                 pNPC->WorldAlignMins(), pNPC->WorldAlignMaxs(), MASK_SOLID,
  ...
  // NONE left. Just return a random assault point, knowing that it's blocked.
  // This is the old behavior, anyway.
  if (pClearAssaultPoints.Count() < 1)
  ...
  return pClearAssaultPoints[random->RandomInt(
      0, (pClearAssaultPoints.Count() - 1))];
}
...
void CAI_AssaultBehavior::ClearAssaultPoint(void) {
  ...
  if (m_hAssaultPoint->m_NextAssaultPointName != NULL_STRING) {
    CAssaultPoint *pNextPoint =
        FindAssaultPoint(m_hAssaultPoint->m_NextAssaultPointName);
...
void CAI_AssaultBehavior::GatherConditions(void) {
  ...
  // then hit and clear the assault point (fire all entity I/O) and move on to
  // the next one without interrupting the NPC's schedule. This provides a more
  // fluid movement from point to point.
...
void CAI_AssaultBehavior::StartTask(const Task_t *pTask) {
  ...
  case TASK_ANNOUNCE_CLEAR: {
    // If we're at an assault point that can never be cleared, keep waiting
    // forever (if it's the last point in the assault)
    if (m_hAssaultPoint &&
        !m_hAssaultPoint->HasSpawnFlags(SF_ASSAULTPOINT_CLEARONARRIVAL) &&
  ...
  case TASK_GET_PATH_TO_RALLY_POINT: {
    ...
    if (GetNavigator()->SetGoal(goal) == false) {
      ...
      AI_NavGoal_t nearGoal(GOALTYPE_LOCATION_NEAREST_NODE,
                            m_hRallyPoint->GetAbsOrigin(), AIN_DEF_ACTIVITY,
                            256);
  ...
  case TASK_GET_PATH_TO_ASSAULT_POINT: {
    ...
    if (GetNavigator()->SetGoal(goal) == false) {
      // Try and get as close as possible otherwise
      AI_NavGoal_t nearGoal(GOALTYPE_LOCATION_NEAREST_NODE,
                            m_hAssaultPoint->GetAbsOrigin(), AIN_DEF_ACTIVITY,
                            256);
      if (GetNavigator()->SetGoal(nearGoal, AIN_CLEAR_PREVIOUS_STATE)) {
        // FIXME: HACK! The internal pathfinding is setting this without our
        // consent, so override it!
  ...
  case TASK_FACE_ASSAULT_POINT: {
    ...
    if (HasCondition(COND_CAN_RANGE_ATTACK1)) {
      // If I can already fight when I arrive, don't bother running any facing
      // code. Let The combat AI do that. Turning here will only make the NPC
      // look dumb in a combat situation because it will take time to turn
      // before attacking.
      TaskComplete();
    } else {
  ...
  case TASK_AWAIT_CUE:
    if (PollAssaultCue()) {
      ...
      if (IsForcingCrouch())
        ...
        int sequence = GetOuter()->LookupSequence(
            STRING(m_hRallyPoint->m_RallySequenceName));
        if (sequence != -1) {
...
void CAI_AssaultBehavior::RunTask(const Task_t *pTask) {
  ...
  case TASK_AWAIT_ASSAULT_TIMEOUT:
    if (m_hAssaultPoint) {
      if (m_hAssaultPoint->m_bInputForcedClear ||
          (m_hAssaultPoint->m_bClearOnContact &&
           HasCondition(COND_SEE_ENEMY))) {
        // If we're on an assault that should clear on contact, clear when we
        // see an enemy
        TaskComplete();
      }
    }
    ...
    if (GetOuter()->IsWaitFinished() &&
        (pTask->iTask == TASK_WAIT_ASSAULT_DELAY ||
         !m_hAssaultPoint->m_bNeverTimeout)) {
      TaskComplete();
    }
    break;
  ...
  case TASK_AWAIT_CUE:
    ...
    if (GetOuter()->GetEnemy() &&
        m_hRallyPoint->m_RallySequenceName == NULL_STRING &&
  ...
  case TASK_WAIT_FOR_MOVEMENT:
    if (ai_debug_assault.GetBool()) {
      if (IsCurSchedule(SCHED_MOVE_TO_ASSAULT_POINT)) {
        NDebugOverlay::Line(WorldSpaceCenter(), GetNavigator()->GetGoalPos(),
                            255, 0, 0, true, 0.1);
        NDebugOverlay::Box(GetNavigator()->GetGoalPos(), -Vector(10, 10, 10),
                           Vector(10, 10, 10), 255, 0, 0, 8, 0.1);
      } else if (IsCurSchedule(SCHED_MOVE_TO_RALLY_POINT)) {
        NDebugOverlay::Line(WorldSpaceCenter(), GetNavigator()->GetGoalPos(), 0,
                            255, 0, true, 0.1);
        NDebugOverlay::Box(GetNavigator()->GetGoalPos(), -Vector(10, 10, 10),
                           Vector(10, 10, 10), 0, 255, 0, 8, 0.1);
      }
    }
    ...
    if (((!GetOuter()->DidChooseEnemy() &&
          gpGlobals->curtime - GetOuter()->GetTimeEnemyAcquired() > 1) ||
         !GetOuter()->GetEnemy())) {
...
//-----------------------------------------------------------------------------
CRallyPoint *
CAI_AssaultBehavior::FindBestRallyPointInRadius(const Vector &vecCenter,
                                                float flRadius) {
  VPROF_BUDGET("CAI_AssaultBehavior::FindBestRallyPointInRadius",
               VPROF_BUDGETGROUP_NPCS);

  const int RALLY_SEARCH_ENTS = 30;
  CBaseEntity *pEntities[RALLY_SEARCH_ENTS];
  int iNumEntities = UTIL_EntitiesInSphere(pEntities, RALLY_SEARCH_ENTS,
                                           vecCenter, flRadius, 0);
...
bool CAI_AssaultBehavior::IsValidShootPosition(const Vector &vLocation,
  ...
  if (pCuePoint && (vLocation - pCuePoint->GetAbsOrigin()).Length2DSqr() >
                       Square(flTolerance - 0.1))
    return false;
...
bool CAI_AssaultBehavior::UpdateForceCrouch(void) {
  if (IsForcingCrouch()) {
    ...
    float flDistanceToTargetSqr = GetOuter()->GetAbsOrigin().DistToSqr(
        AssaultHasBegun() ? m_hAssaultPoint->GetAbsOrigin()
                          : m_hRallyPoint->GetAbsOrigin());
...
//			assaultcue -
//-----------------------------------------------------------------------------
void CAI_AssaultBehavior::SetParameters(CBaseEntity *pRallyEnt,
...
//			assaultcue -
//-----------------------------------------------------------------------------
void CAI_AssaultBehavior::SetParameters(string_t rallypointname,
                                        AssaultCue_t assaultcue,
  ...
  // Firstly, find a rally point.
  CRallyPoint *pRallyEnt = dynamic_cast<CRallyPoint *>(
      gEntList.FindEntityByName(NULL, rallypointname));
  ...
  case RALLY_POINT_SELECT_DEFAULT: {
    while (pRallyEnt) {
      ...
      pRallyEnt = dynamic_cast<CRallyPoint *>(
          gEntList.FindEntityByName(pRallyEnt, rallypointname, NULL));
    }
  } break;
  ...
  case RALLY_POINT_SELECT_RANDOM: {
    ...
    CUtlVector<CRallyPoint *>
        rallyPoints; // List of rally points that are available to choose from.
    ...
    while (pRallyEnt) {
      ...
      pRallyEnt = dynamic_cast<CRallyPoint *>(
          gEntList.FindEntityByName(pRallyEnt, rallypointname));
    }
  ...
  default:
    DevMsg("ERROR: INVALID RALLY POINT SELECTION METHOD. Assault will not "
           "function.\n");
    break;
  }
...
void CAI_AssaultBehavior::InitializeBehavior() {
  ...
  CAssaultPoint *pAssaultEnt =
      FindAssaultPoint(m_hRallyPoint->m_AssaultPointName);
  ...
  // Slam the NPC's schedule so that he starts picking Assault schedules right
  // now.
...
bool CAI_AssaultBehavior::CanSelectSchedule() {
  ...
  // We're letting other AI run for a little while because the assault AI failed
  // recently.
  ...
  // If we've seen an enemy in the last few seconds, and we're allowed to
  // divert, let the base AI decide what I should do.
  if (IsAllowedToDivert()) {
    // Return true, but remember that we're actually allowing them to divert
    // This is done because we don't want the assault behaviour to think it's
...
void CAI_AssaultBehavior::EndScheduleSelection() {
  ...
  if (m_hRallyPoint != NULL) {
    ...
    if (!hl2_episodic.GetBool() || !m_hRallyPoint->IsExclusive() ||
        !GetOuter()->IsAlive()) {
...
int CAI_AssaultBehavior::TranslateSchedule(int scheduleType) {
  ...
  case SCHED_RANGE_ATTACK1:
    if (GetOuter()->GetShotRegulator()->IsInRestInterval()) {
      if (GetOuter()->HasStrategySlotRange(SQUAD_SLOT_ATTACK1,
                                           SQUAD_SLOT_ATTACK2))
        GetOuter()->VacateStrategySlot();
      return SCHED_COMBAT_FACE; // @TODO (toml 07-02-03): Should do something
                                // more tactically sensible
    }
    break;
...
void CAI_AssaultBehavior::ClearSchedule(const char *szReason) {
  // HACKHACK: In reality, we shouldn't be clearing the schedule ever if the
  // assault behavior isn't actually in charge of the NPC. Fix after ship. For
  // now, hacking a fix to Grigori failing to make it over the fence of the
  // graveyard in d1_town_02a
  if (GetOuter()->ClassMatches("npc_monk") &&
  ...
  if (m_hAssaultPoint && m_hAssaultPoint->m_bAllowDiversion) {
    if (m_hAssaultPoint->m_flAllowDiversionRadius == 0.0f ||
        (m_bHitAssaultPoint && GetEnemy() != NULL &&
         GetEnemy()->GetAbsOrigin().DistToSqr(
             m_hAssaultPoint->GetAbsOrigin()) <=
             Square(m_hAssaultPoint->m_flAllowDiversionRadius))) {
      if (m_flLastSawAnEnemyAt && ((gpGlobals->curtime - m_flLastSawAnEnemyAt) <
                                   ASSAULT_DIVERSION_TIME))
        return true;
    }
  }
...
void CAI_AssaultBehavior::BuildScheduleTestBits() {
  ...
  // If we're allowed to divert, add the appropriate interrupts to our movement
  // schedules
  if (IsAllowedToDivert()) {
    if (IsCurSchedule(SCHED_MOVE_TO_ASSAULT_POINT) ||
...
int CAI_AssaultBehavior::SelectSchedule() {
  ...
  if (m_bHitRallyPoint && !m_bHitAssaultPoint && !AssaultHasBegun()) {
    ...
    // Make sure I'm still on my rally point, cause another behavior may have
    // moved me. 2D check to be within 32 units of my rallypoint.
    ...
    // Likewise. If I have hit my assault point, make sure I'm still there.
    // Another behavior (hide and reload) may have moved me away.
  ...
  if (!m_bHitAssaultPoint) {
    if (m_ReceivedAssaultCue == m_AssaultCue ||
        m_ReceivedAssaultCue == CUE_COMMANDER ||
      ...
      if (m_hRallyPoint && !m_hRallyPoint->IsExclusive()) {
        // If this assault chain is not exclusive, then free up the rallypoint
        // so that others can follow me Otherwise, we do not unlock this rally
        // point until we are FINISHED or DEAD. It's exclusively our chain of
        // assault
        UnlockRallyPoint(); // Here we go! Free up the rally point since I'm
                            // moving to assault.
      }
  ...
  if (OnStrictAssault()) {
    // Don't allow the base class to select a schedule cause it will probably
    // move the NPC.
    if (!HasCondition(COND_CAN_RANGE_ATTACK1) &&
        !HasCondition(COND_CAN_RANGE_ATTACK2) &&
...
#ifdef HL2_EPISODIC
  // This ugly patch fixes a bug where Combine Soldiers on an assault would not
  // shoot through glass, because of the way that shooting through glass is
  // implemented in their AI. (sjb)
  if (HasCondition(COND_SEE_ENEMY) &&
    ...
    if (pBlocker && pBlocker->GetHealth() > 0) {
      if (GetOuter()->Classify() == CLASS_COMBINE &&
          FClassnameIs(GetOuter(), "npc_combine_s")) {
  ...
  for (i = 0; i < NumActors(); i++) {
    ...
    if (pActor) {
      ...
      if (pActor->GetBehavior(&pBehavior)) {
        // GOT IT! Now tell the behavior that entity i/o wants to cue the
        // assault.
...
DEFINE_SCHEDULE(
    ...
    "		TASK_SET_SCHEDULE					"
    "	SCHEDULE:SCHED_HOLD_RALLY_POINT"
    "	"
    "	Interrupts"
    "		COND_HEAR_DANGER"
    "		COND_PROVOKED"
