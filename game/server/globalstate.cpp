...
public:
  ...
  void EnableStateUpdates(bool bEnable) { m_disableStateUpdates = !bEnable; }
  ...
  int AddEntity(const char *pGlobalname, const char *pMapName,
                GLOBALESTATE state) {
  ...
  int GetNumGlobals(void) { return m_list.Count(); }
...
void CGlobalState::DumpGlobals(void) {
  ...
  for (int i = 0; i < m_list.Count(); i++) {
    Msg("%s: %s (%s) = %d\n", m_nameList.String(m_list[i].name),
        m_nameList.String(m_list[i].levelName), estates[m_list[i].state],
        m_list[i].counter);
  }
}
// #endif
...
DEFINE_UTLVECTOR(m_list, FIELD_EMBEDDED),
  ...
  if (!save.WriteFields("GLOBAL", this, NULL, m_DataMap.dataDesc,
...
int CGlobalState::Restore(IRestore &restore) {
  Reset();
  if (!restore.ReadFields("GLOBAL", this, NULL, m_DataMap.dataDesc,
                          m_DataMap.dataNumFields))
    return 0;
...
void ResetGlobalState(void) { gGlobalState.Reset(); }

void ShowServerGameTime() { Msg("Server game time: %f\n", gpGlobals->curtime); }

CON_COMMAND(server_game_time,
