...
void CreateWeaponManager(const char *pWeaponName, int iMaxPieces) {
  CGameWeaponManager *pManager =
      (CGameWeaponManager *)CreateEntityByName("game_weapon_manager");
...
void CGameWeaponManager::Think() {
  ...
  if (m_bExpectingWeapon) {
    ...
    pWeapon = assert_cast<CBaseCombatWeapon *>(
        gEntList.FindEntityByClassname(pWeapon, pszWeaponName));
  ...
  // Based on what the player can see, try to clean up the world by removing
  // weapons that the player cannot see right at the moment.
  ...
  for (i = 0; i < candidates.Count() && surplus > 0; i++) {
    ...
    if (gpGlobals->maxClients == 1) {
      ...
      if (!UTIL_FindClientInPVS(pCandidate->edict())) {
        ...
        fRemovedOne = true;
      } else if (UTIL_DistApprox(pPlayer->GetAbsOrigin(),
                                 pCandidate->GetAbsOrigin()) > (30 * 12)) {
        fRemovedOne = true;
      }
    } else {
      fRemovedOne = true;
