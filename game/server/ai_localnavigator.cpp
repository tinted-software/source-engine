...
//						m_FullDirectTimer	(not
//saved) 						m_pPlaneSolver	(not saved) 						m_pMoveProbe	(not saved)
END_DATADESC();
...
CAI_LocalNavigator::~CAI_LocalNavigator() { delete m_pPlaneSolver; }
...
void CAI_LocalNavigator::Init(IAI_MovementSink *pMovementServices) {
  ...
  m_pMoveProbe =
      GetOuter()->GetMoveProbe(); // @TODO (toml 03-30-03): this is a "bad" way
                                  // to grab this pointer. Components should
                                  // have an explcit "init" phase.
}
...
void CAI_LocalNavigator::AddObstacle(const Vector &pos, float radius,
...
bool CAI_LocalNavigator::MoveCalcDirect(AILocalMoveGoal_t *pMoveGoal,
                                        bool bOnlyCurThink, float *pDistClear,
  ...
  if (pMoveGoal->speed) {
    ...
    float probeDist = m_pPlaneSolver->CalcProbeDist(
        pMoveGoal->speed); // having this match steering allows one fewer traces
    float checkDist = MAX(minCheckDist, probeDist);
    float checkStepDist = MAX(16.0, probeDist * 0.5);

    if (pMoveGoal->flags &
        (AILMG_TARGET_IS_TRANSITION | AILMG_TARGET_IS_GOAL)) {
    ...
    if (bOnlyCurThink) // Outer code claims to have done a validation (probably
    ...
    if (!bExpectingArrival) {
      ...
      bTraceClear = GetMoveProbe()->MoveLimit(
          pMoveGoal->navType, GetLocalOrigin(), testPos, MASK_NPCSOLID,
          pMoveGoal->pMoveTarget, 100.0,
          (pMoveGoal->navType == NAV_GROUND) ? AIMLF_2D : AIMLF_DEFAULT,
          &pMoveGoal->directTrace);

      if (!bTraceClear) {
        // Adjust probe top match expected probe dist (relied on later in
        // process)
        pMoveGoal->directTrace.flDistObstructed =
            (checkDist - moveThisInterval) +
            pMoveGoal->directTrace.flDistObstructed;
      }
      ...
      if (!IsRetail() && ai_debug_directnavprobe.GetBool()) {
        if (!bTraceClear) {
          ...
          NDebugOverlay::Line(
              WorldSpaceCenter(),
              Vector(testPos.x, testPos.y, WorldSpaceCenter().z), 255, 0, 0,
              false, 0.1);
          if (pMoveGoal->directTrace.pObstruction)
            NDebugOverlay::Line(
                WorldSpaceCenter(),
                pMoveGoal->directTrace.pObstruction->WorldSpaceCenter(), 255, 0,
                255, false, 0.1);

        } else {
          NDebugOverlay::Line(
              WorldSpaceCenter(),
              Vector(testPos.x, testPos.y, WorldSpaceCenter().z), 0, 255, 0,
              false, 0.1);
        }
      }
    ...
    if (bTraceClear) {
      if (m_FullDirectTimer.Expired()) {
        ...
        bTraceClear = GetMoveProbe()->MoveLimit(
            pMoveGoal->navType, GetLocalOrigin(), testPos, MASK_NPCSOLID,
            pMoveGoal->pMoveTarget, checkStepPct,
            (pMoveGoal->navType == NAV_GROUND) ? AIMLF_2D : AIMLF_DEFAULT,
            &pMoveGoal->directTrace);
        if (bExpectingArrival)
          pMoveGoal->thinkTrace = pMoveGoal->directTrace;
        ...
        if (ai_debug_directnavprobe.GetBool()) {
          if (!bTraceClear) {
            NDebugOverlay::Line(
                ...
                Vector(testPos.x, testPos.y, GetOuter()->EyePosition().z), 255,
                0, 0, false, 0.1);
            ...
            NDebugOverlay::Line(
                GetOuter()->EyePosition(),
                Vector(testPos.x, testPos.y, GetOuter()->EyePosition().z), 0,
                255, 0, false, 0.1);
    ...
    // Should never end up in this function with speed of zero. Probably an
    // activity problem.
...
bool CAI_LocalNavigator::MoveCalcSteer(AILocalMoveGoal_t *pMoveGoal,
                                       float distClear,
                                       AIMoveResult_t *pResult) {
  ...
  if (m_pPlaneSolver->Solve(*pMoveGoal, distClear, &moveSolution)) {
    if (moveSolution != pMoveGoal->dir) {
      ...
      if (dot > COS_HALF_30) {
        ...
        if (pMoveGoal->maxDist < probeDist * 0.33333 &&
            distClear > probeDist * 0.6666) {
...
bool CAI_LocalNavigator::MoveCalcStop(AILocalMoveGoal_t *pMoveGoal,
                                      float distClear,
                                      AIMoveResult_t *pResult) {
...
AIMoveResult_t CAI_LocalNavigator::MoveCalcRaw(AILocalMoveGoal_t *pMoveGoal,
  ...
  bool bDirectClear =
      MoveCalcDirect(pMoveGoal, bOnlyCurThink, &distClear, &result);
  if (OnCalcBaseMove(pMoveGoal, distClear, &result)) {
    SetSolveCookie();
    return DbgResult(result);
  }

  bool bShouldSteer = (!(pMoveGoal->flags & AILMG_NO_STEER) &&
                       (!bDirectClear || HaveObstacles()));
  ...
  // A hopeful result... may get in trouble at next waypoint and obstruction is
  // still there
  ...
  DebugNoteMovementFailure();
  SetSolveCookie();
  return DbgResult(IsMoveBlocked(pMoveGoal->directTrace.fStatus)
                       ? pMoveGoal->directTrace.fStatus
                       : AIMR_ILLEGAL);
}
