...
static ConVar
    scene_maxcaptionradius("scene_maxcaptionradius", "1200", 0,
                           "Only show closed captions if recipient is within "
                           "this many units of speaking actor (0==disabled).");
...
public:
  ...
  bool IsRunningScriptedSceneWithSpeech(CBaseFlex *pActor,
                                        bool bIgnoreInstancedScenes);
  bool
  IsRunningScriptedSceneWithSpeechAndNotPaused(CBaseFlex *pActor,
                                               bool bIgnoreInstancedScenes);
...
// Purpose:
//-----------------------------------------------------------------------------
void FreeSceneFileMemory(void *buffer) { delete[] (byte *)buffer; }
...
public:
  ...
  bool GetSoundNameForPlayer(CChoreoEvent *event, CBasePlayer *player,
                             char *buf, size_t buflen, CBaseEntity *pActor);
...
CChoreoScene *CSceneEntity::GenerateSceneForSound(CBaseFlex *pFlexActor,
  ...
  if (duration <= 0.0f) {
    Warning("CSceneEntity::GenerateSceneForSound:  Couldn't determine duration "
            "of %s\n",
            soundname);
    return NULL;
  }
  ...
  if (!scene) {
    Warning("CSceneEntity::GenerateSceneForSound:  Failed to allocated new "
            "scene!!!\n");
  } else {
    ...
    if (!actor || !channel || !event) {
      Warning("CSceneEntity::GenerateSceneForSound:  Alloc of actor, channel, "
              "or event failed!!!\n");
      delete scene;
      return NULL;
    }
...
bool CSceneEntity::GetSoundNameForPlayer(CChoreoEvent *event,
                                         ...
                                         size_t buflen, CBaseEntity *pActor) {
  Assert(event);
  Assert(player);
  Assert(buf);
  Assert(buflen > 0);
...
#if !defined(_RETAIL)
static ConVar
    scene_async_prefetch_spew("scene_async_prefetch_spew", "0", 0,
  ...
  case CChoreoEvent::MOVETO: {
    ...
    if (pActor && !IsMultiplayer()) {
      ...
      if (event->GetParameters3() && strlen(event->GetParameters3()) > 0) {
        pActor2 = FindNamedEntityClosest(event->GetParameters(), pActor, false,
...
float InstancedAutoGeneratedSoundScene(CBaseFlex *pActor, char const *soundname,
  ...
  if (!pActor) {
    Warning("InstancedAutoGeneratedSoundScene:  Expecting non-NULL pActor for "
            "sound %s\n",
            soundname);
    return 0;
  }
...
void PrecacheInstancedScene(char const *pszScene) {
  ...
  if (!scenefilecache->GetSceneCachedData(pszScene, &sceneData)) {
    ...
    if (developer.GetInt() &&
      ...
      Warning("PrecacheInstancedScene: Missing scene '%s' from scene image "
...
void CSceneManager::RemoveScenesInvolvingActor(CBaseFlex *pActor) {
  ...
  for (int i = 0; i < c; i++) {
    ...
    if (pScene->InvolvesActor(
            pActor)) // NOTE: returns false if scene hasn't loaded yet
    {
      ...
      CInstancedSceneEntity *pInstancedScene =
          dynamic_cast<CInstancedSceneEntity *>(pScene);
        ...
        if (m_hScenes[i]) {
          ...
          if (pScene) {
            ...
            if (pList) {
              ...
              Warning("%s(%s) found an entity that wasn't a "
                      "logic_choreographed_scene or logic_scene_list_manager "
                      "in slot %d, named %s\n",
