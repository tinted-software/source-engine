...
float CResponseSystem::ScoreCriteriaAgainstRuleCriteria(
  ...
  if (Compare(actualValue, c, verbose)) {
    ...
    if (verbose) {
      DevMsg("matched, weight %4.2f (s %4.2f x c %4.2f)", score, w,
             c->weight.GetFloat());
    }
  } else {
    if (c->required) {
...
void CResponseSystem::DescribeResponseGroup(ResponseGroup *group, int selected,
  ...
  for (int i = 0; i < c; i++) {
    ...
    DebugPrint(depth + 1, "%s%20s : %40s %5.3f\n",
               i == selected ? "-> " : "   ",
               AI_Response::DescribeResponse(r->GetType()), r->value,
               r->weight.GetFloat());
  }
}
...
void CResponseSystem::LoadFromBuffer(const char *scriptfile, const char *buffer,
  ...
  while (1) {
    ...
    if (!Q_stricmp(token, "#include")) {
      ...
      Error("CResponseSystem::LoadFromBuffer:  Unknown entry type '%s', "
            "expecting 'response', 'criterion', 'enumeration' or 'rules' in "
            "file %s(offset:%i)\n",
...
public:
  ...
  IResponseSystem *PrecacheCustomResponseSystem(const char *scriptfile) {
    ...
    if (!sys) {
      ...
      if (!sys->Init()) {
        Error("CInstancedResponseSystem:  Failed to init response system from "
              "%s!",
              scriptfile);
      }
...
IResponseSystem *CDefaultResponseSystem::BuildCustomResponseSystemGivenCriteria(
  ...
  if (!pCustomSystem) {
    Error("BuildCustomResponseSystemGivenCriterea: Failed to create custom "
          "response system %s!",
          pszCustomName);
  }
