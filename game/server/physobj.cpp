...
// This is a physically simulated spring, used to join objects together and
// create spring forces
//
// NOTE: Springs are not physical in the sense that they only create force, they
// do not collide with anything or have any REAL constraints.  They can be
// stretched infinitely (though this will create and infinite force), they can
// penetrate any other object (or spring). They do not occupy any space.
//
...
public:
  ...
  void GetSpringObjectConnections(string_t nameStart, string_t nameEnd,
...
private:
  ...
  // These are "template" values used to construct the spring.  After creation,
  // they are not needed
  float m_tempConstant;
  float m_tempLength; // This is the "ideal" length of the spring, not the
                      // length it is currently stretched to.
  float m_tempDamping;
...
DEFINE_PHYSPTR(m_pSpring),
    ...
    // debug function - slow, uses dynamic_cast<> - use this to query the
    // attached objects physics_debug_entity toggles the constraint system for
    // an object using this
    bool GetSpringAttachments(CBaseEntity *pEntity, CBaseEntity *pAttachOut[2],
...
bool PointIsNearer(IPhysicsObject *pObject1, const Vector &point1,
                   const Vector &point2) {
  Vector center;
...
void CPhysicsSpring::GetSpringObjectConnections(string_t nameStart,
                                                string_t nameEnd,
                                                IPhysicsObject **pStart,
                                                IPhysicsObject **pEnd) {
  IPhysicsObject *pStartObject =
      FindPhysicsObjectByName(STRING(nameStart), this);
...
void CPhysicsSpring::Activate(void) {
  ...
  if (!m_pSpring) {
    ...
    GetSpringObjectConnections(m_nameAttachStart, m_nameAttachEnd, &pStart,
                               &pEnd);
...
void CPhysicsSpring::NotifySystemEvent(
  ...
  PhysTeleportConstrainedEntity(
      pNotify, m_pSpring->GetStartObject(), m_pSpring->GetEndObject(),
...
DEFINE_FIELD(m_hCarryingPlayer, FIELD_EHANDLE),
    ...
    DEFINE_KEYFIELD(m_flForceToEnableMotion, FIELD_FLOAT,
                    "forcetoenablemotion"),
    DEFINE_KEYFIELD(m_angPreferredCarryAngles, FIELD_VECTOR,
    ...
    // UNDONE: Save/Restore needs to take the physics object's properties into
    // account UNDONE: Acceleration, velocity, angular velocity, etc. must be
    // preserved UNDONE: Many of these quantities are relative to a coordinate
    // frame UNDONE: Translate when going across transitions? UNDONE: Build
    // transition transformation, and transform data in save/restore for
    // IPhysicsObject UNDONE: Angles are saved in the entity, but not propagated
    // back to the IPhysicsObject on restore
...
bool CPhysBox::CreateVPhysics() {
  ...
  if (tmpSolid.params.rotdamping < 1.0f &&
      ShouldDampRotation(pVCollide->solids[0])) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CPhysBox::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
...
bool CPhysBox::CanBePickedUpByPhyscannon() {
  ...
  if (!pPhysicsObject->IsMotionEnabled() &&
      !HasSpawnFlags(SF_PHYSBOX_ENABLE_ON_PHYSCANNON))
    return false;
...
//-----------------------------------------------------------------------------
// Purpose: If we're being held by the player's hand/physgun, force it to drop
// us
//-----------------------------------------------------------------------------
...
// Update the visible representation of the physic system's representation of
// this object
...
void CPhysBox::VPhysicsCollision(int index, gamevcollisionevent_t *pEvent) {
  ...
  if (m_flForceToEnableMotion) {
    ...
    if ((pOther && pOther->IsPlayer() &&
         HasSpawnFlags(SF_PHYSBOX_PREVENT_PLAYER_TOUCH_ENABLE)) == false) {
      // Large enough to enable motion?
      float flForce =
          pEvent->collisionSpeed * pEvent->pObjects[!index]->GetMass();
...
CBaseEntity *CPhysExplosion::FindEntity(CBaseEntity *pEntity,
  ...
  if (m_targetEntityName != NULL_STRING) {
    ...
    CBaseEntity *pTarget = gEntList.FindEntityByName(
        pEntity, m_targetEntityName, NULL, pActivator, pCaller);
    if (pTarget != NULL)
      return pTarget;

    // Failing that, try a classname
    return gEntList.FindEntityByClassnameWithin(
        pEntity, STRING(m_targetEntityName), GetAbsOrigin(), GetRadius());
  }
...
void CPhysExplosion::Explode(CBaseEntity *pActivator, CBaseEntity *pCaller) {
  ...
  // I've removed the traceline heuristic from phys explosions. SO right now
  // they will affect entities through walls. (sjb) UNDONE: Try tracing
  // world-only?
    ...
    if (pEntity->m_takedamage != DAMAGE_NO &&
      ...
      if (m_radius == 0 || flDist <= m_radius) {
        if (HasSpawnFlags(SF_PHYSEXPLOSION_TEST_LOS)) {
          ...
          trace_t tr;
          UTIL_TraceLine(vecStartPos, pEntity->BodyTarget(vecStartPos, false),
        ...
        if (HasSpawnFlags(SF_PHYSEXPLOSION_PUSH_PLAYER)) {
          if (pEntity->IsPlayer()) {
            Vector vecPushDir =
                (pEntity->BodyTarget(GetAbsOrigin(), false) - GetAbsOrigin());
            float dist = VectorNormalize(vecPushDir);

            float flFalloff =
                RemapValClamped(dist, m_radius, m_radius * 0.75f, 0.0f, 1.0f);
...
int CPhysExplosion::DrawDebugTextOverlays(void) {
  ...
  if (m_debugOverlays & OVERLAY_TEXT_BIT) {
    ...
    // print target entity
    Q_snprintf(tempstr, sizeof(tempstr), "    limit to: %s",
               STRING(m_targetEntityName));
    EntityText(text_offset, tempstr, 0);
    text_offset++;
  }
  return text_offset;
...
void CPhysImpact::InputImpact(inputdata_t &inputdata) {
  ...
  float dist = HasSpawnFlags(bitsPHYSIMPACT_INFINITE_LENGTH) ? MAX_TRACE_LENGTH
                                                             : m_distance;
  ...
  if (trace.startsolid) {
    // ep1_citadel_04 has a phys_impact just behind another entity, so if we
    // startsolid then bump out just a little and retry the trace
    Vector startOffset = start + (dir * 0.1);
    UTIL_TraceLine(startOffset, end, MASK_SHOT, this, COLLISION_GROUP_NONE,
                   &trace);
  }
  ...
  if (trace.fraction != 1.0) {
    ...
    if ((pEnt != NULL) && (pPhysics != NULL)) {
      ...
      float damage = HasSpawnFlags(bitsPHYSIMPACT_NOFALLOFF |
                                   bitsPHYSIMPACT_INFINITE_LENGTH)
                         ? m_damage
      ...
      if (debug_physimpact.GetBool()) {
        NDebugOverlay::Line(trace.endpos,
                            trace.endpos + trace.plane.normal * -128, 255, 0, 0,
                            false, 30);
      }

      // Legacy entities applied the force along the impact normal, which
      // yielded unpredictable results.
      if (!HasSpawnFlags(bitsPHYSIMPACT_IGNORE_NORMAL)) {
        dir = -trace.plane.normal;
      }

      pPhysics->ApplyForceOffset(damage * dir * phys_pushscale.GetFloat(),
                                 trace.endpos);
    }
  }
}
...
// UNDONE: Is this worth it?, just recreate the object instead? (that happens
// when this returns false anyway) recreating works, but is more expensive and
// won't inherit properties (velocity, constraints, etc)
...
static CBaseEntity *CreateSimplePhysicsObject(CBaseEntity *pEntity,
                                              bool createAsleep,
                                              bool createAsDebris) {
...
DEFINE_KEYFIELD(m_swapModel, FIELD_STRING, "swapmodel"),
  ...
  CBaseEntity *pSwap = gEntList.FindEntityByName(
      NULL, m_swapModel, NULL, inputdata.pActivator, inputdata.pCaller);
  CBaseEntity *pEntity = NULL;

  int count = 0;
  while ((pEntity = gEntList.FindEntityByName(pEntity, m_target, NULL,
  ...
  // multiple objects with the same brush model will render, but the dynamic
  // lights and decals will be shared between the two instances...
  if (pSwap && count > 0) {
    count = 1;
  }
  ...
  for (int i = 0; i < count; i++) {
    ...
    CBaseEntity *pPhys =
        CreateSimplePhysicsObject(pEntity, createAsleep, createAsDebris);
    if (pPhys) {
...
// BUGBUG: This won't work!  Right now you can't save physics pointers inside an
// embedded type!
...
DEFINE_PHYSPTR(pConstraint), DEFINE_FIELD(hEntity, FIELD_EHANDLE),
    ...
    //-----------------------------------------------------------------------------
    // Purpose: SendProxy that converts the magnet's attached object UtlVector
    // to entindexes
    //-----------------------------------------------------------------------------
    void SendProxy_MagnetAttachedObjectList(const void *pStruct,
                                            const void *pData, DVariant *pOut,
                                            int iElement, int objectID) {
  CPhysMagnet *pMagnet = (CPhysMagnet *)pData;

  // If this assertion fails, then SendProxyArrayLength_MagnetAttachedArray must
  // have failed.
...
int SendProxyArrayLength_MagnetAttachedArray(const void *pStruct,
                                             int objectID) {
...
SendPropArray2(
        ...
        SendPropInt("magnetattached_array_element", 0, 4, 10, SPROP_UNSIGNED,
SendProxy_MagnetAttachedObjectList), 128, 0, "magnetattached_array"
        )
*/
...
void CPhysMagnet::VPhysicsCollision(int index, gamevcollisionevent_t *pEvent) {
  ...
  // This is a hack to solve players (Erik) stacking stuff on their jeeps in
  // coast_01 and being screwed when the crane can't pick them up. We need to
  ...
  if (HasSpawnFlags(SF_MAGNET_COAST_HACK)) {
    ...
    if (!FClassnameIs(pOther, "prop_vehicle_jeep")) {
      ...
      if (pOther->m_takedamage != DAMAGE_NO &&
        ...
        CTakeDamageInfo info(this, this, pOther->GetHealth(),
                             DMG_GENERIC | DMG_PREVENT_PHYSICS_FORCE);
  ...
  if (cTexType != CHAR_TEX_METAL && cTexType != CHAR_TEX_COMPUTER) {
    ...
    // If we have a model that wants to be metal, even though we hit a non-metal
    // texture, we'll stick to it
    if (Q_strncmp(Studio_GetDefaultSurfaceProps(
                      pOther->GetBaseAnimating()->GetModelPtr()),
                  "metal", 5))
      return;
  }

  IPhysicsObject *pPhysics = pOther->VPhysicsGetObject();
  if (pPhysics && pOther->GetMoveType() == MOVETYPE_VPHYSICS &&
      pPhysics->IsMoveable()) {
    ...
    if (HasSpawnFlags(SF_MAGNET_ALLOWROTATION)) {
      ...
      pMagnetPhysObject->WorldToLocal(&ballsocket.constraintPosition[0],
                                      vecCollisionPoint);
      pPhysics->WorldToLocal(&ballsocket.constraintPosition[1],
                             vecCollisionPoint);

      // newEntityOnMagnet.pConstraint = physenv->CreateBallsocketConstraint(
      // pMagnetPhysObject, pPhysics, m_pConstraintGroup, ballsocket );
      newEntityOnMagnet.pConstraint = physenv->CreateBallsocketConstraint(
          pMagnetPhysObject, pPhysics, NULL, ballsocket);
    } else {
      ...
      // newEntityOnMagnet.pConstraint = physenv->CreateFixedConstraint(
      // pMagnetPhysObject, pPhysics, m_pConstraintGroup, fixed );
      newEntityOnMagnet.pConstraint = physenv->CreateFixedConstraint(
          pMagnetPhysObject, pPhysics, NULL, fixed);
    }
...
void CPhysMagnet::DoMagnetSuck(CBaseEntity *pOther) {
  ...
  for (int i = 0; i < iNumEntities; i++) {
    ...
    if (pPhys && pEntity->GetMoveType() == MOVETYPE_VPHYSICS &&
        pPhys->GetMass() < 5000) {
      // Do we have line of sight to it?
      trace_t tr;
      UTIL_TraceLine(GetAbsOrigin(), pEntity->GetAbsOrigin(), MASK_SHOT, this,
                     0, &tr);
...
void CPhysMagnet::ConstraintBroken(IPhysicsConstraint *pConstraint) {
  ...
  int iCount = m_MagnettedEntities.Count();
  for (int i = 0; i < iCount; i++) {
    if (m_MagnettedEntities[i].hEntity.Get() != NULL &&
        m_MagnettedEntities[i].pConstraint == pConstraint) {
      IPhysicsObject *pPhysObject =
          m_MagnettedEntities[i].hEntity->VPhysicsGetObject();
...
private:
  ...
  float m_flMagnitude;
  float m_flRadius;
  float m_flInnerRadius; // Inner radius where the push eminates from (on a
                         // sphere)
};
...
void CPointPush::PushEntity(CBaseEntity *pTarget) {
  ...
  float flFalloff =
      (HasSpawnFlags(SF_PUSH_NO_FALLOFF))
          ? 1.0f
          : RemapValClamped(dist, m_flRadius, m_flRadius * 0.25f, 0.0f, 1.0f);
  ...
  case MOVETYPE_VPHYSICS: {
    ...
    if (pPhys) {
      ...
      pPhys->ApplyForceCenter(m_flMagnitude * flFalloff * 100.0f * vecPushDir *
                              pPhys->GetMass() * gpGlobals->frametime);
      return;
    }
  } break;
...
void CPointPush::PushThink(void) {
  ...
  int numEnts =
      UTIL_EntitiesInSphere(pEnts, 256, GetAbsOrigin(), m_flRadius, 0);
  for (int i = 0; i < numEnts; i++) {
    ...
    if (pEnts[i]->GetMoveType() == MOVETYPE_VPHYSICS &&
        HasSpawnFlags(SF_PUSH_PHYSICS) == false)
      continue;
    ...
    if (HasSpawnFlags(SF_PUSH_TEST_LOS)) {
      ...
      trace_t tr;
      UTIL_TraceLine(vecStartPos, pEnts[i]->BodyTarget(vecStartPos, false),
                     MASK_SOLID_BRUSHONLY, this, COLLISION_GROUP_NONE, &tr);
