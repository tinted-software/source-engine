...
private:
  ...
  // Makes sure we've picked the right position along the path if we're chasing
  // an enemy
  ...
  // distance
  CPathTrack *FindClosestPointOnPath(CPathTrack *pPath, const Vector &targetPos,
  ...
  void ComputePointFromPerpDistance(const Vector &vecPointOnPath,
                                    const Vector &vecPathDir, float flPerpDist,
                                    Vector *pResult);
...
private:
  ...
  CHandle<CPathTrack>
      m_pTargetNearestPath; // Used only by leading, it specifies the path point
                            // *after* where the target is
  ...
  bool m_bForcedMove; // Means the destination point must be reached regardless
                      // of enemy position
  ...
  float m_flTargetDistanceThreshold; // Distance threshold used to determine
                                     // when a target has moved enough to update
  ...
  Vector m_vecSegmentStartSplinePoint; // Used to define a spline which is used
                                       // to compute path velocity
  ...
  float m_flTargetDistFromPath; // How far is the target from the closest point
                                // on the path?
  float m_flLeadDistance;
  Vector m_vecTargetPathDir;
  Vector
      m_vecTargetPathPoint; // What point on the path is closest to the target?
