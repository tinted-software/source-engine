...
//-----------------------------------------------------------------------------
// Purpose: An entity that mapmakers can use to ensure there's a required entity
// never runs out.
...
public:
  ...
  void SpawnEntity(Vector vecAlternateOrigin = vec3_invalid,
                   QAngle vecAlternateAngles = vec3_angle);
...
DEFINE_FIELD(m_hCurrentInstance, FIELD_EHANDLE),
    ...
    DEFINE_KEYFIELD(m_angPostSpawnDirection, FIELD_VECTOR,
                    "PostSpawnDirection"),
    DEFINE_KEYFIELD(m_flPostSpawnDirectionVariance, FIELD_FLOAT,
                    "PostSpawnDirectionVariance"),
    DEFINE_KEYFIELD(m_flPostSpawnSpeed, FIELD_FLOAT, "PostSpawnSpeed"),
    DEFINE_KEYFIELD(m_bPostSpawnUseAngles, FIELD_BOOLEAN,
    ...
    DEFINE_INPUTFUNC(FIELD_STRING, "ForceSpawnAtEntityOrigin",
...
CPointTemplate *CEnvEntityMaker::FindTemplate() {
  ...
  CPointTemplate *pTemplate = dynamic_cast<CPointTemplate *>(
      gEntList.FindEntityByName(NULL, STRING(m_iszTemplate)));
  if (!pTemplate) {
...
//-----------------------------------------------------------------------------
void CEnvEntityMaker::SpawnEntity(Vector vecAlternateOrigin,
  ...
  if (m_vecEntityMins == vec3_origin) {
    m_hCurrentInstance->CollisionProp()->WorldSpaceAABB(&m_vecEntityMins,
                                                        &m_vecEntityMaxs);
  ...
  if (m_flPostSpawnSpeed) {
    for (int i = 0; i < hNewEntities.Count(); i++) {
      ...
      vecShootDir +=
          vRight * random->RandomFloat(-1, 1) * m_flPostSpawnDirectionVariance;
      vecShootDir += vForward * random->RandomFloat(-1, 1) *
                     m_flPostSpawnDirectionVariance;
      vecShootDir +=
          vUp * random->RandomFloat(-1, 1) * m_flPostSpawnDirectionVariance;
...
bool CEnvEntityMaker::HasRoomToSpawn() {
  ...
  trace_t tr;
  UTIL_TraceHull(GetAbsOrigin(), GetAbsOrigin(), m_vecEntityMins,
                 m_vecEntityMaxs, MASK_SOLID, this, COLLISION_GROUP_NONE, &tr);
