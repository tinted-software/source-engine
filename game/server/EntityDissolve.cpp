...
//
// Purpose: Dissolve entity to be attached to target entity. Serves two
// purposes:
//
...
DEFINE_FIELD(m_flStartTime, FIELD_TIME),
    ...
    SendPropInt(SENDINFO(m_nDissolveType), ENTITY_DISSOLVE_BITS,
                SPROP_UNSIGNED),
...
void CEntityDissolve::Spawn() {
  ...
  if ((m_nDissolveType == ENTITY_DISSOLVE_ELECTRICAL) ||
    ...
    if (dynamic_cast<CRagdollProp *>(GetMoveParent())) {
      SetContextThink(&CEntityDissolve::ElectrocuteThink,
                      gpGlobals->curtime + 0.01f, s_pElectroThinkContext);
    }
  }
  ...
  m_flFadeOutModelLength =
      DISSOLVE_FADE_OUT_MODEL_END_TIME - DISSOLVE_FADE_OUT_MODEL_START_TIME;
...
void CEntityDissolve::InputDissolve(inputdata_t &inputdata) {
  ...
  while ((pTarget = gEntList.FindEntityGeneric(pTarget, STRING(strTarget), this,
                                               inputdata.pActivator)) != NULL) {
    CBaseAnimating *pBaseAnim = pTarget->GetBaseAnimating();
    if (pBaseAnim) {
      pBaseAnim->Dissolve(NULL, gpGlobals->curtime, false, m_nDissolveType,
                          GetAbsOrigin(), m_nMagnitude);
    }
  }
}
...
// Input  : pTarget -
//-----------------------------------------------------------------------------
CEntityDissolve *CEntityDissolve::Create(CBaseEntity *pTarget,
                                         const char *pMaterialName,
                                         float flStartTime, int nDissolveType,
  ...
  CEntityDissolve *pDissolve =
      (CEntityDissolve *)CreateEntityByName("env_entity_dissolver");

  if (pDissolve == NULL)
    return NULL;

  pDissolve->m_nDissolveType = nDissolveType;
  if ((nDissolveType == ENTITY_DISSOLVE_ELECTRICAL) ||
    ...
    if (pTarget->IsNPC() && pTarget->MyNPCPointer()->CanBecomeRagdoll()) {
      ...
      CBaseEntity *pRagdoll = CreateServerRagdoll(
          pTarget->MyNPCPointer(), 0, info, COLLISION_GROUP_DEBRIS, true);
      pRagdoll->SetCollisionBounds(pTarget->CollisionProp()->OBBMins(),
                                   pTarget->CollisionProp()->OBBMaxs());

      // Necessary to cause it to do the appropriate death cleanup
      if (pTarget->m_lifeState == LIFE_ALIVE) {
        CBasePlayer *pPlayer = UTIL_PlayerByIndex(1);
        CTakeDamageInfo ragdollInfo(pPlayer, pPlayer, 10000.0,
                                    DMG_SHOCK | DMG_REMOVENORAGDOLL |
                                        DMG_PREVENT_PHYSICS_FORCE);
...
CEntityDissolve *CEntityDissolve::Create(CBaseEntity *pTarget,
  ...
  for (CBaseEntity *pChild = pSource->FirstMoveChild(); pChild;
       pChild = pChild->NextMovePeer()) {
    CEntityDissolve *pDissolve = dynamic_cast<CEntityDissolve *>(pChild);
    if (!pDissolve)
      continue;

    return Create(pTarget, STRING(pDissolve->GetModelName()),
                  pDissolve->m_flStartTime, pDissolve->m_nDissolveType);
  }
...
void CEntityDissolve::DissolveThink(void) {
  ...
  if (dt >= m_flFadeOutStart + m_flFadeOutLength) {
    ...
    CTakeDamageInfo info(pPlayer, pPlayer, 10000.0,
                         DMG_GENERIC | DMG_REMOVENORAGDOLL | iNoPhysicsDamage);
...
void CEntityDissolve::ElectrocuteThink(void) {
  ...
  SetContextThink(&CEntityDissolve::ElectrocuteThink,
                  gpGlobals->curtime + random->RandomFloat(0.1, 0.2f),
