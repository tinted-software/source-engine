...
bool BasicGameStatsRecord_t::ParseFromBuffer(CUtlBuffer &buf,
  ...
  // Note, don't put the buf.GetInt() in the macro since it'll get evaluated
  // twice!!!
...
void BasicGameStats_t::SaveToBuffer(CUtlBuffer &buf) {
  ...
  buf.PutInt(c);
  for (int i = m_MapTotals.First(); i != m_MapTotals.InvalidIndex();
       i = m_MapTotals.Next(i)) {
...
BasicGameStats_t::FindOrAddRecordForMap(char const *mapname) {
...
bool BasicGameStats_t::ParseFromBuffer(CUtlBuffer &buf,
                                       int iBufferStatsVersion) {
  bool bret = true;
