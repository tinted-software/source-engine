...
void CServerNetworkProperty::AttachEdict(edict_t *pRequiredEdict) {
  ...
  // see if there is an edict allocated for it, otherwise get one from the
  // engine
  if (!pRequiredEdict) {
...
void CServerNetworkProperty::RecomputePVSInformation() {
  if (m_pPev &&
      ((m_pPev->m_fStateFlags & FL_EDICT_DIRTY_PVS_INFORMATION) != 0)) {
...
// PVS rules
//-----------------------------------------------------------------------------
bool CServerNetworkProperty::IsInPVS(const edict_t *pRecipient, const void *pvs,
                                     int pvssize) {
  ...
  {
    return (engine->CheckHeadnodeVisible(m_PVSInfo.m_nHeadNode, pPVS,
                                         pvssize) != 0);
  }

  for (int i = m_PVSInfo.m_nClusterCount; --i >= 0;) {
    if (pPVS[m_PVSInfo.m_pClusters[i] >> 3] &
        (1 << (m_PVSInfo.m_pClusters[i] & 7)))
      return true;
  }
...
bool CServerNetworkProperty::IsInPVS(const CCheckTransmitInfo *pInfo) {
  ...
  if (!m_PVSInfo.m_nAreaNum2) {
    for (i = 0; i < pInfo->m_AreasNetworked; i++) {
      ...
      if (clientArea == m_PVSInfo.m_nAreaNum ||
          engine->CheckAreasConnected(clientArea, m_PVSInfo.m_nAreaNum))
        break;
    }
  } else {
    ...
    for (i = 0; i < pInfo->m_AreasNetworked; i++) {
      int clientArea = pInfo->m_Areas[i];
      if (clientArea == m_PVSInfo.m_nAreaNum ||
          clientArea == m_PVSInfo.m_nAreaNum2)
        break;
