...
#define SF_CONSTRAINT_NO_CONNECT_UNTIL_ACTIVATED                               \
  0x0010 // Will only check the two attached entities at activation

ConVar
    g_debug_constraint_sounds("g_debug_constraint_sounds", "0", FCVAR_CHEAT,
...
public:
  CConstraintAnchor() { m_massScale = 1.0f; }
  void Spawn(void) {
    if (GetParent()) {
...
DEFINE_PHYSPTR(m_pMachine),
    DEFINE_KEYFIELD(m_additionalIterations, FIELD_INTEGER,
...
void PhysTeleportConstrainedEntity(CBaseEntity *pTeleportSource,
  ...
  // figure out which entity needs to be fixed up (the one that isn't
  // pTeleportSource)
  ...
  if (!pFixup->VPhysicsGetObject() ||
      !pFixup->VPhysicsGetObject()->IsMoveable())
    return;
...
static void DrawPhysicsBounds(IPhysicsObject *pObject, int r, int g, int b,
                              int a) {
  ...
  Vector mins, maxs;
  physcollision->CollideGetAABB(&mins, &maxs, pCollide, vec3_origin,
                                vec3_angle);
...
static void DrawConstraintObjectsAxes(CBaseEntity *pConstraintEntity,
...
void CPhysConstraint::Deactivate() {
  ...
  if (m_spawnflags & SF_CONSTRAINT_DISABLE_COLLISION) {
    // constraint may be getting deactivated because an object got deleted, so
    // check them here.
...
void CPhysConstraint::OnBreak(void) {
  ...
  // The Deactivate() call should make sure we don't get more of these
  // callbacks.
...
void CPhysConstraint::InputOnBreak(inputdata_t &inputdata) { OnBreak(); }
...
void CPhysConstraint::InputTurnOn(inputdata_t &inputdata) {
  ...
  if (!m_pConstraint || !m_pConstraint->GetReferenceObject() ||
...
void CPhysConstraint::InputTurnOff(inputdata_t &inputdata) { Deactivate(); }
...
int CPhysConstraint::DrawDebugTextOverlays() {
  ...
  if (m_pConstraint && (m_debugOverlays & OVERLAY_TEXT_BIT)) {
    ...
    if ((params.bodyMassScale[0] != 1.0f && params.bodyMassScale[0] != 0.0f) ||
      ...
      NDebugOverlay::EntityTextAtPosition(GetAbsOrigin(), pos, str.Access(), 0,
                                          255, 255, 0, 255);
    }
    pos++;
  }
...
void CPhysConstraint::GetBreakParams(constraint_breakableparams_t &params,
...
DEFINE_PHYSPTR(m_pConstraint),
    ...
    DEFINE_KEYFIELD(m_minTeleportDistance, FIELD_FLOAT,
...
// debug function - slow, uses dynamic_cast<> - use this to query the attached
// objects physics_debug_entity toggles the constraint system for an object
// using this
bool GetConstraintAttachments(CBaseEntity *pEntity, CBaseEntity *pAttachOut[2],
  ...
  if (pConstraintEntity) {
    ...
    if (pConstraint) {
      ...
      pAttachOut[0] =
          pRef ? static_cast<CBaseEntity *>(pRef->GetGameData()) : NULL;
      IPhysicsObject *pAttach = pConstraint->GetAttachedObject();
      pAttachVPhysics[1] = pAttach;
      pAttachOut[1] =
          pAttach ? static_cast<CBaseEntity *>(pAttach->GetGameData()) : NULL;
      return true;
    }
  }
  return false;
    ...
    if (pEntity) {
      ...
      if (pAnchor->parentAttachment > 0) {
        ...
        if (pAnim) {
          ...
          int listCount =
              pAnchor->hEntity->VPhysicsGetObjectList(list, ARRAYSIZE(list));
          int iPhysicsBone = pAnim->GetPhysicsBone(
              pAnim->GetAttachmentBone(pAnchor->parentAttachment));
...
static IPhysicsConstraintGroup *
GetRagdollConstraintGroup(IPhysicsObject *pObj) {
  if (pObj) {
...
void CPhysConstraint::GetConstraintObjects(hl_constraint_info_t &info) {
  ...
  if (info.pObjects[0] == NULL && info.pObjects[1]) {
    ...
    info.massScale[0] = info.massScale[1] =
        1.0f; // no mass scale on world constraint
    ...
    info.pObjects[0] =
        g_PhysWorldObject; // Try to make the world object consistently object0
...
IPhysicsConstraintGroup *GetConstraintGroup(string_t systemName) {
  ...
  if (pMachine) {
    CPhysConstraintSystem *pGroup =
        dynamic_cast<CPhysConstraintSystem *>(pMachine);
    if (pGroup) {
...
bool CPhysConstraint::ActivateConstraint(void) {
  ...
  // The constraint is specified in the coordinate frame of the "reference"
  // object and constrains the "attached" object
  ...
  if (info.pObjects[0]->IsStatic() && info.pObjects[1]->IsStatic()) {
    Warning("Constraint (%s) attached to two static objects (%s and %s)!!!\n",
            STRING(GetEntityName()), STRING(m_nameAttach1),
            m_nameAttach2 == NULL_STRING ? "world" : STRING(m_nameAttach2));
    return false;
  }

  if (info.pObjects[0]->GetShadowController() &&
      info.pObjects[1]->GetShadowController()) {
...
void CPhysConstraint::NotifySystemEvent(
  ...
  if (eventType != NOTIFY_EVENT_TELEPORT ||
      (unsigned int)gpGlobals->tickcount == m_teleportTick)
    return;

  float distance =
      (params.pTeleport->prevOrigin - pNotify->GetAbsOrigin()).Length();
  ...
  PhysTeleportConstrainedEntity(
      pNotify, m_pConstraint->GetReferenceObject(),
      m_pConstraint->GetAttachedObject(), params.pTeleport->prevOrigin,
      params.pTeleport->prevAngles, params.pTeleport->physicsRotate);
}
...
public:
  ...
  IPhysicsConstraint *CreateConstraint(IPhysicsConstraintGroup *pGroup,
    ...
    return physenv->CreateHingeConstraint(info.pObjects[0], info.pObjects[1],
                                          pGroup, m_hinge);
  }
  ...
  void DrawDebugGeometryOverlays() {
    if (m_debugOverlays &
        (OVERLAY_BBOX_BIT | OVERLAY_PIVOT_BIT | OVERLAY_ABSBOX_BIT)) {
      NDebugOverlay::Line(m_hinge.worldPosition,
                          m_hinge.worldPosition +
                              48 * m_hinge.worldAxisDirection,
                          0, 255, 0, false, 0);
    }
    BaseClass::DrawDebugGeometryOverlays();
  }
  ...
  void InputSetVelocity(inputdata_t &inputdata) {
    ...
    m_pConstraint->SetAngularMotor(speed, speed * loadscale * massLoad *
                                              loadscale *
                                              (1.0 / TICK_INTERVAL));
  }
  ...
  void NotifyVPhysicsStateChanged(IPhysicsObject *pPhysics,
    ...
    // if something woke up, start thinking. If everything is asleep, stop
    // thinking.
    if (bAwake) {
  ...
  virtual void OnConstraintSetup(hl_constraint_info_t &info) {
    CBaseEntity *pEntity0 =
        info.pObjects[0]
            ? static_cast<CBaseEntity *>(info.pObjects[0]->GetGameData())
            : NULL;
    ...
    CBaseEntity *pEntity1 =
        info.pObjects[1]
            ? static_cast<CBaseEntity *>(info.pObjects[1]->GetGameData())
            : NULL;
...
#if HINGE_NOTIFY
    DEFINE_KEYFIELD(
        m_soundInfo.m_soundProfile
            .m_keyPoints[SimpleConstraintSoundProfile::kMIN_THRESHOLD],
        FIELD_FLOAT, "minSoundThreshold"),
    DEFINE_KEYFIELD(m_soundInfo.m_soundProfile
                        .m_keyPoints[SimpleConstraintSoundProfile::kMIN_FULL],
                    FIELD_FLOAT, "maxSoundThreshold"),
    DEFINE_KEYFIELD(m_soundInfo.m_iszTravelSoundFwd, FIELD_SOUNDNAME,
                    "slidesoundfwd"),
    DEFINE_KEYFIELD(m_soundInfo.m_iszTravelSoundBack, FIELD_SOUNDNAME,
                    "slidesoundback"),

    DEFINE_KEYFIELD(m_soundInfo.m_iszReversalSounds[0], FIELD_SOUNDNAME,
                    "reversalsoundSmall"),
    DEFINE_KEYFIELD(m_soundInfo.m_iszReversalSounds[1], FIELD_SOUNDNAME,
                    "reversalsoundMedium"),
    DEFINE_KEYFIELD(m_soundInfo.m_iszReversalSounds[2], FIELD_SOUNDNAME,
                    "reversalsoundLarge"),

    DEFINE_KEYFIELD(m_soundInfo.m_soundProfile.m_reversalSoundThresholds[0],
                    FIELD_FLOAT, "reversalsoundthresholdSmall"),
    DEFINE_KEYFIELD(m_soundInfo.m_soundProfile.m_reversalSoundThresholds[1],
                    FIELD_FLOAT, "reversalsoundthresholdMedium"),
    DEFINE_KEYFIELD(m_soundInfo.m_soundProfile.m_reversalSoundThresholds[2],
                    FIELD_FLOAT, "reversalsoundthresholdLarge"),
...
void CPhysHinge::Spawn(void) {
  ...
  if (HasSpawnFlags(SF_CONSTRAINT_ASSUME_WORLD_GEOMETRY)) {
    ...
    if (m_nameAttach1 == NULL_STRING) {
      params.SnapToAxis(m_nameAttach2, m_hinge.worldPosition,
                        m_hinge.worldAxisDirection);
      PhysSetMassCenterOverride(params);
    } else if (m_nameAttach2 == NULL_STRING) {
      params.SnapToAxis(m_nameAttach1, m_hinge.worldPosition,
                        m_hinge.worldAxisDirection);
      PhysSetMassCenterOverride(params);
    } else {
      RemoveSpawnFlags(SF_CONSTRAINT_ASSUME_WORLD_GEOMETRY);
...
bool CPhysHinge::IsWorldHinge(const hl_constraint_info_t &info, int *pAxisOut) {
  if (HasSpawnFlags(SF_CONSTRAINT_ASSUME_WORLD_GEOMETRY) &&
      info.pObjects[0] == g_PhysWorldObject) {
    Vector localHinge;
    info.pObjects[1]->WorldToLocalVector(&localHinge,
                                         m_hinge.worldAxisDirection);
...
void CPhysHinge::SoundThink(void) {
  ...
  IPhysicsObject *pAttached = m_pConstraint->GetAttachedObject(),
                 *pReference = m_pConstraint->GetReferenceObject();
  Assert(pAttached && pReference);
  if (pAttached && pReference) {
    Vector relativeVel =
        VelocitySampler::GetRelativeAngularVelocity(pAttached, pReference);
    if (g_debug_constraint_sounds.GetBool()) {
      NDebugOverlay::Line(GetAbsOrigin(), GetAbsOrigin() + (relativeVel), 255,
                          255, 0, true, 0.1f);
    }
...
public:
  ...
  IPhysicsConstraint *CreateConstraint(IPhysicsConstraintGroup *pGroup,
    ...
    for (int i = 0; i < 2; i++) {
      info.pObjects[i]->WorldToLocal(&ballsocket.constraintPosition[i],
                                     GetAbsOrigin());
    }
    ...
    return physenv->CreateBallsocketConstraint(
        info.pObjects[0], info.pObjects[1], pGroup, ballsocket);
  }
};
...
public:
  ...
  void InputSetVelocity(inputdata_t &inputdata) {
    ...
    float loadscale = m_systemLoadScale != 0 ? m_systemLoadScale : 1;
    m_pConstraint->SetLinearMotor(speed, speed * loadscale * massLoad *
                                             (1.0 / TICK_INTERVAL));
  }
  ...
  void DrawDebugGeometryOverlays() {
    if (m_debugOverlays &
        (OVERLAY_BBOX_BIT | OVERLAY_PIVOT_BIT | OVERLAY_ABSBOX_BIT)) {
      NDebugOverlay::Box(GetAbsOrigin(), -Vector(8, 8, 8), Vector(8, 8, 8), 0,
                         255, 0, 0, 0);
      NDebugOverlay::Box(m_axisEnd, -Vector(4, 4, 4), Vector(4, 4, 4), 0, 0,
                         255, 0, 0);
  ...
  void NotifyVPhysicsStateChanged(IPhysicsObject *pPhysics,
    ...
    if (bAwake) {
      ...
      if (GetNextThink() == TICK_NEVER_THINK) {
        ...
        m_soundInfo.StartThinking(this,
                                  VelocitySampler::GetRelativeVelocity(
                                      m_pConstraint->GetAttachedObject(),
                                      m_pConstraint->GetReferenceObject()),
                                  axisDirection);
...
DEFINE_KEYFIELD(m_axisEnd, FIELD_POSITION_VECTOR, "slideaxis"),
    ...
    END_DATADESC()
        ...
        IPhysicsConstraint *CPhysSlideConstraint::CreateConstraint(
            IPhysicsConstraintGroup *pGroup, const hl_constraint_info_t &info) {
  ...
  sliding.InitWithCurrentObjectState(info.pObjects[0], info.pObjects[1],
                                     axisDirection);
  ...
  return physenv->CreateSlidingConstraint(info.pObjects[0], info.pObjects[1],
                                          pGroup, sliding);
}
...
void CPhysSlideConstraint::SoundThink(void) {
  ...
  if (pAttached && pReference) {
    Vector relativeVel =
        VelocitySampler::GetRelativeVelocity(pAttached, pReference);
    // project velocity onto my primary axis.:

    Vector axisDirection = m_axisEnd - GetAbsOrigin();
    relativeVel =
        m_axisEnd * relativeVel.Dot(m_axisEnd) / m_axisEnd.Dot(m_axisEnd);
...
void CPhysSlideConstraint::Precache() { m_soundInfo.OnPrecache(this); }
...
//-----------------------------------------------------------------------------
IPhysicsConstraint *
CPhysFixed::CreateConstraint(IPhysicsConstraintGroup *pGroup,
                             const hl_constraint_info_t &info) {
  ...
  return physenv->CreateFixedConstraint(info.pObjects[0], info.pObjects[1],
                                        pGroup, fixed);
}
...
public:
  ...
  void DrawDebugGeometryOverlays() {
    if (m_debugOverlays &
      ...
      NDebugOverlay::Box(origin, -Vector(8, 8, 8), Vector(8, 8, 8), 128, 255,
                         128, 32, 0);
      NDebugOverlay::Box(m_position2, -Vector(8, 8, 8), Vector(8, 8, 8), 255,
                         128, 128, 32, 0);
    }
    BaseClass::DrawDebugGeometryOverlays();
  }
...
//-----------------------------------------------------------------------------
IPhysicsConstraint *
CPhysPulley::CreateConstraint(IPhysicsConstraintGroup *pGroup,
                              const hl_constraint_info_t &info) {
  ...
  pulley.totalLength =
      m_addLength + (world[0] - pulley.pulleyPosition[0]).Length() +
  ...
  return physenv->CreatePulleyConstraint(info.pObjects[0], info.pObjects[1],
                                         pGroup, pulley);
}
...
//-----------------------------------------------------------------------------
IPhysicsConstraint *
CPhysLength::CreateConstraint(IPhysicsConstraintGroup *pGroup,
                              const hl_constraint_info_t &info) {
  ...
  length.InitWorldspace(info.pObjects[0], info.pObjects[1], position[index],
                        position[!index]);
  ...
  return physenv->CreateLengthConstraint(info.pObjects[0], info.pObjects[1],
                                         pGroup, length);
}

//-----------------------------------------------------------------------------
// Purpose: Limited ballsocket constraint with toggle-able translation
// constraints
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
IPhysicsConstraint *
CRagdollConstraint::CreateConstraint(IPhysicsConstraintGroup *pGroup,
                                     const hl_constraint_info_t &info) {
  ...
  ConcatTransforms(worldToEntity, EntityToWorldTransform(),
  ...
  ragdoll.onlyAngularLimits =
      HasSpawnFlags(SF_RAGDOLL_FREEMOVEMENT) ? true : false;

  // FIXME: Why are these friction numbers in different units from what the
  // hinge uses?
  ...
  return physenv->CreateRagdollConstraint(info.pObjects[0], info.pObjects[1],
                                          pGroup, ragdoll);
}
...
class CPhysConstraintEvents : public IPhysicsConstraintEvent {
  void ConstraintBroken(IPhysicsConstraint *pConstraint) {
    ...
    if (pEntity) {
      IPhysicsConstraintEvent *pConstraintEvent =
          dynamic_cast<IPhysicsConstraintEvent *>(pEntity);
...
/// Call this in spawn(). (Not a constructor because those are difficult to use
/// in entities.)
void VelocitySampler::Initialize(float samplerate) {
  m_fIdealSampleRate = samplerate;
}

// This is an old style approach to reversal sounds, from when there was only
// one.
#if 0
...
/// Returns the index of the highest threshold exceeded by the reversal
/// velocity.
int VelocitySampler::HasReversed(const Vector &relativeVelocity,
                                 const float thresholdAcceleration[],
                                 const unsigned short numThresholds) {
  // first, make sure the velocity has reversed (is more than 90deg off) from
  // last time, or is zero now. float rVsq = relativeVelocity.LengthSqr();
  float vDot = relativeVelocity.Dot(m_prevSample);
  if (vDot <= 0) // there is a reversal in direction. compute the magnitude of
                 // acceleration.
  {
    // find the scalar projection of the relative acceleration this fame onto
    // the previous frame's velocity, and compare that to the threshold.
    Vector accel = relativeVelocity - m_prevSample;
    ...
    // divide through by dt to get the accel per sec
    if (prevSampleLength) {
      // the scalar projection is negative because the acceleration is against
      // vel
      projection = -(accel.Dot(m_prevSample) / prevSampleLength) /
                   (gpGlobals->curtime - m_fPrevSampleTime);
    } else {
      projection = accel.Length() / (gpGlobals->curtime - m_fPrevSampleTime);
    }
...
/// small helper function used just below (technique copy-pasted  from
/// sound.cpp)
...
void ConstraintSoundInfo::OnActivate(CPhysConstraint *pOuter) {
  ...
  m_bPlayTravelSound =
      !IsEmpty(m_iszTravelSoundFwd) || !IsEmpty(m_iszTravelSoundBack);
  m_bPlayReversalSound = false;
  for (int i = 0; i < SimpleConstraintSoundProfile::kREVERSAL_SOUND_ARRAY_SIZE;
       ++i) {
...
void ConstraintSoundInfo::ValidateInternals(CPhysConstraint *pOuter) {
  ...
  for (int i = 1; i < SimpleConstraintSoundProfile::kREVERSAL_SOUND_ARRAY_SIZE;
       ++i) {
    // if decreases from small to medium, promote small to medium and warn.
    if (m_soundProfile.m_reversalSoundThresholds[i] <
        m_soundProfile.m_reversalSoundThresholds[i - 1]) {
      Warning("Constraint reversal sounds for %s are out of order!",
              pOuter->GetDebugName());
      m_soundProfile.m_reversalSoundThresholds[i] =
          m_soundProfile.m_reversalSoundThresholds[i - 1];
...
void ConstraintSoundInfo::OnThink(CPhysConstraint *pOuter,
                                  const Vector &relativeVelocity) {
  // have we had a hard reversal?
  int playReversal = m_vSampler.HasReversed(
      relativeVelocity, m_soundProfile.m_reversalSoundThresholds,
      SimpleConstraintSoundProfile::kREVERSAL_SOUND_ARRAY_SIZE);
  float relativeVelMag =
      relativeVelocity.Length(); //< magnitude of relative velocity

  CBaseEntity *pChildEntity = static_cast<CBaseEntity *>(
      pOuter->GetPhysConstraint()->GetAttachedObject()->GetGameData());
  ...
  if (m_bPlayTravelSound) {
    if (soundVol > 0) {
      ...
      if (m_pTravelSound) { // if a sound exists, modify it
        CSoundEnvelopeController::GetController().SoundChangeVolume(
            m_pTravelSound, soundVol, 0.1f);
      } else { // if a sound does not exist, create it
        ...
        m_pTravelSound = controller.SoundCreate(
            filter, pChildEntity->entindex(),
            (travellingForward ? m_iszTravelSoundFwd : m_iszTravelSoundBack)
                .ToCStr());
        controller.Play(m_pTravelSound, soundVol, 100);
      }
    } else {
      // if we want to not play sound
      if (m_pTravelSound) { // and it exists, kill it
...
void ConstraintSoundInfo::StartThinking(CPhysConstraint *pOuter,
                                        const Vector &relativeVelocity,
  ...
  if (pConstraint)
  {
          IPhysicsObject * pAttached = pConstraint->GetAttachedObject(),
  *pReference = pConstraint->GetReferenceObject(); m_vSampler.BeginSampling(
  VelocitySampler::GetRelativeVelocity(pAttached,pReference) );
  }
  */
}
...
ConstraintSoundInfo::~ConstraintSoundInfo() { DeleteAllSounds(); }
