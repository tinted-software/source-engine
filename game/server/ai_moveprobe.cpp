...
CAI_MoveProbe::CAI_MoveProbe(CAI_BaseNPC *pOuter)
    : CAI_Component(pOuter), m_bIgnoreTransientEntities(false),
...
bool CAI_MoveProbe::TestGroundMove(const Vector &vecActualStart,
  ...
  for (;;) {
    float flStepSize =
        MIN(LOCAL_STEP_SIZE, pMoveTrace->flTotalDist - distClear);
    if (flStepSize < 0.001)
      break;
...
void CAI_MoveProbe::JumpMoveLimit(const Vector &vecStart, const Vector &vecEnd,
  ...
  Vector gravity =
      Vector(0, 0, GetCurrentGravity() * GetOuter()->GetJumpGravity());
...
bool CAI_MoveProbe::IterativeFloorPoint(const Vector &vecStart,
  ...
  return FloorPoint(vecStart, collisionMask,
                    StepHeight() * GetOuter()->GetStepDownMultiplier() +
                        flAddedStep,
                    -(12 * 60), pVecResult);
}
