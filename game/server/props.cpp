...
void CBaseProp::Spawn(void) {
  ...
  if (!OverridePropdata()) {
    if (iResult == PARSE_FAILED_BAD_DATA) {
      DevWarning("%s at %.0f %.0f %0.f uses model %s, which has an invalid "
                 "prop_data type. DELETED.\n",
      ...
      if (FClassnameIs(this, "prop_physics")) {
        DevWarning("%s at %.0f %.0f %0.f uses model %s, which has no propdata "
                   "which means it must be used on a prop_static. DELETED.\n",
                   GetClassname(), GetAbsOrigin().x, GetAbsOrigin().y,
                   GetAbsOrigin().z, szModel);
        UTIL_Remove(this);
        return;
      ...
      if (!dynamic_cast<CPhysicsProp *>(this)) {
        DevWarning("%s at %.0f %.0f %0.f uses model %s, which has propdata "
                   "which means that it be used on a prop_physics. DELETED.\n",
                   GetClassname(), GetAbsOrigin().x, GetAbsOrigin().y,
                   GetAbsOrigin().z, szModel);
        UTIL_Remove(this);
        return;
...
void CBaseProp::Activate(void) {
  ...
  if (m_takedamage == DAMAGE_NO && m_iHealth != 0) {
    Warning("%s has a health specified in model '%s'. Use prop_physics or "
...
void CBreakableProp::OnPhysGunPickup(CBasePlayer *pPhysGunUser,
  ...
  if (IsX360()) {
    if (reason != PUNTED_BY_CANNON &&
        (pPhysGunUser->m_nNumCrateHudHints < NUM_SUPPLY_CRATE_HUD_HINTS)) {
...
void CBreakableProp::Break(CBaseEntity *pBreaker, const CTakeDamageInfo &info) {
  ...
  if (m_explodeDamage > 0 || m_explodeRadius > 0) {
    if (HasInteraction(PROPINTER_PHYSGUN_BREAK_EXPLODE)) {
      ...
      ExplosionCreate(WorldSpaceCenter(), angles, pAttacker,
                      m_explodeDamage * flScale, m_explodeRadius * flScale,
                      SF_ENVEXPLOSION_NOSPARKS | SF_ENVEXPLOSION_NODLIGHTS |
...
DEFINE_KEYFIELD(m_iszDefaultAnim, FIELD_STRING, "DefaultAnim"),
    ...
    DEFINE_KEYFIELD(m_bDisableBoneFollowers, FIELD_BOOLEAN,
...
void CDynamicProp::Spawn() {
  ...
  if (pszModelName && pszModelName[0]) {
    if (FStrEq(pszModelName, "models/bots/boss_bot/carrier_parts.mdl")) {
      SetModelIndexOverride(VISION_MODE_NONE,
                            modelinfo->GetModelIndex(pszModelName));
...
static CBreakableProp *
BreakModelCreate_Prop(CBaseEntity *pOwner, breakmodel_t *pModel,
...
void CBasePropDoor::Blocked(CBaseEntity *pOther) {
  ...
  //(CPropDoorRotating *)pTarget;
  //
  //					if (pDoor->m_fAutoReturnDelay >= 0)
  //					{
  //						if (pDoor->GetAbsVelocity() ==
  //GetAbsVelocity() && pDoor->GetLocalAngularVelocity() ==
  //GetLocalAngularVelocity())
  //						{
  //							// this is the most
  //hacked, evil, bastardized thing I've ever seen. kjb 							if
  //(FClassnameIs(pTarget, "prop_door_rotating"))
  //							{
  //								// set angles to
  //realign rotating doors 								pDoor->SetLocalAngles(GetLocalAngles());
...
public:
  ...
  CTraceFilterDoor(const IHandleEntity *pDoor, const IHandleEntity *passentity,
                   int collisionGroup)
      : m_pDoor(pDoor), m_pPassEnt(passentity),
...
bool CPropDoorRotating::CheckDoorClear(doorCheck_e state) {
  ...
  case DOOR_CHECK_FULL:
    UTIL_ComputeAABBForBounds(m_vecForwardBoundsMin, m_vecForwardBoundsMax,
                              m_vecBackBoundsMin, m_vecBackBoundsMax, &moveMins,
                              &moveMaxs);
    break;
  }
...
DEFINE_KEYFIELD(m_iPhysicsMode, FIELD_INTEGER, "physicsmode"),
    ...
    END_DATADESC()
        ...
        IMPLEMENT_SERVERCLASS_ST(CPhysicsPropMultiplayer,
...
void CC_Prop_Physics_Create(const CCommand &args) {
  ...
  CreatePhysicsProp(pModelName, pPlayer->EyePosition(),
                    pPlayer->EyePosition() + forward * MAX_TRACE_LENGTH,
                    pPlayer, true);
}
...
CPhysicsProp *CreatePhysicsProp(const char *pModelName,
                                ...
                                bool bRequireVCollide, const char *pClassName) {
  MDLCACHE_CRITICAL_SECTION();
  ...
  trace_t tr;
  UTIL_TraceHull(vTraceStart, vTraceEnd, vecSweepMins, vecSweepMaxs,
                 MASK_NPCSOLID, pTraceIgnore, COLLISION_GROUP_NONE, &tr);

  // No hit? We're done.
  if ((tr.fraction == 1.0 && (vTraceEnd - vTraceStart).Length() > 0.01) ||
      tr.allsolid)
    return NULL;
  ...
  // Try to create entity
  CPhysicsProp *pProp =
      dynamic_cast<CPhysicsProp *>(CreateEntityByName(pClassName));
  if (pProp) {
    char buf[512];
    // Pass in standard key values
    Q_snprintf(buf, sizeof(buf), "%.10f %.10f %.10f", tr.endpos.x, tr.endpos.y,
...
//-----------------------------------------------------------------------------
// Purpose: Scale the object to a new size, taking its render verts and physical
// verts into account
//-----------------------------------------------------------------------------
bool UTIL_CreateScaledPhysObject(CBaseAnimating *pInstance, float flScale) {
  ...
  if (pObject == NULL) {
    AssertMsg(0, "UTIL_CreateScaledPhysObject: Failed to scale physics for "
                 "object-- It has no physics.");
    return false;
  }
  ...
  CPhysCollide *pNewCollide =
      pCollide->solids[0]; // FIXME: Needs to iterate over the solids

  if (flScale != 1.0f) {
    // Create a query to get more information from the collision object
    ICollisionQuery *pQuery = physcollision->CreateQueryModel(
        pCollide->solids[0]); // FIXME: This should iterate over all solids!
    if (pQuery == NULL)
      return false;
    ...
    CPhysConvex **pConvexes =
        (CPhysConvex **)stackalloc(sizeof(CPhysConvex *) * nNumConvex);

    // For each convex, collect the verts and create a convex from it we'll
    // retain for later
  ...
  // Get our solid info
  solid_t tmpSolid;
  if (!PhysModelParseSolidByIndex(tmpSolid, pInstance,
                                  pInstance->GetModelIndex(), -1))
    return false;

  // Physprops get keyvalues that effect the mass, this block is to respect
  // those fields when we scale
  ...
  if (bWasStatic) {
    pNewObject = physenv->CreatePolyObjectStatic(
        pNewCollide, surfaceProp, pInstance->GetAbsOrigin(),
        pInstance->GetAbsAngles(), &tmpSolid.params);
  } else {
    pNewObject = physenv->CreatePolyObject(
        pNewCollide, surfaceProp, pInstance->GetAbsOrigin(),
        pInstance->GetAbsAngles(), &tmpSolid.params);
  }
  Assert(pNewObject);
  ...
  if (pInstance->GetParent()) {
    ...
    pNewObject->UpdateShadow(pInstance->GetAbsOrigin(),
                             pInstance->GetAbsAngles(), false, 0);
  }
