...
//--------------------------------------------------------------------------------------------------------
CON_COMMAND_F(nav_chop_selected,
...
//--------------------------------------------------------------------------------------------------------
// Simplifies the selected set by reducing to 1x1 areas and re-merging them up
// with loosened tolerances
void CNavMesh::SimplifySelectedAreas(void) {
  // Save off somve cvars: we need to place nodes on ground, we need snap to
  // grid set, and we loosen slope tolerances
  ...
  float savedCoplanarSlopeDisplacementLimit =
      nav_coplanar_slope_limit_displacement.GetFloat();
  nav_coplanar_slope_limit_displacement.SetValue(
      MIN(0.5f, savedCoplanarSlopeDisplacementLimit));
  ...
  FOR_EACH_VEC(m_selectedSet, it) {
    ...
    if (FindGroundForNode(&corner, &normal)) {
      ...
      if (area->m_node[NORTH_EAST]) {
        area->m_node[NORTH_WEST] =
            area->m_node[NORTH_EAST]->GetConnectedNode(WEST);
        area->m_node[SOUTH_EAST] =
            area->m_node[NORTH_EAST]->GetConnectedNode(SOUTH);
        if (area->m_node[SOUTH_EAST]) {
          area->m_node[SOUTH_WEST] =
              area->m_node[SOUTH_EAST]->GetConnectedNode(WEST);
    ...
    Assert(area->m_node[NORTH_EAST] && area->m_node[NORTH_WEST] &&
           area->m_node[SOUTH_EAST] && area->m_node[SOUTH_WEST]);
    if (!(area->m_node[NORTH_EAST] && area->m_node[NORTH_WEST] &&
          area->m_node[SOUTH_EAST] && area->m_node[SOUTH_WEST])) {
  ...
  nav_coplanar_slope_limit_displacement.SetValue(
      savedCoplanarSlopeDisplacementLimit);
...
CON_COMMAND_F(nav_simplify_selected,
              "Chops all selected areas into their component 1x1 areas and "
