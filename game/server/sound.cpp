...
static soundlevel_t ComputeSoundlevel(float radius, bool playEverywhere) {
  ...
  if (radius > 0 && !playEverywhere) {
    ...
    soundlevel =
        (soundlevel_t)(int)(40 +
                            dB_loss); // sound at 40dB at reference distance
  }
...
DEFINE_KEYFIELD(m_iszSound, FIELD_SOUNDNAME, "message"),
    ...
    // and re-init this struct in Precache(), but it's unlikely that the struct
    // will change, so it's not worth the time right now.
...
//innactive if not. 			If the sound is not a looping sound, never mark it as
//active.
// Input  : pActivator -
//			pCaller -
//			useType -
