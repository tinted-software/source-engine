...
static ConCommand
    ai_disable("ai_disable", CC_AI_Disable,
               "Bi-passes all AI logic routines and puts all NPCs into their "
               "idle animations.  Can be used to get NPCs out of your way and "
...
void CC_AI_ShowHints(void) {
  ...
  g_pAINetworkManager->GetEditOps()->SetDebugBits("BigNet",
                                                  bits_debugOverlayHints);
  CBaseEntity::m_nDebugPlayer = UTIL_GetCommandClientIndex();
}
static ConCommand ai_show_hints(
    "ai_show_hints", CC_AI_ShowHints,
    "Displays all hints as small boxes\n\tBlue		- hint is available "
    "for use\n\tRed		- hint is currently being used by an "
    "NPC\n\tOrange		- hint not being used by timed "
...
static ConCommand ai_show_hull(
    ...
    "Displays the allowed hulls between each node for the currently selected "
    "hull type.  Hulls are color code as follows:\n\tGreen		- "
...
void CC_AI_ShowConnect(const CCommand &args) {
  ...
  g_pAINetworkManager->GetEditOps()->SetDebugBits("BigNet",
                                                  bits_debugOverlayConnections);
  CBaseEntity::m_nDebugPlayer = UTIL_GetCommandClientIndex();
}
static ConCommand ai_show_connect(
    "ai_show_connect", CC_AI_ShowConnect,
    "Displays the allowed connections between each node for the currently "
    "selected hull type.  Hulls are color code as follows:\n\tGreen	"
    "	- ground movement \n\tBlue		- jumping "
...
void CC_AI_ShowJumpConnect(const CCommand &args) {
  ...
  g_pAINetworkManager->GetEditOps()->SetDebugBits("BigNet",
                                                  bits_debugOverlayConnections);
  g_pAINetworkManager->GetEditOps()->SetDebugBits(
      "BigNet", bits_debugOverlayJumpConnections);
  CBaseEntity::m_nDebugPlayer = UTIL_GetCommandClientIndex();
}
...
void CC_AI_ShowFlyConnect(const CCommand &args) {
  ...
  g_pAINetworkManager->GetEditOps()->SetDebugBits("BigNet",
                                                  bits_debugOverlayConnections);
  g_pAINetworkManager->GetEditOps()->SetDebugBits(
      "BigNet", bits_debugOverlayFlyConnections);
  CBaseEntity::m_nDebugPlayer = UTIL_GetCommandClientIndex();
}
...
void CC_AI_ShowGrid(const CCommand &args) {
  ...
  g_pAINetworkManager->GetEditOps()->SetDebugBits("BigNet",
                                                  bits_debugOverlayGrid);
  CBaseEntity::m_nDebugPlayer = UTIL_GetCommandClientIndex();
}
static ConCommand ai_show_grid("ai_show_grid", CC_AI_ShowGrid,
...
static ConCommand
    ai_step("ai_step", CC_AI_Step,
            "NPCs will freeze after completing their current task.  To "
            "complete the next task, use 'ai_step' again.  To resume "
...
static ConCommand ai_resume("ai_resume", CC_AI_Resume,
                            "If NPC is stepping through tasks (see ai_step ) "
                            "will resume normal processing.",
                            FCVAR_CHEAT);
...
static ConCommand ai_next_hull(
    ...
    "Cycles through the various hull sizes.  Currently selected hull size is "
    "written to the screen.  Controls which connections are shown when "
    "ai_show_hull or ai_show_connect commands are used\n\tArguments:	-none-",
    FCVAR_CHEAT);
...
static ConCommand
    ai_hull("ai_hull", CC_AI_Hull,
            "Controls which connections are shown when ai_show_hull or "
            "ai_show_connect commands are used\n\tArguments:	NPC name or "
            "classname, <none>=NPC under crosshair",
            FCVAR_CHEAT);
...
void CC_AI_Nodes(const CCommand &args) {
  ...
  g_pAINetworkManager->GetEditOps()->SetDebugBits("BigNet",
                                                  bits_debugOverlayNodes);
  CBaseEntity::m_nDebugPlayer = UTIL_GetCommandClientIndex();
}
static ConCommand ai_nodes(
    "ai_nodes", CC_AI_Nodes,
    "Toggles node display.  First call displays the nodes for the given "
    "network as green objects.  Second call  displays the nodes and their IDs. "
    " Nodes are color coded as follows:\n\tGreen		- ground "
    "node\n\tCyan		- air node\n\tMagenta	- climb "
    "node\n\tGrey		- node not available for selected hull "
...
CON_COMMAND(ai_show_node, "Highlight the specified node") {
  if (args.ArgC() > 1) {
    ...
    if (pAINode) {
      NDebugOverlay::Cross3D(pAINode->GetOrigin(), 1024, 255, 255, 255, true,
                             5.0);
    }
  }
}
...
static ConCommand ai_show_visibility(
    ...
    "Toggles visibility display for the node that the player is looking at.  "
    "Nodes that are visible from the selected node will be drawn in red with "
    "yellow lines connecting to the selected node.  Nodes that are not visible "
    "from the selected node will be drawn in blue.",
    FCVAR_CHEAT);
...
void CC_AI_GraphConnect(const CCommand &args) {
  ...
  // Q_strncpy( entName, args[1],sizeof(entName) );
  g_pAINetworkManager->GetEditOps()->SetDebugBits(
      "BigNet", bits_debugOverlayGraphConnect);
  CAI_Node *pAINode = FindPickerAINode(UTIL_GetCommandClient(), NODE_ANY);
  if (pAINode != NULL) {
...
static ConCommand ai_show_graph_connect(
    ...
    "Toggles graph connection display for the node that the player is looking "
    "at.  Nodes that are connected to the selected node by the net graph will "
    "be drawn in red with magenta lines connecting to the selected node.  "
    "Nodes that are not connected via the net graph from the selected node "
...
static ConCommand npc_bipass(
    ...
    "Displays the local movement attempts by the given NPC(s) (triangulation "
    "detours).  Failed bypass routes are displayed in red, successful bypasses "
...
static ConCommand npc_destroy(
    "npc_destroy", CC_NPC_Destroy,
...
static ConCommand npc_enemies(
    ...
    "entities drawn in blue (don't know where it went)\n\tUnreachable entities "
    "drawn in green (can't get to it)\n\tCurrent enemy drawn in red\n\tCurrent "
    "target entity drawn in magenta\n\tAll other entities drawn in "
    "pink\n\tArguments:   	{npc_name} / {npc class_name} / no argument "
...
static ConCommand npc_focus(
    ...
    "Displays red line to NPC's enemy (if has one) and blue line to NPC's "
    "target entity (if has one)\n\tArguments:   	{npc_name} / {npc "
...
void CC_NPC_Create(const CCommand &args) {
  ...
  CAI_BaseNPC *baseNPC =
      dynamic_cast<CAI_BaseNPC *>(CreateEntityByName(args[1]));
  if (baseNPC) {
    ...
    if (tr.fraction != 1.0) {
      ...
      AI_TraceHull(baseNPC->GetAbsOrigin(), vUpBit, baseNPC->GetHullMins(),
                   baseNPC->GetHullMaxs(), MASK_NPCSOLID, baseNPC,
...
static ConCommand
    npc_create("npc_create", CC_NPC_Create,
               "Creates an NPC of the given type where the player is looking "
               "(if the given NPC can actually stand at that location).  Note "
               "that this only works for npc classes that are already in the "
               "world.  You can not create an entity that doesn't have an "
...
static ConCommand npc_create_aimed(
    ...
    "Creates an NPC aimed away from the player of the given type where the "
    "player is looking (if the given NPC can actually stand at that location). "
    " Note that this only works for npc classes that are already in the world. "
    " You can not create an entity that doesn't have an instance in the "
    "level.\n\tArguments:	{npc_class_name}",
    FCVAR_CHEAT);
...
void CC_NPC_DestroyUnselected(void) {
  ...
  while (pNPC) {
    if (!(pNPC->m_debugOverlays & OVERLAY_NPC_SELECTED_BIT) &&
        !pNPC->ClassMatches("npc_bullseye")) {
      pNPC->m_debugOverlays |= OVERLAY_NPC_ZAP_BIT;
    }
    pNPC = gEntList.NextEntByClass(pNPC);
  }
}
static ConCommand npc_destroy_unselected(
    "npc_destroy_unselected", CC_NPC_DestroyUnselected,
...
static ConCommand npc_freeze(
    "npc_freeze", CC_NPC_Freeze,
    "Selected NPC(s) will freeze in place (or unfreeze). If there are no "
    "selected NPCs, uses the NPC under the crosshair.\n\tArguments:	-none-",
    FCVAR_CHEAT);
...
static ConCommand
    npc_teleport("npc_teleport", CC_NPC_Teleport,
                 "Selected NPC will teleport to the location that the player "
                 "is looking (shown with a purple box)\n\tArguments:	-none-",
                 FCVAR_CHEAT);

static ConVar npc_go_do_run("npc_go_do_run", "1", 0,
...
void CC_NPC_Go(void) {
  ...
  AI_TraceLine(pPlayer->EyePosition(),
               ...
               MASK_NPCSOLID, pPlayer, COLLISION_GROUP_NONE, &tr);
  if (tr.fraction != 1.0) {
    CAI_BaseNPC::ForceSelectedGo(pPlayer, tr.endpos, forward,
                                 npc_go_do_run.GetBool());
  }
}
static ConCommand
    npc_go("npc_go", CC_NPC_Go,
           "Selected NPC(s) will go to the location that the player is looking "
           "(shown with a purple box)\n\tArguments:	-none-",
           FCVAR_CHEAT);

//------------------------------------------------------------------------------
// Purpose: Tell seclected NPC to go to a random node location
//------------------------------------------------------------------------------
void CC_NPC_GoRandom(void) { CAI_BaseNPC::ForceSelectedGoRandom(); }
static ConCommand npc_go_random(
    "npc_go_random", CC_NPC_GoRandom,
...
static ConCommand npc_reset("npc_reset", CC_NPC_Reset,
                            "Reloads schedules for all NPC's from their script "
                            "files\n\tArguments:	-none-",
                            FCVAR_CHEAT);
...
static ConCommand
    npc_nearest("npc_nearest", CC_NPC_Nearest,
                "Draw's a while box around the NPC(s) nearest "
...
static ConCommand
    npc_route("npc_route", CC_NPC_Route,
              "Displays the current route of the given NPC as a line on the "
              "screen.  Waypoints along the route are drawn as small cyan "
              "rectangles.  Line is color coded in the following "
              "manner:\n\tBlue	- path to a node\n\tCyan	- detour "
              "around an object (triangulation)\n\tRed	- jump\n\tMaroon - "
...
static ConCommand
    npc_select("npc_select", CC_NPC_Select,
               "Select or deselects the given NPC(s) for later manipulation.  "
               "Selected NPC's are shown surrounded by a red translucent "
               "box\n\tArguments:   	{entity_name} / {class_name} / no "
               "argument picks what player is looking at ",
               FCVAR_CHEAT);
...
static ConCommand npc_combat(
    ...
    "Displays text debugging information about the squad and enemy of the "
    "selected NPC  (See Overlay Text)\n\tArguments:   	{npc_name} / {npc "
    "class_name} / no argument picks what player is looking at",
    FCVAR_CHEAT);
// For backwards compatibility
static ConCommand npc_squads("npc_squads", CC_NPC_Combat,
                             "Obsolete.  Replaced by npc_combat", FCVAR_CHEAT);
...
static ConCommand
    npc_tasks("npc_tasks", CC_NPC_Tasks,
              "Displays detailed text debugging information about the all the "
              "tasks of the selected NPC current schedule (See Overlay "
...
static ConCommand
    npc_task_text("npc_task_text", CC_NPC_Task_Text,
                  "Outputs text debugging information to the console about the "
                  "all the tasks + break conditions of the selected NPC "
                  "current schedule\n\tArguments:   	{npc_name} / {npc "
                  "class_name} / no argument picks what player is looking at ",
                  FCVAR_CHEAT);
...
static ConCommand
    npc_conditions("npc_conditions", CC_NPC_Conditions,
                   "Displays all the current AI conditions that an NPC has in "
                   "the overlay text.\n\tArguments:   	{npc_name} / {npc "
                   "class_name} / no argument picks what player is looking at",
                   FCVAR_CHEAT);
...
static ConCommand npc_viewcone(
    ...
    "Displays the viewcone of the NPC (where they are currently looking and "
    "what the extents of there vision is)\n\tArguments:   	{entity_name} "
    "/ {class_name} / no argument picks what player is looking at",
    FCVAR_CHEAT);
...
static ConCommand
    npc_relationships("npc_relationships", CC_NPC_Relationships,
                      "Displays the relationships between this NPC and all "
                      "others.\n\tArguments:   	{entity_name} / {class_name} / "
                      "no argument picks what player is looking at",
                      FCVAR_CHEAT);
...
void CC_NPC_ViewSteeringRegulations(const CCommand &args) {
  SetDebugBits(UTIL_GetCommandClient(), args[1],
               OVERLAY_NPC_STEERING_REGULATIONS);
}
static ConCommand
    npc_steering("npc_steering", CC_NPC_ViewSteeringRegulations,
                 "Displays the steering obstructions of the NPC (used to "
...
static ConCommand npc_steering_all("npc_steering_all",
                                   CC_NPC_ViewSteeringRegulationsAll,
                                   "Displays the steering obstructions of all "
...
CON_COMMAND(ai_clear_bad_links,
...
CON_COMMAND(ai_test_los, "Test AI LOS from the player's POV") {
  ...
  UTIL_TraceLine(UTIL_GetLocalPlayer()->EyePosition(),
                 UTIL_GetLocalPlayer()->EyePosition() +
                     UTIL_GetLocalPlayer()->EyeDirection3D() * MAX_COORD_RANGE,
                 MASK_BLOCKLOS_AND_NPCS, &traceFilter, &tr);
  NDebugOverlay::Line(UTIL_GetLocalPlayer()->EyePosition(), tr.endpos, 127, 127,
                      127, true, 5);
