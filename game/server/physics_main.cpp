...
//the time 			interval.  Since that would effect a lot of code, and since most of
//			that code is going away, it's easier to just add in the
//average effect 			of gravity on the velocity over the interval at the beginning
//of similation, 			then add it in again at the end of simulation so that the final
//velocity is 			correct for the entire interval.
//-----------------------------------------------------------------------------
...
void CBaseEntity::PerformPush(float movetime) {
  ...
  if (VPhysicsGetObject()) {
    ...
    if (movetime > 0 && !m_pBlocker && GetSolid() == SOLID_VPHYSICS &&
        g_pPushedEntities->CountMovedEntities() > 0) {
