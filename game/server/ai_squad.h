...
public:
  ...
  CAI_Squad *
  FindCreateSquad(string_t squadName); // Returns squad of the given name
  CAI_Squad *
  FindCreateSquad(CAI_BaseNPC *pNPC,
...
public:
  ...
  template <typename T> void SetSquadData(unsigned slot, const T &data) {
  ...
  template <typename T> void GetSquadData(unsigned slot, T *pData) {
    Assert(slot < MAX_SQUAD_DATA_SLOTS);
    if (slot < MAX_SQUAD_DATA_SLOTS) {
