...
//
//			It will rotate around the Z axis by default. Spawnflags
//can be set 			to make it rotate around the X or Y axes.
//
...
protected:
  ...
  float m_flVolume;
  float m_flTargetSpeed; // Target value for m_flSpeed, used for spinning up and
                         // down.
...
/*
extern void SendProxy_Angles( const SendProp *pProp, const void *pStruct, const
void *pData, DVariant *pOut, int iElement, int objectID ); void
SendProxy_FuncRotatingAngles( const SendProp *pProp, const void *pStruct, const
...
SendPropExclude("DT_BaseEntity", "m_angRotation"),
    ...
    SendPropAngle(SENDINFO_VECTORELEM(m_angRotation, 0), 13,
                  SPROP_CHANGES_OFTEN, SendProxy_FuncRotatingAngle),
...
void CFuncRotating::RampPitchVol(void) {
  ...
  float fvol =
      clamp(m_flVolume * fpct, 0.f, 1.f); // slowdown volume ramps down to 0
...
// current
//			direction of rotation, so that positive values are
//forward and negative 			values are backward.
