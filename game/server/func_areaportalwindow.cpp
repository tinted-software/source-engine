...
SendPropFloat(SENDINFO(m_flFadeDist), 0, SPROP_NOSCALE),
    ...
    DEFINE_INPUTFUNC(FIELD_FLOAT, "SetFadeStartDistance",
                     InputSetFadeStartDistance),
    DEFINE_INPUTFUNC(FIELD_FLOAT, "SetFadeEndDistance",
...
void CFuncAreaPortalWindow::Activate() {
  ...
  CBaseEntity *pBackground =
      gEntList.FindEntityByName(NULL, m_iBackgroundBModelName);
  if (pBackground) {
    m_iBackgroundModelIndex =
        modelinfo->GetModelIndex(STRING(pBackground->GetModelName()));
...
bool CFuncAreaPortalWindow::IsWindowOpen(const Vector &vOrigin,
...
bool CFuncAreaPortalWindow::UpdateVisibility(const Vector &vOrigin,
  ...
  if (IsWindowOpen(vOrigin, fovDistanceAdjustFactor)) {
    return BaseClass::UpdateVisibility(vOrigin, fovDistanceAdjustFactor,
                                       bIsOpenOnClient);
  } else {
