...
DEFINE_KEYFIELD(m_iszActor, FIELD_STRING, "Actor"),
    ...
    DEFINE_FIELD(flAbandonTimeLimit, FIELD_FLOAT), END_DATADESC();
...
DEFINE_FIELD(m_fActive, FIELD_BOOLEAN),
    ...
    DEFINE_EMBEDDED(m_PlayerMoveMonitor), DEFINE_EMBEDDED(m_TimeForceCoverHint),
    ...
    //											m_ActivityMap
    //(not saved, rebuilt) 											m_bHasLowCoverActivity			(not
    //saved, rebuilt)
...
Activity
CAI_MappedActivityBehavior_Temporary::GetMappedActivity(AI_Posture_t posture,
                                                        Activity activity) {
      ...
      if (coverPos == vec3_origin) {
        ...
        if (!GetTacticalServices()->FindCoverPos(enemyPos, enemyEyePos, 0,
          ...
          if (!GetTacticalServices()->FindLateralCover(enemyEyePos, 0,
                                                       &coverPos)) {
            // At this point, try again ignoring front lines. Any cover probably
            // better than hanging out in the open
...
void CAI_StandoffBehavior::UpdateTranslateActivityMap() {
  ...
  if (pWeapon &&
      (GetOuter()->TranslateActivity(lowCoverActivity) == ACT_INVALID))
    DevMsg("Note: NPC class %s lacks ACT_COVER_LOW, therefore cannot "
           "participate in standoff\n",
...
AI_StandoffParams_t g_StandoffParamsByAgression[] = {
    ...
    //minTimeShots,	maxTimeShots, 	minShots, 	maxShots, oddsCover
    //flAbandonTimeLimit
