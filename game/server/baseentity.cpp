...
SendPropDataTable("AnimTimeMustBeFirst", 0,
    ...
    SendPropInt(SENDINFO(m_ubInterpolationFrame), NOINTERP_PARITY_MAX_BITS,
                SPROP_UNSIGNED),
    ...
    SendPropArray3(SENDINFO_ARRAY3(m_nModelIndexOverrides),
                   SendPropInt(SENDINFO_ARRAY(m_nModelIndexOverrides),
                               SP_MODEL_INDEX_BITS, 0)),
#endif
...
static CUtlHashtable<CBaseEntityModelLoadProxy, empty_t, PointerHashFunctor,
                     PointerEqualFunctor, CBaseEntity *>
    sg_DynamicLoadHandlers;

void CBaseEntityModelLoadProxy::Handler::OnModelLoadComplete(
...
void CBaseEntity::SetModelIndex(int index) {
  if (IsDynamicModelIndex(index) &&
      !(GetBaseAnimating() && m_bDynamicModelAllowed)) {
  ...
  if (index != m_nModelIndex) {
    ...
    if (IsDynamicModelIndex(index)) {
      ...
      sg_DynamicLoadHandlers[sg_DynamicLoadHandlers.Insert(this)].Register(
          index);
    } else {
      m_bDynamicModelPending = false;
      OnNewModel();
...
int CBaseEntity::DrawDebugTextOverlays(void) {
  ...
  if (m_debugOverlays & OVERLAY_TEXT_BIT) {
    ...
    if (m_iGlobalname != NULL_STRING) {
      Q_snprintf(tempstr, sizeof(tempstr), "GLOBALNAME: %s",
                 STRING(m_iGlobalname));
      EntityText(offset, tempstr, 0);
      offset++;
    }
...
//absolute offset 			from the parent entity and will then follow the parent entity.
  ...
  if (VPhysicsGetObject()) {
    if (VPhysicsGetObject()->IsStatic()) {
      if (VPhysicsGetObject()->IsAttachedToConstraint(false)) {
        Warning("SetParent on static object, all constraints attached to %s "
    ...
    for (int i = 0; i < fields; i++) {
      ...
      if ((dataDesc->fieldType == FIELD_CUSTOM) &&
        ...
        CBaseEntityOutput *pOutput =
            (CBaseEntityOutput *)((intp)this + (intp)dataDesc->fieldOffset[0]);
...
void CBaseEntity::TakeDamage(const CTakeDamageInfo &inputInfo) {
  ...
  if (bHasPhysicsForceDamage && inputInfo.GetDamageType() != DMG_GENERIC) {
    ...
    if (inputInfo.GetDamageForce() == vec3_origin ||
      ...
      if (++warningCount < 10) {
        if (inputInfo.GetDamageForce() == vec3_origin) {
          DevWarning("CBaseEntity::TakeDamage:  with "
                     "inputInfo.GetDamageForce() == vec3_origin\n");
        }
...
#else
    // this was spamming the console for Payload maps in TF (trigger_hurt entity
  ...
  virtual void Restore(const SaveRestoreFieldInfo_t &fieldInfo,
    ...
    for (int i = 0; i < pUtlVector->Size(); i++) {
      ...
      if (bHasFunc) {
        ...
        pRestore->ReadFunction(pOwner->GetDataDescMap(), (inputfunc_t **)ppV, 1,
                               header.size);
      } else {
        *ppV = NULL;
      }
...
//	DEFINE_FIELD( m_hPlayerSimulationOwner, FIELD_EHANDLE ),
    ...
    // read in, which may or may not actually occur before the entity is parsed.
    ...
    DEFINE_FUNCTION(SUB_StartFadeOut), DEFINE_FUNCTION(SUB_StartFadeOutInstant),
    DEFINE_FUNCTION(SUB_FadeOut), DEFINE_FUNCTION(SUB_Vanish),
  ...
  if (pEdict && !IsWorld()) {
    ...
    bool isSolidCheckTriggers =
        ...
        !isTriggerCheckSolids; // NOTE: Moving triggers (items, ammo etc) are
                               // not checked against other triggers to reduce
...
FORCEINLINE bool NamesMatch(const char *pszQuery, string_t nameToMatch) {
  ...
  while (*pszNameToMatch && *pszQuery) {
    ...
    if (cName == cQuery)
      ;
    else if (cName - 'A' <= (unsigned char)'Z' - 'A' &&
             cName - 'A' + 'a' == cQuery)
      ;
    else if (cName - 'a' <= (unsigned char)'z' - 'a' &&
             cName - 'a' + 'A' == cQuery)
...
#if defined(PORTAL) || defined(HL2_EPISODIC) || defined(HL2_DLL) ||            \
    defined(HL2_LOSTCOAST)
  // We had a short period during the 2013 beta where the FL_* flags had a bogus
  // value near the top, so detect these bad saves and just give up. Only saves
  // from the short beta period should have been effected.
  if (GetFlags() & FL_FAKECLIENT) {
    char szMsg[256];
    V_snprintf(szMsg, sizeof(szMsg),
               "\nInvalid save, unable to load. Please run \"map %s\" to "
               "restart this level manually\n\n",
...
void CBaseEntity::FunctionCheck(void *pFunction, const char *name) {
  ...
  if (pFunction &&
      !UTIL_FunctionToName(GetDataDescMap(), *(inputfunc_t *)pFunction)) {
...
void CBaseEntity::SetMoveType(MoveType_t val, MoveCollide_t moveCollide) {
  ...
  case MOVETYPE_STEP: {
    // This will probably go away once I remove the cvar that controls the test
    // code
...
int CBaseEntity::ShouldTransmit(const CCheckTransmitInfo *pInfo) {
  ...
  sv_netvisdist.GetFloat() ) return TRANSMIT_NO;	// TODO doesn't work
  with HLTV #endif*/
                      ...
                      FIELD_STRING)) // allow empty strings
            {
              if (!Value.Convert((fieldtype_t)dmap->dataDesc[i].fieldType)) {
                ...
                Warning("!! ERROR: bad input/output link:\n!! %s(%s,%s) "
                        "doesn't match type from %s(%s)\n",
                        STRING(m_iClassname), GetDebugName(), szInputName,
                        (pCaller != NULL) ? STRING(pCaller->m_iClassname)
                                          : "<null>",
                        (pCaller != NULL) ? STRING(pCaller->m_iName)
                                          : "<null>");
                return false;
              }
            }
...
void CBaseEntity::SetParentAttachment(const char *szInputName,
  ...
  if (iAttachment <= 0) {
    Warning("ERROR: Tried to %s for entity %s (%s), but it has no attachment "
            "named %s.\n",
...
//to prevent 			lazy updates of abs origins and angles from messing things up.
//-----------------------------------------------------------------------------
...
//origins and angles 			from messing up our child/constrained entity fixup.
//-----------------------------------------------------------------------------
...
static CUtlCachedFileData<CModelSoundsCache>
    g_ModelSoundsCache("modelsounds.cache", MODELSOUNDSCACHE_VERSION, 0,
          ...
          if (IsX360()) {
            ...
            case AE_CL_PLAYSOUND: {
              ...
              if (pEvent->pszOptions()[0]) {
                ...
                Warning("-- Error --:  empty soundname, .qc error on "
...
static ConCommand
    ent_bbox("ent_bbox", CC_Ent_BBox,
             "Displays the movement bounding box for the given entity(ies) in "
             "orange.  Some entites will also display entity specific "
...
static ConCommand
    ent_absbox("ent_absbox", CC_Ent_AbsBox,
               "Displays the total bounding box for the given entity(s) in "
               "green.  Some entites will also display entity specific "
               "overlays.\n\tArguments:   	{entity_name} / {class_name} / "
               "no argument picks what player is looking at ",
               FCVAR_CHEAT);
...
static ConCommand
    ent_attachments("ent_attachments", CC_Ent_AttachmentPoints,
                    "Displays the attachment points on an "
                    "entity.\n\tArguments:   	{entity_name} / {class_name} / "
                    "no argument picks what player is looking at ",
                    FCVAR_CHEAT);
...
static ConCommand
    ent_viewoffset("ent_viewoffset", CC_Ent_ViewOffset,
                   "Displays the eye position for the given entity(ies) in "
                   "red.\n\tArguments:   	{entity_name} / {class_name} / "
                   "no argument picks what player is looking at ",
                   FCVAR_CHEAT);
...
static ConCommand
    ent_remove("ent_remove", CC_Ent_Remove,
...
static ConCommand
    ent_remove_all("ent_remove_all", CC_Ent_RemoveAll,
...
static ConCommand
    ent_setname("ent_setname", CC_Ent_SetName,
                "Sets the targetname of the given entity(s)\n\tArguments:   "
                "	{new entity name} {entity_name} / {class_name} / no "
...
static ConCommand
    find_ent("find_ent", CC_Find_Ent,
             "Find and list all entities with classnames or targetnames that "
...
public:
  virtual void CommandCallback(const CCommand &command) {
    ...
    if (command.ArgC() < 2) {
      ...
      //
      // Robin: Unfortunately, they get around point_servercommand checks with
      // this:
      //	  ent_create point_servercommand; ent_setname mine; ent_fire
      //mine command "rcon_password mynewpassword"
      // So, I'm removing the ability for anyone to execute ent_fires on
      // dedicated servers (we can't check to see if this command is going to
      // connect with a point_servercommand entity here, because they could
      // delay the event and create it later).
      if (engine->IsDedicatedServer()) {
        // We allow people with disabled autokick to do it, because they already
        // have rcon.
        if (pPlayer->IsAutoKickDisabled() == false)
          return;
      } else if (gpGlobals->maxClients > 1) {
        // On listen servers with more than 1 player, only allow the host to
        // issue ent_fires.
...
static ConCommand
    ent_fire("ent_fire", &g_EntFireAutoComplete,
             "Usage:\n   ent_fire <target> [action] [value] [delay]\n",
...
static ConCommand
    ent_cancelpendingentfires("ent_cancelpendingentfires",
                              CC_Ent_CancelPendingEntFires,
                              "Cancels all ent_fire created outputs that are "
...
static ConCommand
    picker("picker", CC_Ent_Picker,
           "Toggles 'picker' mode.  When picker is on, the bounding box, pivot "
           "and debugging text is displayed for whatever entity the player is "
...
static ConCommand
    ent_pivot("ent_pivot", CC_Ent_Pivot,
...
void CBaseEntity::SetLocalVelocity(const Vector &inVecVelocity) {
  ...
  case -1:
    Warning("Discarding SetLocalVelocity(%f,%f,%f) on %s\n", vecVelocity.x,
            vecVelocity.y, vecVelocity.z, GetDebugName());
    Assert(false);
    return;
  case 0:
    if (CheckEmitReasonablePhysicsSpew()) {
      Warning("Clamping SetLocalVelocity(%f,%f,%f) on %s\n", inVecVelocity.x,
              inVecVelocity.y, inVecVelocity.z, GetDebugName());
    }
    break;
  }
  ...
  if (!IsEntityQAngleVelReasonable(vecAngVelocity)) {
    if (CheckEmitReasonablePhysicsSpew()) {
      Warning("Bad SetLocalAngularVelocity(%f,%f,%f) on %s\n", vecAngVelocity.x,
              vecAngVelocity.y, vecAngVelocity.z, GetDebugName());
    }
    Assert(false);
    return;
  }
...
//-----------------------------------------------------------------------------
CBaseEntity *
CBaseEntity::CreatePredictedEntityByName(const char *classname,
...
//following the same 			path. Each train has a sprite in hierarchy with it that
//needs to 			toggle on/off as it passes each path_track. You would hook each
//train's 			OnUser1 output to it's sprite's Toggle input, then connect each
  ...
  if (sChar) {
    *sChar = '\0';
    ...
    Warning("AddOutput input fired with bad string. Format: <output name> "
            "<targetname>,<inputname>,<parameter>,<delay>,<max times to fire "
            "(-1 == infinite)>\n");
  }
}
  ...
  if (step->m_bOriginActive) {
    ...
    if (GetStepOrigin() != step->m_Next.vecOrigin) {
      ...
      if (step->m_Previous2.nTickCount == 0 ||
        ...
        VectorMA(step->m_Previous.vecOrigin, frac, delta,
  ...
  if (step->m_bAnglesActive) {
    ...
    if (GetStepAngles() != step->m_angNextRotation) {
      ...
      if (step->m_Previous2.nTickCount == 0 ||
        ...
        QuaternionBlend(step->m_Previous.qRotation, step->m_Next.qRotation,
                        frac, outangles);
...
bool CBaseEntity::UseStepSimulationNetworkOrigin(const Vector **out_v) {
  ...
  if (g_bTestMoveTypeStepSimulation && GetMoveType() == MOVETYPE_STEP &&
...
void CBaseEntity::EmitSentenceByIndex(
  ...
  enginesound->EmitSentenceByIndex(filter, iEntIndex, iChannel, iSentenceIndex,
                                   flVolume, iSoundlevel, iFlags, iPitch, 0,
                                   pOrigin, pDirection, &dummy,
                                   bUpdatePositions, soundtime);
}

void CBaseEntity::SetRefEHandle(const CBaseHandle &handle) {
  m_RefEHandle = handle;
  if (edict()) {
    COMPILE_TIME_ASSERT(NUM_NETWORKED_EHANDLE_SERIAL_NUMBER_BITS <=
                        8 * sizeof(edict()->m_NetworkSerialNumber));
...
void CC_Ent_Create(const CCommand &args) {
  ...
  // Don't allow regular users to create point_servercommand entities for the
  // same reason as blocking ent_fire
...
static ConCommand
    ent_create("ent_create", CC_Ent_Create,
               "Creates an entity of the given type where the player is "
               "looking.  Additional parameters can be passed in in the form: "
               "ent_create <entity name> <param 1 name> <param 1> <param 2 "
               "name> <param 2>...<param N name> <param N>",
...
static ConCommand
    ent_teleport("ent_teleport", CC_Ent_Teleport,
                 "Teleport the specified entity to where the player is "
