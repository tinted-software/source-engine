...
DEFINE_FIELD(m_bWaitingToRappel, FIELD_BOOLEAN),
    ...
    DEFINE_FIELD(m_vecRopeAnchor, FIELD_POSITION_VECTOR), END_DATADESC();
...
void CAI_RappelBehavior::StartTask(const Task_t *pTask) {
  ...
  case TASK_HIT_GROUND:
    ...
    if (GetOuter()->GetGroundEntity() != NULL &&
      ...
      // Although I tried to get NPC's out from under me, I landed on one. Kill
      // it, so long as it's the same type of character as me.
      variant_t val;
      val.SetFloat(0);
      g_EventQueue.AddEvent(GetOuter()->GetGroundEntity(), "sethealth", val, 0,
                            GetOuter(), GetOuter());
    }
...
void CAI_RappelBehavior::RunTask(const Task_t *pTask) {
  switch (pTask->iTask) {
  case TASK_RAPPEL: {
    // If we don't do this, the beam won't show up sometimes. Ideally, all beams
    // would update their bboxes correctly, but we're close to shipping and we
    ...
    if (GetOuter()->GetFlags() & FL_ONGROUND) {
      ...
      if (pGroundEnt && pGroundEnt->IsPlayer()) {
        // try to shove the player in the opposite direction as they are facing
        // (so they'll see me)
        Vector vecForward;
...
DEFINE_SCHEDULE(
    ...
    "	Interrupts"
    ""
    "		COND_NEW_ENEMY" // Only so the enemy selection code will pick an
                                // enemy!
);
