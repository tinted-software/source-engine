...
#define MAX_NODE_LINK_DIST_SQ                                                  \
  (MAX_NODE_LINK_DIST *                                                        \
   MAX_NODE_LINK_DIST) //   distance allowed to travel to node via local moves
...
enum NodeInfoBits_e {
  ...
  /****** NOTE: will need to change node graph save/load code if exceed 16 bits
     here ******/
...
public:
  ...
  CAI_Link *
  GetShuffeledLink(int nNum); // Used to get links in different order each time
  ...
  float m_flVOffset[NUM_HULLS]; // vertical offset for each hull type, assuming
                                // ground node, 0 otherwise
  float m_flYaw; // NPC on this node should face this yaw to face the hint, or
                 // climb a ladder
