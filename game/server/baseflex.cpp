...
static ConVar
    scene_showlook("scene_showlook", "0", FCVAR_ARCHIVE,
...
static ConVar
    scene_showunlock("scene_showunlock", "0", FCVAR_ARCHIVE,
  ...
  case CChoreoEvent::MOVETO: {
    ...
    if (npc) {
      ...
      if (npc->GetNavigator()->IsGoalActive()) {
        ...
        // again
        Warning("%s : %8.2f: waiting for actor %s to complete MOVETO but actor "
...
CBaseFlex::FlexControllerLocalToGlobal(const flexsettinghdr_t *pSettinghdr,
                                       int key) {
  FS_LocalToGlobal_t entry(pSettinghdr);
...
flexsetting_t const *
CBaseFlex::FindNamedSetting(flexsettinghdr_t const *pSettinghdr,
                            const char *expr) {
  int i;
  ...
  if (info->m_iLayer >= 0) {
    ...
    if (myNpc && !(myNpc->IsCurSchedule(SCHED_SCENE_GENERIC) ||
      ...
      if (info->m_flWeight == 1.0) {
        Warning("%s playing CChoreoEvent::SEQUENCE but AI has forced them to "
...
DEFINE_FIELD(m_flextime, FIELD_TIME), DEFINE_FIELD(m_flexnum, FIELD_INTEGER),
    ...
    DEFINE_FIELD(m_blinktime, FIELD_TIME), DEFINE_FIELD(m_looktime, FIELD_TIME),
