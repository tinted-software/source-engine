...
public:
  ...
  GetEnemyZone(void) const; // where we expect enemies to be during this mission

  virtual const char *GetName(void) const = 0; // return name of this mission
};

inline const CTacticalMissionZone *
CTacticalMission::GetDeployZone(CBasePlayer *who) const {
  return NULL;
}

inline const CTacticalMissionZone *
CTacticalMission::GetObjectiveZone(void) const {
  return NULL;
}
...
public:
  ...
  virtual const CTacticalMission *GetMission(
      const char *name); // given a mission name, return the mission (or NULL)
