...
CPropVehicleDriveable::CPropVehicleDriveable(void)
    : m_pServerVehicle(NULL), m_hKeepUpright(NULL), m_flTurnOffKeepUpright(0),
...
void CPropVehicleDriveable::Spawn(void) {
  ...
  if (m_pServerVehicle->Initialize(STRING(m_vehicleScript)) == false) {
    Warning("Vehicle (%s) unable to properly initialize due to script error in "
            "(%s)!\n",
...
// Purpose:
//-----------------------------------------------------------------------------
const vehicle_controlparams_t *
CFourWheelServerVehicle::GetVehicleControlParams(void) {
...
CFourWheelServerVehicle::GetFourWheelVehiclePhysics(void) {
  CPropVehicleDriveable *pVehicle = GetFourWheelVehicle();
