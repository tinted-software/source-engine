...
DEFINE_FIELD(m_nTexture, FIELD_INTEGER),
    DEFINE_KEYFIELD(
        m_bLowPriority, FIELD_BOOLEAN,
        "LowPriority"), // Don't mark as FDECAL_PERMANENT so not save/restored
                        // and will be reused on the client preferentially
...
void CDecal::StaticDecal(void) {
  ...
  if (entityIndex) {
    ...
    if (ent) {
      ...
      if (!canDraw) {
        Warning("Suppressed StaticDecal which would have hit entity %i "
                "(class:%s, name:%s) with modelindex = 0\n",
