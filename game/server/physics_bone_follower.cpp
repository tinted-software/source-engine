...
CBoneFollowerManager::~CBoneFollowerManager() {
  // if this fires then someone isn't destroying their bonefollowers in
  // UpdateOnRemove
...
//			**pFollowerBoneNames -
//-----------------------------------------------------------------------------
void CBoneFollowerManager::InitBoneFollowers(CBaseAnimating *pParentEntity,
                                             int iNumBones,
  ...
  for (int i = 0; i < iNumBones; i++) {
    CreatePhysicsFollower(pParentEntity, m_physBones[i], pFollowerBoneNames[i],
                          NULL);
  }
}
...
// Purpose:
//-----------------------------------------------------------------------------
void CBoneFollowerManager::AddBoneFollower(CBaseAnimating *pParentEntity,
                                           const char *pFollowerBoneName,
                                           solid_t *pSolid) {
  m_iNumBones++;

  int iIndex = m_physBones.AddToTail();
  CreatePhysicsFollower(pParentEntity, m_physBones[iIndex], pFollowerBoneName,
                        pSolid);
}

// walk the hitboxes and find the first one that is attached to the physics bone
// in question return the hitgroup of that box
...
//-----------------------------------------------------------------------------
bool CBoneFollowerManager::CreatePhysicsFollower(CBaseAnimating *pParentEntity,
  ...
  if (boneIndex >= 0) {
    ...
    if (!pSolid) {
      if (!PhysModelParseSolidByIndex(solidTmp, pParentEntity,
                                      pParentEntity->GetModelIndex(),
                                      physicsBone))
        return false;
      pSolid = &solidTmp;
    }

    // fixup in case ragdoll is assigned to a parent of the requested follower
    // bone
    ...
    follow.hFollower = CBoneFollower::Create(
        pParentEntity, STRING(pParentEntity->GetModelName()), *pSolid,
        bonePosition, boneAngles);
    follow.hFollower->SetTraceData(
        physicsBone, HitGroupFromPhysicsBone(pParentEntity, physicsBone));
...
DEFINE_FIELD(m_modelIndex, FIELD_MODELINDEX),
    ...
    END_SEND_TABLE()
        ...
        bool CBoneFollower::Init(CBaseEntity *pOwner, const char *pModelName,
  ...
  // we can't use the default model bounds because each entity is only one bone
  // of the model so compute the OBB of the physics model and use that.
  Vector mins, maxs;
  physcollision->CollideGetAABB(&mins, &maxs, pPhysics->GetCollide(),
                                vec3_origin, vec3_angle);
  SetCollisionBounds(mins, maxs);

  pPhysics->SetCallbackFlags(pPhysics->GetCallbackFlags() |
                             CALLBACK_GLOBAL_TOUCH);
  pPhysics->EnableGravity(false);
  // This is not a normal shadow controller that is trying to go to a space
  // occupied by an entity in the game physics This entity is not running
  // PhysicsPusher(), so Vphysics is supposed to move it This line of code
  // informs vphysics of that fact
  if (pOwner->IsNPC()) {
...
public:
  ...
  ~CPhysicsSwapTemp() { m_pEntity->VPhysicsSwapObject(m_pPhysics); }
...
void CBoneFollower::VPhysicsCollision(int index,
...
void CBoneFollower::VPhysicsShadowCollision(int index,
                                            gamevcollisionevent_t *pEvent) {
  CBaseEntity *pOwner = GetOwnerEntity();
  if (pOwner) {
    CPhysicsSwapTemp tmp(pOwner, pEvent->pObjects[index]);
...
void CBoneFollower::VPhysicsFriction(IPhysicsObject *pObject, float energy,
...
bool CBoneFollower::TestCollision(const Ray_t &ray, unsigned int mask,
                                  trace_t &trace) {
...
void CBoneFollower::UpdateFollower(const Vector &position,
                                   const QAngle &orientation,
                                   float flInterval) {
...
CBoneFollower *CBoneFollower::Create(CBaseEntity *pOwner,
                                     const char *pModelName, solid_t &solid,
                                     const Vector &position,
                                     const QAngle &orientation) {
  CBoneFollower *pFollower =
      (CBoneFollower *)CreateEntityByName("phys_bone_follower");
  if (pFollower) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CBoneFollower::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
...
void CBoneFollower::Touch(CBaseEntity *pOther) {
  CBaseEntity *pOwner = GetOwnerEntity();
  if (pOwner) {
    // TODO: fill in the touch trace with the hitbox number associated with this
    // bone
    pOwner->Touch(pOther);
    return;
  }
...
//-----------------------------------------------------------------------------
void CBoneFollower::TraceAttack(const CTakeDamageInfo &info,
