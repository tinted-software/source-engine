...
CAI_Schedule *CAI_BaseNPC::ScheduleInList(const char *pName,
                                          CAI_Schedule **pList, int listCount) {
  int i;
...
#define ADD_DEF_SCHEDULE(name, localId)                                        \
  idSpace.AddSchedule(name, localId, "CAI_BaseNPC")

  CAI_ClassScheduleIdSpace &idSpace =
      CAI_BaseNPC::AccessClassScheduleIdSpaceDirect();
  ...
  ADD_DEF_SCHEDULE("SCHED_ALERT_REACT_TO_COMBAT_SOUND",
  ...
  ADD_DEF_SCHEDULE("SCHED_SWITCH_TO_PENDING_WEAPON",
  ...
  ADD_DEF_SCHEDULE("SCHED_INTERACTION_MOVE_TO_PARTNER",
                   SCHED_INTERACTION_MOVE_TO_PARTNER);
  ADD_DEF_SCHEDULE("SCHED_INTERACTION_WAIT_FOR_PARTNER",
...
bool CAI_BaseNPC::LoadDefaultSchedules(void) {
  //	AI_LOAD_DEF_SCHEDULE( CAI_BaseNPC,
  //SCHED_NONE);
...
int CAI_BaseNPC::TranslateSchedule(int scheduleType) {
  ...
  case SCHED_IDLE_STAND: {
    // FIXME: crows are set into IDLE_STAND as an failure schedule, not sure if
    // ALERT_STAND or COMBAT_STAND is a better choice Assert( m_NPCState ==
    // NPC_STATE_IDLE );
  } break;
  ...
  case SCHED_COMBAT_FACE: {
    // FIXME: failure schedule for SCHED_PATROL which can be called when in
    // alert Assert( m_NPCState == NPC_STATE_COMBAT );
  } break;
...
CAI_Schedule *CAI_BaseNPC::GetScheduleOfType(int scheduleType) {
  // allow the derived classes to pick an appropriate version of this schedule
  // or override base schedule types.
...
const char *CAI_BaseNPC::ConditionName(int conditionID) {
  if (AI_IdIsLocal(conditionID))
    conditionID =
        GetClassScheduleIdSpace()->ConditionLocalToGlobal(conditionID);
...
public:
  ...
  // UNDONE: Schedule / strings stuff should probably happen once each GAME, not
  // each level
...
AI_DEFINE_SCHEDULE(
    ...
    "	Interrupts"
    "		COND_NEW_ENEMY"
    "		COND_SEE_ENEMY" // in deference to scripted schedule where the
                                // enemy was slammed, thus no COND_NEW_ENEMY
    "		COND_LIGHT_DAMAGE"
    "		COND_HEAVY_DAMAGE"
    "		COND_SMELL"
    "		COND_PROVOKED"
...
AI_DEFINE_SCHEDULE(
    ...
    //SCHEDULE:SCHED_NEW_WEAPON_CHEAT"
...
AI_DEFINE_SCHEDULE(
    ...
    "	Tasks"
    "		TASK_SET_FAIL_SCHEDULE			"
    "SCHEDULE:SCHED_RUN_FROM_ENEMY_FALLBACK"
    "		TASK_STOP_MOVING				0"
    "		TASK_FIND_COVER_FROM_ENEMY		0"
    "		TASK_RUN_PATH					0"
    "		TASK_WAIT_FOR_MOVEMENT			0"
...
AI_DEFINE_SCHEDULE(
    ...
    "		TASK_SET_ROUTE_SEARCH_TIME		3" // Spend 3 seconds
                                                           // trying to build a
                                                           // path if stuck
    "		TASK_GET_PATH_TO_LASTPOSITION	0"
    "		TASK_WALK_PATH					0"
...
AI_DEFINE_SCHEDULE(
    ...
    "		TASK_STOP_MOVING				0"
    "		TASK_SET_FAIL_SCHEDULE			"
    "SCHEDULE:SCHED_PRE_FAIL_ESTABLISH_LINE_OF_FIRE"
    "		TASK_GET_CHASE_PATH_TO_ENEMY	300"
    "		TASK_RUN_PATH					0"
    "		TASK_WAIT_FOR_MOVEMENT			0"
    "		TASK_FACE_ENEMY			0"
...
AI_DEFINE_SCHEDULE(
    ...
    "	Tasks"
    //	"		TASK_SET_TOLERANCE_DISTANCE		48"
    "		TASK_SET_ROUTE_SEARCH_TIME		1" // Spend 1 seconds
                                                           // trying to build a
                                                           // path if stuck
...
//=========================================================
// > SCHED_MOVE_AWAY_END (allows derived class to translate to appropriate
// behavior)
//=========================================================
AI_DEFINE_SCHEDULE(
    SCHED_MOVE_AWAY_END,
