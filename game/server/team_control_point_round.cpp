...
void CTeamControlPointRound::FindControlPoints(void) {
  ...
  if (pMaster) {
    ...
    CBaseEntity *pEnt =
        gEntList.FindEntityByClassname(NULL, pMaster->GetControlPointName());
    ...
    while (pEnt) {
      ...
      pEnt =
          gEntList.FindEntityByClassname(pEnt, pMaster->GetControlPointName());
    }
  }
...
//-----------------------------------------------------------------------------
int CTeamControlPointRound::TeamOwnsAllPoints(
...
// Purpose:
//-----------------------------------------------------------------------------
bool CTeamControlPointRound::WouldNewCPOwnerWinGame(CTeamControlPoint *pPoint,
                                                    int iNewOwner) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CTeamControlPointRound::SelectedToPlay(void) { SetupSpawnPoints(); }
...
bool CTeamControlPointRound::MakePlayable(void) {
  ...
  if (pMaster) {
    if (!IsPlayable()) {
      // we need to try switching the owners of the teams to make this round
      // playable
      for (int iTeam = FIRST_GAME_TEAM; iTeam < GetNumberOfTeams(); iTeam++) {
        for (int iControlPoint = 0; iControlPoint < m_ControlPoints.Count();
             iControlPoint++) {
          if ((!pMaster->IsBaseControlPoint(
                  m_ControlPoints[iControlPoint]
                      ->GetPointIndex())) && // this is NOT the base point for
                                             // one of the teams (we don't want
                                             // to assign the base to the wrong
                                             // team)
