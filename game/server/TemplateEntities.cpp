...
//us to spawn 			entities with arbitrary sets of key/value data and entity I/O
//			connections.
//
//			Template entities are marked with a special spawnflag
//which causes 			them not to spawn, but to be saved as a string containing all the
//			map data (keyvalues and I/O connections) from the BSP.
//Template 			entities are looked up by name by the spawner, which copies the 			map
//data into a local string (that's how the template data is saved 			and restored).
//Once all the entities in the map have been activated, 			the template database is
//freed.
//
//=============================================================================//
...
// DEFINE_FIELD( pszName,			FIELD_STRING	),
// // Saved custom, see below DEFINE_FIELD( pszMapData,
// FIELD_STRING	),		// Saved custom, see below
DEFINE_FIELD(iszMapData, FIELD_STRING),
    ...
    // DEFINE_FIELD( pszFixedMapData,	FIELD_STRING	),		// Not
    // saved at all
    END_DATADESC()
...
//for values 			that match a name within the group's entity	names. Append %d
//to any 			found values, which will later be filled out by a unique identifier
...
//the template 			with unique key/values where the template requires them.
//-----------------------------------------------------------------------------
char *Templates_GetEntityIOFixedMapData(int iIndex) {
  ...
  int iFixupSize = strlen(
      ENTITYIO_FIXUP_STRING); // don't include \0 when copying in the fixup
  char *sOurFixup =
      new char[iFixupSize + 1]; // do alloc room here for the null terminator
  Q_snprintf(sOurFixup, iFixupSize + 1, "%c%.4d", ENTITYIO_FIXUP_STRING[0],
             g_iCurrentTemplateInstance);
