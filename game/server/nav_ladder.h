...
public:
  ...
  static void
  CompressIDs(void); ///< re-orders ladder ID's so they are continuous
  ...
  void ConnectGeneratedLadder(
      float maxHeightAboveTopArea); ///< Connect a generated ladder to nav areas
                                    ///< at the end of nav generation
  ...
  void OnSplit(CNavArea *original, CNavArea *alpha,
               CNavArea *beta); ///< when original is split into alpha and beta,
  ...
  bool IsInUse(const CBasePlayer *ignore = NULL)
      const; ///< return true if someone is on this ladder (other than 'ignore')
