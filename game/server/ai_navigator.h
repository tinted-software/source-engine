...
// 			specifically "flying" about it, other than it came from
// an attempte
//			to consolodate duplicated code in the various fliers. It
//may serve 			a more general purpose in the future. The constructor takes those
...
struct AI_ProgressFlyPathParams_t {
  AI_ProgressFlyPathParams_t(unsigned _collisionMask,
      ...
      : collisionMask(_collisionMask),
        ...
        blockTolerance(_blockTolerance), waypointTolerance(_waypointTolerance),
        goalTolerance(_goalTolerance), blockHandling(_blockHandling),
        pTarget(NULL), bTrySimplify(true) {}
  ...
  float goalTolerance; // @TODO (toml 07-03-02): goalTolerance appears to have
                       // come into existence because noone had set a good
                       // tolerance in the path itself. It is therefore
...
public:
  ...
  ProgressFlyPath(const AI_ProgressFlyPathParams_t
...
private:
  ...
  float m_timePathRebuildMax; // How long to try rebuilding path before failing
                              // task
  float
      m_timePathRebuildDelay; // How long to wait before trying to rebuild again
...
inline AI_NavGoal_t::AI_NavGoal_t(GoalType_t type, Activity activity,
                                  ...
                                  CBaseEntity *pTarget)
    : type(type), dest(AIN_NO_DEST), destNode(AIN_NO_NODE), activity(activity),
      tolerance(tolerance), maxInitialSimplificationDist(-1), flags(flags),
      pTarget(pTarget), arrivalActivity(AIN_DEF_ACTIVITY),
      arrivalSequence(ACT_INVALID) {}

inline AI_NavGoal_t::AI_NavGoal_t(const Vector &dest, Activity activity,
                                  float tolerance, unsigned flags,
                                  CBaseEntity *pTarget)
    : type(GOALTYPE_LOCATION), dest(dest), destNode(AIN_NO_NODE),
      activity(activity), tolerance(tolerance),
      maxInitialSimplificationDist(-1), flags(flags), pTarget(pTarget),
...
inline AI_NavGoal_t::AI_NavGoal_t(GoalType_t type, const Vector &dest,
    ...
    : type(type), dest(dest), destNode(AIN_NO_NODE), activity(activity),
      tolerance(tolerance), maxInitialSimplificationDist(-1), flags(flags),
      pTarget(pTarget), arrivalActivity(AIN_DEF_ACTIVITY),
      arrivalSequence(ACT_INVALID) {}
...
inline AI_NavGoal_t::AI_NavGoal_t(AI_PathNode_t destNode, Activity activity,
                                  float tolerance, unsigned flags,
                                  CBaseEntity *pTarget)
    : type(GOALTYPE_LOCATION), dest(AIN_NO_DEST), destNode(destNode),
      activity(activity), tolerance(tolerance),
      maxInitialSimplificationDist(-1), flags(flags), pTarget(pTarget),
      arrivalActivity(AIN_DEF_ACTIVITY), arrivalSequence(ACT_INVALID) {}
...
inline AI_NavGoal_t::AI_NavGoal_t(GoalType_t type, AI_PathNode_t destNode,
                                  Activity activity, float tolerance,
                                  unsigned flags, CBaseEntity *pTarget)
    : type(type), dest(AIN_NO_DEST), destNode(destNode), activity(activity),
      tolerance(tolerance), maxInitialSimplificationDist(-1), flags(flags),
      pTarget(pTarget), arrivalActivity(AIN_DEF_ACTIVITY),
      arrivalSequence(ACT_INVALID) {}
