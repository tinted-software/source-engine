...
ConVar nav_slope_limit("nav_slope_limit", "0.7", FCVAR_CHEAT,
                       "The ground unit normal's Z component must be greater "
                       "than this for nav areas to be generated.");
ConVar
    nav_slope_tolerance("nav_slope_tolerance", "0.1", FCVAR_CHEAT,
                        "The ground unit normal's Z component must be this "
                        "close to the nav area's Z component to be generated.");
ConVar nav_displacement_test("nav_displacement_test", "10000", FCVAR_CHEAT,
                             "Checks for nodes embedded in displacements "
                             "(useful for in-development maps)");
ConVar
    nav_generate_fencetops("nav_generate_fencetops", "1", FCVAR_CHEAT,
                           "Autogenerate nav areas on fence and obstacle tops");
ConVar nav_generate_fixup_jump_areas(
    "nav_generate_fixup_jump_areas", "1", FCVAR_CHEAT,
...
ConVar nav_generate_incremental_range("nav_generate_incremental_range", "2000",
                                      FCVAR_CHEAT);
ConVar
    nav_generate_incremental_tolerance("nav_generate_incremental_tolerance",
                                       "0", FCVAR_CHEAT,
...
bool FindGroundForNode(Vector *pos,
                       Vector *normal); // find a ground Z for pos that is clear
                                        // for NavTraceMins -> NavTraceMaxs

const float MaxTraversableHeight =
    StepHeight; // max internal obstacle height that can occur between nav nodes
                // and safely disregarded
const float MinObstacleAreaWidth =
    10.0f; // min width of a nav area we will generate on top of an obstacle
...
inline CNavArea *findFirstAreaInDirection(const Vector *start, NavDirType dir,
                                          float range, float beneathLimit,
                                          CBaseEntity *traceIgnore = NULL,
...
void CNavMesh::BuildLadders(void) {
  ...
  while ((ladder = dynamic_cast<CFuncSimpleLadder *>(
              gEntList.FindEntityByClassname(ladder, "func_simpleladder"))) !=
         NULL) {
    Vector mins, maxs;
...
void CNavMesh::CreateLadder(const Vector &absMin, const Vector &absMax,
  ...
  if (xSize > ySize) {
    ...
    // "pull in" traceline from bottom and top in case ladder abuts floor and/or
    // ceiling
  ...
  ladder->SetDir(ladder->GetDir()); // now that we've adjusted the top and
                                    // bottom, re-check the normal
...
void CNavMesh::CreateLadder(const Vector &top, const Vector &bottom,
                            float width, const Vector2D &ladderDir,
                            float maxHeightAboveTopArea) {
  CNavLadder *ladder = new CNavLadder;
...
void CNavLadder::ConnectGeneratedLadder(float maxHeightAboveTopArea) {
  ...
  // find "ahead" area
  m_topForwardArea = findFirstAreaInDirection(
      &center, OppositeDirection(m_dir), nearLadderRange, beneathLimit, NULL);
  ...
  // find "right" area
  m_topRightArea = findFirstAreaInDirection(
      &center, DirectionRight(m_dir), nearLadderRange, beneathLimit, NULL);
  if (m_topRightArea == m_bottomArea)
    m_topRightArea = NULL;

  // find "behind" area - must look farther, since ladder is against the wall
  // away from this area
  m_topBehindArea = findFirstAreaInDirection(
      &center, m_dir, 2.0f * nearLadderRange, beneathLimit, NULL);
...
public:
  bool operator()(CNavArea *jumpArea) {
    ...
    for (int i = 0; i < NUM_DIRECTIONS; ++i) {
      ...
      const NavConnectVector *incoming =
          jumpArea->GetIncomingConnections(incomingDir);
  ...
  void TryToConnect(CNavArea *jumpArea, CNavArea *sourceArea,
    ...
    FOR_EACH_VEC((*dest), destIt) {
      ...
      if (destArea->HasAttributes(NAV_MESH_JUMP)) {
        // Don't connect areas across 2 jump areas.  This means we'll have some
        // missing links due to sampling errors. This is preferable to
        // generating incorrect links across multiple jump areas, which is far
        // more common.
        continue;
      }
      ...
      if (halfWidth > 0.0f) {
        ...
        if ((sourcePos - destPos)
                .AsVector2D()
                .IsLengthLessThan(GenerationStepSize * 3)) {
          sourceArea->ConnectTo(destArea, outgoingDir);
          //					DevMsg( "Connected %d->%d via %d
          //(len %f)\n", 						sourceArea->GetID(), destArea->GetID(),
          //jumpArea->GetID(), sourcePos.DistTo( destPos ) );
        }
      }
    }
  }
...
void CNavMesh::MarkPlayerClipAreas(void) {
  ...
  FOR_EACH_VEC(TheNavAreas, it) {
    ...
    // Trace upward a bit from our center point just colliding wtih PLAYERCLIP
    // to see if we're in one, if we are, mark us as accordingly.
    trace_t trace;
    Vector start = area->GetCenter() + Vector(0.0f, 0.0f, 16.0f);
    Vector end = area->GetCenter() + Vector(0.0f, 0.0f, 32.0f);
    UTIL_TraceHull(start, end, Vector(0, 0, 0), Vector(0, 0, 0),
                   CONTENTS_PLAYERCLIP, NULL, &trace);

    if (trace.fraction < 1.0) {
      area->SetAttributes(area->GetAttributes() |
                          TerrorNavArea::NAV_PLAYERCLIP);
    }
  }
#endif
}
  ...
  FOR_EACH_VEC(TheNavAreas, it) {
    ...
    if (lowestNormalZ < nav_slope_limit.GetFloat()) {
      ...
      if (GetSimpleGroundHeight(testPos, &dummy, &groundNormal)) {
        // If the ground normal is divergent from the area's normal, mark it as
        // a jump area - it's not really representative of the ground.
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Remove all areas marked as jump areas and connect the areas connecting to
 * them
 *
 */
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Adjusts obstacle start and end distances such that obstacle width (end-start)
 * is not less than MinObstacleAreaWidth, and end distance is not greater than
 * maxAllowedDist
 */
void AdjustObstacleDistances(float *pObstacleStartDist, float *pObstacleEndDist,
  ...
  if (obstacleWidth < MinObstacleAreaWidth) {
    ...
    *pObstacleEndDist = *pObstacleStartDist + MinObstacleAreaWidth;
    ...
    // if this pushes the end past max allowed distance, pull start and end back
    // so that end is within allowed distance
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Makes sure tall, slim obstacles like fencetops, railings and narrow walls
 * have nav areas placed on top of them to allow climbing & traversal
 */
void CNavMesh::HandleObstacleTopAreas(void) {
  if (!nav_generate_fencetops.GetBool())
    return;

  // For any 1x1 area that is internally blocked by an obstacle, raise it on top
  // of the obstacle and size to fit.
...
//--------------------------------------------------------------------------------------------------------------
/**
 * For any nav area that has internal obstacles between its corners of greater
 * than traversable height, raise that nav area to sit at the top of the
 * obstacle, and shrink it to fit the obstacle.  Such nav areas are already
...
void CNavMesh::RaiseAreasWithInternalObstacles() {
  ...
  FOR_EACH_VEC(TheNavAreas, it) {
    ...
    // any nav area with internal obstacles will be 1x1 (width and height =
    // GenerationStepSize), so only need to consider areas of that size
    ...
    // Look at all 4 directions and determine if there are obstacles in that
    // direction.  Find the direction with the highest obstacle, if any.
    for (int i = 0; i < NUM_DIRECTIONS; i++) {
      NavDirType dir = (NavDirType)i;

      // For this direction, look at the left and right edges of the nav area
      // relative to this direction and determined if they are both blocked by
      // obstacles.  We only consider this area obstructed if both edges are
      // blocked (e.g. fence runs all the way through it).

      NavCornerType corner[2];
      int iEdgesBlocked = 0;
      corner[0] = (NavCornerType)((i + 3) %
                                  NUM_CORNERS); // lower left-hand corner
                                                // relative to current direction
      corner[1] = (NavCornerType)((i + 2) %
                                  NUM_CORNERS); // lower right-hand corner
                                                // relative to current direction
      float obstacleZThisDir[2] = {
          -FLT_MAX, -FLT_MAX}; // absolute Z pos of obstacle for left and right
                               // edge in this direction
      float obstacleStartDistThisDir =
          GenerationStepSize; // closest obstacle start distance in this
                              // direction
      float obstacleEndDistThisDir =
          0; // farthest obstacle end distance in this direction
      ...
      for (int iEdge = 0; iEdge < 2; iEdge++) {
        ...
        if (nodeFrom) {
          ...
          if (obstacleHeight > MaxTraversableHeight) {
            ...
            iEdgesBlocked++;
            // keep track of obstacle height and start and end distance for this
            // edge
            float obstacleZ = nodeFrom->GetPosition()->z + obstacleHeight;
            if (obstacleZ > obstacleZThisDir[iEdge]) {
              obstacleZThisDir[iEdge] = obstacleZ;
            }
            obstacleStartDistThisDir = MIN(nodeFrom->m_obstacleStartDist[dir],
                                           obstacleStartDistThisDir);
            obstacleEndDistThisDir =
                MAX(nodeFrom->m_obstacleEndDist[dir], obstacleEndDistThisDir);
          }
        }
      }
      ...
      if (connections) {
        for (int conIndex = 0; conIndex < connections->Count(); ++conIndex) {
          ...
          if (connectedArea && connectedArea->HasAttributes(NAV_MESH_STAIRS)) {
            ...
            BlockedEdgeCutoff = 1; // one blocked edge is already too much when
                                   // we're next to a stair
            break;
          }
        }
      }

      // are both edged blocked in this direction, and is the obstacle height in
      // this direction the tallest we've seen?
      if ((iEdgesBlocked >= BlockedEdgeCutoff) &&
          (MAX(obstacleZThisDir[0], obstacleZThisDir[1])) > obstacleZMax) {
        // this is the tallest obstacle we've encountered so far, remember its
        // details
    ...
    if (obstacleZMax > -FLT_MAX) {
      // enforce minimum obstacle width so we don't shrink to become a teensy
      // nav area
      ...
      case SOUTH:
        ...
        ::V_swap(obstacleZ[0],
                 obstacleZ[1]); // swap left and right Z heights for obstacle so
                                // we can run common code below
        break;
      case EAST:
      ...
      // move the area
      RemoveNavArea(area);
      area->Build(corner[NORTH_WEST], corner[NORTH_EAST], corner[SOUTH_EAST],
                  corner[SOUTH_WEST]);
      Assert(!area->IsDegenerate());
      AddNavArea(area);

      // remove side-to-side connections if there are any so AI does try to do
      // things like run along fencetops
      area->RemoveOrthogonalConnections(obstacleDir);
      area->SetAttributes(area->GetAttributes() | NAV_MESH_NO_MERGE |
                          NAV_MESH_OBSTACLE_TOP);
      area->SetAttributes(area->GetAttributes() & (~NAV_MESH_JUMP));
      // clear out the nodes associated with this area's corners -- corners
      // don't match the node positions any more
...
//--------------------------------------------------------------------------------------------------------------
/**
 * For any two nav areas that have an obstacle between them such as a fence,
 * railing or small wall, creates a new nav area on top of the obstacle and
 * connects it between the areas
 */
void CNavMesh::CreateObstacleTopAreas() {
  ...
  FOR_EACH_VEC(TheNavAreas, it) {
    ...
    // if this is a jump node (which will ultimately get removed) or is an
    // obstacle top, ignore it
    ...
    for (int i = NORTH; i < NUM_DIRECTIONS; i++) {
      ...
      for (int j = 0; j < iConnections; j++) {
        ...
        // create an obstacle top if there is a one-node separation between the
        // areas and there is an intra-node obstacle within that separation
        if (!CreateObstacleTopAreaIfNecessary(area, areaOther, dir, false)) {
          // if not, create an obstacle top if there is a two-node separation
          // between the areas and the intermediate node is significantly higher
          // than the two areas, which means there's some geometry there that
          // causes the middle node to be higher
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Creates a new nav area if an obstacle exists between the two nav areas.  If
 * bMultiNode is false, this checks if there's a one-node separation between the
 * areas, and if so if there is an obstacle detected between the nodes. If
 * bMultiNode is true, checks if there is a two-node separation between the
 * areas, and if so if the middle node is higher than the two areas, suggesting
...
bool CNavMesh::CreateObstacleTopAreaIfNecessary(CNavArea *area,
  ...
  if (halfPortalWidth > 0) {
    ...
    // along the edge of this area that faces the other area, look at every node
    // that's in the portal between the two
    while (node) {
      ...
      if (vecToPortalCenter.IsLengthLessThan(halfPortalWidth + 1.0f)) {
        ...
        if (!bMultiNode) {
          // use the inter-node obstacle height from this node toward the next
          // area
            ...
            if (nodeTowardOtherArea) {
              ...
              if (deltaZ > MaxTraversableHeight) {
                ...
                if (areaOther->Contains(*nodeTowardOtherArea->GetPosition())) {
                  ...
                  float deltaZ =
                      fabs(nodeTowardOtherArea->GetPosition()->z - z);
                  if (deltaZ < 2.0f) {
                    bInOtherArea = true;
                  }
                }

                // if we have not arrived in the other area yet, take one more
                ...
                if (!bInOtherArea) {
                  ...
                  if (nodeTowardOtherArea2 &&
                    ...
                    float areaDeltaZ = node->GetPosition()->z -
                                       nodeTowardOtherArea2->GetPosition()->z;
                    if (fabs(areaDeltaZ) <= MaxTraversableHeight) {
                      // if we arrived in the other area, the obstacle height to
                      // get here was the peak deltaZ of the node above to get
                      // here
                      obstacleHeight = deltaZ;
                      // make a nav area MinObstacleAreaWidth wide centered on
                      // the peak node, which is GenerationStepSize away from
                      // where we started
        ...
        if (obstacleHeightStart == 0) {
          // keep track of the obstacle height and node z pos at the start of
          // the edge
          obstacleHeightStart = obstacleHeight;
          zStart = node->GetPosition()->z;
        }
        // keep track of the obstacle height and node z pos at the end of the
        // edge
    ...
    // if we had some obstacle height from EVERY node along the portal, then
    // getting from this area to the other requires scaling an obstacle, need to
    // generate a nav area on top of it
    if ((obstacleHeightMax > MaxTraversableHeight) &&
        (obstacleHeightMin > MaxTraversableHeight)) {
      // If the maximum obstacle height was greater than both the height at
      // start and end of the edge, then the obstacle is highest somewhere in
      // the middle.  Use that as the height of both ends.
      ...
      // for south and west, swap "start" and "end" values of edges so we can
      // use common code below
      ...
      AdjustObstacleDistances(&obstacleDistMin, &obstacleDistMax,
                              bMultiNode ? GenerationStepSize * 2
                                         : GenerationStepSize);
      ...
      case SOUTH:
        ...
        cornerNE.Init(center.x + halfPortalWidth, center.y - (newAreaWidth / 2),
                      zEnd + obstacleHeightEnd);
        cornerSE.Init(center.x + halfPortalWidth, center.y + (newAreaWidth / 2),
                      zEnd + obstacleHeightEnd);
      ...
      case WEST:
        cornerNW.Init(center.x - (newAreaWidth / 2), center.y - halfPortalWidth,
                      zStart + obstacleHeightStart);
        cornerNE.Init(center.x + (newAreaWidth / 2), center.y - halfPortalWidth,
                      zEnd + obstacleHeightEnd);
        cornerSE.Init(center.x + (newAreaWidth / 2), center.y + halfPortalWidth,
                      zEnd + obstacleHeightEnd);
      ...
      Msg("Created new fencetop area %d(%x) between %d(%x) and %d(%x)\n",
          ...
          area->GetDebugID(), areaOther->GetID(), areaOther->GetDebugID());

      areaNew->SetAttributes(area->GetAttributes());
      areaNew->SetAttributes(area->GetAttributes() | NAV_MESH_NO_MERGE |
                             NAV_MESH_OBSTACLE_TOP);
...
void CNavMesh::RemoveOverlappingObstacleTopAreas() {
  // What we really want is the union of all obstacle top areas that get
  // generated.  That would be hard to compute exactly, so instead we'll just
  // remove any that overlap.  The obstacle top areas don't have to be exact, we
  // just need enough of them so there is generally a path to get over any
  // obstacle.

  // make a list of just the obstacle top areas to reduce the N of the N squared
  // operation we're about to do
    ...
    for (int it2 = it + 1; it2 < vecObstacleTopAreas.Count(); it2++) {
      ...
      if (area->IsOverlapping(areaOther)) {
        if (area->Contains(areaOther)) {
          ...
          // if they overlap without one being a superset of the other, just
          // remove the smaller area
...
static void CommandNavCheckStairs(void) { TheNavMesh->MarkStairAreas(); }
...
StairTestType IsStairs(const Vector &start, const Vector &end,
  ...
  const float MinStairNormal =
      0.97f; // we don't care about ramps, just actual flat steps
  ...
  if (abs(start.z - end.z) > StepHeight) {
    ...
    UTIL_TraceHull(start + traceOffset, start - traceOffset, hullMins, hullMaxs,
                   MASK_NPCSOLID, &filter, &trace);
    ...
    for (t = 0.0f; t <= 1.0f; t += traceIncrement) {
      ...
      UTIL_TraceHull(pos + traceOffset, pos - traceOffset, hullMins, hullMaxs,
                     MASK_NPCSOLID, &filter, &trace);
      if (trace.startsolid || trace.IsDispSurface()) {
        return STAIRS_NO;
      }
...
bool CNavArea::TestStairs(void) {
  ...
  const float inset =
      5.0f; // inset to keep the tests completely inside the nav area
...
//--------------------------------------------------------------------------------------------------------------
CON_COMMAND_F(nav_test_stairs, "Test the selected set for being on stairs",
              FCVAR_CHEAT) {
  int count = 0;
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Jump areas aren't used by the NextBot.  Delete them, connecting adjacent
 * areas.
 */
...
public:
  bool operator()(CNavArea *area) { return area->HasNodes(); }
};

//--------------------------------------------------------------------------------------------------------
/**
 * Incremental generation fixup for where edges lap up against the existing nav
 * mesh: we have nodes, but the surrounding areas don't.  So, we trace outward,
 * to see if we can walk/fall to an adjacent area.  This handles dropping down
 * into existing areas etc.
...
class AreaSet {
public:
  AreaSet(CUtlVector<CNavArea *> *areas) { m_areas = areas; }

  bool operator()(CNavArea *area) { return (m_areas->HasElement(area)); }
...
//--------------------------------------------------------------------------------------------------------
/**
 * Stitches an arbitrary set of areas (newly-merged, for example) into the
 * existing mesh
 */
...
inline CNavArea *findJumpDownArea(const Vector *fromPos, NavDirType dir) {
  ...
  Vector toPos;
  CNavArea *downArea = findFirstAreaInDirection(
      &start, dir, 4.0f * GenerationStepSize, DeathDrop, NULL, &toPos);
...
void CNavMesh::StitchAreaIntoMesh(CNavArea *area, NavDirType dir,
  ...
  for (float n = 0; n < edgeLength - 1.0f; n += GenerationStepSize) {
    ...
    if (targetArea && !func(targetArea)) {
      targetPos.z =
          targetArea->GetZ(targetPos.x, targetPos.y) + HalfHumanHeight;
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Checks to see if there is a cliff - a drop of at least CliffHeight - in
 * specified direction.
 */
inline bool CheckCliff(const Vector *fromPos, NavDirType dir,
                       bool bExhaustive = true) {
  // cliffs are half-baked, not used by any existing AI, and create poorly
  // behaved nav areas (ie: long, thin, strips) (MSB 8/7/09)
  return false;
  ...
  if (TraceAdjacentNode(0, *fromPos, toPos, &trace, DeathDrop * 10) &&
    ...
    // if not, special case for south and east.  South and east edges are not
    // considered part of a nav area, so we look ahead two steps for south and
    // east.  This ensures that the n-1th row and column of nav nodes on the
    // south and east sides of a nav area reflect any cliffs on the nth row and
    // column.

    // if we're looking to south or east, and the first node we found was
    // approximately flat, and this is the top-level call, recurse one level to
    // check one more step in this direction
...
void CNavMesh::ConnectGeneratedAreas(void) {
  ...
  FOR_EACH_VEC(TheNavAreas, it) {
    ...
    // south edge part 2 - scan the actual south edge.  If the node is not part
    // of an adjacent area, then it really belongs to us.  This will happen if
    // our area runs right up against a ledge.
    for (node = area->m_node[SOUTH_WEST]; node != area->m_node[SOUTH_EAST];
      ...
      if (node->IsBlockedInAnyDirection() ||
        ...
        continue; // The space around this node is blocked, so don't connect
                  // across it

      // Don't directly connect to adj's area, since it's already 1 cell removed
      // from our area. There was no area in between, presumably for good
      // reason.  Only look for jump down links.
      if (!adj || !adj->GetArea()) {
        CNavArea *downArea = findJumpDownArea(node->GetPosition(), SOUTH);
        if (downArea && downArea != area)
          area->ConnectTo(downArea, SOUTH);
    ...
    // east edge part 2 - scan the actual east edge.  If the node is not part of
    // an adjacent area, then it really belongs to us.  This will happen if our
    // area runs right up against a ledge.
  ...
  do {
    ...
    FOR_EACH_VEC(TheNavAreas, it) {
      ...
      FOR_EACH_VEC(area->m_connect[NORTH], nit) {
        ...
        if (!area->IsAbleToMergeWith(
                adjArea)) // pre-existing areas in incremental generates won't
                          // have nodes
          continue;
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Given arbitrary corners of a compass grid-aligned rectangle, classify them by
 * compass direction. Input: vec[4]: arbitrary corners Output: vecNW, vecNE,
 * vecSE, vecSW: filled in with which corner is in which compass direction
 */
...
void CNavMesh::FixConnections(void) {
  // Test the steep sides of stairs for any outgoing links that cross nodes that
  // were partially obstructed.
  FOR_EACH_VEC(TheNavAreas, it) {
    CNavArea *area = TheNavAreas[it];
    ...
    for (int dir = 0; dir < NUM_DIRECTIONS; ++dir) {
      ...
      // Flat edges of stairs need to connect.  It's the slopes we don't want to
      // climb over things for.
      float cornerDeltaZ = fabs(area->GetCorner(cornerType[0]).z -
                                area->GetCorner(cornerType[1]).z);
      if (cornerDeltaZ < StepHeight)
        continue;

      const NavConnectVector *connectedAreas =
          area->GetAdjacentAreas((NavDirType)dir);
      ...
      for (int i = 0; i < connectedAreas->Count(); ++i) {
        ...
        CNavNode *adjNode = adjArea->FindClosestNode(
            adjPos, OppositeDirection((NavDirType)dir));
        ...
        // From the stair's perspective, we can't go up more than step height to
        // reach the adjacent area. Also, if the adjacent area has to jump up
        // higher than StepHeight above the stair area to reach the stairs,
        // there's an obstruction close to the adjacent area that could prevent
        // walking from the stairs down.
  ...
  // Test to prevent A->C if A->B->C.  This can happen in doorways and dropdowns
  // from rooftops.
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Fix any spots where we there are nav nodes touching only corner-on-corner but
 * we intend bots to be able to traverse
 */
void CNavMesh::FixCornerOnCornerAreas(void) {
  const float MaxDrop =
      StepHeight; // don't make corner on corner areas that are too steep

  FOR_EACH_VEC(TheNavAreas, it) {
    CNavArea *area = TheNavAreas[it];

    // determine if we have any corners where the only nav area we touch is
    // diagonally corner-to-corner. if there are, generate additional small (0.5
    // x 0.5 grid size) nav areas in the corners between them if map geometry
    // allows and make connections in cardinal compass directions to create a
    ...
    for (int iCorner = NORTH_WEST; iCorner < NUM_CORNERS; iCorner++) {
      ...
      // if we have any connections on cardinal compass directions on edge on
      // either side of corner we're OK, skip this nav area
      ...
      // consider 2 potential new nav areas, to left and right of the corner
      // we're considering
      for (int iDir = 0; iDir < ARRAYSIZE(dirsAlongOtherEdge); iDir++) {
        ...
        if (!areaOther)
          continue; // no other area in that location, we're not touching on
                    // corner

        // see if we can move from our corner in that direction
        trace_t result;
        if (!TraceAdjacentNode(0, cornerPos, vecOtherEdgePos, &result, MaxDrop))
          continue; // something is blocking movement, don't create additional
                    // nodes to aid movement

        // get the corner of the other nav area that might touch our corner
        int iCornerOther = ((iCorner + 2) % NUM_CORNERS);
        Vector cornerPosOther =
            areaOther->GetCorner((NavCornerType)iCornerOther);

        if (cornerPos != cornerPosOther)
          continue; // that nav area does not touch us on corner

        // we are touching corner-to-corner with the other nav area and don't
        // have connections in cardinal directions around the corner that
        // touches, this is a candidate to generate new small helper nav areas.

        // calculate the corners of the 0.5 x 0.5 nav area we would consider
        // building between us and the other nav area whose corner we touch
        ...
        Vector vecCorner[4];
        vecCorner[0] = cornerPos + vecDeltaOtherEdge +
                       vecDeltaOurEdge; // far corner of new nav area
        vecCorner[1] = cornerPos +
                       vecDeltaOtherEdge; // intersection of far edge of new nav
                                          // area with other nav area we touch
        vecCorner[2] = cornerPos; // common corner of this nav area, nav area we
                                  // touch, and new nav area
        vecCorner[3] =
            cornerPos + vecDeltaOurEdge; // intersection of far edge of new nav
                                         // area with this nav area

        CTraceFilterWalkableEntities filter(NULL, COLLISION_GROUP_NONE,
                                            WALK_THRU_EVERYTHING);
        if (!TraceAdjacentNode(0, vecCorner[1], vecCorner[0], &result,
                               MaxDrop) || // can we move from edge of other
                                           // area to far corner of new node
            !TraceAdjacentNode(0, vecCorner[3], vecCorner[0], &result,
                               MaxDrop)) // can we move from edge of this area
                                         // to far corner of new node
          continue;                      // new node would not fit

        // as sanity check, make sure there's not already a nav area there,
        // shouldn't be
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Fix any areas where one nav area overhangs another and the two nav areas are
 * connected.  Subdivide the lower nav area such that the upper nav area doesn't
 * overhang any area it's connected to.
 */
  ...
  do {
    ...
    for (int it = 0; it < TheNavAreas.Count() && !bRestartProcessing; it++) {
      ...
      for (int dir = NORTH; dir < NUM_DIRECTIONS && !bRestartProcessing;
        ...
        const NavConnectVector *pConnections =
            area->GetAdjacentAreas((NavDirType)dir);
        for (int iConnection = 0;
             iConnection < pConnections->Count() && !bRestartProcessing;
             iConnection++) {
          ...
          if (area->IsOverlapping(otherArea)) {
            // if the upper area isn't at least crouch height above the lower
            // area, this is some weird minor overlap, disregard it
            ...
            NavDirType dirFromBelowToAbove =
            ...
            float splitCoord; // absolute world coordinate along which we will
                              // split lower nav area (X or Y, depending on axis
                              // we split on)
            float splitLen;   // length of the segment of lower nav area that is
                              // in shadow of the upper nav area
            float splitEdgeSize; // current length of the edge of nav area that
                                 // is getting split
            bool bSplitAlongX = false;

            // determine along what edge we are splitting and make some key
            // measurements
            ...
            // if we split the lower nav area right where it's in shadow of the
            // upper nav area, will it create a really tiny strip?
            if (splitLen < GenerationStepSize) {
              // if the "in shadow" part of the lower nav area is really small
              // or the lower nav area is really small to begin with, don't
              // split it, we're better off as is
              if ((splitLen < GenerationStepSize * 0.3) ||
                  (splitEdgeSize <= GenerationStepSize * 2))
                continue;

              // Move our split point so we don't create a really tiny strip on
              // the lower nav area.  Move the split point away from the upper
              // nav area so the "in shadow" area expands to be
              // GenerationStepSize.  The checks above ensure we have room to do
              // this.
              float splitDelta = GenerationStepSize - splitLen;
              splitCoord += splitDelta * (((dirFromAboveToBelow == NORTH) ||
                                           (dirFromAboveToBelow == WEST))
                                              ? -1
                                              : 1);
            }

            // remove any connections between the two areas (so they don't get
            // inherited by the new areas when we split the lower area), but
            // remember what the connections were.
            ...
            //						int idBelow =
            //areaBelow->GetID(); 						AddToSelectedSet( areaBelow );
            // split the lower nav area
            if (areaBelow->SplitEdit(bSplitAlongX, splitCoord, &pNewAlpha,
                                     &pNewBeta)) {
              //							Msg(
              //"Split area %d into %d and %d\n", idBelow, pNewAlpha->GetID(),
              //pNewBeta->GetID() );

              // determine which of the two new lower areas is the one *not* in
              // shadow of the upper nav area.  This is the one we want to
              // reconnect to
              ...
              // restore the previous connections from the upper nav area to the
              // new lower nav area that is not in shadow of the upper
              ...
              // Now we need to just start the whole process over.  We've just
              // perturbed the list we're iterating on (removed a nav area,
              // added two new ones, when we did the split), and it's possible
              // we may have to subdivide a lower nav area twice if the upper
              // nav area overhangs a corner of the lower area.  We just start
              // all over again each time we do a split until no more overhangs
              // occur.
              bRestartProcessing = true;
            } else {
              //							Msg(
              //"Failed to split area %d\n", idBelow );
            }
          }
        }
      }
...
bool TestForValidCrouchArea(CNavNode *node) {
  // must make sure we don't have a bogus crouch area.  check up to
  // JumpCrouchHeight above the node for a HumanCrouchHeight space.
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Make sure that if other* are similar, test is also close.  Used in
 * TestForValidJumpArea.
 */
bool IsHeightDifferenceValid(float test, float other1, float other2,
  ...
  // Now make sure the test node is near the others.  If it is more than
  // StepHeight away, it'll form a distorted jump area.
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Check that a 1x1 area with 'node' at the northwest corner has a valid shape -
 * if 3 corners are flat, and the 4th is significantly higher or lower, it would
 * form a jump area that bots can't navigate over well.
 */
bool TestForValidJumpArea(CNavNode *node) {
  ...
  if (!IsHeightDifferenceValid(node->GetPosition()->z, south->GetPosition()->z,
                               southEast->GetPosition()->z,
                               east->GetPosition()->z))
    return false;

  if (!IsHeightDifferenceValid(south->GetPosition()->z, node->GetPosition()->z,
                               southEast->GetPosition()->z,
                               east->GetPosition()->z))
    return false;
  ...
  if (!IsHeightDifferenceValid(east->GetPosition()->z, south->GetPosition()->z,
                               southEast->GetPosition()->z,
                               node->GetPosition()->z))
    return false;
...
public:
  ...
  // This approximates CNavArea::GetZ, so we can pretend our four corners
  // delineate a nav area
  ...
  bool OverlapsExistingArea(void) {
    ...
    while (start.x < se.x) {
      ...
      while (start.y < se.y) {
        ...
        if (TheNavMesh->FindNavAreaOrLadderAlongRay(
                start, end, &overlappingArea, &overlappingLadder, NULL)) {
          if (overlappingArea) {
            return true;
          }
        }
 ...
 * All of the nodes must be approximately co-planar w.r.t the NW node's normal,
 * with the exception of 1x1 areas which can be any angle.
 */
bool CNavMesh::TestArea(CNavNode *node, int width, int height) {
  ...
  for (y = 0; y < height; y++) {
    ...
    for (x = 0; x < width; x++) {
      //
      // Compute the crouch attributes for the test node, taking into account
      // only the side(s) of the node that are in the area

      // NOTE: The nodes on the south and east borders of an area aren't
      // contained in the area.  This means that crouch attributes and blocked
      // state need to be checked to the south and east of the southEdge and
      // eastEdge nodes.
      ...
      if (northEdge && westEdge) {
        // The area's interior will be the south-east side of this north-west
        // node. If that interior space is blocked, there's no space to build an
        // area.
        ...
        // interior space of the area extends one more cell to the east past the
        // easternmost nodes. This means we need to check to the southeast as
        // well as the southwest.
        horizNodeCrouch =
            horizNode->m_crouch[SOUTH_EAST] || horizNode->m_crouch[SOUTH_WEST];
        if (horizNode->m_isBlocked[SOUTH_EAST] ||
            horizNode->m_isBlocked[SOUTH_WEST]) {
          return false;
        }
      } else if (southEdge && westEdge) {
        // The interior space of the area extends one more cell to the south
        // past the southernmost nodes. This means we need to check to the
        // southeast as well as the southwest.
        horizNodeCrouch =
            horizNode->m_crouch[SOUTH_EAST] || horizNode->m_crouch[NORTH_EAST];
        if (horizNode->m_isBlocked[SOUTH_EAST] ||
            horizNode->m_isBlocked[NORTH_EAST]) {
          return false;
        }
      } else if (southEdge && eastEdge) {
        // This node is completely in the interior of the area, so we need to
      ...
      if (width > 1 || height > 1) {
        float dist =
            (float)fabs(DotProduct(*horizNode->GetPosition(), normal) + d);
    ...
    if (width > 1 || height > 1) {
      float dist =
          (float)fabs(DotProduct(*vertNode->GetPosition(), normal) + d);
      if (dist > offPlaneTolerance)
        return false;
    }
  }
  ...
  for (y = 0; y < height; ++y) {
    ...
    for (int x = 0; x < width; ++x) {
      // look for odd jump areas (3 points on the ground, 1 point floating much
      // higher or lower)
      ...
      // This finds pillars etc in the middle of 4 nodes, that weren't found
      // initially.
  ...
  if (m_generationMode == GENERATE_INCREMENTAL) {
    // Incremental generation needs to check that it's not overlapping existing
    // areas...
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Checks if a node has an untraversable obstacle in any direction to a
 * neighbor. width and height are size of nav area this node would be a part of,
 * x and y are node's position within that grid
 */
bool CNavMesh::CheckObstacles(CNavNode *node, int width, int height, int x,
                              int y) {
  // any area bigger than 1x1 can't have obstacles in any connection between
  // nodes
  if (width > 1 || height > 1) {
...
int CNavMesh::BuildArea(CNavNode *node, int width, int height) {
  ...
  // since all internal nodes have the same attributes, set this area's
  // attributes

  area->SetAttributes(node->GetAttributes());

  // If any of the corners have an obstacle in the direction of another corner,
  // then there's an internal obstruction of this nav node. Mark it as not
  // mergable so it doesn't become a part of anything else and we will fix it up
  // later.
  if (nwNode->m_obstacleHeight[SOUTH] > MaxTraversableHeight ||
    ...
    Assert(width == 1); // We should only ever try to build a 1x1 area out of
                        // any two nodes that have an obstruction between them
    Assert(height == 1);
...
void CNavMesh::CreateNavAreasFromNodes(void) {
  ...
  MarkPlayerClipAreas();
  MarkJumpAreas(); // mark jump areas before we merge generated areas, so we
                   // don't merge jump and non-jump areas
  MergeGeneratedAreas();
...
void CNavMesh::AddWalkableSeeds(void) {
  CBaseEntity *spawn =
      gEntList.FindEntityByClassname(NULL, GetPlayerSpawnName());
...
void CNavMesh::BeginGeneration(bool incremental) {
  ...
  // Right now, incrementally-generated areas won't connect to existing areas
  // automatically. Since this means hand-editing will be necessary, don't do a
  // full analyze.
  if (incremental) {
  ...
  // build internal representations of ladders, which are used to find new
  // walkable areas
  if (!incremental) ///< @incremental update doesn't build ladders to avoid
  ...
  if (m_walkableSeeds.Count() == 0) {
    ...
    Msg("No valid walkable seed positions.  Cannot generate Navigation "
        "Mesh.\n");
    return;
  }
...
void CNavMesh::BeginAnalysis(bool quitWhenFinished) {
  ...
  // Remove and re-add elements in TheNavAreas, to ensure indices are useful for
  // progress feedback
  NavAreaVector tmpSet;
  {
    FOR_EACH_VEC(TheNavAreas, it) { tmpSet.AddToTail(TheNavAreas[it]); }
  }
  TheNavAreas.RemoveAll();
  {
    FOR_EACH_VEC(tmpSet, it) { TheNavAreas.AddToTail(tmpSet[it]); }
  }
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Process the auto-generation for 'maxTime' seconds. return false if generation
 * is complete.
 */
bool CNavMesh::UpdateGeneration(float maxTime) {
  double startTime = Plat_FloatTime();
  static unsigned int s_movedPlayerToArea =
      0; // Last area we moved a player to for lighting calcs
  static CountdownTimer s_playerSettleTimer; // Settle time after moving the
                                             // player for lighting calcs
  ...
  case CREATE_AREAS_FROM_SAMPLES: {
    ...
    // Remove and re-add elements in TheNavAreas, to ensure indices are useful
  ...
  case FIND_EARLIEST_OCCUPY_TIMES: {
    ...
    bool shouldSkipLightComputation =
    ...
    if (shouldSkipLightComputation) {
      m_generationState = CUSTOM; // no light intensity calcs for incremental
                                  // generation or dedicated servers
    } else {
    ...
    if (s_unlitAreas.Count()) {
      if (s_unlitSeedAreas.Count()) {
        ...
        if (GetGroundHeight(eyePos, &height)) {
          eyePos.z =
              height + HalfHumanHeight -
              StepHeight; // players light from their centers, and we light from
  ...
  case SAVE_NAV_MESH: {
    ...
    if (m_bQuitWhenFinished) {
      ...
      FOR_EACH_VEC(TheNavAreas, it) { TheNavAreas[it]->ResetNodes(); }
...
CNavNode *CNavMesh::AddNode(const Vector &destPos, const Vector &normal,
                            NavDirType dir, CNavNode *source,
                            bool isOnDisplacement, float obstacleHeight,
                            float obstacleStartDist, float obstacleEndDist) {
  ...
  // optimization: if deltaZ changes very little, assume connection is
  // commutative
...
inline CNavNode *LadderEndSearch(const Vector *pos, NavDirType mountDir) {
  ...
  //
  // Test the ladder dismount point first, then each cardinal direction one and
  // two steps away
  //
  for (int d = (-1); d < 2 * NUM_DIRECTIONS; ++d) {
    ...
    trace_t result;
    UTIL_TraceHull(center + Vector(0, 0, fudge), tryPos + Vector(0, 0, fudge),
...
void DrawTrace(const trace_t *trace) {
  ...
  {
          NDebugOverlay::SweptBox( trace->startpos, trace->endpos, NavTraceMins,
  NavTraceMaxs, vec3_angle, 0, 255, 0, 45, 100 );
  }
  else
  {
          NDebugOverlay::SweptBox( trace->startpos, trace->endpos, NavTraceMins,
  NavTraceMaxs, vec3_angle, 255, 0, 0, 45, 100 );
  }
  */
}
...
bool TraceAdjacentNode(int depth, const Vector &start, const Vector &end,
  ...
  const float MinDistance =
      1.0f; // if we can't move at least this far, don't bother stepping up.
  ...
  if (depth && start.AsVector2D().DistToSqr(trace->endpos.AsVector2D()) <
                   MinDistance * MinDistance) {
    return false;
  }

  // We made it more than MinDistance.  If the slope is too steep, we can't go
  // on.
 ...
 * The algorithm begins at the bot's current location, and does a recursive
 * search outwards, tracking all valid steps and generating a directed graph of
 * CNavNodes.
 *
  ...
  while (true) {
    if (m_currentNode == NULL) {
      ...
      if (m_currentNode == NULL) {
        ...
        for (int i = 0; i < m_ladders.Count(); ++i) {
          ...
          // check ladder bottom
          if ((m_currentNode =
                   LadderEndSearch(&ladder->m_bottom, ladder->GetDir())) != 0)
            break;

          // check ladder top
          if ((m_currentNode =
                   LadderEndSearch(&ladder->m_top, ladder->GetDir())) != 0)
            break;
        }
    ...
    for (int dir = NORTH; dir < NUM_DIRECTIONS; dir++) {
      if (!m_currentNode->HasVisited((NavDirType)dir)) {
        ...
        // sanity check to not generate across the world for incremental
        // generation
        ...
        if (TraceAdjacentNode(0, from, pos, &result)) {
          ...
          for (float height = StepHeight; height <= ClimbUpHeight;
            ...
            if (!tr.startsolid && tr.fraction == 1.0f) {
              ...
              // keep track of far up we had to go to find a path to the next
              // node
              obstacleHeight = height;
              success = true;
              break;
            } else {
              // Could not trace from node to node at this height, something is
              // in the way. Trace in the other direction to see if we hit
              // something
              Vector vecToObstacleStart = tr.endpos - start;
              Assert(vecToObstacleStart.LengthSqr() <=
                     Square(GenerationStepSize));
              if (vecToObstacleStart.LengthSqr() <=
                  Square(GenerationStepSize)) {
                UTIL_TraceHull(end, start, NavTraceMins, NavTraceMaxs,
                               GetGenerationTraceMask(), &filter, &tr);
                if (!tr.startsolid && tr.fraction < 1.0) {
                  // We hit something going the other direction.  There is some
                  // obstacle between the two nodes.
                  Vector vecToObstacleEnd = tr.endpos - start;
                  Assert(vecToObstacleEnd.LengthSqr() <=
                         Square(GenerationStepSize));
                  if (vecToObstacleEnd.LengthSqr() <=
                      Square(GenerationStepSize)) {
                    // Remember the distances to start and end of the obstacle
                    // (with respect to the "from" node). Keep track of the last
                    // distances to obstacle as we keep increasing the height we
                    // do a trace for. If we do eventually clear the obstacle,
                    // these values will be the start and end distance to the
        ...
        if (nav_displacement_test.GetInt() > 0) {
          ...
          // This happens during development, and is a pain because the space
          // underneath a displacement is not 'solid'.
          ...
          if (result.fraction > 0) {
            ...
            if (result.fraction < 1) {
              // if we made it down to within StepHeight, maybe we're on a
              // static prop
              if (result.endpos.z > to.z + StepHeight) {
                return true;
              }
        ...
        // If there's an obstacle in the way and it's traversable, or the
        // obstacle is not higher than the destination node itself minus a small
        // epsilon (meaning the obstacle was just the height change to get to
        // the destination node, no extra obstacle between the two), clear
        // obstacle height and distances
        if ((obstacleHeight < MaxTraversableHeight) ||
        ...
        AddNode(to, toNormal, m_generationDir, m_currentNode, isOnDisplacement,
                obstacleHeight, obstacleStartDist, obstacleEndDist);

        return true;
      }
    }

    // all directions have been searched from this node - pop back to its parent
    // and continue
...
class Subdivider {
public:
  Subdivider(int depth) { m_depth = depth; }
...
CON_COMMAND_F(nav_subdivide, "Subdivides all selected areas.",
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Debugging code to verify that all nav area connections are internally
 * consistent
 */
    ...
    for (NavDirType dir = NORTH; dir < NUM_DIRECTIONS;
      ...
      for (int iConnect = 0; iConnect < pOutgoing->Count(); iConnect++) {
        ...
        if (pIncoming->Find(connect) != pIncoming->InvalidIndex()) {
          Msg("Area %d has area %d on both 2-way and incoming list, should "
              "only be on one\n",
        ...
        for (int iConnectCheck = iConnect + 1;
          ...
          if (areaOther == areaCheck) {
            Msg("Area %d has multiple outgoing connections to area %d in "
                "direction %d\n",
        ...
        const NavConnectVector *pIncomingOther =
            areaOther->GetIncomingConnections(OppositeDirection(dir));

        // if we have a one-way outgoing connection, make sure we are on the
        // other area's incoming list
        connect.area = area;
        bool bIsTwoWay =
            pOutgoingOther->Find(connect) != pOutgoingOther->InvalidIndex();
        if (!bIsTwoWay) {
          connect.area = area;
          bool bOnOthersIncomingList =
              pIncomingOther->Find(connect) != pIncomingOther->InvalidIndex();
          if (!bOnOthersIncomingList) {
            Msg("Area %d has one-way connect to area %d but does not appear on "
                "the latter's incoming list\n",
                area->GetID(), areaOther->GetID());
          }
        }
      }
      ...
      for (int iConnect = 0; iConnect < pIncoming->Count(); iConnect++) {
        ...
        for (int iConnectCheck = iConnect + 1;
          ...
          if (areaOther == areaCheck) {
            Msg("Area %d has multiple incoming connections to area %d in "
                "direction %d\n",
                area->GetID(), areaOther->GetID(), dir);
            Assert(false);
          }
        ...
        const NavConnectVector *pOutgoingOther =
            areaOther->GetAdjacentAreas(OppositeDirection(dir));
        connect.area = area;
        bool bOnOthersOutgoingList =
            pOutgoingOther->Find(connect) != pOutgoingOther->InvalidIndex();
        if (!bOnOthersOutgoingList) {
          Msg("Area %d has incoming connection from area %d but does not "
              "appear on latter's outgoing connection list\n",
              area->GetID(), areaOther->GetID());
          Assert(false);
        }
      }
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Temp way to mark cliff areas after generation without regen'ing.  Any area
 * that is adjacent to a cliff gets marked as a cliff.  This will leave some big
 * areas marked as cliff just because one edge is adjacent to a cliff so it's
 * not great.  The code that does this at generation time is better because it
 * ensures that areas next to cliffs don't get merged with no-cliff areas.
 */
...
CON_COMMAND_F(nav_gen_cliffs_approx,
              "Mark cliff areas, post-processing approximation", FCVAR_CHEAT) {
