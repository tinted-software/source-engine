...
//-----------------------------------------------------------------------------
AI_Waypoint_t *
CAI_Pathfinder::BuildSimpleRoute(Navigation_t navType, const Vector &vStart,
                                 const Vector &vEnd, const CBaseEntity *pTarget,
                                 int endFlags, int nodeID, int nodeTargetType,
                                 float flYaw) {
...
//			and vEnd, ignoring entity pTarget the the given
//tolerance and 			triangulating around a blocking object at blockDist
// Input  :
// Output : Returns a route if sucessful or NULL if no local route was possible
//-----------------------------------------------------------------------------
...
//			Currently this function is meant for NPCs moving around
//a vehicle, 			and is very specialized as such
//
...
public:
  CPathfindNearestNodeFilter(CAI_Pathfinder *pPathfinder, const Vector &vGoal,
      ...
      : m_pPathfinder(pPathfinder), m_nTries(0), m_vGoal(vGoal),
        m_bToNode(bToNode), m_goalTolerance(goalTolerance),
