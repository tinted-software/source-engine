...
DEFINE_FIELD(m_iCurrentMaxRagdollCount, FIELD_INTEGER),
    ...
    DEFINE_INPUTFUNC(FIELD_INTEGER, "SetMaxRagdollCount",
                     InputSetMaxRagdollCount),
    DEFINE_INPUTFUNC(FIELD_INTEGER, "SetMaxRagdollCountDX8",
...
#ifdef HL2_DLL
  CRagdollManager *pEnt = (CRagdollManager *)gEntList.FindEntityByClassname(
      NULL, "game_ragdoll_manager");
...
int CRagdollManager::DrawDebugTextOverlays(void) {
  ...
  if (m_debugOverlays & OVERLAY_TEXT_BIT) {
    ...
    Q_snprintf(tempstr, sizeof(tempstr), "max ragdoll count: %d",
               m_iCurrentMaxRagdollCount.Get());
