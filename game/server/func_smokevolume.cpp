...
DEFINE_KEYFIELD(m_Color1, FIELD_COLOR32, "Color1"),
    ...
    DEFINE_KEYFIELD(m_ParticleSpacingDistance, FIELD_FLOAT,
    ...
    END_DATADESC()
        ...
        IMPLEMENT_SERVERCLASS_ST(CFuncSmokeVolume, DT_FuncSmokeVolume)
            SendPropInt(SENDINFO(m_Color1), 32, SPROP_UNSIGNED,
    ...
    SendPropInt(SENDINFO(m_spawnflags), 8, SPROP_UNSIGNED) END_SEND_TABLE()

        LINK_ENTITY_TO_CLASS(func_smokevolume, CFuncSmokeVolume);

CFuncSmokeVolume::CFuncSmokeVolume() { m_Density = 1.0f; }

void CFuncSmokeVolume::SetDensity(float density) { m_Density = density; }
