...
ConVar
    ai_debug_eventresponses("ai_debug_eventresponses", "0", FCVAR_NONE,
                            "Set to 1 to see all NPC response events trigger, "
                            "and which NPCs choose to respond to them.");
...
void CNPCEventResponseSystem::TriggerEvent(const char *pResponse, bool bForce,
  ...
  if (iIndex == m_ActiveEvents.InvalidIndex()) {
    ...
    if (ai_debug_eventresponses.GetBool()) {
      Msg("NPCEVENTRESPONSE: (%.2f) Trigger resetting already-active event "
          "firing named: %s\n",
          gpGlobals->curtime, pResponse);
    }
  }
...
void CNPCEventResponseSystem::FrameUpdatePreEntityThink() {
  ...
  // Move through all events, removing expired ones and finding NPCs for active
  // ones.
  for (int i = m_ActiveEvents.First(); i != m_ActiveEvents.InvalidIndex();) {
    ...
    if (!m_ActiveEvents[i].bPreventExpiration &&
        (flTime + NPCEVENTRESPONSE_GIVEUP_TIME) < gpGlobals->curtime) {
      if (ai_debug_eventresponses.GetBool()) {
      ...
      // If we've fired once, and our current event should expire now, then
      // expire.
      if (m_ActiveEvents[i].bPreventExpiration &&
          (flTime + NPCEVENTRESPONSE_GIVEUP_TIME) < gpGlobals->curtime) {
        if (ai_debug_eventresponses.GetBool()) {
          Msg("NPCEVENTRESPONSE: (%.2f) Removing expired fired event named: "
              "%s\n",
              gpGlobals->curtime, pResponse);
        }
        ...
        for (int j = 0; j < g_AI_Manager.NumAIs(); j++) {
          if (ppAIs[j]->CanRespondToEvent(pResponse)) {
            float flDistToPlayer =
                (vecPlayerCenter - ppAIs[j]->WorldSpaceCenter()).LengthSqr();
        ...
        if (pNearestNPC) {
          if (pNearestNPC->RespondedTo(pResponse, m_ActiveEvents[i].bForce,
                                       m_ActiveEvents[i].bCancelScript)) {
            // Don't remove the response yet. Leave it around until the refire
            // time has expired. This stops repeated firings of the same concept
            // from spamming the NPCs.
            m_ActiveEvents[i].bPreventExpiration = true;
            m_ActiveEvents[i].flNextResponseTime =
                gpGlobals->curtime + NPCEVENTRESPONSE_REFIRE_TIME;

            if (ai_debug_eventresponses.GetBool()) {
              Msg("NPCEVENTRESPONSE: (%.2f) Event '%s' responded to by NPC "
                  "'%s'. Refire available at: %.2f\n",
...
DEFINE_INPUTFUNC(FIELD_STRING, "TriggerResponseEvent",
                 InputTriggerResponseEvent),
    DEFINE_INPUTFUNC(FIELD_STRING, "ForceTriggerResponseEvent",
                     InputForceTriggerResponseEvent),
    DEFINE_INPUTFUNC(FIELD_STRING, "ForceTriggerResponseEventNoCancel",
...
// Purpose:
//-----------------------------------------------------------------------------
void CNPCEventResponseSystemEntity::InputTriggerResponseEvent(
...
// Purpose:
//-----------------------------------------------------------------------------
void CNPCEventResponseSystemEntity::InputForceTriggerResponseEvent(
    inputdata_t &inputdata) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CNPCEventResponseSystemEntity::InputForceTriggerResponseEventNoCancel(
    inputdata_t &inputdata) {
