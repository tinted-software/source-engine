...
DEFINE_FIELD(m_controlType, FIELD_INTEGER),
    DEFINE_FIELD(m_linear, FIELD_VECTOR), DEFINE_FIELD(m_angular, FIELD_VECTOR),
    DEFINE_FIELD(m_linearSave, FIELD_VECTOR),
    DEFINE_FIELD(m_angularSave, FIELD_VECTOR),
    END_DATADESC()

        void CConstantForceController::SetConstantForce(
            const Vector &linear, const AngularImpulse &angular) {
  m_linear = linear;
  m_angular = angular;
...
CConstantForceController::Simulate(IPhysicsMotionController *pController,
                                   IPhysicsObject *pObject, float deltaTime,
                                   Vector &linear, AngularImpulse &angular) {
  linear = m_linear;
  angular = m_angular;
...
//			Calculate the force (and optional torque) that the
//engine would create
//-----------------------------------------------------------------------------
...
void CPhysMotor::Spawn(void) {
  ...
  if (axisLength > 1.0f) {
    ...
    Warning("phys_motor %s does not have a valid axis helper, and "
            "self-destructed!\n",
            GetDebugName());
...
IMotionEvent::simresult_e
CKeepUpright::Simulate(IPhysicsMotionController *pController,
