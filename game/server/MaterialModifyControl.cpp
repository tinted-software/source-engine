...
DEFINE_AUTO_ARRAY(m_szMaterialName, FIELD_CHARACTER),
    ...
    DEFINE_INPUTFUNC(FIELD_VOID, "SetMaterialVarToCurrentTime",
    ...
    END_DATADESC()
        ...
        IMPLEMENT_SERVERCLASS_ST(CMaterialModifyControl,
...
// Purpose :
//------------------------------------------------------------------------------
bool CMaterialModifyControl::KeyValue(const char *szKeyName,
                                      const char *szValue) {
  if (FStrEq(szKeyName, "materialName")) {
    Q_strncpy(m_szMaterialName.GetForModify(), szValue,
              MATERIAL_MODIFY_STRING_SIZE);
    return true;
  }

  if (FStrEq(szKeyName, "materialVar")) {
    Q_strncpy(m_szMaterialVar.GetForModify(), szValue,
              MATERIAL_MODIFY_STRING_SIZE);
    return true;
  }
...
void CMaterialModifyControl::SetMaterialVar(inputdata_t &inputdata) {
  // if( debug_materialmodifycontrol.GetBool() && Q_stristr( GetDebugName(),
  // "alyx" ) )
  //{
  // DevMsg( 1, "CMaterialModifyControl::SetMaterialVar %s %s %s=\"%s\"\n",
  // GetDebugName(), m_szMaterialName.Get(), m_szMaterialVar.Get(),
  // inputdata.value.String() );
  //}
  Q_strncpy(m_szMaterialVarValue.GetForModify(), inputdata.value.String(),
            MATERIAL_MODIFY_STRING_SIZE);
...
// Purpose:
//-----------------------------------------------------------------------------
void CMaterialModifyControl::SetMaterialVarToCurrentTime(
...
void CMaterialModifyControl::InputStartAnimSequence(inputdata_t &inputdata) {
  ...
  Warning("%s (%s) received StartAnimSequence input without correct "
          "parameters. Syntax: <Frame Start> <Frame End> <Frame Rate> "
          "<Loop>\nSetting <Frame End> to -1 uses the last frame of the "
          "texture. <Loop> should be 1 or 0.\n",
...
void CMaterialModifyControl::InputStartFloatLerp(inputdata_t &inputdata) {
  ...
  //	if( debug_materialmodifycontrol.GetBool() )//&& Q_stristr(
  //GetDebugName(), "alyx" ) )
  //	{
  //		DevMsg( 1, "CMaterialModifyControl::InputStartFloatLerp %s %s %s
  //\"%s\"\n", 			GetDebugName(), m_szMaterialName.Get(), m_szMaterialVar.Get(),
  //inputdata.value.String() );
  //	}
  ...
  Warning("%s (%s) received StartFloatLerp input without correct parameters. "
          "Syntax: <Start Value> <End Value> <Transition Time> <Loop>\n<Loop> "
          "should be 1 or 0.\n",
          GetClassname(), GetDebugName());
}
