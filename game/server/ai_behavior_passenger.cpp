...
ConVar
    passenger_impact_response_threshold("passenger_impact_response_threshold",
                                        "-350.0");
...
CAI_PassengerBehavior::CAI_PassengerBehavior(void)
    : m_bEnabled(false), m_hVehicle(NULL),
      m_PassengerState(PASSENGER_STATE_OUTSIDE),
      m_PassengerIntent(PASSENGER_INTENT_NONE), m_nTransitionSequence(-1) {}
...
//angles.  This 			should be consolidated to not replicate so much code! -- jdw
//-----------------------------------------------------------------------------
...
void CAI_PassengerBehavior::PrescheduleThink(void) {
  ...
  if (GetPassengerState() == PASSENGER_STATE_INSIDE) {
    ...
    if (m_hVehicle &&
      ...
      Warning("Passenger is in vehicle without a valid seat position! -- "
              "EJECTED\n");
      GetOuter()->SetParent(NULL);
      Disable();
