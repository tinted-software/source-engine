...
DEFINE_FIELD(bClampEyeAngles, FIELD_BOOLEAN),
    ...
    DEFINE_FIELD(flFOV, FIELD_FLOAT), DEFINE_FIELD(flYawMin, FIELD_FLOAT),
    DEFINE_FIELD(flYawMax, FIELD_FLOAT), DEFINE_FIELD(flPitchMin, FIELD_FLOAT),
...
public:
  ...
  // Tuned to match HL2s definition, but this should probably return false in
  // all cases
...
public:
  ...
  bool m_bForcePlayerEyePoint; // Uses player's eyepoint instead of
                               // 'vehicle_driver_eyes' attachment
...
DEFINE_INPUTFUNC(FIELD_VOID, "Lock", InputLock),
    ...
    DEFINE_INPUTFUNC(FIELD_VOID, "EnterVehicleImmediate",
    ...
    DEFINE_OUTPUT(m_playerOff, "PlayerOff"), DEFINE_OUTPUT(m_OnOpen, "OnOpen"),
    ...
    END_DATADESC()
        ...
        IMPLEMENT_SERVERCLASS_ST(CPropVehicleChoreoGeneric,
...
bool ShouldVehicleIgnoreEntity(CBaseEntity *pVehicle, CBaseEntity *pCollide) {
  ...
  CPropVehicleChoreoGeneric *pChoreoVehicle =
      dynamic_cast<CPropVehicleChoreoGeneric *>(pVehicle);
...
// Purpose:
//-----------------------------------------------------------------------------
void CPropVehicleChoreoGeneric::TraceAttack(const CTakeDamageInfo &info,
                                            const Vector &vecDir,
                                            trace_t *ptr) {
...
// Purpose:
//-----------------------------------------------------------------------------
Vector CPropVehicleChoreoGeneric::BodyTarget(const Vector &posSrc,
                                             bool bNoisy) {
  Vector shotPos;
...
void CPropVehicleChoreoGeneric::InputViewlock(inputdata_t &inputdata) {
  if (inputdata.value.Bool()) // lock
  {
    if (m_savedVehicleView.flFOV == 0) // not already locked
    {
      ...
      m_vehicleView.flYawMax = m_vehicleView.flYawMin =
          m_vehicleView.flPitchMin = m_vehicleView.flPitchMax = 0.0f;
    }
  } else {                            // unlock
    Assert(m_savedVehicleView.flFOV); // is nonzero if something is saved, is
                                      // zero if nothing was saved.
    if (m_savedVehicleView.flFOV) {
      ...
      /* // note: the straight assignments, as in the lower two lines below, do
      not call the = overload and thus are never transmitted!
...
// Purpose:
//-----------------------------------------------------------------------------
void CPropVehicleChoreoGeneric::Use(CBaseEntity *pActivator,
...
bool CPropVehicleChoreoGeneric::CanExitVehicle(CBaseEntity *pEntity) {
  // Prevent exiting if the vehicle's locked, rotating, or playing an entry/exit
  // anim.
...
//-----------------------------------------------------------------------------
void CPropVehicleChoreoGeneric::GetVectors(Vector *pForward, Vector *pRight,
                                           Vector *pUp) const {
...
// Purpose:
//-----------------------------------------------------------------------------
CBaseEntity *CPropVehicleChoreoGeneric::GetDriver(void) { return m_hPlayer; }
...
void CPropVehicleChoreoGeneric::InputEnterVehicle(inputdata_t &inputdata) {
  ...
  // FIXME: I hate code like this. I should really add a parameter to
...
// Input  : &inputdata -
//-----------------------------------------------------------------------------
void CPropVehicleChoreoGeneric::InputEnterVehicleImmediate(
...
// Purpose:
//-----------------------------------------------------------------------------
void CChoreoGenericServerVehicle::GetVehicleViewPosition(
    int nRole, Vector *pAbsOrigin, QAngle *pAbsAngles, float *pFOV /*= NULL*/) {
  ...
  *pAbsAngles = pPlayer->EyeAngles(); // yuck. this is an in/out parameter.
  ...
  GetVehicle()->GetAttachment("vehicle_driver_eyes", vehicleEyeOrigin,
                              vehicleEyeAngles);
...
bool CPropVehicleChoreoGeneric::ShouldCollide(int collisionGroup,
...
// Purpose:
//-----------------------------------------------------------------------------
void CVehicleChoreoViewParser::ParseKeyValue(void *pData, const char *pKey,
                                             const char *pValue) {
