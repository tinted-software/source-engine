...
enum MicrophoneResult_t {
  ...
  MicrophoneResult_Swallow, // The microphone swallowed the sound. Don't play
                            // it.
  MicrophoneResult_Remove,  // The microphone should be removed from the list of
                            // microphones.
};
...
public:
  ...
  static bool OnSoundPlayed(int entindex, const char *soundname,
                            soundlevel_t soundlevel, float flVolume, int iFlags,
                            int iPitch, const Vector *pOrigin, float soundtime,
...
private:
  ...
  float
      m_flSmoothFactor; // 0 = no smoothing of samples, 0.9 = maximum smoothing
  float m_flMaxRange; // Maximum sound hearing range, irrelevant of attenuation
  string_t
      m_iszSpeakerName; // Name of a speaker to output any heard sounds through
  EHANDLE m_hSpeaker;   // Speaker to output any heard sounds through
  bool m_bAvoidFeedback;
  int m_iSpeakerDSPPreset; // Speaker DSP preset to use when this microphone is
                           // enabled
  ...
  COutputEvent
      m_OnRoutedSound; // Fired when a sound has been played through our speaker
