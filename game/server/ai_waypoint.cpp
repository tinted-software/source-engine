...
DEFINE_FIXEDSIZE_ALLOCATOR(AI_Waypoint_t, WAYPOINT_POOL_SIZE,
                           CUtlMemoryPool::GROW_FAST);
...
DEFINE_FIELD(vecLocation, FIELD_POSITION_VECTOR),
    ...
    //				iNodeID			(not saved, cannot rely
    //on consistent mapping) 				flPathDistGoal	(not saved )
...
AI_Waypoint_t::AI_Waypoint_t(const Vector &initPosition, float initYaw,
                             Navigation_t initNavType, int initWaypointFlags,
                             int initNodeID) {
  memset(this, 0, sizeof(*this));
...
void CAI_WaypointList::PrependWaypoint(const Vector &newPoint,
                                       Navigation_t navType,
                                       unsigned waypointFlags, float flYaw) {
  PrependWaypoints(
      new AI_Waypoint_t(newPoint, flYaw, navType, waypointFlags, NO_NODE));
}
...
#ifdef _GOALDEBUG
      if (!waypoint->GetNext() &&
          !(waypoint->Flags() & (bits_WP_TO_GOAL | bits_WP_TO_PATHCORNER))) {
...
void AddWaypointLists(AI_Waypoint_t *oldRoute, AI_Waypoint_t *addRoute) {
  ...
  if (waypoint->iNodeID != NO_NODE && waypoint->iNodeID == addRoute->iNodeID) {
    //		waypoint->iWPType = addRoute->iWPType; <<TODO>> found case where
    //this was bad
