...
void CPhysicsHook::LevelInitPreEntity() {
  ...
  physenv->EnableConstraintNotify(true); // callback when an object gets deleted
                                         // that is attached to a constraint

  physenv->SetObjectEventHandler(&g_Collisions);

  physenv->SetSimulationTimestep(
      gpGlobals->interval_per_tick); // 15 ms per tick
...
penetrateevent_t &
CCollisionEvent::FindOrAddPenetrateEvent(CBaseEntity *pEntity0,
  ...
  if (phys_dontprintint.GetBool() == false) {
    ...
    if (pEntity0 == pEntity1) {
      ...
      DevMsg(1, "***Inter-penetration on %s (%d & %d) (%.0f, %.0f)\n",
             pName1 ? pName1 : "(null)", index0, index1, gpGlobals->curtime,
             eventTime);
    } else {
      DevMsg(1, "***Inter-penetration between %s(%s) AND %s(%s) (%.0f, %.0f)\n",
             pName1 ? pName1 : "(null)", pEntity0->GetDebugName(),
             pName2 ? pName2 : "(null)", pEntity1->GetDebugName(),
...
CON_COMMAND(physics_budget, "Times the cost of each active object") {
  ...
  if (activeCount) {
    ...
    //with sets of objects/constraints/etc 	so these are only generally useful
    //for broad questions, not real metrics!
  ...
  while (1) {
    ...
    if (pEntity->VPhysicsGetObject()) {
      if (pBestObject) {
        ...
        DevWarning("entity %s at %s has physics attachment to more than one "
                   "entity with the name %s!!!\n",
...
static ConCommand
    air_density("air_density", CC_AirDensity,
