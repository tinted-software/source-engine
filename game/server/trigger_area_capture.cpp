...
void CTriggerAreaCapture::StartTouch(CBaseEntity *pOther) {
  ...
  if (PassesTriggerFilters(pOther) && m_hPoint) {
    ...
    IGameEvent *event =
        gameeventmanager->CreateEvent("controlpoint_starttouch");
    if (event) {
    ...
    // Call capture think immediately to make it update our area's player
    // counts. If we don't do this, the player can receive the above event
    // telling him he's in a zone, but the objective resource still thinks he's
    // not.
    m_bStartTouch = true;
    CaptureThink();
    m_bStartTouch = false;
...
ConVar mp_simulatemultiplecappers("mp_simulatemultiplecappers", "1",
                                  FCVAR_CHEAT);
...
void CTriggerAreaCapture::CaptureThink(void) {
  ...
  int iNumBlockablePlayers[MAX_CAPTURE_TEAMS]; // Players in the zone who can't
                                               // cap, but can block / pause
                                               // caps
    ...
    for (int i = 0; i < m_hTouchingEntities.Count(); i++) {
      ...
      if (ent && ent->IsPlayer()) {
        ...
        if (pPlayer->IsAlive()) {
          ...
          // If a team's not allowed to cap a point, don't count players in it
          // at all
          if (!TeamplayGameRules()->TeamMayCapturePoint(
                  iTeam, m_hPoint->GetPointIndex()))
            continue;

          if (!TeamplayGameRules()->PlayerMayCapturePoint(
                  pPlayer, m_hPoint->GetPointIndex())) {
            if (TeamplayGameRules()->PlayerMayBlockPoint(
                    pPlayer, m_hPoint->GetPointIndex())) {
              if (iNumPlayers[iTeam] == 0 && iNumBlockablePlayers[iTeam] == 0) {
                pFirstPlayerTouching[iTeam] = pPlayer;
              }

              iNumBlockablePlayers[iTeam] +=
                  TeamplayGameRules()->GetCaptureValueForPlayer(pPlayer);
            }
            continue;
          }
  ...
  if (iTeamsInZone > 1) {
    ...
    // If we've got non-cappable, yet blockable players here for the team that's
    // defending, they need to block the cap. This catches cases like the TF
    // invulnerability, which needs to block caps, but isn't allowed to
    // contribute to a cap.
    for (int i = FIRST_GAME_TEAM; i < GetNumberOfTeams(); i++) {
  ...
  if (m_bCapturing) {
    ...
    if (iTeamsInZone > 1) {
      ...
      if (CaptureModeScalesWithPlayers()) {
        flPercentToGo =
            m_fTimeRemaining / ((m_flCapTime * 2) *
                                m_TeamData[m_nCapturingTeam].iNumRequiredToCap);
      }

      if ((flPercentToGo <= 0.5 ||
        ...
        if (pBlockingPlayer) {
          ...
          for (int i = m_Blockers.Count() - 1; i >= 0; i--) {
            ...
            // If this guy's was a blocker, but not valid now, remove him from
            // the list
            if (m_Blockers[i].iCapAttemptNumber != m_iCapAttemptNumber ||
                !IsTouching(m_Blockers[i].hPlayer) ||
                (TeamplayGameRules()->PointsMayAlwaysBeBlocked() &&
                 m_Blockers[i].flNextBlockTime < gpGlobals->curtime &&
                 m_bStartTouch)) {
              m_Blockers.Remove(i);
              continue;
            }
    ...
    if (CaptureModeScalesWithPlayers()) {
      flTotalTimeToCap =
          ((m_flCapTime * 2) * m_TeamData[m_nCapturingTeam].iNumRequiredToCap);
    }

    // Now remove the reduction amount after we've determined there's only 1
    // team in the area
    if (m_nCapturingTeam == m_nTeamInZone) {
      SetCapTimeRemaining(m_fTimeRemaining - flReduction);
    } else if (m_nOwningTeam == TEAM_UNASSIGNED &&
               m_nTeamInZone != TEAM_UNASSIGNED) {
      SetCapTimeRemaining(m_fTimeRemaining + flReduction);
    } else {
      // Caps deteriorate over time
      if (TeamplayRoundBasedRules() && m_hPoint &&
          TeamplayRoundBasedRules()->TeamMayCapturePoint(
              m_nCapturingTeam, m_hPoint->GetPointIndex())) {
        float flDecreaseScale = CaptureModeScalesWithPlayers()
                                    ? mp_capdeteriorate_time.GetFloat()
                                    : flTotalTimeToCap;
    ...
    int iTeamMembersHere = m_TeamData[m_nCapturingTeam].iNumTouching +
    iNumBlockablePlayers[m_nCapturingTeam]; if ( (iTeamMembersHere == 0 ) ||
    (mp_capstyle.GetInt() == 0 && iTeamMembersHere <
    m_TeamData[m_nCapturingTeam].iNumRequiredToCap) )
    {
            BreakCapture( true );
            return;
    ...
    */
    ...
    if (m_fTimeRemaining <= 0) {
      ...
      // We may get several simultaneous CaptureThink calls from StartTouch if
      // there are several players on the trigger when it is enabled (like in
      // Raid mode). We haven't started reducing m_fTimeRemaining yet but the
      // second call to CaptureThink from StartTouch has m_bCapturing set to
      // true and we hit this condition and call BreakCapture right away. We put
      // this check here to prevent calling BreakCapture from the StartTouch
      // call to CaptureThink. If the capture should really be broken it will
      // happen the next time the trigger thinks on its own.
      if (!m_bStartTouch) {
    ...
    // If there are any teams in the zone that aren't the owner, try to start
    // capping
    if (iTeamsInZone > 0) {
      for (int i = FIRST_GAME_TEAM; i < GetNumberOfTeams(); i++) {
        ...
        if (!CaptureModeScalesWithPlayers() &&
            m_TeamData[i].iNumTouching < m_TeamData[i].iNumRequiredToCap)
          continue;
...
void CTriggerAreaCapture::SetCapTimeRemaining(float flTime) {
  ...
  ObjectiveResource()->SetCPCapPercentage(m_hPoint->GetPointIndex(),
                                          flCapPercentage);
...
// Purpose:
//-----------------------------------------------------------------------------
void CTriggerAreaCapture::HandleRespawnTimeAdjustments(int oldTeam,
                                                       int newTeam) {
  if (oldTeam > LAST_SHARED_TEAM) {
    // reverse the adjust made when the old team captured this point (if we made
    // one)
    if (m_TeamData[oldTeam].iSpawnAdjust != 0) {
      TeamplayRoundBasedRules()->AddTeamRespawnWaveTime(
          oldTeam, -m_TeamData[oldTeam].iSpawnAdjust);
    }
  }
  ...
  if (newTeam > LAST_SHARED_TEAM) {
    if (m_TeamData[newTeam].iSpawnAdjust != 0) {
      TeamplayRoundBasedRules()->AddTeamRespawnWaveTime(
          newTeam, m_TeamData[newTeam].iSpawnAdjust);
    }
  }
}
...
void CTriggerAreaCapture::StartCapture(int team, int capmode) {
  ...
  if (CaptureModeScalesWithPlayers()) {
    SetCapTimeRemaining(
        ((m_flCapTime * 2) * m_TeamData[team].iNumRequiredToCap));
  } else {
      ...
      if (pTeam) {
        for (int i = 0; i < pTeam->GetNumPlayers(); i++) {
          CBaseMultiplayerPlayer *pPlayer =
              ToBaseMultiplayerPlayer(pTeam->GetPlayer(i));
...
// Purpose:
//-----------------------------------------------------------------------------
void CTriggerAreaCapture::GetNumCappingPlayers(int team, int &numcappers,
                                               int *cappingplayers) {
  numcappers = 0;
    ...
    if (ent) {
      ...
      if (IsTouching(player) &&
          (player->GetTeamNumber() ==
           team)) // need to make sure disguised spies aren't included in the
                  // list of capping players
      {
...
void CTriggerAreaCapture::EndCapture(int team) {
  ...
  // he hasn't gotten points yet, and his name will go in the cap string if its
  // needed first capper gets name sent and points given by flag. other cappers
  // get points manually above, no name in message
...
void CTriggerAreaCapture::BreakCapture(bool bNotEnoughPlayers) {
  if (m_bCapturing) {
    ...
    if (m_hPoint) {
      ...
      IGameEvent *event =
          gameeventmanager->CreateEvent("teamplay_capture_broken");
      if (event) {
...
void CTriggerAreaCapture::InputRoundSpawn(inputdata_t &inputdata) {
  ...
  if (!m_hPoint) {
    m_hPoint = dynamic_cast<CTeamControlPoint *>(
        gEntList.FindEntityByName(NULL, STRING(m_iszCapPointName)));
    ...
    if (m_hPoint) {
      ...
      for (int i = FIRST_GAME_TEAM; i < GetNumberOfTeams(); i++) {
        ...
        ObjectiveResource()->SetCPRequiredCappers(
            m_hPoint->GetPointIndex(), i, m_TeamData[i].iNumRequiredToCap);
        ObjectiveResource()->SetTeamCanCap(m_hPoint->GetPointIndex(), i,
                                           m_TeamData[i].bCanCap);

        if (CaptureModeScalesWithPlayers()) {
          ObjectiveResource()->SetCPCapTime(
              m_hPoint->GetPointIndex(), i,
        ...
        ObjectiveResource()->SetCPCapTimeScalesWithPlayers(
            m_hPoint->GetPointIndex(), CaptureModeScalesWithPlayers());
      }
    }
  }
}
...
void CTriggerAreaCapture::InputSetTeamCanCap(inputdata_t &inputdata) {
  ...
  if (pszParam && pszParam[0]) {
    ...
    if (pszParam && pszParam[0]) {
      ...
      if (iTeam >= 0 && iTeam < GetNumberOfTeams()) {
        ...
        if (m_hPoint) {
          ObjectiveResource()->SetTeamCanCap(m_hPoint->GetPointIndex(), iTeam,
                                             m_TeamData[iTeam].bCanCap);
        }
        return;
      }
    }
  ...
  Warning("%s(%s) received SetTeamCanCap input with invalid format. Format "
          "should be: <team number> <can cap (0/1)>.\n",
...
void CTriggerAreaCapture::InputSetControlPoint(inputdata_t &inputdata) {
  BreakCapture(false); // clear the capping for the previous point, forces us to
                       // recalc on the new one
...
// return TRUE otherwise ( eg player is in area, but his death does not cause
// break )
//-----------------------------------------------------------------------------
bool CTriggerAreaCapture::CheckIfDeathCausesBlock(
    CBaseMultiplayerPlayer *pVictim, CBaseMultiplayerPlayer *pKiller) {
  if (!pVictim || !pKiller)
    return false;
  ...
  if (CaptureModeScalesWithPlayers()) {
    ...
    bBreakCap = (m_TeamData[m_nCapturingTeam].iBlockedTouching - 1 <
                 m_TeamData[m_nCapturingTeam].iNumRequiredToCap);
  }
...
void CTriggerAreaCapture::UpdateOwningTeam(void) {
  if (m_hPoint) {
    ObjectiveResource()->SetOwningTeam(m_hPoint->GetPointIndex(),
                                       m_nOwningTeam);
  }
}
...
void CTriggerAreaCapture::UpdateTeamInZone(void) {
  if (m_hPoint) {
    ObjectiveResource()->SetTeamInZone(m_hPoint->GetPointIndex(),
                                       m_nTeamInZone);
  }
}
...
// Purpose:
//-----------------------------------------------------------------------------
void CTriggerAreaCapture::SetNumCappers(int nNumCappers,
                                        bool bBlocked /* = false */) {
  m_OnNumCappersChanged.Set(nNumCappers, this, this);

  // m_OnNumCappersChanged2 sets -1 for a blocked cart (for movement decisions
  // on hills)
  if (bBlocked) {
    nNumCappers = -1;
  }
