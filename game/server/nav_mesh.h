...
public:
  ...
  IndexType
  GetIndex(Place place) const; /// return the directory index corresponding to
                               /// this Place (0 = no entry)
  void
  AddPlace(Place place); /// add the place to the directory if not already known
  Place
  IndexToPlace(IndexType entry) const; /// given an index, return the Place
...
public:
  ...
  virtual void
  Reset(void); // destroy Navigation Mesh data and revert to initial state
  ...
  virtual NavErrorType
  PostLoad(unsigned int version); // (EXTEND) invoked after all areas have been
  ...
  const CUtlVector<Place> *GetPlacesFromNavFile(
      bool *hasUnnamedPlaces); // Reads the used place names from the nav file
                               // (can be used to selectively precache before
                               // the nav is loaded)
  ...
  GetSubVersionNumber(void) const; // returns sub-version number of data format
  ...
  } // store custom mesh data for derived classes that needs to be loaded before
    // areas are read in
  virtual void LoadCustomDataPreArea(CUtlBuffer &fileBuffer,
                                     unsigned int subVersion) {
  } // load custom mesh data for derived classes that needs to be loaded before
    // areas are read in

  // events
  virtual void
  OnServerActivate(void); // (EXTEND) invoked when server loads a new map
  virtual void OnRoundRestart(void); // invoked when a game round restarts
  virtual void
  OnRoundRestartPreEntity(void); // invoked when a game round restarts, but
                                 // before entities are deleted and recreated
  ...
  virtual void
  OnAreaBlocked(CNavArea *area); // invoked when the area becomes blocked
  virtual void
  OnAreaUnblocked(CNavArea *area); // invoked when the area becomes un-blocked
  ...
  virtual void
  OnEditCreateNotify(CNavArea *newArea); // invoked when given area has just
                                         // been added to the mesh in edit mode
  virtual void OnEditDestroyNotify(
      CNavArea *deadArea); // invoked when given area has just been deleted from
  ...
  CNavArea *GetNavArea(const Vector &pos, float beneathLimt = 120.0f)
      const; // given a position, return the nav area that IsOverlapping and is
  ...
  Place
  NameToPlace(const char *name) const; // given a place name, return a place ID
                                       // or zero if no place is defined
  Place PartialNameToPlace(const char *name)
      const; // given the first part of a place name, return a place ID or zero
  ...
  int PlaceNameAutocomplete(
      ...
      char commands[COMMAND_COMPLETION_MAXITEMS]
                   [COMMAND_COMPLETION_ITEM_LENGTH]); // Given a partial place
                                                      // name, fill in possible
                                                      // place names for
                                                      // ConCommand autocomplete

  bool GetGroundHeight(const Vector &pos, float *height, Vector *normal = NULL)
      const; // get the Z coordinate of the topmost ground level below the given
             // point
  bool GetSimpleGroundHeight(
      const Vector &pos, float *height,
      Vector *normal = NULL) const; // get the Z coordinate of the ground level
  ...
  void
  DrawPlayerCounts(void) const; // draw the current player counts for each area
...
#ifdef NEXT_BOT
  void DrawFuncNavPrerequisite(void)
      const; // draw bot prerequisite areas from func_nav_prerequisite entities
#endif
//-------------------------------------------------------------------------------------
// Auto-generation
//
#define INCREMENTAL_GENERATION true
  void
  BeginGeneration(bool incremental = false); // initiate the generation process
  void BeginAnalysis(
      bool quitWhenFinished = false); // re-analyze an existing Mesh.  Determine
  ...
  const char *
  GetPlayerSpawnName(void) const; // return name of player spawn entity
  void SetPlayerSpawnName(
      const char *name); // define the name of player spawn entities
  void
  AddWalkableSeed(const Vector &pos,
                  const Vector &normal); // add given walkable position to list
                                         // of seed positions for map sampling
  ...
  void
  CommandNavStoreSelectedSet(void); // stores the current selected set for later
  ...
  void
  CommandNavBeginDragDeselecting(void);    // start dragging a deselection area
  ...
  void
  CommandNavRaiseDragVolumeMin(void); // raise the bottom of the drag volume
  void
  CommandNavLowerDragVolumeMin(void); // lower the bottom of the drag volume
  ...
  void CommandNavToggleDeselecting(
      bool playSound = true); // start/stop continuously de-selecting areas from
                              // the selected set
  void
  CommandNavSelectInvalidAreas(void); // adds invalid areas to the selected set
  void
  CommandNavSelectBlockedAreas(void); // adds blocked areas to the selected set
  ...
  void CommandNavSelectHalfSpace(
      const CCommand &args); // selects all areas that intersect the half-space
  ...
  void
  CommandNavMark(const CCommand &args); // mark an area for further operations
  ...
  void
  CommandNavBeginShiftXY(void); // begin shifting selected set in the XY plane
  ...
  void CommandNavDisconnectOutgoingOneWays(
      void); // disconnect all outgoing one-way connects from each area in the
             // selected set
  ...
  void
  CommandNavTogglePlaceMode(void);   // switch between normal and place editing
  ...
  void
  CommandNavMarkUnnamed(void); // mark an unnamed area for further operations
  ...
  void
  CommandNavMakeSniperSpots(void); // cuts up the marked area into individual
                                   // areas suitable for sniper spots
  ...
  void CommandNavSubdivide(
      const CCommand &args); // subdivide each nav area in X and Y to create 4
                             // new areas - limit min size
  void
  CommandNavSaveSelected(const CCommand &args); // Save selected set to disk
  void CommandNavMergeMesh(
      const CCommand &args); // Merge a saved selected set into the current mesh
  ...
  CNavArea *
  GetMarkedArea(void) const; // return area marked by user in edit mode
  ...
  bool IsContinuouslySelecting(void) const { return m_isContinuouslySelecting; }
  ...
  Vector SnapToGrid(
      ...
      bool forceGrid = false) const; // snap given vector's X & Y coordinates to
                                     // generation grid boundary
  ...
  const char *
  GetFilename(void) const; // return the filename for this map's "nav" file

  /// @todo Remove old select code and make all commands use this selected set
  void
  AddToSelectedSet(CNavArea *area); // add area to the currently selected set
  ...
  bool
  IsSelectedSetEmpty(void) const; // return true if the selected set is empty
   ...
   */
  template <typename Functor> bool ForAllSelectedAreas(Functor &func) {
   ...
   * If functor returns false, stop processing and return false.
   */
  template <typename Functor> bool ForAllAreas(Functor &func) {
  ...
  template <typename Functor> bool ForAllAreas(Functor &func) const {
    FOR_EACH_VEC(TheNavAreas, it) {
    ...
    for (int x = startX; x <= endX; ++x) {
      for (int y = startY; y <= endY; ++y) {
        ...
        if (iGrid >= m_grid.Count()) {
          ExecuteNTimes(10, Warning("** Walked off of the CNavMesh::m_grid in "
  ...
  //-------------------------------------------------------------------------------------
  /**
   * Populate the given vector with all navigation areas that overlap the given
   * extent.
   */
   ...
   * If functor returns false, stop processing and return false.
   */
  template <typename Functor> bool ForAllLadders(Functor &func) {
   ...
   * If functor returns false, stop processing and return false.
   */
  template <typename Functor> bool ForAllLadders(Functor &func) const {
    for (int i = 0; i < m_ladders.Count(); ++i) {
   ...
   * does no processing, and it's return value is true if the area is in the new
   * set to be stiched, and false if it's a pre-existing area.
   */
  template <typename Functor> bool StitchMesh(Functor &func) {
    FOR_EACH_VEC(TheNavAreas, it) {
      CNavArea *area = TheNavAreas[it];
...
protected:
  ...
  bool FindActiveNavArea(
      void); // Finds the area or ladder the local player is currently pointing
             // at.  Returns true if a surface was hit by the traceline.
...
private:
  ...
  bool
      m_isOutOfDate; // true if the Navigation Mesh is older than the actual BSP
  ...
  void
  AllocateGrid(float minX, float maxX, float minY,
  ...
  void DestroyNavigationMesh(
      bool incremental =
          false); // free all resources of the mesh and reset it to empty state
  ...
  CountdownTimer m_showAreaInfoTimer; // Timer that controls how long area info
                                      // is displayed
  ...
  bool m_isContinuouslySelecting; // if true, we are continuously adding to the
                                  // selected set
  ...
  bool UpdateGeneration(
      float maxTime =
          0.25f); // process the auto-generation for 'maxTime' seconds. return
  ...
  CNavNode *AddNode(const Vector &destPos, const Vector &destNormal,
                    ...
                    float flObstacleEndDist); // add a nav node and connect it,
                                              // update current node
  ...
  void StitchAreaSet(
      CUtlVector<CNavArea *>
          *areas); // Stitches an arbitrary set of areas into the existing mesh
  void
  HandleObstacleTopAreas(void); // Handles fixing/generating areas on top of
                                // slim obstacles such as fences and railings
  ...
  CNavNode *
  GetNextWalkableSeedNode(void); // return the next walkable seed as a node
  int m_seedIdx;
  ...
  CUtlVector<int> m_storedSelectedSet; // "Stored" selected set, so we can do
                                       // some editing and then restore the old
                                       // selected set.  Done by ID, so we don't
                                       // have to worry about split/delete/etc.
