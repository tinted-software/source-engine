...
DEFINE_KEYFIELD(m_iszPrintName, FIELD_STRING, "point_printname"),
    ...
    DEFINE_KEYFIELD(m_iszCaptureStartSound, FIELD_STRING,
                    "point_capture_start_sound"),
    DEFINE_KEYFIELD(m_iszCaptureEndSound, FIELD_STRING,
                    "point_capture_end_sound"),
    DEFINE_KEYFIELD(m_iszCaptureInProgress, FIELD_STRING,
                    "point_capture_progress_sound"),
    DEFINE_KEYFIELD(m_iszCaptureInterrupted, FIELD_STRING,
                    "point_capture_interrupted_sound"),
    DEFINE_KEYFIELD(m_bRandomOwnerOnRestart, FIELD_BOOLEAN,
    ...
    DEFINE_OUTPUT(
        m_OnCapTeam1,
        "OnCapTeam1"), // these are fired whenever the point changes modes
    DEFINE_OUTPUT(m_OnCapTeam2, "OnCapTeam2"),
    DEFINE_OUTPUT(m_OnCapReset, "OnCapReset"),

    DEFINE_OUTPUT(m_OnOwnerChangedToTeam1,
                  "OnOwnerChangedToTeam1"), // these are fired when a team does
                                            // the work to change the owner
...
void CTeamControlPoint::Precache(void) {
  for (int i = 0; i < m_TeamData.Count(); i++) {
    ...
    if (!m_TeamData[i].iIcon) {
      Warning("Invalid hud icon material for team %d in control point '%s' ( "
              "point index %d )\n",
    ...
    if (m_TeamData[i].iszOverlay != NULL_STRING) {
      ...
      m_TeamData[i].iOverlay =
          GetMaterialIndex(STRING(m_TeamData[i].iszOverlay));
      Assert(m_TeamData[i].iOverlay != 0);

      if (!m_TeamData[i].iOverlay) {
        Warning("Invalid hud overlay material for team %d in control point "
                "'%s' ( point index %d )\n",
                i, GetDebugName(), GetPointIndex());
      }
    }
...
void CTeamControlPoint::HandleScoring(int iTeam) {
  if (TeamplayRoundBasedRules() &&
      !TeamplayRoundBasedRules()->ShouldScorePerRound()) {
...
int CTeamControlPoint::GetPreviousPointForTeam(int iGameTeam, int iPrevPoint) {
  ...
  CBaseEntity *pEntity = gEntList.FindEntityByName(
      NULL, STRING(m_TeamData[iGameTeam].iszPreviousPoint[iPrevPoint]));
...
// Purpose:
//-----------------------------------------------------------------------------
void CTeamControlPoint::SetOwner(int iCapTeam, bool bMakeSound, int iNumCappers,
                                 int *pCappingPlayers) {
  if (TeamplayGameRules()->PointsMayBeCaptured()) {
    // must be done before setting the owner
    HandleScoring(iCapTeam);
...
// Purpose:
//-----------------------------------------------------------------------------
void CTeamControlPoint::CaptureStart(int iCapTeam, int iNumCappingPlayers,
                                     int *pCappingPlayers) {
  int iNumCappers = iNumCappingPlayers;

  float flLastOwnershipChangeTime = -1.f;
  CBaseEntity *pEnt =
      gEntList.FindEntityByClassname(NULL, GetControlPointMasterName());
  while (pEnt) {
    CTeamControlPointMaster *pMaster =
        dynamic_cast<CTeamControlPointMaster *>(pEnt);
  ...
  IGameEvent *event =
      gameeventmanager->CreateEvent("teamplay_point_startcapture");
  if (event) {
...
void CTeamControlPoint::CaptureInterrupted(bool bBlocked) {
  ...
  if (m_pCaptureInProgressSound == NULL && pSoundName != NULL) {
    ...
    m_pCaptureInProgressSound =
        controller.SoundCreate(filter, entindex(), pSoundName);
...
//-----------------------------------------------------------------------------
// Purpose: Sets the new owner of the point, plays the appropriate sound and
// shows the right model
//-----------------------------------------------------------------------------
void CTeamControlPoint::InternalSetOwner(int iCapTeam, bool bMakeSound,
                                         int iNumCappers,
                                         int *pCappingPlayers) {
  Assert(iCapTeam >= 0 && iCapTeam < GetNumberOfTeams());
  ...
  // We add 1 to the index because we consider the default "no points capped" as
  // 0.
  ...
  for (int i = 0; i < m_TeamData.Count(); i++) {
    ...
    if (GetModelPtr() && GetModelPtr()->SequencesAvailable()) {
      m_TeamData[i].iTeamPoseParam =
          LookupPoseParameter(UTIL_VarArgs("cappoint_%d_percentage", i));
    } else {
...
// Purpose:
//-----------------------------------------------------------------------------
void CTeamControlPoint::SendCapString(int iCapTeam, int iNumCappingPlayers,
                                      int *pCappingPlayers) {
...
//-----------------------------------------------------------------------------
// Purpose: Return true if this point has ever been contested, false if the
// enemy has never contested this point yet
//-----------------------------------------------------------------------------
  ...
  if (m_debugOverlays & OVERLAY_TEXT_BIT) {
    ...
    for (int i = 0; i < MAX_PREVIOUS_POINTS; i++) {
      if (m_TeamData[2].iszPreviousPoint[i] != NULL_STRING) {
        Q_strncat(tempstr, STRING(m_TeamData[2].iszPreviousPoint[i]), 1024,
                  COPY_ALL_CHARACTERS);
    ...
    for (int i = 0; i < MAX_PREVIOUS_POINTS; i++) {
      if (m_TeamData[3].iszPreviousPoint[i] != NULL_STRING) {
        Q_strncat(tempstr, STRING(m_TeamData[3].iszPreviousPoint[i]), 1024,
                  COPY_ALL_CHARACTERS);
        Q_strncat(tempstr, ", ", 1024, COPY_ALL_CHARACTERS);
      }
    }
    ...
    for (int i = 0; i < MAX_CONTROL_POINT_TEAMS; i++) {
      if (ObjectiveResource()->GetBaseControlPointForTeam(i) ==
          GetPointIndex()) {
        Q_snprintf(tempstr, sizeof(tempstr), "Base Control Point for Team %d",
                   i);
        EntityText(text_offset, tempstr, 0);
        text_offset++;
      }
...
void CTeamControlPoint::InternalSetLocked(bool bLocked) {
  if (!bLocked && m_bLocked) {
    // unlocked this point
    IGameEvent *event =
        gameeventmanager->CreateEvent("teamplay_point_unlocked");
    if (event) {
      event->SetInt("cp", m_iPointIndex);
      event->SetString("cpname", STRING(m_iszPrintName));
      event->SetInt("team", m_iTeam);
  ...
  if (ObjectiveResource() &&
      GetPointIndex() < ObjectiveResource()->GetNumControlPoints()) {
...
void CTeamControlPoint::InputSetUnlockTime(inputdata_t &inputdata) {
  ...
  SetContextThink(&CTeamControlPoint::UnlockThink, gpGlobals->curtime + 0.1,
                  CONTROL_POINT_UNLOCK_THINK);
}
