...
DEFINE_FIELD(m_navType, FIELD_INTEGER),
    ...
    //								m_pClippedWaypoints
    //(not saved) 								m_flTimeClipped		(not saved)
    //								m_PreviousMoveActivity
    //(not saved) 								m_PreviousArrivalActivity (not saved)
    ...
    //path, which are not saved) 								m_PeerWaitMoveTimer	(ibid)
    //								m_PeerWaitClearTimer(ibid)
    //								m_NextSidestepTimer
    //(ibid)
    DEFINE_FIELD(m_hBigStepGroundEnt, FIELD_EHANDLE),
    DEFINE_FIELD(m_hLastBlockingEnt, FIELD_EHANDLE),
    //								m_vPosBeginFailedSteer
    //(reset on load) 								m_timeBeginFailedSteer (reset on load) 								m_nNavFailCounter
    //(reset on load) 								m_flLastNavFailTime  (reset on load)
...
bool CAI_Navigator::SetRadialGoal(const Vector &destination,
  ...
  GetPath()->SetWaypoints(GetPathfinder()->BuildRadialRoute(
                              GetLocalOrigin(), center, destination, radius,
                              arc, stepDist, bClockwise,
                              GetPath()->GetGoalTolerance(), bAirRoute),
                          true);
...
bool CAI_Navigator::PreMove() {
  ...
  if (goalType == NAV_GROUND && curType != NAV_GROUND) {
    DevMsg("Warning: %s(%s) appears to have wrong nav type in "
    ...
    if (fShouldMove) {
      ...
      if (result == AINR_OK) {
        ...
        while (moveResult >= AIMR_OK && !m_fNavComplete) {
          ...
          ++nLoopCount;
          if (nLoopCount > 16) {
            DevMsg("ERROR: %s navigation not terminating. Possibly bad "
                   "cyclical solving?\n",
      ...
      // needed until navigation can handle commencing 						  a navigation while in the
      // middle of a climb
...
bool CAI_Navigator::SimplifyPathForward(float maxDist) {
  ...
  if (ShouldAttemptSimplifyTo(pNextWaypoint->GetPos()) &&
      ...
      ShouldSimplifyTo(
          ...
          pNextWaypoint
              ->GetPos())) // @TODO (toml 04-25-03): need to handle this better.
                           // this is here because forward scan may look out so
                           // far that a close obvious solution is skipped (due
                           // to test limiting)
  {
    delete pCurWaypoint;
...
CAI_Navigator::ProgressFlyPath(const AI_ProgressFlyPathParams_t &params) {
  if (IsGoalActive()) {
...
bool CAI_Navigator::DoFindPath(void) {
  ...
  case GOALTYPE_LOCATION_NEAREST_NODE: {
    ...
    if (myNodeID != NO_NODE) {
      destNodeID =
          GetPathfinder()->NearestNodeToPoint(GetPath()->ActualGoalPosition());
    ...
    if (bMustCompleteCurrent || distRemaining > 0.1) {
      ...
      while (distRemaining > 0.01 && pNextPoint) {
        ...
        if (distToNext <= distRemaining + 0.01) {
          ...
          //vPosPrev, 16, 255, 255, 0, false, 10.0f );
