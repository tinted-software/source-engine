...
public:
  const char *GetBlockName() { return "AI"; }
  ...
  void Save(ISave *pSave) {
    ...
    while (pSquad) {
      ...
      pSave->WriteString(
          "", &squadName); // Strings require a header to be read properly
  ...
  void Restore(IRestore *pRestore, bool createPlayers) {
    ...
    if (m_fDoLoad) {
      ...
      while (nSavedSquads--) {
        ...
        if (!pSquad)
          pSquad = &ignored; // if all of the AIs in a squad failed to spawn,
                             // there would be no squad
    ...
    if (g_AI_Manager.NumAIs() && g_pBigAINet->NumNodes() == 0 &&
      ...
      Msg("***\n");
      Msg("ERROR: Loaded save game with no node graph. Load map and build node "
          "graph first!\n");
      Msg("***\n");
