...
inline intp MakeFailCode(const char *pszGeneralError) {
...
enum sharedtasks_e {
  ...
  // Path to node with line of sight to enemy, at least flTaskData degrees away
  // from m_vSavePosition from the enemy's POV
  ...
  // Allow a little slop, and allow for some Z offset (like the target is a gun
  // on a table).
  ...
  // Turn to face directly away from stored save position (save position must be
  // stored first!)
  ...
  // regather conditions at the start of a schedule (all conditions are cleared
  // between schedules)
  TASK_GATHER_CONDITIONS,

  // Require an enemy be seen after the task is run to be considered a candidate
  // enemy
