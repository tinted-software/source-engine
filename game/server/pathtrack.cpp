...
void CPathTrack::Link(void) {
  ...
  if (m_target != NULL_STRING) {
    ...
    if (pTarget == this) {
      ...
      // weren't updating the other linked 		 end, causing problems with walking
...
CPathTrack *CPathTrack::GetNext(void) {
  if (m_paltpath && FBitSet(m_spawnflags, SF_PATH_ALTERNATE) &&
      !FBitSet(m_spawnflags, SF_PATH_ALTREVERSE)) {
    Assert(!m_paltpath.IsValid() || m_paltpath.Get() != NULL);
    return m_paltpath;
  }

  // The paths shouldn't normally be getting deleted so assert that if it was
  // set, it's valid.
