...
//
//			Looks at its target's anglular velocity every frame and
//fires outputs 			as the angular velocity passes a given threshold value.
//
//=============================================================================//
...
enum { AVELOCITY_SENSOR_NO_LAST_RESULT = -2 };
...
private:
  ...
  float
      m_flThreshold; // The threshold angular velocity that we are looking for.
  int m_nLastCompareResult; // The comparison result from our last measurement,
  ...
  // Compare the target's angular velocity to the threshold velocity and fire
  // the appropriate output. These outputs are filtered by m_flFireInterval to
  // ignore excessive oscillations.
...
void CPointAngularVelocitySensor::DrawDebugLines(void) {
  if (m_hTargetEntity) {
    ...
    NDebugOverlay::Line(GetAbsOrigin(), GetAbsOrigin() + vForward * 64, 255, 0,
                        0, false, 0);
    NDebugOverlay::Line(GetAbsOrigin(), GetAbsOrigin() + vRight * 64, 0, 255, 0,
                        false, 0);
    NDebugOverlay::Line(GetAbsOrigin(), GetAbsOrigin() + vUp * 64, 0, 0, 255,
                        false, 0);
  }
  ...
  if (m_bUseHelper == true) {
    ...
    NDebugOverlay::Line(GetAbsOrigin(), GetAbsOrigin() + vAxisForward * 64, 255,
                        0, 0, false, 0);
    NDebugOverlay::Line(GetAbsOrigin(), GetAbsOrigin() + vAxisRight * 64, 0,
                        255, 0, false, 0);
    NDebugOverlay::Line(GetAbsOrigin(), GetAbsOrigin() + vAxisUp * 64, 0, 0,
                        255, false, 0);
  }
}
...
float CPointAngularVelocitySensor::SampleAngularVelocity(CBaseEntity *pEntity) {
  if (pEntity->GetMoveType() == MOVETYPE_VPHYSICS) {
    ...
    if (pPhys != NULL) {
      ...
      // HACKHACK: We don't expect a real 'delta' orientation here, just enough
      // of an error estimate to tell if this thing is trying to move, but
      // failing.
...
//-----------------------------------------------------------------------------
// Purpose: Compares the given entity's angular velocity to the threshold
// velocity. Input  : pEntity - Entity whose angular velocity is being measured.
//			flThreshold -
// Output : Returns -1 if less than, 0 if equal to, or 1 if greater than the
// threshold.
//-----------------------------------------------------------------------------
int CPointAngularVelocitySensor::CompareToThreshold(CBaseEntity *pEntity,
                                                    float flThreshold,
                                                    bool bFireVelocityOutput) {
  if (pEntity == NULL) {
    return 0;
...
//-----------------------------------------------------------------------------
void CPointAngularVelocitySensor::InputTestWithInterval(
    inputdata_t &inputdata) {
  if (m_hTargetEntity != NULL) {
    m_flFireTime = gpGlobals->curtime + m_flFireInterval;
    m_nLastFireResult = AVELOCITY_SENSOR_NO_LAST_RESULT;
    m_nLastCompareResult =
        CompareToThreshold(m_hTargetEntity, m_flThreshold, true);
...
void CPointAngularVelocitySensor::InputTest(inputdata_t &inputdata) {
  int nCompareResult =
      CompareToThreshold(m_hTargetEntity, m_flThreshold, false);
...
//			pActivator -
//-----------------------------------------------------------------------------
void CPointAngularVelocitySensor::FireCompareOutput(int nCompareResult,
...
private:
  ...
  // Outputs
  float m_fPrevVelocity; // stores velocity from last frame, so we only write
                         // the output if it has changed
...
void CPointVelocitySensor::SampleVelocity(void) {
  ...
  float flDot = ( m_vecAxis != vec3_origin ) ? DotProduct( vecVelocity,
  m_vecAxis ) : 1.0f;
  */
  // We want the component of the velocity vector in the direction of the axis,
  // which since the axis is normalized is simply their dot product (eg V . A =
  // |V|*|A|*cos(theta) )
  m_fPrevVelocity =
      (m_vecAxis != vec3_origin) ? DotProduct(vecVelocity, m_vecAxis) : 1.0f;
