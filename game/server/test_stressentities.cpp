...
Vector GetRandomSpot() {
  ...
  if (pEnt) {
    ...
    return Vector(RandomFloat(vMin.x, vMax.x), RandomFloat(vMin.y, vMax.y),
...
void Test_InitRandomEntitySpawner(const CCommand &args) {
  ...
  for (CStressEntityReg *pCur = CStressEntityReg::GetListHead(); pCur;
       pCur = pCur->GetNext())
...
void Test_SpawnRandomEntities(const CCommand &args) {
  if (args.ArgC() < 3) {
    Error("Test_SpawnRandomEntities <min # entities> <max # entities> missing "
          "arguments.");
  }

  if (g_StressEntities.Count() == 0) {
    Error("Test_SpawnRandomEntities: not initialized (call "
          "Test_InitRandomEntitySpawner frst).");
  }
...
ConCommand cc_Test_InitRandomEntitySpawner("Test_InitRandomEntitySpawner",
                                           Test_InitRandomEntitySpawner, 0,
                                           FCVAR_CHEAT);
ConCommand cc_Test_SpawnRandomEntities("Test_SpawnRandomEntities",
                                       Test_SpawnRandomEntities, 0,
                                       FCVAR_CHEAT);
ConCommand cc_Test_RandomizeInPVS("Test_RandomizeInPVS", Test_RandomizeInPVS, 0,
                                  FCVAR_CHEAT);
ConCommand cc_Test_RemoveAllRandomEntities("Test_RemoveAllRandomEntities",
                                           Test_RemoveAllRandomEntities, 0,
                                           FCVAR_CHEAT);
