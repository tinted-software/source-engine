...
public:
  ...
  int AdditionalCollisionChecksThisTick(int currentChecksDone) {
    ...
    if (currentChecksDone < 1200) {
      DevMsg(1,
             "%s: VPhysics Collision detection getting expensive, check for "
...
private:
  ...
  int m_inCallback;
  int m_lastTickFrictionError; // counter to control printing of the dev warning
                               // for large contact systems
