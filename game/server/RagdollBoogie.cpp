...
CRagdollBoogie *CRagdollBoogie::Create(CBaseEntity *pTarget, float flMagnitude,
                                       float flStartTime, float flLengthTime,
                                       int nSpawnFlags) {
  CRagdollProp *pRagdoll = dynamic_cast<CRagdollProp *>(pTarget);
  if (!pRagdoll)
    return NULL;

  CRagdollBoogie *pBoogie =
      (CRagdollBoogie *)CreateEntityByName("env_ragdoll_boogie");
  if (pBoogie == NULL)
    return NULL;
...
void CRagdollBoogie::Spawn() {
  ...
  if (HasSpawnFlags(SF_RAGDOLL_BOOGIE_ELECTRICAL)) {
    SetContextThink(&CRagdollBoogie::ZapThink,
...
void CRagdollBoogie::ZapThink() {
  ...
  if (m_nSuppressionCount == 0) {
    ...
    data.m_flScale =
        HasSpawnFlags(SF_RAGDOLL_BOOGIE_ELECTRICAL_NARROW_BEAM) ? 1.0f : 2.0f;
...
void CRagdollBoogie::BoogieThink(void) {
  ...
  if (m_flBoogieLength != 0) {
    ...
    if (dt >= m_flBoogieLength) {
      // Don't remove while suppressed... this helps if we try to start another
      // boogie
      if (m_nSuppressionCount == 0) {
        UTIL_Remove(this);
      }
    ...
    flMagnitude =
        SimpleSplineRemapVal(dt, 0.0f, m_flBoogieLength, m_flMagnitude, 0.0f);
  }
