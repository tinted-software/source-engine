...
DEFINE_FIELD(m_iMaterialName, FIELD_MATERIALINDEX), DEFINE_EMBEDDED(m_Info),
    ...
    IMPLEMENT_SERVERCLASS_ST(CEntityParticleTrail, DT_EntityParticleTrail)
        SendPropInt(SENDINFO(m_iMaterialName), MAX_MATERIAL_STRING_BITS,
                    SPROP_UNSIGNED),
    SendPropDataTable(SENDINFO_DT(m_Info),
                      &REFERENCE_SEND_TABLE(DT_EntityParticleTrailInfo)),
...
CEntityParticleTrail::Create(CBaseEntity *pTarget,
  ...
  CBaseEntity *pNext;
  for (CBaseEntity *pChild = pTarget->FirstMoveChild(); pChild;
       pChild = pNext) {
...
void CEntityParticleTrail::Spawn() {
  ...
  {
          SetContextThink( ZapThink, gpGlobals->curtime + random->RandomFloat(
  0.1f, 0.3f ), s_pZapContext );
  }
  */
}
...
//-----------------------------------------------------------------------------
void CEntityParticleTrail::SetTransmit(CCheckTransmitInfo *pInfo,
                                       bool bAlways) {
...
void CEntityParticleTrail::DecrementRefCount() {
  ...
  if (m_nRefCount == 0) {
    FollowEntity(NULL);
    g_pNotify->ClearEntity(this);
    SetContextThink(&CEntityParticleTrail::SUB_Remove,
                    gpGlobals->curtime + m_Info.m_flLifetime, s_pRetireContext);
  }
}
...
//-----------------------------------------------------------------------------
void CEntityParticleTrail::NotifySystemEvent(
  ...
  if (eventType == NOTIFY_EVENT_DESTROY) {
    ...
    if (m_nRefCount != 0) {
      m_nRefCount = 0;
      SetContextThink(&CEntityParticleTrail::SUB_Remove,
                      gpGlobals->curtime + m_Info.m_flLifetime,
                      s_pRetireContext);
    }
  }
}
...
// Suppression count
//-----------------------------------------------------------------------------
void CEntityParticleTrail::Destroy(CBaseEntity *pTarget,
                                   const EntityParticleTrailInfo_t &info) {
  int iMaterialName = GetMaterialIndex(STRING(info.m_strMaterialName));
