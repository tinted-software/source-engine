...
void CNPC_Tentacle::Cycle(void) {
  ...
  if (IsSequenceFinished()) {
    ...
    case TENTACLE_ANIM_Lev3_Tap: {
      ...
      UTIL_TraceLine(vecSrc, vecSrc + v_forward * 512, MASK_NPCSOLID_BRUSHONLY,
...
void CNPC_Tentacle::HandleAnimEvent(animevent_t *pEvent) {
  ...
  case 1: // bang
  {
    ...
    // GetAbsAngles().y * (3.14192653 / 180.0) ), sin( GetAbsAngles().y * (M_PI
    // / 180.0) ), 0.0 );
    ...
    switch (m_iTapSound) {
    case TE_SILO:
      UTIL_EmitAmbientSound(GetSoundSourceIndex(), vecSrc, "Tentacle.HitSilo",
                            1.0, SNDLVL_GUNFIRE, 0, 100);
      break;
    case TE_NONE:
      break;
    case TE_DIRT:
      UTIL_EmitAmbientSound(GetSoundSourceIndex(), vecSrc, "Tentacle.HitDirt",
                            1.0, SNDLVL_GUNFIRE, 0, 100);
      break;
    case TE_WATER:
      UTIL_EmitAmbientSound(GetSoundSourceIndex(), vecSrc, "Tentacle.HitWater",
                            1.0, SNDLVL_GUNFIRE, 0, 100);
      break;
    }
  } break;
...
bool CNPC_Tentacle::CreateVPhysics(void) {
  ...
  m_BoneFollowerManager.InitBoneFollowers(
      this, ARRAYSIZE(pTentacleFollowerBoneNames), pTentacleFollowerBoneNames);
