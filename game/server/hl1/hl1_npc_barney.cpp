...
void CNPC_Barney::Spawn() {
  ...
  SetViewOffset(
      Vector(0, 0, 100)); // position of the eyes relative to monster's origin.
  m_flFieldOfView = VIEW_FIELD_WIDE; // NOTE: we need a wide field of view so
                                     // npc will notice player and say hello
  ...
  CapabilitiesAdd(bits_CAP_MOVE_GROUND | bits_CAP_OPEN_DOORS |
                  bits_CAP_AUTO_DOORS | bits_CAP_USE | bits_CAP_DOORS_GROUP);
  CapabilitiesAdd(bits_CAP_INNATE_RANGE_ATTACK1 | bits_CAP_TURN_HEAD |
...
void CNPC_Barney::ModifyOrAppendCriteria(AI_CriteriaSet &criteriaSet) {
  ...
  criteriaSet.AppendCriteria("disaster", predisaster ? "[disaster::pre]"
                                                     : "[disaster::post]");
}
...
int CNPC_Barney::GetSoundInterests(void) {
  return SOUND_WORLD | SOUND_COMBAT | SOUND_CARCASS | SOUND_MEAT |
         SOUND_GARBAGE | SOUND_DANGER | SOUND_PLAYER;
}
...
// relationship table.
//=========================================================
Class_T CNPC_Barney::Classify(void) { return CLASS_PLAYER_ALLY; }
...
bool CNPC_Barney::CheckRangeAttack1(float flDot, float flDist) {
  if (gpGlobals->curtime > m_flCheckAttackTime) {
    ...
    Vector shootTarget =
        ((pEnemy->BodyTarget(shootOrigin) - pEnemy->GetAbsOrigin()) +
         GetEnemyLKP());

    UTIL_TraceLine(shootOrigin, shootTarget, MASK_SOLID, this,
...
void CNPC_Barney::BarneyFirePistol(void) {
  ...
  FireBullets(1, vecShootOrigin, vecShootDir, VECTOR_CONE_2DEGREES, 1024,
              m_iAmmoType);
...
int CNPC_Barney::OnTakeDamage_Alive(const CTakeDamageInfo &inputInfo) {
  ...
  if (m_NPCState != NPC_STATE_PRONE &&
      (inputInfo.GetAttacker()->GetFlags() & FL_CLIENT)) {
    // This is a heurstic to determine if the player intended to harm me
    // If I have an enemy, we can't establish intent (may just be crossfire)
    if (GetEnemy() == NULL) {
      // If the player was facing directly at me, or I'm already suspicious, get
      // mad
      if (HasMemory(bits_MEMORY_SUSPICIOUS) ||
          IsFacing(inputInfo.GetAttacker(), GetAbsOrigin())) {
...
void CNPC_Barney::TraceAttack(const CTakeDamageInfo &inputInfo,
...
int CNPC_Barney::SelectSchedule(void) {
  ...
  if (GetFollowTarget() == NULL) {
    if (HasCondition(COND_PLAYER_PUSHING) &&
        !(GetSpawnFlags() & SF_NPC_PREDISASTER)) // Player wants me to move
  ...
  case NPC_STATE_COMBAT: {
    ...
    if (HasCondition(COND_ENEMY_DEAD))
      return BaseClass::SelectSchedule(); // call base class, all code to handle
                                          // dead enemies is centralized there.
...
char *CNPC_DeadBarney::m_szPoses[] = {"lying_on_back", "lying_on_side",
                                      "lying_on_stomach"};
