...
public:
  ...
  bool m_fCourseAdjust; // followers set this flag TRUE to override flocking
                        // while they avoid something
  bool m_fPathBlocked;  // TRUE if there is an obstacle ahead
  Vector m_vecReferencePoint;   // last place we saw leader
  Vector m_vecAdjustedVelocity; // adjusted velocity (used when fCourseAdjust is
                                // TRUE)
  float m_flGoalSpeed;
...
DEFINE_FIELD(m_pSquadLeader, FIELD_CLASSPTR),
    ...
    DEFINE_FIELD(m_flAlertTime, FIELD_TIME), DEFINE_THINKFUNC(IdleThink),
    DEFINE_THINKFUNC(Start), DEFINE_THINKFUNC(FlockLeaderThink),
    DEFINE_THINKFUNC(FlockFollowerThink), DEFINE_THINKFUNC(FallHack),
...
bool CNPC_FlockingFlyerFlock::KeyValue(const char *szKeyName,
                                       const char *szValue) {
...
void CNPC_FlockingFlyer::FlockLeaderThink(void) {
  ...
  if (!m_fTurning) // something in the way and boid is not already turning to
                   // avoid
  {
    // measure clearance on left and right to pick the best dir to turn
    UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + vRight * AFLOCK_CHECK_DIST,
  ...
  // check and make sure we aren't about to plow into the ground, don't let it
  // happen
  UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() - vUp * 16,
                 MASK_SOLID_BRUSHONLY, this, COLLISION_GROUP_NONE, &tr);
...
bool CNPC_FlockingFlyer::FPathBlocked(void) {
  ...
  UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + vForward * AFLOCK_CHECK_DIST,
                 MASK_SOLID_BRUSHONLY, this, COLLISION_GROUP_NONE, &tr);
  ...
  // extra wide checks
  UTIL_TraceLine(GetAbsOrigin() + vRight * 12,
                 GetAbsOrigin() + vRight * 12 + vForward * AFLOCK_CHECK_DIST,
                 MASK_SOLID_BRUSHONLY, this, COLLISION_GROUP_NONE, &tr);
  ...
  if (!fBlocked && gpGlobals->curtime - m_flLastBlockedTime > 6) {
    // not blocked, and it's been a few seconds since we've actually been
    // blocked.
...
void CNPC_FlockingFlyer::SpreadFlock() {
  ...
  while (pList) {
    if (pList != this &&
        (GetAbsOrigin() - pList->GetAbsOrigin()).Length() <= AFLOCK_TOO_CLOSE) {
      // push the other away
      vecDir = (pList->GetAbsOrigin() - GetAbsOrigin());
      VectorNormalize(vecDir);

      // store the magnitude of the other boid's velocity, and normalize it so
      // we can average in a course that points away from the leader.
...
//
// This function should **ONLY** be called when Caller's velocity is
// normalized!!
//=========================================================
...
void CNPC_FlockingFlyer::FlockFollowerThink(void) {
  ...
  if (IsLeader() || !InSquad()) {
    // the leader has been killed and this flyer suddenly finds himself the
    // leader.
    SetThink(&CNPC_FlockingFlyer::FlockLeaderThink);
    return;
  }
  ...
  // if we are too far from leader, average a vector towards it into our current
  // velocity
  if (flDistToLeader > AFLOCK_TOO_FAR) {
