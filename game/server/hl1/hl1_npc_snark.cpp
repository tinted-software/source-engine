...
DEFINE_FIELD(m_flDie, FIELD_TIME), DEFINE_FIELD(m_vecTarget, FIELD_VECTOR),
...
void CSnark::Spawn(void) {
  ...
  m_flNextBounceSoundTime =
      gpGlobals->curtime; // reset each time a snark is spawned.
...
void CSnark::Event_Killed(const CTakeDamageInfo &inputInfo) {
  ...
  // since squeak grenades never leave a body behind, clear out their takedamage
  // now. Squeaks do a bit of radius damage when they pop, and that radius
  // damage will continue to call this function unless we acknowledge the
  // Squeak's death now. (sjb)
  ...
  CSoundEnt::InsertSound(SOUND_COMBAT, GetAbsOrigin(), SNARK_EXPLOSION_VOLUME,
                         3.0);

  UTIL_BloodDrips(WorldSpaceCenter(), Vector(0, 0, 0), BLOOD_COLOR_YELLOW, 80);

  if (m_hOwner != NULL) {
    RadiusDamage(CTakeDamageInfo(this, m_hOwner, GetDamage(), DMG_BLAST),
                 GetAbsOrigin(), GetDamage() * 2.5, CLASS_NONE, NULL);
  } else {
    RadiusDamage(CTakeDamageInfo(this, this, GetDamage(), DMG_BLAST),
                 GetAbsOrigin(), GetDamage() * 2.5, CLASS_NONE, NULL);
  }
...
void CSnark::HuntThink(void) {
  ...
  // FIXME: There's a problem in this movetype that causes it to set a ground
  // entity but never recheck to clear it 		 For now, we stomp it clear and force
  //it to revalidate -- jdw
  ...
  if ((m_flDie - gpGlobals->curtime <= 0.5) &&
      (m_flDie - gpGlobals->curtime >= 0.3)) {
    CPASAttenuationFilter filter(this);
  ...
  float flpitch =
      155.0 - 60.0 * ((m_flDie - gpGlobals->curtime) / SQUEEK_DETONATE_DELAY);
  if (flpitch < 80)
    flpitch = 80;
...
void CSnark::SuperBounceTouch(CBaseEntity *pOther) {
  ...
  if (pOther->m_takedamage && m_flNextAttack < gpGlobals->curtime) {
    ...
    if (tr.m_pEnt == pOther) {
      ...
      if (tr.m_pEnt->GetModelIndex() != GetModelIndex()) {
        ...
        if (m_hOwner != NULL) {
          CTakeDamageInfo info(this, m_hOwner, sk_snark_dmg_bite.GetFloat(),
                               DMG_SLASH);
          CalculateMeleeDamageForce(&info, vecForward, tr.endpos);
          pOther->DispatchTraceAttack(info, vecForward, &tr);
        } else {
          CTakeDamageInfo info(this, this, sk_snark_dmg_bite.GetFloat(),
                               DMG_SLASH);
          CalculateMeleeDamageForce(&info, vecForward, tr.endpos);
          pOther->DispatchTraceAttack(info, vecForward, &tr);
        }
        ...
        SetDamage(GetDamage() +
                  sk_snark_dmg_pop.GetFloat()); // add more explosion damage
  ...
  if (g_pGameRules->IsMultiplayer()) {
    // in multiplayer, we limit how often snarks can make their bounce sounds to
    // prevent overflows.
