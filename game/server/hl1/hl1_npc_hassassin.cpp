...
enum {
  TASK_ASSASSIN_FALL_TO_GROUND =
      LAST_SHARED_TASK + 1, // falling and waiting to hit ground
};
...
Class_T CNPC_HAssassin::Classify(void) { return CLASS_HUMAN_MILITARY; }
...
int CNPC_HAssassin::MeleeAttack1Conditions(float flDot, float flDist) {
  if (m_flNextJump < gpGlobals->curtime &&
      (flDist <= 128 || HasMemory(MEMORY_BADJUMP)) && GetEnemy() != NULL) {
    trace_t tr;

    Vector vecMin =
        Vector(random->RandomFloat(0, -64), random->RandomFloat(0, -64), 0);
    Vector vecMax =
        Vector(random->RandomFloat(0, 64), random->RandomFloat(0, 64), 160);

    Vector vecDest = GetAbsOrigin() + Vector(random->RandomFloat(-64, 64),
                                             random->RandomFloat(-64, 64), 160);

    UTIL_TraceHull(GetAbsOrigin() + Vector(0, 0, 36),
                   GetAbsOrigin() + Vector(0, 0, 36), vecMin, vecMax,
                   MASK_SOLID, this, COLLISION_GROUP_NONE, &tr);

    // NDebugOverlay::Box( GetAbsOrigin() + Vector( 0, 0, 36 ), vecMin, vecMax,
    // 0,0, 255, 0, 2.0 );
...
int CNPC_HAssassin::RangeAttack1Conditions(float flDot, float flDist) {
  if (!HasCondition(COND_ENEMY_OCCLUDED) && flDist > 64 && flDist <= 2048) {
    ...
    // world.
    UTIL_TraceLine(vecSrc, GetEnemy()->BodyTarget(vecSrc), MASK_SOLID, this,
...
int CNPC_HAssassin::RangeAttack2Conditions(float flDot, float flDist) {
  ...
  if (m_flNextGrenadeCheck < gpGlobals->curtime &&
      !HasCondition(COND_ENEMY_OCCLUDED) && flDist <= 512) {
    Vector vTossPos;
    QAngle vAngles;
    ...
    Vector vecToss =
        VecCheckThrow(this, vTossPos, GetEnemy()->WorldSpaceCenter(), flDist,
...
void CNPC_HAssassin::RunTask(const Task_t *pTask) {
  ...
  case TASK_ASSASSIN_FALL_TO_GROUND:
    ...
    if (GetFlags() & FL_ONGROUND) {
      TaskComplete();
    } else if (gpGlobals->curtime > m_flWaitFinished ||
               GetAbsVelocity().z == 0.0) {
      ...
      UTIL_TraceEntity(this, GetAbsOrigin(), GetAbsOrigin() - Vector(0, 0, 1),
                       MASK_NPCSOLID, this, COLLISION_GROUP_NONE, &trace);
...
void CNPC_HAssassin::HandleAnimEvent(animevent_t *pEvent) {
  ...
  case ASSASSIN_AE_TOSS1: {
    ...
    CHandGrenade *pGrenade =
        (CHandGrenade *)Create("grenade_hand", vTossPos, vec3_angle);
    if (pGrenade) {
...
void CNPC_HAssassin::Shoot(void) {
  ...
  Vector vecShellVelocity = vRight * random->RandomFloat(40, 90) +
                            vUp * random->RandomFloat(75, 200) +
                            vForward * random->RandomFloat(-40, 40);
  EjectShell(GetAbsOrigin() + vUp * 32 + vForward * 12, vecShellVelocity,
             GetAbsAngles().y, 0);
  FireBullets(1, vecShootOrigin, vecShootDir,
              Vector(m_flDiviation, m_flDiviation, m_flDiviation), 2048,
              m_iAmmoType); // shoot +-8 degrees

  // NDebugOverlay::Line( vecShootOrigin, vecShootOrigin + vecShootDir * 2048,
  // 255, 0, 0, true, 2.0 );
...
void CNPC_HAssassin::RunAI(void) {
  ...
  if (g_iSkillLevel != SKILL_HARD || GetEnemy() == NULL ||
      m_lifeState == LIFE_DEAD || GetActivity() == ACT_RUN ||
...
DEFINE_SCHEDULE(
    ...
    "		TASK_SET_SCHEDULE		"
    "SCHEDULE:SCHED_ASSASSIN_JUMP_ATTACK"
    "	"
    "	Interrupts")
...
DEFINE_SCHEDULE(SCHED_ASSASSIN_HUNT,
                ...
                "		TASK_STOP_MOVING			0"
                "		TASK_SET_FAIL_SCHEDULE		"
                "SCHEDULE:SCHED_ASSASSIN_TAKE_COVER_FROM_ENEMY2"
