...
DEFINE_FIELD(m_flTurning, FIELD_FLOAT),
    ...
    DEFINE_FIELD(m_obstacle, FIELD_FLOAT), DEFINE_FIELD(m_top, FIELD_FLOAT),
    DEFINE_FIELD(m_bottom, FIELD_FLOAT), DEFINE_FIELD(m_height, FIELD_FLOAT),
    DEFINE_FIELD(m_waterTime, FIELD_TIME), DEFINE_FIELD(m_sideTime, FIELD_TIME),
    DEFINE_FIELD(m_zTime, FIELD_TIME), DEFINE_FIELD(m_stateTime, FIELD_TIME),
...
void CNPC_Leech::Spawn(void) {
  ...
  // Don't push the minz down too much or the water check will fail because this
  // entity is really point-sized
  SetSolid(SOLID_BBOX);
...
void CNPC_Leech::DeadThink(void) {
  ...
  if (GetAbsVelocity().x != 0 || GetAbsVelocity().y != 0) {
    ...
    UTIL_TraceLine(GetLocalOrigin(), GetLocalOrigin() + GetAbsVelocity() * 0.5,
...
void CNPC_Leech::HandleAnimEvent(animevent_t *pEvent) {
  ...
  case LEECH_AE_ATTACK:
    ...
    if (pEnemy != NULL) {
      ...
      if (DotProduct(dir, face) >
          0.9) // Only take damage if the leech is facing the prey
      {
...
void CNPC_Leech::RecalculateWaterlevel(void) {
  ...
  UTIL_TraceLine(GetLocalOrigin(), vecTest, MASK_SOLID, this,
  ...
  m_top = UTIL_WaterLevel(GetLocalOrigin(), GetLocalOrigin().z,
                          GetLocalOrigin().z + 400) -
          1;

#if DEBUG_BEAMS
  NDebugOverlay::Line(GetLocalOrigin(),
                      GetLocalOrigin() + Vector(0, 0, m_bottom), 0, 255, 0,
                      false, 0.1f);
  NDebugOverlay::Line(GetLocalOrigin(), GetLocalOrigin() + Vector(0, 0, m_top),
                      0, 255, 255, false, 0.1f);
#endif
...
void CNPC_Leech::SwimThink(void) {
  ...
  ); SetAbsVelocity( vec3_origin ); return;
  }
  else*/
  SetNextThink(gpGlobals->curtime + 0.1);
  ...
  default:
    ...
    if ((GetLocalOrigin() - m_oldOrigin).Length() < 1) {
      // If leech didn't move, there must be something blocking it, so try to
      // turn
      m_sideTime = 0;
    }
  ...
  if (m_obstacle == 1.0) {
    ...
    m_fPathBlocked = FALSE;
    m_flSpeed = UTIL_Approach(targetSpeed, m_flSpeed,
                              LEECH_SWIM_ACCEL * LEECH_FRAMETIME);
    ...
    if (m_flTurning ==
        0) // something in the way and leech is not already turning to avoid
    {
      Vector vecTest;
      // measure clearance on left and right to pick the best dir to turn
      vecTest = GetLocalOrigin() + (vRight * LEECH_SIZEX) +
                (vForward * LEECH_CHECK_DIST);
      UTIL_TraceLine(GetLocalOrigin(), vecTest, MASK_SOLID, this,
                     COLLISION_GROUP_NONE, &tr);
...
float CNPC_Leech::ObstacleDistance(CBaseEntity *pTarget) {
  ...
  if (m_sideTime < gpGlobals->curtime) {
    // extra wide checks
    vecTest = GetLocalOrigin() + vRight * LEECH_SIZEX * 2 +
              vForward * LEECH_CHECK_DIST;
    UTIL_TraceLine(GetLocalOrigin(), vecTest, MASK_SOLID, this,
                   COLLISION_GROUP_NONE, &tr);
...
void CNPC_Leech::UpdateMotion(void) {
  ...
  if (!GetWaterLevel()) {
    ...
    RecalculateWaterlevel();
    m_waterTime =
        gpGlobals->curtime + 2; // Recalc again soon, water may be rising
  }
  ...
  if (m_fPathBlocked) {
    ...
    if (color > 255)
      color = 255;
    NDebugOverlay::Line(GetLocalOrigin(),
                        GetLocalOrigin() + vForward * LEECH_CHECK_DIST, 255,
                        color, color, false, 0.1f);
  } else
    NDebugOverlay::Line(GetLocalOrigin(),
                        GetLocalOrigin() + vForward * LEECH_CHECK_DIST, 255,
                        255, 0, false, 0.1f);
  ...
  NDebugOverlay::Line(GetLocalOrigin(),
                      GetLocalOrigin() + vRight * (vAngularVelocity.y * 0.25),
                      0, 0, 255, false, 0.1f);
#endif
}
...
void CNPC_Leech::Event_Killed(const CTakeDamageInfo &info) {
  ...
  //  tell owner ( if any ) that we're dead.This is mostly for MonsterMaker
  //  functionality.
