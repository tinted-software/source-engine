...
void CWeaponTripMine::PrimaryAttack(void) {
  ...
  UTIL_TraceLine(vecSrc, vecSrc + vecAiming * 64, MASK_SHOT, pPlayer,
  ...
  if (tr.fraction < 1.0) {
    ...
    if (pEntity && !(pEntity->GetFlags() & FL_CONVEYOR)) {
      ...
      CBaseEntity::Create("monster_tripmine", tr.endpos + tr.plane.normal * 2,
                          angles, pPlayer);
    ...
    SetWeaponIdleTime(m_flTimeWeaponIdle =
                          gpGlobals->curtime + random->RandomFloat(10, 15));
  }
}
...
void CTripmineGrenade::Spawn(void) {
  ...
  // Don't collide with the player (the beam will still be tripped by one,
  // however)
...
void CTripmineGrenade::PowerupThink(void) {
  if (m_hStuckOn == NULL) {
    ...
    UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + m_vecDir * 32, MASK_SHOT,
                   NULL, COLLISION_GROUP_NONE, &tr);

    if (tr.m_pEnt && pOldOwner && (tr.m_pEnt == pOldOwner) &&
        pOldOwner->IsPlayer()) {
    ...
    UTIL_TraceLine(GetAbsOrigin() + m_vecDir * 8,
                   GetAbsOrigin() - m_vecDir * 32, MASK_SHOT, pOldOwner,
                   COLLISION_GROUP_NONE, &tr);
    ...
    if (tr.fraction < 1.0) {
      ...
      // somehow we've been deployed on nothing, or something that was there,
      // but now isn't. remove ourselves

      StopSound("TripmineGrenade.Deploy");
      StopSound("TripmineGrenade.Charge");
      SetThink(&CBaseEntity::SUB_Remove);
      SetNextThink(gpGlobals->curtime + 0.1f);
      //			ALERT( at_console, "WARNING:Tripmine at %.0f,
      ...
      return;
             ...
             (m_angStuckOn != m_hStuckOn->GetAbsAngles())) {
    // what we were stuck on has moved, or rotated. Create a tripmine weapon and
    // drop to ground

    StopSound("TripmineGrenade.Deploy");
    StopSound("TripmineGrenade.Charge");
    CBaseEntity *pMine = Create("weapon_tripmine",
                                GetAbsOrigin() + m_vecDir * 24, GetAbsAngles());
...
void CTripmineGrenade::MakeBeam(void) {
  ...
  Vector vecTmpEnd =
      GetAbsOrigin() + m_vecDir * MAX_TRACE_LENGTH * m_flBeamLength;
  ...
  m_hBeam->AddSpawnFlags(
      SF_BEAM_TEMPORARY); // so it won't save and come back to haunt us later..
}
...
void CTripmineGrenade::BeamBreakThink(void) {
  ...
  if (!m_hBeam) {
    ...
    UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() - forward * 12.0f, MASK_SOLID,
                   this, COLLISION_GROUP_NONE, &stuckOnTrace);
...
GetAngles() ); SetThink( &CBaseEntity::SUB_Remove ); SetNextThink(
...
void CTripmineGrenade::DelayDeathThink(void) {
  KillBeam();
  trace_t tr;
  UTIL_TraceLine(GetAbsOrigin() + m_vecDir * 8, GetAbsOrigin() - m_vecDir * 64,
