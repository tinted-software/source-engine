...
DEFINE_FIELD(m_flStopAttack, FIELD_TIME),
    ...
    DEFINE_ENTITYFUNC(DieTouch), DEFINE_THINKFUNC(StartDart),
    DEFINE_THINKFUNC(StartTrack), DEFINE_ENTITYFUNC(DartTouch),
    DEFINE_ENTITYFUNC(TrackTouch), DEFINE_THINKFUNC(TrackTarget),
    END_DATADESC()
...
void CNPC_Hornet::DieTouch(CBaseEntity *pOther) {
  ...
  CalculateBulletDamageForce(&info, GetAmmoDef()->Index("Hornet"),
...
void CNPC_Hornet::IgniteTrail(void) {
  ...
  TE_BeamFollow(filter, 0.0, entindex(), iHornetTrail, 0, 1, 2, 0.5, 0.5,
                vColor.x, vColor.y, vColor.z, 128);
}
...
void CNPC_Hornet::TrackTouch(CBaseEntity *pOther) {
  ...
  if (pOther == GetOwnerEntity() ||
      pOther->GetModelIndex() ==
          GetModelIndex()) { // bumped into the guy that shot it.
    // SetSolid( SOLID_NOT );
    return;
  }

  int nRelationship = IRelationType(pOther);
  if ((nRelationship == D_FR || nRelationship == D_NU ||
       nRelationship == D_LI)) {
...
void CNPC_Hornet::DartTouch(CBaseEntity *pOther) { DieTouch(pOther); }
...
void CNPC_Hornet::TrackTarget(void) {
  ...
  // UNDONE: The player pointer should come back after returning from another
  // level
  if (GetEnemy() == NULL) { // enemy is dead.
    GetSenses()->Look(1024);
    SetEnemy(BestEnemy());
  }
  ...
  // measure how far the turn is, the wider the turn, the slow we'll go this
  // time.
  ...
  if (flDelta <= 0 &&
      m_iHornetType == HORNET_TYPE_RED) { // no flying backwards, but we don't
                                          // want to invert this, cause we'd go
                                          // fast when we have to turn REAL far.
    flDelta = 0.25;
  }
  ...
  if (GetOwnerEntity() && (GetOwnerEntity()->GetFlags() & FL_NPC)) {
    ...
    vecVel.x +=
        random->RandomFloat(-0.10, 0.10); // scramble the flight dir a bit.
  ...
  case HORNET_TYPE_RED:
    SetAbsVelocity(vecVel *
                   (m_flFlySpeed *
                    flDelta)); // scale the dir by the ( speed * width of turn )
