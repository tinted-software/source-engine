...
enum {
  ...
  TASK_NODE_DELAY, // Delay walking toward node for a bit. You've failed to get
                   // there
...
// #define bits_COND_NODE_SEQUENCE			( COND_SPECIAL1 )
// // pev->netname contains the name of a sequence to play
...
Vector VecCheckSplatToss(CBaseEntity *pEntity, const Vector &vecSpot1,
                         Vector vecSpot2, float maxHeight);
void MortarSpray(const Vector &position, const Vector &direction,
                 int spriteModel, int count);
...
public:
  ...
  static CBMortar *Shoot(CBaseEntity *pOwner, Vector vecStart,
                         Vector vecVelocity);
...
DEFINE_FIELD(m_maxFrame, FIELD_INTEGER), DEFINE_FIELD(m_flDmgTime, FIELD_FLOAT),
    DEFINE_FUNCTION(Animate), DEFINE_FIELD(m_iFrame, FIELD_INTEGER),
...
DEFINE_FIELD(m_nodeTime, FIELD_TIME), DEFINE_FIELD(m_crabTime, FIELD_TIME),
...
void CNPC_BigMomma::Spawn() {
  ...
  //	pev->view_ofs		= Vector ( 0, 0, 128 );// position of the eyes
  //relative to monster's origin.
  m_flFieldOfView = 0.3; // indicates the width of this monster's forward view
...
void CNPC_BigMomma::NodeReach(void) {
  ...
  if (pTarget->m_iHealth >= 1)
    m_iMaxHealth = m_iHealth =
        pTarget->m_iHealth * sk_bigmomma_health_factor.GetFloat();
...
void CNPC_BigMomma::TraceAttack(const CTakeDamageInfo &info,
  ...
  if (ptr->hitbox <= 9) {
    ...
    // don't hurt the monster much, but allow bits_COND_LIGHT_DAMAGE to be
    // generated
...
void CNPC_BigMomma::RunTask(const Task_t *pTask) {
  ...
  case TASK_PLAY_NODE_SEQUENCE:
    ...
    if (IsSequenceFinished()) {
      ...
      if (GetTarget())
        pTarget =
            gEntList.FindEntityByName(NULL, STRING(GetTarget()->m_target));
...
void CNPC_BigMomma::HandleAnimEvent(animevent_t *pEvent) {
  ...
  case BIG_AE_MELEE_ATTACK1: {
    ...
    if (pHurt) {
      ...
      CalculateMeleeDamageForce(&info, (pHurt->GetAbsOrigin() - GetAbsOrigin()),
                                pHurt->GetAbsOrigin());
      ...
      case BIG_AE_MELEE_ATTACKBR:
        //						pHurt->pev->velocity =
        //pHurt->pev->velocity + (vecFwd * 150) + Vector(0,0,250) - (vecRight *
        //200);
      ...
      case BIG_AE_MELEE_ATTACK1:
        pHurt->SetAbsVelocity(pHurt->GetAbsVelocity() + (vecFwd * 220) +
                              Vector(0, 0, 200));
        break;
      }
  ...
  case BIG_AE_JUMP_FORWARD:
    ...
    SetAbsOrigin(GetAbsOrigin() +
                 Vector(0, 0, 1)); // take him off ground so engine doesn't
                                   // instantly reset onground
...
void CNPC_BigMomma::LayHeadcrab(void) {
  CBaseEntity *pChild =
      CBaseEntity::Create(BIG_CHILDCLASS, GetAbsOrigin(), GetAbsAngles(), this);
  ...
  trace_t tr;
  UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() - Vector(0, 0, 100), MASK_SOLID,
...
void CNPC_BigMomma::DeathNotice(CBaseEntity *pevChild) {
  if (m_crabCount >
      0) // Some babies may cross a transition, but we reset the count then
  {
    m_crabCount--;
  }
  if (IsAlive()) {
...
Vector VecCheckSplatToss(CBaseEntity *pEnt, const Vector &vecSpot1,
                         Vector vecSpot2, float maxHeight) {
  trace_t tr;
  ...
  UTIL_TraceLine(vecMidPoint, vecMidPoint + Vector(0, 0, maxHeight),
                 MASK_SOLID_BRUSHONLY, pEnt, COLLISION_GROUP_NONE, &tr);
  vecApex = tr.endpos;

  UTIL_TraceLine(vecSpot1, vecApex, MASK_SOLID, pEnt, COLLISION_GROUP_NONE,
                 &tr);
  if (tr.fraction != 1.0) {
    // fail!
    return vec3_origin;
  ...
  // How high should the grenade travel (subtract 15 so the grenade doesn't hit
  // the ceiling)?
  ...
  // How fast does the grenade need to travel to reach that height given
  // gravity?
  ...
  // Travel half the distance to the target in that time (apex is at the
  // midpoint)
...
int CNPC_BigMomma::RangeAttack1Conditions(float flDot, float flDist) {
  ...
  if (flDist <= BIG_MORTARDIST && m_mortarTime < gpGlobals->curtime) {
    ...
    if (pEnemy) {
      ...
      m_vTossDir =
          VecCheckSplatToss(this, startPos, pEnemy->BodyTarget(GetAbsOrigin()),
                            random->RandomFloat(150, 500));
...
void MortarSpray(const Vector &position, const Vector &direction,
  ...
  te->SpriteSpray(filter, 0.0, &position, &direction, spriteModel, 200, 80,
                  count);
}

// UNDONE: right now this is pretty much a copy of the squid spit with minor
// changes to the way it does damage
void CBMortar::Spawn(void) {
  ...
  pSprite =
      CSprite::SpriteCreate("sprites/mommaspit.vmt", GetAbsOrigin(), true);
...
CBMortar *CBMortar::Shoot(CBaseEntity *pOwner, Vector vecStart,
                          Vector vecVelocity) {
...
void CBMortar::Touch(CBaseEntity *pOther) {
  ...
  RadiusDamage(
      CTakeDamageInfo(this, pOwner, sk_bigmomma_dmg_blast.GetFloat(), DMG_ACID),
      GetAbsOrigin(), sk_bigmomma_radius_blast.GetFloat(), CLASS_NONE, NULL);
