...
int CNPC_Scientist::SelectSchedule(void) {
  ...
  case NPC_STATE_IDLE:
    ...
    if (GetFollowTarget()) {
      ...
      } else // UNDONE: When afraid, scientist won't move out of your way.  Keep
             // This?  If not, write move away scared
      {
        if (HasCondition(
  ...
  case NPC_STATE_COMBAT:
    ...
    if (HasCondition(COND_HEAR_COMBAT) || HasCondition(COND_HEAR_DANGER))
      return SCHED_TAKE_COVER_FROM_BEST_SOUND; // Cower and panic from the scary
                                               // sound!
