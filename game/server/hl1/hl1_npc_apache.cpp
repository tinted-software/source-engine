...
public:
  ...
  void LaunchRocket(Vector &viewDir, int damage, int radius,
                    Vector vecLaunchPoint);
  ...
  /* int		Save( CSave &save );
       ...
       void EXPORT StartupUse( CBaseEntity *pActivator, CBaseEntity *pCaller,
     USE_TYPE useType, float value ); void EXPORT NullThink( void );
...
DEFINE_FIELD(m_iRockets, FIELD_INTEGER), DEFINE_FIELD(m_flForce, FIELD_FLOAT),
    ...
    DEFINE_FIELD(m_vecGoal, FIELD_VECTOR), DEFINE_FIELD(m_angGun, FIELD_VECTOR),
...
void CNPC_Apache::Spawn(void) {
  ...
  // &s_vecSurroundingMins, &s_vecSurroundingMaxs ); AddSolidFlags(
  // FSOLID_CUSTOMRAYTEST | FSOLID_CUSTOMBOXTEST );
...
void CNPC_Apache::Flight(void) {
  ...
  float flDistToDesiredPosition =
      (GetAbsOrigin() - m_vecDesiredPosition).Length();
  ...
  AngleVectors(GetAbsAngles() + GetLocalAngularVelocity() * 2 + angAdj,
               &forward, &right, &up);
  // Vector vecEst1 = GetAbsOrigin() + pev->velocity + gpGlobals->v_up *
  // m_flForce - Vector( 0, 0, 384 ); float flSide = DotProduct( m_posDesired -
  // vecEst1, gpGlobals->v_right );
  ...
  AngleVectors(GetAbsAngles() + GetLocalAngularVelocity() * 1 + angAdj,
               &forward, &right, &up);
  Vector vecEst = GetAbsOrigin() + vecVel * 2.0 + up * m_flForce * 20 -
                  Vector(0, 0, 384 * 2);
  ...
  if (flDist > 0 && flSpeed < m_flGoalSpeed &&
      GetAbsAngles().x + angVel.x < 40) {
    // ALERT( at_console, "F " );
    // lean forward
    angVel.x += 12.0;
  ...
  // GetAbsOrigin().x, pev->velocity.x, flDist, flSpeed, GetAbsAngles().x,
  // pev->avelocity.x, m_flForce ); ALERT( at_console, "%.0f %.0f : %.0f %0.f :
  // %.0f\n", GetAbsOrigin().z, pev->velocity.z, vecEst.z, m_posDesired.z,
  // m_flForce );
}
...
bool CNPC_Apache::FireGun() {
  ...
  // forward
  //	NDebugOverlay::Line(  GetAbsOrigin(), GetAbsOrigin() + ( forward * 200
  //), 255,0,0, false, 0.1); LOS
  //	NDebugOverlay::Line( posGun, m_vecTargetPosition , 0,0,255, false, 0.1);
  //	NDebugOverlay::Box( GetAbsOrigin(), s_vecSurroundingMins,
  //s_vecSurroundingMaxs, 0, 255,0, false, 0.1);
  ...
  if (flDot > 0.98) {
    ...
    FireBullets(1, posGun, vecTarget, VECTOR_CONE_4DEGREES, 8192, m_iAmmoType,
                2);
...
void CNPC_Apache::FireRocket(void) {
  ...
  Vector vecSrc =
      GetAbsOrigin() + 1.5 * (vForward * 21 + vRight * 70 * side + vUp * -79);
...
void CNPC_Apache::AimRocketGun(void) {
  ...
  trace_t tr1;
  UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + vecEst * 4096, MASK_ALL, this,
                 COLLISION_GROUP_NONE, &tr1);

  //	NDebugOverlay::Line(GetAbsOrigin(), tr1.endpos, 255,255,0, false, 0.1);

  UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + vTargetDir * 4096, MASK_ALL,
                 this, COLLISION_GROUP_NONE, &tr1);

  //	NDebugOverlay::Line(GetAbsOrigin(), tr1.endpos, 0,255,0, false, 0.1);

  // ALERT( at_console, "%d %d %d %4.2f\n", GetAbsAngles().x < 0, DotProduct(
  // pev->velocity, gpGlobals->v_forward ) > -100, m_flNextRocket <
  // gpGlobals->curtime, DotProduct( m_vecTarget, vecEst ) );
  ...
  } else if (DotProduct(GetAbsVelocity(), vForward) > -100 &&
    ...
    if (m_flLastSeen + 60 > gpGlobals->curtime) {
      if (GetEnemy() != NULL) {
        ...
        // if (DotProduct( vTargetDir, vecEst) > 0.5)
        {
          ...
          UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + vecEst * 4096,
                         MASK_ALL, this, COLLISION_GROUP_NONE, &tr);

          //		NDebugOverlay::Line(GetAbsOrigin(), tr.endpos,
          //255,0,255, false, 5);

          //					if ((tr.endpos -
          //m_vecTargetPosition).Length() < 512)
        ...
        UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + vecEst * 4096, MASK_ALL,
...
void CNPC_Apache::LaunchRocket(Vector &viewDir, int damage, int radius,
                               Vector vecLaunchPoint) {

  CGrenadeHomer *pGrenade = CGrenadeHomer::CreateGrenadeHomer(
      MAKE_STRING("models/weapons/w_missile.mdl"), MAKE_STRING("Apache.RPG"),
  ...
  pGrenade->Launch(this, GetEnemy(), viewDir * 1500, 500, 0,
...
void CNPC_Apache::DyingThink(void) {
  ...
  if (gpGlobals->curtime > m_flNextCrashExplosion) {
    ...
    te->Explosion(pasFilter, 0.0, &pos, g_sModelIndexFireball, 10, 15,
                  TE_EXPLFLAG_NONE, 100, 0);

    Vector vecSize = Vector(500, 500, 60);
    CPVSFilter pvsFilter(GetAbsOrigin());

    te->BreakModel(pvsFilter, 0.0, GetAbsOrigin(), vec3_angle, vecSize,
...
void CNPC_Apache::TraceAttack(const CTakeDamageInfo &info, const Vector &vecDir,
                              trace_t *ptr, CDmgAccumulator *pAccumulator) {
  ...
  if (info.GetDamage() > 50 || ptr->hitgroup == 1 || ptr->hitgroup == 2 ||
      ptr->hitgroup == 3) {
    ...
    if (info.GetDamage() > 50) {
      if (m_hSmoke == NULL &&
          (m_hSmoke = SmokeTrail::CreateSmokeTrail()) != NULL) {
