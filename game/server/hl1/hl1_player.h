...
public:
  ...
  //	float		m_flNextAmmoBurn;	// while charging, when to
  //absorb another unit of player's ammo?

  CNetworkVar(float, m_flStartCharge);
  CNetworkVar(float, m_flAmmoStartCharge);
  CNetworkVar(float, m_flPlayAftershock);
  CNetworkVar(float, m_flNextAmmoBurn); // while charging, when to absorb
                                        // another unit of player's ammo?
