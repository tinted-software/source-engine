...
#define GARG_DAMAGE                                                            \
  (DMG_ENERGYBEAM | DMG_CRUSH | DMG_MISSILEDEFENSE | DMG_BLAST)
...
DEFINE_FIELD(m_pEyeGlow, FIELD_CLASSPTR),
    ...
    DEFINE_FIELD(m_flameX, FIELD_FLOAT), DEFINE_FIELD(m_flameY, FIELD_FLOAT),
  ...
  floor.z -= 1024;
  UTIL_TraceHull(*position, floor, mins, maxs, MASK_NPCSOLID, ignore,
...
public:
  ...
  void Spawn(void);
  void Think(void);
  static CStomp *StompCreate(Vector &origin, Vector &end, float speed,
                             CBaseEntity *pOwner);
...
CStomp *CStomp::StompCreate(Vector &origin, Vector &end, float speed,
                            CBaseEntity *pOwner) {
...
void CStomp::Think(void) {
  ...
  UTIL_TraceHull(vecStart, vecEnd, Vector(-32, -32, -32), Vector(32, 32, 32),
                 MASK_SOLID, m_pOwner, COLLISION_GROUP_NONE, &tr);
  ...
  while (gpGlobals->curtime - m_flDmgTime > STOMP_INTERVAL) {
    ...
    for (int i = 0; i < 2; i++) {
      CSprite *pSprite =
          CSprite::SpriteCreate(GARG_STOMP_SPRITE_NAME, GetAbsOrigin(), TRUE);
      if (pSprite) {
        UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() - Vector(0, 0, 500),
                       MASK_SOLID, this, COLLISION_GROUP_NONE, &tr);
        pSprite->SetAbsOrigin(tr.endpos);
        //				pSprite->pev->velocity =
        //Vector(RandomFloat(-200,200),RandomFloat(-200,200),175);
...
void CNPC_Gargantua::Spawn() {
  ...
  m_flFieldOfView =
      -0.2; // width of forward view cone ( as a dotproduct result )
  ...
  m_pEyeGlow =
      CSprite::SpriteCreate(GARG_EYE_SPRITE_NAME, GetAbsOrigin(), FALSE);
  m_pEyeGlow->SetTransparency(kRenderGlow, 255, 255, 255, 0,
...
Class_T CNPC_Gargantua::Classify(void) { return CLASS_ALIEN_MONSTER; }
...
// players head)
//=========================================================
CBaseEntity *CNPC_Gargantua::GargantuaCheckTraceHullAttack(float flDist,
                                                           int iDamage,
                                                           int iDmgType) {
  trace_t tr;
...
void CNPC_Gargantua::HandleAnimEvent(animevent_t *pEvent) {
  ...
  case GARG_AE_SLASH_LEFT: {
    ...
    CBaseEntity *pHurt = GargantuaCheckTraceHullAttack(
        GARG_ATTACKDIST + 10.0, sk_gargantua_dmg_slash.GetFloat(), DMG_SLASH);
...
bool CNPC_Gargantua::ShouldGib(const CTakeDamageInfo &info) { return false; }
...
void CNPC_Gargantua::RunTask(const Task_t *pTask) {
  ...
  case TASK_DIE:
    ...
    if (gpGlobals->curtime > m_flWaitFinished) {
      ...
      int parts =
          modelinfo->GetModelFrameCount(modelinfo->GetModel(gGargGibModel));
      ...
      for (i = 0; i < 10; i++) {
        ...
        pGib->SetAbsVelocity(UTIL_RandomBloodVector() *
                             random->RandomFloat(300, 500));
      ...
      te->BreakModel(filter, 0.0, GetAbsOrigin(), vec3_angle, vecSize,
                     vec3_origin, gGargGibModel, 200, 50, 3.0, BREAK_FLESH);
  ...
  case TASK_FLAME_SWEEP:
    if (gpGlobals->curtime > m_flWaitFinished) {
      ...
      // TEMP TEMP
      // FlameControls( angles.x + 2 * sin(gpGlobals->curtime*8), angles.y + 28
      // * sin(gpGlobals->curtime*8.5) );
...
void CNPC_Gargantua::FlameCreate(void) {
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (m_pFlame[i]) {
      ...
      UTIL_TraceLine(posGun, vecEnd, MASK_SOLID, this, COLLISION_GROUP_NONE,
                     &trace);
      //			NDebugOverlay::Line( posGun, vecEnd, 255, 255,
      //255, false, 10.0f );
...
void CNPC_Gargantua::FlameUpdate(void) {
  ...
  for (i = 0; i < 2; i++) {
    if (m_pFlame[i]) {
      ...
      Vector vecEnd =
          vecStart +
          (vForward * GARG_FLAME_LENGTH); //  - offset[i] * gpGlobals->v_right;
      ...
      // RadiusDamage( trace.vecEndPos, pev, pev, gSkillData.gargantuaDmgFire,
      // CLASS_ALIEN_MONSTER, DMG_BURN );
      FlameDamage(vecStart, trace.endpos, this, this,
                  sk_gargantua_dmg_fire.GetFloat(), CLASS_ALIEN_MONSTER,
                  DMG_BURN);
...
void CNPC_Gargantua::FlameDamage(Vector vecStart, Vector vecEnd,
                                 CBaseEntity *pevInflictor,
                                 CBaseEntity *pevAttacker, float flDamage,
  ...
  while ((pEntity = gEntList.FindEntityInSphere(pEntity, GetAbsOrigin(),
    ...
    if (pEntity->m_takedamage != DAMAGE_NO) {
      ...
      UTIL_TraceLine(vecStart, vecSpot, MASK_SOLID, this, COLLISION_GROUP_NONE,
                     &tr);
      //			NDebugOverlay::Line( vecStart, vecSpot, 0, 255,
      //0, false, 10.0f );
      ...
      if (tr.fraction == 1.0 ||
        ...
        if (tr.fraction != 1.0) {
          ...
          CTakeDamageInfo info(pevInflictor, pevAttacker, flAdjustedDamage,
                               bitsDamageType);
          CalculateMeleeDamageForce(&info, vDir, tr.endpos);
          pEntity->DispatchTraceAttack(info, vDir, &tr);
          ApplyMultiDamage();
        } else {
          pEntity->TakeDamage(CTakeDamageInfo(
              pevInflictor, pevAttacker, flAdjustedDamage, bitsDamageType));
        }
      }
    }
  }
...
void CNPC_Gargantua::EyeOn(int level) { m_eyeBrightness = level; }

void CNPC_Gargantua::EyeOff(void) { m_eyeBrightness = 0; }

void CNPC_Gargantua::EyeUpdate(void) {
  if (m_pEyeGlow) {
    m_pEyeGlow->SetBrightness(
        UTIL_Approach(m_eyeBrightness, m_pEyeGlow->GetBrightness(), 26), 0.5f);
...
void CNPC_Gargantua::StompAttack(void) {
  ...
  UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() - Vector(0, 0, 20), MASK_SOLID,
...
void CNPC_Gargantua::DeathEffect(void) {
  ...
  for (i = 0; i < 7; i++) {
    te->Explosion(filter, i * 0.2, &GetAbsOrigin(), g_sModelIndexFireball, 10,
                  15, TE_EXPLFLAG_NONE, 100, 0);
    position.z += 15;
  }
...
void CNPC_Gargantua::TraceAttack(const CTakeDamageInfo &info,
...
int CNPC_Gargantua::OnTakeDamage_Alive(const CTakeDamageInfo &info) {
  if (GetState() == NPC_STATE_SCRIPT) {
    ...
    // explode in C2A1 because for some reason he wouldn't die while scripted,
    // he'd only freeze in place. Now he's just immune until he gets to the
    // script and stops.
    return 0;
  }
...
DEFINE_SCHEDULE(
    ...
    "SCHEDULE:SCHED_GARG_CHASE_ENEMY_FAILED"
