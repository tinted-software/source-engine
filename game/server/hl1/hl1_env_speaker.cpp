...
void CSpeaker::SpeakerThink(void) {
  ...
  if (!g_AIFriendliesTalkSemaphore.IsAvailable(this) ||
    ...
    float releaseTime = MAX(g_AIFriendliesTalkSemaphore.GetReleaseTime(),
  ...
  if (szSoundFile[0] == '!') {
    ...
    if (SENTENCEG_PlayRndSz(edict(), szSoundFile, flvolume, SNDLVL_120dB, flags,
                            pitch) < 0)
    ...
    SetNextThink(gpGlobals->curtime +
                 random->RandomFloat(ANNOUNCE_MINUTES_MIN * 60.0,
                                     ANNOUNCE_MINUTES_MAX * 60.0));

    // time delay until it's ok to speak: used so that two NPCs don't talk at
    // once
...
//
// ToggleUse - if an announcement is pending, cancel it.  If no announcement is
// pending, start one.
//
void CSpeaker::ToggleUse(CBaseEntity *pActivator, CBaseEntity *pCaller,
                         USE_TYPE useType, float value) {
