...
// houndeye does 20 points of damage spread over a sphere 384 units in diameter,
// and each additional squad member increases the BASE damage by 110%, per the
// spec.
...
#define HOUNDEYE_SOUND_STARTLE_VOLUME                                          \
  128 // how loud a sound has to be to badly scare a sleeping houndeye
...
void CNPC_Houndeye::SonicAttack(void) {
  ...
  while ((pEntity = gEntList.FindEntityInSphere(
              pEntity, GetAbsOrigin(), HOUNDEYE_MAX_ATTACK_RADIUS)) != NULL) {
    if (pEntity->m_takedamage != DAMAGE_NO) {
      if (!FClassnameIs(pEntity,
                        "monster_houndeye")) { // houndeyes don't hurt other
                                               // houndeyes with their attack

        // houndeyes do FULL damage if the ent in question is visible. Half
        // damage otherwise. This means that you must get out of the houndeye's
        // attack range entirely to avoid damage. Calculate full damage first
        ...
        if (m_pSquad && m_pSquad->NumMembers() > 1) {
          ...
          flAdjustedDamage =
              ...
              sk_houndeye_dmg_blast.GetFloat() *
                  (HOUNDEYE_SQUAD_BONUS * (m_pSquad->NumMembers() - 1));
        } else {
          // solo
        ...
        flAdjustedDamage -=
            (flDist / HOUNDEYE_MAX_ATTACK_RADIUS) * flAdjustedDamage;

        if (!FVisible(pEntity)) {
          if (pEntity->IsPlayer()) {
            // if this entity is a client, and is not in full view, inflict half
            // damage. We do this so that players still take the residual damage
            // if they don't totally leave the houndeye's effective radius. We
            // restrict it to clients so that monsters in other parts of the
            // level don't take the damage and get pissed.
            flAdjustedDamage *= 0.5;
          } else if (!FClassnameIs(pEntity, "func_breakable") &&
                     !FClassnameIs(pEntity, "func_pushable")) {
            // do not hurt nonclients through walls, but allow damage to be done
            // to breakables
            flAdjustedDamage = 0;
          }
        }
        ...
        if (flAdjustedDamage > 0) {
          CTakeDamageInfo info(this, this, flAdjustedDamage,
                               DMG_SONIC | DMG_ALWAYSGIB);
          CalculateExplosiveDamageForce(
              &info, (pEntity->GetAbsOrigin() - GetAbsOrigin()),
              pEntity->GetAbsOrigin());

          pEntity->TakeDamage(info);

          if ((pEntity->GetAbsOrigin() - GetAbsOrigin()).Length2D() <=
...
bool CNPC_Houndeye::ShouldGoToIdleState(void) {
  if (m_pSquad) {
    AISquadIter_t iter;
    for (CAI_BaseNPC *pMember = m_pSquad->GetFirstMember(&iter); pMember;
         pMember = m_pSquad->GetNextMember(&iter)) {
      if (pMember != this && pMember->GetHintNode() &&
          pMember->GetHintNode()->HintType() != NO_NODE)
        return true;
    }
...
void CNPC_Houndeye::SetActivity(Activity NewActivity) {
  ...
  if (m_NPCState == NPC_STATE_COMBAT && NewActivity == ACT_IDLE &&
      random->RandomInt(0, 1)) {
    // play pissed idle.
    iSequence = LookupSequence("madidle");

    SetActivity(NewActivity); // Go ahead and set this so it doesn't keep trying
                              // when the anim is not present
    ...
    if (iSequence > ACTIVITY_NOT_AVAILABLE) {
      ...
      SetCycle(0); // FIX: frame counter shouldn't be reset when its the same
                   // activity as before
      ResetSequenceInfo();
    }
  } else {
...
void CNPC_Houndeye::StartTask(const Task_t *pTask) {
  ...
  case TASK_HOUND_FALL_ASLEEP: {
    m_fAsleep = TRUE; // signal that hound is lying down (must stand again
  ...
  case TASK_HOUND_OPEN_EYE: {
    m_fDontBlink = FALSE; // turn blinking back on and that code will
                          // automatically open the eye
    TaskComplete();
    break;
  }
...
void CNPC_Houndeye::PrescheduleThink(void) {
  ...
  if (m_NPCState == NPC_STATE_COMBAT && GetActivity() == ACT_RUN &&
      random->RandomFloat(0, 1) < 0.2) {
    WarnSound();
  }

  // at random, initiate a blink if not already blinking or sleeping
  if (!m_fDontBlink) {
    if ((m_nSkin == 0) && random->RandomInt(0, 0x7F) == 0) { // start blinking!
      m_nSkin = HOUNDEYE_EYE_FRAMES - 1;
    } else if (m_nSkin != 0) { // already blinking
      m_nSkin--;
    }
  }

  // if you are the leader, average the origins of each pack member to get an
  // approximate center.
  if (m_pSquad && m_pSquad->IsLeader(this)) {
    int iSquadCount = 0;
...
int CNPC_Houndeye::TranslateSchedule(int scheduleType) {
  if (m_fAsleep) {
    ...
    if (HasCondition(COND_HEAR_DANGER) || HasCondition(COND_HEAR_THUMPER) ||
  ...
  case SCHED_IDLE_STAND: {
    ...
    if (m_pSquad && !m_pSquad->IsLeader(this) && !m_fAsleep &&
        random->RandomInt(0, 29) < 1) {
  ...
  case NPC_STATE_COMBAT: {
    ...
    if (HasCondition(COND_LIGHT_DAMAGE) || HasCondition(COND_HEAVY_DAMAGE)) {
      if (random->RandomFloat(0, 1) <= 0.4) {
        ...
        Vector v_forward;
        GetVectors(&v_forward, NULL, NULL);
        UTIL_TraceEntity(this, GetAbsOrigin(),
                         GetAbsOrigin() + v_forward * -128, MASK_SOLID, &trace);
    ...
    if (HasCondition(COND_CAN_RANGE_ATTACK1)) {
      // don't constraint attacks based on squad slots, just let em all go for
      // it
...
float CNPC_Houndeye::FLSoundVolume(CSound *pSound) {
  return (pSound->Volume() -
          ((pSound->GetSoundOrigin() - GetAbsOrigin()).Length()));
}
