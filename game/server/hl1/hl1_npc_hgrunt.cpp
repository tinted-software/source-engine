...
int g_fGruntQuestion; // true if an idle grunt asked a question. Cleared when
                      // someone answers.
int g_iSquadIndex = 0;
...
//=========================================================
#define GRUNT_CLIP_SIZE                                                        \
  36 // how many bullets in a clip? - NOTE: 3 round burst sound, so keep as 3 *
     // x!
#define GRUNT_VOL 0.35           // volume of grunt sounds
#define GRUNT_SNDLVL SNDLVL_NORM // soundlevel of grunt sentences
#define HGRUNT_LIMP_HEALTH 20
#define HGRUNT_DMG_HEADSHOT                                                    \
  (DMG_BULLET |                                                                \
   DMG_CLUB) // damage types that can kill a grunt with a single headshot.
#define HGRUNT_NUM_HEADS 2 // how many grunt heads are there?
#define HGRUNT_MINIMUM_HEADSHOT_DAMAGE                                         \
  15 // must do at least this much damage in one shot to head to score a
     // headshot kill
...
DEFINE_FIELD(m_flNextGrenadeCheck, FIELD_TIME),
  ...
  m_fFirstEncounter = true; // this is true when the grunt spawns, because he
...
void CNPC_HGrunt::SpeakSentence(void) {
  ...
  if (FOkToSpeak()) {
    SENTENCEG_PlayRndSz(edict(), pGruntSentences[m_iSentence],
                        HGRUNT_SENTENCE_VOLUME, GRUNT_SNDLVL, 0, m_voicePitch);
    JustSpoke();
  }
}
...
void CNPC_HGrunt::PrescheduleThink(void) {
  ...
  if (m_pSquad && GetEnemy() != NULL) {
    ...
    CNPC_HGrunt *pSquadLeader =
        (CNPC_HGrunt *)m_pSquad->GetLeader()->MyNPCPointer();
...
Class_T CNPC_HGrunt::Classify(void) { return CLASS_HUMAN_MILITARY; }
...
int CNPC_HGrunt::SquadRecruit(int searchRadius, int maxMembers) {
  ...
  if (m_SquadName != NULL_STRING) {
    // I have a netname, so unconditionally recruit everyone else with that
    // name.
    ...
    while ((pEntity = gEntList.FindEntityInSphere(pEntity, GetAbsOrigin(),
                                                  searchRadius)) != NULL) {
      ...
      if (pRecruit && pRecruit != this && pRecruit->IsAlive() &&
          !pRecruit->m_hCine) {
        // Can we recruit this guy?
        if (!pRecruit->m_pSquad && pRecruit->Classify() == iMyClass &&
            ((iMyClass != CLASS_ALIEN_MONSTER) ||
             FClassnameIs(this, pRecruit->GetClassname())) &&
            !pRecruit->m_SquadName) {
          trace_t tr;
          UTIL_TraceLine(GetAbsOrigin() + GetViewOffset(),
                         pRecruit->GetAbsOrigin() + GetViewOffset(),
                         MASK_NPCSOLID_BRUSHONLY, pRecruit,
                         COLLISION_GROUP_NONE,
                         &tr); // try to hit recruit with a traceline.

          if (tr.fraction == 1.0) {
            // We're ready to recruit people, so start a squad if I don't have
            // one.
            if (!m_pSquad) {
              InitSquad();
            }
...
int CNPC_HGrunt::RangeAttack1Conditions(float flDot, float flDist) {
  if (!HasCondition(COND_ENEMY_OCCLUDED) && flDist <= 2048 && flDot >= 0.5 &&
    ...
    // NDebugOverlay::Line( GetAbsOrigin() + GetViewOffset(),
    // GetEnemy()->BodyTarget(GetAbsOrigin() + GetViewOffset()), 255, 0, 0,
    // false, 0.1 );
    //  verify that a bullet fired from the gun will hit the enemy before the
    //  world.
    UTIL_TraceLine(GetAbsOrigin() + GetViewOffset(),
                   GetEnemy()->BodyTarget(GetAbsOrigin() + GetViewOffset()),
...
int CNPC_HGrunt::GetGrenadeConditions(float flDot, float flDist) {
  ...
  if (FBitSet(m_iWeapons, HGRUNT_HANDGRENADE)) {
    ...
    if (random->RandomInt(0, 1)) {
      ...
      pEnemy->CollisionProp()->NormalizedToWorldSpace(Vector(0.5f, 0.5f, 0.0f),
                                                      &vecTarget);
    } else {
    ...
    vecTarget =
        ...
        (GetEnemy()->BodyTarget(GetAbsOrigin()) - GetEnemy()->GetAbsOrigin());
    // estimate position
    if (HasCondition(COND_SEE_ENEMY)) {
      vecTarget = vecTarget + ((vecTarget - GetAbsOrigin()).Length() /
                               sk_hgrunt_gspeed.GetFloat()) *
  ...
  if (FBitSet(m_iWeapons, HGRUNT_HANDGRENADE)) {
    ...
    Vector vecToss = VecCheckThrow(this, vGunPos, vecTarget,
                                   sk_hgrunt_gspeed.GetFloat(), 0.5);
...
//=========================================================
void CNPC_HGrunt::TraceAttack(const CTakeDamageInfo &inputInfo,
  ...
  if (ptr->hitgroup == 11) {
    ...
    if (GetBodygroup(1) == HEAD_GRUNT &&
        (info.GetDamageType() &
         (DMG_BULLET | DMG_SLASH | DMG_BLAST | DMG_CLUB))) {
      // absorb damage
...
void CNPC_HGrunt::IdleSound(void) {
  if (FOkToSpeak() && (g_fGruntQuestion || random->RandomInt(0, 1))) {
    if (!g_fGruntQuestion) {
      ...
      case 0: // check in
        SENTENCEG_PlayRndSz(edict(), "HG_CHECK", HGRUNT_SENTENCE_VOLUME,
                            SNDLVL_NORM, 0, m_voicePitch);
        g_fGruntQuestion = 1;
        break;
      case 1: // question
        SENTENCEG_PlayRndSz(edict(), "HG_QUEST", HGRUNT_SENTENCE_VOLUME,
                            SNDLVL_NORM, 0, m_voicePitch);
        g_fGruntQuestion = 2;
        break;
      case 2: // statement
        SENTENCEG_PlayRndSz(edict(), "HG_IDLE", HGRUNT_SENTENCE_VOLUME,
                            SNDLVL_NORM, 0, m_voicePitch);
        break;
      }
    } else {
      ...
      case 1: // check in
        SENTENCEG_PlayRndSz(edict(), "HG_CLEAR", HGRUNT_SENTENCE_VOLUME,
                            SNDLVL_NORM, 0, m_voicePitch);
        break;
      case 2: // question
        SENTENCEG_PlayRndSz(edict(), "HG_ANSWER", HGRUNT_SENTENCE_VOLUME,
                            SNDLVL_NORM, 0, m_voicePitch);
        break;
      }
      g_fGruntQuestion = 0;
...
bool CNPC_HGrunt::HandleInteraction(int interactionType, void *data,
...
CBaseEntity *CNPC_HGrunt::Kick(void) {
  ...
  UTIL_TraceHull(vecStart, vecEnd, Vector(-16, -16, -18), Vector(16, 16, 18),
...
void CNPC_HGrunt::HandleAnimEvent(animevent_t *pEvent) {
  ...
  case HGRUNT_AE_GREN_TOSS: {
    CHandGrenade *pGrenade = (CHandGrenade *)Create(
        "grenade_hand", GetAbsOrigin() + Vector(0, 0, 60), vec3_angle);
    if (pGrenade) {
    ...
    m_flNextGrenadeCheck =
        gpGlobals->curtime + 6; // wait six seconds before even looking again to
                                // see if a grenade can be thrown.
  ...
  case HGRUNT_AE_GREN_LAUNCH: {
    ...
    CGrenadeMP5 *m_pMyGrenade =
        (CGrenadeMP5 *)Create("grenade_mp5", vecSrc, angAngles, this);
    m_pMyGrenade->SetAbsVelocity(m_vecTossVelocity);
    m_pMyGrenade->SetLocalAngularVelocity(
        QAngle(random->RandomFloat(-100, -500), 0, 0));
    ...
    if (g_iSkillLevel == SKILL_HARD)
      ...
      m_flNextGrenadeCheck =
          gpGlobals->curtime + 6; // wait six seconds before even looking again
                                  // to see if a grenade can be thrown.
  ...
  case HGRUNT_AE_GREN_DROP: {
    CHandGrenade *pGrenade = (CHandGrenade *)Create(
        "grenade_hand", Weapon_ShootPosition(), vec3_angle);
    if (pGrenade) {
      pGrenade->ShootTimed(this, m_vecTossVelocity, 3.5);
    }
  ...
  case HGRUNT_AE_BURST1: {
    if (FBitSet(m_iWeapons, HGRUNT_9MMAR)) {
      ...
      // the first round of the three round burst plays the sound and puts a
      // sound in the world sound list.
  ...
  case HGRUNT_AE_CAUGHT_ENEMY: {
    if (FOkToSpeak()) {
      SENTENCEG_PlayRndSz(edict(), "HG_ALERT", HGRUNT_SENTENCE_VOLUME,
                          GRUNT_SNDLVL, 0, m_voicePitch);
      JustSpoke();
    }
  }
...
void CNPC_HGrunt::SetAim(const Vector &aimDir) {
  ...
  float newPitch =
      curPitch +
      UTIL_AngleDiff(UTIL_ApproachAngle(angDir.x, curPitch, 60), curPitch);
...
void CNPC_HGrunt::Shoot(void) {
  ...
  Vector vecShellVelocity = right * random->RandomFloat(40, 90) +
                            up * random->RandomFloat(75, 200) +
                            forward * random->RandomFloat(-40, 40);
  EjectShell(vecShootOrigin - vecShootDir * 24, vecShellVelocity,
             GetAbsAngles().y, 0);
  FireBullets(1, vecShootOrigin, vecShootDir, VECTOR_CONE_10DEGREES, 2048,
              m_iAmmoType); // shoot +-5 degrees
...
void CNPC_HGrunt::Shotgun(void) {
  ...
  EjectShell(vecShootOrigin - vecShootDir * 24, vecShellVelocity,
             GetAbsAngles().y, 1);
  FireBullets(sk_hgrunt_pellets.GetFloat(), vecShootOrigin, vecShootDir,
              VECTOR_CONE_15DEGREES, 2048, m_iAmmoType,
              0); // shoot +-7.5 degrees
...
void CNPC_HGrunt::RunTask(const Task_t *pTask) {
  ...
  case TASK_GRUNT_FACE_TOSS_DIR: {
    ...
    GetMotor()->SetIdealYawToTargetAndUpdate(
        GetAbsOrigin() + m_vecTossVelocity * 64, AI_KEEP_YAW_SPEED);
...
int CNPC_HGrunt::SelectSchedule(void) {
  ...
  if (HasCondition(COND_HEAR_DANGER)) {
    ...
    //!!!KELLY - currently, this is the grunt's signal that a grenade has landed
    //!nearby,
    ...
    if (FOkToSpeak()) {
      SENTENCEG_PlayRndSz(edict(), "HG_GREN", HGRUNT_SENTENCE_VOLUME,
                          GRUNT_SNDLVL, 0, m_voicePitch);
      JustSpoke();
    }
  ...
  case NPC_STATE_COMBAT: {
    ...
    if (HasCondition(COND_NEW_ENEMY)) {
      if (m_pSquad) {
        if (!m_pSquad->IsLeader(this)) {
          ...
          //!!!KELLY - the leader of a squad of grunts has just seen the player
          //!or a
          // monster and has made it the squad's enemy. You
          // can check pev->flags for FL_CLIENT to determine whether this is the
          // player or a monster. He's going to immediately start firing,
          // though. If you'd like, we can make an alternate "first sight"
          // schedule where the leader plays a handsign anim
          // that gives us enough time to hear a short sentence or spoken
          // command before he starts pluggin away.
          if (FOkToSpeak()) // && RANDOM_LONG(0,1))
          {
            if ((GetEnemy() != NULL) && GetEnemy()->IsPlayer())
              ...
              // monster
              SENTENCEG_PlayRndSz(edict(), "HG_MONST", HGRUNT_SENTENCE_VOLUME,
                                  GRUNT_SNDLVL, 0, m_voicePitch);
    ...
    // damaged just a little
      ...
      if (iPercent <= 90 && GetEnemy() != NULL) {
        ...
        if (FOkToSpeak()) // && RANDOM_LONG(0,1))
        {
          // SENTENCEG_PlayRndSz( ENT(pev), "HG_COVER", HGRUNT_SENTENCE_VOLUME,
          // GRUNT_SNDLVL, 0, m_voicePitch);
    ...
    // can grenade launch
             ...
             OccupyStrategySlotRange(SQUAD_SLOT_GRENADE1,
                                     SQUAD_SLOT_GRENADE2)) {
    ...
    // can shoot
      ...
      if (m_pSquad) {
        if (m_pSquad->GetLeader() != NULL) {
          ...
          // if the enemy has eluded the squad and a squad member has just
          // located the enemy and the enemy does not see the squad member,
          // issue a call to the squad to waste a little time and give the
          // player a chance to turn.
          if (pSquadLeader && pSquadLeader->EnemyHasEludedMe() &&
              !HasCondition(COND_ENEMY_FACING_ME)) {
    ...
    // can't see enemy
      ...
      if (HasCondition(COND_CAN_RANGE_ATTACK2) &&
        ...
        //!!!KELLY - this grunt is about to throw or fire a grenade at the
        //!player. Great place for "fire in the hole"  "frag out" etc
        if (FOkToSpeak()) {
          SENTENCEG_PlayRndSz(edict(), "HG_THROW", HGRUNT_SENTENCE_VOLUME,
                              GRUNT_SNDLVL, 0, m_voicePitch);
          JustSpoke();
        }
        return SCHED_RANGE_ATTACK2;
      } else if (OccupyStrategySlotRange(SQUAD_SLOT_ENGAGE1,
                                         SQUAD_SLOT_ENGAGE2)) {
        //!!!KELLY - grunt cannot see the enemy and has just decided to
        // charge the enemy's position.
        if (FOkToSpeak()) // && RANDOM_LONG(0,1))
        {
          // SENTENCEG_PlayRndSz( ENT(pev), "HG_CHARGE", HGRUNT_SENTENCE_VOLUME,
          // GRUNT_SNDLVL, 0, m_voicePitch);
        ...
        if (FOkToSpeak() && random->RandomInt(0, 1)) {
          SENTENCEG_PlayRndSz(edict(), "HG_TAUNT", HGRUNT_SENTENCE_VOLUME,
                              GRUNT_SNDLVL, 0, m_voicePitch);
          JustSpoke();
        }
...
int CNPC_HGrunt::TranslateSchedule(int scheduleType) {
  ...
  case SCHED_GRUNT_SUPPRESS: {
    if (GetEnemy()->IsPlayer() && m_fFirstEncounter) {
      m_fFirstEncounter = FALSE; // after first encounter, leader won't issue
                                 // handsigns anymore when he has a new enemy
  ...
  case SCHED_FAIL: {
    if (GetEnemy() != NULL) {
      // grunt has an enemy, so pick a different default fail schedule most
      // likely to help recover.
...
public:
  void Spawn(void);
  void Precache(void);
  void RepelUse(CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType,
                float value);
...
void CNPC_HGruntRepel::RepelUse(CBaseEntity *pActivator, CBaseEntity *pCaller,
  ...
  CBaseEntity *pEntity =
      Create("monster_human_grunt", GetAbsOrigin(), GetAbsAngles());
  ...
  SetNextThink(gpGlobals->curtime +
               -4096.0 * tr.fraction / pGrunt->GetAbsVelocity().z + 0.5);
...
DEFINE_SCHEDULE(
    ...
    "		TASK_STOP_MOVING			0"
    "		TASK_SET_FAIL_SCHEDULE		"
    "SCHEDULE:SCHED_GRUNT_TAKE_COVER_FAILED"
    "		TASK_WAIT					0.2"
    ...
    "		TASK_SET_SCHEDULE			"
    "SCHEDULE:SCHED_GRUNT_WAIT_IN_COVER"
    "	"
    "	Interrupts")
...
//=========================================================
// > SCHED_GRUNT_RANGE_ATTACK1A
// primary range attack. Overriden because base class stops attacking when the
// enemy is occluded. grunt's grenade toss requires the enemy be occluded.
//=========================================================
DEFINE_SCHEDULE(
    SCHED_GRUNT_RANGE_ATTACK1A,
...
//=========================================================
// > SCHED_GRUNT_RANGE_ATTACK2
// secondary range attack. Overriden because base class stops attacking when the
// enemy is occluded. grunt's grenade toss requires the enemy be occluded.
//=========================================================
DEFINE_SCHEDULE(SCHED_GRUNT_RANGE_ATTACK2,
                ...
                "		TASK_SET_SCHEDULE			"
                "SCHEDULE:SCHED_GRUNT_WAIT_IN_COVER" // don't run immediately
...
DEFINE_SCHEDULE(SCHED_GRUNT_BARNACLE_HIT,
                ...
                "		TASK_SET_SCHEDULE			"
                "SCHEDULE:SCHED_GRUNT_BARNACLE_PULL"
                ""
                "	Interrupts")
...
DEFINE_SCHEDULE(SCHED_GRUNT_BARNACLE_CHOMP,
                ...
                "		 TASK_SET_SCHEDULE			"
                "SCHEDULE:SCHED_GRUNT_BARNACLE_CHEW"
                ""
                "	Interrupts")
