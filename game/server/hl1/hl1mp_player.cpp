...
void TE_PlayerAnimEvent(CBasePlayer *pPlayer, PlayerAnimEvent_t event,
  ...
  //	if ( !ToolsEnabled() && ( cl_showanimstate.GetInt() ==
  //pPlayer->entindex() ) )
  {
...
SendPropExclude("DT_BaseAnimating", "m_flPoseParameter"),
    ...
    //	SendPropDataTable( SENDINFO_DT( m_Shared ), &REFERENCE_SEND_TABLE(
    //DT_TFCPlayerShared ) )
    END_SEND_TABLE()
...
CHL1MP_Player::CHL1MP_Player() {
  ...
  //	SetContextThink( &CTFCPlayer::TFCPlayerThink, gpGlobals->curtime,
  //"TFCPlayerThink" );
}
...
void CHL1MP_Player::SetAnimation(PLAYER_ANIM playerAnim) {
  ...
  if (playerAnim == PLAYER_JUMP) {
    ...
    if (!(GetFlags() & FL_ONGROUND) &&
        (GetActivity() == ACT_HOP ||
         GetActivity() == ACT_LEAP)) // Still jumping
    {
      idealActivity = GetActivity();
...
static ConVar sv_debugweaponpickup(
    "sv_debugweaponpickup", "0", FCVAR_CHEAT,
...
void CHL1MP_Player::SetPlayerTeamModel(void) {
  ...
  Q_snprintf(szModelName, 256, "%s%s/%s.mdl", s_szModelPath, pTeam->GetName(),
             pTeam->GetName());
...
void CHL1MP_Player::SetPlayerModel(void) {
  char szBaseName[128];
  Q_FileBase(engine->GetClientConVarValue(engine->IndexOfEdict(edict()),
                                          "cl_playermodel"),
             szBaseName, 128);
  ...
  char szModelName[256];
  Q_snprintf(szModelName, 256, "%s%s/%s.mdl", s_szModelPath, szBaseName,
             szBaseName);
...
SendPropVector(SENDINFO(m_vecRagdollOrigin), -1, SPROP_COORD),
  ...
  if (!pRagdoll) {
    // Create a new one
    pRagdoll =
        dynamic_cast<CHL1MPRagdoll *>(CreateEntityByName("hl1mp_ragdoll"));
  }
