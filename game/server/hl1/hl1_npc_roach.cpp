...
public:
  ...
  // UNDONE: These don't necessarily need to be save/restored, but if we add
  // more data, it may
  bool m_fLightHacked;
  int m_iMode;
...
void CNPC_Roach::Spawn() {
  ...
  SetViewOffset(
      Vector(0, 0, 1)); // position of the eyes relative to monster's origin.
...
void CNPC_Roach::NPCThink(void) {
  ...
  if (!m_fLightHacked) {
    ...
    // we'll collect the light level.
    SetNextThink(gpGlobals->curtime + 1);
    m_fLightHacked = TRUE;
    return;
  } else if (m_flLastLightLevel < 0) {
    // collect light level for the first time, now that all of the lightmaps in
    // the roach's area have been calculated.
  ...
  case ROACH_IDLE:
  case ROACH_EAT: {
    // if not moving, sample environment to see if anything scary is around. Do
    // a radius search 'look' at random.
    if (random->RandomInt(0, 3) == 1) {
      ...
      if (HasCondition(COND_SEE_FEAR)) {
        ...
        Eat(30 + (random->RandomInt(
                     0, 14))); // roach will ignore food for 30 to 45 seconds
        PickNewDest(ROACH_SCARED_BY_ENT);
        SetActivity(ACT_WALK);
      } else if (random->RandomInt(0, 149) == 1) {
        // if roach doesn't see anything, there's still a chance that it will
        // move. (boredom)
        ...
        if (m_iMode == ROACH_EAT) {
          // roach will ignore food for 30 to 45 seconds if it got bored while
          // eating.
    ...
    if (m_iMode == ROACH_IDLE) {
      ...
      if (0 > m_flLastLightLevel) {
        ...
        // roach smells food and is just standing around. Go to food unless food
        // isn't on same z-plane.
  ...
  case ROACH_SCARED_BY_LIGHT: {
    // if roach was scared by light, then stop if we're over a spot at least as
    // dark as where we started!
    if (0 <= m_flLastLightLevel) {
      SetActivity(ACT_IDLE);
...
void CNPC_Roach::PickNewDest(int iCondition) {
  ...
  if (random->RandomInt(0, 9) == 1) {
    // every once in a while, a roach will play a skitter sound when they decide
    // to run
...
void CNPC_Roach::Look(int iDistance) {
  CBaseEntity *pSightEnt =
      NULL; // the current visible entity that we're dealing with

  // DON'T let visibility information from last frame sit around!
  ClearCondition(COND_SEE_HATE | COND_SEE_DISLIKE | COND_SEE_ENEMY |
                 COND_SEE_FEAR);

  // don't let monsters outside of the player's PVS act up, or most of the
  // interesting things will happen before the player gets there!
  if (FNullEnt(UTIL_FindClientInPVS(edict()))) {
    return;
  }
  ...
  // !!!PERFORMANCE - let's trivially reject the ent list before radius
  // searching!

  for (CEntitySphereQuery sphere(GetAbsOrigin(), iDistance);
       (pSightEnt = sphere.GetCurrentEntity()) != NULL; sphere.NextEntity()) {
    // only consider ents that can be damaged. !!!temporarily only considering
    // other monsters and clients
    if (pSightEnt->IsPlayer() || FBitSet(pSightEnt->GetFlags(), FL_NPC)) {
      if (/*FVisible( pSightEnt ) &&*/ !FBitSet(pSightEnt->GetFlags(),
                                                FL_NOTARGET) &&
          pSightEnt->m_iHealth > 0) {
        // don't add the Enemy's relationship to the conditions. We only want to
        // worry about conditions when we see monsters other than the Enemy.
...
void CNPC_Roach::Move(float flInterval) {
  ...
  // if the waypoint is closer than step size, then stop after next step (ok for
  // roach to overshoot)
  ...
  if (random->RandomInt(0, 149) == 1 && m_iMode != ROACH_SCARED_BY_LIGHT &&
      m_iMode != ROACH_SMELL_FOOD) {
    // random skitter while moving as long as not on a b-line to get out of
    // light or going to food
    PickNewDest(FALSE);
  }
}
...
void CNPC_Roach::Touch(CBaseEntity *pOther) {
  ...
  // UTIL_TraceLine ( vecSpot, vecSpot + Vector ( 0, 0, -24 ),  ignore_monsters,
  // ENT(pev), & tr);

  UTIL_TraceLine(vecSpot, vecSpot + Vector(0, 0, -24), MASK_ALL, this,
                 COLLISION_GROUP_NONE, &tr);

  // This isn't really blood.  So you don't have to screen it out based on
  // violence levels (UTIL_ShouldShowBlood())
