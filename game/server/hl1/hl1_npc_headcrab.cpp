  ...
  /*	if (m_NPCState == NPC_STATE_DEAD)
          {
                  if (GetAbsVelocity().Length() > 250)
                  {
                          ...
                          UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + vecDir
     * 100, MASK_SOLID_BRUSHONLY, pev, COLLISION_GROUP_NONE, &tr); float dotPr =
     DotProduct(vecDir,tr.plane.normal); if ((tr.fraction
     != 1.0) && (dotPr <  -0.8) )
                          {
                                  Event_Gibbed();
                                  // Throw headcrab guts
                                  CGib::SpawnSpecificGibs( this,
     HEADCRAB_GUTS_GIB_COUNT, 300, 400, "models/gibs/hc_gibs.mdl");
                          }
...
//-----------------------------------------------------------------------------
// Purpose: Returns the real center of the monster. The bounding box is much
// larger
//			than the actual creature so this is needed for
//targetting.
// Output : Vector
//-----------------------------------------------------------------------------
...
void CNPC_Headcrab::HandleAnimEvent(animevent_t *pEvent) {
  ...
  case HC_AE_JUMPATTACK: {
    ...
    if (pEnemy) {
      ...
      //
      // How fast does the headcrab need to travel to reach my enemy's eyes
      // given gravity?
      //
...
void CNPC_BabyCrab::Spawn(void) {
  ...
  m_iHealth =
      sk_headcrab_health.GetFloat() * 0.25; // less health than full grown
}
...
int CNPC_BabyCrab::RangeAttack1Conditions(float flDot, float flDist) {
  if (GetFlags() & FL_ONGROUND) {
    if (GetGroundEntity() &&
        (GetGroundEntity()->GetFlags() & (FL_CLIENT | FL_NPC)))
      return COND_CAN_RANGE_ATTACK1;
