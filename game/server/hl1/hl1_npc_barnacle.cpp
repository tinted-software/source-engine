...
DEFINE_FIELD(m_flAltitude, FIELD_FLOAT),
    ...
    DEFINE_FIELD(m_cGibs, FIELD_INTEGER), // barnacle loads up on gibs each time
                                          // it kills something.
...
void CNPC_Barnacle::BarnacleThink(void) {
  ...
  if (CAI_BaseNPC::m_nDebugBits & bits_debugDisableAI) {
    ...
    if (m_fLiftingPrey) {
      ...
      vecNewEnemyOrigin.x -=
          6 * cos(GetEnemy()->GetLocalAngles().y * M_PI / 180.0);
      vecNewEnemyOrigin.y -=
          6 * sin(GetEnemy()->GetLocalAngles().y * M_PI / 180.0);

      m_flAltitude -= BARNACLE_PULL_SPEED;
      vecNewEnemyOrigin.z += BARNACLE_PULL_SPEED;

      if (fabs(GetLocalOrigin().z -
               (vecNewEnemyOrigin.z + GetEnemy()->GetViewOffset().z)) <
          BARNACLE_BODY_HEIGHT) {
        // prey has just been lifted into position ( if the victim origin + eye
        // height + 8 is higher than the bottom of the barnacle, it is assumed
        // that the head is within barnacle's body )
        m_fLiftingPrey = FALSE;
        ...
        if (pVictim) {
          pVictim->DispatchInteraction(g_interactionBarnacleVictimDangle, NULL,
                                       this);
      ...
      trace_t trace;
      UTIL_TraceEntity(pEnemy, pEnemy->GetAbsOrigin(), vecNewEnemyOrigin,
                       MASK_SOLID_BRUSHONLY, pEnemy, COLLISION_GROUP_NONE,
                       &trace);

      if (trace.fraction != 1.0) {
        // The victim cannot be moved from their current origin to this new
        // origin. So drop them.
        SetEnemy(NULL);
        m_fLiftingPrey = FALSE;

        if (pEnemy->MyCombatCharacterPointer()) {
          pEnemy->MyCombatCharacterPointer()->DispatchInteraction(
              g_interactionBarnacleVictimReleased, NULL, this);
        }
      ...
      if (m_flKillVictimTime != -1 && gpGlobals->curtime > m_flKillVictimTime) {
        ...
        if (pVictim) {
          ...
          pVictim->TakeDamage(
              CTakeDamageInfo(this, this, pVictim->m_iHealth,
                              DMG_SLASH | DMG_ALWAYSGIB | DMG_CRUSH));
          m_cGibs = 3;
        }
    ...
    // If idle and no nearby client, don't think so often. Client should be out
    // of PVS and not within 50 feet.
    ...
    // NDebugOverlay::Box( GetAbsOrigin() - Vector( 0, 0, flLength ), Vector(
    // -2, -2, -2 ), Vector( 2, 2, 2 ), 255,0,0, 0, 0.1 );
    ...
    if (pTouchEnt != NULL) {
      ...
      if (pBCC && pBCC->DispatchInteraction(g_interactionBarnacleVictimGrab,
                                            &vecGrabPos, this)) {
        CPASAttenuationFilter filter(this);
        ...
        m_flKillVictimTime =
            -1; // set this to a bogus time while the victim is lifted.

        m_flAltitude = (GetAbsOrigin().z - vecGrabPos.z);
      }
    } else {
      // calculate a new length for the tongue to be clear of anything else that
      // moves under it.
...
CBaseEntity *CNPC_Barnacle::TongueTouchEnt(float *pflLength) {
  ...
  // position.
  UTIL_TraceLine(origin, origin - Vector(0, 0, 2048), MASK_SOLID_BRUSHONLY,
  ...
  // NOTENOTE: (this relieves the need to know if the tongue is currently
  // moving)
  ...
  if (count) {
    for (int i = 0; i < count; i++) {
      ...
      if (pList[i] != this && bCanHurt == true &&
          pVictim->m_lifeState == LIFE_ALIVE) {
        return pList[i];
      }
    }
  }
