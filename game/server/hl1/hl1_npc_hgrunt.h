...
private:
  // checking the feasibility of a grenade toss is kind of costly, so we do it
  // every couple of seconds, not every server frame.
  ...
  bool m_fStanding;
  bool m_fFirstEncounter; // only put on the handsign show in the squad's first
                          // encounter.
  int m_iClipSize;
