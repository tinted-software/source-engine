...
private:
  ...
  virtual Activity ChooseIntersectionPointAndActivity(trace_t &hitTrace,
                                                      const Vector &mins,
                                                      const Vector &maxs,
                                                      CBasePlayer *pOwner);
...
#define BLUDGEON_HULL_DIM 16
        ...
        static const Vector
    g_bludgeonMins(-BLUDGEON_HULL_DIM, -BLUDGEON_HULL_DIM, -BLUDGEON_HULL_DIM);
...
void CWeaponCrowbar::Hit(void) {
  ...
  CSoundEnt::InsertSound(SOUND_BULLET_IMPACT, m_traceHit.endpos, 400, 0.2f,
                         pPlayer);
  ...
  if (pHitEntity != NULL) {
    ...
    ClearMultiDamage();
    CTakeDamageInfo info(GetOwner(), GetOwner(), sk_plr_dmg_crowbar.GetFloat(),
                         DMG_CLUB);
    ...
    TraceAttackToTriggers(CTakeDamageInfo(GetOwner(), GetOwner(),
                                          sk_plr_dmg_crowbar.GetFloat(),
                                          DMG_CLUB),
...
#ifndef CLIENT_DLL
  if (!bIsWorld) {
    bIsWorld |= pHitEntity->Classify() == CLASS_NONE ||
...
Activity CWeaponCrowbar::ChooseIntersectionPointAndActivity(
...
void CWeaponCrowbar::Swing(void) {
  ...
  if (m_traceHit.fraction == 1.0) {
    float bludgeonHullRadius =
        1.732f * BLUDGEON_HULL_DIM; // hull is +/- 16, so use cuberoot of 2 to
                                    // determine how big the hull is from center
                                    // to the corner point
    ...
    UTIL_TraceHull(swingStart, swingEnd, g_bludgeonMins, g_bludgeonMaxs,
                   MASK_SHOT_HULL, pOwner, COLLISION_GROUP_NONE, &m_traceHit);
    if (m_traceHit.fraction < 1.0) {
      m_nHitActivity = ChooseIntersectionPointAndActivity(
          m_traceHit, g_bludgeonMins, g_bludgeonMaxs, pOwner);
    }
  }
