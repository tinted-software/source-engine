...
bool CHL1BaseNPC::NoFriendlyFire(void) {
  ...
  if (GetEnemy() != NULL) {
    ...
    VectorAngles((GetEnemy()->WorldSpaceCenter() - GetAbsOrigin()),
                 vAngleToEnemy);

    AngleVectors(vAngleToEnemy, &vForward, &vRight, &vUp);
  } else {
    // if there's no enemy, pretend there's a friendly in the way, so the grunt
    // won't shoot.
    return false;
  }
...
void CHL1BaseNPC::TraceAttack(const CTakeDamageInfo &info, const Vector &vecDir,
...
void CHL1BaseNPC::EjectShell(const Vector &vecOrigin, const Vector &vecVelocity,
