...
void CGrenadeSpit::Spawn(void) {
  ...
  // FIXME, if these is a sprite, then we need a base class derived from CSprite
  // rather than CBaseAnimating.  pev->scale becomes m_flSpriteScale in that
  // case.
...
void CGrenadeSpit::GrenadeSpitTouch(CBaseEntity *pOther) {
  ...
  if (!pOther->m_takedamage) {
    ...
    te->SpriteSpray(filter, 0.0, &tr.endpos, &tr.plane.normal,
                    m_nSquidSpitSprite, random->RandomInt(90, 160), 50,
