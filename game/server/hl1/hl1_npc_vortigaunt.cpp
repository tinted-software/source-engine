...
void CNPC_Vortigaunt::Spawn() {
  ...
  // pev->view_ofs		= VEC_VIEW;// position of the eyes relative to
  // monster's origin.
...
Disposition_t CNPC_Vortigaunt::IRelationType(CBaseEntity *pTarget) {
  if ((pTarget->IsPlayer())) {
    if ((GetSpawnFlags() & VORTIGAUNT_IGNORE_PLAYER) &&
...
void CNPC_Vortigaunt::CallForHelp(char *szClassname, float flDist,
                                  CBaseEntity *pEnemy, Vector &vecLocation) {
...
void CNPC_Vortigaunt::AlertSound(void) {
  if (GetEnemy() != NULL) {
    SENTENCEG_PlayRndSz(edict(), "SLV_ALERT", 0.85, SNDLVL_NORM, 0,
                        m_iVoicePitch);
...
void CNPC_Vortigaunt::IdleSound(void) {
  if (random->RandomInt(0, 2) == 0)
    SENTENCEG_PlayRndSz(edict(), "SLV_IDLE", 0.85, SNDLVL_NORM, 0,
                        m_iVoicePitch);
}
...
void CNPC_Vortigaunt::HandleAnimEvent(animevent_t *pEvent) {
  ...
  case ISLAVE_AE_CLAW: {
    // SOUND HERE!
    CBaseEntity *pHurt =
        CheckTraceHullAttack(40, Vector(-10, -10, -10), Vector(10, 10, 10),
  ...
  case ISLAVE_AE_ZAP_POWERUP: {
    ...
    te->DynamicLight(filter, 0.0, &GetAbsOrigin(), 125, 200, 100, 2, 120,
  ...
  case ISLAVE_AE_ZAP_SHOOT: {
    ...
    if (m_hDead != NULL) {
      ...
      trace_t trace;
      UTIL_TraceHull(vecDest, vecDest, GetHullMins(), GetHullMaxs(), MASK_SOLID,
                     m_hDead, COLLISION_GROUP_NONE, &trace);

      if (!trace.startsolid) {
        CBaseEntity *pNew =
            Create("monster_alien_slave", m_hDead->GetAbsOrigin(),
                   m_hDead->GetAbsAngles());
...
void CNPC_Vortigaunt::TraceAttack(const CTakeDamageInfo &info,
...
void CNPC_Vortigaunt::ArmBeam(int side) {
  ...
  for (int i = 0; i < 3; i++) {
    vecAim = right * side * random->RandomFloat(0, 1) +
             up * random->RandomFloat(-1, 1);
    trace_t tr1;
    UTIL_TraceLine(vecSrc, vecSrc + vecAim * 512, MASK_SOLID, this,
...
void CNPC_Vortigaunt::ZapBeam(int side) {
  ...
  vecAim = vecAim + side * right * random->RandomFloat(0, deflection) +
           up * random->RandomFloat(-deflection, deflection);
  UTIL_TraceLine(vecSrc, vecSrc + vecAim * 1024, MASK_SOLID, this,
