...
DEFINE_ENTITYFUNC(Touch), DEFINE_FIELD(m_bInBarnacleMouth, FIELD_BOOLEAN),
  ...
  case TASK_TALKER_LOOK_AT_CLIENT: {
    ...
    if (m_NPCState == NPC_STATE_IDLE && !IsMoving() &&
    ...
    if (pTask->iTask == TASK_TALKER_CLIENT_STARE) {
      ...
      if (UTIL_DotPoints(pPlayer->GetAbsOrigin(), GetAbsOrigin(), vForward) <
          m_flFieldOfView) {
        // player looked away
...
int CHL1NPCTalker::FIdleSpeak(void) {
  ...
  // response time, return try to talk to any standing or sitting scientists
  // nearby
  ...
  if (pentTalker && random->RandomInt(0, 1)) {
    ...
    pentTalker->GetExpresser()->BlockSpeechUntil(
...
bool CHL1NPCTalker::IsValidSpeechTarget(int flags, CBaseEntity *pEntity) {
  ...
  if (pentTarget) {
    ...
    if (pNPC) {
      ...
      // if ( (pNPC->m_NPCState == NPC_STATE_SCRIPT || pNPC->m_NPCState ==
      // NPC_STATE_PRONE)) 	return false;
...
bool CHL1NPCTalker::HandleInteraction(int interactionType, void *data,
...
int CHL1NPCTalker::PlayScriptedSentence(const char *pszSentence, float delay,
  ...
  return BaseClass::PlayScriptedSentence(pszSentence, delay, volume, soundlevel,
...
void CHL1NPCTalker::TraceAttack(const CTakeDamageInfo &info,
...
void CHL1NPCTalker::FollowerUse(CBaseEntity *pActivator, CBaseEntity *pCaller,
...
float CHL1NPCTalker::PickLookTarget(bool bExcludePlayers, float minTime,
                                    float maxTime) {
  return random->RandomFloat(5.0f, 10.0f);
}

void CHL1NPCTalker::IdleHeadTurn(CBaseEntity *pTarget, float flDuration,
                                 float flImportance) {
...
void CHL1NPCTalker::SetHeadDirection(const Vector &vTargetPos,
                                     float flInterval) {
#ifdef TALKER_LOOK
...
// Purpose:
//-----------------------------------------------------------------------------
bool CHL1NPCTalker::OnObstructingDoor(AILocalMoveGoal_t *pMoveGoal,
...
DEFINE_SCHEDULE(SCHED_HL1TALKER_IDLE_SPEAK_WAIT,
                ...
                "	Tasks"
                "		TASK_SET_ACTIVITY			"
                "ACTIVITY:ACT_IDLE" // Stop and talk
                "		TASK_FACE_PLAYER			0"
                ""
                "	Interrupts"
                "		COND_NEW_ENEMY"
...
DEFINE_SCHEDULE(SCHED_HL1TALKER_BARNACLE_HIT,
                ...
                "		TASK_SET_SCHEDULE			"
                "SCHEDULE:SCHED_HL1TALKER_BARNACLE_PULL"
                ""
                "	Interrupts")
...
DEFINE_SCHEDULE(SCHED_HL1TALKER_BARNACLE_CHOMP,
                ...
                "		 TASK_SET_SCHEDULE			"
                "SCHEDULE:SCHED_HL1TALKER_BARNACLE_CHEW"
                ""
                "	Interrupts")
