...
DEFINE_ENTITYFUNC(BiteTouch),
    ...
    DEFINE_FIELD(m_idealDist, FIELD_FLOAT), DEFINE_FIELD(m_flBlink, FIELD_TIME),
...
void CNPC_Ichthyosaur::MoveExecute_Alive(float flInterval) {
  ...
  if (GetNavigator()->IsGoalActive()) {
    Vector vecDir =
        (GetNavigator()->GetPath()->CurWaypointPos() - GetAbsOrigin());
  ...
  angAbsAngles.x =
      clamp(UTIL_Approach(angSaveAngles.x, angAbsAngles.x, 10), -60, 60);
  ...
  // don't touch bone controller, makes swim animation look funky with all these
  // hard turns.
  ...
  if (WalkMove(m_SaveVelocity * flInterval, MASK_NPCSOLID) == false) {
    // Attempt a half-step
    if (WalkMove((m_SaveVelocity * 0.5f) * flInterval, MASK_NPCSOLID) ==
        false) {
...
void CNPC_Ichthyosaur::RunTask(const Task_t *pTask) {
  ...
  case TASK_ICHTHYOSAUR_CIRCLE_ENEMY:
    ...
    if (GetEnemy() == NULL) {
      ...
      //			UTIL_TraceHull( vecFrom, vecPos,
      //ignore_monsters, large_hull, m_hEnemy->edict(), &tr );
  ...
  case ICHTHYOSAUR_AE_SHAKE_LEFT: {
    ...
    if (hEnemy != NULL && FVisible(hEnemy)) {
      ...
      if (m_flEnemyTouched > gpGlobals->curtime &&
          (pHurt->BodyTarget(GetAbsOrigin()) - GetAbsOrigin()).Length() >
              (32 + 16 + 32))
        break;
      ...
      if (DotProduct(vecShootDir, vForward) > 0.707) {
        ...
        CTakeDamageInfo info(this, this, sk_ichthyosaur_shake.GetInt(),
                             DMG_SLASH);
...
Vector CNPC_Ichthyosaur::DoProbe(const Vector &Probe) {
  Vector WallNormal =
      Vector(0, 0, -1); // WATER normal is Straight Down for fish.
  float frac = 1.0;
  ...
  // NDebugOverlay::Line( tr.startpos, tr.endpos, 255.0f*(1.0f-tr.fraction),
  // 255.0f * tr.fraction, 0.0f, true, 0.05f ); NOTENOTE: Debug end

  if (bBumpedSomething && (GetEnemy() == NULL || !tr.m_pEnt ||
                           tr.m_pEnt->entindex() != GetEnemy()->entindex())) {
    ...
    float SteeringForce =
        m_flFlyingSpeed * (1 - frac) * (DotProduct(WallNormal, m_SaveVelocity));
    ...
    // NDebugOverlay::Line( GetAbsOrigin(), GetAbsOrigin() +
    // (SteeringVector*4.0f), 0, 255, 255, true, 0.1f );
...
bool CNPC_Ichthyosaur::ProbeZ(const Vector &position, const Vector &probe,
                              float *pFraction) {
  ...
  if (!(iPositionContents & MASK_WATER)) {
    ...
    *pFraction = 0.0;
    return true; // We hit a water boundary because we are where we don't
                 // belong.
  }
