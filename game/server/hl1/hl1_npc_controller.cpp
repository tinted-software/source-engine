...
class CControllerNavigator
    : public CAI_ComponentWithOuter<CNPC_Controller, CAI_Navigator> {
...
public:
  ...
  CAI_Navigator *CreateNavigator() { return new CControllerNavigator(this); }
...
DEFINE_ARRAY(m_pBall, FIELD_CLASSPTR, 2),
  ...
  m_flFieldOfView =
      VIEW_FIELD_FULL; // indicates the width of this monster's forward view
  ...
  CapabilitiesAdd(bits_CAP_MOVE_FLY | bits_CAP_INNATE_RANGE_ATTACK1 |
...
void CNPC_Controller::HandleAnimEvent(animevent_t *pEvent) {
  ...
  case CONTROLLER_AE_HEAD_OPEN: {
    ...
    te->DynamicLight(filter, 0.0, &vecStart, 255, 192, 64, 0, 1 /*radius*/, 0.2,
                     -32);
  ...
  case CONTROLLER_AE_BALL_SHOOT: {
    ...
    CBroadcastRecipientFilter filter;
    te->DynamicLight(filter, 0.0, &vecStart, 255, 192, 64, 0, 1 /*radius*/, 0.1,
                     32);

    CAI_BaseNPC *pBall =
        (CAI_BaseNPC *)Create("controller_head_ball", vecStart, angleGun);
...
void CNPC_Controller::RunTask(const Task_t *pTask) {
    ...
    while (m_flShootTime < m_flShootEnd && m_flShootTime < gpGlobals->curtime) {
      ...
      if (GetEnemy() != NULL) {
        if (HasCondition(COND_SEE_ENEMY)) {
          m_vecEstVelocity =
              m_vecEstVelocity * 0.5 + GetEnemy()->GetAbsVelocity() * 0.5;
        } else {
          m_vecEstVelocity = m_vecEstVelocity * 0.8;
        }
        vecDir =
            Intersect(vecSrc, GetEnemy()->BodyTarget(GetAbsOrigin()),
                      m_vecEstVelocity, sk_controller_speedball.GetFloat());

        float delta = 0.03490; // +-2 degree
        vecDir = vecDir + Vector(random->RandomFloat(-delta, delta),
                                 random->RandomFloat(-delta, delta),
                                 random->RandomFloat(-delta, delta)) *
                              sk_controller_speedball.GetFloat();

        vecSrc = vecSrc + vecDir * (gpGlobals->curtime - m_flShootTime);
        CAI_BaseNPC *pBall = (CAI_BaseNPC *)Create(
            "controller_energy_ball", vecSrc, GetAbsAngles(), this);
        pBall->SetAbsVelocity(vecDir);

        //				DevMsg( 2, "controller shooting energy
        //ball\n" );
      }
  ...
  case TASK_WAIT_PVS: {
    if (GetEnemy()) {
      float idealYaw =
          UTIL_VecToYaw(GetEnemy()->GetAbsOrigin() - GetAbsOrigin());
...
void CNPC_Controller::RunAI(void) {
  ...
  for (int i = 0; i < 2; i++) {
    if (m_pBall[i] == NULL) {
      m_pBall[i] =
          CSprite::SpriteCreate("sprites/xspark4.vmt", GetAbsOrigin(), TRUE);
      m_pBall[i]->SetTransparency(kRenderGlow, 255, 255, 255, 255,
    ...
    te->DynamicLight(filter, 0.0, &vecStart, 255, 192, 64, 0 /*exponent*/,
                     m_iBallCurrent[i] / 8 /*radius*/, 0.5, 0);
  }
}
...
bool CNPC_Controller::OverridePathMove(float flInterval) {
  ...
  // ------------------------------------------------------------------
  Vector checkPos =
      GetLocalOrigin() + (waypointDir * (m_flGroundSpeed * flInterval));

  AIMoveTrace_t moveTrace;
  GetMoveProbe()->MoveLimit(NAV_FLY, GetLocalOrigin(), checkPos,
                            MASK_NPCSOLID | CONTENTS_WATER, pMoveTarget,
                            &moveTrace);
  ...
  if (ProgressFlyPath(flInterval, pMoveTarget, MASK_NPCSOLID, false, 64) ==
      AINPP_COMPLETE) {
    {
  ...
  if (pMoveTarget /*|| HaveInspectTarget()*/) {
    ...
    if (pMoveTarget) {
      UTIL_TraceEntity(this, GetAbsOrigin(), vMoveTargetPos,
                       MASK_NPCSOLID_BRUSHONLY, pMoveTarget,
                       GetCollisionGroup(), &tr);
    } else {
      UTIL_TraceEntity(this, GetAbsOrigin(), vMoveTargetPos,
                       MASK_NPCSOLID_BRUSHONLY, &tr);
    }
    /*
                    float fTargetDist = (1-tr.fraction)*(GetAbsOrigin() -
       vMoveTargetPos).Length(); if (fTargetDist > 50)
                    {
...
void CNPC_Controller::MoveToTarget(float flInterval,
...
DEFINE_THINKFUNC(HuntThink), DEFINE_THINKFUNC(KillThink),
  ...
  m_pSprite =
      CSprite::SpriteCreate("sprites/xspark4.vmt", GetAbsOrigin(), FALSE);
  m_pSprite->SetTransparency(kRenderTransAdd, 255, 255, 255, 255,
                             kRenderFxNoDissipation);
...
void CNPC_ControllerHeadBall::HuntThink(void) {
  ...
  CBroadcastRecipientFilter filter;
  te->DynamicLight(filter, 0.0, &GetAbsOrigin(), 255, 255, 255, 0,
                   m_pSprite->GetBrightness() / 16, 0.2, 0);

  // check world boundaries
  if (gpGlobals->curtime - m_flSpawnTime > 5 ||
      m_pSprite->GetBrightness() <
          64 /*|| GetEnemy() == NULL || m_hOwner == NULL*/
      || !IsInWorld()) {
    SetTouch(NULL);
  ...
  if ((GetEnemy()->WorldSpaceCenter() - GetAbsOrigin()).Length() < 64) {
    ...
    UTIL_TraceLine(GetAbsOrigin(), GetEnemy()->WorldSpaceCenter(), MASK_ALL,
    ...
    if (pEntity != NULL && pEntity->m_takedamage == DAMAGE_YES) {
      ...
      VectorNormalize(dir);
      CTakeDamageInfo info(this, this, sk_controller_dmgball.GetFloat(),
                           DMG_SHOCK);
      ...
      te->BeamEntPoint(filter, 0.0, entindex(), NULL, 0,
                       &(tr.m_pEnt->GetAbsOrigin()), g_sModelIndexLaser,
                       haloindex /* no halo */, 0, 10, 3, 20, 20, fadelength,
                       amplitude, 255, 255, 255, 255, 10);
    }

    UTIL_EmitAmbientSound(GetSoundSourceIndex(), GetAbsOrigin(),
                          "Controller.ElectroSound", 0.5, SNDLVL_NORM, 0, 100);
...
void CNPC_ControllerZapBall::ExplodeTouch(CBaseEntity *pOther) {
  if (m_takedamage = DAMAGE_YES) {
    ...
    if (m_hOwner != NULL) {
      CTakeDamageInfo info(this, m_hOwner, sk_controller_dmgball.GetFloat(),
                           DMG_ENERGYBEAM);
    ...
    //	void UTIL_EmitAmbientSound( CBaseEntity *entity, const Vector
    //&vecOrigin, const char *samp, float vol, soundlevel_t soundlevel, int
    //fFlags, int pitch, float soundtime /*= 0.0f*/ )

    UTIL_EmitAmbientSound(GetSoundSourceIndex(), tr.endpos,
                          "Controller.ElectroSound", 0.3, SNDLVL_NORM, 0,
