...
public:
  ...
  flDamage, int bitsDamageType ); void TraceAttack( entvars_t *pevAttacker,
...
DEFINE_FIELD(m_flForce, FIELD_FLOAT),
    ...
    DEFINE_FIELD(m_flMinZ, FIELD_FLOAT), DEFINE_FIELD(m_flMaxZ, FIELD_FLOAT),
    ...
    DEFINE_FIELD(m_bDead, FIELD_BOOLEAN), DEFINE_THINKFUNC(NullThink),
    DEFINE_THINKFUNC(StartupThink), DEFINE_THINKFUNC(HuntThink),
...
public:
  ...
  void TeleportInit(CNPC_Nihilanth *pOwner, CBaseEntity *pEnemy,
                    CBaseEntity *pTarget, CBaseEntity *pTouch);
  ...
  void SetBeam(CBaseEntity *pBeam) { m_hBeam = pBeam; }

  CBaseEntity *GetBeam(void) { return m_hBeam.Get(); }
...
DEFINE_FIELD(m_flIdealVel, FIELD_FLOAT),
    ...
    DEFINE_THINKFUNC(HoverThink), DEFINE_ENTITYFUNC(BounceTouch),
    DEFINE_THINKFUNC(ZapThink), DEFINE_ENTITYFUNC(ZapTouch),
    DEFINE_THINKFUNC(DissipateThink), DEFINE_THINKFUNC(TeleportThink),
  ...
  CollisionProp()->SetSurroundingBoundsType(
      USE_SPECIFIED_BOUNDS, &vecSurroundingMins, &vecSurroundingMaxs);
...
void CNPC_Nihilanth::TraceAttack(const CTakeDamageInfo &info,
  ...
  if (m_irritation != 3) {
    ...
    UTIL_BloodStream(
        ptr->endpos, vecBlood, BloodColor(),
        info.GetDamage() +
            (100 - 100 * (m_iHealth / sk_nihilanth_health.GetFloat())));
  }

  // SpawnBlood(ptr->vecEndPos, BloodColor(), flDamage * 5.0);// a little
  // surface blood.
...
bool CNPC_Nihilanth::EmitSphere(void) {
  ...
  CNihilanthHVR *pEntity =
      (CNihilanthHVR *)CREATE_ENTITY(CNihilanthHVR, "nihilanth_energy_ball");
...
void CNPC_Nihilanth::HuntThink(void) {
  ...
  if (GetEnemy() != NULL && m_hRecharger == NULL) {
    if (FVisible(GetEnemy())) {
      ...
      m_posDesired =
          Vector(GetAbsOrigin().x, GetAbsOrigin().y, m_posTarget.z + m_flAdj);
    } else {
...
void CNPC_Nihilanth::Flight(void) {
  ...
  QAngle vAngle =
      QAngle(GetAbsAngles().x + m_avelocity.x, GetAbsAngles().y + m_avelocity.y,
             GetAbsAngles().z + m_avelocity.z);
  ...
  float flDir = DotProduct(Vector(vForward.x, vForward.y, 0),
  ...
  vAngle = QAngle(GetAbsAngles().x + m_avelocity.x * 0.1,
                  GetAbsAngles().y + m_avelocity.y * 0.1,
                  GetAbsAngles().z + m_avelocity.z * 0.1);

  SetAbsAngles(vAngle);

  // ALERT( at_console, "%5.0f %5.0f : %4.0f : %3.0f : %2.0f\n", m_posDesired.z,
  // pev->origin.z, m_velocity.z, m_avelocity.y, m_flForce );
}
...
void CNPC_Nihilanth::NextActivity() {
  ...
  if (m_irritation >= 2) {
    if (m_pBall == NULL) {
      m_pBall =
          CSprite::SpriteCreate("sprites/tele1.vmt", GetAbsOrigin(), true);
      if (m_pBall) {
        m_pBall->SetTransparency(kRenderTransAdd, 255, 255, 255, 255,
  ...
  if ((m_iHealth < sk_nihilanth_health.GetFloat() / 2 ||
       m_iActiveSpheres < N_SPHERES / 2) &&
      m_hRecharger == NULL && m_iLevel <= 9) {
    char szName[64];
    ...
    if (pRecharger) {
      ...
      m_posDesired = Vector(GetAbsOrigin().x, GetAbsOrigin().y,
                            pRecharger->GetAbsOrigin().z);
  ...
  if (GetEnemy() != NULL && m_irritation != 0) {
    if (m_flLastSeen + 5 > gpGlobals->curtime && flDist < 256 && flDot > 0) {
      if (m_irritation >= 2 &&
          m_iHealth < sk_nihilanth_health.GetFloat() / 2.0) {
        ...
        if (random->RandomInt(0, 1) == 0) {
          ...
          Q_snprintf(szText, sizeof(szText), "%s%d", m_szTeleportTouch,
                     m_iTeleport);
          CBaseEntity *pTouch = gEntList.FindEntityByName(NULL, szText);

          Q_snprintf(szText, sizeof(szText), "%s%d", m_szTeleportUse,
                     m_iTeleport);
...
void CNPC_Nihilanth::MakeFriend(Vector vecStart) {
  ...
  for (i = 0; i < 3; i++) {
    if (m_hFriend[i] != NULL && !m_hFriend[i]->IsAlive()) {
      if (m_nRenderMode ==
          kRenderNormal) // don't do it if they are already fading
    ...
    if (m_hFriend[i] == NULL) {
      if (random->RandomInt(0, 1) == 0) {
        ...
        if (pNode && pNode->GetType() == NODE_AIR) {
          ...
          UTIL_TraceHull(vNodeOrigin + Vector(0, 0, 32),
                         vNodeOrigin + Vector(0, 0, 32), Vector(-40, -40, 0),
                         Vector(40, 40, 100), MASK_SOLID, this,
                         COLLISION_GROUP_NONE, &tr);

          if (tr.startsolid == 0)
            m_hFriend[i] =
                Create("monster_alien_controller", vNodeOrigin, GetAbsAngles());
        }
      } else {
        int iNode = GetNavigator()->GetNetwork()->NearestNodeToPoint(vecStart);
        CAI_Node *pNode = GetNavigator()->GetNetwork()->GetNode(iNode);
        ...
        if (pNode && (pNode->GetType() == NODE_GROUND ||
                      pNode->GetType() == NODE_WATER)) {
          trace_t tr;
          Vector vNodeOrigin = pNode->GetOrigin();

          UTIL_TraceHull(vNodeOrigin + Vector(0, 0, 36),
                         vNodeOrigin + Vector(0, 0, 36), Vector(-15, -15, 0),
                         Vector(20, 15, 72), MASK_SOLID, this,
                         COLLISION_GROUP_NONE, &tr);

          if (tr.startsolid == 0)
            m_hFriend[i] =
                Create("monster_alien_slave", vNodeOrigin, GetAbsAngles());
        }
      }
      if (m_hFriend[i] != NULL) {
        CPASAttenuationFilter filter(this);
  ...
  if (m_flShootEnd > gpGlobals->curtime) {
    ...
    while (m_flShootTime < m_flShootEnd && m_flShootTime < gpGlobals->curtime) {
      if (GetEnemy() != NULL) {
        ...
        vecSrc =
            vecHand + GetAbsVelocity() * (m_flShootTime - gpGlobals->curtime);
        vecDir = m_posTarget - GetAbsOrigin();
        VectorNormalize(vecDir);
        vecSrc = vecSrc + vecDir * (gpGlobals->curtime - m_flShootTime);

        pEntity = (CNihilanthHVR *)CREATE_ENTITY(CNihilanthHVR,
...
void CNPC_Nihilanth::DyingThink(void) {
  ...
  if (m_lifeState == LIFE_DYING) {
    ...
    if (fabs(GetAbsOrigin().z - m_flMaxZ) < 16) {
      ...
      while ((pTrigger = gEntList.FindEntityByName(pTrigger, m_szDeadUse)) !=
             NULL) {
  ...
  do {
    vecDir = Vector(random->RandomFloat(-1, 1), random->RandomFloat(-1, 1),
  ...
  UTIL_TraceLine(vecSrc, vecSrc + vecDir * 4096, MASK_SOLID, this,
                 COLLISION_GROUP_NONE, &tr);
  ...
  pEntity->SetAbsAngles(GetAbsAngles());
  pEntity->SetOwnerEntity(this);
  pEntity->SetAbsVelocity(Vector(random->RandomFloat(-0.7, 0.7),
                                 random->RandomFloat(-0.7, 0.7), 1.0) *
                          600.0);
  pEntity->Spawn();
...
void CNPC_Nihilanth::HandleAnimEvent(animevent_t *pEvent) {
  ...
  case 2: // zen
    if (GetEnemy() != NULL) {
      ...
      te->DynamicLight(filterlight, 0.0, &vOrigin, 128, 128, 255, 0, 256, 1.0f,
                       128);
  ...
  case 3: // prayer
    if (GetEnemy() != NULL) {
      ...
      if (pTrigger != NULL || pTouch != NULL) {
        ...
        CNihilanthHVR *pEntity = (CNihilanthHVR *)CREATE_ENTITY(
        ...
        pEntity->SetAbsVelocity(GetAbsOrigin() - vecSrc);
        pEntity->SetAbsVelocity(Vector(GetAbsVelocity().x, GetAbsVelocity().y,
                                       GetAbsVelocity().z * 0.2));
        ...
        te->DynamicLight(filterlight, 0.0, &vOrigin, 128, 128, 255, 0, 256,
                         1.0f, 128);
  ...
  case 7:
    ...
    CNihilanthHVR *pEntity = (CNihilanthHVR *)Create( "nihilanth_energy_ball",
    vecSrc, pev->angles, edict() ); pEntity->pev->velocity = Vector (
    RANDOM_FLOAT( -0.7, 0.7 ), RANDOM_FLOAT( -0.7, 0.7 ), 1.0 ) * 600.0;
    pEntity->GreenBallInit( );
    */
    break;
  }
...
void CNihilanthHVR::Spawn(void) { Precache(); }
...
void CNihilanthHVR::CircleInit(CBaseEntity *pTarget) {
  ...
  if (pSprite) {
    ...
    pSprite->SetTransparency(kRenderTransAdd, 255, 224, 192, 255,
                             kRenderFxNone);
  }
...
CSprite *CNihilanthHVR::SpriteInit(const char *pSpriteName,
                                   CNihilanthHVR *pOwner) {
  pOwner->SetSprite(
      CSprite::SpriteCreate(pSpriteName, pOwner->GetAbsOrigin(), true));
...
bool CNihilanthHVR::CircleTarget(Vector vecTarget) {
  ...
  SetLocalAngularVelocity(QAngle(GetLocalAngularVelocity().x,
                                 GetLocalAngularVelocity().y, d1 * 20));

  if (d1 < 32) {
    fClose = true;
  }

  m_vecIdeal = m_vecIdeal + Vector(random->RandomFloat(-2, 2),
                                   random->RandomFloat(-2, 2),
                                   random->RandomFloat(-2, 2));
...
void CNihilanthHVR::ZapThink(void) {
  ...
  if (GetEnemy() == NULL || GetAbsOrigin().x < -4096 ||
      GetAbsOrigin().x > 4096 || GetAbsOrigin().y < -4096 ||
      GetAbsOrigin().y > 4096 || GetAbsOrigin().z < -4096 ||
      GetAbsOrigin().z > 4096) {
    SetTouch(NULL);
    UTIL_Remove(GetSprite());
    UTIL_Remove(this);
  ...
  if ((GetEnemy()->WorldSpaceCenter() - GetAbsOrigin()).Length() < 256) {
    ...
    UTIL_TraceLine(GetAbsOrigin(), GetEnemy()->WorldSpaceCenter(), MASK_SOLID,
    ...
    UTIL_EmitAmbientSound(GetSoundSourceIndex(), tr.endpos,
                          "Controller.ElectroSound", 0.5, SNDLVL_NORM, 0,
  ...
  CBroadcastRecipientFilter filterlight;
  te->DynamicLight(filterlight, 0.0, &GetAbsOrigin(), 128, 128, 255, 0, 128, 10,
                   128);
}

void CNihilanthHVR::ZapTouch(CBaseEntity *pOther) {
  UTIL_EmitAmbientSound(GetSoundSourceIndex(), GetAbsOrigin(),
                        "Controller.ElectroSound", 1.0, SNDLVL_NORM, 0,
                        random->RandomInt(90, 95));

  RadiusDamage(CTakeDamageInfo(this, this, 50, DMG_SHOCK), GetAbsOrigin(), 125,
               CLASS_NONE, NULL);
...
void CNihilanthHVR::TeleportInit(CNPC_Nihilanth *pOwner, CBaseEntity *pEnemy,
                                 CBaseEntity *pTarget, CBaseEntity *pTouch) {
  SetMoveType(MOVETYPE_FLY);
  SetSolid(SOLID_BBOX);
...
void CNihilanthHVR::TeleportThink(void) {
  ...
  // check world boundaries
  if (GetEnemy() == NULL || !GetEnemy()->IsAlive() ||
      GetAbsOrigin().x < -4096 || GetAbsOrigin().x > 4096 ||
      GetAbsOrigin().y < -4096 || GetAbsOrigin().y > 4096 ||
      GetAbsOrigin().z < -4096 || GetAbsOrigin().z > 4096) {
  ...
  CBroadcastRecipientFilter filterlight;
  te->DynamicLight(filterlight, 0.0, &GetAbsOrigin(), 0, 255, 0, 0, 256, 1.0,
                   256);
}
