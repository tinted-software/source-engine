...
bool CNPCMaker::CanMakeNPC(void) {
  if (m_iMaxLiveChildren > 0 &&
      m_cLiveChildren >=
          m_iMaxLiveChildren) { // not allowed to make a new one yet. Too many
...
void CNPCMaker::MakeNPC(void) {
  ...
  CBaseEntity *pent =
      (CBaseEntity *)CreateEntityByName(STRING(m_iszNPCClassname));
