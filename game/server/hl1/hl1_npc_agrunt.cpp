...
void CNPC_AlienGrunt::Spawn() {
  ...
  m_flNextSpeakTime = m_flNextWordTime =
      gpGlobals->curtime + 10 + random->RandomInt(0, 10);
...
void CNPC_AlienGrunt::HandleAnimEvent(animevent_t *pEvent) {
  ...
  case AGRUNT_AE_HORNET5: {
    ...
    te->Sprite(filter, 0.0, &vecArmPos, iAgruntMuzzleFlash,
               random->RandomFloat(0.4, 0.8), 128);

    CBaseEntity *pHornet =
        CBaseEntity::Create("hornet", vecArmPos, QAngle(0, 0, 0), this);
  ...
  case AGRUNT_AE_LEFT_PUNCH: {
    ...
    CBaseEntity *pHurt =
        CheckTraceHullAttack(AGRUNT_MELEE_DIST, vecMins, vecMaxs,
                             sk_agrunt_dmg_punch.GetFloat(), DMG_CLUB);
    ...
    if (pHurt) {
      ...
      // OK to use gpGlobals without calling MakeVectors, cause
      // CheckTraceHullAttack called it above.
      if (pHurt->IsPlayer()) {
      ...
      SpawnBlood(vecArmPos, g_vecAttackDir, pHurt->BloodColor(),
                 25); // a little surface blood.
    } else {
...
int CNPC_AlienGrunt::RangeAttack1Conditions(float flDot, float flDist) {
  ...
  if (HasCondition(COND_SEE_ENEMY)) {
    ...
    // verify that a shot fired from the gun will hit the enemy before the
    // world.
    // !!!LATER - we may wish to do something different for projectile weapons
    // as opposed to instant-hit
    GetAttachment("0", vecArmPos, angArmDir);
    UTIL_TraceLine(vecArmPos, GetEnemy()->BodyTarget(vecArmPos), MASK_SOLID,
                   this, COLLISION_GROUP_NONE, &tr);

    if (tr.fraction == 1.0 || tr.m_pEnt == GetEnemy()) {
      m_flNextHornetAttackCheck =
          gpGlobals->curtime + random->RandomFloat(2, 5);
  ...
  m_flNextHornetAttackCheck =
      gpGlobals->curtime +
      0.2; // don't check for half second if this check wasn't successful
...
void CNPC_AlienGrunt::StopTalking(void) {
  m_flNextWordTime = m_flNextSpeakTime =
      gpGlobals->curtime + 10 + random->RandomInt(0, 10);
}
...
void CNPC_AlienGrunt::StartTask(const Task_t *pTask) {
  ...
  case TASK_AGRUNT_SETUP_HIDE_ATTACK:
    // alien grunt shoots hornets back out into the open from a concealed
    // location. try to find a spot to throw that gives the smart weapon a good
    // chance of finding the enemy. ideally, this spot is along a line that is
    // perpendicular to a line drawn from the agrunt to the enemy.
    ...
    if (pEnemyMonsterPtr) {
      ...
      UTIL_TraceLine(WorldSpaceCenter() + vForward * 128, vecEnemyLKP,
      ...
      if (!fSkip) {
        UTIL_TraceLine(WorldSpaceCenter() + vForward * 256, vecEnemyLKP,
                       MASK_SOLID_BRUSHONLY, this, COLLISION_GROUP_NONE, &tr);
        if (tr.fraction == 1.0) {
          GetMotor()->SetIdealYawToTargetAndUpdate(GetAbsOrigin() +
                                                   vRight * 256);
...
int CNPC_AlienGrunt::SelectSchedule(void) {
  ...
  case NPC_STATE_COMBAT: {
    ...
    if (HasCondition(COND_CAN_RANGE_ATTACK1) &&
        OccupyStrategySlotRange(AGRUNT_SQUAD_SLOT_HORNET1,
                                AGRUNT_SQUAD_SLOT_HORNET2)) {
...
void CNPC_AlienGrunt::TraceAttack(const CTakeDamageInfo &info,
  ...
  if (ptr->hitgroup == 10 &&
      (ainfo.GetDamageType() & (DMG_BULLET | DMG_SLASH | DMG_CLUB))) {
    // hit armor
...
// Suppress
//=========================================================
DEFINE_SCHEDULE(SCHED_AGRUNT_SUPPRESS_HORNET,
                "	Tasks"
                "	TASK_STOP_MOVING			0"
...
//=========================================================
DEFINE_SCHEDULE(SCHED_AGRUNT_RANGE_ATTACK,
                "	Tasks"
                "	TASK_STOP_MOVING			0"
                "	TASK_FACE_ENEMY				0"
                "	TASK_RANGE_ATTACK1			0"
...
DEFINE_SCHEDULE(
    ...
    "SCHEDULE:SCHED_AGRUNT_THREAT_DISPLAY"
    "	TASK_WAIT							"
    "		0.2"
