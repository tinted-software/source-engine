...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CAI_SpeechFilter::InputEnable(inputdata_t &inputdata) { Enable(true); }

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CAI_SpeechFilter::InputDisable(inputdata_t &inputdata) { Enable(false); }
      ...
      if (pAlly) {
        if (purge) {
          ...
          if (pAlly->GetSpeechFilter() != NULL) {
            DevWarning("ai_speechfilter %s is slamming NPC %s's current speech "
                       "filter.\n",
                       STRING(GetEntityName()),
                       STRING(pSearch->GetEntityName()));
          }

          pAlly->SetSpeechFilter(this);
        }
      } else if (pSearch->IsNPC()) {
        DevWarning("ai_speechfilter %s tries to use %s as a subject, but it's "
                   "not a talking NPC.\n",
      ...
      if (pSearch) {
        ...
        if (pAlly) {
          if (purge) {
            ...
            if (pAlly->GetSpeechFilter() != NULL) {
              DevWarning("ai_speechfilter %s is slamming NPC %s's current "
                         "speech filter.\n",
                         STRING(GetEntityName()),
                         STRING(pSearch->GetEntityName()));
            }
          ...
          DevWarning("ai_speechfilter %s tries to use %s as a subject, but "
                     "it's not a talking NPC.\n",
                     STRING(GetEntityName()), STRING(pSearch->GetEntityName()));
        }
#endif
        iNumSubjects++;
...
void CAI_SpeechFilter::OnEntityCreated(CBaseEntity *pEntity) {
  if (!m_bDisabled && (pEntity->NameMatches(m_iszSubject) ||
                       pEntity->ClassMatches(m_iszSubject))) {
#ifndef CSTRIKE_DLL
