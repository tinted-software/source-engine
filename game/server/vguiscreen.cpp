...
SendPropFloat(SENDINFO(m_flWidth), 0, SPROP_NOSCALE),
    ...
    SendPropInt(SENDINFO(m_nPanelName), MAX_VGUI_SCREEN_STRING_BITS,
                SPROP_UNSIGNED),
    SendPropInt(SENDINFO(m_fScreenFlags), VGUI_SCREEN_MAX_BITS, SPROP_UNSIGNED),
    SendPropInt(SENDINFO(m_nOverlayMaterial), MAX_MATERIAL_STRING_BITS,
                SPROP_UNSIGNED),
    SendPropEHandle(SENDINFO(m_hPlayerOwner)), END_SEND_TABLE();
...
// Purpose:
//-----------------------------------------------------------------------------
void CVGuiScreen::InputSetActive(inputdata_t &inputdata) { SetActive(true); }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CVGuiScreen::InputSetInactive(inputdata_t &inputdata) { SetActive(false); }
...
//-----------------------------------------------------------------------------
// Purpose: Screens attached to view models only go to client if viewmodel is
// being sent, too. Input  : *recipient -
//			*pvs -
//			clientArea -
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
int CVGuiScreen::UpdateTransmitState() {
  if (IsAttachedToViewModel()) {
    ...
    // Let the parent object trigger the send. This is more efficient than
    // having it call CBaseEntity::ShouldTransmit for all the vgui screens in
    // the map.
...
void CVGuiScreen::SetPanelName(const char *pPanelName) {
  m_nPanelName =
      g_pStringTableVguiScreen->AddString(CBaseEntity::IsServer(), pPanelName);
}
...
//
//-----------------------------------------------------------------------------
void CVGuiScreen::SetPlayerOwner(CBasePlayer *pPlayer,
                                 bool bOwnerOnlyInput /* = false */) {
...
CVGuiScreen *CreateVGuiScreen(const char *pScreenClassname,
  ...
  Assert(pAttachedTo);
  CVGuiScreen *pScreen = (CVGuiScreen *)CBaseEntity::Create(
      pScreenClassname, vec3_origin, vec3_angle, pAttachedTo);
