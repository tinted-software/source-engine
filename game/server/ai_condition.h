...
enum SCOND_t {
  ...
  COND_IN_PVS,
  COND_IDLE_INTERRUPT, // The schedule in question is a low priority idle, and
                       // therefore a candidate for translation into something
                       // else
  ...
  COND_TOO_CLOSE_TO_ATTACK, // FIXME: most of this next group are meaningless
                            // since they're shared between all attack checks!
  ...
  COND_NO_HEAR_DANGER, // Since we can't use ~CONDITION. Mutually exclusive with
                       // COND_HEAR_DANGER
  ...
  COND_NO_CUSTOM_INTERRUPTS, // Don't call BuildScheduleTestBits for this
                             // schedule. Used for schedules that must strictly
                             // control their interruptibility.
