...
public:
  ...
  int CountFollowers(CBaseEntity *pFollowTarget, string_t iszClassname) {
    ...
    if (iszClassname == NULL_STRING) {
      ...
      int result = 0;
      for (intp i = pGroup->followers.Head();
...
DEFINE_FIELD(flags, FIELD_INTEGER),
    ...
    DEFINE_FIELD(chaseEnemyTolerance, FIELD_FLOAT), END_DATADESC();
...
DEFINE_FIELD(m_hFollowTarget, FIELD_EHANDLE), DEFINE_EMBEDDED(m_FollowNavGoal),
    ...
    DEFINE_FIELD(m_vFollowMoveAnchor, FIELD_POSITION_VECTOR), END_DATADESC();
...
bool CAI_FollowBehavior::IsFollowTargetInRange(float rangeMultiplier) {
  ...
  if (GetNpcState() == NPC_STATE_COMBAT) {
    if (IsFollowGoalInRange(MAX(m_FollowNavGoal.coverTolerance,
...
bool CAI_FollowBehavior::IsFollowPointInRange() {
  return (
      GetHintNode() && GetHintNode()->HintType() == HINT_FOLLOW_WAIT_POINT &&
...
int CAI_FollowBehavior::TranslateSchedule(int scheduleType) {
  ...
  case SCHED_TARGET_FACE: {
    ...
    return SCHED_FACE_FOLLOW_TARGET; // @TODO (toml 03-03-03): should select a
                                     // facing sched
  }
...
void CAI_FollowBehavior::StartTask(const Task_t *pTask) {
  ...
  case TASK_FIND_COVER_FROM_ENEMY: {
    ...
    if (pLeader) {
      ...
      float coverRadius =
          MIN(GetOuter()->CoverRadius(), m_FollowNavGoal.coverTolerance);
  ...
  case TASK_GET_PATH_TO_FOLLOW_POSITION: {
    ...
    case 1: {
      if (GetOuter()->m_vInterruptSavePosition != vec3_invalid) {
        AI_NavGoal_t goal(GOALTYPE_COVER, GetOuter()->m_vInterruptSavePosition,
                          ACT_RUN, AIN_HULL_TOLERANCE, AIN_DEF_FLAGS);
  ...
  case TASK_MOVE_TO_FOLLOW_POSITION: {
    if (m_hFollowTarget == NULL) {
      ...
      if (!GetNavigator()->IsGoalActive()) {
        ...
        // tolerance So for now, just call it good and block another attempt for
        // a bit
        TaskComplete();
        if (!IsFollowPointInRange())
          ClearFollowPoint();
...
AI_FollowGroup_t *
CAI_FollowManager::FindCreateGroup(CBaseEntity *pTarget,
                                   AI_Formations_t formation) {
...
DEFINE_SCHEDULE(SCHED_MOVE_TO_FACE_FOLLOW_TARGET,
                ...
                "	Tasks"
                //		"		TASK_SET_ACTIVITY
                //ACTIVITY:ACT_IDLE" 		"		TASK_FACE_FOLLOW_TARGET
                //0" 		"		TASK_SET_ACTIVITY
                //ACTIVITY:ACT_IDLE"
                "		TASK_SET_SCHEDULE			"
...
DEFINE_SCHEDULE(SCHED_FOLLOWER_GO_TO_WAIT_POINT,
                ...
                "	Tasks"
                "		TASK_LOCK_HINTNODE			"
                "0		" // this will fail the schedule if no hint node
                                  // or not already lockable
                "		TASK_SET_FAIL_SCHEDULE		"
