...
// label, and a default value. Voila, dynamicly generated checkboxes on the
// Flags tab of Entity Properties.
...
public:
  ...
  bool m_bIgnoreNonPlayerEntsOnBlock; // Non-player entities should never block.
                                      // This variable needs more letters.
  ...
  COutputEvent m_OnFullyClosed; // Triggered when the door reaches the fully
                                // closed position.
  COutputEvent
      m_OnFullyOpen; // Triggered when the door reaches the fully open position.
...
private:
  ...
  void
  ChainTouch(CBaseEntity *pOther); ///< Chains touch on through to m_ChainTarget
  ...
  void
  CloseAreaPortalsThink(void); ///< Delays turning off area portals when closing
                               ///< doors to prevent visual artifacts
};
