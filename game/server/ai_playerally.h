...
#define TLK_PASSENGER_NEW_RADAR_CONTACT                                        \
  "TLK_PASSENGER_NEW_RADAR_CONTACT" // Noticed a brand new contact on the radar
#define TLK_PASSENGER_PUNTED                                                   \
  "TLK_PASSENGER_PUNTED" // The player has punted us while we're sitting in the
                         // vehicle
...
struct AISpeechSelection_t {
  AISpeechSelection_t() : pResponse(NULL) {}

  void Set(AIConcept_t newConcept, AI_Response *pNewResponse,
...
public:
  ...
  bool SelectAnswerFriend(CBaseEntity *pFriend, AISpeechSelection_t *pSelection,
  ...
  virtual bool SpeakIfAllowed(AIConcept_t concept, const char *modifiers = NULL,
...
protected:
  ...
  // Whether we are a vital ally (useful for wrting Classify() for classes that
  // are only sometimes vital, such as the Lone Vort in Ep2.) The usual means by
  // which any other function should determine if a character is vital is to
  // determine Classify() == CLASS_PLAYER_ALLY_VITAL. Do not use this function
...
private:
  ...
  EHANDLE m_hPotentialSpeechTarget; // NPC to tell the response rules about when
                                    // trying to find a response to talk to them
                                    // with
