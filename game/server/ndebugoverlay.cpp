...
// Output :
//-----------------------------------------------------------------------------
void UTIL_AddDebugLine(const Vector &startPos, const Vector &endPos,
  ...
  if (testLOS) {
    trace_t tr;
    UTIL_TraceLine(debugLine->origin, debugLine->dest, MASK_BLOCKLOS, NULL,
...
float GetLongFloorZ(const Vector &origin) {
  ...
  //
  trace_t tr;
  UTIL_TraceLine(origin, origin - Vector(0, 0, 2048), MASK_NPCSOLID_BRUSHONLY,
                 NULL, COLLISION_GROUP_NONE, &tr);

  // This trace is ONLY used if we hit an entity flagged with FL_WORLDBRUSH
  trace_t trEnt;
  UTIL_TraceLine(origin, origin - Vector(0, 0, 2048), MASK_NPCSOLID, NULL,
                 COLLISION_GROUP_NONE, &trEnt);
...
void UTIL_DrawPositioningOverlay(float flCrossDistance) {
  ...
  trace_t tr;
  Vector endPos;
  UTIL_TraceLine(pPlayer->EyePosition(), topPos, MASK_NPCSOLID_BRUSHONLY,
                 pPlayer, COLLISION_GROUP_NONE, &tr);
  if (tr.fraction == 1.0) {
    Vector rightTrace = topPos + pRight * 400;
    float traceLen = (topPos - rightTrace).Length();
    UTIL_TraceLine(topPos, rightTrace, MASK_NPCSOLID_BRUSHONLY, pPlayer,
                   COLLISION_GROUP_NONE, &tr);
    endPos = topPos + (pRight * traceLen * tr.fraction);
    NDebugOverlay::DrawTickMarkedLine(topPos, endPos, 24.0, 5, 255, 0, 0, false,
                                      0);

    Vector leftTrace = topPos - pRight * 400;
    traceLen = (topPos - leftTrace).Length();
    UTIL_TraceLine(topPos, leftTrace, MASK_NPCSOLID_BRUSHONLY, pPlayer,
                   COLLISION_GROUP_NONE, &tr);
    endPos = topPos - (pRight * traceLen * tr.fraction);
    NDebugOverlay::DrawTickMarkedLine(topPos, endPos, 24.0, 5, 255, 0, 0, false,
                                      0);
    ...
    UTIL_TraceLine(topPos, upTrace, MASK_NPCSOLID_BRUSHONLY, pPlayer,
                   COLLISION_GROUP_NONE, &tr);
    endPos = topPos + (Vector(0, 0, 1) * traceLen * tr.fraction);
    NDebugOverlay::DrawTickMarkedLine(bottomPos, endPos, 24.0, 5, 255, 0, 0,
                                      false, 0);

    // Draw white cross at center
    NDebugOverlay::Cross3D(topPos, Vector(-2, -2, -2), Vector(2, 2, 2), 255,
                           255, 255, true, 0);
  } else {
  ...
  if (m_nDebugOverlayIndex != -1) {
    for (int i = 0; i < NUM_DEBUG_OVERLAY_LINES; i++) {
      if (m_debugOverlayLine[i]->draw) {
        NDebugOverlay::Line(
            m_debugOverlayLine[i]->origin, m_debugOverlayLine[i]->dest,
            m_debugOverlayLine[i]->r, m_debugOverlayLine[i]->g,
            m_debugOverlayLine[i]->b, m_debugOverlayLine[i]->noDepthTest, 0);
      }
    }
  }
}
...
void DebugDrawLine(const Vector &vecAbsStart, const Vector &vecAbsEnd, int r,
  ...
  NDebugOverlay::Line(vecAbsStart + Vector(0, 0, 0.1),
                      vecAbsEnd + Vector(0, 0, 0.1), r, g, b, test, duration);
}
