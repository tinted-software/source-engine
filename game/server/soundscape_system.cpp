...
CON_COMMAND(soundscape_flush, "Flushes the server & client side soundscapes") {
  ...
  g_SoundscapeSystem
      .FlushSoundscapes(); // don't bother forgetting about the entities
        ...
        if (g_pDeveloper->GetBool()) {
          if (strstr(pKeys->GetName(), "{")) {
            Msg("Error parsing soundscape file %s after %s\n", filename,
                m_soundscapeCount > 0
                    ? m_soundscapes.GetStringText(m_soundscapeCount - 1)
                    : "FIRST");
          }
        }
...
CON_COMMAND_F(sv_soundscape_printdebuginfo, "print soundscapes",
...
void CSoundscapeSystem::PrintDebugInfo() {
  Msg("\n------- SERVER SOUNDSCAPES -------\n");
  for (int key = m_soundscapes.First(); key != m_soundscapes.InvalidIndex();
       key = m_soundscapes.Next(key)) {
  ...
  for (int entityIndex = 0; entityIndex < m_soundscapeEntities.Size();
       ++entityIndex) {
...
bool CSoundscapeSystem::Init() {
  ...
  if (manifest->LoadFromFile(filesystem, SOUNDSCAPE_MANIFEST_FILE, "GAME")) {
    for (KeyValues *sub = manifest->GetFirstSubKey(); sub != NULL;
      ...
      Warning("CSoundscapeSystem::Init:  Manifest '%s' with bogus file type "
              "'%s', expecting 'file'\n",
...
void CSoundscapeSystem::LevelInitPostEntity() {
  ...
  // add this soundscape to the list of soundscapes for that cluster, clip
  // cluster bounds to radius
  for (int i = 0; i < m_soundscapeEntities.Count(); i++) {
    ...
    engine->GetPVSForCluster(engine->GetClusterForOrigin(position),
                             sizeof(myPVS), myPVS);
    for (int j = 0; j < clusterCount; j++) {
      if (myPVS[j >> 3] & (1 << (j & 7))) {
        float distSq = CalcSqrDistanceToAABB(clusterbounds[j].mins,
                                             clusterbounds[j].maxs, position);
        if (distSq < radiusSq || radius < 0) {
          m_soundscapesInCluster[j].soundscapeCount++;
          clusterIndexList.AddToTail(j);
          // UNDONE: Technically you just need a soundscape index and a count
          // for this list.
  ...
  // The resulting list is precomputing all soundscapes that need to be checked
  // for a player in each cluster.  This is used to accelerate the per-frame
  // operations
  for (int i = 0; i < soundscapeIndexList.Count(); i++) {
    int cluster = clusterIndexList[i];
    int outIndex = m_soundscapesInCluster[cluster].soundscapeCount +
                   m_soundscapesInCluster[cluster].firstSoundscape;
...
void CSoundscapeSystem::FrameUpdatePostEntityThink() {
  ...
  if (total > 0) {
    int traceCount = 0;
    int playerCount = 0;
    // budget tuned for TF.  Do a max of 20 traces.  That's going to happen
    // anyway because a bunch of the maps use radius -1 for all soundscapes.  So
    // to trace one player you'll often need that many and this code must always
    // trace one player's soundscapes. If the map has been optimized, then allow
    // more players to update per frame.
    ...
    // load balance across server ticks a bit by limiting the numbers of players
    // (get cluster for origin) and traces processed in a single tick.  In
    // single player this will update the player every tick because it always
    // does at least one player's full load of work
    for (int i = 0; i < gpGlobals->maxClients && traceCount <= maxTraces &&
      ...
      if (pPlayer && pPlayer->IsNetClient()) {
        ...
        if (clusterIndex >= 0 &&
            clusterIndex < m_soundscapesInCluster.Count()) {
          // find all soundscapes that could possibly attach to this player and
          // update them
          for (int j = 0;
               j < m_soundscapesInCluster[clusterIndex].soundscapeCount; j++) {
            int ssIndex = m_soundscapeIndexList
                [m_soundscapesInCluster[clusterIndex].firstSoundscape + j];
...
void CSoundscapeSystem::AddSoundscapeSounds(KeyValues *pSoundscape,
                                            int soundscapeIndex) {
  if (!IsX360()) {
    return;
  }
