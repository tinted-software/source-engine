...
LINK_ENTITY_TO_CLASS(model_studio,
                     CGenericCycler); // For now model_studios build as cyclers.
...
//
void CCycler::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
...
void CWreckage::Think(void) {
  ...
  if (m_flDieTime) {
    if (m_flDieTime < gpGlobals->curtime) {
      UTIL_Remove(this);
      return;
    } else if (random->RandomFloat(0, m_flDieTime - m_flStartTime) >
  ...
  te->Smoke(filter, 0.0, &vecSrc, g_sModelIndexSmoke,
            random->RandomFloat(0, 4.9) + 5.0, random->RandomInt(0, 3) + 8);
}
...
DEFINE_FIELD(m_iLowerBound, FIELD_INTEGER),
  ...
  GenericCyclerSpawn((char *)STRING(GetModelName()), Vector(-16, -16, -16),
                     Vector(16, 16, 16));
  if (!m_iBlendspeed)
    m_iBlendspeed = 5;
