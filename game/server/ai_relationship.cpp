...
//
// Purpose: Entity which alters the relationships between entities via entity
// I/O
//
//=====================================================================================//
...
#define SF_RELATIONSHIP_NOTIFY_SUBJECT                                         \
  (1 << 0) // Alert the subject of the change and give them a memory of the
           // target entity
#define SF_RELATIONSHIP_NOTIFY_TARGET                                          \
  (1 << 1) // Alert the target of the change and give them a memory of the
           // subject entity
...
public:
  ...
  void ChangeRelationships(int disposition, int iReverting,
                           CBaseEntity *pActivator = NULL,
                           CBaseEntity *pCaller = NULL);

  void ApplyRelationship(CBaseEntity *pActivator = NULL,
                         CBaseEntity *pCaller = NULL);
  void RevertRelationship(CBaseEntity *pActivator = NULL,
                          CBaseEntity *pCaller = NULL);
  void RevertToDefaultRelationship(CBaseEntity *pActivator = NULL,
                                   CBaseEntity *pCaller = NULL);
...
private:
  ...
  CBaseEntity *FindEntityForProceduralName(string_t iszName,
                                           CBaseEntity *pActivator,
                                           CBaseEntity *pCaller);
  void DiscloseNPCLocation(CBaseCombatCharacter *pSubject,
...
DEFINE_THINKFUNC(ApplyRelationshipThink),
    ...
    DEFINE_INPUTFUNC(FIELD_VOID, "RevertToDefaultRelationship",
...
void CAI_Relationship::Activate() {
  if (m_fStartActive) {
    ...
    // Clear this flag so that nothing happens when the level is loaded (which
...
//---------------------------------------------------------
//---------------------------------------------------------
void CAI_Relationship::InputRevertToDefaultRelationship(
...
//			spawned, after all the NPCs have spawned.  Once that
//occurs, this 			function terminates.
//-----------------------------------------------------------------------------
...
//---------------------------------------------------------
void CAI_Relationship::ApplyRelationship(CBaseEntity *pActivator,
                                         CBaseEntity *pCaller) {
  // @TODO (toml 10-22-04): sort out MP relationships

  // The player spawns slightly after the NPCs, meaning that if we don't wait,
  // the player will miss any relationships placed on them.
...
//---------------------------------------------------------
//---------------------------------------------------------
void CAI_Relationship::RevertRelationship(CBaseEntity *pActivator,
                                          CBaseEntity *pCaller) {
  if (m_bIsActive) {
    ChangeRelationships(m_iPreviousDisposition, REVERTING_TO_PREV, pActivator,
                        pCaller);
    SetActive(false);
  }
}
...
//---------------------------------------------------------
//---------------------------------------------------------
void CAI_Relationship::RevertToDefaultRelationship(CBaseEntity *pActivator,
                                                   CBaseEntity *pCaller) {
  if (m_bIsActive) {
...
void CAI_Relationship::OnEntitySpawned(CBaseEntity *pEntity) {
  // NOTE: This cannot use the procedural entity finding code since that only
  // occurs on
...
CBaseEntity *CAI_Relationship::FindEntityForProceduralName(
    string_t iszName, CBaseEntity *pActivator, CBaseEntity *pCaller) {
...
//-----------------------------------------------------------------------------
// Purpose: Disclose the location of the target entity to the subject via a
// memory Input  : *pSubject - Entity to gain the memory of the target's
// location
//			*pTarget - Entity who's location will be disclosed
//-----------------------------------------------------------------------------
void CAI_Relationship::DiscloseNPCLocation(CBaseCombatCharacter *pSubject,
                                           CBaseCombatCharacter *pTarget) {
...
//---------------------------------------------------------
//---------------------------------------------------------
void CAI_Relationship::ChangeRelationships(int disposition, int iReverting,
                                           CBaseEntity *pActivator,
                                           CBaseEntity *pCaller) {
  ...
  CBaseEntity *pSpecialSubject =
      FindEntityForProceduralName(m_iszSubject, pActivator, pCaller);
  ...
  CBaseEntity *pSpecialTarget =
      FindEntityForProceduralName(m_target, pActivator, pCaller);
  ...
  for (int i = 1; i <= gpGlobals->maxClients; i++) {
    if (subjectList.Count() == MAX_HANDLED ||
        targetList.Count() == MAX_HANDLED) {
    ...
    if (pPlayer) {
      if (IsASubject(pPlayer)) {
        if (m_flRadius == 0.0 ||
            GetAbsOrigin().DistToSqr(pPlayer->GetAbsOrigin()) <= radiusSq)
  ...
  for (int i = 0; i < g_AI_Manager.NumAIs(); i++) {
    ...
    if (pNPC) {
      if (IsASubject(pNPC)) {
        if (m_flRadius == 0.0 ||
            GetAbsOrigin().DistToSqr(pNPC->GetAbsOrigin()) <= radiusSq)
  ...
  for (int i = 0; i < subjectList.Count(); i++) {
    ...
    for (int j = 0; j < targetList.Count(); j++) {
      ...
      if (iReverting == REVERTING_TO_PREV) {
        pSubject->AddEntityRelationship(
            pTarget, (Disposition_t)m_iPreviousDisposition, m_iPreviousRank);

        if (m_bReciprocal) {
          pTarget->AddEntityRelationship(
              pSubject, (Disposition_t)m_iPreviousDisposition, m_iPreviousRank);
        }
        ...
        pSubject->AddEntityRelationship(pTarget, (Disposition_t)disposition,
                                        m_iRank);
        ...
        if (m_bReciprocal) {
          ...
          pTarget->AddEntityRelationship(pSubject, (Disposition_t)disposition,
                                         m_iRank);
        }
      }
    }
