...
AIMotorMoveResult_t
CAI_BlendedMotor::MoveGroundExecute(const AILocalMoveGoal_t &move,
...
AIMotorMoveResult_t
CAI_BlendedMotor::MoveFlyExecute(const AILocalMoveGoal_t &move,
                                 AIMoveTrace_t *pTraceResult) {
...
ConVar
    ai_path_insert_pause_at_obstruction("ai_path_insert_pause_at_obstruction",
                                        "1");
...
void CAI_BlendedMotor::BuildVelocityScript(const AILocalMoveGoal_t &move) {
  ...
  if (npc_height_adjust.GetBool() && move.bHasTraced &&
    ...
    m_flPredictiveSpeedAdjust =
        clamp(m_flPredictiveSpeedAdjust, (flHeight > 0.0f) ? 0.5f : 0.8f, 1.0f);
  ...
  if (npc_height_adjust.GetBool()) {
    ...
    newSpeedAdjust =
        clamp(newSpeedAdjust, (flHeight > 0.0f) ? 0.5f : 0.8f, 1.0f);

    // debounce speed adjust
    if (newSpeedAdjust < m_flReactiveSpeedAdjust) {
      m_flReactiveSpeedAdjust =
          m_flReactiveSpeedAdjust * 0.2f + newSpeedAdjust * 0.8f;
    } else {
      m_flReactiveSpeedAdjust =
          m_flReactiveSpeedAdjust * 0.5f + newSpeedAdjust * 0.5f;
    }
  ...
  while (
      ...
      (pCurWaypoint->NavType() == NAV_GROUND ||
       pCurWaypoint->NavType() ==
           NAV_FLY) /*&& flTotalDist / idealVelocity < 3.0*/) // limit lookahead
                                                              // to 3 seconds
  {
    script.Init();
    ...
    if (pNext) {
      ...
      case NAV_JUMP:
        ...
        // FIXME: information about what the jump should look like isn't stored
        // in the waypoints this'll need to call
        //    GetMoveProbe()->MoveLimit( NAV_JUMP, GetLocalOrigin(),
        //    GetPath()->CurWaypointPos(), MASK_NPCSOLID, GetNavTargetEntity(),
        //    &moveTrace );
        // to get how far/fast the jump will be, but this is also stateless, so
        // it'd call it per frame. So far it's not clear that the moveprobe
