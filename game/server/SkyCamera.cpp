...
//-----------------------------------------------------------------------------
CSkyCamera *GetCurrentSkyCamera() { return g_SkyList.m_pClassList; }

CSkyCamera *GetSkyCameraList() { return g_SkyList.m_pClassList; }
...
DEFINE_KEYFIELD(m_skyboxData.scale, FIELD_INTEGER, "scale"),
    ...
    DEFINE_KEYFIELD(m_skyboxData.fog.colorSecondary, FIELD_COLOR32,
                    "fogcolor2"),
...
void CSkyCamera::Activate() {
  ...
  // secondary fog color when blending was enabled. The bug is fixed, but to
  // make the maps look the same as before the bug fix without having to
  // download new maps, I have to cheat here and slam the primary and secondary
  // colors to be the average of the primary and secondary colors.
    ...
    for (int i = 0; s_pBogusFogMaps[i]; ++i) {
      if (!Q_stricmp(s_pBogusFogMaps[i], STRING(gpGlobals->mapname))) {
        m_skyboxData.fog.colorPrimary.SetR(
            (m_skyboxData.fog.colorPrimary.GetR() +
             m_skyboxData.fog.colorSecondary.GetR()) *
            0.5f);
        m_skyboxData.fog.colorPrimary.SetG(
            (m_skyboxData.fog.colorPrimary.GetG() +
             m_skyboxData.fog.colorSecondary.GetG()) *
            0.5f);
        m_skyboxData.fog.colorPrimary.SetB(
            (m_skyboxData.fog.colorPrimary.GetB() +
             m_skyboxData.fog.colorSecondary.GetB()) *
            0.5f);
        m_skyboxData.fog.colorPrimary.SetA(
            (m_skyboxData.fog.colorPrimary.GetA() +
             m_skyboxData.fog.colorSecondary.GetA()) *
            0.5f);
