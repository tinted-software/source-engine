...
enum DesiredWeaponState_t {
  ...
  DESIREDWEAPONSTATE_CHANGING_DESTROY, // Destroy the weapon when this change is
                                       // complete.
};
...
public:
  ...
  //
  //-----------------------------------------------------
  template <class BEHAVIOR_TYPE> bool GetBehavior(BEHAVIOR_TYPE **ppBehavior) {
...
public:
  ...
  virtual void
  OnStateChange(NPC_STATE OldState,
...
protected:
  ...
  const char *
  GetScriptedNPCInteractionSequence(ScriptedNPCInteraction_t *pInteraction,
                                    int iPhase);
...
public:
  ...
  virtual bool
  OverrideMove(float flInterval); // Override to take total control of movement
  ...
  CHandle<CBasePropDoor> m_hOpeningDoor; // The CBasePropDoor that we are in the
                                         // midst of opening for navigation.
...
public:
  ...
  virtual CBaseEntity *
  EyeLookTarget(void); // Overridden by subclass to force look at an entity
...
protected:
  float m_flOriginalYaw; // This is the direction facing when the level designer
                         // placed the NPC in the level.
...
public:
  ...
  virtual void
  SetScriptedScheduleIgnoreConditions(Interruptability_t interrupt);
...
public:
  ...
  enum SCRIPTSTATE {
    ...
    SCRIPT_WAIT, // Waiting on everyone in the script to be ready. Plays the pre
                 // idle animation if there is one.
...
protected:
  CAI_Enemies *m_pEnemies; // Holds information about enemies / danger positions
                           // / shared between sqaud members
  int m_afMemory;
...
protected:
  ...
  int m_iMySquadSlot; // this is the behaviour slot that the npc currently holds
                      // in the squad.
...
public:
  ...
  Vector m_vecLastPosition; // npc sometimes wants to return to where it started
                            // after an operation.
