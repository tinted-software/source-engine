...
DEFINE_THINKFUNC(EvaluationThink),
    ...
    DEFINE_EMBEDDED(m_Timer), DEFINE_EMBEDDED(m_Timeout),
    END_DATADESC()
...
#define EVALUATOR(name) {&CAI_ScriptConditions::Eval##name, #name}
        ...
        CAI_ScriptConditions::EvaluatorInfo_t
...
bool CAI_ScriptConditions::EvalState(const EvalArgs_t &args) {
  ...
  if (valLow > valHigh) {
    DevMsg("Script condition warning: Invalid setting for Maximum/Minimum "
           "state\n");
    Disable();
    return false;
  }
...
void CAI_ScriptConditions::EvaluationThink() {
  ...
  if (AI_GetSinglePlayer()->GetFlags() & FL_NOTARGET) {
    ScrCondDbgMsg(("%s WARNING: Player is NOTARGET. This will affect all LOS "
                   "conditiosn involving the player!\n",
                   GetDebugName()));
  }
#endif
  ...
  for (int i = 0; i < m_ElementList.Count();) {
    ...
    if (m_Actor != NULL_STRING && !pActor) {
      if (m_ElementList.Count() == 1) {
        DevMsg("Warning: Active AI script conditions associated with an "
...
void CAI_ScriptConditions::Enable(void) {
  ...
  if (m_ElementList.Count() == 0) {
    if (m_Actor != NULL_STRING && pActor == NULL) {
      DevMsg("Warning: Spawning AI script conditions (%s) associated with an "
             "non-existant NPC\n",
             GetDebugName());
      m_NoValidActors.FireOutput(this, this, 0);
      Disable();
