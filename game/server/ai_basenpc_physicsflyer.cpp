...
DEFINE_FIELD(m_vCurrentVelocity, FIELD_VECTOR),
    ...
    // Output  :
    //------------------------------------------------------------------------------
    void CAI_BasePhysicsFlyingBot::GetVelocity(Vector *vVelocity,
...
// Output :
//-----------------------------------------------------------------------------
void CAI_BasePhysicsFlyingBot::TurnHeadToTarget(float flInterval,
                                                const Vector &MoveTarget) {
  ...
  // RIGHT NOW, this affects every flying bot. This rate should be member data
  // that individuals can manipulate. THIS change for MANHACKS E3 2003 (sjb)
  float iRate = 0.8;
...
// Output  :
//------------------------------------------------------------------------------
float CAI_BasePhysicsFlyingBot::MinGroundDist(void) { return 0; }
...
Vector CAI_BasePhysicsFlyingBot::VelocityToAvoidObstacles(float flInterval) {
  ...
  AI_TraceEntity(this, GetAbsOrigin(), endPos, MASK_NPCSOLID | CONTENTS_WATER,
                 &tr);
  if (tr.fraction != 1.0) {
  ...
  AI_TraceLine(GetAbsOrigin(), GetAbsOrigin() + Vector(0, 0, -flMinGroundDist),
               MASK_NPCSOLID_BRUSHONLY | CONTENTS_WATER, this,
...
void CAI_BasePhysicsFlyingBot::MoveToTarget(float flInterval,
                                            const Vector &MoveTarget) {
...
AI_NavPathProgress_t CAI_BasePhysicsFlyingBot::ProgressFlyPath(
    ...
    bool bNewTrySimplify, float strictPointTolerance) {
  ...
  case AINPP_BLOCKED: // function is not supposed to test blocking, just simple
                      // path progression
  default: {
...
//			&chasePosition -
//-----------------------------------------------------------------------------
void CAI_BasePhysicsFlyingBot::TranslateNavGoal(CBaseEntity *pTarget,
...
CAI_BasePhysicsFlyingBot::Simulate(IPhysicsMotionController *pController,
  ...
  */
  ...
  angular.x = (UTIL_AngleDiff(m_vCurrentBanking.z, actualAngles.z) -
               actualAngularVelocity.x) *
              (1 / deltaTime);
  angular.y = (UTIL_AngleDiff(m_vCurrentBanking.x, actualAngles.x) -
               actualAngularVelocity.y) *
              (1 / deltaTime);

  // turn toward target
  angular.z = UTIL_AngleDiff(m_fHeadYaw,
                             actualAngles.y + actualAngularVelocity.z * 0.1) *
              (1 / deltaTime);

  // angular = m_vCurrentAngularVelocity - actualAngularVelocity;

  // DevMsg("Sim %d : %.1f %.1f %.1f (%.1f)\n", count++, actualAngles.x,
  // actualAngles.y, actualAngles.z, m_fHeadYaw );
