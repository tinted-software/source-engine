...
// TODO: Was changed from 15, update when latest 360 code is integrated (MSB
// 5/5/09)
...
char *GetBspFilename(const char *navFilename) {
  ...
  Q_snprintf(bspFilename, sizeof(bspFilename), FORMAT_BSPFILE,
...
void CNavArea::Save(CUtlBuffer &fileBuffer, unsigned int version) const {
  ...
  unsigned int id = (m_inheritVisibilityFrom.area)
                        ? m_inheritVisibilityFrom.area->GetID()
                        : 0;
  fileBuffer.PutUnsignedInt(id);
}
 ...
 */
NavErrorType CNavArea::Load(CUtlBuffer &fileBuffer, unsigned int version,
  ...
  if (!IsX360()) {
    ...
    /* TODO: Re-enable when latest 360 code gets integrated (MSB 5/5/09)
                    ...
                    new ( &m_potentiallyVisibleAreas ) CAreaBindInfoArray(
       (AreaBindInfo *)engine->AllocLevelStaticData( nBytes ), visibleAreaCount
       );
    */
  }
...
NavErrorType CNavArea::PostLoad(void) {
  ...
  m_inheritVisibilityFrom.area =
      TheNavMesh->GetNavAreaByID(m_inheritVisibilityFrom.id);
...
float NavAreaTravelDistance(const Vector &startPos, const Vector &goalPos,
  ...
  if (NavAreaBuildPath(startArea, NULL, &goalPos, costFunc, &goalArea) ==
      false) {
    return -1.0f;
  }
  ...
  if (goalArea->GetParent() == NULL) {
    ...
    // goalPos is assumed to be inside goalArea (or very close to it) - skip to
    // next area
...
const char *CNavMesh::GetFilename(void) const {
  ...
  Q_snprintf(filename, sizeof(filename), "%s\\" FORMAT_NAVFILE, gamePath,
             STRING(gpGlobals->mapname));
...
bool CNavMesh::Save(void) const {
  ...
  // 10 - Added sub-version number to allow derived classes to have custom area
  // data 11 - Added light intensity to each area 12 - Storing presence of
  // unnamed areas in the PlaceDirectory 13 - Widened NavArea attribute bits
  // from unsigned short to int 14 - Added a bool for if the nav needs analysis
  // 15 - removed approach areas
  // 16 - Added visibility data to the base mesh
  fileBuffer.PutUnsignedInt(NavCurrentVersion);

  // The sub-version number is maintained and owned by classes derived from
  // CNavMesh and CNavArea and allows them to track their custom data just as we
  // do at this top level
...
static ConCommand
    nav_check_file_consistency("nav_check_file_consistency",
                               CommandNavCheckFileConsistency,
                               "Scans the maps directory and reports any "
                               "missing/out-of-date navigation files.",
                               FCVAR_GAMEDLL | FCVAR_CHEAT);

//--------------------------------------------------------------------------------------------------------------
/**
 * Reads the used place names from the nav file (can be used to selectively
 * precache before the nav is loaded)
 */
const CUtlVector<Place> *
CNavMesh::GetPlacesFromNavFile(bool *hasUnnamedPlaces) {
  placeDirectory.Reset();
  // nav filename is derived from map filename
  char filename[256];
  Q_snprintf(filename, sizeof(filename), FORMAT_NAVFILE,
             STRING(gpGlobals->mapname));

  CUtlBuffer fileBuffer(4096, 1024 * 1024, CUtlBuffer::READ_ONLY);
  if (!filesystem->ReadFile(
          filename, "GAME",
          fileBuffer)) // this ignores .nav files embedded in the .bsp ...
  {
    if (!filesystem->ReadFile(filename, "BSP",
                              fileBuffer)) // ... and this looks for one if it's
                                           // the only one around.
    {
      return NULL;
    }
...
NavErrorType CNavMesh::Load(void) {
  ...
  if (version >= 4) {
    ...
    if (bspSize != saveBspSize && !navIsInBsp) {
      if (engine->IsDedicatedServer()) {
        ...
        // Msg instead
        DevMsg("The Navigation Mesh was built using a different version of "
               "this map.\n");
      } else {
        DevWarning("The Navigation Mesh was built using a different version of "
                   "this map.\n");
      }
      m_isOutOfDate = true;
    }
  ...
  // mark stairways (TODO: this can be removed once all maps are re-saved with
  // this attribute in them)
  MarkStairAreas();
...
NavErrorType CNavMesh::PostLoad(unsigned int version) {
  ...
  //
  // Allow each nav area to know what other areas have one-way connections to
  // it. Need to gather then sort due to allocation restrictions on the 360
  //
  ...
  FOR_EACH_VEC(TheNavAreas, oit) {
    ...
    for (int d = 0; d < NUM_DIRECTIONS; d++) {
      const NavConnectVector *connectList =
          oneWayLink.area->GetAdjacentAreas((NavDirType)d);

      FOR_EACH_VEC((*connectList), it) {
        NavConnect connect = (*connectList)[it];
        oneWayLink.destArea = connect.area;

        // if the area we connect to has no connection back to us, allow that
        // area to remember us as an incoming connection
  ...
  for (int i = 0; i < oneWayLinks.Count(); i++) {
    ...
    oneWayLinks[i].destArea->AddIncomingConnection(
        oneWayLinks[i].area, (NavDirType)oneWayLinks[i].backD);
  }

  ValidateNavAreaConnections();

  // TERROR: loading into a map directly creates entities before the mesh is
  // loaded.  Tell the preexisting entities now that the mesh is loaded so they
  // can update areas.
