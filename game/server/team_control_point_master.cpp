...
DEFINE_KEYFIELD(m_bDisabled, FIELD_BOOLEAN, "StartDisabled"),
    ...
    DEFINE_KEYFIELD(m_iInvalidCapWinner, FIELD_INTEGER,
    ...
    DEFINE_KEYFIELD(m_flPartialCapturePointsRate, FIELD_FLOAT,
    ...
    //* > ), 	DEFINE_FIELD( m_iCurrentRoundIndex, FIELD_INTEGER ), 	DEFINE_ARRAY(
    //m_iszTeamBaseIcons, FIELD_STRING, MAX_TEAMS ), 	DEFINE_ARRAY(
    //m_iTeamBaseIcons, FIELD_INTEGER, MAX_TEAMS ), 	DEFINE_FIELD(
    //m_bFirstRoundAfterRestart, FIELD_BOOLEAN ),

    DEFINE_INPUTFUNC(FIELD_VOID, "Enable", InputEnable),
    DEFINE_INPUTFUNC(FIELD_VOID, "Disable", InputDisable),

    DEFINE_INPUTFUNC(FIELD_INTEGER, "SetWinner", InputSetWinner),
    DEFINE_INPUTFUNC(FIELD_INTEGER, "SetWinnerAndForceCaps",
                     InputSetWinnerAndForceCaps),
    DEFINE_INPUTFUNC(FIELD_VOID, "RoundSpawn", InputRoundSpawn),
    DEFINE_INPUTFUNC(FIELD_VOID, "RoundActivate", InputRoundActivate),
    DEFINE_INPUTFUNC(FIELD_STRING, "SetCapLayout", InputSetCapLayout),
    DEFINE_INPUTFUNC(FIELD_FLOAT, "SetCapLayoutCustomPositionX",
                     InputSetCapLayoutCustomPositionX),
    DEFINE_INPUTFUNC(FIELD_FLOAT, "SetCapLayoutCustomPositionY",
    ...
    END_DATADESC()
        ...
        LINK_ENTITY_TO_CLASS(team_control_point_master,
                             CTeamControlPointMaster);

ConVar mp_time_between_capscoring(
    "mp_time_between_capscoring", "30", FCVAR_GAMEDLL,
    "Delay between scoring of owned capture points.", true, 1, false, 0);

// sort function for the list of control_point_rounds (we're sorting them by
// priority...highest first)
int ControlPointRoundSort(CTeamControlPointRound *const *p1,
                          CTeamControlPointRound *const *p2) {
  // check the priority
...
void CTeamControlPointMaster::Spawn(void) {
  ...
  if (g_hControlPointMasters.Find(this) ==
      g_hControlPointMasters.InvalidIndex()) {
...
// Purpose:
//-----------------------------------------------------------------------------
bool CTeamControlPointMaster::KeyValue(const char *szKeyName,
                                       const char *szValue) {
...
bool CTeamControlPointMaster::FindControlPoints(void) {
  ...
  CBaseEntity *pEnt =
      gEntList.FindEntityByClassname(NULL, GetControlPointName());
  ...
  while (pEnt) {
    ...
    if (pPoint->IsActive() && !pPoint->IsMarkedForDeletion()) {
      ...
      if (m_ControlPoints.Find(index) == m_ControlPoints.InvalidIndex()) {
        DevMsg(2,
               "**** Adding control point %s with index %d to control point "
               "master\n",
               pPoint->GetName(), index);
        m_ControlPoints.Insert(index, pPoint);
        numFound++;
      } else {
        Warning("!!!!\nMultiple control points with the same index, duplicates "
                "ignored\n!!!!\n");
        UTIL_Remove(pPoint);
      }
    }
  ...
  for (unsigned int i = 0; i < m_ControlPoints.Count(); i++) {
    ...
    for (int team = 0; team < GetNumberOfTeams(); team++) {
      ObjectiveResource()->SetCPIcons(iPointIndex, team,
                                      pPoint->GetHudIconIndexForTeam(team));
      ObjectiveResource()->SetCPOverlays(
          iPointIndex, team, pPoint->GetHudOverlayIndexForTeam(team));
      for (int prevpoint = 0; prevpoint < MAX_PREVIOUS_POINTS; prevpoint++) {
        ObjectiveResource()->SetPreviousPoint(
            iPointIndex, team, prevpoint,
...
bool CTeamControlPointMaster::FindControlPointRounds(void) {
  ...
  CBaseEntity *pEnt =
      gEntList.FindEntityByClassname(NULL, GetControlPointRoundName());

  while (pEnt) {
    CTeamControlPointRound *pRound =
        assert_cast<CTeamControlPointRound *>(pEnt);

    if (pRound && (m_ControlPointRounds.Find(pRound) ==
                   m_ControlPointRounds.InvalidIndex())) {
  ...
  if (g_pObjectiveResource) {
    ...
    g_pObjectiveResource->SetCapLayoutCustomPosition(m_flCustomPositionX,
                                                     m_flCustomPositionY);
  }
...
bool CTeamControlPointMaster::IsInRound(CTeamControlPoint *pPoint) {
  ...
  if (m_ControlPointRounds.Count() > 0 && m_iCurrentRoundIndex != -1) {
    return m_ControlPointRounds[m_iCurrentRoundIndex]->IsControlPointInRound(
        pPoint);
  }
...
bool CTeamControlPointMaster::SelectSpecificRound(void) {
  ...
  if (pRules) {
    if (pRules->GetRoundToPlayNext() != NULL_STRING) {
      ...
      pRound = dynamic_cast<CTeamControlPointRound *>(gEntList.FindEntityByName(
          NULL, STRING(pRules->GetRoundToPlayNext())));

      if (pRound) {
        if ((m_ControlPointRounds.Find(pRound) ==
             m_ControlPointRounds.InvalidIndex()) ||
...
void CTeamControlPointMaster::RegisterRoundBeingPlayed(void) {
  ...
  if (pRules) {
    string_t iszEntityName =
        m_ControlPointRounds[m_iCurrentRoundIndex]->GetEntityName();
...
bool CTeamControlPointMaster::GetControlPointRoundToPlay(void) {
  ...
  // we have a priority value, now we need to randomly pick a round with this
  // priority that's playable
  int nPriority = m_ControlPointRounds[i]->GetPriorityValue();
  CUtlVector<int> nRounds;

  CTeamplayRoundBasedRules *pRules =
      dynamic_cast<CTeamplayRoundBasedRules *>(GameRules());
  string_t iszLastRoundPlayed =
      pRules ? pRules->GetLastPlayedRound() : NULL_STRING;
  int iLastRoundPlayed = -1;

  string_t iszFirstRoundPlayed =
      pRules ? pRules->GetFirstRoundPlayed() : NULL_STRING;
  ...
  for (i = 0; i < m_ControlPointRounds.Count(); ++i) {
    ...
    if (pRound) {
      ...
      if (pRound->IsPlayable() && pRound->GetPriorityValue() == nPriority) {
        if (iszLastRoundPlayed ==
            iszRoundName) // is this the last round we played?
        {
          iLastRoundPlayed = i;
        }
        ...
        if (m_bFirstRoundAfterRestart) {
          ...
          if ((iszFirstRoundPlayed != NULL_STRING) &&
              (iszFirstRoundPlayed == iszRoundName)) {
            iFirstRoundPlayed = i;
          }
        }
  ...
  // if we have more than one and the last played round is in our list, remove
  // it
  if (nRounds.Count() > 1) {
  ...
  // if this is the first round after a full restart, we still have more than
  // one round in our list, and the first played round (after the last full
  // restart) is in our list, remove it
  if (m_bFirstRoundAfterRestart) {
    if (nRounds.Count() > 1) {
  ...
  // pick one to play but try to avoid picking one that we have recently played
  // if there are other rounds to play
  int index = random->RandomInt(0, nRounds.Count() - 1);

  // only need to check this if we have more than one round with this priority
  // value
  if (pRules && nRounds.Count() > 1) {
    // keep picking a round until we find one that's not a previously played
    // round or until we don't have any more rounds to choose from
    while (pRules->IsPreviouslyPlayedRound(
               m_ControlPointRounds[nRounds[index]]->GetEntityName()) &&
           nRounds.Count() > 1) {
      nRounds.Remove(index); // we have played this round recently so get it out
                             // of the list
...
//-----------------------------------------------------------------------------
// Purpose: Called every 0.1 seconds and checks the status of all the control
// points if one team owns them all, it gives points and resets Think also gives
// the time based points at the specified time intervals
//-----------------------------------------------------------------------------
void CTeamControlPointMaster::CPMThink(void) {
  if (m_bDisabled || !TeamplayGameRules()->PointsMayBeCaptured()) {
    SetContextThink(&CTeamControlPointMaster::CPMThink,
                    gpGlobals->curtime + 0.2, CPM_THINK);
    return;
  }
  ...
  SetContextThink(&CTeamControlPointMaster::CPMThink, gpGlobals->curtime + 0.2,
                  CPM_THINK);
}
...
void CTeamControlPointMaster::CheckWinConditions(void) {
  ...
  if (m_ControlPointRounds.Count() > 0) {
    if (m_iCurrentRoundIndex != -1) {
      ...
      int iWinners =
          m_ControlPointRounds[m_iCurrentRoundIndex]->CheckWinConditions();
      if (iWinners != -1 && iWinners >= FIRST_GAME_TEAM) {
        bool bForceMapReset = (NumPlayableControlPointRounds() ==
                               0); // are there any more rounds to play?

        if (!bForceMapReset) {
          // we have more rounds to play
          TeamplayGameRules()->SetWinningTeam(
              iWinners, WINREASON_ALL_POINTS_CAPTURED, bForceMapReset);
        } else {
    ...
    if ((m_iInvalidCapWinner != 1) && (iWinners >= FIRST_GAME_TEAM) &&
      ...
      if (TFGameRules() && TFGameRules()->IsInKothMode()) {
        ...
        if (pTimer) {
          if (pTimer->GetTimeRemaining() > 0 ||
              TFGameRules()->TimerMayExpire() == false) {
            bWinner = false;
          }
        }
      }
      ...
      if (bWinner) {
        TeamplayGameRules()->SetWinningTeam(
            iWinners, WINREASON_ALL_POINTS_CAPTURED, true, m_bSwitchTeamsOnWin);
        FireTeamWinOutput(iWinners);
      }
    }
  }
...
void CTeamControlPointMaster::InputSetWinnerAndForceCaps(inputdata_t &input) {
  ...
  for (unsigned int i = 0; i < m_ControlPoints.Count(); i++) {
    CTeamControlPoint *pPoint = m_ControlPoints[i];
    if (pPoint && (!PlayingMiniRounds() || ObjectiveResource()->IsInMiniRound(
...
void CTeamControlPointMaster::InternalSetWinner(int iTeam) {
  ...
  if (iTeam == TEAM_UNASSIGNED) {
    ...
    if (!bForceMapReset) {
      TeamplayGameRules()->SetWinningTeam(iTeam, WINREASON_ALL_POINTS_CAPTURED,
                                          bForceMapReset);
    } else {
      TeamplayGameRules()->SetWinningTeam(iTeam, WINREASON_ALL_POINTS_CAPTURED,
                                          bForceMapReset, m_bSwitchTeamsOnWin);
    }
...
void CTeamControlPointMaster::HandleRandomOwnerControlPoints(void) {
  ...
  // loop through and find all of the points that want random owners after a
  // full restart
  ...
  // now loop through and mix up the owners (if we found any points with this
  // flag set)
...
void CTeamControlPointMaster::InputRoundSpawn(inputdata_t &input) {
  ...
  if (FindControlPoints()) {
    /*		if ( m_bFirstRoundAfterRestart )
                    {
                            CTeamplayRoundBasedRules *pRules =
       dynamic_cast<CTeamplayRoundBasedRules*>( GameRules() ); if ( pRules && (
       pRules->GetRoundToPlayNext() == NULL_STRING ) )
                            {
                                    // we only want to handle the random points
       if we don't have a specific round to play next
                                    // (prevents points being randomized again
       after "waiting for players" has finished and we're going to play the same
       round) HandleRandomOwnerControlPoints();
                            }
                    }
    */
    SetContextThink(&CTeamControlPointMaster::CPMThink,
                    gpGlobals->curtime + 0.1, CPM_THINK);
  }
...
void CTeamControlPointMaster::InputRoundActivate(inputdata_t &input) {
  ...
  if (PlayingMiniRounds()) {
    // Tell the objective resource what control points are in use in the
    // selected mini-round
    ...
    if (pRound) {
      for (unsigned int i = 0; i < m_ControlPoints.Count(); i++) {
        CTeamControlPoint *pPoint = m_ControlPoints[i];
        if (pPoint) {
          ObjectiveResource()->SetInMiniRound(
              pPoint->GetPointIndex(), pRound->IsControlPointInRound(pPoint));
        }
      }
    }
  }
...
// Purpose:
//-----------------------------------------------------------------------------
void CTeamControlPointMaster::InputSetCapLayoutCustomPositionX(
...
// Purpose:
//-----------------------------------------------------------------------------
void CTeamControlPointMaster::InputSetCapLayoutCustomPositionY(
    inputdata_t &inputdata) {
...
//			If its not the case that one team owns them all, it
//returns 0. 			CPs are broken into groups. A team can win by owning all flags
//within a single group.
//
//			Can be passed an overriding team. If this is not null,
//the passed team 			number will be used for that cp. Used to predict if that CP
//changing would 			win the game.
//-----------------------------------------------------------------------------
int CTeamControlPointMaster::TeamOwnsAllPoints(
    CTeamControlPoint *pOverridePoint /* = NULL */,
  ...
  for (i = 0; i < m_ControlPoints.Count(); i++) {
    ...
    // if we find another one in the group that isn't the same owner, set the
    // win to false
...
// Purpose:
//-----------------------------------------------------------------------------
bool CTeamControlPointMaster::WouldNewCPOwnerWinGame(CTeamControlPoint *pPoint,
                                                     int iNewOwner) {
...
int CTeamControlPointMaster::GetBaseControlPoint(int iTeam) {
  ...
  for (int i = 0; i < (int)m_ControlPoints.Count(); i++) {
    ...
    if (PlayingMiniRounds() && iTeam > LAST_SHARED_TEAM) {
      ...
      if (iPointIndex == 0 ||
          iPointIndex == (((int)m_ControlPoints.Count()) - 1)) {
        iRetVal = iPointIndex;
        break;
      }
    }
    ...
    if (nLowestValue != 999 && nHighestValue != -1) {
      ...
      if ((pLowestPoint->GetDefaultOwner() == iTeam &&
           pHighestPoint->GetDefaultOwner() ==
               iTeam) || // if the same team owns both, take the highest value
...
int CTeamControlPointMaster::CalcNumRoundsRemaining(int iTeam) {
  // To determine how many rounds remain for a given team if it consistently
  // wins mini-rounds, we have to simulate forward each mini-round and track the
  // control point ownership that would result
  ...
  while (vecControlPointsOwned.Count() < (int)m_ControlPoints.Count()) {
    ...
    for (int i = 0; i < m_ControlPointRounds.Count(); ++i) {
      ...
      int iControlPoint;
      for (iControlPoint = 0; iControlPoint < pRound->m_ControlPoints.Count();
           iControlPoint++) {
        CTeamControlPoint *pControlPoint =
            pRound->m_ControlPoints[iControlPoint];
        int iControlPointOwningTeam = TEAM_INVALID;

        // determine who owns this control point.
        // First, check our simulated ownership
        if (vecControlPointsOwned.InvalidIndex() !=
            vecControlPointsOwned.Find(pControlPoint)) {
        ...
        if (0 == iControlPoint) {
          // if this is the first control point, assign ownership to the team
          // that owns this control point
          iRoundOwningTeam = iControlPointOwningTeam;
        } else {
          // for all other control points, if the control point ownership does
          // not match other control points, reset round ownership to no team
          if (iRoundOwningTeam != iControlPointOwningTeam) {
            iRoundOwningTeam = TEAM_INVALID;
          }
        }
      }
      // this round is playable if all control points are not owned by one team
      // (or owned by a team that can't win by capping them)
      bool bPlayable = ((iRoundOwningTeam < FIRST_GAME_TEAM) ||
                        (pRound->GetInvalidCapWinner() == 1) ||
                        (iRoundOwningTeam == pRound->GetInvalidCapWinner()));
      if (!bPlayable)
        continue;

      // Pretend this team played and won this round.  It now owns all control
      // points from this round.  Add all the control points from this round
      // that are not already own the owned list to the owned list
      ...
      // sanity check: team being simulated should be owning at least one more
      // new control point per round, or they're not making progress
      Assert(iNewControlPointsOwned > 0);

      // now go back and pick the next playable round (if any) given the control
      // points this team now owns, repeat until all control points are owned.
      // The number of iterations it takes is the # of rounds remaining for this
      // team to win.
      break;
    }
  }
...
void CTeamControlPointMaster::ListRounds( void )
{
        if ( PlayingMiniRounds() )
        {
                ...
                {
                        CTeamControlPointRound* pRound =
m_ControlPointRounds[i];

                        if ( pRound )
                        {
                                const char *pszName = STRING(
pRound->GetEntityName() ); ConMsg( "%s\n", pszName );
                        }
                }
        }
        else
...
g_hControlPointMasters[0] : NULL; if ( pMaster )
        {
                pMaster->ListRounds();
        }
}

static ConCommand listrounds( "listrounds", cc_ListRounds, "List the rounds for
the current map", FCVAR_CHEAT );
...
void cc_PlayRound( const CCommand& args )
{
        ...
        {
                CTeamplayRoundBasedRules *pRules =
dynamic_cast<CTeamplayRoundBasedRules*>( GameRules() ); CTeamControlPointMaster
*pMaster = g_hControlPointMasters.Count() ? g_hControlPointMasters[0] : NULL;
...
pRound->GetEntityName() ); mp_restartgame.SetValue( 5 );
                                }
                                else
                                {
                                        ConMsg( "* Round \"%s\" not found in
this map *\n", args[1] );
                                }
                        }
                }
...
static ConCommand playround( "playround", cc_PlayRound, "Play the selected
