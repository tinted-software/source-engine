...
#pragma GCC diagnostic ignored                                                 \
    "-Wdangling-else" // warning: add explicit braces to avoid dangling else
                      // [-Wdangling-else]
#endif
...
ConVar
    ai_spread_defocused_cone_multiplier("ai_spread_defocused_cone_multiplier",
                                        "3.0");
...
public:
  CTriggerTraceEnum(Ray_t *pRay, const CTakeDamageInfo &info, const Vector &dir,
                    int contentsMask)
      : m_info(info), m_VecDir(dir), m_ContentsMask(contentsMask),
        m_pRay(pRay) {}
...
void CBaseEntity::HandleShotImpactingGlass(const FireBulletsInfo_t &info,
  ...
  // hitting multiple targets in one call 		 would do exactly the same anyway...
  ...
  if (pResult == NULL)
    DevMsg(
        "Warning: NULL Return from GetBestSound\n"); // condition previously set
                                                     // now no longer true. Have
                                                     // seen this when play too
                                                     // many sounds...
  return pResult;
}
...
void CAI_BaseNPC::MaintainLookTargets(float flInterval) {
  ...
  if (m_flNextEyeLookTime < gpGlobals->curtime) {
    ...
    random->RandomFloat(-0.5,0.5) + random->RandomFloat(-0.5,0.5); float
    z			= random->RandomFloat(-0.5,0.5) +
    ...
    lookSpread.y * right) + (z * lookSpread.z * up); float  lookDist =
    random->RandomInt(50,2000); m_vEyeLookTarget	= EyePosition() +
    lookDist*vecDir;
    */
...
void CAI_BaseNPC::UpdateEfficiency(bool bInPVS) {
  ...
  if (bInPVS) {
    ...
    range = (playerDist < 50 * 12)    ? DIST_NEAR
            : (playerDist < 200 * 12) ? DIST_MID
                                      : DIST_FAR;
  } else {
    range = (playerDist < 25 * 12)    ? DIST_NEAR
            : (playerDist < 100 * 12) ? DIST_MID
                                      : DIST_FAR;
  }
    ...
    if (g_pAINetworkManager && g_pAINetworkManager->IsInitialized()) {
      ...
      AI_PROFILE_SCOPE_BEGIN_(
          GetClassScheduleIdSpace()
              ->GetClassName()); // need to use a string stable from map load to
                                 // map load
  ...
  if (m_NPCState != NPC_STATE_NONE && m_NPCState != NPC_STATE_DEAD) {
    ...
    if (!IsFlaggedEfficient() &&
      ...
      if (GetCurSchedule() &&
        ...
        // respond
        //						  to COND_NEW_ENEMY.
        //Only evidence right now is save 						  games after the fact, so for now, just
        //patching it up
      ...
      for (CAI_BaseNPC *pSquadMember = pSquad->GetFirstMember(&iter, false);
        ...
        if (pSquadMember != this)
          NDebugOverlay::Line(EyePosition(), pSquadMember->EyePosition(), 0,
                              CAI_Squad::IsSilentMember(pSquadMember) ? 127
                                                                      : 255,
                              0, false, 0.1);
      }
    }
...
void CAI_BaseNPC::GiveWeapon(string_t iszWeaponName) {
  ...
  if (GetEntityName() != NULL_STRING) {
    pWeapon->SetName(
        AllocPooledString(UTIL_VarArgs("%s_weapon", STRING(GetEntityName()))));
  }
...
// the senses. right 			now, it yields different result Input  : Output :
//-----------------------------------------------------------------------------
...
Activity CAI_BaseNPC::NPC_TranslateActivity(Activity eNewActivity) {
  ...
  //						which causes the character to
  //pop up and down in their idle state of firing while crouched. -- jdw
...
void CAI_BaseNPC::SetHullSizeNormal(bool force) {
  if (m_fIsUsingSmallHull || force) {
    // Find out what the height difference will be between the versions and
    // adjust our bbox accordingly to keep us level
...
void CAI_BaseNPC::NPCInit(void) {
  ...
  if (GetSolid() != SOLID_VPHYSICS && !IsSolidFlagSet(FSOLID_NOT_SOLID)) {
    if (!IsNavHullValid()) {
      Warning("NPC Entity %s (%d) has a bounding box which extends outside its "
              "nav box!\n",
...
void CAI_BaseNPC::AddRelationship(const char *pszRelationship,
  ...
  while (entityString) {
    ...
    if (dispositionString) {
      ...
      Warning("Can't parse relationship info (%s) - Expecting 'name [D_HT, "
              "D_FR, D_LI, D_NU] [1-99]'\n",
              pszRelationship);
      Assert(0);
      return;
...
//unreachable, 			regardless of priority.  For enemies that are both reachable or
//			unreachable picks by priority.  If priority is the same,
//picks 			by distance.
// Input  :
// Output :
//-----------------------------------------------------------------------------
...
void CAI_BaseNPC::HandleAnimEvent(animevent_t *pEvent) {
  ...
  case SCRIPT_EVENT_NOT_DEAD:
    if (m_NPCState == NPC_STATE_SCRIPT) {
      m_lifeState = LIFE_ALIVE;
      // This is for life/death sequences where the player can determine whether
      // a character is dead or alive after the script
  ...
  case NPC_EVENT_SWISHSOUND: {
    // NO NPC may use this anim event unless that npc's precache precaches this
    // sound!!!
  ...
  case NPC_EVENT_RIGHTFOOT:
    // For right now, do nothing. All functionality for this lives in individual
    // npcs.
    break;
    ...
    if ((pEvent->type & AE_TYPE_NEWEVENTSYSTEM) &&
      ...
      if (pEvent->event == AE_NPC_HOLSTER) {
        ...
        if ((pWeapon) && (pEvent->options)) {
          ...
          if (act == ACT_INVALID) {
            ...
            act = Weapon_TranslateActivity(
                (Activity)CAI_BaseNPC::GetActivityID(pEvent->options), NULL);
          }
        ...
        if (m_hInteractionPartner) {
          ...
          DevWarning(
              1,
              "Bad %s format. Should be: { AE_NPC_HURT_INTERACTION_PARTNER "
              "<frame number> \"<ME/THEM> <Amount of damage done>\" }\n",
              pName);
          return;
        }

        DevWarning("%s received AE_NPC_HURT_INTERACTION_PARTNER anim event, "
                   "but it's not interacting with anything.\n",
                   GetDebugName());
        return;
      }
    }
...
int CAI_BaseNPC::DrawDebugTextOverlays(void) {
  ...
  if (m_debugOverlays & OVERLAY_TEXT_BIT) {
    ...
    // Print Health
    // --------------
    Q_snprintf(tempstr, sizeof(tempstr), "Health: %i  (DACC:%1.2f)",
               m_iHealth.Get(), GetDamageAccumulator());
    EntityText(text_offset, tempstr, 0);
    text_offset++;
    ...
    if (GetHintGroup() != NULL_STRING) {
      Q_snprintf(tempstr, sizeof(tempstr), "Hint Group: %s",
                 STRING(GetHintGroup()));
      EntityText(text_offset, tempstr, 0);
      text_offset++;
    }
...
void CAI_BaseNPC::ReportOverThinkLimit(float time) {
  DevMsg("%s thinking for %.02fms!!! (%s); r%.2f (c%.2f, pst%.2f, ms%.2f), "
         "p-r%.2f, m%.2f\n",
...
//is set, the npc 			will be sucked into the script no matter what state it is in.
//ONLY 			Scripted AI ents should allow this.
// Input  : fDisregardNPCState -
//			interruptLevel -
//			eMode - If the function returns true, eMode will be one
//of the following values: 				CAN_PLAY_NOW 				CAN_PLAY_ENQUEUED
// Output :
//-----------------------------------------------------------------------------
...
CBaseEntity *CAI_BaseNPC::FindNamedEntity(const char *name,
  ...
  if (!stricmp(name, "!player")) {
    ...
    if (++playerwarningcount < 5) {
      DevMsg("ERROR: \"player\" is no longer used, use \"!player\" in vcd "
             "instead!\n");
    }
    return (CBaseEntity *)AI_GetSinglePlayer();
  } else {
...
//=========================================================
CBaseEntity *CAI_BaseNPC::DropItem(const char *pszItemName, Vector vecPos,
                                   QAngle vecAng) {
  if (!pszItemName) {
...
DEFINE_EMBEDDED(m_ScheduleState),
    DEFINE_FIELD(
        m_IdealSchedule,
        FIELD_INTEGER), // handled specially but left in for "virtual" schedules
    DEFINE_FIELD(
        m_failSchedule,
        FIELD_INTEGER), // handled specially but left in for "virtual" schedules
    ...
    //								m_iFrameBlocked
    //(not saved) 								m_bInChoreo (not saved) 								m_bDoPostRestoreRefindPath (not saved)
    //								gm_flTimeLastSpawn
    //(static) 								gm_nSpawnedThisFrame (static) 								m_Conditions (custom save)
    //								m_CustomInterruptConditions
    //(custom save) 								m_ConditionsPreIgnore (custom save)
    //								m_InverseIgnoreConditions
    //(custom save) 								m_poseAim_Pitch (not saved; recomputed on restore)
    //								m_poseAim_Yaw
    //(not saved; recomputed on restore) 								m_poseMove_Yaw (not saved; recomputed
    //on restore)
    ...
    // 							m_failText
    // DEBUG 							m_interruptText				DEBUG 							m_failedSchedule
    // DEBUG 							m_interuptSchedule			DEBUG 							m_nDebugCurIndex
    // DEBUG

    // 							m_LastShootAccuracy
    // DEBUG 							m_RecentShotAccuracy		DEBUG 							m_TotalShots
    // DEBUG 							m_TotalHits					DEBUG
    //							m_bSelected
    //DEBUG
    // 							m_TimeLastShotMark
    // DEBUG
...
void CAI_BaseNPC::Precache(void) {
  gm_iszPlayerSquad =
      AllocPooledString(PLAYER_SQUADNAME); // cache for fast IsPlayerSquad calls
...
ConVar sv_test_scripted_sequences(
    "sv_test_scripted_sequences", "0", FCVAR_NONE,
    "Tests for scripted sequences that are embedded in the world. Run through "
    "your map with this set to check for NPCs falling through the world.");
...
bool CAI_BaseNPC::CineCleanup() {
  ...
  if (m_lifeState == LIFE_DYING) {
    ...
    if (CanBecomeRagdoll() == false) {
      StopAnimation();
      IncrementInterpolationFrame(); // Don't interpolate either, assume the
                                     // corpse is positioned in its final
                                     // resting place
    }
...
//anim event 			or as a fallback when we don't have an appropriate open activity.
//-----------------------------------------------------------------------------
...
void CAI_BaseNPC::InputForceInteractionWithNPC(inputdata_t &inputdata) {
  ...
  if (!pszParam || !pszParam[0]) {
    Warning("%s(%s) received ForceInteractionWithNPC input with bad "
            "parameters: %s\nFormat should be: ForceInteractionWithNPC <target "
            "NPC> <interaction name>\n",
  ...
  if (!pTarget) {
    Warning("%s(%s) received ForceInteractionWithNPC input, but couldn't find "
            "entity named: %s\n",
  ...
  if (!pNPC || !pNPC->GetModelPtr()) {
    Warning("%s(%s) received ForceInteractionWithNPC input, but entity named "
            "%s cannot run dynamic interactions.\n",
            GetClassname(), GetDebugName(), pszParam);
    return;
  }
  ...
  if (iInteraction == -1) {
    Warning("%s(%s) received ForceInteractionWithNPC input, but couldn't find "
            "an interaction named %s that entity named %s could run.\n",
...
//TARGET 				  NPC TO BE SOME DISTANCE DIRECTLY IN FRONT OF ME.
//-----------------------------------------------------------------------------
