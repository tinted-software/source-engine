...
CPointCamera *GetPointCameraList() { return g_PointCameraList.m_pClassList; }
...
// Purpose:
//-----------------------------------------------------------------------------
CPointCamera::~CPointCamera() { g_PointCameraList.Remove(this); }
...
//-----------------------------------------------------------------------------
// Purpose: Override ShouldTransmit since we want to be sent even though we
// don't have a model, etc.
...
void CPointCamera::InputSetOnAndTurnOthersOff(inputdata_t &inputdata) {
  ...
  while ((pEntity = gEntList.FindEntityByClassname(pEntity, "point_camera")) !=
         NULL) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CPointCamera::InputSetOn(inputdata_t &inputdata) { m_bIsOn = true; }
...
DEFINE_KEYFIELD(m_FOV, FIELD_FLOAT, "FOV"),
    ...
    DEFINE_KEYFIELD(m_bUseScreenAspectRatio, FIELD_BOOLEAN,
    ...
    DEFINE_INPUTFUNC(FIELD_VOID, "SetOnAndTurnOthersOff",
