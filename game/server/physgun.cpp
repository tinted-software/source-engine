...
public:
  ...
  bool MakeConstraint(CBaseEntity *pObject) {
    ...
    m_pConstraint =
        physenv->CreateFixedConstraint(pReference, pAttached, NULL, fixed);
...
DEFINE_PHYSPTR(m_pConstraint), DEFINE_FIELD(m_isInert, FIELD_BOOLEAN),
    // physics system will fire this input if the constraint breaks due to
    // physics
...
public:
  ...
  void SetAutoAlign(const Vector &localDir, const Vector &localPos,
                    const Vector &worldAlignDir, const Vector &worldAlignPos) {
    m_align = true;
...
CGravControllerPoint::Simulate(IPhysicsMotionController *pController,
                               IPhysicsObject *pObject, float deltaTime,
                               Vector &linear, AngularImpulse &angular) {
  Vector vel;
  ...
  if (m_align) {
    ...
    // Solve for the rotation around the target normal (at the local align pos)
    // that will move the grabbed spot to the destination.
    ...
    if (len > 0.08) {
      ...
      Vector planeSrc(DotProduct(rotSrc, tangent), DotProduct(rotSrc, binormal),
                      0);
    ...
    if (factor < 5) {
      ...
      // Limit movement to the part along m_targetAlignNormal if worldRotCenter
      // is on the backside of of the target plane (one inch epsilon)!
...
public:
  ...
  void AttachObject(CBaseEntity *pEdict, const Vector &start, const Vector &end,
                    float distance);
  ...
  void AddPellet(CGravityPellet *pPellet, CBaseEntity *pParent,
  ...
  void GetPelletWorldCoords(int pelletIndex, Vector *worldPos,
                            Vector *worldNormal) {
    if (worldPos) {
    ...
    if (worldNormal) {
      if (m_activePellets[pelletIndex].parent) {
        ...
        *worldNormal =
            tmp.LocalToWorldRotation(m_activePellets[pelletIndex].localNormal);
      } else {
...
DEFINE_FIELD(localNormal, FIELD_VECTOR), DEFINE_FIELD(pellet, FIELD_EHANDLE),
...
void CWeaponGravityGun::EffectUpdate(void) {
  ...
  if (pObject) {
    ...
    if (m_useDown) {
      ...
      if (pOwner->m_nButtons & IN_FORWARD) {
        m_distance =
            UTIL_Approach(1024, m_distance, gpGlobals->frametime * 100);
      }
    ...
    // HACKHACK: We do this so we can "ignore" the player and the object we're
    // manipulating If we had a filter for tracelines, we could simply filter
    // both ents and start from "start"
    ...
    // If I'm looking more than 20 degrees away from the glue point, then give
    // up This lets the player "gesture" for the glue to let go.
  ...
  if (m_pelletHeld >= 0 && gpGlobals->curtime > m_glueTime) {
    ...
    m_gravCallback.SetAutoAlign(
        ...
        m_activePellets[m_pelletHeld].pellet->GetLocalOrigin(), worldNormal,
        worldPos);
  } else {
...
// * scale
//			e.g. UTIL_LineFraction( 1.5, 1, 2, 1 ); will return 0.5
//since 1.5 is 			halfway between 1 and 2
...
void CWeaponGravityGun::SoundStart(void) {
  ...
  case SS_LOCKEDON: {
    // BUGBUG - If you start a sound with a pitch of 100, the pitch shift
    // doesn't work!
...
void CWeaponGravityGun::AddPellet(CGravityPellet *pPellet, CBaseEntity *pAttach,
                                  const Vector &surfaceNormal) {
  ...
  if (pAttach) {
    ...
    m_activePellets[m_pelletCount].localNormal =
        tmp.WorldToLocalRotation(surfaceNormal);
  }
...
void CWeaponGravityGun::SortPelletsForObject(CBaseEntity *pObject) {
  ...
  for (int i = 0; i < m_pelletCount; i++) {
    ...
    if (m_activePellets[i].parent == pObject &&
        !m_activePellets[i].pellet->IsInert()) {
      if (i != 0) {
...
void CWeaponGravityGun::CreatePelletAttraction(float radius,
                                               CBaseEntity *pObject) {
  int nearPellet = -1;
  int objectPellet = -1;
    ...
    for (int i = 0; i < m_objectPelletCount; i++) {
      ...
      for (int j = m_objectPelletCount; j < m_pelletCount; j++) {
        ...
        if (pTest->IsInert())
          continue;
        float distSqr =
            (pTest->GetAbsOrigin() - pPellet->GetAbsOrigin()).LengthSqr();
        if (distSqr < best) {
          Vector worldPos, worldNormal;
          GetPelletWorldCoords(j, &worldPos, &worldNormal);
          // don't attract backside pellets (unless current pellet - prevent
          // oscillation)
          float dist = DotProduct(worldPos, worldNormal);
          if (m_pelletAttract == j ||
              DotProduct(pPellet->GetAbsOrigin(), worldNormal) - dist >= 0) {
            best = distSqr;
            nearPellet = j;
            objectPellet = i;
          }
  ...
  // check for bonding
  if (best < 3 * 3) {
    // This makes the pull towards the pellet stop getting stronger since some
    // part of the object is touching
    m_glueTouching = true;
  }
}
...
void CWeaponGravityGun::AttachObject(CBaseEntity *pObject, const Vector &start,
...
void CWeaponGravityGun::SecondaryAttack(void) {
  ...
  if (pHit->entindex() == 0) {
    ...
    // if the object has no physics object, or isn't a physprop or brush entity,
    // then don't glue
  ...
  CGravityPellet *pPellet = (CGravityPellet *)CBaseEntity::Create(
      "gravity_pellet", endPoint, angles, this);
  if (pHit) {
...
void CC_CollisionTest(const CCommand &args) {
  ...
  for (int j = 0; j < 2; j++) {
    ...
    if (testType == 1) {
      ...
      for (i = 0; i < NUM_COLLISION_TESTS; i++) {
        physcollision->TraceBox(start, targets[i], -size[j], size[j], pCollide,
      ...
      for (i = 0; i < NUM_COLLISION_TESTS; i++) {
        UTIL_TraceModel(start, targets[i], -size[j], size[j], pWorld,
                        COLLISION_GROUP_NONE, &tr);
        // results[i] = tr.endpos;
      }
    }
