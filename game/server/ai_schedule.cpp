...
int CAI_SchedulesManager::GetPathID(const char *token) {
  ...
  //PATH_FLANK;		} 	else if ( !stricmp( token, "FLANK_LOS" ) )	{
  //return PATH_FLANK_LOS;	}
...
bool CAI_SchedulesManager::LoadSchedulesFromBuffer(
    const char *prefix, const char *pStartFile,
  ...
  while (!stricmp("Schedule", token)) {
    ...
    if (GetScheduleByName(token)) {
      DevMsg("ERROR: file contains a schedule (%s) that has already been "
             "defined!\n",
             token);
    ...
    if (stricmp(token, "Tasks")) {
      DevMsg("ERROR: LoadSchd (%s): (%s) Malformed AI Schedule.  Expecting "
             "'Tasks' keyword.\n",
    ...
    while ((token[0] != '\0') && (stricmp("Interrupts", token))) {
      ...
      if (!stricmp("Activity", token)) {
        ...
        Warning("ERROR: LoadSchd (%s): (%s) Bad syntax at task #%d (wasn't "
                "expecting %s)\n",
