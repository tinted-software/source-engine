...
int CHintCriteria::NumHintTypes() const { return m_HintTypes.Count(); }

int CHintCriteria::GetHintType(int idx) const { return m_HintTypes[idx]; }
...
// Input  : bitmask -
//-----------------------------------------------------------------------------
void CHintCriteria::SetFlag(int bitmask) { m_iFlags |= bitmask; }

//-----------------------------------------------------------------------------
// Purpose:
// Input  : bitmask -
//-----------------------------------------------------------------------------
void CHintCriteria::ClearFlag(int bitmask) { m_iFlags &= ~bitmask; }

//-----------------------------------------------------------------------------
// Purpose:
// Input  : group -
//-----------------------------------------------------------------------------
void CHintCriteria::SetGroup(string_t group) { m_strGroup = group; }
...
//-----------------------------------------------------------------------------
void CHintCriteria::AddZone(zoneList_t &list, const Vector &position,
                            float radius) {
...
//-----------------------------------------------------------------------------
// Purpose: Test to see if this position falls within any of the zones in the
// list Input  : *zone - list of zones to test against
//			&testPosition - position to test with
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
inline bool CHintCriteria::InZone(const zoneList_t &zone,
                                  const Vector &testPosition) const {
...
CUtlMap<int, CAIHintVector> CAI_HintManager::gm_TypedHints(0, 0,
                                                           DefLessFunc(int));
...
CAI_Hint *CAI_HintManager::AddFoundHint(CAI_Hint *hint) {
  if (hint) {
    CAI_HintManager::gm_nFoundHintIndex =
        (CAI_HintManager::gm_nFoundHintIndex + 1) &
        CAI_HintManager::HINT_HISTORY_MASK;
...
CAI_Hint *CAI_HintManager::GetFoundHint(int index) {
  return gm_pLastFoundHints[(CAI_HintManager::gm_nFoundHintIndex + index) &
                            CAI_HintManager::HINT_HISTORY_MASK];
}
...
CAI_Hint *CAI_HintManager::GetLastFoundHint() {
  for (int i = 0; i < CAI_HintManager::HINT_HISTORY; ++i) {
    // Walk backward
    int slot = ((CAI_HintManager::gm_nFoundHintIndex - i) &
                CAI_HintManager::HINT_HISTORY_MASK);
...
//-----------------------------------------------------------------------------
int CAI_HintManager::FindAllHints(CAI_BaseNPC *pNPC, const Vector &position,
...
// Output : CAI_Hint
//-----------------------------------------------------------------------------
CAI_Hint *CAI_HintManager::FindHintRandom(CAI_BaseNPC *pNPC,
  ...
  if (FindAllHints(pNPC, position, hintCriteria, &hintList) > 0) {
    // Pick one randomly
    return (CAI_HintManager::AddFoundHint(
        hintList[random->RandomInt(0, hintList.Size() - 1)]));
  }
...
static void AppendTimer(int idx, char *buf, size_t bufsize, CFastTimer &timer) {
  char s[32];
  Q_snprintf(s, sizeof(s), "%d %6.3f ms", idx,
...
// Output : CAI_Hint
//-----------------------------------------------------------------------------
CAI_Hint *CAI_HintManager::FindHint(CAI_BaseNPC *pNPC, const Vector &position,
                                    const CHintCriteria &hintCriteria) {
#if defined(HINT_PROFILING)
  CFastTimer timer;
  timer.Start();
  ...
  if (singleType) {
    int slot =
        CAI_HintManager::gm_TypedHints.Find(hintCriteria.GetFirstHintType());
    ...
    if (typeCount > 0) {
      for (int listType = 0; listType < typeCount; ++listType) {
        int slot = CAI_HintManager::gm_TypedHints.Find(
            hintCriteria.GetHintType(listType));
        if (slot != CAI_HintManager::gm_TypedHints.InvalidIndex()) {
          lists.AddToTail(&CAI_HintManager::gm_TypedHints[slot]);
        }
      }
  ...
  if (!lookingForNearest) {
    ...
    for (i = 0; i < count; ++i) {
      ...
      if (pTestHint) {
        ...
        ++visited;
        if (pTestHint->HintMatchesCriteria(pNPC, hintCriteria, position,
                                           &flBestDistance)) {
#if defined(HINT_PROFILING)
  ...
  for (int listNum = 0; listNum < listCount; ++listNum) {
    ...
    for (i = 0; i < count; ++i) {
      ...
      Assert(dynamic_cast<CAI_Hint *>(pTestHint) != NULL);
      if (pTestHint->HintMatchesCriteria(pNPC, hintCriteria, position,
                                         &flBestDistance, false,
                                         bIgnoreHintType)) {
        // If we were searching for the nearest, just note that this is now the
        // nearest node
        if (lookingForNearest) {
          pBestHint = pTestHint;
        } else {
  ...
  Msg("visited %d\n", visited);
  if (!pBestHint) {
    Msg("%i search failed for [%d] at pos %.3f %.3f %.3f [%.4f msec ~ %.4f "
        "msec per node]\n",
        gpGlobals->tickcount, pNPC ? pNPC->entindex() : -1, position.x,
        position.y, position.z, timer.GetDuration().GetMillisecondsF(),
...
//			try to use it.
//
// Input  : nFlags - Search criterea. Can currently be one or more of the
// following:
//				bits_HINT_NODE_VISIBLE - searches for visible
//hint nodes. 				bits_HINT_NODE_RANDOM - calls through the FindHintRandom and
//builds list of all matching 				nodes and picks randomly from among them.  Note:
//Depending on number of hint nodes, this 				could be slower, so use with care.
//
// Output : Returns pointer to hint node if available hint node was found that
// matches the
//			given criterea that this NPC also cares about.
//Otherwise, returns NULL
//-----------------------------------------------------------------------------
CAI_Hint *CAI_HintManager::FindHint(CAI_BaseNPC *pNPC, Hint_e nHintType,
                                    int nFlags, float flMaxDist,
  ...
  Vector vecPosition =
      (pMaxDistFrom != NULL) ? (*pMaxDistFrom) : pNPC->GetAbsOrigin();
...
// Output : CAI_Hint
//-----------------------------------------------------------------------------
CAI_Hint *CAI_HintManager::FindHint(const Vector &position,
                                    const CHintCriteria &hintCriteria) {
...
// Output : CAI_Hint
//-----------------------------------------------------------------------------
CAI_Hint *CAI_HintManager::FindHint(CAI_BaseNPC *pNPC,
                                    const CHintCriteria &hintCriteria) {
  assert(pNPC != NULL);
  if (pNPC == NULL)
    return NULL;
...
// Output  :
//------------------------------------------------------------------------------
CAI_Hint *CAI_HintManager::CreateHint(HintNodeData *pNodeData,
                                      const char *pMapData) {
...
int CAI_HintManager::GetFlags(const char *token) {
  ...
  if ((bits & bits_HINT_NODE_NEAREST) && (bits & bits_HINT_NODE_RANDOM)) {
    ...
    DevMsg("HINTFLAGS:%s, inconsistent, the nearest node is never a random "
           "hint node, treating as nearest request!\n",
           token);
  }
...
void CAI_HintManager::DumpHints() {
  ...
  while (pCurHint) {
    ...
    Msg("(%.1f, %.1f, %.1f) -- Node ID: %d; WC id %d; type %d\n", v.x, v.y, v.z,
        pCurHint->GetNodeId(), pCurHint->GetWCId(), pCurHint->HintType());
...
void CAI_HintManager::ValidateHints() {
  ...
  for (int i = gm_TypedHints.FirstInorder(); i != gm_TypedHints.InvalidIndex();
       i = gm_TypedHints.NextInorder(i)) {
...
void CAI_HintManager::DrawHintOverlays(float flDrawDuration) {
  ...
  for (int i = 0; i < c; ++i) {
    ...
    if (pHint->m_NodeData.nNodeID != NO_NODE) {
      vHintPos =
          g_pBigAINet->GetNode(pHint->m_NodeData.nNodeID)
    ...
    if (pHint->GetNodeId() != NO_NODE)
      NDebugOverlay::Text(
          vHintPos + Vector(0, 6, 8),
          CFmtStr("(%d), (%d)", pHint->HintType(), pHint->GetNodeId()), true,
          flDrawDuration);
    else
      NDebugOverlay::Text(vHintPos + Vector(0, 6, 8),
                          CFmtStr("(%d)", pHint->HintType()), true,
                          flDrawDuration);
    ...
    if (pHint->m_NodeData.iDisabled) {
      ...
      if (pOwner) {
        ...
        NDebugOverlay::Line(vHintPos, pOwner->WorldSpaceCenter(), 128, 128, 128,
                            false, 0);
      }
    ...
    NDebugOverlay::Box(vHintPos, Vector(-3, -3, -3), Vector(3, 3, 3), r, g, b,
                       0, flDrawDuration);

    // Draw line in facing direction
    Vector offsetDir = 12.0 * Vector(cos(DEG2RAD(pHint->Yaw())),
                                     sin(DEG2RAD(pHint->Yaw())), 0);
    NDebugOverlay::Line(vHintPos, vHintPos + offsetDir, r, g, b, false,
                        flDrawDuration);
  }
}
...
// Output  :
//------------------------------------------------------------------------------
Vector CAI_Hint::GetDirection() { return UTIL_YawToVector(Yaw()); }
...
// Output : Returns true if the node was available for locking, false on
// failure.
//-----------------------------------------------------------------------------
...
// Output :
//-----------------------------------------------------------------------------
bool CAI_Hint::IsLockedBy(CBaseEntity *pNPC) { return (m_hHintOwner == pNPC); };
...
//-----------------------------------------------------------------------------
// Purpose: Return true if pTestHint passes the criteria specified in
// hintCriteria
//-----------------------------------------------------------------------------
bool CAI_Hint::HintMatchesCriteria(CAI_BaseNPC *pNPC,
                                   const CHintCriteria &hintCriteria,
                                   const Vector &position,
                                   float *flNearestDistance, bool bIgnoreLock,
  ...
  if (GetMinState() > NPC_STATE_IDLE || GetMaxState() < NPC_STATE_COMBAT) {
    if (pNPC && (pNPC->GetState() < GetMinState() ||
                 pNPC->GetState() > GetMaxState())) {
  ...
  if (hintCriteria.GetGroup() != NULL_STRING) {
    ...
    if (GetGroup() == NULL_STRING || GetGroup() != hintCriteria.GetGroup()) {
      Assert(GetGroup() == NULL_STRING ||
             strcmp(STRING(GetGroup()), STRING(hintCriteria.GetGroup())) != 0);
  ...
  if ((hintCriteria.HasIncludeZones()) &&
      (hintCriteria.InIncludedZone(GetAbsOrigin()) == false)) {
    REPORTFAILURE("Not inside include zones.");
    return false;
  }

  // If we're watching for exclude zones, test it
  if ((hintCriteria.HasExcludeZones()) &&
      (hintCriteria.InExcludedZone(GetAbsOrigin()))) {
  ...
  if (hintCriteria.HasFlag(bits_HINT_NPC_IN_NODE_FOV)) {
    if (pNPC == NULL) {
      AssertMsg(0,
                "Hint node attempted to verify NPC in node FOV without NPC!\n");
    } else {
      if (!IsInNodeFOV(pNPC)) {
  ...
  if (hintCriteria.HasFlag(bits_HINT_NODE_IN_AIMCONE)) {
    if (pNPC == NULL) {
      AssertMsg(0, "Hint node attempted to find node in aimcone without "
                   "specifying NPC!\n");
    } else {
  ...
  if (hintCriteria.HasFlag(bits_HINT_NODE_IN_VIEWCONE)) {
    if (pNPC == NULL) {
      AssertMsg(0, "Hint node attempted to find node in viewcone without "
                   "specifying NPC!\n");
    } else {
  ...
  if (hintCriteria.HasFlag(bits_HINT_NOT_CLOSE_TO_ENEMY)) {
    if (pNPC == NULL) {
      AssertMsg(0, "Hint node attempted to find node not close to enemy "
                   "without specifying NPC!\n");
    } else {
      if (pNPC->GetEnemy()) {
        float flDistHintToEnemySqr =
            GetAbsOrigin().DistToSqr(pNPC->GetEnemy()->GetAbsOrigin());
...

  {
    ...
    if (hintCriteria.HasFlag(bits_HINT_NODE_VISIBLE)) {
      if (pNPC == NULL) {
        // NOTENOTE: If you're hitting this, you've asked for a visible node
        // without specifing an NPC!
        AssertMsg(0, "Hint node attempted to find visible node without "
                     "specifying NPC!\n");
      } else {
        if (m_NodeData.nNodeID == NO_NODE) {
          ...
          AI_TraceLine(pNPC->EyePosition(), vHintPos + pNPC->GetViewOffset(),
                       MASK_NPCSOLID_BRUSHONLY, pNPC, COLLISION_GROUP_NONE,
                       &tr);
          if (tr.fraction != 1.0f) {
  ...
  if (hintCriteria.HasFlag(bits_HINT_NODE_CLEAR)) {
    if (pNPC == NULL) {
      // NOTENOTE: If you're hitting this, you've asked for a clear node without
      // specifing an NPC!
      AssertMsg(
          0,
          "Hint node attempted to find clear node without specifying NPC!\n");
    } else {
      trace_t tr;
      // Can my bounding box fit there?
      AI_TraceHull(GetAbsOrigin(), GetAbsOrigin(), pNPC->WorldAlignMins(),
                   pNPC->WorldAlignMaxs(), MASK_SOLID, pNPC,
  ...
  if (hintCriteria.HasFlag(bits_HINT_NODE_NOT_VISIBLE_TO_PLAYER |
                           bits_HINT_NODE_VISIBLE_TO_PLAYER)) {
    bool bWasSeen = false;
    ...
    for (int i = 1; i <= gpGlobals->maxClients; i++) {
      ...
      if (pPlayer) {
        ...
        if (fDotPr > 0) {
          trace_t tr;
          UTIL_TraceLine(pPlayer->EyePosition(), GetAbsOrigin(),
                         MASK_SOLID_BRUSHONLY, pPlayer, COLLISION_GROUP_NONE,
                         &tr);
...
int CAI_Hint::DrawDebugTextOverlays(void) {
  ...
  if (m_debugOverlays & OVERLAY_TEXT_BIT) {
    char tempstr[512];
    Q_snprintf(tempstr, sizeof(tempstr), "%s (%i)",
               GetHintTypeDescription(HintType()), HintType());
    EntityText(text_offset, tempstr, 0);
    text_offset++;
    Q_snprintf(tempstr, sizeof(tempstr), "delay %f",
               MAX(0.0f, m_flNextUseTime - gpGlobals->curtime));
    EntityText(text_offset, tempstr, 0);
    text_offset++;
...
//-----------------------------------------------------------------------------
// Purpose: Sometimes FValidateHint, etc. will want to examine the underlying
// node to
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CAI_Hint::DisableForSeconds(float flSeconds) { Unlock(flSeconds); }
...
void CAI_Hint::FixupTargetNode() {
  if (m_NodeData.nTargetWCNodeID != -1)
    m_nTargetNodeID = g_pAINetworkManager->GetEditOps()->GetNodeIdFromWCId(
...
void CAI_Hint::OnRestore() {
  ...
  m_NodeData.nNodeID = g_pAINetworkManager->GetEditOps()->GetNodeIdFromWCId(
...
void CC_ai_drop_hint(const CCommand &args) {
  ...
  if (args.ArgC() < 2) {
    ...
    for (int i = 0; i < ARRAYSIZE(g_pszHintDescriptions); i++) {
      Msg("%d : %s\n", g_pszHintDescriptions[i].iType,
          g_pszHintDescriptions[i].pszDesc);
    }
    return;
  }
...
ConCommand ai_drop_hint("ai_drop_hint", CC_ai_drop_hint,
