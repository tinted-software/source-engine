  ...
  for (node = 0; node < m_pNetwork->m_iNumNodes; node++) {
    ...
    if (iPreviousNodeBinding != wcIDs.InvalidIndex()) {
      if (!bCheckForProblems) {
        DevWarning("******* MAP CONTAINS DUPLICATE HAMMER NODE IDS! CHECK FOR "
                   "PROBLEMS IN HAMMER TO CORRECT *******\n");
        bCheckForProblems = true;
      }
      DevWarning("   AI node %d is associated with Hammer node %d, but %d is "
                 "already bound to node %d\n",
                 node, GetEditOps()->m_pNodeIndexTable[node],
                 GetEditOps()->m_pNodeIndexTable[node],
...
void CAI_NetworkManager::LoadNetworkGraph(void) {
  ...
  if (mapversion != gpGlobals->mapversion && !g_ai_norebuildgraph.GetBool()) {
    ...
    // they were rebuilt a week after they were actually shipped so allow the
    // slightly older node graphs to load for these maps
  ...
  if (printedHeader)
    DevMsg("\n** Should run \"Check For Problems\" on the VMF then verify "
           "dynamic links\n");
#endif
...
bool CAI_NetworkManager::IsAIFileCurrent(const char *szMapName) {
  ...
  if (IsX360() && (filesystem->GetDVDMode() == DVDMODE_STRICT)) {
    // dvd build process validates and guarantees correctness, timestamps are
    // allowed to be wrong
    return true;
  }
  ...
  if (!g_ai_norebuildgraph.GetInt()) {
    ...
    // --------------------------------------------
      ...
      DevMsg("Node Graph out of Date. Rebuilding... (%d, %d, %d)\n",
             (int)m_bDontSaveGraph, (int)!CAI_NetworkManager::NetworksLoaded(),
             (int)engine->IsInEditMode());
