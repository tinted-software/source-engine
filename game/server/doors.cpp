...
#define BUTTON_SOUNDWAIT 0.5
    ...
    // Input  : pEdict -
    //			pls -
    //			flocked - if true, play 'door is locked' sound,
    //otherwise play 'door 				is unlocked' sound. 			fbutton -
    //-----------------------------------------------------------------------------
...
void CBaseDoor::Spawn() {
  ...
  if (!IsRotatingDoor()) {
    if ((m_eSpawnPosition == FUNC_DOOR_SPAWN_OPEN) ||
        HasSpawnFlags(SF_DOOR_START_OPEN_OBSOLETE)) { // swap pos1 and pos2, put
                                                      // door at pos2
  ...
  if (!FClassnameIs(this, "func_water")) {
    ...
    if (m_bIgnoreDebris) {
      ...
      if (HasSpawnFlags(SF_DOOR_NONSOLID_TO_PLAYER)) {
        Warning("Door %s with conflicting collision settings, removing "
                "ignoredebris\n",
                GetDebugName());
      } else {
  ...
  if ((m_eSpawnPosition == FUNC_DOOR_SPAWN_OPEN) &&
    ...
    Warning("Door %s using obsolete 'Start Open' spawnflag with 'Spawn "
            "Position' set to 'Open'. Reverting to old behavior.\n",
            GetDebugName());
  }
...
bool CBaseDoor::CreateVPhysics() {
  if (!FClassnameIs(this, "func_water")) {
    // normal door
    //  NOTE: Create this even when the door is not solid to support
    //  constraints.
...
void CBaseDoor::Blocked(CBaseEntity *pOther) {
  ...
  // If set, ignore non-player ents that block us.  Mainly of use in multiplayer
  // to prevent exploits.
...
DEFINE_KEYFIELD(m_bSolidBsp, FIELD_BOOLEAN, "solidbsp"), END_DATADESC()
