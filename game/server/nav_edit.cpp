...
ConVar nav_show_area_info(
    "nav_show_area_info", "0.5", FCVAR_CHEAT,
    "Duration in seconds to show nav area ID and attributes while editing");
ConVar nav_snap_to_grid(
    "nav_snap_to_grid", "0", FCVAR_CHEAT,
    "Snap to the nav generation grid when creating new nav areas");
ConVar nav_create_place_on_ground("nav_create_place_on_ground", "0",
                                  FCVAR_CHEAT,
                                  "If true, nav areas will be placed flush "
                                  "with the ground when created by hand.");
#ifdef DEBUG
ConVar nav_draw_limit("nav_draw_limit", "50", FCVAR_CHEAT,
                      "The maximum number of areas to draw in edit mode");
#else
ConVar nav_draw_limit("nav_draw_limit", "500", FCVAR_CHEAT,
                      "The maximum number of areas to draw in edit mode");
#endif
ConVar nav_solid_props("nav_solid_props", "0", FCVAR_CHEAT,
                       "Make props solid to nav generation/editing");
ConVar
    nav_create_area_at_feet("nav_create_area_at_feet", "0", FCVAR_CHEAT,
                            "Anchor nav_begin_area Z to editing player's feet");

ConVar nav_drag_selection_volume_zmax_offset(
    "nav_drag_selection_volume_zmax_offset", "32", FCVAR_REPLICATED,
    "The offset of the nav drag volume top from center");
ConVar nav_drag_selection_volume_zmin_offset(
    "nav_drag_selection_volume_zmin_offset", "32", FCVAR_REPLICATED,
...
//--------------------------------------------------------------------------------------------------------------
Vector CNavMesh::SnapToGrid(const Vector &in, bool snapX, bool snapY,
                            bool forceGrid) const {
...
//--------------------------------------------------------------------------------------------------------------
bool CNavMesh::FindNavAreaOrLadderAlongRay(const Vector &start,
                                           const Vector &end,
                                           CNavArea **bestArea,
...
//--------------------------------------------------------------------------------------------------------------
/**
 *  Convenience function to find the nav area a player is looking at, for
 * editing commands
 */
bool CNavMesh::FindActiveNavArea(void) {
  ...
  UTIL_TraceLine(from, to,
                 (nav_solid_props.GetBool()) ? MASK_NPCSOLID
  ...
  if (result.fraction != 1.0f) {
    if (!IsEditMode(CREATING_AREA)) {
      m_climbableSurface =
          physprops->GetSurfaceData(result.surface.surfaceProps)
              ->game.climbable != 0;
      ...
      if (m_climbableSurface) {
        // check if we're on the same plane as the original point when we're
        // building a ladder
        ...
        if (m_surfaceNormal.z > 0.9f) {
          m_climbableSurface =
              false; // don't try to build ladders on flat ground
        }
      }
    }

    if ((m_climbableSurface && !IsEditMode(CREATING_LADDER)) ||
        !IsEditMode(CREATING_AREA)) {
    ...
    if (!m_climbableSurface && !m_selectedLadder) {
      ...
      FindNavAreaOrLadderAlongRay(
          result.startpos, result.endpos + 100.0f * dir, &m_selectedArea,
      ...
      if (!m_selectedArea && !m_selectedLadder) {
        m_selectedArea =
            TheNavMesh->GetNearestNavArea(result.endpos, false, 500.0f);
      }
    }
  ...
  return (m_selectedArea != NULL || m_selectedLadder != NULL ||
          isClippingRayAtFeet);
}

//--------------------------------------------------------------------------------------------------------------
bool CNavMesh::FindLadderCorners(Vector *corner1, Vector *corner2,
                                 Vector *corner3) {
  ...
  *corner3 = m_ladderAnchor - ladderUp * maxDist - ladderRight * maxDist;
  float dist =
      IntersectRayWithTriangle(ray, *corner1, *corner2, *corner3, false);
  if (dist < 0) {
    *corner2 = m_ladderAnchor - ladderUp * maxDist + ladderRight * maxDist;
...
bool CheckForClimbableSurface(const Vector &start, const Vector &end) {
  ...
  if (result.fraction != 1.0f) {
    climbableSurface = physprops->GetSurfaceData(result.surface.surfaceProps)
                           ->game.climbable != 0;
    if (!climbableSurface) {
...
//--------------------------------------------------------------------------------------------------------------
void StepAlongClimbableSurface(Vector &pos, const Vector &increment,
                               const Vector &probe) {
  while (CheckForClimbableSurface(pos + increment - probe,
                                  pos + increment + probe)) {
    pos += increment;
  }
}
...
void CNavMesh::CommandNavBuildLadder(void) {
  ...
  CreateLadder(topEdge, bottomEdge, leftEdge.DistTo(rightEdge),
               m_ladderNormal.AsVector2D(), 0.0f);
}
...
public:
  PlaceFloodFillFunctor(CNavArea *area) { m_initialPlace = area->GetPlace(); }
...
public:
  ...
  bool operator()(CNavArea *area) {
    ...
    if ((TheNavMesh->IsInSelectedSet(area) == bShouldBeInSelectedSet) &&
        area->IsOverlapping(m_dragArea) && area->GetCenter().z >= m_zMin &&
        area->GetCenter().z <= m_zMax) {
...
void CNavMesh::UpdateDragSelectionSet(void) {
  ...
  AddToDragSet add(dragArea, m_anchor.z - m_nDragSelectionVolumeZMin,
                   m_anchor.z + m_nDragSelectionVolumeZMax,
 ...
 * @todo Clean the whole edit system up - its structure is legacy from
 * peculiarities in GoldSrc.
 */
...
void CNavMesh::DrawEditMode(void) {
  ...
  // TODO: remove this when host_thread_mode 1 stops breaking
  // NDEBUG_PERSIST_TILL_NEXT_SERVER overlays
  ...
  if (nav_show_nodes.GetBool()) {
    for (CNavNode *node = CNavNode::GetFirst(); node != NULL;
         node = node->GetNext()) {
  ...
  if ( !IsEditMode( PLACE_PAINTING ) && nav_snap_to_grid.GetBool() )
  {
          ...
          for ( x=1; x<GridArraySize; ++x )
          {
                  for ( y=1; y<GridArraySize; ++y )
                  {
                          ...
                          {
                                  NavDrawLine( pos[x][y-1], pos[x][y],
  NavGridColor );
                          }

                          if ( y == GridArraySize-1 )
                          {
                                  NavDrawLine( pos[x-1][y], pos[x][y],
  NavGridColor );
                          }
                  }
          }
  ...
  */
  ...
  if (FindActiveNavArea() || m_markedArea || m_markedLadder ||
      !IsSelectedSetEmpty() || IsEditMode(CREATING_AREA) ||
      IsEditMode(CREATING_LADDER)) {
    // draw cursor
    float cursorSize = 10.0f;

    if (m_climbableSurface) {
      NDebugOverlay::Cross3D(m_editCursorPos, cursorSize, 0, 255, 0, true,
                             NDEBUG_PERSIST_TILL_NEXT_SERVER);
    } else {
      NavDrawLine(m_editCursorPos + Vector(0, 0, cursorSize), m_editCursorPos,
                  NavCursorColor);
      NavDrawLine(m_editCursorPos + Vector(cursorSize, 0, 0),
                  m_editCursorPos + Vector(-cursorSize, 0, 0), NavCursorColor);
      NavDrawLine(m_editCursorPos + Vector(0, cursorSize, 0),
                  m_editCursorPos + Vector(0, -cursorSize, 0), NavCursorColor);
    ...
    if (IsEditMode(CREATING_AREA)) {
      ...
      NavDrawLine(Vector(m_editCursorPos.x, m_editCursorPos.y, z),
                  Vector(m_anchor.x, m_editCursorPos.y, z), NavCreationColor);
      NavDrawLine(Vector(m_anchor.x, m_anchor.y, z),
                  Vector(m_anchor.x, m_editCursorPos.y, z), NavCreationColor);
      NavDrawLine(Vector(m_anchor.x, m_anchor.y, z),
                  Vector(m_editCursorPos.x, m_anchor.y, z), NavCreationColor);
      NavDrawLine(Vector(m_editCursorPos.x, m_editCursorPos.y, z),
                  Vector(m_editCursorPos.x, m_anchor.y, z), NavCreationColor);
      ...
      Color dragSelectionColor = m_bIsDragDeselecting
    ...
    if (m_selectedLadder) {
      ...
      if (m_selectedLadder != m_lastSelectedLadder ||
        ...
        NDebugOverlay::ScreenText(0.5, 0.53, buffer, 255, 255, 0, 128,
                                  NDEBUG_PERSIST_TILL_NEXT_SERVER);
      }
    ...
    if (m_selectedArea) {
      ...
      if (m_showAreaInfoTimer.HasStarted() &&
          !m_showAreaInfoTimer.IsElapsed()) {
        char buffer[80];
        char attrib[80];
        char locName[80];
        ...
        if (m_selectedArea->GetPlace()) {
          const char *name =
              TheNavMesh->PlaceToName(m_selectedArea->GetPlace());
          if (name)
        ...
        if (IsEditMode(PLACE_PAINTING)) {
          ...
          Q_strncat(attrib, UTIL_VarArgs("%d Connections ", connected),
                    sizeof(attrib), -1);
        }
    ...
    if (!IsSelectedSetEmpty()) {
      ...
      if (m_selectedSet.Count() < nav_draw_limit.GetInt()) {
        ...
        FOR_EACH_VEC(m_selectedSet, it) {
          ...
          float range =
              (player->GetAbsOrigin() - area->GetCenter()).LengthSqr();
          if (range < nearRange) {
            nearRange = range;
            nearest = area;
          }
        ...
        SearchSurroundingAreas(nearest, nearest->GetCenter(), draw, -1,
                               INCLUDE_INCOMING_CONNECTIONS |
                                   INCLUDE_BLOCKED_AREAS);
      }
    }
  }
}
...
public:
  SelectCollector(void) { m_count = 0; }
...
void CNavMesh::CommandNavFloodSelect(const CCommand &args) {
  ...
  if (start) {
    ...
    if (args.ArgC() == 2 && FStrEq("in", args[1])) {
      connections = INCLUDE_BLOCKED_AREAS | INCLUDE_INCOMING_CONNECTIONS |
                    EXCLUDE_OUTGOING_CONNECTIONS;
    }
...
void CNavMesh::CommandNavAddToSelectedSetByID(const CCommand &args) {
  ...
  if (area) {
    ...
    Msg("Added area %d.  ( to go there: setpos %f %f %f )\n", id,
        area->GetCenter().x, area->GetCenter().y, area->GetCenter().z + 5);
  } else {
...
class ShiftSet {
public:
  ShiftSet(const Vector &shift) { m_shift = shift; }

  bool operator()(CNavArea *area) {
    area->Shift(m_shift);

    const NavLadderConnectVector *ladders =
...
//--------------------------------------------------------------------------------------------------------
CON_COMMAND_F(nav_shift, "Shifts the selected areas by the specified amount",
              FCVAR_CHEAT) {
...
ConCommand nav_world_center("nav_world_center", CommandNavCenterInWorld,
                            "Centers the nav mesh in the world", FCVAR_CHEAT);
...
void CNavMesh::CommandNavSelectInvalidAreas(void) {
  ...
  FOR_EACH_VEC(TheNavAreas, it) {
    ...
    if (area) {
      ...
      for (float x = areaExtent.lo.x; x + GenerationStepSize <= areaExtent.hi.x;
           x += GenerationStepSize) {
        for (float y = areaExtent.lo.y;
             y + GenerationStepSize <= areaExtent.hi.y;
...
void CNavMesh::CommandNavMerge(void) {
  ...
  if (m_selectedArea) {
    ...
    if (other && other != m_selectedArea) {
      ...
      Msg("To merge, mark an area, highlight a second area, then invoke the "
          "merge command");
      player->EmitSound("EDIT_MERGE.Disable");
    }
  }
...
void CNavMesh::CommandNavBeginArea(void) {
  ...
  if (!(IsEditMode(CREATING_AREA) || IsEditMode(CREATING_LADDER) ||
        IsEditMode(NORMAL))) {
    player->EmitSound("EDIT_END_AREA.NotCreating");
    return;
  }
  ...
  if (IsEditMode(CREATING_AREA)) {
    ...
    if (!nearby) {
      nearby = TheNavMesh->GetNearestNavArea(m_editCursorPos +
                                                 Vector(0, 0, HalfHumanHeight),
                                             false, 10000.0f, true);
    }
    if (!nearby) {
      nearby = TheNavMesh->GetNearestNavArea(
          endPos + Vector(0, 0, HalfHumanHeight), false, 10000.0f, true);
    }
    if (!nearby) {
      nearby = TheNavMesh->GetNearestNavArea(m_editCursorPos);
    }
    ...
    if (m_climbableSurface && FindLadderCorners(&corner1, &corner2, &corner3)) {
      // m_ladderAnchor and corner2 are at the same Z, and corner1 & corner3
      // share Z.
  ...
  if (m_selectedSet.Count() > 1) {
    ...
    if (bValid) {
      for (int i = 1; i < m_selectedSet.Count(); ++i) {
        ...
        NavDirType dir =
            second->ComputeLargestPortal(first, &center, &halfWidth);
    ...
    if (m_markedLadder) {
      ...
      NavDirType dir = GetMarkedArea()->ComputeLargestPortal(
          m_selectedArea, &center, &halfWidth);
      if (dir == NUM_DIRECTIONS) {
        player->EmitSound("EDIT_CONNECT.AllDirections");
      } else {
      ...
      if (m_selectedSet.Count() == 1) {
        ...
        NavDirType dir =
            area->ComputeLargestPortal(m_selectedArea, &center, &halfWidth);
        if (dir == NUM_DIRECTIONS) {
          player->EmitSound("EDIT_CONNECT.AllDirections");
        } else {
        ...
        Msg("To connect areas, mark an area, highlight a second area, then "
            "invoke the connect command. Make sure the cursor is directly "
            "north, south, east, or west of the marked area.");
        player->EmitSound("EDIT_CONNECT.AllDirections");
      }
    }
  } else if (m_selectedLadder) {
    if (m_markedArea) {
      ...
      Msg("To connect areas, mark an area, highlight a second area, then "
          "invoke the connect command. Make sure the cursor is directly north, "
          "south, east, or west of the marked area.");
      player->EmitSound("EDIT_CONNECT.AllDirections");
    }
  }
...
void CNavMesh::CommandNavDisconnect(void) {
  ...
  if (m_selectedSet.Count() > 1) {
    ...
    for (int i = 1; i < m_selectedSet.Count(); ++i) {
      ...
      CNavArea *first = m_selectedSet[0];
      CNavArea *second = m_selectedSet[i];
      if (!first->IsConnected(second, NUM_DIRECTIONS) &&
          !second->IsConnected(first, NUM_DIRECTIONS)) {
        player->EmitSound("EDIT_CONNECT.AllDirections");
        bValid = false;
        break;
      }
    ...
    if (m_markedArea) {
      ...
      if (m_markedLadder) {
        ...
        Msg("To disconnect areas, mark an area, highlight a second area, then "
            "invoke the disconnect command. This will remove all connections "
...
void CNavMesh::CommandNavSplice(void) {
  ...
  if (m_selectedArea) {
    if (GetMarkedArea()) {
      ...
      Msg("To splice, mark an area, highlight a second area, then invoke the "
          "splice command to create an area between them");
      player->EmitSound("EDIT_SPLICE.NoMarkedArea");
    }
  }
  ...
  if (m_selectedArea) {
    if (GetMarkedArea()) {
      ...
      if (!GetMarkedArea()) {
        ...
        Msg("Marked Area is connected to %d other Areas - there are %d total "
            "unnamed areas\n",
...
//--------------------------------------------------------------------------------------------------------------
CON_COMMAND_F(nav_select_radius,
              "Adds all areas in a radius to the selection set", FCVAR_CHEAT) {
 ...
 */
void CNavMesh::ClearDragSelectionSet(void) { m_dragSelectionSet.RemoveAll(); }

//--------------------------------------------------------------------------------------------------------------
/**
 * Clear the currently selected set to empty
 */
void CNavMesh::ClearSelectedSet(void) { m_selectedSet.RemoveAll(); }
 ...
 */
int CNavMesh::GetSelecteSetSize(void) const { return m_selectedSet.Count(); }
