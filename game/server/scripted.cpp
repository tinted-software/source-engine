...
//and grab them 			to play the script. If the NPC is already playing a script, the
//			new script may enqueue itself unless there is already a
//non critical 			script in the queue.
//
//=============================================================================//
...
CAI_BaseNPC *CAI_ScriptedSequence::FindScriptEntity() {
  ...
  while (pEntity != NULL) {
    ...
    if (m_hForcedTarget) {
      Warning("Code forced %s(%s), to be the target of scripted sequence %s, "
...
void CAI_ScriptedSequence::StartScript(void) {
  ...
  if (pTarget) {
    ...
    if (pTarget->IsEFlagSet(EFL_NO_THINK_FUNCTION) &&
      ...
      DevWarning("scripted_sequence %d:%s - restarting dormant entity %d:%s : "
                 "%.1f:%.1f\n",
                 entindex(), GetDebugName(), pTarget->entindex(),
                 pTarget->GetDebugName(), gpGlobals->curtime,
...
//
//			When triggered, scripts call this first with a state of
//1 to indicate that 			they are not ready to play (while NPCs move to their cue
...
void CAI_ScriptedSchedule::InputStartSchedule(inputdata_t &inputdata) {
  ...
  if (!m_bDidFireOnce || (m_spawnflags & SF_SCRIPT_REPEATABLE)) {
    ...
    DevMsg(2, "aiscripted_schedule - not playing schedule again: not flagged "
              "to repeat\n");
  }
}
...
void CAI_ScriptedSchedule::InputStopSchedule(inputdata_t &inputdata) {
  if (!m_bDidFireOnce) {
    DevMsg(2, "aiscripted_schedule - StopSchedule called, but schedule's never "
              "started.\n");
    return;
  }
...
// state.
const char *
CAI_ScriptedSequence::GetSpawnPreIdleSequenceForScript(CBaseEntity *pEntity) {
