...
SendPropInt(SENDINFO(m_iActiveIssueIndex)),
    ...
    SendPropArray3(SENDINFO_ARRAY3(m_nVoteOptionCount),
                   SendPropInt(SENDINFO_ARRAY(m_nVoteOptionCount), 8,
                               SPROP_UNSIGNED)),
    SendPropInt(SENDINFO(m_nPotentialVotes)),
    SendPropBool(SENDINFO(m_bIsYesNoVote)) END_SEND_TABLE()
...
ConVar sv_vote_timer_duration("sv_vote_timer_duration", "15",
                              FCVAR_DEVELOPMENTONLY,
                              "How long to allow voting on an issue");
ConVar sv_vote_command_delay(
    "sv_vote_command_delay", "2", FCVAR_DEVELOPMENTONLY,
    "How long after a vote passes until the action happens", false, 0, true,
    4.5);
ConVar sv_allow_votes("sv_allow_votes", "1", 0, "Allow voting?");
ConVar sv_vote_failure_timer(
    "sv_vote_failure_timer", "300", 0,
    "A vote that fails cannot be re-submitted for this long");
#ifdef TF_DLL
ConVar sv_vote_failure_timer_mvm(
    "sv_vote_failure_timer_mvm", "120", 0,
    "A vote that fails in MvM cannot be re-submitted for this long");
#endif // TF_DLL
ConVar
    sv_vote_creation_timer("sv_vote_creation_timer", "120",
                           FCVAR_DEVELOPMENTONLY,
                           "How often someone can individually call a vote.");
ConVar sv_vote_quorum_ratio(
    "sv_vote_quorum_ratio", "0.6", 1,
    "The minimum ratio of players needed to vote on an issue to resolve it.",
    true, 0.1, true, 1.0);
ConVar sv_vote_allow_spectators("sv_vote_allow_spectators", "0", 0,
                                "Allow spectators to vote?");
ConVar sv_vote_ui_hide_disabled_issues(
    "sv_vote_ui_hide_disabled_issues", "1", 0,
...
// Purpose:
//-----------------------------------------------------------------------------
ConCommand ListIssues("listissues", CommandListIssues,
                      "List all the issues that can be voted on.", 0);
...
CON_COMMAND(callvote, "Start a vote on an issue.") {
  ...
  if (!sv_vote_allow_spectators.GetBool()) {
    if (pVoteCaller->GetTeamNumber() == TEAM_SPECTATOR) {
      g_voteController->SendVoteFailedMessage(VOTE_FAILED_SPECTATOR,
                                              pVoteCaller);
      return;
    }
  }
  ...
  if (!g_voteController->CanEntityCallVote(pVoteCaller, nCooldown)) {
    g_voteController->SendVoteFailedMessage(VOTE_FAILED_RATE_EXCEEDED,
                                            pVoteCaller, nCooldown);
    return;
  }
#endif
...
CVoteController::~CVoteController() {
  ...
  for (int issueIndex = 0; issueIndex < m_potentialIssues.Count();
       ++issueIndex) {
...
bool CVoteController::SetupVote(int iEntIndex) {
  ...
  // Passing an nIssueCount of 0 triggers a "Voting disabled on server" message
  // in the setup UI
    ...
    for (int iIndex = 0; iIndex < m_potentialIssues.Count(); ++iIndex) {
      ...
      if (pCurrentIssue) {
        if (!pCurrentIssue->IsEnabled() &&
            sv_vote_ui_hide_disabled_issues.GetBool())
          continue;
  ...
  for (int iIndex = 0; iIndex < m_potentialIssues.Count(); ++iIndex) {
    ...
    if (pCurrentIssue) {
      if (pCurrentIssue->IsEnabled()) {
        ...
        V_strcat(szDisabledIssueStr, " (Disabled on Server)",
...
//-----------------------------------------------------------------------------
bool CVoteController::CreateVote(int iEntIndex, const char *pszTypeString,
                                 const char *pszDetailString) {
  ...
  for (int issueIndex = 0; issueIndex < m_potentialIssues.Count();
    ...
    if (FStrEq(pszTypeString, pCurrentIssue->GetTypeString())) {
      ...
      int nTime = 0;
      if (pCurrentIssue->CanCallVote(iEntIndex, pszDetailString, nErrorCode,
                                     nTime)) {
...
//-----------------------------------------------------------------------------
void CVoteController::SendVoteFailedMessage(vote_create_failed_t nReason,
...
//-----------------------------------------------------------------------------
CVoteController::TryCastVoteResult
CVoteController::TryCastVote(int iEntIndex, const char *pszVoteString) {
  if (!sv_allow_votes.GetBool())
  ...
  if (m_potentialIssues[m_iActiveIssueIndex] &&
      m_potentialIssues[m_iActiveIssueIndex]->IsAllyRestrictedVote()) {
    CBaseEntity *pVoteHolder = UTIL_EntityByIndex(m_iEntityHoldingVote);
    CBaseEntity *pVoter = UTIL_EntityByIndex(iEntIndex);

    if ((pVoteHolder == NULL) || (pVoter == NULL) ||
        (GetVoterTeam(pVoteHolder) != GetVoterTeam(pVoter))) {
...
void CVoteController::VoteControllerThink(void) {
  ...
  if (m_acceptingVotesTimer.HasStarted() && m_acceptingVotesTimer.IsElapsed()) {
    ...
    if (m_potentialIssues[m_iActiveIssueIndex]->IsYesNoVote()) {
      m_potentialIssues[m_iActiveIssueIndex]->SetYesNoVoteCount(
          m_nVoteOptionCount[VOTE_OPTION1], m_nVoteOptionCount[VOTE_OPTION2],
          m_nPotentialVotes);
    }
    ...
    if (nVoteTally >= m_nPotentialVotes * sv_vote_quorum_ratio.GetFloat()) {
      ...
      Assert(nWinningVoteOption >= 0 &&
             nWinningVoteOption < m_VoteOptions.Count());
      ...
      if (nWinningVoteOption >= 0 && nWinningVoteOption < MAX_VOTE_OPTIONS) {
        ...
        // we finally have a parameter to pass along and execute
          ...
          m_potentialIssues[m_iActiveIssueIndex]->SetIssueDetails(
    ...
    if (bVotePassed) {
      ...
      WRITE_BYTE(m_iOnlyTeamToVote);
      WRITE_STRING(
          m_potentialIssues[m_iActiveIssueIndex]->GetVotePassedString());
  ...
  // Vote passed check moved down to FrameUpdatePostEntityThink at bottom of
  // this file...
  ...
  if (m_VoteCallers.Count() >= MAX_VOTER_HISTORY) {
    ...
    for (int iIdx = m_VoteCallers.FirstInorder();
         iIdx != m_VoteCallers.InvalidIndex();
...
#ifndef DEBUG // Don't want to do this check for debug builds (so we can test
              // with bots)
  if (pWhom->IsBot())
    return false;
...
int CVoteController::GetWinningVoteOption(void) {
  if (m_potentialIssues[m_iActiveIssueIndex]->IsYesNoVote()) {
    return (m_nVoteOptionCount[VOTE_OPTION1] > m_nVoteOptionCount[VOTE_OPTION2])
               ? VOTE_OPTION1
               : VOTE_OPTION2;
  } else {
    ...
    for (int iIndex = 0; iIndex < m_nVoteOptionCount.Count(); iIndex++) {
      nHighest =
          ((nHighest < m_nVoteOptionCount[iIndex]) ? m_nVoteOptionCount[iIndex]
                                                   : nHighest);
...
void CVoteController::TrackVoteCaller(CBasePlayer *pPlayer) {
  ...
  m_VoteCallers.Insert(steamID.ConvertToUint64(),
                       gpGlobals->curtime + sv_vote_creation_timer.GetInt());
};

//-----------------------------------------------------------------------------
// Purpose: Check the history of steamIDs that called votes and test against a
// timer
//-----------------------------------------------------------------------------
...
void CBaseIssue::OnVoteFailed(int iEntityHoldingVote) {
  ...
  if (BRecordVoteFailureEventForEntity(iEntityHoldingVote)) {
    ...
    for (int index = 0; index < m_FailedVotes.Count(); index++) {
      FailedVote *pFailedVote = m_FailedVotes[index];
      if (Q_strcmp(pFailedVote->szFailedVoteParameter, GetDetailsString()) ==
          0) {
    ...
    m_FailedVotes[iIndex]->flLockoutTime =
        gpGlobals->curtime + sv_vote_failure_timer.GetFloat();
  }
}
...
// Purpose:
//-----------------------------------------------------------------------------
bool CBaseIssue::CanCallVote(int iEntIndex, const char *pszDetails,
  ...
  for (int iIndex = 0; iIndex < m_FailedVotes.Count(); iIndex++) {
    ...
    // If this issue requires a parameter, see if we're voting for the same one
    // again (i.e. changelevel ctf_2fort)
    if (Q_strlen(pCurrentFailure->szFailedVoteParameter) > 0) {
      if (nTimeRemaining > 1 &&
          FStrEq(pCurrentFailure->szFailedVoteParameter, pszDetails)) {
        bFailed = true;
      }
    }
    // Otherwise we have a parameter-less vote, so just check the lockout timer
    // (i.e. restartgame)
    else {
      if (nTimeRemaining > 1) {
        bFailed = true;
...
int CBaseIssue::GetNumberVoteOptions(void) {
  return 2; // The default issue is Yes/No (so 2), but it can be anywhere
            // between 1 and MAX_VOTE_COUNT
}
...
// Purpose:
//-----------------------------------------------------------------------------
void CBaseIssue::SetYesNoVoteCount(int iNumYesVotes, int iNumNoVotes,
...
// Purpose:
//-----------------------------------------------------------------------------
void CBaseIssue::ListStandardNoArgCommand(CBasePlayer *forWhom,
                                          const char *issueString) {
...
public:
  ...
  virtual void FrameUpdatePostEntityThink(void) {
    // Executing the vote controller command needs to happen in the
    // PostEntityThink as it can restart levels and
    //	blast entities, etc. If you're doing this during a regular think, this
    //can cause entities thinking after 	you in Physics_RunThinkFunctions() to
    //get grumpy and crash.
    if (g_voteController) {
      ...
      if (g_voteController->m_executeCommandTimer.HasStarted() &&
        ...
        g_voteController
            ->m_potentialIssues[g_voteController->m_iActiveIssueIndex]
            ->ExecuteCommand();
      }
    }
  }
