...
static ConVar sv_benchmark_numticks(
    "sv_benchmark_numticks", "3300", 0,
    "If > 0, then it only runs the benchmark for this # of ticks.");
static ConVar sv_benchmark_autovprofrecord(
    "sv_benchmark_autovprofrecord", "0", 0,
...
static int s_nBenchmarkPhysicsObjects =
    100; // Create this many physics objects.
...
public:
  CServerBenchmark() {
    ...
    // The benchmark should always have the same seed and do exactly the same
    // thing on the same ticks.
  ...
  bool InternalStartBenchmark(int nBenchmarkMode, float flCountdown) {
    ...
    if (!CServerBenchmarkHook::s_pBenchmarkHook)
      Error("This game doesn't support server benchmarks (no "
            "CServerBenchmarkHook found).");
    ...
    engine->SetDedicatedServerBenchmarkMode(
        true); // Run 1 tick per frame and ignore all timing stuff.
  ...
  virtual void UpdateBenchmark() {
    ...
    if (m_BenchmarkState == BENCHMARKSTATE_START_WAIT) {
      if ((Plat_FloatTime() - m_flBenchmarkStartTime) <
          m_flBenchmarkStartWaitTime) {
        UpdateStartWaitCounter();
        return;
      } else {
        // Ok, now we're officially starting it.
        Msg("Starting benchmark!\n");
        m_flLastBenchmarkCounterUpdate = m_flBenchmarkStartTime =
            Plat_FloatTime();
  ...
  virtual void EndBenchmark(void) {
    ...
    if (m_nBenchmarkMode == 2) {
      FileHandle_t fh = filesystem->Open("sv_benchmark_results.txt", "wt",
                                         "DEFAULT_WRITE_PATH");

      // If this file doesn't get written out, then the build script will
      // generate an email that there's a problem somewhere.
      if (fh) {
  ...
  void UpdateVPhysicsObjects() {
    int nPhysicsObjectInterval =
        sv_benchmark_numticks.GetInt() / s_nBenchmarkPhysicsObjects;
    ...
    if (GetTickOffset() >= nNextSpawnTick) {
      ...
      if (m_PhysicsObjects.Count() < s_nBenchmarkPhysicsObjects) {
        ...
        if (curPlayers.Count() > 0 && m_PhysicsModelNames.Count() > 0) {
          ...
          Vector vSpawnPos =
              curPlayers[iPlayer]->EyePosition() + Vector(0, 0, 50);

          // We'll try 15 locations around the player to spawn this thing.
          for (int i = 0; i < 15; i++) {
            Vector vOffset(this->RandomFloat(-2000, 2000),
                           this->RandomFloat(-2000, 2000), 0);
            CPhysicsProp *pProp = CreatePhysicsProp(
                pModelName, vSpawnPos, vSpawnPos + vOffset, curPlayers[iPlayer],
                false, "prop_physics_multiplayer");
            if (pProp) {
              m_PhysicsObjects.AddToTail(pProp);
              pProp->SetAbsVelocity(Vector(this->RandomFloat(-500, 500),
        ...
        if (pEnt) {
          ...
          if (pPhysicsObject) {
            ...
            AngularImpulse vAngularImpulse(
                ...
                this->RandomFloat(-flAngImpulse, flAngImpulse),
                this->RandomFloat(flAngImpulse, flAngImpulse));
            pPhysicsObject->ApplyForceCenter(
                Vector(this->RandomFloat(-flForce, flForce),
  ...
  void UpdateStartWaitCounter() {
    int nSecondsLeft = (int)ceil(m_flBenchmarkStartWaitTime -
                                 (Plat_FloatTime() - m_flBenchmarkStartTime));
  ...
  void UpdateBenchmarkCounter() {
    ...
    if ((flCurTime - m_flLastBenchmarkCounterUpdate) > 3.0f) {
      ...
      Msg("Benchmark: %d%% complete.\n",
          ((gpGlobals->tickcount - m_nBenchmarkStartTick) * 100) /
...
CON_COMMAND(sv_benchmark_force_start,
            "Force start the benchmark. This is only for debugging. It's "
            "better to set sv_benchmark to 1 and restart the level.") {
