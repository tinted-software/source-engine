...
//----------------------------------------------------------------------------------
// Purpose: Returns z value of floor below given point (up to fMaxDrop inches
// below) Input  : Output :
//----------------------------------------------------------------------------------
float GetFloorZ(const Vector &origin, float fMaxDrop) {
  ...
  //
  trace_t tr;
  AI_TraceLine(origin, origin - Vector(0, 0, fMaxDrop), MASK_NPCSOLID_BRUSHONLY,
               NULL, COLLISION_GROUP_NONE, &tr);

  // This trace is ONLY used if we hit an entity flagged with FL_WORLDBRUSH
  trace_t trEnt;
  AI_TraceLine(origin, origin - Vector(0, 0, fMaxDrop), MASK_NPCSOLID, NULL,
...
//			shifted away from the climb surface according to the
//hull size 			of the NPC using the climb
// Input  :
// Output :
//-----------------------------------------------------------------------------
Vector CAI_Node::GetPosition(int hull) {
  if (m_eNodeType == NODE_CLIMB) {
    ...
    // this is the floor resting position of this hull, adjusted to account for
    // mins.z
