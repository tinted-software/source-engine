...
static ConVar sv_lagcompensation_teleport_dist(
    ...
    "How far a player got moved by game code before we can't lag compensate "
    "their position back");
...
static void RestorePlayerTo(CBasePlayer *pPlayer, const Vector &vWantedPos) {
  ...
  if (tr.startsolid || tr.allsolid) {
    if (sv_unlag_debug.GetBool()) {
      DevMsg("RestorePlayerTo() could not restore player position for client "
             "\"%s\" ( %.1f %.1f %.1f )\n",
             pPlayer->GetPlayerName(), vWantedPos.x, vWantedPos.y,
             vWantedPos.z);
    }
...
void CLagCompensationManager::FrameUpdatePostEntityThink() {
  ...
  m_flTeleportDistanceSqr = sv_lagcompensation_teleport_dist.GetFloat() *
                            sv_lagcompensation_teleport_dist.GetFloat();
...
void CLagCompensationManager::BacktrackPlayer(CBasePlayer *pPlayer,
  ...
  if (prevRecord && (record->m_flSimulationTime < flTargetTime) &&
    ...
    minsPreScaled =
        Lerp(frac, record->m_vecMinsPreScaled, prevRecord->m_vecMinsPreScaled);
    maxsPreScaled =
        Lerp(frac, record->m_vecMaxsPreScaled, prevRecord->m_vecMaxsPreScaled);
  } else {
  ...
  if (sv_unlag_fixstuck.GetBool()) {
    ...
    if (tr.startsolid || tr.allsolid) {
      if (sv_unlag_debug.GetBool())
        DevMsg("WARNING: BackupPlayer trying to back player into a bad "
               "position - client %s\n",
               pPlayer->GetPlayerName());
...
void CLagCompensationManager::FinishLagCompensation(CBasePlayer *player) {
  ...
  for (int i = 1; i <= gpGlobals->maxClients; i++) {
    ...
    if (restore->m_fFlags & LC_SIZE_CHANGED) {
      ...
      // otherwise,
      //  leave new values in
      if (pPlayer->CollisionProp()->OBBMinsPreScaled() ==
              change->m_vecMinsPreScaled &&
          pPlayer->CollisionProp()->OBBMaxsPreScaled() ==
              change->m_vecMaxsPreScaled) {
        // Restore it
        pPlayer->SetSize(restore->m_vecMinsPreScaled,
                         restore->m_vecMaxsPreScaled);
      }
#ifdef STAGING_ONLY
      else {
