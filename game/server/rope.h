...
public:
  ...
  static CRopeKeyframe *
  Create(CBaseEntity *pStartEnt, CBaseEntity *pEndEnt, int iStartAttachment = 0,
  ...
  static CRopeKeyframe *CreateWithSecondPointDetached(
      ...
      int iStartAttachment =
          0, // must be 0 if you don't want to use a specific model attachment.
...
public:
  ...
  CNetworkVar(int, m_iRopeMaterialModelIndex); // Index of sprite model with the
                                               // rope's material.
  ...
  CNetworkVar(
      int, m_RopeLength); // Rope length at startup, used to calculate tension.
...
private:
  ...
  CNetworkVar(short, m_iStartAttachment); // StartAttachment/EndAttachment are
                                          // attachment points.
