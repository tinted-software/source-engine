    ...
    if (pEntity) {
      if (pEntity->m_flSpeed > 0) {
        if (pEntity->m_flSpeed <= 1.0) {
          ...
          Warning("path_track %s tried to tell the npc_vehicledriver to set "
                  "speed to %.3f. npc_vehicledriver only accepts values "
                  "between 0 and 1.\n",
...
bool CNPC_VehicleDriver::OverridePathMove(float flInterval) {
  ...
  m_flDistanceAlongSpline =
      clamp(m_flDistanceAlongSpline + flIncrement, 0.f, 2.f);
  if (m_flDistanceAlongSpline > 1) {
