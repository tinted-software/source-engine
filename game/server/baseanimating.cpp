...
DEFINE_FIELD(m_flGroundSpeed, FIELD_FLOAT),
    ...
    // Sendtable for fields we don't want to send to clientside animating
    // entities
    ...
    SendPropFloat(SENDINFO(m_flCycle), ANIMATION_CYCLE_BITS,
                  SPROP_CHANGES_OFTEN | SPROP_ROUNDDOWN, 0.0f,
                  1.0f) END_SEND_TABLE()
...
float CBaseAnimating::GetAnimTimeInterval(void) const {
  ...
  if (m_flAnimTime < gpGlobals->curtime) {
    ...
    flInterval =
        clamp(gpGlobals->curtime - m_flAnimTime, 0.f, MAX_ANIMTIME_INTERVAL);
  } else {
    // report actual
    flInterval =
        clamp(m_flAnimTime - m_flPrevAnimTime, 0.f, MAX_ANIMTIME_INTERVAL);
  }
  return flInterval;
}
  ...
  while ((index = GetAnimationEvent(pstudiohdr, GetSequence(), &event, flStart,
    ...
    */
    ...
    if (pNowStudioHdr != pstudiohdr) {
      ...
      Warning("%s has changed its model while processing AnimEvents on "
              "sequence %d. Aborting dispatch.\n",
              GetDebugName(), GetSequence());
      break;
    }
  ...
  if (m_flIKGroundContactTime > 0.2 &&
    ...
    if ((GetFlags() & (FL_FLY | FL_SWIM)) == 0 && GetMoveParent() == NULL &&
      ...
      if (toAbs.z == 0.0) {
        ...
        float bias =
            clamp((m_flIKGroundMaxHeight - m_flIKGroundMinHeight) - height, 0.f,
                  height);
        // save off reasonable offset
        m_flEstIkOffset =
            clamp(m_flEstIkFloor - GetAbsOrigin().z, -height + bias, 0.0f);
        return;
      }
    }
  }
  ...
  if (m_pIk) {
    ...
    for (int i = 0; i < m_pIk->m_target.Count(); i++) {
      ...
      case IK_GROUND: {
        ...
        /*
        debugoverlay->AddBoxOverlay( p1, Vector(-r,-r,0), Vector(r,r,1), QAngle(
        0, 0, 0 ), 255, 0, 0, 0, 1.0f ); debugoverlay->AddBoxOverlay(
        trace.endpos, Vector(-r,-r,0), Vector(r,r,1), QAngle( 0, 0, 0 ), 255, 0,
        0, 0, 1.0f ); debugoverlay->AddLineOverlay( p1, trace.endpos, 255, 0, 0,
        0, 1.0f );
        */
...
int CBaseAnimating::ExtractBbox(int sequence, Vector &mins, Vector &maxs) {
  ...
  return IsDynamicModelLoading()
             ? 0
             : ::ExtractBbox(GetModelPtr(), sequence, mins, maxs);
}
...
//probably means 			there's a typo in the model QC file, or the sequence is missing
//			entirely.
//
//
...
//remember each 			entity that registered an activity there, and the activity name
...
const char *
CBaseAnimating::GetFlexControllerName(LocalFlexController_t iFlexController) {
  CStudioHdr *pstudiohdr = GetModelPtr();
  if (!pstudiohdr)
    return 0;
...
const char *
CBaseAnimating::GetFlexControllerType(LocalFlexController_t iFlexController) {
  CStudioHdr *pstudiohdr = GetModelPtr();
  if (!pstudiohdr)
    return 0;
  ...
  if (mdl) {
    ...
    if (hStudioHdr != MDLHANDLE_INVALID) {
      ...
      if (pStudioHdrContainer && pStudioHdrContainer->GetVirtualModel()) {
        MDLHandle_t hVirtualModel = VoidPtrToMDLHandle(
            pStudioHdrContainer->GetRenderHdr()->VirtualModel());
...
void CBaseAnimating::GetSkeleton(CStudioHdr *pStudioHdr, Vector pos[],
  ...
  boneSetup.AccumulatePose(pos, q, GetSequence(), GetCycle(), 1.0,
...
void CBaseAnimating::DrawServerHitboxes(float duration /*= 0.0f*/,
  ...
  for (int i = 0; i < set->numhitboxes; i++) {
    ...
    NDebugOverlay::BoxAngles(position, pbox->bbmin * GetModelScale(),
                             pbox->bbmax * GetModelScale(), angles, r, g, b, 0,
                             duration);
  }
}
...
bool CBaseAnimating::ComputeHitboxSurroundingBox(Vector *pVecWorldMins,
  ...
  for (int i = 0; i < set->numhitboxes; i++) {
    ...
    if (pMatrix) {
      TransformAABB(*pMatrix, pbox->bbmin * GetModelScale(),
                    pbox->bbmax * GetModelScale(), vecBoxAbsMins,
                    vecBoxAbsMaxs);
