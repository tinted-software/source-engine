...
*/
#define MAX_ALARM_TIME_NO_RECEDE                                               \
  18 // max amount of time to play the alarm if the train isn't going to recede

BEGIN_DATADESC(CTeamTrainWatcher)

// Inputs.
DEFINE_INPUTFUNC(FIELD_VOID, "RoundActivate", InputRoundActivate),
    DEFINE_INPUTFUNC(FIELD_INTEGER, "SetNumTrainCappers",
                     InputSetNumTrainCappers),
    DEFINE_INPUTFUNC(FIELD_VOID, "OnStartOvertime", InputOnStartOvertime),
    DEFINE_INPUTFUNC(FIELD_VOID, "Enable", InputEnable),
    DEFINE_INPUTFUNC(FIELD_VOID, "Disable", InputDisable),
    DEFINE_INPUTFUNC(FIELD_FLOAT, "SetSpeedForwardModifier",
                     InputSetSpeedForwardModifier),
    DEFINE_INPUTFUNC(FIELD_INTEGER, "SetTrainRecedeTime",
                     InputSetTrainRecedeTime),
    DEFINE_INPUTFUNC(FIELD_BOOLEAN, "SetTrainCanRecede",
    ...
    DEFINE_KEYFIELD(m_iszLinkedPathTracks[0], FIELD_STRING,
                    "linked_pathtrack_1"),
    DEFINE_KEYFIELD(m_iszLinkedCPs[0], FIELD_STRING, "linked_cp_1"),

    DEFINE_KEYFIELD(m_iszLinkedPathTracks[1], FIELD_STRING,
                    "linked_pathtrack_2"),
    DEFINE_KEYFIELD(m_iszLinkedCPs[1], FIELD_STRING, "linked_cp_2"),

    DEFINE_KEYFIELD(m_iszLinkedPathTracks[2], FIELD_STRING,
                    "linked_pathtrack_3"),
    DEFINE_KEYFIELD(m_iszLinkedCPs[2], FIELD_STRING, "linked_cp_3"),

    DEFINE_KEYFIELD(m_iszLinkedPathTracks[3], FIELD_STRING,
                    "linked_pathtrack_4"),
    DEFINE_KEYFIELD(m_iszLinkedCPs[3], FIELD_STRING, "linked_cp_4"),

    DEFINE_KEYFIELD(m_iszLinkedPathTracks[4], FIELD_STRING,
                    "linked_pathtrack_5"),
    DEFINE_KEYFIELD(m_iszLinkedCPs[4], FIELD_STRING, "linked_cp_5"),

    DEFINE_KEYFIELD(m_iszLinkedPathTracks[5], FIELD_STRING,
                    "linked_pathtrack_6"),
    DEFINE_KEYFIELD(m_iszLinkedCPs[5], FIELD_STRING, "linked_cp_6"),

    DEFINE_KEYFIELD(m_iszLinkedPathTracks[6], FIELD_STRING,
                    "linked_pathtrack_7"),
    DEFINE_KEYFIELD(m_iszLinkedCPs[6], FIELD_STRING, "linked_cp_7"),

    DEFINE_KEYFIELD(m_iszLinkedPathTracks[7], FIELD_STRING,
                    "linked_pathtrack_8"),
    ...
    DEFINE_KEYFIELD(m_bHandleTrainMovement, FIELD_BOOLEAN,
    ...
    DEFINE_KEYFIELD(m_flSpeedForwardModifier, FIELD_FLOAT,
...
bool CTeamTrainWatcherMaster::FindTrainWatchers( void )
{
        ...
        CTeamTrainWatcher *pTrainWatcher = (CTeamTrainWatcher
*)gEntList.FindEntityByClassname( NULL, "team_train_watcher" ); while (
pTrainWatcher )
        {
                ...
                pTrainWatcher = (CTeamTrainWatcher
*)gEntList.FindEntityByClassname( pTrainWatcher, "team_train_watcher" );
        }
...
{
        if ( TeamplayRoundBasedRules() && TeamplayRoundBasedRules()->State_Get()
!= GR_STATE_RND_RUNNING )
        {
                // the next time we 'think'
                SetContextThink( &CTeamTrainWatcherMaster::TWMThink,
gpGlobals->curtime + 0.2, TWMASTER_THINK ); return;
        }



        // the next time we 'think'
        SetContextThink( &CTeamTrainWatcherMaster::TWMThink, gpGlobals->curtime
+ 0.2, TWMASTER_THINK );
}
...
TeamplayRoundBasedRules()->HasMultipleTrains() )
                {
                        if ( FindTrainWatchers() )
                        {
                                // we found train watchers so start thinking
                                SetContextThink(
&CTeamTrainWatcherMaster::TWMThink, gpGlobals->curtime + 0.2, TWMASTER_THINK );
                        }
                }
        }
        else if ( FStrEq( "teamplay_round_win", eventname ) )
        {
                if ( TeamplayRoundBasedRules() )
                {
                        ...
                        int iLosingTeam = ( iWinningTeam == TF_TEAM_RED ) ?
TF_TEAM_BLUE : TF_TEAM_RED; bool bFullRound = event->GetBool( "full_round" );
                        ...
                        if ( bFullRound )
                        {
                                EmitSound( filterWinner, entindex(),
TWM_FINALSTAGEOUTCOME01 ); EmitSound( filterLoser, entindex(),
TWM_FINALSTAGEOUTCOME02 );
                        }
                        else
                        {
                                EmitSound( filterWinner, entindex(),
TWM_FIRSTSTAGEOUTCOME01 ); EmitSound( filterLoser, entindex(),
TWM_FIRSTSTAGEOUTCOME02 );
                        }
                }
        }
}
...
CTeamTrainWatcher::CTeamTrainWatcher() {
  ...
  /*
          ...
          {
                  g_hTeamTrainWatcherMaster = CreateEntityByName(
     "team_train_watcher_master" );
          }
  */
  ListenForGameEvent("path_track_passed");
}

CTeamTrainWatcher::~CTeamTrainWatcher() { m_Sparks.Purge(); }
...
ConVar tf_escort_recede_time("tf_escort_recede_time", "30", 0, "", true, 0,
                             false, 0);
ConVar tf_escort_recede_time_overtime("tf_escort_recede_time_overtime", "5", 0,
                                      "", true, 0, false, 0);
...
void CTeamTrainWatcher::FireGameEvent(IGameEvent *event) {
  ...
  if (FStrEq(pszEventName, "path_track_passed")) {
    ...
    if (pNode) {
      ...
      if (bHandleEvent) {
        // If we're receding and we've hit a node but the next node (going
        // backwards) is disabled the train is going to stop (like at the base
        // of a downhill section) when we start forward again we won't pass this
        // node again so don't change our hill state based on this node.
        if (m_bReceding) {
        ...
        if (!bUpdate) {
          // the hill settings are the same, but are we leaving an uphill or
          // downhill segment?
          if (m_iCurrentHillType != HILL_TYPE_NONE) {
            ...
            if (pNextNode) {
              ...
              if (m_iCurrentHillType != iNextHillType) {
                // we're leaving an uphill or downhill segment...so reset our
                // state until we pass the next node
                bUpdate = true;
  ...
  if (m_hTrain) {
    ...
    if (bStartReceding) {
      ...
      if (m_nNumCappers > 0) {
        ...
        // we'll get a -1 for a blocked cart (speed should be 0 for that unless
        // we're on a hill)
        if (m_iCurrentHillType == HILL_TYPE_DOWNHILL) {
          flSpeed = 1.0f;
        }
...
void CTeamTrainWatcher::InternalSetNumTrainCappers(int iNumCappers,
  ...
  if (iNumCappers <= 0 && !m_bCapBlocked && m_bTrainCanRecede) {
    if (!m_bWaitingToRecede) {
      ...
      if (TeamplayRoundBasedRules() &&
          TeamplayRoundBasedRules()->InOvertime()) {
...
void CTeamTrainWatcher::SetNumTrainCappers(int iNumCappers,
                                           CBaseEntity *pTrigger) {
  if (IsDisabled() || !m_bHandleTrainMovement)
    return;
...
void CTeamTrainWatcher::FindGlowEntity(void) {
  if (m_hTrain && (m_hTrain->GetEntityName() != NULL_STRING)) {
    ...
    CPhysFixed *pPhysConstraint = dynamic_cast<CPhysFixed *>(
        gEntList.FindEntityByClassname(NULL, "phys_constraint"));
    while (pPhysConstraint) {
      ...
      pPhysConstraint = dynamic_cast<CPhysFixed *>(
          gEntList.FindEntityByClassname(pPhysConstraint, "phys_constraint"));
    }

    if (!pGlowEnt) {
      // if we're here, we haven't found the glow entity yet...try all of the
      // prop_dynamic entities
      CDynamicProp *pPropDynamic = dynamic_cast<CDynamicProp *>(
          gEntList.FindEntityByClassname(NULL, "prop_dynamic"));
      while (pPropDynamic) {
        ...
        pPropDynamic = dynamic_cast<CDynamicProp *>(
            gEntList.FindEntityByClassname(pPropDynamic, "prop_dynamic"));
      }
    }

    // if we still haven't found a glow entity, just have the CFuncTrackTrain
    // glow
    if (!pGlowEnt) {
...
void CTeamTrainWatcher::WatcherActivate(void) {
  ...
  // init our train
  m_hTrain = dynamic_cast<CFuncTrackTrain *>(
      gEntList.FindEntityByName(NULL, m_iszTrain));
  if (!m_hTrain) {
    Warning("%s failed to find train named '%s'\n", GetClassname(),
            STRING(m_iszTrain));
  }

  // find the trigger area that will give us movement updates and find the
  // sparks (if we're going to handle the train movement)
  if (m_bHandleTrainMovement) {
    if (m_hTrain) {
      for (int i = 0; i < ITriggerAreaCaptureAutoList::AutoList().Count();
           ++i) {
        CTriggerAreaCapture *pArea = static_cast<CTriggerAreaCapture *>(
            ITriggerAreaCaptureAutoList::AutoList()[i]);
        if (pArea->GetParent() == m_hTrain.Get()) {
          // this is the capture area we care about, so let it know that we want
          // updates on the capture numbers
    ...
    CEnvSpark *pSpark = dynamic_cast<CEnvSpark *>(
        gEntList.FindEntityByName(NULL, m_iszSparkName));
    while (pSpark) {
      m_Sparks.AddToTail(pSpark);
      pSpark = dynamic_cast<CEnvSpark *>(
          gEntList.FindEntityByName(pSpark, m_iszSparkName));
    }
  }
  ...
  for (i = 0; i < MAX_CONTROL_POINTS; i++) {
    CPathTrack *pPathTrack = dynamic_cast<CPathTrack *>(
        gEntList.FindEntityByName(NULL, m_iszLinkedPathTracks[i]));
    CTeamControlPoint *pCP = dynamic_cast<CTeamControlPoint *>(
        gEntList.FindEntityByName(NULL, m_iszLinkedCPs[i]));
    if (pPathTrack && pCP) {
      m_CPLinks[m_iNumCPLinks].hPathTrack = pPathTrack;
      m_CPLinks[m_iNumCPLinks].hCP = pCP;
      m_CPLinks[m_iNumCPLinks].flDistanceFromStart =
          0; // filled in when we parse the nodes
  ...
  m_hStartNode = dynamic_cast<CPathTrack *>(
      gEntList.FindEntityByName(NULL, m_iszStartNode));
  if (!m_hStartNode) {
    Warning("%s failed to find path_track named '%s'\n", GetClassname(),
            STRING(m_iszStartNode));
  }

  m_hGoalNode = dynamic_cast<CPathTrack *>(
      gEntList.FindEntityByName(NULL, m_iszGoalNode));
  if (!m_hGoalNode) {
    Warning("%s failed to find path_track named '%s'\n", GetClassname(),
            STRING(m_iszGoalNode));
  }
  ...
  if (m_hStartNode.Get() && m_hGoalNode.Get()) {
    ...
    // don't check the start node for links. If it's linked, it will have 0
    // distance anyway
    while (pNode) {
      ...
      if (pNode->GetHillType() != iHillType) {
        ...
        {
          float flDistance = m_flTotalPathDistance -
                             length; // subtract length because the prev node
                                     // was the end of the hill (not this one)

          if (pHillStart && (pHillStart == pPrev)) {
            flDistance =
                m_flTotalPathDistance; // we had a single node marked as a hill,
                                       // so we'll use the current distance as
                                       // the next marker
          }
  ...
  // if we don't have an even number of entries in our hill data (beginning/end)
  ...
  if (ObjectiveResource()) {
    ...
    if (m_flTotalPathDistance > 0 && hillData.Count() > 0) {
      ...
      while (i < hillData.Count()) {
        if (i < TEAM_TRAIN_HILLS_ARRAY_SIZE -
                    1) // - 1 because we want to use 2 entries
        {
          // add/subtract to the hill start/end to fix rounding errors in the
          // HUD when the train stops at the bottom/top of a hill but the HUD
          // thinks the train is still on the hill
          ObjectiveResource()->SetHillData(
              GetTeamNumber(), (hillData[i] / m_flTotalPathDistance) + 0.005f,
...
// This can be pulled once DoD includes team_objectiveresource.* and
// c_team_objectiveresource.*
#ifndef DOD_DLL
    ObjectiveResource()->SetTrainPathDistance(
  ...
  SetContextThink(&CTeamTrainWatcher::WatcherThink, gpGlobals->curtime + 0.1,
                  TW_THINK);
}
...
void CTeamTrainWatcher::StartCaptureAlarm(CTeamControlPoint *pPoint) {
  ...
  if (pPoint) {
    ...
    m_pAlarm = controller.SoundCreate(filter, pPoint->entindex(), CHAN_STATIC,
                                      TEAM_TRAIN_ALARM, ATTN_NORM);
...
void CTeamTrainWatcher::PlayCaptureAlert(CTeamControlPoint *pPoint,
...
void CTeamTrainWatcher::WatcherThink(void) {
  ...
  if (pTrain) {
    ...
    if (m_iTrainSpeedLevel != iOldTrainSpeedLevel) {
      ...
      if (TeamplayRoundBasedRules()) {
        if (m_iTrainSpeedLevel == 0 && iOldTrainSpeedLevel != 0) {
          TeamplayRoundBasedRules()->HaveAllPlayersSpeakConceptIfAllowed(
    ...
    if (m_iTrainSpeedLevel > 0 &&
        m_flNextSpeakForwardConceptTime < gpGlobals->curtime) {
      if (m_hAreaCap.Get()) {
    ...
    if (pNode) {
      ...
      m_flTotalProgress =
          clamp(1.0 - (flDistanceToGoal / m_flTotalPathDistance), 0.0, 1.0);
      ...
      for (int iCount = 0; iCount < m_iNumCPLinks; iCount++) {
        if (m_flTrainDistanceFromStart <
            m_CPLinks[iCount].flDistanceFromStart - TEAM_TRAIN_ALERT_DISTANCE) {
          // back up twice the alert distance before resetting our flag to play
          // the warning again
          if ((m_flTrainDistanceFromStart <
               m_CPLinks[iCount].flDistanceFromStart -
                   (TEAM_TRAIN_ALERT_DISTANCE *
                    2)) || // has receded back twice the alert distance or...
              (!m_bTrainCanRecede)) // used to catch the case where the train
                                    // doesn't normally recede but has rolled
                                    // back down a hill away from the CP
          {
            ...
            if (pMaster) {
              ...
              if (!pMaster->PlayingMiniRounds()) {
                for (int i = FIRST_GAME_TEAM; i < MAX_CONTROL_POINT_TEAMS;
                     i++) {
                  if (ObjectiveResource() &&
                      ObjectiveResource()->TeamCanCapPoint(
                          pCurrentPoint->GetPointIndex(), i)) {
        ...
        if (!bDisableAlarm) {
          if (!m_pAlarm) {
            if (m_iNumCPLinks > 0 && !m_bAlarmPlayed) {
              ...
              m_bAlarmPlayed =
                  true; // used to prevent the alarm from starting again on maps
                        // where the train doesn't recede (alarm loops for short
                        // time then only plays singles)
            }
          } else {
            if (!m_bTrainCanRecede) // if the train won't recede, we only want
                                    // to play the alarm for a short time
            {
              if (m_flAlarmEndTime > 0 &&
                  m_flAlarmEndTime < gpGlobals->curtime) {
                StopCaptureAlarm();
                SetContextThink(&CTeamTrainWatcher::WatcherAlarmThink,
                                gpGlobals->curtime + TW_ALARM_THINK_INTERVAL,
                                TW_ALARM_THINK);
              }
            }
          }
    ...
    if (tf_show_train_path.GetBool()) {
      ...
      while (node) {
        ...
        NDebugOverlay::Line(node->GetAbsOrigin(), nextNode->GetAbsOrigin(), 255,
                            255, 0, true, NDEBUG_PERSIST_TILL_NEXT_SERVER);
      ...
      if (node && node->GetNext()) {
        NDebugOverlay::HorzArrow(node->GetAbsOrigin(),
                                 node->GetNext()->GetAbsOrigin(), 5.0f, 255, 0,
                                 0, 255, true, NDEBUG_PERSIST_TILL_NEXT_SERVER);
      }
    }
  }
...
// Project the given position onto the track and return the point and how far
// along that projected position is
void CTeamTrainWatcher::ProjectPointOntoPath(
    const Vector &pos, Vector *posOnPathResult,
...
bool CTeamTrainWatcher::IsTrainNearCheckpoint(void) const {
  for (int i = 0; i < m_iNumCPLinks; ++i) {
    if (m_flTrainDistanceFromStart >
            m_CPLinks[i].flDistanceFromStart - TEAM_TRAIN_ALERT_DISTANCE &&
...
CON_COMMAND_F(tf_dumptrainstats,
              "Dump the stats for the current train watcher to the console",
              FCVAR_GAMEDLL) {
  ...
  while ((pWatcher =
              dynamic_cast<CTeamTrainWatcher *>(gEntList.FindEntityByClassname(
                  pWatcher, "team_train_watcher"))) != NULL) {
    pWatcher->DumpStats();
  }
}
...
void CTeamTrainWatcher::DumpStats(void) {
  ...
  V_strcat_safe(szOutput,
                (GetTeamNumber() == TF_TEAM_RED) ? "Red\n" : "Blue\n");

  for (int i = 0; i < m_iNumCPLinks; ++i) {
    float flDistance = m_CPLinks[i].flDistanceFromStart - flLastPosition;
    if (i == 0) {
      V_sprintf_safe(szTemp,
                     "\tControl Point: %d\tDistance from start: %0.2f\n", i + 1,
                     flDistance);
    } else {
      V_sprintf_safe(
          szTemp, "\tControl Point: %d\tDistance from previous point: %0.2f\n",
          i + 1, flDistance);
    }
