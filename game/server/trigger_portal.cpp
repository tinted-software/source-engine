...
ConVar portal_debug("portal_debug", "0", FCVAR_CHEAT,
                    "Turn on debugging for portal connections.");
...
public:
  ...
  // TEMP: Since brushes have no directionality, give this wall a forward face
  // specified in hammer
...
void CTriggerPortal::Activate() {
  ...
  // keep the remote portal's pointer at activate time to avoid redundant
  // FindEntity calls
...
void CTriggerPortal::SetRemotePortal(const char *strRemotePortalName) {
  m_hRemotePortal = dynamic_cast<CTriggerPortal *>(
      gEntList.FindEntityByName(NULL, strRemotePortalName, NULL, NULL, NULL));
...
//-----------------------------------------------------------------------------
// Purpose: Upon touching a non-filtered entity, CTriggerPortal teleports them
// to it's
...
void CTriggerPortal::Touch(CBaseEntity *pOther) {
  ...
  if (PassesTriggerFilters(pOther)) {
    ...
    EHANDLE hHandle;
    hHandle = pOther;
    if (m_hDisabledForEntities.Find(hHandle) !=
        m_hDisabledForEntities.InvalidIndex()) {
      Msg("    IGNORED\n", GetDebugName(), pOther->GetDebugName());
      return;
    }
    ...
    // Teleport our object
    VMatrix matRemotePortalTransform =
        m_hRemotePortal->EntityToWorldTransform();
    ...
    ptNewOrigin = matRemotePortalTransform *
                  Vector(ptNewOrigin.x, -ptNewOrigin.y, ptNewOrigin.z);

    // Re-aim camera
    vNewLook = matMyInverse.ApplyRotation(vLook);
    vNewLook = matRemotePortalTransform.ApplyRotation(
        Vector(-vNewLook.x, -vNewLook.y, vNewLook.z));
    ...
    vVelocity = matRemotePortalTransform.ApplyRotation(
        Vector(-vVelocity.x, -vVelocity.y, vVelocity.z));
    ...
    if (bDebug) {
      NDebugOverlay::Box(pOther->GetAbsOrigin(), pOther->WorldAlignMins(),
                         pOther->WorldAlignMaxs(), 255, 0, 0, 8, 20);
      NDebugOverlay::Axis(pOther->GetAbsOrigin(), pOther->GetAbsAngles(), 10.0f,
                          true, 50);
    }
    ...
    if (bDebug) {
      NDebugOverlay::Box(pOther->GetAbsOrigin(), pOther->WorldAlignMins(),
                         pOther->WorldAlignMaxs(), 0, 255, 0, 8, 20);
      NDebugOverlay::Line(vecOldPos, pOther->WorldSpaceCenter(), 0, 255, 0,
                          true, 20);
      NDebugOverlay::Axis(pOther->GetAbsOrigin(), pOther->GetAbsAngles(), 10.0f,
                          true, 50);
...
void CTriggerPortal::DisableForIncomingEntity(CBaseEntity *pEntity) {
  EHANDLE hHandle;
  hHandle = pEntity;
  Assert(m_hDisabledForEntities.Find(hHandle) ==
         m_hDisabledForEntities.InvalidIndex());
  ...
  SetContextThink(DisabledThink, gpGlobals->curtime + 0.1,
                  TRIGGER_DISABLED_THINK);
}
...
bool CTriggerPortal::IsTouchingPortal(CBaseEntity *pEntity) {
  // First, check the touchlinks. This will find non-vphysics entities touching
  // us
  ...
  // Then check the friction snapshot. This will find vphysics objects touching
  // us.
