...
public:
  ...
  virtual void UpdatePosition(
      void) // could be seen or heard, but now the entity's position is known
  {
    if (m_who.Get()) {
      m_lastKnownPostion = m_who->GetAbsOrigin();
      m_lastKnownArea =
          m_who->MyCombatCharacterPointer()
              ? m_who->MyCombatCharacterPointer()->GetLastKnownArea()
              : NULL;
  ...
  // This encapsulates the idea of "I just saw a guy right over *there* a few
  // seconds ago, but I don't know where he is now"
  ...
  virtual bool
  IsVisibleInFOVNow(void) const // return true if this entity is currently
                                // visible and in my field of view
  {
    return m_isVisible;
  }

  virtual bool
  IsVisibleRecently(void) const // return true if this entity is visible or was
                                // very recently visible
  {
    if (m_isVisible)
      return true;
  ...
  virtual bool IsObsolete(void) const {
    return GetEntity() == NULL || !m_who->IsAlive() ||
           GetTimeSinceLastKnown() > 10.0f;
  }
