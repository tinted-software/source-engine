 ...
 */
ILocomotion::ILocomotion(INextBot *bot) : INextBotComponent(bot) { Reset(); }
...
void ILocomotion::Update(void) {
  ...
  if (GetBot()->IsDebugging(NEXTBOT_LOCOMOTION)) {
    // show motion vector
    NDebugOverlay::HorzArrow(GetFeet(),
                             GetFeet() + 25.0f * m_groundMotionVector, 3.0f,
                             100, 255, 0, 255, true, 0.1f);
    NDebugOverlay::HorzArrow(GetFeet(), GetFeet() + 25.0f * m_motionVector,
                             5.0f, 255, 255, 0, 255, true, 0.1f);
  }
}
...
void ILocomotion::AdjustPosture(const Vector &moveGoal) {
  ...
  Vector goal =
      feet + moveLength * CrossProduct(left, groundNormal).Normalized();
  ...
  TraceHull(feet, goal, mins, crouchMaxs, body->GetSolidMask(), &filter,
            &trace);
 ...
 */
bool ILocomotion::IsPotentiallyTraversable(const Vector &from, const Vector &to,
  ...
  // use a small hull since we cannot simulate collision resolution and
  // avoidance along the way
  const float probeSize =
      0.25f * GetBot()
                  ->GetBodyInterface()
                  ->GetHullWidth(); // Cant be TOO small, or open
                                    // stairwells/grates/etc will cause problems
  const float probeZ = GetStepHeight();

  Vector hullMin(-probeSize, -probeSize, probeZ);
  Vector hullMax(probeSize, probeSize,
                 GetBot()->GetBodyInterface()->GetCrouchHullHeight());
  TraceHull(from, to, hullMin, hullMax,
            GetBot()->GetBodyInterface()->GetSolidMask(), &filter, &result);

  /*
          if ( result.DidHit() )
          {
                  NDebugOverlay::SweptBox( from, result.endpos, hullMin,
     hullMax, vec3_angle, 255, 0, 0, 255, 9999.9f ); NDebugOverlay::SweptBox(
     result.endpos, to, hullMin, hullMax, vec3_angle, 255, 255, 0, 255, 9999.9f
     );
          }
          else
          {
                  NDebugOverlay::SweptBox( from, to, hullMin, hullMax,
     vec3_angle, 255, 255, 0, 255, 0.1f );
          }
  */
 ...
 */
bool ILocomotion::HasPotentialGap(const Vector &from, const Vector &desiredTo,
                                  float *fraction) const {
...
bool ILocomotion::IsGap(const Vector &pos, const Vector &forward) const {
  ...
  NextBotTraceFilterIgnoreActors filter(GetBot()->GetEntity(),
                                        COLLISION_GROUP_NONE);

  TraceHull(
      pos + Vector(0, 0,
                   GetStepHeight()), // start up a bit to handle rough terrain
  ...
  // 							 pos + Vector( 0, 0,
  // -GetStepHeight() ), 							 Vector( -halfWidth, -halfWidth, 0 ), Vector( halfWidth,
  // halfWidth, hullHeight ), 							 vec3_angle, 							 r, g, b, 255, 3.0f );
...
//----------------------------------------------------------------------------------------------------------
bool ILocomotion::IsEntityTraversable(CBaseEntity *obstacle,
                                      TraverseWhenType when) const {
  if (obstacle->IsWorld())
    return false;

  // assume bot will open a door in its path
  if (FClassnameIs(obstacle, "prop_door*") ||
      FClassnameIs(obstacle, "func_door*")) {
  ...
  if (when == IMMEDIATELY) {
    // special rules in specific games can immediately break some breakables,
    // etc.
    return false;
  }
...
void ILocomotion::ClearStuckStatus(const char *reason) {
  ...
  // always reset stuck monitoring data in case we cleared preemptively are were
  // not yet stuck
  m_stuckPos = GetFeet();
  m_stuckTimer.Start();
  ...
  if (GetBot()->IsDebugging(NEXTBOT_LOCOMOTION)) {
    DevMsg("%3.2f: ClearStuckStatus: %s %s\n", gpGlobals->curtime,
           GetBot()->GetDebugIdentifier(), reason);
  }
}
...
void ILocomotion::StuckMonitor(void) {
  // a timer is needed to smooth over a few frames of inactivity due to state
  // changes, etc. we only want to detect idle situations when the bot really
  ...
  if (IsStuck()) {
    // we are/were stuck - have we moved enough to consider ourselves
    // "dislodged"
    if (GetBot()->IsRangeGreaterThan(m_stuckPos, STUCK_RADIUS)) {
      ...
      if (m_stillStuckTimer.IsElapsed()) {
        ...
        if (GetBot()->IsDebugging(NEXTBOT_LOCOMOTION)) {
          ...
          NDebugOverlay::Circle(m_stuckPos + Vector(0, 0, 5.0f),
                                QAngle(-90.0f, 0, 0), 5.0f, 255, 0, 0, 255,
                                true, 1.0f);
        }
    ...
    if (/*IsClimbingOrJumping() || */ GetBot()->IsRangeGreaterThan(
        m_stuckPos, STUCK_RADIUS)) {
      ...
      if (GetBot()->IsDebugging(NEXTBOT_LOCOMOTION)) {
        NDebugOverlay::Line(GetBot()->GetEntity()->WorldSpaceCenter(),
                            m_stuckPos, 255, 0, 255, true, 0.1f);
      }
      ...
      if (m_stuckTimer.IsGreaterThen(escapeTime)) {
        ...
        if (GetBot()->IsDebugging(NEXTBOT_ERRORS)) {
          DevMsg("%3.2f: %s STUCK at position( %3.2f, %3.2f, %3.2f )\n",
                 ...
                 m_stuckPos.x, m_stuckPos.y, m_stuckPos.z);

          NDebugOverlay::Circle(m_stuckPos + Vector(0, 0, 15.0f),
                                QAngle(-90.0f, 0, 0), 3.0f, 255, 255, 0, 255,
                                true, 1.0f);
          NDebugOverlay::Circle(m_stuckPos + Vector(0, 0, 5.0f),
                                QAngle(-90.0f, 0, 0), 5.0f, 255, 0, 0, 255,
                                true, 9999999.9f);
