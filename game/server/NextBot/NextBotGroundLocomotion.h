...
/**
 * Basic ground-based movement for NextBotCombatCharacters.
 * This locomotor resolves collisions and assumes a ground-based bot under the
...
public:
  ...
  virtual void
  Approach(const Vector &pos,
           float goalWeight = 1.0f); // move directly towards the given position
  virtual void DriveTo(const Vector &pos); // Move the bot to the precise given
                                           // position immediately,

  virtual bool ClimbUpToLedge(
      const Vector &landingGoal, const Vector &landingForward,
      const CBaseEntity *obstacle); // initiate a jump to an adjacent high
                                    // ledge, return false if climb can't start
  virtual void JumpAcrossGap(
      const Vector &landingGoal,
      const Vector &landingForward); // initiate a jump across an empty volume
                                     // of space to far side
  ...
  virtual void
  SetDesiredSpeed(float speed); // set desired speed for locomotor movement
  virtual float
  GetDesiredSpeed(void) const; // returns the current desired speed

  virtual float GetSpeedLimit(void)
      const; // get maximum speed bot can reach, regardless of desired speed
  ...
  virtual void ClimbLadder(
      ...
      const CNavArea
          *dismountGoal); // climb the given ladder to the top and dismount
  virtual void DescendLadder(
      const CNavLadder *ladder,
      const CNavArea
          *dismountGoal); // descend the given ladder to the bottom and dismount
  ...
  virtual void
  FaceTowards(const Vector &target); // rotate body to face towards "target"
  ...
  virtual const Vector &
  GetAcceleration(void) const; // return current world space acceleration
  virtual void
  SetAcceleration(const Vector &accel); // set world space acceleration
  ...
  virtual void OnMoveToSuccess(
      const Path *path); // invoked when an bot reaches its MoveTo goal
  virtual void
  OnMoveToFailure(const Path *path,
                  MoveToFailureType reason); // invoked when an bot fails to
                                             // reach a MoveTo goal

private:
  void UpdatePosition(const Vector &newPos); // move to newPos, resolving any
  ...
  Vector ResolveZombieCollisions(
      const Vector &pos); // push away zombies that are interpenetrating
  Vector
  ResolveCollision(const Vector &from, const Vector &to,
                   int recursionLimit); // check for collisions along move
  bool DetectCollision(trace_t *pTrace, int &nDestructionAllowed,
  ...
  virtual float
  GetFrictionForward(void) const; // return magnitude of forward friction
  virtual float
  GetFrictionSideways(void) const; // return magnitude of lateral friction
...
private:
  ...
  bool m_isJumpingAcrossGap; // if true, we have jumped across a gap and have
                             // not yet hit the ground
  EHANDLE m_ground;      // have to manage this ourselves, since MOVETYPE_CUSTOM
                         // always NULLs out GetGroundEntity()
  ...
  bool m_isUsingFullFeetTrace; // true if we're in the air and tracing the
                               // lowest StepHeight in ResolveCollision

  const CNavLadder *m_ladder; // ladder we are currently climbing/descending
  const CNavArea *m_ladderDismountGoal; // the area we enter when finished with
                                        // our ladder move
  bool m_isGoingUpLadder;               // if false, we're going down

  CountdownTimer m_inhibitObstacleAvoidanceTimer; // when active, turn off path
                                                  // following feelers
  ...
  Vector m_accumApproachVectors; // weighted sum of Approach() calls since last
                                 // update
  float m_accumApproachWeights;
  bool m_bRecomputePostureOnCollision;

  CountdownTimer
      m_ignorePhysicsPropTimer; // if active, don't collide with physics props
                                // (because we got stuck in one)
  EHANDLE m_ignorePhysicsProp;  // which prop to ignore
};

inline float NextBotGroundLocomotion::GetGravity(void) const { return 1000.0f; }
...
inline bool NextBotGroundLocomotion::IsRunning(void) const {
  /// @todo Rethink interface to distinguish actual state vs desired state (do
  /// we want to be running, or are we actually at running speed right now)
...
inline float NextBotGroundLocomotion::GetRunSpeed(void) const { return 150.0f; }

inline float NextBotGroundLocomotion::GetWalkSpeed(void) const { return 75.0f; }
