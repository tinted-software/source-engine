 ...
 * to derived classes via FirstContainedResponder() and
 * NextContainedResponder(). NOTE: Also add a translator to the Action class in
 * NextBotBehavior.h.
 */
...
public:
  ...
  //
  // Events.  All events must be 'extended' by calling the derived class
  // explicitly to ensure propagation. Each event must implement its propagation
  // in this interface class.
  //
  virtual void OnLeaveGround(
      CBaseEntity *ground); // invoked when bot leaves ground for any reason
  virtual void
  OnLandOnGround(CBaseEntity *ground); // invoked when bot lands on the ground
                                       // after being in the air

  virtual void
  OnContact(CBaseEntity *other,
            CGameTrace *result = NULL); // invoked when bot touches 'other'

  virtual void OnMoveToSuccess(
      const Path *path); // invoked when a bot reaches the end of the given Path
  virtual void
  OnMoveToFailure(const Path *path,
                  MoveToFailureType reason); // invoked when a bot fails to
                                             // reach the end of the given Path
  virtual void
  OnStuck(void); // invoked when bot becomes stuck while trying to move
  virtual void OnUnStuck(void); // invoked when a previously stuck bot becomes
                                // un-stuck and can again move

  virtual void OnPostureChanged(
      void); // when bot has assumed new posture (query IBody for posture)

  virtual void OnAnimationActivityComplete(
      int activity); // when animation activity has finished playing
  virtual void
  OnAnimationActivityInterrupted(int activity); // when animation activity was
                                                // replaced by another animation
  virtual void OnAnimationEvent(
      animevent_t *event); // when a QC-file animation event is triggered by the
                           // current animation sequence

  virtual void OnIgnite(void); // when bot starts to burn
  virtual void
  OnInjured(const CTakeDamageInfo &info); // when bot is damaged by something
  virtual void
  OnKilled(const CTakeDamageInfo &info); // when the bot's health reaches zero
  virtual void
  OnOtherKilled(CBaseCombatCharacter *victim,
                const CTakeDamageInfo &info); // when someone else dies

  virtual void OnSight(CBaseEntity *subject); // when subject initially enters
                                              // bot's visual awareness
  virtual void OnLostSight(CBaseEntity *subject); // when subject leaves enters
                                                  // bot's visual awareness

  virtual void OnSound(CBaseEntity *source, const Vector &pos,
                       KeyValues *keys); // when an entity emits a sound. "pos"
                                         // is world coordinates of sound.
                                         // "keys" are from sound's GameData
  virtual void
  OnSpokeConcept(CBaseCombatCharacter *who, AIConcept_t concept,
                 AI_Response *response); // when an Actor speaks a concept
  virtual void
  OnWeaponFired(CBaseCombatCharacter *whoFired,
                CBaseCombatWeapon *weapon); // when someone fires a weapon

  virtual void
  OnNavAreaChanged(CNavArea *newArea,
                   CNavArea *oldArea); // when bot enters a new navigation area

  virtual void OnModelChanged(void); // when the entity's model has been changed

  virtual void OnPickUp(
      CBaseEntity *item,
      CBaseCombatCharacter *giver); // when something is added to our inventory
  virtual void
  OnDrop(CBaseEntity *item); // when something is removed from our inventory
  virtual void OnActorEmoted(CBaseCombatCharacter *emoter,
                             int emote); // when "emoter" does an "emote" (ie:
                                         // manual voice command, etc)

  virtual void OnCommandAttack(CBaseEntity *victim); // attack the given entity
  virtual void OnCommandApproach(
      const Vector &pos,
      float range = 0.0f); // move to within range of the given position
  virtual void OnCommandApproach(CBaseEntity *goal); // follow the given leader
  virtual void
  OnCommandRetreat(CBaseEntity *threat,
                   float range = 0.0f); // retreat from the threat at least
                                        // range units away (0 == infinite)
  virtual void OnCommandPause(
      float duration = 0.0f); // pause for the given duration (0 == forever)
  virtual void OnCommandResume(void); // resume after a pause

  virtual void OnCommandString(
      const char *command); // for debugging: respond to an arbitrary string
                            // representing a generalized command

  virtual void OnShoved(CBaseEntity *pusher); // 'pusher' has shoved me
  virtual void OnBlinded(
      CBaseEntity *blinder); // 'blinder' has blinded me with a flash of light

  virtual void OnTerritoryContested(
      int territoryID); // territory has been invaded and is changing ownership
  virtual void
  OnTerritoryCaptured(int territoryID); // we have captured enemy territory
  virtual void
  OnTerritoryLost(int territoryID); // we have lost territory to the enemy
...
inline void INextBotEventResponder::OnLeaveGround(CBaseEntity *ground) {
  for (INextBotEventResponder *sub = FirstContainedResponder(); sub;
       sub = NextContainedResponder(sub)) {
...
inline void INextBotEventResponder::OnContact(CBaseEntity *other,
                                              CGameTrace *result) {
  for (INextBotEventResponder *sub = FirstContainedResponder(); sub;
       sub = NextContainedResponder(sub)) {
...
inline void INextBotEventResponder::OnMoveToFailure(const Path *path,
...
inline void
INextBotEventResponder::OnAnimationActivityInterrupted(int activity) {
  for (INextBotEventResponder *sub = FirstContainedResponder(); sub;
       sub = NextContainedResponder(sub)) {
...
inline void INextBotEventResponder::OnOtherKilled(CBaseCombatCharacter *victim,
...
inline void INextBotEventResponder::OnSound(CBaseEntity *source,
                                            const Vector &pos,
                                            KeyValues *keys) {
  for (INextBotEventResponder *sub = FirstContainedResponder(); sub;
       sub = NextContainedResponder(sub)) {
...
inline void INextBotEventResponder::OnSpokeConcept(CBaseCombatCharacter *who,
                                                   AIConcept_t concept,
...
inline void
INextBotEventResponder::OnWeaponFired(CBaseCombatCharacter *whoFired,
...
inline void INextBotEventResponder::OnNavAreaChanged(CNavArea *newArea,
                                                     CNavArea *oldArea) {
  for (INextBotEventResponder *sub = FirstContainedResponder(); sub;
       sub = NextContainedResponder(sub)) {
...
inline void INextBotEventResponder::OnPickUp(CBaseEntity *item,
...
inline void INextBotEventResponder::OnActorEmoted(CBaseCombatCharacter *emoter,
                                                  int emote) {
  for (INextBotEventResponder *sub = FirstContainedResponder(); sub;
       sub = NextContainedResponder(sub)) {
...
inline void INextBotEventResponder::OnCommandApproach(const Vector &pos,
                                                      float range) {
  for (INextBotEventResponder *sub = FirstContainedResponder(); sub;
       sub = NextContainedResponder(sub)) {
...
inline void INextBotEventResponder::OnCommandRetreat(CBaseEntity *threat,
                                                     float range) {
  for (INextBotEventResponder *sub = FirstContainedResponder(); sub;
       sub = NextContainedResponder(sub)) {
...
inline void
INextBotEventResponder::OnCommandMoveToAggressive(const Vector &pos) {
  for (INextBotEventResponder *sub = FirstContainedResponder(); sub;
       sub = NextContainedResponder(sub)) {
...
inline void
INextBotEventResponder::OnCastAbilityNoTarget(CDOTABaseAbility *ability) {
  for (INextBotEventResponder *sub = FirstContainedResponder(); sub;
       sub = NextContainedResponder(sub)) {
...
inline void
INextBotEventResponder::OnCastAbilityOnPosition(CDOTABaseAbility *ability,
                                                const Vector &pos) {
  for (INextBotEventResponder *sub = FirstContainedResponder(); sub;
       sub = NextContainedResponder(sub)) {
...
inline void
INextBotEventResponder::OnCastAbilityOnTarget(CDOTABaseAbility *ability,
                                              CBaseEntity *target) {
  for (INextBotEventResponder *sub = FirstContainedResponder(); sub;
       sub = NextContainedResponder(sub)) {
...
inline void INextBotEventResponder::OnDropItem(const Vector &pos,
                                               CBaseEntity *item) {
  for (INextBotEventResponder *sub = FirstContainedResponder(); sub;
       sub = NextContainedResponder(sub)) {
...
inline void INextBotEventResponder::OnFriendThreatened(CBaseEntity *friendly,
                                                       CBaseEntity *threat) {
  for (INextBotEventResponder *sub = FirstContainedResponder(); sub;
       sub = NextContainedResponder(sub)) {
