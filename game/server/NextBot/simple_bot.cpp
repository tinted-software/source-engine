...
CON_COMMAND_F(simple_bot_add, "Add a simple bot.", FCVAR_CHEAT) {
  ...
  UTIL_TraceLine(player->EyePosition(),
                 player->EyePosition() + 999999.9f * forward,
                 MASK_BLOCKLOS_AND_NPCS | CONTENTS_IGNORE_NODRAW_OPAQUE, player,
...
//---------------------------------------------------------------------------------------------
/**
 * This action causes the bot to pick a random nav area in the mesh and move to
 * it, then pick another, etc. Actions usually each have their own .cpp/.h file
 * and are organized into folders since there are often many of them. For this
 * example, we're keeping everything to a single .cpp/.h file.
 */
class CSimpleBotRoam : public Action<CSimpleBot> {
public:
  //----------------------------------------------------------------------------------
  // OnStart is called once when the Action first becomes active
  virtual ActionResult<CSimpleBot> OnStart(CSimpleBot *me,
                                           Action<CSimpleBot> *priorAction) {
    // smooth out the bot's path following by moving toward a point farther down
    // the path
  ...
  //----------------------------------------------------------------------------------
  // Update is called repeatedly (usually once per server frame) while the
  // Action is active
  virtual ActionResult<CSimpleBot> Update(CSimpleBot *me, float interval) {
    if (m_path.IsValid() && !m_timer.IsElapsed()) {
      // PathFollower::Update() moves the bot along the path using the bot's
      // ILocomotion and IBody interfaces
      m_path.Update(me);
    } else {
      SelectNthAreaFunctor pick(
          RandomInt(0, TheNavMesh->GetNavAreaCount() - 1));
  ...
  //----------------------------------------------------------------------------------
  // this is an event handler - many more are available (see declaration of
  // Action< Actor > in NextBotBehavior.h)
  virtual EventDesiredResult<CSimpleBot> OnStuck(CSimpleBot *me) {
    // we are stuck trying to follow the current path - invalidate it so a new
    // one is chosen
    m_path.Invalidate();
...
//---------------------------------------------------------------------------------------------
/**
 * Instantiate the bot's Intention interface and start the initial Action
 * (CSimpleBotRoam in this case)
 */
