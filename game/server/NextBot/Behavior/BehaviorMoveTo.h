...
public:
  BehaviorMoveTo(const Vector &goal, Action<Actor> *successAction = NULL,
                 Action<Actor> *failAction = NULL);
...
//----------------------------------------------------------------------------------------------
template <typename Actor, typename PathCost>
inline BehaviorMoveTo<Actor, PathCost>::BehaviorMoveTo(
    const Vector &goal, Action<Actor> *successAction,
...
//----------------------------------------------------------------------------------------------
template <typename Actor, typename PathCost>
inline bool BehaviorMoveTo<Actor, PathCost>::ComputePath(Actor *me,
                                                         const Vector &goal,
                                                         PathFollower *path) {
  PathCost cost(me);
...
//----------------------------------------------------------------------------------------------
template <typename Actor, typename PathCost>
inline ActionResult<Actor>
BehaviorMoveTo<Actor, PathCost>::OnStart(Actor *me,
...
template <typename Actor, typename PathCost>
inline ActionResult<Actor>
BehaviorMoveTo<Actor, PathCost>::Update(Actor *me, float interval) {
  ...
  // move along path - success/fail event handlers will exit behavior when goal
  // is reached
  m_path.Update(me);
...
//----------------------------------------------------------------------------------------------
template <typename Actor, typename PathCost>
inline EventDesiredResult<Actor>
BehaviorMoveTo<Actor, PathCost>::OnMoveToSuccess(Actor *me, const Path *path) {
  if (m_successAction) {
    return this->TryChangeTo(m_successAction, RESULT_CRITICAL,
                             "OnMoveToSuccess");
  }
...
template <typename Actor, typename PathCost>
inline EventDesiredResult<Actor>
BehaviorMoveTo<Actor, PathCost>::OnMoveToFailure(Actor *me, const Path *path,
                                                 MoveToFailureType reason) {
  if (m_failAction) {
