...
ConVar nb_head_aim_steady_max_rate("nb_head_aim_steady_max_rate", "100",
                                   FCVAR_CHEAT);
ConVar nb_head_aim_settle_duration("nb_head_aim_settle_duration", "0.3",
                                   FCVAR_CHEAT);
ConVar nb_head_aim_resettle_angle(
    "nb_head_aim_resettle_angle", "100", FCVAR_CHEAT,
    "After rotating through this angle, the bot pauses to 'recenter' its "
    "virtual mouse on its virtual mousepad");
ConVar nb_head_aim_resettle_time("nb_head_aim_resettle_time", "0.3",
                                 FCVAR_CHEAT,
                                 "How long the bot pauses to 'recenter' its "
                                 "virtual mouse on its virtual mousepad");
...
void PressFireButtonReply::OnSuccess(INextBot *bot) {
  INextBotPlayerInput *playerInput =
      dynamic_cast<INextBotPlayerInput *>(bot->GetEntity());
  if (playerInput) {
...
void PlayerBody::Upkeep(void) {
  ...
  if (GetBot()->IsDebugging(NEXTBOT_LOOK_AT)) {
    if (IsHeadSteady()) {
      ...
      t = clamp(t, 0.f, 1.0f);
      NDebugOverlay::Circle(player->EyePosition(), t * 10.0f, 0, 255, 0, 255,
                            true, 2.0f * deltaT);
    }
  }
  ...
  if (deltaAngle > nb_head_aim_resettle_angle.GetFloat()) {
    ...
    m_anchorRepositionTimer.Start(RandomFloat(0.9f, 1.1f) *
                                  nb_head_aim_resettle_time.GetFloat());
    m_anchorForward = forward;
    return;
  }

  // if we're currently recentering our "virtual mouse", wait
  if (m_anchorRepositionTimer.HasStarted() &&
      !m_anchorRepositionTimer.IsElapsed()) {
    return;
  }
  m_anchorRepositionTimer.Invalidate();
  ...
  if (subject) {
    if (m_lookAtTrackingTimer.IsElapsed()) {
      // update subject tracking by periodically estimating linear aim velocity,
      // allowing for "slop" between updates
      Vector desiredLookAtPos;

      if (subject->MyCombatCharacterPointer()) {
        desiredLookAtPos = GetBot()->GetIntentionInterface()->SelectTargetPoint(
            GetBot(), subject->MyCombatCharacterPointer());
      } else {
        desiredLookAtPos = subject->WorldSpaceCenter();
      }

      desiredLookAtPos +=
          GetHeadAimSubjectLeadTime() * subject->GetAbsVelocity();
  ...
  if (GetBot()->IsDebugging(NEXTBOT_LOOK_AT)) {
    NDebugOverlay::Line(GetEyePosition(), GetEyePosition() + 100.0f * forward,
                        255, 255, 0, false, 2.0f * deltaT);

    float thickness = isSteady ? 2.0f : 3.0f;
    int r = m_isSightedIn ? 255 : 0;
    int g = subject ? 255 : 0;
    NDebugOverlay::HorzArrow(GetEyePosition(), m_lookAtPos, thickness, r, g,
                             255, 255, false, 2.0f * deltaT);
  }
  ...
  angles.y =
      ApproachAngle(desiredAngles.y, currentAngles.y, approachRate * deltaT);
  angles.x = ApproachAngle(desiredAngles.x, currentAngles.x,
                           0.5f * approachRate * deltaT);
  angles.z = 0.0f;
 ...
 */
void PlayerBody::AimHeadTowards(const Vector &lookAtPos,
                                LookAtPriorityType priority, float duration,
  ...
  if (m_lookAtPriority == priority) {
    if (!IsHeadSteady() ||
        GetHeadSteadyDuration() < nb_head_aim_settle_duration.GetFloat()) {
      ...
      if (GetBot()->IsDebugging(NEXTBOT_LOOK_AT)) {
        ConColorMsg(Color(255, 0, 0, 255),
                    "%3.2f: %s Look At '%s' rejected - previous aim not %s\n",
  ...
  // do NOT clear this here, or continuous calls to AimHeadTowards will keep
  // IsHeadAimingOnTarget returning false all of the time m_isSightedIn = false;

  m_hasBeenSightedIn = false;

  if (GetBot()->IsDebugging(NEXTBOT_LOOK_AT)) {
    NDebugOverlay::Cross3D(lookAtPos, 2.0f, 255, 255, 100, true,
                           2.0f * duration);
    ...
    ConColorMsg(
        Color(255, 100, 0, 255),
        "%3.2f: %s Look At ( %g, %g, %g ) for %3.2f s, Pri = %s, Reason = %s\n",
        gpGlobals->curtime, m_player->GetPlayerName(), lookAtPos.x, lookAtPos.y,
        lookAtPos.z, duration, priName, (reason) ? reason : "");
  }
}
 ...
 * Aim the bot's head towards the given goal
 */
void PlayerBody::AimHeadTowards(CBaseEntity *subject,
                                LookAtPriorityType priority, float duration,
                                INextBotReply *replyWhenAimed,
                                const char *reason) {
  if (duration <= 0.0f) {
  ...
  if (GetBot()->IsDebugging(NEXTBOT_LOOK_AT)) {
    ...
    ConColorMsg(
        Color(255, 100, 0, 255),
        "%3.2f: %s Look At subject %s for %3.2f s, Pri = %s, Reason = %s\n",
        gpGlobals->curtime, m_player->GetPlayerName(), subject->GetClassname(),
        duration, priName, (reason) ? reason : "");
  }
}
...
float PlayerBody::GetHeadSteadyDuration(void) const {
  // return ( IsHeadAimingOnTarget() ) ? m_headSteadyTimer.GetElapsedTime() :
  // 0.0f;
  return m_headSteadyTimer.HasStarted() ? m_headSteadyTimer.GetElapsedTime()
                                        : 0.0f;
}
 ...
 */
Activity PlayerBody::GetActivity(void) const { return ACT_INVALID; }

//-----------------------------------------------------------------------------------------------
/**
 * Return true if currently animating activity matches the given one
 */
bool PlayerBody::IsActivity(Activity act) const { return false; }

//-----------------------------------------------------------------------------------------------
/**
 * Return true if currently animating activity has any of the given flags
 */
bool PlayerBody::HasActivityType(unsigned int flags) const { return false; }

//-----------------------------------------------------------------------------------------------
/**
 * Request a posture change
 */
void PlayerBody::SetDesiredPosture(PostureType posture) { m_posture = posture; }
 ...
 */
bool PlayerBody::IsInDesiredPosture(void) const { return true; }
 ...
 */
bool PlayerBody::IsPostureMobile(void) const { return true; }

//-----------------------------------------------------------------------------------------------
/**
 * Return true if body's posture is in the process of changing to new posture
 */
bool PlayerBody::IsPostureChanging(void) const { return false; }

//-----------------------------------------------------------------------------------------------
/**
 * Arousal level change
 */
void PlayerBody::SetArousal(ArousalType arousal) { m_arousal = arousal; }

//-----------------------------------------------------------------------------------------------
/**
 * Get arousal level
 */
IBody::ArousalType PlayerBody::GetArousal(void) const { return m_arousal; }
...
float PlayerBody::GetCrouchHullHeight(void) const {
  return VEC_DUCK_HULL_MAX_SCALED(m_player).z -
         VEC_DUCK_HULL_MIN_SCALED(m_player).z;
}
...
//-----------------------------------------------------------------------------------------------
/**
 * Return the bot's collision mask (hack until we get a general hull trace
 * abstraction here or in the locomotion interface)
 */
