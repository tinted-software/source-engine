...
PlayerLocomotion::LadderState PlayerLocomotion::ApproachAscendingLadder(void) {
  ...
  // it is important to approach precisely, so use a very large weight to wash
  // out all other Approaches
  ...
  if (GetBot()->IsDebugging(NEXTBOT_LOCOMOTION)) {
    NDebugOverlay::EntityText(GetBot()->GetEntity()->entindex(), 0,
                              "Approach ascending ladder", 0.1f, 255, 255, 255,
                              255);
  }
...
PlayerLocomotion::LadderState PlayerLocomotion::ApproachDescendingLadder(void) {
  ...
  Vector mountPoint = m_ladderInfo->m_top +
                      0.25f * GetBot()->GetBodyInterface()->GetHullWidth() *
                          m_ladderInfo->GetNormal();
  ...
  if (GetBot()->IsDebugging(NEXTBOT_LOCOMOTION)) {
    NDebugOverlay::EntityText(GetBot()->GetEntity()->entindex(), 0,
                              "Approach descending ladder", 0.1f, 255, 255, 255,
                              255);
  }
...
PlayerLocomotion::LadderState PlayerLocomotion::AscendLadder(void) {
  ...
  Vector goal =
      GetFeet() + 100.0f * (-m_ladderInfo->GetNormal() + Vector(0, 0, 2));

  GetBot()->GetBodyInterface()->AimHeadTowards(goal, IBody::MANDATORY, 0.1f,
                                               NULL, "Ladder");
  ...
  if (GetBot()->IsDebugging(NEXTBOT_LOCOMOTION)) {
    NDebugOverlay::EntityText(GetBot()->GetEntity()->entindex(), 0, "Ascend",
...
PlayerLocomotion::LadderState PlayerLocomotion::DescendLadder(void) {
  ...
  if (GetFeet().z <= m_ladderInfo->m_bottom.z +
                         GetBot()->GetLocomotionInterface()->GetStepHeight()) {
  ...
  if (GetBot()->IsDebugging(NEXTBOT_LOCOMOTION)) {
    NDebugOverlay::EntityText(GetBot()->GetEntity()->entindex(), 0, "Descend",
                              0.1f, 255, 255, 255, 255);
  }
...
PlayerLocomotion::LadderState PlayerLocomotion::DismountLadderTop(void) {
  ...
  body->AimHeadTowards(body->GetEyePosition() + 100.0f * toGoal,
                       IBody::MANDATORY, 0.1f, NULL, "Ladder dismount");
  ...
  if (GetBot()->IsDebugging(NEXTBOT_LOCOMOTION)) {
    NDebugOverlay::EntityText(GetBot()->GetEntity()->entindex(), 0,
                              "Dismount top", 0.1f, 255, 255, 255, 255);
    NDebugOverlay::HorzArrow(GetFeet(), m_ladderDismountGoal->GetCenter(), 5.0f,
                             255, 255, 0, 255, true, 0.1f);
  }

  // test 2D vector here in case nav area is under the geometry a bit
  const float tolerance = 10.0f;
  if (GetBot()->GetEntity()->GetLastKnownArea() == m_ladderDismountGoal &&
      range < tolerance) {
...
void PlayerLocomotion::Update(void) {
  ...
  if (m_isJumpingAcrossGap || m_isClimbingUpToLedge) {
    ...
    if (m_hasLeftTheGround) {
      ...
      GetBot()->GetBodyInterface()->AimHeadTowards(
          GetBot()->GetEntity()->EyePosition() + 100.0 * toLanding,
      ...
      if (m_isJumpingAcrossGap) {
        // cheat and max our velocity in case we were stopped at the edge of
        // this gap
...
void PlayerLocomotion::Approach(const Vector &pos, float goalWeight) {
  ...
  INextBotPlayerInput *playerButtons =
      dynamic_cast<INextBotPlayerInput *>(GetBot());
  ...
  // If we're climbing ledges, we need to stay crouched to prevent player
  // movement code from messing with our origin.
  ...
  if (m_player->IsOnLadder() && IsUsingLadder() &&
    ...
    // Stay in center of ladder.  The gamemovement will autocenter us in most
    // cases, but this is needed in case it doesn't.
    if (m_ladderInfo) {
      Vector posOnLadder;
      CalcClosestPointOnLine(GetFeet(), m_ladderInfo->m_bottom,
                             m_ladderInfo->m_top, posOnLadder);
      ...
      const float tolerance =
          5.0f + 0.25f * GetBot()->GetBodyInterface()->GetHullWidth();
      if (error > tolerance) {
    ...
    if (ahead > epsilon) {
      ...
      if (GetBot()->IsDebugging(NEXTBOT_LOCOMOTION)) {
        NDebugOverlay::HorzArrow(m_player->GetAbsOrigin(),
                                 m_player->GetAbsOrigin() +
                                     50.0f * Vector(forward.x, forward.y, 0.0f),
    ...
    if (side <= -epsilon) {
      ...
      if (GetBot()->IsDebugging(NEXTBOT_LOCOMOTION)) {
        NDebugOverlay::HorzArrow(m_player->GetAbsOrigin(),
                                 m_player->GetAbsOrigin() -
                                     50.0f * Vector(right.x, right.y, 0.0f),
...
//----------------------------------------------------------------------------------------------------
bool PlayerLocomotion::IsClimbPossible(INextBot *me,
                                       const CBaseEntity *obstacle) const {
  // don't jump unless we have to
  const PathFollower *path = GetBot()->GetCurrentPath();
  if (path) {
    const float watchForClimbRange = 75.0f;
    if (!path->IsDiscontinuityAhead(GetBot(), Path::CLIMB_UP,
                                    watchForClimbRange)) {
...
//----------------------------------------------------------------------------------------------------
bool PlayerLocomotion::ClimbUpToLedge(const Vector &landingGoal,
...
//----------------------------------------------------------------------------------------------------
void PlayerLocomotion::JumpAcrossGap(const Vector &landingGoal,
                                     const Vector &landingForward) {
  Jump();

  // face forward
  GetBot()->GetBodyInterface()->AimHeadTowards(
      landingGoal, IBody::MANDATORY, 1.0f, NULL,
...
const Vector &PlayerLocomotion::GetGroundNormal(void) const {
  ...
  // TODO: Integrate movehelper_server for this:  return
  // m_player->GetGroundNormal();
}
 ...
 */
void PlayerLocomotion::ClimbLadder(const CNavLadder *ladder,
                                   const CNavArea *dismountGoal) {
  // look up and push forward
  // 	Vector goal =  GetBot()->GetPosition() + 100.0f * ( Vector( 0, 0, 1.0f )
  // - ladder->GetNormal() ); 	Approach( goal ); 	FaceTowards( goal );
 ...
 */
void PlayerLocomotion::DescendLadder(const CNavLadder *ladder,
                                     const CNavArea *dismountGoal) {
...
void PlayerLocomotion::FaceTowards(const Vector &target) {
  ...
  GetBot()->GetBodyInterface()->AimHeadTowards(look, IBody::BORING, 0.1f, NULL,
                                               "Body facing");
}
