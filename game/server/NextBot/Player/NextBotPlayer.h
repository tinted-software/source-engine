...
//--------------------------------------------------------------------------------------------------
/**
 * Instantiate a NextBot derived from CBasePlayer and spawn it into the
 * environment. Assumes class T is derived from CBasePlayer, and has the
 * following method that creates a new entity of type T and returns it:
 *
 * static CBasePlayer *T::AllocatePlayerEntity( edict_t *pEdict, const char
 * *playerName )
 *
 */
template <typename T>
T *NextBotCreatePlayerBot(const char *name, bool bReportFakeClient = true) {
  ...
  UTIL_ClientsInGame(), gpGlobals->maxClients ); return NULL;
  }
  */
  ...
  if (botEdict == NULL) {
    Msg("CreatePlayerBot: Unable to create bot %s - CreateFakeClient() "
        "returned NULL.\n",
        name);
    return NULL;
  }
 ...
 * Unless a duration is given, each button is released at the start of the next
 * frame. The release methods allow releasing a button before its duration has
 * elapsed.
 */
...
class NextBotPlayer : public PlayerType,
                      public INextBot,
                      public INextBotPlayerInput {
public:
  ...
  virtual void SetSpawnPoint(
      CBaseEntity
          *spawnPoint); // define place in environment where bot will (re)spawn
  ...
  // this is valid because the templatized PlayerType must be derived from
  // CBasePlayer, which is derived from CBaseCombatCharacter
  ...
  virtual bool IsDormantWhenDead(void) const {
    return true;
  } // should this player-bot continue to update itself when dead (respawn
    // logic, etc)

  // allocate a bot and bind it to the edict
  static CBasePlayer *AllocatePlayerEntity(edict_t *edict,
                                           const char *playerName);

  //------------------------------------------------------------------------
  // utility methods
  float GetDistanceBetween(CBaseEntity *other)
      const; // return distance between us and the given entity
  bool IsDistanceBetweenLessThan(CBaseEntity *other, float range)
      const; // return true if distance between is less than the given value
  bool IsDistanceBetweenGreaterThan(CBaseEntity *other, float range)
      const; // return true if distance between is greater than the given value

  float GetDistanceBetween(const Vector &target)
      const; // return distance between us and the given entity
  bool IsDistanceBetweenLessThan(const Vector &target, float range)
      const; // return true if distance between is less than the given value
  bool IsDistanceBetweenGreaterThan(const Vector &target, float range)
      const; // return true if distance between is greater than the given value
  ...
  virtual void OnNavAreaChanged(
      ...
      CNavArea *leftArea); // invoked (by UpdateLastKnownArea) when we enter a
                           // new nav area (or it is reset to NULL)
  virtual void Touch(CBaseEntity *other);
  virtual void Weapon_Equip(CBaseCombatWeapon *weapon); // for OnPickUp
  virtual void Weapon_Drop(CBaseCombatWeapon *weapon, const Vector *target,
                           const Vector *velocity); // for OnDrop
  virtual void OnMainActivityComplete(Activity newActivity,
                                      Activity oldActivity);
  virtual void OnMainActivityInterrupted(Activity newActivity,
                                         Activity oldActivity);
  //------------------------------------------------------------------------
  ...
  } // some game types allow players to pass through each other, this method
    // pushes them apart
...
protected:
  int m_inputButtons; // this is still needed to guarantee each button press is
...
template <typename PlayerType>
inline float
NextBotPlayer<PlayerType>::GetDistanceBetween(CBaseEntity *other) const {
  return (this->GetAbsOrigin() - other->GetAbsOrigin()).Length();
}

template <typename PlayerType>
inline bool
NextBotPlayer<PlayerType>::IsDistanceBetweenLessThan(CBaseEntity *other,
                                                     float range) const {
...
template <typename PlayerType>
inline bool
NextBotPlayer<PlayerType>::IsDistanceBetweenGreaterThan(CBaseEntity *other,
                                                        float range) const {
  return (this->GetAbsOrigin() - other->GetAbsOrigin())
      .IsLengthGreaterThan(range);
}

template <typename PlayerType>
inline float
NextBotPlayer<PlayerType>::GetDistanceBetween(const Vector &target) const {
  return (this->GetAbsOrigin() - target).Length();
}

template <typename PlayerType>
inline bool
NextBotPlayer<PlayerType>::IsDistanceBetweenLessThan(const Vector &target,
                                                     float range) const {
...
template <typename PlayerType>
inline bool
NextBotPlayer<PlayerType>::IsDistanceBetweenGreaterThan(const Vector &target,
                                                        float range) const {
...
template <typename PlayerType>
inline void NextBotPlayer<PlayerType>::SetButtonScale(float forward,
                                                      float right) {
...
//-----------------------------------------------------------------------------------------------------
inline void _NextBot_BuildUserCommand(CUserCmd *cmd, const QAngle &viewangles,
...
inline void NextBotPlayer<PlayerType>::PhysicsSimulate(void) {
  ...
  if ((IsDormantWhenDead() && PlayerType::m_lifeState == LIFE_DEAD) ||
      NextBotStop.GetBool()) {
    // death animation complete - nothing left to do except let PhysicsSimulate
    // run PreThink etc
    PlayerType::PhysicsSimulate();
    return;
  }
  ...
  float verticalSpeed =
      (m_inputButtons & IN_JUMP) ? mover->GetRunSpeed() : 0.0f;
  ...
  // "Look" in the direction we're climbing/stumbling etc.  We can't do anything
  // anyway, and it keeps motion extraction working.
  ...
  CUserCmd userCmd;
  _NextBot_BuildUserCommand(&userCmd, angles, forwardSpeed, strafeSpeed,
                            verticalSpeed, inputButtons, 0);

  AvoidPlayers(&userCmd);

  // allocate a new command and add it to the player's list of command to
  // process
...
//----------------------------------------------------------------------------------------------------------
template <typename PlayerType>
inline void NextBotPlayer<PlayerType>::OnNavAreaChanged(CNavArea *enteredArea,
                                                        CNavArea *leftArea) {
...
//----------------------------------------------------------------------------------------------------------
template <typename PlayerType>
inline void NextBotPlayer<PlayerType>::Weapon_Drop(CBaseCombatWeapon *weapon,
                                                   const Vector *target,
...
template <typename PlayerType>
inline void
NextBotPlayer<PlayerType>::OnMainActivityComplete(Activity newActivity,
                                                  Activity oldActivity) {
#ifdef TERROR
...
template <typename PlayerType>
inline void
NextBotPlayer<PlayerType>::OnMainActivityInterrupted(Activity newActivity,
                                                     Activity oldActivity) {
#ifdef TERROR
...
inline void NextBotPlayer<PlayerType>::Update(void) {
  ...
  if ((this->IsAlive() || !IsDormantWhenDead()) &&
...
template <typename PlayerType>
inline int
NextBotPlayer<PlayerType>::OnTakeDamage_Alive(const CTakeDamageInfo &info) {
...
template <typename PlayerType>
inline int
NextBotPlayer<PlayerType>::OnTakeDamage_Dying(const CTakeDamageInfo &info) {
  if (info.GetDamageType() & DMG_BURN) {
    if (!m_burningTimer.HasStarted() || m_burningTimer.IsGreaterThen(1.0f)) {
      // emit ignite event periodically as long as we are burning
      OnIgnite();
...
template <typename PlayerType>
inline void
NextBotPlayer<PlayerType>::Event_Killed(const CTakeDamageInfo &info) {
