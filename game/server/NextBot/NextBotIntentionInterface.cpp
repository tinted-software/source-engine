 ...
 */
Vector
IIntention::SelectTargetPoint(const INextBot *me,
 ...
 */
const CKnownEntity *IIntention::SelectMoreDangerousThreat(
  ...
  for (INextBotEventResponder *sub = FirstContainedResponder(); sub;
    ...
    if (query) {
      // return the response of the first responder that gives a definitive
      // answer
      const CKnownEntity *result =
          query->SelectMoreDangerousThreat(me, subject, threat1, threat2);
      if (result) {
        return result;
      }
    }
  ...
  float range1 =
      (subject->GetAbsOrigin() - threat1->GetLastKnownPosition()).LengthSqr();
  float range2 =
      (subject->GetAbsOrigin() - threat2->GetLastKnownPosition()).LengthSqr();
