...
//----------------------------------------------------------------------------------------------
/**
 * A ChasePath extends a PathFollower to periodically recompute a path to a
 * chase subject, and to move along the path towards that subject.
 */
class ChasePath : public PathFollower {
public:
  enum SubjectChaseType { LEAD_SUBJECT, DONT_LEAD_SUBJECT };
  ChasePath(SubjectChaseType chaseHow = DONT_LEAD_SUBJECT);

  virtual ~ChasePath() {}

  virtual void
  Update(INextBot *bot, CBaseEntity *subject, const IPathCost &cost,
         Vector *pPredictedSubjectPos =
             NULL); // update path to chase target and move bot along path

  virtual float
  GetLeadRadius(void) const; // range where movement leading begins - beyond
                             // this just head right for the subject
  ...
  virtual Vector PredictSubjectPosition(INextBot *bot, CBaseEntity *subject)
      const; // try to cutoff our chase subject, knowing our relative positions
             // and velocities
  virtual bool IsRepathNeeded(INextBot *bot, CBaseEntity *subject)
      const; // return true if situation has changed enough to warrant
             // recomputing the current path

  virtual float
  GetLifetime(void) const; // Return duration this path is valid. Path will
                           // become invalid at its earliest opportunity once
                           // this duration elapses. Zero = infinite lifetime

  virtual void Invalidate(void); // (EXTEND) cause the path to become invalid

private:
  void RefreshPath(INextBot *bot, CBaseEntity *subject, const IPathCost &cost,
                   Vector *pPredictedSubjectPos);
  ...
  EHANDLE
      m_lastPathSubject; // the subject used to compute the current/last path
 ...
 */
inline void ChasePath::Update(INextBot *bot, CBaseEntity *subject,
                              const IPathCost &cost,
                              Vector *pPredictedSubjectPos) {
...
//----------------------------------------------------------------------------------------------
/**
 * Return true if situation has changed enough to warrant recomputing the
 * current path
 */
inline bool ChasePath::IsRepathNeeded(INextBot *bot,
                                      CBaseEntity *subject) const {
  ...
  return (subject->GetAbsOrigin() - GetEndPosition())
      .IsLengthGreaterThan(tolerance);
}
 ...
 */
inline void ChasePath::RefreshPath(INextBot *bot, CBaseEntity *subject,
                                   const IPathCost &cost,
                                   Vector *pPredictedSubjectPos) {
  ...
  if (IsValid() && mover->IsUsingLadder()) {
    if (bot->IsDebugging(NEXTBOT_PATH)) {
      DevMsg("%3.2f: bot(#%d) ChasePath::RefreshPath failed. Bot is on a "
             "ladder.\n",
  ...
  if (!m_failTimer.IsElapsed()) {
    // 		if ( bot->IsDebugging( NEXTBOT_PATH ) )
    // 		{
    // 			DevMsg( "%3.2f: bot(#%d) ChasePath::RefreshPath failed.
    // Fail timer not elapsed.\n", gpGlobals->curtime,
  ...
  if (IsValid() && !m_throttleTimer.IsElapsed()) {
    // require a minimum time between repaths, as long as we have a path to
    // follow
    // 		if ( bot->IsDebugging( NEXTBOT_PATH ) )
    // 		{
    // 			DevMsg( "%3.2f: bot(#%d) ChasePath::RefreshPath failed.
  ...
  if (IsValid() && m_lifetimeTimer.HasStarted() &&
      m_lifetimeTimer.IsElapsed()) {
  ...
  if (!IsValid() || IsRepathNeeded(bot, subject)) {
    ...
    if (m_chaseHow == LEAD_SUBJECT) {
      pathTarget = pPredictedSubjectPos ? *pPredictedSubjectPos
                                        : PredictSubjectPosition(bot, subject);
      isPath = Compute(bot, pathTarget, cost, GetMaxPathLength());
    } else if (subject->MyCombatCharacterPointer() &&
               subject->MyCombatCharacterPointer()->GetLastKnownArea()) {
      isPath = Compute(bot, subject->MyCombatCharacterPointer(), cost,
                       GetMaxPathLength());
    } else {
      isPath = Compute(bot, pathTarget, cost, GetMaxPathLength());
    }
    ...
    if (isPath) {
      if (bot->IsDebugging(NEXTBOT_PATH)) {
        ...
        // NDebugOverlay::VertArrow( bot->GetPosition() + Vector( 0, 0, size ),
        // bot->GetPosition(), size, 255, RandomInt( 0, 200 ), 255, 255,
        // true, 30.0f );
      ...
      if (bot->IsDebugging(NEXTBOT_PATH)) {
        ...
        // NDebugOverlay::VertArrow( bot->GetPosition() + Vector( 0, 0, size ),
        // bot->GetPosition(), size, 255, c, c, 255, true, dT );
        NDebugOverlay::HorzArrow(bot->GetPosition(), pathTarget, 5.0f, 255, c,
                                 c, 255, true, dT);
...
public:
  DirectChasePath(
      ChasePath::SubjectChaseType chaseHow = ChasePath::DONT_LEAD_SUBJECT)
      : ChasePath(chaseHow) {}

  //-------------------------------------------------------------------------------------------------------
  virtual void
  Update(INextBot *me, CBaseEntity *victim, const IPathCost &pathCost,
         Vector *pPredictedSubjectPos =
             NULL) // update path to chase target and move bot along path
  {
    ...
    if (!DirectChase(&bComputedPredictedPosition, &vecPredictedPosition, me,
                     victim)) {
      // path around obstacles to reach our victim
      ChasePath::Update(me, victim, pathCost,
                        bComputedPredictedPosition ? &vecPredictedPosition
                                                   : NULL);
    }
  ...
  //-------------------------------------------------------------------------------------------------------
  bool DirectChase(bool *pPredictedPositionComputed, Vector *pPredictedPos,
                   INextBot *me,
                   CBaseEntity *victim) // if there is nothing between us and
                                        // our victim, run directly at them
  {
            ...
            // recomputing the current path
  {
    ...
    return bot->GetLocomotionInterface()->IsStuck() &&
           bot->GetLocomotionInterface()->GetStuckDuration() > 2.0f;
  }
