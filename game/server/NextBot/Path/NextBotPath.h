...
#define PATH_NO_LENGTH_LIMIT                                                   \
  0.0f // non-default argument value for Path::Compute()
#define PATH_TRUNCATE_INCOMPLETE_PATH                                          \
  false // non-default argument value for Path::Compute()
 ...
 * TODO: Replace all template cost functors with this interface, so we can
 * virtualize and derive from them.
 */
class IPathCost {
public:
...
public:
  // compare "newArea" to "currentGoal" and return the area that is the better
  // goal area
  virtual CNavArea *operator()(CNavArea *currentGoal,
                               CNavArea *newArea) const = 0;
};
...
public:
  ...
  struct Segment {
    ...
    float curvature; // how much the path 'curves' at this point in the XY plane
                     // (0 = none, 1 = 180 degree doubleback)

    Vector m_portalCenter;   // position of center of 'portal' between previous
                             // area and this area
    float m_portalHalfWidth; // half width of 'portal'
  };

  virtual float
  GetLength(void) const; // return length of path from start to finish
  virtual const Vector &GetPosition(float distanceFromStart,
                                    const Segment *start = NULL)
      const; // return a position on the path at the given distance from the
             // path start
  virtual const Vector &GetClosestPosition(const Vector &pos,
                                           const Segment *start = NULL,
                                           float alongLimit = 0.0f)
      const; // return the closest point on the path to the given position

  virtual const Vector &
  GetStartPosition(void) const; // return the position where this path starts
  virtual const Vector &
  GetEndPosition(void) const; // return the position where this path ends
  virtual CBaseCombatCharacter *
  GetSubject(void) const; // return the actor this path leads to, or NULL if
                          // there is no subject
  ...
  virtual float
  GetAge(void) const; // return "age" of this path (time since it was built)

  enum SeekType {
    SEEK_ENTIRE_PATH, // search the entire path length
    SEEK_AHEAD, // search from current cursor position forward toward end of
                // path
    SEEK_BEHIND // search from current cursor position backward toward path
                // start
  };
  virtual void MoveCursorToClosestPosition(const Vector &pos,
                                           SeekType type = SEEK_ENTIRE_PATH,
                                           float alongLimit = 0.0f)
      const; // Set cursor position to closest point on path to given position

  enum MoveCursorType { PATH_ABSOLUTE_DISTANCE, PATH_RELATIVE_DISTANCE };
  ...
  virtual void
  MoveCursor(float value,
             MoveCursorType type =
                 PATH_ABSOLUTE_DISTANCE); // change seek cursor position
  virtual float GetCursorPosition(
      void) const; // return position of seek cursor (distance along path)
  ...
  virtual const Data &
  GetCursorData(void) const; // return path state at the current cursor position

  virtual bool IsValid(void) const;
  virtual void Invalidate(void); // make path invalid (clear it)

  virtual void
  Draw(const Path::Segment *start = NULL) const; // draw the path for debugging
  virtual void DrawInterpolated(
      float from,
      float to); // draw the path for debugging - MODIFIES cursor position

  virtual const Segment *
  FirstSegment(void) const; // return first segment of path
  virtual const Segment *NextSegment(const Segment *currentSegment)
      const; // return next segment of path, given current one
  virtual const Segment *PriorSegment(const Segment *currentSegment)
      const; // return previous segment of path, given current one
  virtual const Segment *LastSegment(void) const; // return last segment of path

  enum ResultType { COMPLETE_PATH, PARTIAL_PATH, NO_PATH };
  ...
  virtual void
  Copy(INextBot *bot,
       const Path &path); // Replace this path with the given path's data

  //-----------------------------------------------------------------------------------------------------------------
  /**
   * Compute shortest path from bot to given actor via A* algorithm.
   * If returns true, path was found to the subject.
   * If returns false, path may either be invalid (use IsValid() to check), or
   * valid but doesn't reach all the way to the subject.
   */
  template <typename CostFunctor>
  bool Compute(INextBot *bot, CBaseCombatCharacter *subject,
    ...
    bool pathResult = NavAreaBuildPath(startArea, subjectArea, &subjectPos,
                                       costFunc, &closestArea, maxPathLength,
    ...
    for (area = closestArea; area; area = area->GetParent()) {
      ...
      if (area == startArea) {
        // startArea can be re-evaluated during the pathfind and given a
        // parent...
        break;
      }
   ...
   * If returns false, path may either be invalid (use IsValid() to check), or
   * valid but doesn't reach all the way to the goal.
   */
  template <typename CostFunctor>
  bool Compute(INextBot *bot, const Vector &goal, CostFunctor &costFunc,
               float maxPathLength = 0.0f, bool includeGoalIfPathFails = true) {
    ...
    CNavArea *goalArea =
        TheNavMesh->GetNearestNavArea(goal, true, maxDistanceToArea, true);
    ...
    CNavArea *closestArea = NULL;
    bool pathResult =
        NavAreaBuildPath(startArea, goalArea, &goal, costFunc, &closestArea,
                         maxPathLength, bot->GetEntity()->GetTeamNumber());
  ...
  virtual bool ComputeWithOpenGoal(INextBot *bot, const IPathCost &costFunc,
                                   const IPathOpenGoalSelector &goalSelector,
    ...
    while (!CNavArea::IsOpenListEmpty()) {
      ...
      for (int i = 0; i < m_adjAreaIndex; ++i) {
        ...
        if (maxSearchRadius > 0.0f &&
            (newArea->GetCenter() - bot->GetEntity()->GetAbsOrigin())
                .IsLengthGreaterThan(maxSearchRadius))
          continue;

        // determine cost of traversing this area
        float newCost =
            costFunc(newArea, area, m_adjAreaVector[i].ladder, NULL, -1.0f);
        ...
        if (newArea->IsOpen() && newArea->GetTotalCost() <= newCost) {
          ...
          // whether this area has been visited or not, we now have a better
          // path to it
          ...
          if (newArea->IsOpen()) {
            // area already on open list, update the list order to keep costs
            // sorted
   ...
   */
  void AssemblePrecomputedPath(INextBot *bot, const Vector &goal,
                               CNavArea *endArea) {
   ...
   */
  virtual void ComputeAreaCrossing(INextBot *bot, const CNavArea *from,
                                   const Vector &fromPos, const CNavArea *to,
...
private:
  ...
  bool
  ComputePathDetails(INextBot *bot,
                     const Vector &start); // determine actual path positions

  void Optimize(INextBot *bot);
  void PostProcess(void);
  int FindNextOccludedNode(INextBot *bot, int anchor); // used by Optimize()

  void InsertSegment(Segment newSegment,
                     int i); // insert new segment at index i
  ...
  void CollectAdjacentAreas(CNavArea *area) {
    ...
    const NavLadderConnectVector &adjUpLadder =
    ...
    const NavLadderConnectVector &adjDownLadder =
        *area->GetLadders(CNavLadder::LADDER_DOWN);
...
inline const Path::Segment *
Path::NextSegment(const Segment *currentSegment) const {
...
inline const Path::Segment *
Path::PriorSegment(const Segment *currentSegment) const {
  if (currentSegment == NULL || !IsValid())
    return NULL;
