...
ConVar NextBotPathDrawSegmentCount("nb_path_draw_segment_count", "100",
                                   FCVAR_CHEAT);
ConVar NextBotPathSegmentInfluenceRadius("nb_path_segment_influence_radius",
                                         "100", FCVAR_CHEAT);
...
bool Path::ComputePathDetails(INextBot *bot, const Vector &start) {
  ...
  for (int i = 1; i < m_segmentCount; ++i) {
    ...
    if (to->how <= GO_WEST) // walk along the floor to the next area
    {
      ...
      from->area->ComputePortal(to->area, (NavDirType)to->how,
                                &to->m_portalCenter, &to->m_portalHalfWidth);

      // compute next point
      ComputeAreaCrossing(bot, from->area, from->pos, to->area,
                          (NavDirType)to->how, &to->pos);
      ...
      // on the path
      // float expectedHeightDrop = from->area->GetZ( from->pos ) -
      // to->area->GetZ( to->pos );
      ...
      if (expectedHeightDrop > mover->GetStepHeight()) {
        // NOTE: We can't know this is a drop-down yet, because of subtle
        // interactions between nav area links and "portals" and "area
        // crossings"
        ...
        for (pushDist = 0.0f; pushDist <= maxPushDist; pushDist += inc) {
          Vector pos =
              to->pos + Vector(pushDist * dir.x, pushDist * dir.y, 0.0f);
          Vector lowerPos = Vector(pos.x, pos.y, toPos.z);

          trace_t result;
          NextBotTraceFilterIgnoreActors filter(bot->GetEntity(),
                                                COLLISION_GROUP_NONE);
          UTIL_TraceHull(
              pos, lowerPos, Vector(-halfWidth, -halfWidth, stepHeight),
        ...
        Vector startDrop(to->pos.x + pushDist * dir.x,
                         to->pos.y + pushDist * dir.y, to->pos.z);
        ...
        if (bot->IsDebugging(NEXTBOT_PATH)) {
          ...
          NDebugOverlay::VertArrow(startDrop, endDrop, 5.0f, 255, 100, 0, 255,
                                   true, 5.0f);
        }

        // verify that there is actually ground down there in case this is a far
        // jump dropdown
        float ground;
               ...
               GO_LADDER_UP) // to get to next area, must go up a ladder
    {
      ...
      const NavLadderConnectVector *ladders =
          from->area->GetLadders(CNavLadder::LADDER_UP);
      int it;
               ...
               GO_LADDER_DOWN) // to get to next area, must go down a ladder
    {
      // find our ladder
      const NavLadderConnectVector *ladders =
          from->area->GetLadders(CNavLadder::LADDER_DOWN);
      int it;
      for (it = 0; it < ladders->Count(); ++it) {
        CNavLadder *ladder = (*ladders)[it].ladder;
  ...
  for (int i = 0; i < m_segmentCount - 1; ++i) {
    ...
    if ((closeFrom - closeTo)
            .AsVector2D()
            .IsLengthGreaterThan(separationTolerance) &&
        (closeTo - closeFrom)
            .AsVector2D()
            .IsLengthGreaterThan(0.5f * fabs(closeTo.z - closeFrom.z))) {
      ...
      // use center of climb-up destination area to make sure bot moves onto
      // actual ground once they finish their climb
      to->pos = to->area->GetCenter();
      ...
      if (bot->IsDebugging(NEXTBOT_PATH)) {
        NDebugOverlay::Cross3D(newSegment.pos, 15.0f, 255, 100, 255, true,
                               3.0f);
      }
    ...
    /** RETHINK THIS.  It doesn't work in general cases, and messes up on
    doorways else if ( from->type == ON_GROUND && from->how <= GO_WEST )
    {
    ...
    )
            {
                    ...
                    {
                            NDebugOverlay::HorzArrow( from->pos, to->pos, 3.0f,
    255, 0, 0, 255, true, 3.0f );
                    }

                    //newSegment.pos = from->area->GetCenter();

                    Vector2D shift;
                    DirectionToVector2D( OppositeDirection( (NavDirType)to->how
    ), &shift );
                    ...
                    if ( bot->IsDebugging( INextBot::PATH ) )
                    {
                            NDebugOverlay::Cross3D( newSegment.pos, 15.0f, 255,
    0, 255, true, 3.0f );
                    }
...
void Path::Draw(const Path::Segment *start) const {
  ...
  while (s && count--) {
    ...
    if (s->ladder) {
      NDebugOverlay::VertArrow(s->ladder->m_bottom, s->ladder->m_top, 5.0f, r,
                               g, b, 255, true, 0.1f);
    } else {
    ...
    if (horiz > vert) {
      NDebugOverlay::HorzArrow(s->pos, s->pos + nodeLength * s->forward, 5.0f,
                               r, g, b, 255, true, 0.1f);
    } else {
      NDebugOverlay::VertArrow(s->pos, s->pos + nodeLength * s->forward, 5.0f,
                               r, g, b, 255, true, 0.1f);
    }

    NDebugOverlay::Text(s->pos, msg.sprintf("%d", i), true, 0.1f);

    // NDebugOverlay::Text( s->pos, msg.sprintf( "%d  (%3.2f)", i, s->curvature
    // ), false, 0.1f );
...
void Path::DrawInterpolated(float from, float to) {
  ...
  do {
    ...
    NDebugOverlay::Line( data.pos, data.pos + 10.0f * data.forward, 0, i, i,
    true, 0.1f );
    */
...
int Path::FindNextOccludedNode(INextBot *bot, int anchorIndex) {
  ...
  for (int i = anchorIndex + 1; i < m_segmentCount; ++i) {
    ...
    if (!to->type == ON_GROUND ||
        (to->area->GetAttributes() & NAV_MESH_PRECISE)) {
      return i;
    }

    if (!mover->IsPotentiallyTraversable(anchor->pos, to->pos,
 ...
 */
const Vector &Path::GetPosition(float distanceFromStart,
                                const Segment *start) const {
  if (!IsValid()) {
    return vec3_origin;
  }
 ...
 */
const Vector &Path::GetClosestPosition(const Vector &pos, const Segment *start,
                                       float alongLimit) const {
 ...
 */
void Path::MoveCursorToClosestPosition(const Vector &pos, SeekType type,
                                       float alongLimit) const {
  if (!IsValid()) {
    return;
  }
  ...
  if (type == SEEK_ENTIRE_PATH || type == SEEK_AHEAD) {
    ...
    while (alongLimit == 0.0f || distanceSoFar <= alongLimit) {
      ...
      if (nextSegment) {
        Vector close;
        CalcClosestPointOnLineSegment(pos, segment->pos, nextSegment->pos,
                                      close);
      ...
      if (m_cursorPos < epsilon || m_segmentCount < 2) {
        ...
        while (next) {
          ...
          if (lengthSoFar + length >= m_cursorPos) {
            ...
            float t = 1.0f; // 0-length segments are assumed to be complete, to
                            // avoid NaNs
            if (length > 0.0f) {
              t = overlap / length;
            }
            ...
            m_cursorData.forward =
                segment->forward + t * (next->forward - segment->forward);
            m_cursorData.segmentPrior = segment;
            ...
            if (overlap < NextBotPathSegmentInfluenceRadius.GetFloat()) {
              if (length - overlap <
                  NextBotPathSegmentInfluenceRadius.GetFloat()) {
                // near start and end - interpolate
                float startCurvature =
                    segment->curvature *
                    (1.0f -
                     (overlap / NextBotPathSegmentInfluenceRadius.GetFloat()));
                float endCurvature =
                    next->curvature *
                    (1.0f - ((length - overlap) /
                             NextBotPathSegmentInfluenceRadius.GetFloat()));
 ...
 */
void Path::ComputeAreaCrossing(INextBot *bot, const CNavArea *from,
  ...
  // move goal position into the goal area a bit to avoid running directly along
  // the edge of an area against a wall, etc don't do this unless area is
  // against a wall - and what if our hull is wider than the area?
  // AddDirectionVector( crossPos, dir,
  // bot->GetBodyInterface()->GetHullWidth()/2.0f );
}
