...
public:
  ...
  void Update(INextBot *bot,
              CBaseEntity *threat); // update path away from threat and move bot
                                    // along path
...
public:
  RetreatPathBuilder(INextBot *me, CBaseEntity *threat,
  ...
  CNavArea *ComputePath(void) {
    ...
    CNavArea *retreatFromArea =
        TheNavMesh->GetNearestNavArea(m_threat->GetAbsOrigin());
    ...
    while (!CNavArea::IsOpenListEmpty()) {
      ...
      for (int i = 0; i < m_adjAreaIndex; ++i) {
        ...
        if ((newArea->GetCenter() - m_me->GetEntity()->GetAbsOrigin())
                .IsLengthGreaterThan(m_retreatRange))
          continue;
        ...
        if (newArea->IsOpen() && newArea->GetTotalCost() <= newCost) {
          ...
          float threatRange =
              (newArea->GetCenter() - m_threat->GetAbsOrigin()).Length();
  ...
  /**
   * Cost minimizes path length traveled thus far and "danger" (proximity to
   * threat(s))
   */
  float Cost(CNavArea *area, CNavArea *fromArea, const CNavLadder *ladder) {
    ...
    if (fromArea == NULL) {
      ...
      if (area->Contains(m_threat->GetAbsOrigin())) {
        ...
        float rangeToThreat =
            (m_threat->GetAbsOrigin() - m_me->GetEntity()->GetAbsOrigin())
                .Length();
        ...
        if (rangeToThreat < maxThreatRange) {
          ...
          if (m_me->IsDebugging(INextBot::PATH)) {
            NDebugOverlay::Line(m_me->GetEntity()->GetAbsOrigin(),
                                m_threat->GetAbsOrigin(), 255, 0, 0, true,
                                debugDeltaT);
          }
        }
      }
      ...
      Vector close;
      float t;
      CalcClosestPointOnLineSegment(m_threat->GetAbsOrigin(), area->GetCenter(),
                                    fromArea->GetCenter(), close, &t);
      if (t < 0.0f) {
      ...
      if (rangeToThreat < maxThreatRange) {
        ...
        if (m_me->IsDebugging(INextBot::PATH)) {
          NDebugOverlay::HorzArrow(fromArea->GetCenter(), area->GetCenter(), 5,
                                   255 * dangerFactor, 0, 0, 255, true,
                                   debugDeltaT);

          Vector to = close - m_threat->GetAbsOrigin();
          to.NormalizeInPlace();

          NDebugOverlay::Line(close, close - 50.0f * to, 255, 0, 0, true,
                              debugDeltaT);
        }
      }
...
inline void RetreatPath::RefreshPath(INextBot *bot, CBaseEntity *threat) {
  ...
  if (IsValid() && mover && mover->IsUsingLadder()) {
    if (bot->IsDebugging(INextBot::PATH)) {
      DevMsg("%3.2f: bot(#%d) RetreatPath::RefreshPath failed. Bot is on a "
             "ladder.\n",
             gpGlobals->curtime, bot->GetEntity()->entindex());
    }
    return;
  ...
  if (!IsValid() || (threat->GetAbsOrigin() - m_pathThreatPos)
    ...
    if (!m_throttleTimer.IsElapsed()) {
      ...
      // follow
      if (bot->IsDebugging(INextBot::PATH)) {
        DevMsg("%3.2f: bot(#%d) RetreatPath::RefreshPath failed. Rate "
               "throttled.\n",
               gpGlobals->curtime, bot->GetEntity()->entindex());
      }
      return;
    ...
    if (goalArea) {
      ...
      // all adjacent areas are too far away - just move directly away from
      // threat
      Vector to = threat->GetAbsOrigin() - bot->GetPosition();
