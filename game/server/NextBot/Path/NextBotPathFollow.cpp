...
ConVar NextBotSpeedLookAheadRange("nb_speed_look_ahead_range", "150",
                                  FCVAR_CHEAT);
...
class CDetachPath {
public:
  CDetachPath(PathFollower *path) { m_path = path; }
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Invoked when the path is (re)computed (path is valid at the time of this
 * call)
 */
...
void PathFollower::AdjustSpeed(INextBot *bot) {
  ...
  mover->SetDesiredSpeed(mover->GetRunSpeed() +
                         fabs(data.curvature) *
                             (mover->GetWalkSpeed() - mover->GetRunSpeed()));
}
...
bool PathFollower::IsAtGoal(INextBot *bot) const {
  ...
  if (current == NULL) {
    ...
    // m_goal is the top of the drop-down, and the following segment is the
    // landing point
    ...
    /// @todo: it is possible to fall into a bad place and get stuck - should
    /// move back onto the path
    ...
    if (landing == NULL) {
      // passed goal or corrupt path
      return true;
    } else if (/*!mover->IsOnGround() && */ mover->GetFeet().z >
               m_goal->pos.z + mover->GetStepHeight()) {
      // we're off the ground, presumably climbing - assume we reached the goal
      return true;
    }
    /* This breaks infected climbing up holes in the ceiling - they can get
    within 2D range of m_goal before finding a ledge to climb up to else if (
    mover->IsOnGround() )
    {
            // proximity check
            // Z delta can be anything, since we may be climbing over a tall
    fence, a physics prop, etc. const float rangeTolerance = 10.0f; if (
    toGoal.AsVector2D().IsLengthLessThan( rangeTolerance ) )
            {
                    // reached goal
                    return true;
            }
    ...
    */
    ...
    if (next) {
      // because mover may be off the path, check if it crossed the plane of the
      // goal check against average of current and next forward vectors
      ...
      if (current->ladder) {
        ...
        dividingPlane =
            current->forward.AsVector2D() + m_goal->forward.AsVector2D();
      }
    ...
    // proximity check
    // Z delta can be anything, since we may be climbing over a tall fence, a
    // physics prop, etc.
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Move bot along ladder. Return true if ladder motion is in progress, false if
 * complete.
 */
bool PathFollower::LadderUpdate(INextBot *bot) {
  ...
  if (m_goal->ladder == NULL) {
    // Check if we have somehow ended up on a ladder, if so, and its a tall
    // down-ladder we are expecting, jump the path ahead. This happens for
    // players, who run off ledges and the gamemovement sticks them onto
    // ladders.  We only care about tall down-ladders, because up ladders work
    // without this, and short ladders aren't dangerous to miss and drop down
    ...
    if (bot->GetEntity()->GetMoveType() == MOVETYPE_LADDER) {
      ...
      // Start with current, the segment we are currently traversing.  Skip the
      // distance check for that segment, because the pos is (hopefully) behind
      // us.  And if it's a long path segment, it's already outside the
      // climbLookAheadRange, and thus it would prevent us looking at m_goal and
      // further for imminent planned climbs. 'current' is the segment we are
      // on/just passed over
      const float ladderLookAheadRange = 50.0f;
      for (const Segment *s = current; s; s = NextSegment(s)) {
        if (s != current && (s->pos - mover->GetFeet())
                                .AsVector2D()
                                .IsLengthGreaterThan(ladderLookAheadRange)) {
          break;
        }

        // Only consider reasonably tall down ladders - if we don't grab onto a
        // short ladder, it hopefully won't be a bad fall.
        if (s->ladder != NULL && s->how == GO_LADDER_DOWN &&
            s->ladder->m_length > mover->GetMaxJumpHeight()) {
  ...
  if (m_goal->how == GO_LADDER_UP) {
    ...
    if (!mover->IsUsingLadder() &&
        mover->GetFeet().z > m_goal->ladder->m_top.z - mover->GetStepHeight()) {
      // we're up
    ...
    body->AimHeadTowards(m_goal->ladder->m_top -
                             50.0f * m_goal->ladder->GetNormal() +
    ...
    if (range < NextBotLadderAlignRange.GetFloat()) {
      ...
      // approach the base of the ladder - use normal path following in case
      // there are jumps/climbs on the way to the ladder
      return false;
    }
  } else // go down ladder
  {
    ...
    if (mover->GetFeet().z <
        m_goal->ladder->m_bottom.z + mover->GetStepHeight()) {
      // we fell
      m_goal = NextSegment(m_goal);
    } else {
      // approach the ladder
      Vector mountPoint =
          m_goal->ladder->m_top +
          0.5f * body->GetHullWidth() * m_goal->ladder->GetNormal();
      ...
      body->AimHeadTowards(
          m_goal->ladder->m_bottom + 50.0f * m_goal->ladder->GetNormal() +
              Vector(0, 0, body->GetCrouchHullHeight()),
          IBody::CRITICAL, 1.0f, NULL, "Mounting downward ladder");

      float range = to.NormalizeInPlace();

      // Approach the top of the ladder.  If we're already on the ladder, start
      // descending.
      if (range < mountRange ||
          bot->GetEntity()->GetMoveType() == MOVETYPE_LADDER) {
        // go down ladder
        ...
        // approach the top of the ladder - use normal path following in case
        // there are jumps/climbs on the way to the ladder
        return false;
      }
    }
...
bool PathFollower::CheckProgress(INextBot *bot) {
  ...
  if (m_minLookAheadRange > 0.0f) {
    ...
    while (pSkipToGoal && pSkipToGoal->type == ON_GROUND &&
           mover->IsOnGround()) {
      if ((pSkipToGoal->pos - myFeet).IsLengthLessThan(m_minLookAheadRange)) {
        ...
        if (nextSegment->pos.z > myFeet.z + mover->GetStepHeight()) {
          // going uphill or up stairs tends to cause problems if we skip ahead,
          // so don't
          break;
        }

#ifdef DOTA_DLL
        if (DotProduct(mover->GetMotionVector(), nextSegment->forward) <=
            0.1f) {
        ...
        if (mover->IsPotentiallyTraversable(myFeet, nextSegment->pos) &&
          ...
          // can't directly reach next segment - keep heading towards current
          // goal
          break;
        }
      } else {
  ...
  if (IsAtGoal(bot)) {
    ...
    const Path::Segment *nextSegment =
        pSkipToGoal ? pSkipToGoal : NextSegment(m_goal);
    ...
    if (nextSegment == NULL) {
      ...
      if (bot->IsDebugging(NEXTBOT_PATH) &&
          !mover->IsPotentiallyTraversable(mover->GetFeet(),
                                           nextSegment->pos)) {
...
void PathFollower::Update(INextBot *bot) {
  ...
  if (left.IsZero()) {
    ...
    if (bot->IsDebugging(NEXTBOT_PATH)) {
      DevMsg("PathFollower: OnMoveToFailure( FAIL_STUCK ) because forward and "
             "left are ZERO\n");
    }
  ...
  if (bot->IsDebugging(NEXTBOT_PATH)) {
    ...
    NDebugOverlay::Line(mover->GetFeet(), mover->GetFeet() + axisSize * forward,
                        255, 0, 0, true, 0.1f);
    NDebugOverlay::Line(mover->GetFeet(), mover->GetFeet() + axisSize * normal,
                        0, 255, 0, true, 0.1f);
    NDebugOverlay::Line(mover->GetFeet(), mover->GetFeet() + axisSize * left, 0,
                        0, 255, true, 0.1f);
  }
  ...
  // limit too high distance to reasonable value for bots that can climb very
  // high
  ...
  if (!m_goal->ladder && !mover->IsClimbingOrJumping() && !isOnStairs &&
    ...
    Vector2D to(mover->GetFeet().x - m_goal->pos.x,
                mover->GetFeet().y - m_goal->pos.y);
    if (mover->IsStuck() || to.IsLengthLessThan(closeRange)) {
      // the goal is too high to reach

      // check if we can reach the next segment, in case this was a "jump down"
      // situation
      const Path::Segment *next = NextSegment(m_goal);
      if (mover->IsStuck() || !next ||
          (next->pos.z - mover->GetFeet().z > mover->GetMaxJumpHeight()) ||
          !mover->IsPotentiallyTraversable(mover->GetFeet(), next->pos)) {
        ...
        // reset stuck status since we're (likely) repathing anyways. otherwise,
        // we could be stuck in a loop here and not move
  ...
  if (true ||
      m_goal != LastSegment()) // think more about this - we often need to avoid
                               // to reach the final goal pos, too (MSB 5/15/09)
  {
  ...
  if (bot->IsDebugging(NEXTBOT_PATH)) {
    ...
    */
    ...
    NDebugOverlay::Line(bot->GetEntity()->WorldSpaceCenter(), goalPos, 255, 255,
                        0, true, 0.1f);
  }
}
...
CBaseEntity *PathFollower::FindBlocker(INextBot *bot) {
  ...
  const float size =
      body->GetHullWidth() /
      4.0f; // keep this small to avoid lockups when groups of bots get close
  ...
  // because our path goal may be far ahead of us if the way to there is
  // unobstructed, we need to start looking from the point of the path we are
  // actually standing on
  MoveCursorToClosestPosition(mover->GetFeet());

  for (const Segment *s = GetCursorData().segmentPrior;
       s && range < maxHindranceRangeAlong; s = NextSegment(s)) {
    // trace along direction toward goal a minimum range, in case goal and
    // hindrance are very close, but goal is closer
    ...
    mover->TraceHull(from, from + traceRange * traceForward, blockerMins,
                     blockerMaxs, body->GetSolidMask(), &filter, &result);

    if (result.DidHitNonWorldEntity()) {
      // if blocker is close, they could be behind us - check
      Vector toBlocker = result.m_pEnt->GetAbsOrigin() -
                         bot->GetLocomotionInterface()->GetFeet();
      ...
      if (DotProduct(toBlocker, alongPath) > 0.0f) {
        ...
        if (think->IsHindrance(bot, result.m_pEnt) == ANSWER_YES) {
          if (bot->IsDebugging(NEXTBOT_PATH)) {
            NDebugOverlay::Circle(bot->GetLocomotionInterface()->GetFeet(),
                                  QAngle(-90.0f, 0, 0), 10.0f, 255, 0, 0, 255,
                                  true, 1.0f);
            NDebugOverlay::HorzArrow(bot->GetLocomotionInterface()->GetFeet(),
                                     result.m_pEnt->GetAbsOrigin(), 1.0f, 255,
                                     0, 0, 255, true, 1.0f);
          }
 ...
 */
Vector PathFollower::Avoid(INextBot *bot, const Vector &goalPos,
                           const Vector &forward, const Vector &left) {
  ...
  // only use crouch-high avoid volumes, since we'll just crouch if higher
  // obstacles are near
  ...
  mover->TraceHull(m_leftFrom, m_leftTo, m_hullMin, m_hullMax, mask,
                   &traverseFilter, &result);
  if (result.fraction < 1.0f || result.startsolid) {
    // if this sensor is starting in a solid, set fraction to emulate being
    // against a wall
    ...
    // 		mover->TraceHull( m_leftFrom, m_leftTo, nextStepHullMin,
    // m_hullMax, mask, &filter, &result ); 		if ( result.fraction <= firstHit )
    // //+ mover->GetStepHeight()/2.0f )
    // 		{
  ...
  mover->TraceHull(m_rightFrom, m_rightTo, m_hullMin, m_hullMax, mask,
                   &traverseFilter, &result);
  if (result.fraction < 1.0f || result.startsolid) {
    // if this sensor is starting in a solid, set fraction to emulate being
    // against a wall
    ...
    // check for steps
    // 		float firstHit = result.fraction;
    // 		mover->TraceHull( m_rightFrom, m_rightTo, nextStepHullMin,
    // m_hullMax, mask, &filter, &result ); 		if ( result.fraction <= firstHit )
    // // + mover->GetStepHeight()/2.0f)
    // 		{
    // 			// it's not a step - we hit something
  ...
  if (door && !m_isLeftClear && !m_isRightClear) {
    ...
    if (bot->IsDebugging(NEXTBOT_PATH)) {
      NDebugOverlay::Axis(door->GetAbsOrigin(), door->GetAbsAngles(), 20.0f,
                          true, 10.0f);
      NDebugOverlay::Line(door->GetAbsOrigin(), doorEdge, 255, 255, 0, true,
                          10.0f);
    }
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Given a hull that defines the area of space that may contain a climbable
 * ledge, subdivide it until we find the ledge.
 */
bool PathFollower::FindClimbLedge(INextBot *bot, Vector startTracePos,
                                  Vector ledgeRegionMins,
                                  Vector ledgeRegionMaxs) {
  ...
  mover->TraceHull(startTracePos, startTracePos, ledgeRegionMins,
                   ledgeRegionMaxs, bot->GetBodyInterface()->GetSolidMask(),
                   &filter, &result);
  ...
  if (result.DidHit()) {
    ...
    FindClimbLedge(bot, startTracePos, upperLedgeRegionMins,
                   upperLedgeRegionMaxs);

    Vector lowerLedgeRegionMins = ledgeRegionMins;
    Vector lowerLedgeRegionMaxs(ledgeRegionMaxs.x, ledgeRegionMaxs.y, midZ);
    FindClimbLedge(bot, startTracePos, lowerLedgeRegionMins,
                   lowerLedgeRegionMaxs);
  } else {
    // volume is clear, trace straight down to find ledge and keep lowest one
    // we've found
    mover->TraceHull(startTracePos, startTracePos + Vector(0, 0, -100.0f),
 ...
 * Climb up ledges
 */
bool PathFollower::Climbing(INextBot *bot, const Path::Segment *goal,
  ...
  // we can't have this as large as our hull width, or we'll find ledges ahead
  // of us that we will fall from when we climb up because our hull wont
  // actually touch at the top.
  const float ledgeLookAheadRange = body->GetHullWidth() - 1;

  if (mover->IsClimbingOrJumping() || mover->IsAscendingOrDescendingLadder() ||
      !mover->IsOnGround()) {
    return false;
  }
  ...
  if (TheNavMesh->IsAuthoritative()) {
    ...
    // No need for expensive ledge-finding for games with simpler geometry (like
    // TF2)
    //
    if (m_goal->type == CLIMB_UP) {
      ...
      if (afterClimb && afterClimb->area) {
        ...
        afterClimb->area->GetClosestPointOnArea(mover->GetFeet(),
                                                &nearClimbGoal);
  ...
  // If we're approaching a CLIMB_UP link, save off the height delta for it, and
  // trust the nav *just* enough to climb up to that ledge and only that ledge.
  // We keep as large a tolerance as possible, to trust the nav as little as
  // possible.  There's no valid way to have another CLIMB_UP link within crouch
  // height, because we can't actually fit in between the two areas, so one
  // climb is invalid.
  ...
  if (m_goal->area->HasAttributes(NAV_MESH_STAIRS) ||
      (myArea && myArea->HasAttributes(NAV_MESH_STAIRS))) {
    if (bot->IsDebugging(NEXTBOT_PATH)) {
  ...
  // If path segment immediately ahead of us is not obstructed, don't try to
  // climb. This is required to try to avoid accidentally climbing onto valid
  // high ledges when we really want to run UNDER them to our destination. We
  // need to check "immediate" traversability to pay attention to breakable
  // objects in our way that we should climb over. We also need to check
  // traversability out to 2 * ledgeLookAheadRange in case our goal is just
  // before a tricky ledge climb and once we pass the goal it will be too late.
  // When we're in a CLIMB_UP segment, allow us to look for ledges - we know the
  // destination ledge height, and will only grab the correct ledge.
  Vector toGoal = m_goal->pos - mover->GetFeet();
  toGoal.NormalizeInPlace();

  if (toGoal.z < mover->GetTraversableSlopeLimit() && !mover->IsStuck() &&
      m_goal->type != CLIMB_UP &&
      mover->IsPotentiallyTraversable(mover->GetFeet(),
                                      mover->GetFeet() +
                                          2.0f * ledgeLookAheadRange * toGoal,
  ...
  // Start with current, the segment we are currently traversing.  Skip the
  // distance check for that segment, because the pos is (hopefully) behind us.
  // And if it's a long path segment, it's already outside the
  // climbLookAheadRange, and thus it would prevent us looking at m_goal and
  // further for imminent planned climbs.
  ...
  for (const Segment *s = current; s; s = NextSegment(s)) {
    if (s != current && (s->pos - mover->GetFeet())
                            .AsVector2D()
                            .IsLengthGreaterThan(climbLookAheadRange)) {
      break;
    }
  ...
  // need to use minimum actual hull height here to catch porous fences and
  // railings
  ...
  // Using the full width hull catches on small lips/ledges, so back up and try
  // again.
  float ceilingFraction;

  // We can't use IsPotentiallyTraversable to test for ledges, because it's
  // smaller Hull can cause the next trace (trace the ceiling height forward) to
  // start solid.
  //	mover->IsPotentiallyTraversable( mover->GetFeet(), mover->GetFeet() +
  //Vector( 0, 0, mover->GetMaxJumpHeight() ), ILocomotion::IMMEDIATELY,
  //&ceilingFraction );

  // Instead of IsPotentiallyTraversable, we back up the same distance and use a
  // second upward trace to see if that one finds a higher ceiling.  If so, we
  // use that ceiling height, and use the backed-up feet position for the ledge
  // finding traces.
  Vector feet(mover->GetFeet());
  Vector ceiling(feet + Vector(0, 0, mover->GetMaxJumpHeight()));
  mover->TraceHull(feet, ceiling, skipStepHeightHullMin, skipStepHeightHullMax,
                   mask, &filter, &result);
  ...
  if (ceilingFraction < 1.0f || result.startsolid) {
    ...
    const float backupDistance =
        hullWidth * 0.25f; // The IsPotentiallyTraversable check this replaces
                           // uses a 1/4 hull width trace
    Vector backupFeet(feet - climbDirection * backupDistance);
    Vector backupCeiling(backupFeet + Vector(0, 0, mover->GetMaxJumpHeight()));
    mover->TraceHull(backupFeet, backupCeiling, skipStepHeightHullMin,
                     skipStepHeightHullMax, mask, &filter, &backupTrace);
  ...
  mover->TraceHull(feet, feet + climbDirection * ledgeLookAheadRange,
  ...
  if (bot->IsDebugging(NEXTBOT_PATH) && NextBotDebugClimbing.GetBool()) {
    ...
    NDebugOverlay::SweptBox(feet, feet + climbDirection * ledgeLookAheadRange,
                            skipStepHeightHullMin, climbHullMax, vec3_angle,
                            255, 100, 0, 255, 0.1f);
  }

  bool wasPotentialLedgeFound = result.DidHit() && !result.startsolid;
  // To test climbing up past small lips on walls, we can force the bot to run
  // past the overhang and use the backup trace: wasPotentialLedgeFound =
  // wasPotentialLedgeFound && (result.fraction == 0 || isBackupTraceUsed);
  if (wasPotentialLedgeFound) {
    VPROF_BUDGET("PathFollower::Climbing( Search for ledge to climb )",
                 "NextBot");
    ...
    if (bot->IsDebugging(NEXTBOT_PATH) && NextBotDebugClimbing.GetBool()) {
      ...
      NDebugOverlay::HorzArrow(feet, feet + 50.0f * climbDirection, 2.0f, 0,
                               255, 0, 255, true, 9999.9f);
    }
    ...
    if (!result.DidHitNonWorldEntity() || bot->IsAbleToClimbOnto(obstacle)) {
      ...
      if (m_goal->type == CLIMB_UP) {
        // Climbing up to a narrow nav area indicates a narrow ledge.  We need
        // to reduce our minLedgeDepth here or our path will say we should climb
        // but we'll forever fail to find a wide enough ledge.
        const Segment *afterClimb = NextSegment(m_goal);
        if (afterClimb && afterClimb->area) {
          Vector depthVector = climbDirection * minLedgeDepth;
          depthVector.z = 0;
          if (fabs(depthVector.x) > afterClimb->area->GetSizeX()) {
            depthVector.x = (depthVector.x > 0) ? afterClimb->area->GetSizeX()
                                                : -afterClimb->area->GetSizeX();
          }
          if (fabs(depthVector.y) > afterClimb->area->GetSizeY()) {
            depthVector.y = (depthVector.y > 0) ? afterClimb->area->GetSizeY()
                                                : -afterClimb->area->GetSizeY();
          }
      ...
      // once we have found the ledge's front wall, we must look at least
      // minLedgeDepth farther in to verify it is a ledge NOTE: This *must* be
      // ledgeLookAheadRange since ledges are compared against the initial trace
      // which was ledgeLookAheadRange deep
      float ledgeTopLookAheadRange = ledgeLookAheadRange;

      // TODO: we also must look far enough ahead in case the ledge we actually
      // find is "deeper" than the initial wall at the base
      ...
      while (true) {
        // trace forward to find the wall in front of us, or the empty space of
        // the ledge above us
        mover->TraceHull(feet + Vector(0, 0, ledgeHeight),
                         feet + Vector(0, 0, ledgeHeight) +
                             climbDirection * ledgeTopLookAheadRange,
        ...
        if (!result.startsolid) {
          ...
          if (foundWall) {
            ...
            if ((traceDepth - ledgeFrontWallDepth) > minLedgeDepth) {
              ...
              // The ledge can be no lower, or we would've found it in the last
              // iteration.
              mover->TraceHull(
                  ledgePos, ledgePos + Vector(0, 0, -ledgeHeightIncrement),
                  climbHullMin, climbHullMax, mask, &filter, &result);

              ledgePos = result.endpos;

              // if the whole trace is in solid, we're out of luck, but
              // if the trace just started solid, 'ledgePos' should still be
              // valid since the trace left the solid and then hit. if the trace
              // hit nothing, the potential ledge is actually deeper in
              const float MinGroundNormal =
                  0.7f; // players can't stand on ground steeper than 0.7
              if (result.allsolid || !result.DidHit() ||
                  result.plane.normal.z < MinGroundNormal) {
                // not a usable ledge, try again
                isUsable = false;
              } else {
                if (climbUpLedgeHeightDelta > 0.0f) {
                  // if we're climbing to a specific ledge via a CLIMB_UP link,
                  // only climb to that ledge. Do this only for the world (which
                  // includes static props) so we can still opportunistically
                  // climb up onto breakable railings and physics props.
                  if (result.DidHitWorld()) {
                    float potentialLedgeHeight = result.endpos.z - feet.z;
                    if (fabs(potentialLedgeHeight - climbUpLedgeHeightDelta) >
                        climbUpLedgeTolerance) {
                      isUsable = false;
                    }
                  }
                }
              ...
              if (isUsable) {
                // back up until we no longer are hitting the ledge to determine
                // the exact ledge edge position
                ...
                while (backUpSoFar < maxBackUp) {
                  ...
                  mover->TraceHull(
                      testPos, testPos + Vector(0, 0, -ledgeHeightIncrement),
                      climbHullMin, climbHullMax, mask, &filter, &result);

                  if (bot->IsDebugging(NEXTBOT_PATH) &&
                      NextBotDebugClimbing.GetBool()) {
                    // show edge-finder hulls
                    NDebugOverlay::SweptBox(
                        testPos,
                        testPos + Vector(0, 0, -mover->GetStepHeight()),
                        climbHullMin, climbHullMax, vec3_angle, 255, 0, 0, 255,
                        999.9f);
                  }
                  ...
                  if (result.DidHit() &&
                    ...
                    // nothing but air or a steep slope below us, we have found
                    // the edge
                    break;
                  }
                }
                ...
                // Make sure this doesn't embed us in the far wall if the ledge
                // is narrow, since we would have backed up less than halfSize.
                Vector climbHullMinStep(
                    climbHullMin); // skip StepHeight for sloped ledges
                mover->TraceHull(validLedgePos, ledgePos, climbHullMinStep,
                                 climbHullMax, mask, &filter, &result);

                ledgePos = result.endpos;

                // Now since ledgePos + StepHeight is valid, trace down to find
                // ground on sloped ledges.
                mover->TraceHull(ledgePos + Vector(0, 0, StepHeight), ledgePos,
                                 climbHullMin, climbHullMax, mask, &filter,
                                 &result);
                if (!result.startsolid) {
                  ledgePos = result.endpos;
                }
              ...
              /*** NOTE: While this saves us from climbing into a window below
               the window we want to get in,
               *** it also causes us to climb in midair high over crates sitting
               against walls we need to climb over. if ( isUsable &&
               m_goal->type == CLIMB_UP )
                                                                      {
                                                                              //
               ...
               CLIMB_UP destination
                                                                              //
               NOTE: Can't use plannedClimbZ here, since that could be 2 or 3
               short climbs ahead const Segment *ledge = NextSegment( m_goal );

                                                                              if
               ( !ledge || ledgeHeight < ledge->pos.z - feet.z -
               mover->GetStepHeight() )
                                                                              {
            ...
            // meaning the next iteration that reaches far enough will be our
            // ledge
            ...
            // make sure the subsequent traces are at least minLedgeDepth deeper
            // than the wall we just found, or all ledge checks will fail
            ...
            if (bot->IsDebugging(NEXTBOT_PATH)) {
              ...
              if (NextBotDebugClimbing.GetBool()) {
                NDebugOverlay::HorzArrow(
                    result.endpos, result.endpos + 20.0f * result.plane.normal,
                    5.0f, 255, 100, 0, 255, true, 9999.9f);
              }
              wallPos = result.endpos;
            }
          } else if (ledgeHeight > body->GetCrouchHullHeight() &&
                     !isPlannedClimbImminent) {
            // we haven't hit anything yet, and we're already above our heads -
            // no obstacle
            if (bot->IsDebugging(NEXTBOT_PATH)) {
              DevMsg("%3.2f: Climbing - skipping overhead climb we can "
                     "walk/crawl under.\n",
                     gpGlobals->curtime);
            }
            break;
      ...
      if (foundLedge) {
        if (bot->IsDebugging(NEXTBOT_PATH)) {
          ...
          if (NextBotDebugClimbing.GetBool()) {
            ...
            NDebugOverlay::Triangle(wallBottomRight, wallBottomLeft,
                                    wallTopLeft, 255, 100, 0, 100, true,
                                    9999.9f);
            NDebugOverlay::Triangle(wallBottomRight, wallTopLeft, wallTopRight,
                                    255, 100, 0, 100, true, 9999.9f);

            Vector ledgeLeft = ledgePos + halfSize * side;
            Vector ledgeRight = ledgePos - halfSize * side;

            NDebugOverlay::Triangle(wallTopRight, wallTopLeft, ledgeLeft, 0,
                                    100, 255, 100, true, 9999.9f);
            NDebugOverlay::Triangle(wallTopRight, ledgeLeft, ledgeRight, 0, 100,
                                    255, 100, true, 9999.9f);
          }
        }
 ...
 * Jump over gaps
 */
bool PathFollower::JumpOverGaps(INextBot *bot, const Path::Segment *goal,
                                const Vector &forward, const Vector &right,
                                float goalRange) {
  ...
  if (gap) {
    ...
    if (mover->IsGap(mover->GetFeet() + halfWidth * gap->forward,
                     gap->forward)) {
      ...
      if (landing) {
        ...
        // if we're jumping over this gap, make sure our goal is the landing so
        // we aim for it
        m_goal = landing;
...
void PathFollower::Draw(const Path::Segment *start) const {
  ...
  if (m_didAvoidCheck) {
    ...
    if (m_isLeftClear)
      NDebugOverlay::SweptBox(m_leftFrom, m_leftTo, m_hullMin, m_hullMax,
                              angles, 0, 255, 0, 255, 0.1f);
    else
      NDebugOverlay::SweptBox(m_leftFrom, m_leftTo, m_hullMin, m_hullMax,
                              angles, 255, 0, 0, 255, 0.1f);

    if (m_isRightClear)
      NDebugOverlay::SweptBox(m_rightFrom, m_rightTo, m_hullMin, m_hullMax,
                              angles, 0, 255, 0, 255, 0.1f);
    else
      NDebugOverlay::SweptBox(m_rightFrom, m_rightTo, m_hullMin, m_hullMax,
                              angles, 255, 0, 0, 255, 0.1f);
  ...
  if (m_goal) {
    ...
    case GO_SOUTH:
      NDebugOverlay::Line(
          m_goal->m_portalCenter - Vector(m_goal->m_portalHalfWidth, 0, 0),
          m_goal->m_portalCenter + Vector(m_goal->m_portalHalfWidth, 0, 0), 255,
          0, 255, true, 0.1f);
      break;

    default:
      NDebugOverlay::Line(
          m_goal->m_portalCenter - Vector(0, m_goal->m_portalHalfWidth, 0),
          m_goal->m_portalCenter + Vector(0, m_goal->m_portalHalfWidth, 0), 255,
          0, 255, true, 0.1f);
      break;
    }
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Return true if there is a the given discontinuity ahead in the path within
 * the given range (-1 = entire remaining path)
 */
bool PathFollower::IsDiscontinuityAhead(INextBot *bot, Path::SegmentType type,
                                        float range) const {
  if (m_goal) {
    ...
    float rangeSoFar =
        (m_goal->pos - bot->GetLocomotionInterface()->GetFeet()).Length();
