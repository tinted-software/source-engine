 ...
 */
Vector ChasePath::PredictSubjectPosition(INextBot *bot,
  ...
  if (lead.LengthSqr() > 36.0f) {
    float fraction;
    if (!mover->IsPotentiallyTraversable(subjectPos, pathTarget,
                                         ILocomotion::IMMEDIATELY, &fraction)) {
  ...
  if (!leadArea || leadArea->GetZ(pathTarget.x, pathTarget.y) <
                       pathTarget.z - mover->GetMaxJumpHeight()) {
