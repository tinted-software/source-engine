...
enum ActionResultType {
  ...
  SUSTAIN, // for use with event handlers - a way to say "It's important to keep
 ...
 * Do not assemble this yourself - use the Continue(), ChangeTo(), Done(), and
 * SuspendFor() methods within Action.
 */
template <typename Actor> struct IActionResult {
  IActionResult(ActionResultType type = CONTINUE, Action<Actor> *action = NULL,
  ...
  bool IsDone(void) const { return (m_type == DONE); }

  bool IsContinue(void) const { return (m_type == CONTINUE); }
 ...
 * SuspendFor() methods within Action.
 */
template <typename Actor> struct ActionResult : public IActionResult<Actor> {
  // this is derived from IActionResult to ensure that ActionResult and
  // EventDesiredResult cannot be silently converted
  ActionResult(ActionResultType type = CONTINUE, Action<Actor> *action = NULL,
               const char *reason = NULL)
 ...
 * Do not assemble this yourself - use the TryContinue(), TryChangeTo(),
 * TryDone(), TrySustain(), and TrySuspendFor() methods within Action.
 */
...
struct EventDesiredResult : public IActionResult<Actor> {
  EventDesiredResult(ActionResultType type = CONTINUE,
                     Action<Actor> *action = NULL,
                     EventResultPriorityType priority = RESULT_TRY,
                     const char *reason = NULL)
      : IActionResult<Actor>(type, action, reason) {
    m_priority = priority;
  }
...
//-------------------------------------------------------------------------------------------------------------
/**
 * A Behavior is the root of an Action hierarchy as well as its
 * container/manager. Instantiate a Behavior with the root Action of your
 * behavioral system, and call Behavior::Update() to drive it.
 */
template <typename Actor>
...
public:
   ...
   */
  bool IsEmpty(void) const { return m_action == NULL; }
  ...
  void Update(Actor *me, float interval) {
    ...
    m_action = m_action->ApplyResult(
        me, this, m_action->InvokeUpdate(me, this, interval));

    if (m_action && me->IsDebugging(NEXTBOT_BEHAVIOR)) {
      CFmtStr msg;
      me->DisplayDebugText(
          msg.sprintf("%s: %s", GetName(), m_action->DebugString()));
    }
  ...
  NextContainedResponder(INextBotEventResponder *current) const {
    return NULL;
  }
  ...
  ShouldPickUp(const INextBot *me,
               CBaseEntity *item) const // if the desired item was available
                                        // right now, should we pick it up?
  {
  ...
  virtual QueryResultType
  ShouldHurry(const INextBot *me) const // are we in a hurry?
  {
    QueryResultType result = ANSWER_UNDEFINED;
  ...
  virtual QueryResultType
  ShouldRetreat(const INextBot *me) const // is it time to retreat?
  {
    QueryResultType result = ANSWER_UNDEFINED;
  ...
  virtual QueryResultType
  ShouldAttack(const INextBot *me,
  ...
  virtual QueryResultType IsHindrance(const INextBot *me, CBaseEntity *blocker)
      const // return true if we should wait for 'blocker' that is across our
  ...
  virtual Vector SelectTargetPoint(const INextBot *me,
  ...
  virtual const CKnownEntity *SelectMoreDangerousThreat(
      const INextBot *me, const CBaseCombatCharacter *subject,
      const CKnownEntity *threat1, const CKnownEntity *threat2)
      const // return the more dangerous of the two threats, or NULL if we have
            // no opinion
  {
    ...
    if (m_action) {
      ...
      while (action && result == NULL) {
        ...
        while (action && result == NULL) {
          result =
              action->SelectMoreDangerousThreat(me, subject, threat1, threat2);
          action = action->GetActionBuriedUnderMe();
        }
...
#define MAX_NAME_LENGTH 32
  ...
  CUtlVector<Action<Actor> *>
      m_deadActionVector; // completed Actions pending deletion
};

//----------------------------------------------------------------------------------------------
/**
 * Something an Actor does.
 * Actions can contain Actions, representing the precise context of the Actor's
 * behavior. A system of Actions is contained within a Behavior, which acts as
...
public:
  ...
  virtual bool IsNamed(const char *name)
      const; // return true if given name matches the name of this Action
  virtual const char *
  GetFullName(void) const; // return a temporary string showing the full lineage
                           // of this one action
  Actor *GetActor(void) const; // return the Actor performing this Action (valid
                               // just before OnStart() is invoked)

  //-----------------------------------------------------------------------------------------
  /**
   * Try to start the Action. Result is immediately processed,
   * which can cause an immediate transition, another OnStart(), etc.
   * An Action can count on each OnStart() being followed (eventually) with an
   * OnEnd().
   */
   ...
   */
  virtual ActionResult<Actor> OnSuspend(Actor *me,
                                        Action<Actor> *interruptingAction) {
    return Continue();
  }

  // When an Action is resumed after being suspended
  virtual ActionResult<Actor> OnResume(Actor *me,
                                       Action<Actor> *interruptingAction) {
    return Continue();
  }
  ...
  ActionResult<Actor> ChangeTo(Action<Actor> *action,
                               const char *reason = NULL) const;
  ActionResult<Actor> SuspendFor(Action<Actor> *action,
                                 const char *reason = NULL) const;
  ActionResult<Actor> Done(const char *reason = NULL) const;

  // create and return an Action to start as sub-action within this Action when
  // it starts
  virtual Action<Actor> *InitialContainedAction(Actor *me) { return NULL; }

  //-----------------------------------------------------------------------------------------
  /**
   * Override the event handler methods below to respond to events that occur
   * during this Action NOTE: These are identical to the events in
   * INextBotEventResponder with the addition of an actor argument and a return
   * result. Their translators are located in the private area below.
   */
  virtual EventDesiredResult<Actor> OnLeaveGround(Actor *me,
                                                  CBaseEntity *ground) {
    return TryContinue();
  }
  virtual EventDesiredResult<Actor> OnLandOnGround(Actor *me,
                                                   CBaseEntity *ground) {
    return TryContinue();
  }
  virtual EventDesiredResult<Actor> OnContact(Actor *me, CBaseEntity *other,
                                              CGameTrace *result = NULL) {
    return TryContinue();
  }
  virtual EventDesiredResult<Actor> OnMoveToSuccess(Actor *me,
                                                    const Path *path) {
    return TryContinue();
  }
  virtual EventDesiredResult<Actor> OnMoveToFailure(Actor *me, const Path *path,
  ...
  virtual EventDesiredResult<Actor> OnAnimationActivityComplete(Actor *me,
                                                                int activity) {
    return TryContinue();
  }
  virtual EventDesiredResult<Actor>
  OnAnimationActivityInterrupted(Actor *me, int activity) {
    return TryContinue();
  }
  virtual EventDesiredResult<Actor> OnAnimationEvent(Actor *me,
                                                     animevent_t *event) {
    return TryContinue();
  }
  ...
  virtual EventDesiredResult<Actor> OnInjured(Actor *me,
                                              const CTakeDamageInfo &info) {
    return TryContinue();
  }
  virtual EventDesiredResult<Actor> OnKilled(Actor *me,
                                             const CTakeDamageInfo &info) {
    return TryContinue();
  }
  virtual EventDesiredResult<Actor> OnOtherKilled(Actor *me,
                                                  CBaseCombatCharacter *victim,
                                                  const CTakeDamageInfo &info) {
    return TryContinue();
  }
  ...
  virtual EventDesiredResult<Actor> OnLostSight(Actor *me,
                                                CBaseEntity *subject) {
    return TryContinue();
  }
  virtual EventDesiredResult<Actor>
  OnSound(Actor *me, CBaseEntity *source, const Vector &pos, KeyValues *keys) {
    return TryContinue();
  }
  virtual EventDesiredResult<Actor> OnSpokeConcept(Actor *me,
                                                   CBaseCombatCharacter *who,
                                                   AIConcept_t concept,
                                                   AI_Response *response) {
    return TryContinue();
  }
  virtual EventDesiredResult<Actor>
  OnWeaponFired(Actor *me, CBaseCombatCharacter *whoFired,
                CBaseCombatWeapon *weapon) {
    return TryContinue();
  }
  virtual EventDesiredResult<Actor>
  OnNavAreaChanged(Actor *me, CNavArea *newArea, CNavArea *oldArea) {
    return TryContinue();
  }
  ...
  virtual EventDesiredResult<Actor>
  OnActorEmoted(Actor *me, CBaseCombatCharacter *emoter, int emote) {
    return TryContinue();
  }

  virtual EventDesiredResult<Actor> OnCommandAttack(Actor *me,
                                                    CBaseEntity *victim) {
    return TryContinue();
  }
  virtual EventDesiredResult<Actor>
  OnCommandApproach(Actor *me, const Vector &pos, float range) {
    return TryContinue();
  }
  virtual EventDesiredResult<Actor> OnCommandApproach(Actor *me,
                                                      CBaseEntity *goal) {
    return TryContinue();
  }
  virtual EventDesiredResult<Actor>
  OnCommandRetreat(Actor *me, CBaseEntity *threat, float range) {
    return TryContinue();
  }
  ...
  virtual EventDesiredResult<Actor> OnCommandString(Actor *me,
                                                    const char *command) {
    return TryContinue();
  }
  ...
  virtual EventDesiredResult<Actor> OnTerritoryContested(Actor *me,
                                                         int territoryID) {
    return TryContinue();
  }
  virtual EventDesiredResult<Actor> OnTerritoryCaptured(Actor *me,
                                                        int territoryID) {
    return TryContinue();
  }
  virtual EventDesiredResult<Actor> OnTerritoryLost(Actor *me,
                                                    int territoryID) {
    return TryContinue();
  }
  ...
  virtual EventDesiredResult<Actor> OnCommandMoveTo(Actor *me,
                                                    const Vector &pos) {
    return TryContinue();
  }
  virtual EventDesiredResult<Actor>
  OnCommandMoveToAggressive(Actor *me, const Vector &pos) {
    return TryContinue();
  }
  virtual EventDesiredResult<Actor>
  OnCommandAttack(Actor *me, CBaseEntity *victim, bool bDeny) {
    return TryContinue();
  }
  virtual EventDesiredResult<Actor>
  OnCastAbilityNoTarget(Actor *me, CDOTABaseAbility *ability) {
    return TryContinue();
  }
  virtual EventDesiredResult<Actor>
  OnCastAbilityOnPosition(Actor *me, CDOTABaseAbility *ability,
                          const Vector &pos) {
    return TryContinue();
  }
  virtual EventDesiredResult<Actor>
  OnCastAbilityOnTarget(Actor *me, CDOTABaseAbility *ability,
                        CBaseEntity *target) {
    return TryContinue();
  }
  virtual EventDesiredResult<Actor> OnDropItem(Actor *me, const Vector &pos,
                                               CBaseEntity *item) {
    return TryContinue();
  }
  ...
  virtual EventDesiredResult<Actor>
  OnFriendThreatened(Actor *me, CBaseEntity *friendly, CBaseEntity *threat) {
    return TryContinue();
  }
  virtual EventDesiredResult<Actor> OnCancelAttack(Actor *me,
                                                   CBaseEntity *pTarget) {
    return TryContinue();
  }
   ...
   */
  EventDesiredResult<Actor>
  TryContinue(EventResultPriorityType priority = RESULT_TRY) const;
  EventDesiredResult<Actor>
  TryChangeTo(Action<Actor> *action,
              EventResultPriorityType priority = RESULT_TRY,
              const char *reason = NULL) const;
  EventDesiredResult<Actor>
  TrySuspendFor(Action<Actor> *action,
                EventResultPriorityType priority = RESULT_TRY,
                const char *reason = NULL) const;
  EventDesiredResult<Actor>
  TryDone(EventResultPriorityType priority = RESULT_TRY,
          const char *reason = NULL) const;
  EventDesiredResult<Actor>
  TryToSustain(EventResultPriorityType priority = RESULT_TRY,
               const char *reason = NULL) const;

  //-----------------------------------------------------------------------------------------
  Action<Actor> *GetActiveChildAction(void) const;
  Action<Actor> *
  GetParentAction(void) const; // the Action that I'm running inside of

  bool IsSuspended(void)
      const; // return true if we are currently suspended for another Action

  const char *DebugString(void) const; // return a temporary string describing
                                       // the current action stack for debugging
...
#define PROCESS_EVENT(METHOD)                                                  \
  {                                                                            \
    ...
    EventDesiredResult<Actor> _result;                                         \
                                                                               \
    while (_action) {                                                          \
      if (m_actor && (m_actor->IsDebugging(NEXTBOT_EVENTS) ||                  \
                      NextBotDebugHistory.GetBool())) {                        \
        m_actor->DebugConColorMsg(                                             \
            NEXTBOT_EVENTS, Color(100, 100, 100, 255),                         \
    ...
    if (_action) {                                                             \
      if (m_actor && _result.IsRequestingChange() &&                           \
          (m_actor->IsDebugging(NEXTBOT_BEHAVIOR) ||                           \
           NextBotDebugHistory.GetBool())) {                                   \
        m_actor->DebugConColorMsg(NEXTBOT_BEHAVIOR, Color(255, 255, 0, 255),   \
                                  "%3.2f: %s:%s: ", gpGlobals->curtime,        \
                                  m_actor->GetDebugIdentifier(),               \
                                  m_behavior->GetName());                      \
        m_actor->DebugConColorMsg(NEXTBOT_BEHAVIOR, Color(255, 255, 255, 255), \
                                  "%s ", _action->GetFullName());              \
        m_actor->DebugConColorMsg(NEXTBOT_BEHAVIOR, Color(255, 255, 0, 255),   \
                                  "reponded to EVENT %s with ", #METHOD);      \
        m_actor->DebugConColorMsg(                                             \
            NEXTBOT_BEHAVIOR, Color(255, 0, 0, 255), "%s %s ",                 \
            _result.GetTypeName(),                                             \
            _result.m_action ? _result.m_action->GetName() : "");              \
        m_actor->DebugConColorMsg(NEXTBOT_BEHAVIOR, Color(0, 255, 0, 255),     \
                                  "%s\n",                                      \
...
#define PROCESS_EVENT_WITH_3_ARGS(METHOD, ARG1, ARG2, ARG3)                    \
  ...
  virtual void OnOtherKilled(CBaseCombatCharacter *victim,
                             const CTakeDamageInfo &info) {
  ...
  virtual void OnCastAbilityOnPosition(CDOTABaseAbility *ability,
                                       const Vector &pos) {
    PROCESS_EVENT_WITH_2_ARGS(OnCastAbilityOnPosition, ability, pos);
  }
  virtual void OnCastAbilityOnTarget(CDOTABaseAbility *ability,
                                     CBaseEntity *target) {
  ...
  Action<Actor> *m_child; // the ACTIVE Action we contain, top of the stack. Use
                          // m_buriedUnderMe, m_coveringMe on the child to
                          // traverse to other suspended children

  Action<Actor> *m_buriedUnderMe; // the Action just "under" us in the stack
                                  // that we will resume to when we finish
  Action<Actor> *m_coveringMe; // the Action just "above" us in the stack that
                               // will resume to us when it finishes
  ...
  Action<Actor> *
  GetActionBuriedUnderMe(void) const // return Action just "under" us that we
                                     // will resume to when we finish
  {
    return m_buriedUnderMe;
  }

  Action<Actor> *
  GetActionCoveringMe(void) const // return Action just "above" us that will
                                  // resume to us when it finishes
  {
    return m_coveringMe;
  }
  ...
  bool IsOutOfScope(void) const {
    for (Action<Actor> *under = GetActionBuriedUnderMe(); under;
  ...
  ActionResult<Actor> ProcessPendingEvents(void) const {
    ...
    if (m_eventResult.IsRequestingChange()) {
      ActionResult<Actor> result(m_eventResult.m_type, m_eventResult.m_action,
                                 m_eventResult.m_reason);

      // clear event result in case this change is a suspend and we later resume
      // this action
    ...
    while (under) {
      if (under->m_eventResult.m_type == SUSPEND_FOR) {
        // process this pending event in-place and push new Action on the top of
        // the stack
        ActionResult<Actor> result(under->m_eventResult.m_type,
                                   under->m_eventResult.m_action,
                                   under->m_eventResult.m_reason);

        // clear event result in case this change is a suspend and we later
        // resume this action
  ...
  // given the result of this Action's work, apply the result to potentially
  // cause a state transition
  Action<Actor> *ApplyResult(Actor *me, Behavior<Actor> *behavior,
                             ActionResult<Actor> result);

  /**
   * The methods below do the bookkeeping of each event, propagate the activity
   * through the hierarchy, and invoke the virtual event for each.
   */
  ActionResult<Actor> InvokeOnStart(Actor *me, Behavior<Actor> *behavior,
                                    Action<Actor> *priorAction,
                                    Action<Actor> *buriedUnderMeAction);
  ActionResult<Actor> InvokeUpdate(Actor *me, Behavior<Actor> *behavior,
                                   float interval);
  void InvokeOnEnd(Actor *me, Behavior<Actor> *behavior,
                   Action<Actor> *nextAction);
  Action<Actor> *InvokeOnSuspend(Actor *me, Behavior<Actor> *behavior,
                                 Action<Actor> *interruptingAction);
  ActionResult<Actor> InvokeOnResume(Actor *me, Behavior<Actor> *behavior,
                                     Action<Actor> *interruptingAction);

  /**
   * Store the given event result, attending to priorities
   */
  void StorePendingEventResult(const EventDesiredResult<Actor> &result,
    ...
    if (result.m_priority >= m_eventResult.m_priority) {
      ...
      // We keep the most recently processed event because this allows code to
      // check history/state to do custom event collision handling. If we keep
      // the first event at this priority and discard subsequent events
      // (original behavior) there is no way to predict future collision
      // resolutions (MSB).
  ...
  char *BuildDecoratedName(char *name, const Action<Actor> *action)
      const; // recursive name outMsg for DebugString()

  void PrintStateToConsole(void) const;
};

//-------------------------------------------------------------------------------------------
template <typename Actor> Action<Actor>::Action(void) {
  m_parent = NULL;
  m_child = NULL;
...
//-------------------------------------------------------------------------------------------
template <typename Actor> Action<Actor>::~Action() {
#ifdef DEBUG_BEHAVIOR_MEMORY
...
template <typename Actor> bool Action<Actor>::IsNamed(const char *name) const {
  return FStrEq(GetName(), name);
}

template <typename Actor> Actor *Action<Actor>::GetActor(void) const {
  return m_actor;
}
...
template <typename Actor>
ActionResult<Actor> Action<Actor>::ChangeTo(Action<Actor> *action,
                                            const char *reason) const {
  return ActionResult<Actor>(CHANGE_TO, action, reason);
}

template <typename Actor>
ActionResult<Actor> Action<Actor>::SuspendFor(Action<Actor> *action,
                                              const char *reason) const {
...
template <typename Actor>
EventDesiredResult<Actor>
Action<Actor>::TryContinue(EventResultPriorityType priority) const {
  return EventDesiredResult<Actor>(CONTINUE, NULL, priority);
}

template <typename Actor>
EventDesiredResult<Actor>
Action<Actor>::TryChangeTo(Action<Actor> *action,
...
template <typename Actor>
EventDesiredResult<Actor>
Action<Actor>::TrySuspendFor(Action<Actor> *action,
                             EventResultPriorityType priority,
                             const char *reason) const {
...
template <typename Actor>
EventDesiredResult<Actor>
Action<Actor>::TryDone(EventResultPriorityType priority,
                       const char *reason /*= NULL*/) const {
...
template <typename Actor>
EventDesiredResult<Actor>
Action<Actor>::TryToSustain(EventResultPriorityType priority,
                            const char *reason /*= NULL*/) const {
 ...
 */
template <typename Actor> bool Action<Actor>::IsSuspended(void) const {
...
template <typename Actor>
ActionResult<Actor>
Action<Actor>::InvokeOnStart(Actor *me, Behavior<Actor> *behavior,
                             Action<Actor> *priorAction,
                             Action<Actor> *buriedUnderMeAction) {
  // debug display
  if ((me->IsDebugging(NEXTBOT_BEHAVIOR) || NextBotDebugHistory.GetBool())) {
    me->DebugConColorMsg(NEXTBOT_BEHAVIOR, Color(255, 255, 150, 255),
                         "%3.2f: %s:%s: ", gpGlobals->curtime,
  ...
  // these value must be valid before invoking OnStart, in case an OnSuspend
  // happens
  m_isStarted = true;
  m_actor = me;
  m_behavior = behavior;
  ...
  // we are always on top of the stack. if our priorAction was buried, it
  // cleared everything covering it when it ended (which happens before we
  // start)
  m_coveringMe = NULL;
  ...
  if (m_child) {
    ...
    m_child = m_child->ApplyResult(me, behavior,
                                   ChangeTo(m_child, "Starting child Action"));
  }
...
//-------------------------------------------------------------------------------------------
template <typename Actor>
ActionResult<Actor> Action<Actor>::InvokeUpdate(Actor *me,
                                                Behavior<Actor> *behavior,
                                                float interval) {
  // an explicit "out of scope" check is needed here to prevent any
  // pending events causing an out of scope action to linger
  if (IsOutOfScope()) {
    // exit self to make this Action active and allow result to take effect on
    // its next Update
  ...
  if (m_child) {
    m_child = m_child->ApplyResult(
        me, behavior, m_child->InvokeUpdate(me, behavior, interval));
  }
...
//-------------------------------------------------------------------------------------------
/**
 * This method calls the virtual OnEnd() method for the Action, its children,
 * and Actions stacked on top of it. It does NOT delete resources, or disturb
 * pointer relationships, because this Action needs to remain valid for a short
 * while as an argument to OnStart(), OnSuspend(), etc for the next Action. The
 * destructor for the Action frees memory for this Action, its children, etc.
 */
template <typename Actor>
void Action<Actor>::InvokeOnEnd(Actor *me, Behavior<Actor> *behavior,
                                Action<Actor> *nextAction) {
  if (!m_isStarted) {
...
Action<Actor>::InvokeOnSuspend(Actor *me, Behavior<Actor> *behavior,
  ...
  if (me->IsDebugging(NEXTBOT_BEHAVIOR) || NextBotDebugHistory.GetBool()) {
    me->DebugConColorMsg(NEXTBOT_BEHAVIOR, Color(255, 255, 150, 255),
                         "%3.2f: %s:%s: ", gpGlobals->curtime,
                         me->GetDebugIdentifier(), behavior->GetName());
    me->DebugConColorMsg(NEXTBOT_BEHAVIOR, Color(255, 0, 255, 255),
                         " SUSPENDING ");
    me->DebugConColorMsg(NEXTBOT_BEHAVIOR, Color(255, 255, 255, 255),
                         GetName());
    me->DebugConColorMsg(NEXTBOT_BEHAVIOR, Color(255, 255, 255, 255), "\n");
...
template <typename Actor>
ActionResult<Actor>
Action<Actor>::InvokeOnResume(Actor *me, Behavior<Actor> *behavior,
                              Action<Actor> *interruptingAction) {
  if (me->IsDebugging(NEXTBOT_BEHAVIOR) || NextBotDebugHistory.GetBool()) {
    me->DebugConColorMsg(NEXTBOT_BEHAVIOR, Color(255, 255, 150, 255),
                         "%3.2f: %s:%s: ", gpGlobals->curtime,
  ...
  if (m_child) {
    m_child = m_child->ApplyResult(
        me, behavior,
        m_child->InvokeOnResume(me, behavior, interruptingAction));
  }
 ...
 */
template <typename Actor>
Action<Actor> *Action<Actor>::ApplyResult(Actor *me, Behavior<Actor> *behavior,
                                          ActionResult<Actor> result) {
  ...
  case CHANGE_TO: {
    ...
    if (me->IsDebugging(NEXTBOT_BEHAVIOR) || NextBotDebugHistory.GetBool()) {
      ...
      if (this == newAction) {
        ...
        me->DebugConColorMsg(NEXTBOT_BEHAVIOR, Color(255, 255, 255, 255),
                             this->GetName());
        me->DebugConColorMsg(NEXTBOT_BEHAVIOR, Color(255, 0, 0, 255),
                             " CHANGE_TO ");
        me->DebugConColorMsg(NEXTBOT_BEHAVIOR, Color(255, 255, 255, 255),
                             newAction->GetName());
      }
      ...
      if (result.m_reason) {
        me->DebugConColorMsg(NEXTBOT_BEHAVIOR, Color(150, 255, 150, 255),
                             "  (%s)\n", result.m_reason);
      } else {
        me->DebugConColorMsg(NEXTBOT_BEHAVIOR, Color(255, 255, 255, 255), "\n");
      }
    ...
    ActionResult<Actor> startResult =
        newAction->InvokeOnStart(me, behavior, this, this->m_buriedUnderMe);
  ...
  case SUSPEND_FOR: {
    ...
    ActionResult<Actor> startResult =
        newAction->InvokeOnStart(me, behavior, topAction, topAction);
  ...
  case DONE: {
    ...
    if (me->IsDebugging(NEXTBOT_BEHAVIOR) || NextBotDebugHistory.GetBool()) {
      ...
      if (resumedAction) {
        me->DebugConColorMsg(NEXTBOT_BEHAVIOR, Color(0, 255, 0, 255),
                             " DONE, RESUME ");
        me->DebugConColorMsg(NEXTBOT_BEHAVIOR, Color(255, 255, 255, 255),
                             resumedAction->GetName());
      } else {
    ...
    ActionResult<Actor> resumeResult =
        resumedAction->InvokeOnResume(me, behavior, this);
...
Action<Actor>::NextContainedResponder(INextBotEventResponder *current) const {
  return NULL;
}
 ...
 */
template <typename Actor> const char *Action<Actor>::DebugString(void) const {
  static char str[256];
...
//-------------------------------------------------------------------------------------------
template <typename Actor>
char *Action<Actor>::BuildDecoratedName(char *name,
                                        const Action<Actor> *action) const {
  const int fudge = 256;
 ...
 */
template <typename Actor> const char *Action<Actor>::GetFullName(void) const {
  const int fudge = 256;
  ...
  for (const Action<Actor> *action = this; stackIndex < maxStack && action;
...
//-------------------------------------------------------------------------------------------
template <typename Actor> void Action<Actor>::PrintStateToConsole(void) const {
