...
ConVar nb_debug_known_entities("nb_debug_known_entities", "0", FCVAR_CHEAT,
                               "Show the 'known entities' for the bot that is "
                               "the current spectator target");
 ...
 */
const CKnownEntity *
IVision::GetPrimaryKnownThreat(bool onlyVisibleThreats) const {
  ...
  for (i = 0; i < m_knownEntityVector.Count(); ++i) {
    ...
    if (IsAwareOf(firstThreat) && !firstThreat.IsObsolete() &&
        !IsIgnored(firstThreat.GetEntity()) &&
  ...
  for (++i; i < m_knownEntityVector.Count(); ++i) {
    ...
    // check in case status changes between updates
    if (IsAwareOf(newThreat) && !newThreat.IsObsolete() &&
        !IsIgnored(newThreat.GetEntity()) &&
        GetBot()->IsEnemy(newThreat.GetEntity())) {
      if (!onlyVisibleThreats || newThreat.IsVisibleRecently()) {
        threat = GetBot()->GetIntentionInterface()->SelectMoreDangerousThreat(
            GetBot(), GetBot()->GetEntity(), threat, &newThreat);
      }
    }
  }
 ...
 */
const CKnownEntity *
IVision::GetClosestKnown(const INextBotEntityFilter &filter) const {
  const Vector &myPos = GetBot()->GetPosition();
...
//------------------------------------------------------------------------------------------
/**
 * Given an entity, return our known version of it (or NULL if we don't know of
 * it)
 */
const CKnownEntity *IVision::GetKnown(const CBaseEntity *entity) const {
  ...
  for (int i = 0; i < m_knownEntityVector.Count(); ++i) {
    ...
    if (known.GetEntity() &&
        known.GetEntity()->entindex() == entity->entindex() &&
        !known.IsObsolete()) {
      return &known;
    }
  }
 ...
 * and will be updated, and it is assumed to not yet have been seen by us,
 * allowing for learning of known entities by being told about them, hearing
 * them, etc.
 */
...
// Useful if we've moved to where we last saw the entity, but it's not there any
// longer.
void IVision::ForgetEntity(CBaseEntity *forgetMe) {
  ...
  FOR_EACH_VEC(m_knownEntityVector, it) {
    ...
    if (known.GetEntity() &&
        known.GetEntity()->entindex() == forgetMe->entindex()) {
...
//------------------------------------------------------------------------------------------
void IVision::ForgetAllKnownEntities(void) { m_knownEntityVector.RemoveAll(); }

//------------------------------------------------------------------------------------------
/**
 * Return the number of entity on the given team known to us closer than
 * rangeLimit
 */
    ...
    if (!known.IsObsolete() && IsAwareOf(known)) {
      if (team == TEAM_ANY || known.GetEntity()->GetTeamNumber() == team) {
        if (!onlyVisible || known.IsVisibleRecently()) {
          if (rangeLimit < 0.0f ||
              GetBot()->IsRangeLessThan(known.GetLastKnownPosition(),
                                        rangeLimit)) {
            ++count;
          }
        }
 ...
 */
void IVision::CollectPotentiallyVisibleEntities(
...
public:
  CollectVisible(IVision *vision) { m_vision = vision; }

  bool operator()(CBaseEntity *entity) {
    if (entity && !m_vision->IsIgnored(entity) && entity->IsAlive() &&
  ...
  // update known set with new data
  {
    ...
    for (i = 0; i < m_knownEntityVector.Count(); ++i) {
      ...
      if (visibleNow.Contains(known.GetEntity())) {
        // this visible entity was already known (but perhaps not visible until
        // now)
        known.UpdatePosition();
        known.UpdateVisibilityStatus(true);

        // has our reaction time just elapsed?
        if (gpGlobals->curtime - known.GetTimeWhenBecameVisible() >=
                GetMinRecognizeTime() &&
            m_lastVisionUpdateTimestamp - known.GetTimeWhenBecameVisible() <
                GetMinRecognizeTime()) {
          if (GetBot()->IsDebugging(NEXTBOT_VISION)) {
            ...
            NDebugOverlay::Line(GetBot()->GetBodyInterface()->GetEyePosition(),
                                known.GetLastKnownPosition(), 255, 255, 0,
                                false, 0.2f);
          }
  ...
  // check for new recognizes that were not in the known set
  {
    ...
    for (i = 0; i < visibleNow.m_recognized.Count(); ++i) {
      ...
      if (j == m_knownEntityVector.Count()) {
        // recognized a previously unknown entity (emit OnSight() event after
        ...
        for (int i = 0; i < knownVector.Count(); ++i) {
          ...
          if (GetBot()->IsFriend(known.GetEntity())) {
            if (IsAwareOf(known)) {
              if (known.IsVisibleInFOVNow())
                NDebugOverlay::HorzArrow(GetBot()->GetEntity()->GetAbsOrigin(),
                                         known.GetLastKnownPosition(), 5.0f, 0,
                                         255, 0, 255, true,
                                         NDEBUG_PERSIST_TILL_NEXT_SERVER);
              else
                NDebugOverlay::HorzArrow(GetBot()->GetEntity()->GetAbsOrigin(),
                                         known.GetLastKnownPosition(), 2.0f, 0,
                                         100, 0, 255, true,
                                         NDEBUG_PERSIST_TILL_NEXT_SERVER);
            } else {
              NDebugOverlay::HorzArrow(GetBot()->GetEntity()->GetAbsOrigin(),
                                       known.GetLastKnownPosition(), 1.0f, 0,
                                       100, 0, 128, true,
                                       NDEBUG_PERSIST_TILL_NEXT_SERVER);
            }
          } else {
            if (IsAwareOf(known)) {
              if (known.IsVisibleInFOVNow())
                NDebugOverlay::HorzArrow(GetBot()->GetEntity()->GetAbsOrigin(),
                                         known.GetLastKnownPosition(), 5.0f,
                                         255, 0, 0, 255, true,
                                         NDEBUG_PERSIST_TILL_NEXT_SERVER);
              else
                NDebugOverlay::HorzArrow(GetBot()->GetEntity()->GetAbsOrigin(),
                                         known.GetLastKnownPosition(), 2.0f,
                                         100, 0, 0, 255, true,
                                         NDEBUG_PERSIST_TILL_NEXT_SERVER);
            } else {
              NDebugOverlay::HorzArrow(GetBot()->GetEntity()->GetAbsOrigin(),
                                       known.GetLastKnownPosition(), 1.0f, 100,
                                       0, 0, 128, true,
                                       NDEBUG_PERSIST_TILL_NEXT_SERVER);
            }
          }
...
//------------------------------------------------------------------------------------------
bool IVision::IsAbleToSee(CBaseEntity *subject, FieldOfViewCheckType checkFOV,
                          Vector *visibleSpot) const {
...
//------------------------------------------------------------------------------------------
bool IVision::IsAbleToSee(const Vector &pos,
...
bool IVision::IsInFieldOfView(const Vector &pos) const {
  ...
  bool bCheck = PointWithinViewAngle(
      GetBot()->GetBodyInterface()->GetEyePosition(), pos,
  ...
  if ((cosDiff > m_cosHalfFOV) != bCheck) {
    Assert(0);
    bool bCheck2 = PointWithinViewAngle(
        GetBot()->GetBodyInterface()->GetEyePosition(), pos,
        GetBot()->GetBodyInterface()->GetViewVector(), m_cosHalfFOV);
  }
...
//------------------------------------------------------------------------------------------
bool IVision::IsLineOfSightClearToEntity(const CBaseEntity *subject,
                                         Vector *visibleSpot) const {
#ifdef TERROR
  ...
  bool bClear = IsLineOfSightBetweenTwoEntitiesClear(
      GetBot()->GetBodyInterface()->GetEntity(), EOFFSET_MODE_EYEPOSITION,
      subject, EOFFSET_MODE_WORLDSPACE_CENTER, subject, COLLISION_GROUP_NONE,
      MASK_BLOCKLOS_AND_NPCS | CONTENTS_IGNORE_NODRAW_OPAQUE,
  ...
  UTIL_TraceLine(GetBot()->GetBodyInterface()->GetEyePosition(),
  ...
  if (subject->IsPlayer() && !bClear) {
    UTIL_TraceLine(GetBot()->GetBodyInterface()->GetEyePosition(),
                   ...
                   MASK_BLOCKLOS_AND_NPCS | CONTENTS_IGNORE_NODRAW_OPAQUE,
                   &filter, &result);
    bClear = IsLineOfSightBetweenTwoEntitiesClear(
        GetBot()->GetEntity(), EOFFSET_MODE_EYEPOSITION, subject,
        EOFFSET_MODE_EYEPOSITION, subject, COLLISION_GROUP_NONE,
        MASK_BLOCKLOS_AND_NPCS | CONTENTS_IGNORE_NODRAW_OPAQUE,
        IgnoreActorsTraceFilterFunction, 1.0);

    // this WILL assert - the query interface happens at a different time, and
    // has hysteresis.
    Assert(result.DidHit() != bClear);
  }
#endif
 ...
 */
bool IVision::IsLookingAt(const CBaseCombatCharacter *actor,
