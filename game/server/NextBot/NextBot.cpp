...
public:
  CSendBotCommand(const char *command) { m_command = command; }
...
//-----------------------------------------------------------------------------------------------------
CON_COMMAND_F(nb_delete_all, "Delete all non-player NextBot entities.",
              FCVAR_CHEAT) {
...
public:
  NextBotApproacher(void) {
    ...
    if (player) {
      ...
      trace_t result;
      unsigned int mask = MASK_BLOCKLOS_AND_NPCS |
                          CONTENTS_IGNORE_NODRAW_OPAQUE | CONTENTS_GRATE |
                          CONTENTS_WINDOW;
      UTIL_TraceLine(player->EyePosition(),
                     player->EyePosition() + 999999.9f * forward, mask, player,
...
CON_COMMAND_F(nb_move_to_cursor,
...
//----------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------
bool IgnoreActorsTraceFilterFunction(IHandleEntity *pServerEntity,
                                     int contentsMask) {
  CBaseEntity *entity = EntityFromEntityHandle(pServerEntity);
  return (entity->MyCombatCharacterPointer() ==
          NULL); // includes all bots, npcs, players, and TF2 buildings
}
...
void NextBotCombatCharacter::Touch(CBaseEntity *other) {
  if (ShouldTouch(other)) {
    ...
    // OnContact refers to *physical* contact, not triggers or other
...
//----------------------------------------------------------------------------------------------------------
void NextBotCombatCharacter::Ignite(float flFlameLifetime, bool bNPCOnly,
                                    float flSize, bool bCalledByLevelDesigner) {
...
//----------------------------------------------------------------------------------------------------------
void NextBotCombatCharacter::Ignite(float flFlameLifetime,
                                    CBaseEntity *pAttacker) {
  if (IsOnFire())
    return;

  // BaseClass::Ignite stuff, plus SetAttacker on the flame, so our attacker
  // gets credit
...
//----------------------------------------------------------------------------------------------------------
/**
 * Can't use CBaseCombatCharacter's Event_Killed because it will immediately
 * ragdoll us
 */
...
//----------------------------------------------------------------------------------------------------------
void NextBotCombatCharacter::PerformCustomPhysics(Vector *pNewPosition,
                                                  Vector *pNewVelocity,
                                                  QAngle *pNewAngles,
...
//----------------------------------------------------------------------------------------------------------
bool NextBotCombatCharacter::BecomeRagdoll(const CTakeDamageInfo &info,
 ...
 */
void NextBotCombatCharacter::OnNavAreaChanged(CNavArea *enteredArea,
                                              CNavArea *leftArea) {
 ...
 */
bool NextBotCombatCharacter::IsUseableEntity(CBaseEntity *entity,
