...
ConVar NextBotDebugHistory(
    "nb_debug_history", IsX360() ? "0" : "1", FCVAR_CHEAT,
    "If true, each bot keeps a history of debug output in memory");

//----------------------------------------------------------------------------------------------------------------
INextBot::INextBot(void)
    : m_debugHistory(MAX_NEXTBOT_DEBUG_HISTORY,
                     0) // CUtlVector: grow to max length, alloc 0 initially
{
...
INextBot::~INextBot() {
  ...
  // delete Intention first, since destruction of Actions may access other
  // components
  if (m_baseIntention)
...
void INextBot::Reset(void) {
  ...
  for (INextBotComponent *comp = m_componentList; comp;
...
//----------------------------------------------------------------------------------------------------------------
void INextBot::EndUpdate(void) { TheNextBots().NotifyEndUpdate(this); }
...
bool INextBot::IsEnemy(const CBaseEntity *them) const {
  ...
  return const_cast<INextBot *>(this)->GetEntity()->GetTeamNumber() !=
         them->GetTeamNumber();
}
...
bool INextBot::IsSelf(const CBaseEntity *them) const {
  ...
  return const_cast<INextBot *>(this)->GetEntity()->entindex() ==
         them->entindex();
}
...
const char *INextBot::GetDebugIdentifier(void) const {
  ...
  Q_snprintf(name, nameSize, "%s(#%d)",
             const_cast<INextBot *>(this)->GetEntity()->GetClassname(),
...
bool INextBot::IsAbleToBreak(const CBaseEntity *object) const {
  if (object && object->m_takedamage == DAMAGE_YES) {
    ...
    if (FClassnameIs(const_cast<CBaseEntity *>(object),
                     "func_breakable_surf")) {
      return true;
    }
...
void INextBot::DisplayDebugText(const char *text) const {
  const_cast<INextBot *>(this)->GetEntity()->EntityText(m_debugDisplayLine++,
                                                        text, 0.1);
}

//--------------------------------------------------------------------------------------------------------
void INextBot::DebugConColorMsg(NextBotDebugType debugType, const Color &color,
                                const char *fmt, ...) {
  ...
  // Don't bother with event data - it's spammy enough to overshadow everything
  // else.
...
void INextBot::GetDebugHistory(
    unsigned int type, CUtlVector<const NextBotDebugLineType *> *lines) const {
  if (!lines)
    return;
...
void INextBot::UpdateImmobileStatus(void) {
  if (m_immobileCheckTimer.IsElapsed()) {
    ...
    if ((GetEntity()->GetAbsOrigin() - m_immobileAnchor)
            .IsLengthGreaterThan(GetImmobileSpeedThreshold())) {
