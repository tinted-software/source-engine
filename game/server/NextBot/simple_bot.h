...
//--------------------------------------------------------------------------------------------------------------
/**
 * Functor used with the A* algorithm of NavAreaBuildPath() to determine the
 * "cost" of moving from one area to another. "Cost" is generally the weighted
 * distance between the centers of the areas. If you want the bot to avoid an
 * area/ladder/elevator, increase the cost.  If you want to disallow an
 * area/ladder/elevator, return -1.
 */
class CSimpleBotPathCost : public IPathCost {
public:
  CSimpleBotPathCost(CSimpleBot *me) { m_me = me; }

  // return the cost (weighted distance between) of moving from "fromArea" to
  // "area", or -1 if the move is not allowed
    ...
    if (fromArea == NULL) {
      ...
      if (deltaZ >= m_me->GetLocomotionInterface()->GetStepHeight()) {
        ...
        cost += jumpPenalty * dist;
                 ...
                 -m_me->GetLocomotionInterface()->GetDeathDropHeight()) {
        // too far to drop
        return -1.0f;
      }
