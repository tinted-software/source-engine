...
// trace filter callback functions. needed for use with the
// querycache/optimization functionality
...
public:
  NextBotTraceFilterIgnoreActors(const IHandleEntity *passentity,
                                 int collisionGroup)
...
public:
  NextBotTraceFilterIgnoreNextBots(const IHandleEntity *passentity,
                                   int collisionGroup)
      : CTraceFilterSimple(passentity, collisionGroup) {}
...
public:
  NextBotTraceFilter(const IHandleEntity *passentity, int collisionGroup)
      : CTraceFilterSimple(passentity, collisionGroup) {
    CBaseEntity *entity =
        const_cast<CBaseEntity *>(EntityFromEntityHandle(passentity));
  ...
  virtual bool ShouldHitEntity(IHandleEntity *pServerEntity, int contentsMask) {
    if (CTraceFilterSimple::ShouldHitEntity(pServerEntity, contentsMask)) {
      ...
      // Skip players on the same team - they're not solid to us, and we'll
      // avoid them
...
public:
  NextBotTraceFilterOnlyActors(const IHandleEntity *passentity,
                               int collisionGroup)
      : CTraceFilterSimple(passentity, collisionGroup) {}
 ...
 * a temporal context for asking if an entity is IMMEDIATELY traversable (like
 * thin glass that just breaks as we walk through it) or EVENTUALLY traversable
 * (like a breakable object that will take some time to break through)
 */
class NextBotTraversableTraceFilter : public CTraceFilterSimple {
public:
  NextBotTraversableTraceFilter(
      INextBot *bot,
      ILocomotion::TraverseWhenType when = ILocomotion::EVENTUALLY)
  ...
  virtual bool ShouldHitEntity(IHandleEntity *pServerEntity, int contentsMask) {
    ...
    if (CTraceFilterSimple::ShouldHitEntity(pServerEntity, contentsMask)) {
      return !m_bot->GetLocomotionInterface()->IsEntityTraversable(entity,
                                                                   m_when);
    }
 ...
 */
inline CBaseEntity *SelectClosestEntityByTravelDistance(
    INextBot *me, const CUtlVector<CBaseEntity *> &candidateEntities,
  ...
  for (int i = 0; i < candidateEntities.Count(); ++i) {
    ...
    CNavArea *area =
        TheNavMesh->GetNearestNavArea(candidate, GETNAVAREA_CHECK_LOS, 500.0f);
...
public:
  ...
  virtual bool ShouldHitEntity(IHandleEntity *pServerEntity, int contentsMask) {
    ...
    if (CTraceFilterSimple::ShouldHitEntity(pServerEntity, contentsMask)) {
      return !m_bot->GetLocomotionInterface()->IsEntityTraversable(
          entity, ILocomotion::IMMEDIATELY);
    }
