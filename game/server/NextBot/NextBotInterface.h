...
public:
  virtual bool IsSelected(const CBaseEntity *candidate)
      const = 0; // return true if this entity passes the filter
};
...
public:
  ...
  virtual void Upkeep(void); // (EXTEND) lightweight update guaranteed to occur
                             // every server tick
  ...
  GetPosition(void) const; // get the global position of the bot

  /**
   * Friend/enemy/neutral queries
   */
  virtual bool IsEnemy(const CBaseEntity *them)
      const; // return true if given entity is our enemy
  virtual bool IsFriend(const CBaseEntity *them)
      const; // return true if given entity is our friend
  virtual bool
  IsSelf(const CBaseEntity *them) const; // return true if 'them' is actually me
  ...
  /**
   * This immobile system is used to track the global state of "am I actually
   * moving or not". The OnStuck() event is only emitted when following a path,
   * and paths can be recomputed, etc.
   */
  virtual bool
  IsImmobile(void) const; // return true if we haven't moved in awhile
  virtual float
  GetImmobileDuration(void) const; // how long have we been immobile
  virtual void ClearImmobileStatus(void);
  virtual float
  GetImmobileSpeedThreshold(void) const; // return units/second below which this
                                         // actor is considered "immobile"
  ...
  virtual void NotifyPathDestruction(
      const PathFollower *path); // this PathFollower is going away, which may
                                 // or may not be ours
  ...
  virtual bool IsDebugging(unsigned int type)
      const; // return true if this bot is debugging any of the given types
  virtual const char *GetDebugIdentifier(
      void) const; // return the name of this bot for debugging purposes
  virtual bool IsDebugFilterMatch(
      const char *name) const; // return true if we match the given debug symbol
  virtual void DisplayDebugText(
      const char *text) const; // show a line of text on the bot in the world
  void DebugConColorMsg(NextBotDebugType debugType, const Color &color,
  ...
  void GetDebugHistory(unsigned int type,
                       CUtlVector<const NextBotDebugLineType *> *lines)
...
private:
  ...
  void RegisterComponent(
      INextBotComponent *comp); // components call this to register themselves
                                // with the bot that contains them
...
inline ILocomotion *INextBot::GetLocomotionInterface(void) const {
  // these base interfaces are lazy-allocated (instead of being fully instanced
  // classes) for two reasons: 1) so the memory is only used if needed 2) so the
  // component is registered properly
...
inline INextBotEventResponder *
INextBot::NextContainedResponder(INextBotEventResponder *current) const {
