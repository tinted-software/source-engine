...
//--------------------------------------------------------------------------------------------------------------------------
/**
 * Various processes can invoke a "reply" (ie: callback) via instances of this
 * interface
 */
class INextBotReply {
public:
  ...
  enum FailureReason { DENIED, INTERRUPTED, FAILED };
  virtual void OnFail(INextBot *bot, FailureReason reason) {
  } // invoked when process failed
};
...
public:
  ...
  } // lightweight update guaranteed to occur every server tick

  inline bool ComputeUpdateInterval(); // return false is no time has elapsed
                                       // (interval is zero)
...
private:
  ...
  INextBot *m_bot;
  INextBotComponent
      *m_nextComponent; // simple linked list of components in the bot
};
...
inline float INextBotComponent::GetUpdateInterval() { return m_curInterval; }
