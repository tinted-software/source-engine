...
ConVar nb_update_framelimit("nb_update_framelimit", (IsDebug()) ? "30" : "15",
                            FCVAR_CHEAT);
...
static const char *debugTypeName[] = {
    "BEHAVIOR", "LOOK_AT", "PATH",   "ANIMATION", "LOCOMOTION",
...
static ConCommand
    SetDebug("nb_debug", CC_SetDebug,
             "Debug NextBots.  Categories are: BEHAVIOR, LOOK_AT, PATH, "
...
static ConCommand
    SetDebugFilter("nb_debug_filter", CC_SetDebugFilter,
                   "Add items to the NextBot debug filter. Items can be "
                   "entindexes or part of the indentifier of one or more bots.",
                   FCVAR_CHEAT);
  ...
  bool operator()(INextBot *bot) {
    ...
    if (botEntity->IsAlive()) {
      ...
      if (DotProduct(m_forward, to) > 0.98f && range < m_pickRange) {
        if (!m_useLOS || m_player->IsAbleToSee(
                             botEntity, CBaseCombatCharacter::DISREGARD_FOV)) {
          m_pick = bot;
          m_pickRange = range;
        }
      }
...
static void CC_SelectBot(const CCommand &args) {
  ...
  if (player) {
    ...
    if (select.m_pick) {
      NDebugOverlay::Circle(select.m_pick->GetLocomotionInterface()->GetFeet() +
                                Vector(0, 0, 5),
                            Vector(1, 0, 0), Vector(0, -1, 0), 25.0f, 0, 255, 0,
                            255, false, 1.0f);
    }
  }
}
static ConCommand
    SelectBot("nb_select", CC_SelectBot,
...
static void CC_ForceLookAt(const CCommand &args) {
  ...
  if (player && pick) {
    pick->GetBodyInterface()->AimHeadTowards(player, IBody::CRITICAL,
                                             9999999.9f, NULL, "Aim forced");
  }
}
static ConCommand
    ForceLookAt("nb_force_look_at", CC_ForceLookAt,
...
void CC_WarpSelectedHere(const CCommand &args) {
  ...
  UTIL_TraceLine(me->EyePosition(), me->EyePosition() + 999999.9f * forward,
                 MASK_BLOCKLOS_AND_NPCS | CONTENTS_IGNORE_NODRAW_OPAQUE, me,
...
static ConCommand
    WarpSelectedHere("nb_warp_selected_here", CC_WarpSelectedHere,
...
void NextBotManager::Reset(void) {
  // remove the NextBots that should go away during a reset (they will
  // unregister themselves as they go)
  ...
  while (i != m_botList.InvalidIndex()) {
    ...
    if (m_botList[i]->IsRemovedOnReset()) {
      ...
      // Assert( !m_botList.IsInList( i ) );	// UTIL_Remove() calls
      // UpdateOnRemove, adds EFL_KILLME, but doesn't delete until the end of
      // the frame
    }
    i = iNext;
  }
...
void NextBotManager::Update(void) {
  ...
  for (int u = m_botList.Head(); u != m_botList.InvalidIndex();
       u = m_botList.Next(u)) {
    m_botList[u]->Upkeep();
  }
  ...
  if (m_botList.Count()) {
    ...
    if (m_iUpdateTickrate != tickRate) {
      Msg("NextBot tickrate changed from %d (%.3fms) to %d (%.3fms)\n",
          m_iUpdateTickrate, TICKS_TO_TIME(m_iUpdateTickrate), tickRate,
          TICKS_TO_TIME(tickRate));
    ...
    if (m_iUpdateTickrate > 0) {
      ...
      // Count dead bots, they won't update and balancing calculations should
      // exclude them
      for (i = m_botList.Head(); i != m_botList.InvalidIndex();
           i = m_botList.Next(i)) {
      ...
      int nTargetToRun =
          ceilf((float)(m_botList.Count() - nDead) / (float)m_iUpdateTickrate);
      int curtickcount = gpGlobals->tickcount;

      for (i = m_botList.Head(); nTargetToRun && i != m_botList.InvalidIndex();
           i = m_botList.Next(i)) {
        pBot = m_botList[i];
        if (pBot->IsFlaggedForUpdate()) {
          // Was offered a run last tick but didn't take it, push it back
          // Leave the flag set so that bot will run right away later, but be
          // ignored until then
          nNonResponsive++;
        } else {
    ...
    if (nb_update_debug.GetBool()) {
      ...
      if (m_iUpdateTickrate > 0) {
        for (; i != m_botList.InvalidIndex(); i = m_botList.Next(i)) {
          if (gpGlobals->tickcount - m_botList[i]->GetTickLastUpdate() >=
              m_iUpdateTickrate) {
            nIntentionalSliders++;
          }
        }
      ...
      Msg("Frame %8d/tick %8d: %3d run of %3d, %3d sliders, %3d blocked "
          "slides, scheduled %3d for next tick, %3d intentional sliders, %d "
          "nonresponsive, %d dead\n",
          gpGlobals->framecount - 1, gpGlobals->tickcount - 1, g_nRun,
          m_botList.Count() - nDead, g_nSlid, g_nBlockedSlides, nScheduled,
          nIntentionalSliders, nNonResponsive, nDead);
...
bool NextBotManager::ShouldUpdate(INextBot *bot) {
    ...
    if (frameLimit > 0.0f) {
      if (sumFrameTime < frameLimit) {
        ...
        Msg("Frame %8d/tick %8d: frame out of budget (%.2fms > %.2fms)\n",
            gpGlobals->framecount, gpGlobals->tickcount, sumFrameTime,
            frameLimit);
      }
    }
  }
  ...
  int nTicksSlid =
      (gpGlobals->tickcount - bot->GetTickLastUpdate()) - m_iUpdateTickrate;

  if (nTicksSlid >= nb_update_maxslide.GetInt()) {
    if (frameLimit == 0.0 ||
        sumFrameTime < nb_update_framelimit.GetFloat() * 2.0) {
      g_nBlockedSlides++;
      return true;
    }
  }
...
void NextBotManager::NotifyEndUpdate(INextBot *bot) {
  // This might be a good place to detect a particular bot had spiked [3/14/2008
  // tom]
 ...
 */
void NextBotManager::OnMapLoaded(void) { Reset(); }

//---------------------------------------------------------------------------------------------
/**
 * When the scenario restarts
 */
void NextBotManager::OnRoundRestart(void) { Reset(); }

//---------------------------------------------------------------------------------------------
int NextBotManager::Register(INextBot *bot) { return m_botList.AddToHead(bot); }
...
void NextBotManager::UnRegister(INextBot *bot) {
  ...
  if (bot == m_selectedBot) {
    // we can't access virtual methods because this is called from a destructor,
    // so just clear it
    m_selectedBot = NULL;
  }
}
...
public:
  NextBotKilledNotifyScan(CBaseCombatCharacter *victim,
 ...
 */
void NextBotManager::OnKilled(CBaseCombatCharacter *victim,
                              const CTakeDamageInfo &info) {
...
public:
  NextBotSoundNotifyScan(CBaseEntity *source, const Vector &pos,
                         KeyValues *keys)
 ...
 */
void NextBotManager::OnSound(CBaseEntity *source, const Vector &pos,
                             KeyValues *keys) {
  ...
  if (source && IsDebugging(NEXTBOT_HEARING)) {
    ...
    NDebugOverlay::Circle(pos, Vector(1, 0, 0), Vector(0, -1, 0), 5.0f, r, g, b,
                          255, true, 3.0f);
  }
}
...
public:
  NextBotResponseNotifyScan(CBaseCombatCharacter *who, AIConcept_t concept,
 ...
 */
void NextBotManager::OnSpokeConcept(CBaseCombatCharacter *who,
                                    AIConcept_t concept,
                                    AI_Response *response) {
  ...
  if (IsDebugging(NEXTBOT_HEARING)) {
    // const char *who = response->GetCriteria()->GetValue(
    // response->GetCriteria()->FindCriterionIndex( "Who" ) );

    // TODO: Need concept.GetStringConcept()
    DevMsg("%3.2f: OnSpokeConcept( %s, %s )\n", gpGlobals->curtime,
           who->GetDebugName(), "concept.GetStringConcept()");
  }
}
...
public:
  NextBotWeaponFiredNotifyScan(CBaseCombatCharacter *who,
 ...
 */
void NextBotManager::OnWeaponFired(CBaseCombatCharacter *whoFired,
                                   CBaseCombatWeapon *weapon) {
  ...
  if (IsDebugging(NEXTBOT_EVENTS)) {
    DevMsg("%3.2f: OnWeaponFired( %s, %s )\n", gpGlobals->curtime,
           whoFired->GetDebugName(), weapon->GetName());
  }
}
...
void NextBotManager::DebugFilterRemove(const char *name) {
  for (int i = 0; i < m_debugFilterList.Count(); ++i) {
    if (m_debugFilterList[i].name[0] != '\000' &&
        !Q_strnicmp(name, m_debugFilterList[i].name,
                    MIN(Q_strlen(name), sizeof(m_debugFilterList[i].name)))) {
      m_debugFilterList.Remove(i);
      break;
    }
  }
 ...
 */
void NextBotManager::DebugFilterClear(void) { m_debugFilterList.RemoveAll(); }
...
bool NextBotManager::IsDebugFilterMatch(const INextBot *bot) const {
  ...
  for (int i = 0; i < m_debugFilterList.Count(); ++i) {
    ...
    if (m_debugFilterList[i].index ==
        const_cast<INextBot *>(bot)->GetEntity()->entindex()) {
      return true;
    }

    // compare debug filter
    if (m_debugFilterList[i].name[0] != '\000' &&
        bot->IsDebugFilterMatch(m_debugFilterList[i].name)) {
      return true;
    }

    // compare special keyword meaning local player is looking at them
    if (!Q_strnicmp(m_debugFilterList[i].name, "lookat",
                    Q_strlen(m_debugFilterList[i].name))) {
    ...
    if (!Q_strnicmp(m_debugFilterList[i].name, "selected",
                    Q_strlen(m_debugFilterList[i].name))) {
...
INextBot *NextBotManager::GetBotUnderCrosshair(CBasePlayer *picker) {
  ...
  TargetScan<CBaseCombatCharacter> scan(picker, TEAM_ANY, 1.0f - MaxDot,
                                        MaxRange);
...
void NextBotManager::CollectAllBots(CUtlVector<INextBot *> *botVector) {
  ...
  for (int i = m_botList.Head(); i != m_botList.InvalidIndex();
       i = m_botList.Next(i)) {
