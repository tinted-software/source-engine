...
#pragma warning(disable : 4355) // warning 'this' used in base member
                                // initializer list - we're using it safely
...
void NextBotGroundLocomotion::Update(void) {
  ...
  // compute new position, taking into account MOTION_CONTROLLED animations in
  // progress
  ...
  // set actual velocity based on position change after collision resolution
  // step
  ...
  // collision resolution may create very high instantaneous velocities, limit
  // it
  ...
  if (IsOnGround()) {
    ...
    // we're falling. if our velocity has become zero for any reason, shove it
    // forward
  ...
  float maxLeanAngle =
      NextBotLeanMaxAngle.GetFloat() * m_actualSpeed / GetRunSpeed();
...
void NextBotGroundLocomotion::ApplyAccumulatedApproach(void) {
  ...
  /*
          ...
          float desiredLean = m_acceleration /
     NextBotLeanForwardAccel.GetFloat();

          QAngle lean = GetDesiredLean();

          lean.x = NextBotLeanMaxAngle.GetFloat() * clamp( desiredLean,
     -1.0f, 1.0f );
  ...
  */
  ...
  // if we just started a jump, don't snap to the ground - let us get in the air
  // first
  if (DidJustJump() || !IsOnGround()) {
    if (false &&
        m_isClimbingUpToLedge) // causes bots to hang in air stuck against edges
    {
...
public:
  GroundLocomotionCollisionTraceFilter(INextBot *me,
 ...
 */
bool NextBotGroundLocomotion::DetectCollision(
    trace_t *pTrace, int &recursionLimit, const Vector &from, const Vector &to,
    const Vector &vecMins, const Vector &vecMaxs) {
  IBody *body = GetBot()->GetBodyInterface();

  CBaseEntity *ignore =
      m_ignorePhysicsPropTimer.IsElapsed() ? NULL : m_ignorePhysicsProp;
  GroundLocomotionCollisionTraceFilter filter(GetBot(), ignore,
  ...
  if (pTrace->DidHitNonWorldEntity() && pTrace->m_pEnt != NULL) {
    ...
    if (!other->MyCombatCharacterPointer() &&
        IsEntityTraversable(other, IMMEDIATELY) /*&& IsFlimsy( other )*/) {
      if (recursionLimit <= 0)
        return true;
      ...
      CTakeDamageInfo damageInfo(GetBot()->GetEntity(), GetBot()->GetEntity(),
                                 100.0f, DMG_CRUSH);
      CalculateExplosiveDamageForce(&damageInfo, GetMotionVector(),
                                    pTrace->endpos);
      other->TakeDamage(damageInfo);

      // retry trace now that the breakable is out of the way
      return DetectCollision(pTrace, recursionLimit, from, to, vecMins,
                             vecMaxs);
    }
  }
...
//----------------------------------------------------------------------------------------------------------
Vector NextBotGroundLocomotion::ResolveCollision(const Vector &from,
                                                 const Vector &to,
                                                 int recursionLimit) {
  ...
  if (m_bRecomputePostureOnCollision) {
    if (!body->IsActualPosture(IBody::STAND) &&
        !body->IsActualPosture(IBody::CROUCH)) {
      m_bRecomputePostureOnCollision = false;
    }
  }
  ...
  while (true) {
    bool bCollided =
        DetectCollision(&trace, recursionLimit, from, desiredGoal, mins, maxs);
    if (!bCollided) {
    ...
    // Also don't bother checking for crouch if we hit a plane that pushes us
    // upwards
    if (bPerformCrouchTest) {
      ...
      if (!trace.m_pEnt->MyNextBotPointer() && !trace.m_pEnt->IsPlayer()) {
        ...
        if (nPosture != IBody::CROUCH) {
          trace_t crouchTrace;
          NextBotTraversableTraceFilter crouchFilter(GetBot(),
                                                     ILocomotion::IMMEDIATELY);
          Vector vecCrouchMax(maxs.x, maxs.y, body->GetCrouchHullHeight());
          TraceHull(from, desiredGoal, mins, vecCrouchMax, body->GetSolidMask(),
                    &crouchFilter, &crouchTrace);
        ...
        // is currently crouching, *and* his first trace (with the standing
        // hull) hits an actor *and* if he didn't hit that actor, he could have
        // moved standing the entire way to his desired endpoint
        trace_t standTrace;
        NextBotTraversableTraceFilter standFilter(GetBot(),
                                                  ILocomotion::IMMEDIATELY);
        TraceHull(from, desiredGoal, mins, maxs, body->GetSolidMask(),
                  &standFilter, &standTrace);
      ...
      if (trace.m_pEnt && !trace.m_pEnt->IsWorld()) {
        ...
        if (dynamic_cast<CPhysicsProp *>(trace.m_pEnt) != NULL &&
          ...
          if (physics && physics->IsMoveable()) {
            // we've intersected a (likely moving) physics prop - ignore it for
            // awhile so we can move out of it
    ...
    if (GetBot()->IsDebugging(NEXTBOT_LOCOMOTION)) {
      NDebugOverlay::Line(trace.endpos,
                          trace.endpos + 20.0f * trace.plane.normal, 255, 0,
                          150, true, 15.0f);
    }
...
public:
  ClosestActorsScan(const Vector &spot, int team, float maxRange = 0.0f,
  ...
  CBaseCombatCharacter *GetActor(void) const { return m_close; }

  bool IsCloserThan(float range) { return (m_closeRangeSq < (range * range)); }

  bool IsFartherThan(float range) { return (m_closeRangeSq > (range * range)); }
...
Vector NextBotGroundLocomotion::ResolveZombieCollisions(const Vector &pos) {
  ...
  if (me != NULL && !IsUsingLadder() &&
      !IsClimbingOrJumping() && IsOnGround() && m_nextBot->IsAlive() && IsAttemptingToMove() /*&& GetBot()->GetBodyInterface()->IsArousal( IBody::INTENSE )*/) {
...
void NextBotGroundLocomotion::UpdatePosition(const Vector &newPos) {
  ...
  if (NextBotStop.GetBool() || (m_nextBot->GetFlags() & FL_FROZEN) != 0 ||
      newPos == m_nextBot->GetPosition()) {
    return;
  }

  // avoid very nearby Actors to simulate "mushy" collisions between actors in
  ...
  Vector safePos = ResolveCollision(m_nextBot->GetPosition(), adjustedNewPos,
                                    recursionLimit);

  // set the bot's position
  if (GetBot()->GetIntentionInterface()->IsPositionAllowed(GetBot(), safePos) !=
      ANSWER_NO) {
...
//----------------------------------------------------------------------------------------------------------
/**
 * Prevent bot from sliding through floor, and snap to the ground if we're very
 * near it
 */
void NextBotGroundLocomotion::UpdateGroundConstraint(void) {
  VPROF_BUDGET("NextBotGroundLocomotion::UpdateGroundConstraint",
               "NextBotExpensive");

  // if we're up on the upward arc of our jump, don't interfere by snapping to
  // ground don't do ground constraint if we're climbing a ladder
  ...
  // since we only care about ground collisions, keep hull short to avoid issues
  // with low ceilings
  /// @TODO: We need to also check actual hull height to avoid interpenetrating
  /// the world
  float hullHeight = GetStepHeight();

  // always need tolerance even when jumping/falling to make sure we detect
  // ground penetration must be at least step height to avoid 'falling' down
  // stairs
  ...
  TraceHull(m_nextBot->GetPosition() + Vector(0, 0, GetStepHeight() + 0.001f),
            ...
            Vector(halfWidth, halfWidth, hullHeight), body->GetSolidMask(),
            &filter, &ground);

  if (ground.startsolid) {
    // we're inside the ground - bad news
    if (GetBot()->IsDebugging(NEXTBOT_LOCOMOTION) &&
        !(gpGlobals->framecount % 60)) {
      DevMsg("%3.2f: Inside ground, ( %.0f, %.0f, %.0f )\n", gpGlobals->curtime,
             m_nextBot->GetPosition().x, m_nextBot->GetPosition().y,
             m_nextBot->GetPosition().z);
    }
    return;
  }
  ...
  if (ground.fraction < 1.0f) {
    ...
    if (ground.plane.normal.z < GetTraversableSlopeLimit()) {
      ...
      if ((m_velocity.x * ground.plane.normal.x +
           m_velocity.y * ground.plane.normal.y) <= 0.0f) {
      ...
      if (GetBot()->IsDebugging(NEXTBOT_LOCOMOTION)) {
        DevMsg("%3.2f: NextBotGroundLocomotion - Too steep to stand here\n",
               gpGlobals->curtime);
        NDebugOverlay::Line(GetFeet(), GetFeet() + 20.0f * ground.plane.normal,
                            255, 150, 0, true, 5.0f);
      }
    ...
    if (IsOnGround()) {
      ...
      if (!IsClimbingUpToLedge() && !IsJumpingAcrossGap()) {
        m_isUsingFullFeetTrace = true;    // We're in the air and there's space
                                          // below us, so use the full trace
...
void NextBotGroundLocomotion::StandUp( void )
{
        ...
        TraceHull( GetFeet(), GetFeet(), standHullMin, standHullMax,
MASK_NPCSOLID, m_nextBot, MASK_DEFAULTPLAYERSOLID, &result );
 ...
 */
bool NextBotGroundLocomotion::ClimbUpToLedge(const Vector &landingGoal,
 ...
 */
void NextBotGroundLocomotion::JumpAcrossGap(const Vector &landingGoal,
                                            const Vector &landingForward) {
  ...
  // ballistic equation to find initial velocity assuming 45 degree inclination
  // and landing at give range and height
  float launchVel =
      (range / cos45) / sqrt((2.0f * (range - height)) / GetGravity());
 ...
 */
void NextBotGroundLocomotion::Run(void) { m_desiredSpeed = GetRunSpeed(); }

//----------------------------------------------------------------------------------------------------------
/**
 * Set movement speed to walking
 */
void NextBotGroundLocomotion::Walk(void) { m_desiredSpeed = GetWalkSpeed(); }

//----------------------------------------------------------------------------------------------------------
/**
 * Set movement speed to stopeed
 */
void NextBotGroundLocomotion::Stop(void) { m_desiredSpeed = 0.0f; }
 ...
 */
void NextBotGroundLocomotion::ClimbLadder(const CNavLadder *ladder,
  ...
  if (body) {
    ...
    Vector mountSpot = m_ladder->m_bottom +
                       m_ladder->GetNormal() * (0.75f * body->GetHullWidth());
 ...
 */
void NextBotGroundLocomotion::DescendLadder(const CNavLadder *ladder,
                                            const CNavArea *dismountGoal) {
  ...
  if (body) {
    // line them up to climb in XY
    Vector mountSpot = m_ladder->m_top +
                       m_ladder->GetNormal() * (0.75f * body->GetHullWidth());
    mountSpot.z = GetBot()->GetPosition().z;
 ...
 */
void NextBotGroundLocomotion::OnMoveToFailure(const Path *path,
