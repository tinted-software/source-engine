...
ConVar sv_maxusrcmdprocessticks_warning(
    "sv_maxusrcmdprocessticks_warning", "-1", FCVAR_NONE,
...
void CPlayerMove::FinishMove(CBasePlayer *player, CUserCmd *ucmd,
  ...
  // NOTE: Don't copy this.  the movement code modifies its local copy but is
  // not expecting to be authoritative
...
void CPlayerMove::RunCommand(CBasePlayer *player, CUserCmd *ucmd,
  ...
  const float flTimeAllowedForProcessing =
      player->ConsumeMovementTimeForUserCmdProcessing(playerFrameTime);
  if (!player->IsBot() && (flTimeAllowedForProcessing < playerFrameTime)) {
    ...
    double dblWarningFrequencyThrottle =
        sv_maxusrcmdprocessticks_warning.GetFloat();
    if (dblWarningFrequencyThrottle >= 0) {
      static double s_dblLastWarningTime = 0;
      double dblTimeNow = Plat_FloatTime();
      if (!s_dblLastWarningTime ||
          (dblTimeNow - s_dblLastWarningTime >= dblWarningFrequencyThrottle)) {
        s_dblLastWarningTime = dblTimeNow;
        Warning("sv_maxusrcmdprocessticks_warning at server tick %u: Ignored "
                "client %s usrcmd (%.6f < %.6f)!\n",
                gpGlobals->tickcount, player->GetPlayerName(),
                flTimeAllowedForProcessing, playerFrameTime);
      }
    }
  ...
  // Prevent hacked clients from sending us invalid view angles to try to get
  // leaf server code to crash
  if (!ucmd->viewangles.IsValid() ||
      !IsEntityQAngleReasonable(ucmd->viewangles)) {
