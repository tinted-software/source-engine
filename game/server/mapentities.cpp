...
//
// Purpose: Controls the loading, parsing and creation of the entities from the
// BSP.
//
//=============================================================================//
...
CBaseEntity *CreateEntityByName(const char *className, int iForceEdictIndex) {
  if (iForceEdictIndex != -1) {
    ...
    if (!g_pForceAttachEdict)
      Error("CreateEntityByName( %s, %d ) - CreateEdict failed.", className,
            iForceEdictIndex);
  }
...
//-----------------------------------------------------------------------------
static int __cdecl CompareSpawnOrder(HierarchicalSpawn_t *pEnt1,
                                     HierarchicalSpawn_t *pEnt2) {
  if (pEnt1->m_nDepth == pEnt2->m_nDepth) {
    if (g_pClassnameSpawnPriority) {
      int o1 = pEnt1->m_pEntity ? g_pClassnameSpawnPriority->GetStringID(
                                      pEnt1->m_pEntity->GetClassname())
                                : -1;
      int o2 = pEnt2->m_pEntity ? g_pClassnameSpawnPriority->GetStringID(
                                      pEnt2->m_pEntity->GetClassname())
                                : -1;
      if (o1 < o2)
        return 1;
      if (o2 < o1)
...
static int ComputeSpawnHierarchyDepth_r(CBaseEntity *pEntity) {
  ...
  CBaseEntity *pParent =
      gEntList.FindEntityByName(NULL, ExtractParentName(pEntity->m_iParent));
  if (!pParent)
    return 1;
...
static void ComputeSpawnHierarchyDepth(int nEntities,
                                       HierarchicalSpawn_t *pSpawnList) {
  // NOTE: This isn't particularly efficient, but so what? It's at the beginning
  // of time I did it this way because it simplified the parent setting in
  // hierarchy (basically eliminated questions about whether you should
  // transform origin from global to local or not)
  int nEntity;
...
static void SortSpawnListByHierarchy(int nEntities,
                                     HierarchicalSpawn_t *pSpawnList) {
  MEM_ALLOC_CREDIT();
  ...
  g_pClassnameSpawnPriority->AddString(
      "info_mass_center", 8); // spawn these before physbox/prop_physics
  g_pClassnameSpawnPriority->AddString(
      "trigger_vphysics_motion", 8); // spawn these before physbox/prop_physics

  g_pClassnameSpawnPriority->AddString("prop_physics", 7);
  g_pClassnameSpawnPriority->AddString("prop_ragdoll", 7);
  // Sort the entities (other than the world) by hierarchy depth, in order to
  // spawn them in that order. This insures that each entity's parent spawns
  // before it does so that it can properly set up anything that relies on
  // hierarchy.
#ifdef _WIN32
...
#elif POSIX
  qsort(&pSpawnList[0], nEntities, sizeof(pSpawnList[0]),
        (int (*)(const void *, const void *))CompareSpawnOrder);
#endif
    ...
    if (pEntity) {
      if (strchr(STRING(pEntity->m_iParent), ',')) {
        ...
        const char *pAttachmentName =
            nexttoken(szToken, STRING(pEntity->m_iParent), ',');
        pEntity->m_iParent = AllocPooledString(szToken);
        CBaseEntity *pParent =
            gEntList.FindEntityByName(NULL, pEntity->m_iParent);

        // setparent in the spawn pass instead - so the model will have been set
        // & loaded
...
void RememberInitialEntityPositions(int nEntities,
                                    HierarchicalSpawn_t *pSpawnList) {
...
void SpawnAllEntities(int nEntities, HierarchicalSpawn_t *pSpawnList,
  ...
  for (nEntity = 0; nEntity < nEntities; nEntity++) {
    ...
    if (pSpawnList[nEntity].m_pDeferredParent) {
      ...
      if (pAnim) {
        iAttachment = pAnim->LookupAttachment(
            pSpawnList[nEntity].m_pDeferredParentAttachment);
      }
      ...
      if (DispatchSpawn(pEntity) < 0) {
        for (int i = nEntity + 1; i < nEntities; i++) {
          ...
          if (pSpawnList[i].m_pEntity &&
              pSpawnList[i].m_pEntity->IsMarkedForDeletion()) {
...
//-----------------------------------------------------------------------------
// Purpose: Only called on BSP load. Parses and spawns all the entities in the
// BSP. Input  : pMapData - Pointer to the entity data block to parse.
//-----------------------------------------------------------------------------
void MapEntity_ParseAllEntities(const char *pMapData, IMapEntityFilter *pFilter,
                                bool bActivateEntities) {
  VPROF("MapEntity_ParseAllEntities");

  HierarchicalSpawnMapData_t *pSpawnMapData =
      new HierarchicalSpawnMapData_t[NUM_ENT_ENTRIES];
  ...
  for (; true; pMapData = MapEntity_SkipToNextEntity(pMapData, szTokenBuffer)) {
    ...
    if (pEntity->IsTemplate()) {
      ...
      // Remove the template entity so that it does not show up in FindEntityXXX
      // searches.
      UTIL_Remove(pEntity);
      gEntList.CleanupDeleteList();
      continue;
    ...
    if (dynamic_cast<CWorld *>(pEntity)) {
      ...
      pEntity->m_iParent =
          NULL_STRING; // don't allow a parent on the first entity (worldspawn)
    ...
    if (pNode) {
      ...
      // solution prevents nodes & lights from being used inside
      // point_templates.
      //
      // NOTE: Nodes spawn other entities (ai_hint) if they need to have a
      // persistent presence.
      //		 To ensure keys are copied over into the new entity, we
      //pass the mapdata into the 		 node spawn function.
    ...
    // Build a list of all point_template's so we can spawn them before
    // everything else
  ...
  // Now loop through all our point_template entities and tell them to make
  // templates of everything they're pointing to
    ...
    for (int iTemplateNum = 0; iTemplateNum < iNumTemplates; iTemplateNum++) {
      ...
      for (int iEntNum = 0; iEntNum < nEntities; iEntNum++) {
        if (pSpawnList[iEntNum].m_pEntity == pEntity) {
          ...
          pPointTemplate->AddTemplate(pEntity,
                                      pSpawnMapData[iEntNum].m_pMapData,
                                      pSpawnMapData[iEntNum].m_iMapDataLength);

          if (pPointTemplate->ShouldRemoveTemplateEntities()) {
            // Remove the template entity so that it does not show up in
            // FindEntityXXX searches.
            UTIL_Remove(pEntity);
            gEntList.CleanupDeleteList();
...
void SpawnHierarchicalList(int nEntities, HierarchicalSpawn_t *pSpawnList,
  ...
  // Set up entity movement hierarchy in reverse hierarchy depth order. This
  // allows each entity to use its parent's world spawn origin to calculate its
  // local origin.
  SetupParentsForSpawnList(nEntities, pSpawnList);

  // Spawn all the entities in hierarchy depth order so that parents spawn
...
void MapEntity_PrecacheEntity(const char *pEntData, int &nStringSize) {
  ...
  //
  // Set up keyvalues, which can set the model name, which is why we don't just
  // do UTIL_PrecacheOther here...
  //
  if (pEntity != NULL) {
