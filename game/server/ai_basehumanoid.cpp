...
//-----------------------------------------------------------------------------
bool CAI_BaseHumanoid::HandleInteraction(int interactionType, void *data,
                                         CBaseCombatCharacter *sourceEnt) {
#ifdef HL2_DLL
  // Annoying to ifdef this out. Copy it into all the HL2 specific humanoid
  // NPC's instead?
  if (interactionType == g_interactionBarnacleVictimDangle) {
    ...
    // Destroy the entity, the barnacle is going to use the ragdoll that it is
    // releasing as the corpse.
    UTIL_Remove(this);
    return true;
  }
#endif
    ...
    if (!GetActiveWeapon()->HasPrimaryAmmo()) {
      SetCondition(COND_NO_PRIMARY_AMMO);
               ...
               GetActiveWeapon()->Clip1() <
                   (GetActiveWeapon()->GetMaxClip1() / 4 + 1)) {
...
bool CAI_BaseHumanoid::OnMoveBlocked(AIMoveResult_t *pResult) {
  if (*pResult != AIMR_BLOCKED_NPC && GetNavigator()->GetBlockingEntity() &&
    ...
    if (pBlocker->GetMoveType() == MOVETYPE_VPHYSICS &&
        pBlocker != GetGroundEntity() && !pBlocker->IsNavIgnored() &&
        ...
        (pBlocker->VPhysicsGetObject()->GetMass() <= 35.0 + massBonus + 0.1 ||
         (pBlocker->VPhysicsGetObject()->GetMass() <= 50.0 + massBonus + 0.1 &&
          IsSmall(pBlocker)))) {
...
void CAI_BaseHumanoid::TraceAttack(const CTakeDamageInfo &info,
  ...
  if (ptr->hitgroup == HITGROUP_HEAD) {
    if (info.GetAttacker() && info.GetAttacker()->IsPlayer() &&
        info.GetAttacker() != GetEnemy() && !IsInAScript()) {
      // Shot in the head by a player I've never seen. In this case the player
      // has gotten the drop on this enemy and such an attack is always lethal
      // (at close range)
      bSneakAttacked = true;

      AIEnemiesIter_t iter;
      for (AI_EnemyInfo_t *pMemory = GetEnemies()->GetFirst(&iter);
           pMemory != NULL; pMemory = GetEnemies()->GetNext(&iter)) {
