...
//	  or must have bits_CAP_INNATE_RANGE_ATTACK1 or
...
void CAI_MoveAndShootOverlay::NoShootWhileMove() { m_bNoShootWhileMove = true; }
...
void CAI_MoveAndShootOverlay::StartShootWhileMove() {
  if (GetOuter()->GetState() == NPC_STATE_SCRIPT ||
      ...
      !GetOuter()->HaveSequenceForActivity(
          GetOuter()->TranslateActivity(ACT_WALK_AIM)) ||
      !GetOuter()->HaveSequenceForActivity(
          GetOuter()->TranslateActivity(ACT_RUN_AIM))) {
    NoShootWhileMove();
    return;
  }

  GetOuter()->GetShotRegulator()->FireNoEarlierThan(gpGlobals->curtime +
                                                    m_initialDelay);
  m_bNoShootWhileMove = false;
}
...
void CAI_MoveAndShootOverlay::RunShootWhileMove() {
  ...
  if (GetEnemy() == NULL) {
    if (pOuter->GetAlternateMoveShootTarget()) {
      ...
      pOuter->AddFacingTarget(
          pOuter->GetAlternateMoveShootTarget(),
  ...
  Assert(
      HasAvailableRangeAttack()); // This should have been caught at task start

  Activity activity;
  bool bIsReloading = false;

  if ((activity = pOuter->TranslateActivity(ACT_GESTURE_RELOAD)) !=
      ACT_INVALID) {
    ...
    if (pOuter->HasCondition(COND_CAN_RANGE_ATTACK1, false)) {
      if (pOuter->GetShotRegulator()->IsInRestInterval()) {
        ...
        if (m_bMovingAndShooting || pOuter->OnBeginMoveAndShoot()) {
          ...
          pOuter->Weapon_SetActivity(
              pOuter->Weapon_TranslateActivity(ACT_RANGE_ATTACK1), 0);
        }
      }
      ...
      if (pOuter->GetNavigator()->GetPathTimeToGoal() > 1.0) {
        ...
        if (activity != ACT_INVALID &&
            GetOuter()->HaveSequenceForActivity(activity))
...
void CAI_MoveAndShootOverlay::EndShootWhileMove() {
  if (m_bMovingAndShooting) {
    // Reset the shot regulator so that we always start the next motion with a
    // new burst
