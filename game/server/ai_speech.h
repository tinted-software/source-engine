...
public:
  ...
  // These two methods allow looking up a response and dispatching it to be two
  // different steps
...
private:
  ...
  float m_flStopTalkTime; // when in the future that I'll be done saying this
                          // sentence.
  float m_flStopTalkTimeWithoutDelay; // same as the above, but minus the delay
                                      // before other people can speak
...
//-----------------------------------------------------------------------------
extern void
CAI_ExpresserHost_NPC_DoModifyOrAppendCriteria(CAI_BaseNPC *pSpeaker,
...
//-----------------------------------------------------------------------------
template <class BASE_NPC>
inline AI_Response *CAI_ExpresserHost<BASE_NPC>::SpeakFindResponse(
    AIConcept_t concept, const char *modifiers /*= NULL*/) {
...
template <class BASE_NPC>
inline bool
CAI_ExpresserHost<BASE_NPC>::SpeakDispatchResponse(AIConcept_t concept,
                                                   AI_Response *response) {
...
template <class BASE_NPC>
inline float
CAI_ExpresserHost<BASE_NPC>::GetResponseDuration(AI_Response *response) {
...
template <class BASE_NPC>
inline void
CAI_ExpresserHost<BASE_NPC>::DispatchResponse(const char *conceptName) {
