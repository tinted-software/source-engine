...
bool CAI_BaseActor::ProcessSceneEvent(CSceneEventInfo *info,
  ...
  case CChoreoEvent::FACE: {
    ...
    // scripted_sequences.  By allowing torso turns during post-idles,
    // pre-idles, act-busy's, scripted_sequences, normal AI movements, etc., it
    // increases the functionality of those AI features without breaking their
    // assuptions that the entity won't be made to "turn" by something outside
    // of those AI's control. lockbody facing is also usefull when npcs are
    // moving and you want them to turn towards something but still walk in the
    // direction of travel.
    ...
    255, 255, 0, true, 0.1 ); NDebugOverlay::YawArrow( GetAbsOrigin() + Vector(
    0, 0, 8 ), GetLocalAngles().y + m_goalBodyYaw, 64, 16, 255, 128, 128, 0,
    true, 0.1 ); NDebugOverlay::YawArrow( GetAbsOrigin() + Vector( 0, 0, 16 ),
    GetLocalAngles().y + m_goalSpineYaw, 64, 16, 128, 255, 128, 0, true, 0.1 );
    NDebugOverlay::YawArrow( GetAbsOrigin() + Vector( 0, 0, 24 ),
...
static ConVar
    scene_clamplookat("scene_clamplookat", "1", FCVAR_NONE,
...
void CAI_BaseActor::UpdateHeadControl(const Vector &vHeadTarget,
  ...
  if (scene_clamplookat.GetBool()) {
    ...
    flHeadInfluence =
        flHeadInfluence * clamp(vTargetLocal.x * 2.0f + 2.0f, 0.0f, 1.0f);
  }
  ...
  if (m_hLookTarget != hTarget) {
    ...
    if ((m_debugOverlays & OVERLAY_NPC_SELECTED_BIT) &&
      ...
      if (m_hLookTarget != this) {
        ...
        NDebugOverlay::Text(vecEyePos,
                            UTIL_VarArgs("%s (%s)",
                                         m_hLookTarget->GetClassname(),
