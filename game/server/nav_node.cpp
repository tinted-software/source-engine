...
public:
  ...
  bool operator()(const CNavNode *pLhs, const CNavNode *pRhs) const {
    return pRhs->GetPosition()->AsVector2D() ==
           pLhs->GetPosition()->AsVector2D();
  }
...
CNavNode::CNavNode(const Vector &pos, const Vector &normal, CNavNode *parent,
  ...
  if (!g_pNavNodeHash) {
    g_pNavNodeHash =
        new CUtlHash<CNavNode *, CNodeHashFuncs, CNodeHashFuncs>(16 * 1024);
  }
...
void CNavNode::Draw(void) {
  ...
  if (GetAttributes() & NAV_MESH_CROUCH) {
    ...
    for (i = 0; i < NUM_CORNERS; i++) {
      if (m_isBlocked[i] || m_crouch[i]) {
        ...
        if (m_isBlocked[i]) {
          NDebugOverlay::HorzArrow(m_pos, m_pos + scaled, 0.5, 255, 0, 0, 255,
                                   true, 0.1f);
        } else {
  ...
  if (nav_show_node_grid.GetBool()) {
    for (int i = NORTH; i < NUM_DIRECTIONS; i++) {
      ...
      if (nodeNext) {
        NDebugOverlay::Line(*GetPosition(), *nodeNext->GetPosition(), 255, 255,
                            0, false, 0.1f);
...
//--------------------------------------------------------------------------------------------------------
// return ground height above node in given corner direction (NUM_CORNERS for
// highest in any direction)
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Look up to JumpCrouchHeight in the air to see if we can fit a whole
 * HumanHeight box
 */
bool CNavNode::TestForCrouchArea(NavCornerType cornerNum, const Vector &mins,
                                 const Vector &maxs,
  ...
  for (float height = 0; height <= maxHeight; height += 1.0f) {
    ...
    UTIL_TraceHull(start, start, mins, realMaxs, MASK_NPCSOLID_BRUSHONLY,
                   &filter, &tr);
    if (!tr.startsolid) {
 ...
 */
void CNavNode::ConnectTo(CNavNode *node, NavDirType dir, float obstacleHeight,
...
CNavNode *CNavNode::GetNode(const Vector &pos) {
  ...
  if (g_pNavNodeHash) {
    ...
    if (hNode != g_pNavNodeHash->InvalidHandle()) {
      for (pNode = g_pNavNodeHash->Element(hNode); pNode;
           pNode = pNode->m_nextAtXY) {
