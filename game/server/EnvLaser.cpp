...
DEFINE_KEYFIELD(m_iszLaserTarget, FIELD_STRING, "LaserTarget"),
  ...
  if (!m_pSprite && m_iszSpriteName != NULL_STRING) {
    m_pSprite =
        CSprite::SpriteCreate(STRING(m_iszSpriteName), GetAbsOrigin(), TRUE);
  } else {
    m_pSprite = NULL;
  }
  ...
  if (m_pSprite) {
    ...
    m_pSprite->SetTransparency(kRenderGlow, m_clrRender->r, m_clrRender->g,
...
void CEnvLaser::StrikeThink(void) {
  ...
  UTIL_TraceLine(GetAbsOrigin(), vecFireAt, MASK_SOLID, NULL,
