...
DEFINE_KEYFIELD(m_bNegated, FIELD_BOOLEAN, "Negated"),
    ...
    bool CBaseFilter::PassesFilterImpl(CBaseEntity *pCaller,
                                       CBaseEntity *pEntity) {
  return true;
}
...
//			filter test, the OnPass output is fired. If not, the
//OnFail output is fired.
//-----------------------------------------------------------------------------
...
DEFINE_KEYFIELD(m_nFilterType, FIELD_INTEGER, "FilterType"),
  ...
  for (int i = 0; i < MAX_FILTERS; i++) {
    if (m_iFilterName[i] != NULL_STRING) {
      ...
      if (pFilter == NULL) {
        Warning("filter_multi: Tried to add entity (%s) which is not a filter "
                "entity!\n",
