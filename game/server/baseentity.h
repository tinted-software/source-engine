...
public:
  static bool
      sm_bAccurateTriggerBboxChecks; // SOLID_BBOX entities do a fully accurate
                                     // trigger vs bbox check when this is set
...
public:
  ...
  CNetworkArray(int, m_nModelIndexOverrides,
                MAX_VISION_MODES); // used to override the base model index on
  ...
  CNetworkVar(float,
              m_flAnimTime); // this is the point in time that the client will
                             // interpolate to position,angle,frame,etc.
...
public:
  ...
  void DispatchTraceAttack(const CTakeDamageInfo &info, const Vector &vecDir,
                           trace_t *ptr, CDmgAccumulator *pAccumulator = NULL);
...
public:
  ...
  virtual bool IsViewable(
      void); // is this something that would be looked at (model, sprite, etc.)?
  ...
  bool InSameTeam(
      CBaseEntity *pEntity) const; // Returns true if the specified entity is on
                                   // the same team as this one
  bool IsInAnyTeam(void) const;    // Returns true if this entity is in any team
  const char *
  TeamID(void) const; // Returns the name of the team this entity is on.
...
public:
  ...
  // Damage filtering
  string_t m_iszDamageFilterName; // The name of the entity to use as our damage
                                  // filter.
  ...
  //	void			SetAbsAngularVelocity( const QAngle
  //&vecAngVelocity ); 	const QAngle&	GetAbsAngularVelocity( ) const;
  ...
  void EmitSound(
      ...
      float *duration =
          NULL); // Override for doing the general case of CPASAttenuationFilter
                 // filter( this ), and EmitSound( filter, entindex(), etc. );
  void EmitSound(
      const char *soundname, HSOUNDSCRIPTHANDLE &handle, float soundtime = 0.0f,
      float *duration =
          NULL); // Override for doing the general case of CPASAttenuationFilter
                 // filter( this ), and EmitSound( filter, entindex(), etc. );
...
private:
  ...
  IPhysicsObject *
      m_pPhysicsObject; // pointer to the entity's physics object (vphysics.dll)
...
//			is in single step mode, this is called every time an
//output is fired.
...
inline const QAngle &
CBaseEntity::EyeAngles(void) const // Direction of eyes in world space
{
  return const_cast<CBaseEntity *>(this)->EyeAngles();
}

inline const QAngle &
CBaseEntity::LocalEyeAngles(void) const // Direction of eyes
{
