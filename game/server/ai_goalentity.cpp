...
DEFINE_KEYFIELD(m_iszActor, FIELD_STRING, "Actor"),
    ...
    DEFINE_KEYFIELD(m_iszConceptModifiers, FIELD_STRING,
...
void CAI_GoalEntity::PruneActors() {
  for (int i = m_actors.Count() - 1; i >= 0; i--) {
    if (m_actors[i] == NULL || m_actors[i]->IsMarkedForDeletion() ||
        m_actors[i]->GetState() == NPC_STATE_DEAD)
...
void CAI_GoalEntity::InputUpdateActors(inputdata_t &inputdata) {
  ...
  for (index = prevActors.FirstInorder(); index != prevActors.InvalidIndex();
       index = prevActors.NextInorder(index)) {
    if (m_flags & ACTIVE)
