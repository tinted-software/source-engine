...
DEFINE_FIELD(timeSpoken, FIELD_TIME), // Relative to server time
    // DEFINE_EMBEDDED( response,	FIELD_??? ),	// This is manually
    // saved/restored by the ConceptHistory saverestore ops below
    END_DATADESC()
...
CAI_Expresser::CAI_Expresser(CBaseFlex *pOuter)
    : m_pOuter(pOuter), m_pSink(NULL), m_flStopTalkTime(0),
      m_flLastTimeAcceptedSpeak(0), m_flBlockedTalkTime(0),
      m_flStopTalkTimeWithoutDelay(0), m_voicePitch(100) {
#ifdef DEBUG
  g_nExpressers++;
#endif
}
...
//-----------------------------------------------------------------------------
AI_Response *
CAI_Expresser::SpeakFindResponse(AIConcept_t concept,
...
bool CAI_Expresser::SpeakDispatchResponse(
  ...
  if (IsSpeaking() && concept[0] != 0) {
    DevMsg("SpeakDispatchResponse:  Entity ( %i/%s ) already speaking, forcing "
           "'%s'\n",
    ...
    if (IsRunningScriptedScene(GetOuter())) {
      DevMsg("SpeakDispatchResponse:  Entity ( %i/%s ) refusing to speak due "
             "to scene entity, tossing '%s'\n",
             GetOuter()->entindex(), STRING(GetOuter()->GetEntityName()),
             concept);
      delete result;
      return false;
...
//and won't 			return true until that delay time has passed after finishing the
