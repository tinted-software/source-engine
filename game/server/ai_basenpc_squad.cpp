...
bool CAI_BaseNPC::OccupyStrategySlotRange(int slotIDStart, int slotIDEnd) {
  ...
  return (!m_pSquad ||
          m_pSquad->OccupyStrategySlotRange(GetEnemy(), slotIDStart, slotIDEnd,
                                            &m_iMySquadSlot));
}
...
bool CAI_BaseNPC::IsStrategySlotRangeOccupied(int slotIDStart, int slotIDEnd) {
  return m_pSquad && m_pSquad->IsStrategySlotRangeOccupied(
                         GetEnemy(), slotIDStart, slotIDEnd);
}
...
bool CAI_BaseNPC::IsSquadmateInSpread(const Vector &sourcePos,
  ...
  while (pSquadmate) {
    // Ignore squadmates that can't take damage. This is primarily to ignore
    // npc_enemyfinders.
    if (pSquadmate->m_takedamage != DAMAGE_NO) {
      if (pSquadmate != this) {
        if (PointInSpread(pSquadmate, sourcePos, targetPos,
                          pSquadmate->GetAbsOrigin(), flSpread,
                          maxDistOffCenter))
          return true;
      }
    }
...
void CAI_BaseNPC::CheckSquad() {
  ...
  if (VPhysicsGetObject() != NULL &&
    ...
    // I AM the leader, and I'm currently being held. This will screw up all of
    // my relationship checks if I'm a manhack or a rollermine, so just bomb out
    // and try next time.
    return;
  }
  ...
  while (pSquadmate) {
    if (IRelationType(pSquadmate) < D_LI) {
      ...
      // Rollermines and manhacks set their Class to NONE when held by the
      // player, which makes all of their squadmates complain that an enemy is
      // in the squad. Suppress this.
      if (pSquadmate->VPhysicsGetObject() != NULL) {
        if (pSquadmate->VPhysicsGetObject()->GetGameFlags() &
...
int CAI_BaseNPC::NumWeaponsInSquad(const char *pszWeaponClassname) {
  ...
  if (!GetSquad()) {
    if (GetActiveWeapon() &&
        GetActiveWeapon()->m_iClassname == iszWeaponClassname) {
  ...
  while (pSquadmate) {
    if (pSquadmate->GetActiveWeapon() &&
        pSquadmate->GetActiveWeapon()->m_iClassname == iszWeaponClassname) {
      count++;
    }
    pSquadmate = m_pSquad->GetNextMember(&iter);
  }
