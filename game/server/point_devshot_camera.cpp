...
//			dev screenshots everytime the map is checked into source
//control.
//
// $NoKeywords: $
//=============================================================================//
...
#define DEVSHOT_INITIAL_WAIT                                                   \
  5 // Time after the level spawn before the first devshot camera takes it's
    // shot
...
DEFINE_FUNCTION(DevShotThink_Setup), DEFINE_FUNCTION(DevShotThink_TakeShot),
...
void CPointDevShotCamera::Spawn(void) {
  ...
  SetNextThink(gpGlobals->curtime + DEVSHOT_INITIAL_WAIT +
               (g_iDevShotCameraCount * DEVSHOT_INTERVAL));
...
void CPointDevShotCamera::DevShotThink_TakeShot(void) {
  ...
  engine->ClientCommand(pPlayer->edict(), "devshots_screenshot \"%s\"",
                        STRING(m_iszCameraName));
...
public:
  ...
  virtual void FrameUpdatePostEntityThink(void) {
    ...
    if (!m_bParsedMapFile) {
      ...
      char szFullName[512];
      Q_snprintf(szFullName, sizeof(szFullName), "maps/%s.txt",
      ...
      if (pkvMapCameras->LoadFromFile(filesystem, szFullName, "MOD")) {
        ...
        while (pkvCamera) {
          ...
          CPointDevShotCamera *pCamera =
              (CPointDevShotCamera *)CreateEntityByName("point_devshot_camera");
          Assert(pCamera);
