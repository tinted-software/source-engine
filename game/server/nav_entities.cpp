...
//--------------------------------------------------------------------------------------------------------
// Reevaluate all func_nav_cost entities and update the nav decoration
// accordingly. This is required to handle overlapping func_nav_cost entities.
  ...
  if (m_debugOverlays & OVERLAY_TEXT_BIT) {
    ...
    for (int i = FIRST_GAME_TEAM; i < FIRST_GAME_TEAM + MAX_NAV_TEAMS; ++i) {
      if (IsBlockingNav(i)) {
        ...
        if (team) {
          EntityText(offset++, str.sprintf("blocking team %s", team->GetName()),
                     0);
        } else {
    ...
    for (int i = 0; i < collector.m_area.Count(); ++i) {
      ...
      debugoverlay->AddBoxOverlay(vec3_origin, areaExtent.lo, areaExtent.hi,
                                  vec3_angle, 0, 255, 0, 10,
...
//--------------------------------------------------------------------------------------------------------
// Forces nav areas to unblock when the nav blocker is deleted (round restart)
// so flow can compute properly
...
//--------------------------------------------------------------------------------------------------------
bool CFuncNavBlocker::CalculateBlocked(bool *pResultByTeam,
        ...
        if (!pResultByTeam[i]) {
          if (bIsIntersecting ||
              (bIsIntersecting = IsBoxIntersectingBox(
                   pBlocker->m_CachedMins, pBlocker->m_CachedMaxs, vecMins,
                   vecMaxs)) != false) {
            bBlocked = true;
...
//-----------------------------------------------------------------------------------------------------
/**
 * An entity that can obstruct nav areas.  This is meant for semi-transient
 * areas that obstruct pathfinding but can be ignored for longer-term queries
 * like computing L4D flow distances and escape routes.
 */
...
int CFuncNavObstruction::DrawDebugTextOverlays(void) {
  ...
  if (m_debugOverlays & OVERLAY_TEXT_BIT) {
    if (CanObstructNavAreas()) {
      ...
      EntityText(offset++, "Not obstructing nav",
                 NDEBUG_PERSIST_TILL_NEXT_SERVER);
    }
  }
