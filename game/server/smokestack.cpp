...
SendPropFloat(SENDINFO(m_SpreadSpeed), 0, SPROP_NOSCALE),
    ...
    // Note: the base color is specified in the smokestack entity, but the
    // directional and ambient light must come from env_particlelight entities.
    ...
    SendPropFloat(SENDINFO_NOCHECK(m_AmbientLight.m_flIntensity), 0,
                  SPROP_NOSCALE),
...
bool CSmokeStack::KeyValue(const char *szKeyName, const char *szValue) {
  if (stricmp(szKeyName, "Wind") == 0) {
    sscanf(szValue, "%f %f %f", &m_vWind.GetForModify().x,
           &m_vWind.GetForModify().y, &m_vWind.GetForModify().z);
    return true;
...
void CSmokeStack::RecalcWindVector() {
  m_vWind = Vector(cos(DEG2RAD((float)m_WindAngle)) * m_WindSpeed,
                   sin(DEG2RAD((float)m_WindAngle)) * m_WindSpeed, 0);
}
