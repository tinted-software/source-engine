...
SendPropInt(SENDINFO(m_clrRender), 32, SPROP_UNSIGNED, SendProxy_Color32ToInt),
    SendPropInt(SENDINFO(m_clrOverlay), 32, SPROP_UNSIGNED,
    ...
    SendPropFloat(SENDINFO_NAME(m_flHDRColorScale, HDRColorScale), 0,
                  SPROP_NOSCALE, 0.0f, 100.0f),
    END_SEND_TABLE()
...
void CSun::Activate() {
  ...
  // Find our target.
  if (m_bUseAngles) {
    SetupLightNormalFromProps(GetAbsAngles(), m_flYaw, m_flPitch,
                              m_vDirection.GetForModify());
  ...
  if (m_strOverlayMaterial == NULL_STRING) {
    m_strOverlayMaterial =
        AllocPooledString("sprites/light_glow02_add_noz.vmt");
  } else {
    const char *pExtension = V_GetFileExtension(STRING(m_strOverlayMaterial));
    if (!pExtension) {
      char szFixedString[MAX_PATH];
      V_strncpy(szFixedString, STRING(m_strOverlayMaterial),
                sizeof(szFixedString));
      V_strncat(szFixedString, ".vmt", sizeof(szFixedString));
...
int CSun::UpdateTransmitState() { return SetTransmitState(FL_EDICT_ALWAYS); }
