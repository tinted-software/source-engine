...
#define ShouldDebugLos(node)                                                   \
  (ai_debug_tactical_los.GetBool() &&                                          \
   (g_AIDebugFindLosNode == -1 || g_AIDebugFindLosNode == (node)) &&           \
   GetOuter()->m_bSelected)
#else
...
bool CAI_TacticalServices::FindLos(const Vector &threatPos,
                                   const Vector &threatEyePos,
                                   float minThreatDist, float maxThreatDist,
                                   float blockTime, FlankType_t eFlankType,
  ...
  int node = FindLosNode(threatPos, threatEyePos, minThreatDist, maxThreatDist,
...
bool CAI_TacticalServices::FindLos(const Vector &threatPos,
  ...
  return FindLos(threatPos, threatEyePos, minThreatDist, maxThreatDist,
                 blockTime, FLANKTYPE_NONE, vec3_origin, 0, pResult);
}

//-------------------------------------

bool CAI_TacticalServices::FindBackAwayPos(const Vector &vecThreat,
                                           Vector *pResult) {
  MARK_TASK_EXPENSIVE();
  ...
  if (GetOuter()->GetNavigator()->FindVectorGoal(pResult, vMoveAway, 10 * 12,
                                                 10 * 12, true))
    return true;
  ...
  if (GetOuter()->GetNavigator()->FindVectorGoal(
          pResult, vMoveAway, GetHullWidth() * 4, GetHullWidth() * 2, true))
    return true;
...
bool CAI_TacticalServices::FindCoverPos(const Vector &vThreatPos,
                                        const Vector &vThreatEyePos,
                                        float flMinDist, float flMaxDist,
                                        Vector *pResult) {
  return FindCoverPos(GetLocalOrigin(), vThreatPos, vThreatEyePos, flMinDist,
                      flMaxDist, pResult);
}

//-------------------------------------

bool CAI_TacticalServices::FindCoverPos(const Vector &vNearPos,
  ...
  int node =
      FindCoverNode(vNearPos, vThreatPos, vThreatEyePos, flMinDist, flMaxDist);
...
//-------------------------------------
bool CAI_TacticalServices::TestLateralCover(const Vector &vecCheckStart,
                                            const Vector &vecCheckEnd,
                                            float flMinDist) {
  trace_t tr;

  if ((vecCheckStart - vecCheckEnd).LengthSqr() > Square(flMinDist)) {
    if (GetOuter()->IsCoverPosition(
            vecCheckStart, vecCheckEnd + GetOuter()->GetViewOffset())) {
      if (GetOuter()->IsValidCover(vecCheckEnd, NULL)) {
        AIMoveTrace_t moveTrace;
        GetOuter()->GetMoveProbe()->MoveLimit(NAV_GROUND, GetLocalOrigin(),
                                              vecCheckEnd, MASK_NPCSOLID, NULL,
                                              &moveTrace);
        if (moveTrace.fStatus == AIMR_OK) {
          DebugFindCover(NO_NODE, vecCheckEnd + GetOuter()->GetViewOffset(),
                         vecCheckStart, 0, 255, 0);
          return true;
        }
      }
    }
...
bool CAI_TacticalServices::FindLateralCover(const Vector &vecThreat,
                                            float flMinDist, Vector *pResult) {
  return FindLateralCover(vecThreat, flMinDist, COVER_CHECKS * COVER_DELTA,
                          COVER_CHECKS, pResult);
}
...
bool CAI_TacticalServices::FindLateralCover(const Vector &vecThreat,
                                            ...
                                            int numChecksPerDir,
                                            Vector *pResult) {
  return FindLateralCover(GetAbsOrigin(), vecThreat, flMinDist, distToCheck,
                          numChecksPerDir, pResult);
}

bool CAI_TacticalServices::FindLateralCover(const Vector &vNearPos,
  ...
  if (!ai_find_lateral_cover.GetBool()) {
    // Force the NPC to use the nodegraph to find cover. NOTE: We let the above
    // code run to detect the case where the NPC may already be standing in
    // cover, but we don't make any additional lateral checks.
    return false;
  }
...
//			min range of my current weapon if there is one or just
//futher 			away than my current location if I don't have a weapon. 			Used to back
//away for attacks
//-------------------------------------
...
int CAI_TacticalServices::FindBackAwayNode(const Vector &vecThreat) {
  ...
  for (int link = 0; link < GetNetwork()->GetNode(iMyNode)->NumLinks();
       link++) {
    ...
    float flTestDist =
        (vecThreat - GetNetwork()->GetNode(destID)->GetPosition(GetHullType()))
            .Length();

    if (flTestDist > flCurDist) {
      // Make sure this node doesn't take me past the enemy's position.
      Vector vecToNode;
      vecToNode = GetNetwork()->GetNode(destID)->GetPosition(GetHullType()) -
                  GetLocalOrigin();
...
int CAI_TacticalServices::FindCoverNode(const Vector &vThreatPos,
                                        const Vector &vThreatEyePos,
                                        float flMinDist, float flMaxDist) {
  return FindCoverNode(GetLocalOrigin(), vThreatPos, vThreatEyePos, flMinDist,
                       flMaxDist);
}

//-------------------------------------

int CAI_TacticalServices::FindCoverNode(const Vector &vNearPos,
                                        const Vector &vThreatPos,
                                        const Vector &vThreatEyePos,
                                        float flMinDist, float flMaxDist) {
  if (!CAI_NetworkManager::NetworksLoaded())
  ...
  DebugFindCover(NO_NODE, GetOuter()->EyePosition(), vThreatEyePos, 0, 255,
                 255);
  ...
  // ------------------------------------------------------------------------------------
  // We're going to search for a cover node by expanding to our current node's
  // neighbors and then their neighbors, until cover is found, or all nodes are
  // beyond MaxDist
  // ------------------------------------------------------------------------------------
  AI_NearNode_t *pBuffer = (AI_NearNode_t *)stackalloc(
      sizeof(AI_NearNode_t) * GetNetwork()->NumNodes());
  ...
  static int nSearchRandomizer = 0; // tries to ensure the links are searched in
                                    // a different order each time;

  // Search until the list is empty
  while (list.Count()) {
    // Get the node that is closest in the number of steps and remove from the
    // list
    ...
    if (dist >= flMinDistSqr && dist < flMaxDistSqr) {
      ...
      if (GetOuter()->IsValidCover(nodeOrigin, pNode->GetHint())) {
        ...
        if (GetOuter()->IsCoverPosition(vThreatEyePos, vEyePos)) {
          ...
          if (pNode->GetHint() &&
              (pNode->GetHint()->HintType() == HINT_TACTICAL_COVER_MED ||
               pNode->GetHint()->HintType() == HINT_TACTICAL_COVER_LOW)) {
            if (GetOuter()->GetHintNode()) {
              GetOuter()->GetHintNode()->Unlock(GetOuter()->GetHintDelay(
                  GetOuter()->GetHintNode()->HintType()));
    ...
    for (int link = 0; link < GetNetwork()->GetNode(nodeIndex)->NumLinks();
         link++) {
      int index = (link + nSearchRandomizer) %
                  GetNetwork()->GetNode(nodeIndex)->NumLinks();
      CAI_Link *nodeLink =
          GetNetwork()->GetNode(nodeIndex)->GetLinkByIndex(index);
      ...
      if (!wasVisited.IsBitSet(newID)) {
        ...
        if (GetNetwork()->GetNode(newID)->GetType() != NODE_CLIMB &&
            !GetNetwork()->GetNode(newID)->IsLocked()) {
          // UNDONE: Shouldn't we really accumulate the distance by path rather
          // than absolute distance.  After all, we are performing essentially
          // an A* here.
          nodeOrigin = GetNetwork()->GetNode(newID)->GetPosition(GetHullType());
          dist = (vNearPos - nodeOrigin).LengthSqr();

          // use distance to threat as a heuristic to keep AIs from running
          // toward the threat in order to take cover from it.
...
//
// Input  :	pNPC			- npc that's looking for a place to
// shoot from
//			vThreatPos		- position of entity/location
//I'm trying to shoot 			vThreatEyePos	- eye position of entity I'm trying to
//shoot. If 							  entity has no eye position, just give vThreatPos again
//			flMinThreatDist	- minimum distance that node must be
//from vThreatPos 			flMaxThreadDist	- maximum distance that node can be from
//vThreadPos 			vThreatFacing	- optional argument.  If given the returned node
//							  will also be behind
//the given facing direction (flanking) 			flBlockTime		- how long to
//block this node from use
// Output :	int				- ID number of node that meets
// qualifications
//-------------------------------------

int CAI_TacticalServices::FindLosNode(
    const Vector &vThreatPos, const Vector &vThreatEyePos,
    float flMinThreatDist, float flMaxThreatDist, float flBlockTime,
    FlankType_t eFlankType, const Vector &vecFlankRefPos, float flFlankParam) {
  if (!CAI_NetworkManager::NetworksLoaded())
    return NO_NODE;
  ...
  // ------------------------------------------------------------------------------------
  // We're going to search for a shoot node by expanding to our current node's
  // neighbors and then their neighbors, until a shooting position is found, or
  // all nodes are beyond MaxDist
  // ------------------------------------------------------------------------------------
  AI_NearNode_t *pBuffer = (AI_NearNode_t *)stackalloc(
      sizeof(AI_NearNode_t) * GetNetwork()->NumNodes());
  CNodeList list(pBuffer, GetNetwork()->NumNodes());
  ...
  while (list.Count()) {
    ...
    const Vector &nodeOrigin =
        GetNetwork()->GetNode(nodeIndex)->GetPosition(GetHullType());

    // HACKHACK: Can't we rework this loop and get rid of this?
    // skip the starting node, or we probably wouldn't have called this
    // function.
    if (nodeIndex != iMyNode) {
      ...
      // Don't accept climb nodes, and assume my nearest node isn't valid
      // because we decided to make this check in the first place.  Keep moving
      if (!skip && !GetNetwork()->GetNode(nodeIndex)->IsLocked() &&
        ...
        if (flThreatDist < flMaxThreatDist && flThreatDist > flMinThreatDist) {
          CAI_Node *pNode = GetNetwork()->GetNode(nodeIndex);
          if (GetOuter()->IsValidShootPosition(nodeOrigin, pNode,
                                               pNode->GetHint())) {
            if (GetOuter()->TestShootPosition(nodeOrigin, vThreatEyePos)) {
              ...
              if (ShouldDebugLos(nodeIndex)) {
                NDebugOverlay::Text(nodeOrigin, CFmtStr("%d:los!", nodeIndex),
                                    false, 1);
              }
              ...
              if (ShouldDebugLos(nodeIndex)) {
                NDebugOverlay::Text(nodeOrigin, CFmtStr("%d:!shoot", nodeIndex),
                                    false, 1);
              }
            }
          } else {
            if (ShouldDebugLos(nodeIndex)) {
              NDebugOverlay::Text(nodeOrigin, CFmtStr("%d:!valid", nodeIndex),
                                  false, 1);
            }
          }
        } else {
          if (ShouldDebugLos(nodeIndex)) {
            CFmtStr msg("%d:%s", nodeIndex,
                        (flThreatDist < flMaxThreatDist) ? "too close"
                                                         : "too far");
    ...
    for (int link = 0; link < GetNetwork()->GetNode(nodeIndex)->NumLinks();
      ...
      if (!wasVisited.IsBitSet(newID)) {
        float dist = (GetLocalOrigin() -
                      GetNetwork()->GetNode(newID)->GetPosition(GetHullType()))
                         .LengthSqr();
        list.Insert(AI_NearNode_t(newID, dist));
        wasVisited.Set(newID);
      }
...
// Checks lateral LOS
//-------------------------------------
bool CAI_TacticalServices::TestLateralLos(const Vector &vecCheckStart,
                                          const Vector &vecCheckEnd) {
  trace_t tr;

  // it's faster to check the SightEnt's visibility to the potential spot than
  // to check the local move, so we do that first.
  AI_TraceLOS(vecCheckStart, vecCheckEnd + GetOuter()->GetViewOffset(), NULL,
              &tr);
...
bool CAI_TacticalServices::FindLateralLos(const Vector &vecThreat,
                                          Vector *pResult) {
  ...
  Vector vecStepRight;
  Vector vecCheckStart;
  bool bLookingForEnemy =
      GetEnemy() && VectorsAreEqual(vecThreat, GetEnemy()->EyePosition(), 0.1f);
  int i;

  if (!bLookingForEnemy || GetOuter()->HasCondition(COND_SEE_ENEMY) ||
      GetOuter()->HasCondition(COND_HAVE_ENEMY_LOS) ||
      GetOuter()->GetTimeScheduleStarted() ==
          gpGlobals->curtime) // Conditions get nuked before tasks run, assume
                              // should try
  {
  ...
  if (!ai_find_lateral_los.GetBool()) {
    // Allows us to turn off lateral LOS at the console. Allow the above code to
    // run just in case the NPC has line of sight to begin with.
    return false;
  }
  ...
  // If we're limited in how far we're allowed to move laterally, don't bother
  // checking past it
