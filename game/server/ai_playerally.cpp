...
ConVar sk_ally_regen_time(
    "sk_ally_regen_time", "0.3003", FCVAR_NONE,
    "Time taken for an ally to regenerate a point of health.");
ConVar sv_npc_talker_maxdist(
    "sv_npc_talker_maxdist", "1024", 0,
    "NPCs over this distance from the player won't attempt to speak.");
ConVar ai_no_talk_delay("ai_no_talk_delay", "0");

ConVar rr_debug_qa("rr_debug_qa", "0", FCVAR_NONE,
                   "Set to 1 to see debug related to the Question & Answer "
                   "system used to create conversations between allied NPCs.");
...
ConceptInfo_t g_ConceptInfos[] = {
    ...
    },
    {
        ...
        -1,
        -1,
        AICF_DEFAULT | AICF_SPEAK_ONCE | AICF_PROPAGATE_SPOKEN |
            AICF_TARGET_PLAYER,
    },
    {
        TLK_PHELLO,
...
void CAI_AllySpeechManager::Spawn() {
  ...
  for (int i = 0; i < ARRAYSIZE(g_ConceptInfos); i++)
    m_ConceptTimers.Insert(AllocPooledString(g_ConceptInfos[i].concept),
                           CSimpleSimTimer());
}
...
void CAI_AllySpeechManager::AddCustomConcept(const ConceptInfo_t &conceptInfo) {
  ...
  if (g_ConceptInfoMap.Find(conceptInfo.concept) ==
      g_ConceptInfoMap.InvalidIndex()) {
    g_ConceptInfoMap.Insert(conceptInfo.concept,
                            new ConceptInfo_t(conceptInfo));
  }

  if (m_ConceptTimers.Find(AllocPooledString(conceptInfo.concept)) ==
      m_ConceptTimers.InvalidIndex()) {
    m_ConceptTimers.Insert(AllocPooledString(conceptInfo.concept),
                           CSimpleSimTimer());
  }
}

ConceptCategoryInfo_t *
CAI_AllySpeechManager::GetConceptCategoryInfo(ConceptCategory_t category) {
...
ConceptInfo_t *CAI_AllySpeechManager::GetConceptInfo(AIConcept_t concept) {
  ...
  return (iResult != g_ConceptInfoMap.InvalidIndex())
             ? g_ConceptInfoMap[iResult]
             : NULL;
}

void CAI_AllySpeechManager::OnSpokeConcept(CAI_PlayerAlly *pPlayerAlly,
                                           AIConcept_t concept,
                                           AI_Response *response) {
  ConceptInfo_t *pConceptInfo = GetConceptInfo(concept);
  ConceptCategory_t category =
      (pConceptInfo) ? pConceptInfo->category : SPEECH_IDLE;
    ...
    if (pConceptInfo->flags & AICF_PROPAGATE_SPOKEN) {
      ...
      for (int i = 0; i < g_AI_Manager.NumAIs(); i++) {
        ...
        if (pTalker && pTalker != pPlayerAlly &&
            (pTalker->GetAbsOrigin() - pPlayerAlly->GetAbsOrigin())
                    .LengthSqr() < Square(TALKRANGE_MIN * 2) &&
  ...
  if (!ai_no_talk_delay.GetBool()) {
    if (pConceptInfo && pConceptInfo->minGlobalCategoryDelay != -1) {
      ...
      SetCategoryDelay(pConceptInfo->category,
                       pConceptInfo->minGlobalCategoryDelay,
                       pConceptInfo->maxGlobalCategoryDelay);
    } else if (pCategoryInfo->maxGlobalDelay > 0) {
      SetCategoryDelay(category, pCategoryInfo->minGlobalDelay,
                       pCategoryInfo->maxGlobalDelay);
    }

    if (pConceptInfo && pConceptInfo->minPersonalCategoryDelay != -1) {
      Assert(pConceptInfo->maxPersonalCategoryDelay != -1);
      pPlayerAlly->SetCategoryDelay(pConceptInfo->category,
                                    pConceptInfo->minPersonalCategoryDelay,
                                    pConceptInfo->maxPersonalCategoryDelay);
    } else if (pCategoryInfo->maxPersonalDelay > 0) {
      pPlayerAlly->SetCategoryDelay(category, pCategoryInfo->minPersonalDelay,
                                    pCategoryInfo->maxPersonalDelay);
    }
    ...
    if (pConceptInfo && pConceptInfo->minConceptDelay != -1) {
      ...
      if (iConceptTimer != m_ConceptTimers.InvalidIndex())
        m_ConceptTimers[iConceptTimer].Set(pConceptInfo->minConceptDelay,
                                           pConceptInfo->minConceptDelay);
    }
  }
}
...
DEFINE_EMBEDDED(m_PendingResponse), DEFINE_STDSTRING(m_PendingConcept),
    ...
    DEFINE_INPUTFUNC(FIELD_STRING, "SpeakResponseConcept",
    ...
    DEFINE_INPUTFUNC(FIELD_VOID, "EnableSpeakWhileScripting",
                     InputEnableSpeakWhileScripting),
    DEFINE_INPUTFUNC(FIELD_VOID, "DisableSpeakWhileScripting",
...
void CAI_PlayerAlly::GatherConditions(void) {
  ...
  if (HasCondition(COND_SEE_PLAYER)) {
    ...
    if ((pLocalPlayer->GetAbsOrigin() - GetAbsOrigin()).Length2DSqr() <
        Square(TALKER_STARE_DIST)) {
...
void CAI_PlayerAlly::GatherEnemyConditions(CBaseEntity *pEnemy) {
  ...
  if (GetLastEnemyTime() == 0 || gpGlobals->curtime - GetLastEnemyTime() > 30) {
    ...
    if (HasCondition(COND_SEE_ENEMY) &&
        (pEnemy->Classify() != CLASS_BULLSEYE)) {
      if (Classify() == CLASS_PLAYER_ALLY_VITAL && hl2_episodic.GetBool()) {
        CBasePlayer *pPlayer = AI_GetSinglePlayer();

        if (pPlayer) {

          // If I can see the player, and the player would see this enemy if he
          // turned around...
          if (!pPlayer->FInViewCone(pEnemy) && FVisible(pPlayer) &&
              pPlayer->FVisible(pEnemy)) {
            Vector2D vecPlayerView = pPlayer->EyeDirection2D().AsVector2D();
            Vector2D vecToEnemy =
                (pEnemy->GetAbsOrigin() - pPlayer->GetAbsOrigin()).AsVector2D();
...
void CAI_PlayerAlly::PrescheduleThink(void) {
  ...
  if (GetHealth() >= GetMaxHealth()) {
    // Avoid huge deltas on first regeneration of health after long period of
    // time at full health.
...
int CAI_PlayerAlly::SelectSchedule(void) {
  if (!HasCondition(COND_RECEIVED_ORDERS)) {
    ...
    if (m_iHealth <= m_iMaxHealth * 0.75 && IsAllowedToSpeak(TLK_WOUND) &&
        !GetExpresser()->SpokeConcept(TLK_WOUND)) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CAI_PlayerAlly::SelectSpeechResponse(AIConcept_t concept,
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CAI_PlayerAlly::SetPendingSpeech(AIConcept_t concept,
...
bool CAI_PlayerAlly::SelectIdleSpeech(AISpeechSelection_t *pSelection) {
  ...
  CBasePlayer *pTarget = assert_cast<CBasePlayer *>(
      FindSpeechTarget(AIST_PLAYERS | AIST_FACING_TARGET));
  if (pTarget) {
    ...
    if (ShouldSpeakRandom(TLK_IDLE, chance) &&
        SelectSpeechResponse(TLK_IDLE, NULL, pTarget, pSelection))
      return true;
  }
  return false;
}
...
bool CAI_PlayerAlly::SelectAlertSpeech(AISpeechSelection_t *pSelection) {
  ...
  if (pTarget) {
    if (pTarget->IsAlive()) {
      float flHealthPerc =
          ((float)pTarget->m_iHealth / (float)pTarget->m_iMaxHealth);
      if (flHealthPerc < 1.0) {
...
bool CAI_PlayerAlly::SelectPlayerUseSpeech() {
  if (IsOkToSpeakInResponseToPlayer()) {
    if (Speak(TLK_USE))
      ...
      return Speak((!GetExpresser()->SpokeConcept(TLK_HELLO)) ? TLK_HELLO
                                                              : TLK_IDLE);
  }
  return false;
}
...
// Purpose:
//-----------------------------------------------------------------------------
bool CAI_PlayerAlly::SelectQuestionAndAnswerSpeech(
    AISpeechSelection_t *pSelection) {
  if (!IsOkToSpeak(SPEECH_IDLE))
    return false;
  ...
  CAI_PlayerAlly *pFriend =
      dynamic_cast<CAI_PlayerAlly *>(FindSpeechTarget(AIST_NPCS));
...
// Purpose:
//-----------------------------------------------------------------------------
void CAI_PlayerAlly::PostSpeakDispatchResponse(AIConcept_t concept,
                                               AI_Response *response) {
#ifdef HL2_EPISODIC
  ...
  if (pConceptInfo && (pConceptInfo->flags & AICF_QUESTION) &&
      GetSpeechTarget()) {
    bool bSaidHelloToNPC = !Q_strcmp(concept, "TLK_HELLO_NPC");

    float duration =
        GetExpresser()->GetSemaphoreAvailableTime(this) - gpGlobals->curtime;
    ...
    if (rr_debug_qa.GetBool()) {
      ...
      NDebugOverlay::HorzArrow(GetAbsOrigin(),
                               GetSpeechTarget()->GetAbsOrigin(), 8, 0, 255, 0,
                               64, true, duration);
    }
    ...
    g_EventQueue.AddEvent(GetSpeechTarget(), pszInput, value, duration + .2,
                          this, this);

    if (GetSpeechTarget()->MyNPCPointer()) {
      AddLookTarget(GetSpeechTarget()->MyNPCPointer(), 1.0,
                    duration + random->RandomFloat(0.4, 1.2), 0.5);
      GetSpeechTarget()->MyNPCPointer()->AddLookTarget(
  ...
  } else if (pConceptInfo && (pConceptInfo->flags & AICF_ANSWER) &&
             GetSpeechTarget()) {
    float duration =
        GetExpresser()->GetSemaphoreAvailableTime(this) - gpGlobals->curtime;
    if (rr_debug_qa.GetBool()) {
    ...
    if (GetSpeechTarget()->MyNPCPointer()) {
      AddLookTarget(GetSpeechTarget()->MyNPCPointer(), 1.0,
                    duration + random->RandomFloat(0, 0.3), 0.5);
      GetSpeechTarget()->MyNPCPointer()->AddLookTarget(
...
//-----------------------------------------------------------------------------
bool CAI_PlayerAlly::SelectQuestionFriend(CBaseEntity *pFriend,
                                          AISpeechSelection_t *pSelection) {
  ...
  if (!GetExpresser()->SpokeConcept(TLK_HELLO_NPC) &&
      !FClassnameIs(this, pFriend->GetClassname())) {
...
//-----------------------------------------------------------------------------
bool CAI_PlayerAlly::SelectAnswerFriend(CBaseEntity *pFriend,
                                        AISpeechSelection_t *pSelection,
...
void CAI_PlayerAlly::InputAnswerQuestion(inputdata_t &inputdata) {
  AnswerQuestion(dynamic_cast<CAI_PlayerAlly *>(inputdata.pActivator),
                 inputdata.value.Int(), false);
}
...
void CAI_PlayerAlly::InputAnswerQuestionHello(inputdata_t &inputdata) {
  AnswerQuestion(dynamic_cast<CAI_PlayerAlly *>(inputdata.pActivator),
                 inputdata.value.Int(), true);
}

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CAI_PlayerAlly::AnswerQuestion(CAI_PlayerAlly *pQuestioner,
                                    int iQARandomNum, bool bAnsweringHello) {
  ...
  // Use the random number that the questioner used to determine his Question
  // (so we can match answers via response rules)
...
int CAI_PlayerAlly::SelectNonCombatSpeechSchedule() {
  ...
  if (HasPendingSpeech()) {
    if (m_TimePendingSet == gpGlobals->curtime ||
        IsAllowedToSpeak(m_PendingConcept.c_str()))
...
void CAI_PlayerAlly::StartTask(const Task_t *pTask) {
  ...
  case TASK_MOVE_AWAY_PATH: {
    if (HasCondition(COND_PLAYER_PUSHING) && AI_IsSinglePlayer()) {
      ...
      GetMotor()->SetIdealYawToTarget(
          UTIL_GetLocalPlayer()->WorldSpaceCenter());
    }
  ...
  if (pKilled) {
    if (!pKilled->IsNPC() ||
        (pKilled->MyNPCPointer()->GetLastPlayerDamageTime() == 0 ||
         gpGlobals->curtime -
                 pKilled->MyNPCPointer()->GetLastPlayerDamageTime() >
             5)) {
...
//-----------------------------------------------------------------------------
void CAI_PlayerAlly::TraceAttack(const CTakeDamageInfo &info,
  ...
  } else if (ptr->hitgroup == HITGROUP_LEFTARM ||
             ptr->hitgroup == HITGROUP_RIGHTARM) {
  ...
  CFmtStrN<128> modifiers("%s,damageammo:%s", pszHitLocCriterion,
                          info.GetAmmoName());
...
int CAI_PlayerAlly::OnTakeDamage_Alive(const CTakeDamageInfo &info) {
  ...
  // Vital allies never take more than 25% of their health in a single hit
  // (except for physics damage)
#ifdef HL2_DLL
  // Don't do damage reduction for DMG_GENERIC. This allows SetHealth inputs to
  // still do full damage.
  if (subInfo.GetDamageType() != DMG_GENERIC) {
    if (Classify() == CLASS_PLAYER_ALLY_VITAL &&
        !(subInfo.GetDamageType() & DMG_CRUSH)) {
...
bool CAI_PlayerAlly::IsValidSpeechTarget(int flags, CBaseEntity *pEntity) {
  ...
  if (pNPC) {
    ...
    if ((pNPC->m_NPCState == NPC_STATE_SCRIPT ||
         pNPC->m_NPCState == NPC_STATE_PRONE))
      return false;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CAI_PlayerAlly::PlayScriptedSentence(const char *pszSentence, float delay,
                                         float volume, soundlevel_t soundlevel,
                                         bool bConcurrent,
                                         CBaseEntity *pListener) {
  ClearCondition(
      COND_PLAYER_PUSHING); // Forget about moving!  I've got something to say!
...
void CAI_PlayerAlly::DeferAllIdleSpeech(float flDelay, CAI_BaseNPC *pIgnore) {
  CAI_AllySpeechManager *pSpeechManager = GetAllySpeechManager();
  if (flDelay == -1) {
    ConceptCategoryInfo_t *pCategoryInfo =
        pSpeechManager->GetConceptCategoryInfo(SPEECH_IDLE);
    pSpeechManager->SetCategoryDelay(SPEECH_IDLE, pCategoryInfo->minGlobalDelay,
                                     pCategoryInfo->maxGlobalDelay);
  } else
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
bool CAI_PlayerAlly::IsOkToSpeak(ConceptCategory_t category,
                                 bool fRespondingToPlayer) {
  CAI_AllySpeechManager *pSpeechManager = GetAllySpeechManager();
  ...
  if (!fRespondingToPlayer) {
    if (!pSpeechManager->CategoryDelayExpired(category) ||
        !CategoryDelayExpired(category))
      return false;
  }
  ...
  if (category != SPEECH_PRIORITY) {
    ...
    if (pPlayer) {
      ...
      flDist *= flDist;
      if ((pPlayer->WorldSpaceCenter() - WorldSpaceCenter()).LengthSqr() >
          flDist)
        return false;
    }
  }
  ...
  if (fRespondingToPlayer) {
    // If we're responding to the player, don't respond if the scene has speech
    // in it
    ...
    // If we're not responding to the player, don't talk if running a
    // logic_choreo
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CAI_PlayerAlly::IsAllowedToSpeak(AIConcept_t concept,
                                      bool bRespondingToPlayer) {
  CAI_AllySpeechManager *pSpeechManager = GetAllySpeechManager();
  ConceptInfo_t *pInfo = pSpeechManager->GetConceptInfo(concept);
  ConceptCategory_t category = (pInfo) ? pInfo->category : SPEECH_IDLE;
  ...
  if ((pInfo && pInfo->flags & AICF_SPEAK_ONCE) &&
      GetExpresser()->SpokeConcept(concept))
    return false;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CAI_PlayerAlly::SpeakIfAllowed(AIConcept_t concept, const char *modifiers,
                                    bool bRespondingToPlayer,
...
void CAI_PlayerAlly::ModifyOrAppendCriteria(AI_CriteriaSet &set) {
  ...
  if (m_hPotentialSpeechTarget) {
    set.AppendCriteria("speechtarget",
                       m_hPotentialSpeechTarget->GetClassname());
    set.AppendCriteria("speechtargetname",
                       STRING(m_hPotentialSpeechTarget->GetEntityName()));
...
void CAI_PlayerAlly::OnStartSpeaking() {
  // If you say anything, don't greet the player - you may have already spoken
  // to them
...
bool CAI_PlayerAlly::RespondedTo(const char *ResponseConcept, bool bForce,
  ...
  if (bForce) {
    ...
    if (result) {
      // We've got something to say. Stop any scenes we're in, and speak the
      // response.
      if (bCancelScene)
