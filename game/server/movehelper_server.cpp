...
public:
  ...
  virtual void StartSound(const Vector &origin, int channel, char const *sample,
                          float volume, soundlevel_t soundlevel, int fFlags,
                          int pitch);
  virtual void StartSound(const Vector &origin, const char *soundname);

  virtual void PlaybackEventFull(int flags, int clientindex,
                                 unsigned short eventindex, float delay,
                                 Vector &origin, Vector &angles, float fparam1,
...
private:
  ...
  // results, tallied on client and server, but only used by server to run
  // SV_Impact. we store off our velocity in the trace_t structure so that we
  // can determine results of shoving boxes etc. around.
  struct touchlist_t {
...
CMoveHelperServer::~CMoveHelperServer(void) { SetSingleton(0); }
...
bool CMoveHelperServer::AddToTouched(const trace_t &tr,
...
//			*soundname -
//-----------------------------------------------------------------------------
void CMoveHelperServer::StartSound(const Vector &origin,
                                   const char *soundname) {
  // MDB - Changing this to send to PAS, as the overloaded function below has
  // done. Also removed the UsePredictionRules, client does not yet play the
  // equivalent sound
...
// plays a sound
//-----------------------------------------------------------------------------
void CMoveHelperServer::StartSound(const Vector &origin, int channel,
                                   char const *sample, float volume,
                                   soundlevel_t soundlevel, int fFlags,
                                   int pitch) {
...
// Umm...
//-----------------------------------------------------------------------------
void CMoveHelperServer::PlaybackEventFull(
...
//-----------------------------------------------------------------------------
// Purpose: Note that this only works on a listen server (since it requires
// graphical output)
//			*pFormat -
//			... -
//-----------------------------------------------------------------------------
...
//			damage, according to the rules in
...
bool CMoveHelperServer::PlayerFallingDamage(void) {
  ...
  if (flFallDamage > 0) {
    m_pHostPlayer->TakeDamage(CTakeDamageInfo(GetContainingEntity(INDEXENT(0)),
    ...
    if (pPlayer) {
      CCS_GameStats.IncrementStat(pPlayer, CSSTAT_FALL_DAMAGE,
                                  (int)flFallDamage);
    }
