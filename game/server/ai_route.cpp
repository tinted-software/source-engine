...
void CAI_Path::PrependWaypoint(const Vector &newPoint, Navigation_t navType,
...
float CAI_Path::GetPathDistanceToGoal(const Vector &startPos) {
  AI_Waypoint_t *pCurrent = GetCurWaypoint();
  if (pCurrent) {
    return (GetPathLength() + ComputePathDistance(pCurrent->NavType(), startPos,
                                                  pCurrent->GetPos()));
  }
  return 0;
}
...
void CAI_Path::SetGoalDirection(const Vector &goalDirection) {
  ...
  if ( pLast )
  {
          NDebugOverlay::Box( pLast->vecLocation, Vector( -2, -2, -2 ), Vector(
  2, 2, 2 ), 0,0,255, 0, 2.0 ); NDebugOverlay::Line( pLast->vecLocation,
  pLast->vecLocation + m_goalDirection * 32, 0,0,255, true, 2.0 );
  }
  */
}
...
void CAI_Path::SetGoalDirection(CBaseEntity *pTarget) {
  ...
  if (pTarget) {
    ...
    if (pLast) {
      ...
      VectorNormalize(m_goalDirection);
      /*
      NDebugOverlay::Box( pLast->vecLocation, Vector( -2, -2, -2 ), Vector( 2,
      2, 2 ), 0,0,255, 0, 2.0 ); NDebugOverlay::Line( pLast->vecLocation,
      pLast->vecLocation + m_goalDirection * 32, 0,0,255, true, 2.0 );
      */
    }
  }
...
Vector CAI_Path::GetGoalDirection(const Vector &startPos) {
  if (m_goalDirectionTarget) {
    ...
    if (pLast) {
      ...
      if (pPrev) {
        Vector goalDirection =
            m_goalDirectionTarget->GetAbsOrigin() - pPrev->vecLocation;
    ...
    // Assert(0); // comment out the default directions in SetGoal() to find
    // test cases for missing initialization
    AI_Waypoint_t *pLast = m_Waypoints.GetLast();
    if (pLast) {
      AI_Waypoint_t *pPrev = pLast->GetPrev();
      if (pPrev) {
...
void CAI_Path::Clear(void) {
  ...
  // FIXME: split m_goalTolerance into m_buildTolerance and m_moveTolerance, let
  // them be seperatly controllable.
...
void CAI_Path::ComputeRouteGoalDistances(AI_Waypoint_t *pGoalWaypoint) {
  ...
  while (pCurWaypoint->GetPrev()) {
    ...
    float flWaypointDist = ComputePathDistance(
        pCurWaypoint->NavType(), pPrev->GetPos(), pCurWaypoint->GetPos());
