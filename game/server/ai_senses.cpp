...
DEFINE_FIELD(m_LookDist, FIELD_FLOAT),
    ...
    // Could fold these three and above timer into one concept, but would
    // invalidate savegames
  ...
  if (pSound->GetSoundOrigin().DistToSqr(GetOuter()->EarPosition()) <=
      flHearDistanceSq) {
...
void CAI_Senses::Listen(void) {
  ...
  if (iSoundMask != SOUND_NONE &&
      !(GetOuter()->HasSpawnFlags(SF_NPC_WAIT_TILL_SEEN))) {
    int iSound = CSoundEnt::ActiveList();

    while (iSound != SOUNDLIST_EMPTY) {
      CSound *pCurrentSound = CSoundEnt::SoundPointerForIndex(iSound);

      if (pCurrentSound && (iSoundMask & pCurrentSound->SoundType()) &&
...
#ifdef PORTAL
bool CAI_Senses::CanSeeEntityThroughPortal(const CProp_Portal *pPortal,
...
bool CAI_Senses::WaitingUntilSeen(CBaseEntity *pSightEnt) {
  if (GetOuter()->m_spawnflags & SF_NPC_WAIT_TILL_SEEN) {
    if (pSightEnt->IsPlayer()) {
      ...
      // don't link this client in the list if the npc is wait till seen and the
      // player isn't facing the npc
      if (pPlayer
          // && pPlayer->FVisible( GetOuter() )
          && pPlayer->FInViewCone(GetOuter()) &&
          FBoxVisible(pSightEnt, static_cast<CBaseEntity *>(GetOuter()),
                      zero)) {
...
CBaseEntity *CAI_Senses::GetFirstSeenEntity(AISightIter_t *pIter,
                                            seentype_t iSeenType) const {
...
#ifdef PORTAL
bool CAI_Senses::LookThroughPortal(const CProp_Portal *pPortal,
                                   CBaseEntity *pSightEnt) {
  if (WaitingUntilSeen(pSightEnt))
    return false;

  if (ShouldSeeEntity(pSightEnt) &&
      CanSeeEntityThroughPortal(pPortal, pSightEnt)) {
    return SeeEntity(pSightEnt);
  }
  return false;
}
...
int CAI_Senses::LookForHighPriorityEntities(int iDistance) {
  int nSeen = 0;
  if (gpGlobals->curtime - m_TimeLastLookHighPriority >
      AI_HIGH_PRIORITY_SEARCH_TIME) {
    ...
    for (int i = 1; i <= gpGlobals->maxClients; i++) {
      ...
      if (pPlayer) {
        if (origin.DistToSqr(pPlayer->GetAbsOrigin()) < distSq &&
            Look(pPlayer)) {
          nSeen++;
        }
#ifdef PORTAL
          ...
          if (pPortal &&
              UTIL_Portal_DistanceThroughPortalSqr(
                  pPortal, origin, pPlayer->GetAbsOrigin()) < distSq &&
...
int CAI_Senses::LookForNPCs(int iDistance) {
  ...
  float timeNPCs = (efficiency < AIE_VERY_EFFICIENT)
                       ? AI_STANDARD_NPC_SEARCH_TIME
                       : AI_EFFICIENT_NPC_SEARCH_TIME;
  if (gpGlobals->curtime - m_TimeLastLookNPCs > timeNPCs) {
    ...
    if (efficiency < AIE_SUPER_EFFICIENT) {
      ...
      for (i = 0; i < g_AI_Manager.NumAIs(); i++) {
        if (ppAIs[i] != GetOuter() &&
            (ppAIs[i]->ShouldNotDistanceCull() ||
             origin.DistToSqr(ppAIs[i]->GetAbsOrigin()) < distSq)) {
          if (Look(ppAIs[i])) {
            nSeen++;
          }
        }
  ...
  while (pCurrent) {
    if ((!fScent && pCurrent->FIsSound()) || (fScent && pCurrent->FIsScent())) {
      if (pCurrent->IsSoundType(validTypes) &&
          !GetOuter()->ShouldIgnoreSound(pCurrent)) {
        if (!bUsePriority ||
            GetOuter()->GetSoundPriority(pCurrent) >= iBestPriority) {
...
void CAI_SensedObjectsManager::OnEntitySpawned(CBaseEntity *pEntity) {
  if ((pEntity->GetFlags() & FL_OBJECT) && !pEntity->IsPlayer() &&
      !pEntity->IsNPC()) {
