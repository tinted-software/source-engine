...
static ConCommand
    wc_create("wc_create", CC_WC_Create,
              "When in WC edit mode, creates a node where the player is "
              "looking if a node is allowed at that location for the currently "
              "selected hull size (see ai_next_hull)",
              FCVAR_CHEAT);
...
static ConCommand
    wc_destroy_undo("wc_destroy_undo", CC_WC_DestroyUndo,
