...
void ClientKill(edict_t *pEdict, const Vector &vecForce,
                bool bExplode = false) {
  CBasePlayer *pPlayer =
      static_cast<CBasePlayer *>(GetContainingEntity(pEdict));
...
void Host_Say(edict_t *pEdict, const CCommand &args, bool teamonly) {
  ...
  {
    if (args.ArgC() >= 2) {
      Q_snprintf(szTemp, sizeof(szTemp), "%s %s", (char *)pcmd,
                 (char *)args.ArgS());
    } else {
  ...
  if (pEdict) {
    ...
    pPlayer->CheckChatText(
        p, 127); // though the buffer szTemp that p points to is 256,
  ...
  if (pszPrefix && strlen(pszPrefix) > 0) {
    if (pszLocation && strlen(pszLocation)) {
      Q_snprintf(text, sizeof(text), "%s %s @ %s: ", pszPrefix, pszPlayerName,
                 pszLocation);
    } else {
  ...
  // This may return the world in single player if the client types something
  // between levels or during spawn so check it, or it will infinite loop
  ...
  for (int i = 1; i <= gpGlobals->maxClients; i++) {
    ...
    if (teamonly &&
        g_pGameRules->PlayerCanHearChat(client, pPlayer) != GR_TEAMMATE)
      continue;

    if (pPlayer && !client->CanHearAndReadChatFrom(pPlayer))
      continue;

    if (pPlayer && GetVoiceGameMgr() &&
        GetVoiceGameMgr()->IsPlayerIgnoringPlayer(pPlayer->entindex(), i))
      continue;
  ...
  // Adrian: Only do this if we're running a dedicated server since we already
  // print to console on the client.
...
CON_COMMAND_F(cast_ray, "Tests collision detection", FCVAR_CHEAT) {
  ...
  UTIL_TraceLine(start, start + forward * MAX_COORD_RANGE, MASK_SOLID, pPlayer,
                 COLLISION_GROUP_NONE, &tr);

  if (tr.DidHit()) {
    DevMsg(1, "Hit %s\nposition %.2f, %.2f, %.2f\nangles %.2f, %.2f, %.2f\n",
           tr.m_pEnt->GetClassname(), tr.m_pEnt->GetAbsOrigin().x,
           tr.m_pEnt->GetAbsOrigin().y, tr.m_pEnt->GetAbsOrigin().z,
           tr.m_pEnt->GetAbsAngles().x, tr.m_pEnt->GetAbsAngles().y,
           tr.m_pEnt->GetAbsAngles().z);
    DevMsg(1,
           "Hit: hitbox %d, hitgroup %d, physics bone %d, solid %d, surface "
           "%s, surfaceprop %s, contents %08x\n",
           tr.hitbox, tr.hitgroup, tr.physicsbone, tr.m_pEnt->GetSolid(),
           tr.surface.name, physprops->GetPropName(tr.surface.surfaceProps),
           tr.contents);
    NDebugOverlay::Line(start, tr.endpos, 0, 255, 0, false, 10);
    NDebugOverlay::Line(tr.endpos, tr.endpos + tr.plane.normal * 12, 255, 255,
                        0, false, 10);
  }
}
...
CON_COMMAND_F(cast_hull, "Tests hull collision detection", FCVAR_CHEAT) {
  ...
  pPlayer->EyeVectors(&forward);
  Vector start = pPlayer->EyePosition();
  UTIL_TraceHull(start, start + forward * MAX_COORD_RANGE, -extents, extents,
  ...
  if (tr.DidHit()) {
    ...
    DevMsg(1,
           ...
           tr.hitbox, tr.hitgroup, tr.physicsbone, tr.m_pEnt->GetSolid(),
           tr.surface.name, physprops->GetPropName(tr.surface.surfaceProps));
    NDebugOverlay::SweptBox(start, tr.endpos, -extents, extents, vec3_angle, 0,
                            0, 255, 0, 10);
    Vector end = tr.endpos; // - tr.plane.normal * DotProductAbs(
                            // tr.plane.normal, extents );
    NDebugOverlay::Line(end, end + tr.plane.normal * 24, 255, 255, 64, false,
                        10);
  }
}
...
//			Without a name, returns the entity under the player's
//crosshair. 			With a name it finds entities via name/classname/index
//-----------------------------------------------------------------------------
CBaseEntity *GetNextCommandEntity(CBasePlayer *pPlayer, const char *name,
                                  CBaseEntity *ent) {
  if (!pPlayer)
    return NULL;
...
void CPointClientCommand::InputCommand(inputdata_t &inputdata) {
  ...
  if (gpGlobals->maxClients == 1) {
    ...
    if (IsInCommentaryMode() && !pClient) {
      // Commentary is stuffing a command in. We'll pretend it came from the
      // first player.
      pClient = engine->PEntityOfEntIndex(1);
    }
  }
...
//------------------------------------------------------------------------------
// Purpose : Draw a line betwen two points.  White if no world collisions, red
// if collisions Input   : Output  :
//------------------------------------------------------------------------------
...
static ConCommand drawline(
    "drawline", CC_DrawLine,
...
static ConCommand
    drawcross("drawcross", CC_DrawCross,
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
CON_COMMAND_F(killvector,
              "Kills a player applying force. Usage: killvector <player> <x "
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
CON_COMMAND_F(buddha,
              "Toggle.  Player takes damage but won't die. (Shows red cross "
              "when health is zero)",
              FCVAR_CHEAT) {
...
CON_COMMAND(say, "Display player message") {
  CBasePlayer *pPlayer = ToBasePlayer(UTIL_GetCommandClient());
  if (pPlayer) {
    if ((pPlayer->LastTimePlayerTalked() + TALK_INTERVAL) <
        gpGlobals->curtime) {
...
CON_COMMAND(give, "Give item to player.\n\tArguments: <item_name>") {
  ...
  if (pPlayer && (gpGlobals->maxClients == 1 || sv_cheats->GetBool()) &&
    ...
    string_t iszItem =
        AllocPooledString(item_to_give); // Make a copy of the classname
...
static ConCommand setmodel("setmodel", CC_Player_SetModel,
                           "Changes's player's model", FCVAR_CHEAT);
...
void CC_Player_TestDispatchEffect(const CCommand &args) {
  ...
  if (args.ArgC() < 2) {
    Msg(" Usage: test_dispatcheffect <effect name> <distance away> <flags> "
        "<magnitude> <scale>\n ");
    Msg("		 defaults are: <distance 1024> <flags 0> <magnitude 0> "
        "<scale 0>\n");
    return;
  }
...
static ConCommand test_dispatcheffect(
    ...
    "Test a clientside dispatch effect.\n\tUsage: test_dispatcheffect <effect "
    "name> <distance away> <flags> <magnitude> <scale>\n\tDefaults are: "
    "<distance 1024> <flags 0> <magnitude 0> <scale 0>\n",
    FCVAR_CHEAT);
...
static ConCommand physswap("phys_swap", CC_Player_PhysSwap,
                           "Automatically swaps the current weapon for the "
                           "physcannon and back again.");
#endif
...
static ConCommand bugswap(
    "bug_swap", CC_Player_BugBaitSwap,
...
static ConCommand use("use", CC_Player_Use,
...
static bool TestEntityPosition(CBasePlayer *pPlayer) {
  trace_t trace;
  UTIL_TraceEntity(pPlayer, pPlayer->GetAbsOrigin(), pPlayer->GetAbsOrigin(),
...
//------------------------------------------------------------------------------
static int FindPassableSpace(CBasePlayer *pPlayer, const Vector &direction,
                             float step, Vector &oldorigin) {
  int i;
  for (i = 0; i < 100; i++) {
...
static ConCommand noclip("noclip", CC_Player_NoClip,
...
// Sets client to godmode
//------------------------------------------------------------------------------
CON_COMMAND_F(setpos, "Move player to specified origin (must have sv_cheats).",
              FCVAR_CHEAT) {
  if (!sv_cheats->GetBool())
    return;
...
static ConCommand setang("setang", CC_setang_f,
                         "Snap player eyes to specified pitch yaw "
                         "<roll:optional> (must have sv_cheats).",
                         FCVAR_CHEAT);
...
CON_COMMAND_F(setang_exact,
              "Snap player eyes and orientation to specified pitch yaw "
              "<roll:optional> (must have sv_cheats).",
              FCVAR_CHEAT) {
  if (!sv_cheats->GetBool())
    return;
...
ConCommand notarget("notarget", CC_Notarget_f,
                    "Toggle. Player becomes hidden to NPCs.", FCVAR_CHEAT);
...
void CC_HurtMe_f(const CCommand &args) {
  ...
  pPlayer->TakeDamage(
      CTakeDamageInfo(pPlayer, pPlayer, iDamage, DMG_PREVENT_PHYSICS_FORCE));
}

static ConCommand hurtme("hurtme", CC_HurtMe_f,
...
static ConCommand groundlist("groundlist", CC_GroundList_f,
...
void ClientCommand(CBasePlayer *pPlayer, const CCommand &args) {
  ...
  */
  ...
  if (FStrEq(pCmd, "killtarget")) {
    if (g_pDeveloper->GetBool() && sv_cheats->GetBool() &&
    ...
    if (sv_cheats->GetBool() && UTIL_IsCommandIssuedByServerAdmin()) {
      if (FStrEq(args[1], "stop")) {
        ...
        CTempEntTester::Create(pPlayer->WorldSpaceCenter(),
                               pPlayer->EyeAngles(), args[1], args[2]);
      }
    }
  } else {
