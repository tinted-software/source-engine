...
enum Capability_t {
  ...
  // bits_CAP_STRAFE					= 0x00400000, // strafe
  // ( walk/run sideways)
...
struct Relationship_t {
  ...
  int priority; // Relative importance of this relationship (higher numbers mean
                // more important)
...
public:
  ...
  virtual bool IsHiddenByFog(CBaseEntity *target)
      const; ///< return true if given target cant be seen because of fog
  virtual bool
  IsHiddenByFog(float range) const; ///< return true if given distance is too
                                    ///< far to see through the fog
  virtual float GetFogObscuredRatio(
      const Vector &target) const; ///< return 0-1 ratio where zero is not
                                   ///< obscured, and 1 is completely obscured
  virtual float GetFogObscuredRatio(
      CBaseEntity *target) const; ///< return 0-1 ratio where zero is not
                                  ///< obscured, and 1 is completely obscured
  virtual float GetFogObscuredRatio(
      float range) const; ///< return 0-1 ratio where zero is not obscured, and
  ...
  // Visible starts with line of sight, and adds all the extra game checks like
  // fog, smoke, camo...
  ...
  virtual bool
  IsLookingTowards(const CBaseEntity *target,
  ...
  virtual bool
  IsLookingTowards(const Vector &target,
                   float cosTolerance = BCC_DEFAULT_LOOK_TOWARDS_TOLERANCE)
      const; // return true if our view direction is pointing at the given
             // target, within the cosine of the angular tolerance. LINE OF
             // SIGHT IS NOT CHECKED.
  ...
  virtual void
  Weapon_Equip(CBaseCombatWeapon *pWeapon); // Adds weapon to player
  virtual bool Weapon_EquipAmmoOnly(
      CBaseCombatWeapon *pWeapon); // Adds weapon ammo to player, leaves weapon
  bool Weapon_Detach(
  ...
  virtual bool
  Weapon_Switch(CBaseCombatWeapon *pWeapon,
                int viewmodelindex =
  ...
  virtual bool HasEverBeenInjured(
      int team = TEAM_ANY) const; // return true if we have ever been injured by
  ...
  virtual void
  CorpseFade(void); // Called instead of GibNPC() when gibs are disabled
  ...
  virtual CBaseEntity *
  CheckTraceHullAttack(const Vector &vStart, const Vector &vEnd,
                       const Vector &mins, const Vector &maxs, int iDamage,
                       int iDmgType, float flForceScale = 1.0f,
                       bool bDamageAnyNPC = false);
...
public:
  ...
  virtual void UpdateLastKnownArea(
      void); // invoke this to update our last known nav area (since there is no
             // think method chained to CBaseCombatCharacter)
  virtual void OnNavAreaChanged(CNavArea *enteredArea, CNavArea *leftArea) {
  } // invoked (by UpdateLastKnownArea) when we enter a new nav area (or it is
    // reset to NULL)
  virtual bool IsAreaTraversable(
      const CNavArea *area) const; // return true if we can use the given area
...
public:
  ...
  virtual bool CanPowerupNow(int iPowerup); // Return true if I can be powered
                                            // by this powerup right now
...
public:
  ...
  float m_flFractionalBoost; // POWERUP_BOOST health fraction - specific powerup
                             // data
...
private:
  ...
  float
      m_flDamageAccumulator; // so very small amounts of damage do not get lost.
  int m_iDamageCount; // # of times NPC has been damaged.  used for tracking
                      // 1-shot kills.

  // Weapon proficiency gets calculated each time an NPC changes his weapon, and
  // then cached off as the CurrentWeaponProficiency.
...
protected:
  ...
  unsigned int
      m_hasBeenInjured; // bitfield corresponding to team ID that did the injury
  ...
  int m_registeredNavTeam; // ugly, but needed to clean up player team counts in
                           // nav mesh
};
...
public:
  ...
  CTraceFilterMelee(const IHandleEntity *passentity, int collisionGroup,
      ...
      : m_pPassEnt(passentity), m_collisionGroup(collisionGroup),
        m_dmgInfo(dmgInfo), m_pHit(NULL), m_flForceScale(flForceScale),
