...
void CShower::Touch(CBaseEntity *pOther) {
  ...
  if ((vecNewVelocity.x * vecNewVelocity.x +
       vecNewVelocity.y * vecNewVelocity.y) < 10.0)
    m_flSpeed = 0;
...
public:
  ...
  int m_iRadiusOverride; // For use when m_iMagnitude results in larger radius
                         // than designer desires.
...
DEFINE_KEYFIELD(m_iMagnitude, FIELD_INTEGER, "iMagnitude"),
    ...
    END_DATADESC()
        ...
        bool CEnvExplosion::KeyValue(const char *szKeyName,
                                     const char *szValue) {
...
void CEnvExplosion::InputExplode(inputdata_t &inputdata) {
  ...
  UTIL_TraceLine(vecSpot, vecSpot + Vector(0, 0, -40),
                 (MASK_SOLID_BRUSHONLY | MASK_WATER), this,
                 COLLISION_GROUP_NONE, &tr);

  // Pull out of the wall a bit. We used to move the explosion origin itself,
  // but that seems unnecessary, not to mention a little weird when you consider
  // that it might be in hierarchy. Instead we just calculate a new virtual
  // position at which to place the explosion. We don't use that new position to
  // calculate radius damage because according to Steve's comment, that
  // adversely affects the force vector imparted on explosion victims when they
  // ragdoll.
  ...
  int iRadius =
      (m_iRadiusOverride > 0) ? m_iRadiusOverride : (m_iMagnitude * 2.5f);

  CPASFilter filter(vecExplodeOrigin);
  te->Explosion(
      filter, 0.0, &vecExplodeOrigin,
      (m_sFireballSprite < 1) ? g_sModelIndexFireball : m_sFireballSprite,
      !(m_spawnflags & SF_ENVEXPLOSION_NOFIREBALL) ? (m_spriteScale / 10.0)
                                                   : 0.0,
      15, nFlags, iRadius, m_iMagnitude);
  ...
  if (!(m_spawnflags & SF_ENVEXPLOSION_NODAMAGE)) {
    ...
    if (iDamageType == -1) {
      iDamageType = HasSpawnFlags(SF_ENVEXPLOSION_GENERIC_DAMAGE) ? DMG_GENERIC
                                                                  : DMG_BLAST;
    }

    CTakeDamageInfo info(m_hInflictor ? m_hInflictor : this, pAttacker,
                         m_iMagnitude, iDamageType);
    ...
    RadiusDamage(info, GetAbsOrigin(), iRadius, m_iClassIgnore,
                 m_hEntityIgnore.Get());
  }
...
// HACKHACK -- create one of these and fake a keyvalue to get the right
// explosion setup
void ExplosionCreate(const Vector &center, const QAngle &angles,
                     CBaseEntity *pOwner, int magnitude, int radius,
                     int nSpawnFlags, float flExplosionForce,
                     CBaseEntity *pInflictor, int iCustomDamageType,
                     const EHANDLE *ignoredEntity, Class_T ignoredClass) {
  char buf[128];

  CEnvExplosion *pExplosion = (CEnvExplosion *)CBaseEntity::Create(
      "env_explosion", center, angles, pOwner);
...
void ExplosionCreate(const Vector &center, const QAngle &angles,
                     CBaseEntity *pOwner, int magnitude, int radius,
                     bool doDamage, float flExplosionForce, bool bSurfaceOnly,
                     bool bSilent, int iCustomDamageType) {
  // For E3, no sparks
  int nFlags = SF_ENVEXPLOSION_NOSPARKS | SF_ENVEXPLOSION_NODLIGHTS |
  ...
  ExplosionCreate(center, angles, pOwner, magnitude, radius, nFlags,
...
void ExplosionCreate(const Vector &center, const QAngle &angles,
                     ...
                     Class_T ignoredClass, float flExplosionForce,
  ...
  ExplosionCreate(center, angles, pOwner, magnitude, radius, nFlags,
                  flExplosionForce, NULL, iCustomDamageType, ignoredEntity,
                  ignoredClass);
}
