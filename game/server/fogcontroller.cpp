...
// Purpose:
//-----------------------------------------------------------------------------
CFogSystem *FogSystem(void) { return &s_FogSystem; }
...
DEFINE_INPUTFUNC(FIELD_FLOAT, "SetStartDist", InputSetStartDist),
    ...
    DEFINE_INPUTFUNC(FIELD_COLOR32, "SetColorSecondary",
    ...
    DEFINE_INPUTFUNC(FIELD_COLOR32, "SetColorSecondaryLerpTo",
                     InputSetColorSecondaryLerpTo),
    DEFINE_INPUTFUNC(FIELD_FLOAT, "SetStartDistLerpTo",
    ...
    SendPropInt(SENDINFO_STRUCTELEM(m_fog.colorPrimaryLerpTo), 32,
                SPROP_UNSIGNED),
    SendPropInt(SENDINFO_STRUCTELEM(m_fog.colorSecondaryLerpTo), 32,
                SPROP_UNSIGNED),
...
//-----------------------------------------------------------------------------
void CFogSystem::LevelInitPreEntity(void) { m_pMasterController = NULL; }
...
void CFogSystem::LevelInitPostEntity(void) {
  ...
  do {
    pFogController = static_cast<CFogController *>(
        gEntList.FindEntityByClassname(pFogController, "env_fog_controller"));
    if (pFogController) {
  ...
  // HACK: Singleplayer games don't get a call to CBasePlayer::Spawn on level
  // transitions. CBasePlayer::Activate is called before this is called so
  // that's too soon to set up the fog controller. We don't have a hook similar
  // to Activate that happens after LevelInitPostEntity is called, or we could
  // just do this in the player itself.
