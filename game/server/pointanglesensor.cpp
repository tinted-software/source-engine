...
//
//			Looks at its target's angles every frame and fires an
//output if its 			target's forward vector points at a specified lookat entity for
//more 			than a specified length of time.
//
//			It also fires an output whenever the target's angles
//change.
//
//=============================================================================//
...
#define SF_USE_TARGET_FACING                                                   \
  (1 << 0) // Use the target entity's direction instead of position
...
protected:
  ...
  bool IsFacingWithinTolerance(CBaseEntity *pEntity, CBaseEntity *pTarget,
                               float flTolerance, float *pflDot = NULL);

  bool m_bDisabled;       // When disabled, we do not think or fire outputs.
  string_t m_nLookAtName; // Name of the entity that the target must point at to
                          // fire the OnTrue output.

  EHANDLE m_hTargetEntity; // Entity whose angles are being monitored.
  EHANDLE m_hLookAtEntity; // Entity that the target must look at to fire the
                           // OnTrue output.

  float m_flDuration; // Time in seconds for which the entity must point at the
                      // target.
  float m_flDotTolerance; // Degrees of error allowed to satisfy the condition,
                          // expressed as a dot product.
  float m_flFacingTime;   // The time at which the target entity pointed at the
                          // lookat entity.
  bool m_bFired;          // Latches the output so it only fires once per true.

  // Outputs
  COutputEvent
      m_OnFacingLookat; // Fired when the target points at the lookat entity.
  COutputEvent
      m_OnNotFacingLookat; // Fired in response to a Test input if the target is
                           // not looking at the lookat entity.
  COutputVector m_TargetDir;
  COutputFloat m_FacingPercentage; // Normalize value representing how close the
                                   // entity is to facing directly at the target
...
DEFINE_KEYFIELD(m_bDisabled, FIELD_BOOLEAN, "StartDisabled"),
    ...
    //-----------------------------------------------------------------------------
    bool CPointAngleSensor::KeyValue(const char *szKeyName,
                                     const char *szValue) {
...
//-----------------------------------------------------------------------------
void CPointAngleSensor::Spawn(void) { BaseClass::Spawn(); }
...
void CPointAngleSensor::Activate(void) {
  ...
  // It's okay to not have a look at entity, it just means we measure and output
  // the angles of the target entity without testing them against the look at
  // entity.
...
//-----------------------------------------------------------------------------
// Purpose: Determines if one entity is facing within a given tolerance of
// another Input  : pEntity -
//			pTarget -
//			flTolerance -
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CPointAngleSensor::IsFacingWithinTolerance(CBaseEntity *pEntity,
                                                CBaseEntity *pTarget,
                                                float flTolerance,
                                                float *pflDot) {
  if (pflDot) {
...
void CPointAngleSensor::Think(void) {
  if (m_hTargetEntity != NULL) {
    ...
    if (m_hLookAtEntity != NULL) {
      ...
      //
      float flDot;
      if (IsFacingWithinTolerance(m_hTargetEntity, m_hLookAtEntity,
                                  m_flDotTolerance, &flDot)) {
        if (!m_bFired) {
          if (!m_flFacingTime) {
...
void CPointAngleSensor::InputSetTargetEntity(inputdata_t &inputdata) {
  if ((inputdata.value.String() == NULL) ||
      (inputdata.value.StringID() == NULL_STRING) ||
    ...
    m_hTargetEntity = gEntList.FindEntityByName(
        NULL, m_target, NULL, inputdata.pActivator, inputdata.pCaller);
    if (!m_bDisabled && m_hTargetEntity) {
      SetNextThink(gpGlobals->curtime);
    }
  }
...
// Purpose:
//-----------------------------------------------------------------------------
void CPointAngleSensor::InputEnable(inputdata_t &inputdata) { Enable(); }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CPointAngleSensor::InputDisable(inputdata_t &inputdata) { Disable(); }
...
int CPointAngleSensor::DrawDebugTextOverlays(void) {
  ...
  if (m_debugOverlays & OVERLAY_TEXT_BIT) {
    float flDot;
    bool bFacing = IsFacingWithinTolerance(m_hTargetEntity, m_hLookAtEntity,
                                           m_flDotTolerance, &flDot);

    char tempstr[512];
    Q_snprintf(tempstr, sizeof(tempstr), "delta ang (dot)    : %.2f (%f)",
               RAD2DEG(acos(flDot)), flDot);
    EntityText(nOffset, tempstr, 0);
    nOffset++;

    Q_snprintf(tempstr, sizeof(tempstr), "tolerance ang (dot): %.2f (%f)",
               RAD2DEG(acos(m_flDotTolerance)), m_flDotTolerance);
    EntityText(nOffset, tempstr, 0);
    nOffset++;
...
DEFINE_KEYFIELD(m_bDisabled, FIELD_BOOLEAN, "StartDisabled"),
    ...
    //-----------------------------------------------------------------------------
    // Purpose: Called after all entities have spawned on new map or savegame
    // load.
    //-----------------------------------------------------------------------------
...
// Purpose:
//-----------------------------------------------------------------------------
void CPointProximitySensor::InputEnable(inputdata_t &inputdata) { Enable(); }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CPointProximitySensor::InputDisable(inputdata_t &inputdata) { Disable(); }
...
void CPointProximitySensor::Think(void) {
  if (m_hTargetEntity != NULL) {
    ...
    // If we're only interested in the distance along a vector, modify the
    // length the accomodate that
