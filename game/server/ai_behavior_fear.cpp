...
DEFINE_FIELD(m_flTimeToSafety, FIELD_TIME),
    ...
    DEFINE_FIELD(m_flDeferUntil, FIELD_TIME), END_DATADESC();
  ...
  case TASK_FEAR_GET_PATH_TO_SAFETY_HINT: {
    ...
    case 0: // Find the hint node
    {
      ...
      if (pHint == NULL) {
        ...
        m_flDeferUntil =
            gpGlobals->curtime +
            3.0f; // Don't bang the hell out of this behavior. If we don't find
                  // a node, take a short break and run regular AI.
      } else {
    ...
    case 1: // Do the pathfinding.
    {
      ...
      if (GetNavigator()->SetGoal(goal) == false) {
        ...
        m_hRallyPoint->GetAbsOrigin(), AIN_DEF_ACTIVITY, 256 ); if (
        GetNavigator()->SetGoal( nearGoal, AIN_CLEAR_PREVIOUS_STATE ) )
        {
        //FIXME: HACK! The internal pathfinding is setting this without our
        consent, so override it! ClearCondition( COND_TASK_FAILED );
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CAI_FearBehavior::SpoilSafePlace() { m_SafePlaceMoveMonitor.ClearMark(); }
...
//			does NOT hate or fear me (meaning they aren't going to
//fight me)
//-----------------------------------------------------------------------------
...
void CAI_FearBehavior::GatherConditions() {
  ...
  if (GetEnemy()) {
    ...
    } else if (flEnemyDistSqr < FEAR_ENEMY_TOLERANCE_CLOSE_DIST_SQR &&
               GetEnemy()->GetEnemy() == GetOuter()) {
  ...
  if (pPlayer != NULL && GetAbsOrigin().DistToSqr(pPlayer->GetAbsOrigin()) >=
                             Square(ai_fear_player_dist.GetFloat() * 1.5f)) {
...
void CAI_FearBehavior::EndScheduleSelection() {
  ...
  // Just because we ran other AI for a while doesn't mean we aren't still in a
  // safe place.
  // ReleaseAllHints();
}
...
// Purpose:
// Output : int
// Notes  : If fear behavior is running at all, we know we're afraid of our
// enemy
//-----------------------------------------------------------------------------
...
CAI_Hint *CAI_FearBehavior::FindFearWithdrawalDest() {
  ...
  hintCriteria
      .SetFlag(bits_HINT_NODE_VISIBLE_TO_PLAYER | bits_HINT_NOT_CLOSE_TO_ENEMY /*| bits_HINT_NODE_IN_VIEWCONE | bits_HINT_NPC_IN_NODE_FOV*/);
  hintCriteria.AddIncludePosition(AI_GetSinglePlayer()->GetAbsOrigin(),
                                  (ai_fear_player_dist.GetFloat()));

  pHint = CAI_HintManager::FindHint(pOuter, hintCriteria);

  if (pHint) {
    // Reserve this node while I try to get to it. When I get there I will lock
    // it. Otherwise, if I fail to get there, the node will come available again
    // soon.
...
DEFINE_SCHEDULE(
    ...
    "		TASK_SET_SCHEDULE					"
    "SCHEDULE:SCHED_FEAR_STAY_IN_SAFE_PLACE"
    ""
    "	Interrupts"
    ""
    "		COND_HEAR_DANGER"
