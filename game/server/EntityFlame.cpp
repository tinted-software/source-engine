...
//
//			1) An entity that can be placed by a level designer and
//triggered 			   to ignite a target entity.
//
//			2) An entity that can be created at runtime to ignite a
//target entity.
//
//=============================================================================//
...
void CEntityFlame::InputIgnite(inputdata_t &inputdata) {
  if (m_target != NULL_STRING) {
    ...
    while ((pTarget = gEntList.FindEntityGeneric(pTarget, STRING(m_target),
      ...
      if (pBCC) {
        // DVS TODO: consider promoting Ignite to CBaseEntity and doing
        // everything here
...
CEntityFlame *CEntityFlame::Create(CBaseEntity *pTarget, bool useHitboxes) {
  ...
  float xSize = pTarget->CollisionProp()->OBBMaxs().x -
                pTarget->CollisionProp()->OBBMins().x;
  float ySize = pTarget->CollisionProp()->OBBMaxs().y -
                pTarget->CollisionProp()->OBBMins().y;
...
void CEntityFlame::FlameThink(void) {
  // Assure that this function will be ticked again even if we early-out in the
  // if below.
  ...
  if (m_flLifetime < gpGlobals->curtime || m_hEntAttached == NULL) {
    ...
    if (m_hEntAttached) {
      CBaseCombatCharacter *pAttachedCC =
          m_hEntAttached->MyCombatCharacterPointer();
  ...
  if (m_hEntAttached) {
    // Do radius damage ignoring the entity I'm attached to. This will harm
    // things around me.
    RadiusDamage(CTakeDamageInfo(this, this, 4.0f, DMG_BURN), GetAbsOrigin(),
                 m_flSize / 2, CLASS_NONE, m_hEntAttached);

    // Directly harm the entity I'm attached to. This is so we can precisely
    // control how much damage the entity that is on fire takes without worrying
    // about the flame's position relative to the bodytarget (which is the
    // distance that the radius damage code uses to determine how much damage to
    // inflict)
    m_hEntAttached->TakeDamage(CTakeDamageInfo(this, this, FLAME_DIRECT_DAMAGE,
                                               DMG_BURN | DMG_DIRECT));

    if (!m_hEntAttached->IsNPC() && hl2_episodic.GetBool()) {
      const float ENTITYFLAME_MOVE_AWAY_DIST = 24.0f;
      // Make a sound near my origin, and up a little higher (in case I'm on the
      // ground, so NPC's still hear it)
      CSoundEnt::InsertSound(SOUND_MOVE_AWAY, GetAbsOrigin(),
                             ENTITYFLAME_MOVE_AWAY_DIST, 0.1f, this,
                             SOUNDENT_CHANNEL_REPEATED_DANGER);
      CSoundEnt::InsertSound(
          SOUND_MOVE_AWAY, GetAbsOrigin() + Vector(0, 0, 48.0f),
    ...
    RadiusDamage(CTakeDamageInfo(this, this, FLAME_RADIUS_DAMAGE, DMG_BURN),
                 GetAbsOrigin(), m_flSize / 2, CLASS_NONE, NULL);
  }
