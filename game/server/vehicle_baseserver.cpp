...
//-----------------------------------------------------------------------------
const PassengerSeatAnims_t *
CBaseServerVehicle::NPC_GetPassengerSeatAnims(CBaseCombatCharacter *pPassenger,
...
//		  distance from origin to drop player (allows for different
//shaped vehicles
...
int CBaseServerVehicle::GetExitAnimToUse(Vector &vecEyeExitEndpoint,
  ...
  for (int i = 0; i < iCount; i++) {
    ...
    //		 the player would exit to.  This was rendered unnecessary in
    //Ep2, but the choreo vehicles of these older products 		 did not have proper
    //exit animations and relied on the exact queries that were happening
    //before.  For choreo vehicles, 		 we now just allow them to perform those
    //older queries to keep those products happy.  - jdw
...
sound_states
CBaseServerVehicle::SoundState_ChooseState(vbs_sound_update_t &params) {
