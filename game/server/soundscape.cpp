...
ConVar soundscape_debug(
    "soundscape_debug", "0", FCVAR_CHEAT,
    "When on, draws lines to all env_soundscape entities. Green lines show the "
    "active soundscape, red lines show soundscapes that aren't in range, and "
    "white lines show soundscapes that are in range, but not the active "
    "soundscape.");
...
void CEnvSoundscapeProxy::Activate() {
  if (m_MainSoundscapeName != NULL_STRING) {
    CBaseEntity *pEntity =
        gEntList.FindEntityByName(NULL, m_MainSoundscapeName);
    if (pEntity) {
...
//-----------------------------------------------------------------------------
bool CEnvSoundscape::IsEnabled(void) const { return !m_bDisabled; }
...
bool CEnvSoundscape::InRangeOfPlayer(CBasePlayer *pTarget) {
  ...
  if (m_flRadius > range || m_flRadius == -1) {
    ...
    UTIL_TraceLine(vecSpot1, vecSpot2, MASK_SOLID_BRUSHONLY | MASK_WATER,
...
void CEnvSoundscape::WriteAudioParamsTo(audioparams_t &audio) {
  ...
  for (int i = 0; i < ARRAYSIZE(m_positionNames); i++) {
    if (m_positionNames[i] != NULL_STRING) {
      ...
      CBaseEntity *pEntity =
          gEntList.FindEntityByName(NULL, m_positionNames[i], this, this);
      if (pEntity) {
        audio.localBits |= 1 << i;
...
void CEnvSoundscape::UpdateForPlayer(ss_update_t &update) {
  ...
  if (update.pCurrentSoundscape == this) {
    ...
    if (m_flRadius > range || m_flRadius == -1) {
      ...
      update.traceCount++;
      UTIL_TraceLine(target, update.playerPosition,
                     MASK_SOLID_BRUSHONLY | MASK_WATER, update.pPlayer,
    ...
    if ((!update.bInRange || range < update.currentDistance) &&
        (m_flRadius > range || m_flRadius == -1)) {
      trace_t tr;
  ...
  if (soundscape_debug.GetBool()) {
    // draw myself
    NDebugOverlay::Box(GetAbsOrigin(), Vector(-10, -10, -10),
                       Vector(10, 10, 10), 255, 0, 255, 64,
      ...
      if (audio.ent.Get() != this) {
        if (InRangeOfPlayer(update.pPlayer)) {
          NDebugOverlay::Line(GetAbsOrigin(),
                              update.pPlayer->WorldSpaceCenter(), 255, 255, 255,
                              true, NDEBUG_PERSIST_TILL_NEXT_SERVER);
        } else {
          NDebugOverlay::Line(GetAbsOrigin(),
                              update.pPlayer->WorldSpaceCenter(), 255, 0, 0,
                              true, NDEBUG_PERSIST_TILL_NEXT_SERVER);
        }
      } else {
        if (InRangeOfPlayer(update.pPlayer)) {
          NDebugOverlay::Line(GetAbsOrigin(),
                              update.pPlayer->WorldSpaceCenter(), 0, 255, 0,
                              true, NDEBUG_PERSIST_TILL_NEXT_SERVER);
        } else {
          NDebugOverlay::Line(GetAbsOrigin(),
                              update.pPlayer->WorldSpaceCenter(), 255, 170, 0,
                              true, NDEBUG_PERSIST_TILL_NEXT_SERVER);
        }

        // also draw lines to each sound position.
        // we don't store the number of local sound positions, just a bitvector
        // of which ones are on.
        unsigned int soundbits = audio.localBits.Get();
        float periodic = 2.0f * sin((fmod(gpGlobals->curtime, 2.0f) - 1.0f) *
                                    M_PI); // = -4f .. 4f
    ...
    NDebugOverlay::EntityTextAtPosition(GetAbsOrigin(), 0,
...
void CEnvSoundscape::Spawn() {
  Precache();
  // Because the soundscape has no model, need to make sure it doesn't get
  // culled from the PVS for this reason and therefore
...
void CEnvSoundscape::Precache() {
  ...
  m_soundscapeIndex =
      g_SoundscapeSystem.GetSoundscapeIndex(STRING(m_soundscapeName));
  if (IsX360()) {
  ...
  if (m_debugOverlays &
    ...
    if (pPlayer) {
      ...
      if (audio.ent.Get() != this) {
        ...
        if (pEnt) {
          NDebugOverlay::Line(GetAbsOrigin(), pEnt->WorldSpaceCenter(), 255, 0,
                              255, false, 0);
        }
      }
    }
...
void CEnvSoundscapeTriggerable::DelegateStartTouch(CBaseEntity *pEnt) {
  ...
  // Just in case.. we shouldn't already be in the player's list because it
  // should have called DelegateEndTouch, but this seems to happen when they're
  // noclipping.
...
void CEnvSoundscapeTriggerable::DelegateEndTouch(CBaseEntity *pEnt) {
  ...
  while (pPlayer->m_hTriggerSoundscapeList.Count() > 0) {
    CEnvSoundscapeTriggerable *pSS = dynamic_cast<CEnvSoundscapeTriggerable *>(
        pPlayer->m_hTriggerSoundscapeList[0].Get());
    if (pSS) {
...
void CTriggerSoundscape::Activate() {
  m_hSoundscape = dynamic_cast<CEnvSoundscapeTriggerable *>(
      gEntList.FindEntityByName(NULL, m_SoundscapeName));
  BaseClass::Activate();
}
...
void CTriggerSoundscape::PlayerUpdateThink() {
  ...
  for (i = 1; i <= gpGlobals->maxClients; ++i) {
    ...
    // if the spectator is intersecting the trigger, track it, and start a touch
    // if it is just starting to touch
