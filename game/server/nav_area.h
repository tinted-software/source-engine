...
public:
  ...
  virtual void PostIteration(bool wasCompleteIteration) {
    ...
    // true if the entire set was iterated (ie: Inspect() never returned false)
};
 ...
 */
struct SpotOrder {
  float t; // parametric distance along ray where this spot first has LOS to our
           // path
  union {
...
protected:
  // --- Begin critical data, which is heavily hit during pathing operations and
  // carefully arranged for cache performance [7/24/2008 tom] ---
  ...
  /* 52 */ unsigned char m_playerCount[MAX_NAV_TEAMS]; // the number of players
                                                       // currently in this area

  /* 54 */ bool
      m_isBlocked[MAX_NAV_TEAMS]; // if true, some part of the world is
                                  // preventing movement through this nav area

  /* 56 */ unsigned int m_marker; // used to flag the area as visited
  /* 60 */ float
      m_totalCost; // the distance so far plus an estimate of the distance left
  ...
  /* 100*/ NavLadderConnectVector
      m_ladder[CNavLadder::NUM_LADDER_DIRECTIONS]; // list of ladders leading up
                                                   // and down from this area
  /* 108*/ NavConnectVector
      m_elevatorAreas; // a list of areas reachable via elevator from this area
...
public:
  ...
  virtual void
  OnServerActivate(void); // (EXTEND) invoked when map is initially loaded
  ...
  virtual void
  OnDestroyNotify(CNavArea *dead); // invoked when given area is going away
  virtual void
  OnDestroyNotify(CNavLadder *dead); // invoked when given ladder is going away
  ...
  virtual NavErrorType PostLoad(void); // (EXTEND) invoked after all areas have
                                       // been loaded - for pointer binding, etc
  ...
  void MarkAsBlocked(int teamID, CBaseEntity *blocker,
                     bool bGenerateEvent =
                         true); // An entity can force a nav area to be blocked
  virtual void
  UpdateBlocked(bool force = false,
                int teamID = TEAM_ANY); // Updates the (un)blocked status of the
                                        // nav area (throttled)
  ...
  void
  CheckFloor(CBaseEntity *ignore); // Checks if there is a floor under the nav
                                   // area, in case a breakable floor is gone
  ...
  float GetAvoidanceObstacleHeight(void)
      const; // returns the maximum height of the obstruction above the ground

#ifdef NEXT_BOT
  bool HasPrerequisite(CBaseCombatCharacter *actor = NULL)
      const; // return true if this area has a prerequisite that applies to the
             // given actor
  const CUtlVector<CHandle<CFuncNavPrerequisite>> &
  GetPrerequisiteVector(void) const; // return vector of prerequisites that must
                                     // be met before this area can be traversed
  ...
  void AddFuncNavCostEntity(
      CFuncNavCost
          *cost); // add the given func_nav_cost entity to the cost of this area
  float ComputeFuncNavCost(CBaseCombatCharacter *who)
      const; // return the cost multiplier of this area's func_nav_cost entities
             // for the given actor
  ...
  inline float
  GetZ(const Vector *RESTRICT pPos) const; // return Z of area at (x,y) of 'pos'
  inline float
  GetZ(const Vector &pos) const; // return Z of area at (x,y) of 'pos'
  float GetZ(float x,
             float y) const RESTRICT;     // return Z of area at (x,y) of 'pos'
  bool Contains(const Vector &pos) const; // return true if given point is on or
                                          // above this area, but no others
  bool Contains(const CNavArea *area) const;
  bool
  IsCoplanar(const CNavArea *area) const; // return true if this area and given
                                          // area are approximately co-planar
  void GetClosestPointOnArea(const Vector *RESTRICT pPos, Vector *close)
      const RESTRICT; // return closest point to 'pos' on this area - returned
                      // point in 'close'
  ...
  CNavNode *FindClosestNode(const Vector &pos, NavDirType dir)
      const; // returns the closest node along the given edge to the given point

  bool IsContiguous(const CNavArea *other)
      const; // return true if the given area and 'other' share a colinear edge
             // (ie: no drop-down or step/jump/climb)
  ...
  bool IsEdge(NavDirType dir) const; // return true if there are no
                                     // bi-directional links on the given side
  ...
  bool IsVisible(const Vector &eye, Vector *visSpot = NULL)
      const; // return true if area is visible from the given eyepoint, return
             // visible spot
  ...
  float ComputeGroundHeightChange(
      const CNavArea *area); // compute change in actual ground height from this
                             // area to given area
  ...
  const NavLadderConnectVector *
  GetLadders(CNavLadder::LadderDirectionType dir) const {
    return &m_ladder[dir];
  }
  ...
  //---------------------------------------------------------------------------
  void
  SetClearedTimestamp(int teamID); // set this area's "clear" timestamp to now
  ...
  SpotEncounter *GetSpotEncounter(
      const CNavArea *from,
      const CNavArea *to); // given the areas we are moving between, return the
  ...
  float
  GetDanger(int teamID); // return the danger of this area (decays over time)
  virtual float
  GetDangerDecayRate(void) const; // return danger decay rate per second
  ...
  void
  GetExtent(Extent *extent) const; // return a computed extent (XY is in
                                   // m_nwCorner and m_seCorner, Z is computed)
  ...
  void ComputeNormal(Vector *normal, bool alternate = false)
      const; // Computes the area's normal based on m_nwCorner.  If 'alternate'
  ...
  //--------------------------------------------------------------------------------
  void
  IncrementPlayerCount(int teamID,
                       int entIndex); // add one player to this area's count
  void DecrementPlayerCount(
      int teamID, int entIndex); // subtract one player from this area's count
  unsigned char GetPlayerCount(
      int teamID = 0) const; // return number of players of given team currently
                             // within this area (team of zero means any/all)
  ...
  static CNavArea *
  PopOpenList(void); // remove and return the first element of the open list
  ...
  static void
  ClearSearchLists(void); // clears the open and closed lists for a new search
  ...
  void RaiseCorner(
      ...
      bool raiseAdjacentCorners = true); // raise/lower a corner (or all corners
                                         // if corner == NUM_CORNERS)
  void
  PlaceOnGround(NavCornerType corner,
                float inset = 0.0f); // places a corner (or all corners if
                                     // corner == NUM_CORNERS) on the ground
  ...
  //-------------------------------------------------------------------------
  virtual void
  ComputeHidingSpots(void); // analyze local area neighborhood to find "hiding
                            // spots" in this area - for map learning
  virtual void
  ComputeSniperSpots(void); // analyze local area neighborhood to find "sniper
                            // spots" in this area - for map learning
  virtual void
  ComputeSpotEncounters(void); // compute spot encounter data - for map learning
  ...
  virtual bool
  ComputeLighting(void); // compute 0..1 light intensity at corners and center
                         // (requires client via listenserver)
  ...
  VisibilityType ComputeVisibility(const CNavArea *area, bool isPVSValid,
      ...
      const; // do actual line-of-sight traces to determine if any part of given
             // area is visible from this area
  void SetupPVS(void) const;
  bool
  IsInPVS(void) const; // return true if this area is within the current PVS
  ...
  virtual bool IsEntirelyVisible(const Vector &eye, CBaseEntity *ignore = NULL)
      const; // return true if entire area is visible from given eyepoint (CPU
             // intensive)
  virtual bool IsPartiallyVisible(const Vector &eye, CBaseEntity *ignore = NULL)
  ...
  virtual bool IsPotentiallyVisible(const CNavArea *area)
      const; // return true if given area is potentially visible from somewhere
             // in this area (very fast)
  virtual bool IsPotentiallyVisibleToTeam(
      int team) const; // return true if any portion of this area is visible to
                       // anyone on the given team (very fast)

  virtual bool IsCompletelyVisible(const CNavArea *area)
      const; // return true if given area is completely visible from somewhere
             // in this area (very fast)
  virtual bool IsCompletelyVisibleToTeam(int team)
      const; // return true if given area is completely visible from somewhere
             // in this area by someone on the team (very fast)
   ...
   */
  template <typename Functor> bool ForAllCompletelyVisibleAreas(Functor &func) {
    int i;
...
private:
  ...
  friend class CCSNavArea; // allow CS load code to complete replace our default
                           // load behavior
  ...
  */
  ...
  float m_avoidanceObstacleHeight; // if nonzero, a prop is obstructing movement
                                   // through this nav area
  CountdownTimer m_avoidanceObstacleTimer; // Throttle checks on our obstructed
                                           // state while obstructed

  //- for hunting
  //-------------------------------------------------------------------------------------
  float m_clearedTimestamp[MAX_NAV_TEAMS]; // time this area was last "cleared"
                                           // of enemies

  //- "danger"
  //----------------------------------------------------------------------------------------
  float m_danger[MAX_NAV_TEAMS]; // danger of this area, allowing bots to avoid
                                 // areas where they died in the past - zero is
                                 // no danger
  ...
  //---------------------------------------------------------------------------------
  SpotEncounterVector
      m_spotEncounters; // list of possible ways to move thru this area, and the
                        // spots to look at as we do
  ...
  float m_earliestOccupyTime[MAX_NAV_TEAMS]; // min time to reach this spot from
                                             // spawn
  ...
  NavConnectVector
      m_incomingConnect[NUM_DIRECTIONS]; // a list of adjacent areas for each
                                         // direction that connect TO us, but we
                                         // have no connection back to them
  ...
  void
  FinishSplitEdit(CNavArea *newArea,
                  NavDirType ignoreEdge); // given the portion of the original
                                          // area, update its internal data
  ...
  AreaBindInfo
      m_inheritVisibilityFrom; // if non-NULL, m_potentiallyVisibleAreas becomes
                               // a list of additions and deletions
                               // (NOT_VISIBLE) to the list of this area
  CAreaBindInfoArray
      m_potentiallyVisibleAreas; // list of areas potentially visible from
                                 // inside this area (after PostLoad(), use area
                                 // portion of union)
  bool
      m_isInheritedFrom; // latch used during visibility inheritance computation

  const CAreaBindInfoArray &ComputeVisibilityDelta(const CNavArea *other)
      const; // return a list of the delta between our visibility list and the
             // given adjacent area
...
inline Vector CNavArea::GetCorner(NavCornerType corner) const {
  // @TODO: Confirm compiler does the "right thing" in release builds, or change
  // this function to to take a pointer [2/4/2009 tom]
  Vector pos;
