...
SendPropVector(SENDINFO(m_vecOrigin), -1, SPROP_COORD | SPROP_CHANGES_OFTEN,
    ...
    SendPropInt(SENDINFO(m_iEffectIndex), MAX_PARTICLESYSTEMS_STRING_BITS,
                SPROP_UNSIGNED),
    SendPropBool(SENDINFO(m_bActive)), SendPropFloat(SENDINFO(m_flStartTime)),

    SendPropArray3(SENDINFO_ARRAY3(m_hControlPointEnts),
                   SendPropEHandle(SENDINFO_ARRAY(m_hControlPointEnts))),
    SendPropArray3(SENDINFO_ARRAY3(m_iControlPointParents),
                   SendPropInt(SENDINFO_ARRAY(m_iControlPointParents), 3,
                               SPROP_UNSIGNED)),
    ...
    // DEFINE_FIELD( m_iEffectIndex, FIELD_INTEGER ),	// Don't save. Refind
    // after loading.
    ...
    DEFINE_KEYFIELD(m_iControlPointParents[0], FIELD_CHARACTER,
                    "cpoint1_parent"),
    DEFINE_KEYFIELD(m_iControlPointParents[1], FIELD_CHARACTER,
                    "cpoint2_parent"),
    DEFINE_KEYFIELD(m_iControlPointParents[2], FIELD_CHARACTER,
                    "cpoint3_parent"),
    DEFINE_KEYFIELD(m_iControlPointParents[3], FIELD_CHARACTER,
                    "cpoint4_parent"),
    DEFINE_KEYFIELD(m_iControlPointParents[4], FIELD_CHARACTER,
                    "cpoint5_parent"),
    DEFINE_KEYFIELD(m_iControlPointParents[5], FIELD_CHARACTER,
                    "cpoint6_parent"),
    DEFINE_KEYFIELD(m_iControlPointParents[6], FIELD_CHARACTER,
                    "cpoint7_parent"),
...
// Purpose:
//-----------------------------------------------------------------------------
CParticleSystem::CParticleSystem() { m_bWeatherEffect = false; }
...
// Purpose:
//-----------------------------------------------------------------------------
void CParticleSystem::StartParticleSystemThink(void) { StartParticleSystem(); }
...
void CParticleSystem::StartParticleSystem(void) {
  if (m_bActive == false) {
    ...
    // Setup our control points at this time (in case our targets weren't around
    // at spawn time)
...
// Purpose:
//-----------------------------------------------------------------------------
void CParticleSystem::StopParticleSystem(void) { m_bActive = false; }
...
void CParticleSystem::ReadControlPointEnts(void) {
  for (int i = 0; i < kMAXCONTROLPOINTS; ++i) {
    ...
    CBaseEntity *pPointEnt = gEntList.FindEntityGeneric(
        NULL, STRING(m_iszControlPointNames[i]), this);
