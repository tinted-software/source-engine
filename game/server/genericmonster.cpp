...
DEFINE_EMBEDDED(m_BoneFollowerManager),
    DEFINE_INPUTFUNC(FIELD_VOID, "DisablePlayerCollision",
                     InputDisablePlayerCollision),
    DEFINE_INPUTFUNC(FIELD_VOID, "EnablePlayerCollision",
    ...
    //			now it only initializes moving furniture in scripted
    //sequences
    //-----------------------------------------------------------------------------
  ...
  CapabilitiesAdd(bits_CAP_MOVE_FLY | bits_CAP_TURN_HEAD |
  ...
  // Furniture just wastes CPU doing sensing code, since all they do is idle and
  // play scripts
  GetSenses()->AddSensingFlags(SENSING_FLAGS_DONT_LOOK |
                               SENSING_FLAGS_DONT_LISTEN);
}
...
void CNPC_Furniture::UpdateBoneFollowerState(void) {
  if (m_BoneFollowerManager.GetNumBoneFollowers()) {
    ...
    if (pBone && pBone->hFollower &&
        pBone->hFollower->GetCollisionGroup() != GetCollisionGroup()) {
