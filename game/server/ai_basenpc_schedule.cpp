...
//schedules but with 			different interrupt conditions. Implement this function in
//your 			derived class, and Set or Clear condition bits as you please.
//
//			NOTE: Always call the base class in your implementation,
//but be 				  aware of the difference between changing the bits before vs. 				  changing
//the bits after calling the base implementation.
//
...
//in Half-Life. 			If you change this function, please supply good comments.
//
...
bool CAI_BaseNPC::FindCoverFromBestSound(Vector *pCoverPos) {
  ...
  if (pBestSound) {
    ...
    DevMsg(2, "Attempting to find cover from best sound, but best sound not "
              "founc.\n");
  }
...
void CAI_BaseNPC::StartTask(const Task_t *pTask) {
  ...
  case TASK_FIND_LOCK_HINTNODE: {
    if (!GetHintNode()) {
      SetHintNode(
          CAI_HintManager::FindHint(this, HINT_NONE, pTask->flTaskData, 2000));
    }
    if (GetHintNode()) {
      TaskComplete();
    } else {
  ...
  case TASK_FIND_COVER_FROM_ENEMY: {
    ...
    float flMinDistance =
        (task == TASK_FIND_FAR_NODE_COVER_FROM_ENEMY) ? pTask->flTaskData : 0.0;
  ...
  case TASK_FIND_COVER_FROM_ORIGIN: {
    ...
    if (GetTacticalServices()->FindCoverPos(GetLocalOrigin(), EyePosition(), 0,
  ...
  case TASK_FACE_HINTNODE:
    ...
    GetMotor()->SetIdealYaw(CalcReasonableFacing(
        true)); // CalcReasonableFacing() is based on previously set ideal yaw
    if (FacingIdeal())
      TaskComplete();
    else
      SetTurnActivity();
  ...
  case TASK_GET_PATH_TO_RANGE_ENEMY_LKP_LOS: {
    if (GetEnemy()) {
      ...
      if (dist <= flRange || GetNavigator()->SetVectorGoalFromTarget(
                                 GetEnemy()->GetAbsOrigin(), dist - flRange)) {
        TaskComplete();
        break;
      }
    }
  ...
  case TASK_GET_PATH_TO_ENEMY_LKP_LOS: {
    ...
    if (GetActiveWeapon()) {
      flMaxRange =
          MAX(GetActiveWeapon()->m_fMaxRange1, GetActiveWeapon()->m_fMaxRange2);
      flMinRange =
          MIN(GetActiveWeapon()->m_fMinRange1, GetActiveWeapon()->m_fMinRange2);
  ...
  case TASK_GET_PATH_TO_SAVEPOSITION_LOS: {
    ...
    if (GetTacticalServices()->FindLos(m_vSavePosition, m_vSavePosition,
                                       flMinRange, flMaxRange, 1.0, &posLos)) {
  ...
  case TASK_GET_PATH_TO_TARGET_WEAPON: {
    ...
    if (m_hTargetEnt == NULL) {
      ...
      // verifying line-of-sight, since weapons on the table will not be able to
      // see nodes very nearby.
  ...
  case TASK_MARK_COMMAND_GOAL_POS:
    // Start watching my position to detect whether another AI process has moved
    // me from my mark.
  ...
  case TASK_ENABLE_SCRIPT: {
    ...
    if (m_hCine->m_iszPreIdle != NULL_STRING) {
      ...
      //
#ifdef HL2_EPISODIC
      // dvs: Check current activity rather than ideal activity. Since scripted
      // NPCs early out in MaintainActivity,
...
void CAI_BaseNPC::RunTask(const Task_t *pTask) {
    ...
    if (pTask->flTaskData == 1) {
      ...
      if (GetNavType() == NAV_JUMP) {
        if (GetFlags() & FL_ONGROUND) {
          ...
          SetNavType(
              NAV_GROUND); // this assumes that NAV_JUMP only happens with npcs
                           // that use NAV_GROUND as base movement
      ...
      // navigation can handle commencing 						  a navigation while in the middle of a
      // climb
    ...
    case 0: {
      if (IsPlayerAlly()) {
        ...
        hintCriteria.AddExcludePosition(
            GetAbsOrigin(), 28.0f); // don't plant on an hint that you start on
  ...
  case TASK_PLAY_SCRIPT_POST_IDLE: {
    ...
    //
    // Playing a scripted post idle sequence. Quit early if another sequence has
    // grabbed the NPC.
    //
  ...
  case TASK_FALL_TO_GROUND:
    if (GetFlags() & FL_ONGROUND) {
      ...
      if (IsWaitFinished()) {
        ...
        // bad case where the NPC isn't actually falling, and make an attempt to
        // slam the ground entity to whatever's under the NPC.
...
int CAI_BaseNPC::GetScriptCustomMoveSequence(void) {
  ...
  if (m_hCine != NULL) {
    ...
    if (iSequence == ACTIVITY_NOT_AVAILABLE) {
      Warning("SCRIPT_CUSTOM_MOVE: %s failed scripted custom move. Has no "
              "sequence called: %s\n",
