...
enum {
  ...
  SF_TRIGGER_ONLY_CLIENTS_IN_VEHICLES =
      0x20, // *if* Players can fire this trigger, this flag means only players
            // inside vehicles can
  SF_TRIGGER_ALLOW_ALL =
      0x40, // Everything can fire this trigger EXCEPT DEBRIS!
  SF_TRIGGER_ONLY_CLIENTS_OUT_OF_VEHICLES =
      0x200, // *if* Players can fire this trigger, this flag means only players
             // outside vehicles can
...
public:
  CTriggerHurt() {
    // This field came along after levels were built so the field defaults to 20
  ...
  float m_flDmgResetTime; // For forgiveness, the time to reset the counter that
                          // accumulates damage.
