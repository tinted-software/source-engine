...
SendProp SendPropEHandle(const char *pVarName, int offset, int sizeofVar,
                         int flags, SendVarProxyFn proxyFn) {
  return SendPropInt(pVarName, offset, sizeofVar, NUM_NETWORKED_EHANDLE_BITS,
                     SPROP_UNSIGNED | flags, proxyFn);
}

SendProp SendPropIntWithMinusOneFlag(const char *pVarName, int offset,
                                     int sizeofVar, int nBits,
...
static void SendProxy_Time(const SendProp *pProp, const void *pStruct,
                           const void *pVarData, DVariant *pOut, int iElement,
                           int objectID) {
...
SendProp SendPropTime(const char *pVarName, int offset, int sizeofVar) {
  //	return SendPropInt( pVarName, offset, sizeofVar, TIME_BITS, 0,
  //SendProxy_Time );
