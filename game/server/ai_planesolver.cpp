...
CAI_PlaneSolver::CAI_PlaneSolver(CAI_BaseNPC *pNpc)
    : m_pNpc(pNpc), m_fSolvedPrev(false),
      m_PrevTarget(FLT_MAX, FLT_MAX, FLT_MAX), m_PrevSolution(0),
      m_ClosestHaveBeenToCurrent(FLT_MAX), m_TimeLastProgress(FLT_MAX),
      m_fCannotSolveCurrent(false),
      m_RefreshSamplesTimer(PLANE_SOLVER_THINK_FREQUENCY[AIStrongOpt()] -
                            0.05) {}

//-----------------------------------------------------------------------------
// Convenience accessors
//-----------------------------------------------------------------------------
inline CAI_BaseNPC *CAI_PlaneSolver::GetNpc() { return m_pNpc; }

inline CAI_Motor *CAI_PlaneSolver::GetMotor() { return m_pNpc->GetMotor(); }
...
bool CAI_PlaneSolver::MoveLimit(Navigation_t navType, const Vector &target,
...
bool CAI_PlaneSolver::MoveLimit(Navigation_t navType, const Vector &target,
  ...
  return MoveLimit(navType, target, ignoreTransients, fCheckStep, MASK_NPCSOLID,
                   pMoveTrace);
}
...
bool CAI_PlaneSolver::DetectUnsolvable(const AILocalMoveGoal_t &goal) {
  ...
  float curDistance =
      (goal.target.AsVector2D() - GetLocalOrigin().AsVector2D()).Length();
...
float CAI_PlaneSolver::AdjustRegulationWeight(CBaseEntity *pEntity,
                                              float weight) {
  if (pEntity->MyNPCPointer() != NULL) {
    // @TODO (toml 10-03-02): How to do this with non-NPC entities. Should be
    // using intended solve velocity...
    Vector2D velOwner = GetNpc()->GetMotor()->GetCurVel().AsVector2D();
    Vector2D velBlocker =
        ((CAI_BaseNPC *)pEntity)->GetMotor()->GetCurVel().AsVector2D();
...
float CAI_PlaneSolver::CalculateRegulationWeight(const AIMoveTrace_t &moveTrace,
                                                 float pctBlocked) {
  float weight = 0;
...
void CAI_PlaneSolver::GenerateSuggestionFromTrace(
    const AILocalMoveGoal_t &goal, const AIMoveTrace_t &moveTrace,
  ...
  if (moveTrace.fStatus == AIMR_BLOCKED_NPC) {
    Vector vecToOther =
        moveTrace.pObstruction->GetLocalOrigin() - GetLocalOrigin();
  ...
  suggestion.Set(type, (favorRight) ? favoredWeight : weight,
                 arcCenter - thirdSpan, thirdSpan, moveTrace.pObstruction);
...
void CAI_PlaneSolver::CalcYawsFromOffset(float yawScanCenter,
...
void CAI_PlaneSolver::GenerateObstacleNpcs(const AILocalMoveGoal_t &goal,
                                           float probeDist) {
  if (!ProbeForNpcs()) {
    ...
    float radiusSelf =
        (minsSelf.AsVector2D() - maxsSelf.AsVector2D()).Length() * 0.5;

    for (int i = 0; i < g_AI_Manager.NumAIs(); i++) {
      CAI_BaseNPC *pAI = ppAIs[i];
      if (pAI != m_pNpc && pAI->IsAlive() &&
          (!goal.pPath || pAI != goal.pPath->GetTarget())) {
        Vector mins, maxs;
        ...
        if (mins.z < maxsSelf.z + 12.0 && maxs.z > minsSelf.z - 12.0) {
          ...
          float distance = (pAI->GetAbsOrigin().AsVector2D() -
                            m_pNpc->GetAbsOrigin().AsVector2D())
                               .Length();
    ...
    if (pPlayer) {
      ...
      if (mins.z < maxsSelf.z + 12.0 && maxs.z > minsSelf.z - 12.0) {
        ...
        float distance = (pPlayer->GetAbsOrigin().AsVector2D() -
                          m_pNpc->GetAbsOrigin().AsVector2D())
                             .Length();
        if (distance - radius < radiusSelf + probeDist) {
          AddObstacle(pPlayer->WorldSpaceCenter(), radius, pPlayer,
                      AIMST_AVOID_NPC);
        }
      }
    }
...
AI_SuggestorResult_t CAI_PlaneSolver::GenerateObstacleSuggestion(
  ...
  CalcYawsFromOffset(yawScanCenter, spanPerProbe, probeOffset, &yawTest,
                     &arcCenter);
  ...
  if (probeDist > requiredMovement) {
    if (!MoveLimit(goal.navType, GetLocalOrigin() + probeDir * requiredMovement,
                   !ProbeForNpcs(), true, &moveTrace)) {
      fTraceClear = false;
      moveTrace.flDistObstructed =
          (probeDist - requiredMovement) + moveTrace.flDistObstructed;
    }
  }

  if (fTraceClear) {
    fTraceClear =
        MoveLimit(goal.navType, GetLocalOrigin() + probeDir * probeDist,
                  !ProbeForNpcs(), false, &moveTrace);
  }

  if (!fTraceClear) {
    GenerateSuggestionFromTrace(goal, moveTrace, probeDist, arcCenter,
                                spanPerProbe, probeOffset);
    return SR_OK;
  }
...
AI_SuggestorResult_t CAI_PlaneSolver::GenerateObstacleSuggestions(
  ...
  if (PLANE_SOLVER_THINK_FREQUENCY[AIStrongOpt()] == 0.0 ||
    ...
    // Generate suggestion from direct trace, or probe if direct trace doesn't
    // match
    if (fabs(probeDist - (distClear + directTrace.flDistObstructed)) < 0.1 &&
      ...
      if (directTrace.fStatus != AIMR_OK) {
        seekResult = SR_OK;
        GenerateSuggestionFromTrace(goal, directTrace, probeDist, yawGoalDir,
                                    spanPerProbe, 0);
      }
    } else if (GenerateObstacleSuggestion(goal, yawGoalDir, probeDist,
                                          spanPerProbe, 0) == SR_OK) {
      seekResult = SR_OK;
    }

    // Scan left. Note that in the left and right scan, the algorithm stops as
    // soon as there is a clear path. This is an optimization in anticipation of
    // the behavior of the underlying solver. This will break more often the
    // higher PLANE_SOLVER_THINK_FREQUENCY becomes
    bool foundClear = false;

    for (i = 1; i <= nSideProbes; i++) {
      if (!foundClear) {
        AI_SuggestorResult_t curSeekResult = GenerateObstacleSuggestion(
            goal, yawGoalDir, probeDist, spanPerProbe, i);
        ...
        m_Solver.AddRegulation(AI_MoveSuggestion_t(AIMST_NO_KNOWLEDGE, 1,
                                                   arcCenter, spanPerProbe));
      }
    }
    ...
    if (seekResult == SR_OK) {
      ...
      // Since these are not sampled every think, place a negative arc in all
      // directions not sampled
      m_Solver.AddRegulation(AI_MoveSuggestion_t(
          AIMST_NO_KNOWLEDGE, 1, arcCenter, 360 - degreesToProbe));
    }
...
void CAI_PlaneSolver::VisualizeSolution(const Vector &vecGoal,
...
void CAI_PlaneSolver::AdjustSolutionForFliers(const AILocalMoveGoal_t &goal,
                                              float flSolutionYaw,
                                              Vector *pSolution) {
  ...
  *pSolution *= flRadius;
  ...
  // Move up 0 when we have to move forward as much as we have to move down z
  // (45 degree angle) Move up max when we have to move forward 5x as much as we
  // have to move down z, or if we have to move up z.
...
unsigned
CAI_PlaneSolver::ComputeTurnBiasFlags(const AILocalMoveGoal_t &goal,
                                      const AIMoveTrace_t &directTrace) {
  if (directTrace.fStatus == AIMR_BLOCKED_WORLD) {
    // @TODO (toml 11-11-02): stuff plane normal of hit into trace Use here to
    // compute a bias?
    //
    return 0;
  }
...
bool CAI_PlaneSolver::RunMoveSolver(const AILocalMoveGoal_t &goal,
                                    const AIMoveTrace_t &directTrace,
  ...
  if (m_Solver.HaveRegulations()) {
    // @TODO (toml 07-19-02): add a movement threshhold here (the target may be
    // the same, but the ai is nowhere near where the last solution was derived)
    bool fNewTarget = (!m_fSolvedPrev || m_PrevTarget != goal.target);
    ...
    moveSuggestions[0].Set(AIMST_MOVE, 1, UTIL_VecToYaw(goal.dir),
                           degreesPositiveArc);
    moveSuggestions[0].flags |= ComputeTurnBiasFlags(goal, directTrace);

    if (fDeterOscillation && !fNewTarget) {
#ifndef TESTING_SUGGESTIONS
      moveSuggestions[nSuggestions++].Set(AIMST_OSCILLATION_DETERRANCE, 1,
                                          m_PrevSolution - 180, 180);
#endif
    }
    ...
    if (m_Solver.Solve(moveSuggestions, nSuggestions, &solution)) {
      *pResult = UTIL_YawToVector(solution.dir);
      ...
      // A crude attempt at oscillation detection: if we solved last time, and
      // this time, and the same target is involved, and we resulted in nearly a
      // 180, we are probably oscillating
#ifndef TESTING_SUGGESTIONS
      if (!fNewTarget) {
      ...
      if (curVelocity != vec3_origin) {
        ...
        if (!fNewTarget) {
          *pResult =
              curVelocity * 0.1 + m_PrevSolutionVector * 0.1 + *pResult * 0.8;
        } else {
...
void CAI_PlaneSolver::AddObstacle(const Vector &center, float radius,
                                  CBaseEntity *pEntity,
                                  AI_MoveSuggType_t type) {
...
//-----------------------------------------------------------------------------
bool CAI_PlaneSolver::GenerateCircleObstacleSuggestions(
    const AILocalMoveGoal_t &moveGoal, float probeDist) {
  bool result = false;
  ...
  for (int i = 0; i < m_Obstacles.Count(); i++) {
    ...
    if (m_Obstacles[i].hEntity && m_Obstacles[i].hEntity->CollisionProp()) {
      ...
      if (pObstacleEntity == moveGoal.pMoveTarget &&
          (pObstacleEntity->IsNPC() || pObstacleEntity->IsPlayer())) {
        // HEY! I'm trying to avoid the very thing I'm trying to get to. This
        // will make we wobble like a drunk as I approach. Don't do it.
        continue;
      }

      pObstacleEntity->CollisionProp()->WorldSpaceSurroundingBounds(&mins,
                                                                    &maxs);
    ...
    if (distToObstacleSq > 0.001 && distToObstacleSq < sq(radius + probeDist)) {
      ...
      // Don't steer around to avoid obstacles we've already passed, unless
      // we're right up against them. That is, do this computation without the
      // probeDist added in.
      ...
      if (radiusSq < distToObstacleSq) {
        ...
        vecTangent.x = (-distToTangent * vecToNpc.x + radius * vecToNpc.y) *
                       oneOverDistToObstacleSq;
        vecTangent.y = (-distToTangent * vecToNpc.y - radius * vecToNpc.x) *
                       oneOverDistToObstacleSq;
        vecTangent.z = 0;
        ...
        NDebugOverlay::Line(npcLoc - Vector(0, 0, 64),
                            npcLoc + Vector(0, 0, 64), 0, 255, 0, false, 0.1);
        NDebugOverlay::Line(center - Vector(0, 0, 64),
                            center + Vector(0, 0, 64), 0, 255, 0, false, 0.1);
        NDebugOverlay::Line(pointTangent - Vector(0, 0, 64),
                            pointTangent + Vector(0, 0, 64), 0, 255, 0, false,
                            0.1);

        NDebugOverlay::Line(npcLoc + Vector(0, 0, 64),
                            center + Vector(0, 0, 64), 0, 0, 255, false, 0.1);
        NDebugOverlay::Line(center + Vector(0, 0, 64),
                            pointTangent + Vector(0, 0, 64), 0, 0, 255, false,
                            0.1);
        NDebugOverlay::Line(pointTangent + Vector(0, 0, 64),
                            npcLoc + Vector(0, 0, 64), 0, 0, 255, false, 0.1);
#endif
      } else {
        arc = 210;
        weight = 1.0;
      ...
      AI_MoveSuggestion_t suggestion(m_Obstacles[i].type, weight,
                                     UTIL_VecToYaw(vecToObstacle), arc);
      m_Solver.AddRegulation(suggestion);
      result = true;
    }
  }
...
bool CAI_PlaneSolver::Solve(const AILocalMoveGoal_t &goal, float distClear,
                            Vector *pSolution) {
  bool solved = false;
  ...
  float degreesPositiveArc = (!fVeryClose)
                                 ? DEGREES_POSITIVE_ARC
                                 : DEGREES_POSITIVE_ARC_CLOSE_OBSTRUCTION;
  ...
  if (GenerateObstacleSuggestions(goal, goal.directTrace, distClear, probeDist,
                                  degreesPositiveArc, NUM_PROBES) != SR_FAIL) {
    if (RunMoveSolver(goal, goal.directTrace, degreesPositiveArc, !fVeryClose,
                      pSolution)) {
      ...
      MoveLimit(goal.navType, GetLocalOrigin() + *pSolution * requiredMovement,
                false, true, &moveTrace);
  ...
  m_fSolvedPrev =
      (solved &&
       goal.speed !=
           0); // a solution found when speed is zero is not meaningful
