...
DEFINE_EMBEDDED(m_args),
    ...
    DEFINE_EMBEDDED(m_LostLOSTimer), END_DATADESC();
...
bool CAI_LeadBehavior::Connect(CAI_LeadBehaviorHandler *pSink) {
  ...
  if (m_hSinkImplementor == NULL)
    DevMsg(2, "Note: CAI_LeadBehaviorHandler connected to a sink that isn't an "
...
DEFINE_FIELD(m_fArrived, FIELD_BOOLEAN),
    ...
    DEFINE_OUTPUT(m_OnFailure, "OnFailure"), DEFINE_OUTPUT(m_OnDone, "OnDone"),
...
void CAI_LeadGoal::InputActivate(inputdata_t &inputdata) {
  ...
  CAI_LeadBehavior *pBehavior = GetLeadBehavior();
  if (!pBehavior) {
    DevMsg("Lead goal entity activated for an NPC that doesn't have the lead "
           "behavior\n");
    return;
  }
#ifdef HL2_EPISODIC
  if ((m_flLeadDistance * 4) < m_flRetrieveDistance) {
    Warning("ai_goal_lead '%s': lead distance (%.2f) * 4 is < retrieve "
            "distance (%.2f). This will make the NPC act stupid. Either reduce "
            "the retrieve distance, or increase the lead distance.\n",
...
#ifdef HL2_EPISODIC
    Warning("ai_goal_lead '%s': retrieve distance (%.2f) < lead distance "
            "(%.2f) + %d. Retrieve distance should be at least %d greater than "
            "the lead distance, or NPC will ping-pong while retrieving.\n",
...
DEFINE_KEYFIELD(m_iszWeaponName, FIELD_STRING, "WeaponName"),
    ...
    const
    char *CAI_LeadGoal_Weapon::GetConceptModifiers(const char *pszConcept) {
