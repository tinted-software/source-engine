...
CTacticalMissionManager &TheTacticalMissions(void) {
  static CTacticalMissionManager *manager =
      g_pGameRules->TacticalMissionManagerFactory();
...
CON_COMMAND_F(mission_list, "List all available tactical missions",
              FCVAR_GAMEDLL) {
...
CON_COMMAND_F(mission_show, "Show the given mission", FCVAR_GAMEDLL) {
  ...
  const CTacticalMission *mission =
      TheTacticalMissions().GetMission(args.Arg(1));
  if (mission) {
 ...
 * If functor returns false, stop iterating and return false.
 */
bool CTacticalMissionManager::ForEachMission(
    CTacticalMissionManager::IForEachMission &func) {
  FOR_EACH_VEC(m_missionVector, it) {
