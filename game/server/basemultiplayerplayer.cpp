...
// Purpose:
//-----------------------------------------------------------------------------
void CBaseMultiplayerPlayer::ModifyOrAppendCriteria(
...
// Purpose:
//-----------------------------------------------------------------------------
bool CBaseMultiplayerPlayer::SpeakIfAllowed(AIConcept_t concept,
...
IResponseSystem *CBaseMultiplayerPlayer::GetResponseSystem() {
  ...
  //	return
  //<*>GameRules()->m_ResponseRules[iIndex].m_ResponseSystems[m_iCurrentConcept];
}

//-----------------------------------------------------------------------------
// Purpose: Doesn't actually speak the concept. Just finds a response in the
// system. You then have to play it yourself.
//-----------------------------------------------------------------------------
...
// Purpose:
//-----------------------------------------------------------------------------
bool CBaseMultiplayerPlayer::SpeakConceptIfAllowed(int iConcept,
                                                   const char *modifiers,
                                                   char *pszOutResponseChosen,
                                                   size_t bufsize,
                                                   IRecipientFilter *filter) {
  ...
  m_iCurrentConcept = iConcept;
  return SpeakIfAllowed(g_pszMPConcepts[iConcept], modifiers,
                        pszOutResponseChosen, bufsize, filter);
}
...
bool CBaseMultiplayerPlayer::CanHearAndReadChatFrom(CBasePlayer *pPlayer) {
  ...
  if (m_iIgnoreGlobalChat == CHAT_IGNORE_TEAM &&
...
// Purpose:
//-----------------------------------------------------------------------------
bool CBaseMultiplayerPlayer::ShouldRunRateLimitedCommand(
...
int CBaseMultiplayerPlayer::CalculateTeamBalanceScore(void) {
  ...
  if (flLastSwitchedTime > 0 &&
      (gpGlobals->curtime - flLastSwitchedTime) < 300) {
    iScore -= 10000;
  }
  return iScore;
}
...
void DumpAchievementCounters(const CCommand &args) {
  ...
  CBaseMultiplayerPlayer *pPlayer =
      ToBaseMultiplayerPlayer(UTIL_PlayerByIndex(iPlayerIndex));
...
ConCommand dump_achievement_counters(
    "dump_achievement_counters", DumpAchievementCounters,
...
ConVar
    tf_escort_score_rate("tf_escort_score_rate", "1", FCVAR_CHEAT,
...
void CBaseMultiplayerPlayer::EscortScoringThink(void) {
  ...
  SetContextThink(&CBaseMultiplayerPlayer::EscortScoringThink,
                  gpGlobals->curtime + ESCORT_SCORE_INTERVAL,
