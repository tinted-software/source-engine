...
static ConCommand
    global_set("global_set", CC_Global_Set,
               "global_set <globalname> <state>: Sets the state of the given "
               "env_global (0 = OFF, 1 = ON, 2 = DEAD).",
               FCVAR_CHEAT);
...
private:
  ...
  bool m_bHitMin; // Set when we reach or go below our minimum value, cleared if
                  // we go above it again.
...
//a default 			output if the input value didn't match any of the case values.
//
...
// fields.
//				We use a variant so that we can convert any
//input type to a string.
//-----------------------------------------------------------------------------
...
private:
  ...
  COutputFloat m_OnNotEqualTo; // Fired when the input value is not equal to the
                               // compare value.
...
DEFINE_KEYFIELD(m_flCompareValue, FIELD_FLOAT, "CompareValue"),
    ...
    //-----------------------------------------------------------------------------
    // Purpose: Input handler for a new input value without performing a
    // comparison.
    //-----------------------------------------------------------------------------
  ...
  for (int i = 0; i < MAX_LOGIC_BRANCH_NAMES; i++) {
    ...
    while ((pEntity = gEntList.FindEntityGeneric(
      ...
      if (FClassnameIs(pEntity, "logic_branch")) {
        ...
        DevWarning("logic_branchlist %s refers to entity %s, which is not a "
                   "logic_branch\n",
