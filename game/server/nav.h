...
enum NavAttributeType {
  ...
  NAV_MESH_TRANSIENT =
      0x00000100, // area may become blocked, and should be periodically checked
  NAV_MESH_DONT_HIDE =
  ...
  NAV_MESH_STAIRS = 0x00001000,   // this area represents stairs, do not attempt
  ...
  NAV_MESH_OBSTACLE_TOP =
      0x00004000, // this nav area is the climb point on the tip of an obstacle
  NAV_MESH_CLIFF =
      0x00008000, // this nav area is adjacent to a drop of at least CliffHeight

  NAV_MESH_FIRST_CUSTOM = 0x00010000, // apps may define custom app-specific
                                      // bits starting with this value
  NAV_MESH_LAST_CUSTOM = 0x04000000, // apps must not define custom app-specific
                                     // bits higher than with this value
  ...
  NAV_MESH_NAV_BLOCKER =
      0x80000000 // area is blocked by nav blocker ( Alas, needed to hijack a
                 // bit in the attributes to get within a cache line [7/24/2008
                 // tom])
};
...
inline bool IsEntityWalkable(CBaseEntity *entity, unsigned int flags) {
  ...
  if (FClassnameIs(entity, "func_door*")) {
    ...
    if (!entity->HasSpawnFlags(SF_DOOR_PTOUCH)) {
      // this door is not opened by touching it, if it is closed, the area is
      // blocked
  ...
  // if we hit a breakable object, assume its walkable because we will shoot it
  // when we touch it
