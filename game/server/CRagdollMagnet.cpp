...
// Input  : &inputdata -
//-----------------------------------------------------------------------------
void CRagdollMagnet::InputDisable(inputdata_t &inputdata) { Enable(false); }

//-----------------------------------------------------------------------------
// Purpose: Find the ragdoll magnet entity that should pull this entity's
// ragdoll Input  : *pNPC - the npc that's dying Output : CRagdollMagnet - the
// magnet that's best to use.
//
// NOTES:
//
...
CRagdollMagnet *CRagdollMagnet::FindBestMagnet(CBaseEntity *pNPC) {
  ...
  do {
    pMagnet = (CRagdollMagnet *)gEntList.FindEntityByClassname(
        pMagnet, "phys_ragdollmagnet");
...
Vector CRagdollMagnet::GetForceVector(CBaseEntity *pNPC) {
  ...
  if (IsBarMagnet()) {
    ...
    CalcClosestPointOnLineSegment(pNPC->WorldSpaceCenter(), GetAbsOrigin(),
                                  m_axis, vecClosest, NULL);
...
//-----------------------------------------------------------------------------
// Purpose: How far away is this point? This is different for point and bar
// magnets Input  : &vecPoint - the point Output : float - the dist
//-----------------------------------------------------------------------------
float CRagdollMagnet::DistToPoint(const Vector &vecPoint) {
  if (IsBarMagnet()) {
    ...
    // A bar magnet is a cylinder who's length is AbsOrigin() to m_axis, and
    // whose diameter is m_radius.
