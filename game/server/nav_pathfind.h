    ...
    while (true) {
      ...
      if (searchWhere == SEARCH_FLOOR) {
        ...
        if (ladderUp) {
          ...
          // do not use BEHIND connection, as its very hard to get to when going
          // up a ladder
...
public:
  ...
  /**
   * Collect adjacent areas to continue the search by calling 'IncludeInSearch'
   * on each
   */
...
inline void CollectSurroundingAreas(CUtlVector<CNavArea *> *nearbyAreaVector,
  ...
  if (startArea) {
    ...
    while (!CNavArea::IsOpenListEmpty()) {
      ...
      if (area->GetParent()) {
        float deltaZ =
            area->GetParent()->ComputeAdjacentConnectionHeightChange(area);
      ...
      // mark here to ensure all marked areas are also valid areas that are in
      // the collection
      area->Mark();
...
CNavArea *FindMinimumCostArea(CNavArea *startArea, CostFunctor &costFunc) {
  ...
  FOR_EACH_VEC(TheNavAreas, iter) {
    ...
    // HPE_FIX[pfreese]: changed this to only pass three parameters, in accord
    // with the two functors above
...
void SelectSeparatedShuffleSet(int maxCount, float minSeparation,
  ...
  for (i = 0; i < shuffledVector.Count(); ++i) {
    ...
    CollectSurroundingAreas(&nearVector, area, minSeparation, 2.0f * StepHeight,
                            2.0f * StepHeight);
