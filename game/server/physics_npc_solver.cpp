...
public:
  ...
  ~CPhysicsNPCSolver();
  DECLARE_DATADESC();
  void Init(CAI_BaseNPC *pNPC, CBaseEntity *pPhysicsObject,
            bool disableCollisions, float separationTime);
  static CPhysicsNPCSolver *Create(CAI_BaseNPC *pNPC,
...
private:
  ...
  bool IsIntersecting();
  bool IsContactOnNPCHead(IPhysicsFrictionSnapshot *pSnapshot,
                          IPhysicsObject *pPhysics, CAI_BaseNPC *pNPC);
  bool CheckTouching();
  friend bool NPCPhysics_SolverExists(CAI_BaseNPC *pNPC,
                                      CBaseEntity *pPhysicsObject);
...
CPhysicsNPCSolver *CPhysicsNPCSolver::Create(CAI_BaseNPC *pNPC,
                                             CBaseEntity *pPhysicsObject,
                                             bool disableCollisions,
                                             float separationTime) {
  CPhysicsNPCSolver *pSolver = (CPhysicsNPCSolver *)CBaseEntity::CreateNoSpawn(
      "physics_npc_solver", vec3_origin, vec3_angle, NULL);
...
void CPhysicsNPCSolver::Init(CAI_BaseNPC *pNPC, CBaseEntity *pPhysicsObject,
                             bool disableCollisions, float separationTime) {
  m_hNPC = pNPC;
...
bool CPhysicsNPCSolver::IsIntersecting() {
  ...
  if (pNPC && pPhysics) {
    ...
    trace_t tr;
    enginetrace->ClipRayToEntity(ray, pNPC->PhysicsSolidMaskForEntity(),
                                 pPhysics, &tr);
    if (tr.startsolid)
      return true;
  }
...
bool CPhysicsNPCSolver::IsContactOnNPCHead(IPhysicsFrictionSnapshot *pSnapshot,
...
bool CPhysicsNPCSolver::CheckTouching() {
  ...
  // if the object is penetrating something, check to see if it's intersecting
  // this NPC if so, go ahead and switch over to penetration solver mode
...
CPhysicsNPCSolver::Simulate(IPhysicsMotionController *pController,
                            IPhysicsObject *pObject, float deltaTime,
                            Vector &linear, AngularImpulse &angular) {
  if (IsIntersecting()) {
    ...
    Vector pushImpulse;
    PhysComputeSlideDirection(pObject, dir * PUSH_SPEED, angVel, &pushImpulse,
                              NULL, 0);
...
CBaseEntity *NPCPhysics_CreateSolver(CAI_BaseNPC *pNPC,
                                     CBaseEntity *pPhysicsObject,
                                     bool disableCollisions,
  ...
  return CPhysicsNPCSolver::Create(pNPC, pPhysicsObject, disableCollisions,
                                   separationDuration);
}
...
public:
  DECLARE_DATADESC();
  void Init(CBaseEntity *pMovingEntity, CBaseEntity *pPhysicsBlocker,
            float separationTime);
  static CPhysicsEntitySolver *Create(CBaseEntity *pMovingEntity,
  ...
  // IMotionEvent
  // virtual simresult_e	Simulate( IPhysicsMotionController *pController,
  // IPhysicsObject *pObject, float deltaTime, Vector &linear, AngularImpulse
  // &angular );
...
DEFINE_FIELD(m_hMovingEntity, FIELD_EHANDLE),
    ...
    END_DATADESC()
        ...
        CPhysicsEntitySolver *CPhysicsEntitySolver::Create(
            CBaseEntity *pMovingEntity, CBaseEntity *pPhysicsBlocker,
            float separationTime) {
  CPhysicsEntitySolver *pSolver =
      (CPhysicsEntitySolver *)CBaseEntity::CreateNoSpawn(
          "physics_entity_solver", vec3_origin, vec3_angle, NULL);
...
void CPhysicsEntitySolver::Init(CBaseEntity *pMovingEntity,
                                CBaseEntity *pPhysicsBlocker,
                                float separationTime) {
...
CBaseEntity *EntityPhysics_CreateSolver(CBaseEntity *pMovingEntity,
                                        CBaseEntity *pPhysicsObject,
                                        bool disableCollisions,
                                        float separationDuration) {
  ...
  return CPhysicsEntitySolver::Create(pMovingEntity, pPhysicsObject,
                                      separationDuration);
}
