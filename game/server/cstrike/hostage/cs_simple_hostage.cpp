...
ConVar mp_hostagepenalty("mp_hostagepenalty", "13", FCVAR_NOTIFY,
                         "Terrorist are kicked for killing too much hostages");
ConVar hostage_debug("hostage_debug", "0", FCVAR_CHEAT,
...
// We need hostage-specific pushaway cvars because the hostage doesn't have the
// same friction etc as players
ConVar sv_pushaway_hostage_force(
    "sv_pushaway_hostage_force", "20000", FCVAR_REPLICATED | FCVAR_CHEAT,
    "How hard the hostage is pushed away from physics objects (falls off with "
    "inverse square of distance).");
ConVar sv_pushaway_max_hostage_force(
    "sv_pushaway_max_hostage_force", "1000", FCVAR_REPLICATED | FCVAR_CHEAT,
...
DEFINE_INPUTFUNC(FIELD_VOID, "OnRescueZoneTouch", HostageRescueZoneTouch),
    ...
    SendPropBool(SENDINFO(m_isRescued)), SendPropInt(SENDINFO(m_iHealth), 10),
...
static CountdownTimer
    announceTimer; // used to stop "hostage rescued" announcements from stepping
                   // on each other
...
CWeaponCSBase *CHostage::CSAnim_GetActiveWeapon() { return NULL; }

bool CHostage::CSAnim_CanMove() { return true; }
...
void CHostage::Spawn(void) {
  ...
  // we must set this, because its zero by default thus putting their eyes in
  // their feet
  ...
  SetContextThink(&CHostage::PushawayThink,
  ...
  Vector GroundPos = DropToGround(this, GetAbsOrigin(), HOSTAGE_BBOX_VEC_MIN,
                                  HOSTAGE_BBOX_VEC_MAX);
...
//-----------------------------------------------------------------------------------------------------
void CHostage::TraceAttack(const CTakeDamageInfo &info, const Vector &vecDir,
  ...
  if (hostageKillLimit > 0) {
    ...
    if (player->m_iHostagesKilled == hostageKillLimit - 1) {
      ...
      engine->ServerCommand(
          UTIL_VarArgs("kickid %d \"For killing too many hostages\"\n",
                       player->GetUserID()));
    }
  }
}
...
void CHostage::HostageRescueZoneTouch(inputdata_t &inputdata) {
  if (!m_isRescued) {
    ...
    //=============================================================================
      ...
      if (CSGameRules()->m_iNumRescuers == 1 &&
          !CSGameRules()->WasHostageKilled()) {
...
void CHostage::Touch(CBaseEntity *other) {
  ...
  if ((other->IsPlayer() && other->GetTeamNumber() == TEAM_CT) ||
      FClassnameIs(other, "hostage_entity")) {
    // only push in 2D
    ...
    ApplyForce(pushForce * to);
             ...
             (other->ClassMatches("func_door*") ||
              other->ClassMatches("prop_door*"))) {
...
void CHostage::UpdateFollowing(float deltaT) {
  if (!IsFollowingSomeone() && m_lastLeaderID != 0) {
    ...
    IGameEvent *event =
        gameeventmanager->CreateEvent("hostage_stops_following");
    if (event) {
  ...
  if (leader) {
    ...
    if (toLeader.IsLengthGreaterThan(giveUpRange) ||
        (m_path.IsValid() && m_path.GetLength() > maxPathLength)) {
    ...
    if (m_isWaitingForLeader) {
      ...
      // we are far from our leader, and need to check if we're close enough to
      // wait
    ...
    if (!m_isWaitingForLeader) {
      ...
      m_pathFollower.Update(deltaT,
                            m_inhibitObstacleAvoidanceTimer.IsElapsed());
...
void CHostage::AvoidPhysicsProps(void) {
  ...
  int nEnts = GetPushawayEnts(this, props, ARRAYSIZE(props), 0.0f,
  ...
  if (!m_accel.IsZero()) {
    ...
    UTIL_TraceEntity(this, start, start + forward, MASK_PLAYERSOLID, this,
...
void CHostage::HostageThink(void) {
  ...
  // keep track of which Navigation Area we are in (or were in, if we're "off
  // the mesh" right now)
...
//-----------------------------------------------------------------------------------------------------
bool CHostage::IsFollowingSomeone(void) { return (m_leader.m_Value != NULL); }
...
//-----------------------------------------------------------------------------------------------------
bool CHostage::IsValid(void) const { return (m_iHealth > 0 && !IsRescued()); }
...
//-----------------------------------------------------------------------------------------------------
bool CHostage::IsRescued(void) const { return m_isRescued; }

//-----------------------------------------------------------------------------------------------------
bool CHostage::IsOnGround(void) const { return (GetFlags() & FL_ONGROUND); }
...
bool CHostage::IsVisible(const Vector &pos, bool testFOV) const {
  trace_t result;
  UTIL_TraceLine(EyePosition(), pos, CONTENTS_SOLID, this, COLLISION_GROUP_NONE,
                 &result);
 ...
 * Stand idle
 */
void CHostage::Idle(void) { m_leader = NULL; }
...
void CHostage::Follow(CCSPlayer *leader) {
  //=============================================================================
  // HPE_BEGIN
  // [dwenger] Set variable to track whether player is currently rescuing
  // hostages
  //=============================================================================
 ...
 */
void CHostage::HostageUse(CBaseEntity *pActivator, CBaseEntity *pCaller,
 ...
 */
float CHostage::GetMoveAngle(void) const { return GetAbsAngles().y; }

//-----------------------------------------------------------------------------------------------------
CNavArea *CHostage::GetLastKnownArea(void) const { return m_lastKnownArea; }

//-----------------------------------------------------------------------------------------------------
/**
 * Find "simple" ground height, treating current nav area as part of the floo
 */
bool CHostage::GetSimpleGroundHeightWithFloor(const Vector &pos, float *height,
                                              Vector *normal) {
...
//-----------------------------------------------------------------------------------------------------
void CHostage::Crouch(void) { m_isCrouching = true; }

//-----------------------------------------------------------------------------------------------------
/**
 * un-crouch
 */
void CHostage::StandUp(void) { m_isCrouching = false; }

//-----------------------------------------------------------------------------------------------------
bool CHostage::IsCrouching(void) const { return m_isCrouching; }
...
//-----------------------------------------------------------------------------------------------------
bool CHostage::IsJumping(void) const { return !m_jumpTimer.IsElapsed(); }

//-----------------------------------------------------------------------------------------------------
/**
 * Set movement speed to running
 */
void CHostage::Run(void) { m_isRunning = true; }

//-----------------------------------------------------------------------------------------------------
/**
 * Set movement speed to walking
 */
void CHostage::Walk(void) { m_isRunning = false; }

//-----------------------------------------------------------------------------------------------------
bool CHostage::IsRunning(void) const { return m_isRunning; }

//-----------------------------------------------------------------------------------------------------
/**
 * Invoked when a ladder is encountered while following a path
 */
void CHostage::StartLadder(const CNavLadder *ladder, NavTraverseType how,
                           const Vector &approachPos, const Vector &departPos) {
}
 ...
 */
bool CHostage::TraverseLadder(const CNavLadder *ladder, NavTraverseType how,
                              const Vector &approachPos,
                              const Vector &departPos, float deltaT) {
  return false;
}

//-----------------------------------------------------------------------------------------------------
bool CHostage::IsUsingLadder(void) const { return false; }

//-----------------------------------------------------------------------------------------------------
/**
 * Move Hostage directly toward "pathGoal", causing Hostage to track the current
 * path.
 */
