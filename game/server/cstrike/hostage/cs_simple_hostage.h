...
public:
  ...
  void HostageRescueZoneTouch(
      inputdata_t &inputdata); // invoked when hostage touches a rescue zone

  void HostageThink(void); // periodic update to initiate behaviors

  void GiveCTUseBonus(
      CCSPlayer *rescuer); // give bonus to CT's for talking to a hostage
  void
  CheckForHostageAbuse(CCSPlayer *player); // check for hostage-killer abuse
  ...
  bool IsVisible(const Vector &pos, bool testFOV = false)
      const; // return true if hostage can see position
  ...
  void FaceTowards(const Vector &target,
                   float deltaT); // rotate body to face towards "target"
...
public:
  ...
  virtual void
  StartLadder(const CNavLadder *ladder, NavTraverseType how,
              const Vector &approachPos,
              const Vector &departPos); // invoked when a ladder is encountered
                                        // while following a path
  virtual bool TraverseLadder(const CNavLadder *ladder, NavTraverseType how,
                              const Vector &approachPos,
                              const Vector &departPos,
                              float deltaT); // traverse given ladder
  virtual bool IsUsingLadder(void) const;

  virtual void
  TrackPath(const Vector &pathGoal,
            float deltaT); // move along path by following "pathGoal"
  virtual void OnMoveToSuccess(
      const Vector &goal); // invoked when an improv reaches its MoveTo goal
  virtual void
  OnMoveToFailure(const Vector &goal,
                  MoveToFailureType reason); // invoked when an improv fails to
                                             // reach a MoveTo goal
...
protected:
  virtual void HostageUse(CBaseEntity *pActivator, CBaseEntity *pCaller,
 ...
 * Once we hook up crouching and ladders, this can be removed and
 * ShortestPathCost() can be used instead.
 */
  ...
  float operator()(CNavArea *area, CNavArea *fromArea, const CNavLadder *ladder,
    ...
    if (fromArea == NULL) {
      ...
      if (area->GetAttributes() &
          NAV_MESH_CROUCH) //  && !(area->GetAttributes() & NAV_MESH_JUMP))
      {
 ...
 */
template <typename Functor> bool ForEachHostage(Functor &func) {
