...
DEFINE_THINKFUNC(Think_Detonate), DEFINE_THINKFUNC(Think_Fade),
    DEFINE_THINKFUNC(Think_Remove) END_DATADESC()
  ...
  CSmokeGrenadeProjectile *pGrenade =
      (CSmokeGrenadeProjectile *)CBaseEntity::Create("smokegrenade_projectile",
                                                     position, angles, pOwner);
...
void CSmokeGrenadeProjectile::Think_Detonate() {
  ...
  ParticleSmokeGrenade *pGren = (ParticleSmokeGrenade *)CBaseEntity::Create(
      PARTICLESMOKEGRENADE_ENTITYNAME, GetAbsOrigin(), QAngle(0, 0, 0), NULL);
  if (pGren) {
    pGren->FillVolume();
    ...
    while ((pEntity = gEntList.FindEntityByClassname(
                pEntity, "hostage_entity")) != NULL) {
      // send to hostages that have a resonable chance of being in it while its
      // still smoking
    ...
    if (player) {
      IGameEvent *event =
          gameeventmanager->CreateEvent("smokegrenade_detonate");
      if (event) {
...
void CSmokeGrenadeProjectile::Think_Fade() {
  ...
  if (!c.a) {
    ...
    SetThink(&CSmokeGrenadeProjectile::Think_Remove); // Spit out smoke for 10
                                                      // seconds.
    SetSolid(SOLID_NONE);
  }
}
