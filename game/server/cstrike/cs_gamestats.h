...
const float cDisseminationTimeHigh =
    0.25f; // Time interval for high priority stats sent to the player
const float cDisseminationTimeLow =
    2.5f; // Time interval for medium priority stats sent to the player
...
struct sHappyCamperSnipePosition {
  sHappyCamperSnipePosition(int userid, Vector pos)
      : m_iUserID(userid), m_vPos(pos) {}
...
struct SMarketPurchases : public BaseStatData {
  SMarketPurchases(uint64 ulPlayerID, int iPrice, const char *pName)
      : m_nPlayerID(ulPlayerID), ItemCost(iPrice) {
    if (pName) {
...
struct SCSSDeathData : public BaseStatData {
  SCSSDeathData(CBasePlayer *pVictim, const CTakeDamageInfo &info) {
    ...
    const char *pszMapName =
        gpGlobals->mapname.ToCStr() ? gpGlobals->mapname.ToCStr() : "unknown";
...
public:
  ...
  virtual void Event_ShotFired(CBasePlayer *pPlayer,
  ...
  virtual void Event_PlayerKilled_PreWeaponDrop(CBasePlayer *pPlayer,
                                                const CTakeDamageInfo &info);
  ...
  void Event_PlayerDamage(CBasePlayer *pBasePlayer,
                          const CTakeDamageInfo &info);
  void Event_BombExploded(CCSPlayer *pPlayer);
  void Event_MoneyEarned(CCSPlayer *pPlayer, int moneyEarned);
  void Event_MoneySpent(CCSPlayer *pPlayer, int moneySpent,
  ...
  void CalcDominationAndRevenge(CCSPlayer *pAttacker, CCSPlayer *pVictim,
                                int *piDeathFlags);
  void CalculateOverkill(CCSPlayer *pAttacker, CCSPlayer *pVictim);
  void PlayerKilled(CBasePlayer *pVictim, const CTakeDamageInfo &info);

  void IncrementStat(CCSPlayer *pPlayer, CSStatType_t statId, int iValue,
...
private:
  PlayerStats_t
      m_aPlayerStats[MAX_PLAYERS + 1]; // List of stats for each player for
                                       // current life - reset after each death
  ...
  float m_fDisseminationTimerLow;  // how long since last medium priority stat
                                   // update
  float m_fDisseminationTimerHigh; // how long since last high priority stat
                                   // update
  ...
  // A static list of all the stat containers, one for each data structure being
  // tracked
