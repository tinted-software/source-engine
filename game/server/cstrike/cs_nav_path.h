...
class CCSNavPath {
public:
  CCSNavPath(void) { m_segmentCount = 0; }
  ...
  bool IsAtEnd(const Vector &pos)
      const; ///< return true if position is at the end of the path

  float GetLength(void) const; ///< return length of path from start to finish
  bool GetPointAlongPath(float distAlong, Vector *pointOnPath)
      const; ///< return point a given distance along the path - if distance is
             ///< out of path bounds, point is clamped to start/end
  ...
  void Draw(const Vector &color = Vector(1.0f, 0.3f,
                                         0)); ///< draw the path for debugging

  /// compute closest point on path to given point
  bool FindClosestPointOnPath(const Vector *worldPos, int startIndex,
  ...
  bool Compute(const Vector &start, const Vector &goal, CostFunctor &costFunc) {
    ...
    CNavArea *startArea =
        TheNavMesh->GetNearestNavArea(start + Vector(0.0f, 0.0f, 1.0f));
    if (startArea == NULL) {
      return false;
    }
...
private:
  ...
  bool
  BuildTrivialPath(const Vector &start,
                   const Vector &goal); ///< utility function for when start and
                                        ///< goal are in the same area
...
#define DONT_AVOID_OBSTACLES false
  void Update(float deltaT,
              bool avoidObstacles = true);        ///< move improv along path
...
private:
  ...
  int FindOurPositionOnPath(
      Vector *close, bool local) const; ///< return the closest point to our
                                        ///< current position on current path
  int FindPathPoint(float aheadRange, Vector *point,
                    int *prevIndex); ///< compute a point a fixed distance ahead
                                     ///< along our path.
