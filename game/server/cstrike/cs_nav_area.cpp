...
#ifdef _WIN32
#pragma warning(                                                               \
    disable : 4701) // disable warning that variable *may* not be initialized
#endif
 ...
 */
CCSNavArea::CCSNavArea(void) { m_approachCount = 0; }
...
void CCSNavArea::OnServerActivate(void) { CNavArea::OnServerActivate(); }

void CCSNavArea::OnRoundRestart(void) { CNavArea::OnRoundRestart(); }
...
NavErrorType CCSNavArea::Load(CUtlBuffer &fileBuffer, unsigned int version,
  ...
  case 1:
    ...
    for (int a = 0; a < m_approachCount; ++a) {
      ...
      m_approach[a].prevToHereHow =
          (NavTraverseType)fileBuffer.GetUnsignedChar();

      m_approach[a].next.id = fileBuffer.GetUnsignedInt();
      m_approach[a].hereToNextHow =
          (NavTraverseType)fileBuffer.GetUnsignedChar();
    }
...
void CCSNavArea::Draw(void) const { CNavArea::Draw(); }
 ...
 */
NavErrorType CCSNavArea::LoadLegacy(CUtlBuffer &fileBuffer,
