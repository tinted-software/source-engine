...
//=============================================================================
static void SvNoMVPChangeCallback(IConVar *pConVar, const char *pOldValue,
                                  float flOldValue) {
...
ConVar sv_nomvp("sv_nomvp", "0", 0, "Disable MVP awards.",
                SvNoMVPChangeCallback);
ConVar sv_disablefreezecam("sv_disablefreezecam", "0", FCVAR_REPLICATED,
...
ConVar sv_legacy_grenade_damage("sv_legacy_grenade_damage", "0",
                                FCVAR_REPLICATED,
                                "Enable to replicate grenade damage behavior "
                                "of the original Counter-Strike Source game.");
...
void CCSPlayer::InitialSpawn(void) {
  ...
  if (!engine->IsDedicatedServer() && TheNavMesh->IsOutOfDate() &&
  ...
  //=============================================================================
  // HPE_BEGIN:
  // [tj] We reset the stats at the beginning of the map (including domination
  // tracking)
  //=============================================================================
...
void CCSPlayer::Spawn() {
  ...
  // [tj] Reset this flag if the player is not in observer mode (as happens when
  // a player spawns late)
  ...
  if (!engine->IsDedicatedServer() && CSGameRules()->m_iTotalRoundsPlayed < 2 &&
      TheNavMesh->IsOutOfDate() && this == UTIL_GetListenServerHost()) {
    ClientPrint(
        this, HUD_PRINTCENTER,
        "The Navigation Mesh was built using a different version of this map.");
  }
  ...
  if (GetActiveWeapon() && !IsObserver()) {
    ...
    m_flNextAttack =
        gpGlobals->curtime; // Allow reloads to finish, since we're playing the
                            // deploy anim instead.  This mimics goldsrc
                            // behavior, anyway.
  }
...
int CCSPlayer::GetPercentageOfEnemyTeamKilled() {
  if (m_NumEnemiesAtRoundStart > 0) {
    return (int)(((float)m_NumEnemiesKilledThisRound /
                  (float)m_NumEnemiesAtRoundStart) *
                 100.0f);
  }
...
void CCSPlayer::Event_Killed(const CTakeDamageInfo &info) {
  ...
  // [menglish] Keep track of what the player has dropped for the freeze panel
  // callouts
  //=============================================================================

  CBaseEntity *pAttacker = info.GetAttacker();
  bool friendlyFire =
      pAttacker && pAttacker->GetTeamNumber() == GetTeamNumber();
  ...
  if ((mp_holiday_nogifts.GetBool() == false) &&
      UTIL_IsHolidayActive(3 /*kHoliday_Christmas*/)) {
  ...
  if (!roundWasAlreadyWon && roundIsWonNow) {
    ...
    for (int i = 1; i <= gpGlobals->maxClients; i++) {
      ...
      if (pPlayer) {
        ...
        int nKills = CCS_GameStats.FindPlayerStats(pPlayer)
                         .statsCurrentRound[CSSTAT_KILLS];
        int nDamage = CCS_GameStats.FindPlayerStats(pPlayer)
...
// Event_Killed).
void CCSPlayer::Event_KilledOther(CBaseEntity *pVictim,
...
void CCSPlayer::UpdateMouseoverHints() {
  ...
  if (tr.fraction != 1.0f) {
    if (tr.DidHitNonWorldEntity() && tr.m_pEnt) {
      ...
      case CLASS_PLAYER: {
        const float grenadeBloat =
            1.2f; // Be conservative in estimating what a player can distinguish
...
int CCSPlayer::OnTakeDamage(const CTakeDamageInfo &inputInfo) {
  ...
  if (m_bIsDefusing && m_gooseChaseStep == GC_NONE && pAttacker &&
      pAttacker->GetTeamNumber() != GetTeamNumber()) {
  ...
  if (bFriendlyFire || pInflictor->GetTeamNumber() != GetTeamNumber() ||
    ...
    // HPE_BEGIN:
    //=============================================================================
    // [menglish] Store whether or not the knife did this damage as knives do
    // bullet damage, so we need to specifically check the weapon here
    bool bKnifeDamage = false;
    CCSPlayer *pPlayer = ToCSPlayer(info.GetAttacker());

    if (pPlayer) {

      // [paquin. forest] if  this is blast damage, and we haven't opted out
      // with a cvar, we need to get the armor ratio out of the inflictor

      if ((info.GetDamageType() & DMG_BLAST) &&
          !sv_legacy_grenade_damage.GetBool()) {

        // [paquin] if we know this is a grenade, use it's armor ratio,
        // otherwise use the he grenade armor ratio

        CBaseCSGrenadeProjectile *pGrenade =
            dynamic_cast<CBaseCSGrenadeProjectile *>(pInflictor);
        ...
        if (pWeapon) {
          ...
          // Knives do bullet damage, so we need to specifically check the
          // weapon here
          bKnifeDamage =
              pWeapon->GetCSWpnData().m_WeaponType == WEAPONTYPE_KNIFE;
    ...
    if (ArmorValue() && !(info.GetDamageType() & (DMG_FALL | DMG_DROWN)) &&
      ...
      //=============================================================================
      if (m_LastHitGroup == HITGROUP_HEAD && flDamage > m_iHealth &&
          fDamageToHealth < m_iHealth) {
    ...
    //=============================================================================
    // HPE_BEGIN:
    // [menglish] Achievement award for kill stealing i.e. killing an enemy who
    // was very damaged from other players [Forrest] Moved this check before
    // RecordDamageTaken so that the damage currently being dealt by this player
    //           won't disqualify them from getting the achievement.
    //=============================================================================

    if (m_iHealth - info.GetDamage() <= 0 &&
        m_iHealth <= AchievementConsts::KillLowDamage_MaxHealthLeft) {
      ...
      if (pAttacker && pAttacker->GetTeamNumber() != GetTeamNumber()) {
        ...
        FOR_EACH_LL(m_DamageTakenList, i) {
          if (Q_strncmp(pAttacker->GetPlayerName(),
                        m_DamageTakenList[i]->GetPlayerName(),
    ...
    Q_snprintf(outputString, sizeof(outputString),
               "%f: Player %s incoming %f damage from %s, type %s; applied %d "
               "health and %d armor\n",
               gpGlobals->curtime, GetPlayerName(), inputInfo.GetDamage(),
               info.GetInflictor()->GetDebugName(), dmgtype, m_lastDamageHealth,
               m_lastDamageArmor);
...
void CCSPlayer::TraceAttack(const CTakeDamageInfo &info, const Vector &vecDir,
  ...
  //=============================================================================
  // HPE_BEGIN:
  // [menglish] Used when calculating the position this player was hit at in the
  // bone space
  //=============================================================================
  ...
  if (bHitShield) {
    ...
    //=============================================================================
    // HPE_BEGIN:
    // [menglish] Calculate the position this player was hit at in the bone
    // space
    //=============================================================================
    ...
    VectorTransform(ptr->endpos, boneTransformToObject,
                    m_vLastHitLocationObjectSpace);
...
void CCSPlayer::AddAccount(int amount, bool bTrackChange, bool bItemBought,
...
#ifndef _XBOX
  // CS would like their players to continue to update their LastArea since it
  // is displayed in the hud voice chat UI But we won't do the population
  // tracking while dead.
...
BuyResult_e CCSPlayer::AttemptToBuyAssaultSuit(void) {
  // WARNING: This price logic also exists in
  // C_CSPlayer::GetCurrentAssaultSuitPrice and must be kept in sync if changes
  // are made.
...
//=============================================================================
{
  ...
  if (pWeaponInfo == NULL) {
    ...
    if (bPurchase) {
      ...
      AddAccount(-pWeaponInfo->GetWeaponPrice(), true, true,
...
BuyResult_e CCSPlayer::BuyGunAmmo(CBaseCombatWeapon *pWeapon,
  ...
  if (m_iAccount >= GetCSAmmoDef()->GetCost(nAmmo)) {
    ...
    AddAccount(-GetCSAmmoDef()->GetCost(nAmmo), true, true,
               GetCSAmmoDef()->GetAmmoOfIndex(nAmmo)->pName);
    return BUY_BOUGHT;
  }
  ...
  */
                                  ...
                                  // team so they can just change their model
  {
    ...
    if (!madeRoom) {
      ...
      ClientPrint(this, HUD_PRINTCENTER,
                  (team == TEAM_TERRORIST) ? "#Too_Many_Terrorists"
                                           : "#Too_Many_CTs");
...
bool CCSPlayer::HandleCommand_JoinClass(int iClass) {
  ...
  if (!IsBot() &&
      State_Get() == STATE_ACTIVE) // Bots are responsible about only switching
                                   // classes when they join.
  {
...
void CCSPlayer::GetIntoGame() {
  ...
  if (!MPRules->FPlayerCanRespawn(this)) {
    ...
    // pev->flags		   &= ( FL_PROXY | FL_FAKECLIENT );	// clear
    // flags, but keep proxy and bot flags that might already be set pev->flags
  ...
  } else // else spawn them right in
  {
    ...
    //=============================================================================
    // HPE_BEGIN:
    // [menglish] Have the rules update anything related to a player spawning in
    // late
    //=============================================================================
...
void CCSPlayer::State_Enter_WELCOME() {
  ...
  if (!CSGameRules()->IsLogoMap()) {
            ...
            "-makereslists")) // don't show the MOTD when making reslists
    {
      ...
      data->SetInt(
          "cmd", TEXTWINDOW_CMD_JOINGAME); // exec this command if panel closed
...
void CCSPlayer::State_PreThink_DEATH_ANIM() {
  ...
  float fFreezeEnd = fDeathEnd + spec_freeze_traveltime.GetFloat() +
                     spec_freeze_time.GetFloat();
...
void CCSPlayer::State_Enter_DEATH_WAIT_FOR_KEY() {
  ...
  // Don't do this.  The ragdoll system expects to be able to read from this
  // player on the next update and will read it at the new origin if this is
  // set. Since it is more complicated to redesign the ragdoll system to not
  // need that data it is easier to cause a less obvious bug than popping
  // ragdolls
...
bool CCSPlayer::BumpWeapon(CBaseCombatWeapon *pBaseWeapon) {
  ...
  */
  ...
  if (bPickupGrenade || !Weapon_SlotOccupied(pWeapon)) {
    ...
    if (pDonor && pDonor != this &&
        pWeapon->GetCSWpnData().GetWeaponPrice() > m_iAccount) {
...
void CCSPlayer::ParseAutoBuyString(const char *string, bool &boughtPrimary,
  ...
  char command[32];
  int nBuffSize = sizeof(command) -
                  1; // -1 to leave space for the NULL at the end of the string
  const char *c = string;
  ...
  if (primary == NULL) {
    ...
    if (HasShield()) {
      ...
      m_rebuyStruct.m_szPrimaryWeapon[0] =
          0; // if we don't have a shield and we don't have a primary weapon, we
             // got nuthin.
...
BuyResult_e CCSPlayer::RebuyPrimaryWeapon() {
  ...
  if (primary != NULL) {
    return BUY_ALREADY_HAVE; // don't drop primary weapons via rebuy - if the
                             // player picked up a different weapon, he wants to
                             // keep it.
  }
...
BuyResult_e CCSPlayer::RebuySecondaryWeapon() {
  ...
  if (pistol != NULL && !m_bUsingDefaultPistol) {
    return BUY_ALREADY_HAVE; // don't drop pistols via rebuy if we've bought one
                             // other than the default pistol
  }
...
CBaseEntity *CCSPlayer::FindUseEntity() {
  ...
  // Check to see if the bomb is close enough to use before attempting to use
  // anything else.

  if (CSGameRules()->IsBombDefuseMap() && GetTeamNumber() == TEAM_CT) {
    // This is done separately since there might be something blocking our LOS
    // to it but we might want to use it anyway if it's close enough.  This
    ...
    if (bomb != NULL) {
      ...
      if (vecLOS.LengthSqr() <
          (96 *
           96)) // 64 is the distance in Goldsrc.  However since Goldsrc did
                // distance from the player's origin and we're doing distance
                // from the player's eye, make the radius a bit bigger.
      {
...
// HPE_BEGIN:
// [tj] Added a parameter so we know if it was death that caused the drop
// [menglish] Clear all previously dropped equipment and add the c4 to the
// dropped equipment
//=============================================================================
...
void CCSPlayer::DropWeapons(bool fromDeath, bool friendlyFire) {
  ...
  // NOTE: Function continues beyond comment block. This is just the part I
  // touched.
  ...
  if (HasDefuser()) {
    ...
    if (fromDeath) {
      m_hDroppedEquipment[DROPPED_DEFUSE] =
          static_cast<CBaseEntity *>(pDefuser);
    }
...
void CCSPlayer::ChangeTeam(int iTeamNum) {
  ...
  if (iTeamNum == TEAM_UNASSIGNED) {
    ...
    //=============================================================================
    // HPE_BEGIN:
    // [tj] Removed these lines so players keep their money when switching to
    // spectator.
    //=============================================================================
    // Reset money
    // m_iAccount = 0;
  ...
  } else // active player
  {
    if (iOldTeam == TEAM_SPECTATOR) {
      ...
      //		if they have less than the default, give them the
      //default.
      //=============================================================================
...
// [menglish, tj] This is where we reset all the per-round information for
 ...
 *
 *		Helper function to get the ID of the weapon used to kill a
 *player. This is slightly non-trivial because the grenade because a separate
 *		entity when thrown.
 *
 *  Parameters:
 * 		info -
...
CSWeaponID CCSPlayer::GetWeaponIdCausingDamange(const CTakeDamageInfo &info) {
  ...
  if (pAttacker == pInflictor) {
    CWeaponCSBase *pAttackerWeapon =
        dynamic_cast<CWeaponCSBase *>(pAttacker->GetActiveWeapon());
    if (!pAttackerWeapon)
      return WEAPON_NONE;

    return pAttackerWeapon->GetWeaponID();
  } else if (pInflictor && V_strcmp(pInflictor->GetClassname(),
...
void CCSPlayer::PlayerUsedFirearm(CBaseCombatWeapon *pBaseWeapon) {
  if (pBaseWeapon) {
    ...
    if (pWeapon) {
      ...
      if (weaponType != WEAPONTYPE_KNIFE && weaponType != WEAPONTYPE_C4 &&
 ...
 * 		info -
 */
void CCSPlayer::ProcessPlayerDeathAchievements(CCSPlayer *pAttacker,
                                               CCSPlayer *pVictim,
                                               const CTakeDamageInfo &info) {
  Assert(pVictim != NULL);
  CBaseEntity *pInflictor = info.GetInflictor();
  ...
  if (pAttacker != NULL && pVictim != NULL &&
    ...
    CWeaponCSBase *pVictimWeapon =
        dynamic_cast<CWeaponCSBase *>(pVictim->GetActiveWeapon());
    ...
    if (pVictim->IsRescuing()) {
      ...
      if (!pAttacker->InjuredAHostage() &&
          pVictim->GetNumFollowers() == g_Hostages.Count()) {
        pAttacker->AwardAchievement(CSKilledRescuer);
        pAttacker->m_bKilledRescuer = true;
      }
    }

    // [menglish] Achievement check for doing 95% or more damage to a player and
    // having another player kill them
    FOR_EACH_LL(pVictim->m_DamageTakenList, i) {
      if (pVictim->m_DamageTakenList[i]->GetDamage() >=
              pVictim->GetMaxHealth() -
                  AchievementConsts::DamageNoKill_MaxHealthLeftOnKill &&
          Q_strncmp(pAttacker->GetPlayerName(),
                    pVictim->m_DamageTakenList[i]->GetPlayerName(),
                    MAX_PLAYER_NAME_LENGTH) != 0) {
        ...
        for (int j = 1; j <= MAX_PLAYERS; j++) {
          ...
          if (pPlayerIter &&
              Q_strncmp(pPlayerIter->GetPlayerName(),
                        pVictim->m_DamageTakenList[i]->GetPlayerName(),
                        MAX_PLAYER_NAME_LENGTH) == 0 &&
    ...
    for (int avengedIndex = 0;
         avengedIndex < pVictim->m_enemyPlayersKilledThisRound.Count();
         avengedIndex++) {
      CCSPlayer *avengedPlayer =
          pVictim->m_enemyPlayersKilledThisRound[avengedIndex];

      if (avengedPlayer) {
        // Make sure you are avenging someone on your own team (This is the
        // expected flow. Just here to avoid edge cases like team-switching).
        if (pAttacker->GetTeamNumber() == avengedPlayer->GetTeamNumber()) {
          CCS_GameStats.Event_PlayerAvengedTeammate(
              pAttacker, pVictim->m_enemyPlayersKilledThisRound[avengedIndex]);
        }
      }
    }
    ...
    while (pAttacker->m_killTimes.Count() > 0 &&
           pAttacker->m_killTimes[0] +
                   AchievementConsts::KillingSpree_WindowTime <
               gpGlobals->curtime) {
      pAttacker->m_killTimes.Remove(0);
    }

    // If we killed enough players in the time window, award the achievement
    if (pAttacker->m_killTimes.Count() >=
        AchievementConsts::KillingSpree_Kills) {
    ...
    if (pVictim->m_KillingSpreeStartTime >= 0 &&
        pVictim->m_KillingSpreeStartTime - gpGlobals->curtime <=
            AchievementConsts::KillingSpreeEnder_TimeWindow) {
    ...
    if (pAttacker->m_NumEnemiesKilledThisRound ==
            pAttacker->m_NumEnemiesAtRoundStart &&
        pAttacker->m_NumEnemiesKilledThisRound >=
            AchievementConsts::KillEnemyTeam_MinKills) {
    ...
    if (pAttacker->GetActiveWeapon() &&
        pAttacker->GetActiveWeapon()->Clip1() == 0 && pAttackerWeapon &&
        pAttackerWeapon->GetCSWpnData().m_WeaponType !=
    ...
    if (pVictimWeapon &&
        pVictimWeapon->GetCSWpnData().m_WeaponType == WEAPONTYPE_KNIFE &&
        pAttackerWeapon &&
    ...
    if (attackerWeaponId != WEAPON_NONE) {
      if (pAttacker->m_killWeapons.Find(attackerWeaponId) == -1) {
        ...
        if (pAttacker->m_killWeapons.Count() >=
            AchievementConsts::KillsWithMultipleGuns_MinWeapons) {
    ...
    if (pAttacker->IsBlindForAchievement()) {
      // if this is from a different blinding, restart the kill counter and set
      // the time
      ...
      if (pAttacker->m_killsWhileBlind >=
          AchievementConsts::KillEnemiesWhileBlind_Kills) {
        pAttacker->AwardAchievement(CSKillEnemiesWhileBlind);
      }

      if (pAttacker->m_killsWhileBlind >=
          AchievementConsts::KillEnemiesWhileBlindHard_Kills) {
    ...
    if (pAttacker->IsAlive() &&
        pAttacker->GetHealth() <=
            AchievementConsts::KillWhenAtLowHealth_MaxHealth) {
    ...
    //[tj] See if the attacker or defender are in the air [sbodenbender] dont
    //include ladders
    bool attackerInAir =
        pAttacker->GetMoveType() != MOVETYPE_LADDER &&
        pAttacker->GetNearestSurfaceBelow(
            AchievementConsts::KillInAir_MinimumHeight) == NULL;
    bool victimInAir = pVictim->GetMoveType() != MOVETYPE_LADDER &&
                       pVictim->GetNearestSurfaceBelow(
                           AchievementConsts::KillInAir_MinimumHeight) == NULL;
    ...
    if (pVictim->HasC4() && pVictim->GetBombPickuptime() +
                                    AchievementConsts::KillBombPickup_MaxTime >
                                gpGlobals->curtime) {
      pAttacker->AwardAchievement(CSKillBombPickup);
    }
  }

  // If you kill a friendly player while blind (from an enemy player), give the
  // guy that blinded you an achievement
  if (pAttacker != NULL && pVictim != NULL &&
      pVictim->GetTeamNumber() == pAttacker->GetTeamNumber() &&
      pAttacker->IsBlind()) {
    CCSPlayer *flashbangAttacker = pAttacker->GetLastFlashbangAttacker();
    if (flashbangAttacker &&
        pAttacker->GetTeamNumber() != flashbangAttacker->GetTeamNumber()) {
  ...
  if (pAlivePlayer) {
    ...
    int alivePlayerOpposingTeam =
        alivePlayerTeam == TEAM_CT ? TEAM_TERRORIST : TEAM_CT;
    if (livePlayerCount == 1 &&
        ...
        teamCount[alivePlayerOpposingTeam] -
                teamIgnoreCount[alivePlayerOpposingTeam] >=
...
//[tj]  traces up to maxTrace units down and returns any standable object it
//hits
...
CBaseEntity *CCSPlayer::GetNearestSurfaceBelow(float maxTrace) {
  ...
  Vector minExtent = this->m_Local.m_bDucked ? VEC_DUCK_HULL_MIN_SCALED(this)
                                             : VEC_HULL_MIN_SCALED(this);
  Vector maxExtent = this->m_Local.m_bDucked ? VEC_DUCK_HULL_MAX_SCALED(this)
...
//      It is important to note that this happens before the bomb explodes, so a
//      player may die after this from a bomb explosion or a late kill after a
//      defuse/detonation/rescue.
void CCSPlayer::OnRoundEnd(int winningTeam, int reason) {
  if (winningTeam == WINNER_CT || winningTeam == WINNER_TER) {
    ...
    if (IsAlive() &&
        gpGlobals->curtime - CSGameRules()->GetRoundStartTime() <
            AchievementConsts::FastRoundWin_Time &&
    ...
    if (IsAlive() && reason == Target_Bombed &&
    ...
    if (IsAlive() && reason == Bomb_Defused &&
        m_defuseDefenseStep == DD_KILLED_TERRORIST) {
      AwardAchievement(CSDefuseDefense);
    }

    // Check silent win
    if (m_NumEnemiesKilledThisRound > 0 && GetTeamNumber() == winningTeam &&
    ...
    if (GetTeamNumber() == winningTeam && !m_bMadePurchseThisRound) {
      ...
      if (m_roundsWonWithoutPurchase >
          AchievementConsts::WinRoundsWithoutBuying_Rounds) {
...
void CCSPlayer::OnPreResetRound() {
  ...
  if (IsAlive() && m_grenadeDamageTakenThisRound >
                       AchievementConsts::SurviveGrenade_MinDamage) {
  ...
  if (IsAlive()) {
    ...
    if (numberOfEnemyDamagers >=
        AchievementConsts::SurviveManyAttacks_NumberDamagingPlayers) {
...
void CCSPlayer::AttemptToExitFreezeCam(void) {
  float fEndFreezeTravel = m_flDeathTime + CS_DEATH_ANIMATION_TIME +
                           spec_freeze_traveltime.GetFloat();
...
//-----------------------------------------------------------------------------
// Purpose: Sets the number of rounds this player has caused to be won for their
// team
//-----------------------------------------------------------------------------
void CCSPlayer::SetNumMVPs(int iNumMVP) { m_iMVPs = iNumMVP; }
//-----------------------------------------------------------------------------
// Purpose: Returns the number of rounds this player has caused to be won for
// their team
//-----------------------------------------------------------------------------
...
void CCSPlayer::CommitSuicide(bool bExplode /*= false*/,
                              bool bForce /*= false*/) {
  m_wasNotKilledNaturally = true;
  BaseClass::CommitSuicide(bExplode, bForce);
}

void CCSPlayer::CommitSuicide(const Vector &vecForce, bool bExplode /*= false*/,
                              bool bForce /*= false*/) {
  m_wasNotKilledNaturally = true;
...
int CCSPlayer::GetNumEnemyDamagers() {
  ...
  FOR_EACH_LL(m_DamageTakenList, i) {
    for (int j = 1; j <= MAX_PLAYERS; j++) {
      ...
      if (pPlayer &&
          V_strncmp(pPlayer->GetPlayerName(),
                    m_DamageTakenList[i]->GetPlayerName(),
                    MAX_PLAYER_NAME_LENGTH) == 0 &&
...
int CCSPlayer::GetNumEnemiesDamaged() {
  ...
  FOR_EACH_LL(m_DamageGivenList, i) {
    for (int j = 1; j <= MAX_PLAYERS; j++) {
      ...
      if (pPlayer &&
          V_strncmp(pPlayer->GetPlayerName(),
                    m_DamageGivenList[i]->GetPlayerName(),
                    MAX_PLAYER_NAME_LENGTH) == 0 &&
          pPlayer->GetTeamNumber() != GetTeamNumber()) {
