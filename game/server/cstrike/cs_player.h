...
enum {
  MIN_NAME_CHANGE_INTERVAL =
      10, // minimum number of seconds between name changes
  NAME_CHANGE_HISTORY_SIZE = 5, // number of times a player can change names in
                                // NAME_CHANGE_HISTORY_INTERVAL
  NAME_CHANGE_HISTORY_INTERVAL =
      600, // no more than NAME_CHANGE_HISTORY_SIZE name changes can be made in
           // this many seconds
};
...
#define DHM_ROUND_CLEAR                                                        \
  (DHF_ROUND_STARTED | DHF_HOSTAGE_KILLED | DHF_FRIEND_KILLED |                \
   DHF_BOMB_RETRIEVED)

// DHF_xxx bits to clear when the player is restored

#define DHM_CONNECT_CLEAR                                                      \
  (DHF_HOSTAGE_SEEN_FAR | DHF_HOSTAGE_SEEN_NEAR | DHF_HOSTAGE_USED |           \
   DHF_HOSTAGE_INJURED | DHF_FRIEND_SEEN | DHF_ENEMY_SEEN |                    \
   DHF_FRIEND_INJURED | DHF_ENEMY_KILLED | DHF_AMMO_EXHAUSTED |                \
   DHF_IN_TARGET_ZONE | DHF_IN_RESCUE_ZONE | DHF_IN_ESCAPE_ZONE |              \
...
enum RadioType {
  ...
  RADIO_START_1, ///< radio messages between this and RADIO_START_2 and part of
                 ///< Radio1()
  ...
  RADIO_START_2, ///< radio messages between this and RADIO_START_3 are part of
                 ///< Radio2()
...
enum BuyResult_e {
  ...
  BUY_ALREADY_HAVE,
  BUY_CANT_AFFORD,
  BUY_PLAYER_CANT_BUY, // not in the buy zone, is the VIP, is past the
                       // timelimit, etc
...
//=============================================================================
class CCSPlayer : public CBaseMultiplayerPlayer,
...
public:
  ...
  virtual void TraceAttack(const CTakeDamageInfo &inputInfo,
...
public:
  ...
  void FireBullet(Vector vecSrc, const QAngle &shootAngles, float flDistance,
                  int iPenetration, int iBulletType, int iDamage,
                  float flRangeModifier, CBaseEntity *pevAttacker,
  ...
  void GetBulletTypeParameters(int iBulletType, float &fPenetrationPower,
...
public:
  ...
  void RunPlayerMove(const QAngle &viewangles, float forwardmove,
                     float sidemove, float upmove, unsigned short buttons,
                     byte impulse, float frametime);
  ...
  bool IsProtectedByShield(void) const; // returns true if player has a shield
                                        // and is currently hidden behind it
  ...
  void AddAccount(int amount, bool bTrackChange = true,
                  bool bItemBought = false, const char *pItemName = NULL);

  void HintMessage(const char *pMessage, bool bDisplayIfDead,
                   bool bOverrideClientSettings =
                       false); // Displays a hint message to the player
  ...
  void RemoveDefuser(); // remove defuser from the player and remove the model
                        // attachment
  ...
  //=============================================================================
  // HPE_BEGIN
  // [sbodenbender] Need a different test for player blindness for the
  // achievements
  //=============================================================================

  bool IsBlindForAchievement(); // more stringent than IsBlind; more accurately
                                // represents when the player can see again
  ...
  bool
  IsBlind(void) const; // return true if this player is blind (from a flashbang)
  virtual void
  ...
  void Deafen(float flDistance); // make the player deaf / apply dsp preset to
                                 // muffle sound

  void
  ApplyDeafnessEffect(); // apply the deafness effect for a nearby explosion.

  bool IsAutoFollowAllowed(
      void) const; // return true if this player will allow bots to auto follow
  void InhibitAutoFollow(
      float duration); // prevent bots from auto-following for given duration
  ...
  void EmitPrivateSound(
      const char *soundName); ///< emit given sound that only we can hear

  CWeaponCSBase *GetActiveCSWeapon() const;

  void PreThink();

  // This is the think function for the player when they first join the server
  // and have to select a team
  void JoiningThink();
  ...
  void SwitchTeam(int iTeamNum); // Changes teams without penalty - used for
                                 // auto team balancing
  ...
  // //
public:
  void State_Transition(CSPlayerState newState); // Cleanup the previous state
...
private:
  ...
  // This tells us which state the player is currently in (joining, observer,
  // dying, etc). Each state has a well-defined set of parameters that go with
  // it (ie: observer is movetype_noclip, nonsolid, invisible, etc).
  CNetworkVar(CSPlayerState, m_iPlayerState);

  CCSPlayerStateInfo *m_pCurStateInfo; // This can be NULL if no state info is
                                       // defined for m_iPlayerState.

  // tells us whether or not this player gets money at the start of the next
  // round.
...
public:
  ...
  CNetworkVar(
      int,
      m_iLastZoom); // after firing a shot, set the FOV to 90, and after showing
                    // the animation, bring the FOV back to last zoom level.
  CNetworkVar(
      bool,
      m_bIsDefusing); // tracks whether this player is currently defusing a bomb
  ...
  Vector
      m_vLastHitLocationObjectSpace; // position where last hit occured in space
                                     // of the bone associated with the hitbox
  ...
  int m_iOldTeam;   // Keep what team they were last on so we can allow joining
                    // spec and switching back to their real team
  ...
  float m_flNameChangeHistory[NAME_CHANGE_HISTORY_SIZE]; // index 0 = most
                                                         // recent change
  ...
  // CNetworkVar( bool, m_bPickedUpDefuser );        // Did player pick up the
  // defuser kit as opposed to buying it? CNetworkVar( bool,
  // m_bDefusedWithPickedUpKit);  // Did player defuse the bomb with a picked-up
  // defuse kit?
  ...
  // Backup copy of the menu text so the player can change this and the menu
  // knows when to update.
  char m_MenuStringBuffer[MENU_STRING_BUFFER_SIZE];

  // When the player joins, it cycles their view between trigger_camera
  // entities. This is the current camera, and the time that we'll switch to the
  // next one.
  ...
  CNetworkVar(int, m_iDirection);  // The current lateral kicking direction; 1 =
                                   // right,  0 = left
  ...
  char m_szNewName[MAX_PLAYER_NAME_LENGTH]; // not empty if player requested a
                                            // namechange
...
protected:
  ...
  bool CSWeaponDrop(CBaseCombatWeapon *pWeapon, bool bDropShield = true,
                    bool bThrow = false);
  ...
  // [menglish] New parameter to always know if this is from death and not just
  // an enemy death
  //=============================================================================
...
private:
  ...
  bool m_bPickedUpDefuser; // Did player pick up the defuser kit as opposed to
                           // buying it?
  bool m_bDefusedWithPickedUpKit; // Did player defuse the bomb with a picked-up
                                  // defuse kit?
  ...
  // AutoBuy functions.
public:
  void AutoBuy(); // this should take into account what the player can afford
                  // and should buy the best equipment for them.

  bool IsInAutoBuy(void) { return m_bIsInAutoBuy; }
  bool IsInReBuy(void) { return m_bIsInRebuy; }

private:
  bool ShouldExecuteAutoBuyCommand(const AutoBuyInfoStruct *commandInfo,
                                   bool boughtPrimary, bool boughtSecondary);
  void PostAutoBuyCommandProcessing(const AutoBuyInfoStruct *commandInfo,
                                    bool &boughtPrimary, bool &boughtSecondary);
  void ParseAutoBuyString(const char *string, bool &boughtPrimary,
                          bool &boughtSecondary);
  AutoBuyInfoStruct *GetAutoBuyCommandInfo(const char *command);
  void PrioritizeAutoBuyString(
      char *autobuyString,
      const char
          *priorityString); // reorders the tokens in autobuyString based on the
                            // order of tokens in the priorityString.
...
private:
  ...
  // Clients don't know about holstered weapons, so we need to tell them the
  // weapon type here
...
private:
  ...
  // overflow things like reliable buffers.
  CUtlDict<float, int> m_RateLimitLastCommandTimes;

  CNetworkVar(
      int,
      m_cycleLatch); // Every so often, we are going to transmit our cycle to
                     // the client to correct divergence caused by PVS changes
...
public:
  ...
  float m_firstKillBlindStartTime; // This is the start time of the blind effect
                                   // during which we got our most recent kill.
  int m_killsWhileBlind;

  bool m_bIsRescuing;      // tracks whether this player is currently rescuing a
                           // hostage
  ...
  static void ProcessPlayerDeathAchievements(CCSPlayer *pAttacker,
                                             CCSPlayer *pVictim,
                                             const CTakeDamageInfo &info);
  ...
  bool m_wasNotKilledNaturally; // Set if the player is dead from a kill command
                                // or late login
...
private:
  CNetworkArray(bool, m_bPlayerDominated,
                MAX_PLAYERS + 1); // array of state per other player whether
                                  // player is dominating other players
  CNetworkArray(bool, m_bPlayerDominatingMe,
                MAX_PLAYERS + 1); // array of state per other player whether
                                  // other players are dominating this player
