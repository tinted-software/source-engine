...
void CCSHLTVDirector::CreateShotFromEvent(CHLTVGameEvent *event) {
  ...
  if (!Q_strcmp("hostage_rescued", name) || !Q_strcmp("hostage_hurt", name) ||
      !Q_strcmp("hostage_follows", name) || !Q_strcmp("hostage_killed", name)) {
  ...
  else if (!Q_strcmp("bomb_pickup", name) || !Q_strcmp("bomb_dropped", name) ||
           !Q_strcmp("bomb_planted", name) || !Q_strcmp("bomb_defused", name)) {
    CBaseEntity *player = UTIL_PlayerByUserId(event->m_Event->GetInt("userid"));
