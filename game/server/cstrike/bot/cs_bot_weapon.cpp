...
void CCSBot::FireWeaponAtEnemy(void) {
  ...
  if (IsUsingSniperRifle()) {
    // if we're using a sniper rifle, don't fire until we are standing still,
    // are zoomed in, and not rapidly moving our view
    if (!IsNotMoving() || IsWaitingForZoom() ||
        !HasViewBeenSteady(GetProfile()->GetReactionTime())) {
      return;
    }
  }
  ...
  if (gpGlobals->curtime > m_fireWeaponTimestamp &&
    ...
    if (!(IsRecognizedEnemyProtectedByShield() &&
      ...
      if (IsUsingSniperRifle()) {
        ...
        float fProjectedSpread =
            rangeToEnemy * GetActiveCSWeapon()->GetInaccuracy();
        float fRequiredSpread =
            IsUsing(WEAPON_AWP) ? 50.0f : 25.0f; // AWP will kill with any hit
      ...
      const float halfSize =
          (IsUsingSniperRifle()) ? HalfHumanWidth : 2.0f * HalfHumanWidth;

      // aiming tolerance depends on how close the target is - closer targets
      // subtend larger angles
      float aimTolerance = (float)cos(atan(halfSize / rangeToEnemy));

      if (onTarget > aimTolerance) {
        bool doAttack;

        // if friendly fire is on, don't fire if a teammate is blocking our line
        // of fire
        ...
        if (IsUsingPistol()) {
          ...
          if (GetProfile()->GetSkill() > 0.75f &&
              rangeToEnemy < closePistolRange) {
        ...
        } else // not using a pistol
        {
          ...
          if (GetProfile()->GetSkill() < 0.5f || rangeToEnemy < sprayRange ||
              IsUsingMachinegun()) {
            // spray 'n pray if enemy is close, or we're not that good, or we're
            // using the big machinegun
            m_fireWeaponTimestamp = 0.0f;
          } else {
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Set the current aim offset using given accuracy (1.0 = perfect aim, 0.0f =
 * terrible aim)
 */
void CCSBot::SetAimOffset(float accuracy) {
  // if our accuracy is less than perfect, it will improve as we "focus in"
  // while not rotating our view
  if (accuracy < 1.0f) {
    // if we moved our view, reset our "focus" mechanism
    if (IsViewMoving(100.0f))
      m_aimSpreadTimestamp = gpGlobals->curtime;

    // focusTime is the time it takes for a bot to "focus in" for very good aim,
    // from 2 to 5 seconds
  ...
  // aim error increases with distance, such that actual crosshair error stays
  // about the same
  ...
  m_aimOffsetTimestamp =
      gpGlobals->curtime + RandomFloat(0.25f, 1.0f); // 0.25, 1.5f
}
...
bool CCSBot::AdjustZoom(float range) {
  ...
  if (IsUsingSniperRifle()) {
    const float sniperZoomRange =
        150.0f; // NOTE: This must be less than sniperMinRange in AttackState
...
bool CCSBot::IsSniper(void) const {
  CWeaponCSBase *weapon =
      static_cast<CWeaponCSBase *>(Weapon_GetSlot(WEAPON_SLOT_RIFLE));
...
bool CCSBot::IsPistolEmpty(void) const {
  CWeaponCSBase *weapon =
      static_cast<CWeaponCSBase *>(Weapon_GetSlot(WEAPON_SLOT_PISTOL));
...
void CCSBot::EquipBestWeapon(bool mustEquip) {
  ...
  if (primary) {
    ...
    if ((ctrl->AllowShotguns() && weaponClass == WEAPONTYPE_SHOTGUN) ||
        ...
        (ctrl->AllowSubMachineGuns() &&
         weaponClass == WEAPONTYPE_SUBMACHINEGUN)) {
      if (DoEquip(primary))
        return;
    }
  }
...
bool CCSBot::HasGrenade(void) const {
  CWeaponCSBase *grenade =
      static_cast<CWeaponCSBase *>(Weapon_GetSlot(WEAPON_SLOT_GRENADES));
...
bool CCSBot::IsUsingGrenade(void) const {
  ...
  if (weapon->IsA(WEAPON_FLASHBANG) || weapon->IsA(WEAPON_SMOKEGRENADE) ||
...
void CCSBot::ThrowGrenade(const Vector &target) {
  if (IsUsingGrenade() && m_grenadeTossState == NOT_THROWING && !IsOnLadder()) {
    ...
    SetLookAt("GrenadeThrow", target, PRIORITY_UNINTERRUPTABLE, 4.0f, false,
              angleTolerance);
...
bool CCSBot::FindGrenadeTossPathTarget(Vector *pos) {
  ...
  UTIL_TraceLine(visibleSpot, check, MASK_PLAYERSOLID, this,
...
void CCSBot::LookForGrenadeTargets(void) {
  ...
  } else // explosive and flashbang grenades
  {
    ...
    for (float h = 0.0f; h < 3.0f * tossHeight; h += heightInc) {
      ...
      enginetrace->TraceRay(ray, MASK_VISIBLE_AND_NPCS | CONTENTS_GRATE,
                            &traceFilter, &result);
      if (result.fraction == 1.0f) {
        // NDebugOverlay::SweptBox( eyePosition, target, mins, maxs, angles, 0,
        // 0, 255, 40, 10.0f );
        ...
        // NDebugOverlay::SweptBox( eyePosition, target, mins, maxs, angles,
        // 255, 0, 0, 5, 10.0f );
...
public:
  FOVClearOfFriends(CCSBot *me) { m_me = me; }
...
public:
  GrenadeResponse(CCSBot *me) { m_me = me; }
  ...
  bool operator()(ActiveGrenade *ag) const {
    ...
    if (m_me->IsVisible(ag->GetPosition(), CHECK_FOV,
                        (CBaseEntity *)ag->GetEntity())) {
      // we see it
      if (ag->IsSmoke()) {
        // ignore smokes
        return true;
      ...
      if (ag->IsFlashbang() &&
          ag->GetEntity()->m_flDetonateTime - gpGlobals->curtime <
              aboutToBlow) {
        ...
        m_me->ClearLookAt();
        m_me->SetLookAt("Avoid Flashbang", away, PRIORITY_UNINTERRUPTABLE,
                        duration);
...
void CCSBot::ReloadCheck(void) {
  ...
  } else if (GetTimeSinceLastSawEnemy() > safeReloadWaitTime &&
             GetActiveWeaponAmmoRatio() <= reloadAmmoRatio) {
    // high-skill players use all their ammo and switch to pistol instead of
    // reloading during combat
    if (GetProfile()->GetSkill() > 0.5f && IsAttacking())
      return;
  } else {
...
void CCSBot::SilencerCheck(void) {
  const float safeSilencerWaitTime =
      3.5f; // longer than reload check because reloading should take precedence
  ...
  if (droppedGun && droppedGun->GetSlot() == WEAPON_SLOT_RIFLE) {
    ...
    if (myGun && droppedGun->GetWeaponID() != myGun->GetWeaponID()) {
      ...
      if (GetProfile()->HasPrimaryPreference()) {
        ...
        if (GetTimeSinceLastSawEnemy() >= safeTime) {
          // we have a primary weapon - drop it if the one on the ground is
          // better
          for (int i = 0; i < GetProfile()->GetWeaponPreferenceCount(); ++i) {
            ...
            if (prefID == droppedGun->GetWeaponID()) {
              // the gun on the ground is better than the one we have - drop our
              // gun
              DropRifle();
              break;
            }
...
bool CCSBot::IsFriendInLineOfFire(void) {
  ...
  trace_t result;
  UTIL_TraceLine(EyePosition(), EyePosition() + 10000.0f * aimDir,
...
bool CCSBot::DidPlayerJustFireWeapon(const CCSPlayer *player) const {
  ...
  return (weapon && !weapon->IsSilenced() &&
