...
void CCSBot::SetState(BotState *state) {
  ...
  if ( IsDefusingBomb() )
  {
          ...
          if ( bombPos != NULL )
          {
                  if ( TheCSBots()->GetBombDefuser() == this )
                  {
                          ...
                          {
                                  Msg( "Bot %s is switching from defusing the
  ...
  */
  ...
  // if we changed state from within the special Attack state, we are no longer
  // attacking
  if (m_isAttacking)
    StopAttacking();
 ...
 */
void CCSBot::RescueHostages(void) { SetTask(RESCUE_HOSTAGES); }
 ...
 * NOTE: This state is special, like Attack, in that it suspends the current
 * behavior and returns to it when done.
 */
 ...
 * If 'searchFromArea' is non-NULL, hiding spots are looked for from that area
 * first.
 */
void CCSBot::Hide(CNavArea *searchFromArea, float duration, float hideRange,
                  bool holdPosition) {
  DestroyPath();
  ...
  const Vector *pos =
      FindNearbyHidingSpot(this, sourcePos, hideRange, IsSniper());
  if (pos == NULL) {
 ...
 * first.
 */
bool CCSBot::TryToHide(CNavArea *searchFromArea, float duration,
                       float hideRange, bool holdPosition, bool useNearest) {
  CNavArea *source;
  Vector sourcePos;
  if (searchFromArea) {
    source = searchFromArea;
  ...
  // search around source area for a good hiding spot
  const Vector *pos =
      FindNearbyHidingSpot(this, sourcePos, hideRange, IsSniper(), useNearest);
  if (pos == NULL) {
    PrintIfWatched("No available hiding spots.\n");
    return false;
  }
...
void CCSBot::Attack(CCSPlayer *victim) {
  ...
  // position
  if (IsAtHidingSpot())
    m_attackState.SetCrouchAndHold((RandomFloat(0.0f, 100.0f) < 60.0f) ? true
                                                                       : false);
  else
    m_attackState.SetCrouchAndHold(false);

  // SetState( &m_attackState );
  // PrintIfWatched( "ATTACK BEGIN (reaction time = %g (+ update time), surprise
  // time = %g, attack delay = %g)\n", 				GetProfile()->GetReactionTime(),
  //m_surpriseDelay, GetProfile()->GetAttackDelay() );
  m_isAttacking = true;
  ...
  // compute the angle difference between where are looking, and where we need
  // to look
  ...
  // immediately aim at enemy - accuracy penalty depending on how far we must
  // turn to aim accuracy is halved if we have to turn 180 degrees
...
void CCSBot::StopAttacking(void) {
  ...
  // if we are following someone, go to the Idle state after the attack to
  // decide whether we still want to follow
  if (IsFollowing()) {
    Idle();
  }
}
...
//--------------------------------------------------------------------------------------------------------------
bool CCSBot::IsAttacking(void) const { return m_isAttacking; }
...
bool CCSBot::MoveToInitialEncounter(void) {
  ...
  for (i = 0; i < path.GetSegmentCount(); ++i) {
    if (path[i]->area->GetEarliestOccupyTime(myTeam) >
        path[i]->area->GetEarliestOccupyTime(enemyTeam)) {
      break;
    }
  }
  ...
  const HidingSpot *spot = FindInitialEncounterSpot(
      this, path[i]->area->GetCenter(),
      path[i]->area->GetEarliestOccupyTime(enemyTeam), maxRange, IsSniper());
  ...
  float timeToWait = path[i]->area->GetEarliestOccupyTime(enemyTeam) -
                     spot->GetArea()->GetEarliestOccupyTime(myTeam);
