...
//
// NOTE: The CS Bot code uses Doxygen-style comments. If you run Doxygen over
// this code, it will auto-generate documentation.  Visit www.doxygen.org to
// download the system for free.
//
...
//--------------------------------------------------------------------------------------------------------------
/**
 * The state is invoked when a bot has nothing to do, or has finished what it
 * was doing. A bot never stays in this state - it is the main action selection
 * mechanism.
 */
...
protected:
  ...
  bool m_haveSeenEnemy; ///< false if we haven't yet seen the enemy since we
                        ///< started this attack (told by a friend, etc)
  bool m_isEnemyHidden; ///< true we if we have lost line-of-sight to our enemy
  float m_reacquireTimestamp;    ///< time when we can fire again, after losing
                                 ///< enemy behind cover
  ...
  float m_pinnedDownTimestamp; ///< time when we'll consider ourselves "pinned
                               ///< down" by the enemy
...
private:
  ...
  Vector
      m_checkNoisePosition; ///< the position of the noise we're investigating
  CountdownTimer
      m_minTimer; ///< minimum time we will investigate our current noise
};
 ...
 * NOTE: This state also includes MOVING TO that hiding spot, which may be all
...
private:
  ...
  float m_holdPositionTime; ///< how long to hold our position after we hear
                            ///< nearby enemy noise
  ...
  Vector m_leaderAnchorPos; ///< the position of our follow leader when we
                            ///< decided to hide

  bool m_isPaused; ///< if true, we have paused in our retreat for a moment
  CountdownTimer
      m_pauseTimer; ///< for stoppping and starting our pauses while we retreat
};
...
private:
  ...
  Vector m_lastLeaderPos; ///< where the leader was when we computed our follow
                          ///< path
  bool m_isStopped;
  float m_stoppedTimestamp;

  enum LeaderMotionStateType { INVALID, STOPPED, WALKING, RUNNING };
...
//--------------------------------------------------------------------------------------------------------------
/**
 * When a bot is actually using another entity (ie: facing towards it and
...
public:
  ...
  virtual bool Initialize(const BotProfile *profile,
                          int team); ///< (EXTEND) prepare bot for action

  virtual void Spawn(void); ///< (EXTEND) spawn the bot into the game
  virtual void
  Touch(CBaseEntity *other); ///< (EXTEND) when touched by another entity
  ...
  virtual void BuildUserCmd(CUserCmd &cmd, const QAngle &viewangles,
                            float forwardmove, float sidemove, float upmove,
                            int buttons, byte impulse);
  virtual float
  GetMoveSpeed(void); ///< returns current movement speed (for walk/run)

  virtual void Walk(void);
  virtual bool
  Jump(bool mustJump = false); ///< returns true if jump was started

  //- behavior properties
  //------------------------------------------------------------------------------------------
  float GetCombatRange(void) const;
  bool IsRogue(void) const; ///< return true if we dont listen to teammates or
                            ///< pursue scenario goals
  ...
  bool IsWellPastSafe(void) const;  ///< return true if it is well past the
                                    ///< early, "safe", part of the round
  bool IsEndOfSafeTime(void) const; ///< return true if we were in the safe time
                                    ///< last update, but not now
  float GetSafeTimeRemaining(
      void) const; ///< return the amount of "safe time" we have left
  float GetSafeTime(void)
      const; ///< return what we think the total "safe time" for this map is
  virtual void
  Blind(float holdTime, float fadeTime,
        float startingAlpha = 255); // player blinded by a flashbang
  bool IsUnhealthy(void) const;     ///< returns true if bot is low on health

  bool
  IsAlert(void) const; ///< return true if bot is in heightened "alert" mode
  void
  BecomeAlert(void); ///< bot becomes "alert" for immediately nearby enemies
  ...
  void Hide(CNavArea *searchFromArea = NULL, float duration = -1.0f,
            float hideRange = 750.0f,
            bool holdPosition = false); ///< DEPRECATED: Use TryToHide() instead
#define USE_NEAREST true
  bool TryToHide(
      CNavArea *searchFromArea = NULL, float duration = -1.0f,
      float hideRange = 750.0f, bool holdPosition = false,
      bool useNearest = false); ///< try to hide nearby, return false if cannot
  ...
  bool IsAtHidingSpot(
      void) const; ///< return true if we are hiding and at our hiding spot
  float GetHidingTime(void) const; ///< return number of seconds we have been at
                                   ///< our current hiding spot

  bool MoveToInitialEncounter(
      void); ///< move to a hiding spot and wait for initial encounter with
             ///< enemy team (return false if no spots are available)

  bool TryToRetreat(float maxRange = 1000.0f,
                    float duration = -1.0f); ///< retreat to a nearby hiding
                                             ///< spot, away from enemies
  ...
  void
  FireWeaponAtEnemy(void); ///< fire our active weapon towards our current enemy
  void StopAttacking(void);
  bool IsAttacking(
      void) const; ///< returns true if bot is currently engaging a target

  void MoveTo(
      const Vector &pos,
      RouteType route = SAFEST_ROUTE); ///< move to potentially distant position
  bool IsMovingTo(void) const; ///< return true if we are in the MoveTo state

  void PlantBomb(void);

  void FetchBomb(void); ///< bomb has been dropped - go get it
  bool NoticeLooseBomb(void)
      const; ///< return true if we noticed the bomb on the ground or on radar
  bool CanSeeLooseBomb(
      void) const; ///< return true if we directly see the loose bomb

  void DefuseBomb(void);
  bool IsDefusingBomb(
      void) const; ///< returns true if bot is currently defusing the bomb
  bool CanSeePlantedBomb(
      void) const; ///< return true if we directly see the planted bomb

  void EscapeFromBomb(void);
  bool IsEscapingFromBomb(
      void) const; ///< return true if we are escaping from the bomb

  void RescueHostages(void); ///< begin process of rescuing hostages

  void UseEntity(CBaseEntity *entity); ///< use the entity

  void OpenDoor(CBaseEntity *door); ///< open the door (assumes we are right in
                                    ///< front of it)
  bool IsOpeningDoor(
      void) const; ///< return true if we are in the process of opening a door
  ...
  void ContinueFollowing(void);   ///< continue following our leader after
                                  ///< finishing what we were doing
  void StopFollowing(void);       ///< stop following
  bool IsFollowing(void) const;   ///< return true if we are following someone
                                  ///< (not necessarily in the follow state)
  CCSPlayer *
  GetFollowLeader(void) const; ///< return the leader we are following
  float GetFollowDuration(
      void) const; ///< return how long we've been following our leader
  bool CanAutoFollow(void) const; ///< return true if we can auto-follow

  bool IsNotMoving(float minDuration = 0.0f)
      const; ///< return true if we are currently standing still and have been
             ///< for minDuration

  void AimAtEnemy(void); ///< point our weapon towards our enemy
  void StopAiming(void); ///< stop aiming at enemy
  bool IsAimingAtEnemy(
      void) const; ///< returns true if we are trying to aim at an enemy

  float GetStateTimestamp(void) const; ///< get time current state was entered

  bool IsDoingScenario(
      void) const; ///< return true if we will do scenario-related tasks

  //- scenario / gamestate
  //-----------------------------------------------------------------------------------------
  CSGameState *
  GetGameState(void); ///< return an interface to this bot's gamestate
  const CSGameState *
  GetGameState(void) const; ///< return an interface to this bot's gamestate

  bool IsAtBombsite(void); ///< return true if we are in a bomb planting zone
  bool GuardRandomZone(
      float range = 500.0f); ///< pick a random zone and hide near it

  bool
  IsBusy(void) const; ///< return true if we are busy doing something important
  ...
  void SetTask(TaskType task,
               CBaseEntity *entity = NULL); ///< set our current "task"
  TaskType GetTask(void) const;
  CBaseEntity *GetTaskEntity(void);
  const char *
  GetTaskName(void) const; ///< return string describing current task

  //- behavior modifiers
  //------------------------------------------------------------------------------------------
  enum DispositionType {
    ENGAGE_AND_INVESTIGATE, ///< engage enemies on sight and investigate enemy
                            ///< noises
    OPPORTUNITY_FIRE, ///< engage enemies on sight, but only look towards enemy
                      ///< noises, dont investigate
    SELF_DEFENSE,     ///< only engage if fired on, or very close to enemy
    IGNORE_ENEMIES, ///< ignore all enemies - useful for ducking around corners,
                    ///< running away, etc

    NUM_DISPOSITIONS
  };
  void SetDisposition(
      DispositionType disposition); ///< define how we react to enemies
  DispositionType GetDisposition(void) const;
  const char *GetDispositionName(
      void) const; ///< return string describing current disposition
  ...
  const char *
  GetMoraleName(void) const; ///< return string describing current morale
  ...
  bool HeardInterestingNoise(
      void); ///< return true if we heard an enemy noise worth checking in to
  void InvestigateNoise(void); ///< investigate recent enemy noise
  bool IsInvestigatingNoise(
      void) const; ///< return true if we are investigating a noise
  const Vector *GetNoisePosition(void)
      const; ///< return position of last heard noise, or NULL if none heard
  CNavArea *GetNoiseArea(void) const;   ///< return area where noise was heard
  void ForgetNoise(void);               ///< clear the last heard noise
  bool CanSeeNoisePosition(void) const; ///< return true if we directly see
                                        ///< where we think the noise came from
  float GetNoiseRange(
      void) const; ///< return approximate distance to last noise heard

  bool CanHearNearbyEnemyGunfire(float range = -1.0f)
      const; ///< return true if we hear nearby threatening enemy gunfire within
             ///< given range (-1 == infinite)
  PriorityType
  GetNoisePriority(void) const; ///< return priority of last heard noise

  //- radio and
  //chatter--------------------------------------------------------------------------------------------
  void SendRadioMessage(RadioType event); ///< send a radio message
  void SpeakAudio(const char *voiceFilename, float duration,
                  int pitch); ///< send voice chatter
  BotChatterInterface *
  GetChatter(void); ///< return an interface to this bot's chatter system
  bool RespondToHelpRequest(
      CCSPlayer *player, Place place,
      float maxRange = -1.0f); ///< decide if we should move to help the player,
                               ///< return true if we will
  bool IsUsingVoice() const; ///< new-style "voice" chatter gets voice feedback

  //- enemies
  //------------------------------------------------------------------------------------------------------
  // BOTPORT: GetEnemy() collides with GetEnemy() in CBaseEntity - need to use
  ...
  int GetNearbyEnemyCount(
      void) const; ///< return max number of nearby enemies we've seen recently
  unsigned int GetEnemyPlace(
      void) const; ///< return location where we see the majority of our enemies
  ...
  CCSPlayer *GetClosestVisibleFriend(
      void) const; ///< return the closest friend that we can see
  CCSPlayer *GetClosestVisibleHumanFriend(
      void) const; ///< return the closest human friend that we can see

  bool
  IsOutnumbered(void) const; ///< return true if we are outnumbered by enemies
  int OutnumberedCount(
      void) const; ///< return number of enemies we are outnumbered by

#define ONLY_VISIBLE_ENEMIES true
  CCSPlayer *GetImportantEnemy(bool checkVisibility = false)
      const; ///< return the closest "important" enemy for the given scenario
             ///< (bomb carrier, VIP, hostage escorter)

  void UpdateReactionQueue(void); ///< update our reaction time queue
  CCSPlayer *GetRecognizedEnemy(
      void); ///< return the most dangerous threat we are "conscious" of
  bool IsRecognizedEnemyReloading(
      void); ///< return true if the enemy we are "conscious" of is reloading
  bool IsRecognizedEnemyProtectedByShield(
      void); ///< return true if the enemy we are "conscious" of is hiding
             ///< behind a shield
  float GetRangeToNearestRecognizedEnemy(
      void); ///< return distance to closest enemy we are "conscious" of

  CCSPlayer *GetAttacker(void) const; ///< return last enemy that hurt us
  float GetTimeSinceAttacked(void)
      const; ///< return duration since we were last injured by an attacker
  float
  GetFirstSawEnemyTimestamp(void) const; ///< time since we saw any enemies
  ...
  bool HasNotSeenEnemyForLongTime(void)
      const; ///< return true if we haven't seen an enemy for "a long time"
  ...
  bool IsFriendInLineOfFire(
      void); ///< return true if a friend is in our weapon's way
  bool IsAwareOfEnemyDeath(
      void) const; ///< return true if we *noticed* that our enemy died
  int GetLastVictimID(void)
      const; ///< return the ID (entindex) of the last victim we killed, or zero

  bool CanSeeSniper(void) const; ///< return true if we can see an enemy sniper
  bool HasSeenSniperRecently(
      void) const; ///< return true if we have seen a sniper recently

  float GetTravelDistanceToPlayer(CCSPlayer *player)
      const; ///< return shortest path travel distance to this player
  bool DidPlayerJustFireWeapon(const CCSPlayer *player)
      const; ///< return true if the given player just fired their weapon
...
#define NO_SPEED_CHANGE false
  PathResult UpdatePathMovement(
      bool allowSpeedChange =
          true); ///< move along our computed path - if allowSpeedChange is
                 ///< true, bot will walk when near goal to ensure accuracy

  // bool AStarSearch( CNavArea *startArea, CNavArea *goalArea );	///<
  // find shortest path from startArea to goalArea - don't actually buid the
  // path
  bool ComputePath(
      const Vector &goal,
      RouteType route = SAFEST_ROUTE); ///< compute path to goal position
  bool StayOnNavMesh(void);
  CNavArea *GetLastKnownArea(
      void) const; ///< return the last area we know we were inside of
  const Vector &
  GetPathEndpoint(void) const; ///< return final position of our current path
  float GetPathDistanceRemaining(
      void) const; ///< return estimated distance left to travel along path
  void ResetStuckMonitor(void);
  bool IsAreaVisible(const CNavArea *area)
      const; ///< is any portion of the area visible to this bot
  const Vector &GetPathPosition(int index) const;
  bool GetSimpleGroundHeightWithFloor(
      const Vector &pos, float *height,
      Vector *normal = NULL); ///< find "simple" ground height, treating current
                              ///< nav area as part of the floor
  void BreakablesCheck(void);
  void
  DoorCheck(void); ///< Check for any doors along our path that need opening

  virtual void PushawayTouch(CBaseEntity *pOther);

  Place GetPlace(void) const; ///< get our current radio chatter place

  bool IsUsingLadder(void)
      const; ///< returns true if we are in the process of negotiating a ladder
  void
  GetOffLadder(void); ///< immediately jump off of our ladder, if we're on one
  ...
  float GetApproximateFallDamage(
      float height) const; ///< return how much damage will will take from the
                           ///< given fall height

  void ForceRun(float duration); ///< force the bot to run if it moves for the
                                 ///< given duration
  ...
  bool IsFriendInTheWay(
      const Vector &goalPos); ///< return true if a friend is between us and the
                              ///< given position
  void FeelerReflexAdjustment(
      Vector *goalPosition); ///< do reflex avoidance movements if our "feelers"
                             ///< are touched

  bool HasVisitedEnemySpawn(
      void) const; ///< return true if we have visited enemy spawn at least once
  bool IsAtEnemySpawn(
      void) const; ///< return true if we are at the/an enemy spawn right now
  ...
  void UpdateLookAround(
      bool updateNow = false); ///< update "looking around" mechanism
  void InhibitLookAround(
      float duration); ///< block all "look at" and "looking around" behavior
                       ///< for given duration - just look ahead
  ...
  void SetLookAt(const char *desc, const Vector &pos, PriorityType pri,
                 float duration = -1.0f, bool clearIfClose = false,
                 float angleTolerance = 5.0f, bool attack = false);
  void
  ClearLookAt(void); ///< stop looking at a point in space and just look ahead
  bool IsLookingAtSpot(PriorityType pri = PRIORITY_LOW)
      const; ///< return true if we are looking at spot with equal or higher
             ///< priority
  bool IsViewMoving(float angleVelThreshold = 1.0f)
      const; ///< returns true if bot's view angles are rotating (not still)
  bool HasViewBeenSteady(
      float duration) const; ///< how long has our view been "steady" (ie: not
                             ///< moving) for given duration

  bool HasLookAtTarget(void)
      const; ///< return true if we are in the process of looking at a target

  enum VisiblePartType {
    NONE = 0x00,
    GUT = 0x01,
    HEAD = 0x02,
    LEFT_SIDE = 0x04,  ///< the left side of the object from our point of view
                       ///< (not their left side)
    RIGHT_SIDE = 0x08, ///< the right side of the object from our point of view
                       ///< (not their right side)
    FEET = 0x10
  };
...
#define CHECK_FOV true
  bool IsVisible(const Vector &pos, bool testFOV = false,
                 const CBaseEntity *ignore =
                     NULL) const; ///< return true if we can see the point
  bool IsVisible(CCSPlayer *player, bool testFOV = false,
                 unsigned char *visParts = NULL)
      const; ///< return true if we can see any part of the player

  bool IsNoticable(const CCSPlayer *player, unsigned char visibleParts)
      const; ///< return true if we "notice" given player

  bool IsEnemyPartVisible(
      VisiblePartType part) const; ///< if enemy is visible, return the part we
                                   ///< see for our current enemy
  const Vector &GetPartPosition(CCSPlayer *player, VisiblePartType part)
      const; ///< return world space position of given part on player

  float ComputeWeaponSightRange(void); ///< return line-of-sight distance to
                                       ///< obstacle along weapon fire ray

  bool IsAnyVisibleEnemyLookingAtMe(
      bool testFOV = false) const; ///< return true if any enemy I have LOS to
                                   ///< is looking directly at me

  bool IsSignificantlyCloser(const CCSPlayer *testPlayer,
                             const CCSPlayer *referencePlayer)
      const; ///< return true if testPlayer is significantly closer than
             ///< referencePlayer

  //- approach points
  //---------------------------------------------------------------------------------------------
  void ComputeApproachPoints(
      void); ///< determine the set of "approach points" representing where the
             ///< enemy can enter this region
  void UpdateApproachPoints(
      void); ///< recompute the approach point set if we have moved far enough
             ///< to invalidate the current ones
  void ClearApproachPoints(void);
  void DrawApproachPoints(void) const; ///< for debugging
  float GetHidingSpotCheckTimestamp(
      HidingSpot *spot) const; ///< return time when given spot was last checked
  void SetHidingSpotCheckTimestamp(
      HidingSpot *spot); ///< set the timestamp of the given spot to now

  const CNavArea *GetInitialEncounterArea(
      void) const; ///< return area where we think we will first meet the enemy
  void SetInitialEncounterArea(const CNavArea *area);

//- weapon query and equip
//--------------------------------------------------------------------------------------
#define MUST_EQUIP true
  void EquipBestWeapon(bool mustEquip = false); ///< equip the best weapon we
                                                ///< are carrying that has ammo
  void EquipPistol(void);                       ///< equip our pistol
  void EquipKnife(void);                        ///< equip the knife

#define DONT_USE_SMOKE_GRENADE true
  bool EquipGrenade(
      bool noSmoke = false); ///< equip a grenade, return false if we cant
  ...
  bool
  IsUsingSniperRifle(void) const; ///< returns true if using a "sniper" rifle
  bool IsUsing(
      CSWeaponID weapon) const; ///< returns true if using the specific weapon
  bool IsSniper(
      void) const; ///< return true if we have a sniper rifle in our inventory
  bool IsSniping(void) const;      ///< return true if we are actively sniping
                                   ///< (moving to sniper spot or settled in)
  bool IsUsingShotgun(void) const; ///< returns true if using a shotgun
  bool IsUsingMachinegun(
      void) const; ///< returns true if using the big 'ol machinegun
  void ThrowGrenade(
      const Vector &target); ///< begin the process of throwing the grenade
  bool IsThrowingGrenade(void)
      const; ///< return true if we are in the process of throwing a grenade
  bool
  HasGrenade(void) const; ///< return true if we have a grenade in our inventory
  void AvoidEnemyGrenades(void); ///< react to enemy grenades we see
  bool IsAvoidingGrenade(
      void) const; ///< return true if we are in the act of avoiding a grenade
  bool DoesActiveWeaponHaveSilencer(
      void) const; ///< returns true if we are using a weapon with a removable
                   ///< silencer
  bool CanActiveWeaponFire(
      void) const; ///< returns true if our current weapon can attack
  CWeaponCSBase *
  GetActiveCSWeapon(void) const; ///< get our current Counter-Strike weapon

  void
  GiveWeapon(const char *weaponAlias); ///< Debug command to give a named weapon

  virtual void PrimaryAttack(
      void); ///< presses the fire button, unless we're holding a pistol that
             ///< can't fire yet (so we can just always call PrimaryAttack())

  enum ZoomType { NO_ZOOM, LOW_ZOOM, HIGH_ZOOM };
  ZoomType GetZoomLevel(void); ///< return the current zoom level of our weapon

  bool AdjustZoom(float range); ///< change our zoom level to be appropriate for
                                ///< the given range
  bool IsWaitingForZoom(
      void) const; ///< return true if we are reacquiring after our zoom

  bool
  IsPrimaryWeaponEmpty(void) const; ///< return true if primary weapon doesn't
                                    ///< exist or is totally out of ammo
  bool IsPistolEmpty(void)
      const; ///< return true if pistol doesn't exist or is totally out of ammo

  int GetHostageEscortCount(
      void) const; ///< return the number of hostages following me
  void IncreaseHostageEscortCount(void);
  float GetRangeToFarthestEscortedHostage(
      void) const; ///< return euclidean distance to farthest escorted hostage
  ...
  /// invoked when injured by something (EXTEND) - returns the amount of damage
  /// inflicted
  ...
  BumpWeapon(CBaseCombatWeapon
                 *pWeapon); ///< invoked when in contact with a CWeaponBox
  ...
  void
  OnEnteredNavArea(CNavArea *newArea); ///< invoked when bot enters a nav area

private:
#define IS_FOOTSTEP true
  void OnAudibleEvent(IGameEvent *event, CBasePlayer *player, float range,
                      PriorityType priority, bool isHostile,
                      bool isFootstep = false,
                      const Vector *actualOrigin =
                          NULL); ///< Checks if the bot can hear the event
...
private:
  ...
  mutable bool
      m_isRogue; ///< if true, the bot is a "rogue" and listens to no-one
  ...
  float m_safeTime; ///< duration at the beginning of the round where we feel
                    ///< "safe"
  bool m_wasSafe;   ///< true if we were in the safe time last update
  void AdjustSafeTime(
      void); ///< called when enemy seen to adjust safe time for this round
  ...
  // instances of each possible behavior state, to avoid dynamic memory
  ...
  bool m_isAttacking; ///< if true, special Attack state is overriding the state
                      ///< machine
  bool m_isOpeningDoor; ///< if true, special OpenDoor state is overriding the
                        ///< state machine
  ...
  void MoveTowardsPosition(
      const Vector &pos); ///< move towards position, independant of view angle
  void MoveAwayFromPosition(const Vector &pos); ///< move away from position,
                                                ///< independant of view angle
  void StrafeAwayFromPosition(
      const Vector &pos); ///< strafe (sidestep) away from position, independant
                          ///< of view angle
  ...
  bool m_isStopping; ///< true if we're trying to stop because we entered a
                     ///< 'stop' nav area
  ...
  void BuildTrivialPath(
      const Vector &goal); ///< build trivial path to goal, assuming we are
                           ///< already in the same area

  CountdownTimer
      m_repathTimer; ///< must have elapsed before bot can pathfind again

  bool ComputePathPositions(void); ///< determine actual path positions bot will
                                   ///< move between along the path
  void SetupLadderMovement(void);
  void SetPathIndex(int index); ///< set the current index along the path
  void DrawPath(void);
  int FindOurPositionOnPath(Vector *close, bool local = false)
      const; ///< compute the closest point to our current position on our path
  int FindPathPoint(float aheadRange, Vector *point,
                    int *prevIndex = NULL); ///< compute a point a fixed
                                            ///< distance ahead along our path.
  bool FindClosestPointOnPath(
      const Vector &pos, int startIndex, int endIndex,
      Vector *close) const; ///< compute closest point on path to given point
  bool IsStraightLinePathWalkable(const Vector &goal)
      const; ///< test for un-jumpable height change, or unrecoverable fall
  void ComputeLadderAngles(
      float *yaw, float *pitch); ///< computes ideal yaw/pitch for traversing
                                 ///< the current ladder on our path

  mutable CountdownTimer m_avoidFriendTimer; ///< used to throttle how often we
                                             ///< check for friends in our path
  mutable bool m_isFriendInTheWay; ///< true if a friend is blocking our path
  CountdownTimer
      m_politeTimer; ///< we'll wait for friend to move until this runs out
  bool m_isWaitingBehindFriend; ///< true if we are waiting for a friend to move

#define ONLY_JUMP_DOWN true
  bool DiscontinuityJump(
      float ground, bool onlyJumpDown = false,
  ...
  bool
      m_pathLadderFaceIn; ///< if true, face towards ladder, otherwise face away
  const CNavLadder
      *m_pathLadder; ///< the ladder we need to use to reach the next area
  ...
  float m_pathLadderTimestamp; ///< time when we started using ladder - for
                               ///< timeout check
  ...
  void UpdateTravelDistanceToAllPlayers(
      void); ///< periodically compute shortest path distance to each player
  CountdownTimer m_updateTravelDistanceTimer; ///< for throttling travel
                                              ///< distance computations
  float m_playerTravelDistance[MAX_PLAYERS]; ///< current distance from this bot
                                             ///< to each player
  unsigned char m_travelDistancePhase; ///< a counter for optimizing when to
                                       ///< compute travel distance

  //- game scenario mechanisms
  //-------------------------------------------------------------------------------------
  CSGameState
      m_gameState; ///< our current knowledge about the state of the scenario

  byte m_hostageEscortCount; ///< the number of hostages we're currently
                             ///< escorting
  void UpdateHostageEscortCount(
      void); ///< periodic check of hostage count in case we lost some
  ...
  CountdownTimer
      m_inhibitWaitingForHostageTimer;  ///< if active, inhibits us waiting for
                                        ///< lagging hostages
  ...
  bool UpdateLookAtNoise(void); ///< return true if we decided to look towards
                                ///< the most recent noise source
  CountdownTimer m_noiseBendTimer; ///< for throttling how often we bend our
                                   ///< line of sight to the noise location
  ...
  bool m_lookAtSpotClearIfClose; ///< if true, the look at spot is cleared if it
                                 ///< gets close to us
  ...
  Vector m_approachPointViewPosition; ///< the position used when computing
                                      ///< current approachPoint set

  CBaseEntity *FindEntitiesOnPath(float distance,
                                  CPushAwayEnumerator *enumerator,
                                  bool checkStuck);

  IntervalTimer m_viewSteadyTimer; ///< how long has our view been "steady" (ie:
                                   ///< not moving)

  bool BendLineOfSight(const Vector &eye, const Vector &target, Vector *bend,
                       float angleLimit = 135.0f)
      const; ///< "bend" our line of sight until we can see the target point.
             ///< Return bend point, false if cant bend.
  bool FindApproachPointNearestPath(
      Vector *pos); ///< find the approach point that is nearest to our current
                    ///< path, ahead of us
  bool FindGrenadeTossPathTarget(
      Vector *pos); ///< find spot to throw grenade ahead of us and "around the
                    ///< corner" along our path
  ...
  const CNavArea *m_initialEncounterArea; ///< area where we think we will
                                          ///< initially encounter the enemy
  void LookForGrenadeTargets(
      void); ///< look for grenade throw targets and throw our grenade at them
  void UpdateGrenadeThrow(void);      ///< process grenade throwing
  CountdownTimer m_isAvoidingGrenade; ///< if nonzero we are in the act of
                                      ///< avoiding a grenade

  SpotEncounter *m_spotEncounter; ///< the spots we will encounter as we move
                                  ///< thru our current area
  ...
  //- aim angle mechanism
  //-----------------------------------------------------------------------------------------
  Vector m_aimOffset;     ///< current error added to victim's position to get
                          ///< actual aim spot
  Vector m_aimOffsetGoal; ///< desired aim offset
  float m_aimOffsetTimestamp; ///< time of next offset adjustment
  float m_aimSpreadTimestamp; ///< time used to determine max spread as it
  ...
  void ComputePartPositions(
      CCSPlayer *player); ///< compute part positions from bone location
  ...
  unsigned char
      m_visibleEnemyParts;    ///< which parts of the visible enemy do we see
  ...
  mutable CHandle<CCSPlayer>
      m_closestVisibleFriend; ///< the closest friend we can see
  mutable CHandle<CCSPlayer>
      m_closestVisibleHumanFriend; ///< the closest human friend we can see

  IntervalTimer m_attentionInterval; ///< time between attention checks

  mutable CHandle<CCSPlayer>
      m_attacker; ///< last enemy that hurt us (may not be same as m_enemy)
  ...
  bool m_isRapidFiring; ///< if true, RunUpkeep() will toggle our primary attack
                        ///< as fast as it can
  IntervalTimer
      m_equipTimer; ///< how long have we had our current weapon equipped
  ...
  CountdownTimer
      m_sawEnemySniperTimer; ///< tracking time since saw enemy sniper
  ...
  } m_enemyQueue[MAX_ENEMY_QUEUE]; ///< round-robin queue for simulating
                                   ///< reaction times
  byte m_enemyQueueIndex;
  byte m_enemyQueueCount;
  byte
      m_enemyQueueAttendIndex; ///< index of the timeframe we are "conscious" of

  CCSPlayer *
  FindMostDangerousThreat(void); ///< return most dangerous threat in my field
                                 ///< of view (feeds into reaction time queue)
  ...
  bool IsRadioCommand(RadioType event)
      const; ///< returns true if the radio message is an order to do something
...
inline float CCSBot::GetFeetZ(void) const { return GetAbsOrigin().z; }
...
inline bool CCSBot::IsAwareOfEnemyDeath(void) const {
  ...
  if (!m_enemy->IsAlive() && gpGlobals->curtime - GetEnemyDeathTimestamp() >
                                 (1.0f - 0.8f * GetProfile()->GetSkill()))
    return true;
...
inline void CCSBot::StopPanicking(void) { m_panicTimer.Invalidate(); }

inline bool CCSBot::IsNotMoving(float minDuration) const {
  return (m_stillTimer.HasStarted() &&
          m_stillTimer.GetElapsedTime() >= minDuration);
}
...
inline bool
CCSBot::IsSignificantlyCloser(const CCSPlayer *testPlayer,
                              const CCSPlayer *referencePlayer) const {
  if (!referencePlayer)
    return true;
  ...
  float referenceDist =
      (GetAbsOrigin() - referencePlayer->GetAbsOrigin()).Length();
...
public:
  ...
  float operator()(CNavArea *area, CNavArea *fromArea, const CNavLadder *ladder,
                   const CFuncElevator *elevator, float length) {
    float baseDangerFactor = 100.0f; // 100

    // respond to the danger modulated by our aggression (even super-aggressives
    // pay SOME attention to danger)
    float dangerFactor =
        (1.0f - (0.95f * m_bot->GetProfile()->GetAggression())) *
        baseDangerFactor;
    ...
    } else if ((fromArea->GetAttributes() & NAV_MESH_JUMP) &&
               (area->GetAttributes() & NAV_MESH_JUMP)) {
      // cannot actually walk in jump areas - disallow moving from jump area to
      // jump area
      return -1.0f;
    }
    if (area->GetAttributes() & NAV_MESH_NO_HOSTAGES &&
        m_bot->GetHostageEscortCount()) {
      ...
      if (ladder) {
        ...
        // if we are currently escorting hostages, avoid ladders (hostages are
        // confused by them)
      ...
      if (!area->IsUnderwater() &&
          area->IsConnected(fromArea, NUM_DIRECTIONS) == false) {
        // this is a "jump down" (one way drop) transition - estimate damage we
        // will take to traverse it
        float fallDistance = -fromArea->ComputeGroundHeightChange(area);

        // if it's a drop-down ladder, estimate height from the bottom of the
        // ladder to the lower area
        if (ladder && ladder->m_bottom.z < fromArea->GetCenter().z &&
            ladder->m_bottom.z > area->GetCenter().z) {
        ...
        if (fallDamage > 0.0f) {
          ...
          const float painTolerance =
              15.0f * m_bot->GetProfile()->GetAggression() + 10.0f;
      ...
      if (!m_bot->IsAttacking()) {
        ...
        if (size >= 1.0f) {
          ...
          cost += costPerFriendPerUnit *
                  (float)area->GetPlayerCount(m_bot->GetTeamNumber()) / size;
        }
      }
...
extern const HidingSpot *
FindInitialEncounterSpot(CBaseEntity *me, const Vector &searchOrigin,
                         float enemyArriveTime, float maxRange, bool isSniper);
