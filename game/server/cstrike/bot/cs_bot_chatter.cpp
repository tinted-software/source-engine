...
void BotBombsiteStatusMeme::Interpret(CCSBot *sender, CCSBot *receiver) const {
  ...
  // because GetNextBombsiteToSearch() will return the same zone (since its not
  // cleared) if the bomb was planted, we will head to that bombsite
...
void BotBombStatusMeme::Interpret(CCSBot *sender, CCSBot *receiver) const {
  ...
  case CSGameState::MOVING:
    ...
    if (!receiver->IsRogue() && receiver->IsHunting() &&
        receiver->GetNearbyEnemyCount() == 0)
...
void BotFollowMeme::Interpret(CCSBot *sender, CCSBot *receiver) const {
  ...
  float travelDistance =
      NavAreaTravelDistance(receiver->GetLastKnownArea(),
                            TheNavMesh->GetNearestNavArea(senderOrigin), cost);
...
void BotDefendHereMeme::Interpret(CCSBot *sender, CCSBot *receiver) const {
  ...
  if (place != UNDEFINED_PLACE) {
    ...
    const Vector *spot =
        FindRandomHidingSpot(receiver, place, receiver->IsSniper());
    if (spot) {
 ...
 */
void BotHostageBeingTakenMeme::Interpret(CCSBot *sender,
                                         CCSBot *receiver) const {
...
BotPhrase::~BotPhrase() {
  for (int bank = 0; bank < m_voiceBank.Count(); ++bank) {
    for (int speakable = 0; speakable < m_voiceBank[bank]->Count();
         ++speakable) {
...
char *BotPhrase::GetSpeakable(int bankIndex, float *duration) const {
  if (bankIndex < 0 || bankIndex >= m_numVoiceBanks ||
      m_count[bankIndex] == 0) {
    if (duration)
      *duration = 0.0f;
  ...
  while (true) {
    ...
    if (speak->m_place == ANY_PLACE || speak->m_place == m_placeCriteria) {
      ...
      // if this speakable does not have a count criteria, we dont care what the
      // count is set to
      if (speak->m_count == UNDEFINED_COUNT ||
          speak->m_count == MIN(m_countCriteria, COUNT_MANY)) {
        if (duration)
 ...
 */
void BotPhraseManager::OnMapChange(void) { m_placeCount = 0; }
...
bool BotPhraseManager::Initialize(const char *filename, int bankIndex) {
  ...
  const int RadioPathLen = 128; // wav filenames need to be shorter than this to
                                // go over the net anyway.
  ...
  while (true) {
    ...
    if (!stricmp(token, "Output")) {
      ...
      if (isDefault) {
        ...
        phrase->m_place = (isPlace) ? TheNavMesh->NameToPlace(phrase->m_name)
                                    : UNDEFINED_PLACE;
...
{
        for( BotPhraseList::const_iterator iter = m_list.begin(); iter !=
m_list.end(); ++iter )
        {
...
BotStatement::BotStatement(BotChatterInterface *chatter, BotStatementType type,
...
//---------------------------------------------------------------------------------------------------------------
/**
 * Attach a meme to this statement, to be transmitted to other friendly bots
 * when spoken
 */
void BotStatement::AttachMeme(BotMeme *meme) { m_meme = meme; }
...
bool BotStatement::IsImportant(void) const {
  ...
  for (int i = 0; i < m_count; ++i) {
    ...
    if (!m_statement[i].isPhrase &&
        m_statement[i].context == BotStatement::CURRENT_ENEMY_COUNT)
      return true;
  }
  ...
  for (int i = 0; i < m_conditionCount; ++i) {
    ...
    case IS_IN_COMBAT: {
      ...
      /*
                              ...
                              {
                                      if
         (GetOwner()->GetChatter()->GetRadioSilenceDuration() < 10.0f) return
         false; break;
                              }
      */
...
bool BotStatement::IsRedundant(const BotStatement *say) const {
  // special cases
  if (GetType() == REPORT_MY_PLAN || GetType() == REPORT_REQUEST_HELP ||
      GetType() == REPORT_CRITICAL_EVENT || GetType() == REPORT_ACKNOWLEDGE)
    return false;
...
bool BotStatement::IsObsolete(void) const {
  ...
  if (GetOwner()->GetChatter()->SeesAtLeastOneEnemy() &&
      m_type == REPORT_ENEMY_LOST) {
    return true;
  }
  ...
  if (GetType() == REPORT_MY_PLAN && say->GetType() == REPORT_MY_PLAN) {
    ...
    if (m_statement[0].phrase == say->m_statement[0].phrase) {
      if (m_place == say->m_place) {
        ...
        // same plan at different place - wait a bit to allow others to respond
        // "me too"
...
/**
 * Say our statement
 * m_index refers to the phrase currently being spoken, or -1 if we havent
 * started yet
 */
bool BotStatement::Update(void) {
  CCSBot *me = GetOwner();

  // if all of our teammates are dead, the only non-redundant statements are
  // emotes
  ...
  if (m_index >= 0 &&
      m_statement[m_index].context == ACCUMULATE_ENEMIES_DELAY) {
    // report if we see a lot of enemies, or if enough time has passed
    const float reportTime = 2.0f; // 1
    if (me->GetNearbyEnemyCount() > 3 ||
        gpGlobals->curtime - m_speakTimestamp > reportTime) {
  ...
  if (gpGlobals->curtime > m_nextTime) {
    ...
    if (m_statement[m_index].isPhrase) {
      ...
      case REMAINING_ENEMY_COUNT: {
        static const char *speak[] = {"NoEnemiesLeft", "OneEnemyLeft",
                                      "TwoEnemiesLeft", "ThreeEnemiesLeft"};
    ...
    if (phrase) {
      // if chatter system is in "standard radio" mode, send the equivalent
      // radio command
      if (me->GetChatter()->GetVerbosity() == BotChatterInterface::RADIO) {
        ...
        const char *filename =
            phrase->GetSpeakable(me->GetProfile()->GetVoiceBank(), &duration);
        // CONSOLE_ECHO( "%s: Radio( '%s' )\n", STRING( me->pev->netname ),
        // filename );

        bool sayIt = true;

        if (phrase->IsPlace()) {
          // don't repeat the place if someone just mentioned it not too long
          // ago
          float timeSince =
              TheBotPhrases->GetPlaceStatementInterval(phrase->GetPlace());
        ...
        if (sayIt) {
          ...
          else if ( g_engfuncs.pfnPlayClientVoice &&
          TheBotPhrases->GetOutputType( me->GetProfile()->GetVoiceBank() ) ==
          BOT_CHATTER_VOICE )
          {
                  me->GetChatter()->ResetRadioSilenceDuration();
          ...
          */
            ...
            me->SpeakAudio(filename, duration + 1.0f,
...
bool BotStatement::HasCount(void) const {
  for (int i = 0; i < m_count; ++i)
    if (!m_statement[i].isPhrase &&
        m_statement[i].context == CURRENT_ENEMY_COUNT)
      return true;
...
void BotChatterInterface::AddStatement(BotStatement *statement, bool mustAdd) {
  ...
  // if we only want mission-critical radio chatter, ignore non-important
  // phrases
  ...
  // don't add statements that are redundant with something we're already
  // waiting to say
  BotStatement *s;
  ...
  if (IsTalking()) {
    if (m_me->GetChatter()->GetVerbosity() == BotChatterInterface::MINIMAL ||
      ...
      if (pain) {
        ...
        m_me->GetProfile()->GetVoiceBank() ) == BOT_CHATTER_VOICE )
        {
        ...
        */
        {
          m_me->SpeakAudio(
              pain->GetSpeakable(m_me->GetProfile()->GetVoiceBank()), 0.0f,
...
void BotChatterInterface::Update(void) {
  ...
  if (ShouldSpeak()) {
    ...
    if (m_me->GetEnemiesRemaining() > 0 &&
        GetRadioSilenceDuration() > longTime) {
      ReportIn();
    }
  }
  ...
  // Removed expired statements, re-order statements according to their
  // relavence and importance Remove redundant statements (ie: our teammates
  // already said them)
  //
  ...
  for (say = m_statementList; say; say = nextSay) {
    ...
    if (friendSay) {
      // convert what we're about to say based on what our teammate is currently
      // saying
      ...
      if (say->IsRedundant(friendSay)) {
        ...
        // m_me->PrintIfWatched( "Teammate said what I was going to say -
        // shutting up.\n" );
        m_me->PrintIfWatched(
...
//---------------------------------------------------------------------------------------------------------------
/**
 * Returns the statement that is being spoken, or is next to be spoken if no-one
 * is speaking now
 */
BotStatement *BotChatterInterface::GetActiveStatement(void) {
  ...
  for (int i = 1; i <= gpGlobals->maxClients; i++) {
    ...
    for (BotStatement *say = bot->GetChatter()->m_statementList; say;
         say = say->m_next) {
      // if this statement is currently being spoken, return it
      if (say->IsSpeaking())
        return say;

      // keep track of statement that has been waiting longest to be spoken of
      // anyone on our team
...
void BotChatterInterface::EnemySpotted(void) {
  // NOTE: This could be a few seconds out of date (enemy is in an adjacent
  // place)
...
void BotChatterInterface::ReportingIn(void) {
  ...
  case CCSBot::GUARD_LOOSE_BOMB: {
    if (TheCSBots()->GetLooseBomb()) {
      ...
      say->AttachMeme(new BotBombStatusMeme(
          CSGameState::LOOSE, TheCSBots()->GetLooseBomb()->GetAbsOrigin()));
    }
    break;
  }

  case CCSBot::GUARD_HOSTAGES: {
    m_me->GetChatter()->GuardingHostages(UNDEFINED_PLACE,
                                         !m_me->IsAtHidingSpot());
    break;
  }
  ...
  if (m_me->IsAttacking()) {
    ...
    if (m_me->GetEnemyDeathTimestamp() < recentTime &&
        m_me->GetEnemyDeathTimestamp() >=
            m_me->GetTimeSinceLastSawEnemy() + 0.5f) {
...
//---------------------------------------------------------------------------------------------------------------
/**
 * If a friend said that they heard something, we don't want to say something
 * similar for a while.
 */
...
void BotChatterInterface::SpottedLooseBomb(CBaseEntity *bomb) {
  ...
  if (m_spottedLooseBombTimer.IsElapsed()) {
    ...
    if (TheCSBots()->GetLooseBomb())
      say->AttachMeme(
          new BotBombStatusMeme(CSGameState::LOOSE, bomb->GetAbsOrigin()));
...
void BotChatterInterface::BombsiteClear(int zoneIndex) {
  ...
  say->AttachMeme(
      new BotBombsiteStatusMeme(zoneIndex, BotBombsiteStatusMeme::CLEAR));
...
void BotChatterInterface::FoundPlantedBomb(int zoneIndex) {
  ...
  say->AttachMeme(
      new BotBombsiteStatusMeme(zoneIndex, BotBombsiteStatusMeme::PLANTED));
...
//---------------------------------------------------------------------------------------------------------------
void BotChatterInterface::Encourage(const char *phraseName,
                                    float repeatInterval, float lifetime) {
