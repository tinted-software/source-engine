...
public:
  char *GetSpeakable(int bankIndex, float *duration = NULL)
      const; ///< return a random speakable and its duration in seconds that
...
public:
  ...
  bool Initialize(
      const char *filename,
      int bankIndex); ///< initialize phrase system from database file for a
  ...
  float GetPlaceStatementInterval(
      Place where) const; ///< return time last statement of given type was
                          ///< emitted by a teammate for the given place
  void ResetPlaceStatementInterval(
      Place where); ///< set time of last statement of given type was emitted by
                    ///< a teammate for the given place
...
public:
  ...
  Place GetPlace(void) const; ///< if this statement refers to a specific place,
                              ///< return that place
  ...
  void
  Convert(const BotStatement *say); ///< possibly change what were going to say
                                    ///< base on what teammate is saying
  ...
  void AppendPhrase(ContextType contextPhrase); ///< special phrases that depend
                                                ///< on the context

  bool
  Update(void); ///< emit statement over time, return false if statement is done
  ...
  void
  AttachMeme(BotMeme *meme); ///< attach a meme to this statement, to be
                             ///< transmitted to other friendly bots when spoken
...
private:
  ...
  Place m_place; ///< explicit place - note some phrases have implicit places as
                 ///< well
  ...
  ConditionType
      m_condition[MAX_BOT_CONDITIONS]; ///< conditions that must be true for the
...
public:
  ...
  VerbosityType
  GetVerbosity(void) const; ///< return our current level of verbosity
  ...
  enum { MUST_ADD = 1 };
  void
  AddStatement(BotStatement *statement,
               bool mustAdd = false); ///< register a statement for speaking
  void RemoveStatement(BotStatement *statement); ///< remove a statement

  BotStatement *
  GetActiveStatement(void); ///< returns the statement that is being spoken, or
                            ///< is next to be spoken if no-one is speaking now
...
private:
  ...
  bool m_requestedBombLocation; ///< true if we already asked where the bomb has
                                ///< been planted
  ...
  static CountdownTimer
      m_encourageTimer; ///< timer to know when we can "encourage" the human
                        ///< player again - shared by all bots
};

inline BotChatterInterface::VerbosityType
BotChatterInterface::GetVerbosity(void) const {
