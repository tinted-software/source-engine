...
void CCSBot::Upkeep(void) {
  ...
  if (IsAimingAtEnemy()) {
    ...
    if (m_enemy != NULL && m_enemy->IsAlive()) {
      ...
      if (m_isEnemyVisible) {
        ...
        if (IsUsingMachinegun()) {
          ...
          aimAtPart = GUT;
        } else if (GetProfile()->GetSkill() > 0.5f &&
                   IsActiveWeaponRecoilHigh()) {
          // sprayin' and prayin' - aim at the gut since we're not going to be
          // accurate
          aimAtPart = GUT;
        ...
        // high skill bots lead the target a little to compensate for update
        // tick latency
        /*
        if (false && GetProfile()->GetSkill() > 0.5f)
        {
                const float k = 1.0f;
                m_aimSpot += k * g_flBotCommandInterval *
        (m_enemy->GetAbsVelocity() - GetAbsVelocity());
        }
        */

      } else {
        // aim where we last saw enemy - but bend the ray so we dont point
        // directly into walls if we put this back, make sure you only bend the
        // ray ONCE and keep the bent spot - dont continually recompute
    ...
    if (m_lookAtSpotClearIfClose) {
      // dont look at spots just in front of our face - it causes erratic view
      // rotation
    ...
    case LOOK_AT_SPOT: {
      ...
      if (m_lookAtSpotDuration >= 0.0f &&
          gpGlobals->curtime - m_lookAtSpotTimestamp > m_lookAtSpotDuration) {
        m_lookAtSpotState = NOT_LOOKING_AT_SPOT;
...
void CCSBot::Update(void) {
  ...
  if (m_noiseTimestamp > 0.0f &&
      gpGlobals->curtime - m_noiseTimestamp > rememberNoiseDuration) {
    ForgetNoise();
  }
  ...
  if (threat) {
    ...
    case SELF_DEFENSE: {
      // attack if fired on
      doAttack = (IsPlayerLookingAtMe(threat, 0.99f) &&
                  DidPlayerJustFireWeapon(threat));
    ...
    if (!doAttack && !IsAttacking() && GetDisposition() != IGNORE_ENEMIES) {
      ...
      if (GetTimeSinceAttacked() < recentAttackDuration) {
        // we may not be attacking our attacker, but at least we're not just
        // taking it (since m_attacker isn't reaction-time delayed, we can't
        // directly use it)
        doAttack = true;
    ...
    if (doAttack) {
      if (!IsAttacking() || threat != GetBotEnemy()) {
        if (IsUsingKnife() && IsHiding()) {
          ...
          if ((GetAbsOrigin() - threat->GetAbsOrigin())
                  .IsLengthLessThan(knifeAttackRange)) {
            Attack(threat);
          }
        } else {
          Attack(threat);
  ...
  if (m_enemy != NULL) {
    if (IsAwareOfEnemyDeath()) {
      ...
      // check LOS to current enemy (chest & head), in case he's dead
      // (GetNearestEnemy() only returns live players) note we're not checking
      // FOV - once we've acquired an enemy (which does check FOV), assume we
  ...
  /// @todo Encapsulate aiming and firing on enemies separately from current
  /// task
  //
  ...
  /// @todo This doesn't work if we are restricted to just knives and sniper
  /// rifles because we cant use the rifle at close range
  if (!IsSafe() && !IsUsingGrenade() && IsUsingKnife() &&
      !IsEscapingFromBomb()) {
    EquipBestWeapon();
  }

  // if we haven't seen an enemy in awhile, and we switched to our pistol during
  // combat, switch back to our primary weapon (if it still has ammo left)
  const float safeRearmTime = 5.0f;
  if (!IsReloading() && IsUsingPistol() && !IsPrimaryWeaponEmpty() &&
      GetTimeSinceLastSawEnemy() > safeRearmTime) {
    EquipBestWeapon();
  }
  ...
  // if we're using a sniper rifle and are no longer attacking, stop looking
  // thru scope
  if (!IsAtHidingSpot() && !IsAttacking() && IsUsingSniperRifle() &&
      IsUsingScope()) {
    SecondaryAttack();
  }
  ...
  case CCSBotManager::SCENARIO_DEFUSE_BOMB: {
    // flee if the bomb is ready to blow and we aren't defusing it or attacking
    // and we know where the bomb is (aggressive players wait until its almost
    // too late)
    ...
    if (!IsEscapingFromBomb() &&        // we aren't already escaping the bomb
        ...
        GetGameState()
            ->IsPlantedBombLocationKnown() && // we know where the bomb is
        TheCSBots()->GetBombTimeLeft() <
            gonnaBlowTime && // is the bomb about to explode
  ...
  // If we were just following someone, don't auto-follow again for a short
  // while to give us a chance to do something else.
  //
  ...
  if (cv_bot_auto_follow.GetBool() &&
      ...
      GetProfile()->GetTeamwork() > minAutoFollowTeamwork && CanAutoFollow() &&
      !IsBusy() && !IsFollowing() && !IsBlind() &&
    ...
    if (GetProfile()->GetTeamwork() > RandomFloat(0.0f, 1.0f)) {
        ...
        if (GetBotFollowCount(leader) < maxFollowCount) {
          ...
          if ((leaderOrigin - myOrigin).IsLengthLessThan(autoFollowRange)) {
            ...
            if (leaderArea) {
              ...
              float travelRange =
                  NavAreaTravelDistance(GetLastKnownArea(), leaderArea, cost);
      ...
      m_allowAutoFollowTime = gpGlobals->curtime + 15.0f +
                              (1.0f - GetProfile()->GetTeamwork()) * 30.0f;
    }
  }
  ...
  if (IsFollowing()) {
    ...
    if (GetProfile()->GetTeamwork() < highTeamwork) {
      ...
      if (GetFollowDuration() >
          minFollowDuration + 40.0f * GetProfile()->GetTeamwork()) {
  ...
  //
  if (m_isOpeningDoor) {
    // opening doors takes precedence over attacking because DoorCheck() will
    // stop opening doors if we don't have a knife out.
...
public:
  DrawTravelTime(const CCSBot *me) { m_me = me; }

  bool operator()(CBasePlayer *player) {
    if (player->IsAlive() && !m_me->InSameTeam(player)) {
      CFmtStr msg;
      player->EntityText(0,
                         msg.sprintf("%3.0f", m_me->GetTravelDistanceToPlayer(
                                                  (CCSPlayer *)player)),
                         0.1f);
...
void CCSBot::DebugDisplay(void) const {
  ...
  NDebugOverlay::ScreenText(
      0.5f, 0.34f,
      msg.sprintf("Skill: %d%%", (int)(100.0f * GetProfile()->GetSkill())), 255,
      255, 255, 150, duration);

  if (m_pathLadder) {
    NDebugOverlay::ScreenText(0.5f, 0.36f,
                              msg.sprintf("Ladder: %d", m_pathLadder->GetID()),
  ...
  if (safeTime > 0.0f) {
    NDebugOverlay::ScreenText(0.5f, 0.38f,
                              msg.sprintf("SafeTime: %3.2f", safeTime), 255,
                              255, 255, 150, duration);
  }

  // show if blind
  if (IsBlind()) {
    NDebugOverlay::ScreenText(0.5f, 0.38f, msg.sprintf("<<< BLIND >>>"), 255,
                              255, 255, 255, duration);
  }

  // show if alert
  if (IsAlert()) {
    NDebugOverlay::ScreenText(0.5f, 0.38f, msg.sprintf("ALERT"), 255, 0, 0, 255,
                              duration);
  }

  // show if panicked
  if (IsPanicking()) {
    NDebugOverlay::ScreenText(0.5f, 0.36f, msg.sprintf("PANIC"), 255, 255, 0,
                              255, duration);
  }
  ...
  if (m_isAttacking) {
    NDebugOverlay::ScreenText(
        0.5f, 0.4f,
        msg.sprintf("ATTACKING: %s", GetBotEnemy()->GetPlayerName()), 255, 0, 0,
        255, duration);
  } else {
    NDebugOverlay::ScreenText(0.5f, 0.4f,
                              msg.sprintf("State: %s", m_state->GetName()), 255,
                              255, 0, 255, duration);
    NDebugOverlay::ScreenText(0.5f, 0.42f,
                              msg.sprintf("Task: %s", GetTaskName()), 0, 255, 0,
                              255, duration);
    NDebugOverlay::ScreenText(
        0.5f, 0.44f, msg.sprintf("Disposition: %s", GetDispositionName()), 100,
        100, 255, 255, duration);
    NDebugOverlay::ScreenText(0.5f, 0.46f,
                              msg.sprintf("Morale: %s", GetMoraleName()), 0,
                              200, 200, 255, duration);
  }

  // show look at status
  if (m_lookAtSpotState != NOT_LOOKING_AT_SPOT) {
    const char *string = msg.sprintf("LookAt: %s (%s)", m_lookAtDesc,
  ...
  NDebugOverlay::ScreenText(
      0.5f, 0.62f,
      msg.sprintf("Steady view = %s", HasViewBeenSteady(0.2f) ? "YES" : "NO"),
      255, 255, 0, 150, duration);

  // show friend/foes I know of
  NDebugOverlay::ScreenText(
      0.5f, 0.64f, msg.sprintf("Nearby friends = %d", m_nearbyFriendCount), 100,
      255, 100, 150, duration);
  NDebugOverlay::ScreenText(
      0.5f, 0.66f, msg.sprintf("Nearby enemies = %d", m_nearbyEnemyCount), 255,
      100, 100, 150, duration);

  if (m_lastNavArea) {
    NDebugOverlay::ScreenText(0.5f, 0.68f,
                              msg.sprintf("Nav Area: %d (%s)",
                                          m_lastNavArea->GetID(),
                                          m_szLastPlaceName.Get()),
                              255, 255, 255, 150, duration);
    /*
    ...
    m_currentArea->GetCenter(), 0, 255, 0, true, 0.1f );
            }
            else if ( m_lastKnownArea )
            {
                    NDebugOverlay::Line( GetAbsOrigin(),
    m_lastKnownArea->GetCenter(), 255, 0, 0, true, 0.1f );
            }
            else if ( m_lastNavArea )
            {
                    NDebugOverlay::Line( GetAbsOrigin(),
    m_lastNavArea->GetCenter(), 0, 0, 255, true, 0.1f );
            }
    }
    */
  }
  ...
  for (int i = 0; i < TheBots->GetDebugMessageCount(); ++i) {
    ...
    if (msg->m_age.GetElapsedTime() < maxAge) {
      ...
      if (msg->m_age.GetElapsedTime() > fadeAge) {
        alpha *= (1.0f -
                  (msg->m_age.GetElapsedTime() - fadeAge) / (maxAge - fadeAge));
      }

      NDebugOverlay::ScreenText(0.5f, y, msg->m_string, 255, 255, 255, alpha,
                                duration);
      y += lineHeight;
    }
  }
  ...
  if (noisePos) {
    ...
    NDebugOverlay::VertArrow(*noisePos + Vector(0, 0, size), *noisePos,
                             size / 4.0f, 255, 255, 0, 0, true, duration);
  }
  ...
  if (IsHiding()) {
    ...
    if (false && m_spotEncounter) {
      NDebugOverlay::Line(m_spotEncounter->path.from, m_spotEncounter->path.to,
                          0, 150, 150, true, 0.1f);
      ...
      FOR_EACH_VEC(m_spotEncounter->spots, it) {
        ...
        if (order->spot->GetArea()) {
          if (TheCSBots()->GetElapsedRoundTime() <
              order->spot->GetArea()->GetEarliestOccupyTime(
                  OtherTeam(GetTeamNumber()))) {
            continue;
          }
        }
        ...
        NDebugOverlay::Line(along, order->spot->GetPosition(), 0, 255, 255,
                            true, 0.1f);
      }
    }
  }
  ...
  if (false) {
    ...
    mstudiohitboxset_t *set =
        pStudioHdr->pHitboxSet(const_cast<CCSBot *>(this)->GetHitboxSet());
    if (!set)
      return;
    ...
    for (int i = 0; i < set->numhitboxes; i++) {
      ...
      NDebugOverlay::BoxAngles(position, pbox->bbmin, pbox->bbmax, angles, 255,
                               0, 0, 0, 0.1f);

      const float size = 5.0f;
      NDebugOverlay::Line(position, position + size * forward, 255, 255, 0,
                          true, 0.1f);
      NDebugOverlay::Line(position, position + size * right, 255, 0, 0, true,
                          0.1f);
      NDebugOverlay::Line(position, position + size * up, 0, 255, 0, true,
                          0.1f);
  ...
  NDebugOverlay::ScreenText( 0.3f, 0.66f, msg.sprintf( "Punch angle pitch =
  %3.2f", angles.x ), 255, 255, 0, 150, duration );
  */
  ...
  /*
          // show line of fire
          if ((cv_bot_traceview.GetInt() == 100 && IsLocalPlayerWatchingMe()) ||
     cv_bot_traceview.GetInt() == 101)
          {
                  if (!IsFriendInLineOfFire())
                  {
                          ...
                          if (GetTeamNumber() == TEAM_TERRORIST)
                                  UTIL_DrawBeamPoints( vecSrc, vecSrc + 2000.0f
     * vecAiming, 1, 255, 0, 0 ); else UTIL_DrawBeamPoints( vecSrc, vecSrc +
     2000.0f * vecAiming, 1, 0, 50, 255 );
                  }
          }
          ...
          if (cv_bot_traceview.GetInt() == 1 && IsLocalPlayerWatchingMe())
          {
                  ...
                  //Vector arrow( size * cos( m_forwardAngle * M_PI/180.0f ),
     size * sin( m_forwardAngle * M_PI/180.0f ), 0.0f ); Vector arrow( size *
     (float)cos( m_lookAheadAngle * M_PI/180.0f ), size * (float)sin(
     m_lookAheadAngle * M_PI/180.0f ), 0.0f );
  ...
  if (IsAttacking()) {
    ...
    if (enemy) {
      NDebugOverlay::Cross3D(GetPartPosition(enemy, GUT), crossSize, 0, 255, 0,
                             true, 0.1f);
      NDebugOverlay::Cross3D(GetPartPosition(enemy, HEAD), crossSize, 0, 255, 0,
                             true, 0.1f);
      NDebugOverlay::Cross3D(GetPartPosition(enemy, FEET), crossSize, 0, 255, 0,
                             true, 0.1f);
      NDebugOverlay::Cross3D(GetPartPosition(enemy, LEFT_SIDE), crossSize, 0,
                             255, 0, true, 0.1f);
      NDebugOverlay::Cross3D(GetPartPosition(enemy, RIGHT_SIDE), crossSize, 0,
                             255, 0, true, 0.1f);
    }
  }
}
 ...
 * NOTE: Travel distance is NOT symmetric between players A and B.  Each much be
 * computed separately.
 */
void CCSBot::UpdateTravelDistanceToAllPlayers(void) {
  ...
  if (m_updateTravelDistanceTimer.IsElapsed()) {
    ...
    for (int i = 1; i <= gpGlobals->maxClients; ++i) {
      ...
      if (m_playerTravelDistance[which] < 0.0f ||
          m_playerTravelDistance[which] > veryFarAway) {
        if (m_travelDistancePhase != 0)
          continue;
      } else {
        // if player is far away, update two out of three times (on phases 1 and
        // 2)
        const float farAway = 2000.0f;
        if (m_playerTravelDistance[which] > farAway &&
            m_travelDistancePhase == 0)
          continue;
      }

      // if player is fairly close, update often
      m_playerTravelDistance[which] =
          NavAreaTravelDistance(EyePosition(), player->EyePosition(), pathCost);
    }
