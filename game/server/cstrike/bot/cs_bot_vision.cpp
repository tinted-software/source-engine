...
void CCSBot::ComputeLadderAngles(float *yaw, float *pitch) {
  ...
  Vector faceDir = (m_pathLadderFaceIn) ? -m_pathLadder->GetNormal()
                                        : m_pathLadder->GetNormal();
  QAngle faceAngles;
  ...
  case ASCEND_LADDER:
    ...
    *pitch = (m_pathLadderState == ASCEND_LADDER) ? ladderPitchUpTraverse
                                                  : ladderPitchUpApproach;
    break;

  case MOUNT_DESCENDING_LADDER:
  case DESCEND_LADDER:
    *yaw = faceAngles[YAW] + StayOnLadderLine(this, m_pathLadder);
    *pitch = (m_pathLadderState == DESCEND_LADDER) ? ladderPitchDownTraverse
                                                   : ladderPitchDownApproach;
    break;
...
void CCSBot::UpdateLookAngles(void) {
  ...
  // springs are stiffer when attacking, so we can track and move between
  // targets better
  if (IsAttacking()) {
    stiffness = 300.0f;
    damping = 30.0f;    // 20
  ...
  // If we are on a ladder, we need to look up or down to traverse it - override
  // pitch in this case.
  //
  // If we're trying to break something, though, we actually need to look at it
  // before we can look at the ladder
  //
  if (IsUsingLadder() &&
      !(IsLookingAtSpot(PRIORITY_HIGH) && m_lookAtSpotAttack)) {
  ...
  if (!IsAttacking())
  {
          // do not allow rotation through our reverse facing angle - go the
  "long" way instead float toCurrent = AngleNormalize( pev->v_angle.y -
  m_forwardAngle ); float toDesired = AngleNormalize( useYaw - m_forwardAngle );

          // if angle differences are different signs, they cross the forward
  facing if (toCurrent * toDesired < 0.0f)
          {
                  // if the sum of the angles is greater than 180, turn the
  "long" way around if (abs( toCurrent - toDesired ) >= 180.0f)
                  {
                          if (angleDiff > 0.0f)
                                  angleDiff -= 360.0f;
                          else
  ...
  */
  ...
  if (false && angleDiff < onTargetTolerance &&
      angleDiff > -onTargetTolerance) {
    m_lookPitchVel = 0.0f;
 ...
 */
bool CCSBot::IsVisible(const Vector &pos, bool testFOV,
                       const CBaseEntity *ignore) const {
  ...
  UTIL_TraceLine(EyePositionConst(), pos, MASK_VISIBLE_AND_NPCS, &traceFilter,
                 &result);
 ...
 * Check parts in order of importance. Return the first part seen in "visPart"
 * if it is non-NULL.
 */
bool CCSBot::IsVisible(CCSPlayer *player, bool testFOV,
                       unsigned char *visParts) const {
  VPROF_BUDGET("CCSBot::IsVisible( player )", VPROF_BUDGETGROUP_NPCS);

  // optimization - assume if center is not in FOV, nothing is
  // we're using WorldSpaceCenter instead of GUT so we can skip GetPartPosition
  // below - that's the most expensive part of this, and if we can skip it, so
  // much the better.
  if (testFOV &&
      !(const_cast<CCSBot *>(this)->FInViewCone(player->WorldSpaceCenter()))) {
    return false;
  }
...
void CCSBot::ComputePartPositions(CCSPlayer *player) {
  ...
  if (studioHdr) {
    ...
    if (set && maxBoxIndex < set->numhitboxes) {
      ...
      /// @todo Fix this hack - lower the head target because it's a bit too
      /// high for the current T model
 ...
 */
const Vector &CCSBot::GetPartPosition(CCSPlayer *player,
                                      VisiblePartType part) const {
 ...
 */
void CCSBot::SetLookAt(const char *desc, const Vector &pos, PriorityType pri,
                       float duration, bool clearIfClose, float angleTolerance,
                       bool attack) {
  if (IsBlind())
    return;

  // if currently looking at a point in space with higher priority, ignore this
  // request
  ...
  if (m_lookAtSpotState != NOT_LOOKING_AT_SPOT &&
      VectorsAreEqual(pos, m_lookAtSpot, tolerance)) {
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Block all "look at" and "look around" behavior for given duration - just look
 * ahead
 */
...
void CCSBot::UpdatePeripheralVision() {
  ...
  const float peripheralUpdateInterval =
      0.29f; // if we update at 10Hz, this ensures we test once every three
...
void CCSBot::UpdateLookAround(bool updateNow) {
  ...
  if (!IsNoiseHeard() || GetNoiseRange() > closeRange) {
    ...
    if (!IsLookingAtSpot(PRIORITY_MEDIUM) &&
        gpGlobals->curtime - m_lastSawEnemyTimestamp < recentThreatTime) {
      ClearLookAt();
      ...
      if (TheNavMesh->GetSimpleGroundHeight(m_lastEnemyPosition, &spot.z)) {
        ...
        SetLookAt("Last Enemy Position", spot, PRIORITY_MEDIUM,
                  RandomFloat(2.0f, 3.0f), true);
        return;
      }
    }
  }
  ...
  //
  // If we are hiding (or otherwise standing still), watch all approach points
  // leading into this region
  //
  ...
  if (IsAtHidingSpot() || IsNotMoving(minStillTime)) {
    ...
    if ((m_approachPointViewPosition - GetAbsOrigin())
            .IsLengthGreaterThan(recomputeApproachPointTolerance)) {
    ...
    if (IsUsingSniperRifle())
      m_lookAroundStateTimestamp =
          gpGlobals->curtime + RandomFloat(5.0f, 10.0f);
    else
      m_lookAroundStateTimestamp =
          gpGlobals->curtime + RandomFloat(1.0f, 2.0f); // 0.5, 1.0
...
#define MAX_APPROACHES 16
    ...
    for (int i = 0; i < m_approachPointCount; ++i) {
      float spotTime = m_approachPoint[i].m_area->GetEarliestOccupyTime(
      ...
      if (TheCSBots()->GetElapsedRoundTime() >= spotTime) {
        ...
        // keep track of earliest spot we can see in case we get there very
        // early
    ...
    if (validSpotCount) {
      ...
      // all of the spots we can see can't be reached yet by the enemy - look at
      // the earliest spot
      spot = *earlySpot;
    } else {
      return;
    ...
    /// @todo If this approach point is very near, this will cause us to aim up
    /// in the air if were crouching
    spot.z += HalfHumanHeight;
  ...
  if (m_spotEncounter) {
    ...
    if (!IsSafe() && !IsLookingAtSpot(PRIORITY_LOW)) {
      ...
      m_spotCheckTimestamp =
          gpGlobals->curtime + asleep * RandomFloat(10.0f, 30.0f);
...
#define MAX_DANGER_SPOTS 16
      ...
      FOR_EACH_VEC(m_spotEncounter->spots, it) {
        ...
        if (gpGlobals->curtime - GetHidingSpotCheckTimestamp(spotOrder->spot) <=
            checkTime)
          continue;
        ...
        if (spotOrder->spot->GetArea()) {
          if (TheCSBots()->GetElapsedRoundTime() <
              spotOrder->spot->GetArea()->GetEarliestOccupyTime(
                  OtherTeam(GetTeamNumber()))) {
            continue;
          }
        }
      ...
      if (dangerSpotCount) {
        ...
        SetLookAt("Encounter Spot",
                  dangerSpot[which]->GetPosition() +
 ...
 */
bool CCSBot::BendLineOfSight(const Vector &eye, const Vector &target,
                             Vector *bend, float angleLimit) const {
  ...
  // optiming assumption - previous rays cast "shadow" on subsequent rays since
  // they already enumerated visible space along their length. We should do a
  // dot product and compute the exact length, but since the angular changes are
  // incremental, using the direct length should be close enough.
  ...
  for (float angle = angleInc; angle <= angleLimit; angle += angleInc) {
    ...
    for (int side = 0; side < 2; ++side) {
      ...
      UTIL_TraceLine(eye, rotPoint, MASK_VISIBLE_AND_NPCS, &traceFilter,
                     &result);
      ...
      if (doDebug && cv_bot_debug.GetBool() && IsLocalPlayerWatchingMe()) {
        NDebugOverlay::Line(eye, eye + visibleLength * ray, colorSet[color].r,
                            colorSet[color].g, colorSet[color].b, true,
                            debugDuration);
      }
      ...
      for (float bendLength = startLength; bendLength <= visibleLength;
           bendLength += bendStepSize) {
        // compute point along ray
        Vector bendPoint = eye + bendLength * ray;

        // check if we can see approach point from this bend point
        UTIL_TraceLine(bendPoint, target, MASK_VISIBLE_AND_NPCS, &traceFilter,
                       &result);

        if (doDebug && cv_bot_debug.GetBool() && IsLocalPlayerWatchingMe()) {
          NDebugOverlay::Line(bendPoint, result.endpos, colorSet[color].r / 2,
                              colorSet[color].g / 2, colorSet[color].b / 2,
                              true, debugDuration);
        }

        if (result.fraction == 1.0f && !result.startsolid) {
          // target is visible from this bend point on the ray - use this point
          // on the ray as our point
 ...
 */
bool CCSBot::IsNoticable(const CCSPlayer *player,
                         unsigned char visParts) const {
  ...
  // compute range modifier - farther away players are harder to notice,
  // depeding on what they are doing
  ...
  float dispositionChance =
      closeChance + (farChance - closeChance) * rangeModifier;
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Return most dangerous threat in my field of view (feeds into reaction time
 * queue).
 * @todo Account for lighting levels, cover, and distance to see if we notice
 * enemy
 */
CCSPlayer *CCSBot::FindMostDangerousThreat(void) {
        ...
        if (sniperThreat) {
          if (IsPlayerLookingAtMe(player, lookingAtMeTolerance)) {
            if (sniperThreatIsFacingMe) {
              ...
              // even if this sniper is farther away, keep it because he's
              // aiming at us
              sniperThreat = player;
              sniperThreatRange = distSq;
              sniperThreatIsFacingMe = true;
            ...
            // this sniper is not looking at us, only consider it if we dont
            // have a sniper facing us
          ...
          sniperThreat = player;
          sniperThreatRange = distSq;
          sniperThreatIsFacingMe =
              IsPlayerLookingAtMe(player, lookingAtMeTolerance);
        }
      }
  ...
  {
    VPROF_BUDGET("CCSBot::Count nearby Friends & Enemies",
...

  {
    ...
    if (IsSniper() && sniperThreat) {
      ...
      for (t = 0; t < threatCount; ++t) {
        if (threat[t].range < closeCombatRange &&
            IsPlayerLookingAtMe(threat[t].enemy, lookingAtMeTolerance)) {
...
void CCSBot::Blind(float holdTime, float fadeTime, float startingAlpha) {
  ...
  m_blindMoveDir = static_cast<NavRelativeDirType>(
      RandomInt(1, NUM_RELATIVE_DIRECTIONS - 1));
  ...
  if (m_blindFire) {
    ...
    Vector forward;
    EyeVectors(&forward);
    SetLookAt("Blind", EyePosition() + 10000.0f * forward,
              PRIORITY_UNINTERRUPTABLE, holdTime + 0.5f * fadeTime);
  }
...
public:
  ...
  bool operator()(CBasePlayer *player) {
    ...
    if (m_testFOV &&
        !(const_cast<CCSBot *>(m_me)->FInViewCone(player->WorldSpaceCenter())))
      return true;
