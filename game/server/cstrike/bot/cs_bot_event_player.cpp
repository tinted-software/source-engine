...
void CCSBot::OnPlayerDeath(IGameEvent *event) {
  ...
  CBasePlayer *killer =
      (other && other->IsPlayer()) ? static_cast<CBasePlayer *>(other) : NULL;

  // if the human player died in the single player game, tell the team
  if (CSGameRules()->IsCareer() && victim && !victim->IsBot() &&
      victim->GetTeamNumber() == GetTeamNumber()) {
  ...
  if (victim && victim->GetTeamNumber() == GetTeamNumber()) {
    ...
    if (killer && !killer->IsBot() &&
        killer->GetTeamNumber() == GetTeamNumber() && killer != this) {
    ...
    if (IsAttacking()) {
      if (GetTimeSinceLastSawEnemy() > 0.4f) {
        ...
        // move to last known position of enemy - this could cause us to flank
        // if the danger has changed due to our teammate's recent death
    ...
    {
      //
      // If we just saw a nearby friend die, and we haven't yet acquired an
      // enemy automatically acquire our dead friend's killer
      //
      if (GetDisposition() == ENGAGE_AND_INVESTIGATE ||
          GetDisposition() == OPPORTUNITY_FIRE) {
        CBasePlayer *other = UTIL_PlayerByUserId(event->GetInt("attacker"));
        ...
        if (other && other->IsPlayer()) {
          ...
          if (killer->GetTeamNumber() != GetTeamNumber()) {
            // check if we saw our friend die - dont check FOV - assume we're
            // aware of our surroundings in combat snipers stay put
            if (!IsSniper() && IsVisible(playerOrigin)) {
              ...
              if (!IsHiding() || !IsUsingKnife() ||
                  RandomFloat(0, 100) < knifeAmbushChance) {
            ...
            // if friend was far away and we haven't seen an enemy in awhile, go
            // to where our friend was killed
            const float longHidingTime = 20.0f;
            if (IsHunting() || IsInvestigatingNoise() ||
                (IsHiding() && GetTask() != FOLLOW &&
                 GetHidingTime() > longHidingTime)) {
              const float someTime = 10.0f;
              const float farAway = 750.0f;
              if (GetTimeSinceLastSawEnemy() > someTime &&
                  (playerOrigin - GetAbsOrigin())
  ...
  } else // an enemy was killed
  {
    ...
    if (killer && killer->GetTeamNumber() == GetTeamNumber()) {
      // only chatter about enemy kills if we see them occur, and they were the
      // last one we see
      if (GetNearbyEnemyCount() <= 1) {
        // report if number of enemies left is few and we killed the last one we
        // saw locally
...
void CCSBot::OnPlayerRadio(IGameEvent *event) {
  ...
  if (player && player->GetTeamNumber() == GetTeamNumber()) {
    ...
    if (radioEvent != RADIO_INVALID && radioEvent != RADIO_AFFIRMATIVE &&
        radioEvent != RADIO_NEGATIVE && radioEvent != RADIO_REPORTING_IN) {
...
void CCSBot::OnPlayerFallDamage(IGameEvent *event) {
  ...
  OnAudibleEvent(event, player, 1100.0f, PRIORITY_LOW,
                 false); // player_falldamage
}
...
void CCSBot::OnPlayerFootstep(IGameEvent *event) {
  ...
  OnAudibleEvent(event, player, 1100.0f, PRIORITY_LOW, false,
                 IS_FOOTSTEP); // player_footstep
}
