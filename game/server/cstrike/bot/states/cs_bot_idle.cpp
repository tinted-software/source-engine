...
/**
 * The Idle state.
 * We never stay in the Idle state - it is a "home base" for the state machine
 * that does various checks to determine what we should do next.
 */
void IdleState::OnEnter(CCSBot *me) {
  ...
  //
  // Since Idle assigns tasks, we assume that coming back to Idle means our task
  // is complete
  //
...
void IdleState::OnUpdate(CCSBot *me) {
  ...
  if (me->IsSafe()) {
    ...
    if (!me->EquipGrenade()) {
      // high-skill bots run with the knife, unless using the Scout (which moves
      // faster)
  ...
  if (me->GetGameState()->IsRoundOver()) {
    ...
    if (me->GetHostageEscortCount()) {
      const CCSBotManager::Zone *zone = TheCSBots()->GetClosestZone(
          me->GetLastKnownArea(), PathCost(me, FASTEST_ROUTE));
      ...
      if (zonePos) {
        ...
        me->PrintIfWatched(
            "Trying to rescue hostages at the end of the round\n");
        return;
      }
    }
  ...
  case CCSBotManager::SCENARIO_DEFUSE_BOMB: {
    ...
    if (me->GetTeamNumber() == TEAM_TERRORIST) {
      if (me->GetGameState()->IsBombPlanted()) {
        if (me->GetGameState()->GetPlantedBombsite() != CSGameState::UNKNOWN) {
          ...
          if (zone) {
            ...
            if (place != UNDEFINED_PLACE) {
              ...
              const Vector *spot =
                  FindRandomHidingSpot(me, place, me->IsSniper());
              if (spot) {
                me->Hide(*spot);
                return;
              }
          ...
          // move to bombsite - if we reach it, we'll update its cleared status,
          // causing us to select another
          const Vector *pos = TheCSBots()->GetRandomPositionInZone(
              TheCSBots()->GetZone(zoneIndex));
          if (pos) {
        ...
        // at the start of the round, we may decide to defend "initial
        // encounter" areas where we will first meet the enemy rush
        if (me->IsSafe()) {
        ...
        // small chance of sniper camping on offense, if we aren't carrying the
        // bomb
        if (me->GetFriendsRemaining() && me->IsSniper() &&
            RandomFloat(0, 100.0f) < offenseSniperCampChance) {
          me->SetTask(CCSBot::MOVE_TO_SNIPER_SPOT);
          me->Hide(me->GetLastKnownArea(), RandomFloat(10.0f, 30.0f),
                   sniperHideRange);
          me->SetDisposition(CCSBot::OPPORTUNITY_FIRE);
        ...
        // if bomb has been planted, and we hear it, move to a hiding spot near
        // the bomb and guard it
        if (!me->IsRogue() && me->GetGameState()->IsBombPlanted() &&
            me->GetGameState()->GetBombPosition()) {
           ...
           // ------------------------------------------------------------------------------------------
    {
      if (me->GetGameState()->IsBombPlanted()) {
        ...
        } else if (me->GetGameState()->GetPlantedBombsite() !=
                   CSGameState::UNKNOWN) {
        ...
        AssertMsg(
            0, "A CT bot doesn't know what to do while the bomb is planted!\n");
      }
      ...
      // only decide to camp at the start of the round, or if we haven't seen
        ...
        if (zone) {
          ...
          if (pos) {
            ...
            if (me->GetFriendsRemaining() &&
                TheCSBots()->GetRadioMessageInterval(
                    RADIO_FOLLOW_ME, me->GetTeamNumber()) > repeatTime)
    ...
    if (me->GetTeamNumber() == TEAM_TERRORIST) {
      ...
      if (me->IsSafe()) {
        ...
        const float campZoneChance =
            100.0f * (TheCSBots()->GetElapsedRoundTime() - me->GetSafeTime()) /
            120.0f;
      ...
      if (me->IsSniper()) {
        ...
        if (RandomFloat(0, 100) <= defenseSniperCampChance) {
          const Vector *hostagePos =
              me->GetGameState()->GetRandomFreeHostagePosition();
          if (hostagePos && campHostages) {
            me->SetTask(CCSBot::MOVE_TO_SNIPER_SPOT);
            me->PrintIfWatched("Sniping near hostages\n");
            me->Hide(TheNavMesh->GetNearestNavArea(*hostagePos), -1.0,
                     sniperHideRange);
            me->SetDisposition(CCSBot::OPPORTUNITY_FIRE);
            return;
          } else {
      ...
      if (me->GetFriendsRemaining()) {
        if (me->IsRogue() || TheCSBots()->IsDefenseRushing() ||
            RandomFloat(0, 100) < huntChance) {
          me->Hunt();
          return;
        }
      }

      // at the start of the round, we may decide to defend "initial encounter"
      // areas where we will first meet the enemy rush
      if (me->IsSafe()) {
        float defendRushChance = -17.0f * (me->GetMorale() - 2);
      ...
      if (hostagePos && campHostages) {
        ...
        if (area) {
          ...
          float hostageGuardRange =
              750.0f + 250.0f * (me->GetMorale() + 3); // 2000
           ...
           // ---------------------------------------------------------------------------------
    {
      ...
      if (!me->GetHostageEscortCount()) {
        ...
        if (me->GetFriendsRemaining() && !me->GetHostageEscortCount()) {
          ...
          // if we have friends left, we might go hunting instead of hostage
          // rescuing
      ...
      // look for free hostages - CT's have radar so they know where hostages
      // are at all times
      CHostage *hostage = me->GetGameState()->GetNearestFreeHostage();

      // if we are not allowed to do the scenario, guard the hostages to clear
      // the area for the human(s)
      if (!me->IsDoingScenario()) {
        if (hostage) {
          CNavArea *area = TheNavMesh->GetNearestNavArea(GetCentroid(hostage));
          if (area) {
      ...
      // determine whether it's faster to rescue the ones we have, or go get the
      // remaining ones
      if (hostage) {
        ...
        if (zone) {
          ...
          float toHostage = NavAreaTravelDistance(
              me->GetLastKnownArea(),
              TheNavMesh->GetNearestNavArea(GetCentroid(hostage)), cost);
      ...
      if (fetchHostages) {
        ...
        // if we already have some hostages, move to the others by the quickest
        // route
        RouteType route =
            (me->GetHostageEscortCount()) ? FASTEST_ROUTE : SAFEST_ROUTE;
