...
void MoveToState::OnUpdate(CCSBot *me) {
  ...
  if (me->GetTask() == CCSBot::MOVE_TO_LAST_KNOWN_ENEMY_POSITION) {
    /// @todo Account for reaction time so we take some time to realized the
    /// enemy is dead
  ...
  case CCSBotManager::SCENARIO_DEFUSE_BOMB: {
    ...
    if (me->GetTeamNumber() == TEAM_CT) {
      if (me->GetGameState()->IsBombPlanted()) {
        switch (me->GetTask()) {
        case CCSBot::DEFUSE_BOMB: {
          // if we are near the bombsite and there is time left, sneak in
          // (unless all enemies are dead)
          ...
          // if we are trying to defuse the bomb, and someone has started
          // defusing, guard them instead
          ...
          // if we are near the bomb, defuse it (if we are reloading, don't try
          // to defuse until we finish)
      ...
      if (me->GetTask() == CCSBot::PLANT_BOMB) {
        if (me->GetFriendsRemaining()) {
          ...
          if (!m_askedForCover) {
            ...
            if (me->IsAtBombsite() &&
                me->GetPathDistanceRemaining() < nearPlantSite) {
              // radio to the team
              me->GetChatter()->PlantingTheBomb(me->GetPlace());
              m_askedForCover = true;
            }

            // after we have started to move to the bombsite, tell team we're
            // going to plant, and where don't do this if we have already
            // radioed that we are starting to plant
            if (!m_radioedPlan) {
              const float radioTime = 2.0f;
              if (gpGlobals->curtime - me->GetStateTimestamp() > radioTime) {
                // radio to the team if we're more than 10 seconds (2400 units)
                // out
                const float nearPlantSite = 2400.0f;
                if (me->GetPathDistanceRemaining() >= nearPlantSite) {
                  me->GetChatter()->GoingToPlantTheBomb(
                      TheNavMesh->GetPlace(m_goalPosition));
                }
                m_radioedPlan = true;
              }
            }
  ...
  case CCSBotManager::SCENARIO_RESCUE_HOSTAGES: {
    if (me->GetTask() == CCSBot::COLLECT_HOSTAGES) {
      //
      // Since CT's have a radar, they can directly look at the actual hostage
      // state
      //

      // check if someone else collected our hostage, or the hostage died or was
      // rescued
      CHostage *hostage = static_cast<CHostage *>(me->GetGoalEntity());
      if (hostage == NULL || !hostage->IsValid() ||
          hostage->IsFollowingSomeone()) {
        me->Idle();
        return;
      }
      ...
      if (!me->IsUsingLadder()) {
        ...
        Vector to = pos - me->EyePosition(); // "Use" checks from eye position,
                                             // so we should too
        ...
        if (to.IsLengthLessThan(watchHostageRange)) {
          ...
          // randomly move just a bit to avoid infinite use loops from bad
          // hostage placement
          ...
          const float useRange =
              PLAYER_USE_RADIUS -
              10.0f; // shave off a fudge factor to make sure we're within range
  ...
  if (me->UpdatePathMovement() != CCSBot::PROGRESSING) {
    ...
    case CCSBot::MOVE_TO_LAST_KNOWN_ENEMY_POSITION: {
      ...
      if (victim && victim->IsAlive()) {
        ...
        BotStatement *say =
            new BotStatement(me->GetChatter(), REPORT_ENEMY_LOST, 8.0f);
