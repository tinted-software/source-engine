 ...
 * NOTE: Do not forget this state may include a very long "move-to" time to get
 * to our hidey spot!
 */
void HideState::OnEnter(CCSBot *me) {
  ...
  // if duration is "infinite", set it to a reasonably long time to prevent
  // infinite camping
  if (m_duration < 0.0f) {
    m_duration = RandomFloat(30.0f, 60.0f);
  }

  // decide whether to "ambush" or not - never set to false so as not to
  // override external setting
  ...
  // if we are a sniper, we need to periodically pause while we retreat to
  // squeeze off a shot or two
  if (me->IsSniper()) {
...
void HideState::OnUpdate(CCSBot *me) {
  ...
  if (!me->IsReloading()) {
    // if we are momentarily hiding while following someone, check to see if he
    // has moved on
    if (me->IsFollowing()) {
      CCSPlayer *leader = static_cast<CCSPlayer *>(
          static_cast<CBaseEntity *>(me->GetFollowLeader()));
    ...
    /// @todo - Perhaps tie in to TakeDamage(), so it works for human players,
    /// too
    ...
    case CCSBotManager::SCENARIO_DEFUSE_BOMB: {
      if (me->GetTeamNumber() == TEAM_CT) {
        // if we are just holding position (due to a radio order) and the bomb
        // has just planted, go defuse it
        ...
        // if we are guarding the defuser and he dies/gives up, stop hiding (to
        // choose another defuser)
        if (me->GetTask() == CCSBot::GUARD_BOMB_DEFUSER &&
            TheCSBots()->GetBombDefuser() == NULL) {
          me->Idle();
          return;
        }
        ...
        if (me->GetTask() == CCSBot::GUARD_LOOSE_BOMB &&
            TheCSBots()->GetLooseBomb() == NULL) {
          me->GetChatter()->TheyPickedUpTheBomb();
          me->Idle();
          return;
        }

        // if we are guarding a bombsite and the bomb is dropped and we hear
        // about it, stop guarding
        if (me->GetTask() == CCSBot::GUARD_BOMB_ZONE &&
            me->GetGameState()->IsLooseBombLocationKnown()) {
          me->Idle();
          return;
        }

        // if we are guarding (bombsite, initial encounter, etc) and the bomb is
        // planted, go defuse it
      ...
      {
        // if we are near the ticking bomb and someone starts defusing it,
        // attack!
        if (TheCSBots()->GetBombDefuser()) {
          ...
          if (toDefuser.IsLengthLessThan(hearDefuseRange)) {
            // if we are nearby, attack, otherwise move to the bomb (which will
            // cause us to attack when we see defuser)
    ...
    case CCSBotManager::SCENARIO_RESCUE_HOSTAGES: {
      // if we're guarding the hostages and they all die or are taken, do
      // something else
      if (me->GetTask() == CCSBot::GUARD_HOSTAGES) {
        if (me->GetGameState()->AreAllHostagesBeingRescued() ||
            me->GetGameState()->AreAllHostagesGone()) {
          me->Idle();
          return;
        }
    ...
    // only investigate noises if we are initiating attacks, and we aren't a
    // "settled in" sniper dont investigate noises if we are reloading
    if (!me->IsReloading() && !isSettledInSniper &&
        me->GetDisposition() == CCSBot::ENGAGE_AND_INVESTIGATE) {
      // if we are holding position, and have heard the enemy nearby,
      // investigate after our hold time is up
      if (m_isHoldingPosition && m_heardEnemy &&
          (gpGlobals->curtime - m_firstHeardEnemyTime > m_holdPositionTime)) {
        /// @todo We might need to remember specific location of last enemy
        /// noise here
        me->InvestigateNoise();
        return;
      }
      ...
      if (me->HeardInterestingNoise()) {
        // if we are holding position, check if enough time has elapsed since we
        // first heard the enemy
        if (m_isAtSpot && m_isHoldingPosition) {
          if (!m_heardEnemy) {
            ...
            me->PrintIfWatched(
                "Heard enemy, holding position for %f2.1 seconds...\n",
                m_holdPositionTime);
          }
        } else {
  ...
  if (m_isAtSpot) {
    ...
    if (m_hideTimer.IsElapsed()) {
      if (me->GetTask() == CCSBot::GUARD_LOOSE_BOMB) {
        // if we're guarding the loose bomb, continue to guard it but pick a new
        // spot
        me->Hide(TheCSBots()->GetLooseBombArea());
        return;
      } else if (me->GetTask() == CCSBot::GUARD_BOMB_ZONE) {
        // if we're guarding a bombsite, continue to guard it but pick a new
        // spot
        ...
        // if we're guarding a rescue zone, continue to guard this or another
        // rescue zone
    ...
    /*
                    // if we are watching for an approaching noisy enemy,
       anticipate and fire before they round the corner
                    /// @todo Need to check if we are looking at an ENEMY_NOISE
       here const float veryCloseNoise = 250.0f; if (me->IsLookingAtSpot() &&
       me->GetNoiseRange() < veryCloseNoise)
                    {
                            // fire!
                            me->PrimaryAttack();
                            me->PrintIfWatched( "Firing at anticipated enemy
    ...
    */
    ...
    // while sitting at our hiding spot, if we are being attacked but can't see
    // our attacker, move somewhere else
    ...
    if (!me->IsDoingScenario()) {
      if (me->GetTeamNumber() == TEAM_CT) {
        if (me->GetTask() == CCSBot::GUARD_BOMB_ZONE && me->IsAtHidingSpot() &&
            TheCSBots()->IsBombPlanted()) {
          if (me->GetNearbyEnemyCount() == 0) {
            ...
            if (TheCSBots()->GetBombTimeLeft() > someTime)
              me->GetChatter()->Encourage("BombsiteSecure",
                                          RandomFloat(10.0f, 15.0f));
            else if (TheCSBots()->GetBombTimeLeft() > littleTime)
              me->GetChatter()->Encourage("WaitingForHumanToDefuseBomb",
                                          RandomFloat(5.0f, 8.0f));
            else
              me->GetChatter()->Encourage("WaitingForHumanToDefuseBombPanic",
                                          RandomFloat(3.0f, 4.0f));
          }
        }
        ...
        if (me->GetTask() == CCSBot::GUARD_HOSTAGES && me->IsAtHidingSpot()) {
          if (me->GetNearbyEnemyCount() == 0) {
            CHostage *hostage =
                me->GetGameState()->GetNearestVisibleFreeHostage();
            if (hostage) {
              me->GetChatter()->Encourage("WaitingForHumanToRescueHostages",
                                          RandomFloat(10.0f, 15.0f));
            }
          }
        }
    ...
    float range;
    CCSPlayer *camper =
        static_cast<CCSPlayer *>(UTIL_GetClosestPlayer(m_hidingSpot, &range));

    const float closeRange = 75.0f;
    if (camper && camper != me && range < closeRange &&
        me->IsVisible(camper, CHECK_FOV)) {
    ...
    if (!me->IsEnemyVisible() && !m_isLookingOutward) {
      ...
      if (range < lookOutwardRange && range > nearSpotRange) {
        ...
        me->SetLookAt("Face outward", me->EyePosition() - 1000.0f * toSpot,
                      PRIORITY_HIGH, 3.0f);
      }
    }
    ...
    if (range < atDist) {
      ...
      for (float angle = 0.0f; angle < 360.0f; angle += 45.0f) {
        UTIL_TraceLine(me->EyePosition(),
                       me->EyePosition() +
                           1000.0f * Vector(BotCOS(angle), BotSIN(angle), 0.0f),
    ...
    if (me->UpdatePathMovement() != CCSBot::PROGRESSING && !m_isAtSpot) {
      ...
      const Vector *pos =
          FindNearbyHidingSpot(me, m_hidingSpot, m_range, me->IsSniper());
      if (pos == NULL) {
