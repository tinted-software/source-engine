...
void AttackState::OnEnter(CCSBot *me) {
  ...
  if (me->IsUsingKnife()) {
    ...
    // decide whether to crouch where we are, or run and gun (if we havent
    // already - see CCSBot::Attack())
    if (!m_crouchAndHold) {
      if (enemy) {
        ...
        if (me->IsUsingSniperRifle())
          crouchChance = 50.0f;
        else if ((GetCentroid(me) - GetCentroid(enemy))
                     .IsLengthGreaterThan(crouchFarRange))
          crouchChance = 50.0f;
        else
        ...
        if (RandomFloat(0.0f, 100.0f) < crouchChance) {
          ...
          UTIL_TraceLine(origin, enemy->EyePosition(), MASK_PLAYERSOLID, me,
  ...
  m_isCoward =
      (RandomFloat(0, 100) > 100.0f * me->GetProfile()->GetAggression());
}
...
void AttackState::Dodge(CCSBot *me) {
  ...
  if (m_shouldDodge && !me->IsUsingSniperRifle() && !m_crouchAndHold) {
    ...
    // move towards (or away from) enemy if we are using a knife, behind a
    // corner, or we aren't very skilled
    ...
    if (!me->CanSeeSniper() &&
        (range > dodgeRange || !me->IsPlayerFacingMe(enemy))) {
      ...
      if (me->GetProfile()->GetSkill() > 0.5f && me->CanSeeSniper()) {
        ...
        do {
          // low-skill bots may jump when first engaging the enemy (if they are
          // moving)
          const float jumpChance = 33.3f;
          if (m_firstDodge && me->GetProfile()->GetSkill() < 0.5f &&
              RandomFloat(0, 100) < jumpChance && !me->IsNotMoving())
...
void AttackState::OnUpdate(CCSBot *me) {
  ...
  // if we somehow ended up with the C4 or a grenade in our hands, grab our
  // weapon!
  ...
  if (m_retreatTimer.IsElapsed()) {
    ...
    if (isPinnedDown || (me->CanSeeSniper() && !me->IsSniper()) ||
        (me->IsOutnumbered() && m_isCoward) ||
        (me->OutnumberedCount() >= 2 &&
         me->GetProfile()->GetAggression() < 1.0f)) {
      ...
      if (me->IsAnyVisibleEnemyLookingAtMe(CHECK_FOV)) {
        ...
        // try to retreat
        if (me->TryToRetreat()) {
          // if we are a sniper, equip our pistol so we can fire while
          // retreating
          /*
          if (me->IsUsingSniperRifle())
          {
  ...
  if (me->IsUsingKnife()) {
    ...
    if ((enemy->GetAbsOrigin() - me->GetAbsOrigin())
            .IsLengthGreaterThan(slashRange)) {
      ...
      if (me->HasPath()) {
        ...
        if ((me->GetPathEndpoint() - enemy->GetAbsOrigin())
                .IsLengthGreaterThan(repathRange)) {
          repath = true;
        }
      } else {
        repath = true;
  ...
  if (me->HasShield()) {
    if (me->IsEnemyVisible() && !m_shieldForceOpen) {
      if (!me->IsRecognizedEnemyReloading() && !me->IsReloading() &&
          me->IsPlayerLookingAtMe(enemy)) {
  ...
  if (me->IsUsingSniperRifle()) {
    ...
    const float sniperMinRange =
        160.0f; // NOTE: Must be larger than NO_ZOOM range in AdjustZoom()
  ...
  // if we're sniping, look through the scope - need to do this here in case a
  // reload resets our scope
  if (me->IsUsingSniperRifle()) {
  ...
  if (me->IsAwareOfEnemyDeath()) {
    ...
    if (me->GetLastVictimID() == enemy->entindex() &&
        me->GetNearbyEnemyCount() <= 1) {
      me->GetChatter()->KilledMyEnemy(enemy->entindex());

      // if there are other enemies left, wait a moment - they usually come in
      // groups
  ...
  // if we haven't seen our enemy for a moment, continue on if we dont want to
  // fight, or decide to ambush if we do
  if (!me->IsEnemyVisible()) {
    ...
    if (m_isEnemyHidden) {
      m_reacquireTimestamp =
          gpGlobals->curtime + me->GetProfile()->GetReactionTime();
      m_isEnemyHidden = false;
    }
  }
  ...
  // if we can't see the enemy, and have either seen him but currently lost
  // sight of him, or haven't yet seen him, chase after him (unless we are a
  // sniper)
  if (!me->IsEnemyVisible() &&
      (notSeenEnemyTime > chaseTime || !m_haveSeenEnemy)) {
  ...
  if (!me->IsEnemyVisible() && me->GetTimeSinceAttacked() < hurtRecentlyTime &&
      me->GetAttacker() && me->GetAttacker() != me->GetBotEnemy()) {
