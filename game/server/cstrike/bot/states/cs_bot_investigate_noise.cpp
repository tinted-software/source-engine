...
void InvestigateNoiseState::AttendCurrentNoise(CCSBot *me) {
  ...
  if (me->IsWellPastSafe() && me->HasNotSeenEnemyForLongTime() &&
      me->GetNoisePriority() != PRIORITY_HIGH)
...
//--------------------------------------------------------------------------------------------------------------
void InvestigateNoiseState::OnEnter(CCSBot *me) { AttendCurrentNoise(me); }
...
void InvestigateNoiseState::OnUpdate(CCSBot *me) {
  ...
  if (me->IsUsingKnife()) {
    ...
    if (range < closeToNoiseRange) {
      // if we dont have many friends left, or we are alone, and we are near
      // noise source, sneak quietly
      if ((me->GetNearbyFriendCount() == 0 || me->GetFriendsRemaining() <= 2) &&
          !me->IsHurrying()) {
        me->Walk();
      } else {
        me->Run();
  ...
  // if we can see the noise position and we're close enough to it and looking
  // at it, we don't need to actually move there (it's checked enough)
