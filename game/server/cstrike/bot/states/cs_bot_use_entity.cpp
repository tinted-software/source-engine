 ...
 * NOTE: This state assumes we are standing in range of the entity to be used,
 * with no obstructions.
 */
void UseEntityState::OnEnter(CCSBot *me) {}

void UseEntityState::OnUpdate(CCSBot *me) {
  // in the very rare situation where two or more bots "used" a hostage at the
  // same time, one bot will fail and needs to time out of this state
  ...
  if (me->IsLookingAtPosition(pos)) {
    if (TheCSBots()->GetScenario() == CCSBotManager::SCENARIO_RESCUE_HOSTAGES &&
      ...
      // we are collecting a hostage, assume we were successful - the update
      // check will correct us if we weren't
