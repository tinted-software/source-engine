...
//--------------------------------------------------------------------------------------------------------------
ConVar bot_loadout("bot_loadout", "", FCVAR_CHEAT,
                   "bots are given these items at round start");
ConVar bot_randombuy("bot_randombuy", "0", FCVAR_CHEAT,
                     "should bots ignore their prefered weapons and just buy "
                     "weapons at random?");
...
static bool HasDefaultPistol(CCSBot *me) {
  CWeaponCSBase *pistol =
      (CWeaponCSBase *)me->Weapon_GetSlot(WEAPON_SLOT_PISTOL);
...
void BuyState::OnEnter(CCSBot *me) {
  ...
  if (cheatWeaponString && *cheatWeaponString) {
    m_doneBuying =
        false; // we're going to be given weapons - ignore the eco limit
  } else {
  ...
  if (me->GetTeamNumber() == TEAM_CT) {
    if (TheCSBots()->GetScenario() == CCSBotManager::SCENARIO_DEFUSE_BOMB) {
      // CT's sometimes buy defuse kits in the bomb scenario (except in career
      // mode, where the player should defuse)
      if (CSGameRules()->IsCareer() == false) {
        const float buyDefuseKitChance =
            100.0f * (me->GetProfile()->GetSkill() + 0.2f);
...
void BuyState::OnUpdate(CCSBot *me) {
  ...
  // If we're supposed to buy a specific weapon for debugging, do so and then
  // bail
  const char *cheatWeaponString = bot_loadout.GetString();
  if (cheatWeaponString && *cheatWeaponString) {
  ...
  if (!me->IsInBuyZone()) {
    ...
    CONSOLE_ECHO("%s bot spawned outside of a buy zone (%d, %d, %d)\n",
                 ...
                 (int)me->GetAbsOrigin().x, (int)me->GetAbsOrigin().y,
  ...
  if (gpGlobals->curtime - me->GetStateTimestamp() > buyInterval) {
    ...
    if (m_prefIndex < me->GetProfile()->GetWeaponPreferenceCount() &&
      ...
      Q_snprintf(weaponPreferenceName, sizeof(weaponPreferenceName),
                 "weapon_%s",
                 me->GetProfile()->GetWeaponPreferenceAsString(m_prefIndex));
      if (me->Weapon_OwnsThisType(
              weaponPreferenceName)) // Prefs and buyalias use the short
                                     // version, this uses the long
      {
    ...
    // if we have no preferred primary weapon (or everything we want is
    // disallowed), buy at random
    if (!me->HasPrimaryWeapon() &&
        (isPreferredAllDisallowed ||
         !me->GetProfile()->HasPrimaryPreference())) {
      if (m_buyShield) {
        // buy a shield
        CCommand args;
        ...
        BuyInfo *masterPrimary = (me->GetTeamNumber() == TEAM_TERRORIST)
        ...
        bool wantSniper =
            (RandomFloat(0, 100) < sniperRifleChance) ? true : false;
        ...
        for (int i = 0; i < PRIMARY_WEAPON_BUY_COUNT; ++i) {
          if ((masterPrimary[i].type == SHOTGUN &&
               TheCSBots()->AllowShotguns()) ||
              (masterPrimary[i].type == SUB_MACHINE_GUN &&
               TheCSBots()->AllowSubMachineGuns()) ||
              (masterPrimary[i].type == RIFLE && TheCSBots()->AllowRifles()) ||
              (masterPrimary[i].type == SNIPER_RIFLE &&
               TheCSBots()->AllowSnipers() && wantSniper) ||
              (masterPrimary[i].type == MACHINE_GUN &&
        ...
        if (stockPrimaryCount) {
          ...
          // on hard difficulty levels, bots try to buy preferred weapons on the
          // first pass
    ...
    if (me->HasPrimaryWeapon() || m_retries++ > 5) {
      ...
      if (m_buyGrenade && !me->HasGrenade()) {
        if (UTIL_IsTeamAllBots(me->GetTeamNumber())) {
          // only allow Flashbangs if everyone on the team is a bot (dont want
          // to blind our friendly humans)
