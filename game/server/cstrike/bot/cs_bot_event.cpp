 ...
 */
void CCSBot::OnAudibleEvent(IGameEvent *event, CBasePlayer *player, float range,
  ...
  // If the event occurs far from the triggering player, it may override the
  // origin
  if (actualOrigin) {
  ...
  if (newNoiseDist < range) {
    // we heard the sound
    if ((IsLocalPlayerWatchingMe() && cv_bot_debug.GetInt() == 3) ||
        cv_bot_debug.GetInt() == 4) {
    ...
    if (m_noiseTimestamp > 0.0f) {
      // only overwrite recent sound if we are louder (closer), or more
      // important - if old noise was long ago, its faded
    ...
    /// @todo Make sure noise area is not through a wall or ceiling from source
    /// of noise
    ...
    /// @todo Make sure not to pick a position on the opposite side of
    /// ourselves.
    ...
    m_noisePosition.x =
        newNoisePosition->x + RandomFloat(-errorRadius, errorRadius);
    m_noisePosition.y =
        newNoisePosition->y + RandomFloat(-errorRadius, errorRadius);
    ...
    if (m_noiseTravelDistance < nearbyNoiseRange &&
        m_noiseTravelDistance > 0.0f) {
      BecomeAlert();
    }
  }
}
...
void CCSBot::OnHEGrenadeDetonate(IGameEvent *event) {
  ...
  OnAudibleEvent(event, player, 99999.0f, PRIORITY_HIGH,
                 true); // hegrenade_detonate
}
...
void CCSBot::OnFlashbangDetonate(IGameEvent *event) {
  ...
  OnAudibleEvent(event, player, 1000.0f, PRIORITY_LOW,
                 true); // flashbang_detonate
}
...
void CCSBot::OnBulletImpact(IGameEvent *event) {
  ...
  // Construct an origin for the sound, since it can be far from the originating
  // player
  Vector actualOrigin;
  ...
  /// @todo Ignoring bullet impact events for now - we dont want bots to look
  /// directly at them!
  // OnAudibleEvent( event, player, 1100.0f, PRIORITY_MEDIUM, true, false,
  // &actualOrigin ); // bullet_impact
}
...
void CCSBot::OnHostageFollows(IGameEvent *event) {
  ...
  if (GetTeamNumber() == TEAM_TERRORIST) {
    ...
    if (GetGameState()->GetNearestVisibleFreeHostage() == NULL) {
      ...
      // if we're close enough to "hear" this event, either go to where the
      // event occured, or head for an escape zone to head them off
  ...
  OnAudibleEvent(event, player, range, PRIORITY_MEDIUM,
                 false); // hostage_follows
}
