...
//--------------------------------------------------------------------------------------------------------------
#if DEBUG_BREAKABLES
static void DrawOutlinedQuad(const Vector &p1, const Vector &p2,
                             const Vector &p3, const Vector &p4, int r, int g,
...
CBaseEntity *CheckForEntitiesAlongSegment(const Vector &start,
  ...
  partition->EnumerateElementsAlongRay(PARTITION_ENGINE_SOLID_EDICTS, ray,
                                       false, enumerator);
...
#if DEBUG_BREAKABLES
  if (entity) {
    DrawOutlinedQuad(start + mins, start + maxs, end + maxs, end + mins, 255, 0,
                     0, bot_debug_breakable_duration.GetFloat());
  } else {
    DrawOutlinedQuad(start + mins, start + maxs, end + maxs, end + mins, 0, 255,
                     0, 0.1);
  }
#endif // DEBUG_BREAKABLES
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Look up to 'distance' units ahead on the bot's path for entities.  Returns
 * the closest one.
 */
CBaseEntity *CCSBot::FindEntitiesOnPath(float distance,
                                        CPushAwayEnumerator *enumerator,
                                        bool checkStuck) {
  Vector goal;
  ...
  CBaseEntity *entity = NULL;
  if (isDegeneratePath) {
    entity = CheckForEntitiesAlongSegment(
        WorldSpaceCenter(), m_goalPosition + Vector(0, 0, HalfHumanHeight),
        mins, maxs, enumerator);
#if DEBUG_BREAKABLES
    if (entity) {
      NDebugOverlay::HorzArrow(WorldSpaceCenter(), m_goalPosition, 6, 0, 0, 255,
                               255, true,
    ...
    // HACK: start with an index one lower than normal, so we can trace from the
    // bot's location to the start of the path nodes.
    for (int i = startIndex - 1; i < m_pathLength - 1; ++i) {
      ...
      entity = CheckForEntitiesAlongSegment(
          start + Vector(0, 0, HalfHumanHeight),
      ...
      if (m_path[i].ladder && !IsOnLadder() &&
          distance > NearBreakableCheckDist) // don't try to break breakables on
                                             // the other end of a ladder
        break;
      ...
      if (i != startIndex - 1 && m_path[i + 1].ladder) {
        ...
        start = (m_path[i + 1].how == GO_LADDER_DOWN)
                    ? m_path[i + 1].ladder->m_bottom
...
void CCSBot::PushawayTouch(CBaseEntity *pOther) {
  ...
  if (enumerator.m_nAlreadyHit == 1) {
    ...
    SetLookAt("Breakable", pOther->WorldSpaceCenter(), PRIORITY_HIGH, 0.1f,
              false, 5.0f, true);
  }
}
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Check for breakable physics props and other breakable entities.  We do this
 * here instead of catching them in OnTouch() because players don't collide with
 * physics props, so OnTouch() doesn't get called.  Also, looking ahead like
 * this lets us anticipate when we'll need to break something, and do it before
 * being stopped by it.
 */
void CCSBot::BreakablesCheck(void) {
  ...
  ray, false, &enumerator ); for ( int i=0; i<enumerator.m_nAlreadyHit; ++i )
          {
                  CBaseEntity *prop = props[i];
                  if ( prop && prop->m_takedamage == DAMAGE_YES )
                  {
                          CFmtStr msg;
                          const char *text = msg.sprintf( "%s, %d health",
  prop->GetClassname(), prop->m_iHealth ); if ( prop->m_iHealth > 200 )
                          {
                                  NDebugOverlay::EntityBounds( prop, 255, 0, 0,
  10, 0.2f ); prop->EntityText( 0, text, 0.2f, 255, 0, 0, 255 );
                          }
                          else
                          {
                                  NDebugOverlay::EntityBounds( prop, 0, 255, 0,
  10, 0.2f ); prop->EntityText( 0, text, 0.2f, 0, 255, 0, 255 );
                          }
                  }
          }
  }
  */
  ...
  if (IsAttacking()) {
    ...
    if (IsUsingKnife() && m_enemy != NULL) {
      ...
      breakable = CheckForEntitiesAlongSegment(WorldSpaceCenter(),
                                               m_enemy->WorldSpaceCenter(),
                                               mins, maxs, &enumerator);
      if (breakable) {
#if DEBUG_BREAKABLES
        NDebugOverlay::HorzArrow(WorldSpaceCenter(),
                                 m_enemy->WorldSpaceCenter(), 6, 0, 0, 255, 255,
                                 true, bot_debug_breakable_duration.GetFloat());
#endif // DEBUG_BREAKABLES

        // look at it (chances are we'll already be looking at it, since it's
        // between us and our enemy)
        SetLookAt("Breakable", breakable->WorldSpaceCenter(), PRIORITY_HIGH,
                  0.1f, false, 5.0f, true);

        // break it (again, don't wait: we don't have ammo, since we're using
        // the knife, and we're looking mostly at it anyway)
        PrimaryAttack();
      }
    }
    return;
  ...
  CBotBreakableEnumerator enumerator(breakables, ARRAYSIZE(breakables));
  CBaseEntity *breakable =
      FindEntitiesOnPath(NearBreakableCheckDist, &enumerator, true);
  ...
  if (breakable) {
    // look at it
    SetLookAt("Breakable", breakable->WorldSpaceCenter(), PRIORITY_HIGH, 0.1f,
              false, 5.0f, true);
  }
...
void CCSBot::DoorCheck(void) {
  if (IsAttacking() && !IsUsingKnife()) {
    // If we're attacking with a gun or nade, don't bother with doors.  If we're
    // trying to knife someone, we might need to open a door.
  ...
  CBaseEntity *door =
      FindEntitiesOnPath(NearBreakableCheckDist, &enumerator, false);
...
void CCSBot::StuckCheck(void) {
  if (m_isStuck) {
    ...
    if (m_avgVelCount < MAX_VEL_SAMPLES) {
      ...
      // cannot make this velocity too high, or bots will get "stuck" when going
      // down ladders
      ...
      if (avgVel < stuckVel) {
        ...
        if (IsLocalPlayerWatchingMe() && cv_bot_debug.GetInt() > 0.0f &&
            UTIL_GetListenServerHost()) {
          CBasePlayer *localPlayer = UTIL_GetListenServerHost();
          CSingleUserRecipientFilter filter(localPlayer);
 ...
 */
bool CCSBot::GetSimpleGroundHeightWithFloor(const Vector &pos, float *height,
                                            Vector *normal) {
...
void CCSBot::MoveTowardsPosition(const Vector &pos) {
  ...
  if ((m_lastKnownArea == NULL ||
       !(m_lastKnownArea->GetAttributes() & NAV_MESH_NO_JUMP)) &&
      !IsOnLadder()) {
    float ground;
    ...
    if (!jumped) {
      // close up jumping
      const float lookAheadRange =
          30.0f; // cant be less or will miss jumps over low walls
...
void CCSBot::Wiggle(void) {
  ...
  const float lookAheadRange =
      (m_lastKnownArea && (m_lastKnownArea->GetAttributes() & NAV_MESH_WALK))
          ? 5.0f
          : 30.0f;
  float ground;
  ...
  if (m_stuckJumpTimer.IsElapsed() && m_lastKnownArea &&
      !(m_lastKnownArea->GetAttributes() & NAV_MESH_NO_JUMP)) {
    if (Jump()) {
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Determine approach points from eye position and approach areas of current
 * area
 */
void CCSBot::ComputeApproachPoints(void) {
  ...
  Vector ap;
  float halfWidth;
  for (int i = 0; i < m_lastKnownArea->GetApproachInfoCount() &&
                  m_approachPointCount < MAX_APPROACH_POINTS;
       ++i) {
    ...
    if (info->prevToHereHow <= GO_WEST) {
      info->prev.area->ComputePortal(
          info->here.area, (NavDirType)info->prevToHereHow, &ap, &halfWidth);
    ...
    // "bend" our line of sight around corners until we can see the approach
    // point
    Vector bendPoint;
...
void CCSBot::DrawApproachPoints(void) const {
  for (int i = 0; i < m_approachPointCount; ++i) {
    if (TheCSBots()->GetElapsedRoundTime() >=
        m_approachPoint[i].m_area->GetEarliestOccupyTime(
            OtherTeam(GetTeamNumber())))
      NDebugOverlay::Cross3D(m_approachPoint[i].m_pos, 10.0f, 255, 0, 255, true,
                             0.1f);
    else
      NDebugOverlay::Cross3D(m_approachPoint[i].m_pos, 10.0f, 100, 100, 100,
                             true, 0.1f);
  }
}
...
bool CCSBot::FindApproachPointNearestPath(Vector *pos) {
  ...
  for (int i = 0; i < m_approachPointCount; ++i) {
    if (FindClosestPointOnPath(m_approachPoint[i].m_pos, start, end, &close) ==
        false)
      continue;
...
bool CCSBot::IsAtEnemySpawn(void) const {
  CBaseEntity *spot;
  const char *spawnName = (GetTeamNumber() == TEAM_TERRORIST)
