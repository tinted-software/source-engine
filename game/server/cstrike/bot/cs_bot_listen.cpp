...
bool CCSBot::IsNoiseHeard(void) const {
  ...
  // primitive reaction time simulation - cannot "hear" noise until reaction
  // time has elapsed
...
bool CCSBot::HeardInterestingNoise(void) {
  ...
  if (IsNoiseHeard()) {
    // if we are hiding, only react to noises very nearby, depending on how
    // aggressive we are
    if (IsAtHidingSpot() &&
        GetNoiseRange() > 100.0f + 400.0f * GetProfile()->GetAggression())
      return false;
    ...
    // if we have lots of friends, presumably one of them is closer and will
    // check it out
 ...
 * NOTE: Dont check FOV, since this is used to determine if we should turn our
 * head to look at the noise NOTE: Dont use IsVisible(), because smoke shouldnt
 * cause us to not look toward noises
 */
bool CCSBot::CanSeeNoisePosition(void) const {
  trace_t result;
  CTraceFilterNoNPCsOrPlayer traceFilter(this, COLLISION_GROUP_NONE);
  UTIL_TraceLine(EyePositionConst(),
                 m_noisePosition + Vector(0, 0, HalfHumanHeight),
                 MASK_VISIBLE_AND_NPCS, &traceFilter, &result);
...
bool CCSBot::UpdateLookAtNoise(void) {
  ...
  if (CanSeeNoisePosition()) {
    /// @todo adjust noise Z to keep consistent with current height while
    /// fighting
  ...
  // it's always important to look at enemy noises, because they come from ...
  // enemies!
