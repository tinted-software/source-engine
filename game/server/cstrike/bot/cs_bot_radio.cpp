 ...
 * NOTE: "Report in" is not considered a "command" because it doesnt ask the bot
 * to go somewhere, or change its mind
 */
bool CCSBot::IsRadioCommand(RadioType event) const {
  if (event == RADIO_AFFIRMATIVE || event == RADIO_NEGATIVE ||
      event == RADIO_ENEMY_SPOTTED || event == RADIO_SECTOR_CLEAR ||
      event == RADIO_REPORTING_IN || event == RADIO_REPORT_IN_TEAM ||
...
void CCSBot::RespondToRadioCommands(void) {
  ...
  // unless it is a "report in" request - we can do that while we continue to do
  // other things
  ...
  // delay needs to be long enough for the radio message we're responding to to
  // finish
 ...
 */
bool CCSBot::RespondToHelpRequest(CCSPlayer *them, Place place,
                                  float maxRange) {
  if (IsRogue())
    return false;
  ...
  if (maxRange > 0.0f) {
    ...
    PathCost cost(this);
    float travelDistance = NavAreaTravelDistance(
        m_lastKnownArea, TheNavMesh->GetNearestNavArea(themOrigin), cost);
