...
//
// Purpose: Encapsulation of the current scenario/game state. Allows each bot
// imperfect knowledge.
//
// $NoKeywords: $
//=============================================================================//
...
void CSGameState::Reset(void) {
  ...
  // allows T's to plant at random site, and TEAM_CT's to search in a random
  // order NOTE: VS6 std::random_shuffle() doesn't work well with an array of
  // two elements (most maps)
  for (i = 0; i < m_bombsiteCount; ++i) {
 ...
 * Update game state based on events we have received
 */
void CSGameState::OnRoundEnd(IGameEvent *event) { m_isRoundOver = true; }

//--------------------------------------------------------------------------------------------------------------
/**
 * Update game state based on events we have received
 */
void CSGameState::OnRoundStart(IGameEvent *event) { Reset(); }
 ...
 */
bool CSGameState::IsRoundOver(void) const { return m_isRoundOver; }
...
int CSGameState::GetNextBombsiteToSearch(void) {
  ...
  // all the bombsites are clear, someone must have been mistaken - start search
  // over
  for (i = 0; i < m_bombsiteCount; ++i)
    m_isBombsiteClear[i] = false;
  m_bombsiteSearchIndex = 0;
 ...
 * If NULL is returned, we don't think there are any hostages left, or we dont
 * know where they are. NOTE: a T can remember a hostage who has died.  knowPos
 * will be filled in, but NULL will be returned, since CHostages get deleted
 * when they die.
 */
  ...
  for (int i = 0; i < m_hostageCount; ++i) {
    ...
    if (hostageArea) {
      ...
      float travelDistance =
          NavAreaTravelDistance(startArea, hostageArea, cost);
  ...
  for (i = 0; i < m_hostageCount; ++i) {
    ...
    if (m_owner->IsVisible(pos, CHECK_FOV)) {
      if (info->hostage->IsAlive()) {
        ...
        // if hostage is being escorted by a CT, we don't "see" it, we see the
        // CT
        if (info->hostage->IsFollowingSomeone()) {
          info->isValid = false;
        } else {
    ...
    // check line of sight to where we think this hostage is, to see if we
    // noticed that is has moved
    pos = info->knownPos + Vector(0, 0, HalfHumanHeight);
    if (m_owner->IsVisible(pos, CHECK_FOV)) {
      // we can see where we thought the hostage was - verify it is still there
      // and alive
      ...
      if ((info->hostage->GetAbsOrigin() - info->knownPos)
...
bool CSGameState::AreAllHostagesBeingRescued(void) const {
  // if the hostages have all been rescued, they are not being rescued any
  // longer
