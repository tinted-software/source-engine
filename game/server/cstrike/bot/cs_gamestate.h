...
public:
  ...
  bool IsRoundOver(
      void) const; ///< true if round has been won or lost (but not yet reset)
  ...
  float TimeSinceLastSawLooseBomb(
      void) const; ///< how long has is been since we saw the loose bomb
  bool
  IsLooseBombLocationKnown(void) const; ///< do we know where the loose bomb is

  void UpdateBomber(const Vector &pos); ///< we see the bomber
  float TimeSinceLastSawBomber(
      void) const; ///< how long has is been since we saw the bomber

  void UpdatePlantedBomb(const Vector &pos); ///< we see the planted bomb
  bool IsPlantedBombLocationKnown(
      void) const; ///< do we know where the bomb was planted
  void MarkBombsiteAsPlanted(
      int zoneIndex); ///< mark bombsite as the location of the planted bomb

  enum { UNKNOWN = -1 };
  int GetPlantedBombsite(void)
      const; ///< return the zone index of the planted bombsite, or UNKNOWN
  bool
  IsAtPlantedBombsite(void) const; ///< return true if we are currently in the
                                   ///< bombsite where the bomb is planted

  int GetNextBombsiteToSearch(
      void); ///< return the zone index of the next bombsite to search
  bool IsBombsiteClear(
      int zoneIndex) const; ///< return true if given bombsite has been cleared
  void ClearBombsite(int zoneIndex); ///< mark bombsite as clear

  const Vector *GetBombPosition(void)
      const; ///< return where we think the bomb is, or NULL if we don't know

  // hostage rescue scenario
  // ------------------------------------------------------------------------
  CHostage *GetNearestFreeHostage(
      Vector *knowPos = NULL) const; ///< return the closest free hostage, and
                                     ///< where we think it is (knowPos)
  const Vector *GetRandomFreeHostagePosition(void) const;
  bool AreAllHostagesBeingRescued(
      void) const; ///< return true if there are no free hostages
  bool AreAllHostagesGone(
      void) const;            ///< all hostages have been rescued or are dead
  void AllHostagesGone(void); ///< someone told us all the hostages are gone
  bool HaveSomeHostagesBeenTaken(void)
      const ///< return true if one or more hostages have been moved by the CT's
  {
  ...
  unsigned char
  ValidateHostagePositions(void); ///< update our knowledge with what we
                                  ///< currently see - returns bitflag events
...
private:
  ...
  bool
      m_isBombsiteClear[CCSBotManager::MAX_ZONES]; ///< corresponds to zone
                                                   ///< indices in CCSBotManager
  int m_bombsiteSearchOrder[CCSBotManager::MAX_ZONES]; ///< randomized order of
                                                       ///< bombsites to search
  int m_bombsiteCount;
  int m_bombsiteSearchIndex; ///< the next step in the search

  int m_plantedBombsite; ///< zone index of the bombsite where the planted bomb
                         ///< is

  bool m_isPlantedBombPosKnown; ///< if true, we know the exact location of the
                                ///< bomb
  ...
  CBaseEntity *
  GetNearestHostage(void) const;    ///< return the closest live hostage
  void InitializeHostageInfo(void); ///< initialize our knowledge of the number
                                    ///< and location of hostages

  bool m_allHostagesRescued;
  bool m_haveSomeHostagesBeenTaken; ///< true if a hostage has been moved by a
                                    ///< CT (and we've seen it)
};
