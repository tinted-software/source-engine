 ...
 * This is extended from the base jump to disallow jumping when in a crouch
 * area.
 */
bool CCSBot::Jump(bool mustJump) {
  // prevent jumping if we're crouched, unless we're in a crouchjump area - jump
  // wins
  bool inCrouchJumpArea =
      (m_lastKnownArea &&
 ...
 * NOTE: We dont want to directly call Attack() here, or the bots will have
 * super-human reaction times when injured
 */
int CCSBot::OnTakeDamage(const CTakeDamageInfo &info) {
  ...
  if (attacker->IsPlayer() && IsEnemy(attacker)) {
    // Track previous attacker so we don't try to panic multiple times for a
    // shotgun blast
    ...
    if (!IsSurprised() && (m_attacker != lastAttacker ||
                           m_attackedTimestamp != lastAttackedTimestamp)) {
...
void CCSBot::Event_Killed(const CTakeDamageInfo &info) {
  ...
  TheNavMesh->IncreaseDangerNearby(GetTeamNumber(), deathDanger,
                                   m_lastKnownArea, GetAbsOrigin(),
                                   deathDangerRadius);
...
inline bool IsIntersectingBox(const Vector &start, const Vector &end,
...
extern void UTIL_DrawBox(Extent *extent, int lifetime, int red, int green,
                         int blue);
...
void CCSBot::Touch(CBaseEntity *other) {
  ...
  if (other->IsPlayer()) {
    ...
    // they are higher priority - make way, unless we're already making way for
  ...
  if (IsBreakableEntity(other)) {
    ...
    SetLookAt("Breakable", other->WorldSpaceCenter(), PRIORITY_HIGH, 0.1f,
              false, 5.0f, true);
  }
}
...
bool CCSBot::IsBusy(void) const {
  if (IsAttacking() || IsBuying() || IsDefusingBomb() ||
      GetTask() == PLANT_BOMB || GetTask() == RESCUE_HOSTAGES || IsSniping()) {
    return true;
  }
 ...
 */
void CCSBot::TryToJoinTeam(int team) { m_desiredTeam = team; }
...
bool CCSBot::StayOnNavMesh(void) {
  if (m_currentArea == NULL) {
    ...
    if (goalArea) {
      ...
      const float stepInDist = 5.0f; // how far to "step into" an area - must be
                                     // less than min area size
...
bool CCSBot::IsDoingScenario(void) const {
  // if we are deferring to humans, and there is a live human on our team, don't
  // do the scenario
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Return true if we noticed the bomb on the ground or on the radar (for T's
 * only)
 */
...
void CCSBot::SetHidingSpotCheckTimestamp(HidingSpot *spot) {
  ...
  if (m_checkedHidingSpotCount < MAX_CHECKED_SPOTS) {
    ...
    m_checkedHidingSpot[m_checkedHidingSpotCount].timestamp =
        gpGlobals->curtime;
...
void CCSBot::UpdateHostageEscortCount(void) {
  ...
  if (m_hostageEscortCount == 0 ||
      gpGlobals->curtime - m_hostageEscortCountTimestamp < updateInterval)
    return;
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Return the closest "important" enemy for the given scenario (bomb carrier,
 * VIP, hostage escorter)
 */
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Return true if we are acting like a rogue (not listening to teammates, not
 * doing scenario goals)
...
bool CCSBot::IsRogue(void) const {
  ...
  if (m_rogueTimer.IsElapsed()) {
    ...
    // our chance of going rogue is inversely proportional to our teamwork
    // attribute
...
bool CCSBot::IsHurrying(void) const {
  ...
  // if the bomb has been planted, we are in a hurry, CT or T (they could be
  // defusing it!)
  if (ctrl->GetScenario() == CCSBotManager::SCENARIO_DEFUSE_BOMB &&
      ctrl->IsBombPlanted())
    return true;
 ...
 */
bool CCSBot::IsEndOfSafeTime(void) const { return m_wasSafe && !IsSafe(); }
...
void CCSBot::AdjustSafeTime(void) {
  ...
  // if we spotted an enemy sooner than we thought possible, adjust our notion
  // of "safe" time
  if (ctrl->GetElapsedRoundTime() < m_safeTime) {
...
public:
  ...
  bool operator()(CNavArea *area) {
    ...
    FOR_EACH_VEC((*pSpots), it) {
      ...
      if (m_range > 0.0f)
        if ((spot->GetPosition() - GetCentroid(m_me))
                .IsLengthGreaterThan(m_range))
          continue;
      ...
      if (UTIL_IsVisibleToTeam(spot->GetPosition() +
...
const char *CCSBot::GetDispositionName(void) const {
  static const char *name[NUM_DISPOSITIONS] = {
      "ENGAGE_AND_INVESTIGATE", "OPPORTUNITY_FIRE", "SELF_DEFENSE",
      "IGNORE_ENEMIES"};
 ...
 */
void CCSBot::BuildUserCmd(CUserCmd &cmd, const QAngle &viewangles,
