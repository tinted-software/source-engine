 ...
 */
#define DECLARE_BOTMANAGER_EVENT_LISTENER(BotManagerSingleton, EventClass,     \
                                          EventName)                           \
public:                                                                        \
...
public:
  ...
  virtual CBasePlayer *AllocateBotEntity(
      void); ///< factory method to allocate the appropriate entity for the bot
  ...
  virtual unsigned int GetPlayerPriority(
      CBasePlayer *player) const; ///< return priority of player (0 = max pri)
  virtual bool IsImportantPlayer(
      CCSPlayer *player) const; ///< return true if player is important to
                                ///< scenario (VIP, bomb carrier, etc)

  void ExtractScenarioData(void); ///< search the map entities to determine the
                                  ///< game scenario and define important zones
  ...
  const Zone *GetZone(const Vector &pos)
      const; ///< return the zone that contains the given position
  const Zone *GetClosestZone(const Vector &pos)
      const; ///< return the closest zone to the given position
  const Zone *GetClosestZone(const CBaseEntity *entity)
      const; ///< return the closest zone to the given entity
  int GetZoneCount(void) const { return m_zoneCount; }
  void CheckForBlockedZones(void);

  const Vector *GetRandomPositionInZone(const Zone *zone)
      const; ///< return a random position inside the given zone
  CNavArea *GetRandomAreaInZone(
      const Zone *zone) const; ///< return a random area inside the given zone

  /**
   * Return the zone closest to the given position, using the given cost
   * heuristic
   */
  template <typename CostFunctor>
  const Zone *GetClosestZone(CNavArea *startArea, CostFunctor costFunc,
                             float *travelDistance = NULL) const {
    ...
    for (int i = 0; i < m_zoneCount; ++i) {
      ...
      float dist =
          NavAreaTravelDistance(startArea, m_zone[i].m_area[0], costFunc);
  ...
  /// returns a random spawn point for the given team (no arg means use both
  /// team spawnpoints)
  ...
  bool IsTimeToPlantBomb(
      void) const; ///< return true if it's ok to try to plant bomb
  ...
  float GetBombTimeLeft(
      void) const; ///< get the time remaining before the planted bomb explodes
  ...
  float GetRadioMessageTimestamp(RadioType event, int teamID)
      const; ///< return the last time the given radio message was sent for
             ///< given team
  float GetRadioMessageInterval(RadioType event, int teamID)
      const; ///< return the interval since the last time this message was sent
  ...
  bool IsWeaponUseable(const CWeaponCSBase *weapon)
      const; ///< return true if the bot can use this weapon

  bool IsDefenseRushing(void) const {
    return m_isDefenseRushing;
  } ///< returns true if defense team has "decided" to rush this round
  bool IsOnDefense(const CCSPlayer *player)
      const; ///< return true if this player is on "defense"
  bool IsOnOffense(const CCSPlayer *player)
      const; ///< return true if this player is on "offense"
...
#define FROM_CONSOLE true
  bool BotAddCommand(
      int team, bool isFromConsole = false, const char *profileName = NULL,
      CSWeaponType weaponType = WEAPONTYPE_UNKNOWN,
      BotDifficultyType difficulty =
          NUM_DIFFICULTY_LEVELS); ///< process the "bot_add" console command
...
private:
  ...
  float m_earliestBombPlantTimestamp; ///< don't allow planting until after this
                                      ///< time has elapsed
  ...
  CountdownTimer
      m_checkTransientAreasTimer; ///< when elapsed, all transient nav areas
  ...
  bool m_isDefenseRushing; ///< whether defensive team is rushing this round or
                           ///< not
  ...
  DECLARE_CSBOTMANAGER_EVENT_LISTENER(SmokeGrenadeDetonate,
  ...
  CUtlVector<BotEventInterface *>
      m_commonEventListeners; // These event listeners fire often, and can be
                              // disabled for performance gains when no bots are
                              // present.
...
inline const CCSBotManager::Zone *
CCSBotManager::GetClosestZone(const CBaseEntity *entity) const {
  if (entity == NULL)
    return NULL;
