...
// Some tricky business here - we don't want to include the precompiled header
// for the statreader and trying to #ifdef it out does funky things like
// ignoring the #endif. Define our header file separately and include it based
// on the switch
...
struct ServerStats_t {
  ...
  bool IsMet(int roundStat, int matchStat) {
    ...
    {CSBreakWindows, CSSTAT_NUM_BROKEN_WINDOWS,
     AchievementConsts::BreakWindowsInOfficeRound_Windows, 0, "cs_office"},
    {CSBreakProps, CSSTAT_PROPSBROKEN_ALL,
     AchievementConsts::BreakPropsInRound_Props, 0, NULL},
    {CSUnstoppableForce, CSSTAT_KILLS,
     AchievementConsts::UnstoppableForce_Kills, 0, NULL},
    {CSHeadshotsInRound, CSSTAT_KILLS_HEADSHOT,
     AchievementConsts::HeadshotsInRound_Kills, 0, NULL},
...
//=============================================================================
static void SvNoNemesisChangeCallback(IConVar *pConVar, const char *pOldValue,
                                      float flOldValue) {
...
ConVar sv_nonemesis("sv_nonemesis", "0", 0, "Disable nemesis and revenge.",
                    SvNoNemesisChangeCallback);
//=============================================================================
// HPE_END
//=============================================================================
...
ConVar sv_debugroundstats(
    "sv_debugroundstats", "0", 0,
    "A temporary variable that will print extra information about stats upload "
    "which may be useful in debugging any problems.");
...
CCSGameStats::StatContainerList_t *CCSGameStats::s_StatLists =
    new CCSGameStats::StatContainerList_t();
...
CCSGameStats::CCSGameStats() {
  ...
  for (int i = 0; i < ARRAYSIZE(PropModelStatsTableInit); ++i) {
    m_PropStatTable.Insert(PropModelStatsTableInit[i].szPropModelName,
                           PropModelStatsTableInit[i].statType);
  }
...
// Input  :  -
//-----------------------------------------------------------------------------
void CCSGameStats::Clear(void) { m_PlayerSnipedPosition.Purge(); }
...
void CCSGameStats::LevelShutdownPreClearSteamAPIContext(void) {
  // If we have any unsent round stats we better send them now or we'll lose
  // them
  UploadRoundStats();
...
void CCSGameStats::UploadRoundStats(void) {
  CFastTimer totalTimer, purchasesAndDeathsStatTimer, weaponsStatBuildTimer,
      weaponsStatSubmitTimer, submitTimer, cleanupTimer;
  ...
  for (int iWeapon = 0; iWeapon < WEAPON_MAX; ++iWeapon) {
    ...
    if (!pWeaponName || !pWeaponName[0] ||
        (m_weaponStats[iWeapon][0].shots == 0 &&
         m_weaponStats[iWeapon][1].shots == 0))
      continue;

    m_WeaponData.AddToTail(
        new SCSSWeaponData(pWeaponName, m_weaponStats[iWeapon][0]));
    ...
    if (GetSteamWorksSGameStatsUploader().IsCollectingDetails()) {
      ...
      V_snprintf(pWeaponNameModified, ARRAYSIZE(pWeaponNameModified), "%s_bot",
                 pWeaponName);
      m_WeaponData.AddToTail(
          new SCSSWeaponData(pWeaponNameModified, m_weaponStats[iWeapon][1]));
    }
  }
  ...
  if (sv_debugroundstats.GetBool()) {
    ...
    Msg("UploadRoundStats completed. %.3f msec. Breakdown:\n a: %.3f msec\n b: "
        ...
        cleanupTimer.GetDuration().GetMillisecondsF(), iPurchases, iDeathData,
        iWeaponData, listCount, g_rowCommitTime, g_rowWriteTime);
  }
}
...
void CCSGameStats::SubmitGameStats(KeyValues *pKV) {
  ...
  for (int i = 0; i < listCount; ++i) {
    // Create a master key value that has stats everybody should share (map
    // name, session ID, etc)
...
// Purpose:
//-----------------------------------------------------------------------------
void CCSGameStats::Event_ShotFired(CBasePlayer *pPlayer,
  ...
  if (pCSPlayer) {
    ...
    if (pCSWeapon) {
      ...
      for (int i = 0; WeaponName_StatId_Table[i].shotStatId != CSSTAT_UNDEFINED;
           ++i) {
...
void CCSGameStats::Event_ShotHit(CBasePlayer *pPlayer,
  ...
  if (pInflictor) {
    if (pInflictor == pPlayer) {
      if (pPlayer->GetActiveWeapon()) {
        CWeaponCSBase *pCSWeapon =
            dynamic_cast<CWeaponCSBase *>(pPlayer->GetActiveWeapon());
        if (pCSWeapon) {
          CSWeaponID weaponId = pCSWeapon->GetWeaponID();
          for (int i = 0;
               WeaponName_StatId_Table[i].shotStatId != CSSTAT_UNDEFINED; ++i) {
            if (WeaponName_StatId_Table[i].weaponId == weaponId) {
...
void CCSGameStats::Event_PlayerKilled(CBasePlayer *pPlayer,
                                      const CTakeDamageInfo &info) {
  Assert(pPlayer);
  CCSPlayer *pCSPlayer = ToCSPlayer(pPlayer);
...
void CCSGameStats::Event_PlayerKilled_PreWeaponDrop(
    CBasePlayer *pPlayer, const CTakeDamageInfo &info) {
  Assert(pPlayer);
  CCSPlayer *pCSPlayer = ToCSPlayer(pPlayer);
...
void CCSGameStats::Event_BreakProp(CCSPlayer *pPlayer, CBreakableProp *pProp) {
  ...
  DevMsg("Player %s broke a %s (%i)\n", pPlayer->GetPlayerName(),
         pProp->GetModelName().ToCStr(), pProp->entindex());
...
void CCSGameStats::UpdatePlayerRoundStats(int winner) {
  ...
  CSStatType_t mapStatWinIndex = CSSTAT_UNDEFINED,
  ...
  for (int iPlayerIndex = 1; iPlayerIndex <= MAX_PLAYERS; iPlayerIndex++) {
    ...
    if (pPlayer && pPlayer->IsConnected()) {
      ...
      if (winner == TEAM_CT || winner == TEAM_TERRORIST) {
        ...
        IncrementStat(pPlayer, CSSTAT_PLAYTIME,
                      (int)CSGameRules()->GetRoundElapsedTime(), true);
      }
    }
  }
...
void CCSGameStats::SendRollingStatsAveragesToAllPlayers() {
  // The most stats we can send at once is the max message size minus the
  // header, divided by the total size of each stat.
  const int maxStatsPerMessage =
      (MAX_USER_MSG_DATA - sizeof(CSStatType_t)) / (3 * sizeof(float));

  const int numMessagesNeeded =
      ((CSSTAT_MAX - CSSTAT_FIRST) / maxStatsPerMessage) + 1;
  ...
  for (int batchIndex = 0; batchIndex < numMessagesNeeded; ++batchIndex) {
    ...
    for (int iStat = firstStatInThisBatch;
         iStat < CSSTAT_MAX && iStat <= lastMessageInThisBatch; ++iStat) {
...
void CCSGameStats::ComputeRollingStatAverages() {
  ...
  // for ( int iStat = CSSTAT_FIRST; iStat < CSSTAT_MAX; ++iStat )
  {
    for (int iPlayerIndex = 1; iPlayerIndex <= MAX_PLAYERS; iPlayerIndex++) {
      CCSPlayer *pPlayer = ToCSPlayer(UTIL_PlayerByIndex(iPlayerIndex));
      if (pPlayer && pPlayer->IsConnected()) {
        StatsCollection_t &roundStats =
            m_aPlayerStats[pPlayer->entindex()].statsCurrentRound;
    ...
    m_rollingPlayerStatAverages.RollDataSetIntoAverage(
        currentRoundPlayerStatsAverage);
  }
}
...
void CCSGameStats::ComputeDirectStatAverages() {
  ...
  for (int iPlayerIndex = 1; iPlayerIndex <= MAX_PLAYERS; iPlayerIndex++) {
    CCSPlayer *pPlayer = ToCSPlayer(UTIL_PlayerByIndex(iPlayerIndex));
    if (pPlayer && pPlayer->IsConnected()) {
      StatsCollection_t &matchStats =
          m_aPlayerStats[pPlayer->entindex()].statsCurrentMatch;
  ...
  if (m_numberOfCounterTerroristEntriesForDirectAverages > 0) {
    m_directCTStatAverages /=
        m_numberOfCounterTerroristEntriesForDirectAverages;
    m_directCTStatAverages *= m_numberOfRoundsForDirectAverages;
  }

  int numPlayers = m_numberOfCounterTerroristEntriesForDirectAverages +
                   m_numberOfTerroristEntriesForDirectAverages;
...
void CCSGameStats::DumpMatchWeaponMetrics() {
  ...
  // "wm_%4d%02d%02d_%02d%02d%02d_%s.csv", 	year, month, day, hour, minute,
  //second, gpGlobals->mapname.ToCStr());
  ...
  // filesystem->FPrintf(hLogFile, "%s\n", "WeaponId, Mode, Cost, Bullets,
  // CycleTime, TotalShots, TotalHits, TotalDamage, TotalKills");
  ...
  //	{
  //		filesystem->FPrintf(hLogFile, "%s, %d, %d, %d, %f, %d, %d, %d,
  //%d\n", 			pWeaponName, 			iMode, 			pInfo->GetWeaponPrice(), 			pInfo->m_iBullets,
  //			pInfo->m_flCycleTime,
  ...
  // weapon_accuracy_model.GetInt()); filesystem->FPrintf(hLogFile,
...
// Purpose:
//-----------------------------------------------------------------------------
void CCSGameStats::Event_PlayerKilledOther(CBasePlayer *pAttacker,
                                           CBaseEntity *pVictim,
                                           const CTakeDamageInfo &info) {
  // This also gets called when the victim is a building.  That gets tracked
  // separately as building destruction, don't count it here
  if (!pVictim->IsPlayer())
    return;
    ...
    if (pInflictor == pAttacker) {
      if (pAttacker->GetActiveWeapon()) {
        ...
        if (weapon) {
          ...
          if (info && info->m_iTeam != TEAM_UNASSIGNED &&
              pAttacker->GetTeamNumber() != info->m_iTeam) {
      ...
      if (V_strcmp(pInflictor->m_iClassname.ToCStr(), "hegrenade_projectile") ==
          0)
  ...
  if (pPlayerVictim && pPlayerAttacker &&
      pPlayerAttacker->IsBlindForAchievement()) {
    IncrementStat(pPlayerAttacker, CSSTAT_KILLS_WHILE_BLINDED, 1);
  }

  // [sbodenbender] check for deaths near planted bomb for funfact
  if (pPlayerVictim && pPlayerAttacker &&
      pPlayerAttacker->GetTeamNumber() == TEAM_TERRORIST &&
      CSGameRules()->m_bBombPlanted) {
    float bombCheckDistSq = AchievementConsts::KillEnemyNearBomb_MaxDistance *
                            AchievementConsts::KillEnemyNearBomb_MaxDistance;
    for (int i = 0; i < g_PlantedC4s.Count(); i++) {
      ...
      if (pC4->IsBombActive()) {
        ...
        if (victimToBomb.LengthSqr() < bombCheckDistSq ||
            attackerToBomb.LengthSqr() < bombCheckDistSq) {
          IncrementStat(pPlayerAttacker, CSSTAT_KILLS_WHILE_DEFENDING_BOMB, 1);
          break; // you only get credit for one kill even if you happen to be by
                 // more than one bomb
        }
      }
    }
  ...
  // we don't have a simple way (yet) to check if the victim actually just
  // achieved The Unstoppable Force, so we award this achievement simply if
  // they've met the requirements and would have received it.
  PlayerStats_t &victimStats = m_aPlayerStats[pVictim->entindex()];
  if (victimStats.statsCurrentRound[CSSTAT_KILLS] >=
      AchievementConsts::UnstoppableForce_Kills) {
  ...
  if (playerCounts[iAttackerTeamNumber].totalAlivePlayers == 1 &&
      playerCounts[iAttackerTeamNumber].killedPlayers >= 2) {
    IncrementStat(pPlayerAttacker, CSSTAT_KILLS_WHILE_LAST_PLAYER_ALIVE, 1);
  }

  // if they were damaged by more than one person that must mean that someone
  // else did damage before the killer finished them off.
  if (pPlayerVictim->GetNumEnemyDamagers() > 1) {
    IncrementStat(pPlayerAttacker, CSSTAT_KILLS_ENEMY_WOUNDED, 1);
  }

  // Let's check for the "Happy Camper" achievement where we snipe two players
  // while standing in the same spot.
  if (pPlayerAttacker && !pPlayerAttacker->IsBot() && weaponId != WEAPON_NONE) {
    // Were we using a sniper rifle?
    bool bUsingSniper = (weaponId == WEAPON_AWP || weaponId == WEAPON_SCOUT ||
                         weaponId == WEAPON_SG550 || weaponId == WEAPON_G3SG1);

    // If we're zoomed in
    if (bUsingSniper &&
        pPlayerAttacker->GetFOV() != pPlayerAttacker->GetDefaultFOV()) {
      // Get our position
      ...
      FOR_EACH_LL(m_PlayerSnipedPosition, iElement) {
        sHappyCamperSnipePosition *pSnipePosition =
            &m_PlayerSnipedPosition[iElement];

        // We've found this player's entry. Next, check to see if they meet the
      ...
      if (!bFoundPlayerEntry) {
        m_PlayerSnipedPosition.AddToTail(
            sHappyCamperSnipePosition(userid, position));
      }
    }
  }
}
...
//-----------------------------------------------------------------------------
void CCSGameStats::PlayerKilled(CBasePlayer *pVictim,
                                const CTakeDamageInfo &info) {
  if (!pVictim)
    return;
...
//-----------------------------------------------------------------------------
void CCSGameStats::Event_PlayerDamage(CBasePlayer *pBasePlayer,
                                      const CTakeDamageInfo &info) {
  CCSPlayer *pAttacker = ToCSPlayer(info.GetAttacker());
  if (pAttacker && pAttacker->GetTeam() != pBasePlayer->GetTeam()) {
    ...
    CSWeaponID weaponId =
        AliasToWeaponID(GetTranslatedWeaponAlias(pWeaponName));
    for (int i = 0; WeaponName_StatId_Table[i].shotStatId != CSSTAT_UNDEFINED;
         ++i) {
...
void CCSGameStats::Event_MoneySpent(CCSPlayer *pPlayer, int moneySpent,
  ...
  if (pPlayer && moneySpent > 0) {
    ...
    if (pItemName && !pPlayer->IsBot()) {
      ...
      m_MarketPurchases.AddToTail(new SMarketPurchases(
          steamIDForBuyer.ConvertToUint64(), moneySpent, pItemName));
    }
  }
}
...
void CCSGameStats::FireGameEvent(IGameEvent *event) {
  ...
  if (V_strcmp(pEventName, "round_start") == 0) {
    ...
    CBreakableProp *pProp =
        static_cast<CBreakableProp *>(CBaseEntity::Instance(entindex));
    ...
    int userid = event->GetInt("userid", 0);
    IncrementStat(ToCSPlayer(UTIL_PlayerByUserId(userid)),
                  CSSTAT_SHOTS_HEGRENADE, 1);
  }
}
...
//-----------------------------------------------------------------------------
// Purpose: Resets all stats (including round, match, accumulated and rolling
// averages
//-----------------------------------------------------------------------------
...
void CCSGameStats::IncrementTeamStat(int iTeamIndex, int iStatIndex,
                                     int iAmount) {
...
void CCSGameStats::ResetKillHistory(CBasePlayer *pPlayer) {
  ...
  // for every other player, set all all the kills with respect to this player
  // to 0
  for (int i = 0; i < ARRAYSIZE(m_aPlayerStats); i++) {
...
//-----------------------------------------------------------------------------
void CCSGameStats::IncrementStat(CCSPlayer *pPlayer, CSStatType_t statId,
                                 int iDelta, bool bPlayerOnly /* = false */) {
  if (pPlayer) {
    PlayerStats_t &stats = m_aPlayerStats[pPlayer->entindex()];
    ...
    for (int i = 0; i < ARRAYSIZE(ServerStatBasedAchievements); ++i) {
      if (ServerStatBasedAchievements[i].statId == statId) {
        ...
        if (ServerStatBasedAchievements[i].mapFilter != NULL &&
            V_strcmp(gpGlobals->mapname.ToCStr(),
                     ServerStatBasedAchievements[i].mapFilter) != 0)
          continue;

        bool bWasMet = ServerStatBasedAchievements[i].IsMet(
            stats.statsCurrentRound[statId] - iDelta,
            stats.statsCurrentMatch[statId] - iDelta);
        bool bIsMet = ServerStatBasedAchievements[i].IsMet(
            stats.statsCurrentRound[statId], stats.statsCurrentMatch[statId]);
        if (!bWasMet && bIsMet) {
          pPlayer->AwardAchievement(
              ServerStatBasedAchievements[i].achievementId);
        }
      }
    }
  }
...
//-----------------------------------------------------------------------------
// Purpose:  Sets the specified stat for specified player to the specified
// amount
//-----------------------------------------------------------------------------
void CCSGameStats::SetStat(CCSPlayer *pPlayer, CSStatType_t statId,
                           int iValue) {
  if (pPlayer) {
    ...
    for (int i = 0; i < ARRAYSIZE(ServerStatBasedAchievements); ++i) {
      if (ServerStatBasedAchievements[i].statId == statId) {
        ...
        bool bWasMet =
            ServerStatBasedAchievements[i].IsMet(oldRoundValue, oldMatchValue);
        bool bIsMet = ServerStatBasedAchievements[i].IsMet(
            stats.statsCurrentRound[statId], stats.statsCurrentMatch[statId]);
        if (!bWasMet && bIsMet) {
          pPlayer->AwardAchievement(
...
void CCSGameStats::SendStatsToPlayer(CCSPlayer *pPlayer, int iMinStatPriority) {
  ASSERT(
      CSSTAT_MAX <
      255); // if we add more than 255 stats, we'll need to update this protocol
  if (pPlayer && pPlayer->IsConnected()) {
    StatsCollection_t &deltaStats =
        m_aPlayerStats[pPlayer->entindex()].statsDelta;
    ...
    for (int iStat = CSSTAT_FIRST; iStat < CSSTAT_MAX; ++iStat) {
      ...
      if (CSStatProperty_Table[iStat].statId != iStat) {
        Warning("CSStatProperty_Table[iStat].statId != iStat, (%d)",
                CSStatProperty_Table[iStat].statId);
      }
    ...
    // begin the CRC with a trivially hidden key value to discourage packet
    // modification
    const uint32 key =
        0x82DA9F4C; // this key should match the key in cs_client_gamestats.cpp
    CRC32_ProcessBuffer(&crc, &key, sizeof(key));

    // if we make any change to the ordering of the stats or this message
    // format, update this value
    ...
    for (byte iStat = CSSTAT_FIRST; iStat < CSSTAT_MAX; ++iStat) {
      ...
      if (deltaStats[iStat] != 0 && iPriority >= iMinStatPriority) {
        ...
        WRITE_BYTE(iStat);
        Assert(deltaStats[iStat] <= 0x7FFF &&
               deltaStats[iStat] > 0); // make sure we aren't truncating bits
...
//-----------------------------------------------------------------------------
// Purpose: Sends intermittent stats updates for stats that need to be updated
// during a round and/or life
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
void CCSGameStats::CalcDominationAndRevenge(CCSPlayer *pAttacker,
                                            CCSPlayer *pVictim,
                                            int *piDeathFlags) {
  //=============================================================================
  // HPE_BEGIN:
  ...
  // That is done so that the domination and revenge will be calculated prior to
  // the death message being sent to the clients
  int attackerEntityIndex = pAttacker->entindex();
  int iKillsUnanswered =
      statsVictim.statsKills.iNumKilledByUnanswered[attackerEntityIndex] + 1;

  if (CS_KILLS_FOR_DOMINATION == iKillsUnanswered) {
    // this is the Nth unanswered kill between killer and victim, killer is now
    // dominating victim
  ...
  if (iKillsUnanswered == CS_KILLS_FOR_DOMINATION) {
    // this is the Nth unanswered kill between killer and victim, killer is now
    // dominating victim set victim to be dominated by killer
    ...
    if (numConcurrentDominations >=
        AchievementConsts::ConcurrentDominations_MinDominations) {
...
void CCSGameStats::Event_PlayerDominatedOther(CCSPlayer *pAttacker,
                                              CCSPlayer *pVictim) {
...
void CCSGameStats::Event_PlayerAvengedTeammate(CCSPlayer *pAttacker,
                                               CCSPlayer *pAvengedPlayer) {
  if (pAttacker && pAvengedPlayer) {
    IGameEvent *event =
        gameeventmanager->CreateEvent("player_avenged_teammate");
