...
class LookAtTarget {
public:
  LookAtTarget(const Vector &target) { m_target = target; }
 ...
 * Constructor
 */
CSNavNode::CSNavNode(const Vector &pos, const Vector &normal,
                     CSNavNode *parent) {
  m_pos = pos;
  m_normal = normal;
...
void CSNavNode::Draw(void) {
  ...
  if ((!m_isCovered && nav_show_node_id.GetBool()) ||
      (m_isCovered && nav_show_node_id.GetInt() < 0)) {
    char text[16];
  ...
  if (GetAttributes() & NAV_MESH_CROUCH) {
    ...
    for (i = 0; i < NUM_CORNERS; i++) {
      if (m_crouch[i]) {
        ...
        NDebugOverlay::HorzArrow(m_pos, m_pos + scaled, 0.5, 0, 0, 255, 255,
                                 true, 0.1f);
      }
    }
  }
...
void CSNavNode::CheckCrouch(void) {
  ...
  // Trace downward from duck height to find the max floor height for the node's
  // surroundings
  ...
  UTIL_TraceHull(start, m_pos, mins, maxs, MASK_PLAYERSOLID_BRUSHONLY, &filter,
                 &tr);
  ...
  if (tr.startsolid && !tr.allsolid) {
    ...
    UTIL_TraceHull(start, m_pos, mins, maxs, MASK_PLAYERSOLID_BRUSHONLY,
                   &filter, &tr);
  ...
  if (tr.startsolid) {
    // we don't even have duck height clear.  try a simple check to find floor
    // height.
    float x, y;

    // Find the highest floor z - for a player to stand in this area, we need a
    // full VEC_HULL_MAX.z of clearance above this height at all points.
    float maxFloorZ = m_pos.z;
    for (y = -HalfHumanWidth; y <= HalfHumanWidth + 0.1f; y += HalfHumanWidth) {
      for (x = -HalfHumanWidth; x <= HalfHumanWidth + 0.1f;
           x += HalfHumanWidth) {
        float floorZ;
  ...
  // For each direction, trace upwards from our best ground height to
  // VEC_HULL_MAX.z to see if we have standing room.
  for (int i = 0; i < NUM_CORNERS; ++i) {
#if DEBUG_NAV_NODES
    if (nav_test_node_crouch_dir.GetInt() != NUM_CORNERS &&
        i != nav_test_node_crouch_dir.GetInt())
      continue;
#endif // DEBUG_NAV_NODES
    ...
    Vector actualGroundPos =
        groundPos; // we might need to adjust this if the tracehull failed above
                   // and we fell back to m_pos.z

    // Build a mins/maxs pair for the HumanWidth x HalfHumanWidth box facing the
    ...
    UTIL_TraceHull(actualGroundPos + Vector(0, 0, 0.1f),
                   actualGroundPos + Vector(0, 0, VEC_HULL_MAX.z - 0.2f), mins,
                   maxs, MASK_PLAYERSOLID_BRUSHONLY, &filter, &tr);
    actualGroundPos.z += tr.fractionleftsolid * VEC_HULL_MAX.z;
    float maxHeight = actualGroundPos.z + VEC_DUCK_HULL_MAX.z;
    for (; tr.startsolid && actualGroundPos.z <= maxHeight;
         actualGroundPos.z += 1.0f) {
      // In case we didn't find a good ground pos above, we could start in the
      // ground.  Move us up some.
      UTIL_TraceHull(actualGroundPos + Vector(0, 0, 0.1f),
...
#if DEBUG_NAV_NODES
    if (nav_show_nodes.GetBool()) {
      if (nav_test_node_crouch_dir.GetInt() == i ||
          nav_test_node_crouch_dir.GetInt() == NUM_CORNERS) {
        if (tr.startsolid) {
          NDebugOverlay::Box(actualGroundPos, mins,
                             maxs + Vector(0, 0, VEC_HULL_MAX.z), 255, 0, 0, 10,
                             20.0f);
        } else if (m_crouch[corner]) {
          NDebugOverlay::Box(actualGroundPos, mins,
                             maxs + Vector(0, 0, VEC_HULL_MAX.z), 0, 0, 255, 10,
                             20.0f);
        } else {
          NDebugOverlay::Box(actualGroundPos, mins,
                             maxs + Vector(0, 0, VEC_HULL_MAX.z), 0, 255, 0, 10,
                             10.0f);
        }
      }
    }
 ...
 */
void CSNavNode::ConnectTo(CSNavNode *node, NavDirType dir) { m_to[dir] = node; }
...
BOOL CSNavNode::IsClosedCell(void) const {
  if (IsBiLinked(SOUTH) && IsBiLinked(EAST) && m_to[EAST]->IsBiLinked(SOUTH) &&
