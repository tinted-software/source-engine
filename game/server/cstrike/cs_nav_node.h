...
public:
  ...
  static CSNavNode *
  GetNode(const Vector &pos); ///< return navigation node at the position, or
                              ///< NULL if none exists
  ...
  void ConnectTo(CSNavNode *node,
                 NavDirType dir); ///< create a connection FROM this node TO the
                                  ///< given node, in the given direction
  CSNavNode *GetParent(void) const;

  void MarkAsVisited(
      NavDirType dir); ///< mark the given direction as having been visited
  BOOL HasVisited(NavDirType dir); ///< return TRUE if the given direction has
                                   ///< already been searched
  BOOL
  IsBiLinked(NavDirType dir) const; ///< node is bidirectionally linked to
...
private:
  ...
  CSNavNode *m_to[NUM_DIRECTIONS]; ///< links to north, south, east, and west.
                                   ///< NULL if no link
  ...
  unsigned char
      m_visited; ///< flags for automatic node generation. If direction bit is
                 ///< clear, that direction hasn't been explored yet.
  CSNavNode *m_parent; ///< the node prior to this in the search, which we pop
...
inline const Vector *CSNavNode::GetPosition(void) const { return &m_pos; }

inline CSNavNode *CSNavNode::GetParent(void) const { return m_parent; }
...
inline void CSNavNode::AssignArea(CNavArea *area) { m_area = area; }

inline CNavArea *CSNavNode::GetArea(void) const { return m_area; }
