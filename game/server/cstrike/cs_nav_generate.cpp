...
// ConVar nav_slope_limit( "nav_slope_limit", "0.7", FCVAR_GAMEDLL, "The ground
// unit normal's Z component must be greater than this for nav areas to be
// generated." );
ConVar nav_restart_after_analysis(
    "nav_restart_after_analysis", "1", FCVAR_GAMEDLL,
    "When nav nav_restart_after_analysis finishes, restart the server.  "
    "Turning this off can cause crashes, but is useful for incremental "
    "generation.");
 ...
 * @todo Optimize by search from eye outward and modifying pathfinder to treat
 * all links as bi-directional
 */
void CCSNavArea::ComputeApproachAreas(void) {
  ...
  for (int searchType = FROM_EYE; searchType != SEARCH_FINISHED; ++searchType) {
    ...
    for (it = 0; it < TheNavAreas.Count(); ++it) {
      ...
      // if we can see 'farArea', try again - the whole point is to go "around
      ...
      while (m_approachCount < MAX_APPROACH_AREAS) {
        ...
        if (searchType == FROM_EYE) {
          ...
          // we must pathfind from us in order to pick up one-way paths OUT OF
          // our area
          from = this;
          to = farArea;
        } else // TO_EYE
        {
          ...
          // we must pathfind to us in order to pick up one-way paths INTO our
          // area
          from = farArea;
          to = this;
        }
        ...
        for (i = 1; i < count; ++i) {
          ...
          // we can't see this area - mark this area as "blocked" and unusable
          // by subsequent approach paths
          if (blockedIDCount == MAX_BLOCKED_AREAS) {
            Msg("Overflow computing approach areas for area #%d.\n", GetID());
            return;
          }

          // if the area to be blocked is actually farArea, block the one just
          // prior (blocking farArea will cause all subsequent pathfinds to
          // fail)
          ...
          if (a == m_approachCount) {
            m_approach[m_approachCount].prev.area =
                (block >= 2) ? path[block - 2] : NULL;

            m_approach[m_approachCount].here.area = path[block - 1];
            m_approach[m_approachCount].prevToHereHow =
                path[block - 1]->GetParentHow();

            m_approach[m_approachCount].next.area = path[block];
            m_approach[m_approachCount].hereToNextHow =
                path[block]->GetParentHow();
