...
SendPropInt(SENDINFO(m_iPlayerC4), 8, SPROP_UNSIGNED),
    ...
    SendPropArray3(SENDINFO_ARRAY3(m_bHostageAlive),
                   SendPropInt(SENDINFO_ARRAY(m_bHostageAlive), 1,
                               SPROP_UNSIGNED)),
    SendPropArray3(SENDINFO_ARRAY3(m_isHostageFollowingSomeone),
                   SendPropInt(SENDINFO_ARRAY(m_isHostageFollowingSomeone), 1,
                               SPROP_UNSIGNED)),
    SendPropArray3(SENDINFO_ARRAY3(m_iHostageEntityIDs),
                   SendPropInt(SENDINFO_ARRAY(m_iHostageEntityIDs), -1,
                               SPROP_UNSIGNED)),
    SendPropArray3(SENDINFO_ARRAY3(m_iHostageY),
                   SendPropInt(SENDINFO_ARRAY(m_iHostageY),
                               COORD_INTEGER_BITS + 1, 0)),
    SendPropArray3(SENDINFO_ARRAY3(m_iHostageX),
                   SendPropInt(SENDINFO_ARRAY(m_iHostageX),
                               COORD_INTEGER_BITS + 1, 0)),
    SendPropArray3(SENDINFO_ARRAY3(m_iHostageZ),
                   SendPropInt(SENDINFO_ARRAY(m_iHostageZ),
                               COORD_INTEGER_BITS + 1, 0)),
    ...
    SendPropArray3(SENDINFO_ARRAY3(m_hostageRescueX),
                   SendPropInt(SENDINFO_ARRAY(m_hostageRescueX),
                               COORD_INTEGER_BITS + 1, 0)),
    SendPropArray3(SENDINFO_ARRAY3(m_hostageRescueY),
                   SendPropInt(SENDINFO_ARRAY(m_hostageRescueY),
                               COORD_INTEGER_BITS + 1, 0)),
    SendPropArray3(SENDINFO_ARRAY3(m_hostageRescueZ),
                   SendPropInt(SENDINFO_ARRAY(m_hostageRescueZ),
                               COORD_INTEGER_BITS + 1, 0)),
    SendPropBool(SENDINFO(m_bBombSpotted)),
    SendPropArray3(SENDINFO_ARRAY3(m_bPlayerSpotted),
                   SendPropInt(SENDINFO_ARRAY(m_bPlayerSpotted), 1,
                               SPROP_UNSIGNED)),
    SendPropArray3(SENDINFO_ARRAY3(m_iMVPs),
                   SendPropInt(SENDINFO_ARRAY(m_iMVPs), COORD_INTEGER_BITS + 1,
                               SPROP_UNSIGNED)),
    SendPropArray3(SENDINFO_ARRAY3(m_bHasDefuser),
                   SendPropInt(SENDINFO_ARRAY(m_bHasDefuser), 1,
                               SPROP_UNSIGNED)),
    SendPropArray3(SENDINFO_ARRAY3(m_szClan),
                   SendPropStringT(SENDINFO_ARRAY(m_szClan))),
    END_SEND_TABLE()
    //=============================================================================
    // HPE_END
    //=============================================================================
...
public:
  ...
  bool operator()(CBasePlayer *player) {
    ...
    if ((dot > 0.995f) || (dot > 0.98f && distance < 900) ||
        (dot > 0.8f && distance < 250)) {
      trace_t tr;
      CTraceFilterSkipTwoEntities filter(player, m_targetEntity,
                                         COLLISION_GROUP_DEBRIS);
      UTIL_TraceLine(eye, m_target,
                     (CONTENTS_OPAQUE | CONTENTS_SOLID | CONTENTS_MOVEABLE |
                      CONTENTS_DEBRIS),
                     &filter, &tr);
      ...
      if (tr.fraction == 1.0f) {
        ...
        m_spotted = true;
        return false; // spotted already, so no reason to check for other
                      // players spotting the same thing.
      }
    }
  ...
  bool Spotted(void) const { return m_spotted; }
...
void CCSPlayerResource::UpdatePlayerData(void) {
  ...
  for (i = 0; i < MAX_HOSTAGES; i++) {
    ...
    if (pHostage->IsValid()) {
      ...
      //			engine->Con_NPrintf( i, "ID:%d
      //Pos:(%.0f,%.0f,%.0f)", pHostage->entindex(), pHostage->GetAbsOrigin().x,
      //pHostage->GetAbsOrigin().y, pHostage->GetAbsOrigin().z );
    } else {
  ...
  if (!m_foundGoalPositions) {
    // We only need to update these once a map, but we need the client to know
    // about them.
    ...
    while ((ent = gEntList.FindEntityByClassname(ent, "func_bomb_target")) !=
      ...
      CNavArea *area =
          TheNavMesh->GetNearestNavArea(pos, true, 10000.0f, false, false);
      const char *placeName =
          (area) ? TheNavMesh->PlaceToName(area->GetPlace()) : NULL;
      if (placeName == NULL) {
  ...
  for (int i = 0; i < MAX_PLAYERS + 1; i++) {
    ...
    Spotter spotter(target, target->EyePosition(),
                    (target->GetTeamNumber() == TEAM_CT) ? TEAM_TERRORIST
                                                         : TEAM_CT);
    ForEachPlayer(spotter);
    if (spotter.Spotted()) {
      if (target->HasC4()) {
