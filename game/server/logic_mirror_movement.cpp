...
// entity (most likely a point camera) at a the same offset, mirrored about the
...
private:
  void SetMirrorTarget(
      const char *pName); // Set entity to watch and mirror (ex. the player)
  void SetTarget(const char *pName); // Set entity to move based on the Mirror
                                     // Target entity (ex. a point_camera)
  void SetMirrorRelative(
      const char *pName); // Set the point about which to measure an offset to
                          // orient based upon (ex. portal 1)
  void SetRemoteTarget(
      const char *pName); // Entity's orientation/location from which to offset
                          // the movement target (ex. portal 2)
...
void CLogicMirrorMovement::Think() {
  ...
  if (!m_hMirrorTarget.Get()) {
    ...
    // BUGBUG: If m_strSetMirrorTarget doesn't exist in ent list, we get
    // per-think searches with no results ever...
    SetMirrorTarget(STRING(m_strMirrorTarget));
  }

  // Make sure all entities are valid
  if (m_hMirrorTarget.Get() && m_hMovementTarget.Get() &&
      m_hRemoteTarget.Get() && m_hMirrorRelative.Get()) {
    // Get our two portal's world transforms transforms
    VMatrix matPortal1ToWorldInv, matPortal2ToWorld;
    MatrixInverseGeneral(m_hMirrorRelative->EntityToWorldTransform(),
                         matPortal1ToWorldInv);
    matPortal2ToWorld = m_hRemoteTarget->EntityToWorldTransform();

    VMatrix matTransformToRemotePortal =
        matPortal1ToWorldInv * matPortal2ToWorld;
    ...
    // map this position and orientation to the remote portal, mirrored (invert
    // the result)
    Vector ptNewPosition, vNewLook;
    ptNewPosition = matPortal1ToWorldInv * ptCameraPosition;
    ptNewPosition =
        matPortal2ToWorld *
        (Vector(-ptNewPosition.x, -ptNewPosition.y, ptNewPosition.z));

    vNewLook = matPortal1ToWorldInv.ApplyRotation(vCameraLook);
    vNewLook = matPortal2ToWorld.ApplyRotation(
