...
//			has no networked representation, so has to store origin,
//etc. itself
//
//-----------------------------------------------------------------------------
...
DEFINE_FIELD(m_Origin, FIELD_VECTOR), DEFINE_FIELD(m_Angles, FIELD_VECTOR),
    ...
    //-----------------------------------------------------------------------------
    // Purpose: Converts inputed euler angles to internal angle format
    // (quaternions)
    //-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Adds the keyframe into the path after all the other keys have
// spawned
//-----------------------------------------------------------------------------
...
void CPathKeyFrame::Link(void) {
  m_pNextKey = dynamic_cast<CPathKeyFrame *>(
      gEntList.FindEntityByName(NULL, m_iNextKey));
...
public:
  ...
  // derived classes should override this to get notification of arriving at new
  // keyframes
  ...
  float m_flAverageSpeedAcrossFrame; // for advancing time with speed (not the
                                     // normal visa-versa)
  ...
  CPathKeyFrame *m_pPreKeyFrame,
      *m_pPostKeyFrame; // pre- and post-keyframe's for spline interpolation
...
DEFINE_KEYFIELD(m_iPositionInterpolator, FIELD_INTEGER, "PositionInterpolator"),
    DEFINE_KEYFIELD(m_iRotationInterpolator, FIELD_INTEGER,
...
//-----------------------------------------------------------------------------
// Purpose: Starts the object moving from it's current position, in the
// direction indicated Input  : direction - 1 is forward through the sequence,
// -1 is backwards, and 0 is stop Output : Returns true on success, false on
// failure.
//-----------------------------------------------------------------------------
bool CBaseMoveBehavior::StartMoving(int direction) {
  ...
  if (m_iDirection == direction) {
    ...
    // if we're going in reverse, swap the currentkey and targetkey (since we're
    // going opposite dir)
    if (direction == 1) {
    ...
    // calculate the keyframes before and after the keyframes we're
  ...
  // ->m_flNextTime is the time to traverse to the NEXT key, so we need the
  // opposite if travelling backwards
  if (m_iDirection == 1) {
    m_flAnimStartTime = GetLocalTime() - m_flTimeIntoFrame;
    m_flAnimEndTime =
        GetLocalTime() + m_pCurrentKeyFrame->m_flNextTime - m_flTimeIntoFrame;
  } else {
    ...
    m_flAnimStartTime = GetLocalTime() - m_flTimeIntoFrame;
    m_flAnimEndTime =
        GetLocalTime() + m_pTargetKeyFrame->m_flNextTime - m_flTimeIntoFrame;
  }

  // calculate the average speed at which we cross
  float animDuration = (m_flAnimEndTime - m_flAnimStartTime);
  float dist =
      (m_pCurrentKeyFrame->m_Origin - m_pTargetKeyFrame->m_Origin).Length();
...
//			in the animation.
//			used to advance keyframed objects that have dynamic
//speeds.
// Input  : moveTime -
// Output : float - the new time in the keyframing sequence
//-----------------------------------------------------------------------------
float CBaseMoveBehavior::CalculateTimeAdvancementForSpeed(float moveTime,
                                                          float speed) {
...
float CBaseMoveBehavior::SetObjectPhysicsVelocity(float moveTime) {
  ...
  float newTime =
      (destTime - m_flAnimStartTime) / (m_flAnimEndTime - m_flAnimStartTime);
  Vector newPos;
  QAngle newAngles;

  IPositionInterpolator *pInterp =
      GetPositionInterpolator(m_iPositionInterpolator);
  if (pInterp) {
    // setup key frames
  ...
  // find our velocity vector (newPos - currentPos) and scale velocity vector
  // according to the movetime
