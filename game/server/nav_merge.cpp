...
public:
  ...
  int Count(void) const { return m_areaCount; }

private:
  void WriteCorner(CNavArea *area, KeyValues *areaKey, NavCornerType corner,
  ...
  void WriteConnections(CNavArea *area, KeyValues *areaKey, NavDirType dir,
                        const char *dirName) {
...
void CNavMesh::CommandNavSaveSelected(const CCommand &args) {
  ...
  Msg("Selected set saved to %s.  Use 'nav_merge_mesh %s_selected_%4.4d' to "
      "merge it into another mesh.\n",
      path, fname, i);
  data->deleteThis();
}

//--------------------------------------------------------------------------------------------------------
CON_COMMAND_F(nav_save_selected,
              "Writes the selected set to disk for merging into another mesh "
              "via nav_merge_mesh.",
...
//--------------------------------------------------------------------------------------------------------
void ReconnectMergedArea(CUtlDict<CNavArea *, int> &newAreas,
                         KeyValues *areaKey, NavDirType dir,
                         const char *dirName) {
  ...
  if (dirKey) {
    ...
    while (connection) {
      ...
      if (otherIndex != newAreas.InvalidIndex()) {
        ...
        area->ConnectTo(other, dir); // only a 1-way connection.  the other area
                                     // will connect back to us.
      }
...
void CNavMesh::CommandNavMergeMesh(const CCommand &args) {
  ...
  if (!data->LoadFromFile(filesystem, path)) {
    ...
    // First add the areas, and put them in the correct places.  We can save off
    // the new area ID at the same time.
...
int NavMeshMergeAutocomplete(char const *partial,
  ...
  const char *txtFilename =
      filesystem->FindFirstEx("maps/*_selected_*.txt", "MOD", &findHandle);
  while (txtFilename) {
    Q_FileBase(txtFilename, txtFilenameNoExtension,
               sizeof(txtFilenameNoExtension));
    if (!Q_strnicmp(txtFilenameNoExtension, partial, partialLength) &&
      ...
      Q_snprintf(commands[numMatches++], COMMAND_COMPLETION_ITEM_LENGTH,
                 "%s %s", commandName, txtFilenameNoExtension);
...
//--------------------------------------------------------------------------------------------------------
CON_COMMAND_F_COMPLETION(nav_merge_mesh,
                         "Merges a saved selected set into the current mesh.",
                         FCVAR_GAMEDLL | FCVAR_CHEAT,
                         NavMeshMergeAutocomplete) {
  if (!UTIL_IsCommandIssuedByServerAdmin())
    return;
