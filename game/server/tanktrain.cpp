...
public:
  ...
  // Filter out damage messages that don't contain blast damage (impervious to
...
//			bitsDamageType - Bitfield of damage types that were
//inflicted.
//-----------------------------------------------------------------------------
...
void CTankTargetChange::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
                            USE_TYPE useType, float value) {
  CBaseEntity *pTarget =
      gEntList.FindEntityByName(NULL, m_target, NULL, pActivator, pCaller);
  ...
  while (pTarget) {
    ...
    pTarget =
        gEntList.FindEntityByName(pTarget, m_target, NULL, pActivator, pCaller);
  }
}

// UNDONE: Should be just a logical entity, but we act as another static sound
// channel for the train
...
DEFINE_FIELD(m_hTrain, FIELD_EHANDLE),
    ...
    DEFINE_SOUNDPATCH(m_soundTreads), DEFINE_SOUNDPATCH(m_soundEngine),
...
//			pActivator - The activating entity if this is called
//from an input 				or Use handler, NULL otherwise.
//-----------------------------------------------------------------------------
CBaseEntity *CTankTrainAI::FindTarget(string_t target,
...
int CTankTrainAI::SoundEnginePitch(void) {
  ...
  if (pTrain->GetMaxSpeed()) {
    return 90 +
           (fabs(pTrain->GetCurrentSpeed()) * (20) / pTrain->GetMaxSpeed());
  }
  return 100;
}
...
void CTankTrainAI::SoundEngineStart(void) {
  ...
  if (m_soundEngine) {
    ...
    controller.CommandAdd(m_soundEngine, 0, SOUNDCTRL_CHANGE_PITCH, 1.5,
                          random->RandomInt(130, 145));
    controller.CommandAdd(m_soundEngine, 1.5, SOUNDCTRL_CHANGE_PITCH, 2,
                          random->RandomInt(105, 115));
  }
...
void CTankTrainAI::Activate(void) {
  ...
  if (pTrain) {
    ...
    if (m_movementSoundName != NULL_STRING) {
      ...
      m_soundTreads =
          controller.SoundCreate(filter, pTrain->entindex(), CHAN_STATIC,
                                 STRING(m_movementSoundName), ATTN_NORM * 0.5);
    }
    if (m_engineSoundName != NULL_STRING) {
      CPASAttenuationFilter filter(this);
      m_soundEngine =
          controller.SoundCreate(filter, pTrain->entindex(), CHAN_STATIC,
                                 STRING(m_engineSoundName), ATTN_NORM);
    }
  }
}
...
//-----------------------------------------------------------------------------
int PathFindDirection(CPathTrack *pStart, const Vector &startPosition,
  ...
  do {
    ...
    if (pNext) {
      ...
      float delta =
          (pNext->GetLocalOrigin() - pPath->GetLocalOrigin()).LengthSqr();
      length += delta;
      ...
      length = (float)COORD_EXTENT *
               (float)COORD_EXTENT; // HACKHACK: Max quad length
    }
  ...
  if (pNearest == pStart ||
      (pNearest->GetLocalOrigin() - startPosition).LengthSqr() < 100)
    return 0;
...
void CTankTrainAI::Think(void) {
  ...
  if (pTarget) {
    desired = PathFindDirection(pTrain->m_ppath, pTrain->GetLocalOrigin(),
                                pTarget->GetLocalOrigin());
  }
  ...
  // UNDONE: Align the think time with arrival, and bump this up to a few
  // seconds
  SetNextThink(gpGlobals->curtime + 0.5f);
