...
ConVar nav_split_place_on_ground(
    "nav_split_place_on_ground", "0", FCVAR_CHEAT,
    "If true, nav areas will be placed flush with the ground when split.");
ConVar nav_area_bgcolor(
    "nav_area_bgcolor", "0 0 0 30", FCVAR_CHEAT,
    "RGBA color to draw as the background color for nav areas while editing.");
ConVar
    nav_corner_adjust_adjacent("nav_corner_adjust_adjacent", "18", FCVAR_CHEAT,
                               "radius used to raise/lower corners in nearby "
                               "areas when raising/lowering corners.");
...
ConVar nav_max_view_distance("nav_max_view_distance", "6000", FCVAR_CHEAT,
                             "Maximum range for precomputed nav mesh "
                             "visibility (0 = default 1500 units)");
ConVar nav_update_visibility_on_edit(
    "nav_update_visibility_on_edit", "0", FCVAR_CHEAT,
...
ConVar nav_show_potentially_visible(
    "nav_show_potentially_visible", "0", FCVAR_CHEAT,
...
ConVar nav_selected_set_color(
    "nav_selected_set_color", "255 255 200 96", FCVAR_CHEAT,
    "Color used to draw the selected set background while editing.", false,
    0.0f, false, 0.0f, SelectedSetColorChaged);
ConVar nav_selected_set_border_color(
    "nav_selected_set_border_color", "100 100 0 255", FCVAR_CHEAT,
    "Color used to draw the selected set borders while editing.", false, 0.0f,
    false, 0.0f, SelectedSetColorChaged);
...
class ForgetArea {
public:
  ForgetArea(CNavArea *area) { m_area = area; }
...
public:
  AreaDestroyNotification(CNavArea *area) { m_area = area; }
...
void CNavArea::ConnectElevators(void) {
  ...
  while ((elevator = (CFuncElevator *)gEntList.FindEntityByClassname(
              elevator, "func_elevator")) != NULL) {
    ...
    elevator->CollisionProp()->WorldSpaceSurroundingBounds(&elevatorExtent.lo,
                                                           &elevatorExtent.hi);

    if (IsOverlapping(elevatorExtent)) {
      // overlaps in 2D - check that this area is within the shaft of the
      // elevator
      ...
      for (int f = 0; f < elevator->GetNumFloors(); ++f) {
        ...
        if (center.z <= floor->height + tolerance &&
            center.z >= floor->height - tolerance) {
          if (m_elevator) {
          ...
          for (int of = 0; of < elevator->GetNumFloors(); ++of) {
            ...
            if (floorArea) {
              ...
              Warning("Floor %d ('%s') of elevator at ( %3.2f, %3.2f, %3.2f ) "
                      "has no matching navigation areas\n",
...
void CNavArea::OnRoundRestart(void) {
  // need to redo this here since func_elevators are deleted and recreated at
  // round restart
  ConnectElevators();
  m_damagingTickCount = 0;
  ClearAllNavCostEntities();
...
void CNavArea::IncrementPlayerCount(int teamID, int entIndex) {
  ConColorMsg(Color(128, 255, 128, 255),
              "%f: Adding ent %d (team %d) to area %d\n", gpGlobals->curtime,
              entIndex, teamID, GetID());
...
void CNavArea::DecrementPlayerCount(int teamID, int entIndex) {
  ConColorMsg(Color(128, 128, 255, 255),
              "%f: Removing ent %d (team %d) from area %d\n",
...
float CNavArea::ComputeGroundHeightChange(const CNavArea *area) {
  ...
  float toZ, fromZ;
  if (TheNavMesh->GetSimpleGroundHeight(closeTo + Vector(0, 0, StepHeight),
                                        &toZ) == false) {
    return 0.0f;
  }

  if (TheNavMesh->GetSimpleGroundHeight(closeFrom + Vector(0, 0, StepHeight),
                                        &fromZ) == false) {
    return 0.0f;
  }
...
void CNavArea::FinishSplitEdit(CNavArea *newArea, NavDirType ignoreEdge) {
  ...
  if ((m_seCorner.x - m_nwCorner.x) > 0.0f &&
      (m_seCorner.y - m_nwCorner.y) > 0.0f) {
    newArea->m_invDxCorners =
        1.0f / (newArea->m_seCorner.x - newArea->m_nwCorner.x);
    newArea->m_invDyCorners =
        1.0f / (newArea->m_seCorner.y - newArea->m_nwCorner.y);
  } else {
...
void CNavArea::CalcDebugID() {
  if (m_debugid == 0) {
    // calculate a debug ID which will be constant for this nav area across
    // generation runs
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Removes all connections in directions to left and right of specified
 * direction
 */
...
bool CNavArea::IsCoplanar(const CNavArea *area) const {
  ...
  bool isOnDisplacement =
      (m_node[NORTH_WEST] && m_node[NORTH_WEST]->IsOnDisplacement()) ||
      (m_node[NORTH_EAST] && m_node[NORTH_EAST]->IsOnDisplacement()) ||
      (m_node[SOUTH_EAST] && m_node[SOUTH_EAST]->IsOnDisplacement()) ||
      (m_node[SOUTH_WEST] && m_node[SOUTH_WEST]->IsOnDisplacement());
  ...
  bool areaIsOnDisplacement = (area->m_node[NORTH_WEST] &&
                               area->m_node[NORTH_WEST]->IsOnDisplacement()) ||
                              (area->m_node[NORTH_EAST] &&
                               area->m_node[NORTH_EAST]->IsOnDisplacement()) ||
                              (area->m_node[SOUTH_EAST] &&
                               area->m_node[SOUTH_EAST]->IsOnDisplacement()) ||
                              (area->m_node[SOUTH_WEST] &&
                               area->m_node[SOUTH_WEST]->IsOnDisplacement());
...
void CNavArea::ComputeClosestPointInPortal(const CNavArea *to, NavDirType dir,
  ...
  //	const float margin = 0.0f; //GenerationStepSize/2.0f;  // causes trouble
  //with very small/narrow nav areas
  ...
  if (dir == NORTH || dir == SOUTH) {
    ...
    /// @todo Need better check whether edge is outer edge or not - partial
    /// overlap is missed
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Return true if the given area and 'other' share a colinear edge (ie: no
 * drop-down or step/jump/climb)
 */
bool CNavArea::IsContiguous(const CNavArea *other) const {
  ...
  Vector otherEdge;
  other->ComputePortal(this, OppositeDirection((NavDirType)dir), &otherEdge,
                       &halfWidth);

  // must use stepheight because rough terrain can have gaps/cracks between
  // adjacent nav areas
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Return height change between edges of adjacent nav areas (not actual
 * underlying ground)
 */
...
void CNavArea::Draw(void) const {
  ...
  if (IsDegenerate()) {
    ...
    NDebugOverlay::Text(GetCenter(),
                        UTIL_VarArgs("Degenerate area %d", GetID()), true,
                        DebugDuration);
  }
  ...
  if (nav_show_light_intensity.GetBool()) {
    for (int i = 0; i < NUM_CORNERS; ++i) {
      ...
      NDebugOverlay::Line(end, pos, lightIntensity, lightIntensity,
                          MAX(192, lightIntensity), true, DebugDuration);
    }
  }
  ...
  if (4 == sscanf(nav_area_bgcolor.GetString(), "%d %d %d %d", &(bgcolor[0]),
    ...
    if (bgcolor[3] > 0) {
      ...
      NDebugOverlay::Triangle(nw + offset, se + offset, ne + offset, bgcolor[0],
                              bgcolor[1], bgcolor[2], bgcolor[3], true,
                              DebugDuration);
  ...
  if (IsBlocked(TEAM_ANY) || HasAvoidanceObstacle() || IsDamaging()) {
    NavEditColor color =
        (IsBlocked(TEAM_ANY) && (m_attributeFlags & NAV_MESH_NAV_BLOCKER))
...
void CNavArea::AddToOpenList(void) {
  ...
  // Since costs are positive, IEEE754 let's us compare as integers (see
...
bool IsHidingSpotInCover(const Vector &spot) {
  ...
  for (float angle = 0.0f; angle < 2.0f * M_PI; angle += inc) {
    ...
    UTIL_TraceLine(from, to, MASK_NPCSOLID_BRUSHONLY, NULL,
...
void ClassifySniperSpot(HidingSpot *spot) {
  ...
  FOR_EACH_VEC(TheNavAreas, it) {
    ...
    for (walkable.y = areaExtent.lo.y + GenerationStepSize / 2.0f;
         walkable.y < areaExtent.hi.y; walkable.y += GenerationStepSize) {
      for (walkable.x = areaExtent.lo.x + GenerationStepSize / 2.0f;
           walkable.x < areaExtent.hi.x; walkable.x += GenerationStepSize) {
...
float CNavArea::GetLightIntensity(const Vector &pos) const {
  ...
  float northLight = m_lightIntensity[NORTH_WEST] * (1 - dX) +
                     m_lightIntensity[NORTH_EAST] * dX;
  float southLight = m_lightIntensity[SOUTH_WEST] * (1 - dX) +
                     m_lightIntensity[SOUTH_EAST] * dX;
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Compute light intensity at corners and center (requires client via
 * listenserver)
 */
bool CNavArea::ComputeLighting(void) {
  ...
  for (int i = 0; i < NUM_CORNERS; ++i) {
    ...
    pos.z = GetZ(pos) + HalfHumanHeight -
    ...
    //{
    // NDebugOverlay::Line( pos, pos + Vector( 0, 0, -100 ), 255, 0, 0, false,
    // 100.0f );
    //	return false;
    //}
    ...
    {
      // NDebugOverlay::Line( pos, pos + Vector( 0, 0, -100 ), 255, 127, 0,
      // false, 100.0f );
      return false;
    }

    // NDebugOverlay::Line( pos, pos + Vector( 0, 0, -100 ), 0, 255, 127, false,
    // 100.0f );
    ...
    lightIntensity = clamp(
        lightIntensity, 0.f,
        1.f); // sum can go well over 1.0, but it's the lower region we care
...
void CNavArea::RaiseCorner(NavCornerType corner, int amount,
  ...
  for (int x = gridX - shift; x <= gridX + shift; ++x) {
    ...
    for (int y = gridY - shift; y <= gridY + shift; ++y) {
      ...
      NavAreaVector *areas =
          &TheNavMesh->m_grid[x + y * TheNavMesh->m_gridSizeX];
...
#ifdef TERROR
  if ((teamID == TEAM_SURVIVOR) &&
      (m_attributeFlags & CNavArea::NAV_PLAYERCLIP))
    return true;
#endif
...
//--------------------------------------------------------------------------------------------------------
void CNavArea::MarkAsBlocked(int teamID, CBaseEntity *blocker,
                             bool bGenerateEvent) {
...
void CNavArea::UpdateBlockedFromNavBlockers(void) {
  ...
  bool isBlocked =
      CFuncNavBlocker::CalculateBlocked(m_isBlocked, bounds.lo, bounds.hi);
 ...
 * The semantics of this method have gotten very muddled - needs refactoring
 * (MSB 5/7/09)
 */
void CNavArea::UpdateBlocked(bool force, int teamID) {
  ...
  CTraceFilterWalkableEntities filter(NULL, COLLISION_GROUP_PLAYER_MOVEMENT,
                                      WALK_THRU_PROP_DOORS |
                                          WALK_THRU_BREAKABLES);
#else
  CTraceFilterWalkableEntities filter(NULL, COLLISION_GROUP_PLAYER_MOVEMENT,
...
void CNavArea::CheckFloor(CBaseEntity *ignore) {
  ...
  // See if spot is valid
  trace_t tr;
  UTIL_TraceHull(origin, origin, mins, maxs, MASK_NPCSOLID_BRUSHONLY, ignore,
...
void CNavArea::UpdateAvoidanceObstacles(void) {
  ...
  for (int i = 0; i < TheNavMesh->GetObstructions().Count(); ++i) {
    ...
    obstructingEntity->CollisionProp()->WorldSpaceSurroundingBounds(
...
//--------------------------------------------------------------------------------------------------------------
// Return the cost multiplier of this area's func_nav_cost entities for the
// given actor
...
bool CNavArea::HasFuncNavAvoid(void) const {
  for (int i = 0; i < m_funcNavCostVector.Count(); ++i) {
    CFuncNavAvoid *avoid =
        dynamic_cast<CFuncNavAvoid *>(m_funcNavCostVector[i].Get());
    if (avoid) {
      return true;
    }
  }
...
bool CNavArea::HasFuncNavPrefer(void) const {
  for (int i = 0; i < m_funcNavCostVector.Count(); ++i) {
    CFuncNavPrefer *prefer =
        dynamic_cast<CFuncNavPrefer *>(m_funcNavCostVector[i].Get());
    if (prefer) {
      return true;
    }
  }
...
static ConCommand
    nav_check_floor("nav_check_floor", CommandNavCheckFloor,
...
bool SelectOverlappingAreas::operator()(CNavArea *area) {
  ...
  while (start.x < se.x) {
    ...
    while (start.y < se.y) {
      ...
      if (TheNavMesh->FindNavAreaOrLadderAlongRay(start, end, &overlappingArea,
                                                  &overlappingLadder, area)) {
        if (overlappingArea) {
...
static ConCommand nav_select_overlapping(
    "nav_select_overlapping", CommandNavSelectOverlapping,
...
void CNavArea::SetupPVS(void) const {
  ...
  Vector shift(eye);
  for (shift.y = margin; shift.y <= GetSizeY() - margin;
       shift.y += GenerationStepSize) {
    for (shift.x = margin; shift.x <= GetSizeX() - margin;
         shift.x += GenerationStepSize) {
      // Optimization:
      // If we are already POTENTIALLY_VISIBLE, and no longer
      // COMPLETELY_VISIBLE, there's no way for vis to change again.
...
//--------------------------------------------------------------------------------------------------------
/**
 * Do actual line-of-sight traces to determine if any part of given area is
...
CNavArea::VisibilityType CNavArea::ComputeVisibility(const CNavArea *area,
  ...
  if (bCheckPVS) {
    ...
    if (!engine->CheckBoxInPVS(areaExtent.lo, areaExtent.hi, m_PVS,
                               m_nPVSSize)) {
      if (pOutsidePVS)
        *pOutsidePVS = true;
      return NOT_VISIBLE;
  ...
  vTraceMaxs.z =
      MAX(MAX(MAX(vThisNW.z, vThisNE.z), vThisSE.z), vThisSW.z) + 0.1;
  ...
  UTIL_TraceHull(vThisCenter, vTarget, vTraceMins, vTraceMaxs, MASK_NAV_VISION,
                 &traceFilter, &tr);

  if (tr.fraction == 1.0 ||
      (tr.endpos.x > vOtherMins.x && tr.endpos.x < vOtherMaxs.x &&
       tr.endpos.y > vOtherMins.y && tr.endpos.y < vOtherMaxs.y)) {
    return COMPLETELY_VISIBLE; // Counter-intuitive: the way this function was
                               // written, "COMPLETELY_VISIBLE" actually means
                               // "I am completely visible to the other"
  }
  ...
  float angleTolerance =
      nav_potentially_visible_dot_tolerance
          .GetFloat(); // if corner-to-eye angles are this close to
                       // center-to-eye angles, assume the same result and skip
                       // the trace
  ...
  for (shift.y = margin; shift.y <= GetSizeY() - margin;
    ...
    for (shift.x = margin; shift.x <= GetSizeX() - margin;
      ...
      // Optimization - treat long-distance traces that are effectively
      // collinear as the same
...
//--------------------------------------------------------------------------------------------------------
/**
 * Return a list of the delta between our visibility list and the given adjacent
 * area
 */
const CNavArea::CAreaBindInfoArray &
CNavArea::ComputeVisibilityDelta(const CNavArea *other) const {
  static CAreaBindInfoArray delta;

  delta.RemoveAll();

  // do not delta from a delta - if 'other' is already inheriting, use its
  // inherited source directly
  ...
  // add any visible areas in my list that are not in 'others' list into the
  // delta
  int i, j;
  for (i = 0; i < m_potentiallyVisibleAreas.Count(); ++i) {
    if (m_potentiallyVisibleAreas[i].area) {
      ...
      for (j = 0; j < other->m_potentiallyVisibleAreas.Count(); ++j) {
        if (m_potentiallyVisibleAreas[i].area ==
                other->m_potentiallyVisibleAreas[j].area &&
            m_potentiallyVisibleAreas[i].attributes ==
                other->m_potentiallyVisibleAreas[j].attributes) {
      ...
      if (j == other->m_potentiallyVisibleAreas.Count()) {
        // my vis area not in adjacent area's vis list or has different
        // visibility attributes - add to delta
        delta.AddToTail(m_potentiallyVisibleAreas[i]);
      }
    }
  }

  // add explicit NOT_VISIBLE references to areas in 'others' list that are NOT
  // in mine
  for (j = 0; j < other->m_potentiallyVisibleAreas.Count(); ++j) {
    if (other->m_potentiallyVisibleAreas[j].area) {
      ...
      if (i == m_potentiallyVisibleAreas.Count()) {
        // 'other' has area in their list that we don't - mark it explicitly
        // NOT_VISIBLE
        AreaBindInfo info;
 ...
 * Compute full list of all areas visible for each area.  This list will be
 * compressed into deltas in the PostCustomAnalysis() step.
 */
...
void CNavArea::ComputeVisToArea(CNavArea *&pOtherArea) {
  ...
  if (area != g_pCurVisArea) {
    ...
    visOtherToThis = g_pCurVisArea->ComputeVisibility(
        area, true, true,
        &bOutsidePVS); // TODO: Hacky right now. Compute visibility for the
                       // "complete" case actually returns how completely
                       // visible the area is to the other. Should fix it to be
                       // more clear [1/30/2009 tom]
...
//--------------------------------------------------------------------------------------------------------
/**
 * Determine visibility from this area to all potentially/completely visible
 * areas in the mesh
 */
void CNavArea::ComputeVisibilityToMesh(void) {
  ...
  g_pCurVisArea = this;
  ParallelProcess("CNavArea::ComputeVisibilityToMesh", collector.m_area.Base(),
                  collector.m_area.Count(), &ComputeVisToArea);
  ...
  FOR_EACH_VEC(collector.m_area, it) {
    ...
    Assert(g_pNavVisPairHash->Find(visPair) ==
           g_pNavVisPairHash->InvalidHandle());
...
bool CNavArea::IsCompletelyVisible(const CNavArea *viewedArea) const {
  ...
  for (int i = 0; i < m_potentiallyVisibleAreas.Count(); ++i) {
    if (m_potentiallyVisibleAreas[i].area == viewedArea) {
      // our list is definitive - viewedArea is in our list, but is not
      // completely visible
      return (m_potentiallyVisibleAreas[i].attributes & COMPLETELY_VISIBLE)
                 ? true
                 : false;
    }
  }
...
//--------------------------------------------------------------------------------------------------------
/**
 * Return true if any portion of this area is visible to anyone on the given
 * team
 */
 ...
 * by someone on the team (very fast)
 */
