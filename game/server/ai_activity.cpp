...
void CAI_BaseNPC::AddActivityToSR(const char *actName, int actID) {
  ...
  // NOTE: This assertion generally means that the activity enums are out of
  // order or that new enums were not added to all
  //		 relevant tables.  Make sure that you have included all new
  //enums in: 			game_shared/ai_activity.h 			game_shared/activitylist.cpp
  //			dlls/ai_activity.cpp
  MEM_ALLOC_CREDIT();

  static int lastActID = -2;
  Assert(actID >= LAST_SHARED_ACTIVITY || actID == lastActID + 1 ||
         actID == ACT_INVALID);
  lastActID = actID;
...
const char *CAI_BaseNPC::GetActivityName(int actID) {
  ...
  // m_pActivitySR only contains public activities, ActivityList_NameForIndex()
  // has them all
...
#define ADD_ACTIVITY_TO_SR(activityname)                                       \
  AddActivityToSR(#activityname, activityname)
