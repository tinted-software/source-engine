...
ConVar advisor_throw_stage_distance(
    "advisor_throw_stage_distance", "180.0", FCVAR_NONE,
    "Advisor will try to hold an object this far in front of him just before "
    "throwing it at you. Small values will clobber the shield and be very "
    "bad.");
...
protected:
#if NPC_ADVISOR_HAS_BEHAVIOR
  Vector
  GetThrowFromPos(CBaseEntity *pEnt); ///< Get the position in which we shall
                                      ///< hold an object prior to throwing it
#endif
  ...
  CBaseEntity *PickThrowable(
      bool bRequireInView); ///< choose an object to throw at the player (so it
                            ///< can get stuffed in the handle array)
  ...
  Vector m_vSavedLeadVel; ///< save off player velocity for leading a bit before
                          ///< actually pelting them.
#endif
};
...
#if NPC_ADVISOR_HAS_BEHAVIOR
    DEFINE_KEYFIELD(
        m_iszStagingEntities, FIELD_STRING,
        "staging_ent_names"), ///< entities named this constitute the positions
                              ///< to which we stage objects to be thrown
...
void CNPC_Advisor::Activate() {
  ...
  // positions loaded, null out the m_hvStagedEnts array with exactly as many
  // null spaces
...
void CNPC_Advisor::StartTask(const Task_t *pTask) {
  ...
  case TASK_ADVISOR_FIND_OBJECTS: {
    ...
    AssertMsg(pVolume, "Combine advisor needs 'levitationarea' key pointing to "
                       "a trigger volume.");
    ...
    /*
    // this is the old mechanism, using a hardcoded box and an entity
    enumerator.
    // since deprecated.
    ...
    else
    {
            ...
            //NDebugOverlay::Box( GetAbsOrigin(), Vector( -408, -368, -188 ),
    Vector( 92, 208, 168 ), 255, 255, 0, 1, 5 );

            // one-off class used to determine which entities we want from the
    UTIL_EntitiesInBox class CAdvisorLevitateEntitiesEnum : public
    CFlaggedEntitiesEnum
            {
            public:
                    CAdvisorLevitateEntitiesEnum( CBaseEntity **pList, int
    listMax, int nMinMass, int nMaxMass ) :	CFlaggedEntitiesEnum( pList,
    listMax, 0 ), m_nMinMass( nMinMass ), m_nMaxMass( nMaxMass )
                    {
                    }

                    virtual IterationRetval_t EnumElement( IHandleEntity
    *pHandleEntity )
                    {
                            CBaseEntity *pEntity = gEntList.GetBaseEntity(
    pHandleEntity->GetRefEHandle() ); if ( AdvisorCanLevitateEntity( pEntity,
    m_nMinMass, m_nMaxMass ) )
                            {
                                    return CFlaggedEntitiesEnum::EnumElement(
    pHandleEntity );
                            }
            ...
            CAdvisorLevitateEntitiesEnum levitateEnum( list, ARRAYSIZE( list ),
    10, 220 );

            int nCount = UTIL_EntitiesInBox( GetAbsOrigin() - Vector( 554, 368,
    188 ), GetAbsOrigin() + Vector( 92, 208, 168 ), &levitateEnum ); for ( int i
    = 0; i < nCount; i++ )
            {
                    //Msg( "%d found %s\n", m_physicsObjects.Count(), STRING(
    list[i]->GetModelName() ) ); if ( list[i]->GetMoveType() ==
    MOVETYPE_VPHYSICS )
                    {
                            //Msg( "   %d added %s\n", m_physicsObjects.Count(),
    STRING( list[i]->GetModelName() ) ); m_physicsObjects.AddToTail( list[i] );
                    }
            }
    }
    */
  ...
  // we're about to pelt the player with everything. Start the warning effect on
  // the first object.
...
void CNPC_Advisor::RunTask(const Task_t *pTask) {
  ...
  case TASK_ADVISOR_LEVITATE_OBJECTS: {
    ...
    // set the top and bottom on the levitation volume from the entities. If we
    // don't have both, zero it out so that we can use the old-style simpler
    // mechanism.
    ...
    for ( int i = 0; i < m_physicsObjects.Count(); i++ )
    {
            ...
            {
                    NDebugOverlay::Box( pEnt->GetAbsOrigin(),
    pEnt->CollisionProp()->OBBMins(), pEnt->CollisionProp()->OBBMaxs(), 0, 255,
    0, 1, 0.1 );
            }
    }*/
  ...
  // that object to our enemy's eyes, choose that one to throw. Otherwise, keep
  // looking.
  case TASK_ADVISOR_STAGE_OBJECTS: {
    if (m_iStagingNum > m_hvStagingPositions.Count()) {
      Warning("Advisor tries to stage %d objects but only has %d positions "
              "named %s! Overriding.\n",
              m_iStagingNum, m_hvStagingPositions.Count(),
              m_iszStagingEntities);
      m_iStagingNum = m_hvStagingPositions.Count();
    }

    // advisor_staging_num

    // in the future i'll distribute the staging chronologically. For now, yank
    ...
    if (m_hvStagedEnts.Count() < m_iStagingNum) {
      ...
      if (pThrowable) {
        ...
        DispatchParticleEffect("advisor_object_charge",
                               PATTACH_ABSORIGIN_FOLLOW, pThrowable, 0, false);
      }
    }
  ...
  case TASK_ADVISOR_BARRAGE_OBJECTS: {
    ...
    while (!pThrowable) { // player has destroyed whatever I planned to hit him
                          // with, get something else
    ...
    if ((gpGlobals->curtime >
         m_flThrowPhysicsTime - advisor_throw_lead_prefetch_time.GetFloat()) &&
        !m_vSavedLeadVel.IsValid()) {
      // save off the velocity we will use to lead the player a little early, so
      // that if he jukes at the last moment he'll have a better shot of dodging
      // the object.
  ...
  case TASK_ADVISOR_PIN_PLAYER: {
    ...
    if ( gpGlobals->curtime > m_playerPinFailsafeTime )
    {
            GetEnemy()->SetGravity(1.0f);
            GetEnemy()->SetMoveType( MOVETYPE_WALK );
            Warning( "Advisor did not leave PIN PLAYER mode. Aborting due to ten
    second failsafe!\n" ); TaskFail("Advisor did not leave PIN PLAYER mode.
    Aborting due to ten second failsafe!\n"); break;
    }
...
CBaseEntity *CNPC_Advisor::PickThrowable(bool bRequireInView) {
  ...
  Assert(numObjs < 128); /// I'll come back and utlvector this shortly -- wanted
                         /// easier debugging
  ...
  if (!!m_iszPriorityEntityGroupName) // if the string isn't null
  {
    ...
    for (int ii = 0; ii < numObjs; ++ii) {
      ...
      if (!pThrowEnt->NameMatches(
              m_iszPriorityEntityGroupName)) // if this is not a priority object
        continue;
...
void CNPC_Advisor::PreHurlClearTheWay(CBaseEntity *pThrowable,
  ...
  AssertMsg(nFoundCt < 128, "Found more than 128 obstructions between advisor "
                            "and Gordon while throwing. (safe to continue)\n");
...
Vector CNPC_Advisor::GetThrowFromPos(CBaseEntity *pEnt) {
  ...
  float howFarInFront =
      ...
      effecRadius * 1.43f; // clamp(lenToPlayer - posDist +
                           // effecRadius,effecRadius*2,90.f + effecRadius);
...
// MOTION CALLBACK
//==============================================================================================
CAdvisorLevitate::simresult_e
CAdvisorLevitate::Simulate(IPhysicsMotionController *pController,
                           IPhysicsObject *pObject, float deltaTime,
                           Vector &linear, AngularImpulse &angular) {
    ...
    if (pAdvisor->DidThrow(
      ...
      if (bMovingUp) {
        if (pos.z > m_vecGoalPos2.z) {
          ...
          linear =
              Vector(0, 0, Square((1.0f - m_flFloat)) * GetCurrentGravity());
  ...
  } else // old stateless technique
  {
    ...
    */
    ...
    linear = Vector(
        0, 0,
        m_flFloat *
            GetCurrentGravity()); // + m_flFloat * 0.5 * ( vecDir1 + vecDir2 );
    angular = Vector(0, 0, 10);
