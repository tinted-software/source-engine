...
ConVar hunter_hate_thrown_striderbusters("hunter_hate_thrown_striderbusters",
                                         "1");
ConVar
    hunter_hate_attached_striderbusters("hunter_hate_attached_striderbusters",
                                        "1");
...
ConVar
    hunter_stand_still("hunter_stand_still",
...
static ConCommand
    ent_create("hunter_shoot_flechette", CC_Hunter_Shoot_Flechette,
...
void CHunterFlechette::Explode() {
  ...
  RadiusDamage(CTakeDamageInfo(this, GetOwnerEntity(),
                               sk_hunter_flechette_explode_dmg.GetFloat(),
                               nDamageType),
               GetAbsOrigin(), sk_hunter_flechette_explode_radius.GetFloat(),
               CLASS_NONE, NULL);
...
//-----------------------------------------------------------------------------
inline void
HunterTraceHull_SkipPhysics(const Vector &vecAbsStart, const Vector &vecAbsEnd,
...
public:
  ...
  CAI_HunterEscortBehavior()
      : BaseClass(AI_FollowParams_t(AIF_HUNTER, true)), m_flTimeEscortReturn(0),
        m_bEnabled(false) {}
...
DEFINE_FIELD(m_flTimeEscortReturn, FIELD_TIME),
    ...
    DEFINE_FIELD(m_bEnabled, FIELD_BOOLEAN), END_DATADESC();
...
private:
  ...
  CSimpleSimTimer m_EyeSwitchTimer; // Controls how often we switch which eye is
                                    // focusing on our enemy.
  ...
  int m_nClampedShots; // The number of consecutive shots fired at an out-of-max
                       // yaw target.
  ...
  float m_fCorneredTimer; ///< hunter was cornered when fleeing player; it won't
                          ///< flee again until this time
...
void CNPC_Hunter::Spawn() {
  ...
  //CSoundEnvelopeController::GetController(); 	CPASAttenuationFilter filter(
  //this );
  //
          ...
          if (hunter_dodge_debug.GetBool()) {
            ...
            if (timeDrawnArrow != gpGlobals->curtime) {
              ...
              Vector vEndpoint(vecFuturePos.x, vecFuturePos.y,
                               UTIL_GetLocalPlayer()->WorldSpaceCenter().z -
                                   24);
  ...
  case TASK_RANGE_ATTACK2: {
    ...
    if (gpGlobals->curtime >= m_flNextFlechetteTime) {
      ...
      if (GetEnemy() != NULL && m_nFlechettesQueued > 0) {
        ...
        // If we fired three or more clamped shots in a row, call it quits so we
        // don't look dumb.
  ...
  case TASK_HUNTER_CHARGE: {
    ...
    if (eActivity == ACT_HUNTER_CHARGE_STOP ||
      ...
      if (IsActivityFinished()) {
        ...
        float groupDelay = gpGlobals->curtime +
...
bool CNPC_Hunter::HandleChargeImpact(Vector vecImpact, CBaseEntity *pEntity) {
  ...
  if (!pEntity || pEntity->IsWorld()) {
    ...
    //volume in front of the hunter when we hit the world, and if he is, 		  we hit
    //him anyway.
...
void CNPC_Hunter::HandleAnimEvent(animevent_t *pEvent) {
  ...
  if (pEvent->event == AE_HUNTER_MELEE_ATTACK_LEFT) {
    ...
    MeleeAttack(HUNTER_MELEE_REACH, sk_hunter_dmg_one_slash.GetFloat(), angle,
                dir, HUNTER_BLOOD_LEFT_FOOT);
    return;
  }
...
public:
  void MakeMultipleNPCS(int nNPCs) {
    ...
    DevMsg("Requested %d to spawn, Summoning %d free hunters, spawning %d new "
           "hunters\n",
...
DEFINE_SCHEDULE(
    ...
    "	Tasks"
    "		TASK_SET_FAIL_SCHEDULE			"
    "SCHEDULE:SCHED_HUNTER_FAIL_IMMEDIATE" // used because sched_fail includes a
                                           // one second pause. ick!
    "		TASK_STOP_MOVING					"
    "	0"
...
DEFINE_SCHEDULE(
    ...
    "		TASK_FACE_ENEMY					0"
    "		TASK_SET_ACTIVITY
    ACTIVITY:ACT_IDLE"	// Translated to cover "		TASK_WAIT
    1"
    */
    ""
    "	Interrupts"
...
DEFINE_SCHEDULE(
    ...
    "	Tasks"
    "		TASK_SET_FAIL_SCHEDULE			"
    "SCHEDULE:SCHED_HUNTER_CLEAR_HINTNODE" // used because sched_fail includes a
                                           // one second pause. ick!
