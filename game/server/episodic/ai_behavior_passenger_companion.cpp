...
bool SphereWithinPlayerFOV(CBasePlayer *pPlayer, const Vector &vecCenter,
                           float flRadius);
...
ConVar passenger_collision_response_threshold(
    "passenger_collision_response_threshold", "250.0");
...
CAI_PassengerBehaviorCompanion::CAI_PassengerBehaviorCompanion(void)
    : m_flUnseenDuration(0.0f), m_flNextOverturnWarning(0.0f),
      m_flOverturnedDuration(0.0f), m_nExitAttempts(0),
      m_flNextEnterAttempt(0.0f), m_flLastLateralLean(0.0f),
      m_flNextJostleTime(0.0f) {
  memset(&m_vehicleState, 0, sizeof(m_vehicleState));
  m_VehicleMonitor.ClearMark();
}

void CAI_PassengerBehaviorCompanion::Enable(CPropJeepEpisodic *pVehicle,
...
//-----------------------------------------------------------------------------
void CAI_PassengerBehaviorCompanion::GatherVehicleCollisionConditions(
  ...
  if (localVelocity.y > passenger_collision_response_threshold.GetFloat()) {
    ...
    SinCos(DEG2RAD(m_vehicleState.m_vecLastAngles.z * dt),
           &vecAngularVelocity.y, &vecAngularVelocity.x);

    Vector vecOffset;
    VectorRotate(vecAngularVelocity,
                 m_hVehicle->GetAbsAngles() + QAngle(0, 90, 0), vecOffset);
    ...
    CTraceFilterNoNPCsOrPlayer filter(
        m_hVehicle,
        COLLISION_GROUP_NONE); // We don't care about NPCs or the player
                               // (certainly if they're in the vehicle!)

    trace_t tr;
    UTIL_TraceHull(m_hVehicle->GetAbsOrigin(),
                   m_hVehicle->GetAbsOrigin() + (vForward * distance), mins,
                   maxs, MASK_SOLID, &filter, &tr);
    ...
    if (tr.DidHit()) {
      ...
      if (impactDot < -0.9f && tr.plane.normal.z < 0.75f) {
        ...
        if (tr.m_pEnt) {
          // If it's physical and moveable, then ignore it because we'll
          // probably smash or move it
...
void CAI_PassengerBehaviorCompanion::SpeakVehicleConditions(void) {
  ...
  if (HasCondition(COND_PASSENGER_VEHICLE_STOPPED)) {
    float flDist =
        (WorldSpaceCenter() - m_hVehicle->WorldSpaceCenter()).Length();
  ...
  if (HasCondition(COND_PASSENGER_PLAYER_EXITED_VEHICLE)) {
    CPropJeepEpisodic *pJalopy =
        dynamic_cast<CPropJeepEpisodic *>(m_hVehicle.Get());
...
bool CAI_PassengerBehaviorCompanion::CanPlayJostle(bool bLargeJostle) {
  ...
  if (HasCondition(COND_PASSENGER_HARD_IMPACT) ||
      IsCurSchedule(SCHED_PASSENGER_IMPACT))
    return false;
...
void CAI_PassengerBehaviorCompanion::UpdateStuckStatus(void) {
  ...
  if (m_flOverturnedDuration > MIN_OVERTURNED_WARN_DURATION &&
      m_flNextOverturnWarning < gpGlobals->curtime) {
  ...
  // If we've been overturned for long enough or tried to exit one too many
  // times
...
void CAI_PassengerBehaviorCompanion::GatherConditions(void) {
  ...
  if (GetPassengerState() == PASSENGER_STATE_INSIDE) {
    ...
    if (HasCondition(COND_PASSENGER_ERRATIC_DRIVING)) {
      if (CanPlayJostle(true)) {
        // Add the gesture to be played.  If it's already playing, the
        // underlying function will simply opt-out
        int nSequence =
            GetOuter()->AddGesture(GetOuter()->NPC_TranslateActivity(
                                       ACT_PASSENGER_GESTURE_JOSTLE_LARGE),
                                   true);

        GetOuter()->SetNextAttack(
            gpGlobals->curtime +
            (GetOuter()->SequenceDuration(nSequence) * 2.0f));
        GetOuter()->GetShotRegulator()->FireNoEarlierThan(
            GetOuter()->GetNextAttack());

        // Push out our fidget into the future so that we don't act unnaturally
        // over bumpy terrain
  ...
  if (IsCurSchedule(SCHED_PASSENGER_RUN_TO_ENTER_VEHICLE)) {
    ...
    if (CanEnterVehicleImmediately(&nNearestSequence, &m_vecTargetPosition,
                                   &m_vecTargetAngles)) {
...
//-----------------------------------------------------------------------------
// Purpose: Allow us to deny selecting a schedule if we're not in a state to do
// so
//-----------------------------------------------------------------------------
...
int CAI_PassengerBehaviorCompanion::SelectScheduleInsideVehicle(void) {
  ...
  if (HasCondition(COND_PASSENGER_HARD_IMPACT)) {
    // Push out our fidget into the future so that we don't act unnaturally over
    // bumpy terrain
    ExtendFidgetDelay(random->RandomFloat(1.5f, 3.0f));
  ...
  if (HasCondition(COND_HEAR_DANGER) &&
      IsCurSchedule(SCHED_PASSENGER_COWER) == false) {
  ...
  if (GetEnemy()) {
    ...
    if (HasCondition(COND_NEW_ENEMY) && m_nVisibleEnemies > 1) {
      GetEnemies()->SetTimeValidEnemy(GetEnemy(),
...
int CAI_PassengerBehaviorCompanion::SelectScheduleOutsideVehicle(void) {
  ...
  if (m_PassengerIntent == PASSENGER_INTENT_ENTER) {
    // If we're not attempting to enter the vehicle again, just fall to the base
    // class
    if (m_flNextEnterAttempt > gpGlobals->curtime)
  ...
  // This means that we're outside the vehicle with no intent to enter, which
  // should have disabled us!
  Disable();
  Assert(0);
...
bool SphereWithinPlayerFOV(CBasePlayer *pPlayer, const Vector &vecCenter,
                           float flRadius) {
  // TODO: For safety sake, we might want to do a more fully qualified test
  // against the frustum using the bbox
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CAI_PassengerBehaviorCompanion::CanEnterVehicleImmediately(
    int *pResultSequence, Vector *pResultPos, QAngle *pResultAngles) {
  // Must wait a short time before trying to do this (otherwise we stack up on
  // the player!)
  ...
  // Categorize the passenger in terms of being on the left or right side of the
  // vehicle
  Vector vecRight;
  ...
  Vector vecPassengerOffset =
      (GetOuter()->WorldSpaceCenter() - GetOuter()->GetAbsOrigin());
  ...
  for (int i = 0; i < pEntryAnims->Count(); i++) {
    ...
    if (SphereWithinPlayerFOV(pPlayer, (vecStartPos + vecPassengerOffset),
                              flRadius))
      continue;
    ...
    // We cannot cross between the plane which splits the vehicle laterally in
    // half down the middle This avoids cases where the character magically ends
    // up on one side of the vehicle after they were clearly just on the other
    // side.
...
// Purpose:
//-----------------------------------------------------------------------------
int CAI_PassengerBehaviorCompanion::SelectFailSchedule(
  ...
  case TASK_GET_PATH_TO_VEHICLE_ENTRY_POINT: {
    ...
    if (UTIL_DistApprox(m_hVehicle->GetAbsOrigin(),
...
void CAI_PassengerBehaviorCompanion::EnterVehicle(void) {
  ...
  // Remove this flag because we're sitting so close we always think we're going
  // to hit the player
...
void CAI_PassengerBehaviorCompanion::FinishEnterVehicle(void) {
  ...
  // Push this out into the future so we don't always fidget immediately in the
  // vehicle
...
bool CAI_PassengerBehaviorCompanion::FindPathToVehicleEntryPoint(void) {
  ...
  // bool bFindNearest = ( GetOuter()->m_NPCState == NPC_STATE_COMBAT ||
  // GetOuter()->m_NPCState == NPC_STATE_ALERT );
  bool bFindNearest =
      true; // For the sake of quick gameplay, just make Alyx move directly!
  ...
  if (GetEntryPoint(m_nTransitionSequence, &vecEntryPoint, &vecEntryAngles) ==
      false) {
  ...
  float flDistToGoalSqr =
      (GetOuter()->GetAbsOrigin() - vecEntryPoint).LengthSqr();
  ...
  if (UseRadialRouteToEntryPoint(vecEntryPoint)) {
    ...
    if (GetOuter()->GetNavigator()->SetRadialGoal(vecEntryPoint, vecCenterPoint,
                                                  flRadius, flArc, 64.0f,
                                                  bClockwise) == false) {
      // Try the opposite way
      flArc = 360.0f - flArc;

      // Try the opposite way around
      if (GetOuter()->GetNavigator()->SetRadialGoal(
              vecEntryPoint, vecCenterPoint, flRadius, flArc, 64.0f,
              !bClockwise) == false) {
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CAI_PassengerBehaviorCompanion::CanExitAtPosition(
    const Vector &vecTestPos) {
  CBasePlayer *pPlayer = AI_GetSinglePlayer();
  if (pPlayer == NULL)
    return false;
  ...
  // NOTE: There's no reason to do this since this is only called from a node's
  // reported position Find the exact ground at this position
  // Vector vecGroundPos;
  // if ( FindGroundAtPosition( vecTestPos, 16.0f, 64.0f, &vecGroundPos ) ==
  // false ) 	return false;
  ...
  // FIXME: This piece of code is redundant and happening in
  // IsValidTransitionPoint() as well
  /*
  Vector vecGroundPos;
  if ( FindGroundAtPosition( vecTestPos, GetOuter()->StepHeight(), 64.0f,
  &vecGroundPos ) == false ) return false;
  */
  ...
  // These are very approximated (and magical) numbers to allow passengers
  greater head room and leg room when transitioning Vector vecMins =
  GetOuter()->GetHullMins() + Vector( 0, 0, GetOuter()->StepHeight()*2.0f ); //
  FIXME: Vector vecMaxs = GetOuter()->GetHullMaxs() - Vector( 0, 0,
  GetOuter()->StepHeight() );

  UTIL_TraceHull( GetOuter()->GetAbsOrigin(), vecGroundPos, vecMins, vecMaxs,
  ...
  if ( tr.fraction < 1.0f || tr.allsolid || tr.startsolid )
  {
          ...
          {
                  NDebugOverlay::SweptBox( GetOuter()->GetAbsOrigin(),
  vecGroundPos, vecMins, GetOuter()->GetHullMaxs(), vec3_angle, 255, 0, 0,
  64, 2.0f );
          }
          return false;
  }
...
bool CAI_PassengerBehaviorCompanion::GetStuckExitPos(Vector *vecResult) {
  ...
  for (int i = 0; i <= NUM_EXIT_ITERATIONS - 1; i++) {
    ...
    if (nNearNode != NO_NODE) {
      Vector vecNodePos =
          g_pBigAINet->GetNodePosition(GetOuter()->GetHullType(), nNearNode);
...
//-----------------------------------------------------------------------------
// Purpose: Attempt to get out of an overturned vehicle when the player isn't
// looking Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
...
void CAI_PassengerBehaviorCompanion::StartTask(const Task_t *pTask) {
  ...
  if (GetPassengerState() == PASSENGER_STATE_INSIDE) {
    if (pTask->iTask == TASK_FACE_TARGET || pTask->iTask == TASK_FACE_ENEMY ||
        pTask->iTask == TASK_FACE_IDEAL || pTask->iTask == TASK_FACE_HINTNODE ||
        pTask->iTask == TASK_FACE_LASTPOSITION ||
        pTask->iTask == TASK_FACE_PATH || pTask->iTask == TASK_FACE_PLAYER ||
  ...
  case TASK_GET_PATH_TO_NEAR_VEHICLE: {
    ...
    AI_NavGoal_t goal(GOALTYPE_TARGETENT, vecTargetOffset, AIN_DEF_ACTIVITY,
                      64.0f, AIN_UPDATE_TARGET_POS, m_hVehicle);
    GetOuter()->SetTarget(m_hVehicle);
    if (GetOuter()->GetNavigator()->SetGoal(goal)) {
      TaskComplete();
      return;
...
//-----------------------------------------------------------------------------
// Purpose: Update our path if we're running towards the vehicle (since it can
// move)
//-----------------------------------------------------------------------------
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CAI_PassengerBehaviorCompanion::UseRadialRouteToEntryPoint(
    const Vector &vecEntryPoint) {
  ...
  float flDistToVehicleCenter =
      (vecCenterPos - GetOuter()->GetAbsOrigin()).Length();
...
//			&bClockwise - If we should move clockwise or not to get
//there
// Output : float - degrees around arc to follow
//-----------------------------------------------------------------------------
float CAI_PassengerBehaviorCompanion::GetArcToEntryPoint(
    const Vector &vecCenterPoint, const Vector &vecEntryPoint,
    bool &bClockwise) {
  // We want the entry point to be at the same level as the center to make this
  ...
  float flArcDist =
      UTIL_AngleDistance(flVehicleToEntryYaw, flVehicleToPassengerYaw);
...
//-----------------------------------------------------------------------------
void CAI_PassengerBehaviorCompanion::MarkVehicleEntryFailed(
  ...
  if (passenger_debug_entry.GetBool()) {
    NDebugOverlay::Box(vecPosition, -Vector(8, 8, 8), Vector(8, 8, 8), 255, 0,
                       0, 0, 2.0f);
  }
}
...
// Output : Returns true if the point is near enough another to be considered
// equivalent
//-----------------------------------------------------------------------------
bool CAI_PassengerBehaviorCompanion::PointIsWithinEntryFailureRadius(
    const Vector &vecPosition) {
  // Test this point against our known failed points and reject it if it's too
  // near
  for (int i = 0; i < m_FailedEntryPositions.Count(); i++) {
    ...
    if ((gpGlobals->curtime - m_FailedEntryPositions[i].flTime) > 3.0f) {
      ...
      if (passenger_debug_entry.GetBool()) {
        NDebugOverlay::Box(m_FailedEntryPositions[i].vecPosition,
                           -Vector(12, 12, 12), Vector(12, 12, 12), 255, 255, 0,
                           0, 2.0f);
      }
    ...
    if ((vecPosition - m_FailedEntryPositions[i].vecPosition).LengthSqr() <
        Square(3 * 12)) {
      // Show that this was denied
      if (passenger_debug_entry.GetBool()) {
        NDebugOverlay::Box(m_FailedEntryPositions[i].vecPosition,
                           -Vector(12, 12, 12), Vector(12, 12, 12), 255, 0, 0,
                           128, 2.0f);
      }
...
//-----------------------------------------------------------------------------
int CAI_PassengerBehaviorCompanion::FindEntrySequence(
    bool bNearest /*= false*/) {
  // Get a list of all our animations
  const PassengerSeatAnims_t *pEntryAnims =
      m_hVehicle->GetServerVehicle()->NPC_GetPassengerSeatAnims(
...
//-----------------------------------------------------------------------------
Activity
CAI_PassengerBehaviorCompanion::NPC_TranslateActivity(Activity activity) {
...
bool CAI_PassengerBehaviorCompanion::CanFidget(void) {
  ...
  if (GetOuter()->IsInAScript() ||
      GetOuter()->GetIdealState() == NPC_STATE_SCRIPT ||
...
//-----------------------------------------------------------------------------
bool CAI_PassengerBehaviorCompanion::IsCrouching(void) { return false; }
...
AI_BEGIN_CUSTOM_SCHEDULE_PROVIDER(CAI_PassengerBehaviorCompanion) {
  ...
  DEFINE_SCHEDULE(
      ...
      "SCHEDULE:SCHED_PASSENGER_RUN_TO_ENTER_VEHICLE_FAILED"
      "		TASK_STOP_MOVING				0"
      ...
      "		TASK_SET_SCHEDULE				"
      "SCHEDULE:SCHED_PASSENGER_ENTER_VEHICLE"
      ""
      "	Interrupts"
  ...
  DEFINE_SCHEDULE(
      ...
      "	Tasks"
      "		TASK_SET_FAIL_SCHEDULE			"
      "SCHEDULE:SCHED_PASSENGER_ENTER_VEHICLE_PAUSE"
      "		TASK_STOP_MOVING				0"
