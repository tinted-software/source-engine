...
private:
  ...
  void UpdateAncillaryBeams(float flConvergencePerc, const Vector &vecOrigin,
                            const Vector &vecBasis);
...
CNPC_Combine_Cannon::CNPC_Combine_Cannon(void)
    : m_pBeam(NULL), m_hBarrageTarget(NULL) {
#ifdef _DEBUG
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Combine_Cannon::QuerySeeEntity(CBaseEntity *pEntity,
...
//-----------------------------------------------------------------------------
void CNPC_Combine_Cannon::LaserOn(const Vector &vecTarget,
                                  const Vector &vecDeviance) {
  if (m_pBeam != NULL) {
    m_pBeam->TurnOff();
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Combine_Cannon::GetPaintAim(const Vector &vecStart,
                                      const Vector &vecGoal, float flParameter,
                                      Vector *pProgress) {
  // Quaternions
  Vector vecIdealDir;
...
//			vecBasis -
//-----------------------------------------------------------------------------
void CNPC_Combine_Cannon::UpdateAncillaryBeams(float flConvergencePerc,
                                               const Vector &vecOrigin,
                                               const Vector &vecBasis) {
  // Multiple beams deviate from the basis direction by a certain number of
  // degrees and "converge" at the basis vector over a duration of time, the
  // position in that duration expressed by flConvergencePerc.  The beams are
  // most deviated at 0 and fully converged at 1.

  float flRotationOffset =
      (2 * M_PI) /
      (float)NUM_ANCILLARY_BEAMS; // Degrees separating each beam, in radians
  ...
  for (int i = 0; i < NUM_ANCILLARY_BEAMS; i++) {
    ...
    // Trace a line down that vector to find where we'll eventually stop our
    // line
    UTIL_TraceLine(vecOrigin, vecOrigin + (vecFinal * LINE_LENGTH), MASK_SHOT,
...
//-----------------------------------------------------------------------------
void CNPC_Combine_Cannon::PaintTarget(const Vector &vecTarget,
                                      float flPaintTime) {
...
#define THRESHOLD 0.9f
  ...
  // Find points around that center point and make our designators converge at
...
// Input  : *vecIn -
//-----------------------------------------------------------------------------
void CNPC_Combine_Cannon::AdjustShotPosition(CBaseEntity *pTarget,
                                             Vector *vecIn) {
  if (pTarget == NULL || vecIn == NULL)
    return;

  Vector low = pTarget->WorldSpaceCenter() -
               (pTarget->WorldSpaceCenter() - pTarget->GetAbsOrigin()) * .25;
...
//---------------------------------------------------------
bool CNPC_Combine_Cannon::FireBullet(const Vector &vecTarget,
                                     bool bDirectShot) {
  Vector vecBulletOrigin = GetBulletOrigin();
...
void CNPC_Combine_Cannon::StartTask(const Task_t *pTask) {
  ...
  case TASK_CANNON_PAINT_ENEMY: {
    if (GetEnemy()->IsPlayer()) {
      ...
      // Use a random time
      m_flPaintTime = CANNON_PAINT_ENEMY_TIME +
                      random->RandomFloat(0, CANNON_PAINT_NPC_TIME_NOISE);
...
void CNPC_Combine_Cannon::RunTask(const Task_t *pTask) {
  ...
  case TASK_RANGE_ATTACK1: {
    ...
    Vector vecTarget = (m_hBarrageTarget == NULL)
                           ? m_vecPaintCursor
                           : LeadTarget(m_hBarrageTarget);
    ...
    if (FireBullet(vecTarget, true)) {
      ...
      bool bNoShot = (QuerySeeEntity(m_hBarrageTarget) ==
                      false); // FIXME: Store this info off better
...
bool CNPC_Combine_Cannon::FVisible(CBaseEntity *pEntity, int traceMask,
  ...
  UTIL_TraceLine(EyePosition(), pEntity->EyePosition() + vecEye, MASK_SHOT,
                 this, COLLISION_GROUP_NONE, &tr);
  ...
  if ((pPlayer->GetFlags() & FL_DUCKING) &&
    ...
    UTIL_TraceLine(EyePosition(), vecEye, MASK_SHOT, this, COLLISION_GROUP_NONE,
                   &tr);
