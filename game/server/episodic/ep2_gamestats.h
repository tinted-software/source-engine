...
public:
  ...
  Ep2LevelStats_t(const Ep2LevelStats_t &other) {
    ...
    for (i = other.m_dictEntityDeaths.First();
      ...
      m_dictEntityDeaths.Insert(other.m_dictEntityDeaths.GetElementName(i),
  ...
  void AppendToBuffer(CUtlBuffer &SaveBuffer) {
    ...
    CBaseGameStats::AppendLump(
        EP2_MAX_LUMP_COUNT, SaveBuffer, EP2STATS_LUMP_HEADER, 1,
    ...
    }
    {
      ...
      for (int i = m_dictWeapons.First(); i != m_dictWeapons.InvalidIndex();
           i = m_dictWeapons.Next(i)) {
    ...
    }
    {
      ...
      CBaseGameStats::AppendLump(EP2_MAX_LUMP_COUNT, SaveBuffer,
                                 EP2STATS_LUMP_SAVEGAMEINFO, 1, buf.TellPut(),
                                 buf.Base());
    }
    {
      CUtlBuffer buf;
      ...
      for (int i = m_dictGeneric.First(); i != m_dictGeneric.InvalidIndex();
           i = m_dictGeneric.Next(i)) {
  ...
  static void LoadData(CUtlDict<Ep2LevelStats_t, unsigned short> &items,
    ...
    while (CBaseGameStats::GetLumpHeader(EP2_MAX_LUMP_COUNT, LoadBuffer, iLump,
                                         iLumpCount, true)) {
      switch (iLump) {
      ...
      case EP2STATS_LUMP_TAG: {
        Assert(pItem);
        CBaseGameStats::LoadLump(LoadBuffer, iLumpCount,
                                 sizeof(Ep2LevelStats_t::Tag_t), &pItem->m_Tag);
      } break;
      case EP2STATS_LUMP_DEATH: {
        Assert(pItem);
        pItem->m_aPlayerDeaths.SetCount(iLumpCount);
        CBaseGameStats::LoadLump(
            LoadBuffer, iLumpCount, sizeof(Ep2LevelStats_t::PlayerDeathsLump_t),
