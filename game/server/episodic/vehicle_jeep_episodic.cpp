...
DEFINE_KEYFIELD(m_bDisabled, FIELD_BOOLEAN, "StartDisabled"),
    ...
    DEFINE_INPUTFUNC(FIELD_VOID, "Disable", InputDisable), END_DATADESC();
...
public:
  ...
  static CVehicleCargoTrigger *Create(const Vector &vecOrigin,
                                      const Vector &vecMins,
                                      const Vector &vecMaxs,
                                      CBaseEntity *pOwner) {
    CVehicleCargoTrigger *pTrigger =
        (CVehicleCargoTrigger *)CreateEntityByName("trigger_vehicle_cargo");
    if (pTrigger == NULL)
      return NULL;
  ...
  void CargoTouch(CBaseEntity *pOther) {
    ...
    if ((m_hIgnoreEntity == pOther) ||
        (m_flIgnoreDuration >= gpGlobals->curtime))
      return;
  ...
  bool AddCargo(CBaseEntity *pOther) {
    ...
    CPropJeepEpisodic *pJeep =
        dynamic_cast<CPropJeepEpisodic *>(GetOwnerEntity());
    if (pJeep == NULL)
      return false;
...
DEFINE_FIELD(m_flIgnoreDuration, FIELD_TIME),
    DEFINE_FIELD(m_hIgnoreEntity, FIELD_EHANDLE), DEFINE_ENTITYFUNC(CargoTouch),
    END_DATADESC();
...
LINK_ENTITY_TO_CLASS(info_target_vehicle_transition,
...
DEFINE_FIELD(m_bEntranceLocked, FIELD_BOOLEAN),
    ...
    DEFINE_ARRAY(m_vecRadarContactPos, FIELD_POSITION_VECTOR,
                 RADAR_MAX_CONTACTS),
    ...
    DEFINE_INPUTFUNC(FIELD_VOID, "EnableRadarDetectEnemies",
    ...
    DEFINE_INPUTFUNC(FIELD_VOID, "CreateLinkController",
                     InputCreateLinkController),
    DEFINE_INPUTFUNC(FIELD_VOID, "DestroyLinkController",
                     InputDestroyLinkController),

    DEFINE_INPUTFUNC(FIELD_BOOLEAN, "SetCargoHopperVisibility",
...
SendPropInt(SENDINFO(m_iNumRadarContacts), 8),
    ...
    SendPropArray(SendPropVector(SENDINFO_ARRAY(m_vecRadarContactPos), -1,
                                 SPROP_COORD),
                  m_vecRadarContactPos),

    // CNetworkArray( int, m_iRadarContactType, RADAR_MAX_CONTACTS );
    SendPropArray(SendPropInt(SENDINFO_ARRAY(m_iRadarContactType),
                              RADAR_CONTACT_TYPE_BITS),
                  m_iRadarContactType),
    END_SEND_TABLE()
    ...
    : m_bEntranceLocked(false), m_bExitLocked(false), m_bAddingCargo(false),
...
// Purpose:
//-----------------------------------------------------------------------------
void CPropJeepEpisodic::NPC_FinishedEnterVehicle(CAI_BaseNPC *pPassenger,
                                                 bool bCompanion) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CPropJeepEpisodic::NPC_FinishedExitVehicle(CAI_BaseNPC *pPassenger,
                                                bool bCompanion) {
  // FIXME: This will be moved to the NPCs entering and exiting
  // Fire our outputs
  if (bCompanion) {
...
//-----------------------------------------------------------------------------
bool CPropJeepEpisodic::NPC_CanEnterVehicle(CAI_BaseNPC *pPassenger,
                                            bool bCompanion) {
  // Must be unlocked
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CPropJeepEpisodic::NPC_CanExitVehicle(CAI_BaseNPC *pPassenger,
                                           bool bCompanion) {
  // Must be unlocked
...
//-----------------------------------------------------------------------------
// Purpose: Override velocity if our passenger is transitioning or we're
// upside-down
//-----------------------------------------------------------------------------
Vector CPropJeepEpisodic::PhysGunLaunchVelocity(const Vector &forward,
                                                float flMass) {
  // Disallow
...
void CPropJeepEpisodic::CreateCargoTrigger(void) {
  ...
  if (nAttachment) {
    ...
    // NDebugOverlay::BoxDirection( vecAttachOrigin, vecMins, vecMaxs,
    // vecForward, 255, 0, 0, 64, 4.0f );

    // Create a trigger that lives for a small amount of time
    m_hCargoTrigger =
        CVehicleCargoTrigger::Create(vecAttachOrigin, vecMins, vecMaxs, this);
  }
}
...
// crate instead
//-----------------------------------------------------------------------------
void CPropJeepEpisodic::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
...
void CPropJeepEpisodic::UpdateWheelDust(void) {
  ...
  const vehicle_operatingparams_t *carState =
      m_pServerVehicle->GetVehicleOperatingParams();
  ...
  bool bCreateDust = (bCarOn && bAllowDust &&
                      (m_VehiclePhysics.GetSpeed() >= MIN_WHEEL_DUST_SPEED ||
                       carState->skidSpeed > DEFAULT_SKID_THRESHOLD));
  ...
  for (int i = 0; i < NUM_WHEEL_EFFECTS; i++) {
    ...
    if (bCreateDust) {
      ...
      const vehicle_controlparams_t *vehicleControls =
          m_pServerVehicle->GetVehicleControlParams();
      ...
      if (m_WaterData.m_bWheelInWater[i]) {
        ...
        if (m_flNextWaterSound < gpGlobals->curtime) {
          m_flNextWaterSound =
              gpGlobals->curtime + random->RandomFloat(0.25f, 1.0f);
    ...
    if (pEnt->m_iClassname == iszRadarTarget) {
      ...
      if (pTarget != NULL && !pTarget->IsDisabled()) {
        if (pTarget->m_flRadius < 0 ||
            vecJalopyOrigin.DistToSqr(pTarget->GetAbsOrigin()) <=
                Square(pTarget->m_flRadius)) {
          // This item has been detected.
          type = pTarget->GetType();

          if (type == RADAR_CONTACT_DOG)
            bDetectedDog = true; // used to prevent Alyx talking about the radar
                                 // (see below)
...
void CPropJeepEpisodic::UpdateCargoEntry(void) {
  ...
  if (m_flCargoStartTime + jalopy_cargo_anim_time.GetFloat() <
      gpGlobals->curtime) {
  ...
  // Find where we are in the blend and bias it for a fast entry and slow
  // ease-out
  float flPerc = (jalopy_cargo_anim_time.GetFloat())
                     ? ((gpGlobals->curtime - m_flCargoStartTime) /
                        jalopy_cargo_anim_time.GetFloat())
                     : 1.0f;
  ...
  AngleQuaternion(
      qa, qtTarget); // FIXME: Find the real offset to make this sit properly
  ...
  // Push the closing out into the future to make sure we don't try and close at
  // the same time
...
void CPropJeepEpisodic::CreateAvoidanceZone(void) {
  ...
  CSoundEnt::InsertSound(SOUND_MOVE_AWAY, vecPos, (flHullRadius * 0.4f),
                         VEHICLE_AVOID_BROADCAST_RATE, this);
  // NDebugOverlay::Sphere( vecPos, vec3_angle, flHullRadius*0.4f, 255, 0, 0, 0,
  // true, VEHICLE_AVOID_BROADCAST_RATE );
  ...
  m_flNextAvoidBroadcastTime =
      gpGlobals->curtime + VEHICLE_AVOID_BROADCAST_RATE;
}
...
//-----------------------------------------------------------------------------
// Purpose: If the player is trying to pull the cargo out of the hold using the
// physcannon, let him Output : Returns the cargo to pick up, if all the
// conditions are met
//-----------------------------------------------------------------------------
CBaseEntity *CPropJeepEpisodic::OnFailedPhysGunPickup(Vector vPhysgunPos) {
  ...
  if (m_hCargoProp != NULL) {
    ...
    // We need to make sure the player's position is within a cone near the
    // opening and that they're also facing the right way
    bool bInCargoRange = ((flDist < (15.0f * 12.0f)) &&
                          DotProduct(vecCargoForward, vecPickupDir) < 0.1f);
...
static void SolveBlockingProps(CPropJeepEpisodic *pVehicleEntity,
                               IPhysicsObject *pVehiclePhysics) {
  ...
  IPhysicsFrictionSnapshot *pSnapshot =
      pVehiclePhysics->CreateFrictionSnapshot();
  while (pSnapshot->IsValid()) {
    ...
    if (pOtherEntity && pOtherEntity->GetMoveType() == MOVETYPE_VPHYSICS &&
        pOther->IsMoveable() && (otherMass * 4.0f) < vehicleMass) {
      Vector normal;
      pSnapshot->GetSurfaceNormal(normal);
      // this points down in the car's reference frame, then it's probably
      // trapped under the car
      if (DotProduct(normal, vehicleUp) < -0.9f) {
        Vector point, pointLocal;
        pSnapshot->GetContactPoint(point);
        VectorITransform(point, pVehicleEntity->EntityToWorldTransform(),
                         pointLocal);
        Vector bottomPoint = physcollision->CollideGetExtent(
            pVehiclePhysics->GetCollide(), vec3_origin, vec3_angle,
            Vector(0, 0, -1));
        // make sure it's under the bottom of the car
        float bottomPlane =
            DotProduct(bottomPoint, vehicleUp) + 8; // 8 inches above bottom
...
static void SimpleCollisionResponse(Vector velocityIn, const Vector &normal,
...
static void KillBlockingEnemyNPCs(CBasePlayer *pPlayer,
  ...
  while (pSnapshot->IsValid()) {
    ...
    if (pNPC && pPlayer->IRelationType(pNPC) != D_LI &&
        ((otherMass * 2.0f) < vehicleMass)) {
      ...
      if (index < 0) {
        ...
        if (pNPC->VPhysicsGetObject() &&
            pNPC->VPhysicsGetObject()->GetShadowController() &&
            pNPC->GetMoveType() == MOVETYPE_STEP) {
          if (pNPC->HasDataObjectType(VPHYSICSUPDATEAI)) {
            pUpdate = static_cast<vphysicsupdateai_t *>(
                pNPC->GetDataObject(VPHYSICSUPDATEAI));
            // kill this guy if I've been pushing him for more than half a
            // second and I'm still pushing in his direction
            if ((gpGlobals->curtime - pUpdate->startUpdateTime) > 0.5f &&
                DotProduct(velocity, normal) > 0) {
            ...
            pUpdate = static_cast<vphysicsupdateai_t *>(
                pNPC->CreateDataObject(VPHYSICSUPDATEAI));
          ...
          pNPC->VPhysicsGetObject()->GetShadowController()->MaxSpeed(
              1.0f, maxAngular);
        }
      } else {
  ...
  if (npcList.Count()) {
    for (int i = npcList.Count(); --i >= 0;) {
      ...
      CTakeDamageInfo dmgInfo(pVehicleEntity, pVehicleEntity, damageForce,
                              contactList[i], 200.0f, DMG_CRUSH | DMG_VEHICLE);
      npcList[i]->TakeDamage(dmgInfo);
      pVehiclePhysics->ApplyForceOffset(vehicleForce, contactList[i]);
      PhysCollisionSound(pVehicleEntity, npcList[i]->VPhysicsGetObject(),
                         CHAN_BODY, pVehiclePhysics->GetMaterialIndex(),
                         npcList[i]->VPhysicsGetObject()->GetMaterialIndex(),
                         gpGlobals->frametime, 200.0f);
    }
  }
}

void CPropJeepEpisodic::DriveVehicle(float flFrameTime, CUserCmd *ucmd,
                                     int iButtonsDown, int iButtonsReleased) {
  /* The car headlight hurts perf, there's no timer to turn it off
  automatically, and we haven't built any gameplay around it.
  ...
  if (ucmd->forwardmove != 0.0f) {
    // Msg("Push V: %.2f, %.2f, %.2f\n", ucmd->forwardmove, carState->engineRPM,
    // carState->speed );
...
void CPropJeepEpisodic::CreateHazardLights(void) {
  ...
  for (int i = 0; i < NUM_HAZARD_LIGHTS; i++) {
    if (m_hHazardLights[i] == NULL) {
      m_hHazardLights[i] =
          CSprite::SpriteCreate(s_szHazardSprite, GetLocalOrigin(), false);
      if (m_hHazardLights[i]) {
        m_hHazardLights[i]->SetTransparency(kRenderWorldGlow, 255, 220, 40, 255,
                                            kRenderFxNoDissipation);
        m_hHazardLights[i]->SetAttachment(this,
                                          LookupAttachment(s_szAttach[i]));
  ...
  // Setup our blink
  SetContextThink(&CPropJeepEpisodic::HazardBlinkThink,
                  gpGlobals->curtime + 0.1f, "HazardBlink");
}
...
//-----------------------------------------------------------------------------
// THIS CODE LIFTED RIGHT OUT OF TF2, to defer the pain of making
// vgui-on-an-entity code available to all CBaseAnimating.
//-----------------------------------------------------------------------------
...
void CPropJeepEpisodic::HazardBlinkThink(void) {
  if (m_bBlink) {
    ...
    SetContextThink(&CPropJeepEpisodic::HazardBlinkThink,
                    gpGlobals->curtime + 0.25f, "HazardBlink");
  } else {
    for (int i = 0; i < NUM_HAZARD_LIGHTS; i++) {
      if (m_hHazardLights[i]) {
    ...
    SetContextThink(&CPropJeepEpisodic::HazardBlinkThink,
                    gpGlobals->curtime + 0.5f, "HazardBlink");
  }
...
int CPropJeepEpisodic::DrawDebugTextOverlays(void) {
  ...
  if (m_debugOverlays & OVERLAY_TEXT_BIT) {
    EntityText(
        text_offset,
        CFmtStr("Entrance: %s", m_bEntranceLocked ? "Locked" : "Unlocked"), 0);
    text_offset++;

    EntityText(text_offset,
               CFmtStr("Exit: %s", m_bExitLocked ? "Locked" : "Unlocked"), 0);
    text_offset++;
  }
...
//-----------------------------------------------------------------------------
// Purpose: Teleport the car to a destination that will cause it to transition
// if it's not going to otherwise
//-----------------------------------------------------------------------------
void CPropJeepEpisodic::InputOutsideTransition(inputdata_t &inputdata) {
  ...
  while (
      (pEntity = (CInfoTargetVehicleTransition *)gEntList.FindEntityByClassname(
    ...
    if ((pEntity->GetAbsOrigin() - pPlayer->GetAbsOrigin()).LengthSqr() >
        Square(TRANSITION_SEARCH_RADIUS))
      continue;

    vecTeleportPos = pEntity->GetAbsOrigin();
    vecTeleportAngles =
        pEntity->GetAbsAngles() +
        QAngle(0, -90, 0); // Vehicle is always off by 90 degrees
    ...
    trace_t tr;
    UTIL_TraceHull(vecTeleportPos, vecTeleportPos - Vector(0, 0, 128), vecMins,
                   vecMaxs, MASK_SOLID, this, COLLISION_GROUP_NONE, &tr);
...
void CPropJeepEpisodic::InputCreateLinkController(inputdata_t &data) {
  ...
  CAI_RadialLinkController *pLinkController =
      (CAI_RadialLinkController *)CreateEntityByName(
  ...
  if (pLinkController != NULL && m_hLinkControllerFront.Get() == NULL) {
    ...
    // NDebugOverlay::Circle( vecFront, Vector(0,1,0), Vector(1,0,0), flRadius,
    // 255, 255, 255, 128, false, 100 );
  }

  pLinkController = (CAI_RadialLinkController *)CreateEntityByName(
      "info_radial_link_controller");
  if (pLinkController != NULL && m_hLinkControllerRear.Get() == NULL) {
    ...
    // NDebugOverlay::Circle( vecRear, Vector(0,1,0), Vector(1,0,0), flRadius,
    // 255, 255, 255, 128, false, 100 );
  }
}
...
bool CPropJeepEpisodic::AllowBlockedExit(CBaseCombatCharacter *pPassenger,
                                         int nRole) {
  // Wait until we've settled down before we resort to blocked exits.
  // This keeps us from doing blocked exits in mid-jump, which can cause mayhem
  // like sticking the player through player clips or into geometry.
  return GetSmoothedVelocity().IsLengthLessThan(
      jalopy_blocked_exit_max_speed.GetFloat());
}
