...
ConVar striderbuster_shot_velocity(
    "striderbuster_shot_velocity", "2500.0", FCVAR_NONE,
...
ConVar striderbuster_allow_all_damage(
    ...
    "If set to '1' the bomb will detonate on any damage taken.  Otherwise only "
    "the player may trigger it.");

// ConVar
// striderbuster_magnetic_radius("striderbuster_magnetic_radius","400.0f",
// FCVAR_NONE,"Maximum distance at which magnade experiences attraction to a
// target. Set to 0 to disable magnetism.");
ConVar striderbuster_magnetic_force_strider(
    "striderbuster_magnetic_force_strider", "750000.0f", FCVAR_NONE,
    "Intensity of magnade's attraction to a strider.");
ConVar striderbuster_magnetic_force_hunter(
    "striderbuster_magnetic_force_hunter", "1750000.0f", FCVAR_NONE,
    "Intensity of magnade's attraction to a hunter.");
ConVar striderbuster_falloff_power(
    "striderbuster_falloff_power", "4", FCVAR_NONE,
...
ConVar striderbuster_leg_stick_dist(
    ...
    "If the buster hits a strider's leg, the max distance from the head at "
    "which it sticks anyway.");
ConVar striderbuster_debugseek("striderbuster_debugseek", "0");

ConVar sk_striderbuster_magnet_multiplier("sk_striderbuster_magnet_multiplier",
                                          "2.25");

ConVar striderbuster_die_detach(
    "striderbuster_die_detach",
    "1"); // Drop off the strider if a hunter shoots me. (Instead of exploding)
ConVar
    striderbuster_dive_force("striderbuster_dive_force",
                             "-200"); // How much force to apply to a nosediving
                                      // (dead in the air) striderbuster

ConVar striderbuster_use_particle_flare("striderbuster_use_particle_flare",
                                        "1");
...
#define BUSTER_PING_SOUND_FREQ                                                 \
  3.0f // How often (seconds) to issue the ping sound to remind players we are
       // attached
...
public:
  ...
  } // I added a flags field so we don't have to keep added bools for all of
    // these contingencies (sjb)
...
private:
  ...
  bool m_bDud;
  bool m_bLaunched;
  bool m_bNoseDiving; // No magnetism, nosedive and break. Hunter flechettes set
                      // this.
  ...
  friend bool StriderBuster_IsAttachedStriderBuster(CBaseEntity *pEntity,
                                                    CBaseEntity *);
};
...
DEFINE_KEYFIELD(m_bDud, FIELD_BOOLEAN, "dud"),
    ...
    DEFINE_ENTITYFUNC(BusterTouch), DEFINE_THINKFUNC(BusterFlyThink),
    DEFINE_THINKFUNC(BusterDetachThink), DEFINE_THINKFUNC(BusterPingThink),
    END_DATADESC()

        CWeaponStriderBuster::CWeaponStriderBuster(void)
    : m_pConstraint(NULL), m_flCollisionSpeedSqr(-1.0f),
      m_hConstrainedEntity(NULL), m_nAttachedBoneFollowerIndex(-1) {}
...
void CWeaponStriderBuster::OnRestore(void) {
  ...
  // If we have an entity we're attached to, attempt to reconstruct our bone
  // follower setup
  if (m_hConstrainedEntity != NULL) {
    CNPC_Strider *pStrider =
        dynamic_cast<CNPC_Strider *>(m_hConstrainedEntity.Get());
    if (pStrider != NULL) {
      // Make sure we've done this step or we'll have no controller to attach to
      pStrider->InitBoneFollowers();

      // Attempt to make a connection to the same bone follower we attached to
      // previously
      CBoneFollower *pBoneFollower =
          pStrider->GetBoneFollowerByIndex(m_nAttachedBoneFollowerIndex);
...
bool CWeaponStriderBuster::CreateConstraintToObject(CBaseEntity *pObject) {
  ...
  IPhysicsConstraint *pConstraint = physenv->CreateFixedConstraint(
      pPhysObject, pMyPhysObject, NULL, fixedConstraint);
  if (pConstraint == NULL)
    return false;
...
bool CWeaponStriderBuster::ShouldStickToEntity(CBaseEntity *pEntity) {
  ...
  // Ick, this is kind of ugly, but it's also ugly having to pass pointer into
  // this to avoid multiple castings! Save this to patch up save/restore later
  ...
  if (m_flCollisionSpeedSqr > 50 && !m_bNoseDiving) {
    ...
    if (ShouldStickToEntity(pOther)) {
      ...
      if (CreateConstraintToObject(pOther)) {
        ...
        DispatchParticleEffect("striderbuster_attach", GetAbsOrigin(),
                               GetAbsAngles(), NULL);

        if (striderbuster_use_particle_flare.GetBool()) {
          // We don't have to save any pointers or handles to this because it's
          // parented to the buster. So it will die when the buster dies. Yay.
          CParticleSystem *pFlare =
              (CParticleSystem *)CreateEntityByName("info_particle_system");
          ...
          m_hGlowSprite = CSprite::SpriteCreate("sprites/orangeflare1.vmt",
                                                GetLocalOrigin(), false);

          Assert(m_hGlowSprite);
          if (m_hGlowSprite != NULL) {
            m_hGlowSprite->TurnOn();
            m_hGlowSprite->SetTransparency(kRenderWorldGlow, 255, 255, 255, 255,
        ...
        SetContextThink(&CWeaponStriderBuster::BusterPingThink,
                        gpGlobals->curtime + BUSTER_PING_SOUND_FREQ,
...
void CWeaponStriderBuster::CreateDestroyedEffect(void) {
  ...
  DispatchParticleEffect("striderbuster_explode_core", GetAbsOrigin(),
                         GetAbsAngles());
...
//-----------------------------------------------------------------------------
void CWeaponStriderBuster::VPhysicsCollision(int index,
  ...
  if (pVictim->IsWorld() &&
      ((m_bDud && m_bLaunched) || m_flCollisionSpeedSqr > Square(500))) {
  ...
  bool bVictimIsStrider =
      ((pOwnerEntity != NULL) && FClassnameIs(pOwnerEntity, "npc_strider"));
...
void CWeaponStriderBuster::Detonate(void) {
  ...
  if (!m_bDud && pVictim) {
    ...
    CTakeDamageInfo info(pPlayer, this, RandomVector(-100.0f, 100.0f),
                         GetAbsOrigin(), pVictim->GetHealth(), DMG_GENERIC);
    pVictim->TakeDamage(info);

    gamestats->Event_WeaponHit(ToBasePlayer(pPlayer), true, GetClassname(),
                               info);

    // Tracker 62293:  There's a bug where the inflictor/attacker are reversed
    // when calling TakeDamage above so the player never gets
    //  credit for the strider buster kills.  The code has a bunch of
    //  assumptions lower level, so it's safer to just fix it here by crediting
    //  a kill to the player directly.
  ...
  if (!m_bDud) {
    ...
    DispatchParticleEffect("striderbuster_explode_dummy_core", GetAbsOrigin(),
                           GetAbsAngles());
...
int CWeaponStriderBuster::OnTakeDamage(const CTakeDamageInfo &info) {
  ...
  if (striderbuster_allow_all_damage.GetBool() ||
      (IsAttachedToStrider() && (bAttackerIsPlayer || bInflictorIsPlayer))) {
    Detonate();
    return 0;
  }

  if (pAttacker && (pAttacker->Classify() == CLASS_COMBINE ||
                    pAttacker->Classify() == CLASS_COMBINE_HUNTER)) {
  ...
  if (hunter_hate_held_striderbusters.GetBool() ||
    ...
    if ((GetHealth() > 0) && (pInflictor != NULL) &&
        FClassnameIs(pInflictor, "hunter_flechette")) {
      //
      // Flechette impacts don't hurt the striderbuster unless it's attached to
      // a strider, but the explosions always do. This is so that held or thrown
      // striderbusters fly awry because of the flechette, but attached
      // striderbusters break instantly to make the hunters more effective at
      // defending the strider.
      //
      if (IsAttachedToStrider() || !(info.GetDamageType() & DMG_NEVERGIB)) {
        if (striderbuster_die_detach.GetBool() && IsAttachedToStrider()) {
          ...
          CNPC_Strider *pStrider =
              dynamic_cast<CNPC_Strider *>(GetOwnerEntity());
          ...
          EmitSound("Weapon_StriderBuster.Dud_Detonate");
          DispatchParticleEffect("striderbuster_break_flechette",
      ...
      if (info.GetDamage() < 5) {
        bool bFirst = (m_CarryAngles.x == 45 && m_CarryAngles.y == 0 &&
                       m_CarryAngles.z == 0);
        float sinTime = sin(gpGlobals->curtime);
        bool bSubtractX = (bFirst) ? (sinTime < 0) : (m_CarryAngles.x < 45);

        m_CarryAngles.x +=
            (10.0 + 10.0 * fabsf(sinTime) + random->RandomFloat(-2.5, 2.5) +
             random->RandomFloat(-2.5, 2.5)) *
            ((bSubtractX) ? -1.0 : 1.0);
        m_CarryAngles.y =
            15 * (sin(gpGlobals->curtime) + cos(gpGlobals->curtime * 0.5)) *
                .5 +
            random->RandomFloat(-15, 15);
        m_CarryAngles.z =
            7.5 * (sin(gpGlobals->curtime) + sin(gpGlobals->curtime * 2.0)) *
                .5 +
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CWeaponStriderBuster::OnPhysGunPickup(CBasePlayer *pPhysGunUser,
                                           PhysGunPickup_t reason) {
  m_PickupTime = gpGlobals->curtime;
  m_CarryAngles.Init(45, 0, 0);
  if ((reason == PICKED_UP_BY_CANNON) &&
      (!HasSpawnFlags(SF_DONT_WEAPON_MANAGE))) {
    WeaponManager_RemoveManaged(this);
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CWeaponStriderBuster::OnPhysGunDrop(CBasePlayer *pPhysGunUser,
                                         PhysGunDrop_t Reason) {
  if (Reason == LAUNCHED_BY_CANNON) {
    Launch(pPhysGunUser);
  } else if ((Reason == DROPPED_BY_CANNON) &&
             (!HasSpawnFlags(SF_DONT_WEAPON_MANAGE))) {
...
void CWeaponStriderBuster::Launch(CBasePlayer *pPhysGunUser) {
  ...
  m_hMainGlow =
      CSprite::SpriteCreate("sprites/blueglow1.vmt", GetLocalOrigin(), false);

  if (m_hMainGlow != NULL) {
    m_hMainGlow->FollowEntity(this);
    m_hMainGlow->SetTransparency(kRenderGlow, 255, 255, 255, 140,
                                 kRenderFxNoDissipation);
  ...
  if (!m_bNoseDiving) {
    DispatchParticleEffect("striderbuster_trail", PATTACH_ABSORIGIN_FOLLOW,
                           this);
  } else {
    DispatchParticleEffect("striderbuster_shotdown_trail",
...
// Output : Vector
//-----------------------------------------------------------------------------
Vector CWeaponStriderBuster::PhysGunLaunchVelocity(const Vector &forward,
                                                   float flMass) {
...
void CWeaponStriderBuster::Shatter(CBaseEntity *pAttacker) {
  ...
  if (!IsAttachedToStrider()) {
    // Don't display this particular effect if we're attached to a strider. This
    // effect just gets lost in the big strider explosion anyway, so let's
    // recover some perf.
    DispatchParticleEffect("striderbuster_break", GetAbsOrigin(),
                           GetAbsAngles());
  }
  ...
  CTakeDamageInfo info(pAttacker, pAttacker, RandomVector(-100, 100),
                       GetAbsOrigin(), 100.0f, DMG_CRUSH);
  TakeDamage(info);
}
...
void CWeaponStriderBuster::BusterFlyThink() {
  ...
  if (m_bNoseDiving) {
    if (VPhysicsGetObject())
      VPhysicsGetObject()->ApplyForceCenter(
          Vector(0, 0, striderbuster_dive_force.GetFloat()));
    SetNextThink(gpGlobals->curtime + 0.01f);
    return;
  }
  ...
  const float magradius =
      38.0 * sk_striderbuster_magnet_multiplier
                 .GetFloat(); // radius of strider hull times multiplier
  if (magradius > 0 && GetMoveType() == MOVETYPE_VPHYSICS &&
      VPhysicsGetObject()) {
    ...
    for (int ii = 0; ii < count; ++ii) {
      ...
      if (pStrider &&
          !pStrider->CarriedByDropship()) // ShouldStickToEntity() doesn't work
                                          // because the strider NPC isn't what
                                          // we glue to
      {
        // get distance squared
        VectorSubtract(pStrider->GetAdjustedOrigin(), GetAbsOrigin(), toTarget);

        // NDebugOverlay::Line( GetAbsOrigin(), GetAbsOrigin() + toTarget, 128,
        // 0, 128, false, 0.1 );
    ...
    if (bestFit >= 0) // we found something and should attract towards it.
                      // (hysterisis later?)
    {
      if (striderbuster_debugseek.GetBool()) {
        NDebugOverlay::Circle(GetAbsOrigin() + toTarget, magradius, 255, 255,
                              255, 255, true, .1);
        NDebugOverlay::Cross3D(GetAbsOrigin() + toTarget, magradius, 255, 255,
                               255, true, .1);
      }

      // force magnitude.
      float magnitude =
          GetMass() * striderbuster_magnetic_force_strider.GetFloat();
      ...
      case 1:
        VPhysicsGetObject()->ApplyForceCenter(
            toTarget *
            (magnitude /
             nearestDistSq)); // dividing through by distance squared normalizes
      ...
      case 2:
        VPhysicsGetObject()->ApplyForceCenter(
            ...
            (magnitude /
             (nearestDistSq *
              sqrtf(nearestDistSq)))); // dividing through by distance cubed
                                       // normalizes toTarget and gives a
                                       // quadratic falloff
        break;
      case 3:
        VPhysicsGetObject()->ApplyForceCenter(
            ...
            (magnitude /
             (nearestDistSq *
              nearestDistSq))); // dividing through by distance fourth
      ...
      default: // arbitrary powers
        VPhysicsGetObject()->ApplyForceCenter(
            toTarget * (magnitude *
                        powf(nearestDistSq,
                             (falloff + 1.0f) /
                                 2))); // square root for distance instead of
                                       // squared, add one to normalize toTarget
        break;
      }
    }
...
void CWeaponStriderBuster::BusterDetachThink() {
  ...
  trace_t tr;
  UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() - Vector(0, 0, 1200),
                 MASK_SOLID_BRUSHONLY, this, COLLISION_GROUP_NONE, &tr);

  if (fabs(tr.startpos.z - tr.endpos.z) < 240.0f) {
    SetThink(NULL);
    EmitSound("Weapon_StriderBuster.Dud_Detonate");
    DispatchParticleEffect("striderbuster_break_flechette", GetAbsOrigin(),
                           GetAbsAngles());
    SetHealth(0);
  ...
  if (!GetParent() || !GetParent()->ClassMatches(g_iszVehicle)) {
    if (!m_bNoseDiving) {
      ...
      if (m_iBusterFlags & STRIDERBUSTER_FLAG_KNOCKED_OFF_STRIDER) {
        ...
        DispatchParticleEffect("striderbuster_flechette_attached",
                               PATTACH_ABSORIGIN_FOLLOW, this);
      }
    }
...
bool StriderBuster_IsAttachedStriderBuster(CBaseEntity *pEntity,
  ...
  if (!pAttachedTo)
    return static_cast<CWeaponStriderBuster *>(pEntity)->m_hConstrainedEntity !=
           NULL;
  else
    return static_cast<CWeaponStriderBuster *>(pEntity)->m_hConstrainedEntity ==
           pAttachedTo;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool StriderBuster_OnFlechetteAttach(CBaseEntity *pEntity,
                                     Vector &vecFlechetteVelocity) {
  CWeaponStriderBuster *pBuster = dynamic_cast<CWeaponStriderBuster *>(pEntity);
  if (pBuster) {
...
bool StriderBuster_WasKnockedOffStrider(CBaseEntity *pEntity) {
  CWeaponStriderBuster *pBuster = dynamic_cast<CWeaponStriderBuster *>(pEntity);
  if (pBuster) {
    return ((pBuster->GetStriderBusterFlags() &
             STRIDERBUSTER_FLAG_KNOCKED_OFF_STRIDER) != 0);
  }
