...
#define MAX_DIST_FROM_FOLLOW_TARGET                                            \
  256 // If the follow target is farther than this, the NPC will run to it
...
void CAI_BehaviorAlyxInjured::PopulateActivityMap(void) {
  ...
  ActivityMap_t map[] = {
      ...
      {ACT_RUN_AIM_RELAXED,
       ACT_RUN_AIM_RELAXED}, // FIMXE: No appropriate temp anim right now!
      {ACT_RUN_AIM_STIMULATED,
       ACT_RUN_AIM_STIMULATED}, // FIMXE: No appropriate temp anim right now!
      {ACT_RUN_AIM_AGITATED,
       ACT_RUN_AIM_AGITATED}, // FIMXE: No appropriate temp anim right now!
...
//-----------------------------------------------------------------------------
void CAI_BehaviorAlyxInjured::OnRestore(void) { PopulateActivityMap(); }

//-----------------------------------------------------------------------------
// Purpose: Populate the list on spawn
//-----------------------------------------------------------------------------
void CAI_BehaviorAlyxInjured::Spawn(void) { PopulateActivityMap(); }
...
// Output : Activity
//-----------------------------------------------------------------------------
Activity CAI_BehaviorAlyxInjured::GetFlinchActivity(bool bHeavyDamage,
                                                    bool bGesture) {
  //
...
bool CAI_BehaviorAlyxInjured::ShouldRunToCover(void) {
  ...
  if (FindCoverFromEnemyBehindTarget(GetFollowTarget(), flRetreatRadius,
                                     &vecRetreatPos)) {
    float flDestDistSqr =
        (GetOuter()->WorldSpaceCenter() - vecRetreatPos).LengthSqr();
...
bool CAI_BehaviorAlyxInjured::ShouldRunToFollowGoal(void) {
  ...
  float flDistToFollowGoalSqr =
      (GetOuter()->GetAbsOrigin() - GetFollowTarget()->GetAbsOrigin())
          .LengthSqr();
...
//-----------------------------------------------------------------------------
int CAI_BehaviorAlyxInjured::SelectFailSchedule(
...
//-----------------------------------------------------------------------------
bool CAI_BehaviorAlyxInjured::FindThreatDirection2D(const Vector &vecSource,
                                                    Vector *vecOut) {
  ...
  for (AI_EnemyInfo_t *pMemory = GetOuter()->GetEnemies()->GetFirst(&iter);
       pMemory != NULL; pMemory = GetOuter()->GetEnemies()->GetNext(&iter)) {
...
//			*vecOut - position
//-----------------------------------------------------------------------------
bool CAI_BehaviorAlyxInjured::FindCoverFromEnemyBehindTarget(
    CBaseEntity *pTarget, float flRadius, Vector *vecOut) {
  if (pTarget == NULL)
    return false;
  ...
  if (FindThreatDirection2D(vecTargetPos, &vecThreatDir)) {
    ...
    if (g_debug_injured_follow.GetBool()) {
      NDebugOverlay::HorzArrow(GetOuter()->GetAbsOrigin(), vecTestPos, 8.0f,
                               255, 255, 0, 32, true, 2.0f);
    }
    ...
    GetOuter()->GetMoveProbe()->MoveLimit(
        NAV_GROUND, GetOuter()->GetAbsOrigin(), vecTestPos,
        MASK_SOLID_BRUSHONLY, NULL, 0, &moveTrace);

    bool bWithinRangeToGoal =
        (moveTrace.vEndPosition - vecTestPos).Length2DSqr() <
        Square(GetOuter()->GetHullWidth() * 3.0f);
    bool bCanStandAtGoal = GetOuter()->GetMoveProbe()->CheckStandPosition(
        moveTrace.vEndPosition, MASK_SOLID_BRUSHONLY);

    if (bWithinRangeToGoal == false || bCanStandAtGoal == false) {
      if (g_debug_injured_follow.GetBool()) {
        NDebugOverlay::SweptBox(
            GetOuter()->GetAbsOrigin(), vecTestPos, GetOuter()->GetHullMins(),
    ...
    *vecOut = moveTrace.vEndPosition;
    ...
    if (g_debug_injured_follow.GetBool()) {
      NDebugOverlay::SweptBox(
          GetOuter()->GetAbsOrigin(), (*vecOut), GetOuter()->GetHullMins(),
...
void CAI_BehaviorAlyxInjured::StartTask(const Task_t *pTask) {
  ...
  case TASK_FIND_COVER_FROM_ENEMY: {
    ...
    if (FindCoverFromEnemyBehindTarget(pLeader, COVER_DISTANCE, &coverPos)) {
      AI_NavGoal_t goal(GOALTYPE_LOCATION, coverPos, ACT_RUN,
...
void CAI_BehaviorAlyxInjured::GatherConditions(void) {
  ...
  if (NumKnownEnemiesInRadius(GetOuter()->GetAbsOrigin(), COVER_DISTANCE) >=
      MIN_ENEMY_MOB) {
  ...
  if (AI_IsSinglePlayer() && bInDanger) {
    ...
    if (pPlayer != NULL) {
      // FIXME: This distance may need to be the length of the shortest walked
      // path between the follower and the target

      // Get our approximate distance to the player
      float flDistToPlayer = UTIL_DistApprox2D(GetOuter()->GetAbsOrigin(),
    ...
    // Yell for help!
    if (bWarnPlayer) {
      // FIXME: This should be routed through the normal speaking code with a
      // system to emit from the player's suit.

      CBasePlayer *pPlayer = UTIL_PlayerByIndex(1);
      // float flPlayerDistSqr = ( GetOuter()->GetAbsOrigin() -
      // pPlayer->GetAbsOrigin() ).LengthSqr();

      // If the player is too far away or we can't see him
      // if ( HasCondition( COND_SEE_PLAYER ) == false || flPlayerDistSqr >
      // Square( 128 ) )
      {
        if (m_flNextWarnTime < gpGlobals->curtime) {
          pPlayer->EmitSound("npc_alyx.injured_too_far");
          m_flNextWarnTime =
              gpGlobals->curtime + random->RandomFloat(3.0f, 5.0f);
        }
      }
      /*
      else
      {
              ...
              m_flNextWarnTime = gpGlobals->curtime +
...
//-----------------------------------------------------------------------------
int CAI_BehaviorAlyxInjured::NumKnownEnemiesInRadius(const Vector &vecSource,
                                                     float flRadius) {
  int nNumEnemies = 0;
  ...
  for (AI_EnemyInfo_t *pMemory = GetEnemies()->GetFirst(&iter); pMemory != NULL;
    ...
    if (GetOuter()->IRelationType(pMemory->hEnemy) != D_HT &&
        GetOuter()->IRelationType(pMemory->hEnemy) != D_FR)
      continue;
    ...
    float flEnemyDistSqr =
        (vecSource - pMemory->hEnemy->GetAbsOrigin()).Length2DSqr();
...
AI_BEGIN_CUSTOM_SCHEDULE_PROVIDER(CAI_BehaviorAlyxInjured) {
  ...
  DEFINE_SCHEDULE(SCHED_INJURED_FEAR_FACE,
                  ...
                  "		 TASK_STOP_MOVING			0"
                  "		 TASK_SET_ACTIVITY			"
                  "ACTIVITY:ACT_IDLE" // FIXME: Scared idle?
                  "		 TASK_FACE_ENEMY			0"
                  ""
                  "	Interrupts"
                  "		COND_GIVE_WAY"
