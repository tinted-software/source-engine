...
public:
  ...
  void OnSpawnedNPC(CAI_BaseNPC *pNPC); // Notify this spawn destination that an
                                        // NPC has spawned here.
...
public:
  ...
  virtual void DeathNotice(CBaseEntity *
                           pChild); // NPC maker children use this to tell the
                                    // NPC maker that they have died.
  ...
  int m_nLiveChildren;    // how many NPCs made by this NPC maker that are
                          // currently alive
  int m_nMaxLiveChildren; // max number of NPCs that this maker may have out at
                          // one time.
