...
int CBaseTransmitProxy::ShouldTransmit(const CCheckTransmitInfo *pInfo,
                                       int nPrevShouldTransmitResult) {
  // Anyone implementing a transmit proxy should override this since that's the
  // point!!
  Assert(false);
  return FL_EDICT_DONTSEND;
}

void CBaseTransmitProxy::AddRef() { m_refCount++; }

void CBaseTransmitProxy::Release() {
  if (m_refCount == 0xFFFF) {
    // This means we are inside our destructor already, so we don't want to do
    // anything here.
