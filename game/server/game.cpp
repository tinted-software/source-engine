...
void MapCycleFileChangedCallback(IConVar *var, const char *pOldString,
                                 float flOldValue) {
...
ConVar mapcyclefile(
    "mapcyclefile", "mapcycle.txt", FCVAR_NONE,
...
public:
  virtual bool RegisterConCommandBase(ConCommandBase *pCommand) {
    ...
    if (pValue) {
      ...
      // NOTE:  If not overridden at the command line, then if it's a replicated
      // cvar, make sure that it's
      //  value is the server's value.  This solves a problem where think_limit
      //  is defined in shared code but the value is inside and #if defined(
      //  _DEBUG ) block and if you have a debug game .dll and a release client,
      //  then the limiit was coming from the client even though the server
      //  value was the one that was important during debugging.  Now the server
      //  trumps the client value for replicated ConVars by setting the value
      //  here after the ConVar has been linked.
