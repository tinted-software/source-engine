...
public:
  ...
  void Init(FlexWeight_t &index, const char *szName) {
    ...
    if (!IsServerSideFlexController(szName)) {
      Error("You forgot to add flex controller %s to list in "
...
public:
  CAI_BaseActor()
      : m_fLatchedPositions(0), m_latchedEyeOrigin(vec3_origin),
        m_latchedEyeDirection(vec3_origin), m_latchedHeadDirection(vec3_origin),
        m_flBlinktime(0), m_hLookTarget(NULL),
        m_iszExpressionScene(NULL_STRING), m_iszIdleExpression(NULL_STRING),
        m_iszAlertExpression(NULL_STRING), m_iszCombatExpression(NULL_STRING),
...
private:
  ...
  // // "body_trans_X" PoseParameter_t			m_ParameterBodyLift;
  // // "body_lift"
  ...
  // "spine_pitch" PoseParameter_t			m_ParameterSpineRoll;
  // // "spine_roll"
  ...
  // "move_rightleft" FlexWeight_t
  // m_FlexweightMoveForwardBack;// "move_forwardback" FlexWeight_t
  ...
  // "body_updown" FlexWeight_t			m_FlexweightBodyTilt;
  // // "body_tilt"
  FlexWeight_t m_FlexweightChestRightLeft; // "chest_rightleft"
  // FlexWeight_t			m_FlexweightChestUpDown;	//
  // "chest_updown" FlexWeight_t			m_FlexweightChestTilt;
  // // "chest_tilt"
