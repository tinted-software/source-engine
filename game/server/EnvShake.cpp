...
DEFINE_FIELD(m_force, FIELD_VECTOR), END_DATADESC()
...
DEFINE_KEYFIELD(m_Amplitude, FIELD_FLOAT, "amplitude"),
    ...
    DEFINE_FIELD(m_maxForce, FIELD_VECTOR), DEFINE_PHYSPTR(m_pShakeController),
...
void CEnvShake::Spawn(void) {
  ...
  if (HasSpawnFlags(SF_SHAKE_NO_VIEW) && !HasSpawnFlags(SF_SHAKE_PHYSICS) &&
    ...
    DevWarning("env_shake %s with \"Don't shake view\" spawnflag set without "
               "\"Shake physics\" or \"Shake ropes\" spawnflags set.",
               GetDebugName());
  }
}
