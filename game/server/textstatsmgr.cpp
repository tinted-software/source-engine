...
// ------------------------------------------------------------------------------------------
// //
CTextStatsMgr::CTextStatsMgr(void) { m_szStatFilename[0] = 0; }
...
bool CTextStatsMgr::WriteFile(IFileSystem *pFileSys, const char *pFilename) {
  ...
  for (CTextStatFile *pCurFile = CTextStatFile::s_pHead; pCurFile;
...
char *CTextStatsMgr::GetStatsFilename(void) { return m_szStatFilename; }
...
CTextStat::CTextStat(TextStatPrintFn printFn, void *pUserData,
                     CTextStatsMgr *pMgr) {
  m_pPrev = m_pNext = this;
  Init(printFn, pUserData, pMgr);
}

CTextStat::~CTextStat() { Term(); }

void CTextStat::Init(TextStatPrintFn printFn, void *pUserData,
                     CTextStatsMgr *pMgr) {
  Term();
...
CTextStatInt::CTextStatInt(const char *pName, int initialValue,
                           CTextStatsMgr *pMgr) {
  m_pName = pName;
...
void CTextStatInt::PrintFn(IFileSystem *pFileSys, FileHandle_t hFile,
                           void *pUserData) {
