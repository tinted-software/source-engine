...
bool CFuncBrush::CreateVPhysics(void) {
  // NOTE: Don't init this static.  It's pretty common for these to be
  // constrained and dynamically parented.  Initing shadow avoids having to
  // destroy the physics object later and lose the constraints.
...
//-----------------------------------------------------------------------------
void CFuncBrush::InputTurnOff(inputdata_t &inputdata) { TurnOff(); }

//-----------------------------------------------------------------------------
// Purpose: Input handler for showing the brush.
//-----------------------------------------------------------------------------
void CFuncBrush::InputTurnOn(inputdata_t &inputdata) { TurnOn(); }
...
bool CFuncBrush::IsOn(void) { return !IsEffectActive(EF_NODRAW); }

//-----------------------------------------------------------------------------
// Purpose: Invisible field that activates when touched
//			All inputs are passed up to the main entity, unless
//filtered out
//-----------------------------------------------------------------------------
...
public:
  ...
  // input functions
  void InputEnable(inputdata_t &inputdata) { RemoveFlag(FL_DONTTOUCH); }

  void InputDisable(inputdata_t &inputdata) {
    // this ensures that all the remaining EndTouch() calls still get passed
    // through
...
DEFINE_KEYFIELD(m_iInputFilter, FIELD_INTEGER, "InputFilter"),
    ...
    DEFINE_OUTPUT(m_OnEndTouch, "OnEndTouch"), DEFINE_OUTPUT(m_OnUse, "OnUse"),
...
void CTriggerBrush::StartTouch(CBaseEntity *pOther) {
  if (PassesInputFilter(pOther, m_iInputFilter) &&
      !(m_iInputFilter & TRIGGER_IGNORETOUCH)) {
...
//-----------------------------------------------------------------------------
// Purpose: Called when we are triggered by another entity or used by the
// player. Input  : pActivator -
//			pCaller -
//			useType -
//			value -
//-----------------------------------------------------------------------------
void CTriggerBrush::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
                        USE_TYPE useType, float value) {
  if (PassesInputFilter(pActivator, m_iInputFilter) &&
      !(m_iInputFilter & TRIGGER_IGNOREUSE)) {
...
// Input  : *pOther - the entity that is part of the input
// (touch/untouch/block/use)
...
bool CTriggerBrush::PassesInputFilter(CBaseEntity *pOther, int filter) {
  ...
  // NPCs
  if ((filter & TRIGGER_IGNORENPCS) && pOther->edict() &&
      (pOther->GetFlags() & FL_NPC))
    return false;

  // pushables
  if ((filter & TRIGGER_IGNOREPUSHABLES) &&
      FStrEq(STRING(pOther->m_iClassname), "func_pushable"))
    return false;
