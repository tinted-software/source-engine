...
ConVar sv_portal_placement_debug("sv_portal_placement_debug", "0",
                                 FCVAR_REPLICATED);
ConVar sv_portal_placement_never_bump("sv_portal_placement_never_bump", "0",
...
void TracePortals(const CProp_Portal *pIgnorePortal, const Vector &vForward,
                  const Vector &vStart, const Vector &vEnd, trace_t &tr) {
  UTIL_ClearTrace(tr);
...
bool TraceBumpingEntities(const Vector &vStart, const Vector &vEnd,
                          trace_t &tr) {
  UTIL_ClearTrace(tr);
  ...
  for (int i = 0; i < nCount; i++) {
    ...
    if (FClassnameIs(list[i], "func_portal_bumper")) {
      ...
      if (static_cast<CFuncNoPortalVolume *>(list[i])->IsActive()) {
        ...
        // Bump by an extra 2 units so that the portal isn't touching the no
        // portal volume
    ...
    if (trTemp.fraction < tr.fraction &&
        (!bSoftBumper || !g_FuncBumpingEntityList.HasElement(list[i]))) {
      tr = trTemp;
...
bool TracePortalCorner(const CProp_Portal *pIgnorePortal, const Vector &vOrigin,
                       const Vector &vCorner, const Vector &vForward,
                       int iPlacedBy, ITraceFilter *pTraceFilterPortalShot,
                       trace_t &tr, bool &bSoftBump) {
  Vector vOriginToCorner = vCorner - vOrigin;

  // Check for surface edge
  trace_t trSurfaceEdge;
  UTIL_TraceLine(vOrigin - vForward, vCorner - vForward, MASK_SHOT_PORTAL,
                 pTraceFilterPortalShot, &trSurfaceEdge);

  if (trSurfaceEdge.startsolid) {
    float fTotalFraction = trSurfaceEdge.fractionleftsolid;

    while (trSurfaceEdge.startsolid && trSurfaceEdge.fractionleftsolid > 0.0f &&
           fTotalFraction < 1.0f) {
      UTIL_TraceLine(
          vOrigin + vOriginToCorner * (fTotalFraction + 0.05f) - vForward,
          vCorner + vOriginToCorner * (fTotalFraction + 0.05f) - vForward,
    ...
    if (fTotalFraction < 1.0f) {
      UTIL_TraceLine(vOrigin + vOriginToCorner * (fTotalFraction + 0.05f) -
                         vForward,
                     vOrigin - vForward, MASK_SHOT_PORTAL,
                     pTraceFilterPortalShot, &trSurfaceEdge);
  ...
  UTIL_TraceLine(vOrigin + vForward, vCorner + vForward,
                 MASK_SOLID_BRUSHONLY | CONTENTS_MONSTER,
  ...
  if (iPlacedBy != PORTAL_PLACED_BY_FIXED)
    TracePortals(pIgnorePortal, vForward, vOrigin + vForward,
                 vCorner + vForward, trPortal);
  else
    UTIL_ClearTrace(trPortal);

  bool bSoftBumper = TraceBumpingEntities(vOrigin + vForward,
                                          vCorner + vForward, trBumpingEntity);

  if (trEnclosingWall.fraction >= 1.0f && trPortal.fraction >= 1.0f &&
      trBumpingEntity.fraction >= 1.0f) {
    UTIL_ClearTrace(tr);
    return false;
  }

  if (trEnclosingWall.fraction <= trPortal.fraction &&
      trEnclosingWall.fraction <= trBumpingEntity.fraction) {
    tr = trEnclosingWall;
    bSoftBump = false;
  } else if (trPortal.fraction <= trEnclosingWall.fraction &&
             trPortal.fraction <= trBumpingEntity.fraction) {
    tr = trPortal;
    g_bBumpedByLinkedPortal = true;
    bSoftBump = false;
  } else if (!trBumpingEntity.startsolid &&
             trBumpingEntity.fraction <= trEnclosingWall.fraction &&
...
Vector FindBumpVectorInCorner(const Vector &ptCorner1, const Vector &ptCorner2,
                              const Vector &ptIntersectionPoint1,
                              const Vector &ptIntersectionPoint2,
                              const Vector &vIntersectionDirection1,
                              const Vector &vIntersectionDirection2,
                              const Vector &vIntersectionBumpDirection1,
                              const Vector &vIntersectionBumpDirection2) {
  Vector ptClosestSegment1, ptClosestSegment2;
  float fT1, fT2;

  CalcLineToLineIntersectionSegment(
      ptIntersectionPoint1, ptIntersectionPoint1 + vIntersectionDirection1,
  ...
  // The 2 corner trace intersections and the intersection of those lines makes
  // a triangle. We want to make a similar triangle where the base is large
  // enough to fit the edge of the portal
  ...
  if (fShortLegLength == 0.0f || fShortLeg2Length == 0.0f) {
    // FIXME: Our triangle is actually a point or a line, so there's nothing we
    // can do
    return vec3_origin;
  }
  ...
  if (fPortalEdgeDotLeg < -0.9999f || fPortalEdgeDotLeg > 0.9999f ||
      fPortalEdgeDotLeg2 < -0.9999f || fPortalEdgeDotLeg2 > 0.9999f) {
    // Do a one corner bump with corner 1
    float fBumpDistance1 =
        CalcDistanceToLine(ptCorner1, ptIntersectionPoint1,
                           ptIntersectionPoint1 + vIntersectionDirection1);

    fBumpDistance1 += PORTAL_BUMP_FORGIVENESS;

    // Do a one corner bump with corner 2
    float fBumpDistance2 =
        CalcDistanceToLine(ptCorner2, ptIntersectionPoint2,
                           ptIntersectionPoint2 + vIntersectionDirection2);

    fBumpDistance2 += PORTAL_BUMP_FORGIVENESS;

    return vIntersectionBumpDirection1 * fBumpDistance1 +
           vIntersectionBumpDirection2 * fBumpDistance2;
  }

  float fLegsDot = vShortLeg.Dot(vShortLeg2);

  // Need to know if the triangle is pointing toward the portal or away from the
  // portal
  ...
  if ( !bPointingTowardPortal )*/
  {
    ...
    float fShortBaseLength =
        sinf(fShortBaseAngle) * (fShortLegLength / sinf(fShortLegAngle));
    ...
    float fLongLegLength =
        fLongBaseLength * (fShortLegLength / fShortBaseLength);
...
bool FitPortalOnSurface(
    const CProp_Portal *pIgnorePortal, Vector &vOrigin, const Vector &vForward,
    ...
    const int *p_piIntersectionIndex /*= 0*/,
  ...
  for (int iIntersection = 0; iIntersection < 4; ++iIntersection) {
    // HACK: In weird cases intersection count can go over 3 and index outside
    // of our arrays. Don't let this happen!
    if (iIntersectionCount < 4) {
      ...
      if (!sFitData[iIntersection].bCornerIntersection) {
        ...
        sFitData[iIntersection].bCornerIntersection =
            TracePortalCorner(pIgnorePortal, vOrigin, pptCorner[iIntersection],
                              vForward, iPlacedBy, pTraceFilterPortalShot,
                              sFitData[iIntersection].trCornerTrace,
                              sFitData[iIntersection].bSoftBump);
        ...
        if (sFitData[iIntersection].bCornerIntersection) {
          sFitData[iIntersection].ptIntersectionPoint =
              vOrigin + (pptCorner[iIntersection] - vOrigin) *
                            sFitData[iIntersection].trCornerTrace.fraction;
          VectorNormalize(sFitData[iIntersection].trCornerTrace.plane.normal);
          sFitData[iIntersection].vIntersectionDirection =
              sFitData[iIntersection].trCornerTrace.plane.normal.Cross(
                  vForward);
  ...
  for (int iIntersection = 0; iIntersection < 4; ++iIntersection) {
    ...
    if (sFitData[iIntersection].bSoftBump) {
      g_FuncBumpingEntityList.AddToTail(
          sFitData[iIntersection].trCornerTrace.m_pEnt);
    }
  }
  ...
  case 1: {
    float fBumpDistance = CalcDistanceToLine(
        pptCorner[piIntersectionIndex[0]],
    ...
    return FitPortalOnSurface(
        pIgnorePortal, vOrigin, vForward, vRight, vTopEdge, vBottomEdge,
  ...
  case 2: {
    ...
    float fDot = sFitData[piIntersectionIndex[0]].vBumpDirection.Dot(
    ...
    if (fDot > 0.9f) {
      ...
      // Find the largest amount that the portal needs to bump for the corner to
      ...
      // Bump the portal
      vOrigin +=
          sFitData[piIntersectionIndex[iClosestIntersection]].vBumpDirection *
          pfBumpDistance[iLargestBump];

      // If they were parallel to the intersection line don't invalidate both
      // before recursion
      if (pfBumpDistance[0] == pfBumpDistance[1]) {
        ...
        return FitPortalOnSurface(pIgnorePortal, vOrigin, vForward, vRight,
                                  vTopEdge, vBottomEdge, vRightEdge, vLeftEdge,
                                  iPlacedBy, pTraceFilterPortalShot,
                                  iRecursions + 1, sFitData,
        ...
        sFitData[piIntersectionIndex[((iLargestBump == 0) ? (1) : (0))]]
            .bCornerIntersection = false;
    ...
    vOrigin += FindBumpVectorInCorner(
        pptCorner[piIntersectionIndex[0]], pptCorner[piIntersectionIndex[1]],
        sFitData[piIntersectionIndex[0]].ptIntersectionPoint,
        sFitData[piIntersectionIndex[1]].ptIntersectionPoint,
        sFitData[piIntersectionIndex[0]].vIntersectionDirection,
  ...
  case 3: {
    ...
    float fDot[3];
    fDot[0] = sFitData[piIntersectionIndex[0]].vBumpDirection.Dot(
        sFitData[piIntersectionIndex[1]].vBumpDirection);
    fDot[1] = sFitData[piIntersectionIndex[1]].vBumpDirection.Dot(
        sFitData[piIntersectionIndex[2]].vBumpDirection);
    fDot[2] = sFitData[piIntersectionIndex[2]].vBumpDirection.Dot(
        sFitData[piIntersectionIndex[0]].vBumpDirection);

    int iSimilarWalls = 0;

    for (int iDot = 0; iDot < 3; ++iDot) {
      // If there are parallel intersections try scooting it away from a near
      // wall
      if (fDot[iDot] < -0.99f) {
        // Check if perpendicular wall is near
        trace_t trPerpWall1;
        ...
        bool bDir1 = TracePortalCorner(
            pIgnorePortal, vOrigin,
            vOrigin +
                sFitData[piIntersectionIndex[iDot]].vIntersectionDirection *
                    PORTAL_HALF_WIDTH * 2.0f,
            vForward, iPlacedBy, pTraceFilterPortalShot, trPerpWall1,
            bSoftBump1);
        ...
        bool bDir2 = TracePortalCorner(
            ...
            vOrigin +
                sFitData[piIntersectionIndex[iDot]].vIntersectionDirection *
                    -PORTAL_HALF_WIDTH * 2.0f,
            vForward, iPlacedBy, pTraceFilterPortalShot, trPerpWall2,
            bSoftBump2);
    ...
    if (iSimilarWalls == 0) {
      ...
      if (M_PI_F - 0.01f < fAngleTotal && fAngleTotal < M_PI_F + 0.01f) {
        ...
        if (sFitData[piIntersectionIndex[0]].bSoftBump ||
            sFitData[piIntersectionIndex[1]].bSoftBump ||
          ...
          return FitPortalOnSurface(
              pIgnorePortal, vOrigin, vForward, vRight, vTopEdge, vBottomEdge,
              vRightEdge, vLeftEdge, iPlacedBy, pTraceFilterPortalShot,
              iRecursions + 1, sFitData, piIntersectionIndex,
              &iIntersectionCount);
        } else {
          return false;
        }
    ...
    if (iSimilarWalls == 3) {
      ...
      float fClosestDistance =
          vOrigin.DistTo(sFitData[piIntersectionIndex[0]].ptIntersectionPoint);
      ...
      fDistance =
          vOrigin.DistTo(sFitData[piIntersectionIndex[2]].ptIntersectionPoint);
      if (fClosestDistance > fDistance) {
      ...
      iLargestBump =
          ((pfBumpDistance[iLargestBump] > pfBumpDistance[2]) ? (iLargestBump)
                                                              : (2));
      ...
      for (int iIntersection = 0; iIntersection < 3; ++iIntersection) {
        ...
        if (pfBumpDistance[iIntersection] != pfBumpDistance[iLargestBump]) {
          sFitData[piIntersectionIndex[iIntersection]].bCornerIntersection =
              false;
          --iIntersectionCount;
        } else {
          sFitData[piIntersectionIndex[iIntersection]] =
              sFitData[piIntersectionIndex[iClosestIntersection]];
          piIntersectionIndex[iStillIntersecting] =
              piIntersectionIndex[iIntersection];
          ++iStillIntersecting;
        }
      }
    ...
    float fCornerDot =
        sFitData[piIntersectionIndex[iIndex1]].vIntersectionDirection.Dot(
    ...
    if (fCornerDot < 0.0001f && fCornerDot > -0.0001f) {
      ...
      if ((fPortalDot < 0.0001f && fPortalDot > -0.0001f) ||
          fPortalDot > 0.9999f || fPortalDot < -0.9999f) {
        float fBump1 = CalcDistanceToLine(
            pptCorner[piIntersectionIndex[iIndex1]],
        ...
        float fBump2 = CalcDistanceToLine(
            pptCorner[piIntersectionIndex[iIndex2]],
        ...
        // Bump portal
        vOrigin +=
            sFitData[piIntersectionIndex[iIndex1]].vBumpDirection * fBump1;
        vOrigin +=
            sFitData[piIntersectionIndex[iIndex2]].vBumpDirection * fBump2;
  ...
  default: {
    if (sFitData[piIntersectionIndex[0]].bSoftBump ||
        sFitData[piIntersectionIndex[1]].bSoftBump ||
        sFitData[piIntersectionIndex[2]].bSoftBump ||
        sFitData[piIntersectionIndex[3]].bSoftBump) {
      // Prepare data for recursion
      iIntersectionCount = 0;
      sFitData[piIntersectionIndex[0]].bCornerIntersection = false;
      sFitData[piIntersectionIndex[1]].bCornerIntersection = false;
...
void FitPortalAroundOtherPortals(const CProp_Portal *pIgnorePortal,
                                 Vector &vOrigin, const Vector &vForward,
  ...
  if (iPortalCount != 0) {
    ...
    for (int i = 0; i != iPortalCount; ++i) {
      ...
      if (pTempPortal != pIgnorePortal && pTempPortal->m_bActivated) {
        ...
        if (fProjUpLength < PORTAL_HALF_HEIGHT &&
            fProjRightLength < PORTAL_HALF_WIDTH) {
          vOrigin +=
              vDiffProjRight * (PORTAL_HALF_WIDTH - fProjRightLength + 1.0f);
        }
      }
    }
  }
...
bool IsPortalIntersectingNoPortalVolume(const Vector &vOrigin,
                                        const QAngle &qAngles,
                                        const Vector &vForward) {
  // Walk the no portal volume list, check each with box-box intersection
  for (CFuncNoPortalVolume *pNoPortalEnt = GetNoPortalVolumeList();
       pNoPortalEnt != NULL; pNoPortalEnt = pNoPortalEnt->m_pNext) {
    ...
    vBoxExtents += Vector(((vForward.x > 0.5f || vForward.x < -0.5f)
                               ? (0.0f)
                               : (-PORTAL_BUMP_FORGIVENESS)),
                          ((vForward.y > 0.5f || vForward.y < -0.5f)
                               ? (0.0f)
                               : (-PORTAL_BUMP_FORGIVENESS)),
                          ((vForward.z > 0.5f || vForward.z < -0.5f)
                               ? (0.0f)
                               : (-PORTAL_BUMP_FORGIVENESS)));

    if (UTIL_IsBoxIntersectingPortal(vBoxCenter, vBoxExtents, vOrigin,
                                     qAngles)) {
      if (sv_portal_placement_debug.GetBool()) {
        NDebugOverlay::Box(Vector(0.0f, 0.0f, 0.0f), vMin, vMax, 0, 255, 0, 128,
                           0.5f);
        UTIL_Portal_NDebugOverlay(vOrigin, qAngles, 0, 0, 255, 128, false,
                                  0.5f);
...
bool IsPortalOverlappingOtherPortals(const CProp_Portal *pIgnorePortal,
  ...
  Vector vPortalOBBMin =
      CProp_Portal_Shared::vLocalMins + Vector(1.0f, 1.0f, 1.0f);
  Vector vPortalOBBMax =
      CProp_Portal_Shared::vLocalMaxs - Vector(1.0f, 1.0f, 1.0f);
    ...
    for (int i = 0; i != iPortalCount; ++i) {
      ...
      if (pTempPortal != pIgnorePortal && pTempPortal->m_bActivated) {
        ...
        if (IsOBBIntersectingOBB(vOrigin, qAngles, vPortalOBBMin, vPortalOBBMax,
                                 vOtherOrigin, qOtherAngles, vPortalOBBMin,
                                 vPortalOBBMax, 0.0f)) {
          if (sv_portal_placement_debug.GetBool()) {
            UTIL_Portal_NDebugOverlay(vOrigin, qAngles, 0, 0, 255, 128, false,
                                      0.5f);
...
bool IsPortalOnValidSurface(const Vector &vOrigin, const Vector &vForward,
                            const Vector &vRight, const Vector &vUp,
  ...
  for (int iCorner = 0; iCorner < 5; ++iCorner) {
    ...
    if (iCorner < 4) {
      if (iCorner / 2 == 0)
        ptCorner +=
            vUp * (PORTAL_HALF_HEIGHT - PORTAL_BUMP_FORGIVENESS * 1.1f); // top
      else
        ptCorner += vUp * -(PORTAL_HALF_HEIGHT -
                            PORTAL_BUMP_FORGIVENESS * 1.1f); // bottom

      if (iCorner % 2 == 0)
        ptCorner += vRight * -(PORTAL_HALF_WIDTH -
                               PORTAL_BUMP_FORGIVENESS * 1.1f); // left
      else
        ptCorner += vRight * (PORTAL_HALF_WIDTH -
                              PORTAL_BUMP_FORGIVENESS * 1.1f); // right
    }
...
float VerifyPortalPlacement(const CProp_Portal *pIgnorePortal, Vector &vOrigin,
                            QAngle &qAngles, int iPlacedBy,
                            bool bTest /*= false*/) {
  ...
  trace_t tr;
  CTraceFilterSimpleClassnameList baseFilter(pIgnorePortal,
  ...
  if (iPlacedBy == PORTAL_PLACED_BY_PLAYER &&
      !sv_portal_placement_never_bump.GetBool()) {
  ...
  if (!sv_portal_placement_never_bump.GetBool()) {
    ...
    if (!FitPortalOnSurface(pIgnorePortal, vOrigin, vForward, vRight, vTopEdge,
                            vBottomEdge, vRightEdge, vLeftEdge, iPlacedBy,
    ...
    if (vUp.z > 0.7f) {
      ...
      trace_t FloorTrace;
      UTIL_TraceLine(vOrigin + vSmallForward,
                     vOrigin + vSmallForward -
                         (vUp * (PORTAL_HALF_HEIGHT + 1.5f)),
                     MASK_SOLID_BRUSHONLY, &traceFilterPortalShot, &FloorTrace);
      if (FloorTrace.fraction < 1.0f) {
        // we hit floor in that 1 extra unit, now doublecheck to make sure we
        // didn't hit something else
        trace_t FloorTrace_Verify;
        UTIL_TraceLine(
            vOrigin + vSmallForward,
            vOrigin + vSmallForward - (vUp * (PORTAL_HALF_HEIGHT - 0.1f)),
            MASK_SOLID_BRUSHONLY, &traceFilterPortalShot, &FloorTrace_Verify);
        if (FloorTrace_Verify.fraction == 1.0f) {
          // if we're in here, we're definitely in a floor matching
          // configuration, bump down to match the floor better
  ...
  if (bTest &&
      IsPortalOverlappingOtherPortals(pIgnorePortal, vOrigin, qAngles)) {
    return PORTAL_ANALOG_SUCCESS_OVERLAP_LINKED;
  }

  // Fail if it's on a flagged surface material
  if (!IsPortalOnValidSurface(vOrigin, vForward, vRight, vUp,
                              &traceFilterPortalShot)) {
    if (sv_portal_placement_debug.GetBool()) {
      UTIL_Portal_NDebugOverlay(vOrigin, qAngles, 0, 0, 255, 128, false, 0.5f);
    }
  ...
  float fAnalogSuccessMultiplier =
      1.0f - (fBumpDistance / MAXIMUM_BUMP_DISTANCE);
  fAnalogSuccessMultiplier *= fAnalogSuccessMultiplier;
  fAnalogSuccessMultiplier *= fAnalogSuccessMultiplier;

  return fAnalogSuccessMultiplier *
             (PORTAL_ANALOG_SUCCESS_NO_BUMP - PORTAL_ANALOG_SUCCESS_BUMPED) +
