...
int CDinosaurSignal::DrawDebugTextOverlays(void) {
  ...
  if (m_debugOverlays & OVERLAY_TEXT_BIT) {
    NDebugOverlay::Sphere(GetAbsOrigin(), GetAbsAngles(), m_flInnerRadius, 255,
                          0, 0, 64, false, 0.1f);
    NDebugOverlay::Sphere(GetAbsOrigin(), GetAbsAngles(), m_flOuterRadius, 0,
                          255, 0, 64, false, 0.1f);
  }
  return text_offset;
}
...
void CPortal_Dinosaur::Activate(void) {
  ...
  CBaseAchievement *pTransmissionRecvd = dynamic_cast<CBaseAchievement *>(
      g_AchievementMgrPortal.GetAchievementByName(
...
#if defined(RADIO_DEBUG_SERVER)
    Msg("Not spawning any Dinosaurs: Player has not beat the game, or failed "
        "to get heartbreaker achievement from mgr\n");
#endif
  ...
  for (int i = 0; i < ARRAYSIZE(s_radiolocs); ++i) {
    ...
    if (V_strcmp(STRING(gpGlobals->mapname), loc.mapname) == 0) {
      ...
      Msg("Dinosaur pos: %f %f %f, ang: %f %f %f\n", loc.radiopos[0],
          loc.radiopos[1], loc.radiopos[2], loc.radioang[0], loc.radioang[1],
          loc.radioang[2]);
      Msg("Signal pos: %f %f %f, inner rad: %f, outter rad: %f\n",
          loc.soundpos[0], loc.soundpos[1], loc.soundpos[2], loc.soundinnerrad,
          loc.soundouterrad);
#endif
...
#endif
        // OK, so these really could have been the same class... not worth
        // changing it now though.
...
CPortal_Dinosaur *CSpawnDinosaurHack::SpawnDinosaur(radiolocs &loc) {
  ...
  // origin and angles of zero means skip this Dinosaur creation and look for an
  // existing radio
  if (loc.radiopos[0] == 0 && loc.radiopos[1] == 0 && loc.radiopos[2] == 0 &&
      loc.radioang[0] == 0 && loc.radioang[1] == 0 && loc.radioang[2] == 0) {

#if defined(RADIO_DEBUG_SERVER)
    Msg("Dinosaur found with zero angles and origin. Replacing existing "
        "radio.\n");
#endif
    // Find existing Dinosaur, kill it and spawn at its position
    CPhysicsProp *pOldDinosaur =
        (CPhysicsProp *)gEntList.FindEntityByClassname(NULL, "prop_physics");
    while (pOldDinosaur) {
      if (V_strcmp(STRING(pOldDinosaur->GetModelName()), RADIO_MODEL_NAME) ==
          0) {
...
#if defined(RADIO_DEBUG_SERVER)
        Msg("Found Dinosaur exiting in level, replacing with %f, %f %f and %f "
            "%f %f.\n",
      ...
      pOldDinosaur = (CPhysicsProp *)gEntList.FindEntityByClassname(
          pOldDinosaur, "prop_physics");
    }
  }

  Assert(vSpawnPos != vec3_origin);

  CPortal_Dinosaur *pDinosaur =
      (CPortal_Dinosaur *)CreateEntityByName("updateitem2");
  Assert(pDinosaur);
  if (pDinosaur) {
...
CDinosaurSignal *CSpawnDinosaurHack::SpawnSignal(radiolocs &loc) {
  CDinosaurSignal *pSignal =
      (CDinosaurSignal *)CreateEntityByName("updateitem1");
  Assert(pSignal);
  if (pSignal) {
#if defined(RADIO_DEBUG_SERVER)
    if (loc.soundinnerrad > loc.soundouterrad) {
      Assert(0);
      Warning("Dinosaur BUG: Inner radius is greater than outer radius. Will "
              "swap them.\n");
      swap(loc.soundinnerrad, loc.soundouterrad);
    }
#endif
    pSignal->SetAbsOrigin(
        Vector(loc.soundpos[0], loc.soundpos[1], loc.soundpos[2]));
