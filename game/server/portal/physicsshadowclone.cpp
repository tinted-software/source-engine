...
ConVar sv_debug_physicsshadowclones("sv_debug_physicsshadowclones", "0",
                                    FCVAR_REPLICATED);
ConVar sv_use_shadow_clones("sv_use_shadow_clones", "1",
                            FCVAR_REPLICATED |
                                FCVAR_CHEAT); // should we create shadow clones?
...
CUtlVector<CPhysicsShadowClone *> const
    &CPhysicsShadowClone::g_ShadowCloneList = s_ActiveShadowClones;
...
void CPhysicsShadowClone::FullSync(bool bAllowAssumedSync) {
  ...
  if (pClonedEntity == NULL) {
    AssertMsg(VPhysicsGetObject() != NULL,
              "Been linkless for more than this update, something should have "
              "killed this clone.");
  ...
  if (bAllowAssumedSync) {
    ...
    int iObjectCount =
        pClonedEntity->VPhysicsGetObjectList(pSourceObjects, 1024);
    ...
    if (!bBigChanges) {
      for (int i = 0; i != iObjectCount; ++i) {
        ...
        if ((pSourcePhysics != m_CloneLinks[i].pSource) ||
            (pSourcePhysics->IsCollisionEnabled() !=
             pClonedPhysics->IsCollisionEnabled())) {
          bBigChanges = true;
          bIsSynced = false;
          break;
        }
        ...
        if (!pSourcePhysics->IsAsleep()) // only allow full syncrosity if the
                                         // source entity is entirely asleep
          bIsSynced = false;
    ...
    if (bIsSynced) {
      ...
      if (!m_bInAssumedSyncState) {
        ...
        // if we don't do this, objects just fall out of the world (it happens,
        // I swear)

        for (int i = m_CloneLinks.Count(); --i >= 0;) {
          if ((m_CloneLinks[i].pSource->GetShadowController() == NULL) &&
              m_CloneLinks[i].pClone->IsMotionEnabled()) {
            // m_CloneLinks[i].pClone->SetVelocityInstantaneous( &vec3_origin,
            // &vec3_origin ); m_CloneLinks[i].pClone->SetVelocity(
  ...
  if (bBigChanges) {
    ...
    if ((sourceMoveType == MOVETYPE_CUSTOM) ||
      ...
      // #endif
      // PUSH should be used sparingly, you can't stand on a MOVETYPE_PUSH
      // object :/
      SetMoveType(
          MOVETYPE_VPHYSICS,
          pClonedEntity->GetMoveCollide()); // either an unclonable movetype, or
                                            // a shadow/held object
    }
    ...
    if (m_bShadowTransformIsIdentity)
      iSolidFlags |= FSOLID_CUSTOMBOXTEST; // need this at least for the player
                                           // or they get stuck in themselves
    else
    ...
    SetEffects(pClonedEntity->GetEffects() |
               (EF_NODRAW | EF_NOSHADOW | EF_NORECEIVESHADOW));
...
void CPhysicsShadowClone::SyncEntity(bool bPullChanges) {
  ...
  if ((ptOrigin != pDest->GetAbsOrigin()) ||
      (qAngles != pDest->GetAbsAngles())) {
    pDest->Teleport(&ptOrigin, &qAngles, NULL);
  }

  if (vVelocity != pDest->GetAbsVelocity()) {
    // pDest->IncrementInterpolationFrame();
    pDest->SetAbsVelocity(vec3_origin); // the two step process helps, I don't
                                        // know why, but it does
    pDest->ApplyAbsVelocityImpulse(vVelocity);
  }
}

static void FullSyncPhysicsObject(IPhysicsObject *pSource,
                                  IPhysicsObject *pDest,
                                  const VMatrix *pTransform, bool bTeleport) {
  ...
  float
      fSavedMass = 0.0f,
      fSavedRotationalDamping; // setting mass to 0.0f purely to kill a warning
                               // that I can't seem to kill with pragmas
  if (pSource->GetGameFlags() & FVPHYSICS_PLAYER_HELD) {
    ...
    if (pHoldingPlayer) {
      ...
      if (!pGrabController)
        pGrabController =
            GetGrabControllerForPhysCannon(pHoldingPlayer->GetActiveWeapon());
    }

    AssertMsg(
        pGrabController,
        "Physics object is held, but we can't find the holding controller.");
    GetSavedParamsForCarriedPhysObject(pGrabController, pSource, &fSavedMass,
  ...
  // Boiler plate
  {
    ...
    pDest->SetGameFlags(pSource->GetGameFlags() | FVPHYSICS_NO_SELF_COLLISIONS |
                        FVPHYSICS_IS_SHADOWCLONE);
  ...
  // stuff that we really care about
  {
    ...
    // avoid oversetting variables (I think that even setting them to the same
    // value they already are disrupts the delicate physics balance)
    ...
    if ((vVelocity != vDestVelocity) ||
        (vAngularVelocity != vDestAngularVelocity))
      pDest->SetVelocityInstantaneous(&vVelocity, &vAngularVelocity);

    IPhysicsShadowController *pSourceController =
        pSource->GetShadowController();
    if (pSourceController == NULL) {
      ...
      if (pDestController == NULL) {
        ...
        pDest->SetShadow(fMaxSpeed, fMaxAngularSpeed,
                         pSourceController->AllowsTranslation(),
                         pSourceController->AllowsRotation());
        pDestController = pDest->GetShadowController();
        pDestController->SetTeleportDistance(
            pSourceController->GetTeleportDistance());
        pDestController->SetPhysicallyControlled(
            pSourceController->IsPhysicallyControlled());
      }
      ...
      fTimeOffset = pSourceController->GetTargetPosition(&ptTargetPosition,
                                                         &qTargetAngles);

      if (pTransform) {
        ptTargetPosition = (*pTransform) * ptTargetPosition;
        qTargetAngles =
            TransformAnglesToWorldSpace(qTargetAngles, pTransform->As3x4());
      }
...
static void PartialSyncPhysicsObject(IPhysicsObject *pSource,
                                     IPhysicsObject *pDest,
...
void CPhysicsShadowClone::FullSyncClonedPhysicsObjects(bool bTeleport) {
  ...
  if (iObjectCount == m_CloneLinks.Count()) {
    ...
    {
      for (i = 0; i != iObjectCount; ++i)
        FullSyncPhysicsObject(m_CloneLinks[i].pSource, m_CloneLinks[i].pClone,
                              pTransform, bTeleport);

      return;
    }
  }

  // copy the existing list of clone links to a temp array, we're going to be
  // starting from scratch and copying links as we need them
  PhysicsObjectCloneLink_t *pExistingLinks = NULL;
  int iExistingLinkCount = m_CloneLinks.Count();
  if (iExistingLinkCount != 0) {
    pExistingLinks = (PhysicsObjectCloneLink_t *)stackalloc(
        sizeof(PhysicsObjectCloneLink_t) * m_CloneLinks.Count());
    memcpy(pExistingLinks, m_CloneLinks.Base(),
           sizeof(PhysicsObjectCloneLink_t) * m_CloneLinks.Count());
  }
  m_CloneLinks.RemoveAll();

  // now, go over the object list we just got from the source entity, and either
  // copy or create links as necessary
  int i;
  for (i = 0; i != iObjectCount; ++i) {
    ...
    if (j != iExistingLinkCount) {
      ...
      memset(&pExistingLinks[j], 0,
             sizeof(PhysicsObjectCloneLink_t)); // zero out this slot so we
                                                // don't destroy it in cleanup
    } else {
      // no link found to copy, create a new one
      cloneLink.pSource = pSource;

      // apparently some collision code gets called on creation before we've set
      // extra game flags, so we're going to cheat a bit and temporarily set our
      // extra flags on the source
      ...
      physenv->SerializeObjectToBuffer(
          pSource, pBuffer,
          size); // this should work across physics environments because the
                 // serializer doesn't write anything about itself to the
                 // template
      pSource->SetGameFlags(iOldGameFlags);
      cloneLink.pClone = m_pOwnerPhysEnvironment->UnserializeObjectFromBuffer(
          this, pBuffer, size,
          false); // unserializer has to be in the target environment
      assert(cloneLink.pClone); // there should be absolutely no case where we
                                // can't clone a valid existing physics object

      stackfree(pBuffer);
    }

    FullSyncPhysicsObject(cloneLink.pSource, cloneLink.pClone, pTransform,
                          bTeleport);
  ...
  // now go over the existing links, if any of them haven't been nullified, they
  // need to be deleted
  for (i = 0; i != iExistingLinkCount; ++i) {
    if (pExistingLinks[i].pClone)
      m_pOwnerPhysEnvironment->DestroyObject(
          pExistingLinks[i].pClone); // also destroys shadow controller
  }
...
void CPhysicsShadowClone::PartialSync(bool bPullChanges) {
  ...
  if (bPullChanges) {
    ...
    for (int i = m_CloneLinks.Count(); --i >= 0;)
      PartialSyncPhysicsObject(m_CloneLinks[i].pSource, m_CloneLinks[i].pClone,
                               pTransform);
  } else {
    if (m_bShadowTransformIsIdentity)
      pTransform = NULL;
    ...
    for (int i = m_CloneLinks.Count(); --i >= 0;)
      PartialSyncPhysicsObject(m_CloneLinks[i].pClone, m_CloneLinks[i].pSource,
                               pTransform);
  }
...
int CPhysicsShadowClone::VPhysicsGetObjectList(IPhysicsObject **pList,
                                               int listMax) {
...
bool CPhysicsShadowClone::ShouldCollide(int collisionGroup,
...
bool CPhysicsShadowClone::TestCollision(const Ray_t &ray,
...
void CPhysicsShadowClone::SetCloneTransformationMatrix(
    const matrix3x4_t &sourceMatrix) {
  ...
  if (m_matrixShadowTransform.InverseGeneral(m_matrixShadowTransform_Inverse) ==
    ...
    m_matrixShadowTransform.InverseTR(
        m_matrixShadowTransform_Inverse); // probably not the right matrix, but
...
CPhysicsShadowClone *CPhysicsShadowClone::CreateShadowClone(
    IPhysicsEnvironment *pInPhysicsEnvironment, EHANDLE hEntToClone,
    const char *szDebugMarker,
    const matrix3x4_t *pTransformationMatrix /*= NULL*/) {
  AssertMsg(szDebugMarker != NULL, "All shadow clones must have a debug marker "
                                   "for where it came from in debug builds.");
  ...
  AssertMsg(IsShadowClone(pClonedEntity) == false,
            "Shouldn't attempt to clone clones");
  ...
  if (g_iShadowCloneCount >= MAX_SHADOW_CLONE_COUNT) {
    AssertMsg(false, "Too many shadow clones, consider upping the limit or "
                     "reducing the level's physics props");
    return NULL;
  }
  ++g_iShadowCloneCount;

  CPhysicsShadowClone *pClone =
      (CPhysicsShadowClone *)CreateEntityByName("physicsshadowclone");
  ...
  if (pTransformationMatrix) {
    ...
    pClone->m_bShadowTransformIsIdentity =
        pClone->m_matrixShadowTransform.IsIdentity();

    if (!pClone->m_bShadowTransformIsIdentity) {
      if (pClone->m_matrixShadowTransform.InverseGeneral(
              pClone->m_matrixShadowTransform_Inverse) == false) {
        pClone->m_matrixShadowTransform.InverseTR(
            pClone->m_matrixShadowTransform_Inverse); // probably not the right
                                                      // matrix, but we're out
                                                      // of options
      }
    }
  }
...
IPhysicsObject *CPhysicsShadowClone::TranslatePhysicsToClonedEnt(
    const IPhysicsObject *pPhysics) {
...
void CPhysicsShadowClone::VPhysicsCollision(int index,
                                            gamevcollisionevent_t *pEvent) {
  // the baseclass just screenshakes, makes sounds, and outputs dust, we rely on
  // the original entity to do this when applicable
}
...
CPhysicsShadowCloneLL *
CPhysicsShadowClone::GetClonesOfEntity(const CBaseEntity *pEntity) {
  return s_EntityClones[pEntity->entindex()];
}

static void DrawDebugOverlayForShadowClone(CPhysicsShadowClone *pClone) {
  unsigned char iColorIntensity =
      (pClone->IsInAssumedSyncState()) ? (127) : (255);

  int iRed = (pClone->IsUntransformedClone()) ? (0) : (iColorIntensity);
  int iGreen = iColorIntensity;
  int iBlue = iColorIntensity;

  NDebugOverlay::EntityBounds(pClone, iRed, iGreen, iBlue,
                              (iColorIntensity >> 2), 0.05f);
}

bool CTraceFilterTranslateClones::ShouldHitEntity(IHandleEntity *pEntity,
                                                  int contentsMask) {
  CBaseEntity *pEnt = EntityFromEntityHandle(pEntity);
  if (CPhysicsShadowClone::IsShadowClone(pEnt)) {
    CBaseEntity *pClonedEntity =
        ((CPhysicsShadowClone *)pEnt)->GetClonedEntity();
    CPortalSimulator *pSimulator =
        CPortalSimulator::GetSimulatorThatOwnsEntity(pClonedEntity);
    if (pSimulator->m_DataAccess.Simulation.Dynamic
            .EntFlags[pClonedEntity->entindex()] &
