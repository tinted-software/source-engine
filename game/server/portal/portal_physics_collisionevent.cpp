...
int CPortal_CollisionEvent::ShouldCollide(IPhysicsObject *pObj0,
  ...
  bool bShadowClonesInvolved =
      ((pObj0->GetGameFlags() | pObj1->GetGameFlags()) &
       FVPHYSICS_IS_SHADOWCLONE) != 0;
  ...
  if (bShadowClonesInvolved) {
    ...
    if ((pObj0->GetGameFlags() | pObj1->GetGameFlags()) &
      ...
      // don't let players collide with objects they're holding, they get kinda
      // messed up sometimes
      if (pGameData0 && ((CBaseEntity *)pGameData0)->IsPlayer() &&
          (GetPlayerHeldEntity((CBasePlayer *)pGameData0) ==
           (CBaseEntity *)pGameData1))
        return 0;

      if (pGameData1 && ((CBaseEntity *)pGameData1)->IsPlayer() &&
          (GetPlayerHeldEntity((CBasePlayer *)pGameData1) ==
           (CBaseEntity *)pGameData0))
        return 0;
    }
  }

  // everything is in one environment. This means we must tightly control what
  // collides with what
  if (pGameData0 != pGameData1) {
    // this code only decides what CAN'T collide due to portal environment
    // differences, things that should collide will pass through here to deeper
    // ShouldCollide() code
    CBaseEntity *pEntities[2] = {(CBaseEntity *)pGameData0,
                                 (CBaseEntity *)pGameData1};
    ...
    for (int i = 0; i != 2; ++i)
      pSimulators[i] =
          CPortalSimulator::GetSimulatorThatOwnsEntity(pEntities[i]);

    AssertOnce((bStatic[0] && bStatic[1]) ==
               false); // hopefully the system doesn't even call in for this,
                       // they're both static and can't collide
    if (bStatic[0] && bStatic[1])
      return 0;

#ifdef _DEBUG
    for (int i = 0; i != 2; ++i) {
      if ((pSimulators[i] != NULL) &&
          CPhysicsShadowClone::IsShadowClone(pEntities[i])) {
        CPhysicsShadowClone *pClone = (CPhysicsShadowClone *)pEntities[i];
        CBaseEntity *pSource = pClone->GetClonedEntity();

        CPortalSimulator *pSourceSimulator =
            CPortalSimulator::GetSimulatorThatOwnsEntity(pSource);
        Assert((pSimulators[i]
                    ->m_DataAccess.Simulation.Dynamic
                    .EntFlags[pClone->entindex()] &
                PSEF_IS_IN_PORTAL_HOLE) ==
               (pSourceSimulator->m_DataAccess.Simulation.Dynamic
                    .EntFlags[pSource->entindex()] &
                PSEF_IS_IN_PORTAL_HOLE));
      }
    }
#endif
    ...
    if (pSimulators[0] == pSimulators[1]) // same simulator
    {
      if (pSimulators[0] != NULL) // and not main world
      {
        if (bStatic[0] || bStatic[1]) {
          for (int i = 0; i != 2; ++i) {
            if (bStatic[i]) {
              if (CPSCollisionEntity::IsPortalSimulatorCollisionEntity(
                      pEntities[i])) {
                PS_PhysicsObjectSourceType_t objectSource;
                if (pSimulators[i]->CreatedPhysicsObject(pPhysObjects[i],
                                                         &objectSource) &&
                    ((objectSource == PSPOST_REMOTE_BRUSHES) ||
                     (objectSource == PSPOST_REMOTE_STATICPROPS))) {
                  if ((pSimulators[1 - i]
                           ->m_DataAccess.Simulation.Dynamic
                           .EntFlags[pEntities[1 - i]->entindex()] &
                       PSEF_IS_IN_PORTAL_HOLE) == 0)
                    return 0; // require that the entity be in the portal hole
                              // before colliding with transformed geometry
                  // FIXME: The above requirement might fail horribly for
                  // transformed collision blocking the portal from the other
                  // side and fast moving objects
                }
              }
              break;
            }
          ...
          if (((pSimulators[0]
                    ->m_DataAccess.Simulation.Dynamic
                    .EntFlags[pEntities[0]->entindex()] |
                pSimulators[1]
                    ->m_DataAccess.Simulation.Dynamic
                    .EntFlags[pEntities[1]->entindex()]) &
               PSEF_IS_IN_PORTAL_HOLE) == 0) {
    ...
    {
      if (bShadowClonesInvolved) // entities can only collide with shadow clones
                                 // "owned" by the same simulator.
        return 0;
      ...
      if (bStatic[0] || bStatic[1]) {
        for (int i = 0; i != 2; ++i) {
          if (bStatic[i]) {
            ...
            if (pEntities[i]->IsWorld()) {
              Assert(CPortalSimulator::GetSimulatorThatCreatedPhysicsObject(
                         pPhysObjects[i]) == NULL);
              ...
              CPortalSimulator *pSimulator_Static =
                  CPortalSimulator::GetSimulatorThatCreatedPhysicsObject(
                      pPhysObjects[i]); // might have been a static prop which
                                        // would yield a new simulator
        ...
        Assert(CPSCollisionEntity::IsPortalSimulatorCollisionEntity(
                   pEntities[0]) == false);
        Assert(CPSCollisionEntity::IsPortalSimulatorCollisionEntity(
                   pEntities[1]) == false);
        ...
        for (int i = 0; i != 2; ++i) {
          if (pSimulators[i]) {
            // entities in the physics environment only collide with statics
            // created by the environment (handled above), entities in the same
            // environment (also above), or entities that should be cloned from
            // main to the same environment
            if ((pSimulators[i]
                     ->m_DataAccess.Simulation.Dynamic
                     .EntFlags[pEntities[1 - i]->entindex()] &
...
int CPortal_CollisionEvent::ShouldSolvePenetration(IPhysicsObject *pObj0,
                                                   IPhysicsObject *pObj1,
                                                   void *pGameData0,
  ...
  if ((pObj0->GetGameFlags() | pObj1->GetGameFlags()) & FVPHYSICS_PLAYER_HELD) {
    ...
    // messed up sometimes
    if (pOther->IsPlayer() &&
        (GetPlayerHeldEntity((CBasePlayer *)pOther) == pHeld))
      return 0;

    // held objects are clipping into other objects when travelling across a
    // portal. We're close to ship, so this seems to be the most localized way
    // to make a fix. Note that we're not actually going to change whether it
    // should solve, we're just going to tack on some hacks
    CPortal_Player *pHoldingPlayer =
        (CPortal_Player *)GetPlayerHoldingEntity(pHeld);
    if (!pHoldingPlayer && CPhysicsShadowClone::IsShadowClone(pHeld))
      pHoldingPlayer = (CPortal_Player *)GetPlayerHoldingEntity(
          ((CPhysicsShadowClone *)pHeld)->GetClonedEntity());

    Assert(pHoldingPlayer);
    if (pHoldingPlayer) {
      CGrabController *pGrabController =
          GetGrabControllerForPlayer(pHoldingPlayer);
  ...
  if ((pObj0->GetGameFlags() | pObj1->GetGameFlags()) &
    ...
    if ((pObj0->GetGameFlags() & pObj1->GetGameFlags()) &
        FVPHYSICS_IS_SHADOWCLONE) // don't solve between two shadowclones,
                                  // they're just going to resync in a frame
                                  // anyways
      return 0;
    ...
    for (int i = 0; i != 2; ++i) {
      if (pObjects[i]->GetGameFlags() & FVPHYSICS_IS_SHADOWCLONE) {
        ...
        if (((CPhysicsShadowClone *)(pObjects[i]->GetGameData()))
                ->GetClonedEntity() == (pObjects[j]->GetGameData()))
          return 0; // don't solve between a shadow clone and its source entity
      }
    }
  }

  return BaseClass::ShouldSolvePenetration(pObj0, pObj1, pGameData0, pGameData1,
                                           dt);
}
...
static void ModifyWeight_PreCollision(vcollisionevent_t *pEvent) {
  ...
  for (int i = 0; i != 2; ++i) {
    if (pEvent->pObjects[i]->GetGameFlags() & FVPHYSICS_IS_SHADOWCLONE) {
      CPhysicsShadowClone *pClone =
          ((CPhysicsShadowClone *)pEvent->pObjects[i]->GetGameData());
      pUnshadowedEntities[i] = pClone->GetClonedEntity();

      if (pUnshadowedEntities[i] == NULL)
        return;

      pUnshadowedObjects[i] =
          pClone->TranslatePhysicsToClonedEnt(pEvent->pObjects[i]);

      if (pUnshadowedObjects[i] == NULL)
        return;
    } else {
      pUnshadowedEntities[i] =
          (CBaseEntity *)pEvent->pObjects[i]->GetGameData();
  ...
  for (int i = 0; i != 2; ++i) {
    ...
    // One is a combine ball, if the other is a movable brush, reduce the
    // combine ball mass
    if (dynamic_cast<CPropCombineBall *>(pUnshadowedEntities[j]) != NULL &&
        pUnshadowedEntities[i] != NULL) {
      ...
      // HACKHACK: last minute problem knocking over turrets with energy balls,
      // up the mass of the ball by a lot
      if (FClassnameIs(pUnshadowedEntities[i], "npc_portal_turret_floor")) {
        pUnshadowedObjects[j]->SetMass(
            pUnshadowedEntities[i]->VPhysicsGetObject()->GetMass());
      }
    }
  }

  for (int i = 0; i != 2; ++i) {
    if ((pUnshadowedObjects[i] &&
         pUnshadowedObjects[i]->GetGameFlags() & FVPHYSICS_PLAYER_HELD)) {
      int j = 1 - i;
      if (dynamic_cast<CPropCombineBall *>(pUnshadowedEntities[j]) != NULL) {
        // [j] is the combine ball, set mass low
        // if the above ball vs brush entity check didn't already change the
      ...
      } else if (pEvent->pObjects[j]->GetGameFlags() &
                 FVPHYSICS_IS_SHADOWCLONE) {
        // held object vs shadow clone, set held object mass back to grab
        // controller saved mass
        ...
        AssertMsg(pGrabController, "Physics object is held, but we can't find "
                                   "the holding controller.");
        GetSavedParamsForCarriedPhysObject(pGrabController,
                                           pUnshadowedObjects[i], &fSavedMass,
...
void CPortal_CollisionEvent::PostSimulationFrame() {
  // this actually happens once per physics environment simulation, and we don't
  // want that, so do nothing and we'll get a different version manually called
}
...
void CPortal_CollisionEvent::AddDamageEvent(CBaseEntity *pEntity,
  ...
  if ((info.GetDamageType() & DMG_CRUSH) &&
    ...
    // VERY likely this was caused by the penetration solver. Since a shadow
    // clone is involved we're going to ignore it becuase it causes more
    // problems than it solves in this case
    ReplacementDamageInfo = info;
    ReplacementDamageInfo.SetDamage(0.0f);
    pPassDownInfo = &ReplacementDamageInfo;
  }

  BaseClass::AddDamageEvent(pEntity, *pPassDownInfo, pInflictorPhysics,
                            bRestoreVelocity, savedVel, savedAngVel);
}
