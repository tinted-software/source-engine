...
public:
  ...
  // set when a held entity is penetrating another through a portal. Needed for
  // special fixes
...
private:
  ...
  friend void GetSavedParamsForCarriedPhysObject(
      CGrabController *pGrabController, IPhysicsObject *pObject,
...
DEFINE_EMBEDDED(m_shadow),
    ...
    DEFINE_FIELD(m_errorTime, FIELD_FLOAT), DEFINE_FIELD(m_error, FIELD_FLOAT),
...
void CGrabController::GetTargetPosition(Vector *target,
                                        QAngle *targetOrientation) {
  if (target)
...
float CGrabController::ComputeError() {
  ...
  CPortal_Player *pPortalPlayer =
      (CPortal_Player *)GetPlayerHoldingEntity(pAttached);
  ...
  if (pPortalPlayer->IsHeldObjectOnOppositeSideOfPortal()) {
    ...
    // If the player is upside down then we need to hold the box closer to their
    // feet.
    if (up.z < 0.0f)
    ...
    if (UTIL_IntersectRayWithPortal(
            rayPortalTest, pPortalPlayer->GetHeldObjectPortal()) < 0.0f) {
      m_error *= 2.5f;
    }
  }
...
void CGrabController::AttachEntity(CBasePlayer *pPlayer, CBaseEntity *pEntity,
  ...
  if (pPortalPlayer->IsHeldObjectOnOppositeSideOfPortal()) {
    ...
    Vector radial = physcollision->CollideGetExtent(
        pPhys->GetCollide(), vec3_origin, pEntity->GetAbsAngles(),
        -vPlayerForward);
    ...
    // If our end point hasn't gone into the portal yet we at least need to know
    // what portal is in front of us
    if (!pObjectPortal) {
      Ray_t rayPortalTest;
      ...
      if (iPortalCount != 0) {
        ...
        for (int i = 0; i != iPortalCount; ++i) {
          ...
          if (pTempPortal->m_bActivated &&
            ...
            float fDist =
                UTIL_IntersectRayWithPortal(rayPortalTest, pTempPortal);
    ...
    if (pObjectPortal) {
      UTIL_Portal_AngleTransform(
          pObjectPortal->m_hLinkedPortal->MatrixThisToLinked(), angles, angles);
    }
  }
...
CGrabController::Simulate(IPhysicsMotionController *pController,
                          IPhysicsObject *pObject, float deltaTime,
                          Vector &linear, AngularImpulse &angular) {
...
DEFINE_EMBEDDED(m_grabController),
    ...
    //			*pObject -
    //-----------------------------------------------------------------------------
    void CPlayerPickupController::Init(CBasePlayer *pPlayer,
                                       CBaseEntity *pObject) {
  ...
  if (pPlayer->GetActiveWeapon()) {
    ...
    if (FClassnameIs(pPlayer->GetActiveWeapon(), "weapon_portalgun")) {
      CWeaponPortalgun *pPortalGun =
          (CWeaponPortalgun *)(pPlayer->GetActiveWeapon());
  ...
  HapticSetConstantForce(m_pPlayer,
                         clamp(m_grabController.GetLoadWeight() * 0.1, 1, 6) *
                             Vector(0, -1, 0));
#endif
    ...
    if (!pOwner->m_bSilentDropAndPickup) {
      if (m_pPlayer->GetActiveWeapon()) {
        if (FClassnameIs(m_pPlayer->GetActiveWeapon(), "weapon_portalgun")) {
          ...
          CWeaponPortalgun *pPortalGun =
              (CWeaponPortalgun *)(m_pPlayer->GetActiveWeapon());
          ...
          if (!m_pPlayer->GetActiveWeapon()->Deploy()) {
            ...
            // doesn't autoswitch away when out of ammo. Switch to next best
            // weapon.
  ...
  if (ToBasePlayer(pActivator) == m_pPlayer) {
    ...
    if (m_pPlayer->m_nButtons & IN_ATTACK) {
      ...
      if (((CPortal_Player *)m_pPlayer)->IsHeldObjectOnOppositeSideOfPortal()) {
        CProp_Portal *pHeldPortal =
            ((CPortal_Player *)m_pPlayer)->GetHeldObjectPortal();
        UTIL_Portal_VectorTransform(pHeldPortal->MatrixThisToLinked(),
                                    vecLaunch, vecLaunch);
      }
...
void PlayerPickupObject(CBasePlayer *pPlayer, CBaseEntity *pObject) {
  ...
  if (pObject->GetBaseAnimating() &&
      pObject->GetBaseAnimating()->IsDissolving())
    return;
...
void CWeaponPhysCannon::PrimaryAttack(void) {
  ...
  if (ToPortalPlayer(pOwner)->IsHeldObjectOnOppositeSideOfPortal()) {
    ...
    UTIL_Portal_VectorTransform(pPortal->MatrixThisToLinked(), forward,
                                forward);
  }
...
bool CWeaponPhysCannon::AttachObject(CBaseEntity *pObject,
  ...
  if (pOwner) {
    ...
    HapticSetConstantForce(
        pOwner, clamp(m_grabController.GetLoadWeight() * 0.05, 1, 5) *
                    Vector(0, -1, 0));
#endif
    pOwner->EnableSprint(false);
...
bool CGrabController::UpdateObject(CBasePlayer *pPlayer, float flError) {
  ...
  if (!pPortal) {
    // If the portal is invalid make sure we don't try to hold it across the
    // portal
  ...
  if (pPortal) {
    ...
    UTIL_Portal_AngleTransform(pPortal->m_hLinkedPortal->MatrixThisToLinked(),
                               qEntityAngles, qEntityAngles);
  }
  // Now clamp a sphere of object radius at end to the player's bbox
  Vector radial = physcollision->CollideGetExtent(
      pPhys->GetCollide(), vec3_origin, qEntityAngles, -forward);
  Vector player2d = pPlayer->CollisionProp()->OBBMaxs();
  float playerRadius = player2d.Length2D();
...
void CWeaponPhysCannon::OpenElements(void) {
  ...
  if (!IsMegaPhysCannon()) {
    pOwner->RumbleEffect(RUMBLE_PHYSCANNON_OPEN, 0,
                         RUMBLE_FLAG_RESTART | RUMBLE_FLAG_LOOP);
  }
...
void CWeaponPhysCannon::DoMegaEffectLaunch(Vector *pos) {
  ...
  CBeam *pBeam = CBeam::BeamCreate(IsMegaPhysCannon() ? MEGACANNON_BEAM_SPRITE
  ...
  for (int i = 0; i < numBeams; i++) {
    pBeam = CBeam::BeamCreate(IsMegaPhysCannon() ? MEGACANNON_BEAM_SPRITE
                                                 : PHYSCANNON_BEAM_SPRITE,
                              0.8);
...
//when the 			object was added to the list. This list is checked when a physics
//			object strikes another entity, to resolve whether the
//player is 			accountable for the impact.
//
...
CGrabController *
GetGrabControllerForPhysCannon(CBaseCombatWeapon *pActiveWeapon) {
  CWeaponPhysCannon *pCannon = dynamic_cast<CWeaponPhysCannon *>(pActiveWeapon);
  if (pCannon) {
...
void GetSavedParamsForCarriedPhysObject(CGrabController *pGrabController,
                                        IPhysicsObject *pObject,
                                        float *pSavedMassOut,
...
void UpdateGrabControllerTargetPosition(CBasePlayer *pPlayer, Vector *vPosition,
                                        QAngle *qAngles) {
...
void GrabController_SetPortalPenetratingEntity(CGrabController *pController,
                                               CBaseEntity *pPenetrated) {
