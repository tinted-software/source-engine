...
DEFINE_FIELD(m_bEnabled, FIELD_BOOLEAN),
    ...
    DEFINE_FIELD(m_hAimTarget, FIELD_EHANDLE), DEFINE_THINKFUNC(DisabledThink),
  ...
  pPoseController =
      static_cast<CPoseController *>(m_hTelescopicPoseController.Get());
  if (pPoseController)
    UTIL_Remove(pPoseController);
...
void CPropTelescopicArm::Spawn(void) {
  ...
  pPoseController = static_cast<CPoseController *>(
      CreateEntityByName("point_posecontroller"));
...
void CPropTelescopicArm::Activate(void) { BaseClass::Activate(); }
...
Vector CPropTelescopicArm::FindTargetAimPoint(void) {
  ...
  if (!pTarget) {
    ...
    Warning("CPropTelescopicArm::FindTargetAimPoint called with no valid "
            "target entity.");
    return vec3_invalid;
  } else {
    Vector vFrontPoint;
    ...
    UTIL_Portal_ShortestDistance(vFrontPoint, vAimPoint, &pShortestDistPortal,
                                 true);
...
//-----------------------------------------------------------------------------
Vector
CPropTelescopicArm::FindAimPointThroughPortal(const CProp_Portal *pPortal) {
...
//-----------------------------------------------------------------------------
// Purpose: Tests if this prop's front point has direct line of sight to it's
// target entity Input  : vAimPoint - The point to aim at Output : Returns true
// if target is in direct line of sight, false otherwise.
//-----------------------------------------------------------------------------
...
void CPropTelescopicArm::AimAt(Vector vTarget) {
  ...
  vNormalToTarget =
      vWorldToLocalRotation.InverseTR().ApplyRotation(vNormalToTarget);
  ...
  CPoseController *pPoseController =
      static_cast<CPoseController *>(m_hRotXPoseController.Get());
  if (pPoseController) {
...
void CPropTelescopicArm::SetTarget(const char *pchTargetName) {
  CBaseEntity *pTarget =
      gEntList.FindEntityByName(NULL, pchTargetName, NULL, NULL);
...
void CPropTelescopicArm::InputDisable(inputdata_t &inputdata) {
  if (m_bEnabled) {
    ...
    pPoseController =
        static_cast<CPoseController *>(m_hRotYPoseController.Get());
    if (pPoseController) {
      pPoseController->SetInterpolationTime(TELESCOPE_DISABLE_TIME * 0.5f);
      pPoseController->SetPoseValue(0.0f);
    }
