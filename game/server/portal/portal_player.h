...
private:
  ...
  mutable Vector
      m_vWorldSpaceCenterHolder; // WorldSpaceCenter() returns a reference, need
                                 // an actual value somewhere

public:
  CNetworkVar(bool, m_bPitchReorientation);
  CNetworkHandle(
      CProp_Portal,
      m_hPortalEnvironment); // if the player is in a portal environment, this
                             // is the associated portal
  CNetworkHandle(
      CFunc_LiquidPortal,
      m_hSurroundingLiquidPortal); // if the player is standing in a liquid
                                   // portal, this will point to it
