...
class CNPC_SecurityCamera : public CNPCBaseInteractive<CAI_BaseNPC>,
...
public:
  ...
  bool OnAttemptPhysGunPickup(CBasePlayer *pPhysGunUser,
...
void CNPC_SecurityCamera::Precache(void) {
  ...
  // Scenes for when the player dismounts a security camera. Spoken only if
  // Aperture_AI actor is in the
...
void CNPC_SecurityCamera::CreateSounds() {
  if (!m_pMovementSound) {
    ...
    m_pMovementSound = controller.SoundCreate(filter, entindex(),
...
void CNPC_SecurityCamera::Spawn(void) {
  ...
  SetCollisionBounds(Vector(-16.0f, -16.0f, -16.0f),
...
int CNPC_SecurityCamera::ObjectCaps(void) {
  ...
  return (BaseClass::ObjectCaps() | FCAP_USE_IN_RADIUS | FCAP_USE_ONGROUND |
          FCAP_IMPULSE_USE);
}

void CNPC_SecurityCamera::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
...
bool CNPC_SecurityCamera::OnAttemptPhysGunPickup(CBasePlayer *pPhysGunUser,
                                                 PhysGunPickup_t reason) {
  return !m_bActive;
}
...
bool CNPC_SecurityCamera::UpdateFacing(void) {
  ...
  if (bMoved) {
    if (m_pMovementSound) {
      ...
      controller.SoundChangeVolume(m_pMovementSound, RandomFloat(0.7f, 0.9f),
                                   0.05f);
    }
...
//-----------------------------------------------------------------------------
bool CNPC_SecurityCamera::FVisible(CBaseEntity *pEntity, int traceMask,
...
void CNPC_SecurityCamera::ActiveThink(void) {
  ...
  bool bEnemyVisible =
      pEnemy->IsAlive() && FInViewCone(pEnemy) && FVisible(pEnemy);
  ...
  Vector vProjectedDirToEnemyEyes =
      vecDirToEnemyEyes - vecDirToEnemyEyes.Dot(vUp) * vUp;
  ...
  if (vProjectedDirToEnemyEyes.IsZero())
    ...
    if (vProjectedDirToEnemyEyes.Dot(vForward) > 0.0f)
      vecAnglesToEnemy.y =
          acosf(vProjectedDirToEnemyEyes.Dot(vRight)) * (180.0f / M_PI) - 90.0f;
    else
      vecAnglesToEnemy.y =
          -acosf(vProjectedDirToEnemyEyes.Dot(vRight)) * (180.0f / M_PI) -
          90.0f;
  }
...
void CNPC_SecurityCamera::EyeOn(void) {
  if (!m_hEyeGlow) {
    ...
    m_hEyeGlow = CSprite::SpriteCreate(SECURITY_CAMERA_GLOW_SPRITE,
...
void CNPC_SecurityCamera::InputRagdoll(inputdata_t &inputdata) {
  ...
  // Leave decal on wall (may want to disable this once decal for where cam
  // touches wall is made)
  Vector vForward;
  GetVectors(&vForward, NULL, NULL);

  trace_t tr;
  UTIL_TraceLine(GetAbsOrigin() + 10.0f * vForward,
                 GetAbsOrigin() - 60.0f * vForward, MASK_SHOT, this,
...
void PlayDismountSounds() {
  ...
  CAI_BaseActor *pGlaDOS =
      (CAI_BaseActor *)gEntList.FindEntityByName(NULL, "Aperture_AI");

  if (!pPlayer || !pGlaDOS) {
    DevMsg(2, "Could not play CNPC_SecurityCamera dismount scene, make sure "
              "actor named 'Aperture_AI' is present in map.\n");
    return;
  }
  ...
  // If glados is currently talking, don't let her talk over herself or
  // interrupt a potentially important speech. Should we play the dismount sound
  // after she's done? or is that too disjointed from the camera dismounting act
  // to make sense...
  ...
  {
    // Play different sounds based on progress towards security camera knockdown
    // total.
