...
void FinishClientPutInServer(CPortal_Player *pPlayer) {
  ...
  for (char *pApersand = sName; pApersand != NULL && *pApersand != 0;
       pApersand++) {
...
void ClientActive(edict_t *pEdict, bool bLoadGame) {
  ...
  CPortal_Player *pPlayer =
      dynamic_cast<CPortal_Player *>(CBaseEntity::Instance(pEdict));
  Assert(pPlayer);
...
*/
const char *GetGameDescription() {
  if (g_pGameRules) // this function may be called before the world has spawned,
                    // and the game rules initialized
...
CBaseEntity *FindEntity(edict_t *pEdict, char *classname) {
  ...
  if (FStrEq(classname, "")) {
    return (FindPickerEntityClass(
        static_cast<CBasePlayer *>(GetContainingEntity(pEdict)), classname));
  }
  return NULL;
}
...
//=========================================================
void InstallGameRules() { CreateGameRulesObject("CPortalMPGameRules"); }
