...
void CPortal_Player::UpdateWooshSounds(void) {
  if (m_pWooshSound) {
    ...
    //		controller.SoundChangePitch( m_pWooshSound, fWooshVolume + 0.5f,
    //0.1f );
  }
}
...
const Vector &CPortal_Player::WorldSpaceCenter() const {
  ...
  m_vWorldSpaceCenterHolder.z +=
      ((IsDucked()) ? (VEC_DUCK_HULL_MAX_SCALED(this).z)
                    : (VEC_HULL_MAX_SCALED(this).z)) *
      0.5f;
...
void CPortal_Player::VPhysicsShadowUpdate(IPhysicsObject *pPhysics) {
  ...
  // code with some minor tweaks to avoid getting stuck in stuff when in a
  // portal environment
...
void CPortal_Player::PlayerRunCommand(CUserCmd *ucmd, IMoveHelper *moveHelper) {
  if (m_bFixEyeAnglesFromPortalling) {
    ...
    // handle this here, we end up sending back their old angles which makes
  ...
  for (int i = 0; i < m_Local.m_chAreaBits.Count(); i++) {
    for (int j = 0; j < iPortalCount; ++j) {
      ...
      if (bUsePortalForVis[j]) {
        m_Local.m_chAreaBits.Set(i, m_Local.m_chAreaBits[i] |
                                        portalTempBits[(j * 32) + i]);
      }
    }
  }
}
