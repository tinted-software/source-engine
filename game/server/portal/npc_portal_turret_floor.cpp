...
const char *g_TalkNames[] = {
    ...
    NULL // Must have NULL at end of list in case more states are added to base
         // turret!
};

const char *g_PortalTalkNames[PORTAL_TURRET_STATE_TOTAL - TURRET_STATE_TOTAL] =
    {"NPC_FloorTurret.TalkDisabled", "NPC_FloorTurret.TalkCollide",
     "NPC_FloorTurret.TalkPickup", "NPC_FloorTurret.TalkShotAt",
...
DEFINE_ARRAY(m_hRopes, FIELD_EHANDLE, PORTAL_FLOOR_TURRET_NUM_ROPES),
    ...
    DEFINE_THINKFUNC(HeldThink), DEFINE_THINKFUNC(InactiveThink),
    DEFINE_THINKFUNC(SuppressThink), DEFINE_THINKFUNC(DisabledThink),
    ...
    END_DATADESC()
        ...
        IMPLEMENT_SERVERCLASS_ST(CNPC_Portal_FloorTurret,
...
void CNPC_Portal_FloorTurret::Precache(void) {
  ...
  for (int iTalkScript = 0; iTalkScript < PORTAL_TURRET_STATE_TOTAL;
       ++iTalkScript) {
    if (iTalkScript < TURRET_STATE_TOTAL) {
      if (g_TalkNames[iTalkScript])
        ...
        iTalkScript =
            TURRET_STATE_TOTAL -
            1; // We hit the last script item, so jump to the portal only states
    } else {
...
float CNPC_Portal_FloorTurret::GetAutoAimRadius() { return 64.0f; }
...
// Purpose:
//-----------------------------------------------------------------------------
void CNPC_Portal_FloorTurret::OnPhysGunPickup(CBasePlayer *pPhysGunUser,
                                              PhysGunPickup_t reason) {
...
void CNPC_Portal_FloorTurret::NotifySystemEvent(
...
bool CNPC_Portal_FloorTurret::PreThink(turretState_e state) {
  ...
  if (m_iLastState != iNewState &&
      ((iNewState == TURRET_TIPPED && !m_bDelayTippedTalk) ||
       iNewState == TURRET_RETIRING || iNewState == PORTAL_TURRET_DISSOLVED ||
...
// Purpose: Fire!
//-----------------------------------------------------------------------------
void CNPC_Portal_FloorTurret::Shoot(const Vector &vecSrc,
                                    const Vector &vecDirToEnemy, bool bStrict) {
  ...
  // Shoot out of the left barrel if there's nothing solid between the turret's
  // center and the muzzle
  trace_t tr;
  GetAttachment(m_iBarrelAttachments[iBarrelIndex], info.m_vecSrc,
                angBarrelDir);
  Vector vecCenter = GetAbsOrigin();
  UTIL_TraceLine(vecCenter, info.m_vecSrc, MASK_SHOT, this,
                 COLLISION_GROUP_NONE, &tr);
  if (!tr.m_pEnt || !tr.m_pEnt->IsWorld()) {
    FireBullets(info);
  }

  // Shoot out of the right barrel if there's nothing solid between the turret's
  // center and the muzzle
  GetAttachment(m_iBarrelAttachments[iBarrelIndex + 1], info.m_vecSrc,
                angBarrelDir);
  ...
  // If a turret is partially tipped the recoil with each shot so that it can
  // knock itself over
  Vector up;
...
// Purpose:
//-----------------------------------------------------------------------------
Vector CNPC_Portal_FloorTurret::GetAttackSpread(CBaseCombatWeapon *pWeapon,
                                                CBaseEntity *pTarget) {
...
void CNPC_Portal_FloorTurret::ActiveThink(void) {
  ...
  if (pEnemy->IsAlive()) {
    ...
    if (pPortal && FVisibleThroughPortal(pPortal, pEnemy)) {
      ...
      float flDistToEnemyTransformed =
          VectorNormalize(vecDirToEnemyTransformed);

      // If it's not visible through normal means or the enemy is closer through
      // the portal, use the translated info
      if (!bEnemyInFOV || !bEnemyVisible ||
          flDistToEnemyTransformed < m_flDistToEnemy) {
        bEnemyInFOV = true;
        bEnemyVisible = true;
        vecMidEnemy = vecMidEnemyTransformed;
  ...
  if ((bEnemyVisible == false) ||
      (m_flDistToEnemy > PORTAL_FLOOR_TURRET_RANGE)) {
    ...
    if (bCanShoot) // 10 degree slop XY
    {
      ...
      if (m_bOutOfAmmo) {
        ...
        if (dot3d >= minCos3d) {
          ...
          SetActivity((Activity)((m_bShootWithBottomBarrels)
...
void CNPC_Portal_FloorTurret::SearchThink(void) {
  ...
  if (pEnemy != NULL) {
    ...
    m_flShotTime = gpGlobals->curtime + fDistanceRatio * fDistanceRatio *
  ...
  m_vecGoalAngles.x =
      GetAbsAngles().x +
      (sin((m_flLastSight + gpGlobals->curtime * m_fSearchSpeed) * 1.5f) *
       20.0f);
  m_vecGoalAngles.y =
      GetAbsAngles().y +
      (sin((m_flLastSight + gpGlobals->curtime * m_fSearchSpeed) * 2.5f) *
       20.0f);
...
void CNPC_Portal_FloorTurret::HackFindEnemy(void) {
  ...
  if (GetEnemy() == NULL) {
    ...
    float flClosestDistSqr =
        PORTAL_FLOOR_TURRET_RANGE * PORTAL_FLOOR_TURRET_RANGE;
...
void CNPC_Portal_FloorTurret::StartTouch(CBaseEntity *pOther) {
  ...
  if (m_pMotionController->Enabled()) {
    ...
    if (!pOther->IsPlayer() && pOther->GetMoveType() == MOVETYPE_VPHYSICS &&
        !(pTurretPhys &&
          ((pTurretPhys->GetGameFlags() & FVPHYSICS_PLAYER_HELD) != 0))) {
      ...
      Vector vAngleImpulse((vTurretRight.Dot(vVelocityImpulse) < 0.0f) ? (-1.6f)
                                                                       : (1.6f),
      ...
      CNPC_Portal_FloorTurret *pPortalFloor =
          dynamic_cast<CNPC_Portal_FloorTurret *>(pOther);
      if (pPortalFloor && pPortalFloor->m_lifeState == LIFE_ALIVE) {
        ...
        if (vOtherVelocity.LengthSqr() > vTurretVelocity.LengthSqr()) {
          ...
          // Delay out potential tipped talking so we can here the other turret
          // talk
      ...
      if (pPortalFloor && m_bEnabled && m_bLaserOn && !m_bOutOfAmmo) {
        // Award friendly fire achievement if we're a live turret being knocked
        // over by another turret.
...
void CNPC_Portal_FloorTurret::LaserOff(void) { m_bLaserOn = false; }

void CNPC_Portal_FloorTurret::LaserOn(void) { m_bLaserOn = true; }
...
void CNPC_Portal_FloorTurret::RopesOn(void) {
  for (int iRope = 0; iRope < PORTAL_FLOOR_TURRET_NUM_ROPES; ++iRope) {
    ...
    if (!m_hRopes[iRope]) {
      ...
      int iStartIndex =
          LookupAttachment(str.sprintf("Wire%i_start", iRope + 1));
      int iEndIndex = LookupAttachment(str.sprintf("Wire%i_end", iRope + 1));

      m_hRopes[iRope] =
          CRopeKeyframe::Create(this, this, iStartIndex, iEndIndex);
      if (m_hRopes[iRope]) {
