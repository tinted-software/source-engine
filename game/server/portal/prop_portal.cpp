...
ConVar sv_portal_placement_never_fail("sv_portal_placement_never_fail", "0",
                                      FCVAR_REPLICATED | FCVAR_CHEAT);
ConVar sv_portal_new_velocity_check("sv_portal_new_velocity_check", "1",
                                    FCVAR_CHEAT);
...
DEFINE_FIELD(m_hLinkedPortal, FIELD_EHANDLE),
    ...
    DEFINE_THINKFUNC(TestRestingSurfaceThink), DEFINE_THINKFUNC(FizzleThink),

    DEFINE_INPUTFUNC(FIELD_BOOLEAN, "SetActivatedState",
    ...
    SendPropBool(SENDINFO(m_bActivated)), SendPropBool(SENDINFO(m_bIsPortal2)),
    END_SEND_TABLE()
...
CProp_Portal::CProp_Portal(void) {
  ...
  // create the collision shape.... TODO: consider having one shared collideable
  // between all portals
  float fPlanes[6 * 4];
  fPlanes[(0 * 4) + 0] = 1.0f;
  fPlanes[(0 * 4) + 1] = 0.0f;
  ...
  CPolyhedron *pPolyhedron =
      GeneratePolyhedronFromPlanes(fPlanes, 6, 0.00001f, true);
...
void CProp_Portal::CreateSounds() {
  if (!m_pAmbientSound) {
    ...
    m_pAmbientSound =
        controller.SoundCreate(filter, entindex(), "Portal.ambient_loop");
...
void CProp_Portal::Spawn(void) {
  ...
  SetSolid(SOLID_OBB);
  SetSolidFlags(FSOLID_TRIGGER | FSOLID_NOT_SOLID | FSOLID_CUSTOMBOXTEST |
...
void CProp_Portal::OnRestore() {
  ...
  if (m_bActivated) {
    DispatchParticleEffect(
        ((m_bIsPortal2) ? ("portal_2_particles") : ("portal_1_particles")),
        PATTACH_POINT_FOLLOW, this, "particles_2", true);
    DispatchParticleEffect(
        ((m_bIsPortal2) ? ("portal_2_edge") : ("portal_1_edge")),
        PATTACH_POINT_FOLLOW, this, "particlespin");
  }
}

void DumpActiveCollision(const CPortalSimulator *pPortalSimulator,
                         const char *szFileName);
void PortalSimulatorDumps_DumpCollideToGlView(CPhysCollide *pCollide,
                                              const Vector &origin,
                                              const QAngle &angles,
                                              float fColorScale,
                                              const char *pFilename);

bool CProp_Portal::TestCollision(const Ray_t &ray, unsigned int fContentsMask,
                                 trace_t &tr) {
  physcollision->TraceBox(ray, MASK_ALL, NULL, m_pCollisionShape,
                          GetAbsOrigin(), GetAbsAngles(), &tr);
  return tr.DidHit();
}

//-----------------------------------------------------------------------------
// Purpose: Runs when a fired portal shot reaches it's destination wall. Detects
// current placement valididty state.
//-----------------------------------------------------------------------------
void CProp_Portal::DelayedPlacementThink(void) {
  ...
  if (m_iDelayedFailure != PORTAL_FIZZLE_BAD_SURFACE &&
    ...
    if (IsPortalOverlappingOtherPortals(this, m_vDelayedPosition,
                                        m_qDelayedAngles)) {
      m_iDelayedFailure = PORTAL_FIZZLE_OVERLAPPED_LINKED;
    } else if (IsPortalIntersectingNoPortalVolume(m_vDelayedPosition,
                                                  m_qDelayedAngles, vForward)) {
  ...
  CWeaponPortalgun *pPortalGun =
      dynamic_cast<CWeaponPortalgun *>(m_hPlacedBy.Get());

  if (pPortalGun) {
    CPortal_Player *pFiringPlayer =
        dynamic_cast<CPortal_Player *>(pPortalGun->GetOwner());
    if (pFiringPlayer) {
...
//-----------------------------------------------------------------------------
// Purpose: When placed on a surface that could potentially go away (anything
// but world geo), we test for that condition and fizzle
//-----------------------------------------------------------------------------
void CProp_Portal::TestRestingSurfaceThink(void) {
  ...
  for (int iCorner = 0; iCorner < 4; ++iCorner) {
    ...
    enginetrace->TraceRay(ray, MASK_SOLID_BRUSHONLY, &traceFilterPortalShot,
                          &tr);

    // This corner isn't on a valid brush (skipping phys converts or physboxes
    // because they frequently go through portals and can't be placed upon).
...
void CProp_Portal::DoFizzleEffect(int iEffect, bool bDelayedPos /*= true*/) {
  ...
  if (pPortalGun && (iEffect != PORTAL_FIZZLE_CLOSE) &&
      (iEffect != PORTAL_FIZZLE_SUCCESS) && (iEffect != PORTAL_FIZZLE_NONE)) {
    CBasePlayer *pPlayer = (CBasePlayer *)pPortalGun->GetOwner();
    if (pPlayer) {
      pPlayer->RumbleEffect(RUMBLE_PORTAL_PLACEMENT_FAILURE, 0,
                            RUMBLE_FLAGS_NONE);
    }
  }
  ...
  case PORTAL_FIZZLE_CANT_FIT:
    ...
    DispatchParticleEffect(
        ((m_bIsPortal2) ? ("portal_2_nofit") : ("portal_1_nofit")),
        fxData.m_vOrigin, fxData.m_vAngles, this);
    break;
  ...
  case PORTAL_FIZZLE_OVERLAPPED_LINKED: {
    ...
    if ( pLinkedPortal )
    {
            ...
            fxData.m_vStart = pLink3edPortal->GetAbsOrigin() + vLinkedForward
    * 5.0f;
    }*/
    ...
    VectorAngles(vUp, vForward, fxData.m_vAngles);
    DispatchParticleEffect(
        ((m_bIsPortal2) ? ("portal_2_overlap") : ("portal_1_overlap")),
        fxData.m_vOrigin, fxData.m_vAngles, this);
    ep.m_pSoundName = "Portal.fizzle_invalid_surface";
    break;
  }
  ...
  case PORTAL_FIZZLE_BAD_VOLUME:
    ...
    VectorAngles(vUp, vForward, fxData.m_vAngles);
    DispatchParticleEffect(
        ((m_bIsPortal2) ? ("portal_2_badvolume") : ("portal_1_badvolume")),
  ...
  case PORTAL_FIZZLE_BAD_SURFACE:
    ...
    VectorAngles(vUp, vForward, fxData.m_vAngles);
    DispatchParticleEffect(
        ((m_bIsPortal2) ? ("portal_2_badsurface") : ("portal_1_badsurface")),
        fxData.m_vOrigin, fxData.m_vAngles);
    ep.m_pSoundName = "Portal.fizzle_invalid_surface";
    break;
  ...
  case PORTAL_FIZZLE_KILLED:
    ...
    VectorAngles(vUp, vForward, fxData.m_vAngles);
    DispatchParticleEffect(
        ((m_bIsPortal2) ? ("portal_2_close") : ("portal_1_close")),
        fxData.m_vOrigin, fxData.m_vAngles);
  ...
  case PORTAL_FIZZLE_CLEANSER:
    ...
    VectorAngles(vUp, vForward, fxData.m_vAngles);
    DispatchParticleEffect(
        ((m_bIsPortal2) ? ("portal_2_cleanser") : ("portal_1_cleanser")),
        fxData.m_vOrigin, fxData.m_vAngles, this);
    ep.m_pSoundName = "Portal.fizzle_invalid_surface";
    break;
  ...
  case PORTAL_FIZZLE_CLOSE:
    ...
    DispatchParticleEffect(
        ...
        fxData.m_vOrigin, fxData.m_vAngles);
    ep.m_pSoundName =
        ((m_bIsPortal2) ? ("Portal.close_red") : ("Portal.close_blue"));
    break;
  ...
  case PORTAL_FIZZLE_NEAR_BLUE: {
    if (!m_bIsPortal2) {
      ...
      fxData.m_vOrigin =
          m_hLinkedPortal->GetAbsOrigin() + vLinkedForward * 16.0f;
    ...
    VectorAngles(vUp, vForward, fxData.m_vAngles);
    DispatchParticleEffect(
        ((m_bIsPortal2) ? ("portal_2_near") : ("portal_1_near")),
        fxData.m_vOrigin, fxData.m_vAngles);
    ep.m_pSoundName = "Portal.fizzle_invalid_surface";
    break;
  }
  ...
  case PORTAL_FIZZLE_SUCCESS:
    VectorAngles(vUp, vForward, fxData.m_vAngles);
    DispatchParticleEffect(
        ((m_bIsPortal2) ? ("portal_2_success") : ("portal_1_success")),
        fxData.m_vOrigin, fxData.m_vAngles);
    // Don't make a sound!
    return;
...
void CProp_Portal::Fizzle(void) {
  SetContextThink(&CProp_Portal::FizzleThink, gpGlobals->curtime,
                  s_pFizzleThink);
}

//-----------------------------------------------------------------------------
// Purpose: Removes the portal microphone and speakers. This is done in two
// places
//			(fizzle and UpdateOnRemove) so the code is consolidated
//here.
// Input  :  -
//-----------------------------------------------------------------------------
void CProp_Portal::RemovePortalMicAndSpeaker() {
  ...
  if (m_hSpeaker) {
    ...
    if (pSpeaker) {
      // Remove the remote portal's microphone, as it references the speaker
      ...
      if (m_hLinkedPortal.Get()) {
        ...
        if (pRemotePortal->m_hMicrophone) {
          ...
          CEnvMicrophone *pRemotePortalMic = dynamic_cast<CEnvMicrophone *>(
              pRemotePortal->m_hMicrophone.Get());
          if (pRemotePortalMic) {
...
void CProp_Portal::PunchPenetratingPlayer(CBaseEntity *pPlayer) {
  if (m_PortalSimulator.IsReadyToSimulate()) {
    ...
    if (pCollideable) {
      ...
      if (UTIL_IsBoxIntersectingPortal((vMin + vMax) / 2.0f,
                                       (vMax - vMin) / 2.0f, this)) {
        Vector vForward;
      ...
      for (touchlink_t *link = root->nextLink; link != root;
        ...
        if (CProp_Portal_Shared::IsEntityTeleportable(pOther)) {
          ...
          if (m_plane_Origin.normal.Dot(ptOtherCenter) > m_plane_Origin.dist) {
            // we should be interacting with this object, add it to our
            // environment
            if (SharedEnvironmentCheck(pOther)) {
              Assert(((m_PortalSimulator.GetLinkedPortalSimulator() == NULL) &&
                     ...
                     (m_PortalSimulator.GetLinkedPortalSimulator() ==
                      &m_hLinkedPortal
                           ->m_PortalSimulator)); // make sure this entity is
                                                  // linked to the same portal
                                                  // as our simulator

              CPortalSimulator *pOwningSimulator =
                  CPortalSimulator::GetSimulatorThatOwnsEntity(pOther);
...
bool CProp_Portal::ShouldTeleportTouchingEntity(CBaseEntity *pOther) {
  if (!m_PortalSimulator.OwnsEntity(
          pOther)) // can't teleport an entity we don't own
  {
#if !defined(DISABLE_DEBUG_HISTORY)
    if (!IsMarkedForDeletion()) {
      ADD_DEBUG_HISTORY(HISTORY_PLAYER_DAMAGE,
                        UTIL_VarArgs("Portal %i not teleporting %s because "
                                     "it's not simulated by this portal.\n",
                                     ((m_bIsPortal2) ? (2) : (1)),
                                     pOther->GetDebugName()));
    }
#endif
    if (sv_portal_debug_touch.GetBool()) {
      Msg("Portal %i not teleporting %s because it's not simulated by this "
          "portal. : %f \n",
  ...
  if (m_hLinkedPortal.Get() == NULL) {
    ...
    if (!IsMarkedForDeletion()) {
      ADD_DEBUG_HISTORY(HISTORY_PLAYER_DAMAGE,
                        UTIL_VarArgs("Portal %i not teleporting %s because it "
                                     "has no linked partner portal.\n",
                                     ((m_bIsPortal2) ? (2) : (1)),
                                     pOther->GetDebugName()));
    }
#endif
    if (sv_portal_debug_touch.GetBool()) {
      Msg("Portal %i not teleporting %s because it has no linked partner "
          "portal.\n",
          ((m_bIsPortal2) ? (2) : (1)), pOther->GetDebugName());
    }
    return false;
  ...
  // grab current velocity
  {
    if (sv_portal_new_velocity_check.GetBool()) {
      // we're assuming that physics velocity is the most reliable of all if the
      // convar is true
      if (pOtherPhysObject) {
      ...
      // old style of velocity grabbing, which uses implicit velocity as a last
      // resort
      if (pOther->GetMoveType() == MOVETYPE_VPHYSICS) {
        if (pOtherPhysObject &&
            (pOtherPhysObject->GetShadowController() == NULL))
          pOtherPhysObject->GetVelocity(&vOtherVelocity, NULL);
        else
          pOther->GetVelocity(&vOtherVelocity);
      } else {
      ...
      if (vOtherVelocity == vec3_origin) {
        // Recorded velocity is sometimes zero under pushed or teleported
        // movement, or after position correction. In these circumstances, we
        // want implicit velocity ((last pos - this pos) / timestep )
        if (pOtherPhysObject) {
  ...
  if (m_PortalSimulator.m_DataAccess.Placement.PortalPlane.m_Normal.Dot(
          ptOtherCenter) <
    ...
    if (m_PortalSimulator.EntityIsInPortalHole(pOther)) {
      ...
      if (!IsMarkedForDeletion()) {
        ADD_DEBUG_HISTORY(HISTORY_PLAYER_DAMAGE,
                          UTIL_VarArgs("Portal %i not teleporting %s because "
                                       "it was not in the portal hole.\n",
                                       ((m_bIsPortal2) ? (2) : (1)),
                                       pOther->GetDebugName()));
      }
#endif
      if (sv_portal_debug_touch.GetBool()) {
        Msg("Portal %i not teleporting %s because it was not in the portal "
            "hole.\n",
            ((m_bIsPortal2) ? (2) : (1)), pOther->GetDebugName());
      }
    }
...
void CProp_Portal::TeleportTouchingEntity(CBaseEntity *pOther) {
  if (GetPortalCallQueue()) {
    GetPortalCallQueue()->QueueCall(this, &CProp_Portal::TeleportTouchingEntity,
                                    pOther);
    return;
  }
  ...
  bool bNonPhysical = false; // special case handling for non-physical objects
                             // such as the energy ball and player
  ...
  // grab current velocity
  {
    ...
    if (vOtherVelocity == vec3_origin) {
      // Recorded velocity is sometimes zero under pushed or teleported
      // movement, or after position correction. In these circumstances, we want
      // implicit velocity ((last pos - this pos) / timestep )
      if (pOtherPhysObject) {
        Vector vOtherImplicitVelocity;
        pOtherPhysObject->GetImplicitVelocity(&vOtherImplicitVelocity, NULL);
        vOtherVelocity += vOtherImplicitVelocity;
  ...
  const PS_InternalData_t &RemotePortalDataAccess =
      m_hLinkedPortal->m_PortalSimulator.m_DataAccess;
  const PS_InternalData_t &LocalPortalDataAccess =
      m_PortalSimulator.m_DataAccess;
  ...
  if (bPlayer) {
    ...
    bNonPhysical = true;
    // if( (fabs( RemotePortalDataAccess.Placement.vForward.z ) + fabs(
    // LocalPortalDataAccess.Placement.vForward.z )) > 0.7071f ) //some
    // combination of floor/ceiling
    if (fabs(LocalPortalDataAccess.Placement.vForward.z) > 0.0f) {
      ...
      //(fabs( LocalPortalDataAccess.Placement.vForward.z +
      //RemotePortalDataAccess.Placement.vForward.z ) < 0.01f) )
      {
  ...
  // apply transforms to relevant variables (applied to the entity later)
  {
    ...
    // Reorient object angles, originally we did a transformation on the angles,
    // but that doesn't quite work right for gimbal lock cases
    qNewAngles =
        TransformAnglesToWorldSpace(qOtherAngles, m_matrixThisToLinked.As3x4());
  ...
  if (bPlayer) {
    ...
    float fPlayerFaceDotPortalFace =
        LocalPortalDataAccess.Placement.vForward.Dot(vPlayerForward);
    float fPlayerFaceDotPortalUp =
        LocalPortalDataAccess.Placement.vUp.Dot(vPlayerForward);

    CBaseEntity *pHeldEntity = GetPlayerHeldEntity(pOtherAsPlayer);

    // Sometimes reorienting by pitch is more desirable than by roll depending
    // on the portals' orientations and the relative player facing direction
    if (pHeldEntity) // never pitch reorient while holding an object
    {
      pOtherAsPlayer->m_bPitchReorientation = false;
    } else if (LocalPortalDataAccess.Placement.vUp.z >
                   0.99f &&               // entering wall portal
               (fForwardLength == 0.0f || // facing strait up or down
                fPlayerFaceDotPortalFace >
                    0.5f || // facing mostly away from portal
      ...
      pOtherAsPlayer->m_bPitchReorientation = true;
                ...
                LocalPortalDataAccess.Placement.vForward.z <
                    -0.99f) && // entering floor or ceiling portal
               (RemotePortalDataAccess.Placement.vForward.z > 0.99f ||
                RemotePortalDataAccess.Placement.vForward.z <
                    -0.99f) && // exiting floor or ceiling portal
               (fPlayerForwardZ < -0.5f ||
                fPlayerForwardZ > 0.5f) // facing mustly up or down
    ) {
      pOtherAsPlayer->m_bPitchReorientation = true;
    } else if ((RemotePortalDataAccess.Placement.vForward.z > 0.75f &&
                RemotePortalDataAccess.Placement.vForward.z <=
                    0.99f) && // exiting wedge portal
               (fPlayerFaceDotPortalUp >
                0.0f) // facing toward the top of the portal
    ) {
      pOtherAsPlayer->m_bPitchReorientation = true;
    } else {
      pOtherAsPlayer->m_bPitchReorientation = false;
  ...
  // velocity hacks
  {
    // minimum floor exit velocity if both portals are on the floor or the
    // player is coming out of the floor
    ...
    if (vNewVelocity.LengthSqr() >
        (MAXIMUM_PORTAL_EXIT_VELOCITY * MAXIMUM_PORTAL_EXIT_VELOCITY))
  ...
  if (!IsMarkedForDeletion()) {
    ADD_DEBUG_HISTORY(HISTORY_PLAYER_DAMAGE,
                      UTIL_VarArgs("PORTAL %i TELEPORTING: %s\n",
                                   ((m_bIsPortal2) ? (2) : (1)),
                                   pOther->GetClassname()));
  }
#endif
  ...
  // do the actual teleportation
  {
    ...
    if (bPlayer) {
      QAngle qTransformedEyeAngles = TransformAnglesToWorldSpace(
          qPlayerEyeAngles, m_matrixThisToLinked.As3x4());
      ...
      if (bNonPhysical) {
        ...
        // doing velocity in two stages as a bug workaround, setting the
        // velocity to anything other than 0 will screw up how objects rest on
  ...
  if ((pPhys != NULL) && (pPhys->GetGameFlags() & FVPHYSICS_PLAYER_HELD)) {
    CPortal_Player *pHoldingPlayer =
        (CPortal_Player *)GetPlayerHoldingEntity(pOther);
    ...
    if (pHeldEntity) {
      ...
      if (pOtherAsPlayer->IsHeldObjectOnOppositeSideOfPortal()) {
        ...
        // we need to make sure the held object and player don't interpenetrate
        // when the player's shape changes
        ...
        FindClosestPassableSpace(pHeldEntity,
                                 RemotePortalDataAccess.Placement.vForward);
      }
    }

    // we haven't found a good way of fixing the problem of "how do you reorient
    // an AABB". So we just move the player so that they fit
  ...
  // force the entity to be touching the other portal right this millisecond
  {
    ...
    // UTIL_TraceEntity( pOther, ptNewOrigin, ptNewOrigin, MASK_SOLID, pOther,
    // COLLISION_GROUP_NONE, &Trace ); //fires off some asserts, and we just
    // need a dummy anyways
...
#ifdef _DEBUG
  {
    ...
    fNewDist = RemotePortalDataAccess.Placement.PortalPlane.m_Normal.Dot(
                   ptTestCenter) -
               RemotePortalDataAccess.Placement.PortalPlane.m_Dist;
    fOldDist = LocalPortalDataAccess.Placement.PortalPlane.m_Normal.Dot(
                   ptOtherCenter) -
               LocalPortalDataAccess.Placement.PortalPlane.m_Dist;
    AssertMsg(fNewDist >= 0.0f,
              "Entity portalled behind the destination portal.");
  }
#endif
  ...
  Assert((bPlayer == false) ||
         (pOtherAsPlayer->m_hPortalEnvironment.Get() == m_hLinkedPortal.Get()));
}
...
void CProp_Portal::Touch(CBaseEntity *pOther) {
  ...
  if (!m_bActivated || (m_hLinkedPortal.Get() == NULL)) {
    ...
    Assert(!pOther->IsPlayer() ||
           (((CPortal_Player *)pOther)->m_hPortalEnvironment.Get() != this));
  ...
  Assert(((m_PortalSimulator.GetLinkedPortalSimulator() == NULL) &&
         ...
         (m_PortalSimulator.GetLinkedPortalSimulator() ==
          &m_hLinkedPortal
               ->m_PortalSimulator)); // make sure this entity is linked to the
  ...
  if (vVelocityCheck != vec3_origin || vAngularImpulseCheck != vec3_origin) {
    if (modelinfo->GetModelType(pOther->GetModel()) == mod_brush) {
          ...
          !FClassnameIs(pOther, "simple_physics_brush")) // except CPhysBox
      {
        ...
        if (UTIL_IsBoxIntersectingPortal(
                (vMin + vMax) / 2.0f,
                (vMax - vMin) / 2.0f - Vector(2.0f, 2.0f, 2.0f), this, 0.0f)) {
          ...
          UTIL_TraceLine(vOrigin, vOrigin - vForward * PORTAL_HALF_DEPTH,
  ...
  // see if we should even be interacting with this object, this is a bugfix
  // where some objects get added to physics environments through walls
  if (!m_PortalSimulator.OwnsEntity(pOther)) {
    ...
    if (SharedEnvironmentCheck(pOther)) {
      bool bObjectCenterInFrontOfPortal =
          (m_plane_Origin.normal.Dot(pOther->WorldSpaceCenter()) >
           m_plane_Origin.dist);
      bool bIsStuckPlayer =
          (pOther->IsPlayer())
              ? (!UTIL_IsSpaceEmpty(pOther, pOther->WorldAlignMins(),
                                    pOther->WorldAlignMaxs()))
              : (false);

      if (bIsStuckPlayer) {
        Assert(!"Player stuck");
        DevMsg("Player in solid behind behind portal %i's plane, Adding to "
               "it's environment to run find closest passable space.\n",
               ((m_bIsPortal2) ? (2) : (1)));
      }
...
void CProp_Portal::StartTouch(CBaseEntity *pOther) {
  ...
  // Since prop_portal is a trigger it doesn't send back start touch, so I'm
  // forcing it
  ...
  if (CProp_Portal_Shared::IsEntityTeleportable(pOther)) {
    ...
    if (m_plane_Origin.normal.Dot(ptOtherCenter) > m_plane_Origin.dist) {
      ...
      if (SharedEnvironmentCheck(pOther)) {
        Assert(
            ((m_PortalSimulator.GetLinkedPortalSimulator() == NULL) &&
             (m_hLinkedPortal.Get() == NULL)) ||
            (m_PortalSimulator.GetLinkedPortalSimulator() ==
             &m_hLinkedPortal
                  ->m_PortalSimulator)); // make sure this entity is linked to
                                         // the same portal as our simulator
...
void CProp_Portal::EndTouch(CBaseEntity *pOther) {
  ...
  // Since prop_portal is a trigger it doesn't send back end touch, so I'm
  // forcing it
  ...
  if (ShouldTeleportTouchingEntity(
          pOther)) // an object passed through the plane and all the way out of
                   // the touch box
    TeleportTouchingEntity(pOther);
  else if (pOther->IsPlayer() && // player
           (m_PortalSimulator.m_DataAccess.Placement.vForward.z <
            -0.7071f) && // most likely falling out of the portal
           (m_PortalSimulator.m_DataAccess.Placement.PortalPlane.m_Normal.Dot(
                pOther->WorldSpaceCenter()) <
            m_PortalSimulator.m_DataAccess.Placement.PortalPlane
                .m_Dist) && // but behind the portal plane
           (((CPortal_Player *)pOther)->m_Local.m_bInDuckJump)) // while ducking
  {
    // player has pulled their feet up (moving their center instantaneously)
    // while falling downward out of the portal, send them back (probably only
    // for a frame)

    DevMsg("Player pulled feet above the portal they fell out of, postponing "
           "Releasing ownership\n");
    // TeleportTouchingEntity( pOther );
  } else
    m_PortalSimulator.ReleaseOwnershipOfEntity(pOther);
...
void CProp_Portal::PortalSimulator_ReleasedOwnershipOfEntity(
    CBaseEntity *pEntity) {
  if (pEntity->IsPlayer() &&
      (((CPortal_Player *)pEntity)->m_hPortalEnvironment.Get() == this))
...
bool CProp_Portal::SharedEnvironmentCheck(CBaseEntity *pEntity) {
  ...
  if ((ptCenter - m_PortalSimulator.m_DataAccess.Placement.ptCenter)
          .LengthSqr() <
      (ptCenter - pOwningSimulator->m_DataAccess.Placement.ptCenter)
          .LengthSqr())
    return true;
  ...
  /*if( !m_hLinkedPortal->m_PortalSimulator.EntityIsInPortalHole( pEntity ) )
  {
          ...
          if( vOtherVelocity.Dot(
  m_PortalSimulator.m_DataAccess.Placement.vForward ) < vOtherVelocity.Dot(
  m_hLinkedPortal->m_PortalSimulator.m_DataAccess.Placement.vForward ) ) return
  true; //entity is going towards this portal more than the other
  }*/
  return false;

  // we're in the shared configuration, and the other portal already owns the
  // object, see if we'd be a better caretaker (distance check
...
void CProp_Portal::WakeNearbyEntities(void) {
  ...
  vForward *=
      CProp_Portal_Shared::vLocalMaxs.x - CProp_Portal_Shared::vLocalMins.x;
  vRight *=
      CProp_Portal_Shared::vLocalMaxs.y - CProp_Portal_Shared::vLocalMins.y;
  ...
  for (int i = 0; i < count; i++) {
    ...
    if (pEntity && (pEntity != this)) {
      ...
      // double check intersection at the OBB vs OBB level, we don't want to
      // affect large piles of physics objects if we don't have to. It gets slow
      if (IsOBBIntersectingOBB(
              ptOrigin, qAngles, CProp_Portal_Shared::vLocalMins,
              CProp_Portal_Shared::vLocalMaxs, ptEntityCenter,
        ...
        if (FClassnameIs(pEntity, "func_portal_detector")) {
          ...
          CFuncPortalDetector *pPortalDetector =
              static_cast<CFuncPortalDetector *>(pEntity);

          if (pPortalDetector->IsActive() &&
              pPortalDetector->GetLinkageGroupID() == m_iLinkageGroupID) {
            ...
            if (UTIL_IsBoxIntersectingPortal(vBoxCenter, vBoxExtents, this)) {
              ...
              if (m_bIsPortal2)
                pPortalDetector->m_OnStartTouchPortal2.FireOutput(
                    this, pPortalDetector);
              else
                pPortalDetector->m_OnStartTouchPortal1.FireOutput(
                    this, pPortalDetector);

              if (IsActivedAndLinked()) {
                pPortalDetector->m_OnStartTouchLinkedPortal.FireOutput(
                    this, pPortalDetector);

                if (UTIL_IsBoxIntersectingPortal(vBoxCenter, vBoxExtents,
                                                 m_hLinkedPortal)) {
                  pPortalDetector->m_OnStartTouchBothLinkedPortals.FireOutput(
                      this, pPortalDetector);
                }
              }
            }
...
void CProp_Portal::ForceEntityToFitInPortalWall(CBaseEntity *pEntity) {
  ...
  Vector ptCenter =
      pEntity->WorldSpaceCenter(); //(vWorldMins + vWorldMaxs) / 2.0f;
  ...
  Vector ptProjectedEntityCenter =
      ptPortalCenter +
      (vPortalForward * vPortalCenterToEntityCenter.Dot(vPortalForward));

  Vector ptDest;

  if (m_PortalSimulator.IsReadyToSimulate()) {
    Ray_t ray;
    ray.Init(ptProjectedEntityCenter, ptCenter, vWorldMins - ptCenter,
             vWorldMaxs - ptCenter);

    trace_t ShortestTrace;
    ShortestTrace.fraction = 2.0f;

    if (m_PortalSimulator.m_DataAccess.Simulation.Static.Wall.Local.Brushes
            .pCollideable) {
      physcollision->TraceBox(ray,
                              m_PortalSimulator.m_DataAccess.Simulation.Static
    ...
    /*if( pEnvironment->LocalCollide.pWorldCollide )
    {
            trace_t TempTrace;
            physcollision->TraceBox( ray,
    pEnvironment->LocalCollide.pWorldCollide, vec3_origin, vec3_angle,
    &TempTrace ); if( TempTrace.fraction < ShortestTrace.fraction )
    ...
    if( pEnvironment->LocalCollide.pWallShellCollide )
    {
            trace_t TempTrace;
            physcollision->TraceBox( ray,
    pEnvironment->LocalCollide.pWallShellCollide, vec3_origin, vec3_angle,
    &TempTrace ); if( TempTrace.fraction < ShortestTrace.fraction )
                    ShortestTrace = TempTrace;
    }
    ...
    if( pEnvironment->LocalCollide.pRemoteWorldWallCollide )
    {
            trace_t TempTrace;
            physcollision->TraceBox( ray,
    pEnvironment->LocalCollide.pRemoteWorldWallCollide, vec3_origin, vec3_angle,
    &TempTrace ); if( TempTrace.fraction < ShortestTrace.fraction )
                    ShortestTrace = TempTrace;
    }
...
void CProp_Portal::UpdatePortalTeleportMatrix(void) {
  ...
  if (m_hLinkedPortal != NULL) {
    CProp_Portal_Shared::UpdatePortalTransformationMatrix(
        EntityToWorldTransform(), m_hLinkedPortal->EntityToWorldTransform(),
        &m_matrixThisToLinked);

    m_hLinkedPortal->ResetModel();
    // update the remote portal
    MatrixInverseTR(m_matrixThisToLinked,
                    m_hLinkedPortal->m_matrixThisToLinked);
  } else {
    m_matrixThisToLinked
        .Identity(); // don't accidentally teleport objects to zero space
  }
...
void CProp_Portal::UpdatePortalLinkage(void) {
  if (m_bActivated) {
    ...
    if (!(pLink && pLink->m_bActivated)) {
      ...
      if (iPortalCount != 0) {
        CProp_Portal **pPortals =
            s_PortalLinkageGroups[m_iLinkageGroupID].Base();
        for (int i = 0; i != iPortalCount; ++i) {
          CProp_Portal *pCurrentPortal = pPortals[i];
          if (pCurrentPortal == this)
            continue;
          if (pCurrentPortal->m_bActivated &&
    ...
    if (pLink != NULL) {
      ...
      if (m_hMicrophone == 0) {
        ...
        CEnvMicrophone *pMicrophone =
            static_cast<CEnvMicrophone *>(m_hMicrophone.Get());
        pMicrophone->AddSpawnFlags(SF_MICROPHONE_IGNORE_NONATTENUATED);
        pMicrophone->AddSpawnFlags(
            SF_MICROPHONE_SOUND_COMBAT | SF_MICROPHONE_SOUND_WORLD |
      ...
      if (m_hLinkedPortal->m_hMicrophone == 0) {
        ...
        CEnvMicrophone *pLinkedMicrophone =
            static_cast<CEnvMicrophone *>(m_hLinkedPortal->m_hMicrophone.Get());
        pLinkedMicrophone->AddSpawnFlags(SF_MICROPHONE_IGNORE_NONATTENUATED);
        pLinkedMicrophone->AddSpawnFlags(
            SF_MICROPHONE_SOUND_COMBAT | SF_MICROPHONE_SOUND_WORLD |
            SF_MICROPHONE_SOUND_PLAYER | SF_MICROPHONE_SOUND_BULLET_IMPACT |
            SF_MICROPHONE_SOUND_EXPLOSION);
        ...
        CSpeaker *pLinkedSpeaker =
            static_cast<CSpeaker *>(m_hLinkedPortal->m_hSpeaker.Get());
...
void CProp_Portal::PlacePortal(const Vector &vOrigin, const QAngle &qAngles,
  ...
  if (fPlacementSuccess < 0.5f) {
    ...
    if (pPortalGun) {
      CPortal_Player *pFiringPlayer =
          dynamic_cast<CPortal_Player *>(pPortalGun->GetOwner());
      if (pFiringPlayer) {
        g_PortalGameStats.Event_PortalPlacement(pFiringPlayer->GetAbsOrigin(),
                                                vOrigin, m_iDelayedFailure);
      }
    }
...
void CProp_Portal::NewLocation(const Vector &vOrigin, const QAngle &qAngles) {
  ...
  // Fast moving objects can pass through the hole this frame while it's in the
  // old location.
  m_PortalSimulator.ReleaseAllEntityOwnership();
  Vector vOldForward;
...
void CProp_Portal::InputSetActivatedState(inputdata_t &inputdata) {
  ...
  if (m_bActivated) {
    ...
    float fPlacementSuccess =
        VerifyPortalPlacement(this, tr.endpos, qAngles, PORTAL_PLACED_BY_FIXED);
    PlacePortal(tr.endpos, qAngles, fPlacementSuccess);

    // If the fixed portal is overlapping a portal that was placed before it...
    // kill it!
...
//-----------------------------------------------------------------------------
// Purpose: Map can call new location, so far it's only for debugging purposes
// so it's not made to be very robust. Input  : &inputdata - String with 6 float
// entries with space delimiters, location and orientation
//-----------------------------------------------------------------------------
...
void CProp_Portal::UpdateCorners() {
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    vAddPoint +=
        vRight * ((i & (1 << 0)) ? (PORTAL_HALF_WIDTH) : (-PORTAL_HALF_WIDTH));
    vAddPoint +=
        vUp * ((i & (1 << 1)) ? (PORTAL_HALF_HEIGHT) : (-PORTAL_HALF_HEIGHT));
...
CProp_Portal *CProp_Portal::FindPortal(unsigned char iLinkageGroupID,
                                       bool bPortal2,
...
const CUtlVector<CProp_Portal *> *
CProp_Portal::GetPortalLinkageGroup(unsigned char iLinkageGroupID) {
