...
DEFINE_FIELD(m_hLinkedPortal, FIELD_EHANDLE),
    ...
    DEFINE_KEYFIELD(m_strInitialLinkedPortal, FIELD_STRING,
                    "InitialLinkedPortal"),
    DEFINE_KEYFIELD(m_fFillTime, FIELD_FLOAT, "FillTime"),

    // Inputs
    DEFINE_INPUTFUNC(FIELD_STRING, "SetLinkedLiquidPortal",
    ...
    DEFINE_INPUTFUNC(FIELD_VOID, "AddActivatorToTeleportList",
                     InputAddActivatorToTeleportList),
    DEFINE_INPUTFUNC(FIELD_VOID, "RemoveActivatorFromTeleportList",
  ...
  m_matrixThisToLinked.Identity(); // Zero space is a bad place. No heroes to
                                   // face, but we need 1's in this case.
}
...
void CFunc_LiquidPortal::Spawn(void) {
  ...
  CBaseEntity *pBaseEnt =
      gEntList.FindEntityByName(NULL, STRING(m_strInitialLinkedPortal));
  Assert((pBaseEnt == NULL) ||
         (dynamic_cast<CFunc_LiquidPortal *>(pBaseEnt) != NULL));
...
int CFunc_LiquidPortal::Save(ISave &save) {
  ...
  if (iLeftToTeleportThisFillCount != 0)
    save.WriteEHandle(m_hLeftToTeleportThisFill.Base(),
...
int CFunc_LiquidPortal::Restore(IRestore &restore) {
  ...
  if (!FStrEq(szBlockName,
              "LiquidPortal")) // loading a save without liquid portal save data
    return 1;
  ...
  if (iLeftToTeleportThisFillCount != 0) {
    ...
    restore.ReadEHandle(m_hLeftToTeleportThisFill.Base(),
                        iLeftToTeleportThisFillCount);
  }
...
void CFunc_LiquidPortal::InputAddActivatorToTeleportList(
...
void CFunc_LiquidPortal::InputRemoveActivatorFromTeleportList(
    inputdata_t &inputdata) {
  if (inputdata.pActivator == NULL)
    return;
  ...
  for (int i = m_hTeleportList.Count(); --i >= 0;) {
    if (m_hTeleportList[i].Get() == inputdata.pActivator) {
      ...
      if (inputdata.pActivator->IsPlayer() &&
          (((CPortal_Player *)inputdata.pActivator)
               ->m_hSurroundingLiquidPortal.Get() == this))
        ((CPortal_Player *)inputdata.pActivator)->m_hSurroundingLiquidPortal =
            NULL;
...
void CFunc_LiquidPortal::TeleportImmersedEntity(CBaseEntity *pEntity) {
  ...
  if (pEntity->IsPlayer()) {
    ...
    QAngle qNewAngles = TransformAnglesToWorldSpace(
        pEntityAsPlayer->EyeAngles(), m_matrixThisToLinked.As3x4());
    Vector vNewVelocity =
        m_matrixThisToLinked.ApplyRotation(pEntity->GetAbsVelocity());
    ...
    Vector vNewOrigin = m_matrixThisToLinked * pEntity->GetAbsOrigin();
    QAngle qNewAngles = TransformAnglesToWorldSpace(
        pEntity->GetAbsAngles(), m_matrixThisToLinked.As3x4());
    Vector vNewVelocity =
        m_matrixThisToLinked.ApplyRotation(pEntity->GetAbsVelocity());
...
void CFunc_LiquidPortal::Think(void) {
  if (m_bFillInProgress) {
    if (gpGlobals->curtime < m_fFillEndTime) {
      float fInterp = ((gpGlobals->curtime - m_fFillStartTime) /
                       (m_fFillEndTime - m_fFillStartTime));
      Vector vMins, vMaxs;
      ...
      for (int i = m_hLeftToTeleportThisFill.Count(); --i >= 0;) {
        ...
        pEntity->GetCollideable()->WorldSpaceSurroundingBounds(&vEntMins,
                                                               &vEntMaxs);
