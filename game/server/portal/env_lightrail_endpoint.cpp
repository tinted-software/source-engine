...
DEFINE_KEYFIELD(m_flSmallScale, FIELD_FLOAT, "small_fx_scale"),
    ...
    END_DATADESC()
        ...
        IMPLEMENT_SERVERCLASS_ST(CEnv_Lightrail_Endpoint,
  ...
  BaseClass::Precache();
  PrecacheMaterial("effects/light_rail_endpoint"); // Sprite used for the
                                                   // effects
}
...
void CEnv_Lightrail_Endpoint::Spawn(void) {
  ...
  if (HasSpawnFlags(SF_ENDPOINT_START_SMALLFX)) {
    m_nState = (int)
        ENDPOINT_STATE_LARGEFX; // THIS NEEDS TO BE CHANGED TO SMALL FX STATE
...
CBaseViewModel *IsViewModelMoveParent_(CBaseEntity *pEffect) {
  if (pEffect->GetMoveParent()) {
    CBaseViewModel *pViewModel =
        dynamic_cast<CBaseViewModel *>(pEffect->GetMoveParent());
