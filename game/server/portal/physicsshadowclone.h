...
private:
  EHANDLE
      m_hClonedEntity; // the entity we're supposed to be cloning the physics of
  VMatrix
      m_matrixShadowTransform; // all cloned coordinates and angles will be run
                               // through this matrix before being applied
  VMatrix m_matrixShadowTransform_Inverse;

  CUtlVector<PhysicsObjectCloneLink_t>
      m_CloneLinks; // keeps track of which of our physics objects are linked to
                    // the source's objects
  bool m_bShadowTransformIsIdentity; // the shadow transform doesn't update
                                     // often, so we can cache this
  bool m_bImmovable; // cloning a track train or door, something that doesn't
                     // really work on a force-based level
  bool m_bInAssumedSyncState;

  void FullSyncClonedPhysicsObjects(bool bTeleport);
  void SyncEntity(bool bPullChanges);

  IPhysicsEnvironment *m_pOwnerPhysEnvironment; // clones exist because of
                                                // multi-environment situations
...
public:
  ...
  // do the thing with the stuff, you know, the one that goes
  ...
  // syncs to the source entity in every way possible, assumed sync does some
  // rudimentary tests to see if the object is in sync, and if so, skips the
  // update
  void FullSync(bool bAllowAssumedSync = false);

  // syncs just the physics objects, bPullChanges should be true when this clone
  // should match it's source, false when it should force differences onto the
  // source entity
  ...
  static CPhysicsShadowClone *
  CreateShadowClone(IPhysicsEnvironment *pInPhysicsEnvironment,
                    EHANDLE hEntToClone, const char *szDebugMarker,
                    const matrix3x4_t *pTransformationMatrix = NULL);

  // given a physics object that is part of this clone, tells you which physics
...
class CTraceFilterTranslateClones
    : public CTraceFilter // give it another filter, and it'll translate shadow
                          // clones into their source entity for tests
{
  ITraceFilter *m_pActualFilter; // the filter that tests should be forwarded to
                                 // after translating clones

public:
  CTraceFilterTranslateClones(ITraceFilter *pOtherFilter)
      : m_pActualFilter(pOtherFilter) {};
