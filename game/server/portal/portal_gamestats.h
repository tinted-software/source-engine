...
// #define PORTAL_GAMESTATS_VERBOSE //verbose logging of extra stats, only a
// good idea during internal tests, centralized here for easy on/off

// NEVER REMOVE A STRUCTURE OR CHANGE A CHUNKID. If you need to change how a
// chunk works, make a new structure with a new ID

struct Portal_Gamestats_LevelStats_t // most of this is only tracked in verbose
                                     // mode
{
  ...
  struct PlayerDeaths_t {
    ...
    static const unsigned short TRIMSIZE =
        200; // trim logs if more than this many entries exist for a single map
  ...
  struct PortalPlacement_t {
    ...
    static const unsigned short TRIMSIZE =
        1000; // trim logs if more than this many entries exist for a single map
  ...
  struct PlayerUse_t {
    ...
    static const unsigned short TRIMSIZE =
        500; // trim logs if more than this many entries exist for a single map
    Vector ptTraceStart;
    Vector vTraceDelta;
  ...
  struct StuckEvent_t {
    ...
    static const unsigned short TRIMSIZE =
        100; // trim logs if more than this many entries exist for a single map
  ...
  struct LeafTimes_t {
    ...
    static const unsigned short TRIMSIZE =
        10000; // trim logs if more than this many entries exist for a single
               // map
    float fTimeSpentInVisLeaf;
    LeafTimes_t(void) : fTimeSpentInVisLeaf(0.0f) {};
  };

  // these are created/destroyed by parent CPortalGameStats to avoid
  // create/copy/destroy confusion
  ...
  void LoadSubChunksFromBuffer(CUtlBuffer &LoadBuffer,
...
public:
  ...
  void Event_PortalPlacement(const Vector &ptPlayerFiredFrom,
                             const Vector &ptAttemptedPosition,
                             char iSuccessCode);
  void Event_PlayerJump(const Vector &ptStartPosition,
                        const Vector &vStartVelocity);
  void Event_PlayerUsed(const Vector &ptTraceStart, const Vector &vTraceDelta,
