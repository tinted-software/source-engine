...
ConVar turret_ground_damage_multiplier("turret_ground_damage_multiplier", "8.0",
                                       FCVAR_CHEAT);
...
DEFINE_KEYFIELD(m_fViewconeDegrees, FIELD_FLOAT, "ConeOfFire"),
    ...
    END_DATADESC()
        ...
        LINK_ENTITY_TO_CLASS(npc_portal_turret_ground,
...
void CNPC_Portal_GroundTurret::Shoot() {
  ...
  for (int i = 0; i < 1; i++) {
    ...
    trace_t tr;
    CTraceFilterSkipTwoEntities traceFilter(this, info.m_pAdditionalIgnoreEnt,
                                            COLLISION_GROUP_NONE);
    Vector vecEnd = info.m_vecSrc + vecDir * info.m_flDistance;
    AI_TraceLine(info.m_vecSrc, vecEnd, MASK_SHOT, &traceFilter, &tr);

    if (tr.m_pEnt && !tr.m_pEnt->IsPlayer() &&
        (vecDir * info.m_flDistance * tr.fraction).Length() < 16.0f) {
