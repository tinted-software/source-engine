...
template <>
CFuncPortalOrientation *CEntityClassList<CFuncPortalOrientation>::m_pClassList =
    NULL;
...
//-----------------------------------------------------------------------------
// Purpose: Test for func_orientation_volume ents which could effect the
// placement angles of a portal. Input  : vecCurAngles - Default angles to place
// (may change)
//			vecCurOrigin - origin of the portal on placement
//			pPortal - The portal attempting to place
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool UTIL_TestForOrientationVolumes(QAngle &vecCurAngles,
                                    const Vector &vecCurOrigin,
                                    const CProp_Portal *pPortal) {
  if (!pPortal)
    return false;

  // Walk list of orientation volumes, obb test each with candidate portal
  CFuncPortalOrientation *pList =
      g_FuncPortalOrientationVolumeList.m_pClassList;
  while (pList) {
    if (!pList->IsActive()) {
      pList = pList->m_pNext;
      continue;
    ...
    if (IsOBBIntersectingOBB(
            vecCurOrigin, vecCurAngles, CProp_Portal_Shared::vLocalMins,
            CProp_Portal_Shared::vLocalMaxs, pList->GetAbsOrigin(),
            pList->GetCollideable()->GetCollisionAngles(),
            pList->GetCollideable()->OBBMins(),
      ...
      if (pList->m_bMatchLinkedAngles) {
        // This feature requires a linked portal on a floor or ceiling. Bail
        // without effecting the placement angles if we fail those requirements.
        CProp_Portal *pLinked = pPortal->m_hLinkedPortal.Get();
        if (!pLinked || !(AnglesAreEqual(vecCurAngles.x, -90.0f, 0.1f) ||
                          AnglesAreEqual(vecCurAngles.x, 90.0f, 0.1f)))
          return false;
...
//-----------------------------------------------------------------------------
// Purpose: Test for portals inside our volume when we switch on, and forcibly
// rotate them
//-----------------------------------------------------------------------------
void CFuncPortalOrientation::OnActivate(void) {
  ...
  if (iPortalCount != 0) {
    ...
    for (int i = 0; i != iPortalCount; ++i) {
      ...
      if (IsOBBIntersectingOBB(
              pTempPortal->GetAbsOrigin(), pTempPortal->GetAbsAngles(),
              CProp_Portal_Shared::vLocalMins, CProp_Portal_Shared::vLocalMaxs,
              GetAbsOrigin(), GetCollideable()->GetCollisionAngles(),
              GetCollideable()->OBBMins(), GetCollideable()->OBBMaxs())) {
        QAngle angNewAngles;
        ...
        pTempPortal->PlacePortal(pTempPortal->GetAbsOrigin(), angNewAngles,
                                 PORTAL_ANALOG_SUCCESS_NO_BUMP);
      }
    }
  }
}
