...
#define PORTALSTATS_WRITECHUNKSIZE(bufName, SizePositionVariable)              \
  {                                                                            \
    int SizePositionVariable##_askdjbhas =                                     \
        bufName.TellPut() - SizePositionVariable;                              \
...
class CPortalGameStatsSingleton // used to remove the constructor destructor
...
CPortalGameStats::~CPortalGameStats(void) { Clear(); }

void Portal_Gamestats_LevelStats_t::AppendSubChunksToBuffer(
  ...
  if (m_pTimeSpentInVisLeafs->Count() != 0) {
    ...
    for (int i = 0; i != iLeafTimeStatsCount; ++i) {
      ...
      SaveBuffer.PutFloat(
          LeafTimeStat
              .fTimeSpentInVisLeaf); // assumes visleafs will be the same when
                                     // this data is loaded again, or that there
                                     // will be a way to invalidate the data
    }
...
void Portal_Gamestats_LevelStats_t::LoadSubChunksFromBuffer(
    CUtlBuffer &LoadBuffer, unsigned int iChunkEndPosition) {
  Clear();

  while (((unsigned int)LoadBuffer.TellGet()) != iChunkEndPosition) {
    Assert((iChunkEndPosition - LoadBuffer.TellGet()) >
           (sizeof(unsigned short) +
            sizeof(unsigned int))); // at least an empty chunk left

    unsigned short iChunkID = LoadBuffer.GetUnsignedShort();
    unsigned int iChunkSize =
        LoadBuffer.GetUnsignedInt() -
        sizeof(unsigned int); // chunk size includes the chunk size data itself
#ifdef _DEBUG
    unsigned int iChunkEndPosition =
        LoadBuffer.TellGet() + iChunkSize; // used in an assert later
#endif

    switch (iChunkID) {
#ifdef PORTAL_GAMESTATS_VERBOSE // don't bother loading verbose chunks if we're
                                // not going to save them back out
...
#ifdef PORTAL_GAMESTATS_VERBOSE // we only have verbose chunks for now, so only
                                // write custom data if we're verbosely tracking

  SaveBuffer.PutUnsignedShort(PORTAL_GAMESTATS_VERSION);

  // no headers allowed, chunks were chosen for their flexibility in loading
  // even when parts of the data are unknown you can simulate a header by
  // enclosing the entirety of the custom data in a chunk that starts with a
  // header, but you'll kill loading in old versions

  if (m_CustomMapStats.Count() != 0) // we have some map stats
  {
    // put out a map chunk for each map
    for (int i = m_CustomMapStats.First(); i != m_CustomMapStats.InvalidIndex();
         i = m_CustomMapStats.Next(i)) {
      SaveBuffer.PutShort(Portal_Gamestats_LevelStats_t::CHUNKID);

      // we can trivially find the chunk size after the chunk is written, but
      // chunk size needs to be at the beginning, reserve the space for chunk
      // size now
      ...
      // write out the map stats chunk size
      {
        ...
        Assert(iOldTellPut ==
               SaveBuffer.TellPut()); // writing the chunk size should have
...
void CPortalGameStats::LoadCustomDataFromBuffer(CUtlBuffer &LoadBuffer) {
  ...
  AssertOnce(iSaveStatsVersion <=
             PORTAL_GAMESTATS_VERSION); // useful to know, but shouldn't be a
                                        // failure case
#else
  ...
  while (LoadBuffer.TellGet() != iEndPosition) {
    ...
    switch (iChunkID) {
#ifdef PORTAL_GAMESTATS_VERBOSE // levelstats only have verbose data for the
                                // time being, so only bother to load
...
void CPortalGameStats::Event_PlayerKilled(CBasePlayer *pPlayer,
  ...
  Portal_Gamestats_LevelStats_t::PlayerDeaths_t &DeathStat =
      m_pCurrentMapStats->m_pDeaths->Element(index);
  ...
  if (pInflictor)
    Q_strncpy(DeathStat.szAttackerClassName, pInflictor->GetClassname(),
              sizeof(DeathStat.szAttackerClassName));
#endif
}

void CPortalGameStats::Event_PlayerJump(const Vector &ptStartPosition,
  ...
  Portal_Gamestats_LevelStats_t::JumpEvent_t &JumpStat =
      m_pCurrentMapStats->m_pJumps->Element(index);
...
void CPortalGameStats::Event_PortalPlacement(const Vector &ptPlayerFiredFrom,
                                             const Vector &ptAttemptedPosition,
                                             char iSuccessCode) {
#ifdef PORTAL_GAMESTATS_VERBOSE
  if (CBaseGameStats::StatTrackingAllowed() == false)
    return;
  ...
  Portal_Gamestats_LevelStats_t::PortalPlacement_t &PlacementStat =
      m_pCurrentMapStats->m_pPlacements->Element(index);
...
void CPortalGameStats::Event_PlayerUsed(const Vector &ptTraceStart,
                                        const Vector &vTraceDelta,
                                        CBaseEntity *pUsedEntity) {
#ifdef PORTAL_GAMESTATS_VERBOSE
  if (CBaseGameStats::StatTrackingAllowed() == false)
    return;
  ...
  if (fLastUseTime >
      gpGlobals->curtime) // I'm not positive, but I think curtime resets
                          // between levels, cheap to do this
    fLastUseTime = 0.0f;
  ...
  Portal_Gamestats_LevelStats_t::PlayerUse_t &UseEvent =
      m_pCurrentMapStats->m_pUseEvents->Element(index);
  ...
  if (pUsedEntity)
    Q_strncpy(UseEvent.szUseEntityClassName, pUsedEntity->GetClassname(),
              sizeof(UseEvent.szUseEntityClassName));
#endif
}
...
void CPortalGameStats::Event_PlayerStuck(CPortal_Player *pPlayer) {
  ...
  if ((gpGlobals->curtime - fLastStuckTime) <
      10.0f) // only log one stuck spot per 10 second interval (in case it
             // oscillates)
    return;
  ...
  Portal_Gamestats_LevelStats_t::StuckEvent_t &StuckSpot =
      m_pCurrentMapStats->m_pStuckSpots->Element(index);
...
void CPortalGameStats::Event_MapChange(const char *szOldMapName,
...
Portal_Gamestats_LevelStats_t *
CPortalGameStats::FindOrAddMapStats(const char *szMapName) {
...
void CreateLevelStatPointers(Portal_Gamestats_LevelStats_t *pFillIn) {
  pFillIn->m_pDeaths =
      new CUtlVector<Portal_Gamestats_LevelStats_t::PlayerDeaths_t>;
  pFillIn->m_pPlacements =
      new CUtlVector<Portal_Gamestats_LevelStats_t::PortalPlacement_t>;
  pFillIn->m_pUseEvents =
      new CUtlVector<Portal_Gamestats_LevelStats_t::PlayerUse_t>;
  pFillIn->m_pStuckSpots =
      new CUtlVector<Portal_Gamestats_LevelStats_t::StuckEvent_t>;
  pFillIn->m_pJumps =
      new CUtlVector<Portal_Gamestats_LevelStats_t::JumpEvent_t>;
  pFillIn->m_pTimeSpentInVisLeafs =
      new CUtlVector<Portal_Gamestats_LevelStats_t::LeafTimes_t>;
}
...
static char const *portalMaps[] = {
    ...
    "testchmb_a_13", "testchmb_a_14", "testchmb_a_15", "escape_00", "escape_01",
    "escape_02"};
