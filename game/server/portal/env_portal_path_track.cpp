...
ConVar sv_portal_pathtrack_track_width_on("sv_portal_pathtrack_track_width_on",
                                          "4.0", FCVAR_CHEAT);
...
DEFINE_FIELD(m_bTrackActive, FIELD_BOOLEAN),
    ...
    DEFINE_INPUTFUNC(FIELD_VOID, "DeactivateEndPointFX",
    ...
    SendPropBool(SENDINFO(m_bEndpointActive)), SendPropInt(SENDINFO(m_nState)),
...
CEnvPortalPathTrack::~CEnvPortalPathTrack() {
  ShutDownTrackFX(); // Make sure to deallocate the track beam and particle
                     // effects
}
...
void CEnvPortalPathTrack::Activate(void) {
  BaseClass::Activate(); // Link the happy friends so I know where my next
                         // target entity is
...
void CEnvPortalPathTrack::InitTrackFX() {
  m_pBeam =
      CBeam::BeamCreate("sprites/track_beam.vmt",
                        TRACK_FX_WIDTH_ON); // Allocate a mr. beamy, and set his
                                            // scroll sprite and width

  if (m_pnext) {
    m_pBeam->PointEntInit(
        GetAbsOrigin(),
        m_pnext); // Set up the beam to draw from its center to it's next track.
  }
...
//-----------------------------------------------------------------------------
// Purpose: Activate all of the track's beams (at least the ones that are
// flagged to display) Input  :  -
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Activate all of the endpoint's glowy bits that are flagged to
// display Input  :  -
//-----------------------------------------------------------------------------
