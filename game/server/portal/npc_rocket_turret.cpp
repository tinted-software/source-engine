...
const char *pRocketTurretFollowerBoneNames[] = {
    "Root",          "Base",   "Arm_1",     "Arm_2",      "Arm_3",
    "Arm_4",         "Rot_LR", "Rot_UD",    "Gun_casing", "Gun_Barrel_01",
    "gun_barrel_02", "loader", "missle_01", "missle_02",  "panel",
};
...
public:
  ...
  void
  OpeningThink(void); // Finish open/close animation before using pose params
  ...
  void RocketDied(
      void); // After rocket hits something and self-destructs (or times out)
  ...
  Vector EyeOffset(Activity nActivity) { return vec3_origin; }
...
DEFINE_FIELD(m_bEnabled, FIELD_BOOLEAN),
    ...
    DEFINE_THINKFUNC(SearchThink), DEFINE_THINKFUNC(FollowThink),
    DEFINE_THINKFUNC(LockingThink), DEFINE_THINKFUNC(FiringThink),
    DEFINE_THINKFUNC(DyingThink), DEFINE_THINKFUNC(DeathThink),
...
DEFINE_FIELD(m_hLauncher, FIELD_EHANDLE),
    ...
    END_DATADESC()
        ...
        LINK_ENTITY_TO_CLASS(rocket_turret_projectile,
...
CNPC_RocketTurret::CNPC_RocketTurret(void)
  ...
  m_flTimeLastFired = m_flTimeLocking = m_flDistToEnemy = m_flTimeSpentDying =
      0.0f;

  m_iLightAttachment = m_iMuzzleAttachment = m_nSiteHalo = 0;

  m_flTimeSpentPaused = m_flPauseLength = m_flTotalDivergenceX =
      m_flTotalDivergenceY = 0.0f;
...
bool CNPC_RocketTurret::CreateVPhysics(void) {
  m_BoneFollowerManager.InitBoneFollowers(
      this, ARRAYSIZE(pRocketTurretFollowerBoneNames),
...
//-----------------------------------------------------------------------------
bool CNPC_RocketTurret::FVisible(CBaseEntity *pEntity, int traceMask,
...
void CNPC_RocketTurret::UpdateAimPoint(void) {
  ...
  Vector vecMidEnemy =
      ...
      (GetEnemy()->WorldAlignMins() + GetEnemy()->WorldAlignMaxs()) * 0.5f;
  ...
  if (!(GetEnemy()->GetFlags() & FL_NOTARGET)) {
    ...
    bool bEnemyVisibleThroughPortal =
        TestPortalsForLOS(&vecMidEnemy, bEnemyVisibleInWorld);
...
bool SignDiffers(float f1, float f2) { return !(Sign(f1) == Sign(f2)); }
...
void CNPC_RocketTurret::FollowThink(void) {
  ...
  CProp_Portal *pFirstPortal =
      UTIL_Portal_FirstAlongRay(rayDmg, flRequiredParameter);
  UTIL_Portal_TraceRay_Bullets(pFirstPortal, rayDmg, MASK_VISIBLE_AND_NPCS,
                               &filter, &traceDmg, false);

  if (traceDmg.m_pEnt) {
    // This thing we're hurting is our enemy
    if (traceDmg.m_pEnt == GetEnemy()) {
      // If we're past the cooldown time, fire another rocket
      if ((gpGlobals->curtime - m_flTimeLastFired) >
          ROCKET_TURRET_ROCKET_FIRE_COOLDOWN_TIME) {
...
void CNPC_RocketTurret::LockingThink(void) {
  ...
  if (m_flTimeLocking == 0.0f) {
    ...
    EmitSound(ROCKET_TURRET_SOUND_LOCKING,
              gpGlobals->curtime + ROCKET_TURRET_QUARTER_LOCKON_TIME);
    EmitSound(ROCKET_TURRET_SOUND_LOCKED,
              gpGlobals->curtime + ROCKET_TURRET_HALF_LOCKON_TIME);
...
void CNPC_RocketTurret::FiringThink(void) {
  ...
  CRocket_Turret_Projectile *pRocket =
      dynamic_cast<CRocket_Turret_Projectile *>(m_hCurRocket.Get());

  if (pRocket) {
    // If this rocket has been out too long, detonate it and launch a new one
    if ((gpGlobals->curtime - m_flTimeLastFired) >
        ROCKET_PROJECTILE_DEFAULT_LIFE) {
      pRocket->ShotDown();
...
void CNPC_RocketTurret::FireRocket(void) {
  ...
  CRocket_Turret_Projectile *pRocket =
      (CRocket_Turret_Projectile *)CBaseEntity::Create(
...
void CNPC_RocketTurret::UpdateSkin(int nSkin) { m_nSkin = nSkin; }
...
//-----------------------------------------------------------------------------
// Purpose: Show this rocket turret has overloaded with effects and noise for a
// period of time
//-----------------------------------------------------------------------------
...
void CNPC_RocketTurret::SyncPoseToAimAngles(void) {
  QAngle localAngles = TransformAnglesToLocalSpace(m_vecCurrentAngles.Get(),
                                                   EntityToWorldTransform());

  // Update pitch
  SetPoseParameter(m_iPosePitch, localAngles.x);

  // Update yaw -- NOTE: This yaw movement is screwy for this model, we must
  // invert the yaw delta and also skew an extra 90 deg to get the 'forward
  // face' of the turret to match up with the look direction. If the model and
  // it's pose parameters change, this will be wrong.
...
float CNPC_RocketTurret::UpdateFacing(void) {
  ...
  // 1/10th degree is all the granularity we need, gives rocket player hit box
  // width accuracy at 18k game units.
  if (flDiff < 0.1)
    return flDiff;

  // Slerp 5% of the way to goal (distance dependant speed, but torque minimial
  // and no euler wrapping issues).
...
//-----------------------------------------------------------------------------
// Purpose: Tests if this prop's front point will have direct line of sight to
// it's target entity once the pose parameters are set to face it Input  :
// vAimPoint - The point to aim at Output : Returns true if target is in direct
...
bool CNPC_RocketTurret::TestLOS(const Vector &vAimPoint) {
  ...
  // This aim point does hit target, now make sure there are no blocking objects
  // in the way
...
//-----------------------------------------------------------------------------
// Purpose: Tests all portals in the turret's vis for possible routes to see
// it's target point Input  : pOutVec - The location to aim at in order to hit
// the target ent, choosing least rotation if multiple
//			bConsiderNonPortalAimPoint - Output in pOutVec the non
//portal (direct) aimpoint if it requires the least rotation
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_RocketTurret::TestPortalsForLOS(
    Vector *pOutVec, bool bConsiderNonPortalAimPoint = false) {
  ...
  Vector vAimPoint =
      ...
      (pTarget->WorldAlignMins() + pTarget->WorldAlignMaxs()) * 0.5f;
  ...
  for (int i = 0; i != iPortalCount; ++i) {
    ...
    if (1) {
      ...
      // This value is for choosing the easiest aim point for the turret to see
      // through. 'Easiest' is the least rotation needed.
...
//-----------------------------------------------------------------------------
// Purpose: Find the center of the target entity as seen through the specified
// portal Input  : pPortal - The portal to look through Output : Vector& output
// point in world space where the target *appears* to be as seen through the
// portal
//-----------------------------------------------------------------------------
bool CNPC_RocketTurret::FindAimPointThroughPortal(const CProp_Portal *pPortal,
                                                  Vector *pVecOut) {
...
//-----------------------------------------------------------------------------
void CNPC_RocketTurret::SetTarget(CBaseEntity *pTarget) { SetEnemy(pTarget); }
...
// Purpose:
//-----------------------------------------------------------------------------
void CNPC_RocketTurret::InputToggle(inputdata_t &inputdata) { Toggle(); }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CNPC_RocketTurret::InputEnable(inputdata_t &inputdata) { Enable(); }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CNPC_RocketTurret::InputDisable(inputdata_t &inputdata) { Disable(); }

void CNPC_RocketTurret::InputSetTarget(inputdata_t &inputdata) {
  CBaseEntity *pTarget =
      gEntList.FindEntityByName(NULL, inputdata.value.String(), NULL, NULL);
  SetTarget(pTarget);
}
...
// Input  : &inputdata -
//-----------------------------------------------------------------------------
void CNPC_RocketTurret::InputDestroy(inputdata_t &inputdata) { Destroy(); }
...
void CRocket_Turret_Projectile::NotifySystemEvent(
  ...
  if (eventType == NOTIFY_EVENT_TELEPORT) {
    ...
    // because they didn't ever collide. We do this after a portal teleport so
    // it avoids self-collisions on launch.
    SetOwnerEntity(NULL);

    // Restart smoke trail
    UTIL_Remove(m_hRocketTrail);
    m_hRocketTrail = NULL; // This shouldn't leak cause the pointer has been
                           // handed to the delete list
    CreateSmokeTrail();
  }
}
...
void CRocket_Turret_Projectile::DoExplosion(void) {
  ...
  while ((pTurretIter = gEntList.FindEntityByClassnameWithin(
              pTurretIter, "npc_portal_turret_floor", GetAbsOrigin(), 128)) !=
         NULL) {
    CTakeDamageInfo info(this, this, 200, DMG_BLAST);
    info.SetDamagePosition(GetAbsOrigin());
    CalculateExplosiveDamageForce(
        &info, (pTurretIter->GetAbsOrigin() - GetAbsOrigin()), GetAbsOrigin());
...
void CRocket_Turret_Projectile::CreateSounds() {
  if (!m_pAmbientSound) {
    ...
    m_pAmbientSound = controller.SoundCreate(filter, entindex(),
                                             ROCKET_PROJECTILE_LOOPING_SOUND);
...
static void fire_rocket_projectile_f(void) {
  ...
  CRocket_Turret_Projectile *pRocket =
      (CRocket_Turret_Projectile *)CBaseEntity::Create(
          "rocket_turret_projectile", ptEyes, vLookAng, pPlayer);
...
ConCommand fire_rocket_projectile(
    "fire_rocket_projectile", fire_rocket_projectile_f,
