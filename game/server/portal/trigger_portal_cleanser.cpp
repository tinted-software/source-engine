...
//
// Purpose: A volume which bumps portal placement. Keeps a global list loaded in
// from the map
//			and provides an interface with which prop_portal can get
//this list and avoid successfully 			creating portals partially inside the volume.
//
// $NoKeywords: $
//======================================================================================//
...
static char *g_pszPortalNonCleansable[] = {
    ...
    "prop_door_rotating", "func_tracktrain",
    "env_ghostanimating", "physicsshadowclone",
    "prop_energy_ball",   NULL,
};
...
// Used to avoid higher level functions on a disolving entity, which should be
// inert and not react the way it used to (touches, etc). Uses simple physics
// entities declared in physobj.cpp
...
void CTriggerPortalCleanser::Touch(CBaseEntity *pOther) {
  ...
  if (pOther->IsPlayer()) {
    ...
    if (pPlayer) {
      CWeaponPortalgun *pPortalgun = dynamic_cast<CWeaponPortalgun *>(
      ...
      if (pPortalgun) {
        ...
        if (pPortalgun->CanFirePortal1()) {
          CProp_Portal *pPortal = CProp_Portal::FindPortal(
              pPortalgun->m_iPortalLinkageGroupID, false);

          if (pPortal && pPortal->m_bActivated) {
            pPortal->DoFizzleEffect(PORTAL_FIZZLE_KILLED, false);
            pPortal->Fizzle();
            // HACK HACK! Used to make the gun visually change when going
            // through a cleanser!
          ...
          if (pPortal && pPortal->GetNextThink(s_pDelayedPlacementContext) >
                             gpGlobals->curtime) {
            pPortal->SetContextThink(NULL, gpGlobals->curtime,
        ...
        if (pPortalgun->CanFirePortal2()) {
          CProp_Portal *pPortal = CProp_Portal::FindPortal(
              pPortalgun->m_iPortalLinkageGroupID, true);
  ...
  if (pBaseAnimating && !pBaseAnimating->IsDissolving()) {
    ...
    // The portal weight box, used for puzzles in the portal mod is
    // differentiated by its name always being 'box'. We use special logic when
    // the cleanser dissolves a box so this is a special output for it.
    ...
    if (pOldPhys && (pOldPhys->GetGameFlags() & FVPHYSICS_PLAYER_HELD)) {
      CPortal_Player *pPlayer =
          (CPortal_Player *)GetPlayerHoldingEntity(pBaseAnimating);
      if (pPlayer) {
        ...
        vOldVel = pPlayer->GetAbsVelocity() +
                  Vector(pPlayer->EyeDirection2D().x * 4.0f,
                         pPlayer->EyeDirection2D().y * 4.0f, -32.0f);
      }
    }
    ...
    CBaseAnimating *pDisolvingAnimating =
        dynamic_cast<CBaseAnimating *>(pDisolvingObj);
    if (pDisolvingAnimating) {
      pDisolvingAnimating->Dissolve("", gpGlobals->curtime, false,
