...
//
// Purpose: Defines a combine ball and a combine ball launcher which have
// certain properties
...
public:
  ...
  CHandle<CProp_Portal>
      m_hTouchedPortal;    // Pointer to the portal we are touched most recently
  bool m_bTouchingPortal1; // Are we touching portal 1
  bool m_bTouchingPortal2; // Are we touching portal 2

  // Remember the last known direction of travel, incase our velocity is
  // cleared.
...
void CPropEnergyBall::CreateSounds() {
  if (!m_pAmbientSound) {
    ...
    m_pAmbientSound =
        controller.SoundCreate(filter, entindex(), "EnergyBall.AmbientLoop");
...
//-----------------------------------------------------------------------------
// Purpose: Keep a constant velocity despite collisions, make impact sounds and
// effects
//-----------------------------------------------------------------------------
void CPropEnergyBall::VPhysicsCollision(int index,
  ...
  if (m_bTouchingPortal2 || m_bTouchingPortal1) {
    AssertMsg(m_hTouchedPortal.Get(),
              "Touching a portal, but recorded an invalid handle.");
  }
  ...
  if ((m_bTouchingPortal2 || m_bTouchingPortal1) && m_hTouchedPortal.Get()) {
    // Force our velocity to be either towards or away from the portal, no
    // bouncing at odd angles allowed
    CProp_Portal *pPortal = m_hTouchedPortal.Get();

    // Only lock to the portal's forward axis if we're in it's world bounds
    // We use a tolerance of four, because the render bounds thickness for a
    // portal is 4, and this function intersects with a plane.
    bool bHitPortal = UTIL_IsBoxIntersectingPortal(
        GetAbsOrigin(), WorldAlignSize(), pPortal, 4.0f);
    ...
    if (bHitPortal && pPortal && pPortal->IsActivedAndLinked()) {
      ...
      if (fDot < 0) {
        ...
        // Since we're going 'through', don't do surfaceprop based collision
        // effects because the it will look like we didn't hit anything.
        pEvent->surfaceProps[0] = pEvent->surfaceProps[1] =
            physprops->GetSurfaceIndex("default");
        bIsEnteringPortalAndLockingAxisForward = true;
      } else // Closer to 'away from' the portal. Force the energy ball to go
             // that direction
      {
        vecFinalVelocity = vecTouchedPortalFace;
      }
  ...
  // Record our direction so our fixed direction hacks know we have changed
  ...
  // We will spam updates in our think function to help get us in the direction
  // we want to go.
  PhysCallbackSetVelocity(pEvent->pObjects[index], vecFinalVelocity);
}

void CPropEnergyBall::NotifySystemEvent(
  ...
  if (eventType == NOTIFY_EVENT_TELEPORT) {
    ...
    if (pEnteredPortal) {
      m_vLastKnownDirection =
          pEnteredPortal->m_matrixThisToLinked.ApplyRotation(
              m_vLastKnownDirection);
    ...
    // If an energy ball passes a portal (teleports), add a make sure its life
    // is >= sk_energy_ball_min_life_after_portal
    float fCurTimeTillDeath = GetNextThink("ExplodeTimerContext");
    // If we are set to die, then refresh that time if it is below a set
    // threshold
    if (fCurTimeTillDeath > 0) {
      ...
      SetContextThink(&CPropCombineBall::ExplodeThink,
                      gpGlobals->curtime + fTimeToDie, "ExplodeTimerContext");
    }
  }
...
//-----------------------------------------------------------------------------
// Purpose: Send down the time till death to the client code to help indicate
// when the ball will detonate Input  :  -
//-----------------------------------------------------------------------------
void CPropEnergyBall::Think() {
  // Finite life energy balls send the time till death down to the client for
  // display purposes
...
//-----------------------------------------------------------------------------
// Purpose: Make a sound/effect for the removal of the energy ball, and switch
// to the cleanup think Input  :  -
//-----------------------------------------------------------------------------
...
void CPropEnergyBall::StartTouch(CBaseEntity *pOther) {
  ...
  if (pOther->IsPlayer()) {
    CTakeDamageInfo info(this, GetOwnerEntity(), GetAbsVelocity(),
                         GetAbsOrigin(), 1500.0f, DMG_DISSOLVE);
    pOther->OnTakeDamage(info);

    // Destruct when we hit the player
    SetContextThink(&CPropCombineBall::ExplodeThink, gpGlobals->curtime,
                    "ExplodeTimerContext");
  }
  ...
  if (pPortal) {
    ...
    // The forward direction we want to follow is the forward vector of the
    // portal we've touched most recently
...
DEFINE_KEYFIELD(m_fBallLifetime, FIELD_FLOAT, "BallLifetime"),
    DEFINE_KEYFIELD(m_fMinBallLifeAfterPortal, FIELD_FLOAT,
                    "MinLifeAfterPortal"),
...
void CEnergyBallLauncher::SpawnBall() {
  CPropEnergyBall *pBall =
      static_cast<CPropEnergyBall *>(CreateEntityByName("prop_energy_ball"));
...
ConCommand fire_energy_ball("fire_energy_ball", fire_energy_ball_f,
