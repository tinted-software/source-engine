...
//			near-portal reactions, only clone from a larger area
//near portals.
//
// $NoKeywords: $
//=============================================================================//
...
const Vector
    CPhysicsCloneArea::vLocalMins(3.0f,
                                  -PORTAL_HALF_WIDTH *PHYSICSCLONEAREASCALE,
                                  -PORTAL_HALF_HEIGHT *PHYSICSCLONEAREASCALE);
const Vector CPhysicsCloneArea::vLocalMaxs(
    PORTAL_HALF_HEIGHT
        *PHYSICSCLONEAREASCALE, // x is the forward which is fairly thin for
                                // portals, replacing with halfheight
    PORTAL_HALF_WIDTH *PHYSICSCLONEAREASCALE,
    PORTAL_HALF_HEIGHT *PHYSICSCLONEAREASCALE);
...
void CPhysicsCloneArea::StartTouch(CBaseEntity *pOther) {
  ...
  if (!IsMarkedForDeletion()) {
    ADD_DEBUG_HISTORY(
        ...
        UTIL_VarArgs("PortalCloneArea %i Start Touch: %s : %f\n",
                     ((m_pAttachedPortal->m_bIsPortal2) ? (2) : (1)),
                     pOther->GetClassname(), gpGlobals->curtime));
  }
#endif
...
void CPhysicsCloneArea::EndTouch(CBaseEntity *pOther) {
  ...
  if (!IsMarkedForDeletion()) {
    ADD_DEBUG_HISTORY(
        HISTORY_PLAYER_DAMAGE,
        UTIL_VarArgs("PortalCloneArea %i End Touch: %s : %f\n",
                     ((m_pAttachedPortal->m_bIsPortal2) ? (2) : (1)),
                     pOther->GetClassname(), gpGlobals->curtime));
  }
#endif
...
int CPhysicsCloneArea::ObjectCaps(void) {
  return BaseClass::ObjectCaps() |
         FCAP_DONT_SAVE; // don't save this entity in any way, we naively
                         // recreate them
}
...
void CPhysicsCloneArea::UpdatePosition(void) {
  ...
  CloneNearbyEntities(); // wake new objects so they can figure out that they
                         // touch
}
...
void CPhysicsCloneArea::CloneNearbyEntities(void) {
  ...
  /*{
          ...
          NDebugOverlay::Box( ptAABBCenter, -vAABBExtent, vAABBExtent, 0, 0,
  255, 128, 10.0f );
  }*/
  ...
  for (int i = 0; i < count; i++) {
    ...
    if (pEntity && (pEntity != this)) {
      ...
      if (pPhysicsObject) {
        ...
        // affect large piles of physics objects if we don't have to, it gets
        // slow
        if (IsOBBIntersectingOBB(
                ptOrigin, qAngles, vLocalMins, vLocalMaxs, ptEntityCenter,
...
CPhysicsCloneArea *
CPhysicsCloneArea::CreatePhysicsCloneArea(CProp_Portal *pFollowPortal) {
  if (!pFollowPortal)
    return NULL;

  CPhysicsCloneArea *pCloneArea =
      (CPhysicsCloneArea *)CreateEntityByName("physicsclonearea");
