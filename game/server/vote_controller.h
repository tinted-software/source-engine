...
public:
  ...
  virtual void
  SetIssueDetails(const char *pszDetails); // We need to know the details part
                                           // of the con command for later
  virtual void OnVoteFailed(
      int iEntityHoldingVote); // The moment the vote fails, also has some time
                               // for feedback before the window goes away
  ...
  virtual bool
  CanCallVote(int nEntIndex, const char *pszDetails,
              vote_create_failed_t &nFailCode,
              int &nTime); // Can this guy hold a vote on this issue?
  virtual bool
  IsAllyRestrictedVote(void); // Can only members of the same team vote on this?
  virtual const char *GetDisplayString(
      void) = 0; // The string that will be passed to the client for display
  virtual void
  ExecuteCommand(void) = 0; // Where the magic happens.  Do your thing.
  ...
  virtual int
  GetNumberVoteOptions(void); // How many choices this vote will have.  i.e.
                              // Returns 2 on a Yes/No issue (the default).
  virtual bool IsYesNoVote(void);
  virtual void SetYesNoVoteCount(int iNumYesVotes, int iNumNoVotes,
                                 int iNumPotentialVotes);
  virtual bool
  GetVoteOptions(CUtlVector<const char *>
                     &vecNames); // We use this to generate options for voting
  virtual bool
  BRecordVoteFailureEventForEntity(int iVoteCallingEntityIndex) const {
    return iVoteCallingEntityIndex != DEDICATED_SERVER;
  }

protected:
  static void
  ListStandardNoArgCommand(CBasePlayer *forWhom,
...
public:
  ...
  void SendVoteFailedMessage(vote_create_failed_t nReason = VOTE_FAILED_GENERIC,
                             CBasePlayer *pVoteCaller = NULL, int nTime = -1);
...
protected:
  ...
  CNetworkVar(int, m_iOnlyTeamToVote);   // If an Ally restricted vote, the team
                                         // number that is allowed to vote
  ...
  CountdownTimer m_acceptingVotesTimer; // How long from vote start until we
                                        // count the ballots
  CountdownTimer m_executeCommandTimer; // How long after end of vote time until
                                        // we execute a passed vote
  CountdownTimer m_resetVoteTimer;      // when the current vote will end
  int m_nVotesCast[MAX_PLAYERS +
                   1]; // arrays are zero-based and player indices are one-based
