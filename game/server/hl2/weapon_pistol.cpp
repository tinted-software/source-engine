...
#define PISTOL_ACCURACY_SHOT_PENALTY_TIME                                      \
  0.2f // Applied amount of time each shot adds to the time we must recover from
...
DEFINE_FIELD(m_flSoonestPrimaryAttack, FIELD_TIME),
    ...
    DEFINE_FIELD(m_flAccuracyPenalty,
                 FIELD_FLOAT), // NOTENOTE: This is NOT tracking game time
...
// Output :
//-----------------------------------------------------------------------------
void CWeaponPistol::Operator_HandleAnimEvent(animevent_t *pEvent,
                                             CBaseCombatCharacter *pOperator) {
  switch (pEvent->event) {
  case EVENT_WEAPON_PISTOL_FIRE: {
    ...
    CSoundEnt::InsertSound(SOUND_COMBAT | SOUND_CONTEXT_GUNFIRE,
                           pOperator->GetAbsOrigin(), SOUNDENT_VOLUME_PISTOL,
...
void CWeaponPistol::PrimaryAttack(void) {
  ...
  CSoundEnt::InsertSound(SOUND_COMBAT, GetAbsOrigin(), SOUNDENT_VOLUME_PISTOL,
                         0.2, GetOwner());
  ...
  if (pOwner) {
    ...
    // prevents the aim from 'drifting off' when the player fires very quickly.
    // This may not be the ideal way to achieve this, but it's cheap and it
    // works, which is great for a feature we're evaluating. (sjb)
...
void CWeaponPistol::UpdatePenaltyTime(void) {
  ...
  if (((pOwner->m_nButtons & IN_ATTACK) == false) &&
    ...
    m_flAccuracyPenalty =
        clamp(m_flAccuracyPenalty, 0.0f, PISTOL_ACCURACY_MAXIMUM_PENALTY_TIME);
  }
}
