...
DEFINE_INPUTFUNC(FIELD_VOID, "Lock", InputLock),
    ...
    SendPropBool(SENDINFO(m_bMagnetOn)), SendPropBool(SENDINFO(m_bEnterAnimOn)),
...
void CPropCrane::RunCraneMovement(float flTime) {
  if (m_flExtensionRate) {
    ...
    m_flExtension =
        clamp(m_flExtension + (m_flExtensionRate * 10 * flTime), 0, 2);
    SetPoseParameter("armextensionpose", m_flExtension);
    StudioFrameAdvance();
  }
  ...
  magnet float flVolume = clamp( (m_hCraneMagnet->GetTotalMassAttachedObjects()
  * 0.5), 100.f, 600.f ); CSoundEnt::InsertSound( SOUND_DANGER, tr.endpos,
  flVolume, 0.2, this );
