...
static ConVar sk_suitcharger_citadel_maxarmor("sk_suitcharger_citadel_maxarmor",
                                              "0");
...
void CRecharge::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
  ...
  // if the player doesn't have the suit, or there is no juice left, make the
  // deny noise
  if (m_iJuice <= 0) {
    if (m_flSoundTime <= gpGlobals->curtime) {
      m_flSoundTime = gpGlobals->curtime + 0.62;
...
DEFINE_FIELD(m_flNextCharge, FIELD_TIME),
    ...
    DEFINE_FIELD(m_iJuice, FIELD_INTEGER), DEFINE_FIELD(m_iOn, FIELD_INTEGER),
    DEFINE_FIELD(m_flSoundTime, FIELD_TIME),
    DEFINE_FIELD(m_nState, FIELD_INTEGER), DEFINE_FIELD(m_iCaps, FIELD_INTEGER),
...
void CNewRecharge::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
                       USE_TYPE useType, float value) {
  // if it's not a player, ignore
  if (!pActivator || !pActivator->IsPlayer())
    return;
  ...
  if (HasSpawnFlags(SF_CITADEL_RECHARGER)) {
    ...
    // Also give health for the citadel version.
    if (pActivator->GetHealth() < pActivator->GetMaxHealth() &&
        m_flNextCharge < gpGlobals->curtime) {
      pActivator->TakeHealth(5, DMG_GENERIC);
    }
  }
  ...
  if (pPlayer->ArmorValue() >= nMaxArmor) {
    ...
    if (!HasSpawnFlags(SF_CITADEL_RECHARGER) ||
        (HasSpawnFlags(SF_CITADEL_RECHARGER) &&
         pActivator->GetHealth() >= pActivator->GetMaxHealth())) {
