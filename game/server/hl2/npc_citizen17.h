...
private:
  ...
  float
      m_fNextInspectTime; // Next time I'm allowed to get inspected by a scanner
  float m_flPlayerHealTime;
  float
      m_flNextHealthSearchTime; // Next time I'm allowed to look for a healthkit
  ...
  float m_flTimePlayerStare;   // The game time at which the player started
                               // staring at me.
  float m_flTimeNextHealStare; // Next time I'm allowed to heal a player who is
                               // staring at me.
  ...
  bool m_bNeverLeavePlayerSquad; // Don't leave the player squad unless killed,
                                 // or removed via Entity I/O.
...
private:
  float m_flResponseAddedTime[MAX_CITIZEN_RESPONSES]; // Time at which the
                                                      // response was added. 0
