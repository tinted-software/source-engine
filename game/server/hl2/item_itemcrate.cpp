...
protected:
  virtual void OnBreak(const Vector &vecVelocity, const AngularImpulse &angVel,
...
void CItem_ItemCrate::Spawn(void) {
  ...
  if (NULL_STRING == m_strItemClass) {
    Warning("CItem_ItemCrate(%i):  CRATE_SPECIFIC_ITEM with NULL ItemClass "
            "string (deleted)!!!\n",
            entindex());
    UTIL_Remove(this);
    return;
...
// Purpose:
//-----------------------------------------------------------------------------
void CItem_ItemCrate::VPhysicsCollision(int index,
...
// Purpose:
//-----------------------------------------------------------------------------
void CItem_ItemCrate::OnBreak(const Vector &vecVelocity,
  ...
  for (int i = 0; i < m_nItemCount; ++i) {
    ...
    Vector vecOrigin;
    CollisionProp()->RandomPointInBounds(Vector(0.25, 0.25, 0.25),
                                         Vector(0.75, 0.75, 0.75), &vecOrigin);
    ...
    // Avoid missing items drops by a dynamic resupply because they don't think
    // immediately
...
void CItem_ItemCrate::OnPhysGunPickup(CBasePlayer *pPhysGunUser,
                                      PhysGunPickup_t reason) {
  ...
  if (reason == PUNTED_BY_CANNON &&
      m_CrateAppearance != CRATE_APPEARANCE_RADAR_BEACON) {
    Vector vForward;
    AngleVectors(pPhysGunUser->EyeAngles(), &vForward, NULL, NULL);
    Vector vForce =
        Pickup_PhysGunLaunchVelocity(this, vForward, PHYSGUN_FORCE_PUNTED);
    ...
    TakeDamage(
        CTakeDamageInfo(pPhysGunUser, pPhysGunUser, GetHealth(), DMG_GENERIC));
  }
}
