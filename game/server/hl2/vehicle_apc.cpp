...
void CPropAPC::Activate() {
  ...
  // NOTE: gun_ref must have the same position as gun_base, but rotates with the
  // gun
...
// Input  : *pMoveData -
//-----------------------------------------------------------------------------
Class_T CPropAPC::ClassifyPassenger(CBaseCombatCharacter *pPassenger,
                                    Class_T defaultClassification) {
  return CLASS_COMBINE;
}

//-----------------------------------------------------------------------------
// Purpose: Damage events as modified for the passenger of the APC, not the APC
// itself
//-----------------------------------------------------------------------------
float CPropAPC::PassengerDamageModifier(const CTakeDamageInfo &info) {
  CTakeDamageInfo DmgInfo = info;

  // bullets, slashing and headbutts don't hurt us in the apc, neither do
  // rockets
  if ((DmgInfo.GetDamageType() & DMG_BULLET) ||
      (DmgInfo.GetDamageType() & DMG_SLASH) ||
...
Vector CPropAPC::EyePosition() {
  ...
  CollisionProp()->NormalizedToWorldSpace(Vector(0.5, 0.5, 1.0),
                                          &vecEyePosition);
...
//-----------------------------------------------------------------------------
inline bool CPropAPC::ShouldTriggerDamageEffect(int nPrevHealth,
                                                int nEffectCount) const {
  int nPrevRange =
  ...
  for (int i = 0; i < 5; i++) {
    ...
    te->Explosion(filter, random->RandomFloat(0.0, 1.0), &vecAbsPoint,
                  ...
                  (i < 2)
                      ...
                      : TE_EXPLFLAG_NOPARTICLES | TE_EXPLFLAG_NOFIREBALLSMOKE |
                            TE_EXPLFLAG_NODLIGHTS,
                  100, 0);
  }
...
void CPropAPC::InputFireMissileAt(inputdata_t &inputdata) {
  ...
  CBaseEntity *pTarget = gEntList.FindEntityByName(
      NULL, strMissileTarget, NULL, inputdata.pActivator, inputdata.pCaller);
  if (pTarget == NULL) {
    DevWarning("%s: Could not find target '%s'!\n", GetClassname(),
               STRING(strMissileTarget));
    return;
  }
...
void CPropAPC::ProcessMovement(CBasePlayer *pPlayer, CMoveData *pMoveData) {
  ...
  // put sounds a bit to left and right but slightly closer to APC to make a
  // "cone" of sound in front of it
...
void CPropAPC::AimSecondaryWeaponAt(CBaseEntity *pTarget) {
  ...
  if (m_hRocketTarget) {
    m_hLaserDot->SetAbsOrigin(
        m_hRocketTarget->BodyTarget(WorldSpaceCenter(), false));
  }
...
// Purpose:
//-----------------------------------------------------------------------------
void CPropAPC::DriveVehicle(float flFrameTime, CUserCmd *ucmd, int iButtonsDown,
...
void CPropAPC::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
...
void CPropAPC::AimPrimaryWeapon(const Vector &vecWorldTarget) {
  ...
  if (quadTarget > m_vecBarrelPos.LengthSqr()) {
    ...
    float centerToGunYaw =
        atan2(m_vecBarrelPos.y,
              sqrt(quadTarget - (m_vecBarrelPos.y * m_vecBarrelPos.y)));

    float targetToCenterPitch = atan2(target.z, sqrt(quadTargetXY));
    float centerToGunPitch =
        atan2(-m_vecBarrelPos.z,
              sqrt(quadTarget - (m_vecBarrelPos.z * m_vecBarrelPos.z)));
    ...
    m_bInFiringCone =
        (fabs(curPitch - angles.x) < 1e-3) && (fabs(curYaw - angles.y) < 1e-3);
  } else {
    m_bInFiringCone = false;
  }
}
...
// Purpose:
//-----------------------------------------------------------------------------
const char *CPropAPC::GetTracerType(void) { return "HelicopterTracer"; }
...
void CPropAPC::FireMachineGun(void) {
  ...
  GetAttachment(m_nMachineGunMuzzleAttachment, vecMachineGunShootPos,
                &vecMachineGunDir);

  // Fire the round
  int bulletType = GetAmmoDef()->Index("AR2");
  FireBullets(1, vecMachineGunShootPos, vecMachineGunDir, VECTOR_CONE_8DEGREES,
              MAX_TRACE_LENGTH, bulletType, 1);
  DoMuzzleFlash();
...
void CPropAPC::CreateCorpse() {
  ...
  for (int i = 0; i < APC_MAX_GIBS; ++i) {
    CPhysicsProp *pGib =
        assert_cast<CPhysicsProp *>(CreateEntityByName("prop_physics"));
...
void CPropAPC::FireDying() {
  ...
  Vector vecVelocity;
  VectorMultiply(vecDir, ROCKET_SPEED * random->RandomFloat(0.75f, 1.25f),
                 vecVelocity);

  QAngle angles;
  VectorAngles(vecDir, angles);

  CAPCMissile *pRocket = (CAPCMissile *)CAPCMissile::Create(
      vecRocketOrigin, angles, vecVelocity, this);
  ...
  // Make erratic firing
  m_flRocketTime =
      gpGlobals->curtime + random->RandomFloat(DEATH_VOLLEY_MIN_FIRE_TIME,
                                               DEATH_VOLLEY_MAX_FIRE_TIME);
...
void CPropAPC::FireRocket(void) {
  ...
  if (m_iRocketSalvoLeft > 0) {
    ...
    m_iRocketSalvoLeft = ROCKET_SALVO_SIZE;
    m_flRocketTime =
        gpGlobals->curtime + random->RandomFloat(ROCKET_MIN_BURST_PAUSE_TIME,
                                                 ROCKET_MAX_BURST_PAUSE_TIME);
  }
...
// Purpose:
//-----------------------------------------------------------------------------
float CPropAPC::MaxAttackRange() const { return ROCKET_ATTACK_RANGE_MAX; }
...
// Purpose:
//-----------------------------------------------------------------------------
void CAPCFourWheelServerVehicle::Weapon_PrimaryRanges(float *flMinRange,
                                                      float *flMaxRange) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CAPCFourWheelServerVehicle::Weapon_SecondaryRanges(float *flMinRange,
                                                        float *flMaxRange) {
