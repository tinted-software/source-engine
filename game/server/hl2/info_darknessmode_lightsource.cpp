...
ConVar g_debug_darkness("g_debug_darkness", "0", FCVAR_NONE,
                        "Show darkness mode lightsources.", CV_Debug_Darkness);
ConVar darkness_ignore_LOS_to_sources("darkness_ignore_LOS_to_sources", "1",
                                      FCVAR_NONE);
...
public:
  ...
  void SetLightRadius(float flRadius) { m_flLightRadius = flRadius; }
  ...
  void DebugThink(void) {
    ...
    NDebugOverlay::Box(GetAbsOrigin(), -vecRadius, vecRadius, 255, 255, 255, 8,
                       0.1);
    NDebugOverlay::Box(GetAbsOrigin(), -Vector(5, 5, 5), Vector(5, 5, 5), 255,
                       0, 0, 8, 0.1);
    SetNextThink(gpGlobals->curtime + 0.1);

    int textoffset = 0;
    EntityText(textoffset,
               UTIL_VarArgs("Org: %.2f %.2f %.2f", GetAbsOrigin().x,
  ...
  void IgnoreLOS(void) { m_bIgnoreLOS = true; }

  bool ShouldIgnoreLOS(void) { return m_bIgnoreLOS; }
...
// Purpose:
//-----------------------------------------------------------------------------
void CDarknessLightSourcesSystem::AddLightSource(
    CInfoDarknessLightSource *pEntity, float flRadius) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CDarknessLightSourcesSystem::RemoveLightSource(
    CInfoDarknessLightSource *pEntity) {
...
// Purpose:
//-----------------------------------------------------------------------------
bool CDarknessLightSourcesSystem::IsEntityVisibleToTarget(
    CBaseEntity *pLooker, CBaseEntity *pTarget) {
  if (pTarget->IsEffectActive(EF_BRIGHTLIGHT) ||
      pTarget->IsEffectActive(EF_DIMLIGHT))
    return true;
  ...
  // Loop through all the light sources. Do it backwards, so we can remove dead
  // ones.
  for (int i = m_LightSources.Count() - 1; i >= 0; i--) {
    // Removed?
    if (m_LightSources[i].hEntity == NULL ||
        m_LightSources[i].hEntity->IsMarkedForDeletion()) {
    ...
    float flDistanceSqr =
        (pTarget->WorldSpaceCenter() - pLightSource->GetAbsOrigin())
            .LengthSqr();
    if (flDistanceSqr < m_LightSources[i].flLightRadiusSqr) {
      if (pLightSource->ShouldIgnoreLOS()) {
        if (bDebug) {
          NDebugOverlay::Line(pTarget->WorldSpaceCenter(),
                              pLightSource->GetAbsOrigin(), 0, 255, 0, true,
                              0.1);
        }
        return true;
      }
      ...
      CTraceFilterSkipTwoEntities filter(pTarget, pLooker,
                                         COLLISION_GROUP_NONE);
      AI_TraceLine(pTarget->WorldSpaceCenter(), pLightSource->GetAbsOrigin(),
                   MASK_BLOCKLOS, &filter, &tr);
      if (tr.fraction == 1.0) {
        if (bDebug) {
      ...
      if (bDebug) {
        ...
        NDebugOverlay::Line(tr.endpos, pLightSource->GetAbsOrigin(), 128, 0, 0,
                            true, 0.1);
      }

      // If the target is within the radius of the light, don't do sillhouette
      // checks
      continue;
    }
    ...
    Vector vecLookerToLight =
        (pLightSource->GetAbsOrigin() - pLooker->WorldSpaceCenter());
    Vector vecLookerToTarget =
        (pTarget->WorldSpaceCenter() - pLooker->WorldSpaceCenter());
      ...
      if (flDistToTarget < flDistToSource) {
        if (bDebug) {
          NDebugOverlay::Line(pLooker->WorldSpaceCenter(),
                              pLooker->WorldSpaceCenter() +
                                  (vecLookerToLight * 128),
                              255, 255, 255, true, 0.1);
          NDebugOverlay::Line(pLooker->WorldSpaceCenter(),
                              pLooker->WorldSpaceCenter() +
                                  (vecLookerToTarget * 128),
                              255, 0, 0, true, 0.1);
        }

        // Now, we need to find out if the light source is obscured by anything.
        // To do this, we want to calculate the point of intersection between
        // the light source sphere and the line from the looker through the
        // target.
        ...
        if (flDesc >= 0) {
          ...
          Vector vecSpherePoint =
              pLooker->WorldSpaceCenter() + (vecLookerToTarget * flLength);

          // We've got the point of intersection. See if we can see it.
          CTraceFilterSkipTwoEntities filter(pTarget, pLooker,
                                             COLLISION_GROUP_NONE);
          AI_TraceLine(pLooker->EyePosition(), vecSpherePoint,
                       MASK_SOLID_BRUSHONLY, &filter, &tr);

          if (bDebug) {
            if (tr.fraction != 1.0) {
              NDebugOverlay::Line(pLooker->WorldSpaceCenter(), vecSpherePoint,
                                  255, 0, 0, true, 0.1);
            } else {
              NDebugOverlay::Line(pLooker->WorldSpaceCenter(), vecSpherePoint,
                                  0, 255, 0, true, 0.1);
              NDebugOverlay::Line(pLightSource->GetAbsOrigin(), vecSpherePoint,
                                  255, 0, 0, true, 0.1);
            }
          }
...
// Purpose:
//-----------------------------------------------------------------------------
bool CDarknessLightSourcesSystem::AreThereLightSourcesWithinRadius(
    CBaseEntity *pLooker, float flRadius) {
  ...
  for (int i = m_LightSources.Count() - 1; i >= 0; i--) {
    ...
    // Close enough to a light source?
    float flDistanceSqr =
        (pLooker->WorldSpaceCenter() - pLightSource->GetAbsOrigin())
            .LengthSqr();
    if (flDistanceSqr < flRadiusSqr) {
      trace_t tr;
      AI_TraceLine(pLooker->EyePosition(), pLightSource->GetAbsOrigin(),
                   MASK_SOLID_BRUSHONLY, pLooker, COLLISION_GROUP_NONE, &tr);

      if (g_debug_darkness.GetBool()) {
        if (tr.fraction != 1.0) {
          NDebugOverlay::Line(pLooker->WorldSpaceCenter(), tr.endpos, 255, 0, 0,
                              true, 0.1);
        } else {
          NDebugOverlay::Line(pLooker->WorldSpaceCenter(), tr.endpos, 0, 255, 0,
                              true, 0.1);
          NDebugOverlay::Line(pLightSource->GetAbsOrigin(), tr.endpos, 255, 0,
                              0, true, 0.1);
        }
      }
...
void CDarknessLightSourcesSystem::SetDebug(bool bDebug) {
  for (int i = m_LightSources.Count() - 1; i >= 0; i--) {
    CInfoDarknessLightSource *pLightSource =
        dynamic_cast<CInfoDarknessLightSource *>(
            m_LightSources[i].hEntity.Get());
    if (pLightSource) {
      if (bDebug) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CV_Debug_Darkness(IConVar *pConVar, const char *pOldString,
                       float flOldValue) {
...
// Input  : *pEntity -
//-----------------------------------------------------------------------------
void AddEntityToDarknessCheck(
    CBaseEntity *pEntity, float flLightRadius /*=DARKNESS_LIGHTSOURCE_SIZE*/) {
  // Create a light source, and attach it to the entity
  CInfoDarknessLightSource *pLightSource =
      (CInfoDarknessLightSource *)CreateEntityByName(
...
// Input  : *pEntity -
//-----------------------------------------------------------------------------
bool LookerCouldSeeTargetInDarkness(CBaseEntity *pLooker,
                                    CBaseEntity *pTarget) {
  if (DarknessLightSourcesSystem()->IsEntityVisibleToTarget(pLooker, pTarget)) {
    // NDebugOverlay::Line( pTarget->WorldSpaceCenter(),
    // pLooker->WorldSpaceCenter(), 0,255,0,true, 0.1);
    return true;
  }
...
bool DarknessLightSourceWithinRadius(CBaseEntity *pLooker, float flRadius) {
  return DarknessLightSourcesSystem()->AreThereLightSourcesWithinRadius(
      pLooker, flRadius);
}
