...
DEFINE_FIELD(m_bEnabled, FIELD_BOOLEAN),
    ...
    DEFINE_FIELD(m_iNumEnemiesInSafeZone, FIELD_INTEGER), END_DATADESC();
...
int CActBusyAnimData::FindBusyAnim(Activity iActivity, const char *pSequence) {
  ...
  if (pSequence) {
    Warning("Specified '%s' as a busy anim name, and it's not in the act busy "
            "anim list.\n",
            pSequence);
  } else if (iActivity != ACT_INVALID) {
    Warning("Tried to use Activity %d as a busy anim, and it's not in the act "
            "busy anim list.\n",
            iActivity);
  }
...
void CAI_ActBusyBehavior::ForceActBusy(CAI_ActBusyGoal *pGoal,
  ...
  if (m_bNeedsToPlayExitAnim) {
    ...
    if (HasAnimForActBusy(m_iCurrentBusyAnim, BA_EXIT)) {
      Warning("ACTBUSY: %s(%s) was told to actbusy while inside an actbusy "
              "that needs to exit first. IGNORING.\n",
...
void CAI_ActBusyBehavior::StartTask(const Task_t *pTask) {
  ...
  case TASK_ACTBUSY_PLAY_BUSY_ANIM: {
    ...
    // built from render bounds when a character is still interpolating to their
    // busy pose.
  ...
  case TASK_ACTBUSY_GET_PATH_TO_ACTBUSY: {
    ...
    if (!HasCondition(COND_TASK_FAILED)) {
      ...
      // Set the arrival sequence for the actbusy to be the busy sequence, if we
      // don't have an entry animation
      busyanim_t *pBusyAnim =
          g_ActBusyAnimDataSystem.GetBusyAnim(m_iCurrentBusyAnim);
  ...
  case TASK_ACTBUSY_VERIFY_EXIT: {
    if (GetOuter()->IsWaitFinished()) {
      ...
      if (tr.startsolid) {
        ...
        // Put an entity blocker here for a moment until I get into my bounding
        // box.
...
void CAI_ActBusyGoal::EnableGoal(CAI_BaseNPC *pAI) {
  ...
  if (pAI->GetBehavior(&pBehavior)) {
    ...
    DevMsg("ActBusy goal entity activated for an NPC (%s) that doesn't have "
           "the ActBusy behavior\n",
           pAI->GetDebugName());
    return;
  }
}
  ...
  for (int i = 0; i < NumActors(); i++) {
    ...
    if (pActor) {
      ...
      if (pActor->GetBehavior(&pBehavior)) {
        ...
        DevMsg("ActBusy goal entity deactivated for an NPC that doesn't have "
               "the ActBusy behavior\n");
        return;
      }
    }
...
//-----------------------------------------------------------------------------
CAI_ActBusyBehavior *
CAI_ActBusyQueueGoal::GetQueueBehaviorForNPC(CAI_BaseNPC *pNPC) {
  CAI_ActBusyBehavior *pBehavior;
