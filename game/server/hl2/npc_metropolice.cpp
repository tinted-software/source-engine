...
ConVar
    sk_metropolice_stitch_tight_hitcount("sk_metropolice_stitch_tight_hitcount",
                                         "2");
...
ConVar
    sk_metropolice_stitch_along_hitcount("sk_metropolice_stitch_along_hitcount",
                                         "2");
...
DEFINE_EMBEDDED(m_BatonSwingTimer), DEFINE_EMBEDDED(m_NextChargeTimer),
    ...
    //								m_ActBusyBehavior
    //(auto saved by AI) 								m_StandoffBehavior (auto saved by AI) 								m_AssaultBehavior
    //(auto saved by AI) 								m_FuncTankBehavior (auto saved by AI) 								m_RappelBehavior
    //(auto saved by AI) 								m_PolicingBehavior (auto saved by AI) 								m_FollowBehavior
    //(auto saved by AI)
...
//------------------------------------------------------------------------------
CBaseEntity *
CNPC_MetroPolice::CheckTraceHullAttack(const Vector &vStart, const Vector &vEnd,
...
void CNPC_MetroPolice::Spawn(void) {
  ...
  if (HasSpawnFlags(SF_METROPOLICE_ALWAYS_STITCH)) {
    if (!Weapon_OwnsThisType("weapon_smg1")) {
      Warning("Warning! Metrocop is trying to use the stitch behavior but he "
              "has no smg1!\n");
...
void CNPC_MetroPolice::SpeakAssaultSentence(int nSentenceType) {
  ...
  case ASSAULT_SENTENCE_SQUAD_ADVANCE_TO_RALLY:
    if (m_Sentences.Speak("METROPOLICE_AS_ADV_RALLY", SENTENCE_PRIORITY_MEDIUM,
      ...
      GetSquad()->BroadcastInteraction(g_interactionMetrocopClearSentenceQueues,
                                       NULL);
    }
    break;
...
void CNPC_MetroPolice::SteerBurstTowardTarget() {
  ...
  case BURST_STEER_ADJUST_FOR_SPEED_CHANGES: {
    // Predict the airboat position at the point where we were expecting to hit
    // them
...
//-----------------------------------------------------------------------------
Vector
CNPC_MetroPolice::ComputeBurstLockOnTrajectory(const Vector &shootOrigin) {
...
Vector
CNPC_MetroPolice::ComputeTightBurstTrajectory(const Vector &shootOrigin) {
  CBaseEntity *pEnemy = GetEnemy();
  if (!pEnemy) {
  ...
  case METROPOLICE_CHATTER_ASK_QUESTION: {
    ...
    if (m_Sentences.Speak(pQuestion[bIsCriminal][nQuestionType]) >= 0) {
      GetSquad()->BroadcastInteraction(
          ...
          (void *)(intp)(METROPOLICE_CHATTER_RESPONSE + nQuestionType), this);
...
int CNPC_MetroPolice::TranslateSchedule(int scheduleType) {
  ...
  case SCHED_METROPOLICE_ADVANCE:
    if (m_NextChargeTimer.Expired() && metropolice_charge.GetBool()) {
      if (Weapon_OwnsThisType("weapon_pistol")) {
        if (GetEnemy() &&
            GetEnemy()->GetAbsOrigin().DistToSqr(GetAbsOrigin()) > 300 * 300) {
...
void CNPC_MetroPolice::StartTask(const Task_t *pTask) {
  ...
  case TASK_METROPOLICE_ACTIVATE_BATON: {
    ...
    if (activate) {
      ...
      m_Sentences.Speak("METROPOLICE_ACTIVATE_BATON", SENTENCE_PRIORITY_NORMAL,
                        SENTENCE_CRITERIA_NORMAL);
  ...
  case TASK_METROPOLICE_GET_PATH_TO_BESTSOUND_LOS: {
    ...
    case 0: {
      ...
      if (!pSound) {
        ...
        if (GetTacticalServices()->FindLos(pSound->GetSoundReactOrigin(),
                                           losTarget, flMinRange, flMaxRange,
                                           1.0, &m_vInterruptSavePosition)) {
          TaskInterrupt();
        } else {
...
//-----------------------------------------------------------------------------
WeaponProficiency_t
CNPC_MetroPolice::CalcWeaponProficiency(CBaseCombatWeapon *pWeapon) {
  if (FClassnameIs(pWeapon, "weapon_pistol")) {
