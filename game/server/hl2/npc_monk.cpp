...
//			in Ravenholm. He keeps to the rooftops and uses a big
//ole elephant 			gun to send his zombified former friends to a peaceful death.
//
//=============================================================================//
...
Activity CNPC_Monk::NPC_TranslateActivity(Activity eNewActivity) {
  ...
  if ((m_NPCState == NPC_STATE_COMBAT || m_NPCState == NPC_STATE_ALERT)) {
    ...
    if (bGunUp) {
      ...
      // keep aiming a little longer than normal since the shot takes so long
      // and there's no good way to do a transitions between movement types :/
  ...
  // We need these so that we can pick up the shotgun to throw it in the balcony
  // scene
...
//------------------------------------------------------------------------------
WeaponProficiency_t
CNPC_Monk::CalcWeaponProficiency(CBaseCombatWeapon *pWeapon) {
...
Vector CNPC_Monk::GetActualShootTrajectory(const Vector &shootOrigin) {
  if (GetEnemy() && GetEnemy()->Classify() == CLASS_ZOMBIE) {
    ...
    if (m_bPerfectAccuracy ||
        random->RandomInt(1, monk_headshot_freq.GetInt()) == 1) {
...
bool CNPC_Monk::ShouldBackAway() {
  ...
  if (GetAbsOrigin().z - GetEnemy()->GetAbsOrigin().z >=
      MONK_STAND_GROUND_HEIGHT) {
    // This is a fairly special case. Grigori looks better fighting from his
    // assault points in the elevated places of the Graveyard, so we prevent his
    // back away behavior anytime he has a height advantage on his enemy.
    return false;
  }
...
int CNPC_Monk::SelectSchedule() {
  ...
  if (HasCondition(COND_TALKER_PLAYER_DEAD) && !m_bMournedPlayer &&
      IsOkToSpeak()) {
...
void CNPC_Monk::StartTask(const Task_t *pTask) {
  ...
  case TASK_RELOAD: {
    if (GetActiveWeapon() && GetActiveWeapon()->HasPrimaryAmmo()) {
      // Don't reload if you have done so while moving (See BACK_AWAY_AND_RELOAD
      // schedule).
      TaskComplete();
      return;
    }
  ...
  while (pSightEnt) {
    if (pSightEnt->Classify() == CLASS_ZOMBIE && pSightEnt->IsAlive()) {
      ...
      if (pZombie && pZombie->GetEnemy() == this) {
        ...
        // if this zombie is close enough to attack, add him to the zombie
        // danger!
        float flDist;
    ...
    if (GetActiveWeapon()->UsesPrimaryAmmo()) {
      if (!GetActiveWeapon()->HasPrimaryAmmo()) {
        SetCondition(COND_NO_PRIMARY_AMMO);
                 ...
                 GetActiveWeapon()->UsesClipsForAmmo1() &&
...
void CNPC_Monk::OnKilledNPC(CBaseCombatCharacter *pKilled) {
  ...
  if (pKilled->Classify() == CLASS_ZOMBIE) {
    // Don't speak if the gun is empty, cause grigori will want to speak while
    // he's reloading.
    if (GetActiveWeapon()) {
      if (GetActiveWeapon()->UsesPrimaryAmmo() &&
          !GetActiveWeapon()->HasPrimaryAmmo()) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_Monk::SelectFailSchedule(int failedSchedule, int failedTask,
                                  AI_TaskFailureCode_t taskFailCode) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Monk::IsJumpLegal(const Vector &startPos, const Vector &apex,
                            const Vector &endPos) const {
...
DEFINE_SCHEDULE(
    ...
    "	SCHEDULE:SCHED_MONK_NORMAL_RELOAD"
    "		TASK_STOP_MOVING					"
    "		0"
    "		TASK_STORE_ENEMY_POSITION_IN_SAVEPOSITION	0"
    "		TASK_FIND_BACKAWAY_FROM_SAVEPOSITION		0"
