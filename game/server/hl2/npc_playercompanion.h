...
public:
  CCompanionActivityRemap(void)
      : m_fUsageBits(0), m_readiness(AIRL_RELAXED), m_bAiming(false),
        m_bWeaponRequired(false), m_bInVehicle(false) {}
  ...
  // accessing the data.  This could be encapsulated into this class, but we'll
  // probably move away from this model and closer to something more akin to the
  // response rules -- jdw
...
protected:
  ...
  int m_iNumConsecutiveBarrelsExploded; // Companions keep track of the # of
                                        // consecutive barrels exploded by the
                                        // player and speaks a response as it
                                        // increases
  int m_iNumConsecutivePlayerKills; // Alyx keeps track of the # of consecutive
                                    // kills by the player and speaks a response
                                    // as it increases
