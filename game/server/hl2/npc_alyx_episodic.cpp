...
//
// Purpose: Alyx, the female sidekick and love interest that's taking the world
// by storm!
//
...
#define ALYX_MIN_MOB_DIST_SQR                                                  \
  Square(120) // Any enemy closer than this adds to the 'mob'
#define ALYX_MIN_CONSIDER_DIST                                                 \
  Square(1200) // Only enemies within this range are counted and considered to
               // generate AI speech
...
#define ALYX_MIN_ENEMY_DIST_TO_CROUCH                                          \
  360 // Minimum distance that our enemy must be for me to crouch
#define ALYX_MIN_ENEMY_HEALTH_TO_CROUCH 15
#define ALYX_CROUCH_DELAY                                                      \
  5 // Time after crouching before Alyx will crouch again
...
DEFINE_FIELD(m_hEmpTool, FIELD_EHANDLE),
    ...
    DEFINE_INPUTFUNC(FIELD_VOID, "DisallowInteraction",
    ...
    DEFINE_INPUTFUNC(FIELD_BOOLEAN, "AllowDarknessSpeech",
...
CNPC_Alyx::~CNPC_Alyx() { g_AlyxList.Remove(this); }

//=========================================================
// Classify - indicates this NPC's place in the
// relationship table.
//=========================================================
Class_T CNPC_Alyx::Classify(void) { return CLASS_PLAYER_ALLY_VITAL; }
...
//=========================================================
CNPC_Alyx *CNPC_Alyx::GetAlyx(void) { return g_AlyxList.m_pClassList; }
...
void CNPC_Alyx::Spawn() {
  ...
  // If Alyx has a parent, she's currently inside a pod. Prevent her from
  // moving.
  if (GetMoveParent()) {
...
void CNPC_Alyx::Activate(void) {
  ...
  // Avoids problems with players saving the game in places where she dies
  ...
  if (pSpeechManager) {
    ConceptInfo_t conceptRequestItem = {
        CONCEPT_ALYX_REQUEST_ITEM, SPEECH_IMPORTANT, -1, -1, -1, -1, -1, -1,
        AICF_TARGET_PLAYER};
  ...
  // !!!HACKHACK for Overwatch, If we're in ep2_outland_10, do half damage to
  // Combine Be advised, this will also happen in 10a, but this is not a
  // problem.
...
void CNPC_Alyx::SetupAlyxWithoutParent(void) {
  ...
  CapabilitiesAdd(bits_CAP_MOVE_GROUND | bits_CAP_DOORS_GROUP |
                  bits_CAP_TURN_HEAD | bits_CAP_DUCK | bits_CAP_SQUAD);
...
void CNPC_Alyx::ReadinessLevelChanged(int iPriorLevel) {
  ...
  // If Alyx is going from Relaxed to Agitated or Stimulated, let her raise her
  // weapon before she's able to fire.
  ...
  if (npc_alyx_readiness_transitions.GetBool()) {
    ...
    static ReadinessTransition_t readinessTransitions[] = {
        // Previous Readiness level - Current Readiness Level - Activity NPC
        // needs to be playing - Gesture to play
        {
            AIRL_RELAXED,
            AIRL_STIMULATED,
            ACT_IDLE,
    ...
    for (int i = 0; i < ARRAYSIZE(readinessTransitions); i++) {
      ...
      if (translatedTransitionActivity == ACT_INVALID ||
          translatedTransitionActivity ==
              readinessTransitions[i].transitionActivity)
        continue;

      Activity finalActivity =
          TranslateActivityReadiness(translatedTransitionActivity);
...
void CNPC_Alyx::PrescheduleThink(void) {
  ...
  if (GetState() == NPC_STATE_COMBAT && IsWeaponHolstered() &&
      !m_FuncTankBehavior.IsRunning()) {
    SetDesiredWeaponState(DESIREDWEAPONSTATE_UNHOLSTERED);
  }

  // If we're in stealth mode, and we can still see the stealth node, keep using
  // it
  if (GetReadinessLevel() == AIRL_STEALTH) {
    if (m_hStealthLookTarget && !m_hStealthLookTarget->IsDisabled()) {
      if (m_hStealthLookTarget->IsInNodeFOV(this) &&
          FVisible(m_hStealthLookTarget))
        return;
    }
  ...
  if (m_bIsFlashlightBlind) {
    // we used to have a bug where if we tried to remove alyx from the blind
    // scene before it got loaded asynchronously, she would get stuck in the
    // animation with m_bIsFlashlightBlind set to false.  that should be fixed,
    // but just to be sure, we wait a bit to prevent this from happening.
    if (m_fStayBlindUntil < gpGlobals->curtime) {
      CBasePlayer *pPlayer = UTIL_PlayerByIndex(1);
      if (pPlayer &&
          (!CanBeBlindedByFlashlight(true) ||
           !pPlayer->IsIlluminatedByFlashlight(this, NULL) ||
           !PlayerFlashlightOnMyEyes(pPlayer)) &&
          !BlindedByFlare()) {
        // Remove the actor from the flashlight scene
        ADD_DEBUG_HISTORY(HISTORY_ALYX_BLIND,
                          UTIL_VarArgs("(%0.2f) Alyx: end blind scene '%s'\n",
                                       gpGlobals->curtime,
...
void CNPC_Alyx::SearchForInteractTargets() {
  ...
  m_fTimeNextSearchForInteractTargets =
      gpGlobals->curtime + ALYX_INTERACT_SEARCH_FREQUENCY;
...
void CNPC_Alyx::GatherConditions() {
  ...
  if (HasCondition(COND_HEAR_DANGER)) {
    ...
    // is NOT panicking over a Danger sound, she'll hear the combat sounds as
    // normal.
  ...
  if (HasCondition(COND_HEAR_COMBAT)) {
    ...
    if (IsInAVehicle() ==
        false) // For now, don't do these animations while in the vehicle
    {
      if ((pSound->SoundTypeNoContext() & SOUND_COMBAT) &&
          (pSound->SoundContext() & SOUND_CONTEXT_EXPLOSION)) {
        if (HasShotgun()) {
          if (!IsPlayingGesture(ACT_GESTURE_FLINCH_BLAST_SHOTGUN) &&
            ...
            GetShotRegulator()->FireNoEarlierThan(
                gpGlobals->curtime +
                SequenceDuration(ACT_GESTURE_FLINCH_BLAST_SHOTGUN) +
                0.5f); // Allow another second for Alyx to bring her weapon to
                       // bear after the flinch.
          }
        } else {
          if (!IsPlayingGesture(ACT_GESTURE_FLINCH_BLAST) &&
            ...
            GetShotRegulator()->FireNoEarlierThan(
                gpGlobals->curtime +
                SequenceDuration(ACT_GESTURE_FLINCH_BLAST) +
                0.5f); // Allow another second for Alyx to bring her weapon to
                       // bear after the flinch.
          }
        }
  ...
  // ROBIN: This was here to solve a problem in a playtest. We've since found
  // what we think was the cause. It's a useful piece of debug to have lying
  // there, so I've left it in.
  if ((GetFlags() & FL_FLY) && m_NPCState != NPC_STATE_SCRIPT &&
      !m_ActBusyBehavior.IsActive() && !m_PassengerBehavior.IsEnabled()) {
    Warning("Removed FL_FLY from Alyx, who wasn't running a script or actbusy. "
            "Time %.2f, map %s.\n",
...
void CNPC_Alyx::AnalyzeGunfireSound(CSound *pSound) {
  ...
  // Have to verify a bunch of stuff about the sound. It must have a valid
  // BaseCombatCharacter as the owner, must have a valid target, and we need a
  // valid pointer to the player.
  ...
  CBaseCombatCharacter *pSoundOriginBCC =
      pSound->m_hOwner->MyCombatCharacterPointer();
  ...
  if (pSoundTarget == this) {
    // The shooter is firing at me. Assume if Alyx can hear the gunfire, she can
    // deduce its origin.
    UpdateEnemyMemory(pSoundOriginBCC, pSoundOriginBCC->GetAbsOrigin(), this);
  } else if (pSoundTarget == pPlayer) {
    // The shooter is firing at the player. Assume Alyx can deduce the origin if
    // the player COULD see the origin, and Alyx COULD see the player.
...
bool CNPC_Alyx::IsValidEnemy(CBaseEntity *pEnemy) {
  ...
  // Alyx can only take a stalker as her enemy which is angry at the player or
  // her.
  ...
  if (m_AssaultBehavior.IsRunning() && IsTurret(pEnemy)) {
    ...
    if (pBCC != NULL && !pBCC->FInViewCone(this)) {
      // Don't let turrets that can't shoot me distract me from my assault
      // behavior. This fixes a very specific problem that appeared in Episode 2
      // map ep2_outland_09 Where Alyx wouldn't terminate an assault while
      // standing on an assault point because she was afraid of a turret that
      // was visible from the assault point, but facing the other direction and
      // thus not a threat.
      return false;
    }
  }
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Alyx::Event_KilledOther(CBaseEntity *pVictim,
  ...
  // Alyx builds a proxy for the dead enemy so she has something to shoot at for
  // a short time after the enemy ragdolls.
  if (!(pVictim->GetFlags() & FL_ONGROUND) ||
      pVictim->GetMoveType() != MOVETYPE_STEP) {
    // Don't fire up in the air, since the dead enemy will have fallen.
    return;
  }

  if (pVictim->GetAbsOrigin().DistTo(GetAbsOrigin()) < 96.0f) {
    // Don't shoot at an enemy corpse that dies very near to me. This will
    // prevent Alyx attacking Other nearby enemies.
    return;
  }
  ...
  if (!HasShotgun()) {
    ...
    AddEntityRelationship(pTarget, IRelationType(pVictim),
                          IRelationPriority(pVictim));

    // Update or Create a memory entry for this target and make Alyx think she's
    // seen this target recently. This prevents the baseclass from not
    // recognizing this target and forcing Alyx into SCHED_WAKE_ANGRY, which
    // wastes time and causes her to change animation sequences rapidly.
    GetEnemies()->UpdateMemory(GetNavigator()->GetNetwork(), pTarget,
                               pTarget->GetAbsOrigin(), 0.0f, true);
...
bool CNPC_Alyx::RunningPassengerBehavior(void) {
  ...
  if (m_PassengerBehavior.IsRunning() &&
...
void CNPC_Alyx::DoMobbedCombatAI(void) {
  ...
  for (AI_EnemyInfo_t *pEMemory = GetEnemies()->GetFirst(&iter);
    ...
    if (IRelationType(pEMemory->hEnemy) != D_NU &&
        ...
        pEMemory->hEnemy->GetAbsOrigin().DistToSqr(GetAbsOrigin()) <=
            ALYX_MIN_CONSIDER_DIST) {
      if (pEMemory->hEnemy && pEMemory->hEnemy->IsAlive() &&
        ...
        if (pEMemory->hEnemy->GetAbsOrigin().DistToSqr(GetAbsOrigin()) <=
            ALYX_MIN_MOB_DIST_SQR) {
  ...
  // Alyx's gun can never run out of ammo. Allow Alyx to ignore LOW AMMO
  // warnings if she's in a close quarters fight with several enemies. She'll
  // attempt to reload as soon as her combat situation is less pressing.
...
void CNPC_Alyx::DoCustomSpeechAI(void) {
  ...
  if (HasCondition(COND_NEW_ENEMY) && GetEnemy()) {
    if (GetEnemy()->Classify() == CLASS_HEADCRAB) {
      ...
      // If we see a headcrab for the first time as he's jumping at me, freak
      // out!
      ...
      else if (pHC->GetOwnerEntity() &&
               (pHC->GetOwnerEntity()->Classify() == CLASS_ZOMBIE) &&
  ...
  if (HL2GameRules()->IsAlyxInDarknessMode()) {
    // Even though the darkness light system will take flares into account when
    // Alyx says she's lost the player in the darkness, players still think
    // she's silly when they're too far from the flare to be seen. So, check for
    // lit flares or other dynamic lights, and don't do a bunch of the darkness
    // speech if there's a lit flare nearby.
    ...
    if (!bNearbyFlare) {
      ...
      if (HasCondition(COND_ALYX_PLAYER_TURNED_OFF_FLASHLIGHT) ||
        ...
        // Player just turned off the flashlight. Start ramping up Alyx's
        // breathing.
        if (!m_sndDarknessBreathing) {
          CPASAttenuationFilter filter(this);
          m_sndDarknessBreathing =
              CSoundEnvelopeController::GetController().SoundCreate(
                  filter, entindex(), CHAN_STATIC, "ep_01.al_dark_breathing01",
                  SNDLVL_TALKING);
          CSoundEnvelopeController::GetController().Play(m_sndDarknessBreathing,
                                                         0.0f, PITCH_NORM);
        }

        if (m_sndDarknessBreathing) {
          CSoundEnvelopeController::GetController().SoundChangeVolume(
              m_sndDarknessBreathing, ALYX_BREATHING_VOLUME_MAX,
              RandomFloat(10, 20));
    ...
    if (!HasCondition(COND_SEE_ENEMY) && m_bHadCondSeeEnemy &&
        !HasCondition(COND_TALKER_PLAYER_DEAD)) {
      if (m_bDarknessSpeechAllowed &&
          HasCondition(COND_ALYX_PLAYER_TURNED_OFF_FLASHLIGHT) && GetEnemy() &&
          (GetEnemy()->Classify() != CLASS_BULLSEYE)) {
        SpeakIfAllowed("TLK_DARKNESS_LOSTENEMY_BY_FLASHLIGHT");
      } else if (m_bDarknessSpeechAllowed &&
                 HasCondition(COND_ALYX_PLAYER_FLASHLIGHT_EXPIRED) &&
                 GetEnemy() && (GetEnemy()->Classify() != CLASS_BULLSEYE)) {
        SpeakIfAllowed("TLK_DARKNESS_LOSTENEMY_BY_FLASHLIGHT_EXPIRED");
      } else if (m_bDarknessSpeechAllowed && GetEnemy() &&
                 (GetEnemy()->Classify() != CLASS_BULLSEYE) && pPlayer &&
                 pPlayer->FlashlightIsOn() &&
                 !pPlayer->IsIlluminatedByFlashlight(GetEnemy(), NULL) &&
                 FVisible(GetEnemy())) {
    ...
    } else if ((!GetEnemy() || (GetEnemy()->Classify() == CLASS_BULLSEYE)) &&
    ...
    if (HasCondition(COND_NEW_ENEMY) &&
      ...
      if (gpGlobals->curtime - GetEnemies()->FirstTimeSeen(GetEnemy()) < 0.5) {
        if (pPlayer && pPlayer->IsIlluminatedByFlashlight(GetEnemy(), NULL) &&
            m_bDarknessSpeechAllowed &&
    ...
    if (!bNearbyFlare && m_bDarknessSpeechAllowed) {
      if (!HasCondition(COND_SEE_PLAYER) &&
          !m_SpeechWatch_LostPlayer.IsRunning()) {
        ...
        if (!HasCondition(COND_SEE_PLAYER) &&
            ...
            (!pPlayer || pPlayer->GetAbsOrigin().DistToSqr(GetAbsOrigin()) >
                             ALYX_DARKNESS_LOST_PLAYER_DIST)) {
      ...
      if (pPlayer && !HasCondition(COND_TALKER_PLAYER_DEAD) &&
          !pPlayer->FlashlightIsOn()) {
        // When the player first turns off the light, don't talk about sounds
        // for a bit
        if (HasCondition(COND_ALYX_PLAYER_TURNED_OFF_FLASHLIGHT) ||
            HasCondition(COND_ALYX_PLAYER_FLASHLIGHT_EXPIRED)) {
          ...
          // If we hear anything while the player's flashlight is off, randomly
          // mention it
    ...
    if (bNearbyFlare || HasCondition(COND_ALYX_PLAYER_TURNED_ON_FLASHLIGHT)) {
      ...
      if (m_sndDarknessBreathing) {
        CSoundEnvelopeController::GetController().SoundChangeVolume(
            m_sndDarknessBreathing, 0.0f, 0.5);
        m_SpeechWatch_BreathingRamp.Stop();
      }
    }
  } else {
    ...
    if (!HasCondition(COND_SEE_PLAYER) &&
        !m_SpeechWatch_FoundPlayer.IsRunning()) {
  ...
  if (m_bSpokeLostPlayerInDarkness) {
    ...
    if (!m_bIsFlashlightBlind && HL2GameRules()->IsAlyxInDarknessMode() &&
        m_bDarknessSpeechAllowed) {
      if (HasCondition(COND_SEE_PLAYER) &&
          !HasCondition(COND_TALKER_PLAYER_DEAD)) {
  ...
  if ((!m_bDarknessSpeechAllowed || HasCondition(COND_SEE_PLAYER)) &&
      m_SpeechWatch_LostPlayer.IsRunning()) {
...
// Purpose:
//-----------------------------------------------------------------------------
bool CNPC_Alyx::SpeakIfAllowed(AIConcept_t concept,
                               const char *modifiers /*= NULL*/,
                               bool bRespondingToPlayer /*= false*/,
                               char *pszOutResponseChosen /*= NULL*/,
                               size_t bufsize /* = 0 */) {
  if (BaseClass::SpeakIfAllowed(concept, modifiers, bRespondingToPlayer,
                                pszOutResponseChosen, bufsize)) {
    // If we're breathing in the darkness, drop the volume quickly
    if (m_sndDarknessBreathing &&
        CSoundEnvelopeController::GetController().SoundGetVolume(
            m_sndDarknessBreathing) > 0.0) {
      CSoundEnvelopeController::GetController().SoundChangeVolume(
          m_sndDarknessBreathing, 0.0f, 0.1);
...
Disposition_t CNPC_Alyx::IRelationType(CBaseEntity *pTarget) {
  ...
  if (pTarget->Classify() == CLASS_ANTLION) {
    if (disposition == D_HT) {
      // If Alyx hates this antlion (default relationship), make her fear it, if
      // it is very close.
      if (GetAbsOrigin().DistToSqr(pTarget->GetAbsOrigin()) <
          ALYX_FEAR_ANTLION_DIST_SQR) {
        disposition = D_FR;
      }

      // Fall through...
    }
  } else if (pTarget->Classify() == CLASS_ZOMBIE && disposition == D_HT &&
             GetActiveWeapon()) {
    if (GetAbsOrigin().DistToSqr(pTarget->GetAbsOrigin()) <
        ALYX_FEAR_ZOMBIE_DIST_SQR) {
...
int CNPC_Alyx::IRelationPriority(CBaseEntity *pTarget) {
  ...
  if (pTarget->Classify() == CLASS_ANTLION) {
    ...
    // UNLESS she has a different enemy that could melee attack her while her
    // back is turned.
...
bool CNPC_Alyx::FInViewCone(CBaseEntity *pEntity) {
  // Alyx can see 360 degrees but only at limited distance. This allows her to
  // be aware of a large mob of enemies (usually antlions or zombies) closing
  // in. This situation is so obvious to the player that it doesn't make sense
  // for Alyx to be unaware of the entire group simply because she hasn't seen
  // all of the enemies with her own eyes.
  if ((pEntity->IsNPC() || pEntity->IsPlayer()) &&
      pEntity->GetAbsOrigin().DistToSqr(GetAbsOrigin()) <=
          ALYX_360_VIEW_DIST_SQR) {
...
bool CNPC_Alyx::CanSeeEntityInDarkness(CBaseEntity *pEntity) {
  ...
  if ( pEntity->IsNPC() )
  {
          if ( (pEntity->WorldSpaceCenter() - EyePosition()).LengthSqr() <
  (80*80) ) return true;
  }
  */
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Alyx::QuerySeeEntity(CBaseEntity *pEntity,
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Alyx::IsCoverPosition(const Vector &vecThreat,
...
void CNPC_Alyx::BuildScheduleTestBits() {
  ...
  bIsInteracting =
      (IsCurSchedule(SCHED_ALYX_PREPARE_TO_INTERACT_WITH_TARGET, false) ||
  ...
  // This nugget fixes a bug where Alyx will continue to attack an enemy she no
  // longer hates in the case where her relationship with the enemy changes
  // while she's running a SCHED_SCENE_GENERIC. Since we don't run ChooseEnemy()
  // when we're running a schedule that doesn't interrupt on COND_NEW_ENEMY, we
  // also do not re-evaluate and flush enemies we don't hate anymore. (sjb
  // 6/9/2005)
  if (IsCurSchedule(SCHED_SCENE_GENERIC) && GetEnemy() &&
      GetEnemy()->VPhysicsGetObject()) {
    if (GetEnemy()->VPhysicsGetObject()->GetGameFlags() &
  ...
  if (m_NPCState != NPC_STATE_SCRIPT && !IsInAVehicle() &&
      !IsCurSchedule(SCHED_ALYX_FALL_TO_GROUND, false)) {
...
int CNPC_Alyx::SelectSchedule(void) {
  // If we're in darkness mode, and the player has the flashlight off, and we
  // hear a zombie footstep, and the player isn't nearby, deliberately turn away
  // from the zombie to let the zombie grab me.
  ...
  if (HasCondition(COND_ALYX_HAS_INTERACT_TARGET) &&
      HasCondition(COND_SEE_PLAYER) && IsAllowedToInteract()) {
  ...
  if (!IsEMPHolstered() && !HasInteractTarget() &&
...
int CNPC_Alyx::SelectScheduleDanger(void) {
  if (HasCondition(COND_HEAR_DANGER)) {
    ...
    if (pSound && (pSound->m_iType & SOUND_DANGER) &&
        (pSound->SoundChannel() == SOUNDENT_CHANNEL_ZOMBINE_GRENADE)) {
    ...
    if (GetEnemy()) {
      ...
      if (!IsCrouching()) {
        ...
        if (pEnemy->GetHealth() > ALYX_MIN_ENEMY_HEALTH_TO_CROUCH) {
          // And are they far enough away? Expand the min dist so we don't
...
void CNPC_Alyx::StartTask(const Task_t *pTask) {
  ...
  case TASK_SOUND_WAKE:
    LocateEnemySound();
    // Don't do the half second wait here that the PlayerCompanion class does.
    // (sbj) 1/4/2006
    TaskComplete();
    break;
  ...
  case TASK_REACT_TO_COMBAT_SOUND: {
    ...
    if (pSound && pSound->IsSoundType(SOUND_COMBAT) &&
        pSound->IsSoundType(SOUND_CONTEXT_GUNFIRE)) {
  ...
  case TASK_ALYX_BEGIN_INTERACTION: {
    INPCInteractive *pInteractive =
        dynamic_cast<INPCInteractive *>(GetInteractTarget());
    if (pInteractive) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CNPC_Alyx::TraceAttack(const CTakeDamageInfo &info, const Vector &vecDir,
...
int CNPC_Alyx::OnTakeDamage_Alive(const CTakeDamageInfo &info) {
  ...
  // in freak accidents resembling spontaneous stress damage death (which are
  // now impossible) Also stop her taking damage from flames: Fixes her being
  ...
  if (taken && HL2GameRules()->IsAlyxInDarknessMode() &&
      !HasCondition(COND_TALKER_PLAYER_DEAD)) {
    if (!HasCondition(COND_SEE_ENEMY) &&
        (info.GetDamageType() & (DMG_SLASH | DMG_CLUB))) {
      // I've taken melee damage. If I haven't seen the enemy for a few seconds,
      // make some noise.
      float flLastTimeSeen =
          GetEnemies()->LastTimeSeen(info.GetAttacker(), false);
      if (flLastTimeSeen == AI_INVALID_TIME ||
          gpGlobals->curtime - flLastTimeSeen > 3.0) {
        SpeakIfAllowed("TLK_DARKNESS_UNKNOWN_WOUND");
        m_fTimeUntilNextDarknessFoundPlayer =
            gpGlobals->curtime + RandomFloat(3, 5);
      }
    }
  }
  ...
  if (taken && (info.GetDamageType() & DMG_BLAST)) {
    if (HasShotgun()) {
      if (!IsPlayingGesture(ACT_GESTURE_FLINCH_BLAST) &&
        ...
        GetShotRegulator()->FireNoEarlierThan(
            gpGlobals->curtime +
            SequenceDuration(ACT_GESTURE_FLINCH_BLAST_DAMAGED_SHOTGUN) + 0.5f);
      }
    } else {
      if (!IsPlayingGesture(ACT_GESTURE_FLINCH_BLAST) &&
          !IsPlayingGesture(ACT_GESTURE_FLINCH_BLAST_DAMAGED)) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Alyx::HandleInteraction(int interactionType, void *data,
                                  CBaseCombatCharacter *sourceEnt) {
  if (interactionType == g_interactionZombieMeleeWarning &&
      IsAllowedToDodge()) {
    // If a zombie is attacking, ditch my current schedule and duck if I'm
    // running a schedule that will be interrupted if I'm hit.
  ...
  if (interactionType == g_interactionPlayerPuntedHeavyObject) {
    ...
    if (pProp) {
      ...
      // Do this if the prop is within 60 feet, and is closer to me than the
      // player is.
      if (distToProp < (60.0f * 12.0f) && (distToProp < distToPlayer)) {
        if (fabs(pProp->WorldSpaceCenter().z - WorldSpaceCenter().z) <=
            120.0f) {
          if (sourceEnt->FInViewCone(this)) {
            CSoundEnt::InsertSound(SOUND_MOVE_AWAY, EarPosition(), 16, 1.0f,
                                   pProp);
          }
        }
      }
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Alyx::Weapon_Drop(CBaseCombatWeapon *pWeapon,
...
void CNPC_Alyx::OnSeeEntity(CBaseEntity *pEntity) {
  ...
  if (pEntity->IsPlayer() &&
      pEntity->IsEFlagSet(EFL_IS_BEING_LIFTED_BY_BARNACLE)) {
...
bool CNPC_Alyx::IsValidInteractTarget(CBaseEntity *pTarget) {
  ...
  if (GetAbsOrigin().DistToSqr(pTarget->WorldSpaceCenter()) >
      (360.0f * 360.0f)) {
    // Too far away!
    return false;
  }
...
void CNPC_Alyx::EmpZapTarget(CBaseEntity *pTarget) {
  ...
  CAlyxEmpEffect *pEmpEffect =
      (CAlyxEmpEffect *)CreateEntityByName("env_alyxemp");
...
void CNPC_Alyx::PlayerHasIlluminatedNPC(CBasePlayer *pPlayer, float flDot) {
  ...
  if (PlayerFlashlightOnMyEyes(pPlayer)) {
    ...
    if (SpeakIfAllowed("TLK_FLASHLIGHT_ILLUM", NULL, false, szResponse,
                       AI_Response::MAX_RESPONSE_NAME)) {
      m_iszCurrentBlindScene = AllocPooledString(szResponse);
      ADD_DEBUG_HISTORY(
          HISTORY_ALYX_BLIND,
          UTIL_VarArgs("(%0.2f) Alyx: start flashlight blind scene '%s'\n",
...
void CNPC_Alyx::CheckBlindedByFlare(void) {
  ...
  if (BlindedByFlare()) {
    ...
    if (SpeakIfAllowed("TLK_FLASHLIGHT_ILLUM", NULL, false, szResponse,
      ...
      ADD_DEBUG_HISTORY(
          HISTORY_ALYX_BLIND,
          UTIL_VarArgs("(%0.2f) Alyx: start flare blind scene '%s'\n",
                       gpGlobals->curtime, STRING(m_iszCurrentBlindScene)));
      GetShotRegulator()->DisableShooting();
      m_bIsFlashlightBlind = true;
      m_fStayBlindUntil = gpGlobals->curtime + 0.1f;
...
//-----------------------------------------------------------------------------
// Purpose:
// Input:   bCheckLightSources - if true, checks if any light darkness
// lightsources are near
//-----------------------------------------------------------------------------
bool CNPC_Alyx::CanBeBlindedByFlashlight(bool bCheckLightSources) {
  ...
  */
  ...
  if (IsInAScript() || GetState() == NPC_STATE_COMBAT ||
...
void CNPC_Alyx::AimGun(void) {
  ...
  if (!GetEnemy()) {
    if (GetReadinessLevel() == AIRL_STEALTH && m_hStealthLookTarget != NULL) {
      ...
      Vector vecAimDir =
          m_hStealthLookTarget->GetAbsOrigin() - Weapon_ShootPosition();
...
Vector CNPC_Alyx::GetActualShootPosition(const Vector &shootOrigin) {
  if (HasShotgun() && GetEnemy() && GetEnemy()->Classify() == CLASS_ZOMBIE &&
      random->RandomInt(0, 1) == 1) {
...
void CNPC_Alyx::ModifyOrAppendCriteria(AI_CriteriaSet &set) {
  ...
  set.AppendCriteria("combat_length",
                     UTIL_VarArgs("%.3f", fLengthOfLastCombat));

  iNumEnemies = 0;
  for (AI_EnemyInfo_t *pEMemory = GetEnemies()->GetFirst(&iter);
       pEMemory != NULL; pEMemory = GetEnemies()->GetNext(&iter)) {
    if (pEMemory->hEnemy->IsAlive() &&
        (pEMemory->hEnemy->Classify() != CLASS_BULLSEYE)) {
      iNumEnemies++;
    }
  }
  set.AppendCriteria("num_enemies", UTIL_VarArgs("%d", iNumEnemies));
  set.AppendCriteria("darkness_mode",
                     UTIL_VarArgs("%d", HasCondition(COND_ALYX_IN_DARK)));
  ...
  set.AppendCriteria(
      "num_companions",
      UTIL_VarArgs("%d", pPlayer ? pPlayer->GetNumSquadCommandables() : 0));
  set.AppendCriteria(
      "flashlight_on",
      UTIL_VarArgs("%d", pPlayer ? pPlayer->FlashlightIsOn() : 0));
...
//-----------------------------------------------------------------------------
void CNPC_Alyx::OnChangeActiveWeapon(CBaseCombatWeapon *pOldWeapon,
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Alyx::BarnacleDeathSound(void) { Speak(TLK_SELF_IN_BARNACLE); }
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Alyx::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
                    USE_TYPE useType, float value) {
  // if I'm in the vehicle, the player is probably trying to use the vehicle
  if (GetPassengerState() == PASSENGER_STATE_INSIDE && pActivator->IsPlayer() &&
      GetParent()) {
...
// Purpose:
//-----------------------------------------------------------------------------
bool CNPC_Alyx::PlayerInSpread(const Vector &sourcePos, const Vector &targetPos,
  ...
  for (int i = 1; i <= gpGlobals->maxClients; i++) {
    ...
    if (pPlayer && (!ignoreHatedPlayers || IRelationType(pPlayer) != D_HT)) {
      // If the player is being lifted by a barnacle then go ahead and ignore
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_Alyx::ForceVehicleInteraction(const char *lpszInteractionName,
                                        CBaseCombatCharacter *pOther) {
  return m_PassengerBehavior.ForceVehicleInteraction(lpszInteractionName,
                                                     pOther);
}
...
void CNPC_Alyx::InputOutsideTransition(inputdata_t &inputdata) {
  ...
  // If the player is in the vehicle and we're not, then we need to enter the
  // vehicle immediately
...
DEFINE_SCHEDULE(SCHED_ALYX_PREPARE_TO_INTERACT_WITH_TARGET,
                ...
                "			0"
                "		TASK_PLAY_SEQUENCE			"
                "			ACTIVITY:ACT_ALYX_DRAW_TOOL"
                "		TASK_SET_ACTIVITY			"
                "			ACTIVITY:ACT_ALYX_IDLE_TOOL"
                "		TASK_FACE_PLAYER			"
                "			0"
                ""
                "	Interrupts"
...
DEFINE_SCHEDULE(
    ...
    "		TASK_PLAY_SEQUENCE					"
    "	ACTIVITY:ACT_ALYX_ZAP_TOOL"
    "		TASK_SET_SCHEDULE					"
    "	SCHEDULE:SCHED_ALYX_FINISH_INTERACTING_WITH_TARGET"
    ""
    "	Interrupts"
    "		COND_ALYX_NO_INTERACT_TARGET"
...
DEFINE_SCHEDULE(
    ...
    "		TASK_PLAY_SEQUENCE					"
    "	ACTIVITY:ACT_ALYX_HOLSTER_TOOL"
    ""
    "	Interrupts"
    "")
...
DEFINE_SCHEDULE(
    ...
    "	0"
    "		TASK_WAIT						"
    "		1" // Don't move before done standing up
    ""
    "	Interrupts")
