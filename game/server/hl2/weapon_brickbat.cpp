...
BrickbatAmmo_s BrickBatAmmoArray[NUM_BRICKBAT_AMMO_TYPES] = {
    {"grenade_rockbb", BRICKBAT_ROCK, 5, "models/weapons/v_bb_bottle.mdl",
     "models/props_junk/Rock001a.mdl"},
    {"grenade_beerbottle", BRICKBAT_BOTTLE, 3, "models/weapons/v_bb_bottle.mdl",
     "models/weapons/w_bb_bottle.mdl"},
};
...
void CWeaponBrickbat::BrickbatTouch(CBaseEntity *pOther) {
  ...
  // FIXME: This ammo handling code is a bit bogus, need a real solution if
  // brickbats are going to live
  ...
  if (pOther->GetFlags() & FL_CLIENT)
  {
          ...
          // ------------------------------------------------
          CWeaponBrickbat* oldWeapon =
  (CWeaponBrickbat*)pBCC->Weapon_OwnsThisType( GetClassname() );
          ...
          if ( ( oldWeapon != NULL ) && ( oldWeapon != this ) )
          {
                  ...
                  if (oldWeapon->m_nAmmoCount[m_iCurrentAmmoType] <
  BrickBatAmmoArray[m_iCurrentAmmoType].m_nMaxCarry)
                  {
          ...
          {
                  // Only pick up if not at max ammo amount
                  if (m_nAmmoCount[m_iCurrentAmmoType] <
  BrickBatAmmoArray[m_iCurrentAmmoType].m_nMaxCarry)
                  {
...
// Output :
//-----------------------------------------------------------------------------
void CWeaponBrickbat::Operator_HandleAnimEvent(
...
// Output :
//-----------------------------------------------------------------------------
bool CWeaponBrickbat::WeaponLOSCondition(const Vector &ownerPos,
...
int CWeaponBrickbat::WeaponRangeAttack1Condition(float flDot, float flDist) {
  ...
  // If moving, can't throw.
    ...
    float throwLimit =
        pNPC->ThrowLimit(throwStart, vecTarget, fGravity, 35, WorldAlignMins(),
                         WorldAlignMaxs(), pEnemy, &vecToss, &pBlocker);

    // If I can make the throw (or most of the throw)
    if (!throwLimit ||
        (throwLimit != throwDist && throwLimit > 0.8 * throwDist)) {
...
//
//-----------------------------------------------------------------------------
void CWeaponBrickbat::ThrowBrickbat(Vector vecSrc, Vector vecVelocity,
                                    float damage) {
  CGrenade_Brickbat *pBrickbat = (CGrenade_Brickbat *)Create(
      BrickBatAmmoArray[m_iCurrentAmmoType].m_sClassName, vecSrc, vec3_angle,
      GetOwner());
...
void CWeaponBrickbat::DrawAmmo(void) {
  ...
  // Don't fire again until fire animation has completed
  float flSequenceEndTime = gpGlobals->curtime + SequenceDuration();
  pOwner->m_flNextAttack = m_flNextPrimaryAttack = m_flNextSecondaryAttack =
      flSequenceEndTime;
}
...
// Output :
//-----------------------------------------------------------------------------
void CWeaponBrickbat::TraceAttack(const CTakeDamageInfo &info,
