...
ConVar sk_antlion_worker_spit_grenade_dmg(
    "sk_antlion_worker_spit_grenade_dmg", "20", FCVAR_NONE,
    "Total damage done by an individual antlion worker loogie.");
ConVar sk_antlion_worker_spit_grenade_radius(
    "sk_antlion_worker_spit_grenade_radius", "40", FCVAR_NONE,
...
ConVar sk_antlion_worker_spit_grenade_poison_ratio(
    ...
    "Percentage of an antlion worker's spit damage done as poison (which "
    "regenerates)");
...
void CGrenadeSpit::Event_Killed(const CTakeDamageInfo &info) { Detonate(); }
...
void CGrenadeSpit::GrenadeSpitTouch(CBaseEntity *pOther) {
  ...
  // copy out some important things about this trace, because the first
  // TakeDamage call below may cause another trace that overwrites the one
  // global pTrace points at.
  ...
  if (pTraceEnt) {
    pTraceEnt->TakeDamage(CTakeDamageInfo(
        this, GetThrower(), m_flDamage * (1.0f - poisonratio), DMG_ACID));
    pTraceEnt->TakeDamage(CTakeDamageInfo(
        this, GetThrower(), m_flDamage * poisonratio, DMG_POISON));
  }

  CSoundEnt::InsertSound(SOUND_DANGER, GetAbsOrigin(), m_DmgRadius * 2.0f, 0.5f,
                         GetThrower());
...
void CGrenadeSpit::InitHissSound(void) {
  ...
  if (m_pHissSound == NULL) {
    ...
    m_pHissSound =
        controller.SoundCreate(filter, entindex(), "NPC_Antlion.PoisonBall");
...
void CGrenadeSpit::Think(void) {
  ...
  if (pPlayer != NULL) {
    ...
    CSoundEnvelopeController::GetController().SoundChangePitch(m_pHissSound,
                                                               iPitch, 0.1f);
  }
...
void CGrenadeSpit::Precache(void) {
  // m_nSquidSpitSprite = PrecacheModel("sprites/greenglow1.vmt");// client side
  // spittle.
