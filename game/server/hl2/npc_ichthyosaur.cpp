...
public:
  ...
  void Precache(void);
  void Spawn(void);
  void MoveFlyExecute(CBaseEntity *pTargetEnt, const Vector &vecDir,
                      float flDistance, float flInterval);
...
private:
  bool SteerAvoidObstacles(Vector &Steer, const Vector &Velocity,
  ...
  void ClampSteer(Vector &SteerAbs, Vector &SteerRel, Vector &forward,
...
void CNPC_Ichthyosaur::InitCustomSchedules(void) {
  ...
  // Conditions	ADD_CUSTOM_CONDITION( CNPC_CombineGuard,
...
void CNPC_Ichthyosaur::Spawn(void) {
  ...
  // "xxxCONVERTTOGAMESOUNDS!!!npc/ichthyosaur/ich_amb1wav", ATTN_NORM );
  // m_pVoiceSound	= ENVELOPE_CONTROLLER.SoundCreate( edict(), CHAN_VOICE,
  // "xxxCONVERTTOGAMESOUNDS!!!npc/ichthyosaur/water_breathwav", ATTN_IDLE );
...
Vector CNPC_Ichthyosaur::DoProbe(const Vector &probe) {
  ...
  float waterLevel =
      UTIL_WaterLevel(GetAbsOrigin(), GetAbsOrigin().z, GetAbsOrigin().z + 150);
  ...
  AI_TraceHull(GetAbsOrigin(), probe, GetHullMins(), GetHullMaxs(),
  ...
  if ((fraction < 1.0f) &&
      (GetEnemy() == NULL || tr.u.ent != GetEnemy()->pev)) {
    ...
    float steeringForce =
        m_flGroundSpeed * (1.0f - fraction) * normal.Dot(velDir);
...
void CNPC_Ichthyosaur::DragVictim(float moveDist) {
  ...
  Vector newPos = GetAbsOrigin() +
                  ((forward + (up * 0.25f)) * (moveDist + width + DRAG_OFFSET));
...
//-----------------------------------------------------------------------------
// Purpose: Determines the pose parameters for the bending of the body and tail
// speed Input  : moveRel - the dot products for the deviation off of each
// direction (f,r,u)
...
void CNPC_Ichthyosaur::SetPoses(Vector moveRel, float speed) {
  ...
  GetMotor()->SetIdealYawAndUpdate(
      UTIL_AngleMod(CalcIdealYaw(GetAbsOrigin() + GetAbsVelocity())),
      AI_KEEP_YAW_SPEED);
...
//			eMoveType -
//-----------------------------------------------------------------------------
void CNPC_Ichthyosaur::DoMovement(float flInterval, const Vector &MoveTarget,
                                  int eMoveType) {
  // dvs: something is setting this bit, causing us to stop moving and get stuck
  // that way
  ...
  // NDebugOverlay::Line( GetLocalOrigin(), GetLocalOrigin()+SteerAvoid, 255,
  // 255, 0, false, 0.1f );
...
void CNPC_Ichthyosaur::SteerArrive(Vector &Steer, const Vector &Target) {
  ...
  float fIdealSpeed =
      m_flGroundSpeed * (fTargetDistance / ICH_WAYPOINT_DISTANCE);
...
//-----------------------------------------------------------------------------
bool CNPC_Ichthyosaur::SteerAvoidObstacles(Vector &Steer,
  ...
  AI_TraceHull(GetAbsOrigin(), GetAbsOrigin() + (dir * speed), GetHullMins(),
  ...
  if (tr.fraction < 1.0f) {
    ...
    if ((pBlocker != NULL) && (pBlocker->MyNPCPointer() != NULL)) {
      ...
      if (tr.fraction > 0) {
        ...
        // NDebugOverlay::Line( GetLocalOrigin(), GetLocalOrigin()+Steer, 255,
        // 0, 0, false, 0.1f );
      } else {
      ...
      // NDebugOverlay::Line( GetLocalOrigin(), GetLocalOrigin()+Steer, 255, 0,
      // 0, false, 0.1f );
    }
  ...
  AI_TraceLine(GetAbsOrigin(),
               GetAbsOrigin() + Vector(0, 0, -ICH_HEIGHT_PREFERENCE),
  ...
  if (m_bIgnoreSurface == false) {
    float waterLevel =
        (UTIL_WaterLevel(GetAbsOrigin(), GetAbsOrigin().z,
                         GetAbsOrigin().z + ICH_DEPTH_PREFERENCE) -
         GetAbsOrigin().z) /
...
//				the clamped steering vector in absolute (x,y,z)
//coordinates. 			SteerRel - Receives the clamped steering vector in relative
//(forward, right, up) 				coordinates. 			forward - Our current forward vector. 			right
//- Our current right vector. 			up - Our current up vector.
//-----------------------------------------------------------------------------
void CNPC_Ichthyosaur::ClampSteer(Vector &SteerAbs, Vector &SteerRel,
  ...
  SteerAbs =
      (fForwardSteer * forward) + (fRightSteer * right) + (fUpSteer * up);
...
//			flInterval -
//-----------------------------------------------------------------------------
void CNPC_Ichthyosaur::MoveFlyExecute(CBaseEntity *pTargetEnt,
                                      const Vector &vecDir, float flDistance,
                                      float flInterval) {
  IchthyosaurMoveType_t eMoveType = (GetNavigator()->CurWaypointIsGoal())
                                        ? ICH_MOVETYPE_ARRIVE
                                        : ICH_MOVETYPE_SEEK;
  ...
  if ((GetEnemy() != NULL) &&
      (GetNavigator()->GetGoalTarget() == (CBaseEntity *)GetEnemy())) {
    trace_t tr;
    Vector goalPos =
        GetEnemy()->GetAbsOrigin() + (GetEnemy()->GetSmoothedVelocity() * 0.5f);

    AI_TraceHull(GetAbsOrigin(), goalPos, GetHullMins(), GetHullMaxs(),
                 MASK_NPCSOLID, GetEnemy(), COLLISION_GROUP_NONE, &tr);
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_Ichthyosaur::FVisible(CBaseEntity *pEntity, int traceMask,
...
int CNPC_Ichthyosaur::MeleeAttack1Conditions(float flDot, float flDist) {
  Vector predictedDir =
      ((GetEnemy()->GetAbsOrigin() + (GetEnemy()->GetSmoothedVelocity())) -
       GetAbsOrigin());
  ...
  if ((flPredictedDist > (GetAbsVelocity().Length() * 0.5f)) &&
      (flDist > 128.0f))
...
void CNPC_Ichthyosaur::Bite(void) {
  ...
  if ((GetEnemy() != NULL) && (GetEnemy()->Classify() == CLASS_BULLSEYE)) {
    ...
    pHurt = CheckTraceHullAttack(108, Vector(-32, -32, -32), Vector(32, 32, 32),
                                 0, DMG_CLUB);
  }

  // Hit something
  if (pHurt != NULL) {
    CTakeDamageInfo info(this, this, sk_ichthyosaur_melee_dmg.GetInt(),
                         DMG_CLUB);
    ...
    if (pHurt->IsPlayer()) {
      ...
      if (pPlayer) {
        if (((m_flHoldTime < gpGlobals->curtime) &&
             (pPlayer->m_iHealth < (pPlayer->m_iMaxHealth * 0.5f))) ||
            (pPlayer->GetWaterLevel() < 1)) {
          // EnsnareVictim( pHurt );
        } else {
          info.SetDamage(sk_ichthyosaur_melee_dmg.GetInt() * 3);
        }
        CalculateMeleeDamageForce(&info, GetAbsVelocity(),
                                  pHurt->GetAbsOrigin());
    ...
    UTIL_Bubbles(pHurt->GetAbsOrigin() + Vector(-32.0f, -32.0f, -32.0f),
                 pHurt->GetAbsOrigin() + Vector(32.0f, 32.0f, 0.0f),
                 random->RandomInt(16, 32));
...
bool CNPC_Ichthyosaur::Beached(void) {
  ...
  AI_TraceHull(GetAbsOrigin(), testPos, GetHullMins(), GetHullMaxs(),
               MASK_NPCSOLID, this, COLLISION_GROUP_NONE, &tr);
...
void CNPC_Ichthyosaur::PrescheduleThink(void) {
  ...
  if ( random->RandomInt( 0, 20 ) == 10 )
  {
          ...
          {
                  ENVELOPE_CONTROLLER.SoundChangeVolume( m_pSwimSound,
  random->RandomFloat( 0.0f, 0.5f ), 1.0f );
          }
          else
          {
                  ENVELOPE_CONTROLLER.SoundChangeVolume( m_pVoiceSound,
  random->RandomFloat( 0.0f, 0.5f ), 1.0f );
          }
  }
  */
  ...
  if (random->RandomInt(0, 10) == 0) {
    UTIL_Bubbles(GetAbsOrigin() + (GetHullMins() * 0.5f),
                 GetAbsOrigin() + (GetHullMaxs() * 0.5f), 1);
  }
...
void CNPC_Ichthyosaur::EnsnareVictim(CBaseEntity *pVictim) {
  ...
  if (pBCC &&
      pBCC->DispatchInteraction(g_interactionBarnacleVictimGrab, NULL, this)) {
    if (pVictim->IsPlayer()) {
      CBasePlayer *pPlayer =
          dynamic_cast<CBasePlayer *>((CBaseEntity *)pVictim);

      if (pPlayer) {
        m_flHoldTime =
            MAX(gpGlobals->curtime + 3.0f, pPlayer->PlayerDrownTime() - 2.0f);
      }
    } else {
...
void CNPC_Ichthyosaur::StartTask(const Task_t *pTask) {
  ...
  case TASK_ICH_GET_PATH_TO_RANDOM_NODE: {
    if (GetEnemy() == NULL ||
        !GetNavigator()->SetRandomGoal(GetEnemy()->GetLocalOrigin(),
                                       pTask->flTaskData)) {
...
float CNPC_Ichthyosaur::MaxYawSpeed(void) {
  ...
  return ICH_MIN_TURN_SPEED +
         ((ICH_MAX_TURN_SPEED - ICH_MIN_TURN_SPEED) *
          (fabs(GetAbsVelocity().Length()) / ICH_SWIM_SPEED_RUN));
}
...
//			&tolerance -
//-----------------------------------------------------------------------------
void CNPC_Ichthyosaur::TranslateNavGoal(CBaseEntity *pEnemy,
...
AI_DEFINE_SCHEDULE(
    ...
    "SCHEDULE:SCHED_ICH_PATROL_WALK"
