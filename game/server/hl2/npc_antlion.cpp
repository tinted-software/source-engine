...
bool CNPC_Antlion::GetGroundPosition(const Vector &testPos, Vector &result) {
  ...
  if (tr.allsolid) {
    if (g_debug_antlion.GetInt() == 3) {
      NDebugOverlay::BoxDirection(testPos, NAI_Hull::Mins(GetHullType()),
                                  NAI_Hull::Maxs(GetHullType()) +
                                      Vector(0, 0, 128),
                                  Vector(0, 0, 1), 255, 0, 0, true, 2.0f);
    }
  ...
  if (g_debug_antlion.GetInt() == 3) {
    NDebugOverlay::BoxDirection(testPos, NAI_Hull::Mins(GetHullType()),
                                NAI_Hull::Maxs(GetHullType()) +
                                    Vector(0, 0, 128),
                                Vector(0, 0, 1), 0, 255, 0, true, 2.0f);
  }
...
void CNPC_Antlion::HandleAnimEvent(animevent_t *pEvent) {
  ...
  if (pEvent->event == AE_ANTLION_MELEE_HIT1) {
    ...
    MeleeAttack(ANTLION_MELEE1_RANGE, sk_antlion_swipe_damage.GetFloat(), qa,
                vec);
    return;
  }
  ...
  if (pEvent->event == AE_ANTLION_MELEE_POUNCE) {
    ...
    Vector vec(-250.0f, 1.0f, 1.0f);
    MeleeAttack(ANTLION_MELEE2_RANGE, sk_antlion_swipe_damage.GetFloat(), qa,
                vec);
    return;
  }
...
void CNPC_Antlion::StartTask(const Task_t *pTask) {
  ...
  case TASK_ANTLION_FIND_BURROW_IN_POINT:
    ...
    if (FindBurrow(GetAbsOrigin(), pTask->flTaskData, ANTLION_BURROW_IN) ==
        false) {
      TaskFail("TASK_ANTLION_FIND_BURROW_IN_POINT: Unable to find burrow in "
               "position\n");
    } else {
      TaskComplete();
    }
  ...
  case TASK_ANTLION_FIND_BURROW_OUT_POINT:
    ...
    if (FindBurrow(GetAbsOrigin(), pTask->flTaskData, ANTLION_BURROW_OUT) ==
        false) {
      TaskFail("TASK_ANTLION_FIND_BURROW_OUT_POINT: Unable to find burrow out "
               "position\n");
    } else {
      TaskComplete();
    }
  ...
  case TASK_ANTLION_VANISH:
    ...
    if (pTask->flTaskData) {
      ...
      // NOTE: We can't UTIL_Remove here, because we're in the middle of running
      // our AI, and
...
bool CNPC_Antlion::CheckLanding(void) {
  ...
  testPos[2] -=
      (0.5 * GetCurrentGravity() * GetGravity() * timeStep * timeStep);
...
bool CNPC_Antlion::CorpseGib(const CTakeDamageInfo &info) {
  ...
  static breakablepropparams_t params(EyePosition(), GetAbsAngles(), velocity,
                                      angVelocity);
...
void CNPC_Antlion::InputJumpAtTarget(inputdata_t &inputdata) {
  ...
  Vector rawJumpVel = GetMoveProbe()->CalcJumpLaunchVelocity(
      GetAbsOrigin(), targetPos, GetCurrentGravity() * GetJumpGravity(),
