...
#define SF_TANK_ALLOW_PLAYER_HITS                                              \
  0x1000 // 4096		Allow friendly NPCs to fire upon enemies near
         // the player
#define SF_TANK_IGNORE_RANGE_IN_VIEWCONE                                       \
  0x2000 // 8192		Don't use range as a factor in determining if
         // something is in view cone
#define SF_TANK_NOTSOLID 0x8000 // 32768
#define SF_TANK_SOUNDON                                                        \
  0x10000 // FIXME: This is not really a spawnflag! It holds transient state!!!
...
#ifdef HL2_EPISODIC
  string_t m_iszAmmoType; // The name of the ammodef that we use when we fire.
                          // Bullet damage still comes from keyvalues.
...
private:
  ...
  float
      m_persist; // Persistence of firing (how long do I shoot when I can't see)
  float m_persist2; // Secondary persistence of firing (randomly shooting when I
                    // can't see)
  ...
  // Used when the gun is actually a part of the parent entity, and pose params
  // aim it
