...
void CBaseHeadcrab::RunTask(const Task_t *pTask) {
  ...
  case TASK_HEADCRAB_CEILING_FALL: {
    ...
    UTIL_TraceHull(vecPrPos, vecPrPos, GetHullMins(), GetHullMaxs(), MASK_SOLID,
...
void CBaseHeadcrab::StartTask(const Task_t *pTask) {
  ...
  case TASK_HEADCRAB_FIND_BURROW_IN_POINT: {
    if (FindBurrow(GetAbsOrigin(), pTask->flTaskData, true) == false) {
      TaskFail("TASK_HEADCRAB_FIND_BURROW_IN_POINT: Unable to find burrow in "
               "position\n");
    } else {
      TaskComplete();
    }
...
int CBaseHeadcrab::SelectSchedule(void) {
  ...
  if (m_bHidden || HasSpawnFlags(SF_NPC_WAIT_TILL_SEEN)) {
    ...
    return m_bBurrowed ? (int)SCHED_HEADCRAB_BURROW_WAIT
                       : (int)SCHED_IDLE_STAND;
  }
  ...
  if (IsHangingFromCeiling()) {
    ...
    if (bIsAlyxInDarknessMode == false &&
...
void CFastHeadcrab::RunTask(const Task_t *pTask) {
  ...
  case TASK_HEADCRAB_HOP_ASIDE:
    ...
    if (GetFlags() & FL_ONGROUND) {
      ...
      if (GetEnemy() && (GetEnemy()->GetAbsOrigin() - GetAbsOrigin()).Length() >
                            HEADCRAB_MAX_JUMP_DIST) {
        TaskFail("");
      }
  ...
  case TASK_HEADCRAB_HOP_ASIDE: {
    ...
    if (tr.fraction == 1.0) {
      ...
      if (!IsMoveBlocked(moveTrace)) {
        SetAbsVelocity(
            m_vecJumpVel); // + 0.5f * Vector(0,0,GetCurrentGravity()) *
                           // flInterval;
...
//velocity. 			pEnemy - Who we should acquire as our enemy. Usually our zombie
//host's enemy.
//-----------------------------------------------------------------------------
...
//ambush, and 			you could walk up behind them without having them attack you. 			This
//vision extends only 24 feet.
//-----------------------------------------------------------------------------
...
DEFINE_SCHEDULE(
    ...
    "	Interrupts"
    "		COND_TASK_FAILED"
    "		COND_NEW_ENEMY" // HACK: We don't actually choose a new schedule
                                // on new enemy, but we need this interrupt so
                                // that the headcrab actually acquires new
                                // enemies while burrowed. (look in
                                // ai_basenpc.cpp for "DO NOT mess")
    "		COND_CAN_RANGE_ATTACK1")
