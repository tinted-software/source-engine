...
public:
  CHLPlayerMove()
      : m_bWasInVehicle(false), m_bVehicleFlipped(false), m_bInGodMode(false),
        m_bInNoClip(false) {
    m_vecSaveOrigin.Init();
  }

  void SetupMove(CBasePlayer *player, CUserCmd *ucmd, IMoveHelper *pHelper,
                 CMoveData *move);
...
void CHLPlayerMove::SetupMove(CBasePlayer *player, CUserCmd *ucmd,
...
void CHLPlayerMove::FinishMove(CBasePlayer *player, CUserCmd *ucmd,
                               CMoveData *move) {
  ...
  if (gpGlobals->frametime != 0) {
    ...
    if (pVehicle) {
      ...
      CPropVehicleDriveable *driveable =
          dynamic_cast<CPropVehicleDriveable *>(player->GetVehicleEntity());
      if (driveable) {
    ...
    if (distance > 0) {
      gamestats->Event_PlayerTraveled(
          player, distance, pVehicle ? true : false,
