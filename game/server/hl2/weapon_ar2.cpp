...
DEFINE_FIELD(m_flDelayedFire, FIELD_TIME),
    ...
    END_DATADESC()
        ...
        IMPLEMENT_SERVERCLASS_ST(CWeaponAR2, DT_WeaponAR2) END_SEND_TABLE()
...
void CWeaponAR2::DelayedAttack(void) {
  ...
  pOwner->ViewPunch(
      QAngle(random->RandomInt(-8, -12), random->RandomInt(1, 2), 0));
...
// Input  : *pOperator -
//-----------------------------------------------------------------------------
void CWeaponAR2::FireNPCPrimaryAttack(CBaseCombatCharacter *pOperator,
...
// Purpose:
//-----------------------------------------------------------------------------
void CWeaponAR2::FireNPCSecondaryAttack(CBaseCombatCharacter *pOperator,
                                        bool bUseWeaponAngles) {
  WeaponSound(WPN_DOUBLE);
  ...
  if (bUseWeaponAngles) {
    ...
    if (pSoldier) {
      ...
      // Therefore, we must ask them specifically what direction they are
      // shooting.
  ...
  float flDuration =
      RemapValClamped(flAmmoRatio, 0.0f, 1.0f, 0.5f,
  ...
  // Fire the combine ball
  CreateCombineBall(vecSrc, vecVelocity, flRadius,
                    sk_weapon_ar2_alt_fire_mass.GetFloat(), flDuration, pNPC);
}
...
//			*pOperator -
//-----------------------------------------------------------------------------
void CWeaponAR2::Operator_HandleAnimEvent(animevent_t *pEvent,
...
#define SLIDE_LIMIT 5.0f       // Seconds
  ...
  if (g_pGameRules->GetAutoAimMode() == AUTOAIM_ON_CONSOLE) {
    // On the 360 (or in any configuration using the 360 aiming scheme), don't
    // let the AR2 progressive into the late, highly inaccurate stages of its
    // kick. Just spoof the time to make it look (to the kicking code) like we
    // haven't been firing for very long.
    flDuration = MIN(flDuration, 0.75f);
  }

  DoMachineGunKick(pPlayer, EASY_DAMPEN, MAX_VERTICAL_KICK, flDuration,
                   SLIDE_LIMIT);
}
