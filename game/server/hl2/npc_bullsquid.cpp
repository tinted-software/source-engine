...
void CNPC_Bullsquid::Precache() {
  ...
  m_nSquidSpitSprite =
      PrecacheModel("sprites/greenspit1.vmt"); // client side spittle.
...
// AlertSound
//=========================================================
void CNPC_Bullsquid::AlertSound(void) { EmitSound("NPC_Bullsquid.Alert"); }
...
// AttackSound
//=========================================================
void CNPC_Bullsquid::AttackSound(void) { EmitSound("NPC_Bullsquid.Attack1"); }
...
void CNPC_Bullsquid::HandleAnimEvent(animevent_t *pEvent) {
  ...
  case BSQUID_AE_SPIT: {
    if (GetEnemy()) {
      ...
      ThrowLimit(vSpitPos, vTarget, flGravity, 3, Vector(0, 0, 0),
                 Vector(0, 0, 0), GetEnemy(), &vToss, &pBlocker);

      CGrenadeSpit *pGrenade = (CGrenadeSpit *)CreateNoSpawn(
          "grenade_spit", vSpitPos, vec3_angle, this);
      ...
      pGrenade->SetLocalAngularVelocity(QAngle(
          random->RandomFloat(-100, -500), random->RandomFloat(-100, -500),
          random->RandomFloat(-100, -500)));

      AttackSound();

      CPVSFilter filter(vSpitPos);
      te->SpriteSpray(filter, 0.0, &vSpitPos, &vToss, m_nSquidSpitSprite, 5, 10,
                      15);
    }
  } break;
  ...
  case BSQUID_AE_WHIP_SND: {
    ...
    /*
                    ...
                    {
                            CBaseEntity *pHurt = CheckTraceHullAttack( 70,
       Vector(-16,-16,-16), Vector(16,16,16), sk_bullsquid_dmg_whip.GetFloat(),
       DMG_SLASH | DMG_ALWAYSGIB ); if ( pHurt )
                            {
                                    Vector right, up;
                                    AngleVectors( GetAbsAngles(), NULL, &right,
       &up );

                                    if ( pHurt->GetFlags() & ( FL_NPC |
       FL_CLIENT ) ) pHurt->ViewPunch( QAngle( 20, 0, -20 ) );

                                    pHurt->ApplyAbsVelocityImpulse( 100 *
       (up+2*right) );
                            }
                    }
                    break;
  ...
  case BSQUID_AE_THROW: {
    ...
    if (pHurt) {
      ...
      // If not the player see if has bullsquid throw interatcion
        ...
        if (pVictim) {
          if (pVictim->DispatchInteraction(g_interactionBullsquidThrow, NULL,
                                           this)) {
            Vector forward, up;
            AngleVectors(GetLocalAngles(), &forward, NULL, &up);
...
int CNPC_Bullsquid::MeleeAttack2Conditions(float flDot, float flDist) {
  if (flDist <= 85 && flDot >= 0.7 &&
      !HasCondition(COND_CAN_MELEE_ATTACK1)) // The player & bullsquid can be as
                                             // much as their bboxes
...
NPC_STATE CNPC_Bullsquid::SelectIdealState(void) {
  ...
  case NPC_STATE_COMBAT: {
    ...
    if (GetEnemy() != NULL &&
        ...
        FClassnameIs(GetEnemy(), "monster_headcrab")) {
      // if the squid has a headcrab enemy and something hurts it, it's going to
      // forget about the crab for a while.
      SetEnemy(NULL);
