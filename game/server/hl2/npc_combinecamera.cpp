...
//			until they either acquire an enemy to track or are told
//to track 			an entity via an input. If they lose their target they will 			revert to
//tracking their default target. They acquire enemies 			using the relationship
//table just like any other NPC.
//
//			Cameras have two zones of awareness, an inner zone
//formed by the 			intersection of an inner FOV and an inner radius. The camera is
//			fully aware of entities in the inner zone and will
//acquire enemies 			seen there.
//
//			The outer zone of awareness is formed by the
//intersection of an 			outer FOV and an outer radius. The camera is only vaguely
//aware 			of entities in the outer zone and will flash amber when enemies 			are
//there, but will otherwise ignore them.
//
//			They can be made angry via an input, at which time they
//sound an 			alarm and snap a few pictures of whatever they are tracking. They 			can
//also be set to become angry anytime they acquire an enemy.
//
//=============================================================================//
...
// Eye states
enum eyeState_t {
  CAMERA_EYE_IDLE, // Nothing abnormal in the inner or outer viewcone, dim
                   // green.
  CAMERA_EYE_SEEKING_TARGET, // Something in the outer viewcone, flashes amber
                             // as it converges on the target.
  ...
  CAMERA_EYE_DISABLED, // Turned off, must be reactivated before it'll deploy
...
protected:
  ...
  int m_nInnerRadius; // The camera will only lock onto enemies that are within
                      // the inner radius.
  int m_nOuterRadius; // The camera will flash amber when enemies are within the
                      // outer radius, but outside the inner radius.
...
DEFINE_FIELD(m_iAmmoType, FIELD_INTEGER),
    ...
    DEFINE_THINKFUNC(Deploy), DEFINE_THINKFUNC(ActiveThink),
...
void CNPC_CombineCamera::Spawn() {
  ...
  m_pEyeFlash = CSprite::SpriteCreate(COMBINE_CAMERA_FLASH_SPRITE,
  ...
  m_pEyeGlow = CSprite::SpriteCreate(COMBINE_CAMERA_GLOW_SPRITE,
                                     GetLocalOrigin(), false);
  m_pEyeGlow->SetTransparency(kRenderWorldGlow, 255, 0, 0, 128,
                              kRenderFxNoDissipation);
...
int CNPC_CombineCamera::OnTakeDamage(const CTakeDamageInfo &inputInfo) {
  ...
  if (m_iHealth <= 0) {
    ...
    // FIXME: This needs to throw a ragdoll gib or something other than
    // animating the retraction -- jdw
...
bool CNPC_CombineCamera::UpdateFacing() {
  ...
  float flDiff = AngleNormalize(
      UTIL_ApproachAngle(vecGoalLocalAngles.x, 0.0, 0.1f * MaxYawSpeed()));
  ...
  flDiff = AngleNormalize(
      UTIL_ApproachAngle(vecGoalLocalAngles.y, 0.0, 0.1f * MaxYawSpeed()));
...
//-----------------------------------------------------------------------------
bool CNPC_CombineCamera::FVisible(CBaseEntity *pEntity, int traceMask,
...
//-----------------------------------------------------------------------------
// Purpose: Called when we have no scripted target. Looks for new enemies to
// track.
//-----------------------------------------------------------------------------
...
void CNPC_CombineCamera::TrackTarget(CBaseEntity *pTarget) {
  ...
  if (g_debug_combine_camera.GetBool()) {
    NDebugOverlay::Cross3D(vecMid, -Vector(2, 2, 2), Vector(2, 2, 2), 0, 255, 0,
                           false, 0.05);
    NDebugOverlay::Cross3D(pTarget->WorldSpaceCenter(), -Vector(2, 2, 2),
                           Vector(2, 2, 2), 0, 255, 0, false, 0.05);
    NDebugOverlay::Line(vecMid, pTarget->WorldSpaceCenter(), 0, 255, 0, false,
                        0.05);

    NDebugOverlay::Cross3D(vecMid, -Vector(2, 2, 2), Vector(2, 2, 2), 0, 255, 0,
                           false, 0.05);
    NDebugOverlay::Cross3D(vecMidTarget, -Vector(2, 2, 2), Vector(2, 2, 2), 0,
                           255, 0, false, 0.05);
...
void CNPC_CombineCamera::MaintainEye() {
  ...
  if ((m_bAngry) && (m_nClickCount <= 3)) {
    ...
    } else if ((m_flTurnOffEyeFlashTime != 0) &&
               (m_flTurnOffEyeFlashTime < gpGlobals->curtime)) {
...
void CNPC_CombineCamera::SearchThink() {
  ...
  m_vecGoalAngles.y =
      GetAbsAngles().y + (sin(gpGlobals->curtime * 2.0f) * 45.0f);
...
void CNPC_CombineCamera::DrawDebugGeometryOverlays(void) {
  ...
  if ((m_debugOverlays & OVERLAY_NPC_VIEWCONE_BIT)) {
    ...
    NDebugOverlay::BoxDirection(EyePosition(), Vector(0, 0, -40),
                                Vector(200, 0, 40), vLeftDir, 255, 255, 0, 50,
                                0);
    NDebugOverlay::BoxDirection(EyePosition(), Vector(0, 0, -40),
                                Vector(200, 0, 40), vRightDir, 255, 255, 0, 50,
                                0);
    NDebugOverlay::Box(EyePosition(), -Vector(2, 2, 2), Vector(2, 2, 2), 255,
                       255, 0, 128, 0);
  }
