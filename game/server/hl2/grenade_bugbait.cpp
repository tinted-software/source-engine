...
void CGrenadeBugBait::BugBaitTouch(CBaseEntity *pOther) {
  ...
  UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + traceDir * 64, MASK_SHOT,
  ...
  if (ActivateBugbaitTargets(GetThrower(), GetAbsOrigin(), false) == false) {
    ...
    CSoundEnt::InsertSound(SOUND_BUGBAIT, GetAbsOrigin(),
                           bugbait_hear_radius.GetInt(),
...
//			Returns true if the bugbait target wants to suppress the
//call.
//-----------------------------------------------------------------------------
bool CGrenadeBugBait::ActivateBugbaitTargets(CBaseEntity *pOwner,
                                             Vector vecOrigin, bool bSqueezed) {
  // Attempt to activate any spawners in a radius around the bugbait
  CBaseEntity *pList[100];
  Vector delta(bugbait_grenade_radius.GetFloat(),
               bugbait_grenade_radius.GetFloat(),
               bugbait_grenade_radius.GetFloat());
  bool suppressCall = false;

  int count =
      UTIL_EntitiesInBox(pList, 100, vecOrigin - delta, vecOrigin + delta, 0);
  ...
  if (!bSqueezed) {
    for (int i = 0; i < count; i++) {
      ...
      if (UTIL_DistApprox(pList[i]->WorldSpaceCenter(), vecOrigin) <
          bugbait_grenade_radius.GetFloat()) {
        // Must be a soldier
        if (FClassnameIs(pList[i], "npc_combine_s")) {
          ...
          if (pCombine != NULL) {
            trace_t tr;
            UTIL_TraceLine(vecOrigin, pCombine->EyePosition(), MASK_ALL, pOwner,
            ...
            if (tr.fraction == 1.0 || tr.m_pEnt == pCombine) {
              ...
              g_EventQueue.AddEvent(pCombine, "HitByBugbait",
                                    RandomFloat(0, 0.5), pOwner, pOwner);
            }
          }
        }
      }
  ...
  for (CBugBaitSensor *pSensor = GetBugBaitSensorList(); pSensor != NULL;
       pSensor = pSensor->m_pNext) {
    if (pSensor == NULL)
      continue;
    ...
    if (pSensor->GetRadius() > (pSensor->GetAbsOrigin() - vecOrigin).Length()) {
      ...
      if (pSensor->Baited(pOwner)) {
        // If we're suppressing the call to antlions, then don't make a bugbait
        // sound
...
// Output : CBaseGrenade
//-----------------------------------------------------------------------------
CGrenadeBugBait *BugBaitGrenade_Create(const Vector &position,
                                       const QAngle &angles,
                                       const Vector &velocity,
                                       const QAngle &angVelocity,
                                       CBaseEntity *owner) {
  CGrenadeBugBait *pGrenade = (CGrenadeBugBait *)CBaseEntity::Create(
      "npc_grenade_bugbait", position, angles, owner);
