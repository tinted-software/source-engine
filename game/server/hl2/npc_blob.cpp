...
//
// npc_blob - experimental, cpu-intensive monster made of lots of smaller
// elements
//
//=============================================================================//
...
public:
  Vector
      m_vecPrevOrigin; // Only exists for debugging (isolating stuck elements)
  int m_iStuckCount;
  bool m_bOnWall;
...
DEFINE_FIELD(m_vecPrevOrigin, FIELD_POSITION_VECTOR),
    ...
    END_DATADESC()
        ...
        const
    char *pszBlobModels[] = {"models/gibs/agibs.mdl",
                             "models/props_junk/watermelon01.mdl",
                             "models/w_squeak.mdl", "models/baby_headcrab.mdl"};
...
void CBlobElement::ModifyVelocityForSurface(float flInterval, float flSpeed) {
  ...
  UTIL_TraceLine(vecStart + up, vecWishedGoal + up, MASK_SHOT, this,
  ...
  if (tr.fraction == 1.0f) {
    UTIL_TraceLine(vecWishedGoal + up, vecWishedGoal - (up * 2.0f), MASK_SHOT,
    ...
    if (tr.m_pEnt != NULL && !tr.m_pEnt->IsWorld()) {
      ...
      if (pPhysics != NULL) {
        ...
        if (tr.endpos.z > vecMassCenterWorld.z) {
          pPhysics->ApplyForceOffset(
              (-150.0f * m_flRandomEightyPercent) * tr.plane.normal, tr.endpos);
        }
      }
    }
  }
...
// Output :
//-----------------------------------------------------------------------------
Class_T CNPC_Blob::Classify(void) { return CLASS_PLAYER_ALLY; }
...
void CNPC_Blob::RunAI() {
  ...
  if (iIdealNumElements != m_iNumElements) {
    ...
    if (delta < 0) {
      ...
      if (m_iReconfigureElement > m_iNumElements) {
        // Start this index over at zero, if it is past the new end of the
        // utlvector.
  ...
  if (npc_blob_show_centroid.GetBool()) {
    NDebugOverlay::Cross3D(m_vecCentroid + Vector(0, 0, 12), 32, 0, 255, 0,
                           false, 0.025f);
  }
...
void CNPC_Blob::DoBlobBatchedAI(int iStart, int iEnd) {
  ...
  // Group cohesion
  float flBlobRadiusSqr =
      Square(blob_radius.GetFloat() + 48.0f); // Four feet of fudge

  // Build a right-hand vector along which we'll add some sine wave data to give
  // each element a unique insect-like undulation along an axis perpendicular to
  // their path, which makes the entire group look far less orderly
  if (GetEnemy() != NULL) {
    // If I have an enemy, the right-hand vector is perpendicular to a straight
    // line from the group's centroid to the enemy's origin.
  ...
  for (int i = iStart; i < iEnd; i++) {
    ...
    // Initial movement
    //--
    // Start out with bEnforceSpeedLimit set to false. This is because an
    // element can't overspeed if it's moving undisturbed towards its target
    // entity or target location. An element can only under or overspeed when it
    // is repelled by multiple other elements in the group. See "Relative
    // Positions" below.
    //
    // Initialize some 'defaults' that may be changed for each iteration of this
    // loop
    ...
    case BLOB_MOVE_DONT_MOVE: {
      ...
      UTIL_TraceLine(vecOrigin, vecOrigin - Vector(0, 0, 16), MASK_SHOT, this,
                     COLLISION_GROUP_NONE, &tr);
    ...
    case BLOB_MOVE_TO_TARGET_LOCATION: {
      Vector vecDiff =
          pThisElement->GetAbsOrigin() - pThisElement->m_vecTargetLocation;

      if (vecDiff.Length2DSqr() <= Square(80.0f)) {
        // Don't shove this guy around any more, let him get to his goal
        // position.
        flSpeed *= 0.5f;
    ...
    case BLOB_MOVE_TO_TARGET_ENTITY: {
      if (!IsMoving() && GetEnemy() == NULL) {
        if (pThisElement->GetAbsOrigin().DistToSqr(GetAbsOrigin()) <=
            flBlobRadiusSqr) {
          flSpeed = (flSpeed * flIdleSpeedFactor) *
                    pThisElement->m_flRandomEightyPercent;
        }
      }
    ...
    // Relative positions
    //--
    // Check this element against ALL other elements. If the two elements are
    // closer than the allowed minimum distance, repel this element away. (The
    // other element will repel when its AI runs). A single element can be
    // repelled by many other elements. This is why bEnforceSpeedLimit is set to
    // true if any of the repelling code runs for this element. Multiple
    // attempts to repel an element in the same direction will cause overspeed.
    // Conflicting attempts to repel an element in opposite directions will
    // cause underspeed.
    ...
    if (bEnforceRelativePositions) {
      for (int j = 0; j < m_Elements.Count(); j++) {
        ...
        // If this element is on the wall, then don't be repelled by anyone.
        // Repelling elements that are trying to climb a wall usually make them
        // look like they fall off the wall a few times while climbing.
        ...
        if (i != j) {
          ...
          if (distSqr < minDistSqr) {
            ...
            flRepelSpeed = (flSpeed * (1.0f - (distSqr / minDistSqr))) *
                           pThatElement->GetSinePhase();
            pThisElement->AddElementVelocity(vecRepelDir * flRepelSpeed, true);

            // Since we altered this element's velocity after it was initially
            // set, there's a chance that the sums of multiple vectors will
            // cause the element to over or underspeed, so mark it for speed
            // limit enforcement
    ...
    if (bDoMovementVariation) {
      ...
      pThisElement->AddElementVelocity(vecRight * (flMySine * flMyAmplitude),
                                       true);
    }
    ...
    for (int a = 0; a < m_iNumAvoidOrigins; a++) {
      ...
      if (vecAvoidDir.LengthSqr() <=
          (m_flAvoidRadiusSqr * pThisElement->m_flRandomEightyPercent)) {
    ...
    pThisElement->m_flDistFromCentroidSqr =
        pThisElement->m_vecPrevOrigin.DistToSqr(m_vecCentroid);
    ...
    /*
                    ...
                    //
                    if( pThisElement->m_flDistFromCentroidSqr >
       flStragglerDistSqr )
                    {
                            NDebugOverlay::Line( pThisElement->GetAbsOrigin(),
       m_vecCentroid, 255, 0, 0, false, 0.025f );
                    }
    */
  }
}
...
void CNPC_Blob::AddNewElements(int iNumElements) {
  ...
  // Since the new elements copy their origins from existing elements, we only
  // want to copy origins from elements that existed before we came into this
  // function. Otherwise, the more elements we create while in this function,
  // the more likely it becomes that several of them will stack on the same
  // origin.
  ...
  for (i = 0; i < iNumElements; i++) {
    ...
    if (pElement != NULL) {
      // Copy the origin of some element that is not me. This will make the
      // expansion of the group easier on the eye, since this element will spawn
      // inside of some other element, and then be pushed out by the blob's
      // repel rules.
...
void CNPC_Blob::FormShapeFromPath(string_t iszPathName) {
  ...
  int element = 0;
  for (i = 0; i < iNumVerts - 1; i++) {
    // NDebugOverlay::Line( vertex[ i ], vertex[ i + 1 ], 0, 255, 0,
    // false, 10.0f );
    ...
    for (flStep = 0.0f; flStep < flSegmentLength; flStep += flDistribution) {
      // NDebugOverlay::Cross3D( vecStart + vecDiff * flStep, 16, 255, 255, 255,
      // false, 10.0f );
...
//-----------------------------------------------------------------------------
int CNPC_Blob::GetBatchStart() { return m_iBatchStart; }
...
CBlobElement *CNPC_Blob::CreateNewElement() {
  CBlobElement *pElement =
      static_cast<CBlobElement *>(CreateEntityByName("blob_element"));
...
void CNPC_Blob::InitializeElements() {
  ...
  for (i = 0; i < iNumElements; i++) {
    ...
    trace_t tr;
    UTIL_TraceLine(vecDest, vecDest + Vector(0, 0, MIN_COORD_FLOAT), MASK_SHOT,
                   pElement, COLLISION_GROUP_NONE, &tr);
