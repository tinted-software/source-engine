...
ConVar rpg_missle_use_custom_detonators("rpg_missle_use_custom_detonators",
                                        "1");
...
CLaserDot *GetLaserDotList() { return g_LaserDotList.m_pClassList; }
...
void CMissile::Explode(void) {
  ...
  trace_t tr;
  UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + forward * 16, MASK_SHOT, this,
...
void CMissile::ComputeActualDotPosition(CLaserDot *pLaserDot,
  ...
  *pHomingSpeed = RPG_HOMING_SPEED;
  ...
  //0, true, 0.05f ); 	NDebugOverlay::Line( GetAbsOrigin(), *pActualDotPosition,
  //255, 0, 0, true, 0.05f ); 	NDebugOverlay::Cross3D( *pActualDotPosition,
...
void CMissile::SeekThink(void) {
  ...
  if (hl2_episodic.GetBool()) {
    if (flBestDist <= (GetAbsVelocity().Length() * 2.5f) &&
        FVisible(pBestDot->GetAbsOrigin())) {
      // Scare targets
      CSoundEnt::InsertSound(SOUND_DANGER, pBestDot->GetAbsOrigin(),
                             CMissile::EXPLOSION_RADIUS, 0.2f, pBestDot,
  ...
  if (rpg_missle_use_custom_detonators.GetBool()) {
    for (int i = gm_CustomDetonators.Count() - 1; i >= 0; --i) {
      ...
      if (!detonator.hEntity) {
        ...
        const Vector &vPos =
            detonator.hEntity->CollisionProp()->WorldSpaceCenter();
        if (detonator.halfHeight > 0) {
          if (fabsf(vPos.z - GetAbsOrigin().z) < detonator.halfHeight) {
            if ((GetAbsOrigin().AsVector2D() - vPos.AsVector2D()).LengthSqr() <
                detonator.radiusSq) {
              Explode();
              return;
            }
  ...
  if (pLaserDot->GetTargetEntity() != NULL && flDist <= 240.0f &&
    ...
    // Prevent the missile circling the Strider like a Halo in ep1_c17_06. If
    // the missile gets within 20 feet of a Strider, tighten up the turn speed
    // of the missile so it can break the halo and strike. (sjb 4/27/2006)
  ...
  if (gpGlobals->frametime > 0.0f) {
    if (flSpeed != 0) {
      ...
      // This computation may happen to cancel itself out exactly. If so, slam
      // to targetdir.
  ...
  if (m_bCreateDangerSounds == true) {
    trace_t tr;
    UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + GetAbsVelocity() * 0.5,
                   MASK_SOLID, this, COLLISION_GROUP_NONE, &tr);

    CSoundEnt::InsertSound(SOUND_DANGER, tr.endpos, 100, 0.2, this,
...
void CMissile::AddCustomDetonator(CBaseEntity *pEntity, float radius,
                                  float height) {
...
CBaseEntity *CInfoAPCMissileHint::FindAimTarget(
  ...
  for (int i = s_APCMissileHints.Count(); --i >= 0;) {
    ...
    // target If we can reach the target within the time it takes for the enemy
    // to reach the
...
CAPCMissile *GetAPCMissileList() { return g_APCMissileList.m_pClassList; }
...
DEFINE_FIELD(m_flReachedTargetTime, FIELD_TIME),
    ...
    DEFINE_THINKFUNC(ExplodeThink), DEFINE_THINKFUNC(APCSeekThink),
...
void CAPCMissile::APCSeekThink(void) {
  ...
  // If we can't find a dot to follow around then just send me wherever I'm
  // facing so I can blow up in peace.
  for (CLaserDot *pEnt = GetLaserDotList(); pEnt != NULL;
       pEnt = pEnt->m_pNext) {
    if (!pEnt->IsOn())
      continue;
...
#ifdef HL2_EPISODIC
    ExplosionCreate(GetAbsOrigin(), GetAbsAngles(), this, APC_MISSILE_DAMAGE,
                    100, true, 20000);
#else
    ...
    if (m_flReachedTargetTime != 0.0f) {
      *pHomingSpeed = APC_HOMING_SPEED;
      float flDeltaTime = clamp(gpGlobals->curtime - m_flReachedTargetTime,
                                0.0f, CORRECTION_TIME);
      *pHomingSpeed = SimpleSplineRemapVal(flDeltaTime, 0.0f, CORRECTION_TIME,
                                           0.2f, *pHomingSpeed);
      flMinHomingDistance =
          SimpleSplineRemapVal(flDeltaTime, 0.0f, CORRECTION_TIME,
                               MIN_NEAR_HOMING_DISTANCE, flMinHomingDistance);
      flMaxHomingDistance =
          SimpleSplineRemapVal(flDeltaTime, 0.0f, CORRECTION_TIME,
                               MAX_NEAR_HOMING_DISTANCE, flMaxHomingDistance);
    } else {
      ...
      if (vecDelta.z > MIN_HEIGHT_DIFFERENCE) {
        float flClampedHeight =
            clamp(vecDelta.z, MIN_HEIGHT_DIFFERENCE, MAX_HEIGHT_DIFFERENCE);
        ...
        if (flDist > flForwardOffset) {
          ...
          VectorLerp(*pActualDotPosition, vecNewPosition, flHeightAdjustFactor,
                     *pActualDotPosition);
        }
      } else {
...
//			*pOperator -
//-----------------------------------------------------------------------------
void CWeaponRPG::Operator_HandleAnimEvent(animevent_t *pEvent,
  ...
  case EVENT_WEAPON_SMG1: {
    ...
    if (GetAttachment("missile", altLaunchPoint)) {
      ...
      trace_t tr;
      AI_TraceHull(
          altLaunchPoint, altLaunchPoint + vecShootDir * (10.0f * 12.0f),
          Vector(-24, -24, -24), Vector(24, 24, 24), MASK_NPCSOLID, NULL, &tr);
  ...
  if (hl2_episodic.GetBool()) {
    ...
    for (int i = 0; i < nAIs; i++) {
      if (ppAIs[i]->m_iClassname == iszStriderClassname) {
        ppAIs[i]->DispatchInteraction(g_interactionPlayerLaunchedRPG, NULL,
                                      m_hMissile);
      }
    }
  }
...
void CWeaponRPG::ItemPostFrame(void) {
  ...
  // Adrian: Players are not allowed to remove the laser guide in single player
  // anymore, bye!
...
void CWeaponRPG::SetNPCLaserPosition(const Vector &vecTarget) {
  ...
  // NDebugOverlay::Box( m_vecNPCLaserDot, -Vector(10,10,10), Vector(10,10,10),
  // 255,0,0, 8, 3 );
}
...
void CWeaponRPG::UpdateLaserPosition(Vector vecMuzzlePos, Vector vecEndPos) {
  ...
  UTIL_Portal_TraceRay(rayLaser, (MASK_SHOT & ~CONTENTS_WINDOW), this,
                       COLLISION_GROUP_NONE, &tr);
  g_bBulletPortalTrace = false;
#else
  UTIL_TraceLine(vecMuzzlePos, vecEndPos, (MASK_SHOT & ~CONTENTS_WINDOW), this,
                 COLLISION_GROUP_NONE, &tr);
#endif
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CWeaponRPG::WeaponLOSCondition(const Vector &ownerPos,
                                    const Vector &targetPos,
                                    bool bSetConditions) {
  bool bResult =
      BaseClass::WeaponLOSCondition(ownerPos, targetPos, bSetConditions);
  ...
  if (bResult) {
    ...
    if (npcOwner) {
      ...
      // Make sure I have a good 10 feet of wide clearance in front, or I'll
      // blow my teeth out.
      AI_TraceHull(vecMuzzle, vecMuzzle + vecShootDir * (10.0f * 12.0f),
                   Vector(-24, -24, -24), Vector(24, 24, 24), MASK_NPCSOLID,
                   NULL, &tr);
...
int CWeaponRPG::WeaponRangeAttack1Condition(float flDot, float flDist) {
  ...
  if (pOwner) {
    ...
    // Make sure I have a good 10 feet of wide clearance in front, or I'll blow
    // my teeth out.
    AI_TraceHull(vecMuzzle, vecMuzzle + vecShootDir * (10.0f * 12.0f),
                 Vector(-24, -24, -24), Vector(24, 24, 24), MASK_NPCSOLID, NULL,
                 &tr);
...
void CWeaponRPG::StartLaserEffects(void) {
  ...
  CBaseViewModel *pBeamEnt =
      static_cast<CBaseViewModel *>(pOwner->GetViewModel());
  ...
  if (m_hLaserMuzzleSprite == NULL) {
    m_hLaserMuzzleSprite =
        CSprite::SpriteCreate(RPG_LASER_SPRITE, GetAbsOrigin(), false);
    ...
    m_hLaserMuzzleSprite->SetAttachment(pOwner->GetViewModel(),
                                        LookupAttachment("laser"));
    m_hLaserMuzzleSprite->SetTransparency(kRenderTransAdd, 255, 255, 255, 255,
...
CLaserDot *CLaserDot::Create(const Vector &origin, CBaseEntity *pOwner,
                             bool bVisibleDot) {
  CLaserDot *pLaserDot =
      (CLaserDot *)CBaseEntity::Create("env_laserdot", origin, QAngle(0, 0, 0));
  ...
  pLaserDot->SetTransparency(kRenderGlow, 255, 255, 255, 255,
                             kRenderFxNoDissipation);
  pLaserDot->SetScale(0.5f);

  pLaserDot->SetOwnerEntity(pOwner);

  pLaserDot->SetContextThink(&CLaserDot::LaserThink, gpGlobals->curtime + 0.1f,
                             g_pLaserDotThink);
