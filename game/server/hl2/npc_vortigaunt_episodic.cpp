...
ConVar
    sk_vortigaunt_armor_charge_per_token("sk_vortigaunt_armor_charge_per_token",
                                         "5");
...
CNPC_Vortigaunt::CNPC_Vortigaunt(void)
    : m_bPlayerRequestedHeal(false),
      ...
      m_nNumTokensToSpawn(0), m_flAimDelay(0.0f),
...
void CNPC_Vortigaunt::RunTask(const Task_t *pTask) {
  ...
  case TASK_RANGE_ATTACK1: {
    // If our enemy is gone, dead or out of sight, pick a new one (only if we're
    // not delaying this behavior)
...
//			long swing arm, this causes stability issues when an npc
//is trying to 			face a vort that's also turning their head
// Output :
//-----------------------------------------------------------------------------
...
// but
//			the vort's eyes are so far offset, that this is usually
...
int CNPC_Vortigaunt::TranslateSchedule(int scheduleType) {
  ...
  case SCHED_RANGE_ATTACK1:
    ...
    // If we're told to fire when we're already firing, just face our target. If
    // we don't do this, we get a bizarre double-shot
    if (IsPlayingGesture(ACT_GESTURE_RANGE_ATTACK1))
      return SCHED_COMBAT_FACE;
...
DEFINE_FIELD(m_hTarget, FIELD_EHANDLE), DEFINE_FIELD(m_flLifetime, FIELD_TIME),
    ...
    END_DATADESC()
        ...
        IMPLEMENT_SERVERCLASS_ST(CVortigauntChargeToken,
...
DEFINE_FIELD(m_bFadeOut, FIELD_BOOLEAN),
    END_DATADESC()
        ...
        IMPLEMENT_SERVERCLASS_ST(CVortigauntEffectDispel,
