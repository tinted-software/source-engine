...
enum TacticalVariant_T {
  ...
  TACTICAL_VARIANT_PRESSURE_ENEMY_UNTIL_CLOSE, // Act like
                                               // VARIANT_PRESSURE_ENEMY, but go
                                               // to VARIANT_DEFAULT once within
                                               // 30 feet
};
...
DEFINE_FIELD(m_nKickDamage, FIELD_INTEGER),
    ...
    //							m_AssaultBehavior (auto
    //saved by AI) 							m_StandoffBehavior (auto saved by AI) 							m_FollowBehavior (auto
    //saved by AI) 							m_FuncTankBehavior (auto saved by AI) 							m_RappelBehavior (auto
    //saved by AI) 							m_ActBusyBehavior (auto saved by AI)
...
void CNPC_Combine::InputThrowGrenadeAtTarget(inputdata_t &inputdata) {
  ...
  if (!pEntity) {
    DevMsg("%s (%s) received ThrowGrenadeAtTarget input, but couldn't find "
           "target entity '%s'\n",
...
void CNPC_Combine::Spawn(void) {
  ...
  m_flFieldOfView = -0.2; // indicates the width of this NPC's forward view cone
...
void CNPC_Combine::StartTask(const Task_t *pTask) {
  ...
  case TASK_COMBINE_DEFER_SQUAD_GRENADES: {
    if (m_pSquad) {
      // iterate my squad and stop everyone from throwing grenades for a little
      // while.
      AISquadIter_t iter;
  ...
  case NPC_STATE_ALERT: {
    ...
    if (HasCondition(COND_HEAR_COMBAT)) {
      ...
      if (pSound && pSound->IsSoundType(SOUND_COMBAT)) {
        if (m_pSquad &&
            m_pSquad->GetSquadMemberNearestTo(pSound->GetSoundReactOrigin()) ==
                this &&
...
int CNPC_Combine::TranslateSchedule(int scheduleType) {
  ...
  case SCHED_COMBINE_ASSAULT: {
    ...
    // FIXME: this should be generalized by the schedules that are selected, or
    // in the definition of what "cover" means (i.e., trace attack vulnerability
    // vs. physical attack vulnerability
  ...
  case SCHED_ESTABLISH_LINE_OF_FIRE: {
    ...
    if (CanAltFireEnemy(true) &&
      ...
      // If an elite in the squad could fire a combine ball at the player's last
      // known position, do so!
      return SCHED_COMBINE_AR2_ALTFIRE;
    }
  ...
  case SCHED_HIDE_AND_RELOAD: {
    ...
    if (CanGrenadeEnemy() && OccupyStrategySlot(SQUAD_SLOT_GRENADE1) &&
      ...
      // If I COULD throw a grenade and I need to reload, 20% chance I'll throw
      // a grenade before I hide to reload.
...
#define MIN_SIGNAL_DIST 256
    if (GetEnemy() != NULL && GetEnemy()->IsPlayer() && m_bFirstEncounter) {
      ...
      if (flDistToEnemy >= MIN_SIGNAL_DIST) {
        m_bFirstEncounter = false; // after first encounter, leader won't issue
  ...
  if (pEvent->type & AE_TYPE_NEWEVENTSYSTEM) {
    ...
    case COMBINE_AE_GREN_TOSS: {
      ...
      if (m_NPCState == NPC_STATE_SCRIPT) {
        ...
        // Grenades thrown from a script do not count against grenades remaining
        // for the AI to use.
    ...
    case COMBINE_AE_GREN_LAUNCH: {
      ...
      if (g_pGameRules->IsSkillLevel(SKILL_HARD))
        ...
        m_flNextGrenadeCheck =
            gpGlobals->curtime + 6; // wait six seconds before even looking
    ...
    case COMBINE_AE_KICK: {
      // Does no damage, because damage is applied based upon whether the target
...
//attacks 			is expensive we don't want to do it every frame.  Return true 			if we
//meet minimum set of requirements and then test for actual 			throw later if we
//actually decide to do a grenade attack.
// Input  :
// Output :
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
WeaponProficiency_t
CNPC_Combine::CalcWeaponProficiency(CBaseCombatWeapon *pWeapon) {
...
DEFINE_SCHEDULE(
    ...
    //								comibine will
    //fire where player was after 								he has moved for a little while.  Good
    //effect!!
