...
//-----------------------------------------------------------------------------
HeadcrabRelease_t
CNPC_BaseZombie::ShouldReleaseHeadcrab(const CTakeDamageInfo &info,
                                       float flDamageThreshold) {
  if (m_iHealth <= 0) {
    if (info.GetDamageType() & DMG_REMOVENORAGDOLL)
      return RELEASE_NO;
...
void CNPC_BaseZombie::HandleAnimEvent(animevent_t *pEvent) {
  ...
  if (pEvent->event == AE_ZOMBIE_ATTACK_RIGHT) {
    ...
    ClawAttack(GetClawAttackRange(), sk_zombie_dmg_one_slash.GetFloat(), qa,
               vec, ZOMBIE_BLOOD_RIGHT_HAND);
    return;
  }
  ...
  if (pEvent->event == AE_ZOMBIE_POPHEADCRAB) {
    ...
    if (boneIndex == -1) {
      Warning("AE_ZOMBIE_POPHEADCRAB event using invalid bone name! Usage: "
              "event AE_ZOMBIE_POPHEADCRAB \"<BoneName> <Speed>\" \n");
      return;
    }
    ...
    if (!token) {
      Warning("AE_ZOMBIE_POPHEADCRAB event format missing velocity parameter! "
