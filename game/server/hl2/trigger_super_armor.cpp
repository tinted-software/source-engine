...
void CTriggerSuperArmor::StartLoopingSounds(CBaseEntity *pEntity) {
  ...
  m_pChargingSound = controller.SoundCreate(filter, pEntity->entindex(),
...
void CTriggerSuperArmor::StartTouch(CBaseEntity *pOther) {
  ...
  if (m_hTouchingEntities.Count() == 1) {
    SetContextThink(RechargeThink, gpGlobals->curtime + 0.01f,
                    s_pRechargeThinkContext);
...
void CTriggerSuperArmor::RechargeThink() {
  ...
  for (int i = m_hTouchingEntities.Count(); --i >= 0;) {
    CBasePlayer *pPlayer =
        assert_cast<CBasePlayer *>(m_hTouchingEntities[i].Get());

    if ((pPlayer->ArmorValue() < MAX_SUPER_ARMOR) ||
        (pPlayer->GetHealth() < 100)) {
  ...
  SetContextThink(RechargeThink, gpGlobals->curtime + 0.1f,
                  s_pRechargeThinkContext);
}