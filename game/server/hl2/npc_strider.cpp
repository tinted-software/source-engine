...
ConVar
    strider_always_use_procedural_height("strider_always_use_procedural_height",
                                         "0");
...
ConVar
    strider_free_pass_after_escorts_dead("strider_free_pass_after_escorts_dead",
                                         "2.5");
...
#define STRIDER_SHOOT_ON_TARGET_TIME                                           \
  0.5 // How much of DEFAULT_SHOOT_DURATION is spent on-target (vs. stitching up
      // to a target)
...
void CNPC_Strider::Activate() {
  ...
  if (m_BodyTargetBone == -1) {
    DevMsg("Couldn't find npc_strider bone %s, which is used as target for "
           "others\n",
           pszBodyTargetBone);
  }
  ...
  if (AIGetNumFollowers(this, m_iszHunterClassname) == 0) {
    ...
    if (m_PlayerFreePass.HasPass() ||
        (!m_pMinigun->IsShooting() ||
         GetEnemy() !=
             m_PlayerFreePass.GetPassTarget())) // no free pass when in midst of
                                                // shooting at target
...
void CNPC_Strider::StartTask(const Task_t *pTask) {
  ...
  case TASK_STRIDER_STOMP: {
    m_nextStompTime = gpGlobals->curtime + 5;
    Activity stompAct = (Activity)(pTask->flTaskData > 0 ? ACT_STRIDER_STOMPR
                                                         : ACT_STRIDER_STOMPL);
  ...
  case TASK_STRIDER_REFRESH_HUNT_PATH:
    ...
    if (GetGoalEnt()) {
      ...
      if ((m_debugOverlays & OVERLAY_NPC_SELECTED_BIT) &&
        ...
        NDebugOverlay::Line(GetAbsOrigin() + Vector(0, 0, 4), goal.dest, 255, 0,
                            255, true, 3);
      }
  ...
  case TASK_STRIDER_GET_PATH_TO_CANNON_LOS: {
    ...
    if (!found && GetTacticalServices()->FindLos(vecEnemy, vecEnemy, flMinRange,
    ...
    if (found) {
      ...
      AddFacingTarget(GetCannonTarget(), GetCannonTarget()->WorldSpaceCenter(),
                      5.0, 5.0);
    } else {
...
/*
                IPhysicsObject *pPhysics0 = FindPhysicsObjectByName(
   STRING(m_nameAttach1), this ); IPhysicsObject *pPhysics1 =
   FindPhysicsObjectByName( STRING(m_nameAttach2), this );
                ...
                if ( pPhysics0 != pPhysics1 )
                {
                        ...
                        {
                                PhysDisableEntityCollisions( pPhysics0,
   pPhysics1 );
                        }
                }
                else
...
{
        IPhysicsObject *pIgnorePhys  = FindPhysicsObjectByName(
inputdata.value.String(), this ); if ( !pIgnorePhys ) return;

        int idx;
        CBoneFollower *pFol;

        for (idx = 0 ; pFol = GetBoneFollowerByIndex(idx) ; ++idx) // stop when
the function starts returning null (idx is no longer good)
        {
...
void CNPC_Strider::InputDisableCollisionWith(inputdata_t &inputdata) {
  ...
  for (int idx = m_BoneFollowerManager.GetNumBoneFollowers() - 1; idx >= 0;
       --idx) // stop when the function starts returning null (idx is no longer
              // good)
  {
...
void CNPC_Strider::ShootMinigun(const Vector *pTarget, float aimError,
  ...
  if (pTarget) {
    ...
    // g_pEffects->MuzzleFlash( muzzlePos, muzzleAng,
    // random->RandomFloat( 2.0f, 4.0f ) , MUZZLEFLASH_TYPE_STRIDER );
    DoMuzzleFlash();
...
void CNPC_Strider::CannonHitThink() {
  ...
  if (pCannonTarget) {
    ...
    if (fAlive && pCannonTarget->IsAlive()) {
      DevWarning("* * * * * * * * * * * * * * *\n");
      DevWarning("NASTYGRAM: STRIDER failed to kill its cannon target. Killing "
                 "directly...\n");
      DevWarning("* * * * * * * * * * * * * * *\n");
