...
DEFINE_FIELD(m_flAltitude, FIELD_FLOAT),
    ...
    DEFINE_OUTPUT(m_OnGrab, "OnGrab"), DEFINE_OUTPUT(m_OnRelease, "OnRelease"),
#endif
...
#ifndef _XBOX
  UTIL_BloodSpray(m_vecBloodPos + jitterPos, Vector(0, 0, -1), m_nBloodColor,
                  RandomInt(4, 8),
                  RandomInt(0, 2) == 0 ? FX_BLOODSPRAY_ALL
                                       : FX_BLOODSPRAY_CLOUD);
#else
  UTIL_BloodSpray(m_vecBloodPos + jitterPos, Vector(0, 0, -1),
                  BLOOD_COLOR_YELLOW, RandomInt(4, 8),
                  RandomInt(0, 2) == 0 ? FX_BLOODSPRAY_ALL
                                       : FX_BLOODSPRAY_CLOUD);
#endif
}
...
void CNPC_Barnacle::SwallowPrey(void) {
  ...
  if (flDistanceToGo <= 0) {
    ...
    // looks wierd if it's instant) Parentheses were probably intended around
    // the ?: part of the expression, but putting them there now would change
    // the behavior which is undesirable, so parentheses were placed around the
    // '+' to suppress compiler warnings.
    m_flDigestFinish =
        (gpGlobals->curtime + m_bSwallowingPoison) ? 0.48f : 10.0f;
#else
    m_flDigestFinish = gpGlobals->curtime + 10.0;
#endif
  }
...
void CNPC_Barnacle::UpdateTongue(void) {
  ...
  float flRestStretch = (BARNACLE_TONGUE_TIP_MASS * GetCurrentGravity()) /
                        BARNACLE_TONGUE_SPRING_CONSTANT_HANGING;
...
IterationRetval_t
CTongueEntitiesEnum::EnumElement(IHandleEntity *pHandleEntity) {
...
//-----------------------------------------------------------------------------
CBarnacleTongueTip *
CBarnacleTongueTip::CreateTongueRoot(const Vector &vecOrigin,
