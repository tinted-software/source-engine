...
void CNPC_CombineGunship::PlayAngryLoop(void) {
  ...
  CSoundEnvelopeController &controller =
  CSoundEnvelopeController::GetController(); controller.SoundChangeVolume(
  m_pPatrolSound, 0.0, 1.0 ); controller.SoundChangeVolume(
  m_pAngrySound, 1.0, 1.0 );
  */
}
...
//through the 			cover that the enemy is hiding behind. (Such as the Depot
//lighthouse)
//-----------------------------------------------------------------------------
...
void CNPC_CombineGunship::BeginDestruct(void) {
  ...
  m_hRagdoll->SetName(
      AllocPooledString(UTIL_VarArgs("%s_ragdoll", STRING(GetEntityName()))));
...
void CNPC_CombineGunship::Flight(void) {
  ...
  float speed =
      m_flForce *
      DotProduct(vecVelDir, vecAccelDir); // * DotProduct( forward, vecVelDir );
...
void CNPC_CombineGunship::UpdateRotorSoundPitch(int iPitch) {
  ...
  if (pPlayer) {
    ...
    float flVolume =
        clamp(RemapVal(flDistance, (900 * 900), (1800 * 1800), 1, 0), 0, 1);
    ...
    flVolume =
        clamp(RemapVal(flDistance, (600 * 600), (700 * 700), 1, 0), 0, 1);
...
//the 'stitching' 			 behavior that's so popular with the kids these days (sjb)
//
...
int CNPC_CombineGunship::OnTakeDamage_Alive(const CTakeDamageInfo &inputInfo) {
  ...
  if (m_bInvulnerable == false) {
    ...
    case 1:
      // If we're on Easy, we're half dead now, so fire the rest of our outputs
      // too This is done in case the mapmaker's connected those inputs to
      // something important that has to happen before the gunship dies.
      if (g_pGameRules->IsSkillLevel(SKILL_EASY)) {
