...
protected:
  // FIXME: The m_strSpawnGroup is redundant to the m_iszDestinationGroup in the
  // base class NPC template maker
  string_t m_strSpawnGroup; // if present, spawn children on the nearest node of
                            // this group (to the player)
  string_t m_strSpawnTarget; // name of target to spawn near
  float m_flSpawnRadius;     // radius around target to attempt to spawn in
  float m_flWorkerSpawnRate; // Percentage chance of spawning a worker when we
                             // spawn an antlion [0..1].
  ...
  EHANDLE m_hProxyTarget; // This is a self-held target that is created and used
                          // when a vector is passed in as a fight goal, instead
                          // of an entity
