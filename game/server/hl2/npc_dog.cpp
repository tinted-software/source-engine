...
DEFINE_EMBEDDED(m_BoneFollowerManager),
    ...
    DEFINE_INPUTFUNC(FIELD_STRING, "StartCatchThrowBehavior",
                     InputStartCatchThrowBehavior),
    DEFINE_INPUTFUNC(FIELD_STRING, "StopCatchThrowBehavior",
    ...
    DEFINE_INPUTFUNC(FIELD_FLOAT, "SetThrowArcModifier",
                     InputSetThrowArcModifier),
    DEFINE_INPUTFUNC(FIELD_STRING, "SetThrowTarget", InputSetThrowTarget),
    DEFINE_INPUTFUNC(FIELD_VOID, "TurnBoneFollowersOff",
                     InputTurnBoneFollowersOff),
    DEFINE_INPUTFUNC(FIELD_VOID, "TurnBoneFollowersOn",
...
// relationship table.
//-----------------------------------------------------------------------------
Class_T CNPC_Dog::Classify(void) { return CLASS_PLAYER_ALLY_VITAL; }
...
bool CNPC_Dog::CreateVPhysics(void) {
  ...
  if (m_bBoneFollowersActive == true &&
      !m_BoneFollowerManager.GetNumBoneFollowers()) {
...
//-----------------------------------------------------------------------------
// This function checks if Dog's collision group doesn't match his bone
// follower's and fixes them up.
//-----------------------------------------------------------------------------
void CNPC_Dog::MantainBoneFollowerCollisionGroups(int iCollisionGroup) {
  ...
  if (pBone && pBone->hFollower &&
      pBone->hFollower->GetCollisionGroup() != iCollisionGroup) {
  ...
  if (m_bPerformAvoidance || (ShouldPlayerAvoid() && bIsMoving)) {
    ...
    if (pLocalPlayer) {
      ...
      for (i = 0; i < m_BoneFollowerManager.GetNumBoneFollowers(); i++) {
        pBone = m_BoneFollowerManager.GetBoneFollower(i);
        if (pBone && pBone->hFollower) {
          pBone->hFollower->CollisionProp()->WorldSpaceSurroundingBounds(
              &vMins, &vMaxs);
      ...
      bIntersectingNPCBox = IsBoxIntersectingBox(GetAbsOrigin() + vWorldMins,
                                                 GetAbsOrigin() + vWorldMaxs,
                                                 vPlayerMins, vPlayerMaxs);

      if (ai_debug_avoidancebounds.GetBool()) {
        int iRed = (bIntersectingNPCBox == true) ? 255 : 0;

        NDebugOverlay::Box(GetAbsOrigin(), vWorldMins, vWorldMaxs, iRed, 0, 255,
                           64, 0.1);
        ...
        for (i = 0; i < m_BoneFollowerManager.GetNumBoneFollowers(); i++) {
          ...
          if (pBone && pBone->hFollower) {
            pBone->hFollower->CollisionProp()->WorldSpaceSurroundingBounds(
                &vMins, &vMaxs);
            iRed =
                (IsBoxIntersectingBox(vMins, vMaxs, vPlayerMins, vPlayerMaxs))
                    ? 255
                    : 0;

            NDebugOverlay::Box(vec3_origin, vMins, vMaxs, iRed, 0, 255, 64,
                               0.1);
          }
        }
      }
...
void CNPC_Dog::Spawn(void) {
  ...
  CapabilitiesAdd(bits_CAP_MOVE_GROUND | bits_CAP_OPEN_DOORS |
                  bits_CAP_TURN_HEAD | bits_CAP_ANIMATEDFACE);
  ...
  if (m_hPhysicsEnt) {
    if (m_bHasObject == true) {
      ...
      if (pPhysObj) {
        ...
        Warning("CleanCatchAndThrow:   m_hPhysicsEnt->VPhysicsGetObject == "
                "NULL!\n");
      }
  ...
  if (m_hPhysicsEnt) {
    ...
    if (pPhysObj) {
      ...
      AngularImpulse angVelocity =
          RandomAngularImpulse(-250, -250) / pPhysObj->GetMass();
      ...
      if (pPhysObj->GetShadowController()) {
        ...
        pPhysObj->RemoveShadowController();
        pPhysObj->SetPosition(m_hPhysicsEnt->GetLocalOrigin(),
                              m_hPhysicsEnt->GetLocalAngles(), true);
      ...
      if (m_hThrowTarget) {
        ...
        if (m_hThrowTarget->IsPlayer())
          vThrowOrigin =
              vThrowOrigin + Vector(random->RandomFloat(-128, 128),
                                    random->RandomFloat(-128, 128), 0);

        Vector vecToss = VecCheckToss(this, vGunPos, vThrowOrigin,
                                      m_flThrowArcModifier, 1.0f, true);

        if (vecToss == vec3_origin) {
          // Fix up an impossible throw so dog will at least toss the box in the
          // target's general direction instead of dropping it. Also toss it up
          // in the air so it will fall down and break. (Just throw the box up
        ...
        vThrowDirection =
            vThrowDirection + (unitVel * (flDrag * flDrag)) / flTest;
      ...
      if (m_bBeamEffects == true) {
        ...
        te->GaussExplosion(filter, 0.0f,
                           m_hPhysicsEnt->WorldSpaceCenter() - (shotDir * 4.0f),
...
void CNPC_Dog::HandleAnimEvent(animevent_t *pEvent) {
  ...
  if (pEvent->event == AE_DOG_PICKUP || pEvent->event == AE_DOG_CATCH ||
      pEvent->event == AE_DOG_PICKUP_NOEFFECT) {
...
void CNPC_Dog::CreateSprites(void) {
  ...
  for (int i = 0; i < EFFECT_COUNT; i++) {
    ...
    m_hGlowSprites[i] =
        CSprite::SpriteCreate("sprites/glow04_noz.vmt", GetAbsOrigin(), false);

    m_hGlowSprites[i]->SetAttachment(this, LookupAttachment(attachNames[i]));
    m_hGlowSprites[i]->SetTransparency(kRenderGlow, 255, 128, 0, 64,
...
bool CNPC_Dog::FindPhysicsObject(const char *pPickupName,
                                 CBaseEntity *pIgnore) {
  ...
  while ((pEnt = gEntList.FindEntityByClassname(pEnt, "prop_physics")) !=
         NULL) {
  ...
  if (dog_debug.GetBool() == true) {
    if (pNearest)
      NDebugOverlay::Box(
          pNearest->WorldSpaceCenter(), pNearest->CollisionProp()->OBBMins(),
...
void CNPC_Dog::RunTask(const Task_t *pTask) {
  ...
  case TASK_DOG_GET_PATH_TO_PHYSOBJ: {
    ...
    vecGoalPos =
        m_hPhysicsEnt->WorldSpaceCenter() + (vecDir * DOG_PHYSOBJ_MOVE_TO_DIST);
    ...
    bBuiltRoute = GetNavigator()->SetGoal(AI_NavGoal_t(vecGoalPos, aActivity),
                                          AIN_NO_PATH_TASK_FAIL);
  ...
  case TASK_WAIT_FOR_MOVEMENT: {
    ...
    if (m_hPhysicsEnt != NULL) {
      ...
      if (!pPhysObj) {
        Warning("npc_dog TASK_WAIT_FOR_MOVEMENT with NULL "
                "m_hPhysicsEnt->VPhysicsGetObject\n");
      }
      ...
      if (pPhysObj && pPhysObj->IsAsleep() == false &&
          GetNavigator()->IsGoalActive() == false) {
        Vector vecGoalPos;
        Vector vecDir;
        ...
        vecGoalPos = m_hPhysicsEnt->WorldSpaceCenter() +
                     (vecDir * DOG_PHYSOBJ_MOVE_TO_DIST);
        ...
        GetNavigator()->SetGoal(AI_NavGoal_t(vecGoalPos, aActivity),
                                AIN_NO_PATH_TASK_FAIL);
  ...
  case TASK_DOG_WAIT_FOR_OBJECT: {
    if (m_hPhysicsEnt != NULL) {
      ...
      if (pPlayer) {
        flDistanceToPlayer =
            (pPlayer->GetAbsOrigin() - m_hPhysicsEnt->WorldSpaceCenter())
                .Length();
      }

      IPhysicsObject *pPhysObj = m_hPhysicsEnt->VPhysicsGetObject();
      if (!pPhysObj) {
        Warning("npc_dog:  TASK_DOG_WAIT_FOR_OBJECT with "
                "m_hPhysicsEnt->VPhysicsGetObject == NULL\n");
      }
      ...
      if (pPhysObj && !(pPhysObj->GetGameFlags() & FVPHYSICS_PLAYER_HELD) &&
        ...
        if (m_flTimeToPull <= gpGlobals->curtime) {
          ...
          if (flDistance >= DOG_PULL_DISTANCE &&
              flDistance <= (DOG_PULL_DISTANCE * 2) && flDot > -0.3) {
            if (pPhysObj->IsAsleep() == false &&
                !(pPhysObj->GetGameFlags() & FVPHYSICS_PLAYER_HELD)) {
              Vector vecGoalPos;
              Vector vecDir;
...
void CNPC_Dog::StartTask(const Task_t *pTask) {
  ...
  case TASK_DOG_GET_PATH_TO_PHYSOBJ: {
    ...
    if (GetNavigator()->SetGoal(AI_NavGoal_t(vecGoalPos, aActivity),
                                AIN_NO_PATH_TASK_FAIL) == false) {
      if (m_hUnreachableObjects.Find(m_hPhysicsEnt) == -1)
        m_hUnreachableObjects.AddToTail(m_hPhysicsEnt);
