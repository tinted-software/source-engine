...
enum { NUGGET_NONE, NUGGET_SMALL = 1, NUGGET_MEDIUM, NUGGET_LARGE };
...
protected:
  ...
  inline bool ProbeSurface(const Vector &vecTestPos, const Vector &vecDir,
                           Vector *vecResult, Vector *vecNormal);
...
DEFINE_FIELD(m_hGlowSprite, FIELD_EHANDLE),
    ...
    // Functions
    DEFINE_ENTITYFUNC(GrubTouch), DEFINE_ENTITYFUNC(IdleThink),
...
void CAntlionGrub::CreateGlow(void) {
  ...
  m_hGlowSprite->SetTransparency(kRenderWorldGlow, 156, 169, 121, 164,
...
int CAntlionGrub::GetNuggetDenomination(void) {
  ...
  // The larger to discrepancy, the higher the chance to move quickly to close
  // it
  ...
  // Msg("Player: %.02f, Desired: %.02f, Seed: %.02f, Perc: %.02f, Result:
  // %d\n", flPlayerHealthPerc, flDesiredHealthPerc, flSeed, flRandomPerc,
  // nDenomination );
...
void CAntlionGrub::CreateNugget(void) {
  ...
  if (pPhys) {
    ...
    Vector vecVelocity =
        RandomVector(-35.0f, 35.0f) + (vecForward * -RandomFloat(50.0f, 75.0f));
...
void CAntlionGrub::Event_Killed(const CTakeDamageInfo &info) {
  ...
  bool bSquashed =
      (info.GetDamageType() & (DMG_CRUSH | DMG_CLUB)) ? true : false;
...
void CAntlionGrub::SetNextThinkByDistance(void) {
  ...
  float flDistToPlayerSqr =
      (GetAbsOrigin() - pPlayer->GetAbsOrigin()).LengthSqr();
  float scale =
      RemapValClamped(flDistToPlayerSqr, Square(400), Square(5000), 1.0f, 5.0f);
...
void CAntlionGrub::Spawn(void) {
  ...
  RotateAABB(EntityToWorldTransform(), CollisionProp()->OBBMins(),
             CollisionProp()->OBBMaxs(), vecMins, vecMaxs);
...
// Output : inline bool
//-----------------------------------------------------------------------------
inline bool CAntlionGrub::ProbeSurface(const Vector &vecTestPos,
                                       const Vector &vecDir, Vector *vecResult,
                                       Vector *vecNormal) {
  // Trace down to find a surface
  trace_t tr;
  UTIL_TraceLine(vecTestPos, vecTestPos + (vecDir * 256.0f),
                 MASK_NPCSOLID & (~CONTENTS_MONSTER), this,
...
void CAntlionGrub::AttachToSurface(void) {
  ...
  if (ProbeSurface(WorldSpaceCenter() + vecOffset, vecDown, &vecMid,
                   &vecMidNormal) == false) {
    // A grub was left hanging in the air, it must not be near any valid
    // surfaces!
    Warning(
  ...
  // First test our tail (more crucial that it doesn't interpenetrate with the
  // world)
  if (ProbeSurface(WorldSpaceCenter() - (vecForward * 12.0f) + vecOffset,
                   vecDown, &vecPivot, &vecPivotNormal) == false) {
...
void CAntlionGrub::MakeIdleSounds(void) {
  if (m_State == GRUB_STATE_AGITATED) {
    if (m_flNextSquealSoundTime < gpGlobals->curtime) {
      ...
      m_flNextSquealSoundTime =
          gpGlobals->curtime + random->RandomFloat(1.5f, 3.0f);
      m_flNextIdleSoundTime =
          gpGlobals->curtime + random->RandomFloat(4.0f, 8.0f);
    }
  } else {
...
void CAntlionGrub::IdleThink(void) {
  ...
  if (m_bOutsidePVS) {
    m_flNextIdleSoundTime =
        gpGlobals->curtime + random->RandomFloat(1.0f, 4.0f);
    m_bOutsidePVS = false;
  }
  ...
  float flDistToPlayerSqr =
      (GetAbsOrigin() - pPlayerEnt->GetAbsOrigin()).LengthSqr();
...
void CAntlionGrub::GrubTouch(CBaseEntity *pOther) {
  ...
  IPhysicsObject *pPhysOther =
      pOther->VPhysicsGetObject(); // bool bThrown = (
                                   // pTarget->VPhysicsGetObject()->GetGameFlags()
...
void CAntlionGrub::SpawnSquashedGrub(void) {
  ...
  CBaseEntity *pGib = CreateRagGib(ANTLIONGRUB_SQUASHED_MODEL, GetAbsOrigin(),
                                   GetAbsAngles(), vecUp * 16.0f);
  if (pGib) {
...
void CAntlionGrub::MakeSquashDecals(const Vector &vecOrigin) {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    UTIL_TraceLine(vecStart, vecStart + (vecTraceDir * (5 * 12)),
...
// Purpose:
//-----------------------------------------------------------------------------
void CAntlionGrub::Squash(CBaseEntity *pOther, bool bDealDamage,
                          bool bSpawnBlood) {
  ...
  if (bDealDamage) {
    CTakeDamageInfo info(pOther, pOther, Vector(0, 0, -1), GetAbsOrigin(),
                         GetHealth() + 1, DMG_CRUSH);
    TakeDamage(info);
  }
}
...
//			*ptr -
//-----------------------------------------------------------------------------
void CAntlionGrub::TraceAttack(const CTakeDamageInfo &info,
...
//-----------------------------------------------------------------------------
bool CGrubNugget::VPhysicsIsFlesh(void) { return false; }
...
void CGrubNugget::Event_Killed(const CTakeDamageInfo &info) {
  AddEffects(EF_NODRAW);
  DispatchParticleEffect("antlion_spit_player", GetAbsOrigin(),
                         QAngle(-90, 0, 0));
