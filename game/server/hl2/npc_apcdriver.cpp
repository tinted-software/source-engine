...
public:
  ...
  virtual bool FVisible(CBaseEntity *pTarget, int traceMask,
...
//------------------------------------------------------------------------------
bool CNPC_APCDriver::WeaponLOSCondition(const Vector &ownerPos,
...
// Is the enemy visible?
//-----------------------------------------------------------------------------
bool CNPC_APCDriver::FVisible(CBaseEntity *pTarget, int traceMask,
                              CBaseEntity **ppBlocker) {
  if (m_hAPC->m_lifeState != LIFE_ALIVE)
    return false;
  ...
  if (pTarget->IsPlayer() || pTarget->Classify() == CLASS_BULLSEYE) {
    if (!bVisible) {
      if ((gpGlobals->curtime - m_flTimeLastSeenEnemy) <=
          NPC_APCDRIVER_REMEMBER_TIME)
        return true;
    }
  }
...
int CNPC_APCDriver::RangeAttack1Conditions(float flDot, float flDist) {
  ...
  if (m_pVehicleInterface->Weapon_PrimaryCanFireAt() >
...
int CNPC_APCDriver::RangeAttack2Conditions(float flDot, float flDist) {
  ...
  // Vehicle not ready to fire again yet?
  if (m_pVehicleInterface->Weapon_SecondaryCanFireAt() >
      gpGlobals->curtime + 0.1f)
    return COND_NONE;
