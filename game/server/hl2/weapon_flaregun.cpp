...
DEFINE_FIELD(m_pOwner, FIELD_CLASSPTR), DEFINE_FIELD(m_nBounces, FIELD_INTEGER),
    ...
    DEFINE_FIELD(m_flNextDamage, FIELD_TIME), DEFINE_SOUNDPATCH(m_pBurnSound),
    ...
    // Function Pointers
    DEFINE_FUNCTION(FlareTouch), DEFINE_FUNCTION(FlareBurnTouch),
...
CFlare *CFlare::GetActiveFlares(void) { return CFlare::activeFlares; }

Class_T CFlare::Classify(void) { return CLASS_FLARE; }
...
void KillFlare(CBaseEntity *pOwnerEntity, CBaseEntity *pEntity,
  ...
  if (pFlare) {
    float flDieTime =
        (pFlare->m_flTimeBurnOut - gpGlobals->curtime) - flKillTime;
...
// Output : CFlare
//-----------------------------------------------------------------------------
CFlare *CFlare::Create(Vector vecOrigin, QAngle vecAngles, CBaseEntity *pOwner,
                       float lifetime) {
...
void CFlare::FlareThink(void) {
  ...
  if (!m_bInActiveList &&
      ((deltaTime > FLARE_BLIND_TIME) || (m_flTimeBurnOut == -1.0f))) {
    AddToActiveFlares();
  }
  ...
  if (m_flTimeBurnOut != -1.0f) {
    ...
    if ((deltaTime <= FLARE_DECAY_TIME) && (m_bFading == false)) {
      m_bFading = true;
      CSoundEnvelopeController::GetController().SoundChangePitch(m_pBurnSound,
                                                                 60, deltaTime);
      CSoundEnvelopeController::GetController().SoundFadeOut(m_pBurnSound,
                                                             deltaTime);
    }
...
void CFlare::FlareBurnTouch(CBaseEntity *pOther) {
  if (pOther && pOther->m_takedamage && (m_flNextDamage < gpGlobals->curtime)) {
    pOther->TakeDamage(
        CTakeDamageInfo(this, m_pOwner, 1, (DMG_BULLET | DMG_BURN)));
...
void CFlare::FlareTouch(CBaseEntity *pOther) {
  ...
  if (pOther && pOther->m_takedamage) {
    /*
            The Flare is the iRifle round right now. No damage, just ignite.
    (sjb)
    ...
    (DMG_BULLET|DMG_BURN) ) ); m_flNextDamage = gpGlobals->curtime + 1.0f;
    */
    ...
    // hit the world, check the material type here, see if the flare should
    // stick.
    trace_t tr;
        ...
        // )
        {
          ...
          if ((tr.plane.normal.z > -0.5f) && (surfDot < -0.9f)) {
            ...
            if (index >= 0) {
              ...
              te->Decal(filter, 0.0, &tr.endpos, &tr.startpos,
                        ENTINDEX(tr.m_pEnt), tr.hitbox, index);
            }
    ...
    if (GetAbsVelocity().LengthSqr() > (250 * 250)) {
      ...
      if (index >= 0) {
        CBroadcastRecipientFilter filter;
        te->Decal(filter, 0.0, &tr.endpos, &tr.startpos, ENTINDEX(tr.m_pEnt),
                  tr.hitbox, index);
      }
    }
    ...
    // After the first bounce, smacking into whoever fired the flare is fair
    // game
    SetOwnerEntity(this);
...
void CFlare::Start(float lifeTime) {
  ...
  if (m_pBurnSound != NULL) {
    ...
    CSoundEnvelopeController::GetController().SoundChangeVolume(m_pBurnSound,
                                                                0.8f, 2.0f);
    CSoundEnvelopeController::GetController().SoundChangePitch(m_pBurnSound,
                                                               100, 2.0f);
  }
...
// Input  : &inputdata -
//-----------------------------------------------------------------------------
void CFlare::InputDie(inputdata_t &inputdata) { Die(inputdata.value.Float()); }
...
void CFlare::RemoveFromActiveFlares(void) {
  ...
  pPrevFlare = NULL;
  for (pFlare = CFlare::activeFlares; pFlare != NULL;
