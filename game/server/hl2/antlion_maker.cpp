...
DEFINE_KEYFIELD(m_strSpawnGroup, FIELD_STRING, "spawngroup"),
    ...
    DEFINE_THINKFUNC(PoolRegenThink), DEFINE_THINKFUNC(FindNodesCloseToPlayer),
...
CBaseEntity *
CAntlionTemplateMaker::AllHintsFromClusterBlocked(CAI_Hint *pNode,
...
void CAntlionTemplateMaker::FindNodesCloseToPlayer(void) {
  ...
  // figure out if all of them are blocked. If they are then we only need to
  // play the blocked effects once
  //---
  for (int i = 0; i < hintList.Count(); i++) {
