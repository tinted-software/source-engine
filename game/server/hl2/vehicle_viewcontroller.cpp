...
//			group who want to have smooth view lerping code out of a
//keyframed 			controlled player viewpoint.
//
// $NoKeywords: $
//=============================================================================//
...
DEFINE_INPUTFUNC(FIELD_STRING, "ForcePlayerIn", InputForcePlayerIn),
    ...
    END_DATADESC()
        ...
        LINK_ENTITY_TO_CLASS(vehicle_viewcontroller,
...
void CPropVehicleViewController::Think(void) {
  ...
  // If the exit anim has finished, move the player to the right spot and stop
  // animating
...
// Purpose:
//-----------------------------------------------------------------------------
void CPropVehicleViewController::SetupMove(CBasePlayer *player, CUserCmd *ucmd,
                                           IMoveHelper *pHelper,
                                           CMoveData *move) {
...
void CPropVehicleViewController::InputForcePlayerIn(inputdata_t &inputdata) {
  ...
  if (inputdata.value.StringID() != NULL_STRING) {
    ...
    if (iEntryAnim == ACTIVITY_NOT_AVAILABLE) {
      Warning("vehicle_viewcontroller %s could not find specified entry "
              "animation %s\n",
