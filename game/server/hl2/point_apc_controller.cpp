...
//			pActivator -
//-----------------------------------------------------------------------------
CBaseEntity *CAPCController::FindTarget(string_t targetName,
...
QAngle CAPCController::AimBarrelAt(const Vector &parentTarget) {
  ...
  // along the surface of that sphere to intersect with the gun's shooting
  // direction To calculate that rotation, we simply calculate the intersection
  // of the ray coming out of the barrel with the target sphere (that's the new
  // target position) and use atan2() to get angles

  // angles from target pos to center
  float targetToCenterYaw = atan2(target.y, target.x);
  float centerToGunYaw =
      atan2(m_barrelPos.y, sqrt(quadTarget - (m_barrelPos.y * m_barrelPos.y)));

  float targetToCenterPitch = atan2(target.z, sqrt(quadTargetXY));
  float centerToGunPitch =
      atan2(-m_barrelPos.z, sqrt(quadTarget - (m_barrelPos.z * m_barrelPos.z)));
...
void CAPCController::TrackTarget(void) {
  ...
  UTIL_TraceLine(barrelEnd, worldTargetPosition, MASK_BLOCKLOS, this,
  ...
  if (lineOfSight == TRUE) {
    // FIXME: This will ultimately have to deal with NPCs being in the vehicle
    // as well See if the target is in a vehicle. If so, check its relationship
    ...
    if (pPlayer && pPlayer->IsInAVehicle()) {
      ...
      if (pVehicle->ClassifyPassenger(pPlayer, CLASS_PLAYER) == CLASS_PLAYER) {
        if (!m_bFireDelayed) {
          m_bFireDelayed = true;
          m_flFiringDelay = gpGlobals->curtime +
                            1.5; // setup delay time before we start firing
          return;
        }
        if (gpGlobals->curtime > m_flFiringDelay) {
          m_OnFireAtTarget.Set(
              forward, this,
              this); // tell apc to fire rockets, and what direction
        }
      }
    }
  } else {
