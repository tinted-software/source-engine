...
void CNPC_Hydra::Spawn() {
  ...
  ClearEffects();
  m_iHealth = 20;
  m_flFieldOfView = -1.0; // indicates the width of this NPC's forward view cone
...
void CNPC_Hydra::RunTask(const Task_t *pTask) {
  ...
  case TASK_HYDRA_PREP_STAB: {
    ...
    for (i = i; i < m_body.Count() - 1; i++) {
      if (m_body[i].bStuck) {
        Vector delta =
            DotProduct(m_body[i].vecGoalPos - m_body[i].vecPos, m_vecHeadDir) *
            m_vecHeadDir;
...
//-----------------------------------------------------------------------------
IPhysicsConstraint *
CHydraImpale::CreateConstraint(CNPC_Hydra *pHydra, IPhysicsObject *pTargetPhys,
                               IPhysicsConstraintGroup *pGroup) {
  m_hHydra = pHydra;
