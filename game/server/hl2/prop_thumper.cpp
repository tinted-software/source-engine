...
#if HL2_EPISODIC
#define THUMPER_RADIUS                                                         \
  m_iEffectRadius // this const is only used inside the thumper anyway
#else
...
ConVar thumper_show_radius(
    "thumper_show_radius", "0", FCVAR_CHEAT,
...
DEFINE_FIELD(m_bEnabled, FIELD_BOOLEAN),
...
#endif
    DEFINE_SOUNDPATCH(m_sndMotor), DEFINE_THINKFUNC(Think),
  ...
  CAntlionRepellant *pRepellant =
      (CAntlionRepellant *)CreateEntityByName("point_antlion_repellant");
...
void CPropThumper::InitMotorSound(void) {
  ...
  m_sndMotor = (CSoundEnvelopeController::GetController())
                   .SoundCreate(filter, entindex(), CHAN_STATIC,
                                "coast.thumper_ambient", ATTN_NORM);
...
void CPropThumper::HandleState(void) {
  ...
  (CSoundEnvelopeController::GetController())
      .Play(m_sndMotor, 1.0f, m_flPlaybackRate * 100);
}
...
void CPropThumper::Thump(void) {
  if (m_iHammerAttachment != -1) {
    ...
    UTIL_ScreenShake(vOrigin, 10.0 * m_flPlaybackRate, m_flPlaybackRate,
                     m_flPlaybackRate / 2, THUMPER_RADIUS * m_flPlaybackRate,
                     SHAKE_START, false);
  }

  EmitSound("coast.thumper_dust");
  CSoundEnt::InsertSound(SOUND_THUMPER, GetAbsOrigin(),
                         THUMPER_RADIUS * m_flPlaybackRate,
                         THUMPER_SOUND_DURATION, this);

  if (thumper_show_radius.GetBool()) {
    NDebugOverlay::Box(
        GetAbsOrigin(),
        Vector(-THUMPER_RADIUS, -THUMPER_RADIUS, -THUMPER_RADIUS),
        Vector(THUMPER_RADIUS, THUMPER_RADIUS, THUMPER_RADIUS), 255, 64, 64,
        255, THUMPER_SOUND_DURATION);
  }
