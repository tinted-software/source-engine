...
//
// TODO:
//		Take advantage of new NPC fields like GetEnemy() and get rid of
//that OFFSET() stuff 		Revisit enemy validation stuff, maybe it's not necessary
//with the newest NPC code
//
// $NoKeywords: $
//=============================================================================//
...
public:
  ...
  // void TurretUse( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE
  // useType, float value );
...
bool CBaseTurret::KeyValue(const char *szKeyName, const char *szValue) {
  ...
  } else if (FStrEq(szKeyName, "style") || FStrEq(szKeyName, "height") ||
             FStrEq(szKeyName, "value1") || FStrEq(szKeyName, "value2") ||
...
void CBaseTurret::Spawn() {
  ...
  if ((m_spawnflags & SF_NPC_TURRET_AUTOACTIVATE) &&
      !(m_spawnflags & SF_NPC_TURRET_STARTINACTIVE)) {
    m_iAutoStart = true;
  }
...
void CBaseTurret::ActiveThink(void) {
  ...
  if (m_fBeserk) {
    ...
    if (random->RandomInt(0, 9) == 0) {
      ...
      OnTakeDamage(
          CTakeDamageInfo(this, this, 1, DMG_GENERIC)); // don't beserk forever
      return;
    }
...
//
// This think function will deploy the turret when something comes into range.
...
void CBaseTurret::TraceAttack(const CTakeDamageInfo &inputInfo,
  ...
  if (ptr->hitgroup == 10) {
    ...
    info.SetDamage(0.1); // don't hurt the NPC much, but allow
...
int CBaseTurret::MoveTurret(void) {
  ...
  flDiff = AngleNormalize(
      UTIL_ApproachAngle(vecGoalLocalAngles.x, 0.0, 0.1 * m_iBaseTurnRate));
...
#else
  flDiff = AngleNormalize(
      UTIL_ApproachAngle(vecGoalLocalAngles.y, 0.0, 0.1 * m_iBaseTurnRate));
#endif
  ...
  if (bDidMove) {
    // DevMsg( "(%.2f, %.2f)\n", AngleNormalize( vecGoalLocalAngles.x ),
    // AngleNormalize( vecGoalLocalAngles.y ) );
  }
  return bDidMove;
}
...
void CCeilingTurret::Shoot(const Vector &vecSrc, const Vector &vecDirToEnemy) {
  // NDebugOverlay::Line( vecSrc, vecSrc + vecDirToEnemy * 512, 0, 255, 255,
  // false, 0.1 );
