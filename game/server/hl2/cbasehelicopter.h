...
#define BASECHOPPER_WASH_RAMP_TIME                                             \
  1.0 // Time it takes to ramp from the initial to the max force on an object in
      // the wash (at the center of the wash)
...
protected:
  CSoundPatch *
      m_pRotorSound; // Rotor loop played when the player can see the helicopter
  ...
  float
      m_flMaxSpeedFiring; // Maximum speed of the helicopter whilst firing guns.
  ...
  void
  InputSetAngles(inputdata_t &inputdata); // Sets the angles of the helicopter
