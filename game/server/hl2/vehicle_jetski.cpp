...
private:
  ...
  float m_flHandbrakeTime; // handbrake after the fact to keep vehicles from
                           // rolling
...
// Purpose:
//-----------------------------------------------------------------------------
void CPropJetski::TraceAttack(const CTakeDamageInfo &info, const Vector &vecDir,
                              trace_t *ptr) {
...
void CPropJetski::Think(void) {
  ...
  } else if (!m_bInitialHandbrake) // after initial timer expires, set the
                                   // handbrake
  {
...
void CPropJetski::OnTurn(CUserCmd *ucmd) {
  ...
  if (bRight) {
    float flLengthRight =
        JETSKI_SHOCK_LENGTH_SHORT +
        (JETSKI_SHOCK_LENGTH_REST - JETSKI_SHOCK_LENGTH_SHORT) *
            flInvAbsSteering;
    float flLengthLeft =
        JETSKI_SHOCK_LENGTH_REST +
        (JETSKI_SHOCK_LENGTH_LONG - JETSKI_SHOCK_LENGTH_REST) * flAbsSteering;
...
float CPropJetski::CalculateFriction(CUserCmd *ucmd) {
  ...
  flFriction = JETSKI_FRICTION_MIN +
               (JETSKI_FRICTION_MAX - JETSKI_FRICTION_MIN) * flRatio;
...
#if 1
  // Update springs.
  m_springLen[0] = UTIL_Approach(m_flSpringLengthApproach[0], m_springLen[0],
                                 JETSKI_PITCH_AND_ROLL_RATE);
  m_pVehicle->SetSpringLength(0, m_springLen[0]);
  m_springLen[1] = UTIL_Approach(m_flSpringLengthApproach[1], m_springLen[1],
                                 JETSKI_PITCH_AND_ROLL_RATE);
  m_pVehicle->SetSpringLength(1, m_springLen[1]);
  m_springLen[2] = UTIL_Approach(m_flSpringLengthApproach[2], m_springLen[2],
                                 JETSKI_PITCH_AND_ROLL_RATE);
  m_pVehicle->SetSpringLength(2, m_springLen[2]);
  m_springLen[3] = UTIL_Approach(m_flSpringLengthApproach[3], m_springLen[3],
                                 JETSKI_PITCH_AND_ROLL_RATE);
...
// Purpose:
//-----------------------------------------------------------------------------
void CPropJetski::SetupMove(CBasePlayer *player, CUserCmd *ucmd,
                            IMoveHelper *pHelper, CMoveData *move) {
...
void CPropJetski::CreateSplash(int nSplashType) {
  ...
  case JETSKI_SPLASH_SPRAY: {
    ...
    data.m_flScale = JETSKI_SPLASH_SPRAY_SIZE +
                     random->RandomFloat(0, JETSKI_SPLASH_SPRAY_SIZE * 0.25);
  ...
  case JETSKI_SPLASH_GURGLE: {
    ...
    data.m_flScale = JETSKI_SPLASH_GURGLE_SIZE +
                     random->RandomFloat(0, JETSKI_SPLASH_GURGLE_SIZE * 0.25);
    DispatchEffect("waterripple", data);
    DispatchEffect("watersplash", data);
  }
  case JETSKI_SPLASH_RIPPLE: {
    ...
    vecSplashDir = vecUp;
    data.m_vNormal = vecSplashDir;
    data.m_flScale = JETSKI_SPLASH_RIPPLE_SIZE +
                     random->RandomFloat(0, JETSKI_SPLASH_RIPPLE_SIZE * 0.25);
    DispatchEffect("waterripple", data);
  }
  default: {
    return;
