...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_GroundTurret::MakeTracer(const Vector &vecTracerSrc,
...
void CNPC_GroundTurret::GatherConditions() {
  ...
  for (AI_EnemyInfo_t *pEMemory = GetEnemies()->GetFirst(&iter);
    ...
    if (pEMemory->timeLastSeen <
        gpGlobals->curtime - GROUNDTURRET_RETIRE_TIME) {
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CNPC_GroundTurret::FVisible(CBaseEntity *pEntity, int traceMask,
                                 CBaseEntity **ppBlocker) {
  if (BaseClass::FVisible(pEntity, traceMask, ppBlocker))
    return true;
  if ((pEntity->GetAbsOrigin().AsVector2D() - GetAbsOrigin().AsVector2D())
              .LengthSqr() < Square(10 * 12) &&
      FInViewCone(pEntity->GetAbsOrigin()) &&
      BaseClass::FVisible(pEntity->GetAbsOrigin() + Vector(0, 0, 1), traceMask,
                          ppBlocker))
    return true;
  return false;
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_GroundTurret::ProjectBeam(const Vector &vecStart,
                                    const Vector &vecDir, int width,
  ...
  trace_t tr;
  AI_TraceLine(vecStart, vecStart + vecDir * m_flSensingDist, MASK_SHOT, this,
...
void CNPC_GroundTurret::Scan() {
  if (m_bSeeEnemy) {
    // Using a bool for this check because the condition gets wiped out by
    // changing schedules.
    return;
  }
  ...
  scanAngle = GetAbsAngles();
  scanAngle.y +=
      (GROUNDTURRET_VIEWCONE / 2.0f) * sin(gpGlobals->curtime * 3.0f);
