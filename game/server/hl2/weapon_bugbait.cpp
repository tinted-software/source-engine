...
void CWeaponBugBait::Spawn(void) {
  ...
  // Increase the bugbait's pickup volume. It spawns inside the antlion guard's
  // body, and playtesters seem to be wary about moving into the body.
...
void CWeaponBugBait::FallInit(void) {
  // Bugbait shouldn't be physics, because it musn't roll/move away from it's
  // spawnpoint. The game will break if the player can't pick it up, so it must
  // stay still.
...
void CWeaponBugBait::Drop(const Vector &vecVelocity) {
  ...
  // On touch, stick & stop moving. Increase our thinktime a bit so we don't
  // stomp the touch for a bit
...
void CWeaponBugBait::SecondaryAttack(void) {
  ...
  if (CGrenadeBugBait::ActivateBugbaitTargets(GetOwner(), GetAbsOrigin(),
                                              true) == false) {
...
void CWeaponBugBait::ThrowGrenade(CBasePlayer *pPlayer) {
  ...
  CGrenadeBugBait *pGrenade = BugBaitGrenade_Create(
      vThrowPos, vec3_angle, vThrowVel,
  ...
  if (pGrenade != NULL) {
    ...
    UTIL_TraceLine(pPlayer->EyePosition(),
                   pPlayer->EyePosition() + (vForward * 128), MASK_SHOT, this,
...
//			*pOperator -
//-----------------------------------------------------------------------------
void CWeaponBugBait::Operator_HandleAnimEvent(animevent_t *pEvent,
                                              CBaseCombatCharacter *pOperator) {
  CBasePlayer *pOwner = ToBasePlayer(GetOwner());
  switch (pEvent->event) {
...
// Input  : true -
//-----------------------------------------------------------------------------
void CWeaponBugBait::SetSporeEmitterState(bool state) { m_bEmitSpores = state; }
