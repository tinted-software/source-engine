...
int FindBullseyesInCone(CBaseEntity **pList, int listMax,
  ...
  for (int i = g_BullseyeList.m_list.Count() - 1; i >= 0; --i) {
    ...
    if (IsPointInCone(pTest->GetAbsOrigin(), coneOrigin, coneAxis, coneAngleCos,
                      coneLength)) {
      pList[count] = pTest;
      count++;
      if (count >= listMax)
...
void CNPC_Bullseye::Event_Killed(const CTakeDamageInfo &info) {
  ...
  if (GetParent()) {
    if (GetParent()->ClassMatches("prop_combine_ball")) {
      // If this bullseye is parented to a combine ball, explode the combine
      // ball and remove this bullseye.
...
void CNPC_Bullseye::DecalTrace(trace_t *pOldTrace, char const *decalName) {
  ...
  trace_t pNewTrace;
  AI_TraceLine(vStartTrace, vEndTrace, MASK_SHOT, this, COLLISION_GROUP_NONE,
               &pNewTrace);
...
// Purpose:
//-----------------------------------------------------------------------------
void CNPC_Bullseye::ImpactTrace(trace_t *pTrace, int iDamageType,
...
bool CNPC_Bullseye::CanBeAnEnemyOf(CBaseEntity *pEnemy) {
  ...
  if (m_flMinDistValidEnemy > 0) {
    float distSq =
        (GetAbsOrigin().AsVector2D() - pEnemy->GetAbsOrigin().AsVector2D())
            .LengthSqr();
...
//-----------------------------------------------------------------------------
// Purpose: Bullseyes should always report light damage if any amount of damage
// is taken Input  : fDamage - amount of damage
...
//			bitsDamageType -
//-----------------------------------------------------------------------------
void CNPC_Bullseye::TraceAttack(const CTakeDamageInfo &info,
