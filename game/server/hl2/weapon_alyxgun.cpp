...
acttable_t CWeaponAlyxGun::m_acttable[] = {
    ...
    {ACT_READINESS_RELAXED_TO_STIMULATED,
     ACT_READINESS_PISTOL_RELAXED_TO_STIMULATED, false},
    {ACT_READINESS_RELAXED_TO_STIMULATED_WALK,
     ACT_READINESS_PISTOL_RELAXED_TO_STIMULATED_WALK, false},
    {ACT_READINESS_AGITATED_TO_STIMULATED,
     ACT_READINESS_PISTOL_AGITATED_TO_STIMULATED, false},
    {ACT_READINESS_STIMULATED_TO_RELAXED,
     ACT_READINESS_PISTOL_STIMULATED_TO_RELAXED, false},

    //	{ ACT_ARM,				ACT_ARM_PISTOL,
    //true }, 	{ ACT_DISARM,			ACT_DISARM_PISTOL,
    //true },
};
...
#define ALYX_TOOCLOSETIMER                                                     \
  1.0f // Time an enemy must be tooclose before Alyx is allowed to shoot it.
...
// Purpose:
//-----------------------------------------------------------------------------
void CWeaponAlyxGun::Precache(void) { BaseClass::Precache(); }
...
//			but don't prevent her from defending herself if
//cornered.
// Input  : flDot -
//			flDist -
// Output : int
...
int CWeaponAlyxGun::WeaponRangeAttack1Condition(float flDot, float flDist) {
  ...
  if (flDist < m_fMinRange1) {
    ...
    // If the condition persists, allow her to ignore it and defend herself. The
    // idea is to stop Alyx being content to fire point blank at enemies if
    // she's able to move away, without making her defenseless if she's not able
    // to move.
    float flTime;
  ...
  // While in a vehicle, we extend our aiming cone (this relies on
  // COND_NOT_FACING_ATTACK
  // TODO: This needs to be rolled in at the animation level
  if (GetOwner()->IsInAVehicle()) {
    Vector vecRoughDirection =
        (GetOwner()->GetEnemy()->WorldSpaceCenter() - WorldSpaceCenter());
    Vector vecRight;
...
// Input  : *pOperator -
//-----------------------------------------------------------------------------
void CWeaponAlyxGun::FireNPCPrimaryAttack(CBaseCombatCharacter *pOperator,
...
// Purpose:
//-----------------------------------------------------------------------------
void CWeaponAlyxGun::Operator_ForceNPCFire(CBaseCombatCharacter *pOperator,
                                           bool bSecondary) {
...
//-----------------------------------------------------------------------------
void CWeaponAlyxGun::Operator_HandleAnimEvent(animevent_t *pEvent,
