...
void CWeaponSMG1::Operator_HandleAnimEvent(animevent_t *pEvent,
  ...
  case EVENT_WEAPON_AR2_ALTFIRE: {
    ...
    // If on the rare case the vector is 0 0 0, cancel for avoid launching the
    // grenade without speed This should be on WeaponRangeAttack2Condition(),
    // but for some unknown reason return CASE_NONE doesn't stop the launch
    if (vecThrow == Vector(0, 0, 0)) {
      break;
    }

    CGrenadeAR2 *pGrenade =
        (CGrenadeAR2 *)Create("grenade_ar2", vecShootOrigin, vec3_angle, npc);
    ...
    pGrenade->SetGravity(0.5); // lower gravity since grenade is aerodynamic and
