...
DEFINE_FIELD(m_vecCameraView, FIELD_VECTOR),
    ...
    // Inputs
    DEFINE_INPUTFUNC(FIELD_STRING, "SetCameraViewEntity",
    ...
    // Fov & fov blends
    SendPropInt(SENDINFO(m_iFOV), 9), SendPropInt(SENDINFO(m_iNextFOV), 9),
    ...
    SendPropArray(SendPropFloat(SENDINFO_ARRAY(m_flFadeColor), 32,
                                SPROP_NOSCALE),
                  m_flFadeColor),
...
void CScriptIntro::Activate(void) {
  // Restore our script pointer, this is necessary to trigger other internal
  // logic to due with PVS checks
  if (m_bActive) {
    g_hIntroScript = this;
  }
...
void CScriptIntro::InputSetCameraViewEntity(inputdata_t &inputdata) {
  ...
  CBaseEntity *pEntity = gEntList.FindEntityByName(
      NULL, iszEntityName, NULL, inputdata.pActivator, inputdata.pCaller);
  if (!pEntity) {
...
//-----------------------------------------------------------------------------
bool CScriptIntro::GetIncludedPVSOrigin(Vector *pOrigin,
                                        CBaseEntity **ppCamera) {
...
void CScriptIntro::InputSetFOVBlendTime(inputdata_t &inputdata) {
  ...
  if (m_flNextFOVBlendTime >= gpGlobals->curtime) {
    ...
    // If we weren't blending, then we need to construct a proper starting point
    // from scratch
...
void CScriptIntro::InputSetNextBlendTime(inputdata_t &inputdata) {
  ...
  if (m_iQueuedNextBlendMode < 0) {
    Warning("script_intro: Warning!! Set blend time without setting next blend "
            "mode!\n");
  ...
  if (cl_spewscriptintro.GetInt()) {
    DevMsg(1, "%.2f BLEND STARTED: %d to %d, end at %.2f\n", gpGlobals->curtime,
           m_iBlendMode.Get(), m_iNextBlendMode.Get(), m_flNextBlendTime.Get());
  }

  SetContextThink(&CScriptIntro::BlendComplete, m_flNextBlendTime,
                  "BlendComplete");
}
...
void CScriptIntro::InputFadeTo(inputdata_t &inputdata) {
  ...
  if (!pszParam || !pszParam[0]) {
    Warning("%s (%s) received FadeTo input without an alpha. Syntax: <fade "
            "alpha> <fade duration>\n",
  ...
  if (!pszParam || !pszParam[0]) {
    Warning("%s (%s) received FadeTo input without a duration. Syntax: <fade "
            "alpha> <fade duration>\n",
            GetClassname(), GetDebugName());
    return;
  }
  ...
  // m_flFadeAlpha.Get(), gpGlobals->curtime + m_flFadeDuration.Get() );
}
...
void CScriptIntro::InputSetFadeColor(inputdata_t &inputdata) {
  ...
  char *pszParam = strtok(parseString, " ");
  if (!pszParam || !pszParam[0]) {
    Warning("%s (%s) received SetFadeColor input without correct parameters. "
