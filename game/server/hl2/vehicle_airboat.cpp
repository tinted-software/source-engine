...
private:
  ...
  // handbrake after the fact to keep vehicles from rolling bool
  // m_bInitialHandbrake;
  ...
  float
      m_flWaterStoppedPitchTime; // Time to pitch shift the water stopped sound.
...
#define ENGINE_DUCK_TIME 4.0
  ...
  if (controller.SoundGetVolume(m_pEngineSound) == 0) {
    ...
    controller.SoundChangeVolume(m_pEngineSound,
                                 RemapValClamped(speedRatio, 0, 1.0,
                                                 ENGINE_MIN_VOLUME,
                                                 ENGINE_MAX_VOLUME),
                                 0.0);
...
#define FAN_DUCK_TIME 2.0          // seconds over which to duck the fan sound
  ...
  if (speedRatio >= 0.8) {
    ...
    controller.SoundChangeVolume(m_pFanSound,
                                 RemapValClamped(fabs(m_flThrottle), 0, 1.0,
                                                 FAN_MIN_VOLUME,
                                                 FAN_MAX_VOLUME),
                                 0.25);
    controller.SoundChangeVolume(m_pFanMaxSpeedSound, 0.0, 0.0);
