...
ConVar
    ai_readiness_decay("ai_readiness_decay",
...
//------------------------------------------------------------------------------
WeaponProficiency_t
CNPC_PlayerCompanion::CalcWeaponProficiency(CBaseCombatWeapon *pWeapon) {
...
int CNPC_PlayerCompanion::OnTakeDamage_Alive(const CTakeDamageInfo &info) {
  if (info.GetAttacker()) {
    ...
    // getting injured by
    //						  fires that have huge damage
    //radii that extend outside the rendered 						  fire. Recovery from being injured
    //by fire will also not be done 						  before we ship/ Here we trade one bug (guys
    //standing around dying 						  from flames they appear to not be near), for a
    //lesser one 						  this guy was standing in a fire and didn't react. Since 						  the
    //levels are supposed to have the centers of all the fires 						  npc clipped, this
...
void CNPC_PlayerCompanion::InputOutsideTransition(inputdata_t &inputdata) {
  ...
  while (pHint) {
    pHint->Lock(this);
    pHint->Unlock(
        0.5); // prevent other squadmates and self from using during transition.
