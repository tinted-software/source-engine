...
public:
  ...
  void Ignite(float flFlameLifetime, bool bNPCOnly = true, float flSize = 0.0f,
...
void CZombie::PainSound(const CTakeDamageInfo &info) {
  // We're constantly taking damage when we are on fire. Don't make all those
  // noises!
  if (IsOnFire()) {
    return;
  }
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CZombie::ShouldBecomeTorso(const CTakeDamageInfo &info,
...
void CZombie::GatherConditions(void) {
  ...
  if (ConditionInterruptsCurSchedule(COND_ZOMBIE_CHARGE_TARGET_MOVED)) {
    if (GetNavigator()->IsGoalActive()) {
      ...
      if (!GetEnemy() || (m_vPositionCharged - GetEnemyLKP()).Length() >
                             CHARGE_RESET_TOLERANCE) {
...
void CZombie::StartTask(const Task_t *pTask) {
  ...
  case TASK_ZOMBIE_YAW_TO_DOOR: {
    AssertMsg(
        m_hBlockingDoor != NULL,
        "Expected condition handling to break schedule before landing here");
    if (m_hBlockingDoor != NULL) {
  ...
  case TASK_ZOMBIE_CHARGE_ENEMY: {
    ...
    else if (GetNavigator()->SetVectorGoalFromTarget(
                 GetEnemy()->GetLocalOrigin())) {
...
//---------------------------------------------------------
void CZombie::Ignite(float flFlameLifetime, bool bNPCOnly, float flSize,
                     bool bCalledByLevelDesigner) {
  if (!IsOnFire() && IsAlive()) {
    BaseClass::Ignite(flFlameLifetime, bNPCOnly, flSize,
                      bCalledByLevelDesigner);
...
int CZombie::OnTakeDamage_Alive(const CTakeDamageInfo &inputInfo) {
  ...
  if (inputInfo.GetDamageType() & DMG_BUCKSHOT) {
    if (!m_fIsTorso && inputInfo.GetDamage() > (m_iMaxHealth / 3)) {
      // Always flinch if damaged a lot by buckshot, even if not shot in the
      // head. The reason for making sure we did at least 1/3rd of the zombie's
      // max health is so the zombie doesn't flinch every time the odd shotgun
      // pellet hits them, and so the maximum number of times you'll see a
      // zombie flinch like this is 2.(sjb)
...
bool CZombie::IsHeavyDamage(const CTakeDamageInfo &info) {
  ...
  if (info.GetDamageType() & (DMG_BULLET | DMG_BUCKSHOT)) {
    ...
    if (!HasCondition(COND_CAN_MELEE_ATTACK1) && (RandomFloat() > 0.5)) {
      // Randomly forget I've flinched, so that I'll be forced to play a big
      // flinch If this doesn't happen, it means I may not fully flinch if I
      // recently flinched
      if (RandomFloat() > 0.75) {
...
//---------------------------------------------------------
//---------------------------------------------------------
#define ZOMBIE_SQUASH_MASS                                                     \
  300.0f // Anything this heavy or heavier squashes a zombie good. (show special
         // fx)
bool CZombie::IsSquashed(const CTakeDamageInfo &info) {
  ...
  if (info.GetDamageType() & DMG_CRUSH) {
    ...
    if (pCrusher && pCrusher->GetMass() >= ZOMBIE_SQUASH_MASS &&
      ...
      // This heuristic detects when a zombie has been squashed from above by a
      // heavy item. Done specifically so we can add gore effects to Ravenholm
      // cartraps. The zombie must take physics damage from a 300+kg object that
      // is centered above its eyes (comes from above)
      return true;
    }
  }
...
DEFINE_SCHEDULE(
    ...
    "		TASK_PLAY_SEQUENCE				"
    "ACTIVITY:ACT_ZOMBIE_TANTRUM" /* placeholder until frustration/rage/fence
