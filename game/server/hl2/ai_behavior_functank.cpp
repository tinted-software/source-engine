...
DEFINE_FIELD(m_hFuncTank, FIELD_EHANDLE),
    ...
    DEFINE_FIELD(m_bSpottedPlayerOutOfCover, FIELD_BOOLEAN), END_DATADESC();
...
void CAI_FuncTankBehavior::StartTask(const Task_t *pTask) {
  ...
  case TASK_SCAN_LEFT_FUNCTANK: {
    ...
    QAngle vecTargetAngles( flCenterPitch + flPitch, UTIL_AngleMod( flCenterYaw
    + flScanAmount ), 0 );
    */
...
void CAI_FuncTankBehavior::RunTask(const Task_t *pTask) {
  ...
  case TASK_FIRE_FUNCTANK: {
    ...
    if (m_hFuncTank->NPC_HasEnemy()) {
      ...
      // The NPC may have decided to stop using the func_tank, because it's out
      // of ammo.
      if (!m_hFuncTank) {
        TaskComplete();
        break;
      ...
      bBestSeen =
          (pNPC->GetSenses()->DidSeeEntity(pEnemy) ||
           pNPC->FVisible(pEnemy)); // @TODO (toml 04-02-03): Need to optimize
                                    // CanSeeEntity() so multiple calls in frame
                                    // do not recalculate, rather cache
