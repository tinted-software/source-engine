...
DEFINE_KEYFIELD(m_yawRate, FIELD_FLOAT, "yawrate"),
    ...
    DEFINE_KEYFIELD(m_iBulletDamageVsPlayer, FIELD_INTEGER,
    ...
    DEFINE_KEYFIELD(m_flPlayerLockTimeBeforeFire, FIELD_FLOAT,
...
void CFuncTank::NPC_FindController(void) {
  ...
  float flMinDistToEnemy2 =
      (FUNCTANK_DISTANCE_MIN_TO_ENEMY * FUNCTANK_DISTANCE_MIN_TO_ENEMY);
  ...
  for (int iAI = 0; iAI < nAICount; ++iAI) {
    ...
    CAI_FuncTankBehavior *pBehavior;
    if (pNPC->GetBehavior(&pBehavior)) {
      // Don't mount the func_tank if your "enemy" is within X feet or it or the
      // npc.
      ...
      if (pEnemy) {
        ...
        float flDist2 =
            (pEnemy->GetAbsOrigin() - pNPC->GetAbsOrigin()).LengthSqr();
      ...
      trace_t tr;
      UTIL_TraceEntity(pNPC, vecMountPos, vecMountPos, MASK_NPCSOLID, this,
                       pNPC->GetCollisionGroup(), &tr);
      if (tr.startsolid || tr.fraction < 1.0) {
        // Don't mount the tank if someone/something is located on the control
        // point.
        continue;
      }
...
void CFuncTank::DrawDebugGeometryOverlays(void) {
  ...
  NDebugOverlay::Line(GetAbsOrigin(), GetAbsOrigin() + (vecForward * 64), 255,
                      255, 255, true, 0.1);

  // Draw the yaw ranges
  angCenter = QAngle(0, YawCenterWorld() + m_yawRange, 0);
  AngleVectors(angCenter, &vecForward);
  NDebugOverlay::Line(GetAbsOrigin(), GetAbsOrigin() + (vecForward * 128), 0,
                      255, 0, true, 0.1);
  angCenter = QAngle(0, YawCenterWorld() - m_yawRange, 0);
  AngleVectors(angCenter, &vecForward);
  NDebugOverlay::Line(GetAbsOrigin(), GetAbsOrigin() + (vecForward * 128), 0,
  ...
  AngleVectors(angCenter, &vecForward);
  NDebugOverlay::Line(GetAbsOrigin(), GetAbsOrigin() + (vecForward * 128), 255,
                      0, 0, true, 0.1);
  angCenter = QAngle(PitchCenterWorld() - m_pitchRange, 0, 0);
  AngleVectors(angCenter, &vecForward);
  NDebugOverlay::Line(GetAbsOrigin(), GetAbsOrigin() + (vecForward * 128), 255,
...
void CFuncTank::Spawn(void) {
  ...
  if (GetParent() && GetParent()->GetBaseAnimating()) {
    ...
    m_bUsePoseParameters = (m_iszYawPoseParam != NULL_STRING) &&
    ...
    if (m_iszBarrelAttachment != NULL_STRING) {
      ...
      m_nBarrelAttachment =
          pAnim->LookupAttachment(STRING(m_iszBarrelAttachment));

      Vector vecWorldBarrelPos;
      QAngle worldBarrelAngle;
      pAnim->GetAttachment(m_nBarrelAttachment, vecWorldBarrelPos,
                           worldBarrelAngle);
      VectorITransform(vecWorldBarrelPos, EntityToWorldTransform(),
                       m_barrelPos);
    }
...
bool CFuncTank::OnControls(CBaseEntity *pTest) {
  ...
  if (!m_hControlVolume) {
    ...
    if ((!m_hControlVolume) && IsControllable()) {
      Msg("ERROR: Couldn't find control volume for player-controllable "
          "func_tank %s.\n",
...
void CFuncTank::ControllerPostFrame(void) {
  ...
  if (HasSpawnFlags(SF_TANK_AIM_ASSISTANCE)) {
    // Trace out a hull and if it hits something, adjust the shot to hit that
    // thing.
    trace_t tr;
    Vector start = WorldBarrelPosition();
    Vector dir = forward;

    UTIL_TraceHull(start, start + forward * 8192, -Vector(8, 8, 8),
                   Vector(8, 8, 8), MASK_SHOT, this, COLLISION_GROUP_NONE, &tr);

    if (tr.m_pEnt && tr.m_pEnt->m_takedamage != DAMAGE_NO &&
        (tr.m_pEnt->GetFlags() & FL_AIMTARGET)) {
  ...
  CSoundEnt::InsertSound(SOUND_COMBAT, WorldSpaceCenter(), FUNCTANK_FIREVOLUME,
                         0.2);
...
//-----------------------------------------------------------------------------
bool CFuncTank::HasController(void) { return (m_hController != NULL); }

//-----------------------------------------------------------------------------
// Purpose:
// Output : CBaseCombatCharacter
//-----------------------------------------------------------------------------
CBaseCombatCharacter *CFuncTank::GetController(void) { return m_hController; }
...
//-----------------------------------------------------------------------------
// Purpose: The NPC manning this gun just saw a player for the first time since
// he left cover
//-----------------------------------------------------------------------------
...
void CFuncTank::NPC_Fire(void) {
  ...
  // m_fireLast looks like it is only needed for Active non-controlled
  // func_tank.
  //		m_fireLast = gpGlobals->curtime - (1/m_fireRate) - 0.01;  // to
  //make sure the gun doesn't fire too many bullets

  Vector vecBarrelEnd = WorldBarrelPosition();
  Vector vecForward;
  AngleVectors(GetAbsAngles(), &vecForward);

  if ((pNPC->CapabilitiesGet() & bits_CAP_NO_HIT_SQUADMATES) &&
      pNPC->IsInSquad()) {
    // Avoid shooting squadmates.
    if (pNPC->IsSquadmateInSpread(vecBarrelEnd,
                                  vecBarrelEnd + vecForward * 2048,
                                  gTankSpread[m_spread].x, 8 * 12)) {
      return;
    }
  }

  if (!HasSpawnFlags(SF_TANK_ALLOW_PLAYER_HITS) &&
      (pNPC->CapabilitiesGet() & bits_CAP_NO_HIT_PLAYER)) {
    // Avoid shooting player.
    if (pNPC->PlayerInSpread(vecBarrelEnd, vecBarrelEnd + vecForward * 2048,
                             gTankSpread[m_spread].x, 8 * 12)) {
      return;
    }
  }
...
void CFuncTank::Think(void) {
  ...
  if (IsControllable() && !HasController() &&
      (m_flNextControllerSearch <= gpGlobals->curtime)) {
    ...
    if (pPlayer) {
      ...
      if (m_hControlVolume) {
        if (m_hControlVolume->IsTouching(pPlayer) &&
            pPlayer->FInViewCone(WorldSpaceCenter())) {
          // If my control volume is touching a player that's facing the mounted
          // gun, automatically use the gun.
          // !!!BUGBUG - this only works in cases where the player can see the
          // gun whilst standing in the control volume. (This works just fine
          // for all func_tanks mounted on combine walls and small barriers)
          variant_t emptyVariant;
          AcceptInput("Use", pPlayer, pPlayer, emptyVariant, USE_TOGGLE);
        } else {
          // If the player is nearby, think faster for snappier response to XBox
          // auto mounting
...
void CFuncTank::CalcPlayerCrosshairTarget(Vector *pVecTarget) {
  ...
  UTIL_TraceLine(vecStart + vecDir * 24, vecStart + vecDir * 8192,
                 MASK_BLOCKLOS_AND_NPCS, this, COLLISION_GROUP_NONE, &tr);
    ...
    *pVecTarget = pEnemy->BodyTarget(GetAbsOrigin(), false);
    ...
    if (m_vecNPCIdleTarget != vec3_origin) {
      *pVecTarget = m_vecNPCIdleTarget;
      ...
      UTIL_TraceLine(vecBarrel, vecBarrel + vecForward * 8192, MASK_SHOT, this,
...
//-----------------------------------------------------------------------------
bool CFuncTank::RotateTankToAngles(const QAngle &angles, float *pDistX,
                                   float *pDistY) {
  bool bClamped = false;
  ...
  if ((fabs(offsetY) > m_yawRange + m_yawTolerance) ||
    ...
    flActualYaw =
        clamp(flActualYaw, m_yawCenter - m_yawRange, m_yawCenter + m_yawRange);
    flActualPitch = clamp(flActualPitch, m_pitchCenter - m_pitchRange,
...
// Purpose:
//-----------------------------------------------------------------------------
void CFuncTank::ComputeLeadingPosition(const Vector &vecShootPosition,
                                       CBaseEntity *pTarget,
  ...
  if (m_flNextLeadFactorTime < gpGlobals->curtime) {
    ...
    m_flNextLeadFactorTime =
        gpGlobals->curtime + random->RandomFloat(2.0f, 4.0f);
  }

  float flFactor = (gpGlobals->curtime - m_flStartLeadFactorTime) /
                   (m_flNextLeadFactorTime - m_flStartLeadFactorTime);
  float flLeadFactor = SimpleSplineRemapVal(
      flFactor, 0.0f, 1.0f, m_flStartLeadFactor, m_flNextLeadFactor);
  ...
  // y = flTargetToShooter
  // solve for predicted time using at^2 + bt + c = 0, t = (-b +/- sqrt( b^2 -
  // 4ac )) / 2a
...
void CFuncTank::AimFuncTankAtTarget(void) {
  ...
  if (m_spawnflags & SF_TANK_AIM_AT_POS) {
    ...
    if (tr.fraction == 1.0 || tr.m_pEnt == pTarget ||
        (pTargetVehicle && (tr.m_pEnt == pTargetVehicle))) {
      if (InRange2(range2) && pTarget && pTarget->IsAlive()) {
        bUpdateTime = true;

        // Sight position is BodyTarget with no noise (so gun doesn't bob up and
        // down)
  ...
  if (bUpdateTime) {
    if ((gpGlobals->curtime - m_lastSightTime >= 1.0) &&
        (gpGlobals->curtime > m_flNextAttack)) {
      // Enemy was hidden for a while, and I COULD fire right now. Instead, tack
      // a delay on.
  ...
  if (CanFire() &&
    ...
    if (m_spawnflags & SF_TANK_LINEOFSIGHT) {
      AI_TraceLine(barrelEnd, pTarget->WorldSpaceCenter(), MASK_SHOT, this,
                   COLLISION_GROUP_NONE, &tr);
...
void CFuncTank::Fire(int bulletCount, const Vector &barrelEnd,
  ...
  if (m_iEffectHandling == EH_AR2) {
    ...
    if (m_iszSpriteSmoke != NULL_STRING) {
      ...
      pSprite->SetTransparency(kRenderTransAlpha, m_clrRender->r,
                               m_clrRender->g, m_clrRender->b, 255,
                               kRenderFxNone);
  ...
  if (pAttacker && pAttacker->IsPlayer()) {
    if (IsX360()) {
      UTIL_PlayerByIndex(1)->RumbleEffect(
          RUMBLE_AR2, 0, RUMBLE_FLAG_RESTART | RUMBLE_FLAG_RANDOM_AMPLITUDE);
    } else {
      CSoundEnt::InsertSound(SOUND_MOVE_AWAY, barrelEnd + forward * 32.0f,
                             32.0f, 0.2f, pAttacker, SOUNDENT_CHANNEL_WEAPON);
    }
  }
...
bool CFuncTank::IsEntityInViewCone(CBaseEntity *pEntity) {
  ...
  // If we're trying to shoot at a player, and we've seen a non-player recently,
  ...
  // Check to see if the entity center lies within the yaw and pitch
  // constraints. This isn't horribly accurate, but should do for now.
  QAngle angGun;
  ...
  if ((fabs(flOffsetY) > m_yawRange + m_yawTolerance) ||
      (fabs(flOffsetX) > m_pitchRange + m_pitchTolerance))
    return false;
...
bool CFuncTank::HasLOSTo(CBaseEntity *pEntity) {
  ...
  CTraceFilterSkipTwoEntities traceFilter(this, GetParent(),
                                          COLLISION_GROUP_NONE);

  // UNDONE: Should this hit BLOCKLOS brushes?
  AI_TraceLine(vecBarrelEnd, vecTarget, MASK_BLOCKLOS_AND_NPCS, &traceFilter,
               &tr);

  CBaseEntity *pHitEntity = tr.m_pEnt;

  // Is entity in a vehicle? if so, verify vehicle is target and return if so
  ...
  if (pCCEntity != NULL && pCCEntity->IsInAVehicle()) {
    ...
    if (pHitEntity == pVehicle ||
        (pHitEntity != NULL && pHitEntity->GetOwnerEntity() == pVehicle))
      return true;
  }
...
void CFuncTankRocket::Fire(int bulletCount, const Vector &barrelEnd,
                           const Vector &forward, CBaseEntity *pAttacker,
                           bool bIgnoreSpread) {
  CMissile *pRocket = (CMissile *)CBaseEntity::Create("rpg_missile", barrelEnd,
                                                      GetAbsAngles(), this);
...
void CFuncTankAirboatGun::Activate() {
  ...
  if (m_iszAirboatGunModel != NULL_STRING) {
    m_hAirboatGunModel = dynamic_cast<CBaseAnimating *>(
        gEntList.FindEntityByName(NULL, m_iszAirboatGunModel));
...
void CFuncTankAirboatGun::StartFiring() {
  if (!m_bIsFiring) {
    ...
    pController->SoundChangeVolume(m_pGunFiringSound, 1.0f,
                                   0.1f * (1.0f - flVolume));
    m_bIsFiring = true;
  }
}
...
void CFuncTankAirboatGun::Fire(int bulletCount, const Vector &barrelEnd,
                               const Vector &forward, CBaseEntity *pAttacker,
                               bool bIgnoreSpread) {
  ...
  if (gpGlobals->curtime >= m_flNextHeavyShotTime) {
    m_flNextHeavyShotTime =
        gpGlobals->curtime + AIRBOAT_GUN_HEAVY_SHOT_INTERVAL;
  }
}
...
void CFuncTankAPCRocket::FireDying(const Vector &barrelEnd) {
  ...
  Vector vecVelocity;
  VectorMultiply(vecDir, m_flRocketSpeed * random->RandomFloat(0.75f, 1.25f),
                 vecVelocity);
  ...
  CAPCMissile *pRocket =
      (CAPCMissile *)CAPCMissile::Create(barrelEnd, angles, vecVelocity, this);
  ...
  // Make erratic firing
  m_fireRate = random->RandomFloat(DEATH_VOLLEY_MIN_FIRE_RATE,
                                   DEATH_VOLLEY_MAX_FIRE_RATE);
  if (--m_nBulletCount <= 0) {
    UTIL_Remove(this);
  }
}

void CFuncTankAPCRocket::Fire(int bulletCount, const Vector &barrelEnd,
                              const Vector &forward, CBaseEntity *pAttacker,
                              bool bIgnoreSpread) {
...
public:
  ...
  static CMortarShell *Create(const Vector &vecStart, const Vector &vecTarget,
                              const Vector &vecShotDir, float flImpactDelay,
                              float flWarnDelay, string_t warnSound);
...
private:
  void FixUpImpactPoint(const Vector &initialPos, const Vector &initialNormal,
                        Vector *endPos, Vector *endNormal);
...
#define MORTAR_TEST_RADIUS 16.0f
    ...
    //			*endNormal -
    //-----------------------------------------------------------------------------
    void CMortarShell::FixUpImpactPoint(const Vector &initialPos,
  ...
  trace_t tr;
  UTIL_TraceLine(vecStartOffset, vecStartOffset - Vector(0, 0, 256), MASK_SHOT,
...
CMortarShell *CMortarShell::Create(const Vector &vecStart,
                                   const Vector &vecTarget,
                                   const Vector &vecShotDir,
                                   float flImpactDelay, float flWarnDelay,
                                   string_t warnSound) {
  CMortarShell *pShell = (CMortarShell *)CreateEntityByName("mortarshell");

  // Place the mortar shell at the target location so that it can make the sound
  // and explode.
  trace_t tr;
  UTIL_TraceLine(vecTarget, vecTarget + (vecShotDir * 128.0f),
                 MASK_SOLID_BRUSHONLY, pShell, COLLISION_GROUP_NONE, &tr);

  Vector targetPos, targetNormal;
  pShell->FixUpImpactPoint(tr.endpos, tr.plane.normal, &targetPos,
                           &targetNormal);
  ...
  pShell->m_pBeamEffect[0]->PointsInit(
      vecStart,
  ...
  pShell->m_pBeamEffect[1]->PointsInit(
      vecStart,
      vecStart + Vector(vecStartSkew[0], vecStartSkew[1], MORTAR_BLAST_HEIGHT));
  ...
  trace_t skyTrace;
  UTIL_TraceLine(targetPos,
                 targetPos +
                     Vector(vecEndSkew[0], vecEndSkew[1], MORTAR_BLAST_HEIGHT),
  ...
  if (skyTrace.fraction <= 1.0f && skyTrace.surface.flags & SURF_SKY) {
    ...
    pShell->m_pBeamEffect[2]->PointsInit(
        targetPos,
    ...
    pShell->m_pBeamEffect[3]->PointsInit(
        targetPos,
        targetPos + Vector(vecEndSkew[0], vecEndSkew[1], MORTAR_BLAST_HEIGHT));
...
void UTIL_VisualizeCurve(int type, int steps, float bias) {
  ...
  for (int i = 0; i < steps; i++) {
    ...
    renderOffs = (vRight * (-steps * 0.5f) * renderScale) +
                 (vUp * (renderScale * -(steps * 0.5f))) +
                 (vRight * i * renderScale) +
                 (vUp * perc * (renderScale * steps));

    NDebugOverlay::Cross3D(renderOrigin + renderOffs, -Vector(2, 2, 2),
                           Vector(2, 2, 2), 255, 0, 0, true, 0.05f);

    if (i > 0) {
      NDebugOverlay::Line(renderOrigin + renderOffs,
                          renderOrigin + lastRenderOffs, 255, 0, 0, true,
                          0.05f);
    }
...
void CMortarShell::FlyThink() {
  ...
  if (gpGlobals->curtime > m_flNPCWarnTime) {
    // Warn the AI. Make this radius a little larger than the explosion will be,
    // and make the sound last a little longer.
    CSoundEnt::InsertSound(SOUND_DANGER | SOUND_CONTEXT_MORTAR, GetAbsOrigin(),
                           MORTAR_BLAST_RADIUS * 1.25,
  ...
  float lifePerc = 1.0f - ((m_flImpactTime - gpGlobals->curtime) /
                           (m_flImpactTime - m_flSpawnedTime));
...
void CMortarShell::Impact(void) {
  ...
  RadiusDamage(CTakeDamageInfo(this, GetOwnerEntity(), MORTAR_BLAST_DAMAGE,
                               (DMG_BLAST | DMG_DISSOLVE)),
...
void CMortarShell::FadeThink(void) {
  ...
  float lifePerc =
      1.0f - ((gpGlobals->curtime - m_flFadeTime) / MORTAR_FADE_LENGTH);
...
void CFuncTankMortar::Fire(int bulletCount, const Vector &barrelEnd,
  ...
  if (m_hTarget) {
    ...
    // vecProjectedPosition += GetEnemy()->GetSmoothedVelocity() * (m_fireDelay
    // * 1.1);
  } else
    return;
...
#define TARGET_SEARCH_DEPTH 100
  ...
  // Make a really rough approximation of the last half of the mortar trajectory
  // and trace it. Do this so that mortars fired into windows land on rooftops,
  // and that targets projected inside buildings (or out of the world) clip to
  // the world. (usually a building facade)
  ...
  // Trace up to find the fake 'apex' of the shell. The skybox or 1024 units,
  // whichever comes first.
  UTIL_TraceLine(vecSpot, vecSpot + Vector(0, 0, 1024), MASK_SOLID_BRUSHONLY,
  ...
  UTIL_TraceLine(vecSpot, vecProjectedPosition, MASK_SOLID_BRUSHONLY, NULL,
                 COLLISION_GROUP_NONE, &tr);
  ...
  CMortarShell::Create(barrelEnd, tr.endpos, vecFinalDir, m_fireDelay,
                       m_flWarningTime, m_incomingSound);
...
private:
  ...
  bool m_bLastTargetWasNPC; // Tells whether the last entity we fired a shot at
                            // was an NPC (otherwise it was the player)
};
...
void CFuncTankCombineCannon::CreateBeam() {
  if (!m_hBeam && gpGlobals->curtime >= m_flTimeBeamOn) {
    ...
    SetContextThink(&CFuncTankCombineCannon::UpdateBeamThink,
                    gpGlobals->curtime, s_pUpdateBeamThinkContext);
  } else {
  ...
  m_hBeam->PointsInit(WorldBarrelPosition(),
                      WorldBarrelPosition() + vecInitialAim);
...
void CFuncTankCombineCannon::AdjustRateOfFire() {
  ...
  /*
          if( m_hTarget.Get() != NULL && m_hTarget->IsPlayer() )
          {
                  ...
                  {
                          // Cheat, and be able to fire RIGHT NOW if the target
     is a player and the
                          // last target I fired at was an NPC. This prevents
     the player from running
...
void CFuncTankCombineCannon::UpdateBeamThink() {
  SetContextThink(&CFuncTankCombineCannon::UpdateBeamThink,
                  gpGlobals->curtime + 0.025, s_pUpdateBeamThinkContext);
  ...
  AI_TraceLine(vecBarrel, vecBarrel + vecAim * COMBINE_CANNON_BEAM_MAX_DIST,
               MASK_SHOT, this, COLLISION_GROUP_NONE, &trBeam);
  ...
  if (m_hTarget.Get() == NULL) {
    if (gpGlobals->curtime > m_flTimeNextSweep) {
      ...
      for (i = 0; i < 5; i++) {
        ...
        // Ignore the func_tank and any prop it's parented to, and check line of
        // sight to the point Trace to the point. If an opaque trace doesn't
        // reach the point, that means the beam hit something closer, (including
        // a blockLOS), so try again.
        CTraceFilterSkipTwoEntities traceFilter(this, GetParent(),
                                                COLLISION_GROUP_NONE);
        AI_TraceLine(vecBarrelEnd, vecTest, MASK_BLOCKLOS_AND_NPCS,
                     &traceFilter, &trLOS);
      ...
      if (bHarass) {
        m_flTimeNextSweep =
            gpGlobals->curtime + random->RandomFloat(0.25f, 0.75f);
      } else {
...
// adjusted angle.
//---------------------------------------------------------
void CFuncTankCombineCannon::Fire(int bulletCount, const Vector &barrelEnd,
                                  const Vector &forward, CBaseEntity *pAttacker,
                                  bool bIgnoreSpread) {
  ...
  BaseClass::Fire(bulletCount, barrelEnd, vecAdjustedForward, pAttacker,
                  bIgnoreSpread);

  // Turn off the beam and tell it to stay off for a bit. We want it to look
  // like the beam became the ion cannon 'rail gun' effect.
  DestroyBeam();
...
//---------------------------------------------------------
//---------------------------------------------------------
void CFuncTankCombineCannon::MakeTracer(const Vector &vecTracerSrc,
  ...
  if (AI_IsSinglePlayer()) {
    ...
    Vector vecNearestPoint =
        PointOnLineNearestPoint(vecTracerSrc, tr.endpos, vecPlayer);

    float flDist = vecPlayer.DistTo(vecNearestPoint);

    if (flDist >= 10.0f && flDist <= 120.0f) {
      // Don't shake the screen if we're hit (within 10 inches), but do shake if
      // a shot otherwise comes within 10 feet.
      UTIL_ScreenShake(vecNearestPoint, 10, 60, 0.3, 120.0f, SHAKE_START,
                       false);
    }
  }

  // Send the railgun effect
  DispatchParticleEffect("Weapon_Combine_Ion_Cannon", vecTracerSrc, tr.endpos,
                         vec3_angle, NULL);
}
