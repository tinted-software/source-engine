...
ConVar fire_extinguisher_strength("fire_extinguisher_strength",
                                  "0.97"); // TODO: Stub for real numbers
ConVar fire_extinguisher_explode_radius("fire_extinguisher_explode_radius",
                                        "256");
ConVar fire_extinguisher_explode_strength("fire_extinguisher_explode_strength",
                                          "0.3"); // TODO: Stub for real numbers
...
void CWeaponExtinguisher::Event_Killed(const CTakeDamageInfo &info) {
  ...
  if (AR2Explosion *pExplosion =
          AR2Explosion::CreateAR2Explosion(GetAbsOrigin())) {
  ...
  te->Explosion(filter, 0.0, &GetAbsOrigin(), g_sModelIndexFireball, 2.0, 15,
                TE_EXPLFLAG_NONE, 250, 100);

  // Put out fire in a radius
  FireSystem_ExtinguishInRadius(GetAbsOrigin(),
                                fire_extinguisher_explode_radius.GetInt(),
...
void CWeaponExtinguisher::ItemPostFrame(void) {
  ...
  if (pOwner->m_nButtons & IN_ATTACK) {
    ...
    // FIXME: This needs to be adjusted so the server collision matches the
    // visuals on the client
    ...
    trace_t tr;
    UTIL_TraceLine(vMuzzlePos, vTestPos, MASK_SHOT, this, COLLISION_GROUP_NONE,
                   &tr);
    ...
    if (fire_extinguisher_debug.GetInt()) {
      ...
      NDebugOverlay::Box(vMuzzlePos, Vector(-1, -1, -1), Vector(1, 1, 1), 0, 0,
                         128, false, 0.0f);
      NDebugOverlay::Box(tr.endpos, Vector(-2, -2, -2), Vector(2, 2, 2), 0, 0,
                         128, false, 0.0f);
      NDebugOverlay::Box(tr.endpos, Vector(-radius, -radius, -radius),
                         Vector(radius, radius, radius), 0, 0, 255, false,
                         0.0f);
    }
  } else {
    StopJet();
...
//			value -
//-----------------------------------------------------------------------------
void CExtinguisherCharger::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
