...
// Output  :
//------------------------------------------------------------------------------
void CWeapon_Tripwire::SetPickupTouch(void) { SetTouch(TripwireTouch); }
...
void CWeapon_Tripwire::TripwireTouch(CBaseEntity *pOther) {
  ...
  if (pOther->GetFlags() & FL_CLIENT) {
    ...
    CWeapon_Tripwire *oldWeapon =
        (CWeapon_Tripwire *)pBCC->Weapon_OwnsThisType(GetClassname());
    if (oldWeapon != this) {
      UTIL_Remove(this);
    } else {
  ...
  if (tr.fraction < 1.0) {
    ...
    if (pEntity && !(pEntity->GetFlags() & FL_CONVEYOR)) {
      ...
      CBaseEntity *pEnt = CBaseEntity::Create(
          "npc_tripwire", tr.endpos + tr.plane.normal * 3, angles, NULL);
