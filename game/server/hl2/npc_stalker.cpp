...
void CNPC_Stalker::PrescheduleThink() {
  if (gpGlobals->curtime > m_flNextBreatheSoundTime) {
    ...
    m_flNextBreatheSoundTime =
        gpGlobals->curtime + 3.0 + random->RandomFloat(0.0, 5.0);
  }
}
...
bool CNPC_Stalker::IsValidEnemy(CBaseEntity *pEnemy) {
  ...
  if (enemyClass == CLASS_PLAYER || enemyClass == CLASS_PLAYER_ALLY ||
      enemyClass == CLASS_PLAYER_ALLY_VITAL) {
...
void CNPC_Stalker::Spawn(void) {
  ...
  m_flFieldOfView = 0.1; // indicates the width of this NPC's forward view cone
  ...
  m_flNextBreatheSoundTime =
      gpGlobals->curtime + random->RandomFloat(0.0, 10.0);
...
void CNPC_Stalker::UpdateAttackBeam(void) {
  ...
  if (pEnemy) {
    if (gpGlobals->curtime > m_fBeamEndTime) {
      ...
      trace_t tr;
      AI_TraceLine(vecSrc, vecSrc + m_vLaserDir * MAX_STALKER_FIRE_RANGE,
...
void CNPC_Stalker::CalcBeamPosition(void) {
  ...
  if (GetEnemy()) {
    ...
    m_vLaserDir.x +=
        5 * noiseScale * sin(m_fNoiseModX * gpGlobals->curtime + m_fNoiseModX);
    m_vLaserDir.y +=
        5 * noiseScale * sin(m_fNoiseModY * gpGlobals->curtime + m_fNoiseModY);
    m_vLaserDir.z +=
        5 * noiseScale * sin(m_fNoiseModZ * gpGlobals->curtime + m_fNoiseModZ);
  }
}

void CNPC_Stalker::StartAttackBeam(void) {
  if (m_fBeamEndTime > gpGlobals->curtime ||
  ...
  if (!m_pBeam) {
    ...
    case STALKER_BEAM_LOW:
      ...
      m_pLightGlow =
          CSprite::SpriteCreate("sprites/redglow1.vmt", GetAbsOrigin(), FALSE);
      break;
    case STALKER_BEAM_MED:
      m_pBeam->SetColor(255, 50, 0);
      m_pLightGlow = CSprite::SpriteCreate("sprites/orangeglow1.vmt",
                                           GetAbsOrigin(), FALSE);
      break;
    case STALKER_BEAM_HIGH:
      m_pBeam->SetColor(255, 150, 0);
      m_pLightGlow = CSprite::SpriteCreate("sprites/yellowglow1.vmt",
                                           GetAbsOrigin(), FALSE);
      break;
    }
    ...
    // ----------------------------
    m_pLightGlow->SetTransparency(kRenderGlow, 255, 200, 200, 0,
...
void CNPC_Stalker::DrawAttackBeam(void) {
  ...
  Vector vecSrc = LaserStartPosition(GetAbsOrigin());
  trace_t tr;
  AI_TraceLine(vecSrc, vecSrc + m_vLaserDir * MAX_STALKER_FIRE_RANGE, MASK_SHOT,
...
// Output :
//-----------------------------------------------------------------------------
bool CNPC_Stalker::InnateWeaponLOSCondition(const Vector &ownerPos,
...
void CNPC_Stalker::HandleAnimEvent(animevent_t *pEvent) {
  ...
  case STALKER_AE_MELEE_HIT: {
    ...
    pHurt = CheckTraceHullAttack(32, Vector(-16, -16, -16), Vector(16, 16, 16),
                                 sk_stalker_melee_dmg.GetFloat(), DMG_SLASH);
...
void CNPC_Stalker::AddZigZagToPath(void) {
  ...
  if (waypointDir.LengthSqr() > ZIG_ZAG_SIZE) {
    ...
    // Now make sure that we can still get to the zigzag position and the
    // waypoint
    AIMoveTrace_t moveTrace1, moveTrace2;
    GetMoveProbe()->MoveLimit(NAV_GROUND, GetAbsOrigin(), zigZagPos,
                              MASK_NPCSOLID, NULL, &moveTrace1);
    GetMoveProbe()->MoveLimit(NAV_GROUND, zigZagPos, waypointPos, MASK_NPCSOLID,
                              NULL, &moveTrace2);
...
DEFINE_SCHEDULE(
    ...
    "		COND_NEW_ENEMY"
    "		COND_ENEMY_DEAD"
    "		COND_ENEMY_OCCLUDED" // Don't break on this.  Keep shooting at
                                     // last location
)
...
DEFINE_SCHEDULE(SCHED_STALKER_PATROL,
                ...
                "	0"
                "		TASK_WAIT				"
                "		0.5" // This makes them look a bit more
                                     // vigilant, instead of INSTANTLY
                                     // patrolling after some other action.
                "		TASK_WAIT_RANDOM			"
                "	0.5"
                "		TASK_WANDER				"
                "		18000600"
