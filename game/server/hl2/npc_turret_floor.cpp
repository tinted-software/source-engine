...
DEFINE_FIELD(m_iAmmoType, FIELD_INTEGER),
    ...
    DEFINE_THINKFUNC(Retire), DEFINE_THINKFUNC(Deploy),
    DEFINE_THINKFUNC(ActiveThink), DEFINE_THINKFUNC(SearchThink),
    DEFINE_THINKFUNC(AutoSearchThink), DEFINE_THINKFUNC(TippedThink),
    DEFINE_THINKFUNC(InactiveThink), DEFINE_THINKFUNC(SuppressThink),
    DEFINE_THINKFUNC(DisabledThink), DEFINE_THINKFUNC(SelfDestructThink),
...
CNPC_FloorTurret::CNPC_FloorTurret(void)
    : m_bActive(false), m_hEyeGlow(NULL), m_hLaser(NULL), m_iAmmoType(-1),
      m_bAutoStart(false), m_flPingTime(0.0f), m_flNextActivateSoundTime(0.0f),
      m_bCarriedByPlayer(false), m_bUseCarryAngles(false),
      m_flPlayerDropTime(0.0f), m_flShotTime(0.0f), m_flLastSight(0.0f),
      m_bBlinkState(false), m_flThrashTime(0.0f), m_pMotionController(NULL),
      m_bEnabled(false), m_bSelfDestructing(false) {
...
void CNPC_FloorTurret::Precache(void) {
  ...
  pModelName =
      (pModelName && pModelName[0] != '\0') ? pModelName : FLOOR_TURRET_MODEL;
...
void CNPC_FloorTurret::Spawn(void) {
  ...
  const char *pModelName = STRING(GetModelName());
  SetModel((pModelName && pModelName[0] != '\0') ? pModelName
                                                 : FLOOR_TURRET_MODEL);

  // If we're a citizen turret, we use a different skin
  if (IsCitizenTurret()) {
    if (m_iKeySkin == 0) { // select a "random" skin -- rather than being truly
                           // random, use a static variable
      // to cycle through them evenly. The static won't be saved across
      // save/load, but frankly I don't care so much about that. m_nSkin =
    ...
    } else { // at least make sure that it's in the right range
...
void CNPC_FloorTurret::Activate(void) {
  ...
  // Force the eye state to the current state so that our glows are recreated
  // after transitions
...
// Purpose:
//-----------------------------------------------------------------------------
void CNPC_FloorTurret::OnPhysGunPickup(CBasePlayer *pPhysGunUser,
  ...
  if (reason != PUNTED_BY_CANNON) {
    ...
    // Correct our angles only if we're not upright or we're mostly behind the
    // turret
...
// Purpose:
//-----------------------------------------------------------------------------
void CNPC_FloorTurret::OnPhysGunDrop(CBasePlayer *pPhysGunUser,
...
// Purpose:
//-----------------------------------------------------------------------------
bool CNPC_FloorTurret::OnAttemptPhysGunPickup(CBasePlayer *pPhysGunUser,
                                              PhysGunPickup_t reason) {
  // Prevent players pulling enemy turrets from afar if they're in front of the
  // turret
...
// Purpose:
//-----------------------------------------------------------------------------
bool CNPC_FloorTurret::HandleInteraction(int interactionType, void *data,
                                         CBaseCombatCharacter *sourceEnt) {
  if (interactionType == g_interactionCombineBash) {
    // We've been bashed by a combine soldier. Remember who it was, if we
    // haven't got an active kicker
...
bool CNPC_FloorTurret::UpdateFacing(void) {
  ...
  if (g_debug_turret.GetBool()) {
    ...
    NDebugOverlay::Cross3D(vecMuzzle, -Vector(2, 2, 2), Vector(2, 2, 2), 255,
                           255, 0, false, 0.05);
    NDebugOverlay::Cross3D(vecMuzzle + (vecMuzzleDir * 256), -Vector(2, 2, 2),
                           Vector(2, 2, 2), 255, 255, 0, false, 0.05);
    NDebugOverlay::Line(vecMuzzle, vecMuzzle + (vecMuzzleDir * 256), 255, 255,
                        0, false, 0.05);

    NDebugOverlay::Cross3D(vecMuzzle, -Vector(2, 2, 2), Vector(2, 2, 2), 255, 0,
                           0, false, 0.05);
    NDebugOverlay::Cross3D(vecMuzzle + (vecGoalDir * 256), -Vector(2, 2, 2),
                           Vector(2, 2, 2), 255, 0, 0, false, 0.05);
    NDebugOverlay::Line(vecMuzzle, vecMuzzle + (vecGoalDir * 256), 255, 0, 0,
                        false, 0.05);
  }
  ...
  // Update pitch
  float flDiff = AngleNormalize(
      UTIL_ApproachAngle(vecGoalLocalAngles.x, 0.0, 0.05f * MaxYawSpeed()));

  SetPoseParameter(m_poseAim_Pitch,
                   GetPoseParameter(m_poseAim_Pitch) + (flDiff / 1.5f));
  ...
  // Update yaw
  flDiff = AngleNormalize(
      UTIL_ApproachAngle(vecGoalLocalAngles.y, 0.0, 0.05f * MaxYawSpeed()));

  SetPoseParameter(m_poseAim_Yaw,
                   GetPoseParameter(m_poseAim_Yaw) + (flDiff / 1.5f));

  if (fabs(flDiff) > 0.1f) {
    bMoved = true;
  }

  // You're going to make decisions based on this info.  So bump the bone cache
  // after you calculate everything
  InvalidateBoneCache();
...
//-----------------------------------------------------------------------------
// Purpose: Turret will continue to fire on a target's position when it loses
// sight of it
//-----------------------------------------------------------------------------
void CNPC_FloorTurret::SuppressThink(void) {
  ...
  if (gpGlobals->curtime > m_flLastSight) {
    ...
    if (m_spawnflags & SF_FLOOR_TURRET_FASTRETIRE) {
      // Retire quickly in this case. (The case where we saw the player, but he
      // hid again).
  ...
  // Draw debug info
  if (g_debug_turret.GetBool()) {
    NDebugOverlay::Cross3D(vecMid, -Vector(2, 2, 2), Vector(2, 2, 2), 0, 255, 0,
                           false, 0.05);
    NDebugOverlay::Cross3D(vecMidEnemy, -Vector(2, 2, 2), Vector(2, 2, 2), 0,
                           255, 0, false, 0.05);
...
void CNPC_FloorTurret::ActiveThink(void) {
  ...
  // the muzzle can be on the other side of the wall, where it's then able to
  // see and shoot at targets. This check ensures that nothing has come between
  // the center of the turret and the muzzle.
  if (bEnemyVisible) {
    trace_t tr;
    Vector vecCenter;
    CollisionProp()->CollisionToWorldSpace(Vector(0, 0, 52), &vecCenter);
    UTIL_TraceLine(vecCenter, vecMid, MASK_SHOT, this, COLLISION_GROUP_NONE,
                   &tr);
    if (tr.fraction != 1.0) {
      bEnemyVisible = false;
    }
  ...
  if (g_debug_turret.GetBool()) {
    NDebugOverlay::Cross3D(vecMid, -Vector(2, 2, 2), Vector(2, 2, 2), 0, 255, 0,
                           false, 0.05);
    NDebugOverlay::Cross3D(GetEnemy()->WorldSpaceCenter(), -Vector(2, 2, 2),
                           Vector(2, 2, 2), 0, 255, 0, false, 0.05);
    NDebugOverlay::Line(vecMid, GetEnemy()->WorldSpaceCenter(), 0, 255, 0,
                        false, 0.05);
  ...
  if (g_debug_turret.GetBool()) {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      NDebugOverlay::Line(vecMuzzle, vecMuzzle + (vecMaxDownPitch * 256), 255,
                          255, 255, false, 0.1);
    }
  }
...
void CNPC_FloorTurret::SearchThink(void) {
  ...
  m_vecGoalAngles.x =
      GetAbsAngles().x + (sin(gpGlobals->curtime * 1.0f) * 15.0f);
  m_vecGoalAngles.y =
      GetAbsAngles().y + (sin(gpGlobals->curtime * 2.0f) * 60.0f);
...
void CNPC_FloorTurret::TippedThink(void) {
  ...
  if (gpGlobals->curtime < m_flThrashTime) {
    if (m_flShotTime < gpGlobals->curtime) {
      if (m_spawnflags & SF_FLOOR_TURRET_OUT_OF_AMMO) {
        DryFire();
      } else if (IsCitizenTurret() ==
                 false) // Citizen turrets don't wildly fire
      {
        Vector vecMuzzle, vecMuzzleDir;
        UpdateMuzzleMatrix();
        MatrixGetColumn(m_muzzleToWorld, 0, vecMuzzleDir);
...
void CNPC_FloorTurret::InactiveThink(void) {
  ...
  if (IsCitizenTurret()) {
    // Blink if we have ammo or our current blink is "on" and we need to turn it
    // off again
...
bool CNPC_FloorTurret::PreThink(turretState_e state) {
  ...
  if (!IsBeingCarriedByPlayer()) {
    if (OnSide() == false) {
      // If I still haven't fallen over after an NPC has tried to knock me down,
      // let them know
      if (m_hLastNPCToKickMe && m_flKnockOverFailedTime < gpGlobals->curtime) {
        m_hLastNPCToKickMe->DispatchInteraction(
            g_interactionTurretStillStanding, NULL, this);
        m_hLastNPCToKickMe = NULL;
      }
      ...
      if (g_debug_turret.GetBool()) {
        ...
        NDebugOverlay::Line(GetAbsOrigin() + (up * 32),
                            GetAbsOrigin() + (up * 128), 0, 255, 0, false,
                            2.0f);
        NDebugOverlay::Cross3D(GetAbsOrigin() + (up * 32), -Vector(2, 2, 2),
                               Vector(2, 2, 2), 0, 255, 0, false, 2.0f);
        NDebugOverlay::Cross3D(GetAbsOrigin() + (up * 128), -Vector(2, 2, 2),
                               Vector(2, 2, 2), 0, 255, 0, false, 2.0f);
      }
    } else {
      ...
      if (g_debug_turret.GetBool()) {
        ...
        NDebugOverlay::Line(GetAbsOrigin() + (up * 32),
                            GetAbsOrigin() + (up * 128), 255, 0, 0, false,
                            2.0f);
        NDebugOverlay::Cross3D(GetAbsOrigin() + (up * 32), -Vector(2, 2, 2),
                               Vector(2, 2, 2), 255, 0, 0, false, 2.0f);
        NDebugOverlay::Cross3D(GetAbsOrigin() + (up * 128), -Vector(2, 2, 2),
                               Vector(2, 2, 2), 255, 0, 0, false, 2.0f);
      }
...
void CNPC_FloorTurret::SetEyeState(eyeState_t state) {
  ...
  if (!m_hEyeGlow) {
    ...
    m_hEyeGlow = CSprite::SpriteCreate(FLOOR_TURRET_GLOW_SPRITE,
                                       GetLocalOrigin(), false);
    if (!m_hEyeGlow)
      return;

    m_hEyeGlow->SetTransparency(kRenderWorldGlow, 255, 0, 0, 128,
  ...
  if (IsCitizenTurret() &&
      HasSpawnFlags(SF_FLOOR_TURRET_OUT_OF_AMMO) == false && m_hLaser == NULL) {
...
void CNPC_FloorTurret::Enable(void) {
  ...
  // if the turret is flagged as an autoactivate turret, re-enable its ability
  // open self.
...
//-----------------------------------------------------------------------------
void CNPC_FloorTurret::InputToggle(inputdata_t &inputdata) { Toggle(); }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CNPC_FloorTurret::InputEnable(inputdata_t &inputdata) { Enable(); }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CNPC_FloorTurret::InputDisable(inputdata_t &inputdata) { Disable(); }
...
//-----------------------------------------------------------------------------
void CNPC_FloorTurret::ToggleUse(CBaseEntity *pActivator, CBaseEntity *pCaller,
...
int CNPC_FloorTurret::OnTakeDamage(const CTakeDamageInfo &info) {
  ...
  // Manually apply vphysics because AI_BaseNPC takedamage doesn't call back to
  ...
  if (IsAlive() && m_bEnabled && m_bAutoStart &&
      GetActivity() == ACT_FLOOR_TURRET_CLOSED_IDLE &&
...
// Purpose:
//-----------------------------------------------------------------------------
Vector CNPC_FloorTurret::GetAttackSpread(CBaseCombatWeapon *pWeapon,
                                         CBaseEntity *pTarget) {
...
//			tricky realm of player avoidance.  Since we don't go
//through the 			normal NPC thinking but we ARE an NPC (...) we miss a bunch of
//			book keeping.  This means we can become invisible and
//then never 			reappear.
//-----------------------------------------------------------------------------
...
void CNPC_FloorTurret::BreakThink(void) {
  ...
  // K-boom
  RadiusDamage(CTakeDamageInfo(this, this, 15.0f, DMG_BLAST), vecOrigin,
               (10 * 12), CLASS_NONE, this);

  EmitSound("NPC_FloorTurret.Destruct");

  breakablepropparams_t params(GetAbsOrigin(), GetAbsAngles(), vec3_origin,
                               RandomAngularImpulse(-800.0f, 800.0f));
  params.impactEnergyScale = 1.0f;
  params.defCollisionGroup = COLLISION_GROUP_INTERACTIVE;

  // no damage/damage force? set a burst of 100 for some movement
  params.defBurstScale = 100;
  PropBreakableCreateAll(GetModelIndex(), VPhysicsGetObject(), params, this, -1,
                         true);
  ...
  for (int i = 0; i < 4; i++) {
    ...
    te->BreakModel(filter, 0.0, vecOrigin, GetAbsAngles(), Vector(40, 40, 40),
                   gibVelocity, iModelIndex, 150, 4, 2.5, BREAK_METAL);
  }
...
void CNPC_FloorTurret::SelfDestructThink(void) {
  ...
  float flDestructPerc = clamp((gpGlobals->curtime - m_flDestructStartTime) /
                                   SELF_DESTRUCT_DURATION,
                               0.0f, 1.0f);

  // Figure out when our next beep should occur
  float flBeepTime =
      SELF_DESTRUCT_BEEP_MAX_DELAY +
      ((SELF_DESTRUCT_BEEP_MIN_DELAY - SELF_DESTRUCT_BEEP_MAX_DELAY) *
       flDestructPerc);
  ...
  if (gpGlobals->curtime > (m_flPingTime + flBeepTime)) {
    ...
    float flBeepPitch =
        SELF_DESTRUCT_BEEP_MIN_PITCH +
        ((SELF_DESTRUCT_BEEP_MAX_PITCH - SELF_DESTRUCT_BEEP_MIN_PITCH) *
         flDestructPerc);
    ...
    m_vecGoalAngles.x =
        GetAbsAngles().x +
        random->RandomFloat(-60 * flDestructPerc, 60 * flDestructPerc);
    m_vecGoalAngles.y =
        GetAbsAngles().y +
        random->RandomFloat(-60 * flDestructPerc, 60 * flDestructPerc);
  }
...
DEFINE_FIELD(m_bEnabled, FIELD_BOOLEAN),
    ...
    DEFINE_FIELD(m_localTestAxis, FIELD_VECTOR), DEFINE_PHYSPTR(m_pController),
...
CTurretTipController::Simulate(IPhysicsMotionController *pController,
  ...
  if (m_pParentTurret->WasJustDroppedByPlayer()) {
    ...
    if ((vecVelocity.LengthSqr() >
         CNPC_FloorTurret::fMaxTipControllerVelocity) ||
        (angImpulse.LengthSqr() >
  ...
  angular = ComputeRotSpeedToAlignAxes(m_localTestAxis, currentLocalTargetAxis,
                                       angVel, 1.0, invDeltaTime * invDeltaTime,
                                       flAngularLimit * invDeltaTime);
...
// Purpose:
//-----------------------------------------------------------------------------
void CTurretTipController::Enable(bool state) { m_bEnabled = state; }
...
float CTurretTipController::SuspendedTill(void) { return m_flSuspendTime; }
