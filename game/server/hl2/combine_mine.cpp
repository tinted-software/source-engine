...
enum {
  ...
  MINE_STATE_TRIGGERED, // No turning back. I'm going to explode when I touch
                        // something.
...
DEFINE_THINKFUNC(ExplodeThink), DEFINE_ENTITYFUNC(ExplodeTouch),
    DEFINE_THINKFUNC(SearchThink), DEFINE_THINKFUNC(BounceThink),
    DEFINE_THINKFUNC(SettleThink), DEFINE_THINKFUNC(CaptiveThink),
...
void CBounceBomb::Spawn() {
  ...
  // default to a different skin for cavern turrets (unless explicitly
  // overridden)
  if (m_iModification == MINE_MODIFICATION_CAVERN) {
    ...
    if (!bFoundSkin) {
      // select a random skin for the mine. Actually, we'll cycle through the
      // available skins using a static variable to provide better distribution.
      // The static isn't saved but really it's only cosmetic.
      static unsigned int nextSkin = MINE_CITIZEN_SKIN_MIN;
      m_nSkin = nextSkin;
      // increment the skin for next time
      nextSkin = (nextSkin >= MINE_CITIZEN_SKIN_MAX) ? MINE_CITIZEN_SKIN_MIN
                                                     : nextSkin + 1;
    }
...
void CBounceBomb::OnRestore() {
  BaseClass::OnRestore();
  if (gpGlobals->eLoadType == MapLoad_Transition && !m_hSprite &&
      m_LastSpriteColor.GetRawColor() != 0) {
    UpdateLight(true, m_LastSpriteColor.r(), m_LastSpriteColor.g(),
                m_LastSpriteColor.b(), m_LastSpriteColor.a());
  }
...
bool CBounceBomb::IsValidLocation() {
  ...
  if (pHint) {
    ...
    while (pEntity) {
      if (pEntity->m_iClassname == m_iClassname && pEntity != this) {
        ...
        if (vecMyPosition.DistToSqr(pEntity->GetAbsOrigin()) <
            MINE_MIN_PROXIMITY_SQR) {
  ...
  if (pAvoidObject) {
    ...
    // Now add some force in the direction that takes us away from the
    // inhibitor.
...
void CBounceBomb::BounceThink() {
  ...
  if (pPhysicsObject != NULL) {
    ...
    // Figure out how much headroom the mine has, and hop to within a few inches
    // of that.
    trace_t tr;
    UTIL_TraceLine(GetAbsOrigin(),
                   GetAbsOrigin() + Vector(0, 0, MINE_MAX_JUMP_HEIGHT),
                   MASK_SHOT, this, COLLISION_GROUP_INTERACTIVE, &tr);

    float height;

    if (tr.m_pEnt && tr.m_pEnt->VPhysicsGetObject()) {
      // Physics object resting on me. Jump as hard as allowed to try to knock
      // it away.
    ...
    pPhysicsObject->ApplyTorqueCenter(AngularImpulse(
        random->RandomFloat(5, 25), random->RandomFloat(5, 25), 0));
...
//---------------------------------------------------------
// A different bounce behavior for the citizen-modified mine. Detonates at the
// top of its apex, and does not attempt to track enemies.
//---------------------------------------------------------
void CBounceBomb::CavernBounceThink() {
  ...
  if (pPhysicsObject != NULL) {
    ...
    if (m_hNearestNPC) {
      ...
      pPhysicsObject->ApplyForceCenter(vecPredict *
                                       (pPhysicsObject->GetMass() * 0.65f));
    }

    pPhysicsObject->ApplyTorqueCenter(
        AngularImpulse(random->RandomFloat(15, 40), random->RandomFloat(15, 40),
                       random->RandomFloat(30, 60)));
  ...
  if (!m_bDisarmed) {
    if (VPhysicsGetObject()->IsAsleep() &&
      ...
      UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() - Vector(0, 0, 1024),
                     MASK_SHOT | CONTENTS_GRATE, this, COLLISION_GROUP_NONE,
                     &tr);
      ...
      ballsocket.InitWithCurrentObjectState(
          g_PhysWorldObject, VPhysicsGetObject(), GetAbsOrigin());
      m_pConstraint = physenv->CreateBallsocketConstraint(
          g_PhysWorldObject, VPhysicsGetObject(), NULL, ballsocket);
      CloseHooks();
...
//---------------------------------------------------------
//---------------------------------------------------------
void CBounceBomb::UpdateLight(bool bTurnOn, unsigned int r, unsigned int g,
  ...
  if (bTurnOn) {
    ...
    if (!m_hSprite.Get()) {
      ...
      // Light isn't on.
      m_hSprite = CSprite::SpriteCreate("sprites/glow01.vmt",
                                        GetAbsOrigin() + up * 10.0f, false);
...
bool CBounceBomb::IsFriend(CBaseEntity *pEntity) {
  ...
  if (classify == CLASS_METROPOLICE || classify == CLASS_COMBINE ||
      classify == CLASS_MILITARY || classify == CLASS_COMBINE_HUNTER ||
...
void CBounceBomb::SearchThink() {
  ...
  if (flNearestNPCDist <= BOUNCEBOMB_DETONATE_RADIUS &&
      !IsFriend(m_hNearestNPC)) {
    if (m_bBounce) {
      SetMineState(MINE_STATE_TRIGGERED);
    } else {
      // Don't pop up in the air, just explode if the NPC gets closer than
      // explode radius.
      SetThink(&CBounceBomb::ExplodeThink);
...
void CBounceBomb::ExplodeTouch(CBaseEntity *pOther) {
  ...
  // detonate against anything that's probably not an NPC (such as physics
  // props)
  if (m_flIgnoreWorldTime > gpGlobals->curtime &&
      !pOther->MyCombatCharacterPointer()) {
    return;
  }
...
void CBounceBomb::ExplodeThink() {
  ...
  if (m_iModification == MINE_MODIFICATION_CAVERN) {
    ExplosionCreate(GetAbsOrigin(), GetAbsAngles(),
                    (pThrower) ? pThrower : this, BOUNCEBOMB_EXPLODE_DAMAGE,
...
void CBounceBomb::OpenHooks(bool bSilent) {
  ...
  if (VPhysicsGetObject()) {
    // It's possible to not have a valid physics object here, since this
    // function doubles as an initialization function.
...
//---------------------------------------------------------
//---------------------------------------------------------
void CBounceBomb::OnPhysGunDrop(CBasePlayer *pPhysGunUser,
...
//---------------------------------------------------------
//---------------------------------------------------------
void CBounceBomb::OnPhysGunPickup(CBasePlayer *pPhysGunUser,
  ...
  if (reason != PUNTED_BY_CANNON) {
    if (m_iMineState == MINE_STATE_ARMED) {
      ...
      // Try to scatter NPCs without panicking them. Make a move away sound up
      // around their ear level.
      CSoundEnt::InsertSound(SOUND_MOVE_AWAY, GetAbsOrigin() + Vector(0, 0, 60),
                             32.0f, 0.2f);
      return;
    } else {
  ...
  if (reason == PUNTED_BY_CANNON) {
    ...
    // Since being punted causes the mine to flip, sometimes it 'catches an
    // edge' and ends up touching the ground from whence it came, exploding
    // instantly. This little stunt prevents that by ignoring world collisions
