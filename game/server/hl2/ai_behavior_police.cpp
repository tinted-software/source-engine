...
// Purpose:
//-----------------------------------------------------------------------------
void CAI_PolicingBehavior::HostSpeakSentence(const char *pSentence,
...
void CAI_PolicingBehavior::GatherConditions(void) {
  ...
  float flDistSqr =
      (m_hPoliceGoal->WorldSpaceCenter() - pTarget->WorldSpaceCenter())
          .Length2DSqr();
  float radius = (m_hPoliceGoal->GetRadius() * PATROL_RADIUS_RATIO);
  float zDiff =
      fabs(m_hPoliceGoal->WorldSpaceCenter().z - pTarget->WorldSpaceCenter().z);
  ...
  if (m_bTargetIsHostile && m_flAggressiveTime < gpGlobals->curtime &&
...
void CAI_PolicingBehavior::AnnouncePolicing(void) {
  ...
  if (m_nNumWarnings <= 3) {
    HostSpeakSentence(pWarnings[m_nNumWarnings - 1], SENTENCE_PRIORITY_MEDIUM,
                      SENTENCE_CRITERIA_NORMAL);
  } else {
    // We loop at m_nNumWarnings == 4 for players who aren't moving
    // but still pissing us off, and we're not allowed to do anything about it.
    // (i.e. can't leave post) First two sentences sound pretty good, so
    // randomly pick one of them.
    int iSentence = RandomInt(0, 1);
    HostSpeakSentence(pWarnings[iSentence], SENTENCE_PRIORITY_MEDIUM,
                      SENTENCE_CRITERIA_NORMAL);
  }
}
...
CBaseEntity *CAI_PolicingBehavior::GetGoalTarget(void) {
  if (m_hPoliceGoal == NULL) {
    // NOTENOTE: This has been called before the behavior is actually active, or
    // the goal has gone invalid
    Assert(0);
    return NULL;
  }
...
void CAI_PolicingBehavior::StartTask(const Task_t *pTask) {
  ...
  case TASK_POLICE_GET_PATH_TO_HARASS_GOAL: {
    Vector harassDir =
        (m_hPoliceGoal->GetTarget()->WorldSpaceCenter() - WorldSpaceCenter());
    ...
    Vector harassPos =
        GetAbsOrigin() + (harassDir * (flDist - pTask->flTaskData));

    // Find a point on our policing radius to stand on
    if (IntersectInfiniteRayWithSphere(
            GetAbsOrigin(), harassDir, m_hPoliceGoal->GetAbsOrigin(),
            m_hPoliceGoal->GetRadius(), &flInter1, &flInter2)) {
      Vector vPos =
          m_hPoliceGoal->GetAbsOrigin() + harassDir * (MAX(flInter1, flInter2));

      // See how far away the default one is
      float testDist =
          UTIL_DistApprox2D(m_hPoliceGoal->GetAbsOrigin(), harassPos);
  ...
  case TASK_POLICE_GET_PATH_TO_POLICE_GOAL: {
    if (GetNavigator()->SetGoal(m_hPoliceGoal->GetAbsOrigin(),
                                pTask->flTaskData)) {
  ...
  case TASK_POLICE_ANNOUNCE_HARASS: {
    ...
    CSoundEnt::InsertSound(SOUND_MOVE_AWAY, GetAbsOrigin(), 256.0f, 2.0f,
                           GetOuter());
  }
    TaskComplete();
    break;
  ...
  case TASK_POLICE_FACE_ALONG_GOAL: {
    // We may have lost our police goal in the 2 seconds we wait before this
    // task
    if (m_hPoliceGoal) {
...
bool CAI_PolicingBehavior::ShouldKnockOutTarget(CBaseEntity *pTarget) {
  ...
  return m_hPoliceGoal->ShouldKnockOutTarget(pTarget->WorldSpaceCenter(),
                                             bVisible);
}
...
int CAI_PolicingBehavior::SelectSuppressSchedule(void) {
  ...
  if (m_bTargetIsHostile == false) {
    ...
    // We need to let the system pickup the new enemy and deal with it on the
    // next frame
...
int CAI_PolicingBehavior::SelectSchedule(void) {
  ...
  if (fabs(UTIL_AngleDiff(GetAbsAngles().y, m_hPoliceGoal->GetAbsAngles().y)) >
      15)
...
// Purpose:
//-----------------------------------------------------------------------------
int CAI_PolicingBehavior::SelectFailSchedule(
