...
public:
  ...
  void ImmolationDamage(const CTakeDamageInfo &info, const Vector &vecSrcIn,
...
void CWeaponImmolator::StartImmolating() {
  ...
  CSoundEnt::InsertSound(SOUND_DANGER, m_vecImmolatorTarget, 256, 5.0,
                         GetOwner());
}
...
//-----------------------------------------------------------------------------
bool CWeaponImmolator::WeaponLOSCondition(const Vector &ownerPos,
...
int CWeaponImmolator::WeaponRangeAttack1Condition(float flDot, float flDist) {
  ...
  } else if (flDot < 0.5f) // UNDONE: Why check this here? Isn't the AI checking
                           // this already?
  {
...
void CWeaponImmolator::Update() {
  ...
  trace_t tr;
  UTIL_TraceLine(vecSrc, vecSrc + vecAiming * MAX_TRACE_LENGTH, MASK_SHOT,
  ...
  if (tr.DidHitWorld()) {
    ...
    if (m_flBurnRadius > 64.0) {
      ImmolationDamage(CTakeDamageInfo(this, this, 1, DMG_BURN), tr.endpos,
                       m_flBurnRadius, CLASS_NONE);
    }
  } else {
...
void CWeaponImmolator::ImmolationDamage(const CTakeDamageInfo &info,
