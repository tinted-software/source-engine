...
int CNPC_Manhack::OnTakeDamage_Alive(const CTakeDamageInfo &info) {
  ...
  if (info.GetDamageType() & DMG_PHYSGUN) {
    ...
    //		 if fudging this vector a little bit could help me slam into a
    //physics object, 		 then make that adjustment. This is a simple heuristic. The
    //manhack will be 		 directed towards the physics object that is closest to
    //g_vecAttackDir
    //

    //		-Take 150% damage from club attacks. This makes crowbar duels
    //take two hits.
...
void CNPC_Manhack::Slice(CBaseEntity *pHitEntity, float flInterval,
  ...
  if (IsHeldByPhyscannon()) {
    ...
    flDamage =
        sk_combine_guard_health.GetFloat(); // the highest healthed fleshy enemy
...
void CNPC_Manhack::RunTask(const Task_t *pTask) {
  ...
  case TASK_MANHACK_MOVEAT_SAVEPOSITION: {
    ...
    //			NDebugOverlay::Line( GetAbsOrigin(), m_vSavePosition, 0,
    //255, 0, true, 0.1);
