...
#define SPOTLIGHT_HINT_SEARCH_DIST                                             \
  1000 // How far from self do I look for hints?
#define SPOTLIGHT_ENTITY_SEARCH_DIST                                           \
  100 // How far from spotlight do I look for entities?
#define SPOTLIGHT_ACTIVE_SEARCH_DIST                                           \
  200 // How far from spotlight do I look when already have entity
...
enum SpotlightFlags_t {
  ...
  BITS_SPOTLIGHT_SMOOTH_RETURN =
      0x00001000, // If out of range, don't pop back to position
};
...
CBaseEntity *CNPC_Spotlight::BestInspectTarget(void) {
  ...
  if (pBestEntity) {
    ...
    // If he has eluded me or isn't in the legal range of my spotligth reject
             ...
             !SpotlightIsPositionLegal(
                 GetEnemies()->LastKnownPosition(pBestEntity))) {
      pBestEntity = NULL;
    }
  }
  ...
  for (CEntitySphereQuery sphere(vSearchOrigin, SPOTLIGHT_ENTITY_SEARCH_DIST);
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_Spotlight::SetInspectTargetToEntity(CBaseEntity *pEntity,
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_Spotlight::SetInspectTargetToHint(CAI_Hint *pHintNode,
                                            float fInspectDuration) {
  ClearInspectTarget();
  ...
  trace_t tr;
  AI_TraceLine(vHintOrigin, vHintEnd, MASK_OPAQUE, this, COLLISION_GROUP_NONE,
               &tr);
  if (tr.fraction == 1.0) {
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_Spotlight::SetInspectTargetToPos(const Vector &vInspectPos,
                                           float fInspectDuration) {
  ClearInspectTarget();
  ...
  if (m_fSpotlightFlags & BITS_SPOTLIGHT_TRACK_ON) {
    ...
    if (pBestEntity) {
        ...
        if (GetEnemy() == NULL) {
          ...
          if (m_pSquad) {
            ...
            for (CAI_BaseNPC *pSquadMember = m_pSquad->GetFirstMember(&iter);
              ...
              if (pSquadMember->GetEnemy() != pBestEntity &&
                  !pSquadMember->HasCondition(COND_SEE_ENEMY)) {
      ...
      // If its not an enemy
        ...
        if (GetTarget() == NULL) {
          SetInspectTargetToEntity(pBestEntity,
                                   SPOTLIGHT_ENTITY_INSPECT_LENGTH);
        }
    ...
    if (GetEnemy() == NULL) {
      ...
      if (HaveInspectTarget() && gpGlobals->curtime > m_flInspectEndTime) {
        m_flNextEntitySearchTime =
            gpGlobals->curtime + SPOTLIGHT_ENTITY_INSPECT_DELAY;
        m_flNextHintSearchTime =
            gpGlobals->curtime + SPOTLIGHT_HINT_INSPECT_DELAY;
        ClearInspectTarget();
      }
      ...
      if (gpGlobals->curtime > m_flNextHintSearchTime && !HaveInspectTarget()) {
        SetHintNode(CAI_HintManager::FindHint(this, HINT_NONE, 0,
                                              SPOTLIGHT_HINT_SEARCH_DIST));

        if (GetHintNode()) {
          m_flNextHintSearchTime =
              gpGlobals->curtime + SPOTLIGHT_HINT_INSPECT_LENGTH;
    ...
    // -------------------------------------------------------
    if (GetTarget()) {
      if (!SpotlightIsPositionLegal(
              GetEnemies()->LastKnownPosition(GetTarget()))) {
        ClearInspectTarget();
...
bool CNPC_Spotlight::SpotlightIsPositionLegal(const Vector &vTestPos) {
  ...
  if (UTIL_AngleDistance(vTargetAngles[YAW], m_flYawCenter) > m_flYawRange) {
    return false;
  } else if (UTIL_AngleDistance(vTargetAngles[YAW], m_flYawCenter) <
             -m_flYawRange) {
    return false;
  }
  if (UTIL_AngleDistance(vTargetAngles[PITCH], m_flPitchCenter) >
      m_flPitchMax) {
    return false;
  } else if (UTIL_AngleDistance(vTargetAngles[PITCH], m_flPitchCenter) <
             m_flPitchMin) {
    return false;
  }
  return true;
...
Vector CNPC_Spotlight::SpotlightCurrentPos(void) {
  ...
  m_flPitchSpeed =
      (myPitchDecay * m_flPitchSpeed + (1 - myPitchDecay) * flNewPitchSpeed);
...
void CNPC_Spotlight::SpotlightUpdate(void) {
  ...
  // --------------------------------------------------------------
  Vector vTargetDir =
      (m_vSpotlightCurrentPos - m_pSpotlightTarget->GetAbsOrigin());
  ...
  m_pSpotlightTarget->m_vSpotlightDir = m_pSpotlightTarget->GetLocalOrigin() -
                                        m_pSpotlightTarget->m_vSpotlightOrg;
  ...
  float flNewWidth =
      m_flSpotlightGoalWidth * (flBeamLength / m_flSpotlightMaxLength);
  ...
  NDebugOverlay::Cross3D(m_vSpotlightCurrentPos, Vector(-5, -5, -5),
                         Vector(5, 5, 5), 0, 255, 0, true, 0.1);
  NDebugOverlay::Cross3D(m_vSpotlightTargetPos, Vector(-5, -5, -5),
                         Vector(5, 5, 5), 255, 0, 0, true, 0.1);
#endif
}
...
void CNPC_Spotlight::Spawn(void) {
  ...
  if (m_flSpotlightMaxLength <= 0) {
    DevMsg("CNPC_Spotlight::Spawn: Invalid spotlight length <= 0, setting to "
           "500\n");
  ...
  if (m_flSpotlightGoalWidth <= 0) {
    DevMsg(
        "CNPC_Spotlight::Spawn: Invalid spotlight width <= 0, setting to 10\n");
...
//-----------------------------------------------------------------------------
bool CNPC_Spotlight::HandleInteraction(int interactionType, void *data,
                                       CBaseCombatCharacter *sourceEnt) {
