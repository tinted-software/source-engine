...
void CNPC_CScanner::Spawn(void) {
  ...
  if (m_flSpotlightMaxLength <= 0) {
    DevMsg("CNPC_CScanner::Spawn: Invalid spotlight length <= 0, setting to "
           "500\n");
...
bool CNPC_CScanner::OverrideMove(float flInterval) {
  ...
  if (m_nFlyMode == SCANNER_FLY_DIVE) {
    ...
    // The original line of code was, due to the accidental use of '|' instead
    // of
    // '&', always true. Replacing with 'true' to suppress the warning without
    // changing the (long-standing) behavior.
    if (true) //! GetNavigator()->IsGoalActive() || (
