...
//-----------------------------------------------------------------------------
inline bool
CCombineDropshipContainer::ShouldTriggerDamageEffect(int nPrevHealth,
...
void CNPC_CombineDropship::Spawn(void) {
  ...
  case CRATE_SOLDIER:
    ...
    if (m_hContainer) {
      ...
      // NOTE: gun_ref must have the same position as gun_base, but rotates with
      // the gun
  ...
  case CRATE_APC: {
    m_soldiersToDrop = 0;
    m_hContainer =
        (CBaseAnimating *)gEntList.FindEntityByName(NULL, m_iszAPCVehicleName);
    if (!m_hContainer) {
...
void CNPC_CombineDropship::InputDropStrider(inputdata_t &inputdata) {
  if (!m_hContainer || !FClassnameIs(m_hContainer, "npc_strider")) {
    Warning("npc_combinedropship %s was told to drop Strider, but isn't "
            "carrying one!\n",
...
void CNPC_CombineDropship::InputDropAPC(inputdata_t &inputdata) {
  if (!m_hContainer || !FClassnameIs(m_hContainer, "prop_vehicle_apc")) {
    Warning("npc_combinedropship %s was told to drop APC, but isn't carrying "
            "one!\n",
            STRING(GetEntityName()));
    return;
  }
...
void CNPC_CombineDropship::InputPickup(inputdata_t &inputdata) {
  ...
  if (m_hContainer) {
    Warning("npc_combinedropship %s was told to pickup, but is already "
            "carrying something.\n",
            STRING(GetEntityName()));
    return;
  }
  ...
  if (iszTargetName == NULL_STRING) {
    Warning("npc_combinedropship %s tried to pickup with no specified pickup "
            "target.\n",
            STRING(GetEntityName()));
    return;
  }
...
#define ALTITUDE_CAP 512.0f
    ...
    // place danger sounds 1 foot above ground to get troops to scatter if they
    // are below dropship
    ...
    // now check to see if player is below us, if so, cause heat damage to them
    // (i.e. get them to move)
    trace_t tr;
    ...
    AI_TraceHull(vecBottom, pEndPoint, vecBBoxMin, vecBBoxMax, MASK_SOLID, this,
  ...
  case LANDING_UNLOADING: {
    // If we've got no specified takeoff time, we're still waiting for troops to
    // exit. Idle.
    if (!m_flTimeTakeOff) {
        ...
        */
        {
          ...
          if (pPhysicsObject) {
            pPhysicsObject->EnableMotion(true);
            pPhysicsObject->SetShadow(1e4, 1e4, false, false);
            pPhysicsObject->UpdateShadow(GetAbsOrigin(), GetAbsAngles(), false,
                                         0);
          }
...
void CNPC_CombineDropship::SpawnTroop(void) {
  ...
  pSequence->KeyValue("OnEndSequence",
                      UTIL_VarArgs("%s,NPCFinishDustoff,%s,0,-1",
                                   STRING(GetEntityName()),
