...
public:
  ...
  // this function only used in episodic.
    ...
    0 // FIXME: HandleInteraction() is no longer called now that base grenade
...
void CGrenadeFrag::CreateEffects(void) {
  ...
  if (m_pMainGlow != NULL) {
    ...
    m_pMainGlow->SetTransparency(kRenderGlow, 255, 255, 255, 200,
  ...
  if (!m_pGlowTrail)
    m_pGlowTrail = CSpriteTrail::SpriteTrailCreate("sprites/bluelaser1.vmt",
                                                   GetLocalOrigin(), false);

  if (m_pGlowTrail != NULL) {
    m_pGlowTrail->FollowEntity(this);
    m_pGlowTrail->SetAttachment(this, nAttachment);
    m_pGlowTrail->SetTransparency(kRenderTransAdd, 255, 0, 0, 255,
                                  kRenderFxNone);
...
void CGrenadeFrag::VPhysicsUpdate(IPhysicsObject *pPhysics) {
  ...
  // UNDONE: Hull won't work with hitboxes - hits outer hull.  But the whole
  // point of this test is to hit hitboxes.
#if 0
...
void CGrenadeFrag::OnPhysGunPickup(CBasePlayer *pPhysGunUser,
                                   PhysGunPickup_t reason) {
  SetThrower(pPhysGunUser);

#ifdef HL2MP
  SetTimer(FRAG_GRENADE_GRACE_TIME_AFTER_PICKUP,
           FRAG_GRENADE_GRACE_TIME_AFTER_PICKUP / 2);
...
void CGrenadeFrag::SetVelocity(const Vector &velocity,
...
int CGrenadeFrag::OnTakeDamage(const CTakeDamageInfo &inputInfo) {
  // Manually apply vphysics because BaseCombatCharacter takedamage doesn't call
  // back to CBaseEntity OnTakeDamage
  ...
  return BaseClass::OnTakeDamage(inputInfo);
    ...
    0 // FIXME: HandleInteraction() is no longer called now that base grenade
      // derives from CBaseAnimating
extern int g_interactionBarnacleVictimGrab; ///< usually declared in
                                            ///< ai_interactions.h but no reason
                                            ///< to haul all of that in here.
extern int g_interactionBarnacleVictimBite;
extern int g_interactionBarnacleVictimReleased;
bool CGrenadeFrag::HandleInteraction(int interactionType, void *data,
                                     CBaseCombatCharacter *sourceEnt) {
  // allow fragnades to be grabbed by barnacles.
  if (interactionType == g_interactionBarnacleVictimGrab) {
    // give the grenade another five seconds seconds so the player can have the
    // satisfaction of blowing up the barnacle with it
...
void CGrenadeFrag::InputSetTimer(inputdata_t &inputdata) {
  SetTimer(inputdata.value.Float(),
           inputdata.value.Float() - FRAG_GRENADE_WARN_TIME);
}
...
CBaseGrenade *Fraggrenade_Create(const Vector &position, const QAngle &angles,
                                 ...
                                 bool combineSpawned) {
  // Don't set the owner here, or the player can't interact with grenades he's
  // thrown
  CGrenadeFrag *pGrenade = (CGrenadeFrag *)CBaseEntity::Create(
      "npc_grenade_frag", position, angles, pOwner);
