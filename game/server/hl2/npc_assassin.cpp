...
void CNPC_Assassin::Spawn(void) {
  ...
  CapabilitiesClear();
  CapabilitiesAdd(bits_CAP_MOVE_CLIMB | bits_CAP_MOVE_GROUND |
                  bits_CAP_MOVE_JUMP);
  CapabilitiesAdd(bits_CAP_SQUAD | bits_CAP_USE_WEAPONS | bits_CAP_AIM_GUN |
  ...
  m_pEyeSprite =
      CSprite::SpriteCreate("sprites/redglow1.vmt", GetLocalOrigin(), false);

  if (m_pEyeSprite != NULL) {
    m_pEyeSprite->SetAttachment(this, attachment);
    m_pEyeSprite->SetTransparency(kRenderTransAdd, 255, 255, 255, 200,
                                  kRenderFxNone);
    m_pEyeSprite->SetScale(0.25f);
  }

  // Start up the eye trail
  m_pEyeTrail = CSpriteTrail::SpriteTrailCreate("sprites/bluelaser1.vmt",
                                                GetLocalOrigin(), false);

  if (m_pEyeTrail != NULL) {
    m_pEyeTrail->SetAttachment(this, attachment);
    m_pEyeTrail->SetTransparency(kRenderTransAdd, 255, 0, 0, 200,
                                 kRenderFxNone);
...
// Output :
//-----------------------------------------------------------------------------
bool CNPC_Assassin::IsJumpLegal(const Vector &startPos, const Vector &apex,
...
int CNPC_Assassin::RangeAttack2Conditions(float flDot, float flDist) {
  ...
  float lungeRange = GetSequenceMoveDist(
      SelectWeightedSequence((Activity)ACT_ASSASSIN_FLIP_FORWARD));
  ...
  trace_t tr;
  UTIL_TraceHull(GetAbsOrigin(), GetEnemy()->GetAbsOrigin(), GetHullMins(),
...
void CNPC_Assassin::FirePistol(int hand) {
  ...
  FireBullets(1, muzzlePos, muzzleDir, VECTOR_CONE_5DEGREES, 1024, bulletType,
              2);
...
void CNPC_Assassin::HandleAnimEvent(animevent_t *pEvent) {
  ...
  if (pEvent->event == AE_ASSASIN_KICK_HIT) {
    ...
    trace_t tr;
    UTIL_TraceHull(WorldSpaceCenter(), attackPos, -Vector(8, 8, 8),
                   Vector(8, 8, 8), MASK_SHOT_HULL, this, COLLISION_GROUP_NONE,
                   &tr);
...
//-----------------------------------------------------------------------------
// Purpose: Causes the assassin to prefer to run away, rather than towards her
// target
//-----------------------------------------------------------------------------
bool CNPC_Assassin::MovementCost(int moveType, const Vector &vecStart,
...
int CNPC_Assassin::SelectSchedule(void) {
  ...
  case NPC_STATE_COMBAT: {
    ...
    // Need to move
    if (/*(	HasCondition( COND_SEE_ENEMY ) && HasCondition(
           COND_ASSASSIN_ENEMY_TARGETTING_ME ) && random->RandomInt( 0, 32 ) ==
           0 && m_flNextFlipTime < gpGlobals->curtime ) )*/
        (m_nNumFlips > 0) ||
...
//-----------------------------------------------------------------------------
bool CNPC_Assassin::CanFlip(int flipType, Activity &activity,
                            const Vector *avoidPosition) {
  Vector testDir;
  ...
  if ((flipType != FLIP_BACKWARD) && (avoidPosition != NULL)) {
    UTIL_TraceLine((*avoidPosition), endPos, MASK_SHOT, this,
  ...
  /*
  UTIL_TraceHull( GetAbsOrigin(), endPos, NAI_Hull::Mins(m_eHull) + Vector( 0,
0, StepHeight() ), NAI_Hull::Maxs(m_eHull), MASK_NPCSOLID, this,
COLLISION_GROUP_NONE, &tr );
  ...
  if ( tr.fraction < 1.0f )
  {
          ...
          {
                  NDebugOverlay::BoxDirection( GetAbsOrigin(),
NAI_Hull::Mins(m_eHull) + Vector( 0, 0, StepHeight() ), NAI_Hull::Maxs(m_eHull)
+ Vector( testDist, 0, StepHeight() ), testDir, 255, 0, 0, true, 2.0f );
          }
...
#define NUM_STEPS 2
  ...
  for ( int i = 1; i <= NUM_STEPS; i++ )
  {
          ...
          UTIL_TraceHull( endPos, endPos - Vector( 0, 0, StepHeight() * 4.0f ),
NAI_Hull::Mins(m_eHull) + Vector( 0, 0, StepHeight() ), NAI_Hull::Maxs(m_eHull),
MASK_NPCSOLID, this, COLLISION_GROUP_NONE, &tr );
          ...
          if ( tr.fraction == 1.0f )
          {
                  if ( g_debug_assassin.GetBool() )
                  {
                          NDebugOverlay::BoxDirection( endPos,
NAI_Hull::Mins(m_eHull) + Vector( 0, 0, StepHeight() ), NAI_Hull::Maxs(m_eHull)
+ Vector( StepHeight() * 4.0f, 0, StepHeight() ), Vector(0,0,-1), 255, 0, 0,
true, 2.0f );
                  }
  ...
  if ( g_debug_assassin.GetBool() )
  {
          NDebugOverlay::BoxDirection( GetAbsOrigin(), NAI_Hull::Mins(m_eHull) +
Vector( 0, 0, StepHeight() ), NAI_Hull::Maxs(m_eHull) + Vector( testDist, 0,
StepHeight() ), testDir, 0, 255, 0, true, 2.0f );
  }
  */

  AIMoveTrace_t moveTrace;
  GetMoveProbe()->TestGroundMove(GetAbsOrigin(), endPos, MASK_NPCSOLID,
                                 AITGM_DEFAULT, &moveTrace);
...
void CNPC_Assassin::StartTask(const Task_t *pTask) {
  ...
  case TASK_ASSASSIN_EVADE: {
    ...
    const Vector *avoidPos =
        (GetEnemy() != NULL) ? &(GetEnemy()->GetAbsOrigin()) : NULL;
  ...
  case TASK_ASSASSIN_GET_PATH_TO_VANTAGE_POINT: {
    ...
    CAI_Hint *pHint =
        CAI_HintManager::FindHint(this, GetEnemy()->GetAbsOrigin(), &hint);
...
bool CNPC_Assassin::FValidateHintType(CAI_Hint *pHint) {
  ...
  case HINT_TACTICAL_ENEMY_DISADVANTAGED: {
    ...
    trace_t tr;
    UTIL_TraceLine(hintPos, GetEnemy()->BodyTarget(hintPos, true), MASK_SHOT,
...
DEFINE_SCHEDULE(
    ...
    "		TASK_SET_SCHEDULE					"
    "	SCHEDULE:SCHED_ASSASSIN_STALK_ENEMY"
    "	"
    "	Interrupts"
    "		COND_LIGHT_DAMAGE"
    "		COND_HEAVY_DAMAGE"
...
DEFINE_SCHEDULE(SCHED_ASSASSIN_EVADE,
                ...
                "	Tasks"
                "		TASK_SET_FAIL_SCHEDULE		"
                "SCHEDULE:SCHED_ASSASSIN_FIND_VANTAGE_POINT"
                "		TASK_STOP_MOVING			0"
