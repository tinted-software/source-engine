...
inline void TraceHull_SkipPhysics(const Vector &vecAbsStart,
...
#if HL2_EPISODIC
// When enabled, add code to have the antlion bleed profusely as it is badly
// injured.
#define ANTLIONGUARD_BLOOD_EFFECTS 2
ConVar g_antlionguard_hemorrhage(
    "g_antlionguard_hemorrhage", "1", FCVAR_NONE,
...
#define ANTLIONGUARD_POISON_TO                                                 \
  12 // we only poison Gordon down to twelve to give him a chance to regen up to
     // 20 by the next charge
#endif
...
int g_interactionAntlionGuardFoundPhysicsObject =
    0; // We're moving to a physics object to shove it, don't all choose the
       // same object
int g_interactionAntlionGuardShovedPhysicsObject =
    0; // We've punted an object, it is now clear to be chosen by others
...
struct PhysicsObjectCriteria_t {
  ...
  float flTargetCone;
  bool bPreferObjectsAlongTargetVector; // Prefer objects that we can strike
                                        // easily as we move towards our target
  float flNearRadius; // If we won't hit the player with the object, but get
                      // this close, throw anyway
};
...
public:
  ...
  //---------------------------------
  class CNavigator
      : public CAI_ComponentWithOuter<CNPC_AntlionGuard, CAI_Navigator> {
...
private:
  ...
  void ImpactShock(const Vector &origin, float radius, float magnitude,
                   CBaseEntity *pIgnored = NULL);
  ...
  void
  UpdatePhysicsTarget(bool bPreferObjectsAlongTargetVector,
  ...
  Vector GetPhysicsHitPosition(CBaseEntity *pObject, CBaseEntity *pTarget,
                               Vector *vecTrajectory, float *flClearDistance);
  bool CanStandAtShoveTarget(CBaseEntity *pShoveObject, CBaseEntity *pTarget,
                             Vector *pOut);
  CBaseEntity *GetNextShoveTarget(CBaseEntity *pLastEntity,
                                  AISightIter_t &iter);
  ...
  CNetworkVar(bool, m_bCavernBreed); // If this guard is meant to be a cavern
                                     // dweller (uses different assets)
  CNetworkVar(
      bool,
      m_bInCavern); // Behavioral hint telling the guard to change his behavior
...
DEFINE_FIELD(m_nFlinchActivity, FIELD_INTEGER),
    ...
    DEFINE_SOUNDPATCH(m_pGrowlHighSound), DEFINE_SOUNDPATCH(m_pGrowlLowSound),
    DEFINE_SOUNDPATCH(m_pGrowlIdleSound), DEFINE_SOUNDPATCH(m_pBreathSound),
    ...
    DEFINE_INPUTFUNC(FIELD_VOID, "SummonedAntlionDied",
                     InputSummonedAntlionDied),
    DEFINE_INPUTFUNC(FIELD_VOID, "EnablePreferPhysicsAttack",
                     InputEnablePreferPhysicsAttack),
    DEFINE_INPUTFUNC(FIELD_VOID, "DisablePreferPhysicsAttack",
...
CNPC_AntlionGuard::GetPhysicsImpactDamageTable(void) {
...
void CNPC_AntlionGuard::CreateGlow(CSprite **pSprite, const char *pAttachName) {
  ...
  *pSprite =
      CSprite::SpriteCreate("sprites/grubflare1.vmt", GetLocalOrigin(), false);
  Assert(*pSprite);
  if (*pSprite == NULL)
    return;

  (*pSprite)->TurnOn();
  (*pSprite)->SetTransparency(kRenderWorldGlow, 156, 169, 121, 164,
...
void CNPC_AntlionGuard::Spawn(void) {
  ...
  CollisionProp()->SetSurroundingBoundsType(USE_SPECIFIED_BOUNDS, &absMin,
                                            &absMax);
}
...
void CNPC_AntlionGuard::Activate(void) {
  ...
  // Find all nearby physics objects and add them to the list of objects we will
  // sense
  CBaseEntity *pObject = NULL;
  while ((pObject = gEntList.FindEntityInSphere(pObject, WorldSpaceCenter(),
                                                2500)) != NULL) {
...
int CNPC_AntlionGuard::SelectUnreachableSchedule(void) {
  ...
  if (pTarget) {
    ...
    if (GetSquad() != NULL) {
      GetSquad()->BroadcastInteraction(
          g_interactionAntlionGuardFoundPhysicsObject,
  ...
  float flDistToEnemySqr =
      (GetEnemy()->GetAbsOrigin() - GetAbsOrigin()).LengthSqr();
...
int CNPC_AntlionGuard::SelectCombatSchedule(void) {
  ...
  if ( HasCondition( COND_SEE_ENEMY ) )
  {
          bCanCharge = ShouldCharge( GetAbsOrigin(), GetEnemy()->GetAbsOrigin(),
  true, false );
  }
  */
...
//-----------------------------------------------------------------------------
bool CNPC_AntlionGuard::ShouldCharge(const Vector &startPos,
                                     const Vector &endPos, bool useTime,
                                     bool bCheckForCancel) {
  // Don't charge in tight spaces unless forced to
  if (hl2_episodic.GetBool() && m_bInCavern &&
      !(m_hChargeTarget.Get() && m_hChargeTarget->IsAlive()))
    return false;
  ...
  if (IsStrategySlotRangeOccupied(SQUAD_SLOT_ANTLIONGUARD_CHARGE,
  ...
  if (!bCheckForCancel) {
    ...
    if ((distance < ANTLIONGUARD_CHARGE_MIN) ||
        (distance > ANTLIONGUARD_CHARGE_MAX))
      return false;
  }

  if (GetSquad()) {
    // If someone in our squad is closer to the enemy, then don't charge (we end
    // up hitting them more often than not!)
    float flOurDistToEnemySqr =
        (GetAbsOrigin() - GetEnemy()->GetAbsOrigin()).LengthSqr();
    AISquadIter_t iter;
    for (CAI_BaseNPC *pSquadMember = GetSquad()->GetFirstMember(&iter);
      ...
      if ((pSquadMember->GetAbsOrigin() - GetEnemy()->GetAbsOrigin())
              .LengthSqr() < flOurDistToEnemySqr)
        return false;
    }
  }
...
int CNPC_AntlionGuard::SelectSchedule(void) {
  ...
  if (!m_bInCavern && HasCondition(COND_HEAVY_DAMAGE) &&
      m_flNextHeavyFlinchTime < gpGlobals->curtime) {
  ...
  if (m_hChargeTarget && m_hChargeTargetPosition) {
    ...
    if (m_hChargeTarget->IsAlive() == false) {
      ...
      if (ShouldCharge(GetAbsOrigin(), GetEnemy()->GetAbsOrigin(), false,
                       false) == false)
...
int CNPC_AntlionGuard::MeleeAttack1Conditions(float flDot, float flDist) {
  ...
  trace_t tr;
  TraceHull_SkipPhysics(WorldSpaceCenter(), GetEnemy()->WorldSpaceCenter(),
                        Vector(-10, -10, -10), Vector(10, 10, 10),
                        MASK_SHOT_HULL, this, COLLISION_GROUP_NONE, &tr,
                        VPhysicsGetObject()->GetMass() * 0.5);
...
float CNPC_AntlionGuard::MaxYawSpeed(void) {
  ...
  if (hl2_episodic.GetBool() && m_bInCavern) {
    // Allow a better turning rate when moving quickly but not charging the
    // player
    if ((eActivity == ACT_ANTLIONGUARD_CHARGE_RUN) &&
        IsCurSchedule(SCHED_ANTLIONGUARD_CHARGE) == false)
      return 16.0f;
  }
  ...
  if (eActivity == ACT_ANTLIONGUARD_CHARGE_RUN) {
    ...
    float dist =
        UTIL_DistApprox2D(GetEnemy()->WorldSpaceCenter(), WorldSpaceCenter());
...
bool CNPC_AntlionGuard::OverrideMove(float flInterval) {
  ...
  // TODO: This code increases the guard's ability to successfully hit a target,
  // but adds a new dimension of navigation
  //		 trouble to do with him not being able to "close the distance"
  //between himself and the object he wants to hit. 		 Fixing this will require
  //some thought on how he picks the correct distances to his targets and when
  //he's "close enough". -- jdw
  ...
  m_hPhysicsTarget->WorldSpaceCenter(), flWidth * 0.75f, AIMST_AVOID_OBJECT );
          //NDebugOverlay::Sphere( m_hPhysicsTarget->WorldSpaceCenter(),
  vec3_angle, flWidth, 255, 255, 255, 0, true, 0.5f );
  }
  */
...
void CNPC_AntlionGuard::Shove(void) {
  ...
  float damage =
      (pTarget->IsPlayer()) ? sk_antlionguard_dmg_shove.GetFloat() : 250;

  // If the target's still inside the shove cone, ensure we hit him
  Vector vecForward, vecEnd;
  AngleVectors(GetAbsAngles(), &vecForward);
  float flDistSqr =
      (pTarget->WorldSpaceCenter() - WorldSpaceCenter()).LengthSqr();
  Vector2D v2LOS =
      (pTarget->WorldSpaceCenter() - WorldSpaceCenter()).AsVector2D();
  Vector2DNormalize(v2LOS);
  float flDot = DotProduct2D(v2LOS, vecForward.AsVector2D());
  if (flDistSqr < (ANTLIONGUARD_MELEE1_RANGE * ANTLIONGUARD_MELEE1_RANGE) &&
      flDot >= ANTLIONGUARD_MELEE1_CONE) {
    vecEnd = pTarget->WorldSpaceCenter();
  } else {
    vecEnd =
        WorldSpaceCenter() + (BodyDirection3D() * ANTLIONGUARD_MELEE1_RANGE);
  }

  // Use the melee trace to ensure we hit everything there
  trace_t tr;
  CTakeDamageInfo dmgInfo(this, this, damage, DMG_SLASH);
  CTraceFilterMelee traceFilter(this, COLLISION_GROUP_NONE, &dmgInfo, 1.0,
                                true);
  Ray_t ray;
  ray.Init(WorldSpaceCenter(), vecEnd, Vector(-16, -16, -16),
           Vector(16, 16, 16));
  ...
  if (pHurt) {
    ...
    if (pHurt->IsPlayer()) {
...
#if HL2_EPISODIC
      // If I am a cavern guard attacking the player, and he still lives, then
      // poison him too.
      if (m_bInCavern && pHurt->IsPlayer() && pHurt->IsAlive() &&
          pHurt->m_iHealth > ANTLIONGUARD_POISON_TO) {
        // That didn't finish them. Take them down to one point with poison
        // damage. It'll heal.
        pHurt->TakeDamage(CTakeDamageInfo(
            this, this, pHurt->m_iHealth - ANTLIONGUARD_POISON_TO, DMG_POISON));
      }
#endif
      ...
      if (pVictim) {
        ...
        // FIXME: This causes NPCs that are not physically motivated to hop into
        // the air strangely -- jdw pVictim->ApplyAbsVelocityImpulse(
        // BodyDirection2D() * 400 + Vector( 0, 0, 250 ) );
      }
...
public:
  ...
  CTraceFilterCharge(const IHandleEntity *passentity, int collisionGroup,
                     CNPC_AntlionGuard *pAttacker)
  ...
  virtual bool ShouldHitEntity(IHandleEntity *pHandleEntity, int contentsMask) {
    ...
    if (pEntity) {
      ...
      Vector attackDir =
          pEntity->WorldSpaceCenter() - m_pAttacker->WorldSpaceCenter();
      VectorNormalize(attackDir);

      float flDamage =
          (pEntity->IsPlayer()) ? sk_antlionguard_dmg_shove.GetFloat() : 250;
      ;

      CTakeDamageInfo info(m_pAttacker, m_pAttacker, flDamage, DMG_CRUSH);
      CalculateMeleeDamageForce(&info, attackDir,
                                info.GetAttacker()->WorldSpaceCenter(), 4.0f);
...
void CNPC_AntlionGuard::Footstep(bool bHeavy) {
  ...
  float flDistanceToPlayerSqr =
      (pPlayer->GetAbsOrigin() - GetAbsOrigin()).LengthSqr();
  float flNearVolume =
      RemapValClamped(flDistanceToPlayerSqr, Square(10 * 12.0f),
                      MIN_FOOTSTEP_NEAR_DIST, VOL_NORM, 0.0f);
...
//			*pOut -
//-----------------------------------------------------------------------------
void CNPC_AntlionGuard::GetPhysicsShoveDir(CBaseEntity *pObject, float flMass,
                                           Vector *pOut) {
  ...
  float flSpeed =
      RemapValClamped(flMass, 0.0f, 150.0f, flBaseSpeed * 2.0f, flBaseSpeed);
...
void CNPC_AntlionGuard::HandleAnimEvent(animevent_t *pEvent) {
  if (pEvent->event == AE_ANTLIONGUARD_CHARGE_EARLYOUT) {
    // Robin: Removed this because it usually made him look less intelligent,
    // not more.
    //		  This code left here so we don't get warnings in the console.

    /*
    // Cancel the charge if it's no longer valid
    if ( ShouldCharge( GetAbsOrigin(), GetEnemy()->GetAbsOrigin(), false, true )
    == false )
    {
  ...
  if (pEvent->event == AE_ANTLIONGUARD_SHOVE_PHYSOBJECT) {
    ...
    // in. But, we still want to clobber it so the animevent doesn't happen
    // fruitlessly, and we want to clear the condition flags and other state
    // regarding the m_hPhysicsTarget. So, skip the code relevant to computing a
    // launch vector specific to the object I'm striking, but use the
    // ImpactShock call further below to punch everything in the neighborhood.
    CBaseEntity *pEnemy = GetEnemy();
    if (pEnemy != NULL) {
      // Setup the throw velocity
      IPhysicsObject *physObj = m_hPhysicsTarget->VPhysicsGetObject();
      Vector vecShoveVel =
          (pEnemy->GetAbsOrigin() - m_hPhysicsTarget->WorldSpaceCenter());
      float flTargetDist = VectorNormalize(vecShoveVel);

      // Must still be close enough to our target
      Vector shoveDir =
          m_hPhysicsTarget->WorldSpaceCenter() - WorldSpaceCenter();
      ...
      if (hl2_episodic.GetBool()) {
        ...
        // If the player isn't looking at me, and isn't reachable, be more
        // forgiving about hitting them
        if (HasCondition(COND_ENEMY_UNREACHABLE) &&
            HasCondition(COND_ENEMY_FACING_ME) == false) {
          // Build an arc around the top of the target that we'll offset our aim
          // by
          Vector vecOffset;
          float flSin, flCos;
          ...
          float flOffsetRadius =
              (m_hPhysicsTarget->CollisionProp()->BoundingRadius() +
        ...
        // FIXME: We need to restore this on the object at some point if we do
        // this!
        ...
        vecShoveVel *=
            flTargetDist * 3 * physObj->GetMass(); // FIXME: Scale by skill
      ...
      // Send it flying
      AngularImpulse angVel(random->RandomFloat(-180, 180), 100,
    ...
    g_pEffects->Dust(m_hPhysicsTarget->WorldSpaceCenter(), vecRandom, 64.0f,
                     32);
    ...
    // Disrupt other objects near it, including the m_hPhysicsTarget if we had
    // no valid enemy
    ImpactShock(m_hPhysicsTarget->WorldSpaceCenter(), 150, 250.0f,
                pEnemy != NULL ? m_hPhysicsTarget : NULL);

    // Notify any squad members that we're no longer monopolizing this object
    if (GetSquad() != NULL) {
      GetSquad()->BroadcastInteraction(
          g_interactionAntlionGuardShovedPhysicsObject,
          (void *)((CBaseEntity *)m_hPhysicsTarget), this);
    }
  ...
  if (pEvent->event == AE_ANTLIONGUARD_CHARGE_HIT) {
    ...
    if (g_debug_antlionguard.GetInt() == 1) {
      ...
      NDebugOverlay::BoxDirection(startPos, GetHullMins(), hullMaxs,
                                  BodyDirection2D(), 100, 255, 255, 20, 1.0f);
    }
    ...
    // Cause a shock wave from this point which will distrupt nearby physics
    // objects
  ...
  if (pEvent->event == AE_ANTLIONGUARD_VOICE_GROWL) {
    ...
    if (random->RandomInt(0, 10) < 6) {
      duration = ENVELOPE_CONTROLLER.SoundPlayEnvelope(
          m_pGrowlHighSound, SOUNDCTRL_CHANGE_VOLUME, envAntlionGuardFastGrowl,
          ARRAYSIZE(envAntlionGuardFastGrowl));
    } else {
      duration = 1.0f;
  ...
  if (pEvent->event == AE_ANTLIONGUARD_VOICE_BARK) {
    ...
    float duration = ENVELOPE_CONTROLLER.SoundPlayEnvelope(
        m_pGrowlHighSound, SOUNDCTRL_CHANGE_VOLUME, envAntlionGuardBark1,
        ARRAYSIZE(envAntlionGuardBark1));
    ENVELOPE_CONTROLLER.SoundPlayEnvelope(
        m_pConfusedSound, SOUNDCTRL_CHANGE_VOLUME, envAntlionGuardBark2,
  ...
  if (pEvent->event == AE_ANTLIONGUARD_VOICE_PAIN) {
    ...
    float duration = ENVELOPE_CONTROLLER.SoundPlayEnvelope(
        m_pConfusedSound, SOUNDCTRL_CHANGE_VOLUME, envAntlionGuardPain1,
        ARRAYSIZE(envAntlionGuardPain1));
    ENVELOPE_CONTROLLER.SoundPlayEnvelope(
        m_pGrowlHighSound, SOUNDCTRL_CHANGE_VOLUME, envAntlionGuardBark2,
        ARRAYSIZE(envAntlionGuardBark2));
  ...
  if (pEvent->event == AE_ANTLIONGUARD_VOICE_SQUEEZE) {
    ...
    float duration = ENVELOPE_CONTROLLER.SoundPlayEnvelope(
        m_pGrowlHighSound, SOUNDCTRL_CHANGE_VOLUME, envAntlionGuardSqueeze,
        ARRAYSIZE(envAntlionGuardSqueeze));

    ENVELOPE_CONTROLLER.SoundChangeVolume(m_pBreathSound, 0.6f,
                                          random->RandomFloat(2.0f, 4.0f));
    ENVELOPE_CONTROLLER.SoundChangePitch(m_pBreathSound,
                                         random->RandomInt(60, 80),
                                         random->RandomFloat(2.0f, 4.0f));
  ...
  if (pEvent->event == AE_ANTLIONGUARD_VOICE_SCRATCH) {
    ...
    ENVELOPE_CONTROLLER.SoundChangePitch(m_pBreathSound,
                                         random->RandomInt(60, 80), duration);
  ...
  if (pEvent->event == AE_ANTLIONGUARD_VOICE_GRUNT) {
    ...
    float duration = ENVELOPE_CONTROLLER.SoundPlayEnvelope(
        m_pConfusedSound, SOUNDCTRL_CHANGE_VOLUME, envAntlionGuardGrunt,
        ARRAYSIZE(envAntlionGuardGrunt));
    ENVELOPE_CONTROLLER.SoundPlayEnvelope(
        m_pGrowlHighSound, SOUNDCTRL_CHANGE_VOLUME, envAntlionGuardGrunt2,
...
int CNPC_AntlionGuard::OnTakeDamage_Alive(const CTakeDamageInfo &info) {
  ...
  if (dInfo.GetAttacker() && dInfo.GetAttacker() != this &&
      FClassnameIs(dInfo.GetAttacker(), "npc_antlionguard"))
    return 0;

  if ((dInfo.GetDamageType() & DMG_CRUSH) &&
      !(dInfo.GetDamageType() & DMG_VEHICLE)) {
  ...
  if (IsHeavyDamage(dInfo)) {
    ...
    // TODO: This will require a more complete solution; for now let's shelve
    // it!
    /*
    if ( dInfo.GetDamageType() & DMG_BLAST )
    {
            // Create the dust effect in place
            CParticleSystem *pParticle = (CParticleSystem *) CreateEntityByName(
    "info_particle_system" ); if ( pParticle != NULL )
            {
                    // Setup our basic parameters
                    pParticle->KeyValue( "start_active", "1" );
                    pParticle->KeyValue( "effect_name", "fire_medium_02_nosmoke"
    ); pParticle->SetParent( this ); pParticle->SetParentAttachment(
    "SetParentAttachment", "attach_glow2", true ); pParticle->SetLocalOrigin(
    Vector( -16, 24, 0 ) ); DispatchSpawn( pParticle ); if ( gpGlobals->curtime
    > 0.5f ) pParticle->Activate();

                    pParticle->SetThink( &CBaseEntity::SUB_Remove );
                    pParticle->SetNextThink( gpGlobals->curtime +
    random->RandomFloat( 2.0f, 3.0f ) );
            }
    }
    */
  }
  ...
  // See if we've crossed a measured phase in our health and flinch from that to
  // show we do take damage
  if (!m_bInCavern && HasCondition(COND_HEAVY_DAMAGE) == false &&
...
//			bitsDamageType -
//-----------------------------------------------------------------------------
void CNPC_AntlionGuard::TraceAttack(const CTakeDamageInfo &inputInfo,
...
void CNPC_AntlionGuard::StartTask(const Task_t *pTask) {
  ...
  case TASK_ANTLIONGUARD_GET_PATH_TO_PHYSOBJECT: {
    ...
    if (GetNavigator()->SetGoal(goal)) {
      if (g_debug_antlionguard.GetInt() == 1) {
        NDebugOverlay::Cross3D(vecGoalPos, Vector(8, 8, 8), -Vector(8, 8, 8), 0,
                               255, 0, true, 2.0f);
        NDebugOverlay::Line(vecGoalPos, m_hPhysicsTarget->WorldSpaceCenter(), 0,
                            255, 0, true, 2.0f);
        NDebugOverlay::Line(m_hPhysicsTarget->WorldSpaceCenter(),
                            GetEnemy()->WorldSpaceCenter(), 0, 255, 0, true,
                            2.0f);
      }
      ...
      if (g_debug_antlionguard.GetInt() == 1) {
        NDebugOverlay::Cross3D(vecGoalPos, Vector(8, 8, 8), -Vector(8, 8, 8),
                               255, 0, 0, true, 2.0f);
        NDebugOverlay::Line(vecGoalPos, m_hPhysicsTarget->WorldSpaceCenter(),
                            255, 0, 0, true, 2.0f);
        NDebugOverlay::Line(m_hPhysicsTarget->WorldSpaceCenter(),
                            GetEnemy()->WorldSpaceCenter(), 255, 0, 0, true,
                            2.0f);
      }
    ...
    //!!!HACKHACK - this is a hack that covers a bug in antlion guard physics
    //!target selection! (Tracker #77601)
    // This piece of code (combined with some code in GatherConditions) COVERS
    // THE BUG by escaping the schedule if 30 seconds have passed (it should
    // never take this long for the guard to get to an object and hit it). It's
    // too scary to figure out why this particular antlion guard can't get to
    // its object, but we're shipping like, tomorrow. (sjb) 8/22/2007
  ...
  case TASK_ANTLIONGUARD_GET_PATH_TO_CHARGE_POSITION: {
    ...
    AI_NavGoal_t goal(GOALTYPE_LOCATION,
                      m_hChargeTargetPosition->GetAbsOrigin(), ACT_RUN);
    if (GetNavigator()->SetGoal(goal)) {
      // We want to face towards the charge target
      Vector vecDir = m_hChargeTarget->GetAbsOrigin() -
                      m_hChargeTargetPosition->GetAbsOrigin();
      VectorNormalize(vecDir);
      vecDir.z = 0;
  ...
  case TASK_ANTLIONGUARD_GET_PATH_TO_NEAREST_NODE: {
    ...
    int node = GetNavigator()->GetNetwork()->NearestNodeToPoint(
        this, GetEnemy()->GetAbsOrigin(), false);
  ...
  case TASK_ANTLIONGUARD_GET_CHASE_PATH_ENEMY_TOLERANCE: {
    // Chase the enemy, but allow local navigation to succeed if it gets within
    // the goal tolerance
  ...
  case TASK_ANTLIONGUARD_OPPORTUNITY_THROW: {
    ...
    if (m_iNumLiveAntlions >= 2 && RandomFloat(0, 1) > 0.5) {
      m_FailedPhysicsTargets.RemoveAll();
      UpdatePhysicsTarget(false, m_bPreferPhysicsAttack
                                     ? (100 * 12)
                                     : ANTLIONGUARD_FARTHEST_PHYSICS_OBJECT);
      if (HasCondition(COND_ANTLIONGUARD_PHYSICS_TARGET) && !m_bInCavern) {
...
//			Done outside of the guard because we need to do this
//inside a trace filter.
//-----------------------------------------------------------------------------
void ApplyChargeDamage(CBaseEntity *pAntlionGuard, CBaseEntity *pTarget,
                       float flDamage) {
  Vector attackDir =
      (pTarget->WorldSpaceCenter() - pAntlionGuard->WorldSpaceCenter());
  VectorNormalize(attackDir);
  ...
  // Deal the damage
  CTakeDamageInfo info(pAntlionGuard, pAntlionGuard, vecForce, offset, flDamage,
                       DMG_CLUB);
  ...
  if (static_cast<CNPC_AntlionGuard *>(pAntlionGuard)->IsInCavern() &&
      pTarget->IsPlayer() && pTarget->IsAlive() &&
      pTarget->m_iHealth > ANTLIONGUARD_POISON_TO) {
    // That didn't finish them. Take them down to one point with poison damage.
    // It'll heal.
    pTarget->TakeDamage(CTakeDamageInfo(
        pAntlionGuard, pAntlionGuard,
        pTarget->m_iHealth - ANTLIONGUARD_POISON_TO, DMG_POISON));
  }
#endif
}
...
public:
  ...
  CTraceFilterSkipPhysics(const IHandleEntity *passentity, int collisionGroup,
                          float minMass)
      : m_pPassEnt(passentity), m_collisionGroup(collisionGroup),
        m_minMass(minMass) {}
  virtual bool ShouldHitEntity(IHandleEntity *pHandleEntity, int contentsMask) {
...
bool CNPC_AntlionGuard::EnemyIsRightInFrontOfMe(CBaseEntity **pEntity) {
  ...
  if ((GetEnemy()->WorldSpaceCenter() - WorldSpaceCenter()).LengthSqr() <
    ...
    if (DotProduct(vecLOS, vBodyDir) > 0.8) {
      ...
      trace_t tr;
      UTIL_TraceLine(WorldSpaceCenter(), GetEnemy()->EyePosition(), MASK_SOLID,
...
//-----------------------------------------------------------------------------
// Purpose: While charging, look ahead and see if we're going to run into
// anything.
//			If we are, start the gesture so it looks like we're
...
void CNPC_AntlionGuard::ChargeLookAhead(void) {
  ...
  TraceHull_SkipPhysics(GetAbsOrigin(), vecTestPos, testHullMins, GetHullMaxs(),
                        MASK_SHOT_HULL, this, COLLISION_GROUP_NONE, &tr,
                        VPhysicsGetObject()->GetMass() * 0.5);

  // NDebugOverlay::Box( tr.startpos, testHullMins, GetHullMaxs(), 0, 255, 0,
  // true, 0.1f ); NDebugOverlay::Box( vecTestPos, testHullMins, GetHullMaxs(),
  // 255, 0, 0, true, 0.1f );
...
//-----------------------------------------------------------------------------
// Purpose: Handles the guard charging into something. Returns true if it hit
// the world.
//-----------------------------------------------------------------------------
bool CNPC_AntlionGuard::HandleChargeImpact(Vector vecImpact,
                                           CBaseEntity *pEntity) {
  ...
  if (!pEntity || pEntity->IsWorld()) {
    // Robin: Due to some of the finicky details in the motor, the guard will
    // hit
    //		  the world when it is blocked by our enemy when trying to step
    //up 		  during a moveprobe. To get around this, we see if the enemy's within 		  a
    //volume in front of the guard when we hit the world, and if he is, 		  we hit
    //him anyway.
  ...
  if (pEntity->GetMoveType() == MOVETYPE_VPHYSICS) {
    ...
    if (pPhysics) {
      ...
      if ((!pPhysics->IsMoveable() ||
           pPhysics->GetMass() > VPhysicsGetObject()->GetMass() * 0.5f))
        return true;
    }
  }
  ...
  ROBIN: Wrote & then removed this. If we want to have large rocks that the
  guard should smack around, then we should enable it.

  else
  {
          // If we hit a physics prop, smack the crap out of it. (large rocks)
          // Factor the object mass into it, because we want to move it no
  matter how heavy it is. if ( pEntity->GetMoveType() == MOVETYPE_VPHYSICS )
          {
                  CTakeDamageInfo info( this, this, 250, DMG_BLAST );
                  info.SetDamagePosition( vecImpact );
                  float flForce = ImpulseScale(
  pEntity->VPhysicsGetObject()->GetMass(), 250 ); flForce *=
  random->RandomFloat( 0.85, 1.15 );
...
float CNPC_AntlionGuard::ChargeSteer(void) {
  ...
  // Probe
  TraceHull_SkipPhysics(GetAbsOrigin(), testPos, testHullMins, GetHullMaxs(),
...
case TASK_RUN_PATH:
  ...
  case TASK_ANTLIONGUARD_CHARGE: {
    ...
    if (eActivity == ACT_ANTLIONGUARD_CHARGE_STOP ||
        eActivity == ACT_ANTLIONGUARD_CHARGE_CRASH) {
      if (IsActivityFinished()) {
        TaskComplete();
        return;
      }
        ...
        if (GetEnemy() != NULL) {
          ...
          if (DotProduct(BodyDirection2D(), goalDir) < 0.25f) {
            if (!m_bDecidedNotToStop) {
              // We've missed the target. Randomly decide not to stop, which
              // will cause the guard to just try and swing around for another
              // pass.
      ...
      if (HandleChargeImpact(moveTrace.vEndPosition, moveTrace.pObstruction)) {
        ...
        if (moveTrace.pObstruction->Classify() == CLASS_ANTLION) {
          if (FClassnameIs(moveTrace.pObstruction, "npc_antlionguard")) {
            ...
            ApplyChargeDamage(this, moveTrace.pObstruction,
...
void CNPC_AntlionGuard::SummonAntlions(void) {
  ...
  spawnpos_t sAntlionSpawnPositions[] = {
      {0, 200},    {0, -200},    {128, 128}, {128, -128},
      {-128, 128}, {-128, -128}, {200, 0},   {-200, 0},
  };

  // Only spawn up to our max count
  int iSpawnPoint = 0;
  for (int i = 0; (m_iNumLiveAntlions < ANTLIONGUARD_SUMMON_COUNT) &&
                  (iSpawnPoint < ARRAYSIZE(sAntlionSpawnPositions));
       i++) {
    // Determine spawn position for the antlion
    Vector vecSpawn =
        GetAbsOrigin() +
        (sAntlionSpawnPositions[iSpawnPoint].flForward * vecForward) +
    ...
    trace_t tr;
    UTIL_TraceHull(vecSpawn, vecSpawn - Vector(0, 0, 128),
    ...
    if (tr.startsolid || tr.allsolid || tr.fraction == 1.0) {
      if (g_debug_antlionguard.GetInt() == 2) {
        NDebugOverlay::Box(tr.endpos, NAI_Hull::Mins(HULL_MEDIUM),
                           NAI_Hull::Maxs(HULL_MEDIUM), 255, 0, 0, true, 5.0f);
      }
      continue;
    }
    ...
    if ((pdata->game.material != CHAR_TEX_DIRT) &&
        (pdata->game.material != CHAR_TEX_SAND)) {
      if (g_debug_antlionguard.GetInt() == 2) {
        NDebugOverlay::Box(tr.endpos, NAI_Hull::Mins(HULL_MEDIUM),
                           NAI_Hull::Maxs(HULL_MEDIUM), 255, 128, 128, true,
                           5.0f);
      }
      continue;
    }
    ...
    trace_t tr_vis;
    UTIL_TraceLine(WorldSpaceCenter(), tr.endpos, MASK_NPCSOLID, this,
                   COLLISION_GROUP_NONE, &tr_vis);
    if (tr_vis.fraction != 1.0) {
      if (g_debug_antlionguard.GetInt() == 2) {
        NDebugOverlay::Line(WorldSpaceCenter(), tr.endpos, 255, 0, 0, true,
                            5.0f);
      }
      continue;
    }
    ...
    if (g_debug_antlionguard.GetInt() == 2) {
      NDebugOverlay::Box(tr.endpos, NAI_Hull::Mins(HULL_MEDIUM),
                         NAI_Hull::Maxs(HULL_MEDIUM), 0, 255, 0, true, 5.0f);
    ...
    pAntlion->KeyValue("OnDeath", UTIL_VarArgs("%s,SummonedAntlionDied,,0,-1",
    ...
    pAntlion->Activate();
    g_EventQueue.AddEvent(pAntlion, "Unburrow", RandomFloat(0.1, 1.0), this,
                          this);
...
void CNPC_AntlionGuard::InputSetChargeTarget(inputdata_t &inputdata) {
  ...
  CBaseEntity *pPosition = gEntList.FindEntityByName(
      NULL, pszParam, NULL, inputdata.pActivator, inputdata.pCaller);
  if (!pPosition) {
    Warning("ERROR: Guard %s cannot find charge position '%s'\nMake sure "
            "you've specified the parameters as [target start]!\n",
            STRING(GetEntityName()), pszParam);
    return;
  }
...
Activity CNPC_AntlionGuard::NPC_TranslateActivity(Activity baseAct) {
  ...
  // (unless scripted)
  if (hl2_episodic.GetBool() && m_bInCavern && baseAct == ACT_RUN &&
      IsInAScript() == false)
    return (Activity)ACT_ANTLIONGUARD_CHARGE_RUN;
...
void CNPC_AntlionGuard::UpdateHead(void) {
  ...
  if (ShouldWatchEnemy() && (GetEnemy() != NULL)) {
    ...
    SetPoseParameter(m_poseHead_Pitch,
                     UTIL_Approach(pitch + angleDiff, pitch, 50));
  } else {
...
void CNPC_AntlionGuard::MaintainPhysicsTarget(void) {
  ...
  float flTargetDistSqr =
      (m_hPhysicsTarget->WorldSpaceCenter() - m_vecPhysicsTargetStartPos)
          .LengthSqr();
  ...
  if (g_debug_antlionguard.GetInt() == 3) {
    NDebugOverlay::Cross3D(m_hPhysicsTarget->WorldSpaceCenter(),
                           -Vector(32, 32, 32), Vector(32, 32, 32), 255, 255,
                           255, true, 1.0f);
  }
}
...
// Purpose:
//-----------------------------------------------------------------------------
void CNPC_AntlionGuard::UpdatePhysicsTarget(
    bool bPreferObjectsAlongTargetVector, float flRadius) {
  if (GetEnemy() == NULL)
    return;
  ...
  criteria.flNearRadius = (20 * 12); // TODO: It may preferable to disable this
                                     // for legacy products as well -- jdw
...
void CNPC_AntlionGuard::PrescheduleThink(void) {
  ...
  if (IsCurSchedule(SCHED_ANTLIONGUARD_CHASE_ENEMY) ||
    ...
    bool bCheckAlongLine =
        (IsCurSchedule(SCHED_ANTLIONGUARD_CHASE_ENEMY) ||
         IsCurSchedule(SCHED_ANTLIONGUARD_CHASE_ENEMY_TOLERANCE));
  ...
  if ((m_flGroundSpeed <= 0.0f)) {
    if (m_bStopped == false) {
      ...
      ENVELOPE_CONTROLLER.SoundChangePitch(m_pBreathSound,
                                           random->RandomInt(40, 60), duration);

      ENVELOPE_CONTROLLER.SoundChangeVolume(m_pGrowlIdleSound, 0.0f, duration);
      ENVELOPE_CONTROLLER.SoundChangePitch(
          m_pGrowlIdleSound, random->RandomInt(120, 140), duration);
    ...
    if (m_flBreathTime < gpGlobals->curtime) {
      ...
      ENVELOPE_CONTROLLER.SoundChangeVolume(m_pGrowlIdleSound,
                                            random->RandomFloat(0.2f, 0.3f),
                                            random->RandomFloat(0.5f, 1.0f));
      ENVELOPE_CONTROLLER.SoundChangePitch(m_pGrowlIdleSound,
                                           random->RandomInt(80, 120),
                                           random->RandomFloat(0.5f, 1.0f));
  ...
  for (int i = 0; i < 3; i++) {
    CSoundEnt::InsertSound(
        SOUND_DANGER, WorldSpaceCenter() + (BodyDirection3D() * 128 * (i + 1)),
        128, 0.1f, this);
  }

#if ANTLIONGUARD_BLOOD_EFFECTS
  // compute and if necessary transmit the bleeding level for the particle
  // effect
...
//-----------------------------------------------------------------------------
// Purpose: Return the point at which the guard wants to stand on to knock the
// physics object at the target entity Input  : *pObject - Object to be shoved.
//			*pTarget - Target to be shoved at.
//			*vecTrajectory - Trajectory to our target
//			*flClearDistance - Distance behind the entity we're
//clear to use
// Output : Position at which to attempt to strike the object
//-----------------------------------------------------------------------------
Vector CNPC_AntlionGuard::GetPhysicsHitPosition(CBaseEntity *pObject,
                                                CBaseEntity *pTarget,
                                                Vector *vecTrajectory,
                                                float *flClearDistance) {
  // Get the trajectory we want to knock the object along
  Vector vecToTarget =
      pTarget->WorldSpaceCenter() - pObject->WorldSpaceCenter();
  VectorNormalize(vecToTarget);
  vecToTarget.z = 0;

  // Get the distance we want to be from the object when we hit it
  IPhysicsObject *pPhys = pObject->VPhysicsGetObject();
  Vector extent = physcollision->CollideGetExtent(
      pPhys->GetCollide(), pObject->GetAbsOrigin(), pObject->GetAbsAngles(),
      -vecToTarget);
  float flDist = (extent - pObject->WorldSpaceCenter()).Length() +
                 CollisionProp()->BoundingRadius() + 32.0f;
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
inline bool CNPC_AntlionGuard::CanStandAtPoint(const Vector &vecPos,
                                               Vector *pOut) {
  ...
  // Start high and try to go lower, looking for the ground between here and
  // there We do this first because it's more likely to succeed in the typical
  // guard arenas (with open terrain)
  UTIL_TraceHull(vecStart, vecEnd, GetHullMins(), GetHullMaxs(), MASK_NPCSOLID,
                 this, COLLISION_GROUP_NONE, &tr);
  if (tr.startsolid && !tr.allsolid) {
    // We started in solid but didn't end up there, see if we can stand where we
    // ended up
    UTIL_TraceHull(tr.endpos, tr.endpos, GetHullMins(), GetHullMaxs(),
                   MASK_NPCSOLID, this, COLLISION_GROUP_NONE, &tr);

    // Must not be in solid
    bTraceCleared = (!tr.allsolid && !tr.startsolid);
  } else {
    // Must not be in solid and must have found a floor (otherwise we're
    // potentially hanging over a ledge)
  ...
  if (bTraceCleared == false) {
    if (g_debug_antlionguard.GetInt() == 3) {
      NDebugOverlay::Box(vecPos, GetHullMins(), GetHullMaxs(), 255, 0, 0, 0,
                         15.0f);
    }
    return false;
  }
  ...
  if (g_debug_antlionguard.GetInt() == 3) {
    NDebugOverlay::Box((*pOut), GetHullMins(), GetHullMaxs(), 0, 255, 0, 0,
                       15.0f);
  }
...
//-----------------------------------------------------------------------------
// Purpose: Determines whether or not the guard can stand in a position to
// strike a specified object Input  : *pShoveObject - Object being shoved
...
//-----------------------------------------------------------------------------
bool CNPC_AntlionGuard::CanStandAtShoveTarget(CBaseEntity *pShoveObject,
                                              CBaseEntity *pTarget,
                                              Vector *pOut) {
  // Get the position we want to be at to swing at the object
  float flClearDistance;
  Vector vecTrajectory;
  Vector vecHitPosition = GetPhysicsHitPosition(
      pShoveObject, pTarget, &vecTrajectory, &flClearDistance);
  Vector vecStandPosition;

  if (g_debug_antlionguard.GetInt() == 3) {
    NDebugOverlay::HorzArrow(pShoveObject->WorldSpaceCenter(),
                             pShoveObject->WorldSpaceCenter() +
                                 vecTrajectory * 64.0f,
                             16.0f, 255, 255, 0, 16, true, 15.0f);
  }
  ...
  if (CanStandAtPoint(vecHitPosition, &vecStandPosition) == false) {
    ...
    // Try to the side
    if (CanStandAtPoint((pShoveObject->WorldSpaceCenter() +
                         (vecSkewTrajectory * flClearDistance)),
...
//-----------------------------------------------------------------------------
CBaseEntity *CNPC_AntlionGuard::GetNextShoveTarget(CBaseEntity *pLastEntity,
                                                   AISightIter_t &iter) {
  // Try to find scripted items first
  if (m_strShoveTargets != NULL_STRING) {
    CBaseEntity *pFound =
        gEntList.FindEntityByName(pLastEntity, m_strShoveTargets);
    if (pFound)
      return pFound;
  }
...
//-----------------------------------------------------------------------------
CBaseEntity *CNPC_AntlionGuard::FindPhysicsObjectTarget(
    const PhysicsObjectCriteria_t &criteria) {
  // Must have a valid target entity
  if (criteria.pTarget == NULL)
    return NULL;

  if (g_debug_antlionguard.GetInt() == 3) {
    NDebugOverlay::Circle(GetAbsOrigin(), QAngle(-90, 0, 0), criteria.flRadius,
                          255, 0, 0, 8, true, 2.0f);
  }
  ...
  // Cost is determined by distance to the object, modified by how "in line" it
  // is with our target direction of travel Use the distance to the player as
  // the base cost for throwing an object (avoids pushing things too close to
  // the player)
  float flLeastCost =
      (criteria.bPreferObjectsAlongTargetVector)
  ...
  while ((pObject = GetNextShoveTarget(pObject, iter)) != NULL) {
    ...
    if (m_FailedPhysicsTargets.Find(pObject) !=
        m_FailedPhysicsTargets.InvalidIndex())
      continue;
    ...
    // Ignore objects moving too quickly (they'll be too hard to catch
    // otherwise)
    Vector velocity;
    ...
    if (criteria.bPreferObjectsAlongTargetVector) {
      ...
      float flCostScale =
          RemapValClamped(flDot, 1.0f, criteria.flTargetCone, 1.0f, 4.0f);
    ...
    if (flCost >= flLeastCost) {
      if (g_debug_antlionguard.GetInt() == 3) {
        NDebugOverlay::Box(vecObjCenter, -Vector(16, 16, 16),
                           Vector(16, 16, 16), 255, 0, 0, 0, 2.0f);
      }
    ...
    trace_t tr;
    UTIL_TraceLine(vecObjCenter, criteria.pTarget->BodyTarget(vecObjCenter),
                   MASK_SOLID_BRUSHONLY, this, COLLISION_GROUP_NONE, &tr);

    // See how close to our target we got (we still look good hurling things
    // that won't necessarily hit)
    if ((tr.endpos - criteria.pTarget->WorldSpaceCenter()).LengthSqr() >
        Square(criteria.flNearRadius))
      continue;

    // Must be able to stand at a position to hit the object
    Vector vecHitPosition;
    if (CanStandAtShoveTarget(pObject, criteria.pTarget, &vecHitPosition) ==
        false) {
      if (g_debug_antlionguard.GetInt() == 3) {
        NDebugOverlay::HorzArrow(GetAbsOrigin(), pObject->WorldSpaceCenter(),
                                 32.0f, 255, 0, 0, 64, true, 2.0f);
      }
      continue;
    }
  ...
  if (pNearest != NULL) {
    ...
    if (g_debug_antlionguard.GetInt() == 3) {
      NDebugOverlay::HorzArrow(GetAbsOrigin(), pNearest->WorldSpaceCenter(),
                               32.0f, 0, 255, 0, 128, true, 2.0f);
    }
  }
...
//			magnitude -
//-----------------------------------------------------------------------------
void CNPC_AntlionGuard::ImpactShock(const Vector &origin, float radius,
                                    float magnitude, CBaseEntity *pIgnored) {
  ...
  while ((pEntity = gEntList.FindEntityInSphere(pEntity, origin, radius)) !=
         NULL) {
    ...
    // UNDONE: Ask the object if it should get force if it's not
    // MOVETYPE_VPHYSICS?
    if (pEntity->GetMoveType() == MOVETYPE_VPHYSICS ||
      ...
      if (radius == 0 || flDist <= radius) {
        ...
        CalculateExplosiveDamageForce(&info, (vecSpot - GetAbsOrigin()),
                                      GetAbsOrigin());
...
void CNPC_AntlionGuard::ChargeDamage(CBaseEntity *pTarget) {
  ...
  float flDamage =
      (pPlayer == NULL) ? 250 : sk_antlionguard_dmg_charge.GetFloat();
...
//-----------------------------------------------------------------------------
void CNPC_AntlionGuard::InputDisablePreferPhysicsAttack(
...
// Purpose:
//-----------------------------------------------------------------------------
int CNPC_AntlionGuard::SelectFailSchedule(int failedSchedule, int failedTask,
                                          AI_TaskFailureCode_t taskFailCode) {
  // Figure out what to do next
  if (failedSchedule == SCHED_ANTLIONGUARD_CHASE_ENEMY &&
      HasCondition(COND_ENEMY_UNREACHABLE))
    return SelectUnreachableSchedule();
...
void CNPC_AntlionGuard::StartSounds(void) {
  ...
  if (m_pGrowlHighSound == NULL) {
    m_pGrowlHighSound = ENVELOPE_CONTROLLER.SoundCreate(
        filter, entindex(), CHAN_VOICE, "NPC_AntlionGuard.GrowlHigh",
        ATTN_NORM);
  ...
  if (m_pGrowlIdleSound == NULL) {
    m_pGrowlIdleSound = ENVELOPE_CONTROLLER.SoundCreate(
        filter, entindex(), CHAN_STATIC, "NPC_AntlionGuard.GrowlIdle",
        ATTN_NORM);
  ...
  if (m_pBreathSound == NULL) {
    m_pBreathSound = ENVELOPE_CONTROLLER.SoundCreate(
        filter, entindex(), CHAN_ITEM, "NPC_AntlionGuard.BreathSound",
        ATTN_NORM);
  ...
  if (m_pConfusedSound == NULL) {
    m_pConfusedSound =
        ENVELOPE_CONTROLLER.SoundCreate(filter, entindex(), CHAN_WEAPON,
                                        "NPC_AntlionGuard.Confused", ATTN_NORM);
...
void CNPC_AntlionGuard::Event_Killed(const CTakeDamageInfo &info) {
  ...
  if (m_iNumLiveAntlions) {
    ...
    while ((pSearch = gEntList.FindEntityByClassname(pSearch, "npc_antlion")) !=
      ...
      if (pAntlion && pAntlion->GetOwnerEntity() == NULL &&
          pAntlion->IsAlive()) {
        g_EventQueue.AddEvent(pAntlion, "BurrowAway", RandomFloat(0.1, 2.0),
                              this, this);
      }
    }
  }
...
// Output :
//-----------------------------------------------------------------------------
bool CNPC_AntlionGuard::OverrideMoveFacing(const AILocalMoveGoal_t &move,
                                           float flInterval) {
...
bool CNPC_AntlionGuard::IsHeavyDamage(const CTakeDamageInfo &info) {
  ...
  if (info.GetDamageType() & DMG_CRUSH) {
    ...
    if ((pPhysObject != NULL) &&
        (pPhysObject->GetGameFlags() & FVPHYSICS_WAS_THROWN)) {
...
bool CNPC_AntlionGuard::QueryHearSound(CSound *pSound) {
  ...
  if (pSound->SoundType() == SOUND_DANGER &&
      (pSound->m_hOwner != NULL &&
       pSound->m_hOwner->Classify() == CLASS_ANTLION))
    return false;
...
#if HL2_EPISODIC
//---------------------------------------------------------
// Prevent the cavern guard from using stopping paths, as it occasionally forces
// him off the navmesh.
//---------------------------------------------------------
bool CNPC_AntlionGuard::CNavigator::GetStoppingPath(
...
bool CNPC_AntlionGuard::RememberFailedPhysicsTarget(CBaseEntity *pTarget) {
  // Already in the list?
  if (m_FailedPhysicsTargets.Find(pTarget) !=
      m_FailedPhysicsTargets.InvalidIndex())
    return true;
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_AntlionGuard::HandleInteraction(int interactionType, void *data,
                                          CBaseCombatCharacter *sender) {
...
unsigned char CNPC_AntlionGuard::GetBleedingLevel(void) const {
  if (m_iHealth > (m_iMaxHealth >> 1)) { // greater than 50%
    return 0;
  } else if (m_iHealth >
             (m_iMaxHealth >> 2)) { // less than 50% but greater than 25%
    return 1;
  } else {
    return 2;
  }
...
DEFINE_SCHEDULE(
    ...
    "	Tasks"
    "		TASK_SET_FAIL_SCHEDULE					"
    "	SCHEDULE:SCHED_ANTLIONGUARD_CANT_ATTACK"
    "		TASK_ANTLIONGUARD_FIND_PHYSOBJECT			0"
    "		TASK_SET_SCHEDULE					"
    "		SCHEDULE:SCHED_ANTLIONGUARD_PHYSICS_ATTACK"
    ""
    "	Interrupts"
...
//==================================================
// SCHED_ANTLIONGUARD_CANT_ATTACK
//		If we're here, the guard can't chase enemy, can't find a
//physobject to attack with, and can't summon
//==================================================
...
DEFINE_SCHEDULE(
    ...
    "	Tasks"
    "		TASK_SET_ROUTE_SEARCH_TIME		2" // Spend 5 seconds
                                                           // trying to build a
                                                           // path if stuck
...
DEFINE_SCHEDULE(
    ...
    "				0"
    "		TASK_SET_FAIL_SCHEDULE					"
    "			SCHEDULE:SCHED_ANTLIONGUARD_PATROL_RUN"
    "		TASK_ANTLIONGUARD_GET_PATH_TO_NEAREST_NODE		"
    "	500"
    "		TASK_RUN_PATH						"
    "				0"
...
DEFINE_SCHEDULE(
    ...
    "	3" // Spend 3 seconds trying to build a path if stuck
