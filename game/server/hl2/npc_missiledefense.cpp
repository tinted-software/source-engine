...
void CNPC_MissileDefense::FireCannons(void) {
  ...
  // ----------------------------------------------
  trace_t tr;
  AI_TraceLine(GetEnemy()->EyePosition(), GetAbsOrigin(), MASK_SOLID_BRUSHONLY,
  ...
  UTIL_Tracer(vecStart, vecEnd, 0, TRACER_DONT_USE_ATTACHMENT,
              3000 + random->RandomFloat(0, 2000), fSound);
  ...
  UTIL_Tracer(vecStart, vecEnd, 0, TRACER_DONT_USE_ATTACHMENT,
              3000 + random->RandomFloat(0, 2000));
  ...
  if (flDamage > 0) {
    ...
    CalculateBulletDamageForce(&info, GetAmmoDef()->Index("SMG1"), vecDir,
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_MissileDefense::EnemyShootPosition(CBaseEntity *pEnemy,
                                             Vector *vPosition) {
