...
//
//			Primary attack: fires a single high-powered shot, then
//reloads. 			Secondary attack: cycles sniper scope through zoom levels.
//
...
#define SNIPER_BULLET_COUNT_PLAYER                                             \
  1 // Fire n bullets per shot fired by the player.
#define SNIPER_BULLET_COUNT_NPC 1 // Fire n bullets per shot fired by NPCs.
#define SNIPER_TRACER_FREQUENCY_PLAYER                                         \
  0 // Draw a tracer every nth shot fired by the player.
#define SNIPER_TRACER_FREQUENCY_NPC                                            \
  0                       // Draw a tracer every nth shot fired by NPCs.
...
DEFINE_FIELD(m_fNextZoom, FIELD_FLOAT),
    ...
    //-----------------------------------------------------------------------------
    // Maps base activities to weapons-specific ones so our characters do the
    // right things.
    //-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Same as base reload but doesn't change the owner's next attack time.
// This
//			lets us zoom out while reloading. This hack is necessary
//because our 			ItemPostFrame is only called when the owner's next attack time has
//			expired.
...
bool CWeaponSniperRifle::Reload(void) {
  ...
  if (pOwner->GetAmmoCount(m_iPrimaryAmmoType) > 0) {
    int primary =
        MIN(GetMaxClip1() - m_iClip1, pOwner->GetAmmoCount(m_iPrimaryAmmoType));
...
void CWeaponSniperRifle::PrimaryAttack(void) {
  ...
  if (gpGlobals->curtime >= m_flNextPrimaryAttack) {
    ...
    // dvs: does this apply to the sniper rifle? I don't know.
    WeaponSound(SINGLE);
    ...
    // Fire the bullets
    pPlayer->FireBullets(SNIPER_BULLET_COUNT_PLAYER, vecSrc, vecAiming,
                         GetBulletSpread(), MAX_TRACE_LENGTH,
                         m_iPrimaryAmmoType, SNIPER_TRACER_FREQUENCY_PLAYER);

    CSoundEnt::InsertSound(SOUND_COMBAT, GetAbsOrigin(), 600, 0.2);

    QAngle vecPunch(random->RandomFloat(-SNIPER_KICKBACK, SNIPER_KICKBACK), 0,
                    0);
...
//			*pOperator -
//-----------------------------------------------------------------------------
void CWeaponSniperRifle::Operator_HandleAnimEvent(
  ...
  case EVENT_WEAPON_SNIPER_RIFLE_FIRE: {
    ...
    pOperator->FireBullets(SNIPER_BULLET_COUNT_NPC, vecShootOrigin, vecShootDir,
                           vecSpread, MAX_TRACE_LENGTH, m_iPrimaryAmmoType,
                           SNIPER_TRACER_FREQUENCY_NPC);
