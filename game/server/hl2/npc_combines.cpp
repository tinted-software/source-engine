...
//
// Purpose: This is the soldier version of the combine, analogous to the HL1
// grunt.
//
//=============================================================================//
...
#define AE_SOLDIER_BLOCK_PHYSICS                                               \
  20 // trying to block an incoming physics object
...
void CNPC_CombineS::Spawn(void) {
  ...
  if (m_iUseMarch && !HasSpawnFlags(SF_NPC_START_EFFICIENT)) {
    Msg("Soldier %s is set to use march anim, but is not an efficient AI. The "
        "blended march anim can only be used for dead-ahead walks!\n",
        GetDebugName());
  }
#endif
}
...
void CNPC_CombineS::DeathSound(const CTakeDamageInfo &info) {
  ...
  GetSentences()->Speak("COMBINE_DIE", SENTENCE_PRIORITY_INVALID,
                        SENTENCE_CRITERIA_ALWAYS);
}

//-----------------------------------------------------------------------------
// Purpose: Soldiers use CAN_RANGE_ATTACK2 to indicate whether they can throw
//			a grenade. Because they check only every half-second or
//so, this 			condition must persist until it is updated again by the code 			that
//determines whether a grenade can be thrown, so prevent the 			base class from
//clearing it out. (sjb)
//-----------------------------------------------------------------------------
...
void CNPC_CombineS::PrescheduleThink(void) {
  ...
  if( HasCondition( COND_HEAR_PHYSICS_DANGER ) )
  {
          ...
          if( pSound )
          {
                  ...
                  {
                          DevMsg( "OH CRAP!\n" );
                          NDebugOverlay::Line( EyePosition(),
  pSound->GetSoundReactOrigin(), 0, 0, 255, false, 2.0f );
                  }
          }
  }
  */
...
void CNPC_CombineS::BuildScheduleTestBits(void) {
  // Interrupt any schedule with physics danger (as long as I'm not moving or
  // already trying to block)
...
#if HL2_EPISODIC
  // Give each trooper a varied look for his march. Done here because if you do
  // it earlier (eg Spawn, StartTask), the pose param gets overwritten.
  if (m_iUseMarch) {
...
void CNPC_CombineS::Event_Killed(const CTakeDamageInfo &info) {
  ...
  if (!pPlayer) {
    CPropVehicleDriveable *pVehicle =
        dynamic_cast<CPropVehicleDriveable *>(info.GetAttacker());
  ...
  if (pPlayer != NULL) {
...
#endif
      {
        CBaseEntity *pItem = DropItem("item_ammo_ar2_altfire",
                                      WorldSpaceCenter() + RandomVector(-4, 4),
                                      RandomAngle(0, 360));
        ...
        if (pItem) {
          ...
          if (info.GetDamageType() & DMG_DISSOLVE) {
            ...
            if (pAnimating) {
              pAnimating->Dissolve(NULL, gpGlobals->curtime, false,
    ...
    if (HasSpawnFlags(SF_COMBINE_NO_GRENADEDROP) == false) {
      ...
      if (pHL2GameRules->NPC_ShouldDropGrenade(pPlayer)) {
        DropItem("weapon_frag", WorldSpaceCenter() + RandomVector(-4, 4),
                 RandomAngle(0, 360));
...
bool CNPC_CombineS::IsHeavyDamage(const CTakeDamageInfo &info) {
  ...
  if (info.GetDamageType() & DMG_BUCKSHOT) {
    int iHalfMax = sk_plr_dmg_buckshot.GetFloat() *
                   sk_plr_num_shotgun_pellets.GetInt() * 0.5;
...
Activity CNPC_CombineS::NPC_TranslateActivity(Activity eNewActivity) {
  // If the special ep2_outland_05 "use march" flag is set, use the more casual
  // marching anim.
