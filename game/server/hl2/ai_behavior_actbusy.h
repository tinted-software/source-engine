...
public:
  ...
  void ForceActBusy(CAI_ActBusyGoal *pGoal, CAI_Hint *pHintNode = NULL,
                    ...
                    bool bTeleportToBusy = false, bool bUseNearestBusy = false,
  ...
  bool IsActive(void);
  // Returns true if the current NPC is actually acting busy (i.e. inside an act
  // busy anim)
...
private:
  ...
  int m_iNumConsecutivePathFailures; // Count how many times we failed to find a
                                     // path to a node, so we can consider
                                     // teleporting.
...
public:
  CAI_ActBusyGoal() {
    ...
    // Hammer. Take care of old maps by setting this in the constructor. (sjb)
