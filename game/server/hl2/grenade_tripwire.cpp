...
DEFINE_FIELD(m_flPowerUp, FIELD_TIME),
    ...
    // Function Pointers
    DEFINE_FUNCTION(WarningThink), DEFINE_FUNCTION(PowerupThink),
    DEFINE_FUNCTION(RopeBreakThink), DEFINE_FUNCTION(FireThink),
...
void CTripwireGrenade::MakeRope(void) {
  ...
  m_pHook = (CTripwireHook *)CBaseEntity::Create(
      "tripwire_hook", GetLocalOrigin(), GetLocalAngles());
  if (m_pHook) {
    Vector vShootVel =
        800 * (m_vecDir + Vector(0, 0, 0.3) + RandomVector(-0.01, 0.01));
...
void CTripwireGrenade::Attach(void) { StopSound("TripwireGrenade.ShootRope"); }
...
void CTripwireGrenade::RopeBreakThink(void) {
  ...
  trace_t tr;
  UTIL_TraceLine(GetAbsOrigin(), m_pHook->GetAbsOrigin(), MASK_SOLID, this,
...
void CTripwireGrenade::FireMissile(const Vector &vTargetPos) {
  ...
  CGrenadeHomer *pGrenade =
      CGrenadeHomer::CreateGrenadeHomer(MAKE_STRING(GRENADETRIPWIRE_MISSILEMDL),
                                        MAKE_STRING("TripwireGrenade.FlySound"),
                                        GetAbsOrigin(), vec3_angle, edict());
  ...
  pGrenade->Launch(this, NULL, vLaunchVelocity, flHomingSpeed, flGravity,
                   bSmokeTrail);

  // Calculate travel time
  float flTargetDist = (GetAbsOrigin() - vTargetPos).Length();

  pGrenade->m_flDetonateTime =
      gpGlobals->curtime + flTargetDist / TGRENADE_LAUNCH_VEL;
}
...
void CTripwireHook::EndTouch(CBaseEntity *pOther) {
  ...
  if (!m_bAttached) {
    ...
    CTripwireGrenade *pGrenade =
        dynamic_cast<CTripwireGrenade *>((CBaseEntity *)m_hGrenade);
    if (pGrenade) {
      pGrenade->Attach();
    }
  }
...
void CTripwireHook::SetVelocity(const Vector &velocity,
