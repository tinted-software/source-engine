...
// Models!
//-----------------------------------------------------------------------------
#define ENV_HEADCRABCANISTER_MODEL                                             \
  "models/props_combine/headcrabcannister01a.mdl"
#define ENV_HEADCRABCANISTER_BROKEN_MODEL                                      \
  "models/props_combine/headcrabcannister01b.mdl"
#define ENV_HEADCRABCANISTER_SKYBOX_MODEL                                      \
  "models/props_combine/headcrabcannister01a_skybox.mdl"
#define ENV_HEADCRABCANISTER_INCOMING_SOUND_TIME 1.0f

ConVar sk_env_headcrabcanister_shake_amplitude(
    "sk_env_headcrabcanister_shake_amplitude", "50");
ConVar
    sk_env_headcrabcanister_shake_radius("sk_env_headcrabcanister_shake_radius",
                                         "1024");
ConVar sk_env_headcrabcanister_shake_radius_vehicle(
...
private:
  ...
  void ComputeWorldEntryPoint(Vector *pStartPosition, QAngle *pStartAngles,
                              Vector *pStartDirection);
...
DEFINE_FIELD(m_bLanded, FIELD_BOOLEAN), DEFINE_EMBEDDED(m_Shared),
    ...
    DEFINE_KEYFIELD(m_iszLaunchPositionName, FIELD_STRING,
                    "LaunchPositionName"),
    ...
    DEFINE_KEYFIELD(m_nSkyboxCannisterCount, FIELD_INTEGER,
...
SendPropDataTable(SENDINFO_DT(m_Shared),
                  &REFERENCE_SEND_TABLE(DT_EnvHeadcrabCanisterShared)),
...
void CEnvHeadcrabCanister::Spawn(void) {
  ...
  if (m_iszLaunchPositionName != NULL_STRING) {
    ...
    if (!HasSpawnFlags(SF_LAND_AT_INITIAL_POSITION)) {
      ...
      trace_t trace;
      UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + vecForward * 10000,
...
// Figure out where we enter the world
//-----------------------------------------------------------------------------
void CEnvHeadcrabCanister::ComputeWorldEntryPoint(Vector *pStartPosition,
                                                  QAngle *pStartAngles,
                                                  Vector *pStartDirection) {
  SetupWorldModel();
  ...
  // Raycast up to the place where we should start from (start raycast slightly
  // off the ground, since it'll be buried in the ground oftentimes)
  trace_t tr;
  CTraceFilterWorldOnly filter;
  UTIL_TraceLine(GetAbsOrigin() + vecForward * 100,
                 GetAbsOrigin() + vecForward * 10000, CONTENTS_SOLID, &filter,
                 &tr);
...
CSkyCamera *CEnvHeadcrabCanister::PlaceCanisterInWorld() {
  ...
  if (m_iszLaunchPositionName != NULL_STRING) {
    ...
    CBaseEntity *pLaunchPos =
        gEntList.FindEntityByName(NULL, m_iszLaunchPositionName);
    if (!pLaunchPos) {
      Warning("%s (%s) could not find an entity matching LaunchPositionName of "
              "'%s'\n",
              GetEntityName().ToCStr(), GetDebugName(),
              STRING(m_iszLaunchPositionName));
      SUB_Remove();
    } else {
      SetupWorldModel();
      ...
      m_Shared.InitInWorld(gpGlobals->curtime, pLaunchPos->GetAbsOrigin(),
                           GetAbsAngles(), vecImpactDirection,
    ...
    m_Shared.InitInSkybox(gpGlobals->curtime, m_vecImpactPosition,
                          GetAbsAngles(), vecForward, m_vecImpactPosition,
                          pCamera->m_skyboxData.origin,
                          pCamera->m_skyboxData.scale);
    ...
    if (pCamera) {
      m_Shared.InitInSkybox(gpGlobals->curtime, vecStartPosition,
                            vecStartAngles, vecDirection, m_vecImpactPosition,
                            pCamera->m_skyboxData.origin,
                            pCamera->m_skyboxData.scale);
...
void CEnvHeadcrabCanister::InputFireCanister(inputdata_t &inputdata) {
  ...
  m_hTrail = CSpriteTrail::SpriteTrailCreate("sprites/smoke.vmt",
                                             GetAbsOrigin(), true);
  ...
  if (pCamera && m_Shared.IsInSkybox()) {
    m_hTrail->SetSkybox(pCamera->m_skyboxData.origin,
                        pCamera->m_skyboxData.scale);
  }
...
// Test for impact!
//-----------------------------------------------------------------------------
void CEnvHeadcrabCanister::TestForCollisionsAgainstEntities(
    const Vector &vecEndPosition) {
  // Debugging!!
  //	NDebugOverlay::Box( GetAbsOrigin(), m_vecMin * 0.5f, m_vecMax * 0.5f,
  //255, 255, 0, 0, 5 ); 	NDebugOverlay::Box( vecEndPosition, m_vecMin, m_vecMax,
  //255, 0, 0, 0, 5 );
  ...
  for (int iEntity = collideList.m_Entities.Count(); --iEntity >= 0;) {
    ...
    if (pEntity->m_takedamage && (m_flDamage != 0.0f)) {
      ...
      CalculateExplosiveDamageForce(&info, vecForceDir,
...
void CEnvHeadcrabCanister::TestForCollisionsAgainstWorld(
    const Vector &vecEndPosition) {
  // Splash damage!
  ...
  CBaseEntity *pEntity;
  for (CEntitySphereQuery sphere(vecEndPosition, flDamageRadius);
    ...
    if (pEntity->IsPlayer() &&
        !(static_cast<CBasePlayer *>(pEntity)->IsInAVehicle())) {
      if (vecSegment.z < 0.1f) {
        vecSegment.z = 0.1f;
        VectorNormalize(vecSegment);
      }
      float flAmount =
          SimpleSplineRemapVal(flScale, 0.0f, 1.0f, 250.0f, 1000.0f);
...
void CEnvHeadcrabCanister::HeadcrabCanisterSpawnHeadcrabThink() {
  ...
  if (GetAttachment(nHeadCrabAttachment, vecSpawnPosition, vecSpawnAngles)) {
    ...
    // NOTE: Hierarchical attachment is necessary here to get the animations to
    // work
  ...
  if (m_nHeadcrabCount != 0) {
    ...
    SetContextThink(&CEnvHeadcrabCanister::HeadcrabCanisterSpawnHeadcrabThink,
                    gpGlobals->curtime + flWaitTime, s_pHeadcrabThinkContext);
  } else {
...
void CEnvHeadcrabCanister::StartSpawningHeadcrabs(float flDelay) {
  ...
  if (m_nHeadcrabCount != 0) {
    SetContextThink(&CEnvHeadcrabCanister::HeadcrabCanisterSpawnHeadcrabThink,
                    gpGlobals->curtime + flDelay, s_pHeadcrabThinkContext);
  }
}
...
void CEnvHeadcrabCanister::WaitForOpenSequenceThink() {
  ...
  if ((GetSequence() == LookupSequence("open")) && IsSequenceFinished()) {
    ...
    SetContextThink(&CEnvHeadcrabCanister::WaitForOpenSequenceThink,
                    gpGlobals->curtime + 0.01f, s_pOpenThinkContext);
  }
}
...
void CEnvHeadcrabCanister::Detonate() {
  ...
  if (!HasSpawnFlags(SF_NO_SHAKE)) {
    ...
    UTIL_ScreenShake(m_vecImpactPosition,
                     sk_env_headcrabcanister_shake_amplitude.GetFloat(), 150.0,
                     1.0, shakeRadius, SHAKE_START);
  }
  ...
  if (!HasSpawnFlags(SF_NO_IMPACT_EFFECTS)) {
    ...
    ExplosionCreate(m_vecImpactPosition, GetAbsAngles(), this, 50.0f, 500.0f,
                    SF_ENVEXPLOSION_NODLIGHTS | SF_ENVEXPLOSION_NOSPARKS |
                        SF_ENVEXPLOSION_NODAMAGE | SF_ENVEXPLOSION_NOSOUND,
                    1300.0f);

    // Dust explosion
    AR2Explosion *pExplosion =
        AR2Explosion::CreateAR2Explosion(m_vecImpactPosition);
...
//-----------------------------------------------------------------------------
// Purpose: This think function simulates (moves/collides) the HeadcrabCanister
// while in
//          the world.
//-----------------------------------------------------------------------------
void CEnvHeadcrabCanister::HeadcrabCanisterWorldThink(void) {
  ...
  if (!m_bIncomingSoundStarted && !HasSpawnFlags(SF_NO_IMPACT_SOUND)) {
    float flDistSq =
        ENV_HEADCRABCANISTER_INCOMING_SOUND_TIME * m_Shared.m_flFlightSpeed;
    flDistSq *= flDistSq;
  ...
  if (!m_bHasDetonated) {
    if (vecEndPosition.DistToSqr(m_vecImpactPosition) <
...
//-----------------------------------------------------------------------------
// Purpose: This think function should be called at the time when the
// HeadcrabCanister
...
void CEnvHeadcrabCanister::HeadcrabCanisterSkyboxOnlyThink(void) {
  ...
  float flRefireTime =
      random->RandomFloat(m_flMinRefireTime, m_flMaxRefireTime) +
      ENV_HEADCRABCANISTER_TRAIL_TIME;
...
//			bAlways -
//-----------------------------------------------------------------------------
void CEnvHeadcrabCanister::SetTransmit(CCheckTransmitInfo *pInfo,
                                       bool bAlways) {
