...
DEFINE_FIELD(m_tSlamState, FIELD_INTEGER),
    ...
    END_DATADESC()
        ...
        IMPLEMENT_SERVERCLASS_ST(CWeapon_SLAM, DT_Weapon_SLAM) END_SEND_TABLE()
...
// Output  :
//------------------------------------------------------------------------------
void CWeapon_SLAM::SetPickupTouch(void) { SetTouch(SlamTouch); }
...
void CWeapon_SLAM::SlamTouch(CBaseEntity *pOther) {
  ...
  if (pOther->GetFlags() & FL_CLIENT) {
    ...
    // ------------------------------------------------
    CWeapon_SLAM *oldWeapon =
        (CWeapon_SLAM *)pBCC->Weapon_OwnsThisType(GetClassname());
    if (oldWeapon != this) {
      UTIL_Remove(this);
    } else {
...
void CWeapon_SLAM::SLAMThink(void) {
  if (m_flWallSwitchTime <= gpGlobals->curtime) {
    ...
    if ((m_tSlamState != SLAM_TRIPMINE_READY) &&
        (pOwner && pOwner->GetAmmoCount(m_iSecondaryAmmoType) > 0)) {
      if (CanAttachSLAM()) {
      ...
      if (pOwner->GetAmmoCount(m_iSecondaryAmmoType) > 0) {
        ...
        case SLAM_SATCHEL_ATTACH: {
          if (m_bNeedDetonatorHolster) {
            ...
            iAnim = m_bNeedDetonatorDraw ? ACT_SLAM_DETONATOR_STICKWALL_DRAW
        ...
        case SLAM_SATCHEL_THROW: {
          if (m_bNeedDetonatorHolster) {
            ...
            iAnim = m_bNeedDetonatorDraw ? ACT_SLAM_DETONATOR_THROW_DRAW
                                         : ACT_SLAM_THROW_DRAW;
            m_bNeedDetonatorDraw = false;
          } else {
    ...
    // If I don't need to reload just do the appropriate idle
      ...
      case SLAM_SATCHEL_THROW: {
        if (m_bNeedDetonatorHolster) {
          ...
          iAnim =
              m_bDetonatorArmed ? ACT_SLAM_THROW_IDLE : ACT_SLAM_THROW_ND_IDLE;
          m_flWallSwitchTime = 0;
        }
      } break;
      case SLAM_SATCHEL_ATTACH: {
