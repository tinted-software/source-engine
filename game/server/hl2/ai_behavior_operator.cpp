...
DEFINE_FIELD(m_hGoalEntity, FIELD_EHANDLE),
    ...
    DEFINE_EMBEDDED(m_WatchSeeEntity), END_DATADESC();
...
bool CAI_OperatorBehavior::CanSeePositionEntity() {
  ...
  bool bSpotted =
      (pOuter->EyePosition().DistToSqr(m_hPositionEnt->GetAbsOrigin()) <=
       POSITION_ENT_ALWAYS_SEE_DIST);
  if (!bSpotted) {
    bSpotted = (pOuter->FInViewCone(m_hPositionEnt) &&
                pOuter->FVisible(m_hPositionEnt));
  }

  if (bSpotted) {
    // If we haven't seen it up until now, start a timer. If we have seen it,
    // wait for the timer to finish. This prevents edge cases where turning on
    // the flashlight makes NPC spot the position entity a frame before she
    // spots an enemy.
...
void CAI_OperatorBehavior::GatherConditionsNotActive() {
  if (m_hPositionEnt) {
    // If we're not currently the active behavior, we have a position ent, and
    // the NPC can see it, coax the AI out of IDLE/ALERT schedules with this
    // condition.
...
void CAI_OperatorBehavior::StartTask(const Task_t *pTask) {
  ...
  case TASK_OPERATOR_GET_PATH_TO_POSITION: {
    ...
    if (GetNavigator()->SetGoal(goal) == false) {
      ...
      m_hTargetObject->GetAbsOrigin(), AIN_DEF_ACTIVITY, 256 ); if (
      GetNavigator()->SetGoal( nearGoal, AIN_CLEAR_PREVIOUS_STATE ) )
      {
              //FIXME: HACK! The internal pathfinding is setting this without
      our consent, so override it! ClearCondition( COND_TASK_FAILED );
              GetNavigator()->SetArrivalDirection(
      m_hTargetObject->GetAbsAngles() ); TaskComplete(); return;
      }
      */
    }
...
CAI_OperatorGoal *CAI_OperatorBehavior::GetGoalEntity() {
  CAI_OperatorGoal *pGoal =
      dynamic_cast<CAI_OperatorGoal *>(m_hGoalEntity.Get());
...
// Purpose:
//-----------------------------------------------------------------------------
void CAI_OperatorBehavior::SetParameters(CAI_OperatorGoal *pGoal,
...
bool CAI_OperatorBehavior::CanSelectSchedule() {
  ...
  if (GetOuter()->m_NPCState == NPC_STATE_COMBAT ||
      GetOuter()->m_NPCState == NPC_STATE_SCRIPT)
    return false;

  // Don't grab NPCs who have been in combat recently
  if (GetOuter()->GetLastEnemyTime() &&
      (gpGlobals->curtime - GetOuter()->GetLastEnemyTime()) < 3.0)
    return false;
...
int CAI_OperatorBehavior::SelectSchedule() {
  ...
  if (GetGoalEntity() &&
      GetGoalEntity()->GetState() != OPERATOR_STATE_FINISHED) {
...
DEFINE_SCHEDULE(SCHED_OPERATOR_OPERATE,
                "	Tasks"
                "		TASK_WAIT				"
                "	0.2" // Allow pending entity I/O to settle
