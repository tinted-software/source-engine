...
private:
  ...
  void ThumperUse(CBaseEntity *pActivator, CBaseEntity *pCaller,
...
void CPortableThumper::ThumperUse(CBaseEntity *pActivator, CBaseEntity *pCaller,
                                  USE_TYPE useType, float value) {
...
void CPortableThumper::ThumpThink(void) {
  ...
  CSoundEnt::InsertSound(SOUND_THUMPER, GetAbsOrigin(), thumpRadius.GetInt(),
                         0.2, this);
}
...
void CWeaponThumper::PrimaryAttack(void) {
  ...
  UTIL_TraceLine(vecStart, vecStart - Vector(0, 0, 128), MASK_SHOT, pPlayer,
...
void CWeaponThumper::PrimaryAttack( void )
{
        ...
        UTIL_TraceLine( vecStart, vecStart - Vector( 0, 0, 128 ), MASK_SHOT,
m_hOwner, COLLISION_GROUP_NONE, &tr );
