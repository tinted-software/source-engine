...
class CNPC_Zombine : public CAI_BlendingHost<CNPC_BaseZombie>,
...
public:
  ...
  // Called when we want to let go of a grenade and let the physcannon pick it
  // up.
...
void CNPC_Zombine::PrescheduleThink(void) {
  ...
  if (HasGrenade()) {
    CSoundEnt::InsertSound(SOUND_DANGER,
                           GetAbsOrigin() + GetSmoothedVelocity() * 0.5f, 256,
                           0.1, this, SOUNDENT_CHANNEL_ZOMBINE_GRENADE);

    if (IsSprinting() && GetEnemy() &&
...
int CNPC_Zombine::MeleeAttack1Conditions(float flDot, float flDist) {
  ...
  if (HasGrenade()) {
    // Adrian: stop spriting if we get close enough to melee and we have a
    // grenade this gives NPCs time to move away from you (before it was almost
    // impossible cause of the high sprint speed)
...
void CNPC_Zombine::GatherGrenadeConditions(void) {
  ...
  if (pPlayer && pPlayer->FVisible(this)) {
    float flLengthToPlayer =
        (pPlayer->GetAbsOrigin() - GetAbsOrigin()).Length();
    ...
    if (flLengthToPlayer <= GRENADE_PULL_MAX_DISTANCE &&
        flLengthToEnemy <= GRENADE_PULL_MAX_DISTANCE) {
...
//-----------------------------------------------------------------------------
bool CNPC_Zombine::HandleInteraction(int interactionType, void *data,
                                     CBaseCombatCharacter *sourceEnt) {
...
void CNPC_Zombine::TraceAttack(const CTakeDamageInfo &info,
...
void CNPC_Zombine::HandleAnimEvent(animevent_t *pEvent) {
  if (pEvent->event == AE_ZOMBINE_PULLPIN) {
    ...
    CBaseGrenade *pGrenade =
        Fraggrenade_Create(vecStart, vec3_angle, vec3_origin,
                           AngularImpulse(0, 0, 0), this, 3.5f, true);
...
bool CNPC_Zombine::AllowedToSprint(void) {
  ...
  if (pPlayer) {
    if (HL2GameRules()->IsAlyxInDarknessMode() &&
        pPlayer->FlashlightIsOn() == false) {
      iChance = SPRINT_CHANCE_VALUE_DARKNESS;
    }

    // Bigger chance of this happening if the player is not looking at the
    // zombie
  ...
  if ((GetHealth() > GetMaxHealth() * 0.5f)) {
    if (IsStrategySlotRangeOccupied(SQUAD_SLOT_ZOMBINE_SPRINT1,
                                    SQUAD_SLOT_ZOMBINE_SPRINT2) == true)
      return false;
...
void CNPC_Zombine::Sprint(bool bMadSprint) {
  ...
  OccupyStrategySlotRange(SQUAD_SLOT_ZOMBINE_SPRINT1,
                          SQUAD_SLOT_ZOMBINE_SPRINT2);
      ...
      if (HasGrenade()) {
        if (IsSprinting()) {
          GetNavigator()->SetMovementActivity(
              (Activity)ACT_ZOMBINE_GRENADE_RUN);
        } else {
          GetNavigator()->SetMovementActivity(
              (Activity)ACT_ZOMBINE_GRENADE_WALK);
        }
...
//-----------------------------------------------------------------------------
bool CNPC_Zombine::ShouldBecomeTorso(const CTakeDamageInfo &info,
                                     float flDamageThreshold) {
  return false;
}
...
DEFINE_SCHEDULE(SCHED_ZOMBINE_PULL_GRENADE,
                ...
                "	Tasks"
                "		TASK_PLAY_SEQUENCE			"
                "		ACTIVITY:ACT_ZOMBINE_GRENADE_PULL"
