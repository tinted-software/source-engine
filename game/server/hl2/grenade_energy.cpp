...
// Output  :
//------------------------------------------------------------------------------
void CGrenadeEnergy::Shoot(CBaseEntity *pOwner, const Vector &vStart,
                           Vector vVelocity) {
  CGrenadeEnergy *pEnergy =
      (CGrenadeEnergy *)CreateEntityByName("grenade_energy");
  pEnergy->Spawn();
...
void CGrenadeEnergy::Animate(void) {
  float flLifeLeft =
      1 - (gpGlobals->curtime - m_flLaunchTime) / ENERGY_GRENADE_LIFETIME;
...
void CGrenadeEnergy::GrenadeEnergyTouch(CBaseEntity *pOther) {
  if (pOther->m_takedamage) {
    ...
    if (pOther->GetFlags() & (FL_CLIENT)) {
      ...
      pPlayer->m_Local.m_vecPunchAngle.SetX(
          flKick * (random->RandomInt(0, 1) == 1) ? -1 : 1);
      pPlayer->m_Local.m_vecPunchAngle.SetY(
          flKick * (random->RandomInt(0, 1) == 1) ? -1 : 1);
    }
    ...
    trace_t tr;
    tr = GetTouchTrace();
    CTakeDamageInfo info(this, GetThrower(), m_flDamage * flLifeLeft,
                         DMG_SONIC);
