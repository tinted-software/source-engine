...
void CNPC_Houndeye::HandleAnimEvent(animevent_t *pEvent) {
  ...
  case HOUND_AE_LEAP_HIT: {
    ...
    if (GetEnemy() != NULL) {
      ...
      //
      // How fast does the houndeye need to travel to reach my enemy's eyes
      // given gravity?
      //
...
// DeathSound
//=========================================================
void CNPC_Houndeye::DeathSound(void) { EmitSound("NPC_Houndeye.Die"); }
...
void CNPC_Houndeye::NPCThink(void) {
  ...
  /*
          ...
          {
                  NDebugOverlay::Box(GetAbsOrigin(), GetHullMins(),
     GetHullMaxs(), 0, 255, 0, 0, 0);
          }
          else
          {
                  NDebugOverlay::Box(GetAbsOrigin(), GetHullMins(),
     GetHullMaxs(), 255, 0, 0, 0, 0);
          }
  */
  BaseClass::NPCThink();
}
...
int CNPC_Houndeye::OnTakeDamage_Alive(const CTakeDamageInfo &info) {
  if (m_pSquad && random->RandomInt(0, 10) == 10) {
    ...
    m_pSquad->BroadcastInteraction(g_interactionHoundeyeGroupRetreat, NULL,
                                   this);
  }
...
void CNPC_Houndeye::SonicAttack(void) {
  ...
  m_pEnergyWave =
      (CEnergyWave *)Create("energy_wave", EyePosition(), GetLocalAngles());
  m_flEndEnergyWaveTime = gpGlobals->curtime + 1; //<<TEMP>> magic
  m_pEnergyWave->SetAbsVelocity(100 * vFacingDir);

  CBaseEntity *pEntity = NULL;
  // iterate on all entities in the vicinity.
  for (CEntitySphereQuery sphere(GetAbsOrigin(), HOUNDEYE_MAX_ATTACK_RADIUS);
    ...
    if (pEntity->m_takedamage != DAMAGE_NO || pPhysicsObject) {
      ...
      if (!FVisible(pEntity)) {
        if (pEntity->IsPlayer()) {
          ...
          // restrict it to clients so that monsters in other parts of the level
          // don't take the damage and get pissed.
      ...
      if (pEntity->m_takedamage != DAMAGE_NO) {
        CTakeDamageInfo info(
            this, this, flDamageAdjuster * sk_Houndeye_dmg_blast.GetFloat(),
        ...
        if (pEntity->IsPlayer()) {
          ...
          pEntity->ViewPunch(QAngle(random->RandomInt(-20, 20), 0,
                                    random->RandomInt(-20, 20)));
        }
      }
      // ------------------------------
      //  Apply physics foces
      ...
      if (pPhysicsObject) {
        ...
        pPhysicsObject->ApplyForceCenter((vEntDir + Vector(0, 0, 0.2)) *
                                         flForce);
...
void CNPC_Houndeye::StartTask(const Task_t *pTask) {
  ...
  case TASK_HOUND_GET_PATH_TO_CIRCLE: {
    if (GetEnemy() == NULL) {
      ...
      if (GetNavigator()->SetRadialGoal(vTargetPos, random->RandomInt(50, 500),
                                        90, 175, m_bLoopClockwise)) {
        TaskComplete();
        return;
      }
  ...
  case TASK_GET_PATH_TO_ENEMY_LOS: {
    ...
    if (GetEnemy() != NULL) {
      foundLos = GetTacticalServices()->FindLos(
          GetEnemyLKP(), GetEnemy()->EyePosition(), flMinRange, flMaxRange, 0.0,
          &posLos);
    } else {
    ...
    if (HasCondition(COND_CAN_RANGE_ATTACK1)) {
      if (m_pSquad && random->RandomInt(0, 4) == 0) {
        if (!IsAnyoneInSquadAttacking()) {
          ...
          m_pSquad->BroadcastInteraction(g_interactionHoundeyeGroupAttack, NULL,
                                         this);
          return SCHED_HOUND_GROUP_ATTACK;
        }
      }
      ...
      if (m_pSquad && random->RandomInt(0, 5) == 0) {
        if (!IsAnyoneInSquadAttacking()) {
          ...
          m_pSquad->BroadcastInteraction(g_interactionHoundeyeGroupRalley, NULL,
                                         this);
...
//-----------------------------------------------------------------------------
bool CNPC_Houndeye::HandleInteraction(int interactionType, void *data,
...
AI_DEFINE_SCHEDULE(
    ...
    "SCHEDULE:SCHED_HOUND_ATTACK_STRAFE_REVERSE"
...
AI_DEFINE_SCHEDULE(
    ...
    "		TASK_WAIT					1"
    "		TASK_SET_SCHEDULE			"
    "SCHEDULE:SCHED_HOUND_RANGE_ATTACK1"
    ""
    "	Interrupts "
    "		COND_NEW_ENEMY"
    "		COND_ENEMY_DEAD"
...
AI_DEFINE_SCHEDULE(
    ...
    "		TASK_SET_SCHEDULE				"
    "SCHEDULE:SCHED_HOUND_COVER_WAIT"
    ""
    "	Interrupts"
    "		COND_NEW_ENEMY");
