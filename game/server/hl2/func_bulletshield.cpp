...
bool IntersectRayWithOBB( const Vector &vecRayStart, const Vector &vecRayDelta,
        const matrix3x4_t &matOBBToWorld, const Vector &vecOBBMins, const Vector
&vecOBBMaxs, float flTolerance, CBaseTrace *pTrace );
...
bool IntersectRayWithOBB( const Vector &vecRayOrigin, const Vector &vecRayDelta,
        ...
        const Vector &vecOBBMins, const Vector &vecOBBMaxs, float flTolerance,
CBaseTrace *pTrace );

bool IntersectRayWithOBB( const Ray_t &ray, const Vector &vecBoxOrigin, const
QAngle &angBoxRotation, const Vector &vecOBBMins, const Vector &vecOBBMaxs,
...
bool IntersectRayWithOBB( const Vector &vecRayStart, const Vector &vecRayDelta,
        const matrix3x4_t &matOBBToWorld, const Vector &vecOBBMins, const Vector
&vecOBBMaxs, float flTolerance, BoxTraceInfo_t *pTrace );
        */

bool CFuncBulletShield::TestCollision(const Ray_t &ray, unsigned int mask,
                                      trace_t &trace) {
  ...
  if ((mask & MASK_SHOT) == MASK_SHOT) {
    ...
    return IntersectRayWithOBB(ray, pCol->GetCollisionOrigin(),
                               pCol->GetCollisionAngles(), pCol->OBBMins(),
    ...
    if ( pModel && pModel->type == mod_brush )
    {
            ...
            CM_TransformedBoxTrace( ray, nHeadNode, fMask,
    this->GetCollisionOrigin(), this->GetCollisionAngles(), *pTrace ); return
    true;
    }
    return false;
    */
