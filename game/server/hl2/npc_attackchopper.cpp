...
ConVar
    sk_helicopter_burstcount("sk_helicopter_burstcount", "12", 0,
                             "How many shot bursts to fire after charging up. "
                             "The bigger the number, the longer the firing is");
...
ConVar
    sk_helicopter_grenaderadius("sk_helicopter_grenaderadius", "275.0", 0,
...
ConVar
    sk_npc_dmg_helicopter_to_plr("sk_npc_dmg_helicopter_to_plr", "3", 0,
...
ConVar
    g_helicopter_bullrush_bomb_speed("g_helicopter_bullrush_bomb_speed",
                                     "850.0", 0,
                                     "The maximum distance the player can be "
                                     "from the chopper before it stops firing");
...
DEFINE_ENTITYFUNC(FlyTouch),
    ...
    : m_bNonCombat(false), m_flGracePeriod(2.0f),
...
//-----------------------------------------------------------------------------
CBaseEntity *
CNPC_AttackHelicopter::FindTrackBlocker(const Vector &vecViewPoint,
                                        const Vector &vecTargetPos) {
...
//-----------------------------------------------------------------------------
CGrenadeHelicopter *
CNPC_AttackHelicopter::SpawnBombEntity(const Vector &vecPos,
                                       const Vector &vecVelocity) {
...
//-----------------------------------------------------------------------------
inline bool
CNPC_AttackHelicopter::ShouldTriggerDamageEffect(int nPrevHealth,
...
void CNPC_AttackHelicopter::UpdateEnemyLeading(void) {
  ...
  case ATTACK_MODE_TRAIL_VEHICLE:
    if ((flSpeed <= MIN_ENEMY_SPEED) && (bEnemyInVehicle)) {
      flLeadingDist =
          CreepTowardEnemy(flSpeed, 150.0f, MIN_ENEMY_SPEED, 500.0f, -1000.0f);
    } else {
      flLeadingDist = ClampSplineRemapVal(flSpeedAlongPath, -600.0f, -200.0f,
                                          -2500.0f, -1000.0f);
    }
...
void CGrenadeHelicopter::BecomeActive() {
  ...
  if (!bMegaBomb) {
    SetContextThink(&CGrenadeHelicopter::RampSoundThink,
                    gpGlobals->curtime + GetBombLifetime() -
                        BOMB_RAMP_SOUND_TIME,
                    s_pRampSoundContext);
