...
ConVar
    sk_citizen_heal_toss_player_delay("sk_citizen_heal_toss_player_delay", "26",
                                      FCVAR_NONE,
...
void CNPC_Citizen::PostNPCInit() {
  ...
  if (IsInPlayerSquad()) {
    if (m_pSquad->NumMembers() > MAX_PLAYER_SQUAD)
      DevMsg("Error: Spawning citizen in player squad but exceeds squad limit "
             "of %d members\n",
             MAX_PLAYER_SQUAD);
...
int CNPC_Citizen::SelectFailSchedule(int failedSchedule, int failedTask,
  ...
  case SCHED_NEW_WEAPON:
    ...
    // assumption that the citizen would be able to pick up the weapon that they
    // found.
...
//don't 			try to send all 3 selectedcitizens to pick up the same gun.
//-----------------------------------------------------------------------------
...
DEFINE_SCHEDULE(
    ...
    "		TASK_FACE_IDEAL						0"
    //		"		TASK_SAY_HEAL
    //0" 		"		TASK_PLAY_SEQUENCE_FACE_TARGET ACTIVITY:ACT_ARM"
    "		TASK_CIT_HEAL						"
    "	0"
