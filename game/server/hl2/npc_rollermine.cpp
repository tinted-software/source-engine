...
int CNPC_RollerMine::TranslateSchedule(int scheduleType) {
  ...
  case SCHED_ROLLERMINE_RANGE_ATTACK1:
    if (HasCondition(COND_ENEMY_OCCLUDED)) {
      ...
      // the rollermine could end up here with instructions to drive towards the
      // target, although the target is not in sight. Nudge around randomly
      // until we're back on the nodegraph.
...
void CNPC_RollerMine::StartTask(const Task_t *pTask) {
  ...
  case TASK_WAIT_FOR_MOVEMENT: {
    // TASK_RUN_PATH and TASK_WALK_PATH work different on the rollermine and run
    // until movement is done, so movement is already complete when entering
    // this task.
    TaskComplete();
  } break;
...
void CNPC_RollerMine::RunTask(const Task_t *pTask) {
  ...
  case TASK_WALK_PATH:
    ...
    {
      float yaw =
          UTIL_VecToYaw(GetNavigator()->GetCurWaypointPos() - GetLocalOrigin());
  ...
  case TASK_ROLLERMINE_CHARGE_ENEMY: {
    ...
    if (EnemyInVehicle()) {
      ...
      CalcClosestPointOnLine2D(vecOrigin2D, vecTargetPosition2D, vecProjected2D,
                               vecIntercept2D, &flT);
    ...
    m_RollerController.m_vecAngular =
        WorldToLocalRotation(SetupMatrixAngles(GetLocalAngles()), vecCompensate,
                             m_flForwardSpeed * -0.75);
    m_RollerController.m_vecAngular +=
        WorldToLocalRotation(SetupMatrixAngles(GetLocalAngles()), vecRight,
    ...
    if ((m_bIsOpen == false) &&
      ...
      if (flDistance >= flThreshold) {
        ...
        // Keep trying to hop when we're ramming a vehicle, so we're visible to
        // the player
