...
private:
  ...
  virtual bool GetSpitVector(const Vector &vecStartPos, const Vector &vecTarget,
                             Vector *vecOut);
  ...
  Vector m_vecSaveSpitVelocity; // Saved when we start to attack and used if we
  ...
  float m_flEludeDistance; // Distance until the antlion will consider himself
                           // "eluded" if so flagged
  bool m_bLeapAttack;
  bool m_bDisableJump;
  float m_flTimeDrown;
  float m_flTimeDrownSplash;
  bool m_bDontExplode; // Suppresses worker poison burst when drowning, failing
                       // to unburrow, etc.
  bool m_bLoopingStarted;
  bool m_bSuppressUnburrowEffects; // Don't kick up dust when spawning
#if HL2_EPISODIC
  bool
      m_bHasDoneAirAttack; ///< only allowed to apply this damage once per glide
#endif
