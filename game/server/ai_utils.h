...
private:
  ...
  float m_invDecay;  //	0.8	// maintain X percent of velocity when slowing
                     //down
  float m_decayTime; //	0.4161	// Sum( 1..cycle, HEIGHTINVDECAY^cycle )
  float m_accel;     //	0.5		// accel toward maxVelocity by X percent
                     //each cycle
...
struct AI_FreePassParams_t {
  ...
  float coverDist; // When hiding, how far from an obstructing object needed to
                   // be considered in cover
  ...
  float
      peekEyeDistZ; // how far below eye position to test eyes (handles peek up)
