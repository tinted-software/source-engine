...
public:
  ...
  // portal off for purposes of flowing through portals to determine which areas
  ...
  //   and dareaportal_t::m_PortalKey). Each dareaportal_t leads into one of the
  //   connected areas and the dareaportal_t's have opposite-facing planes.
  //
  ...
  //	 |------------------|
  //
  // "aaaa" and "bbbb" each represent a different dareaportal_t, (aaa leads into
  // area 2 and bbb leads into area 1). They both represent the same portal key
  // though (call it PortalKey1).
  //
  // When standing in area 1 (where the "area 1" text is), the engine will check
  // aaaa and it'll notice that you're on the wrong side of aaaa to be looking
  // through it, so it'll say that PortalKey1 is closed for purposes of flowing
  // through areas on the server (it turns out you can get to area 2 through the
  // portal right above the "area 1" text).
  //
