...
private:
  ...
  friend CBaseEntity *CreateInfoCameraLink(CBaseEntity *pTarget,
                                           CPointCamera *pCamera);
  friend void PointCameraSetupVisibility(CBaseEntity *pPlayer, int area,
...
//-----------------------------------------------------------------------------
CInfoCameraLink::CInfoCameraLink() { g_InfoCameraLinkList.AddToTail(this); }
...
CBaseEntity *CreateInfoCameraLink(CBaseEntity *pTarget, CPointCamera *pCamera) {
  CInfoCameraLink *pInfoCameraLink =
      (CInfoCameraLink *)CreateEntityByName("info_camera_link");
  if (!pInfoCameraLink)
    return NULL;
...
void PointCameraSetupVisibility(CBaseEntity *pPlayer, int area,
                                unsigned char *pvs, int pvssize) {
  for (CPointCamera *pCameraEnt = GetPointCameraList(); pCameraEnt != NULL;
       pCameraEnt = pCameraEnt->m_pNext) {
    pCameraEnt->SetActive(false);
  }

  intp nNext;
  for (intp i = g_InfoCameraLinkList.Head();
       i != g_InfoCameraLinkList.InvalidIndex(); i = nNext) {
    ...
    if (engine->CheckAreasConnected(area,
                                    pTargetEnt->NetworkProp()->AreaNum())) {
