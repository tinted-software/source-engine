...
public:
  ...
  void SynchNewSequence(CAI_BaseNPC::SCRIPTSTATE newState, string_t iszSequence,
...
private:
  ...
  string_t m_iszEntry; // String index for animation that must be played before
                       // entering the main action anim
  string_t m_iszPreIdle;  // String index for idle animation to play before
                          // playing the action anim (only played while waiting
                          // for the script to begin)
  ...
  string_t
      m_iszNextScript;  // Name of the script to run immediately after this one.
  ...
  bool m_bDelayed; // This moderately hacky hack ensures that we don't calls to
                   // DelayStart(true) or DelayStart(false) twice in succession.
                   // This is necessary because we didn't want to remove the
                   // call to DelayStart(true) from StartScript, even though
                   // DelayStart(true) is called from TASK_PRE_SCRIPT. All of
                   // this is necessary in case the NPCs schedule gets cleared
                   // during the script and then they reselect the schedule to
                   // play the script. Without this you can get NPCs stuck with
                   // m_iDelay = -1

  float m_startTime; // Time when script actually started, used for
                     // synchronization
  bool
      m_bWaitForBeginSequence; // Set to true when we are told to
                               // MoveToPosition. Holds the actor in the
                               // pre-action idle until BeginSequence is called.
  ...
  EHANDLE m_hNextCine; // The script to hand the NPC off to when we finish with
                       // them.
  ...
  static const char *
  GetSpawnPreIdleSequenceForScript(CBaseEntity *pTargetEntity);
