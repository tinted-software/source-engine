...
CBaseGameStats_Driver::CBaseGameStats_Driver(void)
    : BaseClass("CGameStats"), m_iLoadedVersion(-1), m_bEnabled(false),
      ...
      m_flLastMovementTime(0.0f), m_bGamePaused(false) {
...
bool CBaseGameStats::UploadStatsFileNOW(void) {
  if (!StatsTrackingIsFullyEnabled()) {
    StatsLog("UploadStatsFileNOW: stats tracking not fully enabled, not "
             "uploading file\n");
    return false;
  }
  ...
  if (gamestatsuploader) {
    bool bRet =
        gamestatsuploader->UploadGameStats("", 1, uBlobSize, pvBlobData);
