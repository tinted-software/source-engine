...
// Central point for defining colors and drawing routines for Navigation Mesh
// edit mode Author: Matthew Campbell, 2004
...
void NavDrawLine(const Vector &from, const Vector &to, NavEditColor navColor) {
  ...
  NDebugOverlay::Line(from + offset, to + offset, color[0], color[1], color[2],
                      false, NDEBUG_PERSIST_TILL_NEXT_SERVER);
  NDebugOverlay::Line(from + offset, to + offset, color[0] / 2, color[1] / 2,
                      color[2] / 2, true, NDEBUG_PERSIST_TILL_NEXT_SERVER);
}
...
void NavDrawFilledTriangle(const Vector &point1, const Vector &point2,
  ...
  NDebugOverlay::Triangle(point1, point2, point3, color[0], color[1], color[2],
                          255, true, NDEBUG_PERSIST_TILL_NEXT_SERVER);
}
...
void NavDrawHorizontalArrow(const Vector &from, const Vector &to, float width,
  ...
  Color color = NavColors[navColor];
  NDebugOverlay::HorzArrow(from + offset, to + offset, width, color[0],
                           color[1], color[2], 255, false,
                           NDEBUG_PERSIST_TILL_NEXT_SERVER);
  NDebugOverlay::HorzArrow(from + offset, to + offset, width, color[0] / 2,
                           color[1] / 2, color[2] / 2, 255, true,
                           NDEBUG_PERSIST_TILL_NEXT_SERVER);
}
...
void NavDrawDashedLine(const Vector &from, const Vector &to,
  ...
  while (distance < totalDistance) {
    ...
    NDebugOverlay::Line(start + offset, end + offset, color[0], color[1],
                        color[2], false, NDEBUG_PERSIST_TILL_NEXT_SERVER);
    NDebugOverlay::Line(start + offset, end + offset, color[0] / 2,
