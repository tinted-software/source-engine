...
public:
  ...
  void
  SetNumTrainCappers(int iNumCappers,
                     CBaseEntity *pTrigger); // only used for train watchers
                                             // that control the train movement
  ...
  void ProjectPointOntoPath(const Vector &pos, Vector *posOnPath,
      ...
      const; // project the given position onto the track and return the point
             // and how far along that projected position is
  bool IsAheadOfTrain(
      const Vector &pos) const; // return true if the given position is farther
                                // down the track than the train is

  bool IsTrainAtStart(void)
      const; // return true if the train hasn't left its starting position yet
  bool IsTrainNearCheckpoint(
      void) const; // return true if the train is almost at the next checkpoint
...
private:
  ...
  float m_flTrainDistanceFromStart; // actual distance along path of train, for
                                    // comparing against
