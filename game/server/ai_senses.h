...
public:
  CAI_Senses()
      : m_LookDist(2048), m_LastLookDist(-1), m_TimeLastLook(-1),
        m_iAudibleList(0), m_TimeLastLookHighPriority(-1),
        m_TimeLastLookNPCs(-1), m_TimeLastLookMisc(-1) {
  ...
  bool
  CanSeeEntity(CBaseEntity *pSightEnt); // more expensive cone & raycast test
#ifdef PORTAL
  ...
  bool DidSeeEntity(CBaseEntity *pSightEnt)
      const; //  a less expensive query that looks at cached results from recent
             //  conditionsa gathering
...
private:
  ...
  int m_iAudibleList; // first index of a linked list of sounds that the npc can
                      // hear.
