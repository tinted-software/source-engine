...
private:
  struct HIST_ITEM {
    HIST_ITEM() {
      // init this here, because the code that overwrites previous history items
      // will use this to check to see if the item is empty
      DisplayTime = 0.0f;
    }
    int type;
    float DisplayTime; // the time at which this item should be removed from the
                       // history
    int iCount;
    int iId;
