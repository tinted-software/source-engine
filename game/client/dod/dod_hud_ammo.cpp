...
private:
  ...
  CPanelAnimationVarAliasType(float, digit2_ypos, "digit2_ypos", "0",
                              "proportional_float");
  CPanelAnimationVarAliasType(float, bar_xpos, "bar_xpos", "0",
                              "proportional_float");
  CPanelAnimationVarAliasType(float, bar_ypos, "bar_ypos", "0",
                              "proportional_float");
  CPanelAnimationVarAliasType(float, bar_width, "bar_width", "2",
                              "proportional_float");
  CPanelAnimationVarAliasType(float, bar_height, "bar_height", "2",
                              "proportional_float");
...
CHudAmmo::CHudAmmo(const char *pElementName)
    : vgui::Panel(NULL, "HudAmmo"), CHudElement(pElementName) {
...
void CHudAmmo::PaintGrenadeAmmo(CWeaponDODBase *pWpn) {
  ...
  if (m_iAmmo > 0 && pAmmoIcon) {
    ...
    pAmmoIcon->DrawSelf(xpos, ypos, pAmmoIcon->Width(), pAmmoIcon->Height(),
                        m_clrIcon);

    char buf[16];
    Q_snprintf(buf, sizeof(buf), "x %d", m_iAmmo);

    DrawText(buf, xpos + pAmmoIcon->Width(), ypos + pAmmoIcon->Height() / 2,
             m_clrIcon);
  }
}
...
void CHudAmmo::PaintMGAmmo(CWeaponDODBase *pWpn) {
  ...
  if (pFullClip) {
    ...
    // Haxoration! The box that contains the numbers must be in the same
    // position
  ...
  if (pExtraClip && clips > 0) {
    ...
    DrawText(buf, xpos + pExtraClip->Width(), ypos + pExtraClip->Height() / 2,
             m_clrIcon);
  }
}
...
void CHudAmmo::PaintGunAmmo(CWeaponDODBase *pWpn) {
  ...
  if (pEmptyClip) {
    ...
    pEmptyClip->DrawSelfCropped(
        xpos, ypos + nOffset, 0, nOffset, pEmptyClip->Width(),
