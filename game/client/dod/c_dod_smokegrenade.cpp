...
RecvPropTime(RECVINFO(m_flSmokeSpawnTime)), END_NETWORK_TABLE()
...
float C_DODSmokeGrenade::CalcSmokeCloudRadius(void) {
  ...
  float flRadius =
      SMOKE_CLOUD_RADIUS * (float)sin(flLifetime * M_PI * 0.5 / EXPAND_TIME);
...
float C_DODSmokeGrenade::CalcSmokeCloudAlpha(void) {
  ...
  if (flLifetime < flFadedInTime) {
    ...
    float fadePercent = (flLifetime - flStartFadingOutTime) /
                        (flEndFadingOutTime - flStartFadingOutTime);
    ...
    if (testDist < flExpandRadius) {
      ...
      if (testDist < flCoreDistance) {
        ...
        EngineGetSmokeFogOverlayAlpha() +=
            (1 -
             (testDist - flCoreDistance) / (flExpandRadius - flCoreDistance)) *
            flFadeAlpha;
      }
    }
  }
