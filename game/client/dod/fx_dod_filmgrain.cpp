...
private:
  void DrawSplotch(float x, float y, float width, float height, float u,
                   float v, float uWidth, float vHeight, int orientation,
                   int alpha = 255);
...
void CFilmGrainEffect::Init() {
  ...
  m_pFlickerMaterial = materials->FindMaterial("effects/flicker_128",
...
void CFilmGrainEffect::SetParameters(KeyValues *params) {
  if (params->FindKey("split_screen")) {
    ...
    extern void EnableHUDFilmDemo(bool bEnable, const char *left_string_id,
                                  const char *right_string_id);
...
//-----------------------------------------------------------------------------
void CFilmGrainEffect::DrawSplotch(float x, float y, float flWidth,
                                   float flHeight, float u, float v,
                                   float uWidth, float vWidth, int orientation,
                                   int alpha) {
...
void CFilmGrainEffect::Render(int x, int y, int w, int h) {
  ...
  if (m_bSplitScreen) {
    DrawSplotch(0.0f, -1.0f, 2.0f, 2.0f, flBiasU, flBiasV, flScaleU, flScaleV,
                0);
  } else {
    DrawSplotch(-1.0f, -1.0f, 2.0f, 2.0f, flBiasU, flBiasV, flScaleU, flScaleV,
                0);
  }
  ...
  if (m_bEnableFlicker) {
    if (!IsErrorMaterial(m_pFlickerMaterial)) {
      ...
      if (!m_bSplitScreen) {
        DrawSplotch(-1.0f, -1.0f, 2.0f, 2.0f, 0.0f, 1.0f, 1.0f, -1.0f, 0,
                    nFlickerAlpha);
      } else {
        DrawSplotch(0.0f, -1.0f, 2.0f, 2.0f, 0.5f, 1.0f, 0.5f, -1.0f, 0,
                    nFlickerAlpha);
      }
    }
  }
  ...
  if (nTime != m_nCachedParticleTime) {
    ...
    if (flDustProb < m_flChanceOfDust) {
      ...
      for (int i = 0; i < numDustParticles; i++) {
        ...
        particle.m_flWidth =
            particle.m_flHeight * (float)screenHeight / (float)screenWidth;

        if ((!m_bSplitScreen) ||
            (particle.m_flPositionX - particle.m_flWidth > 0.0f)) {
  ...
  for (int i = 0; i < m_CachedParticles.Count(); i++) {
    ...
    for (int i = 0; i < paramCount; i++) {
      ...
      if (!Q_stricmp(pVar->GetName(), "$channel_select")) {
        pVar->SetVecValue(channelSelect[0], channelSelect[1], channelSelect[2],
                          channelSelect[3]);
      }
    }
    ...
    DrawSplotch(particle->m_flPositionX, particle->m_flPositionY,
                particle->m_flWidth * m_flEffectAlpha,
                particle->m_flHeight * m_flEffectAlpha, flUOffset, flVOffset,
                0.25f, 0.25f, particle->m_nOrientation);
  }
}
...
// Console Interface
//------------------------------------------------------------------------------
static void EnableFilmGrain(IConVar *pConVar, char const *pOldString,
                            float flOldValue) {
...
static ConVar mat_filmgrain("mat_filmgrain", "0", FCVAR_CLIENTDLL,
                            "Enable/disable film grain post effect",
                            EnableFilmGrain);
