...
RecvPropInt(RECVINFO(m_iState)), RecvPropInt(RECVINFO(m_iBombingTeam)),
    ...
    END_NETWORK_TABLE()
        ...
        void C_DODBombTarget::NotifyShouldTransmit(
...
void C_DODBombTarget::ClientThink(void) {
  ...
  if (pPlayer && pPlayer->IsAlive()) {
    ...
    if (vecDist.Length() < 200) {
      int iOppositeTeam =
          (m_iBombingTeam == TEAM_ALLIES) ? TEAM_AXIS : TEAM_ALLIES;
      if (pPlayer->GetTeamNumber() == m_iBombingTeam &&
          m_iState == BOMB_TARGET_ACTIVE) {
        pPlayer->CheckBombTargetPlantHint();
      } else if (pPlayer->GetTeamNumber() == iOppositeTeam &&
                 m_iState == BOMB_TARGET_ARMED) {
