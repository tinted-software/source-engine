...
// Purpose:
//-----------------------------------------------------------------------------
CDoDCapturePanelProgressBar::CDoDCapturePanelProgressBar(vgui::Panel *parent,
                                                         const char *name)
...
// Purpose:
//-----------------------------------------------------------------------------
CDoDCapturePanelIcon::CDoDCapturePanelIcon(vgui::Panel *parent,
                                           const char *name)
    : vgui::ImagePanel(parent, name) {
  m_bActive = false;
...
CDoDHudCapturePanel::CDoDHudCapturePanel(const char *pElementName)
  ...
  m_pProgressBar =
      new CDoDCapturePanelProgressBar(this, "CapturePanelProgressBar");
...
void CDoDHudCapturePanel::OnThink() {
  ...
  if (pPlayer) {
    ...
    if (!bInCapZone || !hud_capturepanel.GetBool()) {
      ...
      if (pPlayer->m_iProgressBarDuration > 0 && pPlayer) {
        ...
        if (m_pProgressBar) {
          ...
          float flPercent = (pPlayer->m_flSimulationTime -
                             pPlayer->m_flProgressBarStartTime) /
    ...
    int nNumTeammates = g_pObjectiveResource->GetNumPlayersInArea(
        iCPIndex, nPlayerTeam == TEAM_ALLIES ? TEAM_ALLIES : TEAM_AXIS);
    int nRequiredTeammates = g_pObjectiveResource->GetRequiredCappers(
        iCPIndex, nPlayerTeam == TEAM_ALLIES ? TEAM_ALLIES : TEAM_AXIS);

    int nNumEnemies = g_pObjectiveResource->GetNumPlayersInArea(
        iCPIndex, nPlayerTeam == TEAM_ALLIES ? TEAM_AXIS : TEAM_ALLIES);
    int nRequiredEnemies = g_pObjectiveResource->GetRequiredCappers(
        iCPIndex, nPlayerTeam == TEAM_ALLIES ? TEAM_AXIS : TEAM_ALLIES);
    ...
    if (iCappingTeam == TEAM_UNASSIGNED || iCappingTeam != nPlayerTeam) {
      ...
      if (m_pProgressBar) {
        ...
        m_pProgressBar->SetPercentage(
            g_pObjectiveResource->GetCPCapPercentage(pPlayer->GetCPIndex()));
      }
