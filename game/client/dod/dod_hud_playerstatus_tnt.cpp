...
// Purpose:
//-----------------------------------------------------------------------------
CDoDHudTNT::CDoDHudTNT(vgui::Panel *parent, const char *name)
    : CDoDCutEditablePanel(parent, name) {
...
void CDoDHudTNT::ApplySettings(KeyValues *inResourceData) {
  ...
  m_iIconX = vgui::scheme()->GetProportionalScaledValueEx(
      GetScheme(), inResourceData->GetInt("icon_x", 0));
  m_iIconY = vgui::scheme()->GetProportionalScaledValueEx(
      GetScheme(), inResourceData->GetInt("icon_y", 0));
  m_iIconW = vgui::scheme()->GetProportionalScaledValueEx(
      GetScheme(), inResourceData->GetInt("icon_w", 32));
  m_iIconH = vgui::scheme()->GetProportionalScaledValueEx(
      GetScheme(), inResourceData->GetInt("icon_h", 32));
}
