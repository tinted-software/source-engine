...
ConVar hud_c4pickuppanel("hud_c4pickuppanel", "1",
...
void CDODHudTNTPickupPanel::FireGameEvent(IGameEvent *event) {
  ...
  if (!Q_strcmp(pszEventName, "dod_tnt_pickup") && pPlayer &&
      pPlayer->ShouldShowHints()) {
...
void CDODHudTNTPickupPanel::OnThink(void) {
  ...
  if (m_bInitLayout) {
    ...
    m_pPickupLabel->SetPos(newX + 2 * XRES(10) + imageW,
                           bgY + (bgH - labelY) / 2);
