...
int CDODDeathStatsPanel::DrawDeathNoticeItem(int x, int y) {
  ...
  g_pVGuiLocalize->ConvertANSIToUnicode(m_DeathRecord.Victim.szName, victim,
                                        sizeof(victim));
  g_pVGuiLocalize->ConvertANSIToUnicode(m_DeathRecord.Killer.szName, killer,
                                        sizeof(killer));
  ...
  if (!m_DeathRecord.bSuicide) {
    ...
    Panel::DrawBox(x - boxBorder, y - boxBorder, boxWidth + 2 * boxBorder,
                   boxHeight + 2 * boxBorder, m_ActiveBackgroundColor, 1.0);
