...
// Purpose:
//-----------------------------------------------------------------------------
CDoDHudHealthBar::CDoDHudHealthBar(vgui::Panel *parent, const char *name)
  ...
  vgui::surface()->DrawSetTextureFile(m_iMaterialIndex, "vgui/white", true,
                                      false);
}
...
C_DODPlayer *CDoDHudHealthBar::GetHealthDelegatePlayer(void) {
  if (m_hHealthDelegatePlayer.Get()) {
    C_DODPlayer *pPlayer =
        dynamic_cast<C_DODPlayer *>(m_hHealthDelegatePlayer.Get());
    if (pPlayer)
      return pPlayer;
  }
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CDoDHudHealth::CDoDHudHealth(vgui::Panel *parent, const char *name)
    ...
    if (nTeam == TEAM_ALLIES || nTeam == TEAM_AXIS) {
      ...
      if (nClass != PLAYERCLASS_UNDEFINED) {
        if ((nClass != m_nPrevClass) ||
            (nTeam != TEAM_INVALID &&
             (nTeam == TEAM_AXIS || nTeam == TEAM_ALLIES) &&
             nTeam != m_nPrevTeam)) {
          m_nPrevClass = nClass;
          m_nPrevTeam = nTeam;

          if (m_pClassImage) {
            m_pClassImage->SetImage(
                (pTeam->GetPlayerClassInfo(nClass)).m_szClassHealthImage);
          }

          if (m_pClassImageBG) {
            m_pClassImageBG->SetImage(
                (pTeam->GetPlayerClassInfo(nClass)).m_szClassHealthImageBG);
          }
        }
      }
    }
