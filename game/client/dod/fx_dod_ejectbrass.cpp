...
void DOD_EjectBrassCallback(const CEffectData &data) {
  ...
  static model_t *pSmallShell =
      (model_t *)engine->LoadModel("models/shells/shell_small.mdl");
  static model_t *pMediumShell =
      (model_t *)engine->LoadModel("models/shells/shell_medium.mdl");
  static model_t *pLargeShell =
      (model_t *)engine->LoadModel("models/shells/shell_large.mdl");
  static model_t *pGarandClip =
      (model_t *)engine->LoadModel("models/shells/garand_clip.mdl");

  int flags = FTENT_FADEOUT | FTENT_GRAVITY | FTENT_COLLIDEALL |
              FTENT_HITSOUND | FTENT_ROTATE;
  ...
  C_LocalTempEntity *pTemp =
      tempents->SpawnTempModel(pModel, data.m_vOrigin, vecShellAngles,
                               vecVelocity, flLifeTime, FTENT_NEVERDIE);
  if (pTemp == NULL)
    return;
