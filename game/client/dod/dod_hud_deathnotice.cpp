...
// a very useful function for getting the ideal scale factor of a sprite that's
// to be scaled into a space
...
struct DeathNoticeItem {
  ...
  bool bLocalPlayerInvolved; // Is the local player a capper, killer or victim
                             // in this message
...
private:
  ...
  CPanelAnimationVar(Color, m_BackgroundColor, "BackgroundColor",
                     "255 255 255 100");
  CPanelAnimationVar(Color, m_ActiveBackgroundColor, "ActiveBackgroundColor",
                     "255 255 255 140");
...
int CHudDeathNotice::DrawDefenseItem(DeathNoticeItem *pItem, int xRight,
                                     int y) {
  ...
  g_pVGuiLocalize->ConvertANSIToUnicode(pItem->Victim.szName, victim,
                                        sizeof(victim));
  g_pVGuiLocalize->ConvertANSIToUnicode(pItem->Killer.szName, killer,
                                        sizeof(killer));
  ...
  float scale = ((float)ScreenHeight() / 480.0f) * 0.6; // scale based on
                                                        // 800x600
  ...
  DrawBackgroundBox(x - boxBorder, y - boxBorder, boxWidth + 2 * boxBorder,
                    boxHeight + 2 * boxBorder, pItem->bLocalPlayerInvolved);
  ...
  m_pIconDefended->DrawSelf(x, y, iconDefSize, iconDefSize,
  ...
  vgui::surface()->DrawSetTextureFile(m_iMaterialTexture, szMatName, true,
                                      false);
...
int CHudDeathNotice::DrawDeathNoticeItem(DeathNoticeItem *pItem, int xRight,
                                         int y) {
  ...
  if (bCapMsg) {
    float scale =
        ((float)ScreenHeight() / 480.0f) * 0.6; // scale based on 800x600
    ...
    if (icon->bRenderUsingFont) {
      iconWide =
          surface()->GetCharacterWidth(icon->hFont, icon->cCharacterInFont);
      iconTall = surface()->GetFontTall(icon->hFont);
    } else {
      float scale = GetScale(icon->Width(), icon->Height(),
                             XRES(cl_deathicon_width.GetInt()),
  ...
  if (!pItem->bSuicide) {
    int nameWidth =
        UTIL_ComputeStringWidth(m_hTextFont, killer) + spacerX; // gap
...
int CHudDeathNotice::DrawDominationNoticeItem(DeathNoticeItem *pItem,
                                              int xRight, int y) {
  // Get the team numbers for the players involved
  int iKillerTeam = TEAM_UNASSIGNED;
  int iVictimTeam = TEAM_UNASSIGNED;
  ...
  int iDominatingLen =
      UTIL_ComputeStringWidth(m_hTextFont, pItem->wzInfoText) + XRES(2);
  x -= iDominatingLen;
  ...
  // Draw victims name
  // surface()->DrawSetTextFont( m_hTextFont );	//reset the font, draw icon can
  // change it
  surface()->DrawSetTextPos(x, yText);
  p = victim;
  while (*p) {
...
void CHudDeathNotice::DrawBackgroundBox(int x, int y, int w, int h,
                                        bool bLocalPlayerInvolved) {
  Panel::DrawBox(x, y, w, h,
                 bLocalPlayerInvolved ? m_ActiveBackgroundColor
                                      : m_BackgroundColor,
  ...
  if (Q_strcmp("dod_point_captured", pEventName) == 0) {
    ...
    for (int i = 0; i < len; i++) {
      ...
      if (i == 0) {
        ...
        capMsg.iMaterial =
            g_pObjectiveResource->GetIconForTeam(cp, capMsg.Killer.iEntIndex);
    ...
    if (pBuf) {
      g_pVGuiLocalize->ConvertUnicodeToANSI(pBuf, capMsg.Victim.szName,
                                            sizeof(capMsg.Victim.szName));
    } else {
    ...
    Q_snprintf(szLogMsg, sizeof(szLogMsg), "%s captured %s for the %s\n",
               capMsg.Killer.szName, capMsg.Victim.szName,
               capMsg.Killer.iEntIndex == TEAM_ALLIES ? "U.S. Army"
                                                      : "Wermacht");
    ...
    Q_strncpy(capMsg.Killer.szName, g_PR->GetPlayerName(iBlocker),
              sizeof(capMsg.Killer.szName));
    ...
    Q_snprintf(capMsg.Victim.szName, sizeof(capMsg.Victim.szName), " - %s",
               pName);
    ...
    deathMsg.bLocalPlayerInvolved =
        (killer == iLocalPlayerIndex || victim == iLocalPlayerIndex);
...
//-----------------------------------------------------------------------------
void CHudDeathNotice::AddAdditionalMsg(int iKillerID, int iVictimID,
                                       const char *pMsgKey) {
  ...
  Q_strncpy(msg.Killer.szName, g_PR->GetPlayerName(iKillerID),
            ARRAYSIZE(msg.Killer.szName));
  Q_strncpy(msg.Victim.szName, g_PR->GetPlayerName(iVictimID),
...
void CHudDeathNotice::PlayRivalrySounds(int iKillerIndex, int iVictimIndex,
                                        int iType) {
