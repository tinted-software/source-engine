...
void DODExplosionCallback(const Vector &vecOrigin, const Vector &vecNormal) {
  ...
  // Cannot use zeros here because we are sending the normal at a smaller bit
  // size.
  if (fabs(vecNormal.x) < 0.05f && fabs(vecNormal.y) < 0.05f &&
      fabs(vecNormal.z) < 0.05f) {
    bInAir = true;
    angExplosion.Init();
  } else {
  ...
  if (UTIL_PointContents(vecOrigin) & CONTENTS_WATER) {
    WaterExplosionEffect().Create(vecOrigin, 1 /*m_nMagnitude*/, 1 /*m_fScale*/,
                                  0 /*m_nFlags*/);
    return;
  } else if (bInAir) {
...
IMPLEMENT_CLIENTCLASS_EVENT_DT(C_TEDODExplosion, DT_TEDODExplosion,
                               CTEDODExplosion)
RecvPropFloat(RECVINFO(m_vecOrigin[0])),
    RecvPropFloat(RECVINFO(m_vecOrigin[1])),
    RecvPropFloat(RECVINFO(m_vecOrigin[2])),
    RecvPropVector(RECVINFO(m_vecNormal)), END_RECV_TABLE()
