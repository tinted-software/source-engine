...
ConVar cl_voicesubtitles(
    "cl_voicesubtitles", "1", 0,
...
void OpenVoiceMenu(int index) {
  ...
  case 3: {
    ...
    char szMenu[128];
    Q_snprintf(szMenu, sizeof(szMenu), "#Menu_%sVoice%c", pszNation,
               cMenuNumber);
...
CON_COMMAND(menuselect, "menuselect") {
  ...
  case 0:
  default:
    // if we didn't have a menu open, maybe a plugin did.  send it on to the
    // server.
...
void CHudChat::MsgFunc_SayText(bf_read &msg) {
  ...
  if (bWantsToChat) {
    ...
    Printf(GetFilterForString(szString), "%s",
           hudtextmessage->LookupString(szString));
  }
...
void CHudChat::MsgFunc_VoiceSubtitle(bf_read &msg) {
  ...
  if (iTeam == TEAM_AXIS &&
      g_VoiceCommands[iVoiceCommand].pszAxisSubtitle != NULL)
  ...
  ChatPrintf(client, CHAT_FILTER_NONE, "%c(%s) %s%c: %s", COLOR_PLAYERNAME,
             szPrefix, g_PR->GetPlayerName(client), COLOR_NORMAL,
...
void CHudChat::MsgFunc_HandSignalSubtitle(bf_read &msg) {
  ...
  ChatPrintf(client, CHAT_FILTER_NONE, "%c(%s) %s%c: %s", COLOR_USEOLDCOLORS,
             szPrefix, g_PR->GetPlayerName(client), COLOR_NORMAL,
             ConvertCRtoNL(szString));
}
...
wchar_t *ReadLocalizedVoiceCommandString(bf_read &msg, wchar_t *pOut,
                                         int outSize, bool bStripNewline) {
  char szString[2048];
  msg.ReadString(szString, sizeof(szString));
...
// Find words in the chat that can be localized and replace them with localized
// text
void CHudChat::FindLocalizableSubstrings(char *szChat, int chatLength) {
  ...
  for (char *pSrc = szChat; pSrc != NULL && *pSrc != 0 && pos < chatLength - 1;
       pSrc++) {
    // if its a #
    if (*pSrc == '#') {
      // trim into a word
      ...
      if (pwcLocalized) {
        ...
        g_pVGuiLocalize->ConvertUnicodeToANSI(pwcLocalized, szTemp,
                                              sizeof(szTemp));
      }
      ...
      int remainingSpace =
          chatLength - pos - 2; // minus one for the NULL and one for the space
