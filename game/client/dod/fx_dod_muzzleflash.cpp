...
void DOD_MuzzleFlashCallback(const CEffectData &data) {
  CSmartPtr<CLocalSpaceEmitter> pEmitter = CLocalSpaceEmitter::Create(
      "DOD_MuzzleFlash", data.m_hEntity, data.m_nAttachmentIndex, 0);
  ...
  Assert(pEmitter);
  pEmitter->GetBinding().SetBBox(vCenter - Vector(10, 10, 10),
                                 vCenter + Vector(10, 10, 10));
  ...
  if (data.m_nHitBox == DOD_MUZZLEFLASH_MG) // Machine gun
  {
    SimpleParticle *pParticle = (SimpleParticle *)pEmitter->AddParticle(
        sizeof(SimpleParticle), g_Mat_SMG_Muzzleflash[0], vFlashOffset);
    Assert(pParticle);
    if (pParticle) {
      ...
      pParticle->m_uchStartSize =
          random->RandomFloat(120, 130) * data.m_flMagnitude;
    ...
    for (i = 0; i < 3; i++) {
      ...
      SimpleParticle *pParticle = (SimpleParticle *)pEmitter->AddParticle(
          sizeof(SimpleParticle), g_Mat_SMG_Muzzleflash[1], vFlashOffset);
      Assert(pParticle);
      if (pParticle) {
        pParticle->m_flLifetime = 0.0f;
        pParticle->m_flDieTime = 0.1f;
