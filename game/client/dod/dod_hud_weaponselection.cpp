...
private:
  ...
  void DrawBox(int x, int y, int wide, int tall, int type, int number,
               int valid);

private:
  int m_iBackgroundTexture;

  CPanelAnimationVarAliasType(float, m_flSmallBoxWide, "SmallBoxWide", "108",
                              "proportional_float");
  CPanelAnimationVarAliasType(float, m_flSmallBoxTall, "SmallBoxTall", "72",
                              "proportional_float");
  CPanelAnimationVarAliasType(float, m_flSmallBoxCutSize, "SmallBoxCutSize",
                              "5", "proportional_float");
  ...
  CPanelAnimationVarAliasType(float, m_flLargeBoxTall, "LargeBoxTall", "72",
                              "proportional_float");
  CPanelAnimationVarAliasType(float, m_flLargeBoxCutSize, "LargeBoxCutSize",
                              "5", "proportional_float");

  CPanelAnimationVarAliasType(float, m_flBoxGap, "BoxGap", "8",
                              "proportional_float");
  ...
  CPanelAnimationVarAliasType(float, m_flIconXPos, "IconXPos", "16",
                              "proportional_float");
  CPanelAnimationVarAliasType(float, m_flIconYPos, "IconYPos", "8",
                              "proportional_float");
  ...
  CPanelAnimationVar(Color, m_ActiveTextColor, "ActiveTextColor",
                     "255 255 255 255");
  CPanelAnimationVar(Color, m_InactiveTextColor, "InactiveTextColor",
                     "80 80 80 255");
  ...
  CPanelAnimationVar(Color, m_SelectedBoxColor, "SelectedBoxClor",
  ...
  CPanelAnimationVar(Color, m_ActiveBoxColor, "ActiveBoxColor",
                     "255 255 255 255");
  CPanelAnimationVar(Color, m_ActiveBoxBorder, "ActiveBoxBorder",
                     "255 255 255 255");
  CPanelAnimationVar(Color, m_InactiveBoxColor, "InactiveBoxColor", "0 0 0 0");
  CPanelAnimationVar(Color, m_InactiveBoxBorder, "InactiveBoxBorder",
                     "255 255 255 255");
  ...
  for (int iSlot = m_iMaxSlots - 1; iSlot >= 0; iSlot--) {
    for (int iPos = 0; iPos < MAX_WEAPON_POSITIONS; iPos++) {
      ...
      if (pWeapon->GetSpriteActive()) {
        ...
        DrawBox(xpos, ypos, finalBoxWide, finalBoxTall, nType, iSlot + 1,
                (pWeapon->CanDeploy() && bHasAmmo));
...
//-----------------------------------------------------------------------------
void CHudWeaponSelection::DrawBox(int x, int y, int wide, int tall, int type,
                                  int number, int valid) {
  ...
  if (number >= 0) {
    ...
    vgui::surface()->DrawSetTextPos(x + m_flSelectionNumberXPos,
                                    y + ntall - m_flSelectionNumberYPos);
...
void CHudWeaponSelection::CancelWeaponSelection(void) {
  ...
  // Fastswitches happen in a single frame, so the Weapon Selection HUD Element
  // isn't visible yet, but it's going to be next frame. We need to ask it if it
  // thinks it's going to draw, instead of checking it's IsActive flag.
  if (ShouldDraw()) {
    HideSelection();
