...
#if DEBUG_WINDOW_RESIZING
  DevMsg("Resizing '%s' from (%d,%d) %dx%d to (%d,%d) %dx%d -- initially was "
         "(%d,%d) %dx%d\n",
...
void ApplyBackgroundSchemeSettings(EditablePanel *pWindow,
  ...
  // Logo -----------------------------------------------
  /*	pBitmapPanel = dynamic_cast< CBitmapImagePanel *
     >(pWindow->FindChildByName( "ExclamationPanel" )); if ( pBitmapPanel )
          {
...
static void FixupBackgroundPanels(EditablePanel *pWindow, int offsetX,
  ...
  /*	int logoInset = (cornerSize + titleHeight - logoSize)/2;
          ...
          if ( pPanel )
          {
                  ...
                  pPanel->SetBounds( offsetX + inset + logoInset, offsetY +
     inset + logoInset, logoSize, logoSize );
          }
  */
  // Title caption --------------------------------------
...
void ResizeWindowControls(EditablePanel *pWindow, int tall, int wide,
  ...
  for (int i = 0; i < panelList->Size(); ++i) {
    ...
    if (panel != pWindow) {
      ...
      DevMsg("Repositioning '%s' from (%d,%d) to (%d,%d) -- a distance of "
             "(%d,%d)\n",
             panel->GetName(), x, y, x + offsetX, y + offsetY, offsetX,
             offsetY);
#endif
    }
  }
