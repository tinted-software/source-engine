...
// Purpose:
//-----------------------------------------------------------------------------
CDoDCutEditablePanel::CDoDCutEditablePanel(vgui::Panel *parent,
                                           const char *name)
...
void CDoDCutEditablePanel::ApplySettings(KeyValues *inResourceData) {
  ...
  Q_strncpy(m_szBackgroundTexture,
            inResourceData->GetString("BackgroundTexture", "vgui/white"),
            sizeof(m_szBackgroundTexture));
  Q_strncpy(m_szBackgroudColor,
            inResourceData->GetString("BackgroundColor", "HudPanelForeground"),
            sizeof(m_szBackgroudColor));
  Q_strncpy(m_szBorderColor,
            inResourceData->GetString("BackgroundBorder", "HudPanelBorder"),
            sizeof(m_szBorderColor));

  m_iBackgroundTexture =
      vgui::surface()->DrawGetTextureId(m_szBackgroundTexture);
  if (m_iBackgroundTexture == -1) {
    m_iBackgroundTexture = vgui::surface()->CreateNewTextureID();
  }

  vgui::surface()->DrawSetTextureFile(m_iBackgroundTexture,
                                      m_szBackgroundTexture, true, true);

  m_nCornerCutSize = inResourceData->GetInt("CornerCutSize", 1);

  // scale the cut size to our screen co-ords
  if (IsProportional()) {
    m_nCornerCutSize = vgui::scheme()->GetProportionalScaledValueEx(
        GetScheme(), m_nCornerCutSize);
  }
...
void CDoDCutEditablePanel::PaintBackground() {
  ...
  int wide =
      nwide - 1; // -1 because we can't draw all the way out to the width of our
                 // panel (it gets clipped), we can only draw to width - 1
