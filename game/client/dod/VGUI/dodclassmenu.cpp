...
void CDODClassMenu::ShowPanel(bool bShow) {
  ...
  CDODRandomButton<CDODClassInfoPanel> *pRandom =
      dynamic_cast<CDODRandomButton<CDODClassInfoPanel> *>(
          FindChildByName("random"));
...
void CDODClassMenu::Update() {
  ...
  if (pPlayer &&
      pPlayer->m_Shared.DesiredPlayerClass() == PLAYERCLASS_UNDEFINED) {
...
void CDODClassMenu::OnTick(void) {
  ...
  // necessarily before the command to update the class menu. This leads to the
  // cancel button being visible and people cancelling before they have a class.
  // check for class == DOD_CLASS_NONE and if so hide the cancel button
...
void CDODClassMenu::SetVisible(bool state) { BaseClass::SetVisible(state); }
