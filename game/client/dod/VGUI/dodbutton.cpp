...
//===============================================
void CDODButtonShape::DrawShapedBorder(int x, int y, int wide, int tall,
                                       Color fgcolor) {
  ...
  // diagonal
  surface()->DrawLine(wide - 1, tall - halfheight - 1, wide - halfheight - 1,
                      tall - 1);
}

void CDODButtonShape::DrawShapedBackground(int x, int y, int wide, int tall,
                                           Color bgcolor) {
  int halfheight = tall / 3;

  if (m_iWhiteTexture < 0) {
    m_iWhiteTexture = vgui::surface()->CreateNewTextureID();
    vgui::surface()->DrawSetTextureFile(m_iWhiteTexture, "vgui/white", true,
                                        false);
  }
