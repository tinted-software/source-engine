...
CDODTeamMenu::CDODTeamMenu(IViewPort *pViewPort) : CTeamMenu(pViewPort) {
  ...
  LoadControlSettings(
      "Resource/UI/TeamMenu.res"); // reload this to catch DODButtons
...
//-----------------------------------------------------------------------------
// Purpose: Make the first buttons page get displayed when the menu becomes
// visible
//-----------------------------------------------------------------------------
void CDODTeamMenu::SetVisible(bool state) {
  ...
  for (int i = 0; i < GetChildCount();
       i++) // get all the buy buttons to performlayout
  {
    CDODMouseOverButton<EditablePanel> *button =
        dynamic_cast<CDODMouseOverButton<EditablePanel> *>(GetChild(i));
    if (button) {
...
void CDODTeamMenu::OnTick(void) {
  ...
  if (iNumAllies != iLastAlliesCount) {
    ...
    if (iNumAllies == 1) {
      g_pVGuiLocalize->ConstructString(
          wbuf, sizeof(wbuf), g_pVGuiLocalize->Find("#teammenu_numAllies_1"),
          0);
    } else {
      wchar_t wnum[6];
      _snwprintf(wnum, ARRAYSIZE(wnum), L"%d", iNumAllies);
      g_pVGuiLocalize->ConstructString(
          wbuf, sizeof(wbuf), g_pVGuiLocalize->Find("#teammenu_numAllies"), 1,
          wnum);
    }
  ...
  if (iNumAxis != iLastAxisCount) {
    ...
    if (iNumAxis == 1) {
      g_pVGuiLocalize->ConstructString(
          wbuf, sizeof(wbuf), g_pVGuiLocalize->Find("#teammenu_numAxis_1"), 0);
    } else {
      wchar_t wnum[6];
      _snwprintf(wnum, ARRAYSIZE(wnum), L"%d", iNumAxis);
      g_pVGuiLocalize->ConstructString(
          wbuf, sizeof(wbuf), g_pVGuiLocalize->Find("#teammenu_numAxis"), 1,
          wnum);
    }
...
Panel *CDODTeamMenu::CreateControlByName(const char *controlName) {
  if (!Q_stricmp("DODMouseOverPanelButton", controlName)) {
    CDODMouseOverButton<EditablePanel> *newButton =
        new CDODMouseOverButton<EditablePanel>(this, NULL, m_pPanel);
