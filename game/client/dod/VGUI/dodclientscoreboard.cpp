...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CDODClientScoreBoardDialog::CDODClientScoreBoardDialog(IViewPort *pViewPort)
...
void CDODClientScoreBoardDialog::ApplySchemeSettings(vgui::IScheme *pScheme) {
  ...
  if (m_pImageList) {
    m_iImageDead = m_pImageList->AddImage(
        scheme()->GetImage("../hud/leaderboard_dead", true));
    m_iImageDominated = m_pImageList->AddImage(
        scheme()->GetImage("../hud/leaderboard_dominated", true));
    m_iImageNemesis = m_pImageList->AddImage(
        scheme()->GetImage("../hud/leaderboard_nemesis", true));
  ...
  if (m_pAllies_PlayerCount && m_pAllies_Score && m_pAllies_Kills &&
      m_pAllies_Deaths && m_pAllies_Ping) {
  ...
  if (m_pAxis_PlayerCount && m_pAxis_Score && m_pAxis_Kills && m_pAxis_Deaths &&
      m_pAxis_Ping) {
...
//-----------------------------------------------------------------------------
bool CDODClientScoreBoardDialog::DODPlayerSortFunc(
...
//-----------------------------------------------------------------------------
void CDODClientScoreBoardDialog::InitPlayerList(SectionedListPanel *pPlayerList,
                                                int teamNumber) {
  ...
  if (teamNumber && GameResources()) {
    pPlayerList->SetSectionFgColor(0,
                                   GameResources()->GetTeamColor(teamNumber));
  }
  ...
  pPlayerList->AddColumnToSection(0, "status", "",
                                  SectionedListPanel::COLUMN_IMAGE |
                                      SectionedListPanel::COLUMN_CENTER,
                                  m_iStatusWidth);
  pPlayerList->AddColumnToSection(0, "class", "", 0, m_iClassWidth);
  pPlayerList->AddColumnToSection(
      0, "score", "", SectionedListPanel::COLUMN_RIGHT, m_iScoreWidth);
  pPlayerList->AddColumnToSection(
      0, "frags", "", SectionedListPanel::COLUMN_RIGHT, m_iFragsWidth);
  pPlayerList->AddColumnToSection(
...
void CDODClientScoreBoardDialog::UpdateTeamInfo() {
  ...
  for (int teamIndex = TEAM_ALLIES; teamIndex <= TEAM_AXIS; teamIndex++) {
    ...
    if (team) {
      ...
      _snwprintf(wNumPlayers, ARRAYSIZE(wNumPlayers), L"%i",
                 team->Get_Number_Players());
      if (!teamName && team) {
      ...
      _snwprintf(wRoundsWon, ARRAYSIZE(wRoundsWon), L"%i",
                 team->GetRoundsWon());
      _snwprintf(wTickScore, ARRAYSIZE(wTickScore), L"%i", team->Get_Score());
      g_pVGuiLocalize->ConstructString(
          wTeamScore, sizeof(wTeamScore),
          g_pVGuiLocalize->Find("#scoreboard_teamscore"), 2, wRoundsWon,
          wTickScore);
  ...
  for (int playerIndex = 1; playerIndex <= MAX_PLAYERS; playerIndex++) {
    if (g_PR->IsConnected(playerIndex)) {
      ...
      if (pLocalPlayer->m_Shared.IsPlayerDominatingMe(playerIndex)) {
        ...
        // if this player is dominated by the local player, show the domination
        // icon
...
void CDODClientScoreBoardDialog::UpdateSpectatorList() {
  ...
  for (int playerIndex = 1; playerIndex <= MAX_PLAYERS; playerIndex++) {
    if (ShouldShowAsSpectator(playerIndex)) {
      ...
      Q_strncat(szSpectatorList, g_PR->GetPlayerName(playerIndex),
                ARRAYSIZE(szSpectatorList));
      nSpectators++;
    }
  }
...
void CDODClientScoreBoardDialog::FireGameEvent(IGameEvent *event) {
  ...
  if (0 == Q_strcmp(type, "server_spawn")) {
    ...
    g_pVGuiLocalize->ConvertANSIToUnicode(hostname, wzHostName,
                                          sizeof(wzHostName));
    g_pVGuiLocalize->ConstructString(
        wzServerLabel, sizeof(wzServerLabel),
...
bool CDODClientScoreBoardDialog::GetPlayerScoreInfo(int playerIndex,
  ...
  // If we are on a team that shows class/status, and the local player is
  // allowed to see this information
  if ((localteam == TEAM_SPECTATOR || localteam == TEAM_UNASSIGNED ||
       team == localteam)) {
    // class name
    if (g_PR->IsConnected(playerIndex)) {
      ...
      if (cls != PLAYERCLASS_UNDEFINED) {
        ...
        g_pVGuiLocalize->ConvertUnicodeToANSI(
            g_pVGuiLocalize->Find(info.m_szPrintName), szClassName,
            sizeof(szClassName));
      }
    ...
    // status
    // display whether player is alive or dead (all players see this for all
