...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CDODSpectatorGUI::CDODSpectatorGUI(IViewPort *pViewPort)
...
bool CDODSpectatorGUI::NeedsUpdate(void) {
  ...
  // if ( DODGameRules()->IsGameUnderTimeLimit() && m_nLastTime !=
  // DODGameRules()->GetTimeLeft() ) 	return true;
...
void CDODSpectatorGUI::UpdateTimer(void) {
  ...
  if (!pPlayer || pPlayer->IsHLTV()) {
    ...
    _snwprintf(wText, sizeof(wText) / sizeof(wchar_t), L"%d:%02d", (timer / 60),
               (timer % 60));
    wText[62] = 0;
    ...
    // once we know that, we can ask dodgamerules if we are going to make the
    // next wave. If we aren't, gamerules can tell us the new time based on the
    // reserve wave
    ...
    int timer = DODGameRules()->GetReinforcementTimerSeconds(
        pPlayer->GetTeamNumber(), flSpawnEligibleTime);
    ...
    if (timer < 0 || (pPlayer->GetObserverMode() == OBS_MODE_DEATHCAM)) {
      ...
      if (mins == 1) //"1 minute"
      {
        g_pVGuiLocalize->ConstructString(
            ...
            g_pVGuiLocalize->Find("#Dod_Reinforcements_in_min"), 2, wMins,
            wSecs);
      } else if (mins > 0) //"2 minutes"
      {
        g_pVGuiLocalize->ConstructString(
            wLabel, sizeof(wLabel),
            g_pVGuiLocalize->Find("#Dod_Reinforcements_in_mins"), 2, wMins,
            wSecs);
