...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CDODMenuBackground::CDODMenuBackground(Panel *parent)
    : EditablePanel(parent, "MenuBackground") {
...
void CDODMenuBackground::ApplySchemeSettings(IScheme *pScheme) {
  ...
  for (i = 0; i < 8; i++) {
    top[i] =
        vgui::scheme()->GetProportionalScaledValueEx(GetScheme(), iTopDims[i]);
    main[i] =
        vgui::scheme()->GetProportionalScaledValueEx(GetScheme(), iMainDims[i]);
    box[i] =
        vgui::scheme()->GetProportionalScaledValueEx(GetScheme(), iBoxDims[i]);

    if (i < 6)
      m_LineDims[i] = vgui::scheme()->GetProportionalScaledValueEx(
          GetScheme(), iLineDims[i]);
  }
...
void CDODMenuBackground::Paint(void) {
  ...
  vgui::surface()->DrawLine(m_LineDims[0], m_LineDims[1], m_LineDims[2],
                            m_LineDims[3]);
  vgui::surface()->DrawLine(m_LineDims[2], m_LineDims[3], m_LineDims[4],
                            m_LineDims[5]);
