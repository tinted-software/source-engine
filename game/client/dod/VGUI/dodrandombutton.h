...
template <class T> class CDODRandomButton : public CDODMouseOverButton<T> {
private:
...
//===============================================
template <class T> void CDODRandomButton<T>::PaintBackground() {
  int wide, tall;
  ...
  if (CDODRandomButton<T>::m_iWhiteTexture < 0) {
    CDODRandomButton<T>::m_iWhiteTexture =
        vgui::surface()->CreateNewTextureID();
    vgui::surface()->DrawSetTextureFile(CDODRandomButton<T>::m_iWhiteTexture,
                                        "vgui/white", true, false);
  }
...
template <class T> void CDODRandomButton<T>::PaintBorder() {
  int wide, tall;
  this->GetSize(wide, tall);
