...
ConVar cl_dod_freezecam("cl_dod_freezecam", "1", FCVAR_ARCHIVE,
                        "Client option to not show freeze camera on death");
...
// Purpose:
//-----------------------------------------------------------------------------
void CDODFreezePanel::Reset() { Hide(); }
...
void CDODFreezePanel::ApplySchemeSettings(vgui::IScheme *pScheme) {
  ...
  m_pBasePanel =
      dynamic_cast<EditablePanel *>(FindChildByName("FreezePanelBase"));

  Assert(m_pBasePanel);

  if (m_pBasePanel) {
    m_pFreezeLabel =
        dynamic_cast<Label *>(m_pBasePanel->FindChildByName("FreezeLabel"));
    m_pFreezePanelBG = m_pBasePanel->FindChildByName("FreezePanelBG");
    m_pNemesisSubPanel = dynamic_cast<EditablePanel *>(
        m_pBasePanel->FindChildByName("NemesisSubPanel"));
    m_pHealthStatus = dynamic_cast<CDoDHudHealth *>(
        m_pBasePanel->FindChildByName("PlayerStatusHealth"));
    m_pAvatar = dynamic_cast<CAvatarImagePanel *>(
        m_pBasePanel->FindChildByName("AvatarImage"));
  ...
  m_pScreenshotPanel =
      dynamic_cast<EditablePanel *>(FindChildByName("ScreenshotPanel"));
  ...
  if (Q_strcmp("player_death", pEventName) == 0) {
    ...
    if (pKiller) {
      if (pKiller->IsPlayer()) {
        ...
        // If this was just a regular kill but this guy is our nemesis then just
        // show it.
        if (pVictim && pDODKiller &&
            pDODKiller->m_Shared.IsPlayerDominated(pVictim->entindex())) {
          if (!pKiller->IsAlive()) {
        ...
        m_pBasePanel->SetDialogVariable("killername",
                                        g_PR->GetPlayerName(m_iKillerIndex));
    ...
    case SHOW_NO_NEMESIS: {
      ...
      // show it.
      if (pTFKiller &&
          pTFKiller->m_Shared.IsPlayerDominated(pVictim->entindex())) {
    ...
    case SHOW_NEW_NEMESIS: {
      C_DODPlayer *pVictim = C_DODPlayer::GetLocalDODPlayer();
      C_DODPlayer *pTFKiller = ToDODPlayer(pKiller);
      // check to see if killer is still the nemesis of victim; victim may have
      // managed to kill him after victim's death by grenade or some such,
      // extracting revenge and clearing nemesis condition
      if (pTFKiller &&
          pTFKiller->m_Shared.IsPlayerDominated(pVictim->entindex())) {
...
// Purpose:
//-----------------------------------------------------------------------------
CDODFreezePanelCallout *CDODFreezePanel::TestAndAddCallout(
    Vector &origin, Vector &vMins, Vector &vMaxs,
  ...
  // !engine->CullBox( vMins + origin, vMaxs + origin ) )
  {
    if (GetVectorInHudSpace(
            origin, *iX,
            *iY)) // TODO: GetVectorInHudSpace or GetVectorInScreenSpace?
    {
      ...
      *iY -= iYOffset;
      ...
      if (*iX > 0 && *iY > 0 && (iRight < ScreenWidth()) &&
          (iBottom < (ScreenHeight() - YRES(40)))) {
        ...
        if (!QuickBoxIntersectTest(vecCalloutTL, vecCalloutBR, vecFreezeTL,
                                   vecFreezeBR) &&
            !QuickBoxIntersectTest(vecCalloutTL, vecCalloutBR, vecStatTL,
                                   vecStatBR)) {
          // Make sure it doesn't intersect any other callouts
          bool bClear = true;
          for (int iCall = 0; iCall < vecCalloutsTL->Count(); iCall++) {
            if (QuickBoxIntersectTest(vecCalloutTL, vecCalloutBR,
                                      vecCalloutsTL->Element(iCall),
          ...
          if (bClear) {
            ...
            trace_t tr;
            UTIL_TraceLine(origin, MainViewOrigin(), MASK_OPAQUE, NULL,
                           COLLISION_GROUP_NONE, &tr);
            bClear = (tr.fraction >= 1.0f);
          }

          if (bClear) {
            CDODFreezePanelCallout *pCallout = new CDODFreezePanelCallout(
                g_pClientMode->GetViewport(), "FreezePanelCallout");
...
void CDODFreezePanel::UpdateCallout(void) {
  ...
  Vector vecFreezeBR(iX + m_pFreezePanelBG->GetWide(),
                     iY + m_pFreezePanelBG->GetTall(), 1);
...
void CDODFreezePanel::OnThink(void) {
  ...
  if (m_flShowSnapshotReminderAt &&
      m_flShowSnapshotReminderAt < gpGlobals->curtime) {
    if (ShouldDraw()) {
...
void CDODFreezePanel::ShowSnapshotPanel(bool bShow) {
  ...
  if (bShow) {
    ...
    g_pVGuiLocalize->ConstructString(
        wLabel, sizeof(wLabel), g_pVGuiLocalize->Find("#TF_freezecam_snapshot"),
        1, wKey);

    m_pScreenshotPanel->SetDialogVariable("text", wLabel);

    g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(
        this, "HudSnapShotReminderIn");
  }

  m_pScreenshotPanel->SetVisible(bShow);
}

int CDODFreezePanel::HudElementKeyInput(int down, ButtonCode_t keynum,
                                        const char *pszCurrentBinding) {
  if (ShouldDraw() && pszCurrentBinding) {
    if (FStrEq(pszCurrentBinding, "screenshot") ||
        FStrEq(pszCurrentBinding, "jpeg")) {
          ...
          if (pszKillerName) {
            ...
            if (cl_screenshotname.IsValid()) {
              ...
              Q_snprintf(szScreenShotName, sizeof(szScreenShotName), "%s %s",
                         pszKillerName, FREEZECAM_SCREENSHOT_STRING);
...
void CDODFreezePanel::ShowNemesisPanel(bool bShow) {
  ...
  if (bShow) {
    vgui::Label *pLabel = dynamic_cast<vgui::Label *>(
        m_pNemesisSubPanel->FindChildByName("NemesisLabel"));
    vgui::Panel *pBG = m_pNemesisSubPanel->FindChildByName("NemesisPanelBG");
    vgui::ImagePanel *pIcon = dynamic_cast<vgui::ImagePanel *>(
        m_pNemesisSubPanel->FindChildByName("NemesisIcon"));

    // check that our Nemesis panel and resize it to the length of the string
    // (the right side is pinned and doesn't move)
    if (pLabel && pBG && pIcon) {
      int wide, tall;
...
// Purpose:
//-----------------------------------------------------------------------------
CDODFreezePanelCallout::CDODFreezePanelCallout(Panel *parent, const char *name)
