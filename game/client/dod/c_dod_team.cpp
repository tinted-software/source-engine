...
void C_DODTeam::AddPlayerClass(const char *szClassName) {
  ...
  if (ReadPlayerClassDataFromFileForSlot(
          filesystem, szClassName, &hPlayerClassInfo, GetEncryptionKey())) {
...
const CDODPlayerClassInfo &
C_DODTeam::GetPlayerClassInfo(int iPlayerClass) const {
...
bool C_DODTeam::IsClassOnTeam(const char *pszClassName, int &iClassNum) const {
  ...
  for (int i = 0; i < m_hPlayerClassInfoHandles.Count(); i++) {
    FilePlayerClassInfo_t *pPlayerClassInfo =
        GetFilePlayerClassInfoFromHandle(m_hPlayerClassInfoHandles[i]);
