...
const char *g_pszStatNames[DODSTAT_MAX] = {
    "iPlayTime", "iRoundsWon", "iRoundsLost",   "iKills",        "iDeaths",
...
int iValidWeaponStatBitMask =
    ((1 << DODSTAT_KILLS) | (1 << DODSTAT_SHOTS_HIT) |
     (1 << DODSTAT_SHOTS_FIRED) | (1 << DODSTAT_HEADSHOTS));
int iValidPlayerStatBitMask =
    0xFFFF & ~((1 << DODSTAT_SHOTS_HIT) | (1 << DODSTAT_SHOTS_FIRED) |
               (1 << DODSTAT_HEADSHOTS));
      ...
      for (int iClass = 0; iClass < NUM_DOD_PLAYERCLASSES; iClass++) {
        for (int iStat = 0; iStat < DODSTAT_MAX; iStat++) {
          if (iValidPlayerStatBitMask & (1 << iStat)) {
            ...
            Q_snprintf(szStatName, ARRAYSIZE(szStatName), "%s.%s.%s",
                       g_pszShortTeamNames[iTeam], g_pszClassNames[iClass],
                       g_pszStatNames[iStat]);

            if (steamapicontext->SteamUserStats()->GetStat(szStatName,
                                                           &iData)) {
              // use Steam's value
    ...
    for (int iWeapon = WEAPON_NONE + 1; iWeapon < WEAPON_MAX; iWeapon++) {
      for (int iStat = 0; iStat < DODSTAT_MAX; iStat++) {
        if (iValidWeaponStatBitMask & (1 << iStat)) {
          ...
          Q_snprintf(szStatName, ARRAYSIZE(szStatName), "%s.%s",
                     s_WeaponAliasInfo[iWeapon], g_pszStatNames[iStat]);
...
void CDODPlayerStats::MsgFunc_DODPlayerStatsUpdate(bf_read &msg) {
  ...
  // the bitfield indicates which stats are contained in the message.  Set the
  // stats appropriately.
  ...
  // if byte count isn't correct, bail out and don't use this data, rather than
  // risk polluting player stats with garbage
        ...
        if (iValidPlayerStatBitMask & (1 << iStat)) {
          if (m_PlayerStats[iTeam][iClass].m_bDirty[iStat]) {
            ...
            steamapicontext->SteamUserStats()->SetStat(
                szStatName, m_PlayerStats[iTeam][iClass].m_iStat[iStat]);
          }
        }
      }
    }
    ...
    for (int iStat = 0; iStat < DODSTAT_MAX; iStat++) {
      if (m_WeaponStats[iWeapon].m_bDirty[iStat]) {
        if (iValidWeaponStatBitMask & (1 << iStat)) {
          ...
          steamapicontext->SteamUserStats()->SetStat(
              szStatName, m_WeaponStats[iWeapon].m_iStat[iStat]);
        }
      }
    }
  }
...
void CDODPlayerStats::UpdateStats(int iPlayerClass, int iTeam,
                                  dod_stat_accumulator_t *playerStats,
                                  CUtlVector<weapon_stat_t> *vecWeaponStats) {
  ...
  for (int iStat = 0; iStat < DODSTAT_MAX; iStat++) {
    if (iValidPlayerStatBitMask & (1 << iStat)) {
      if (playerStats->m_iStat[iStat] > 0) {
        m_PlayerStats[iTeamIndex][iPlayerClass].m_iStat[iStat] +=
            playerStats->m_iStat[iStat];
...
void CDODPlayerStats::SetNextForceUploadTime() {
  // pick a time a while from now (an hour +/- 15 mins) to upload stats if we
  // haven't gotten a map change by then
