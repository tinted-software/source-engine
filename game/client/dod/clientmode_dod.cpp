...
void CDODModeManager::LevelShutdown(void) { g_pClientMode->LevelShutdown(); }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
ClientModeDODNormal::ClientModeDODNormal() { m_pFreezePanel = NULL; }
...
void ClientModeDODNormal::FireGameEvent(IGameEvent *event) {
  ...
  if (Q_strcmp("dod_round_start", eventname) == 0) {
    ...
    if (dod_playwinmusic.GetBool() == false) {
      if (FStrEq(pszSoundName, "Game.USWin") ||
          FStrEq(pszSoundName, "Game.GermanWin")) {
        return;
      }
    }
    ...
    g_pVGuiLocalize->ConvertANSIToUnicode(pPlayer->GetPlayerName(),
                                          szPlayerName, sizeof(szPlayerName));

    wchar_t wszPrint[128];
    char szPrint[128];

    g_pVGuiLocalize->ConstructString(wszPrint, sizeof(wszPrint),
                                     g_pVGuiLocalize->Find(pTemplate), 1,
                                     szPlayerName);
...
void ClientModeDODNormal::RadioMessage(const char *pszSoundName,
                                       const char *pszSubtitle,
                                       const char *pszSender /* = NULL */,
                                       int iSenderIndex /* = 0 */) {
  C_DODPlayer *pLocalPlayer = C_DODPlayer::GetLocalDODPlayer();
  if (!pLocalPlayer) {
    return;
  ...
  g_pVGuiLocalize->ConstructString(wszPrint, sizeof(wszPrint),
                                   g_pVGuiLocalize->Find(pszSubtitle), 0);
  g_pVGuiLocalize->ConvertUnicodeToANSI(wszPrint, szPrint, sizeof(szPrint));
  ...
  if (pszSender) {
    hudChat->ChatPrintf(
        iSenderIndex, CHAT_FILTER_NONE, "%c%s %s %c: %s", COLOR_PLAYERNAME,
        szPrefix, g_PR->GetPlayerName(iSenderIndex), COLOR_NORMAL, szSuffix);
  } else {
    hudChat->Printf(CHAT_FILTER_NONE, "%c%s %c: %s", color, szPrefix,
                    COLOR_NORMAL, szSuffix);
  }
}
...
//-----------------------------------------------------------------------------
int ClientModeDODNormal::HudElementKeyInput(int down, ButtonCode_t keynum,
