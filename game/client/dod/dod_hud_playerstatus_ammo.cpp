...
float GetScale(int nIconWidth, int nIconHeight, int nWidth, int nHeight) {
  ...
  {
    return flScale;
  } else if (nIconHeight == nHeight &&
             nIconWidth <= nWidth) // no scaling necessary
  {
    return flScale;
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CDoDHudAmmo::CDoDHudAmmo(vgui::Panel *parent, const char *name)
    : vgui::Panel(parent, name) {
  m_iAdditiveWhiteID = vgui::surface()->CreateNewTextureID();
  vgui::surface()->DrawSetTextureFile(m_iAdditiveWhiteID, "vgui/white_additive",
                                      true, false);
...
void CDoDHudAmmo::PaintGrenadeAmmo(CWeaponDODBase *pWpn) {
  ...
  if (pAmmoIcon) {
    ...
    if (nIconWidth <
        small_icon_width -
            XRES(
                2)) // 2 is our buffer for when we need to re-calculate the xpos
    {
    ...
    if (nIconHeight <
        small_icon_height -
            YRES(
                2)) // 2 is our buffer for when we need to re-calculate the ypos
    {
...
void CDoDHudAmmo::PaintBazookaAmmo(CWeaponDODBase *pWpn) {
  ...
  if (m_iAmmo2 > 0 && pExtraIcon) {
    ...
    if (nIconWidth > extra_clip_width || nIconHeight > extra_clip_height) {
      scale = GetScale(nIconWidth, nIconHeight, extra_clip_width,
                       extra_clip_height);
      nIconWidth *= scale;
      nIconHeight *= scale;
    }
    ...
    pExtraIcon->DrawSelf(xpos, ypos, pExtraIcon->Width() * scale,
                         pExtraIcon->Height() * scale, m_clrIcon);
...
void CDoDHudAmmo::PaintMGAmmo(CWeaponDODBase *pWpn) {
  ...
  if (pFullClip) {
    ...
    scale =
        GetScale(nIconWidth, nIconHeight, large_icon_width, large_icon_height);
    ...
    DrawText(buf, xpos + nIconWidth - ((float)nIconWidth / 3.0),
             ypos + nIconHeight - ((float)nIconHeight / 3.0), m_clrTextColor);
  }
  ...
  if (clips > 0 && pExtraClip) {
    ...
    pExtraClip->DrawSelf(xpos, ypos, pExtraClip->Width() * scale,
                         pExtraClip->Height() * scale, m_clrIcon);
    DrawAmmoCount(clips);
  }
}
...
void CDoDHudAmmo::PaintGunAmmo(CWeaponDODBase *pWpn) {
  ...
  if (pFullClip && pEmptyClip) {
    ...
    float flBasePercent = (float)pWpn->GetDODWpnData().m_iHudClipBaseHeight /
                          (float)pWpn->GetDODWpnData().m_iHudClipHeight;
    float flBulletHeightPercent =
        (float)pWpn->GetDODWpnData().m_iHudClipBulletHeight /
        (float)pWpn->GetDODWpnData().m_iHudClipHeight;
    ...
    float flDrawHeight =
        flHeight * (1.0 - (flBasePercent + flBulletHeightPercent * m_iAmmo));

    int nOffset = (int)flDrawHeight;
    int yPosOffset = nOffset * scale;

    pEmptyClip->DrawSelfCropped(xpos, ypos + yPosOffset, 0, nOffset,
                                pEmptyClip->Width(),
                                pEmptyClip->Height() - nOffset, nIconWidth,
                                nIconHeight - yPosOffset, m_clrIcon);
  }
