...
void CDoDHudMGHeatProgressBar::ApplySchemeSettings(vgui::IScheme *pScheme) {
  ...
  m_clrInactiveLow =
      pScheme->GetColor("HudMGHeatBar.InActiveLow", GetFgColor());
}
...
void CDoDHudMGHeatProgressBar::Paint() {
  ...
  while (xpos + m_flSliceWidth < w) {
    ...
    vgui::surface()->DrawFilledRect(xpos, ypos, xpos + m_flSliceWidth,
                                    ypos + t);
...
void CDoDHudMGHeatIcon::ApplySettings(KeyValues *inResourceData) {
  //	Q_strncpy( m_szIcon30cal, inResourceData->GetString( "icon_30cal",
  //"mgheat_30cal" ), sizeof( m_szIcon30cal ) );
  Q_strncpy(m_szIconMg42, inResourceData->GetString("icon_mg42", "mgheat_mg42"),
            sizeof(m_szIconMg42));
...
void CDoDHudMGHeatIcon::Paint() {
  ...
  if (m_icon) {
    m_icon->DrawSelf(0, 0, w, t,
                     (m_flPercentage > m_flWarningLevel) ? m_clrActiveLow
                                                         : m_clrActive);
  }
}
...
// Purpose:
//-----------------------------------------------------------------------------
CDoDHudMGHeat::CDoDHudMGHeat(vgui::Panel *parent, const char *name)
  ...
  if (pPlayer) {
    ...
    if (pWeapon) {
      /*			if ( pWeapon->IsA( WEAPON_30CAL ) )
                              {
                                      CWeapon30cal *p30Cal = (CWeapon30cal
         *)pWeapon;

                                      if( p30Cal )
                                      {
                                              float flPercentage =
         (float)p30Cal->GetWeaponHeat() / 100.0;
