...
ConVar cl_autorezoom("cl_autorezoom", "1", FCVAR_USERINFO | FCVAR_ARCHIVE,
                     "When set to 1, sniper rifles and bazooka weapons will "
                     "automatically raise after each shot");
...
void RecvProxy_StunTime(const CRecvProxyData *pData, void *pStruct,
                        void *pOut) {
  ...
  if ((pPlayerData->m_flStunDuration != pData->m_Value.m_Float) &&
      pData->m_Value.m_Float > 0) {
    pPlayerData->m_flStunAlpha = 1;
  }

  pPlayerData->m_flStunDuration = pData->m_Value.m_Float;
  pPlayerData->m_flStunEffectTime =
      gpGlobals->curtime + pPlayerData->m_flStunDuration;
}
...
RecvPropEHandle(RECVINFO(m_hPlayer)), RecvPropInt(RECVINFO(m_iEvent)),
    RecvPropInt(RECVINFO(m_nData)) END_RECV_TABLE()
        ...
        void RecvProxy_CapAreaIndex(const CRecvProxyData *pData, void *pStruct,
                                    void *pOut) {
...
RecvPropInt(RECVINFO(m_iPlayerClass)),
    ...
    RecvPropBool(RECVINFO(m_bPlanting)), RecvPropBool(RECVINFO(m_bDefusing)),
    RecvPropDataTable("dodsharedlocaldata", 0, 0,
                      &REFERENCE_RECV_TABLE(DT_DODSharedLocalPlayerExclusive)),
    END_RECV_TABLE()
    ...
    IMPLEMENT_CLIENTCLASS_DT(C_DODPlayer, DT_DODPlayer, CDODPlayer)
        RecvPropDataTable(RECVINFO_DT(m_Shared), 0,
                          &REFERENCE_RECV_TABLE(DT_DODPlayerShared)),

    RecvPropDataTable("dodlocaldata", 0, 0,
                      &REFERENCE_RECV_TABLE(DT_DODLocalPlayerExclusive)),
    RecvPropDataTable("dodnonlocaldata", 0, 0,
                      &REFERENCE_RECV_TABLE(DT_DODNonLocalPlayerExclusive)),
...
ConVar cl_ragdoll_pronecheck_distance("cl_ragdoll_pronecheck_distance", "64",
                                      FCVAR_GAMEDLL);
...
void C_DODRagdoll::ImpactTrace(trace_t *pTrace, int iDamageType,
                               const char *pCustomImpactName) {
...
void C_DODRagdoll::CreateDODRagdoll() {
  ...
  if (pPlayer && !pPlayer->IsDormant()) {
    ...
    if (!pPlayer->IsLocalPlayer() && pPlayer->dod_IsInterpolationEnabled()) {
      ...
      int iSeq =
          LookupSequence("RagdollSpawn"); // hax, find a neutral standing pose
      if (iSeq == -1) {
...
void C_DODPlayer::OnAchievementAchieved(int iAchievement) {
  // don't draw the head icon for a length of time after showing the particle
  // effect
...
int C_DODPlayer::DrawModel(int flags) {
  ...
  if (nRetval != 0) {
    // register to draw the head icon, unless we're hiding it due to the
    // "achieved" particle effect
...
void C_DODPlayer::DoRecoil(int iWpnID, float flWpnRecoil) {
  ...
  if (m_Shared.IsInMGDeploy()) {
    flPitchRecoil = 0.0;
    flYawRecoil = 0.0;
  } else if (m_Shared.IsProne() && iWpnID != WEAPON_30CAL &&
...
// Set the amount of pitch and yaw recoil we want to do over the next
// RECOIL_DURATION seconds
...
void C_DODPlayer::GetRecoilToAddThisFrame(float &flPitchRecoil,
                                          float &flYawRecoil) {
...
bool C_DODPlayer::CreateMove(float flInputSampleTime, CUserCmd *pCmd) {
  ...
  if (m_Shared.IsProne() && IsAlive()) {
    ...
    if (flSpeed > 10) {
      ...
      // Quick Checks to make sure it isn't bigger or smaller than our sway
      // amount
...
void C_DODPlayer::GetToolRecordingState(KeyValues *msg) {
  ...
  BaseEntityRecordingState_t *pBaseEntityState =
      (BaseEntityRecordingState_t *)msg->GetPtr("baseentity");
  if (IsLocalPlayer()) {
    pBaseEntityState->m_bVisible =
        !IsDormant() && IsAlive() && (GetTeamNumber() != TEAM_SPECTATOR) &&
        (GetRenderMode() != kRenderNone) &&
        (GetObserverMode() != OBS_MODE_DEATHCAM) && !IsEffectActive(EF_NODRAW);
  }
#endif
}
...
void C_DODPlayer::FireEvent(const Vector &origin, const QAngle &angles,
  ...
  if( event == 7001 )
  {
          bool bInWater = ( enginetrace->GetPointContents(origin) &
  CONTENTS_WATER );
  ...
  COLLISION_GROUP_NONE, &tr ); if ( tr.fractionleftsolid )
                  {
                          data.m_vOrigin = origin + (vecTrace *
...
// NVNT check grenade things ( -- this is here to avoid modificaions to the
// grenade class -- )
...
void C_DODPlayer::ClientThink() {
  ...
  if (IsLocalPlayer()) {
    ...
    // NVNT check grenade things ( -- this is here to avoid modificaions to the
    // grenade class -- )
    C_WeaponDODBaseGrenade *heldGrenade =
        dynamic_cast<C_WeaponDODBaseGrenade *>(GetActiveDODWeapon());
    if (heldGrenade) {
      if (!s_holdingGrenade) {
      ...
      if (pinPulled != s_grenadePinPulled) {
        if (pinPulled) {
          if (haptics)
            haptics->ProcessHapticEvent(
                3, "Weapons", heldGrenade->GetClassname(), "PinPulled");
        } else {
          if (haptics)
            haptics->ProcessHapticEvent(
                3, "Weapons", heldGrenade->GetClassname(), "PinReplaced");
        }
      ...
      if (grenadeArmed != s_grenadeArmed) {
        if (grenadeArmed) {
          if (haptics)
            haptics->ProcessHapticEvent(3, "Weapons",
                                        heldGrenade->GetClassname(), "Armed");
        } else {
          if (haptics)
            haptics->ProcessHapticEvent(3, "Weapons",
                                        heldGrenade->GetClassname(), "Unarmed");
        }
      ...
      C_DODBaseBombWeapon *heldBomb =
          dynamic_cast<C_DODBaseBombWeapon *>(GetActiveDODWeapon());
        ...
        if (isPlanting != s_bombPlanting) {
          if (isPlanting) {
            if (!s_bombPlanting) {
              ...
              if (haptics)
                haptics->ProcessHapticEvent(3, "Weapons",
                                            heldBomb->GetClassname(), "Plant");
            }
          } else {
            if (s_bombPlanting) {
              if (haptics)
                haptics->ProcessHapticEvent(
                    3, "Weapons", heldBomb->GetClassname(), "StopPlant");
            }
          }
        }
...
void C_DODPlayer::StaminaSoundThink(void) {
  if (m_bPlayingLowStaminaSound) {
    if (!IsAlive() || m_Shared.GetStamina() >= LOW_STAMINA_THRESHOLD) {
      ...
      // SoundFadeOut will destroy this sound, so we will have to create another
      // one if we go below the threshold again soon
      m_pStaminaSound = NULL;
    ...
    if (IsAlive() && m_Shared.GetStamina() < LOW_STAMINA_THRESHOLD) {
      ...
      if (!m_pStaminaSound)
        m_pStaminaSound =
            controller.SoundCreate(filter, entindex(), "Player.Sprint");
...
void C_DODPlayer::PopHelmet(Vector vecDir, Vector vecForceOrigin, int iModel) {
...

  {
    ...
    GetAttachment(iAttachment, vecHead,
                  angHeadAngles); // attachment 1 is the head attachment
  }
...
bool C_DODPlayer::IsWeaponLowered(void) {
  ...
  if (GetWaterLevel() > 2 &&
      pWeapon->GetDODWpnData().m_WeaponType != WPN_TYPE_MELEE)
    return true;
  ...
  if (m_nButtons & IN_SPEED && (m_nButtons & IN_FORWARD) &&
      m_Shared.GetStamina() >= 5 && !m_Shared.IsDucking())
    return true;
...
ShadowType_t C_DODPlayer::ShadowCastType(void) {
  ...
  if (pLocalPlayer && pLocalPlayer->GetObserverTarget() == this &&
...
void C_DODPlayer::GetShadowRenderBounds(Vector &mins, Vector &maxs,
...
bool C_DODPlayer::GetShadowCastDirection(Vector *pDirection,
...
void C_DODPlayer::ProcessMuzzleFlashEvent() {
  ...
  bool bInToolRecordingMode =
      ToolsEnabled() && clienttools->IsInRecordingMode();
  ...
  if (cl_muzzleflash_dlight_3rd.GetBool() &&
      pWeapon->GetAttachment(iMuzzleFlashAttachment, vecOrigin, angAngles)) {
    // Muzzleflash light
  ...
  if (pszMuzzleFlashEffect) {
    DispatchParticleEffect(pszMuzzleFlashEffect, PATTACH_POINT_FOLLOW, pWeapon,
                           1);
  }
...
bool C_DODPlayer::SetFOV(CBaseEntity *pRequester, int FOV,
                         float zoomRate /* = 0.0f */) {
  /*
  if( FOV < 30 )
  {
  ...
  ); sf.fadeFlags = FFADE_IN; vieweffects->Fade( sf );
  }
  else
  {
...
void C_DODPlayer::CalcObserverView(Vector &eyeOrigin, QAngle &eyeAngles,
                                   float &fov) {
...
void C_DODPlayer::CalcDODDeathCamView(Vector &eyeOrigin, QAngle &eyeAngles,
                                      float &fov) {
  CBaseEntity *killer = GetObserverTarget();

  // float interpolation = ( gpGlobals->curtime - m_flDeathTime ) /
  ...
  m_flObserverChaseDistance =
      clamp(m_flObserverChaseDistance, CHASE_CAM_DISTANCE_MIN,
  ...
  if (pRagdoll) {
    ...
    origin.z +=
        VEC_DEAD_VIEWHEIGHT_SCALED(this).z; // look over ragdoll, not through
  }
...
void C_DODPlayer::CalcChaseCamView(Vector &eyeOrigin, QAngle &eyeAngles,
                                   float &fov) {
  ...
  C_BaseEntity::PushEnableAbsRecomputations(
      false); // HACK don't recompute positions while doing RayTrace
  UTIL_TraceHull(origin, viewpoint, WALL_MIN, WALL_MAX, MASK_SOLID, target,
                 COLLISION_GROUP_NONE, &trace);
  C_BaseEntity::PopEnableAbsRecomputations();
...
// observer mode
//-----------------------------------------------------------------------------
void C_DODPlayer::CalcFreezeCamView(Vector &eyeOrigin, QAngle &eyeAngles,
                                    float &fov) {
  ...
  C_BaseEntity::PushEnableAbsRecomputations(
      false); // HACK don't recompute positions while doing RayTrace
  UTIL_TraceLine(vecCamTarget, vecTargetPos, MASK_SOLID, pTarget,
                 COLLISION_GROUP_NONE, &trace);
  C_BaseEntity::PopEnableAbsRecomputations();
  if (trace.fraction < 1.0) {
...
static ConVar cl_coldbreath_forcestamina("cl_coldbreath_forcestamina", "0",
                                         FCVAR_CHEAT);
...
bool C_DODPlayer::CreateColdBreathEmitter(void) {
  ...
  if (!m_hColdBreathEmitter) {
    ...
    m_hColdBreathMaterial =
        m_hColdBreathEmitter->GetPMaterial("sprites/frostbreath");
  ...
  if (m_flColdBreathTimeStart < gpGlobals->curtime) {
    ...
    if (m_flColdBreathTimeEnd < gpGlobals->curtime) {
      ...
      if (m_Shared.m_flStamina < LOW_STAMINA_THRESHOLD ||
          cl_coldbreath_forcestamina.GetBool()) {
        m_flColdBreathTimeStart =
            gpGlobals->curtime +
            RandomFloat(COLDBREATH_EMIT_MIN * COLDBREATH_EMIT_SCALE,
                        COLDBREATH_EMIT_MAX * COLDBREATH_EMIT_SCALE);
        float flDuration =
            RandomFloat(COLDBREATH_DURATION_MIN, COLDBREATH_DURATION_MAX);
        m_flColdBreathTimeEnd = m_flColdBreathTimeStart + flDuration;
      } else {
        m_flColdBreathTimeStart =
            gpGlobals->curtime +
            RandomFloat(COLDBREATH_EMIT_MIN, COLDBREATH_EMIT_MAX);
        float flDuration =
            RandomFloat(COLDBREATH_DURATION_MIN, COLDBREATH_DURATION_MAX);
        m_flColdBreathTimeEnd = m_flColdBreathTimeStart + flDuration;
      }
...
void C_DODPlayer::CalculateIKLocks(float currentTime) {
  ...
  for (int i = 0; i < targetCount; i++) {
    ...
    case IK_GROUND: {
      pTarget->SetPos(
          Vector(pTarget->est.pos.x, pTarget->est.pos.y, GetRenderOrigin().z));
    ...
    case IK_ATTACHMENT: {
      ...
      for (CEntitySphereQuery sphere(pTarget->est.pos, 64);
        ...
        // debugoverlay->AddBoxOverlay( origin, Vector( -1, -1, -1 ), Vector( 1,
        // 1, 1 ), QAngle( 0, 0, 0 ), 255, 0, 0, 0, 0 );
...
void C_DODPlayer::EmitColdBreathParticles(void) {
  // Get the position to emit from - look into caching this off we are doing
  // redundant work in the case of allies (see dod_headiconmanager.cpp).
  Vector vecOrigin;
  QAngle vecAngle;
  ...
  SimpleParticle *pParticle =
      static_cast<SimpleParticle *>(m_hColdBreathEmitter->AddParticle(
          sizeof(SimpleParticle), m_hColdBreathMaterial, vecOrigin));
  if (pParticle) {
    pParticle->m_flLifetime = 0.0f;
    pParticle->m_flDieTime =
        RandomFloat(COLDBREATH_PARTICLE_LIFE_MIN, COLDBREATH_PARTICLE_LIFE_MAX);
    if (m_Shared.m_flStamina < LOW_STAMINA_THRESHOLD ||
        cl_coldbreath_forcestamina.GetBool()) {
    ...
    if (m_Shared.m_flStamina < LOW_STAMINA_THRESHOLD ||
      ...
      pParticle->m_vecVelocity = (vecForward * RandomFloat(10.0f, 30.0f)) +
                                 (vecRight * RandomFloat(-2.0f, 2.0f)) +
    ...
    float flParticleSize =
        RandomFloat(COLDBREATH_PARTICLE_SIZE_MIN, COLDBREATH_PARTICLE_SIZE_MAX);
    float flParticleScale =
        RandomFloat(COLDBREATH_ENDSCALE_MIN, COLDBREATH_ENDSCALE_MAX);
    if (m_Shared.m_flStamina < LOW_STAMINA_THRESHOLD ||
        cl_coldbreath_forcestamina.GetBool()) {
...
void C_DODPlayer::ComputeWorldSpaceSurroundingBox(Vector *pVecWorldMins,
                                                  Vector *pVecWorldMaxs) {
  m_Shared.ComputeWorldSpaceSurroundingBox(pVecWorldMins, pVecWorldMaxs);
}

//-----------------------------------------------------------------------------
// Purpose: Try to steer away from any players and objects we might
// interpenetrate
//-----------------------------------------------------------------------------
#define DOD_AVOID_MAX_RADIUS_SQR                                               \
  5184.0f // Based on player extents and max buildable extents.
...
void C_DODPlayer::AvoidPlayers(CUserCmd *pCmd) {
  ...
  for (int i = 0; i < pTeam->GetNumPlayers(); ++i) {
    ...
    if (IsBoxIntersectingBox(vecDODPlayerMin, vecDODPlayerMax, vecAvoidMin,
                             vecAvoidMax)) {
  ...
  if (pIntersectPlayer) {
    VectorSubtract(pIntersectPlayer->WorldSpaceCenter(), vecDODPlayerCenter,
                   vecDelta);

    Vector vRad =
        pIntersectPlayer->WorldAlignMaxs() - pIntersectPlayer->WorldAlignMins();
    vRad.z = 0;
  ...
  float flPushStrength =
      RemapValClamped(vecDelta.Length(), flAvoidRadius, 0, 0,
                      DOD_MAX_SEPARATION_FORCE); // flPushScale;
...
bool C_DODPlayer::ShouldShowNemesisIcon() {
  /*
  // we should show the nemesis effect on this player if he is the nemesis of
  the local player,
