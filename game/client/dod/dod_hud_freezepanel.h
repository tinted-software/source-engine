...
CTFHudPlayerHealth( parent, name )
        ...
        virtual const char *GetResFilename( void ) { return
"resource/UI/FreezePanelKillerHealth.res"; } virtual void OnThink()
        {
                // Do nothing. We're just preventing the base health panel from
updating.
        }
};
*/
...
public:
  ...
  int HudElementKeyInput(int down, ButtonCode_t keynum,
                         const char *pszCurrentBinding);

  bool IsHoldingAfterScreenShot(void) { return m_bHoldingAfterScreenshot; }

protected:
  CDODFreezePanelCallout *
  TestAndAddCallout(Vector &origin, Vector &vMins, Vector &vMaxs,
