...
bool CHeadIconManager::Init() {
  if (!m_pAlliesIconMaterial) {
    m_pAlliesIconMaterial = materials->FindMaterial(
        "sprites/player_icons/american", TEXTURE_GROUP_VGUI);
  }

  if (!m_pAxisIconMaterial) {
    m_pAxisIconMaterial = materials->FindMaterial("sprites/player_icons/german",
                                                  TEXTURE_GROUP_VGUI);
  }
...
//-----------------------------------------------------------------------------
// Call from player render calls to indicate a head icon should be drawn for
// this player this frame
//-----------------------------------------------------------------------------
