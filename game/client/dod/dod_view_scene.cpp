...
ConVar cl_enablespectatoreffects("cl_enablespectatoreffects", "0",
                                 FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
                                 "Enable/disable spectator camera effects");
ConVar cl_enabledeatheffects("cl_enabledeatheffects", "1",
                             FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
                             "Enable/disable death camera effects");
ConVar cl_enabledeathfilmgrain("cl_enabledeathfilmgrain", "1",
                               FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
                               "Enable/disable the death camera film grain");

ConVar cl_deatheffect_force_on("cl_deatheffect_always_on", "0",
                               FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
                               "Always show the death effect");

// Console variables to define lookup maps
static void UpdateCameraLookups(IConVar *var, char const *pOldString,
                                float flOldValue) {
...
ConVar cl_spectatorlookup("cl_spectatorlookup",
                          "materials\\colorcorrection\\bnw_c.raw",
                          FCVAR_CLIENTDLL | FCVAR_CHEAT,
                          "Sets the lookup map to use for spectator cameras",
                          UpdateCameraLookups);
ConVar cl_deathlookup("cl_deathlookup", "materials\\colorcorrection\\bnw_c.raw",
                      FCVAR_CLIENTDLL | FCVAR_CHEAT,
                      "Sets the lookup map to use for death cameras",
                      UpdateCameraLookups);
...
void CDODViewRender::PerformStunEffect(const CViewSetup &view) {
  ...
  IMaterial *pMaterial = materials->FindMaterial(
      "effects/stun", TEXTURE_GROUP_CLIENT_EFFECTS, true);
  ...
  if (pPlayer->m_flStunAlpha < pPlayer->m_flStunMaxAlpha) {
    ...
    pPlayer->m_flStunAlpha =
        MIN(pPlayer->m_flStunAlpha, pPlayer->m_flStunMaxAlpha);
    ...
    pRenderContext->CopyRenderTargetToTextureEx(m_pStunTexture, 0, &srcRect,
                                                NULL);
    ...
    float flAlpha = pPlayer->m_flStunMaxAlpha *
                    (pPlayer->m_flStunEffectTime - gpGlobals->curtime) /
                    pPlayer->m_flStunDuration;
...
void CDODViewRender::InitColorCorrection() {
  m_SpectatorLookupHandle = g_pColorCorrectionMgr->AddColorCorrection(
      "spectator", cl_spectatorlookup.GetString());
  m_DeathLookupHandle = g_pColorCorrectionMgr->AddColorCorrection(
      "death", cl_deathlookup.GetString());
}
...
//-----------------------------------------------------------------------------
// Purpose: Do the per-frame setup required for the spectator cam color
...
void CDODViewRender::SetupColorCorrection() {
  ...
  if (pPlayer->GetTeamNumber() == TEAM_SPECTATOR) {
    if (cl_enablespectatoreffects.GetBool()) {
      ...
      g_pColorCorrectionMgr->SetColorCorrectionWeight(m_SpectatorLookupHandle,
                                                      1.0f);
      g_pColorCorrectionMgr->SetColorCorrectionWeight(m_DeathLookupHandle,
                                                      0.0f);
    ...
    if (cl_deatheffect_force_on.GetBool()) {
      ...
      flEffectAlpha =
          MIN(1.0f, (gpGlobals->curtime - pPlayer->GetDeathTime()) / (2.0f));
    } else {
      ...
      if (roundstate < STATE_RND_RUNNING) {
        ...
        float flFadeOutStartTime =
            MAX(DODGameRules()->m_flLastRoundStateChangeTime,
                pPlayer->m_flLastRespawnTime);

        // fade in from round start time
        flEffectAlpha =
            1.0 - (gpGlobals->curtime - flFadeOutStartTime) / (2.0f);
...
void CDODViewRender::RenderView(const CViewSetup &view, int nClearFlags,
                                int whatToDraw) {
