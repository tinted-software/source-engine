...
CDODRestartRoundLabel::CDODRestartRoundLabel(const char *pElementName)
  ...
  g_pVGuiLocalize->ConstructString(labelText, sizeof(labelText),
                                   g_pVGuiLocalize->Find("#clan_game_restart"),
                                   2, L"0", L"00");
...
void CDODRestartRoundLabel::OnThink() {
  ...
  if (GetAlpha() > 0) {
    ...
    _snwprintf(minutes, sizeof(minutes) / sizeof(wchar_t), L"%d",
               (iSecondsRemaining / 60));
    _snwprintf(seconds, sizeof(seconds) / sizeof(wchar_t), L"%02d",
               (iSecondsRemaining % 60));

    wchar_t labelText[128];
    g_pVGuiLocalize->ConstructString(
        labelText, sizeof(labelText),
        g_pVGuiLocalize->Find("#clan_game_restart"), 2, minutes, seconds);
