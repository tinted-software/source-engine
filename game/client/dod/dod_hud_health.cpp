...
// Purpose:
//-----------------------------------------------------------------------------
void CHudHealth::Init() { m_iHealth = 100; }
...
void CHudHealth::Paint(void) {
  ...
  int nOffset =
      m_pIconHealthOverlay->Height() * (1.0 - ((float)m_iHealth / 100.0));
  if (nOffset < m_pIconHealthOverlay->Height()) {
    m_pIconHealthOverlay->DrawSelfCropped(xpos + 55, ypos + 27, 0, 0,
                                          m_pIconHealthOverlay->Width(),
                                          nOffset, clrIcon);
  }

  nOffset = m_pIconHealthBar->Height() * (1.0 - ((float)m_iHealth / 100.0));
  if (nOffset < m_pIconHealthBar->Height()) {
    m_pIconHealthBar->DrawSelfCropped(
        xpos + 99, ypos + 27 + nOffset, 0, nOffset, m_pIconHealthBar->Width(),
  ...
  if (nOffset < m_pIconStaminaBar->Height()) {
    m_pIconStaminaBar->DrawSelfCropped(
        xpos + 8, ypos + 12 + nOffset, 0, nOffset, m_pIconStaminaBar->Width(),
