...
static void FX_Flecks(const Vector &origin, trace_t *trace, char materialType,
                      int iScale) {
  Vector color;
...
//-----------------------------------------------------------------------------
void DOD_PerformCustomEffects(const Vector &vecOrigin, trace_t &tr,
...
void ImpactCallback(const CEffectData &data) {
  ...
  if (Impact(vecOrigin, vecStart, iMaterial, iDamageType, iHitbox, pEntity,
    ...
    if (pPlayer) {
      ...
      flScale =
          RemapValClamped(flDistSqr, (400 * 400), (1000 * 1000), 0.8, 1.2);
    }
    ...
    if (random->RandomInt(1, 10) <= 3 && (iDamageType == DMG_BULLET)) {
      ...
      C_BaseEntity::EmitSound(filter, SOUND_FROM_WORLD, "Bounce.Shrapnel",
                              &vecOrigin);
    }
  }
}
