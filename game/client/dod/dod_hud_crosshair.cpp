...
void CHudDODCrosshair::Paint() {
  ...
  if (Q_stricmp(m_szPreviousCrosshair, crosshairfile) != 0) {
    ...
    vgui::surface()->DrawSetTextureFile(m_iCrosshairTextureID, buf, true,
                                        false);
    ...
    Q_strncpy(m_szPreviousCrosshair, crosshairfile,
              sizeof(m_szPreviousCrosshair));
  }
  ...
  if (m_pFrameVar) {
    if (cl_dynamic_crosshair.GetBool() == false) {
      ...
      float accuracy =
          pWeapon->GetWeaponAccuracy(pPlayer->GetAbsVelocity().Length2D());
      ...
      m_flAccuracy = Approach(accuracy, m_flAccuracy,
                              cl_crosshair_approach_speed.GetFloat());
  ...
  vgui::surface()->DrawTexturedRect(iX - iWidth, iY - iHeight, iX + iWidth,
                                    iY + iHeight);
