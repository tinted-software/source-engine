...
public:
  ...
  static void RunFlexDelay(int nFlexWeightCount, float *pFlexWeights,
                           float *pFlexDelayedWeights, float &flFlexDelayTime);
...
public:
  ...
  // Assumes m_flexWeight array has been set up, this adds the actual currently
  // playing
...
private:
  ...
  bool ProcessFlexAnimationSceneEvent(CSceneEventInfo *info,
                                      CChoreoScene *scene, CChoreoEvent *event);
  bool ProcessFlexSettingSceneEvent(CSceneEventInfo *info, CChoreoScene *scene,
                                    CChoreoEvent *event);
