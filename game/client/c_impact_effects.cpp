...
static void CreateFleckParticles(const Vector &origin, const Vector &color,
  ...
  float fScale =
      g_pParticleSystemMgr->ParticleThrottleScaling() * (float)iScale;
...
void FX_DustImpact(const Vector &origin, trace_t *tr, float flScale) {
  ...
  int nParticles2 =
      (int)(0.83f + fNumParticles); // <-- most visible particle type.
  ...
  for (i = 0; i < nParticles1; i++) {
    ...
    if (pParticle != NULL) {
      ...
      // scaled
      pParticle->m_uchStartSize =
          (unsigned char)(flScale * random->RandomInt(3, 4) * (nId + 1));

      // scaled
      pParticle->m_uchEndSize =
          (unsigned char)(flScale * pParticle->m_uchStartSize * 4);
