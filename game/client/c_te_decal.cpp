...
RecvPropVector(RECVINFO(m_vecOrigin)), RecvPropVector(RECVINFO(m_vecStart)),
    RecvPropInt(RECVINFO(m_nEntity)), RecvPropInt(RECVINFO(m_nHitbox)),
...
void TE_Decal(IRecipientFilter &filter, float delay, const Vector *pos,
  ...
  if ((entity == 0) && (hitbox != 0)) {
    Ray_t ray;
    ray.Init(*start, *pos);
    staticpropmgr->AddDecalToStaticProp(*start, *pos, hitbox - 1, index, false,
                                        tr);
  } else {
...
void C_TEDecal::PostDataUpdate(DataUpdateType_t updateType) {
  ...
  TE_Decal(filter, 0.0f, &m_vecOrigin, &m_vecStart, m_nEntity, m_nHitbox,
           m_nIndex);
}
...
void TE_Decal(IRecipientFilter &filter, float delay, KeyValues *pKeyValues) {
  ...
  TE_Decal(filter, 0.0f, &vecOrigin, &vecStart, 0, nHitbox,
