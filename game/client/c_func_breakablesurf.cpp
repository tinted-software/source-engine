...
private:
  ...
  inline void SetStyleType(int w, int h, int type) {
    ...
    // Clear old value
    m_nPanelBits[w][h] &=
        0xF0; // ( ~0x03 << 2 ); Left shifting a negative value has undefined
              // behavior. Use the constant 0xF0 instead.
    // Insert new value
...
RecvPropInt(RECVINFO(m_nNumWide)), RecvPropInt(RECVINFO(m_nNumHigh)),
  ...
  if (materialCount != 1) {
    Warning("Encountered func_breakablesurf that has a material applied to "
