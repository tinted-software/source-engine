...
void C_BaseExplosionEffect::CreateCore(void) {
  ...
  for (i = 0; i < 16; i++) {
    ...
    if (pParticle != NULL) {
      ...
      pParticle->m_uchStartSize =
          clamp(pParticle->m_uchStartSize, (uint8)4, (uint8)32);
  ...
  for (i = 0; i < numFireballs; i++) {
    ...
    if (pParticle != NULL) {
      ...
      pParticle->m_uchStartSize =
          clamp(pParticle->m_uchStartSize, (uint8)32, (uint8)85);
...
void C_WaterExplosionEffect::CreateMisc(void) {
  ...
  for (i = 0; i < numDrops; i++) {
    ...
    tParticle->m_flWidth =
        clamp(random->RandomFloat(1.0f, 3.0f) * flScale, 0.1f, 4.0f);
    tParticle->m_flLength =
        random->RandomFloat(length * 0.25f, length) /* * flScale*/;

    colorRamp = random->RandomFloat(1.5f, 2.0f);

    FloatToColor32(tParticle->m_color, MIN(1.0f, m_vecColor[0] * colorRamp),
                   MIN(1.0f, m_vecColor[1] * colorRamp),
  ...
  for (i = 0; i < 4; i++) {
    ...
    tParticle->m_flWidth =
        clamp(random->RandomFloat(2.0f, 3.0f) * flScale, 0.1f, 4.0f);
    tParticle->m_flLength =
        random->RandomFloat(length * 0.25f, length) /* * flScale*/;
