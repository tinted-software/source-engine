...
int CPlayerAndObjectEnumerator::GetObjectCount() { return m_Objects.Size(); }
...
// Actual work code
IterationRetval_t
CPlayerAndObjectEnumerator::EnumElement(IHandleEntity *pHandleEntity) {
  if (!m_pLocal)
...
#ifdef INVASION_CLIENT_DLL
  // If it's solid to player movement, don't steer around it since we'll just
  // bump into it
  if (pEnt->GetCollisionGroup() ==
