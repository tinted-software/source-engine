...
public:
  virtual void
  Update(int startframe,  // World update ( un-modded ) most recently received
...
protected:
  ...
  // Helpers to call pre and post think for player, and to call think if a think
  // function is set
...
private:
  virtual void
  _Update(bool received_new_world_update,
          bool validframe,           // Is frame data valid
          int incoming_acknowledged, // Last command acknowledged to have been
                                     // run by server (un-modded)
          int outgoing_command // Last command (most recent) sent to server
