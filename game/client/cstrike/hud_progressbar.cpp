...
ConVar cl_c4progressbar("cl_c4progressbar", "1", 0,
...
public:
  ...
  CPanelAnimationVar(Color, m_clrProgress, "ProgressBarFg",
                     "ProgressBar.FgColor");
};
...
void CHudProgressBar::Paint() {
  ...
  vgui::surface()->DrawOutlinedRect(xOffset, yOffset, xOffset + wide,
                                    yOffset + tall);

  if (pPlayer->m_iProgressBarDuration > 0) {
    // ProgressBarStartTime is now with respect to m_flSimulationTime rather
    // than local time
    float percent =
    ...
    clr[3] = 240;
    vgui::surface()->DrawSetColor(clr);
    vgui::surface()->DrawFilledRect(xOffset + 2, yOffset + 2,
                                    xOffset + (int)(percent * wide) - 2,
                                    yOffset + tall - 2);
  }
}
...
bool CHudProgressBar::ShouldDraw() {
  ...
  if (!pPlayer || pPlayer->m_iProgressBarDuration == 0 ||
