...
ConVar cl_radaralpha("cl_radaralpha", "200", FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
                     NULL, true, 0, true, 255);
ConVar cl_locationalpha("cl_locationalpha", "150",
                        FCVAR_CLIENTDLL | FCVAR_ARCHIVE, NULL, true, 0, true,
                        255);
...
CHudRadar::CHudRadar(const char *pName)
    : vgui::Panel(NULL, "HudRadar"), CHudElement(pName) {
...
void CHudRadar::SetVisible(bool state) {
  ...
  if (g_pMapOverview &&
      g_pMapOverview->GetMode() == CCSMapOverview::MAP_MODE_RADAR) {
...
void CHudRadar::Paint() {
  // We are the hud element still, but Overview is in charge of the new style
  // now.
  return;
}

void CHudRadar::WorldToRadar(const Vector location, const Vector origin,
  ...
  // The dot is out of the radar's range.. Scale it back so that it appears on
  // the border
  if ((-1 * y_diff) > fRange) {
    float flScale;
...
void CHudRadar::DrawPlayerOnRadar(int iPlayer, C_CSPlayer *pLocalPlayer) {
  ...
  bool bOppositeTeams =
      (pLocalPlayer->GetTeamNumber() != TEAM_UNASSIGNED &&
       pCSPR->GetTeam(iPlayer) != pLocalPlayer->GetTeamNumber());

  if (bOppositeTeams && pPlayer->m_bDetected == false)
    return;

  WorldToRadar(pPlayer->GetAbsOrigin(), pLocalPlayer->GetAbsOrigin(),
               pLocalPlayer->LocalEyeAngles(), x, y, z_delta);

  if (pCSPR->HasC4(iPlayer) || pCSPR->IsVIP(iPlayer) || bOppositeTeams) {
    r = 250;
    g = 0;
    b = 0;
  } else if (0 /*m_bTrackArray[i-1] == true */) // Tracked players (friends we
                                                // want to keep track of on the
                                                // radar)
  {
    iBaseDotSize *= 2;
    r = 185;
  ...
  if (g_RadarFlashes.Count() > iPlayer)
    bRadarFlash = g_RadarFlashes[iPlayer].m_bRadarFlash &&
                  g_RadarFlashes[iPlayer].m_iNumRadarFlashes > 0;
  ...
  if (bRadarFlash || GetClientVoiceMgr()->IsPlayerSpeaking(iPlayer)) {
    ...
    DrawRadarDot(x, y, z_delta, iBaseDotSize, RADAR_DOT_LARGE_FLASH, r, g, b,
                 a);
  } else {
...
void CHudRadar::DrawEntityOnRadar(CBaseEntity *pEnt, C_CSPlayer *pLocalPlayer,
                                  int flags, int r, int g, int b, int a) {
  float x, y, z_delta;
  int iBaseDotSize = 4;

  WorldToRadar(pEnt->GetAbsOrigin(), pLocalPlayer->GetAbsOrigin(),
               pLocalPlayer->LocalEyeAngles(), x, y, z_delta);
...
CHudLocation::CHudLocation(const char *pName)
    : vgui::Label(NULL, "HudLocation", ""), CHudElement(pName) {
  SetParent(g_pClientMode->GetViewport());
}
...
bool CHudLocation::ShouldDraw() {
  CCSMapOverview *pCSMapOverview =
      (CCSMapOverview *)GET_HUDELEMENT(CCSMapOverview);

  if (g_pMapOverview &&
      g_pMapOverview->GetMode() == CMapOverview::MAP_MODE_RADAR &&
      pCSMapOverview && pCSMapOverview->ShouldDraw() == true)
    return true;
  else if (g_pMapOverview &&
           g_pMapOverview->GetMode() == CMapOverview::MAP_MODE_INSET)
    return true;
