...
float CCSReplay::GetSentryKillScreenshotDelay() {
  ConVarRef replay_screenshotsentrykilldelay(
      "replay_screenshotsentrykilldelay");
  return replay_screenshotsentrykilldelay.IsValid()
             ? replay_screenshotsentrykilldelay.GetFloat()
             : 0.5f;
}
...
void CCSReplay::FireGameEvent(IGameEvent *pEvent) {
  ...
  if (FStrEq(pEvent->GetName(), "player_death")) {
    ...
    // Player death?
      ...
      ConVarRef replay_deathcammaxverticaloffset(
          "replay_deathcammaxverticaloffset");
      ConVarRef replay_playerdeathscreenshotdelay(
          "replay_playerdeathscreenshotdelay");
      params.m_flDelay = replay_playerdeathscreenshotdelay.IsValid()
                             ? replay_playerdeathscreenshotdelay.GetFloat()
                             : 0.0f;
      params.m_nEntity = pLocalPlayer->entindex();
      params.m_posCamera.Init(0, 0,
                              replay_deathcammaxverticaloffset.IsValid()
                                  ? replay_deathcammaxverticaloffset.GetFloat()
                                  : 150);
...
public:
  virtual CReplay *Create() { return new CCSReplay(); }
};

static CCSReplayFactory s_ReplayManager;
IReplayFactory *g_pReplayFactory = &s_ReplayManager;

EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CCSReplayFactory, IReplayFactory,
                                  INTERFACE_VERSION_REPLAY_FACTORY,
                                  s_ReplayManager);
