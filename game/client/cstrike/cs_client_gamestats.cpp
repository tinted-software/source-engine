...
// Desc: 		Manages client side stat storage, accumulation, and
// access Author: 		Peter Freese <peter@hiddenpath.com> Date:
// 2009/09/11 Copyright:	© 2009 Hidden Path Entertainment
//
// Keywords:
//-------------------------------------------------------------
...
void CCSClientGameStats::FireGameEvent(IGameEvent *event) {
  ...
  if (0 == Q_strcmp(pEventName, "player_stats_updated")) {
    ...
    if (GetGlobalTeam(TEAM_TERRORIST) != NULL)
      iCurrentPlayerCount +=
          GetGlobalTeam(TEAM_TERRORIST)->Get_Number_Players();

    m_matchMaxPlayerCount = MAX(m_matchMaxPlayerCount, iCurrentPlayerCount);
  }

  // The user stats for a round get sent piece meal, so we'll wait until a new
  // round starts before sending the previous round stats.
  else if (Q_strcmp(pEventName, "round_start") == 0 &&
           m_roundStats[CSSTAT_PLAYTIME] > 0) {
    SRoundData *pRoundStatData = new SRoundData(&m_roundStats);
    C_CSPlayer *pPlayer = ToCSPlayer(C_BasePlayer::GetLocalPlayer());
    if (pPlayer) {
      // Our current money + what we spent is what we started with at the
      // beginning of round
      pRoundStatData->nStartingMoney =
          pPlayer->GetAccount() + m_roundStats[CSSTAT_MONEY_SPENT];
    }
...
void CCSClientGameStats::RetrieveSteamStats() {
  ...
  for (int i = 0; i < CSSTAT_MAX; ++i) {
    ...
    if (steamapicontext->SteamUserStats()->GetStat(
            CSStatProperty_Table[i].szSteamName, &iData)) {
...
void CCSClientGameStats::UpdateSteamStats() {
  ...
  if (!m_bSteamStatsDownload) {
    // try to periodically download stats from Steam if we haven't gotten them
    // yet
  ...
  for (int i = 0; i < CSSTAT_MAX; ++i) {
    ...
    steamapicontext->SteamUserStats()->SetStat(
        CSStatProperty_Table[i].szSteamName,
...
CON_COMMAND_F(stats_dump, "Dumps all player stats", FCVAR_DEVELOPMENTONLY) {
  ...
  const StatsCollection_t &accumulatedStats =
  ...
  for (int i = 0; i < CSSTAT_MAX; ++i) {
    ...
    Msg("%42s: %i\n", CSStatProperty_Table[i].szSteamName,
        accumulatedStats[CSStatProperty_Table[i].statId]);
  }
}

#if defined(_DEBUG)
CON_COMMAND_F(stats_preload,
              "Load stats with data ripe for getting achievmenets",
  ...
  StatsCollection_t &lifetimeStats =
      const_cast<StatsCollection_t &>(g_CSClientGameStats.GetLifetimeStats());
...
#if defined(_DEBUG)
CON_COMMAND_F(stats_corrupt, "Load stats with corrupt values",
              FCVAR_DEVELOPMENTONLY) {
  struct DataSet {
    CSStatType_t statId;
    int value;
...
PlayerStatData_t CCSClientGameStats::GetStatByIndex(int index) {
  ...
  statData.pStatDisplayName =
      g_pVGuiLocalize->Find(CSStatProperty_Table[index].szLocalizationToken);
...
void CCSClientGameStats::MsgFunc_MatchStatsUpdate(bf_read &msg) {
  ...
  for (int iStat = firstStat; iStat < CSSTAT_MAX && msg.GetNumBytesLeft() > 0;
       iStat++) {
  ...
  // sanity check: the message should contain exactly the # of bytes we expect
...
void CCSClientGameStats::MsgFunc_PlayerStatsUpdate(bf_read &msg) {
  // Note: if any check fails while decoding this message, bail out and
  // disregard this data to avoid potentially polluting player stats
  ...
  const uint32 key =
      0x82DA9F4C; // this key should match the key in cs_gamestats.cpp
  ...
  for (int i = 0; i < iStatsToRead; ++i) {
    ...
    if (iStat >= CSSTAT_MAX) {
      Warning("PlayerStatsUpdate: invalid statId encountered; ignoring stats "
              "update\n");
      return;
    }
  ...
  for (int iStat = CSSTAT_FIRST; iStat < CSSTAT_MAX; ++iStat) {
    if (deltaStats[iStat] < 0 || deltaStats[iStat] >= 0x4000) {
      Warning("PlayerStatsUpdate message from server has out of band values; "
              "ignoring\n");
      return;
    }
  }

  // everything looks okay at this point; add these stats for the player's
  // round, match, and lifetime stats
...
void CCSClientGameStats::UpdateLastMatchStats() {
  ...
  // check to see if the player materially participate; they could have been
  // spectating or joined just in time for the ending.
  int s = 0;
  ...
  m_lifetimeStats[CSSTAT_LASTMATCH_CT_ROUNDS_WON] =
      m_matchStats[CSSTAT_CT_ROUNDS_WON];
  m_lifetimeStats[CSSTAT_LASTMATCH_ROUNDS_WON] =
      m_matchStats[CSSTAT_ROUNDS_WON];
  ...
  m_lifetimeStats[CSSTAT_LASTMATCH_MONEYSPENT] =
      m_matchStats[CSSTAT_MONEY_SPENT];
  m_lifetimeStats[CSSTAT_LASTMATCH_DOMINATIONS] =
      m_matchStats[CSSTAT_DOMINATIONS];
...
//-----------------------------------------------------------------------------
// Purpose: Calculate and store the match favorite weapon for each player as
// only deltaStats for that weapon are stored on Steam
//-----------------------------------------------------------------------------
void CCSClientGameStats::CalculateMatchFavoriteWeapons() {
  ...
  if (maxKillId == -1) {
    ...
    int statTableID = -1;
    for (int j = 0; WeaponName_StatId_Table[j].killStatId != CSSTAT_UNDEFINED;
         ++j) {
    ...
    m_lifetimeStats[CSSTAT_LASTMATCH_FAVWEAPON_ID] =
        WeaponName_StatId_Table[statTableID].weaponId;
    m_lifetimeStats[CSSTAT_LASTMATCH_FAVWEAPON_SHOTS] =
        m_matchStats[WeaponName_StatId_Table[statTableID].shotStatId];
    m_lifetimeStats[CSSTAT_LASTMATCH_FAVWEAPON_HITS] =
        m_matchStats[WeaponName_StatId_Table[statTableID].hitStatId];
