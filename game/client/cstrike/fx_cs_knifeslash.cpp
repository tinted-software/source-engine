...
void KnifeSlash(const CEffectData &data) {
  ...
  int decalNumber = decalsystem->GetDecalIndexForName(
      GetImpactDecal(pEntity, iMaterial, iDamageType));
  if (decalNumber == -1)
    return;
  ...
  if (decals && decals->GetInt()) {
    if ((pEntity->entindex() == 0) && (iHitbox != 0)) {
      ...
      effects->DecalShoot(decalNumber, pEntity->entindex(), pEntity->GetModel(),
                          pEntity->GetAbsOrigin(), pEntity->GetAbsAngles(),
                          vecOrigin, &vecPerp, 0);
    }
  }
