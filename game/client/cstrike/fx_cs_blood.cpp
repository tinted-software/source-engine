...
void InterpolateRamp(const CHitEffectRamp &a, const CHitEffectRamp &b,
                     CHitEffectRamp &out, int iDamage) {
...
void FX_HitEffectSmoke(CSmartPtr<CBloodSprayEmitter> pEmitter, int iDamage,
                       const Vector &vEntryPoint, const Vector &vDirection,
                       float flScale) {
  ...
  for (int i = 0; i < 2; i++) {
    SimpleParticle &newParticle =
        *pEmitter->AddSimpleParticle(hMaterial, vEntryPoint, 0, 0);

    newParticle.m_flLifetime = 0.0f;
    newParticle.m_flDieTime = 3.0f;

    newParticle.m_uchStartSize =
        random->RandomInt(interpolatedRamp.m_flMinSize,
    ...
    newParticle.m_uchStartAlpha = random->RandomInt(
        interpolatedRamp.m_flMinSize, interpolatedRamp.m_flMaxSize);
...
void FX_HitEffectBloodSpray(CSmartPtr<CBloodSprayEmitter> pEmitter, int iDamage,
                            const Vector &vEntryPoint,
                            const Vector &vSprayNormal,
                            const char *pMaterialName, float flLODDistance,
                            float flDistanceScale, float flScale,
                            float flSpeed) {
  ...
  for (int i = 0; i < 6; i++) {
    ...
    if (pParticle != NULL) {
      ...
      // All the particles are between red and white. The whiter the particle
      // is, the slower it goes.
      ...
      pParticle->m_vecVelocity +=
          vSprayNormal *
          random->RandomInt(interpolatedRamp.m_flMinVelocity,
                            interpolatedRamp.m_flMaxVelocity) *
          flSpeed * speedFactor;
      ...
      pParticle->m_uchStartSize =
          random->RandomFloat(interpolatedRamp.m_flMinSize,
                              interpolatedRamp.m_flMaxSize) *
          flDistanceScale;
      pParticle->m_uchEndSize = pParticle->m_uchStartSize * 4 * flDistanceScale;

      pParticle->m_uchStartAlpha = random->RandomInt(
          interpolatedRamp.m_flMinAlpha, interpolatedRamp.m_flMaxAlpha);
...
void FX_HitEffectBloodSplatter(CSmartPtr<CBloodSprayEmitter> pTrailEmitter,
                               int iDamage, const Vector &vExitPoint,
  ...
  for (int i = 0; i < 20; i++) {
    ...
    SimpleParticle *tParticle =
        (SimpleParticle *)pTrailEmitter->AddSimpleParticle(
    ...
    tParticle->m_vecVelocity =
        offDir * random->RandomFloat(interpolatedRamp.m_flMinVelocity,
                                     interpolatedRamp.m_flMaxVelocity);
...
//			scale -
//-----------------------------------------------------------------------------
void FX_CS_BloodSpray(const Vector &origin, const Vector &normal,
                      float flDamage) {
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (pParticle != NULL) {
      ...
      pParticle->m_vecVelocity[2] -=
          random->RandomFloat(16.0f, 32.0f) * (i + 1);
...
//			b -
//-----------------------------------------------------------------------------
void GetBloodColorForTeam(int iTeam, unsigned char &r, unsigned char &g,
                          unsigned char &b) {}
