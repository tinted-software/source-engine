...
private:
  ...
  CPanelAnimationVar(Color, m_clrTerroristText, "TerroristTextColor",
                     "TerroristTextColor");
...
bool CHudDeathNotice::ShouldDraw(void) {
  ...
  if (pPlayer->IsAlive() && pPlayer->m_flFlashBangTime >= gpGlobals->curtime) {
    float flAlpha = pPlayer->m_flFlashMaxAlpha *
                    (pPlayer->m_flFlashBangTime - gpGlobals->curtime) /
                    pPlayer->m_flFlashDuration;
...
void CHudDeathNotice::Paint() {
  ...
  int dominationDrawWidth = scheme()->GetProportionalScaledValueEx(
      GetScheme(), DOMINATION_DRAW_WIDTH);
  int dominationDrawHeight = scheme()->GetProportionalScaledValueEx(
      GetScheme(), DOMINATION_DRAW_HEIGHT);

  int iconHeadshotWide;
  int iconHeadshotTall;

  if (m_iconD_headshot->bRenderUsingFont) {
    iconHeadshotWide = surface()->GetCharacterWidth(
        m_iconD_headshot->hFont, m_iconD_headshot->cCharacterInFont);
  ...
  for (int i = 0; i < iCount; i++) {
    ...
    g_pVGuiLocalize->ConvertANSIToUnicode(m_DeathNotices[i].Victim.szClan,
                                          victimclan, sizeof(victimclan));
    g_pVGuiLocalize->ConvertANSIToUnicode(m_DeathNotices[i].Killer.szClan,
                                          killerclan, sizeof(killerclan));
    ...
    if (m_DeathNotices[i].iDominationImageId >= 0) {
      ...
      surface()->DrawTexturedRect(x, y, x + dominationDrawWidth,
                                  y + dominationDrawHeight);
      x += dominationDrawWidth;
    }
    ...
    if (m_DeathNotices[i].bHeadshot) {
      m_iconD_headshot->DrawSelf(x, y, iconHeadshotWide, iconHeadshotTall,
                                 iconColor);
      x += iconHeadshotWide;
    }
...
void CHudDeathNotice::FireGameEvent(IGameEvent *event) {
  ...
  deathMsg.Killer.color =
      iKiller > 0 ? m_teamColors[g_PR->GetTeam(iKiller)] : COLOR_WHITE;
  deathMsg.Victim.color =
      iVictim > 0 ? m_teamColors[g_PR->GetTeam(iVictim)] : COLOR_WHITE;
  Q_snprintf(deathMsg.Killer.szClan, sizeof(deathMsg.Killer.szClan), "%s ",
             killer_clan);
  Q_snprintf(deathMsg.Victim.szClan, sizeof(deathMsg.Victim.szClan), "%s ",
             victim_clan);
  ...
  if (event->GetInt("dominated") > 0 ||
      (pKiller != NULL && pKiller->IsPlayerDominated(iVictim))) {
  ...
  if (deathMsg.iSuicide) {
    if (!strcmp(fullkilledwith, "d_planted_c4")) {
      Q_snprintf(sDeathMsg, sizeof(sDeathMsg),
                 "%s was a bit too close to the c4.\n", deathMsg.Victim.szName);
