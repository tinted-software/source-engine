...
ConVar cl_buy_favorite_quiet(
    "cl_buy_favorite_quiet", "0", FCVAR_ARCHIVE | FCVAR_CLIENTCMD_CAN_EXECUTE,
    "Skips the prompt when saving a buy favorite in the buy menu");
ConVar cl_buy_favorite_nowarn(
    "cl_buy_favorite_nowarn", "0", FCVAR_ARCHIVE | FCVAR_CLIENTCMD_CAN_EXECUTE,
  ...
  if (TheBuyPresets->GetNumPresets()) {
    ...
    for (int i = 0; i < TheBuyPresets->GetNumPresets(); ++i) {
      ...
      if (preset && preset->GetName() && preset->GetName()[0]) {
        char buffer[64];
        g_pVGuiLocalize->ConvertUnicodeToANSI(preset->GetName(), buffer,
                                              sizeof(buffer));
        Msg(" %d. %s\n", i + 1, buffer);
      }
    }
 ...
 */
CON_COMMAND_F(cl_buy_favorite, "Purchase a favorite weapon/equipment loadout",
 ...
 */
CON_COMMAND_F(cl_buy_favorite_set, "Saves the current loadout as a favorite",
              FCVAR_CLIENTCMD_CAN_EXECUTE) {
  if (!engine->IsConnected())
    return;
...
ConCommand cl_buy_favorite_reset("cl_buy_favorite_reset",
                                 __CmdFunc_BuyPresetsReset,
 ...
 */
BuyPresetManager::BuyPresetManager() { m_loadedTeam = TEAM_UNASSIGNED; }

//--------------------------------------------------------------------------------------------------------------
/**
 *  Resets the BuyPresetManager, loading BuyPresets from disk.  If no presets
 * are defined, it loads the default presets instead.
 */
...
//--------------------------------------------------------------------------------------------------------------
/**
 *  Loads the default presets into the editing presets (e.g. when hitting the
 * "Use Defaults" button)
 */
