...
static bool CanBuyDefuser() {
  ...
  return (pPlayer && pPlayer->GetTeamNumber() == TEAM_CT &&
          CSGameRules()->IsBombDefuseMap());
}
...
void BuyPresetManager::GetCurrentLoadout(WeaponSet *weaponSet) {
  ...
  pWeapon =
      dynamic_cast<CWeaponCSBase *>(player->GetCSWeapon(WEAPON_SMOKEGRENADE));
  ...
  pWeapon =
      dynamic_cast<CWeaponCSBase *>(player->GetCSWeapon(WEAPON_HEGRENADE));
  ...
  pWeapon =
      dynamic_cast<CWeaponCSBase *>(player->GetCSWeapon(WEAPON_FLASHBANG));
  ...
  CSWeaponID primaryID =
      GetClientWeaponID(true); ///< The local player's current primary weapon
  ...
  // Grab current pistol
  CSWeaponID secondaryID =
      GetClientWeaponID(false); ///< The local player's current pistol
...
//--------------------------------------------------------------------------------------------------------------
/**
 *  Sets reasonable defaults for an empty weapon set: no primary/secondary
 * weapons, no equipment, and everything optional.
 */
...
//--------------------------------------------------------------------------------------------------------------
/**
 *  Constructs a WeaponSet containing everything that could be purchased right
 * now.  The cost is also returned, and is -1 if the set is not purchasable (as
 * opposed to completely purchased already, in which case cost is 0.)
 */
void WeaponSet::GetCurrent(int &cost, WeaponSet &ws) const {
  ...
  if (player->GetTeamNumber() != TEAM_CT &&
      player->GetTeamNumber() != TEAM_TERRORIST)
    return;
  ...
  //-------------------------------------------------------------------------
  // defuser
  if (m_defuser && player->GetTeamNumber() == TEAM_CT &&
      !player->HasDefuser() && CanBuyDefuser()) {
   ...
   *
   *  For each primary weapon in the list, do the following until one satisfies
   * the current money situation:
   *    1. buy primary weapon and its ammo (NEED TO WATCH FOR WEAPONS PLAYER
   * CANNOT BUY!!!)
   *    2. for each non-optional secondary weapon (if any),
   *         try to buy it and its ammo
   *    3. when a set can be purchased, add it to the total cost and stop
   * processing If we can't purchase the primary weapon, or a required secondary
   * weapon, the WeaponSet can't be bought, so we return -1.
   *
   *  To find if a weapon is owned, we can compare it's weaponID to either
   * primaryID or secondaryID from above.
   */
  ...
  for (i = 0; i < primaryWeapons.Count() && !doneBuyingWeapons; ++i) {
    ...
    if (primaryInfo) {
      ...
      int primaryWeaponCost =
          (weaponID != primaryID) ? primaryInfo->GetWeaponPrice() : 0;
      ...
      CURRENTCOST_DEBUG("\t%5.5d (ammo x %d)\n", primaryAmmoCost * primaryClips,
                        primaryClips);
      if (currentCash < currentCost) {
        currentCost =
            currentNonWeaponCost; // can't afford it, so try the next weapon
        continue;
      }

      // check for as_* maps, and CTs buying AK47s, etc
      if (!CanBuyWeapon(primaryID, secondaryID, weaponID) &&
          weaponID != primaryID) {
        currentCost =
            currentNonWeaponCost; // can't buy it, so try the next weapon
        continue;
      }
    ...
    for (j = 0; j < secondaryWeapons.Count(); ++j) {
      ...
      // add in ammo we're already buying for the primary weapon to the client's
      // actual ammo
      if (primaryInfo) {
        currentCost +=
            (weaponID != primaryID) ? primaryInfo->GetWeaponPrice() : 0;
      ...
      int secondaryClips =
          CalcClipsNeeded(secondaryWeapon, secondaryInfo, ammo);
      int secondaryWeaponCost =
          (weaponID != secondaryID) ? secondaryInfo->GetWeaponPrice() : 0;
      int secondaryAmmoCost = GetCSAmmoDef()->GetCost(secondaryInfo->iAmmoType);
      int secondaryAmmoBuySize =
          GetCSAmmoDef()->GetBuySize(secondaryInfo->iAmmoType);
      ...
      // We found both a primary and secondary weapon to buy.  Stop looking for
      // more.
  ...
  if (primaryWeaponToBuy) {
    ...
    // If we failed to buy a primary weapon, and one was in the list, bail - the
    // player can't afford this WeaponSet.
  ...
  if (secondaryWeaponToBuy) {
    ...
    // If we failed to buy a required secondary weapon, and one was in the list,
    // bail - the player can't afford this WeaponSet.
      ...
      if (primaryWeaponToBuy) {
        ...
        if (primaryWeaponToBuy->GetFillAmmo()) {
          ...
          if (info) {
            ...
            int clipsLeft =
                (ammoBuySize > 0) ? ceil(roundsLeft / (float)ammoBuySize) : 0;
            while (clipsLeft > 0 && cost + ammoCost <= currentCash) {
              ws.m_primaryWeapon.SetAmmoAmount(
                  ws.m_primaryWeapon.GetAmmoAmount() + 1);
              --clipsLeft;
        ...
        if (secondaryWeaponToBuy->GetFillAmmo()) {
          ...
          if (info) {
            ...
            int clipsLeft =
                (ammoBuySize > 0) ? ceil(roundsLeft / (float)ammoBuySize) : 0;
            while (clipsLeft > 0 && cost + ammoCost <= currentCash) {
              ws.m_secondaryWeapon.SetAmmoAmount(
                  ws.m_secondaryWeapon.GetAmmoAmount() + 1);
              --clipsLeft;
              ammo[info->iAmmoType] += MIN(maxRounds, ammoBuySize);
              cost += ammoCost;
            }
...
void WeaponSet::GetFromScratch(int &cost, WeaponSet &ws) const {
  ...
  if (secondaryInfo) {
    int secondaryClips =
        CalcClipsNeeded(&m_secondaryWeapon, secondaryInfo, ammo);
...
//--------------------------------------------------------------------------------------------------------------
/**
 *  Convenience function that wraps GetFromScratch() and discards the generated
 * WeaponSet.  Returns the cost to buy the full WeaponSet from scratch.
 */
 ...
 */
void WeaponSet::GenerateBuyCommands(
    char command[BUY_PRESET_COMMAND_LEN]) const {
  command[0] = 0;
  char *tmp = command;
...
//--------------------------------------------------------------------------------------------------------------
/**
 *  Return images out of a subdir, so when the buy preset system resizes the
 * images, they don't resize in the main buy menu.
 */
...
void BuyPreset::SetName(const wchar_t *name) {
  ...
  if (m_name[0] == 0) {
    const wchar_t *defaultName =
        g_pVGuiLocalize->Find("#Cstrike_BuyPresetBlank");
    if (defaultName) {
...
static void ParseWeaponString(const char *str, BuyPresetWeaponList &weapons,
                              bool isPrimary) {
  weapons.RemoveAll();
  ...
  while (remainder) {
    ...
    // sscanf is safe, since the size of the string being scanned is at least as
    // small as any individual destination buffer
...
void BuyPreset::Parse(KeyValues *data) {
  ...
  if (equipmentString) {
    ...
    while (remainder) {
      ...
      // sscanf is safe, since the size of the string being scanned is at least
      // as small as any individual destination buffer
...
static const char *ConstructWeaponString(const BuyPresetWeapon &weapon) {
  ...
  tmp = BufPrintf(tmp, remainder, "%s/%d%c",
                  WeaponIDToAlias(weapon.GetWeaponID()), weapon.GetAmmoAmount(),
                  (weapon.GetFillAmmo()) ? '+' : '=');
  return weaponString;
}
...
void BuyPreset::Save(KeyValues *data) {
   ...
   *      Individual items could be marked optional.  Cash reserve could be
   * specified.
   *  2.  3/2004
   *      Second-pass implementation.  Removed multiple weapons per fallback.
   * The pistol could be marked optional.  A preset could be marked as 'use for
   *      Autobuy'.
   *
   *  CZ RELEASE
   *  3.  4/22/2004
   *      The first released version.  Removed optional/required status.  Also
   *      removed the cash reserve and autobuy status.  Corresponds to
   * streamlined editing interface mocked up by Greg Coomer.
   *
   *  CS:S RELEASE
   *  4.  3/10/2004
  ...
  if (m_weaponList.Count() > 0) {
    ...
    presetKey->SetString("Secondary",
                         ConstructWeaponString(ws.m_secondaryWeapon));

    presetKey->SetString(
        "Equipment",
        SharedVarArgs(
            "vest%s/%d flash/%d sgren/%d hegren/%d defuser/%d nvgs/%d",
            (ws.m_helmet) ? "helm" : "", ws.m_armor, ws.m_flashbangs,
            ws.m_smokeGrenade, ws.m_HEGrenade, ws.m_defuser, ws.m_nightvision));
  }
}

//--------------------------------------------------------------------------------------------------------------
/**
 *  Calculates the full cost of the preset, which is exactly the full cost of
 * the first fallback WeaponSet
 */
