...
//
// Purpose: BuyPresetWeapon implementation, and misc knowledge relating to
// weapons
//
//=============================================================================
...
int CalcClipsNeeded(const BuyPresetWeapon *pWeapon, const CCSWeaponInfo *pInfo,
  ...
  if (buySize && pInfo->iAmmoType >= 0) {
    ...
    case AMMO_PERCENT: {
      int roundsNeeded =
          maxRounds * pWeapon->GetAmmoAmount() - ammo[pInfo->iAmmoType];
      roundsNeeded *= 0.01f;
      if (roundsNeeded > 0) {
        numClips = ceil(roundsNeeded / (float)buySize);
      } else {
...
bool CanBuyWeapon(CSWeaponID currentPrimaryID, CSWeaponID currentSecondaryID,
  ...
  /// @TODO: assasination maps have a specific set of weapons that can be used
  /// in them.
  if (info->m_iTeam != TEAM_UNASSIGNED &&
      pPlayer->GetTeamNumber() != info->m_iTeam)
    return false;
...
CWeaponCSBase *GetWeaponInSlot(int iSlot, int iSlotPos) {
  ...
  for (int i = 0; i < MAX_WEAPONS; i++) {
    CWeaponCSBase *pWeapon =
        dynamic_cast<CWeaponCSBase *>(player->GetWeapon(i));
...
//--------------------------------------------------------------------------------------------------------------
/**
 *  Returns the client's WEAPON_* value for the currently owned weapon, or
 * WEAPON_NONE if no weapon is owned
 */
