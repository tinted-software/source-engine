...
ConVar cl_left_hand_ik("cl_left_hand_ik", "0", 0,
                       "Attach player's left hand to rifle with IK.");

ConVar cl_ragdoll_physics_enable("cl_ragdoll_physics_enable", "1", 0,
                                 "Enable/disable ragdoll physics.");

ConVar cl_minmodels("cl_minmodels", "0", 0,
                    "Uses one player model for each team.");
ConVar cl_min_ct("cl_min_ct", "1", 0,
                 "Controls which CT model is used when cl_minmodels is set.",
                 true, 1, true, 4);
ConVar
    cl_min_t("cl_min_t", "1", 0,
             "Controls which Terrorist model is used when cl_minmodels is set.",
             true, 1, true, 4);
const float CycleLatchTolerance = 0.15; // amount we can diverge from the
                                        // server's cycle before we're corrected
...
public:
  ...
  const char
      *m_pWeaponClassName;  // The addon uses the w_ model from this weapon.
  const char *m_pModelName; // If this is present, will use this model instead
                            // of looking up the weapon
...
CAddonInfo g_AddonInfo[] = {
    ...
    {"primary", 0, 0,
     0}, // Primary addon model is looked up based on m_iPrimaryAddon
    {"pistol", 0, 0,
     0}, // Pistol addon model is looked up based on m_iSecondaryAddon
    {"eholster", 0, "models/weapons/w_eq_eholster_elite.mdl",
...
RecvPropEHandle(RECVINFO(m_hPlayer)), RecvPropInt(RECVINFO(m_iEvent)),
    RecvPropInt(RECVINFO(m_nData)) END_RECV_TABLE()
        ...
        BEGIN_PREDICTION_DATA(C_CSPlayer)
            ...
            DEFINE_PRED_FIELD(m_bShieldDrawn, FIELD_BOOLEAN,
                              FTYPEDESC_INSENDTABLE),
#endif
    DEFINE_PRED_FIELD_TOL(m_flStamina, FIELD_FLOAT, FTYPEDESC_INSENDTABLE,
                          0.1f),
...
public:
  ...
  void GetRagdollInitBoneArrays(matrix3x4_t *pDeltaBones0,
...
RecvPropVector(RECVINFO_NAME(m_vecNetworkOrigin, m_vecOrigin)),
    ...
    RecvPropInt(RECVINFO(m_iDeathPose)), RecvPropInt(RECVINFO(m_iDeathFrame)),
...
C_CSRagdoll::~C_CSRagdoll() { PhysCleanupFrictionSounds(this); }

void C_CSRagdoll::GetRagdollInitBoneArrays(matrix3x4_t *pDeltaBones0,
                                           matrix3x4_t *pDeltaBones1,
                                           matrix3x4_t *pCurrentBones,
                                           float boneDt) {
  // otherwise use the death pose to set up the ragdoll
  ForceSetupBonesAtTime(pDeltaBones0, gpGlobals->curtime - boneDt);
  GetRagdollCurSequenceWithDeathPose(this, pDeltaBones1, gpGlobals->curtime,
                                     m_iDeathPose, m_iDeathFrame);
...
void C_CSRagdoll::ImpactTrace(trace_t *pTrace, int iDamageType,
                              const char *pCustomImpactName) {
...
void C_CSRagdoll::CreateCSRagdoll() {
  ...
  // mark this to prevent model changes from overwriting the death sequence with
  // the server sequence
  SetReceivedSequence();
  ...
  if (pPlayer && !pPlayer->IsDormant()) {
    ...
    if (bRemotePlayer) {
      ...
      // go ahead and set these on the player in case the code below decides to
      // set up bones using that entity instead of this one.  The local player
      // may not have valid animation
  ...
  if (cl_ragdoll_physics_enable.GetInt()) {
    ...
    // death pose animations in C_CSRagdoll::GetRagdollInitBoneArrays. The
    // version in C_BasePlayer::GetRagdollInitBoneArrays doesn't attempt to
    // blend death poses, so if the player is relevant, use that one regardless
    // of whether the player is the local one or not.
    //=============================================================================
    if (pPlayer && !pPlayer->IsDormant()) {
    ...
    DestroyShadow();
    ClientLeafSystem()->SetRenderGroup(GetRenderHandle(),
...
void C_CSRagdoll::OnDataChanged(DataUpdateType_t type) {
  ...
  if (type == DATA_UPDATE_CREATED) {
    // Prevent replays from creating ragdolls on the first frame of playback
    // after skipping through playback. If a player died (leaving a ragdoll)
    // previous to the first frame of replay playback, their ragdoll wasn't yet
    // initialized because OnDataChanged events are queued but not processed
...
IRagdoll *C_CSRagdoll::GetIRagdoll() const { return m_pRagdoll; }
...
//			*pOut -
//-----------------------------------------------------------------------------
void RecvProxy_NightVision(const CRecvProxyData *pData, void *pStruct,
                           void *pOut) {
...
void RecvProxy_FlashTime(const CRecvProxyData *pData, void *pStruct,
                         void *pOut) {
  C_CSPlayer *pPlayerData = (C_CSPlayer *)pStruct;
  ...
  if ((pPlayerData->m_flFlashDuration != pData->m_Value.m_Float) &&
      pData->m_Value.m_Float > 0) {
    pPlayerData->m_flFlashAlpha = 1;
  }

  pPlayerData->m_flFlashDuration = pData->m_Value.m_Float;
  pPlayerData->m_flFlashBangTime =
      gpGlobals->curtime + pPlayerData->m_flFlashDuration;
}

void RecvProxy_HasDefuser(const CRecvProxyData *pData, void *pStruct,
                          void *pOut) {
  C_CSPlayer *pPlayerData = (C_CSPlayer *)pStruct;
...
void C_CSPlayer::RecvProxy_CycleLatch(const CRecvProxyData *pData,
                                      void *pStruct, void *pOut) {
  // This receive proxy looks to see if the server's value is close enough to
  // what we think it should be.  We've been running the same code; this is an
  // error correction for changes we didn't simulate while they were out of PVS.
  C_CSPlayer *pPlayer = (C_CSPlayer *)pStruct;
  if (pPlayer->IsLocalPlayer())
    return; // Don't need to fixup ourselves.

  float incomingCycle =
      (float)(pData->m_Value.m_Int) / 16; // Came in as 4 bit fixed point
  ...
  if (!closeEnough) {
    ...
    if (pPlayer && pPlayer->GetTeam()) {
      DevMsg(2, "%s %s(%d): Cycle latch wants to correct %.2f in to %.2f.\n",
             pPlayer->GetTeam()->Get_Name(), pPlayer->GetPlayerName(),
             pPlayer->entindex(), currentCycle, incomingCycle);
    }
...
void __MsgFunc_ReloadEffect(bf_read &msg) {
  ...
  C_CSPlayer *pPlayer =
      dynamic_cast<C_CSPlayer *>(C_BaseEntity::Instance(iPlayer));
  if (pPlayer)
...
RecvPropFloat(RECVINFO(m_flStamina)), RecvPropInt(RECVINFO(m_iDirection)),
    ...
    END_RECV_TABLE()
        ...
        IMPLEMENT_CLIENTCLASS_DT(C_CSPlayer, DT_CSPlayer, CCSPlayer)
            RecvPropDataTable("cslocaldata", 0, 0,
                              &REFERENCE_RECV_TABLE(DT_CSLocalPlayerExclusive)),
    RecvPropDataTable("csnonlocaldata", 0, 0,
                      &REFERENCE_RECV_TABLE(DT_CSNonLocalPlayerExclusive)),
    ...
    RecvPropInt(RECVINFO(m_iPlayerState)), RecvPropInt(RECVINFO(m_iAccount)),
    RecvPropInt(RECVINFO(m_bInBombZone)), RecvPropInt(RECVINFO(m_bInBuyZone)),
    RecvPropInt(RECVINFO(m_iClass)), RecvPropInt(RECVINFO(m_ArmorValue)),
    ...
    RecvPropInt(RECVINFO(m_ArmorValue)), RecvPropBool(RECVINFO(m_bIsDefusing)),
    RecvPropBool(RECVINFO(m_bResumeZoom)), RecvPropInt(RECVINFO(m_iLastZoom)),
...
bool C_CSPlayer::HasDefuser() const { return m_bHasDefuser; }

void C_CSPlayer::GiveDefuser() { m_bHasDefuser = true; }

void C_CSPlayer::RemoveDefuser() { m_bHasDefuser = false; }

bool C_CSPlayer::HasNightVision() const { return m_bHasNightVision; }
...
CSPlayerState C_CSPlayer::State_Get() const { return m_iPlayerState; }
...
int C_CSPlayer::GetAccount() const { return m_iAccount; }

int C_CSPlayer::PlayerClass() const { return m_iClass; }

bool C_CSPlayer::IsInBuyZone() { return m_bInBuyZone; }

bool C_CSPlayer::CanShowTeamMenu() const { return true; }

int C_CSPlayer::ArmorValue() const { return m_ArmorValue; }

bool C_CSPlayer::HasHelmet() const { return m_bHasHelmet; }
...
void C_CSPlayer::GetShadowRenderBounds(Vector &mins, Vector &maxs,
                                       ShadowType_t shadowType) {
  if (shadowType == SHADOWS_SIMPLE) {
    // Don't let the render bounds change when we're using blobby shadows, or
    // else the shadow will pop and stretch.
    mins = CollisionProp()->OBBMins();
    maxs = CollisionProp()->OBBMaxs();
  } else {
    GetRenderBounds(mins, maxs);

    // We do this because the normal bbox calculations don't take pose params
    // into account, and the rotation of the guy's upper torso can place his gun
    // a ways out of his bbox, and the shadow will get cut off as he rotates.
    //
...
void C_CSPlayer::GetRenderBounds(Vector &theMins, Vector &theMaxs) {
  // TODO POSTSHIP - this hack/fix goes hand-in-hand with a fix in
  // CalcSequenceBoundingBoxes in utils/studiomdl/simplify.cpp. When we enable
  // the fix in CalcSequenceBoundingBoxes, we can get rid of this.
  //
  // What we're doing right here is making sure it only uses the bbox for our
  // lower-body sequences since, with the current animations and the bug in
  // CalcSequenceBoundingBoxes, are WAY bigger than they need to be.
  C_BaseAnimating::GetRenderBounds(theMins, theMaxs);

  // If we're ducking, we should reduce the render height by the difference in
  // standing and ducking heights. This prevents shadows from drawing above
  // ducking players etc.
...
bool C_CSPlayer::GetShadowCastDirection(Vector *pDirection,
...
void InitializeAddonModelFromWeapon(CWeaponCSBase *weapon,
                                    C_BreakableProp *addon) {
  if (!weapon) {
    return;
  }
  ...
  if (weaponInfo.m_szAddonModel[0] == 0) {
    addon->InitializeAsClientEntity(weaponInfo.szWorldModel,
                                    RENDER_GROUP_OPAQUE_ENTITY);
  } else {
    addon->InitializeAsClientEntity(weaponInfo.m_szAddonModel,
                                    RENDER_GROUP_OPAQUE_ENTITY);
  }
}

void C_CSPlayer::CreateAddonModel(int i) {
  COMPILE_TIME_ASSERT((sizeof(g_AddonInfo) / sizeof(g_AddonInfo[0])) ==
                      NUM_ADDON_BITS);
  ...
  if (addonType == ADDON_PISTOL || addonType == ADDON_PRIMARY) {
    CCSWeaponInfo *weaponInfo = GetWeaponInfo(
        (CSWeaponID)((addonType == ADDON_PRIMARY) ? m_iPrimaryAddon.Get()
                                                  : m_iSecondaryAddon.Get()));
    if (!weaponInfo) {
    ...
    if (weaponInfo->m_szAddonModel[0] == 0) {
      pEnt->InitializeAsClientEntity(weaponInfo->szWorldModel,
                                     RENDER_GROUP_OPAQUE_ENTITY);
    } else {
      pEnt->InitializeAsClientEntity(weaponInfo->m_szAddonModel,
                                     RENDER_GROUP_OPAQUE_ENTITY);
    }
  } else if (pAddonInfo->m_pModelName) {
    if (addonType == ADDON_PISTOL2 && !(m_iAddonBits & ADDON_PISTOL)) {
      pEnt->InitializeAsClientEntity(pAddonInfo->m_pHolsterName,
                                     RENDER_GROUP_OPAQUE_ENTITY);
    } else {
      pEnt->InitializeAsClientEntity(pAddonInfo->m_pModelName,
                                     RENDER_GROUP_OPAQUE_ENTITY);
    }
  } else {
    WEAPON_FILE_INFO_HANDLE hWpnInfo =
        LookupWeaponInfoSlot(pAddonInfo->m_pWeaponClassName);
    ...
    CCSWeaponInfo *pWeaponInfo =
        dynamic_cast<CCSWeaponInfo *>(GetFileWeaponInfoFromHandle(hWpnInfo));
    if (pWeaponInfo) {
      if (pWeaponInfo->m_szAddonModel[0] == 0)
        pEnt->InitializeAsClientEntity(pWeaponInfo->szWorldModel,
                                       RENDER_GROUP_OPAQUE_ENTITY);
      else
        pEnt->InitializeAsClientEntity(pWeaponInfo->m_szAddonModel,
                                       RENDER_GROUP_OPAQUE_ENTITY);
    } else {
      pEnt->Release();
      Warning(
...
void C_CSPlayer::UpdateAddonModels() {
  ...
  // If the local player is observing this entity in first-person mode, get rid
  // of its addons.
  ...
  if (m_iSecondaryAddon == WEAPON_ELITE &&
      ((m_iLastAddonBits ^ iCurAddonBits) & ADDON_PISTOL) != 0) {
  ...
  for (i = m_AddonModels.Head(); i != m_AddonModels.InvalidIndex(); i = iNext) {
    ...
    if (!(iCurAddonBits & addonBit) ||
        (rebuildPistol2Addon && addonBit == ADDON_PISTOL2)) {
...
void C_CSPlayer::UpdateSoundEvents() {
  int iNext;
  for (int i = m_SoundEvents.Head(); i != m_SoundEvents.InvalidIndex();
       i = iNext) {
...
void C_CSPlayer::UpdateMinModels(void) {
  ...
  if (!IsVIP() && sv_allowminmodels.GetBool() && cl_minmodels.GetBool() &&
      !IsLocalPlayer()) {
    if (GetTeamNumber() == TEAM_CT) {
      int index = cl_min_ct.GetInt() - 1;
      if (index >= 0 && index < CTPlayerModels.Count()) {
...
void C_CSPlayer::ClientThink() {
  ...
  if (IsLocalPlayer()) {
    ...
    if (m_iObserverMode == OBS_MODE_FREEZECAM) {
      ...
      if (!m_bPlayingFreezeCamSound && !cl_disablefreezecam.GetBool() &&
          !sv_disablefreezecam.GetBool())
      //=============================================================================
      // HPE_END
      //=============================================================================
      {
...
void C_CSPlayer::ValidateModelIndex(void) { UpdateMinModels(); }

void C_CSPlayer::PostDataUpdate(DataUpdateType_t updateType) {
  // C_BaseEntity assumes we're networking the entity's angles, so pretend that
  // it networked the same value we already have.
...
int C_CSPlayer::GetMaxHealth() const { return 100; }
...
void C_CSPlayer::UpdateIDTarget() {
  ...
  // if it's more than 75 then we can't see what's going on so we don't display
  // the id.
  byte color[4];
  bool blend;
  ...
  if (color[3] > MAX_FLASHBANG_OPACITY &&
      (IsAlive() || GetObserverMode() == OBS_MODE_IN_EYE))
    return;
  ...
  UTIL_TraceLine(vecStart, vecEnd, MASK_VISIBLE_AND_NPCS,
                 GetLocalOrInEyeCSPlayer(), COLLISION_GROUP_NONE, &tr);
  if (!tr.startsolid && !tr.DidHitNonWorldEntity()) {
    CTraceFilterSimple filter(GetLocalOrInEyeCSPlayer(), COLLISION_GROUP_NONE);

    // Check for player hitboxes extending outside their collision bounds
    const float rayExtension = 40.0f;
    UTIL_ClipTraceToPlayers(vecStart, vecEnd + MainViewForward() * rayExtension,
  ...
  if (!tr.startsolid && tr.DidHitNonWorldEntity()) {
    ...
    if (pEntity && (pEntity != this)) {
      ...
      // We check this AFTER we found a player, just so we don't go thru this
      // for nothing.
      for (int i = 0; i < m_SmokeGrenades.Count(); i++) {
        C_BaseParticleEntity *pSmokeGrenade =
            (C_BaseParticleEntity *)m_SmokeGrenades.Element(i);

        if (pSmokeGrenade) {
          float flHit1, flHit2;

          float flRadius =
              (SMOKEGRENADE_PARTICLERADIUS * NUM_PARTICLES_PER_DIMENSION + 1) *
              0.5f;

          Vector vPos = pSmokeGrenade->GetAbsOrigin();

          /*debugoverlay->AddBoxOverlay( pSmokeGrenade->GetAbsOrigin(), Vector(
           flRadius, flRadius, flRadius ), Vector( -flRadius, -flRadius,
           -flRadius ), QAngle( 0, 0, 0 ), 255, 0, 0, 255, 0.2 );*/

          if (IntersectInfiniteRayWithSphere(MainViewOrigin(),
                                             MainViewForward(), vPos, flRadius,
                                             &flHit1, &flHit2)) {
            return;
          }
        }
      ...
      if (!GetIDTarget() &&
          (!m_iOldIDEntIndex || m_holdTargetIDTimer.IsElapsed())) {
...
//-----------------------------------------------------------------------------
bool C_CSPlayer::IsInHostageRescueZone() { return m_bInHostageRescueZone; }
...
void C_CSPlayer::SetFireAnimation( PLAYER_ANIM playerAnim )
{
        ...
        // Now figure out what to do based on the current state and the new
state. switch ( playerAnim )
        {
        default:
        case PLAYER_RELOAD:
        case PLAYER_ATTACK1:
        ...
        case PLAYER_WALK:
                ...
                if ( !isStillJumping )
                {
                        ...
                        if ( isDucked )
                        {
                                idealActivity = !isMoving ? ACT_CROUCHIDLE :
ACT_RUN_CROUCH;
                        }
                        else
                        {
                                ...
                                {
                                        idealActivity = isMoving ? ACT_WALK :
ACT_IDLE;
                                }
                        }

                        // Allow body yaw to override for standing and turning
in place idealActivity = m_PlayerAnimState.BodyYawTranslateActivity(
idealActivity );
                }
                break;
        ...
        if ( pWeapon )
        {
                ...
                if ( playerAnim == PLAYER_ATTACK1 )
                {
                        switch ( idealActivity )
                        {
                                ...
                                case ACT_CROUCHIDLE:
                                        aWeaponActivity =
ACT_PLAYER_CROUCH_FIRE; break; case ACT_RUN_CROUCH: aWeaponActivity =
ACT_PLAYER_CROUCH_WALK_FIRE; break;
                        }
                }

                m_PlayerAnimState.SetWeaponLayerSequence(
pWeapon->GetCSWpnData().m_szAnimExtension, aWeaponActivity );
        }
}
*/
...
void C_CSPlayer::UpdateClientSideAnimation() {
  ...
  // We need our cycle to be valid for when we call the playeranimstate update
  // code, or else it'll synchronize the upper body anims with the wrong cycle.
  if (GetSequence() != -1) {
    // move frame forward
    FrameAdvance(
        0.0f); // 0 means to use the time we last advanced instead of a constant
  }

  // Update the animation data. It does the local check here so this works when
  // using a third-person camera (and we don't have valid player angles).
...
void C_CSPlayer::ProcessMuzzleFlashEvent() {
  ...
  if (hasMuzzleFlash) {
    ...
    if (iAttachment > 0) {
      ...
      flScale *= 0.75;
      FX_MuzzleEffectAttached(flScale, pWeapon->GetRefEHandle(), iAttachment,
                              NULL, false);
    }
  }
}
...
bool FindMyAttachmentBone(C_BaseAnimating *pModel, int &iBone,
                          CStudioHdr *pHdr) {
  if (!pHdr)
    return false;
...
void ApplyDifferenceTransformToChildren(C_BaseAnimating *pModel,
...
void GetCorrectionMatrices(const matrix3x4_t &mShoulder,
                           const matrix3x4_t &mElbow, const matrix3x4_t &mHand,
...
void C_CSPlayer::BuildTransformations(CStudioHdr *pHdr, Vector *pos,
                                      Quaternion q[],
                                      const matrix3x4_t &cameraTransform,
                                      int boneMask,
                                      CBoneBitList &boneComputed) {
  // First, setup our model's transformations like normal.
  BaseClass::BuildTransformations(pHdr, pos, q, cameraTransform, boneMask,
                                  boneComputed);
  ...
  // If our current weapon has a bone named L_Hand_Attach, then we attach the
  // player's left hand (Valvebiped.Bip01_L_Hand) to it.
  ...
  if (FindWeaponAttachmentBone(pWeapon, iWeaponBone)) {
    ...
    if (FindMyAttachmentBone(this, iMyBone, pHdr)) {
      ...
      // Figure out the rotation offset from the current shoulder and elbow bone
      // rotations and what the IK code's alignment code is going to produce,
      // because we'll have to re-apply that offset after the IK runs.
      matrix3x4_t mShoulderCorrection, mElbowCorrection;
      GetCorrectionMatrices(pBones[iShoulder], pBones[iElbow], pBones[iHand],
                            mShoulderCorrection, mElbowCorrection);
...
C_BaseAnimating *C_CSPlayer::BecomeRagdollOnClient() { return NULL; }
...
void C_CSPlayer::PlayReloadEffect() {
  ...
  // The weapon needs two models, world and view, but can only cache one.
  // Synthesize the other.
  const CCSWeaponInfo &info = pWeapon->GetCSWpnData();
  const model_t *pModel =
      modelinfo->GetModel(modelinfo->GetModelIndex(info.szViewModel));
  if (!pModel)
    return;
  ...
  for (int iSeq = 0; iSeq < studioHdr.GetNumSeq(); iSeq++) {
    ...
    if (pSeq->activity == ACT_VM_RELOAD) {
      ...
      float cyclesPerSecond =
          Studio_CPS(&studioHdr, *pSeq, iSeq, poseParameters);
      ...
      for (int iEvent = 0; iEvent < pSeq->numevents; iEvent++) {
        ...
        if (pEvent->event == CL_EVENT_SOUND) {
          ...
          event.m_flEventTime =
              gpGlobals->curtime + pEvent->cycle / cyclesPerSecond;
...
void C_CSPlayer::DoAnimationEvent(PlayerAnimEvent_t event, int nData) {
  if (event == PLAYERANIMEVENT_THROW_GRENADE) {
    ...
    // and the client will pick up the event in CCSPlayerAnimState.
  } else {
    m_PlayerAnimState->DoAnimationEvent(event, nData);
  }
}

void C_CSPlayer::FireEvent(const Vector &origin, const QAngle &angles,
  ...
  if (event == 7001) {
    ...
    if (bInWater) {
      ...
      UTIL_TraceLine(origin, origin + vecTrace, MASK_WATER, NULL,
...
void C_CSPlayer::SetActivity(Activity eActivity) { m_Activity = eActivity; }

Activity C_CSPlayer::GetActivity() const { return m_Activity; }
...
void C_CSPlayer::Simulate(void) {
  if (this != C_BasePlayer::GetLocalPlayer()) {
    if (IsEffectActive(EF_DIMLIGHT)) {
      ...
      trace_t tr;
      UTIL_TraceLine(vecOrigin, vecOrigin + (vForward * 200), MASK_SHOT, this,
                     COLLISION_GROUP_NONE, &tr);
      ...
      if (!m_pFlashlightBeam) {
        ...
        beamInfo.m_nFlags =
            FBEAM_FOREVER | FBEAM_ONLYNOISEONCE | FBEAM_NOTILE | FBEAM_HALOBEAM;
...
void C_CSPlayer::ImpactTrace(trace_t *pTrace, int iDamageType,
                             const char *pCustomImpactName) {
...
//-----------------------------------------------------------------------------
void C_CSPlayer::CalcObserverView(Vector &eyeOrigin, QAngle &eyeAngles,
                                  float &fov) {
  /**
   * TODO: Fix this!
  // CS:S standing eyeheight is above the collision volume, so we need to pull
  it
  // down when we go into close quarters.
  float maxEyeHeightAboveBounds = VEC_VIEW_SCALED( this ).z -
  VEC_HULL_MAX_SCALED( this ).z; if ( GetObserverMode() == OBS_MODE_IN_EYE &&
          maxEyeHeightAboveBounds > 0.0f &&
          GetObserverTarget() &&
          GetObserverTarget()->IsPlayer() )
  {
          const float eyeClearance = 12.0f; // eye pos must be this far below
  the ceiling
          ...
          start.z += vHullMax.z;
          Vector end = start;
          end.z += eyeClearance + VEC_VIEW_SCALED( this ).z - vHullMax_SCALED(
  this ).z;
  ...
  COLLISION_GROUP_PLAYER_MOVEMENT, &trace );
          ...
          {
                  float est = start.z + trace.fraction * (end.z - start.z) -
  GetAbsOrigin().z - eyeClearance; if ( ( target->GetFlags() & FL_DUCKING ) == 0
  && !target->GetFallVelocity() && !target->IsDucked() )
                  {
                          offset.z = est;
                  }
                  else
...
// HPE_BEGIN:
//=============================================================================
// [tj] checks if this player has another given player on their Steam friends
// list.
bool C_CSPlayer::HasPlayerAsFriend(C_CSPlayer *player) {
  if (!steamapicontext || !steamapicontext->SteamFriends() ||
      !steamapicontext->SteamUtils() || !player) {
    return false;
  }
  ...
  return steamapicontext->SteamFriends()->HasFriend(steamID,
                                                    k_EFriendFlagImmediate);
}

// [menglish] Returns whether this player is dominating or is being dominated by
...
// observer mode
//-----------------------------------------------------------------------------
void C_CSPlayer::CalcFreezeCamView(Vector &eyeOrigin, QAngle &eyeAngles,
                                   float &fov) {
  ...
  //=============================================================================
  static ConVarRef sv_disablefreezecam("sv_disablefreezecam");
  if (!pTarget || cl_disablefreezecam.GetBool() ||
      sv_disablefreezecam.GetBool())
  //=============================================================================
  // HPE_END
  //=============================================================================
  ...
  Vector vLookAt = pTarget->GetObserverCamOrigin(); // Returns ragdoll origin if
                                                    // they're ragdolled
  vLookAt += GetChaseCamViewOffset(pTarget);

  // look over ragdoll, not through
  if (!pTarget->IsAlive())
    vLookAt.z += pTarget->GetBaseAnimating()
  ...
  // goal position of camera is pulled away from target by
  ...
  UTIL_TraceHull(vLookAt, vTargetPos, WALL_MIN, WALL_MAX, MASK_SOLID, pTarget,
  ...
  float fInterpolant =
      clamp(fCurTime / spec_freeze_traveltime.GetFloat(), 0.0f, 1.0f);
...
float C_CSPlayer::GetDeathCamInterpolationTime() {
  static ConVarRef sv_disablefreezecam("sv_disablefreezecam");
  if (cl_disablefreezecam.GetBool() || sv_disablefreezecam.GetBool() ||
      !GetObserverTarget())
