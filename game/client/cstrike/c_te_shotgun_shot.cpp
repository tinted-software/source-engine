...
void C_TEFireBullets::PostDataUpdate(DataUpdateType_t updateType) {
  ...
  FX_FireBullets(m_iPlayer + 1, m_vecOrigin, m_vecAngles, m_iWeaponID, m_iMode,
                 m_iSeed, m_fInaccuracy, m_fSpread);
}
...
RecvPropVector(RECVINFO(m_vecOrigin)), RecvPropFloat(RECVINFO(m_vecAngles[0])),
    RecvPropFloat(RECVINFO(m_vecAngles[1])), RecvPropInt(RECVINFO(m_iWeaponID)),
    RecvPropInt(RECVINFO(m_iMode)), RecvPropInt(RECVINFO(m_iSeed)),
    RecvPropInt(RECVINFO(m_iPlayer)), RecvPropFloat(RECVINFO(m_fInaccuracy)),
...
RecvPropVector(RECVINFO(m_vecOrigin)), RecvPropInt(RECVINFO(m_iPlayer)),
    RecvPropInt(RECVINFO(m_option)), END_RECV_TABLE()
