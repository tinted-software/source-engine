...
void CS_MuzzleFlashCallback(const CEffectData &data) {
  CSmartPtr<CLocalSpaceEmitter> pEmitter = CLocalSpaceEmitter::Create(
      "CS_MuzzleFlash", data.m_hEntity, data.m_nAttachmentIndex, 0);

  if (!pEmitter)
    return;

  // SetBBox() manually on the particle system so it doesn't have to be
  // recalculated more than once.
  ...
  Assert(pEmitter);
  pEmitter->GetBinding().SetBBox(vCenter - Vector(3, 3, 3),
                                 vCenter + Vector(3, 3, 3));

  // haxors - make the clip much shorter so the alpha is not
  // changed based on large clip distances
  pEmitter->SetNearClip(0, 5);

  PMaterialHandle hFlashMaterial =
      pEmitter->GetPMaterial("sprites/muzzleflash4");

  for (int i = 0; i < 3; i++) {
    SimpleParticle *pParticle = (SimpleParticle *)pEmitter->AddParticle(
        sizeof(SimpleParticle), hFlashMaterial, vec3_origin);
    if (pParticle) {
  ...
  TE_DynamicLight(filter, 0.0, &(pEmitter->GetSortOrigin()), 255, 192, 64, 5,
                  70, 0.05, 768);
}
...
void CS_MuzzleFlashXCallback(const CEffectData &data) {
  CSmartPtr<CLocalSpaceEmitter> pEmitter = CLocalSpaceEmitter::Create(
      "CS_MuzzleFlashX", data.m_hEntity, data.m_nAttachmentIndex, 0);
  ...
  PMaterialHandle hFlashMaterial =
      pEmitter->GetPMaterial("effects/muzzleflashX");
  ...
  if (pParticle) {
    ...
    pParticle->m_uchStartSize =
        6.0f * data.m_flScale * random->RandomFloat(0.9, 1.1);
