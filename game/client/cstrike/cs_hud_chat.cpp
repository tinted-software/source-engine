...
CHudChat::CHudChat(const char *pElementName) : BaseClass(pElementName) {
  ...
  //
  // [pmf] Removed from render group so that chat still works during
  // intermission (when the scoreboard is forced to be up). The downside is that
  // chat shows over the scoreboard during regular play, but this might be less
  // of an issue if we reduce the need to display it constantly by adding HUD
  // support for live player counts.
  //=============================================================================
...
void CHudChat::MsgFunc_RawAudio(bf_read &msg) {
  ...
  if (feedbackDuration > 0.0f) {
    ...
    C_CSPlayer *pPlayer =
        static_cast<C_CSPlayer *>(cl_entitylist->GetEnt(playerIndex));
