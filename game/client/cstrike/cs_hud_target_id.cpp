...
static ConVar hud_showtargetpos(
    "hud_showtargetpos", "0", FCVAR_ARCHIVE,
...
void CTargetID::VidInit() {
  ...
  SetSize(g_pClientMode->GetViewport()->GetWide(),
          g_pClientMode->GetViewport()->GetTall());
    ...
    if (IsPlayerIndex(iEntIndex)) {
      if (!pPlayer) {
        ...
        if (cs_PR && Q_strlen(cs_PR->GetClanTag(iEntIndex)) > 1) {
          Q_snprintf(szClan, sizeof(szClan), "%s ",
                     cs_PR->GetClanTag(iEntIndex));
        } else {
          szClan[0] = 0;
        }
        g_pVGuiLocalize->ConvertANSIToUnicode(szClan, wszClanTag,
                                              sizeof(wszClanTag));
        ...
        if (pPlayer->InSameTeam(pLocalPlayer)) {
          ...
          bShowHealth = true;
                   ...
                   pLocalPlayer->GetTeamNumber() != TEAM_TERRORIST) {
        ...
        if (bShowHealth) {
          _snwprintf(
              wszHealthText, ARRAYSIZE(wszHealthText) - 1, L"%.0f%%",
      ...
      else
      {
                      ...
                      m_flDisplayHistory |= DHF_HOSTAGE_SEEN_FAR;
      // Don't want the other msg to appear now HintMessage(
      "#Hint_press_use_so_hostage_will_follow", FALSE );
      }
      */
      ...
      if (pHostage != NULL) {
        ...
        _snwprintf(
            wszHealthText, ARRAYSIZE(wszHealthText) - 1, L"%.0f%%",
            ((float)pHostage->GetHealth() / (float)pHostage->GetMaxHealth()) *
                100);
        wszHealthText[ARRAYSIZE(wszHealthText) - 1] = '\0';
        bShowHealth = true;
        ...
        c = m_cHostageColor;
        g_pVGuiLocalize->ConvertANSIToUnicode(pEnt->GetIDString(), sIDString,
                                              sizeof(sIDString));
    ...
    if (printFormatString) {
      if (bShowPlayerName && bShowHealth) {
        g_pVGuiLocalize->ConstructString(
            ...
            g_pVGuiLocalize->Find(printFormatString), 3, wszClanTag,
            wszPlayerName, wszHealthText);
      } else if (bShowPlayerName) {
        g_pVGuiLocalize->ConstructString(
            sIDString, sizeof(sIDString),
            g_pVGuiLocalize->Find(printFormatString), 2, wszClanTag,
            wszPlayerName);
      } else if (bShowHealth) {
        g_pVGuiLocalize->ConstructString(
            sIDString, sizeof(sIDString),
