...
void CHudWeaponSelection::Paint() {
  ...
  int width =
      (m_iMaxSlots - 1) * (m_flSmallBoxSize + m_flBoxGap) + largeBoxWide;
  ...
  for (int i = 0; i < m_iMaxSlots; i++) {
    if (i == iActiveSlot) {
      ...
      for (int slotpos = 0; slotpos < MAX_WEAPON_POSITIONS; slotpos++) {
        ...
        DrawBox(xpos, ypos, largeBoxWide, largeBoxTall, selectedColor,
                m_flSelectionAlphaOverride, bFirstItem ? i + 1 : -1);
        ...
        if (pWeapon->GetSpriteActive()) {
          ...
          pWeapon->GetSpriteActive()->DrawSelf(xpos + m_flIconXPos,
                                               ypos + m_flIconYPos, col);
        }
      ...
      if (GetFirstPos(i)) {
        ...
        // draw empty slot
        DrawBox(xpos, ypos, m_flSmallBoxSize, m_flSmallBoxSize, m_EmptyBoxColor,
                m_flAlphaOverride, -1);
      }
...
void CHudWeaponSelection::SelectWeaponSlot(int iSlot) {
  ...
  if (pActiveWeapon != NULL) {
    ...
    for (int i = 0; i < MAX_WEAPON_POSITIONS; i++) {
      C_BaseCombatWeapon *pSlotWpn =
          GetWeaponInSlot(pActiveWeapon->GetSlot(), i);
    ...
    // if fast weapon switch is on, then weapons can be selected in a single
    // keypress but only if there is only one item in the bucket
