...
public:
  ...
  int m_iAttachmentPoint; // Which attachment point on the player model this guy
                          // is on.
};
...
public:
  ...
  bool CanShowTeamMenu()
      const; // Returns true if we're allowed to show the team menu right now.
  ...
  // Get the ID target entity index. The ID target is the player that is behind
  // our crosshairs, used to display the player's name.
  ...
  void ClearSoundEvents() { m_SoundEvents.RemoveAll(); }

  //=============================================================================
  // HPE_BEGIN:
  // [menglish] Returns whether this player is dominating or is being dominated
...
public:
  ...
  CNetworkVar(
      CSPlayerState,
      m_iPlayerState); // SupraFiend: this gives the current state in the
                       // joining process, the states are listed above
  CNetworkVar(
      bool,
  ...
  // Clients don't know about holstered weapons, so we need to be told about
  // them here
  CNetworkVar(int, m_iPrimaryAddon);
  CNetworkVar(int, m_iSecondaryAddon);

  // How long the progress bar takes to get to the end. If this is 0, then the
  // progress bar should not be drawn.
  ...
  static void RecvProxy_CycleLatch(const CRecvProxyData *pData, void *pStruct,
                                   void *pOut);
...
private:
  ...
  public:
    ...
    float
        m_flEventTime; // Play the event when gpGlobals->curtime goes past this.
  };
  CUtlLinkedList<CCSSoundEvent, int> m_SoundEvents;

  // This is the list of addons hanging off the guy (grenades, C4, nightvision,
  // etc).
  ...
  int m_cycleLatch; // server periodically updates this to fix up our anims,
                    // here it is a 4 bit fixed point
  float m_serverIntendedCycle; // server periodically updates this to fix up our
                               // anims, here it is the float we want, or -1 for
                               // no override
