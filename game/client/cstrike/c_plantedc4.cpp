...
ConVar cl_c4dynamiclight("cl_c4dynamiclight", "0", 0,
                         "Draw dynamic light when planted c4 flashes");

IMPLEMENT_CLIENTCLASS_DT(C_PlantedC4, DT_PlantedC4, CPlantedC4)
RecvPropBool(RECVINFO(m_bBombTicking)), RecvPropFloat(RECVINFO(m_flC4Blow)),
...
C_PlantedC4::~C_PlantedC4() {
  ...
  //=============================================================================
  // HPE_BEGIN:
  // [menglish] Upon the new round remove the remaining bomb explosion particle
  // effect
  //=============================================================================
...
void C_PlantedC4::ClientThink(void) {
  ...
  if (gpGlobals->curtime > m_flNextGlow) {
    ...
    // See if the c4 ended up underwater - we need to pull the flash up, or it
    // won't get seen
    if (enginetrace->GetPointContents(vPos) &
        (CONTENTS_WATER | CONTENTS_SLIME)) {
      C_CSPlayer *player = GetLocalOrInEyeCSPlayer();
      if (player) {
        const Vector &eyes = player->EyePosition();

        if ((enginetrace->GetPointContents(eyes) &
             (CONTENTS_WATER | CONTENTS_SLIME)) == 0) {
          // trace from the player to the water
          trace_t waterTrace;
          UTIL_TraceLine(eyes, vPos, (CONTENTS_WATER | CONTENTS_SLIME), player,
                         COLLISION_GROUP_NONE, &waterTrace);

          if (waterTrace.allsolid != 1) {
            // now trace from the C4 to the edge of the water (in case there was
            // something solid in the water)
            trace_t solidTrace;
            UTIL_TraceLine(vPos, waterTrace.endpos, MASK_SOLID, this,
                           COLLISION_GROUP_NONE, &solidTrace);
    ...
    tempents->TempSprite(vPos, vec3_origin, scale, modelindex, kRenderTransAdd,
                         0, 1.0, 0.05, FTENT_SPRANIMATE | FTENT_SPRANIMATELOOP);
    ...
    if (cl_c4dynamiclight.GetBool()) {
      ...
      if (dl) {
        dl->origin = GetAbsOrigin() +
                     offset; // can't use vPos because it might have been moved
        dl->color.r = 255;
        dl->color.g = 0;
        dl->color.b = 0;
        dl->radius = 64;
    ...
    float freq =
        0.1 + 0.9 * ((m_flC4Blow - gpGlobals->curtime) / m_flTimerLength);
...
//=============================================================================
// HPE_BEGIN:
// [menglish] Create the client side explosion particle effect for when the bomb
// explodes and hide the bomb
//=============================================================================
void C_PlantedC4::Explode(void) {
  m_pC4Explosion =
      ParticleProp()->Create("bomb_explosion_huge", PATTACH_ABSORIGIN);
