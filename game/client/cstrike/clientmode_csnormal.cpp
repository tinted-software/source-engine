...
// [tj] We need to forward declare this, since the definition is all inside the
// implementation file
...
// This is a temporary entity used to render the player's model while drawing
...
//-----------------------------------------------------------------------------
// HACK: the detail sway convars are archive, and default to 0.  Existing CS:S
// players thus have no detail prop sway.  We'll force them to DoD's default
// values for now.  What we really need in the long run is a system to apply
// changes to archived convars' defaults to existing players.
...
//-----------------------------------------------------------------------------
ConVar cl_autobuy("cl_autobuy", "", FCVAR_USERINFO,
                  "The order in which autobuy will attempt to purchase items");

//-----------------------------------------------------------------------------
ConVar cl_rebuy("cl_rebuy", "", FCVAR_USERINFO,
...
void SetBuyData(const ConVar &buyVar, const char *filename) {
  ...
  const char *pfile =
      engine->ParseFile((const char *)buf.Base(), token, sizeof(token));
...
void MsgFunc_KillCam(bf_read &msg) {
  ...
  if (newMode != g_nKillCamMode) {
    ...
    if (g_nKillCamMode == OBS_MODE_NONE) {
      ...
      // kill cam is switched off, restore old prediction setting is we switch
      // back to normal mode
...
void CCSModeManager::LevelInit(const char *newmap) {
  ...
  // players thus have no detail prop sway.  We'll force them to DoD's default
  // values for now.
  if (!cl_detail_max_sway.GetFloat() && !cl_detail_avoid_radius.GetFloat() &&
...
void ClientModeCSNormal::Init() {
  ...
  //=============================================================================
  // HPE_BEGIN:
  // [tj] Add the shared HUD elements to the render groups responsible for
  // hiding
  //		conflicting UI
  //=============================================================================
  ...
  CHudElement *historyResource =
      (CHudElement *)GET_HUDELEMENT(CHudHistoryResource);
  if (historyResource) {
...
void ClientModeCSNormal::UpdateSpectatorMode( void )
{
        ...
        if ( target && (obs_mode == OBS_MODE_IN_EYE || obs_mode ==
OBS_MODE_CHASE) )
        {
...
//-----------------------------------------------------------------------------
int ClientModeCSNormal::KeyInput(int down, ButtonCode_t keynum,
...
void ClientModeCSNormal::FireGameEvent(IGameEvent *event) {
  ...
  if (Q_strcmp("round_start", eventname) == 0) {
    ...
    // [Forrest] Show all centerprint messages if the end-of-round panel is
    // disabled.
    //=============================================================================
    static ConVarRef sv_nowinpanel("sv_nowinpanel");
    static ConVarRef cl_nowinpanel("cl_nowinpanel");
    if (reason == Game_Commencing || sv_nowinpanel.GetBool() ||
        cl_nowinpanel.GetBool()) {
      internalCenterPrint->Print(
          hudtextmessage->LookupString(event->GetString("message")));
    ...
    if (iTeam == TEAM_SPECTATOR)
      pHudChat->Printf(CHAT_FILTER_NONE,
                       hudtextmessage->LookupString("#Game_join_spectators"),
                       pPlayer->GetPlayerName());
    else if (iTeam == TEAM_TERRORIST)
      pHudChat->Printf(CHAT_FILTER_NONE,
                       hudtextmessage->LookupString("#Game_join_terrorist"),
                       pPlayer->GetPlayerName());
    else if (iTeam == TEAM_CT)
      pHudChat->Printf(CHAT_FILTER_NONE,
                       hudtextmessage->LookupString("#Game_join_ct"),
                       pPlayer->GetPlayerName());
  }
  ...
  //=============================================================================
  // HPE_BEGIN:
  // [menglish] Tell the client side bomb that the bomb has exploding here
  // creating the explosion particle effect
  //=============================================================================
  ...
  //=============================================================================
    ...
    if (pLocalPlayer->IsObserver() ||
        (pLocalPlayer->GetTeamNumber() == TEAM_CT)) {
      C_BaseEntity::EmitSound(filter, SOUND_FROM_LOCAL_PLAYER,
                              "Event.HostageKilled");
    }
  ...
  //=============================================================================
    ...
    if (pAchievement) {
      if (!pPlayer->IsDormant() && pPlayer->ShouldAnnounceAchievement()) {
        ...
        // Do something for the player - Actually we should probably do this
        // client-side when the achievement is first earned.
        if (pPlayer->IsLocalPlayer()) {
        }
      ...
      if (g_PR) {
        ...
        g_pVGuiLocalize->ConvertANSIToUnicode(g_PR->GetPlayerName(iPlayerIndex),
                                              wszPlayerName,
        ...
        if (constAchievementName) {
          ...
          hudChat->ChatPrintf(iPlayerIndex, CHAT_FILTER_ACHIEVEMENT, "%s",
                              szLocalized);
...
pAchievement->GetName(), "desc" ); hudChat->ChatPrintf( iPlayerIndex,
CHAT_FILTER_ACHIEVEMENT, "(%s)", constAchievementDescription );
}
          */
        }
      }
...
bool ShouldRecreateClassImageEntity(C_BaseAnimating *pEnt,
                                    const char *pNewModelName) {
...
void UpdateClassImageEntity(const char *pModelName, int x, int y, int width,
                            int height) {
  C_BasePlayer *pLocalPlayer = C_BasePlayer::GetLocalPlayer();
  ...
  if (pLocalPlayer->IsAlive() && pLocalPlayer->GetActiveWeapon()) {
    C_WeaponCSBase *weapon =
        dynamic_cast<C_WeaponCSBase *>(pLocalPlayer->GetActiveWeapon());
    if (weapon) {
      pWeaponName = weapon->GetWorldModel();
      pWeaponSequence =
          VarArgs("Walk_Upper_%s", weapon->GetCSWpnData().m_szAnimExtension);
    }
  }
  ...
  bool recreatePlayer =
      ShouldRecreateClassImageEntity(pPlayerModel, pModelName);
  if (recreatePlayer) {
    if (pPlayerModel)
      pPlayerModel->Remove();

    pPlayerModel = new C_BaseAnimatingOverlay;
    pPlayerModel->InitializeAsClientEntity(pModelName,
                                           RENDER_GROUP_OPAQUE_ENTITY);
    pPlayerModel->AddEffects(
        EF_NODRAW); // don't let the renderer draw the model normally
    ...
    pPlayerModel->SetPoseParameter("body_pitch",
                                   10.0f); // body_pitch, look down a bit
  ...
  // Does the entity even exist yet?
  if (recreatePlayer ||
      ShouldRecreateClassImageEntity(pWeaponModel, pWeaponName)) {
    if (pWeaponModel)
      pWeaponModel->Remove();

    pWeaponModel = new C_BaseAnimating;
    pWeaponModel->InitializeAsClientEntity(pWeaponName,
                                           RENDER_GROUP_OPAQUE_ENTITY);
    pWeaponModel->AddEffects(
        EF_NODRAW); // don't let the renderer draw the model normally
  ...
  // find a spot inside the world for the dlight's origin, or it won't
  // illuminate the model
  Vector testPos(origin.x - 100, origin.y, origin.z + 100);
  trace_t tr;
  UTIL_TraceLine(origin, testPos, MASK_OPAQUE, pLocalPlayer,
  ...
  if (tr.fraction == 1.0f) {
    ...
    UTIL_TraceLine(start, end, MASK_OPAQUE, pLocalPlayer, COLLISION_GROUP_NONE,
                   &tr);
    origin = tr.endpos;
  }
  ...
  pPlayerModel->m_SequenceTransitioner.CheckForSequenceChange(
      pPlayerModel->GetModelPtr(), pPlayerModel->LookupSequence("walk_lower"),
      false, true);
  pPlayerModel->m_SequenceTransitioner.UpdateCurrent(
      pPlayerModel->GetModelPtr(), pPlayerModel->LookupSequence("walk_lower"),
      pPlayerModel->GetCycle(), pPlayerModel->GetPlaybackRate(),
      gpGlobals->realtime);
  ...
  // always be lit normal like even if you are standing in a dark (or green)
  // area in the world.
  //=============================================================================
...
void ClientModeCSNormal::PostRenderVGui() {
  // If the team menu is up, then we will render the model of the character that
...
bool ClientModeCSNormal::CanRecordDemo(char *errorMsg, int length) const {
  ...
  // don't start recording while smoke grenades are spewing smoke, as the
  // existing smoke would be destroyed
  C_BaseEntityIterator it;
  C_BaseEntity *ent;
  while ((ent = it.Next()) != NULL) {
    if (Q_strcmp(ent->GetClassname(), "class C_ParticleSmokeGrenade") == 0) {
      Q_strncpy(errorMsg,
                "Cannot record demos while a smoke grenade is active.", length);
      return false;
    }
  }
...
// Receive the PlayerIgnited user message and send out a clientside event for
// achievements to hook.
void __MsgFunc_MatchEndConditions(bf_read &msg) {
