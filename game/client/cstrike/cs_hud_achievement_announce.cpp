...
CCSAchievementAnnouncePanel::CCSAchievementAnnouncePanel(
    ...
    : EditablePanel(NULL, "AchievementAnnouncePanel"),
...
void CCSAchievementAnnouncePanel::Reset() {
  // We don't want to hide the UI here since we want the achievement popup to
  // show through death
}
...
ConVar cl_show_achievement_popups("cl_show_achievement_popups", "1",
...
void CCSAchievementAnnouncePanel::FireGameEvent(IGameEvent *event) {
  ...
  if (cl_show_achievement_popups.GetBool()) {
    if (Q_strcmp("achievement_earned_local", pEventName) == 0) {
      // Add achievement to queue and show the UI (since the UI doesn't
      // "Think()" until it is shown
      Show();
      m_achievementQueue.Insert(
          (eCSAchievementType)event->GetInt("achievement"));
    }
  }
}
...
void CCSAchievementAnnouncePanel::OnThink(void) {
  ...
  if (m_currentDisplayedAchievement != CSInvalidAchievement) {
    // If the match restarts, we need to move the start time for the achievement
    // back.
    ...
    if (GetAchievementPanelAlpha(timeSinceDisplayStart,
    ...
    // Check if we need to start the next announcement in the queue. We won't
    // update it this frame, but no time has elapsed anyway.
    if (m_achievementQueue.Count() > 0) {
      ...
      // Here we get the achievement to be displayed and set that in the popup
      // windows
      ...
      IAchievement *pAchievement =
          pAchievementMgr->GetAchievementByID(m_currentDisplayedAchievement);
      if (pAchievement) {
...
bool CCSAchievementAnnouncePanel::GetAlphaFromTime(
    float elapsedTime, float delay, float fadeInTime, float holdTime,
    float fadeOutTime, float &alpha) {
  // We just pass through each phase, subtracting off the full time if we are
  // already done with that phase We return whether the control should still be
  // active
...
ConVar glow_fadeInTime("glow_fadeInTime", "0.1",
                       FCVAR_CLIENTDLL | FCVAR_DEVELOPMENTONLY);
ConVar glow_holdTime("glow_holdTime", "0.1",
                     FCVAR_CLIENTDLL | FCVAR_DEVELOPMENTONLY);
ConVar glow_fadeOutTime("glow_fadeOutTime", "2.5",
                        FCVAR_CLIENTDLL | FCVAR_DEVELOPMENTONLY);
...
bool CCSAchievementAnnouncePanel::GetGlowAlpha(float time, float &alpha) {
  ...
  */
  ...
  // return GetAlphaFromTime(time, delay, fadeInTime, holdTime, fadeOutTime,
  // alpha);

  return GetAlphaFromTime(time, glow_delay.GetFloat(),
                          glow_fadeInTime.GetFloat(), glow_holdTime.GetFloat(),
                          glow_fadeOutTime.GetFloat(), alpha);
}

ConVar popup_delay("popup_delay", "0", FCVAR_CLIENTDLL | FCVAR_DEVELOPMENTONLY);
ConVar popup_fadeInTime("popup_fadeInTime", "0.3",
                        FCVAR_CLIENTDLL | FCVAR_DEVELOPMENTONLY);
ConVar popup_holdTime("popup_holdTime", "3.5",
                      FCVAR_CLIENTDLL | FCVAR_DEVELOPMENTONLY);
ConVar popup_fadeOutTime("popup_fadeOutTime", "3.0",
                         FCVAR_CLIENTDLL | FCVAR_DEVELOPMENTONLY);

bool CCSAchievementAnnouncePanel::GetAchievementPanelAlpha(float time,
                                                           float &alpha) {
  /*
  float delay = 0.2f;
  float fadeInTime = 0.3f;
  ...
  */
  ...
  return GetAlphaFromTime(
      time, popup_delay.GetFloat(), popup_fadeInTime.GetFloat(),
      popup_holdTime.GetFloat(), popup_fadeOutTime.GetFloat(), alpha);
}
...
// res files.
//-----------------------------------------------------------------------------
CCSAchivementInfoPanel::CCSAchivementInfoPanel(vgui::Panel *parent,
                                               const char *name)
...
void CCSAchivementInfoPanel::ApplySchemeSettings(vgui::IScheme *pScheme) {
  ...
  m_pAchievementDescLabel->SetFgColor(
      pScheme->GetColor("White", Color(255, 255, 255, 255)));
}
