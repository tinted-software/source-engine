...
private:
  void Interp_Copy(VarMapping_t *pDest, CBaseEntity *pSourceEntity,
                   VarMapping_t *pSrc);
...
//-----------------------------------------------------------------------------
void C_LowViolenceHostageDeathModel::Interp_Copy(VarMapping_t *pDest,
...
//-----------------------------------------------------------------------------
bool C_LowViolenceHostageDeathModel::SetupLowViolenceModel(
    C_CHostage *pHostage) {
  const model_t *model = pHostage->GetModel();
  const char *pModelName = modelinfo->GetModelName(model);
  if (InitializeAsClientEntity(pModelName, RENDER_GROUP_OPAQUE_ENTITY) ==
      false) {
    Release();
    return false;
  }
  ...
  if (pHostage && !pHostage->IsDormant()) {
    ...
    for (int i = 0; i < NumInterestingPoseParameters; ++i) {
      int poseParameterIndex =
          LookupPoseParameter(pStudioHdr, InterestingPoseParameters[i]);
      SetPoseParameter(pStudioHdr, poseParameterIndex,
...
void C_LowViolenceHostageDeathModel::ClientThink(void) {
  ...
  iAlpha =
      MAX(iAlpha - (g_ragdoll_fadespeed.GetInt() * gpGlobals->frametime), 0);
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void C_CHostage::RecvProxy_Rescued(const CRecvProxyData *pData, void *pStruct,
                                   void *pOut) {
...
RecvPropInt(RECVINFO(m_isRescued), 0, C_CHostage::RecvProxy_Rescued),
  ...
  // TODO: Get IK working on the steep slopes CS has, then enable it on
  // characters.
...
C_BaseAnimating *C_CHostage::BecomeRagdollOnClient() {
  if (g_RagdollLVManager.IsLowViolence()) {
    // We can't just play the low-violence anim ourselves, since we're about to
    // be deleted by the server. So, let's create another entity that can play
    // the anim and stick around.
    C_LowViolenceHostageDeathModel *pLowViolenceModel =
        new C_LowViolenceHostageDeathModel();
    m_createdLowViolenceRagdoll =
        pLowViolenceModel->SetupLowViolenceModel(this);
    if (m_createdLowViolenceRagdoll) {
      UpdateVisibility();
...
//-----------------------------------------------------------------------------
CWeaponCSBase *C_CHostage::CSAnim_GetActiveWeapon() { return NULL; }

//-----------------------------------------------------------------------------
bool C_CHostage::CSAnim_CanMove() { return true; }
...
void C_CHostage::UpdateLookAt(CStudioHdr *pStudioHdr) {
  ...
  // head yaw
  if (m_headYawPoseParam < 0 || m_bodyYawPoseParam < 0 ||
      m_headPitchPoseParam < 0)
    return;
  ...
  bodyAngles[YAW] =
      GetRenderAngles()[YAW] + RemapVal(poseParams[m_bodyYawPoseParam], 0, 1,
                                        m_bodyYawMin, m_bodyYawMax);
  ...
  m_flCurrentHeadYaw =
      ApproachAngle(desired, m_flCurrentHeadYaw,
                    HOSTAGE_HEAD_TURN_RATE * gpGlobals->frametime);

  // Counterrotate the head from the body rotation so it doesn't rotate past its
  // target.
  ...
  m_flCurrentHeadPitch =
      ApproachAngle(desired, m_flCurrentHeadPitch,
                    HOSTAGE_HEAD_TURN_RATE * gpGlobals->frametime);
...
//-----------------------------------------------------------------------------
void C_CHostage::ImpactTrace(trace_t *pTrace, int iDamageType,
