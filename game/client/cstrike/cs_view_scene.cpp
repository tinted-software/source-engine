  ...
  if (pPlayer && pPlayer->m_flNightVisionAlpha > 0) {
    ...
    if (pMaterial) {
      ...
      if (pPlayer->m_bNightVisionOn) {
        ...
        pPlayer->m_flNightVisionAlpha =
            MIN(pPlayer->m_flNightVisionAlpha, iMaxValue);
      } else {
      ...
      /*			if (
         ...
         materials ); pRenderContext->DrawScreenSpaceQuad( pMaterial );
                                      render->ViewDrawFade( overlaycolor,
         pMaterial ); pRenderContext->DrawScreenSpaceQuad( pMaterial );
                              }*/
    }
  }
}
...
void CCSViewRender::PerformFlashbangEffect(const CViewSetup &view) {
  ...
  IMaterial *pMaterial = materials->FindMaterial(
      "effects/flashbang", TEXTURE_GROUP_CLIENT_EFFECTS, true);
  ...
  if (pPlayer->m_flFlashAlpha < pPlayer->m_flFlashMaxAlpha) {
    ...
    pPlayer->m_flFlashAlpha =
        MIN(pPlayer->m_flFlashAlpha, pPlayer->m_flFlashMaxAlpha);
    ...
    pRenderContext->CopyRenderTargetToTextureEx(m_pFlashTexture, 0, &srcRect,
                                                NULL);
    ...
    /*
                    if (g_pMaterialSystemHardwareConfig->GetDXSupportLevel() >=
       80)
                    {
                            pRenderContext->DrawScreenSpaceRectangle( pMaterial,
       view.x, view.y, view.width, view.height, 0, 0,
       m_pFlashTexture->GetActualWidth()-1,
       m_pFlashTexture->GetActualHeight()-1, m_pFlashTexture->GetActualWidth(),
       m_pFlashTexture->GetActualHeight() ); render->ViewDrawFade( overlaycolor,
       pMaterial ); pRenderContext->DrawScreenSpaceRectangle( pMaterial, view.x,
       view.y, view.width, view.height, 0, 0,
       m_pFlashTexture->GetActualWidth()-1,
       m_pFlashTexture->GetActualHeight()-1, m_pFlashTexture->GetActualWidth(),
  ...
  pMaterial = materials->FindMaterial("effects/flashbang_white",
  ...
  if (pPlayer->m_flFlashAlpha < pPlayer->m_flFlashMaxAlpha) {
    ...
    const float certainBlindnessTimeThresh =
        3.0; // yes this is a magic number, necessary to match CS/CZ flashbang
             // effectiveness cause the rendering system is completely
             // different.

    if (flFlashTimeLeft > certainBlindnessTimeThresh) {
      // if we still have enough time of blindness left, make sure the player
      // can't see anything yet.
      flAlphaPercentage = 1.0;
    } else {
      // blindness effects shorter than 'certainBlindnessTimeThresh' will start
      // off at less than 255 alpha.
    ...
    flAlpha = flAlphaPercentage *=
        pPlayer->m_flFlashMaxAlpha; // scale a [0..1) value to a [0..MaxAlpha]
                                    // value for the alpha.
...
void CCSViewRender::Render2DEffectsPreHUD(const CViewSetup &view) {
  PerformNightVisionEffect(view); // this needs to come before the HUD is drawn,
                                  // or it will wash the HUD out
}
