...
void CHudAmmo::SetAmmo(int ammo, bool playAnimation) {
  if (ammo != m_iAmmo) {
    if (ammo == 0) {
      ...
      // ammo has decreased
      g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(
          "PrimaryAmmoDecrement");
    } else {
      // ammunition has increased
      g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(
          "PrimaryAmmoIncrement");
    }
...
void CHudAmmo::SetAmmo2(int ammo2, bool playAnimation) {
  if (ammo2 != m_iAmmo2) {
    if (ammo2 == 0) {
      ...
      // ammo has decreased
      g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(
          "SecondaryAmmoDecrement");
    } else {
      // ammunition has increased
      g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(
          "SecondaryAmmoIncrement");
    }
...
void CHudAmmo::Paint(void) {
  ...
  if (m_bUsesClips) {
    ...
    vgui::surface()->DrawTexturedRect(bar_xpos, bar_ypos, bar_xpos + bar_width,
                                      bar_ypos + bar_height);
