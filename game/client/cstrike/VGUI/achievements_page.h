...
// Loads an achievement's icon into a specified image panel, or turns the panel
// off if no achievement icon was found.
...
public:
  CHiddenHUDToggleButton(vgui::Panel *pParent, const char *pName,
                         const char *pText);
...
public:
  CAchievementsPageGroupPanel(vgui::PanelListPanel *parent,
                              CAchievementsPage *owner, const char *name,
                              int iListItemID);
  ~CAchievementsPageGroupPanel();

  void SetGroupInfo(const wchar_t *name, int firstAchievementID,
