...
const float kScaleMVP =
    0.75f; // scale of the MVP star relative to the player name height
...
const float kTeamScoreLineLeadingRatio =
    0.25f; // padding as a ratio of avatar height

// CT player data colors
ConVar cl_scoreboard_ct_color_red("cl_scoreboard_ct_color_red", "150",
                                  FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
                                  "Scoreboard CT player data red channel", true,
                                  0.0f, true, 255.0f);
ConVar cl_scoreboard_ct_color_green("cl_scoreboard_ct_color_green", "200",
                                    FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
                                    "Scoreboard CT player data green channel",
                                    true, 0.0f, true, 255.0f);
ConVar cl_scoreboard_ct_color_blue("cl_scoreboard_ct_color_blue", "255",
                                   FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
                                   "Scoreboard CT player data blue channel",
                                   true, 0.0f, true, 255.0f);

// T player data colors
ConVar cl_scoreboard_t_color_red("cl_scoreboard_t_color_red", "240",
                                 FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
                                 "Scoreboard T player data red channel", true,
                                 0.0f, true, 255.0f);
ConVar cl_scoreboard_t_color_green("cl_scoreboard_t_color_green", "90",
                                   FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
                                   "Scoreboard T player data green channel",
                                   true, 0.0f, true, 255.0f);
ConVar cl_scoreboard_t_color_blue("cl_scoreboard_t_color_blue", "90",
                                  FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
                                  "Scoreboard T player data blue channel", true,
                                  0.0f, true, 255.0f);

// Dead player data colors
ConVar cl_scoreboard_dead_color_red("cl_scoreboard_dead_color_red", "125",
                                    FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
                                    "Scoreboard dead player data red channel",
                                    true, 0.0f, true, 255.0f);
ConVar cl_scoreboard_dead_color_green(
    "cl_scoreboard_dead_color_green", "125", FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
    "Scoreboard dead player data green channel", true, 0.0f, true, 255.0f);
ConVar cl_scoreboard_dead_color_blue("cl_scoreboard_dead_color_blue", "125",
                                     FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
                                     "Scoreboard dead player data blue channel",
                                     true, 0.0f, true, 255.0f);

// Clan colors
ConVar cl_scoreboard_clan_ct_color_red(
    "cl_scoreboard_clan_ct_color_red", "150", FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
    "Scoreboard CT player clan tag red channel", true, 0.0f, true, 255.0f);
ConVar cl_scoreboard_clan_ct_color_green(
    "cl_scoreboard_clan_ct_color_green", "200", FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
    "Scoreboard CT player clan tag green channel", true, 0.0f, true, 255.0f);
ConVar cl_scoreboard_clan_ct_color_blue(
    "cl_scoreboard_clan_ct_color_blue", "255", FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
    "Scoreboard CT player clan tag blue channel", true, 0.0f, true, 255.0f);

ConVar cl_scoreboard_clan_t_color_red(
    "cl_scoreboard_clan_t_color_red", "240", FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
    "Scoreboard T player clan tag red channel", true, 0.0f, true, 255.0f);
ConVar cl_scoreboard_clan_t_color_green(
    "cl_scoreboard_clan_t_color_green", "90", FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
    "Scoreboard T player clan tag green channel", true, 0.0f, true, 255.0f);
ConVar cl_scoreboard_clan_t_color_blue(
    "cl_scoreboard_clan_t_color_blue", "90", FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
    "Scoreboard T player clan tag blue channel", true, 0.0f, true, 255.0f);

ConVar cl_scoreboard_dead_clan_color_red(
    "cl_scoreboard_dead_clan_color_red", "125", FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
    "Scoreboard dead player clan tag red channel", true, 0.0f, true, 255.0f);
ConVar cl_scoreboard_dead_clan_color_green(
    "cl_scoreboard_dead_clan_color_green", "125",
    FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
    "Scoreboard dead player clan tag green channel", true, 0.0f, true, 255.0f);
ConVar cl_scoreboard_dead_clan_color_blue(
    "cl_scoreboard_dead_clan_color_blue", "125",
    FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
    "Scoreboard dead player clan tag blue channel", true, 0.0f, true, 255.0f);

// [tj] These ConVars are defined at various places in the global scope. Just
// declaring them here so we can use them
...
CCSClientScoreBoardDialog::CCSClientScoreBoardDialog(IViewPort *pViewPort)
  ...
  // [pfreese] Make the scoreboard a popup so it renders over the chat interface
  // (which is also a popup). Hacky.
  MakePopup();
...
void CCSClientScoreBoardDialog::ApplySchemeSettings(vgui::IScheme *pScheme) {
  ...
  //
  // [smessick] Note: ApplySchemeSettings is called multiple times for the
  // scoreboard. Therefore, we must make sure to delete previously allocated
  // items.
  //
  ...
  m_listItemFontSmaller =
      pScheme->GetFont("ScoreboardBody_2", IsProportional());
  m_listItemFontSmallest =
      pScheme->GetFont("ScoreboardBody_3", IsProportional());
  ...
  if (m_pServerName[0] == L'\0' && g_pClientMode->GetServerName() != NULL) {
    V_wcsncpy(m_pServerName, g_pClientMode->GetServerName(),
              sizeof(m_pServerName));
  }

  // Cache the stats enabled string.
  if (m_pStatsEnabled[0] == L'\0') {
    V_wcsncpy(m_pStatsEnabled,
              LocalizeFindSafe("#Cstrike_Scoreboard_StatsEnabled"),
              sizeof(m_pStatsEnabled));
  }

  // Cache the stats disabled string.
  if (m_pStatsDisabled[0] == L'\0') {
    V_wcsncpy(m_pStatsDisabled,
              LocalizeFindSafe("#Cstrike_Scoreboard_StatsDisabled"),
              sizeof(m_pStatsDisabled));
  }

  SetVisible(false);
}

void CCSClientScoreBoardDialog::SetupTeamDisplay(TeamDisplayInfo &teamDisplay,
                                                 const char *szTeamPrefix) {
  const int selectMargin =
      scheme()->GetProportionalScaledValueEx(GetScheme(), 1);
  const int mvpLabelWidth =
      scheme()->GetProportionalScaledValueEx(GetScheme(), 20);
  const int mvpLabelYOffset =
      scheme()->GetProportionalScaledValueEx(GetScheme(), 2);
  ...
  if (pPlayerAvatar0 != NULL) {
    ...
    teamDisplay.scoreAreaLineHeight =
        MAX(tall, teamDisplay.scoreAreaLineHeight);
  ...
  int marginY =
      RoundFloatToInt(teamDisplay.scoreAreaLineHeight * kTeamScoreMargin);

  teamDisplay.scoreAreaInnerHeight = playerTall - 2 * marginY;
  teamDisplay.scoreAreaLinePreferredLeading = RoundFloatToInt(
      teamDisplay.scoreAreaLineHeight * kTeamScoreLineLeadingRatio);
  teamDisplay.scoreAreaStartY = playerY + marginY;
  teamDisplay.maxPlayersVisible =
      MIN(cMaxScoreLines,
          teamDisplay.scoreAreaInnerHeight / teamDisplay.scoreAreaLineHeight);

  // Calculate the starting point for player data.
  int startY = teamDisplay.scoreAreaStartY;
  int spacingY = teamDisplay.scoreAreaLineHeight +
  ...
  for (int i = 0; i < cMaxScoreLines; ++i, startY += spacingY) {
    ...
    if (pPlayerAvatar0 != NULL) {
      ...
      playerDisplay.pAvatar = (CAvatarImagePanel *)SETUP_PANEL(
          new CAvatarImagePanel(this, tmpName));
      playerDisplay.pAvatar->SetBounds(x, startY, wide, tall);
      playerDisplay.pAvatar->SetDefaultAvatar(scheme()->GetImage(
          &teamDisplay == &m_teamDisplayCT ? CSTRIKE_DEFAULT_CT_AVATAR
                                           : CSTRIKE_DEFAULT_T_AVATAR,
          true));
    ...
    if (pPlayerClan0 != NULL) {
      ...
      playerDisplay.pClanLabel =
          (Label *)SETUP_PANEL(new Label(this, tmpName, tmpName));
    ...
    if (pPlayerName0 != NULL) {
      ...
      // mvp
      int wideMVP =
          RoundFloatToInt(teamDisplay.scoreAreaLineHeight * kScaleMVP);
      int tallMVP =
          RoundFloatToInt(teamDisplay.scoreAreaLineHeight * kScaleMVP);
      ...
      playerDisplay.pMVPImage =
          (ImagePanel *)SETUP_PANEL(new ImagePanel(this, tmpName));
      ...
      playerDisplay.pMVPCountLabel =
          (Label *)SETUP_PANEL(new Label(this, tmpName, ""));
      playerDisplay.pMVPCountLabel->SetFont(m_MVPFont);
      playerDisplay.pMVPCountLabel->SetBounds(0, -mvpLabelYOffset,
                                              mvpLabelWidth, tallMVP);
      playerDisplay.pMVPCountLabel->SetVisible(false);

      // Pin to the mvp image.
      V_snprintf(tmpName, 32, "_%s_mvp%d", szTeamPrefix, i);
      playerDisplay.pMVPCountLabel->PinToSibling(tmpName, PIN_CENTER_LEFT,
                                                 PIN_CENTER_RIGHT);
    }
...
//-----------------------------------------------------------------------------
int CCSClientScoreBoardDialog::PlayerSortFunction(
    PlayerScoreInfo *const *pLeft, PlayerScoreInfo *const *pRight) {
  ...
  // bail out early if either player is an empty slot, i.e. has a player index
  // of -1
  ...
  // if score and deaths are the same, use player index to get deterministic
  // sort
    ...
    if (team) {
      ...
      for (int playerIndex = 1; playerIndex <= MAX_PLAYERS; playerIndex++) {
        if (g_PR->IsConnected(playerIndex) &&
            g_PR->GetTeam(playerIndex) == teamIndex) {
          numPlayers++;
      ...
      V_snwprintf(numAliveString, ARRAYSIZE(numAliveString), L"%i / %i",
                  numAlive, numPlayers);
...
//-----------------------------------------------------------------------------
void CCSClientScoreBoardDialog::AdjustFontToFit(const char *pString,
                                                vgui::Label *pLabel) {
  if (!pString || !pLabel)
    return;
  ...
  g_pMatSystemSurface->GetTextSize(m_listItemFont, pWideString, stringWidth,
                                   stringHeight);
  ...
  if (stringWidth <= labelWidth) {
    ...
    g_pMatSystemSurface->GetTextSize(m_listItemFontSmaller, pWideString,
...
void CCSClientScoreBoardDialog::UpdatePlayerList() {
  ...
  // Force the local player to be visible when he is below the visible portion
  // of the sorted list
...
void CCSClientScoreBoardDialog::UpdateTeamPlayerDisplay(
    TeamDisplayInfo &teamDisplay) {
  const int selectMargin =
      scheme()->GetProportionalScaledValueEx(GetScheme(), 1);
  ...
  int maxTeamSize = MAX(m_teamDisplayT.playerScores.Count(),
                        m_teamDisplayCT.playerScores.Count());

  // adjust spacing
  int leadingAvailable = teamDisplay.scoreAreaInnerHeight -
                         maxTeamSize * teamDisplay.scoreAreaLineHeight;
  int leading = 0;
  if (maxTeamSize > 1) // only makes sense if we have more than one player
  {
    leading = clamp(leadingAvailable / (maxTeamSize - 1), 0,
                    teamDisplay.scoreAreaLinePreferredLeading);
  }
  ...
  int i = 0;
  int startY = teamDisplay.scoreAreaStartY;
  for (i = 0; i < MIN(cMaxScoreLines, teamDisplay.playerScores.Count());
       ++i, startY += spacingY) {
    if (startY + teamDisplay.scoreAreaLineHeight >
        teamDisplay.scoreAreaStartY + teamDisplay.scoreAreaInnerHeight)
      break;
    ...
    if (pPlayerScore) {
      ...
      // 				bufsize = strlen( oldName ) * 2 + 14 +
      // 1; 			else 				bufsize = strlen( oldName ) * 2 + 1;
      //
      ...
      if (pUTF8Name != NULL && V_strlen(pUTF8Name) > 0) {
        ...
        Color fgColor =
            (isAlive ? teamDisplay.playerDataColor : m_DeadPlayerDataColor);
        Color fgClanColor =
            (isAlive ? teamDisplay.playerClanColor : m_DeadPlayerClanColor);
      ...
      if (playerDisplay.pStatusImage != NULL) {
        if (pPlayerScore->szStatus == NULL) {
          ...
          playerDisplay.pStatusImage->SetDrawColor(
              pPlayerScore->bStatusPlayerColor ? teamDisplay.playerDataColor
                                               : COLOR_WHITE);
        }
      }
      ...
      if (playerDisplay.pSelect != NULL) {
        playerDisplay.pSelect->SetVisible(pPlayerScore->playerIndex ==
                                          iLocalPlayerIndex);
        playerDisplay.pSelect->SetPos(teamDisplay.scoreAreaMinX - selectMargin,
                                      startY - selectMargin);
      }
    }
  }
...
void CCSClientScoreBoardDialog::UpdateSpectatorList() {
  ...
  if (nSpectators == 0) {
    // No spectators.
    wchar_t *noSpectators =
        g_pVGuiLocalize->Find("#Cstrike_Scoreboard_NoSpectators");
    ...
    wchar_t *formatLabel = g_pVGuiLocalize->Find(
        (nSpectators == 1) ? "#Cstrike_Scoreboard_Spectator"
                           : "#Cstrike_Scoreboard_Spectators");
    if (formatLabel != NULL) {
      g_pVGuiLocalize->ConstructString(labelText, sizeof(labelText),
                                       formatLabel, 2, countText, listText);
    }
  }
...
void CCSClientScoreBoardDialog::UpdateHLTVList(void) {
  ...
  if (formatLabel != NULL) {
    g_pVGuiLocalize->ConstructString(labelText, sizeof(labelText), formatLabel,
                                     1, countText);
  }
...
void CCSClientScoreBoardDialog::UpdateMvpElements() {
  ...
  for (int teamIndex = TEAM_TERRORIST; teamIndex <= TEAM_CT; ++teamIndex) {
    TeamDisplayInfo &teamDisplay =
        (teamIndex == TEAM_TERRORIST) ? m_teamDisplayT : m_teamDisplayCT;

    for (int i = 0; i < cMaxScoreLines && i < teamDisplay.playerScores.Count();
         ++i) {
      PlayerDisplay &playerDisplay = teamDisplay.playerDisplay[i];
      ...
      g_pMatSystemSurface->GetTextSize(playerDisplay.pNameLabel->GetFont(),
                                       playerName, actualWideName,
                                       actualTallName);
      ...
      int wideMVPImage = 0;
      int tallMVPImage = 0;
      playerDisplay.pMVPImage->GetBounds(xMVPImage, yMVPImage, wideMVPImage,
                                         tallMVPImage);
      ...
      if (showMVPNumber) {
        ...
        g_pMatSystemSurface->GetTextSize(
            playerDisplay.pMVPCountLabel->GetFont(), mvpText,
            actualWideMVPLabel, actualTallMVPLabel);
      }
...
void CCSClientScoreBoardDialog::FireGameEvent(IGameEvent *event) {
  ...
  if (Q_strcmp(pEventName, "server_spawn") == 0) {
    ...
    if (hostname != NULL) {
      ...
      g_pVGuiLocalize->ConvertANSIToUnicode(hostname, wzHostName,
                                            sizeof(wzHostName));
      g_pVGuiLocalize->ConstructString(
          m_pServerName, sizeof(m_pServerName),
    ...
    if (mapName != NULL) {
      g_pVGuiLocalize->ConvertANSIToUnicode(mapName, m_pMapName,
                                            sizeof(m_pMapName));
      SetDialogVariable("mapname", m_pMapName);
...
//-----------------------------------------------------------------------------
bool CCSClientScoreBoardDialog::GetPlayerScoreInfo(
    int playerIndex, PlayerScoreInfo &playerScoreInfo) {
  if (g_PR == NULL)
    return false;
  ...
  bool bShowExtraInfo =
      (pLocalPlayer->GetTeamNumber() ==
       TEAM_UNASSIGNED) || // we're not spawned yet
      (pLocalPlayer->GetTeamNumber() == TEAM_SPECTATOR) || // we are a spectator
      (pLocalPlayer->IsPlayerDead() &&
       mp_forcecamera.GetInt() ==
           OBS_ALLOW_ALL) || // we are dead and allowed to spectate opponents
      (pLocalPlayer->GetTeamNumber() ==
       g_PR->GetTeam(playerIndex)); // we're on the same team

  playerScoreInfo.szStatus = NULL;
  playerScoreInfo.bStatusPlayerColor = false;

  // set the status icon; lowest priority icons are tested first, and highest
  // last
  ...
  if (!g_PR->IsAlive(playerIndex) &&
      g_PR->GetTeam(playerIndex) > TEAM_SPECTATOR) {
    playerScoreInfo.szStatus = "../hud/scoreboard_dead";
  }

  if (g_PR->IsHLTV(playerIndex)) {
    //         // show #spectators in class field, it's transmitted as player's
    //         score char numspecs[32]; Q_snprintf( numspecs, sizeof( numspecs
    //         ), "%i Spectators", m_HLTVSpectators ); kv->SetString( "class",
    //         numspecs );
  }
...
void CCSClientScoreBoardDialog::UpdateMatchEndText() {
  ...
  if (!m_gameOver) {
    ...
    if (mp_timelimit.GetInt() != 0) {
      ...
      if (showTime) {
        if (m_pWinConditionLabel) {
          V_snwprintf(wzMatchEndCause, ARRAYSIZE(wzMatchEndCause), L"%.2i:%.2i",
                      timeTillEndOfMatch / 60, timeTillEndOfMatch % 60);
          g_pVGuiLocalize->ConstructString(
              wzMatchEndCausesLabel, sizeof(wzMatchEndCausesLabel),
    ...
    // Round limit
      ...
      if (m_pWinConditionLabel) {
        ...
        int roundsPlayed = 0;
        for (int teamIndex = TEAM_TERRORIST; teamIndex <= TEAM_CT;
             teamIndex++) {
          C_Team *team = GetGlobalTeam(teamIndex);
          if (team) {
        ...
        V_snwprintf(wzMatchEndCause, ARRAYSIZE(wzMatchEndCause), L"%d",
                    mp_maxrounds.GetInt() - roundsPlayed);
        g_pVGuiLocalize->ConstructString(
            wzMatchEndCausesLabel, sizeof(wzMatchEndCausesLabel),
...
// [tj] We hook into the show command so we can lock or unlock all the elements
// that need to be hidden
//
// [pfreese] This used to enable/disable keyboard input, but since the
// scoreboard is now a popup, we have to leave the keyboard disabled
...
void CCSClientScoreBoardDialog::UpdatePlayerColors(void) {
  m_teamDisplayT.playerDataColor.SetColor(
      cl_scoreboard_t_color_red.GetInt(), cl_scoreboard_t_color_green.GetInt(),
      cl_scoreboard_t_color_blue.GetInt(), 255);
  m_teamDisplayCT.playerDataColor.SetColor(
      cl_scoreboard_ct_color_red.GetInt(),
      cl_scoreboard_ct_color_green.GetInt(),
      cl_scoreboard_ct_color_blue.GetInt(), 255);
  m_DeadPlayerDataColor.SetColor(cl_scoreboard_dead_color_red.GetInt(),
                                 cl_scoreboard_dead_color_green.GetInt(),
                                 cl_scoreboard_dead_color_blue.GetInt(), 255);
  m_teamDisplayT.playerClanColor.SetColor(
      cl_scoreboard_clan_t_color_red.GetInt(),
      cl_scoreboard_clan_t_color_green.GetInt(),
      cl_scoreboard_clan_t_color_blue.GetInt(), 255);
  m_teamDisplayCT.playerClanColor.SetColor(
      cl_scoreboard_clan_ct_color_red.GetInt(),
  ...
  m_DeadPlayerClanColor.SetColor(cl_scoreboard_dead_clan_color_red.GetInt(),
                                 ...
                                 cl_scoreboard_dead_clan_color_blue.GetInt(),
                                 255);
}
...
bool CCSClientScoreBoardDialog::ForceLocalPlayerVisible(
    TeamDisplayInfo &teamDisplay) {
  int iLocalPlayerIndex = GetLocalPlayerIndex();

  // Look for the local player in the non-visible portion of the member list
  for (int i = teamDisplay.maxPlayersVisible;
       i < teamDisplay.playerScores.Count(); ++i) {
    ...
    if (pPlayerScore->playerIndex == iLocalPlayerIndex) {
      ...
      teamDisplay.playerScores.InsertBefore(teamDisplay.maxPlayersVisible - 1,
                                            pPlayerScore);
