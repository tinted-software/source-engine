...
struct ImageInfo {
  ...
  void FitInBounds(int baseX, int baseY, int width, int height, bool center,
                   int scaleAt1024, bool halfHeight = false);
  void Paint();
};
...
public:
  EquipmentLabel(vgui::Panel *parent, const char *panelName,
...
//--------------------------------------------------------------------------------------------------------------
/**
 * BuyPresetEditPanel is a panel displaying a graphical representation of a buy
 * preset.
 */
...
public:
  BuyPresetEditPanel(vgui::Panel *parent, const char *panelName,
                     const char *resourceFilename, int fallbackIndex,
                     bool editableName);
