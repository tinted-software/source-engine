...
private:
  ...
  int GetMasterAlpha(
      void); // The main alpha that the map part should be, determined by using
             // the mode to look at the right convar
  int GetBorderSize(
      void); // How far in from the edge of the panel we draw, based on mode.
             // Let's the background fancy corners show.
