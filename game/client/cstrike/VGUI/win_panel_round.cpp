...
//
// Purpose: Create and display a win panel at the end of a round displaying
// interesting stats and info about the round.
//
// $NoKeywords: $
//=============================================================================//
...
ConVar cl_round_win_fade_time("cl_round_win_fade_time", "1.5",
...
WinPanel_Round::WinPanel_Round(const char *pElementName)
    : BorderedPanel(NULL, pElementName), CHudElement(pElementName),
      m_bIsFading(false), m_fFadeBeginTime(0.0f) {
...
// Purpose:
//-----------------------------------------------------------------------------
void WinPanel_Round::Reset() { Hide(); }
...
void WinPanel_Round::InitLayout() {
  // reload control settings when resolution changes to force update of
  // proportional layout
  LoadControlSettings("Resource/UI/Win_Round.res");

  CAvatarImagePanel *pMVP_Avatar =
      dynamic_cast<CAvatarImagePanel *>(FindChildByName("MVP_Avatar"));
  pMVP_Avatar->SetDefaultAvatar(
      scheme()->GetImage(CSTRIKE_DEFAULT_AVATAR, true));
...
//=============================================================================
ConVar cl_nowinpanel("cl_nowinpanel", "0", FCVAR_ARCHIVE,
...
void WinPanel_Round::FireGameEvent(IGameEvent *event) {
  ...
  if (Q_strcmp("round_end", pEventName) == 0) {
    ...
    "final_event"		"byte"		// 0 - no event, 1 - bomb
    exploded, 2 - flag capped, 3 - timer expired

    "funfact_type"		"byte"		//WINPANEL_FUNFACT in
    cs_shareddef.h "funfact_player"	"byte" "funfact_data1"		"long"
    "funfact_data2"		"long"
    "funfact_data3"		"long"
    */
    ...
    if (strlen(funfactToken) != 0) {
      ...
      if (iFunFactPlayer >= 1 && iFunFactPlayer <= MAX_PLAYERS) {
        ...
        if (playerName && Q_strcmp(playerName, PLAYER_UNCONNECTED_NAME) != 0 &&
          ...
          V_strtowcs(g_PR->GetPlayerName(iFunFactPlayer), 64, playerText,
                     sizeof(playerText));
        } else {
#ifdef WIN32
          _snwprintf(playerText, ARRAYSIZE(playerText), L"%s",
                     LocalizeFindSafe("#winpanel_former_player"));
#else
          _snwprintf(playerText, ARRAYSIZE(playerText), L"%S",
                     LocalizeFindSafe("#winpanel_former_player"));
#endif
      ...
      g_pVGuiLocalize->ConstructString(
          funFactText, sizeof(funFactText),
    ...
    endEventToString[Hostages_Not_Rescued] =
    ...
    endEventToString[Escaping_Terrorists_Neutralized] =
    ...
    if (wszEventMessage != NULL) {
      SetDialogVariable("WIN_DESCRIPTION",
                        UpperCaseWideString(wszEventMessage));
    } else {
      SetDialogVariable("WIN_DESCRIPTION", "");
    }
    ...
    case VIP_Not_Escaped:
      pWinLabel->SetText(
          UpperCaseWideString(LocalizeFindSafe("#winpanel_t_win")));
    ...
    case Terrorists_Not_Escaped:
      pWinLabel->SetText(
          UpperCaseWideString(LocalizeFindSafe("#winpanel_ct_win")));
      pWinLabel->SetFgColor(Color(71, 152, 237, 255));
      break;

    case Round_Draw:
      pWinLabel->SetText(
          UpperCaseWideString(LocalizeFindSafe("#winpanel_draw")));
...
void WinPanel_Round::SetMVP(C_CSPlayer *pPlayer, CSMvpReason_t reason) {
  ...
  //=============================================================================
  {
    ...
    g_pVGuiLocalize->ConvertANSIToUnicode(
        UTIL_SafeName(pPlayer->GetPlayerName()), wszPlayerName,
    ...
    g_pVGuiLocalize->ConstructString(wszBuf, sizeof(wszBuf), pReason, 1,
                                     wszPlayerName);
  ...
  // [Forrest] Allow MVP to be turned off for a server
  //=============================================================================
  // The avatar image and its accompanying elements should be hidden if there is
  // no MVP for the round.
  if (pMVP_Avatar) {
    pMVP_Avatar->SetVisible(isThereAnMVP);
  }
  ImagePanel *pMVP_AvatarGlow =
      dynamic_cast<ImagePanel *>(FindChildByName("MVP_AvatarGlow"));
  if (pMVP_AvatarGlow) {
    pMVP_AvatarGlow->SetVisible(isThereAnMVP);
  }
  ImagePanel *pMVP_Foreground_Star =
      dynamic_cast<ImagePanel *>(FindChildByName("MVP_Foreground_Star"));
...
void WinPanel_Round::OnThink() {
  if (m_bShouldBeVisible && m_bIsFading) {
    float fAlpha = 1.0f - (gpGlobals->realtime - m_fFadeBeginTime) /
                              cl_round_win_fade_time.GetFloat();

    if (fAlpha >= 0.0f) {
      SetAlpha(RoundFloatToInt(fAlpha * 255.0f));
    } else {
      int iRenderGroup =
          gHUD.LookupRenderGroupIndexByName("hide_for_round_panel");
      if (iRenderGroup >= 0) {
