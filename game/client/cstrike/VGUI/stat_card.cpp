...
// Purpose: Constructor
//-----------------------------------------------------------------------------
StatCard::StatCard(vgui::Panel *parent, const char *name)
    : BaseClass(parent, "CSStatCard") {
...
void StatCard::UpdateInfo() {
  ...
  g_pVGuiLocalize->ConstructString(
      buf, sizeof(buf), g_pVGuiLocalize->Find("#GameUI_Stats_LastMatch_MVPS"),
      1, numBuf);
  ...
  if (m_pAvatar && steamapicontext && steamapicontext->SteamUser()) {
    m_pAvatar->SetPlayer(steamapicontext->SteamUser()->GetSteamID(),
                         k_EAvatarSize64x64);
