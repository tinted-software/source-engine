...
ConVar cl_radar_locked("cl_radar_locked", "0", FCVAR_ARCHIVE,
                       "Lock the angle of the radar display?");

void PreferredOverviewModeChanged(IConVar *pConVar, const char *oldString,
                                  float flOldValue) {
...
ConVar overview_preferred_mode("overview_preferred_mode", "1", FCVAR_ARCHIVE,
                               "Preferred overview mode",
                               PreferredOverviewModeChanged);

ConVar overview_preferred_view_size("overview_preferred_view_size", "600",
                                    FCVAR_ARCHIVE,
...
// To make your own green radar file from the map overview file, turn this on,
// and include vtf.lib
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CCSSpectatorGUI::CCSSpectatorGUI(IViewPort *pViewPort)
...
void CCSSpectatorGUI::UpdateTimer() {
  ...
  wchar_t szText[63];
  _snwprintf(szText, ARRAYSIZE(szText), L"%d:%02d", (m_nLastTime / 60),
             (m_nLastTime % 60));
  szText[62] = 0;
...
/*bool CCSSpectatorGUI::CanSpectateTeam( int iTeam )
{
        ...
        if ( C_BasePlayer::GetLocalPlayer()->GetTeamNumber() // &&
...
void CCSSpectatorGUI::StoreWidths(void) {
  ...
  if (!m_modifiedWidths) {
    ...
    if (m_scoreWidth != terScoreWidth) {
      m_pTerScore = NULL; // We're working with a modified res file.  Don't muck
                          // things up playing with positioning.
    }
  }
}
...
bool CCSSpectatorGUI::ControlsPresent(void) const {
  return (m_pCTLabel != NULL && m_pCTScore != NULL && m_pTerLabel != NULL &&
          m_pTerScore != NULL && m_pTimer != NULL && m_pTimerLabel != NULL &&
          m_pDivider != NULL && m_pExtraInfo != NULL);
}
...
void CCSMapOverview::InitTeamColorsAndIcons() {
  ...
  m_TeamIconsOffscreen[MAP_ICON_T] =
      AddIconTexture("sprites/player_red_offscreen");
  m_TeamIconsDeadOffscreen[MAP_ICON_T] =
      AddIconTexture("sprites/player_red_dead_offscreen");
  ...
  m_TeamIconsOffscreen[MAP_ICON_CT] =
      AddIconTexture("sprites/player_blue_offscreen");
  m_TeamIconsDeadOffscreen[MAP_ICON_CT] =
      AddIconTexture("sprites/player_blue_dead_offscreen");

  // setup team other
  m_TeamColors[MAP_ICON_HOSTAGE] = COLOR_GREY;
  m_TeamIcons[MAP_ICON_HOSTAGE] =
      AddIconTexture("sprites/player_hostage_small");
  m_TeamIconsSelf[MAP_ICON_HOSTAGE] = -1;
  m_TeamIconsDead[MAP_ICON_HOSTAGE] =
      AddIconTexture("sprites/player_hostage_dead");
  m_TeamIconsOffscreen[MAP_ICON_HOSTAGE] =
      AddIconTexture("sprites/player_hostage_offscreen");
  m_TeamIconsDeadOffscreen[MAP_ICON_HOSTAGE] =
      AddIconTexture("sprites/player_hostage_dead_offscreen");
  ...
  m_bombRingCarriedOffscreen =
      AddIconTexture("sprites/bomb_carried_ring_offscreen");

  m_hostageFollowing = AddIconTexture("sprites/hostage_following");
  m_hostageFollowingOffscreen =
      AddIconTexture("sprites/hostage_following_offscreen");
...
void CCSMapOverview::Update(void) {
  ...
  if (mp_fadetoblack.GetBool() && team > TEAM_SPECTATOR &&
      !pPlayer->IsAlive()) {
  ...
  if (GetSpectatorMode() == OBS_MODE_CHASE) {
    // Follow the local player in chase cam, so the map rotates using the local
    // player's angles
    SetFollowEntity(pPlayer->entindex());
  }
}
...
//-----------------------------------------------------------------------------
CCSMapOverview::CSMapPlayer_t *
CCSMapOverview::GetCSInfoForPlayerIndex(int index) {
...
//-----------------------------------------------------------------------------
CCSMapOverview::CSMapPlayer_t *
CCSMapOverview::GetCSInfoForPlayer(MapPlayer_t *player) {
  if (player == NULL)
    return NULL;
...
//-----------------------------------------------------------------------------
CCSMapOverview::CSMapPlayer_t *
CCSMapOverview::GetCSInfoForHostage(MapPlayer_t *hostage) {
  if (hostage == NULL)
    return NULL;
...
bool CCSMapOverview::CanPlayerBeSeen(MapPlayer_t *player) {
  ...
  if (GetMode() == MAP_MODE_RADAR) {
    // This level will be for all the RadarMode thinking.  Base class will be
    // the old way for the other modes.
    ...
    if (csPlayer->overrideExpirationTime != -1 &&
        csPlayer->overrideExpirationTime > gpGlobals->curtime)
      return true;
    ...
    // and a living enemy needs to have been seen recently, and have been for a
    // while
...
bool CCSMapOverview::CanHostageBeSeen(MapPlayer_t *hostage) {
  ...
  if (GetMode() == MAP_MODE_RADAR) {
    ...
    // draw special icons if within time
    if (csHostage->overrideExpirationTime != -1 &&
        csHostage->overrideExpirationTime > gpGlobals->curtime)
      return true;
...
CCSMapOverview::CCSMapOverview(const char *pElementName)
...
void CCSMapOverview::UpdatePlayers() {
  ...
  UpdateBomb(); // Before players so player can properly spot where it is in
                // this update
    ...
    if (pCSPR->IsConnected(i)) {
      ...
      if (player->team != pCSPR->GetTeam(i)) {
        ...
        if (player == localMapPlayer)
          player->icon =
              m_TeamIconsSelf[GetIconNumberFromTeamNumber(player->team)];
        else
  ...
  if (GetMode() == MAP_MODE_RADAR) {
    ...
    for (int i = 1; i <= gpGlobals->maxClients; ++i) {
      ...
      if (pCSPR->IsPlayerSpotted(i)) {
        ...
        if (baseEnemy->health > 0) {
          // They were just seen, so if they are alive get rid of their "last
          // known" icon
    ...
    for (int i = 1; i <= gpGlobals->maxClients; i++) {
      ...
      // Now that everyone has had a say on people they can see for us, go
      // through and handle baddies that can no longer be seen.
      if (playerCS->timeLastSeen != now && player->health > 0) {
        // We are not seen now, but if we were seen recently (and for long
        // enough), put up a "last known" icon and clear timelastseen if they
        // are alive.  Death icon is more important, which is why the health
        // check above.
        if (timeSinceLastSeen < 0.5f && (playerCS->timeLastSeen != -1)) {
          if (now - playerCS->timeFirstSeen > TIME_UNTIL_ENEMY_SEEN) {
            playerCS->overrideIcon =
                m_TeamIcons[GetIconNumberFromTeamNumber(player->team)];
            ;
...
void CCSMapOverview::UpdateHostages() {
  ...
  for (int i = 0; i < MAX_HOSTAGES; i++) {
    if (pCSPR->IsHostageAlive(i)) {
      ...
      if (hostage == NULL)
        hostage = &m_Hostages[i]; // Don't have entry yet, so need one.  This'll
                                  // only happen once, at start of map
      ...
      if (!hostageCS->isDead) {
        ...
        hostage->health =
            100; // Hostages don't have health available from pCSPR.
        ...
        //				engine->Con_NPrintf( i + 15, "ID:%d
        //Pos:(%.0f,%.0f,%.0f)", hostage->index, hostage->position.x,
        //hostage->position.y, hostage->position.z );
      } else {
        //				engine->Con_NPrintf( i + 15, "Mostly
        //Dead" );
      }
    } else {
...
void CCSMapOverview::UpdateBomb() {
  ...
  // First, decide if it has been too long since the bomb has been seen to clear
  // visibility timers.
  if (now - m_bomb.timeLastSeen >= TIME_SPOTS_STAY_SEEN &&
  ...
  if (g_PlantedC4s.Count() > 0) {
    ...
    if (pos.x != 0 || pos.y != 0 || pos.z != 0) {
      ...
      // Not a bomb map.  Man, what a weird system instead of IsBombMap.  If
      // nobody has it, and it isn't on the ground, then it isn't a bomb map.
    }
  } else {
    for (int i = 1; i <= gpGlobals->maxClients; i++) {
      if (pCSPR->HasC4(i)) {
        ...
        if (pPlayer == NULL || pPlayer->IsDormant()) {
          // Dormant or no player means we are relying on RadarUpdate messages
          // so we can trust the MapOverview position.
          MapPlayer_t *player = &m_Players[i - 1];
          m_bomb.position = player->position;
        } else {
          // Update players is about to put this Real Data in the player sturct,
          // and we don't want the bomb pos lagged one update behind
  ...
  if (m_bomb.currentRingRadius == m_bomb.maxRingRadius ||
    ...
    m_bomb.currentRingRadius += (m_bomb.maxRingRadius - m_flIconSize) *
                                gpGlobals->frametime / m_bomb.ringTravelTime;
    m_bomb.currentRingRadius =
        MIN(m_bomb.currentRingRadius, m_bomb.maxRingRadius);
    m_bomb.currentRingAlpha =
        (alpha - 55) * ((m_bomb.maxRingRadius - m_bomb.currentRingRadius) /
                        (m_bomb.maxRingRadius - m_flIconSize)) +
        55;
  }
}
...
CCSMapOverview::MapPlayer_t *
CCSMapOverview::GetHostageByEntityID(int entityID) {
  for (int i = 0; i < MAX_HOSTAGES; i++) {
...
bool CCSMapOverview::AdjustPointToPanel(Vector2D *pos) {
  ...
  int mapInset = GetBorderSize(); // This gives us the amount inside the panel
                                  // that the background is drawing.
  if (mapInset != 0)
    mapInset += BORDER_WIDTH; // And this gives us the border inside the map
                              // edge to give us room for offscreen icons.

  int x, y, w, t;

  // MapTpPanel has already offset the x and y.  That's why we use 0 for left
  // and top.
  GetBounds(x, y, w, t);
...
void CCSMapOverview::DrawMapTexture() {
  ...
  if (textureIDToUse > 0) {
    ...
    Vector2D textureTL =
        PanelToMap(panelTL); // The top left corner of the display is where on
                             // the master map?
    ...
    Vertex_t points[4] = {
        // To draw a textured polygon, the first column is where you want to
        // draw (to), and the second is what you want to draw (from). We want to
        // draw to the panel (pulled in for a border), and we want to draw the
        // part of the map texture that should be seen. First column is in panel
        // coords, second column is in 0-1 texture coords
        Vertex_t(panelTL, textureTL), Vertex_t(panelTR, textureTR),
        Vertex_t(panelBR, textureBR), Vertex_t(panelBL, textureBL)};
  ...
  // If we didn't find the greenscale version of the map, then at least do a
  // tint.
  if (!foundRadarVersion && GetMode() == MAP_MODE_RADAR) {
    surface()->DrawSetColor(0, 255, 0, alpha / 4);
    surface()->DrawFilledRect(mapInset, mapInset, m_vSize.x - 1 - mapInset,
                              m_vSize.y - 1 - mapInset);
  }
}
...
void CCSMapOverview::DrawBomb() {
  ...
  if (m_bomb.timeGone != -1 && m_bomb.timeFade <= gpGlobals->curtime)
    alpha *= 1 - ((float)(gpGlobals->curtime - m_bomb.timeFade) /
                  (float)(m_bomb.timeGone - m_bomb.timeFade));

  if (m_bomb.state != CSMapBomb_t::BOMB_GONE)
    DrawIconCS(bombRing, bombRingOffscreen, m_bomb.position,
               m_bomb.currentRingRadius, 0, m_bomb.currentRingAlpha);
  DrawIconCS(bombIcon, bombIcon, m_bomb.position, m_flIconSize, 0, alpha);
}

bool CCSMapOverview::DrawIconCS(int textureID, int offscreenTextureID,
                                Vector pos, float scale, float angle, int alpha,
                                bool allowRotation, const char *text,
  ...
  if (AdjustPointToPanel(&pospanel)) {
    ...
    Vector adjustment3D(adjustment.x, -adjustment.y,
                        0); // Y gets flipped in WorldToMap
  ...
  Vertex_t points[4] = {
      Vertex_t(pos1Panel, Vector2D(0, 0)), Vertex_t(pos2Panel, Vector2D(1, 0)),
...
void CCSMapOverview::DrawMapPlayers() {
  ...
  for (int i = 0; i < MAX_PLAYERS; i++) {
    ...
    // Now draw them
    if (playerCS->overrideExpirationTime >
        gpGlobals->curtime) // If dead, an X, if alive, an alpha'd normal icon
    {
      int alphaToUse = alpha;
      if (playerCS->overrideFadeTime != -1 &&
          playerCS->overrideFadeTime <= gpGlobals->curtime) {
        // Fade linearly from fade start to disappear
        alphaToUse *=
            1 - (float)(gpGlobals->curtime - playerCS->overrideFadeTime) /
                    (float)(playerCS->overrideExpirationTime -
                            playerCS->overrideFadeTime);
      }

      DrawIconCS(playerCS->overrideIcon, playerCS->overrideIconOffscreen,
                 playerCS->overridePosition, m_flIconSize * 1.1f,
                 GetViewAngle(),
                 player->health > 0 ? alphaToUse / 2 : alphaToUse, true, name,
                 &player->color, -1, &colorGreen);
      if (player->health > 0)
        DrawIconCS(m_playerFacing, -1, playerCS->overridePosition,
                   m_flIconSize * 1.1f, playerCS->overrideAngle[YAW],
                   player->health > 0 ? alphaToUse / 2 : alphaToUse, true, name,
      ...
      if (localPlayer) {
        if ((localPlayer->GetObserverMode() != OBS_MODE_NONE) &&
          ...
          zDifference = player->position.z -
                        localPlayer->GetObserverTarget()->GetAbsOrigin().z;
        else
      ...
      float sizeForPlayer =
          m_flIconSize *
          1.1f; // The 1.1 is because the player dots are shrunken a little, so
      ...
      bool showTalkRing =
          localPlayer && (localPlayer->GetTeamNumber() == player->team ||
                          localPlayer->GetTeamNumber() == TEAM_SPECTATOR);

      if (showTalkRing && playerCS->currentFlashAlpha > 0) // Flash type
      {
        // Make them flash a halo
        DrawIconCS(m_radioFlash, m_radioFlashOffscreen, player->position,
                   sizeForRing, player->angle[YAW],
                   playerCS->currentFlashAlpha);
      } else if (showTalkRing && pCSPR->IsAlive(i + 1) &&
                 GetClientVoiceMgr()->IsPlayerSpeaking(i +
                                                       1)) // Or solid on type
      {
        // Make them show a halo
        DrawIconCS(m_radioFlash, m_radioFlashOffscreen, player->position,
                   sizeForRing, player->angle[YAW], 255);
      }

      bool doingLocalPlayer =
          GetPlayerByUserID(localPlayer->GetUserID()) == player;
      float angleForPlayer = GetViewAngle();

      if (doingLocalPlayer) {
        sizeForPlayer *= 4.0f; // The self icon is really big since it has a
                               // camera view cone attached.
        angleForPlayer =
            player->angle[YAW]; // And, the self icon now rotates, natch.
      }

      int offscreenIcon =
          m_TeamIconsOffscreen[GetIconNumberFromTeamNumber(player->team)];
      DrawIconCS(player->icon, offscreenIcon, player->position, sizeForPlayer,
                 angleForPlayer, alpha, true, name, &player->color, status,
                 &colorGreen);
      if (!doingLocalPlayer) {
        // Draw the facing for everyone but the local player.
        if (player->health > 0)
          DrawIconCS(m_playerFacing, -1, player->position, sizeForPlayer,
                     player->angle[YAW], alpha, true, name, &player->color,
                     status, &colorGreen);
      }
    }
  }
...
void CCSMapOverview::DrawHostages() {
  ...
  for (int i = 0; i < MAX_HOSTAGES; i++) {
    ...
    if (hostageCS->overrideExpirationTime >
        gpGlobals->curtime) // If dead, an X, if alive, an alpha'd normal icon
    {
      //			engine->Con_NPrintf( i + 30, "ID:%d Override
      //Pos:(%.0f,%.0f,%.0f)", hostage->index, hostageCS->overridePosition.x,
      //hostageCS->overridePosition.y, hostageCS->overridePosition.z );
      int alphaToUse = alpha;
      if (hostageCS->overrideFadeTime != -1 &&
          hostageCS->overrideFadeTime <= gpGlobals->curtime) {
        // Fade linearly from fade start to disappear
        alphaToUse *=
            1 - (float)(gpGlobals->curtime - hostageCS->overrideFadeTime) /
                    (float)(hostageCS->overrideExpirationTime -
                            hostageCS->overrideFadeTime);
      }

      DrawIconCS(hostageCS->overrideIcon, hostageCS->overrideIconOffscreen,
                 hostageCS->overridePosition, m_flIconSize,
                 hostageCS->overrideAngle[YAW],
                 hostage->health > 0 ? alphaToUse / 2 : alphaToUse, true, name,
                 &hostage->color, status, &colorGreen);
    } else {
      if (localPlayer && localPlayer->GetTeamNumber() == hostage->team &&
          hostageCS->currentFlashAlpha > 0) {
        // Make them flash a halo
        DrawIconCS(m_radioFlash, m_radioFlashOffscreen, hostage->position,
                   m_flIconSize * 1.4f, hostage->angle[YAW],
                   hostageCS->currentFlashAlpha);
      }

      //			engine->Con_NPrintf( i + 30, "ID:%d
      //Pos:(%.0f,%.0f,%.0f)", hostage->index, hostage->position.x,
      //hostage->position.y, hostage->position.z );
      ...
      if (localPlayer) {
        if ((localPlayer->GetObserverMode() != OBS_MODE_NONE) &&
            localPlayer->GetObserverTarget())
          zDifference = hostage->position.z -
                        localPlayer->GetObserverTarget()->GetAbsOrigin().z;
        else
      ...
      DrawIconCS(normalIcon, offscreenIcon, hostage->position, sizeForHostage,
                 GetViewAngle(), alpha, true, name, &hostage->color, status,
                 &colorGreen);

      if (pCSPR->IsHostageFollowingSomeone(i)) {
        // If they are following a CT, then give them a little extra symbol to
        // show it.
        DrawIconCS(m_hostageFollowing, m_hostageFollowingOffscreen,
                   hostage->position, sizeForHostage, hostage->angle[YAW],
                   alpha);
      }
    }
  }
...
void CCSMapOverview::SetMap(const char *levelname) {
  ...
  Q_snprintf(radarFileName, MAX_PATH, "%s_radar",
             m_MapKeyValues->GetString("material"));
  m_nRadarMapTextureID = surface()->CreateNewTextureID();
  surface()->DrawSetTextureFile(m_nRadarMapTextureID, radarFileName, true,
                                false);
  int radarWide = -1;
  int radarTall = -1;
  ...
  if (radarWide == wide && radarTall == tall) {
    ...
    // even a ValidTexture check on the ID.  So I can check if it is different
    // from the original.  It'll be a 32x32 default if not present.
...
bool CCSMapOverview::CreateRadarImage(const char *mapName,
                                      const char *radarFileName) {
  ...
  buf.SeekGet(CUtlBuffer::SEEK_HEAD,
              0); // Need to set these explicitly since ->Read goes straight to
                  // memory and skips them.
...
void CCSMapOverview::DrawCamera() {
  ...
  if (localPlayer->GetObserverMode() == OBS_MODE_ROAMING) {
    // Instead of the programmer-art red dot, we'll draw an icon for when our
    // camera is roaming.
    int alpha = 255;
    DrawIconCS(m_cameraIconFree, m_cameraIconFree, localPlayer->GetAbsOrigin(),
    ...
    if (localPlayer->GetObserverTarget()) {
      ...
      int alpha = 255 * 0.5f;
      DrawIconCS(m_cameraIconFirst, m_cameraIconFirst,
                 localPlayer->GetObserverTarget()->GetAbsOrigin(),
                 m_flIconSize * 1.5f, GetViewAngle(), alpha);
    }
  } else if (localPlayer->GetObserverMode() == OBS_MODE_CHASE) {
    if (localPlayer->GetObserverTarget()) {
      // Or Draw the third-camera a little bigger. (Needs room to be off the dot
      // being followed)
      int alpha = 255;
      DrawIconCS(m_cameraIconThird, m_cameraIconThird,
                 localPlayer->GetObserverTarget()->GetAbsOrigin(),
                 m_flIconSize * 3.0f, localPlayer->EyeAngles()[YAW], alpha);
    }
  }
...
void CCSMapOverview::FireGameEvent(IGameEvent *event) {
  ...
  if (Q_strcmp(type, "hostage_killed") == 0) {
    ...
    hostageCS->overrideExpirationTime =
        gpGlobals->curtime + DEATH_ICON_DURATION;
    ...
    hostageCS->overrideFadeTime = gpGlobals->curtime;
    hostageCS->overrideExpirationTime =
        gpGlobals->curtime + HOSTAGE_RESCUE_DURATION;
    ...
    playerCS->isDead = true;
    playerCS->overrideIcon =
        m_TeamIconsDead[GetIconNumberFromTeamNumber(player->team)];
...
void CCSMapOverview::SetMode(int mode) {
  if (mode == MAP_MODE_RADAR) {
    ...
    // We want the _output_ of the radar to be consistant, so we need to take
    // the map scale in to account.
    float desiredZoom = (DESIRED_RADAR_RESOLUTION * m_fMapScale) /
                        (OVERVIEW_MAP_SIZE * m_fFullZoom);

    g_pClientMode->GetViewportAnimationController()->RunAnimationCommand(
        this, "zoom", desiredZoom, 0.0, 0,
    ...
    float desiredZoom =
        (overview_preferred_view_size.GetFloat() * m_fMapScale) /
        (OVERVIEW_MAP_SIZE * m_fFullZoom);

    g_pClientMode->GetViewportAnimationController()->RunAnimationCommand(
        this, "zoom", desiredZoom, 0.0f, 0.2f,
        vgui::AnimationController::INTERPOLATOR_LINEAR);
  } else {
...
void CCSMapOverview::UpdateSizeAndPosition() {
  ...
  switch (m_nMode) {
  case MAP_MODE_RADAR: {
    // To allow custom hud scripts to work, get our size from the HudRadar
    // element that people already tweak.
    int x, y, w, t;
    ...
    m_vSize.x = w;
    m_vSize.y =
        w; // Intentionally not 't'.  We need to enforce square-ness to prevent
           // people from seeing more of the map by fiddling their HudLayout
    break;
  }
  ...
  for (int i = 0; i < MAX_PLAYERS; i++) {
    ...
    if (playerCS->flashUntilTime <= now) {
      ...
      if (playerCS->nextFlashPeakTime <= now) {
        ...
        playerCS->nextFlashPeakTime =
            MIN(playerCS->nextFlashPeakTime, playerCS->flashUntilTime);
      } else {
        // Just fade away
        playerCS->currentFlashAlpha -=
            ((playerCS->currentFlashAlpha * gpGlobals->frametime) /
             (playerCS->nextFlashPeakTime - now));
...
void CCSMapOverview::SetPlayerPreferredMode(int mode) {
  // A player has given an explicit overview_mode command, so we need to honor
  // that when we are done being the radar.
...
void CCSMapOverview::UpdateGoalIcons() {
  // The goal entities don't exist on the client, so we have to get them from
  // the CS Resource.
  C_CS_PlayerResource *pCSPR = (C_CS_PlayerResource *)GameResources();
  if (!pCSPR)
    return;
...
void CCSMapOverview::DrawGoalIcons() {
  for (int iconIndex = 0; iconIndex < m_goalIcons.Count(); iconIndex++) {
    ...
    int alpha = 255;
    DrawIconCS(currentIcon->iconToUse, currentIcon->iconToUse,
               currentIcon->position, m_flIconSize, GetViewAngle(), alpha,
               false);
  }
}
...
int CCSMapOverview::GetMasterAlpha(void) {
  // The master alpha is the alpha that the map wants to draw at.  The
  // background will be at half that, and the icons will always be full.  (The
  // icons fade themselves for functional reasons like seen-recently.)
  int alpha = 255;
  if (GetMode() == MAP_MODE_RADAR)
