...
{
public:
        PresetNameTextEntry(Panel *parent, CBuyPresetEditMainMenu *menu, const
char *name ) : TextEntry( parent, name )
        {
                m_pMenu = menu;
        }
...
public:
  PresetBackgroundPanel(vgui::Panel *parent, const char *panelName)
...
BuyPresetEditPanel::BuyPresetEditPanel(Panel *parent, const char *panelName,
  ...
  m_pCostLabel = NULL;
  /*
  m_pTitleEntry = new PresetNameTextEntry( this,
  dynamic_cast<CBuyPresetEditMainMenu *>(parent), "titleEntry" ); m_pTitleLabel
  = new Label( this, "title", "" ); m_pCostLabel = new Label( this, "cost", ""
  );
  */
...
//--------------------------------------------------------------------------------------------------------------
void BuyPresetEditPanel::SetWeaponSet(const WeaponSet *pWeaponSet,
                                      bool current) {
  // set to empty state
  Reset();
  ...
  if (pWeaponSet) {
    if (m_pTitleLabel) {
      m_pTitleLabel->SetText(
          SharedVarArgs("#Cstrike_BuyPresetChoice%d", m_fallbackIndex));
    }
    if (m_pTitleEntry) {
    ...
    if (m_pCostLabel) {
      ...
      g_pVGuiLocalize->ConstructString(
          ...
          NumAsWString(pWeaponSet->FullCost()));
    ...
    if (pWeaponSet->m_flashbangs)
      m_pFlashbangs->SetItem("gfx/vgui/flashbang_square",
                             pWeaponSet->m_flashbangs);
  ...
  if (stricmp(command, "close")) {
    PostActionSignal(
        new KeyValues("Command", "command",
                      SharedVarArgs("%s %d", command, m_fallbackIndex)));
  }
...
void BuyPresetEditPanel::ApplySchemeSettings(IScheme *pScheme) {
  ...
  const char *buttonNames[4] = {"editPrimary", "editSecondary", "editGrenades",
                                "editEquipment"};
    ...
    if (pPanel) {
      ...
      if (!stricmp("button", pPanel->GetClassName())) {
        ...
        if (pButton) {
          ...
          fgColor =
              GetSchemeColor("Label.TextDullColor", GetFgColor(), pScheme);
...
//--------------------------------------------------------------------------------------------------------------
/**
 *  Overrides EditablePanel's resizing of children to scale them proportionally
 * to the main panel's change.
 */
