...
public:
  ConVarToggleCheckButton(vgui::Panel *parent, const char *panelName,
                          const char *text);
...
//--------------------------------------------------------------------------------------------------------------
ConVarToggleCheckButton::ConVarToggleCheckButton(Panel *parent,
...
//--------------------------------------------------------------------------------------------------------------
//--------------------------------------------------------------------------------------------------------------
CCareerBaseBox::CCareerBaseBox(Panel *parent, const char *panelName,
                               bool loadResources, bool useCareerButtons)
...
vgui::Panel *CCareerBaseBox::CreateControlByName(const char *controlName) {
  if (Q_stricmp(controlName, "ConVarCheckButton") == 0) {
    ConVarToggleCheckButton *button =
        new ConVarToggleCheckButton(NULL, controlName, "");
...
//--------------------------------------------------------------------------------------------------------------
CCareerQueryBox::CCareerQueryBox(vgui::Panel *parent, const char *panelName,
...
//--------------------------------------------------------------------------------------------------------------
CCareerQueryBox::CCareerQueryBox(const char *title, const char *labelText,
                                 const char *panelName, vgui::Panel *parent)
    : CCareerBaseBox(parent, panelName) {}

//--------------------------------------------------------------------------------------------------------------
CCareerQueryBox::CCareerQueryBox(const wchar_t *title, const wchar_t *labelText,
                                 const char *panelName, vgui::Panel *parent)
    : CCareerBaseBox(parent, panelName) {}
...
static CSWeaponID s_secondaryWeapons[NUM_SECONDARY_WEAPONS] = {
    WEAPON_NONE,  WEAPON_USP,  WEAPON_GLOCK,     WEAPON_DEAGLE,
    WEAPON_ELITE, WEAPON_P228, WEAPON_FIVESEVEN,
};
...
static CSWeaponID s_primaryWeapons[NUM_PRIMARY_WEAPONS] = {
    ...
    // Assault Rifles
    CSWeaponID(-WEAPONTYPE_RIFLE), WEAPON_SG552, WEAPON_AUG, WEAPON_AK47,
    WEAPON_M4A1, WEAPON_GALIL, WEAPON_FAMAS,

    // Snipers
    CSWeaponID(-WEAPONTYPE_SNIPER_RIFLE), WEAPON_AWP, WEAPON_SG550,
    WEAPON_G3SG1, WEAPON_SCOUT,

    // SMG
    CSWeaponID(-WEAPONTYPE_SUBMACHINEGUN), WEAPON_P90, WEAPON_UMP45,
    WEAPON_MP5NAVY, WEAPON_MAC10, WEAPON_TMP,
...
public:
  CWeaponButton(BuyPresetListBox *pParent, CSWeaponID weaponID, bool isPrimary)
      : BaseClass(pParent->GetParent(),
                  SharedVarArgs("WeaponButton%d", weaponID), "") {
...
void CWeaponButton::ApplySchemeSettings(IScheme *pScheme) {
  CWeaponButton *current =
      s_current; // save off current so Button::ApplySchemeSettings() can get
                 // the right colors
  s_current = NULL;
  Button::ApplySchemeSettings(pScheme);
  m_selectedBgColor =
      GetSchemeColor("Button.ArmedBgColor", Button::GetBgColor(), pScheme);
  m_selectedFgColor =
      GetSchemeColor("Button.ArmedTextColor", Button::GetFgColor(), pScheme);
  s_current = current;
...
void CWeaponButton::SetWeapon(const BuyPresetWeapon &weapon) {
  ...
  if (m_weapon.GetName()) {
    ...
    if (info) {
      ...
      g_pVGuiLocalize->ConstructString(
          wbuf, sizeof(wbuf),
          g_pVGuiLocalize->Find("#Cstrike_BuyPresetPlainCost"), 1,
          NumAsWString(info->GetWeaponPrice()));
  ...
  m_pImage = scheme()->GetImage(
      ImageFnameFromWeaponID(m_weapon.GetWeaponID(), m_isPrimary), true);
}
...
void CWeaponButton::PerformLayout() {
  ...
  // Position the weapon cost
  {
    ...
    if (textContentTall < textTall) {
      ...
      m_pCostImage->SetPos(oldWide - maxCostWide,
                           (textTall - textContentTall) / 2);
    } else {
...
public:
  WeaponComboBox(CWeaponSelectBox *parent, const char *name, int numEntries,
                 bool editable)
...
//--------------------------------------------------------------------------------------------------------------
//--------------------------------------------------------------------------------------------------------------
CWeaponSelectBox::CWeaponSelectBox(vgui::Panel *parent, WeaponSet *pWeaponSet,
                                   bool isSecondary)
  ...
  if (!m_isSecondary) {
    ...
    if (!IsPrimaryWeapon(weapon->GetWeaponID()) &&
        weapon->GetWeaponID() != WEAPON_NONE) {
    ...
    if (!IsSecondaryWeapon(weapon->GetWeaponID()) &&
        weapon->GetWeaponID() != WEAPON_NONE) {
      BuyPresetWeapon tmp(WEAPON_NONE);
      *weapon = tmp;
    }
  ...
  for (i = 0; i < m_numWeapons; ++i) {
    if (m_weaponIDs[i] < 0) {
      ...
      Label *pLabel =
          new Label(m_pListBox, SharedVarArgs("weaponlabel%d", i), text);
      m_pListBox->AddItem(pLabel, NULL);
    } else {
      CWeaponButton *pWeaponButton =
          new CWeaponButton(m_pListBox, m_weaponIDs[i], !m_isSecondary);
...
void CWeaponSelectBox::PopulateControls() {
  ...
  for (i = maxClips - 1; i >= 0; --i) {
    ...
    g_pVGuiLocalize->ConstructString(buf, sizeof(buf),
                                     g_pVGuiLocalize->Find(clipsOrMore), 1,
                                     NumAsWString(i));
    m_pClips->AddItem(buf, NULL);
    g_pVGuiLocalize->ConstructString(
        buf, sizeof(buf), g_pVGuiLocalize->Find(clips), 1, NumAsWString(i));
    m_pClips->AddItem(buf, NULL);
  }
  ...
  if (m_isSecondary) {
    ...
    if (pPanel) {
      const wchar_t *title =
          g_pVGuiLocalize->Find("#Cstrike_BuyPresetWizardSecondary");
      if (title)
...
void CWeaponSelectBox::UpdateClips() {
  ...
  if (info) {
    ...
    wchar_t buf[BufLen];
    g_pVGuiLocalize->ConstructString(
        buf, sizeof(buf), g_pVGuiLocalize->Find("#Cstrike_BuyPresetsBullets"),
        2, NumAsWString(MIN(maxRounds, numClips * buyClipSize)),
        NumAsWString(maxRounds));
...
void CWeaponSelectBox::ApplySchemeSettings(IScheme *pScheme) {
  ...
  for (int i = 0; i < m_pListBox->GetItemCount(); ++i) {
    if (m_weaponIDs[i] >= 0) {
      CWeaponButton *pButton =
          static_cast<CWeaponButton *>(m_pListBox->GetItemPanel(i));
      ...
      if (pLabel) {
        ...
        pLabel->SetBgColor(
            pScheme->GetColor("Frame.BgColor", Color(0, 0, 0, 255)));
      }
    }
  }
}
...
public:
  EquipmentComboBox(CBaseSelectBox *parent, const char *name, int numEntries,
                    bool editable)
      : ComboBox(parent, name, numEntries, editable) {
    m_pBox = parent;
  }
...
//--------------------------------------------------------------------------------------------------------------
//--------------------------------------------------------------------------------------------------------------
CEquipmentSelectBox::CEquipmentSelectBox(vgui::Panel *parent,
  ...
  m_pHelmet->ActivateItemByRow(
      (m_pWeaponSet->m_armor && m_pWeaponSet->m_helmet));
...
void CEquipmentSelectBox::OnControlChanged() {
  ...
  g_pVGuiLocalize->ConstructString(
      wbuf, sizeof(wbuf), g_pVGuiLocalize->Find("#Cstrike_BuyPresetPlainCost"),
...
void CEquipmentSelectBox::OnCommand(const char *command) {
  if (!stricmp(command, "popup_ok")) {
    ...
    m_pWeaponSet->m_helmet =
        m_pWeaponSet->m_armor && m_pHelmet->GetActiveItem();
