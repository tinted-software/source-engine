...
CLifetimeStatsPage::CLifetimeStatsPage(vgui::Panel *parent, const char *name)
  ...
  m_mapVictoryStatsGroupPanel =
      AddGroup(L"map", "Stats_Button_Victories", L"Map Victories");
...
void CLifetimeStatsPage::ApplySchemeSettings(vgui::IScheme *pScheme) {
  ...
  m_statsList->AddColumnToSection(0, "name", "",
                                  SectionedListPanel::COLUMN_CENTER, 280);
  m_statsList->AddColumnToSection(0, "playerValue", "",
                                  SectionedListPanel::COLUMN_CENTER, 250);
}

void CLifetimeStatsPage::RepopulateStats() {
  m_statsList->RemoveAll();

  const StatsCollection_t &personalLifetimeStats =
  ...
  if (m_mapVictoryStatsGroupPanel->IsGroupActive() ||
      m_allStatsGroupPanel->IsGroupActive()) {
...
int CLifetimeStatsPage::AddSimpleStat(
    int desiredStat, const StatsCollection_t &personalLifetimeStats) {
  ...
  pKeyValues->SetString("playerValue",
                        (personalLifetimeStats[stat.iStatId]) ? buf : "");
...
int CLifetimeStatsPage::AddFavoriteWeaponStat(
    const StatsCollection_t &personalLifetimeStats) {
  ...
  for (CSStatType_t statId = CSSTAT_KILLS_DEAGLE; statId <= CSSTAT_KILLS_M249;
       statId = (CSStatType_t)(statId + 1)) {
  ...
  KeyValues *pKeyValues = new KeyValues("data");
  pKeyValues->SetWString("name", LocalizeTagOrUseDefault("Stats_FavoriteWeapon",
                                                         L"Favorite Weapon"));

  pKeyValues->SetWString("playerValue", TranslateWeaponKillIDToAlias(
...
int CLifetimeStatsPage::AddKillToDeathStat(
    const StatsCollection_t &personalLifetimeStats) {
...
int CLifetimeStatsPage::AddAccuracyStat(
    const StatsCollection_t &personalLifetimeStats) {
