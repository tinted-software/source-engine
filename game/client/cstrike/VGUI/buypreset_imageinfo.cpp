...
//--------------------------------------------------------------------------------------------------------------
WeaponImageInfo::~WeaponImageInfo() { delete m_pAmmoText; }

//--------------------------------------------------------------------------------------------------------------
void WeaponImageInfo::ApplyTextSettings(vgui::IScheme *pScheme,
                                        bool isProportional) {
...
//--------------------------------------------------------------------------------------------------------------
void WeaponImageInfo::SetWeapon(const BuyPresetWeapon *pWeapon, bool isPrimary,
                                bool useCurrentAmmoType) {
  ...
  wchar_t *multiplierString =
      g_pVGuiLocalize->Find("#Cstrike_BuyMenuPresetMultiplier");
  if (!multiplierString)
    multiplierString = L"";
  const int BufLen = 32;
  wchar_t buf[BufLen];
  ...
  if (pWeapon->GetAmmoType() == AMMO_CLIPS) {
    ...
    if (info) {
      ...
      int maxClips =
          (buyClipSize > 0) ? ceil(maxRounds / (float)buyClipSize) : 0;
      numClips = MIN(numClips, maxClips);
      m_weapon.image = scheme()->GetImage(
          ImageFnameFromWeaponID(weaponID, m_isPrimary), true);
      if (numClips == 0) {
        m_ammo.image = NULL;
      ...
      if (numClips > 1) {
        g_pVGuiLocalize->ConstructString(buf, sizeof(buf), multiplierString, 1,
                                         NumAsWString(numClips));
      ...
      if (useCurrentAmmoType) {
        ...
        m_weapon.image = scheme()->GetImage(
            ImageFnameFromWeaponID(currentID, m_isPrimary), true);
...
void WeaponImageInfo::PerformLayout() {
  ...
  m_weapon.FitInBounds(m_left, m_top, m_wide * 0.8, m_tall, m_isCentered,
                       m_weaponScale);
  ...
  m_ammo.FitInBounds(m_left + ammoX, m_top + m_tall - ammoSize, ammoSize,
                     ammoSize, false, m_ammoScale);
  ...
  if (m_isPrimary) {
    if (m_ammoScale < 75) {
      m_pAmmoText->SetPos(m_left + ammoX + ammoSize * 1.25f - w,
                          m_top + m_tall - h);
    } else {
...
//--------------------------------------------------------------------------------------------------------------
//--------------------------------------------------------------------------------------------------------------
WeaponLabel::WeaponLabel(Panel *parent, const char *panelName)
...
//--------------------------------------------------------------------------------------------------------------
ItemImageInfo::~ItemImageInfo() { delete m_pText; }

//--------------------------------------------------------------------------------------------------------------
void ItemImageInfo::ApplyTextSettings(vgui::IScheme *pScheme,
                                      bool isProportional) {
...
//--------------------------------------------------------------------------------------------------------------
//--------------------------------------------------------------------------------------------------------------
EquipmentLabel::EquipmentLabel(Panel *parent, const char *panelName,
...
void ImageInfo::Paint() {
  ...
  image->SetPos(x, y);
  image->Paint();
  image->SetSize(0, 0); // restore image size to content size to not mess up
                        // other places that use the same image
}

//--------------------------------------------------------------------------------------------------------------
void ImageInfo::FitInBounds(int baseX, int baseY, int width, int height,
                            bool center, int scaleAt1024, bool halfHeight) {
  if (!image) {
    x = y = w = h = 0;
    return;
  }
