...
public:
  BuyMouseOverPanelButton(vgui::Panel *parent, const char *panelName,
    ...
    m_pBlackMarketPrice =
        NULL; // new EditablePanel( parent, "BlackMarket_Labels" );
    if (m_pBlackMarketPrice) {
      m_pBlackMarketPrice->LoadControlSettings(
      ...
      m_pBlackMarketPrice->SetAutoResize(GetClassPanel()->GetPinCorner(),
                                         GetClassPanel()->GetAutoResize(), px,
                                         py, rx, ry);
    }
  }
  ...
  void SetPriceState() {
    if (CSGameRules() && CSGameRules()->IsBlackMarket()) {
      ...
      if (GetParent()) {
        Panel *pPanel = dynamic_cast<Panel *>(
            GetParent()->FindChildByName("MarketSticker"));
  ...
  void SetMarketState(void) {
    ...
    if (m_pBlackMarketPrice) {
      Label *pLabel = dynamic_cast<Label *>(
          m_pBlackMarketPrice->FindChildByName("pricelabel"));

      if (pLabel) {
        const int BufLen = 2048;
        wchar_t wbuf[BufLen] = L"";
        const wchar_t *formatStr =
            g_pVGuiLocalize->Find("#Cstrike_MarketPreviousPrice");
        ...
        g_pVGuiLocalize->ConvertANSIToUnicode(strPrice, szPrice,
                                              sizeof(szPrice));
      ...
      pLabel =
          dynamic_cast<Label *>(m_pBlackMarketPrice->FindChildByName("price"));

      if (pLabel) {
        const int BufLen = 2048;
        wchar_t wbuf[BufLen] = L"";
        const wchar_t *formatStr =
            g_pVGuiLocalize->Find("#Cstrike_MarketCurrentPrice");
      ...
      pLabel = dynamic_cast<Label *>(
          m_pBlackMarketPrice->FindChildByName("difference"));

      if (pLabel) {
        const int BufLen = 2048;
        wchar_t wbuf[BufLen] = L"";
        const wchar_t *formatStr =
            g_pVGuiLocalize->Find("#Cstrike_MarketDeltaPrice");
      ...
      ImagePanel *pImage = dynamic_cast<ImagePanel *>(
          m_pBlackMarketPrice->FindChildByName("classimage"));

      if (pImage) {
        ImagePanel *pClassImage = dynamic_cast<ImagePanel *>(
            GetClassPanel()->FindChildByName("classimage"));
  ...
  void SetBargainButton(bool state) { m_bIsBargain = state; }

  void SetCurrentPrice(int iPrice) { m_iPrice = iPrice; }

  void SetPreviousPrice(int iPrice) { m_iPreviousPrice = iPrice; }

  const char *GetBuyCommand(void) { return m_command; }

  virtual void ShowPage() {
    if (g_lastPanel) {
      for (int i = 0; i < g_lastPanel->GetParent()->GetChildCount(); i++) {
        MouseOverPanelButton *buyButton = dynamic_cast<MouseOverPanelButton *>(
            g_lastPanel->GetParent()->GetChild(i));
