...
//--------------------------------------------------------------------------------------------------------------
//--------------------------------------------------------------------------------------------------------------
CCareerButton::CCareerButton(vgui::Panel *parent, const char *buttonName,
                             const char *buttonText, const char *image,
                             bool textFirst)
...
void CCareerButton::Paint() {
  ...
  if (m_textFirst) {
    if (m_image) {
      m_image->SetPos(buttonWide - imageWide - m_imagePad,
  ...
  m_textImage->SetPos(textOffset + (textSpace - textWide) / 2,
...
//--------------------------------------------------------------------------------------------------------------
IBorder *CCareerButton::GetBorder(bool depressed, bool armed, bool selected,
                                  bool keyfocus) {
...
void CCareerButton::ApplySchemeSettings(vgui::IScheme *pScheme) {
  ...
  m_textDisabledColor =
      pScheme->GetColor("Label.DisabledFgColor2", Color(128, 128, 128, 255));
  m_textImage->SetColor(m_textNormalColor);
  if (m_image) {
  ...
  if (IsProportional()) {
    ...
    m_imagePad =
        scheme()->GetProportionalScaledValueEx(GetScheme(), m_imagePad);
  }
