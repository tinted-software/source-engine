...
// icon file.
//-----------------------------------------------------------------------------
bool CSLoadAchievementIconForPage(vgui::ImagePanel *pIconPanel,
...
// icon file.
//-----------------------------------------------------------------------------
bool CSLoadIconForPage(vgui::ImagePanel *pIconPanel, const char *pFilename,
                       const char *pszExt /*= NULL*/) {
  char imagePath[_MAX_PATH];
  Q_strncpy(imagePath, "achievements\\", sizeof(imagePath));
...
//-----------------------------------------------------------------------------
void CSUpdateProgressBarForPage(vgui::EditablePanel *pPanel,
                                CCSBaseAchievement *pAchievement,
...
// TODO: revisit this once other games are rebuilt using the updated
...
int AchivementSortPredicate(CCSBaseAchievement *const *pLeft,
                            CCSBaseAchievement *const *pRight) {
...
// res files.
//-----------------------------------------------------------------------------
CAchievementsPage::CAchievementsPage(vgui::Panel *parent, const char *name)
  ...
  for (int i = 0; i < iCount; ++i) {
    CCSBaseAchievement *pAchievement = dynamic_cast<CCSBaseAchievement *>(
        g_AchievementMgrCS.GetAchievementByIndex(i));
...
CAchievementsPage::~CAchievementsPage() {
  g_AchievementMgrCS.SaveGlobalStateIfDirty(
      false); // check for saving here to store achievements we want pinned to
              // HUD
...
void CAchievementsPage::CreateNewAchievementGroup(int iMinRange,
                                                  int iMaxRange) {
...
//----------------------------------------------------------
void CAchievementsPage::UpdateAchievementList(
    CAchievementsPageGroupPanel *groupPanel) {
  if (!groupPanel)
    return;

  UpdateAchievementList(groupPanel->GetFirstAchievementID(),
                        groupPanel->GetLastAchievementID());
  ...
  for (int i = 0; i < m_pGroupsList->GetItemCount(); i++) {
    CAchievementsPageGroupPanel *pPanel =
        (CAchievementsPageGroupPanel *)m_pGroupsList->GetItemPanel(i);
    if (pPanel) {
...
void CAchievementsPage::UpdateTotalProgressDisplay() {
  ...
  if (iCount > 0) {
    ...
    flCompletion = (((float)nUnlocked) /
                    ((float)g_AchievementMgrCS.GetAchievementCount()));
  }

  char szPercentageText[64];
  V_sprintf_safe(szPercentageText, "%d / %d", nUnlocked,
                 g_AchievementMgrCS.GetAchievementCount());
...
void CAchievementsPage::UpdateAchievementList(int minID, int maxID) {
  ...
  FOR_EACH_VEC(sortedAchivementList, i) {
    ...
    CAchievementsPageItemPanel *pAchievementItemPanel =
        new CAchievementsPageItemPanel(m_pAchievementsList,
...
void CAchievementsPage::UpdateAchievementDialogInfo(void) {
  ...
  int i;
  for (i = 0; i < iCount; i++) {
    CAchievementsPageItemPanel *pPanel =
        (CAchievementsPageItemPanel *)m_pAchievementsList->GetItemPanel(i);
    if (pPanel) {
...
void CAchievementsPage::CreateOrUpdateComboItems(bool bCreate) {
  ...
  for (int i = 0; i < m_iNumAchievementGroups; i++) {
    ...
    int achievementRangeStart =
        (m_AchievementGroups[i].m_iMinRange / 1000) * 1000;
    ...
    if (wzGroupName) {
      ...
      int numAwarded = 0;
      int numTested = 0;
      for (int j = m_AchievementGroups[i].m_iMinRange;
           j < m_AchievementGroups[i].m_iMaxRange; j++) {
    ...
    if (bCreate) {
      ...
      CAchievementsPageGroupPanel *achievementGroupPanel =
          new CAchievementsPageGroupPanel(m_pGroupsList, this,
                                          "AchievementDialogGroupPanel", i);
      achievementGroupPanel->SetGroupInfo(wzGroupTitle,
...
CAchievementsPageItemPanel::CAchievementsPageItemPanel(
    vgui::PanelListPanel *parent, const char *name)
  ...
  m_pHiddenHUDToggleButton =
      new CHiddenHUDToggleButton(this, "HiddenHUDToggle", "");
...
void CAchievementsPageItemPanel::UpdateAchievementInfo(vgui::IScheme *pScheme) {
  if (m_pSourceAchievement && m_pSchemeSettings) {
    //=============================================================================
    // HPE_BEGIN:
    // [dwenger] Get achievement name and description text from the localized
    // file
    //=============================================================================
    ...
    if (m_pSourceAchievement->IsAchieved()) {
      ...
      Color fgColor =
          pScheme->GetColor("Label.TextBrightColor", Color(255, 255, 255, 255));
      ...
      m_pAwardDate->SetFgColor(
          pScheme->GetColor("SteamLightGreen", Color(255, 255, 255, 255)));

      // Assign the award date text
      int year, month, day, hour, minute, second;
      if (m_pSourceAchievement->GetAwardTime(year, month, day, hour, minute,
                                             second)) {
        char dateBuffer[32] = "";
      ...
      Color fgColor = pScheme->GetColor("AchievementsInactiveFG",
      ...
      if (CSGameSupportsAchievementTrackerForPage()) {
        m_pShowOnHUDButton->SetVisible(
            !m_pSourceAchievement->ShouldHideUntilAchieved());
        m_pShowOnHUDButton->SetSelected(
            m_pSourceAchievement->ShouldShowOnHUD());

        m_pHiddenHUDToggleButton->SetVisible(
            !m_pSourceAchievement->ShouldHideUntilAchieved());
      } else {
        m_pShowOnHUDButton->SetVisible(false);
        m_pHiddenHUDToggleButton->SetVisible(false);
...
// the client.
//-----------------------------------------------------------------------------
void CAchievementsPageItemPanel::SetAchievementInfo(
    CCSBaseAchievement *pAchievement) {
  if (!pAchievement) {
    Assert(0);
    return;
...
void CAchievementsPageItemPanel::PreloadResourceFile(void) {
  ...
  g_pPreloadedCSAchievementPageItemLayout->LoadFromFile(g_pFullFileSystem,
                                                        controlResourceName);
  ...
  /*
          ...
          {
                  CCSBaseAchievement* pAchievement =
     dynamic_cast<CCSBaseAchievement*>(g_AchievementMgrCS.GetAchievementByIndex(
     i )); char imagePath[_MAX_PATH];

                  Q_strncpy( imagePath, "achievements\\", sizeof(imagePath) );
                  Q_strncat( imagePath, pAchievement->GetName(),
     sizeof(imagePath), COPY_ALL_CHARACTERS ); Q_strncat( imagePath, "_bw",
     sizeof(imagePath), COPY_ALL_CHARACTERS ); Q_strncat( imagePath, ".vtf",
     sizeof(imagePath), COPY_ALL_CHARACTERS );
...
void CAchievementsPageItemPanel::OnCheckButtonChecked(Panel *panel) {
  if (CSGameSupportsAchievementTrackerForPage() &&
      panel == m_pShowOnHUDButton && m_pSourceAchievement) {
...
CAchievementsPageGroupPanel::CAchievementsPageGroupPanel(
    vgui::PanelListPanel *parent, CAchievementsPage *owner, const char *name,
    int iListItemID)
    : BaseClass(parent, name) {
  m_pParent = parent;
  m_pOwner = owner;
...
//-----------------------------------------------------------------------------
void CAchievementsPageGroupPanel::UpdateAchievementInfo(
  ...
  if (m_pSchemeSettings) {
    ...
    vgui::ImagePanel *pPercentageBar =
        (vgui::ImagePanel *)FindChildByName("GroupPercentageBar");
    vgui::ImagePanel *pPercentageBarBkg =
        (vgui::ImagePanel *)FindChildByName("GroupPercentageBarBackground");
...
void CAchievementsPageGroupPanel::PreloadResourceFile(void) {
  ...
  g_pPreloadedCSAchievementPageGroupLayout->LoadFromFile(g_pFullFileSystem,
                                                         controlResourceName);
}
...
//-----------------------------------------------------------------------------
void CAchievementsPageGroupPanel::SetGroupInfo(const wchar_t *name,
                                               int firstAchievementID,
                                               int lastAchievementID) {
...
CGroupButton::CGroupButton(vgui::Panel *pParent, const char *pName,
                           const char *pText)
...
void CGroupButton::DoClick(void) {
  ...
  CAchievementsPageGroupPanel *pParent =
      static_cast<CAchievementsPageGroupPanel *>(GetParent());

  if (pParent) {
    CAchievementsPage *pAchievementsPage =
        static_cast<CAchievementsPage *>(pParent->GetOwner());
  ...
  if (m_bAchievementsDirty) {
    ...
    while (itemId != m_pAchievementsList->InvalidItemID()) {
      CAchievementsPageItemPanel *pAchievementItem =
          dynamic_cast<CAchievementsPageItemPanel *>(
              m_pAchievementsList->GetItemPanel(itemId));
...
CHiddenHUDToggleButton::CHiddenHUDToggleButton(vgui::Panel *pParent,
                                               const char *pName,
                                               const char *pText)
    : BaseClass(pParent, pName, pText) {}

//-----------------------------------------------------------------------------
// Purpose: Handle the case where the user shift-clicks on an un-awarded
// achievement.
//-----------------------------------------------------------------------------
void CHiddenHUDToggleButton::DoClick(void) {
  if (input()->IsKeyDown(KEY_LSHIFT) || input()->IsKeyDown(KEY_RSHIFT)) {
    // Process when a group button is hit
    CAchievementsPageItemPanel *pParent =
        static_cast<CAchievementsPageItemPanel *>(GetParent());
