...
protected:
  ...
  CBaseStatGroupPanel *AddGroup(const wchar_t *name, const char *title_tag,
                                const wchar_t *def = NULL);
  const wchar_t *TranslateWeaponKillIDToAlias(int statKillID);
  const wchar_t *LocalizeTagOrUseDefault(const char *tag,
                                         const wchar_t *def = NULL);
...
public:
  CBaseStatGroupButton(vgui::Panel *pParent, const char *pName,
                       const char *pText);
...
public:
  CBaseStatGroupPanel(vgui::PanelListPanel *parent, CBaseStatsPage *owner,
...
protected:
  // Loads an icon into a specified image panel, or turns the panel off if no
  // icon was found.
