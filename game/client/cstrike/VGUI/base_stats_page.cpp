...
// res files.
//-----------------------------------------------------------------------------
CBaseStatsPage::CBaseStatsPage(vgui::Panel *parent, const char *name)
    : BaseClass(parent, "CSBaseStatsDialog") {
...
CBaseStatsPage::~CBaseStatsPage() { delete m_statsList; }
...
void CBaseStatsPage::ApplySchemeSettings(vgui::IScheme *pScheme) {
  ...
  // Remove any pre-existing sections and add then fresh (this can happen on a
  // resolution change)
...
void CBaseStatsPage::SetActiveStatGroup(CBaseStatGroupPanel *groupPanel) {
  for (int i = 0; i < m_pGroupsList->GetItemCount(); i++) {
    CBaseStatGroupPanel *pPanel =
        (CBaseStatGroupPanel *)m_pGroupsList->GetItemPanel(i);
    if (pPanel) {
...
const wchar_t *CBaseStatsPage::LocalizeTagOrUseDefault(const char *tag,
                                                       const wchar_t *def) {
...
CBaseStatGroupPanel *CBaseStatsPage::AddGroup(const wchar_t *name,
                                              const char *title_tag,
                                              const wchar_t *def) {
  CBaseStatGroupPanel *newGroup =
      new CBaseStatGroupPanel(m_pGroupsList, this, "StatGroupPanel", 0);
...
CBaseStatGroupPanel::CBaseStatGroupPanel(vgui::PanelListPanel *parent,
                                         CBaseStatsPage *owner,
...
void CBaseStatGroupPanel::PreloadResourceFile(void) {
  ...
  g_pPreloadedCSBaseStatGroupLayout->LoadFromFile(g_pFullFileSystem,
                                                  controlResourceName);
}
...
//-----------------------------------------------------------------------------
void CBaseStatGroupPanel::SetGroupInfo(const wchar_t *name,
                                       const wchar_t *title) {
...
CBaseStatGroupButton::CBaseStatGroupButton(vgui::Panel *pParent,
...
void CBaseStatGroupButton::DoClick(void) {
  ...
  CBaseStatGroupPanel *pParent =
      static_cast<CBaseStatGroupPanel *>(GetParent());

  if (pParent) {
    CBaseStatsPage *pBaseStatsPage =
        static_cast<CBaseStatsPage *>(pParent->GetOwner());
