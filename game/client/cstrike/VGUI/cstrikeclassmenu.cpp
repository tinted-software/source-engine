...
// -----------------------------------------------------------------------------
// // Class image panels. These maintain a list of the class image panels so it
// can render 3D images into them.
// -----------------------------------------------------------------------------
// //
...
void CClassMenu_TER::Update() {
  ...
  if (pLocalPlayer && pLocalPlayer->PlayerClass() >= FIRST_T_CLASS &&
      pLocalPlayer->PlayerClass() <= LAST_T_CLASS) {
...
void CClassMenu_CT::Update() {
  ...
  if (pPlayer && pPlayer->PlayerClass() >= FIRST_CT_CLASS &&
