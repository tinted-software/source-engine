...
public:
  BuyPresetButton(vgui::Panel *parent, const char *buttonName,
  ...
  virtual void SetFgColor(Color c) { BaseClass::SetFgColor(c); }
  ...
  virtual void ApplySchemeSettings(vgui::IScheme *pScheme) {
    ...
    m_unavailableColor =
        pScheme->GetColor("Label.DisabledFgColor2", Color(0, 0, 0, 0));
  }
...
//-----------------------------------------------------------------------------
BuyPresetButton::BuyPresetButton(vgui::Panel *parent, const char *buttonName,
                                 const char *buttonText)
...
//-----------------------------------------------------------------------------
BuyPresetButton::~BuyPresetButton() { ClearImages(); }
...
CCSBuyMenu_CT::CCSBuyMenu_CT(IViewPort *pViewPort)
    : CCSBaseBuyMenu(pViewPort, "BuySubMenu_CT") {
...
CCSBuyMenu_TER::CCSBuyMenu_TER(IViewPort *pViewPort)
    : CCSBaseBuyMenu(pViewPort, "BuySubMenu_TER") {
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CCSBaseBuyMenu::CCSBaseBuyMenu(IViewPort *pViewPort, const char *subPanelName)
    : CBuyMenu(pViewPort) {
  ...
  for (int i = 0; i < NUM_BUY_PRESET_BUTTONS; ++i) {
    m_pBuyPresetButtons[i] = new BuyPresetButton(
        m_pMainMenu, VarArgs("BuyPresetButton%c", 'A' + i), "");
  }
  ...
  // HPE_END
  //=============================================================================
  m_pLoadout = new BuyPresetEditPanel(m_pMainMenu, "loadoutPanel",
                                      "Resource/UI/Loadout.res", 0, false);
#else
  for (int i = 0; i < NUM_BUY_PRESET_BUTTONS; ++i) {
...
void CCSBaseBuyMenu::Paint() {
  ...
  if (m_pMoney && m_lastMoney != account) {
    ...
    if (!formatStr)
      formatStr = L"%s1";
    g_pVGuiLocalize->ConstructString(wbuf, sizeof(wbuf), formatStr, 1,
                                     NumAsWString(m_lastMoney));
  ...
  for (i = 0; i < numPresets; ++i) {
    ...
    if (fullSet) {
      if (fullSet->GetPrimaryWeapon().GetWeaponID() != WEAPON_NONE) {
        imageName = ImageFnameFromWeaponID(
            fullSet->GetPrimaryWeapon().GetWeaponID(), true);
        BuyPresetImage *image =
            new BuyPresetImage(scheme()->GetImage(imageName, true));
        m_pBuyPresetButtons[i]->AddImage(image, 0);
      }
      if (fullSet->GetSecondaryWeapon().GetWeaponID() != WEAPON_NONE) {
        imageName = ImageFnameFromWeaponID(
            fullSet->GetSecondaryWeapon().GetWeaponID(), false);
        BuyPresetImage *image =
            new BuyPresetImage(scheme()->GetImage(imageName, true));
        m_pBuyPresetButtons[i]->AddImage(image, 0);
      }
...
int GetWeeklyBargain(void) {
  ...
  for (int i = 1; i < WEAPON_MAX; i++) {
    ...
    if (info->m_iTeam == TEAM_UNASSIGNED ||
        info->m_iTeam == pPlayer->m_iTeamNum) {
...
void CCSBaseBuyMenu::HandleBlackMarket(void) {
  ...
  if (m_pLoadout) {
    if (CSGameRules()->IsBlackMarket()) {
      ...
      Label *pLabel =
          dynamic_cast<Label *>(m_pMainMenu->FindChildByName("loadoutLabel"));

      if (pLabel) {
        pLabel->SetVisible(false);
      }

      pLabel = dynamic_cast<Label *>(
          m_pBlackMarket->FindChildByName("MarketHeadline"));
      ...
      if (pLabel) {
        ...
        const wchar_t *formatStr =
            g_pVGuiLocalize->Find("#Cstrike_MarketHeadline");
      ...
      pLabel = dynamic_cast<Label *>(
          m_pBlackMarket->FindChildByName("MarketBargain"));

      if (pLabel) {
        const int BufLen = 2048;
        wchar_t wbuf[BufLen] = L"";
        const wchar_t *formatStr =
            g_pVGuiLocalize->Find("#Cstrike_MarketBargain");
      ...
      pLabel = dynamic_cast<Label *>(
          m_pBlackMarket->FindChildByName("MarketStickerPrice"));

      if (pLabel) {
        char wbuf[16];

        Q_snprintf(wbuf, 16, "%d",
                   CSGameRules()->m_pPrices->iCurrentPrice[iBargain]);

        pLabel->SetText(wbuf);
      }

      RichText *pText = dynamic_cast<RichText *>(
          m_pBlackMarket->FindChildByName("MarketDescription"));

      if (pText) {
        char wbuf[2048];
        g_pVGuiLocalize->ConvertUnicodeToANSI(
            g_pVGuiLocalize->Find("#Cstrike_MarketDescription"), wbuf, 2048);

        pText->SetText("");
        pText->InsertPossibleURLString(wbuf, Color(255, 255, 255, 255),
                                       Color(255, 176, 0, 255));
      ...
      pLabel = dynamic_cast<Label *>(
          m_pBlackMarket->FindChildByName("MarketBargainIcon"));
      ...
      Button *pButton = dynamic_cast<Button *>(
          m_pMainMenu->FindChildByName("BargainbuyButton"));
...
//-----------------------------------------------------------------------------
// Purpose: The CS background is painted by image panels, so we should do
// nothing
//-----------------------------------------------------------------------------
...
void CCSBaseBuyMenu::ApplySchemeSettings(vgui::IScheme *pScheme) {
  ...
  if (m_pMainBackground) {
    ...
    m_pMainBackground->SetBgColor(
        GetSchemeColor("Button.BgColor", GetBgColor(), pScheme));
  }
...
// the game, we need to update the enable/disable state of the menu item
// dynamically.
//-----------------------------------------------------------------------------
void CCSBuySubMenu::UpdateVestHelmPrice() {
  ...
  BuyMouseOverPanelButton *pButton = dynamic_cast<BuyMouseOverPanelButton *>(
      FindChildByName("kevlar_helmet", false));
  if (pButton) {
...
void CCSBuySubMenu::OnCommand(const char *command) {
  ...
  if (!strnicmp(command, buyPresetSetString, strlen(buyPresetSetString))) {
    bool invalid = IsWeaponInvalid(GetClientWeaponID(true)) ||
                   IsWeaponInvalid(GetClientWeaponID(false));
    if (invalid) {
      // can't save the favorite because it has an invalid weapon (colt for a T,
      // etc)
      C_CSPlayer *pPlayer = C_CSPlayer::GetLocalCSPlayer();
      if (pPlayer) {
      ...
      if (cl_buy_favorite_nowarn.GetBool()) {
        ...
        CCareerQueryBox *pBox = new CCareerQueryBox(
            this, "SetLoadoutError", "Resource/UI/SetLoadoutError.res");
      ...
      if (cl_buy_favorite_quiet.GetBool()) {
        BaseClass::OnCommand(
            VarArgs("cl_buy_favorite_set %d",
                    atoi(command + strlen(buyPresetSetString))));
      } else {
        CCareerQueryBox *pBox = new CCareerQueryBox(
            this, "SetLoadoutQuery", "Resource/UI/SetLoadoutQuery.res");
...
void CCSBuySubMenu::HandleBlackMarket(void) {
  ...
  for (int i = 0; i < GetChildCount(); i++) {
    BuyMouseOverPanelButton *pButton =
        dynamic_cast<BuyMouseOverPanelButton *>(GetChild(i));
    if (!pButton)
      continue;
    ...
    if (pWeaponName) {
      ...
      if (CSGameRules()->IsBlackMarket() == false) {
        //=============================================================================
        // HPE_BEGIN:
        // [dwenger] Removed to avoid clearing of default price when not in
        // black market mode
        //=============================================================================
