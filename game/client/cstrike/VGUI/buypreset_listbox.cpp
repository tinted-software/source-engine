...
//--------------------------------------------------------------------------------------------------------------
//--------------------------------------------------------------------------------------------------------------
BuyPresetListBox::BuyPresetListBox(vgui::Panel *parent, char const *panelName)
  ...
  if (IsProportional()) {
    ...
    m_iScrollbarSize =
        static_cast<int>(static_cast<float>(SCROLLBAR_SIZE) *
                         (static_cast<float>(sw) / static_cast<float>(width)));
    m_iDefaultHeight =
        static_cast<int>(static_cast<float>(DEFAULT_HEIGHT) *
                         (static_cast<float>(sw) / static_cast<float>(width)));
    m_iPanelBuffer =
        static_cast<int>(static_cast<float>(PANELBUFFER) *
                         (static_cast<float>(sw) / static_cast<float>(width)));
  } else {
...
//--------------------------------------------------------------------------------------------------------------
/**
 *  Adds an item to the end of the listbox.  UserData is assumed to be a pointer
 * that can be freed by the listbox if non-NULL.
 */
int BuyPresetListBox::AddItem(vgui::Panel *panel,
...
void BuyPresetListBox::SwapItems(int index1, int index2) {
  if (index1 < 0 || index2 < 0 || index1 >= m_items.Count() ||
 ...
 */
int BuyPresetListBox::GetItemCount(void) const { return m_items.Count(); }
 ...
 */
auto BuyPresetListBox::GetItemUserData(int index)
 ...
 */
void BuyPresetListBox::SetItemUserData(int index,
                                       IBuyPresetListBoxUserData *userData) {
  if (index < 0 || index >= m_items.Count())
    return;
...
//--------------------------------------------------------------------------------------------------------------
/**
 *  Removes an item from the table (changing the indices of all following
...
void BuyPresetListBox::PerformLayout() {
  ...
  m_vbar->SetButtonPressedScrollValue(
      m_iDefaultHeight); // standard height of labels/buttons etc.
  ...
  if (m_visibleIndex >= 0 && m_visibleIndex < m_items.Count()) {
    ...
    m_items[m_visibleIndex].panel->GetBounds(itemLeft, itemTop, itemRight,
                                             itemBottom);
    itemBottom += itemTop;
    itemRight += itemLeft;
