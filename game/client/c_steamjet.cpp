...
public:
  ...
  bool m_bFaceLeft; // For support of legacy env_steamjet entity, which faced
...
private:
  // Stored the last time it updates the lighting ramp, so it can cache the
  // values.
...
RecvPropFloat(RECVINFO(m_SpreadSpeed), 0), RecvPropFloat(RECVINFO(m_Speed), 0),
    ...
    RecvPropInt(RECVINFO(m_bFaceLeft), 0), RecvPropInt(RECVINFO(m_nType), 0),
  ...
  m_bEmit = true;
  m_bFaceLeft = false;
  m_ParticleEffect.SetAlwaysSimulate(
      false); // Don't simulate outside the PVS or frustum.
...
/*
void CalcFastApproximateRenderBoundsAABB( C_BaseEntity *pEnt, float flBloatSize,
Vector *pMin, Vector *pMax )
{
        ...
        if ( pParent )
        {
                ...
                // Add the maximum of our local render bounds. This is making
the assumption that we can be at any
                // point and at any angle within the parent's world space
bounds. Vector vAddMins, vAddMaxs; pEnt->GetRenderBounds( vAddMins, vAddMaxs );

                flBloatSize += MAX( vAddMins.Length(), vAddMaxs.Length() );
        }
        else
        {
                // Start out with our own render bounds. Since we don't have a
parent, this won't incur any nasty pEnt->GetRenderBoundsWorldspace( *pMin, *pMax
);
        }
...
void C_SteamJet::Update(float fTimeDelta) {
  ...
  if (m_bEmit) {
    ...
    if (nToEmit > 0) {
      ...
      // EVIL: Ideally, we could tell the renderer our OBB, and let it build a
      // big box that encloses the entity with its parent so it doesn't have to
      // setup its parent's bones here.
      ...
      if (m_ParticleEffect.WasDrawnPrevFrame()) {
        while (nToEmit--) {
          ...
          if (SteamJetParticle *pParticle =
                  (SteamJetParticle *)m_ParticleEffect.AddParticle(
...
inline void RenderParticle_ColorSizePerturbNormal(ParticleDraw *pDraw,
                                                  const Vector &pos,
                                                  const Vector &color,
                                                  const float alpha,
                                                  const float size) {
...
void C_SteamJet::RenderParticles(CParticleRenderIterator *pIterator) {
  const SteamJetParticle *pParticle =
      (const SteamJetParticle *)pIterator->GetFirst();
  while (pParticle) {
    // Render.
    Vector tPos;
    ...
    Vector vRampColor =
        m_Ramps[iRamp] + (m_Ramps[iRamp + 1] - m_Ramps[iRamp]) * fraction;
    ...
    float sinLifetime =
        sin(pParticle->m_Lifetime * 3.14159f / pParticle->m_DieTime);
    ...
    if (m_nType == STEAM_HEATWAVE) {
      ...
      RenderParticle_ColorSizeAngle(
          pIterator->GetParticleDraw(), tPos, vRampColor,
          sinLifetime * (m_clrRender->a / 255.0f),
          FLerp(pParticle->m_uchStartSize, pParticle->m_uchEndSize,
                pParticle->m_Lifetime),
          pParticle->m_flRoll);
    }
...
void C_SteamJet::SimulateParticles(CParticleSimulateIterator *pIterator) {
  ...
  // This fixes the cases where looking away from a steam jet and then looking
  // back would cause a break on the stream.
