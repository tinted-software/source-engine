...
ConVar cl_rumblescale("cl_rumblescale", "1.0",
                      FCVAR_ARCHIVE | FCVAR_ARCHIVE_XBOX,
                      "Scale sensitivity of rumble effects (0 to 1.0)");
ConVar cl_debugrumble("cl_debugrumble", "0", FCVAR_ARCHIVE,
...
typedef struct {
  ...
  int waveformIndex; // Type of effect waveform used (an enum from
                     // rumble_shared.h)
  int priority;      // How important this effect is (for making replacement
                     // decisions)
  bool in_use; // Is this channel in use?? (true if effect is currently playing,
               // false if done or otherwise available)
  unsigned char rumbleFlags; // Flags pertaining to the effect currently playing
                             // on this channel.
...
typedef struct tagWaveGenParams {
  ...
  void Set(float c_cycles, float c_amplitudescale, bool c_leftChannel,
           float c_minAmplitude, float c_maxAmplitude) {
    cycles = c_cycles;
  ...
  // CTOR
  tagWaveGenParams(float c_cycles, float c_amplitudescale, bool c_leftChannel,
                   float c_minAmplitude, float c_maxAmplitude) {
    Set(c_cycles, c_amplitudescale, c_leftChannel, c_minAmplitude,
        c_maxAmplitude);
  }
...
//---------------------------------------------------------
//---------------------------------------------------------
void GenerateSawtoothEffect(RumbleWaveform_t *pWaveform,
                            const WaveGenParams_t &params) {
...
//---------------------------------------------------------
//---------------------------------------------------------
void GenerateSquareWaveEffect(RumbleWaveform_t *pWaveform,
                              const WaveGenParams_t &params) {
  int i = 0;
  int j;
...
// samples long.
//---------------------------------------------------------
void GenerateFlatEffect(RumbleWaveform_t *pWaveform,
                        const WaveGenParams_t &params) {
  for (int i = 0; i < NUM_WAVE_SAMPLES; i++) {
    if (params.leftChannel) {
      pWaveform->amplitude_left[i] = params.maxAmplitude;
...
//---------------------------------------------------------
//---------------------------------------------------------
void GenerateSineWaveEffect(RumbleWaveform_t *pWaveform,
                            const WaveGenParams_t &params) {
...
public:
  ...
  void StartEffect(unsigned char effectIndex, unsigned char rumbleData,
                   unsigned char rumbleFlags);
  void StopEffect(int effectIndex);
  void StopAllEffects();
  void ComputeAmplitudes(RumbleChannel_t *pChannel, float curtime, float *pLeft,
                         float *pRight);
...
void CRumbleEffects::Init() {
  ...
  // Every effect defaults to this many samples. Call TerminateWaveform() to
  // trim these.
  ...
  GenerateSquareWaveEffect(&m_Waveforms[RUMBLE_PORTAL_PLACEMENT_FAILURE],
                           params);
...
RumbleChannel_t *CRumbleEffects::FindAvailableChannel(int priority) {
  ...
  for (i = 0; i < MAX_RUMBLE_CHANNELS; i++) {
    ...
    if (pChannel->priority < lowestPriority) {
      ...
      if (pBestChannel) {
        // If we already have a channel of the same priority to discard, make
        // sure we discard the oldest.
  ...
  return pBestChannel; // Can still be NULL if we couldn't find a channel to
                       // slam.
}
...
//---------------------------------------------------------
//---------------------------------------------------------
void CRumbleEffects::StartEffect(unsigned char effectIndex,
...
//---------------------------------------------------------
//---------------------------------------------------------
void CRumbleEffects::ComputeAmplitudes(RumbleChannel_t *pChannel, float curtime,
...
//---------------------------------------------------------
//---------------------------------------------------------
void RumbleEffect(unsigned char effectIndex, unsigned char rumbleData,
                  unsigned char rumbleFlags) {
