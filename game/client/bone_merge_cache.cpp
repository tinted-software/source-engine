...
void CBoneMergeCache::UpdateCache() {
  ...
  const studiohdr_t *pTestStudioHDR =
      (pTestHdr ? pTestHdr->GetRenderHdr() : NULL);
  if (pTestFollow != m_pFollow || pTestHdr != m_pFollowHdr ||
      pTestStudioHDR != m_pFollowRenderHdr || pOwnerHdr != m_pOwnerHdr) {
    m_MergedBones.Purge();
    m_BoneMergeBits.Purge();
    ...
    if (pTestFollow && pTestHdr && pOwnerHdr) {
      ...
      for (int i = 0; i < m_pOwnerHdr->numbones(); i++) {
        int parentBoneIndex =
            Studio_BoneIndexByName(m_pFollowHdr, pOwnerBones[i].pszName());
        ...
        if ((m_pFollowHdr->boneFlags(parentBoneIndex) &
             BONE_USED_BY_BONE_MERGE) == 0) {
          m_nFollowBoneSetupMask = BONE_USED_BY_ANYTHING;
          //					Warning("Performance warning:
          //Merge with '%s'. Mark bone '%s' in model '%s' as being used by bone
          //merge in the .qc!\n", 						pOwnerHdr->pszName(), m_pFollowHdr->pBone(
          //parentBoneIndex )->pszName(), m_pFollowHdr->pszName() );
        }
      }
...
void CBoneMergeCache::MergeMatchingBones(int boneMask) {
  ...
  bool bWorked = m_pFollow->SetupBones(NULL, -1, m_nFollowBoneSetupMask,
                                       gpGlobals->curtime);
  // We suspect there's some cases where SetupBones couldn't do its thing, and
  // then this causes Captain Canteen.
  Assert(bWorked);
  if (!bWorked) {
    // Usually this means your parent is invisible or gone or whatever.
    // This routine has no way to tell its caller not to draw itself
    // unfortunately. But we can shrink all the bones down to zero size. But it
    // might still spawn particle systems? :-(
    matrix3x4_t NewBone;
    MatrixScaleByZero(NewBone);
    MatrixSetTranslation(Vector(0.0f, 0.0f, 0.0f), NewBone);
#ifdef STAGING_ONLY
    if (r_captain_canteen_is_angry.GetBool()) {
      // We actually want to see when Captain Canteen happened, and make it
      // really obvious that (a) he was here and (b) this code would have fixed
      // him.
    ...
    for (int i = 0; i < m_MergedBones.Count(); i++) {
      ...
      MatrixCopy(m_pFollow->GetBone(iParentBone),
                 m_pOwner->GetBoneForWrite(iOwnerBone));
    }
  }
}

// copy bones instead of matrices
void CBoneMergeCache::CopyParentToChild(const Vector parentPos[],
                                        const Quaternion parentQ[],
  ...
  for (int i = 0; i < m_MergedBones.Count(); i++) {
    ...
    if (m_pOwnerHdr->boneParent(iOwnerBone) == -1 ||
        m_pFollowHdr->boneParent(iParentBone) == -1)
      continue;
...
void CBoneMergeCache::CopyChildToParent(const Vector childPos[],
                                        const Quaternion childQ[],
                                        Vector parentPos[],
...
bool CBoneMergeCache::GetAimEntOrigin(Vector *pAbsOrigin, QAngle *pAbsAngles) {
  ...
  // We want the abs origin such that if we put the entity there, the first
  // merged bone will be aligned. This way the entity will be culled in the
  // correct position.
  //
  // ie: mEntity * mBoneLocal = mFollowBone
  // so: mEntity = mFollowBone * Inverse( mBoneLocal )
  //
  // Note: the code below doesn't take animation into account. If the attached
  // entity animates all over the place, then this won't get the right results.

  // Get mFollowBone.
  m_pFollow->SetupBones(NULL, -1, m_nFollowBoneSetupMask, gpGlobals->curtime);
  const matrix3x4_t &mFollowBone =
      m_pFollow->GetBone(m_MergedBones[0].m_iParentBone);

  // Get Inverse( mBoneLocal )
  matrix3x4_t mBoneLocal, mBoneLocalInv;
  SetupSingleBoneMatrix(m_pOwnerHdr, m_pOwner->GetSequence(), 0,
                        m_MergedBones[0].m_iMyBone, mBoneLocal);
