...
ConVar zoom_sensitivity_ratio(
    "zoom_sensitivity_ratio", "1.0", 0,
    "Additional mouse sensitivity scale factor applied when FOV is zoomed in.");

CViewRender g_DefaultViewRender;
IViewRender *view =
    NULL; // set in cldll_client_init.cpp if no mod creates their own

#if _DEBUG
bool g_bRenderingCameraView = false;
#endif

// These are the vectors for the "main" view - the one the player is looking
// down. For stereo views, they are the vectors for the middle eye.
...
#ifdef TF_CLIENT_DLL
// 54 degrees approximates a 35mm camera - we determined that this makes the
// viewmodels and motions look the most natural.
ConVar v_viewmodel_fov("viewmodel_fov", "54", FCVAR_ARCHIVE);
#else
ConVar v_viewmodel_fov("viewmodel_fov", "54", FCVAR_CHEAT);
#endif
ConVar mat_viewportscale(
    "mat_viewportscale", "1.0", FCVAR_ARCHIVE,
    "Scale down the main viewport (to reduce GPU impact on CPU profiling)",
    true, (1.0f / 640.0f), true, 1.0f);
ConVar mat_viewportupscale("mat_viewportupscale", "1", FCVAR_ARCHIVE,
...
static ConVar r_mapextents("r_mapextents", MAPEXTENTS_DEFAULT, FCVAR_CHEAT,
                           "Set the max dimension for the map.  This "
...
ConVar gl_clear_randomcolor("gl_clear_randomcolor", "0", FCVAR_CHEAT,
                            "Clear the back buffer to random colors every "
                            "frame. Helps spot open seams in geometry.");

static ConVar r_farz("r_farz", "-1", FCVAR_CHEAT,
                     "Override the far clipping plane. -1 means to use the "
                     "value in env_fog_controller.");
static ConVar cl_demoviewoverride("cl_demoviewoverride", "0", 0,
                                  "Override view during demo playback");

void SoftwareCursorChangedCB(IConVar *pVar, const char *pOldValue,
                             float fOldValue) {
...
static ConVar cl_software_cursor("cl_software_cursor", "0", FCVAR_ARCHIVE,
                                 "Switches the game to use a larger software "
                                 "cursor instead of the normal OS cursor",
...
static void CalcDemoViewOverride(Vector &origin, QAngle &angles) {
  ...
  float speed =
      gpGlobals->absoluteframetime * cl_demoviewoverride.GetFloat() * 320;
...
// Selects the relevant member variable to update. You could do it manually,
// but... We always set up the MONO eye, even when doing stereo, and it's set up
// to be mid-way between the left and right, so if you don't really care about
// L/R (e.g. culling, sound, etc), just use MONO.
...
//-----------------------------------------------------------------------------
const Vector &MainViewOrigin() { return g_vecRenderOrigin; }

const QAngle &MainViewAngles() { return g_vecRenderAngles; }

const Vector &MainViewForward() { return g_vecVForward; }

const Vector &MainViewRight() { return g_vecVRight; }

const Vector &MainViewUp() { return g_vecVUp; }

const VMatrix &MainWorldToViewMatrix() { return g_matCamInverse; }

const Vector &PrevMainViewOrigin() { return g_vecPrevRenderOrigin; }

const QAngle &PrevMainViewAngles() { return g_vecPrevRenderAngles; }
...
bool R_CullSphere(VPlane const *pPlanes, int nPlanes, Vector const *pCenter,
                  float radius) {
...
// Purpose:
//-----------------------------------------------------------------------------
static void StartPitchDrift(void) { view->StartPitchDrift(); }
...
//-----------------------------------------------------------------------------
// Purpose: Moves the client pitch angle towards cl.idealpitch sent by the
// server. If the user is adjusting pitch manually, either with lookup/lookdown,
...
void CViewRender::DriftPitch(void) {
  ...
  if (engine->IsHLTV() || g_pEngineClientReplay->IsPlayingReplayDemo() ||
      (player->GetGroundEntity() == NULL) || engine->IsPlayingDemo())
#else
  if (engine->IsHLTV() || (player->GetGroundEntity() == NULL) ||
...
// This is called by cdll_client_int to setup view model origins. This has to be
// done before simulation so entities can access attachment points on view
// models during simulation.
void CViewRender::OnRenderStart() {
  VPROF_("CViewRender::OnRenderStart", 2, VPROF_BUDGETGROUP_OTHER_UNACCOUNTED,
         false, 0);
  ...
  if (player) {
    ...
    if (gHUD.m_flMouseSensitivityFactor) {
      gHUD.m_flMouseSensitivity =
          sensitivity.GetFloat() * gHUD.m_flMouseSensitivityFactor;
    } else
#endif
    {
      ...
      if (localFOV == iDefaultFOV) {
        ...
        // Set a new sensitivity that is proportional to the change from the FOV
        // default and scaled
        //  by a separate compensating factor
        if (iDefaultFOV == 0) {
          Assert(
              0); // would divide by zero, something is broken with iDefatulFOV
          iDefaultFOV = 1;
        }
...
// Purpose:
//-----------------------------------------------------------------------------
void CViewRender::DisableVis(void) { m_bForceNoVis = true; }
...
// Gets znear + zfar
//-----------------------------------------------------------------------------
float CViewRender::GetZNear() { return VIEW_NEARZ; }
...
void CViewRender::SetUpViews() {
  ...
  if (engine->IsHLTV()) {
    ...
    if (pPlayer) {
      pPlayer->CalcView(view.origin, view.angles, view.zNear, view.zFar,
                        view.fov);

      // If we are looking through another entities eyes, then override the
      // angles/origin for view
  ...
  if (engine->IsPlayingDemo()) {
    if (cl_demoviewoverride.GetFloat() > 0.0f) {
      // Retreive view angles from engine ( could have been set in
      // IN_AdjustAngles above )
  ...
  if (UseVR()) {
    ...
    // This call can go almost anywhere, but it needs to know the player FOV for
    // sniper weapon zoom, etc
    if (flFOVOffset == 0.0f) {
    ...
    HeadtrackMovementMode_t hmmOverrideMode =
        g_pClientMode->ShouldOverrideHeadtrackControl();
    g_ClientVirtualReality.OverrideView(&m_View, &ViewModelOrigin,
                                        &ViewModelAngles, hmmOverrideMode);

    // left and right stereo views should default to being the same as the
    // mono/middle view
    m_ViewLeft = m_View;
    m_ViewRight = m_View;
    ...
    g_ClientVirtualReality.OverrideStereoView(&m_View, &m_ViewLeft,
                                              &m_ViewRight);
  } else {
    // left and right stereo views should default to being the same as the
    // mono/middle view
  ...
  C_BaseAnimating::PopBoneAccess(
      "OnRenderStart->CViewRender::SetUpView"); // pops the (true, false) bone
                                                // access set in OnRenderStart
  C_BaseAnimating::PushAllowBoneAccess(
      true, true,
      "CViewRender::SetUpView->OnRenderEnd"); // pop is in OnRenderEnd()

  // Compute the world->main camera transform
  // This is only done for the main "middle-eye" view, not for the various other
  // views.
  ComputeCameraVariables(view.origin, view.angles, &g_vecVForward, &g_vecVRight,
                         &g_vecVUp, &g_matCamInverse);

  // set up the hearing origin...
  AudioState_t audioState;
  audioState.m_Origin = view.origin;
  audioState.m_Angles = view.angles;
  audioState.m_bIsUnderwater =
      pPlayer && pPlayer->AudioStateIsUnderwater(view.origin);
...
void CViewRender::WriteSaveGameScreenshotOfSize(
  ...
  viewSetup.fov = ScaleFOVByWidthRatio(
      viewSetup.fov, ((float)width / (float)height) / (4.0f / 3.0f));
  ...
  if (bWriteVTF) {
    ...
    if (pVTFTexture->Init(nSrcWidth, nSrcHeight, 1, IMAGE_FORMAT_RGB888, nFlags,
                          1, 1)) {
    ...
    bWriteResult =
        TGAWriter::WriteToBuffer(pSrcImage, buffer, nSrcWidth, nSrcHeight,
  ...
  filesystem->AsyncWrite(szPathedFileName, buffer.Base(), buffer.TellPut(),
                         true);
...
void CViewRender::UpdateReplayScreenshotCache() {
  ...
  // Create a new one
  m_pReplayScreenshotTaker =
      new CReplayScreenshotTaker(this, GetView(STEREO_EYE_MONO));
#endif
}
...
void CViewRender::WriteSaveGameScreenshot(const char *pFilename) {
  WriteSaveGameScreenshotOfSize(pFilename, SAVEGAME_SCREENSHOT_WIDTH,
...
void CViewRender::SetUpOverView() {
  ...
  int size_y =
      1024.0f *
      cl_leveloverview.GetFloat(); // scale factor, 1024 = OVERVIEW_MAP_SIZE
...
void CViewRender::Render(vrect_t *rect) {
  ...
  for (StereoEye_t eEye = GetFirstEye(); eEye <= GetLastEye();
       eEye = (StereoEye_t)(eEye + 1)) {
    ...
    static ConVarRef sv_restrict_aspect_ratio_fov(
        "sv_restrict_aspect_ratio_fov");
    float aspectRatio = engine->GetScreenAspectRatio() * 0.75f; // / (4/3)
    float limitedAspectRatio = aspectRatio;
    if ((sv_restrict_aspect_ratio_fov.GetInt() > 0 &&
         engine->IsWindowedMode() && gpGlobals->maxClients > 1) ||
        sv_restrict_aspect_ratio_fov.GetInt() == 2) {
      limitedAspectRatio =
          MIN(aspectRatio,
              1.85f * 0.75f); // cap out the FOV advantage at a 1.85:1 ratio
                              // (about the widest any legit user should be)
    }
    ...
    case STEREO_EYE_MONO: {
      ...
      view.m_flAspectRatio = (engineAspectRatio > 0.0f)
                                 ? engineAspectRatio
    ...
    case STEREO_EYE_LEFT: {
      g_pSourceVR->GetViewportBounds((ISourceVirtualReality::VREye)(eEye - 1),
    ...
    if (gl_clear_randomcolor.GetBool()) {
      ...
      materials->GetDisplayAdapterInfo(materials->GetCurrentAdapter(),
                                       adapterInfo);

      // On Posix, on ATI, we always clear color if we're antialiasing
      if (adapterInfo.m_VendorID == 0x1002) {
        if (g_pMaterialSystem->GetCurrentConfigForVideoCard().m_nAASamples >
            0) {
          nClearFlags |= VIEW_CLEAR_COLOR;
        }
      }
    ...
    if (eEye == STEREO_EYE_MONO || eEye == STEREO_EYE_LEFT ||
        (g_ClientVirtualReality.ShouldRenderHUDInWorld())) {
  ...
  // (this is not health, ammo, etc. Nor is it pre-game briefing interface stuff
  // - this is the stuff that appears when you hit Esc in-game) In stereo mode
  // this is rendered inside of RenderView so it goes into the render target
