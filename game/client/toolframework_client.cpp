...
bool ToolsEnabled() {
  return g_ToolFrameworkClient.m_pTools &&
         g_ToolFrameworkClient.m_pTools->InToolMode();
}

#endif

IGameSystem *ToolFrameworkClientSystem() { return &g_ToolFrameworkClient; }

bool CToolFrameworkClient::Init() {
  factorylist_t list;
  FactoryList_Retrieve(list);

  m_pTools = (IClientEngineTools *)list.appSystemFactory(
      VCLIENTENGINETOOLS_INTERFACE_VERSION, NULL);
...
//-----------------------------------------------------------------------------
void CToolFrameworkClient::PostToolMessage(HTOOLHANDLE hEntity,
                                           KeyValues *msg) {
  if (m_pTools) {
...
// View manipulation
//-----------------------------------------------------------------------------
void CToolFrameworkClient::AdjustEngineViewport(int &x, int &y, int &width,
                                                int &height) {
  if (m_pTools) {
...
// View manipulation
//-----------------------------------------------------------------------------
bool CToolFrameworkClient::SetupEngineView(Vector &origin, QAngle &angles,
                                           float &fov) {
  if (!m_pTools)
    return false;
...
//-----------------------------------------------------------------------------
CRecordEffectOwner::CRecordEffectOwner(C_BaseEntity *pEntity,
                                       bool bIsViewModel) {
  ...
  for (int i = 0; i < nParams; ++i) {
    ...
    case MATERIAL_VAR_TYPE_VECTOR: {
      ...
      case 2:
        ...
        break;
      case 3:
        V_snprintf(str, sizeof(str), "vector3d: %f %f %f", pVal[0], pVal[1],
                   pVal[2]);
        break;
      case 4:
        V_snprintf(str, sizeof(str), "vector4d: %f %f %f %f", pVal[0], pVal[1],
                   pVal[2], pVal[3]);
        break;
      default:
        Assert(0);
    ...
    case MATERIAL_VAR_TYPE_MATRIX: {
      ...
      V_snprintf(str, sizeof(str),
                 ...
                 pVal[7], pVal[8], pVal[9], pVal[10], pVal[11], pVal[12],
                 pVal[13], pVal[14], pVal[15]);
      msg->SetString(pVarName, str);
    } break;

    case MATERIAL_VAR_TYPE_TEXTURE:
      //			V_snprintf( str, sizeof( str ), "texture: %x",
      //pVar->GetTextureValue() ); 			msg->SetString( pVarName, str );
      break;

    case MATERIAL_VAR_TYPE_MATERIAL:
      //			V_snprintf( str, sizeof( str ), "material: %x",
      //pVar->GetMaterialValue() ); 			msg->SetString( pVarName, str );
      break;

    case MATERIAL_VAR_TYPE_UNDEFINED:
      //			Assert( 0 ); // these appear to be (mostly?
      //all?) textures, although I don't know why they're not caught by the
      //texture case above...
      break; // JDTODO
