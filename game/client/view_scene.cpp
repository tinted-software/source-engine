...
ConVar r_depthoverlay("r_depthoverlay", "0", FCVAR_CHEAT,
                      "Replaces opaque objects with their grayscaled depth "
...
public:
  void LevelInitPreEntity() {
    // EP1 core room needs many refract updates per frame to avoid looking
    // broken (ep1_citadel_03) Same with Kleiner's lab (d1_trainstation_05)
    g_bAllowMultipleRefractUpdatesPerScenePerFrame =
        FStrEq(MapName(), "ep1_citadel_03") ||
        FStrEq(MapName(), "d1_trainstation_05");
  }
};
...
//-----------------------------------------------------------------------------
// Purpose: Transforms a world-space position into a 2D position inside a
// supplied frustum.
//-----------------------------------------------------------------------------
int FrustumTransform(const VMatrix &worldToSurface, const Vector &point,
                     Vector &screen) {
  // UNDONE: Clean this up some, handle off-screen vertices
  float w;

  screen.x = worldToSurface[0][0] * point[0] + worldToSurface[0][1] * point[1] +
             worldToSurface[0][2] * point[2] + worldToSurface[0][3];
  screen.y = worldToSurface[1][0] * point[0] + worldToSurface[1][1] * point[1] +
             worldToSurface[1][2] * point[2] + worldToSurface[1][3];
  //	z		 = worldToSurface[2][0] * point[0] +
  //worldToSurface[2][1] * point[1] + worldToSurface[2][2] * point[2] +
  //worldToSurface[2][3];
  w = worldToSurface[3][0] * point[0] + worldToSurface[3][1] * point[1] +
...
int HudTransform(const Vector &point, Vector &screen) {
  if (UseVR()) {
    return FrustumTransform(g_ClientVirtualReality.GetHudProjectionFromWorld(),
                            point, screen);
  } else {
    return FrustumTransform(engine->WorldToScreenMatrix(), point, screen);
  }
...
void UpdateFullScreenDepthTexture(void) {
  ...
  if (r_depthoverlay.GetBool()) {
    ...
    pRenderContext->OverrideDepthEnable(
        true,
        false); // don't write to depth, or else we'll never see translucents
