...
public:
  ...
  float m_flBlackFade; ///< [0.00 .. 1.00] where 1.00 is all black. Locally
                       ///< interpolated.
};

IMPLEMENT_CLIENTCLASS_DT(C_NPC_Vortigaunt, DT_NPC_Vortigaunt, CNPC_Vortigaunt)
RecvPropTime(RECVINFO(m_flBlueEndFadeTime)), RecvPropBool(RECVINFO(m_bIsBlue)),
    RecvPropBool(RECVINFO(m_bIsBlack)),
    END_RECV_TABLE()

#define VORTIGAUNT_BLUE_FADE_TIME                                              \
  2.25f // takes this long to fade from green to blue or back
#define VORT_BLACK_FADE_TIME                                                   \
  2.2f // time to interpolate up or down in fading to black
...
void C_NPC_Vortigaunt::ReceiveMessage(int classID, bf_read &msg) {
  ...
  case VORTFX_ZAPBEAM: {
    ...
    CNewParticleEffect *pEffect = ParticleProp()->Create(
        "vortigaunt_beam", PATTACH_POINT_FOLLOW, nAttachment);
    if (pEffect) {
  ...
  case VORTFX_ARMBEAM: {
    ...
    C_BaseEntity *pEnt = ClientEntityList().GetBaseEntityFromHandle(
        ClientEntityList().EntIndexToHandle(nIndex));
    ...
    if (pEnt) {
      ...
      CNewParticleEffect *pEffect = pEnt->ParticleProp()->Create(
          "vortigaunt_beam_charge", PATTACH_POINT_FOLLOW, nAttachment);
      if (pEffect) {
...
void RecvProxy_FadeOutDuration(const CRecvProxyData *pData, void *pStruct,
                               void *pOut) {
...
IMPLEMENT_CLIENTCLASS_DT(C_VortigauntChargeToken, DT_VortigauntChargeToken,
...
//-----------------------------------------------------------------------------
void C_VortigauntChargeToken::NotifyShouldTransmit(
    ShouldTransmitState_t state) {
  ...
  if (state == SHOULDTRANSMIT_START) {
    m_hEffect = ParticleProp()->Create("vortigaunt_charge_token",
...
void RecvProxy_DispelFadeOutDuration(const CRecvProxyData *pData, void *pStruct,
                                     void *pOut) {
...
IMPLEMENT_CLIENTCLASS_DT(C_VortigauntEffectDispel, DT_VortigauntEffectDispel,
...
// Purpose:
//-----------------------------------------------------------------------------
void C_VortigauntEffectDispel::NotifyShouldTransmit(
    ShouldTransmitState_t state) {
  BaseClass::NotifyShouldTransmit(state);
  ...
  // Turn on
  if (state == SHOULDTRANSMIT_START) {
    m_hEffect = ParticleProp()->Create("vortigaunt_hand_glow",
                                       PATTACH_ABSORIGIN_FOLLOW);
    m_hEffect->SetControlPointEntity(0, this);
  }
}
...
void DispelCallback(const CEffectData &data) {
  ...
  trace_t tr;
  UTIL_TraceLine(startPos, endPos, MASK_SOLID_BRUSHONLY, NULL,
                 COLLISION_GROUP_NONE, &tr);

  if (tr.fraction < 1.0f) {
    // Add a ripple quad to the surface
    FX_AddQuad(tr.endpos + (tr.plane.normal * 8.0f), Vector(0, 0, 1), 64.0f,
               600.0f, 0.8f,
               1.0f, // start alpha
               0.0f, // end alpha
               0.3f, random->RandomFloat(0, 360), 0.0f,
               Vector(0.5f, 1.0f, 0.5f), 0.75f, "effects/ar2_altfire1b",
               (FXQUAD_BIAS_SCALE | FXQUAD_BIAS_ALPHA | FXQUAD_COLOR_FADE));

    // Add a ripple quad to the surface
    FX_AddQuad(tr.endpos + (tr.plane.normal * 8.0f), Vector(0, 0, 1), 16.0f,
               300.0f, 0.9f,
               1.0f, // start alpha
               0.0f, // end alpha
               0.9f, random->RandomFloat(0, 360), 0.0f,
               Vector(0.5f, 1.0f, 0.5f), 1.25f, "effects/rollerglow",
...
bool CVortEmissiveProxy::Init(IMaterial *pMaterial, KeyValues *pKeyValues) {
  ...
  bool bFound;
  m_pMatEmissiveStrength =
      pMaterial->FindVar("$emissiveblendstrength", &bFound);

  if (bFound) {
    // Optional
    bool bFound2;
    m_pMatDetailBlendStrength =
        pMaterial->FindVar("$detailblendfactor", &bFound2);
  }
...
void CVortEmissiveProxy::OnBind(C_BaseEntity *pEnt) {
  ...
  if (pVort) {
    ...
    if (gpGlobals->curtime < pVort->m_flBlueEndFadeTime) {
      ...
      float fadeRatio = (pVort->m_flBlueEndFadeTime - gpGlobals->curtime) /
                        VORTIGAUNT_BLUE_FADE_TIME;
      if (pVort->m_bIsBlue) {
    ...
    // ALEX VLACHOS:
    // The following variable varies on [0 .. 1]. 0.0 means the vort wants to be
    // his normal color. 1.0 means he wants to be all black. It is interpolated
    // in the C_NPC_Vortigaunt::ClientThink() function.
    //
    // pVort->m_flBlackFade
  } else { // if you bind this proxy to anything non-vort (eg a ragdoll) it's
           // always green
    flBlendValue = 0.0f;
  }
  ...
  // Clamp 0-1
  flBlendValue = ( flBlendValue < 0.0f ) ? 0.0f : ( flBlendValue > 1.0f ) ? 1.0f
  : flBlendValue;
  */
...
EXPOSE_INTERFACE(CVortEmissiveProxy, IMaterialProxy,
                 "VortEmissive" IMATERIAL_PROXY_INTERFACE_VERSION);
