...
// FIXME: Move out
extern void DrawSpriteTangentSpace(const Vector &vecOrigin, float flWidth,
...
bool C_HopwireExplosion::SetupEmitters(void) {
  ...
  if (m_pAttractorEmitter.IsValid() == false) {
    m_pAttractorEmitter =
        CParticleAttractor::Create(GetRenderOrigin(), "hopwireattractor");
...
void C_HopwireExplosion::AddParticles(void) {
  ...
  while (m_ParticleTimer.NextEvent(tempDelta)) {
    ...
    sParticle = (SimpleParticle *)m_pAttractorEmitter->AddParticle(
        sizeof(SimpleParticle), g_Mat_Fleck_Cement[0], offset);
    ...
    for (int i = 0; i < numRingSprites; i++) {
      ...
      UTIL_TraceLine(GetRenderOrigin(),
                     GetRenderOrigin() + (Vector(0, 0, -1024)),
      ...
      if (sParticle != NULL) {
        ...
        debugoverlay->AddLineOverlay(m_vecOrigin,
                                     m_vecOrigin + sParticle->m_vecVelocity,
                                     255, 0, 0, false, 3);
#endif
...
// Input  : *pOwner -
//-----------------------------------------------------------------------------
void C_HopwireExplosion::SetOwner(C_BaseEntity *pOwner) { m_hOwner = pOwner; }
...
int C_HopwireExplosion::DrawModel(int flags) {
  ...
  DrawSpriteTangentSpace(GetRenderOrigin() + (CurrentViewForward() * 128.0f),
                         scale + scaleX, scale + scaleY, white);
...
private:
  C_HopwireExplosion
      m_ExplosionEffect; // Explosion effect information and drawing
};
