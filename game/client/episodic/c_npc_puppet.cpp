...
// Purpose:
//-----------------------------------------------------------------------------
void C_NPC_Puppet::AccumulateLayers(IBoneSetup &boneSetup, Vector pos[],
                                    Quaternion q[], float currentTime) {
  if (m_hAnimationTarget == NULL)
    return;
    ...
    if (i < pTarget->m_AnimOverlay.Count()) {
      ...
      if (fWeight > 0) {
        ...
        if (nSequence >= 0) {
          ...
          boneSetup.AccumulatePose(pos, q, nSequence, fCycle, fWeight,
                                   currentTime, NULL);

#if _DEBUG
          if (Q_stristr(boneSetup.GetStudioHdr()->pszName(),
                        r_sequence_debug.GetString()) != NULL) {
            DevMsgRT("%6.2f : %30s : %5.3f : %4.2f : %1d\n", currentTime,
                     boneSetup.GetStudioHdr()->pSeqdesc(nSequence).pszLabel(),
                     fCycle, fWeight, i);
          }
#endif
        }
