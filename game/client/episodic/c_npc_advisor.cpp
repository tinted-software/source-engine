...
inline C_BaseEntity *IndexToEntity(int eindex) {
  return ClientEntityList().GetBaseEntityFromHandle(
      ClientEntityList().EntIndexToHandle(eindex));
}

#define ADVISOR_ELIGHT_CVARS                                                   \
  1 // enable/disable tuning advisor elight with console variables
...
private:
  ...
  */
  ...
  int m_ElightKey; // test using an elight to make the escape sequence more
                   // visible. 0 is invalid.
};
...
void C_NPC_Advisor::StartBeamFX(C_BaseEntity *pOnEntity) {
  ...
  CNewParticleEffect *pEffect =
      ParticleProp()->Create("Advisor_Psychic_Beam", PATTACH_ABSORIGIN_FOLLOW);

  Assert(pEffect);
  if (!pEffect)
    return;

  ParticleProp()->AddControlPoint(pEffect, 1, pOnEntity,
...
void C_NPC_Advisor::StartElight() {
  AssertMsg(m_ElightKey == 0,
            "Advisor trying to create new elight on top of old one!");
  if (m_ElightKey != 0) {
