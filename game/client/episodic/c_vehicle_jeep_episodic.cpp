...
IMPLEMENT_CLIENTCLASS_DT(C_PropJeepEpisodic, DT_CPropJeepEpisodic,
                         CPropJeepEpisodic)
...
RecvPropInt(RECVINFO(m_iNumRadarContacts)),
    ...
    RecvPropArray(RecvPropVector(RECVINFO(m_vecRadarContactPos[0])),
                  m_vecRadarContactPos),

    // CNetworkArray( int, m_iRadarContactType, RADAR_MAX_CONTACTS );
    RecvPropArray(RecvPropInt(RECVINFO(m_iRadarContactType[0])),
                  m_iRadarContactType),
  ...
  // Sometimes we update more quickly when we need to track something in high
  // resolution. Usually we do not, so default to false.
  ...
  for (int i = 0; i < g_pJalopy->m_iNumRadarContacts; i++) {
    GetHudRadar()->AddRadarContact(g_pJalopy->m_vecRadarContactPos[i],
                                   g_pJalopy->m_iRadarContactType[i],
                                   flContactTimeToLive);
  }
}
...
void C_PropJeepEpisodic::Simulate(void) {
  ...
  if (GetHudRadar() != NULL) {
    // This is not our ideal long-term solution. This will only work if you only
    // have one jalopy in a given level. The Jalopy and the Radar Screen are
    // currently both assumed to be singletons. This is appropriate for EP2,
    // however. (sjb)
