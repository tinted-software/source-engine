...
static ConVar
    particle_simulateoverflow("particle_simulateoverflow", "0", FCVAR_CHEAT,
                              "Used for stress-testing particle systems. "
                              "Randomly denies creation of particles.");
ConVar cl_particleeffect_aabb_buffer(
    "cl_particleeffect_aabb_buffer", "2", FCVAR_CHEAT,
...
ConVar cl_particle_show_bbox_cost(
    ...
    "Show # of particles: green->blue->red. Use a negative number to show ALL "
    "particles even cheap ones");
...
static ConCommand cl_particle_stats_start(
    "cl_particle_stats_start", StatsParticlesStart,
    "Start or restart particle stats - also dumps to particle_stats.csv");
static ConCommand
    cl_particle_stats_stop("cl_particle_stats_stop", StatsParticlesStop,
                           "Stop particle stats, or snapshot this frame - also "
                           "dumps to particle_stats.csv");
static ConVar cl_particle_stats_trigger_count(
    "cl_particle_stats_trigger_count", "0", 0,
...
int CParticleEffectBinding::DrawModel(int flags) {
  ...
  if (!IsRetail()) {
    ...
    if (g_cl_particle_show_bbox || (g_cl_particle_show_bbox_cost != 0)) {
      ...
      if (nParticlesShowBboxCost > 0) {
        float fAmount =
            (float)m_nActiveParticles / (float)nParticlesShowBboxCost;
        if (fAmount < 0.5f) {
          if (bShowCheapSystems) {
            ...
            // Prevent the screen getting spammed with low-count particles which
      ...
      if (bDraw) {
        debugoverlay->AddBoxOverlay(center, mins, maxs, QAngle(0, 0, 0), r, g,
                                    b, 16, 0);
        debugoverlay->AddTextOverlayRGB(center, 0, 0, r, g, b, 64, "%s:(%d)",
...
PMaterialHandle
CParticleEffectBinding::FindOrAddMaterial(const char *pMaterialName) {
  if (!m_pParticleMgr) {
    return NULL;
  }
...
Particle *CParticleEffectBinding::AddParticle(int sizeInBytes,
  ...
  m_pParticleMgr->RepairPMaterial(
      hMaterial); // HACKHACK: Remove this when we can stop leaking handles from
                  // level to level.
...
void CParticleEffectBinding::SetDrawThruLeafSystem(int bDraw) {
  // NOTE (2012/11/27, TomF) - this whole system seems to be deprecated -
  // nothing ever checks these flags, and
...
int CParticleEffectBinding::DrawMaterialParticles(bool bBucketSort,
  ...
  if (m_nActiveParticles > MAX_TOTAL_PARTICLES)
    Error("CParticleEffectBinding::DrawMaterialParticles: too many particles "
...
CEffectMaterial *
CParticleEffectBinding::GetEffectMaterial(CParticleSubTexture *pSubTexture) {
  // Hash the IMaterial pointer.
  unsigned int index =
      (((uintp)pSubTexture->m_pGroup) >> 6) % EFFECT_MATERIAL_HASH_SIZE;
...
bool CParticleMgr::Init(unsigned long count, IMaterialSystem *pMaterials) {
  ...
  if (IsX360()) {
    ...
    startParams.iAffinityTable[1] = CommandLine()->FindParm("-swapcores")
                                        ? XBOX_PROCESSOR_5
                                        : XBOX_PROCESSOR_3;
...
void CParticleMgr::DrawBeforeViewModelEffects() {
  ...
  // CParticleMgr::DrawBeforeViewModelEffects is never called by anything!
  Assert(
      !"Nothing ever calls CParticleMgr::DrawBeforeViewModelEffects any more!");
...
static ConVar
    r_particle_sim_spike_threshold_ms("r_particle_sim_spike_threshold_ms", "5");
...
void CParticleMgr::UpdateNewEffects(float flTimeDelta) {
  ...
  for (int i = 0; i < nCount; i++) {
    ...
    if (nParticleStatsTriggerCount > 0) {
      nParticleActiveParticlesCount +=
          CountParticleSystemActiveParticles(particlesToSimulate[i]);
    }
  }
  ...
  if (nCount) {
    ...
    if (!r_threaded_particles.GetBool()) {
      ...
      if (!m_pThreadPool[1] || nAltCore == 0) {
        ParallelProcess("CParticleMgr::UpdateNewEffects",
                        particlesToSimulate.Base(), nCount, ProcessPSystem);
      } else {
        if (nAltCore > 2) {
          nAltCore = 2;
        }
        CParallelProcessor<CNewParticleEffect *,
            ...
            processor("CParticleMgr::UpdateNewEffects");
  ...
  bool bFrameWarningNeeded =
      g_pParticleSystemMgr->Debug_FrameWarningNeededTestAndReset();

  m_nStatsFramesSinceLastAlert++;
  if ((nParticleStatsTriggerCount > 0 &&
       (nParticleActiveParticlesCount >= nParticleStatsTriggerCount)) ||
      bFrameWarningNeeded) {
    if (m_nStatsFramesSinceLastAlert >=
        (300 * 3)) // 3 seconds at the clamp of 300 fps (or 15 secs at 60fps).
                   // Just stop it spamming too much.
    {
      m_nStatsFramesSinceLastAlert = 0;
      if (m_bStatsRunning) {
      ...
      // And turn the stats gathering on so we'll have more useful results if it
      // does it again.
      m_bStatsRunning = true;

      // This single-frame capture doesn't work that well because the "actual
      // drawn" numbers will just be zero. Ah well - better than nothing.
      StatsReset();
      StatsAccumulateActiveParticleSystems();
      StatsSpewResults();
      StatsReset();
...
CParticleMgr::FindOrAddSubTextureGroup(IMaterial *pPageMaterial) {
...
PMaterialHandle CParticleMgr::GetPMaterial(const char *pMaterialName) {
  ...
  if (hMat == m_SubTextures.InvalidIndex()) {
    ...
    RepairPMaterial(
        m_SubTextures[hMat]); // HACKHACK: Remove this when we can stop leaking
                              // handles from level to level.
...
struct ParticleInfo_t {
  ParticleInfo_t()
      : m_nCount(0), m_nChildCount(0), m_nTotalActiveParticles(0),
        m_nTotalDrawnParticles(0), m_nCountMax(0), m_nChildCountMax(0),
        m_nTotalActiveParticlesMax(0), m_nTotalDrawnParticlesMax(0),
        pDef(NULL) {}
  int m_nCount;
  int m_nChildCount;
...
// These functions will be called by the particles as they're actually drawn.
// (TODO: thread safety?)
void CParticleMgr::StatsNewParticleEffectDrawn(CNewParticleEffect *pParticles) {
#ifdef STAGING_ONLY
  ParticleInfo_t *pParticleInfo =
      &(SingleFrameHistogram[pParticles->GetName()]);
  pParticleInfo->m_nTotalDrawnParticles +=
      CountParticleSystemActiveParticles(pParticles);
#endif
}

void CParticleMgr::StatsOldParticleEffectDrawn(
    CParticleEffectBinding *pParticles) {
#ifdef STAGING_ONLY
  ParticleInfo_t *pParticleInfo =
      &(SingleFrameHistogram[pParticles->m_pSim->GetEffectName()]);
...
void CParticleMgr::StatsAccumulateActiveParticleSystems() {
  ...
  Profiling_nMaxParticles =
      Max(Profiling_nMaxParticles,
          g_pParticleSystemMgr->Debug_GetTotalParticleCount());
  ...
  for (CNewParticleEffect *pNewEffect = m_NewEffects.m_pHead; pNewEffect;
       pNewEffect = pNewEffect->m_pNext) {
    ParticleInfo_t *pParticleInfo =
        &(SingleFrameHistogram[pNewEffect->GetName()]);
    pParticleInfo->m_nCount++;
    pParticleInfo->m_nTotalActiveParticles +=
        CountParticleSystemActiveParticles(pNewEffect);
  ...
  FOR_EACH_LL(m_Effects, i) {
    ...
    pParticleInfo->m_nTotalActiveParticles +=
        pParticleBinding->m_nActiveParticles;
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    pGlobalInfo->m_nTotalActiveParticles +=
        pSingleInfo->m_nTotalActiveParticles;
    pGlobalInfo->m_nTotalDrawnParticles += pSingleInfo->m_nTotalDrawnParticles;
    pGlobalInfo->m_nCountMax =
        Max(pGlobalInfo->m_nCountMax, pSingleInfo->m_nCount);
    pGlobalInfo->m_nChildCountMax =
        Max(pGlobalInfo->m_nChildCountMax, pSingleInfo->m_nChildCount);
    pGlobalInfo->m_nTotalActiveParticlesMax =
        Max(pGlobalInfo->m_nTotalActiveParticlesMax,
            pSingleInfo->m_nTotalActiveParticles);
    pGlobalInfo->m_nTotalDrawnParticlesMax =
        Max(pGlobalInfo->m_nTotalDrawnParticlesMax,
            pSingleInfo->m_nTotalDrawnParticles);
...
void CParticleMgr::StatsSpewResults() {
  ...
  Msg("Active particle systems. Numbers are averages over %d frames. Max num "
      "particles %d.\n",
      Profiling_nFrames, Profiling_nMaxParticles);
  Msg("Name\t\t\t\t\t\tSystems\t\tActive\t\tDrawn\tAv Children\t\tMaximums per "
      "frame\t\t\tMax draw dist\n");
  for (int i = 0; i < nCount; ++i) {
    ...
    if (pParticleInfo->m_nTotalActiveParticles > 0) {
      Msg("%38s\t\t%d\t\t%d\t\t%d\t\t%d\t\t%d\t%d\t%d\t%d\t\t%.1f\n",
          ...
          pParticleInfo->m_nCountMax, pParticleInfo->m_nTotalActiveParticlesMax,
          pParticleInfo->m_nTotalDrawnParticlesMax,
          pParticleInfo->m_nChildCountMax / pParticleInfo->m_nCountMax,
          (pParticleInfo->pDef == NULL)
              ? 0.0f
              : pParticleInfo->pDef->m_flMaxDrawDistance);
    }
  }
  ...
  // Also dump to CSV.
  FileHandle_t fh = g_pFullFileSystem->Open("particle_stats.csv",
                                            "at"); // at = append + text mode
  g_pFullFileSystem->FPrintf(fh, "\nNumframes,%d,Max particles,%d\n",
                             Profiling_nFrames, Profiling_nMaxParticles);
  g_pFullFileSystem->FPrintf(
      fh, "Date,%d,%d,%d,Time,%d,%d,%d\n", CurrentTime.GetYear(),
      CurrentTime.GetMonth() + 1,  // GetMonth() returns 0...11
      CurrentTime.GetDayOfMonth(), // GetDay() returns 1...31
      CurrentTime.GetHour(), CurrentTime.GetMinute(), CurrentTime.GetSecond());
  g_pFullFileSystem->FPrintf(
      fh, "Name, Systems, Particles active, Particles drawn, Av Children, Max "
          "systems, Max particles active, Max particles drawn, Max children, "
          "Max draw distance\n");
  for (int i = 0; i < nCount; ++i) {
    ParticleInfo_t *pParticleInfo = &(ProfilingHistogram[i]);
    if (pParticleInfo->m_nTotalActiveParticles > 0) {
