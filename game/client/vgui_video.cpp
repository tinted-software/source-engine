...
VideoPanel::VideoPanel(unsigned int nXPos, unsigned int nYPos,
    ...
    : BaseClass(NULL, "VideoPanel"), m_VideoMaterial(NULL), m_nPlaybackWidth(0),
      m_nPlaybackHeight(0), m_bAllowAlternateMedia(allowAlternateMedia) {
...
bool VideoPanel::BeginPlayback(const char *pFilename) {
  ...
  if ((videoMode.RefreshRate < 59.0f) &&
      (Q_stricmp(pFilename, "media/ep1_recap.bik") == 0)) {
...
bool VideoPanel_Create(unsigned int nXPos, unsigned int nYPos,
  ...
  VideoPanel *pVideoPanel =
      new VideoPanel(nXPos, nYPos, nHeight, nWidth, false);
  if (pVideoPanel == NULL)
    return false;
