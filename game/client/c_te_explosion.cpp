...
CRagdollExplosionEnumerator::CRagdollExplosionEnumerator(Vector origin,
                                                         float radius,
                                                         float magnitude) {
  m_vecOrigin = origin;
...
// Actual work code
IterationRetval_t
CRagdollExplosionEnumerator::EnumElement(IHandleEntity *pHandleEntity) {
...
CRagdollExplosionEnumerator::~CRagdollExplosionEnumerator() {
  for (int i = 0; i < m_Entities.Count(); i++) {
    ...
    trace_t tr;
    UTIL_TraceLine(m_vecOrigin, position, MASK_SHOT_HULL, NULL,
...
RecvPropInt(RECVINFO(m_nModelIndex)), RecvPropFloat(RECVINFO(m_fScale)),
    RecvPropInt(RECVINFO(m_nFrameRate)), RecvPropInt(RECVINFO(m_nFlags)),
    RecvPropVector(RECVINFO(m_vecNormal)),
    RecvPropInt(RECVINFO(m_chMaterialType)), RecvPropInt(RECVINFO(m_nRadius)),
...
void C_TEExplosion::AffectRagdolls(void) {
  ...
  partition->EnumerateElementsInSphere(PARTITION_CLIENT_RESPONSIVE_EDICTS,
                                       m_vecOrigin, m_nRadius, false,
                                       &ragdollEnum);
}
...
void C_TEExplosion::RecordExplosion() {
  ...
  if (clienttools->IsInRecordingMode()) {
    const model_t *pModel =
        (m_nModelIndex != 0) ? modelinfo->GetModel(m_nModelIndex) : NULL;
...
void C_TEExplosion::PostDataUpdate(DataUpdateType_t updateType) {
  ...
  if (UTIL_PointContents(m_vecOrigin) & CONTENTS_WATER) {
    WaterExplosionEffect().Create(m_vecOrigin, m_nMagnitude, m_fScale,
                                  m_nFlags);
    return;
  }
...
void TE_Explosion(IRecipientFilter &filter, float delay, const Vector *pos,
  ...
  // Major hack to access singleton object for doing this event (simulate
...
void TE_Explosion(IRecipientFilter &filter, float delay,
  ...
  TE_Explosion(filter, 0.0f, &vecOrigin, nModelIndex, flScale, nFrameRate,
               nFlags, nRadius, nMagnitude, &vecNormal,
