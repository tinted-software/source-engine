...
IMPLEMENT_CLIENTCLASS_DT(C_PropVehicleDriveable, DT_PropVehicleDriveable,
                         CPropVehicleDriveable)
RecvPropEHandle(RECVINFO(m_hPlayer)), RecvPropInt(RECVINFO(m_nSpeed)),
    RecvPropInt(RECVINFO(m_nRPM)), RecvPropFloat(RECVINFO(m_flThrottle)),
    RecvPropInt(RECVINFO(m_nBoostTimeLeft)), RecvPropInt(RECVINFO(m_nHasBoost)),
...
//-----------------------------------------------------------------------------
int C_PropVehicleDriveable::GetPassengerRole(
...
ShadowType_t C_PropVehicleDriveable::ShadowCastType() {
  ...
  // Always use render-to-texture. We'll always the dirty bits in our think
  // function
...
void C_PropVehicleDriveable::ClientThink(void) {
  // The vehicle is always dirty owing to pose parameters while it's being
  // driven.
...
// Purpose:
//-----------------------------------------------------------------------------
void C_PropVehicleDriveable::DampenEyePosition(Vector &vecVehicleEyePos,
                                               QAngle &vecVehicleEyeAngles) {}

//-----------------------------------------------------------------------------
// Purpose: Modify the player view/camera while in a vehicle
//-----------------------------------------------------------------------------
void C_PropVehicleDriveable::GetVehicleViewPosition(int nRole,
                                                    Vector *pAbsOrigin,
                                                    QAngle *pAbsAngles,
                                                    float *pFOV /*=NULL*/) {
...
//-----------------------------------------------------------------------------
void C_PropVehicleDriveable::GetVehicleClipPlanes(float &flZNear,
                                                  float &flZFar) const {
...
//-----------------------------------------------------------------------------
int GetFlashColorIntensity(int LowIntensity, int HighIntensity, bool Dimming,
                           int Increment, int Timer) {
  if (Dimming)
    ...
    if (pIcon != NULL) {
      ...
      if (UseVR()) {
        C_BasePlayer *pPlayer =
            (C_BasePlayer *)GetPassenger(VEHICLE_ROLE_DRIVER);
        ...
        trace_t tr;
        UTIL_TraceLine(vecStart, vecEnd, MASK_SHOT, this, COLLISION_GROUP_NONE,
                       &tr);
      ...
      if (pIcon) {
        ...
        if (!m_bScannerWepIcon) {
          ...
          pIcon->DrawSelf(
              ...
              Color(0, 0,
                    GetFlashColorIntensity(55, 255, m_bScannerWepDim, 10,
    ...
    if (pIcon) {
      ...
      if (!m_bScannerVehicleIcon) {
        ...
        pIcon->DrawSelf(
            XRES(iIconX), YRES(iIconY),
            Color(0, 0,
                  GetFlashColorIntensity(55, 255, m_bScannerVehicleDim, 10,
...
// Purpose:
//-----------------------------------------------------------------------------
void C_PropVehicleDriveable::RestrictView(float *pYawBounds,
                                          float *pPitchBounds,
                                          float *pRollBounds,
...
// Purpose:
//-----------------------------------------------------------------------------
void C_PropVehicleDriveable::UpdateViewAngles(C_BasePlayer *pLocalPlayer,
                                              CUserCmd *pCmd) {
...
// Purpose:
//-----------------------------------------------------------------------------
void C_PropVehicleDriveable::OnEnteredVehicle(
    C_BaseCombatCharacter *pPassenger) {
...
// NVNT - added function
void C_PropVehicleDriveable::OnExitedVehicle(
    C_BaseCombatCharacter *pPassenger) {
#if defined(WIN32) && !defined(_X360)
  // NVNT notify haptics system of navigation exit
