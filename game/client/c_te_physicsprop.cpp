...
// Networking
//-----------------------------------------------------------------------------
IMPLEMENT_CLIENTCLASS_EVENT_DT(C_TEPhysicsProp, DT_TEPhysicsProp,
                               CTEPhysicsProp)
RecvPropVector(RECVINFO(m_vecOrigin)),
    ...
    RecvPropInt(RECVINFO(m_nModelIndex)), RecvPropInt(RECVINFO(m_nFlags)),
...
static inline void RecordPhysicsProp(const Vector &start, const QAngle &angles,
                                     const Vector &vel, int nModelIndex,
                                     bool bBreakModel, int nSkin,
                                     int nEffects) {
  if (!ToolsEnabled())
    return;
...
void TE_PhysicsProp(IRecipientFilter &filter, float delay, int modelindex,
  ...
  tempents->PhysicsProp(modelindex, skin, pos, angles, vel, breakmodel,
                        effects);
...
void C_TEPhysicsProp::PostDataUpdate(DataUpdateType_t updateType) {
  ...
  tempents->PhysicsProp(m_nModelIndex, m_nSkin, m_vecOrigin, m_angRotation,
                        m_vecVelocity, m_nFlags, m_nEffects);
  RecordPhysicsProp(m_vecOrigin, m_angRotation, m_vecVelocity, m_nModelIndex,
                    m_nFlags, m_nSkin, m_nEffects);
}
...
void TE_PhysicsProp(IRecipientFilter &filter, float delay,
  ...
  TE_PhysicsProp(filter, delay, nModelIndex, nSkin, vecOrigin, angles, vecVel,
                 bBreakModel, nEffects);
}
