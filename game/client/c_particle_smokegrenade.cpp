...
RecvPropTime(RECVINFO(m_flSpawnTime)), RecvPropFloat(RECVINFO(m_FadeStartTime)),
...
inline void
C_ParticleSmokeGrenade::UpdateParticleDuringTrade(int iParticle,
                                                  float fTimeDelta) {
...
void C_ParticleSmokeGrenade::Update(float fTimeDelta) {
  ...
  Vector vMins =
      m_SmokeBasePos - Vector(m_SpacingRadius + SMOKEGRENADE_PARTICLERADIUS,
  ...
  if (m_CurrentStage == 1) {
    ...
    //		debugoverlay->AddBoxOverlay( GetPos(), Vector( -m_ExpandRadius,
    //-m_ExpandRadius, -m_ExpandRadius), Vector( m_ExpandRadius, m_ExpandRadius,
    //m_ExpandRadius), vec3_angle, 0, 255, 0, 1, 1.0f );
...
inline void
C_ParticleSmokeGrenade::ApplyDynamicLight(const Vector &vParticlePos,
                                          Vector &color) {
  if (m_nActiveLights) {
...
void C_ParticleSmokeGrenade::RenderParticles(
  ...
  while (pParticle) {
    ...
    if (len > m_ExpandRadius) {
      ...
      // debugoverlay->AddBoxOverlay( renderPos, Vector( -2, -2, -2), Vector( 2,
      // 2, 2), vec3_angle, 255, 255, 255, 255, 1.0f );
  ...
  for (int x = 0; x < m_xCount; x++) {
    ...
    for (int y = 0; y < m_yCount; y++) {
      ...
      for (int z = 0; z < m_zCount; z++) {
        ...
        //				int contents =
        //enginetrace->GetPointContents( vPos );

        // Culling out particles in solid makes smoke not fill up small
        // passageways.
          ...
          */
          {
            ...
            if (pParticle) {
              ...
              // debugoverlay->AddBoxOverlay( vPos, Vector( -2, -2, -2), Vector(
              // 2, 2, 2), vec3_angle, 255, 0, 0, 255, 5.0f );
            }
...
void C_ParticleSmokeGrenade::CleanupToolRecordingState(KeyValues *msg) {
  ...
  if (m_bVolumeFilled &&
    ...
    Color c1(FastFToC(clamp(m_MinColor.x, 0.f, 1.f)),
             FastFToC(clamp(m_MinColor.y, 0.f, 1.f)),
             FastFToC(clamp(m_MinColor.z, 0.f, 1.f)), 255);
    Color c2(FastFToC(clamp(m_MaxColor.x, 0.f, 1.f)),
