...
//			radius -
//-----------------------------------------------------------------------------
void CBaseSimpleCollision::Setup(const Vector &origin, float speed,
                                 float gravity) {
...
//			coarse - tests again with a real trace unless coarse is
//set
//-----------------------------------------------------------------------------
void CBaseSimpleCollision::TraceLine(const Vector &start, const Vector &end,
                                     trace_t *pTrace, bool coarse) {
  ...
  for (int i = 0; i < m_nActivePlanes; i++) {
    ...
    if ((coarse == false) && (pTrace->fraction < 1.0f)) {
      UTIL_TraceLine(start, end, MASK_SOLID_BRUSHONLY, NULL,
                     COLLISION_GROUP_NONE, pTrace);
    }

#if __DEBUG_PARTICLE_COLLISION_OVERLAY
    debugoverlay->AddBoxOverlay(vIntersection, Vector(-1, -1, -1),
                                Vector(1, 1, 1), QAngle(0, 0, 0), 0, 255, 0, 16,
...
void CBaseSimpleCollision::ConsiderPlane(cplane_t *plane) {
  ...
  for (int i = 0; i < m_nActivePlanes; i++) {
    if (m_collisionPlanes[i].m_Dist != -1.0f) {
      // Test for coplanar
      if ((m_collisionPlanes[i].m_Normal == plane->normal) &&
          (m_collisionPlanes[i].m_Dist == plane->dist))
        return;
    }
  }
...
//-----------------------------------------------------------------------------
// Purpose: Runs a simulation of the average particle's movement, looking for
// collisions along the way Input  : &start - start of the simulation
//			&dir - direction of travel
//			speed - speed of the particle
//			gravity - gravity being used
//-----------------------------------------------------------------------------
void CBaseSimpleCollision::TestForPlane(const Vector &start, const Vector &dir,
  ...
  for (int i = 1; i <= NUM_DISCREET_STEPS; i++) {
    ...
    // Trace the line
    UTIL_TraceLine(testStart, testEnd, MASK_SOLID_BRUSHONLY, NULL,
    ...
    if (tr.fraction != 1.0f) {
      ...
      debugoverlay->AddBoxOverlay(tr.endpos, Vector(-64, -64, 0),
                                  Vector(64, 64, 0), angles, 255, 0, 0, 16,
                                  __DEBUG_PARTICLE_COLLISION_OVERLAY_LIFETIME);
#endif //__DEBUG_PARTICLE_COLLISION_OVERLAY
...
//-----------------------------------------------------------------------------
void CParticleCollision::Setup(const Vector &origin, const Vector *dir,
                               float angularSpread, float minSpeed,
...
//			timeDelta - time step
//-----------------------------------------------------------------------------
bool CParticleCollision::MoveParticle(Vector &origin, Vector &velocity,
  ...
  if (m_nActivePlanes > 0) {
    ...
    if (pTrace->fraction != 1.0f) {
      ...
      UTIL_TraceLine(origin, testPosition, MASK_SOLID_BRUSHONLY, NULL,
                     COLLISION_GROUP_NONE, pTrace);
      ...
      if (pTrace->fraction != 1.0f) {
        ...
        if ((pTrace->plane.normal[2] >= 0.5f) && (fabs(velocity[2]) <= 48.0f)) {
          ...
          origin +=
              velocity * ((pTrace->fraction - COLLISION_EPSILON) * timeDelta);
          ...
          // Apply dampening
          velocity *= random->RandomFloat((m_flCollisionDampen - 0.1f),
                                          (m_flCollisionDampen + 0.1f));
        ...
        debugoverlay->AddBoxOverlay(
            pTrace->endpos, Vector(-1, -1, -1), Vector(1, 1, 1),
            QAngle(0, 0, 0), 255, 0, 0, 16,
            __DEBUG_PARTICLE_COLLISION_OVERLAY_LIFETIME);
#endif //__DEBUG_PARTICLE_COLLISION_OVERLAY
      }
    }
