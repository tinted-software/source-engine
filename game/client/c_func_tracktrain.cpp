...
void C_FuncTrackTrain::OnDataChanged(DataUpdateType_t updateType) {
  ...
  if (updateType == DATA_UPDATE_CREATED) {
    // Compute the cross-sectional area and dimension and length of the line
    // segment
    int nIndex1, nIndex2;
    ...
    m_flRadius = sqrt(vecOBBSize[nIndex1] * vecOBBSize[nIndex1] +
                      vecOBBSize[nIndex2] * vecOBBSize[nIndex2]) *
                 0.5f;
...
bool C_FuncTrackTrain::GetSoundSpatialization(SpatializationInfo_t &info) {
  ...
  if (info.pOrigin) {
    ...
    float t;
    CalcClosestPointOnLine(info.info.vListenerOrigin, vecStart, vecEnd,
                           *info.pOrigin, &t);
    if (t < 0.0f) {
