...
struct loopingsound_t {
  ...
  int id; // Used to fade out sounds that don't belong to the most current
          // setting
  bool isAmbient; // Ambient sounds have no spatialization - they play from
                  // everywhere
};

ConVar
    soundscape_fadetime("soundscape_fadetime", "3.0", FCVAR_CHEAT,
...
public:
  ...
  C_SoundscapeSystem() { m_nRestoreFrame = -1; }
  ...
  virtual void LevelShutdownPostEntity() { OnStopAllSounds(); }

  virtual void OnSave() {}
  virtual void OnRestore() { m_nRestoreFrame = gpGlobals->framecount; }
  ...
  int AddLoopingSound(const char *pSoundName, bool isAmbient, float volume,
                      soundlevel_t soundLevel, int pitch,
  ...
  void StartSubSoundscape(KeyValues *pSoundscape,
  ...
  // "playlooping"
  void ProcessPlayLooping(KeyValues *pPlayLooping,
                          const subsoundscapeparams_t &params);
  // "playrandom"
  void ProcessPlayRandom(KeyValues *pPlayRandom,
                         const subsoundscapeparams_t &params);
  // "playsoundscape"
  void ProcessPlaySoundscape(KeyValues *pPlaySoundscape,
                             subsoundscapeparams_t &params);
  // "soundmixer"
...
private:
  ...
  CUtlVector<KeyValues *>
      m_SoundscapeScripts;               // The whole script file in memory
  ...
  CUtlVector<loopingsound_t>
      m_loopingSounds;                      // list of currently playing sounds
  ...
  int m_forcedSoundscapeIndex; // >= 0 if this a "forced" soundscape? i.e. debug
                               // mode?
...
IGameSystem *ClientSoundscapeSystem() { return &g_SoundscapeSystem; }

void Soundscape_OnStopAllSounds() { g_SoundscapeSystem.OnStopAllSounds(); }
...
#else
  if (filesystem->LoadKeyValues(*script, IFileSystem::TYPE_SOUNDSCAPE, filename,
                                "GAME"))
#endif
  {
...
bool C_SoundscapeSystem::Init() {
  ...
  if (filesystem->LoadKeyValues(*manifest, IFileSystem::TYPE_SOUNDSCAPE,
                                SOUNDSCAPE_MANIFEST_FILE, "GAME")) {
    for (KeyValues *sub = manifest->GetFirstSubKey(); sub != NULL;
      ...
      if (!Q_stricmp(sub->GetName(), "file")) {
        ...
        if (mapSoundscapeFilename &&
            FStrEq(sub->GetString(), mapSoundscapeFilename)) {
          mapSoundscapeFilename =
              NULL; // we've already loaded the map's soundscape
        }
        continue;
      }

      Warning("C_SoundscapeSystem::Init:  Manifest '%s' with bogus file type "
              "'%s', expecting 'file'\n",
              SOUNDSCAPE_MANIFEST_FILE, sub->GetName());
    }

    if (mapSoundscapeFilename &&
        filesystem->FileExists(mapSoundscapeFilename)) {
...
static int SoundscapeCompletion(const char *partial,
  ...
  while (pSoundscapeName && current < COMMAND_COMPLETION_MAXITEMS) {
    if (!substring ||
        !Q_strncasecmp(pSoundscapeName, substring, substringLen)) {
      Q_snprintf(commands[current], sizeof(commands[current]), "%s %s", cmdname,
                 pSoundscapeName);
      current++;
    }
    i++;
...
CON_COMMAND_F_COMPLETION(playsoundscape, "Forces a soundscape to play",
                         FCVAR_CHEAT, SoundscapeCompletion) {
  if (args.ArgC() < 2) {
    g_SoundscapeSystem.DevReportSoundscapeName(
        g_SoundscapeSystem.GetCurrentSoundscape());
    return;
  }
...
void C_SoundscapeSystem::ForceSoundscape(const char *pSoundscapeName,
                                         float radius) {
...
void C_SoundscapeSystem::UpdateLoopingSounds(float frametime) {
  ...
  while (fadeCount > 0) {
    ...
    if (sound.volumeCurrent != sound.volumeTarget) {
      sound.volumeCurrent =
          Approach(sound.volumeTarget, sound.volumeCurrent, amount);
...
void C_SoundscapeSystem::Update(float frametime) {
  if (m_forcedSoundscapeIndex >= 0) {
    ...
    if (pPlayer) {
      ...
      m_params.localSound.Set(0, origin + m_forcedSoundscapeRadius *
                                              (forward - right));
      m_params.localSound.Set(1, origin + m_forcedSoundscapeRadius *
                                              (forward + right));
      m_params.localSound.Set(2, origin + m_forcedSoundscapeRadius *
                                              (-forward - right));
      m_params.localSound.Set(3, origin + m_forcedSoundscapeRadius *
                                              (-forward + right));
...
void C_SoundscapeSystem::UpdateAudioParams(audioparams_t &audio) {
  if (m_params.soundscapeIndex == audio.soundscapeIndex &&
      m_params.ent.Get() == audio.ent.Get())
    return;

  m_params = audio;
  m_forcedSoundscapeIndex = -1;
  if (audio.ent.Get() && audio.soundscapeIndex >= 0 &&
      audio.soundscapeIndex < m_soundscapes.Count()) {
...
// Called when a soundscape is activated (leading edge of becoming the active
// soundscape)
...
void C_SoundscapeSystem::StartSubSoundscape(KeyValues *pSoundscape,
                                            subsoundscapeparams_t &params) {
...
void C_SoundscapeSystem::ProcessSoundMixer(KeyValues *pSoundMixer,
                                           subsoundscapeparams_t &params) {
  C_BasePlayer *pPlayer = C_BasePlayer::GetLocalPlayer();
...
void C_SoundscapeSystem::ProcessDSPVolume(KeyValues *pKey,
                                          subsoundscapeparams_t &params) {
...
void C_SoundscapeSystem::ProcessPlayLooping(
    KeyValues *pAmbient, const subsoundscapeparams_t &params) {
  float volume = 0;
  ...
  while (pKey) {
    if (!Q_strcasecmp(pKey->GetName(), "volume")) {
      volume =
          params.masterVolume * RandomInterval(ReadInterval(pKey->GetString()));
      ...
      soundlevel =
          ATTN_TO_SNDLVL(RandomInterval(ReadInterval(pKey->GetString())));
      ...
      if (!Q_strncasecmp(pKey->GetString(), "SNDLVL_", strlen("SNDLVL_"))) {
        ...
        soundlevel = (soundlevel_t)((int)RandomInterval(
            ReadInterval(pKey->GetString())));
      }
  ...
  if (volume != 0 && pSoundName != NULL) {
    if (positionIndex < 0) {
      ...
      AddLoopingSound(pSoundName, false, volume, soundlevel, pitch,
                      m_params.localSound[positionIndex]);
    }
  }
}

void C_SoundscapeSystem::TouchSoundFile(char const *wavefile) {
  filesystem->GetFileTime(VarArgs("sound/%s", PSkipSoundChars(wavefile)),
                          "GAME");
}
...
Vector C_SoundscapeSystem::GenerateRandomSoundPosition() {
  ...
  if (pPlayer) {
    ...
    pPlayer->EyePositionAndVectors(&origin, &forward, &right, NULL);
    return origin +
           DEFAULT_SOUND_RADIUS * (cosAngle * right + sinAngle * forward);
  } else {
    return CurrentViewOrigin() +
           DEFAULT_SOUND_RADIUS * (cosAngle * CurrentViewRight() +
                                   sinAngle * CurrentViewForward());
  }
}
...
// puts a recurring random sound event into the queue
void C_SoundscapeSystem::ProcessPlayRandom(
    KeyValues *pPlayRandom, const subsoundscapeparams_t &params) {
  randomsound_t sound;
  sound.Init();
  ...
  while (pKey) {
    if (!Q_strcasecmp(pKey->GetName(), "volume")) {
      ...
      sound.soundlevel.range =
          ATTN_TO_SNDLVL(atten.start + atten.range) - sound.soundlevel.start;
    } else if (!Q_strcasecmp(pKey->GetName(), "soundlevel")) {
      if (!Q_strncasecmp(pKey->GetString(), "SNDLVL_", strlen("SNDLVL_"))) {
  ...
  if (sound.waveCount != 0) {
    if (positionIndex < 0 && !randomPosition) {
      ...
      if (randomPosition) {
        ...
        if (positionIndex > 31 ||
            !(m_params.localBits & (1 << positionIndex))) {
          // suppress sounds if the position isn't available
          // DevMsg( 1, "Bad position %d\n", positionIndex );
          return;
        }
...
void C_SoundscapeSystem::ProcessPlaySoundscape(
    KeyValues *pPlaySoundscape, subsoundscapeparams_t &paramsIn) {
  ...
  while (pKey) {
    if (!Q_strcasecmp(pKey->GetName(), "volume")) {
      subParams.masterVolume = paramsIn.masterVolume *
                               RandomInterval(ReadInterval(pKey->GetString()));
    } else if (!Q_strcasecmp(pKey->GetName(), "position")) {
      ...
      if (paramsIn.positionOverride < 0) {
        ...
        // positionoverride is only ever used to make a whole soundscape come
        // from a point in space So go ahead and default ambients there too.
        subParams.ambientPositionOverride =
            paramsIn.startingPosition + pKey->GetInt();
      }
      ...
      DevMsg(1, "Playsoundscape %s:Unknown command %s\n",
             pSoundscapeName ? pSoundscapeName : pPlaySoundscape->GetName(),
             pKey->GetName());
    }
    pKey = pKey->GetNextKey();
  }
  ...
  if (pSoundscapeName) {
    KeyValues *pSoundscapeKeys =
        SoundscapeByIndex(FindSoundscapeByName(pSoundscapeName));
    if (pSoundscapeKeys) {
...
int C_SoundscapeSystem::AddLoopingAmbient(const char *pSoundName, float volume,
                                          int pitch) {
  return AddLoopingSound(pSoundName, true, volume, SNDLVL_NORM, pitch,
                         vec3_origin);
}
...
//		this prevents pops
int C_SoundscapeSystem::AddLoopingSound(const char *pSoundName, bool isAmbient,
  ...
  while (soundSlot >= 0) {
    ...
    if (sound.id != m_loopingSoundId && sound.pitch == pitch &&
      ...
      // Positional sounds can reuse the slots if the positions are the same.
        ...
        if (VectorsAreEqual(position, sound.position, 0.1f)) {
          ...
          // If it's trying to fade out one positional sound and fade in
          // another, then it gets screwy because it'll be sending alternating
          // commands to the sound engine, referencing the same sound
          // (SOUND_FROM_WORLD, CHAN_STATIC, pSoundName). One of the alternating
          // commands will be as it fades the sound out, and one will be fading
          // the sound in. Because this will occasionally cause the sound to
          // vanish entirely, we stop the old sound immediately.
          StopLoopingSound(sound);
          pSoundSlot = &sound;

          // make a note to update the sound immediately. Otherwise, if its
          // volume happens to be the same as the old sound's volume, it will
          // never update at all.
...
void C_SoundscapeSystem::UpdateLoopingSound(loopingsound_t &loopSound) {
  if (loopSound.isAmbient) {
    enginesound->EmitAmbientSound(loopSound.pWaveName, loopSound.volumeCurrent,
                                  loopSound.pitch, SND_CHANGE_VOL);
  } else {
    CLocalPlayerFilter filter;
...
int C_SoundscapeSystem::AddRandomSound(const randomsound_t &sound) {
  ...
  m_randomSounds[index].nextPlayTime =
      gpGlobals->curtime + 0.5 * RandomInterval(sound.time);
...
void C_SoundscapeSystem::PlayRandomSound(randomsound_t &sound) {
  ...
  if (sound.isAmbient) {
    enginesound->EmitAmbientSound(
        pWaveName, sound.masterVolume * RandomInterval(sound.volume),
...
void C_SoundscapeSystem::UpdateRandomSounds(float gameTime) {
  ...
  m_nextRandomTime =
      gameTime + 3600; // add some big time to check again (an hour)
  ...
  for (int i = m_randomSounds.Count() - 1; i >= 0; i--) {
    ...
    if (gameTime >= m_randomSounds[i].nextPlayTime) {
      ...
      m_randomSounds[i].nextPlayTime =
          gameTime + RandomInterval(m_randomSounds[i].time);
    }
