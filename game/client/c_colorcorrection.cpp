...
static ConVar
    mat_colcorrection_disableentities("mat_colcorrection_disableentities", "0",
                                      FCVAR_NONE,
...
IMPLEMENT_CLIENTCLASS_DT(C_ColorCorrection, DT_ColorCorrection,
                         CColorCorrection)
RecvPropVector(RECVINFO(m_vecOrigin)), RecvPropFloat(RECVINFO(m_minFalloff)),
...
void C_ColorCorrection::OnDataChanged(DataUpdateType_t updateType) {
  ...
  if (updateType == DATA_UPDATE_CREATED) {
    if (m_CCHandle == INVALID_CLIENT_CCHANDLE) {
      ...
      SetNextClientThink((m_CCHandle != INVALID_CLIENT_CCHANDLE)
                             ? CLIENT_THINK_ALWAYS
                             : CLIENT_THINK_NEVER);
    }
  }
...
// We don't draw...
//------------------------------------------------------------------------------
bool C_ColorCorrection::ShouldDraw() { return false; }
...
void C_ColorCorrection::ClientThink() {
  ...
  if (mat_colcorrection_disableentities.GetInt()) {
    // Allow the colorcorrectionui panel (or user) to turn off color-correction
    // entities
  ...
  float weight = 0;
  if ((m_minFalloff != -1) && (m_maxFalloff != -1) &&
      m_minFalloff != m_maxFalloff) {
  ...
  g_pColorCorrectionMgr->SetColorCorrectionWeight(
      m_CCHandle, m_flCurWeight * (1.0 - weight));
