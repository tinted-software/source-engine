...
RecvPropInt(RECVINFO(m_nSceneStringIndex)),
    RecvPropBool(RECVINFO(m_bIsPlayingBack)), RecvPropBool(RECVINFO(m_bPaused)),
...
void C_SceneEntity::ProcessEvent(float currenttime, CChoreoScene *scene,
  ...
  if (currenttime < event->m_flPrevTime) {
    ...
    //event->GetName(), event->m_flPrevTime, m_flCurrentTime );
    // }
    ...
    if (actor) {
      ...
      if (NULL == pActor) {
        ...
        // This can occur if we haven't been networked an actor yet... we need
        // to queue it so that we can
        //  fire off the process event as soon as we have the actor resident on
        //  the client.
        return;
      }
    }
...
//			*parameters -
//-----------------------------------------------------------------------------
void C_SceneEntity::DispatchProcessGesture(CChoreoScene *scene,
                                           C_BaseFlex *actor,
                                           CChoreoEvent *event) {
  // Ingore null gestures
  if (!Q_stricmp(event->GetName(), "NULL"))
...
// Input  : *actor -
//-----------------------------------------------------------------------------
void C_SceneEntity::DispatchProcessSequence(CChoreoScene *scene,
                                            CBaseFlex *actor,
                                            CChoreoEvent *event) {
  actor->RemoveSceneEvent(scene, event, false);
  actor->AddSceneEvent(scene, event, NULL, IsClientOnly());
