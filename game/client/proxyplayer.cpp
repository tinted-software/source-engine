...
void CPlayerProximityProxy::OnBind(void *pC_BaseEntity) {
  ...
  Vector delta;
  VectorSubtract(pEntity->WorldSpaceCenter(), pPlayer->WorldSpaceCenter(),
                 delta);
...
EXPOSE_INTERFACE(CPlayerProximityProxy, IMaterialProxy,
                 "PlayerProximity" IMATERIAL_PROXY_INTERFACE_VERSION);

//-----------------------------------------------------------------------------
// Returns true if the player's team matches that of the entity the proxy
...
void CPlayerTeamMatchProxy::OnBind(void *pC_BaseEntity) {
  ...
  Assert(m_pResult);
  SetFloatResult((pEntity->GetTeamNumber() == pPlayer->GetTeamNumber()) ? 1.0
                                                                        : 0.0);
...
EXPOSE_INTERFACE(CPlayerTeamMatchProxy, IMaterialProxy,
                 "PlayerTeamMatch" IMATERIAL_PROXY_INTERFACE_VERSION);
...
EXPOSE_INTERFACE(CPlayerViewProxy, IMaterialProxy,
                 "PlayerView" IMATERIAL_PROXY_INTERFACE_VERSION);
...
EXPOSE_INTERFACE(CPlayerSpeedProxy, IMaterialProxy,
                 "PlayerSpeed" IMATERIAL_PROXY_INTERFACE_VERSION);
...
EXPOSE_INTERFACE(CPlayerPositionProxy, IMaterialProxy,
                 "PlayerPosition" IMATERIAL_PROXY_INTERFACE_VERSION);
...
EXPOSE_INTERFACE(CEntitySpeedProxy, IMaterialProxy,
                 "EntitySpeed" IMATERIAL_PROXY_INTERFACE_VERSION);
...
EXPOSE_INTERFACE(CEntityRandomProxy, IMaterialProxy,
                 "EntityRandom" IMATERIAL_PROXY_INTERFACE_VERSION);
...
bool CPlayerLogoProxy::Init(IMaterial *pMaterial, KeyValues *pKeyValues) {
  ...
  m_pDefaultTexture =
      materials->FindTexture(DEFAULT_DECAL_NAME, TEXTURE_GROUP_DECAL);
...
void CPlayerLogoProxy::OnLogoBindInternal(int playerindex) {
  ...
  if (lookup == m_Logos.InvalidIndex()) {
    ...
    char logohex[16];
    Q_binarytohex((byte *)&info.customFiles[0], sizeof(info.customFiles[0]),
                  logohex, sizeof(logohex));
...
EXPOSE_INTERFACE(CPlayerLogoProxy, IMaterialProxy,
                 "PlayerLogo" IMATERIAL_PROXY_INTERFACE_VERSION);

/* @note Tom Bui: This is here for reference, but we don't want people to use
it!
//-----------------------------------------------------------------------------
//
//-----------------------------------------------------------------------------
...
pRend->GetIClientUnknown()->GetBaseEntity(); if ( pEntity )
                {
                        if ( !pEntity->IsPlayer() )
                        {
...
EXPOSE_INTERFACE( CPlayerLogoOnModelProxy, IMaterialProxy, "PlayerLogoOnModel"
