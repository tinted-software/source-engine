...
#ifdef PORTAL
bool g_bUpsideDown =
    false; // Set when the player is upside down in Portal to invert the mouse.
#endif     // #ifdef PORTAL
...
public:
  ConVar_m_pitch()
      : ConVar_ServerBounded("m_pitch", "0.022", FCVAR_ARCHIVE,
                             "Mouse pitch factor.") {}
  ...
  virtual float GetFloat() const {
    ...
    // If sv_cheats is off than it can only be 0.022 or -0.022 (if they've
    // reversed the mouse in the options).
    if (flBaseValue > 0)
      return 0.022f;
    else
      return -0.022f;
...
static ConVar m_forward("m_forward", "1", FCVAR_ARCHIVE,
                        "Mouse forward factor.");

static ConVar m_customaccel(
    ...
    "\n2: Same as 1, with but x and y sensitivity are scaled by m_pitch and "
    "m_yaw respectively."
    "\n3: mouse_acceleration = pow(raw_mouse_delta, m_customaccel_exponent - "
    "1) * sensitivity");
static ConVar m_customaccel_scale("m_customaccel_scale", "0.04", FCVAR_ARCHIVE,
                                  "Custom mouse acceleration value.", true, 0,
                                  false, 0.0f);
static ConVar m_customaccel_max("m_customaccel_max", "0", FCVAR_ARCHIVE,
                                "Max mouse move scale factor, 0 for no limit");
static ConVar m_customaccel_exponent(
    "m_customaccel_exponent", "1", FCVAR_ARCHIVE,
    "Mouse move is raised to this power before being scaled by scale factor.",
    true, 1.0f, false, 0.0f);

static ConVar
    m_mousespeed("m_mousespeed", "1", FCVAR_ARCHIVE,
                 "Windows mouse acceleration (0 to disable, 1 to enable "
                 "[Windows 2000: enable initial threshold], 2 to enable "
                 "secondary threshold [Windows 2000 only]).",
                 true, 0, true, 2);
static ConVar
    m_mouseaccel1("m_mouseaccel1", "0", FCVAR_ARCHIVE,
                  "Windows mouse acceleration initial threshold (2x movement).",
                  true, 0, false, 0.0f);
static ConVar m_mouseaccel2(
    "m_mouseaccel2", "0", FCVAR_ARCHIVE,
    "Windows mouse acceleration secondary threshold (4x movement).", true, 0,
    false, 0.0f);

static ConVar m_rawinput("m_rawinput", "0", FCVAR_ARCHIVE,
                         "Use Raw Input for mouse input.");

#if DEBUG
ConVar cl_mouselook("cl_mouselook", "1", FCVAR_ARCHIVE,
...
void CInput::ActivateMouse(void) {
  ...
  if (m_fMouseInitialized) {
    if (m_fMouseParmsValid) {
      ...
      m_fRestoreSPI =
          SystemParametersInfo(SPI_SETMOUSE, 0, m_rgNewMouseParms, 0) ? true
                                                                      : false;
#endif
    }
    m_fMouseActive = true;
...
void CInput::DeactivateMouse(void) {
  // This gets called whenever the mouse should be inactive. We only respond to
  // it if we had previously activated the mouse. We'll show the cursor in here.
  if (!m_fMouseActive)
    return;
...
void CInput::CheckMouseAcclerationVars() {
  // Don't change them if the mouse is inactive, invalid, or not using
  // parameters for restore
  if (!m_fMouseActive || !m_fMouseInitialized || !m_fMouseParmsValid ||
      !m_fRestoreSPI) {
    return;
  }
  ...
  if (CommandLine()->FindParm("-useforcedmparms")) {
#ifdef WIN32
    m_fMouseParmsValid =
        SystemParametersInfo(SPI_GETMOUSE, 0, m_rgOrigMouseParms, 0) ? true
                                                                     : false;
#else
    m_fMouseParmsValid = false;
#endif
    if (m_fMouseParmsValid) {
      if (CommandLine()->FindParm("-noforcemspd")) {
        m_rgNewMouseParms[MOUSE_SPEED_FACTOR] =
            m_rgOrigMouseParms[MOUSE_SPEED_FACTOR];

        /*
                                        int mouseAccel[3];
                                        SystemParametersInfo(SPI_GETMOUSE, 0,
           &mouseAccel, 0); mouseAccel[2] = 0; bool ok =
           SystemParametersInfo(SPI_SETMOUSE, 0, &mouseAccel,
           SPIF_UPDATEINIFILE);

                                        // Now check registry and close/re-open
           Control Panel > Mouse and see 'Enhance pointer precision' is OFF
                                        mouseAccel[2] = 1;
                                        ok = SystemParametersInfo(SPI_SETMOUSE,
           0, &mouseAccel, SPIF_UPDATEINIFILE);

                                        // Now check registry and close/re-open
           Control Panel > Mouse and see 'Enhance pointer precision' is ON
        */
      } else {
      ...
      if (CommandLine()->FindParm("-noforcemaccel")) {
        m_rgNewMouseParms[MOUSE_ACCEL_THRESHHOLD1] =
            m_rgOrigMouseParms[MOUSE_ACCEL_THRESHHOLD1];
...
// frame and
//  these results are accumulated each time. This function gets the accumulated
//  mouse changes and resets the accumulators
// Input  : *mx -
//			*my -
//-----------------------------------------------------------------------------
void CInput::GetAccumulatedMouseDeltasAndResetAccumulators(float *mx,
                                                           float *my) {
  Assert(mx);
  Assert(my);
...
//-----------------------------------------------------------------------------
// Purpose: GetMouseDelta -- Filters the mouse and stores results in old
// position Input  : mx -
//			my -
//			*oldx -
//			*oldy -
...
//			*y -
//-----------------------------------------------------------------------------
void CInput::GetMouseDelta(float inmousex, float inmousey, float *pOutMouseX,
                           float *pOutMouseY) {
  // Apply filtering?
  if (m_filter.GetBool()) {
...
//-----------------------------------------------------------------------------
// Purpose: Multiplies mouse values by sensitivity.  Note that for windows mouse
// settings
//  the input x,y offsets are already scaled based on that.  The custom
//  acceleration, therefore, is totally engine-specific and applies as a
//  post-process to allow more user tuning.
// Input  : *x -
//			*y -
//-----------------------------------------------------------------------------
void CInput::ScaleMouse(float *x, float *y) {
  ...
  if (m_customaccel.GetInt() == 1 || m_customaccel.GetInt() == 2) {
    ...
    float accelerated_sensitivity =
        ((float)pow(raw_mouse_movement_distance,
    ...
    *y *= accelerated_sensitivity;
    ...
    // Further re-scale by yaw and pitch magnitude if user requests alternate
    // mode 2/4 This means that they will need to up their value for
    // m_customaccel_scale greatly (>40x) since m_pitch/yaw default
    //  to 0.022
    ...
    float accelerated_sensitivity =
        powf(raw_mouse_movement_distance_squared, fExp) * mouse_sensitivity;
...
//			mouse_y -
//-----------------------------------------------------------------------------
void CInput::ApplyMouse(QAngle &viewangles, CUserCmd *cmd, float mouse_x,
                        float mouse_y) {
  if (!((in_strafe.state & 1) || lookstrafe.GetInt())) {
...
#endif // #ifdef PORTAL
    {
      if (CAM_IsThirdPerson() && thirdperson_platformer.GetInt()) {
        if (mouse_x) {
          ...
          // use the mouse to orbit the camera around the player, and update the
          // idealAngle
          ...
          // cam_idealyaw.SetValue( cam_idealyaw.GetFloat() - m_yaw.GetFloat() *
          // mouse_x );
        }
      } else {
    ...
    // If holding strafe key or mlooking and have lookstrafe set to true, then
    // apply
...
#endif // #ifdef PORTAL
    {
      if (CAM_IsThirdPerson() && thirdperson_platformer.GetInt()) {
        if (mouse_y) {
          ...
          // why doesn't this work??? CInput::AdjustYaw is why
          // cam_idealpitch.SetValue( cam_idealpitch.GetFloat() +
...
void CInput::AccumulateMouse(void) {
  ...
  if (!m_fCameraInterceptingMouse && vgui::surface()->IsCursorLocked()) {
    ...
    //  By design, we follow the old mouse path even when using SDL for Windows,
    //  to retain old mouse behavior.
#if defined(PLATFORM_WINDOWS)
...
//			&oy -
//-----------------------------------------------------------------------------
void CInput::GetMousePos(int &ox, int &oy) { GetVGUICursorPos(ox, oy); }
...
//			y -
//-----------------------------------------------------------------------------
void CInput::SetMousePos(int x, int y) { SetVGUICursorPos(x, y); }
...
//			*unclampedy -
//-----------------------------------------------------------------------------
void CInput::GetFullscreenMousePos(int *mx, int *my, int *unclampedx /*=NULL*/,
                                   int *unclampedy /*=NULL*/) {
  Assert(mx);
  Assert(my);
...
//			my -
//-----------------------------------------------------------------------------
void CInput::SetFullscreenMousePos(int mx, int my) { SetMousePos(mx, my); }

//-----------------------------------------------------------------------------
// Purpose: ClearStates -- Resets mouse accumulators so you don't get a pop when
// returning to trapped mouse
//-----------------------------------------------------------------------------
