...
protected:
  ...
  CPanelAnimationVarAliasType(int, m_iTextX, "text_xpos", "8",
                              "proportional_int");
  CPanelAnimationVarAliasType(int, m_iTextY, "text_ypos", "8",
                              "proportional_int");
  CPanelAnimationVarAliasType(int, m_iCenterX, "center_x", "0",
                              "proportional_int");
  CPanelAnimationVarAliasType(int, m_iCenterY, "center_y", "0",
                              "proportional_int");
...
void CHudHintDisplay::OnThink() {
  ...
  if ((m_flLabelSizePercentage != 0.0 && m_flLabelSizePercentage != 1.0) ||
      m_bLastLabelUpdateHack) {
    m_bLastLabelUpdateHack =
        (m_flLabelSizePercentage != 0.0 && m_flLabelSizePercentage != 1.0);
    InvalidateLayout();
  }
}
...
ConVar cl_hudhint_sound("cl_hudhint_sound", "1",
...
//-----------------------------------------------------------------------------
void CHudHintDisplay::LocalizeAndDisplay(const char *pszHudTxtMsg,
...
//-----------------------------------------------------------------------------
// Purpose: Displays small key-centric hints on the right hand side of the
// screen
//-----------------------------------------------------------------------------
...
private:
  ...
  CPanelAnimationVarAliasType(float, m_iTextX, "text_xpos", "8",
                              "proportional_float");
  CPanelAnimationVarAliasType(float, m_iTextY, "text_ypos", "8",
                              "proportional_float");
  CPanelAnimationVarAliasType(float, m_iTextGapX, "text_xgap", "8",
                              "proportional_float");
  CPanelAnimationVarAliasType(float, m_iTextGapY, "text_ygap", "8",
                              "proportional_float");
  CPanelAnimationVarAliasType(float, m_iYOffset, "YOffset", "0",
                              "proportional_float");
};
    ...
    if (isVar) {
      ...
      if (IsX360()) {
        ...
        for (int iCode = 0; iCode < BUTTON_CODE_LAST; ++iCode) {
          ...
          // Only check against bind name if we haven't already forced this
          // binding to be used
          ...
          if (!bUseThisKey &&
              (Q_stricmp(pBinding, "+duck") == 0 ||
               Q_stricmp(pBinding, "toggle_duck") == 0) &&
              (Q_stricmp(binding, "+duck") == 0 ||
               Q_stricmp(binding, "toggle_duck") == 0)) {
            // +duck and toggle_duck are interchangable
            bUseThisKey = true;
          }

          if (!bUseThisKey &&
              (Q_stricmp(pBinding, "+zoom") == 0 ||
               Q_stricmp(pBinding, "toggle_zoom") == 0) &&
              (Q_stricmp(binding, "+zoom") == 0 ||
               Q_stricmp(binding, "toggle_zoom") == 0)) {
          ...
          Q_snprintf(szBuff, sizeof(szBuff), "#GameUI_Icons_%s",
                     g_pInputSystem->ButtonCodeToString(
                         static_cast<ButtonCode_t>(iCode)));
        ...
        // set the variable text - key may need to be localized (button images
        // for example)
        wchar_t *locName = g_pVGuiLocalize->Find(friendlyName);
        if (!locName || wcslen(locName) <= 0) {
          label->SetText(friendlyName + 1);
        } else {
          // Assuming localized vars must be using a bitmap image. *May* not be
          // the case, but since keyboard bindings have never been localized in
          // the past, they probably won't in the future either.
          bIsBitmap = true;
...
// Enable this small block of code to test formatting and layout of hint
// messages with varying numbers of lines
#define TEST_KEYHINT_DISPLAY 0
  ...
  const char *sampleText[] = {"This is a test",
  ...
  for (int i = 0; i < ARRAYSIZE(sampleText); ++i) {
    // put it in a label
    vgui::Label *label =
        vgui::SETUP_PANEL(new vgui::Label(this, NULL, sampleText[i]));
