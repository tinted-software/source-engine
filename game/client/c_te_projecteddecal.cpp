...
// Networking
//-----------------------------------------------------------------------------
IMPLEMENT_CLIENTCLASS_EVENT_DT(C_TEProjectedDecal, DT_TEProjectedDecal,
                               CTEProjectedDecal)
RecvPropVector(RECVINFO(m_vecOrigin)), RecvPropQAngles(RECVINFO(m_angRotation)),
    RecvPropFloat(RECVINFO(m_flDistance)), RecvPropInt(RECVINFO(m_nIndex)),
    END_RECV_TABLE()
...
void TE_ProjectDecal(IRecipientFilter &filter, float delay, const Vector *pos,
  ...
  if (tr.hitbox != 0) {
    staticpropmgr->AddDecalToStaticProp(*pos, endpos, hitbox - 1, index, false,
                                        tr);
  } else {
...
void C_TEProjectedDecal::PostDataUpdate(DataUpdateType_t updateType) {
  ...
  TE_ProjectDecal(filter, 0.0f, &m_vecOrigin, &m_angRotation, m_flDistance,
                  m_nIndex);
}
