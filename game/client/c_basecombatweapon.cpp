...
void C_BaseCombatWeapon::SetDormant(bool bDormant) {
  // If I'm going from active to dormant and I'm carried by another player,
  // holster me.
  ...
  if (state == SHOULDTRANSMIT_END) {
    ...
    if (m_iState == WEAPON_IS_CARRIED_BY_PLAYER) {
      if (GetOwner() && GetOwner()->GetActiveWeapon() == this) {
        // Restore the Activeness of the weapon if we client-twiddled it off in
...
int C_BaseCombatWeapon::GetWorldModelIndex(void) {
  if (GameRules()) {
    const char *pBaseName =
        modelinfo->GetModelName(modelinfo->GetModel(m_iWorldModelIndex));
    const char *pTranslatedName =
        GameRules()->TranslateEffectForVisionFilter("weapons", pBaseName);
...
void C_BaseCombatWeapon::OnDataChanged(DataUpdateType_t updateType) {
  ...
  if (bIsLocalPlayer &&
      ShouldDrawLocalPlayerViewModel()) // TODO: figure out the purpose of the
                                        // ShouldDrawLocalPlayer() test.
  {
    // If I was just picked up, or created & immediately carried, add myself to
    // this client's list of weapons
...
void C_BaseCombatWeapon::DrawCrosshair() {
  ...
  /*
     ...
     light the crosshair with a different color. Vector vShootPos, vShootAngles;
          ...
          // Change the color depending on if we're looking at a friend or an
     enemy. CPartitionFilterListMask filter( PARTITION_ALL_CLIENT_EDICTS );
          trace_t tr;
          traceline->TraceLine( vShootPos, vShootPos + vForward * 10000,
     COLLISION_GROUP_NONE, MASK_SHOT, &tr, true, ~0, &filter );

          if ( tr.index != 0 && tr.index != INVALID_CLIENTENTITY_HANDLE )
          {
                  C_BaseEntity *pEnt =
     ClientEntityList().GetBaseEntityFromHandle( tr.index ); if ( pEnt )
                  {
...
//-----------------------------------------------------------------------------
// Purpose: This weapon is the active weapon, and the viewmodel for it was just
// drawn.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Returns true if this weapon is the local client's currently wielded
// weapon
//-----------------------------------------------------------------------------
...
bool C_BaseCombatWeapon::ShouldDraw(void) {
  ...
  // FIXME: All weapons with owners are set to transmit in
  // CBaseCombatWeapon::UpdateTransmitState, even if they have EF_NODRAW set, so
  // we have to check this here. Ideally they would never transmit except for
  // the weapons owned by the local player.
  ...
  if (pOwner == pLocalPlayer) {
    ...
    if (!pOwner->ShouldDraw()) {
      ...
      // This also tests whether the player is zoomed in, in which case you
      // don't want to draw the weapon.
      return false;
    }
    ...
    // don't draw active weapon if not in some kind of 3rd person mode, the
    // viewmodel will do that
    return false;
  }
...
//-----------------------------------------------------------------------------
// Purpose: Return true if a weapon-pickup icon should be displayed when this
// weapon is received
//-----------------------------------------------------------------------------
...
int C_BaseCombatWeapon::DrawModel(int flags) {
  VPROF_BUDGET("C_BaseCombatWeapon::DrawModel",
...
//-----------------------------------------------------------------------------
// Allows the client-side entity to override what the network tells it to use
// for a model. This is used for third person mode, specifically in HL2 where
// the the weapon timings are on the view model and not the world model. That
// means the server needs to use the view model, but the client wants to use the
// world model.
//-----------------------------------------------------------------------------
