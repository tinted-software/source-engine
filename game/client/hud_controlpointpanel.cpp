...
private:
  ...
  CPanelAnimationVar(Color, m_clrActive, "color_active",
                     "HudCaptureProgressBar.Active");
  CPanelAnimationVar(Color, m_clrInActive, "color_inactive",
                     "HudCaptureProgressBar.InActive");
};
...
private:
  ...
  CPanelAnimationVar(Color, m_clrActive, "color_active",
                     "HudCaptureIcon.Active");
  CPanelAnimationVar(Color, m_clrInActive, "color_inactive",
                     "HudCaptureIcon.InActive");
};
...
private:
  ...
  CPanelAnimationVarAliasType(float, m_nSpaceBetweenIcons, "icon_space", "2",
                              "proportional_float");
...
ConVar hud_capturepanel("hud_capturepanel", "1",
...
// Purpose:
//-----------------------------------------------------------------------------
CHudCapturePanelProgressBar::CHudCapturePanelProgressBar(vgui::Panel *parent,
                                                         const char *name)
  ...
  if (m_iTexture == -1) // we didn't find it, so create a new one
  {
    ...
    vgui::surface()->DrawSetTextureFile(m_iTexture, "vgui/progress_bar", true,
                                        false);
  }
}
...
void CHudCapturePanelProgressBar::Paint() {
  ...
  float flEndAngle =
      flCompleteCircle * (1.0f - m_flPercent); // count DOWN (counter-clockwise)
  //	float flEndAngle = flCompleteCircle * m_flPercent; // count UP
  //(clockwise)
  ...
  if (flEndAngle >= fl90degrees * 3.0f) // >= 270 degrees
  {
    ...
    if (flEndAngle > fl90degrees * 3.5f) // > 315 degrees
    {
      ...
      verts[1].Init(
          Vector2D(flHalfWide -
                       (tan(fl90degrees * 4.0f - flEndAngle) * flHalfTall),
                   0.0f),
          uv21);
    ...
    } else // <= 315 degrees
    {
      ...
      verts[0].Init(Vector2D(0.0f, flHalfTall), uv11);
      verts[1].Init(
          Vector2D(0.0f, flHalfTall - (tan(flEndAngle - fl90degrees * 3.0f) *
                                       flHalfWide)),
          uv21);
      verts[2].Init(Vector2D(flHalfWide, flHalfTall), uv22);
      verts[3].Init(Vector2D(0.0f, flHalfTall), uv12);
  ...
  } else if (flEndAngle >= fl90degrees * 2.0f) // >= 180 degrees
  {
    ...
    if (flEndAngle > fl90degrees * 2.5f) // > 225 degrees
    {
      ...
      verts[3].Init(
          Vector2D(0.0f, flHalfTall + (tan(fl90degrees * 3.0f - flEndAngle) *
                                       flHalfWide)),
          uv12);
    ...
    } else // <= 225 degrees
    {
      ...
      verts[2].Init(
          Vector2D(flHalfWide -
                       (tan(flEndAngle - fl90degrees * 2.0f) * flHalfTall),
                   tall),
          uv22);
  ...
  } else if (flEndAngle >= fl90degrees) // >= 90 degrees
  {
    ...
    if (flEndAngle > fl90degrees * 1.5f) // > 135 degrees
    {
      ...
      verts[3].Init(
          Vector2D(flHalfWide +
                       (tan(fl90degrees * 2.0f - flEndAngle) * flHalfTall),
                   tall),
          uv12);
    ...
    } else // <= 135 degrees
    {
      ...
      verts[0].Init(Vector2D(flHalfWide, flHalfTall), uv11);
      verts[1].Init(Vector2D(wide, flHalfTall), uv21);
      verts[2].Init(Vector2D(wide, flHalfTall + (tan(flEndAngle - fl90degrees) *
                                                 flHalfWide)),
                    uv22);
      verts[3].Init(Vector2D(flHalfWide, flHalfTall), uv12);
  ...
  } else // > 0 degrees
  {
    if (flEndAngle > fl90degrees / 2.0f) // > 45 degrees
    {
      ...
      verts[0].Init(Vector2D(flHalfWide, 0.0f), uv11);
      verts[1].Init(Vector2D(wide, 0.0f), uv21);
      verts[2].Init(Vector2D(wide, flHalfTall - (tan(fl90degrees - flEndAngle) *
                                                 flHalfWide)),
                    uv22);
      verts[3].Init(Vector2D(flHalfWide, flHalfTall), uv12);
    ...
    } else // <= 45 degrees
    {
      ...
      verts[0].Init(Vector2D(flHalfWide, 0.0f), uv11);
      verts[1].Init(Vector2D(flHalfWide + (tan(flEndAngle) * flHalfTall), 0.0f),
                    uv21);
      verts[2].Init(Vector2D(flHalfWide, flHalfTall), uv22);
...
// Purpose:
//-----------------------------------------------------------------------------
CHudCapturePanelIcon::CHudCapturePanelIcon(vgui::Panel *parent,
                                           const char *name)
    : vgui::ImagePanel(parent, name) {
  m_bActive = false;
  ...
  {
    m_iTexture = vgui::surface()->CreateNewTextureID();
    vgui::surface()->DrawSetTextureFile(m_iTexture, "vgui/capture_icon", true,
                                        false);
  }
}
...
CHudCapturePanel::CHudCapturePanel(const char *pElementName)
  ...
  m_pProgressBar =
      new CHudCapturePanelProgressBar(this, "CapturePanelProgressBar");
...
void CHudCapturePanel::OnThink() {
  ...
  if (!m_bInitializedFlags) {
    ...
    for (int i = 0; i < GetNumberOfTeams(); i++) {
      ...
      m_pTeamFlags[i] = dynamic_cast<vgui::ImagePanel *>(
          FindChildByName(VarArgs("CapturePanelTeamFlag_%d", i)));
    }
    InvalidateLayout();
  }
  ...
  if (pPlayer) {
    ...
    // Turn off the panel and children if the player is dead or not in a cap
    // zone
    if (!m_bFakingCapture &&
        (!bInCapZone || !hud_capturepanel.GetBool() || !pPlayer->IsAlive())) {
      if (IsVisible()) {
        SetVisible(false);
      }
      return;
    ...
    int nNumTeammates =
        ObjectiveResource()->GetNumPlayersInArea(m_iCurrentCP, nPlayerTeam);
    int nRequiredTeammates =
        ObjectiveResource()->GetRequiredCappers(m_iCurrentCP, nPlayerTeam);
    ...
    for (int i = LAST_SHARED_TEAM + 1; i < GetNumberOfTeams(); i++) {
      ...
      int iTeamInArea =
          ObjectiveResource()->GetNumPlayersInArea(m_iCurrentCP, i);
      nNumEnemies += iTeamInArea;

      if (iTeamInArea >=
          ObjectiveResource()->GetRequiredCappers(m_iCurrentCP, i)) {
    ...
    // or we're playing minirounds and the current cap zone is not in the
    // current round
    if ((nOwningTeam == nPlayerTeam && !bEnemyTeamReadyToCap) ||
        (ObjectiveResource()->PlayingMiniRounds() &&
         !ObjectiveResource()->IsInMiniRound(m_iCurrentCP))) {
    ...
    int width = (nRequiredTeammates * iconWide) +
                ((nRequiredTeammates - 1) * m_nSpaceBetweenIcons);
    ...
    if (iCappingTeam == TEAM_UNASSIGNED || iCappingTeam != nPlayerTeam) {
      ...
      if (m_pMessage) {
        ...
        // If a team's not allowed to cap a point, don't count players in it at
        // all
        if (!TeamplayGameRules()->TeamMayCapturePoint(nPlayerTeam,
                                                      m_iCurrentCP)) {
        ...
        } else if (!TeamplayGameRules()->PlayerMayCapturePoint(
                       pPlayer, m_iCurrentCP, szReason, sizeof(szReason))) {
        ...
        } else if (mp_blockstyle.GetInt() == 1 &&
                   iCappingTeam != TEAM_UNASSIGNED) {
          m_pMessage->SetText("#Team_Blocking_Capture");
        ...
        } else if (!ObjectiveResource()->TeamCanCapPoint(m_iCurrentCP,
                                                         nPlayerTeam)) {
        ...
        if (m_pBackground) {
          ...
          if (bgW < textW) {
            m_pBackground->SetBounds(bgX + (bgW / 2.0) -
                                         ((textW + XRES(3)) / 2.0),
                                     bgY, textW + XRES(3), bgH);
          }
        }
      }
    } else {
      ...
      if (m_pProgressBar) {
        ...
        if (m_bFakingCapture) {
          float flProgress =
              RemapVal(m_flFakeCaptureTime - gpGlobals->curtime, 0, 5.0, 0, 1);
...
void CHudCapturePanel::FireGameEvent(IGameEvent *event) {
  ...
  if (FStrEq("controlpoint_starttouch", eventname)) {
    ...
    if (pPlayer && iPlayer == pPlayer->entindex()) {
      ...
      UTIL_TraceLine(MainViewOrigin(), vecEnd, MASK_SOLID_BRUSHONLY, pPlayer,
      ...
      if (!tr.startsolid && tr.fraction < 1.0) {
        ...
        for (int i = 0; i < INTRO_NUM_FAKE_PLAYERS; i++) {
          ...
          if (m_pFakePlayers[i]->InitializeAsClientEntity(
                  pszModels[i], RENDER_GROUP_OPAQUE_ENTITY)) {
