...
RecvPropInt(RECVINFO(m_Color)), RecvPropInt(RECVINFO(m_SpawnRate)),
    RecvPropFloat(RECVINFO(m_flSizeMin)), RecvPropFloat(RECVINFO(m_flSizeMax)),
    RecvPropInt(RECVINFO(m_LifetimeMin)), RecvPropInt(RECVINFO(m_LifetimeMax)),
    RecvPropInt(RECVINFO(m_DustFlags)), RecvPropInt(RECVINFO(m_SpeedMax)),
    RecvPropInt(RECVINFO(m_DistMax)), RecvPropInt(RECVINFO(m_nModelIndex)),
...
#define DUST_ACCEL 50
  ...
  const CFuncDustParticle *pParticle =
      (const CFuncDustParticle *)pIterator->GetFirst();
  while (pParticle) {
    // Velocity.
    float flAlpha;
    if (m_pDust->m_DustFlags & DUSTFLAGS_FROZEN) {
      ...
      // Alpha.
      float flAngle =
          (pParticle->m_flLifetime / pParticle->m_flDieTime) * M_PI * 2;
    ...
    if (-tPos.z <= m_pDust->m_DistMax) {
      ...
      RenderParticle_Color255Size(
          pIterator->GetParticleDraw(), tPos,
  ...
  while (pParticle) {
    ...
    if (!(m_pDust->m_DustFlags & DUSTFLAGS_FROZEN)) {
      ...
      if (pParticle->m_flLifetime >= pParticle->m_flDieTime) {
        ...
        // Apply velocity.
        pParticle->m_Pos.MulAdd(pParticle->m_Pos, pParticle->m_vVelocity,
                                pIterator->GetTimeDelta());
      }
    }
...
C_Func_Dust::C_Func_Dust() : m_Effect("C_Func_Dust") {
  ...
  m_Effect.SetDynamicallyAllocated(
      false); // So it doesn't try to delete itself.
}
...
bool C_Func_Dust::ShouldDraw() { return false; }
...
void C_Func_Dust::AttemptSpawnNewParticle() {
  ...
  for (int iTest = 0; iTest < nTests; iTest++) {
    ...
    Vector vTest =
        WorldAlignMins() + (WorldAlignMaxs() - WorldAlignMins()) * vPercent;

    int contents =
        enginetrace->GetPointContents_Collideable(GetCollideable(), vTest);
    if (contents & CONTENTS_SOLID) {
      CFuncDustParticle *pParticle =
          (CFuncDustParticle *)m_Effect.AddParticle(10, m_hMaterial, vTest);
      if (pParticle) {
        ...
        pParticle->m_flDieTime =
            RemapVal(rand(), 0, VALVE_RAND_MAX, m_LifetimeMin, m_LifetimeMax);

        if (m_DustFlags & DUSTFLAGS_SCALEMOTES)
          pParticle->m_flSize =
              RemapVal(rand(), 0, VALVE_RAND_MAX, m_flSizeMin / 10000.0f,
                       m_flSizeMax / 10000.0f);
        else
          pParticle->m_flSize =
              RemapVal(rand(), 0, VALVE_RAND_MAX, m_flSizeMin, m_flSizeMax);
...
void FX_Dust(const Vector &vecOrigin, const Vector &vecDirection, float flSize,
  ...
  for (int i = 0; i < numPuffs; i++) {
    ...
    particle.m_vecVelocity =
        vecDirection * random->RandomFloat(speed * 0.5f, speed) * i;
...
RecvPropFloat(RECVINFO(m_flSize)), RecvPropFloat(RECVINFO(m_flSpeed)),
