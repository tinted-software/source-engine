...
IMPLEMENT_CLIENTCLASS_EVENT_DT(C_TEFootprintDecal, DT_TEFootprintDecal,
                               CTEFootprintDecal)
RecvPropVector(RECVINFO(m_vecOrigin)), RecvPropVector(RECVINFO(m_vecDirection)),
    RecvPropInt(RECVINFO(m_nEntity)), RecvPropInt(RECVINFO(m_nIndex)),
...
void C_TEFootprintDecal::PostDataUpdate(DataUpdateType_t updateType) {
  ...
  if (r_decals.GetInt()) {
    ...
    if (ent) {
      effects->DecalShoot(m_nIndex, m_nEntity, ent->GetModel(),
                          ent->GetAbsOrigin(), ent->GetAbsAngles(), m_vecOrigin,
                          &m_vecDirection, 0);
    }
  }
}
...
void TE_FootprintDecal(IRecipientFilter &filter, float delay,
  ...
  if (r_decals.GetInt()) {
    ...
    if (ent) {
      effects->DecalShoot(index, entity, ent->GetModel(), ent->GetAbsOrigin(),
                          ent->GetAbsAngles(), *origin, right, 0);
    }
  }
}