...
// Robin HACKHACK: HL2 doesn't use deathmsgs, so I just forced these down below
// our minimap. It should be positioned by TF2/HL2 separately, and TF2 should
...
// Purpose:
//-----------------------------------------------------------------------------
void CHudDeathNotice::VidInit(void) { m_iconD_skull = gHUD.GetIcon("d_skull"); }
...
void CHudDeathNotice::Paint() {
  ...
  for (int i = 0; i < MAX_DEATHNOTICES; i++) {
    ...
    rgDeathNoticeList[i].flDisplayTime =
        MIN(rgDeathNoticeList[i].flDisplayTime,
            gpGlobals->curtime + DEATHNOTICE_DISPLAY_TIME);
...
void CHudDeathNotice::FireGameEvent(KeyValues *event) {
  ...
  char killedwith[32];
  Q_snprintf(killedwith, sizeof(killedwith), "d_%s",
  ...
  if (i == MAX_DEATHNOTICES) { // move the rest of the list forward to make room
                               // for this item
    Q_memmove(rgDeathNoticeList, rgDeathNoticeList + 1,
  ...
  if (killedwith && *killedwith && (*killedwith > 13) &&
      strcmp(killedwith, "d_world") && !rgDeathNoticeList[i].iTeamKill) {
    Msg(" with ");
