...
// no inputs, assumes that the results go into $CHEAPWATERSTARTDISTANCE and
...
bool CWaterLODMaterialProxy::Init(IMaterial *pMaterial, KeyValues *pKeyValues) {
  ...
  m_pCheapWaterStartDistanceVar =
      pMaterial->FindVar("$CHEAPWATERSTARTDISTANCE", &foundVar, false);
  if (!foundVar)
    return false;

  m_pCheapWaterEndDistanceVar =
      pMaterial->FindVar("$CHEAPWATERENDDISTANCE", &foundVar, false);
  if (!foundVar)
    return false;
...
EXPOSE_INTERFACE(CWaterLODMaterialProxy, IMaterialProxy,
                 "WaterLOD" IMATERIAL_PROXY_INTERFACE_VERSION);
