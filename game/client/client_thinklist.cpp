...
void CClientThinkList::LevelInitPreEntity() { m_nIterEnum = 0; }
...
//-----------------------------------------------------------------------------
void CClientThinkList::SetNextClientThink(ClientThinkHandle_t hThink,
                                          float flNextTime) {
...
void CClientThinkList::SetNextClientThink(ClientEntityHandle_t hEnt,
                                          float flNextTime) {
  if (flNextTime == CLIENT_THINK_NEVER) {
  ...
  IClientThinkable *pThink =
      ClientEntityList().GetClientThinkableFromHandle(hEnt);
  if (!pThink)
    return;
...
void CClientThinkList::RemoveThinkable(ClientThinkHandle_t hThink) {
  ...
  ThinkEntry_t *pEntry = GetThinkEntry(hThink);
  IClientThinkable *pThink =
      ClientEntityList().GetClientThinkableFromHandle(pEntry->m_hEnt);
  if (pThink) {
...
//-----------------------------------------------------------------------------
void CClientThinkList::PerformThinkFunction(ThinkEntry_t *pEntry,
                                            float flCurtime) {
  IClientThinkable *pThink =
      ClientEntityList().GetClientThinkableFromHandle(pEntry->m_hEnt);
  if (!pThink) {
...
//-----------------------------------------------------------------------------
void CClientThinkList::AddEntityToFrameThinkList(
    ThinkEntry_t *pEntry, bool bAlwaysChain, int &nCount,
  ...
  // If we're not thinking this frame, we don't have to worry about thinking
  // after our parents
  bool bThinkThisInterval =
      (pEntry->m_flNextClientThink == CLIENT_THINK_ALWAYS) ||
  ...
  C_BaseEntity *pEntity =
      ClientEntityList().GetBaseEntityFromHandle(pEntry->m_hEnt);
  if (pEntity) {
...
void CClientThinkList::PerformThinkFunctions() {
  VPROF_("Client Thinks", 1, VPROF_BUDGETGROUP_CLIENT_SIM, false,
         BUDGETFLAG_CLIENT);
  ...
  ThinkEntry_t **ppThinkEntryList =
      (ThinkEntry_t **)stackalloc(nMaxList * sizeof(ThinkEntry_t *));
  int nThinkCount = 0;
  for (unsigned short iCur = m_ThinkEntries.Head();
       iCur != m_ThinkEntries.InvalidIndex();
       iCur = m_ThinkEntries.Next(iCur)) {
    AddEntityToFrameThinkList(&m_ThinkEntries[iCur], false, nThinkCount,
                              ppThinkEntryList);
  ...
  for (i = 0; i < nCount; ++i) {
    ...
    if (hThink != INVALID_THINK_HANDLE) {
      ...
      SetNextClientThink(m_aChangeList[i].m_hEnt,
                         m_aChangeList[i].m_flNextTime);
    }
  }
...
void CClientThinkList::CleanUpDeleteList() {
  ...
  for (int iThink = 0; iThink < nThinkCount; ++iThink) {
    ...
    if (handle != ClientEntityList().InvalidHandle()) {
      C_BaseEntity *pEntity =
          ClientEntityList().GetBaseEntityFromHandle(handle);
      if (pEntity) {
        pEntity->SetRemovalFlag(false);
      }

      IClientThinkable *pThink =
          ClientEntityList().GetClientThinkableFromHandle(handle);
      if (pThink) {
        pThink->Release();
      }
    }
