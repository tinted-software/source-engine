...
static ConVar cl_customsounds("cl_customsounds", "1", 0,
...
ConVar spec_freeze_traveltime(
    ...
    0.01, false, 0);
ConVar
    spec_freeze_distance_min("spec_freeze_distance_min", "80", FCVAR_CHEAT,
                             "Minimum random distance from the target to stop "
                             "when framing them in observer freeze cam.");
ConVar
    spec_freeze_distance_max("spec_freeze_distance_max", "90", FCVAR_CHEAT,
                             "Maximum random distance from the target to stop "
                             "when framing them in observer freeze cam.");
#else
...
ConVar spec_freeze_traveltime(
    ...
    0.01, false, 0);
ConVar
    spec_freeze_distance_min("spec_freeze_distance_min", "96", FCVAR_CHEAT,
                             "Minimum random distance from the target to stop "
                             "when framing them in observer freeze cam.");
ConVar
    spec_freeze_distance_max("spec_freeze_distance_max", "200", FCVAR_CHEAT,
                             "Maximum random distance from the target to stop "
                             "when framing them in observer freeze cam.");
#endif
...
CBaseEntity *
C_BasePlayer::GetObserverTarget() const // returns players target or NULL
{
#ifndef _XBOX
  if (IsHLTV()) {
  ...
  if (GetObserverMode() == OBS_MODE_ROAMING) {
    ...
    if (IsLocalPlayer() && UseVR()) {
      ...
      case OBS_MODE_FREEZECAM: // zooms to a target, and freeze-frames on them
        // These are both terrible - they get overriden to chase, but here we
        // change it to "chase" your own body (which will be ragdolled).
...
void C_BasePlayer::SetObserverTarget(EHANDLE hObserverTarget) {
  ...
  if (hObserverTarget.GetEntryIndex() != m_hObserverTarget.GetEntryIndex() ||
    ...
    if (IsLocalPlayer()) {
      // On a change of viewing mode or target, we may want to reset both head
      // and torso to point at the new target.
...
void C_BasePlayer::CalcChaseCamView(Vector &eyeOrigin, QAngle &eyeAngles,
  ...
  if (GetObserverMode() == OBS_MODE_IN_EYE) {
    ...
    if (UseVR()) {
      // Don't let people play with the pitch - they drive it into the ground or
      // into the air and it's distracting at best, nauseating at worst (e.g.
      // when it clips through the ground plane).
  ...
  if (target) {
    ...
    if (pTargetAnimating) {
      float flScaleSquared =
          pTargetAnimating->GetModelScale() * pTargetAnimating->GetModelScale();
  ...
  if (target && !target->IsPlayer() && target->IsNextBot()) {
    // if this is a boss, we want to be back a little further so we can see more
    // of it
    flMaxDistance *= 2.5f;
    m_flObserverChaseDistance = flMaxDistance;
  }
...
void C_BasePlayer::CalcInEyeCamView(Vector &eyeOrigin, QAngle &eyeAngles,
...
#endif
  {
    ...
    if (target->GetFlags() & FL_DUCKING) {
      eyeOrigin += pTargetAnimating ? VEC_DUCK_VIEW_SCALED(pTargetAnimating)
                                    : VEC_DUCK_VIEW;
    } else {
      eyeOrigin +=
...
bool C_BasePlayer::InFirstPersonView() {
  ...
  // If this is who we're observing in first person, it's counted as the "local"
  // player.
...
void C_BasePlayer::PlayPlayerJingle() {
  ...
  if (!filesystem->FileExists(fullsoundname)) {
    char custname[512];
    Q_snprintf(custname, sizeof(custname), "download/user_custom/%c%c/%s.dat",
               soundhex[0], soundhex[1], soundhex);
...
#else // old
      pID->InstancedSet(pi.friendsID, 1,
...
void C_BasePlayer::BuildFirstPersonMeathookTransformations(
  ...
  // rendering the main view(s), meathook and headless. If we're in first-person
  // view rendering shadowbuffers/reflections, don't do anything special either
  // (we could do meathook but with a head?)
  if (IsAboutToRagdoll()) {
    // We're re-animating specifically to set up the ragdoll.
    // Meathook can push the player through the floor, which makes the ragdoll
    // fall through the world, which is no good. So do nothing.
    return;
  }
  ...
  // If we aren't drawing the player anyway, don't mess with the bones. This can
  // happen in Portal.
  if (!ShouldDrawThisPlayer()) {
    return;
  }
  ...
  // We can't move this with animations or effects without causing nausea, so we
  // need to move the whole body so that the animated head is in the right place
  // to match the player-controlled head.
  Vector vHeadUp;
  ...
  if (UseVR()) {
    ...
    // * Go downwards in that space by cl_meathook_neck_pivot_ingame_* - this is
    // now the neck-pivot in the game world of where the player is actually
    // looking.
    // * Now place the body of the animated character so that the head bone is
    // at that position. The head bone is the neck pivot point of the in-game
    // character.
    ...
    vRealPivotPoint = MainViewOrigin() -
  ...
  // This prevents us rendering junk all over the screen, e.g. inside of mouth,
  // etc.
  ...
  // For example, for one sniper taunt he takes his hat off and waves it - would
  // be nice to see it then.
