...
public:
  ...
  static C_RopeKeyframe *
  Create(C_BaseEntity *pStartEnt, C_BaseEntity *pEndEnt,
  ...
  // CSimplePhysics::IHelper. The default implementation is returned so you can
  // call through to it if you want.
  CSimplePhysics::IHelper *HookPhysics(CSimplePhysics::IHelper *pHook);

  // Attach to things (you can also just lock the endpoints down yourself if you
  // hook the physics).
  ...
  void BuildRope(RopeSegData_t *pRopeSegment, const Vector &vCurrentViewForward,
...
private:
  ...
  Vector
      m_LightValues[ROPE_MAX_SEGMENTS]; // light info when the rope is created.
  ...
  bool m_bConstrainBetweenEndpoints; // Simulated segment points won't stretch
