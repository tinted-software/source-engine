...
RecvPropVector(RECVINFO(m_vecOrigin)),
    ...
    RecvPropInt(RECVINFO(m_nRandomization)), RecvPropInt(RECVINFO(m_nCount)),
    RecvPropFloat(RECVINFO(m_fTime)), RecvPropInt(RECVINFO(m_nFlags)),
    END_RECV_TABLE()
...
// Recording
//-----------------------------------------------------------------------------
static inline void RecordBreakModel(const Vector &start, const QAngle &angles,
                                    const Vector &size, const Vector &vel,
                                    int nModelIndex, int nRandomization,
                                    int nCount, float flDuration, int nFlags) {
  if (!ToolsEnabled())
    return;
...
void TE_BreakModel(IRecipientFilter &filter, float delay, const Vector &pos,
                   ...
                   int flags) {
  tempents->BreakModel(pos, angles, size, vel, randomization, time, count,
                       modelindex, flags);
  RecordBreakModel(pos, angles, size, vel, randomization, time, count,
                   modelindex, flags);
}
...
void C_TEBreakModel::PostDataUpdate(DataUpdateType_t updateType) {
  ...
  tempents->BreakModel(m_vecOrigin, m_angRotation, m_vecSize, m_vecVelocity,
                       m_nRandomization, m_fTime, m_nCount, m_nModelIndex,
                       m_nFlags);
...
void TE_BreakModel(IRecipientFilter &filter, float delay,
  ...
  TE_BreakModel(filter, 0.0f, vecOrigin, angles, vecSize, vecVel, nModelIndex,
                nRandomization, nCount, flDuration, nFlags);
}
