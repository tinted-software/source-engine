...
static ConVar
    cl_predictionentitydumpbyclass("cl_pclass", "", FCVAR_CHEAT,
                                   "Dump entity by prediction classname.");
static ConVar cl_pred_optimize(
    "cl_pred_optimize", "2", 0,
    "Optimize for not copying data if didn't receive a network update (1), and "
    ...
    for (i = 0; i < c; i++) {
      ...
      if (showlist) {
        ...
        if (showlist >= 2) {
          ...
          engine->Con_NXPrintf(&np, "%15s %30s (%5i / %5i bytes): %15s", sz,
                               ent->GetClassname(), size, intermediate_size,
                               ent->GetPredictable() ? "predicted"
                                                     : "client created");
