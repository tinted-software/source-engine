...
//
//			This class encompasses both entities that are created on
//the server 			and networked to the client AND entities that are created on the
//			client.
//
// $NoKeywords: $
//===========================================================================//
...
public:
  ...
  virtual bool
  InSameTeam(C_BaseEntity *pEntity); // Returns true if the specified entity is
...
public:
  ...
  virtual const QAngle &
  LocalEyeAngles(void); // Direction of eyes in local space (pl.v_angle)
  ...
  void
  SetMoveType(MoveType_t val,
              MoveCollide_t moveCollide =
                  MOVECOLLIDE_DEFAULT); // Set to one of the MOVETYPE_ defines.
  void
  SetMoveCollide(MoveCollide_t val); // Set to one of the MOVECOLLIDE_ defines.
  ...
  //&vecAngAbsVelocity ); 	const QAngle&		GetAbsAngularVelocity( ) const;
...
public:
  ...
  float *GetRenderClipPlane(
      void); // Rendering clip plane, should be 4 floats, return value of NULL
             // indicates a disabled render clip plane

protected:
  void AddToInterpolationList();
  void RemoveFromInterpolationList();
  unsigned short m_InterpolationListEntry; // Entry into g_InterpolationList (or
...
inline const QAngle &
C_BaseEntity::EyeAngles(void) const // Direction of eyes in world space
{
  return const_cast<C_BaseEntity *>(this)->EyeAngles();
}

inline const QAngle &
C_BaseEntity::LocalEyeAngles(void) const // Direction of eyes
{
