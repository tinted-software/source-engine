...
//
// Purpose: Implements the Sticky Bolt code. This constraints ragdolls to the
// world
//			after being hit by a crossbow bolt. If something here is
//acting funny 			let me know - Adrian.
//
// $Workfile:     $
// $Date:         $
//
...
public:
  ...
  IterationRetval_t EnumElement(IHandleEntity *pHandleEntity) {
    ...
    if (tr.fraction < 1.0) {
      ...
      pPhysicsObject->WorldToLocal(&ballsocket.constraintPosition[1],
                                   tr.endpos);

      physenv->CreateBallsocketConstraint(pReference, pPhysicsObject, NULL,
                                          ballsocket);
...
void CreateCrossbowBolt(const Vector &vecOrigin, const Vector &vecDirection) {
  ...
  if (gpGlobals->maxClients > 1) {
    tempents->SpawnTempModel(pModel, vecOrigin - vecDirection * 8, vAngles,
                             Vector(0, 0, 0), 30.0f, FTENT_NONE);
  } else {
    tempents->SpawnTempModel(pModel, vecOrigin - vecDirection * 8, vAngles,
                             Vector(0, 0, 0), 1, FTENT_NEVERDIE);
  }
}
...
void StickRagdollNow(const Vector &vecOrigin, const Vector &vecDirection) {
  ...
  UTIL_TraceLine(vecOrigin, vecOrigin + vecDirection * 16, MASK_SOLID_BRUSHONLY,
