...
void DrawSpriteModel(IClientEntity *baseentity, CEngineSprite *psprite,
  ...
  if (ShouldDrawInWireFrameMode() || r_drawsprites.GetInt() == 2) {
    IMaterial *pMaterial = materials->FindMaterial("debug/debugspritewireframe",
                                                   TEXTURE_GROUP_OTHER);
  ...
  IMaterialVar *pHDRColorScaleVar =
      material->FindVarFast("$HDRCOLORSCALE", &s_nHDRColorScaleCache);
  if (pHDRColorScaleVar) {
    pHDRColorScaleVar->SetVecValue(flHDRColorScale, flHDRColorScale,
                                   flHDRColorScale);
  }
...
//-----------------------------------------------------------------------------
// Purpose: Determine glow brightness/scale based on distance to render origin
// and trace results Input  : entorigin -
//			rendermode -
//			renderfx -
//			alpha -
//			pscale - Pointer to the value for scale, will be changed
//based on distance and rendermode.
//-----------------------------------------------------------------------------
float StandardGlowBlend(const pixelvis_queryparams_t &params,
  ...
  if (rendermode != kRenderWorldGlow) {
    // Make the glow fixed size in screen space, taking into consideration the
    // scale setting.
    if (*pscale == 0.0f) {
      *pscale = 1.0f;
    }
...
float C_SpriteRenderer::GlowBlend(CEngineSprite *psprite,
                                  const Vector &entorigin, int rendermode,
                                  int renderfx, int alpha, float *pscale) {
  pixelvis_queryparams_t params;
  float aspect = SpriteAspect(psprite);
  params.Init(entorigin, PIXELVIS_DEFAULT_PROXY_SIZE, aspect);
  return StandardGlowBlend(params, &m_queryHandle, rendermode, renderfx, alpha,
                           pscale);
}

// since sprites can network down a glow proxy size, handle that here
float CSprite::GlowBlend(CEngineSprite *psprite, const Vector &entorigin,
...
//			up -
//-----------------------------------------------------------------------------
void C_SpriteRenderer::GetSpriteAxes(SPRITETYPE type, const Vector &origin,
  ...
  case SPR_VP_PARALLEL_UPRIGHT: {
    ...
    // vectors and starts to approach an undefined state, so we don't draw if
    // the two vectors are less than 1 degree apart
    dot = CurrentViewForward()[2]; // same as DotProduct (vpn,
                                   // r_spritedesc.g_vecVUp) because
    //  r_spritedesc.vup is 0, 0, 1
    if ((dot > 0.999848f) || (dot < -0.999848f)) // cos(1 degree) = 0.999848
      return;
    up[0] = 0;
...
int C_SpriteRenderer::DrawSprite(IClientEntity *entity, const model_t *model,
                                 ...
                                 float frame, IClientEntity *attachedto,
                                 int attachmentindex, int rendermode,
                                 int renderfx, int alpha, int r, int g, int b,
                                 float scale, float flHDRColorScale) {
  VPROF_BUDGET("C_SpriteRenderer::DrawSprite",
               VPROF_BUDGETGROUP_PARTICLE_RENDERING);

  if (!r_drawsprites.GetBool() || !model ||
      modelinfo->GetModelType(model) != mod_sprite) {
    return 0;
  }
  ...
  if (rendermode != kRenderNormal) {
    ...
    if ((rendermode == kRenderGlow) || (rendermode == kRenderWorldGlow)) {
      blend *= GlowBlend(psprite, effect_origin, rendermode, renderfx, alpha,
                         &scale);
  ...
  GetSpriteAxes((SPRITETYPE)psprite->GetOrientation(), origin, angles, forward,
                right, up);

  // Draw
  DrawSpriteModel(entity, psprite, effect_origin, scale, frame, rendermode, r,
                  g, b, alpha, forward, right, up, flHDRColorScale);
...
void CSprite::GetToolRecordingState(KeyValues *msg) {
  ...
  if (m_hAttachedToEntity) {
    ...
    if (ent) {
      BaseEntityRecordingState_t *pState =
  ...
  state.m_Color.SetColor(m_clrRender.GetR(), m_clrRender.GetG(),
                         m_clrRender.GetB(), GetRenderBrightness());
