...
static ConVar net_graphtext("net_graphtext", "1", FCVAR_ARCHIVE,
                            "Draw text fields");
static ConVar
    net_graphmsecs("net_graphmsecs", "400", FCVAR_ARCHIVE,
...
void CNetGraphPanel::OnFontChanged() {
  ...
  if (m_hFontProportional == vgui::INVALID_FONT) {
    ...
    g_pMatSystemSurface->GetTextSize(m_hFontProportional, ustr,
                                     m_EstimatedWidth, textTall);
  }
...
void CNetGraphPanel::DrawTextFields(int graphvalue, int x, int y, int w,
  ...
  g_pMatSystemSurface->DrawColoredText(font, x + textWidth, y, interpcolor[0],
                                       interpcolor[1], interpcolor[2], 255,
                                       "%s", sz);
  ...
  if (graphvalue > 2) {
    ...
    if (graphvalue > 3) {
      ...
      g_pMatSystemSurface->DrawColoredText(font, x, y, servercolor[0],
                                           servercolor[1], servercolor[2], 255,
                                           "%s", sz);
...
void CNetGraphPanel::GraphGetXY(vrect_t *rect, int width, int *x, int *y) {
  *x = rect->x + 5;
  ...
  default:
    *x = rect->x +
         clamp((int)XRES(net_graphpos.GetInt()), 5, rect->width - width - 5);
  }
...
void CNetGraphPanel::Paint() {
  ...
  // Don't display net_graph if taking freezecam screenshot and
  // hud_freezecamhide is enabled
