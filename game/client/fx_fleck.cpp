...
// enable this to have the fleck_merge cvar as well as the current system count
// displayed as it changes (for profiling)
...
//{
// public:
//	bool MergeParticleSystems( CFleckParticles *pSystem, const char
//*pEffectName, const Vector &center, const Vector &extents ) 	{ merge; return
//true; }
//};

// a singly linked list through all particle effects of a specific type
// with a specific rule for sharing them.
// Needs a hook to the particle effect's constructor/destructor and factory
// method The factory needs to support optionally merging the new particles into
// a previously built particle effect this cuts down on lots of scene management
// overhead as well as rendering/batch overhead
...
public:
  ...
  PARTICLE_EFFECT *FindAndMergeParticleSystem(const char *pEffectName,
                                              const Vector &center,
                                              const Vector &extents);
  bool MergeParticleSystems(PARTICLE_EFFECT *pSystem, const char *pEffectName,
                            const Vector &center, const Vector &extents);
...
template <class PARTICLE_EFFECT, class PARTICLE_MERGE>
void CParticleMergeList<PARTICLE_EFFECT, PARTICLE_MERGE>::AddParticleSystem(
    PARTICLE_EFFECT *pSystem) {
#if REPORT_MERGED_FLECKS
  g_PCount++;
  Msg("PS: %d\n", g_PCount);
#endif
...
template <class PARTICLE_EFFECT, class PARTICLE_MERGE>
void CParticleMergeList<PARTICLE_EFFECT, PARTICLE_MERGE>::RemoveParticleSystem(
    PARTICLE_EFFECT *pRemove) {
#if REPORT_MERGED_FLECKS
  g_PCount--;
  Msg("PS: %d\n", g_PCount);
#endif
...
template <class PARTICLE_EFFECT, class PARTICLE_MERGE>
PARTICLE_EFFECT *
CParticleMergeList<PARTICLE_EFFECT, PARTICLE_MERGE>::FindAndMergeParticleSystem(
  ...
  for (PARTICLE_EFFECT *pMerge = m_pHead; pMerge != NULL;
       pMerge = pMerge->m_pNextParticleSystem) {
...
template <class PARTICLE_EFFECT> class CMergeSameNameBbox {
public:
  bool MergeParticleSystems(PARTICLE_EFFECT *pSystem, const char *pEffectName,
                            const Vector &center, const Vector &extents) {
...
CParticleMergeList<CFleckParticles, CMergeSameNameBbox<CFleckParticles>>
    g_FleckMergeList;

//
// CFleckParticles
//
CSmartPtr<CFleckParticles> CFleckParticles::Create(const char *pDebugName,
                                                   const Vector &vCenter,
                                                   const Vector &extents) {
  CFleckParticles *pMerge =
      g_FleckMergeList.FindAndMergeParticleSystem(pDebugName, vCenter, extents);
  if (pMerge)
    return pMerge;
...
//-----------------------------------------------------------------------------
void CFleckParticles::Setup(const Vector &origin, const Vector *direction,
...
void CFleckParticles::RenderParticles(CParticleRenderIterator *pIterator) {
  ...
  while (pParticle) {
    ...
    RenderParticle_ColorSizeAngle(
        pIterator->GetParticleDraw(), tPos, color,
        1.0f - (pParticle->m_flLifetime / pParticle->m_flDieTime),
        pParticle->m_uchSize, pParticle->m_flRoll);
...
void CFleckParticles::SimulateParticles(CParticleSimulateIterator *pIterator) {
  ...
  while (pParticle) {
    ...
    if (pParticle->m_flLifetime >= pParticle->m_flDieTime) {
      ...
      trace_t trace;
      m_ParticleCollision.MoveParticle(
          pParticle->m_Pos, pParticle->m_vecVelocity, &pParticle->m_flRollDelta,
          timeDelta, &trace);
