...
enum ETouchState { state_none = 0, state_edit, state_edit_move };

enum ETouchRound { round_none = 0, round_grid, round_aspect };

struct rgba_t {
  rgba_t(unsigned char r, unsigned char g, unsigned char b,
         unsigned char a = 255)
      : r(r), g(g), b(b), a(a) {}
...
public:
  VTouchPanel(void) { touchPanel = NULL; }

  void Create(vgui::VPANEL parent) { touchPanel = new CTouchPanel(parent); }
...
public:
  ...
  void AddButton(const char *name, const char *texturefile, const char *command,
                 float x1, float y1, float x2, float y2,
                 rgba_t color = rgba_t(255, 255, 255, 255), int round = 2,
                 float aspect = 1.f, int flags = 0);
  ...
  void GetTouchAccumulators(float *forward, float *side, float *yaw,
                            float *pitch);
