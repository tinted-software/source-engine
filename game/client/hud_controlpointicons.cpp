...
void CControlPointIconPulseable::ApplySchemeSettings(IScheme *pScheme) {
  ...
  if (!m_pPulseImage) {
    m_pPulseImage =
        scheme()->GetImage("../sprites/obj_icons/icon_obj_white", true);
  }
}
  ...
  if (m_flStartCapAnimStart && gpGlobals->curtime > m_flStartCapAnimStart) {
    ...
    if (m_bAccelerateOverCapture) {
      float flCapPercentage =
          ObjectiveResource()->GetCPCapPercentage(m_iCPIndex);
...
// Purpose:
//-----------------------------------------------------------------------------
void CControlPointIconPulseable::StartPulsing(float flDelay, float flPulseTime,
                                              bool bAccelerate) {
...
// Purpose:
//-----------------------------------------------------------------------------
CControlPointIcon::CControlPointIcon(Panel *parent, const char *pName,
                                     int iIndex)
...
void CControlPointIcon::ApplySchemeSettings(IScheme *pScheme) {
  ...
  if (!m_pCapHighlightImage) {
    m_pCapHighlightImage =
        new CControlPointIconSwoop(this, "CapHighlightImage");
  ...
  if (!m_pBaseImage) {
    m_pBaseImage =
        new CControlPointIconPulseable(this, "BaseImage", m_iCPIndex);
  ...
  m_pCapPlayerImage =
      dynamic_cast<vgui::ImagePanel *>(FindChildByName("CapPlayerImage"));
  m_pCapNumPlayers =
      dynamic_cast<vgui::Label *>(FindChildByName("CapNumPlayers"));
  m_pOverlayImage =
      dynamic_cast<vgui::ImagePanel *>(FindChildByName("OverlayImage"));
...
void CControlPointIcon::UpdateImage(void) {
  ...
  if (m_pBaseImage) {
    int iOwnerIcon =
        ObjectiveResource()->GetCPCurrentOwnerIcon(m_iCPIndex, iOwner);
  ...
  if (m_pOverlayImage) {
    int iOverlayIcon =
        ObjectiveResource()->GetOverlayForTeam(m_iCPIndex, iOwner);
    if (iOverlayIcon) {
...
void CControlPointIcon::UpdateCapImage(void) {
  ...
  if (m_pCapImage) {
    ...
    if (iCappingTeam != TEAM_UNASSIGNED && iCappingTeam != iOwningTeam) {
      const char *pszCapSwipe =
          ObjectiveResource()->GetGameSpecificCPCappingSwipe(m_iCPIndex,
                                                             iCappingTeam);
      if (m_bSwipeUp) {
...
//-----------------------------------------------------------------------------
// Purpose: Lock cap points when neither team can cap them for map-specific
// reasons
//-----------------------------------------------------------------------------
bool CControlPointIcon::IsPointLocked(void) {
  bool bAnyTeamCanCap = false;
  for (int gameteam = FIRST_GAME_TEAM; gameteam < GetNumberOfTeams();
       gameteam++) {
...
bool CControlPointIcon::IsPointUnlockCountdownRunning(void) {
  if (m_pCountdown && (TeamplayRoundBasedRules() &&
                       !TeamplayRoundBasedRules()->IsInWaitingForPlayers())) {
...
void CControlPointIcon::FakePulse(float flTime) {
  if (m_pCapPulseImage) {
    ...
    m_pBaseImage->StartPulsing(
        flTime + FINISHCAPANIM_SWOOP_LENGTH - gpGlobals->curtime, 0.8, false);
...
void CControlPointIcon::PerformLayout(void) {
  ...
  if (m_pCapHighlightImage->IsVisible()) {
    ...
    m_pCapHighlightImage->SetBounds(iBaseXPos + CAP_BOX_INDENT_X,
                                    iBaseYPos - iHeight,
                                    GetWide() - (CAP_BOX_INDENT_X * 2),
                                    iHeight + GetTall() - CAP_BOX_INDENT_Y);
  }

  int iCappingTeam = ObjectiveResource()->GetCappingTeam(m_iCPIndex);
  int iPlayers =
      ObjectiveResource()->GetNumPlayersInArea(m_iCPIndex, iCappingTeam);
...
CHudControlPointIcons::CHudControlPointIcons(const char *pName)
  ...
  if (m_iBackgroundTexture == -1) {
    ...
    vgui::surface()->DrawSetTextureFile(m_iBackgroundTexture, "vgui/white",
                                        true, true);
  }
...
void CHudControlPointIcons::FireGameEvent(IGameEvent *event) {
  ...
  if (FStrEq("controlpoint_starttouch", eventname)) {
    ...
    if (pPlayer && iPlayer == pPlayer->entindex()) {
      ...
      m_flFakeCaptureTime =
          gpGlobals->curtime + FAKE_CAPTURE_TIME + FAKE_CAPTURE_POST_PAUSE;
...
void CHudControlPointIcons::PerformLayout(void) {
  ...
  memset(iCapPointLines, 0,
         sizeof(int) * MAX_CONTROL_POINTS * MAX_CONTROL_POINTS);
  ...
  if (ObjectiveResource()) {
    ...
    if (pszLayout && pszLayout[0]) {
      ...
      // Cap layout is a string with indexes of cap points seperated by ',' to
      // denote a new line. So "3,1 2" would create a pyramid, with cap point 3
      // on the first line, and caps 1 & 2 on the second line.
      int iLine = 0;
      int iCapIndex = 0;
  ...
  if (ObjectiveResource()) {
    ...
    ObjectiveResource()->GetCapLayoutCustomPosition(flCustomPositionX,
                                                    flCustomPositionY);
  ...
  for (int iLine = 0; iLine < MAX_CONTROL_POINTS; iLine++) {
    ...
    int iLineYPos =
        (iLine * m_iIconGapHeight) + (iLine * iTallest) + m_iIconGapHeight;
    for (int iPosition = 0; iPosition < MAX_CONTROL_POINTS; iPosition++) {
      int iIconIndex = iCapPointLines[iLine][iPosition];
      if (!iIconIndex)
        break;
      ...
      if (iIconIndex >= 0 && iIconIndex < m_Icons.Count()) {
        ...
        int iDir = CP_DIR_N;
        if (iLine > 0) {
          if (((float)(iPosition + 1) / (float)iTotalIconsPerLine[iLine]) >
              0.5) {
            iDir = CP_DIR_NE;
          } else {
            iDir = CP_DIR_NW;
...
void CHudControlPointIcons::UpdateProgressBarFor(int iIndex) {
  // If they tell us to update all progress bars, update only the one we're
  // standing on
  if (iIndex == -1) {
    iIndex = m_iCurrentCP;
  }
  ...
  if (pStatus && pStatus->GetControlPointProgressBar()) {
    CControlPointProgressBar *pProgressBar =
        pStatus->GetControlPointProgressBar();
    if (!IsVisible() || iIndex < 0 ||
        iIndex >= ObjectiveResource()->GetNumControlPoints()) {
...
//-----------------------------------------------------------------------------
// Purpose: Create the icons we need to display the state of this map's control
// points
//-----------------------------------------------------------------------------
void CHudControlPointIcons::InitIcons(void) {
  ...
  for (int i = 0; i < iPoints; i++) {
    if (ObjectiveResource()->IsInMiniRound(i) &&
      ...
      CControlPointIcon *pIcon =
          new CControlPointIcon(this, VarArgs("ControlPointIcon%d", i), i);
...
void CHudControlPointIcons::ShutdownIcons(void) {
  ...
  // if we remove all the icons, we need to make sure the progress bar isn't
  // holding onto one
  CTFHudObjectiveStatus *pStatus = GET_HUDELEMENT(CTFHudObjectiveStatus);
  if (pStatus) {
    CControlPointProgressBar *pProgressBar =
...
// Purpose:
//-----------------------------------------------------------------------------
void CHudControlPointIcons::DrawBackgroundBox(int xpos, int ypos, int nBoxWidth,
...
//-----------------------------------------------------------------------------
bool CHudControlPointIcons::PaintTeamBaseIcon(int index, float flXPos,
                                              float flYPos, float flIconSize) {
  float uv1 = 0.0f;
  float uv2 = 1.0f;
  ...
  for (int i = 0; i < MAX_TEAMS; i++) {
    if (ObjectiveResource()->GetBaseControlPointForTeam(i) == index) {
      ...
      if (iTeamBaseIcon) {
        ...
        vgui::surface()->DrawSetTextureFile(m_iTeamBaseTextures[i], szMatName,
                                            true, false);
...
void CControlPointProgressBar::ApplySchemeSettings(IScheme *pScheme) {
  ...
  m_pBar =
      dynamic_cast<vgui::CircularProgressBar *>(FindChildByName("ProgressBar"));
...
bool CControlPointProgressBar::IsVisible(void) {
  ...
  if (m_iMidGroupIndex != -1 &&
      gHUD.IsRenderGroupLockedFor(NULL, m_iMidGroupIndex))
    return false;
...
void CControlPointProgressBar::SetupForPoint(CControlPointIcon *pIcon) {
  ...
  bool bInWinState = TeamplayRoundBasedRules()
                         ? TeamplayRoundBasedRules()->RoundHasBeenWon()
                         : false;
  ...
  if (m_pAttachedToIcon && !bInWinState) {
    ...
    if (!bCapBlocked && iCappingTeam != TEAM_UNASSIGNED &&
        iCappingTeam != iOwnerTeam && iCappingTeam == iPlayerTeam) {
      m_pBar->SetBgImage(
          ObjectiveResource()->GetGameSpecificCPBarBG(iCP, iCappingTeam));
      m_pBar->SetFgImage(
          ObjectiveResource()->GetGameSpecificCPBarFG(iCP, iOwnerTeam));
...
void CControlPointProgressBar::UpdateBarText(void) {
  ...
  if (mp_blockstyle.GetInt() == 1 && iCappingTeam != TEAM_UNASSIGNED &&
      iCappingTeam != iPlayerTeam) {
  ...
  if (ObjectiveResource()->GetOwningTeam(iCP) == iPlayerTeam) {
    // If the opponents can never recapture this point back, we use a different
    // string
    if (iPlayerTeam != TEAM_UNASSIGNED) {
      int iEnemyTeam =
          (iPlayerTeam == TF_TEAM_RED) ? TF_TEAM_BLUE : TF_TEAM_RED;
  ...
  if (!TeamplayGameRules()->TeamMayCapturePoint(iPlayerTeam, iCP)) {
    if (TeamplayRoundBasedRules() &&
        TeamplayRoundBasedRules()->IsInArenaMode() == true) {
      m_pBarText->SetText("#Team_Capture_NotNow");
    } else {
  ...
  char szReason[256];
  if (!TeamplayGameRules()->PlayerMayCapturePoint(pPlayer, iCP, szReason,
                                                  sizeof(szReason))) {
  ...
  if (mp_capstyle.GetInt() != 1) {
    int nNumTeammates =
        ObjectiveResource()->GetNumPlayersInArea(iCP, iPlayerTeam);
    int nRequiredTeammates =
        ObjectiveResource()->GetRequiredCappers(iCP, iPlayerTeam);
...
// Purpose:
//-----------------------------------------------------------------------------
CControlPointIconCapArrow::CControlPointIconCapArrow(CControlPointIcon *pIcon,
                                                     Panel *parent,
                                                     const char *name)
...
void CControlPointIconCapArrow::Paint(void) {
  ...
  int iCappingTeam =
      ObjectiveResource()->GetCappingTeam(m_pAttachedToIcon->GetCapIndex());
  float flCapPercentage =
      ObjectiveResource()->GetCPCapPercentage(m_pAttachedToIcon->GetCapIndex());

  // The image needs to be remapped to the width of the underlying box,
  // because we want the arrow head to move off the box at the exact time the
  // cap ends.
  ...
  // going until the arrow is fully off the edge. So we include extra width for
  // it.
  float flImageSize = (flArrowHeadPixelWidth + flArrowBodyPixelWidth);
  float flMovementInTextureSpace =
      ((flBoxSize + flArrowHeadPixelWidth) / flImageSize);
  float flArrowSizeInTextureSpace = (flArrowHeadPixelWidth / flImageSize);

  // To help players spot the start & end of a cap, we indent a little on either
  // side.
  float flIndent = 0.07;

  if (m_pAttachedToIcon->ShouldSwipeUp()) {
    flYa = RemapVal(flCapPercentage, 0.0, 1.0,
                    -flMovementInTextureSpace - flIndent,
                    flArrowSizeInTextureSpace - flIndent);
    flYb = RemapVal(flCapPercentage, 0.0, 1.0, flIndent,
                    flMovementInTextureSpace - flIndent);
  } else {
    flIndent = 0.1;
    ...
    if (bSwipeLeftToRight) {
      flXa = RemapVal(flCapPercentage, 0.0, 1.0,
                      flStart + flMovementInTextureSpace,
      ...
      flXb = RemapVal(flCapPercentage, 0.0, 1.0,
                      flStart + flMovementInTextureSpace,
                      flEnd - flArrowSizeInTextureSpace);
    }
  }
...
void CControlPointCountdown::PerformLayout() {
  CExLabel *pLabel =
      dynamic_cast<CExLabel *>(FindChildByName("CapCountdownLabel"));
  if (pLabel) {
