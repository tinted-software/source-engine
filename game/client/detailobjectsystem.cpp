...
CDetailObjectSystem::CDetailObjectSystem()
    : m_DetailSpriteDict(0, 32), m_DetailObjectDict(0, 32),
...
void CDetailObjectSystem::UnserializeDetailSprites(CUtlBuffer &buf) {
  ...
  while (--count >= 0) {
    ...
    ::V_swap(m_DetailSpriteDictFlipped[flipi].m_TexUL.x,
...
void CDetailObjectSystem::UnserializeModels(CUtlBuffer &buf) {
  ...
  if (nMaxFastInLeaf) {
    ...
    m_pBuildoutBuffer = reinterpret_cast<FastSpriteQuadBuildoutBufferX4_t *>(
        MemAlloc_AllocAligned((1 + nMaxFastInLeaf / 4) *
      ...
      while (nCount) {
        ...
        while (nToDraw--) {
          ...
          pquad = (FastSpriteQuadBuildoutBufferNonSIMDView_t const
                       *)(((intp)(pquad)) + (nSubIdx << 2));
          uint8 const *pColorsCasted =
              reinterpret_cast<uint8 const *>(pquad->m_Alpha);
          ...
          if (nSubIdx == 1)
            pDict =
                ((FastSpriteQuadBuildoutBufferNonSIMDView_t *)((intp)pquad + 4))
                    ->m_pSpriteDefs[0];
          else if (nSubIdx == 3)
            pDict =
                ((FastSpriteQuadBuildoutBufferNonSIMDView_t *)((intp)pquad - 4))
...
void CDetailObjectSystem::RenderFastTranslucentDetailObjectsInLeaf(
  ...
  while (nCount && (pDraw->m_flDistance >= flMinDistance)) {
    ...
    while (nToDraw--) {
      ...
      // couple of floats to handle sub-index
      pquad =
          (FastSpriteQuadBuildoutBufferNonSIMDView_t const *)(((intp)(pquad)) +
                                                              (nSubIdx << 2));
...
#ifdef PLATFORM_64BITS
      if (nSubIdx == 1)
        pDict = ((FastSpriteQuadBuildoutBufferNonSIMDView_t *)((intp)pquad + 4))
                    ->m_pSpriteDefs[0];
      else if (nSubIdx == 3)
        pDict = ((FastSpriteQuadBuildoutBufferNonSIMDView_t *)((intp)pquad - 4))
                    ->m_pSpriteDefs[0];
