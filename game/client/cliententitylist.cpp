...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CClientEntityList, IClientEntityList,
...
bool PVSNotifierMap_LessFunc(IClientUnknown *const &a,
                             IClientUnknown *const &b) {
  return a < b;
}
...
// Purpose:
//-----------------------------------------------------------------------------
CClientEntityList::~CClientEntityList(void) { Release(); }
...
int CClientEntityList::GetMaxEntities(void) { return m_iMaxServerEnts; }
...
IClientNetworkable *
CClientEntityList::GetClientNetworkableFromHandle(ClientEntityHandle_t hEnt) {
  IClientUnknown *pEnt = GetClientUnknownFromHandle(hEnt);
  return pEnt ? pEnt->GetClientNetworkable() : 0;
}

IClientEntity *
CClientEntityList::GetClientEntityFromHandle(ClientEntityHandle_t hEnt) {
  IClientUnknown *pEnt = GetClientUnknownFromHandle(hEnt);
  return pEnt ? pEnt->GetIClientEntity() : 0;
}

IClientRenderable *
CClientEntityList::GetClientRenderableFromHandle(ClientEntityHandle_t hEnt) {
  IClientUnknown *pEnt = GetClientUnknownFromHandle(hEnt);
  return pEnt ? pEnt->GetClientRenderable() : 0;
}

C_BaseEntity *
CClientEntityList::GetBaseEntityFromHandle(ClientEntityHandle_t hEnt) {
  IClientUnknown *pEnt = GetClientUnknownFromHandle(hEnt);
  return pEnt ? pEnt->GetBaseEntity() : 0;
}

ICollideable *
CClientEntityList::GetCollideableFromHandle(ClientEntityHandle_t hEnt) {
  IClientUnknown *pEnt = GetClientUnknownFromHandle(hEnt);
  return pEnt ? pEnt->GetCollideable() : 0;
}

IClientThinkable *
CClientEntityList::GetClientThinkableFromHandle(ClientEntityHandle_t hEnt) {
  IClientUnknown *pEnt = GetClientUnknownFromHandle(hEnt);
...
void CClientEntityList::RemovePVSNotifier(IClientUnknown *pUnknown) {
  ...
  if (pRenderable) {
    ...
    if (pNotify) {
      ...
      Assert(m_PVSNotifyInfos[indexIntoPVSNotifyInfos].m_pRenderable ==
             pRenderable);
  ...
  // If it didn't report itself as a notifier, let's hope it's not in the
  // notifier list now (which would mean that it reported itself as a notifier
  // earlier, but not now).
#ifdef _DEBUG
  unsigned short index = m_PVSNotifierMap.Find(pUnknown);
...
void CClientEntityList::OnAddEntity(IHandleEntity *pEnt, CBaseHandle handle) {
  ...
  if (entnum >= 0 && entnum < MAX_EDICTS) {
    ...
    Assert(((IClientUnknown *)pEnt)
               ->GetClientNetworkable()); // Server entities should all be
                                          // networkable.
...
void CClientEntityList::OnRemoveEntity(IHandleEntity *pEnt,
                                       CBaseHandle handle) {
  int entnum = handle.GetEntryIndex();
  EntityCacheInfo_t *pCache = &m_EntityCacheInfo[entnum];
...
// --------------------------------------------------------------------------------------------------
// //
C_AllBaseEntityIterator::C_AllBaseEntityIterator() { Restart(); }
...
// --------------------------------------------------------------------------------------------------
// //
C_BaseEntityIterator::C_BaseEntityIterator() { Restart(); }
