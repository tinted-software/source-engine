...
// Here it's setup to allow you to see it if you're looking within 40 degrees of
// the source.
...
void Do2DRotation(Vector vIn, Vector &vOut, float flDegrees, int i1, int i2,
                  int i3) {
  float c, s;
...
void CGlowOverlay::UpdateSkyGlowObstruction(float zFar,
  ...
  // If we already cached the sky obstruction and are still using that,
  // early-out
  ...
  if (PixelVisibility_IsAvailable()) {
    ...
    // UNDONE: Can probably do only the pixelvis query in this case if you can
    // figure out where to put it - or save the position of this trace
    pixelvis_queryparams_t params;
    params.Init(pos, m_flProxyRadius);
    params.bSizeInScreenspace = true;
    m_skyObstructionScale =
        PixelVisibility_FractionVisible(params, &m_queryHandle);
    return;
  }
  // Trace a ray at the object.
  trace_t trace;
  UTIL_TraceLine(CurrentViewOrigin(),
                 CurrentViewOrigin() + (m_vDirection * MAX_TRACE_LENGTH),
...
void CGlowOverlay::UpdateGlowObstruction(const Vector &vToGlow,
                                         bool bCacheFullSceneState) {
  // If we already cached the glow obstruction and are still using that,
  // early-out
  if (bCacheFullSceneState && m_bCacheGlowObstruction)
    return;

  if (bCacheFullSceneState &&
      !m_bCacheGlowObstruction) // If turning on sky obstruction caching mode
  {
  ...
  if (PixelVisibility_IsAvailable()) {
    if (m_bInSky) {
      ...
      Vector pos =
          CurrentViewOrigin() + m_vDirection * (pViewSetup->zFar * 0.999f);
      pixelvis_queryparams_t params;
  ...
  if (m_bInSky) {
    ...
    trace_t trace;
    UTIL_TraceLine(CurrentViewOrigin(),
                   CurrentViewOrigin() + (vToGlow * MAX_TRACE_LENGTH),
                   CONTENTS_SOLID, NULL, COLLISION_GROUP_NONE, &trace);
  ...
  if (bFade) {
    if (building_cubemaps.GetBool()) {
      ...
      m_flGlowObstructionScale -=
          gpGlobals->frametime / cl_sun_decay_rate.GetFloat();
...
void CGlowOverlay::CalcSpriteColorAndSize(float flDot, CGlowSprite *pSprite,
  ...
  *flHorzSize = RemapValClamped(flDot, g_flOverlayRange, 1,
                                flSizeAtOverlayRangeMul * pSprite->m_flHorzSize,
                                flSizeAtOneMul * pSprite->m_flHorzSize);

  *flVertSize = RemapValClamped(flDot, g_flOverlayRange, 1,
                                flSizeAtOverlayRangeMul * pSprite->m_flVertSize,
                                flSizeAtOneMul * pSprite->m_flVertSize);

  float flOpacity = RemapValClamped(flDot, g_flOverlayRange, 1,
                                    flOpacityAtOverlayRange, flOpacityAtOne);

  flOpacity = flOpacity * m_flGlowObstructionScale;
  *vColor = pSprite->m_vColor * flOpacity;
}

void CGlowOverlay::CalcBasis(const Vector &vToGlow, float flHorzSize,
...
void CGlowOverlay::Draw(bool bCacheFullSceneState) {
  ...
  bool bWireframe =
      ShouldDrawInWireFrameMode() || (r_drawsprites.GetInt() == 2);
  ...
  for (int iSprite = 0; iSprite < m_nSprites; iSprite++) {
    ...
    if (m_Sprites[iSprite].m_pMaterial == NULL) {
      m_Sprites[iSprite].m_pMaterial = materials->FindMaterial(
          "sprites/light_glow02_add_noz", TEXTURE_GROUP_CLIENT_EFFECTS);
    }

    Assert(m_Sprites[iSprite].m_pMaterial);
    static unsigned int nHDRColorScaleCache = 0;
    IMaterialVar *pHDRColorScaleVar =
        m_Sprites[iSprite].m_pMaterial->FindVarFast("$hdrcolorscale",
                                                    &nHDRColorScaleCache);
    ...
    IMesh *pMesh = pRenderContext->GetDynamicMesh(
        false, 0, 0, m_Sprites[iSprite].m_pMaterial);
    ...
    if (bWireframe) {
      IMaterial *pWireframeMaterial = materials->FindMaterial(
          "debug/debugwireframevertexcolor", TEXTURE_GROUP_OTHER);
      pRenderContext->Bind(pWireframeMaterial);

      // Draw the sprite.
      IMesh *pMesh =
          pRenderContext->GetDynamicMesh(false, 0, 0, pWireframeMaterial);
...
void CGlowOverlay::DrawOverlays(bool bCacheFullSceneState) {
  ...
  for (unsigned short i = g_GlowOverlaySystem.m_GlowOverlays.Head();
    ...
    if (pOverlay->Update()) {
      pRenderContext->EnableClipping(
          ((pOverlay->m_bInSky)
               ? (false)
               : (bClippingEnabled))); // disable clipping in skybox, restore
                                       // clipping to pre-existing state when
  ...
  pRenderContext->EnableClipping(
      bClippingEnabled); // restore clipping to original state
}
...
void CGlowOverlay::UpdateSkyOverlays(float zFar, bool bCacheFullSceneState) {
  ...
  for (unsigned short i = g_GlowOverlaySystem.m_GlowOverlays.Head();
    ...
    if (!pOverlay->m_bActivated || !pOverlay->m_bDirectional ||
        !pOverlay->m_bInSky)
      continue;
...
void CGlowOverlay::BackupSkyOverlayData(int iBackupToSlot) {
  ...
  for (unsigned short i = g_GlowOverlaySystem.m_GlowOverlays.Head();
    ...
    pOverlay->m_skyObstructionScaleBackups[iBackupToSlot] =
        pOverlay->m_skyObstructionScale;
  }
}
...
void CGlowOverlay::RestoreSkyOverlayData(int iRestoreFromSlot) {
  ...
  for (unsigned short i = g_GlowOverlaySystem.m_GlowOverlays.Head();
    ...
    pOverlay->m_skyObstructionScale =
        pOverlay->m_skyObstructionScaleBackups[iRestoreFromSlot];
  }
}
