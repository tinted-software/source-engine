...
public:
  ...
  virtual void ChildLayerBlend(Vector pos[], Quaternion q[], float currentTime,
                               int boneMask);
  ...
  inline bool IsModelScaleFractional()
      const; /// very fast way to ask if the model scale is < 1.0f  (faster than
  ...
  bool IsBoneCacheValid() const; // Returns true if the bone cache is considered
                                 // good for this frame.
  ...
  DoMuzzleFlash(); // Force a muzzle flash event. Note: this only QUEUES an
                   // event, so ProcessMuzzleFlashEvent will get called later.
...
inline bool C_BaseAnimating::IsModelScaleFractional()
    const /// very fast way to ask if the model scale is < 1.0f
{
