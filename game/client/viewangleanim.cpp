...
// create a view animation object to be used for creating an animation.
// parameter is flags
...
ConCommand viewanim_reset("viewanim_reset", ResetViewAngles,
                          "reset view angles!", FCVAR_CHEAT);

// add a key frame to the test animation. first parameter is the time taken to
// get to this keyframe
CON_COMMAND_F(viewanim_addkeyframe, "", FCVAR_CHEAT) {
  if (g_pTestAnimation) {
    ...
    g_pTestAnimation->AddKeyFrame(
        new CViewAngleKeyFrame(vecTarget, flDelay, iFlags));
  } else
...
void CViewAngleAnimation::Spawn(void) {
  ...
  if ( m_iFlags & VIEWANIM_RELATIVE )
  {
          ...
          // seed this so we can add keyframes and have them calc the delta
  properly m_vecBaseAngles = angles;
  }
  else
  {
...
void CViewAngleAnimation::SaveAsAnimFile(const char *pKeyFrameFileName) {
  ...
  for (i = 0; i < c; i++) {
    ...
    Q_snprintf(buf, sizeof(buf), "%f %f %f", m_KeyFrames[i]->m_vecAngles[0],
               m_KeyFrames[i]->m_vecAngles[1], m_KeyFrames[i]->m_vecAngles[2]);
...
void CViewAngleAnimation::RunAnimation(QAngle angles) {
  if (m_KeyFrames.Count() == 0) {
    Warning("CViewAngleAnimation::RunAnimation called on an empty view "
            "animation\n");
    return;
  }
...
void CViewAngleAnimation::ClientThink() {
  ...
  if (m_KeyFrames[i]->m_iFlags != m_iFlags) {
    if ((m_KeyFrames[i]->m_iFlags & VIEWANIM_RELATIVE) &&
...
void CViewAngleAnimation::SetAngles(QAngle vecCalculatedAngles) {
  ...
  // clamp pitch
  vecViewAngle[PITCH] = clamp(vecViewAngle[PITCH], -cl_pitchup.GetFloat(),
