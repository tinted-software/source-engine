...
void C_MovieExplosion::Start(CParticleMgr *pParticleMgr,
  ...
  // Get our materials.
  m_iFireballMaterial =
      m_ParticleEffect.FindOrAddMaterial("particle/particle_sphere");
...
void C_MovieExplosion::Update(float fTimeDelta) {
  ...
  m_EmitterAlpha =
      (float)sin(m_EmitterLifetime * 3.14159f / EXPLOSION_EMITTER_LIFETIME);
  ...
  for (int iEmitter = 0; iEmitter < NUM_MOVIEEXPLOSION_EMITTERS; iEmitter++) {
    ...
    while (pEmitter->m_ParticleSpawn.NextEvent(tempDelta)) {
      StandardParticle_t *pParticle =
          (StandardParticle_t *)m_ParticleEffect.AddParticle(
              sizeof(StandardParticle_t), m_iFireballMaterial);

      if (pParticle) {
        pParticle->m_Pos = pEmitter->m_Pos;
        pParticle->m_Velocity =
            pEmitter->m_Velocity * 0.2f + RandomVector(-20, 20);
      }
    }
  }
}
