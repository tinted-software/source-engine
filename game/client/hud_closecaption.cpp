...
static ConVar
    cc_subtitles("cc_subtitles", "0", FCVAR_ARCHIVE | FCVAR_ARCHIVE_XBOX,
                 "If set, don't show sound effect captions, just voice overs "
                 "(i.e., won't help hearing impaired players).");
...
CCloseCaptionWorkUnit::CCloseCaptionWorkUnit()
    : m_nWidth(0), m_nHeight(0), m_bBold(false), m_bItalic(false),
      m_pszStream(0), m_Color(Color(255, 255, 255, 255)), m_hFont(0),
      m_flFadeStartTime(0) {}
...
public:
  CCloseCaptionItem(const wchar_t *stream, float timetolive, float addedtime,
      ...
      : m_flTimeToLive(0.0f), m_flAddedTime(addedtime), m_bValid(false),
        m_nTotalWidth(0), m_nTotalHeight(0), m_bSizeComputed(false),
...
struct AsyncCaptionData_t {
  ...
  AsyncCaptionData_t()
      : m_nBlockNum(-1), m_pBlockData(0), m_nFileIndex(-1), m_nBlockSize(0),
        m_bLoadPending(false), m_bLoadCompleted(false), m_hAsyncControl(NULL) {}
  ...
  // -----------------------------------------------------------
  static AsyncCaptionData_t *
  CreateResource(const asynccaptionparams_t &params) {
...
public:
  ...
  void SpewMemoryUsage() {
    ...
    DevMsg("CAsyncCaptionResourceManager:  %i blocks total %s, %.2f %% of "
           "capacity\n",
...
CHudCloseCaption::CHudCloseCaption(const char *pElementName)
    : CHudElement(pElementName), vgui::Panel(NULL, "HudCloseCaption"),
      ...
      m_CurrentLanguage(UTL_INVAL_SYMBOL), m_bPaintDebugInfo(false) {
...
void CHudCloseCaption::CreateFonts(void) {
  ...
  if (IsPC()) {
    ...
    m_hFonts[CCFONT_ITALICBOLD] =
        pScheme->GetFont("CloseCaption_BoldItalic", true);
  } else {
...
#else
  // params.stream is still in ucs2 format here so just do a basic zero compare
  // for length or just space
...
public:
  ...
  void OnDataArrived(CUtlVector<AsyncCaption_t> &directories, int nFileIndex,
    ...
    for (int i = 0; i < c; ++i) {
      ...
      V_UCS2ToUnicode((ucs2 *)&pData->m_pBlockData[entry.offset],
                      caption->stream, entry.length << 1);
#endif
    }
  }
  ...
  bool GetStream(OUT_Z_BYTECAP(bufSizeInBytes) wchar_t *buf,
                 int bufSizeInBytes) {
...
static int
EmitCaptionCompletion(const char *partial,
      ...
      if (Q_stristr(streamANSI, pchANSI)) {
        ...
        if (IsPC()) {
          for (int r = g_pVGuiLocalize->GetFirstStringIndex();
               r != INVALID_LOCALIZE_STRING_INDEX;
