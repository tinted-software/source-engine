...
public:
  CBoundedCvar_InterpRatio()
      : ConVar_ServerBounded("cl_interp_ratio", "2.0",
                             FCVAR_USERINFO | FCVAR_NOT_CONNECTED,
                             "Sets the interpolation amount (final amount is "
...
public:
  CBoundedCvar_Interp()
      : ConVar_ServerBounded("cl_interp", "0.1",
                             FCVAR_USERINFO | FCVAR_NOT_CONNECTED,
                             "Sets the interpolation amount (bounded on low "
                             "side by server interp ratio settings).",
                             true, 0.0f, true, 0.5f) {}
  ...
  virtual float GetFloat() const {
    ...
    if (pUpdateRate && pMin && pMin->GetFloat() != -1) {
      return MAX(GetBaseFloatValue(),
                 pMin->GetFloat() / pUpdateRate->GetFloat());
    } else {
      return GetBaseFloatValue();
    }
  }
...
float GetClientInterpAmount() {
  ...
  if (pUpdateRate) {
    ...
    if (!CommandLine()->FindParm("-hushasserts")) {
      AssertMsgOnce(false,
                    "GetInterpolationAmount: can't get cl_updaterate cvar.");
    }
