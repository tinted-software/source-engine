...
template <class T> inline T LoopingLerp(float flPercent, T flFrom, T flTo) {
...
inline void
Lerp_Clamp(CRangeCheckedVar<T, minValue, maxValue, startValue> &val) {
  val.Clamp();
}
...
template <class T> inline T LoopingLerp_Hermite(float t, T p0, T p1, T p2) {
