...
RecvPropFloat(RECVINFO(m_flStartScale)),
    ...
    RecvPropInt(RECVINFO(m_nFlags)), RecvPropInt(RECVINFO(m_nFlameModelIndex)),
...
void C_FireSmoke::Start(void) {
  ...
  case 2:
    lpszEffectName = (m_nFlags & bitsFIRESMOKE_SMOKE) ? "env_fire_medium_smoke"
                                                      : "env_fire_medium";
    break;
...
RecvPropEHandle(RECVINFO(m_hEntAttached)), END_RECV_TABLE()
...
#ifdef TF_CLIENT_DLL
  m_hEffect =
      ParticleProp()->Create("burningplayer_red", PATTACH_ABSORIGIN_FOLLOW);
#else
  m_hEffect =
