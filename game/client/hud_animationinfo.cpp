...
public:
  ...
  void SetWatch(Panel *element) { m_pWatch = element; }
...
CHudAnimationInfo::CHudAnimationInfo(const char *pElementName)
  ...
  // Make sure we render on top of other hud elements since we are debugging
  // info for them.
  SetZPos(100);
}
...
void CHudAnimationInfo::PaintString(int &x, int &y, const char *sz,
                                    Color *pLegendColor) {
...
void CHudAnimationInfo::PaintMappingInfo(int &x, int &y, Panel *element,
  ...
  for (int i = 0; i < c; i++) {
    ...
    if (element->RequestInfo(kv)) {
      ...
      if (dat && dat->GetDataType() == KeyValues::TYPE_COLOR) {
        col = dat->GetColor();
        Q_snprintf(value, sizeof(value), "%i, %i, %i, %i", col[0], col[1],
                   col[2], col[3]);
        pColor = &col;
      } else {
...
void CHudAnimationInfo::Paint() {
  ...
  char sz[256];
  Q_snprintf(sz, sizeof(sz), "%-30s %-20s (%i %i)", "Position", "pos",
             bounds[0], bounds[1]);
  PaintString(x, y, sz, NULL);
  Q_snprintf(sz, sizeof(sz), "%-30s %-20s (%i %i)", "Size", "size", bounds[2],
             bounds[3]);
  PaintString(x, y, sz, NULL);
}
...
CON_COMMAND_F_COMPLETION(cl_animationinfo, "Hud element to examine.", 0,
                         HudElementCompletion) {
