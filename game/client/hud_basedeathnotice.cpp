...
// Purpose:
//-----------------------------------------------------------------------------
void CHudBaseDeathNotice::VidInit(void) { m_DeathNotices.RemoveAll(); }
...
// Purpose:
//-----------------------------------------------------------------------------
Color CHudBaseDeathNotice::GetTeamColor(
    int iTeamNumber, bool bLocalPlayerInvolved /* = false */) {
...
void CHudBaseDeathNotice::Paint() {
  ...
  CBaseViewport *pViewport =
      dynamic_cast<CBaseViewport *>(GetClientModeNormal()->GetViewport());
  ...
  for (int i = 0; i < iCount; i++) {
    ...
    g_pVGuiLocalize->ConvertANSIToUnicode(msg.Victim.szName, victim,
                                          sizeof(victim));
    g_pVGuiLocalize->ConvertANSIToUnicode(msg.Killer.szName, killer,
                                          sizeof(killer));

    int iVictimTextWide =
        UTIL_ComputeStringWidth(m_hTextFont, victim) + xSpacing;
    int iDeathInfoTextWide =
        msg.wzInfoText[0]
            ? UTIL_ComputeStringWidth(m_hTextFont, msg.wzInfoText) + xSpacing
            : 0;
    int iDeathInfoEndTextWide =
        msg.wzInfoTextEnd[0]
            ? UTIL_ComputeStringWidth(m_hTextFont, msg.wzInfoTextEnd) + xSpacing
            : 0;

    int iKillerTextWide =
        killer[0] ? UTIL_ComputeStringWidth(m_hTextFont, killer) + xSpacing : 0;
    int iLineTall = m_flLineHeight;
    int iTextTall = surface()->GetFontTall(m_hTextFont);
    int iconWide = 0, iconTall = 0, iDeathInfoOffset = 0, iVictimTextOffset = 0,
        iconActualWide = 0;

    int iPreKillerTextWide =
        msg.wzPreKillerText[0]
            ? UTIL_ComputeStringWidth(m_hTextFont, msg.wzPreKillerText) -
                  xSpacing
            : 0;
    int iconPrekillerWide = 0, iconPrekillerActualWide = 0,
        iconPreKillerTall = 0;
    ...
    int iTotalWide = iKillerTextWide + iconWide + iVictimTextWide +
                     iDeathInfoTextWide + iDeathInfoEndTextWide + (xMargin * 2);
    ...
    GetBackgroundPolygonVerts(x, y + 1, x + iTotalWide, y + iLineTall - 1,
                              ARRAYSIZE(vert), vert);
    ...
    if (msg.wzPreKillerText[0]) {
      x += xSpacing;
      DrawText(x + iDeathInfoOffset, yText, m_hTextFont, GetInfoTextColor(i),
               msg.wzPreKillerText);
    ...
    if (iconPrekiller) {
      ...
      iconPrekiller->DrawSelf(x, yPreIconTall, iconPrekillerActualWide,
                              iconPreKillerTall, m_clrIcon);
      x += iconPrekillerWide + xSpacing;
    }

    // Draw glow behind weapon icon to show it was a crit death
    if (msg.bCrit && msg.iconCritDeath) {
      msg.iconCritDeath->DrawSelf(x, yIcon, iconActualWide, iconTall,
                                  m_clrIcon);
    }
    ...
    // Draw victims name
    DrawText(x + iVictimTextOffset, yText, m_hTextFont,
             GetTeamColor(msg.Victim.iTeam, msg.bLocalPlayerInvolved), victim);
    x += iVictimTextWide;
...
void CHudBaseDeathNotice::RetireExpiredDeathNotices() {
  // Remove any expired death notices.  Loop backwards because we might remove
  // one
  int iCount = m_DeathNotices.Count();
  ...
  if (m_DeathNotices.Count() > 0 &&
    ...
    // First, remove any notices not involving the local player, since they are
    // lower priority.
    iCount = m_DeathNotices.Count();
    int iNeedToRemove = iCount - (int)m_flMaxDeathNotices;
    // loop condition is iCount-1 because we won't remove the most recent death
    // notice, otherwise new non-local-player-involved messages would not appear
    // if the queue was full of messages involving the local player
    ...
    // Now that we've culled any non-local-player-involved messages up to the
    // amount we needed to remove, see if we've removed enough
    iCount = m_DeathNotices.Count();
    iNeedToRemove = iCount - (int)m_flMaxDeathNotices;
    if (iNeedToRemove > 0) {
      // if we still have too many messages, then just remove however many we
      // need, oldest first
...
void CHudBaseDeathNotice::FireGameEvent(IGameEvent *event) {
  ...
  // Add a new death message.  Note we always look it up by index rather than
  // create a reference or pointer to it; additional messages may get added
  // during this function that cause the underlying array to get realloced, so
  // don't ever keep a pointer to memory here.
  int iMsg = -1;
  if (bPlayerDeath) {
  ...
  if (bPlayerDeath || bObjectDeath) {
    ...
    if (event->GetInt("death_flags") & TF_DEATH_AUSTRALIUM) {
      ...
      m_DeathNotices[iMsg].iconCritDeath = GetIcon(
          "d_australium", bLocalPlayerInvolved ? kDeathNoticeIcon_Inverted
                                               : kDeathNoticeIcon_Standard);
    } else if (event->GetInt("damagebits") & DMG_CRITICAL) {
      m_DeathNotices[iMsg].bCrit = true;
      m_DeathNotices[iMsg].iconCritDeath =
          GetIcon("d_crit", bLocalPlayerInvolved ? kDeathNoticeIcon_Inverted
                                                 : kDeathNoticeIcon_Standard);
    } else {
    ...
    m_DeathNotices[iMsg].Killer.iTeam =
        (killer > 0) ? g_PR->GetTeam(killer) : 0;
    m_DeathNotices[iMsg].Victim.iTeam = g_PR->GetTeam(victim);
    Q_strncpy(m_DeathNotices[iMsg].Killer.szName, killer_name,
              ARRAYSIZE(m_DeathNotices[iMsg].Killer.szName));
    Q_strncpy(m_DeathNotices[iMsg].Victim.szName, victim_name,
              ARRAYSIZE(m_DeathNotices[iMsg].Victim.szName));
    if (killedwith && *killedwith) {
      Q_snprintf(m_DeathNotices[iMsg].szIcon,
                 sizeof(m_DeathNotices[iMsg].szIcon), "d_%s", killedwith);
    }
    ...
    if (m_DeathNotices[iMsg].bSelfInflicted) {
      ...
      if (killedwithweaponlog && killedwithweaponlog[0] &&
          (killedwithweaponlog[0] > 13)) {
        Q_strncat(sDeathMsg, VarArgs(" with %s.", killedwithweaponlog),
                  sizeof(sDeathMsg), COPY_ALL_CHARACTERS);
      } else if (m_DeathNotices[iMsg].szIcon[0] &&
                 (m_DeathNotices[iMsg].szIcon[0] > 13)) {
        Q_strncat(sDeathMsg,
                  VarArgs(" with %s.", &m_DeathNotices[iMsg].szIcon[2]),
                  sizeof(sDeathMsg), COPY_ALL_CHARACTERS);
      }
    }
    ...
    Q_strncpy(m_DeathNotices[iMsg].Killer.szName, szCappers,
              sizeof(m_DeathNotices[iMsg].Killer.szName));
    V_wcsncpy(m_DeathNotices[iMsg].wzInfoText,
              g_pVGuiLocalize->Find(len > 1 ? "#Msg_Captured_Multiple"
                                            : "#Msg_Captured"),
              sizeof(m_DeathNotices[iMsg].wzInfoText));

    // print a log message
    Msg("%s captured %s for team #%d\n", m_DeathNotices[iMsg].Killer.szName,
        m_DeathNotices[iMsg].Victim.szName, m_DeathNotices[iMsg].Killer.iTeam);
  } else if (FStrEq("teamplay_capture_blocked", pszEventName)) {
    GetLocalizedControlPointName(event, m_DeathNotices[iMsg].Victim.szName,
                                 ARRAYSIZE(m_DeathNotices[iMsg].Victim.szName));
    V_wcsncpy(m_DeathNotices[iMsg].wzInfoText,
              g_pVGuiLocalize->Find("#Msg_Defended"),
              sizeof(m_DeathNotices[iMsg].wzInfoText));

    int iPlayerIndex = event->GetInt("blocker");
    const char *blocker_name = g_PR->GetPlayerName(iPlayerIndex);
    Q_strncpy(m_DeathNotices[iMsg].Killer.szName, blocker_name,
              ARRAYSIZE(m_DeathNotices[iMsg].Killer.szName));
    m_DeathNotices[iMsg].Killer.iTeam = g_PR->GetTeam(iPlayerIndex);
    if (iLocalPlayerIndex == iPlayerIndex)
      m_DeathNotices[iMsg].bLocalPlayerInvolved = true;
    ...
    Assert(pwzEventText);
    if (pwzEventText) {
      V_wcsncpy(m_DeathNotices[iMsg].wzInfoText, pwzEventText,
                sizeof(m_DeathNotices[iMsg].wzInfoText));
    } else {
      V_memset(m_DeathNotices[iMsg].wzInfoText, 0,
               sizeof(m_DeathNotices[iMsg].wzInfoText));
    }

    int iPlayerIndex = event->GetInt("player");
    const char *szPlayerName = g_PR->GetPlayerName(iPlayerIndex);
    Q_strncpy(m_DeathNotices[iMsg].Killer.szName, szPlayerName,
              ARRAYSIZE(m_DeathNotices[iMsg].Killer.szName));
    m_DeathNotices[iMsg].Killer.iTeam = g_PR->GetTeam(iPlayerIndex);
    if (iLocalPlayerIndex == iPlayerIndex)
      m_DeathNotices[iMsg].bLocalPlayerInvolved = true;
  ...
  if (!m_DeathNotices[iMsg].iconDeath && m_DeathNotices[iMsg].szIcon) {
    ...
    m_DeathNotices[iMsg].iconDeath = GetIcon(
        m_DeathNotices[iMsg].szIcon,
        bInverted ? kDeathNoticeIcon_Inverted : kDeathNoticeIcon_Standard);
    if (!m_DeathNotices[iMsg].iconDeath) {
      // Can't find it, so use the default skull & crossbones icon
      m_DeathNotices[iMsg].iconDeath =
          GetIcon("d_skull_tf", m_DeathNotices[iMsg].bLocalPlayerInvolved
...
//-----------------------------------------------------------------------------
void CHudBaseDeathNotice::GetLocalizedControlPointName(IGameEvent *event,
                                                       char *namebuf,
                                                       int namelen) {
...
//-----------------------------------------------------------------------------
void CHudBaseDeathNotice::DrawText(int x, int y, HFont hFont, Color clr,
...
//-----------------------------------------------------------------------------
void CHudBaseDeathNotice::GetBackgroundPolygonVerts(int x0, int y0, int x1,
                                                    int y1, int iVerts,
                                                    vgui::Vertex_t vert[]) {
  Assert(iVerts == NUM_BACKGROUND_COORD);
  // use the offsets we generated for one corner and apply those to the
  // passed-in dimensions to create verts for the poly
  for (int i = 0; i < NUM_CORNER_COORD; i++) {
    int j = (NUM_CORNER_COORD - 1) - i;
    // upper left corner
    vert[i].Init(Vector2D(x0 + m_CornerCoord[i].x, y0 + m_CornerCoord[i].y));
    // upper right corner
    vert[i + NUM_CORNER_COORD].Init(
        Vector2D(x1 - m_CornerCoord[j].x, y0 + m_CornerCoord[j].y));
    // lower right corner
    vert[i + (NUM_CORNER_COORD * 2)].Init(
        Vector2D(x1 - m_CornerCoord[i].x, y1 - m_CornerCoord[i].y));
    // lower left corner
    vert[i + (NUM_CORNER_COORD * 3)].Init(
        Vector2D(x0 + m_CornerCoord[j].x, y1 - m_CornerCoord[j].y));
  }
}
...
void CHudBaseDeathNotice::CalcRoundedCorners() {
  ...
  for (int i = 0; i < iMax; i++) {
    m_CornerCoord[i].x = m_flCornerRadius *
                         (1 - cos(((float)i / (float)(iMax - 1)) * (M_PI / 2)));
    m_CornerCoord[i].y = m_flCornerRadius *
                         (1 - sin(((float)i / (float)(iMax - 1)) * (M_PI / 2)));
  }
}
...
//-----------------------------------------------------------------------------
CHudTexture *CHudBaseDeathNotice::GetIcon(const char *szIcon,
                                          EDeathNoticeIconFormat eIconFormat) {
  // adjust the style (prefix) of the icon if requested
  if (eIconFormat != kDeathNoticeIcon_Standard &&
      V_strncmp("d_", szIcon, 2) == 0) {
  ...
  // we either requested the default style or we requested an alternate style
  // but didn't have the art for it; either way, we can't, so fall back to our
  // default
