...
ConVar hud_fastswitch("hud_fastswitch", "0",
...
DECLARE_HUD_COMMAND_NAME(CBaseHudWeaponSelection, Slot10,
                         "CHudWeaponSelection");
DECLARE_HUD_COMMAND_NAME(CBaseHudWeaponSelection, Close, "CHudWeaponSelection");
DECLARE_HUD_COMMAND_NAME(CBaseHudWeaponSelection, NextWeapon,
                         "CHudWeaponSelection");
DECLARE_HUD_COMMAND_NAME(CBaseHudWeaponSelection, PrevWeapon,
                         "CHudWeaponSelection");
DECLARE_HUD_COMMAND_NAME(CBaseHudWeaponSelection, LastWeapon,
                         "CHudWeaponSelection");
...
//-----------------------------------------------------------------------------
// Purpose: Returns whether a weapon can be selected in the HUD, based on hud
// type
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
int CBaseHudWeaponSelection::KeyInput(int down, ButtonCode_t keynum,
...
void CBaseHudWeaponSelection::UserCmd_Slot5(void) { SelectSlot(5); }

void CBaseHudWeaponSelection::UserCmd_Slot6(void) { SelectSlot(6); }

void CBaseHudWeaponSelection::UserCmd_Slot7(void) { SelectSlot(7); }

void CBaseHudWeaponSelection::UserCmd_Slot8(void) { SelectSlot(8); }

void CBaseHudWeaponSelection::UserCmd_Slot9(void) { SelectSlot(9); }

void CBaseHudWeaponSelection::UserCmd_Slot0(void) { SelectSlot(0); }
...
//-----------------------------------------------------------------------------
void CBaseHudWeaponSelection::UserCmd_Close(void) { CancelWeaponSelection(); }
...
C_BaseCombatWeapon *CBaseHudWeaponSelection::GetFirstPos(int iSlot) {
  ...
  for (int i = 0; i < MAX_WEAPONS; i++) {
    ...
    if ((pWeapon->GetSlot() == iSlot) &&
      ...
      // If this weapon is lower in the slot than the current lowest, it's our
      // new winner
...
C_BaseCombatWeapon *CBaseHudWeaponSelection::GetNextActivePos(int iSlot,
  ...
  for (int i = 0; i < MAX_WEAPONS; i++) {
    ...
    if (CanBeSelectedInHUD(pWeapon) && pWeapon->GetSlot() == iSlot) {
      // If this weapon is lower in the slot than the current lowest, and above
      // our desired position, it's our new winner
      if (pWeapon->GetPosition() <= iLowestPosition &&
