...
const char *COM_GetModDirectory(); // return the mod dir (rather than the
                                   // complete -game param, which can be a path)
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CClientVirtualReality, IClientVirtualReality,
                                  CLIENTVIRTUALREALITY_INTERFACE_VERSION,
...
ConVar vr_activate_default("vr_activate_default", "0", FCVAR_ARCHIVE,
                           "If this is true the game will switch to VR mode "
                           "once startup is complete.");

ConVar
    vr_moveaim_mode("vr_moveaim_mode", "3", FCVAR_ARCHIVE,
                    "0=move+shoot from face. 1=move with torso. 2,3,4=shoot "
                    "with face+mouse cursor. 5+ are probably not that useful.");
ConVar vr_moveaim_mode_zoom(
    "vr_moveaim_mode_zoom", "3", FCVAR_ARCHIVE,
    "0=move+shoot from face. 1=move with torso. 2,3,4=shoot with face+mouse "
    "cursor. 5+ are probably not that useful.");

ConVar vr_moveaim_reticle_yaw_limit(
    "vr_moveaim_reticle_yaw_limit", "10", FCVAR_ARCHIVE,
    "Beyond this number of degrees, the mouse drags the torso");
ConVar vr_moveaim_reticle_pitch_limit(
    "vr_moveaim_reticle_pitch_limit", "30", FCVAR_ARCHIVE,
    "Beyond this number of degrees, the mouse clamps");
// Note these are scaled by the zoom factor.
ConVar vr_moveaim_reticle_yaw_limit_zoom(
    "vr_moveaim_reticle_yaw_limit_zoom", "0", FCVAR_ARCHIVE,
    "Beyond this number of degrees, the mouse drags the torso");
ConVar vr_moveaim_reticle_pitch_limit_zoom(
    "vr_moveaim_reticle_pitch_limit_zoom", "-1", FCVAR_ARCHIVE,
    "Beyond this number of degrees, the mouse clamps");

// This are somewhat obsolete.
ConVar vr_aim_yaw_offset("vr_aim_yaw_offset", "90", 0,
                         "This value is added to Yaw when returning the "
                         "vehicle aim angles to Source.");

ConVar vr_stereo_swap_eyes("vr_stereo_swap_eyes", "0", 0, "1=swap eyes.");

// Useful for debugging wacky-projection problems, separate from multi-rendering
// problems.
ConVar vr_stereo_mono_set_eye(
    "vr_stereo_mono_set_eye", "0", 0,
...
ConVar vr_translation_limit(
    "vr_translation_limit", "10.0", 0,
    "How far the in-game head will translate before being clamped.");

// HUD config values
ConVar vr_render_hud_in_world("vr_render_hud_in_world", "1");
ConVar vr_hud_max_fov("vr_hud_max_fov", "60", FCVAR_ARCHIVE,
                      "Max FOV of the HUD");
ConVar vr_hud_forward("vr_hud_forward", "500", FCVAR_ARCHIVE,
...
ConVar vr_hud_axis_lock_to_world(
    "vr_hud_axis_lock_to_world", "0", FCVAR_ARCHIVE,
    "Bitfield - locks HUD axes to the world - 0=pitch, 1=yaw, 2=roll");

// Default distance clips through rocketlauncher, heavy's body, etc.
ConVar vr_projection_znear_multiplier(
    "vr_projection_znear_multiplier", "0.3", 0,
    "Allows moving the ZNear plane to deal with body clipping");

// Should the viewmodel (weapon) translate with the HMD, or remain fixed to the
// in-world body (but still rotate with the head)? Purely a graphics effect - no
// effect on actual bullet aiming. Has no effect in aim modes where aiming is
// not controlled by the head.
ConVar vr_viewmodel_translate_with_head(
    "vr_viewmodel_translate_with_head", "0", 0,
    "1=translate the viewmodel with the head motion.");

ConVar vr_zoom_multiplier("vr_zoom_multiplier", "2.0", FCVAR_ARCHIVE,
                          "When zoomed, how big is the scope on your HUD?");
ConVar vr_zoom_scope_scale("vr_zoom_scope_scale", "6.0", 0,
                           "Something to do with the default scope HUD overlay "
                           "size."); // Horrible hack - should work out the math
                                     // properly, but we need to ship.

ConVar vr_viewmodel_offset_forward("vr_viewmodel_offset_forward", "-8", 0);
ConVar vr_viewmodel_offset_forward_large("vr_viewmodel_offset_forward_large",
                                         "-15", 0);

ConVar vr_force_windowed("vr_force_windowed", "0", FCVAR_ARCHIVE);

ConVar vr_first_person_uses_world_model(
    "vr_first_person_uses_world_model", "1", 0,
...
static ConCommand vr_cycle_aim_move_mode("vr_cycle_aim_move_mode",
                                         CC_VR_Cycle_Aim_Move_Mode,
...
void CalcFovFromProjection(float *pFov, const VMatrix &proj) {
  ...
  // \    0     0       -1     0 /
  //
  // Then the result is normalized (i.e. divide by w) and the result clipped to
  // the [-1,+1] unit cube. (ignore Z for now, and the clipping is slightly
  // different). So, we want to know what vectors produce a clip value of -1 and
  // +1 in each direction, e.g. in the X direction:
  //    +-1 = ( xscale*x + xoffset*z ) / (-1*z)
  //        = xscale*(x/z) + xoffset            (I flipped the signs of both
  //        sides)
  ...
  *pFov = Max(Max(fov_px, fov_nx), Max(fov_py, fov_ny));
  // FIXME: hey you know, I could do the Max() series before I call all those
  // expensive atanf()s...
}
...
// Purpose:
// --------------------------------------------------------------------
void CClientVirtualReality::Disconnect() { BaseClass::Disconnect(); }
...
// Purpose:
// --------------------------------------------------------------------
void CClientVirtualReality::Shutdown() { BaseClass::Shutdown(); }

// --------------------------------------------------------------------
// Purpose: Draws the main menu in Stereo
// --------------------------------------------------------------------
void CClientVirtualReality::DrawMainMenu() {
  // have to draw the UI in stereo via the render texture or it won't fuse
  // properly
  ...
  pRenderContext->PushRenderTargetAndViewport(
      pTexture, NULL, 0, 0, viewActualWidth, viewActualHeight);
  ...
  g_pSourceVR->GetViewportBounds(ISourceVirtualReality::VREye_Left, &leftX,
                                 &leftY, &leftW, &leftH);
  g_pSourceVR->GetViewportBounds(ISourceVirtualReality::VREye_Right, &rightX,
                                 &rightY, &rightW, &rightH);
  ...
  viewEye[STEREO_EYE_MONO].width = viewEye[STEREO_EYE_MONO].m_nUnscaledWidth =
      leftW;
  viewEye[STEREO_EYE_MONO].height = viewEye[STEREO_EYE_MONO].m_nUnscaledHeight =
      leftH;

  viewEye[STEREO_EYE_LEFT] = viewEye[STEREO_EYE_RIGHT] =
      viewEye[STEREO_EYE_MONO];
  ...
  OverrideView(&viewEye[STEREO_EYE_MONO], &vViewModelOrigin, &qViewModelAngles,
               HMM_NOOVERRIDE);
  g_ClientVirtualReality.OverrideStereoView(&viewEye[STEREO_EYE_MONO],
                                            &viewEye[STEREO_EYE_LEFT],
                                            &viewEye[STEREO_EYE_RIGHT]);

  // render both eyes
  for (int nView = STEREO_EYE_LEFT; nView <= STEREO_EYE_RIGHT; nView++) {
    CMatRenderContextPtr pRenderContext(materials);
    PIXEvent pixEvent(pRenderContext,
                      nView == STEREO_EYE_LEFT ? "left eye" : "right eye");

    ITexture *pColor =
        g_pSourceVR->GetRenderTarget((ISourceVirtualReality::VREye)(nView - 1),
                                     ISourceVirtualReality::RT_Color);
    ITexture *pDepth =
        g_pSourceVR->GetRenderTarget((ISourceVirtualReality::VREye)(nView - 1),
                                     ISourceVirtualReality::RT_Depth);
    render->Push3DView(viewEye[nView], VIEW_CLEAR_DEPTH | VIEW_CLEAR_COLOR,
                       pColor, NULL, pDepth);
...
// --------------------------------------------------------------------
bool CClientVirtualReality::OverrideView(
    CViewSetup *pViewMiddle, Vector *pViewModelOrigin, QAngle *pViewModelAngles,
  ...
  if (hmmMovementOverride == HMM_NOOVERRIDE) {
    if (CurrentlyZoomed()) {
      m_hmmMovementActual =
          static_cast<HeadtrackMovementMode_t>(vr_moveaim_mode_zoom.GetInt());
    } else {
      m_hmmMovementActual =
          static_cast<HeadtrackMovementMode_t>(vr_moveaim_mode.GetInt());
    }
  } else {
    m_hmmMovementActual = hmmMovementOverride;
  }

  // Incoming data may or may not be useful - it is the origin and aim of the
  // "player", i.e. where bullets come from. In some modes it is an independent
  // thing, guided by the mouse & keyboard = useful. In other modes it's just
  // where the HMD was pointed last frame, modified slightly by kbd+mouse. In
  // those cases, we should use our internal reference (which keeps track thanks
  // to OverridePlayerMotion)
  QAngle originalMiddleAngles = pViewMiddle->angles;
  Vector originalMiddleOrigin = pViewMiddle->origin;

  // Figure out the in-game "torso" concept, which corresponds to the player's
  // physical torso.
  m_PlayerTorsoOrigin = pViewMiddle->origin;

  // Ignore what was passed in - it's just the direction the weapon is pointing,
  // which was determined by last frame's HMD orientation! Instead use our
  // cached value.
  ...
  //// Scale translation e.g. to allow big in-game leans with only a small head
  ///movement. / Clamp HMD movement to a reasonable amount to avoid wallhacks,
  ///vis problems, etc.
  ...
  // Now figure out the three principal matrices: m_TorsoFromMideye,
  // m_WorldFromMidEye, m_WorldFromWeapon m_TorsoFromMideye is done so that
  // OverridePlayerMotion knows what to do with WASD.
  ...
  case HMM_SHOOTMOVEMOUSE_LOOKFACE:
    // Aim point is independent of view - leave it as it was, just copy it into
    // m_WorldFromWeapon for our use.
    m_TorsoFromMideye = matMideyeZeroFromMideyeCurrent;
    m_WorldFromMidEye = worldFromTorso * matMideyeZeroFromMideyeCurrent;
    m_WorldFromWeapon.SetupMatrixOrgAngles(originalMiddleOrigin,
                                           originalMiddleAngles);
    break;
  ...
  case HMM_SHOOTMOVELOOKMOUSE:
    ...
    worldFromTorso.SetupMatrixOrgAngles(m_PlayerTorsoOrigin,
                                        originalMiddleAngles);
  ...
  VectorAngles(m_WorldFromMidEye.GetForward(), m_WorldFromMidEye.GetUp(),
               pViewMiddle->angles);
  ...
  *pViewModelAngles = pViewMiddle->angles;
  ...
  if (vr_debug_remote_cam.GetBool()) {
    Vector vOffset(vr_debug_remote_cam_pos_x.GetFloat(),
                   vr_debug_remote_cam_pos_y.GetFloat(),
                   vr_debug_remote_cam_pos_z.GetFloat());
    Vector vLookat(vr_debug_remote_cam_target_x.GetFloat(),
                   vr_debug_remote_cam_target_y.GetFloat(),
    ...
    m_WorldFromMidEye.SetupMatrixOrgAngles(pViewMiddle->origin,
                                           pViewMiddle->angles);
...
// --------------------------------------------------------------------
bool CClientVirtualReality::OverrideWeaponHudAimVectors(Vector *pAimOrigin,
                                                        Vector *pAimDirection) {
  if (!UseVR()) {
    return false;
  }
  ...
  // So give it some nice high-fps numbers, not the low-fps ones we get from the
  // game.
...
// --------------------------------------------------------------------
// Purpose:
//		Set up the left and right eyes from the middle eye if stereo is
//on. 		Advise calling soonish after OverrideView().
// --------------------------------------------------------------------
bool CClientVirtualReality::OverrideStereoView(CViewSetup *pViewMiddle,
                                               CViewSetup *pViewLeft,
  ...
  VMatrix matOffsetLeft =
      g_pSourceVR->GetMidEyeFromEye(ISourceVirtualReality::VREye_Left);
  VMatrix matOffsetRight =
      g_pSourceVR->GetMidEyeFromEye(ISourceVirtualReality::VREye_Right);
  ...
  MatrixAngles(worldFromRightEye.As3x4(), pViewRight->angles,
               pViewRight->origin);
  ...
  g_pSourceVR->GetEyeProjectionMatrix(
      &pViewLeft->m_ViewToProjection, ISourceVirtualReality::VREye_Left,
      pViewMiddle->zNear, pViewMiddle->zFar, 1.0f / headtrackFovScale);
  g_pSourceVR->GetEyeProjectionMatrix(
      &pViewRight->m_ViewToProjection, ISourceVirtualReality::VREye_Right,
      pViewMiddle->zNear, pViewMiddle->zFar, 1.0f / headtrackFovScale);
  ...
  for (int i = 0; i < 4; i++) {
    for (int j = 0; j < 4; j++) {
      pViewMiddle->m_ViewToProjection.m[i][j] =
          (pViewLeft->m_ViewToProjection.m[i][j] +
           pViewRight->m_ViewToProjection.m[i][j]) *
          0.5f;
    }
  }
  ...
  // To make culling work correctly, calculate the widest FOV of each projection
  // matrix.
  ...
  // The aspect ratio of the HMD may be something bizarre (e.g. Rift is
  // 640x800), and the pixels may not be square, so don't use that!
  ...
  m_fHudHalfHeight =
      tan(DEG2RAD(fVFOV * 0.5f)) * fHudForward * m_WorldZoomScale;

  QAngle HudAngles;
  switch (m_hmmMovementActual) {
  case HMM_SHOOTFACE_MOVETORSO:
    // Put the HUD in front of the player's torso.
    // This helps keep you oriented about where "forwards" is, which is
    // otherwise surprisingly tricky!
  ...
  // This is a bitfield. A set bit means lock to the world, a clear bit means
  // don't.
  ...
  if ((iVrHudAxisLockToWorld & (1 << YAW)) != 0) {
    // Locking the yaw to the world is not particularly helpful, so what it
    // actually means is lock it to the weapon.
    QAngle aimAngles;
  ...
  // We need to transform to a more conventional X right, Y up, Z backwards
...
//			to HMD status.
// --------------------------------------------------------------------
bool CClientVirtualReality::OverridePlayerMotion(float flInputSampleFrametime,
                                                 const QAngle &oldAngles,
                                                 const QAngle &curAngles,
                                                 const Vector &curMotion,
                                                 QAngle *pNewAngles,
                                                 Vector *pNewMotion) {
  ...
  *pNewMotion = curMotion;
  ...
  // originalAngles tells us what the weapon angles were before whatever mouse,
  // joystick, etc thing changed them - called "old" curAngles holds the new
  // weapon angles after mouse, joystick, etc. applied. We need to compute what
  // weapon angles WE want and return them in *pNewAngles - called "new"

  VMatrix worldFromTorso;

  // Whatever position is already here (set up by OverrideView) needs to be
  // preserved.
  ...
  case HMM_SHOOTFACE_MOVETORSO: {
    ...
    // We ignore mouse pitch, the mouse can't do rolls, so it's just yaw
    // changes.
    ...
    // Weapon view = mideye view, so apply that to the torso to find the world
    // view direction.
  ...
  case HMM_SHOOTMOVELOOKMOUSE: {
    ...
    *pNewMotion = curMotion;
    ...
    // Move the torso by the yaw angles - torso should not have roll or pitch or
    // you'll make folks ill.
    if (!m_bOverrideTorsoAngle &&
        m_hmmMovementActual != HMM_SHOOTMOVELOOKMOUSEFACE) {
  ...
  case HMM_SHOOTBOUNDEDMOUSE_LOOKFACE_MOVEMOUSE: {
    ...
    *pNewMotion = curMotion;
    ...
    if (CurrentlyZoomed()) {
      fReticleYawLimit =
          vr_moveaim_reticle_yaw_limit_zoom.GetFloat() * m_WorldZoomScale;
      fReticlePitchLimit =
          vr_moveaim_reticle_pitch_limit_zoom.GetFloat() * m_WorldZoomScale;
    ...
    if (fReticlePitchLimit >= 0.0f) {
      ...
      (*pNewAngles)[PITCH] =
          Clamp(curAngles[PITCH], m_PlayerViewAngle[PITCH] - fReticlePitchLimit,
                m_PlayerViewAngle[PITCH] + fReticlePitchLimit);
    }

    // For yaw the concept here is the torso stays within a set number of
    // degrees of the weapon in yaw. However, with drifty tracking systems (e.g.
    // IMUs) the concept of "torso" is hazy. Really it's just a mechanism to
    // turn the view without moving the head - its absolute orientation is not
    // that useful. So... if the mouse is to the right greater than the chosen
    // angle from the view, and then moves more right, it will drag the torso
    // (and thus the view) right, so it stays on the edge of the view. But if it
    // moves left towards the view, it does no dragging. Note that if the mouse
    // does not move, but the view moves, it will NOT drag at all. This allows
    // people to mouse-aim within their view, but also to flick-turn with the
    // mouse, and to flick-glance with the head.
    if (fReticleYawLimit >= 0.0f) {
      float fViewToWeaponYaw =
          AngleDiff(curAngles[YAW], m_PlayerViewAngle[YAW]);
  ...
  case HMM_SHOOTMOUSE_MOVEFACE: {
    (*pNewAngles)[PITCH] =
        clamp((*pNewAngles)[PITCH], m_PlayerViewAngle[PITCH] - 15.f,
              m_PlayerViewAngle[PITCH] + 15.f);
    ...
    if (fDiff > 15.f) {
      ...
      m_PlayerTorsoAngle[YAW] +=
          AngleDiff(curAngles[YAW], oldAngles[YAW]) / 2.f;
    }
  ...
  switch (m_hmmMovementActual) {
  case HMM_SHOOTBOUNDEDMOUSE_LOOKFACE_MOVEFACE: {
    // The motion passed in is meant to be relative to the face, so jimmy it to
    // be relative to the new weapon aim.
  ...
  case HMM_SHOOTFACE_MOVETORSO: {
    // The motion passed in is meant to be relative to the torso, so jimmy it to
    // be relative to the new weapon aim.
...
//			different.
// --------------------------------------------------------------------
void CClientVirtualReality::OverrideTorsoTransform(const Vector &position,
                                                   const QAngle &angles) {
  if (m_iAlignTorsoAndViewToWeaponCountdown > 0) {
    m_iAlignTorsoAndViewToWeaponCountdown--;
    ...
    // this is how far off the torso we actually set will need to be to keep the
    // current "forward" vector while the torso angle is being overridden.
...
//			position the HUD.
// --------------------------------------------------------------------
void CClientVirtualReality::GetHUDBounds(Vector *pViewer, Vector *pUL,
                                         Vector *pUR, Vector *pLL,
                                         Vector *pLR) {
  Vector vHalfWidth = m_WorldFromHud.GetLeft() * -m_fHudHalfWidth;
  Vector vHalfHeight = m_WorldFromHud.GetUp() * m_fHudHalfHeight;
  Vector vHUDOrigin = m_PlayerViewOrigin +
                      m_WorldFromHud.GetForward() * vr_hud_forward.GetFloat();
...
void CClientVirtualReality::RenderHUDQuad(bool bBlackout, bool bTranslucent) {
  // If we can overlay the HUD directly onto the target later, we'll do that
  // instead (higher image quality).
...

  {
    ...
    if (bTranslucent) {
      ...
      mymat =
          materials->FindMaterial("vgui/inworldui_opaque", TEXTURE_GROUP_VGUI);
    }
  ...
  if (bBlackout) {
    ...
    // "Reflect" the HUD bounds through the viewer to find the ones behind the
    // head.
    vbUL = 2 * vHead - vLR;
    vbUR = 2 * vHead - vLL;
    vbLL = 2 * vHead - vUR;
...
bool CClientVirtualReality::ProcessCurrentTrackingState(float fGameFOV) {
  ...
  if (fGameFOV != 0.0f) {
    ...
    // The game has overridden the FOV, e.g. because of a sniper scope. So we
    // need to match this view with whatever actual FOV the HUD has.
    float wantedGameTanfov = tanf(DEG2RAD(fGameFOV * 0.5f));
    // OK, so now in stereo mode, we're going to also draw an overlay, but that
    // overlay usually covers more of the screen (because in a good HMD usually
    // our actual FOV is much wider)
    float overlayActualPhysicalTanfov =
        tanf(DEG2RAD(m_fHudHorizontalFov * 0.5f));
    // Therefore... (remembering that a zoom > 1.0 means you zoom *out*)
    m_WorldZoomScale = wantedGameTanfov / overlayActualPhysicalTanfov;
  }

  return g_pSourceVR->SampleTrackingState(fGameFOV,
                                          0.f /* seconds to predict */);
}
...
const VMatrix &CClientVirtualReality::GetHudProjectionFromWorld() {
  // This matrix will transform a world-space position into a homogenous
  // HUD-space vector. So if you divide x+y by w, you will get the position on
  // the HUD in [-1,1] space.
...
// --------------------------------------------------------------------
void CClientVirtualReality::GetTorsoRelativeAim(Vector *pPosition,
                                                QAngle *pAngles) {
...
// --------------------------------------------------------------------
void CClientVirtualReality::OverrideViewModelTransform(Vector &vmorigin,
                                                       QAngle &vmangles,
                                                       bool bUseLargeOverride) {
  Vector vForward, vRight, vUp;
  AngleVectors(vmangles, &vForward, &vRight, &vUp);

  float fForward = bUseLargeOverride
                       ? vr_viewmodel_offset_forward_large.GetFloat()
                       : vr_viewmodel_offset_forward.GetFloat();
...
void CClientVirtualReality::PostProcessFrame(StereoEye_t eEye) {
  ...
  g_pSourceVR->DoDistortionProcessing(eEye == STEREO_EYE_LEFT
...
// --------------------------------------------------------------------
void CClientVirtualReality::OverlayHUDQuadWithUndistort(
    const CViewSetup &eyeView, bool bDoUndistort, bool bBlackout,
    bool bTranslucent) {
  if (!UseVR())
    return;

  // If we can't overlay the HUD, it will be handled on another path (rendered
  // into the scene with RenderHUDQuad()).
  if (!CanOverlayHudQuad())
    return;

  // Get the position of the HUD quad in world space as used by RenderHUDQuad().
  // Then convert to a rectangle in normalized device coordinates.

  Vector vHead, vUL, vUR, vLL, vLR;
  GetHUDBounds(&vHead, &vUL, &vUR, &vLL, &vLR);

  VMatrix worldToView, viewToProjection, worldToProjection, worldToPixels;
  render->GetMatricesForView(eyeView, &worldToView, &viewToProjection,
  ...
  ISourceVirtualReality::VREye sourceVrEye =
      (eyeView.m_eStereoEye == STEREO_EYE_LEFT)
          ? ISourceVirtualReality::VREye_Left
          : ISourceVirtualReality::VREye_Right;

  g_pSourceVR->CompositeHud(sourceVrEye, ndcHudBounds, bDoUndistort, bBlackout,
                            bTranslucent);
}
...
void CClientVirtualReality::Activate() {
  ...
  if (!ShouldForceVRActive()) {
    ...
    if (nVRModeAdapter == -1) {
      Warning("Unable to get VRMode adapter from OpenVR. VR mode cannot be "
              "enabled. Try restarting and then enabling VR again.\n");
      return;
    }

    // we can only activate if we've got a VR device
    if (materials->GetCurrentConfigForVideoCard().m_nVRModeAdapter !=
        nVRModeAdapter) {
      Warning(
          "VR Mode expects adapter %d which is different from %d which we are "
          "currently using. Try restarting and enabling VR mode again.\n",
          nVRModeAdapter,
  ...
  m_bNonVRWindowed =
      g_pMaterialSystem->GetCurrentConfigForVideoCard().Windowed();
  ...
  g_pSourceVR->GetViewportBounds(ISourceVirtualReality::VREye_Left, NULL, NULL,
                                 &nViewportWidth, &nViewportHeight);
  g_pMatSystemSurface->SetFullscreenViewportAndRenderTarget(
      0, 0, nViewportWidth, nViewportHeight,
      g_pSourceVR->GetRenderTarget(ISourceVirtualReality::VREye_Left,
                                   ISourceVirtualReality::RT_Color));
...
void CClientVirtualReality::Deactivate() {
  ...
  g_pMaterialSystem->GetRenderContext()->Viewport(0, 0, m_nNonVRWidth,
                                                  m_nNonVRHeight);
  g_pMatSystemSurface->SetFullscreenViewportAndRenderTarget(
      0, 0, m_nNonVRWidth, m_nNonVRHeight, NULL);
  ...
  char szCmd[256];
  Q_snprintf(szCmd, sizeof(szCmd), "mat_setvideomode %i %i %i\n", m_nNonVRWidth,
             m_nNonVRHeight, m_bNonVRWindowed ? 1 : 0);
  engine->ClientCmd_Unrestricted(szCmd);
}
