  ...
  if (r_decals.GetInt()) {
    ...
    if (ent) {
      ...
      if (!(bNoBlood && bIsBlood)) {
        effects->DecalShoot(m_nIndex, 0, ent->GetModel(), ent->GetAbsOrigin(),
                            ent->GetAbsAngles(), m_vecOrigin, 0, 0);
      }
    }
  }
...
void TE_WorldDecal(IRecipientFilter &filter, float delay,
  ...
  TE_WorldDecal(filter, 0.0f, &vecOrigin,
