...
#define FTENT_ATTACHTOTARGET                                                   \
  0x00040000 // attach to whatever we hit, and stay there until die time is up
#define FTENT_NOMODEL                                                          \
  0x00080000 // Doesn't have a model, never try to draw ( it just triggers other
             // things )
#define FTENT_CLIENTCUSTOM                                                     \
  0x00100000 // Must specify callback.  Callback function is responsible for
             // killing tempent and updating fields ( unless other flags specify
             // how to do things )
