...
static const char *s_NetworkMessageNames[MAX_NETMESSAGE] = {
...
// Purpose:
//-----------------------------------------------------------------------------
float CHudMessage::FadeBlend(float fadein, float fadeout, float hold,
                             float localTime) {
  ...
  case 2:
    ...
    if (m_parms.charTime > m_parms.time) {
      ...
      if (m_parms.time > m_parms.fadeTime) {
        ...
        blend =
            255 - (deltaTime * (1.0 / m_parms.pMessage->fxtime) * 255.0 + 0.5);
      }
    }
    break;
  }
...
void CHudMessage::MessageScanStart(void) {
  ...
  case 0:
    ...
    if (m_parms.time < m_parms.pMessage->fadein) {
      m_parms.fadeBlend = ((m_parms.pMessage->fadein - m_parms.time) *
                           (1.0 / m_parms.pMessage->fadein) * 255);
    } else if (m_parms.time > m_parms.fadeTime) {
      if (m_parms.pMessage->fadeout > 0)
        m_parms.fadeBlend =
            (((m_parms.time - m_parms.fadeTime) / m_parms.pMessage->fadeout) *
             255);
      else
  ...
  case 2:
    m_parms.fadeTime = (m_parms.pMessage->fadein * m_parms.length) +
                       m_parms.pMessage->holdtime;
...
void CHudMessage::MessageDrawScan(client_textmessage_t *pMessage, float time) {
  ...
  if (pMessage->bRoundedRectBackdropBox) {
    ...
    float flAlphaScale =
        clamp((255.0f - (float)m_parms.fadeBlend) / 255.0f, 0.0f, 1.0f);
    Color boxColor(pMessage->boxcolor[0], pMessage->boxcolor[1],
                   pMessage->boxcolor[2], pMessage->boxcolor[3] * flAlphaScale);

    DrawBox(boxx, boxy, m_parms.totalWidth + 2.0f * flBoxPixels,
            m_parms.totalHeight + 2.0f * flBoxPixels * 0.5f, boxColor, 1.0f);
  }
  ...
  for (i = 0; i < m_parms.lines; i++) {
    ...
    for (j = 0; j < m_parms.lineLength; j++) {
      m_parms.text = line[j];
      MessageScanNextChar();
      textmessage->AddChar(m_parms.r, m_parms.g, m_parms.b,
                           255 - m_parms.fadeBlend, m_parms.text);
    }
...
void CHudMessage::Paint() {
  ...
  if (m_gameTitleTime > 0) {
    ...
    if (localTime > (m_pGameTitle->fadein + m_pGameTitle->holdtime +
                     m_pGameTitle->fadeout)) {
      m_gameTitleTime = 0;
    } else {
      brightness = FadeBlend(m_pGameTitle->fadein, m_pGameTitle->fadeout,
                             m_pGameTitle->holdtime, localTime);
      ...
      m_iconTitleHalf->DrawSelf(x, y,
                                Color(m_pGameTitle->r1, m_pGameTitle->g1,
                                      m_pGameTitle->b1, brightness * 255));
  ...
  for (i = 0; i < maxHUDMessages; i++) {
    ...
    if (m_pMessages[i]) {
      ...
      if (m_startTime[i] > gpGlobals->curtime)
        m_startTime[i] =
            gpGlobals->curtime + m_parms.time - m_startTime[i] +
  ...
  for (i = 0; i < maxHUDMessages; i++) {
    if (m_pMessages[i]) {
      ...
      case 0:
      case 1:
        endTime = m_startTime[i] + pMessage->fadein + pMessage->fadeout +
                  pMessage->holdtime;
        break;

      // Fade in is per character in scanning messages
      case 2:
        endTime = m_startTime[i] +
                  (pMessage->fadein * strlen(pMessage->pMessage)) +
                  pMessage->fadeout + pMessage->holdtime;
        break;
...
void CHudMessage::MessageAdd(const char *pName) {
  ...
  if (pMessage->pClearMessage) {
    for (i = 0; i < maxHUDMessages; i++) {
      if (m_pMessages[i] &&
          !Q_stricmp(m_pMessages[i]->pName, pMessage->pClearMessage)) {
        m_startTime[i] = 0.0f;
        m_pMessages[i] = NULL;
        break;
      }
...
void CHudMessage::MsgFunc_HudMsg(bf_read &msg) {
  ...
  client_textmessage_t *pNetMessage =
      TextMessageGet(s_NetworkMessageNames[channel]);
...
//-----------------------------------------------------------------------------
void CHudMessage::ResetCharacters(void) { m_Messages.Purge(); }
...
// Purpose:
//-----------------------------------------------------------------------------
void CHudMessage::SetDefaultFont(void) { SetFont(m_hDefaultFont); }
