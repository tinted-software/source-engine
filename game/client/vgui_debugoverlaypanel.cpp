...
CDebugOverlay::CDebugOverlay(vgui::VPANEL parent)
  ...
  vgui::HScheme scheme = vgui::scheme()->LoadSchemeFromFileEx(
      enginevgui->GetPanel(PANEL_CLIENTDLL_TOOLS), "resource/ClientScheme.res",
      "Client");
  SetScheme(scheme);
    ...
    if (pCurrText->text != NULL) {
      ...
      if (pCurrText->bUseOrigin) {
        if (!debugoverlay->ScreenPosition(pCurrText->origin, screenPos)) {
          ...
          float yPos =
              screenPos[1] + (pCurrText->lineOffset * 13); // Line spacing;
          g_pMatSystemSurface->DrawColoredText(m_hFont, xPos, yPos, r, g, b, a,
                                               "%s", pCurrText->text);
        }
      } else {
        if (!debugoverlay->ScreenPosition(pCurrText->flXPos, pCurrText->flYPos,
                                          screenPos)) {
          float xPos = screenPos[0];
          float yPos =
              screenPos[1] + (pCurrText->lineOffset * 13); // Line spacing;
...
public:
  CDebugOverlayPanel(void) { debugOverlayPanel = NULL; }
...
void DebugDrawLine(const Vector &vecAbsStart, const Vector &vecAbsEnd, int r,
  ...
  debugoverlay->AddLineOverlay(vecAbsStart + Vector(0, 0, 0.1),
                               vecAbsEnd + Vector(0, 0, 0.1), r, g, b, test,
                               duration);
}
