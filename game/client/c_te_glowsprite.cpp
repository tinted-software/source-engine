...
RecvPropVector(RECVINFO(m_vecOrigin)), RecvPropInt(RECVINFO(m_nModelIndex)),
    RecvPropFloat(RECVINFO(m_fScale)), RecvPropFloat(RECVINFO(m_fLife)),
...
void C_TEGlowSprite::PostDataUpdate(DataUpdateType_t updateType) {
  ...
  C_LocalTempEntity *ent = tempents->TempSprite(
      m_vecOrigin, vec3_origin, m_fScale, m_nModelIndex, kRenderTransAdd, 0, a,
      m_fLife, FTENT_SPRANIMATE | FTENT_SPRANIMATELOOP);
  if (ent) {
    ent->bounceFactor = 0.2;
  }
  RecordGlowSprite(m_vecOrigin, m_nModelIndex, m_fLife, m_fScale,
                   m_nBrightness);
}
...
void TE_GlowSprite(IRecipientFilter &filter, float delay, const Vector *pos,
  ...
  C_LocalTempEntity *ent =
      tempents->TempSprite(*pos, vec3_origin, size, modelindex, kRenderTransAdd,
                           0, a, life, FTENT_SPRANIMATE | FTENT_SPRANIMATELOOP);
  if (ent) {
    ent->bounceFactor = 0.2;
  }
...
void TE_GlowSprite(IRecipientFilter &filter, float delay,
  ...
  TE_GlowSprite(filter, delay, &vecOrigin, nModelIndex, flDuration, flSize,
                nBrightness);
}
