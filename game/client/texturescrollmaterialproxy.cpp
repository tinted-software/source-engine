...
bool CTextureScrollMaterialProxy::Init(IMaterial *pMaterial,
...
void CTextureScrollMaterialProxy::OnBind(void *pC_BaseEntity) {
  ...
  if (m_pTextureScrollVar->GetType() == MATERIAL_VAR_TYPE_MATRIX) {
    VMatrix mat(scale, 0.0f, 0.0f, sOffset, 0.0f, scale, 0.0f, tOffset, 0.0f,
...
EXPOSE_INTERFACE(CTextureScrollMaterialProxy, IMaterialProxy,
                 "TextureScroll" IMATERIAL_PROXY_INTERFACE_VERSION);
