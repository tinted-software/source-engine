...
public:
  ...
  // Note: there are two ways the directional light can be specified. The
  // default is to use DirLightColor and a default dirlight source (from above
  // or below).
  //		In this case, m_DirLight.m_vPos and m_DirLight.m_flIntensity are
  //ignored.
  //
  // The other is to attach a directional env_particlelight to us.
  //		In this case, m_DirLightSource is ignored and all the m_DirLight
  //parameters are used.
...
RecvPropFloat(RECVINFO(m_SpreadSpeed), 0), RecvPropFloat(RECVINFO(m_Speed), 0),
    RecvPropFloat(RECVINFO(m_StartSize), 0),
    RecvPropFloat(RECVINFO(m_EndSize), 0), RecvPropFloat(RECVINFO(m_Rate), 0),
    RecvPropFloat(RECVINFO(m_JetLength), 0), RecvPropInt(RECVINFO(m_bEmit), 0),
    RecvPropFloat(RECVINFO(m_flBaseSpread)), RecvPropFloat(RECVINFO(m_flTwist)),
...
void C_SmokeStack::Start(CParticleMgr *pParticleMgr,
  ...
  while (filesystem->FileExists(VarArgs("materials/%s", szNames)) &&
         iCount < SMOKESTACK_MAX_MATERIALS) {
  ...
  m_ParticleSpawn.Init(mat_reduceparticles.GetBool()
                           ? m_Rate / 4
                           : m_Rate); // Obey mat_reduceparticles in episodic
#else
  ...
  IMaterial *pMaterial =
      pParticleMgr->PMaterialToIMaterial(m_MaterialHandle[0]);
  if (pMaterial) {
...
void C_SmokeStack::Update(float fTimeDelta) {
  ...
  if (m_bEmit && (m_ParticleEffect.WasDrawnPrevFrame() ||
                  m_ParticleEffect.GetAlwaysSimulate())) {
    // Add new particles.
    ...
    while (m_ParticleSpawn.NextEvent(tempDelta)) {
      ...
      // Make a new particle.
      if (SmokeStackParticle *pParticle =
              (SmokeStackParticle *)m_ParticleEffect.AddParticle(
        ...
        pParticle->m_flRollDelta =
            random->RandomFloat(-m_flRollSpeed, m_flRollSpeed);
...
void C_SmokeStack::QueueLightParametersInRenderer() {
  m_Renderer.SetBaseColor(Vector(m_clrRender->r / 255.0f,
...
void C_SmokeStack::RenderParticles(CParticleRenderIterator *pIterator) {
  const SmokeStackParticle *pParticle =
      (const SmokeStackParticle *)pIterator->GetFirst();
  while (pParticle) {
    // Transform.
    Vector tPos;
    TransformParticle(m_pParticleMgr->GetModelView(), pParticle->m_Pos, tPos);

    // Figure out its alpha. Squaring it after it gets halfway through its
    // lifetime makes it get translucent and fade out for a longer time.
    ...
    m_Renderer.RenderParticle(pIterator->GetParticleDraw(), pParticle->m_Pos,
                              tPos, alpha * m_flAlphaScale,
                              FLerp(m_StartSize, m_EndSize, tLifetime),
                              DEG2RAD(pParticle->m_flAngle));

    pParticle =
        (const SmokeStackParticle *)pIterator->GetNext(pParticle->m_flSortPos);
  }
}
...
void C_SmokeStack::SimulateParticles(CParticleSimulateIterator *pIterator) {
  ...
  while (pParticle) {
    ...
    if (tLifetime >= 1) {
      ...
      Vector tPos;
      if (m_bTwist) {
        Vector vTwist(pParticle->m_Pos.x - GetAbsOrigin().x,
                      pParticle->m_Pos.y - GetAbsOrigin().y, 0);

        pParticle->m_Pos.x = vTwist.x * m_TwistMat[0][0] +
                             vTwist.y * m_TwistMat[0][1] + GetAbsOrigin().x;
        pParticle->m_Pos.y = vTwist.x * m_TwistMat[1][0] +
                             vTwist.y * m_TwistMat[1][1] + GetAbsOrigin().y;
      }

#ifndef HL2_EPISODIC
      pParticle->m_Pos =
          pParticle->m_Pos + pParticle->m_Velocity * pIterator->GetTimeDelta() +
          pParticle->m_vAccel *
              (0.5f * pIterator->GetTimeDelta() * pIterator->GetTimeDelta());
      ...
      pParticle->m_flAngle +=
          pParticle->m_flRollDelta * pIterator->GetTimeDelta();

      if (bSortNow == true) {
        Vector tPos;
        TransformParticle(m_pParticleMgr->GetModelView(), pParticle->m_Pos,
                          tPos);
