...
template <typename Type, bool IS_ARRAY> struct CInterpolatedVarEntryBase {
...
template <typename Type> struct CInterpolatedVarEntryBase<Type, false> {
...
template <typename T> class CSimpleRingBuffer {
public:
...
public:
  ...
  void GetDerivative_SmoothVelocity(
      Type *pOut,
      float currentTime); // See notes on ::Derivative_HermiteLinearVelocity for
                          // info.
...
template <typename Type, bool IS_ARRAY>
inline void
CInterpolatedVarArrayBase<Type, IS_ARRAY>::RemoveOldEntries(float oldesttime) {
...
template <typename Type, bool IS_ARRAY>
inline void
CInterpolatedVarArrayBase<Type, IS_ARRAY>::DebugInterpolate(Type *pOut,
                                                            float currentTime) {
...
inline int CInterpolatedVarArrayBase<Type, IS_ARRAY>::Interpolate(
  ...
  if (m_bDebug) {
    // "value will hold" means we are either extrapolating, or the samples in
    // GetInterpolationInfo are all the same... In either case there are no more
    // "changes" until we latch a new
...
template <typename Type, bool IS_ARRAY>
inline int
CInterpolatedVarArrayBase<Type, IS_ARRAY>::Interpolate(float currentTime) {
  return Interpolate(currentTime, m_InterpolationAmount);
}

template <typename Type, bool IS_ARRAY>
inline void
CInterpolatedVarArrayBase<Type, IS_ARRAY>::Copy(IInterpolatedVar *pInSrc) {
  ...
  if (!pSrc || pSrc->m_nMaxCount != m_nMaxCount) {
    if (pSrc) {
      AssertMsg3(false, "pSrc->m_nMaxCount (%i) != m_nMaxCount (%i) for %s.",
                 pSrc->m_nMaxCount, m_nMaxCount, m_pDebugName);
    } else {
      AssertMsg(
          false,
          "pSrc was null in CInterpolatedVarArrayBase<Type, IS_ARRAY>::Copy.");
    }
...
template <typename Type, bool IS_ARRAY>
inline const Type &
CInterpolatedVarArrayBase<Type, IS_ARRAY>::GetPrev(int iArrayIndex) const {
  Assert(m_pValue);
...
template <typename Type, bool IS_ARRAY>
inline const Type &
CInterpolatedVarArrayBase<Type, IS_ARRAY>::GetCurrent(int iArrayIndex) const {
  Assert(m_pValue);
  Assert(iArrayIndex >= 0 && iArrayIndex < m_nMaxCount);
...
template <typename Type, bool IS_ARRAY>
inline void
CInterpolatedVarArrayBase<Type, IS_ARRAY>::SetLooping(bool looping,
                                                      int iArrayIndex) {
  Assert(iArrayIndex >= 0 && iArrayIndex < m_nMaxCount);
