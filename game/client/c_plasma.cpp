...
RecvPropFloat(RECVINFO(m_flStartScale)),
    ...
    RecvPropInt(RECVINFO(m_nFlags)), RecvPropInt(RECVINFO(m_nPlasmaModelIndex)),
...
void C_Plasma::AddEntity(void) {
  ...
  m_entGlow.SetScale(
      MAX(0.0000001f, (m_flScaleRegister * 1.5f) + GetFlickerScale()));
  m_entGlow.SetLocalOriginDim(Z_INDEX, m_entGlow.GetLocalOriginDim(Z_INDEX) +
                                           (dScale * 32.0f));
}
