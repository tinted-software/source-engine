...
bool IsLocalPlayerUsingVisionFilterFlags(int nFlags,
  ...
  if (!bWeaponsCheck) {
    // We can only modify the RJ flags with normal checks that won't take the
    // forced kill cam flags that can happen in weapon checks
    int nRJShaderFlags = nLocalPlayerFlags;
    if (nRJShaderFlags != 0 && GameRules() &&
        !GameRules()->AllowMapVisionFilterShaders()) {
      nRJShaderFlags = 0;
    }
...
bool GetTargetInHudSpace(C_BaseEntity *pTargetEntity, int &iX, int &iY,
                         Vector *vecOffset) {
  return GetVectorInHudSpace(pTargetEntity->WorldSpaceCenter(), iX, iY,
                             vecOffset);
}
...
// Output : int
//-----------------------------------------------------------------------------
int UTIL_EntitiesAlongRay(C_BaseEntity **pList, int listMax, const Ray_t &ray,
...
int UTIL_ComputeStringWidth(vgui::HFont &font, const char *str) {
  ...
  while (*p) {
    ...
    float wide, abcA;
    vgui::surface()->GetKernedCharWidth(font, *p, *pBefore, *pAfter, wide,
                                        abcA);
    pixels += wide;
#else
...
//control, e.g. %attack2% 			Input buffer sizes are in bytes rather than unicode
//character count 			for consistency with other APIs.  If inbufsizebytes is 0 a
//NULL-terminated 			input buffer is assumed, or you can pass the size of the input
//buffer if 			not NULL-terminated.
//-----------------------------------------------------------------------------
...
byte *UTIL_LoadFileForMe(const char *filename, int *pLength) {
  ...
  if (!buffer) {
    Warning("UTIL_LoadFileForMe:  Couldn't allocate buffer of size %i for file "
            "%s\n",
            size + 1, filename);
...
unsigned char UTIL_ComputeEntityFade(C_BaseEntity *pEntity, float flMinDist,
...
#endif
  {
    ...
    unsigned char nGlobalAlpha =
        IsXbox() ? 255
                 : modelinfo->ComputeLevelScreenFade(vecAbsCenter, flRadius,
                                                     flFadeScale);
...
bool UTIL_GetMapLoadCountFileName(const char *pszFilePrependName,
  ...
  if (IsX360()) {
    Q_snprintf(pszBuffer, iBuflen, "%s:/%s", MAP_KEY_FILE_DIR,
               pszFilePrependName);
  } else {
    Q_snprintf(pszBuffer, iBuflen, "%s/%s", MAP_KEY_FILE_DIR,
               pszFilePrependName);
