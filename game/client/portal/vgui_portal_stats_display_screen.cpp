...
private:
  ...
  float m_flNextDigitRandomizeTime; // next time to grab a new digit while
                                    // scrolling random digits in un-decoded
                                    // digits
...
DECLARE_VGUI_SCREEN_FACTORY(CPortalStatsDisplayScreen,
                            "portal_stats_display_screen");

//-----------------------------------------------------------------------------
// Constructor:
//-----------------------------------------------------------------------------
CPortalStatsDisplayScreen::CPortalStatsDisplayScreen(vgui::Panel *parent,
...
void CPortalStatsDisplayScreen::ApplySchemeSettings(IScheme *pScheme) {
  ...
  m_cUnknown =
      pScheme->GetColor("CPortalStatsDisplayScreen_Unknown", GetFgColor());
...
// Initialization
//-----------------------------------------------------------------------------
bool CPortalStatsDisplayScreen::Init(KeyValues *pKeyValues,
...
void CPortalStatsDisplayScreen::OnTick() {
  ...
  for (int iDisplayScreens = 0;
       iDisplayScreens < g_PropPortalStatsDisplays.Count(); ++iDisplayScreens) {
    C_PropPortalStatsDisplay *pPropPortalStatsDisplayTemp =
        g_PropPortalStatsDisplays[iDisplayScreens];
    if (pPropPortalStatsDisplayTemp &&
        pPropPortalStatsDisplayTemp->IsEnabled()) {
...
void CPortalStatsDisplayScreen::UpdateTextFields(
    C_PropPortalStatsDisplay *pPropPortalStatsDisplay) {
  ...
  m_pNumPlayerLabel->SetFgColor((fNumPlayer != 0.0f) ? (*pColors[iColor])
                                                     : (m_cInvisible));
  m_pNumGoalLabel->SetFgColor((pPropPortalStatsDisplay->GetGoalVisible())
                                  ? (*pColors[iColor])
                                  : (m_cInvisible));
...
void CPortalStatsDisplayScreen::UpdateMedals(
    C_PropPortalStatsDisplay *pPropPortalStatsDisplay) {
  ...
  for (int iGoal = 0; iGoal < PORTAL_LEVEL_STAT_TOTAL; ++iGoal) {
    ...
    for (int iLevel = 0; iLevel < 3; ++iLevel) {
      ...
      if (iGoal != pPropPortalStatsDisplay->GetDisplayObjective()) {
        ...
        // Set if "no medal" is visible
        pBitmapPanel = dynamic_cast<CBitmapPanel *>(GetChild(iMedalIndex + 1));
        pBitmapPanel->SetVisible(
            (iMedalComplete == -1 && iLevel == 0) ? (true) : (false));
      }
    }
  }
  ...
  iFirstChildIndex =
      FindChildIndexByName(TESTCHAMBER_STATS_FIRST_MEDAL_POINTER);

  // Loop through bronze, silver, and gold
  for (int iLevel = 0; iLevel < 3; ++iLevel) {
    pBitmapPanel =
        dynamic_cast<CBitmapPanel *>(GetChild(iFirstChildIndex + iLevel));
    pBitmapPanel->SetVisible(
        (iLevel == pPropPortalStatsDisplay->GetGoalLevelDisplay()) ? (true)
                                                                   : (false));
  }
  ...
  iFirstChildIndex =
      FindChildIndexByName(TESTCHAMBER_STATS_FIRST_SUBJECT_REACTION);

  // Loop through idle, happy, sad
  for (int iReaction = 0; iReaction < 3; ++iReaction) {
    pBitmapPanel =
        dynamic_cast<CBitmapPanel *>(GetChild(iFirstChildIndex + iReaction));
    pBitmapPanel->SetVisible((iReaction == iSubjectReaction) ? (true)
                                                             : (false));
  }
}