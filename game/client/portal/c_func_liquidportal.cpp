...
IMPLEMENT_CLIENTCLASS_DT(C_Func_LiquidPortal, DT_Func_LiquidPortal,
                         CFunc_LiquidPortal)
...
#define LIQUIDPORTAL_DYNAMICMESH_BOX_ADDVERTEX(vertnum, xtexbase, xtexscale,   \
                                               ytexbase, ytexscale)            \
  meshBuilder.Position3fv(&vVertices[vertnum].x);                              \
  meshBuilder.TexCoord2f(0, vVertices[vertnum].xtexbase *xtexscale,            \
                         vVertices[vertnum].ytexbase *ytexscale);              \
...
void C_Func_LiquidPortal::ComputeLinkMatrix(void) {
  ...
  if (pLinkedPortal) {
    ...
    // pLinkedPortal->CollisionProp()->WorldSpaceCenter() );
...
void CPortalRenderable_Func_LiquidPortal::DrawStencilMask(void) {
  ...
  DrawInnerLiquid(true, 1.0f,
                  g_pPortalRender->m_MaterialsAccess.m_WriteZ_Model);
}
...
void CPortalRenderable_Func_LiquidPortal::RenderPortalViewToBackBuffer(
  ...
  portalView.m_flAspectRatio =
      cameraView.m_flAspectRatio; // use the screen aspect ratio, 0.0f doesn't
                                  // work as advertised
...
void CPortalRenderable_Func_LiquidPortal::RenderPortalViewToTexture(
    CViewRender *pViewRender, const CViewSetup &cameraView) {}

void CPortalRenderable_Func_LiquidPortal::AddToVisAsExitPortal(
  ...
  pCustomVisibility->ForceViewLeaf(
      enginetrace->GetLeafContainingPoint(vOrigin));

  pCustomVisibility->AddVisOrigin(vOrigin);
}

bool CPortalRenderable_Func_LiquidPortal::DoesExitViewIntersectWaterPlane(
    float waterZ, int leafWaterDataID) const {
  return ((m_vAABBMins.z < waterZ) && (m_vAABBMaxs.z > waterZ));
}

SkyboxVisibility_t
CPortalRenderable_Func_LiquidPortal::SkyBoxVisibleFromPortal(void) {
  return SKYBOX_NOT_VISIBLE;
}

bool CPortalRenderable_Func_LiquidPortal::CalcFrustumThroughPortal(
    const Vector &ptCurrentViewOrigin, Frustum OutputFrustum,
    const VPlane *pInputFrustum, int iInputFrustumPlaneCount) {
  return false;
}
...
bool CPortalRenderable_Func_LiquidPortal::ShouldUpdatePortalView_BasedOnView(
    const CViewSetup &currentView, Frustum currentFrustum) {
  // return false;
  return IsFillingNow();
}

CPortalRenderable *
CPortalRenderable_Func_LiquidPortal::GetLinkedPortal() const {
  return m_pLinkedPortal;
}

bool CPortalRenderable_Func_LiquidPortal::ShouldUpdateDepthDoublerTexture(
...
void CPortalRenderable_Func_LiquidPortal::DrawPortal(void) {
  if (IsFillingNow()) {
    ...
    // "glass/glasswindow_refract01", TEXTURE_GROUP_OTHER );
...
void CPortalRenderable_Func_LiquidPortal::GetToolRecordingState(
    bool bActive, KeyValues *msg) {}

void CPortalRenderable_Func_LiquidPortal::HandlePortalPlaybackMessage(
    KeyValues *pKeyValues) {}

void CPortalRenderable_Func_LiquidPortal::DrawOutwardBox(
    const IMaterial *pMaterial) {
  if (pMaterial == NULL)
    pMaterial = materials->FindMaterial("glass/glasswindow_refract01",
                                        TEXTURE_GROUP_OTHER);

  const float fVerticalTextureScale = 1.0f / 100.0f;
  const float fHorizontalTextureScale = 1.0f / 100.0f;

  float fMaxZ = m_vAABBMins.z + ((m_vAABBMaxs.z - m_vAABBMins.z) *
                                 GetFillInterpolationAmount());
  ...
  CMatRenderContextPtr pRenderContext(materials);
  pRenderContext->Bind((IMaterial *)pMaterial,
                       (CPortalRenderable_Func_LiquidPortal *)this);
  ...
  // x min
  LIQUIDPORTAL_DYNAMICMESH_BOX_ADDVERTEX(2, y, fHorizontalTextureScale, z,
                                         -fVerticalTextureScale);
  LIQUIDPORTAL_DYNAMICMESH_BOX_ADDVERTEX(6, y, fHorizontalTextureScale, z,
                                         -fVerticalTextureScale);
  LIQUIDPORTAL_DYNAMICMESH_BOX_ADDVERTEX(4, y, fHorizontalTextureScale, z,
                                         -fVerticalTextureScale);
  LIQUIDPORTAL_DYNAMICMESH_BOX_ADDVERTEX(0, y, fHorizontalTextureScale, z,
                                         -fVerticalTextureScale);

  // x max
  LIQUIDPORTAL_DYNAMICMESH_BOX_ADDVERTEX(1, y, fHorizontalTextureScale, z,
                                         -fVerticalTextureScale);
  LIQUIDPORTAL_DYNAMICMESH_BOX_ADDVERTEX(5, y, fHorizontalTextureScale, z,
                                         -fVerticalTextureScale);
  LIQUIDPORTAL_DYNAMICMESH_BOX_ADDVERTEX(7, y, fHorizontalTextureScale, z,
                                         -fVerticalTextureScale);
  LIQUIDPORTAL_DYNAMICMESH_BOX_ADDVERTEX(3, y, fHorizontalTextureScale, z,
                                         -fVerticalTextureScale);

  // y min
  LIQUIDPORTAL_DYNAMICMESH_BOX_ADDVERTEX(0, x, fHorizontalTextureScale, z,
                                         -fVerticalTextureScale);
  LIQUIDPORTAL_DYNAMICMESH_BOX_ADDVERTEX(4, x, fHorizontalTextureScale, z,
                                         -fVerticalTextureScale);
  LIQUIDPORTAL_DYNAMICMESH_BOX_ADDVERTEX(5, x, fHorizontalTextureScale, z,
                                         -fVerticalTextureScale);
  LIQUIDPORTAL_DYNAMICMESH_BOX_ADDVERTEX(1, x, fHorizontalTextureScale, z,
                                         -fVerticalTextureScale);

  // y max
  LIQUIDPORTAL_DYNAMICMESH_BOX_ADDVERTEX(3, x, fHorizontalTextureScale, z,
                                         -fVerticalTextureScale);
  LIQUIDPORTAL_DYNAMICMESH_BOX_ADDVERTEX(7, x, fHorizontalTextureScale, z,
                                         -fVerticalTextureScale);
  LIQUIDPORTAL_DYNAMICMESH_BOX_ADDVERTEX(6, x, fHorizontalTextureScale, z,
                                         -fVerticalTextureScale);
  LIQUIDPORTAL_DYNAMICMESH_BOX_ADDVERTEX(2, x, fHorizontalTextureScale, z,
                                         -fVerticalTextureScale);

  // z min
  LIQUIDPORTAL_DYNAMICMESH_BOX_ADDVERTEX(1, x, fHorizontalTextureScale, y,
                                         fVerticalTextureScale);
  LIQUIDPORTAL_DYNAMICMESH_BOX_ADDVERTEX(3, x, fHorizontalTextureScale, y,
                                         fVerticalTextureScale);
  LIQUIDPORTAL_DYNAMICMESH_BOX_ADDVERTEX(2, x, fHorizontalTextureScale, y,
                                         fVerticalTextureScale);
  LIQUIDPORTAL_DYNAMICMESH_BOX_ADDVERTEX(0, x, fHorizontalTextureScale, y,
                                         fVerticalTextureScale);

  // z max
  LIQUIDPORTAL_DYNAMICMESH_BOX_ADDVERTEX(4, x, fHorizontalTextureScale, y,
                                         fVerticalTextureScale);
  LIQUIDPORTAL_DYNAMICMESH_BOX_ADDVERTEX(6, x, fHorizontalTextureScale, y,
                                         fVerticalTextureScale);
  LIQUIDPORTAL_DYNAMICMESH_BOX_ADDVERTEX(7, x, fHorizontalTextureScale, y,
                                         fVerticalTextureScale);
  LIQUIDPORTAL_DYNAMICMESH_BOX_ADDVERTEX(5, x, fHorizontalTextureScale, y,
                                         fVerticalTextureScale);
...
void CPortalRenderable_Func_LiquidPortal::DrawInwardBox(
    const IMaterial *pMaterial) {
  if (pMaterial == NULL)
    pMaterial = materials->FindMaterial("glass/glasswindow_refract01",
                                        TEXTURE_GROUP_OTHER);
...
void CPortalRenderable_Func_LiquidPortal::DrawInnerLiquid(
    ...
    const IMaterial
        *pMaterial) // quads in front of camera clipped to box dimensions
{
  ...
  // vOrigin = CurrentViewOrigin() + vForward * (view->GetViewSetup()->zNear +
  // 0.011f); //experimentation has shown this to be the optimal distance on the
  // Nvidia 6800 cards we develop on
  vOrigin =
      CurrentViewOrigin() + vForward * (view->GetViewSetup()->zNear + 1.0f);
  ...
  PortalMeshPoint_t *pInVerts = (PortalMeshPoint_t *)stackalloc(
      4 * (6) * 2 * sizeof(PortalMeshPoint_t)); // really only should need 2x
                                                // points, but I'm paranoid
  PortalMeshPoint_t *pOutVerts =
      (PortalMeshPoint_t *)stackalloc(4 * (6) * 2 * sizeof(PortalMeshPoint_t));
  ...
  if (bClipToBounds) {
    ...
    iVertCount = ClipPolyToPlane_LerpTexCoords(
        ...
        -(m_vAABBMins.z +
          ((m_vAABBMaxs.z - m_vAABBMins.z) * GetFillInterpolationAmount())),
        0.01f);
    ...
    for (int i = 0; i != 5; ++i) {
      ...
      iVertCount = ClipPolyToPlane_LerpTexCoords(
          pInVerts, iVertCount, pOutVerts, *(Vector *)m_fBoundingPlanes[i],
  ...
  if (bInterpOpacity) {
    IMaterial *pEditMaterial = (IMaterial *)
        pMaterial; // we'll be making changes, then changing it back

    IMaterialVar *pRefractAmount =
        pEditMaterial->FindVar("$refractamount", NULL);
    ...
    Clip_And_Render_Convex_Polygon(pFinalVerts, iVertCount, pEditMaterial,
                                   this);
    materials->Flush();
...
ADD_SCREENSPACE_EFFECT(CLiquidPortal_InnerLiquidEffect,
                       LiquidPortal_InnerLiquid);
...
CLiquidPortal_InnerLiquidEffect::CLiquidPortal_InnerLiquidEffect(void)
    : m_bEnable(true), m_pImmersionPortal(NULL), m_bFadeBackToReality(false),
...
void CLiquidPortal_InnerLiquidEffect::SetParameters(KeyValues *params) {
  ...
  if( pImmersionPortal )
          m_pImmersionPortal = (C_Func_LiquidPortal
  *)pImmersionPortal->GetPtr();*/
}
...
void CLiquidPortal_InnerLiquidEffect::Render(int x, int y, int w, int h) {
  ...
  if (m_bFadeBackToReality) {
    // effect should cover whole screen and have a alpha-like fade back to
    // normal view
