...
void CHudPortalCredits::ReadNames(KeyValues *pKeyValue) {
  ...
  while (pKVNames) {
    ...
    V_strcpy_safe(Credits.szFontName,
...
void CHudPortalCredits::ReadAscii(KeyValues *pKeyValue) {
  ...
  while (pKVNames) {
    ...
    V_strcpy_safe(Credits.szCreditName,
...
void CHudPortalCredits::ReadParams(KeyValues *pKeyValue) {
  ...
  V_strcpy_safe(m_szAsciiArtFont,
                pKeyValue->GetString("asciiartfont", "Default"));
    ...
    if (!bBorders) {
      ...
      while (iBorderY < (iTall - (iFontTall * 3))) {
        ...
        if (iBorderY < (iTall / 2)) {
          ...
          surface()->DrawSetTextPos((iWidth - iBorderPipeWidth) +
                                        m_iScreenXOffset,
                                    iBorderY + m_iScreenYOffset);
          surface()->DrawUnicodeString(bVunicode);
        } else {
  ...
  for (int i = 0; i < m_CreditsList.Count(); i++) {
    ...
    surface()->DrawSetTextColor(m_cColor[0], m_cColor[1], m_cColor[2],
                                FadeBlend(m_flFadeInTime, m_flFadeOutTime,
                                          m_flFadeHoldTime + pCredit->flTimeAdd,
                                          localTime) *
                                    m_cColor[3]);
