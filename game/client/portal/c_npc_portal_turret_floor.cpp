...
IMPLEMENT_CLIENTCLASS_DT(C_NPC_Portal_FloorTurret, DT_NPC_Portal_FloorTurret,
                         CNPC_Portal_FloorTurret)

RecvPropBool(RECVINFO(m_bOutOfAmmo)), RecvPropBool(RECVINFO(m_bLaserOn)),
...
void C_NPC_Portal_FloorTurret::LaserOn(void) {
  ...
  if (!m_pBeam) {
    ...
    m_pBeam->SetWidth(IsX360() ? (1.5f)
                               : (0.75f)); // On low end TVs these lasers are
                                           // very hard to see at a distance
  ...
  float fEndFraction;
  Ray_t rayPath;
  rayPath.Init(vecMuzzle,
               vecMuzzle + vecMuzzleDir * FLOOR_TURRET_PORTAL_LASER_RANGE);

  CTraceFilterSkipClassname traceFilter(this, "prop_energy_ball",
                                        COLLISION_GROUP_NONE);

  if (UTIL_Portal_TraceRay_Beam(rayPath, MASK_SHOT, &traceFilter,
                                &fEndFraction))
    vEndPoint =
        vecMuzzle +
        vecMuzzleDir *
            FLOOR_TURRET_PORTAL_LASER_RANGE; // Trace went through portal and
                                             // endpoint is unknown
  else
    vEndPoint = vecMuzzle + vecMuzzleDir * FLOOR_TURRET_PORTAL_LASER_RANGE *
                                fEndFraction; // Trace hit a wall
...
float C_NPC_Portal_FloorTurret::LaserEndPointSize(void) {
  return ((MAX(0.0f, sinf(gpGlobals->curtime * M_PI + m_fPulseOffset))) *
              FLOOR_TURRET_PORTAL_END_POINT_PULSE_SCALE +
          3.0f) *
         (IsX360() ? (3.0f) : (1.5f));
}