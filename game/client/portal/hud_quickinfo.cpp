...
void CHUDQuickInfo::Init(void) { m_flLastEventTime = 0.0f; }
...
void CHUDQuickInfo::Paint() {
  C_Portal_Player *pPortalPlayer =
      (C_Portal_Player *)(C_BasePlayer::GetLocalPlayer());
  ...
  if (!hud_quickinfo.GetInt() || !pPortalgun ||
      (!pPortalgun->CanFirePortal1() && !pPortalgun->CanFirePortal2())) {
    // no quickinfo or we can't fire either portal, just draw the small versions
    // of the crosshairs
    clrNormal[3] = 196;
    m_icon_lbnone->DrawSelf(xCenter - (m_icon_lbnone->Width() * 2), yCenter,
                            clrNormal);
    m_icon_rbnone->DrawSelf(xCenter + m_icon_rbnone->Width(), yCenter,
                            clrNormal);
    return;
  }
  ...
  Color lastPlaced1Color = Color(portal1Color[0], portal1Color[1],
                                 portal1Color[2], iBaseLastPlacedAlpha);
  Color lastPlaced2Color = Color(portal2Color[0], portal2Color[1],
                                 portal2Color[2], iBaseLastPlacedAlpha);
  ...
  if (pPortalgun->CanFirePortal1() && pPortalgun->CanFirePortal2()) {
    ...
    if (m_bLastPlacedAlphaCountingUp[iDrawLastPlaced]) {
      m_fLastPlacedAlpha[iDrawLastPlaced] +=
          gpGlobals->absoluteframetime * fLastPlacedAlphaLerpSpeed * 2.0f;
      if (m_fLastPlacedAlpha[iDrawLastPlaced] > 255.0f) {
        m_bLastPlacedAlphaCountingUp[iDrawLastPlaced] = false;
        m_fLastPlacedAlpha[iDrawLastPlaced] =
            255.0f - (m_fLastPlacedAlpha[iDrawLastPlaced] - 255.0f);
      }
    } else {
      m_fLastPlacedAlpha[iDrawLastPlaced] -=
          gpGlobals->absoluteframetime * fLastPlacedAlphaLerpSpeed;
    ...
    m_fLastPlacedAlpha[1 - iDrawLastPlaced] -=
        gpGlobals->absoluteframetime * fLastPlacedAlphaLerpSpeed;
  ...
  // can't fire both portals, and we want the crosshair to remain somewhat
  // symmetrical without being confusing
  ...
  if (!hud_quickinfo_swap.GetBool()) {
    if (bPortalPlacability[0])
      m_icon_lb->DrawSelf(xCenter - (m_icon_lb->Width() * 0.64f),
                          yCenter - (m_icon_rb->Height() * 0.17f),
                          portal1Color);
    else
    ...
    m_icon_lbe->DrawSelf(xCenter - (m_icon_lbe->Width() * 1.85f), yCenter,
                         lastPlaced1Color);
    m_icon_rbe->DrawSelf(xCenter + (m_icon_rbe->Width() * 0.75f), yCenter,
                         lastPlaced2Color);
  } else {
    if (bPortalPlacability[1])
      m_icon_lb->DrawSelf(xCenter - (m_icon_lb->Width() * 0.64f),
