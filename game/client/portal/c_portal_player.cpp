...
ConVar cl_reorient_in_air("cl_reorient_in_air", "1", FCVAR_ARCHIVE,
                          "Allows the player to only reorient from being "
                          "upside down while in the air.");
...
IMPLEMENT_CLIENTCLASS_DT_NOBASE(C_PortalRagdoll, DT_PortalRagdoll,
                                CPortalRagdoll)
...
// Input  :  -
//-----------------------------------------------------------------------------
C_PortalRagdoll::~C_PortalRagdoll() { (this); }
...
//-----------------------------------------------------------------------------
void C_PortalRagdoll::SetupWeights(const matrix3x4_t *pBoneToWorld,
                                   int nFlexWeightCount, float *pFlexWeights,
                                   float *pFlexDelayedWeights) {
  // While we're dying, we want to mimic the facial animation of the player.
  // Once they're dead, we just stay as we are.
  if ((m_hPlayer && m_hPlayer->IsAlive()) || !m_hPlayer) {
    BaseClass::SetupWeights(pBoneToWorld, nFlexWeightCount, pFlexWeights,
                            pFlexDelayedWeights);
  } else if (m_hPlayer) {
    m_hPlayer->SetupWeights(pBoneToWorld, nFlexWeightCount, pFlexWeights,
                            pFlexDelayedWeights);
  }
}
...
// Output : IRagdoll*
//-----------------------------------------------------------------------------
IRagdoll *C_PortalRagdoll::GetIRagdoll() const { return m_pRagdoll; }
...
RecvPropFloat(RECVINFO(m_angEyeAngles[0])),
    ...
    END_RECV_TABLE()
        ...
        BEGIN_PREDICTION_DATA(C_Portal_Player) END_PREDICTION_DATA()
...
int C_Portal_Player::GetIDTarget() const { return m_iIDEntIndex; }
...
void C_Portal_Player::TraceAttack(const CTakeDamageInfo &info,
...
void C_Portal_Player::UpdateLookAt(void) {
  ...
  // Player looks at themselves through portals. Pick the portal we're turned
  // towards.
  ...
  if (UTIL_IntersectEntityExtentsWithPortal(this) != NULL) {
    ...
    // Test through any active portals: This may be a shorter distance to the
    // target
    for (int i = 0; i != iPortalCount; ++i) {
      CProp_Portal *pTempPortal = pPortals[i];

      if (pTempPortal && pTempPortal->m_bActivated &&
          pTempPortal->m_hLinkedPortal.Get()) {
  ...
  m_flCurrentHeadYaw =
      m_flCurrentHeadYaw + flSpeed * (desiredYaw - m_flCurrentHeadYaw);
  m_flCurrentHeadYaw = AngleNormalize(m_flCurrentHeadYaw);
  SetPoseParameter(m_headYawPoseParam, m_flCurrentHeadYaw);

  m_flCurrentHeadPitch =
      m_flCurrentHeadPitch + flSpeed * (desiredPitch - m_flCurrentHeadPitch);
...
void C_Portal_Player::ClientThink(void) {
  ...
  //		Vector vLook = pEntity1->GetAbsOrigin() -
  //pEntity2->GetAbsOrigin(); 		VectorNormalize( vLook );
  ...
  if (m_CCDeathHandle != INVALID_CLIENT_CCHANDLE) {
    ...
    g_pColorCorrectionMgr->SetColorCorrectionWeight(m_CCDeathHandle,
                                                    m_flDeathCCWeight);
  }
...
void C_Portal_Player::FixTeleportationRoll(void) {
  if (IsInAVehicle()) // HL2 compatibility fix. do absolutely nothing to the
                      // view in vehicles
    return;
  ...
  bool bForcePitchReorient =
      (vAbsAngles[ROLL] > 175.0f && vCurrentForward.z > 0.99f);
  ...
  if (vCurrentUp.z < 0.75f) {
    m_fReorientationRate +=
        gpGlobals->frametime * REORIENTATION_ACCELERATION_RATE;

    // Upright faster if on the ground
    float fMaxReorientationRate =
        REORIENTATION_RATE * ((bOnGround) ? (2.0f) : (1.0f));
    ...
    if (!C_BasePlayer::ShouldDrawThisPlayer()) {
      ...
      if ((g_pPortalRender->GetViewRecursionLevel() == 1) &&
          (m_iForceNoDrawInPortalSurface !=
           -1)) // CPortalRender::s_iRenderingPortalView )
        return 0;
    }
  }
...
ShadowType_t C_Portal_Player::ShadowCastType(void) {
  // Drawing player shadows looks bad in first person when they get close to
  // walls It doesn't make sense to have shadows in the portal view, but not in
  // the main view So no shadows for the player
  return SHADOWS_NONE;
}
...
IRagdoll *C_Portal_Player::GetRepresentativeRagdoll() const {
  if (m_hRagdoll.Get()) {
    C_PortalRagdoll *pRagdoll =
        static_cast<C_PortalRagdoll *>(m_hRagdoll.Get());
    if (!pRagdoll)
      return NULL;
...
void C_Portal_Player::OnDataChanged(DataUpdateType_t type) {
  ...
  if (m_hSurroundingLiquidPortal !=
      PreDataChanged_Backup.m_hSurroundingLiquidPortal) {
    CLiquidPortal_InnerLiquidEffect *pLiquidEffect =
        (CLiquidPortal_InnerLiquidEffect *)g_pScreenSpaceEffects
            ->GetScreenSpaceEffect("LiquidPortal_InnerLiquid");
    if (pLiquidEffect) {
      C_Func_LiquidPortal *pSurroundingPortal =
          m_hSurroundingLiquidPortal.Get();
      if (pSurroundingPortal != NULL) {
        C_Func_LiquidPortal *pOldSurroundingPortal =
            PreDataChanged_Backup.m_hSurroundingLiquidPortal.Get();
        if (pOldSurroundingPortal !=
            pSurroundingPortal->m_hLinkedPortal.Get()) {
          ...
          pLiquidEffect->m_fFadeBackTimeLeft =
              pLiquidEffect->s_fFadeBackEffectTime;
        }
      } else {
  ...
  if (type == DATA_UPDATE_CREATED) {
    ...
    m_CCDeathHandle =
        g_pColorCorrectionMgr->AddColorCorrection(DEATH_CC_LOOKUP_FILENAME);
    if (m_CCDeathHandle != INVALID_CLIENT_CCHANDLE) {
...
// CalcView() gets called between OnPreDataChanged() and OnDataChanged(), and
// these changes need to be known about in both before CalcView() gets called,
// and if CalcView() doesn't get called
bool C_Portal_Player::DetectAndHandlePortalTeleportation(void) {
  if (m_bPortalledMessagePending) {
    ...
    // C_Prop_Portal *pOldPortal =
    // PreDataChanged_Backup.m_hPortalEnvironment.Get(); Assert( pOldPortal );
    // if( pOldPortal )
    {
      Vector ptNewPosition = GetNetworkOrigin();

      UTIL_Portal_PointTransform(
          m_PendingPortalMatrix,
      ...
      UTIL_Portal_AngleTransform(m_PendingPortalMatrix,
                                 m_qEyeAngles_LastCalcView, m_angEyeAngles);
      ...
      if (IsLocalPlayer()) {
        // DevMsg( "FPT: %.2f %.2f %.2f\n", m_angEyeAngles.x, m_angEyeAngles.y,
        // m_angEyeAngles.z );
      ...
      for (int i = 0; i < MAX_VIEWMODELS; i++) {
        ...
        UTIL_Portal_VectorTransform(m_PendingPortalMatrix, vm->m_vecLastFacing,
                                    vm->m_vecLastFacing);
      }
    }
    m_bPortalledMessagePending = false;
...
void C_Portal_Player::UpdatePortalEyeInterpolation(void) {
  ...
  if (PortalEyeInterpolation.m_bUpdatePosition_FreeMove) {
    ...
    C_Prop_Portal *pOldPortal =
        PreDataChanged_Backup.m_hPortalEnvironment.Get();
    if (pOldPortal) {
      UTIL_Portal_PointTransform(
          pOldPortal->MatrixThisToLinked(),
          PortalEyeInterpolation.m_vEyePosition_Interpolated,
          PortalEyeInterpolation.m_vEyePosition_Interpolated);
      // PortalEyeInterpolation.m_vEyePosition_Interpolated =
      // pOldPortal->m_matrixThisToLinked *
      // PortalEyeInterpolation.m_vEyePosition_Interpolated;
  ...
  // find offset between this and last frame's uninterpolated movement, and
  // apply this as freebie movement to the interpolated position
  PortalEyeInterpolation.m_vEyePosition_Interpolated +=
      (vThisFrameUninterpolatedPosition -
       PortalEyeInterpolation.m_vEyePosition_Uninterpolated);
  PortalEyeInterpolation.m_vEyePosition_Uninterpolated =
      vThisFrameUninterpolatedPosition;

  Vector vDiff = vThisFrameUninterpolatedPosition -
                 PortalEyeInterpolation.m_vEyePosition_Interpolated;
  float fLength = vDiff.Length();
  float fFollowSpeed = gpGlobals->frametime * 100.0f;
  const float fMaxDiff = 150.0f;
  if (fLength > fMaxDiff) {
    // camera lagging too far behind, give it a speed boost to bring it within
    // maximum range
...
Vector C_Portal_Player::EyeFootPosition(const QAngle &qEyeAngles) {
  ...
  // interpolate between feet and normal eye position based on view roll (gets
  // us wall/ceiling & ceiling/ceiling teleportations without an eye position
  // pop)
  float fFootInterp =
      fabs(qEyeAngles[ROLL]) *
      ((1.0f / 180.0f) *
       0.75f); // 0 when facing straight up, 0.75 when facing straight down
  return (
      BaseClass::EyePosition() -
      (fFootInterp *
       m_vecViewOffset)); // TODO: Find a good Up vector for this rolled player
                          // and interpolate along actual eye/foot axis
}

void C_Portal_Player::CalcView(Vector &eyeOrigin, QAngle &eyeAngles,
  ...
  m_bEyePositionIsTransformedByPortal =
      false; // assume it's not transformed until it provably is
  ...
  if (m_lifeState != LIFE_ALIVE) {
    ...
    UTIL_TraceHull(origin, eyeOrigin, WALL_MIN, WALL_MAX, MASK_SOLID_BRUSHONLY,
...
void C_Portal_Player::CalcPortalView(Vector &eyeOrigin, QAngle &eyeAngles) {
  // although we already ran CalcPlayerView which already did these copies, they
  // also fudge these numbers in ways we don't like, so recopy
  ...
  bool bOverrideSpecialEffects =
      false; // sometimes to get the best effect we need to kill other effects
             // that are simply for cleanliness

  float fEyeDist = vPortalForward.Dot(eyeOrigin) - fPortalPlaneDist;
  bool bTransformEye = false;
  if (fEyeDist < 0.0f) // eye behind portal
  {
    if (pPortal->m_PortalSimulator.EntityIsInPortalHole(
            this)) // player standing in portal
    {
      bTransformEye = true;
    } else if (vPortalForward.z <
               -0.01f) // there's a weird case where the player is ducking below
                       // a ceiling portal. As they unduck their eye moves
                       // beyond the portal before the code detects that they're
                       // in the portal hole.
    {
      ...
      if (fOriginDist > 0.0f) {
        float fInvTotalDist =
            1.0f / (fOriginDist - fEyeDist); // fEyeDist is negative
        Vector ptPlaneIntersection =
            (eyeOrigin * fOriginDist * fInvTotalDist) -
            (ptPlayerOrigin * fEyeDist * fInvTotalDist);
        Assert(fabs(vPortalForward.Dot(ptPlaneIntersection) -
                    fPortalPlaneDist) < 0.01f);
  ...
  if (bTransformEye) {
    ...
    // eyeOrigin.x, eyeOrigin.y, eyeOrigin.z, eyeAngles.x, eyeAngles.y,
    // eyeAngles.z );
...
void C_Portal_Player::CalcViewModelView(const Vector &eyeOrigin,
                                        const QAngle &eyeAngles) {
  // HACK: Manually adjusting the eye position that view model looking up and
  // down are similar (solves view model "pop" on floor to floor transitions)
  ...
  if (vForward.z < 0.0f) {
    ...
    vInterpEyeOrigin +=
        vRight * (fT * 4.7f) + vForward * (fT * 5.0f) + vUp * (fT * 4.0f);
  }
