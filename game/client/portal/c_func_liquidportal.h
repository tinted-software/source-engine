...
public:
  ...
  bool CalcFrustumThroughPortal(const Vector &ptCurrentViewOrigin,
                                ...
                                const VPlane *pInputFrustum = NULL,
                                int iInputFrustumPlaneCount = 0);
  ...
  virtual bool ShouldUpdatePortalView_BasedOnView(
      ...
      Frustum currentFrustum); // portal is both visible, and will display at
                               // least some portion of a remote view
  virtual CPortalRenderable *GetLinkedPortal() const;
  virtual bool ShouldUpdateDepthDoublerTexture(const CViewSetup &viewSetup);
  virtual void DrawPortal(void); // sort of like what you'd expect to happen in
                                 // C_BaseAnimating::DrawModel() if portals were
  ...
  void DrawInnerLiquid(
      bool bClipToBounds = true, float fOpacity = 1.0f,
      const IMaterial *pMaterial =
          NULL); // quads in front of camera clipped to box dimensions
  ...
  float m_fBoundingPlanes[5][4]; // top is highly variable and therefore not
                                 // actually stored
...
inline float
CPortalRenderable_Func_LiquidPortal::GetFillInterpolationAmount(void) {
  if (m_fFillEndTime < gpGlobals->curtime)
    return 0.0f;

  return ((gpGlobals->curtime - m_fFillStartTime) /
          (m_fFillEndTime - m_fFillStartTime));
}

inline bool CPortalRenderable_Func_LiquidPortal::IsFillingNow(void) {
  return ((m_fFillEndTime >= gpGlobals->curtime) &&
          (m_fFillStartTime <= gpGlobals->curtime));
}

class C_Func_LiquidPortal : public C_BaseEntity,
                            public CPortalRenderable_Func_LiquidPortal {
public:
...
public:
  C_Func_LiquidPortal
      *m_pImmersionPortal; // portal that did the filling + teleporting
