...
IMPLEMENT_CLIENTCLASS_DT(C_PropPortalStatsDisplay, DT_PropPortalStatsDisplay,
...
void C_PropPortalStatsDisplay::OnDataChanged(DataUpdateType_t updateType) {
  ...
  if (updateType == DATA_UPDATE_DATATABLE_CHANGED) {
    ...
    if (m_bEnabled && !m_bPrevEnabled) {
      ...
      // Don't update if the player has cheated (and isn't at least developer
      // level 2)
      if (!m_bHasCheated || developer.GetInt() >= 2) {
        ...
        BonusMapChallengeUpdate(szChallengeFileName, szChallengeMapName,
                                szChallengeName, iStat);
      }
    }
  }
}
...
void C_PropPortalStatsDisplay::ClientThink(void) {
  ...
  // this is only to prevent players from accidentally wiping their stats while
  // no clipping through a map
  if (!sv_cheats) {
  ...
  case 1:
    ...
    case PORTAL_LEVEL_STAT_NUM_PORTALS:
      if (m_fEnabledCounter >
          1.5f / static_cast<float>(m_iNumPortalsPlaced + 1)) {
        m_fEnabledCounter = 0.0f;
    ...
    case PORTAL_LEVEL_STAT_NUM_STEPS:
      if (m_fEnabledCounter > 1.5f / static_cast<float>(m_iNumStepsTaken + 1)) {
        int iNumStepsThisFrame =
            m_fEnabledCounter * static_cast<float>(m_iNumStepsTaken + 1);
        m_fEnabledCounter = 0.0f;

        if (m_fNumPlayerDisplay < m_iNumStepsTaken) {
          m_fNumPlayerDisplay +=
              MIN(iNumStepsThisFrame, m_iNumStepsTaken - m_fNumPlayerDisplay);
  ...
  case 2:
    if (m_fEnabledCounter > 1.5f) {
      ...
      int(*(pObjectives[3])) = {&m_iBronzeObjective, &m_iSilverObjective,
                                &m_iGoldObjective};
