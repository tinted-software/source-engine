...
bool CHudPortalCrosshair::ShouldDraw() {
  // NOTE: Portal crosshair should no longer be in use, but I'm leaving the code
  // here until X360 lock down... we don't want to draw this ever. -Jeep
  return false;
  ...
  CWeaponPortalBase *pWeapon =
      dynamic_cast<CWeaponPortalBase *>(pPlayer->GetActiveWeapon());

  if (!pWeapon)
    return false;

  bool bNeedsDraw =
      m_pCrosshair && crosshair.GetInt() && !engine->IsDrawingLoadingImage() &&
      !engine->IsPaused() && g_pClientMode->ShouldDrawCrosshair() &&
...
void CHudPortalCrosshair::Paint(void) {
  ...
  m_pCrosshair->DrawSelf(x - 0.5f * m_pCrosshair->Width(),
                         y - 0.5f * m_pCrosshair->Height(), m_clrCrosshair);
}
