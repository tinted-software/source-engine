...
C_PortalGhostRenderable::C_PortalGhostRenderable(
    C_Prop_Portal *pOwningPortal, C_BaseEntity *pGhostSource,
    ...
    : m_pGhostedRenderable(pGhostSource),
      ...
      m_bLocalPlayer(bLocalPlayer), m_pOwningPortal(pOwningPortal) {
  m_bSourceIsBaseAnimating =
      (dynamic_cast<C_BaseAnimating *>(pGhostSource) != NULL);
...
void C_PortalGhostRenderable::PerFrameUpdate(void) {
  ...
  Vector ptNewOrigin =
      m_matGhostTransform * m_pGhostedRenderable->GetAbsOrigin();
  QAngle qNewAngles = TransformAnglesToWorldSpace(
      m_pGhostedRenderable->GetAbsAngles(), m_matGhostTransform.As3x4());
...
Vector const &C_PortalGhostRenderable::GetRenderOrigin(void) {
  ...
  m_ReferencedReturns.vRenderOrigin =
      m_matGhostTransform * m_pGhostedRenderable->GetRenderOrigin();
  return m_ReferencedReturns.vRenderOrigin;
}
...
QAngle const &C_PortalGhostRenderable::GetRenderAngles(void) {
  ...
  m_ReferencedReturns.qRenderAngle = TransformAnglesToWorldSpace(
      m_pGhostedRenderable->GetRenderAngles(), m_matGhostTransform.As3x4());
  return m_ReferencedReturns.qRenderAngle;
}

bool C_PortalGhostRenderable::SetupBones(matrix3x4_t *pBoneToWorldOut,
                                         int nMaxBones, int boneMask,
                                         float currentTime) {
  if (m_pGhostedRenderable == NULL)
    return false;

  int nModelIndex = 0;
  CBaseCombatWeapon *pParent =
      dynamic_cast<CBaseCombatWeapon *>(m_pGhostedRenderable);
  if (pParent) {
    nModelIndex = pParent->GetModelIndex();
    pParent->SetModelIndex(pParent->GetWorldModelIndex());
  }

  if (m_pGhostedRenderable->SetupBones(pBoneToWorldOut, nMaxBones, boneMask,
                                       currentTime)) {
    if (pBoneToWorldOut) {
      for (int i = 0; i != nMaxBones;
           ++i) // FIXME: nMaxBones is most definitely greater than the actual
                // number of bone transforms actually used, find the subset
                // somehow
      {
...
void C_PortalGhostRenderable::GetRenderBoundsWorldspace(Vector &mins,
                                                        Vector &maxs) {
  if (m_pGhostedRenderable == NULL) {
    mins = maxs = vec3_origin;
    return;
...
void C_PortalGhostRenderable::GetShadowRenderBounds(Vector &mins, Vector &maxs,
                                                    ShadowType_t shadowType) {
  m_pGhostedRenderable->GetShadowRenderBounds(mins, maxs, shadowType);
  TransformAABB(m_matGhostTransform.As3x4(), mins, maxs, mins, maxs);
}

/*bool C_PortalGhostRenderable::GetShadowCastDistance( float *pDist,
...
bool C_PortalGhostRenderable::GetShadowCastDirection( Vector *pDirection,
ShadowType_t shadowType ) const
{
        if( m_pGhostedRenderable == NULL )
                return false;
        ...
        {
                if( pDirection )
                        *pDirection = m_matGhostTransform.ApplyRotation(
*pDirection );
...
const matrix3x4_t &C_PortalGhostRenderable::RenderableToWorldTransform() {
  ...
  ConcatTransforms(m_matGhostTransform.As3x4(),
                   m_pGhostedRenderable->RenderableToWorldTransform(),
...
bool C_PortalGhostRenderable::GetAttachmentVelocity(int number,
                                                    Vector &originVel,
                                                    Quaternion &angleVel) {
  if (m_pGhostedRenderable == NULL)
    return false;
  ...
  if (m_pGhostedRenderable->GetAttachmentVelocity(number, ghostVel, angleVel)) {
    ...
    Vector3DMultiply(m_matGhostTransform, *(Vector *)(&angleVel),
                     *(Vector *)(&angleVel));
    return true;
  }
  return false;
