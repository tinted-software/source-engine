...
DECLARE_VGUI_SCREEN_FACTORY(CNeurotoxinCountdownScreen,
                            "neurotoxin_countdown_screen");

//-----------------------------------------------------------------------------
// Constructor:
//-----------------------------------------------------------------------------
CNeurotoxinCountdownScreen::CNeurotoxinCountdownScreen(vgui::Panel *parent,
                                                       const char *panelName)
    : BaseClass(parent, "CNeurotoxinCountdownScreen",
                vgui::scheme()->LoadSchemeFromFileEx(
                    enginevgui->GetPanel(PANEL_CLIENTDLL),
                    "resource/NeurotoxinCountdownScreen.res",
                    "NeuroToxinScreen")) {
...
void CNeurotoxinCountdownScreen::ApplySchemeSettings(IScheme *pScheme) {
  ...
  m_cDefault =
      pScheme->GetColor("CNeurotoxinCountdownScreen_Default", GetFgColor());
...
// Initialization
//-----------------------------------------------------------------------------
bool CNeurotoxinCountdownScreen::Init(KeyValues *pKeyValues,
  ...
  //	{
  //		m_pNumberImages[ iDigit ][ iNumber ] = SETUP_PANEL( new
  //ImagePanel( this, "SlideshowImage" ) ); 		m_pNumberImages[ iDigit ][ iNumber
  //]->SetImage( "" ); 		m_pNumberImages[ iDigit ][ iNumber ]->SetZPos( -2 );
...
void CNeurotoxinCountdownScreen::OnTick() {
  ...
  for (int iDisplayScreens = 0;
       iDisplayScreens < g_NeurotoxinCountdowns.Count(); ++iDisplayScreens) {
    C_NeurotoxinCountdown *pNeurotoxinCountdownTemp =
        g_NeurotoxinCountdowns[iDisplayScreens];
...
void CNeurotoxinCountdownScreen::Update(
    C_NeurotoxinCountdown *pNeurotoxinCountdown) {
  char szBuff[256];
  ...
  if (iMinutes <= 0 && iSeconds <= 0) {
    ...
    // Blink!
    m_pDisplayTextLabel->SetVisible(
        static_cast<int>(gpGlobals->curtime * 10.0f) % 2 == 0);
  } else {
