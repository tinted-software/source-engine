...
bool CPortalPickAlphaMaskProxy::Init(IMaterial *pMaterial,
  ...
  m_pOpeningTexture = materials->FindTexture(pszOpeningTextureInput,
                                             TEXTURE_GROUP_CLIENT_EFFECTS);
  m_pOpeningTexture->IncrementReferenceCount();
  m_pIdleTexture =
      materials->FindTexture(pszIdleTextureInput, TEXTURE_GROUP_CLIENT_EFFECTS);
...
void CPortalPickAlphaMaskProxy::OnBind(void *pBind) {
  ...
  if (pFlatBasic->m_fOpenAmount == 1.0f) {
    ...
    m_AlphaMaskTextureFrame->SetIntValue(
        pFlatBasic->m_fOpenAmount * m_pOpeningTexture->GetNumAnimationFrames());
  }
...
EXPOSE_INTERFACE(CPortalPickAlphaMaskProxy, IMaterialProxy,
                 "PortalPickAlphaMask" IMATERIAL_PROXY_INTERFACE_VERSION);