...
public:
  ...
  bool DetectAndHandlePortalTeleportation(
      void); // detects if the player has portalled and fixes views
  ...
  Vector
  EyeFootPosition(const QAngle &qEyeAngles); // interpolates between eyes and
                                             // feet based on view angle roll
  ...
  inline bool IsCloseToPortal(
      void) // it's usually a good idea to turn on draw hacks when this is true
  {
    return ((PortalEyeInterpolation.m_bEyePositionIsInterpolating) ||
            (m_hPortalEnvironment.Get() != NULL));
  }
  ...
  void SetHeldObjectOnOppositeSideOfPortal(
      bool p_bHeldObjectOnOppositeSideOfPortal) {
...
private:
  ...
  int m_iForceNoDrawInPortalSurface; // only valid for one frame, used to temp
                                     // disable drawing of the player model in a
                                     // surface because of freaky artifacts

  struct PortalEyeInterpolation_t {
    bool m_bEyePositionIsInterpolating; // flagged when the eye position would
                                        // have popped between two distinct
                                        // positions and we're smoothing it over
    Vector m_vEyePosition_Interpolated; // we'll be giving the interpolation a
                                        // certain amount of instant movement
                                        // per frame based on how much an
                                        // uninterpolated eye would have moved
    Vector m_vEyePosition_Uninterpolated; // can't have smooth movement without
                                          // tracking where we just were
    // bool	m_bNeedToUpdateEyePosition;
    // int		m_iFrameLastUpdated;

    int m_iTickLastUpdated;
    float m_fTickInterpolationAmountLastUpdated;
    bool m_bDisableFreeMovement; // used for one frame usually when error in
                                 // free movement is likely to be high
    bool m_bUpdatePosition_FreeMove;

    PortalEyeInterpolation_t(void)
        : m_iTickLastUpdated(0), m_fTickInterpolationAmountLastUpdated(0.0f),
          m_bDisableFreeMovement(false), m_bUpdatePosition_FreeMove(false) {};
  ...
  QAngle
      m_qEyeAngles_LastCalcView; // we've got some VERY persistent single frame
                                 // errors while teleporting, this will be
                                 // updated every frame in CalcView() and will
                                 // serve as a central source for fixed angles
  ...
  bool m_bPortalledMessagePending; // Player portalled. It's easier to wait
                                   // until we get a OnDataChanged() event or a
                                   // CalcView() before we do anything about it.
                                   // Otherwise bits and pieces can get undone
  VMatrix m_PendingPortalMatrix;

public:
  bool m_bPitchReorientation;
  float m_fReorientationRate;
  bool m_bEyePositionIsTransformedByPortal; // when the eye and body positions
                                            // are not on the same side of a
                                            // portal

  CHandle<C_Prop_Portal>
      m_hPortalEnvironment; // a portal whose environment the player is
                            // currently in, should be invalid most of the time
  CHandle<C_Func_LiquidPortal>
      m_hSurroundingLiquidPortal; // a liquid portal whose volume the player is
                                  // standing in
};
