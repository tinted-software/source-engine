...
static ConVar
    r_forcecheapwater("r_forcecheapwater", "0", FCVAR_CLIENTDLL | FCVAR_CHEAT,
                      "Force all water to be cheap water, will show old "
                      "renders if enabled after water has been seen");

ConVar r_portal_use_stencils(
    "r_portal_use_stencils", "1", FCVAR_CLIENTDLL,
    "Render portal views using stencils (if available)"); // draw portal views
                                                          // using stencil
                                                          // rendering
ConVar r_portal_stencil_depth("r_portal_stencil_depth", "2",
                              FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
                              "When using stencil views, this changes how many "
                              "views within views we see");
...
//-------------------------------------------
CUtlVector<int> s_iFreedViewIDs; // when a view id node gets freed, it's primary
                                 // view id gets added here

PortalViewIDNode_t *AllocPortalViewIDNode(int iChildLinkCount) {
  PortalViewIDNode_t *pNode =
      new PortalViewIDNode_t; // for now we just new/delete
  ...
  if (iChildLinkCount != 0) {
    ...
    memset(pNode->ChildNodes.Base(), NULL,
           sizeof(PortalViewIDNode_t *) * iChildLinkCount);
  }
...
void RemovePortalViewIDChildLinkIndex(PortalViewIDNode_t *pNode,
                                      int iRemoveIndex) {
  ...
  // I know the current behavior for CUtlVector::FastRemove() is to move the
  // tail into the removed index. But I need that behavior to be true in the
  // future as well so I'm doing it explicitly
...
CPortalRenderable::~CPortalRenderable(void) {
  ...
  s_PortalRender.m_AllPortals[iLast]->m_iPortalViewIDNodeIndex =
      m_iPortalViewIDNodeIndex;

  // I know the current behavior for CUtlVector::FastRemove() is to move the
  // tail into the removed index. But I need that behavior to be true in the
  // future as well so I'm doing it explicitly
  s_PortalRender.m_AllPortals[m_iPortalViewIDNodeIndex] =
      s_PortalRender.m_AllPortals.Tail();
  s_PortalRender.m_AllPortals.Remove(iLast);

  RemovePortalViewIDChildLinkIndex(
      &s_PortalRender.m_HeadPortalViewIDNode,
      m_iPortalViewIDNodeIndex); // does the same transplant operation as above
                                 // to all portal view id nodes
}
...
void CPortalRenderable::BeginPortalPixelVisibilityQuery(void) {
  ...
  if (g_pPortalRender
          ->ShouldUseStencilsToRenderPortals()) // this function exists because
                                                // we require help in texture
                                                // mode, we need no assistance
                                                // in stencil mode. Moreover,
                                                // doing the query twice will
                                                // probably fubar the results
    return;

  PortalViewIDNode_t *pCurrentPortalViewNode =
      g_pPortalRender
          ->m_PortalViewIDNodeChain[g_pPortalRender->m_iViewRecursionLevel]
          ->ChildNodes[m_iPortalViewIDNodeIndex];

  if (pCurrentPortalViewNode) {
    CMatRenderContextPtr pRenderContext(materials);
    pRenderContext->BeginOcclusionQueryDrawing(
        pCurrentPortalViewNode->occlusionQueryHandle);
...
void CPortalRenderable::EndPortalPixelVisibilityQuery(void) {
  ...
  if (pCurrentPortalViewNode) {
    CMatRenderContextPtr pRenderContext(materials);
    pRenderContext->EndOcclusionQueryDrawing(
        pCurrentPortalViewNode->occlusionQueryHandle);
  }
}
...
CPortalRender::CPortalRender() : m_MaterialsAccess(m_Materials) {
  m_iRemainingPortalViewDepth =
      1; // let's portals know that they should do "end of the line" kludges to
...
void CPortalRender::LevelInitPreEntity() {
  ...
  m_Materials.m_Wireframe.Init("shadertest/wireframe",
                               TEXTURE_GROUP_CLIENT_EFFECTS);
  m_Materials.m_WriteZ_Model.Init("engine/writez_model",
                                  TEXTURE_GROUP_CLIENT_EFFECTS);
  m_Materials.m_TranslucentVertexColor.Init("engine/TranslucentVertexColor",
                                            TEXTURE_GROUP_CLIENT_EFFECTS);
}
...
//-----------------------------------------------------------------------------
// only use stencils when it's requested, and expensive water won't cause it to
// freak out
//-----------------------------------------------------------------------------
bool CPortalRender::ShouldUseStencilsToRenderPortals() const {
  // only use stencils when it's requested, and available
  return r_portal_use_stencils.GetBool() &&
         (materials->StencilBufferBits() != 0);
}
...
int CPortalRender::ShouldForceCheaperWaterLevel() const {
  ...
  if (m_iViewRecursionLevel > 0) {
    if (portalrendertargets->GetWaterReflectionTextureForStencilDepth(
            m_iViewRecursionLevel) == NULL)
      return 0;

    PortalViewIDNode_t *pPixelVisNode =
        m_PortalViewIDNodeChain[m_iViewRecursionLevel - 1]
            ->ChildNodes[m_pRenderingViewForPortal->m_iPortalViewIDNodeIndex];

    if (pPixelVisNode->fScreenFilledByPortalSurfaceLastFrame_Normalized >=
        0.0f) {
      if (pPixelVisNode->fScreenFilledByPortalSurfaceLastFrame_Normalized <
          0.005f)
        return 0;
...
void Recursive_UpdatePortalPixelVisibility(PortalViewIDNode_t *pNode,
                                           IMatRenderContext *pRenderContext) {
  if (pNode->iWindowPixelsAtQueryTime > 0) {
    if (pNode->iOcclusionQueryPixelsRendered < -1) {
      // First couple queries. We seem to be getting bogus 0's on the first
      // queries sometimes. ignore the results.
      ++pNode->iOcclusionQueryPixelsRendered;
      pNode->fScreenFilledByPortalSurfaceLastFrame_Normalized = -1.0f;
    } else {
      pNode->iOcclusionQueryPixelsRendered =
          pRenderContext->OcclusionQuery_GetNumPixelsRendered(
              pNode->occlusionQueryHandle);
      pNode->fScreenFilledByPortalSurfaceLastFrame_Normalized =
          ((float)pNode->iOcclusionQueryPixelsRendered) /
          ((float)pNode->iWindowPixelsAtQueryTime);
    }
  } else {
    pNode->fScreenFilledByPortalSurfaceLastFrame_Normalized = -1.0f;
  }
...
//-----------------------------------------------------------------------------
// Purpose: Invalidates pixel visibility data for all portals for this next
// frame.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Preserves pixel visibility data when view id's are getting swapped
// around
//-----------------------------------------------------------------------------
void CPortalRender::EnteredPortal(CPortalRenderable *pEnteredPortal) {
  ...
  PortalViewIDNode_t *pNewHead =
      m_HeadPortalViewIDNode
          .ChildNodes[pEnteredPortal->m_iPortalViewIDNodeIndex];
  m_HeadPortalViewIDNode.ChildNodes[pEnteredPortal->m_iPortalViewIDNodeIndex] =
      NULL;

  // Create a new node that will preserve main's visibility. This new node will
  // be linked to the new head node at the exit portal's index (imagine entering
  // a portal walking backwards)
  PortalViewIDNode_t *pExitPortalsNewNode =
      AllocPortalViewIDNode(iNodeLinkCount);
  {
    ...
    PixelVisibility_ShiftVisibilityViews(VIEW_MAIN,
                                         pExitPortalsNewNode->iPrimaryViewID);
    PixelVisibility_ShiftVisibilityViews(
        VIEW_3DSKY, pExitPortalsNewNode->iPrimaryViewID + 1);
  }

  if (pNewHead) // it's possible we entered a portal we couldn't see through
  {
    Assert(pNewHead->ChildNodes.Count() ==
           m_HeadPortalViewIDNode.ChildNodes.Count());
    Assert(pNewHead->ChildNodes[pExitPortal->m_iPortalViewIDNodeIndex] ==
           NULL); // seeing out of an exit portal back into itself should be
                  // impossible

    for (int i = 0; i != iNodeLinkCount; ++i) {
      m_HeadPortalViewIDNode.ChildNodes[i] = pNewHead->ChildNodes[i];
      pNewHead->ChildNodes[i] =
          NULL; // going to be freeing the node in a minute, don't want to kill
                // transplanted children
    }

    // Since the primary views will always be 0 and 1, we have to shift results
    // instead of replacing the id's
    PixelVisibility_ShiftVisibilityViews(pNewHead->iPrimaryViewID, VIEW_MAIN);
    PixelVisibility_ShiftVisibilityViews(pNewHead->iPrimaryViewID + 1,
                                         VIEW_3DSKY);

    FreePortalViewIDNode(pNewHead);
  }

  Assert(m_HeadPortalViewIDNode
             .ChildNodes[pExitPortal->m_iPortalViewIDNodeIndex] ==
         NULL); // asserted above in pNewHead code, but call me paranoid
  m_HeadPortalViewIDNode.ChildNodes[pExitPortal->m_iPortalViewIDNodeIndex] =
      pExitPortalsNewNode;

  // Because pixel visibility is based off of *last* frame's visibility. We can
  // get cases where a certain portal wasn't visible last frame, but is takes up
  // most of the screen this frame. Set all portal pixel visibility to unknown
  // visibility.
  for (int i = m_HeadPortalViewIDNode.ChildNodes.Count(); --i >= 0;) {
    PortalViewIDNode_t *pChildNode = m_HeadPortalViewIDNode.ChildNodes[i];
    if (pChildNode)
...
bool CPortalRender::DrawPortalsUsingStencils(CViewRender *pViewRender) {
  ...
  if (((iDrawFlags & DF_CLIP_Z) != 0) &&
      ((iDrawFlags & DF_CLIP_BELOW) == 0)) // clipping above the water height
    return false;

  int iNumRenderablePortals = m_ActivePortals.Count();

  // This loop is necessary because tools can suppress rendering without telling
  // the portal system
  ...
  for (int i = 0; i < iNumRenderablePortals; ++i) {
    ...
    C_BaseEntity *pPairedEntity =
        pPortalRenderable->PortalRenderable_GetPairedEntity();
    bool bIsVisible =
        (pPairedEntity == NULL) ||
        (pPairedEntity->IsVisible() &&
         pPairedEntity->ShouldDraw()); // either unknown visibility or
                                       // definitely visible.

    if (!pPortalRenderable->m_bIsPlaybackPortal) {
      if (!bIsVisible) {
        // can't see through the portal, free up it's view id node for use
        // elsewhere
        if (m_PortalViewIDNodeChain[m_iViewRecursionLevel]
                ->ChildNodes[pPortalRenderable->m_iPortalViewIDNodeIndex] !=
            NULL) {
          FreePortalViewIDNode(
  ...
  const int iMaxDepth = MIN(
      r_portal_stencil_depth.GetInt(),
      MIN(MAX_PORTAL_RECURSIVE_VIEWS, (1 << materials->StencilBufferBits())) -
          1);

  if (m_iViewRecursionLevel >= iMaxDepth) // can't support any more views
  {
    m_iRemainingPortalViewDepth =
        0; // special case handler for max depth 0 cases
    for (int i = 0; i != iNumRenderablePortals; ++i) {
  ...
  // queued mode makes us pass the barrier of just noticeable difference when
  // using a previous frame's occlusion as a draw skip check
  ...
  CViewSetup
      ViewBackup; // = *pViewSetup; //backup the view, we'll need to restore it
  ...
  if (m_iViewRecursionLevel == 0) // first entry into the stencil drawing
  {
    ...
    m_RecursiveViewComplexFrustums[0]
        .RemoveAll(); // clear any garbage leftover in the complex frustums from
                      // last frame
  }

  if (m_RecursiveViewComplexFrustums[m_iViewRecursionLevel].Count() == 0) {
    // nothing in the complex frustum from the current view, copy the standard
    // frustum in
    m_RecursiveViewComplexFrustums[m_iViewRecursionLevel].AddMultipleToTail(
        FRUSTUM_NUMPLANES, pViewRender->GetFrustum());
  }
  ...
  for (int i = 0; i != iNumRenderablePortals; ++i) {
    ...
    if ((pCurrentPortal->GetLinkedPortal() == NULL) ||
        ...
        (pCurrentPortal->ShouldUpdatePortalView_BasedOnView(
             *pViewSetup,
             m_RecursiveViewComplexFrustums[m_iViewRecursionLevel]) == false)) {
      // can't see through the portal, free up it's view id node for use
      // elsewhere
      if (m_PortalViewIDNodeChain[m_iViewRecursionLevel]
              ->ChildNodes[pCurrentPortal->m_iPortalViewIDNodeIndex] != NULL) {
        FreePortalViewIDNode(
            m_PortalViewIDNodeChain[m_iViewRecursionLevel]
    ...
    // Step 0, Allow for special effects to happen before cutting a hole
    {
      ...
      pRenderContext->SetStencilReferenceValue(
          iParentLevelStencilReferenceValue);

      pCurrentPortal->DrawPreStencilMask();
    }

    // step 1, write out the stencil values (and colors if you want, but really
    // not necessary)
    {
      // pRenderContext->SetStencilCompareFunction(
      // STENCILCOMPARISONFUNCTION_EQUAL );
    ...
    if (bIsQueuedMode || // don't use pixel visibly as a skip check in queued
                         // mode, the data is simply too old.
        pCurrentPortal->ShouldUpdatePortalView_BasedOnPixelVisibility(
            pCurrentPortalViewNode
                ->fScreenFilledByPortalSurfaceLastFrame_Normalized)) {
      // step 2, clear the depth buffer in stencil areas so we can render a new
      // scene to them
      {
        pRenderContext->SetStencilPassOperation(STENCILOPERATION_KEEP);
      ...
      // step 3, fill in stencil views (remember that in multiple depth
      // situations that any subportals will run through this function again
      // before this section completes, thereby screwing with stencil settings)
      {
        ...
        pRenderContext->GetFogDistances(&fFogStartBackup, &fFogEndBackup,
                                        &fFogZBackup);
        CGlowOverlay::BackupSkyOverlayData(m_iViewRecursionLevel);

        Assert(
            m_PortalViewIDNodeChain[m_iViewRecursionLevel]->ChildNodes.Count() >
            pCurrentPortal->m_iPortalViewIDNodeIndex);
        ...
        // do a full reset of what we think the stencil operations are in case
        // the recursive calls got weird
        pRenderContext->SetStencilCompareFunction(
            STENCILCOMPARISONFUNCTION_EQUAL);
        pRenderContext->SetStencilPassOperation(STENCILOPERATION_KEEP);
        pRenderContext->SetStencilFailOperation(STENCILOPERATION_KEEP);
      ...
      // step 4, patch up the fact that we just made a hole in the wall because
      // it's not *really* a hole at all
      {
  ...
  // step 6, go back to non-stencil rendering mode in preparation to resume
  ...
  if (bRebuildDrawListsWhenDone) {
    memcpy(
        (void *)pViewSetup, &ViewBackup,
        sizeof(
            CViewSetup)); // if we don't restore this, the view is permanently
                          // altered (in mid render of an existing scene)
  }
...
void CPortalRender::DrawPortalsToTextures(CViewRender *pViewRender,
  ...
  m_RecursiveViewComplexFrustums[0]
      .RemoveAll(); // clear any garbage leftover in the complex frustums from
                    // last frame
  m_RecursiveViewComplexFrustums[0].AddMultipleToTail(
      FRUSTUM_NUMPLANES, pViewRender->GetFrustum());
...
//-----------------------------------------------------------------------------
// normalized for how many of the screen's possible pixels it takes up, less
// than zero indicates a lack of data from last frame
//-----------------------------------------------------------------------------
float CPortalRender::GetPixelVisilityForPortalSurface(
    const CPortalRenderable *pPortal) const {
  PortalViewIDNode_t *pNode =
      m_PortalViewIDNodeChain[m_iViewRecursionLevel]
          ->ChildNodes[pPortal->m_iPortalViewIDNodeIndex];
  if (pNode)
...
//-----------------------------------------------------------------------------
// Methods to query about the exit portal associated with the currently
// rendering portal
//-----------------------------------------------------------------------------
...
bool CPortalRender::DoesExitPortalViewIntersectWaterPlane(
    float waterZ, int leafWaterDataID) const {
  return m_pRenderingViewExitPortal->DoesExitViewIntersectWaterPlane(
      waterZ, leafWaterDataID);
}

//-----------------------------------------------------------------------------
// Returns the remaining number of portals to render within other portals
// lets portals know that they should do "end of the line" kludges to cover up
...
void CPortalRender::OverlayPortalRenderTargets(float w, float h) {
  ...
  OverlayCameraRenderTarget("engine/debug_water_reflect_1", w + 10, h + 10, w,
                            h);
  OverlayCameraRenderTarget("engine/debug_water_reflect_2", (w + 10) * 2,
                            h + 10, w, h);

  OverlayCameraRenderTarget("engine/debug_water_refract_0", 0, (h + 10) * 2, w,
                            h);
  OverlayCameraRenderTarget("engine/debug_water_refract_1", w + 10,
                            (h + 10) * 2, w, h);
  OverlayCameraRenderTarget("engine/debug_water_refract_2", (w + 10) * 2,
                            (h + 10) * 2, w, h);
}
...
CPortalRenderable *
CPortalRender::FindRecordedPortal(IClientRenderable *pRenderable) {
  int nCount = m_RecordedPortals.Count();
  for (int i = 0; i < nCount; ++i) {
...
void CPortalRender::HandlePortalPlaybackMessage(KeyValues *pKeyValues) {
  // Iterate through all the portal ids of all the portals in the keyvalues
  // message
  CUtlVector<int> foundIds;
  for (KeyValues *pCurr = pKeyValues->GetFirstTrueSubKey(); pCurr;
       pCurr = pCurr->GetNextTrueSubKey()) {
    // Create new area portals for those ids that don't exist
    int nPortalId = pCurr->GetInt("portalId");
    IClientRenderable *pRenderable =
        (IClientRenderable *)pCurr->GetPtr("clientRenderable");
    int nIndex = FindRecordedPortalIndex(nPortalId);
    if (nIndex < 0) {
      CPortalRenderable *pPortal = NULL;
      const char *szType = pCurr->GetString(
          "portalType", "flatBasic"); //"flatBasic" being the type commonly
                                      //found in "Portal" mod
      // search through registered creation functions for one that makes this
      // type of portal
      ...
      if (pPortal == NULL) {
        ...
        Warning("CPortalRender::HandlePortalPlaybackMessage() unable to find "
                "creation function for portal type: %s\n",
                szType);
      } else {
...
void CPortalRender::AddPortalCreationFunc(
    const char *szPortalType, PortalRenderableCreationFunc creationFunc) {
#ifdef _DEBUG
  for (int i = m_PortalRenderableCreators.Count(); --i >= 0;) {
    AssertMsg(FStrEq(m_PortalRenderableCreators[i].portalType.String(),
                     szPortalType) == false,
              "Multiple portal renderable creation functions for same type of "
              "portal renderable.");
  }
#endif
...
//-----------------------------------------------------------------------------
// Purpose: Tests the parameter view ID against ID's used by portal pixel vis
// queries Input  : id - id tested against used portal view ids Output : Returns
// true if id matches an ID used by a portal, or it's recursive sub portals
//-----------------------------------------------------------------------------
