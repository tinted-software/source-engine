...
IMPLEMENT_CLIENTCLASS_DT(C_Env_Lightrail_Endpoint, DT_Env_Lightrail_Endpoint,
                         CEnv_Lightrail_Endpoint)
RecvPropFloat(RECVINFO(m_flSmallScale)),
    RecvPropFloat(RECVINFO(m_flLargeScale)), RecvPropInt(RECVINFO(m_nState)),
...
bool C_Env_Lightrail_Endpoint::SetupEmitters(void) {
  ...
  if (m_pAttractorEmitter.IsValid() == false) {
    m_pAttractorEmitter =
        CParticleAttractor::Create(GetAbsOrigin(), "energyattractor");
...
void C_Env_Lightrail_Endpoint::UpdateIdle(float percentage) {
  ...
  for (int i = 0; i < numParticles; i++) {
    ...
    dist = RemapValClamped(dist, 4.0f * percentage, 64.0f * percentage, 6.0f,
                           1.0f);
    ...
    sParticle = (SimpleParticle *)m_pAttractorEmitter->AddParticle(
        ...
        m_pAttractorEmitter->GetPMaterial("effects/strider_muzzle"), offset);
...
void C_Env_Lightrail_Endpoint::UpdateCharging(float percentage) {
  ...
  for (int i = 0; i < 2; i++) {
    sParticle = (SimpleParticle *)m_pSimpleEmitter->AddParticle(
        sizeof(SimpleParticle),
        m_pSimpleEmitter->GetPMaterial("effects/strider_muzzle"),
        GetAbsOrigin());
...
void C_Env_Lightrail_Endpoint::UpdateLargeFX(void) {
  ...
  sParticle = (SimpleParticle *)m_pSimpleEmitter->AddParticle(
      sizeof(SimpleParticle),
      m_pSimpleEmitter->GetPMaterial("effects/strider_muzzle"), GetAbsOrigin());
  ...
  for (int i = 0; i < 2; i++) {
    sParticle = (SimpleParticle *)m_pSimpleEmitter->AddParticle(
        sizeof(SimpleParticle),
        m_pSimpleEmitter->GetPMaterial("effects/combinemuzzle2"),
        GetAbsOrigin());
  ...
  for (int i = 0; i < 4; i++) {
    ...
    dist = RemapValClamped(dist, 4.0f * m_flLargeScale, 64.0f * m_flLargeScale,
                           6.0f, 1.0f);
    offset += right * random->RandomFloat(-2.0f * dist * m_flLargeScale,
                                          2.0f * dist * m_flLargeScale);
    offset += up * random->RandomFloat(-2.0f * dist * m_flLargeScale,
                                       2.0f * dist * m_flLargeScale);
    ...
    sParticle = (SimpleParticle *)m_pAttractorEmitter->AddParticle(
        sizeof(SimpleParticle),
        m_pAttractorEmitter->GetPMaterial("effects/combinemuzzle2_dark"),
        offset);
...
inline float C_Env_Lightrail_Endpoint::GetStateDurationPercentage(void) {
  ...
  return RemapValClamped((gpGlobals->curtime - m_flStartTime), 0, m_flDuration,
                         0, 1.0f);
  ;
}
...
// Purpose:
//-----------------------------------------------------------------------------
void C_Env_Lightrail_Endpoint::NotifyShouldTransmit(
