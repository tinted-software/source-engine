...
private:
  ...
  CSmartPtr<CSimpleEmitter>
      m_pSimpleEmitter; // particle system, emits particles
  CSmartPtr<CParticleAttractor>
      m_pAttractorEmitter; // particle system, attracts particles
};

IMPLEMENT_CLIENTCLASS_DT(C_EnvPortalPathTrack, DT_EnvPortalPathTrack,
                         CEnvPortalPathTrack)

RecvPropBool(RECVINFO(m_bTrackActive)),
    RecvPropBool(RECVINFO(m_bEndpointActive)), RecvPropInt(RECVINFO(m_nState)),
...
bool C_EnvPortalPathTrack::SetupEmitters(void) {
  ...
  if (m_pAttractorEmitter.IsValid() == false) {
    m_pAttractorEmitter = CParticleAttractor::Create(
        GetAbsOrigin(), "portaltracktrainendpointattractor");
