...
//
// Purpose: Portal version of the combine ball. This client code is needed to
// provide a different
//			look when the energy ball has infinite life and to have
//modified client effects.
//
//=====================================================================================//
...
ConVar cl_energy_ball_start_fade_time("cl_energy_ball_start_fade_time", "8",
                                      FCVAR_CHEAT);
...
protected:
  ...
  float m_fTimeTillDeath; // If this is a finite life energy ball, the time
                          // remaining until detonation
  float m_fCurAlpha; // The amount of alpha to apply at DrawModel, to simulate a
                     // decaying energy ball
};
...
RecvPropBool(RECVINFO(m_bIsInfiniteLife)),
    ...
    END_RECV_TABLE()
        ...
        BEGIN_PREDICTION_DATA(C_PropEnergyBall)
            ...
            DEFINE_PRED_FIELD(m_bIsInfiniteLife, FIELD_BOOLEAN,
...
//-----------------------------------------------------------------------------
// Purpose: Use our custom body materials for energy ball, but otherwise use the
// base class materials (base being C_PropCombineBall) Output : bool
//-----------------------------------------------------------------------------
bool C_PropEnergyBall::InitMaterials() {
  // Use the same materials as a combine ball
  bool bRetVal = BaseClass::InitMaterials();

  // If we're an infinite life combine ball, swap out the body material (and the
  // base implementation didnt fail)
  IMaterial *pBodyMat;
  if (m_bIsInfiniteLife) {
    pBodyMat =
        materials->FindMaterial("effects/eball_infinite_life", NULL, false);
  } else {
    pBodyMat =
        materials->FindMaterial("effects/eball_finite_life", NULL, false);
  }
