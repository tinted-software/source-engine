...
RecvPropEHandle(RECVINFO(m_hLinkedPortal)),
    RecvPropBool(RECVINFO(m_bActivated)), RecvPropBool(RECVINFO(m_bIsPortal2)),
    END_RECV_TABLE()
  ...
  bool bAlreadyTeleported =
      ((ptCurrentPosition - ptNewPosition).LengthSqr() <
       (ptInvCurrentPosition - ptNewPosition)
           .LengthSqr()); // newest network update closer to destination than
                          // transformed to destination indicates it's already
                          // been transformed

  UTIL_TransformInterpolatedAngle(rotInterp, matTransform.As3x4(),
                                  bAlreadyTeleported);

  if (bIsPlayer) // the player's origin != player's center, transforms are
                 // performed about centers
  {
  ...
  UTIL_TransformInterpolatedPosition(posInterp, matTransform,
                                     bAlreadyTeleported);
...
static ConVar portal_demohack("portal_demohack", "0", FCVAR_ARCHIVE,
                              "Do the demo_legacy_rollback setting to help "
                              "during demo playback of going through portals.");
  ...
  virtual bool Init() {
    ...
    if (portal_demohack.GetBool()) {
      ...
      demo_legacy_rollback_ref.SetValue(
          false); // Portal demos are wrong if the eyes rollback as far as
                  // regular demos
    }
...
void C_Prop_Portal::Activate(void) { BaseClass::Activate(); }
...
void C_Prop_Portal::Simulate() {
  ...
  // clear list of ghosted entities from last frame, and clear the clipping
  // planes we put on them
  ...
  // Find objects that are intersecting the portal and mark them for later
  // replication on the remote portal's side
  ...
  int iEntsNearPortal = UTIL_EntitiesInSphere(
      pEntsNearPortal, 1024, GetNetworkOrigin(), PORTAL_HALF_HEIGHT, 0,
  ...
  if (iEntsNearPortal != 0) {
    ...
    for (int i = 0; i != iEntsNearPortal; ++i) {
      ...
      Assert(
          dynamic_cast<C_Prop_Portal *>(pEntity) ==
          NULL); // should have been killed with (pEntity->GetMoveType() ==
                 // MOVETYPE_NONE) check. Infinite recursion is infinitely bad.
      ...
      if ((m_plane_Origin.normal.Dot(ptEntCenter) - m_plane_Origin.dist) <
          -5.0f)
        continue; // entity is behind the portal, most likely behind the wall
                  // the portal is placed on

      if (!CProp_Portal_Shared::IsEntityTeleportable(pEntity))
        continue;

      if (bActivePlayerWeapon) {
        if (!m_PortalSimulator.EntityHitBoxExtentIsInPortalHole(
                pWeapon->GetOwner()) &&
  ...
  {
    bool *bStillInUse =
        (bool *)stackalloc(sizeof(bool) * (m_GhostRenderables.Count() +
                                           m_hGhostingEntities.Count()));
    memset(bStillInUse, 0,
           sizeof(bool) *
               (m_GhostRenderables.Count() + m_hGhostingEntities.Count()));
    ...
    for (int i = m_hGhostingEntities.Count(); --i >= 0;) {
      ...
      C_PortalGhostRenderable *pNewGhost = new C_PortalGhostRenderable(
          this, pRenderable, pEntity->GetRenderGroup(), m_matrixThisToLinked,
...
void C_Prop_Portal::UpdateOnRemove(void) {
  if (TransformedLighting.m_LightShadowHandle != CLIENTSHADOW_INVALID_HANDLE) {
    g_pClientShadowMgr->DestroyFlashlight(
        TransformedLighting.m_LightShadowHandle);
    TransformedLighting.m_LightShadowHandle = CLIENTSHADOW_INVALID_HANDLE;
  }
...
void C_Prop_Portal::OnNewParticleEffect(
    const char *pszParticleName, CNewParticleEffect *pNewParticleEffect) {
  if (Q_stricmp(pszParticleName, "portal_1_overlap") == 0 ||
      Q_stricmp(pszParticleName, "portal_2_overlap") == 0) {
    ...
    if (iPortalCount != 0) {
      ...
      for (int i = 0; i != iPortalCount; ++i) {
        ...
        if (pTempPortal != this && pTempPortal->m_bActivated) {
          ...
          float fDistanceSqr =
              pNewParticleEffect->GetRenderOrigin().DistToSqr(vPosition);
...
// ConVar r_portal_light_innerangle( "r_portal_light_innerangle", "90.0",
// FCVAR_CLIENTDLL ); ConVar r_portal_light_outerangle(
// "r_portal_light_outerangle", "90.0", FCVAR_CLIENTDLL ); ConVar
// r_portal_light_forward( "r_portal_light_forward", "0.0", FCVAR_CLIENTDLL );
...
void C_Prop_Portal::OnDataChanged(DataUpdateType_t updateType) {
  ...
  bool bNewLinkage =
      ((PreDataChanged.m_hLinkedTo.Get() != m_hLinkedPortal.Get()));
  if (bNewLinkage)
    m_PortalSimulator
        .DetachFromLinked(); // detach now so moves are theoretically faster
  ...
  if (m_bActivated) {
    ...
    g_pPortalRender->AddPortal(
        this); // will know if we're already added and avoid adding twice
    ...
    if (bPortalMoved) {
      ...
      m_fOpenAmount = 0.0f;
      // m_fStaticAmount = 1.0f; // This will cause the portal we are opening to
      // show the static effect
      SetNextClientThink(CLIENT_THINK_ALWAYS); // we need this to help open up

      // add static to the remote
      if (pRemote) {
        pRemote->m_fStaticAmount =
            1.0f; // This will cause the other portal to show the static effect
      ...
      if (pRemote) {
        ...
        pRemote->TransformedLighting.m_LightShadowHandle =
            CLIENTSHADOW_INVALID_HANDLE;
      }
      ...
      if (TransformedLighting.m_LightShadowHandle !=
          CLIENTSHADOW_INVALID_HANDLE) {
      ...
      if (pFakeLight != NULL) {
        // turn off the light so it doesn't interfere with absorbed light
        // calculations
        pFakeLight->color.r = 0;
        pFakeLight->color.g = 0;
        pFakeLight->color.b = 0;
        pFakeLight->flags =
            DLIGHT_NO_WORLD_ILLUMINATION | DLIGHT_NO_MODEL_ILLUMINATION;
  ...
  if ((PreDataChanged.m_hLinkedTo.Get() != m_hLinkedPortal.Get()) &&
      m_hLinkedPortal.Get())
...
void C_Prop_Portal::GetToolRecordingState(KeyValues *msg) {
  ...
  if (!m_bActivated) {
    BaseEntityRecordingState_t *pBaseEntity =
...
void C_Prop_Portal::SetIsPortal2(bool bValue) { m_bIsPortal2 = bValue; }
