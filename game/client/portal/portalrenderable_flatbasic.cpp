...
#if !defined(_X360) // XBox 360 is guaranteed to use stencil mode, and therefore
                    // doesn't need texture mode materials
...
// CLIENTEFFECT_MATERIAL( "effects/flashlight001" ) //light transfers disabled
// indefinitely
...
public:
  ...
  void LevelInitPreEntity() {
    m_Materials.m_PortalMaterials[0].Init("models/portals/portal_1_dynamicmesh",
                                          TEXTURE_GROUP_CLIENT_EFFECTS);
    m_Materials.m_PortalMaterials[1].Init("models/portals/portal_2_dynamicmesh",
                                          TEXTURE_GROUP_CLIENT_EFFECTS);
    m_Materials.m_PortalRenderFixMaterials[0].Init(
        "models/portals/portal_1_renderfix_dynamicmesh",
        TEXTURE_GROUP_CLIENT_EFFECTS);
    m_Materials.m_PortalRenderFixMaterials[1].Init(
        "models/portals/portal_2_renderfix_dynamicmesh",
        TEXTURE_GROUP_CLIENT_EFFECTS);
    m_Materials.m_PortalDepthDoubler.Init("models/portals/portal_depthdoubler",
                                          TEXTURE_GROUP_CLIENT_EFFECTS);
    m_Materials.m_PortalStaticOverlay[0].Init(
        "models/portals/portalstaticoverlay_1", TEXTURE_GROUP_CLIENT_EFFECTS);
    m_Materials.m_PortalStaticOverlay[1].Init(
        "models/portals/portalstaticoverlay_2", TEXTURE_GROUP_CLIENT_EFFECTS);
    m_Materials.m_Portal_Stencil_Hole.Init("models/portals/portal_stencil_hole",
                                           TEXTURE_GROUP_CLIENT_EFFECTS);
    m_Materials.m_Portal_Refract[0].Init("models/portals/portal_refract_1",
                                         TEXTURE_GROUP_CLIENT_EFFECTS);
    m_Materials.m_Portal_Refract[1].Init("models/portals/portal_refract_2",
                                         TEXTURE_GROUP_CLIENT_EFFECTS);
    // m_Materials.m_PortalLightTransfer_ShadowTexture.Init(
    // "effects/flashlight001", TEXTURE_GROUP_OTHER ); //light transfers
    // disabled indefinitely

    m_Materials.m_pDepthDoubleViewMatrixVar =
        m_Materials.m_PortalDepthDoubler->FindVar("$alternateviewmatrix", NULL,
                                                  false);
    Assert(m_Materials.m_pDepthDoubleViewMatrixVar != NULL);
  }
};
static CAutoInitFlatBasicPortalDrawingMaterials
    s_FlatBasicPortalDrawingMaterials;

const FlatBasicPortalRenderingMaterials_t
    &CPortalRenderable_FlatBasic::m_Materials =
...
CPortalRenderable_FlatBasic::CPortalRenderable_FlatBasic(void)
    : m_pLinkedPortal(NULL), m_ptOrigin(0.0f, 0.0f, 0.0f),
      m_vForward(1.0f, 0.0f, 0.0f), m_vUp(0.0f, 0.0f, 1.0f),
      m_vRight(0.0f, 1.0f, 0.0f), m_fStaticAmount(0.0f),
      m_fSecondaryStaticAmount(0.0f), m_fOpenAmount(0.0f), m_bIsPortal2(false) {
...
void CPortalRenderable_FlatBasic::GetToolRecordingState(bool bActive,
                                                        KeyValues *msg) {
  if (!ToolsEnabled())
    return;

  VPROF_BUDGET("CPortalRenderable_FlatBasic::GetToolRecordingState",
...
void CPortalRenderable_FlatBasic::PortalMoved(void) {
  ...
  // Update the points on the portal which we add to PVS
  {
    ...
    m_InternallyMaintainedData.m_ptCorners[0] =
        (m_InternallyMaintainedData.m_ptForwardOrigin + vScaledRight) +
        vScaledUp;
    m_InternallyMaintainedData.m_ptCorners[1] =
        (m_InternallyMaintainedData.m_ptForwardOrigin - vScaledRight) +
        vScaledUp;
    ...
    m_InternallyMaintainedData.m_VisData.m_vecVisOrigin =
        m_InternallyMaintainedData.m_ptForwardOrigin;
    m_InternallyMaintainedData.m_VisData.m_fDistToAreaPortalTolerance = 64.0f;
  ...
  m_InternallyMaintainedData.m_nSkyboxVisibleFromCorners =
      engine->IsSkyboxVisibleFromPoint(
          m_InternallyMaintainedData.m_ptForwardOrigin);
  for (int i = 0;
       i < 4 && (m_InternallyMaintainedData.m_nSkyboxVisibleFromCorners !=
                 SKYBOX_3DSKYBOX_VISIBLE);
       ++i) {
    SkyboxVisibility_t nCornerVis = engine->IsSkyboxVisibleFromPoint(
        m_InternallyMaintainedData.m_ptCorners[i]);
    if ((m_InternallyMaintainedData.m_nSkyboxVisibleFromCorners ==
         SKYBOX_NOT_VISIBLE) ||
  ...
  // render fix bounding planes
  {
    for (int i = 0; i != PORTALRENDERFIXMESH_OUTERBOUNDPLANES; ++i) {
      float fCirclePos =
          ((float)(i)) *
          ((M_PI * 2.0f) / (float)PORTALRENDERFIXMESH_OUTERBOUNDPLANES);
      float fUpBlend = cosf(fCirclePos);
      float fRightBlend = sinf(fCirclePos);

      Vector vNormal = -fUpBlend * m_vUp - fRightBlend * m_vRight;
      Vector ptOnPlane = m_ptOrigin +
                         (m_vUp * (fUpBlend * PORTAL_HALF_HEIGHT * 1.1f)) +
                         (m_vRight * (fRightBlend * PORTAL_HALF_WIDTH * 1.1f));

      m_InternallyMaintainedData.m_BoundingPlanes[i].Init(
          vNormal, vNormal.Dot(ptOnPlane));
    }

    m_InternallyMaintainedData
        .m_BoundingPlanes[PORTALRENDERFIXMESH_OUTERBOUNDPLANES]
        .Init(-m_vForward, (-m_vForward).Dot(m_ptOrigin));
    m_InternallyMaintainedData
        .m_BoundingPlanes[PORTALRENDERFIXMESH_OUTERBOUNDPLANES + 1]
        .Init(m_vForward, m_vForward.Dot(m_ptOrigin - (m_vForward * 5.0f)));
  }
  ...
  m_InternallyMaintainedData.m_bUsableDepthDoublerConfiguration =
      (m_pLinkedPortal && // linked to another portal
       (m_vForward.Dot(m_pLinkedPortal->m_ptOrigin - m_ptOrigin) >
        0.0f) && // this portal looking in the general direction of the other
                 // portal
       (m_vForward.Dot(m_pLinkedPortal->m_vForward) <
        -0.7071f)); // within 45 degrees of facing directly at each other

  if (m_pLinkedPortal)
    m_pLinkedPortal->m_InternallyMaintainedData
        .m_bUsableDepthDoublerConfiguration = true;
  ...
  if (m_pLinkedPortal != NULL) {
    ...
    matrix3x4_t remoteToWorld(
        m_pLinkedPortal->m_vForward, -m_pLinkedPortal->m_vRight,
        m_pLinkedPortal->m_vUp, m_pLinkedPortal->m_ptOrigin);
    CProp_Portal_Shared::UpdatePortalTransformationMatrix(
        localToWorld, remoteToWorld, &m_matrixThisToLinked);

    // update the remote portal
    MatrixInverseTR(m_matrixThisToLinked,
                    m_pLinkedPortal->m_matrixThisToLinked);
  } else {
    m_matrixThisToLinked
        .Identity(); // don't accidentally teleport objects to zero space
  }
}
...
ConVar r_portal_use_complex_frustums(
    "r_portal_use_complex_frustums", "1", FCVAR_CLIENTDLL,
    "View optimization, turn this off if you get odd visual bugs.");

bool CPortalRenderable_FlatBasic::CalcFrustumThroughPortal(
    const Vector &ptCurrentViewOrigin, Frustum OutputFrustum) {
  ...
  if ((iViewRecursionLevel == 0) &&
      ((ptCurrentViewOrigin - m_ptOrigin).LengthSqr() <
       (PORTAL_HALF_HEIGHT *
        PORTAL_HALF_HEIGHT))) // FIXME: Player closeness check might need
                              // reimplementation
  {
    // calculations are most likely going to be completely useless, return
    // nothing
    return false;
  }
  ...
  if (m_vForward.Dot(ptCurrentViewOrigin) <=
      m_InternallyMaintainedData.m_fPlaneDist)
    return false; // looking at portal backface

  // VPlane *pInputFrustum = view->GetFrustum();
  // //g_pPortalRender->m_RecursiveViewComplexFrustums[iViewRecursionLevel].Base();
  // int iInputFrustumPlaneCount = 6;
  // //g_pPortalRender->m_RecursiveViewComplexFrustums[iViewRecursionLevel].Count();
  VPlane *pInputFrustum =
      g_pPortalRender->m_RecursiveViewComplexFrustums[iViewRecursionLevel]
          .Base();
  ...
  Vector *pInVerts =
      (Vector *)stackalloc(sizeof(Vector) * iAllocSize *
                           2); // possible to add 1 point per cut, 4 starting
                               // points, iInputFrustumPlaneCount cuts
  Vector *pOutVerts = pInVerts + iAllocSize;
  Vector *pTempVerts;

  // clip by first plane and put output into pInVerts
  int iVertCount = ClipPolyToPlane(m_InternallyMaintainedData.m_ptCorners, 4,
                                   pInVerts, pInputFrustum[0].m_Normal,
  ...
  for (i = 1; i != iInputFrustumPlaneCount; ++i) {
    ...
    iVertCount = ClipPolyToPlane(pInVerts, iVertCount, pOutVerts,
                                 pInputFrustum[i].m_Normal,
                                 pInputFrustum[i].m_Dist, 0.01f);
    pTempVerts = pInVerts;
    pInVerts = pOutVerts;
  ...
  g_pPortalRender->m_RecursiveViewComplexFrustums[iNextViewRecursionLevel]
      .SetCount(iVertCount + 2); //+2 for near and far z planes

  Vector ptTransformedCamera = m_matrixThisToLinked * ptCurrentViewOrigin;

  // generate planes defined by each line around the convex and the camera
  // origin
  for (i = 0; i != iVertCount; ++i) {
    ...
    g_pPortalRender->m_RecursiveViewComplexFrustums[iNextViewRecursionLevel]
        .Element(i)
        .Init(vNormal, vNormal.Dot(ptTransformedCamera));
  }

  // Near Z
  g_pPortalRender->m_RecursiveViewComplexFrustums[iNextViewRecursionLevel]
      .Element(i)
      .Init(m_pLinkedPortal->m_vForward,
            m_pLinkedPortal->m_InternallyMaintainedData.m_fPlaneDist);

  // Far Z
  ++i;
  {
    Vector vNormal = m_matrixThisToLinked.ApplyRotation(
        pInputFrustum[iInputFrustumPlaneCount - 1].m_Normal);
    Vector ptOnPlane = pInputFrustum[iInputFrustumPlaneCount - 1].m_Dist *
                       pInputFrustum[iInputFrustumPlaneCount - 1].m_Normal;
    g_pPortalRender->m_RecursiveViewComplexFrustums[iNextViewRecursionLevel]
        .Element(i)
        .Init(vNormal, vNormal.Dot(m_matrixThisToLinked * ptOnPlane));
  }
  ...
  if (iVertCount > 4) {
    ...
    while (iVertCount > 4) {
      ...
      // so, we're going to eliminate the smallest sides and bridge the
      // surrounding sides until we're down to 4

      float fMinSide = fLineLengthSqr[0];
      int iMinSideFirstPoint = 0;
      int iOldVertCount = iVertCount;
      --iVertCount; // we're going to decrement this sometime in this block, it
                    // makes math easier to do it now
      ...
      // now we know the two points that we have to merge to one, project and
      // make a merged point from the surrounding lines
      if (fMinSide >= 0.1f) // only worth doing the math if it's actually going
                            // to be accurate and make a difference
      {
        Vector vLine1 = *p2 - *p1;
        Vector vLine2 = *p3 - *p4;
        ...
        AssertMsgOnce(fNormalDot != 0.0f,
                      "Tell Dave Kircher if this pops up. It won't interfere "
                      "with gameplay though");
        if (fNormalDot == 0.0f) {
          return false; // something went horribly wrong, bail and just suffer a
                        // slight framerate penalty for now
        }
      ...
      fLineLengthSqr[i2] =
          (*p4 - *p2).LengthSqr(); // must do this BEFORE possibly shifting
                                   // points p4+ left
    ...
    for (i = 0; i != iVertCount; ++i) {
      OutputFrustum[i] =
          g_pPortalRender
              ->m_RecursiveViewComplexFrustums[iNextViewRecursionLevel]
              .Element(i);
    }

    for (; i != 4; ++i) {
      // we had less than 4 planes for the sides, just copy from the last valid
      // plane
  ...
  OutputFrustum[FRUSTUM_NEARZ] =
      g_pPortalRender->m_RecursiveViewComplexFrustums[iNextViewRecursionLevel]
          .Element(iComplexCount - 2);
  OutputFrustum[FRUSTUM_FARZ] =
      g_pPortalRender->m_RecursiveViewComplexFrustums[iNextViewRecursionLevel]
          .Element(iComplexCount - 1);

  return true;
}

void CPortalRenderable_FlatBasic::RenderPortalViewToBackBuffer(
    CViewRender *pViewRender, const CViewSetup &cameraView) {
  ...
  if (m_pLinkedPortal ==
      NULL) // not linked to any portal, so we'll be all static anyways
    return;
  ...
  bUseSeeThroughFrustum =
      CalcFrustumThroughPortal(cameraView.origin, seeThroughFrustum);
  ...
  QAngle qPOVAngles = TransformAnglesToWorldSpace(cameraView.angles,
                                                  m_matrixThisToLinked.As3x4());
...

  {
    ...
    fCustomClipPlane[3] = vRemotePortalForward.Dot(
        ...
        (vRemotePortalForward * 0.5f)); // moving it back a smidge to eliminate
                                        // visual artifacts for half-in objects

    pRenderContext->PushCustomClipPlane(
        fCustomClipPlane); // this is technically the same plane within
                           // recursive views, but pushing it anyway in-case
                           // something else has been added to the stack
  }

  {
    ...
    render->Push3DView(portalView, 0, NULL, pViewRender->GetFrustum());
    {
      ...
      CPortalRenderable *pRenderingViewForPortalBackup =
          g_pPortalRender->GetCurrentViewEntryPortal();
      CPortalRenderable *pRenderingViewExitPortalBackup =
          g_pPortalRender->GetCurrentViewExitPortal();
      SetViewEntranceAndExitPortals(this, m_pLinkedPortal);

      // DRAW!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ViewDrawScene_PortalStencil(pViewRender, portalView, &customVisibility);

      SetViewEntranceAndExitPortals(pRenderingViewForPortalBackup,
                                    pRenderingViewExitPortalBackup);
      ...
      if (m_InternallyMaintainedData.m_bUsableDepthDoublerConfiguration &&
        ...
        // It's important that we do this AFTER using the depth doubler this
        // frame to compensate for the fact that the front buffer is 1 frame
        // behind the current view matrix otherwise we get a lag effect when the
        // player changes their viewing angles
        pRenderContext->GetMatrix(
            MATERIAL_VIEW,
            &m_InternallyMaintainedData.m_DepthDoublerTextureView);
      }
...
void CPortalRenderable_FlatBasic::RenderPortalViewToTexture(
    CViewRender *pViewRender, const CViewSetup &cameraView) {
  if (m_fStaticAmount == 1.0f)
    return; // not going to see anything anyways
  ...
  float fPixelVisibilty =
      g_pPortalRender->GetPixelVisilityForPortalSurface(this);
  if ((fPixelVisibilty >= 0.0f) &&
      (fPixelVisibilty <= PORTALRENDERABLE_FLATBASIC_MINPIXELVIS))
    return;
  ...
  portalView.m_flAspectRatio =
      ...
      (float)cameraView.height; // use the screen aspect ratio, 0.0f doesn't
                                // work as advertised
  ...
  fCustomClipPlane[3] = m_pLinkedPortal->m_vForward.Dot(
      ...
      (m_pLinkedPortal->m_vForward *
       0.5f)); // moving it back a smidge to eliminate visual artifacts for
               // half-in objects

  pRenderContext->PushCustomClipPlane(fCustomClipPlane);

  {
    render->Push3DView(portalView, VIEW_CLEAR_DEPTH, pRenderTarget,
...

    {
      ...
      int nClearFlags = 0;
      Draw3dSkyboxworld_Portal(pViewRender, portalView, nClearFlags,
                               bDrew3dSkybox, nSkyboxVisible, pRenderTarget);
      ...
      ViewDrawScene(pViewRender, bDrew3dSkybox, nSkyboxVisible, portalView,
                    nClearFlags, (view_id_t)g_pPortalRender->GetCurrentViewId(),
                    false, 0, &customVisibility);
...
// input -  pViewRender: pointer to the CViewRender class used to render this
// scene.
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void CPortalRenderable_FlatBasic::AddToVisAsExitPortal(
  ...
  for (int i = 0; i < 4; ++i) {
    if (enginetrace->GetLeafContainingPoint(
            m_InternallyMaintainedData.m_ptCorners[i]) != -1)
      pCustomVisibility->AddVisOrigin(
          m_InternallyMaintainedData.m_ptCorners[i]);
  }
...
void CPortalRenderable_FlatBasic::DrawPreStencilMask(void) {
  if ((m_fOpenAmount > 0.0f) && (m_fOpenAmount < 1.0f)) {
    DrawSimplePortalMesh(
        m_Materials.m_Portal_Refract[((m_bIsPortal2) ? (1) : (0))]);
  }
}
...
bool CPortalRenderable_FlatBasic::ShouldUpdateDepthDoublerTexture(
    const CViewSetup &viewSetup) {
  return ((m_InternallyMaintainedData.m_bUsableDepthDoublerConfiguration) &&
          (m_pLinkedPortal != NULL) && (m_fStaticAmount < 1.0f));
}

void CPortalRenderable_FlatBasic::HandlePortalPlaybackMessage(
  ...
  m_pLinkedPortal =
      nLinkedPortalId >= 0
          ? (CPortalRenderable_FlatBasic *)FindRecordedPortal(nLinkedPortalId)
          : NULL;
...
static void DrawComplexPortalMesh_SubQuad(
    Vector &ptBottomLeft, Vector &vUp, Vector &vRight, float *fSubQuadRect,
    void *pBindEnt, const IMaterial *pMaterial,
    const VMatrix *pReplacementViewMatrixForTexCoords = NULL) {
  PortalMeshPoint_t Vertices[4];

  Vertices[0].vWorldSpacePosition =
      ptBottomLeft + (vRight * fSubQuadRect[2]) + (vUp * fSubQuadRect[3]);
  Vertices[0].texCoord.x = fSubQuadRect[2];
  Vertices[0].texCoord.y = fSubQuadRect[3];

  Vertices[1].vWorldSpacePosition =
      ptBottomLeft + (vRight * fSubQuadRect[2]) + (vUp * fSubQuadRect[1]);
  Vertices[1].texCoord.x = fSubQuadRect[2];
  Vertices[1].texCoord.y = fSubQuadRect[1];

  Vertices[2].vWorldSpacePosition =
      ptBottomLeft + (vRight * fSubQuadRect[0]) + (vUp * fSubQuadRect[1]);
  Vertices[2].texCoord.x = fSubQuadRect[0];
  Vertices[2].texCoord.y = fSubQuadRect[1];

  Vertices[3].vWorldSpacePosition =
      ptBottomLeft + (vRight * fSubQuadRect[0]) + (vUp * fSubQuadRect[3]);
...
void CPortalRenderable_FlatBasic::DrawComplexPortalMesh(
    const IMaterial *pMaterialOverride,
    float fForwardOffsetModifier) // generates and draws the portal mesh (Needed
                                  // for compatibility with fixed function
                                  // rendering)
{
  PortalMeshPoint_t BaseVertices[4];

  Vector ptBottomLeft = m_ptOrigin + (m_vForward * (fForwardOffsetModifier)) -
                        (m_vRight * PORTAL_HALF_WIDTH) -
  ...
  for (int i = 0; i != PORTAL_PROJECTION_MESH_SUBDIVIDE_HEIGHTCHUNKS; ++i) {
    float fHeightEnd =
        fHeightBegin +
        (1.0f / ((float)PORTAL_PROJECTION_MESH_SUBDIVIDE_HEIGHTCHUNKS));
    ...
    for (int j = 0; j != PORTAL_PROJECTION_MESH_SUBDIVIDE_WIDTHCHUNKS; ++j) {
      float fWidthEnd =
          fWidthBegin +
          (1.0f / ((float)PORTAL_PROJECTION_MESH_SUBDIVIDE_WIDTHCHUNKS));

      fSubQuadRect[0] = fWidthBegin;
      fSubQuadRect[2] = fWidthEnd;

      DrawComplexPortalMesh_SubQuad(ptBottomLeft, vScaledUp, vScaledRight,
                                    fSubQuadRect, GetClientRenderable(),
                                    pMaterial);
...
void CPortalRenderable_FlatBasic::DrawDepthDoublerMesh(
    float fForwardOffsetModifier) {
  IMaterialVar *pDepthDoubleViewMatrixVar =
      s_FlatBasicPortalDrawingMaterials.m_Materials.m_PortalDepthDoubler
          ->FindVar("$alternateviewmatrix", NULL, false);
  if (pDepthDoubleViewMatrixVar)
    pDepthDoubleViewMatrixVar->SetMatrixValue(
        m_InternallyMaintainedData.m_DepthDoublerTextureView);
  DrawSimplePortalMesh(m_Materials.m_PortalDepthDoubler,
                       fForwardOffsetModifier);
}

void CPortalRenderable_FlatBasic::DrawSimplePortalMesh(
    const IMaterial *pMaterialOverride, float fForwardOffsetModifier) {
  const IMaterial *pMaterial;
  if (pMaterialOverride)
    pMaterial = pMaterialOverride;
  else
  ...
  Vector verts[4];
  verts[0] =
      ptCenter + (m_vRight * PORTAL_HALF_WIDTH) - (m_vUp * PORTAL_HALF_HEIGHT);
  verts[1] =
      ptCenter + (m_vRight * PORTAL_HALF_WIDTH) + (m_vUp * PORTAL_HALF_HEIGHT);
  verts[2] =
      ptCenter - (m_vRight * PORTAL_HALF_WIDTH) - (m_vUp * PORTAL_HALF_HEIGHT);
  ...
  if (mat_wireframe.GetBool()) {
    pRenderContext->Bind((IMaterial *)(const IMaterial *)
                             g_pPortalRender->m_MaterialsAccess.m_Wireframe,
...
void CPortalRenderable_FlatBasic::DrawRenderFixMesh(
    const IMaterial *pMaterialOverride, float fFrontClipDistance) {
  const IMaterial *pMaterial;
  if (pMaterialOverride)
    pMaterial = pMaterialOverride;
  else
  ...
  if ((vPortalCenterToCamera.Dot(m_vForward) <
       -1.0f)) // camera coplanar (to 1.0 units) or in front of portal plane
    return;
  ...
  if (vPortalCenterToCamera.LengthSqr() <
      (PORTAL_HALF_HEIGHT *
       PORTAL_HALF_HEIGHT)) // FIXME: Player closeness check might need
                            // reimplementation
  {
    // if the player is this close to the portal, immediately get rid of any
    // static it has as well as draw the fix
    m_fStaticAmount = 0.0f;
    // m_fSecondaryStaticAmount = 0.0f;

    float fOldDist = m_InternallyMaintainedData
                         .m_BoundingPlanes[PORTALRENDERFIXMESH_OUTERBOUNDPLANES]
                         .m_Dist;

    float fCameraDist = m_vForward.Dot(ptCameraOrigin - m_ptOrigin);

    if (fFrontClipDistance >
        fCameraDist) // never clip further out than the camera, we can see into
                     // the garbage space of the portal view's texture
      fFrontClipDistance = fCameraDist;

    m_InternallyMaintainedData
        .m_BoundingPlanes[PORTALRENDERFIXMESH_OUTERBOUNDPLANES]
        .m_Dist -= fFrontClipDistance;
...
void CPortalRenderable_FlatBasic::Internal_DrawRenderFixMesh(
    const IMaterial *pMaterial) {
  ...
  vOrigin =
      CurrentViewOrigin() + vForward * (view->GetViewSetup()->zNear + 0.05f);

  for (int i = 0; i != 4; ++i) {
    WorkVertices[i].texCoord.x = WorkVertices[i].texCoord.y = 0.0f;
  }

  WorkVertices[0].vWorldSpacePosition =
      vOrigin + (vRight * 40.0f) + (vUp * -40.0f);
  WorkVertices[1].vWorldSpacePosition =
      vOrigin + (vRight * 40.0f) + (vUp * 40.0f);
  WorkVertices[2].vWorldSpacePosition =
      vOrigin + (vRight * -40.0f) + (vUp * 40.0f);
  WorkVertices[3].vWorldSpacePosition =
      vOrigin + (vRight * -40.0f) + (vUp * -40.0f);
...
void CPortalRenderable_FlatBasic::ClipFixToBoundingAreaAndDraw(
    PortalMeshPoint_t *pVerts, const IMaterial *pMaterial) {
  PortalMeshPoint_t *pInVerts = (PortalMeshPoint_t *)stackalloc(
      4 * (PORTALRENDERFIXMESH_OUTERBOUNDPLANES + 2) * 16 *
      sizeof(PortalMeshPoint_t)); // really only should need 4x points, but I'm
                                  // paranoid
  PortalMeshPoint_t *pOutVerts = (PortalMeshPoint_t *)stackalloc(
      4 * (PORTALRENDERFIXMESH_OUTERBOUNDPLANES + 2) * 16 *
  ...
  // clip by the viewing frustum
  {
    ...
    for (int i = 0; i != FRUSTUM_NUMPLANES; ++i) {
      ...
      iVertCount = ClipPolyToPlane_LerpTexCoords(
          pInVerts, iVertCount, pOutVerts, pFrustum[i].m_Normal,
  ...
  // project the points so we can fudge the numbers a bit and move them to
  // exactly 0.0f depth
  {
    ...
    for (int i = 0; i != iVertCount; ++i) {
      ...
      pInVerts[i].vWorldSpacePosition =
          matViewProj * pInVerts[i].vWorldSpacePosition;
      pInVerts[i].vWorldSpacePosition *= inverseW;
      pInVerts[i].vWorldSpacePosition.z =
          0.00001f; // the primary reason we're projecting on the CPU to begin
                    // with
    }
  }
  ...
  // render with identity transforms and clipping disabled
  {
    ...
    if (mat_wireframe.GetBool())
      RenderPortalMeshConvexPolygon(
          pInVerts, iVertCount,
          materials->FindMaterial("shadertest/wireframe",
                                  TEXTURE_GROUP_CLIENT_EFFECTS, false),
          this);
...
//-----------------------------------------------------------------------------
// renders a quad that simulates fog as an overlay for something else (most
// notably the hole we create for stencil mode portals)
//-----------------------------------------------------------------------------
void CPortalRenderable_FlatBasic::RenderFogQuad(void) {
  ...
  /*float fColorScale = LinearToGammaFullRange(
  ...
  pRenderContext->Bind((IMaterial *)(const IMaterial *)g_pPortalRender
                           ->m_MaterialsAccess.m_TranslucentVertexColor);
  ...
  for (int i = 0; i != 4; ++i) {
    ...
    float fFogAmount = ((-projZ * ooFogRange) +
                        FogEndOverFogRange); // projZ should be negative
...
void CPortalRenderable_FlatBasic::DrawPortal(void) {
  ...
  if (g_pPortalRender->ShouldUseStencilsToRenderPortals()) {
    ...
    if (g_pPortalRender->IsRenderingPortal() == false) // main view
    {
      ...
      DrawSimplePortalMesh(
          m_Materials.m_PortalStaticOverlay[((m_bIsPortal2) ? (1) : (0))]);
      DrawRenderFixMesh(g_pPortalRender->m_MaterialsAccess.m_WriteZ_Model);
               ...
               this) // don't render portals that our view is exiting from
    {
      ...
      DrawSimplePortalMesh(m_Materials.m_PortalStaticOverlay[(
          (m_bIsPortal2) ? (1)
                         : (0))]); // FIXME: find out why the projection mesh
                                   // screws up at the second level of rendering
                                   // in -nouserclip situations
    }
...
bool CPortalRenderable_FlatBasic::DoesExitViewIntersectWaterPlane(
    float waterZ, int leafWaterDataID) const {
...
bool CPortalRenderable_FlatBasic::ShouldUpdatePortalView_BasedOnView(
    const CViewSetup &currentView, CUtlVector<VPlane> &currentComplexFrustum) {
  if (m_pLinkedPortal == NULL)
    return false;
      ...
      ((m_ptOrigin - vCameraPos).LengthSqr() <
       (PORTAL_HALF_HEIGHT *
        PORTAL_HALF_HEIGHT))) // FIXME: Player closeness check might need
                              // reimplementation
  {
    return true; // fudgery time. The player might not be able to see the
                 // surface, but they can probably see the render fix
  }
  ...
  int allocSize =
      (6 + currentComplexFrustum
               .Count()); // possible to add 1 point per cut, 4 starting points,
                          // N plane cuts, 2 extra because I'm paranoid
  ...
  // clip by first plane and put output into pInVerts
  int iVertCount = ClipPolyToPlane(m_InternallyMaintainedData.m_ptCorners, 4,
                                   pInVerts, currentFrustum[0].m_Normal,
...
bool CPortalRenderable_FlatBasic::ShouldUpdatePortalView_BasedOnPixelVisibility(
  ...
  return (fScreenFilledByStencilMaskLastFrame_Normalized <
          0.0f) || // < 0 is an error value
         (fScreenFilledByStencilMaskLastFrame_Normalized >
...
static CPortalRenderableCreator
    CreatePortal_FlatBasic("flatBasic", CreatePortal_FlatBasic_Fn);
