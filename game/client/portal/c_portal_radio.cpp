...
void C_Portal_Dinosaur::SetupSounds() {
  ...
  if (m_pNormalSound == NULL) {
    m_pNormalSound = CSoundEnvelopeController::GetController().SoundCreate(
        filter, entindex(), "Portal.room1_radio");
    CSoundEnvelopeController::GetController().Play(m_pNormalSound, 0.0,
                                                   PITCH_NORM);
  }

  if (m_pStaticSound == NULL) {
    m_pStaticSound = CSoundEnvelopeController::GetController().SoundCreate(
        filter, entindex(), "UpdateItem.Static");
    CSoundEnvelopeController::GetController().Play(m_pStaticSound, 0.0,
                                                   PITCH_NORM);
  }

  if (m_pSignalSound == NULL && m_hDinosaur_Signal.Get() != NULL) {
    m_pSignalSound = CSoundEnvelopeController::GetController().SoundCreate(
        filter, entindex(), m_hDinosaur_Signal->m_szSoundName);
    // m_pSignalSound = CSoundEnvelopeController::GetController().SoundCreate(
    // filter, entindex(), "UpdateItem.Signal" );
    CSoundEnvelopeController::GetController().Play(m_pSignalSound, 0.0,
                                                   PITCH_NORM);
  }
}
...
void C_Portal_Dinosaur::ClientThink() {
  ...
  if (0) // m_hDinosaur_Signal.Get() && m_hDinosaur_Signal.Get()->m_nSignalID ==
         // 0 )
  {
...
void C_Portal_Dinosaur::ScanForSounds() {
  ...
  float flRadiusDelta = fabs(m_hDinosaur_Signal.Get()->m_flOuterRadius -
                             m_hDinosaur_Signal.Get()->m_flInnerRadius);
  float flDist =
      m_hDinosaur_Signal.Get()->GetAbsOrigin().DistTo(GetAbsOrigin());

  float flOuterBlend = RemapValClamped(
      flDist, m_hDinosaur_Signal.Get()->m_flOuterRadius,
      m_hDinosaur_Signal.Get()->m_flOuterRadius - (flRadiusDelta * 0.5f), 1.0f,
      0.0f);
  float flInnerBlend = RemapValClamped(
      flDist,
      m_hDinosaur_Signal.Get()->m_flOuterRadius - (flRadiusDelta * 0.5f),
  ...
  if (m_pNormalSound) {
    CSoundEnvelopeController::GetController().SoundChangeVolume(
        m_pNormalSound, flOuterBlend, 0.1f);
  }

  float flMidBlend = 0.0f;
  if (m_pStaticSound) {
    if (flOuterBlend <= 0.0f) {
      flMidBlend = RemapValClamped(
          flDist,
          m_hDinosaur_Signal.Get()->m_flOuterRadius - (flRadiusDelta * 0.5f),
    ...
    CSoundEnvelopeController::GetController().SoundChangeVolume(
        m_pStaticSound, flMidBlend, 0.1f);
  }

  if (m_pSignalSound) {
    CSoundEnvelopeController::GetController().SoundChangeVolume(
        m_pSignalSound, flInnerBlend, 0.1f);
  }
