...
float CPortalStaticProxy::ComputeStaticAmount(
    CPortalRenderable_FlatBasic *pFlatBasic) {
  ...
  if (pFlatBasic->WillUseDepthDoublerThisDraw()) {
    flStaticAmount = 0.0f;
  } else if (g_pPortalRender->GetRemainingPortalViewDepth() ==
             0) // end of the line, no more views
  {
    flStaticAmount = 1.0f;
  } else if ((g_pPortalRender->GetRemainingPortalViewDepth() == 1) &&
             (pFlatBasic->m_fSecondaryStaticAmount >
              flStaticAmount)) // fading in from no views to another view
                               // (player just walked through it)
  {
...
void CPortalStaticProxy::OnBind(void *pBind) {
  ...
  CPortalRenderable *pRecordedPortal =
      g_pPortalRender->FindRecordedPortal(pRenderable);

  if (pRecordedPortal) {
    CPortalRenderable_FlatBasic *pRecordedFlatBasic =
        dynamic_cast<CPortalRenderable_FlatBasic *>(pRecordedPortal);
    ...
    CPortalRenderable_FlatBasic *pFlatBasic =
        dynamic_cast<CPortalRenderable_FlatBasic *>(pRenderable);
    if (!pFlatBasic)
      return;
...
EXPOSE_INTERFACE(CPortalStaticProxy, IMaterialProxy,
                 "PortalStaticModel" IMATERIAL_PROXY_INTERFACE_VERSION);
...
EXPOSE_INTERFACE(CPortalStaticPortalProxy, IMaterialProxy,
                 "PortalStatic" IMATERIAL_PROXY_INTERFACE_VERSION);
...
EXPOSE_INTERFACE(CPortalOpenAmountProxy, IMaterialProxy,
                 "PortalOpenAmount" IMATERIAL_PROXY_INTERFACE_VERSION);
