...
IMPLEMENT_CLIENTCLASS_DT(C_NPC_RocketTurret, DT_NPC_RocketTurret,
                         CNPC_RocketTurret)

RecvPropInt(RECVINFO(m_iLaserState)), RecvPropInt(RECVINFO(m_nSiteHalo)),
...
void C_NPC_RocketTurret::LaserOn(void) {
  ...
  if (UTIL_Portal_TraceRay_Beam(rayPath, MASK_SHOT, &m_filterBeams,
                                &fEndFraction))
    vEndPoint =
        vecMuzzle +
        vecAimDir * ROCKET_TURRET_LASER_RANGE; // Trace went through portal and
                                               // endpoint is unknown
  else
