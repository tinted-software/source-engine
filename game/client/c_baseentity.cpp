...
int C_BaseEntity::GetTextureFrameIndex(void) { return m_iTextureFrameIndex; }
...
C_BaseEntity::C_BaseEntity()
  ...
  // CGameMovement::FinishDuck(), but that re-introduces a third-person hitching
  // bug.  One possible cause is the abrupt change in player size/position that
  // occurs when ducking, and how prediction tries to work through that.
  //
...
void C_BaseEntity::UpdateVisibility() {
#ifdef TF_CLIENT_DLL
  // TF prevents drawing of any entity attached to players that aren't items in
  // the inventory of the player. This is to prevent servers creating fake
  ...
  if (!engine->IsPlayingDemo()) {
    ...
    if (!m_bValidatedOwner) {
      ...
      // Check it the first time we call update visibility (Source TV doesn't
      // bother doing validation)
      m_bDeemedInvalid =
          engine->IsHLTV() ? false : !ValidateEntityAttachedToPlayer(bRetry);
    ...
    if (m_bDeemedInvalid) {
      ...
      if (bIsStaging) {
        ...
        SetRenderColor(m_PreviousRenderColor.r, m_PreviousRenderColor.g,
                       m_PreviousRenderColor.b, m_PreviousRenderColor.a);
      }
...
//-----------------------------------------------------------------------------
const Vector &C_BaseEntity::GetRenderOrigin(void) { return GetAbsOrigin(); }

const QAngle &C_BaseEntity::GetRenderAngles(void) { return GetAbsAngles(); }
...
// Output : CMouthInfo
//-----------------------------------------------------------------------------
CMouthInfo *C_BaseEntity::GetMouth(void) { return NULL; }
...
int C_BaseEntity::DrawModel(int flags) {
  ...
  case mod_studio:
    ...
    Warning("ERROR:  Can't draw studio model %s because %s is not derived from "
            "C_BaseAnimating\n",
  ...
  case mod_sprite:
    // drawn = DrawSprite();
    Warning("ERROR:  Sprite model's not supported any more except in legacy "
            "temp ents\n");
    break;
  default:
    break;
...
void C_BaseEntity::UnlinkFromHierarchy() {
  ...
  while (pChild) {
    if (pChild->m_pMoveParent != this) {
      Warning("C_BaseEntity::UnlinkFromHierarchy(): Entity has a child with "
              "the wrong parent!\n");
      Assert(0);
...
void C_BaseEntity::AddColoredStudioDecal(const Ray_t &ray, int hitbox,
  ...
  if (doTrace && (GetSolid() == SOLID_VPHYSICS) && !tr.startsolid &&
    ...
    modelrender->AddColoredDecal(m_ModelInstance, betterRay, up, decalIndex,
                                 GetStudioBody(), cColor, true, maxLODToDecal);
  } else {
    modelrender->AddColoredDecal(m_ModelInstance, ray, up, decalIndex,
                                 GetStudioBody(), cColor, false, maxLODToDecal);
  }
}
...
void C_BaseEntity::AddColoredDecal(const Vector &rayStart, const Vector &rayEnd,
  ...
  case mod_brush: {
    color32 cColor32 = {(uint8)cColor.r(), (uint8)cColor.g(), (uint8)cColor.b(),
                        (uint8)cColor.a()};
    effects->DecalColorShoot(decalIndex, index, model, GetAbsOrigin(),
                             GetAbsAngles(), decalCenter, 0, 0, cColor32);
  } break;
...
// TEAM HANDLING
//========================================================================================
C_Team *C_BaseEntity::GetTeam(void) { return GetGlobalTeam(m_iTeamNum); }
...
// Purpose:
//-----------------------------------------------------------------------------
int C_BaseEntity::GetRenderTeamNumber(void) { return GetTeamNumber(); }
...
//-----------------------------------------------------------------------------
bool C_BaseEntity::InLocalTeam(void) { return (GetTeam() == GetLocalTeam()); }
...
void C_BaseEntity::CalcAbsolutePosition() {
  ...
  if ((m_iEFlags & EFL_DIRTY_ABSTRANSFORM) ==
      0) // need second check in event another thread grabbed mutex and did the
         // calculation
  {
    return;
  }
...
const char *C_BaseEntity::GetDebugName(void) { return GetClassname(); }
...
//-----------------------------------------------------------------------------
C_BaseEntity *
C_BaseEntity::CreatePredictedEntityByName(const char *classname,
...
static ConCommand
    cl_removedecals("cl_removedecals", RemoveDecals_f,
...
int C_BaseEntity::SaveData(const char *context, int slot, int type) {
  ...
  char sz[64];
  sz[0] = 0;
  // don't build debug strings per entity per frame, unless we are watching the
  // entity
...
int C_BaseEntity::RestoreData(const char *context, int slot, int type) {
  ...
  // "unused" slot with the networked data and reporting all kinds of prediction
  // errors possibly.
...
void C_BaseEntity::OnPostRestoreData() {
  ...
  // pointer. (Mostly superseded by new modelindex delta check in RestoreData,
  // but I'm leaving it because it might be band-aiding any other missed calls
  // to SetModelByIndex --henryg)
...
// capabilities
//-----------------------------------------------------------------------------
int C_BaseEntity::ObjectCaps(void) { return 0; }
...
static ConCommand
    cl_find_ent_index("cl_find_ent_index", CC_CL_Find_Ent_Index,
                      "Display data for clientside entity matching specified "
