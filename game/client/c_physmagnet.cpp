...
public:
  ...
  bool GetShadowCastDirection(Vector *pDirection,
...
//-----------------------------------------------------------------------------
// Purpose: RecvProxy that converts the Magnet's attached object entindexes to
// handles
//-----------------------------------------------------------------------------
void RecvProxy_MagnetAttachedObjectList(const CRecvProxyData *pData,
                                        void *pStruct, void *pOut) {
  C_PhysMagnet *pMagnet = (C_PhysMagnet *)pOut;
  pMagnet->m_aAttachedObjectsFromServer[pData->m_iElement] =
      pData->m_Value.m_Int;
}

void RecvProxyArrayLength_MagnetAttachedArray(void *pStruct, int objectID,
...
RecvPropArray2(
        ...
        RecvPropInt( "magnetattached_array_element", 0, SIZEOF_IGNORE, 0,
RecvProxy_MagnetAttachedObjectList ), 128, 0, "magnetattached_array"
        )
*/
...
void C_PhysMagnet::PostDataUpdate(DataUpdateType_t updateType) {
  ...
  /*
  // First, detect any entities removed from the magnet and restore their
  shadows int iCount = m_aAttachedObjects.Count(); int iServerCount =
  m_aAttachedObjectsFromServer.Count(); for ( int i = 0; i < iCount; i++ )
  {
  ...
  {
          C_BaseEntity *pEntity = cl_entitylist->GetEnt(
  m_aAttachedObjectsFromServer[i] ); if ( m_aAttachedObjects.Find( pEntity ) ==
  m_aAttachedObjects.InvalidIndex() )
          {
...
//-----------------------------------------------------------------------------
bool C_PhysMagnet::GetShadowCastDirection(Vector *pDirection,
                                          ShadowType_t shadowType) const {
