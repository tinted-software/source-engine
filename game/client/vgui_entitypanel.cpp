...
void CEntityPanel::ComputeParent(void) {
  ...
  if (IsLocalPlayerInTactical() || !m_bShowInNormal) {
    CClientModeCommander *commander =
        (CClientModeCommander *)ClientModeCommander();
    Assert(commander);
...
void CEntityPanel::ComputeAndSetSize(void) {
  ...
  if (IsLocalPlayerInTactical()) {
    ...
    m_flScale =
        ...
        (0.25 *
         (1.0 -
          flZoom)); // 1/2 size at max zoomed out, full size by half zoomed in
  } else if (m_pBaseEntity) {
    // Get distance to entity
    float flDistance =
        (m_pBaseEntity->GetRenderOrigin() - MainViewOrigin()).Length();
    flDistance *= 2;
...
bool CEntityPanel::ShouldDraw() {
  return ((IsLocalPlayerInTactical() &&
           ClientModeCommander()->ShouldDrawEntity(m_pBaseEntity)) ||
