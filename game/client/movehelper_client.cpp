...
// Constructor
//-----------------------------------------------------------------------------
CMoveHelperClient::CMoveHelperClient(void) { SetSingleton(this); }
...
bool CMoveHelperClient::AddToTouched(const trace_t &tr,
...
void CMoveHelperClient::ProcessImpacts(void) {
  ...
  for (int i = 0; i < m_TouchList.Size(); i++) {
    ...
    C_BaseEntity *entity =
        ClientEntityList().GetEnt(m_TouchList[i].trace.m_pEnt->entindex());
    if (!entity)
      continue;
...
void CMoveHelperClient::StartSound(const Vector &origin,
...
void CMoveHelperClient::StartSound(const Vector &origin, int channel,
                                   char const *pSample, float volume,
...
void CMoveHelperClient::PlaybackEventFull(
...
bool CMoveHelperClient::PlayerFallingDamage(void) {
  // Do nothing; falling damage is applied in
  // MoveHelper_Server::PlayerFallingDamage.
  return (true);
}
