...
EXPOSE_INTERFACE(CBaseToggleTextureProxy, IMaterialProxy,
                 "ToggleTexture" IMATERIAL_PROXY_INTERFACE_VERSION);

//-----------------------------------------------------------------------------
// Constructor, destructor:
//-----------------------------------------------------------------------------

CBaseToggleTextureProxy::CBaseToggleTextureProxy() { Cleanup(); }
...
//-----------------------------------------------------------------------------
bool CBaseToggleTextureProxy::Init(IMaterial *pMaterial,
  ...
  char const *pTextureFrameNumVarName =
      pKeyValues->GetString("toggleTextureFrameNumVar");
  if (!pTextureFrameNumVarName)
    return false;

  m_TextureFrameNumVar =
      pMaterial->FindVar(pTextureFrameNumVarName, &foundVar, false);
  if (!foundVar)
    return false;
