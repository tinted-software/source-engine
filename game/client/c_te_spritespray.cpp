...
// Networking
//-----------------------------------------------------------------------------
IMPLEMENT_CLIENTCLASS_EVENT_DT(C_TESpriteSpray, DT_TESpriteSpray,
                               CTESpriteSpray)
RecvPropVector(RECVINFO(m_vecOrigin)), RecvPropVector(RECVINFO(m_vecDirection)),
    RecvPropInt(RECVINFO(m_nModelIndex)), RecvPropFloat(RECVINFO(m_fNoise)),
    RecvPropInt(RECVINFO(m_nCount)), RecvPropInt(RECVINFO(m_nSpeed)),
    END_RECV_TABLE()
...
// Recording
//-----------------------------------------------------------------------------
static inline void RecordSpriteSpray(const Vector &start,
                                     const Vector &direction, int nModelIndex,
                                     int nSpeed, float flNoise, int nCount) {
  if (!ToolsEnabled())
    return;

  if (clienttools->IsInRecordingMode()) {
    const model_t *pModel =
        (nModelIndex != 0) ? modelinfo->GetModel(nModelIndex) : NULL;
...
void C_TESpriteSpray::PostDataUpdate(DataUpdateType_t updateType) {
  ...
  tempents->Sprite_Spray(m_vecOrigin, m_vecDirection, m_nModelIndex, m_nCount,
                         m_nSpeed * 0.2, m_fNoise * 100.0);
  RecordSpriteSpray(m_vecOrigin, m_vecDirection, m_nModelIndex, m_nSpeed,
                    m_fNoise, m_nCount);
}
...
void TE_SpriteSpray(IRecipientFilter &filter, float delay, const Vector *pos,
                    ...
                    int count) {
  tempents->Sprite_Spray(*pos, *dir, modelindex, count, speed * 0.2,
                         noise * 100.0);
...
void TE_SpriteSpray(IRecipientFilter &filter, float delay,
  ...
  TE_SpriteSpray(filter, delay, &vecOrigin, &vecDirection, nModelIndex, nSpeed,
                 flNoise, nCount);
}
