...
void usrCmd_Start() { s_PerfVisualBenchmark.Start(); }

void usrCmd_Abort() { s_PerfVisualBenchmark.Stop(); }

static ConCommand perfvisualbenchmark("perfvisualbenchmark", usrCmd_Start);
static ConCommand perfvisualbenchmark_abort("perfvisualbenchmark_abort",
                                            usrCmd_Abort);
...
void CPerfVisualBenchmark::Stop() {
  ...
  //// unpause the mofo 	engine->ClientCmd_Unrestricted("wait");
...
void CPerfVisualBenchmark::Print() //  sort and print into console
{
  for (int i = 0; i < m_RunInfo.Count(); i++) {
    ...
    Msg("%.0f fps - %s\n", m_RunInfo[curMax].m_flFPS,
        m_RunInfo[curMax].m_pDescription);
