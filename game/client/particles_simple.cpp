...
CParticleEffect::~CParticleEffect(void) {
  ...
  // For some reason we'll get a callback into NotifyRemove() after being
  // deleted! Investigate dangling pointer
  m_Flags = 0;
...
void CParticleEffect::NotifyDestroyParticle(Particle *pParticle) {
  ...
  if (m_ParticleEffect.GetNumActiveParticles() == 0 && IsReleased() &&
      (m_Flags & FLAG_ALLOCATED) && !(m_Flags & FLAG_DONT_REMOVE)) {
    m_ParticleEffect.SetRemoveFlag();
  }
}
...
void CParticleEffect::SetSortOrigin(const Vector &vSortOrigin) {
  if (GetBinding().GetAutoUpdateBBox()) {
    ...
    // If not auto-updating bbox, don't change the bbox, just set the sort
    // origin.
    m_vSortOrigin = vSortOrigin;
  }
}
...
//-----------------------------------------------------------------------------
Particle *CParticleEffect::AddParticle(unsigned int particleSize,
  ...
  Particle *pParticle =
      (Particle *)m_ParticleEffect.AddParticle(particleSize, material);
...
SimpleParticle *CSimpleEmitter::AddSimpleParticle(PMaterialHandle hMaterial,
                                                  const Vector &vOrigin,
                                                  float flDieTime,
                                                  unsigned char uchSize) {
  SimpleParticle *pRet =
      (SimpleParticle *)AddParticle(sizeof(SimpleParticle), hMaterial, vOrigin);
  if (pRet) {
    pRet->m_Pos = vOrigin;
...
float CSimpleEmitter::UpdateScale(const SimpleParticle *pParticle) {
  return (float)pParticle->m_uchStartSize +
         ((float)pParticle->m_uchEndSize - (float)pParticle->m_uchStartSize) *
             (pParticle->m_flLifetime / pParticle->m_flDieTime);
}
...
void CSimpleEmitter::UpdateVelocity(SimpleParticle *pParticle,
                                    float timeDelta) {
...
void CSimpleEmitter::RenderParticles(CParticleRenderIterator *pIterator) {
  ...
  while (pParticle) {
    ...
    RenderParticle_ColorSizeAngle(
        pIterator->GetParticleDraw(), tPos, UpdateColor(pParticle),
...
//			timeDelta -
//-----------------------------------------------------------------------------
void CFireSmokeEffect::UpdateVelocity(SimpleParticle *pParticle,
                                      float timeDelta) {}
...
Vector CFireParticle::UpdateColor(const SimpleParticle *pParticle) {
  for (int i = 0; i < 3; i++) {
    // FIXME: This is frame dependant... but I don't want to store off start/end
    // colors yet pParticle->m_uchColor[i] = MAX( 0, pParticle->m_uchColor[i]-2
    // );
  }
