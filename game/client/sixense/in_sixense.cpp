...
static void SixenseSensitivityLevelChanged(IConVar *var, const char *pOldValue,
                                           float flOldValue);
static void SixenseConvarChanged(IConVar *var, const char *pOldValue,
                                 float flOldValue);
...
// sixense variables
ConVar sixense_weapon_select_sensitivity("sixense_weapon_select_sensitivity",
                                         "1.65", FCVAR_ARCHIVE);
ConVar sixense_crouch_sensitivity("sixense_crouch_sensitivity", "1.5",
                                  FCVAR_ARCHIVE);
ConVar sixense_jump_sensitivity("sixense_jump_sensitivity", "1.2",
                                FCVAR_ARCHIVE);
ConVar sixense_reload_sensitivity("sixense_reload_sensitivity", "1.5",
                                  FCVAR_ARCHIVE);

ConVar sixense_left_handed("sixense_left_handed", "0", FCVAR_ARCHIVE);

#ifdef PORTAL2
ConVar sixense_scale_disables_jump_timer("sixense_scale_disables_jump_timer",
                                         "1.0");
ConVar sixense_auto_one_to_one_enabled("sixense_auto_one_to_one_enabled", "0",
                                       FCVAR_ARCHIVE);
ConVar sixense_auto_one_to_one_center_z_bias(
    "sixense_auto_one_to_one_center_z_bias", "0", FCVAR_ARCHIVE);
ConVar sixense_large_objects_lock_one_to_one(
    "sixense_large_objects_lock_one_to_one", "0", FCVAR_ARCHIVE);
ConVar sixense_large_objects_lock_one_to_one_size(
    "sixense_large_objects_lock_one_to_one_size", "3.0", FCVAR_ARCHIVE);
ConVar sixense_dist_one_to_one_enabled("sixense_dist_one_to_one_enabled", "1",
                                       FCVAR_ARCHIVE);
ConVar sixense_dist_one_to_one_dist("sixense_dist_one_to_one_dist", "65",
                                    FCVAR_ARCHIVE);
ConVar sixense_dist_one_to_one_end_ratchet_delay(
    "sixense_dist_one_to_one_end_ratchet_delay", "0.05", FCVAR_ARCHIVE);
ConVar sixense_pickup_momentary_time("sixense_pickup_momentary_time", "45",
                                     FCVAR_ARCHIVE);
ConVar sixense_auto_one_to_one_start_vel("sixense_auto_one_to_one_start_vel",
                                         "550.0f", FCVAR_ARCHIVE);
ConVar
    sixense_auto_one_to_one_start_accel("sixense_auto_one_to_one_start_accel",
                                        "2500.0f", FCVAR_ARCHIVE);
ConVar sixense_auto_one_to_one_start_accel_timer(
    "sixense_auto_one_to_one_start_accel_timer", "800", FCVAR_ARCHIVE);
ConVar
    sixense_auto_one_to_one_angle_thresh("sixense_auto_one_to_one_angle_thresh",
                                         "0.5235f", FCVAR_ARCHIVE);
ConVar
    sixense_auto_one_to_one_stop_xy_dist("sixense_auto_one_to_one_stop_xy_dist",
                                         "100", FCVAR_ARCHIVE);
ConVar
    sixense_auto_one_to_one_stop_z_dist("sixense_auto_one_to_one_stop_z_dist",
                                        "10.0f", FCVAR_ARCHIVE);
ConVar
    sixense_hold_spin_start_screen_ratio("sixense_hold_spin_start_screen_ratio",
                                         "0.5", FCVAR_ARCHIVE);
ConVar sixense_hold_spin_speed("sixense_hold_spin_speed", "1.0", FCVAR_ARCHIVE);
ConVar sixense_hold_spin_speed_bias("sixense_hold_spin_speed_bias", "0.2",
                                    FCVAR_ARCHIVE);
ConVar sixense_hold_spin_fade_min_dist("sixense_hold_spin_fade_min_dist", "0.0",
                                       FCVAR_ARCHIVE);
ConVar sixense_hold_spin_fade_max_dist("sixense_hold_spin_fade_max_dist", "0.0",
                                       FCVAR_ARCHIVE);
ConVar
    sixense_hold_spin_damp("sixense_hold_spin_damp", "0.95", FCVAR_ARCHIVE,
...
ConVar sixense_hold_spin_max_angle(
    ...
    "disallow camera tracking if the held object is greater than this angle "
    "relative to the players view");
ConVar sixense_hold_slide_z_min_dist("sixense_hold_slide_z_min_dist", "20.0f",
                                     FCVAR_ARCHIVE);
ConVar sixense_hold_slide_xy_radius("sixense_hold_slide_xy_radius", "80.0f",
                                    FCVAR_ARCHIVE);
ConVar sixense_played_tutorial("sixense_played_tutorial", "0", FCVAR_ARCHIVE);
#endif

// 0 = low, 1 = med, 2 = high, 3 = custom
ConVar sixense_sensitivity_level("sixense_sensitivity_level", "-1",
                                 FCVAR_ARCHIVE);
ConVar sixense_controller_angle_mode("sixense_controller_angle_mode", "0.0f",
                                     FCVAR_ARCHIVE);
ConVar sixense_roll_correct_blend("sixense_roll_correct_blend", "0.965f",
                                  FCVAR_ARCHIVE);

ConVar sixense_exit_one_to_one_dot("sixense_exit_one_to_one_dot", "0.85",
                                   FCVAR_ARCHIVE);
ConVar sixense_exit_metroid_blend("sixense_exit_metroid_blend", "0.95f",
                                  FCVAR_ARCHIVE);

ConVar sixense_max_charge_spin("sixense_max_charge_spin", "3.0f",
                               FCVAR_ARCHIVE);

ConVar sixense_zoom_momentary_time("sixense_zoom_momentary_time", "500",
                                   FCVAR_ARCHIVE);
...
ConVar sixense_trigger_threshold("sixense_trigger_threshold", "0.05",
                                 FCVAR_ARCHIVE);

ConVar
    sixense_tilt_gesture_angle_threshold("sixense_tilt_gesture_angle_threshold",
                                         "35.0", FCVAR_ARCHIVE);
ConVar sixense_point_gesture_angle_threshold(
    "sixense_point_gesture_angle_threshold", "15.0", FCVAR_ARCHIVE);

ConVar sixense_mouse_enabled("sixense_mouse_enabled", "1.0", FCVAR_ARCHIVE);
ConVar sixense_mouse_sensitivity("sixense_mouse_sensitivity", "1.0",
                                 FCVAR_ARCHIVE);

ConVar sixense_spring_view_enabled("sixense_spring_view_enabled", "1.0f",
                                   FCVAR_ARCHIVE);
ConVar sixense_spring_view_min_spring("sixense_spring_view_min_spring",
                                      "0.025f", FCVAR_ARCHIVE);
ConVar sixense_spring_view_max_spring("sixense_spring_view_max_spring",
                                      "0.9999f", FCVAR_ARCHIVE);
ConVar sixense_spring_view_min_angle("sixense_spring_view_min_angle", "1.0f",
                                     FCVAR_ARCHIVE);
ConVar sixense_spring_view_max_angle("sixense_spring_view_max_angle", "45.0f",
                                     FCVAR_ARCHIVE);

// mode 0:
ConVar sixense_aim_freeaim_heading_multiplier(
    "sixense_aim_freeaim_heading_multiplier", "1.5", FCVAR_ARCHIVE);
ConVar
    sixense_aim_freeaim_pitch_multiplier("sixense_aim_freeaim_pitch_multiplier",
                                         "1.5", FCVAR_ARCHIVE);
ConVar
    sixense_aim_freeaim_dead_zone_radius("sixense_aim_freeaim_dead_zone_radius",
                                         "0.25", FCVAR_ARCHIVE);
ConVar
    sixense_aim_freeaim_accel_band_size("sixense_aim_freeaim_accel_band_size",
                                        "20.0", FCVAR_ARCHIVE);
ConVar sixense_aim_freeaim_max_speed("sixense_aim_freeaim_max_speed", "7.0",
                                     FCVAR_ARCHIVE);
ConVar
    sixense_aim_freeaim_auto_level_rate("sixense_aim_freeaim_auto_level_rate",
                                        "1.0", FCVAR_ARCHIVE);
ConVar sixense_aim_freeaim_accel_band_exponent(
    "sixense_aim_freeaim_accel_band_exponent", "1.0", FCVAR_ARCHIVE);
ConVar sixense_aim_freeaim_switch_blend_time_enter(
    "sixense_aim_freeaim_switch_blend_time_enter", "0.5", FCVAR_CHEAT);
ConVar sixense_aim_freeaim_switch_blend_time_exit(
    "sixense_aim_freeaim_switch_blend_time_exit", "0.25", FCVAR_ARCHIVE);
ConVar
    sixense_teleport_metroid_blend_time("sixense_teleport_metroid_blend_time",
                                        "3.0", FCVAR_CHEAT);
ConVar
    sixense_teleport_wait_to_blend_time("sixense_teleport_wait_to_blend_time",
                                        "0.75", FCVAR_CHEAT);

// mode 1:
ConVar sixense_aim_1to1_ratchet_vertical("sixense_aim_1to1_ratchet_vertical",
                                         "1.0", FCVAR_ARCHIVE);
ConVar
    sixense_aim_1to1_heading_multiplier("sixense_aim_1to1_heading_multiplier",
                                        "3.0", FCVAR_ARCHIVE);
ConVar sixense_aim_1to1_pitch_multiplier("sixense_aim_1to1_pitch_multiplier",
                                         "2.0", FCVAR_ARCHIVE);
ConVar sixense_feet_angles_offset_stick_spin_horiz_multiplier(
    "sixense_feet_angles_offset_stick_spin_horiz_multiplier", "4.0",
    FCVAR_ARCHIVE);
ConVar sixense_feet_angles_offset_stick_spin_vert_multiplier(
    "sixense_feet_angles_offset_stick_spin_vert_multiplier", "2.0",
    FCVAR_ARCHIVE);
ConVar sixense_feet_angles_offset_stick_spin_invert_pitch(
    "sixense_feet_angles_offset_stick_spin_invert_pitch", "1.0", FCVAR_ARCHIVE);
ConVar sixense_feet_angles_offset_stick_spin_exponent(
    "sixense_feet_angles_offset_stick_spin_exponent", "1.0", FCVAR_ARCHIVE);

ConVar
    sixense_aim_scope_heading_multiplier("sixense_aim_scope_heading_multiplier",
                                         "0.6", FCVAR_ARCHIVE);
ConVar sixense_aim_scope_pitch_multiplier("sixense_aim_scope_pitch_multiplier",
                                          "0.6", FCVAR_ARCHIVE);

ConVar sixense_melee_pitch_blend_val("sixense_melee_pitch_blend_val", "0.99",
                                     FCVAR_ARCHIVE);

ConVar sixense_crosshair_horiz_multiplier("sixense_crosshair_horiz_multiplier",
                                          "1.0", FCVAR_ARCHIVE);
ConVar sixense_crosshair_vert_multiplier("sixense_crosshair_vert_multiplier",
                                         "1.0", FCVAR_ARCHIVE);
ConVar sixense_always_draw_crosshair("sixense_always_draw_crosshair", "1",
                                     FCVAR_ARCHIVE);

// walking
ConVar sixense_walking_dead_zone_percent("sixense_walking_dead_zone_percent",
                                         "10.0", FCVAR_ARCHIVE);
ConVar sixense_walking_exponent("sixense_walking_exponent", "1.0",
                                FCVAR_ARCHIVE);
...
// A bunch of our convars have their values pushed down into the sixense_utils
// lib, so install a handler on each to let us know if there are any changes
void SixenseInput::InstallConvarCallbacks() {
  ...
  sixense_aim_freeaim_heading_multiplier.InstallChangeCallback(
      SixenseConvarChanged);
  sixense_aim_freeaim_pitch_multiplier.InstallChangeCallback(
      SixenseConvarChanged);
  sixense_aim_freeaim_dead_zone_radius.InstallChangeCallback(
      SixenseConvarChanged);
  sixense_aim_freeaim_accel_band_size.InstallChangeCallback(
      SixenseConvarChanged);
  sixense_aim_freeaim_auto_level_rate.InstallChangeCallback(
      SixenseConvarChanged);
  sixense_aim_freeaim_accel_band_exponent.InstallChangeCallback(
      SixenseConvarChanged);

#ifdef PORTAL2
  sixense_auto_one_to_one_start_vel.InstallChangeCallback(SixenseConvarChanged);
  sixense_auto_one_to_one_start_accel.InstallChangeCallback(
      SixenseConvarChanged);
  sixense_auto_one_to_one_start_accel_timer.InstallChangeCallback(
      SixenseConvarChanged);
  sixense_auto_one_to_one_angle_thresh.InstallChangeCallback(
      SixenseConvarChanged);
  sixense_auto_one_to_one_stop_xy_dist.InstallChangeCallback(
      SixenseConvarChanged);
  sixense_auto_one_to_one_stop_z_dist.InstallChangeCallback(
      SixenseConvarChanged);
#endif

  sixense_aim_freeaim_switch_blend_time_exit.InstallChangeCallback(
      SixenseConvarChanged);
  ...
  sixense_aim_1to1_heading_multiplier.InstallChangeCallback(
      SixenseConvarChanged);
  ...
  sixense_feet_angles_offset_stick_spin_horiz_multiplier.InstallChangeCallback(
      SixenseConvarChanged);
  sixense_feet_angles_offset_stick_spin_vert_multiplier.InstallChangeCallback(
      SixenseConvarChanged);
  sixense_feet_angles_offset_stick_spin_invert_pitch.InstallChangeCallback(
      SixenseConvarChanged);
  sixense_feet_angles_offset_stick_spin_exponent.InstallChangeCallback(
      SixenseConvarChanged);
  ...
  sixense_tilt_gesture_angle_threshold.InstallChangeCallback(
      SixenseConvarChanged);
  sixense_point_gesture_angle_threshold.InstallChangeCallback(
      SixenseConvarChanged);

  sixense_trigger_threshold.InstallChangeCallback(SixenseConvarChanged);

  sixense_aim_scope_heading_multiplier.InstallChangeCallback(
      SixenseConvarChanged);
}

// If any of the interesting convars changed this frame, this call will push all
// the values to where they need to be.
void SixenseInput::UpdateValuesFromConvars() {
  ...
  m_pFPSEvents->setParameter(
      sixenseUtils::IFPSEvents::WEAPON_SELECT_SENSITIVITY,
      sixense_weapon_select_sensitivity.GetFloat());
  m_pFPSEvents->setParameter(sixenseUtils::IFPSEvents::CROUCH_SENSITIVITY,
                             sixense_crouch_sensitivity.GetFloat());
  m_pFPSEvents->setParameter(sixenseUtils::IFPSEvents::JUMP_SENSITIVITY,
                             sixense_jump_sensitivity.GetFloat());
  m_pFPSEvents->setParameter(sixenseUtils::IFPSEvents::RELOAD_SENSITIVITY,
                             sixense_reload_sensitivity.GetFloat());
  m_pFPSEvents->setParameter(sixenseUtils::IFPSEvents::CONTROLLER_ANGLE_MODE,
                             sixense_controller_angle_mode.GetFloat());

#ifdef PORTAL2
  m_pFPSEvents->setParameter(
      sixenseUtils::IFPSEvents::AUTO_ONE_TO_ONE_START_VEL,
      sixense_auto_one_to_one_start_vel.GetFloat());
  m_pFPSEvents->setParameter(
      sixenseUtils::IFPSEvents::AUTO_ONE_TO_ONE_START_ACCEL,
      sixense_auto_one_to_one_start_accel.GetFloat());
  m_pFPSEvents->setParameter(
      sixenseUtils::IFPSEvents::AUTO_ONE_TO_ONE_START_ACCEL_TIMER,
      sixense_auto_one_to_one_start_accel_timer.GetFloat());
  m_pFPSEvents->setParameter(
      sixenseUtils::IFPSEvents::AUTO_ONE_TO_ONE_START_ANGLE_THRESH,
      sixense_auto_one_to_one_angle_thresh.GetFloat());
  m_pFPSEvents->setParameter(
      sixenseUtils::IFPSEvents::AUTO_ONE_TO_ONE_STOP_XY_DIST,
      sixense_auto_one_to_one_stop_xy_dist.GetFloat());
  m_pFPSEvents->setParameter(
      sixenseUtils::IFPSEvents::AUTO_ONE_TO_ONE_STOP_Z_DIST,
      sixense_auto_one_to_one_stop_z_dist.GetFloat());
#endif

  m_pFPSViewAngles->setParameter(
      sixenseUtils::IFPSViewAngles::AIM_METROID_HEADING_MULTIPLIER,
      sixense_aim_freeaim_heading_multiplier.GetFloat());
  m_pFPSViewAngles->setParameter(
      sixenseUtils::IFPSViewAngles::AIM_METROID_PITCH_MULTIPLIER,
      sixense_aim_freeaim_pitch_multiplier.GetFloat());
  m_pFPSViewAngles->setParameter(
      sixenseUtils::IFPSViewAngles::AIM_METROID_DEAD_ZONE_RADIUS,
      sixense_aim_freeaim_dead_zone_radius.GetFloat());
  m_pFPSViewAngles->setParameter(
      sixenseUtils::IFPSViewAngles::AIM_METROID_ACCEL_BAND_SIZE,
      sixense_aim_freeaim_accel_band_size.GetFloat());
  m_pFPSViewAngles->setParameter(
      sixenseUtils::IFPSViewAngles::AIM_METROID_AUTO_LEVEL_RATE,
      sixense_aim_freeaim_auto_level_rate.GetFloat());
  m_pFPSViewAngles->setParameter(
      sixenseUtils::IFPSViewAngles::AIM_METROID_ACCEL_BAND_EXPONENT,
      sixense_aim_freeaim_accel_band_exponent.GetFloat());

  m_pFPSViewAngles->setParameter(
      sixenseUtils::IFPSViewAngles::AIM_METROID_SWITCH_BLEND_TIME_EXIT,
      sixense_aim_freeaim_switch_blend_time_exit.GetFloat());
  m_pFPSViewAngles->setParameter(
      sixenseUtils::IFPSViewAngles::CONTROLLER_ANGLE_MODE,
      sixense_controller_angle_mode.GetFloat());
  m_pFPSViewAngles->setParameter(
      sixenseUtils::IFPSViewAngles::ROLL_CORRECTION_BLEND,
      sixense_roll_correct_blend.GetFloat());
  m_pFPSViewAngles->setParameter(
      sixenseUtils::IFPSViewAngles::EXIT_METROID_BLEND,
      sixense_exit_metroid_blend.GetFloat());

  m_pFPSViewAngles->setParameter(
      sixenseUtils::IFPSViewAngles::SPRING_VIEW_MIN_SPRING,
      sixense_spring_view_min_spring.GetFloat());
  m_pFPSViewAngles->setParameter(
      sixenseUtils::IFPSViewAngles::SPRING_VIEW_MAX_SPRING,
      sixense_spring_view_max_spring.GetFloat());
  m_pFPSViewAngles->setParameter(
      sixenseUtils::IFPSViewAngles::SPRING_VIEW_MIN_ANGLE,
      sixense_spring_view_min_angle.GetFloat());
  m_pFPSViewAngles->setParameter(
      sixenseUtils::IFPSViewAngles::SPRING_VIEW_MAX_ANGLE,
      sixense_spring_view_max_angle.GetFloat());
  m_pFPSViewAngles->setParameter(
      sixenseUtils::IFPSViewAngles::PITCH_CHANGE_BLEND_VAL,
      sixense_melee_pitch_blend_val.GetFloat());

  m_pFPSViewAngles->setParameter(
      sixenseUtils::IFPSViewAngles::AIM_1TO1_RATCHET_VERTICAL,
      sixense_aim_1to1_ratchet_vertical.GetFloat());

  // In metroid mode, we switch into mouselook when looking down scope, so
  // configure mouselook differently.
  if (sixense_mode.GetInt() == 1) {
    m_pFPSViewAngles->setParameter(
        sixenseUtils::IFPSViewAngles::AIM_1TO1_HEADING_MULTIPLIER,
        sixense_aim_1to1_heading_multiplier.GetFloat());
    m_pFPSViewAngles->setParameter(
        sixenseUtils::IFPSViewAngles::AIM_1TO1_PITCH_MULTIPLIER,
        sixense_aim_1to1_pitch_multiplier.GetFloat());
    m_pFPSViewAngles->setParameter(
        sixenseUtils::IFPSViewAngles::SPRING_VIEW_ENABLED,
        sixense_spring_view_enabled.GetFloat());
  } else if (sixense_mode.GetInt() == 0) {

    // Use scope sensitivites in metroid mode, so when we look down the scope
    // it's set correctly
    m_pFPSViewAngles->setParameter(
        sixenseUtils::IFPSViewAngles::AIM_1TO1_HEADING_MULTIPLIER,
    ...
    m_pFPSViewAngles->setParameter(
        sixenseUtils::IFPSViewAngles::SPRING_VIEW_ENABLED, 0);
  }

  m_pFPSViewAngles->setParameter(
      sixenseUtils::IFPSViewAngles::
  ...
  m_pFPSViewAngles->setParameter(
      sixenseUtils::IFPSViewAngles::FEET_ANGLES_OFFSET_STICK_SPIN_INVERT_PITCH,
      sixense_feet_angles_offset_stick_spin_invert_pitch.GetFloat());
  m_pFPSViewAngles->setParameter(
      sixenseUtils::IFPSViewAngles::FEET_ANGLES_OFFSET_STICK_SPIN_EXPONENT,
      sixense_feet_angles_offset_stick_spin_exponent.GetFloat());

  m_pFPSPlayerMovement->setParameter(
      sixenseUtils::IFPSPlayerMovement::DEAD_ZONE_PERCENT,
      sixense_walking_dead_zone_percent.GetFloat());
  m_pFPSPlayerMovement->setParameter(
      sixenseUtils::IFPSPlayerMovement::EXPONENTIAL,
      sixense_walking_exponent.GetFloat());

  m_pLeftButtonStates->setAbsoluteTiltAngleThresholdInDeg(
      sixense_tilt_gesture_angle_threshold.GetFloat());
  m_pLeftButtonStates->setRelativeTiltAngleThresholdInDeg(
      sixense_point_gesture_angle_threshold.GetFloat());

  m_pLeftButtonStates->setTriggerThreshold(
      sixense_trigger_threshold.GetFloat());
  m_pRightButtonStates->setTriggerThreshold(
      sixense_trigger_threshold.GetFloat());
}
...
bool SixenseInput::LoadModules() {
  ...
  if (g_pSixenseModule) {
    ...
    if (factory) {
      m_pSixenseAPI =
          reinterpret_cast<ISixenseAPI *>(factory("SixenseAPI", NULL));
  ...
  if (g_pSixenseUtilsModule) {
    ...
    if (factory) {
      ...
      m_pFPSViewAngles = reinterpret_cast<sixenseUtils::IFPSViewAngles *>(
          factory("FPSViewAngles0", NULL));
      m_pFPSPlayerMovement =
          reinterpret_cast<sixenseUtils::IFPSPlayerMovement *>(
              factory("FPSPlayerMovement0", NULL));
      m_pFPSEvents = reinterpret_cast<sixenseUtils::IFPSEvents *>(
          factory("FPSEvents0", NULL));

      m_pLaserPointer = reinterpret_cast<sixenseUtils::ILaserPointer *>(
          factory("LaserPointer0", NULL));

      m_pLeftDeriv = reinterpret_cast<sixenseUtils::IDerivatives *>(
          factory("Derivatives0", NULL));
      m_pRightDeriv = reinterpret_cast<sixenseUtils::IDerivatives *>(
          factory("Derivatives1", NULL));

      m_pLeftButtonStates = reinterpret_cast<sixenseUtils::IButtonStates *>(
          factory("ButtonStates0", NULL));
      m_pRightButtonStates = reinterpret_cast<sixenseUtils::IButtonStates *>(
          factory("ButtonStates1", NULL));

      m_pControllerManager =
          reinterpret_cast<sixenseUtils::IControllerManager *>(
              factory("ControllerManager", NULL));

      if (m_pFPSViewAngles && m_pFPSPlayerMovement && m_pFPSEvents &&
          m_pLaserPointer && m_pLeftDeriv && m_pRightDeriv &&
          m_pLeftButtonStates && m_pRightButtonStates && m_pControllerManager) {
        found_objects = true;
      }
    }
  }
...
static void controller_manager_setup_callback(
    sixenseUtils::IControllerManager::setup_step SetupStep) {
...
static void reset_view(const CCommand &args) {
  ...
  if (args.ArgC() != 4) {
    Warning("Incorrect parameters. Format: sixense_reset_view <pitch> <yaw> "
            "<roll>\n");
    return;
  }
...
static void SixenseAutosave(const CCommand &args) {
  if (sixense_enabled.GetInt()) {
    ...
    engine->SaveGame(szSaveName, IsX360(), szFullSaveFileName,
                     sizeof(szFullSaveFileName), szComment, sizeof(szComment));
  }
}
...
void SixenseConvarChanged(IConVar *var, const char *pOldValue,
                          float flOldValue) {
  if (g_pSixenseInput) {
...
void SixenseInput::ConvarChanged() { m_bConvarChanged = true; }
...
SixenseInput::SixenseInput() {
  ...
  sixense_sensitivity_level.InstallChangeCallback(
      SixenseSensitivityLevelChanged);
  ...
  // For keeping track of the previous mode when looking down the scope changes
  // it.
...
void SixenseInput::FireGameEvent(IGameEvent *pEvent) {
  ...
  if (Q_strcmp(type, "sixense_player_teleported") == 0) {
    ...
    if (pPlayer && pPlayer->GetUserID() == playerId) {
      ...
      m_fTeleportWaitToBlendTime =
          gpGlobals->curtime + sixense_teleport_wait_to_blend_time.GetFloat();
    }
  }
...
void SixenseInput::PlayerDroppedEntity(int entityID) {
  if (UTIL_IsScaledCube(cl_entitylist->GetBaseEntity(entityID)) &&
      !m_bIsIn1to1Mode && m_pFPSViewAngles &&
      (m_pFPSViewAngles->getParameter(
           sixenseUtils::IFPSViewAngles::AIM_METROID_MAX_SPEED) <=
...
void SixenseInput::PlayerPortalled(const VMatrix &PortalMatrix) {
  ...
  sixenseMath::Vector3 ss_view_angles =
      g_pSixenseInput->m_pFPSViewAngles->getViewAngles();
  ...
  UTIL_Portal_AngleTransform(PortalMatrix, new_viewangles,
                             corrected_viewangles);

  // Msg("Fixed local view angles %f %f %f\n", corrected_viewangles[YAW],
  // corrected_viewangles[PITCH], corrected_viewangles[ROLL] );
...
void SixenseInput::SetPortalTweakingParameters(bool bIsTweaking) {
  ...
  if (m_bIsTweaking != bIsTweaking) {
    ...
    if (m_bIsTweaking) {
      ...
      m_fTweakSixenseAimFreeaimAccelBandExponent =
          sixense_aim_freeaim_accel_band_exponent.GetFloat();
      m_fTweakSixenseAimFreeaimAutoLevelRate =
          sixense_aim_freeaim_auto_level_rate.GetFloat();
      m_fTweakSixenseAimFreeaimAccelBandSize =
          sixense_aim_freeaim_accel_band_size.GetFloat();
      m_fTweakSixenseAimFreeaimMaxSpeed =
          sixense_aim_freeaim_max_speed.GetFloat();
      m_fTweakSixenseAimFreeaimDeadZoneRadius =
          sixense_aim_freeaim_dead_zone_radius.GetFloat();
      m_fTweakSixenseAimFreeaimHeadingMultiplier =
          sixense_aim_freeaim_heading_multiplier.GetFloat();
      m_fTweakSixenseAimFreeaimPitchMultiplier =
          sixense_aim_freeaim_pitch_multiplier.GetFloat();
      m_fTweakSixenseAim1to1HeadingMultiplier =
          sixense_aim_1to1_heading_multiplier.GetFloat();
      m_fTweakSixenseAim1to1PitchMultiplier =
          sixense_aim_1to1_pitch_multiplier.GetFloat();
      ...
      // restore values
      sixense_aim_freeaim_accel_band_exponent.SetValue(
          m_fTweakSixenseAimFreeaimAccelBandExponent);
      sixense_aim_freeaim_auto_level_rate.SetValue(
          m_fTweakSixenseAimFreeaimAutoLevelRate);
      sixense_aim_freeaim_accel_band_size.SetValue(
          m_fTweakSixenseAimFreeaimAccelBandSize);
      sixense_aim_freeaim_max_speed.SetValue(m_fTweakSixenseAimFreeaimMaxSpeed);
      sixense_aim_freeaim_dead_zone_radius.SetValue(
          m_fTweakSixenseAimFreeaimDeadZoneRadius);
      sixense_aim_freeaim_heading_multiplier.SetValue(
          m_fTweakSixenseAimFreeaimHeadingMultiplier);
      sixense_aim_freeaim_pitch_multiplier.SetValue(
          m_fTweakSixenseAimFreeaimPitchMultiplier);
      sixense_aim_1to1_heading_multiplier.SetValue(
          m_fTweakSixenseAim1to1HeadingMultiplier);
      sixense_aim_1to1_pitch_multiplier.SetValue(
          m_fTweakSixenseAim1to1PitchMultiplier);

      // force blend back to original mode
      if (m_pFPSViewAngles->getParameter(
              sixenseUtils::IFPSViewAngles::AIM_METROID_MAX_SPEED) <=
          sixense_aim_freeaim_max_speed.GetFloat()) {
        m_pFPSViewAngles->forceMetroidBlend(
            m_pFPSViewAngles->getParameter(
...
void SixenseInput::SetOneToOneMode(bool bOnOrOff) {
  ...
  if (bOnOrOff) {
    ...
    // Start glowing
    m_nGlowIndex = g_GlowObjectManager.RegisterGlowObject(
        GetHeldObject(), glowColor, 0.25f, GET_ACTIVE_SPLITSCREEN_SLOT());
  ...
  m_pFPSViewAngles->update(&m_pACD->controllers[m_nLeftIndex],
                           &m_pACD->controllers[m_nRightIndex]);
}
...
// Turns sixense support on and off. Note this is different than m_bIsActive,
// which gets set when the controllers are not in the dock.
    ...
    if (m_bPlayerValid) {
      ...
      if (pPlayer) {
        ...
        pPlayer->SetEyeAngleOffset(
            ident); // This is the way the player is looking, and the
                    // orientation of the weapon model
      }
    }
...
void SixenseInput::ResetView(QAngle SpawnAngles) {
  ...
  Vector3 ss_spawn_angles(SpawnAngles[YAW], SpawnAngles[PITCH],
                          SpawnAngles[ROLL]);

  m_pFPSViewAngles->forceViewAngles(m_pFPSViewAngles->getMode(),
                                    ss_spawn_angles);
}
...
void SixenseInput::Init() {
  ...
  m_pControllerManager->setGameType(
      sixenseUtils::IControllerManager::ONE_PLAYER_TWO_CONTROLLER);
  m_pControllerManager->registerSetupCallback(
...
void SixenseInput::PostInit() {
  ...
  if (m_pGestureBindings->GetNumBindings() == 0) {
    // Try to create the default sixense bindings file if it doesn't already
    // exist\n");
...
bool SixenseInput::IsEnabled() { return m_bIsEnabled && m_bIsActive; }

bool SixenseInput::IsLeftHanded() {
  return sixense_left_handed.GetInt() == 0 ? false : true;
}

void SixenseInput::SetBaseOffset() { m_bShouldSetBaseOffset = true; }

void SixenseInput::GetFOV(float *hfov, float *vfov) {

#if (defined(HL2_CLIENT_DLL) || defined(TF_CLIENT_DLL) ||                      \
     defined(CSTRIKE_DLL)) &&                                                  \
    !defined(CSTRIKE15) && !defined(TERROR)
  ...
  float engineAspectRatio =
      engine->GetScreenAspectRatio(ScreenWidth(), ScreenHeight());
#endif
...
void SixenseInput::SetMode(int nNewMode) {
  ...
  sixenseUtils::IFPSViewAngles::fps_mode mode =
      sixenseUtils::IFPSViewAngles::FREE_AIM_TWO_CONTROLLER;
...
bool SixenseInput::InMenuMode() {
  ...
  char *panel_names[] = {
      PANEL_OVERVIEW,       PANEL_CLASS,        PANEL_TEAM,
  ...
  for (int i = 0; i < num_panels; i++) {
    ...
    IViewPortPanel *panel =
        GetViewPortInterface()->FindPanelByName(panel_names[i]);
#else
...
#if defined(TF_CLIENT_DLL)
  CTFPlayer *pTFPlayer =
      dynamic_cast<CTFPlayer *>(C_BasePlayer::GetLocalPlayer());
...
#endif
      (SixenseInput::m_SixenseFrame &&
       SixenseInput::m_SixenseFrame->IsVisible()) ||
      engine->IsPaused() || (enginevgui && enginevgui->IsGameUIVisible()) ||
...
bool SixenseInput::SixenseFrame(float flFrametime, CUserCmd *pCmd) {
  ...
  if (pLocalPlayer) {
    if (!m_bIsEnabled) {
      ...
      pLocalPlayer->SetSixenseFlags(pLocalPlayer->GetSixenseFlags() &
                                    ~CBasePlayer::PLAYER_SIXENSE_ENABLED);
      return false;
    } else {
  ...
  if (m_pACD->controllers[0].enabled && m_pACD->controllers[1].enabled) {
    ...
    bool controllers_docked = m_pACD->controllers[0].is_docked ||
                              m_pACD->controllers[1].is_docked ||
    ...
    if (controllers_docked && m_bIsActive) {
      ...
      // If we're unpausing, flip the mode around so the metroid rotation is
      // blended
      sixenseUtils::IFPSViewAngles::fps_mode cur_mode =
          m_pFPSViewAngles->getMode();
      m_pFPSViewAngles->setMode(sixenseUtils::IFPSViewAngles::DUAL_ANALOG);
      m_pFPSViewAngles->setMode(cur_mode);
    }
  }
  ...
  if (!sixense_left_handed.GetInt()) {
    m_nLeftIndex =
        m_pControllerManager->getIndex(sixenseUtils::IControllerManager::P1L);
    m_nRightIndex =
        m_pControllerManager->getIndex(sixenseUtils::IControllerManager::P1R);
  } else {
    m_nLeftIndex =
        m_pControllerManager->getIndex(sixenseUtils::IControllerManager::P1R);
    m_nRightIndex =
  ...
  // If the controllers are docked we are inactive, return until they are picked
  // up
  if (!m_bIsActive)
    return false;
  ...
  m_pGestureBindings->UpdateBindings(m_pLeftButtonStates, m_pRightButtonStates,
                                     AreBindingsDisabled());
...
#ifdef SIXENSE_PLAYER_DATA
  if (!engine->IsPaused() && !(enginevgui && enginevgui->IsGameUIVisible()) &&
      sixense_features_enabled.GetInt())
  ...
  if (m_nFreeaimSpinDisabled > 0) {
    float fCurrentSpeed = m_pFPSViewAngles->getParameter(
        sixenseUtils::IFPSViewAngles::AIM_METROID_MAX_SPEED);
    ...
    if (pScaledCube) {
      ...
      float fSpinMult =
          1.0 - sqrt(clamp((fMaxScale - 1.0f) /
                               (sixense_scaling_max.GetFloat() - 1.0f),
                           0.0f, 1.0f));
      freeAimSpinSpeed =
          MAX(freeAimSpinSpeed * fSpinMult, sixense_hold_spin_speed.GetFloat());
    }
#endif
  }
  ...
  if (m_fTeleportWaitToBlendTime - gpGlobals->curtime <= 0.0f) {
    ...
    if (m_fTeleportWaitToBlendTime > 0.0f) {
      ...
      float fBlendTime =
          clamp(freeAimSpinSpeed * 0.16f *
                    sixense_aim_freeaim_switch_blend_time_enter.GetFloat(),
                0.1f, 2.0f);
      m_pFPSViewAngles->setParameter(
          sixenseUtils::IFPSViewAngles::AIM_METROID_SWITCH_BLEND_TIME_ENTER,
          fBlendTime);
    }
  } else {
    m_pFPSViewAngles->setParameter(
        sixenseUtils::IFPSViewAngles::AIM_METROID_MAX_SPEED, 0.0f);
  }
  ...
  // Update returns false if there aren't enough controllers to play, so disable
  // all the sixense stuff
  if (m_pFPSViewAngles->update(&m_pACD->controllers[m_nLeftIndex],
                               &m_pACD->controllers[m_nRightIndex],
  ...
  filtered_frametime = filtered_frametime * frametime_filt_param +
                       flFrametime * 1000.0f * (1.0f - frametime_filt_param);

  m_pFPSPlayerMovement->update(&m_pACD->controllers[m_nLeftIndex],
                               &m_pACD->controllers[m_nRightIndex],
                               filtered_frametime);
  m_pFPSEvents->update(&m_pACD->controllers[m_nLeftIndex],
                       &m_pACD->controllers[m_nRightIndex], filtered_frametime);
...
void SixenseInput::CheckWeaponForScope() {
...
#if defined(HL2_CLIENT_DLL)
  C_BaseHLPlayer *hlPlayer =
      dynamic_cast<C_BaseHLPlayer *>(C_BasePlayer::GetLocalPlayer());
  ...
  if (zoomed && !m_bScopeSwitchedMode) {
    ...
    m_nScopeSwitchedPrevSpringViewEnabled =
        sixense_spring_view_enabled.GetInt();
    ...
    // In TF2 wait until we're done reloading before switching back to metroid
    // mode
    C_BasePlayer *pPlayer = C_BasePlayer::GetLocalPlayer();

    if (pPlayer) {
      CTFSniperRifle *tfSniperRifle =
          dynamic_cast<CTFSniperRifle *>(pPlayer->GetActiveWeapon());
...
#if defined(CSTRIKE15) || defined(CSTRIKE_DLL)
    C_CSPlayer *csPlayer =
        dynamic_cast<C_CSPlayer *>(C_BasePlayer::GetLocalPlayer());
    ...
    if (m_bScopeSwitchedMode) {
      ...
      sixense_spring_view_enabled.SetValue(
          m_nScopeSwitchedPrevSpringViewEnabled);
...
void SixenseInput::SwitchViewModes(CUserCmd *pCmd) {
  ...
  if (sixense_large_objects_lock_one_to_one.GetInt()) {
    ...
    if (held) {
      ...
      if (scaled_cube) {
        ...
        const float scale_force_1to1 =
            sixense_large_objects_lock_one_to_one_size.GetFloat();
  ...
  // Whenever we scale, lock into one to one mode until the next drop (block
  // dist one to one from exiting)
  // if( pPortalPlayer->IsScalingUseItem() ) {
  //	m_bScalingLockedOneToOne = true;
  //}

  // This was old code for controlling 1to1 with a button, but this code was
  // getting hit on drop. Go into 1-to-1 when the button is pressed (and we're
  // holding an object, and we're not scaling)
  // if ( !m_bIs1to1ModeLocked &&
  //        !m_bIs1to1ModeScaling &&
  //        IsHoldingObject() &&
  //        ( lock_button && m_pRightButtonStates->justPressed( lock_button ) )
  //        &&
  //		( pPortalPlayer && !pPortalPlayer->IsScalingUseItem() &&
  //!pPortalPlayer->IsScalingUseItemTurret() ) )
  //{
  //	SetOneToOneMode( true );
  //}
  // exit 1-to-1 if we dropped the object we were holding, or if the lock button
  // was pressed again
  if ((!IsHoldingObject() && m_bIsIn1to1Mode &&
       !m_bExitOneWhenAimingForwards) ||
  ...
  // Object was dropped while in 1-to-1, don't leave one-to-1 mode until the
  // hand comes back a bit
...
void SixenseInput::SetPlayerHandPositions(CUserCmd *pCmd, float flFrametime) {
  if (m_bShouldSetBaseOffset) {
    ...
    sixense_base_offset_z.SetValue(-m_pACD->controllers[m_nRightIndex].pos[2] -
                                   base_offset_z_bias);
  }

  sixenseMath::Vector3 pos;
  sixenseMath::Vector3 base_offset(sixense_base_offset_x.GetFloat(),
                                   sixense_base_offset_y.GetFloat(),
                                   sixense_base_offset_z.GetFloat());

  // Tell the client player about the hand positions. This will get sent to the
  // server player as part of the usercmd
  ...
  if (m_bIsIn1to1Mode && IsHoldingObject()) {
    ...
    player->SetSixenseFlags(player->GetSixenseFlags() |
                            CBasePlayer::PLAYER_SIXENSE_HOLDING_OBJECT);

    if (m_bIs1to1ModeRatcheting) {
      pCmd->sixense_flags |=
          C_BasePlayer::PLAYER_SIXENSE_HOLDING_OBJECT_RATCHETING;
      player->SetSixenseFlags(
          player->GetSixenseFlags() |
          CBasePlayer::PLAYER_SIXENSE_HOLDING_OBJECT_RATCHETING);
    } else {
      pCmd->sixense_flags &=
          ~C_BasePlayer::PLAYER_SIXENSE_HOLDING_OBJECT_RATCHETING;
      player->SetSixenseFlags(
  ...
  if (sixense_dist_one_to_one_enabled.GetInt()) {
    pCmd->sixense_flags |=
        C_BasePlayer::PLAYER_SIXENSE_HOLDING_OBJECT_ALWAYS_ONE_TO_ONE;
    player->SetSixenseFlags(
        player->GetSixenseFlags() |
        CBasePlayer::PLAYER_SIXENSE_HOLDING_OBJECT_ALWAYS_ONE_TO_ONE);
  } else {
  ...
  // Get the angle mode from the fps angles so we can remove it from the hand
  // orientations
  float angle_mode_angle = m_pFPSViewAngles->getParameter(
      sixenseUtils::IFPSViewAngles::CONTROLLER_ANGLE_MODE);
  ...
  // Set rotation first
  left_mat = sixenseMath::Matrix4::rotation(
      sixenseMath::Quat(m_pACD->controllers[m_nLeftIndex].rot_quat[0],
                        m_pACD->controllers[m_nLeftIndex].rot_quat[1],
                        m_pACD->controllers[m_nLeftIndex].rot_quat[2],
                        m_pACD->controllers[m_nLeftIndex].rot_quat[3]));

  // Correct for controller angle mode
  left_mat = sixenseMath::Matrix4::rotation(
                 -angle_mode_angle, sixenseMath::Vector3(1.0f, 0.0f, 0.0f)) *
             left_mat;

  // Fill in translation
  Vector3 ss_left_pos =
      sixenseMath::Vector3(m_pACD->controllers[m_nLeftIndex].pos[0],
                           m_pACD->controllers[m_nLeftIndex].pos[1],
  ...
  // Set rotation first
  right_mat = sixenseMath::Matrix4::rotation(
      sixenseMath::Quat(m_pACD->controllers[m_nRightIndex].rot_quat[0],
                        m_pACD->controllers[m_nRightIndex].rot_quat[1],
                        m_pACD->controllers[m_nRightIndex].rot_quat[2],
                        m_pACD->controllers[m_nRightIndex].rot_quat[3]));
  ...
  Vector3 ss_right_pos =
      sixenseMath::Vector3(m_pACD->controllers[m_nRightIndex].pos[0],
                           m_pACD->controllers[m_nRightIndex].pos[1],
  ...
  if (!m_bScalingLockedOneToOne &&
      (Vector3(ss_right_pos[0], ss_right_pos[1], 0.0f).length() < xy_radius) &&
      (ss_right_pos[2] > min_z_dist)) {
    sixense_base_offset_z.SetValue(sixense_base_offset_z.GetFloat() -
                                   (ss_right_pos[2] - min_z_dist));

    // Also adjust the position at which the object was grabbed so that the dist
    // one to one slides also
  ...
  pCmd->controller_pos_left.Init(ss_left_pos[0], ss_left_pos[1],
                                 ss_left_pos[2]);
  pCmd->controller_pos_right.Init(ss_right_pos[0], ss_right_pos[1],
                                  ss_right_pos[2]);
  player->SetControllerPosLeft(pCmd->controller_pos_left);
  player->SetControllerPosRight(pCmd->controller_pos_right);

  // set controller rotations for portal tweaking, fix roll so that it goes from
  // (-180 to 180) instead of (-90 to 90)
  ...
  QAngle controller_rot_left = QAngle(RAD2DEG(left_contoller_angles[2]),
                                      RAD2DEG(left_contoller_angles[0]),
                                      RAD2DEG(left_contoller_angles[1]));
  QAngle controller_rot_right = QAngle(RAD2DEG(right_contoller_angles[2]),
                                       RAD2DEG(right_contoller_angles[0]),
  ...
  //		controller_rot_left.x = (controller_rot_left.x > 0.0f) ? (180.0f
  //- controller_rot_left.x) : (-180.0f + controller_rot_left.x);
  //	}
  //	if( right_mat_source[2][2] < 0.0f ) {
  //		controller_rot_right.x = (controller_rot_right.x > 0.0f) ?
  //(180.0f - controller_rot_right.x) : (-180.0f + controller_rot_right.x);
  //	}
  pCmd->controller_rot_left.Init(controller_rot_left.x, controller_rot_left.y,
                                 controller_rot_left.z);
  pCmd->controller_rot_right.Init(
      controller_rot_right.x, controller_rot_right.y, controller_rot_right.z);
  ...
  if (m_bIsIn1to1Mode && !m_nFreeaimSpinDisabled) {
    ...
    if (pHeldObject) {
      float hold_spin_start_screen_ratio =
          sixense_hold_spin_start_screen_ratio.GetFloat();
      float hold_spin_speed = sixense_hold_spin_speed.GetFloat();
      float hold_spin_speed_bias = sixense_hold_spin_speed_bias.GetFloat();
      float hold_spin_fade_min_dist =
          sixense_hold_spin_fade_min_dist.GetFloat();
      float hold_spin_fade_max_dist =
          sixense_hold_spin_fade_max_dist.GetFloat();
      ...
      // Don't pan if player is reaching behind, he's probably trying to wind up
      // for a throw
      ...
      float angle = acos((objectLocation - pPlayer->EyePosition())
                             .Normalized()
                             .Dot(vForward)) *
                    180 / M_PI;
      ...
      if (angle < hold_spin_max_angle) {
        ...
        if (abs(screen.x) > 1.0 - hold_spin_start_screen_ratio) {
          horiz_speed_mult =
              ((abs(screen.x) - (1.0 - hold_spin_start_screen_ratio)) /
               hold_spin_start_screen_ratio);
        ...
        if (abs(screen.y) > 1.0 - hold_spin_start_screen_ratio) {
          vert_speed_mult =
              ((abs(screen.y) - (1.0 - hold_spin_start_screen_ratio)) /
               hold_spin_start_screen_ratio);
        ...
        if (hold_spin_fade_max_dist != 0.0f &&
            hold_spin_fade_max_dist != 0.0f) {
      ...
      horiz_speed_mult = horiz_speed_mult * (1.0f - spin_damp) +
                         m_fLastHorizSpeedMult * spin_damp;
      vert_speed_mult = vert_speed_mult * (1.0f - spin_damp) +
                        m_fLastVertSpeedMult * spin_damp;
  ...
  // Compute the angles that point from the player to the hand positions. This
  // can be used to aim the weapon model at the object being moved. The goofy
  // offsets are because the gun doesn't rotate around the right position, so
  // they fudge it so it looks like it's pointing correctly
...
void SixenseInput::SetView(float flInputSampleFrametime, CUserCmd *pCmd) {
  ...
  // Set the new engine angles. This is the direction the player is really
  // aiming, and the only direction the server really cares about.
  ...
  // Keep track of the total accumulated spin angle. This accumulation is done
  // outisde of sixenseUtils because we need to keep a close look on the engine
  // time to do proper time sync.
  QAngle accumulated_spin_angles;
  accumulated_spin_angles =
      FixAngles(m_pFPSViewAngles->getFeetAnglesMetroid()) +
      (spin_speed * flInputSampleFrametime * 30.0f);
  ...
  // Tell the view angle system about the total angle so GetViewAngleOffset can
  // compute the player camera angle
  ...
  // Also set the player direction, this is the view frustum direction, which in
  // metroid mode counter-rotates from the aim direction engine angles.
  C_BasePlayer::GetLocalPlayer()->SetEyeAngleOffset(
      GetViewAngleOffset()); // This is the way the player is looking, and the
                             // orientation of the weapon model
...
void SixenseInput::SixenseUpdateKeys(float flFrametime, CUserCmd *pCmd) {
  ...
  // Sometimes we get called but Sixense isn't ready to produce any new data
  // yet. We want to preseve whatever we were doing before, ie walking, so keep
  // track of the last state we used.
  ...
  if (InMenuMode() ||
      (C_BasePlayer::GetLocalPlayer() &&
       C_BasePlayer::GetLocalPlayer()->IsObserver()) ||
      (m_nLeftIndex == -1) || (m_nRightIndex == -1) ||
      (m_nGesturesDisabled == 1)) {

    // No new data, set the keys to what they were last time we did have new
    // data

#ifdef PORTAL2
    // Unduck here too so if gestures are disabled like in the tutorial we're
    // not stuck ducking
    ...
    if (pCmd) {
      // Sometimes we get called but Sixense isn't ready to produce any new data
      // yet. We want to preseve whatever we were doing before, ie walking, so
      // keep track of the last state we used.
    ...
    if (pPlayer && pPlayer->IsObserver()) {
      if (m_pLeftButtonStates->stickJustPressed(
              sixenseUtils::IButtonStates::DIR_RIGHT)) {
        engine->ExecuteClientCmd("spec_next");
      }
      if (m_pLeftButtonStates->stickJustPressed(
              sixenseUtils::IButtonStates::DIR_LEFT)) {
        engine->ExecuteClientCmd("spec_prev");
      }
      if (m_pLeftButtonStates->stickJustPressed(
              sixenseUtils::IButtonStates::DIR_UP)) {
        engine->ExecuteClientCmd("spec_mode");
      }
      if (m_pLeftButtonStates->stickJustPressed(
              sixenseUtils::IButtonStates::DIR_DOWN)) {
        engine->ExecuteClientCmd("spec_mode");
      }

      if (m_pRightButtonStates->stickJustPressed(
              sixenseUtils::IButtonStates::DIR_RIGHT)) {
        engine->ExecuteClientCmd("spec_next");
      }
      if (m_pRightButtonStates->stickJustPressed(
  ...
  // If the radial menu is open, all we care about is the release of the radial
  // menu buttons.
  if (engine->IsLocalPlayerResolvable() && IsRadialMenuOpen()) {
    if (m_pLeftButtonStates->justReleased(SIXENSE_BUTTON_2) ||
        m_pLeftButtonStates->justReleased(SIXENSE_BUTTON_4)) {
      engine->ClientCmd_Unrestricted("-mouse_menu");
    }

    if (m_pLeftButtonStates->justReleased(SIXENSE_BUTTON_1) ||
        m_pLeftButtonStates->justReleased(SIXENSE_BUTTON_3)) {
  ...
  if (pCmd) {
    float forward =
        m_pFPSPlayerMovement->getWalkDir()[1] * cl_forwardspeed.GetFloat();
    float side =
        m_pFPSPlayerMovement->getWalkDir()[0] * cl_sidespeed.GetFloat();
...
#ifdef TF_CLIENT_DLL
  CHudMenuSpyDisguise *pSpyMenu =
      (CHudMenuSpyDisguise *)GET_HUDELEMENT(CHudMenuSpyDisguise);
  ...
  CHudMenuEngyBuild *pEngBuildMenu =
      (CHudMenuEngyBuild *)GET_HUDELEMENT(CHudMenuEngyBuild);
  ...
  CHudMenuEngyDestroy *pEngDestroyMenu =
      (CHudMenuEngyDestroy *)GET_HUDELEMENT(CHudMenuEngyDestroy);
  ...
  if (TFGameRules() && TFGameRules()->IsInTraining()) {
    if (m_pLeftButtonStates->absoluteTiltJustStarted(
            sixenseUtils::IButtonStates::DIR_UP)) {
...
#ifdef PORTAL2
  // coop ping
  if (m_pLeftButtonStates->justPressed(SIXENSE_BUTTON_1) ||
      m_pLeftButtonStates->justPressed(SIXENSE_BUTTON_2)) {
    engine->ClientCmd_Unrestricted("+mouse_menu");
  }

  if (m_pLeftButtonStates->justReleased(SIXENSE_BUTTON_1) ||
      m_pLeftButtonStates->justReleased(SIXENSE_BUTTON_2)) {
    engine->ClientCmd_Unrestricted("-mouse_menu");
  }
  ...
  // coop gesture
  if (m_pLeftButtonStates->justPressed(SIXENSE_BUTTON_3) ||
      m_pLeftButtonStates->justPressed(SIXENSE_BUTTON_4)) {
    engine->ClientCmd_Unrestricted("+mouse_menu_taunt");
  }

  if (m_pLeftButtonStates->justReleased(SIXENSE_BUTTON_3) ||
      m_pLeftButtonStates->justReleased(SIXENSE_BUTTON_4)) {
    engine->ClientCmd_Unrestricted("-mouse_menu_taunt");
  }
  ...
  // zooom
  {
    ...
    // check to see how long the zoom button was pressed. If it was really
    // short, we should lock the zoom
    if ((m_pACD->controllers[m_nRightIndex].buttons &
         SIXENSE_BUTTON_JOYSTICK) &&
        !momentary_zoom &&
        (C_BasePlayer::GetLocalPlayer() &&
         C_BasePlayer::GetLocalPlayer()->IsZoomed())) {
      double current_time = sixenseUtils::Time::getTimeInMilliseconds();
      double button_held_time = current_time - zoom_button_press_time;
      const double momentary_thresh_time =
          sixense_zoom_momentary_time.GetFloat();
    ...
    if (momentary_zoom &&
        m_pRightButtonStates->buttonJustReleased(SIXENSE_BUTTON_JOYSTICK)) {
      new_buttons |= IN_ZOOM;
      momentary_zoom_release_count = 30;
    }
  ...
  if (sixense_features_enabled.GetInt()) {
    ...
    // Button one-to-one
    if (!sixense_dist_one_to_one_enabled.GetInt() &&
        sixense_auto_one_to_one_enabled.GetInt() && IsHoldingObject() &&
        !m_bIs1to1ModeScaling &&
      ...
      // one to one
      if (m_pFPSEvents->eventStarted(
              sixenseUtils::IFPSEvents::ONE_TO_ONE_CARRY)) {
        SetOneToOneMode(true);
      }

      if (m_pFPSEvents->eventStopped(
              sixenseUtils::IFPSEvents::ONE_TO_ONE_CARRY)) {
        SetOneToOneMode(false);
      }
    }
    ...
    // dist one-to-one
    if (sixense_dist_one_to_one_enabled.GetInt() && IsHoldingObject() &&
        !m_bIs1to1ModeScaling && !m_bScalingLockedOneToOne &&
        (C_Portal_Player::GetLocalPortalPlayer() &&
         !C_Portal_Player::GetLocalPortalPlayer()->IsScalingUseItem() &&
         !C_Portal_Player::GetLocalPortalPlayer()->IsScalingUseItemTurret())) {
      ...
      if (!IsInOneToOneMode() &&
          ...
          ((delta_vec[2] <
            -sixense_dist_one_to_one_dist
                 .GetFloat()) || // start one to one if you reach forwards
           (delta_vec[1] >
            1.5f * sixense_dist_one_to_one_dist
                       .GetFloat()))) // start one to one if you raise up 1.5
                                      // times the dist too...
      {
        SetOneToOneMode(true);
        fEndOneToOneRatchetDelayTime = 0.0f;
                 ...
                 (delta_vec[1] <
                  1.5f * sixense_dist_one_to_one_dist.GetFloat()) &&
                 (delta_vec.length() <
                  2.0f * sixense_dist_one_to_one_dist.GetFloat()) &&
                 IsAimingForwards()) {
        // if ratcheting when ending 1to1 mode, first end ratchet, then end 1to1
        // after a short delay so end ratchet can propogate to server
        if (m_bIs1to1ModeRatcheting) {
          m_pFPSViewAngles->setRatcheting(false);
          m_bIs1to1ModeRatcheting = false;
  ...
  if (m_pLeftButtonStates->justReleased(SIXENSE_BUTTON_BUMPER)) {
    m_fDisableJumpUntil =
        (sixenseUtils::Time::getTimeInMilliseconds()) +
        sixense_scale_disables_jump_timer.GetFloat() * 1000.0f;
  }

  // If the disable jump timer is set
  if (m_fDisableJumpUntil != 0.0 &&
      (m_fDisableJumpUntil > (sixenseUtils::Time::getTimeInMilliseconds()))) {
    jump_disabled = true;
  }
  ...
  if (!jump_disabled &&
      (m_pFPSEvents->eventStarted(sixenseUtils::IFPSEvents::JUMP) ||
       (m_pRightButtonStates->justPressed(SIXENSE_BUTTON_3) ||
        m_pRightButtonStates->justPressed(SIXENSE_BUTTON_4)))) {
    new_buttons |= IN_JUMP;
  }

  if (m_pFPSEvents->eventStopped(sixenseUtils::IFPSEvents::JUMP) ||
      (m_pRightButtonStates->justReleased(SIXENSE_BUTTON_3) ||
       m_pRightButtonStates->justReleased(SIXENSE_BUTTON_4))) {
    new_buttons &= ~IN_JUMP;
  }

  {
    ...
    if (m_pRightButtonStates->justPressed(SIXENSE_BUTTON_1)) {
      ...
      // This is a hack to center wheatley. Basically recenter whenever the 1
      // button is pressed. It's ok to recenter on drop.
      SetBaseOffset();
    ...
    if (((m_pACD->controllers[m_nRightIndex].buttons & SIXENSE_BUTTON_1) ||
        ...
        !momentary_hold) {
      // Check to see how long the hold button was pressed. If it was really
      // short, we should lock the hold
      double current_time = sixenseUtils::Time::getTimeInMilliseconds();

      double button_held_time = current_time - hold_button_press_time;

      const double momentary_thresh_time =
          sixense_pickup_momentary_time.GetFloat();
...
void SixenseInput::SixenseUpdateMouseCursor() {
...
#if defined(HL2_CLIENT_DLL) || defined(TF_CLIENT_DLL)
  // If there's no mouse cursor visible, don't ever move the mouse or click
  // here. Gesture bindings can still call 'sixense_left_click' for times when a
  // click is necessary but no mouse cursor.
  ...
  // Keep track of when the left button is down so we can turn it off if we
  // leave mouse mode
  ...
  if (!InMenuMode()) {
    ...
    // If we're not in mouse mode, check to see if we just entered it and need
    // to switch out.
    if (left_clicked) {
      ::sendMouseClick(0, 1);
      left_clicked = false;
  ...
  if (m_pRightButtonStates->triggerJustPressed() ||
      m_pRightButtonStates->buttonJustPressed(SIXENSE_BUTTON_1)) {
    ::sendMouseClick(1, 0);
    left_clicked = true;
  } else if (m_pRightButtonStates->triggerJustReleased() ||
             m_pRightButtonStates->buttonJustReleased(SIXENSE_BUTTON_1)) {
    ::sendMouseClick(0, 1);
    left_clicked = false;
  }
  ...
  if (sixense_mouse_enabled.GetInt() && InMenuMode() &&
      (GetActiveWindow() == FindWindow(NULL, window_name)))
#else
  if (sixense_mouse_enabled.GetInt() && InMenuMode())
#endif
  {
    ...
    if (m_pLaserPointer) {
...
#ifdef PORTAL2
      if (engine->IsLocalPlayerResolvable() &&
          (IsRadialMenuOpen() != radial_menu_up_last_frame)) {
      ...
      sixenseMath::Vector3 xz_projection(controller_forwards[0], 0.0f,
                                         controller_forwards[2]);
      ...
      sixenseMath::Vector3 cross =
          xz_projection ^ sixenseMath::Vector3(0, 0, -1);
      ...
      Vector2 norm_coord(heading_angle / hfov + 0.5f,
                         pitch_angle / vfov + 0.5f);
      ...
      // make top actually reference the top of the screen (windows has y=0 at
      // the top)
      ...
      int title_bar_height =
          ...
          2; // fudge these a little bit so we can't click on the borders
      ...
      float desktop_width =
          (float)desktop_rect.right - (float)desktop_rect.left;
      float desktop_height =
          (float)desktop_rect.bottom - (float)desktop_rect.top;
      ...
      Vector2 abs_mouse_pos(
          win_norm_left + norm_coord[0] * (win_norm_right - win_norm_left),
      ...
      if (filtered_pixel_pos == Vector2(-999, -999)) {
        ...
        filtered_pixel_pos = filtered_pixel_pos * mouse_filter_val +
                             abs_mouse_pos * (1.0f - mouse_filter_val);
      }
...
matrix3x4_t ConvertMatrix(sixenseMath::Matrix4 ss_mat) {
  ...
  tmp_mat = tmp_mat * sixenseMath::Matrix4::rotation(-3.1415926f / 2.0f,
                                                     Vector3(1, 0, 0));
  tmp_mat = tmp_mat * sixenseMath::Matrix4::rotation(-3.1415926f / 2.0f,
                                                     Vector3(0, 0, 1));
  tmp_mat =
      tmp_mat * sixenseMath::Matrix4::rotation(3.1415926f, Vector3(0, 1, 0));

  tmp_mat =
      sixenseMath::Matrix4::rotation(-3.1415926f / 2.0f, Vector3(1, 0, 0)) *
      tmp_mat;
...
void SixenseInput::Rumble(unsigned char index, unsigned char rumbleData,
                          unsigned char rumbleFlags) {
  m_pSixenseAPI->sixenseTriggerVibration(0, 10, 0);
  m_pSixenseAPI->sixenseTriggerVibration(1, 10, 0);
}

void SixenseInput::Rumble(unsigned char playerIndex, unsigned char handIndex,
                          unsigned char rumbleData, unsigned char rumbleFlags) {
  // find controller index based on player and hand indexes
  int controller_index = -1;

  switch (playerIndex) {
  case 0:
    controller_index = m_pControllerManager->getIndex(
        (0 == handIndex) ? sixenseUtils::IControllerManager::P1L
                         : sixenseUtils::IControllerManager::P1R);
    break;
  case 1:
    controller_index = m_pControllerManager->getIndex(
        (0 == handIndex) ? sixenseUtils::IControllerManager::P2L
                         : sixenseUtils::IControllerManager::P2R);
    break;
  case 2:
    controller_index = m_pControllerManager->getIndex(
        (0 == handIndex) ? sixenseUtils::IControllerManager::P3L
                         : sixenseUtils::IControllerManager::P3R);
    break;
  case 3:
    controller_index = m_pControllerManager->getIndex(
        (0 == handIndex) ? sixenseUtils::IControllerManager::P4L
...
void SixenseInput::CreateGUI(vgui::VPANEL parent) {
  ...
  m_SixenseFrame =
      new SixenseGUIFrame(enginevgui->GetPanel(PANEL_ROOT), "SixenseGUIFrame");
...
void SixenseInput::SetFilterLevel(float near_range, float near_val,
  ...
  if (near_range == 0.0f && far_range == 0.0f) {
    ...
    m_pSixenseAPI->sixenseSetFilterParams(near_range, near_val, far_range,
                                          far_val);
  }
}
...
void SixenseInput::DisableGestures(int disable) {
  ...
  // if the player is ducking when we disable gestures do one last unduck to
  // make sure they don't get stuck ducking
...
SixenseBaseWarning::SixenseBaseWarning(vgui::Panel *parent,
...
static ConCommand sixense_show_frame("sixense_show_frame", ToggleFrame,
                                     "Show/hide Sixense UI.");

//////

static void SetBaseOffset() { g_pSixenseInput->SetBaseOffset(); }
...
ConCommand sixense_disable_gestures("sixense_disable_gestures",
                                    DisableGestures);
...
ConCommand
    sixense_aim_freeaim_spin_disabled("sixense_aim_freeaim_spin_disabled",
                                      DisableFreeAimSpin);
...
void set_filter_params(const CCommand &args) {
  ...
  if (args.ArgC() < 5) {
    Msg("Usage: set_filter_params <near_range> <near_val> <far_range> "
        "<far_val>\n");
    return;
  }

  g_pSixenseInput->SetFilterLevel(atof(args[1]), atof(args[2]), atof(args[3]),
                                  atof(args[4]));
}

static ConCommand sixense_set_filter_params("sixense_set_filter_params",
                                            set_filter_params);

//////

void SixenseSensitivityLevelChanged(IConVar *var, const char *pOldValue,
                                    float flOldValue) {
...
bool directionFromString(CUtlString dir_str,
  ...
  if (dir_str == "up") {
    ...
    *dir = sixenseUtils::IButtonStates::DIR_CCW;
    ...
    Msg("Unknown direction %s, shoud be 'up' 'down' 'left' 'right' 'cw' or "
        "'ccw'\n",
        dir_str.String());
    return false;
  }
...
bool buttonMaskFromString(CUtlString button, unsigned short *button_token) {
  ...
  if (button == "1") {
    ...
    *button_token = SIXENSE_BUTTON_JOYSTICK;
    ...
    Msg("Unknown button %s, shoud be '1' '2' '3' '4' 'start' 'trigger' or "
        "'joystick'\n",
        button.String());
    return false;
  }
...
bool actionFromString(CUtlString action_str,
  ...
  if (action_str == "button_press") {
    ...
    *action = sixenseUtils::IButtonStates::ACTION_JOYSTICK_MOVE;
    ...
    Msg("Unknown action %s, shoud be 'button_press' 'trigger_press' "
        "'tilt_gesture' 'point_gesture' 'velocity_gesture' or "
        "'joystick_move'\n",
        action_str.String());
    *action = sixenseUtils::IButtonStates::ACTION_BUTTON_PRESS;
    return false;
...
static void SixenseBind(const CCommand &args) {
  if (args.ArgC() != 5 && args.ArgC() != 6) {
    Msg("Usage: sixense_bind <hand_left_or_right> <action> <argument> "
        "<on_press_command> [<on_release_command>]\n");
    return;
  }

  CUtlString hand_str(args[1]), action_str(args[2]), argument_str(args[3]),
      press_command(args[4]), release_command;
  ...
  if (g_pSixenseInput->GetGestureBindings()) {
    g_pSixenseInput->GetGestureBindings()->AddBinding(
        hand_str, action_str, argument_str, press_command, release_command);
  }
}
...
static ConCommand sixense_bind_command("sixense_bind", SixenseBind,
                                       "Bind a concommand to a button.");
static ConCommand sixense_list_bindings_cc("sixense_list_bindings",
                                           SixenseListBindings,
                                           "List the sixense bindings.");
static ConCommand
    sixense_write_bindings_cc("sixense_write_bindings", SixenseWriteBindings,
                              "Save the sixense bindings to a file.");
static ConCommand sixense_clear_bindings_cc("sixense_clear_bindings",
                                            SixenseClearBindings,
                                            "Clear all sixense bindings.");
static ConCommand
    sixense_delete_binding_cc("sixense_delete_binding", SixenseDeleteBinding,
...
void SelectMachinegun() { GetHudWeaponSelection()->SelectSlot(2); }

ConCommand sixense_select_machinegun("sixense_select_machinegun",
                                     SelectMachinegun);
...
void StartRatchet() { g_pSixenseInput->StartRatchet(); }
...
void StopRatchet() { g_pSixenseInput->StopRatchet(); }

ConCommand sixense_stop_ratchet("-sixense_ratchet", StopRatchet);

//////

void SelectPistol() { GetHudWeaponSelection()->SelectSlot(1); }
...
void SelectMelee() { GetHudWeaponSelection()->SelectSlot(3); }
...
void StartLeftPointGesture() { g_pSixenseInput->LeftPointGesture(true); }

ConCommand sixense_start_left_point_gesture("+sixense_left_point_gesture",
                                            StartLeftPointGesture);

//////

void StopLeftPointGesture() { g_pSixenseInput->LeftPointGesture(false); }

ConCommand sixense_stop_left_point_gesture("-sixense_left_point_gesture",
                                           StopLeftPointGesture);

//////

void StartRightPointGesture() { g_pSixenseInput->RightPointGesture(true); }

ConCommand sixense_start_right_point_gesture("+sixense_right_point_gesture",
                                             StartRightPointGesture);

//////

void StopRightPointGesture() { g_pSixenseInput->RightPointGesture(false); }

ConCommand sixense_stop_right_point_gesture("-sixense_right_point_gesture",
                                            StopRightPointGesture);
