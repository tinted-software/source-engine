...
//   action_str = 'button_press' 'trigger_press' 'tilt_gesture' 'point_gesture'
//   'velocity_gesture' or 'joystick_move' argument_str depends on action_str,
//   see below for details press_command_str is the concommand executed on start
//   of the action (ie button press) and release_command_str on stop (ie button
//   release) release_command_str can be the empty string if no stop action is
//   desired if the press_command_str begins with "+", an equivalent "-" is set
//   to the release_command automatically
void SixenseGestureBindings::AddBinding(CUtlString hand_str,
                                        CUtlString action_str,
                                        CUtlString argument_str,
                                        CUtlString press_command_str,
                                        CUtlString release_command_str) {
  ...
  // Try to keep a single binding per 'action' 'hand' 'arg' pair, ie one per
  // button. We may want to allow multiple if people think it would be useful...
...
void SixenseGestureBindings::ListBindings() {
  ...
  // Just go through all the bindings, use helpers to convert from tokens to
  // strings, and print.
  int i = 0;
  FOR_EACH_LL(m_GestureBindingList, it) {
    ...
    if (ActionTokenToStr(
            (sixenseUtils::IButtonStates::ActionType)binding.m_Action, strbuf,
            strbuflen)) {
      Msg("\t%s", strbuf);
    }
    ...
    // these all have the same arguments (left, right, up...)
    else if (binding.m_Action ==
                 sixenseUtils::IButtonStates::ACTION_JOYSTICK_MOVE ||
             binding.m_Action ==
                 sixenseUtils::IButtonStates::ACTION_POINT_GESTURE ||
             binding.m_Action ==
                 sixenseUtils::IButtonStates::ACTION_VELOCITY_GESTURE ||
             binding.m_Action ==
                 sixenseUtils::IButtonStates::ACTION_TILT_GESTURE) {
      ...
      if (DirectionTokenToStr(binding.m_iArgument, strbuf, strbuflen)) {
        Msg("\t%s", strbuf);
               ...
               sixenseUtils::IButtonStates::ACTION_TRIGGER_PRESS) {
      Msg("\t"); // no argument
    }

    Msg("\t\"%s\"", binding.m_pActivateCommand);
    if (binding.m_pDeactivateCommand &&
        !binding.m_bAutoMirrored) // only print deactivated command if we didn't
                                  // generate it.
    {
...
// Write the current set of bindings to a file, formatted so that the file can
// be 'exec'ed.
void SixenseGestureBindings::WriteBindings(CUtlString filename_str) {
  ...
  if (!filename_str.IsEmpty()) {
    Q_snprintf(filename, filenamelen, "%s\\cfg\\%s", engine->GetGameDirectory(),
               filename_str.String());
  } else {
    Q_snprintf(filename, filenamelen, "%s\\cfg\\sixense_bindings.cfg",
  ...
  // Go through all the bindings, convert from tokens to strings, build up the
  // command string and write it to the file
  FOR_EACH_LL(m_GestureBindingList, it) {
    GestureBinding binding = m_GestureBindingList[it];
...
// Erase all the bindings. Right now this will cause code in in_sixense.cpp to
// detect the lack of bindings and immediately create defaults. I think that's
// good.
...
// Delete a specific binding by index. 'sixense_list_bindings' prints the
// indicies that can be passed to delete.
...
void SixenseGestureBindings::CreateDefaultBindings() {
  ...
  AddBinding("right", "button_press", "bumper", "+sixense_left_shift",
             ""); // queue
...
// Called each frame with the latest controller info.
// sixenseUtils::IButtonStates are sixenseUtils classes that look for various
// transitions of controller state, whether it's 'buttonJustPressed', or analog
// joysticks 'joystickJustPushed(up)'. It now has support for motion triggers
// like 'controllerJustTilted(left)'
void SixenseGestureBindings::UpdateBindings(
    sixenseUtils::IButtonStates *pLeftButtonStates,
    sixenseUtils::IButtonStates *pRightButtonStates, bool bIsMenuVisible) {
  // go through all the bindings and see if any of the desired actions just
  // transitioned on or off
  FOR_EACH_LL(m_GestureBindingList, it) {
    GestureBinding binding = m_GestureBindingList[it];
    ...
    if (binding.m_iHand == 0) {
      ...
      // just started?
      if (pLeftButtonStates->justStarted(
              (sixenseUtils::IButtonStates::ActionType)binding.m_Action,
              binding.m_iArgument)) {
        if (binding.m_pActivateCommand) {
      ...
      // just stopped?
      if (pLeftButtonStates->justStopped(
              (sixenseUtils::IButtonStates::ActionType)binding.m_Action,
              binding.m_iArgument)) {
        if (binding.m_pDeactivateCommand) {
      ...
      // just started?
      if (pRightButtonStates->justStarted(
              (sixenseUtils::IButtonStates::ActionType)binding.m_Action,
              binding.m_iArgument)) {
        if (binding.m_pActivateCommand) {
      ...
      // just stopped?
      if (pRightButtonStates->justStopped(
              (sixenseUtils::IButtonStates::ActionType)binding.m_Action,
              binding.m_iArgument)) {
        if (binding.m_pDeactivateCommand) {
          engine->ClientCmd_Unrestricted(binding.m_pDeactivateCommand);
...
bool SixenseGestureBindings::AllowCommand(char *pActivateCommand) {
  ...
  if (TFGameRules() && TFGameRules()->IsInTraining() &&
      TFGameRules()->IsWaitingForTrainingContinue()) {
...
// If the menu is up, most bindings are blocked. Allow some of them to be
// activated...
...
// from here down are just helper funcs to convert between enums and strings and
// back

bool SixenseGestureBindings::DirectionFromString(
    CUtlString dir_str, sixenseUtils::IButtonStates::Direction *dir) {
...
bool SixenseGestureBindings::ButtonMaskFromString(
    CUtlString button, unsigned short *button_token) {
...
bool SixenseGestureBindings::ActionFromString(
    CUtlString action_str, sixenseUtils::IButtonStates::ActionType *action) {
...
bool SixenseGestureBindings::DirectionTokenToStr(int arg, char *buf,
                                                 int buflen) {
  if (buflen < 10)
    return false;
...
bool SixenseGestureBindings::ActionTokenToStr(
    sixenseUtils::IButtonStates::ActionType action, char *buf, int buflen) {
  if (buflen < 20)
    return false;
