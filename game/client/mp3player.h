...
struct MP3File_t {
  enum {
    FLAG_UNKNOWN = 0,
    // File came from steam cache/game data, rather than the user's "My Music"
    // folder...
    FLAG_FROMGAME,
  ...
  FileNameHandle_t
      playbackfilename; // in case we had to make a local copy somewhere else...
...
struct SoundDirectory_t {
  explicit SoundDirectory_t(int index)
      : m_nIndex(index), m_Root(UTL_INVAL_SYMBOL), m_pTree(0),
        m_bGameSound(false) {}
