...
static ConVar g15_update_msec("g15_update_msec", "250", FCVAR_ARCHIVE,
...
///-----------------------------------------------------------------------------
CLCD::CLCD(void)
    : m_lcd(NULL), m_nCurrentPage(0), m_nSubPage(0), m_bHadPlayer(false),
      m_dwNextUpdateTime(0u), m_nMaxChatHistory(G15_DEFAULT_MAX_CHAT_HISTORY),
      m_pG15Module(0), m_G15Factory(0) {
  m_Size[0] = m_Size[1] = 0;
}
...
void CLCD::Init(void) {
  ...
  if (kv->LoadFromFile(filesystem, G15_RESOURCE_FILE, "MOD")) {
    ...
    m_nMaxChatHistory =
        clamp(1, kv->GetInt("chatlines", m_nMaxChatHistory), 64);
    Assert(title);
    m_Title = title;
...
void CLCD::ShowItems_R(CLCDPage *page, unsigned int dwCurTime,
                       CUtlVector<CLCDItem *> &list, bool bShowItems) {
  ...
  for (int j = 0; j < itemCount; ++j) {
      ...
      case LCDITEM_AGGREGATE: {
        ...
        case AGGTYPE_PERTEAM: {
          ...
          if (local) {
            for (int pl = 1; pl <= gpGlobals->maxClients; ++pl) {
              if (g_PR && g_PR->IsConnected(pl) &&
                  local->GetTeamNumber() == g_PR->GetTeam(pl)) {
                validIndices.AddToTail(pl);
              }
            }
          }
        ...
        for (int e = 0; e < ecount; ++e) {
          ...
          for (int r = 0; r < ag->m_Definition.Count(); ++r) {
            ...
            case LCDITEM_TEXT: {
              ...
              CLCDItemText *copy =
                  static_cast<CLCDItemText *>(page->Alloc(item->m_Type));
              *copy = *text;
              copy->m_bActive = true;
            ...
            case LCDITEM_ICON: {
              CLCDItemIcon *icon = static_cast<CLCDItemIcon *>(item);
              CLCDItemIcon *copy =
                  static_cast<CLCDItemIcon *>(page->Alloc(item->m_Type));
              *copy = *icon;
              copy->m_bActive = true;
              copy->Create(m_lcd);
    ...
    m_lcd->SetVisible(item->m_Handle,
                      bShowItems && ((item->m_nSubPage == -1) ||
                                     item->m_nSubPage == m_nSubPage));
...
CLCDItemIcon *CLCD::ParseItemIcon(CLCDPage *page, bool bCreateHandles,
                                  KeyValues *sub) {
...
CLCDItemText *CLCD::ParseItemText(CLCDPage *page, bool bCreateHandles,
                                  KeyValues *sub) {
...
void CLCD::ParseItems_R(CLCDPage *page, bool bCreateHandles, KeyValues *kv,
  ...
  for (KeyValues *sub = kv->GetFirstSubKey(); sub; sub = sub->GetNextKey()) {
    ...
    if (!Q_stricmp(keyName, "iterate_players") ||
      ...
      CLCDItemAggregate *item =
          static_cast<CLCDItemAggregate *>(page->Alloc(LCDITEM_AGGREGATE));
      ...
      // Add the definition items as "inactive" items to the scene (so they get
      // destroyed)
...
void CLCD::ParseIconMappings(KeyValues *kv) {
  for (KeyValues *icon = kv->GetFirstSubKey(); icon;
       icon = icon->GetNextKey()) {
    IconInfo_t info;
    HICON hIcon = 0;
    char const *name = icon->GetName();
    char fullpath[512];
    filesystem->RelativePathToFullPath(icon->GetString(), "GAME", fullpath,
                                       sizeof(fullpath));
#ifdef WIN32
    hIcon = (HICON)::LoadImageA(NULL, fullpath, IMAGE_ICON, 32, 32,
                                LR_LOADFROMFILE);
#else
    hIcon = 0;
#endif
...
void CDescribeData::DescribeColor(const Color *invalue, int count) {
  for (int i = 0; i < count; ++i) {
    if (count == 1) {
      Describe(" color (%i %i %i %i)\n", invalue[i].r(), invalue[i].g(),
               invalue[i].b(), invalue[i].a());
    } else {
      Describe("[%i] color (%i %i %i %i)\n", i, invalue[i].r(), invalue[i].g(),
               invalue[i].b(), invalue[i].a());
    }
  }
}
...
void CDescribeData::DescribeVector(const Vector *inValue, int count) {
  for (int i = 0; i < count; ++i) {
    if (count == 1) {
      Describe(" vector (%f %f %f)\n", inValue[i].x, inValue[i].y,
               inValue[i].z);
    } else {
      Describe("[%i] vector (%f %f %f)\n", i, inValue[i].x, inValue[i].y,
               inValue[i].z);
    }
  }
}

void CDescribeData::DescribeFields_R(int chain_count, datamap_t *pRootMap,
...
void CLCD::DumpPlayer() {
  ...
  // Global replacements
  for (int i = m_GlobalStats.First(); i != m_GlobalStats.InvalidIndex();
       i = m_GlobalStats.Next(i)) {
...
void CLCD::LookupToken(char const *in, CUtlString &value) {
  ...
  *o = '\0';
  ...
  // Fixme, also need to get array reference removed from end and do the . field
  // searching stuff
  ...
  case FIELD_BOOLEAN:
    Q_snprintf(sz, sizeof(sz), "%s",
               *((bool *)pInputData + iIndex) ? "true" : "false");
    break;
  case FIELD_INTEGER:
...
void CLCD::UpdateChat() {
  for (int i = 0; i < m_nMaxChatHistory; ++i) {
    ...
    SetGlobalStat(name,
                  i < m_ChatHistory.Count() ? m_ChatHistory[i].String() : " ");
  }
}
