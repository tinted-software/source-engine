...
static ConVar cam_snapto("cam_snapto", "0",
                         FCVAR_ARCHIVE |
                             FCVAR_CHEAT); // snap to thirdperson view
...
static ConVar cam_idealdelta("cam_idealdelta", "4.0",
                             FCVAR_ARCHIVE | FCVAR_CHEAT,
                             "Controls the speed when matching offset to ideal "
                             "angles in thirdperson view");
ConVar cam_idealyaw("cam_idealyaw", "0",
                    FCVAR_ARCHIVE | FCVAR_CHEAT); // thirdperson yaw
ConVar cam_idealpitch("cam_idealpitch", "0",
                      FCVAR_ARCHIVE | FCVAR_CHEAT); // thirperson pitch
ConVar cam_idealdist("cam_idealdist", "150",
                     FCVAR_ARCHIVE | FCVAR_CHEAT); // thirdperson distance
ConVar cam_idealdistright("cam_idealdistright", "0",
                          FCVAR_ARCHIVE | FCVAR_CHEAT); // thirdperson distance
ConVar cam_idealdistup("cam_idealdistup", "0",
                       FCVAR_ARCHIVE | FCVAR_CHEAT); // thirdperson distance
static ConVar cam_collision(
    "cam_collision", "1", FCVAR_ARCHIVE | FCVAR_CHEAT,
    "When in thirdperson and cam_collision is set to 1, an attempt is made to "
    "keep the camera from passing though walls.");
static ConVar
    cam_showangles("cam_showangles", "0", FCVAR_CHEAT,
                   "When in thirdperson, print "
...
void CInput::CAM_Think(void) {
  ...
  idealAngles[PITCH] =
      clamp(idealAngles[PITCH], c_minpitch.GetFloat(), c_maxpitch.GetFloat());
  idealAngles[YAW] =
      clamp(idealAngles[YAW], c_minyaw.GetFloat(), c_maxyaw.GetFloat());
  idealAngles[DIST] = clamp(idealAngles[DIST], c_mindistance.GetFloat(),
  ...
  if (cam_collision.GetInt()) {
    QAngle desiredCamAngles =
        QAngle(camOffset[PITCH], camOffset[YAW], camOffset[DIST]);

    if (g_ThirdPersonManager.IsOverridingThirdPerson() == false) {
      desiredCamAngles = viewangles;
    }

    g_ThirdPersonManager.PositionCamera(C_BasePlayer::GetLocalPlayer(),
                                        desiredCamAngles);
  }
  ...
  if (cam_showangles.GetInt()) {
    ...
    engine->Con_NPrintf(8, "Pitch: %6.1f   Yaw: %6.1f   Dist: %6.1f %16s",
                        ...
                        g_ThirdPersonManager.GetCameraOffsetAngles()[DIST],
                        "camera offset");
  }
...
void CInput::CAM_CameraThirdThink(void) {
  ...
  if (pLocalPlayer) {
    QAngle desiredCamAngles =
        QAngle(vecCamOffset[PITCH], vecCamOffset[YAW], vecCamOffset[DIST]);
...
void CInput::CAM_ToThirdPerson(void) {
  ...
  if (!m_fCameraInThirdPerson) {
    ...
    g_ThirdPersonManager.SetCameraOffsetAngles(
        Vector(viewangles[YAW], viewangles[PITCH], CAM_MIN_DIST));
  }
...
static ConCommand
    thirdperson_mayamode("thirdperson_mayamode", ::CAM_ToThirdPerson_MayaMode,
