...
public:
  ...
  // causes the font manager to generate the glyph, prevents run time hitches on
  // platforms that have slow font managers
  void Precache(void);

  // returns width & height of icon with scale applied (scale is ignored if font
  // is used to render)
  ...
  void DrawSelfCropped(int x, int y, int cropx, int cropy, int cropw, int croph,
                       Color clr) const;
  // new version to scale the texture over a finalWidth and finalHeight passed
  // in
  void DrawSelfCropped(int x, int y, int cropx, int cropy, int cropw, int croph,
                       int finalWidth, int finalHeight, Color clr) const;
...
public:
  ...
  void DrawIconProgressBar(int x, int y, CHudTexture *icon, CHudTexture *icon2,
                           float percentage, Color &clr, int type);
