...
RecvPropVector(RECVINFO(m_vecOrigin)), RecvPropInt(RECVINFO(m_nModelIndex)),
    RecvPropFloat(RECVINFO(m_fScale)), RecvPropInt(RECVINFO(m_nBrightness)),
    END_RECV_TABLE()
...
void C_TESprite::PostDataUpdate(DataUpdateType_t updateType) {
  ...
  tempents->TempSprite(m_vecOrigin, vec3_origin, m_fScale, m_nModelIndex,
                       kRenderTransAdd, 0, a, 0, FTENT_SPRANIMATE);
...
void TE_Sprite(IRecipientFilter &filter, float delay, const Vector *pos,
  ...
  tempents->TempSprite(*pos, vec3_origin, size, modelindex, kRenderTransAdd, 0,
                       a, 0, FTENT_SPRANIMATE);
