...
FireRamp g_FireRamps[] = {FireRamp(Vector(1, 0, 0), Vector(1, 1, 0)),
...
// If you follow g_Offset[index], you can follow
// g_Offsets[GetOppositeOffset(index)] to get back.
...
public:
  ...
  public:
    Vector m_StartPos; // The particle moves from m_StartPos to
                       // (m_StartPos+m_Direction) over its lifetime.
    Vector m_Direction;
...
void C_ParticleFire::Start(CParticleMgr *pParticleMgr,
  ...
  m_MaterialHandle =
      m_ParticleEffect.FindOrAddMaterial("particle/particle_fire");
...
void C_ParticleFire::Update(float fTimeDelta) {
  ...
  if (m_nEmitters < MAX_FIRE_EMITTERS) {
    ...
    while (m_EmitterSpawn.NextEvent(tempDelta)) {
      ...
      if (m_nEmitters == 0) {
        ...
        trace_t trace;
        UTIL_TraceLine(m_vOrigin, m_vOrigin + m_vDirection * 1000,
        ...
        for (int iTry = 0; iTry < nTries; iTry++) {
          ...
          // Test the corners of the new cube. If some points are solid and some
          // are not, then we can put fire here.
          Vector basePos =
              pSourceEmitter->m_Pos + g_Offsets[iOffset] * fireSpreadDist;
          int nSolidCorners = 0;
  ...
  for (int i = 0; i < m_nEmitters; i++) {
    ...
    while (pEmitter->m_SpawnEvent.NextEvent(tempDelta)) {
      FireParticle *pParticle = (FireParticle *)m_ParticleEffect.AddParticle(
          sizeof(FireParticle), m_MaterialHandle);
      if (pParticle) {
        static float particleSpeed = 15;
        pParticle->m_StartPos = pEmitter->m_Pos;
        pParticle->m_Direction = g_FireSpreadDirection * particleSpeed +
                                 RandomVector(0, particleSpeed * 0.5);
...
void C_ParticleFire::RenderParticles(CParticleRenderIterator *pIterator) {
  ...
  while (pParticle) {
    float smooth01 =
        1 -
        (cos(pParticle->m_Lifetime * 3.14159 / FIRE_PARTICLE_LIFETIME) * 0.5 +
         0.5);
    float smooth00 =
        1 - (cos(pParticle->m_Lifetime * 3.14159 * 2 / FIRE_PARTICLE_LIFETIME) *
                 0.5 +
             0.5);

    FireRamp *pRamp = &g_FireRamps[pParticle->m_iRamp];
    Vector curColor =
        pRamp->m_Start + (pRamp->m_End - pRamp->m_Start) * smooth01;
    ...
    RenderParticle_ColorSize(pIterator->GetParticleDraw(), tPos, curColor,
                             smooth00, size);
    ...
    if (pParticle->m_Lifetime > FIRE_PARTICLE_LIFETIME) {
      ...
      float smooth01 =
          ...
          (cos(pParticle->m_Lifetime * 3.14159 / FIRE_PARTICLE_LIFETIME) * 0.5 +
           0.5);
      pParticle->m_Pos =
          pParticle->m_StartPos + pParticle->m_Direction * smooth01;
    }
