...
//-----------------------------------------------------------------------------
IMPLEMENT_CLIENTCLASS_DT(C_NextBotCombatCharacter, DT_NextBot,
...
void C_NextBotCombatCharacter::UpdateShadowLOD(void) {
  ...
  if (m_bForceShadowType) {
    ...
    FOR_EACH_VALID_SPLITSCREEN_PLAYER(hh) {
      ...
      if (pl) {
        Vector delta =
            GetAbsOrigin() - C_BasePlayer::GetLocalPlayer(hh)->GetAbsOrigin();
#else
    {
      if (C_BasePlayer::GetLocalPlayer()) {
        Vector delta =
            GetAbsOrigin() - C_BasePlayer::GetLocalPlayer()->GetAbsOrigin();
#endif
...
//--------------------------------------------------------------------------------------------------------
bool C_NextBotCombatCharacter::GetForcedShadowCastType(
...
bool C_NextBotManager::SetupInFrustumData(void) {
  ...
  FOR_EACH_VALID_SPLITSCREEN_PLAYER(iSlot) {
    ...
    for (int iBot = 0; iBot < nBotCount; ++iBot) {
      ...
      bool bInFrustum =
          !FrustumCache()->m_Frustums[iSlot].CullBox(vecMin, vecMax);

      if (bInFrustum) {
        Vector vecSegment;
        VectorSubtract(pBot->GetAbsOrigin(), pPlayer->GetAbsOrigin(),
                       vecSegment);
