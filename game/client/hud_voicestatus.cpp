...
private:
  ...
  CPanelAnimationVarAliasType(float, item_tall, "item_tall", "16",
                              "proportional_float");
  CPanelAnimationVarAliasType(float, item_wide, "item_wide", "160",
                              "proportional_float");
  CPanelAnimationVarAliasType(float, item_spacing, "item_spacing", "2",
                              "proportional_float");
  ...
  CPanelAnimationVarAliasType(float, avatar_ypos, "avatar_ypos", "0",
                              "proportional_float");
  CPanelAnimationVarAliasType(float, avatar_xpos, "avatar_xpos", "16",
                              "proportional_float");
  CPanelAnimationVarAliasType(float, avatar_tall, "avatar_tall", "16",
                              "proportional_float");
  CPanelAnimationVarAliasType(float, avatar_wide, "avatar_wide", "16",
                              "proportional_float");

  CPanelAnimationVarAliasType(bool, show_voice_icon, "show_voice_icon", "1",
                              "bool");
  CPanelAnimationVarAliasType(float, voice_icon_ypos, "icon_ypos", "0",
                              "proportional_float");
  CPanelAnimationVarAliasType(float, voice_icon_xpos, "icon_xpos", "24",
                              "proportional_float");
  CPanelAnimationVarAliasType(float, voice_icon_tall, "icon_tall", "16",
                              "proportional_float");
  CPanelAnimationVarAliasType(float, voice_icon_wide, "icon_wide", "16",
                              "proportional_float");

  CPanelAnimationVarAliasType(bool, show_dead_icon, "show_dead_icon", "1",
                              "bool");
  CPanelAnimationVarAliasType(float, dead_icon_ypos, "dead_ypos", "0",
                              "proportional_float");
  CPanelAnimationVarAliasType(float, dead_icon_xpos, "dead_xpos", "0",
                              "proportional_float");
  CPanelAnimationVarAliasType(float, dead_icon_tall, "dead_tall", "16",
                              "proportional_float");
  CPanelAnimationVarAliasType(float, dead_icon_wide, "dead_wide", "16",
                              "proportional_float");

  CPanelAnimationVarAliasType(float, text_xpos, "text_xpos", "40",
                              "proportional_float");

  CPanelAnimationVarAliasType(float, fade_in_time, "fade_in_time", "0.0",
                              "float");
  CPanelAnimationVarAliasType(float, fade_out_time, "fade_out_time", "0.0",
                              "float");
};
...
CHudVoiceStatus::CHudVoiceStatus(const char *pName)
  ...
  if (m_iDeadImageID == -1) // we didn't find it, so create a new one
  {
    ...
    surface()->DrawSetTextureFile(m_iDeadImageID, "hud/leaderboard_dead", true,
                                  false);
  }
}

CHudVoiceStatus::~CHudVoiceStatus() { ClearActiveList(); }
...
void CHudVoiceStatus::Init(void) { ClearActiveList(); }
...
void CHudVoiceStatus::OnThink(void) {
  for (int iPlayerIndex = 1; iPlayerIndex <= gpGlobals->maxClients;
       iPlayerIndex++) {
    ...
    if (activeSpeakerIndex != m_SpeakingList.InvalidIndex()) {
      ...
      //=============================================================================
      // HPE_BEGIN:
      // [Forrest] Don't use UTIL_PlayerByIndex here.  It may be null for some
      // players when a match starts because the server only passes full player
      // info as it affects the client.
      //=============================================================================
      ...
      if (bSpeaking) {
...
#ifdef CSTRIKE_DLL
        // [jpaquin] this allows counter strike to display default avatars for
        // bots.  It can't be a virtual function on C_BasePlayer because there
        // would be no way to get a game specific default image if the player is
        // null.
        ...
        if (engine->GetPlayerInfo(iPlayerIndex, &pi)) {
          if (steamapicontext != NULL &&
            ...
            activeSpeaker.pAvatar->SetAvatarSteamID(steamIDForPlayer,
                                                    k_EAvatarSize32x32);
          }
        }
...
void CHudVoiceStatus::Paint() {
  ...
  FOR_EACH_LL(m_SpeakingList, i) {
    ...
    surface()->DrawSetAlphaMultiplier(oldAlphaMultiplier *
                                      m_SpeakingList[i].fAlpha);

    Color c =
        g_PR->GetTeamColor(g_PR ? g_PR->GetTeam(playerId) : TEAM_UNASSIGNED);
    ...
    if (show_dead_icon && bIsAlive == false && m_iDeadImageID != -1) {
      ...
      // surface()->DrawFilledRect(dead_icon_xpos, ypos, 0, ypos +
      // dead_icon_tall);
      ...
      vert[0].Init(Vector2D(dead_icon_xpos, ypos + dead_icon_ypos),
                   Vector2D(uv1, uv1));
      vert[1].Init(
          Vector2D(dead_icon_xpos + dead_icon_wide, ypos + dead_icon_ypos),
          Vector2D(uv2, uv1));
      vert[2].Init(Vector2D(dead_icon_xpos + dead_icon_wide,
                            ypos + dead_icon_ypos + dead_icon_tall),
                   Vector2D(uv2, uv2));
      vert[3].Init(
          Vector2D(dead_icon_xpos, ypos + dead_icon_ypos + dead_icon_tall),
          Vector2D(uv1, uv2));
    ...
    // Draw the voice icon
    if (show_voice_icon)
      m_pVoiceIcon->DrawSelf(voice_icon_xpos, ypos + voice_icon_ypos,
                             voice_icon_wide, voice_icon_tall, m_clrIcon);

    // Draw the player's name
    surface()->DrawSetTextColor(COLOR_WHITE);
    surface()->DrawSetTextPos(text_xpos,
                              ypos + (item_tall / 2) - (iFontHeight / 2));
    ...
    for (int j = 0; j < iNameLength; j++) {
      iTextWidthCounter +=
          surface()->GetCharacterWidth(m_NameFont, pszconverted[j]);
...
void CHudVoiceStatus::ClearActiveList() {
  FOR_EACH_LL(m_SpeakingList, i) { delete m_SpeakingList[i].pAvatar; }
