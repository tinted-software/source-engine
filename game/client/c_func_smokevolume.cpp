...
IMPLEMENT_CLIENTCLASS_DT(C_FuncSmokeVolume, DT_FuncSmokeVolume,
                         CFuncSmokeVolume)
...
void C_FuncSmokeVolume::OnDataChanged(DataUpdateType_t updateType) {
  ...
  if (updateType == DATA_UPDATE_CREATED) {
    ...
    m_pSmokeParticleInfos =
        new SmokeParticleInfo[m_xCount * m_yCount * m_zCount];
...
void C_FuncSmokeVolume::Update(float fTimeDelta) {
  ...
  // We do this manually because sometimes people make HUGE bboxes, and if
  // they're constantly changing because their particles wander outside the
  // current bounds sometimes, it'll be linking them into all the leaves
  // repeatedly.
  ...
  if (!VectorsAreEqual(curOrigin, m_vLastOrigin, 0.1) ||
      ...
      fabs(curAngles.z - m_vLastAngles.z) > 0.1 || m_bFirstUpdate) {
    ...
    if (pInfo->m_TradeIndex == -1) {
        ...
        for (int yCount = 0; yCount < 3 && !bFound; yCount++) {
          for (int zCount = 0; zCount < 3; zCount++) {
            ...
            if (IsValidXYZCoords(testX, testY, testZ)) {
              SmokeParticleInfo *pOther =
                  GetSmokeParticleInfo(testX, testY, testZ);
              if (pOther->m_pParticle && pOther->m_TradeIndex == -1) {
                // Ok, this one is looking to trade also.
                pInfo->m_TradeIndex =
                    GetSmokeParticleIndex(testX, testY, testZ);
      ...
      if (pInfo < pOther) {
        ...
        if (pInfo->m_TradeClock >= pInfo->m_TradeDuration) {
          ...
          InterpColor(pInfo->m_pParticle->m_Color, pInfo->m_Color,
                      pOther->m_Color, 1 - percent);
          InterpColor(pOther->m_pParticle->m_Color, pInfo->m_Color,
                      pOther->m_Color, percent);

          pInfo->m_pParticle->m_Pos =
              myPos + (otherPos - myPos) * (1 - percent);
...
void C_FuncSmokeVolume::RenderParticles(CParticleRenderIterator *pIterator) {
  ...
  while (pParticle) {
    ...
    // If we're reducing particle cost, only render sufficiently opaque
    // particles
...
void C_FuncSmokeVolume::SimulateParticles(
  ...
  while (pParticle) {
    pParticle->m_CurRotation += pParticle->m_RotationFactor * (M_PI / 180.0f) *
                                m_RotationSpeed * pIterator->GetTimeDelta();
          ...
          if (contents & CONTENTS_SOLID) {
            ...
            if (pParticle) {
              ...
              pParticle->m_ColorInterp =
                  (unsigned char)((rand() * 255) / VALVE_RAND_MAX);
              pParticle->m_RotationFactor =
                  FRand(-1.0f, 1.0f); // Rotation factor.
              pParticle->m_CurRotation =
                  FRand(-m_RotationSpeed, m_RotationSpeed);
            }
            ...
            for (int i = 0; i < NUM_FADE_PLANES; i++) {
              ...
              WorldTraceLine(vPos, vPos + s_FadePlaneDirections[i] * 100,
                             MASK_SOLID_BRUSHONLY, &trace);
              if (trace.fraction < 1.0f) {
                float dist =
                    DotProduct(trace.plane.normal, vPos) - trace.plane.dist;
                if (dist < 0) {
                  pInfo->m_FadeAlpha = 0;
