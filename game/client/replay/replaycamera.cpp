...
ConVar replay_editor_camera_length(
    ...
    "This is the camera length used to simulate camera shake in the replay "
    "editor.  The larger this number, the more the actual position will "
    "change.  It can also be set to negative values.");

// ConVar spec_autodirector( "spec_autodirector", "1", FCVAR_CLIENTDLL |
// FCVAR_CLIENTCMD_CAN_EXECUTE, "Auto-director chooses best view modes while
// spectating" );
...
void C_ReplayCamera::Reset() {
  ...
  m_flRoamingShakeDir =
      Lerp(0.5f, FREE_CAM_SHAKE_DIR_MIN, FREE_CAM_SHAKE_DIR_MAX);
...
void C_ReplayCamera::CalcChaseCamView(Vector &eyeOrigin, QAngle &eyeAngles,
                                      float &fov, float flDelta) {
  bool bManual = true;
...
int C_ReplayCamera::GetMode() { return m_nCameraMode; }
...
void C_ReplayCamera::CalcInEyeCamView(Vector &eyeOrigin, QAngle &eyeAngles,
                                      float &fov, float flDelta) {
  ...
  // This fixes the bug where going from third or first person to free cam
  // defaults to some arbitrary angle, because free cam uses
...
bool C_ReplayCamera::ShouldOverrideView(Vector &origin, QAngle &angles,
                                        float &fov) {
  if (!m_bOverrideView)
    return false;
...
void C_ReplayCamera::CalcRoamingView(Vector &eyeOrigin, QAngle &eyeAngles,
                                     float &fov, float flDelta) {
  ...
  if (!IsPVSLocked()) {
    ...
    Vector wishdir;
    float wishspeed;
    float factor = ShouldUseDefaultRoamingSettings() ? DEFAULT_ROAMING_SPEED
                                                     : m_flRoamingSpeed;
    float maxspeed = sv_maxspeed.GetFloat() * factor;

    AngleVectors(m_aCamAngle, &forward, &right,
  ...
  m_aSmoothedRoamingAngles =
      Lerp(flPercent, m_aSmoothedRoamingAngles, m_aCamAngle);
  ...
  if (!ShouldUseDefaultRoamingSettings() && m_flRoamingShakeAmount > 0.0f) {
    ...
    float flNoiseX =
        Lerp(FractalNoise(Vector(m_flNoiseSample, 0.0f, 0.0f), 1), -1.0f, 1.0f);
    float flNoiseY =
        Lerp(FractalNoise(Vector(0.0f, 1000 + m_flNoiseSample, 0.0f), 1), -1.0f,
             1.0f);

    // Vertical shake
    const float flAmplitudeX =
        m_flRoamingShakeAmount *
        (m_flRoamingShakeDir < 0.0f ? (1.0f + m_flRoamingShakeDir) : 1.0f);
    ...
    // The math below simulates a camera with length
    // "replay_editor_camera_length," so that the camera position bounces around
    // as if it were on someone's shoulder.  If we were to just use angShake at
    // this point with no translation, we would get a camera that looks around
    // but is anchored and doesn't feel quite right.  With the code below, the
    // camera will translate, but be centered around the same point as when
    // camera shake is off completely, rather than actually offsetting that
    // point by the camera length.
    ...
    Vector vCameraOffset =
        vShakeForward * replay_editor_camera_length.GetFloat();
...
void C_ReplayCamera::CalcFixedView(Vector &eyeOrigin, QAngle &eyeAngles,
                                   float &fov, float flDelta) {
  eyeOrigin = m_vCamOrigin;
  eyeAngles = m_aCamAngle;
  fov = m_flFOV;
...
void C_ReplayCamera::SmoothFov(float flDelta) {
  m_flRoamingFov[0] =
      clamp(Lerp(7 * flDelta, m_flRoamingFov[0], m_flRoamingFov[1]),
            //		Approach( m_flRoamingFov[1], m_flRoamingFov[0], 40 *
            //m_flFrameTime ),
...
void C_ReplayCamera::EnableInput(bool bEnable) { m_bInputEnabled = bEnable; }
...
void C_ReplayCamera::ClearOverrideView() {
  ...
  // Set view angles in engine so that CalcRoamingView() won't pop to some
  // stupid angle
  engine->SetViewAngles(m_aCamAngle);
}

void C_ReplayCamera::OverrideView(const Vector *pOrigin, const QAngle *pAngles,
                                  float flFov) {
...
void C_ReplayCamera::CalcView(Vector &origin, QAngle &angles, float &fov) {
  // NOTE ABOUT CLOCKS:  'realtime' is used, because otherwise we can't move the
  // camera round while the game is paused.
...
void C_ReplayCamera::FireGameEvent(IGameEvent *event) {
  ...
  if (Q_strcmp("game_newmap", type) == 0) {
    ...
    if (g_pEngineClientReplay->IsPlayingReplayDemo()) {
      ...
      SetPrimaryTarget((pReplay && pReplay->m_nPlayerSlot >= 0)
                           ? pReplay->m_nPlayerSlot
                           : 0);
    } else {
...
bool C_ReplayCamera::IsPVSLocked() { return false; }
