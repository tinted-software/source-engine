...
void CGenericClassBasedReplay::OnComplete() { BaseClass::OnComplete(); }

bool CGenericClassBasedReplay::ShouldAllowDelete() const {
  return g_pClientReplayContext->GetMovieManager()
             ->GetNumMoviesDependentOnReplay(this) == 0;
}

void CGenericClassBasedReplay::OnDelete() { BaseClass::OnDelete(); }
...
float CGenericClassBasedReplay::GetKillScreenshotDelay() {
  ...
  return replay_screenshotkilldelay.IsValid()
             ? replay_screenshotkilldelay.GetFloat()
             : 0.5f;
}
...
bool CGenericClassBasedReplay::Read(KeyValues *pIn) {
  ...
  AddKillStats(m_vecDominations, pIn, "dominations",
               REPLAY_GAMESTATS_DOMINATIONS);
  AddKillStats(m_vecAssisterDominations, pIn, "assister_dominations",
               REPLAY_GAMESTATS_UNDEFINED);
  AddKillStats(m_vecRevenges, pIn, "revenges", REPLAY_GAMESTATS_REVENGE);
  AddKillStats(m_vecAssisterRevenges, pIn, "assister_revenges",
               REPLAY_GAMESTATS_UNDEFINED);
...
void CGenericClassBasedReplay::AddKillStats(
    CUtlVector<GenericStatInfo_t *> &vecKillStats, KeyValues *pIn,
  ...
  if (pSubKey) {
    FOR_EACH_TRUE_SUBKEY(pSubKey, pCurKillStat) {
      ...
      pNewKillStat->m_nVictimFriendId =
          pCurKillStat->GetInt("victim_friend_id");
      pNewKillStat->m_nAssisterFriendId =
          pCurKillStat->GetInt("assister_friend_id");
...
void CGenericClassBasedReplay::Write(KeyValues *pOut) {
  ...
  WriteKillStatVector(m_vecAssisterDominations, "assister_dominations",
                      "assister_domination", pOut, 2);
  WriteKillStatVector(m_vecRevenges, "revenges", "revenge", pOut, 1);
  WriteKillStatVector(m_vecAssisterRevenges, "assister_revenges",
                      "assister_revenge", pOut, 2);
...
void CGenericClassBasedReplay::WriteKillStatVector(
    CUtlVector<CGenericClassBasedReplay::GenericStatInfo_t *> const &vec,
    const char *pSubKeyName, const char *pElementKeyName, KeyValues *pRootKey,
...
void CGenericClassBasedReplay::AddKill(const char *pPlayerName,
                                       int nPlayerClass) {
  ...
  if (replay_debug.IsValid() && replay_debug.GetBool()) {
    DevMsg("\n\nRecorded kill: name=%s, class=%s (this=%i)\n\n", pPlayerName,
           GetPlayerClass(nPlayerClass), (int)this);
  }
...
void CGenericClassBasedReplay::AddAssisterDomination(int nVictimID,
                                                     int nAssiterID) {
...
void CGenericClassBasedReplay::AddAssisterRevenge(int nVictimID,
                                                  int nAssiterID) {
  AddKillStatFromUserIds(m_vecAssisterRevenges, nVictimID, nAssiterID);
}

void CGenericClassBasedReplay::AddKillStatFromUserIds(
    CUtlVector<GenericStatInfo_t *> &vec, int nVictimId,
    int nAssisterId /*=0*/) {
  uint32 nVictimFriendId;
  if (!GetFriendIdFromUserId(engine->GetPlayerForUserID(nVictimId),
                             nVictimFriendId))
    return;

  uint32 nAssisterFriendId = 0;
  if (nAssisterId &&
      !GetFriendIdFromUserId(engine->GetPlayerForUserID(nAssisterId),
                             nAssisterFriendId))
    return;
...
void CGenericClassBasedReplay::AddKillStatFromFriendIds(
    CUtlVector<GenericStatInfo_t *> &vec, uint32 nVictimFriendId,
    uint32 nAssisterFriendId /*=0*/) {
  GenericStatInfo_t *pNewKillStat = new GenericStatInfo_t;
...
bool CGenericClassBasedReplay::GetFriendIdFromUserId(
    int nPlayerIndex, uint32 &nFriendIdOut) const {
  player_info_t pi;
...
void CGenericClassBasedReplay::DumpGameSpecificData() const {
  ...
  for (int i = 0; i < GetKillCount(); ++i) {
    KillData_t *pCurKill = m_vecKills[i];
    Msg("    kill %i: name=%s class=%s\n", i, pCurKill->m_szPlayerName,
        GetPlayerClass(pCurKill->m_nPlayerClass));
  }
...
void CGenericClassBasedReplay::RecordPlayerDeath(const char *pKillerName,
                                                 int nKillerClass) {
