...
static int s_DoFQualityToSamples[MAX_DOF_QUALITY + 1] = {8, 16,
                                                         32}; //, 64, 128 };

// 4-entry table of values in 2D -1 to +1 range using Poisson disk distribution
static Vector2D g_vJitterTable4[4] = {
    Vector2D(0.5318f, -0.6902f), Vector2D(-0.5123f, 0.8362f),
...
static Vector2D g_vJitterTable8[8] = {
    Vector2D(0.3475f, 0.0042f),   Vector2D(0.8806f, 0.3430f),
...
// 16-entry table of values in 2D -1 to +1 range using Poisson disk distribution
// (disk size 0.38f)
static Vector2D g_vJitterTable16[16] = {
    Vector2D(0.0747f, -0.8341f),  Vector2D(-0.9138f, 0.3251f),
    Vector2D(0.8667f, -0.3029f),  Vector2D(-0.4642f, 0.2187f),
    Vector2D(-0.1505f, 0.7320f),  Vector2D(0.7310f, -0.6786f),
    Vector2D(0.2859f, -0.3254f),  Vector2D(-0.1311f, -0.2292f),
    Vector2D(0.3518f, 0.6470f),   Vector2D(-0.7485f, -0.6307f),
    Vector2D(0.1687f, 0.1873f),   Vector2D(-0.3604f, -0.7483f),
    Vector2D(-0.5658f, -0.1521f), Vector2D(0.7102f, 0.0536f),
    Vector2D(-0.6056f, 0.7747f),  Vector2D(0.7793f, 0.6194f)};

// 32-entry table of values in 2D -1 to +1 range using Poisson disk distribution
// (disk size 0.28f)
static Vector2D g_vJitterTable32[32] = {
    Vector2D(0.0854f, -0.0644f),  Vector2D(0.8744f, 0.1665f),
    Vector2D(0.2329f, 0.3995f),   Vector2D(-0.7804f, 0.5482f),
    Vector2D(-0.4577f, 0.7647f),  Vector2D(-0.1936f, 0.5564f),
    Vector2D(0.4205f, -0.5768f),  Vector2D(-0.0304f, -0.9050f),
    Vector2D(-0.5215f, 0.1854f),  Vector2D(0.3161f, -0.2954f),
    Vector2D(0.0666f, -0.5564f),  Vector2D(-0.2137f, -0.0072f),
    Vector2D(-0.4112f, -0.3311f), Vector2D(0.6438f, -0.2484f),
    Vector2D(-0.9055f, -0.0360f), Vector2D(0.8323f, 0.5268f),
    Vector2D(0.5592f, 0.3459f),   Vector2D(-0.6797f, -0.5201f),
    Vector2D(-0.4325f, -0.8857f), Vector2D(0.8768f, -0.4197f),
    Vector2D(0.3090f, -0.8646f),  Vector2D(0.5034f, 0.8603f),
    Vector2D(0.3752f, 0.0627f),   Vector2D(-0.0161f, 0.2627f),
    Vector2D(0.0969f, 0.7054f),   Vector2D(-0.2291f, -0.6595f),
    Vector2D(-0.5887f, -0.1100f), Vector2D(0.7048f, -0.6528f),
    Vector2D(-0.8438f, 0.2706f),  Vector2D(-0.5061f, 0.4653f),
...
void CAccumBuffProxy::OnBind(void *pC_BaseEntity) {
  ...
  if (pAccumParams->m_bClear) {
    ...
    m_pAccumBuffWeights->SetVecValue(pAccumParams->m_fSampleWeight,
                                     1.0f - pAccumParams->m_fSampleWeight, 0.0f,
                                     0.0f);
  }
}
...
EXPOSE_INTERFACE(CAccumBuffProxy, IMaterialProxy,
                 "accumbuff4sample" IMATERIAL_PROXY_INTERFACE_VERSION);

//-----------------------------------------------------------------------------

CReplayRenderer::CReplayRenderer(CReplayRenderOverlay *pOverlay)
    : m_bIsAudioSyncFrame(false), m_pRenderOverlay(pOverlay),
      m_nCurrentPingPong(0), m_nCurSample(0), m_nTimeStep(0),
      m_curSampleTime(0), m_nFrame(0), m_nNumJitterSamples(0), m_iTgaFrame(0),
      m_pLayoffBuf(NULL), m_pMovie(NULL), m_pMovieMaker(NULL),
      m_pJitterTable(NULL), m_pViewmodelFov(NULL), m_pDefaultFov(NULL),
      m_bCacheFullSceneState(false), m_bShutterClosed(false),
...
const CReplayPerformance *CReplayRenderer::GetPerformance() const {
  ...
  return m_RenderParams.m_iPerformance >= 0
             ? pReplay->GetPerformance(m_RenderParams.m_iPerformance)
             : NULL;
}
...
bool CReplayRenderer::SetupRenderer(RenderMovieParams_t &params,
                                    IReplayMovie *pMovie) {
...
#ifdef REPLAY_RECORDING_ENABLE
  // Record directly to a .wav file if desired via 'startmovie' and write out
  // TGA's
  if (params.m_bExportRaw) {
    ...
    m_fmtTgaRenderDirName =
        g_pClientReplayContext->GetMovieManager()->GetRawExportDir();
    ...
    CFmtStr fmtMovieFullFilename("%s%s", g_pReplayMovieManager->GetRenderDir(),
                                 pMovie->GetMovieFilename());

    bool bSuccess = false;
    if (m_pMovieMaker->CreateNewMovieFile(fmtMovieFullFilename.Access(),
                                          true)) {
      ...
      VideoEncodeGamma_t encodeGamma =
          (QTEncodeGamma.IsValid()) ? (VideoEncodeGamma_t)QTEncodeGamma.GetInt()
                                    : VideoEncodeGamma::GAMMA_2_2;
#else
      VideoEncodeGamma_t encodeGamma = VideoEncodeGamma::GAMMA_2_2;
#endif

      if (m_pMovieMaker->SetMovieVideoParameters(
              Settings.m_Codec, Settings.m_nEncodingQuality,
              (int)Settings.m_nWidth, (int)Settings.m_nHeight, Settings.m_FPS,
              encodeGamma)) {
        if (m_pMovieMaker->SetMovieSourceImageParameters(
                VideoEncodeSourceFormat::BGRA_32BIT, (int)Settings.m_nWidth,
                (int)Settings.m_nHeight)) {
          ...
          if (m_pMovieMaker->SetMovieSourceAudioParameters(
                  AudioEncodeSourceFormat::AUDIO_16BIT_PCMStereo, 44100,
                  audioOptions)) {
            bSuccess = true;
          }
        }
...
void CReplayRenderer::InitBuffers(const RenderMovieParams_t &params) {
  ...
  m_AccumBuffSample.Init(g_pMaterialSystem->CreateNamedRenderTargetTextureEx2(
      "_rt_Replay_Accum_Sample", Settings.m_nWidth, Settings.m_nHeight,
      RT_SIZE_OFFSCREEN, AccumSampleFormat, MATERIAL_RT_DEPTH_SHARED,
  ...
  m_AccumBuffPingPong[0].Init(
      g_pMaterialSystem->CreateNamedRenderTargetTextureEx2(
          "_rt_Replay_Ping", Settings.m_nWidth, Settings.m_nHeight,
          RT_SIZE_OFFSCREEN, PingPongFormat, MATERIAL_RT_DEPTH_NONE,
          TEXTUREFLAGS_CLAMPS | TEXTUREFLAGS_CLAMPT |
              TEXTUREFLAGS_POINTSAMPLE));
  m_AccumBuffPingPong[1].Init(
      g_pMaterialSystem->CreateNamedRenderTargetTextureEx2(
          "_rt_Replay_Pong", Settings.m_nWidth, Settings.m_nHeight,
          RT_SIZE_OFFSCREEN, PingPongFormat, MATERIAL_RT_DEPTH_NONE,
          TEXTUREFLAGS_CLAMPS | TEXTUREFLAGS_CLAMPT |
              TEXTUREFLAGS_POINTSAMPLE));
  ...
  m_LayoffResult.Init(g_pMaterialSystem->CreateNamedRenderTargetTextureEx2(
      "_rt_LayoffResult", Settings.m_nWidth, Settings.m_nHeight,
      RT_SIZE_OFFSCREEN, g_pMaterialSystem->GetBackBufferFormat(),
      MATERIAL_RT_DEPTH_SHARED,
      TEXTUREFLAGS_BORDER | TEXTUREFLAGS_POINTSAMPLE));

  g_pMaterialSystem->EndRenderTargetAllocation(); // Begin allocating RTs which
  ...
  KeyValues *pProxiesKV =
      pVMTKeyValues->FindKey("proxies", true);   // create a subkey
...
void CReplayRenderer::ShutdownRenderer() {
  ...
  if (m_pMovieMaker) {
    ...
    CFmtStr fmtTmpFilename("%s%c%s.wav", engine->GetGameDirectory(),
                           CORRECT_PATH_SEPARATOR, TMP_WAVE_FILENAME);
    CFmtStr fmtDstFilename("%s%s", m_fmtTgaRenderDirName.Access(), "audio.wav");
    g_pFullFileSystem->RenameFile(fmtTmpFilename.Access(),
                                  fmtDstFilename.Access());
  }
...
void CReplayRenderer::BeginRenderingSample(int nSample, int x, int y,
  ...
  CMatRenderContextPtr pRenderContext(g_pMaterialSystem);
  pRenderContext->PushRenderTargetAndViewport(m_AccumBuffSample, x, y, nWidth,
                                              nHeight);
}

void CReplayRenderer::ResolveSamples(int nSample, DmeTime_t frametime, int x,
  ...
  pRenderContext->PushRenderTargetAndViewport(
      m_AccumBuffPingPong[m_nCurrentPingPong], x, y, nWidth, nHeight);
  ...
  if (bLayoffResult) {
    ...
    pRenderContext->PushRenderTargetAndViewport(m_LayoffResult, x, y, nWidth,
                                                nHeight);
    DrawResolvingQuad(nWidth, nHeight);
    pRenderContext->PopRenderTargetAndViewport();
  }
  ...
  m_nCurrentPingPong =
      (m_nCurrentPingPong + 1) % 2; // Flip the ping-pong buffers
}
...
float CReplayRenderer::GetViewModelFOVOffset() {
  //	float flVMDefaultFov = m_pViewmodelFov ? m_pViewmodelFov->GetFloat()
  //: 54.0f;
...
void CReplayRenderer::SetupSampleView(int x, int y, int w, int h, int nSample,
  ...
  float t =
      tan(fHalfAngleRadians) * (viewSetup.m_flAspectRatio / (4.0f / 3.0f));
  ...
  SetupDOFMatrixSkewView(viewSetup.origin, viewSetup.angles, nSample,
                         viewSetup); // Sheared matrix method more comparable to
                                     // image-space DoF approximation
...
void CReplayRenderer::SetupDOFMatrixSkewView(const Vector &pos,
                                             const QAngle &angles, int nSample,
                                             CViewSetup &viewSetup) {
  ...
  // Set up a non-skewed off-center projection matrix to start with...  (Posters
  ...
  if (IsAntialiasingEnabled() && !IsDepthOfFieldEnabled() &&
      !m_bForceCheapDoF) // AA jitter but no DoF
  {
    Vector2D vAAJitter = m_pJitterTable[nSample % m_nNumJitterSamples];
    const float fHalfPixelRadius = 0.65;
    viewSetup.m_flOffCenterBottom +=
        (vAAJitter.y / (float)viewSetup.height) * fHalfPixelRadius;
    viewSetup.m_flOffCenterTop +=
        (vAAJitter.y / (float)viewSetup.height) * fHalfPixelRadius;
    viewSetup.m_flOffCenterLeft +=
        (vAAJitter.x / (float)viewSetup.width) * fHalfPixelRadius;
...
int CReplayRenderer::NumMotionBlurTimeSteps() const {
  return (IsMotionBlurEnabled())
             ? GetNumMotionBlurTimeSteps(GetMotionBlurQuality())
             : 1;
}
...
void CReplayRenderer::ComputeSampleCounts(int *pNSamplesPerTimeStep,
...
void CReplayRenderer::CompositeAndLayoffFrame(int nFrame) {
  ...
  // If we aren't doing motion blur, just render the frame and add it to the
  // video
  if (!IsMotionBlurEnabled()) {
  ...
  if (m_bShutterClosed) {
    ...
    // If nMotionBlurTimeSteps subframes have passed, open the shutter for the
    // next frame.
  ...
  // scope to avoid compiler warnings
  {
    ...
    // Loop through all samples for the current timestep, jittering the camera
    ...
    if (m_nTimeStep == nMotionBlurTimeSteps) {
      ...
      m_bShutterClosed =
          true; // Close or open the shutter for nMotionBlurTimeSteps subframes
      bAppendToMovie =
          true; // Add a frame to the movie we've just closed the shutter
    }
  }
...
void CReplayRenderer::LayoffFrame(int nFrame) {
  ...
  // We're rendering into the full preview size, but stretching down to the
  // actual size
  Rect_t srcRect;
  srcRect.x = 0;
  srcRect.y = 0;
  ...
  // 	pRenderContext->ReadPixelsAndStretch( &srcRect, &dstRect, (unsigned
  // char*)m_pLayoffBuf, 		IMAGE_FORMAT_BGRA8888, dstRect.width *
  // ImageLoader::SizeInBytes( IMAGE_FORMAT_BGRA8888 ) );

  pRenderContext->ReadPixels(0, 0, (int)m_RenderParams.m_Settings.m_nWidth,
                             (int)m_RenderParams.m_Settings.m_nHeight,
                             (unsigned char *)m_pLayoffBuf,
  ...
  if (m_RenderParams.m_bExportRaw) {
    CUtlBuffer bufOut;
    if (TGAWriter::WriteToBuffer((unsigned char *)m_pLayoffBuf, bufOut,
                                 m_RenderParams.m_Settings.m_nWidth,
                                 m_RenderParams.m_Settings.m_nHeight,
                                 IMAGE_FORMAT_BGRA8888, IMAGE_FORMAT_RGB888)) {
      // Format filename and write the TGA
      CFmtStr fmtFilename("%sFrame_%04i.tga", m_fmtTgaRenderDirName.Access(),
                          m_iTgaFrame++);
    ...
    VPROF_BUDGET("CReplayRenderer::LayoffFrame - AppendVideoFrame",
                 VPROF_BUDGETGROUP_REPLAY);
...
void CReplayRenderer::GetViewSetup(CViewSetup &viewsetup) {
  ...
  // HACK: Override the view - this will keep the view from popping if the user
  // toggles the render preview checkbox.
  ReplayCamera()->CalcView(viewsetup.origin, viewsetup.angles, viewsetup.fov);
  viewsetup.fovViewmodel = ScaleFOVByWidthRatio(
      v_viewmodel_fov.GetFloat(), viewsetup.m_flAspectRatio / (4.0f / 3.0f));
}

void CReplayRenderer::RenderLayoffFrame(DmeTime_t time, int nCurSample,
                                        int nNumTotalSamples) {
  CViewSetup viewSetup;
  GetViewSetup(viewSetup);

  int x = 0, y = 0, w = m_RenderParams.m_Settings.m_nWidth,
      h = m_RenderParams.m_Settings.m_nHeight;

  // FIXME: Using the preview size here is something of a hack
  // to get layoff working again. We're actually going to stretch down from the
  // preview size to layoff size during frame capture
...
void CReplayRenderer::ClearToBlack(CTextureReference &buf, int x, int y,
                                   int nWidth, int nHeight) {
  CMatRenderContextPtr pRenderContext(g_pMaterialSystem);

  // Bind the resolving material
  AccumParams_t accParms = {m_AccumBuffSample, m_AccumBuffSample, 0.0f,
                            true}; // true to clear to black
  pRenderContext->Bind(m_FourSampleResolveMatRef, &accParms);
...
void CReplayRenderer::RenderAudio(unsigned char *pBuffer, int nSize,
                                  int nNumSamples) {
#ifdef REPLAY_RECORDING_ENABLE
  if (m_RenderParams.m_bExportRaw) {
    g_pEngineClientReplay->Wave_AppendTmpFile(TMP_WAVE_FILENAME, pBuffer,
                                              nNumSamples);
  } else if (m_pMovieMaker) {
