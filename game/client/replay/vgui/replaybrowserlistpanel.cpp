...
#define MAX_MOVIE_THUMBNAILS                                                   \
  12 // The remaining movies will be put into a single collection

//-----------------------------------------------------------------------------

CReplayListPanel::CReplayListPanel(Panel *pParent, const char *pName)
    : BaseClass(pParent, pName), m_pPrevHoverPanel(NULL),
      m_pPreviewPanel(NULL) {
  ivgui()->AddTickSignal(GetVPanel(), 10);

  m_pBorderArrowImg = new ImagePanel(this, "ArrowImage");

  // Add replays and movies collections, which will contain all replays &
  // movies.
  m_pReplaysCollection = new CReplayThumbnailCollection(
      this, "ReplayThumbnailCollection", GetReplayItemManager());
  m_pMoviesCollection = new CMovieThumbnailCollection(
      this, "MovieThumbnailCollection", GetReplayMovieItemManager(), true);
...
void CReplayListPanel::ApplySchemeSettings(vgui::IScheme *pScheme) {
  ...
  vgui::ScrollBar *pScrollBar =
      dynamic_cast<vgui::ScrollBar *>(FindChildByName("PanelListPanelVScroll"));
  pScrollBar->SetScrollbarButtonsVisible(false);
  Color clrButtonColor =
      GetSchemeColor("Yellow", Color(255, 255, 255, 255), pScheme);
  Color clrBgColor =
...
void CReplayListPanel::PerformLayout() { BaseClass::PerformLayout(); }
...
void CReplayListPanel::SetupBorderArrow(bool bLeft) {
  ...
  m_pBorderArrowImg->SetImage(bLeft ? "replay/replay_balloon_arrow_left"
                                    : "replay/replay_balloon_arrow_right");
  m_pBorderArrowImg->SetZPos(1000);

  m_pBorderArrowImg->GetImage()->GetContentSize(m_aBorderArrowDims[0],
                                                m_aBorderArrowDims[1]);
...
void CReplayListPanel::ApplyFilter(const wchar_t *pFilterText) {
  ...
  FOR_EACH_VEC(m_vecCollections, i) { m_vecCollections[i]->OnUpdated(); }
...
void CReplayListPanel::OnItemPanelEntered(vgui::Panel *pPanel) {
  CReplayBrowserThumbnail *pThumbnail =
      dynamic_cast<CReplayBrowserThumbnail *>(pPanel);
  ...
  if (IsVisible() && pThumbnail && pThumbnail->IsVisible()) {
    ...
    IQueryableReplayItem *pReplayItem =
        FindReplayItem(pThumbnail->m_hReplayItem, &pItemManager);
    ...
    if (pReplayItem->IsItemAMovie()) {
      m_pPreviewPanel = new CReplayPreviewPanelBase(
          this, pReplayItem->GetItemHandle(), pItemManager);
    } else {
      m_pPreviewPanel = new CReplayPreviewPanelSlideshow(
          this, pReplayItem->GetItemHandle(), pItemManager);
    }
    ...
    int nXPos, nYPos;
    int nOffset = XRES(1);
    nXPos = (x > GetWide() / 2) ? (x - m_pPreviewPanel->GetWide() - nOffset)
                                : (x + pThumbnail->GetWide() + nOffset);
    ...
    SetupBorderArrow(bLeftArrow); // Sets proper image and caches image dims in
                                  // m_aBorderArrowDims
    int nArrowXPos, nArrowYPos;
    const int nPreviewBorderWidth =
        2; // Should be just big enough to cover the preview's border width
    if (bLeftArrow) {
    ...
    nArrowYPos = MIN(nYPos + m_pPreviewPanel->GetTall() -
                         m_pBorderArrowImg->GetTall() * 2,
                     y + (pThumbnail->m_pScreenshotThumb->GetTall() -
                          m_aBorderArrowDims[1]) /
                             2);
...
CBaseThumbnailCollection *CReplayListPanel::FindOrAddReplayThumbnailCollection(
...
void CReplayListPanel::AddReplaysToList() {
  ...
  FOR_EACH_VEC(m_vecCollections, i) { AddItem(NULL, m_vecCollections[i]); }
}

void CReplayListPanel::RemoveCollection(CBaseThumbnailCollection *pCollection) {
  // Never remove our two base collections. If they have no entries, they
...
bool CReplayListPanel::PassesFilter(IQueryableReplayItem *pItem) {
  CGenericClassBasedReplay *pReplay =
      ToGenericClassBasedReplay(pItem->GetItemReplay());
  if (!pReplay)
    return false;
  ...
  V_wcscat_safe(wszSearchableText, L" ");
  // player class
  g_pVGuiLocalize->ConvertANSIToUnicode(pReplay->GetPlayerClass(), wszTemp,
                                        sizeof(wszTemp));
  V_wcscat_safe(wszSearchableText, wszTemp);
  V_wcscat_safe(wszSearchableText, L" ");
  // killer name
  if (pReplay->WasKilled()) {
    g_pVGuiLocalize->ConvertANSIToUnicode(pReplay->GetKillerName(), wszTemp,
                                          sizeof(wszTemp));
    V_wcscat_safe(wszSearchableText, wszTemp);
    V_wcscat_safe(wszSearchableText, L" ");
  }
...
void CReplayListPanel::AddReplayItem(ReplayItemHandle_t hItem) {
  ...
  CBaseThumbnailCollection *pCollection =
      FindOrAddReplayThumbnailCollection(pItem, pItemManager);
...
void CReplayListPanel::CleanupUIForReplayItem(ReplayItemHandle_t hReplayItem) {
  ...
  const IQueryableReplayItem *pReplayItem =
      FindReplayItem(hReplayItem, &pItemManager);
  ...
  // Find the collection associated with the given replay - NOTE: we pass false
  // here for the "bAddIfNotFound" param
  if (!pCollection) {
    AssertMsg(0, "REPLAY: Should have found collection while attempting to "
                 "delete a replay from the browser.");
    return;
  }

  // Clear the previous hover pointer to avoid a potential crash where we've got
  // a stale ptr
  m_pPrevHoverPanel = NULL;

  // Clear out the preview panel if it exists and is for the given replay
  // necessary
  if (m_pPreviewPanel && m_pPreviewPanel->GetReplayHandle() ==
                             pReplayItem->GetItemReplayHandle()) {
    ClearPreviewPanel();
    m_pBorderArrowImg->SetVisible(false);
  }
