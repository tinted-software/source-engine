...
CKeyValueLabelPanel::CKeyValueLabelPanel(Panel *pParent, const char *pKey,
                                         const char *pValue)
...
void CKeyValueLabelPanel::ApplySchemeSettings(IScheme *pScheme) {
  ...
  HFont hFont =
      scheme()->GetIScheme(GetScheme())->GetFont("ReplayBrowserSmallest", true);
...
CBaseDetailsPanel::CBaseDetailsPanel(Panel *pParent, const char *pName,
...
void CRecordsPanel::PerformLayout() {
  ...
  const CGenericClassBasedReplay *pReplay =
      GetGenericClassBasedReplay(m_hReplay);
  V_snprintf(szImage, sizeof(szImage), "class_sel_sm_%s_%s",
             pReplay->GetMaterialFriendlyPlayerClass(),
  ...
  if (pImage) {
    ...
    // Compute height of the records panel as a little smaller than the image
    // itself
    nHeight = nImageHeight * 11 / 16;

    // Setup the image panel - parent to records panel parent so it goes out of
    // the records panel's bounds a bit
    const int nMargin = 7;
    ...
    int nX, nY;
    GetPos(nX, nY);
    m_pClassImage->SetBounds(nX + nMargin,
                             nY - flScale * nImageHeight + GetTall() - nMargin,
                             nImageWidth * flScale, nImageHeight * flScale);
...
CStatsPanel::CStatsPanel(Panel *pParent, ReplayHandle_t hReplay)
  ...
  for (int i = 0; i < REPLAY_MAX_DISPLAY_GAMESTATS; ++i) {
    ...
    m_paStatLabels[i] = new CKeyValueLabelPanel(
        GetInset(), g_pReplayDisplayGameStats[i].m_pStatLocalizationToken,
        szValue);
...
CDominationsPanel::CDominationsPanel(Panel *pParent, ReplayHandle_t hReplay)
  ...
  V_snprintf(szImage, sizeof(szImage), "../hud/leaderboard_dom%i",
             nNumDominations);
  ...
  if (steamapicontext && steamapicontext->SteamUtils() &&
    ...
    for (int i = 0; i < nNumDominations; ++i) {
      ...
      CSteamID id(pReplay->GetDomination(i)->m_nVictimFriendId, 1,
                  steamapicontext->SteamUtils()->GetConnectedUniverse(),
...
CKillsPanel::CKillsPanel(Panel *pParent, ReplayHandle_t hReplay)
  ...
  if (pReplay) {
    for (int i = 0; i < pReplay->GetKillCount(); ++i) {
      ...
      char const *pClass =
          pKill->m_nPlayerClass == TF_CLASS_DEMOMAN
              ? "demo"
              : g_aPlayerClassNames_NonLocalized[pKill->m_nPlayerClass];

      V_snprintf(szImgPath, sizeof(szImgPath), "../hud/leaderboard_class_%s",
                 pClass);
  ...
  // Create labels
  m_pKillLabels =
      new CKeyValueLabelPanel(GetInset(), "#Replay_Kills", szKillCount);
}
...
CBasicLifeInfoPanel::CBasicLifeInfoPanel(Panel *pParent, ReplayHandle_t hReplay)
  ...
  // Create labels
  CGenericClassBasedReplay *pReplay = GetGenericClassBasedReplay(hReplay);
  m_pKilledByLabels = new CKeyValueLabelPanel(
      GetInset(), "#Replay_StatKilledBy",
      pReplay->WasKilled() ? pReplay->GetKillerName() : "#Replay_None");
  m_pMapLabels = new CKeyValueLabelPanel(
      GetInset(), "#Replay_OnMap", GetMapDisplayName(pReplay->m_szMapName));
  m_pLifeLabels = new CKeyValueLabelPanel(
      GetInset(), "#Replay_Life",
      CReplayTime::FormatTimeString((int)pReplay->m_flLength));
}
...
void CBasicLifeInfoPanel::PerformLayout() {
  ...
  m_pMapLabels->SetBounds(0, m_pKilledByLabels->GetTall() + nBuffer, GetWide(),
                          m_pMapLabels->GetHeight());

  int nLifeLabelsY = (m_pMapLabels->GetTall() + nBuffer) * 2;
  m_pLifeLabels->SetBounds(0, nLifeLabelsY, GetWide(),
                           m_pLifeLabels->GetHeight());
...
CYouTubeInfoPanel::CYouTubeInfoPanel(Panel *pParent)
    : CBaseDetailsPanel(pParent, "YouTubeInfo", NULL), m_pLabels(NULL) {
  m_pLabels = new CKeyValueLabelPanel(GetInset(), "#Replay_YouTube",
                                      g_pVGuiLocalize->Find("YouTube_NoStats"));
}
...
CTitleEditPanel::CTitleEditPanel(Panel *pParent,
                                 QueryableReplayItemHandle_t hReplayItem,
                                 IReplayItemManager *pItemManager)
    : EditablePanel(pParent, "TitleEditPanel"), m_hReplayItem(hReplayItem),
      m_pItemManager(pItemManager), m_bMouseOver(false), m_pTitleEntry(NULL),
      m_pHeaderLine(NULL), m_pClickToEditLabel(NULL), m_pCaratLabel(NULL) {
  ivgui()->AddTickSignal(GetVPanel(), 10);
}

CTitleEditPanel::~CTitleEditPanel() { ivgui()->RemoveTickSignal(GetVPanel()); }
...
void CTitleEditPanel::ApplySchemeSettings(IScheme *pScheme) {
  ...
  m_pClickToEditLabel =
      dynamic_cast<CExLabel *>(FindChildByName("ClickToEditLabel"));
...
void CTitleEditPanel::PerformLayout() {
  ...
  m_pClickToEditLabel->SetBounds(
      ...
      nTitleEntryY + (nTitleEntryTall - nClickToEditH) / 2, nClickToEditW,
      nClickToEditH);
...
void CTitleEditPanel::PaintBackground() {
  ...
  bool bDrawExtraStuff =
      !vgui::input()->GetAppModalSurface() &&
      (m_bMouseOver ||
       bEditing); // Don't draw extra stuff when render dialog is up
  ...
  if (bDrawExtraStuff) {
    // Use the game UI panel here, since using this panel's vpanel
    // (PushMakeCurrent() is set in Panel::PaintTraverse(), the function that
    // calls PaintBackground()) causes dimmed top and left lines.  Using the
    // game UI panel allows painting outside of the text entry itself.
    vgui::VPANEL vGameUI = enginevgui->GetPanel(PANEL_GAMEUIDLL);

    // Calculate title entry rect (x0,y0,x1,y1) - move the absolute upper-left
    // corner 1 pixel left & up
    int aTitleRect[4];
    ipanel()->GetAbsPos(m_pTitleEntry->GetVPanel(), aTitleRect[0],
                        aTitleRect[1]);
    ...
    surface()->DrawFilledRect(aTitleRect[0], aTitleRect[1], aTitleRect[2],
                              aTitleRect[3]);

    // Draw stroke
    surface()->DrawSetColor(Color(202, 190, 164, 255));
    surface()->DrawLine(aTitleRect[0], aTitleRect[1], aTitleRect[2],
                        aTitleRect[1]); // Top
    surface()->DrawLine(aTitleRect[0], aTitleRect[1], aTitleRect[0],
                        aTitleRect[3]); // Left
    surface()->DrawLine(aTitleRect[0], aTitleRect[3], aTitleRect[2],
                        aTitleRect[3]); // Bottom
    surface()->DrawLine(aTitleRect[2], aTitleRect[1], aTitleRect[2],
                        aTitleRect[3]); // Right
  ...
  if (m_pTitleEntry->HasFocus() && pReplayItem) {
    if (code == KEY_ESCAPE) {
      ...
      } else // Save title...
      {
        ...
        // Save!
        g_pReplayManager->FlagReplayForFlush(pReplayItem->GetItemReplay(),
                                             true);
        ...
        if (pUserData) {
          CReplayBrowserThumbnail *pThumbnail =
              (CReplayBrowserThumbnail *)pUserData;
...
CPlaybackPanel::~CPlaybackPanel() { ivgui()->RemoveTickSignal(GetVPanel()); }
...
void CPlaybackPanel::PerformLayout() { BaseClass::PerformLayout(); }

//-----------------------------------------------------------------------------

CPlaybackPanelSlideshow::CPlaybackPanelSlideshow(Panel *pParent,
                                                 ReplayHandle_t hReplay)
    : CPlaybackPanel(pParent), m_hReplay(hReplay) {
  m_pScreenshotImage =
      new CReplayScreenshotSlideshowPanel(this, "Screenshot", hReplay);
}

void CPlaybackPanelSlideshow::ApplySchemeSettings(IScheme *pScheme) {
  BaseClass::ApplySchemeSettings(pScheme);

  LoadControlSettings("resource/ui/replaybrowser/playbackpanelslideshow.res",
                      "GAME");

  m_pNoScreenshotLabel =
      dynamic_cast<CExLabel *>(FindChildByName("NoScreenshotLabel"));
  ...
  if (!pReplay->GetScreenshotCount() &&
      m_pNoScreenshotLabel) // Show no-screenshot label
  {
...
void CPlaybackPanelSlideshow::PerformLayout() {
  ...
  if (m_pScreenshotImage) {
    m_pScreenshotImage->SetBounds(nMarginWidth, nMarginWidth, nScreenshotWidth,
                                  GetTall() - 2 * nMarginWidth);

    // Setup screenshot scale based on width of first screenshot (if there are
    // any screenshots at all) - otherwise don't scale
    float flScale = pReplay->GetScreenshotCount() == 0
                        ? 1.0f
                        : ((float)nScreenshotWidth /
                           (.95f * pReplay->GetScreenshot(0)->m_nWidth));
  ...
  m_pNoScreenshotLabel->SetBounds(0, (GetTall() - nLabelH) / 2, GetWide(),
                                  nLabelH);
}
...
CPlaybackPanelMovie::CPlaybackPanelMovie(Panel *pParent, ReplayHandle_t hMovie)
  ...
  m_pMoviePlayerPanel =
      new CMoviePlayerPanel(this, "MoviePlayer", pMovie->GetMovieFilename());
...
IBorder *CCutImagePanel::GetBorder(bool bDepressed, bool bArmed, bool bSelected,
                                   bool bKeyFocus) {
  if (bSelected) {
...
CCutsPanel::CCutsPanel(Panel *pParent, ReplayHandle_t hReplay,
                       int iSelectedPerformance)
    : BaseClass(pParent, "CutsPanel", hReplay), m_iPage(0),
      m_nVisibleButtons(0), m_pVerticalLine(NULL), m_pNoCutsLabel(NULL),
      m_pOriginalLabel(NULL), m_pCutsLabel(NULL) {
  ...
  FOR_EACH_BUTTON(i) {
    ...
    CExButton *pAddToRenderQueueButton =
        new CExButton(pNewButton, "AddToRenderQueue", "+", this);
  ...
  // Calculate page
  SetPage((1 + iSelectedPerformance) / BUTTONS_PER_PAGE,
...
void CCutsPanel::ApplySchemeSettings(IScheme *pScheme) {
  ...
  m_pVerticalLine =
      dynamic_cast<EditablePanel *>(FindChildByName("VerticalLine"));
...
void CCutsPanel::ApplySettings(KeyValues *pInResourceData) {
  ...
  if (pButtonSettings) {
    KeyValues *pAddToRenderQueueButtonSettings =
        pButtonSettings->FindKey("addtorenderqueuebutton_settings");
    ...
    FOR_EACH_BUTTON(i) {
      ...
      if (pReplay && pReplay->GetScreenshotCount()) {
        const float flScale =
            (float)m_nCutButtonHeight / pReplay->GetScreenshot(0)->m_nHeight;
        int nImageWidth = m_nCutButtonWidth - 2 * m_nCutButtonBuffer;
        int nImageHeight = m_nCutButtonHeight - 2 * m_nCutButtonBuffer;

        CFmtStr fmtFile("replay\\thumbnails\\%s",
                        pReplay->GetScreenshot(0)->m_szBaseFilename);
        pCurButton->SetSubImage(fmtFile.Access());
        pCurButton->GetImage()->SetScaleAmount(flScale);
        pCurButton->GetImage()->SetBounds(
            m_nCutButtonBuffer, m_nCutButtonBuffer, nImageWidth, nImageHeight);
      }
...
void CCutsPanel::PerformLayout() {
  ...
  FOR_EACH_BUTTON(i) {
    ...
    pCurButton->SetBounds(nX, m_nButtonStartY, m_nCutButtonWidth,
                          m_nCutButtonHeight);
  ...
  const int nRightOfVerticalLineX = m_nCutButtonWidth +
                                    m_nCutButtonSpaceWide * 2 +
                                    m_pVerticalLine->GetWide();
  ...
  if (m_pCutsLabel) {
    ...
    m_pCutsLabel->SetPos(m_nCutButtonWidth + 2 * m_nCutButtonSpaceWide +
                             m_pVerticalLine->GetWide(),
                         0);
  }
...
void CCutsPanel::OnTick() {
  ...
  FOR_EACH_BUTTON(i) {
    ...
    if (!bFoundHoverButton && pCurButton->IsWithin(nMouseX, nMouseY) &&
        pCurButton->IsVisible()) {
    ...
    CExButton *pAddToRenderQueueButton =
        m_aButtons[i].m_pAddToRenderQueueButton;
    if (pAddToRenderQueueButton) {
      pAddToRenderQueueButton->SetVisible(bIsHoverButton);

      if (iHoverPerformance >= -1) {
        // Set the text and command based on whether or not the take's already
        // been queued
        const bool bInQueue =
            g_pClientReplayContext->GetRenderQueue()->IsInQueue(
                m_hReplay, iHoverPerformance);
        CFmtStr fmtCmd("%srenderqueue_%i", bInQueue ? "removefrom" : "addto",
                       iHoverPerformance);
  ...
  // If the mouse is over a performance button, use that, otherwise use the
...
void CCutsPanel::OnCommand(const char *pCommand) {
  if (!V_strnicmp(pCommand, "select_", 7)) {
    ...
    if (!replay_renderqueue_first_add.GetInt()) {
      ShowMessageBox("#Replay_FirstRenderQueueAddTitle",
                     "#Replay_FirstRenderQueueAddMsg", "#GameUI_OK");
...
void CCutsPanel::UpdateNameLabel(int iPerformance) {
  ...
  const int iPerformanceButton =
      PerformanceToButton(iPerformance); // Not necessarily the selected button
                                         // - can be hover button

  // Get position of the button so we can use it's x position.
  int aSelectedButtonPos[2];
  m_aButtons[iPerformanceButton].m_pButton->GetPos(aSelectedButtonPos[0],
                                                   aSelectedButtonPos[1]);

  if (m_pNameLabel) {
    const int nNameLabelX = aSelectedButtonPos[0];
    const int nNameLabelY =
        m_nButtonStartY + m_nCutButtonHeight + m_nNameLabelTopMargin;
    m_pNameLabel->SetBounds(nNameLabelX, nNameLabelY, GetWide() - nNameLabelX,
                            GetTall() - nNameLabelY);
  }
}
...
void CCutsPanel::OnPerformanceDeleted(int iPerformance) {
  ...
  CReplay *pReplay = GetGenericClassBasedReplay(m_hReplay);
  const int nNumCuts = pReplay->GetPerformanceCount();
  if (iPerformance == m_aButtons[0].m_iPerformance &&
      iPerformance == nNumCuts) {
    SetPage(m_iPage - 1, BUTTONS_PER_PAGE - 1);
  } else {
    SelectButtonFromPerformance(
        ButtonToPerformance(MIN(m_nVisibleButtons - 1, MAX(0, iButton))));
  }
...
static void ConfirmUploadMovie(bool bConfirmed, void *pContext) {
  if (bConfirmed) {
    ...
    IQueryableReplayItem *pReplayItem =
        pPanel->m_pItemManager->GetItem(pPanel->m_hReplayItem);
...
public:
  ...
  static bool GetEmptyElementTagContents(const char *pXML, const char *pTag,
                                         CUtlString &strTagContents) {
  ...
  static bool GetEmptyTagValue(const char *pTagContents, const char *pKeyName,
                               CUtlString &value) {
  ...
  virtual void HandleResponse(long responseCode, const char *pResponse) {
    ...
    if (strstr(pResponse, "<internalReason>Private video</internalReason>") !=
        NULL) {
      m_pPanel->m_pYouTubeInfoPanel->SetInfo(
          g_pVGuiLocalize->Find("#YouTube_PrivateVideo"));
    ...
    if (GetEmptyElementTagContents(pResponse, "yt:statistics",
                                   strTagStatistics)) {
      ...
      g_pVGuiLocalize->ConvertANSIToUnicode(favoriteCount.Get(), wszFavorited,
                                            sizeof(wszFavorited));
      g_pVGuiLocalize->ConvertANSIToUnicode(viewCount.Get(), wszViews,
                                            sizeof(wszViews));
    }
    ...
    if (GetEmptyElementTagContents(pResponse, "yt:rating", strTagRating)) {
      ...
      g_pVGuiLocalize->ConvertANSIToUnicode(likes.Get(), wszLikes,
                                            sizeof(wszLikes));
    }

    // const char *kLinkStartTag = "<link rel='alternate' type='text/html'
    // href='";
    CUtlString strTagLink;
    if (GetEmptyElementTagContents(pResponse, "link rel='alternate'",
                                   strTagLink)) {
      GetEmptyTagValue(strTagLink, "href", m_strVideoURL);
    }

    wchar_t wszStats[256] = L"";
    g_pVGuiLocalize->ConstructString(wszStats, sizeof(wszStats),
                                     g_pVGuiLocalize->Find("#YouTube_Stats"), 3,
                                     wszFavorited, wszViews, wszLikes);

    if (m_strVideoURL.IsEmpty() == false) {
      m_pPanel->m_pYouTubeInfoPanel->SetInfo(wszStats);
      m_pPanel->SetYouTubeStatus(
          CReplayDetailsPanel::kYouTubeStatus_RetrievedInfo);
      ...
      m_pPanel->m_pYouTubeInfoPanel->SetInfo(
          g_pVGuiLocalize->Find("#YouTube_CouldNotRetrieveStats"));
      m_pPanel->SetYouTubeStatus(
          CReplayDetailsPanel::kYouTubeStatus_CouldNotRetrieveInfo);
    }
  }
...
CReplayDetailsPanel::CReplayDetailsPanel(
    Panel *pParent, QueryableReplayItemHandle_t hReplayItem, int iPerformance,
    IReplayItemManager *pItemManager)
    : EditablePanel(pParent, "DetailsPanel"), m_hReplayItem(hReplayItem),
      m_pItemManager(pItemManager), m_pCutsPanel(NULL),
      m_iSelectedPerformance(iPerformance), m_pYouTubeResponseHandler(NULL),
  ...
  m_pInsetPanel = new EditablePanel(this, "InsetPanel");
  m_pTitleEditPanel =
      new CTitleEditPanel(GetInset(), m_hReplayItem, m_pItemManager);
  ...
  m_pScrollPanel = new vgui::ScrollableEditablePanel(GetInset(), m_pInfoPanel,
                                                     "StatsScroller");
  ...
  if (bIsMoviePanel) {
    ...
    m_pCutsPanel =
        new CCutsPanel(GetInset(), m_hReplay, m_iSelectedPerformance);
  }
...
void CReplayDetailsPanel::ApplySchemeSettings(IScheme *pScheme) {
  ...
  m_pExportMovie =
      dynamic_cast<CExButton *>(FindChildByName("ExportMovieButton"));
  ...
  m_pYouTubeUpload =
      dynamic_cast<CExButton *>(FindChildByName("YouTubeUploadButton"));
  m_pYouTubeView =
      dynamic_cast<CExButton *>(FindChildByName("ViewYouTubeButton"));
  m_pYouTubeShareURL =
      dynamic_cast<CExButton *>(FindChildByName("ShareYouTubeURLButton"));
  m_pShowRenderInfoButton =
      dynamic_cast<CExImageButton *>(FindChildByName("ShowRenderInfoButton"));
...
void CReplayDetailsPanel::PerformLayout() {
  ...
  int nRightColumnWidth =
      nInsetWidth - nScreenshotWidth - nLeftRightBuffer - XRES(20);

#if defined(TF_CLIENT_DLL)
  if (m_pRecordsPanel->ShouldShow()) {
    m_pRecordsPanel->SetPos(nScreenshotWidth + nLeftRightBuffer,
                            nInfoPanelsStartY);
  ...
  m_pScrollPanel->SetPos(nScreenshotWidth + nLeftRightBuffer,
                         nInfoPanelsCurrentY);
  ...
  for (int i = 0; i < m_vecInfoPanels.Count(); ++i) {
    ...
    if (pPanel->ShouldShow()) {
      ...
      // Update the current y position based on the panel's height (set in its
      // PerformLayout())
  ...
  // @note Tom Bui: set the positions AGAIN now that we've invalidated, cause
  // VGUI hates me
  nCurrentY = 0;
  for (int i = 0; i < m_vecInfoPanels.Count(); ++i) {
    CBaseDetailsPanel *pPanel = m_vecInfoPanels[i];
  ...
  if (m_pItemManager->AreItemsMovies()) {
    ...
    m_pShowRenderInfoButton->SetPos(
        aScreenshotPos[0] + m_pPlaybackPanel->GetWide() -
            m_pShowRenderInfoButton->GetWide() - XRES(8),
        aScreenshotPos[1] + m_pPlaybackPanel->GetTall() -
            m_pShowRenderInfoButton->GetTall() - YRES(8));

    // retrieve stats
    if (m_pYouTubeResponseHandler->m_handle == NULL) {
      IQueryableReplayItem *pReplayItem =
          m_pItemManager->GetItem(m_hReplayItem);
      if (pReplayItem && pReplayItem->IsItemAMovie()) {
        IReplayMovie *pMovie = static_cast<IReplayMovie *>(pReplayItem);
        if (pMovie->IsUploaded()) {
          m_pYouTubeResponseHandler->m_handle = YouTube_GetVideoInfo(
              pMovie->GetUploadURL(), *m_pYouTubeResponseHandler);
    ...
    // Without this, the name label won't show when we automatically select the
    // recently watched/saved performance, because the cuts panel width/height
    // isn't set when UpdateNameLabel() gets called from within
    // CCutsPanel::CCutsPanel().
    m_pCutsPanel->UpdateNameLabel(m_iSelectedPerformance);
  }
}

/*static*/ void CReplayDetailsPanel::OnPlayerWarningDlgConfirm(bool bConfirmed,
                                                               void *pContext) {
  CReplayDetailsPanel *pPanel = (CReplayDetailsPanel *)pContext;
...
void CReplayDetailsPanel::ShowExportDialog() {
  ...
  if (pReplayItem && pReplayItem->IsItemAMovie()) {
    IReplayMovie *pMovie = static_cast<IReplayMovie *>(pReplayItem);
    CFmtStr srcMovieFullFilename("%s%s", g_pReplayMovieManager->GetRenderDir(),
                                 pMovie->GetMovieFilename());
    if (!g_pFullFileSystem->FileExists(srcMovieFullFilename.Access())) {
      ShowMessageBox("#Replay_ExportMovieError_Title",
                     "#Replay_ExportMovieNoFile_Text", "#GameUI_OK");
      return;
    }
  }

  if (m_hExportMovieDialog == NULL) {
    m_hExportMovieDialog =
        new FileOpenDialog(NULL, "#Replay_FindExportMovieLocation", FOD_SAVE);
    ...
    if (!FStrEq(replay_movie_export_last_dir.GetString(), "")) {
      m_hExportMovieDialog->SetStartDirectory(
          replay_movie_export_last_dir.GetString());
    }
  }
...
void CReplayDetailsPanel::OnFileSelected(const char *fullpath) {
  ...
  if (pReplayItem && pReplayItem->IsItemAMovie()) {
    ...
    if (!engine->CopyLocalFile(srcMovieFullFilename.Access(), fullpath)) {
      ShowMessageBox("#Replay_ExportMovieError_Title",
                     "#Replay_ExportMovieError_Text", "#GameUI_OK");
    } else {
      ShowMessageBox("#Replay_ExportMovieSuccess_Title",
                     "#Replay_ExportMovieSuccess_Text", "#GameUI_OK");
    }
...
void CReplayDetailsPanel::OnCommand(const char *pCommand) {
  if (FStrEq(pCommand, "delete_replayitem")) {
    ReplayUI_GetBrowserPanel()->AttemptToDeleteReplayItem(
        this, m_hReplayItem, m_pItemManager, m_iSelectedPerformance);
    return;
  }
    ...
    if (engine->IsInGame()) {
      ...
      g_pClientReplayContext->PlayReplay(m_hReplay, m_iSelectedPerformance,
                                         true);
    }
    return;
  }
    ...
    if (replay_movie_reveal_warning.GetBool()) {
      ...
      CTFMessageBoxDialog *pDialog =
          ShowMessageBox("#Replay_Tip", "#Replay_UseVLCPlayer",
                         "#Replay_ThanksIWill", OnPlayerWarningDlgConfirm);
#else
      CTFMessageBoxDialog *pDialog =
          ShowMessageBox("#Replay_Tip", "#Replay_UseQuickTimePlayer",
                         "#Replay_ThanksIWill", OnPlayerWarningDlgConfirm);
#endif
      pDialog->SetContext(this);
      replay_movie_reveal_warning.SetValue(0);
    } else if (pMovie->GetRenderSettings().m_bRaw) {
      ShowMessageBox("#Replay_CantExport", "#YouTube_Upload_MovieIsRaw",
                     "#GameUI_OK");
    } else {
      ShowExportDialog();
    }
    ...
    if (pReplayItem && pReplayItem->IsItemAMovie()) {
      ...
      if (pMovie->GetRenderSettings().m_bRaw) {
        ShowMessageBox("#Replay_CantUpload", "#YouTube_Upload_MovieIsRaw",
                       "#GameUI_OK");
        return;
      }

      // Movie already exists?
      CFmtStr srcMovieFullFilename("%s%s",
                                   g_pReplayMovieManager->GetRenderDir(),
                                   pMovie->GetMovieFilename());
      if (!g_pFullFileSystem->FileExists(srcMovieFullFilename.Access())) {
        ShowMessageBox("#YouTube_Upload_Title", "#YouTube_Upload_MissingFile",
                       "#GameUI_OK");
        return;
      } else if (pMovie->IsUploaded()) {
        CTFGenericConfirmDialog *pDialog = ShowConfirmDialog(
            "#YouTube_Upload_Title", "#YouTube_FileAlreadyUploaded",
            "#GameUI_OK", "#GameuI_CancelBold", &ConfirmUploadMovie, this);
        pDialog->SetContext(this);
      } else {
    ...
    ShowMessageBox("#Replay_CopyURL_Title", "#Replay_CopyURL_Text",
                   "#GameUI_OK");
  }
...
void CReplayDetailsPanel::ShowRenderInfo() {
  ...
  const wchar_t *pCodecName =
      g_pVideo ? g_pVideo->GetCodecName(Settings.m_Codec) : L"?";
  wchar_t *pAAEnabled = g_pVGuiLocalize->Find(
      Settings.m_bAAEnabled ? "#Replay_Enabled" : "#Replay_Disabled");
  wchar_t *pRaw =
      g_pVGuiLocalize->Find(Settings.m_bRaw ? "#Replay_Yes" : "#Replay_No");
  ...
  kvParams->SetString("rendertime",
                      CReplayTime::FormatTimeString(pMovie->GetRenderTime()));
  kvParams->SetWString("raw", pRaw);

  wchar_t wszStr[1024];
  g_pVGuiLocalize->ConstructString(wszStr, sizeof(wszStr),
                                   "#Replay_MovieRenderInfo", kvParams);
...
void CReplayDetailsPanel::ShowPlayConfirmationDialog() {
  CConfirmDisconnectFromServerDialog *pConfirm =
      SETUP_PANEL(new CConfirmDisconnectFromServerDialog(this));
  if (pConfirm) {
    pConfirm->Show();
  }
}
...
void CReplayDetailsPanel::OnMessage(const KeyValues *pParams,
                                    VPANEL hFromPanel) {
  if (FStrEq(pParams->GetName(), "ReplayItemDeleted")) {
    const int iPerformance =
        const_cast<KeyValues *>(pParams)->GetInt("perf", -1);
    if (iPerformance >= 0) {
      CReplayPerformance *pPerformance =
          GetGenericClassBasedReplay(m_hReplay)->GetPerformance(
              m_iSelectedPerformance);
      g_pReplayPerformanceManager->DeletePerformance(pPerformance);
      m_pCutsPanel->InvalidateLayout(
          true, false); // Without this, m_nVisibleButtons will be wrong.
...
void CReplayDetailsPanel::SetYouTubeStatus(eYouTubeStatus status) {
  m_pYouTubeUpload->SetVisible(status == kYouTubeStatus_CouldNotRetrieveInfo ||
                               status == kYouTubeStatus_NotUploaded);
  m_pYouTubeUpload->SetEnabled(status == kYouTubeStatus_CouldNotRetrieveInfo ||
                               status == kYouTubeStatus_NotUploaded);
