...
public:
  ...
  virtual void CleanupUIForReplayItem(
      ReplayItemHandle_t
          hReplay); // After a replay has been deleted - deletes all UI
                    // (thumbnail, but maybe also row and/or collection as well)

  virtual void ApplySchemeSettings(vgui::IScheme *pScheme);
  virtual void PerformLayout(void);
  virtual void OnCommand(const char *command);
  virtual void ShowPanel(bool bShow,
                         ReplayHandle_t hReplayDetails = REPLAY_HANDLE_INVALID,
                         int iPerformance = -1);
...
private:
  ...
  void ConfirmReplayItemDelete(Panel *pHandler, ReplayItemHandle_t hReplayItem,
                               IReplayItemManager *pItemManager,
                               int iPerformance);
