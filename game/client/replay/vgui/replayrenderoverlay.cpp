...
ConVar replay_enablerenderpreview("replay_enablerenderpreview", "1",
...
CReplayRenderOverlay::CReplayRenderOverlay(Panel *pParent)
    : BaseClass(pParent, "ReplayRenderOverlay"), m_pBottom(NULL),
      m_pCancelButton(NULL), m_pTitleLabel(NULL), m_pProgressLabel(NULL),
      m_pFilenameLabel(NULL), m_pRenderProgress(NULL), m_pRenderer(NULL),
      m_pPreviewCheckButton(NULL), m_unNumFrames(0), m_flStartTime(0.0f),
...
void CReplayRenderOverlay::ApplySchemeSettings(IScheme *pScheme) {
  ...
  m_pRenderProgress =
      dynamic_cast<ProgressBar *>(FindChildByName("RenderProgress"));
  ...
  m_pPreviewCheckButton =
      dynamic_cast<CheckButton *>(FindChildByName("PreviewCheckButton"));
...
void CReplayRenderOverlay::PerformLayout() {
  ...
  // Only show progress bar if replay is valid and length of render is non-zero,
  // and the record start tick exists
  ...
  if (pReplay) {
    ...
    const int nServerRecordStartTick =
        g_pClientReplayContext->GetRecordingSessionManager()
            ->GetServerStartTickForSession(
  ...
  // Layout title label
  const int nTitleLabelY =
      nBottomPanelStartY +
      (m_pBottom->GetTall() - m_pTitleLabel->GetTall()) / 2;
  if (m_pTitleLabel) {
    m_pTitleLabel->SizeToContents();
    m_pTitleLabel->SetPos((nProgX - m_pTitleLabel->GetWide()) / 2,
                          nTitleLabelY);
  }
  ...
  if (m_pPreviewCheckButton) {
    ...
    m_pPreviewCheckButton->SetPos(
        (nProgX - m_pPreviewCheckButton->GetWide()) / 2,
  ...
  if (m_pFilenameLabel) {
    ...
    m_pFilenameLabel->SetPos(
        nProgX,
        nProgBottomY + (sh - nProgBottomY - m_pFilenameLabel->GetTall()) / 2);
  }
  ...
  if (m_pProgressLabel) {
    ...
    m_pProgressLabel->SetPos(
        nProgX,
        nProgBottomY + (sh - nProgBottomY - m_pProgressLabel->GetTall()) / 2);
  ...
  m_pCancelButton->SetPos(
      nProgRightX +
          (m_pBottom->GetWide() - nProgRightX - m_pCancelButton->GetWide()) / 2,
      nBottomPanelStartY +
          (m_pBottom->GetTall() - m_pCancelButton->GetTall()) / 2);
...
void CReplayRenderOverlay::OnTick() {
  ...
  if (m_pRenderProgress) {
    ...
    if (pReplay && m_pRenderProgress->IsVisible()) {
      ...
      g_pClientReplayContext->GetPlaybackTimes(flCurTime, flTotalTime, pReplay,
                                               m_pRenderer->GetPerformance());
      const float flProgress =
          (flTotalTime == 0.0f) ? 1.0f : (flCurTime / flTotalTime);

      Assert(flTotalTime >
             0.0f); // NOTE: Progress bar will always be invisible if total time
                    // is 0, but check anyway to be safe.
      m_pRenderProgress->SetProgress(MAX(
          m_pRenderProgress->GetProgress(),
          flProgress)); // The MAX() here keeps the progress bar from thrashing

      if (m_pProgressLabel) {
        // @note Tom Bui: this is a horribly ugly hack, but the first couple of
        // frames take a really freaking long time, so that really blows out the
        // estimate
        ...
        if (m_unNumFrames < kNumFramesToWait) {
          ...
          float flEstimatedTimeLeft =
              flProgress > 0.0f ? (flTimePassed / flProgress) - flTimePassed
                                : 0.0f;
          ...
          if (m_flPreviousTimeLeft == 0.0f) {
            ...
            m_flPreviousTimeLeft =
                kSmoothingFactor * flEstimatedTimeLeft +
                (1 - kSmoothingFactor) * m_flPreviousTimeLeft;
          }
        }
        ...
        wchar_t wszTime[256];
        {
          const char *pRenderTime = CReplayTime::FormatTimeString(
              RoundFloatToInt(m_flPreviousTimeLeft));
          g_pVGuiLocalize->ConvertANSIToUnicode(pRenderTime, wszTimeLeft,
                                                sizeof(wszTimeLeft));
        }
        {
          const char *pRenderTime =
              CReplayTime::FormatTimeString(RoundFloatToInt(flTimePassed));
          g_pVGuiLocalize->ConvertANSIToUnicode(pRenderTime, wszTime,
                                                sizeof(wszTime));
        }
        wchar_t wszText[256];
        g_pVGuiLocalize->ConstructString(
            wszText, sizeof(wszText),
            g_pVGuiLocalize->Find("#Replay_RenderOverlay_TimeLeft"), 2, wszTime,
            wszTimeLeft);
...
void CReplayRenderOverlay::OnCommand(const char *pCommand) {
  if (!V_stricmp(pCommand, "confirmcancel")) {
    ShowConfirmDialog("#Replay_CancelRenderTitle",
                      "#Replay_ConfirmCancelRender", "#Replay_YesCancel",
                      "#Replay_No", OnRenderCancelDialogButtonPressed, this,
                      NULL, "replay\\replaydialog_warn.wav");
    return;
  }
...
void ReplayUI_OpenReplayRenderOverlay() {
  ...
  s_pRenderOverlay = SETUP_PANEL(new CReplayRenderOverlay(
      NULL)); // Parenting to NULL allows us to turn off world rendering in
              // engine/view.cpp (V_RenderView())

  // Set the panel as the movie renderer, so it can receive begin/end render
