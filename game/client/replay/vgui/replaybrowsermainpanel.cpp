...
public:
  CConfirmDeleteReplayDialog(Panel *pParent, IReplayItemManager *pItemManager,
                             int iPerformance)
      : BaseClass(pParent) {
    m_pTextId = iPerformance >= 0 ? "#Replay_DeleteEditConfirm"
                : pItemManager->AreItemsMovies()
                    ? "#Replay_DeleteMovieConfirm"
                    : "#Replay_DeleteReplayConfirm";
  }

  const wchar_t *GetText() { return g_pVGuiLocalize->Find(m_pTextId); }
...
CReplayBrowserPanel::CReplayBrowserPanel(Panel *parent)
  ...
  // Create this now, so that it can be the default button (if created in .res
  // file, it fights with PropertyDialog's OkButton & generates asserts)
...
// Purpose:
//-----------------------------------------------------------------------------
void CReplayBrowserPanel::ShowPanel(
  ...
  if (hReplayDetails != REPLAY_HANDLE_INVALID) {
    char szDetails[32];
    V_snprintf(szDetails, sizeof(szDetails), "details%i_%i",
               (int)hReplayDetails, iPerformance);
...
void CReplayBrowserPanel::OnCommand(const char *command) {
  if (!Q_stricmp(command, "back")) {
    if (m_pReplaysPage->IsDetailsViewOpen()) {
      ...
      // TODO: Properly manage the browser so that we don't have to recreate it
      // ever time its opened
      MarkForDeletion();
...
void CReplayBrowserPanel::ShowDeleteReplayDenialDlg() {
  ShowMessageBox("#Replay_DeleteDenialTitle", "#Replay_DeleteDenialText",
                 "#GameUI_OK");
}
...
// Purpose:
//-----------------------------------------------------------------------------
void CReplayBrowserPanel::AttemptToDeleteReplayItem(
    Panel *pHandler, ReplayItemHandle_t hReplayItem,
  ...
  if (!pItemManager->AreItemsMovies() && iPerformance < 0 &&
      !pReplay->ShouldAllowDelete()) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CReplayBrowserPanel::ConfirmReplayItemDelete(
    Panel *pHandler, ReplayItemHandle_t hReplayItem,
    IReplayItemManager *pItemManager, int iPerformance) {
  CConfirmDeleteReplayDialog *pConfirm = vgui::SETUP_PANEL(
      new CConfirmDeleteReplayDialog(this, pItemManager, iPerformance));
  if (pConfirm) {
...
void CReplayBrowserPanel::OnConfirmDelete(KeyValues *data) {
  ...
  if (m_DeleteInfo.m_iPerformance < 0) {
    ...
    m_DeleteInfo.m_pItemManager->DeleteItem(GetActivePage(), hReplayItem,
                                            false);
  }
...
// Purpose:
//-----------------------------------------------------------------------------
void CReplayBrowserPanel::CleanupUIForReplayItem(
...
//-----------------------------------------------------------------------------
CReplayBrowserPanel *
ReplayUI_OpenReplayBrowserPanel(ReplayHandle_t hReplayDetails,
                                int iPerformance) {
...
// Purpose:
//-----------------------------------------------------------------------------
CON_COMMAND_F(replay_reloadbrowser,
