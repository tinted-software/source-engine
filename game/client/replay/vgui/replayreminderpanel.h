...
private:
  ...
  float m_flShowTime; // Used by the HUD only, to display the panel only for a
                      // certain period of time
  bool m_bShouldDraw; // Store this state for ShouldDraw(), which allows us to
                      // use a single panel for both the post-win reminder and
                      // the freezepanel reminder.
};
