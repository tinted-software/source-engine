...
ConVar replay_quitmsg_dontaskagain(
    ...
    "The replay system will ask you to render your replays on quit, unless "
    "this cvar is 1.",
    true, 0, true, 1);
...
CReplayConfirmQuitDialog::CReplayConfirmQuitDialog(Panel *pParent)
    : BaseClass(pParent, "confirmquitdlg"), m_pDontShowAgain(NULL),
      m_pQuitButton(NULL) {
...
void CReplayConfirmQuitDialog::ApplySchemeSettings(vgui::IScheme *pScheme) {
  ...
  m_pDontShowAgain =
      dynamic_cast<CheckButton *>(FindChildByName("DontShowThisAgainCheckbox"));
...
void CReplayConfirmQuitDialog::OnCommand(const char *pCommand) {
  // Store the setting of our "never show this again" checkbox if the user
  // picked anything except cancel.
  if (!FStrEq(pCommand, "cancel") && m_pDontShowAgain &&
      m_pDontShowAgain->IsSelected()) {
  ...
  if (FStrEq(pCommand, "rendernow_delay")) {
    ...
    // Sometimes this message comes in just before input is processed when using
    // a controller Refire after a delay
    PostMessage(this, new KeyValues("Command", "command", "rendernow_delay"),
                0.001f);
...
bool ReplayUI_ShowConfirmQuitDlg() {
  ...
  CReplayConfirmQuitDialog *pConfirmQuitDlg =
      vgui::SETUP_PANEL(new CReplayConfirmQuitDialog(NULL));
  if (pConfirmQuitDlg) {
