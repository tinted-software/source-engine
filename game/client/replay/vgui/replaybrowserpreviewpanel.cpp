...
CReplayPreviewPanelBase::CReplayPreviewPanelBase(
    Panel *pParent, QueryableReplayItemHandle_t hItem,
    IReplayItemManager *pItemManager)
    : EditablePanel(pParent, "PreviewPanel"), m_hItem(hItem),
  ...
  const wchar_t *pDateAndTime = CReplayTime::GetLocalizedDate(
      g_pVGuiLocalize, nDay, nMonth, nYear, &nHour, &nMin, &nSec);
  ...
  for (int i = 0; i < NUM_INFO_LABELS; ++i) {
    for (int j = 0; j < 2; ++j) {
      m_pReplayInfoLabels[i][j] =
          new CExLabel(m_pInfoPanel, VarArgs("Label%d_%d", i, j), "");
    }
  }
  m_pReplayInfoLabels[LABEL_PLAYED_AS][1]->SetText(pReplay->GetPlayerClass());
  m_pReplayInfoLabels[LABEL_KILLED_BY][1]->SetText(
      pReplay->WasKilled() ? pReplay->GetKillerName() : "#Replay_NoKiller");
  m_pReplayInfoLabels[LABEL_LIFE_LENGTH][1]->SetText(
      CReplayTime::FormatTimeString((int)pItem->GetItemLength()));
}
...
void CReplayPreviewPanelBase::PerformLayout() {
  ...
  int nTall = YRES(18);
  int nScreenshotH =
      0; // Represents the height of the screenshot OR the "no screenshot" label
  ...
  m_pInfoPanel->SetBounds(nLabelX, nTall - iInfoHeight, iInfoWidth,
                          iInfoHeight);
...
CGenericClassBasedReplay *CReplayPreviewPanelBase::GetReplay() {
  return ToGenericClassBasedReplay(
      m_pItemManager->GetItem(m_hItem)->GetItemReplay());
}
...
CReplayPreviewPanelSlideshow::CReplayPreviewPanelSlideshow(
    Panel *pParent, QueryableReplayItemHandle_t hReplay,
    IReplayItemManager *pItemManager)
  ...
  if (nScreenshotCount) {
    m_pScreenshotPanel = new CReplayScreenshotSlideshowPanel(
        this, "ScreenshotSlideshowPanel", hReplay);
  ...
  m_pNoScreenshotLabel =
      new CExLabel(this, "NoScreenshotLabel", "#Replay_NoScreenshot");
...
void CReplayPreviewPanelSlideshow::LayoutView(int &nWide, int &nTall,
                                              int &nCurY) {
  if (m_pScreenshotPanel) {
    // Use the dimensions from the first screenshot to figure out the scale,
    // even though the dimensions may vary if the user changed resolutions
    // during gameplay
    CGenericClassBasedReplay *pReplay = GetReplay();
    ...
    m_pScreenshotPanel->SetBounds((nWide - nScreenshotW) * 0.5, YRES(9),
                                  nScreenshotW, nCurY);
  } else {
    int w, h;
...
void CReplayPreviewPanelMovie::OnTick() {
  if (gpGlobals->realtime >= m_flCreateTime + 0.5f) {
    if (!m_pMoviePlayerPanel) {
      m_pMoviePlayerPanel = new CMoviePlayerPanel(
          this, "MoviePlayer", GetReplayMovie()->GetMovieFilename());
