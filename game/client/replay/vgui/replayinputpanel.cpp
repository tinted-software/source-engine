...
//-----------------------------------------------------------------------------
CReplayInputPanel::CReplayInputPanel(Panel *pParent, const char *pName,
                                     ReplayHandle_t hReplay)
    : BaseClass(pParent, pName), m_hReplay(hReplay), m_pDlg(NULL),
      m_pTitleEntry(NULL) {
...
void CReplayInputPanel::OnCommand(const char *command) {
  ...
  if (!Q_strnicmp(command, "save", 4)) {
    if (m_hReplay != REPLAY_HANDLE_INVALID) {
      ...
      CReplay *pReplay = g_pReplayManager->GetReplay(m_hReplay);
      if (pReplay) {
        m_pTitleEntry->GetText(pReplay->m_wszTitle,
                               sizeof(pReplay->m_wszTitle));
      }
      ...
      // Display a message - if we somehow disconnect, we can crash here if
      // local player isn't checked
      C_BasePlayer *pLocalPlayer = C_BasePlayer::GetLocalPlayer();
      if (pLocalPlayer) {
        g_pClientMode->DisplayReplayMessage(
            pLocalPlayer->IsAlive() ? "#Replay_ReplaySavedAlive"
                                    : "#Replay_ReplaySavedDead",
            -1.0f, false, "replay\\saved.wav", false);

        // Check to see if player's dead - used later to determine if we should
        // show items window
  ...
  if (bCloseWindow) {
    ...
    // This logic is perhaps a smidge of a hack.  We have to be careful about
    // executing "gameui_hide" since it will hide the item pickup panel.  If
    // there are no items to be picked up, we can safely hide the gameui panel,
    // but we have to call CheckForRoomAndForceDiscard() (as ShowItemsPickedUp()
    // does if no items are picked up).  Otherwise, skip the "gameui_hide" call
    // and show the item pickup panel.
#if defined(TF_CLIENT_DLL)
...
void ShowReplayInputPanel(ReplayHandle_t hReplay) {
  ...
  hReplayInputPanel = vgui::SETUP_PANEL(
      new CReplayInputPanel(NULL, "ReplayInputPanel", hReplay));
...
//-----------------------------------------------------------------------------
CON_COMMAND_F(open_replayinputpanel, "Open replay input panel test",
              FCVAR_NONE) {
