...
CReplayScreenshotSlideshowPanel::CReplayScreenshotSlideshowPanel(
    Panel *pParent, const char *pName, ReplayHandle_t hReplay)
  ...
  for (int i = 0; i < pReplay->GetScreenshotCount(); ++i) {
    ...
    V_snprintf(szImage, sizeof(szImage), "replay/thumbnails/%s.vtf",
...
CReplayBrowserThumbnail::CReplayBrowserThumbnail(
    ...
    : CReplayBasePanel(pParent, pName), m_hReplayItem(hReplayItem),
      m_pReplayItemManager(pReplayItemManager), m_bMouseOver(false),
      m_pMoviePlayer(NULL), m_flLastMovieScrubTime(0.0f),
      m_flHoverStartTime(0.0f), m_flLastProgressChangeTime(0.0f) {
...
void CReplayBrowserThumbnail::SetReplayItem(
  ...
  if (m_hReplayItem != REPLAY_HANDLE_INVALID && m_hReplayItem != hReplayItem) {
    IQueryableReplayItem *pReplayItem =
        m_pReplayItemManager->GetItem(m_hReplayItem);
    if (pReplayItem) {
      pReplayItem->SetUserData(NULL);

      CGenericClassBasedReplay *pTFReplay =
          ToGenericClassBasedReplay(pReplayItem->GetItemReplay());
...
void CReplayBrowserThumbnail::OnTick() {
  ...
  // Need to update layout if status has changed, since some buttons may need to
  // be shifted around
  ...
  const char *pBorderName =
      m_bMouseOver ? "ReplayHighlightBorder" : "ReplayDefaultBorder";
  IBorder *pBorder = scheme()->GetIScheme(GetScheme())->GetBorder(pBorderName);
  m_pBorderPanel->SetBorder(pBorder);

  // Set visibility of buttons and such - a player may have saved their replay
  // but not died yet, in which case pReplay->m_bComplete will be false.
  const IQueryableReplayItem *pItem =
      m_pReplayItemManager->GetItem(m_hReplayItem);
  bool bIncomplete = !pReplay->m_bComplete;
  bool bDownloadPhase =
      !pItem->IsItemAMovie() &&
      pReplay->m_nStatus == CReplay::REPLAYSTATUS_DOWNLOADPHASE;
...
void CReplayBrowserThumbnail::UpdateProgress(bool bDownloadPhase,
                                             const CReplay *pReplay) {
  if (!bDownloadPhase)
    return;

  // Get current download progress
  const float flProgress =
      g_pClientReplayContext->GetReplayManager()->GetDownloadProgress(pReplay);
  ...
  if (ReplayUI_GetBrowserPanel() &&
      (flCurTime - ReplayUI_GetBrowserPanel()->GetTimeOpened() > 2.0f)) {
  ...
  if (pLocalizedText) {
    ...
    m_pDownloadLabel->SetPos(
        3, (m_pDownloadProgress->GetTall() - m_pDownloadLabel->GetTall()) / 2);
  }
}
...
void CReplayBrowserThumbnail::ApplySchemeSettings(vgui::IScheme *pScheme) {
  ...
  m_clrHighlight =
      GetSchemeColor("TanDark", Color(255, 255, 255, 255), pScheme);
}
...
void CReplayBrowserThumbnail::PerformLayout() {
  ...
  if (!bHasScreenshots || !g_pFullFileSystem ||
      !g_pFullFileSystem->FileExists(szSearch, "GAME")) {
    // Hide it
    bShowScreenshotThumb = false;
  }

  // Scale the screenshot so that it clips off the dead area of the power of 2
  // texture
  float flScale = pReplay->GetScreenshotCount() == 0
                      ? 1.0f
                      : ((float)m_pScreenshotThumb->GetWide() /
  ...
  if (bShowScreenshotThumb) {
    // We don't want to actually hide it (via SetVisible()), since we need to
    // get mouse click events from the image panel
  ...
  m_pRecordingInProgressLabel->InvalidateLayout(
      true, false); // Need this for centerwrap to work properly
  ...
  if (bDownloadPhase) {
    ...
    nDeleteButtonY = nThumbY + m_pScreenshotThumb->GetTall() -
                     m_pDeleteButton->GetTall() - 12;
  } else {
    nDownloadButtonY =
        0; // We don't care about this now, since it will not be visible
    nDeleteButtonY =
        (m_pScreenshotThumb->GetTall() - m_pDeleteButton->GetTall()) / 2;
  }
...
void CReplayBrowserThumbnail::OnCommand(const char *pCommand) {
  ...
  } else if (FStrEq(pCommand, "delete_replayitem")) // Delete the replay?
  {
    ReplayUI_GetBrowserPanel()->AttemptToDeleteReplayItem(
        this, m_hReplayItem, m_pReplayItemManager, -1);
  } else {
...
CReplayBrowserThumbnailRow::CReplayBrowserThumbnailRow(
    Panel *pParent, const char *pName, IReplayItemManager *pReplayItemManager)
    : BaseClass(pParent, pName), m_pReplayItemManager(pReplayItemManager) {
  SetProportional(true);
}

void CReplayBrowserThumbnailRow::AddReplayThumbnail(
    const IQueryableReplayItem *pItem) {
  CReplayBrowserThumbnail *pThumbnail = new CReplayBrowserThumbnail(
      this, "ListThumbnail", pItem->GetItemHandle(), m_pReplayItemManager);
  m_vecThumbnails.AddToTail(pThumbnail);
}
void CReplayBrowserThumbnailRow::AddReplayThumbnail(
    QueryableReplayItemHandle_t hReplayItem) {
  CReplayBrowserThumbnail *pThumbnail = new CReplayBrowserThumbnail(
      this, "ListThumbnail", hReplayItem, m_pReplayItemManager);
  m_vecThumbnails.AddToTail(pThumbnail);
}

void CReplayBrowserThumbnailRow::DeleteReplayItemThumbnail(
  ...
  if (nThumbnailElement == m_vecThumbnails.InvalidIndex()) {
    AssertMsg(0, "REPLAY: Should have found replay thumbnail while attempting "
                 "to delete it from the browser.");
    return;
  }
...
CReplayBrowserThumbnail *CReplayBrowserThumbnailRow::FindThumbnail(
    const IQueryableReplayItem *pReplayItem) {
...
void CReplayBrowserThumbnailRow::PerformLayout() {
  for (int i = 0; i < m_vecThumbnails.Count(); ++i) {
    CReplayBrowserThumbnail *pThumbnail = m_vecThumbnails[i];
    pThumbnail->SetPos(i * (pThumbnail->GetWide() + 2 * REPLAY_BORDER_WIDTH),
                       0);
...
CBaseThumbnailCollection::CBaseThumbnailCollection(
    CReplayListPanel *pParent, const char *pName,
    IReplayItemManager *pReplayItemManager)
    : EditablePanel(pParent, pName), m_pReplayItemManager(pReplayItemManager),
      m_nStartX(XRES(15)), m_pCaratLabel(NULL), m_pTitleLabel(NULL),
      m_pNoReplayItemsLabel(NULL), m_pRenderAllButton(NULL) {
...
void CBaseThumbnailCollection::CleanupUIForReplayItem(
    ReplayItemHandle_t hReplayItem) {
  IQueryableReplayItem *pReplayItem =
      m_pReplayItemManager->GetItem(hReplayItem);
  Assert(pReplayItem);
  if (!pReplayItem)
    return;

  // Find the replay thumbnail
  CReplayBrowserThumbnailRow *pThumbnailRow =
      FindReplayItemThumbnailRow(pReplayItem);
  if (!pThumbnailRow) {
    AssertMsg(0, "REPLAY: Should have found replay thumbnail row while "
                 "attempting to delete it from the browser.");
    return;
  }
...
int CBaseThumbnailCollection::GetRowStartY() {
  ...
  Panel *pLowestPanel = m_pReplayItemManager->GetItemCount() == 0
                            ? m_pNoReplayItemsLabel
                            : GetLowestPanel(nVerticalBuffer);

  int x, y;
  pLowestPanel->GetPos(x, y);

  bool bMakeRoomForNextPrev =
      (m_pShowPrevButton && m_pShowNextButton &&
...
CReplayBrowserThumbnailRow *
CBaseThumbnailCollection::FindReplayItemThumbnailRow(
    const IQueryableReplayItem *pReplayItem) {
  AssertValidReadPtr(pReplayItem);
...
void CBaseThumbnailCollection::OnCommand(const char *pCommand) {
  if (FStrEq(pCommand, "render_queued_replays")) {
    ...
    m_iViewingPage = clamp(
        m_iViewingPage + 1, 0,
        Ceil2Int((float)m_vecReplays.Count() / (float)iThumbnailsPerPage));
    UpdateViewingPage();
    return;
...
void CBaseThumbnailCollection::UpdateViewingPage(void) {
  ...
  // If the page we're on is not the first page, and we have no replays on it,
  // move back a page.
  ...
  for (int i = 0; i < iThumbnailsPerPage; i++) {
    ...
    if (iRow >= m_vecRows.Count()) {
      // If we don't actually have any more replays, we don't need to make the
      // new row.
      if (iReplayIndex >= m_vecReplays.Count())
        break;

      // Create a new row and add there
      CReplayBrowserThumbnailRow *pNewRow = new CReplayBrowserThumbnailRow(
          this, "ThumbnailRow", m_pReplayItemManager);
    ...
    if (iReplayIndex >= m_vecReplays.Count()) {
      m_vecRows[iRow]->m_vecThumbnails[iColumn]->SetReplayItem(
          REPLAY_HANDLE_INVALID);
      m_vecRows[iRow]->m_vecThumbnails[iColumn]->SetVisible(false);
    } else {
      m_vecRows[iRow]->m_vecThumbnails[iColumn]->SetReplayItem(
          m_vecReplays[iReplayIndex]);
  ...
  wchar_t wszCount[10];
  int iNextReplays =
      clamp(m_vecReplays.Count() - iFirstReplayOnPage - iThumbnailsPerPage, 0,
            iThumbnailsPerPage);
  if (iNextReplays > 0) {
    _snwprintf(wszCount, ARRAYSIZE(wszCount), L"%d", iNextReplays);
    g_pVGuiLocalize->ConstructString(wszTemp, sizeof(wszTemp),
                                     g_pVGuiLocalize->Find("#Replay_NextX"), 1,
                                     wszCount);
  ...
  if (iPrevReplays > 0) {
    ...
    g_pVGuiLocalize->ConstructString(wszTemp, sizeof(wszTemp),
                                     g_pVGuiLocalize->Find("#Replay_PrevX"), 1,
                                     wszCount);
  ...
  // We may have changed our size, so we need to tell our parent that it should
  // re-layout
...
void CBaseThumbnailCollection::ApplySchemeSettings(vgui::IScheme *pScheme) {
  ...
  LoadControlSettings("resource/ui/replaybrowser/thumbnailcollection.res",
                      "GAME");
  ...
  m_pNoReplayItemsLabel =
      dynamic_cast<CExLabel *>(FindChildByName("NoReplayItemsLabel"));
  m_pShowNextButton =
      dynamic_cast<CExButton *>(FindChildByName("ShowNextButton"));
  if (m_pShowNextButton) {
    m_pShowNextButton->AddActionSignalTarget(this);
  }
  m_pShowPrevButton =
      dynamic_cast<CExButton *>(FindChildByName("ShowPrevButton"));
...
void CBaseThumbnailCollection::PerformLayout() {
  ...
  m_pNoReplayItemsLabel->SetPos(
      (GetWide() - m_pNoReplayItemsLabel->GetWide()) / 2, YRES(40));
  ...
  const char *pszLocString =
      IsMovieCollection() ? "#Replay_SavedMovies" : "#Replay_UnrenderedReplays";
  g_pVGuiLocalize->ConstructString(wszTemp, sizeof(wszTemp),
  ...
  if (m_pShowPrevButton) {
    m_pShowPrevButton->SetPos(
        nButtonX, nCurrentY - m_pShowPrevButton->GetTall() - YRES(2));
    nCurrentY += YRES(2);
  }
  ...
  // TODO: Resizing can cause an InvalidateLayout() call if the new & old
  // dimensions differ, perhaps calling this here is not the best idea. Adjust
...
CReplayThumbnailCollection::CReplayThumbnailCollection(
    CReplayListPanel *pParent, const char *pName,
    IReplayItemManager *pReplayItemManager)
    : BaseClass(pParent, pName, pReplayItemManager), m_pWarningLabel(NULL),
      m_pUnconvertedBg(NULL) {
  // Create additional panels for unconverted rows
  m_pLinePanel = new Panel(this, "Line");
  m_pWarningLabel =
      new CExLabel(this, "WarningLabel", "#Replay_ConversionWarning");
  m_pUnconvertedBg = new Panel(this, "UnconvertedBg");
  m_pRenderAllButton =
      new CExButton(this, "RenderAllButton", "#Replay_RenderAll");
}

bool CReplayThumbnailCollection::IsMovieCollection() const { return false; }

void CReplayThumbnailCollection::PerformLayout() { BaseClass::PerformLayout(); }
...
void CReplayThumbnailCollection::ApplySchemeSettings(IScheme *pScheme) {
  ...
  if (!pBoundKey || FStrEq(pBoundKey, "(null)")) {
    ...
    g_pVGuiLocalize->ConstructString(wLabel, sizeof(wLabel),
                                     g_pVGuiLocalize->Find("#Replay_NoReplays"),
                                     1, wKey);
...
void CReplayThumbnailCollection::LayoutUpperPanels(int nStartY, int nBgWidth) {
  ...
  m_pTitleLabel->SetBounds(m_nStartX, nUnconvertedY, GetWide(),
                           m_pTitleLabel->GetTall());
  m_pTitleLabel->SetVisible(true);

  int cx, cy;
  int nWarningStartY =
      nUnconvertedY + m_pTitleLabel->GetTall() + REPLAY_BUFFER_HEIGHT;
  ...
  // Setup line
  int nLineBuffer =
      m_pReplayItemManager->GetItemCount() > 0 ? YRES(15) : nStartY;
...
void CReplayThumbnailCollection::LayoutBackgroundPanel(int nWide, int nTall) {
  // Setup bounds for dark background, if there are unconverted replays in this
  // collection
  if (m_pUnconvertedBg) {
    ...
    int nRenderAllX =
        m_pUnconvertedBg->GetWide() - m_pRenderAllButton->GetWide() - XRES(5);
    m_pRenderAllButton->SetPos(
        nRenderAllX, nWarningLabelY - m_pRenderAllButton->GetTall() / 2);
...
CMovieThumbnailCollection::CMovieThumbnailCollection(
    CReplayListPanel *pParent, const char *pName,
    IReplayItemManager *pReplayItemManager, int nDay, int nMonth, int nYear,
...
void CMovieThumbnailCollection::Init(int nDay, int nMonth, int nYear,
                                     bool bShowSavedMoviesLabel) {
  m_nDay = nDay;
  m_nMonth = nMonth;
  m_nYear = nYear;
  ...
  if (m_nDay == OLDER_MOVIES_COLLECTION) {
    ...
    m_pDateLabel = m_nDay >= 0
                       ? new CExLabel(this, "DateLabel",
                                      CReplayTime::GetLocalizedDate(
                                          g_pVGuiLocalize, nDay, nMonth, nYear))
                       : NULL;
  }
  m_bShowSavedMoviesLabel = bShowSavedMoviesLabel;
}

bool CMovieThumbnailCollection::IsMovieCollection() const { return true; }

void CMovieThumbnailCollection::PerformLayout() {
  BaseClass::PerformLayout();

  // Movies have multiple collections under a single header. So we use the total
  // movies, not the amount in this collection.
  if (g_pReplayMovieManager) {
    wchar_t wszCount[10];
    _snwprintf(wszCount, ARRAYSIZE(wszCount), L"%d",
               g_pReplayMovieManager->GetMovieCount());
    wchar_t wszTemp[256];
    g_pVGuiLocalize->ConstructString(
        wszTemp, sizeof(wszTemp), g_pVGuiLocalize->Find("#Replay_SavedMovies"),
        1, wszCount);
    SetDialogVariable("titleandcount", wszTemp);
  }
...
void CMovieThumbnailCollection::LayoutUpperPanels(int nStartY, int nBgWidth) {
  if (m_pTitleLabel && m_pTitleLabel->IsVisible()) {
    ...
    m_pCaratLabel->SizeToContents();
    m_pCaratLabel->SetPos(
        m_nStartX - m_pCaratLabel->GetWide() - XRES(3),
