...
// TODO: Make these archive?  Right now, the tips are reset every time the game
// starts
ConVar replay_perftip_count_enter("replay_perftip_count_enter", "0",
                                  FCVAR_CLIENTDLL | FCVAR_DONTRECORD |
                                      FCVAR_HIDDEN,
                                  "", true, 0, false, 0);
ConVar replay_perftip_count_exit("replay_perftip_count_exit", "0",
                                 FCVAR_CLIENTDLL | FCVAR_DONTRECORD |
                                     FCVAR_HIDDEN,
                                 "", true, 0, false, 0);
ConVar replay_perftip_count_freecam_enter("replay_perftip_count_freecam_enter",
                                          "0",
                                          FCVAR_CLIENTDLL | FCVAR_DONTRECORD |
                                              FCVAR_HIDDEN,
                                          "", true, 0, false, 0);
ConVar replay_perftip_count_freecam_exit("replay_perftip_count_freecam_exit",
                                         "0",
                                         FCVAR_CLIENTDLL | FCVAR_DONTRECORD |
                                             FCVAR_HIDDEN,
                                         "", true, 0, false, 0);
ConVar replay_perftip_count_freecam_exit2("replay_perftip_count_freecam_exit2",
                                          "0",
                                          FCVAR_CLIENTDLL | FCVAR_DONTRECORD |
                                              FCVAR_HIDDEN,
                                          "", true, 0, false, 0);
...
ConVar replay_editor_fov_mousewheel_multiplier(
    ...
    "The multiplier on mousewheel input for adjusting camera FOV in the replay "
    "editor.");
ConVar replay_editor_fov_mousewheel_invert(
    "replay_editor_fov_mousewheel_invert", "0",
    FCVAR_ARCHIVE | FCVAR_CLIENTDLL | FCVAR_DONTRECORD,
    "Invert FOV zoom/unzoom on mousewheel in the replay editor.");

ConVar replay_replayeditor_rewindmsgcounter(
...
inline float LerpScale(float flIn, float flInMin, float flInMax, float flOutMin,
                       float flOutMax) {
...
void HighlightTipWords(Label *pLabel) {
  ...
  wchar_t *pwNumWords =
      g_pVGuiLocalize->Find("#Replay_PerfTip_Highlight_NumWords");
  if (!pwNumWords)
    return;
  ...
  for (int i = 0; i < nNumWords; ++i) {
    ...
    V_snprintf(szWordFindStr, sizeof(szWordFindStr),
               "#Replay_PerfTip_Highlight_Word%i", i);
    ...
    do {
      ...
      if (nStartPos == 0) {
        ...
        // If this is not the beginning of the string, check the previous
        // character.  If it's not whitespace, etc, we found an instance of a
        // keyword within another word.  Skip.
        bChangeColor = false;
      }

      if (bChangeColor) {
        pLabel->GetTextImage()->AddColorChange(Color(200, 80, 60, 255),
                                               nStartPos);
...
public:
  ...
  CPerformanceTip(const char *pText)
      : BaseClass(g_pClientMode->GetViewport(), "Tip"),
        m_flBornTime(gpGlobals->realtime), m_flAge(0.0f),
  ...
  virtual void OnThink() {
    ...
    if (gpGlobals->realtime >= flEndTime - flFadeDuration) {
      flAlpha = LerpScale(gpGlobals->realtime, flEndTime - flFadeDuration,
                          flEndTime, 1.0f, 0.0f);
    }

    // Fade in?
    else if (gpGlobals->realtime <= m_flBornTime + flFadeDuration) {
      flAlpha = LerpScale(gpGlobals->realtime, m_flBornTime,
                          m_flBornTime + flFadeDuration, 0.0f, 1.0f);
    }
  ...
  virtual void ApplySchemeSettings(IScheme *pScheme) {
    ...
    SetBounds(0, 3 * nScreenH / 4 - nLabelHeight / 2, nScreenW,
              nLabelHeight + 2 * m_nTopBottomMargin);
    m_pTextLabel->SetBounds(m_nLeftRightMarginWidth, m_nTopBottomMargin,
  ...
  CPanelAnimationVarAliasType(int, m_nLeftRightMarginWidth, "left_right_margin",
                              "0", "proportional_xpos");
  CPanelAnimationVarAliasType(int, m_nTopBottomMargin, "top_bottom_margin", "0",
                              "proportional_ypos");
...
// Display the performance tip if we haven't already displayed it
// nMaxTimesToDisplay times or more
inline void DisplayPerformanceTip(const char *pText, ConVar *pCountCv = NULL,
                                  int nMaxTimesToDisplay = -1) {
...
public:
  CPlayerCell(Panel *pParent, const char *pName, int *pCurTargetPlayerIndex)
      : CExImageButton(pParent, pName, ""), m_iPlayerIndex(-1),
  ...
  MESSAGE_FUNC(DoClick, "PressButton") {
    ...
    *m_pCurTargetPlayerIndex = m_iPlayerIndex;
    ...
    extern IReplayPerformanceController *g_pReplayPerformanceController;
    g_pReplayPerformanceController->AddEvent_Camera_ChangePlayer(
        flCurTime, m_iPlayerIndex);
  }

  int m_iPlayerIndex;
  int *m_pCurTargetPlayerIndex; // Allow the button to write current target in
                                // outer class when pressed
};
...
public:
  ...
  virtual void ApplySchemeSettings(IScheme *pScheme) {
    ...
    FOR_EACH_LL(m_lstSliderInfos, it) {
      ...
      pSlider->SetValue(SLIDER_RANGE_MAX *
                        fabs(pInfo->m_flDefault - pInfo->m_flRange[0]) /
                        flDenom);
    }
  }
  ...
  virtual void PerformLayout() {
    ...
    FOR_EACH_LL(m_lstControls, i) {
      ...
      if (dynamic_cast<CExButton *>(pPanel)) {
        ...
        pPanel->SetPos(
            pPrevPanel ? (GetWide() - nMargins[0] - pPanel->GetWide()) : 0,
            nLastY);
      } else {
  ...
  CExButton *NewSetDefaultButton(int nSliderInfoId) {
    CExButton *pButton =
        new CExButton(this, "DefaultButton", "#Replay_SetDefaultSetting");
    pButton->SetTall(YRES(11));
    pButton->SetPos(XRES(30),
                    -1); // Set y to -1 so it will stay on the same line
...
protected:
  MESSAGE_FUNC_PARAMS(OnSliderMoved, "SliderMoved", pParams) {
    ...
    FOR_EACH_LL(m_lstSliderInfos, it) {
      ...
      if (pSlider == pInfo->m_pSlider) {
        *pInfo->m_pValueOut =
            Lerp(flPercent, pInfo->m_flRange[0], pInfo->m_flRange[1]);
      }
    }
  }
  ...
  void SetValue(const SliderInfo_t *pSliderInfo, float flValue) {
    ...
    // Calculate the range
    const float flRange =
        fabs(pSliderInfo->m_flRange[1] - pSliderInfo->m_flRange[0]);
...
public:
  CTimeScaleOptionsPanel(Panel *pParent, float *pTimeScaleProxy)
      : BaseClass(pParent, "TimeScaleSettings", "#Replay_TimeScale"),
        m_pTimeScaleSlider(NULL), m_pTimeScaleProxy(pTimeScaleProxy) {}
  ...
  virtual void AddControls() {
    m_pTimeScaleSlider =
        dynamic_cast<Slider *>(FindChildByName("TimeScaleSlider"));

    AddSliderToLayout(SLIDER_TIMESCALE, m_pTimeScaleSlider, "#Replay_Scale",
                      TIMESCALE_MIN, TIMESCALE_MAX, *m_pTimeScaleProxy);
  }
...
public:
  CCameraOptionsPanel_Free(Panel *pParent)
      : BaseClass(pParent, "FreeCameraSettings", "#Replay_FreeCam"),
        m_pAccelSlider(NULL), m_pSpeedSlider(NULL), m_pFovSlider(NULL),
        m_pRotFilterSlider(NULL), m_pShakeSpeedSlider(NULL),
  ...
  virtual void AddControls() {
    ...
    m_pRotFilterSlider =
        dynamic_cast<Slider *>(FindChildByName("RotFilterSlider"));
    m_pShakeSpeedSlider =
        dynamic_cast<Slider *>(FindChildByName("ShakeSpeedSlider"));
    m_pShakeAmountSlider =
        dynamic_cast<Slider *>(FindChildByName("ShakeAmountSlider"));
    m_pShakeDirSlider =
        dynamic_cast<Slider *>(FindChildByName("ShakeDirSlider"));

    AddSliderToLayout(SLIDER_ACCEL, m_pAccelSlider, "#Replay_Accel",
                      FREE_CAM_ACCEL_MIN, FREE_CAM_ACCEL_MAX,
                      ReplayCamera()->m_flRoamingAccel);
    AddSliderToLayout(SLIDER_SPEED, m_pSpeedSlider, "#Replay_Speed",
                      FREE_CAM_SPEED_MIN, FREE_CAM_SPEED_MAX,
                      ReplayCamera()->m_flRoamingSpeed);
    AddSliderToLayout(SLIDER_FOV, m_pFovSlider, "#Replay_Fov", FREE_CAM_FOV_MIN,
                      FREE_CAM_FOV_MAX, ReplayCamera()->m_flRoamingFov[1]);
    AddSliderToLayout(SLIDER_ROTFILTER, m_pRotFilterSlider, "#Replay_RotFilter",
                      FREE_CAM_ROT_FILTER_MIN, FREE_CAM_ROT_FILTER_MAX,
                      ReplayCamera()->m_flRoamingRotFilterFactor);
    AddSliderToLayout(SLIDER_SHAKE_SPEED, m_pShakeSpeedSlider,
                      "#Replay_ShakeSpeed", FREE_CAM_SHAKE_SPEED_MIN,
                      FREE_CAM_SHAKE_SPEED_MAX,
                      ReplayCamera()->m_flRoamingShakeSpeed);
    AddSliderToLayout(SLIDER_SHAKE_AMOUNT, m_pShakeAmountSlider,
                      "#Replay_ShakeAmount", FREE_CAM_SHAKE_AMOUNT_MIN,
                      FREE_CAM_SHAKE_AMOUNT_MAX,
                      ReplayCamera()->m_flRoamingShakeAmount);
    AddSliderToLayout(SLIDER_SHAKE_DIR, m_pShakeDirSlider, "#Replay_ShakeDir",
                      FREE_CAM_SHAKE_DIR_MIN, FREE_CAM_SHAKE_DIR_MAX,
                      ReplayCamera()->m_flRoamingShakeDir);
  }
...
#define MAX_FF_RAMP_TIME                                                       \
  8.0f // The amount of time until we ramp to max scale value.
...
public:
  CReplayEditorFastForwardButton(Panel *pParent, const char *pName,
                                 const char *pText)
  ...
  void OnTick() {
    ...
    if (m_flPressTime == 0.0f) {
      ...
      const float flElapsed =
          clamp(gpGlobals->realtime - m_flPressTime, 0.0f, MAX_FF_RAMP_TIME);
...
DECLARE_BUILD_FACTORY_DEFAULT_TEXT(CReplayEditorFastForwardButton,
                                   CExImageButton);
...
public:
  CRecLightPanel(Panel *pParent)
      : EditablePanel(pParent, "RecLightPanel"), m_flPlayPauseTime(0.0f),
        m_bPaused(false), m_bPerforming(false) {
    m_pRecLights[0] = NULL;
    m_pRecLights[1] = NULL;
    m_pPlayPause[0] = NULL;
    m_pPlayPause[1] = NULL;
  ...
  virtual void ApplySchemeSettings(IScheme *pScheme) {
    ...
    LoadControlSettings("resource/ui/replayperformanceeditor/reclight.res",
                        "GAME");

    m_pRecLights[0] =
        dynamic_cast<ImagePanel *>(FindChildByName("RecLightOffImg"));
    m_pRecLights[1] =
        dynamic_cast<ImagePanel *>(FindChildByName("RecLightOnImg"));
    ...
    m_pCameraFringe =
        dynamic_cast<ImagePanel *>(FindChildByName("CameraFringe"));
    m_pCameraCrosshair =
        dynamic_cast<ImagePanel *>(FindChildByName("CameraCrosshair"));
  }
  ...
  virtual void OnThink() {
    ...
    if (bPauseAnimating) {
      const float t =
          clamp((flTime - m_flPlayPauseTime) / m_flAnimTime, 0.0f, 1.0f);
      ...
      const int nScreenXCenter =
          aCrossHairPos[0] + m_pCameraCrosshair->GetWide() / 2;
      const int nScreenYCenter =
          aCrossHairPos[1] + m_pCameraCrosshair->GetTall() / 2;

      m_pPlayPause[iPlayPauseActive]->SetBounds(
          nScreenXCenter - nSize / 2, nScreenYCenter - nSize / 2, nSize, nSize);

      m_pPlayPause[iPlayPauseActive]->SetAlpha(
          (int)(MIN(0.5f, 1.0f - s) * 255));
    }
...
CReplayPerformanceEditorPanel::CReplayPerformanceEditorPanel(
    Panel *parent, ReplayHandle_t hReplay)
    : EditablePanel(parent, "ReplayPerformanceEditor"), m_hReplay(hReplay),
      m_flLastTime(-1), m_nRedBlueLabelRightX(0), m_nBottomPanelStartY(0),
      m_nBottomPanelHeight(0), m_nLastRoundedTime(-1), m_flSpaceDownStart(0.0f),
      m_flOldFps(-1.0f), m_flLastTimeSpaceBarPressed(0.0f),
      m_flActiveTimeInEditor(0.0f), m_flTimeScaleProxy(1.0f),
      m_iCameraSelection(CAM_FIRST), m_bMousePressed(false),
      m_bMouseDown(false), m_nMouseClickedOverCameraSettingsPanel(CAM_INVALID),
      m_bShownAtLeastOnce(false), m_bAchievementAwarded(false),
      m_pImageList(NULL), m_pCurTimeLabel(NULL), m_pTotalTimeLabel(NULL),
      m_pPlayerNameLabel(NULL), m_pMouseTargetPanel(NULL),
      m_pSlowMoButton(NULL), m_pRecLightPanel(NULL), m_pPlayerCellData(NULL),
      m_pBottom(NULL), m_pMenuButton(NULL), m_pMenu(NULL),
      m_pPlayerCellsPanel(NULL), m_pButtonTip(NULL), m_pSavingDlg(NULL) {
  ...
  m_pCameraOptionsPanels[COMPONENT_TIMESCALE] =
      new CTimeScaleOptionsPanel(this, &m_flTimeScaleProxy);
  ...
  HScheme hScheme = scheme()->LoadSchemeFromFileEx(
  ...
  for (int i = 0; i < 2; ++i) {
    for (int j = 0; j <= MAX_PLAYERS; ++j) {
      m_pPlayerCells[i][j] = new CPlayerCell(m_pPlayerCellsPanel, "PlayerCell",
                                             &m_iCurPlayerTarget);
  ...
  m_pRecLightPanel =
      SETUP_PANEL(new CRecLightPanel(g_pClientMode->GetViewport()));

  // Display "enter performance mode" tip
  DisplayPerformanceTip("#Replay_PerfTip_EnterPerfMode",
                        &replay_perftip_count_enter, MAX_TIP_DISPLAYS);

  // Create menu
  m_pMenu = new Menu(this, "Menu");
  m_aMenuItemIds[MENU_SAVE] =
      m_pMenu->AddMenuItem("#Replay_Save", "menu_save", this);
  m_aMenuItemIds[MENU_SAVEAS] =
      m_pMenu->AddMenuItem("#Replay_SaveAs", "menu_saveas", this);
  m_pMenu->AddSeparator();
  m_aMenuItemIds[MENU_EXIT] =
      m_pMenu->AddMenuItem("#Replay_Exit", "menu_exit", this);

  m_pMenu->EnableUseMenuManager(
      false); // The menu manager doesn't play nice with the menu button
}
...
void CReplayPerformanceEditorPanel::AddPanelKeyboardInputDisableList(
    Panel *pPanel) {
...
void CReplayPerformanceEditorPanel::ApplySchemeSettings(IScheme *pScheme) {
  ...
  if (m_pBottom) {
    ...
    m_pBottom->SetBounds(0, m_nBottomPanelStartY, nParentWidth,
  ...
  const char *pCameraButtonNames[nNumCameraButtons] = {
      "CameraFree", "CameraThird", "CameraFirst", "TimeScaleButton"};
  int nCurButtonX = nParentWidth - m_nRightMarginWidth;
  int nLeftmostCameraButtonX = 0;
  for (int i = 0; i < nNumCameraButtons; ++i) {
    m_pCameraButtons[i] =
        dynamic_cast<CExImageButton *>(FindChildByName(pCameraButtonNames[i]));
  ...
  static const char *s_pControlButtonNames[NUM_CTRLBUTTONS] = {
      "InButton",          "GotoBeginningButton", "RewindButton", "PlayButton",
      "FastForwardButton", "GotoEndButton",       "OutButton"};
  for (int i = 0; i < NUM_CTRLBUTTONS; ++i) {
    CExImageButton *pCurButton = dynamic_cast<CExImageButton *>(
        FindChildByName(s_pControlButtonNames[i]));
    Assert(pCurButton);
    if (!pCurButton)
      continue;
  ...
  // If the performance in tick is set, highlight the in point button
  {
    ...
    m_pCtrlButtons[CTRLBUTTON_IN]->SetSelected(pSavedPerformance &&
                                               pSavedPerformance->HasInTick());
    m_pCtrlButtons[CTRLBUTTON_OUT]->SetSelected(
        pSavedPerformance && pSavedPerformance->HasOutTick());
  }
  ...
  m_pTotalTimeLabel =
      dynamic_cast<CExLabel *>(FindChildByName("TotalTimeLabel"));
  ...
  m_pPlayerNameLabel =
      dynamic_cast<CExLabel *>(FindChildByName("PlayerNameLabel"));

  // Get mouse target panel
  m_pMouseTargetPanel =
      dynamic_cast<EditablePanel *>(FindChildByName("MouseTargetPanel"));
...
void CReplayPerformanceEditorPanel::HandleMouseWheel(int nDelta) {
  if (ReplayCamera()->GetMode() == OBS_MODE_ROAMING) {
    ...
    flFov = clamp(
        flFov - nDelta * replay_editor_fov_mousewheel_multiplier.GetFloat(),
        FREE_CAM_FOV_MIN, FREE_CAM_FOV_MAX);

    // Update FOV slider in free camera settings
    CCameraOptionsPanel_Free *pFreeCamOptions =
        static_cast<CCameraOptionsPanel_Free *>(
            m_pCameraOptionsPanels[CAM_FREE]);
...
CameraMode_t CReplayPerformanceEditorPanel::IsMouseOverActiveCameraOptionsPanel(
    int nMouseX, int nMouseY) {
  // In one of the camera options panels?
  for (int i = 0; i < NCAMS; ++i) {
    CCameraOptionsPanel *pCurPanel = m_pCameraOptionsPanels[i];
    if (pCurPanel && pCurPanel->IsVisible() &&
        pCurPanel->IsWithin(nMouseX, nMouseY))
...
void CReplayPerformanceEditorPanel::OnTick() {
  ...
  //	engine->Con_NPrintf( 0, "timescale: %f",
  //g_pReplayPerformanceController->GetPlaybackTimeScale() );
  ...
  // If this is the first time we're running and camera is valid, get primary
  // target
  ...
  bool bInAControllableCameraMode =
      nCameraMode == OBS_MODE_ROAMING || nCameraMode == OBS_MODE_CHASE;
  ...
  if (IsVisible() && m_bMousePressed) {
    CameraMode_t nActiveOptionsPanel =
        IsMouseOverActiveCameraOptionsPanel(nMouseX, nMouseY);
    if (nActiveOptionsPanel != CAM_INVALID) {
      ...
      ToggleMenu();
    } else if (bInAControllableCameraMode && !bMouseCursorOverPerfEditor &&
               !bMouseOverMenuButton && !bMouseOverMenu && bNoDialogsUp) {
      if (bRecording) {
        bool bMouseInputEnabled = IsMouseInputEnabled();

        // Already in a controllable camera mode?
        if (bMouseInputEnabled) {
          DisplayPerformanceTip("#Replay_PerfTip_ExitFreeCam",
  ...
  bool bSpaceDown = bNoDialogsUp && !enginevgui->IsGameUIVisible() &&
  ...
  C_ReplayGame_PlayerResource_t *pGamePlayerResource =
      dynamic_cast<C_ReplayGame_PlayerResource_t *>(g_PR);
  for (int iPlayer = 1; iPlayer <= MAX_PLAYERS; ++iPlayer) {
    ...
    char const *pClassName =
        iPlayerClass == TF_CLASS_DEMOMAN
            ? "demo"
            : g_aPlayerClassNames_NonLocalized[iPlayerClass];
    V_snprintf(szClassImg, sizeof(szClassImg), "../HUD/leaderboard_class_%s",
               pClassName);
  ...
  // Check to see if we're hovering over a camera-mode, and if so, display its
  // options panel if it has one
  if (bRecording) {
    for (int i = 0; i < NCAMS; ++i) {
      ...
      bool bMouseOverOptionsPanel =
          pCurOptionsPanel->IsWithin(nMouseX, nMouseY);
      bool bInCameraModeThatMouseIsOver =
          ReplayCamera()->GetMode() ==
          GetCameraModeFromButtonIndex((CameraMode_t)i);
      bool bDontCareAboutCameraMode = i == COMPONENT_TIMESCALE;
      bool bActivate =
          (i == m_nMouseClickedOverCameraSettingsPanel) ||
          (((bInCameraModeThatMouseIsOver || bDontCareAboutCameraMode) &&
            bMouseOverButton) ||
  ...
  if (m_pPlayerNameLabel && pGamePlayerResource && pMouseOverCell) {
    m_pPlayerNameLabel->SetText(
        pGamePlayerResource->GetPlayerName(iMouseOverPlayerIndex));
    ...
    int nLabelY = m_nBottomPanelStartY +
                  (m_nBottomPanelHeight - m_pPlayerNameLabel->GetTall()) / 2;
  ...
  // If user clicked, assume it was the selected cell and set primary target in
  // camera
  ...
  // fixes a case where the replay would be paused and the player would cycle
  // cameras but the target's visibility wouldn't be updated until the replay
  // was unpaused (they would be invisible)
  if (m_bCurrentTargetNeedsVisibilityUpdate) {
    C_BaseEntity *pTarget =
        ClientEntityList().GetEnt(pCamera->GetPrimaryTargetIndex());
    if (pTarget) {
...
void CReplayPerformanceEditorPanel::Achievements_Think(float flElapsed) {
  ...
  //	engine->Con_NPrintf( 11, "last time space bar pressed: %f",
  //m_flLastTimeSpaceBarPressed );
...
int CReplayPerformanceEditorPanel::GetCameraModeFromButtonIndex(
    CameraMode_t iCamera) {
  switch (iCamera) {
  case CAM_FREE:
...
void CReplayPerformanceEditorPanel::UpdateTimeLabels() {
  ...
  g_pClientReplayContext->GetPlaybackTimes(flCurTime, flTotalTime,
                                           pPlayingReplay, GetPerformance());
  ...
  char szTimeText[64];
  V_snprintf(szTimeText, sizeof(szTimeText), "%s",
             CReplayTime::FormatTimeString(nCurRoundedTime));
  m_pCurTimeLabel->SetText(szTimeText);

  // Set total time text
  V_snprintf(szTimeText, sizeof(szTimeText), "%s",
             CReplayTime::FormatTimeString((int)flTotalTime));
...
void CReplayPerformanceEditorPanel::UpdateCameraSelectionPosition(
    CameraMode_t nCameraMode) {
...
void CReplayPerformanceEditorPanel::UpdateFreeCamSettings(
    const SetViewParams_t &params) {
  CCameraOptionsPanel_Free *pSettingsPanel =
      dynamic_cast<CCameraOptionsPanel_Free *>(
          m_pCameraOptionsPanels[CAM_FREE]);
  if (!pSettingsPanel)
    return;

  pSettingsPanel->SetValue(CCameraOptionsPanel_Free::SLIDER_ACCEL,
                           params.m_flAccel);
  pSettingsPanel->SetValue(CCameraOptionsPanel_Free::SLIDER_SPEED,
                           params.m_flSpeed);
  pSettingsPanel->SetValue(CCameraOptionsPanel_Free::SLIDER_FOV,
                           params.m_flFov);
  pSettingsPanel->SetValue(CCameraOptionsPanel_Free::SLIDER_ROTFILTER,
...
void CReplayPerformanceEditorPanel::UpdateTimeScale(float flScale) {
  CTimeScaleOptionsPanel *pSettingsPanel =
      dynamic_cast<CTimeScaleOptionsPanel *>(
          m_pCameraOptionsPanels[COMPONENT_TIMESCALE]);
  if (!pSettingsPanel)
    return;
...
void CReplayPerformanceEditorPanel::LayoutPlayerCells() {
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    for (int j = 0; j <= MAX_PLAYERS; ++j) {
      ...
      const int nY = nPanelHeight / 2 + m_nRedBlueSigns[i] * nPanelHeight / 4 -
                     pCurCell->GetTall() / 2;
...
void CReplayPerformanceEditorPanel::PerformLayout() {
  ...
  for (int i = 0; i < NCAMS; ++i) {
    ...
    // Layout the panel now - it should set its own size, which we need to know
    // to position it properly
    pCurOptionsPanel->InvalidateLayout(true, true);

    // Position it
    pCurOptionsPanel->SetPos(
        aBottomPos[0] + aCameraButtonPos[0] + pCurCameraButton->GetWide() -
  ...
  m_pMenu->SetPos(aMenuButtonPos[0],
                  aMenuButtonPos[1] + m_pMenuButton->GetTall());

  // Set player cell panel to be the size of half the bottom panel
  int aBottomSize[2];
  m_pBottom->GetSize(aBottomSize[0], aBottomSize[1]);
  m_pPlayerCellsPanel->SetBounds(0, 0, aBottomSize[0] / 2,
                                 m_pPlayerCellsPanel->GetTall());

  CExLabel *pRedBlueLabels[2] = {
      dynamic_cast<CExLabel *>(
          m_pPlayerCellsPanel->FindChildByName("RedLabel")),
      dynamic_cast<CExLabel *>(
          m_pPlayerCellsPanel->FindChildByName("BlueLabel"))};
  int nMargins[2] = {XRES(5), YRES(2)};
  for (int i = 0; i < 2; ++i) {
    pRedBlueLabels[i]->SizeToContents();

    const int nY = m_pPlayerCellsPanel->GetTall() / 2 +
                   m_nRedBlueSigns[i] * m_pPlayerCellsPanel->GetTall() / 4 -
                   pRedBlueLabels[i]->GetTall() / 2;
    pRedBlueLabels[i]->SetPos(nMargins[0], nY);

    m_nRedBlueLabelRightX =
        MAX(m_nRedBlueLabelRightX,
            nMargins[0] + pRedBlueLabels[i]->GetWide() + nMargins[0]);
  }
...
bool CReplayPerformanceEditorPanel::OnStateChangeRequested(
    const char *pEventStr) {
  // If we're already recording, allow the change.
  if (g_pReplayPerformanceController->IsRecording())
    return true;

  // If we aren't recording and there is no forthcoming data in the playback
  // stream, allow the change.
  if (!g_pReplayPerformanceController->IsPlaybackDataLeft())
    return true;

  // Otherwise, record the event string and show a dialog asking the user if
  // they're sure they want to nuke.
  V_strncpy(m_szSuspendedEvent, pEventStr, sizeof(m_szSuspendedEvent));
  ShowConfirmDialog("#Replay_Warning", "#Replay_NukePerformanceChanges",
                    "#GameUI_Confirm", "#GameUI_CancelBold",
                    OnConfirmDestroyChanges, this, this,
                    REPLAY_SOUND_DIALOG_POPUP);

  return false;
}

void CReplayPerformanceEditorPanel::SetButtonTip(wchar_t *pTipText,
                                                 Panel *pContextPanel) {
  // Set the text
  ...
  const int nX = clamp(aPos[0] - m_pButtonTip->GetWide() / 2, 0,
...
void CReplayPerformanceEditorPanel::ShowPanel(bool bShow) {
  ...
  if (bShow) {
    ...
    DisplayPerformanceTip("#Replay_PerfTip_ExitPerfMode",
                          &replay_perftip_count_exit, MAX_TIP_DISPLAYS);

    // Fire a message the game DLL can intercept (for achievements, etc).
    IGameEvent *event =
        gameeventmanager->CreateEvent("entered_performance_mode");
    if (event) {
  ...
  // Keep controller informed about pause state so that it can throw away
  // unimportant events during pause if it's recording.
...
void CReplayPerformanceEditorPanel::AddTimeScaleEvent(float flTimeScale) {
  ...
  g_pReplayPerformanceController->AddEvent_TimeScale(GetPlaybackTime(),
                                                     flTimeScale);
}

void CReplayPerformanceEditorPanel::UpdateCameraButtonImages(
    bool bForceUnselected /*=false*/) {
  CReplayPerformance *pPerformance = GetPerformance();
  for (int i = 0; i < NCAMS; ++i) {
    CFmtStr fmtFile(
        gs_pBaseComponentNames[i], gs_pCamNames[i],
        (!bForceUnselected &&
         (!pPerformance || g_pReplayPerformanceController->IsRecording()) &&
...
/*static*/ void CReplayPerformanceEditorPanel::OnConfirmSaveAs(bool bShouldSave,
                                                               wchar_t *pTitle,
                                                               void *pContext) {
  ...
  CReplayPerformanceEditorPanel *pThis =
      (CReplayPerformanceEditorPanel *)pContext;
  pThis->SaveAs(pTitle);
...
void CReplayPerformanceEditorPanel::ShowRewindConfirmMessage() {
  ShowMessageBox("#Replay_RewindWarningTitle", "#Replay_RewindWarningMsg",
                 "#GameUI_OK", OnConfirmRewind, NULL, (void *)this);
  surface()->PlaySound("replay\\replaydialog_warn.wav");
}

/*static*/ void CReplayPerformanceEditorPanel::OnConfirmRewind(bool bConfirmed,
                                                               void *pContext) {
  if (bConfirmed) {
    if (pContext) {
      CReplayPerformanceEditorPanel *pEditor =
          (CReplayPerformanceEditorPanel *)pContext;
...
void CReplayPerformanceEditorPanel::OnMenuCommand_Save(
    bool bExitEditorWhenDone /*=false*/) {
...
void CReplayPerformanceEditorPanel::OnMenuCommand_SaveAs(
    bool bExitEditorWhenDone /*=false*/) {
  ReplayUI_ShowPerformanceSaveDlg(OnConfirmSaveAs, this, GetReplay(),
                                  bExitEditorWhenDone);
}

void CReplayPerformanceEditorPanel::DisplaySavedTip(bool bSucceess) {
  DisplayPerformanceTip(bSucceess ? "#Replay_PerfTip_Saved"
...
void CReplayPerformanceEditorPanel::Exit_ShowDialogs() {
  if (g_pReplayPerformanceController->IsDirty()) {
    ShowConfirmDialog("#Replay_DiscardTitle", "#Replay_DiscardChanges",
                      "#Replay_Discard", "#Replay_Cancel", OnConfirmDiscard,
                      NULL, this, REPLAY_SOUND_DIALOG_POPUP);
  } else {
    ShowConfirmDialog("#Replay_ExitEditorTitle", "#Replay_BackToReplays",
                      "#GameUI_Confirm", "#Replay_Cancel", OnConfirmExit, NULL,
...
void CReplayPerformanceEditorPanel::OnCommand(const char *command) {
  ...
  if (!V_stricmp(command, "toggle_menu")) {
    ...
    if (!V_stricmp(pTimeScaleCmd, "showpanel")) {
      // If we're playing back, pop up a dialog asking if the user is sure they
      // want to nuke the rest of whatever is playing back.
    ...
    CReplay *pReplay = GetReplay();
    if (pReplay) {
      const CReplayPerformance *pScratchPerformance =
          g_pReplayPerformanceController->GetPerformance();
      const CReplayPerformance *pSavedPerformance =
          g_pReplayPerformanceController->GetSavedPerformance();
      const CReplayPerformance *pPerformance =
          pScratchPerformance ? pScratchPerformance : pSavedPerformance;

      const int nCurTick = engine->GetDemoPlaybackTick();

      // If in or out ticks are set in the performance, use those for the 'full'
      // rewind/fast-forward
      const int nStartTick = MAX(0, (pPerformance && pPerformance->HasInTick())
                                        ? pPerformance->m_nTickIn
                                        : pReplay->m_nSpawnTick);
      const int nEndTick = MAX( // The MAX() here will keep us from going back
                                // in time if we're already past the "end" tick
          nCurTick, ((pPerformance && pPerformance->HasOutTick())
      ...
      // If going back...
      if (bGoingBack) {
        // ...and notify the recorder that we're skipping, which we only need to
        // do if we're going backwards
    ...
    if (!V_stricmp(pCamType, "first")) {
      ...
      g_pReplayPerformanceController->AddEvent_Camera_Change_FirstPerson(
          flCurTime, nEntIndex);
    } else if (!V_stricmp(pCamType, "third")) {
      ReplayCamera()->SetMode(OBS_MODE_CHASE);
      UpdateCameraSelectionPosition(CAM_THIRD);
      m_bCurrentTargetNeedsVisibilityUpdate = true;
      g_pReplayPerformanceController->AddEvent_Camera_Change_ThirdPerson(
          flCurTime, nEntIndex);
      AddSetViewEvent();
...
void CReplayPerformanceEditorPanel::OnConfirmDestroyChanges(bool bConfirmed,
                                                            void *pContext) {
  AssertMsg(pContext, "Should have a context!  Fix me!");
  if (pContext && bConfirmed) {
    CReplayPerformanceEditorPanel *pEditorPanel =
        (CReplayPerformanceEditorPanel *)pContext;
    if (bConfirmed) {
...
/*static*/ void
CReplayPerformanceEditorPanel::OnConfirmDiscard(bool bConfirmed,
                                                void *pContext) {
  CReplayPerformanceEditorPanel *pEditor =
      (CReplayPerformanceEditorPanel *)pContext;
  if (bConfirmed) {
...
/*static*/ void CReplayPerformanceEditorPanel::OnConfirmExit(bool bConfirmed,
                                                             void *pContext) {
  CReplayPerformanceEditorPanel *pEditor =
      (CReplayPerformanceEditorPanel *)pContext;
  if (bConfirmed) {
...
void CReplayPerformanceEditorPanel::SetOrRemoveInTick(int nTick,
                                                      bool bRemoveIfSet) {
  SetOrRemoveTick(nTick, true, bRemoveIfSet);
}

void CReplayPerformanceEditorPanel::SetOrRemoveOutTick(int nTick,
                                                       bool bRemoveIfSet) {
  SetOrRemoveTick(nTick, false, bRemoveIfSet);
}

void CReplayPerformanceEditorPanel::SetOrRemoveTick(int nTick, bool bUseInTick,
                                                    bool bRemoveIfSet) {
  CReplayPerformance *pPerformance = GetPerformance();
  ...
  pButton->InvalidateLayout(
      true, true); // Without this, buttons don't update immediately
...
void CReplayPerformanceEditorPanel::OnRewindComplete() {
  // Get rid of any "selected" icon - this will happen as soon as we actually
  // start playing back events, but if we aren't playing back events yet we need
  // to explicitly tell the icons not to display their "selected" versions.
...
ReplayUI_InitPerformanceEditor(ReplayHandle_t hReplay) {
  if (!g_ReplayPerformanceEditorPanel.Get()) {
    g_ReplayPerformanceEditorPanel =
        SETUP_PANEL(new CReplayPerformanceEditorPanel(NULL, hReplay));
    g_ReplayPerformanceEditorPanel->InvalidateLayout(false, true);
  }

  // Notify recorder of editor
  g_pReplayPerformanceController->SetEditor(
      g_ReplayPerformanceEditorPanel.Get());
...
#if _DEBUG
CON_COMMAND_F(replay_showperfeditor, "Show performance editor",
              FCVAR_CLIENTDLL) {
