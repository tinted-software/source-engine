...
CMoviePlayerPanel::CMoviePlayerPanel(Panel *pParent, const char *pName,
                                     const char *pMovieFilename)
    : CReplayBasePanel(pParent, pName), m_flCurFrame(0.0f), m_flLastTime(0.0f),
      m_nLastMouseXPos(0), m_bPlaying(false), m_bLooping(false),
      m_bFullscreen(false), m_bMouseOverScrub(false), m_pOldParent(NULL),
      m_pVideoMaterial(NULL) {
  if (g_pVideo) {
    m_pVideoMaterial =
        g_pVideo->CreateVideoMaterial(pMovieFilename, pMovieFilename, "GAME");
    if (m_pVideoMaterial) {
...
void CMoviePlayerPanel::PerformLayout() {
  ...
  if (m_bFullscreen) {
    ...
    GetBounds(m_aOldBounds[0], m_aOldBounds[1], m_aOldBounds[2],
              m_aOldBounds[3]);
    ...
    SetBounds(m_aOldBounds[0], m_aOldBounds[1], m_aOldBounds[2],
              m_aOldBounds[3]);
  }
}
