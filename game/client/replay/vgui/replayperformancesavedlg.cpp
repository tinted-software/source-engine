...
public:
  ...
  static void Show(OnConfirmSaveCallback pfnCallback, void *pContext,
                   CReplay *pReplay, bool bExitEditorWhenDone);
...
CReplayPerformanceSaveDlg::CReplayPerformanceSaveDlg(
    Panel *pParent, const char *pName, OnConfirmSaveCallback pfnCallback,
    void *pContext, CReplay *pReplay, bool bExitEditorWhenDone)
    : BaseClass(pParent, pName), m_pfnCallback(pfnCallback),
      m_pContext(pContext), m_pReplay(pReplay),
      m_bExitEditorWhenDone(bExitEditorWhenDone), m_pDlg(NULL),
      m_pTitleEntry(NULL) {
  Assert(m_pContext);
...
CReplayPerformanceSaveDlg::~CReplayPerformanceSaveDlg() { ms_hDlg = NULL; }

/*static*/ void
CReplayPerformanceSaveDlg::Show(OnConfirmSaveCallback pfnCallback,
                                void *pContext, CReplay *pReplay,
                                bool bExitEditorWhenDone) {
  Assert(!ms_hDlg.Get());

  ms_hDlg = vgui::SETUP_PANEL(
      new CReplayPerformanceSaveDlg(NULL, "ReplayInputPanel", pfnCallback,
                                    pContext, pReplay, bExitEditorWhenDone));
...
void CReplayPerformanceSaveDlg::ApplySchemeSettings(IScheme *pScheme) {
  ...
  LoadControlSettings("resource/ui/replayperformanceeditor/savedlg.res",
                      "GAME");
  ...
  pDiscardButton =
      dynamic_cast<CExButton *>(m_pDlg->FindChildByName("DiscardButton"));
  ...
  m_pTitleEntry =
      static_cast<TextEntry *>(m_pDlg->FindChildByName("TitleInput"));
  m_pTitleEntry->SelectAllOnFocusAlways(true);
  m_pTitleEntry->SetSelectionBgColor(
...
void CReplayPerformanceSaveDlg::OnSetFocus() { m_pTitleEntry->RequestFocus(); }

/*static*/ void CReplayPerformanceSaveDlg::OnConfirmOverwrite(bool bConfirm,
                                                              void *pContext) {
...
bool CReplayPerformanceSaveDlg::ConfirmOverwriteOrSaveNow() {
  ...
  CReplayPerformance *pExistingPerformance =
      m_pReplay->GetPerformanceWithTitle(m_wszTitle);
  if (pExistingPerformance) {
    ShowConfirmDialog("#Replay_OverwriteDlgTitle", "#Replay_OverwriteDlgText",
                      "#Replay_ConfirmOverwrite", "#Replay_Cancel",
                      OnConfirmOverwrite, NULL, this);
    return false;
  }
...
void CReplayPerformanceSaveDlg::OnCommand(const char *command) {
  ...
  if (!Q_strnicmp(command, "save", 4)) {
    ...
    // If we aren't overwriting an existing performance, this func will return
    // true.
...
void ReplayUI_ShowPerformanceSaveDlg(OnConfirmSaveCallback pfnCallback,
                                     void *pContext, CReplay *pReplay,
                                     bool bExitEditorWhenDone) {
  CReplayPerformanceSaveDlg::Show(pfnCallback, pContext, pReplay,
                                  bExitEditorWhenDone);
}
...
//-----------------------------------------------------------------------------
CON_COMMAND_F(replay_test_take_save_dlg, "Open replay save take dlg",
              FCVAR_NONE) {
