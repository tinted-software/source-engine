...
void CReplayBrowserBasePage::CleanupUIForReplayItem(
...
void CReplayBrowserBasePage::DeleteReplay(ReplayHandle_t hReplayItem) {
  ...
  if (FindReplayItem(hReplayItem, &pItemManager)) {
    ReplayUI_GetBrowserPanel()->AttemptToDeleteReplayItem(this, hReplayItem,
                                                          pItemManager, -1);
  }
}
...
void CReplayBrowserBasePage::GoBack() { DeleteDetailsPanelAndShowReplayList(); }
...
void CReplayBrowserBasePage::OnCommand(const char *pCommand) {
  ...
  if (!V_strnicmp(pCommand, "details", 7)) {
    ...
    QueryableReplayItemHandle_t hReplayItem =
        (QueryableReplayItemHandle_t)atoi(pCommand + 7);
    IReplayItemManager *pItemManager;
    ...
    if (pReplayItem) {
      ...
      m_hReplayDetailsPanel = vgui::SETUP_PANEL(new CReplayDetailsPanel(
          this, hReplayItem, iPerformance, pItemManager));
...
void CReplayBrowserBasePage::PerformLayout() {
  ...
  if (m_pSearchTextEntry) {
    const bool bHasReplays =
        g_pReplayManager && g_pReplayManager->GetReplayCount();
    const bool bHasMovies =
        g_pReplayMovieManager && g_pReplayMovieManager->GetMovieCount();
    int aListPos[2];
    m_pReplayList->GetPos(aListPos[0], aListPos[1]);
    m_pSearchTextEntry->SetPos(aListPos[0] + m_pReplayList->GetWide() -
                                   m_pSearchTextEntry->GetWide(),
                               YRES(5));
    m_pSearchTextEntry->SetVisible(bHasReplays || bHasMovies);
  }

  // Invalidate the list too, because we might be laying out due to a replay
...
bool CReplayBrowserBasePage::IsDetailsViewOpen() {
  return m_hReplayDetailsPanel.Get() != NULL &&
         m_hReplayDetailsPanel->IsVisible();
}
