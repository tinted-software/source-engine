...
CON_COMMAND(testreplaymessagepanel, "") {
  CReplayMessagePanel *pPanel = new CReplayMessagePanel(
      "#Replay_StartRecord", replay_msgduration_misc.GetFloat(),
      rand() % 2 == 0);
  pPanel->Show();
}
...
ConVar replay_msgduration_startrecord("replay_msgduration_startrecord", "6",
                                      FCVAR_DONTRECORD,
                                      "Duration for start record message.",
                                      true, 0.0f, true, 10.0f);
ConVar replay_msgduration_stoprecord("replay_msgduration_stoprecord", "6",
                                     FCVAR_DONTRECORD,
                                     "Duration for stop record message.", true,
                                     0.0f, true, 10.0f);
ConVar replay_msgduration_replaysavailable(
    "replay_msgduration_replaysavailable", "6", FCVAR_DONTRECORD,
    "Duration for replays available message.", true, 0.0f, true, 10.0f);
ConVar replay_msgduration_error("replay_msgduration_error", "6",
                                FCVAR_DONTRECORD,
                                "Duration for replays available message.", true,
                                0.0f, true, 10.0f);
ConVar replay_msgduration_misc(
    "replay_msgduration_misc", "5", FCVAR_DONTRECORD,
    "Duration for misc replays messages (server errors and such).", true, 0.0f,
    true, 10.0f);
ConVar replay_msgduration_connectrecording(
    "replay_msgduration_connectrecording", "8", FCVAR_DONTRECORD,
    "Duration for the message that pops up when you connect to a server "
    "already recording replays.",
    true, 0.0f, true, 15.0f);

//-----------------------------------------------------------------------------

CReplayMessageDlg::CReplayMessageDlg(const char *pText)
    : BaseClass(NULL, "ReplayMessageDlg"), m_pOKButton(NULL), m_pDlg(NULL),
      m_pMsgLabel(NULL) {
...
void CReplayMessageDlg::PerformLayout() {
  ...
  m_pDlg->SetPos((ScreenWidth() - m_pDlg->GetWide()) / 2,
                 (ScreenHeight() - m_pDlg->GetTall()) / 2);
...
ReplayMessagePanelHandle_t
GetReplayMessagePanelHandle(CReplayMessagePanel *pPanel) {
  ReplayMessagePanelHandle_t hThis;
  hThis = pPanel;
  return hThis;
}

CReplayMessagePanel::CReplayMessagePanel(const char *pLocalizeName,
  ...
  const char *pBorderName =
      bUrgent ? "ReplayFatLineBorderRedBGOpaque" : "ReplayFatLineBorderOpaque";
  V_strncpy(m_szBorderName, pBorderName, sizeof(m_szBorderName));
#endif

  g_vecReplayMessagePanels.AddToTail(
      GetReplayMessagePanelHandle(const_cast<CReplayMessagePanel *>(this)));
...
void CReplayMessagePanel::OnTick() {
  ...
  if (gpGlobals->curtime >= flEndTime - flFadeDuration) {
    flAlpha = LerpScale(gpGlobals->curtime, flEndTime - flFadeDuration,
                        flEndTime, 1.0f, 0.0f);
  }

  // Fade in?
  else if (gpGlobals->curtime <= m_flShowStartTime + flFadeDuration) {
    flAlpha = LerpScale(gpGlobals->curtime, m_flShowStartTime,
                        m_flShowStartTime + flFadeDuration, 0.0f, 1.0f);
  }
...
void CReplayMessagePanel::ApplySchemeSettings(IScheme *pScheme) {
  ...
  SetBgColor(pScheme->GetColor(m_bUrgent ? "DarkRed" : "DarkGray",
...
void CReplayMessagePanel::PerformLayout() {
  ...
  // Only display replay icon and "replay" label if this is the top-most
  // (vertically) panel and we're not in min-mode
  Assert(InstanceCount() > 0);
  if (!InstanceCount() || bMinMode ||
      g_vecReplayMessagePanels[0].Get() != this) {
    m_pIcon->SetTall(0);
  ...
  // NOTE: Intentionally using YRES() for xpos, since we want to match offsets
  // in both x & y margins
