...
private:
  ...
  void EnsureRecording(
      bool bShouldSnip = true); // Start recording now if not already doing so
...
private:
  ...
  float m_flSpaceDownStart; // The time at which user started holding down space
                            // bar
  bool m_bSpaceDown;
  ...
  bool m_bAchievementAwarded; // Was an achievement awarded during this editing
                              // session?
