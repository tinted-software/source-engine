...
public:
  ...
  virtual void OnSaveReplay(ReplayHandle_t hNewReplay, bool bShowInputDlg) {
    if (bShowInputDlg) {
      // Get a name for the replay, saves to disk, add thumbnail to replay
      // browser
  ...
  virtual void DisplayReplayMessage(const char *pLocalizeStr, bool bUrgent,
                                    bool bDlg, const char *pSound) {
    // Display a message?
    if (!pLocalizeStr || !pLocalizeStr[0])
      return;

    g_pClientMode->DisplayReplayMessage(pLocalizeStr, -1.0f, bUrgent, pSound,
                                        bDlg);
  }
  ...
  virtual bool OnConfirmQuit() { return ReplayUI_ShowConfirmQuitDlg(); }

  virtual void OnRenderStart() { ReplayUI_OpenReplayRenderOverlay(); }
  ...
  virtual void OnRenderComplete(const RenderMovieParams_t &RenderParams,
    ...
    GetReplayGameStatsHelper().SW_ReplayStats_WriteRenderDataEnd(
        RenderParams, bCancelled ? "cancelled"
                      : bSuccess ? "success"
                                 : "failed");
  }
  ...
  virtual void HidePerformanceEditor() { ReplayUI_ClosePerformanceEditor(); }

  virtual bool ShouldRender() {
    extern ConVar replay_enablerenderpreview;
    return !g_pReplayMovieManager->IsRendering() ||
           replay_enablerenderpreview.GetBool();
  }
  ...
  virtual IReplayCamera *GetReplayCamera() { return ReplayCamera(); }
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CClientReplayImp, IClientReplay,
                                  CLIENT_REPLAY_INTERFACE_VERSION,
                                  s_ClientReplayImp);
