...
void OnReplayCacheClientRagdollsCvarChange(IConVar *pVar, const char *pOldValue,
                                           float flOldValue) {
...
static ConVar
    replay_ragdoll_dbg("replay_ragdoll_dbg", "0", FCVAR_CLIENTDLL,
                       "Display replay ragdoll debugging information.");
static ConVar
    replay_cache_client_ragdolls("replay_cache_client_ragdolls", "0",
                                 FCVAR_CLIENTDLL,
...
void DrawBones(matrix3x4_t const *pBones, int nNumBones,
               ragdoll_t const *pRagdoll, int nRed, int nGreen, int nBlue,
...
/*static*/ RagdollSimulationFrame_t *
RagdollSimulationFrame_t::Alloc(int nNumBones) {
  // TODO: use a mempool
  ...
  gs_nBytesAllocated +=
      sizeof(pNew) + nNumBones * (sizeof(Vector) + sizeof(QAngle));
  return pNew;
}

//--------------------------------------------------------------------------------

RagdollSimulationData_t::RagdollSimulationData_t(C_BaseAnimating *pEntity,
                                                 int nStartTick, int nNumBones)
    : m_pEntity(pEntity), m_nEntityIndex(-1), m_nStartTick(nStartTick),
      m_nNumBones(nNumBones), m_nDuration(-1) {
  if (pEntity) {
...
bool _ComputeRagdollBones(const ragdoll_t *pRagdoll,
                          matrix3x4_t &parentTransform, matrix3x4_t *pBones,
  ...
  for (int i = 0; i < pRagdoll->listCount; ++i) {
    ...
    // during restore if a model has changed since the file was saved, this
    // could be NULL
    if (!element.pObject)
      return false;
    ...
    if (element.parentIndex >= 0 && !pRagdoll->allowStretch) {
      ...
      // NOTE: On the client we actually override this with the proper parent
      // bone in each LOD
      Vector out;
...
void RagdollSimulationData_t::Record() {
  ...
  // Allocate a frame
  RagdollSimulationFrame_t *pNewFrame =
      RagdollSimulationFrame_t::Alloc(m_nNumBones);
  if (!pNewFrame)
    return;
  ...
  _ComputeRagdollBones(m_pEntity->m_pRagdoll->GetRagdoll(), parentTransform,
                       pBones, pNewFrame->pPositions, pNewFrame->pAngles);

  // Draw bones
  if (replay_ragdoll_dbg.GetBool()) {
    DrawBones(pBones, m_pEntity->m_pRagdoll->RagdollBoneCount(),
              m_pEntity->m_pRagdoll->GetRagdoll(), 255, 0, 0, m_pEntity);
  }
}
...
void CReplayRagdollRecorder::Shutdown() {
  ...
  // RemoveAll() purges, and there is no UnlinkAll() - is there an easier way to
  // do this?
...
void CReplayRagdollRecorder::AddEntry(C_BaseAnimating *pEntity, int nStartTick,
                                      int nNumBones) {
  DevMsg("Replay: Processing Ragdoll at time %d\n", nStartTick);

  Assert(pEntity);
  RagdollSimulationData_t *pNewEntry =
      new RagdollSimulationData_t(pEntity, nStartTick, nNumBones);
...
void CReplayRagdollRecorder::StopRecordingSleepingRagdolls() {
  ...
  while (i != m_lstRagdollsToRecord.InvalidIndex()) {
    if (RagdollIsAsleep(
            *m_lstRagdollsToRecord[i]->m_pEntity->m_pRagdoll->GetRagdoll())) {
...
bool CReplayRagdollRecorder::FindEntryInRecordingList(
    C_BaseAnimating *pEntity, CReplayRagdollRecorder::Iterator_t &nOutIndex) {
  // Find the entry
...
void CReplayRagdollRecorder::PrintDebug() {
  ...
  // Print memory usage
  engine->Con_NPrintf(nLine++, "ragdolls: %.2f MB",
                      gs_nBytesAllocated / 1048576.0f);
  ...
  FOR_EACH_LL(m_lstRagdolls, i) {
    engine->Con_NPrintf(
        nLine++, "entity %d: start time=%d  duration=%d  num bones=%d",
...
bool CReplayRagdollRecorder::DumpRagdollsToDisk(char const *pFilename) const {
  ...
  CUtlVector<CDmxElement *> &ragdolls =
      pRagdolls->GetArrayForEdit<CDmxElement *>();
  ...
  FOR_EACH_LL(m_lstRagdolls, i) {
    ...
    CUtlVector<CDmxElement *> &frames =
        pFrames->GetArrayForEdit<CDmxElement *>();
...
bool CReplayRagdollCache::Init(char const *pFilename) {
  ...
  CUtlVector<CDmxElement *> const &ragdolls =
      pRagdolls->GetArray<CDmxElement *>("ragdolls");
  for (int i = 0; i < ragdolls.Count(); ++i) {
    ...
    // Read
    pCurRagdollInput->UnpackIntoStructure(pNewSimData, sizeof(*pNewSimData),
                                          s_RagdollSimulationDataUnpack);

    // NOTE: Entity ptr doesn't get linked up here because it doesn't
    // necessarily exist at this point

    // Read frames
    CUtlVector<CDmxElement *> const &frames =
        pCurRagdollInput->GetArray<CDmxElement *>("frames");
    for (int j = 0; j < frames.Count(); ++j) {
      CDmxElement *pCurFrameInput = frames[j];

      // Create a new frame and add it to list of frames
      RagdollSimulationFrame_t *pNewFrame =
          RagdollSimulationFrame_t::Alloc(pNewSimData->m_nNumBones);
      ...
      pNewFrame->vRootPosition =
          pCurFrameInput->GetValue("root_pos", vec3_origin);
      pNewFrame->angRootAngles =
          pCurFrameInput->GetValue("root_angles", vec3_angle);

      CUtlVector<Vector> const &positions =
          pCurFrameInput->GetArray<Vector>("positions");
      CUtlVector<QAngle> const &angles =
          pCurFrameInput->GetArray<QAngle>("angles");
...
bool CReplayRagdollCache::GetFrame(C_BaseAnimating *pEntity, int nTick,
                                   bool *pBoneSimulated,
  ...
  // Find ragdoll for the given entity - will return NULL if nTick is out of the
  // entry's time window
  const RagdollSimulationData_t *pRagdollEntry =
      FindRagdollEntry(pEntity, nTick);
  if (!pRagdollEntry)
    return false;
  ...
  if (pNextFrame) {
    AngleMatrix(
        (const QAngle &)Lerp(
            flInterpAmount, pFrame->angRootAngles,
  ...
  for (int k = 0; k < pRagdoll->listCount; ++k) {
    ...
    if (pNextFrame && replay_ragdoll_blending.GetInt()) {
      // Get blended Eular angles - NOTE: The Lerp() here actually calls
      // Lerp<QAngle>() which converts to quats and back
      float flInterpAmount = gpGlobals->interpolation_amount;
      Assert(flInterpAmount >= 0.0f && flInterpAmount <= 1.0f);
      AngleMatrix((const QAngle &)Lerp(flInterpAmount,
                                       pFrame->pAngles[objectIndex],
                                       pNextFrame->pAngles[objectIndex]),
                  Lerp(flInterpAmount, pFrame->pPositions[objectIndex],
                       pNextFrame->pPositions[objectIndex]),
                  localTransform);
    } else {
      // Last frame
      AngleMatrix(pFrame->pAngles[objectIndex], pFrame->pPositions[objectIndex],
                  localTransform);
    }
  ...
  if (replay_ragdoll_dbg.GetBool()) {
    DrawBones(pBoneAccessor->GetBoneArrayForWrite(), pRagdollEntry->m_nNumBones,
              pRagdoll, 0, 0, 255, pEntity);
  }

  return true;
}

RagdollSimulationData_t *
CReplayRagdollCache::FindRagdollEntry(C_BaseAnimating *pEntity, int nTick) {
  Assert(pEntity);
...
bool CReplayRagdollCache::FindFrame(
  ...
  FOR_EACH_LL(pRagdollEntry->m_lstFrames, j) {
    ...
    RagdollSimulationFrame_t *pNextFrame =
        nNext == pRagdollEntry->m_lstFrames.InvalidIndex()
            ? NULL
            : pRagdollEntry->m_lstFrames[nNext];

    // Use this frame?
    if (nTick >= pFrame->nTick &&
        ((pNextFrame && nTick <= pNextFrame->nTick) ||
         !pNextFrame)) // Use the last frame if the tick is past the range of
                       // frames -
...
void CReplayRagdollCache::Think() {
  ...
  FOR_EACH_LL( m_lstRagdolls, i )
  {
          engine->Con_NPrintf( 10 + i, "entity %d: start time=%d  duration=%d
  num bones=%d", m_lstRagdolls[i]->m_nEntityIndex,
  m_lstRagdolls[i]->m_nStartTick, m_lstRagdolls[i]->m_nDuration,
...
bool Replay_CacheRagdolls(const char *pFilename, int nStartTick) {
  CReplayRagdollRecorder::Instance().CleanupStartupTicksAndDurations(
      nStartTick);
