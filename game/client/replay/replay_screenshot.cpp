...
// Purpose:
//-----------------------------------------------------------------------------
CReplayScreenshotTaker::CReplayScreenshotTaker(IViewRender *pViewRender,
                                               CViewSetup &view)
    : m_pViewRender(pViewRender), m_View(view) {
  ...
  g_pClientReplayContext->GetScreenshotManager()->GetUnpaddedScreenshotSize(
      m_aUnpaddedDims[0], m_aUnpaddedDims[1]);
  ...
  if (!pVTFTexture->Init(m_aPaddedDims[0], m_aPaddedDims[1], 1,
                         IMAGE_FORMAT_RGB888, nFlags, 1, 1))
    return;
...
//-----------------------------------------------------------------------------
void CReplayScreenshotTaker::TakeScreenshot(
  ...
  pRenderContext->PushRenderTargetAndViewport(m_pScreenshotTarget, 0, 0, width,
                                              height);
  ...
  viewSetup.fov = ScaleFOVByWidthRatio(
      m_View.fov, ((float)width / (float)height) / (4.0f / 3.0f));
  ...
  pRenderContext->ReadPixels(0, 0, width, height, m_pUnpaddedPixels,
                             IMAGE_FORMAT_RGB888);
  ...
  Q_snprintf(szPathedFileName, sizeof(szPathedFileName), "//MOD/%s",
             params.m_pFilename);

  timer.Start();
  filesystem->AsyncWrite(szPathedFileName, m_pBuffer->Base(),
                         m_pBuffer->TellPut(), false);
  timer.End();
  if (bDbg)
...
void CReplayScreenshotTaker::CreateRenderTarget(
    IMaterialSystem *pMaterialSystem) {
  m_pScreenshotTarget = pMaterialSystem->CreateNamedRenderTargetTextureEx2(
      "rt_ReplayScreenshot", 0, 0, RT_SIZE_REPLAY_SCREENSHOT,
      IMAGE_FORMAT_RGB888, MATERIAL_RT_DEPTH_SEPARATE);
  m_pScreenshotTarget->AddRef(); // we will leak this ref, but only at shutdown
                                 // of the app, which will be cleaned up then
}
