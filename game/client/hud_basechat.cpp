...
ConVar cl_chatfilters("cl_chatfilters", "63", FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
                      "Stores the chat filter settings ");
ConVar cl_chatfilter_version("cl_chatfilter_version", "0",
                             FCVAR_CLIENTDLL | FCVAR_ARCHIVE | FCVAR_HIDDEN,
...
// translatable
//-----------------------------------------------------------------------------
wchar_t *ReadLocalizedString(bf_read &msg,
                             OUT_Z_BYTECAP(outSizeInBytes) wchar_t *pOut,
                             int outSizeInBytes, bool bStripNewline,
                             OUT_Z_CAP(originalSize) char *originalString,
                             int originalSize) {
  char szString[2048];
  szString[0] = 0;
  ...
  for (wchar_t *test = pOut; test && *test; ++test) {
    if (*test && (*test < COLOR_MAX)) {
      if (*test == COLOR_HEXCODE || *test == COLOR_HEXCODE_ALPHA) {
        // mark the next seven or nine characters. one for the control character
        // and six or eight for the code itself.
...
//-----------------------------------------------------------------------------
#ifndef _XBOX
CBaseHudChatInputLine::CBaseHudChatInputLine(vgui::Panel *parent,
...
void CBaseHudChatInputLine::GetMessageText(OUT_Z_BYTECAP(buffersizebytes)
                                               wchar_t *buffer,
                                           int buffersizebytes) {
...
CHudChatFilterPanel::CHudChatFilterPanel(vgui::Panel *pParent,
  ...
  new CHudChatFilterCheckButton(this, "achivement_button", "Sky is blue?",
                                CHAT_FILTER_ACHIEVEMENT);
...
CHudChatHistory::CHudChatHistory(vgui::Panel *pParent, const char *panelName)
  ...
  if (IsAndroid() && !CommandLine()->FindParm("-nocustomchat"))
    scheme = vgui::scheme()->LoadSchemeFromFileEx(
        enginevgui->GetPanel(PANEL_CLIENTDLL), "resource/customchatscheme.res",
        "ChatScheme");
  else
    scheme = vgui::scheme()->LoadSchemeFromFileEx(
    ...
    if (m_pFilterPanel) {
      ...
      if (IsAndroid() && !CommandLine()->FindParm("-nocustomchat"))
        scheme = vgui::scheme()->LoadSchemeFromFileEx(
            ...
            "resource/customchatscheme.res", "ChatScheme");
      else
        scheme = vgui::scheme()->LoadSchemeFromFileEx(
            enginevgui->GetPanel(PANEL_CLIENTDLL), "resource/ChatScheme.res",
            "ChatScheme");
...
void CBaseHudChat::MsgFunc_TextMsg(bf_read &msg) {
    ...
    if (pBuf) {
      ...
      if (i) {
        StripEndNewlineFromString(
            tmpStr); // these strings are meant for subsitution into the main
                     // strings, so cull the automatic end newlines
      }
  ...
  case HUD_PRINTCENTER:
    g_pVGuiLocalize->ConstructString(outputBuf, sizeof(outputBuf), szBuf[0], 4,
                                     szBuf[1], szBuf[2], szBuf[3], szBuf[4]);
...
void CBaseHudChat::OnTick(void) {
  ...
  if (line) {
    ...
    m_iFontHeight = vgui::scheme()->GetProportionalScaledValue(
...
void CBaseHudChat::StartMessageMode(int iMessageModeType) {
  ...
  const wchar_t *pszPrompt = (m_nMessageMode == MM_SAY)
                                 ? g_pVGuiLocalize->Find("#chat_say")
                                 : g_pVGuiLocalize->Find("#chat_say_team");
  if (pszPrompt) {
    ...
    while (txt && *txt) {
      ...
      case COLOR_HEXCODE_ALPHA: {
        ...
        if (range.end > range.start) {
          ...
          // Not enough characters remaining for a hex code. Skip the rest of
          // the string.
          bDone = true;
        }
      } break;
