...
//			h -
//-----------------------------------------------------------------------------
CBasePanel::CBasePanel(vgui::Panel *pParent, const char *panelName, int x,
                       int y, int w, int h)
...
void CBasePanel::PaintBackground(void) {
  ...
  if (m_nBackgroundMaterial == -1) {
    ...
    vgui::surface()->DrawSetTextureFile(m_nBackgroundMaterial, m_szBgTexture,
                                        true, true);
  }
  ...
  if (m_bTiled && (!m_nTextureSize[0])) {
    vgui::surface()->DrawGetTextureSize(m_nBackgroundMaterial,
                                        m_nTextureSize[0], m_nTextureSize[1]);
  }
  ...
  if (m_bTiled && m_nTextureSize[0] && m_nTextureSize[1]) {
    ...
    while (y < tall) {
      vgui::surface()->DrawTexturedRect(0, 0, m_nTextureSize[0],
                                        m_nTextureSize[1]);
...
void CBasePanel::SetReflectMouse(bool reflect) { m_bReflectMouse = true; }
...
void CBasePanel::OnCursorMoved(int x, int y) {
  ...
  vgui::ivgui()->PostMessage(GetParent()->GetVPanel(),
...
// Purpose:
//-----------------------------------------------------------------------------
CHudLabel::CHudLabel(vgui::Panel *parent, const char *panelName,
                     const char *text)
