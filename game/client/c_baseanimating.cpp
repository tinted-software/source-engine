...
RecvPropInt(RECVINFO(m_nSequence), 0, RecvProxy_Sequence),
    ...
    RecvPropFloat(RECVINFO_NAME(
        m_flModelScale, m_flModelWidthScale)), // for demo compatibility only
...
void C_BaseAnimating::RemoveBaseAnimatingInterpolatedVars() {
  ...
  //  but m_flCycle is not touched, so it's only set during prediction (which
  //  occurs on tick boundaries) and so needs to continue to be interpolated for
  //  smooth rendering of the lower body of the local player in third person,
  //  etc.
  if (!GetPredictable())
#endif
  {
...
void C_BaseAnimating::LockStudioHdr() {
  ...
  m_pStudioHdr =
      pNewWrapper; // must be last to ensure virtual model correctly set up
}
...
#endif
    {
      ...
      if (pStudioHdr->GetVirtualModel()) {
        MDLHandle_t hVirtualModel =
            VoidPtrToMDLHandle(m_pStudioHdr->GetRenderHdr()->VirtualModel());
...
CStudioHdr *C_BaseAnimating::OnNewModel() {
  ...
  if (modelinfo->GetModel(nNewIndex) != GetModel()) {
    // XXX what's authoritative? the model pointer or the model index? what a
    // mess.
  ...
  if (IsDynamicModelIndex(nNewIndex) &&
      modelinfo->IsDynamicModelLoading(nNewIndex)) {
    m_bDynamicModelPending = true;
    modelinfo->RegisterModelLoadCallback(nNewIndex, this);
  }

  if (IsDynamicModelLoading()) {
    // Called while dynamic model still loading -> new model, clear deferred
    // state
    m_bResetSequenceInfoOnLoad = false;
    return NULL;
  }
  ...
  // multiplayer game player entities), so we may need to clear it out here.
  // Force a SetSequence call no matter what, though.
    ...
    if (!m_bNoModelParticles) {
      ...
      if (pkvAllParticleEffects) {
        ...
        for (KeyValues *pSingleEffect = pkvAllParticleEffects->GetFirstSubKey();
          ...
          if (iAttachType == -1) {
            Warning("Invalid attach type specified for particle effect in "
                    "model '%s' keyvalues section. Trying to spawn effect '%s' "
                    "with attach type of '%s'\n",
          ...
          if (pszAttachment[0] != '0' && iAttachment == 0) {
            ...
            if (iAttachment <= 0) {
              Warning("Failed to find attachment point specified for particle "
                      "effect in model '%s' keyvalues section. Trying to spawn "
                      "effect '%s' on attachment named '%s'\n",
          ...
          if (!V_strcmp("sentry_rocket", pszParticleEffect)) {
            ...
            CBaseObject *pSentry =
                dynamic_cast<CBaseObject *>(GetOwnerEntity());
            if (pSentry) {
              CALL_ATTRIB_HOOK_INT_ON_OTHER(pSentry->GetOwner(),
                                            iHalloweenSpell,
...
void C_BaseAnimating::BuildTransformations(CStudioHdr *hdr, Vector *pos,
  ...
  if (m_pRagdoll) {
    ...
    // If we're playing back a demo, override the ragdoll bones with cached
    // version if available - otherwise, simulate.
    if ((!engine->IsPlayingDemo() && !engine->IsPlayingTimeDemo()) ||
        !CReplayRagdollCache::Instance().IsInitialized() ||
        !CReplayRagdollCache::Instance().GetFrame(
            this, engine->GetDemoPlaybackTick(), boneSimulated,
            &m_BoneAccessor))
#endif
    {
    ...
    // skip bones that the IK has already setup
      ...
      if ((hdr->boneFlags(i) & BONE_ALWAYS_PROCEDURAL) &&
        ...
        m_pJiggleBones->BuildJiggleTransformations(
            i, gpGlobals->realtime, jiggleInfo, goalMX, GetBoneForWrite(i));
...
void C_BaseAnimating::ApplyBoneMatrixTransform(matrix3x4_t &transform) {
  ...
  if (IsModelScaled()) {
    // The bone transform is in worldspace, so to scale this, we need to
    // translate it back
...
//-----------------------------------------------------------------------------
void C_BaseAnimating::MaintainSequenceTransitions(IBoneSetup &boneSetup,
                                                  float flCycle, Vector pos[],
                                                  Quaternion q[]) {
  ...
  m_SequenceTransitioner.CheckForSequenceChange(
      boneSetup.GetStudioHdr(), GetSequence(),
  ...
  m_SequenceTransitioner.UpdateCurrent(boneSetup.GetStudioHdr(), GetSequence(),
                                       flCycle, m_flPlaybackRate,
                                       gpGlobals->curtime);
  ...
  for (int i = m_SequenceTransitioner.m_animationQueue.Count() - 2; i >= 0;
    ...
    flCycle =
        blend->m_flCycle +
        dt * blend->m_flPlaybackRate *
            GetSequenceCycleRate(boneSetup.GetStudioHdr(), blend->m_nSequence);
    flCycle = ClampCycle(flCycle, IsSequenceLooping(boneSetup.GetStudioHdr(),
                                                    blend->m_nSequence));
    ...
    if (/*Q_stristr( hdr->pszName(), r_sequence_debug.GetString()) != NULL || */
      ...
      DevMsgRT(
          "%8.4f : %30s : %5.3f : %4.2f  +\n", gpGlobals->curtime,
          boneSetup.GetStudioHdr()->pSeqdesc(blend->m_nSequence).pszLabel(),
          flCycle, (float)blend->m_flWeight);
    }
#endif

    boneSetup.AccumulatePose(pos, q, blend->m_nSequence, flCycle,
                             blend->m_flWeight, gpGlobals->curtime, m_pIk);
  }
}
...
void C_BaseAnimating::AccumulateLayers(IBoneSetup &boneSetup, Vector pos[],
                                       Quaternion q[], float currentTime) {
  // Nothing here
}

void C_BaseAnimating::ChildLayerBlend(Vector pos[], Quaternion q[],
  ...
  for (C_BaseEntity *pChild = FirstMoveChild(); pChild;
    ...
    if (pChildAnimating) {
      ...
      if (pChildHdr && pChild->IsEffectActive(EF_BONEMERGE) &&
          pChildHdr->SequencesAvailable() &&
        ...
        pChildAnimating->m_pBoneMergeCache->CopyParentToChild(pos, q, childPos,
                                                              childQ, boneMask);

        // FIXME: needs some kind of sequence
        // merge over whatever bones the childs sequence modifies
        childBoneSetup.AccumulatePose(childPos, childQ, 0, GetCycle(), 1.0,
                                      currentTime, NULL);

        // copy the result back into the parents bones
        pChildAnimating->m_pBoneMergeCache->CopyChildToParent(childPos, childQ,
                                                              pos, q, boneMask);
...
void C_BaseAnimating::StandardBlendingRules(CStudioHdr *hdr, Vector pos[],
  ...
  boneSetup.AccumulatePose(pos, q, GetSequence(), fCycle, 1.0, currentTime,
                           m_pIk);
  ...
  for (int i = 0; i < targetCount; i++) {
    ...
    case IK_GROUND: {
      ...
      if (trace.startsolid) {
        ...
        // debugoverlay->AddLineOverlay( ray.m_Start, ray.m_Start + ray.m_Delta,
        // 255, 0, 0, 0, 0 );
      ...
      pTarget->est.pos, Vector( -r, -r, -1 ), Vector( r, r, 1), QAngle( 0, 0, 0
      ), 255, 0, 0, 0, 0 );
      */
...
ConVar
    cl_warn_thread_contested_bone_setup("cl_warn_thread_contested_bone_setup",
                                        "0");
#endif
...
void C_BaseAnimating::ThreadedBoneSetup() {
  ...
  if (g_bDoThreadedBoneSetup) {
    ...
    if (nCount > 1) {
      ...
      ParallelProcess("C_BaseAnimating::ThreadedBoneSetup",
                      g_PreviousBoneSetups.Base(), nCount,
...
bool C_BaseAnimating::SetupBones(matrix3x4_t *pBoneToWorldOut, int nMaxBones,
  ...
  // actually accessed as part of the setup process, but since I'm not clear on
  // the purpose of this dev warning, I'm including this comment block.
  //=============================================================================
  ...
  if ((m_BoneAccessor.GetReadableBones() & boneMask) != boneMask) {
    ...
    if (hdr->flags() & STUDIOHDR_FLAGS_STATIC_PROP) {
      ...
      // NOTE: For model scaling, we need to opt out of IK because it will mark
      // the bones as already being calculated
      if (!IsModelScaled()) {
...
void C_BaseAnimating::PushAllowBoneAccess(bool bAllowForNormalModels,
  ...
  Assert(
      g_BoneAccessStack.Count() <
      32); // Most likely we are leaking "PushAllowBoneAccess" calls if
           // PopBoneAccess is never called. Consider using AutoAllowBoneAccess.
...
void C_BaseAnimating::DoAnimationEvents(CStudioHdr *pStudioHdr) {
...
#ifdef DEBUG
  bool watch = dbganimmodel.GetString()[0] &&
               V_stristr(pStudioHdr->pszName(), dbganimmodel.GetString());
#else
  ...
  if (nSeqNum >= nStudioNumSeq) {
    // This can happen e.g. while reloading Heavy's shotgun, switch to the
    // minigun.
    ...
    for (int i = 0; i < (int)seqdesc.numevents; i++) {
      ...
      if (pevent[i].type & AE_TYPE_NEWEVENTSYSTEM) {
        if (!(pevent[i].type & AE_TYPE_CLIENT))
          continue;
      } else if (pevent[i].event < 5000) // Adrian - Support the old event
                                         // system
        continue;
...
void C_BaseAnimating::FireEvent(const Vector &origin, const QAngle &angles,
  ...
  case AE_CL_CREATE_PARTICLE_EFFECT: {
    ...
    if (token) {
      ...
      if (iAttachType == -1) {
        Warning("Invalid attach type specified for particle effect anim event. "
    ...
    if (token) {
      ...
      if (token[0] != '0' && iAttachment == 0) {
        ...
        if (iAttachment <= 0) {
          Warning("Failed to find attachment point specified for particle "
                  "effect anim event. Trying to spawn effect '%s' on "
                  "attachment named '%s'\n",
                  szParticleEffect, token);
          return;
        }
    ...
    // Spawn the particle effect
    ParticleProp()->Create(szParticleEffect, (ParticleAttachment_t)iAttachType,
                           iAttachment);
  } break;
...
bool C_BaseAnimating::Interpolate(float flCurrentTime) {
  ...
  int bNoMoreChanges;
  int retVal = BaseInterpolatePart1(flCurrentTime, oldOrigin, oldAngles, oldVel,
                                    bNoMoreChanges);
...
void C_BaseAnimating::PostDataUpdate(DataUpdateType_t updateType) {
  ...
  bool bAnimationChanged = m_flOldCycle != GetCycle() ||
...
#if defined(REPLAY_ENABLED)
  // If Replay is enabled on server, add an entry to the ragdoll recorder for
  // this entity
  ConVar *pReplayEnable = (ConVar *)cvar->FindVar("replay_enable");
  if (m_pRagdoll && pReplayEnable && pReplayEnable->GetInt() &&
      !engine->IsPlayingDemo() && !engine->IsPlayingTimeDemo()) {
    CReplayRagdollRecorder &RagdollRecorder =
        CReplayRagdollRecorder::Instance();
...
void C_BaseAnimating::ClientSideAnimationChanged() {
  ...
  m_SequenceTransitioner.CheckForSequenceChange(GetModelPtr(), GetSequence(),
...
void C_BaseAnimating::UpdateClientSideAnimation() {
  ...
  if (m_bClientSideAnimation) {
    ...
    if (GetSequence() != -1) {
      ...
      // move frame forward
      FrameAdvance(0.0f); // 0 means to use the time we last advanced instead of
                          // a constant
    }
  } else {
    Assert(m_ClientSideAnimationListHandle ==
...
bool C_BaseAnimating::TestHitboxes(const Ray_t &ray, unsigned int fContentsMask,
  ...
  if (TraceToStudio(physprops, ray, pStudioHdr, set, hitboxbones, fContentsMask,
                    GetRenderOrigin(), GetModelScale(), tr)) {
...
void C_BaseAnimating::StudioFrameAdvance() {
...
#ifdef DEBUG
  bool watch = dbganimmodel.GetString()[0] &&
               V_stristr(hdr->pszName(), dbganimmodel.GetString());
#else
  bool watch = false; // Q_strstr( hdr->name, "rifle" ) ? true : false;
#endif
  ...
  m_flGroundSpeed =
      GetSequenceGroundSpeed(hdr, GetSequence()) * GetModelScale();
...
float C_BaseAnimating::FrameAdvance(float flInterval) {
...
#ifdef DEBUG
  bool bWatch = dbganimmodel.GetString()[0] &&
                V_stristr(hdr->pszName(), dbganimmodel.GetString());
#else
  ...
  if (GetServerIntendedCycle() != -1.0f) {
    ...
    bool adjustOkay =
        serverAdvance > 0.0f; // only want to go forward. backing up looks
                              // really jarring, even when slight
    ...
    SetServerIntendedCycle(-1.0f); // Only use a correction once, it isn't valid
...
void C_BaseAnimating::ResetSequenceInfo(void) {
  ...
  CStudioHdr *pStudioHdr = GetModelPtr();
  m_flGroundSpeed =
      GetSequenceGroundSpeed(pStudioHdr, GetSequence()) * GetModelScale();
  m_bSequenceLoops =
...
void C_BaseAnimating::SetBodygroup(int iGroup, int iValue) {
  // SetBodygroup is not supported on pending dynamic models. Wait for it to
  // load!
  // XXX TODO we could buffer up the group and value if we really needed to.
  // -henryg
  Assert(GetModelPtr());
...
int C_BaseAnimating::GetBodygroup(int iGroup) {
  ...
  return IsDynamicModelLoading()
             ? 0
             : ::GetBodygroup(GetModelPtr(), m_nBody, iGroup);
}

const char *C_BaseAnimating::GetBodygroupName(int iGroup) {
  Assert(IsDynamicModelLoading() || GetModelPtr());
  return IsDynamicModelLoading() ? ""
                                 : ::GetBodygroupName(GetModelPtr(), iGroup);
}

int C_BaseAnimating::FindBodygroupByName(const char *name) {
  Assert(IsDynamicModelLoading() || GetModelPtr());
  return IsDynamicModelLoading() ? -1
                                 : ::FindBodygroupByName(GetModelPtr(), name);
}

int C_BaseAnimating::GetBodygroupCount(int iGroup) {
  Assert(IsDynamicModelLoading() || GetModelPtr());
  return IsDynamicModelLoading() ? 0
                                 : ::GetBodygroupCount(GetModelPtr(), iGroup);
}
...
void C_BaseAnimating::ClearRagdoll() {
  if (m_pRagdoll) {
    ...
    ConVar *pReplayEnable = (ConVar *)cvar->FindVar("replay_enable");
    if (pReplayEnable && pReplayEnable->GetInt() && !engine->IsPlayingDemo() &&
...
void C_BaseAnimating::SetModelScale(float scale,
  ...
  if (change_duration > 0.0f) {
    ...
    mvs->m_flModelScaleFinishTime =
        mvs->m_flModelScaleStartTime + change_duration;
  } else {
...
void C_BaseAnimating::UpdateModelScale() {
  ...
  if (gpGlobals->curtime >= mvs->m_flModelScaleFinishTime) {
    ...
    m_flModelScale =
        Lerp(frac, mvs->m_flModelScaleStart, mvs->m_flModelScaleGoal);
  }
...
//VISUALIZE_FOLLOWERS is 			undefined in the server's physics_bone_followers.cpp
//-----------------------------------------------------------------------------
...
const char *
C_BaseAnimating::GetFlexControllerName(LocalFlexController_t iFlexController) {
  CStudioHdr *pstudiohdr = GetModelPtr();
  if (!pstudiohdr)
    return 0;
...
const char *
C_BaseAnimating::GetFlexControllerType(LocalFlexController_t iFlexController) {
  CStudioHdr *pstudiohdr = GetModelPtr();
  if (!pstudiohdr)
    return 0;
...
// Purpose:
//-----------------------------------------------------------------------------
void C_BaseAnimating::AttachEntityToBone(C_BaseAnimating *attachTarget,
  ...
  //	Assert( boneIndexAttached >= 0 );		// We should be
  //attaching to a bone.
...
void C_BaseAnimating::NotifyBoneAttached(C_BaseAnimating *attachTarget) {
  ...
  // Special case: if we just attached to the local player and he is hidden,
  // hide us as well.
