...
void FormatViewModelAttachment(Vector &vOrigin, bool bInverse) {
  ...
  // of the coefficients of the projection matrices (tan (fov/2) is that
  // coefficient)
  ...
  if (pWeapon) {
    ...
    if (haptics)
      haptics->ProcessHapticEvent(4, "Weapons", pWeapon->GetName(),
                                  "AnimationEvents", VarArgs("%i", event));
...
void C_BaseViewModel::ApplyBoneMatrixTransform(matrix3x4_t &transform) {
  if (ShouldFlipViewModel()) {
    ...
    // We could get MATERIAL_VIEW here, but this is called sometimes before the
    // renderer has set that matrix. Luckily, this is called AFTER the
...
void C_BaseViewModel::UpdateAnimationParity(void) {
  ...
  // only the server changes the animations, so a parity mismatch tells us if we
