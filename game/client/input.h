...
private:
  ...
  void ApplyMouse(QAngle &viewangles, CUserCmd *cmd, float mouse_x,
                  float mouse_y);
...
private:
  ...
  enum {
    ...
    MOUSE_ACCEL_THRESHHOLD2, // if mouse moves > this many mickey's double it a
                             // second time
    MOUSE_SPEED_FACTOR, // 0 = disabled, 1 = threshold 1 enabled, 2 = threshold
                        // 2 enabled
  ...
  // Used to support hotplugging by reinitializing the advanced joystick system
  // when we toggle between some/none joysticks.
