...
public:
  ...
  template <class T> CSmartPtr<T> InitSingleton(CSmartPtr<T> pEmitter) {
    if (!pEmitter) {
      Error("InitSingleton: pEmitter is NULL");
    }

    pEmitter->GetBinding().SetDrawThruLeafSystem(
        false); // Draw in DrawSingletons instead.
  ...
  virtual void LevelInitPreEntity() {
    g_pSimpleSingleton[0] =
        InitSingleton(CSimpleEmitter::Create("Simple Particle Singleton"));
    g_pSimpleSingleton[1] = InitSingleton(
        CSimpleEmitter::Create("Simple Particle Singleton [sky]"));

    g_pEmberSingleton[0] =
        InitSingleton(CEmberEffect::Create("Ember Particle Singleton"));
    g_pEmberSingleton[1] =
        InitSingleton(CEmberEffect::Create("Ember Particle Singleton [sky]"));

    g_pFireSmokeSingleton[0] = InitSingleton(
        CFireSmokeEffect::Create("Fire Smoke Particle Singleton"));
    g_pFireSmokeSingleton[1] = InitSingleton(
        CFireSmokeEffect::Create("Fire Smoke Particle Singleton [sky]"));

    g_pFireSingleton[0] =
        InitSingleton(CFireParticle::Create("Fire Particle Singleton"));
    g_pFireSingleton[1] =
        InitSingleton(CFireParticle::Create("Fire Particle Singleton [sky]"));
  }
...
template <class T> inline void CopyParticle(const T *pSrc, T *pDest) {
  if (pDest) {
...
void AddSimpleParticle(const SimpleParticle *pParticle,
                       PMaterialHandle hMaterial, bool bInSkybox) {
  if (g_pSimpleSingleton[bInSkybox].IsValid()) {
    SimpleParticle *pNew = g_pSimpleSingleton[bInSkybox]->AddSimpleParticle(
        hMaterial, pParticle->m_Pos);
...
void AddEmberParticle(const SimpleParticle *pParticle,
  ...
  if (g_pEmberSingleton[bInSkybox].IsValid()) {
    SimpleParticle *pNew = g_pEmberSingleton[bInSkybox]->AddSimpleParticle(
        hMaterial, pParticle->m_Pos);
    CopyParticle(pParticle, pNew);
  }
}
...
void AddFireSmokeParticle(const SimpleParticle *pParticle,
  ...
  if (g_pFireSmokeSingleton[bInSkybox].IsValid()) {
    SimpleParticle *pNew = g_pFireSmokeSingleton[bInSkybox]->AddSimpleParticle(
        hMaterial, pParticle->m_Pos);
    CopyParticle(pParticle, pNew);
  }
}
...
void AddFireParticle(const SimpleParticle *pParticle, PMaterialHandle hMaterial,
  ...
  if (g_pFireSingleton[bInSkybox].IsValid()) {
    SimpleParticle *pNew = g_pFireSingleton[bInSkybox]->AddSimpleParticle(
        hMaterial, pParticle->m_Pos);
    CopyParticle(pParticle, pNew);
  }
}
