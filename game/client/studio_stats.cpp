...
static ConVar
    r_studio_stats_lock("r_studio_stats_lock", "0", FCVAR_CHEAT,
...
void StudioStats_FindClosestEntity(
  ...
  partition->EnumerateElementsAlongRay(PARTITION_ALL_CLIENT_EDICTS, shotRay,
                                       false, &studioEnum);

  if (g_pStudioStatsEntity) {
    C_BaseEntity *pEntity =
        g_pStudioStatsEntity->GetIClientUnknown()->GetBaseEntity();
    ...
    if (pEntity && (pEntity != C_BasePlayer::GetLocalPlayer())) {
      ...
      case RSTUDIOSTATMODE_HELDWEAPON: {
        ...
        if (pTargetPlayer && pTargetPlayer->GetActiveWeapon()) {
          g_pStudioStatsEntity =
              pTargetPlayer->GetActiveWeapon()->GetClientRenderable();
        }
      } break;
      }
    }
