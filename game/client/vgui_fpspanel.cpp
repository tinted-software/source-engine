...
void CFPSPanel::Paint() {
  ...
  if (nShowPosMode > 0) {
    ...
    if (nShowPosMode > 1)
      g_pMatSystemSurface->DrawColoredText(
          m_hFont, x, 2 + i * (vgui::surface()->GetFontTall(m_hFont) + 2), 255,
          255, 255, 255, "vel:  %.2f  %.2f  %.2f", vel.x, vel.y, vel.z);
    else
      g_pMatSystemSurface->DrawColoredText(
          m_hFont, x, 2 + i * (vgui::surface()->GetFontTall(m_hFont) + 2), 255,
  ...
  if (cl_showbattery.GetInt() > 0) {
    if (steamapicontext && steamapicontext->SteamUtils() &&
        (m_lastBatteryPercent == -1.0f ||
         (gpGlobals->realtime - m_lastBatteryPercent) > 10.0f)) {
      m_BatteryPercent =
          steamapicontext->SteamUtils()->GetCurrentBatteryPower();
    ...
    if (m_BatteryPercent > 0) {
      if (m_BatteryPercent == 255) {
        g_pMatSystemSurface->DrawColoredText(
            m_hFont, x, 2 + i * (vgui::surface()->GetFontTall(m_hFont) + 2),
...
static ConVar cl_showblocking("cl_showblocking", "0", 0,
                              ...
                              ShowBlockingChanged);
static ConVar
    cl_blocking_recentsize("cl_blocking_recentsize", "40", 0,
...
static ConVar
    cl_blocking_msec("cl_blocking_msec", "100", 0,
