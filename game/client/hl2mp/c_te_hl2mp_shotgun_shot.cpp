...
public:
  virtual bool ShouldHitEntity(IHandleEntity *pServerEntity, int contentsMask) {
    ...
    if (pEntity && ((dynamic_cast<C_BaseViewModel *>(pEntity) != NULL) ||
  ...
  if (pEnt) {
    ...
    if (pPlayer && pPlayer->GetActiveWeapon()) {
      C_BaseCombatWeapon *pWpn =
          dynamic_cast<C_BaseCombatWeapon *>(pPlayer->GetActiveWeapon());
      ...
      if (pWpn) {
        ...
        for (int iShot = 0; iShot < m_iShots; iShot++) {
          ...
          Vector vecDir = Manipulator.ApplySpread(
          ...
          if (m_iShots > 1 && iShot % 2) {
            ...
            UTIL_TraceHull(m_vecOrigin, vecEnd, Vector(-3, -3, -3),
                           Vector(3, 3, 3), MASK_SHOT, &traceFilter, &tr);
          } else {
...
IMPLEMENT_CLIENTCLASS_EVENT(C_TEHL2MPFireBullets, DT_TEHL2MPFireBullets,
                            CTEHL2MPFireBullets);

BEGIN_RECV_TABLE_NOBASE(C_TEHL2MPFireBullets, DT_TEHL2MPFireBullets)
RecvPropVector(RECVINFO(m_vecOrigin)), RecvPropVector(RECVINFO(m_vecDir)),
    RecvPropInt(RECVINFO(m_iAmmoID)), RecvPropInt(RECVINFO(m_iSeed)),
    RecvPropInt(RECVINFO(m_iShots)), RecvPropInt(RECVINFO(m_iPlayer)),
