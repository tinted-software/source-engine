...
struct Particle {
  ...
  // Which sub texture this particle uses (so we can get at the tcoord mins and
  // maxs).
...
public:
  ...
  // coordinates, so this is the material of the first subtexture that is bound.
  //
...
public:
  ...
  float
      m_tCoordMins[2]; // bbox in texel space that this particle material uses.
...
public:
  ...
  // You can change this matrix if you want your particles relative to something
  ...
  // touch the IParticleEffect or its associated CParticleEffectBinding anymore.
  ...
  // NOTE: The particle effect's GetNumActiveParticles is updated BEFORE this is
  // called
  //       so if GetNumActiveParticles returns 0, then you know this is the last
  //       particle in the system being removed.
...
public:
  ...
  // effect's bounding box. Normally, the bounding box is managed automatically,
  // but in certain cases it is more efficient to set it manually.
  //
  ...
  // This tells the particle manager that your particles are transformed by the
  // specified matrix. That way, it can transform the bbox defined by
  // Particle::m_Pos into world space correctly.
  //
  // It also sets up the matrix returned by CParticleMgr::GetModelView() to
  // include this matrix, so you can do TransformParticle with it like any other
  // particle system.
  const matrix3x4_t &GetLocalSpaceTransform() const;
  void SetLocalSpaceTransform(const matrix3x4_t &transform);

  // This expands the bbox to contain the specified point. Returns true if bbox
  // changed
  ...
  // if the particle system isn't visible. Tempents and fast effects can always
  // use this if they want since they want to simulate their particles until
  // they go away. This flag is ON by default.
  ...
  // When the effect is in camera space mode, then the transforms are setup such
  // that the particle vertices are specified in camera space (in CParticleDraw)
  // rather than world space.
  //
  // This makes it faster to specify the particles - you only have to transform
  // the center by CParticleMgr::GetModelView then add to X and Y to build the
  // quad.
  //
  // Effects that want to specify verts (in CParticleDraw) in world space should
  // set this to false and ignore CParticleMgr::GetModelView.
  //
  ...
  // returned by CParticleMgr::GetModelView(). Usually, you'll want this, so you
  // can just say TransformParticle( pMgr->GetModelView(), vPos ), but you may
  // want to manually apply your local transform before saying
  // TransformParticle.
  //
  ...
  // If this is true, then the bbox is calculated from particle positions. This
  // works fine if you always simulate (SetAlwaysSimulateFlag) so the system can
  // become visible if it moves into the PVS. If you don't use this, then you
  // should call SetBBox at least once to tell the particle manager where your
  // entity is.
...
private:
  ...
  void StartDrawMaterialParticles(CEffectMaterial *pMaterial, float flTimeDelta,
  ...
  void GrowBBoxFromParticlePositions(CEffectMaterial *pMaterial, bool &bboxSet,
  ...
  // Set on creation and cleared after the first PostRender (whether or not the
  // system was rendered).
...
private:
  enum {
    ...
    FLAGS_REMOVALINPROGRESS = (1 << 1), // Set while the effect is being removed
                                        // to prevent infinite recursion.
    FLAGS_NEEDS_BBOX_UPDATE =
        (1 << 2), // This is set until the effect's bbox has been updated once.
    ...
    FLAGS_DRAW_THRU_LEAF_SYSTEM =
        (1 << 8), // This is the default - do the effect's visibility through
                  // the leaf system.
    ...
    FLAGS_FIRST_FRAME =
        (1 << 11), // Cleared after the first frame that this system exists (so
                   // it can simulate after rendering once).
  ...
  bool m_bLocalSpaceTransformIdentity; // If this is true, then
                                       // m_LocalSpaceTransform is assumed to be
                                       // identity.
...
public:
  ...
  // Note: it's much safer to call CParticleEffectBinding::SetRemoveFlag instead
  // of
  ...
  // (usually CParticleEffect-derived effects that can linger with noone holding
  // onto them).
...
private:
  ...
  // Frame code, used to prevent CParticleEffects from simulating multiple times
  // per frame. Their DrawModel can be called multiple times per frame because
  // of water reflections, but we only want to simulate the particles once.
  unsigned short m_FrameCode;
  ...
  CParticleSubTexture m_DefaultInvalidSubTexture; // Used when they specify an
...
inline const matrix3x4_t &
CParticleEffectBinding::GetLocalSpaceTransform() const {
...
struct StandardParticle_t : public Particle {
  ...
  // How this is used is up to the effect's discretion. Some use it for how long
  // it has been alive and others use it to count down until the particle
  // disappears.
  float m_Lifetime;

  unsigned char m_EffectData; // Data specific to the IParticleEffect. This can
                              // be used to distinguish between different types
                              // of particles the effect is simulating.
...
inline void TransformParticle(const VMatrix &vMat, const Vector &vIn,
  ...
  vOut.x = vMat.m[0][0] * vIn.x + vMat.m[0][1] * vIn.y + vMat.m[0][2] * vIn.z +
           vMat.m[0][3];
  vOut.y = vMat.m[1][0] * vIn.x + vMat.m[1][1] * vIn.y + vMat.m[1][2] * vIn.z +
           vMat.m[1][3];
  vOut.z = vMat.m[2][0] * vIn.x + vMat.m[2][1] * vIn.y + vMat.m[2][2] * vIn.z +
           vMat.m[2][3];
}
