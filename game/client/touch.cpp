...
ConVar touch_forwardzone("touch_forwardzone", "0.06", FCVAR_ARCHIVE,
                         "forward touch zone");
ConVar touch_sidezone("touch_sidezone", "0.06", FCVAR_ARCHIVE,
                      "side touch zone");
ConVar touch_pitch("touch_pitch", "90", FCVAR_ARCHIVE,
                   "touch pitch sensitivity");
ConVar touch_yaw("touch_yaw", "120", FCVAR_ARCHIVE, "touch yaw sensitivity");
ConVar touch_config_file("touch_config_file", "touch.cfg", FCVAR_ARCHIVE,
                         "current touch profile file");
ConVar touch_grid_count("touch_grid_count", "50", FCVAR_ARCHIVE,
                        "touch grid count");
ConVar touch_grid_enable("touch_grid_enable", "1", FCVAR_ARCHIVE,
                         "enable touch grid");
ConVar touch_precise_amount("touch_precise_amount", "0.5", FCVAR_ARCHIVE,
...
void CTouchPanel::Paint() { gTouch.Frame(); }
...
CON_COMMAND(touch_addbutton, "add native touch button") {
  ...
  if (argc >= 12) {
    ...
    color = rgba_t(Q_atoi(args[8]), Q_atoi(args[9]), Q_atoi(args[10]),
                   Q_atoi(args[11]));
    gTouch.AddButton(args[1], args[2], args[3], Q_atof(args[4]),
                     Q_atof(args[5]), Q_atof(args[6]), Q_atof(args[7]), color,
                     round_aspect, aspect, flags);
  ...
  Msg("Usage: touch_addbutton <name> <texture> <command> [<x1> <y1> <x2> <y2> "
      "[ r g b a ] ]\n");
}
...
CON_COMMAND(touch_setcolor, "change button color") {
  if (args.ArgC() >= 6) {
    rgba_t color(Q_atoi(args[2]), Q_atoi(args[3]), Q_atoi(args[4]),
                 Q_atoi(args[5]));
...
CON_COMMAND(touch_list, "list buttons") { gTouch.ListButtons(); }

CON_COMMAND(touch_removeall, "remove all buttons") { gTouch.RemoveButtons(); }

CON_COMMAND(touch_writeconfig, "save current config") { gTouch.WriteConfig(); }
...
CON_COMMAND(touch_setgridcolor, "change grid color") {
  if (args.ArgC() >= 5)
    gTouch.gridcolor = rgba_t(Q_atoi(args[1]), Q_atoi(args[2]), Q_atoi(args[3]),
                              Q_atoi(args[4]));
  else
...
CON_COMMAND( touch_toggleselection, "toggle visibility on selected button in
editor" )
{

}*/

void CTouchControls::GetTouchAccumulators(float *side, float *forward,
...
void CTouchControls::GetTouchDelta(float yaw, float pitch, float *dx,
                                   float *dy) {
  // Apply filtering?
...
void CTouchControls::ResetToDefaults() {
  ...
  if (!filesystem->FileExists(buf)) {
    ...
    AddButton("use", "vgui/touch/use", "+use", 0.880000, 0.213333, 1.000000,
              0.426667, color);
    AddButton("jump", "vgui/touch/jump", "+jump", 0.880000, 0.462222, 1.000000,
              0.675556, color);
    AddButton("attack", "vgui/touch/shoot", "+attack", 0.760000, 0.583333,
              0.880000, 0.796667, color);
    AddButton("attack2", "vgui/touch/shoot_alt", "+attack2", 0.760000, 0.320000,
              0.880000, 0.533333, color);
    AddButton("duck", "vgui/touch/crouch", "+duck", 0.880000, 0.746667,
              1.000000, 0.960000, color);
    AddButton("tduck", "vgui/touch/tduck", ";+duck", 0.560000, 0.817778,
              0.620000, 0.924444, color);
    AddButton("zoom", "vgui/touch/zoom", "+zoom", 0.680000, 0.00000, 0.760000,
              0.142222, color);
    AddButton("speed", "vgui/touch/speed", "+speed", 0.180000, 0.568889,
              0.280000, 0.746667, color);
    AddButton("loadquick", "vgui/touch/load", "load quick", 0.760000, 0.000000,
              0.840000, 0.142222, color);
    AddButton("savequick", "vgui/touch/save", "save quick", 0.840000, 0.000000,
              0.920000, 0.142222, color);
    AddButton("reload", "vgui/touch/reload", "+reload", 0.000000, 0.320000,
              0.120000, 0.533333, color);
    AddButton("flashlight", "vgui/touch/flash_light_filled", "impulse 100",
              0.920000, 0.000000, 1.000000, 0.142222, color);
    AddButton("invnext", "vgui/touch/next_weap", "invnext", 0.000000, 0.533333,
              0.120000, 0.746667, color);
    AddButton("invprev", "vgui/touch/prev_weap", "invprev", 0.000000, 0.071111,
              0.120000, 0.284444, color);
    AddButton("edit", "vgui/touch/settings", "touch_enableedit", 0.420000,
              0.000000, 0.500000, 0.151486, color);
    AddButton("menu", "vgui/touch/menu", "gameui_activate", 0.000000, 0.00000,
              0.080000, 0.142222, color);
  } else {
...
void CTouchControls::Init() {
  ...
  AddButton("attack2", "vgui/touch/shoot_alt", "+attack2", 0.760000, 0.320000,
            0.880000, 0.533333, color);
  AddButton("duck", "vgui/touch/crouch", "+duck", 0.880000, 0.746667, 1.000000,
            0.960000, color);
  AddButton("tduck", "vgui/touch/tduck", ";+duck", 0.560000, 0.817778, 0.620000,
            0.924444, color);
  AddButton("zoom", "vgui/touch/zoom", "+zoom", 0.680000, 0.00000, 0.760000,
            0.142222, color);
  AddButton("speed", "vgui/touch/speed", "+speed", 0.180000, 0.568889, 0.280000,
            0.746667, color);
  AddButton("loadquick", "vgui/touch/load", "load quick", 0.760000, 0.000000,
            0.840000, 0.142222, color);
  AddButton("savequick", "vgui/touch/save", "save quick", 0.840000, 0.000000,
...
void CTouchControls::CreateAtlasTexture() {
  ...
  stbrp_rect *rects =
      (stbrp_rect *)malloc(textureList.Count() * sizeof(stbrp_rect));
  ...
  for (int i = 0; i < textureList.Count(); i++) {
    ...
    buf.SeekGet(CUtlBuffer::SEEK_HEAD,
                0); // Need to set these explicitly since ->Read goes straight
                    // to memory and skips them.
    buf.SeekPut(CUtlBuffer::SEEK_HEAD, bytesRead);

    t->vtf = CreateVTFTexture();
    if (t->vtf->Unserialize(buf)) {
      if (t->vtf->Format() != IMAGE_FORMAT_RGBA8888 &&
          t->vtf->Format() != IMAGE_FORMAT_BGRA8888) {
        t->textureID = vgui::surface()->CreateNewTextureID();
        vgui::surface()->DrawSetTextureFile(t->textureID, t->szName, true,
                                            false);
        DestroyVTFTexture(t->vtf);
        continue;
      }
      if (t->vtf->Height() != t->vtf->Width() ||
          (t->vtf->Height() & (t->vtf->Height() - 1)) != 0)
  ...
  for (int i = 0; i < textureList.Count(); i++) {
    ...
    for (int row = 0; row < t->height; row++) {
      unsigned char *row_dest = dest +
                                (row + rects[rectCount].y) * atlasHeight * 4 +
                                rects[rectCount].x * 4;
  ...
  vgui::surface()->DrawSetTextureRGBA(touchTextureID, dest, atlasHeight,
                                      atlasHeight, 1, true);
...
void CTouchControls::ListButtons() {
  ...
  for (it = btns.begin(); it != btns.end(); it++) {
    ...
    Msg("%s %s %s %f %f %f %f %d %d %d %d %d\n", b->name, b->texturefile,
        b->command, b->x1, b->y1, b->x2, b->y2, b->color.r, b->color.g,
        b->color.b, b->color.a, b->flags);
  }
}

void CTouchControls::IN_CheckCoords(float *x1, float *y1, float *x2,
                                    float *y2) {
...
void CTouchControls::Frame() {
  ...
  if (touch_enable.GetBool() && touch_draw.GetBool() &&
...
void CTouchControls::Paint() {
  ...
  if (state == state_edit) {
    vgui::surface()->DrawSetColor(gridcolor.r, gridcolor.g, gridcolor.b,
                                  gridcolor.a *
                                      3); // 255, 0, 0, 200 <- default here
    float x, y;
      ...
      if (!(btn->flags & TOUCH_FL_NOEDIT)) {
        if (touch_button_info.GetInt()) {
          g_pMatSystemSurface->DrawColoredText(
              2, btn->x1 * screen_w, btn->y1 * screen_h, 255, 255, 255, 255,
              "N: %s", btn->name); // name
          g_pMatSystemSurface->DrawColoredText(
              2, btn->x1 * screen_w, btn->y1 * screen_h + 10, 255, 255, 255,
              255, "T: %s", btn->texturefile); // texture
          g_pMatSystemSurface->DrawColoredText(
              2, btn->x1 * screen_w, btn->y1 * screen_h + 20, 255, 255, 255,
              255, "C: %s", btn->command); // command
          g_pMatSystemSurface->DrawColoredText(
              2, btn->x1 * screen_w, btn->y1 * screen_h + 30, 255, 255, 255,
              255, "F: %i", btn->flags); // flags
          g_pMatSystemSurface->DrawColoredText(
              2, btn->x1 * screen_w, btn->y1 * screen_h + 40, 255, 255, 255,
              255, "RGBA: %d %d %d %d", btn->color.r, btn->color.g,
              btn->color.b, btn->color.a); // color
        }

        vgui::surface()->DrawSetColor(
            buttonEditClr.r, buttonEditClr.g, buttonEditClr.b,
            buttonEditClr.a); // 255, 0, 0, 50 <- default here
        vgui::surface()->DrawFilledRect(btn->x1 * screen_w, btn->y1 * screen_h,
                                        btn->x2 * screen_w, btn->y2 * screen_h);
      }
    }
  }
    ...
    if (btn->texture != NULL && !(btn->flags & TOUCH_FL_HIDE)) {
      ...
      if (t->textureID) {
        m_pMesh = pRenderContext->GetDynamicMesh(
            true, NULL, NULL,
            g_pMatSystemSurface->DrawGetTextureMaterial(t->textureID));
  ...
  m_pMesh = pRenderContext->GetDynamicMesh(
      true, NULL, NULL,
      g_pMatSystemSurface->DrawGetTextureMaterial(touchTextureID));
...
void CTouchControls::AddButton(const char *name, const char *texturefile,
                               const char *command, float x1, float y1,
                               float x2, float y2, rgba_t color, int round,
...
void CTouchControls::SetTexture(const char *name, const char *file) {
  ...
  if (btn) {
    ...
    //		vgui::surface()->DrawSetTextureFile( btn->textureID, file, true,
    //false);
  }
}
...
void CTouchControls::SetCommand(const char *name, const char *cmd) {
  CTouchButton *btn = FindButton(name);
  if (btn)
    Q_strncpy(btn->command, cmd, sizeof btn->command);
}
...
void CTouchControls::EditEvent(touch_event_t *ev) {
  ...
  if (ev->type == IE_FingerDown) {
    ...
    if (ev->fingerid == move_finger) {
      move_finger = -1;
      IN_CheckCoords(&selection->x1, &selection->y1, &selection->x2,
                     &selection->y2);
      selection = nullptr;
...
void CTouchControls::EnableTouchEdit(bool enable) {
  if (enable) {
    ...
    move_button = NULL;
    configchanged = true;
    AddButton("close_edit", "vgui/touch/back", "touch_disableedit", 0.020000,
              0.800000, 0.100000, 0.977778, rgba_t(255, 255, 255, 255), 0, 1.f,
              TOUCH_FL_NOEDIT);
  } else {
    state = state_none;
    resize_finger = move_finger = look_finger = wheel_finger = -1;
...
void CTouchControls::WriteConfig() {
  ...
  if (f) {
    ...
    /*		filesystem->FPrintf( f, "touch_nonlinear_look
       \"%d\"\n",touch_nonlinear_look.GetBool() ); filesystem->FPrintf( f,
       "touch_pow_factor \"%f\"\n", touch_pow_factor->value );
                    filesystem->FPrintf( f, "touch_pow_mult \"%f\"\n",
       touch_pow_mult->value ); filesystem->FPrintf( f, "touch_exp_mult
    ...
    /*
                    filesystem->FPrintf( f, "\n// global overstroke (width, r,
       g, b, a)\n" ); filesystem->FPrintf( f, "touch_set_stroke %d %d %d %d
       %d\n", touch.swidth, touch.scolor[0], touch.scolor[1], touch.scolor[2],
       touch.scolor[3] ); filesystem->FPrintf( f, "\n// highlight when
       pressed\n" ); filesystem->FPrintf( f, "touch_highlight_r \"%f\"\n",
       touch_highlight_r->value ); filesystem->FPrintf( f, "touch_highlight_g
       \"%f\"\n", touch_highlight_g->value ); filesystem->FPrintf( f,
       ...
       touch_dpad_radius->value ); filesystem->FPrintf( f, "touch_joy_radius
    ...
    */
    ...
    for (it = btns.begin(); it != btns.end(); it++) {
      ...
      float aspect =
          (b->y2 - b->y1) / ((b->x2 - b->x1) / (screen_h / screen_w));

      filesystem->FPrintf(f,
                          "touch_addbutton \"%s\" \"%s\" \"%s\" %f %f %f %f %d "
                          "%d %d %d %d %f\n",
                          b->name, b->texturefile, b->command, b->x1, b->y1,
                          b->x2, b->y2, b->color.r, b->color.g, b->color.b,
                          b->color.a, b->flags, aspect);
    }
