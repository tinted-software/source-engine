...
public:
  float m_flFadeStartDist; // Distance at which it starts fading (when <= this,
                           // alpha=m_flTranslucencyLimit).
...
IMPLEMENT_CLIENTCLASS_DT(C_FuncAreaPortalWindow, DT_FuncAreaPortalWindow,
...
RecvPropFloat(RECVINFO(m_flFadeStartDist)),
    ...
    RecvPropInt(RECVINFO(m_iBackgroundModelIndex)) END_RECV_TABLE()
...
int C_FuncAreaPortalWindow::DrawModel(int flags) {
  ...
  render->DrawBrushModelEx(this, (model_t *)GetModel(), GetAbsOrigin(),
                           GetAbsAngles(), mode);
  ...
  if (m_iBackgroundModelIndex >= 0) {
    render->SetBlend(1);
    model_t *pBackground =
        (model_t *)modelinfo->GetModel(m_iBackgroundModelIndex);
    if (pBackground && modelinfo->GetModelType(pBackground) == mod_brush) {
      render->DrawBrushModelEx(this, pBackground, GetAbsOrigin(),
                               GetAbsAngles(), mode);
    }
  }
...
float C_FuncAreaPortalWindow::GetDistanceBlend() {
  ...
  return RemapValClamped(flDist, m_flFadeStartDist, m_flFadeDist,
                         m_flTranslucencyLimit, 1);
}
