...
RecvProp RecvPropIntWithMinusOneFlag(const char *pVarName, int offset,
...
//-----------------------------------------------------------------------------
// Purpose: Okay, so we have to queue up the actual ehandle to entity lookup for
// the following reason:
//  If a player has an EHandle/CHandle to an object such as a weapon, since the
//  player is in slot 1-31, then if the weapon is created and given to the
//  player in the same frame, then the weapon won't have been created at the
//  time we parse this EHandle index, since the player is ahead of every other
//  entity in the packet (except for the world).
// So instead, we remember which ehandles need to be set and we set them after
// all network data has
//  been received.  Sigh.
// Input  : *pData -
//			*pStruct -
//			*pOut -
...
RecvProp RecvPropEHandle(const char *pVarName, int offset, int sizeofVar,
...
//			objectID -
//-----------------------------------------------------------------------------
static void RecvProxy_Time(const CRecvProxyData *pData, void *pStruct,
                           void *pOut) {
  float t;
  float clock_base;
  float offset;
...
static void RecvProxy_IntToPredictableId(const CRecvProxyData *pData,
                                         void *pStruct, void *pOut) {
...
RecvProp RecvPropPredictableId(const char *pVarName, int offset,
                               int sizeofVar /*=SIZEOF_IGNORE*/) {
  return RecvPropInt(pVarName, offset, sizeofVar, 0,
                     RecvProxy_IntToPredictableId);
}
#endif
