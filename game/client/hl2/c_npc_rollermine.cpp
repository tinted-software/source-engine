...
RecvPropInt(RECVINFO(m_bIsOpen)), RecvPropFloat(RECVINFO(m_flActiveTime)),
    RecvPropInt(RECVINFO(m_bHackedByAlyx)), RecvPropInt(RECVINFO(m_bPowerDown)),
    END_RECV_TABLE()
    ...
    if (gpGlobals->frametime != 0) {
      ...
      if (beamInfo.m_nStartAttachment == beamInfo.m_nEndAttachment) {
        ...
        beamInfo.m_nEndAttachment =
            (beamInfo.m_nStartAttachment + nextStep) % NUM_ATTACHMENTS;
      }
      ...
      IMaterial *pMaterial =
          materials->FindMaterial("effects/rollerglow", NULL, false);

      CMatRenderContextPtr pRenderContext(materials);
      pRenderContext->Bind(pMaterial);
      DrawHalo(pMaterial, GetAbsOrigin(),
               random->RandomFloat(6.0f * scale, 6.5f * scale), color);
      ...
      DrawHalo(pMaterial, attachOrigin,
               random->RandomFloat(1.0f * scale, 1.5f * scale), color);
