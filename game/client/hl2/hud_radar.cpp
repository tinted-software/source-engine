...
#define RADAR_IGNORE_Z                                                         \
  (1 << 6) // always draw this item as if it was at the same Z as the player
...
CHudRadar *GetHudRadar() { return s_Radar; }
...
//---------------------------------------------------------
void CHudRadar::AddRadarContact(const Vector &vecOrigin, int iType,
                                float flTimeToLive) {
  ...
  if (iExistingContact > -1) {
    ...
    m_radarContacts[iExistingContact].m_flTimeToRemove =
        gpGlobals->curtime + flTimeToLive;
    return;
  }

  m_radarContacts[m_iNumRadarContacts].m_vecOrigin = vecOrigin;
  m_radarContacts[m_iNumRadarContacts].m_iType = iType;
  m_radarContacts[m_iNumRadarContacts].m_flTimeToRemove =
      gpGlobals->curtime + flTimeToLive;
  m_iNumRadarContacts++;
}
  ...
  while (bKeepWorking) {
    ...
    for (int i = 0; i < m_iNumRadarContacts; i++) {
      ...
      if (gpGlobals->curtime >= pContact->m_flTimeToRemove) {
        // Time for this guy to go. Easiest thing is just to copy the last
        // element into this element's spot and then decrement the count of
        // entities.
        bKeepWorking = true;
...
void CHudRadar::SetVisible(bool state) {
  ...
  if (g_pMapOverview &&
      g_pMapOverview->GetMode() == CMapOverview::MAP_MODE_RADAR) {
...
void CHudRadar::Paint() {
  if (m_iImageID == -1) {
    ...
    vgui::surface()->DrawSetTextureFile(
        m_textureID_IconLambda, RADAR_CONTACT_LAMBDA_MATERIAL, true, false);

    m_textureID_IconBuster = vgui::surface()->CreateNewTextureID();
    vgui::surface()->DrawSetTextureFile(
        m_textureID_IconBuster, RADAR_CONTACT_BUSTER_MATERIAL, true, false);

    m_textureID_IconStrider = vgui::surface()->CreateNewTextureID();
    vgui::surface()->DrawSetTextureFile(
        m_textureID_IconStrider, RADAR_CONTACT_STRIDER_MATERIAL, true, false);

    m_textureID_IconDog = vgui::surface()->CreateNewTextureID();
    vgui::surface()->DrawSetTextureFile(
        m_textureID_IconDog, RADAR_CONTACT_DOG_MATERIAL, true, false);

    m_textureID_IconBase = vgui::surface()->CreateNewTextureID();
    vgui::surface()->DrawSetTextureFile(
        m_textureID_IconBase, RADAR_CONTACT_BASE_MATERIAL, true, false);

    m_iImageID = vgui::surface()->CreateNewTextureID();
    vgui::surface()->DrawSetTextureFile(m_iImageID, RADAR_PANEL_MATERIAL, true,
                                        false);
  }
  ...
  if (gpGlobals->curtime > m_flTimeStartGhosting) {
    if (m_ghostAlpha < RADAR_MAX_GHOST_ALPHA) {
      ...
      m_flTimeStopGhosting =
          gpGlobals->curtime + RandomFloat(1.0f, 2.0f); // How long to ghost for
    }
    ...
    if (m_ghostAlpha > 0) {
      ...
      m_flTimeStartGhosting =
          gpGlobals->curtime +
          RandomFloat(2.0f, 3.0f); // how long between ghosts
      m_flTimeStopGhosting = FLT_MAX;
    }
  }

  // Now go through the list of radar targets and represent them on the radar
  // screen by drawing their icons on top of the background.
  ...
  for (int i = 0; i < m_iNumRadarContacts; i++) {
    ...
    if (RADAR_USE_ICONS) {
      ...
      DrawIconOnRadar(pContact->m_vecOrigin, pLocalPlayer, pContact->m_iType,
                      RADAR_IGNORE_Z, 255, 255, 255, alpha + flicker);
    } else {
      DrawPositionOnRadar(pContact->m_vecOrigin, pLocalPlayer,
                          pContact->m_iType, RADAR_IGNORE_Z, 255, 255, 255,
                          alpha);
    }
  }
...
//		-etc-
//---------------------------------------------------------
bool CHudRadar::WorldToRadar(const Vector location, const Vector origin,
                             const QAngle angles, float &x, float &y,
  ...
  // This magic /2.15 makes the radar scale seem smaller than the VGUI panel so
  // the icons clamp to the outer ring in the radar graphic, not the very edge
  // of the panel itself.
  ...
  if ((-1 * y_diff) > fRange) {
    ...
    // scale
    float flDist =
        sqrt(((xnew_diff) * (xnew_diff) + (ynew_diff) * (ynew_diff)));
    scale = flDist / fRange;
  }
...
void CHudRadar::DrawPositionOnRadar(Vector vecPos, C_BasePlayer *pLocalPlayer,
                                    int type, int flags, int r, int g, int b,
                                    int a) {
  float x, y, z_delta;
  int iBaseDotSize = 3;
  ...
  WorldToRadar(vecPos, pLocalPlayer->GetAbsOrigin(), viewAngle, x, y, z_delta,
               flScale);
...
void CHudRadar::DrawIconOnRadar(Vector vecPos, C_BasePlayer *pLocalPlayer,
                                int type, int flags, int r, int g, int b,
                                int a) {
  float x, y, z_delta;
  int wide, tall;
  ...
  flScale =
      RemapVal(flScale, 1.0f, 0.0f, RADAR_ICON_MIN_SCALE, RADAR_ICON_MAX_SCALE);
...
void CHudRadar::DrawRadarDot(int x, int y, float z_diff, int iBaseDotSize,
