...
void C_EnvStarfield::OnDataChanged(DataUpdateType_t updateType) {
  if (updateType == DATA_UPDATE_CREATED) {
    ...
    Vector vecCenter = MainViewOrigin() +
                       (MainViewForward() * cl_starfield_distance.GetFloat());
    m_pEmitter->Setup((Vector &)vecCenter, NULL, 0.0, 0, 64, 0, 0,
                      bitsPARTICLE_TRAIL_VELOCITY_DAMPEN |
                          bitsPARTICLE_TRAIL_FADE | bitsPARTICLE_TRAIL_FADE_IN);
...
void C_EnvStarfield::ClientThink(void) {
  ...
  PMaterialHandle hParticleMaterial =
      m_pEmitter->GetPMaterial("effects/spark_noz");
  ...
  for (int i = 0; i < iNumParticles; i++) {
    ...
    Vector vecStart =
        vecViewOrigin + (MainViewForward() * cl_starfield_distance.GetFloat());
    Vector vecEnd = vecViewOrigin +
                    (MainViewRight() * RandomFloat(-flDiameter, flDiameter)) +
    ...
    TrailParticle *pParticle = (TrailParticle *)m_pEmitter->AddParticle(
        sizeof(TrailParticle), hParticleMaterial, vecStart);
    if (pParticle) {
