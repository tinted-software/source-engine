...
public:
  ...
  int DrawModel(int flags) {
    ...
    if (m_bActive) {
      ...
      IMaterial *pMaterial =
          materials->FindMaterial("effects/stunstick", NULL, false);
  ...
  void ClientThink(void) {
    // Update our effects
    if (m_bActive && gpGlobals->frametime != 0.0f &&
...
//			*pOut -
//-----------------------------------------------------------------------------
void RecvProxy_StunActive(const CRecvProxyData *pData, void *pStruct,
                          void *pOut) {
...
IMPLEMENT_CLIENTCLASS_DT(C_WeaponStunStick, DT_WeaponStunStick,
                         CWeaponStunStick)
RecvPropInt(RECVINFO(m_bActive), 0, RecvProxy_StunActive), END_RECV_TABLE()
