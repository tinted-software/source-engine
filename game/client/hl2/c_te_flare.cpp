...
RecvPropFloat(RECVINFO(m_flTimeBurnOut)), RecvPropFloat(RECVINFO(m_flScale)),
    RecvPropInt(RECVINFO(m_bLight)), RecvPropInt(RECVINFO(m_bSmoke)),
...
void C_Flare::RestoreResources(void) {
  if (m_pParticle[0] == NULL) {
    m_pParticle[0] = (SimpleParticle *)AddParticle(
        sizeof(SimpleParticle), GetPMaterial("effects/redflare"),
        GetAbsOrigin());

    if (m_pParticle[0] != NULL) {
      m_pParticle[0]->m_uchColor[0] = m_pParticle[0]->m_uchColor[1] =
  ...
  if (m_pParticle[1] == NULL) {
    m_pParticle[1] = (SimpleParticle *)AddParticle(
        sizeof(SimpleParticle), GetPMaterial("effects/yellowflare_noz"),
        GetAbsOrigin());

    if (m_pParticle[1] != NULL) {
      m_pParticle[1]->m_uchColor[0] = m_pParticle[1]->m_uchColor[1] =
...
void C_Flare::Update(float timeDelta) {
...
#else
  // NOTE!!!  This is bigger by a factor of 1.2 to deal with fixing a bug from
  // HL2.  See dlight_t.h
  ...
  if ((m_flTimeBurnOut != -1.0f) &&
      ((m_flTimeBurnOut - gpGlobals->curtime) <= 10.0f)) {
  ...
  if (m_bLight) {
    ...
    if (m_bPropFlare == false) {
      ...
      if (m_iAttachment != -1) {
        ...
        // Raise the light a little bit away from the flare so it lights it up
        // better.
  ...
  if (m_bSmoke) {
    while (m_teSmokeSpawn.NextEvent(dt)) {
      ...
      SimpleParticle *sParticle = (SimpleParticle *)AddParticle(
          sizeof(SimpleParticle), g_Mat_DustPuff[1], smokeOrg);
      ...
      sParticle->m_vecVelocity =
          Vector(random->RandomFloat(-16.0f, 16.0f),
      ...
      sParticle->m_flRollDelta =
          random->RandomFloat(-(M_PI / 6.0f), M_PI / 6.0f);
    }
  }
  ...
  if (m_pParticle[0] != NULL) {
    ...
    m_pParticle[0]->m_uchStartSize =
        baseScale * (float)random->RandomInt(32, 48);
