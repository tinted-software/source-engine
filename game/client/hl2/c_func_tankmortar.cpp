...
void C_MortarShell::AddRisingParticles(float flPerc) {
  ...
  float val = RemapValClamped(gpGlobals->curtime, m_flStarttime,
                              m_flStarttime + m_flLifespan, 0.0f, 1.0f);
  ...
  while (m_ParticleEvent.NextEvent(flCur)) {
    ...
    sParticle = (SimpleParticle *)m_pEmitter->AddParticle(
        sizeof(SimpleParticle), m_pEmitter->GetPMaterial("effects/spark"),
        offset);
    ...
    sParticle->m_flRollDelta =
        Helper_RandomFloat(-8.0f * flPerc, 8.0f * flPerc);
...
inline float C_MortarShell::GetEndPerc(void) {
  float val = RemapValClamped(gpGlobals->curtime, m_flStarttime + m_flLifespan,
                              m_flStarttime + m_flLifespan + 1.0f, 1.0f, 0.0f);
...
int C_MortarShell::DrawModel(int flags) {
  ...
  float flScale = (ending) ? m_flRadius
                           : ((m_flRadius * 0.1f) +
                              ((m_flRadius - (m_flRadius * 0.1f)) * flPerc));

  // Do the ground effect
  FX_AddQuad(GetAbsOrigin() + (m_vecSurfaceNormal * 2.0f), m_vecSurfaceNormal,
             flScale, flScale, 1.0f, flAlpha, flAlpha, 1.0f, 0, 0,
             Vector(1.0f, 1.0f, 1.0f), 0.0001f, "effects/combinemuzzle2_nocull",
             0);
