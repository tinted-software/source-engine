...
//-----------------------------------------------------------------------------
void CHudLocator::Reset(void) { m_vecLocation = Vector(0, 0, 0); }
...
void CHudLocator::DrawGraduations(float flYawPlayerFacing) {
  ...
  if (m_textureID_IconBigTick == -1) {
    ...
    vgui::surface()->DrawSetTextureFile(m_textureID_IconBigTick,
                                        LOCATOR_MATERIAL_BIG_TICK, true, false);
  }

  if (m_textureID_IconSmallTick == -1) {
    m_textureID_IconSmallTick = vgui::surface()->CreateNewTextureID();
    vgui::surface()->DrawSetTextureFile(
        m_textureID_IconSmallTick, LOCATOR_MATERIAL_SMALL_TICK, true, false);
  }
  ...
  for (float angle = -180; angle <= 180; angle += angleStep) {
    ...
    if (tallLine) {
      ...
      vgui::surface()->DrawGetTextureSize(m_textureID_IconBigTick, icon_wide,
                                          icon_tall);
      tallLine = false;
    } else {
      vgui::surface()->DrawSetTexture(m_textureID_IconSmallTick);
      vgui::surface()->DrawGetTextureSize(m_textureID_IconSmallTick, icon_wide,
                                          icon_tall);
      tallLine = true;
    }
    ...
    vgui::surface()->DrawTexturedRect(xPos, yPos, xPos + icon_wide,
                                      yPos + icon_tall);
  }
}
...
void CHudLocator::Paint() {
  ...
  if (m_textureID_IconJalopy == -1) {
    ...
    vgui::surface()->DrawSetTextureFile(m_textureID_IconJalopy,
                                        LOCATOR_MATERIAL_JALOPY, true, false);
  }
  ...
  if (bObjectInFOV) {
    // The object's location maps to a valid position along the tape, so draw an
    // icon.
    ...
    vgui::surface()->DrawGetTextureSize(m_textureID_IconJalopy, icon_wide,
                                        icon_tall);
