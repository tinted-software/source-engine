...
bool CHudSuitPower::ShouldDraw() {
  ...
  bNeedsDraw = ((pPlayer->m_HL2Local.m_flSuitPower != m_flSuitPower) ||
                (m_AuxPowerColor[3] > 0));
...
void CHudSuitPower::OnThink(void) {
  ...
  if (flCurrentPower >= 100.0f && m_flSuitPower < 100.0f) {
    ...
    g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(
        "SuitAuxPowerMax");
  } else if (flCurrentPower < 100.0f &&
             (m_flSuitPower >= 100.0f || m_flSuitPower == SUITPOWER_INIT)) {
    // we've lost power
    g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(
        "SuitAuxPowerNotMax");
  }
  ...
  int activeDevices =
      (int)flashlightActive + (int)sprintActive + (int)breatherActive;
...
void CHudSuitPower::Paint() {
  ...
  int enabledChunks =
      (int)((float)chunkCount * (m_flSuitPower * 1.0f / 100.0f) + 0.5f);
