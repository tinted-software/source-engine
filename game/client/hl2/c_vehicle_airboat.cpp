...
//			- Autocenters driver's view after a period of
//inactivity.
//			- Controls headlights.
...
ConVar r_AirboatViewBlendToScale("r_AirboatViewBlendToScale", "0.03",
                                 FCVAR_CHEAT);
...
ConVar r_AirboatRollCurveZero(
    "r_AirboatRollCurveZero", "90.0",
    FCVAR_CHEAT); // Roll less than this is clamped to zero.
ConVar
    r_AirboatRollCurveLinear("r_AirboatRollCurveLinear", "120.0",
                             FCVAR_CHEAT); // Roll greater than this is mapped
                                           // directly. Spline in between.

ConVar r_AirboatPitchCurveZero(
    "r_AirboatPitchCurveZero", "25.0",
    FCVAR_CHEAT); // Pitch less than this is clamped to zero.
ConVar
    r_AirboatPitchCurveLinear("r_AirboatPitchCurveLinear", "60.0",
                              FCVAR_CHEAT); // Pitch greater than this is mapped
                                            // directly. Spline in between.

ConVar airboat_joy_response_move("airboat_joy_response_move",
                                 "1"); // Quadratic steering response
...
private:
  ...
  void DrawPontoonWake(Vector startPos, Vector wakeDir, float wakeLength,
                       float speed);
...
int C_PropAirboat::GetPrimaryAmmoCount() const { return m_nAmmoCount; }

bool C_PropAirboat::PrimaryAmmoUsesClips() const { return false; }

int C_PropAirboat::GetPrimaryAmmoClip() const { return -1; }
...
void C_PropAirboat::DrawHudElements() {
  ...
  CHudTexture *pIcon =
      gHUD.GetIcon(IsX360() ? "crosshair_default" : "plushair");
  if (pIcon != NULL) {
    float x, y;
    Vector screen;
...
//-----------------------------------------------------------------------------
void C_PropAirboat::UpdateViewAngles(C_BasePlayer *pLocalPlayer,
                                     CUserCmd *pCmd) {
  if (r_AirboatViewBlendTo.GetInt()) {
    ...
    if ((pCmd->mousedx != 0 || pCmd->mousedy != 0) ||
      ...
      if (IsX360()) {
        ...
        if (pLocalHLPlayer) {
          ...
          CBaseEntity *pTarget =
              pLocalHLPlayer->m_HL2Local.m_hAutoAimTarget.Get();
    ...
    if (m_flViewAngleDeltaTime > r_AirboatViewBlendToTime.GetFloat()) {
      ...
      Vector vehicleEyeOrigin;
      QAngle vehicleEyeAngles;
      GetAttachmentLocal(eyeAttachmentIndex, vehicleEyeOrigin,
                         vehicleEyeAngles);

      QAngle outAngles;
      InterpolateAngles(pCmd->viewangles, vehicleEyeAngles, outAngles,
                        r_AirboatViewBlendToScale.GetFloat());
...
// Purpose:
//-----------------------------------------------------------------------------
void C_PropAirboat::DampenEyePosition(Vector &vecVehicleEyePos,
...
// Purpose:
//-----------------------------------------------------------------------------
void C_PropAirboat::DampenForwardMotion(Vector &vecVehicleEyePos,
                                        QAngle &vecVehicleEyeAngles,
                                        float flFrameTime) {
  ...
  // vecPredEyePos = predicted eye position this frame (assuming no acceleration
  // - it will get that from the pd controller). vecPredEyeSpeed = predicted eye
  // speed
  Vector vecPredEyePos = m_vecLastEyePos + m_vecEyeSpeed * flFrameTime;
  Vector vecPredEyeSpeed = m_vecEyeSpeed;

  // m_vecLastEyeTarget = real eye position last frame (used for speed
  // calculation). Calculate the approximate speed based on the current vehicle
  // eye position and the eye position last frame.
  ...
  // Calculate the delta between the predicted eye position and speed and the
  // current eye position and speed.
  ...
  if (flDeltaLength > AIRBOAT_DELTA_LENGTH_MAX) {
    ...
    if (flDelta > 40.0f) {
      // This part is a bit of a hack to get rid of large deltas (at level load,
      // etc.).
      ...
      m_vecEyeSpeed += ((flCoefficients[0] * vecDeltaPos +
                         flCoefficients[1] * vecDeltaSpeed) *
                        flFrameTime);
    }
  } else {
...
// Purpose:
//-----------------------------------------------------------------------------
void C_PropAirboat::DampenUpMotion(Vector &vecVehicleEyePos,
                                   QAngle &vecVehicleEyeAngles,
                                   float flFrameTime) {
  // Get up vector.
  Vector vecUp;
...
void C_PropAirboat::UpdateHeadlight() {
  if (m_bHeadlightIsOn) {
    ...
    m_pHeadlight->UpdateLight(vecLightPos, vecLightDir, vecLightRight,
                              vecLightUp, HEADLIGHT_DISTANCE);
...
void C_PropAirboat::UpdateWake(void) {
  ...
  if ((pLast == NULL) || (pLast->m_vecScreenPos.DistToSqr(screenPos) > 4.0f)) {
    ...
    if (pLast) {
      pNewPoint->m_flTexCoord =
          pLast->m_flTexCoord + pLast->m_vecScreenPos.DistTo(screenPos);
...
// Input  : &beamSeg -
//-----------------------------------------------------------------------------
void C_PropAirboat::DrawSegment(const BeamSeg_t &beamSeg,
...
// Input  : position -
//-----------------------------------------------------------------------------
void C_PropAirboat::DrawPontoonSplash(Vector origin, Vector direction,
                                      float speed) {
  Vector offset;
...
//			wakeLength -
//-----------------------------------------------------------------------------
void C_PropAirboat::DrawPontoonWake(Vector startPos, Vector wakeDir,
                                    float wakeLength, float speed) {
#define WAKE_STEPS 6
  ...
  IMaterial *pMaterial =
      materials->FindMaterial("effects/splashwake1", NULL, false);
  ...
  for (int i = 0; i < WAKE_STEPS; i++) {
    ...
    Vector rRight = (Vector(1, 0, 0) * cos(DEG2RAD(yaw))) -
                    (Vector(0, 1, 0) * sin(DEG2RAD(yaw)));
    Vector rUp = (Vector(1, 0, 0) * cos(DEG2RAD(yaw + 90.0f))) -
                 (Vector(0, 1, 0) * sin(DEG2RAD(yaw + 90.0f)));
...
int C_PropAirboat::DrawWake(void) {
  ...
  //  Get all attachments
  GetAttachment(LookupAttachment("raytrace_fl"), vecPontoonFrontLeft,
                fooAngles);
  GetAttachment(LookupAttachment("raytrace_fr"), vecPontoonFrontRight,
                fooAngles);
  GetAttachment(LookupAttachment("raytrace_rl"), vecPontoonRearLeft, fooAngles);
  GetAttachment(LookupAttachment("raytrace_rr"), vecPontoonRearRight,
                fooAngles);
  ...
  if (bDriven && vehicleSpeed > 128.0f) {
    DrawPontoonWake(vecPontoonFrontLeft, vecLeftWakeDir, flWakeLeftLength,
                    vehicleSpeed);
    DrawPontoonWake(vecPontoonFrontRight, vecRightWakeDir, flWakeRightLength,
                    vehicleSpeed);
    ...
    if (m_vecPhysVelocity.x < -64.0f) {
      ...
      if (m_vecPhysVelocity.y > 0.0f) {
        DrawPontoonSplash(vecPontoonFrontLeft + (vecLeftWakeDir * 1.0f),
                          vecSplashDir, m_vecPhysVelocity.y);
      }
      ...
      // Don't do this if we're going backwards
      if (m_vecPhysVelocity.y > 0.0f) {
        DrawPontoonSplash(vecPontoonFrontRight + (vecRightWakeDir * 1.0f),
                          vecSplashDir, m_vecPhysVelocity.y);
      }
    }
  }
  ...
  currentPoint.m_flTexCoord =
      pLast->m_flTexCoord +
      currentPoint.m_vecScreenPos.DistTo(pLast->m_vecScreenPos);
  ...
  for (int i = 0; i <= m_nStepCount; ++i) {
    ...
    TrailPoint_t *pPoint =
        (i != m_nStepCount) ? GetTrailPoint(i) : &currentPoint;

    float flLifePerc =
        RemapValClamped((pPoint->m_flDieTime - gpGlobals->curtime), 0,
                        WAKE_LIFETIME, 0.0f, 1.0f);
    ...
    float alpha =
        RemapValClamped(fabs(m_vecPhysVelocity.y), 128, 600, 0.0f, 1.0f);
    ...
    float widthBase =
        SimpleSplineRemapVal(fabs(m_vecPhysVelocity.y), 128, 600, 32, 48);
