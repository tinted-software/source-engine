...
public:
  ...
  void StandardBlendingRules(CStudioHdr *pStudioHdr, Vector pos[],
                             Quaternion q[], float currentTime, int boneMask);
...
static void RecvProxy_VecTip(const CRecvProxyData *pData, void *pStruct,
                             void *pOut) {
...
void C_NPC_Barnacle::InitTonguePhysics(void) {
  ...
  m_TonguePhysics.SetupSimulation(m_flAltitude / (BARNACLE_TONGUE_POINTS - 1),
                                  &m_PhysicsDelegate);
  m_TonguePhysics.Restart();

  // Initialize the positions of the nodes.
  m_TonguePhysics.GetFirstNode()->m_vPos = m_vecRoot;
  m_TonguePhysics.GetFirstNode()->m_vPrevPos =
      m_TonguePhysics.GetFirstNode()->m_vPos;
  ...
  for (int i = 1; i < m_TonguePhysics.NumNodes(); i++) {
    ...
    pNode->m_vPos =
        m_TonguePhysics.GetNode(i - 1)->m_vPos - Vector(0, 0, flAltitude);
...
void C_NPC_Barnacle::ClientThink(void) {
  ...
  m_TonguePhysics.ResetSpringLength(m_flAltitude /
                                    (BARNACLE_TONGUE_POINTS - 1));
...
// Purpose:
//-----------------------------------------------------------------------------
void C_NPC_Barnacle::StandardBlendingRules(CStudioHdr *hdr, Vector pos[],
  ...
  for (int i = 0; i <= BARNACLE_TONGUE_POINTS; i++) {
    ...
    // debugoverlay->AddBoxOverlay( vecCurr, -Vector(2,2,2), Vector(2,2,2),
    // vec3_angle, 0,255,0, 128, 0.1 );
    ...
    // Forward vector has to follow the tongue, right + up have to minimize
    // twist from the previous bone
    ...
    VectorMA(vecPrevRight, -DotProduct(vecPrevRight, vecForward), vecForward,
             vecRight);
...
// Purpose:
//-----------------------------------------------------------------------------
void C_NPC_Barnacle::CBarnaclePhysicsDelegate::GetNodeForces(
...
// Purpose:
//-----------------------------------------------------------------------------
void C_NPC_Barnacle::CBarnaclePhysicsDelegate::ApplyConstraints(
