...
private:
  ...
  CAntlionDustEmitter(
      const CAntlionDustEmitter &); // not defined, not accessible
};
...
IMPLEMENT_CLIENTCLASS_EVENT_DT(C_TEAntlionDust, DT_TEAntlionDust,
                               CTEAntlionDust)
...
void C_TEAntlionDust::PostDataUpdate(DataUpdateType_t updateType) {
  // This style of creating dust emitters is now deprecated; we use the simple
  // particle singleton exclusively.
  /*
  CSmartPtr<CAntlionDustEmitter> pDustEmitter = CAntlionDustEmitter::Create(
  "TEAntlionDust" ); Assert( pDustEmitter ); if ( pDustEmitter == NULL ) return;

  pDustEmitter->SetSortOrigin( m_vecOrigin );
  pDustEmitter->SetNearClip( 32, 64 );
  pDustEmitter->GetBinding().SetBBox( m_vecOrigin - Vector( 32, 32, 32 ),
  m_vecOrigin + Vector( 32, 32, 32 ) );
  */
  ...
  for (int i = 0; i < iParticleCount; i++) {
    ...
    // Though it appears there are two particle handle entries in
    // g_Mat_DustPuff, in fact only the one present at index 0 actually draws.
    // Trying to spawn a particle with the other material will give you no
    // particle at all. Therefore while instead of this: AddSimpleParticle(
    // &particle, g_Mat_DustPuff[random->RandomInt(0,1)  ); we have to do this:
