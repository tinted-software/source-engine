...
void CShieldProxy::OnBind(C_BaseEntity *pEnt) {
  ...
  sOffset =
      gpGlobals->curtime * sin(m_ScrollAngle * (M_PI / 180.0f)) * m_ScrollRate;
  tOffset =
      gpGlobals->curtime * cos(m_ScrollAngle * (M_PI / 180.0f)) * m_ScrollRate;
...
EXPOSE_INTERFACE(CShieldProxy, IMaterialProxy,
                 "Shield" IMATERIAL_PROXY_INTERFACE_VERSION);
