...
public:
  ...
  virtual void StandardBlendingRules(Vector pos[], Quaternion q[],
...
bool C_NPC_Hydra::SetupBones(matrix3x4_t *pBoneToWorldOut, int nMaxBones,
                             int boneMask, float currentTime) {
  return BaseClass::SetupBones(pBoneToWorldOut, nMaxBones, boneMask,
                               currentTime);
}

void C_NPC_Hydra::StandardBlendingRules(Vector pos[], Quaternion q[],
                                        float currentTime, int boneMask) {
  ...
  if (m_numHydraBones != hdr->numbones) {
    ...
    for (i = 0; i < m_numHydraBones; i++) {
      m_iv_vecPos[i].Setup(&m_vecPos[i], LATCH_SIMULATION_VAR |
                                             EXCLUDE_AUTO_LATCH |
                                             EXCLUDE_AUTO_INTERPOLATE);
    }
  ...
  if (m_bNewChain) {
    ...
    for (i = 0; i < m_numHydraBones; i++) {
      //	debugoverlay->AddLineOverlay( m_vecPos[i],
      //m_vecPos[i<m_numHydraBones-1?i+1:m_numHydraBones-1], 0, 255, 0, false,
      //0.1 );
  ...
  // rotate the last bone of the hydra 90 degrees since it's oriented
...
//			Adjusts overall hydra so that "m_flRelaxedLength" of
//texture fits over 			the actual length of the chain
//-----------------------------------------------------------------------------
...
void C_NPC_Hydra::CalcBoneChain(Vector pos[], const Vector chain[]) {
  ...
  float totalLength = 0;
  for (int k = i; k > 0; k--) {
    // debugoverlay->AddLineOverlay( chain[k], chain[k-1], 255, 255, 255, false,
    // 0 );
  ...
  float dist = -16;
  while (j >= 0 && i > 0) {
    // debugoverlay->AddLineOverlay( chain[i], chain[i-1], 255, 255, 255, false,
    // 0 );
    ...
    while (j >= 0 && dist + dt >= m_boneLength[j]) {
      ...
      Catmull_Rom_Spline(chain[(i < CHAIN_LINKS - 1) ? i + 1 : CHAIN_LINKS - 1],
                         chain[i], chain[(i > 0) ? i - 1 : 0],
                         chain[(i > 1) ? i - 2 : 0], s, pos[j]);
...
bool C_NPC_Hydra::GetSoundSpatialization(SpatializationInfo_t &info) {
  ...
  if (bret) {
    // TODO:  Note, this is where you could override the sound position and
    // orientation and use
    //  an attachment points position as the sound source
    // You might have to issue C_BaseAnimating::AllowBoneAccess( true, false );
    // to allow
