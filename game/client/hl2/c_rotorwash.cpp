...
IMPLEMENT_CLIENTCLASS_DT(C_RotorWashEmitter, DT_RotorWashEmitter,
                         CRotorWashEmitter)
RecvPropFloat(RECVINFO(m_flAltitude)), END_RECV_TABLE()
...
void C_RotorWashEmitter::ClientThink(void) {
  ...
  UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + (Vector(0, 0, -1024)),
                 (MASK_SOLID_BRUSHONLY | CONTENTS_WATER | CONTENTS_SLIME), NULL,
                 COLLISION_GROUP_NONE, &tr);

  if (/*!m_bIgnoreSolid && */ (tr.fraction == 1.0f || tr.startsolid ||
                               tr.allsolid))
    return;
  ...
  float heightScale =
      RemapValClamped(tr.fraction * 1024, 512, 1024, 1.0f, 0.0f);
  ...
  if (tr.contents & (CONTENTS_WATER | CONTENTS_SLIME)) {
    ...
    if (tr.fraction < 1.0f) {
      ...
      FX_AddQuad(tr.endpos + (tr.plane.normal * 0.5f), tr.plane.normal,
                 64.0f * flScale, 128.0f * flScale, 0.8f, 0.75f * heightScale,
                 0.0f, 0.75f, random->RandomFloat(0, 360),
                 random->RandomFloat(-2.0f, 2.0f), vecDustColor, 0.2f,
                 "effects/splashwake3",
  ...
  float yaw =
      random->RandomFloat(0, 2 * M_PI); // Randomly placed on the unit circle
  ...
  for (int i = 0; i < numRingSprites; i++) {
    ...
    // @NOTE toml (3-28-07): broke out following expression because vc2005
    // optimizer was screwing up in presence of SinCos inline assembly. Would
    // also go away if offset were referenced below as in the AddLineOverlay()
    // offset = ( RandomVector( -4.0f, 4.0f ) + tr.endpos ) + ( vecForward *
    // 128.0f );

    offset = vecForward * 128.0f;
    offset += tr.endpos + RandomVector(-4.0f, 4.0f);

    pParticle = (SimpleParticle *)m_pSimple->AddParticle(
        sizeof(SimpleParticle), hMaterial[random->RandomInt(0, 1)], offset);
