...
void CHudAmmo::UpdateVehicleAmmo(C_BasePlayer *player,
...
public:
  CHudSecondaryAmmo(const char *pElementName)
    ...
    SetHiddenBits(HIDEHUD_HEALTH | HIDEHUD_WEAPONSELECTION |
                  HIDEHUD_PLAYERDEAD | HIDEHUD_NEEDSUIT);
  }
  ...
  virtual void Paint(void) {
    ...
    if (m_iconSecondaryAmmo) {
      int nLabelHeight;
      int nLabelWidth;
      surface()->GetTextSize(m_hTextFont, m_LabelText, nLabelWidth,
                             nLabelHeight);
...
protected:
  virtual void OnThink() {
    // set whether or not the panel draws based on if we have a weapon that
    // supports secondary ammo
    C_BaseCombatWeapon *wpn = GetActiveWeapon();
    C_BasePlayer *player = C_BasePlayer::GetLocalPlayer();
    IClientVehicle *pVehicle = player ? player->GetVehicle() : NULL;
  ...
  void UpdateAmmoState() {
    ...
    if (m_hCurrentActiveWeapon != wpn) {
      ...
      m_iconSecondaryAmmo = gWR.GetAmmoIconFromWeapon(
