...
RecvPropString(RECVINFO(m_szMaterialName)), END_RECV_TABLE()
...
#define NUM_DUSTEMITTER_SURFINFOS 128
  ...
  if (IsXbox()) {
    m_ParticleEffect.SetBBox(vCenter - Vector(300, 300, 300),
                             vCenter + Vector(300, 300, 300));
  }
...
#define MAX_SURFINFO_INTERSECTIONS 4
        ...
        if (nIntersections) {
          ...
          // + ((float)rand()/VALVE_RAND_MAX) * 2.0f, -1.0f +
          // ((float)rand()/VALVE_RAND_MAX) * 2.0f); velocity = velocity *
          // FRand(m_MinSpeed, m_MaxSpeed);
...
void C_AR2Explosion::RenderParticles(CParticleRenderIterator *pIterator) {
  ...
  while (pParticle) {
    ...
    if (pParticle->m_Lifetime >= 0.0f) {
      ...
      alpha *=
          GetAlphaDistanceFade(tPos, IsXbox() ? 100 : 50, IsXbox() ? 200 : 150);
