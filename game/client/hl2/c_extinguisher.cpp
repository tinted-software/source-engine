...
// Datatable
IMPLEMENT_CLIENTCLASS_DT(C_ExtinguisherJet, DT_ExtinguisherJet,
                         CExtinguisherJet)
RecvPropInt(RECVINFO(m_bEmit), 0), RecvPropInt(RECVINFO(m_bUseMuzzlePoint), 0),
    RecvPropInt(RECVINFO(m_nLength), 0), RecvPropInt(RECVINFO(m_nSize), 0),
    END_RECV_TABLE()
...
void C_ExtinguisherJet::Update(float fTimeDelta) {
  ...
  UTIL_TraceLine(GetAbsOrigin(), endPoint, MASK_SHOT, NULL,
  ...
  if (m_pEmitter.IsValid()) {
    ...
    while (m_ParticleSpawn.NextEvent(tempDelta)) {
      pParticle = (SimpleParticle *)m_pEmitter->AddParticle(
          sizeof(SimpleParticle), m_MaterialHandle, GetAbsOrigin());
      ...
      if (hitWall) {
        ...
        pParticle = (SimpleParticle *)m_pEmitter->AddParticle(
            sizeof(SimpleParticle), m_MaterialHandle,
      ...
      if (random->RandomInt(0, 1) == 0) {
        ...
        pParticle = (SimpleParticle *)m_pEmberEmitter->AddParticle(
            sizeof(SimpleParticle), g_Mat_DustPuff[0], GetAbsOrigin());
  ...
  for (i = 0; i < numPoints; i++) {
    ...
    beamPoints[i] +=
        shotDir * sin(gpGlobals->curtime * (16.0f * i)) * (1.0f * i);
  }

  IMaterial *pMat = materials->FindMaterial("particle/particle_smokegrenade",
  ...
  for (i = 0; i < numPoints; i++) {
    ...
    seg.m_flTexCoord = (float)i / (float)(numPoints - 1) -
                       ((gpGlobals->curtime - (int)gpGlobals->curtime) * 4.0f);
    seg.m_flWidth =
        4.0f + ((64.0f * t) * (fabs(sin(gpGlobals->curtime * 16.0f))));
    seg.m_flAlpha = color;
