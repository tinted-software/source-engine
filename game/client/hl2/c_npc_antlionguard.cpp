...
private:
  ...
  unsigned char
      m_iPerformingBleedingLevel; //< the version we're currently performing
                                  //(for comparison to one above)
...
// Networking
//-----------------------------------------------------------------------------
IMPLEMENT_CLIENTCLASS_DT(C_NPC_AntlionGuard, DT_NPC_AntlionGuard,
                         CNPC_AntlionGuard)
...
void C_NPC_AntlionGuard::UpdateBleedingPerformance() {
  ...
  {
    m_pBleedingFX = pProp->Create("blood_antlionguard_injured_light",
                                  PATTACH_ABSORIGIN_FOLLOW);
    AssertMsg1(m_pBleedingFX, "Particle system couldn't make %s",
  ...
  {
    m_pBleedingFX = pProp->Create("blood_antlionguard_injured_heavy",
                                  PATTACH_ABSORIGIN_FOLLOW);
    AssertMsg1(m_pBleedingFX, "Particle system couldn't make %s",
...
void C_NPC_AntlionGuard::ClientThink() {
  // update the dlight. (always done because clienthink only exists for
  // cavernguard)
  if (!m_dlight) {
