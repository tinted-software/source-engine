...
void CPlasmaSpray::RenderParticles(CParticleRenderIterator *pIterator) {
  ...
  while (pParticle) {
    ...
    Vector3DMultiply(CurrentWorldToViewMatrix(), pParticle->m_vecVelocity,
                     delta);
    ...
    Tracer_Draw(pIterator->GetParticleDraw(), start, delta,
                random->RandomInt(2, 8), 0);
...
void C_PlasmaBeamNode::OnDataChanged(DataUpdateType_t updateType) {
  if (updateType == DATA_UPDATE_CREATED) {
    ...
    m_pFirePlasmaSpray->m_ParticleCollision.Setup(
        GetAbsOrigin(), &vMoveDir, 0.3, flVel - 50, flVel + 50, 800, 0.5);
...
void C_PlasmaBeamNode::ClientThink(void) {
  ...
  UTIL_TraceLine(GetAbsOrigin(), vEndTrace, MASK_SHOT, NULL,
  ...
  PMaterialHandle handle =
      m_pFirePlasmaSpray->GetPMaterial("sprites/plasmaember");
  for (int i = 0; i < SPRAYS_PER_THINK; i++) {
    ...
    if (random->RandomInt(0, 2) == 0) {
      ...
      Vector vAdd =
          Vector(GetAbsAngles().x, GetAbsAngles().y, GetAbsAngles().z) *
          random->RandomFloat(-60, 120);
      vNewPos += vAdd;

      sParticle = (SimpleParticle *)m_pFirePlasmaSpray->AddParticle(
          sizeof(SimpleParticle), handle, vNewPos);
...
RecvPropVector(RECVINFO(m_vecVelocity), 0, RecvProxy_LocalVelocity),
    RecvPropInt(RECVINFO(m_bSprayOn)), END_RECV_TABLE()
