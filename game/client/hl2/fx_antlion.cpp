...
void CAntlionGibManager::LevelInitPreEntity(void) { m_LRU.Purge(); }
...
public:
  static C_AntlionGib *
  CreateClientsideGib(const char *pszModelName, Vector vecOrigin,
                      Vector vecForceDir, AngularImpulse vecAngularImp,
                      float m_flLifetime = DEFAULT_GIB_LIFETIME) {
    C_AntlionGib *pGib = new C_AntlionGib;

    if (pGib == NULL)
      return NULL;

    if (pGib->InitializeGib(pszModelName, vecOrigin, vecForceDir, vecAngularImp,
                            m_flLifetime) == false)
      return NULL;
  ...
  virtual void HitSurface(C_BaseEntity *pOther) {
    ...
    pOther->GetAbsOrigin(), pOther->GetAbsAngles(), GetAbsOrigin(), 0, 0 );
    }
    */
  }
};
...
void FX_AntlionGib(const Vector &origin, const Vector &direction, float scale) {
  ...
  for (int i = 0; i < NUM_ANTLION_GIBS; i++) {
    ...
    C_AntlionGib::CreateClientsideGib(pszAntlionGibs[i], offset,
                                      (direction + RandomVector(-0.8f, 0.8f)) *
                                          (250 * scale),
  ...
  pSimple->GetBinding().SetBBox(origin - Vector(64, 64, 64),
  ...
  for (int i = 0; i < 4; i++) {
    SimpleParticle *sParticle = (SimpleParticle *)pSimple->AddParticle(
        sizeof(SimpleParticle), g_Material_Blood[0],
  ...
  // Middle core
  SimpleParticle *sParticle = (SimpleParticle *)pSimple->AddParticle(
      sizeof(SimpleParticle), g_Material_Blood[1], origin);
  if (sParticle == NULL)
    return;
