...
RecvPropBool(RECVINFO(m_bHeadlightIsOn)), END_RECV_TABLE()
...
void C_PropJeep::Simulate(void) {
  ...
  if (m_bHeadlightIsOn) {
    ...
    if (iAttachment != INVALID_PARTICLE_ATTACHMENT) {
      ...
      m_pHeadlight->UpdateLight(vVector, vecForward, vecRight, vecUp,
                                JEEP_HEADLIGHT_DISTANCE);
    }
...
void C_PropJeep::UpdateViewAngles(C_BasePlayer *pLocalPlayer, CUserCmd *pCmd) {
  if (r_JeepViewBlendTo.GetInt()) {
    // Check to see if the mouse has been touched in a bit or that we are not
    // throttling.
    ...
    if (m_flViewAngleDeltaTime > r_JeepViewBlendToTime.GetFloat()) {
      ...
      InterpolateAngles(pCmd->viewangles, vehicleEyeAngles, outAngles,
                        r_JeepViewBlendToScale.GetFloat());
...
// Purpose:
//-----------------------------------------------------------------------------
void C_PropJeep::DampenEyePosition(Vector &vecVehicleEyePos,
...
// Purpose:
//-----------------------------------------------------------------------------
void C_PropJeep::DampenForwardMotion(Vector &vecVehicleEyePos,
                                     QAngle &vecVehicleEyeAngles,
                                     float flFrameTime) {
  ...
  if (flDeltaLength > JEEP_DELTA_LENGTH_MAX) {
    ...
    if (flDelta > 40.0f) {
      ...
      ComputePDControllerCoefficients(
          flCoefficients, r_JeepViewDampenFreq.GetFloat(),
...
// Purpose:
//-----------------------------------------------------------------------------
void C_PropJeep::DampenUpMotion(Vector &vecVehicleEyePos,
                                QAngle &vecVehicleEyeAngles,
                                float flFrameTime) {
  // Get up vector.
  Vector vecUp;
...
void WheelDustCallback(const CEffectData &data) {
  ...
  if (pParticle != NULL) {
    ...
    pParticle->m_vecVelocity[2] +=
        random->RandomFloat(16.0f, 32.0f) * (data.m_flScale * 2.0f);
    ...
    pParticle->m_uchStartAlpha =
        random->RandomInt(64.0f * data.m_flScale, 128.0f * data.m_flScale);
