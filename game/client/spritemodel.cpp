...
// Sprites are clipped to this rectangle (x,y,width,height) if ScissorTest is
// enabled
...
// Purpose:
// Set the scissor
//  the coordinate system for gl is upsidedown (inverted-y) as compared to
//  software, so the specified clipping rect must be flipped
// Input  : x -
//			y -
//			width -
//			height -
...
// Output : int
//-----------------------------------------------------------------------------
static int IntersectWRect(const wrect_t *prc1, const wrect_t *prc2,
                          wrect_t *prc) {
  wrect_t rc;
...
// Output : static bool
//-----------------------------------------------------------------------------
static bool Scissor(int &x, int &y, int &width, int &height, float &u0,
...
// Output : static void
//-----------------------------------------------------------------------------
static void AdjustSubRect(CEngineSprite *pSprite, int frame, float *pfLeft,
                          float *pfRight, float *pfTop, float *pfBottom,
                          int *pw, int *ph, const wrect_t *prcSubRect) {
  wrect_t rc;
  float f;
...
bool CEngineSprite::Init(const char *pName) {
  ...
  if (g_pVideo != NULL &&
      g_pVideo->LocateVideoSystemForPlayingFile(pName) != VideoSystem::NONE) {
    m_VideoMaterial = g_pVideo->CreateVideoMaterial(
    ...
    if (!bIsUNC) {
      ...
      Q_strncat(pMaterialName, pTemp, sizeof(pMaterialName),
                COPY_ALL_CHARACTERS);
    } else {
    ...
    for (int i = 0; i < kRenderModeCount; ++i) {
      ...
      Q_snprintf(pMaterialPath, sizeof(pMaterialPath), "%s_rendermode_%d",
                 pMaterialName, i);
      KeyValues *pMaterialKV = kv->MakeCopy();
      pMaterialKV->SetInt("$spriteRenderMode", i);
      m_material[i] = g_pMaterialSystem->FindProceduralMaterial(
          pMaterialPath, TEXTURE_GROUP_CLIENT_EFFECTS, pMaterialKV);
  ...
  IMaterialVar *orientationVar =
      m_material[0]->FindVarFast("$spriteorientation", &spriteOrientationCache);
  m_orientation = orientationVar ? orientationVar->GetIntValue()
                                 : C_SpriteRenderer::SPR_VP_PARALLEL_UPRIGHT;

  IMaterialVar *originVar =
      m_material[0]->FindVarFast("$spriteorigin", &spriteOriginCache);
...
//-----------------------------------------------------------------------------
bool CEngineSprite::IsVideo() { return (m_VideoMaterial != NULL); }

//-----------------------------------------------------------------------------
// Returns the texture coordinate range	used to draw the sprite
//-----------------------------------------------------------------------------
void CEngineSprite::GetTexCoordRange(float *pMinU, float *pMinV, float *pMaxU,
                                     float *pMaxV) {
  *pMaxU = 1.0f;
  *pMaxV = 1.0f;
  if (IsVideo()) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CEngineSprite::DrawFrame(RenderMode_t nRenderMode, int frame, int x, int y,
                              const wrect_t *prcSubRect) {
  DrawFrameOfSize(nRenderMode, frame, x, y, GetWidth(), GetHeight(),
                  prcSubRect);
}
...
//			*prcSubRect -
//-----------------------------------------------------------------------------
void CEngineSprite::DrawFrameOfSize(RenderMode_t nRenderMode, int frame, int x,
                                    int y, int iWidth, int iHeight,
                                    const wrect_t *prcSubRect) {
  // FIXME: If we ever call this with AVIs, need to have it call
  // GetTexCoordRange and make that work
  Assert(!IsVideo());
  float fLeft = 0;
  float fRight = 1;
  float fTop = 0;
  ...
  if (prcSubRect) {
    AdjustSubRect(this, frame, &fLeft, &fRight, &fTop, &fBottom, &iWidth,
                  &iHeight, prcSubRect);
  }

  if (giScissorTest &&
      !Scissor(x, y, iWidth, iHeight, fLeft, fTop, fRight, fBottom))
    return;
