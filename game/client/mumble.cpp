...
ConVar sv_mumble_positionalaudio(
    "sv_mumble_positionalaudio", "1", FCVAR_REPLICATED,
...
IGameSystem *MumbleSystem() { return &g_MumbleSystem; }
...
void CMumbleSystem::LevelInitPostEntity() {
  ...
  g_pMumbleMemory = (MumbleSharedMemory_t *)MapViewOfFile(
      g_hMapObject, FILE_MAP_ALL_ACCESS, 0, 0, sizeof(MumbleSharedMemory_t));
  ...
  g_pMumbleMemory = (MumbleSharedMemory_t *)(mmap(
      NULL, sizeof(struct MumbleSharedMemory_t), PROT_READ | PROT_WRITE,
      MAP_SHARED, shmfd, 0));
...
void CMumbleSystem::PostRender() {
  ...
  if (g_pMumbleMemory->uiVersion != 2) {
    ...
    V_wcscpy_safe(g_pMumbleMemory->description,
                  L"Links Source engine games to Mumble.");
  ...
  if (pPlayer && m_bHasSetPlayerUniqueId &&
      m_nTeamSetInUniqueId != pPlayer->GetTeamNumber()) {
    // Player changed team since we set the unique ID. Set it again.
    m_bHasSetPlayerUniqueId = false;
  }

  if (!m_bHasSetPlayerUniqueId && steamapicontext &&
    ...
    if (steamid.IsValid()) {
      ...
      char szSteamId[256];
      V_sprintf_safe(
          szSteamId, "universe:%u;account_type:%u;id:%u;instance:%u;team:%d",
          steamid.GetEUniverse(), steamid.GetEAccountType(),
          steamid.GetAccountID(), steamid.GetUnAccountInstance(), unTeam);
  ...
  // Context should be equal for players which should be able to hear each other
  // positional and differ for those who shouldn't (e.g. it could contain the
  // server+port and team)
  memcpy(g_pMumbleMemory->context, &m_szSteamIDCurrentServer,
         m_cubSteamIDCurrentServer);
