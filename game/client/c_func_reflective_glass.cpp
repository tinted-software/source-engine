...
IMPLEMENT_CLIENTCLASS_DT(C_FuncReflectiveGlass, DT_FuncReflectiveGlass,
...
template <>
C_FuncReflectiveGlass *C_EntityClassList<C_FuncReflectiveGlass>::m_pClassList =
    NULL;
...
bool C_FuncReflectiveGlass::ShouldDraw() { return true; }
...
bool IsReflectiveGlassInView(const CViewSetup &view, cplane_t &plane) {
  ...
  Frustum_t frustum;
  GeneratePerspectiveFrustum(view.origin, view.angles, view.zNear, view.zFar,
                             view.fov, view.m_flAspectRatio, frustum);

  cplane_t localPlane;
  Vector vecOrigin, vecWorld, vecDelta, vecForward;
  AngleVectors(view.angles, &vecForward, NULL, NULL);

  for (; pReflectiveGlass != NULL;
       pReflectiveGlass = pReflectiveGlass->m_pNext) {
    ...
    for (int i = 0; i < nCount; ++i) {
      ...
      if (view.origin.Dot(plane.normal) <=
          plane.dist) // Check for view behind plane
        continue;
