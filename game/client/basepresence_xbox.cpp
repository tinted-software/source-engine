...
//-----------------------------------------------------------------------------
void CBasePresence::UserSetContext(unsigned int nUserIndex,
                                   unsigned int nContextId,
                                   unsigned int nContextValue, bool bAsync) {
  if (!xboxsystem->UserSetContext(nUserIndex, nContextId, nContextValue,
                                  bAsync)) {
...
//-----------------------------------------------------------------------------
// Properties are (usually) numeric values that can be insterted into context
// strings.
//-----------------------------------------------------------------------------
void CBasePresence::UserSetProperty(unsigned int nUserIndex,
                                    unsigned int nPropertyId,
                                    unsigned int nBytes, const void *pvValue,
                                    bool bAsync) {
  if (!xboxsystem->UserSetProperty(nUserIndex, nPropertyId, nBytes, pvValue,
                                   bAsync)) {
...
//-----------------------------------------------------------------------------
void CBasePresence::GetPropertyDisplayString(uint id, uint value, char *pOutput,
                                             int nBytes) {
  Assert(0);
}
...
//-----------------------------------------------------------------------------
void CBasePresence::SetStat(uint iPropertyId, int iPropertyValue,
                            int dataType) {
...
//-----------------------------------------------------------------------------
void CBasePresence::UploadStats() { Assert(0); }
...
void CBasePresence::DebugUserSetContext(const CCommand &args) {
  if (args.ArgC() == 3) {
    UserSetContext(XBX_GetPrimaryUserId(), atoi(args.Arg(1)),
                   atoi(args.Arg(2)));
  } else {
...
void CBasePresence::DebugUserSetProperty(const CCommand &args) {
  if (args.ArgC() == 3) {
    ...
    UserSetProperty(XBX_GetPrimaryUserId(), strtoul(args.Arg(1), NULL, 0),
                    sizeof(int), &value);
  } else {
