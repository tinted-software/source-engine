...
RecvPropInt(RECVINFO(m_iRopeMaterialModelIndex)),
    ...
    RecvPropInt(RECVINFO(m_RopeFlags)), RecvPropFloat(RECVINFO(m_TextureScale)),
...
static ConVar
    rope_smooth_minwidth("rope_smooth_minwidth", "0.3", 0,
                         "When using smoothing, this is the min screenspace "
                         "width it lets a rope shrink to");
...
static ConVar r_ropes_holiday_lights_allowed("r_ropes_holiday_lights_allowed",
...
public:
  ...
  inline void *Alloc(size_t bytes) {
    ...
    if (pReturn == NULL) {
      ...
      Warning("Overflowed rope queued rendering memory stack. Needed %d, have "
              "%d/%d\n",
...
private:
  ...
  CThreadFastMutex m_RenderCacheMutex; // there's only any contention during the
                                       // switch from r_queued_ropes on to off
...
void CRopeManager::DrawRenderCache_NonQueued(
  ...
  for (int iRenderCache = 0; iRenderCache < nRenderCacheCount; ++iRenderCache) {
      ...
      if (pRope) {
        ...
        if (pBuildRopeQueuedData) {
          pRope->BuildRope(pRopeSegment, vCurrentViewForward,
                           vCurrentViewOrigin, pBuildRopeQueuedData, true);
          ...
          pRope->BuildRope(pRopeSegment, vCurrentViewForward,
                           vCurrentViewOrigin, &stackQueuedData, false);
        }
      } else {
        if (pBuildRopeQueuedData) {
    ...
    if (materials->GetRenderContext()->GetCallQueue() != NULL &&
        pBuildRopeQueuedData == NULL) {
...
void CRopeManager::DrawRenderCache(bool bShadowDepth) {
  ...
  if (r_queued_ropes.GetBool() &&
    ...
    Assert(((void *)pVectorWrite ==
            (void *)(((uint8 *)pMemory) + iMemoryNeeded)) &&
    ...
    if (IsHolidayLightMode()) {
      // With holiday lights we need to also build the ropes non-queued without
      // rendering them
...
int CRopeManager::GetHolidayLightStyle(void) { return m_nHolidayLightsStyle; }
...
CSimplePhysics::IHelper *
C_RopeKeyframe::HookPhysics(CSimplePhysics::IHelper *pHook) {
...
void C_RopeKeyframe::BuildRope(
    ...
    C_RopeKeyframe::BuildRopeQueuedData_t *pQueuedData, bool bQueued) {
  if (!pSegmentData)
    return;
  ...
  for (int iNode = 0; iNode < nodeCount; ++iNode) {
    ...
    if (!bQueued && RopeManager()->IsHolidayLightMode() &&
...
IMaterial *C_RopeKeyframe::GetBackMaterial(void) { return m_pBackMaterial; }
