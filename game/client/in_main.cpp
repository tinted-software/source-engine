...
ConVar cl_yawspeed("cl_yawspeed", "210", FCVAR_NONE, "Client yaw speed.", true,
                   -100000, true, 100000);
ConVar cl_pitchspeed("cl_pitchspeed", "225", FCVAR_NONE, "Client pitch speed.",
                     true, -100000, true, 100000);
...
ConVar
    thirdperson_platformer("thirdperson_platformer", "0", 0,
...
int KB_ConvertString(char *in, char **ppout) {
  ...
  *ppout = NULL;
  ...
  while (*p) {
    if (*p == '+') {
      pEnd = binding;
      while (*p && (V_isalnum(*p) || (pEnd == binding)) &&
             ((pEnd - binding) < 63)) {
        *pEnd++ = *p++;
      }
...
void CInput::AdjustYaw(float speed, QAngle &viewangles) {
  ...
  if (CAM_IsThirdPerson() && thirdperson_platformer.GetInt()) {
    ...
    if (side || forward) {
      viewangles[YAW] = RAD2DEG(atan2(side, forward)) +
                        g_ThirdPersonManager.GetCameraOffsetAngles()[YAW];
    }
    if (side || forward || KeyState(&in_right) || KeyState(&in_left)) {
      cam_idealyaw.SetValue(g_ThirdPersonManager.GetCameraOffsetAngles()[YAW] -
                            viewangles[YAW]);
    }
  }
}
...
void CInput::ControllerMove(float frametime, CUserCmd *cmd) {
  ...
  if (haptics && haptics->HasDevice()) {
    if (engine->IsPaused() || engine->IsLevelMainMenuBackground() ||
        vgui::surface()->IsCursorVisible() || !engine->IsInGame()) {
...
void CInput::ExtraMouseSample(float frametime, bool active) {
  ...
  if (active && UseVR()) {
    ...
    if (pPlayer && !pPlayer->GetVehicle()) {
      ...
      g_ClientVirtualReality.OverridePlayerMotion(frametime, originalViewangles,
                                                  curViewangles, curMotion,
...
void CInput::CreateMove(int sequence_number, float input_sample_frametime,
  ...
  CVerifiedUserCmd *pVerified =
      &m_pVerifiedCommands[sequence_number % MULTIPLAYER_BACKUP];
  ...
  if (g_pClientMode->CreateMove(input_sample_frametime, cmd)) {
    // Get current view angles after the client mode tweaks with it
#ifdef SIXENSE
    // Only set the engine angles if sixense is not enabled. It is done in
    // SixenseInput::SetView otherwise.
    if (!g_pSixenseInput->IsEnabled()) {
      engine->SetViewAngles(cmd->viewangles);
    }
#else
    ...
    if (UseVR()) {
      ...
      if (pPlayer && !pPlayer->GetVehicle()) {
        ...
        engine->GetViewAngles(curViewangles);
        curMotion.Init(cmd->forwardmove, cmd->sidemove, cmd->upmove);
        g_ClientVirtualReality.OverridePlayerMotion(
            input_sample_frametime, originalViewangles, curViewangles,
...
void CInput::Init_All(void) {
  ...
  m_rgNewMouseParms[MOUSE_SPEED_FACTOR] =
      1; // 0 = disabled, 1 = threshold 1 enabled, 2 = threshold 2 enabled
