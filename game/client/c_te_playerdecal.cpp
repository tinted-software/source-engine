...
static ConVar cl_playerspraydisable("cl_playerspraydisable", "0",
...
// Purpose:
//-----------------------------------------------------------------------------
IMaterial *CreateTempMaterialForPlayerLogo(int iPlayerIndex,
                                           player_info_t *info, char *texname,
                                           int nchars) {
  // Doesn't have a logo?
  if (!info->customFiles[0])
    return NULL;

  IMaterial *logo = materials->FindMaterial(
      VarArgs("decals/playerlogo%2.2d", iPlayerIndex), TEXTURE_GROUP_DECAL);
...
void TE_PlayerDecal(IRecipientFilter &filter, float delay, const Vector *pos,
  ...
  char texname[512];
  IMaterial *logo =
      CreateTempMaterialForPlayerLogo(player, &info, texname, 512);
  if (!logo)
    return;
  ...
  effects->PlayerDecalShoot(logo, (void *)(intp)player, entity, ent->GetModel(),
                            ent->GetAbsOrigin(), ent->GetAbsAngles(), *pos, 0,
                            0, rgbaColor);
}
...
IMPLEMENT_CLIENTCLASS_EVENT_DT(C_TEPlayerDecal, DT_TEPlayerDecal,
                               CTEPlayerDecal)
RecvPropVector(RECVINFO(m_vecOrigin)), RecvPropInt(RECVINFO(m_nEntity)),
    RecvPropInt(RECVINFO(m_nPlayer)), END_RECV_TABLE()
