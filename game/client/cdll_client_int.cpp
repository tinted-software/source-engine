...
static ConVar
    s_CV_ShowParticleCounts("showparticlecounts", "0", 0,
...
public:
  ...
  const char *GetHolidayString() { return UTIL_GetActiveHolidayString(); }
};
...
public:
  ...
  virtual void
  WriteSaveGameScreenshotOfSize(const char *pFilename, int width, int height,
  ...
  virtual const char *TranslateEffectForVisionFilter(const char *pchEffectType,
...
int CHLClient::Init(CreateInterfaceFn appSystemFactory,
  ...
  IGameSystem::Add(
      g_pColorCorrectionMgr); // NOTE: This must happen prior to ClientThinkList
                              // (color correction is updated there)
...
#ifdef SIXENSE
  // If we're not connected, update sixense so we can move the mouse cursor when
  // in the menus
...
void CHLClient::PrecacheMaterial(const char *pMaterialName) {
  ...
  if (!IsErrorMaterial(pMaterial)) {
    ...
    if (IsOSX()) {
      printf("\n ##### CHLClient::PrecacheMaterial could not find material %s "
             "(%s)",
...
#ifdef PORTAL
  g_pPortalRender
      ->UpdatePortalPixelVisibility(); // updating this one or two lines before
                                       // querying again just isn't cutting it.
                                       // Update as soon as it's cheap to do so.
#endif
...
// Get the mouthinfo for the sound being played inside UI panels
CMouthInfo *CHLClient::GetClientUIMouthInfo() { return &g_ClientUIMouth; }
...
void CHLClient::ClientAdjustStartSoundParams(StartSoundParams_t &params) {
  ...
  if (params.entchannel == CHAN_VOICE && GameRules() && pEntity &&
      pEntity->IsPlayer()) {
    // Use high-pitched voices for other players if the local player has an item
    // that allows them to hear it (Pyro Goggles)
...
const char *
CHLClient::TranslateEffectForVisionFilter(const char *pchEffectType,
                                          const char *pchEffectName) {
  if (!GameRules())
...
CSteamID GetSteamIDForPlayerIndex(int iPlayerIndex) {
  ...
  if (steamapicontext && steamapicontext->SteamUtils()) {
    if (engine->GetPlayerInfo(iPlayerIndex, &pi)) {
      if (pi.friendsID) {
        return CSteamID(pi.friendsID, 1,
