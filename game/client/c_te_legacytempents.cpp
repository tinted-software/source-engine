...
bool C_LocalTempEntity::Frame(float frametime, int framenumber) {
  ...
  if (flags & FTENT_PLYRATTACHMENT) {
    ...
    SetLocalOrigin(Vector(x + sin(m_vecTempEntVelocity[2] +
                                  gpGlobals->curtime /* * anim.prevframe */) *
                                  (10 * m_flSpriteScale),
                          y + sin(m_vecTempEntVelocity[2] + fastFreq + 0.7) *
                                  (8 * m_flSpriteScale),
                          GetLocalOriginDim(Z_INDEX) +
    ...
    if (flags & (FTENT_COLLIDEALL | FTENT_COLLIDEPROPS)) {
      ...
      if (bShouldCollide == true) {
        ...
        if ((flags & FTENT_COLLIDEPROPS) && trace.m_pEnt) {
          bool bIsDynamicProp =
              (NULL != dynamic_cast<CDynamicProp *>(trace.m_pEnt));
          bool bIsDoor = (NULL != dynamic_cast<CBaseDoor *>(trace.m_pEnt));
          if (!bIsDynamicProp && !bIsDoor &&
              !trace.m_pEnt->IsWorld()) // Die on props, doors, and the world.
            return true;
        }
...
//-----------------------------------------------------------------------------
CNewParticleEffect *
C_LocalTempEntity::AddParticleEffect(const char *pszParticleEffect) {
  ...
  FOR_EACH_LL(m_TempEnts, i) {
    ...
    if (pTemp->flags & FTENT_PLYRATTACHMENT) {
      ...
      if (pTemp->clientIndex == client) {
        pTemp->die =
            gpGlobals->curtime; // good enough, it will die on next tent update.
      }
    }
  }
}
...
void CTempEnts::EjectBrass(const Vector &pos1, const QAngle &angles,
  ...
  pTemp->die = gpGlobals->curtime + 1.0f +
...
//-----------------------------------------------------------------------------
C_LocalTempEntity *CTempEnts::SpawnTempModel(const model_t *pModel,
...
C_LocalTempEntity *CTempEnts::FindTempEntByID(int nID, int nSubID) {
  // HACK HACK: We're using skin and hitsounds as a hacky way to store an ID and
  // sub-ID for later identification
  FOR_EACH_LL(m_TempEnts, i) {
    C_LocalTempEntity *p = m_TempEnts[i];
...
// Output : C_LocalTempEntity
//-----------------------------------------------------------------------------
C_LocalTempEntity *CTempEnts::TempEntAlloc(const Vector &org,
                                           const model_t *model) {
  C_LocalTempEntity *pTemp;
