  ...
  virtual void CalcSpriteColorAndSize(float flDot, CGlowSprite *pSprite,
    ...
    *flVertSize = pSprite->m_flVertSize;
    ...
    if (m_nOuterMaxDist > m_nMaxDist && distToViewer > m_nMaxDist) {
      fade = RemapValClamped(distToViewer, m_nMaxDist, m_nOuterMaxDist, 1.0f,
                             0.0f);
    } else {
...
RecvPropInt(RECVINFO(m_clrRender), 0, RecvProxy_IntToColor32),
    ...
    RecvPropInt(RECVINFO(m_nVerticalSize)), RecvPropInt(RECVINFO(m_nMinDist)),
    RecvPropInt(RECVINFO(m_nMaxDist)), RecvPropInt(RECVINFO(m_nOuterMaxDist)),
...
void C_LightGlow::OnDataChanged(DataUpdateType_t updateType) {
  ...
  if (updateType == DATA_UPDATE_CREATED) {
    // Setup our flare.
    Vector vColor(m_clrRender->r / 255.0f, m_clrRender->g / 255.0f,
    ...
    SetNextClientThink(gpGlobals->curtime + RandomFloat(0, 3.0));
             ...
             DATA_UPDATE_DATATABLE_CHANGED) // Right now only color should
                                            // change.
  {
    // Setup our flare.
    Vector vColor(m_clrRender->r / 255.0f, m_clrRender->g / 255.0f,
