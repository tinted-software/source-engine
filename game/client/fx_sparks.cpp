...
static ConVar fx_drawmetalspark("fx_drawmetalspark", "1", FCVAR_DEVELOPMENTONLY,
...
bool EffectOccluded(const Vector &pos, pixelvis_handle_t *queryHandle) {
  if (!queryHandle) {
    // NOTE: This is called by networking code before the current view is set
    // up. so use the main view instead
    trace_t tr;
    UTIL_TraceLine(pos, MainViewOrigin(), MASK_OPAQUE, NULL,
  ...
  return PixelVisibility_FractionVisible(params, queryHandle) > 0.0f ? false
                                                                     : true;
}

CSimpleGlowEmitter::CSimpleGlowEmitter(const char *pDebugName,
...
CSimpleGlowEmitter *CSimpleGlowEmitter::Create(const char *pDebugName,
                                               const Vector &sortOrigin,
                                               float flDeathTime) {
  return new CSimpleGlowEmitter(pDebugName, sortOrigin, flDeathTime);
}

void CSimpleGlowEmitter::SimulateParticles(
...
void CSimpleGlowEmitter::SetVisibleInView(unsigned char viewMask,
                                          bool bVisible) {
...
//-----------------------------------------------------------------------------
// Purpose: Test for surrounding collision surfaces for quick collision testing
// for the particle system Input  : &origin - starting position
//			*dir - direction of movement (if NULL, will do a point
//emission test in four directions) 			angularSpread - looseness of the spread
...
//-----------------------------------------------------------------------------
void CTrailParticles::Setup(const Vector &origin, const Vector *direction,
                            float angularSpread, float minSpeed, float maxSpeed,
                            float gravity, float dampen, int flags,
                            bool bNotCollideable) {
  // Take the flags
  if (!bNotCollideable) {
    SetFlag((flags | bitsPARTICLE_TRAIL_COLLIDE)); // Force this if they've
  ...
  m_ParticleCollision.Setup(origin, direction, angularSpread, minSpeed,
                            maxSpeed, gravity, dampen);
}
...
void CTrailParticles::RenderParticles(CParticleRenderIterator *pIterator) {
  ...
  while (pParticle) {
    ...
    Vector3DMultiply(ParticleMgr()->GetModelView(), pParticle->m_vecVelocity,
                     delta);
    ...
    if (pParticle->m_flLifetime <= 0.3 &&
        m_fFlags & bitsPARTICLE_TRAIL_FADE_IN) {
    ...
    float flLength =
        (pParticle->m_vecVelocity * scale).Length(); //( delta - pos ).Length();
    float flWidth =
        (flLength < pParticle->m_flWidth) ? flLength : pParticle->m_flWidth;

    // See if we should fade
    Vector vecScaledDelta = (delta * scale);
    Tracer_Draw(pIterator->GetParticleDraw(), start, vecScaledDelta, flWidth,
                color);
...
void CTrailParticles::SimulateParticles(CParticleSimulateIterator *pIterator) {
  ...
  while (pParticle) {
    ...
    trace_t trace;
    m_ParticleCollision.MoveParticle(pParticle->m_Pos, pParticle->m_vecVelocity,
                                     NULL, timeDelta, &trace);
...
void FX_ElectricSpark(const Vector &pos, int nMagnitude, int nTrailLength,
  ...
  pSparkEmitter->Setup((Vector &)pos, NULL, SPARK_ELECTRIC_SPREAD,
  ...
  for (i = 0; i < numSparks; i++) {
    ...
    pParticle->m_vecVelocity =
        dir *
        random->RandomFloat(SPARK_ELECTRIC_MINSPEED, SPARK_ELECTRIC_MAXSPEED);
  ...
  sParticle = (SimpleParticle *)pSimple->AddParticle(
      sizeof(SimpleParticle), pSimple->GetPMaterial("effects/yellowflare_noz"),
      pos);
...
void FX_MetalScrape(Vector &position, Vector &normal) {
  ...
  CSmartPtr<CTrailParticles> sparkEmitter =
      CTrailParticles::Create("FX_MetalScrape 1");
  ...
  // Setup our collision information
  sparkEmitter->Setup(offset, &normal, METAL_SCRAPE_SPREAD,
                      METAL_SCRAPE_MINSPEED, METAL_SCRAPE_MAXSPEED,
                      METAL_SCRAPE_GRAVITY, METAL_SCRAPE_DAMPEN,
                      bitsPARTICLE_TRAIL_VELOCITY_DAMPEN);
  ...
  for (int i = 0; i < numSparks; i++) {
    ...
    dir[0] = normal[0] + random->RandomFloat(-(METAL_SCRAPE_SPREAD * spreadOfs),
                                             (METAL_SCRAPE_SPREAD * spreadOfs));
    dir[1] = normal[1] + random->RandomFloat(-(METAL_SCRAPE_SPREAD * spreadOfs),
                                             (METAL_SCRAPE_SPREAD * spreadOfs));
    dir[2] = normal[2] + random->RandomFloat(-(METAL_SCRAPE_SPREAD * spreadOfs),
                                             (METAL_SCRAPE_SPREAD * spreadOfs));
    ...
    pParticle->m_vecVelocity =
        dir * random->RandomFloat((METAL_SCRAPE_MINSPEED * (2.0f - spreadOfs)),
...
void FX_MetalSpark(const Vector &position, const Vector &direction,
                   const Vector &surfaceNormal, int iScale) {
  ...
  CSmartPtr<CTrailParticles> sparkEmitter =
      CTrailParticles::Create("FX_MetalSpark 1");
  ...
  numSparks = (int)(0.5f + (float)numSparks *
                               g_pParticleSystemMgr->ParticleThrottleScaling());
  ...
  for (int i = 0; i < numSparks; i++) {
    ...
    if (iScale > 1 && i % 3 == 0) {
      // Every third spark goes flying far if we're having a big batch of
      // sparks.
    ...
    dir[0] =
        direction[0] + random->RandomFloat(-(METAL_SPARK_SPREAD * spreadOfs),
                                           (METAL_SPARK_SPREAD * spreadOfs));
    dir[1] =
        direction[1] + random->RandomFloat(-(METAL_SPARK_SPREAD * spreadOfs),
                                           (METAL_SPARK_SPREAD * spreadOfs));
    dir[2] =
        direction[2] + random->RandomFloat(-(METAL_SPARK_SPREAD * spreadOfs),
                                           (METAL_SPARK_SPREAD * spreadOfs));
    ...
    pParticle->m_vecVelocity =
        dir * random->RandomFloat((METAL_SPARK_MINSPEED * (2.0f - spreadOfs)),
...
void FX_Sparks(const Vector &pos, int nMagnitude, int nTrailLength,
               ...
               float flMaxSpeed, char *pSparkMaterial) {
  VPROF_BUDGET("FX_Sparks", VPROF_BUDGETGROUP_PARTICLE_RENDERING);
  CSmartPtr<CTrailParticles> pSparkEmitter =
      CTrailParticles::Create("FX_Sparks 1");
  ...
  // Setup our collision information
  pSparkEmitter->Setup((Vector &)pos, NULL, SPARK_SPREAD, flMinSpeed,
                       flMaxSpeed, SPARK_GRAVITY, SPARK_DAMPEN,
                       bitsPARTICLE_TRAIL_VELOCITY_DAMPEN);
  ...
  for (i = 0; i < numSparks; i++) {
    ...
    float spreadOfs = random->RandomFloat(0.0f, 2.0f);
    dir[0] = vecDir[0] + random->RandomFloat(-(SPARK_SPREAD * spreadOfs),
                                             (SPARK_SPREAD * spreadOfs));
    dir[1] = vecDir[1] + random->RandomFloat(-(SPARK_SPREAD * spreadOfs),
                                             (SPARK_SPREAD * spreadOfs));
    dir[2] = vecDir[2] + random->RandomFloat(-(SPARK_SPREAD * spreadOfs),
                                             (SPARK_SPREAD * spreadOfs));
    pParticle->m_vecVelocity =
        dir * random->RandomFloat((flMinSpeed * (2.0f - spreadOfs)),
  ...
  for (i = 0; i < numSparks; i++) {
    ...
    pParticle->m_vecVelocity =
        dir * random->RandomFloat(flMinSpeed, flMaxSpeed);
...
void FX_EnergySplash(const Vector &pos, const Vector &normal, int nFlags) {
  ...
  // Quick flash
  FX_AddQuad(pos, normal, 64.0f, 0, 0.75f, 1.0f, 0.0f, 0.4f,
             random->RandomInt(0, 360), 0, Vector(1.0f, 1.0f, 1.0f), 0.25f,
             "effects/combinemuzzle1_nocull",
             (FXQUAD_BIAS_SCALE | FXQUAD_BIAS_ALPHA));

  // Lingering burn
  FX_AddQuad(pos, normal, 16, 32, 0.75f, 1.0f, 0.0f, 0.4f,
  ...
  for (int j = 0; j < 8; j++) {
    ...
    sParticle = (SimpleParticle *)pEmitter->AddParticle(
        sizeof(SimpleParticle), g_Material_Spark, offset);

    if (sParticle == NULL)
      return;

    sParticle->m_vecVelocity =
        Vector(Helper_RandomFloat(-4.0f, 4.0f), Helper_RandomFloat(-4.0f, 4.0f),
...
void FX_MicroExplosion(Vector &position, Vector &normal) {
  ...
  CSmartPtr<CTrailParticles> sparkEmitter =
      CTrailParticles::Create("FX_MicroExplosion 1");
  ...
  // Setup our collision information
  sparkEmitter->Setup(offset, &normal, MICRO_EXPLOSION_SPREAD,
                      MICRO_EXPLOSION_MINSPEED, MICRO_EXPLOSION_MAXSPEED,
                      MICRO_EXPLOSION_GRAVITY, MICRO_EXPLOSION_DAMPEN,
                      bitsPARTICLE_TRAIL_VELOCITY_DAMPEN);
  ...
  for (int i = 0; i < numSparks; i++) {
    ...
    if (pParticle) {
      ...
      dir[0] = normal[0] + random->RandomFloat(-MICRO_EXPLOSION_SPREAD * ramp,
                                               MICRO_EXPLOSION_SPREAD * ramp);
      dir[1] = normal[1] + random->RandomFloat(-MICRO_EXPLOSION_SPREAD * ramp,
                                               MICRO_EXPLOSION_SPREAD * ramp);
      dir[2] = normal[2] + random->RandomFloat(-MICRO_EXPLOSION_SPREAD * ramp,
                                               MICRO_EXPLOSION_SPREAD * ramp);
      ...
      pParticle->m_vecVelocity =
          dir * random->RandomFloat(MICRO_EXPLOSION_MINSPEED * (1.5f - ramp),
  ...
  CSmartPtr<CSimpleEmitter> pSimple =
      CSimpleEmitter::Create("FX_MicroExplosion 2");
  pSimple->SetSortOrigin(offset);

  SimpleParticle *sParticle = (SimpleParticle *)pSimple->AddParticle(
      sizeof(SimpleParticle), pSimple->GetPMaterial("sprites/rico1"), offset);
...
void FX_Explosion(Vector &origin, Vector &normal, char materialType) {
  ...
  CSmartPtr<CTrailParticles> pSparkEmitter =
      CTrailParticles::Create("FX_Explosion 1");
  if (!pSparkEmitter)
    return;
  ...
  pTraceMaterial = engine->TraceLineMaterialAndLighting(
      origin, normal * -16.0f, diffuseColor, baseColor);
  ...
  // Setup our collision information
  pSparkEmitter->Setup(offset, &normal, EXPLOSION_SPREAD, EXPLOSION_MINSPEED,
                       EXPLOSION_MAXSPEED, EXPLOSION_GRAVITY, EXPLOSION_DAMPEN,
                       bitsPARTICLE_TRAIL_VELOCITY_DAMPEN);
  ...
  int i;
  for (i = 0; i < numSparks; i++) {
    TrailParticle *pParticle = (TrailParticle *)pSparkEmitter->AddParticle(
        sizeof(TrailParticle), g_Material_Spark, offset);
    ...
    dir[0] =
        normal[0] + random->RandomFloat(-EXPLOSION_SPREAD, EXPLOSION_SPREAD);
    dir[1] =
        normal[1] + random->RandomFloat(-EXPLOSION_SPREAD, EXPLOSION_SPREAD);
    dir[2] =
        normal[2] + random->RandomFloat(-EXPLOSION_SPREAD, EXPLOSION_SPREAD);
    pParticle->m_vecVelocity =
        dir * random->RandomFloat(EXPLOSION_MINSPEED, EXPLOSION_MAXSPEED);
  ...
  if (materialType == 'C' || materialType == 'W') {
    CSmartPtr<CFleckParticles> fleckEmitter =
        CFleckParticles::Create("FX_Explosion 10", offset, Vector(5, 5, 5));
    if (!fleckEmitter)
      return;

    // Setup our collision information
    fleckEmitter->m_ParticleCollision.Setup(
        offset, &normal, EXPLOSION_FLECK_ANGULAR_SPRAY,
    ...
    for (i = 0; i < numFlecks; i++) {
      FleckParticle *pParticle = (FleckParticle *)fleckEmitter->AddParticle(
          sizeof(FleckParticle), hMaterialArray[random->RandomInt(0, 1)],
          offset);
      ...
      dir[0] = normal[0] + random->RandomFloat(-EXPLOSION_FLECK_ANGULAR_SPRAY,
                                               EXPLOSION_FLECK_ANGULAR_SPRAY);
      dir[1] = normal[1] + random->RandomFloat(-EXPLOSION_FLECK_ANGULAR_SPRAY,
                                               EXPLOSION_FLECK_ANGULAR_SPRAY);
      dir[2] = normal[2] + random->RandomFloat(-EXPLOSION_FLECK_ANGULAR_SPRAY,
                                               EXPLOSION_FLECK_ANGULAR_SPRAY);
      pParticle->m_uchSize = random->RandomInt(2, 6);
      pParticle->m_vecVelocity =
          dir * (random->RandomFloat(EXPLOSION_FLECK_MIN_SPEED,
                                     EXPLOSION_FLECK_MAX_SPEED) *
  ...
  // Large sphere bursts
  CSmartPtr<CSimpleEmitter> pSimpleEmitter =
      CSimpleEmitter::Create("FX_Explosion 1");
  ...
  SimpleParticle *pSphereParticle =
      (SimpleParticle *)pSimpleEmitter->AddParticle(
          sizeof(SimpleParticle), hSphereMaterial, vecBurstOrigin);
  if (pSphereParticle) {
  ...
  for (i = 0; i < numBalls; i++) {
    SimpleParticle *pParticle = (SimpleParticle *)pSimpleEmitter->AddParticle(
        sizeof(SimpleParticle), hSphereMaterial, vecBurstOrigin);
    if (pParticle) {
      pParticle->m_flLifetime = 0.0f;
  ...
  CSmartPtr<CSimpleEmitter> pSmokeEmitter =
      CSimpleEmitter::Create("FX_Explosion 2");
  pSmokeEmitter->SetSortOrigin(offset);
  for (i = 0; i < 2; i++) {
    SimpleParticle *pParticle = (SimpleParticle *)pSmokeEmitter->AddParticle(
        sizeof(SimpleParticle), g_Mat_DustPuff[1], offset);
    if (pParticle == NULL)
      break;
...
void FX_ConcussiveExplosion(Vector &origin, Vector &normal) {
  ...
  CSmartPtr<CSimpleEmitter> pSimple =
      CSimpleEmitter::Create("FX_ConcussiveExplosion 2");

  pSimple->SetSortOrigin(offset);

  SimpleParticle *pParticle = (SimpleParticle *)pSimple->AddParticle(
      sizeof(SimpleParticle), pSimple->GetPMaterial("effects/blueflare1"),
      offset);
  ...
  CSmartPtr<CTrailParticles> pSparkEmitter =
      CTrailParticles::Create("FX_ConcussiveExplosion 3");
  PMaterialHandle hMaterial;
  int numSparks;
  ...
  if (pSparkEmitter.IsValid()) {
    ...
    // Dump out sparks
    for (i = 0; i < numSparks; i++) {
      pTrailParticle = (TrailParticle *)pSparkEmitter->AddParticle(
  ...
  // Moving particles
  CSmartPtr<CTrailParticles> pCollisionEmitter =
      CTrailParticles::Create("FX_ConcussiveExplosion 4");

  if (pCollisionEmitter.IsValid()) {
    // Setup our collision information
    pCollisionEmitter->Setup((Vector &)offset, NULL, SPARK_ELECTRIC_SPREAD,
                             SPARK_ELECTRIC_MINSPEED * 6,
                             SPARK_ELECTRIC_MAXSPEED * 6, -400,
                             SPARK_ELECTRIC_DAMPEN, bitsPARTICLE_TRAIL_FADE);
    ...
    // Dump out sparks
    for (i = 0; i < numSparks; i++) {
      pTrailParticle = (TrailParticle *)pCollisionEmitter->AddParticle(
          sizeof(TrailParticle), hMaterial, offset);
