...
public:
  CTFVoteNotification(const char *pPlayerName) : CEconNotification() {
    g_pVGuiLocalize->ConvertANSIToUnicode(pPlayerName, m_wszPlayerName,
                                          sizeof(m_wszPlayerName));
    SetLifetime(7);
  ...
  virtual void Trigger() {
    CTFGenericConfirmDialog *pDialog = ShowConfirmDialog(
        "#Vote_notification_title", "#Vote_notification_text",
        "#Vote_notification_view", "#cancel", &ConfirmShowVoteSetup);
    pDialog->SetContext(this);
    pDialog->AddStringToken("initiator", m_wszPlayerName);
    // so we aren't deleted
    SetIsInUse(true);
  }
  virtual bool CanBeAcceptedOrDeclined() { return true; }
  virtual void Accept() { ConfirmShowVoteSetup(true, this); }
  virtual void Decline() { ConfirmShowVoteSetup(false, this); }
...
void VoteBarPanel::Paint(void) {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    vgui::surface()->DrawFilledRect(x + m_iBoxInset, m_iBoxInset,
                                    x + m_iBoxSize - m_iBoxInset,
  ...
  vgui::surface()->DrawTexturedRect(x + iImageInset, iImageInset,
                                    x + m_iBoxSize - iImageInset,
...
void VoteBarPanel::FireGameEvent(IGameEvent *event) {
  ...
  if (FStrEq(eventName, "vote_changed")) {
    for (int index = 0; index < MAX_VOTE_OPTIONS; index++) {
      ...
      Q_strncat(szVoteOption, szOption, sizeof(szVoteOption),
                COPY_ALL_CHARACTERS);
...
CVoteSetupDialog::CVoteSetupDialog(vgui::Panel *parent)
  ...
  m_pCallVoteButton =
      new Button(this, "CallVoteButton", "CallVote", this, "CallVote");
...
void CVoteSetupDialog::ApplySchemeSettings(vgui::IScheme *pScheme) {
  ...
  m_pComboBox->GetComboButton()->SetDefaultColor(Color(117, 107, 94, 255),
                                                 Color(0, 0, 0, 0));
  m_pComboBox->GetComboButton()->SetArmedColor(Color(117, 107, 94, 255),
                                               Color(0, 0, 0, 0));
  m_pComboBox->GetComboButton()->SetDepressedColor(Color(117, 107, 94, 255),
                                                   Color(0, 0, 0, 0));
...
void CVoteSetupDialog::PostApplySchemeSettings(vgui::IScheme *pScheme) {
  ...
  for (int i = 0; i < m_pImageList->GetImageCount(); i++) {
    ...
    m_pImageList->GetImage(i)->SetSize(
        scheme()->GetProportionalScaledValueEx(GetScheme(), wide),
...
void CVoteSetupDialog::ApplySettings(KeyValues *inResourceData) {
  ...
  const char *pszColor =
      inResourceData->GetString("issue_fgcolor", "Label.TextColor");
  m_IssueFGColor = pScheme->GetColor(pszColor, Color(255, 255, 255, 255));

  pszColor =
      inResourceData->GetString("issue_fgcolor_disabled", "Label.TextColor");
  m_IssueFGColorDisabled =
      pScheme->GetColor(pszColor, Color(255, 255, 255, 255));
...
//-----------------------------------------------------------------------------
void CVoteSetupDialog::AddVoteIssueParams_MapCycle(
...
//-----------------------------------------------------------------------------
void CVoteSetupDialog::AddVoteIssueParams_PopFiles(
...
void CVoteSetupDialog::Activate() {
  ...
  m_pVoteSetupList->AddColumnToSection(0, "Issue", "#TF_Vote_Column_Issue",
  ...
  m_pVoteParameterList->AddColumnToSection(
      0, "Avatar", "",
      SectionedListPanel::COLUMN_IMAGE | SectionedListPanel::COLUMN_RIGHT, 55);
  m_pVoteParameterList->AddColumnToSection(0, "", "", 0, 10); // Spacer
  m_pVoteParameterList->AddColumnToSection(0, "Name", "#TF_Vote_Column_Name", 0,
                                           m_iParameterWidth * 0.6);
  m_pVoteParameterList->AddColumnToSection(
      0, "Properties", "#TF_Vote_Column_Properties",
  ...
  for (int index = 0; index < m_VoteIssues.Count(); index++) {
    ...
    if (m_hIssueFont != INVALID_FONT) {
      ...
      bool bDisabled = V_stristr(
          pszIssue, "(Disabled on Server)"); // driller: need to localize
        ...
        if (iSelectedParam >= 0) {
          ...
          if (pKeyValues) {
            ...
            const char *pReasonString =
                m_pComboBox->GetActiveItemUserData()
            ...
            if (engine->GetPlayerInfo(playerIndex, &playerInfo)) {
              ...
              if (pSubject && pSubject->GetSteamID(&steamID) &&
                  steamID.GetAccountID() != 0) {
                GCSDK::CProtoBufMsg<CMsgTFVoteKickBanPlayer> msg(
  ...
  if (panel == m_pVoteSetupList) {
    ...
    if (iSelectedItem >= 0) {
      ...
      if (V_stricmp("Voting disabled on this Server", szName) == 0) {
        m_bVoteButtonEnabled = false;
      } else if (V_stristr(szName,
                           "(Disabled on Server)")) // driller: need to localize
      {
        m_bVoteButtonEnabled = false;
      }
      // CHANGELEVEL / NEXTLEVEL
      else if (V_stricmp("changelevel", szName) == 0 ||
               V_stricmp("nextlevel", szName) == 0) {
...
#ifdef TF_CLIENT_DLL
        SetDialogVariable("combo_label",
                          g_pVGuiLocalize->Find("#TF_VoteKickReason"));
        m_pComboBox->AddItem(g_pVGuiLocalize->Find("TF_VoteKickReason_Other"),
                             new KeyValues("other"));
        m_pComboBox->AddItem(
            g_pVGuiLocalize->Find("TF_VoteKickReason_Cheating"),
            new KeyValues("cheating"));
        m_pComboBox->AddItem(g_pVGuiLocalize->Find("TF_VoteKickReason_Idle"),
                             new KeyValues("idle"));
        m_pComboBox->AddItem(
            g_pVGuiLocalize->Find("TF_VoteKickReason_Scamming"),
            new KeyValues("scamming"));
      ...
      // CHANGE POP FILE
        ...
        for (int index = 0; index < m_VoteIssuesPopFiles.Count(); index++) {
          ...
          const char *pszPopFileName =
              TFObjectiveResource()->GetMvMPopFileName();
          if (!pszPopFileName || !pszPopFileName[0]) {
            // Use the map name
            char szShortMapName[MAX_MAP_NAME];
            V_strncpy(szShortMapName, engine->GetLevelName(),
                      sizeof(szShortMapName));
            V_StripExtension(szShortMapName, szShortMapName,
                             sizeof(szShortMapName));

            if (V_strncmp(m_VoteIssuesPopFiles[index],
                          V_GetFileName(szShortMapName),
        ...
        // User selected an issue that doesn't require a parameter -
        // Scrambleteams, Restartgame, etc
        m_bVoteButtonEnabled = true;
      }
    }
  } else if (panel == m_pVoteParameterList) {
    // If this issue requires a parameter, make sure we have one selected before
    // enabling the CallVote button
    int iSelectedParam = m_pVoteParameterList->GetSelectedItem();
    if (iSelectedParam >= 0) {
      KeyValues *pParameterKeyValues =
          m_pVoteParameterList->GetItemData(iSelectedParam);
...
void CVoteSetupDialog::RefreshIssueParameters() {
  // In the case of the KICK issue, we list players and show additional
  // properties (Bot, Disconnected)
  int iSelectedItem = m_pVoteSetupList->GetSelectedItem();
  if (iSelectedItem >= 0) {
    KeyValues *pIssueKeyValues = m_pVoteSetupList->GetItemData(iSelectedItem);
    const char *szName = pIssueKeyValues->GetString("Issue");
    if (V_stricmp("kick", szName) == 0) {
      if (m_pVoteParameterList->GetItemCount() > 0) {
        for (int index = 0; index < m_pVoteParameterList->GetItemCount();
             index++) {
...
// Purpose:
//-----------------------------------------------------------------------------
int CHudVote::KeyInput(int down, ButtonCode_t keynum,
...
void CHudVote::MsgFunc_CallVoteFailed(bf_read &msg) {
  ...
  case VOTE_FAILED_TRANSITIONING_PLAYERS:
    m_pCallVoteFailed->SetControlString("FailedReason",
                                        "#GameUI_vote_failed_transition_vote");
    break;

  case VOTE_FAILED_RATE_EXCEEDED:
    g_pVGuiLocalize->ConstructString(
        wszHeaderString, sizeof(wszHeaderString),
        g_pVGuiLocalize->Find("#GameUI_vote_failed_vote_spam"), 1, wszTime);
  ...
  case VOTE_FAILED_ISSUE_DISABLED:
    m_pCallVoteFailed->SetControlString("FailedReason",
                                        "#GameUI_vote_failed_disabled_issue");
    break;

  case VOTE_FAILED_MAP_NOT_FOUND:
    m_pCallVoteFailed->SetControlString("FailedReason",
                                        "#GameUI_vote_failed_map_not_found");
    break;

  case VOTE_FAILED_MAP_NOT_VALID:
    m_pCallVoteFailed->SetControlString("FailedReason",
                                        "#GameUI_vote_failed_map_not_valid");
    break;

  case VOTE_FAILED_MAP_NAME_REQUIRED:
    m_pCallVoteFailed->SetControlString(
        "FailedReason", "#GameUI_vote_failed_map_name_required");
    break;

  case VOTE_FAILED_FAILED_RECENTLY:
    g_pVGuiLocalize->ConstructString(
        wszHeaderString, sizeof(wszHeaderString),
        g_pVGuiLocalize->Find("#GameUI_vote_failed_recently"), 1, wszTime);
    pwszHeaderString = wszHeaderString;
    m_pCallVoteFailed->SetDialogVariable("FailedReason", pwszHeaderString);
    break;
  ...
  case VOTE_FAILED_WAITINGFORPLAYERS:
    m_pCallVoteFailed->SetControlString(
        "FailedReason", "#GameUI_vote_failed_waitingforplayers");
    break;

  case VOTE_FAILED_CANNOT_KICK_ADMIN:
    m_pCallVoteFailed->SetControlString(
        "FailedReason", "#GameUI_vote_failed_cannot_kick_admin");
    break;
  ...
  case VOTE_FAILED_CANNOT_KICK_FOR_TIME:
    g_pVGuiLocalize->ConstructString(
        wszHeaderString, sizeof(wszHeaderString),
        g_pVGuiLocalize->Find("#GameUI_vote_failed_cannot_kick_for_time"), 1,
        wszTime);
    pwszHeaderString = wszHeaderString;
    m_pCallVoteFailed->SetDialogVariable("FailedReason", pwszHeaderString);
    break;
  ...
  case VOTE_FAILED_CANNOT_KICK_DURING_ROUND:
    m_pCallVoteFailed->SetControlString("FailedReason",
                                        "#GameUI_vote_failed_round_active");
    break;

  case VOTE_FAILED_MODIFICATION_ALREADY_ACTIVE:
    m_pCallVoteFailed->SetControlString(
        "FailedReason", "#GameUI_vote_failed_event_already_active");
    break;
  }
}
...
void CHudVote::MsgFunc_VoteFailed(bf_read &msg) {
  ...
  case VOTE_FAILED_YES_MUST_EXCEED_NO:
    m_pVoteFailed->SetControlString("FailedReason",
                                    "#GameUI_vote_failed_yesno");
    break;

  case VOTE_FAILED_QUORUM_FAILURE:
    m_pVoteFailed->SetControlString("FailedReason",
                                    "#GameUI_vote_failed_quorum");
    break;
  }
...
void CHudVote::MsgFunc_VoteStart(bf_read &msg) {
  ...
  g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(
      m_pVoteActive, "HideVoteBackgrounds");
  ...
  // ...sometimes more
  m_pVoteActive->SetControlVisible(
      "LabelOption3", m_VoteSetupChoices.Count() > 2 ? true : false);
  m_pVoteActive->SetControlVisible("Option3Background_Selected",
                                   m_VoteSetupChoices.Count() > 2 ? true
                                                                  : false);
  m_pVoteActive->SetControlVisible(
      "LabelOption4", m_VoteSetupChoices.Count() > 3 ? true : false);
  m_pVoteActive->SetControlVisible("Option4Background_Selected",
                                   m_VoteSetupChoices.Count() > 3 ? true
                                                                  : false);
  m_pVoteActive->SetControlVisible(
      "LabelOption5", m_VoteSetupChoices.Count() > 4 ? true : false);
  m_pVoteActive->SetControlVisible("Option5Background_Selected",
                                   m_VoteSetupChoices.Count() > 4 ? true
                                                                  : false);
  ...
  // Player
  g_pVGuiLocalize->ConvertANSIToUnicode(pszCallerName, wszCallerName,
                                        sizeof(wszCallerName));

  // String
  g_pVGuiLocalize->ConstructString(wszHeaderString, sizeof(wszHeaderString),
                                   g_pVGuiLocalize->Find("#GameUI_vote_header"),
                                   1, wszCallerName);
  pwszHeaderString = wszHeaderString;
  ...
  if (Q_strlen(szParam1) > 0) {
    ...
    g_pVGuiLocalize->ConstructString(
        wcIssue, sizeof(wcIssue), g_pVGuiLocalize->Find(szIssue), 1, pwcParam);
    pwcIssue = wcIssue;
  } else {
  ...
  if (m_bIsYesNoVote) {
    ...
    if (m_VoteSetupChoices.Count()) {
      ...
      for (int iIndex = 0; iIndex < MAX_VOTE_OPTIONS; iIndex++) {
        ...
        Q_strncat(szVoteOptionCount, szOptionNum, sizeof(szVoteOptionCount),
                  COPY_ALL_CHARACTERS);
      ...
      for (int iIndex = 0; iIndex < m_nVoteChoicesCount; iIndex++) {
        ...
        Q_strncat(szOptionName, pszChoiceName, sizeof(szOptionName),
                  COPY_ALL_CHARACTERS);
...
void CHudVote::MsgFunc_VotePass(bf_read &msg) {
  ...
  if (Q_strlen(szParam1) > 0) {
    ...
    g_pVGuiLocalize->ConstructString(
        wcIssue, sizeof(wcIssue), g_pVGuiLocalize->Find(szResult), 1, pwcParam);
    pwcIssue = wcIssue;
  } else {
...
void CHudVote::MsgFunc_VoteSetup(bf_read &msg) {
  ...
  if (pStringTable) {
    int index = bMvM ? pStringTable->FindStringIndex("ServerMapCycleMvM")
                     : pStringTable->FindStringIndex("ServerMapCycle");
    if (index != ::INVALID_STRING_INDEX) {
      int nLength = 0;
      const char *pszMapCycle =
          (const char *)pStringTable->GetStringUserData(index, &nLength);
  ...
  if (g_pStringTableServerPopFiles) {
    ...
    if (index != ::INVALID_STRING_INDEX) {
      int nLength = 0;
      const char *pszPopFiles =
          (const char *)g_pStringTableServerPopFiles->GetStringUserData(
              index, &nLength);
...
void CHudVote::FireGameEvent(IGameEvent *event) {
  ...
  if (FStrEq(eventName, "vote_changed")) {
    for (int index = 0; index < MAX_VOTE_OPTIONS; index++) {
      ...
      Q_strncat(szVoteOptionCount, szOption, sizeof(szVoteOptionCount),
                COPY_ALL_CHARACTERS);
    ...
    for (int iIndex = 0; iIndex < m_nVoteChoicesCount; iIndex++) {
      ...
      Q_strncat(szOptionName, szNumber, sizeof(szOptionName),
                COPY_ALL_CHARACTERS);
    ...
    if (vote_option == VOTE_OPTION1) {
      g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(
          m_pVoteActive, "PulseOption1");
    } else if (vote_option == VOTE_OPTION2) {
      g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(
          m_pVoteActive, "PulseOption2");
    } else if (vote_option == VOTE_OPTION3) {
      g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(
          m_pVoteActive, "PulseOption3");
    } else if (vote_option == VOTE_OPTION4) {
      g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(
          m_pVoteActive, "PulseOption4");
    } else if (vote_option == VOTE_OPTION5) {
      g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(
          m_pVoteActive, "PulseOption5");
    }
...
void CHudVote::OnThink() {
  ...
  if (m_bPlayerVoted && m_flPostVotedHideTime > 0 &&
      m_flPostVotedHideTime < gpGlobals->curtime) {
    m_pVoteActive->SetVisible(false);
    m_bShowVoteActivePanel = false;
    m_flPostVotedHideTime = -1;
  }

  if (m_flVoteResultCycleTime > 0 &&
      m_flVoteResultCycleTime < gpGlobals->curtime) {
    m_pVoteActive->SetVisible(false);
  ...
  if (m_bVoteActive) {
    // driller:  Need to rewrite this to handle all vote types (Yes/No and
    // General)
...
// Purpose:
//-----------------------------------------------------------------------------
bool CHudVote::IsPlayingDemo() const { return engine->IsPlayingDemo(); }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CHudVote::SetVoteActive(bool bActive) { m_bVoteActive = bActive; }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CHudVote::ShowVoteUI(void) { m_bShowVoteActivePanel = true; }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
bool CHudVote::IsVoteUIActive(void) { return m_bShowVoteActivePanel; }
