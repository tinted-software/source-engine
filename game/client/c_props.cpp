...
bool C_DynamicProp::TestBoneFollowers(const Ray_t &ray,
                                      unsigned int fContentsMask, trace_t &tr) {
  // UNDONE: There is no list of the bone followers that is networked to the
  // client so instead we do a search for solid stuff here.  This is not really
  // great - a list would be preferable.
  CBaseEntity *pList[128];
  Vector mins, maxs;
  CollisionProp()->WorldSpaceAABB(&mins, &maxs);
  int count = UTIL_EntitiesInBox(pList, ARRAYSIZE(pList), mins, maxs, 0,
...
bool C_DynamicProp::TestCollision(const Ray_t &ray, unsigned int fContentsMask,
                                  trace_t &tr) {
  ...
  if (m_bUseHitboxesForRenderBox) {
    if (GetModel()) {
      ...
      if (gpGlobals->framecount != m_iCachedFrameCount) {
        ComputeEntitySpaceHitboxSurroundingBox(&m_vecCachedRenderMins,
...
void C_BasePropDoor::OnDataChanged(DataUpdateType_t type) {
  ...
  if (type == DATA_UPDATE_CREATED) {
    ...
    VPhysicsGetObject()->UpdateShadow(GetAbsOrigin(), GetAbsAngles(), false,
                                      TICK_INTERVAL);
  }
}

bool C_BasePropDoor::TestCollision(const Ray_t &ray, unsigned int mask,
                                   trace_t &trace) {
...
IMPLEMENT_CLIENTCLASS_DT(CPhysBoxMultiplayer, DT_PhysBoxMultiplayer,
                         CPhysBoxMultiplayer)
...
IMPLEMENT_CLIENTCLASS_DT(CPhysicsPropMultiplayer, DT_PhysicsPropMultiplayer,
                         CPhysicsPropMultiplayer)
RecvPropInt(RECVINFO(m_iPhysicsMode)), RecvPropFloat(RECVINFO(m_fMass)),
    RecvPropVector(RECVINFO(m_collisionMins)),
    RecvPropVector(RECVINFO(m_collisionMaxs)), END_RECV_TABLE()
#endif
