...
RecvPropInt(RECVINFO(m_iActiveIssueIndex), 0,
            C_VoteController::RecvProxy_VoteType),
    RecvPropInt(RECVINFO(m_iOnlyTeamToVote)),
    RecvPropArray3(RECVINFO_ARRAY(m_nVoteOptionCount),
                   RecvPropInt(RECVINFO(m_nVoteOptionCount[0]), 0,
                               C_VoteController::RecvProxy_VoteOption)),
    RecvPropInt(RECVINFO(m_nPotentialVotes)),
    RecvPropBool(RECVINFO(m_bIsYesNoVote)) END_RECV_TABLE()

    //-----------------------------------------------------------------------------
    // Purpose:
    //-----------------------------------------------------------------------------
    void C_VoteController::RecvProxy_VoteType(const CRecvProxyData *pData,
                                              void *pStruct, void *pOut) {
  C_VoteController *pMe = (C_VoteController *)pStruct;
  if (memcmp(&pMe->m_iActiveIssueIndex, &pData->m_Value.m_Int,
             sizeof(pData->m_Value.m_Int)) == 0)
    return;

  memcpy(&pMe->m_iActiveIssueIndex, &pData->m_Value.m_Int,
         sizeof(pData->m_Value.m_Int));
  pMe->m_bTypeDirty = true;

  // Since the contents of a new vote are in three parts, we can't directly send
  // an event to the Hud because we don't really know if we have all three parts
  // yet.  So we'll mark dirty, and our think can notice that and send the
  // event.
}
...
// Purpose:
//-----------------------------------------------------------------------------
void C_VoteController::RecvProxy_VoteOption(const CRecvProxyData *pData,
                                            void *pStruct, void *pOut) {
...
void C_VoteController::ClientThink() {
  ...
  if (m_bVotesDirty) {
    if (m_nPotentialVotes > 0) {
      ...
      DevMsg("Votes: Option1 - %d, Option2 - %d, Option3 - %d, Option4 - %d, "
             "Option5 - %d\n",
             m_nVoteOptionCount[0], m_nVoteOptionCount[1],
             m_nVoteOptionCount[2], m_nVoteOptionCount[3],
