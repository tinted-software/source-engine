...
public:
  ...
  void CopyParentToChild(const Vector parentPos[], const Quaternion parentQ[],
                         Vector childPos[], Quaternion childQ[], int boneMask);
...
private:
  ...
  // All the cache data is based off these. When they change, the cache data is
  // regenerated. These are either all valid pointers or all NULL.
  ...
  CUtlVector<unsigned char> m_BoneMergeBits; // One bit for each bone. The bit
                                             // is set if the bone gets merged.
};
