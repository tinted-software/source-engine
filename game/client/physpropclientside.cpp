...
ConVar cl_phys_props_max("cl_phys_props_max", "300", 0,
                         "Maximum clientside physic props");
ConVar r_propsmaxdist("r_propsmaxdist", "1200", 0, "Maximum visible distance");

ConVar cl_phys_props_enable(
    "cl_phys_props_enable", "1", 0,
...
ConVar cl_phys_props_respawndist(
    ...
    "Minimum distance from the player that a clientside prop must be before "
    "it's allowed to respawn.");
ConVar cl_phys_props_respawnrate(
    "cl_phys_props_respawnrate", "60", 0,
...
bool C_PhysPropClientside::KeyValue(const char *szKeyName,
                                    const char *szValue) {
...
bool C_PhysPropClientside::Initialize() {
  if (InitializeAsClientEntity(STRING(GetModelName()),
  ...
  if (!PhysModelParseSolid(tmpSolid, this, GetModelIndex())) {
    DevMsg("C_PhysPropClientside::Initialize: PhysModelParseSolid failed for "
           "entity %i.\n",
           GetModelIndex());
    return false;
  } else {
    m_pPhysicsObject = VPhysicsInitNormal(
        SOLID_VPHYSICS, 0, m_spawnflags & SF_PHYSPROP_START_ASLEEP, &tmpSolid);

    if (!m_pPhysicsObject) {
      // failed to create a physics object
      DevMsg(" C_PhysPropClientside::Initialize: VPhysicsInitNormal() failed "
             "for %s.\n",
             STRING(GetModelName()));
      return false;
    }
  ...
  if (m_iPhysicsMode == PHYSICS_MULTIPLAYER_AUTODETECT) {
    m_iPhysicsMode = GetAutoMultiplayerPhysicsMode(CollisionProp()->OBBSize(),
                                                   m_pPhysicsObject->GetMass());
  }
...
void C_PhysPropClientside::ImpactTrace(trace_t *pTrace, int iDamageType,
...
CBaseEntity *BreakModelCreateSingle(CBaseEntity *pOwner, breakmodel_t *pModel,
  ...
  C_PhysPropClientside *pBreakableOwner =
      dynamic_cast<C_PhysPropClientside *>(pOwner);
  ...
  if (pModel->isRagdoll) {
    DevMsg("BreakModelCreateSingle: clientside doesn't support ragdoll "
           "breakmodels.\n");
  }
...
// Purpose:
//-----------------------------------------------------------------------------
bool C_FuncPhysicsRespawnZone::KeyValue(const char *szKeyName,
                                        const char *szValue) {
...
bool C_FuncPhysicsRespawnZone::Initialize(void) {
  ...
  SetNextClientThink(
      gpGlobals->curtime +
      (cl_phys_props_respawnrate.GetFloat() * RandomFloat(1.0, 1.1)));
...
void C_FuncPhysicsRespawnZone::PropDestroyed(C_PhysPropClientside *pProp) {
  ...
  // We've got a clientside prop that thinks it belongs to a zone that doesn't
  // recognise it. Shouldn't happen.
  Assert(0);
}
...
// Purpose:
//-----------------------------------------------------------------------------
bool C_FuncPhysicsRespawnZone::CanMovePropAt(Vector vecOrigin,
                                             const Vector &vecMins,
                                             const Vector &vecMaxs) {
  float flDist = cl_phys_props_respawndist.GetFloat();

  // Do a distance check first. We don't want to move props when the player is
  // near 'em.
...
void C_FuncPhysicsRespawnZone::RespawnProps(void) {
  for (int i = 0; i < m_PropList.Count(); i++) {
    if (m_PropList[i].hClientEntity == INVALID_CLIENTENTITY_HANDLE) {
      if (!CanMovePropAt(m_PropList[i].vecOrigin, -Vector(32, 32, 32),
                         Vector(32, 32, 32)))
        continue;
      ...
      C_BaseEntity *pEntity = ClientEntityList().GetBaseEntityFromHandle(
          m_PropList[i].hClientEntity);
      if (pEntity) {
        if (!CollisionProp()->IsPointInBounds(pEntity->WorldSpaceCenter())) {
          Vector vecMins, vecMaxs;
          pEntity->CollisionProp()->WorldSpaceSurroundingBounds(&vecMins,
                                                                &vecMaxs);
          ...
          if (pPhys) {
            pPhys->SetPosition(pEntity->GetAbsOrigin(), pEntity->GetAbsAngles(),
                               true);
          }
        }
      }
