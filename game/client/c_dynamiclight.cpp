...
RecvPropInt(RECVINFO(m_Flags)), RecvPropInt(RECVINFO(m_LightStyle)),
    RecvPropFloat(RECVINFO(m_Radius)), RecvPropInt(RECVINFO(m_Exponent)),
...
// Purpose :
//------------------------------------------------------------------------------
bool C_DynamicLight::ShouldDraw() { return false; }
...
void C_DynamicLight::ClientThink(void) {
  ...
  if ((m_Flags & DLIGHT_NO_MODEL_ILLUMINATION) == 0) {
    ...
    if (!m_pDynamicLight || (m_pDynamicLight->key != index)) {
      ...
      m_pDynamicLight = ShouldBeElight() != 0 ? effects->CL_AllocElight(index)
    ...
    m_pDynamicLight->color.exponent =
        m_Exponent; // this makes it match the world
    ...
    // In this case, the m_Flags could have changed; which is how we turn the
    // light off
    if (m_pDynamicLight) {
      m_pDynamicLight->die = gpGlobals->curtime;
      m_pDynamicLight = 0;
...
#endif
  {
    ...
    UTIL_TraceLine(GetAbsOrigin(), end, MASK_NPCWORLDSTATIC, NULL,
                   COLLISION_GROUP_NONE, &pm);
    ...
    if (pm.fraction == 1.0f) {
      ...
      m_pSpotlightEnd->flags =
          DLIGHT_NO_MODEL_ILLUMINATION | (m_Flags & DLIGHT_DISPLACEMENT_MASK);
