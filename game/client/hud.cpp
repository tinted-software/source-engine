...
void LoadHudTextures(CUtlDict<CHudTexture *, int> &list,
  ...
  KeyValues *pKeyValuesData =
      ReadEncryptedKVFile(filesystem, szFilenameWithoutExtension, pICEKey);
  if (pKeyValuesData) {
    CUtlVector<HudTextureFileRef> hudTextureFileRefs;

    // By default, add a default entry mapping "file" to no prefix. This will
    // allow earlier-version files to work with no modification.
    ...
    if (pTextureFileRefs) {
      ...
      while (pTemp) {
        hudTextureFileRefs.AddToTail(HudTextureFileRef(
        ...
        if (pTemp->GetString("font", NULL)) {
          ...
          for (int i = 0; i < hudTextureFileRefs.Size(); i++) {
            const char *cszFilename =
                pTemp->GetString(hudTextureFileRefs[i].m_fileKeySymbol, NULL);
            if (cszFilename) {
              CHudTexture *tex = new CHudTexture();
              ...
              Q_strncpy(tex->szShortName,
                        hudTextureFileRefs[i].m_cszHudTexturePrefix,
                        sizeof(tex->szShortName));
              Q_strncpy(tex->szShortName +
                            hudTextureFileRefs[i].m_uiPrefixLength,
                        pTemp->GetName(),
                        sizeof(tex->szShortName) -
                            hudTextureFileRefs[i].m_uiPrefixLength);
              Q_strncpy(tex->szTextureFile, cszFilename,
                        sizeof(tex->szTextureFile));
...
CHudTexture &CHudTexture::operator=(const CHudTexture &src) {
  ...
  if (src.textureId == -1) {
    ...
    vgui::surface()->DrawSetTextureFile(textureId, src.szTextureFile, false,
                                        false);
  }
...
// Purpose:
//-----------------------------------------------------------------------------
void CHudElement::SetActive(bool bActive) { m_bActive = bActive; }
...
// Purpose:
//-----------------------------------------------------------------------------
void CHudElement::SetHiddenBits(int iBits) { m_iHiddenBits = iBits; }
...
bool CHudElement::ShouldDraw(void) {
  ...
  if (bShouldDraw) {
    ...
    for (int iGroupIndex = 0; iGroupIndex < iNumGroups; iGroupIndex++) {
      if (gHUD.IsRenderGroupLockedFor(this,
                                      m_HudRenderGroups.Element(iGroupIndex)))
        return false;
    }
  }
...
//-----------------------------------------------------------------------------
void CHudElement::HideLowerPriorityHudElementsInGroup(
...
//-----------------------------------------------------------------------------
void CHudElement::UnhideLowerPriorityHudElementsInGroup(
    const char *pszGroupName) {
  // look up the render group
  int iGroupIndex = gHUD.LookupRenderGroupIndexByName(pszGroupName);
...
// Purpose:
//-----------------------------------------------------------------------------
int CHudElement::GetRenderGroupPriority(void) { return 0; }
  ...
  if (kv) {
    if (kv->LoadFromFile(filesystem, "scripts/HudLayout.res")) {
      ...
      for (int i = 0; i < numelements; i++) {
        ...
        if (!key) {
          Msg("Hud element '%s' doesn't have an entry '%s' in "
              "scripts/HudLayout.res\n",
              m_HudList[i]->GetName(), pPanel->GetName());
        }

        // Note:  When a panel is parented to the module root, it's "parent" is
        // returned as NULL.
...
void CHud::Shutdown(void) {
  ...
  // Deleting hudlist items can result in them being removed from the same
  // hudlist (m_bNeedsRemove).
...
void CHudTexture::Precache(void) {
  // costly function, used selectively on specific hud elements to get font
  // pages built out at load time
  if (IsX360() && bRenderUsingFont && !bPrecached &&
...
void CHudTexture::DrawSelf(int x, int y, int w, int h, const Color &clr) const {
  if (bRenderUsingFont) {
    ...
    vgui::surface()->DrawTexturedSubRect(x, y, x + w, y + h, texCoords[0],
                                         texCoords[1], texCoords[2],
                                         texCoords[3]);
  }
}

void CHudTexture::DrawSelfCropped(int x, int y, int cropx, int cropy, int cropw,
  ...
  if (bRenderUsingFont) {
    ...
    if (vgui::surface()->DrawGetUnicodeCharRenderInfo(cCharacterInFont, info)) {
      if (cropy) {
        info.verts[0].m_Position.y =
            Lerp(frac, info.verts[0].m_Position.y, info.verts[1].m_Position.y);
        info.verts[0].m_TexCoord.y =
            Lerp(frac, info.verts[0].m_TexCoord.y, info.verts[1].m_TexCoord.y);
      } else if (croph != height) {
        info.verts[1].m_Position.y =
            Lerp(1.0f - frac, info.verts[0].m_Position.y,
                 info.verts[1].m_Position.y);
        info.verts[1].m_TexCoord.y =
            Lerp(1.0f - frac, info.verts[0].m_TexCoord.y,
                 info.verts[1].m_TexCoord.y);
      }
    ...
    vgui::surface()->DrawSetTexture(textureId);
    vgui::surface()->DrawSetColor(clr);
    vgui::surface()->DrawTexturedSubRect(x, y, x + finalWidth, y + finalHeight,
...
void CHud::SetupNewHudTexture(CHudTexture *t) {
  if (t->bRenderUsingFont) {
    vgui::HScheme scheme = vgui::scheme()->GetScheme("ClientScheme");
    t->hFont =
        vgui::scheme()->GetIScheme(scheme)->GetFont(t->szTextureFile, true);
    t->rc.top = 0;
    t->rc.left = 0;
    t->rc.right =
        vgui::surface()->GetCharacterWidth(t->hFont, t->cCharacterInFont);
    ...
    vgui::surface()->DrawSetTextureFile(t->textureId, t->szTextureFile, false,
                                        false);
...
CHudTexture *CHud::AddUnsearchableHudIconToList(CHudTexture &texture) {
  ...
  if (texture.bRenderUsingFont) {
    ...
    Q_snprintf(composedName, sizeof(composedName), "%s_%i_%i_%i_%i",
               texture.szTextureFile, texture.rc.left, texture.rc.top,
               texture.rc.right, texture.rc.bottom);
  }
...
void CHud::RefreshHudTextures() {
  ...
  for (int index = 0; index < c; index++) {
    ...
    // Update file
    Q_strncpy(icon->szTextureFile, tex->szTextureFile,
              sizeof(icon->szTextureFile));

    if (!icon->bRenderUsingFont) {
      // Update subrect
      icon->rc = tex->rc;

      // Keep existing texture id, but now update texture file and texture
      // coordinates
      vgui::surface()->DrawSetTextureFile(icon->textureId, icon->szTextureFile,
                                          false, false);
  ...
  for (int i = m_Icons.First(); m_Icons.IsValidIndex(i); i = m_Icons.Next(i)) {
    ...
    // Update file
    if (icon->bRenderUsingFont) {
      icon->hFont = vgui::scheme()->GetIScheme(scheme)->GetFont(
          icon->szTextureFile, true);
      icon->rc.top = 0;
      icon->rc.left = 0;
      icon->rc.right = vgui::surface()->GetCharacterWidth(
          icon->hFont, icon->cCharacterInFont);
...
float CHud::GetFOVSensitivityAdjust() { return m_flFOVSensitivityAdjust; }
//-----------------------------------------------------------------------------
...
bool CHud::IsHidden(int iHudFlags) {
  ...
  // Local player dead?
  if ((iHudFlags & HIDEHUD_PLAYERDEAD) &&
      (pPlayer->GetHealth() <= 0 && !pPlayer->IsAlive()))
    return true;
  ...
  // Hide all HUD elements during screenshot if the user's set hud_freezecamhide
  // ( TF2 )
...
//-----------------------------------------------------------------------------
bool CHud::UnlockRenderGroup(int iGroupIndex,
                             CHudElement *pLocker /* = NULL */) {
  // does this index exist?
  if (!DoesRenderGroupExist(iGroupIndex))
    return false;
...
//			Return true if this group is locked, hud elem will be
//hidden
//-----------------------------------------------------------------------------
bool CHud::IsRenderGroupLockedFor(CHudElement *pHudElement, int iGroupIndex) {
  ...
  return (pLocker != pHudElement && pLocker->GetRenderGroupPriority() >
                                        pHudElement->GetRenderGroupPriority());
}
