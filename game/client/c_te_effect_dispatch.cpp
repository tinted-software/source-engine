...
CClientEffectRegistration::CClientEffectRegistration(const char *pEffectName,
                                                     ClientEffectCallback fn) {
...
void DispatchEffectToCallback(const char *pEffectName,
  ...
  for (CClientEffectRegistration *pReg = CClientEffectRegistration::s_pHead;
       pReg; pReg = pReg->m_pNext) {
...
static void RecordEffect(const char *pEffectName, const CEffectData &data) {
  ...
  if (clienttools->IsInRecordingMode() &&
      ((data.m_fFlags & EFFECTDATA_NO_RECORD) == 0)) {
    KeyValues *msg = new KeyValues("TempEntity");

    const char *pSurfacePropName = physprops->GetPropName(data.m_nSurfaceProp);

    char pName[1024];
    Q_snprintf(pName, sizeof(pName), "TE_DispatchEffect %s %s", pEffectName,
               pSurfacePropName);
...
void C_TEEffectDispatch::PostDataUpdate(DataUpdateType_t updateType) {
  ...
  const char *pEffectName =
      g_StringTableEffectDispatch->GetString(m_EffectData.GetEffectNameIndex());
  if (pEffectName) {
...
IMPLEMENT_CLIENTCLASS_EVENT_DT(C_TEEffectDispatch, DT_TEEffectDispatch,
                               CTEEffectDispatch)

RecvPropDataTable(RECVINFO_DT(m_EffectData), 0,
...
void TE_DispatchEffect(IRecipientFilter &filter, float delay,
  ...
  data.m_hEntity =
      (intp)pKeyValues->GetPtr("entindex", (void *)(intp)hWorld.ToInt());
