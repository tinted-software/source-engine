...
RecvPropInt(RECVINFO_NAME(m_nSequence, m_nSequence)),
    ...
    RecvPropInt(RECVINFO_NAME(m_nOrder, m_nOrder)) END_RECV_TABLE()

        const
    char *s_m_iv_AnimOverlayNames[C_BaseAnimatingOverlay::MAX_OVERLAYS] = {
...
void ResizeAnimationLayerCallback(void *pStruct, int offsetToUtlVector,
                                  int len) {
  C_BaseAnimatingOverlay *pEnt = (C_BaseAnimatingOverlay *)pStruct;
  CUtlVector<C_AnimationLayer> *pVec = &pEnt->m_AnimOverlay;
  CUtlVector<CInterpolatedVar<C_AnimationLayer>> *pVecIV =
      &pEnt->m_iv_AnimOverlay;
...
RecvPropUtlVector(RECVINFO_UTLVECTOR_SIZEFN(m_AnimOverlay,
                                            ResizeAnimationLayerCallback),
                  C_BaseAnimatingOverlay::MAX_OVERLAYS,
                  RecvPropDataTable(NULL, 0, 0,
                                    &REFERENCE_RECV_TABLE(DT_Animationlayer)))
    END_RECV_TABLE()

        IMPLEMENT_CLIENTCLASS_DT(C_BaseAnimatingOverlay,
                                 DT_BaseAnimatingOverlay, CBaseAnimatingOverlay)
            RecvPropDataTable("overlay_vars", 0, 0,
                              &REFERENCE_RECV_TABLE(DT_OverlayVars))
                END_RECV_TABLE()

                    BEGIN_PREDICTION_DATA(C_BaseAnimatingOverlay)

    /*
            DEFINE_FIELD( C_BaseAnimatingOverlay, m_Layer[0][2].m_nSequence,
       FIELD_INTEGER ), DEFINE_FIELD( C_BaseAnimatingOverlay,
       m_Layer[0][2].m_flCycle, FIELD_FLOAT ), DEFINE_FIELD(
       C_BaseAnimatingOverlay, m_Layer[0][2].m_flPlaybackRate, FIELD_FLOAT),
            DEFINE_FIELD( C_BaseAnimatingOverlay, m_Layer[0][2].m_flWeight,
       FIELD_FLOAT), DEFINE_FIELD( C_BaseAnimatingOverlay,
       m_Layer[1][2].m_nSequence, FIELD_INTEGER ), DEFINE_FIELD(
       C_BaseAnimatingOverlay, m_Layer[1][2].m_flCycle, FIELD_FLOAT ),
            DEFINE_FIELD( C_BaseAnimatingOverlay,
       m_Layer[1][2].m_flPlaybackRate, FIELD_FLOAT), DEFINE_FIELD(
       C_BaseAnimatingOverlay, m_Layer[1][2].m_flWeight, FIELD_FLOAT),
            DEFINE_FIELD( C_BaseAnimatingOverlay, m_Layer[2][2].m_nSequence,
       FIELD_INTEGER ), DEFINE_FIELD( C_BaseAnimatingOverlay,
       m_Layer[2][2].m_flCycle, FIELD_FLOAT ), DEFINE_FIELD(
       C_BaseAnimatingOverlay, m_Layer[2][2].m_flPlaybackRate, FIELD_FLOAT),
            DEFINE_FIELD( C_BaseAnimatingOverlay, m_Layer[2][2].m_flWeight,
       FIELD_FLOAT), DEFINE_FIELD( C_BaseAnimatingOverlay,
       m_Layer[3][2].m_nSequence, FIELD_INTEGER ), DEFINE_FIELD(
       C_BaseAnimatingOverlay, m_Layer[3][2].m_flCycle, FIELD_FLOAT ),
            DEFINE_FIELD( C_BaseAnimatingOverlay,
       m_Layer[3][2].m_flPlaybackRate, FIELD_FLOAT), DEFINE_FIELD(
    ...
    for (i = 0; i < m_AnimOverlay.Count(); i++) {
      if (m_AnimOverlay[i].m_flWeight > 0.0) {
        ...
        mstudioseqdesc_t &seqdesc =
            pStudioHdr->pSeqdesc(m_AnimOverlay[i].m_nSequence);
...
void C_BaseAnimatingOverlay::CheckForLayerChanges(CStudioHdr *hdr,
                                                  float currentTime) {
  ...
  for (i = 0; i < m_iv_AnimOverlay.Count(); i++) {
    ...
    m_iv_AnimOverlay[i].GetInterpolationInfo(currentTime, &iHead, &iPrev1,
                                             &iPrev2);
    ...
    if (pHead && pPrev1 && pHead->m_nSequence != pPrev1->m_nSequence) {
      ...
      if (/* Q_stristr( hdr->pszName(), r_sequence_debug.GetString()) != NULL ||
        ...
        DevMsgRT("(%7.4f : %30s : %5.3f : %4.2f : %1d)\n", t0,
                 hdr->pSeqdesc(pHead->m_nSequence).pszLabel(),
                 (float)pHead->m_flCycle, (float)pHead->m_flWeight, i);
        DevMsgRT("(%7.4f : %30s : %5.3f : %4.2f : %1d)\n", t1,
                 hdr->pSeqdesc(pPrev1->m_nSequence).pszLabel(),
                 (float)pPrev1->m_flCycle, (float)pPrev1->m_flWeight, i);
        if (pPrev2)
          DevMsgRT("(%7.4f : %30s : %5.3f : %4.2f : %1d)\n", t2,
                   hdr->pSeqdesc(pPrev2->m_nSequence).pszLabel(),
                   (float)pPrev2->m_flCycle, (float)pPrev2->m_flWeight, i);
      }
#endif
      ...
      if (pPrev2) {
        ...
        if (IsSequenceLooping(hdr, pHead->m_nSequence)) {
          flTemp = LoopingLerp(num, (float)pHead->m_flPrevCycle,
                               (float)pHead->m_flCycle);
        } else {
          flTemp =
              Lerp(num, (float)pHead->m_flPrevCycle, (float)pHead->m_flCycle);
        }
      ...
      pHead->nSequence ).pszLabel(), (float)pPrev2->m_flCycle,
      (float)pPrev1->m_flCycle, (float)pHead->m_flCycle );
      }
      */

      m_iv_AnimOverlay[i].SetLooping(
          IsSequenceLooping(hdr, pHead->m_nSequence));
...
void C_BaseAnimatingOverlay::AccumulateLayers(IBoneSetup &boneSetup,
  ...
  for (j = 0; j < MAX_OVERLAYS; j++) {
    ...
    if (i < m_AnimOverlay.Count()) {
      ...
      debugoverlay->AddTextOverlay( GetAbsOrigin() + Vector( 0, 0, 64 ), -j - 1,
      0,
              "%2d(%s) : %6.2f : %6.2f",
      ...
      */
      ...
      if (fWeight > 0) {
        ...
        // if ( m_AnimOverlay[i].m_nSequence != m_iv_AnimOverlay.GetPrev( i
        // )->nSequence )
        float fCycle = m_AnimOverlay[i].m_flCycle;

        fCycle =
            ClampCycle(fCycle, IsSequenceLooping(m_AnimOverlay[i].m_nSequence));

        if (fWeight > 1)
          fWeight = 1;

        boneSetup.AccumulatePose(pos, q, m_AnimOverlay[i].m_nSequence, fCycle,
                                 fWeight, currentTime, m_pIk);
          ...
          if (1) {
            DevMsgRT("%8.4f : %30s : %5.3f : %4.2f : %1d\n", currentTime,
                     boneSetup.GetStudioHdr()
                         ->pSeqdesc(m_AnimOverlay[i].m_nSequence)
                         .pszLabel(),
                     fCycle, fWeight, i);
          } else {
            int iHead, iPrev1, iPrev2;
            m_iv_AnimOverlay[i].GetInterpolationInfo(currentTime, &iHead,
                                                     &iPrev1, &iPrev2);

            // fake up previous cycle values.
            float t0;
            C_AnimationLayer *pHead =
                m_iv_AnimOverlay[i].GetHistoryValue(iHead, t0);
            // reset previous
            float t1;
            C_AnimationLayer *pPrev1 =
                m_iv_AnimOverlay[i].GetHistoryValue(iPrev1, t1);
            // reset previous previous
            float t2;
            C_AnimationLayer *pPrev2 =
                m_iv_AnimOverlay[i].GetHistoryValue(iPrev2, t2);

            if (pHead && pPrev1 && pPrev2) {
              DevMsgRT("%6.2f : %30s %6.2f (%6.2f:%6.2f:%6.2f) : %6.2f "
                       "(%6.2f:%6.2f:%6.2f) : %1d\n",
                       currentTime,
                       boneSetup.GetStudioHdr()
                           ->pSeqdesc(m_AnimOverlay[i].m_nSequence)
                           .pszLabel(),
                       fCycle, (float)pPrev2->m_flCycle,
                       (float)pPrev1->m_flCycle, (float)pHead->m_flCycle,
                       fWeight, (float)pPrev2->m_flWeight,
...
void C_BaseAnimatingOverlay::DoAnimationEvents(CStudioHdr *pStudioHdr) {
  ...
  for (j = 0; j < m_AnimOverlay.Count(); j++) {
    ...
    mstudioseqdesc_t &seqdesc =
        pStudioHdr->pSeqdesc(m_AnimOverlay[j].m_nSequence);
    ...
    // in client code, m_flOverlayPrevEventCycle is set to -1 when we first
    // start an overlay, looping or not
      ...
      for (int i = 0; i < (int)seqdesc.numevents; i++) {
        ...
        if (watch) {
          Msg("%i FE %i Looped cycle %f, prev %f ev %f (time %.3f)\n",
              ...
              m_flOverlayPrevEventCycle[j], (float)m_AnimOverlay[j].m_flCycle,
              gpGlobals->curtime);
        }
    ...
    for (int i = 0; i < (int)seqdesc.numevents; i++) {
      ...
      if ((pevent[i].cycle > m_flOverlayPrevEventCycle[j] &&
           pevent[i].cycle <= m_AnimOverlay[j].m_flCycle)) {
        if (watch) {
          Msg("%i (seq: %d) FE %i Normal cycle %f, prev %f ev %f (time %.3f)\n",
              gpGlobals->tickcount, m_AnimOverlay[j].m_nSequence.GetRaw(),
              pevent[i].event, pevent[i].cycle, m_flOverlayPrevEventCycle[j],
              (float)m_AnimOverlay[j].m_flCycle, gpGlobals->curtime);
        }
