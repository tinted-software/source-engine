...
public:
  ...
  CUtlVector<CSimplePhysics::CNode>
      m_Nodes; // This is m_nHairs * m_nNodesPerHair large.
  CUtlVector<Vector> m_HairPositions; // Untransformed base hair positions,
                                      // distributed on the sphere.
  CUtlVector<Vector>
      m_TransformedHairPositions; // Transformed base hair positions,
                                  // distributed on the sphere.
...
void C_Hairball::CHairballDelegate::GetNodeForces(CSimplePhysics::CNode *pNodes,
                                                  int iNode, Vector *pAccel) {
  pAccel->Init(0, 0, -1500);
}

void C_Hairball::CHairballDelegate::ApplyConstraints(
  ...
  for (int iIteration = 0; iIteration < nIterations; iIteration++) {
    for (int iHair = 0; iHair < m_pParent->m_nHairs; iHair++) {
      CSimplePhysics::CNode *pBase =
          &pNodes[iHair * m_pParent->m_nNodesPerHair];
...
C_Hairball::C_Hairball() {
  ...
  // Distribute on the sphere (need a better random distribution for the
  // sphere).
  for (int i = 0; i < m_HairPositions.Count(); i++) {
    ...
    m_HairPositions[i].Init(cos(theta) * cosPhi * m_flSphereRadius,
...
void C_Hairball::ClientThink() {
  ...
  // m_vecAngles = SetAbsAngles( pPlayer->GetAbsAngles() ); // copy player
  // angles.
  ...
  if (m_flSitStillTime > 0) {
    ...
    if (m_flSitStillTime <= 0) {
      ...
      for (int i = 0; i < 15; i++) {
        ...
        trace_t trace;
        UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + vDir * 10000,
                       MASK_SOLID, NULL, COLLISION_GROUP_NONE, &trace);
    ...
    trace_t trace;
    UTIL_TraceLine(GetAbsOrigin(), vEnd, MASK_SOLID, NULL, COLLISION_GROUP_NONE,
                   &trace);
  ...
  for (int i = 0; i < m_HairPositions.Count(); i++) {
    Vector3DMultiplyPosition(mTransform, m_HairPositions[i],
                             m_TransformedHairPositions[i]);
  }
  ...
  m_Physics.Simulate(m_Nodes.Base(), m_Nodes.Count(), &m_Delegate,
                     gpGlobals->frametime, 0.98);
}
...
void CreateHairballCallback() {
  for (int i = 0; i < 20; i++) {
    ...
    pHairball->SetLocalOrigin(pPlayer->GetAbsOrigin() + vForward * 300 +
                              RandomVector(0, 100));
  }
}

ConCommand cc_CreateHairball("CreateHairball", CreateHairballCallback, 0,
                             FCVAR_CHEAT);
