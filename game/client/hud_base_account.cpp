...
CHudBaseAccount::CHudBaseAccount(const char *pName)
  ...
  SetIndent(false); // don't indent small numbers in the drawing code - we're
                    // doing it manually
}
...
void CHudBaseAccount::Paint() {
  ...
  if (m_iPreviousAccount != account) {
    ...
    m_flLastAnimationEnd = gpGlobals->curtime +
                           GetAnimationController()->GetAnimationSequenceLength(
                               m_pszLastAnimationName);
  ...
  if (m_pAccountIcon) {
    m_pAccountIcon->DrawSelf(icon_xpos, icon_ypos, icon_wide, icon_tall,
                             GetFgColor());
  }
  ...
  if (m_iPreviousDelta < 0) {
    if (m_pMinusIcon) {
      m_pMinusIcon->DrawSelf(icon2_xpos, icon2_ypos, icon_wide, icon_tall,
                             m_Ammo2Color);
    }
  } else {
    if (m_pPlusIcon) {
      m_pPlusIcon->DrawSelf(icon2_xpos, icon2_ypos, icon_wide, icon_tall,
                            m_Ammo2Color);
    }
  }
