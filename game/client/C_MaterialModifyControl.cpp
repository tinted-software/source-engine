...
ConVar debug_materialmodifycontrol_client("debug_materialmodifycontrol_client",
                                          "0");
...
public:
  ...
  void SetAnimationStartTime(float flTime) { m_flAnimationStartTime = flTime; }
  float GetAnimationStartTime(void) const { return m_flAnimationStartTime; }
...
IMPLEMENT_CLIENTCLASS_DT(C_MaterialModifyControl, DT_MaterialModifyControl,
...
RecvPropString(RECVINFO(m_szMaterialName)),
    ...
    RecvPropInt(RECVINFO(m_iFrameStart)), RecvPropInt(RECVINFO(m_iFrameEnd)),
    RecvPropInt(RECVINFO(m_bWrap)), RecvPropFloat(RECVINFO(m_flFramerate)),
...
void C_MaterialModifyControl::OnDataChanged(DataUpdateType_t updateType) {
  if (updateType == DATA_UPDATE_CREATED) {
    m_pMaterial =
        materials->FindMaterial(m_szMaterialName, TEXTURE_GROUP_OTHER);
...
// Purpose:
//-----------------------------------------------------------------------------
void C_MaterialModifyControl::GetAnimationCommands(
    materialanimcommands_t *pCommands) {
...
// Purpose:
//-----------------------------------------------------------------------------
void C_MaterialModifyControl::GetFloatLerpCommands(
    materialfloatlerpcommands_t *pCommands) {
...
void CMaterialModifyProxy::OnBind(void *pEntity) {
  ...
  if (pRend) {
    ...
    if (pBaseEntity) {
      ...
      int numChildren = 0;
      bool gotOne = false;
      for (C_BaseEntity *pChild = pBaseEntity->FirstMoveChild(); pChild;
           pChild = pChild->NextMovePeer()) {
        numChildren++;
        C_MaterialModifyControl *pControl =
            dynamic_cast<C_MaterialModifyControl *>(pChild);
        if (!pControl)
          continue;
...
void CMaterialModifyProxy::OnBindSetVar(C_MaterialModifyControl *pControl) {
  ...
  IMaterialVar *pMaterialVar =
      pMaterial->FindVar(pControl->GetMaterialVariableName(), &bFound, false);
  if (!bFound)
    return;
  ...
  if (Q_strcmp(pControl->GetMaterialVariableValue(), "")) {
    ...
    //		const char *pMaterialVarValue =
    //pControl->GetMaterialVariableValue(); 		if(
    //debug_materialmodifycontrol_client.GetBool()
    //			&& Q_stristr( m_pMaterial->GetName(), "faceandhair" )
    //			&& Q_stristr( pMaterialVar->GetName(), "self" )
    //			)
    //		{
    //			static int count = 0;
    //			DevMsg( 1, "CMaterialModifyProxy::OnBindSetVar \"%s\"
    //%s=%s %d pControl=0x%p\n", 				m_pMaterial->GetName(),
    //pMaterialVar->GetName(), pControl->GetMaterialVariableValue(), count++,
    //pControl );
    //		}
...
// Does the dirty deed
//-----------------------------------------------------------------------------
void CMaterialModifyProxy::OnBindAnimatedTexture(
    C_MaterialModifyControl *pControl) {
...
void CMaterialModifyProxy::OnBindFloatLerp(C_MaterialModifyControl *pControl) {
  ...
  bool bFound;
  m_pMaterialVar =
      m_pMaterial->FindVar(pControl->GetMaterialVariableName(), &bFound, false);
  ...
  if (bFound) {
    ...
    if (m_flTransitionTime > 0.0f) {
      currentValue =
          m_flStartValue +
          (m_flEndValue - m_flStartValue) *
              clamp(((gpGlobals->curtime - m_flStartTime) / m_flTransitionTime),
                    0.0f, 1.0f);
    } else {
    ...
    if (debug_materialmodifycontrol_client.GetBool() &&
      ...
      DevMsg(1, "CMaterialFloatLerpProxy::OnBind \"%s\" %s=%f %d\n",
             m_pMaterial->GetName(), m_pMaterialVar->GetName(), currentValue,
             count++);
    }
...
// Purpose:
//-----------------------------------------------------------------------------
bool CMaterialModifyAnimatedProxy::Init(IMaterial *pMaterial,
...
EXPOSE_INTERFACE(CMaterialModifyProxy, IMaterialProxy,
                 "MaterialModify" IMATERIAL_PROXY_INTERFACE_VERSION);
