...
void CHudMenu::OnThink() {
  ...
  if (m_bMenuDisplayed &&
      (gpGlobals->curtime - m_flSelectionTime > flSelectionTimeout)) {
    m_bMenuDisplayed = false;
  }
}
...
//			y -
//-----------------------------------------------------------------------------
void CHudMenu::PaintString(const wchar_t *text, int textlen, vgui::HFont &font,
                           int x, int y) {
...
void CHudMenu::Paint() {
  ...
  DrawBox(x - border / 2, y - border / 2, wide, tall, m_BoxColor,
          m_flSelectionAlphaOverride / 255.0f);

  // DrawTexturedBox( x - border/2, y - border/2, wide, tall, m_BoxColor,
  // m_flSelectionAlphaOverride / 255.0f );
  ...
  for (int i = 0; i < c; i++) {
    ...
    vgui::surface()->DrawSetTextFont(line->menuitem != 0 ? m_hItemFont
                                                         : m_hTextFont);
...
void CHudMenu::ProcessText(void) {
  ...
  for (i = 0; i < c; i++) {
    ...
    for (int ch = 0; ch < l->length; ch++) {
      pixels += vgui::surface()->GetCharacterWidth(
          font, g_szMenuString[ch + l->startchar]);
    }
...
void CHudMenu::ShowMenu(const char *menuName, int validSlots) {
  ...
  Q_strncpy(g_szPrelocalisedMenuString, menuName,
  ...
  Q_strncpy(szMenuString,
            ConvertCRtoNL(hudtextmessage->BufferedLocaliseTextString(
...
void CHudMenu::ShowMenu_KeyValueItems(KeyValues *pKV) {
  ...
  int i = 0;
  for (KeyValues *item = pKV->GetFirstSubKey(); item != NULL;
...
//		char : the duration, in seconds, the menu should stay up. -1
//means is stays until something is chosen. 		byte : a boolean, TRUE if there is
//more string yet to be received before displaying the menu, false if it's the
//last string 		string: menu string to display
//  if this message is never received, then scores will simply be the combined
...
void CHudMenu::MsgFunc_ShowMenu(bf_read &msg) {
  ...
  if (m_bitsValidSlots) {
    ...
    {
      Q_strncpy(g_szPrelocalisedMenuString, szString,
                sizeof(g_szPrelocalisedMenuString));
    } else { // append to the current menu string
      Q_strncat(g_szPrelocalisedMenuString, szString,
                sizeof(g_szPrelocalisedMenuString), COPY_ALL_CHARACTERS);
    }
