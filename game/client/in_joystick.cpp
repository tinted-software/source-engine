...
static ConVar
    joy_response_move("joy_response_move", "1", FCVAR_ARCHIVE,
                      "'Movement' stick response mode: 0=Linear, 1=quadratic, "
                      "2=cubic, 3=quadratic extreme, 4=power function(i.e., "
                      "pow(x,1/sensitivity)), 5=two-stage");
...
// Misc
static ConVar
    joy_diagonalpov("joy_diagonalpov", "0", FCVAR_ARCHIVE,
...
ConVar
    joy_autosprint("joy_autosprint", "0", 0,
                   "Automatically sprint when moving with an analog joystick");

static ConVar
    joy_inverty("joy_inverty", "0", FCVAR_ARCHIVE | FCVAR_ARCHIVE_XBOX,
...
static ConVar option_duck_method_default("option_duck_method_default", "1.0",
                                         FCVAR_NONE);
static ConVar
    joy_inverty_default("joy_inverty_default", "0",
...
static ConVar
    joy_movement_stick("joy_movement_stick", "0",
                       FCVAR_ARCHIVE | FCVAR_ARCHIVE_XBOX,
                       "Which stick controls movement (0 is left stick)",
                       joy_movement_stick_Callback);

static ConVar
    joy_xcontroller_cfg_loaded("joy_xcontroller_cfg_loaded", "0", FCVAR_ARCHIVE,
                               "If 0, the 360controller.cfg file will be "
...
static float ResponseCurve(int curve, float x, int axis, float sensitivity) {
  ...
  case 6: // Custom for driving a vehicle!
  {
    if (axis == YAW) {
      ...
      // which is used for turning in the car. We fall-through and use a linear
      // curve on the PITCH axis, which is the vehicle's throttle. REALLY, these
      // are the 'forward' and 'side' axes, but we don't have constants for
      // those, so we re-use the same axis convention as the look stick. (sjb)
      float sign = 1;
    ...
    // else
    //	fall through and just return x*sensitivity below (as if using default
    //curve)
  }
  }
...
ConVar joy_accel_filter("joy_accel_filter",
                        "0.2"); // If the non-accelerated axis is pushed farther
                                // than this, then accelerate it, too.
...
void CInput::JoyStickMove(float frametime, CUserCmd *cmd) {
  ...
  // Reinitialize the 'advanced joystick' system if hotplugging has caused us
  // toggle between some/none joysticks.
  ...
  if ((in_strafe.state & 1) ||
      (lookstrafe.GetFloat() && (in_jlook.state & 1))) {
  ...
  m_flPreviousJoystickSide =
      ScaleAxisValue(gameAxes[GAME_AXIS_SIDE].value,
  ...
  m_flPreviousJoystickYaw =
      ScaleAxisValue(gameAxes[GAME_AXIS_YAW].value,
  ...
  if (CAM_IsThirdPerson() && thirdperson_platformer.GetInt()) {
    if (m_flPreviousJoystickForward || m_flPreviousJoystickSide) {
      ...
      viewangles[YAW] = RAD2DEG(atan2(-m_flPreviousJoystickSide,
                                      -m_flPreviousJoystickForward)) +
    ...
    if (m_flPreviousJoystickPitch || m_flPreviousJoystickYaw) {
      ...
      vTempOffset[PITCH] +=
          m_flPreviousJoystickPitch * joy_pitchsensitivity.GetFloat();
      vTempOffset[YAW] +=
          m_flPreviousJoystickYaw * joy_yawsensitivity.GetFloat();

      g_ThirdPersonManager.SetCameraOffsetAngles(vTempOffset);
    }

    if (m_flPreviousJoystickForward || m_flPreviousJoystickSide ||
        m_flPreviousJoystickPitch || m_flPreviousJoystickYaw) {
      Vector vTempOffset = g_ThirdPersonManager.GetCameraOffsetAngles();
  ...
  float val = ResponseCurve(iResponseCurve, m_flPreviousJoystickForward, PITCH,
                            joy_forwardsensitivity.GetFloat());
  joyForwardMove += val * cl_forwardspeed.GetFloat();
  val = ResponseCurve(iResponseCurve, m_flPreviousJoystickSide, YAW,
                      joy_sidesensitivity.GetFloat());
  ...
  if (JOY_ABSOLUTE_AXIS == gameAxes[GAME_AXIS_YAW].controlType) {
    float fAxisValue =
        ResponseCurveLook(joy_response_look.GetInt(), m_flPreviousJoystickYaw,
                          YAW, m_flPreviousJoystickPitch, dist, frametime);
    angle = fAxisValue * joy_yawsensitivity.GetFloat() * aspeed *
            cl_yawspeed.GetFloat();
  } else {
    angle = m_flPreviousJoystickYaw * joy_yawsensitivity.GetFloat() * aspeed *
            180.0;
  }
  ...
  if (IsX360() || in_jlook.state & 1) {
    ...
    if (JOY_ABSOLUTE_AXIS == gameAxes[GAME_AXIS_PITCH].controlType) {
      float fAxisValue = ResponseCurveLook(
          joy_response_look.GetInt(), m_flPreviousJoystickPitch, PITCH,
          m_flPreviousJoystickYaw, dist, frametime);
      angle = fAxisValue * joy_pitchsensitivity.GetFloat() * aspeed *
              cl_pitchspeed.GetFloat();
    } else {
      angle = m_flPreviousJoystickPitch * joy_pitchsensitivity.GetFloat() *
              aspeed * 180.0;
    }
