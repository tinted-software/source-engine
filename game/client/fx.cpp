...
void FX_MuzzleEffect(const Vector &origin, const QAngle &angles, float scale,
  ...
  origin );
          ...
          pParticle->m_flLifetime		= 0.0f;
          pParticle->m_flDieTime		= random->RandomFloat(
  0.5f, 1.0f );
...
//			This is an emitter that keeps spitting out particles for
//its lifetime 			It won't create particles if it's not in view, so it's best when
//short lived
//-----------------------------------------------------------------------------
...
void FX_GunshipImpact(const Vector &pos, const Vector &normal, float r, float g,
  ...
  if (CImpactOverlay *pOverlay = new CImpactOverlay) {
    ...
    VectorMA(pos, 1.0f, normal, pOverlay->m_vPos); // Doesn't show up on terrain
                                                   // if you don't do this(sjb)
