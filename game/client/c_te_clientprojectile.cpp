...
void TE_ClientProjectile(IRecipientFilter &filter, float delay,
  ...
  tempents->ClientProjectile(*vecOrigin, *vecVelocity, vec3_origin, modelindex,
                             lifetime, pOwner);
}
...
void C_TEClientProjectile::PostDataUpdate(DataUpdateType_t updateType) {
  ...
  tempents->ClientProjectile(m_vecOrigin, m_vecVelocity, vec3_origin,
                             m_nModelIndex, m_nLifeTime, m_hOwner);
}

IMPLEMENT_CLIENTCLASS_EVENT_DT(C_TEClientProjectile, DT_TEClientProjectile,
                               CTEClientProjectile)
RecvPropVector(RECVINFO(m_vecOrigin)), RecvPropVector(RECVINFO(m_vecVelocity)),
    RecvPropInt(RECVINFO(m_nModelIndex)), RecvPropInt(RECVINFO(m_nLifeTime)),
    RecvPropEHandle(RECVINFO(m_hOwner)), END_RECV_TABLE()
