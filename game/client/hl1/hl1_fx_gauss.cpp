...
void HL1GaussReflect(const CEffectData &data) {
  ...
  tempents->TempSprite(vecStart, vec3_origin, 0.2, m_nGlowIndex, kRenderGlow,
                       kRenderFxNoDissipation, flMagnitude / 255.0,
                       flMagnitude * 0.05, FTENT_FADEOUT);

  Vector vecForward;
  VectorAdd(vecStart, vecNormal, vecForward);

  tempents->Sprite_Trail(vecStart, vecForward, m_nGlowIndex, 3, 0.1,
                         random->RandomFloat(0.1, 0.2), 100, 255, 100);
}
...
void HL1GaussWallPunchExit(const CEffectData &data) {
  ...
  tempents->TempSprite(vecStart, vec3_origin, 0.1, m_nGlowIndex, kRenderGlow,
                       kRenderFxNoDissipation, flMagnitude * 1.2 / 255.0, 6.0,
                       FTENT_FADEOUT);

  Vector vecForward;
  VectorSubtract(vecStart, vecNormal, vecForward);

  tempents->Sprite_Trail(vecStart, vecForward, m_nGlowIndex, flMagnitude * 0.3,
                         0.1, random->RandomFloat(0.1, 0.2), 200, 255, 40);
}
...
void HL1GaussWallImpact1(const CEffectData &data) {
  ...
  tempents->TempSprite(vecStart, vec3_origin, 1, m_nGlowIndex, kRenderGlow,
                       kRenderFxNoDissipation, flMagnitude / 255.0, 6.0,
                       FTENT_FADEOUT);
}
...
void HL1GaussWallImpact2(const CEffectData &data) {
  ...
  tempents->TempSprite(vecStart, vec3_origin, 0.2, m_nGlowIndex, kRenderGlow,
                       kRenderFxNoDissipation, 240.0 / 255.0, 0.3,
                       FTENT_FADEOUT);

  Vector vecForward;
  VectorAdd(vecStart, vecNormal, vecForward);

  tempents->Sprite_Trail(vecStart, vecForward, m_nGlowIndex, 8, 0.6,
                         random->RandomFloat(0.1, 0.2), 100, 255, 200);
}
