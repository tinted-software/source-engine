...
RecvPropEHandle(RECVINFO(m_hPlayer)), RecvPropInt(RECVINFO(m_iEvent)),
    ...
    //	RecvPropDataTable( RECVINFO_DT( m_Shared ), 0, &REFERENCE_RECV_TABLE(
    //DT_TFCPlayerShared ) )
    END_RECV_TABLE()

        BEGIN_PREDICTION_DATA(C_HL1MP_Player) END_PREDICTION_DATA()
...
void C_HL1MPRagdoll::ImpactTrace(trace_t *pTrace, int iDamageType,
...
void C_HL1MP_Player::CalcView(Vector &eyeOrigin, QAngle &eyeAngles,
...
//-----------------------------------------------------------------------------
void C_HL1MPRagdoll::SetupWeights(const matrix3x4_t *pBoneToWorld,
