...
void GetBloodColorHL1(int bloodtype, unsigned char &r, unsigned char &g,
                      unsigned char &b) {
...
public:
  static C_HL1Gib *CreateClientsideGib(const char *pszModelName,
    ...
    if (pGib->InitializeGib(pszModelName, vecOrigin, vecForceDir,
                            vecAngularImp) == false)
      return NULL;
  ...
  virtual void HitSurface(C_BaseEntity *pOther) {
    ...
    if (index >= 0) {
      effects->DecalShoot(index, pOther->entindex(), pOther->GetModel(),
                          pOther->GetAbsOrigin(), pOther->GetAbsAngles(),
                          GetAbsOrigin(), 0, 0);
    }
...
// Input  : &origin -
//-----------------------------------------------------------------------------
void FX_HL1Gib(const Vector &origin, const Vector &direction, float scale,
               int iType, int iHealth, int iColor) {
  Vector offset;
  int i;
  ...
  if (iType == HUMAN_GIBS) {
    ...
    C_HL1Gib *pGib = C_HL1Gib::CreateClientsideGib(pHumanGibsModel, offset,
                                                   vVelocity * 2, aImpulse);
  ...
  for (i = 0; i < MAX_GIBS; i++) {
    ...
    C_HL1Gib *pGib = C_HL1Gib::CreateClientsideGib(pModelName, offset,
                                                   vVelocity * 2, aAImpulse);
  ...
  for (i = 0; i < 4; i++) {
    ...
    GetBloodColorHL1(iColor, sParticle->m_uchColor[0], sParticle->m_uchColor[1],
                     sParticle->m_uchColor[2]);
  ...
  for (i = 0; i < 4; i++) {
    SimpleParticle *sParticle = (SimpleParticle *)pSimple->AddParticle(
        sizeof(SimpleParticle), g_Mat_BloodPuff[1], origin);
...
void HL1GibCallback(const CEffectData &data) {
  FX_HL1Gib(data.m_vOrigin, data.m_vNormal, data.m_flScale, data.m_nMaterial,
            -data.m_nHitBox, data.m_nColor);
}
