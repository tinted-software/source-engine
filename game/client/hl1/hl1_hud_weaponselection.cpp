...
public:
  ...
  C_BaseCombatWeapon *GetSelectedWeapon(void) { return m_hSelectedWeapon; }
...
private:
  ...
  void DrawAmmoBar(C_BaseCombatWeapon *pWeapon, int x, int y, int nWidth,
                   int nHeight);
  ...
  for (int i = 0; i < HL1_MAX_WEAPON_SLOTS; i++) {
    ...
    if (i == iActiveSlot) {
        ...
        if (pWeapon->GetSpriteActive()) {
          ...
          if (pWeapon == pSelectedWeapon) {
            ...
            if (pWeapon->GetSpriteInactive()) {
              pWeapon->GetSpriteInactive()->DrawSelf(xpos, ypos,
                                                     Color(r, g, b, a));
            }
          }
        }
      ...
      for (int iPos = 0; iPos < MAX_WEAPON_POSITIONS; iPos++) {
        ...
        vgui::surface()->DrawFilledRect(xpos, ypos,
                                        xpos + icon_buckets[i]->Width(),
...
void CHudWeaponSelection::DrawAmmoBar(C_BaseCombatWeapon *pWeapon, int x, int y,
  ...
  if (pWeapon->GetPrimaryAmmoType() != -1) {
    ...
    float flPct = (float)nAmmoCount /
                  (float)GetAmmoDef()->MaxCarry(pWeapon->GetPrimaryAmmoType());
...
int CHudWeaponSelection::DrawBar(int x, int y, int nWidth, int nHeight,
                                 float flPct) {
  Color clrBar;
  int r, g, b, a;
...
void CHudWeaponSelection::CycleToPrevWeapon(void) {
  ...
  if (!pNextWeapon) {
    ...
    pNextWeapon = FindPrevWeaponInWeaponSelection(HL1_MAX_WEAPON_SLOTS,
