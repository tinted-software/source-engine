...
CHudAmmo::CHudAmmo(const char *pElementName)
    : CHudElement(pElementName), BaseClass(NULL, "HudAmmo") {
...
void CHudAmmo::Paint(void) {
  ...
  hudlcd->SetGlobalStat("(weapon_print_name)",
                        pActiveWeapon ? pActiveWeapon->GetPrintName() : " ");
  hudlcd->SetGlobalStat("(weapon_name)",
                        pActiveWeapon ? pActiveWeapon->GetName() : " ");

  if ((pActiveWeapon->GetPrimaryAmmoType() == -1) &&
      (pActiveWeapon->GetSecondaryAmmoType() == -1))
    return;
  ...
  if (pActiveWeapon->GetPrimaryAmmoType() != -1) {
    CHudTexture *icon_ammo =
        gWR.GetAmmoIconFromWeapon(pActiveWeapon->GetPrimaryAmmoType());
    ...
    if (pActiveWeapon->UsesClipsForAmmo1()) {
      ...
      x = DrawHudNumber(
          x, y, pPlayer->GetAmmoCount(pActiveWeapon->GetPrimaryAmmoType()),
          clrAmmo);
    } else {
    ...
    hudlcd->SetGlobalStat(
        "(ammo_primary)",
        VarArgs("%d",
                pPlayer->GetAmmoCount(pActiveWeapon->GetPrimaryAmmoType())));
  } else {
    hudlcd->SetGlobalStat("(ammo_primary)", "n/a");
  }
  ...
  if (pActiveWeapon->GetSecondaryAmmoType() != -1) {
    CHudTexture *icon_ammo =
        gWR.GetAmmoIconFromWeapon(pActiveWeapon->GetSecondaryAmmoType());
    ...
    if (pPlayer->GetAmmoCount(pActiveWeapon->GetSecondaryAmmoType()) > 0) {
      ...
      x = nHudElemWidth - 4 * nFontWidth - nIconWidth;
      x = DrawHudNumber(
          x, y, pPlayer->GetAmmoCount(pActiveWeapon->GetSecondaryAmmoType()),
          clrAmmo);
    ...
    hudlcd->SetGlobalStat(
        "(ammo_secondary)",
        VarArgs("%d",
                pPlayer->GetAmmoCount(pActiveWeapon->GetSecondaryAmmoType())));
  } else {
    hudlcd->SetGlobalStat("(ammo_secondary)", "n/a");
  }
}
