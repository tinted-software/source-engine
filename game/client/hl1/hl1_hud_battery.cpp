...
CHudBattery::CHudBattery(const char *pElementName)
    : CHudElement(pElementName), BaseClass(NULL, "HudSuit") {
...
void CHudBattery::Paint() {
  ...
  if (m_iBattery > 0) {
    int nSuitOffset =
        icon_suit_full->Height() *
        ((float)(100 - (MIN(100, m_iBattery))) *
         0.01); // battery can go from 0 to 100 so * 0.01 goes from 0 to 1
    icon_suit_full->DrawSelfCropped(
        x, y - iOffset + nSuitOffset, 0, nSuitOffset, icon_suit_full->Width(),
