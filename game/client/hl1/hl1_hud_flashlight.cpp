...
void CHudFlashlight::Paint(void) {
  ...
  int nOffset = icon_flash_empty->Width() *
                (1.0 - ((float)pPlayer->m_nFlashBattery / 100.0));
  if (nOffset < icon_flash_empty->Width()) {
    icon_flash_full->DrawSelfCropped(x + nOffset, y, nOffset, 0,
                                     icon_flash_full->Width() - nOffset,
