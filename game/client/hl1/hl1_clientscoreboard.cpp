...
// Purpose: Konstructor
//-----------------------------------------------------------------------------
CHL1MPClientScoreBoardDialog::CHL1MPClientScoreBoardDialog(IViewPort *pViewPort)
...
#ifdef WIN32
        _snwprintf(name, ARRAYSIZE(name), L"%s",
                   g_pVGuiLocalize->Find("#ScoreBoard_Deathmatch"));
#else
        _snwprintf(name, ARRAYSIZE(name), L"%S",
                   g_pVGuiLocalize->Find("#ScoreBoard_Deathmatch"));
#endif
...
void CHL1MPClientScoreBoardDialog::AddHeader() {
  ...
  m_pPlayerList->AddColumnToSection(
      0, "name", "", 0,
      scheme()->GetProportionalScaledValue(CSTRIKE_NAME_WIDTH));
  m_pPlayerList->AddColumnToSection(
      0, "class", "", 0,
      scheme()->GetProportionalScaledValue(CSTRIKE_CLASS_WIDTH));
  m_pPlayerList->AddColumnToSection(
      0, "frags", "#PlayerScore", 0 | SectionedListPanel::COLUMN_RIGHT,
      scheme()->GetProportionalScaledValue(CSTRIKE_SCORE_WIDTH));
  m_pPlayerList->AddColumnToSection(
      0, "deaths", "#PlayerDeath", 0 | SectionedListPanel::COLUMN_RIGHT,
      scheme()->GetProportionalScaledValue(CSTRIKE_DEATH_WIDTH));
  m_pPlayerList->AddColumnToSection(
      0, "ping", "#PlayerPing", 0 | SectionedListPanel::COLUMN_RIGHT,
      scheme()->GetProportionalScaledValue(CSTRIKE_PING_WIDTH));
  //	m_pPlayerList->AddColumnToSection(0, "voice", "#PlayerVoice",
  //SectionedListPanel::COLUMN_IMAGE | SectionedListPanel::HEADER_TEXT|
  //SectionedListPanel::COLUMN_CENTER, scheme()->GetProportionalScaledValue(
  //CSTRIKE_VOICE_WIDTH ) ); 	m_pPlayerList->AddColumnToSection(0, "tracker",
  //"#PlayerTracker", SectionedListPanel::COLUMN_IMAGE |
  //SectionedListPanel::HEADER_TEXT, scheme()->GetProportionalScaledValue(
  //CSTRIKE_FRIENDS_WIDTH ) );
}
...
void CHL1MPClientScoreBoardDialog::AddSection(int teamType, int teamNumber) {
  ...
  if (teamType == TYPE_TEAM) {
    ...
    m_pPlayerList->AddColumnToSection(
        ...
        scheme()->GetProportionalScaledValue(CSTRIKE_CLASS_WIDTH));
    m_pPlayerList->AddColumnToSection(
        sectionID, "frags", "", SectionedListPanel::COLUMN_RIGHT,
        scheme()->GetProportionalScaledValue(CSTRIKE_SCORE_WIDTH));
    m_pPlayerList->AddColumnToSection(
        sectionID, "deaths", "", SectionedListPanel::COLUMN_RIGHT,
        scheme()->GetProportionalScaledValue(CSTRIKE_DEATH_WIDTH));
    m_pPlayerList->AddColumnToSection(
        sectionID, "ping", "", SectionedListPanel::COLUMN_RIGHT,
        scheme()->GetProportionalScaledValue(CSTRIKE_PING_WIDTH));

    // set the section to have the team color
    if (teamNumber) {
      if (GameResources())
        m_pPlayerList->SetSectionFgColor(
            sectionID, GameResources()->GetTeamColor(teamNumber));
    }
    ...
    m_pPlayerList->AddColumnToSection(
        sectionID, "name", "#Spectators", 0,
        scheme()->GetProportionalScaledValue(CSTRIKE_NAME_WIDTH));
    m_pPlayerList->AddColumnToSection(
        sectionID, "class", "", 0, scheme()->GetProportionalScaledValue(100));
  }
}
...
enum { MAX_PLAYERS_PER_TEAM = 16, MAX_SCOREBOARD_PLAYERS = 32 };
  ...
  for (i = 1; i <= gpGlobals->maxClients; i++) {
    ...
    if (shouldShow) {
      ...
      m_pPlayerList->SetItemFgColor(itemID,
                                    g_PR->GetTeamColor(g_PR->GetTeam(i)));
