...
void CHudHistoryResource::AddToHistory(C_BaseCombatWeapon *weapon) {
  ...
  // from the bottom
  if (((HISTORY_PICKUP_GAP * m_iCurrentHistorySlot) +
  ...
  for (int i = 0; i < m_PickupHistory.Count(); i++) {
    if (m_PickupHistory[i].type) {
      m_PickupHistory[i].DisplayTime =
          MIN(m_PickupHistory[i].DisplayTime,
              gpGlobals->curtime + hud_drawhistory_time.GetFloat());
      if (m_PickupHistory[i].DisplayTime <=
          gpGlobals->curtime) { // pic drawing time has expired
      ...
      case HISTSLOT_AMMO: {
        ...
        if (icon) {
          // Draw the pic
          int ypos = nHudElemHeight -
                     (HISTORY_PICKUP_PICK_HEIGHT + (HISTORY_PICKUP_GAP * i));
          ...
          vgui::surface()->DrawSetTextPos(
              GetWide() - (ITEM_GUTTER_SIZE * 0.85f), ypos);

          if (m_PickupHistory[i].iCount) {
            char sz[32];
            int len =
                Q_snprintf(sz, sizeof(sz), "%i", m_PickupHistory[i].iCount);
