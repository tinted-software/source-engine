...
// Purpose:
//-----------------------------------------------------------------------------
CCommentaryModelPanel::CCommentaryModelPanel(vgui::Panel *parent,
                                             const char *name)
...
void CCommentaryModelViewer::ApplySchemeSettings(vgui::IScheme *pScheme) {
  ...
  m_pModelPanel =
      dynamic_cast<CCommentaryModelPanel *>(FindChildByName("modelpanel"));
}
...
// Purpose:
//-----------------------------------------------------------------------------
void CCommentaryModelViewer::SetModel(const char *pszName,
                                      const char *pszAttached) {
  if (!m_pModelPanel)
    return;
...
void CCommentaryModelViewer::HandleMovementInput(void) {
  ...
  if (m_flYawSpeed != 0) {
    if (m_bTranslating) {
      m_pModelPanel->m_pModelInfo->m_vecOriginOffset.y =
          clamp(m_pModelPanel->m_pModelInfo->m_vecOriginOffset.y + m_flYawSpeed,
                -100.f, 100.f);
    } else {
      m_pModelPanel->m_pModelInfo->m_vecAbsAngles.y = anglemod(
          m_pModelPanel->m_pModelInfo->m_vecAbsAngles.y + m_flYawSpeed);
    }

    if (!bLeftDown && !bRightDown) {
      m_flYawSpeed = (m_flYawSpeed > 0) ? MAX(0, m_flYawSpeed - 0.1)
                                        : MIN(0, m_flYawSpeed + 0.1);
    }
  }
  ...
  if (m_flZoomSpeed != 0) {
    if (m_bTranslating) {
      m_pModelPanel->m_pModelInfo->m_vecOriginOffset.z = clamp(
          m_pModelPanel->m_pModelInfo->m_vecOriginOffset.z + m_flZoomSpeed,
          -100.f, 300.f);
    } else {
      float flZoomMin = m_pModelPanel->m_flFrameDistance * 0.75;
      float flZoomMax = m_pModelPanel->m_flFrameDistance * 1.5;
      m_pModelPanel->m_pModelInfo->m_vecOriginOffset.x = clamp(
          m_pModelPanel->m_pModelInfo->m_vecOriginOffset.x + m_flZoomSpeed,
          flZoomMin, flZoomMax);
    }

    if (!bForwardDown && !bBackDown) {
      m_flZoomSpeed = (m_flZoomSpeed > 0) ? MAX(0, m_flZoomSpeed - 0.1)
                                          : MIN(0, m_flZoomSpeed + 0.1);
    }
  }
}
...
void CommentaryShowModelViewer(const CCommand &args) {
  if (args.ArgC() < 2) {
    ConMsg("Usage: commentary_showmodelviewer <model name> <optional attached "
           "model name>\n");
    return;
  }

  CBaseViewport *pViewport =
      dynamic_cast<CBaseViewport *>(g_pClientMode->GetViewport());
  if (pViewport) {
    IViewPortPanel *pCommentaryPanel =
        pViewport->FindPanelByName(PANEL_COMMENTARY_MODELVIEWER);
    if (!pCommentaryPanel) {
      pCommentaryPanel =
          pViewport->CreatePanelByName(PANEL_COMMENTARY_MODELVIEWER);
      pViewport->AddNewPanel(pCommentaryPanel, "PANEL_COMMENTARY_MODELVIEWER");
    }

    if (pCommentaryPanel) {
      //((CCommentaryModelViewer*)pCommentaryPanel)->InvalidateLayout( false,
      //true );
...
ConCommand commentary_showmodelviewer(
    "commentary_showmodelviewer", CommentaryShowModelViewer,
    "Display the commentary model viewer. Usage: commentary_showmodelviewer "
    "<model name> <optional attached model name>",
    FCVAR_NONE);
