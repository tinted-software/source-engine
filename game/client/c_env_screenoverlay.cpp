...
IMPLEMENT_CLIENTCLASS_DT(C_EnvScreenOverlay, DT_EnvScreenOverlay,
                         CEnvScreenOverlay)
RecvPropArray(RecvPropString(RECVINFO(m_iszOverlayNames[0])),
              m_iszOverlayNames),
    RecvPropArray(RecvPropFloat(RECVINFO(m_flOverlayTimes[0])),
                  m_flOverlayTimes),
...
void C_EnvScreenOverlay::PostDataUpdate(DataUpdateType_t updateType) {
  ...
  if (m_bIsActive && m_flStartTime > 0 &&
  ...
  if (updateType == DATA_UPDATE_CREATED &&
    ...
    for (int i = 0; i < MAX_SCREEN_OVERLAYS; ++i) {
      if (m_iszOverlayNames[i] && m_iszOverlayNames[i][0]) {
        materials->FindMaterial(m_iszOverlayNames[i],
...
void C_EnvScreenOverlay::StartCurrentOverlay(void) {
  if (m_iCurrentOverlay == MAX_SCREEN_OVERLAYS ||
  ...
  if (m_flOverlayTimes[m_iCurrentOverlay] == -1)
    ...
    m_flCurrentOverlayTime =
        gpGlobals->curtime + m_flOverlayTimes[m_iCurrentOverlay];

  // Bring up the current overlay
  IMaterial *pMaterial =
      materials->FindMaterial(m_iszOverlayNames[m_iCurrentOverlay],
                              TEXTURE_GROUP_CLIENT_EFFECTS, false);
...
void C_EnvScreenOverlay::ClientThink(void) {
  ...
  if (m_flCurrentOverlayTime != -1 &&
      m_flCurrentOverlayTime < gpGlobals->curtime) {
    m_iCurrentOverlay++;
    StartCurrentOverlay();
  }
}
...
IMPLEMENT_CLIENTCLASS_DT(C_EnvScreenEffect, DT_EnvScreenEffect,
                         CEnvScreenEffect)
RecvPropFloat(RECVINFO(m_flDuration)), RecvPropInt(RECVINFO(m_nType)),
  ...
  case 0: // FIXME: Declare
  {
    ...
    if (m_nType == SCREENEFFECT_EP1_INTRO) {
      ...
      g_pScreenSpaceEffects->SetScreenSpaceEffectParams("episodic_intro",
                                                        pKeys);
