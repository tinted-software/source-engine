  ...
  while (pParticle) {
    ...
    if (pParticle->IsDead()) {
      ...
      m_ParticleCollision.MoveParticle(
          pParticle->m_Pos, pParticle->m_vecVelocity, &pParticle->m_flAngSpeed,
          timeDelta, &trace);
...
void CSimple3DEmitter::RenderParticles(CParticleRenderIterator *pIterator) {
  ...
  while (pParticle) {
    float sortKey =
        CurrentViewForward().Dot(CurrentViewOrigin() - pParticle->m_Pos);
    ...
    RenderParticle_ColorSizeAngles(pIterator->GetParticleDraw(),
                                   pParticle->m_Pos, color,
                                   pParticle->GetFadeFraction(),
                                   pParticle->m_uchSize, pParticle->m_vAngles);
