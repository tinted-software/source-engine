...
BitmapImage::~BitmapImage() { DestroyTexture(); }
...
void BitmapImage::SetColor(const Color &clr) { m_clr = clr; }
...
void BitmapImage::SetBitmap(const Bitmap_t &bitmap) {
  ...
  vgui::surface()->DrawSetTextureRGBA(m_nTextureId, bitmap.GetBits(),
                                      bitmap.Width(), bitmap.Height(), 1, true);
