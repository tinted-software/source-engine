...
void TrackerDescriptionChanged(IConVar *var, const char *pOldString,
                               float flOldValue) {
...
ConVar hud_achievement_description(
    "hud_achievement_description", "1", FCVAR_ARCHIVE,
    "Show full descriptions of achievements on the HUD",
    TrackerDescriptionChanged);
#ifdef CSTRIKE_DLL
ConVar hud_achievement_count(
    "hud_achievement_count", "5", FCVAR_ARCHIVE,
    "Max number of achievements that can be shown on the HUD");
#else
ConVar hud_achievement_count(
    "hud_achievement_count", "8", FCVAR_ARCHIVE,
    "Max number of achievements that can be shown on the HUD");
#endif
ConVar hud_achievement_glowtime(
    "hud_achievement_glowtime", "2.5", FCVAR_NONE,
    "Duration of glow effect around incremented achievements");
ConVar hud_achievement_tracker("hud_achievement_tracker", "1", FCVAR_NONE,
                               "Show or hide the achievement tracker");
...
bool CHudBaseAchievementTracker::ShouldShowAchievement(
    IAchievement *pAchievement) {
  return (hud_achievement_tracker.GetBool() && pAchievement &&
          pAchievement->ShouldShowOnHUD() && !pAchievement->IsAchieved());
}
...
//-----------------------------------------------------------------------------
// Purpose: create panels for each achievement the player wants shown on the HUD
// and assign achievements to each one
//-----------------------------------------------------------------------------
void CHudBaseAchievementTracker::UpdateAchievementItems() {
  ...
  for (int i = 0; i < iCount; ++i) {
    ...
    if (!ShouldShowAchievement(pCur)) {
      // don't remove achievements that are still glowing (typically a just
      // completed achievement)
      if (pCur && m_AchievementItem.Count() > iShown &&
          m_AchievementItem[iShown]->GetAchievementID() ==
              pCur->GetAchievementID() &&
...
CHudBaseAchievementTracker::GetAchievementPanel(int i) {
...
CAchievementTrackerItem::CAchievementTrackerItem(vgui::Panel *pParent,
...
void CAchievementTrackerItem::PerformLayout() {
  ...
  m_pAchievementName->GetContentSize(
      w, t); // needed in order to load up font for the name
  m_pAchievementNameGlow->GetContentSize(
      w, t); // needed in order to load up font for the glow
...
void CAchievementTrackerItem::UpdateAchievementDisplay() {
  ...
  CBaseAchievement *pAchievement =
      pAchievementMgr->GetAchievementByID(m_iAchievementID);
  if (!pAchievement)
    return;
  ...
  if (m_iAccumulatedIncrement > 0 &&
      gpGlobals->curtime > m_flShowIncrementsTime) {
    ShowAccumulatedIncrements();
  }

  if (pAchievement->GetCount() != m_iLastProgressBarCount ||
      pAchievement->GetGoal() != m_iLastProgressBarGoal) {
    // need to update progress bar
    float flProgress =
        float(pAchievement->GetCount()) / float(pAchievement->GetGoal());
    int x, y, w, t;
...
void CAchievementTrackerItem::AchievementIncremented(int iNewCount) {
  ...
  if (m_iLastProgressBarGoal > 1500) {
    // for achievements with very high counts, accumulate increments so we don't
    // have too many +1s on screen
    //  also don't play sounds as these achievements tend to increment
    //  constantly
    ...
    if (m_pProgressBarBackground->IsVisible()) {
      ...
      m_pProgressBarBackground->GetBounds(x, y, w, t);
      x += w;
      new CFloatingAchievementNumber(iIncrement, px + x, py + y + (t * 0.5f),
                                     FN_DIR_RIGHT, GetParent());
    }
...
void CAchievementTrackerItem::ShowAccumulatedIncrements() {
  ...
  m_pProgressBarBackground->GetBounds(x, y, w, t);
  x += w;
  new CFloatingAchievementNumber(m_iAccumulatedIncrement, px + x,
                                 py + y + (t * 0.5f), FN_DIR_RIGHT,
                                 GetParent());
...
//-----------------------------------------------------------------------------
// Purpose: Floating numbers showing how much achievement progress bars have
// gone up
//-----------------------------------------------------------------------------

CFloatingAchievementNumber::CFloatingAchievementNumber(
    int iProgress, int x, int y, floating_number_directions iDir,
    vgui::Panel *pParent)
...
void CFloatingAchievementNumber::ApplySchemeSettings(vgui::IScheme *scheme) {
  ...
  GetAnimationController()->RunAnimationCommand(
      m_pNumberLabel, "alpha", 255, 0, 0.3,
  ...
  case FN_DIR_UP:
    vgui::GetAnimationController()->RunAnimationCommand(
        this, "ypos", m_iStartY - m_iScrollDistance, 0, 2.0f,
        vgui::AnimationController::INTERPOLATOR_LINEAR);
    break;
  case FN_DIR_DOWN:
    vgui::GetAnimationController()->RunAnimationCommand(
  ...
  case FN_DIR_LEFT:
    vgui::GetAnimationController()->RunAnimationCommand(
        this, "xpos", m_iStartX - m_iScrollDistance, 0, 2.0f,
        vgui::AnimationController::INTERPOLATOR_LINEAR);
    break;
  case FN_DIR_RIGHT:
    vgui::GetAnimationController()->RunAnimationCommand(
...
void CFloatingAchievementNumber::OnThink() {
  if (gpGlobals->curtime > m_fStartTime + 1.0f) {
    if (m_pNumberLabel->GetAlpha() >= 255) {
      ...
      GetAnimationController()->RunAnimationCommand(
          m_pNumberLabel, "alpha", 0, 0.0, 1.0f,
          vgui::AnimationController::INTERPOLATOR_LINEAR);
...
//-----------------------------------------------------------------------------
// Purpose: Debug command to make one of the achievement panels flash as though
// it just went up
//-----------------------------------------------------------------------------
class CHudAchievementTracker;
void cc_TrackerAnim_f(const CCommand &args) {
  CHudBaseAchievementTracker *pTracker =
      (CHudBaseAchievementTracker *)GET_HUDELEMENT(CHudAchievementTracker);
  if (!pTracker)
    return;
...
ConCommand cc_TrackerAnim("TrackerAnim", cc_TrackerAnim_f,
                          "Test animation of the achievement tracker. "
                          "Parameter is achievement number on HUD to flash",
                          FCVAR_NONE);
