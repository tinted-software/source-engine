...
bool CGrid::SetColumnWidth(int iColumn, int width) {
  AssertCheck(iColumn >= 0 && iColumn < m_xCols,
              "CGrid::SetColumnWidth : invalid location specified");
...
bool CGrid::SetRowHeight(int iRow, int height) {
  AssertCheck(iRow >= 0 && iRow < m_yRows,
              "CGrid::SetColumnWidth : invalid location specified");
...
int CGrid::GetColumnWidth(int iColumn) {
  AssertCheck(iColumn >= 0 && iColumn < m_xCols,
              "CGrid::GetColumnWidth: invalid location specified");
  return m_Widths[iColumn];
}

int CGrid::GetRowHeight(int iRow) {
  AssertCheck(iRow >= 0 && iRow < m_yRows,
              "CGrid::GetRowHeight: invalid location specified");
  return m_Heights[iRow];
}

int CGrid::CalcFitColumnWidth(int iColumn) {
  AssertCheck(iColumn >= 0 && iColumn < m_xCols,
              "CGrid::CalcFitColumnWidth: invalid location specified");
...
int CGrid::CalcFitRowHeight(int iRow) {
  AssertCheck(iRow >= 0 && iRow < m_yRows,
              "CGrid::CalcFitRowHeight: invalid location specified");
...
void CGrid::RepositionContents() {
  for (int x = 0; x < m_xCols; x++) {
    for (int y = 0; y < m_yRows; y++) {
      ...
      pPanel->SetBounds(m_ColOffsets[x], m_RowOffsets[y], m_Widths[x],
                        m_Heights[y]);
    }
  }
  ...
  for (int row = 0; row < m_yRows; row++) {
    ...
    if (cell->m_bUnderline) {
      vgui::surface()->DrawSetColor(
          cell->m_UnderlineColor[0], cell->m_UnderlineColor[1],
          cell->m_UnderlineColor[2], cell->m_UnderlineColor[3]);
      vgui::surface()->DrawFilledRect(x, y - (cell->m_iUnderlineOffset + 1),
                                      GetWide(), y - cell->m_iUnderlineOffset);
    }
  }
}
...
//-----------------------------------------------------------------------------
void CGrid::SetRowUnderline(int row, bool enabled, int offset, int r, int g,
                            int b, int a) {
  CGridEntry *cell = GridEntry(0, row);
...
CGrid::CGridEntry *CGrid::GridEntry(int x, int y) {
  AssertCheck(x >= 0 && x < m_xCols && y >= 0 && y < m_yRows,
              "CGrid::GridEntry: invalid location specified");
