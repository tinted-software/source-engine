...
void CNewParticleEffect::DebugDrawBbox(bool bCulled) {
  ...
  if (bDraw) {
    debugoverlay->AddBoxOverlay(center, mins, maxs, QAngle(0, 0, 0), r, g, b,
                                16, 0);
...
int CNewParticleEffect::DrawModel(int flags) {
  ...
  if (CalcSqrDistanceToAABB(m_MinBounds, m_MaxBounds, vecCamera) >
    ...
    if (!IsRetail() &&
        (g_cl_particle_show_bbox || (g_cl_particle_show_bbox_cost != 0))) {
      DebugDrawBbox(true);
    }

    // Still need to make sure we set this or they won't follow their attachemnt
    // points.
    m_flNextSleepTime =
  ...
  if (flags & STUDIO_TRANSPARENCY) {
    ...
    if (pCameraObject && (m_pDef->m_nSkipRenderControlPoint != -1) &&
      ...
      C_BaseEntity *pEntity =
          (EHANDLE)GetControlPointEntity(m_pDef->m_nSkipRenderControlPoint);
      if (pEntity) {
