...
//			flags -
//-----------------------------------------------------------------------------
void FX_BloodSpray(const Vector &origin, const Vector &normal, float scale,
                   unsigned char r, unsigned char g, unsigned char b,
                   int flags) {
  if (UTIL_IsLowViolence())
    return;

  // debugoverlay->AddLineOverlay( origin, origin + normal * 72, 255, 255, 255,
  // true, 10 );
  ...
  Vector color = Vector((float)(worldLight[0] * r) / 255.0f,
                        (float)(worldLight[1] * g) / 255.0f,
  ...
  if (flags & FX_BLOODSPRAY_DROPS) {
    ...
    CSmartPtr<CTrailParticles> pTrailEmitter =
        CTrailParticles::Create("blooddrops");
    if (!pTrailEmitter)
      return;
    ...
    for (i = 0; i < 14; i++) {
      ...
      tParticle = (TrailParticle *)pTrailEmitter->AddParticle(
      ...
      tParticle->m_vecVelocity =
          offDir * random->RandomFloat(4.0f * scale, 40.0f * scale);
    ...
    for (i = 0; i < 24; i++) {
      ...
      tParticle->m_vecVelocity =
          offDir * random->RandomFloat(2.0f * scale, 25.0f * scale);
      tParticle->m_vecVelocity[2] += random->RandomFloat(4.0f, 16.0f) * scale;
  ...
  if ((flags & FX_BLOODSPRAY_GORE) || (flags & FX_BLOODSPRAY_CLOUD)) {
    CSmartPtr<CBloodSprayEmitter> pSimple =
        CBloodSprayEmitter::Create("bloodgore");
    if (!pSimple)
      return;
    ...
    if (flags & FX_BLOODSPRAY_GORE) {
      ...
      for (i = 0; i < 6; i++) {
        ...
        pParticle = (SimpleParticle *)pSimple->AddParticle(
            sizeof(SimpleParticle), hMaterial, offset);
      ...
      for (i = 0; i < 6; i++) {
        ...
        if (pParticle != NULL) {
          ...
          pParticle->m_uchStartSize =
              random->RandomFloat(scale * 1.5f, scale * 2.0f);
  ...
  // C_BaseEntity::EmitSound( filter, SOUND_FROM_WORLD, CHAN_VOICE,
  // "Physics.WaterSplash", 1.0, ATTN_NORM, 0, 100, &origin );
}
...
//-----------------------------------------------------------------------------
void FX_BloodBulletImpact(const Vector &origin, const Vector &normal,
                          float scale /*NOTE: Unused!*/, unsigned char r,
  ...
  pSimple->GetBinding().SetBBox(origin - Vector(16, 16, 16),
                                origin + Vector(16, 16, 16));
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (pParticle != NULL) {
      ...
      pParticle->m_vecVelocity =
          dir * random->RandomFloat(16.0f, 32.0f) * (i + 1);
      pParticle->m_vecVelocity[2] -=
          random->RandomFloat(32.0f, 64.0f) * (i + 1);
  ...
  for (int i = 0; i < 8; i++) {
    ...
    tParticle = (TrailParticle *)pTrailEmitter->AddParticle(
        sizeof(TrailParticle), g_Blood_Drops, offset);
...
// FIXME: This will be simplified when the initializer can take color parameters
// as an input
...
void BloodSprayCallback(const CEffectData &data) {
  ...
  FX_BloodSpray(data.m_vOrigin, data.m_vNormal, data.m_flScale, color.r,
                color.g, color.b, data.m_fFlags);
}
...
void BloodImpactCallback(const CEffectData &data) {
  ...
  for (int i = 0; i < ARRAYSIZE(bloodCallbacks); i++) {
    if (bloodCallbacks[i].nColor == data.m_nColor) {
      ...
      DispatchParticleEffect(bloodCallbacks[i].lpszParticleSystemName,
                             data.m_vOrigin, vecAngles);
      bFoundBlood = true;
      break;
    }
  }
  ...
  if (bFoundBlood == false) {
    ...
    FX_BloodBulletImpact(vecPosition, data.m_vNormal, data.m_flScale, color.r,
                         color.g, color.b);
  }
}
...
void HunterDamageCallback(const CEffectData &data) {
  CSmartPtr<CSimple3DEmitter> pGlassEmitter =
      CSimple3DEmitter::Create("HunterDamage");
  ...
  pGlassEmitter->m_ParticleCollision.Setup(data.m_vOrigin, &data.m_vNormal,
                                           flAngularSpray, flMinSpeed,
                                           flMaxSpeed, 600.0f, 0.2f);
  ...
  for (int i = 0; i < numFlecks; i++) {
    ...
    dir[0] = data.m_vNormal[0] +
             random->RandomFloat(-flAngularSpray, flAngularSpray);
    dir[1] = data.m_vNormal[1] +
             random->RandomFloat(-flAngularSpray, flAngularSpray);
    dir[2] = data.m_vNormal[2] +
             random->RandomFloat(-flAngularSpray, flAngularSpray);
