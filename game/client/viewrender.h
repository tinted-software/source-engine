...
struct IntroDataBlendPass_t {
  int m_BlendMode;
  float m_Alpha; // in [0.0f,1.0f]  This needs to add up to 1.0 for all passes,
                 // unless you are fading out.
};
...
struct ViewCustomVisibility_t {
  ...
  void AddVisOrigin(const Vector &origin) {
    ...
    // If the vis origin count is greater than the size of our array, just fail
    // to add this origin
  ...
  // The starting leaf to determing which area to start in when performing area
  // portal culling on the engine Default behavior is to use the leaf the camera
  // position is in.
...
protected:
  ...
  // More concise version of the above BuildRenderableRenderLists().  Called for
  ...
  // leaves
  void DrawTranslucentWorldAndDetailPropsInLeaves(int iCurLeaf, int iFinalLeaf,
                                                  int nEngineDrawFlags,
...
protected:
  ...
  // This stores all of the view setup parameters that the engine needs to know
  // about.
...
private:
  ...
  void ViewDrawScene(bool bDrew3dSkybox, SkyboxVisibility_t nSkyboxVisible,
                     const CViewSetup &view, int nClearFlags, view_id_t viewID,
                     bool bDrawViewModel = false, int baseDrawFlags = 0,
                     ViewCustomVisibility_t *pCustomVisibility = NULL);
  ...
  // stencils to be drawn after opaques but before translucents are drawn in the
  // main view.
  ...
  // VIS Overrides
  // Set to true to turn off client side vis ( !!!! rendering will be slow since
  // everything will draw )
  bool m_bForceNoVis;
  ...
  int m_BaseDrawFlags; // Set in ViewDrawScene and OR'd into m_DrawFlags as it
                       // goes.
