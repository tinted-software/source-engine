...
//			functions for VR including head tracking,
//window/viewport information, 			rendering information, and distortion
//
//=============================================================================
...
enum HeadtrackMovementMode_t {
  ...
  HMM_SHOOTFACE_MOVETORSO,    // Shoot from your face, move the direction your
                              // torso is facing.
  HMM_SHOOTMOUSE_MOVEFACE, // Shoot from the mouse cursor which moves within the
                           // HUD, move along your face.
  HMM_SHOOTBOUNDEDMOUSE_LOOKFACE_MOVEFACE,  // Shoot from the mouse cursor which
                                            // moves, bounded within the HUD,
                                            // move along your face.
  HMM_SHOOTBOUNDEDMOUSE_LOOKFACE_MOVEMOUSE, // Shoot from the mouse cursor which
                                            // moves, bounded within the HUD,
                                            // move along your weapon (the
                                            // "mouse")
  ...
  HMM_SHOOTMOVELOOKMOUSEFACE,  // Shoot & move & look along the mouse cursor
                               // (i.e. original unchanged gameplay), face just
                               // looks on top of that.
  HMM_SHOOTMOVEMOUSE_LOOKFACE, // Shoot & move along the mouse cursor (i.e.
                               // original unchanged gameplay), face just looks.
  HMM_SHOOTMOVELOOKMOUSE, // Shoot, move and look along the mouse cursor - HMD
                          // orientation is completely ignored!

  HMM_LAST,

  HMM_NOOVERRIDE =
      HMM_LAST // Used as a retrun from ShouldOverrideHeadtrackControl(), not an
               // actual mode.
};
...
private:
  ...
  // Where the current mideye is relative to the zero (torso) (currently always
  // the same as m_MideyeZeroFromMideyeCurrent!)
  VMatrix m_TorsoFromMideye;

  // The debug cam will play with the above, but some things want the non-debug
  // view.
  ...
  // The amount of zoom to apply to the view of the world (but NOT to the HUD!).
  // Used for sniper weapons, etc.
