...
static void PixelvisDrawChanged(IConVar *pPixelvisVar, const char *pOld,
                                float flOldValue);

ConVar r_pixelvisibility_partial("r_pixelvisibility_partial", "1");
ConVar r_dopixelvisibility("r_dopixelvisibility", "1");
ConVar r_drawpixelvisibility("r_drawpixelvisibility", "0", 0,
                             "Show the occlusion proxies", PixelvisDrawChanged);
ConVar r_pixelvisibility_spew("r_pixelvisibility_spew", "0");

#ifdef OSX
// GLMgr will set this one to "1" if it senses the new post-10.6.4 driver
// (m_hasPerfPackage1)
ConVar gl_can_query_fast("gl_can_query_fast", "0");

static bool HasFastQueries(void) { return gl_can_query_fast.GetBool(); }
#else
// non OSX path
static bool HasFastQueries(void) { return true; }
#endif
...
float PixelVisibility_DrawProxy(IMatRenderContext *pRenderContext,
                                OcclusionQueryObjectHandle_t queryHandle,
                                Vector origin, float scale, float proxyAspect,
                                IMaterial *pMaterial, bool screenspace) {
  Vector point;

  // don't expand this with distance to fit pixels or the sprite will poke
  // through only expand the parts perpendicular to the view
  float forwardScale = scale;
  // draw a pyramid of points touching a sphere of radius "scale" at origin
  float pixelsPerUnit =
      pRenderContext->ComputePixelDiameterOfSphere(origin, 1.0f);
  pixelsPerUnit = MAX(pixelsPerUnit, 1e-4f);
  if (screenspace) {
    // Force this to be the size of a sphere of diameter "scale" at some
    // reference distance (1.0 unit)
    ...
    // make the radius larger to ensure a minimum screen space size of the proxy
    // geometry
  ...
  Vector verts[5];
  const float sqrt2 =
      0.707106781f; // sqrt(2) - keeps all vectors the same length from origin
  scale *= sqrt2;
  float scale45x = scale;
  float scale45y = scale / proxyAspect;
  verts[0] =
      origin - CurrentViewForward() * forwardScale; // the apex of the pyramid
  verts[1] = origin + CurrentViewUp() * scale45y -
             CurrentViewRight() * scale45x; // these four form the base
  verts[2] = origin + CurrentViewUp() * scale45y +
             CurrentViewRight() *
                 scale45x; // the pyramid is a sprite with a point that
  verts[3] = origin - CurrentViewUp() * scale45y +
             CurrentViewRight() *
                 scale45x; // pokes back toward the camera through any nearby
  verts[4] = origin - CurrentViewUp() * scale45y -
             CurrentViewRight() * scale45x; // geometry
...
public:
  ...
  void IssueQuery(IMatRenderContext *pRenderContext, float proxySize,
                  float proxyAspect, IMaterial *pMaterial,
                  bool sizeIsScreenSpace);
  void IssueCountingQuery(IMatRenderContext *pRenderContext, float proxySize,
                          float proxyAspect, IMaterial *pMaterial,
                          bool sizeIsScreenSpace);
...
private:
  ...
  friend void PixelVisibility_ShiftVisibilityViews(
      int iSourceViewID, int iDestViewID); // need direct access to private data
                                           // to make shifting smooth
};
...
void CPixelVisibilityQuery::ResetOcclusionQueries() {
  // NOTE: Since we're keeping the CPixelVisibilityQuery objects around in a
  // pool and not actually deleting them, this means that our material system
  // occlusion queries are not being deleted either. Which means that if a
  // CPixelVisibilityQuery is put into the free list and then immediately
  // re-used, then we have an opportunity for a bug: What can happen on the
  // first frame of the material system query is that if the query isn't done
  // yet, it will use the last queried value which will happen to be set to the
  // value of the last query done for the previous CPixelVisSet the
  // CPixelVisibilityQuery happened to be associated with which makes queries
  // have an invalid value for the first frame

  // This will mark the occlusion query objects as not ever having been read
  // from before
  CMatRenderContextPtr pRenderContext(materials);
  if (m_queryHandle != INVALID_OCCLUSION_QUERY_OBJECT_HANDLE) {
...
bool CPixelVisibilityQuery::IsValid() {
  return (m_queryHandle != INVALID_OCCLUSION_QUERY_OBJECT_HANDLE) ? true
                                                                  : false;
}
...
float CPixelVisibilityQuery::GetFractionVisible(float fadeTimeInv) {
  ...
  if (!m_wasQueriedThisFrame) {
    ...
    if (r_pixelvisibility_partial.GetBool()) {
      if (m_frameIssued != -1) {
        pixelsPossible = pRenderContext->OcclusionQuery_GetNumPixelsRendered(
            m_queryHandleCount);
        pixels =
            pRenderContext->OcclusionQuery_GetNumPixelsRendered(m_queryHandle);
      }

      if (r_pixelvisibility_spew.GetBool() && CurrentViewID() == 0) {
        DevMsg(1,
               "Pixels visible: %d (qh:%d) Pixels possible: %d (qh:%d) "
               "(frame:%d)\n",
               pixels, (int)(intp)m_queryHandle, pixelsPossible,
               (int)(intp)m_queryHandleCount, gpGlobals->framecount);
      }
      ...
      if (pixelsPossible > 0) {
        ...
        m_brightnessTarget =
            Approach(target, m_brightnessTarget, rate); // fade in / out
      } else {
        m_brightnessTarget = 0.0f;
      }
    } else {
      ...
      if (r_pixelvisibility_spew.GetBool() && CurrentViewID() == 0) {
        DevMsg(1, "Pixels visible: %d (qh:%d) (frame:%d)\n", pixels,
               (int)(intp)m_queryHandle, gpGlobals->framecount);
      }
...
void CPixelVisibilityQuery::IssueQuery(IMatRenderContext *pRenderContext,
                                       float proxySize, float proxyAspect,
                                       IMaterial *pMaterial,
                                       bool sizeIsScreenSpace) {
  if (!m_failed) {
    Assert(IsValid());
    ...
    if (r_pixelvisibility_spew.GetBool() && CurrentViewID() == 0) {
      DevMsg(1, "Draw Proxy: qh:%d org:<%d,%d,%d> (frame:%d)\n",
             (int)(intp)m_queryHandle, (int)m_origin[0], (int)m_origin[1],
             (int)m_origin[2], gpGlobals->framecount);
    }

    m_clipFraction = PixelVisibility_DrawProxy(pRenderContext, m_queryHandle,
                                               m_origin, proxySize, proxyAspect,
                                               pMaterial, sizeIsScreenSpace);
    if (m_clipFraction < 0) {
      // NOTE: In this case, the proxy wasn't issued cause it was offscreen
      // can't set the m_frameissued field since that would cause it to get
      // marked as failed
      m_clipFraction = 0;
      m_wasQueriedThisFrame = false;
      m_failed = false;
...
#ifndef PORTAL // FIXME: In portal we query visibility multiple times per frame
               // because of portal renders!
...
void CPixelVisibilityQuery::IssueCountingQuery(
    IMatRenderContext *pRenderContext, float proxySize, float proxyAspect,
...
#endif
    PixelVisibility_DrawProxy(pRenderContext, m_queryHandleCount, m_origin,
                              proxySize, proxyAspect, pMaterial,
                              sizeIsScreenSpace);
  }
}
...
public:
  ...
  float GetFractionVisible(const pixelvis_queryparams_t &params,
  ...
  CPixelVisSet *FindOrCreatePixelVisSet(const pixelvis_queryparams_t &params,
                                        pixelvis_handle_t *queryHandle);
  bool SupportsOcclusion() { return m_hwCanTestGlows; }
  void DebugInfo() {
    Msg("Pixel vis system using %d sets total (%d in free list), %d queries "
        "total (%d in free list)\n",
        m_setList.TotalCount(), m_setList.Count(m_freeSetsList),
        m_queryList.TotalCount(), m_queryList.Count(m_freeQueriesList));
  }
...
void CPixelVisibilitySystem::LevelInitPreEntity() {
  ...
  m_hwCanTestGlows = r_dopixelvisibility.GetBool() && fastqueries &&
                     engine->GetDXSupportLevel() >= 80;
  if (m_hwCanTestGlows) {
    CMatRenderContextPtr pRenderContext(materials);

    OcclusionQueryObjectHandle_t query =
        pRenderContext->CreateOcclusionQueryObject();
  ...
  m_pProxyMaterial = materials->FindMaterial("engine/occlusionproxy",
                                             TEXTURE_GROUP_CLIENT_EFFECTS);
  m_pProxyMaterial->IncrementReferenceCount();
  m_pDrawMaterial = materials->FindMaterial("engine/occlusionproxy_countdraw",
                                            TEXTURE_GROUP_CLIENT_EFFECTS);
...
float CPixelVisibilitySystem::GetFractionVisible(
...
void CPixelVisibilitySystem::EndView() {
  ...
  if (r_pixelvisibility_partial.GetBool() && !m_drawQueries) {
    ...
    while (node != m_setList.InvalidIndex()) {
      ...
      if (queryNode != m_queryList.InvalidIndex()) {
        m_queryList[queryNode].IssueCountingQuery(
            pRenderContext, pSet->proxySize, pSet->proxyAspect, pProxy,
...

  {
    ...
    while (node != m_setList.InvalidIndex()) {
      ...
      unsigned short queryNode = FindQueryForView(pSet, CurrentViewID());
      if (queryNode != m_queryList.InvalidIndex()) {
        m_queryList[queryNode].IssueQuery(pRenderContext, pSet->proxySize,
                                          pSet->proxyAspect, pProxy,
                                          pSet->sizeIsScreenSpace);
      }
      node = m_setList.Next(node);
    }
...
void CPixelVisibilitySystem::EndScene() { DeleteUnusedSets(false); }

unsigned short CPixelVisibilitySystem::FindQueryForView(CPixelVisSet *pSet,
                                                        int viewID) {
...
unsigned short
CPixelVisibilitySystem::FindOrCreateQueryForView(CPixelVisSet *pSet,
                                                 int viewID) {
...
void CPixelVisibilitySystem::DeleteUnusedQueries(CPixelVisSet *pSet,
                                                 bool bDeleteAll) {
  unsigned short node = m_queryList.Head(pSet->queryList);
  while (node != m_queryList.InvalidIndex()) {
...
void CPixelVisibilitySystem::ShowQueries(bool show) { m_drawQueries = show; }
...
CPixelVisSet *CPixelVisibilitySystem::FindOrCreatePixelVisSet(
    const pixelvis_queryparams_t &params, pixelvis_handle_t *queryHandle) {
  if (queryHandle[0]) {
...
void PixelvisDrawChanged(IConVar *pPixelvisVar, const char *pOld,
                         float flOldValue) {
...
public:
  ...
  virtual bool ShouldHitEntity(IHandleEntity *pHandleEntity, int contentsMask) {
    ...
    if (pCollide->GetSolid() != SOLID_VPHYSICS &&
        pCollide->GetSolid() != SOLID_BSP)
      return false;
...
float GlowSightDistance(const Vector &glowOrigin, bool bShouldTrace) {
  ...
  if (bShouldTrace) {
    ...
    // HACKHACK: trace 4" from destination in case the glow is inside some
    // parent object
...
void PixelVisibility_EndCurrentView() { g_PixelVisibilitySystem.EndView(); }
...
bool PixelVisibility_IsAvailable() {
  bool fastqueries = HasFastQueries();
  return r_dopixelvisibility.GetBool() && fastqueries &&
         g_PixelVisibilitySystem.SupportsOcclusion();
}

// this originally called a class function of CPixelVisibiltySystem to keep the
// work clean, but that function needed friend access to CPixelVisibilityQuery
// and I didn't want to make the whole class a friend or shift all the functions
// and class declarations around in this file
void PixelVisibility_ShiftVisibilityViews(int iSourceViewID, int iDestViewID) {
  unsigned short node = g_PixelVisibilitySystem.m_setList.Head(
      g_PixelVisibilitySystem.m_activeSetsList);
  while (node != g_PixelVisibilitySystem.m_setList.InvalidIndex()) {
    ...
    unsigned short iSourceQueryNode =
        g_PixelVisibilitySystem.FindQueryForView(pSet, iSourceViewID);
    unsigned short iDestQueryNode =
        g_PixelVisibilitySystem.FindQueryForView(pSet, iDestViewID);

    if (iDestQueryNode != g_PixelVisibilitySystem.m_queryList.InvalidIndex()) {
      // delete the destination if found
      g_PixelVisibilitySystem.m_queryList.Unlink(pSet->queryList,
                                                 iDestQueryNode);
      g_PixelVisibilitySystem.m_queryList.LinkToHead(
          g_PixelVisibilitySystem.m_freeQueriesList, iDestQueryNode);
    ...
    if (iSourceQueryNode !=
      ...
      g_PixelVisibilitySystem.m_queryList[iSourceQueryNode].m_viewID =
          iDestViewID;
    }
