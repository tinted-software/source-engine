...
private:
  friend void RecvProxy_FishOriginX(const CRecvProxyData *pData, void *pStruct,
                                    void *pOut);
  friend void RecvProxy_FishOriginY(const CRecvProxyData *pData, void *pStruct,
                                    void *pOut);
...
//-----------------------------------------------------------------------------
void RecvProxy_FishOriginX(const CRecvProxyData *pData, void *pStruct,
                           void *pOut) {
...
void RecvProxy_FishOriginY(const CRecvProxyData *pData, void *pStruct,
                           void *pOut) {
  C_Fish *fish = (C_Fish *)pStruct;
  float *out = (float *)pOut;
...
RecvPropVector(RECVINFO(m_poolOrigin)),
    ...
    RecvPropFloat(RECVINFO(
        m_waterLevel)), ///< get this from the server in case we die when
                        ///< slightly out of the water due to error correction
...
void C_Fish::ClientThink() {
  ...
  if (m_localLifeState == LIFE_ALIVE && m_lifeState != LIFE_ALIVE) {
    ...
    m_deathAngle =
        RandomFloat(87.0f, 93.0f) * ((RandomInt(0, 100) < 50) ? 1.0f : -1.0f);
  }
  ...
  case LIFE_DEAD: {
    ...
    m_angles.z += rollAmp *
                  sin(rollFreq * (gpGlobals->curtime + 10.0f * entindex())) *
                  deltaT;

    const float rollAmp2 = 7.0f;
    const float rollFreq2 = 4.0f;
    m_angles.x += rollAmp2 *
                  sin(rollFreq2 * (gpGlobals->curtime + 10.0f * entindex())) *
                  deltaT;

    const float bobAmp = 0.75f;
    const float bobFreq = 4.0f;
    m_vel.z += bobAmp *
               sin(bobFreq * (gpGlobals->curtime + 10.0f * entindex())) *
               deltaT;

    const float bobAmp2 = 0.75f;
    const float bobFreq2 = 3.333f;
    m_vel.z += bobAmp2 *
               sin(bobFreq2 * (gpGlobals->curtime + 10.0f * entindex())) *
               deltaT;
  ...
  case LIFE_ALIVE: {
    ...
    if (!m_wiggleTimer.IsElapsed()) {
      float swimPower = 1.0f - (m_wiggleTimer.GetElapsedTime() /
                                m_wiggleTimer.GetCountdownDuration());
  ...
  // as fish move faster, their error increases - use a stiffer spring when
  // fast, and a weak one when slow
