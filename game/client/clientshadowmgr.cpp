...
public:
  ...
  virtual ClientShadowHandle_t
  CreateFlashlight(const FlashlightState_t &lightState);
...
CClientShadowMgr::CClientShadowMgr()
    : m_DirtyShadows(0, 0, ShadowHandleCompareFunc),
      m_RenderToTextureActive(false), m_bDepthTextureActive(false) {
...
void CClientShadowMgr::InitDepthTextureShadows() {
  ...
  if (!m_bDepthTextureActive) {
    ...
    ImageFormat nullFormat =
        materials
            ->GetNullTextureFormat(); // Vendor-dependent null texture format
                                      // (takes as little memory as possible)
#endif
    ...
    for (int i = 0; i < m_nMaxDepthTextureShadows; i++) {
      ...
      char strRTName[64];
      Q_snprintf(strRTName, ARRAYSIZE(strRTName), "_rt_ShadowDepthTexture_%d",
                 i);
    ...
    for (ClientShadowHandle_t i = m_Shadows.Head();
      ...
      if (shadow.m_Flags & SHADOW_FLAGS_USE_RENDER_TO_TEXTURE) {
        ...
        shadowmgr->SetShadowMaterial(shadow.m_ShadowHandle, m_RenderShadow,
                                     m_RenderModelShadow, (void *)(uintp)i);
      }
    }
  }
}
...
const Vector &
CClientShadowMgr::GetShadowDirection(IClientRenderable *pRenderable) const {
...
CClientShadowMgr::CreateProjectedTexture(ClientEntityHandle_t entity,
                                         int flags) {
...
ClientShadowHandle_t
CClientShadowMgr::CreateFlashlight(const FlashlightState_t &lightState) {
...
inline ShadowType_t
CClientShadowMgr::GetActualShadowCastType(ClientShadowHandle_t handle) const {
...
inline ShadowType_t
CClientShadowMgr::GetActualShadowCastType(IClientRenderable *pEnt) const {
...
void CClientShadowMgr::BuildRenderToTextureShadow(
  ...
  for (int i = 0; i != 3; ++i) {
    ...
    test *= boxSize[i]; // doing after the projection to simplify projection
                        // math
...
//-----------------------------------------------------------------------------
IClientRenderable *
CClientShadowMgr::GetParentShadowEntity(ClientShadowHandle_t handle) {
  ClientShadow_t &shadow = m_Shadows[handle];
  IClientRenderable *pRenderable =
      ClientEntityList().GetClientRenderableFromHandle(shadow.m_Entity);
  if (pRenderable) {
...
void CClientShadowMgr::UpdateProjectedTexture(ClientShadowHandle_t handle,
  ...
  ClientShadow_t &shadow = m_Shadows[handle];
  if ((shadow.m_Flags & SHADOW_FLAGS_FLASHLIGHT) == 0) {
    Warning("CClientShadowMgr::UpdateProjectedTexture can only be used with "
            "flashlights!\n");
    return;
  }
...
void CClientShadowMgr::AddShadowToReceiver(ClientShadowHandle_t handle,
  ...
  case SHADOW_RECEIVER_BRUSH_MODEL:
    ...
    if (shadow.m_Flags & SHADOW_FLAGS_FLASHLIGHT) {
      ...
      shadowmgr->AddShadowToBrushModel(
          shadow.m_ShadowHandle, const_cast<model_t *>(pRenderable->GetModel()),
          pRenderable->GetRenderOrigin(), pRenderable->GetRenderAngles());
    }
    break;
...
bool CClientShadowMgr::DrawRenderToTextureShadow(
  ...
  if (bNeedsRedraw || bDirtyTexture) {
    ...
    if (DrawShadowHierarchy(pRenderable, shadow)) {
      ...
      // change this now.
      DevMsg("Didn't draw shadow hierarchy.. bad shadow texcoords probably "
  ...
  if ((shadow.m_Flags & SHADOW_FLAGS_USING_LOD_SHADOW) == 0) {
    ...
    ClearExtraClipPlanes(
        clientShadowHandle); // this was ClearExtraClipPlanes(
                             // shadow.m_ShadowHandle ), fix is from Joe Demers
    shadow.m_Flags |= SHADOW_FLAGS_USING_LOD_SHADOW;
  }
}
...
void CClientShadowMgr::ComputeShadowTextures(const CViewSetup &view,
  ...
  if (m_bThreaded && g_pThreadPool->NumIdleThreads()) {
    ...
    ParallelProcess("NPCShadowBoneSetups", s_NPCShadowBoneSetups.Base(),
                    s_NPCShadowBoneSetups.Count(), &SetupBonesOnBaseAnimating);
    ParallelProcess("NonNPCShadowBoneSetups", s_NonNPCShadowBoneSetups.Base(),
...
void CShadowModelProxy::OnBind(void *pProxyData) {
  unsigned short clientShadowHandle =
      (unsigned short)((intp)pProxyData & 0xffff);
  ITexture *pTex = s_ClientShadowMgr.GetShadowTexture(clientShadowHandle);
  m_BaseTextureVar->SetTextureValue(pTex);
