...
void GetVGUICursorPos(int &x, int &y) { vgui::input()->GetCursorPos(x, y); }
...
class CHudTextureHandleProperty
...
static void VGui_OneTimeInit() {
  ...
  vgui::Panel::AddPropertyConverter("CHudTextureHandle",
                                    &textureHandleConverter);
...
bool VGui_Startup(CreateInterfaceFn appSystemFactory) {
  ...
  g_InputInternal = (IInputInternal *)appSystemFactory(
...
static ConVar
    cl_showpausedimage("cl_showpausedimage", "1", 0,
...
void VGui_PreRender() {
  ...
  if (IsPC()) {
    loadingdisc->SetLoadingVisible(engine->IsDrawingLoadingImage() &&
                                   !engine->IsPlayingDemo());

    bool bShowPausedImage =
        !enginevgui->IsGameUIVisible() && cl_showpausedimage.GetBool() &&
        engine->IsPaused() && !engine->IsTakingScreenshot() &&
        !engine->IsPlayingDemo();
#if !defined(TF_CLIENT_DLL)
