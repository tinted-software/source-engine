...
CBorderVisualizerPanel::CBorderVisualizerPanel(Panel *pParent,
                                               const char *pName,
                                               IBorder *pBorder)
...
CColorVisualizerPanel::CColorVisualizerPanel(Panel *pParent, const char *pName,
                                             const Color &color)
...
void CColorVisualizerPanel::Paint() {
  ...
  surface()->DrawFilledRect(nHorzBuffer, nVertBuffer,
                            GetWide() - 2 * nHorzBuffer,
                            GetTall() - 2 * nVertBuffer);
}

//----------------------------------------------------------------------------------------

CSchemeVisualizer::CSchemeVisualizer(vgui::Panel *pParent, IScheme *pViewScheme,
                                     const char *pSchemeName)
    : vgui::Frame(pParent, "SchemeVisualizer"), m_pViewScheme(pViewScheme),
      m_pList(NULL), m_nListDataType(LISTDATATYPE_INVALID) {
  ...
  m_aComboDataTypeToItemIDMap[LISTDATATYPE_BORDERS] =
      m_pListDataTypeCombo->AddItem("Borders", NULL);
  m_aComboDataTypeToItemIDMap[LISTDATATYPE_FONTS] =
      m_pListDataTypeCombo->AddItem("Fonts", NULL);
  m_aComboDataTypeToItemIDMap[LISTDATATYPE_COLORS] =
      m_pListDataTypeCombo->AddItem("Colors", NULL);
...
void CSchemeVisualizer::PerformLayout() {
  ...
  m_pListDataTypeCombo->SetBounds(GetWide() - nComboWidth - XRES(10), YRES(2),
                                  nComboWidth, YRES(8));

  const int nHorzBuffer = XRES(2);
  const int nVertBuffer = YRES(10);
  m_pList->SetBounds(nHorzBuffer, nVertBuffer, GetWide() - 2 * nHorzBuffer,
                     GetTall() - 1.5f * nVertBuffer);
}
...
void CSchemeVisualizer::AddBordersToList() {
  ...
  for (int i = 0; i < nBorderCount; ++i) {
    ...
    CBorderVisualizerPanel *pNewBorderPanel =
        new CBorderVisualizerPanel(m_pList, fmtName.Access(), pCurBorder);
    pNewBorderPanel->SetSize(m_pList->GetWide(), YRES(45));
    m_pList->AddItem(new Label(NULL, "Label", pCurBorder->GetName()),
                     pNewBorderPanel);
  }
}
...
void CSchemeVisualizer::AddFontsToList() {
#ifdef POSIX
  const char strOAccent[] = {
      (char)0xc3, (char)0x93,
      (char)0x00}; // UTF-8 for U+00D3 (LATIN CAPITAL LETTER O WITH ACUTE)
#else
  const uint8 strOAccent[] = {(char)0xd3, (char)0x00};
#endif
  // Stick an intl character in here to test accents (O')
  CFmtStr fmtText(
      "ABCDEFGHIJKLMN%sPQRSTUVWXYZabcdefhijklmnopqrstuvwxyz0123456789!@#$%%^&*("
      ")-_=+",
      strOAccent);
  ...
  for (int i = 0; i < nFontCount; ++i) {
    ...
    Label *pNewFontLabel =
        new Label(m_pList, fmtName.Access(), fmtText.Access());
...
void CSchemeVisualizer::AddColorsToList() {
  ...
  FOR_EACH_SUBKEY(pColorData, pCurColor) {
    ...
    CColorVisualizerPanel *pNewColorPanel =
        new CColorVisualizerPanel(m_pList, fmtName.Access(), Color(r, g, b, a));
...
CON_COMMAND(showschemevisualizer,
            "Show borders, fonts and colors for a particular scheme.  The "
            "default is ClientScheme.res") {
  ...
  g_pSchemeVisualizer =
      vgui::SETUP_PANEL(new CSchemeVisualizer(NULL, pScheme, pSchemeName));
  ...
  g_pSchemeVisualizer->SetBounds((ScreenWidth() - nWidth) / 2,
                                 (ScreenHeight() - nHeight) / 2, nWidth,
                                 nHeight);
...
void CSchemeVisualizer::OnTick() {
  ...
  AssertMsg(nType != LISTDATATYPE_INVALID,
            "Couldn't find item ID in list - this should never happen.");
