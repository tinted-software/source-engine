...
// Constructor
//-----------------------------------------------------------------------------
CEntityImagePanel::CEntityImagePanel(vgui::Panel *pParent,
...
//-----------------------------------------------------------------------------
// Purpose:
// Constructor:  Same as image panel, except can handle team specific
// colors/images Input  : pEntity -
//-----------------------------------------------------------------------------
CEntityTeamImagePanel::CEntityTeamImagePanel(vgui::Panel *pParent,
                                             const char *panelName)
...
bool CEntityTeamImagePanel::Init(KeyValues *pInitData, C_BaseEntity *pEntity) {
  ...
  for (int i = 0; i < MAX_TEAMS; i++) {
    ...
    // modulation color
    if (!ParseRGBA(pTeamKV, "color", m_Images[i].m_r, m_Images[i].m_g,
                   m_Images[i].m_b, m_Images[i].m_a))
      return false;
...
void CEntityTeamImagePanel::Paint(void) {
  ...
  vgui::surface()->DrawSetColor(
      m_Images[teamnumber].m_r, m_Images[teamnumber].m_g,
      m_Images[teamnumber].m_b, m_Images[teamnumber].m_b);
  Color color;
  color.SetColor(m_Images[teamnumber].m_r, m_Images[teamnumber].m_g,
                 m_Images[teamnumber].m_b, m_Images[teamnumber].m_b);
