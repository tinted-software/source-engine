...
ConVar
    mat_accelerate_adjust_exposure_down("mat_accelerate_adjust_exposure_down",
                                        "3.0", FCVAR_CHEAT);
...
#define PPP_PROCESS_PARTIAL_SRC_PARTIAL_DEST(srcmatname, dest_rt_name,         \
  ...
  {PPP_ALWAYS, 0,         srcmatname, dest_rt_name, src_tname,                 \
   destscale,  destscale, srcscale,   srcscale}
...
PostProcessingPass HDRSimulate_NonHDR[] = {
    PPP_PROCESS("dev/copyfullframefb_vanilla", NULL), PPP_END};
...
// Note carefully about the downsampling: the first downsampling samples from
// the full rendertarget down to a temp. When doing this sampling, the texture
// source clamping will take care of the out of bounds sampling done because of
// the filter kernels's width. However, on any of the subsequent sampling
// operations, we will be sampling from a partially filled render target. So,
// texture coordinate clamping cannot help us here. So, we need to always render
// a few more pixels to the destination than we actually intend to, so as to
// replicate the border pixels so that garbage pixels do not get sucked into the
// sampling. To deal with this, we always add FILTER_KERNEL_SLOP to our
// widths/heights if there is room for them in the destination.
static void DrawScreenSpaceRectangleWithSlop(
    ITexture *dest_rt, IMaterial *pMaterial, int destx, int desty, int width,
  ...
  int slopwidth =
      width +
      FILTER_KERNEL_SLOP; // min(dest_rt->GetActualWidth()-destx,width+FILTER_KERNEL_SLOP);
  int slopheight =
      height +
      FILTER_KERNEL_SLOP; // min(dest_rt->GetActualHeight()-desty,height+FILTER_KERNEL_SLOP);

  // adjust coordinates for slop
  src_texture_x1 = FLerp(src_texture_x0, src_texture_x1, destx,
                         destx + width - 1, destx + slopwidth - 1);
  src_texture_y1 = FLerp(src_texture_y0, src_texture_y1, desty,
                         desty + height - 1, desty + slopheight - 1);
  width = slopwidth;
  height = slopheight;
...
void CHistogram_entry_t::IssueQuery(int frm_num) {
  ...
  if (mat_tonemapping_occlusion_use_stencil.GetInt()) {
    ...
    // AV - We don't need to clear stencil here because it's already been
    // cleared at the beginning of the frame
    // pRenderContext->ClearStencilBufferRectangle( scrx_min, scry_min,
    // scrx_max, scry_max, 0 );
    ...
    case HESTATE_QUERY_IN_FLIGHT:
      if (cur_query_frame > CurHistogram[i].m_frame_queued + 2) {
                      ...
                      // next time
        {
          ...
          // 						    if
          // (mat_debug_autoexposure.GetInt()) 								Warning("min=%f max=%f np =
...
void CLuminanceHistogramSystem::DisplayHistogram(void) {
  ...
  if (bDrawTextThisFrame == true) {
    ...
    engine->Con_NPrintf(21,
                        "AvgLum @ %4.2f%%  mat_tonemap_min_avglum = %4.2f%%  "
                        "Using %d pixels of %d pixels on screen (%3d%%)",
                        MAX(0.0f, FindLocationOfPercentBrightPixels(50.0f)) *
                            100.0f,
  ...
  for (int l = 0; l < nNumRanges; l++) {
    ...
    if (np) {
      int height =
          MAX(1, MIN(HISTOGRAM_BAR_SIZE, ((float)np / (float)nMaxValidPixels) *
                                             HISTOGRAM_BAR_SIZE));
...
public:
  ...
  static void SetupEnginePostMaterialAA(bool bPerformSoftwareAA,
                                        float flAAStrength);
  static void
  SetupEnginePostMaterialTextureTransform(const Vector4D &fullViewportBloomUVs,
...
static float GetBloomAmount(void) {
  ...
  if (!g_pMaterialSystemHardwareConfig->CanDoSRGBReadFromRTs() &&
...
// Dump a rendertarget to a TGA.  Useful for looking at intermediate render
// target results.
void DumpTGAofRenderTarget(const int width, const int height,
  ...
  if (mat_queue_mode.GetInt() != 0) {
    ...
    mat_dump_rts.SetValue(
        0); // Just report this error once and stop trying to dump images
    return;
  }
  ...
  pRenderContext->ReadPixels(0, 0, width, height, pImage,
  ...
  if (!TGAWriter::WriteToBuffer(pImage, buffer, width, height,
  ...
  Q_snprintf(szPathedFileName, sizeof(szPathedFileName), "//MOD/%d_%s_%s.tga",
             s_nRTIndex++, pFilename, IsOSX() ? "OSX" : "PC");
...
// Counter-Strike: Source uses a different downsample algorithm than other games
  ...
  IMaterial *downsample_mat = materials->FindMaterial(
      "dev/downsample_non_hdr_cstrike", TEXTURE_GROUP_OTHER, true);
#else
  IMaterial *downsample_mat = materials->FindMaterial(
...
static void DoPreBloomTonemapping(IMatRenderContext *pRenderContext, int nX,
  ...
  if (mat_dynamic_tonemapping.GetInt() || mat_show_histogram.GetInt()) {
    ...
    if (mat_dynamic_tonemapping.GetInt() || mat_show_histogram.GetInt()) {
      ...
      float flTargetScalarClamped =
          MAX(flAutoExposureMin, MIN(flAutoExposureMax, flTargetScalar));
      flTargetScalarClamped =
          MAX(0.001f, flTargetScalarClamped); // Don't let this go to 0!
...
static void CreatePyroSide(int nSide, Vector2D &vMaxSize) {
  ...
  pSide->m_flIntensityLimit =
      RandomFloat(pyro_min_intensity.GetFloat(), pyro_max_intensity.GetFloat());
  pSide->m_flRate =
      RandomFloat(pyro_min_rate.GetFloat(), pyro_max_rate.GetFloat());
  pSide->m_bIncreasing = true;
  pSide->m_bHorizontal = ((nSide >> 1) & 1) == 0;
  pSide->m_bAlive = true;

  //	float flWidth = RandomFloat( MIN_PYRO_SIDE_WIDTH, MAX_PYRO_SIDE_WIDTH )
  //* 2.0f; 	float flLength = RandomFloat( MIN_PYRO_SIDE_LENGTH,
  //MAX_PYRO_SIDE_LENGTH );
  float flWidth = RandomFloat(pyro_min_side_width.GetFloat(),
                              pyro_max_side_width.GetFloat()) *
                  2.0f;
  float flLength = RandomFloat(pyro_min_side_length.GetFloat(),
                               pyro_max_side_length.GetFloat());
...
static void DrawPyroVignette(
    int nDestX, int nDestY, int nWidth,
  ...
  IMaterial *pVignetteBorder = materials->FindMaterial(
      "dev/pyro_vignette_border", TEXTURE_GROUP_OTHER, true);
  IMaterial *pMaterial =
      materials->FindMaterial("dev/pyro_vignette", TEXTURE_GROUP_OTHER, true);
  ITexture *pRenderTarget = materials->FindTexture("_rt_ResolvedFullFrameDepth",
  ...
  pRenderContext->DrawScreenSpaceRectangle(
      pVignetteBorder, 0, 0, nScreenWidth, nScreenHeight, 0, 0,
        ...
        if (pSide->m_bAlive) {
          for (int i = 0; i < 6; i++) {
            meshBuilder.Position3f(
                pSide->m_vCornerPos[PryoSideIndexes[i][0]][0],
                pSide->m_vCornerPos[PryoSideIndexes[i][1]][1], 0.0f);
            meshBuilder.Color4f(pSide->m_flIntensity, pSide->m_flIntensity,
                                pSide->m_flIntensity, 1.0f);
            meshBuilder.TexCoord2fv(0, pSide->m_bHorizontal
                                           ? PryoVignetteSTHorizontal[i]
...
static void
DrawPyroPost(IMaterial *pMaterial, int nDestX, int nDestY, int nWidth,
             int nHeight, // Rect to draw into in screen space
             float flSrcTextureX0,
             float flSrcTextureY0, // which texel you want to appear at destx/y
             float flSrcTextureX1,
             float flSrcTextureY1, // which texel you want to appear at
                                   // destx+width-1, desty+height-1
             ...
             void *pClientRenderable) // Used to pass to the bind proxies
{
  ...
  // Top left
  meshBuilder.Position3f(flLeftX + (float)0 * flWidth,
                         flTopY - (float)0 * flHeight, 0.0f);
  meshBuilder.TexCoord2f(0, flLeftU + (float)0 * flUWidth,
                         flTopV + (float)0 * flVHeight);
  meshBuilder.AdvanceVertex();
  ...
  meshBuilder.Position3f(flLeftX + (float)(xSegments + 1) * flWidth,
                         flTopY - (float)(0 + 1) * flHeight, 0.0f);
  meshBuilder.TexCoord2f(0, flLeftU + (float)(xSegments + 1) * flUWidth,
                         flTopV + (float)(0 + 1) * flVHeight);
  meshBuilder.AdvanceVertex();
  ...
  // Top right (x+1)
  meshBuilder.Position3f(flLeftX + (float)(xSegments)*flWidth,
                         flTopY - (float)(ySegments - 1) * flHeight, 0.0f);
  ...
  // Bottom right (x+1), (y+1)
  meshBuilder.Position3f(flLeftX + (float)(xSegments)*flWidth,
                         flTopY - (float)(ySegments)*flHeight, 0.0f);
  meshBuilder.TexCoord2f(0, flLeftU + (float)(xSegments)*flUWidth,
                         flTopV + (float)(ySegments)*flVHeight);
  meshBuilder.AdvanceVertex();
  ...
  // Top left
  meshBuilder.Position3f(flLeftX + (float)0 * flWidth,
                         flTopY - (float)1 * flHeight, 0.0f);
  meshBuilder.TexCoord2f(0, flLeftU + (float)0 * flUWidth,
                         flTopV + (float)1 * flVHeight);
  meshBuilder.AdvanceVertex();

  // Top right (x+1)
  meshBuilder.Position3f(flLeftX + (float)(0 + 1) * flWidth,
                         flTopY - (float)1 * flHeight, 0.0f);
  meshBuilder.TexCoord2f(0, flLeftU + (float)(0 + 1) * flUWidth,
                         flTopV + (float)1 * flVHeight);
  meshBuilder.AdvanceVertex();
...
void DoEnginePostProcessing(int x, int y, int w, int h, bool bFlashlightIsOn,
  ...
  if (mat_dump_rts.GetBool()) {
    ...
    mat_dump_rts.SetValue(
        0); // We only want to capture one frame, on rising edge of this convar
  ...
  case HDR_TYPE_INTEGER: {
    ...
    if (mat_software_aa_strength.GetFloat() == -1.0f) {
      if (IsX360()) {
        ...
        if (g_pMaterialSystem->GetCurrentConfigForVideoCard()
                .m_VideoMode.m_Height > 480) {
          mat_software_aa_quality.SetValue(0);
        } else {
          // For standard-def, we have fewer pixels so we can afford 'high
          // quality' mode (5->9 taps/pixel)
    ...
    if (mat_software_aa_strength_vgui.GetFloat() == -1.0f) {
      if (IsX360() && (g_pMaterialSystem->GetCurrentConfigForVideoCard()
    ...
    bool bPerformSoftwareAA = IsX360() && (engine->GetDXSupportLevel() >= 90) &&
                              (flAAStrength != 0.0f);
    ...
    if (bPerformBloom || bPerformSoftwareAA || bPerformColCorrect) {
      ...
      // UpdateScreenEffectTexture()), so we need to adjust the corner-pixel UVs
      // sent to our drawrect call:
      Vector2D uvScale((nSrcWidth - (nSrcWidth / (float)w)) / (nSrcWidth - 1),
                       (nSrcHeight - (nSrcHeight / (float)h)) /
                           (nSrcHeight - 1));
      ...
      if (!bFirstFrame || !bPerformColCorrect) {
        ...
        if (mat_postprocessing_combine.GetInt()) {
          ...
          if (bSplitScreenHDR) {
            pRenderContext->SetScissorRect(
                partialViewportPostDestRect.width / 2, 0,
                partialViewportPostDestRect.width,
          ...
          pRenderContext->DrawScreenSpaceRectangle(
              ...
              0, 0, // partialViewportPostDestRect.x,
                    // partialViewportPostDestRect.y,
              partialViewportPostDestRect.width,
              partialViewportPostDestRect.height,
          ...
          if (bPerformSoftwareAA) {
            ...
            pRenderContext->DrawScreenSpaceRectangle(
                aa_mat,
                // TODO: check if offsets should be 0,0 here, as with the
                // combined-pass case
        ...
        if (bVisionOverride &&
            g_pMaterialSystemHardwareConfig->SupportsPixelShaders_2_0() &&
          ...
          IMaterial *pPyroVisionPostMaterial = materials->FindMaterial(
              "dev/pyro_post", TEXTURE_GROUP_OTHER, true);
          DrawPyroPost(
              pPyroVisionPostMaterial,
              // TODO: check if offsets should be 0,0 here, as with the
              // combined-pass case
              partialViewportPostDestRect.x, partialViewportPostDestRect.y,
              partialViewportPostDestRect.width,
        ...
        if (g_bDumpRenderTargets) {
          DumpTGAofRenderTarget(partialViewportPostDestRect.width,
                                partialViewportPostDestRect.height,
                                "EnginePost");
        }
      }
  ...
  case HDR_TYPE_FLOAT: {
    ...
    if (mat_dynamic_tonemapping.GetInt() || mat_show_histogram.GetInt()) {
      ...
      if (mat_dynamic_tonemapping.GetInt()) {
        float avg_lum =
            MAX(0.0001, g_HDR_HistogramSystem.GetTargetTonemapScalar());
        float scalevalue =
            MAX(flAutoExposureMin, MIN(flAutoExposureMax, 0.18 / avg_lum));
    ...
    if (mat_dynamic_tonemapping.GetInt()) {
      float avg_lum =
          MAX(0.0001, g_HDR_HistogramSystem.GetTargetTonemapScalar());
      float scalevalue =
          MAX(flAutoExposureMin, MIN(flAutoExposureMax, 0.023 / avg_lum));
...
void DoImageSpaceMotionBlur(const CViewSetup &view, int x, int y, int w,
  ...
  float flMotionBlurRollIntensity =
      0.3f; // * mat_motion_blur_roll_intensity.GetFloat(); // The default is to
            // not blur past 30% of the range
  ...
  if (true) {
    ...
    if ((VectorLength(vPositionChange) > 30.0f) && (flTimeElapsed >= 0.5f)) {
      ...
      if (flPitchDiffOriginal < 0.0f)
        flPitchDiffAdjusted =
            clamp(flPitchDiffAdjusted, flPitchDiffOriginal, 0.0f);
      else
        flPitchDiffAdjusted =
            clamp(flPitchDiffAdjusted, 0.0f, flPitchDiffOriginal);
      ...
      g_vMotionBlurValues[3] =
          flUndampenedYaw; // Roll starts out as undampened yaw intensity and is
                           // then scaled by pitch
      g_vMotionBlurValues[3] *=
      ...
      g_vMotionBlurValues[2] =
          clamp((fabs(g_vMotionBlurValues[2]) - flMotionBlurFallingMin) /
