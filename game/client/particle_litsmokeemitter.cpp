...
//			color -
//-----------------------------------------------------------------------------
void CLitSmokeEmitter::SetDirectionalLight(Vector position, Vector color,
                                           float intensity) {
...
//			intensity -
//-----------------------------------------------------------------------------
void CLitSmokeEmitter::SetLight(Vector position, Vector color,
                                float intensity) {
  CParticleLightInfo info;
  info.m_flIntensity = intensity;
  info.m_vColor = color;
...
void CLitSmokeEmitter::RenderParticles(CParticleRenderIterator *pIterator) {
  const LitSmokeParticle *pParticle =
      (const LitSmokeParticle *)pIterator->GetFirst();
  while (pParticle) {
    ...
    float alpha255 =
        (((float)pParticle->m_uchColor[3] / 255.0f) * sin(M_PI_F * tLifetime)) *
        255.0f;

    Vector color01 = Vector(pParticle->m_uchColor[0], pParticle->m_uchColor[1],
                            pParticle->m_uchColor[2]) *
                     (tLifetime / 255.0f);

    m_Renderer.RenderParticle_AddColor(
        pIterator->GetParticleDraw(), pParticle->m_Pos, tPos, alpha255,
...
void CLitSmokeEmitter::SimulateParticles(CParticleSimulateIterator *pIterator) {
  ...
  while (pParticle) {
    ...
    pParticle->m_Pos = pParticle->m_Pos +
                       (pParticle->m_vecVelocity * pIterator->GetTimeDelta());
