...
void CHudChatLine::PerformFadeout(void) {
  ...
  if (curtime <= m_flExpireTime &&
    ...
    if (m_iNameLength > 0) {
      ...
      InsertColorChange(
          Color(g_ColorYellow[0], g_ColorYellow[1], g_ColorYellow[2], alpha));
      InsertString(wText);
...
wchar_t *ReadLocalizedRadioCommandString(bf_read &msg, wchar_t *pOut,
...
void CHudChat::ChatPrintf(int iPlayerIndex, const char *fmt, ...) {
  ...
  if (pName) {
    ...
    line->InsertColorChange(
        Color(g_ColorYellow[0], g_ColorYellow[1], g_ColorYellow[2], 255));

  char *buf = static_cast<char *>(_alloca(strlen(pmsg) + 1));
  wchar_t *wbuf =
      static_cast<wchar_t *>(_alloca((strlen(pmsg) + 1) * sizeof(wchar_t)));
  if (buf) {
    ...
    line->InsertColorChange(
        Color(g_ColorYellow[0], g_ColorYellow[1], g_ColorYellow[2], 255));
    g_pVGuiLocalize->ConvertANSIToUnicode(buf, wbuf,
                                          strlen(pmsg) * sizeof(wchar_t));
  ...
  C_BaseEntity::EmitSound(filter, -1 /*SOUND_FROM_LOCAL_PLAYER*/,
                          "HudChat.Message");
}
