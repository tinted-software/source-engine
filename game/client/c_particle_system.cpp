...
protected:
  ...
  }; ///< actually one less than the total number of cpoints since 0 is assumed
     ///< to be me

  EHANDLE m_hControlPointEnts[kMAXCONTROLPOINTS];
  //	SendPropArray3( SENDINFO_ARRAY3(m_iControlPointParents), SendPropInt(
  //SENDINFO_ARRAY(m_iControlPointParents), 3, SPROP_UNSIGNED ) ),
...
RecvPropVector(RECVINFO_NAME(m_vecNetworkOrigin, m_vecOrigin)),
    ...
    RecvPropArray3(RECVINFO_ARRAY(m_hControlPointEnts),
                   RecvPropEHandle(RECVINFO(m_hControlPointEnts[0]))),
    RecvPropArray3(RECVINFO_ARRAY(m_iControlPointParents),
                   RecvPropInt(RECVINFO(m_iControlPointParents[0]))),
    RecvPropBool(RECVINFO(m_bWeatherEffect)), END_RECV_TABLE();

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
C_ParticleSystem::C_ParticleSystem() { m_bWeatherEffect = false; }
...
void C_ParticleSystem::PostDataUpdate(DataUpdateType_t updateType) {
  ...
  if (updateType == DATA_UPDATE_CREATED) {
    if (m_bActive) {
      // Delayed here so that we don't get invalid abs queries on level init
      // with active particle systems
  ...
  if (m_bActive) {
    ...
    if (pszName && pszName[0]) {
      ...
      CNewParticleEffect *pEffect =
          ParticleProp()->Create(pszName, PATTACH_ABSORIGIN_FOLLOW);
      AssertMsg1(pEffect, "Particle system couldn't make %s", pszName);
      if (pEffect) {
        for (int i = 0; i < kMAXCONTROLPOINTS; ++i) {
          CBaseEntity *pOnEntity = m_hControlPointEnts[i].Get();
          if (pOnEntity) {
            ParticleProp()->AddControlPoint(pEffect, i + 1, pOnEntity,
                                            PATTACH_ABSORIGIN_FOLLOW);
          }

          AssertMsg2(m_iControlPointParents[i] >= 0 &&
                         m_iControlPointParents[i] <= kMAXCONTROLPOINTS,
                     "Particle system specified bogus control point parent "
                     "(%d) for point %d.",
        ...
        // NOTE: What we really want here is to compare our lifetime and that of
        // our children and see if this delta is
        //		 already past the end of it, denoting that we're
        //finished.  In that case, just destroy us and be done. -- jdw
        ...
        if (flTimeDelta > 0.01f) {
          VPROF_BUDGET("C_ParticleSystem::ClientThink SkipToTime",
                       "Particle Simulation");
...
void ParticleEffectCallback(const CEffectData &data) {
  if (SuppressingParticleEffects())
    return; // this needs to be before using data.m_nHitBox, since that may be a
            // serialized value that's past the end of the current particle
            // system string table
  ...
  if (data.m_fFlags & PARTICLE_DISPATCH_FROM_ENTITY) {
    if (data.m_hEntity.Get()) {
      ...
      if (pEnt && !pEnt->IsDormant()) {
        ...
        pEffect = pEnt->ParticleProp()->Create(
            pszName, (ParticleAttachment_t)data.m_nDamageType,
            data.m_nAttachmentIndex);
        AssertMsg2(pEffect.IsValid() && pEffect->IsValid(),
                   "%s could not create particle effect %s",
                   C_BaseEntity::Instance(data.m_hEntity)->GetDebugName(),
                   pszName);
        if (pEffect.IsValid() && pEffect->IsValid()) {
          if ((ParticleAttachment_t)data.m_nDamageType ==
    ...
    if (GameRules()) {
      pszName =
          GameRules()->TranslateEffectForVisionFilter("particles", pszName);
    }
  ...
  if (pEffect.IsValid() && pEffect->IsValid()) {
    if (data.m_bCustomColors) {
      pEffect->SetControlPoint(CUSTOM_COLOR_CP1,
                               data.m_CustomColors.m_vecColor1);
      pEffect->SetControlPoint(CUSTOM_COLOR_CP2,
                               data.m_CustomColors.m_vecColor2);
    }
