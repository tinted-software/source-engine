...
static ConVar
    r_ForceWaterLeaf("r_ForceWaterLeaf", "1", 0,
                     "Enable for optimization to water - considers view in "
...
struct ClientWorldListInfo_t : public CRefCounted1<WorldListInfo_t> {
  ...
  static ClientWorldListInfo_t *
  AllocPooled(const ClientWorldListInfo_t &exemplar);
...
protected:
  ...
  void DrawInternal(view_id_t iSkyBoxViewID, bool bInvokePreAndPostRender,
                    ITexture *pRenderTarget, ITexture *pDepthTarget);
...
public:
  CAboveWaterView(CViewRender *pMainView)
      : CBaseWaterView(pMainView), m_ReflectionView(pMainView),
        m_RefractionView(pMainView), m_IntersectionView(pMainView) {}
...
static inline unsigned long BuildEngineDrawWorldListFlags(unsigned nDrawFlags) {
  ...
  if (nDrawFlags & DF_SSAO_DEPTH_PASS) {
    nEngineFlags |= DRAWWORLDLISTS_DRAW_SSAO |
                    DRAWWORLDLISTS_DRAW_STRICTLYUNDERWATER |
                    DRAWWORLDLISTS_DRAW_INTERSECTSWATER |
                    DRAWWORLDLISTS_DRAW_STRICTLYABOVEWATER;
    nEngineFlags &=
        ~(DRAWWORLDLISTS_DRAW_WATERSURFACE | DRAWWORLDLISTS_DRAW_REFRACTION |
          DRAWWORLDLISTS_DRAW_REFLECTION);
  }
...
void CViewRender::DrawViewModels(const CViewSetup &view, bool drawViewmodel) {
...
#ifdef PORTAL // in portal, we'd like a copy of the front buffer without the gun
              // in it for use with the depth doubler
  ...
  if (view.m_eStereoEye != STEREO_EYE_MONO) {
    pRTColor = g_pSourceVR->GetRenderTarget(
        (ISourceVirtualReality::VREye)(view.m_eStereoEye - 1),
        ISourceVirtualReality::RT_Color);
    pRTDepth = g_pSourceVR->GetRenderTarget(
        (ISourceVirtualReality::VREye)(view.m_eStereoEye - 1),
  ...
  extern bool LocalPlayerIsCloseToPortal(
      void); // defined in C_Portal_Player.cpp, abstracting to a single bool
             // function to remove explicit dependence on c_portal_player.h/cpp,
             // you can define the function as a "return true" in other build
...
void CViewRender::PerformScreenOverlay(int x, int y, int w, int h) {
  ...
  if (m_ScreenOverlayMaterial) {
    ...
    if (m_ScreenOverlayMaterial->NeedsFullFrameBufferTexture()) {
      // FIXME: check with multi/sub-rect renders. Should this be 0,0,w,h
      // instead?
      ...
      pRenderContext->DrawScreenSpaceRectangle(m_ScreenOverlayMaterial, 0, 0, w,
...
void CViewRender::DrawUnderwaterOverlay(void) {
  ...
  if (pOverlayMat) {
    ...
    if (pOverlayMat->NeedsFullFrameBufferTexture()) {
      ...
      int sh = pTexture->GetActualHeight();
      // Note - don't offset by x,y - already done by the viewport.
      pRenderContext->DrawScreenSpaceRectangle(pOverlayMat, 0, 0, w, h, 0, 0,
                                               sw - 1, sh - 1, sw, sh);
    } else {
      // Note - don't offset by x,y - already done by the viewport.
      // FIXME: actually test this code path.
      pRenderContext->DrawScreenSpaceRectangle(pOverlayMat, 0, 0, w, h, 0, 0, 1,
                                               1, 1, 1);
    }
  }
}
...
static void GetFogColor(fogparams_t *pFogParams, float *pColor) {
  ...
  if (fog_override.GetInt() && fogColorString) {
    ...
    float flPrimaryColor[3] = {(float)pFogParams->colorPrimary.GetR(),
                               (float)pFogParams->colorPrimary.GetG(),
                               (float)pFogParams->colorPrimary.GetB()};
    float flSecondaryColor[3] = {(float)pFogParams->colorSecondary.GetR(),
...
void CViewRender::SetupMain3DView(const CViewSetup &view, int &nClearFlags) {
  ...
  int nDepthStencilFlags =
      nClearFlags & (VIEW_CLEAR_DEPTH | VIEW_CLEAR_STENCIL);
  nClearFlags &= ~(nDepthStencilFlags); // Clear these flags
  if (nClearFlags & VIEW_CLEAR_COLOR) {
    nClearFlags |=
        nDepthStencilFlags; // Add them back in if we're clearing color
  }
...
void CViewRender::RenderView(const CViewSetup &view, int nClearFlags,
  ...
  if (!m_rbTakeFreezeFrame[view.m_eStereoEye] &&
      m_flFreezeFrameUntil > gpGlobals->curtime) {
    ...
    if (m_rbTakeFreezeFrame[view.m_eStereoEye]) {
      ...
      if (IsX360()) {
        ...
        pRenderContext->CopyRenderTargetToTextureEx(
            GetFullFrameFrameBufferTexture(1), 0, &rect, &rect);
      } else {
  ...
  if (mat_viewportupscale.GetBool() && mat_viewportscale.GetFloat() < 1.0f) {
    ...
    ITexture *pFullFrameFB1 =
        materials->FindTexture("_rt_FullFrameFB1", TEXTURE_GROUP_RENDER_TARGET);
    IMaterial *pCopyMaterial =
        materials->FindMaterial("dev/upscale", TEXTURE_GROUP_OTHER);
    ...
    pRenderContext->CopyRenderTargetToTextureEx(pFullFrameFB1, 0,
                                                &DownscaleRect, &DownscaleRect);
    pRenderContext->DrawScreenSpaceRectangle(
        pCopyMaterial, UpscaleRect.x, UpscaleRect.y, UpscaleRect.width,
        UpscaleRect.height, DownscaleRect.x, DownscaleRect.y,
        DownscaleRect.x + DownscaleRect.width - 1,
        DownscaleRect.y + DownscaleRect.height - 1,
  ...
  if (UseVR()) {
    saveRenderTarget = g_pSourceVR->GetRenderTarget(
        (ISourceVirtualReality::VREye)(view.m_eStereoEye - 1),
        ISourceVirtualReality::RT_Color);
  }
  ...
  if (whatToDraw & RENDERVIEW_DRAWHUD) {
    ...
    if (UseVR()) {
      if (g_ClientVirtualReality.ShouldRenderHUDInWorld()) {
        ...
        viewFramebufferX =
            view.m_eStereoEye == STEREO_EYE_RIGHT ? viewWidth : 0;
        viewFramebufferY = 0;
      }
    }

    // Get the render context out of materials to avoid some debug stuff.
    // WARNING THIS REQUIRES THE .SafeRelease below or it'll never release the
    // ref
    pRenderContext = materials->GetRenderContext();
    ...
    pRenderContext->PushRenderTargetAndViewport(
        pTexture, NULL, viewX, viewY, viewActualWidth, viewActualHeight);
    ...
    if (UseVR()) {
      g_pMatSystemSurface->SetFullscreenViewportAndRenderTarget(
          viewFramebufferX, viewFramebufferY, viewFramebufferWidth,
          viewFramebufferHeight, saveRenderTarget);
    }
    ...
    if (UseVR()) {
      ...
      bool bInFreezeCam =
          (pPlayer && pPlayer->GetObserverMode() == OBS_MODE_FREEZECAM);

      // draw the HUD after the view model so its "I'm closer" depth queues work
      // right.
      if (!bInFreezeCam && g_ClientVirtualReality.ShouldRenderHUDInWorld()) {
        // Now we've rendered the HUD to its texture, actually get it on the
        // screen. Since we're drawing it as a 3D object, we need correctly set
        // up frustum, etc.
        int ClearFlags = 0;
        SetupMain3DView(view, ClearFlags);

        // TODO - a bit of a shonky test - basically trying to catch the main
        // menu, the briefing screen, the loadout screen, etc.
        bool bTranslucent = !g_pMatSystemSurface->IsCursorVisible();
        g_ClientVirtualReality.RenderHUDQuad(
            g_pClientMode->ShouldBlackoutAroundHUD(), bTranslucent);
        CleanupMain3DView(view);
      }
    }
...
void CViewRender::DetermineWaterRenderInfo(
  ...
  // water
#ifdef _X360
  if (!bForceExpensive && (bForceCheap || (fogVolumeInfo.m_flDistanceToWater >=
...
//ViewDrawScene resets the view 			and doesn't flag the rendering as ended when it
//ends.
...
void CViewRender::ViewDrawScene_Intro(const CViewSetup &view, int nClearFlags,
  ...
  for (passID = 0; passID < introData.m_Passes.Count(); passID++) {
    ...
    pRenderContext->DrawScreenSpaceRectangle(
        pOverlayMaterial, 0, 0, view.width, view.height, actualRect.x,
...
void CViewRender::DrawMonitors(const CViewSetup &cameraView) {
  ...
  if (IsX360() && cameraNum > 0) {
    ...
    // resolving *after* all monitors drawn to ensure a single blit using
    // fastest resolve path
    CMatRenderContextPtr pRenderContext(materials);
...
ClientWorldListInfo_t *
ClientWorldListInfo_t::AllocPooled(const ClientWorldListInfo_t &exemplar) {
  size_t nBytes =
...
CRendering3dView::CRendering3dView(CViewRender *pMainView)
    : CBase3dView(pMainView), m_pWorldRenderList(NULL),
      m_pRenderablesList(NULL), m_pWorldListInfo(NULL),
...
static void DrawClippedDepthBox(IClientRenderable *pEnt, float *pClipPlane) {
  ...
  meshBuilder.Begin(
      ...
      18); // 6 sides, possible one cut per side. Any side is capable of having
           // 3 tri's. Lots of padding for things that aren't possible
  ...
  for (int i = 0; i != 6; ++i) {
    ...
    float *pStartPoint = 0;
    float *pTriangleFanPoints[4]; // at most, one of our fans will have 5 points
                                  // total, with the first point being stored
                                  // separately as pStartPoint
...
static inline void DrawOpaqueRenderable(IClientRenderable *pEnt, bool bTwoPass,
  ...
  if (pRenderClipPlane) {
    CMatRenderContextPtr pRenderContext(materials);
    if (!materials
             ->UsingFastClipping()) // do NOT change the fast clip plane
                                    // mid-scene, depth problems result. Regular
                                    // user clip planes are fine though
...
ConVar
    r_drawopaquesbucket_stats("r_drawopaquesbucket_stats", "0", FCVAR_CHEAT,
...
static void DrawOpaqueRenderables_DrawStaticProps(
  ...
  for (CClientRenderablesList::CEntry *itEntity = pEntitiesBegin;
    ...
    staticpropmgr->DrawStaticProps(pStatics, numScheduled, DepthMode,
...
static void
DrawOpaqueRenderables_Range(CClientRenderablesList::CEntry *pEntitiesBegin,
...
void CRendering3dView::DrawOpaqueRenderables(ERenderDepthMode DepthMode) {
  ...
  // bool const bDrawopaquestaticpropslast =
  // r_drawopaquestaticpropslast.GetBool();
  ...
  if (r_threaded_renderables.GetBool()) {
    ParallelProcess("BoneSetupNpcsLast",
                    arrBoneSetupNpcsLast.Base() + numOpaqueEnts - numNpcs,
...
// Render sequence debugging
    ...
    if (int iBucket = r_drawopaquesbucket.GetInt()) {
      ...
      for (int bucket = 0; bucket < RENDER_GROUP_CFG_NUM_OPAQUE_ENT_BUCKETS;
           ++bucket) {
        // PVS-Studio pointed out that the two sides of the if/else were
        // identical. Fixing this long-broken behavior would change rendering,
        // so I fixed the code but commented out the new behavior. Uncomment the
        // if statement and else block when needed.
        // if ( bDrawopaquestaticpropslast )
        {
          DrawOpaqueRenderables_Range(pEnts[bucket][0], pEnts[bucket][1],
                                      DepthMode);
          DrawOpaqueRenderables_DrawStaticProps(pProps[bucket][0],
                                                pProps[bucket][1], DepthMode);
        }
        /*else
        {
                DrawOpaqueRenderables_DrawStaticProps( pProps[bucket][0],
        pProps[bucket][1], DepthMode ); DrawOpaqueRenderables_Range(
        pEnts[bucket][0], pEnts[bucket][1], DepthMode );
        }*/
      }
  }
...
void CRendering3dView::DrawTranslucentRenderablesNoWorld(bool bInSkybox) {
  ...
  bool bShadowDepth =
      (m_DrawFlags & (DF_SHADOW_DEPTH_MAP | DF_SSAO_DEPTH_PASS)) != 0;
...
void CRendering3dView::DrawTranslucentRenderables(bool bInSkybox,
  ...
  if (ShouldDrawPortals()) // no recursive stencil views during skybox rendering
                           // (although we might be drawing a skybox while
                           // already in a recursive stencil view)
  {
                                         ...
                                         // this hack cast
    {
      ...
      if (r_depthoverlay.GetBool()) {
        ...
        pRenderContext->OverrideDepthEnable(
            true, false); // don't write to depth, or else we'll never see
                          // translucents
...
//-----------------------------------------------------------------------------
sky3dparams_t *
CSkyboxView::PreRender3dSkyboxWorld(SkyboxVisibility_t nSkyboxVisible) {
...
bool CSkyboxView::Setup(const CViewSetup &view, int *pClearFlags,
  ...
  *pClearFlags &= ~(VIEW_CLEAR_COLOR | VIEW_CLEAR_DEPTH | VIEW_CLEAR_STENCIL |
                    VIEW_CLEAR_FULL_TARGET);
  *pClearFlags |=
      VIEW_CLEAR_DEPTH; // Need to clear depth after rednering the skybox
...
void CSkyboxView::Draw() {
  ...
  if (m_eStereoEye != STEREO_EYE_MONO) {
    pRTColor = g_pSourceVR->GetRenderTarget(
        (ISourceVirtualReality::VREye)(m_eStereoEye - 1),
        ISourceVirtualReality::RT_Color);
    pRTDepth = g_pSourceVR->GetRenderTarget(
        (ISourceVirtualReality::VREye)(m_eStereoEye - 1),
        ISourceVirtualReality::RT_Depth);
...
void CShadowDepthView::Draw() {
  ...
  // Start view
  unsigned int visFlags;
  m_pMainView->SetupVis((*this),
                        visFlags); // @MULTICORE (toml 8/9/2006): Portal
                                   // problem, not sending custom vis down
  ...
  pRenderContext->PushVertexShaderGPRAllocation(
      112); // almost all work is done in vertex shaders for depth rendering,
            // max out their threads
#endif
  ...
  if (IsPC()) {
    ...
    // for the 360, the dummy render target has a separate depth buffer which we
    // Resolve() from afterward
    render->Push3DView((*this), VIEW_CLEAR_DEPTH, m_pRenderTarget,
                       GetFrustum());
  }
...

  {
    ...
    BuildWorldRenderLists(true, -1, true,
                          true); // @MULTICORE (toml 8/9/2006): Portal problem,
                                 // not sending custom vis down
  }
  ...
  if (IsX360()) {
    // Resolve() the depth texture here. Before the pop so the copy will
    // recognize that the resolutions are the same
    pRenderContext->CopyRenderTargetToTextureEx(m_pDepthTexture, -1, NULL,
                                                NULL);
  }
...
#if defined(_X360)
  pRenderContext->PushVertexShaderGPRAllocation(
      16); // max out pixel shader threads
#endif
...
void CBaseWorldView::PushView(float waterHeight) {
  ...
  if (m_ClearFlags &
      (VIEW_CLEAR_DEPTH | VIEW_CLEAR_COLOR | VIEW_CLEAR_STENCIL)) {
...
void MaybeInvalidateLocalPlayerAnimation() {
  ...
  if ((pPlayer != NULL) && pPlayer->InFirstPersonView()) {
    // We sometimes need different animation for the main view versus the shadow
    // rendering, so we need to reset the cache to ensure this actually happens.
...
void CBaseWorldView::DrawExecute(float waterHeight, view_id_t viewID,
  ...
  if (m_DrawFlags & DF_DRAW_ENTITITES) {
    ...
    if (g_CurrentViewID == VIEW_MAIN && bVisionOverride &&
        pyro_dof.GetBool()) // Pyro-vision Goggles
    {
      DrawDepthOfField();
    }
#endif
...
void CBaseWorldView::SSAO_DepthPass() {
...
#if 1
  VPROF_BUDGET("CSimpleWorldView::SSAO_DepthPass",
               VPROF_BUDGETGROUP_SHADOW_DEPTH_TEXTURING);

  int savedViewID = g_CurrentViewID;
  g_CurrentViewID = VIEW_SSAO;

  ITexture *pSSAO = materials->FindTexture("_rt_ResolvedFullFrameDepth",
  ...
  if (IsPC()) {
    render->Push3DView((*this), VIEW_CLEAR_DEPTH | VIEW_CLEAR_COLOR, pSSAO,
                       GetFrustum());
  } else if (IsX360()) {
    render->Push3DView((*this), VIEW_CLEAR_DEPTH | VIEW_CLEAR_COLOR, pSSAO,
                       GetFrustum());
...
void CBaseWorldView::DrawDepthOfField() {
  ...
  IMaterial *pPyroBlurXMaterial =
      materials->FindMaterial("dev/pyro_blur_filter_x", TEXTURE_GROUP_OTHER);
  IMaterial *pPyroBlurYMaterial =
      materials->FindMaterial("dev/pyro_blur_filter_y", TEXTURE_GROUP_OTHER);

  pRenderContext->PushRenderTargetAndViewport(pSmallFB1, 0, 0, w, h);
  pRenderContext->DrawScreenSpaceRectangle(pPyroBlurYMaterial, 0, 0, w, h, 0, 0,
                                           w - 1, h - 1, w, h);
  pRenderContext->PopRenderTargetAndViewport();

  pRenderContext->PushRenderTargetAndViewport(pSmallFB0, 0, 0, w, h);
  pRenderContext->DrawScreenSpaceRectangle(pPyroBlurXMaterial, 0, 0, w, h, 0, 0,
                                           w - 1, h - 1, w, h);
  pRenderContext->PopRenderTargetAndViewport();

  IMaterial *pPyroDepthOfFieldMaterial =
      materials->FindMaterial("dev/pyro_dof", TEXTURE_GROUP_OTHER);

  pRenderContext->DrawScreenSpaceRectangle(pPyroDepthOfFieldMaterial, x, y,
                                           width, height, 0, 0, width - 1,
...
#if defined(_X360)
  pRenderContext->PushVertexShaderGPRAllocation(
      32); // lean toward pixel shader threads
#endif
