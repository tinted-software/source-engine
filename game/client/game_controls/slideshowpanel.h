...
private:
  ...
  float m_flBlend; // blend value in [0,1] where 0 maps to the source image, and
                   // 1 maps to the dest image
  bool m_bBlending; // are we currently transitioning/blending from one image to
                    // another?
...
public:
  ...
  // the slideshow panel will add "0.vmt", "1.vmt" etc. until it can't find
  // anymore images.
...
private:
  ...
  float m_flNextTransitionTime; // gpGlobals->realtime of time when we should
                                // transition next
  float m_flInterval; // Amount of time between blend begins to next image, in
                      // seconds
