...
bool AvatarIndexLessFunc(const int &lhs, const int &rhs) { return lhs < rhs; }
...
CClientScoreBoardDialog::CClientScoreBoardDialog(IViewPort *pViewPort)
  ...
  m_pPlayerList->SetVisible(
      false); // hide this until we load the images in applyschemesettings
...
void CClientScoreBoardDialog::OnThink() {
  ...
  // the scoreboard is up. That feature is impossible if this panel accepts
  // input. because if a vgui panel is up that accepts input, it prevents the
  // engine from receiving that input. So, I'm stuck with a polling solution.
  //
...
void CClientScoreBoardDialog::AddSection(int teamType, int teamNumber) {
  if (teamType == TYPE_TEAM) {
    ...
    g_pVGuiLocalize->ConstructString_safe(
        string1, g_pVGuiLocalize->Find("#Player"), 2, teamName);
    ...
    m_pPlayerList->AddColumnToSection(
        m_iSectionId, "name", string1, 0,
        scheme()->GetProportionalScaledValueEx(GetScheme(), NAME_WIDTH) -
            m_iAvatarWidth);
    m_pPlayerList->AddColumnToSection(
        m_iSectionId, "frags", "", 0,
        scheme()->GetProportionalScaledValueEx(GetScheme(), SCORE_WIDTH));
    m_pPlayerList->AddColumnToSection(
        m_iSectionId, "deaths", "", 0,
        scheme()->GetProportionalScaledValueEx(GetScheme(), DEATH_WIDTH));
    m_pPlayerList->AddColumnToSection(
        m_iSectionId, "ping", "", 0,
        scheme()->GetProportionalScaledValueEx(GetScheme(), PING_WIDTH));
    ...
    m_pPlayerList->AddColumnToSection(
        m_iSectionId, "name", "#Spectators", 0,
        scheme()->GetProportionalScaledValueEx(GetScheme(), NAME_WIDTH) -
            m_iAvatarWidth);
    m_pPlayerList->AddColumnToSection(
        m_iSectionId, "frags", "", 0,
...
//-----------------------------------------------------------------------------
bool CClientScoreBoardDialog::StaticPlayerSortFunc(
  ...
  // the same, so compare itemID's (as a sentinel value to get deterministic
  // sorts)
...
//-----------------------------------------------------------------------------
bool CClientScoreBoardDialog::GetPlayerScoreInfo(int playerIndex,
                                                 KeyValues *kv) {
  IGameResources *gr = GameResources();
...
// Purpose:
//-----------------------------------------------------------------------------
void CClientScoreBoardDialog::UpdatePlayerAvatar(int playerIndex,
                                                 KeyValues *kv) {
  // Update their avatar
  if (kv && ShowAvatars() && steamapicontext->SteamFriends() &&
    ...
    if (engine->GetPlayerInfo(playerIndex, &pi)) {
      if (pi.friendsID) {
        ...
        CAvatarImage *pAvIm =
            (CAvatarImage *)m_pImageList->GetImage(iImageIndex);
...
//			Frame, but we're an EditablePanel, need to roll our
//own.)
//-----------------------------------------------------------------------------
