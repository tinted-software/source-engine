...
CommandMenu::CommandMenu(Panel *parent, const char *panelName,
                         IViewPort *viewport)
...
CommandMenu::~CommandMenu() { ClearMenu(); }
...
void CommandMenu::StartNewSubMenu(KeyValues *params) {
  ...
  menuitem.itemnr = m_CurrentMenu->AddCascadingMenuItem(
      params->GetString("label"), this, menu,
...
void CommandMenu::AddMenuCommandItem(KeyValues *params) {
  ...
  menuitem.itemnr = m_CurrentMenu->AddMenuItem(
      params->GetString("label"), params->MakeCopy(), this, params); // add it
  m_MenuItems.AddToTail(menuitem); // add to global list
}
...
void CommandMenu::AddMenuToggleItem(KeyValues *params) {
  CommandMenuItem menuitem;      // create new menuItem
  menuitem.menu = m_CurrentMenu; // save the current menu context
  menuitem.itemnr = m_CurrentMenu->AddCheckableMenuItem(
      params->GetString("label"), params->MakeCopy(), this, params); // add it
  m_MenuItems.AddToTail(menuitem); // add to global list
}
...
KeyValues *CommandMenu::GetKeyValues() { return m_MenuKeys; }
