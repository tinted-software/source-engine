...
#ifdef TF_CLIENT_DLL
#define HUD_CLASSAUTOKILL_FLAGS                                                \
...
ConVar hud_classautokill(
    "hud_classautokill", "1", HUD_CLASSAUTOKILL_FLAGS,
...
MouseOverPanelButton *
CClassMenu::CreateNewMouseOverPanelButton(EditablePanel *panel) {
...
void CClassMenu::Reset() {
  for (int i = 0; i < GetChildCount(); ++i) {
    ...
    MouseOverPanelButton *pPanel =
        dynamic_cast<MouseOverPanelButton *>(GetChild(i));
  ...
  // Turn the first button back on again (so we have a default description
  // shown)
...
void CClassMenu::OnCommand(const char *command) {
  if (Q_stricmp(command, "vguicancel")) {
    ...
    // They entered a command to change their class, kill them so they spawn
    // with the new class right away
...
void CClassMenu::OnKeyCodePressed(KeyCode code) {
  ...
  if (m_iScoreBoardKey != BUTTON_CODE_INVALID && m_iScoreBoardKey == code) {
    ...
    int nNewArmed =
        VguiPanelNavigateSortedChildButtonList((void *)&vecSortedButtons, nDir);
