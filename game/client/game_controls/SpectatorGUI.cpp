...
ConVar cl_spec_mode("cl_spec_mode", "1",
...
CSpectatorMenu::CSpectatorMenu(IViewPort *pViewPort)
  ...
  CommandMenu *menu =
      new CommandMenu(m_pConfigSettings, "spectatormenu", gViewPortInterface);
...
void CSpectatorMenu::ApplySchemeSettings(IScheme *pScheme) {
  ...
  // need to MakeReadyForUse() on the menus so we can set their bg color before
  // they are displayed
  ...
  if (g_pSpectatorGUI) {
    m_pConfigSettings->GetMenu()->SetBgColor(
        g_pSpectatorGUI->GetBlackBarColor());
...
void CSpectatorMenu::OnTextChanged(KeyValues *data) {
  ...
  if (box ==
      m_pConfigSettings) // don't change the text in the config setting combo
  {
    m_pConfigSettings->SetText("#Spec_Options");
    ...
    if (kv && GameResources()) {
      ...
      const char *currentPlayerName =
          GameResources()->GetPlayerName(currentPlayerNum);
...
void CSpectatorMenu::ShowPanel(bool bShow) {
  ...
  if (engine->IsHLTV() && HLTVCamera()->IsPVSLocked()) {
    // when watching HLTV or Replay with a locked PVS, some elements are
    // disabled
    bIsEnabled = false;
  }
...
void CSpectatorMenu::Update(void) {
  ...
  int iPlayerIndex;
  for (iPlayerIndex = 1; iPlayerIndex <= gpGlobals->maxClients;
       iPlayerIndex++) {
    ...
    g_pVGuiLocalize->ConvertANSIToUnicode(
        UTIL_SafeName(gr->GetPlayerName(iPlayerIndex)), playerName,
        sizeof(playerName));
    ...
    if (teamname) {
      ...
      g_pVGuiLocalize->ConstructString(
          playerText, sizeof(playerText),
          g_pVGuiLocalize->Find("#Spec_PlayerItem_Team"), 2, playerName, team);
    } else {
      g_pVGuiLocalize->ConstructString(
          playerText, sizeof(playerText),
          g_pVGuiLocalize->Find("#Spec_PlayerItem"), 1, playerName);
    }

    Q_snprintf(szPlayerIndex, sizeof(szPlayerIndex), "%d", iPlayerIndex);

    KeyValues *kv =
        new KeyValues("UserData", "player", gr->GetPlayerName(iPlayerIndex),
                      "index", szPlayerIndex);
  ...
  int playernum = GetSpectatorTarget();
  const char *selectedPlayerName = gr->GetPlayerName(playernum);
  for (iPlayerIndex = 0; iPlayerIndex < m_pPlayerList->GetItemCount();
       ++iPlayerIndex) {
...
// Purpose: Destructor
//-----------------------------------------------------------------------------
CSpectatorGUI::~CSpectatorGUI() { g_pSpectatorGUI = NULL; }
...
//-----------------------------------------------------------------------------
// Purpose: checks spec_scoreboard cvar to see if the scoreboard should be
// displayed
//-----------------------------------------------------------------------------
...
void CSpectatorGUI::Update() {
  ...
  IViewPortPanel *overview =
      gViewPortInterface->FindPanelByName(PANEL_OVERVIEW);
  ...
  if (playernum > 0 && playernum <= gpGlobals->maxClients && gr) {
    ...
    V_wcsncpy(playerText, L"Unable to find #Spec_PlayerItem*",
              sizeof(playerText));
    memset(playerName, 0x0, sizeof(playerName));

    g_pVGuiLocalize->ConvertANSIToUnicode(
        UTIL_SafeName(gr->GetPlayerName(playernum)), playerName,
        sizeof(playerName));
    ...
    if (iHealth > 0 && gr->IsAlive(playernum)) {
      ...
      g_pVGuiLocalize->ConstructString(
          playerText, sizeof(playerText),
          g_pVGuiLocalize->Find("#Spec_PlayerItem_Team"), 2, playerName,
          health);
    } else {
      g_pVGuiLocalize->ConstructString(
          playerText, sizeof(playerText),
  ...
  if (engine->IsHLTV()) {
    ...
    Q_snprintf(tempstr, sizeof(tempstr), "Spectators : %d",
               HLTVCamera()->GetNumSpectators());
    ...
    g_pVGuiLocalize->ConstructString(szEtxraInfo, sizeof(szEtxraInfo),
                                     g_pVGuiLocalize->Find("#Spec_Map"), 1,
                                     wMapName);
...
CON_COMMAND_F(spec_prev, "Spectate previous player",
...
CON_COMMAND_F(spec_player, "Spectate player by name",
              FCVAR_CLIENTCMD_CAN_EXECUTE) {
  C_BasePlayer *pPlayer = C_BasePlayer::GetLocalPlayer();
  ...
  if (engine->IsHLTV()) {
    // we can only switch primary spectator targets is PVS isnt locked by
    // auto-director
    if (!HLTVCamera()->IsPVSLocked()) {
