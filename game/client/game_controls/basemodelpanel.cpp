...
void CModelPanel::ParseModelInfo(KeyValues *inResourceData) {
  ...
  m_pModelInfo->m_pszModelName =
      ReadAndAllocStringValue(inResourceData, "modelname");
  m_pModelInfo->m_pszModelName_HWM =
  ...
  for (KeyValues *pData = inResourceData->GetFirstSubKey(); pData != NULL;
    ...
    if (!Q_stricmp(pData->GetName(), "animation")) {
      ...
      CModelPanelAttachedModelInfo *pAttachedModelInfo =
          new CModelPanelAttachedModelInfo;

      if (pAttachedModelInfo) {
        pAttachedModelInfo->m_pszModelName =
            ReadAndAllocStringValue(pData, "modelname");
...
void CModelPanel::OnAddAnimation(KeyValues *pData) {
  ...
  if (pAnimation) {
    ...
    for (KeyValues *pAnimData = pData->GetFirstSubKey(); pAnimData != NULL;
         pAnimData = pAnimData->GetNextKey()) {
...
void CModelPanel::SetDefaultAnimation(const char *pszName) {
  if (m_pModelInfo) {
    for (int i = 0; i < m_pModelInfo->m_Animations.Count(); i++) {
      if (m_pModelInfo->m_Animations[i] &&
          m_pModelInfo->m_Animations[i]->m_pszName) {
...
//-----------------------------------------------------------------------------
// Purpose: Replaces the current model with a new one, without changing the
// camera settings
//-----------------------------------------------------------------------------
...
const char *CModelPanel::GetModelName(void) {
  ...
  if (UseHWMorphModels()) {
    ...
    if (m_pModelInfo->m_pszModelName_HWM &&
        (Q_strlen(m_pModelInfo->m_pszModelName_HWM) > 0)) {
      // does the file exist
      model_t *pModel =
          (model_t *)engine->LoadModel(m_pModelInfo->m_pszModelName_HWM);
      if (pModel) {
...
void CModelPanel::SetupModel(void) {
  ...
  if (pEnt->InitializeAsClientEntity(pszModelName,
  ...
  FOR_EACH_MAP_FAST(m_pModelInfo->m_mapBodygroupValues, i) {
    pEnt->SetBodygroup(m_pModelInfo->m_mapBodygroupValues.Key(i),
                       m_pModelInfo->m_mapBodygroupValues[i]);
  }

  // do we have any animation information?
  if (m_pModelInfo->m_Animations.Count() > 0 &&
      m_pModelInfo->m_Animations.IsValidIndex(m_iDefaultAnimation)) {
    CModelPanelModelAnimation *pAnim =
        m_pModelInfo->m_Animations[m_iDefaultAnimation];
    int sequence = ACT_INVALID;
    if (pAnim->m_pszActivity && pAnim->m_pszActivity[0]) {
      Activity activity =
          (Activity)ActivityList_IndexForName(pAnim->m_pszActivity);
    ...
    if (sequence != ACT_INVALID) {
      ...
      if (pAnim->m_pPoseParameters) {
        for (KeyValues *pData = pAnim->m_pPoseParameters->GetFirstSubKey();
             pData != NULL; pData = pData->GetNextKey()) {
  ...
  for (int i = 0; i < m_pModelInfo->m_AttachedModelsInfo.Count(); i++) {
    ...
    if (pTemp) {
      if (pTemp->InitializeAsClientEntity(
              pInfo->m_pszModelName, RENDER_GROUP_OPAQUE_ENTITY) == false) {
...
void CModelPanel::Paint() {
  ...
  if (Q_strstr(GetModelName(), "models/player/")) {
    ...
    // HACK! HACK! to get our player models to appear the way they do in 4/3 if
    // we're using other aspect ratios
  ...
  m_hModel->SetAbsAngles(QAngle(m_pModelInfo->m_vecAbsAngles.x,
                                m_pModelInfo->m_vecAbsAngles.y,
  ...
  pRenderContext->GetViewport(viewportX, viewportY, viewportWidth,
                              viewportHeight);

  // Now draw it.
  CViewSetup view;
  view.x = x + m_pModelInfo->m_vecViewportOffset.x +
           viewportX; // we actually want to offset by the
  view.y = y + m_pModelInfo->m_vecViewportOffset.y +
           viewportY; // viewport origin here because Push3DView expects global
                      // coords below
  view.width = w;
  view.height = h;
  ...
  if (m_pModelInfo->m_bUseSpotlight) {
    ...
    LightDesc_t spotLight(vec3_origin + Vector(0, 0, 200), Vector(1, 1, 1),
                          m_hModel->GetAbsOrigin() +
                              Vector(0, 0, (vecMaxs.z - vecMins.z) * 0.75),
                          0.035, 0.873);
...
void CModelPanel::CalculateFrameDistanceInternal(const model_t *pModel) {
  ...
  QAngle angPanelAngles(m_pModelInfo->m_vecAbsAngles.x,
                        m_pModelInfo->m_vecAbsAngles.y,
                        m_pModelInfo->m_vecAbsAngles.z);
  ...
  // Offset the view port based on the calculated model 2D center and the center
  // of the viewport.
...
//-----------------------------------------------------------------------------
// Purpose: Calculates the distance the camera should be at to frame the model
// on the screen.
//-----------------------------------------------------------------------------
