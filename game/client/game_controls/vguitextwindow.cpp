...
void CTextWindow::Reset(void) {
  //=============================================================================
  // HPE_BEGIN:
  // [Forrest] Replace strange hard-coded default message with hard-coded error
  // message.
  //=============================================================================
  ...
  if ((bAllowUserToDisable && cl_disablehtmlmotd.GetBool()) ||
    ...
    if (g_pStringTableInfoPanel) {
      ...
      if (index != ::INVALID_STRING_INDEX) {
        int length = 0;
        const char *data =
            (const char *)g_pStringTableInfoPanel->GetStringUserData(index,
                                                                     &length);
        if (data && data[0]) {
...
void CTextWindow::ShowIndex(const char *entry) {
  ...
  if (index != ::INVALID_STRING_INDEX)
    data = (const char *)g_pStringTableInfoPanel->GetStringUserData(index,
                                                                    &length);
  ...
  FileHandle_t hFile =
      g_pFullFileSystem->Open(TEMP_HTML_FILE, "wb", "DEFAULT_WRITE_PATH");
...
void CTextWindow::ShowFile(const char *filename) {
  if (Q_stristr(filename, ".htm") || Q_stristr(filename, ".html")) {
    ...
    int size =
        MIN(g_pFullFileSystem->Size(f), sizeof(buffer) - 1); // just allow 2KB
...
void CTextWindow::Update(void) {
  ...
  if (m_nContentType == TYPE_INDEX) {
    ...
    if (!Q_strncmp(m_szMessage, "http://", 7) ||
        !Q_strncmp(m_szMessage, "https://", 8) ||
...
bool CTextWindow::CMOTDHTML::OnStartRequest(const char *url, const char *target,
