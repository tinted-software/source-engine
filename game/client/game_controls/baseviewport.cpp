...
void hud_autoreloadscript_callback(IConVar *var, const char *pOldValue,
                                   float flOldValue);
...
void hud_autoreloadscript_callback(IConVar *var, const char *pOldValue,
  ...
  if (g_pClientMode && g_pClientMode->GetViewportAnimationController()) {
    g_pClientMode->GetViewportAnimationController()->SetAutoReloadScript(
        hud_autoreloadscript.GetBool());
  }
}
...
// Check if we have any visible panel (that's not the MainMenuOverride or the
// Scoreboard)
bool CBaseViewport::IsAnyPanelVisibleExceptScores() {
  ...
  for (int i = 0; i < count; i++) {
    ...
    if (p->IsVisible() && Q_strcmp("MainMenuOverride", p->GetName()) &&
        Q_strcmp("scores", p->GetName())) {
      return true;
    }
  }
...
IViewPortPanel *CBaseViewport::GetActivePanel(void) { return m_pActivePanel; }
