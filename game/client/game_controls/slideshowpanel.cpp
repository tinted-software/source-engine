...
CCrossfadableImagePanel::CCrossfadableImagePanel(Panel *pParent,
                                                 const char *pName)
    : EditablePanel(pParent, pName), m_iSrcImg(0), m_flBlend(0.0f),
      m_flBlendTime(0.0f), m_flStartBlendTime(0.0f), m_bBlending(false) {
...
void CCrossfadableImagePanel::SetImage(IImage *pImage,
                                       float flBlendTime /*=0.0f*/) {
  DstImg()->SetImage(pImage);
  SetupImageBlend(flBlendTime);
}

void CCrossfadableImagePanel::SetImage(const char *pImageName,
                                       float flBlendTime /*=0.0f*/) {
...
void CCrossfadableImagePanel::OnTick() {
  if (m_bBlending) {
    ...
    if (m_flBlendTime == 0.0f) {
      ...
      float t =
          clamp((gpGlobals->realtime - m_flStartBlendTime) / m_flBlendTime,
                0.0f, 1.0f);
...
CSlideshowPanel::CSlideshowPanel(Panel *pParent, const char *pName)
    : EditablePanel(pParent, pName), m_iCurImg(0), m_flInterval(3.0f),
...
void CSlideshowPanel::AddImage(const char *pImageName) {
  if (pImageName && strlen(pImageName) > 0) {
    AddImage(
        scheme()->GetImage(pImageName, m_pImagePanel->GetShouldScaleImage()));
  }
}
...
void CSlideshowPanel::ApplySettings(KeyValues *pInResourceData) {
  ...
  while (1) {
    ...
    const char *pImagePath =
        pInResourceData->GetString(fmtImageKeyName.Access(), NULL);
    if (!pImagePath)
      break;
  ...
  GetImagePanel()->SetSize(XRES(pInResourceData->GetInt("wide")),
