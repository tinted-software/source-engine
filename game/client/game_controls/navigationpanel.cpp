...
public:
  CNavButton(Panel *parent, const char *name, const char *text = "",
             Panel *pActionSignalTarget = NULL, const char *cmd = NULL)
      : CExImageButton(parent, name, text, pActionSignalTarget, cmd),
        m_iUserData(-1) {}

  CNavButton(Panel *parent, const char *name, const wchar_t *wszText = L"",
             Panel *pActionSignalTarget = NULL, const char *cmd = NULL)
      : CExImageButton(parent, name, wszText, pActionSignalTarget, cmd),
        m_iUserData(-1) {}
  ...
  virtual void ApplySettings(KeyValues *pInResourceData) {
    ...
    if (m_iUserData < 0) {
      ...
      AssertMsg(
          m_iUserData != -1,
          "Any messages sent for this button will have invalid user data!");
    }
  }
...
CNavigationPanel::CNavigationPanel(
    Panel *pParent, const char *pName,
    bool bAddParentAsActionSignalTarget /*=true*/)
    : BaseClass(pParent, pName), m_bAutoLayout(false), m_bAutoScale(true),
      m_bDisplayVertical(false), m_iSelectedButton(0),
...
void CNavigationPanel::AddButton(int iUserData, const char *pTextToken) {
  ...
  CNavButton *pNewButton =
      new CNavButton(this, CFmtStr("button_%i", i).Access(), pTextToken);
...
void CNavigationPanel::ApplySettings(KeyValues *pInResourceData) {
  ...
  */
  ...
  if (!pKVButtonSettings) {
    AssertMsg(0, "No button settings specified.  CNavigationPanel is useless "
                 "without this data.");
    return;
  }
  ...
  FOR_EACH_SUBKEY(pKVButtons, pKVCurButton) {
    CNavButton *pNewButton = new CNavButton(
        this, pKVCurButton->GetString("FieldName", pKVCurButton->GetName()),
        L"");
...
void CNavigationPanel::PerformLayout() {
  ...
  if (m_bAutoScale) {
    ...
    if (m_bDisplayVertical) {
      nWidth = GetWide();
      nHeight =
          nSettingHeight * nWidth / (nSettingWidth > 0 ? nSettingWidth : 1);
    } else {
      nHeight = GetTall();
      nWidth =
          nSettingWidth * nHeight / (nSettingHeight > 0 ? nSettingHeight : 1);
    }
  ...
  FOR_EACH_VEC(m_vecButtons, i) {
    ...
    if (m_bAutoLayout) {
      ...
      if (m_bDisplayVertical) {
        ...
        const int nStartX = 0; // 0.5f * ( GetWide() - NumButtons() * (
                               // nButtonWidth + m_nHorizontalBuffer ) );
        m_vecButtons[i]->SetPos(
            nStartX + i * (nButtonWidth + m_nHorizontalBuffer), 0);
      }
    }
...
CON_COMMAND(open_navpanel_test, "") {
  ...
  pPanel->SetBounds((ScreenWidth() - nWidth) / 2,
                    (ScreenHeight() - nHeight) / 2, nWidth, nHeight);
