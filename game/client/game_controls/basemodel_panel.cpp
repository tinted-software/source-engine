...
CBaseModelPanel::CBaseModelPanel(vgui::Panel *pParent, const char *pName)
    : BaseClass(pParent, pName), m_nActiveSequence(ACT_INVALID),
...
void CBaseModelPanel::ApplySettings(KeyValues *inResourceData) {
  ...
  for (KeyValues *pData = inResourceData->GetFirstSubKey(); pData != NULL;
...
void CBaseModelPanel::ParseModelAnimInfo(KeyValues *inResourceData) {
  ...
  m_BMPResData.m_aAnimations[iAnim].m_pszName =
      ReadAndAllocStringValue(inResourceData, "name");
  m_BMPResData.m_aAnimations[iAnim].m_pszSequence =
      ReadAndAllocStringValue(inResourceData, "sequence");
  m_BMPResData.m_aAnimations[iAnim].m_pszActivity =
      ReadAndAllocStringValue(inResourceData, "activity");
  m_BMPResData.m_aAnimations[iAnim].m_bDefault =
      inResourceData->GetBool("default");

  for (KeyValues *pAnimData = inResourceData->GetFirstSubKey();
       pAnimData != NULL; pAnimData = pAnimData->GetNextKey()) {
    if (!Q_stricmp(pAnimData->GetName(), "pose_parameters")) {
      m_BMPResData.m_aAnimations[iAnim].m_pPoseParameters =
          pAnimData->MakeCopy();
    }
  }
}
...
void CBaseModelPanel::ParseModelAttachInfo(KeyValues *inResourceData) {
  ...
  m_BMPResData.m_aAttachModels[iAttach].m_pszModelName =
      ReadAndAllocStringValue(inResourceData, "modelname");
  m_BMPResData.m_aAttachModels[iAttach].m_nSkin =
      inResourceData->GetInt("skin", -1);
}

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CBaseModelPanel::SetupModelDefaults(void) { SetupModelAnimDefaults(); }
...
// Purpose:
//-----------------------------------------------------------------------------
int CBaseModelPanel::FindSequenceFromActivity(CStudioHdr *pStudioHdr,
...
void CBaseModelPanel::SetModelAnim(int iAnim) {
  ...
  if (m_BMPResData.m_aAnimations[iAnim].m_pszActivity &&
      m_BMPResData.m_aAnimations[iAnim].m_pszActivity[0]) {
    iSequence = FindSequenceFromActivity(
        &studioHdr, m_BMPResData.m_aAnimations[iAnim].m_pszActivity);
  } else if (m_BMPResData.m_aAnimations[iAnim].m_pszSequence &&
             m_BMPResData.m_aAnimations[iAnim].m_pszSequence[0]) {
    iSequence = LookupSequence(&studioHdr,
                               m_BMPResData.m_aAnimations[iAnim].m_pszSequence);
  }
...
void CBaseModelPanel::SetMDL(MDLHandle_t handle, void *pProxyData) {
  ...
  if (pHdr) {
    // SetMDL will cause the base CMdl code to set our localtoglobal indices if
    // they aren't set. We set them up here so that they're left alone by that
    // code.
    CStudioHdr studioHdr(pHdr, g_pMDLCache);
    if (studioHdr.numflexcontrollers() > 0 &&
        studioHdr.pFlexcontroller(LocalFlexController_t(0))->localToGlobal ==
            -1) {
      for (LocalFlexController_t i = LocalFlexController_t(0);
           i < studioHdr.numflexcontrollers(); i++) {
        int j = C_BaseFlex::AddGlobalFlexController(
            studioHdr.pFlexcontroller(i)->pszName());
  ...
  // Need to invalidate the layout so the panel will adjust is LookAt for the
  // new model.
  InvalidateLayout();
}
...
// Purpose:
//-----------------------------------------------------------------------------
void CBaseModelPanel::SetModelAnglesAndPosition(const QAngle &angRot,
                                                const Vector &vecPos) {
...
void CBaseModelPanel::PerformLayout() {
  ...
  if (m_bAllowFullManipulation) {
    // Set this to true if you want to keep the current rotation when changing
    // models or poses
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
Vector CBaseModelPanel::GetPlayerPos() const { return m_vecPlayerPos; }

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
QAngle CBaseModelPanel::GetPlayerAngles() const { return m_angPlayer; }
...
//-----------------------------------------------------------------------------
// Purpose: Set the camera to a distance that allows the object to fill the
// model panel.
//-----------------------------------------------------------------------------
void CBaseModelPanel::LookAtBounds(const Vector &vecBoundsMin,
  ...
  // Get the bounds points and transform them by the desired model panel
  // rotation.
  ...
  for (int iPoint = 0; iPoint < 8; ++iPoint) {
    float flDistY =
        fabs(aXFormPoints[iPoint].y / flTanFOVx) - aXFormPoints[iPoint].x;
    float flDistZ =
        fabs(aXFormPoints[iPoint].z / flTanFOVy) - aXFormPoints[iPoint].x;
  ...
  for (int iPoint = 0; iPoint < 8; ++iPoint) {
    ...
    aScreenPoints[iPoint].x =
        aCameraPoints[iPoint].y / (flTanFOVx * aCameraPoints[iPoint].x);
    aScreenPoints[iPoint].y =
        aCameraPoints[iPoint].z / (flTanFOVy * aCameraPoints[iPoint].x);
...
//-----------------------------------------------------------------------------
void CBaseModelPanel::particle_data_t::UpdateControlPoints(
    CStudioHdr *pStudioHdr, matrix3x4_t *pWorldMatrix,
  ...
  if (m_pParticleSystem) {
    ...
    if (vecAttachments.Count()) {
      for (int i = 0; i < vecAttachments.Count(); ++i) {
        const mstudioattachment_t &attach =
            pStudioHdr->pAttachment(vecAttachments[i]);
        MatrixMultiply(pWorldMatrix[attach.localbone], attach.local,
                       matAttachToWorld);

        MatrixVectors(matAttachToWorld, &vecForward, &vecRight, &vecUp);
        MatrixPosition(matAttachToWorld, vecPosition);

        m_pParticleSystem->SetControlPointOrientation(i, vecForward, vecRight,
                                                      vecUp);
      ...
      m_pParticleSystem->SetControlPointOrientation(0, vecForward, vecRight,
                                                    vecUp);
...
// Purpose: Allocate particle data
//-----------------------------------------------------------------------------
CBaseModelPanel::particle_data_t *
CBaseModelPanel::CreateParticleData(const char *pszParticleName) {
  Assert(m_bUseParticle);
  if (!m_bUseParticle)
    return NULL;

  CParticleCollection *pParticle =
      g_pParticleSystemMgr->CreateParticleCollection(pszParticleName);
  if (!pParticle)
    return NULL;
