...
protected:                  // private structures & types
  float GetViewAngle(void); // The angle that determines the viewport twist from
                            // map texture to panel drawing.
...
public:
  ...
  // Object settings
  virtual int
  AddObject(const char *icon, int entity,
...
protected:
  ...
  /*virtual bool	DrawIcon(	int textureID,
                                                          int
     offscreenTextureID, Vector pos, float scale, float angle, int alpha = 255,
                                                          const char *text =
     NULL, Color *textColor = NULL, float status = -1, Color *statusColor =
  ...
  bool m_bRotateMap; // if true roatate map around 90 degress, so it fits better
                     // to 4:3 screen ratio
