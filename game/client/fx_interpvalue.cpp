...
CInterpolatedValue::CInterpolatedValue(void)
    : m_flStartTime(0.0f), m_flEndTime(0.0f), m_flStartValue(0.0f),
      m_flEndValue(0.0f), m_nInterpType(INTERP_LINEAR) {}

CInterpolatedValue::CInterpolatedValue(float startTime, float endTime,
                                       float startValue, float endValue,
                                       InterpType_t type)
    : m_flStartTime(startTime), m_flEndTime(endTime),
      m_flStartValue(startValue), m_flEndValue(endValue), m_nInterpType(type) {}
...
void CInterpolatedValue::SetType(InterpType_t type) { m_nInterpType = type; }
...
void CInterpolatedValue::Init(float startValue, float endValue, float dt,
...
void CInterpolatedValue::InitFromCurrent(
    float endValue, float dt, InterpType_t type /*= INTERP_LINEAR*/) {
...
float CInterpolatedValue::Interp(float curTime) {
  ...
  case INTERP_LINEAR: {
    ...
    return RemapVal(curTime, m_flStartTime, m_flEndTime, m_flStartValue,
                    m_flEndValue);
  }
  ...
  case INTERP_SPLINE: {
    ...
    return SimpleSplineRemapVal(curTime, m_flStartTime, m_flEndTime,
                                m_flStartValue, m_flEndValue);
  }
  }
