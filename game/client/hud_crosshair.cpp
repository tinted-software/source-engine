...
#ifdef TF_CLIENT_DLL
// If running TF, we use CHudTFCrosshair instead (which is derived from
// CHudCrosshair)
#else
...
bool CHudCrosshair::ShouldDraw(void) {
  ...
  */
  ...
  if (IsX360()) {
    bNeedsDraw =
        m_pCrosshair && !engine->IsDrawingLoadingImage() &&
        !engine->IsPaused() &&
    ...
    bNeedsDraw = m_pCrosshair && crosshair.GetInt() &&
                 !engine->IsDrawingLoadingImage() && !engine->IsPaused() &&
                 g_pClientMode->ShouldDrawCrosshair() &&
                 !(pPlayer->GetFlags() & FL_FROZEN) &&
                 (pPlayer->entindex() == render->GetViewEntity()) &&
...
void CHudCrosshair::GetDrawPosition(float *pX, float *pY, bool *pbBehindCamera,
  ...
  if ((pPlayer != NULL) && (pPlayer->GetObserverMode() == OBS_MODE_NONE)) {
    ...
    if (UseVR()) {
      // These are the correct values to use, but they lag the high-speed view
      // data...
      vecStart = pPlayer->Weapon_ShootPosition();
      Vector vecAimDirection = pPlayer->GetAutoaimVector(1.0f);
      // ...so in some aim modes, they get zapped by something completely
      // up-to-date.
      g_ClientVirtualReality.OverrideWeaponHudAimVectors(&vecStart,
                                                         &vecAimDirection);
...
#ifdef SIXENSE
    // TODO: actually test this Sixsense code interaction with things like HMDs
    // & stereo.
    if (g_pSixenseInput->IsEnabled() && !UseVR()) {
      // Never autoaim a predicted weapon (for now)
      vecStart = pPlayer->Weapon_ShootPosition();
      Vector aimVector;
      AngleVectors(CurrentViewAngles() - g_pSixenseInput->GetViewAngleOffset(),
                   &aimVector);
      // calculate where the bullet would go so we can draw the cross
      // appropriately
    ...
    if (bUseOffset) {
      trace_t tr;
      UTIL_TraceLine(vecStart, vecEnd, MASK_SHOT, pPlayer, COLLISION_GROUP_NONE,
                     &tr);
...
void CHudCrosshair::Paint(void) {
  ...
  if (!m_pCrosshair->bRenderUsingFont)
    ...
    flPlayerScale = 1.0f;
#ifdef TF_CLIENT_DLL
  Color clr(cl_crosshair_red.GetInt(), cl_crosshair_green.GetInt(),
            cl_crosshair_blue.GetInt(), 255);
  flPlayerScale =
      cl_crosshair_scale.GetFloat() /
      32.0f; // the player can change the scale in the options/multiplayer tab
#else
  ...
  m_pCrosshair->DrawSelfCropped(iX - (iWidth / 2), iY - (iHeight / 2), 0, 0,
                                iTextureW, iTextureH, iWidth, iHeight, clr);
}
