...
protected:
  ...
  float m_duckTimeout; /// HUD_POSTURE_FADE_TIME after the last known time the
                       /// player was ducking
...
private:
  ...
  enum { NOT_FADING, FADING_UP, FADING_DOWN } m_kIsFading;
#endif
};
...
CHudPosture::CHudPosture(const char *pElementName)
  ...
  if (IsX360()) {
    vgui::ivgui()->AddTickSignal(GetVPanel(),
                                 (1000 / HUD_POSTURE_UPDATES_PER_SECOND));
  }
}
...
void CHudPosture::OnTick(void) {
  ...
  if (PlayerIsDucking(pPlayer)) {
    m_duckTimeout =
        gpGlobals->curtime + HUD_POSTURE_FADE_TIME; // kick the timer forward
    if (GetAlpha() < 255) {
      ...
      if (m_kIsFading != FADING_UP) {
        ...
        GetAnimationController()->RunAnimationCommand(
            this, "alpha", 255, 0, HUD_POSTURE_FADE_TIME,
  ...
  } else // player is not ducking
  {
    if (GetAlpha() > 0) {
      ...
      if (m_kIsFading != FADING_DOWN) {
        ...
        GetAnimationController()->RunAnimationCommand(
            this, "alpha", 0, 0, HUD_POSTURE_FADE_TIME,
            vgui::AnimationController::INTERPOLATOR_SIMPLESPLINE);
      }
    } else {
      m_kIsFading = NOT_FADING;
