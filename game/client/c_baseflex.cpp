...
bool C_BaseFlex::GetSoundSpatialization(SpatializationInfo_t &info) {
  ...
  if (bret) {
    if ((info.info.nChannel == CHAN_VOICE ||
         info.info.nChannel == CHAN_VOICE2) &&
        m_iMouthAttachment > 0) {
      Vector origin;
      QAngle angles;
...
//-----------------------------------------------------------------------------
// Purpose: run the interpreted FAC's expressions, converting global
// flex_controller
...
//-----------------------------------------------------------------------------
void CFlexSceneFileManager::Shutdown() { DeleteSceneFiles(); }
...
// Output : 	void
//-----------------------------------------------------------------------------
void CFlexSceneFileManager::EnsureTranslations(
    IHasLocalToGlobalFlexSettings *instance,
    const flexsettinghdr_t *pSettinghdr) {
  // The only time instance is NULL is in Init() above, where we're just loading
  // the .vfe files off of the hard disk.
  if (instance) {
...
//-----------------------------------------------------------------------------
void *
CFlexSceneFileManager::FindSceneFile(IHasLocalToGlobalFlexSettings *instance,
                                     const char *filename,
  ...
  for (i = 0; i < m_FileList.Count(); i++) {
    ...
    if (file && !Q_stricmp(file->filename, szFilename)) {
      // Make sure translations (local to global flex controller) are set up for
      // this instance
  ...
  if (IsX360()) {
    ...
    for (int i = 0; i < pHdr->numflexsettings; ++i, ++pFlexSetting) {
      ...
      flexweight_t *pWeight =
          (flexweight_t *)(((byte *)pFlexSetting) + pFlexSetting->settingindex);
        ...
        if (k < pcount - 1) {
          ...
          if (next) {
            ...
            if (next->GetStartTime() == phoneme->GetEndTime()) {
              ...
              // phoneme, as long as it's not longer than the current phoneme
              dt =
                  MAX(dt, MIN(next->GetEndTime() - t,
                              phoneme->GetEndTime() - phoneme->GetStartTime()));
            } else {
              // dead space, so increase the blend length to the start of the
              // next phoneme, as long as it's not longer than the current
              // phoneme
              dt =
                  MAX(dt, MIN(next->GetStartTime() - t,
                              phoneme->GetEndTime() - phoneme->GetStartTime()));
            }
          } else {
            // last phoneme in list, increase the blend length to the length of
            // the current phoneme
...
void C_BaseFlex::LinkToGlobalFlexControllers(CStudioHdr *hdr) {
  if (hdr &&
      hdr->pFlexcontroller(LocalFlexController_t(0))->localToGlobal == -1) {
...
//-----------------------------------------------------------------------------
// Purpose: Rendering callback to allow the client to set up all the model
// specific flex weights
//-----------------------------------------------------------------------------
void C_BaseFlex::SetupWeights(const matrix3x4_t *pBoneToWorld,
  ...
  if (SetupGlobalWeights(pBoneToWorld, nFlexWeightCount, pFlexWeights,
                         pFlexDelayedWeights)) {
    SetupLocalWeights(pBoneToWorld, nFlexWeightCount, pFlexWeights,
                      pFlexDelayedWeights);
  }
}
...
//-----------------------------------------------------------------------------
void C_BaseFlex::BuildTransformations(CStudioHdr *pStudioHdr, Vector *pos,
                                      Quaternion q[],
  ...
  for (int i = 0; i < nBoneFlexDriverCount; ++i) {
    ...
    for (int j = 0; j < nControllerCount; ++j) {
      ...
      SetFlexWeight(static_cast<LocalFlexController_t>(
                        pController->m_nFlexControllerIndex),
                    RemapValClamped(position[pController->m_nBoneComponent],
                                    pController->m_flMin, pController->m_flMax,
                                    0.0f, 1.0f));
    }
  }
...
//-----------------------------------------------------------------------------
// Purpose: process the entities networked state, vcd playback, wav file
// visemes, and blinks into a global shared flex controller array
//-----------------------------------------------------------------------------
bool C_BaseFlex::SetupGlobalWeights(const matrix3x4_t *pBoneToWorld,
                                    int nFlexWeightCount, float *pFlexWeights,
                                    float *pFlexDelayedWeights) {
  CStudioHdr *hdr = GetModelPtr();
  if (!hdr)
...
void C_BaseFlex::RunFlexDelay(int nFlexWeightCount, float *pFlexWeights,
                              float *pFlexDelayedWeights,
  ...
  if (flFlexDelayTime > 0.0f && flFlexDelayTime < gpGlobals->curtime) {
    float d =
        clamp(gpGlobals->curtime - flFlexDelayTime, 0.0, gpGlobals->frametime);
    d = ExponentialDecay(0.8, 0.033, d);

    for (int i = 0; i < nFlexWeightCount; i++) {
      pFlexDelayedWeights[i] =
          pFlexDelayedWeights[i] * d + pFlexWeights[i] * (1.0f - d);
    }
  }
...
//-----------------------------------------------------------------------------
void C_BaseFlex::SetupLocalWeights(const matrix3x4_t *pBoneToWorld,
                                   int nFlexWeightCount, float *pFlexWeights,
                                   float *pFlexDelayedWeights) {
  CStudioHdr *hdr = GetModelPtr();
  if (!hdr)
  ...
  // BUGBUG: We have a bug with SetCustomModel that causes a disagreement
  // between the studio header here and the one used in l_studio.cpp
  // CModelRender::DrawModelExecute So when we hit that case, let's not do any
  // work because otherwise we'd crash since the array sizes
  // (m_flFlexDelayedWeight vs pFlexWeights) don't match. Note that this check
  // is duplicated in CEconEntity::SetupWeights.
  AssertMsg(nFlexWeightCount == m_cFlexDelayedWeight,
            "Disagreement between the number of flex weights. Do the studio "
            "headers match?");
  ...
  AssertOnce(hdr->pFlexcontroller(LocalFlexController_t(0))->localToGlobal !=
             -1);

  if (pFlexDelayedWeights) {
    RunFlexDelay(nFlexWeightCount, pFlexWeights, m_flFlexDelayedWeight,
                 m_flFlexDelayTime);
    memcpy(pFlexDelayedWeights, m_flFlexDelayedWeight,
           sizeof(float) * nFlexWeightCount);
  }
...
const flexsetting_t *
C_BaseFlex::FindNamedSetting(const flexsettinghdr_t *pSettinghdr,
                             const char *expr) {
  int i;
    ...
    for (int i = 0; i < event->GetNumFlexAnimationTracks(); i++) {
      ...
      if (track->IsComboType()) {
        ...
        track->SetFlexControllerIndex(
            MAX(FindFlexController(name), LocalFlexController_t(0)), 0, 0);
        ...
        track->SetFlexControllerIndex(
            MAX(FindFlexController(name), LocalFlexController_t(0)), 0, 1);
      } else {
        track->SetFlexControllerIndex(
            MAX(FindFlexController((char *)track->GetFlexControllerName()),
...
DEFINE_FIELD(id, FIELD_INTEGER), DEFINE_FIELD(version, FIELD_INTEGER),
    ...
    DEFINE_FIELD(index, FIELD_INTEGER), DEFINE_FIELD(obsolete2, FIELD_INTEGER),
    DEFINE_FIELD(settingindex, FIELD_INTEGER),
    END_BYTESWAP_DATADESC()

        BEGIN_BYTESWAP_DATADESC(flexweight_t) DEFINE_FIELD(key, FIELD_INTEGER),
    DEFINE_FIELD(weight, FIELD_FLOAT), DEFINE_FIELD(influence, FIELD_FLOAT),
    END_BYTESWAP_DATADESC()
