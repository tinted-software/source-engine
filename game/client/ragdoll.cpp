...
bool CRagdoll::TransformVectorToWorld(int iBoneIndex, const Vector *vPosition,
                                      Vector *vOut) {
  int listIndex = -1;
...
static ConVar
    ragdoll_sleepaftertime("ragdoll_sleepaftertime", "5.0f", 0,
                           "After this many seconds of being basically "
...
CRagdoll *CreateRagdoll(C_BaseEntity *ent, CStudioHdr *pstudiohdr,
  ...
  pRagdoll->Init(ent, pstudiohdr, forceVector, forceBone, pDeltaBones0,
                 pDeltaBones1, pCurrentBonePosition, dt, bFixedConstraints);
...
void C_ServerRagdoll::AccumulateLayers(IBoneSetup &boneSetup, Vector pos[],
                                       Quaternion q[], float currentTime) {
  BaseClass::AccumulateLayers(boneSetup, pos, q, currentTime);

  if (m_nOverlaySequence >= 0 &&
      m_nOverlaySequence < boneSetup.GetStudioHdr()->GetNumSeq()) {
    boneSetup.AccumulatePose(pos, q, m_nOverlaySequence, GetCycle(),
...
RecvPropInt(RECVINFO(m_boneIndexAttached)),
    ...
    END_RECV_TABLE()
        ...
        void C_ServerRagdollAttached::OnDataChanged(
